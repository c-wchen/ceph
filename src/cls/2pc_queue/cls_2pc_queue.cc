                              ...
                              bufferlist *out)
{
    ...
    CLS_LOG(20, "INFO: cls_2pc_queue_init: max size is %lu (bytes)",
            op.queue_size);
                                 ...
                                 bufferlist *out)
{
    ...
    if (res_op.size + urgent_data.reserved_size + overhead > remaining_size) {
        ...
        CLS_LOG(10, "INFO: cls_2pc_queue_reserve: remaining size: %lu (bytes)",
                remaining_size);
        CLS_LOG(10,
                "INFO: cls_2pc_queue_reserve: current reservations: %lu (bytes)",
                urgent_data.reserved_size);
        CLS_LOG(10, "INFO: cls_2pc_queue_reserve: requested size: %lu (bytes)",
                res_op.size);
        return -ENOSPC;
    }
    ...
    if (!result) {
        ...
        CLS_LOG(1,
                "ERROR: cls_2pc_queue_reserve: reservation id conflict after rollover: %u",
                urgent_data.last_id);
        return -EAGAIN;
    }
    ...
    if (head.max_urgent_data_size < urgent_data_length) {
        ...
        ret =
            cls_cxx_setxattr(hctx, CLS_QUEUE_URGENT_DATA_XATTR_NAME,
                             &bl_xattrs);
        if (ret < 0) {
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_reserve: failed to write xattrs with: %d",
                    ret);
            return ret;
        }
    ...
    CLS_LOG(20, "INFO: cls_2pc_queue_reserve: remaining size: %lu (bytes)",
            remaining_size);
    CLS_LOG(20,
            "INFO: cls_2pc_queue_reserve: current reservations: %lu (bytes)",
            urgent_data.reserved_size);
    CLS_LOG(20, "INFO: cls_2pc_queue_reserve: requested size: %lu (bytes)",
            res_op.size);
    CLS_LOG(20, "INFO: cls_2pc_queue_reserve: urgent data size: %lu (bytes)",
            urgent_data_length);
                                ...
                                bufferlist *out)
{
    ...
    if (it == urgent_data.reservations.end()) {
        if (!urgent_data.has_xattrs) {
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_commit: reservation does not exist: %u",
                    commit_op.id);
            return -ENOENT;
        }
        ...
        if (ret < 0) {
            ...
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_commit: failed to read xattrs with: %d",
                    ret);
            return ret;
        }
        auto iter = bl_xattrs.cbegin();
    ...
    if (xattr_reservations.empty()) {
        ...
        if (ret < 0) {
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_commit: failed to write xattrs with: %d",
                    ret);
            return ret;
        }
    }

    CLS_LOG(20, "INFO: cls_2pc_queue_commit: current reservations: %lu (bytes)",
            urgent_data.reserved_size);
                               ...
                               bufferlist *out)
{
    ...
    if (it == urgent_data.reservations.end()) {
        if (!urgent_data.has_xattrs) {
            CLS_LOG(20,
                    "INFO: cls_2pc_queue_abort: reservation does not exist: %u",
                    abort_op.id);
            return 0;
        }
        // try to look for the reservation in xattrs
        ...
        if (ret < 0) {
            ...
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_abort: failed to read xattrs with: %d",
                    ret);
            return ret;
        }
        auto iter = bl_xattrs.cbegin();
        ...
        if (ret < 0) {
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_abort: failed to write xattrs with: %d",
                    ret);
            return ret;
        }
    } else {
    ...
    CLS_LOG(20, "INFO: cls_2pc_queue_abort: current reservations: %lu (bytes)",
            urgent_data.reserved_size);
        ...
        bufferlist *in, bufferlist *out)
{
    ...
    if (urgent_data.has_xattrs) {
        ...
        if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_list_reservations: failed to read xattrs with: %d",
                    ret);
            return ret;
        }
        if (ret >= 0) {
        ...
        bufferlist *in, bufferlist *out)
{
    ...
    CLS_LOG(20,
            ...
            urgent_data.reservations.size());
    CLS_LOG(20,
            "INFO: cls_2pc_queue_expire_reservations: current reservations: %lu (bytes)",
            urgent_data.reserved_size);
    ...
    if (urgent_data.has_xattrs) {
        ...
        if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
            CLS_LOG(1,
                    "ERROR: cls_2pc_queue_expire_reservations: failed to read xattrs with: %d",
                    ret);
            return ret;
        }
        if (ret >= 0) {
            ...
            if (xattr_stale_found) {
                ...
                if (ret < 0) {
                    CLS_LOG(1,
                            "ERROR: cls_2pc_queue_expire_reservations: failed to write xattrs with: %d",
                            ret);
                    return ret;
                }
            }
...
CLS_INIT(2 pc_queue)
{
    ...
    cls_register_cxx_method(h_class, TPC_QUEUE_INIT,
                            CLS_METHOD_RD | CLS_METHOD_WR, cls_2pc_queue_init,
                            &h_2pc_queue_init);
    cls_register_cxx_method(h_class, TPC_QUEUE_GET_CAPACITY, CLS_METHOD_RD,
                            cls_2pc_queue_get_capacity,
                            &h_2pc_queue_get_capacity);
    cls_register_cxx_method(h_class, TPC_QUEUE_RESERVE,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_2pc_queue_reserve, &h_2pc_queue_reserve);
    cls_register_cxx_method(h_class, TPC_QUEUE_COMMIT,
                            CLS_METHOD_RD | CLS_METHOD_WR, cls_2pc_queue_commit,
                            &h_2pc_queue_commit);
    cls_register_cxx_method(h_class, TPC_QUEUE_ABORT,
                            CLS_METHOD_RD | CLS_METHOD_WR, cls_2pc_queue_abort,
                            &h_2pc_queue_abort);
    cls_register_cxx_method(h_class, TPC_QUEUE_LIST_RESERVATIONS, CLS_METHOD_RD,
                            cls_2pc_queue_list_reservations,
                            &h_2pc_queue_list_reservations);
    cls_register_cxx_method(h_class, TPC_QUEUE_LIST_ENTRIES, CLS_METHOD_RD,
                            cls_2pc_queue_list_entries,
                            &h_2pc_queue_list_entries);
    cls_register_cxx_method(h_class, TPC_QUEUE_REMOVE_ENTRIES,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_2pc_queue_remove_entries,
                            &h_2pc_queue_remove_entries);
    cls_register_cxx_method(h_class, TPC_QUEUE_EXPIRE_RESERVATIONS,
                            CLS_METHOD_RD | CLS_METHOD_WR,
