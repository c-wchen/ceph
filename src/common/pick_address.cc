                                              ...
                                              const std::string & interfaces)
{
    ...
    for (std::list < string >::iterator s = nets.begin(); s != nets.end(); ++s) {
        ...
        const struct ifaddrs *found = find_ip_in_subnet(filtered,
                                                        (struct sockaddr *)&net,
                                                        prefix_len);
        if (found) {
            r = found->ifa_addr;
...
static void fill_in_one_address(CephContext * cct,
                                ...
                                const string interfaces, const char *conf_var)
{
                       ...
                       const struct sockaddr_storage &network)
{
    ...
    const unsigned int prefix_len =
        max(sizeof(in_addr::s_addr), sizeof(in6_addr::s6_addr)) * CHAR_BIT;
                     ...
                     entity_addr_t * match)
{
    ...
    for (struct ifaddrs * addrs = ifa; addrs != NULL; addrs = addrs->ifa_next) {
        if (addrs->ifa_addr) {
            ...
            for (list < entity_addr_t >::const_iterator p = ls.begin();
                 p != ls.end(); ++p) {
                if (a.is_same_host(*p)) {
                    *match = *p;
                    found = true;
