...
void ResizeRequest::resize(ceph::BitVector < 2 > *object_map,
...
} void ResizeRequest::send()
{
    ...
    std::unique_lock l {
        *m_object_map_lock};
    m_num_objs =
        Striper::get_num_objects(m_image_ctx.layout, m_new_size);

    std::string oid(ObjectMap <>::
                    object_map_name(m_image_ctx.id, m_snap_id));
    ldout(cct,
          5) << this << " resizing on-disk object map: " << "ictx=" <<
             &m_image_ctx << ", " << "oid=" << oid << ", num_objs=" <<
             m_num_objs << dendl;
    ...
    cls_client::object_map_resize(&op, m_num_objs,
