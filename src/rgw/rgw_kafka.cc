...
struct connection_t {
    ...
    connection_t(CephContext *_cct, const std::string &_broker, bool _use_ssl, bool _verify_ssl,
        ...
        cct(_cct), broker(_broker), use_ssl(_use_ssl), verify_ssl(_verify_ssl), ca_location(_ca_location), user(_user),
        password(_password), mechanism(_mechanism) {}
...
bool new_producer(connection_t *conn)
{
    ...
    if (rd_kafka_conf_set(conn->temp_conf, "bootstrap.servers", conn->broker.c_str(), errstr,
    ...
    if (conn->use_ssl) {
        if (!conn->user.empty()) {
            ...
            if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SASL_SSL", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK ||
                ...
                rd_kafka_conf_set(conn->temp_conf, "sasl.password", conn->password.c_str(), errstr,
                                  sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                goto conf_error;
            }
            ...
            if (conn->mechanism) {
                if (rd_kafka_conf_set(conn->temp_conf, "sasl.mechanism", conn->mechanism->c_str(), errstr,
                                      sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                    goto conf_error;
                }
                ldout(conn->cct, 20) << "Kafka connect: successfully configured SASL mechanism" << dendl;
            } else {
                if (rd_kafka_conf_set(conn->temp_conf, "sasl.mechanism", "PLAIN", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                    goto conf_error;
                }
            ...
            // use only SSL
            if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SSL", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                goto conf_error;
            }
            ldout(conn->cct, 20) << "Kafka connect: successfully configured SSL security" << dendl;
        }
        if (conn->ca_location) {
            if (rd_kafka_conf_set(conn->temp_conf, "ssl.ca.location", conn->ca_location->c_str(), errstr,
                                  sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                goto conf_error;
            }
        ...
        // use SASL+PLAINTEXT
        if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SASL_PLAINTEXT", errstr,
                              sizeof(errstr)) != RD_KAFKA_CONF_OK ||
            rd_kafka_conf_set(conn->temp_conf, "sasl.username", conn->user.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK ||
            rd_kafka_conf_set(conn->temp_conf, "sasl.password", conn->password.c_str(), errstr,
                              sizeof(errstr)) != RD_KAFKA_CONF_OK) {
    ...
    // create the producer
    if (conn->producer) {
        ldout(conn->cct, 5) << "Kafka connect: producer already exists. detroying the existing before creating a new one" <<
                            dendl;
...
private:
    ...
    void publish_internal(message_wrapper_t *message)
    {
        ...
        if (!conn->is_ok()) {
            ...
            ldout(conn->cct, 1) << "Kafka publish: producer was closed while message was in the queue. error: " << status_to_string(
                                    conn->status) << dendl;
            if (message->cb) {
        ...
        if (topic_it == conn->topics.end()) {
            ...
            if (!topic) {
                ...
                ldout(conn->cct, 1) << "Kafka publish: failed to create topic: " << message->topic << " error: " << status_to_string(
                                        err) << dendl;
                if (message->cb) {
                    message->cb(err);
                }
        ...
        if (tag) {
            ...
            if (q_len < max_inflight) {
                ldout(conn->cct, 20) << "Kafka publish (with callback, tag=" << *tag << "): OK. Queue has: " << q_len << " callbacks" <<
                                     dendl;
...
public:
                 ...
                 boost::optional<const std::string &> mechanism)
    {
        ...
        auto conn = connections.emplace(broker, std::make_unique<connection_t>(cct, broker, use_ssl, verify_ssl, ca_location,
                                        user, password, mechanism)).first->second.get();
