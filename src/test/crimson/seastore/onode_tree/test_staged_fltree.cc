...
namespace {
    ...
    ghobject_t make_ghobj(shard_t shard, pool_t pool, crush_hash_t crush,
                          std::string ns, std::string oid, snap_t snap,
                          gen_t gen) {
        return ghobject_t {
            shard_id_t {
            shard}
        , pool, crush, ns, oid, snap, gen};
    }
...
TEST_F(a_basic_test_t, 1 _basic_sizes)
{
    logger().info("\n"
                  "Bytes of struct:\n"
                  "  node_header_t: {}\n"
                  ...
                  sizeof(shard_pool_crush_t), sizeof(crush_t),
                  sizeof(snap_gen_t), sizeof(slot_0_t), sizeof(slot_1_t),
                  sizeof(slot_3_t), sizeof(node_fields_0_t),
                  sizeof(node_fields_1_t), sizeof(node_fields_2_t),
                  sizeof(internal_fields_3_t), sizeof(leaf_fields_3_t),
...
#define NXT_T(StageType)  staged<typename StageType::next_param_t>
    ...
    logger().info("\n"
                  ...
                  NXT_T(_STAGE_T(InternalNode0))::insert_size(key_view,
                                                              i_value),
                  NXT_T(NXT_T(_STAGE_T(InternalNode0)))::insert_size(key_view,
                                                                     i_value),
                  _STAGE_T(InternalNode1)::insert_size(key_view, i_value),
                  NXT_T(_STAGE_T(InternalNode1))::insert_size(key_view,
                                                              i_value),
                  NXT_T(NXT_T(_STAGE_T(InternalNode1)))::insert_size(key_view,
                                                                     i_value),
                  _STAGE_T(InternalNode2)::insert_size(key_view, i_value),
                  NXT_T(_STAGE_T(InternalNode2))::insert_size(key_view,
                                                              i_value),
...
TEST_F(a_basic_test_t, 2 _node_sizes)
{
    run_async([] {
              auto nm = NodeExtentManager::create_dummy(IS_DUMMY_SYNC);
              auto t = make_test_transaction();
              ValueBuilderImpl < UnboundedValue > vb; context_t c {
              *nm, vb, *t};
              std::array < std::pair < NodeImplURef, NodeExtentMutable >,
              16 > nodes = {
              INTR_WITH_PARAM(InternalNode0::allocate, c, false,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode1::allocate, c, false,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode2::allocate, c, false,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode3::allocate, c, false,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode0::allocate, c, true,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode1::allocate, c, true,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode2::allocate, c, true,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(InternalNode3::allocate, c, true,
                              1u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode0::allocate, c, false,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode1::allocate, c, false,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode2::allocate, c, false,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode3::allocate, c, false,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode0::allocate, c, true,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode1::allocate, c, true,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode2::allocate, c, true,
                              0u).unsafe_get0().make_pair(),
              INTR_WITH_PARAM(LeafNode3::allocate, c, true,
                              0u).unsafe_get0().make_pair()
...
struct b_dummy_tree_test_t:public seastar_test_suite_t {
    ...
    seastar::future <> set_up_fut() override final {
        ...
        tree.
            reset(new
                  UnboundedBtree(NodeExtentManager::create_dummy(IS_DUMMY_SYNC))
            );
        return INTR(tree->mkfs,
                    *ref_t).handle_error(crimson::ct_error::all_same_way([] {
                                                                         ASSERT_FALSE
                                                                         ("Unable to mkfs");}
                                         )
            );
...
TEST_F(b_dummy_tree_test_t, 3 _random_insert_erase_leaf_node)
{
    run_async([this] {
              logger().info("\n---------------------------------------------"
              ...
              return cursor.value();};
              auto f_validate_erase =
              [this, &insert_history] (const ghobject_t & key) {
              auto cursor_erase = INTR_R(tree->find, *ref_t, key).unsafe_get0();
              auto cursor_next =
              INTR(cursor_erase.get_next, *ref_t).unsafe_get0();
              auto cursor_ret =
              INTR_R(tree->erase, *ref_t, cursor_erase).unsafe_get0();
              ceph_assert(cursor_erase.is_end());
              ceph_assert(cursor_ret == cursor_next);
              auto cursor_lb =
              INTR_R(tree->lower_bound, *ref_t, key).unsafe_get0();
              ...
              [&f_validate_insert_new,
               &f_validate_erase] (const ghobject_t & key,
                                   const test_item_t & value) {
              f_validate_insert_new(key, value); f_validate_erase(key);
              ...
              {
              auto cursor1_s =
              INTR_R(tree->lower_bound, *ref_t, key_s).unsafe_get0();
              ASSERT_EQ(cursor1_s.get_ghobj(), key1);
              ASSERT_EQ(cursor1_s.value(), test_value1);
              auto cursor1_e =
              INTR_R(tree->lower_bound, *ref_t, key_e).unsafe_get0();
              ...
              std::for_each(kvs.begin(), kvs.end(),
                            [&f_insert_erase_insert] (auto & kv) {
                            f_insert_erase_insert(kv.first, kv.second);}
      ); ASSERT_EQ(INTR(tree->height, *ref_t).unsafe_get0(), 1); ASSERT_FALSE(tree->test_is_clean()); for (auto &[k, val]:insert_history)
              {
              auto &[v, c] = val;
              ...
              {
          kvs.clear(); for (auto &[k, val]:insert_history) {
              auto &[v, c] = val; kvs.emplace_back(k, v);}
              insert_history.clear();
              std::sort(kvs.begin(), kvs.end(),[](auto & l, auto & r) {
                        return l.first < r.first;}
          ); auto cursor = INTR(tree->begin, *ref_t).unsafe_get0(); for (auto &[k, v]:kvs)
              {
              ...
              tree->dump(*ref_t, oss); logger().info("\n{}\n", oss.str());
              ...
              ).unsafe_get0(); ASSERT_EQ(e_size, 1);}
              auto cursor = INTR(tree->begin, *ref_t).unsafe_get0();
              ASSERT_TRUE(cursor.is_end());
...
static std::set < ghobject_t > build_key_set(std::pair < unsigned,
                                             unsigned >range_2,
                                             std::pair < unsigned,
                                             unsigned >range_1,
                                             std::pair < unsigned,
...
class TestTree {
  public:
    ...
    *moved_nm, vb, t}
    ...
    seastar::future <> build_tree(std::pair < unsigned, unsigned >range_2,
        ...
        return seastar::async([this, range_2, range_1, range_0, value_size] {
          ...
          auto keys = build_key_set(range_2, range_1, range_0); for (auto & key:keys)
                              {
    ...
    seastar::future <> build_tree(const std::vector < ghobject_t > &keys,
    ...
    seastar::future <> split_merge(const ghobject_t & key,
                                   const test_item_t & value,
        ...
        return seastar::async([this, key, value, expected, next_key] {
                              ...
                              value.get_payload_size()};
                              auto[cursor, success] =
                              INTR_R(tree_clone.insert, t_clone, key,
                                     conf).unsafe_get0();
                              initialize_cursor_from_item(t, key, value, cursor,
                                                          success); {
                              std::ostringstream oss;
                              tree_clone.dump(t_clone, oss);
                              logger().info("dump new root:\n{}", oss.str());}
          EXPECT_EQ(INTR(tree_clone.height, t_clone).unsafe_get0(), 2); for (auto &[k, val]:insert_history)
                              {
                              auto &[v, c] = val;
                              auto result =
                              ...
                              auto result =
                              INTR_R(tree_clone.find, t_clone,
                                     key).unsafe_get0();
                              EXPECT_NE(result, tree_clone.end());
                              ...
                              auto nxt_cursor =
                              with_trans_intr(t_clone,
                                              [&cursor = cursor] (auto & tr) {
                                              return cursor.erase < true > (tr);}
                              ).unsafe_get0(); {
                              // track root again to dump
                              auto begin =
                              INTR(tree_clone.begin, t_clone).unsafe_get0();
                              std::ignore = begin; std::ostringstream oss;
                              tree_clone.dump(t_clone, oss);
                              ...
                              validate_cursor_from_item(*next_key,
                                                        std::get < 0 >
                                                        (found->second),
                                                        nxt_cursor);}
                              else {
          ...
          for (auto &[k, val]:insert_history) {
                              ...
                              EXPECT_NE(result, tree_clone.end());
                              validate_cursor_from_item(k, v, result);}
                              EXPECT_EQ(INTR(tree_clone.height, t_clone).
                                        unsafe_get0(), 1);
  ...
  private:
    seastar::future <> insert_tree(const ghobject_t & key,
                                   const test_item_t & value) {
...
TEST_F(c_dummy_test_t, 4 _split_merge_leaf_node)
{
    run_async([] {
              {
              TestTree test; test.build_tree( {
                                             2, 5}, {
                                             2, 5}, {
                                             2, 5}, 120).get0();
              ...
              test.split_merge(make_ghobj(1, 1, 1, "ns3", "oid3", 3, 3), value,
                               ...
                               make_ghobj(2, 2, 2, "ns2", "oid2", 2,
                                          2)}).get0();
              EXPECT_TRUE(last_split.match_split_pos( {
                                                     0, {
                                                     0, {
                                                     0}}}));
              test.split_merge(make_ghobj(2, 2, 2, "ns1", "oid1", 3, 3), value,
              ...
              {
              TestTree test; std::vector < ghobject_t > keys = {
    ...
    class DummyChildPool {
        class DummyChildImpl final:public NodeImpl {
          public:
            ...
            DummyChildImpl(const std::set < ghobject_t > &keys,
                           bool is_level_tail, laddr_t laddr)
            :keys {
            keys}, _is_level_tail {
            is_level_tail}, _laddr {
            laddr} {
                std::tie(key_view, p_mem_key_view) =
                    build_key_view(*keys.crbegin());
                build_name();
            }
            ...
            const std::set < ghobject_t > &get_keys() const {
                return keys;
          ...
          public:
            laddr_t laddr()const override {
                return _laddr;
            } bool is_level_tail() const override {
                return _is_level_tail;
            } std::optional < key_view_t > get_pivot_index() const override {
                return {
                key_view};
            }
            bool is_extent_retired() const override {
          ...
          protected:
            node_type_t node_type()const override {
                return node_type_t::LEAF;
            } field_type_t field_type() const override {
            ...
            void validate_non_empty() const override {
                ...
                ceph_abort("impossible path");
            } std::tuple < match_stage_t,
                search_position_t > erase(const search_position_t &) override {
                ceph_abort("impossible path");
            }
            std::tuple < match_stage_t,
                std::size_t > evaluate_merge(NodeImpl &) override {
                ceph_abort("impossible path");
            }
            search_position_t merge(NodeExtentMutable &, NodeImpl &,
                                    match_stage_t, extent_len_t) override {
                ceph_abort("impossible path");
            }
        ...
        class DummyChild final:public Node {
          public:
            ...
            key_view_t get_pivot_key() const {
                return *impl->get_pivot_index();
            } eagain_ifuture <> populate_split(context_t c,
                                               std::set < Ref <
                                               DummyChild >> &splitable_nodes) {
                ceph_assert(can_split());
                ceph_assert(splitable_nodes.find(this) !=
                            splitable_nodes.end());
                ...
                auto right_child =
                    DummyChild::create_new(right_keys, right_is_tail, pool);
                if (!can_split()) {
                ...
                return apply_split_to_parent(c, std::move(this_ref),
                                             std::move(right_child), false);
            }

            eagain_ifuture <> insert_and_split(context_t c,
            ...
            eagain_ifuture <> merge(context_t c, Ref < DummyChild > &&this_ref) {
                return parent_info().ptr->get_child_peers(c,
                                                          parent_info().
                                                          position).si_then([c,
                                                                             this_ref
                                                                             =
            ...
            eagain_ifuture <> fix_key(context_t c, const ghobject_t & new_key) {
                ...
                impl->reset(new_keys, impl->is_level_tail());
                Ref < Node > this_ref = this;
                return fix_parent_index < true > (c, std::move(this_ref),
                                                  false);
            }
            ...
            } static Ref < DummyChild > create(const std::set < ghobject_t >
                                               &keys, bool is_level_tail,
                                               laddr_t addr,
                                               DummyChildPool & pool) {
                auto ref_impl =
                    std::make_unique < DummyChildImpl > (keys, is_level_tail,
                                                         addr);
                return new DummyChild(ref_impl.get(), std::move(ref_impl),
                                      pool);
            }

            static Ref < DummyChild > create_new(const std::set < ghobject_t >
                                                 &keys, bool is_level_tail,
                                                 DummyChildPool & pool) {
            ...
            static eagain_ifuture < Ref <
                DummyChild >> create_initial(context_t c,
                                             const std::set < ghobject_t >
                                             &keys, DummyChildPool & pool,
                                             RootNodeTracker & root_tracker) {
                auto initial = create_new(keys, true, pool);
                return c.nm.get_super(c.t,
                                      root_tracker).
          ...
          protected:
            eagain_ifuture <> test_clone_non_root(context_t,
                                                  Ref < InternalNode >
                                                  new_parent) const override {
                ceph_assert(!is_root());
                auto p_pool_clone = pool.pool_clone_in_progress;
                ceph_assert(p_pool_clone != nullptr);
                auto clone =
                    create(impl->get_keys(), impl->is_level_tail(),
                           impl->laddr(), *p_pool_clone);
                clone->as_child(parent_info().position, new_parent);
                return eagain_iertr::now();
            } eagain_ifuture < Ref <
                tree_cursor_t >> lookup_smallest(context_t) override {
                ceph_abort("impossible path");
            }
            eagain_ifuture < Ref <
                tree_cursor_t >> lookup_largest(context_t) override {
                ceph_abort("impossible path");
            }
            eagain_ifuture <> test_clone_root(context_t,
                                              RootNodeTracker &) const override
            {
                ceph_abort("impossible path");
            } eagain_ifuture < search_result_t > lower_bound_tracked(context_t,
                                                                     const
                                                                     key_hobj_t
                                                                     &,
                                                                     MatchHistory
            ...
            eagain_ifuture <> do_get_tree_stats(context_t,
                                                tree_stats_t &) override {
                ceph_abort("impossible path");
            }
            bool is_tracking() const override {
                return false;
            } void track_merge(Ref < Node >, match_stage_t,
                               search_position_t &) override {
                ceph_abort("impossible path");
            }

          private:
            DummyChild(DummyChildImpl * impl, DummyChildImpl::URef
                       && ref, DummyChildPool & pool)
          :    Node(std::move(ref)), impl {
            impl}, pool {
            pool} {
            ...
            bool can_split() const {
            ...
            } static eagain_ifuture <> do_merge(context_t c,
                                                Ref < DummyChild > &&left,
                ...
                return left->parent_info().ptr->apply_children_merge < true >
                    (c, std::move(left), left_addr, std::move(right),
                     !stole_key);
            }
      ...
      public:
        ...
        auto build_tree(const std::set < ghobject_t > &keys) {
            ...
            return with_trans_intr(get_context().t,[this, &keys] (auto & tr) {
                                   return DummyChild::
                                   create_initial(get_context(), keys, *this,
                                                  *p_btree->root_tracker).
        ...
        seastar::future <> split_merge(ghobject_t key, search_position_t pos,
            ...
            return seastar::async([this, key, pos, expected] {
                                  ...
                                  info("\n\nINSERT-SPLIT {} at pos({}):",
                                       key_hobj_t(key), pos);
                                  auto node_to_split =
                                  pool_clone.get_node_by_pos(pos);
                                  with_trans_intr(pool_clone.get_context().t,
                                                  [&](auto & t) {
                                                  return node_to_split->
                                                  insert_and_split(pool_clone.
                                                                   get_context
                                                                   (), key,
                                                                   pool_clone.
                                                                   splitable_nodes);}
                                  ...
                                  EXPECT_EQ(INTR
                                            (pool_clone.p_btree->height,
                                             pt).unsafe_get0(), 3);
                                  EXPECT_TRUE(last_split.match(expected));
                                  EXPECT_EQ(pool_clone.p_dummy->size(), 3);
                                  // erase and merge
                                  [[maybe_unused]] auto pivot_key =
                                  node_to_split->get_pivot_key();
                                  logger().info("\n\nERASE-MERGE {}:",
                                                node_to_split->get_name());
                                  ...
                                  with_trans_intr(pool_clone.get_context().t,
                                                  ...
                                                  merge(pool_clone.
                                                        ...
                                                        std::
                                                        move(node_to_split));}
                                  ).unsafe_get0(); auto & pt2 = pool_clone.t();
                                  EXPECT_EQ(INTR
                                            (pool_clone.p_btree->height,
                                             pt2).unsafe_get0(), 2);
        ...
        seastar::future <> fix_index(ghobject_t new_key, search_position_t pos,
                                     bool expect_split) {
            return seastar::async([this, new_key, pos, expect_split] {
                                  ...
                                  pool_clone.get_node_by_pos(pos);
                                  auto old_key =
                                  node_to_fix->get_pivot_key().to_ghobj();
                                  logger().
                                  info
                                  ("\n\nFIX pos({}) from {} to {}, expect_split={}:",
                                   pos, node_to_fix->get_name(),
                                   key_hobj_t(new_key), expect_split);
                                  with_trans_intr(pool_clone.get_context().t,
                                                  [&](auto & t) {
                                                  return node_to_fix->
                                                  fix_key(pool_clone.
                                  ...
                                  with_trans_intr(pool_clone.get_context().t,
                                                  ...
                                                  fix_key(pool_clone.
                                                          get_context(),
                                                          old_key);}
                                  ).unsafe_get0(); auto & pt = pool_clone.t();
                                  EXPECT_EQ(INTR
                                            (pool_clone.p_btree->height,
                                             pt).unsafe_get0(), 2);
                                  EXPECT_EQ(pool_clone.p_dummy->size(), 1);}
      ...
      private:
        ...
        Ref < DummyChild > get_node_by_pos(const search_position_t & pos) const {
            auto iter =
                std::find_if(tracked_children.begin(), tracked_children.end(),
                             [&pos] (auto & child) {
...
TEST_F(c_dummy_test_t, 5 _split_merge_internal_node)
{
    run_async([] {
              DummyChildPool pool; {
              logger().info("\n---------------------------------------------"
                            "\nbefore internal node insert:\n");
              auto padding = std::string(250, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              6}
                                                                              , {
                                                                              2,
              ...
              info("\n---------------------------------------------"
                   "\nsplit at stage 2; insert to right front at stage 0, 1, 2, 1, 0\n");
              pool.
              split_merge(make_ghobj(3, 3, 3, "ns4", "oid4" + padding, 5, 5), {
                          2, {
                          0, {
                          0}
                          }
              ...
              pool.split_merge(make_ghobj(3, 3, 3, "ns5", "oid5", 3, 3), {
                               2, {
                               0, {
                               0}
                               }
              ...
              pool.split_merge(make_ghobj(3, 4, 4, "ns3", "oid3", 3, 3), {
                               2, {
                               0, {
                               0}
                               }
              ...
              pool.split_merge(make_ghobj(4, 4, 4, "ns1", "oid1", 3, 3), {
                               2, {
                               0, {
                               0}
                               }
              ...
              pool.
              split_merge(make_ghobj(4, 4, 4, "ns2", "oid2" + padding, 1, 1), {
                          2, {
                          0, {
                          0}
                          }
              ...
              pool.
              split_merge(make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5), {
                          3, {
                          0, {
                          0}
                          }
              ...
              pool.split_merge(make_ghobj(4, 4, 4, "ns5", "oid5", 3, 3), {
                               3, {
                               0, {
                               0}
                               }
              ...
              pool.split_merge(make_ghobj(4, 4, 5, "ns3", "oid3", 3, 3), {
                               3, {
                               0, {
                               0}
                               }
              ...
              pool.split_merge(make_ghobj(5, 5, 5, "ns1", "oid1", 3, 3), {
                               3, {
                               0, {
                               0}
                               }
              ...
              pool.
              split_merge(make_ghobj(5, 5, 5, "ns2", "oid2" + padding, 1, 1), {
                          3, {
                          0, {
                          0}
                          }
              ...
              logger().info("\n---------------------------------------------"
                            "\nsplit at stage 2; insert to right back at stage 0, 1, 2\n");
              pool.
              split_merge(make_ghobj(5, 5, 5, "ns4", "oid4" + padding_e, 5, 5),
              ...
              pool.split_merge(make_ghobj(1, 1, 1, "ns3", "oid3", 3, 3), {
                               0, {
                               0, {
                               0}
                               }
              ...
              pool.split_merge(make_ghobj(2, 2, 2, "ns1", "oid1", 3, 3), {
                               0, {
                               0, {
                               0}
                               }
              ...
              pool.
              split_merge(make_ghobj(2, 2, 2, "ns2", "oid2" + padding_s, 1, 1),
                          {
                          0, {
                          0, {
                          0}
              ...
              pool.
              split_merge(make_ghobj(2, 2, 2, "ns4", "oid4" + padding, 5, 5), {
                          1, {
                          0, {
                          0}
                          }
              ...
              pool.split_merge(make_ghobj(2, 2, 2, "ns5", "oid5", 3, 3), {
                               1, {
                               0, {
                               0}
                               }
              ...
              pool.
              split_merge(make_ghobj
                          (2, 2, 3, "ns3", "oid3" + std::string(80, '_'), 3, 3),
                          {
                          1, {
                          0, {
                          0}
              ...
              pool.split_merge(make_ghobj(3, 3, 3, "ns1", "oid1", 3, 3), {
                               1, {
                               0, {
                               0}
                               }
              ...
              pool.
              split_merge(make_ghobj(3, 3, 3, "ns2", "oid2" + padding, 1, 1), {
                          1, {
                          0, {
                          0}
                          }
              ...
              logger().info("\n---------------------------------------------"
                            "\nsplit at stage 0; insert to left back at stage 0\n");
              pool.
              split_merge(make_ghobj(3, 3, 3, "ns4", "oid4" + padding, 3, 4), {
                          1, {
                          2, {
                          2}
                          }
              ...
              auto padding = std::string(244, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              6}
                                                                              , {
                                                                              2,
              ...
              pool.split_merge(make_ghobj(3, 3, 3, "ns5", "oid5", 3, 3), {
                               ...
                               , {
                               2u, 1u, true, InsertType::LAST}
              ).get(); pool.split_merge(make_ghobj(3, 4, 4, "n", "o", 3, 3), {
                                        2, {
                                        0, {
                                        0}
                                        }
                                        ...
                                        , {
                                        2u, 2u, true, InsertType::LAST}
              ).get(); pool.split_merge(make_ghobj(4, 4, 4, "n", "o", 3, 3), {
                                        2, {
                                        0, {
                                        0}
                                        }
              ...
              pool.split_merge(make_ghobj(2, 3, 3, "n", "o", 3, 3), {
                               1, {
                               0, {
                               0}
                               }
              ...
              auto padding = std::string(243, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              6}
                                                                              , {
                                                                              2,
              ...
              info("\n---------------------------------------------"
                   "\nsplit at stage 2; insert to left back at stage (0, 1, 2, 1,) 0\n");
              pool.split_merge(make_ghobj(4, 4, 4, "n", "o", 2, 2), {
                               2, {
                               0, {
                               0}
                               }
              ...
              auto padding = std::string(419, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              5}
                                                                              , {
                                                                              2,
              ...
              info("\n---------------------------------------------"
                   "\nsplit at stage 1; insert to right front at stage 0, 1, 0\n");
              pool.
              split_merge(make_ghobj(3, 3, 3, "ns2", "oid2" + padding, 5, 5), {
                          1, {
                          1, {
                          0}
                          }
              ...
              pool.split_merge(make_ghobj(3, 3, 3, "ns2", "oid3", 3, 3), {
                               1, {
                               1, {
                               0}
                               }
              ...
              pool.
              split_merge(make_ghobj(3, 3, 3, "ns3", "oid3" + padding, 1, 1), {
                          1, {
                          1, {
                          0}
                          }
              ...
              auto padding = std::string(361, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              5}
                                                                              , {
                                                                              2,
              ...
              pool.fix_index(make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5),
                             ...
                             , false).get();
              pool.
              fix_index(make_ghobj(4, 4, 4, "ns5", "oid5" + padding1, 3, 3), {
                        2, {
                        2, {
                        2}
                        }
                        ...
                        , true).get();
              pool.
              fix_index(make_ghobj(5, 5, 5, "ns3", "oid3" + padding1, 3, 3), {
                        2, {
                        2, {
                        2}
                        }
              ...
              auto padding = std::string(412, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              5}
                                                                              , {
                                                                              2,
              ...
              info("\n---------------------------------------------"
                   "\nsplit at stage 1; insert to left back at stage (0, 1,) 0\n");
              pool.split_merge(make_ghobj(3, 3, 3, "ns2", "oid3", 2, 2), {
                               1, {
                               1, {
                               0}
                               }
              ...
              auto padding = std::string(328, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              5}
                                                                              , {
                                                                              2,
              ...
              keys.
              insert(make_ghobj
                     (5, 5, 5, "ns3", "oid3" + std::string(270, '_'), 3, 3));
              keys.insert(make_ghobj(9, 9, 9, "ns~last", "oid~last", 9, 9));
              pool.build_tree(keys).unsafe_get0();
              logger().
              info("\n---------------------------------------------"
                   "\nsplit at stage 0; insert to right front at stage 0\n");
              pool.
              split_merge(make_ghobj(3, 3, 3, "ns3", "oid3" + padding, 2, 3), {
                          1, {
                          1, {
                          1}
                          }
              ...
              fix_index(make_ghobj(4, 4, 4, "ns5", "oid5" + padding1, 3, 3), {
                        ...
                        , true).get();
              pool.
              fix_index(make_ghobj(5, 5, 5, "ns4", "oid4" + padding1, 3, 3), {
                        3, {
                        0, {
                        0}
                        }
              ...
              auto padding = std::string(323, '_'); auto keys = build_key_set( {
                                                                              2,
                                                                              5}
                                                                              , {
                                                                              2,
              ...
              pool.fix_index(make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5),
                             ...
                             , false).get();
              pool.
              fix_index(make_ghobj(4, 4, 4, "ns6", "oid6" + padding1, 3, 3), {
                        2, {
                        3, {
                        0}
                        }
...
TEST_F(d_seastore_tm_test_t, 6 _random_tree_insert_erase)
{
    run_async([this] {
              ...
              auto tree =
              std::make_unique < TreeBuilder < TRACK_CURSORS,
              BoundedValue >> (kvs, std::move(moved_nm)); {
              ...
              restart(); tree->reload(NodeExtentManager::create_seastore(*tm));}
              {
...
TEST_F(d_seastore_tm_test_t, 7 _tree_insert_erase_eagain)
{
    run_async([this] {
              ...
              auto kvs = KVPool < test_item_t >::create_raw_range( {
                                                                  ...
                                                                  0, 4});
              auto moved_nm =
              NodeExtentManager::create_seastore(*tm, L_ADDR_MIN,
                                                 EAGAIN_PROBABILITY);
              auto p_nm =
              static_cast < SeastoreNodeExtentManager < true >
              *>(moved_nm.get());
              auto tree =
              std::make_unique < TreeBuilder < TRACK_CURSORS,
              ExtendedValue >> (kvs, std::move(moved_nm)); unsigned num_ops = 0;
              unsigned num_ops_eagain = 0;
              // bootstrap
              ++num_ops; repeat_eagain([this, &tree, &num_ops_eagain] {
                                       ++num_ops_eagain;
                                       return seastar::
                                       do_with(create_mutate_transaction(),
                                               [this, &tree] (auto & t) {
                                               return INTR(tree->bootstrap,
                                                           *t).safe_then([this,
                                                                          &t] {
                                                                         return
                                                                         submit_transaction_fut
                                                                         (*t);}
                                               );}
                                       );}
              ).unsafe_get0(); epm->run_background_work_until_halt().get0();
              // insert
              logger().warn("start inserting {} kvs ...", kvs.size()); {
              auto iter = kvs.random_begin(); while (iter != kvs.random_end()) {
              ++num_ops; repeat_eagain([this, &tree, &num_ops_eagain, &iter] {
                                       ++num_ops_eagain;
                                       return seastar::
                                       do_with(create_mutate_transaction(),
                                               [this, &tree, &iter] (auto & t) {
                                               return INTR_R(tree->insert_one,
                                                             *t,
                                                             iter).
                                               safe_then([this,
                                                          &t] (auto cursor) {
              ...
              epm->run_background_work_until_halt().get0(); ++iter;}
              }
              ...
              auto iter = kvs.begin(); while (iter != kvs.end()) {
              ++num_ops; repeat_eagain([this, &tree, &num_ops_eagain, &iter] {
                                       ++num_ops_eagain;
                                       auto t = create_read_transaction();
                                       return INTR_R(tree->validate_one, *t,
                                                     iter).safe_then([t =
                                                                      std::
                                                                      move(t)] {
                                                                     }
              ...
              eagain_rate /= num_ops;
              logger().info("eagain rate: {}", eagain_rate); tree.reset();}) ;
}
