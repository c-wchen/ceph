...
class ESInfixQueryParser {
  ...
  public:
     ESInfixQueryParser(const string & _query):query(_query),
...
class ESQueryCompiler {
  ...
  public:
     ESQueryCompiler(const string & query, list < pair < string,
                     string > >*prepend_eq_conds,
                     const string & _custom_prefix):parser(query),
    ...
    const string & get_custom_prefix() {
