...
void Locker::include_snap_rdlocks_wlayout(set < SimpleLock * >&rdlocks,
                                          CInode * in, file_layout_t ** layout)
{
    //rdlock ancestor snaps
    CInode *t = in;
    rdlocks.insert(&in->snaplock);
...
struct MarkEventOnDestruct {
    ...
    MarkEventOnDestruct(MDRequestRef & _mdr,
        ...
        message(_message), mark_event(true) {
    } ~MarkEventOnDestruct() {
        if (mark_event)
...
bool Locker::acquire_locks(MDRequestRef & mdr,
                           ...
                           CInode * auth_pin_freeze, bool auth_pin_nonblock)
{
    if (mdr->done_locking && !mdr->is_slave()) {    // not on slaves!  master requests locks piecemeal.
    ...
    // xlocks
    for (set < SimpleLock * >::iterator p = xlocks.begin(); p != xlocks.end();
         ++p) {
        SimpleLock *lock = *p;
        ...
        if ((lock->get_type() == CEPH_LOCK_ISNAP ||
            ...
            mdr->is_master() && !mdr->is_queued_for_replay()) {
    ...
    // wrlocks
    for (set < SimpleLock * >::iterator p = wrlocks.begin(); p != wrlocks.end();
         ++p) {
        ...
        else if (!object->is_auth() && !(*p)->can_wrlock(client) && // we might have to request a scatter
    ...
    // remote_wrlocks
    if (remote_wrlocks) {
        for (map < SimpleLock *, mds_rank_t >::iterator p =
             remote_wrlocks->begin(); p != remote_wrlocks->end(); ++p) {
    ...
    for (set < SimpleLock * >::iterator p = rdlocks.begin();
        ...
        if (object->is_auth())
            mustpin.insert(object);
        else if (!object->is_auth() && !(*p)->can_rdlock(client)) { // we might have to request an rdlock
            dout(15) << " will also auth_pin " << *object
    ...
    if (!mustpin_remote.empty()) {
        ...
        for (map < MDSCacheObject *, mds_rank_t >::iterator p =
             mdr->remote_auth_pins.begin(); p != mdr->remote_auth_pins.end();
             ++p) {
            if (mustpin.count(p->first)) {
                assert(p->second == p->first->authority().first);
                map < mds_rank_t, set < MDSCacheObject * > >::iterator q =
                    mustpin_remote.find(p->second);
                if (q != mustpin_remote.end())
                    q->second.insert(p->first);
            }
        }
        for (map < mds_rank_t, set < MDSCacheObject * > >::iterator p =
             mustpin_remote.begin(); p != mustpin_remote.end(); ++p) {
    ...
    set < SimpleLock *, SimpleLock::ptr_lt >::iterator existing =
        mdr->locks.begin();
    for (set < SimpleLock *, SimpleLock::ptr_lt >::iterator p = sorted.begin();
         p != sorted.end(); ++p) {
        bool need_wrlock = ! !wrlocks.count(*p);
        bool need_remote_wrlock = ! !(remote_wrlocks
                                      && remote_wrlocks->count(*p));
        ...
        if (existing != mdr->locks.end() && *existing == *p) {
            ...
            if (xlocks.count(have) && mdr->xlocks.count(have)) {
                dout(10) << " already xlocked " << *have << " " << *have->
                    get_parent() << dendl;
                continue;
            }
            if (mdr->remote_wrlocks.count(have)) {
                if (!need_remote_wrlock ||
                    ...
                    dout(10) << " unlocking remote_wrlock on wrong mds." <<
                        mdr->remote_wrlocks[have]
                        << " " << *have << " " << *have->get_parent() << dendl;
                    remote_wrlock_finish(have, mdr->remote_wrlocks[have],
                                         mdr.get());
                }
            }
            if (need_wrlock || need_remote_wrlock) {
                if (need_wrlock == ! !mdr->wrlocks.count(have) &&
                    ...
                    if (need_wrlock)
                        dout(10) << " already wrlocked " << *have << " " <<
                            *have->get_parent() << dendl;
                    if (need_remote_wrlock)
                        dout(10) << " already remote_wrlocked " << *have << " "
                            << *have->get_parent() << dendl;
                    continue;
                }
            }
            if (rdlocks.count(have) && mdr->rdlocks.count(have)) {
                dout(10) << " already rdlocked " << *have << " " << *have->
                    get_parent() << dendl;
                continue;
            }
        }
        ...
        while (existing != mdr->locks.end()) {
            ...
            ++existing;
            dout(10) << " unlocking out-of-order " << *stray << " " << *stray->
                get_parent() << dendl;
            bool need_issue = false;
            if (mdr->xlocks.count(stray)) {
                ...
                if (mdr->remote_wrlocks.count(stray))
                    remote_wrlock_finish(stray, mdr->remote_wrlocks[stray],
                                         mdr.get());
            }
            if (need_issue)
        ...
        if (xlocks.count(*p)) {
            ...
            if (need_wrlock && !mdr->wrlocks.count(*p)) {
                ...
                if (need_remote_wrlock && !(*p)->can_wrlock(mdr->get_client())) {
                    ...
                    remote_wrlock_finish(*p, mdr->remote_wrlocks[*p],
                                         mdr.get());
                    remote_wrlock_start(*p, (*remote_wrlocks)[*p], mdr);
                    goto out;
                }
    ...
    while (existing != mdr->locks.end()) {
        SimpleLock *stray = *existing;
        ++existing;
        dout(10) << " unlocking extra " << *stray << " " << *stray->
            get_parent() << dendl;
        bool need_issue = false;
        if (mdr->xlocks.count(stray)) {
            xlock_finish(stray, mdr.get(), &need_issue);
...
void Locker::_drop_non_rdlocks(MutationImpl * mut, set < CInode * >*pneed_issue)
{
    ...
    while (!mut->remote_wrlocks.empty()) {
        map < SimpleLock *, mds_rank_t >::iterator p =
            mut->remote_wrlocks.begin();
    ...
    for (set < mds_rank_t >::iterator p = slaves.begin(); p != slaves.end();
         ++p) {
        if (!mds->is_cluster_degraded()
...
void Locker::eval_gather(SimpleLock * lock, bool first, bool * pneed_issue,
                         list < MDSInternalContextBase * >*pfinishers)
{
    ...
    if (!lock->is_gathering() && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_rdlock, auth) || !lock->is_rdlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_wrlock, auth) || !lock->is_wrlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_xlock, auth) || !lock->is_xlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_lease, auth) || !lock->is_leased()) && !(lock->get_parent()->is_auth() && lock->is_flushing()) &&  // i.e. wait for scatter_writebehind!
        ...
        if (!lock->get_parent()->is_auth()) {
            ...
            if (!mds->is_cluster_degraded() ||
                ...
                case LOCK_SYNC_LOCK:
                    mds->
                        send_message_mds(new
                                         MLock(lock, LOCK_AC_LOCKACK,
                                               mds->get_nodeid()), auth);
                    break;
                ...
                case LOCK_MIX_SYNC:
                    {
                        MLock *reply =
                            new MLock(lock, LOCK_AC_SYNCACK, mds->get_nodeid());
                ...
                case LOCK_SYNC_MIX:
                    {
                        MLock *reply =
                            new MLock(lock, LOCK_AC_MIXACK, mds->get_nodeid());
                        mds->send_message_mds(reply, auth);
                        next = LOCK_SYNC_MIX2;
                    }
                    break;
                        ...
                        MDSInternalContextBase * con)
{
    ...
    // wait!
    if (con) {
        dout(7) << "rdlock_try waiting on " << *lock << " on " << *lock->
            get_parent() << dendl;
...
bool Locker::can_rdlock_set(set < SimpleLock * >&locks)
{
    ...
    for (set < SimpleLock * >::iterator p = locks.begin(); p != locks.end();
         ++p)
        if (!(*p)->can_rdlock(-1)) {
...
void Locker::wrlock_finish(SimpleLock * lock, MutationImpl * mut,
                           bool * pneed_issue)
{
    if (lock->get_type() == CEPH_LOCK_IVERSION ||
        lock->get_type() == CEPH_LOCK_DVERSION)
...
bool Locker::xlock_start(SimpleLock * lock, MDRequestRef & mut)
{
    ...
    if (lock->get_parent()->is_auth()) {
        // auth
        while (1) {
            if (lock->can_xlock(client) && !(lock->get_state() == LOCK_LOCK_XLOCK &&    // client is not xlocker or
...
class C_Locker_FileUpdate_finish:public LockerLogContext {
  ...
  public:
    C_Locker_FileUpdate_finish(Locker * l, CInode * i, MutationRef & m,
                               bool sm = false, bool ni = false, client_t c =
                               -1, MClientCaps * ac = 0)
...
void Locker::file_update_finish(CInode * in, MutationRef & mut, bool share_max,
                                bool issue_client_cap, client_t client,
                                MClientCaps * ack)
{
    ...
    if (!in->is_head() && !in->client_snap_caps.empty()) {
        ...
        while (p != in->client_snap_caps.end()) {
            ...
            assert(lock);
            dout(10) << " completing client_snap_caps for " << ccap_string(p->
                                                                           first)
    ...
    for (; it != in->client_caps.end(); ++it) {
        ...
        if (!(pending & ~allowed)) {
            ...
            if (cap->is_new() || cap->is_suppress()) {
                dout(20) << "  !revoke and new|suppressed, skipping client." <<
                    it->first << dendl;
                continue;
            }
        }
        ...
        if (((wanted & allowed) & ~pending) ||  // missing wanted+allowed caps
            ...
            if (cap->is_new()) {
                ...
                dout(7) << "   sending MClientCaps to client." << it->first
                    ...
                    << " new pending " << ccap_string(after) << " was " <<
                    ccap_string(before)
                    << dendl;

                int op =
                    (before & ~after) ? CEPH_CAP_OP_REVOKE : CEPH_CAP_OP_GRANT;
...
void Locker::request_inode_file_caps(CInode * in)
{
    ...
    if (wanted != in->replica_caps_wanted) {
        ...
        if (!mds->is_cluster_degraded() ||
            ...
            mds->
                send_message_mds(new
                                 MInodeFileCaps(in->ino(),
                                                in->replica_caps_wanted), auth);
    }
}
                                    ...
                                    new_ranges, bool * max_increased)
{
    ...
    for (map < client_t, Capability * >::iterator p = in->client_caps.begin();
         p != in->client_caps.end(); ++p) {
        if ((p->second->issued() | p->second->
             wanted()) & (CEPH_CAP_ANY_FILE_WR)) {
            client_writeable_range_t & nr = (*new_ranges)[p->first];
            nr.range.first = 0;
            if (latest->client_ranges.count(p->first)) {
                client_writeable_range_t & oldr =
                    latest->client_ranges[p->first];
                                  ...
                                  utime_t new_mtime)
{
    ...
    if (can_update < 0) {
        ...
        if (can_update == -1) {
            dout(10) << "check_inode_max_size frozen, waiting on " << *in <<
                dendl;
    ...
    if (update_max) {
        dout(10) << "check_inode_max_size client_ranges " << pi.inode.
    ...
    if (update_size) {          // FIXME if/when we do max_size nested accounting
        mdcache->predirty_journal_parents(mut, metablob, in, 0,
                                          PREDIRTY_PRIMARY);
        // no cow, here!
...
bool Locker::_need_flush_mdlog(CInode * in, int wanted)
{
    ...
    if (((wanted &
          (CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR | CEPH_CAP_FILE_SHARED |
           CEPH_CAP_FILE_EXCL)) && in->filelock.is_unstable_and_locked())
...
void Locker::adjust_cap_wanted(Capability * cap, int wanted, int issue_seq)
{
    ...
    if (cap->wanted() == 0) {
        if (cur->item_open_file.is_on_list() && !cur->is_any_caps_wanted()) {
            dout(10) << " removing unwanted file from open file list " << *cur
                << dendl;
...
void Locker::handle_client_caps(MClientCaps * m)
{
    ...
    if (!mds->is_clientreplay() && !mds->is_active() && !mds->is_stopping()) {
        ...
        if (mds->is_reconnect() &&
            ...
            mdcache->set_reconnected_dirty_caps(client, m->get_ino(),
                                                m->get_dirty());
        }
        mds->wait_for_replay(new C_MDS_RetryMessage(mds, m));
        return;
    ...
    if (m->get_client_tid() > 0 && session &&
        ...
        if (m->get_op() == CEPH_CAP_OP_FLUSHSNAP) {
            ack =
                new MClientCaps(CEPH_CAP_OP_FLUSHSNAP_ACK, m->get_ino(), 0, 0,
                                0, 0, 0, m->get_dirty(), 0,
                                mds->get_osd_epoch_barrier());
        }
        else {
            ack =
                new MClientCaps(CEPH_CAP_OP_FLUSH_ACK, m->get_ino(), 0,
                                m->get_cap_id(), m->get_seq(), m->get_caps(), 0,
                                m->get_dirty(), 0,
                                mds->get_osd_epoch_barrier());
        }
    ...
    if (op == CEPH_CAP_OP_FLUSHSNAP) {
        ...
        if (m->get_dirty()) {
            ack =
                new MClientCaps(CEPH_CAP_OP_FLUSHSNAP_ACK, in->ino(), 0, 0, 0,
                                0, 0, m->get_dirty(), 0,
                                mds->get_osd_epoch_barrier());
            ack->set_snap_follows(follows);
            ack->set_client_tid(m->get_client_tid());
    ...
    if (cap->get_cap_id() != m->get_cap_id()) {
        ...
        if (m->get_dirty() && in->is_auth()) {
            dout(7) << " flush client." << client << " dirty " <<
                ccap_string(m->get_dirty())
                << " seq " << m->get_seq() << " on " << *in << dendl;
            ack =
                new MClientCaps(CEPH_CAP_OP_FLUSH_ACK, in->ino(), 0,
                                cap->get_cap_id(), m->get_seq(), m->get_caps(),
                                0, m->get_dirty(), 0,
                                mds->get_osd_epoch_barrier());
            ack->set_client_tid(m->get_client_tid());
            ack->set_oldest_flush_tid(m->get_oldest_flush_tid());
                                         ...
                                         boost::string_view dname)
{
    ...
    if (dname.length()) {
        ...
        if (dir) {
            ...
            if (dn) {
                ...
                if (l) {
                    dout(10) << "process_cap_release removing lease on " << *dn
                        << dendl;
    ...
    dout(10) << "process_cap_release client." << client << " " <<
        ccap_string(caps) << " on " << *in << (mdr ? "" : " (DEFERRED, no mdr)")
        << dendl;
...
void Locker::_do_snap_update(CInode * in, snapid_t snap, int dirty,
                             snapid_t follows, client_t client, MClientCaps * m,
                             MClientCaps * ack)
{
                            ...
                            bool * need_flush)
{
    ...
    uint64_t old_max =
        latest->client_ranges.count(client) ? latest->client_ranges[client].
        range.last : 0;
...
void Locker::handle_client_cap_release(MClientCapRelease * m)
{
    ...
    for (vector < ceph_mds_cap_item >::iterator p = m->caps.begin();
         p != m->caps.end(); ++p) {
        _do_cap_release(client, inodeno_t((uint64_t) p->ino), p->cap_id,
                        p->migrate_seq, p->seq);
    }
...
void Locker::remove_client_cap(CInode * in, Capability * cap)
{
    ...
    if (in->is_auth()) {
        ...
        if (in->get_projected_inode()->client_ranges.count(client) && !(in->inode.nlink == 0 && !in->is_any_caps()))    // unless it's unlink + stray
            check_inode_max_size(in);
    }
    else {
        request_inode_file_caps(in);
    ...
    for (xlist < Capability * >::iterator p = revoking_caps.begin(); !p.end();
        ...
        if (age >
            ...
            dout(20) << __func__ << " silencing log message (backoff) for " <<
                cap->get_client() << "." << cap->get_inode()->ino() << dendl;
        }
    }
}
                                ...
                                bufferlist & bl, utime_t now, Session * session)
{
    ...
    if (!diri->is_stray() &&    // do not issue dn leases in stray dir!
        ...
        ::encode(e, bl);
        dout(20) << "issue_client_lease seq " << e.seq << " dur " << e.
...
void Locker::revoke_client_leases(SimpleLock * lock)
{
    ...
    for (map < client_t, ClientLease * >::iterator p =
        ...
        CInode *diri = dn->get_dir()->get_inode();
        mds->
            send_message_client_counted(new
                                        MClientLease(CEPH_MDS_LEASE_REVOKE,
                                                     l->seq, mask, diri->ino(),
...
{
    // nobody should be talking to us during recovery.
    assert(mds->is_rejoin() || mds->is_clientreplay() || mds->is_active()
           || mds->is_stopping());
        ...
        if (lock->is_stable()) {
            ...
            lock->
                add_waiter(SimpleLock::WAIT_STABLE | MDSCacheObject::
                           WAIT_UNFREEZE, new C_MDS_RetryMessage(mds,
                                                                 m->get()));
        }
    }
    else {
...
{
    CInode *in = static_cast < CInode * >(lock->get_parent());
    dout(10) << "scatter_writebehind " << in->inode.
    ...
    in->finish_scatter_gather_update_accounted(lock->get_type(), mut,
                                               &le->metablob);
 ...
 * inode.dirstat.
 */
void Locker::scatter_nudge(ScatterLock * lock, MDSInternalContextBase * c,
                           bool forcelockchange)
{
    ...
    if (p->is_auth()) {
        ...
        if (!mds->is_cluster_degraded() ||
            ...
            mds->
                send_message_mds(new
                                 MLock(lock, LOCK_AC_NUDGE, mds->get_nodeid()),
                                 auth);
...
void Locker::file_eval(ScatterLock * lock, bool * need_issue)
{
    ...
    if (lock->get_state() == LOCK_EXCL) {
        ...
        dout(7) << "file_eval loner_issued=" << gcap_string(loner_issued)
            ...
            << dendl;
        if (!((loner_wanted | loner_issued) & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GBUFFER)) || (other_wanted & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GRD)) || (in->inode.is_dir() && in->multiple_nonstale_caps())) { // FIXME.. :/
...
void Locker::file_excl(ScatterLock * lock, bool * need_issue)
{
    ...
    assert((in->get_loner() >= 0 && in->mds_caps_wanted.empty()) || (lock->get_state() == LOCK_XSYN));  // must do xsyn -> excl -> <anything else>
