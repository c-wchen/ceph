...
void MDSMap::mds_info_t::print_summary(ostream & out) const const
{
    out << global_id << ":\t"
        << addr
        << " '" << name << "'"
        << " mds." << rank << "." << inc << " " << ceph_mds_state_name(state)
        << " seq " << state_seq;
    if (laggy()) {
        out << " laggy since " << laggy_since;
    }
    if (standby_for_rank != -1 || !standby_for_name.empty()) {
        out << " (standby for";
                        ...
                        string > >*detail) const const
{
    ...
    if (is_degraded()) {
        ...
        if (detail) {
            ...
            for (mds_rank_t i = mds_rank_t(0); i < get_max_mds(); i++) {
                ...
                map < mds_gid_t, mds_info_t >::const_iterator info =
                    mds_info.find(gid);
                stringstream ss;
                if (is_resolve(i))
                    ss << "mds." << info->second.name << " at " << info->second.
                        addr << " rank " << i << " is resolving";
                if (is_replay(i))
                    ss << "mds." << info->second.name << " at " << info->second.
                        addr << " rank " << i << " is replaying journal";
                if (is_rejoin(i))
                    ss << "mds." << info->second.name << " at " << info->second.
                        addr << " rank " << i << " is rejoining";
                if (is_reconnect(i))
                    ss << "mds." << info->second.name << " at " << info->second.
                        addr << " rank " << i << " is reconnecting to clients";
                if (ss.str().length())
    ...
    set < string > laggy;
  for (const auto & u:up) {
        map < mds_gid_t, mds_info_t >::const_iterator m =
            mds_info.find(u.second);
        if (m == m_end) {
            std::cerr << "Up rank " << u.first << " GID " << u.
                second << " not found!" << std::endl;
        }
        assert(m != m_end);
        ...
        if (mds_info.laggy()) {
            ...
            if (detail) {
                std::ostringstream oss;
                oss << "mds." << mds_info.name << " at " << mds_info.
                    addr << " is laggy/unresponsive";
    ...
    if (!laggy.empty()) {
        std::ostringstream oss;
        oss << "mds " << laggy << ((laggy.size() > 1) ? " are" : " is")
            << " laggy";
    ...
    if (ev >= 6) {
        if (ev < 10) {
            ...
            ever_allowed_features |=
                CEPH_MDSMAP_ALLOW_MULTIMDS | CEPH_MDSMAP_ALLOW_DIRFRAGS;
            ::decode(flag, p);
