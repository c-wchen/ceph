...
void RGWOp_Bucket_Link::execute(optional_yield y)
{
    ...
    RESTArgs::get_string(s, "new-bucket-name", new_bucket_name,
                         &new_bucket_name);
...
void RGWOp_Bucket_Remove::execute(optional_yield y)
{
    ...
    op_ret =
        driver->get_bucket(s, s->user.get(), string(), bucket_name, &bucket, y);
    if (op_ret < 0) {
...
void RGWOp_Set_Bucket_Quota::execute(optional_yield y)
{
    ...
    if (use_http_params) {
        std::unique_ptr < rgw::sal::Bucket > bucket;
        op_ret =
            driver->get_bucket(s, nullptr, uid.tenant, bucket_name, &bucket,
                               s->yield);
        if (op_ret < 0) {
            return;
        }
