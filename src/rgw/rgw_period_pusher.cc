...
public:
    PushAndRetryCR(CephContext *cct, const std::string &zone,
                   RGWRESTConn *conn, RGWHTTPManager *http,
                   RGWPeriod &period)
        : RGWCoroutine(cct), zone(zone), conn(conn), http(http), period(period),
          ...
          timeout_max(cct->_conf->rgw_period_push_interval_max), counter(0)
    {
...
int PushAndRetryCR::operate(const DoutPrefixProvider *dpp)
{
    reenter(this) {
        for (;;) {
            ...
            if (get_ret_status() == 0) {
                ldpp_dout(dpp,
                          10) << "push to " << zone << " succeeded" << dendl;
                return set_cr_done();
            }
            ...
            yield {
                ...
                ldpp_dout(dpp,
                          10) << "waiting " << dur << "s for retry.." << dendl;
                wait(dur);
...
public:
    PushAllCR(CephContext *cct, RGWHTTPManager *http, RGWPeriod && period,
        ...
        : RGWCoroutine(cct), http(http),
          period(std::move(period)), conns(std::move(conns))
    {
    } int operate(const DoutPrefixProvider *dpp) override;
};
...
int PushAllCR::operate(const DoutPrefixProvider *dpp)
{
    reenter(this) {
        ...
        yield {
            ldpp_dout(dpp,
                      4) << "sending " << conns.size() << " periods" << dendl;
            for (auto &c : conns)
                spawn(new PushAndRetryCR(cct, c.first, &c.second, http, period),
                      false);
        }
...
RGWPeriodPusher::RGWPeriodPusher(const DoutPrefixProvider *dpp,
...
void RGWPeriodPusher::handle_notify(RGWZonesNeedPeriod && period)
{
    ...
    if (period.get_realm_epoch() == realm_epoch &&
        ...
        ldout(cct,
              10) << "period epoch " << period.
                  get_epoch() << " is not newer " "than current epoch " <<
    ...
    if (period.get_map().master_zonegroup ==
        ...
        for (auto &zg : zonegroups) {
            ...
            hint = conns.emplace_hint(hint, std::piecewise_construct,
                                      std::forward_as_tuple(zonegroup.get_id()),
                                      std::forward_as_tuple(cct, driver,
                                          zonegroup.get_id(),
                                          zonegroup.endpoints,
                                          zonegroup.
                                          api_name));
    ...
    for (auto &z : my_zonegroup.zones) {
        ...
        hint = conns.emplace_hint(hint, std::piecewise_construct,
                                  ...
                                  std::forward_as_tuple(cct, driver, zone.id,
                                      zone.endpoints,
                                      my_zonegroup.api_name));
    }
