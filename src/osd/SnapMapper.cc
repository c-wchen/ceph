...
{
    ObjectMap::ObjectMapIterator iter = os->get_omap_iterator(cid, hoid);
    if (!iter) {
        ceph_abort();
        return -EINVAL;
    }
...
pair < string, bufferlist > SnapMapper::to_raw(const pair < snapid_t,
                                               hobject_t > &in)
{
    bufferlist bl;
...
void SnapMapper::object_snaps::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(oid, bl);
    ::encode(snaps, bl);
