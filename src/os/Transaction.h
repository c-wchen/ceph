...
namespace ceph::os {
    class Transaction {
      ...
      Transaction(Transaction && other) noexcept:
        ...
        bool has_contexts() const {
            return
                !on_commit.empty() ||
                !on_applied.empty() || !on_applied_sync.empty();
        ...
        void _update_op_bl(ceph::buffer::list & bl,
                           std::vector < uint32_t > &cm,
                           std::vector < uint32_t > &om) {
        ...
        void append(Transaction & other) {
            ...
            ceph::buffer::list other_op_bl;
            {
                ...
                other.op_bl.begin().copy(other.op_bl.length(),
                                         other_op_bl_ptr.c_str());
  ...
  /** Inquires about the Transaction as a whole. */
        ...
        uint64_t get_encoded_bytes() {
            ...
            final_size +=
                (coll_index.size() + object_index.size()) * sizeof(__u32);
   ...
   */
        class iterator {
          ...
          public:
            ...
            void decode_attrset(std::map < std::string,
                                ceph::buffer::ptr > &aset) {
                using ceph::decode;
                decode(aset, data_bl_p);
            }
            void decode_attrset(std::map < std::string,
                                ceph::buffer::list > &aset) {
                using ceph::decode;
                decode(aset, data_bl_p);
            }
   ...
   */
        ...
        uint32_t _get_object_id(const ghobject_t & oid) {
            std::map < ghobject_t, uint32_t >::iterator o =
                object_index.find(oid);
   ...
   */
        ...
        void setattr(const coll_t & cid, const ghobject_t & oid,
                     const char *name, ceph::buffer::list & val) {
            std::string n(name);
            setattr(cid, oid, n, val);
        }
        /// Set an xattr of an object
        void setattr(const coll_t & cid, const ghobject_t & oid,
                     const std::string & s, ceph::buffer::list & val) {
            using ceph::encode;
            Op *_op = _get_next_op();
            _op->op = OP_SETATTR;
            _op->cid = _get_coll_id(cid);
        ...
        void setattrs(const coll_t & cid,
                      ...
                      const std::map < std::string, ceph::buffer::ptr,
                      std::less <>> &attrset) {
            using ceph::encode;
            Op *_op = _get_next_op();
            _op->op = OP_SETATTRS;
   ...
   */
        ...
        void split_collection(const coll_t & cid,
                              uint32_t bits,
                              uint32_t rem, const coll_t & destination) {
            Op *_op = _get_next_op();
        ...
        void set_alloc_hint(const coll_t & cid,
                            const ghobject_t & oid,
