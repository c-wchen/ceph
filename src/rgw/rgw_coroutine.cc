    ...
    for (vector<RGWCoroutinesStack *>::iterator iter = s->entries.begin(); iter != s->entries.end(); ++iter) {
        ...
        if (stack == skip_stack || !stack->is_done()) {
            ...
            if (!stack->is_done()) {
                ...
                ldout(cct, 20) << "collect(): s=" << (void *)this << " stack=" << (void *)stack << " explicitly skipping stack" <<
                               dendl;
            }
            continue;
        }
        ...
        if (r < 0) {
            *ret = r;
            ldout(cct, 20) << "collect(): s=" << (void *)this << " stack=" << (void *)stack << " encountered error (r=" << r <<
...
RGWAioCompletionNotifier::RGWAioCompletionNotifier(RGWCompletionManager *_mgr, const rgw_io_id &_io_id,
        void *_user_data) : completion_mgr(_mgr),
    io_id(_io_id),
...
int RGWCoroutinesManager::run(const DoutPrefixProvider *dpp, list<RGWCoroutinesStack *> &stacks)
{
    ...
    for (list<RGWCoroutinesStack *>::iterator iter = scheduled_stacks.begin(); iter != scheduled_stacks.end() &&
         !going_down;) {
        ...
        if (stack->is_io_blocked()) {
             ...
             */
            ldout(cct, 20) << __func__ << ":" << " stack=" << (void *)stack << " is_blocked_by_stack()=" <<
