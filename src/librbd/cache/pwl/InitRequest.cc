...
InitRequest < I > *InitRequest < I >::create(I &image_ctx,
        ...
        on_finish)
{
    return new InitRequest(image_ctx, image_writeback, plugin_api,
                           on_finish);
} template < typename I > InitRequest <
I >::InitRequest(I &image_ctx,
                 cache::
...
void InitRequest < I >::get_image_cache_state()
{
    ...
    auto cache_state =
        ImageCacheState <
        I >::create_image_cache_state(&m_image_ctx, m_plugin_api,
                                      r);
    ...
    if (r < 0 || !cache_state) {
        ...
        delete cache_state;
        cache_state = nullptr;
        lderr(cct) << "failed to get image cache state: " <<
                   cpp_strerror(r)
                   << dendl;
        save_result(-ENOENT);
        finish();
    ...
    switch (mode) {
        ...
        case cache::IMAGE_CACHE_TYPE_RWL:
            m_image_cache =
                new librbd::cache::pwl::rwl::WriteLog < I >
            (m_image_ctx, cache_state, m_image_writeback,
             m_plugin_api);
            break;
#endif
#ifdef WITH_RBD_SSD_CACHE
        case cache::IMAGE_CACHE_TYPE_SSD:
            m_image_cache =
                new librbd::cache::pwl::ssd::WriteLog < I >
            (m_image_ctx, cache_state, m_image_writeback,
             m_plugin_api);
            break;
#endif
...
void InitRequest < I >::handle_init_image_cache(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to init image cache: " <<
                   cpp_strerror(r)
                   << dendl;
...
template < typename I > void InitRequest < I >::set_feature_bit()
{
    ...
    Context *ctx =
        ...
        &klass::handle_set_feature_bit > (this);
    ctx->complete(r);
}
...
void InitRequest < I >::handle_set_feature_bit(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to set feature bit: " <<
                   cpp_strerror(r)
                   << dendl;
        save_result(r);
    ...
    auto image_dispatch =
        new cache::WriteLogImageDispatch < I > (&m_image_ctx,
            m_image_cache,
            m_plugin_api);
...
void InitRequest < I >::handle_shutdown_image_cache(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to close image cache: " <<
                   cpp_strerror(r)
                   << dendl;
    }
    delete m_image_cache;
