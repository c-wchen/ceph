...
#define dout_prefix *_dout << "librbd::cache::pwl::LogMap: " << this << " " \
            ...
            } template < typename T > LogMap < T >::LogMap(CephContext * cct)
            :m_cct(cct),
                m_lock(ceph::
                       make_mutex(pwl::
                                  unique_lock_name
                                  ("librbd::cache::pwl::LogMap::m_lock",
                                   this))) {
            }
 ...
 */
            template < typename T >
                void LogMap < T >::add_log_entry(std::shared_ptr < T >
                                                 log_entry) {
            ...
            template < typename T >
                void LogMap < T >::add_log_entries(std::list < std::shared_ptr <
                                                   T >> &log_entries) {
                std::lock_guard locker(m_lock);
 ...
 */
            template < typename T >
                void LogMap < T >::remove_log_entry(std::shared_ptr < T >
                                                    log_entry) {
                std::lock_guard locker(m_lock);
            ...
            template < typename T >
                void LogMap < T >::remove_log_entries(std::list <
                                                      std::shared_ptr <
                                                      T >> &log_entries) {
                std::lock_guard locker(m_lock);
                ldout(m_cct, 20) << dendl;
 ...
 */
            template < typename T >
                std::list < std::shared_ptr < T >> LogMap <
                T >::find_log_entries(BlockExtent block_extent) {
                std::lock_guard locker(m_lock);
                ldout(m_cct, 20) << dendl;
 ...
 */
            template < typename T >
                LogMapEntries < T > LogMap <
                T >::find_map_entries(BlockExtent block_extent) {
                std::lock_guard locker(m_lock);
                ldout(m_cct, 20) << dendl;
            ...
            template < typename T >
                void LogMap < T >::add_log_entry_locked(std::shared_ptr < T >
                                                        log_entry) {
              ...
              for (auto & entry:overlap_entries) {
                    ...
                    if (map_entry.block_extent.block_start <=
                        entry.block_extent.block_start) {
                        if (map_entry.block_extent.block_end >=
                            entry.block_extent.block_end) {
                            ldout(m_cct,
                                  20) <<
                                "map entry completely occluded by new log entry"
                                << dendl;
                            remove_map_entry_locked(entry);
                        }
                        else {
                            ceph_assert(map_entry.block_extent.block_end <
                                        entry.block_extent.block_end);
                            /* The new entry occludes the beginning of the old entry */
                            BlockExtent adjusted_extent(map_entry.block_extent.
                                                        block_end,
                                                        entry.block_extent.
                                                        block_end);
                        ...
                        if (map_entry.block_extent.block_end >=
                            ...
                            BlockExtent adjusted_extent(entry.block_extent.
                                                        block_start,
                                                        map_entry.block_extent.
                                                        block_start);
                            adjust_map_entry_locked(entry, adjusted_extent);
                        }
                        else {
                            /* The new entry splits the old entry */
                            split_map_entry_locked(entry,
                                                   map_entry.block_extent);
                        }
                    }
                }
            ...
            template < typename T >
                void LogMap < T >::remove_log_entry_locked(std::shared_ptr < T >
                                                           log_entry) {
            ...
            template < typename T >
                void LogMap < T >::add_map_entry_locked(LogMapEntry < T >
                                                        &map_entry) {
            ...
            template < typename T >
                void LogMap < T >::remove_map_entry_locked(LogMapEntry < T >
                                                           &map_entry) {
            ...
            template < typename T >
                void LogMap < T >::adjust_map_entry_locked(LogMapEntry < T >
                                                           &map_entry,
                                                           BlockExtent &
                                                           new_extent) {
                auto it = m_block_to_log_entry_map.find(map_entry);
                ...
                m_block_to_log_entry_map.insert(LogMapEntry < T >
                                                (new_extent,
                                                 adjusted.log_entry));
            }

            template < typename T >
                void LogMap < T >::split_map_entry_locked(LogMapEntry < T >
                                                          &map_entry,
                                                          BlockExtent &
                                                          removed_extent) {
                auto it = m_block_to_log_entry_map.find(map_entry);
            ...
            template < typename T >
                std::list < std::shared_ptr < T >> LogMap <
                T >::find_log_entries_locked(const BlockExtent & block_extent) {
                std::list < std::shared_ptr < T >> overlaps;
                ldout(m_cct, 20) << "block_extent=" << block_extent << dendl;

                ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
                LogMapEntries < T > map_entries =
 ...
 */
            template < typename T >
                LogMapEntries < T > LogMap <
                T >::find_map_entries_locked(const BlockExtent & block_extent) {
                LogMapEntries < T > overlaps;

                ldout(m_cct, 20) << "block_extent=" << block_extent << dendl;
                ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
                auto p =
                    m_block_to_log_entry_map.equal_range(LogMapEntry < T >
                                                         (block_extent));
                ldout(m_cct, 20) << "count=" << std::distance(p.first,
                                                              p.
                                                              second) << dendl;
 ...
 */
            template < typename T >
                bool LogMap <
                T >::LogMapEntryCompare::operator()(const LogMapEntry < T >
                                                    &lhs,
                                                    const LogMapEntry < T >
                                                    &rhs)const {
