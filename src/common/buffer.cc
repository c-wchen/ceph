...
static std::atomic < uint64_t > buffer_total_alloc {
0};
static std::atomic < uint64_t > buffer_history_alloc_bytes {
0};
static std::atomic < uint64_t > buffer_history_alloc_num {
0};
...
0};
static std::atomic < unsigned >buffer_missed_crc {
0};
...
const char *buffer::error::what() const const throw()
{
    return "buffer::exception";
}

const char *buffer::bad_alloc::what() const const throw()
{
    return "buffer::bad_alloc";
}

const char *buffer::end_of_buffer::what() const const throw()
{
    return "buffer::end_of_buffer";
}

const char *buffer::malformed_input::what() const const throw()
{
    return buf;
}
...
class buffer::raw {
  ...
  :    data(c), len(l), nref(0), mempool(mempool) {
    ...
    virtual ~ raw() {
        mempool::get_pool(mempool::pool_index_t(mempool)).adjust_count(-1,
                                                                       -(int)
                                                                       len);
    }
    ...
    bool get_crc(const pair < size_t, size_t > &fromto,
        ...
        map < pair < size_t, size_t >, pair < uint32_t,
            uint32_t > >::const_iterator i = crc_map.find(fromto);
        if (i == crc_map.end()) {
...
class buffer::raw_combined:public buffer::raw {
  ...
  public:
    ...
    :raw(dataptr, l, mempool), alignment(align) {
        inc_total_alloc(len);
...
class buffer::raw_mmap_pages:public buffer::raw {
  public:
    ...
    explicit raw_mmap_pages(unsigned l):raw(l) {
        data =
            (char *)::mmap(NULL, len, PROT_READ | PROT_WRITE,
  ...
  public:
    ...
    raw_posix_aligned(unsigned l, unsigned _align):raw(l) {
        ...
        bdout << "raw_posix_aligned " << this << " alloc " << (void *)data <<
            " l=" << l << ", align=" << align << " total_alloc=" << buffer::
            get_total_alloc() << bendl;
...
class buffer::raw_pipe:public buffer::raw {
  ...
  private:
    ...
    char *copy_pipe(int *fds) {
        ...
        if (r < (ssize_t) len) {
            bdout << "raw_pipe: error reading from temp pipe:" <<
                cpp_strerror(r)
                << bendl;
            free(data);
            data = NULL;
...
class buffer::xio_msg_buffer:public buffer::raw {
  ...
  public:
    xio_msg_buffer(XioDispatchHook * _m_hook, const char *d,
                   unsigned l):raw((char *)d, l), m_hook(_m_hook->get()) {
    } bool is_shareable() {
        return false;
    }
...
class buffer::xio_mempool:public buffer::raw {
  public:
    ...
    xio_mempool(struct xio_reg_mem *_mp, unsigned l):raw((char *)_mp->addr, l),
        mp(_mp) {
    } ~xio_mempool() {
    }
    raw *clone_empty() {
...
buffer::raw * buffer::create_msg(unsigned len, char *buf,
                                 XioDispatchHook * m_hook)
{
...
buffer::raw * buffer::create_aligned_in_mempool(unsigned len, unsigned align,
                                                int mempool)
{
...
buffer::ptr::ptr(raw * r):_raw(r), _off(0), _len(r->len)
...
bool buffer::ptr::at_buffer_tail() const const
{
...
bool buffer::ptr::can_zero_copy() const const
{
    return _raw->can_zero_copy();
}

int buffer::ptr::zero_copy_to_fd(int fd, int64_t * offset) const const
{
...
_len(other._len), _memcopy_count(other._memcopy_count), last_p(this)
{
...
bool buffer::list::can_zero_copy() const const
{
...
bool buffer::list::is_provided_buffer(const char *dst) constconst
{
    if (_buffers.empty())
        return false;
...
bool buffer::list::is_aligned(unsigned align) constconst
{
    for (std::list < ptr >::const_iterator it = _buffers.begin();
         it != _buffers.end(); ++it)
...
bool buffer::list::is_n_align_sized(unsigned align) constconst
{
    for (std::list < ptr >::const_iterator it = _buffers.begin();
         it != _buffers.end(); ++it)
...
bool buffer::list::is_aligned_size_and_memory(unsigned align_size,
                                              unsigned align_memory) constconst
{
    for (std::list < ptr >::const_iterator it = _buffers.begin();
         it != _buffers.end(); ++it) {
...
void buffer::list::copy(unsigned off, unsigned len, char *dest) const const
{
    if (off + len > length())
        throw end_of_buffer();
...
void buffer::list::copy(unsigned off, unsigned len, list & dest) const const
{
    if (off + len > length())
        throw end_of_buffer();
    if (last_p.get_off() != off)
...
void buffer::list::copy(unsigned off, unsigned len, std::string & dest) const const
{
    if (last_p.get_off() != off)
        last_p.seek(off);
...
int buffer::list::write_file(const char *fn, int mode)
{
    int fd =
        TEMP_FAILURE_RETRY(::
                           open(fn, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC,
                                mode));
    if (fd < 0) {
        int err = errno;
...
int buffer::list::write_fd_zero_copy(int fd) const const
{
    if (!can_zero_copy())
        return -ENOTSUP;
...
__u32 buffer::list::crc32c(__u32 crc) const const
{
    for (std::list < ptr >::const_iterator it = _buffers.begin();
        ...
        if (it->length()) {
            raw *r = it->get_raw();
            pair < size_t, size_t > ofs(it->offset(),
                                        it->offset() + it->length());
...
{
    return out << "buffer::raw(" << (void *)r.data << " len " << r.
        len << " nref " << r.nref.load() << ")";
}
