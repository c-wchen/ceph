    ...
    : RefCountedObject(image_ctx.cct),
      ...
      m_lock(ceph::
             make_shared_mutex(util::
                               unique_lock_name("librbd::ObjectMap::lock",
                                       this))),
...
bool ObjectMap < I >::object_may_not_exist(uint64_t object_no) const
{
    ...
    uint8_t state = (*this)[object_no];
    bool nonexistent = (state != OBJECT_EXISTS
                        && state != OBJECT_EXISTS_CLEAN);
    ldout(m_image_ctx.cct,
          20) << "object_no=" << object_no << " r=" << nonexistent << dendl;
    return nonexistent;
}
...
template < typename I > void ObjectMap < I >::open(Context *on_finish)
{
    ...
    auto req =
        object_map::RefreshRequest < I >::create(m_image_ctx, &m_lock,
            &m_object_map, m_snap_id,
            ctx);
    req->send();
}
                                   ...
                                   Context *on_finish)
{
    ...
    object_map::SnapshotCreateRequest *req =
        new object_map::SnapshotCreateRequest(m_image_ctx, &m_lock,
            &m_object_map, snap_id, ctx);
    req->send();
}
                                      ...
                                      Context *on_finish)
{
    ...
    object_map::SnapshotRemoveRequest *req =
        new object_map::SnapshotRemoveRequest(m_image_ctx, &m_lock,
            &m_object_map, snap_id, ctx);
    req->send();
}
                                 ...
                                 Context *on_finish)
{
    ...
    object_map::ResizeRequest *req =
        new object_map::ResizeRequest(m_image_ctx, &m_lock, &m_object_map,
                                      m_snap_id, new_size,
                                 ...
                                 Context *on_finish)
{
    ...
    auto req =
        object_map::UpdateRequest < I >::create(m_image_ctx, &m_lock,
            &m_object_map, snap_id,
            start_object_no,
