...
static ostream & _prefix(std::ostream * _dout, Monitor & mon, Paxos & paxos,
                         string service_name, version_t fc, version_t lc)
{
...
bool PaxosService::dispatch(MonOpRequestRef op)
{
    ...
    if (m->rx_election_epoch && m->rx_election_epoch < mon.get_epoch()) {
        dout(10) <<
            " discarding forwarded message from previous election epoch " << m->
            rx_election_epoch << " < " << mon.get_epoch() << dendl;
        return true;
    }
    ...
    if (!is_readable(m->version)) {
        dout(10) << " waiting for paxos -> readable (v" << m->
            version << ")" << dendl;
    ...
    /**
       ...
       */
        auto do_propose = new C_MonContext { &mon,[this] (int r){
                                                                 ...
                                                                 ceph_abort_msg
                                                                 ("bad return value for proposal_timer");}
                                                                 }
                                                                 };
                                                                 dout(10) <<
                                                                 " setting proposal_timer "
                                                                 << do_propose
                                                                 <<
                                                                 " with delay of "
                                                                 << delay <<
   ...
   */
                                                                 ...
                                                                 // apply to paxos
                                                                 proposing = true; need_immediate_propose = false;  /* reset whenever we propose */
  /**
  ...
  class C_Committed:                                            public Context
      ...
      explicit C_Committed(PaxosService * p):                   ps(p)
                                                                 ...
                                                                 ceph_abort_msg
                                                                 ("bad return value for C_Committed");}
                                                                 };
                                                                 paxos.
                                                                 queue_pending_finisher
                                                                 (new
                                                                  C_Committed
                                                                  (this));
                                                                 paxos.
     ...
     */
      class C_Active:                                           public Context
          ...
          explicit C_Active(PaxosService * s):                  svc(s)
                                                                 ...
                                                                 dout(7) <<
                                                                 __func__ <<
                                                                 " we are not the leader, hence we propose nothing!"
                                                                 << dendl;}
                                                                 ...
                                                                 " would only trim "
                                                                 << to_remove <<
                                                                 " > paxos_service_trim_max, limiting to "
                                                                 << trim_max <<
                                                                 dendl;
                                                                 return
                                                                 trim_max;}
