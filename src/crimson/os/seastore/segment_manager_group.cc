...
SegmentManagerGroup::read_segment_tail_ret SegmentManagerGroup::read_segment_tail(segment_id_t segment)
{
    ...
    return segment_manager.read(paddr_t::make_seg_paddr(segment,
                                segment_manager.
                                get_segment_size() -
    ...
    &segment_manager](bufferptr bptr)->read_segment_tail_ret {
        LOG_PREFIX(SegmentManagerGroup::read_segment_tail);
        DEBUG("segment {} bptr size {}", segment, bptr.length());
        segment_tail_t tail; bufferlist bl; bl.push_back(bptr);
        DEBUG("segment {} block crc {}", segment,
              bl.begin().crc32c(segment_manager.get_block_size(),
                                0)); auto bp = bl.cbegin();
        try {
            decode(tail, bp);
        ...
        return read_segment_tail_ret(read_segment_tail_ertr::
        ready_future_marker {
        }
        , tail);}
                                                        );
...
SegmentManagerGroup::read_segment_header_ret SegmentManagerGroup::read_segment_header(segment_id_t segment)
{
    assert(has_device(segment.device_id()));
    auto &segment_manager = *segment_managers[segment.device_id()];
    return segment_manager.read(paddr_t::make_seg_paddr(segment, 0),
    ...
    , crimson::ct_error::assert_all {
        "Invalid error in SegmentManagerGroup::read_segment_header"}
                                                     ...
                                                     &segment_manager](bufferptr bptr)->
    read_segment_header_ret {
        LOG_PREFIX(SegmentManagerGroup::read_segment_header);
        DEBUG("segment {} bptr size {}", segment, bptr.length());
        segment_header_t header; bufferlist bl; bl.push_back(bptr);
        DEBUG("segment {} block crc {}", segment,
              bl.begin().crc32c(segment_manager.get_block_size(),
                                0)); auto bp = bl.cbegin();
        try {
        ...
        return read_segment_header_ret(read_segment_header_ertr::
        ready_future_marker {
        }
        , header);}
                                                        );
...
SegmentManagerGroup::
scan_valid_records(scan_valid_records_cursor &cursor,
                   ...
                   found_record_handler_t &handler)
{
    ...
    seastar::stop_iteration > {
        return[ =, &handler, &cursor, &budget_used, this] {
            if (!cursor.last_valid_header_found) {
                ...
                safe_then([ =, &cursor](auto md) {
                    if (!md) {
                        ...
                        DEBUG("found valid {} at {}", header,
                              cursor.seq);
                        cursor.emplace_record_group(header,
                                                    std::
                                                    move
                                                    (md_bl));
                        return scan_valid_records_ertr::now();
                ...
                this] {
                    DEBUG
                    ("processing committed record groups until {}, {} pending",
                     cursor.last_committed,
                     cursor.pending_record_groups.size());
                    return crimson::
                    repeat([ =, &budget_used, &cursor,
                    &handler, this] {
                        if (cursor.
                            ...
                            return scan_valid_records_ertr::
                            make_ready_future <
                            seastar::stop_iteration >
                            (seastar::stop_iteration::yes);
                        }
                        auto &next =
                        cursor.pending_record_groups.
                        front();
            ...
            } else
            {
                ...
                auto &next =
                    cursor.pending_record_groups.front();
                return read_validate_data(next.offset,
                                          next.header).
                       safe_then([this, FNAME, &budget_used, &cursor,
                &handler, &next](auto valid) {
                    if (!valid) {
                        INFO
                    ...
                    return consume_next_records(cursor,
                                                handler,
                                                budget_used);
                }
                                );
    ...
    std::move(retref)]()mutable->scan_valid_records_ret {
        return scan_valid_records_ret(scan_valid_records_ertr::
        ready_future_marker {
        }
        , std::move(*retref));}
              );
...
SegmentManagerGroup::read_validate_record_metadata_ret SegmentManagerGroup::read_validate_record_metadata(paddr_t start,
        segment_nonce_t
        nonce)
{
    ...
    auto segment_size =
        static_cast < int64_t >(segment_manager.get_segment_size());
    if (seg_addr.get_segment_off() + block_size > segment_size) {
        DEBUG
        ("failed -- record group header block {}~4096 > segment_size {}",
         start, segment_size);
        return
    ...
    return segment_manager.read(start,
                                block_size).safe_then([ =,
                                        &segment_manager]
                                        (bufferptr bptr)
                                        mutable->
    read_validate_record_metadata_ret {
        ...
        if (!maybe_header.
            ...
            return
            read_validate_record_metadata_ret
            (read_validate_record_metadata_ertr::
            ready_future_marker {
            }
            , std::nullopt);
        }
        ...
        {
            ERROR
            ("failed, invalid record group header {}",
             start);
            return crimson::
                   ct_error::
                   input_output_error::
        ...
        auto rest_len =
            ...
            block_size;
        TRACE
        ("reading record group header rest {}~{}",
         rest_start,
         rest_len);
        return
            segment_manager.
...
SegmentManagerGroup::read_validate_data_ret SegmentManagerGroup::read_validate_data(paddr_t record_base,
        ...
        header)
{
    ...
    TRACE("reading record group data blocks {}~{}", data_addr,
          header.dlength);
    return segment_manager.read(data_addr,
                                header.dlength).safe_then([ =,
                                        &header](auto
    bptr) {
        bufferlist bl;
        bl.append(bptr);
...
SegmentManagerGroup::
consume_next_records(scan_valid_records_cursor &cursor,
                     found_record_handler_t &handler,
                     std::size_t &budget_used)
{
    ...
    next.header, next.mdbuffer).safe_then([FNAME, &cursor] {
        cursor.
        pop_record_group();
        if (cursor.
            is_complete()) {
            INFO
            ("complete at {}, no more record group",
             cursor);
        }
    }
                                                        ) ;
...
{
    return seastar::do_with(get_segment_managers(),
    ...
    find_journal_segment_headers_ret {
        ...
        sm) {
            ...
            d_segment_id) {
                ...
                handle([](auto) {
                    return
                        find_journal_segment_headers_ertr::
                        now
                        ();
                }
                       ),
