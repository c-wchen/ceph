...
m_shutdown_rd_fd(-1),
m_shutdown_wr_fd(-1),
going_down(false), data_size(0), m_lock("OutputDataSocket::m_lock")
{
}
...
std::string OutputDataSocket::create_shutdown_pipe(int *pipe_rd, int *pipe_wr)
{
    ...
    if (pipe_cloexec(pipefd) < 0) {
        int e = errno;
        ostringstream oss;
        oss << "OutputDataSocket::create_shutdown_pipe error: " <<
            cpp_strerror(e);
        return oss.str();
    }
...
int OutputDataSocket::dump_data(int fd)
{
    ...
    for (list < bufferlist >::iterator iter = l.begin(); iter != l.end();
         ++iter) {
        bufferlist & bl = *iter;
        int ret = safe_write(fd, bl.c_str(), bl.length());
        if (ret >= 0) {
