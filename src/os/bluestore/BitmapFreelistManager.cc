...
bitmap_prefix(bitmap_prefix), kvdb(db), enumerate_bl_pos(0)
{
}
                                  ...
                                  KeyValueDB::Transaction txn)
{
    ...
    if (blocks / blocks_per_key * blocks_per_key != blocks) {
        ...
        dout(10) << __func__ << " rounding blocks up from 0x" << std::
            hex << size << " to 0x" << (blocks * bytes_per_block)
...
int BitmapFreelistManager::init(uint64_t dev_size)
{
    ...
    while (it->valid()) {
        ...
        if (k == "bytes_per_block") {
            ...
            bufferlist::iterator p = bl.begin();
            ::decode(blocks, p);
            dout(10) << __func__ << " blocks 0x" << std::hex << blocks << std::
                dec << dendl;
        }
        else if (k == "size") {
            bufferlist bl = it->value();
            ...
            dout(10) << __func__ << " blocks_per_key 0x" << std::
                hex << blocks_per_key << std::dec << dendl;
        }
        else {
    ...
    // check for http://tracker.ceph.com/issues/21089 inconsistency
    {
        ...
        if (new_size != size) {
            ...
            if (size == bad_size) {
                derr << __func__ << " size is 0x" << std::
                    hex << size << " should be 0x" << new_size <<
                    " and appears to be due to #21089" << std::dec << dendl;
                ...
                if (new_blocks != blocks) {
                    derr << "blocks is 0x" << std::
                        hex << blocks << " should be 0x" << new_blocks << std::
                        dec << dendl;
                    bufferlist bl;
                ...
                size = new_size;
                blocks = new_blocks;
                derr << __func__ << " fixed inconsistency, size now 0x" << std::
                    hex << size << " blocks 0x" << blocks << std::dec << dendl;
            }
        }
    }
    return 0;
    ...
    if (enumerate_p->valid()) {
        while (true) {
            ...
            if (enumerate_bl_pos >= 0) {
                ...
                *length = end - *offset;
                dout(10) << __func__ << std::
                    hex << " 0x" << *offset << "~" << *length << std::
                    dec << dendl;
                return true;
            }
            dout(30) << " no more set bits in 0x" << std::
                hex << enumerate_offset << std::dec << dendl;
            enumerate_p->next();
            enumerate_bl.clear();
            enumerate_bl_pos = 0;
            if (!enumerate_p->valid()) {
    ...
    if (first_key == last_key) {
        ...
        if (bl.length() > 0) {
            ...
            for (unsigned i = s; i <= e; ++i) {
                ...
                if (has != val) {
                    derr << __func__ << " key 0x" << std::
                        hex << first_key << " bit 0x" << i << " has 0x" << has
                        << " expected 0x" << val << std::dec << dendl;
                    ++errors;
                }
            }
        }
        ...
        // first key
        {
            ...
            if (bl.length()) {
                ...
                for (unsigned i = s; i < e; ++i) {
                    ...
                    if (has != val) {
                        derr << __func__ << " key 0x" << std::
                            hex << first_key << " bit 0x" << i << " has 0x" <<
                            has << " expected 0x" << val << std::dec << dendl;
                        ++errors;
                    }
                }
            }
                ...
                if (val) {
                    derr << __func__ << " key 0x" << std::hex << first_key
                        << " not present, expected 0x" << val << std::
                        dec << dendl;
                    ++errors;
                }
            }
        ...
        assert(first_key == last_key);
        {
            ...
            if (bl.length() > 0) {
                const char *p = bl.c_str();
                unsigned e =
                    ((offset + length - 1) & ~key_mask) / bytes_per_block;
                                 ...
                                 KeyValueDB::Transaction txn)
{
    ...
    if (first_key == last_key) {
        ...
        bufferlist bl;
        bl.append(p);
        dout(30) << __func__ << " 0x" << std::hex << first_key << std::
            dec << ": ";
