...
int LevelDBStore::do_open(ostream & out, bool create_if_missing)
{
...
#else
        assert(0 ==
               "bloom size set but installed leveldb doesn't support bloom filters");
#endif
    }
    ...
    PerfCountersBuilder plb(g_ceph_context, "leveldb", l_leveldb_first,
                            l_leveldb_last);
    plb.add_u64_counter(l_leveldb_gets, "leveldb_get", "Gets");
    plb.add_u64_counter(l_leveldb_txns, "leveldb_transaction", "Transactions");
    plb.add_time_avg(l_leveldb_get_latency, "leveldb_get_latency",
                     "Get Latency");
    plb.add_time_avg(l_leveldb_submit_latency, "leveldb_submit_latency",
                     "Submit Latency");
    plb.add_time_avg(l_leveldb_submit_sync_latency,
    ...
    plb.add_u64_counter(l_leveldb_compact_queue_merge,
...
void LevelDBStore::LevelDBTransactionImpl::set(const string & prefix,
                                               const string & k,
                                               const bufferlist & to_set_bl)
{
    ...
    if (to_set_bl.is_contiguous() && bllen > 0) {
        ...
        bat.Put(leveldb::Slice(key),
                leveldb::Slice(to_set_bl.buffers().front().c_str(), bllen));
    }
        ...
        for (pb = to_set_bl.buffers().begin(); pb != to_set_bl.buffers().end();
             ++pb) {
...
void LevelDBStore::LevelDBTransactionImpl::
...
void LevelDBStore::LevelDBTransactionImpl::rm_range_keys(const string & prefix,
                                                         const string & start,
                                                         const string & end)
{
    KeyValueDB::Iterator it = db->get_iterator(prefix);
                      ...
                      std::map < string, bufferlist > *out)
{
    ...
    for (std::set < string >::const_iterator i = keys.begin();
        ...
        auto status =
            db->Get(leveldb::ReadOptions(), leveldb::Slice(bound), &value);
        if (status.ok())
            (*out)[*i].append(value);
    }
    utime_t lat = ceph_clock_now() - start;
...
string LevelDBStore::combine_strings(const string & prefix,
                                     const string & value)
{
    string out = prefix;
    out.push_back(0);
