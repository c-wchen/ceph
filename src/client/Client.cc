...
Client::Client(Messenger *m, MonClient *mc, Objecter *objecter_)
    : Dispatcher(m->cct->get()),
      ...
      cct_deleter{m->cct, [](CephContext * p)
{
    p->put();
}},
                                ...
                                const UserPerm &request_perms)
{
    ...
    ldout(cct, 12) << __func__ << " client inode change_attr: " << in->change_attr << " , mds inodestat change_attr:  " <<
                   st->change_attr << dendl;
                      ...
                      int flush, ceph_tid_t flush_tid)
{
    ...
    if (cct->_conf->client_inject_release_failure && revoking) {
        ...
        ldout(cct, 20) << __func__ << " issued " << ccap_string(cap->issued) << " vs " << ccap_string(
                           would_have_issued) << dendl;
        ldout(cct, 20) << __func__ << " implemented " << ccap_string(cap->implemented) << " vs " << ccap_string(
                           would_have_implemented) << dendl;
    } else {
        // Normal behaviour
        cap->issued &= retain;
...
void Client::queue_cap_snap(Inode *in, SnapContext &old_snapc)
{
    ...
    if (in->cap_snaps.size() &&
        ...
        const auto &capsnapem = in->cap_snaps.emplace(std::piecewise_construct, std::make_tuple(old_snapc.seq),
                                std::make_tuple(in));
                            ...
                            inodeno_t realm, int flags, const UserPerm &cap_perms)
{
    ...
    const auto &capem = in->caps.emplace(std::piecewise_construct, std::forward_as_tuple(mds), std::forward_as_tuple(*in,
                                         mds_session));
...
unsigned Client::statx_to_mask(unsigned int flags, unsigned int want)
{
    ...
    mask |= CEPH_CAP_PIN;
    if (want & (CEPH_STATX_MODE | CEPH_STATX_UID | CEPH_STATX_GID | CEPH_STATX_BTIME | CEPH_STATX_CTIME |
                CEPH_STATX_VERSION)) {
...
void Client::C_Readahead::finish(int r)
{
    lgeneric_subdout(client->cct, client,
                     20) << "client." << client->get_nodeid() << " " << "C_Readahead on " << f->inode << dendl;
                   ...
                   const UserPerm &perms)
{
    ...
    InodeRef quota_root = root->quota.is_enabled(QUOTA_MAX_BYTES) ? root : get_quota_root(root.get(), perms,
                          QUOTA_MAX_BYTES);
