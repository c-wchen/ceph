...
namespace librbd {
    namespace crypto {
        namespace luks {
            ...
            Header::Header(CephContext * cct):m_cct(cct), m_fd(-1),
                m_cd(nullptr) {
            } Header::~Header() {
                if (m_fd != -1) {
                    close(m_fd);
                    ...
                    crypt_free(m_cd);
                    m_cd = nullptr;
            }} void Header::libcryptsetup_log_wrapper(int level,
                                                      const char *msg,
                                                      void *header) {
            ...
            int Header::init() {
                ...
                if (m_fd == -1) {
                    lderr(m_cct) << "error creating anonymous file: " <<
                        cpp_strerror(-errno)
                        << dendl;
                    return -errno;
                }
                ...
                if (r != 0) {
                    lderr(m_cct) << "crypt_init failed: " << cpp_strerror(r) <<
                        dendl;
                    return r;
                }
            ...
            ssize_t Header::read(ceph::bufferlist * bl) {
                ...
                if (r < 0) {
                    r = -errno;
                    lderr(m_cct) << "failed to stat anonymous file: " <<
                        cpp_strerror(r)
                        << dendl;
                    return r;
                }
                ...
                if (r < 0) {
                    lderr(m_cct) << "error reading header: " << cpp_strerror(r)
                        << dendl;
                }
            ...
            int Header::format(const char *type, const char *alg,
                ...
                if (insecure_fast_mode) {
                    ...
                    if (r != 0) {
                        lderr(m_cct) << "crypt_set_pbkdf_type failed: " <<
                            cpp_strerror(r)
                            << dendl;
                        return r;
                    }
                ...
                auto r =
                    crypt_format(m_cd, type, alg, cipher_mode, NULL, key,
                                 key_size, params);
                if (r != 0) {
                    lderr(m_cct) << "crypt_format failed: " << cpp_strerror(r)
                        << dendl;
                    return r;
                }
            ...
            int Header::add_keyslot(const char *passphrase,
                                    size_t passphrase_size) {
                ceph_assert(m_cd != nullptr);

                auto r =
                    crypt_keyslot_add_by_volume_key(m_cd, CRYPT_ANY_SLOT, NULL,
                                                    0, passphrase,
                                                    passphrase_size);
                if (r < 0) {
            ...
            int Header::load(const char *type) {
                ...
                if (r != 0) {
                    ldout(m_cct,
                          20) << "crypt_load failed: " << cpp_strerror(r) <<
                        dendl;
                    return r;
                }

                ldout(m_cct,
                      20) << "sector size: " << get_sector_size() <<
                    ", data offset: " << get_data_offset() << dendl;

                return 0;
            }

            int Header::read_volume_key(const char *passphrase,
                                        size_t passphrase_size,
                                        char *volume_key,
                ...
                auto r =
                    crypt_volume_key_get(m_cd, CRYPT_ANY_SLOT, volume_key,
                                         volume_key_size, passphrase,
                                         passphrase_size);
                if (r < 0) {
                    ldout(m_cct,
                          20) << "crypt_volume_key_get failed: " <<
                        cpp_strerror(r)
                        << dendl;
                    return r;
                }
