...
public:
    ...
    static inode_ptr allocate_inode(Args && ... args)
    {
        ...
        return std::allocate_shared < mempool_inode > (allocator,
                std::forward < Args >
                (args) ...);
    ...
    static xattr_map_ptr allocate_xattr_map(Args && ... args)
    {
        ...
        return std::allocate_shared < mempool_xattr_map > (allocator,
                std::forward < Args >
                (args) ...);
    }
    ...
    using old_inode_map_const_ptr =
        std::shared_ptr < const mempool_old_inode_map >;

    template < typename ... Args >
    static old_inode_map_ptr allocate_old_inode_map(Args && ... args)
    {
        static mempool::mds_co::pool_allocator < mempool_old_inode_map >
        allocator;
        return std::allocate_shared < mempool_old_inode_map > (allocator,
                std::forward <
                Args >
                (args) ...);
    }
    ...
    void encode_bare(ceph::buffer::list &bl, uint64_t features,
                     const ceph::buffer::list *snap_blob = NULL) const;
    void decode_bare(ceph::buffer::list::const_iterator &bl,
                     ceph::buffer::list &snap_blob, __u8 struct_v = 5);
    ...
    static void old_indoes_cb(InodeStoreBase::mempool_old_inode_map &c,
                              JSONObj *obj);
...
public:
    ...
    // ---------------------------
    CInode() = delete;
    CInode(MDCache *c, bool auth = true, snapid_t f = 2, snapid_t l =
               CEPH_NOSNAP);
    ~CInode()override
    {
        close_dirfrags();
    ...
    struct projected_inode {
        ...
        explicit projected_inode(inode_ptr && i, xattr_map_ptr && x, sr_t *s = nullptr):
            inode(std::move(i)), xattrs(std::move(x)),
            snapnode(s)
        {
        }
    };
    ...
    mempool_inode *_get_projected_inode()
    {
        ...
        return const_cast <
               mempool_inode * >(projected_nodes.back().inode.get());
    }
    ...
    void remove_need_snapflush(CInode *snapin, snapid_t snapid,
                               client_t client);
    ...
    // note: this overloads MDSCacheObject bool is_ambiguous_auth() const {
...
// only used for updating newly allocated CInode mempool_inode *_get_inode() {
if (inode == empty_inode)
{
    reset_inode(allocate_inode());
}
...
const xattr_map_const_ptr &get_xattrs() const
{
    return xattrs;
}
bool is_any_old_inodes() const
{
    return old_inodes && !old_inodes->empty();
}
const old_inode_map_const_ptr &get_old_inodes() const
{
    return old_inodes;
}
...
const CDentry *get_parent_dn() const
{
    return parent;
}
...
// -- misc --  bool is_ancestor_of(const CInode * other)const;
bool is_projected_ancestor_of(const CInode *other) const;

void make_path_string(std::string &s, bool projected =
                          false, const CDentry *use_parent = NULL) const;
...
void _fetched(ceph::buffer::list &bl, ceph::buffer::list &bl2,
              Context *fin);
...
void _encode_locks_state_for_replica(ceph::buffer::list &bl,
                                     bool need_recover);
void _encode_locks_state_for_rejoin(ceph::buffer::list &bl, int rep);
void _decode_locks_state_for_replica(ceph::buffer::list::const_iterator &p,
                                     bool is_new);
void _decode_locks_rejoin(ceph::buffer::list::const_iterator &p,
...
int encode_inodestat(ceph::buffer::list &bl, Session *session,
                     SnapRealm *realm, snapid_t snapid =
                         CEPH_NOSNAP, unsigned max_bytes =
                         0, int getattr_wants = 0);
void encode_cap_message(const ceph::ref_t < MClientCaps > &m,
                        Capability *cap);
...
const mempool::mds_co::compact_map < int32_t,
      int32_t > &get_mds_caps_wanted() const
{
    return mds_caps_wanted;
}
void set_mds_caps_wanted(mempool::mds_co::compact_map < int32_t,
                         int32_t > &m);
void set_mds_caps_wanted(mds_rank_t mds, int32_t wanted);

const mempool_cap_map &get_client_caps() const
{
    return client_caps;
}
...
int get_num_caps_notable() const
{
...
Capability *add_client_cap(client_t client, Session *session,
                           SnapRealm *conrealm = nullptr, bool new_inode =
                               false);
void remove_client_cap(client_t client);
void move_to_realm(SnapRealm *realm);

Capability *reconnect_cap(client_t client, const cap_reconnect_t &icr,
                          Session *session);
...
int get_caps_wanted(int *ploner = 0, int *pother = 0, int shift =
                        0, int mask = -1) const;
bool issued_caps_need_gather(SimpleLock *lock);

// client writeable
bool is_clientwriteable() const
{
...
{
    generic_dout(0) << " bad put " << *this << " by " << by << " " <<
                    pin_name(by) << " was " << ref
#ifdef MDS_REF_SET
                    << " (" << ref_map << ")"
#endif
...
{
    generic_dout(0) << " bad get " << *this << " by " << by << " " <<
                    pin_name(by) << " was " << ref
#ifdef MDS_REF_SET
                    << " (" << ref_map << ")"
#endif
...
mempool::mds_co::compact_map < snapid_t,
        mempool::mds_co::set < client_t > >client_need_snapflush;
...
elist < CInode * >::item &item_recover_queue_front =
...
ceph_lock_state_t *get_fcntl_lock_state()
{
    if (!fcntl_locks)
        fcntl_locks =
            new ceph_lock_state_t(g_ceph_context, CEPH_LOCK_FCNTL);
    return fcntl_locks;
}
...
ceph_lock_state_t *get_flock_lock_state()
{
    if (!flock_locks)
        flock_locks =
            new ceph_lock_state_t(g_ceph_context, CEPH_LOCK_FLOCK);
    return flock_locks;
}
...
struct projected_const_node {
    ...
    explicit projected_const_node(const inode_const_ptr &i,
                                  const xattr_map_const_ptr &x,
                                  sr_t *s): inode(i), xattrs(x),
        snapnode(s)
    {
    }
};
