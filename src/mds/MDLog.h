...
public:
    explicit MDLog(MDSRank *m): mds(m),
        replay_thread(this), recovery_thread(this), submit_thread(this)
    {
    } ~MDLog();
...
protected:
    struct PendingEvent {
        PendingEvent(LogEvent *e, MDSContext *c, bool f = false): le(e), fin(c),
            flush(f)
        {
        } LogEvent *le;
        MDSContext *fin;
    ...
    public:
        ...
        {
        } void set_completion(MDSContext *c)
        {
            completion = c;
        }
        void *entry() override
    ...
    void _reformat_journal(JournalPointer const &jp, Journaler *old_journal,
