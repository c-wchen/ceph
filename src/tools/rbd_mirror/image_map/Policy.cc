    ...
    namespace mirror {
        namespace image_map {
            ...
            namespace {
                ...
                bool is_instance_action(ActionType action_type) {
                    ...
                    case ACTION_TYPE_RELEASE:
                        return true;
                        case ACTION_TYPE_NONE:case ACTION_TYPE_MAP_UPDATE:case
                            ACTION_TYPE_MAP_REMOVE:break;
                    } return false;
            }} // anonymous namespace using::operator<<;
          ...
          :    
            m_ioctx(ioctx),
            m_map_lock(ceph::
                       make_shared_mutex(unique_lock_name
                                         ("rbd::mirror::image_map::Policy::m_map_lock",
                                          this))) {
            ...
            void Policy::init(const std::map < std::string,
              ...
              for (auto & it:image_mapping) {
                    ...
                    auto map_result =
                        m_map[it.second.instance_id].emplace(it.first);
                    ceph_assert(map_result.second);

                    auto image_state_result =
                        m_image_states.emplace(it.first,
                                               ImageState { it.second.
                                               instance_id,
            ...
            bool Policy::add_image(const std::string & global_image_id) {
                ...
                m_map_lock};
                auto image_state_result =
                    m_image_states.emplace(global_image_id,
                                           ImageState { });
                auto & image_state = image_state_result.first->second;
                ...
                return set_state(&image_state,
                                 StateTransition::STATE_ASSOCIATING, false);
            }
            ...
            bool Policy::remove_image(const std::string & global_image_id) {
                ...
                return set_state(&image_state,
                                 StateTransition::STATE_DISSOCIATING, false);
            }
            ...
            void Policy::add_instances(const InstanceIds & instance_ids,
              ...
              for (auto & instance:instance_ids) {
                ...
                if (m_initial_update) {
                    ...
                    std::set < std::string >
                        alive_instances(instance_ids.begin(),
                                        instance_ids.end());
                    InstanceIds dead_instances;
                  for (auto & map_pair:m_map) {
                        if (alive_instances.find(map_pair.first) ==
                            alive_instances.end()) {
                    ...
                    if (!dead_instances.empty()) {
                        remove_instances(m_map_lock, dead_instances,
                                         global_image_ids);
                    }
                }

                GlobalImageIds shuffle_global_image_ids;
                do_shuffle_add_instances(m_map, m_image_states.size(),
                                         &shuffle_global_image_ids);
                dout(5) << "shuffling global_image_ids=[" <<
                    shuffle_global_image_ids << "]" << dendl;
              for (auto & global_image_id:shuffle_global_image_ids)
                {
                    ...
                    auto & image_state = it->second;
                    if (set_state
                        (&image_state, StateTransition::STATE_SHUFFLING,
                         false)) {
            ...
            void Policy::remove_instances(const ceph::shared_mutex & lock,
              ...
              for (auto & instance_id:instance_ids) {
                    ...
                    dout(5) << "force shuffling: instance_id=" << instance_id <<
                        ", " << "global_image_ids=[" <<
                        instance_global_image_ids << "]" << dendl;
            ...
            bool Policy::finish_action(const std::string & global_image_id,
                                       int r) {
                std::unique_lock map_lock {
                m_map_lock};
                ...
                StateTransition::transit(image_state.state,
                                         &image_state.transition);
                if (transition.finish_state) {
                    // in-progress state machine complete
                    ceph_assert(StateTransition::
                                is_idle(*transition.finish_state));
                    image_state.state = *transition.finish_state;
                    image_state.transition = {
                    };
                }

                if (StateTransition::is_idle(image_state.state)
                    && image_state.next_state) {
                    // advance to pending state machine
                    bool start_action =
                        set_state(&image_state, *image_state.next_state, false);
                    ceph_assert(start_action);
                }

                // image state may get purged in execute_policy_action()
                bool pending_action =
                    image_state.transition.action_type != ACTION_TYPE_NONE;
            ...
            void Policy::execute_policy_action(const std::
                ...
                case StateTransition::POLICY_ACTION_REMOVE:
                    if (image_state->state ==
                        StateTransition::STATE_UNASSOCIATED) {
                        ceph_assert(image_state->instance_id ==
            ...
            void Policy::map(const std::string & global_image_id,
                             ImageState * image_state) {
                ceph_assert(ceph_mutex_is_wlocked(m_map_lock));
            ...
            void Policy::unmap(const std::string & global_image_id,
                ...
                if (is_dead_instance(instance_id) && m_map[instance_id].empty()) {
                    dout(5) << "removing dead instance_id=" << instance_id <<
                        dendl;
            ...
            bool Policy::set_state(ImageState * image_state,
                ...
                if (!ignore_current_state && image_state->state == state) {
                    ...
                    StateTransition::transit(image_state->state,
                                             &image_state->transition);
                    ceph_assert(image_state->transition.action_type !=
                                ACTION_TYPE_NONE);
