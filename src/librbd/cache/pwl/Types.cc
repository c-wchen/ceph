 ...
 * of 1.
 */ BlockExtent convert_to_block_extent(uint64_t offset_bytes,
...
void WriteLogCacheEntry::dump(Formatter *f) const
{
    ...
    f->dump_unsigned("write_sequence_number",
...
} void WriteLogCacheEntry::generate_test_instances(std::list <
        WriteLogCacheEntry
        * > &ls)
{
...
} void WriteLogPoolRoot::generate_test_instances(std::list <
        WriteLogPoolRoot *
        > &ls)
{
...
ExtentsSummary <
ExtentsType >::ExtentsSummary(const ExtentsType &extents)
    :    total_bytes(0), first_image_byte(0),
         last_image_byte(0)
{
    if (extents.empty()) {
        return;
...
{
    return convert_to_block_extent(image_extent.first,
                                   image_extent.second);
}
...
Context *override_ctx(int r, Context *ctx)
{
    if (r < 0) {
        ...
        return new LambdaContext([r, ctx](int _r) {
            ctx->complete(r);
        });
    } else {
        return ctx;
...
std::string unique_lock_name(const std::string &name,
                             void *address)
{
