...
public:
    ...
    {
        return roleArn;
    } const std::string &getRoleSessionName() const
    {
        return roleSessionName;
    } const std::string &getPolicy() const
    {
        return iamPolicy;
    } static const uint64_t &getMaxPolicySize()
    {
        return MAX_POLICY_SIZE;
    } void setMaxDuration(const uint64_t &maxDuration)
    {
...
public:
    AssumeRoleWithWebIdentityRequest(CephContext *cct,
                                     ...
                                     std::vector < std::pair < std::string,
                                     std::string >> session_princ_tags)
        : AssumeRoleRequestBase(cct, duration, iamPolicy, roleArn,
                                roleSessionName), providerId(providerId),
    ...
    {
        return providerId;
    } const std::string &getIss() const
    {
        return iss;
    } const std::string &getAud() const
    {
        return aud;
    ...
    std::string >> &getPrincipalTags() const
    {
...
public:
    AssumeRoleRequest(CephContext *cct,
        ...
        : AssumeRoleRequestBase(cct, duration, iamPolicy, roleArn,
                                roleSessionName), externalId(externalId),
...
public:
    GetSessionTokenRequest(const std::string &duration,
                           const std::string &serialNumber,
                           const std::string &tokenCode);
    ...
    {
        return duration;
    } static const uint64_t &getMinDuration()
    {
...
public:
    int generateAssumedRoleUser(CephContext *cct,
                                ...
                                const std::string &roleSessionName);
    const std::string &getARN() const
    {
        return arn;
    } const std::string &getAssumeRoleId() const
    {
        return assumeRoleId;
...
public:
    ...
    STSService(CephContext *cct, rgw::sal::Driver *driver,
               rgw_user user_id, rgw::auth::Identity *identity)
        : cct(cct), driver(driver), user_id(user_id), identity(identity)
    {
    } std::tuple < int,
    rgw::sal::RGWRole * >getRoleInfo(const DoutPrefixProvider *dpp,
                                     const std::string &arn,
                                     optional_yield y);
    AssumeRoleResponse assumeRole(const DoutPrefixProvider *dpp,
                                  AssumeRoleRequest &req,
                                  optional_yield y);
    GetSessionTokenResponse getSessionToken(const DoutPrefixProvider *dpp,
                                            GetSessionTokenRequest &req);
    AssumeRoleWithWebIdentityResponse assumeRoleWithWebIdentity(const
            DoutPrefixProvider
            * dpp,
