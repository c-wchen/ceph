...
public:
    C_WriteRequest(T &pwl, const utime_t arrived, io::Extents
                   ...
                   Context *user_req)
        : pwl::C_WriteRequest < T > (pwl, arrived,
...
{
public:
    C_CompAndWriteRequest(T &pwl, const utime_t arrived,
                          io::Extents
                          && image_extents, bufferlist
                          && cmp_bl, bufferlist
                          && bl, uint64_t *mismatch_offset,
                          ...
                          PerfCounters *perfcounter,
                          Context *user_req)
        : C_WriteRequest < T > (pwl, arrived,
                                std::move(image_extents),
                                std::move(cmp_bl), std::move(bl),
                                mismatch_offset, fadvise_flags, lock,
                                perfcounter, user_req)
    {
...
{
public:
    C_WriteSameRequest(T &pwl, const utime_t arrived,
                       io::Extents && image_extents, bufferlist
                       && bl, const int fadvise_flags,
                       ceph::mutex &lock,
                       PerfCounters *perfcounter,
                       Context *user_req)
        : pwl::C_WriteSameRequest < T > (pwl, arrived,
                                         std::move(image_extents),
                                         std::move(bl),
                                         fadvise_flags, lock,
                                         perfcounter, user_req)
