...
#define LARGE_ENOUGH_BUF (128 * 1024)
    ...
    ldout(s->cct, 20) << __func__ << "(): read data: " << string(data,
                                                                 len) << dendl;
...
void RGWOp_BILog_List::execute()
{
    ...
    if (bucket_name.empty() && bucket_instance.empty()) {
        dout(5) << "ERROR: neither bucket nor bucket instance specified" <<
            dendl;
        http_ret = -EINVAL;
        return;
    }
    ...
    int shard_id;
    http_ret =
        rgw_bucket_parse_bucket_instance(bucket_instance, &bucket_instance,
                                         &shard_id);
    if (http_ret < 0) {
        return;
    }
    ...
    if (!bucket_instance.empty()) {
        http_ret =
            store->get_bucket_instance_info(obj_ctx, bucket_instance,
                                            bucket_info, NULL, NULL);
        if (http_ret < 0) {
            dout(5) <<
                "could not get bucket instance info for bucket instance id=" <<
                bucket_instance << dendl;
            return;
        }
    }
    else {                      /* !bucket_name.empty() */
        http_ret =
            store->get_bucket_info(obj_ctx, tenant_name, bucket_name,
                                   bucket_info, NULL, NULL);
        if (http_ret < 0) {
            dout(5) << "could not get bucket info for bucket=" << bucket_name <<
                dendl;
            return;
        }
    }
...
void RGWOp_BILog_Info::execute()
{
    ...
    int ret =
        store->get_bucket_stats(bucket_info, shard_id, &bucket_ver, &master_ver,
...
void RGWOp_BILog_Delete::execute()
{
    ...
    if ((bucket_name.empty() && bucket_instance.empty()) || end_marker.empty()) {
        dout(5) <<
            "ERROR: one of bucket and bucket instance, and also end-marker is mandatory"
            << dendl;
        http_ret = -EINVAL;
        return;
    }
...
void RGWOp_DATALog_Lock::execute()
{
    ...
    http_ret =
        store->data_log->lock_exclusive(shard_id, make_timespan(dur), zone_id,
                                        locker_id);
    if (http_ret == -EBUSY)
        http_ret = -ERR_LOCKED;
}
...
void RGWOp_DATALog_Unlock::execute()
{
    ...
    if (shard_id_str.empty() || locker_id.empty() || zone_id.empty()) {
        dout(5) << "Error invalid parameter list" << dendl;
        http_ret = -EINVAL;
        return;
    }
...
#define LARGE_ENOUGH_BUF (128 * 1024)
    ...
    if (store->ctx()->_conf->subsys.should_gather(ceph_subsys_rgw, 20)) {
        for (map < int, set < string > >::iterator iter =
             updated_shards.begin(); iter != updated_shards.end(); ++iter) {
            ldout(s->cct,
                  20) << __func__ << "(): updated shard=" << iter->
                first << dendl;
            set < string > &keys = iter->second;
            for (set < string >::iterator kiter = keys.begin();
                 kiter != keys.end(); ++kiter) {
                ldout(s->cct,
                      20) << __func__ << "(): modified key=" << *kiter << dendl;
            }
        }
    }
...
void RGWOp_DATALog_Delete::execute()
{
    ...
    http_ret =
        store->data_log->trim_entries(shard_id, ut_st, ut_et, start_marker,
                                      end_marker);
}
...
void RGWOp_BILog_Status::execute()
{
    ...
    RGWBucketInfo info;
    http_ret =
        store->get_bucket_instance_info(ctx, bucket, info, nullptr, nullptr);
    if (http_ret < 0) {
        ldout(s->cct,
...
void RGWOp_DATALog_Status::execute()
{
    ...
    if (sync == nullptr) {
        ldout(s->cct,
              1) << "no sync manager for source-zone " << source_zone << dendl;
        http_ret = -ENOENT;
        return;
    }
    http_ret = sync->read_sync_status(&status);
