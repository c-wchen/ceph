...
void RGWListBuckets_ObjStore_SWIFT::handle_listing_chunk(RGWUserBuckets
                                                         && buckets)
{
    if (wants_reversed) {
...
void RGWListBuckets_ObjStore_SWIFT::
send_response_data_reversed(RGWUserBuckets & buckets)
{
    if (!sent_data) {
        return;
    }
                                        ...
                                        bool * const has_cors)
{
    ...
    *has_cors = false;
    ...
    if (allow_origins) {
        ...
        if (r < 0) {
            dout(0) << "Error creating/updating the cors configuration" <<
                dendl;
            delete swift_cors;
            return r;
        }
...
int RGWPutObj_ObjStore_SWIFT::get_params()
{
    ...
    if (!s->generic_attrs.count(RGW_ATTR_CONTENT_TYPE)) {
        ldout(s->cct,
              5) << "content type wasn't provided, trying to guess" << dendl;
    ...
    RGWBulkUploadOp_ObjStore_SWIFT::create_stream()
{
    ...
    if (!s->length) {
        ...
        return std::unique_ptr < SwiftStreamGetter >
            (new SwiftStreamGetter(s, conlen));
    }
}
...
void RGWInfo_ObjStore_SWIFT::execute()
{
    ...
    if (!swiftinfo_sig.empty() &&
        !swiftinfo_expires.empty() && !is_expired(swiftinfo_expires, s->cct)) {
                                             ...
                                             RGWRados & store)
{
    ...
    const size_t meta_value_limit =
        g_conf->get_val < size_t > ("rgw_max_attr_size");
    if (meta_value_limit) {
  ...
  for (const auto & placement_targets:zonegroup.placement_targets) {
        ...
        if (placement_targets.second.name.
            compare(zonegroup.default_placement) == 0)
                                 ...
                                 RGWUserInfo & owner_info) const const
{
    ...
    int ret =
        store->get_bucket_info(*static_cast < RGWObjectCtx * >(s->obj_ctx),
                                ...
                                rgw::io::BasicClient * cio)
{
    ...
    if (!copy_source.empty()) {
        bool result =
            RGWCopyObj::parse_copy_location(copy_source, t->src_bucket,
                                            s->src_object);
        if (!result)
            return -ERR_BAD_URL;
    }
...
RGWHandler_REST *RGWRESTMgr_SWIFT_Info::get_handler(struct req_state * const s,
                                                    const rgw::auth::
                                                    StrategyRegistry &
                                                    auth_registry,
                                                    const std::
