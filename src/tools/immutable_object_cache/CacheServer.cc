...
namespace ceph {
    namespace immutable_obj_cache {
        ...
        :cct(cct), m_server_process_msg(processmsg),
            ...
            if (ec) {
                ldout(cct,
                      1) << "m_io_service run fails: " << ec.message() << dendl;
                return -1;
            }
            return 0;
        }
        ...
        int CacheServer::start_accept() {
            ...
            if (ec) {
                lderr(cct) << "failed to open domain socket: " << ec.
                    message() << dendl;
                return -ec.value();
            }
            ...
            if (ec) {
                lderr(cct) << "failed to listen on domain socket: " << ec.
                    message()
                    << dendl;
                return -ec.value();
            }
        ...
        void CacheServer::accept() {
            ...
            m_acceptor.async_accept(new_session->socket(),
                                    boost::bind(&CacheServer::handle_accept,
                                                this, new_session,
        ...
        void CacheServer::handle_accept(CacheSessionPtr new_session,
            ...
            if (error) {
                // operation_absort
                lderr(cct) << "async accept fails : " << error.
                    message() << dendl;
                return;
            }
