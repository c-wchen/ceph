...
struct OSDCapPoolTag {
    ...
    OSDCapPoolTag() {
                    ...
                    const std::string & value):application(application),
        key(key), value(value) {
    }
...
BOOST_FUSION_ADAPT_STRUCT(OSDCapPoolTag, (std::string, application)
                          (std::string, key)
                          (std::string, value))
...
bool is_match(const std::string & pool_name, const std::string & nspace_name,
              const app_map_t & app_map, const std::string & object) const;
bool is_match_all() const;
};
...
struct OSDCapGrant {
  ...
  :    profile(profile) {
    ...
    bool is_capable(const std::string & pool_name, const std::string & ns,
                    ...
                    const std::string & object, bool op_may_read,
                    bool op_may_write,
