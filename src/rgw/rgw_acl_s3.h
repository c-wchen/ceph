...
{
public:
    explicit RGWAccessControlList_S3(CephContext *
                                     _cct): RGWAccessControlList(_cct)
    {
    ...
    int create_canned(ACLOwner &owner, ACLOwner &bucket_owner,
...
{
public:
    explicit RGWAccessControlPolicy_S3(CephContext *
                                       _cct): RGWAccessControlPolicy(_cct)
    {
    ...
    void to_xml(std::ostream &out);
    int rebuild(const DoutPrefixProvider *dpp, rgw::sal::Driver *driver,
                ACLOwner *owner, RGWAccessControlPolicy &dest,
                std::string &err_msg);
    bool compare_group_name(std::string &id, ACLGroupTypeEnum group) override;

    virtual int create_canned(ACLOwner &_owner, ACLOwner &bucket_owner,
                              const std::string &canned_acl)
    {
    ...
    int create_from_headers(const DoutPrefixProvider *dpp,
