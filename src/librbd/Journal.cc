...
// threads and reuse librbd's AsioEngine
        ...
        template < typename I > struct C_IsTagOwner:public Context {
            ...
            uint64_t tag_tid = 0;
             ...
             C_IsTagOwner(librados::IoCtx & io_ctx,
                          const std::string & image_id, bool * is_tag_owner,
            ...
            void finish(int r) override {
                ldout(cct,
                      20) << this << " C_IsTagOwner::" << __func__ << ": r=" <<
                    r << dendl;
                if (r < 0) {
                    lderr(cct) << this << " C_IsTagOwner::" << __func__ << ": "
                        << "failed to get tag owner: " << cpp_strerror(r) <<
                        dendl;
                }
                else {
                    *is_tag_owner =
        ...
        struct C_GetTagOwner:public Context {
            ...
            uint64_t tag_tid = 0;
             ...
             C_GetTagOwner(librados::IoCtx & io_ctx,
                           const std::string & image_id,
                           std::string * mirror_uuid, Context * on_finish)
            :mirror_uuid(mirror_uuid), on_finish(on_finish), journaler(io_ctx,
                                                                       image_id,
                                                                       Journal
                                                                       <>::
                                                                       IMAGE_CLIENT_ID,
        ...
        template < typename J > struct GetTagsRequest {
            ...
            Context *on_finish;
             ...
             GetTagsRequest(CephContext * cct, J * journaler,
                            cls::journal::Client * client,
                            journal::ImageClientMeta * client_meta,
                            uint64_t * tag_tid, journal::TagData * tag_data,
                            Context * on_finish)
            :cct(cct), journaler(journaler), client(client),
                client_meta(client_meta), tag_tid(tag_tid), tag_data(tag_data),
                on_finish(on_finish) {
            }
  /**
   * @verbatim
   ...
   */ void send() {
            ...
            void send_get_client() {
                ...
                auto ctx = new LambdaContext([this] (int r){
                                             handle_get_client(r);});
                journaler->get_client(Journal < ImageCtx >::IMAGE_CLIENT_ID,
                                      client, ctx);
            }
            ...
            void send_get_tags() {
                ...
                C_DecodeTags *tags_ctx =
                    new C_DecodeTags(cct, &lock, tag_tid, tag_data,
                                     ctx);
                journaler->get_tags(client_meta->tag_class, &tags_ctx->tags,
                                    tags_ctx);
            }
        ...
        template < typename J >
            ...
            GetTagsRequest < J > *req =
                new GetTagsRequest < J > (cct, journaler, client, client_meta,
        ...
        template < typename J >
            ...
            journaler->allocate_tag(tag_class, tag_bl, new_tag,
                                    &allocate_tag_ctx);
...
// mirror uuid to use for orphaned (demoted) images
    ...
    template < typename I >
        void Journal <
        I >::MetadataListener::handle_update(::journal::JournalMetadata *) {
        auto ctx = new LambdaContext([this] (int r){
    ...
    template < typename I >
        void Journal < I >::get_work_queue(CephContext * cct,
                                           ContextWQ ** work_queue) {
    ...
    template < typename I > Journal < I >::Journal(I & image_ctx)
  :    
    ...
    template < typename I >
        int Journal < I >::create(librados::IoCtx & io_ctx,
                                  const std::string & image_id, uint8_t order,
                                  uint8_t splay_width,
        ...
        journal::CreateRequest < I > *req =
            journal::CreateRequest < I >::create(io_ctx, image_id, order,
                                                 splay_width, object_pool,
                                                 cls::journal::Tag::
                                                 TAG_CLASS_NEW, tag_data,
                                                 IMAGE_CLIENT_ID, work_queue,
                                                 &cond);
        req->send();
    ...
    template < typename I >
        int Journal < I >::remove(librados::IoCtx & io_ctx,
                                  const std::string & image_id) {
        CephContext *cct = reinterpret_cast < CephContext * >(io_ctx.cct());
        ldout(cct, 5) << __func__ << ": image=" << image_id << dendl;
        ...
        journal::RemoveRequest < I > *req =
            journal::RemoveRequest < I >::create(io_ctx, image_id,
                                                 IMAGE_CLIENT_ID, work_queue,
                                                 &cond);
        req->send();
    ...
    template < typename I >
        int Journal < I >::reset(librados::IoCtx & io_ctx,
                                 const std::string & image_id) {
        CephContext *cct = reinterpret_cast < CephContext * >(io_ctx.cct());
        ldout(cct, 5) << __func__ << ": image=" << image_id << dendl;
        ...
        C_SaferCond cond;
        auto req =
            journal::ResetRequest < I >::create(io_ctx, image_id,
                                                IMAGE_CLIENT_ID,
    ...
    template < typename I >
        ...
        C_IsTagOwner < I > *is_tag_owner_ctx =
            new C_IsTagOwner < I > (io_ctx, image_id, is_tag_owner,
    ...
    template < typename I >
        void Journal < I >::get_tag_owner(IoCtx & io_ctx,
                                          std::string & image_id,
                                          std::string * mirror_uuid,
                                          asio::ContextWQ * op_work_queue,
                                          Context * on_finish) {
        ...
        get_tags(cct, &ctx->journaler, &ctx->client, &ctx->client_meta,
                 &ctx->tag_tid, &ctx->tag_data,
    ...
    template < typename I > int Journal < I >::request_resync(I * image_ctx) {
        ...
        auto open_req =
            journal::OpenRequest < I >::create(image_ctx, &journaler, &lock,
                                               &client_meta, &tag_tid,
                                               &tag_data, &open_ctx);
        open_req->send();
    ...
    template < typename I >
        ...
        auto promote_req =
            journal::PromoteRequest < I >::create(image_ctx, false,
                                                  on_finish);
        promote_req->send();
    }
    ...
    template < typename I > void Journal < I >::open(Context * on_finish) {
        ...
        m_image_ctx.io_object_dispatcher->
            register_dispatch(journal::ObjectDispatch <
                              I >::create(&m_image_ctx, this));
    ...
    template < typename I > void Journal < I >::close(Context * on_finish) {
        ...
        on_finish = new LambdaContext([this, on_finish] (int r) {
                                      ...
                                      auto ctx =
                                      new LambdaContext([on_finish, r] (int _) {
                                                        on_finish->
                                                        complete(r);});
                                      m_image_ctx.io_object_dispatcher->
                                      shut_down_dispatch(io::
        ...
        m_lock};
        m_listener_cond.wait(locker,[this] {
                             return !m_listener_notify;
                             }
        );
    ...
    template < typename I >
        ...
        CephContext *cct = m_image_ctx.cct;
        ldout(cct,
              20) << this << " " << __func__ << ":  mirror_uuid=" << mirror_uuid
            << dendl;
    ...
    template < typename I > void Journal < I >::user_flushed() {
        if (m_state == STATE_READY && !m_user_flushed.exchange(true) &&
            ...
            if (m_state == STATE_READY) {
                ...
                ceph_assert(m_journaler != nullptr);
                m_journaler->set_append_batch_options(m_image_ctx.config.
                                                      template get_val <
                                                      uint64_t >
    ...
    template < typename I >
        uint64_t Journal < I >::append_write_event(uint64_t offset,
                                                   size_t length,
                                                   const bufferlist & bl,
                                                   bool flush_entry) {
        ceph_assert(m_max_append_size >
    ...
    {
        ceph_assert(m_max_append_size >
                    journal::AioCompareAndWriteEvent::get_fixed_size());
        ...
        do {
            ...
            journal::EventEntry event_entry(journal::
                                            AioCompareAndWriteEvent(offset +
                                                                    event_offset,
                                                                    event_length,
                                                                    event_cmp_bl,
                                                                    event_write_bl),
    ...
    template < typename I >
        uint64_t Journal < I >::append_io_event(journal::EventEntry
                                                && event_entry, uint64_t offset,
                                                size_t length, bool flush_entry,
                                                int filter_ret_val) {
        bufferlist bl;
        event_entry.timestamp = ceph_clock_now();
        encode(event_entry, bl);
        return append_io_events(event_entry.get_event_type(), {
                                bl}
    ...
    template < typename I >
        void Journal < I >::commit_io_event_extent(uint64_t tid,
                                                   uint64_t offset,
    ...
    template < typename I >
        ...
        CephContext *cct = m_image_ctx.cct;
        ldout(cct,
              10) << this << " " << __func__ << ": op_tid=" << op_tid << ", " <<
            "r=" << r << dendl;
        ...
        op_finish_future.
            flush(create_async_context_callback
                  (m_image_ctx,
                   new C_OpEventSafe(this, op_tid, op_start_future,
                                     op_finish_future, on_safe)));
    }

    template < typename I >
        void Journal < I >::replay_op_ready(uint64_t op_tid,
                                            Context * on_resume) {
        CephContext *cct = m_image_ctx.cct;
        ldout(cct,
              10) << this << " " << __func__ << ": op_tid=" << op_tid << dendl;
    ...
    template < typename I >
        typename Journal < I >::Future Journal <
        I >::wait_event(ceph::mutex & lock, uint64_t tid, Context * on_safe) {
        ...
        event.on_safe_contexts.
            push_back(create_async_context_callback(m_image_ctx, on_safe));
        return event.futures.back();
    }

    template < typename I >
        void Journal < I >::start_external_replay(journal::Replay < I >
                                                  **journal_replay,
                                                  Context * on_start) {
        CephContext *cct = m_image_ctx.cct;
        ldout(cct, 20) << this << " " << __func__ << dendl;
        ...
        on_start = new LambdaContext([this, journal_replay, on_start] (int r) {
                                     handle_start_external_replay(r,
                                                                  journal_replay,
                                                                  on_start);});
        ...
        m_journaler->
            stop_append(util::
                        create_async_context_callback(m_image_ctx, on_start));
    }
    ...
    template < typename I > void Journal < I >::create_journaler() {
        ...
        // disconnected.
        settings.ignored_laggy_clients = {
        IMAGE_CLIENT_ID};
        ...
        Context *ctx =
            create_async_context_callback(m_image_ctx,
                                          create_context_callback < Journal <
                                          I >,
                                          &Journal < I >::handle_open > (this));
        auto open_req =
            journal::OpenRequest < I >::create(&m_image_ctx, m_journaler,
    ...
    template < typename I > void Journal < I >::destroy_journaler(int r) {
        ...
        Context *ctx =
            create_async_context_callback(m_image_ctx,
                                          ...
                                          I >,
                                          &Journal <
                                          I >::handle_journal_destroyed >
                                          (this));
        ctx = new LambdaContext([this, ctx] (int r) {
                                std::lock_guard locker {
                                m_lock}; m_journaler->shut_down(ctx);});
    ...
    template < typename I > void Journal < I >::recreate_journaler(int r) {
        ...
        m_journaler->
            shut_down(create_async_context_callback
                      (m_image_ctx, create_context_callback < Journal < I >,
                       &Journal < I >::handle_journal_destroyed > (this)));
    }

    template < typename I >
        void Journal < I >::complete_event(typename Events::iterator it,
                                           int r) {
        ceph_assert(ceph_mutex_is_locked(m_event_lock));
        ceph_assert(m_state == STATE_READY);
    ...
    template < typename I > void Journal < I >::start_append() {
        ...
        m_journaler->start_append(m_image_ctx.config.template get_val <
                                  uint64_t >
    ...
    template < typename I >
        void Journal < I >::handle_replay_process_safe(ReplayEntry replay_entry,
                                                       int r) {
        CephContext *cct = m_image_ctx.cct;
        ...
        if (r < 0) {
            ...
            if (m_state == STATE_REPLAYING) {
                ...
                Context *ctx = create_context_callback <
                    Journal < I >,
                    &Journal < I >::handle_flushing_restart > (this);
                ctx = new LambdaContext([this, ctx] (int r) {
                                        // ensure the commit position is flushed to disk
                                        m_journaler->
                ...
                ctx = new LambdaContext([this, cct, ctx] (int r) {
                                        ...
                                        m_journal_replay->shut_down(true,
                                                                    ctx);});
                m_journaler->stop_replay(ctx);
    ...
    template < typename I > void Journal < I >::handle_journal_destroyed(int r) {
        ...
        ceph_assert(m_state == STATE_CLOSING
                    || m_state == STATE_RESTARTING_REPLAY);
    ...
    template < typename I >
        ...
        CephContext *cct = m_image_ctx.cct;
        ldout(cct,
              20) << this << " " << __func__ << ": new state=" << state <<
            dendl;
        ceph_assert(ceph_mutex_is_locked(m_lock));
        m_state = state;
    ...
    template < typename I > bool Journal < I >::is_steady_state()const {
        ...
        case STATE_CLOSED:
            return true;
            case STATE_UNINITIALIZED:case STATE_INITIALIZING:case
                STATE_REPLAYING:case STATE_FLUSHING_RESTART:case
                STATE_RESTARTING_REPLAY:case STATE_FLUSHING_REPLAY:case
                STATE_STOPPING:case STATE_CLOSING:break;
        } return false;
    }
    ...
    template < typename I > void Journal < I >::handle_metadata_updated() {
        ...
        if (m_state != STATE_READY && !is_journal_replaying(m_lock)) {
            ...
            ldout(cct,
                  20) << this << " " << __func__ << ": primary image" << dendl;
            return;
        }
        else if (m_listeners.empty()) {
            ldout(cct,
                  20) << this << " " << __func__ << ": no listeners" << dendl;
            return;
        }
        ...
        C_RefreshTags *refresh_ctx =
            new C_RefreshTags(m_async_journal_op_tracker);
        refresh_ctx->on_finish =
            new LambdaContext([this, refresh_sequence, refresh_ctx] (int r) {
                              handle_refresh_metadata(refresh_sequence,
                                                      refresh_ctx->tag_tid,
                                                      refresh_ctx->tag_data,
                                                      r);});
        C_DecodeTags *decode_tags_ctx =
            new C_DecodeTags(cct, &refresh_ctx->lock, &refresh_ctx->tag_tid,
