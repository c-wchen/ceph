...
int BtrfsFileStoreBackend::detect_features()
{
    ...
    if (m_filestore_btrfs_clone_range) {
        ...
        if (fd >= 0) {
            if (::unlinkat(get_basedir_fd(), "clone_range_test", 0) < 0) {
                r = -errno;
                dout(0) <<
                    "detect_feature: failed to unlink test file for CLONE_RANGE ioctl: "
            ...
            if (r < 0 && errno == EBADF) {
                dout(0) << "detect_feature: CLONE_RANGE ioctl is supported" <<
                    dendl;
                ...
                r = -errno;
                dout(0) <<
                    "detect_feature: CLONE_RANGE ioctl is NOT supported: " <<
                    cpp_strerror(r) << dendl;
            }
            ...
            r = -errno;
            dout(0) <<
                "detect_feature: failed to create test file for CLONE_RANGE ioctl: "
                << cpp_strerror(r) << dendl;
        }
    }
    else {
        dout(0) <<
            "detect_feature: CLONE_RANGE ioctl is DISABLED via 'filestore btrfs clone range' option"
            << dendl;
    }
    ...
    if (srcfd < 0) {
        r = -errno;
        dout(0) << "detect_feature: failed to open " << vol_args.
    ...
    if (r == 0 || errno == EEXIST) {
        ...
        if (r == 0) {
            ...
            err = -errno;
            dout(0) << "detect_feature: SNAP_DESTROY failed: " <<
                cpp_strerror(err) << dendl;

            if (err == -EPERM && getuid() != 0) {
                dout(0) <<
                    "detect_feature: failed with EPERM as non-root; remount with -o user_subvol_rm_allowed"
                    << dendl;
                cerr << TEXT_YELLOW <<
                ...
                derr <<
                    "btrfs SNAP_DESTROY ioctl not supported; you need a kernel newer than 2.6.32"
                    << dendl;
            }
        }
    }
    ...
    if (m_filestore_btrfs_snap) {
        if (has_snap_destroy)
            ...
            dout(0) <<
                "detect_feature: snaps enabled, but no SNAP_DESTROY ioctl; DISABLING"
                << dendl;
    }
    ...
    if (r < 0) {
        int err = errno;
        dout(0) << "detect_feature: START_SYNC got " << cpp_strerror(err) <<
            dendl;
    }
    if (r == 0 && transid > 0) {
        dout(0) << "detect_feature: START_SYNC is supported (transid " <<
            transid << ")" << dendl;
    ...
    if (has_wait_sync) {
        ...
        if (::fstatat(get_basedir_fd(), vol_args.name, &st, 0) == 0) {
            ...
            if (r != 0) {
                int err = errno;
                dout(0) <<
                    "detect_feature: failed to remove old async_snap_test: " <<
                    cpp_strerror(err) << dendl;
            }
        }
    ...
    if (r < 0) {
        r = -errno;
        dout(0) << "detect_feature: failed to remove " << vol_args.
            name << ": " << cpp_strerror(r) << dendl;
    }

    if (m_filestore_btrfs_snap && !has_snap_create_v2) {
        dout(0) <<
            "mount WARNING: btrfs snaps enabled, but no SNAP_CREATE_V2 ioctl (from kernel 2.6.37+)"
            << dendl;
        cerr << TEXT_YELLOW <<
...
int BtrfsFileStoreBackend::create_current()
{
    ...
    if (ret == 0) {
        ...
        if (ret < 0) {
            ret = -errno;
            dout(0) << "create_current: cannot fstat basedir " <<
        ...
        if (ret < 0) {
            ret = -errno;
            dout(0) << "create_current: cannot statsf basedir " <<
                cpp_strerror(ret) << dendl;
            return ret;
        }
...
int BtrfsFileStoreBackend::list_checkpoints(list < string > &ls)
{
    ...
    if (ret < 0) {
        ret = -errno;
        dout(0) << "list_checkpoints: cannot fstat basedir " <<
            cpp_strerror(ret) << dendl;
        return ret;
    }
                                             ...
                                             uint64_t * transid)
{
    ...
    if (has_snap_create_v2 && transid) {
        ...
        if (r < 0) {
            r = -errno;
            dout(0) << "create_checkpoint: async snap create '" << name <<
                "' got " << cpp_strerror(r) << dendl;
            return r;
        }
        dout(20) << "create_checkpoint: async snap create '" << name <<
            "' transid " << async_args.transid << dendl;
...
{
    // wait for commit
    dout(10) << "sync_checkpoint: transid " << transid << " to complete" <<
        dendl;
    ...
    dout(20) << "sync_checkpoint: done waiting for transid " << transid <<
        dendl;
    return 0;
}
...
int BtrfsFileStoreBackend::rollback_to(const string & name)
{
    ...
    if (vol_args.fd < 0) {
        ret = -errno;
        dout(0) << "rollback_to: error opening '" << s << "': " <<
            cpp_strerror(ret) << dendl;
        return ret;
    }
                                       ...
                                       uint64_t len, uint64_t dstoff)
{
    ...
    if (err >= 0) {
        ...
        dout(20) <<
            "clone_range: failed CLONE_RANGE call with -EINVAL, using copy" <<
            dendl;
        return _copy_range(from, to, srcoff, len, dstoff);
    }
    else {
