                               ...
                               string * start, string * end)
{
    ...
    if (cid.is_pg(&pgid)) {
        ...
        *temp_start = *start;
        ...
        _key_encode_u64((-2ll - pgid.pool()) + 0x8000000000000000ull,
                        temp_start);
                               ...
                               S * key)
{
    ...
    size_t max_len = 1 + 8 + 4 + (oid.hobj.nspace.length() * 3 + 1) + (oid.hobj.get_key().length() * 3 + 1) + 1 +   // for '<', '=', or '>'
...
void BlueStore::GarbageCollector::process_protrusive_extents(const BlueStore::
                                                             ...
                                                             min_alloc_size)
{
    assert(start_offset <= start_touch_offset
        ...
        if (bi.referenced_bytes == 0) {
            ...
            if (benefit >= g_conf->bluestore_gc_enable_blob_threshold) {
                if (bi.collect_candidate) {
                    ...
                    do {
                        if (it->blob.get() == b) {
                            extents_to_collect.emplace_back(it->logical_offset,
                                                            it->length);
                        }
    ...
    while (num > 0) {
        ...
        if (refs > 1) {
            ...
            if (++skipped >= max_skipped) {
                dout(20) << __func__ <<
                    " maximum skip pinned reached; stopping with " << num <<
                    " left to trim" << dendl;
                break;
            }
...
{
    dout(20) << __func__ << " onodes " << onode_lru.size() << " / " << onode_max
        << " buffers " << buffer_bytes << " / " << buffer_max << dendl;
    ...
    if (buffer_bytes > buffer_max) {
        ...
        if (buffer_num) {
            ...
            kout =
                ...
                cct->_conf->bluestore_2q_cache_kout_ratio;
        }
    ...
    while (num > 0) {
        ...
        dout(30) << __func__ << " " << o->
            oid << " num=" << num << " lru size=" << onode_lru.size() << dendl;
        if (p != onode_lru.begin()) {
            onode_lru.erase(p--);
        }
        else {
...
void BlueStore::TwoQCache::_audit(const char *when)
{
    ...
    if (hot_bytes != buffer_list_bytes[BUFFER_HOT]) {
        derr << __func__ << " hot_list_bytes " << buffer_list_bytes[BUFFER_HOT]
...
int BlueStore::BufferSpace::_discard(Cache * cache, uint32_t offset,
                                     uint32_t length)
{
    // note: we already hold cache->lock
    ldout(cache->cct,
...
void BlueStore::BufferSpace::read(Cache * cache,
                                  uint32_t offset,
                                  uint32_t length,
...
void BlueStore::BufferSpace::split(Cache * cache, size_t pos,
    ...
    while (true) {
        ...
        if (p->second->data.length()) {
            ...
            r._add_buffer(cache,
                          new Buffer(&r, p->second->state, p->second->seq,
                                     p->second->offset - pos,
                                     p->second->length), 0, p->second.get());
        }
...
void BlueStore::OnodeSpace::rename(OnodeRef & oldo,
                                   ...
                                   const mempool::bluestore_cache_other::
                                   string & new_okey)
{
    std::lock_guard < std::recursive_mutex > l(cache->lock);
...
void BlueStore::Blob::discard_unallocated(Collection * coll)
{
    ...
    if (get_blob().is_compressed()) {
      ...
      for (auto e:get_blob().get_extents()) {
            if (!e.is_valid()) {
                ldout(coll->store->cct,
                      20) << __func__ << " 0x" << std::hex << pos << "~" << e.
                              ...
                              uint32_t length, PExtentVector * r)
{
    ...
    bool empty = used_in_blob.put(offset,
                                  length,
                                  &logical);
    r->clear();
    // nothing to release
...
void BlueStore::Blob::decode(Collection * coll,
...
void BlueStore::ExtentMap::reshard(KeyValueDB * db, KeyValueDB::Transaction t)
{
  ...
  for (auto & p:spanning_blob_map) {
        dout(20) << __func__ << "   spanning blob " << p.first << " " << *p.
            second << dendl;
    }
    ...
    for (unsigned i = si_begin; i < si_end; ++i) {
        generate_extent_shard_key_and_apply(onode->key,
                                            shards[i].shard_info->offset, &key,
                                            [&](const string & final_key) {
                      ...
                      for (const auto & sh:shards) {
                            if (bstart < sh.shard_info->offset &&
                                ...
                                if (b->can_split_at(blob_offset)) {
                                    dout(20) << __func__ <<
                                        "    splitting blob, bstart 0x" << std::
                                        hex << bstart << " blob_offset 0x" <<
                                        blob_offset << std::
                                        dec << " " << *b << dendl;
...
void BlueStore::ExtentMap::decode_spanning_blobs(bufferptr::iterator & p)
{
    __u8 struct_v;
    denc(struct_v, p);
    // Version 2 differs from v1 in blob's ref_map
...
void BlueStore::ExtentMap::dirty_range(uint32_t offset, uint32_t length)
{
    ...
    while (start <= last) {
        ...
        auto p = &shards[start];
        if (!p->loaded) {
            dout(20) << __func__ << " shard 0x" << std::hex << p->shard_info->
                offset << std::
                dec << " is not loaded, can't mark dirty" << dendl;
...
seek_lextent(uint64_t offset) const const
{
    Extent dummy(offset);
    auto fp = extent_map.lower_bound(dummy);
    if (fp != extent_map.begin()) {
    ...
    while (p != extent_map.end()) {
        ...
        if (p->logical_offset < offset) {
            if (p->logical_end() > end) {
                ...
                OldExtent *oe =
                    OldExtent::create(c, offset, p->blob_offset + front,
                                      length, p->blob);
                                             ...
                                             bufferlist::const_iterator & blp)
{
    ...
    i.first->second.bl.
        reassign_to_mempool(mempool::mempool_bluestore_writing_deferred);
        ...
        if (end > offset) {
            ...
            if (end > offset + length) {
                ...
                dout(20) << __func__ << "  keep tail " << p->second.seq
                    << " 0x" << std::hex << p->first << "~" << p->second.bl.
                    length()
...
cache(c),
cid(cid),
lock("BlueStore::Collection::lock", true, false), exists(true), onode_map(c)
{
}
...
void BlueStore::Collection::split_cache(Collection * dest)
{
    ...
    std::lock_guard < std::recursive_mutex > l2(dest->cache->lock,
                                                std::adopt_lock);
    ...
    while (p != onode_map.onode_map.end()) {
        if (!p->second->oid.match(destbits, destpg.pgid.ps())) {
            ...
            OnodeRef o = p->second;
            ldout(store->cct,
                  20) << __func__ << " moving " << o << " " << o->oid << dendl;
...
void BlueStore::MempoolThread::_trim_shards(bool interval_stats)
{
    ...
    uint64_t max_shard_onodes = static_cast < uint64_t > ((meta_alloc /
                                                           (double)num_shards) /
                                                          meta_cache.
                                                          get_bytes_per_onode
                                                          ());
    uint64_t max_shard_buffer =
        static_cast < uint64_t > (data_alloc / num_shards);
...
void BlueStore::MempoolThread::_balance_cache(const std::list <
                                              ...
                                              >&caches)
{
    ...
    for (int i = 0; i < PriorityCache::Priority::LAST + 1; i++) {
        ldout(store->cct,
              10) << __func__ << " assigning cache bytes for PRI: " << i <<
            dendl;
    ...
    if (mem_avail > 0) {
        for (auto it = caches.begin(); it != caches.end(); it++) {
            ...
            if (fair_share > 0) {
                (*it)->add_cache_bytes(PriorityCache::Priority::LAST,
                                       fair_share);
            }
        }
    }
                                                  ...
                                                  PriorityCache::Priority pri)
{
    ...
    while (!tmp_caches.empty()
        ...
        for (auto it = tmp_caches.begin(); it != tmp_caches.end();) {
            int64_t cache_wants =
                (*it)->request_cache_bytes(pri,
                                           store->cache_autotune_chunk_size);
            ...
            if (cache_wants > fair_share) {
                ...
                ldout(store->cct,
                      20) << __func__ << " " << (*it)->get_cache_name()
                    << " wanted: " << cache_wants << " fair_share: " <<
                    fair_share << " mem_avail: " << *mem_avail <<
                ...
                if (cache_wants > 0) {
                    ...
                    ldout(store->cct,
                        ...
                        << " wanted: " << cache_wants << " fair_share: " <<
                        fair_share << " mem_avail: " << *mem_avail <<
                        " removing from list.  New size: " << tmp_caches.
                        size() - 1 << dendl;
...
kv_sync_thread(this), kv_finalize_thread(this), mempool_thread(this)
{
    _init_logger();
...
BlueStore::BlueStore(CephContext * cct,
                     const string & path, uint64_t _min_alloc_size)
:ObjectStore(cct, path),
throttle_bytes(cct, "bluestore_throttle_bytes",
               cct->_conf->bluestore_throttle_bytes),
throttle_deferred_bytes(cct, "bluestore_throttle_deferred_bytes",
...
const char **BlueStore::get_tracked_conf_keys() const const
{
                                   ...
                                   const std::set < std::string > &changed)
{
    ...
    if (changed.count("bluestore_throttle_bytes")) {
        ...
        throttle_deferred_bytes.reset_max(conf->bluestore_throttle_bytes +
                                          conf->
                                          bluestore_throttle_deferred_bytes);
    }
...
int BlueStore::_set_cache_sizes()
{
    ...
    osd_memory_cache_min =
        cct->_conf->get_val < uint64_t > ("osd_memory_cache_min");
    ...
    dout(1) << __func__ << " cache_size " << cache_size
        ...
        << " kv " << cache_kv_ratio << " data " << cache_data_ratio << dendl;
    return 0;
}
...
int BlueStore::_reload_logger()
{
    ...
    if (r >= 0) {
        ...
        logger->set(l_bluestore_compressed_allocated,
                    store_statfs.compressed_allocated);
        logger->set(l_bluestore_compressed_original,
                    store_statfs.compressed_original);
    }
    return r;
}
                                ...
                                bluestore_bdev_label_t * label)
{
    ...
    try {
        ...
        dout(2) << __func__ << " unable to decode label at offset " << p.
            get_off()
            << ": " << e.what()
            << dendl;
        return -ENOENT;
...
void BlueStore::_set_alloc_sizes(void)
{
    ...
    dout(10) << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size
        ...
        << std::dec << " deferred_batch_ops " << deferred_batch_ops << dendl;
}
...
int BlueStore::_open_fm(bool create)
{
    ...
    if (create) {
        ...
        if (cct->_conf->bluestore_bluefs) {
            ...
            dout(20) << __func__ << " reserved 0x" << std::
                hex << reserved << std::dec << " for bluefs" << dendl;
            bufferlist bl;
            ::encode(bluefs_extents, bl);
            t->set(PREFIX_SUPER, "bluefs_extents", bl);
            dout(20) << __func__ << " bluefs_extents 0x" << std::
...
int BlueStore::_open_db(bool create)
{
    ...
    string options;
    stringstream err;
    ceph::shared_ptr < Int64ArrayMergeOperator >
    ...
    if (do_bluefs) {
        ...
        if (::stat(bfn.c_str(), &st) == 0) {
            ...
            if (create) {
                bluefs->add_block_extent(BlueFS::BDEV_DB,
                                         SUPER_RESERVED,
                                         bluefs->
                                         get_block_device_size(BlueFS::
                                                               BDEV_DB) -
        ...
        if (create) {
            ...
            if (cct->_conf->bluefs_alloc_size % min_alloc_size) {
                derr << __func__ << " bluefs_alloc_size 0x" << std::hex
                    ...
                    << "min_alloc_size 0x" << min_alloc_size << std::
                    dec << dendl;
                r = -EINVAL;
                goto free_bluefs;
            }
        ...
        if (::stat(bfn.c_str(), &st) == 0) {
            ...
            if (create) {
                bluefs->add_block_extent(BlueFS::BDEV_WAL,
        ...
        if (bluefs_shared_bdev == BlueFS::BDEV_SLOW) {
            ...
            db_paths << fn << "," << (uint64_t) (db_size * 95 /
                                                 100) << " " << fn +
                ".slow" << "," << (uint64_t) (slow_size * 95 / 100);
        ...
        if (create) {
            ...
            if (cct->_conf->get_val < std::string >
...
int BlueStore::_reconcile_bluefs_freespace()
{
    ...
    if (!super_extra.empty()) {
        ...
        for (interval_set < uint64_t >::iterator p = super_extra.begin();
             p != super_extra.end(); ++p) {
            bluefs->add_block_extent(bluefs_shared_bdev, p.get_start(),
                                     p.get_len());
        }
    }
...
int BlueStore::_balance_bluefs_freespace(PExtentVector * extents)
{
    ...
    dout(10) << __func__ << " bluefs " << byte_u_t(bluefs_free)
        << " free (" << bluefs_free_ratio << ") bluestore " << byte_u_t(my_free)
        << " free (" << my_free_ratio
        << "), bluefs_ratio " << bluefs_ratio << dendl;
    ...
    uint64_t min_free =
        cct->_conf->get_val < uint64_t > ("bluestore_bluefs_min_free");
    ...
    if (gift) {
        ...
        if (alloc_len <= 0) {
            ...
            dout(0) << __func__ << " insufficient allocate on 0x" << std::
                hex << gift << " min_alloc_size 0x" << min_alloc_size <<
                " allocated 0x" << alloc_len << std::dec << dendl;
            _dump_alloc_on_rebalance_failure();
        }
      for (auto & e:*extents) {
...
void BlueStore::
_commit_bluefs_freespace(const PExtentVector & bluefs_gift_extents)
{
    dout(10) << __func__ << dendl;
...
int BlueStore::_open_collections(int *errors)
{
    ...
    for (it->upper_bound(string()); it->valid(); it->next()) {
        coll_t cid;
        if (cid.parse(it->key())) {
            CollectionRef c(new Collection(this,
                                           cache_shards[cid.
                                                        hash_to_shard
                                                        (cache_shards.size())],
                                           cid));
...
int BlueStore::_fsck_check_extents(const ghobject_t & oid,
                                   ...
                                   store_statfs_t & expected_statfs)
{
  ...
  for (auto e:extents) {
        ...
        bool already = false;
        apply(e.offset, e.length, granularity, used_blocks,
              [&](uint64_t pos, mempool_dynamic_bitset & bs) {
              assert(pos < bs.size()); if (bs.test(pos))
              already = true;
              else
              bs.set(pos);}
        ) ;
...
{
    dout(1) << __func__ << (repair ? " fsck" : " repair")
    ...
    apply(0, MAX(min_alloc_size, SUPER_RESERVED), fm->get_alloc_size(),
          used_blocks,[&](uint64_t pos, mempool_dynamic_bitset & bs) {
          assert(pos < bs.size()); bs.set(pos);}
    );

    if (bluefs) {
        for (auto e = bluefs_extents.begin(); e != bluefs_extents.end(); ++e) {
            apply(e.get_start(), e.get_len(), fm->get_alloc_size(), used_blocks,
                  [&](uint64_t pos, mempool_dynamic_bitset & bs) {
                  assert(pos < bs.size()); bs.set(pos);}
            );
        }
        ...
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            if (is_extent_shard_key(it->key())) {
                ...
                while (expecting_shards.front() > it->key()) {
                    derr << "fsck error:   saw " << pretty_binary_string(it->
                                                                         key())
                        << dendl;
                    derr << "fsck error:   exp "
                        << pretty_binary_string(expecting_shards.
                                                front()) << dendl;
                    ++errors;
                    expecting_shards.pop_front();
                    if (expecting_shards.empty()) {
            ...
            if (!c ||
                ...
                oid.hobj.pool != (int64_t) pgid.pool() || !c->contains(oid)) {
                c = nullptr;
          ...
          for (auto & s:o->extent_map.shards) {
                ...
                if (s.shard_info->offset >= o->onode.size) {
                    derr << "fsck error: " << oid << " shard 0x" << std::hex
                        << s.shard_info->offset << " past EOF at 0x" << o->
                        onode.size << std::dec << dendl;
                    ++errors;
                }
            }
            // lextents
          ...
          for (auto & l:o->extent_map.extent_map) {
                ...
                if (l.logical_offset < pos) {
                    derr << "fsck error: " << oid << " lextent at 0x"
                        ...
                        << " overlaps with the previous, which ends at 0x" <<
                        pos << std::dec << dendl;
                    ++errors;
                }
                ...
                if (blob.has_unused()) {
                    ...
                    uint64_t end =
                        ROUND_UP_TO(l.blob_offset + l.length,
                    ...
                    for (unsigned p = 0; p < csum_count; ++p) {
                        ...
                        if ((blob.unused & mask) == mask) {
                            ...
                            if (blob.get_csum_item(p) != 0) {
                                derr << "fsck error: " << oid
                                    << " blob claims csum chunk 0x" << std::
                                    hex << pos << "~" << csum_chunk_size <<
                                    " is unused (mask 0x" << mask <<
                                    " of unused 0x" << blob.
                                    unused << ") but csum is non-zero 0x" <<
                                    blob.
                                    get_csum_item(p) << std::
                                    dec << " on blob " << *i.first << dendl;
                                ++errors;
                            }
                        }
                    }
          ...
          for (auto & i:ref_map) {
                ...
                if (!equal) {
                    derr << "fsck error: " << oid << " blob " << *i.first
                        << " doesn't match expected ref_map " << i.
                        second << dendl;
                    ++errors;
                }
                if (blob.is_compressed()) {
                    expected_statfs.compressed +=
                ...
                if (blob.is_shared()) {
                    if (i.first->shared_blob->get_sbid() > blobid_max) {
                        derr << "fsck error: " << oid << " blob " << blob
                            ...
                            get_sbid() << " > blobid_max " << blobid_max <<
                            dendl;
                        ++errors;
                    }
            ...
            if (deep) {
                ...
                do {
                    uint64_t l =
                        std::min(uint64_t(o->onode.size - offset),
                                 max_read_block);
            ...
            if (o->onode.has_omap()) {
                if (used_omap_head.count(o->onode.nid)) {
                    derr << "fsck error: " << oid << " omap_head " << o->onode.
    ...
    if (it) {
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            if (p == sb_info.end()) {
                ...
                dout(20) << __func__ << "  " << *sbi.
                    sb << " " << shared_blob << dendl;
  ...
  for (auto & p:sb_info) {
    ...
    if (it) {
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            for (auto e = wt.released.begin(); e != wt.released.end(); ++e) {
                apply(e.get_start(), e.get_len(), fm->get_alloc_size(),
    ...
    dout(1) << __func__ << " checking freelist vs allocated" << dendl;
    {
        ...
        for (auto e = bluefs_extents.begin(); e != bluefs_extents.end(); ++e) {
            apply(e.get_start(), e.get_len(), fm->get_alloc_size(), used_blocks,
                  [&](uint64_t pos, mempool_dynamic_bitset & bs) {
                  assert(pos < bs.size()); bs.reset(pos);}
            );
        }
        fm->enumerate_reset();
        uint64_t offset, length;
        while (fm->enumerate_next(&offset, &length)) {
            ...
            apply(offset, length, fm->get_alloc_size(), used_blocks,
                  [&](uint64_t pos, mempool_dynamic_bitset & bs) {
                  assert(pos < bs.size()); if (bs.test(pos)) {
                  intersects = true;}
                  else {
                ...
                while (true) {
                    ...
                    if (next != cur + 1) {
                        derr << "fsck error: leaked extent 0x" << std::hex
                            ...
                            << ((cur + 1 - start) *
                                fm->get_alloc_size()) << std::dec << dendl;
                        start = next;
                        break;
                    }
                    cur = next;
...
void BlueStore::collect_metadata(map < string, string > *pm)
{
    ...
    if (bluefs) {
        ...
        (*pm)["bluefs_single_shared_device"] =
            stringify((int)bluefs_single_shared_device);
...
int BlueStore::statfs(struct store_statfs_t *buf)
{
    ...
    if (bluefs) {
        ...
        int64_t shared_available =
            std::min(bluefs->get_free(bluefs_shared_bdev),
                     bluefs->get_total(bluefs_shared_bdev) -
...
void BlueStore::_reap_collections()
{
    ...
    while (p != removed_colls.end()) {
        ...
        if (c->onode_map.map_any([&](OnodeRef o) {
                                 ...
                                 cid << " " << o->oid << " flush_txns " << o->
                                 flushing_count << dendl; return true;}
                                 return false;}
            )) {
            ++p;
            continue;
...
int BlueStore::stat(const coll_t & cid,
...
int BlueStore::set_collection_opts(const coll_t & cid, const pool_opts_t & opts)
{
...
int BlueStore::read(const coll_t & cid,
                    ...
                    size_t length, bufferlist & bl, uint32_t op_flags)
{
    ...
    dout(15) << __func__ << " " << cid << " " << oid
        << " 0x" << std::hex << offset << "~" << length << std::dec << dendl;
    if (!c->exists)
        return -ENOENT;
    ...
    int r;
    {
        ...
        OnodeRef o = c->get_onode(oid, false);
        logger->tinc(l_bluestore_read_onode_meta_lat,
    ...
    if (r >= 0 && _debug_data_eio(oid)) {
        ...
        derr << __func__ << " " << c->
            cid << " " << oid << " INJECT EIO" << dendl;
             ...
             (rand() %
              (int)(cct->_conf->bluestore_debug_random_read_err * 100.0)) ==
             0) {
...
struct region_t {
    ...
    :logical_offset(from.logical_offset),
        blob_xoffset(from.blob_xoffset), length(from.length) {
    }
                        ...
                        uint32_t op_flags, uint64_t retry_count)
{
  ...
  for (auto & p:blobs2read) {
        ...
        if (bptr->get_blob().is_compressed()) {
            ...
            r = bptr->get_blob().map(0, bptr->get_blob().get_ondisk_length(),
                                     ...
                                     r =
                                     bdev->read(offset, length, &bl, &ioc,
                                                false);}
                                     if (r < 0)
                                     return r; return 0;}
            ) ;
          ...
          for (auto & reg:p.second) {
                ...
                // read it
                r = bptr->get_blob().map(reg.r_off, r_len,
                                         [&](uint64_t offset, uint64_t length) {
                                         int r;
                                         // use aio if there is more than one region to read
                                         if (num_regions > 1) {
                                         r =
                                         bdev->aio_read(offset, length, &reg.bl,
                                                        &ioc);}
                                         else {
                                         r =
                                         bdev->read(offset, length, &reg.bl,
                                                    &ioc, false);}
                                         if (r < 0)
                                         return r; return 0;}
                ) ;
    ...
    while (b2r_it != blobs2read.end()) {
        ...
        if (bptr->get_blob().is_compressed()) {
          ...
          for (auto & i:b2r_it->second) {
                ready_regions[i.logical_offset].substr_of(raw_bl,
                                                          i.blob_xoffset,
                                                          i.length);
            }
        }
          ...
          for (auto & reg:b2r_it->second) {
                ...
                if (buffered) {
                    bptr->shared_blob->bc.did_read(bptr->shared_blob->
                                                   get_cache(), reg.r_off,
                                                   reg.bl);
                }

                // prune and keep result
                ready_regions[reg.logical_offset].substr_of(reg.bl, reg.front,
                                                            reg.length);
            }
        }
        ++b2r_it;
...
int BlueStore::_verify_csum(OnodeRef & o,
                            ...
                            uint64_t logical_offset) const const
{
    int bad;
    uint64_t bad_csum;
    utime_t start = ceph_clock_now();
...
int BlueStore::_fiemap(CollectionHandle & c_,
                       const ghobject_t & oid,
                       uint64_t offset,
                       size_t length, interval_set < uint64_t > &destset)
{
    Collection *c = static_cast < Collection * >(c_.get());
    if (!c->exists)
        return -ENOENT;
        ...
        while (length > 0) {
            ...
            if (ep != eend && ep->logical_offset <= offset) {
                ...
                dout(30) << __func__ << " lextent 0x" << std::
                    hex << offset << "~" << x_len << std::
                    dec << " blob " << ep->blob << dendl;
...
int BlueStore::fiemap(const coll_t & cid,
                      const ghobject_t & oid,
...
int BlueStore::fiemap(CollectionHandle & c_,
                      const ghobject_t & oid,
                      uint64_t offset,
                      size_t length, map < uint64_t, uint64_t > &destmap)
{
    interval_set < uint64_t > m;
    int r = _fiemap(c_, oid, offset, length, m);
    if (r >= 0) {
...
int BlueStore::getattr(const coll_t & cid,
                       const ghobject_t & oid,
...
int BlueStore::getattrs(const coll_t & cid,
                        const ghobject_t & oid, map < string, bufferptr > &aset)
{
    CollectionHandle c = _get_collection(cid);
    if (!c)
        return -ENOENT;
...
int BlueStore::collection_list(const coll_t & cid, const ghobject_t & start,
                                ...
                                vector < ghobject_t > *ls, ghobject_t * pnext)
{
    ...
    it = db->get_iterator(PREFIX_OBJ);
    if (start == ghobject_t() ||
        start.hobj == hobject_t() || start == c->cid.get_min_hobj()) {
    ...
    while (true) {
        if (!it->valid() || it->key() >= pend) {
            if (!it->valid())
                ...
                dout(20) << __func__ << " key " << pretty_binary_string(it->
                                                                        key())
                    << " >= " << end << dendl;
            if (temp) {
                if (end.hobj.is_temp()) {
                ...
                pend = end_key;
                dout(30) << __func__ << " pend " << pretty_binary_string(pend)
                    << dendl;
                continue;
            }
            break;
...
int BlueStore::omap_get(const coll_t & cid, ///< [in] Collection containing oid
        ...
        while (it->valid()) {
            if (it->key() == head) {
                ...
                *header = it->value();
                ...
                dout(30) << __func__ << "  got " << pretty_binary_string(it->
                                                                         key())
                    << " -> " << user_key << dendl;
...
int BlueStore::omap_get_header(const coll_t & cid,  ///< [in] Collection containing oid
                               const ghobject_t & oid,  ///< [in] Object containing omap
                               bufferlist * header, ///< [out] omap header
...
int BlueStore::omap_get_keys(const coll_t & cid,    ///< [in] Collection containing oid
                             const ghobject_t & oid,    ///< [in] Object containing omap
...
int BlueStore::omap_get_values(const coll_t & cid,  ///< [in] Collection containing oid
                               const ghobject_t & oid,  ///< [in] Object containing omap
...
int BlueStore::omap_check_keys(const coll_t & cid,  ///< [in] Collection containing oid
                               const ghobject_t & oid,  ///< [in] Object containing omap
...
ObjectMap::ObjectMapIterator BlueStore::get_omap_iterator(const coll_t & cid,   ///< [in] collection
...
int BlueStore::_open_super_meta()
{
    ...
    if (cct->_conf->bluestore_bluefs) {
        ...
        dout(10) << __func__ << " bluefs_extents 0x" << std::
            hex << bluefs_extents << std::dec << dendl;
    }
...
void BlueStore::_txc_finish_io(TransContext * txc)
{
    ...
    if (osr->kv_submitted_waiters && osr->_is_all_kv_submitted()) {
...
void BlueStore::_txc_write_nodes(TransContext * txc, KeyValueDB::Transaction t)
{
  ...
  for (auto o:txc->onodes) {
        ...
        dout(20) << "  onode " << o->oid << " is " << bl.length()
            ...
            << extent_part << " bytes inline extents)" << dendl;
...
{
    os_commit_latency.consume_next(logger.get_tavg_ms(l_bluestore_commit_lat));
    os_apply_latency.consume_next(logger.get_tavg_ms(l_bluestore_commit_lat));
}
...
void BlueStore::_txc_finalize_kv(TransContext * txc, KeyValueDB::Transaction t)
{
    dout(20) << __func__ << " txc " << txc << std::hex
        ...
        << " released 0x" << txc->released << std::dec << dendl;
    ...
    while (true) {
        ...
        if (kv_queue.empty() &&
            ((deferred_done_queue.empty() && deferred_stable_queue.empty()) ||
             !deferred_aggressive) && (bluefs_do_check_balance == false)) {
            if (kv_stop)
                break;
              ...
              for (auto & txc:b->txcs) {
                    ...
                    if (!wt.released.empty()) {
                        ...
                        dout(10) << __func__ << " deferred txn has released "
                            << txc.released
                            << " (we just upgraded from kraken) on " << &txc <<
                            dendl;
...

            {
                ...
                if (deferred_stable_to_finalize.empty()) {
                    ...
                    deferred_stable_to_finalize.
                        insert(deferred_stable_to_finalize.end(),
...
int BlueStore::queue_transactions(Sequencer * posr,
        ...
        case Transaction::OP_COLL_HINT:
            {
                ...
                if (type == Transaction::COLL_HINT_EXPECTED_NUM_OBJECTS) {
                    ...
                    dout(10) << __func__ <<
                        " collection hint objects is a no-op, " << " pg_num " <<
                        pg_num << " num_objects " << num_objs << dendl;
                }
                else {
                    // Ignore the hint
...
void BlueStore::_dump_extent_map(ExtentMap & em, int log_level)
{
  ...
  for (auto & e:em.extent_map) {
        ...
        if (blob.has_csum()) {
            ...
            dout(log_level) << __func__ << "      csum: " << std::
                hex << v << std::dec << dendl;
        }
        std::lock_guard < std::recursive_mutex >
            l(e.blob->shared_blob->get_cache()->lock);
    ...
    do {
        ...
        if (ep != end && ep->logical_offset < offset + max_bsize) {
            ...
            if (bstart >= end_offs) {
                ...
                if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) &&
                    ...
                    Extent *le =
                        o->extent_map.set_lextent(c, offset, b_off + head_pad,
                                                  length,
                                                  b,
                                                  &wctx->old_extents);
                ...
                if (b->get_blob().get_ondisk_length() >= b_off + b_len &&
                    ...
                    if (head_read) {
                        bufferlist head_bl;
                        int r =
                            _do_read(c.get(), o, offset - head_pad - head_read,
                                     head_read,
                                     head_bl, 0);
                    ...
                    if (tail_read) {
                        bufferlist tail_bl;
                        int r =
                            _do_read(c.get(), o, offset + length + tail_pad,
                                     tail_read,
                                     tail_bl, 0);
                    ...
                    int r = b->get_blob().map(b_off, b_len,
                                              [&](uint64_t offset,
                                                  uint64_t length) {
                                              op->extents.
                                              emplace_back(bluestore_pextent_t
                    ...
                    dout(20) << __func__ << "  deferred write 0x" << std::
                        hex << b_off << "~" << b_len << std::
                        dec << " of mutable " << *b << " at " << op->
                        extents << dendl;
                    Extent *le =
                        o->extent_map.set_lextent(c, offset, offset - bstart,
                                                  length,
                                                  b, &wctx->old_extents);
                    b->dirty_blob().mark_used(le->blob_offset, le->length);
                    txc->statfs_delta.stored() += le->length;
        ...
        if (prev_ep != end && prev_ep->logical_offset >= min_off) {
            ...
            dout(20) << __func__ << " considering " << *b
                << " bstart 0x" << std::hex << bstart << std::dec << dendl;
            if (b->can_reuse_blob(min_alloc_size,
                                  max_bsize, offset0 - bstart, &alloc_len)) {
                assert(alloc_len == min_alloc_size);    // expecting data always
                // fit into reused blob
                // Need to check for pending writes desiring to
                // reuse the same pextent. The rationale is that during GC two chunks
                               ...
                               OnodeRef o, WriteContext * wctx)
{
    ...
    if (wctx->compress) {
        c = select_option("compression_algorithm", compressor,[&]() {
                          string val;
                          if (coll->pool_opts.
                              get(pool_opts_t::COMPRESSION_ALGORITHM, &val)) {
                          CompressorRef cp = compressor;
  ...
  for (auto & wi:wctx->writes) {
        if (c && wi.blob_length > min_alloc_size) {
            ...
            logger->tinc(l_bluestore_compress_lat, ceph_clock_now() - start);
        }
        else {
            need += wi.blob_length;
        }
    ...
    if (prealloc_left < 0) {
        derr << __func__ << " failed to allocate 0x" << std::
            hex << need << std::dec << dendl;
        return -ENOSPC;
    }
        ...
        if (wi.compressed) {
            ...
            if (csum != Checksummer::CSUM_NONE) {
                dout(20) << __func__ << " initialize csum setting for new blob "
                    << *b << " csum_type " << Checksummer::
                    get_csum_type_string(csum)
        ...
        if (!g_conf->bluestore_debug_omit_block_device_write) {
            if (l->length() <= prefer_deferred_size.load()) {
                dout(20) << __func__ << " deferring small 0x" << std::hex
                    << l->
                    length() << std::dec << " write via deferred" << dendl;
                bluestore_deferred_op_t *op = _get_deferred_op(txc, o);
                op->op = bluestore_deferred_op_t::OP_WRITE;
                int r = b->get_blob().map(b_off, l->length(),
                                          [&](uint64_t offset, uint64_t length) {
                                          op->extents.
                                          emplace_back(bluestore_pextent_t
                                                       (offset, length));
                                      ...
                                      WriteContext * wctx)
{
    ...
    auto cm = select_option("compression_mode",
                            ...
                            string val;
                            if (c->pool_opts.
                                get(pool_opts_t::COMPRESSION_MODE, &val)) {
    ...
    if ((alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ) &&
        ...
        if (wctx->compress) {
            wctx->target_blob_size = select_option("compression_max_blob_size",
        ...
        if (wctx->compress) {
            wctx->target_blob_size = select_option("compression_min_blob_size",
...
int BlueStore::_do_gc(TransContext * txc,
                      ...
                      uint64_t * dirty_start, uint64_t * dirty_end)
{
                         ...
                         bufferlist & bl, uint32_t fadvise_flags)
{
    ...
    benefit = gc.estimate(offset,
                          length,
                          o->extent_map, wctx.old_extents, min_alloc_size);
...
int BlueStore::_truncate(TransContext * txc,
                         CollectionRef & c, OnodeRef & o, uint64_t offset)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid
        << " 0x" << std::hex << offset << std::dec << dendl;
    int r = 0;
    ...
    dout(10) << __func__ << " " << c->cid << " " << o->oid
        << " 0x" << std::hex << offset << std::dec << " = " << r << dendl;
    return r;
}
...
int BlueStore::_setattr(TransContext * txc,
                        CollectionRef & c,
                        OnodeRef & o, const string & name, bufferptr & val)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid
        << " " << name << " (" << val.length() << " bytes)" << dendl;
    int r = 0;
    if (val.is_partial()) {
...
int BlueStore::_setattrs(TransContext * txc,
                         CollectionRef & c,
                         OnodeRef & o, const map < string, bufferptr > &aset)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid
...
int BlueStore::_rmattr(TransContext * txc,
                       CollectionRef & c, OnodeRef & o, const string & name)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid
        << " " << name << dendl;
    int r = 0;
...
void BlueStore::_do_omap_clear(TransContext * txc, uint64_t id)
{
    ...
    while (it->valid()) {
        ...
        dout(30) << __func__ << "  rm " << pretty_binary_string(it->
                                                                key()) << dendl;
        it->next();
    }
}
...
int BlueStore::_omap_setkeys(TransContext * txc,
                             CollectionRef & c, OnodeRef & o, bufferlist & bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r;
    bufferlist::iterator p = bl.begin();
                                 ...
                                 const string & first, const string & last)
{
    ...
    while (it->valid()) {
        if (it->key() >= key_last) {
            dout(30) << __func__ << "  stop at " <<
                pretty_binary_string(key_last)
                << dendl;
            break;
        }
        txc->t->rmkey(PREFIX_OMAP, it->key());
                       ...
                       OnodeRef & newo, const ghobject_t & new_oid)
{
    ...
    // rewrite shards
    {
      ...
      for (auto & s:oldo->extent_map.shards) {
            generate_extent_shard_key_and_apply(oldo->key, s.shard_info->offset,
                                                &key,
                                                [&](const string & final_key) {
                                                txc->t->rmkey(PREFIX_OBJ,
                                                              final_key);}
...
void BlueStore::DBHistogram::update_hist_entry(map < string, map < int,
                                               ...
                                               size_t value_size)
{
    ...
    key_hist[prefix][key_slab].max_len =
        MAX(key_size, key_hist[prefix][key_slab].max_len);
...
void BlueStore::_apply_padding(uint64_t head_pad,
                               uint64_t tail_pad, bufferlist & padded)
{
    if (head_pad) {
