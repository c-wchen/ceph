    ...
    namespace cache {
        namespace pwl {
            namespace ssd {
                ...
                void WriteLogEntry::init_cache_bl(bufferlist & src_bl,
                    ...
                    *out = cache_bl;
                    ...
                    if (cache_bl.length()) {
                        return round_up_to(cache_bl.length(),
                                           MIN_WRITE_ALLOC_SSD_SIZE);
                    } return round_up_to(write_bytes(),
                                         MIN_WRITE_ALLOC_SSD_SIZE);
                }

                void WriteLogEntry::writeback_bl(librbd::cache::
                ...
                void WriteSameLogEntry::writeback_bl(librbd::cache::
                                                     ImageWritebackInterface &
                                                     image_writeback,
                                                     Context * ctx,
                                                     ceph::bufferlist && bl) {
