...
:  
thread_id(0),
pid(0), ioprio_class(-1), ioprio_priority(-1), cpuid(-1), thread_name(NULL)
{
}
...
void *Thread::entry_wrapper()
{
    ...
    if (p > 0)
        pid = p;
    if (pid && ioprio_class >= 0 && ioprio_priority >= 0) {
        ceph_ioprio_set(IOPRIO_WHO_PROCESS,
                        pid, IOPRIO_PRIO_VALUE(ioprio_class, ioprio_priority));
    }
    if (pid && cpuid >= 0)
        _set_affinity(cpuid);
...
bool Thread::is_started() const const
{
    return thread_id != 0;
}

bool Thread::am_self() const const
{
...
int Thread::set_ioprio(int cls, int prio)
{
    ...
    if (pid && cls >= 0 && prio >= 0)
        return ceph_ioprio_set(IOPRIO_WHO_PROCESS,
                               pid, IOPRIO_PRIO_VALUE(cls, prio));
    return 0;
}
