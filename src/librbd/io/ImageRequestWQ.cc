    ...
    namespace io {
        ...
        template < typename I >
            ...
            ImageRequestWQ *work_queue;
             ...
             C_AcquireLock(ImageRequestWQ * work_queue,
                           ImageRequest < I > *image_request)
        ...
        template < typename I >
            ImageRequestWQ < I >::ImageRequestWQ(I * image_ctx,
        ...
        template < typename I >
            ssize_t ImageRequestWQ < I >::compare_and_write(uint64_t off,
                                                            uint64_t len,
                                                            bufferlist
                                                            && cmp_bl,
                                                            bufferlist
            ...
            CephContext *cct = m_image_ctx.cct;
            ldout(cct,
                  20) << "compare_and_write ictx=" << &m_image_ctx << ", off="
                << off << ", " << "len = " << len << dendl;
        ...
        template < typename I >
            void ImageRequestWQ < I >::aio_read(AioCompletion * c, uint64_t off,
                                                uint64_t len, ReadResult
                                                && read_result, int op_flags,
                                                bool native_async) {
            CephContext *cct = m_image_ctx.cct;
            ZTracer::Trace trace;
            ...
            if (m_image_ctx.non_blocking_aio || writes_blocked()
                || !writes_empty() || require_lock_on_read()) {
        ...
        template < typename I >
            void ImageRequestWQ < I >::aio_write(AioCompletion * c,
            ...
            RWLock::RLocker owner_locker(m_image_ctx.owner_lock);
            if (m_image_ctx.non_blocking_aio || writes_blocked()) {
                queue(ImageRequest < I >::create_write_request(m_image_ctx, c, { {
                                                               off, len}
                                                               }
                                                               , std::move(bl),
                                                               op_flags,
                ...
                c->start_op();
                ImageRequest < I >::aio_write(&m_image_ctx, c, { {
                                              off, len}
                                              }
        ...
        template < typename I >
            void ImageRequestWQ < I >::aio_discard(AioCompletion * c,
                                                   uint64_t off, uint64_t len,
                                                   bool skip_partial_discard,
                                                   bool native_async) {
            CephContext *cct = m_image_ctx.cct;
            ...
            if (m_image_ctx.non_blocking_aio || writes_blocked()) {
                queue(ImageRequest <
                      I >::create_discard_request(m_image_ctx, c, off, len,
        ...
        template < typename I >
            void ImageRequestWQ < I >::aio_flush(AioCompletion * c,
                                                 bool native_async) {
            CephContext *cct = m_image_ctx.cct;
            ZTracer::Trace trace;
            if (m_image_ctx.blkin_trace_all) {
            ...
            if (m_image_ctx.non_blocking_aio || writes_blocked()
                || !writes_empty()) {
                queue(ImageRequest <
                      I >::create_flush_request(m_image_ctx, c, trace));
            }
            else {
        ...
        template < typename I >
            void ImageRequestWQ < I >::aio_writesame(AioCompletion * c,
                                                     uint64_t off, uint64_t len,
                                                     bufferlist
                                                     && bl, int op_flags,
                                                     bool native_async) {
            ...
            if (m_image_ctx.non_blocking_aio || writes_blocked()) {
                queue(ImageRequest <
                      I >::create_writesame_request(m_image_ctx, c, off, len,
                                                    std::move(bl), op_flags,
                                                    trace));
            }
            else {
                c->start_op();
                ImageRequest < I >::aio_writesame(&m_image_ctx, c, off, len,
                                                  std::move(bl), op_flags,
                                                  trace);
                finish_in_flight_io();
            }
                                                             ...
                                                             bool native_async)
        {
            ...
            ZTracer::Trace trace;
            if (m_image_ctx.blkin_trace_all) {
                trace.init("wq: compare_and_write",
                           &m_image_ctx.trace_endpoint);
                trace.event("init");
            }

            c->init_time(util::get_image_ctx(&m_image_ctx),
                         AIO_TYPE_COMPARE_AND_WRITE);
            ldout(cct,
                  20) << "ictx=" << &m_image_ctx << ", " << "completion=" << c
            ...
            if (m_image_ctx.non_blocking_aio || writes_blocked()) {
                queue(ImageRequest <
                      I >::create_compare_and_write_request(m_image_ctx, c, { {
                                                            off, len}
                                                            }
                                                            , std::move(cmp_bl),
                                                            std::move(bl),
                ...
                c->start_op();
                ImageRequest < I >::aio_compare_and_write(&m_image_ctx, c, { {
                                                          off, len}
                                                          }
                                                          ,
                                                          std::move(cmp_bl),
        ...
        template < typename I >
...

            {
                ...
                CephContext *cct = m_image_ctx.cct;
                ldout(cct,
                      5) << __func__ << ": in_flight=" << m_in_flight_ios.load()
                    << dendl;
        ...
        template < typename I > int ImageRequestWQ < I >::block_writes() {
        ...
        template < typename I > void ImageRequestWQ < I >::unblock_writes() {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void ImageRequestWQ < I >::set_require_lock(Direction direction,
                                                        bool enabled) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 20) << dendl;
        ...
        template < typename I > void *ImageRequestWQ < I >::_void_dequeue() {
            CephContext *cct = m_image_ctx.cct;
            ...
            ImageRequest < I > *item =
                reinterpret_cast < ImageRequest < I > *>(ThreadPool::PointerWQ <
                                                         ImageRequest < I >
                                                         >::_void_dequeue());
            ...
            if (lock_required) {
                ...
                if (m_image_ctx.exclusive_lock != nullptr) {
                    ldout(cct,
                        ...
                        << dendl;
                    if (!m_image_ctx.get_exclusive_lock_policy()->
                        may_auto_request_lock()) {
                        lderr(cct) << "op requires exclusive lock" << dendl;
                        fail_in_flight_io(m_image_ctx.exclusive_lock->
                                          get_unlocked_op_error(), item);
        ...
        template < typename I >
            void ImageRequestWQ < I >::finish_queued_io(ImageRequest < I >
                                                        *req) {
            RWLock::RLocker locker(m_lock);
            if (req->is_write_op()) {
        ...
        template < typename I > void ImageRequestWQ < I >::finish_in_flight_io() {
            Context *on_shutdown;
            {
                RWLock::RLocker locker(m_lock);
        ...
        template < typename I >
            void ImageRequestWQ < I >::fail_in_flight_io(int r,
                                                         ImageRequest < I >
                                                         *req) {
            this->process_finish();
            req->fail(r);
        ...
        template < typename I >
            void ImageRequestWQ < I >::handle_acquire_lock(int r,
                                                           ImageRequest < I >
                                                           *req) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void ImageRequestWQ < I >::handle_refreshed(int r,
                                                        ImageRequest < I >
                                                        *req) {
            CephContext *cct = m_image_ctx.cct;
