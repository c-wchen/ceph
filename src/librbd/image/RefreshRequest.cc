    ...
    namespace image {
        ...
        template < typename I >
            ...
            if (*result == 0) {
                ...
                *result =
                    cls_client::old_snapshot_list_finish(&it, &m_snap_names,
                                                         &m_snap_sizes,
                                                         &m_snapc);
            }
            ...
            m_snap_namespaces =
                std::vector < cls::rbd::SnapshotNamespace >
                (m_snap_names.size(), cls::rbd::UserSnapshotNamespace());

            m_snap_timestamps =
                std::vector < utime_t > (m_snap_names.size(), utime_t());
        ...
        template < typename I >
            ...
            if (*result == 0) {
                bufferlist::iterator it = m_out_bl.begin();
                *result =
                    cls_client::get_mutable_metadata_finish(&it, &m_size,
                                                            &m_features,
        ...
        template < typename I >
            ...
            librados::ObjectReadOperation op;
            cls_client::metadata_list_start(&op, m_last_metadata_key,
                                            MAX_METADATA_ITEMS);

            using klass = RefreshRequest < I >;
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_v2_get_metadata > (this);
            m_out_bl.clear();
        ...
        template < typename I > void RefreshRequest < I >::send_v2_get_flags() {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 10) << this << " " << __func__ << dendl;
        ...
        template < typename I >
            ...
            if (*result == 0) {
                bufferlist::iterator it = m_out_bl.begin();
                cls_client::get_flags_finish(&it, &m_flags, m_snapc.snaps,
                                             &m_snap_flags);
            }
            if (*result == -EOPNOTSUPP) {
                ...
                *result = 0;
                ldout(cct,
                      10) <<
                    "OSD does not support RBD flags, disabling object map " <<
                    "optimizations" << dendl;
                ...
                std::vector < uint64_t > default_flags(m_snapc.snaps.size(),
                                                       m_flags);
                ...
                lderr(cct) << "failed to retrieve flags: " <<
                    cpp_strerror(*result)
                    << dendl;
                return m_on_finish;
            }
        ...
        template < typename I >
            Context * RefreshRequest <
            I >::handle_v2_get_snap_timestamps(int *result) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": " << "r=" << *result <<
                dendl;
        ...
        template < typename I >
            Context * RefreshRequest <
            I >::handle_v2_get_snap_namespaces(int *result) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 10) << this << " " << __func__ << ": "
                << "r=" << *result << dendl;
            ...
            if (*result == -ENOENT) {
                ...
                m_snap_namespaces = std::vector
                    < cls::rbd::SnapshotNamespace > (m_snap_names.size(),
                                                     cls::rbd::
        ...
        template < typename I >
            ...
            if (journal_disabled || journal_disabled_by_policy) {
                ...
                if ((m_features & RBD_FEATURE_JOURNALING) != 0 &&
                    ...
                    m_image_ctx.exclusive_lock != nullptr &&
                    m_image_ctx.journal == nullptr) {
                    m_image_ctx.io_work_queue->
                        set_require_lock(librbd::io::DIRECTION_BOTH, true);
                }
        ...
        template < typename I >
            ...
            if (m_image_ctx.snap_name.empty()) {
                ...
                for (size_t snap_idx = 0; snap_idx < m_snap_names.size();
                     ++snap_idx) {
        ...
        template < typename I > void RefreshRequest < I >::apply() {
...

            {
                ...
                for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
                    std::vector < librados::snap_t >::const_iterator it =
                        std::find(m_image_ctx.snaps.begin(),
                                  m_image_ctx.snaps.end(),
                                  m_snapc.snaps[i].val);
                    if (it == m_image_ctx.snaps.end()) {
                        m_flush_aio = true;
                        ldout(cct,
                              20) << "new snapshot id=" << m_snapc.snaps[i].
                            val << " name=" << m_snap_names[i]
                            << " size=" << m_snap_sizes[i]
                            << dendl;
                    }
                }
                ...
                for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
                    uint64_t flags =
                        m_image_ctx.old_format ? 0 : m_snap_flags[i];
                    uint8_t protection_status =
                        m_image_ctx.old_format ? static_cast < uint8_t >
                    ...
                    m_image_ctx.add_snap(m_snap_namespaces[i], m_snap_names[i],
                                         m_snapc.snaps[i].val, m_snap_sizes[i],
                ...
                if (m_image_ctx.exclusive_lock != nullptr &&
                    ...
                    if (!m_image_ctx.test_features(RBD_FEATURE_JOURNALING,
                        ...
                        if (!m_image_ctx.clone_copy_on_read
                            && m_image_ctx.journal != nullptr) {
                            m_image_ctx.io_work_queue->
                                set_require_lock(io::DIRECTION_READ, false);
                        }
