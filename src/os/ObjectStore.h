...
class ObjectStore {
   ...
   */
    static ObjectStore *create(CephContext * cct,
                               const string & type,
                               const string & data,
                               const string & journal, osflagbits_t flags = 0);
   ...
   */
    static int probe_block_device_fsid(CephContext * cct,
   ...
   */
    class Transaction {
      public:
        ...
        struct TransactionData {
            ...
            TransactionData() noexcept:ops(0),
                largest_data_len(0),
                largest_data_off(0),
                largest_data_off_in_data_bl(0), fadvise_flags(0) {
            }
      ...
      Transaction(Transaction && other) noexcept:
        ...
        void register_on_complete(Context * c) {
            ...
            register_on_applied(new ContainerContext < RunOnDeleteRef >
                                (_complete));
            register_on_commit(new ContainerContext < RunOnDeleteRef >
                               (_complete));
        }
        ...
        static void collect_contexts(vector < Transaction > &t,
            ...
            for (vector < Transaction >::iterator i = t.begin();
                ...
                on_applied_sync.splice(on_applied_sync.end(),
                                       (*i).on_applied_sync);
            }
        ...
        void _update_op(Op * op, vector < __le32 > &cm, vector < __le32 > &om) {
        ...
        void _update_op_bl(bufferlist & bl,
                           vector < __le32 > &cm, vector < __le32 > &om) {
        ...
        void append(Transaction & other) {
            ...
            for (object_index_p = other.object_index.begin();
                ...
                om[object_index_p->second] =
                    _get_object_id(object_index_p->first);
            }
    ...
    /** Inquires about the Transaction as a whole. */
        ...
        uint64_t get_encoded_bytes() {
            ...
            final_size +=
                (coll_index.size() + object_index.size()) * sizeof(__le32);
        ...
        uint64_t get_encoded_bytes_test() {
            ...
            return data_bl.length() +
                op_bl.length() + bl.length() + sizeof(data);
        }
        ...
        class iterator {
          ...
          private:
            ...
            :t(t),
                ...
                colls(t->coll_index.size()), objects(t->object_index.size()) {

                ops = t->data.ops;
                op_buffer_p =
                    t->op_bl.get_contiguous(0, t->data.ops * sizeof(Op));
     ...
     */
        void write(const coll_t & cid, const ghobject_t & oid, uint64_t off,
                   uint64_t len, const bufferlist & write_data, uint32_t flags =
                   0) {
     ...
     */
        void zero(const coll_t & cid, const ghobject_t & oid, uint64_t off,
                  uint64_t len) {
            Op *_op = _get_next_op();
            _op->op = OP_ZERO;
            _op->cid = _get_coll_id(cid);
        ...
        void rmattr(const coll_t & cid, const ghobject_t & oid,
                    const char *name) {
            string n(name);
            rmattr(cid, oid, n);
        }
     ...
     */
        void collection_hint(const coll_t & cid, uint32_t type,
                             const bufferlist & hint) {
            Op *_op = _get_next_op();
            _op->op = OP_COLL_HINT;
            _op->cid = _get_coll_id(cid);
            _op->hint_type = type;
        ...
        void collection_move(const coll_t & cid, coll_t oldcid,
        ...
        void collection_move_rename(const coll_t & oldcid,
        ...
        void omap_clear(coll_t cid, ///< [in] Collection containing oid
        ...
        void omap_setkeys(coll_t cid,   ///< [in] Collection containing oid
                          const ghobject_t & oid,   ///< [in] Object to update
        ...
        void omap_rmkeys(coll_t cid,    ///< [in] Collection containing oid
        ...
        void omap_rmkeyrange(coll_t cid,    ///< [in] Collection containing oid
        ...
        /// Set omap header
        void omap_setheader(coll_t cid, ///< [in] Collection containing oid
        ...
        void split_collection(coll_t cid,
                              uint32_t bits, uint32_t rem, coll_t destination) {
            Op *_op = _get_next_op();
        ...
        void collection_set_bits(coll_t cid, int bits) {
            Op *_op = _get_next_op();
        ...
        void set_alloc_hint(coll_t cid,
                            const ghobject_t & oid,
    ...
    unsigned apply_transaction(Sequencer * osr, Transaction
                               && t, Context * ondisk = 0) {
        vector < Transaction > tls;
        tls.push_back(std::move(t));
        return apply_transactions(osr, tls, ondisk);
    }
    unsigned apply_transactions(Sequencer * osr, vector < Transaction > &tls,
                                Context * ondisk = 0);

    int queue_transaction(Sequencer * osr, Transaction
                          && t, Context * onreadable, Context * ondisk =
                          0, Context * onreadable_sync = 0, TrackedOpRef op =
                          TrackedOpRef(), ThreadPool::TPHandle * handle =
                          NULL) {
        vector < Transaction > tls;
        tls.push_back(std::move(t));
    ...
    virtual int queue_transactions(Sequencer * osr, vector < Transaction > &tls,
                                   TrackedOpRef op = TrackedOpRef(),
    ...
    int queue_transactions(Sequencer * osr,
                           ...
                           Context * oncomplete, TrackedOpRef op);

    int queue_transaction(Sequencer * osr,
                          Transaction && t,
                          Context * onreadable,
                          Context * oncommit,
                          Context * onreadable_sync,
        ...
        vector < Transaction > tls;
        tls.push_back(std::move(t));
        return queue_transactions(osr, tls, onreadable, oncommit,
                                  onreadable_sync, oncomplete, op);
    }

  public:
  ObjectStore(CephContext * cct, const std::string & path_):path(path_),
        cct(cct) {
    }
    virtual ~ ObjectStore() {
    }
   ...
   */
    virtual int set_collection_opts(const coll_t & cid,
   ...
   * @returns 0 on success, negative error code on failure.
   */
    virtual int stat(const coll_t & cid, const ghobject_t & oid, struct stat *st, bool allow_eio = false) = 0;  // struct stat?
    virtual int stat(CollectionHandle & c,
                     const ghobject_t & oid,
                     struct stat *st, bool allow_eio = false) {
   ...
   */
    virtual int read(const coll_t & cid,
                     const ghobject_t & oid,
                     uint64_t offset,
                     size_t len, bufferlist & bl, uint32_t op_flags = 0) = 0;
    virtual int read(CollectionHandle & c,
                     const ghobject_t & oid,
                     uint64_t offset,
                     size_t len, bufferlist & bl, uint32_t op_flags = 0) {
   ...
   */
    ...
    int getattr(coll_t cid, const ghobject_t & oid,
   ...
   * @returns 0 on success, negative error code on failure.
   */
    int getattrs(const coll_t & cid, const ghobject_t & oid, map < string,
                 bufferlist > &aset) {
   ...
   */
    virtual int collection_list(const coll_t & c,
        ...
        0;
    virtual int collection_list(CollectionHandle & c, const ghobject_t & start,
                                const ghobject_t & end, int max,
                                vector < ghobject_t > *ls, ghobject_t * next) {
    ...
    /// OMAP
    /// Get omap contents
    virtual int omap_get(const coll_t & c,  ///< [in] Collection containing oid
    ...
    /// Get omap header
    virtual int omap_get_header(const coll_t & c,   ///< [in] Collection containing oid
                                const ghobject_t & oid, ///< [in] Object containing omap
                                bufferlist * header,    ///< [out] omap header
    ...
    virtual int omap_get_keys(const coll_t & c, ///< [in] Collection containing oid
                              const ghobject_t & oid,   ///< [in] Object containing omap
    ...
    /// Get key values
    virtual int omap_get_values(const coll_t & c,   ///< [in] Collection containing oid
                                const ghobject_t & oid, ///< [in] Object containing omap
    ...
    virtual int omap_check_keys(const coll_t & c,   ///< [in] Collection containing oid
                                const ghobject_t & oid, ///< [in] Object containing omap
   ...
   */
    virtual ObjectMap::ObjectMapIterator get_omap_iterator(const coll_t & c,    ///< [in] collection
                                                           const ghobject_t & oid   ///< [in] object
        ) = 0;
