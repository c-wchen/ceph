...
static bool rgw_get_obj_data_pool(const RGWZoneGroup & zonegroup,
...
static bool rgw_obj_to_raw(const RGWZoneGroup & zonegroup,
                           const RGWZoneParams & zone_params,
                           const string & placement_id, const rgw_obj & obj,
                           rgw_raw_obj * raw_obj)
{
    get_obj_bucket_and_oid_loc(obj, raw_obj->oid, raw_obj->loc);

    return rgw_get_obj_data_pool(zonegroup, zone_params, placement_id, obj,
                                 &raw_obj->pool);
}
...
rgw_raw_obj rgw_obj_select::get_raw_obj(RGWRados * store) const const
{
    if (!is_raw) {
        rgw_raw_obj r;
...
int RGWZoneGroup::create_default(bool old_format)
{
    ...
    if (r < 0 && r != -EEXIST) {
        ...
        if (r < 0) {
            ldout(cct,
                  0) << "create_default: error in init existing zone params: "
                << cpp_strerror(-r) << dendl;
            return r;
        }
        ldout(cct,
...
void RGWZoneGroup::post_process_params()
{
    ...
    for (map < string, RGWZone >::iterator iter = zones.begin();
         iter != zones.end(); ++iter) {
        ...
        for (map < string, RGWZonePlacementInfo >::iterator iter =
             zone_params.placement_pools.begin();
...
int RGWSystemMetaObj::init(CephContext * _cct, RGWRados * _store,
                                   ...
                                   const string & oid)
{
    ...
    try {
        ...
        ldout(cct,
              0) << "error decoding data from " << pool << ":" << oid << dendl;
        return -EIO;
    }
...
int RGWSystemMetaObj::read_id(const string & obj_name, string & object_id)
{
    ...
    try {
        ...
        ldout(cct,
              0) << "ERROR: failed to decode obj from " << pool << ":" << oid <<
            dendl;
        return -EIO;
    }
...
int RGWSystemMetaObj::delete_obj(bool old_format)
{
    ...
    if (default_info.default_id == id
        || (old_format && default_info.default_id == name)) {
...
int RGWSystemMetaObj::rename(const string & new_name)
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldout(cct,
              0) << "Error read_id " << new_name << ": " << cpp_strerror(-ret)
            << dendl;
        return ret;
    }
...
int RGWSystemMetaObj::read_info(const string & obj_id, bool old_format)
{
    ...
    if (ret < 0) {
        ldout(cct,
              0) << "failed reading obj info from " << pool << ":" << oid <<
            ": " << cpp_strerror(-ret) << dendl;
        return ret;
    }
...
int RGWSystemMetaObj::create(bool exclusive)
{
    ...
    if (exclusive && ret == 0) {
        ldout(cct,
              10) << "ERROR: name " << name << " already in use for obj id " <<
            id << dendl;
        return -EEXIST;
    }
...
int RGWSystemMetaObj::write(bool exclusive)
{
    ...
    if (ret < 0) {
        ldout(cct,
              20) << __func__ << "(): store_info() returned ret=" << ret <<
            dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldout(cct,
              20) << __func__ << "(): store_name() returned ret=" << ret <<
            dendl;
        return ret;
    }
    return 0;
...
int RGWRealm::set_current_period(RGWPeriod & period)
{
    ...
    if (ret < 0) {
        ldout(cct,
              0) << "ERROR: period update: " << cpp_strerror(-ret) << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldout(cct,
              0) << "ERROR: period.reflect(): " << cpp_strerror(-ret) << dendl;
        return ret;
    }
...
int RGWPeriodConfig::read(RGWRados * store, const std::string & realm_id)
{
    ...
    int ret =
        rgw_get_system_obj(store, obj_ctx, pool, oid, bl, nullptr, nullptr);
    if (ret < 0) {
        return ret;
    }
    try {
...
int RGWPeriod::init(CephContext * _cct, RGWRados * _store,
                    const string & period_realm_id,
    ...
    if (id.empty()) {
        ...
        if (ret < 0) {
            ldout(cct,
                ...
                " id " << realm_id << " : " << cpp_strerror(-ret) << dendl;
            return ret;
        }
                                 ...
                                 RGWObjVersionTracker * objv)
{
    ...
    if (ret < 0) {
        ldout(cct,
              1) << "error read_lastest_epoch " << pool << ":" << oid << dendl;
        return ret;
    }
    try {
        bufferlist::iterator iter = bl.begin();
...
int RGWPeriod::delete_obj()
{
    ...
    rgw_raw_obj oid {
    pool, get_period_oid_prefix() + get_latest_epoch_oid()};
    int ret = store->delete_system_obj(oid);
    if (ret < 0) {
        ldout(cct, 0) << "WARNING: failed to delete period object " << oid
            << ": " << cpp_strerror(-ret) << dendl;
...
int RGWPeriod::read_info()
{
    ...
    RGWObjectCtx obj_ctx(store);
    int ret =
        rgw_get_system_obj(store, obj_ctx, pool, get_period_oid(), bl, NULL,
                           NULL);
    if (ret < 0) {
        ldout(cct,
  ...
  for (auto & iter:zonegroups) {
        ...
        if (zg.realm_id != realm_id) {
            ldout(cct,
                ...
                realm_id << ", not on our realm " << realm_id << dendl;
            continue;
        }
...
int RGWPeriod::reflect()
{
  for (auto & iter:period_map.zonegroups) {
        ...
        if (r < 0) {
            ldout(cct,
                  0) << "ERROR: failed to store zonegroup info for zonegroup="
                << iter.first << ": " << cpp_strerror(-r) << dendl;
            return r;
        }
        if (zg.is_master_zonegroup()) {
            ...
            if (r == 0) {
                ldout(cct,
                      1) << "Set the period's master zonegroup " << zg.get_id()
...
{
    ldout(cct,
          20) << __func__ << " realm " << realm_id << " period " << id << dendl;
    predecessor_uuid = id;
...
static int read_sync_status(RGWRados * store,
...
{
    ldout(cct,
          20) << __func__ << " realm " << realm.
        get_id() << " period " << current_period.get_id() << dendl;
    ...
    if (master_zone != current_period.get_master_zone()) {
        ...
        int r =
            update_sync_status(current_period, error_stream, force_if_stale);
        if (r < 0) {
    ...
    if (r < 0) {
        ldout(cct,
              0) << "failed to store period: " << cpp_strerror(-r) << dendl;
        return r;
    }
    // set as latest epoch
    ...
    if (r < 0) {
        ldout(cct,
              0) << "failed to set latest epoch: " << cpp_strerror(-r) << dendl;
        return r;
    }
    r = reflect();
    if (r < 0) {
                       ...
                       const string & my_zone_id, set < rgw_pool > &pool_names)
{
  for (auto const &iter:zones) {
        ...
        if (r < 0) {
            ldout(cct,
                  0) << "Error: init zone " << iter << ":" << cpp_strerror(-r)
                << dendl;
            return r;
        }
...
int RGWZoneParams::fix_pool_names()
{
    ...
    if (r < 0) {
        ldout(cct,
              10) << "WARNING: store->list_zones() returned r=" << r << dendl;
    }
    ...
    if (!metadata_heap.name.empty()) {
        metadata_heap =
            fix_zone_pool_dup(pools, name, ".rgw.meta:heap", metadata_heap);
    }
    ...
    intent_log_pool =
        fix_zone_pool_dup(pools, name, ".rgw.log:intent", intent_log_pool);
    usage_log_pool =
        fix_zone_pool_dup(pools, name, ".rgw.log:usage", usage_log_pool);
    user_keys_pool =
        fix_zone_pool_dup(pools, name, ".rgw.meta:users.keys", user_keys_pool);
    user_email_pool =
        fix_zone_pool_dup(pools, name, ".rgw.meta:users.email",
                          user_email_pool);
    user_swift_pool =
        fix_zone_pool_dup(pools, name, ".rgw.meta:users.swift",
                          user_swift_pool);
    user_uid_pool =
        fix_zone_pool_dup(pools, name, ".rgw.meta:users.uid", user_uid_pool);
    roles_pool = fix_zone_pool_dup(pools, name, ".rgw.meta:roles", roles_pool);
    reshard_pool =
        fix_zone_pool_dup(pools, name, ".rgw.log:reshard", reshard_pool);

  for (auto & iter:placement_pools) {
        iter.second.index_pool =
            fix_zone_pool_dup(pools, name,
                              "." + default_bucket_index_pool_suffix,
                              iter.second.index_pool);
        iter.second.data_pool =
            fix_zone_pool_dup(pools, name, "." + default_storage_pool_suffix,
                              iter.second.data_pool);
...
int RGWZoneParams::create(bool exclusive)
{
    ...
    if (r < 0) {
        ldout(store->ctx(),
              10) <<
            "couldn't find old data placement pools config, setting up new ones for the zone"
            << dendl;
        /* a new system, let's set new placement info */
        RGWZonePlacementInfo default_placement;
        default_placement.index_pool =
        ...
        default_placement.data_extra_pool =
            name + "." + default_storage_extra_pool_suffix;
    ...
    if (r < 0 && r != -EEXIST) {
        ldout(cct,
              10) << "WARNING: failed to set zone as default, r=" << r << dendl;
    }
...
int RGWZoneParams::init(CephContext * cct, RGWRados * store, bool setup_obj,
                        bool old_format)
{
    if (name.empty()) {
...
get_compression_type(const string & placement_rule) const const
{
...
void RGWPeriodMap::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 1, bl);
    ::encode(id, bl);
...
void RGWRegionMap::encode(bufferlist & bl) const const
{
    ENCODE_START(3, 1, bl);
    ::encode(regions, bl);
...
void RGWZoneGroupMap::encode(bufferlist & bl) const const
{
    ENCODE_START(3, 1, bl);
    ::encode(zonegroups, bl);
    ::encode(master_zonegroup, bl);
...
void RGWObjManifest::obj_iterator::operator++()
{
    ...
    cur_stripe++;
    dout(20) << "RGWObjManifest::operator++(): rule->part_size=" << rule->
        part_size << " rules.size()=" << manifest->rules.size() << dendl;

    if (rule->part_size > 0) {
        /* multi part, multi stripes object */

        dout(20) << "RGWObjManifest::operator++(): stripe_ofs=" << stripe_ofs <<
            " part_ofs=" << part_ofs << " rule->part_size=" << rule->
            part_size << dendl;
        ...
        stripe_size =
            MIN(rule->part_size - (stripe_ofs - part_ofs),
    ...
    dout(20) << "RGWObjManifest::operator++(): result: ofs=" << ofs <<
        " stripe_ofs=" << stripe_ofs << " part_ofs=" << part_ofs <<
        " rule->part_size=" << rule->part_size << dendl;
    update_location();
}

int RGWObjManifest::generator::create_begin(CephContext * cct,
                                            RGWObjManifest * _m,
...
int RGWObjManifest::generator::create_next(uint64_t ofs)
{
    ...
    manifest->get_implicit_location(cur_part_id, cur_stripe, ofs, NULL,
                                    &cur_obj);
...
int RGWObjManifest::append(RGWObjManifest & m, RGWZoneGroup & zonegroup,
                           RGWZoneParams & zone_params)
{
    ...
    for (; miter != m.rules.end(); ++miter) {
        ...
        if (rule.part_size > 0) {
            expected_part_num =
                rule.start_part_num + (obj_size + next_rule.start_ofs -
                                       rule.start_ofs) / rule.part_size;
        }
...
void RGWObjManifest::append_rules(RGWObjManifest & m, map < uint64_t,
                                  RGWObjManifestRule >::iterator & miter,
                                  string * override_prefix)
{
    for (; miter != m.rules.end(); ++miter) {
...
{
    int r =
        do_complete(accounted_size, etag, mtime, set_mtime, attrs, delete_at,
                    if_match, if_nomatch, user_data, zones_trace);
    if (r < 0)
        return r;
...
RGWPutObjProcessor_Aio::~RGWPutObjProcessor_Aio()
{
   ...
   */
    for (iter = written_objs.begin(); iter != written_objs.end(); ++iter) {
        ...
        if (r < 0 && r != -ENOENT) {
            ldout(store->ctx(),
                  5) << "WARNING: failed to remove obj (" << obj << "), leaked"
                << dendl;
        }
    }
...
int RGWPutObjProcessor_Aio::handle_obj_data(rgw_raw_obj & obj, bufferlist & bl,
                                            off_t ofs, off_t abs_ofs,
                                            void **phandle, bool exclusive)
{
    ...
    // do a write_full.
    return store->aio_put_obj_data(NULL, obj, bl, ((ofs != 0) ? ofs : -1),
                                   exclusive, phandle);
}
...
int RGWPutObjProcessor_Aio::throttle_data(void *handle, const rgw_raw_obj & obj,
                                          uint64_t size, bool need_to_wait)
{
    ...
    if (orig_size - pending_size >= window_size) {
        ...
        uint64_t max_window_size =
            store->ctx()->_conf->rgw_put_obj_max_window_size;
                                          ...
                                          bool exclusive)
{
    ...
    *pobj = cur_obj;
    ...
    return RGWPutObjProcessor_Aio::handle_obj_data(cur_obj, bl,
                                                   ofs - cur_part_ofs, ofs,
                                                   phandle, exclusive);
}
...
int RGWPutObjProcessor_Atomic::prepare_init(RGWRados * store, string * oid_rand)
{
    ...
    int r =
        store->get_max_chunk_size(bucket_info.placement_rule, head_obj,
                                  &max_chunk_size);
    if (r < 0) {
        return r;
    }
...
int RGWPutObjProcessor_Atomic::prepare(RGWRados * store, string * oid_rand)
{
    ...
    r = manifest_gen.create_begin(store->ctx(), &manifest,
                                  bucket_info.placement_rule, head_obj.bucket,
                                  head_obj);
    if (r < 0) {
        return r;
    }
...
int RGWPutObjProcessor_Atomic::complete_writing_data()
{
    ...
    while (pending_data_bl.length()) {
        ...
        if (r < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: write_data() returned " << r << dendl;
            return r;
        }
        data_ofs += write_len;
        ...
        if (r < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: throttle_data() returned " << r << dendl;
            return r;
        }
        ...
        if (data_ofs >= next_part_ofs) {
            ...
            if (r < 0) {
                ldout(store->ctx(),
                      0) << "ERROR: prepare_next_part() returned " << r <<
                    dendl;
                return r;
            }
        }
...
int RGWPutObjProcessor_Atomic::do_complete(size_t accounted_size,
                                           const string & etag,
                                           real_time * mtime,
                                           real_time set_mtime, map < string,
                                           bufferlist > &attrs,
                                           real_time delete_at,
                                           const char *if_match,
                                           const char *if_nomatch,
...
int RGWRados::watch(const string & oid, uint64_t * watch_handle,
...
int RGWRados::unwatch(uint64_t watch_handle)
{
    ...
    if (r < 0) {
        ldout(cct,
              0) << "ERROR: rados->watch_flush() returned r=" << r << dendl;
        return r;
    }
    return 0;
}
...
class RGWWatcher:public librados::WatchCtx2 {
  ...
  public:
    RGWWatcher(RGWRados * r, int i, const string & o):rados(r), index(i),
        oid(o), watch_handle(0) {
    }
    ...
    void reinit() {
        ...
        if (ret < 0) {
            ldout(rados->ctx(),
                  0) << "ERROR: register_watch() returned ret=" << ret << dendl;
            return;
        }
    }
...
class RGWMetaNotifierManager:public RGWCoroutinesManager {
  ...
  public:
    RGWMetaNotifierManager(RGWRados *
                           _store):RGWCoroutinesManager(_store->ctx(),
                                                        _store->
                                                        get_cr_registry()),
        store(_store), http_manager(store->ctx(), completion_mgr) {
        ...
        for (map < string, RGWRESTConn * >::iterator iter = conn_map.begin();
             iter != conn_map.end(); ++iter) {
            RGWRESTConn *conn = iter->second;
            RGWCoroutinesStack *stack =
                new RGWCoroutinesStack(store->ctx(), this);
            stack->call(new RGWPostRESTResourceCR < set < int >,
                        int >(store->ctx(), conn, &http_manager, "/admin/log",
                              pairs, shards, NULL));
...
class RGWDataNotifierManager:public RGWCoroutinesManager {
  ...
  public:
    RGWDataNotifierManager(RGWRados *
                           _store):RGWCoroutinesManager(_store->ctx(),
                                                        _store->
                                                        get_cr_registry()),
        store(_store), http_manager(store->ctx(), completion_mgr) {
        http_manager.set_threaded();
    } int notify_all(map < string, RGWRESTConn * >&conn_map, map < int,
                     set < string > >&shards) {
...
class RGWRadosThread {
    class Worker:public Thread {
        ...
        Worker(CephContext * _cct, RGWRadosThread * _p):cct(_cct),
            processor(_p), lock("RGWRadosThread::Worker") {
        }
        void *entry() override;
        void signal() {
            Mutex::Locker l(lock);
  ...
  :    
    worker(NULL), cct(_store->ctx()), store(_store),
...
void *RGWRadosThread::Worker::entry()
{
    ...
    do {
        ...
        if (r < 0) {
            dout(0) << "ERROR: processor->process() returned error r=" << r <<
                dendl;
        }
...
int RGWMetaNotifier::process()
{
    ...
    for (set < int >::iterator iter = shards.begin(); iter != shards.end();
         ++iter) {
        ldout(cct,
              20) << __func__ << "(): notifying mdlog change, shard_id=" <<
            *iter << dendl;
    }
...
class RGWDataNotifier:public RGWRadosThread {
    ...
    uint64_t interval_msec() override {
        return cct->_conf->get_val < int64_t >
            ("rgw_data_notify_interval_msec");
    } void stop_process() override {
        notify_mgr.stop();
    }
  public:
  RGWDataNotifier(RGWRados * _store):RGWRadosThread(_store, "data-notifier"),
        notify_mgr(_store) {
    }
...
int RGWDataNotifier::process()
{
    ...
    for (map < int, set < string > >::iterator iter = shards.begin();
         iter != shards.end(); ++iter) {
        ldout(cct,
              20) << __func__ << "(): notifying datalog change, shard_id=" <<
            iter->first << ": " << iter->second << dendl;
    }
...
class RGWMetaSyncProcessorThread:public RGWSyncProcessorThread {
  ...
  public:
    RGWMetaSyncProcessorThread(RGWRados * _store,
                               RGWAsyncRadosProcessor * async_rados)
  :    RGWSyncProcessorThread(_store, "meta-sync"), sync(_store,
                                                          async_rados) {
    }

    void wakeup_sync_shards(set < int >&shard_ids) {
        for (set < int >::iterator iter = shard_ids.begin();
             iter != shard_ids.end(); ++iter) {
            sync.wakeup(*iter);
        }
    }
    RGWMetaSyncStatusManager *get_manager() {
        return &sync;
    }
    ...
    int init() override {
        ...
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: sync.init() returned " << ret << dendl;
            return ret;
        }
        return 0;
    }
...
#define DATA_SYNC_INIT_WAIT_SEC 20
  ...
  public:
    RGWDataSyncProcessorThread(RGWRados * _store,
                               RGWAsyncRadosProcessor * async_rados,
                               const string & _source_zone)
    :RGWSyncProcessorThread(_store, "data-sync"), sync(_store, async_rados,
                                                       _source_zone),
        initialized(false) {
    }

    void wakeup_sync_shards(map < int, set < string > >&shard_ids) {
        for (map < int, set < string > >::iterator iter = shard_ids.begin();
             iter != shard_ids.end(); ++iter) {
            sync.wakeup(iter->first, iter->second);
        }
    }
    RGWDataSyncStatusManager *get_manager() {
        return &sync;
    }
...
void RGWRados::wakeup_data_sync_shards(const string & source_zone, map < int,
                                       set < string > >&shard_ids)
{
    ldout(ctx(),
          20) << __func__ << ": source_zone=" << source_zone << ", shard_ids="
        << shard_ids << dendl;
    Mutex::Locker l(data_sync_thread_lock);
    map < string, RGWDataSyncProcessorThread * >::iterator iter =
        data_sync_processor_threads.find(source_zone);
    if (iter == data_sync_processor_threads.end()) {
        ldout(ctx(),
              10) << __func__ << ": couldn't find sync thread for zone " <<
            source_zone << ", skipping async data sync processing" << dendl;
        return;
    }
...
RGWDataSyncStatusManager *RGWRados::get_data_sync_manager(const std::
                                                          string & source_zone)
{
    Mutex::Locker l(data_sync_thread_lock);
...
int RGWRados::get_required_alignment(const rgw_pool & pool,
                                     uint64_t * alignment)
{
    IoCtx ioctx;
...
int RGWRados::get_max_chunk_size(const rgw_pool & pool,
...
int RGWRados::get_max_chunk_size(const string & placement_rule,
                                 const rgw_obj & obj, uint64_t * max_chunk_size)
{
    rgw_pool pool;
...
struct complete_op_data {
    Mutex lock {
    "complete_op_data"};
    AioCompletion *rados_completion {
    nullptr};
    int manager_shard_id {
    -1};
    RGWIndexCompletionManager *manager {
    nullptr};
    rgw_obj obj;
    RGWModifyOp op;
    string tag;
...
int RGWIndexCompletionThread::process()
{
  ...
  for (auto c:comps) {
        std::unique_ptr < complete_op_data > up {
        c};
        ...
        ldout(store->ctx(),
              20) << __func__ << "(): handling completion for key=" << c->
            key << dendl;
        ...
        r = store->guard_reshard(&bs, c->obj, bucket_info,
                                 ...
                                 cls_rgw_guard_bucket_resharding(o,
                                                                 -ERR_BUSY_RESHARDING);
                                 cls_rgw_bucket_complete_op(o, c->op, c->tag,
                                                            c->ver, c->key,
                                                            c->dir_meta,
                                                            &c->remove_objs,
                                                            c->log_op,
                                                            c->bilog_op,
                                                            &c->zones_trace);
                                 return bs->index_ctx.operate(bs->bucket_obj,
                                                              &o);}
        );
        if (r < 0) {
            ldout(cct,
...
class RGWIndexCompletionManager {
    ...
    RGWIndexCompletionThread *completion_thread {
    nullptr};
  ...
  public:
    RGWIndexCompletionManager(RGWRados * _store):store(_store) {
        ...
        for (int i = 0; i < num_shards; i++) {
            char buf[64];
            snprintf(buf, sizeof(buf), "RGWIndexCompletionManager::lock::%d",
                     i);
                                                  ...
                                                  complete_op_data ** result)
{
    ...
    if (remove_objs) {
        for (auto iter = remove_objs->begin(); iter != remove_objs->end();
             ++iter) {
    ...
    *result = entry;
    ...
    entry->rados_completion =
        librados::Rados::aio_create_completion(entry, NULL, obj_complete_cb);

    Mutex::Locker l(*locks[shard_id]);
    completions[shard_id].insert(entry);
}

bool RGWIndexCompletionManager::handle_completion(completion_t cb,
...
void RGWRados::finalize()
{
  ...
  for (auto cmd:admin_commands) {
        ...
        if (r < 0) {
            lderr(cct) << "ERROR: fail to unregister admin socket command (r="
                << r << ")" << dendl;
        }
    }
    ...
    for (iter = zonegroup_conn_map.begin(); iter != zonegroup_conn_map.end();
         ++iter) {
        RGWRESTConn *conn = iter->second;
        delete conn;
    }
...
int RGWRados::init_rados()
{
  ...
  for (auto cmd:admin_commands) {
    ...
    auto handles =
        std::vector < librados::Rados > { cct->_conf->rgw_num_rados_handles };
...
int RGWRados::register_to_service_map(const string & daemon_type,
 ...
 */
static void add_new_connection_to_map(map < string,
                                      RGWRESTConn * >&zonegroup_conn_map,
                                      const RGWZoneGroup & zonegroup,
...
int RGWRados::convert_regionmap()
{
    ...
    try {
        ...
        ldout(cct,
              0) << "error decoding regionmap from " << pool << ":" << oid <<
            dendl;
        return -EIO;
    }

    for (map < string, RGWZoneGroup >::iterator iter =
         zonegroupmap.zonegroups.begin(); iter != zonegroupmap.zonegroups.end();
         ++iter) {
        RGWZoneGroup & zonegroup = iter->second;
    ...
    if (ret < 0) {
        ldout(cct, 0) << "Error could not remove " << obj
            << " after upgrading to zonegroup map: " << cpp_strerror(ret) <<
            dendl;
        return ret;
    }
...
int RGWRados::replace_region_with_zonegroup()
{
    ...
    rgw_pool pool {
    default_zonegroup.get_pool(cct)};
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldout(cct,
              0) << __func__ << " failed to read converted: ret " << ret << " "
            << cpp_strerror(-ret)
            << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldout(cct,
              0) << __func__ << " failed init default region: ret " << ret <<
            " " << cpp_strerror(-ret) << dendl;
        return ret;
    }
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldout(cct,
              0) << __func__ << " failed reading old default region: ret " <<
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldout(cct,
              0) << __func__ << " failed to list regions: ret " << ret << " " <<
            cpp_strerror(-ret) << dendl;
        return ret;
    }
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldout(cct,
                  0) << __func__ << ": error initializing default zone params: "
                << cpp_strerror(-ret) << dendl;
            return ret;
        }
        /* update master zone */
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldout(cct,
                  0) << __func__ <<
                ": error in initializing default zonegroup: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
    ...
    for (list < string >::iterator iter = regions.begin();
        ...
        if (*iter != default_zonegroup_name) {
            ...
            if (ret < 0) {
                ldout(cct,
                      0) << __func__ << " failed init region " << *iter << ": "
                    << cpp_strerror(-ret) << dendl;
                return ret;
            }
    ...
    if (realm.get_id().empty() && !master_region.empty()
        && !master_zone.empty()) {
        string new_realm_name = master_region + "." + master_zone;
        unsigned char md5[CEPH_CRYPTO_MD5_DIGESTSIZE];
        char md5_str[CEPH_CRYPTO_MD5_DIGESTSIZE * 2 + 1];
        MD5 hash;
        hash.Update((const byte *)new_realm_name.c_str(),
                    new_realm_name.length());
        hash.Final(md5);
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " Error initing new realm: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0 && ret != -EEXIST) {
            ldout(cct,
                  0) << __func__ << " Error creating new realm: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " Error setting realm as default: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " Error initing realm: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " Error initing current period: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
    }
    ...
    for (iter = regions.begin(); iter != regions.end(); ++iter) {
        ...
        if (ret == 0 && new_zonegroup.get_id() != *iter) {
            ldout(cct,
                  0) << __func__ << " zonegroup  " << *iter <<
                " already exists id " << new_zonegroup.
                get_id() << " skipping conversion " << dendl;
            continue;
        }
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " failed init zonegroup: ret " << ret << " "
                << cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0 && ret != -EEXIST) {
            ldout(cct,
                  0) << __func__ << " failed to update zonegroup " << *iter <<
        ...
        if (ret < 0 && ret != -EEXIST) {
            ldout(cct,
                  0) << __func__ << " failed to update_name for zonegroup " <<
                *iter << ": ret " << ret << " " << cpp_strerror(-ret)
                << dendl;
            return ret;
        }
        if (zonegroup.get_name() == default_region) {
            ...
            if (ret < 0) {
                ldout(cct,
                      0) << __func__ << " failed to set_as_default " << *iter <<
                    ": ret " << ret << " " << cpp_strerror(-ret)
                    << dendl;
                return ret;
            }
        }
        for (map < string, RGWZone >::const_iterator iter =
             zonegroup.zones.begin(); iter != zonegroup.zones.end(); ++iter) {
            ldout(cct,
                  0) << __func__ << " Converting zone" << iter->first << dendl;
            ...
            if (ret < 0 && ret != -ENOENT) {
                ldout(cct,
                      0) << __func__ << " failed to init zoneparams  " << iter->
                    first << ": " << cpp_strerror(-ret) << dendl;
                return ret;
            }
            else if (ret == -ENOENT) {
                ldout(cct,
                      0) << __func__ << " zone is part of another cluster " <<
                    iter->first << " skipping " << dendl;
                continue;
            }
            zonegroup.realm_id = realm.get_id();
            ...
            if (ret < 0 && ret != -EEXIST) {
                ldout(cct,
                      0) << __func__ << " failed to update zoneparams " <<
                    iter->first << ": " << cpp_strerror(-ret) << dendl;
                return ret;
            }
        ...
        if (!current_period.get_id().empty()) {
            ...
            if (ret < 0) {
                ldout(cct,
                      0) << __func__ <<
                    " failed to add zonegroup to current_period: " <<
                    cpp_strerror(-ret) << dendl;
                return ret;
            }
        }
    ...
    if (!current_period.get_id().empty()) {
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " failed to update new period: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " failed to store new period: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " failed to update local objects: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
    }
  ...
  for (auto const &iter:regions) {
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << __func__ << " failed init zonegroup" << iter << ": ret "
                << ret << " " << cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldout(cct,
                  0) << __func__ << " failed to delete region " << iter <<
                ": ret " << ret << " " << cpp_strerror(-ret)
                << dendl;
            return ret;
        }
    ...
    if (ret < 0) {
        ldout(cct,
              0) << __func__ << " failed to mark cluster as converted: ret " <<
...
int RGWRados::init_zg_from_period(bool * initialized)
{
    *initialized = false;
    ...
    if (iter != current_period.get_map().zonegroups.end()) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldout(cct,
                  0) << "failed reading zone params info: " << " " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
        if (ret == -ENOENT && zonegroup.get_name() == default_zonegroup_name) {
            ldout(cct,
                  10) << " Using default name " << default_zone_name << dendl;
        ...
        if (master == zg.zones.end()) {
            ...
            if (zg.master_zone.empty() && zg.zones.size() == 1) {
                ...
                ldout(cct,
                    ...
                    master->second.name << " id:" << master->second.
                    id << " as master" << dendl;
                if (zonegroup.get_id() == zg.get_id()) {
                    ...
                    if (ret < 0) {
                        ldout(cct,
                              0) << "error initializing zonegroup : " <<
                            cpp_strerror(-ret) << dendl;
                        return ret;
                    }
                ...
                ldout(cct,
                      0) << "zonegroup " << zg.
                    get_name() << " missing zone for master_zone=" << zg.
                    master_zone << dendl;
                return -EINVAL;
            }
        }
        const auto & endpoints = master->second.endpoints;
        add_new_connection_to_map(zonegroup_conn_map, zg,
                                  new RGWRESTConn(cct, this, zg.get_id(),
                                                  endpoints));
        if (!current_period.get_master_zonegroup().empty()
            && zg.get_id() == current_period.get_master_zonegroup()) {
            rest_master_conn =
                new RGWRESTConn(cct, this, zg.get_id(), endpoints);
        }
    }
...
int RGWRados::init_zg_from_local(bool * creating_defaults)
{
    int ret = zonegroup.init(cct, this);
    if ((ret < 0 && ret != -ENOENT)
        || (ret == -ENOENT && !cct->_conf->rgw_zonegroup.empty())) {
        ldout(cct,
    ...
    if (zonegroup.is_master_zonegroup()) {
        ...
        const auto & endpoints = master->second.endpoints;
        rest_master_conn =
            new RGWRESTConn(cct, this, zonegroup.get_id(), endpoints);
    }
...
int RGWRados::init_complete()
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ...
        ldout(cct,
              20) << "realm  " << realm.get_name() << " " << realm.
            get_id() << dendl;
        ret = current_period.init(cct, this, realm.get_id(), realm.get_name());
        if (ret < 0 && ret != -ENOENT) {
            ldout(cct,
                  0) << "failed reading current period info: " << " " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
    ...
    if (ret < 0) {
        lderr(cct) << "failed converting region to zonegroup : ret " << ret <<
            " " << cpp_strerror(-ret) << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        lderr(cct) << "failed converting regionmap: " << cpp_strerror(-ret) <<
            dendl;
        return ret;
    }
    ...
    if (using_local) {
        ldout(cct,
              10) <<
            " cannot find current period zonegroup using local zonegroup" <<
            dendl;
    ...
    ret = zone_params.init(cct, this);
    if (ret < 0 && ret != -ENOENT) {
        lderr(cct) << "failed reading zone info: ret " << ret << " " <<
            cpp_strerror(-ret) << dendl;
        return ret;
    }
    map < string, RGWZone >::iterator zone_iter =
        get_zonegroup().zones.find(zone_params.get_id());
    if (zone_iter == get_zonegroup().zones.end()) {
        if (using_local) {
            lderr(cct) << "Cannot find zone id=" << zone_params.
                get_id() << " (name=" << zone_params.get_name() << ")" << dendl;
            return -EINVAL;
        }
        ldout(cct,
              1) << "Cannot find zone id=" << zone_params.
            get_id() << " (name=" << zone_params.
            get_name() << "), switching to local zonegroup configuration" <<
            dendl;
        ret = init_zg_from_local(&creating_defaults);
        if (ret < 0) {
            return ret;
    ...
    if (run_sync_thread) {
        ret =
            sync_modules_manager->create_instance(cct,
                                                  zone_public_config.tier_type,
                                                  zone_params.tier_config,
                                                  &sync_module);
        if (ret < 0) {
            lderr(cct) << "ERROR: failed to init sync module instance, ret=" <<
                ret << dendl;
            return ret;
        }
    }
    ...
    if (need_watch_notify()) {
        ret = init_watch();
        if (ret < 0) {
            lderr(cct) << "ERROR: failed to initialize watch: " <<
                cpp_strerror(-ret) << dendl;
            return ret;
        }
    }
  ...
  for (auto ziter:get_zonegroup().zones) {
        ...
        if (z.endpoints.empty()) {
            ldout(cct,
                ...
                id << " id " << z.name << ": no endpoints defined" << dendl;
            continue;
        }
        ldout(cct,
    ...
    async_rados =
        new RGWAsyncRadosProcessor(this,
    ...
    if (run_sync_thread) {
      ...
      for (auto iter:zone_data_sync_from_map) {
            ldout(cct,
                ...
                first << dendl;
            auto *thread =
                new RGWDataSyncProcessorThread(this, async_rados, iter.first);
            ret = thread->init();
            if (ret < 0) {
                ldout(cct,
                      0) << "ERROR: failed to initialize data sync thread" <<
                    dendl;
                return ret;
            }
            thread->start();
        ...
        if (interval > 0) {
            sync_log_trimmer =
                new RGWSyncLogTrimThread(this, &*bucket_trim, interval);
            ret = sync_log_trimmer->init();
            if (ret < 0) {
                ldout(cct,
                      0) << "ERROR: failed to initialize sync log trim thread"
                    << dendl;
                return ret;
            }
    ...
    bucket_index_max_shards =
        (cct->_conf->rgw_override_bucket_index_max_shards ? cct->_conf->
         rgw_override_bucket_index_max_shards : get_zone().
         bucket_index_max_shards);
    ...
    ldout(cct,
          20) << __func__ << " bucket index max shards: " <<
        bucket_index_max_shards << dendl;
    ...
    run_reshard_thread = run_reshard_thread
        && (get_zonegroup().master_zone == zone_public_config.id);
    if (run_reshard_thread) {
...
int RGWRados::list_raw_prefixed_objs(const rgw_pool & pool,
                                     const string & prefix,
                                     list < string > &result)
{
    bool is_truncated;
...
int RGWRados::list_regions(list < string > &regions)
{
    ...
    return list_raw_prefixed_objs(zonegroup.get_pool(cct),
                                  region_info_oid_prefix, regions);
}
...
int RGWRados::list_zonegroups(list < string > &zonegroups)
{
    ...
    return list_raw_prefixed_objs(zonegroup.get_pool(cct),
                                  zonegroup_names_oid_prefix, zonegroups);
}
...
int RGWRados::list_zones(list < string > &zones)
{
    ...
    return list_raw_prefixed_objs(zoneparams.get_pool(cct),
                                  zone_names_oid_prefix, zones);
}

int RGWRados::list_realms(list < string > &realms)
{
    RGWRealm realm(cct, this);
    return list_raw_prefixed_objs(realm.get_pool(cct), realm_names_oid_prefix,
                                  realms);
}
...
int RGWRados::list_periods(list < string > &periods)
{
    ...
    int ret =
        list_raw_prefixed_objs(period.get_pool(cct),
                               period.get_info_oid_prefix(), raw_periods);
    if (ret < 0) {
        return ret;
    }
...
int RGWRados::list_periods(const string & current_period,
                           list < string > &periods)
{
    int ret = 0;
...
{
    return rgw_init_ioctx(get_rados_handle(), get_zone_params().domain_root,
                          root_pool_ctx, true);
}

int RGWRados::open_gc_pool_ctx()
{
    return rgw_init_ioctx(get_rados_handle(), get_zone_params().gc_pool,
                          gc_pool_ctx, true);
}

int RGWRados::open_lc_pool_ctx()
{
    return rgw_init_ioctx(get_rados_handle(), get_zone_params().lc_pool,
                          lc_pool_ctx, true);
}

int RGWRados::open_objexp_pool_ctx()
{
    return rgw_init_ioctx(get_rados_handle(), get_zone_params().log_pool,
                          objexp_pool_ctx, true);
}

int RGWRados::open_reshard_pool_ctx()
{
    return rgw_init_ioctx(get_rados_handle(), get_zone_params().reshard_pool,
                          reshard_pool_ctx, true);
}

int RGWRados::init_watch()
{
    int r =
        rgw_init_ioctx(&rados[0], get_zone_params().control_pool,
                       control_pool_ctx, true);
    if (r < 0) {
        return r;
    }
...
void RGWRados::build_bucket_index_marker(const string & shard_id_str,
...
int RGWRados::open_bucket_index_ctx(const RGWBucketInfo & bucket_info,
 ...
 * returns 0 on success, -ERR# otherwise.
 */
int RGWRados::list_buckets_next(rgw_bucket_dir_entry & obj,
...
int RGWRados::log_remove(const string & name)
{
    librados::IoCtx io_ctx;
    int r =
        rgw_init_ioctx(get_rados_handle(), get_zone_params().log_pool, io_ctx);
    if (r < 0)
        return r;
...
int RGWRados::log_show_next(RGWAccessHandle handle, rgw_log_entry * entry)
{
    ...
    ldout(cct,
          10) << "log_show_next pos " << state->pos << " bl " << state->bl.
        length()
        << " off " << off << " eof " << (int)state->eof << dendl;
    // read some?
 ...
 */
static void usage_log_hash(CephContext * cct, const string & name,
                           string & hash, uint32_t index)
{
    uint32_t val = index;
    ...
    snprintf(buf, sizeof(buf), RGW_USAGE_OBJ_PREFIX "%u",
             (unsigned)(val % max_shards));
    hash = buf;
}
...
int RGWRados::read_usage(const rgw_user & user, uint64_t start_epoch,
                         ...
                         map < rgw_user_bucket, rgw_usage_log_entry > &usage)
{
    ...
    do {
        ...
        int ret =
            cls_obj_usage_log_read(hash, user_str, start_epoch, end_epoch, num,
...
int RGWRados::trim_usage(rgw_user & user, uint64_t start_epoch,
                         uint64_t end_epoch)
{
    uint32_t index = 0;
    string hash, first_hash;
    ...
    do {
        int ret =
            cls_obj_usage_log_trim(hash, user_str, start_epoch, end_epoch);
...
void RGWRados::shard_name(const string & prefix, unsigned shard_id,
                          string & name)
{
    char buf[16];
...
int RGWRados::time_log_add(const string & oid, const real_time & ut,
                           const string & section, const string & key,
                           bufferlist & bl)
{
    librados::IoCtx io_ctx;
...
int RGWRados::time_log_list(const string & oid, const real_time & start_time,
                            const real_time & end_time, int max_entries,
                            list < cls_log_entry > &entries,
                            const string & marker, string * out_marker,
                            bool * truncated)
{
    librados::IoCtx io_ctx;
...
int RGWRados::time_log_trim(const string & oid, const real_time & start_time,
...
void RGWRados::objexp_get_shard(int shard_num, string & shard)
{                               /* out */
    shard = objexp_hint_get_shardname(shard_num);
}

int RGWRados::objexp_hint_list(const string & oid, const ceph::real_time & start_time, const ceph::real_time & end_time, const int max_entries, const string & marker, list < cls_timeindex_entry > &entries,   /* out */
                               string * out_marker, /* out */
                               bool * truncated)
{                               /* out */
    librados::ObjectReadOperation op;
    cls_timeindex_list(op, utime_t(start_time), utime_t(end_time), marker,
                       max_entries, entries, out_marker, truncated);
...
{
    int ret =
        cls_timeindex_trim(objexp_pool_ctx, oid, utime_t(start_time),
                           utime_t(end_time),
...
int RGWRados::lock_exclusive(rgw_pool & pool, const string & oid,
                             timespan & duration, string & zone_id,
                             string & owner_id)
{
    librados::IoCtx io_ctx;
...
int RGWRados::unlock(rgw_pool & pool, const string & oid, string & zone_id,
                     string & owner_id)
{
    librados::IoCtx io_ctx;
...
int RGWRados::decode_policy(bufferlist & bl, ACLOwner * owner)
{
    ...
    try {
        ...
        ldout(cct,
              0) << "ERROR: could not decode policy, caught buffer::error" <<
            dendl;
        return -EIO;
    }
...
int rgw_policy_from_attrset(CephContext * cct, map < string,
                            bufferlist > &attrset,
                            RGWAccessControlPolicy * policy)
{
    ...
    if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 15)) {
        RGWAccessControlPolicy_S3 *s3policy =
            static_cast < RGWAccessControlPolicy_S3 * >(policy);
...
int RGWRados::Bucket::update_bucket_id(const string & new_bucket_id)
{
    ...
    int ret =
        store->get_bucket_instance_info(obj_ctx, bucket, bucket_info, nullptr,
                                        nullptr);
    if (ret < 0) {
        return ret;
    }
                                                 ...
                                                 bool * is_truncated)
{
    ...
    rgw_obj_key marker_obj(params.marker.name, params.marker.instance,
                           params.ns);
    rgw_obj_index_key cur_marker;
    marker_obj.get_index_key(&cur_marker);

    rgw_obj_key end_marker_obj(params.end_marker.name,
                               params.end_marker.instance, params.ns);
    ...
    while (truncated && count <= max) {
        if (skip_after_delim > cur_marker.name) {
            ...
            ldout(cct,
                ...
                name << "[" << cur_marker.instance << "]" << dendl;
        }
        ...
        for (auto eiter = ent_map.begin(); eiter != ent_map.end(); ++eiter) {
            ...
            if (!params.delim.empty()) {
                int delim_pos =
                    obj.name.find(params.delim, params.prefix.size());

                if (delim_pos >= 0) {
                    string prefix_key = obj.name.substr(0, delim_pos + 1);

                    if (common_prefixes &&
                        common_prefixes->find(prefix_key) ==
                        common_prefixes->end()) {
                        if (count >= max) {
                            truncated = true;
                            goto done;
                        ...
                        int marker_delim_pos =
                            cur_marker.name.find(params.delim,
                                                 cur_prefix.size());
...
void RGWRados::create_bucket_id(string * bucket_id)
{
    ...
    snprintf(buf, sizeof(buf), "%s.%llu.%llu",
             get_zone_params().get_id().c_str(), (long long)iid,
             (long long)bid);
    *bucket_id = buf;
}
...
int RGWRados::create_bucket(RGWUserInfo & owner, rgw_bucket & bucket,
                            ...
                            uint32_t * pmaster_num_shards, bool exclusive)
{
#define MAX_CREATE_RETRIES 20   /* need to bound retries */
    ...
    for (int i = 0; i < MAX_CREATE_RETRIES; i++) {
        ...
        ret =
            put_linked_bucket_info(info, exclusive, ceph::real_time(), pep_objv,
                                   &attrs, true);
        if (ret == -EEXIST) {
            librados::IoCtx index_ctx;
            map < int, string > bucket_objs;
            ...
            RGWObjectCtx obj_ctx(this);
            r = get_bucket_info(obj_ctx, bucket.tenant, bucket.name, info, NULL,
                                NULL);
            if (r < 0) {
                if (r == -ENOENT) {
                    continue;
    ...
    ldout(cct,
          0) <<
        "ERROR: could not create bucket, continuously raced with bucket creation and removal"
        << dendl;
    return -ENOENT;
}

int RGWRados::select_new_bucket_location(RGWUserInfo & user_info,
                                         const string & zonegroup_id,
                                         const string & request_rule,
                                         ...
                                         RGWZonePlacementInfo * rule_info)
{
    ...
    if (!request_rule.empty()) {
        ...
        if (titer == zonegroup.placement_targets.end()) {
            ldout(cct,
                  0) << "could not find user default placement id " <<
                user_info.default_placement << " within zonegroup " << dendl;
            return -ERR_INVALID_LOCATION_CONSTRAINT;
        }
    }
    else {
        if (zonegroup.default_placement.empty()) {  // zonegroup default rule as fallback, it should not be empty.
            ...
            titer =
                zonegroup.placement_targets.find(zonegroup.default_placement);
            if (titer == zonegroup.placement_targets.end()) {
                ldout(cct,
                      0) << "could not find zonegroup default placement id " <<
                    zonegroup.
                    default_placement << " within zonegroup " << dendl;
                return -ERR_INVALID_LOCATION_CONSTRAINT;
            }
        }
    }
...
int RGWRados::select_bucket_location_by_rule(const string & location_rule,
                                             RGWZonePlacementInfo * rule_info)
{
    ...
    map < string, RGWZonePlacementInfo >::iterator piter =
        get_zone_params().placement_pools.find(location_rule);
...
int RGWRados::select_bucket_placement(RGWUserInfo & user_info,
                                      const string & zonegroup_id,
                                      const string & placement_rule,
                                      string * pselected_rule_name,
                                      RGWZonePlacementInfo * rule_info)
{
    if (!get_zone_params().placement_pools.empty()) {
        return select_new_bucket_location(user_info, zonegroup_id,
                                          placement_rule, pselected_rule_name,
                                          rule_info);
    }
...
int RGWRados::select_legacy_bucket_placement(RGWZonePlacementInfo * rule_info)
{
    ...
    RGWObjectCtx obj_ctx(this);
    int ret =
        rgw_get_system_obj(this, obj_ctx, get_zone_params().domain_root,
                           avail_pools, map_bl, NULL, NULL);
    if (ret < 0) {
        goto read_omap;
    }
  ...
  read_omap:
    ...
    if (write_map) {
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << "WARNING: could not save avail pools map info ret=" <<
                ret << dendl;
        }
    }
...
bool RGWRados::get_obj_data_pool(const string & placement_rule,
                                 const rgw_obj & obj, rgw_pool * pool)
{
    return rgw_get_obj_data_pool(zonegroup, zone_params, placement_rule, obj,
                                 pool);
}
...
int RGWRados::create_pools(vector < rgw_pool > &pools, vector < int >&retcodes)
{
    ...
    for (riter = rets.begin(), citer = completions.begin(); riter != rets.end();
         ++riter, ++citer) {
        int r = *riter;
  ...
  for (auto & io_ctx:io_ctxs) {
        ...
        completions.push_back(c);
        int ret =
            io_ctx.application_enable_async(pg_pool_t::APPLICATION_NAME_RGW,
                                            false, c);
        assert(ret == 0);
    }
...
int RGWRados::get_obj_head_ref(const RGWBucketInfo & bucket_info,
 ...
 * up without one
 */
int RGWRados::fix_head_obj_locator(const RGWBucketInfo & bucket_info,
                                   bool copy_obj, bool remove_bad,
                                   rgw_obj_key & key)
{
...
#define HEAD_SIZE 512 * 1024
    ...
    if (ret < 0) {
        lderr(cct) << "ERROR: ioctx.operate(oid=" << oid << ") returned ret=" <<
            ret << dendl;
        return ret;
    }

    if (size > HEAD_SIZE) {
        lderr(cct) << "ERROR: returned object size (" << size <<
    ...
    if (remove_bad) {
        ...
        if (ret < 0) {
            lderr(cct) << "ERROR: failed to remove original bad object" <<
                dendl;
            return ret;
        }
    }
...
#define COPY_BUF_SIZE (4 * 1024 * 1024)
    ...
    if (ofs != size) {
        lderr(cct) << "ERROR: " << __func__ << ": copying " << src_oid << " -> "
  ...
  done_err:
    lderr(cct) << "ERROR: failed to copy " << src_oid << " -> " << dst_oid <<
        dendl;
    return ret;
}
 ...
 * up without one
 */
int RGWRados::fix_tail_obj_locator(const RGWBucketInfo & bucket_info,
                                   rgw_obj_key & key, bool fix, bool * need_fix)
{
    const rgw_bucket & bucket = bucket_info.bucket;
    rgw_obj obj(bucket, key);
    ...
    if (astate->has_manifest) {
        ...
        for (miter = manifest.obj_begin(); miter != manifest.obj_end(); ++miter) {
            ...
            rgw_raw_obj_to_obj(manifest.get_tail_placement().bucket, raw_loc,
                               &loc);
            ...
            ldout(cct,
                  20) << __func__ << ": found bad object part: " << loc <<
                dendl;
            if (need_fix) {
                *need_fix = true;
            }
            if (fix) {
                r = move_rados_obj(src_ioctx, oid, bad_loc, ref.ioctx, oid,
                                   locator);
                if (r < 0) {
                    lderr(cct) << "ERROR: copy_rados_obj() on oid=" << oid <<
                        " returned r=" << r << dendl;
                }
            }
        }
                                ...
                                RGWBucketInfo * bucket_info_out)
{
    ...
    int ret =
        store->get_bucket_instance_info(obj_ctx, bucket, *bucket_info_p, NULL,
                                        NULL);
    if (ret < 0) {
        return ret;
    }
    ...
    ret =
        store->open_bucket_index_shard(*bucket_info_p, index_ctx,
...
int RGWRados::BucketShard::init(const rgw_bucket & _bucket,
                                int sid, RGWBucketInfo * bucket_info_out)
{
    bucket = _bucket;
    shard_id = sid;
    ...
    ret =
        store->open_bucket_index_shard(*bucket_info_p, index_ctx, shard_id,
                                       &bucket_obj);
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: open_bucket_index_shard() returned ret=" << ret <<
...
int RGWRados::BucketShard::init(const RGWBucketInfo & bucket_info, int sid)
{
    ...
    int ret =
        store->open_bucket_index_shard(bucket_info, index_ctx, shard_id,
                                       &bucket_obj);
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: open_bucket_index_shard() returned ret=" << ret <<
                                    ...
                                    RGWBucketInfo & bucket_info, rgw_obj & obj)
{
    ...
    r = get_bucket_info(obj_ctx, bucket_info.bucket.tenant,
                        bucket_info.swift_ver_location, dest_bucket_info, NULL,
                        NULL);
    if (r < 0) {
    ...
    r = copy_obj(obj_ctx, user, client_id, op_id, NULL, /* req_info *info */
                 no_zone, dest_obj, obj, dest_bucket_info, bucket_info, NULL,   /* time_t *src_mtime */
                 NULL,          /* time_t *mtime */
                 ...
                 state->attrset, RGW_OBJ_CATEGORY_MAIN, 0,  /* uint64_t olh_epoch */
                                            ...
                                            void *_index_op)
{
    ...
    r = target->prepare_atomic_modification(op, reset_obj, ptag, meta.if_match,
                                            meta.if_nomatch, false,
                                            modify_tail);
    if (r < 0)
        return r;
    ...
    bool versioned_op = (target->versioning_enabled() || is_olh
                         || versioned_target);
    ...
    if (versioned_op && meta.olh_epoch) {
        r = store->set_olh(target->get_ctx(), target->get_bucket_info(), obj,
                           false, NULL, *meta.olh_epoch, real_time(), false,
                           meta.zones_trace);
        if (r < 0) {
            return r;
        }
    ...
    if (!real_clock::is_zero(meta.delete_at)) {
        ...
        r = store->objexp_hint_add(meta.delete_at,
                                   obj.bucket.tenant, obj.bucket.name,
    ...
    if (meta.completeMultipart) {
        store->quota_handler->update_stats(meta.owner, obj.bucket,
                                           (orig_exists ? 0 : 1), 0, orig_size);
    }
    else {
        store->quota_handler->update_stats(meta.owner, obj.bucket,
                                           (orig_exists ? 0 : 1),
  ...
  done_cancel:
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: index_op.cancel()() returned ret=" << ret << dendl;
    }
                                        ...
                                        map < string, bufferlist > &attrs)
{
    ...
    if (assume_noent) {
        r = _do_write_meta(size, accounted_size, attrs, assume_noent,
                           meta.modify_tail, (void *)&index_op);
        if (r == -EEXIST) {
            assume_noent = false;
        }
    }
...
int RGWRados::put_system_obj_impl(rgw_raw_obj & obj, uint64_t size,
                                  real_time * mtime, map < std::string,
                                  bufferlist > &attrs, int flags,
                                  bufferlist & data,
  ...
  public:
    ...
    :RGWPutObj_Filter(next), buffer_size(buffer_size) {
        assert(ISP2(buffer_size));  // must be power of 2
                      ...
                      rgw_raw_obj * pobj, bool * again) override {
        if (*again || !bl.length()) {
            // flush buffered data
            return RGWPutObj_Filter::handle_data(buffer, ofs, phandle, pobj,
                                                 again);
        }
...
class RGWRadosPutObj:public RGWGetDataCB {
  ...
  public:
    RGWRadosPutObj(CephContext * cct,
        ...
        progress_data(_progress_data), extra_data_left(0), data_len(0) {
    } int process_attrs(void) {
        if (extra_data_bl.length()) {
            JSONParser jp;
            if (!jp.parse(extra_data_bl.c_str(), extra_data_bl.length())) {
                ldout(cct,
                      0) << "failed to parse response extra data. len=" <<
                    extra_data_bl.length() << " data=" << extra_data_bl.
                    c_str() << dendl;
                return -EIO;
            }
            ...
            while (iter != src_attrs.end()) {
                if (!boost::algorithm::
                    starts_with(iter->first, RGW_ATTR_OLH_PREFIX)) {
                    break;
                }
        ...
        do {
            ...
            if (need_opstate && opstate) {
                ...
                if (ret < 0) {
                    ldout(cct,
                          0) <<
                        "ERROR: RGWRadosPutObj::handle_data(): failed to renew op state ret="
                        << ret << dendl;
                    int r = filter->throttle_data(handle, obj, size, false);
                    if (r < 0) {
                        ldout(cct,
                              0) <<
                            "ERROR: RGWRadosPutObj::handle_data(): processor->throttle_data() returned "
                            << r << dendl;
                    }
    ...
    int complete(const string & etag, real_time * mtime, real_time set_mtime,
        ...
        return processor->complete(data_len, etag, mtime, set_mtime, attrs,
                                   delete_at, NULL, NULL, NULL, zones_trace);
    }
...
int RGWRados::rewrite_obj(RGWBucketInfo & dest_bucket_info, rgw_obj & obj)
{
    ...
    ret =
        get_max_chunk_size(dest_bucket_info.placement_rule, obj,
                           &max_chunk_size);
    if (ret < 0) {
        ldout(cct,
    ...
    return copy_obj_data(rctx, dest_bucket_info, read_op, obj_size - 1, obj,
                         obj, max_chunk_size, NULL, mtime, attrset,
                              ...
                              string * ptag, string * petag)
{
    ...
    if (source_zone.empty()) {
        ...
        map < string, RGWRESTConn * >::iterator iter =
            zone_conn_map.find(source_zone);
        if (iter == zone_conn_map.end()) {
            ldout(cct,
                  0) << "could not find zone connection to zone: " <<
                source_zone << dendl;
            return -ENOENT;
        }
        conn = iter->second;
    ...
    ret =
        conn->complete_request(in_stream_req, etag, &set_mtime, psize,
                               req_headers);
    if (ret < 0) {
        return ret;
    }
...
int RGWRados::fetch_remote_obj(RGWObjectCtx & obj_ctx,
                               ...
                               void (*progress_cb) (off_t, void *),
                               void *progress_data, rgw_zone_set * zones_trace)
{
    /* source is in a different zonegroup, copy from there */
    ...
    RGWPutObjProcessor_Atomic processor(obj_ctx,
                                        dest_bucket_info, dest_obj.bucket,
                                        dest_obj.key.name,
    ...
    if (record_op_state) {
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << "ERROR: failed to set opstate ret=" << ret << dendl;
            delete opstate;
            return ret;
        }
    }
    ...
    const auto & compression_type =
        zone_params.get_compression_type(dest_bucket_info.placement_rule);
    ...
    RGWRadosPutObj cb(cct, plugin, compressor, &processor, opstate, progress_cb,
                      progress_data);
    ...
    if (copy_if_newer) {
        ...
        ret =
            get_obj_state(&obj_ctx, dest_bucket_info, dest_obj, &dest_state,
                          false);
        if (ret < 0)
            goto set_err_state;
    ...
    ret = conn->get_obj(user_id, info, src_obj, pmod, unmod_ptr,
                        dest_mtime_weight.zone_short_id,
                        dest_mtime_weight.pg_ver, prepend_meta, get_op,
                        rgwx_stat, sync_manifest, skip_decrypt, &cb,
                        &in_stream_req);
    if (ret < 0) {
        goto set_err_state;
    }
    ...
    ret =
        conn->complete_request(in_stream_req, etag, &set_mtime, nullptr,
                               req_headers);
    if (ret < 0) {
        goto set_err_state;
    }
    ...
    if (copy_if_newer) {
        ...
        if (i != attrs.end() && i->second.length() > 0) {
            ...
            try {
                ...
                ldout(ctx(),
                      0) << "ERROR: failed to decode pg ver attribute, ignoring"
                    << dendl;
                /* non critical error */
            }
        }
...
#define MAX_COMPLETE_RETRY 100
    for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
        ret =
            cb.complete(etag, mtime, set_mtime, attrs, delete_at, zones_trace);
        if (ret < 0) {
            goto set_err_state;
        }
        if (copy_if_newer && cb.is_canceled()) {
            ...
            if (ret < 0) {
                ldout(cct,
                      0) << "ERROR: " << __func__ <<
                    ": get_err_state() returned ret=" << ret << dendl;
                goto set_err_state;
            }
            dest_mtime_weight.init(dest_state);
            ...
            if (!dest_state->exists || dest_mtime_weight < set_mtime_weight) {
                ldout(cct,
                    ...
                    mtime << " dest_state->exists=" << dest_state->
                    exists << dendl;
                continue;
            }
            else {
    ...
    if (i == MAX_COMPLETE_RETRY) {
        ldout(cct,
              0) <<
            "ERROR: retried object completion too many times, something is wrong!"
            << dendl;
        ret = -EIO;
        goto set_err_state;
    }
                                      ...
                                      rgw_obj & dest_obj, real_time * mtime)
{
    ...
    int ret =
        rest_master_conn->put_obj_init(user_id, dest_obj, astate->size,
                                       src_attrs, &out_stream_req);
    if (ret < 0) {
        return ret;
    }
                       ...
                       void (*progress_cb) (off_t, void *), void *progress_data)
{
    ...
    ldout(cct,
          5) << "Copy object " << src_obj.bucket << ":" << src_obj.
        get_oid() << " => " << dest_obj.bucket << ":" << dest_obj.
        get_oid() << dendl;

    if (remote_src || !source_zone.empty()) {
        return fetch_remote_obj(obj_ctx, user_id, client_id, op_id, true, info,
                                source_zone, dest_obj, src_obj,
                                dest_bucket_info, src_bucket_info, src_mtime,
                                mtime, mod_ptr, unmod_ptr, high_precision_time,
                                if_match, if_nomatch, attrs_mod, copy_if_newer,
                                attrs, category, olh_epoch, delete_at,
                                version_id, ptag, petag, progress_cb,
                                progress_data);
    }
    ...
    map < string, bufferlist >::iterator cmp =
        src_attrs.find(RGW_ATTR_COMPRESSION);
    ...
    if (remote_dest) {
        ...
        return copy_obj_to_remote_dest(astate, attrs, read_op, user_id,
                                       dest_obj, mtime);
    }
    uint64_t max_chunk_size;

    ret =
        get_max_chunk_size(dest_bucket_info.placement_rule, dest_obj,
                           &max_chunk_size);
    if (ret < 0) {
        ldout(cct,
              0) << "ERROR: failed to get max_chunk_size() for bucket " <<
    ...
    if (!get_obj_data_pool
        (dest_bucket_info.placement_rule, dest_obj, &dest_pool)) {
        ldout(cct,
    ...
    if (copy_data) {            /* refcounting tail wouldn't work here, just copy the data */
        return copy_obj_data(obj_ctx, dest_bucket_info, read_op, obj_size - 1,
                             dest_obj, src_obj, max_chunk_size, mtime,
                             real_time(), attrs, category, olh_epoch, delete_at,
    ...
    ldout(cct,
          20) << "dest_obj=" << dest_obj << " src_obj=" << src_obj <<
    ...
    if (!copy_itself) {
        ...
        const rgw_bucket_placement & tail_placement =
            manifest.get_tail_placement();
        if (tail_placement.bucket.name.empty()) {
            manifest.set_tail_placement(tail_placement.placement_rule,
                                        src_obj.bucket);
        }
        string ref_tag;
    ...
    if (copy_first) {
        ...
        pmanifest->set_head(dest_bucket_info.placement_rule, dest_obj,
                            first_chunk.length());
    }
    else {
...
int RGWRados::copy_obj_data(RGWObjectCtx & obj_ctx,
                            ...
                            real_time delete_at,
                            string * version_id,
                            string * ptag, ceph::buffer::list * petag)
{
    bufferlist first_chunk;
    RGWObjManifest manifest;
...
bool RGWRados::is_syncing_bucket_meta(const rgw_bucket & bucket)
{
    ...
    if (current_period.is_single_zonegroup()
        && get_zonegroup().zones.size() == 1) {
        return false;
    }
 ...
 * Returns 0 on success, -ERR# otherwise.
 */
int RGWRados::delete_bucket(RGWBucketInfo & bucket_info,
                            RGWObjVersionTracker & objv_tracker,
                            bool check_empty)
{
    const rgw_bucket & bucket = bucket_info.bucket;
    ...
    r = rgw_bucket_delete_bucket_obj(this, bucket.tenant, bucket.name,
                                     objv_tracker);
    if (r < 0)
        return r;
    ...
    if (!is_syncing_bucket_meta(bucket)) {
        RGWObjVersionTracker objv_tracker;
        r = rgw_bucket_instance_remove_entry(this, bucket.get_key(),
                                             &objv_tracker);
        if (r < 0) {
            return r;
        }
...
int RGWRados::set_buckets_enabled(vector < rgw_bucket > &buckets, bool enabled)
{
    ...
    for (iter = buckets.begin(); iter != buckets.end(); ++iter) {
        ...
        RGWObjectCtx obj_ctx(this);
        int r =
            get_bucket_info(obj_ctx, bucket.tenant, bucket.name, info, NULL,
                            &attrs);
        if (r < 0) {
            ldout(cct,
                  0) << "NOTICE: get_bucket_info on bucket=" << bucket.
                name << " returned err=" << r << ", skipping bucket" << dendl;
            ret = r;
            continue;
        }
        if (enabled) {
...
int RGWRados::bucket_suspended(rgw_bucket & bucket, bool * suspended)
{
    ...
    RGWObjectCtx obj_ctx(this);
    int ret =
        get_bucket_info(obj_ctx, bucket.tenant, bucket.name, bucket_info, NULL);
    if (ret < 0) {
        return ret;
    }
...
int RGWRados::Object::complete_atomic_modification()
{
    ...
    string tag =
        (state->tail_tag.length() >
         0 ? state->tail_tag.to_str() : state->obj_tag.to_str());
    return store->gc->send_chain(chain, tag, false);    // do it async
}

void RGWRados::update_gc_chain(rgw_obj & head_obj, RGWObjManifest & manifest,
                               cls_rgw_obj_chain * chain)
{
                                ...
                                map < int, string > *bucket_instance_ids)
{
    ...
    get_bucket_index_objects(bucket_oid_base, bucket_info.num_shards,
...
template < typename T >
    int RGWRados::open_bucket_index(const RGWBucketInfo & bucket_info,
                                    librados::IoCtx & index_ctx, map < int,
                                    string > &oids, map < int, T > &bucket_objs,
                                    int shard_id, map < int,
                                    string > *bucket_instance_ids)
{
    int ret =
        open_bucket_index(bucket_info, index_ctx, oids, shard_id,
                          bucket_instance_ids);
    if (ret < 0)
        return ret;
                                      ...
                                      string * bucket_obj, int *shard_id)
{
    ...
    ret =
        get_bucket_index_object(bucket_oid_base, obj_key,
                                bucket_info.num_shards,
                                (RGWBucketInfo::BIShardsHashType) bucket_info.
                                bucket_index_shard_hash_type, bucket_obj,
                                shard_id);
    if (ret < 0) {
        ldout(cct,
              10) << "get_bucket_index_object() returned ret=" << ret << dendl;
        return ret;
    }
    return 0;
}
...
int RGWRados::open_bucket_index_shard(const RGWBucketInfo & bucket_info,
                                      librados::IoCtx & index_ctx, int shard_id,
                                      string * bucket_obj)
{
    string bucket_oid_base;
    int ret = open_bucket_index_base(bucket_info, index_ctx, bucket_oid_base);
                                 ...
                                 map < RGWObjCategory, RGWStorageStats > &stats)
{
  for (const auto & pair:header.stats) {
        const RGWObjCategory category =
            static_cast < RGWObjCategory > (pair.first);
...
int RGWRados::bucket_check_index(RGWBucketInfo & bucket_info,
                                 map < RGWObjCategory,
                                 RGWStorageStats > *existing_stats,
                                 map < RGWObjCategory,
                                 RGWStorageStats > *calculated_stats)
{
    librados::IoCtx index_ctx;
    ...
    ret =
        CLSRGWIssueBucketCheck(index_ctx, oids, bucket_objs_ret,
                               cct->_conf->rgw_bucket_index_max_aio) ();
    if (ret < 0) {
        return ret;
    }
...
int RGWRados::bucket_set_reshard(const RGWBucketInfo & bucket_info,
                                 const cls_rgw_bucket_instance_entry & entry)
{
    ...
    return CLSRGWIssueSetBucketResharding(index_ctx, bucket_objs, entry,
                                          cct->_conf->
                                          rgw_bucket_index_max_aio) ();
}

int RGWRados::defer_gc(void *ctx, const RGWBucketInfo & bucket_info,
                       const rgw_obj & obj)
{
    ...
    if (state->tail_tag.length() > 0) {
        ...
        ldout(cct,
              20) << "state->obj_tag is empty, not deferring gc operation" <<
            dendl;
        return -EINVAL;
    }
...
void RGWRados::cls_obj_check_prefix_exist(ObjectOperation & op,
                                          const string & prefix,
                                          bool fail_if_exist)
{
...
void RGWRados::cls_obj_check_mtime(ObjectOperation & op,
...
int RGWRados::Object::Delete::delete_obj()
{
    ...
    if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
        if (instance.empty() || explicit_marker_version) {
            ...
            else if ((params.versioning_status & BUCKET_VERSIONS_SUSPENDED) ==
                     0) {
            ...
            int r =
                store->set_olh(target->get_ctx(), target->get_bucket_info(),
                               marker, true, &meta, params.olh_epoch,
                               params.unmod_since, params.high_precision_time,
                               params.zones_trace);
            if (r < 0) {
                return r;
            }
            ...
            int r =
                store->bi_get_instance(target->get_bucket_info(), obj, &dirent);
            if (r < 0) {
                return r;
            }
            result.delete_marker = dirent.is_delete_marker();
            r = store->unlink_obj_instance(target->get_ctx(),
                                           target->get_bucket_info(), obj,
                                           params.olh_epoch,
                                           params.zones_trace);
            if (r < 0) {
                return r;
        ...
        if (r < 0) {
            ldout(store->ctx(),
                  5) << "failed to get BucketShard object: r=" << r << dendl;
            return r;
        }
    ...
    if (!real_clock::is_zero(params.unmod_since)) {
        ...
        struct timespec unmod =
            ceph::real_clock::to_timespec(params.unmod_since);
        ...
        ldout(store->ctx(),
            ...
            unmod_since << " Last-Modified: " << ctime << dendl;
        if (ctime > unmod) {
            return -ERR_PRECONDITION_FAILED;
        }

        /* only delete object if mtime is less than or equal to params.unmod_since */
        store->cls_obj_check_mtime(op, params.unmod_since,
    ...
    if (!real_clock::is_zero(params.expiration_time)) {
        ...
        if (state->get_attr(RGW_ATTR_DELETE_AT, bl)) {
            try {
                ...
                ldout(store->ctx(),
                      0) << "ERROR: couldn't decode RGW_ATTR_DELETE_AT" <<
                    dendl;
                return -EIO;
            }
    ...
    if (r >= 0) {
        ...
        r = index_op.complete_del(poolid, ref.ioctx.get_last_version(),
                                  state->mtime, params.remove_objs);

        int ret = target->complete_atomic_modification();
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: complete_atomic_modification returned ret=" <<
                ret << dendl;
        }
    ...
    /* update quota cache */
    store->quota_handler->update_stats(params.bucket_owner, obj.bucket, -1, 0,
                                       obj_size);
...
int RGWRados::delete_system_obj(rgw_raw_obj & obj,
                                RGWObjVersionTracker * objv_tracker)
{
    if (obj.empty()) {
...
int RGWRados::delete_obj_index(const rgw_obj & obj)
{
    ...
    RGWBucketInfo bucket_info;
    int ret =
        get_bucket_instance_info(obj_ctx, obj.bucket, bucket_info, NULL, NULL);
    if (ret < 0) {
        ldout(cct,
              0) << "ERROR: " << __func__ <<
            "() get_bucket_instance_info(bucket=" << obj.
            bucket << ") returned ret=" << ret << dendl;
        return ret;
    }
...
static void generate_fake_tag(RGWRados * store, map < string,
                              bufferlist > &attrset, RGWObjManifest & manifest,
...
int RGWRados::get_olh_target_state(RGWObjectCtx & obj_ctx,
                                   const RGWBucketInfo & bucket_info,
                                        ...
                                        RGWObjVersionTracker * objv_tracker)
{
    ...
    ldout(cct,
          20) << "get_system_obj_state: rctx=" << (void *)rctx << " obj=" << obj
        << " state=" << (void *)s << " s->prefetch_data=" << s->
        prefetch_data << dendl;
    *state = s;
    if (s->has_attrs) {
        return 0;
    ...
    int r =
        raw_obj_stat(obj, &s->size, &s->mtime, &s->epoch, &s->attrset,
                                 ...
                                 bool follow_olh, bool assume_noent)
{
    ...
    ldout(cct,
          20) << "get_obj_state: rctx=" << (void *)rctx << " obj=" << obj <<
        " state=" << (void *)s << " s->prefetch_data=" << s->
        prefetch_data << dendl;
    *state = s;
    if (s->has_attrs) {
    ...
    if (!assume_noent) {
        r = RGWRados::raw_obj_stat(raw_obj, &s->size, &s->mtime, &s->epoch,
                                   &s->attrset,
    ...
    if (r == -ENOENT) {
        ...
        if (obj_tombstone_cache && obj_tombstone_cache->find(obj, entry)) {
            ...
            ldout(cct,
                  20) << __func__ << "(): found obj in tombstone cache: obj=" <<
                obj << " mtime=" << s->mtime << " pgv=" << s->pg_ver << dendl;
        }
        else {
            s->mtime = real_time();
        }
    ...
    if (compressed) {
        ...
        try {
            ...
            dout(0) << "ERROR: could not decode compression info for object: "
                << obj << dendl;
            return -EIO;
        }
    }
    ...
    if (manifest_bl.length()) {
        ...
        ldout(cct,
              10) << "manifest: total_size = " << s->manifest.
            get_obj_size() << dendl;
        if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 20)
            && s->manifest.has_explicit_objs()) {
            RGWObjManifest::obj_iterator mi;
            for (mi = s->manifest.obj_begin(); mi != s->manifest.obj_end();
                 ++mi) {
                ldout(cct,
                      20) << "manifest: ofs=" << mi.get_ofs() << " loc=" << mi.
        ...
        if (!s->obj_tag.length()) {
             ...
             */
            generate_fake_tag(this, s->attrset, s->manifest, manifest_bl,
                              s->obj_tag);
            s->fake_tag = true;
        }
    }
    map < string, bufferlist >::iterator aiter =
        s->attrset.find(RGW_ATTR_PG_VER);
    if (aiter != s->attrset.end()) {
        ...
        if (pg_ver_bl.length()) {
            ...
            try {
                ...
                ldout(cct,
                      0) << "ERROR: couldn't decode pg ver attr for object " <<
                    s->obj << ", non-critical error, ignoring" << dendl;
            }
        }
    }
    ...
    if (aiter != s->attrset.end()) {
        ...
        if (zone_short_id_bl.length()) {
            ...
            try {
                ...
                ldout(cct,
                      0) <<
                    "ERROR: couldn't decode zone short id attr for object " <<
                    s->obj << ", non-critical error, ignoring" << dendl;
            }
        }
    }
    if (s->obj_tag.length())
        ldout(cct,
              20) << "get_obj_state: setting s->obj_tag to " << s->obj_tag.
            c_str() << dendl;
    else
    ...
    if (is_olh(s->attrset)) {
        ...
        ldout(cct,
              20) << __func__ << ": setting s->olh_tag to " << string(s->
                                                                      olh_tag.
                                                                      c_str(),
                                                                      s->
                                                                      olh_tag.
...
int RGWRados::get_obj_state(RGWObjectCtx * rctx,
                            const RGWBucketInfo & bucket_info,
                            const rgw_obj & obj, RGWObjState ** state,
                            bool follow_olh, bool assume_noent)
{
    ...
    do {
        ret =
            get_obj_state_impl(rctx, bucket_info, obj, state, follow_olh,
                               assume_noent);
    } while (ret == -EAGAIN);
...
int RGWRados::Object::Stat::stat_async()
{
    ...
    int r =
        store->get_obj_head_ioctx(source->get_bucket_info(), obj,
                                  &state.io_ctx);
    if (r < 0) {
        return r;
    }
...
{
    map < string, bufferlist >::iterator iter =
        result.attrs.find(RGW_ATTR_MANIFEST);
    if (iter != result.attrs.end()) {
        ...
        try {
            ...
            RGWRados *store = source->get_store();
            ldout(store->ctx(),
                  0) << "ERROR: " << __func__ << ": failed to decode manifest"
                << dendl;
            return -EIO;
        }
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::system_obj_get_attr(rgw_raw_obj & obj, const char *name,
                                  bufferlist & dest)
{
    rgw_rados_ref ref;
    int r = get_system_obj_ref(obj, &ref);
...
int RGWRados::append_atomic_test(RGWObjectCtx * rctx,
                                 const RGWBucketInfo & bucket_info,
                                 const rgw_obj & obj, ObjectOperation & op,
                                 RGWObjState ** pstate)
{
    if (!rctx)
        return 0;
    ...
    if (state->obj_tag.length() > 0 && !state->fake_tag) {  // check for backward compatibility
        ...
        ldout(cct,
              20) << "state->obj_tag is empty, not appending atomic test" <<
            dendl;
    }
    return 0;
}
                                                  ...
                                                  bool modify_tail)
{
    ...
    if (!state->is_atomic) {
        ldout(store->ctx(),
              20) << "prepare_atomic_modification: state is not atomic. state="
            << (void *)state << dendl;
    ...
    if (need_guard) {
        ...
        if (if_nomatch == NULL || strcmp(if_nomatch, "*") != 0) {
            op.cmpxattr(RGW_ATTR_ID_TAG, LIBRADOS_CMPXATTR_OP_EQ,
                        state->obj_tag);
    ...
    ldout(store->ctx(),
          10) << "setting object write_tag=" << state->write_tag << dendl;
...
int RGWRados::system_obj_set_attr(void *ctx, rgw_raw_obj & obj,
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::set_attr(void *ctx, const RGWBucketInfo & bucket_info,
                       rgw_obj & obj, const char *name, bufferlist & bl)
{
    map < string, bufferlist > attrs;
    attrs[name] = bl;
                        ...
                        map < string, bufferlist > *rmattrs)
{
    ...
    for (iter = attrs.begin(); iter != attrs.end(); ++iter) {
        ...
        if (name.compare(RGW_ATTR_DELETE_AT) == 0) {
            ...
            try {
                ...
                objexp_hint_add(ts, bucket.tenant, bucket.name,
                                bucket.bucket_id, obj_key);
            }
            catch(buffer::error & err) {
                ldout(cct,
    ...
    if (state) {
        if (r >= 0) {
            ...
            string content_type(content_type_bl.c_str(),
                                content_type_bl.length());
            uint64_t epoch = ref.ioctx.get_last_version();
            int64_t poolid = ref.ioctx.get_id();
            r = index_op.complete(poolid, epoch, state->size,
                                  state->accounted_size, mtime, etag,
                                  content_type, &acl_bl, RGW_OBJ_CATEGORY_MAIN,
                                  NULL);
        }
        else {
            int ret = index_op.cancel();
...
int RGWRados::Object::Read::prepare()
{
    ...
    if (params.attrs) {
        *params.attrs = astate->attrset;
        if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 20)) {
            for (iter = params.attrs->begin(); iter != params.attrs->end();
                 ++iter) {
    ...
    if (conds.mod_ptr || conds.unmod_ptr) {
        ...
        if (conds.mod_ptr) {
            dest_weight.init(*conds.mod_ptr, conds.mod_zone_id,
                             conds.mod_pg_ver);
            ldout(cct,
        ...
        if (conds.unmod_ptr) {
            dest_weight.init(*conds.unmod_ptr, conds.mod_zone_id,
                             conds.mod_pg_ver);
            ldout(cct,
    ...
    if (conds.if_match || conds.if_nomatch) {
        ...
        if (conds.if_match) {
            ...
            ldout(cct,
                  10) << "ETag: " << etag.
                c_str() << " " << " If-Match: " << if_match_str << dendl;
        ...
        if (conds.if_nomatch) {
            ...
            ldout(cct,
                  10) << "ETag: " << etag.
                c_str() << " " << " If-NoMatch: " << if_nomatch_str << dendl;
...
int RGWRados::Object::Read::range_to_ofs(uint64_t obj_size, int64_t & ofs,
                                         int64_t & end)
{
    if (ofs < 0) {
        ofs += obj_size;
...
int RGWRados::SystemObject::get_state(RGWRawObjState ** pstate,
                                      RGWObjVersionTracker * objv_tracker)
{
...
int RGWRados::Bucket::UpdateIndex::guard_reshard(BucketShard ** pbs,
                                                 std::function <
                                                 int (BucketShard *) > call)
{
    RGWRados *store = target->get_store();
    BucketShard *bs;
    int r;
...
#define NUM_RESHARD_RETRIES 10
    for (int i = 0; i < NUM_RESHARD_RETRIES; ++i) {
        ...
        ldout(store->ctx(),
              20) << "reshard completion identified, new_bucket_id=" <<
            new_bucket_id << dendl;
        i = 0;                  /* resharding is finished, make sure we can retry */
        r = target->update_bucket_id(new_bucket_id);
        if (r < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: update_bucket_id() new_bucket_id=" <<
                new_bucket_id << " returned r=" << r << dendl;
            return r;
        }
        invalidate_bs();
    }
...
int RGWRados::SystemObject::Read::stat(RGWObjVersionTracker * objv_tracker)
{
    ...
    return store->stat_system_obj(source->get_ctx(), state, obj,
                                  stat_params.attrs, stat_params.lastmod,
                                  stat_params.obj_size, objv_tracker);
}
                                           ...
                                           const string * write_tag)
{
    ...
    int r = guard_reshard(nullptr,[&](BucketShard * bs)->int {
                          return store->cls_obj_prepare_op(*bs, op, optag, obj,
                                                           bilog_flags,
                                                           zones_trace);}
    );
                                            ...
                                            const string * user_data)
{
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              5) << "failed to get BucketShard object: ret=" << ret << dendl;
        return ret;
    }
    ...
    if (acl_bl && acl_bl->length()) {
        ...
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << "WARNING: could not decode policy ret=" << ret << dendl;
        }
    }
    ...
    ret =
        store->cls_obj_complete_add(*bs, obj, optag, poolid, epoch, ent,
                                    category, remove_objs, bilog_flags,
                                    zones_trace);
                                                ...
                                                *remove_objs)
{
    ...
    ret =
        store->cls_obj_complete_del(*bs, optag, poolid, epoch, obj,
                                    removed_mtime, remove_objs, bilog_flags,
                                    zones_trace);
...
int RGWRados::Bucket::UpdateIndex::cancel()
{
    ...
    int ret = guard_reshard(&bs,[&](BucketShard * bs)->int {
                            return store->cls_obj_complete_cancel(*bs, optag,
                                                                  obj,
                                                                  bilog_flags,
                                                                  zones_trace);}
    );
...
int RGWRados::Object::Read::read(int64_t ofs, int64_t end, bufferlist & bl)
{
    ...
    if (r < 0) {
        ldout(cct,
              0) << "ERROR: failed to get max_chunk_size() for pool " <<
            read_obj.pool << dendl;
        return r;
    }
    ...
    if (reading_from_head) {
        ...
        r = store->append_atomic_test(&source->get_ctx(),
                                      source->get_bucket_info(), state.obj, op,
                                      &astate);
        if (r < 0)
            return r;
        ...
        if (astate && astate->prefetch_data) {
            ...
            if (ofs < astate->data.length()) {
                unsigned copy_len =
                    min((uint64_t) astate->data.length() - ofs, len);
    ...
    ldout(cct,
          20) << "rados->read r=" << r << " bl.length=" << bl.length() << dendl;
...
int RGWRados::SystemObject::Read::GetObjState::get_ref(RGWRados * store,
                                                       rgw_raw_obj & obj,
                                                       rgw_rados_ref ** pref)
{
    if (!has_ref) {
...
int RGWRados::get_system_obj(RGWObjectCtx & obj_ctx,
                             RGWRados::SystemObject::Read::
                             GetObjState & read_state,
                             RGWObjVersionTracker * objv_tracker,
                             rgw_raw_obj & obj, bufferlist & bl, off_t ofs,
                             off_t end, map < string, bufferlist > *attrs,
                             ...
                             boost::optional < obj_version >)
{
    ...
    if (r < 0) {
        ldout(cct,
              20) << "read_state.get_ref() on obj=" << obj << " returned " << r
            << dendl;
        return r;
    }
    r = ref->ioctx.operate(ref->oid, &op, NULL);
    if (r < 0) {
        ldout(cct,
              20) << "rados->read r=" << r << " bl.length=" << bl.
            length() << dendl;
        return r;
    }
    ldout(cct,
...
int RGWRados::SystemObject::Read::read(int64_t ofs, int64_t end,
                                       bufferlist & bl,
                                       RGWObjVersionTracker * objv_tracker,
                                       boost::optional < obj_version >
                                       refresh_version)
{
    ...
    return store->get_system_obj(source->get_ctx(), state, objv_tracker, obj,
                                 bl, ofs, end, read_params.attrs,
...
struct get_obj_data:public RefCountedObject {
    ...
    :cct(_cct),
        rados(NULL), ctx(NULL),
        total_read(0), lock("get_obj_data"),
        data_lock("get_obj_data::data_lock"), client_cb(NULL), throttle(cct,
                                                                        "get_obj_data",
                                                                        cct->
                                                                        _conf->
    ...
    int wait_next_io(bool * done) {
        lock.Lock();
        map < off_t, librados::AioCompletion * >::iterator iter =
            completion_map.begin();
    ...
    void add_io(off_t ofs, off_t len, bufferlist ** pbl, AioCompletion ** pc) {
        ...
        const auto & io_iter =
            io_map.insert(map < off_t,
                          get_obj_io >::value_type(ofs, get_obj_io()));
        ...
        *pbl = &io.bl;
        ...
        librados::AioCompletion * c =
            librados::Rados::aio_create_completion((void *)paio_data, NULL,
    ...
    void cancel_io(off_t ofs) {
        ...
        lock.Lock();
        map < off_t, AioCompletion * >::iterator iter =
            completion_map.find(ofs);
        if (iter != completion_map.end()) {
    ...
    void cancel_all_io() {
        ...
        Mutex::Locker l(lock);
        for (map < off_t, librados::AioCompletion * >::iterator iter =
             completion_map.begin(); iter != completion_map.end(); ++iter) {
            librados::AioCompletion * c = iter->second;
            c->release();
        }
    }
    ...
    int get_complete_ios(off_t ofs, list < bufferlist > &bl_list) {
        ...
        if (liter == io_map.end() || liter->first != ofs) {
            return 0;
        }
...
static int _get_obj_iterate_cb(const RGWBucketInfo & bucket_info,
                               const rgw_obj & obj,
                               ...
                               RGWObjState * astate, void *arg)
{
    ...
    return d->rados->get_obj_iterate_cb(d->ctx, astate, bucket_info, obj,
                                        read_obj, obj_ofs, read_ofs, len,
                                        is_head_obj, arg);
}
...
void RGWRados::get_obj_aio_completion_cb(completion_t c, void *arg)
{
    ...
    if (r < 0) {
        ldout(cct,
              0) << "ERROR: got unexpected error when trying to read object: "
            << r << dendl;
        d->set_cancelled(r);
        goto done;
    }
...
int RGWRados::flush_read_list(struct get_obj_data *d)
{
    ...
    for (iter = l.begin(); iter != l.end(); ++iter) {
        ...
        if (r < 0) {
            dout(0) <<
                "ERROR: flush_read_list(): d->client_cb->handle_data() returned "
                << r << dendl;
            break;
        }
    }
                                 ...
                                 bool is_head_obj, void *arg)
{
    ...
    if (is_head_obj) {
        ...
        if (astate && obj_ofs < astate->data.length()) {
            unsigned chunk_len =
                min((uint64_t) astate->data.length() - obj_ofs, (uint64_t) len);
    ...
    ldout(cct,
        ...
        oid << " obj-ofs=" << obj_ofs << " read_ofs=" << read_ofs << " len=" <<
        len << dendl;
  ...
  done_err:
    ldout(cct,
          20) << "cancelling io r=" << r << " obj_ofs=" << obj_ofs << dendl;
    d->set_cancelled(r);
    d->cancel_io(obj_ofs);
...
int RGWRados::Object::Read::iterate(int64_t ofs, int64_t end, RGWGetDataCB * cb)
{
    ...
    int r =
        store->iterate_obj(obj_ctx, source->get_bucket_info(), state.obj, ofs,
                           end, cct->_conf->rgw_get_obj_max_req_size,
    ...
    while (!done) {
        ...
        if (r < 0) {
            dout(10) << "get_obj_iterate() r=" << r << ", canceling all io" <<
                dendl;
            data->cancel_all_io();
            break;
        }
...
int RGWRados::iterate_obj(RGWObjectCtx & obj_ctx,
                          ...
                          uint64_t max_chunk_size,
                          int (*iterate_obj_cb) (const RGWBucketInfo &,
                                                 const rgw_obj & obj,
                                                 const rgw_raw_obj &, off_t,
                                                 off_t, off_t, bool,
                                                 RGWObjState *, void *),
                          void *arg)
{
    ...
    if (astate->has_manifest) {
        ...
        for (; iter != obj_end && ofs <= end; ++iter) {
            ...
            while (ofs < next_stripe_ofs && ofs <= end) {
                ...
                uint64_t read_len =
                    min(len, iter.get_stripe_size() - (ofs - stripe_ofs));
                read_ofs = iter.location_ofs() + (ofs - stripe_ofs);
                ...
                r = iterate_obj_cb(bucket_info, obj, read_obj, ofs, read_ofs,
                                   read_len, reading_from_head, astate, arg);
                if (r < 0) {
                    return r;
                }
        ...
        while (ofs <= end) {
            ...
            r = iterate_obj_cb(bucket_info, obj, read_obj, ofs, ofs, read_len,
...
int RGWRados::obj_operate(const RGWBucketInfo & bucket_info,
                          const rgw_obj & obj, ObjectWriteOperation * op)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(bucket_info, obj, &ref);
    if (r < 0) {
...
int RGWRados::obj_operate(const RGWBucketInfo & bucket_info,
                          const rgw_obj & obj, ObjectReadOperation * op)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(bucket_info, obj, &ref);
    if (r < 0) {
...
int RGWRados::olh_init_modification_impl(const RGWBucketInfo & bucket_info,
                                         RGWObjState & state,
                                         ...
                                         string * op_tag)
{
    ...
    if (!has_tag) {
        ...
        if (ret < 0) {
            ldout(cct,
                  0) << "ERROR: gen_rand_alphanumeric_lower() returned ret=" <<
                ret << dendl;
            return ret;
        }
        bufferlist bl;
...
#define OLH_PENDING_TAG_LEN 32
    ...
    *op_tag = buf;
    ...
    string s;
    int ret =
        gen_rand_alphanumeric_lower(cct, &s,
                                    OLH_PENDING_TAG_LEN - op_tag->size());
    if (ret < 0) {
        ldout(cct,
...
int RGWRados::olh_init_modification(const RGWBucketInfo & bucket_info,
                                    RGWObjState & state, const rgw_obj & obj,
                                    string * op_tag)
{
    int ret;
                                    ...
                                    bool log_data_change)
{
    ...
    r = guard_reshard(&bs, obj_instance, bucket_info,[&](BucketShard * bs)->int {
                      librados::ObjectWriteOperation op;
                      cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
                      return cls_rgw_bucket_link_olh(bs->index_ctx, op,
                                                     bs->bucket_obj, key,
                                                     olh_state.olh_tag,
                                                     delete_marker, op_tag,
                                                     meta, olh_epoch,
                                                     unmod_since,
                                                     high_precision_time,
                                                     get_zone().log_data,
...
void RGWRados::bucket_index_guard_olh_op(RGWObjState & olh_state,
                                         ObjectOperation & op)
{
    ldout(cct,
...
int RGWRados::bucket_index_unlink_instance(const RGWBucketInfo & bucket_info,
                                           const rgw_obj & obj_instance,
                                           const string & op_tag,
                                           ...
                                           rgw_zone_set * _zones_trace)
{
    ...
    r = guard_reshard(&bs, obj_instance, bucket_info,[&](BucketShard * bs)->int {
                      librados::ObjectWriteOperation op;
                      cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
                      return cls_rgw_bucket_unlink_instance(bs->index_ctx, op,
                                                            bs->bucket_obj, key,
                                                            op_tag, olh_tag,
                                                            olh_epoch,
                                                            get_zone().log_data,
...
int RGWRados::bucket_index_read_olh_log(const RGWBucketInfo & bucket_info,
                                        RGWObjState & state,
                                        const rgw_obj & obj_instance,
                                        uint64_t ver_marker, map < uint64_t,
                                        vector < rgw_bucket_olh_log_entry >
                                        >*log, bool * is_truncated)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(bucket_info, obj_instance, &ref);
    if (r < 0) {
    ...
    BucketShard bs(this);
    int ret =
        bs.init(obj_instance.bucket, obj_instance,
    ...
    ret = guard_reshard(&bs, obj_instance, bucket_info,
                        [&](BucketShard * bs)->int {
                        ObjectReadOperation op;
                        cls_rgw_guard_bucket_resharding(op,
                                                        -ERR_BUSY_RESHARDING);
                        return cls_rgw_get_olh_log(bs->index_ctx,
                                                   bs->bucket_obj, op, key,
                                                   ver_marker, olh_tag, log,
                                                   is_truncated);}
    );
    if (ret < 0) {
                         ...
                         const rgw_obj & obj)
{
    ...
    if (r < 0) {
        ldout(cct,
              0) << "repair_olh failed to read olh entry for " << obj << dendl;
        return r;
    }
...
int RGWRados::bucket_index_trim_olh_log(const RGWBucketInfo & bucket_info,
                                        RGWObjState & state,
                                        const rgw_obj & obj_instance,
                                        uint64_t ver)
{
...
int RGWRados::bucket_index_clear_olh(const RGWBucketInfo & bucket_info,
                                     RGWObjState & state,
                                     const rgw_obj & obj_instance)
{
    rgw_rados_ref ref;
    ...
    int ret = guard_reshard(&bs, obj_instance, bucket_info,
                            ...
                            cls_rgw_guard_bucket_resharding(op,
                                                            -ERR_BUSY_RESHARDING);
                            return cls_rgw_clear_olh(pbs->index_ctx, op,
                                                     pbs->bucket_obj, key,
                                                     olh_tag);}
    );
    if (ret < 0) {
                            ...
                            uint64_t * plast_ver, rgw_zone_set * zones_trace)
{
    ...
    *plast_ver = last_ver;
    ...
    map < uint64_t, vector < rgw_bucket_olh_log_entry > >::iterator iter =
        log.begin();
    ...
    for (iter = log.begin(); iter != log.end(); ++iter) {
        vector < rgw_bucket_olh_log_entry >::iterator viter =
            iter->second.begin();
    ...
    for (list < cls_rgw_obj_key >::iterator liter = remove_instances.begin();
        ...
        int ret =
            delete_obj(obj_ctx, bucket_info, obj_instance, 0,
    ...
    if (need_to_remove) {
        ...
        if (r == -ECANCELED) {
            ...
            if (r < 0) {
                ldout(cct,
                      0) << "ERROR: could not clear bucket index olh entries r="
                    << r << dendl;
                return r;
            }
        }
 ...
 */
int RGWRados::update_olh(RGWObjectCtx & obj_ctx, RGWObjState * state,
                         ...
                         rgw_zone_set * zones_trace)
{
    ...
    do {
        int ret =
            bucket_index_read_olh_log(bucket_info, *state, obj, ver_marker,
                                      &log, &is_truncated);
        if (ret < 0) {
            return ret;
        }
        ret =
            apply_olh_log(obj_ctx, *state, bucket_info, obj, state->olh_tag,
                          log, &ver_marker, zones_trace);
        if (ret < 0) {
            return ret;
        }
    } while (is_truncated);
...
int RGWRados::set_olh(RGWObjectCtx & obj_ctx, RGWBucketInfo & bucket_info,
                      const rgw_obj & target_obj, bool delete_marker,
                      rgw_bucket_dir_entry_meta * meta, uint64_t olh_epoch,
...
#define MAX_ECANCELED_RETRY 100
    for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        if (ret < 0) {
            ldout(cct,
                ...
                " delete_marker=" << (int)delete_marker << " returned " << ret
                << dendl;
            if (ret == -ECANCELED) {
                continue;
            }
        ...
        ret =
            bucket_index_link_olh(bucket_info, *state, target_obj,
                                  delete_marker, op_tag, meta, olh_epoch,
                                  unmod_since, high_precision_time, zones_trace,
                                  log_data_change);
        if (ret < 0) {
            ldout(cct,
    ...
    if (i == MAX_ECANCELED_RETRY) {
        ldout(cct,
              0) << "ERROR: exceeded max ECANCELED retries, aborting (EIO)" <<
            dendl;
        return -EIO;
    }
    ...
    if (ret < 0) {
        ldout(cct,
              20) << "update_olh() target_obj=" << target_obj << " returned " <<
            ret << dendl;
        return ret;
    }
...
int RGWRados::unlink_obj_instance(RGWObjectCtx & obj_ctx,
                                  RGWBucketInfo & bucket_info,
                                  ...
                                  rgw_zone_set * zones_trace)
{
    ...
    for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        ret =
            bucket_index_unlink_instance(bucket_info, target_obj, op_tag,
                                         olh_tag, olh_epoch, zones_trace);
        if (ret < 0) {
            ldout(cct,
                  20) << "bucket_index_unlink_instance() target_obj=" <<
...
static void filter_attrset(map < string, bufferlist > &unfiltered_attrset,
                           const string & check_prefix, map < string,
                           bufferlist > *attrset)
{
    attrset->clear();
    map < string, bufferlist >::iterator iter;
...
int RGWRados::get_olh(const RGWBucketInfo & bucket_info, const rgw_obj & obj,
                      RGWOLHInfo * olh)
{
...
void RGWRados::check_pending_olh_entries(map < string,
                                         bufferlist > &pending_entries,
                                         map < string,
                                         bufferlist > *rm_pending_entries)
{
...
int RGWRados::remove_olh_pending_entries(const RGWBucketInfo & bucket_info,
                                         RGWObjState & state,
                                         const rgw_obj & olh_obj, map < string,
                                         bufferlist > &pending_attrs)
{
    ObjectWriteOperation op;

    bucket_index_guard_olh_op(state, op);

    for (map < string, bufferlist >::iterator iter = pending_attrs.begin();
         iter != pending_attrs.end(); ++iter) {
...
int RGWRados::follow_olh(const RGWBucketInfo & bucket_info,
                         RGWObjectCtx & obj_ctx, RGWObjState * state,
                         const rgw_obj & olh_obj, rgw_obj * target)
{
    map < string, bufferlist > pending_entries;
    filter_attrset(state->attrset, RGW_ATTR_OLH_PENDING_PREFIX,
                   &pending_entries);
    ...
    if (!rm_pending_entries.empty()) {
        int ret =
            remove_olh_pending_entries(bucket_info, *state, olh_obj,
                                       rm_pending_entries);
        if (ret < 0) {
            ldout(cct,
    ...
    map < string, bufferlist >::iterator iter =
        state->attrset.find(RGW_ATTR_OLH_INFO);
...
int RGWRados::raw_obj_stat(rgw_raw_obj & obj, uint64_t * psize,
                           real_time * pmtime, uint64_t * epoch, map < string,
                           bufferlist > *attrs, bufferlist * first_chunk,
                           RGWObjVersionTracker * objv_tracker)
{
    rgw_rados_ref ref;
    int r = get_raw_obj_ref(obj, &ref);
...
int RGWRados::get_bucket_stats(RGWBucketInfo & bucket_info, int shard_id,
                               ...
                               string * max_marker, bool * syncstopped)
{
    ...
    map < int, string > bucket_instance_ids;
    int r =
        cls_bucket_head(bucket_info, shard_id, headers, &bucket_instance_ids);
    if (r < 0) {
        return r;
    }
...
int RGWRados::get_bucket_stats_async(RGWBucketInfo & bucket_info, int shard_id,
                                     RGWGetBucketStats_CB * ctx)
{
    int num_aio = 0;
    RGWGetBucketStatsContext *get_ctx =
        new RGWGetBucketStatsContext(ctx, bucket_info.num_shards ? : 1);
    assert(get_ctx);
...
int RGWRados::get_user_stats_async(const rgw_user & user,
                                   RGWGetUserStats_CB * ctx)
{
    string user_str = user.to_str();
...
void RGWRados::get_bucket_instance_obj(const rgw_bucket & bucket,
                                       rgw_raw_obj & obj)
{
    if (!bucket.oid.empty()) {
...
int RGWRados::get_bucket_instance_info(RGWObjectCtx & obj_ctx,
                                       ...
                                       RGWBucketInfo & info, real_time * pmtime,
                                       map < string, bufferlist > *pattrs)
{
...
{
    ldout(cct,
          20) << "reading from " << get_zone_params().
        domain_root << ":" << oid << dendl;
    ...
    try {
        ...
        ldout(cct,
              0) << "ERROR: could not decode buffer info, caught buffer::error"
            << dendl;
        return -EIO;
    }
    info.bucket.oid = oid;
                                      ...
                                      const string & bucket_name)
{
    ...
    int ret =
        get_bucket_entrypoint_info(obj_ctx, tenant_name, bucket_name,
    ...
    ret =
        put_linked_bucket_info(info, false, ep_mtime, &ot.write_version, &attrs,
                               true);
    if (ret < 0) {
        ldout(cct,
                                      ...
                                      map < string, bufferlist > *pattrs)
{
    ...
    return _get_bucket_info(obj_ctx, info.bucket.tenant, info.bucket.name,
                            info, pmtime, pattrs,
                            info.objv_tracker.read_version);
}

int RGWRados::put_bucket_entrypoint_info(const string & tenant_name,
                                         const string & bucket_name,
                                         RGWBucketEntryPoint & entry_point,
                                         bool exclusive,
                                         RGWObjVersionTracker & objv_tracker,
                                         real_time mtime, map < string,
                                         bufferlist > *pattrs)
{
    bufferlist epbl;
    ...
    string bucket_entry;
    rgw_make_bucket_entry_name(tenant_name, bucket_name, bucket_entry);
    return rgw_bucket_store_info(this, bucket_entry, epbl, exclusive, pattrs,
                                 &objv_tracker, mtime);
}
                                       ...
                                       bufferlist > *pattrs)
{
    ...
    int ret =
        rgw_bucket_instance_store_info(this, key, bl, exclusive, pattrs,
                                       &info.objv_tracker, mtime);
    if (ret == -EEXIST) {
...
int RGWRados::put_linked_bucket_info(RGWBucketInfo & info, bool exclusive,
                                     real_time mtime, obj_version * pep_objv,
                                     map < string, bufferlist > *pattrs,
                                     bool create_entry_point)
{
    ...
    ret =
        put_bucket_entrypoint_info(info.bucket.tenant, info.bucket.name,
                                   entry_point, exclusive, ot, mtime, NULL);
    if (ret < 0)
        return ret;
...
int RGWRados::omap_get_vals(rgw_raw_obj & obj, bufferlist & header,
...
int RGWRados::omap_set(rgw_raw_obj & obj, const std::string & key,
                       bufferlist & bl)
{
    rgw_rados_ref ref;
    int r = get_raw_obj_ref(obj, &ref);
...
int RGWRados::omap_set(rgw_raw_obj & obj, std::map < std::string,
                       bufferlist > &m)
{
    rgw_rados_ref ref;
    int r = get_raw_obj_ref(obj, &ref);
...
int RGWRados::update_containers_stats(map < string, RGWBucketEnt > &m)
{
    ...
    for (iter = m.begin(); iter != m.end(); ++iter) {
        ...
        RGWBucketInfo bucket_info;
        int ret =
            get_bucket_instance_info(obj_ctx, bucket, bucket_info, NULL, NULL);
        if (ret < 0) {
            return ret;
        }
        ...
        for (; hiter != headers.end(); ++hiter) {
            ...
            map < uint8_t, struct rgw_bucket_category_stats >::iterator iter =
                (hiter->stats).find((uint8_t) category);
...
int RGWRados::append_async(rgw_raw_obj & obj, size_t size, bufferlist & bl)
{
    ...
    librados::Rados * rad = get_rados_handle();
    librados::AioCompletion * completion =
        rad->aio_create_completion(NULL, NULL, NULL);
...
int RGWRados::pool_iterate_begin(const rgw_pool & pool, const string & cursor,
                                 RGWPoolIterCtx & ctx)
{
    librados::IoCtx & io_ctx = ctx.io_ctx;
    librados::NObjectIterator & iter = ctx.iter;
...
static int do_pool_iterate(CephContext * cct, RGWPoolIterCtx & ctx,
...
int RGWRados::pool_iterate(RGWPoolIterCtx & ctx, uint32_t num,
...
int RGWRados::list_raw_objects_init(const rgw_pool & pool,
                                    const string & marker,
                                    RGWListRawObjsCtx * ctx)
{
    if (!ctx->initialized) {
...
int RGWRados::list_raw_objects(const rgw_pool & pool,
                               const string & prefix_filter, int max,
...
int RGWRados::list_bi_log_entries(RGWBucketInfo & bucket_info, int shard_id,
                                  ...
                                  bool * truncated)
{
    ldout(cct,
        ...
        bucket << " marker " << marker << " shard_id=" << shard_id << " max " <<
        max << dendl;
    result.clear();
    ...
    r = CLSRGWIssueBILogList(index_ctx, marker_mgr, max, oids, bi_log_lists,
                             cct->_conf->rgw_bucket_index_max_aio) ();
    if (r < 0)
        return r;
...
int RGWRados::trim_bi_log_entries(RGWBucketInfo & bucket_info, int shard_id,
                                  string & start_marker, string & end_marker)
{
    ...
    return CLSRGWIssueBILogTrim(index_ctx, start_marker_mgr, end_marker_mgr,
                                bucket_objs,
                                cct->_conf->rgw_bucket_index_max_aio) ();
...
int RGWRados::resync_bi_log_entries(RGWBucketInfo & bucket_info, int shard_id)
{
    ...
    return CLSRGWIssueResyncBucketBILog(index_ctx, bucket_objs,
                                        cct->_conf->
                                        rgw_bucket_index_max_aio) ();
}
...
int RGWRados::stop_bi_log_entries(RGWBucketInfo & bucket_info, int shard_id)
{
    ...
    return CLSRGWIssueBucketBILogStop(index_ctx, bucket_objs,
                                      cct->_conf->rgw_bucket_index_max_aio) ();
}

int RGWRados::bi_get_instance(const RGWBucketInfo & bucket_info,
                              const rgw_obj & obj,
                              rgw_bucket_dir_entry * dirent)
{
...
void RGWRados::bi_put(ObjectWriteOperation & op, BucketShard & bs,
...
int RGWRados::bi_put(rgw_bucket & bucket, rgw_obj & obj,
                     rgw_cls_bi_entry & entry)
{
    BucketShard bs(this);
...
int RGWRados::bi_list(rgw_bucket & bucket, const string & obj_name,
                      ...
                      list < rgw_cls_bi_entry > *entries, bool * is_truncated)
{
    ...
    ret =
        cls_rgw_bi_list(bs.index_ctx, bs.bucket_obj, obj_name, marker, max,
...
int RGWRados::bi_list(BucketShard & bs, const string & filter_obj,
                      const string & marker, uint32_t max,
                      list < rgw_cls_bi_entry > *entries, bool * is_truncated)
{
    int ret =
        cls_rgw_bi_list(bs.index_ctx, bs.bucket_obj, filter_obj, marker, max,
                        entries, is_truncated);
    if (ret < 0)
        return ret;
...
int RGWRados::bi_remove(BucketShard & bs)
{
    ...
    if (ret < 0) {
        ldout(cct,
            ...
            bucket_obj << ") returned ret=" << ret << dendl;
        return ret;
    }
...
int RGWRados::bi_list(rgw_bucket & bucket, int shard_id,
                      const string & filter_obj, const string & marker,
...
int RGWRados::gc_operate(string & oid, librados::ObjectReadOperation * op,
                         bufferlist * pbl)
{
...
int RGWRados::cls_obj_complete_op(BucketShard & bs, const rgw_obj & obj,
                                  RGWModifyOp op, string & tag, int64_t pool,
                                  ...
                                  rgw_zone_set * _zones_trace)
{
    ...
    cls_rgw_bucket_complete_op(o, op, tag, ver, key, dir_meta, remove_objs,
                               get_zone().log_data, bilog_flags, &zones_trace);
    complete_op_data *arg;
    index_completion_manager->create_completion(obj, op, tag, ver, key,
                                                dir_meta, remove_objs,
                                                get_zone().log_data,
...
int RGWRados::cls_obj_complete_add(BucketShard & bs, const rgw_obj & obj,
                                   string & tag, int64_t pool, uint64_t epoch,
                                   rgw_bucket_dir_entry & ent,
                                   RGWObjCategory category,
                                   list < rgw_obj_index_key > *remove_objs,
                                   uint16_t bilog_flags,
                                   rgw_zone_set * zones_trace)
{
    return cls_obj_complete_op(bs, obj, CLS_RGW_OP_ADD, tag, pool, epoch, ent,
                               category, remove_objs, bilog_flags, zones_trace);
}
                                   ...
                                   rgw_zone_set * zones_trace)
{
    ...
    obj.key.get_index_key(&ent.key);
    return cls_obj_complete_op(bs, obj, CLS_RGW_OP_DEL, tag, pool, epoch, ent,
                               RGW_OBJ_CATEGORY_NONE, remove_objs, bilog_flags,
                               zones_trace);
}

int RGWRados::cls_obj_complete_cancel(BucketShard & bs, string & tag,
                                      rgw_obj & obj, uint16_t bilog_flags,
                                      rgw_zone_set * zones_trace)
{
    rgw_bucket_dir_entry ent;
    ...
    return cls_obj_complete_op(bs, obj, CLS_RGW_OP_CANCEL, tag,
                               -1 /* pool id */ , 0, ent, RGW_OBJ_CATEGORY_NONE,
                               NULL, bilog_flags, zones_trace);
}
                                             ...
                                             uint64_t timeout)
{
    ...
    return CLSRGWIssueSetTagTimeout(index_ctx, bucket_objs,
                                    cct->_conf->rgw_bucket_index_max_aio,
                                    timeout) ();
}
...
int RGWRados::cls_bucket_list_ordered(RGWBucketInfo & bucket_info,
                                      ...
                                      bool(*force_check_filter) (const string &
                                                                 name))
{
    ...
    vector < map < string,
        struct rgw_bucket_dir_entry >::iterator >
        vcurrents(list_results.size());
    vector < map < string,
        struct rgw_bucket_dir_entry >::iterator > vends(list_results.size());
...
int RGWRados::cls_bucket_list_unordered(RGWBucketInfo & bucket_info,
                                        ...
                                        uint32_t num_entries,
                                        bool list_versions,
                                        std::vector < rgw_bucket_dir_entry >
                                        &ent_list, bool * is_truncated,
                                        rgw_obj_index_key * last_entry,
                                        bool(*force_check_filter) (const string
                                                                   & name))
{
    ...
    *is_truncated = false;
    ...
    if (shard_id >= 0) {
        ...
        current_shard = rgw_bucket_shard_index(my_start.name, num_shards);
    }
                                     ...
                                     uint64_t start_epoch, uint64_t end_epoch)
{
    ...
    r = cls_rgw_usage_log_trim(ref.ioctx, ref.oid, user, start_epoch,
                               end_epoch);
    return r;
}
                                     ...
                                     list < rgw_obj_index_key > &oid_list)
{
    ...
    uint8_t suggest_flag =
        (get_zone().log_data ? CEPH_RGW_DIR_SUGGEST_LOG_OP : 0);
    ...
    for (auto iter = oid_list.begin(); iter != oid_list.end(); ++iter) {
        ...
        entry.key = *iter;
        dout(2) << "RGWRados::remove_objs_from_index bucket=" << bucket_info.
            bucket << " obj=" << entry.key.name << ":" << entry.key.
            instance << dendl;
    ...
    r = index_ctx.exec(dir_oid, RGW_CLASS, RGW_DIR_SUGGEST_CHANGES, updates,
                       out);
                               ...
                               bufferlist & suggested_updates)
{
    ...
    map < string, bufferlist >::iterator iter =
        astate->attrset.find(RGW_ATTR_ETAG);
    ...
    if (iter != astate->attrset.end()) {
        ...
        if (r < 0) {
            dout(0) << "WARNING: could not decode policy for object: " << obj <<
                dendl;
        }
    }
    ...
    if (astate->has_manifest) {
        ...
        for (miter = manifest.obj_begin(); miter != manifest.obj_end(); ++miter) {
            ...
            if (loc.key.ns == RGW_OBJ_NS_MULTIPART) {
                dout(10) <<
                    "check_disk_state(): removing manifest part from index: " <<
                    loc << dendl;
                r = delete_obj_index(loc);
                if (r < 0) {
                    dout(0) << "WARNING: delete_obj_index() returned r=" << r <<
                        dendl;
                }
            }
        }
    ...
    cls_rgw_encode_suggestion(CEPH_RGW_UPDATE | suggest_flag, list_state,
                              suggested_updates);
    return 0;
}

int RGWRados::cls_bucket_head(const RGWBucketInfo & bucket_info, int shard_id,
                              vector < rgw_bucket_dir_header > &headers,
                              map < int, string > *bucket_instance_ids)
{
    librados::IoCtx index_ctx;
    ...
    map < int, struct rgw_cls_list_ret >list_results;
    int r =
        open_bucket_index(bucket_info, index_ctx, oids, list_results, shard_id,
                          bucket_instance_ids);
    if (r < 0)
        return r;

    r = CLSRGWIssueGetDirHeader(index_ctx, oids, list_results,
                                cct->_conf->rgw_bucket_index_max_aio) ();
    if (r < 0)
        return r;
...
int RGWRados::cls_bucket_head_async(const RGWBucketInfo & bucket_info,
                                    int shard_id, RGWGetDirHeader_CB * ctx,
                                    int *num_aio)
{
    librados::IoCtx index_ctx;
    map < int, string > bucket_objs;
    ...
    for (; iter != bucket_objs.end(); ++iter) {
        r = cls_rgw_get_dir_header_async(index_ctx, iter->second,
                                         static_cast <
                                         RGWGetDirHeader_CB * >(ctx->get()));
        if (r < 0) {
            ctx->put();
            break;
        }
...
int RGWRados::cls_user_get_bucket_stats(const rgw_bucket & bucket,
                                        cls_user_bucket_entry & entry)
{
    vector < rgw_bucket_dir_header > headers;
    RGWBucketInfo bucket_info;
                                    ...
                                    bool * const truncated)
{
    ...
    cls_user_bucket_list(op, in_marker, end_marker, max_entries, entries,
                         out_marker, truncated, &rc);
    bufferlist ibl;
    r = ref.ioctx.operate(ref.oid, &op, &ibl);
    if (r < 0)
        return r;
...
int RGWRados::cls_user_update_buckets(rgw_raw_obj & obj,
                                      list < cls_user_bucket_entry > &entries,
                                      bool add)
{
    rgw_rados_ref ref;
...
int RGWRados::cls_user_add_bucket(rgw_raw_obj & obj,
...
int RGWRados::cls_user_remove_bucket(rgw_raw_obj & obj,
                                  ...
                                  RGWQuotaInfo & bucket_quota)
{
    ...
    int ret =
        quota_handler->check_bucket_shards((uint64_t) cct->_conf->
                                           rgw_max_objs_per_shard,
                                           num_source_shards, bucket_info.owner,
                                           bucket, bucket_quota,
                                           1, need_resharding,
    ...
    if (need_resharding) {
        ldout(cct,
              20) << __func__ << " bucket " << bucket.
            name << " need resharding " << " old num shards " << bucket_info.
            num_shards << " new num shards " << suggested_num_shards << dendl;
                                    ...
                                    uint32_t new_num_shards)
{
    ...
    if (new_num_shards <= num_source_shards) {
        ldout(cct,
              20) << "not resharding bucket name=" << bucket_info.bucket.
            name << ", orig_num=" << num_source_shards << ", new_num_shards=" <<
            new_num_shards << dendl;
        return 0;
    }
...
{
    return quota_handler->check_quota(bucket_owner, bucket, user_quota,
                                      bucket_quota, 1, obj_size);
}
                                        ...
                                        int shard_id)
{
    if (!num_shards) {
        ...
        if (shard_id < 0) {
            ...
            snprintf(buf, sizeof(buf), "%s.%d", bucket_oid_base.c_str(),
                     shard_id);
                                  ...
                                  const string & obj_key, int *shard_id)
{
    ...
    case RGWBucketInfo::MOD:
        if (!bucket_info.num_shards) {
            ...
            uint32_t sid =
                rgw_bucket_shard_index(obj_key, bucket_info.num_shards);
            if (shard_id) {
                *shard_id = (int)sid;
            }
        }
...
int RGWStateLog::store_entry(const string & client_id, const string & op_id,
                             const string & object, uint32_t state,
                             bufferlist * bl, uint32_t * check_state)
{
    if (client_id.empty() || op_id.empty() || object.empty()) {
        ldout(store->ctx(),
              0) << "client_id / op_id / object is empty" << dendl;
    }
    ...
    bufferlist nobl;
    cls_statelog_add(op, client_id, op_id, object, ts, state,
                     (bl ? *bl : nobl));
...
int RGWStateLog::remove_entry(const string & client_id, const string & op_id,
                              const string & object)
{
    if (client_id.empty() || op_id.empty() || object.empty()) {
        ldout(store->ctx(),
...
void RGWStateLog::init_list_entries(const string & client_id,
                              ...
                              list < cls_statelog_entry > &entries, bool * done)
{
    ...
    for (; state->cur_shard <= state->max_shard && max_entries > 0;
        ...
        if (r < 0) {
            ldout(store->ctx(),
                  0) << "cls_statelog_list returned " << r << dendl;
            return r;
        }
...
bool RGWOpState::dump_entry_internal(const cls_statelog_entry & entry,
                                     Formatter * f)
{
    string s;
...
int RGWOpState::set_state(const string & client_id, const string & op_id,
...
int RGWOpState::renew_state(const string & client_id, const string & op_id,
                            const string & object, OpState state)
{
    uint32_t s = (uint32_t) state;
...
RGWOpStateSingleOp::RGWOpStateSingleOp(RGWRados * store, const string & cid, const string & oid, const string & obj):os(store), client_id(cid), op_id(oid),
object
(obj)
{
    cct = store->ctx();
                                                 ...
                                                 bool use_cache)
{
    ...
    if (store->
        initialize(cct, use_gc_thread, use_lc_thread, quota_threads,
...
int RGWRados::delete_raw_obj_aio(const rgw_raw_obj & obj,
                             ...
                             bool keep_index_consistent)
{
    ...
    if (keep_index_consistent) {
        ...
        if (ret < 0) {
            lderr(cct) << "ERROR: failed to prepare index op with ret=" << ret
                << dendl;
            return ret;
        }
    }
    ...
    if (keep_index_consistent) {
        ...
        if (ret < 0) {
            lderr(cct) << "ERROR: failed to delete obj index with ret=" << ret
                << dendl;
            return ret;
        }
    }
...
int rgw_compression_info_from_attrset(map < string, bufferlist > &attrs,
                                      ...
                                      RGWCompressionInfo & cs_info)
{
    map < string, bufferlist >::iterator value =
        attrs.find(RGW_ATTR_COMPRESSION);
    if (value != attrs.end()) {
                    ...
                    bufferlist & out)
{
    if (command == "cache list") {
        ...
        std::unique_ptr < Formatter >
            f(ceph::Formatter::create(format, "table"));
        if (f) {
        ...
        std::unique_ptr < Formatter >
            f(ceph::Formatter::create(format, "json-pretty"));
        if (f) {
            ...
            if (call_inspect(target, f.get())) {
                ...
                out.append(string("Unable to find entry ") + target +
                           string(".\n"));
                return false;
            }
        }
