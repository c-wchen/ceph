...
namespace rgw::sal {
    ...
    int MotrMetaCache::put(const DoutPrefixProvider * dpp,
                           const string & name, const bufferlist & data) {
        ...
        if (rc < 0)
            ldpp_dout(dpp,
                      0) << "ERROR: failed to distribute cache for " << name <<
                dendl;
...
// with starting key `marker`.
    int MotrUser::list_buckets(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  20) << __func__ << ": list_user_buckets: marker=" << marker <<
      ...
      for (const auto & bl:vals) {
            ...
            buckets.add(std::make_unique < MotrBucket >
                        (this->store, ent, this));
            bcount++;
        }
        if (bcount == max)
    ...
    int MotrUser::create_bucket(const DoutPrefixProvider * dpp,
        ...
        if (!*existed) {
            ...
            if (ret < 0)
                ldpp_dout(dpp,
                          0) << "ERROR: failed to create bucket indices! " <<
                    ret << dendl;
            ...
            if (ret < 0)
                ldpp_dout(dpp,
                          0) << "ERROR: failed to add bucket entry! " << ret <<
                    dendl;
        }
        else {
            return -EEXIST;
...
/* stats - Not for first pass */
    int MotrUser::read_stats_async(const DoutPrefixProvider * dpp,
                                   RGWGetUserStats_CB * cb) {
        return 0;
    }

    int MotrUser::complete_flush_stats(const DoutPrefixProvider * dpp,
                                       optional_yield y) {
        return 0;
    }

    int MotrUser::read_usage(const DoutPrefixProvider * dpp,
    ...
    int MotrUser::trim_usage(const DoutPrefixProvider * dpp,
                             uint64_t start_epoch, uint64_t end_epoch) {
        return 0;
    }
    ...
    int MotrUser::load_user(const DoutPrefixProvider * dpp, optional_yield y) {
        ldpp_dout(dpp,
                  20) << "load user: user id =   " << info.user_id.
            to_str() << dendl;
    ...
    int MotrUser::merge_and_store_attrs(const DoutPrefixProvider * dpp,
    ...
    int MotrUser::store_user(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  20) << "Store_user(): User = " << info.user_id.id << dendl;
        ...
        if (rc == 0 && obj_ver.ver > 0) {
            ...
            if (obj_ver.ver != objv_tracker.read_version.ver) {
                rc = -ECANCELED;
                ldpp_dout(dpp,
                          0) << "ERROR: User Read version mismatch" << dendl;
                goto out;
            }
        ...
        if (!info.access_keys.empty()) {
            ...
            access_key = k.id;
            secret_key = k.key;
            MotrAccessKey MGWUserKeys(access_key, secret_key,
                                      info.user_id.to_str());
        ...
        if (access_key_tracker.size() != info.access_keys.size()) {
          ...
          for (auto key:access_key_tracker) {
                if (!info.get_key(key)) {
                    ...
                    ldpp_dout(dpp,
                              0) << "Deleting access key: " << key_for_deletion
                        << dendl;
                    store->delete_access_key(dpp, y, key_for_deletion);
                    if (rc < 0) {
                        ldpp_dout(dpp,
                                  0) << "Unable to delete access key" << rc <<
                            dendl;
                    }
                }
            }
        ...
        if (rc < 0 && rc != -EEXIST) {
            ldpp_dout(dpp,
                      0) << "Failed to create user info index: rc = " << rc <<
                dendl;
            goto out;
        }
    ...
    int MotrUser::remove_user(const DoutPrefixProvider * dpp, optional_yield y) {
        ...
        if (!info.user_email.empty()) {
            ...
            if (rc < 0 && rc != -ENOENT) {
                ldpp_dout(dpp,
                          0) << "Unable to delete email id " << rc << dendl;
            }
        }
        ...
        ldpp_dout(dpp,
                  10) << "Deleted user info index - " << user_info_iname <<
            dendl;
        ...
        if (rc < 0) {
            ldpp_dout(dpp,
                      0) << "Unable to delete user from user index " << rc <<
                dendl;
            return rc;
        }
    ...
    int MotrUser::verify_mfa(const std::string & mfa_str, bool * verified,
                             const DoutPrefixProvider * dpp, optional_yield y) {
        *verified = false;
        return 0;
    }

    int MotrBucket::remove_bucket(const DoutPrefixProvider * dpp,
                                  bool delete_children, bool forward_to_master,
                                  req_info * req_info, optional_yield y) {
        int ret;

        ldpp_dout(dpp,
                  20) << "remove_bucket Entry=" << info.bucket.name << dendl;

        // Refresh info
        ret = load_bucket(dpp, y);
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: remove_bucket load_bucket failed rc=" << ret
                << dendl;
            return ret;
        }
        ...
        do {
            ...
            if (!results.objs.empty() && !delete_children) {
                ldpp_dout(dpp,
                          0) << "ERROR: could not remove non-empty bucket " <<
          ...
          for (const auto & obj:results.objs) {
                ...
                if (ret < 0 && ret != -ENOENT) {
                    ldpp_dout(dpp,
                              0) <<
                        "ERROR: remove_bucket rgw_remove_object failed rc=" <<
                        ret << dendl;
                    return ret;
                }
            }
        ...
        string bucket_multipart_iname =
            "motr.rgw.bucket." + info.bucket.name + ".multiparts";
        ret = store->delete_motr_idx_by_name(bucket_multipart_iname);
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) <<
                "ERROR: remove_bucket failed to remove multipart index rc=" <<
                ret << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: remove_bucket unlink_user failed rc=" << ret
                << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) <<
                "ERROR: remove_bucket failed to remove bucket instance from cache rc="
                << ret << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) <<
                "ERROR: remove_bucket failed to remove bucket instance rc=" <<
                ret << dendl;
            return ret;
        }
        ...
        if (forward_to_master) {
            bufferlist in_data;
            ret =
                store->forward_request_to_master(dpp, owner, &bucket_version,
            ...
            if (ret < 0) {
                ...
                ldpp_dout(dpp,
                          0) << "ERROR: Forward to master failed. ret=" << ret
                    << dendl;
                return ret;
            }
        }
        ...
        ldpp_dout(dpp,
                  20) << "remove_bucket Exit=" << info.bucket.name << dendl;
    ...
    int MotrBucket::put_info(const DoutPrefixProvider * dpp, bool exclusive,
                             ceph::real_time _mtime) {
        bufferlist bl;
        struct MotrBucketInfo mbinfo;

        ldpp_dout(dpp,
                  20) << "put_info(): bucket_id=" << info.bucket.
            bucket_id << dendl;
        mbinfo.info = info;
    ...
    int MotrBucket::load_bucket(const DoutPrefixProvider * dpp,
        ...
        if (store->get_bucket_inst_cache()->get(dpp, info.bucket.name, bl)) {
            // Cache misses.
            ldpp_dout(dpp,
                      20) << "load_bucket(): name=" << info.bucket.
                name << dendl;
            int rc = store->do_idx_op_by_name(RGW_MOTR_BUCKET_INST_IDX_NAME,
        ...
        info = mbinfo.info;
        ldpp_dout(dpp,
                  20) << "load_bucket(): bucket_id=" << info.bucket.
            bucket_id << dendl;
        rgw_placement_rule placement_rule;
        placement_rule.name = "default";
        placement_rule.storage_class = "STANDARD";
    ...
    int MotrBucket::link_user(const DoutPrefixProvider * dpp, User * new_user,
                              optional_yield y) {
        bufferlist bl;
        ...
        std::time_t ctime =
            ceph::real_clock::to_time_t(new_bucket.creation_time);
        ldpp_dout(dpp,
                  20) << "got creation time: << " << std::put_time(std::
                                                                   localtime
                                                                   (&ctime),
        ...
        string user_info_idx_name =
            "motr.rgw.user.info." + new_user->get_info().user_id.to_str();
        return store->do_idx_op_by_name(user_info_idx_name, M0_IC_PUT,
                                        info.bucket.name, bl);

    }

    int MotrBucket::unlink_user(const DoutPrefixProvider * dpp, User * new_user,
                                optional_yield y) {
        // Remove the user into the user info index.
        bufferlist bl;
        string user_info_idx_name =
            "motr.rgw.user.info." + new_user->get_info().user_id.to_str();
        return store->do_idx_op_by_name(user_info_idx_name, M0_IC_DEL,
                                        info.bucket.name, bl);
    }
...
/* stats - Not for first pass */
    ...
    int MotrBucket::create_multipart_indices() {
        ...
        if (rc < 0) {
            ldout(store->cctx,
                  0) << "Failed to create bucket multipart index  " <<
    ...
    int MotrBucket::sync_user_stats(const DoutPrefixProvider * dpp,
                                    optional_yield y) {
        return 0;
    }
    ...
    int MotrBucket::chown(const DoutPrefixProvider * dpp, User & new_user,
                          optional_yield y) {
...
/* Make sure to call load_bucket() if you need it first */
    ...
    int MotrBucket::check_empty(const DoutPrefixProvider * dpp,
                                optional_yield y) {
    ...
    int MotrBucket::merge_and_store_attrs(const DoutPrefixProvider * dpp,
                                          Attrs & new_attrs, optional_yield y) {
      for (auto & it:new_attrs)
            attrs[it.first] = it.second;
    ...
    int MotrBucket::try_refresh_info(const DoutPrefixProvider * dpp,
                                     ceph::real_time * pmtime) {
        return 0;
    }

/* XXX: usage and stats not supported in the first pass */
    int MotrBucket::read_usage(const DoutPrefixProvider * dpp,
                               uint64_t start_epoch, uint64_t end_epoch,
                               uint32_t max_entries, bool * is_truncated,
                               RGWUsageIter & usage_iter, map < rgw_user_bucket,
                               rgw_usage_log_entry > &usage) {
    ...
    int MotrBucket::trim_usage(const DoutPrefixProvider * dpp,
                               uint64_t start_epoch, uint64_t end_epoch) {
        return 0;
    }

    int MotrBucket::remove_objs_from_index(const DoutPrefixProvider * dpp,
    ...
    int MotrBucket::check_index(const DoutPrefixProvider * dpp,
                                std::map < RGWObjCategory,
    ...
    int MotrBucket::set_tag_timeout(const DoutPrefixProvider * dpp,
                                    uint64_t timeout) {
    ...
    int MotrBucket::set_acl(const DoutPrefixProvider * dpp,
    ...
    int MotrBucket::list(const DoutPrefixProvider * dpp, ListParams & params,
    ...
    int MotrBucket::list_multiparts(const DoutPrefixProvider * dpp,
        ...
        rc = store->next_query_by_name(bucket_multipart_iname, key_vec,
                                       val_vec);
        if (rc < 0) {
            ldpp_dout(dpp, 0) << "ERROR: NEXT query failed. " << rc << dendl;
            return rc;
    ...
    } int MotrZoneGroup::get_placement_target_names(std::set < std::string >
                                                    &names) const {
    ...
    int MotrObject::get_obj_state(const DoutPrefixProvider * dpp,
                                  RGWObjState ** _state, optional_yield y,
                                  bool follow_olh) {
        // Get object's metadata (those stored in rgw_bucket_dir_entry).
        bufferlist bl;
        if (this->store->get_obj_meta_cache()->
            get(dpp, this->get_key().get_oid(), bl)) {
            // Cache misses.
            string bucket_index_iname =
                "motr.rgw.bucket.index." + this->get_bucket()->get_name();
            ...
            if (rc < 0) {
                ldpp_dout(dpp,
                          0) <<
                    "Failed to get object's entry from bucket index. " << dendl;
                return rc;
            }

            // Put into cache.
            this->store->get_obj_meta_cache()->put(dpp,
                                                   this->get_key().get_oid(),
                                                   bl);
        }
        ...
        ldpp_dout(dpp,
                  20) << __func__ << ": object's etag:  " << ent.meta.
            etag << dendl;
        etag_bl.append(etag);
...
//  }
    ...
    int MotrObject::set_obj_attrs(const DoutPrefixProvider * dpp,
                                  Attrs * setattrs, Attrs * delattrs,
                                  optional_yield y) {
        // TODO: implement
        ldpp_dout(dpp,
                  20) << __func__ << ": MotrObject::set_obj_attrs()" << dendl;
        return 0;
    }

    int MotrObject::get_obj_attrs(optional_yield y,
        ...
        if (this->store->get_obj_meta_cache()->get(dpp, key, bl)) {
            ...
            int rc =
                this->store->do_idx_op_by_name(bucket_index_iname, M0_IC_GET,
                                               key, bl);
            if (rc < 0) {
                ldpp_dout(dpp,
                          0) <<
    ...
    int MotrObject::modify_obj_attrs(const char *attr_name,
    ...
    int MotrObject::delete_obj_attrs(const DoutPrefixProvider * dpp,
...
// Taken from rgw_rados.cc
    void MotrObject::gen_rand_obj_instance_name() {
        ...
        gen_rand_alphanumeric_no_underscore(store->ctx(), buf,
                                            OBJ_INSTANCE_LEN);
        state.obj.key.set_instance(buf);
    }

    int MotrObject::omap_get_vals(const DoutPrefixProvider * dpp,
    ...
    int MotrObject::omap_get_all(const DoutPrefixProvider * dpp,
                                 std::map < std::string, bufferlist > *m,
                                 optional_yield y) {
        return 0;
    }

    int MotrObject::omap_get_vals_by_keys(const DoutPrefixProvider * dpp,
    ...
    int MotrObject::omap_set_val_by_key(const DoutPrefixProvider * dpp,
    ...
    int MotrObject::chown(User & new_user, const DoutPrefixProvider * dpp,
                          optional_yield y) {
        return 0;
    }

    std::unique_ptr < MPSerializer >
        MotrObject::get_serializer(const DoutPrefixProvider * dpp,
                                   const std::string & lock_name) {
        return std::make_unique < MPMotrSerializer > (dpp, store, this,
                                                      lock_name);
    }
    ...
    bool MotrObject::placement_rules_match(rgw_placement_rule & r1,
                                           rgw_placement_rule & r2) {
        /* XXX: support single default zone and zonegroup for now */
        return true;
    }

    int MotrObject::dump_obj_layout(const DoutPrefixProvider * dpp,
  ...
  MotrObject::MotrReadOp::MotrReadOp(MotrObject * _source):
    ...
    int MotrObject::MotrReadOp::prepare(optional_yield y,
                                        const DoutPrefixProvider * dpp) {
        int rc;
        ldpp_dout(dpp,
                  20) << __func__ << ": bucket=" << source->get_bucket()->
            get_name() << dendl;
        ...
        source->get_attrs().emplace(std::move(RGW_ATTR_ETAG),
                                    std::move(etag_bl));
        ...
        *params.lastmod = ent.meta.mtime;
        ...
        if (params.mod_ptr || params.unmod_ptr) {
            ...
            src_weight.init(*params.lastmod, params.mod_zone_id,
                            params.mod_pg_ver);
            ...
            if (params.mod_ptr && !params.if_nomatch) {
                dest_weight.init(*params.mod_ptr, params.mod_zone_id,
                                 params.mod_pg_ver);
                ldpp_dout(dpp,
    ...
    int MotrObject::MotrReadOp::read(int64_t off, int64_t end, bufferlist & bl,
                                     optional_yield y,
                                     const DoutPrefixProvider * dpp) {
...
// a block of data each time and call 'cb' for post-processing.
    int MotrObject::MotrReadOp::iterate(const DoutPrefixProvider * dpp,
    ...
    int MotrObject::MotrReadOp::get_attr(const DoutPrefixProvider * dpp,
...
// 2. Delete an object when its versioning is turned on.
    int MotrObject::MotrDeleteOp::delete_obj(const DoutPrefixProvider * dpp,
                                             optional_yield y) {
        ldpp_dout(dpp,
                  20) << "delete " << source->get_key().
            get_oid() << " from " << source->get_bucket()->get_name() << dendl;
        ...
        source->store->get_obj_meta_cache()->remove(dpp,
                                                    source->get_key().
                                                    get_oid());
        ...
        bufferlist bl;
        string bucket_index_iname =
            "motr.rgw.bucket.index." + source->get_bucket()->get_name();
        rc = source->store->do_idx_op_by_name(bucket_index_iname, M0_IC_DEL,
                                              source->get_key().get_oid(), bl);
        if (rc < 0) {
            ldpp_dout(dpp,
                      0) << "Failed to del object's entry from bucket index. "
                << dendl;
            return rc;
        }
        ...
        if (ent.meta.size == 0) {
            ldpp_dout(dpp,
                      0) << __func__ <<
                ": Object size is 0, not deleting motr object." << dendl;
            return 0;
        }
        ...
        if (rc < 0) {
            ldpp_dout(dpp,
                      0) << "Failed to delete the object from Motr. " << dendl;
            return rc;
        }
    ...
    int MotrObject::delete_obj_aio(const DoutPrefixProvider * dpp,
  ...
  MotrAtomicWriter::MotrAtomicWriter(const DoutPrefixProvider * dpp, optional_yield y, rgw::sal::Object * obj, MotrStore * _store, const rgw_user & _owner, const rgw_placement_rule * _ptail_placement_rule, uint64_t _olh_epoch, const std::string & _unique_tag):
    StoreWriter(dpp, y),
        store(_store),
        owner(_owner),
    ...
    int MotrObject::create_mobj(const DoutPrefixProvider * dpp, uint64_t sz) {
        if (mobj != nullptr) {
            ldpp_dout(dpp,
                      0) << __func__ << "ERROR: object is already opened" <<
                dendl;
            return -EINVAL;
        }
        ...
        if (rc != 0) {
            ldpp_dout(dpp,
                      0) << __func__ << "ERROR: m0_ufid_next() failed: " << rc
                << dendl;
            return rc;
        }
        ...
        if (rc != 0) {
            this->close_mobj();
            ldpp_dout(dpp,
                      0) << "ERROR: m0_entity_create() failed: " << rc << dendl;
            return rc;
        }
        ...
        if (rc != 0) {
            this->close_mobj();
            ldpp_dout(dpp,
                      0) << "ERROR: failed to create motr object: " << rc <<
                dendl;
            return rc;
        }
        ...
        ldpp_dout(dpp,
                  20) << __func__ << ": lid=0x" << std::hex << meta.
            layout_id << std::dec << " rc=" << rc << dendl;
    ...
    int MotrObject::open_mobj(const DoutPrefixProvider * dpp) {
        ...
        if (meta.layout_id == 0) {
            ...
            if (rc < 0) {
                ldpp_dout(dpp,
                          0) << "ERROR: open_mobj() failed: rc=" << rc << dendl;
                return rc;
            }
        }
        ...
        m0_obj_init(mobj, &store->container.co_realm, &meta.oid,
        ...
        if (rc != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: m0_entity_open() failed: rc=" << rc <<
                dendl;
            this->close_mobj();
            return rc;
        }
        ...
        if (rc < 0) {
            ldpp_dout(dpp,
                      10) << "ERROR: failed to open motr object: rc=" << rc <<
                dendl;
            this->close_mobj();
            return rc;
        }
    ...
    int MotrObject::delete_mobj(const DoutPrefixProvider * dpp) {
        ...
        if (!meta.oid.u_hi || !meta.oid.u_lo) {
            ldpp_dout(dpp,
                      20) << __func__ << ": invalid motr object oid=" << fid_str
                << dendl;
            return -EINVAL;
        }
        ldpp_dout(dpp,
                  20) << __func__ << ": deleting motr object oid=" << fid_str <<
            dendl;
        ...
        if (rc != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: m0_entity_delete() failed: " << rc << dendl;
            return rc;
        }
        m0_op_launch(&op, 1);
        rc = m0_op_wait(op, M0_BITS(M0_OS_FAILED, M0_OS_STABLE),
        ...
        if (rc < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to open motr object: " << rc <<
                dendl;
            return rc;
        }
    ...
    int MotrObject::write_mobj(const DoutPrefixProvider * dpp, bufferlist
        ...
        rc = m0_bufvec_empty_alloc(&buf, 1) ? :
            m0_bufvec_alloc(&attr, 1, 1) ? : m0_indexvec_alloc(&ext, 1);
        if (rc != 0)
            goto out;

        bs = this->get_optimal_bs(left);
        ldpp_dout(dpp,
                  20) << __func__ << ": left=" << left << " bs=" << bs << dendl;
        ...
        for (p = start; left > 0; left -= bs, p += bs, offset += bs) {
            ...
            op = nullptr;
            rc = m0_obj_op(this->mobj, M0_OC_WRITE, &ext, &buf, &attr, 0, 0,
                           &op);
            if (rc != 0)
                goto out;
            m0_op_launch(&op, 1);
    ...
    int MotrObject::read_mobj(const DoutPrefixProvider * dpp, int64_t off,
        ...
        for (; left > 0; off += actual) {
            ...
            // Read from Motr.
            op = nullptr;
            rc = m0_obj_op(this->mobj, M0_OC_READ, &ext, &buf, &attr, 0, 0,
                           &op);
            ldpp_dout(dpp,
                      20) << "MotrObject::read_mobj(): init read op rc=" << rc
                << dendl;
            if (rc != 0) {
                ldpp_dout(dpp,
                          0) << __func__ <<
            ...
            if (rc != 0) {
                ldpp_dout(dpp,
                          0) << __func__ << ": read failed, m0_op_wait rc=" <<
                    rc << dendl;
                goto out;
            }
            // Call `cb` to process returned data.
            ldpp_dout(dpp,
                      20) << "MotrObject::read_mobj(): call cb to process data"
                << dendl;
    ...
    int MotrObject::get_bucket_dir_ent(const DoutPrefixProvider * dpp,
                                       rgw_bucket_dir_entry & ent) {
        int rc = 0;
        string bucket_index_iname =
            "motr.rgw.bucket.index." + this->get_bucket()->get_name();
        ...
        if (this->get_bucket()->get_info().versioning_status() ==
            BUCKET_VERSIONED
            || this->get_bucket()->get_info().versioning_status() ==
            BUCKET_SUSPENDED) {

            rgw_bucket_dir_entry ent_to_check;

            if (this->store->get_obj_meta_cache()->
                get(dpp, this->get_name(), bl) == 0) {
                iter = bl.cbegin();
            ...
            if (rc < 0) {
                ldpp_dout(dpp,
                          0) << __func__ << "ERROR: NEXT query failed. " << rc
                    << dendl;
                return rc;
            }
          ...
          for (const auto & bl:vals) {
                ...
                if (ent_to_check.is_current()) {
                    ldpp_dout(dpp,
                              20) << __func__ << ": found current version!" <<
                        dendl;
                    ent = ent_to_check;
                    rc = 0;
            ...
            if (this->store->get_obj_meta_cache()->
                get(dpp, this->get_key().get_oid(), bl)) {
                ldpp_dout(dpp,
                          20) << __func__ << ": non-versioned bucket!" << dendl;
    ...
    int MotrObject::update_version_entries(const DoutPrefixProvider * dpp) {
        ...
        rc = store->next_query_by_name(bucket_index_iname, keys, vals);
        ldpp_dout(dpp,
                  20) << "get all versions, name = " << this->
            get_name() << "rc = " << rc << dendl;
        if (rc < 0) {
            ldpp_dout(dpp, 0) << "ERROR: NEXT query failed. " << rc << dendl;
            return rc;
        }
      ...
      for (const auto & bl:vals) {
            ...
            if (0 !=
                ent.key.name.compare(0, this->get_name().size(),
                                     this->get_name()))
                continue;
            ...
            if (ent.key.instance.empty())
                ...
                snprintf(buf, sizeof(buf), "%s[%s]", ent.key.name.c_str(),
...
// way of managing opened object may be needed.
    int MotrObject::get_part_objs(const DoutPrefixProvider * dpp,
                                  std::map < int,
                                  std::unique_ptr < MotrObject >> &part_objs) {
        int rc;
        int max_parts = 1000;
        int marker = 0;
        uint64_t off = 0;
        ...
        upload =
            this->get_bucket()->get_multipart_upload(this->get_name(),
                                                     string());

        do {
            rc = upload->list_parts(dpp, store->ctx(), max_parts, marker,
                                    &marker, &truncated);
            if (rc == -ENOENT) {
            ...
            std::map < uint32_t, std::unique_ptr < MultipartPart >> &parts =
                upload->get_parts();
            for (auto part_iter = parts.begin(); part_iter != parts.end();
                 ++part_iter) {

                MultipartPart *mpart = part_iter->second.get();
                MotrMultipartPart *mmpart =
                    static_cast < MotrMultipartPart * >(mpart);
                ...
                std::unique_ptr < rgw::sal::MotrObject > mobj(static_cast <
                                                              rgw::sal::
                                                              MotrObject *
                                                              >(obj.release()));

                ldpp_dout(dpp,
                          20) << "get_part_objs: off = " << off << ", size = "
                    << part_size << dendl;
                mobj->part_off = off;
    ...
    int MotrObject::delete_part_objs(const DoutPrefixProvider * dpp) {
        ...
        upload =
            this->get_bucket()->get_multipart_upload(this->get_name(),
                                                     string());
        std::unique_ptr < rgw::sal::MotrMultipartUpload > mupload(static_cast <
                                                                  rgw::sal::
                                                                  MotrMultipartUpload
                                                                  *
                                                                  >(upload.
    ...
    int MotrObject::read_multipart_obj(const DoutPrefixProvider * dpp,
                                       int64_t off, int64_t end,
                                       RGWGetDataCB * cb, std::map < int,
                                       std::unique_ptr <
                                       MotrObject >> &part_objs) {
        int64_t cursor = off;

        ldpp_dout(dpp,
                  20) << "read_multipart_obj: off=" << off << " end=" << end <<
            dendl;
    ...
    unsigned MotrAtomicWriter::populate_bvec(unsigned len,
    ...
    int MotrAtomicWriter::write() {
        ...
        if (!obj.is_opened()) {
            ...
            if (rc != 0) {
                char fid_str[M0_FID_STR_LEN];
                snprintf(fid_str, ARRAY_SIZE(fid_str), U128X_F,
                         U128_P(&obj.meta.oid));
                ldpp_dout(dpp,
                          0) << "ERROR: failed to create/open motr object " <<
...
// and then launch the write operations.
    ...
    int MotrAtomicWriter::complete(size_t accounted_size,
        ...
        ent.meta.mtime =
            real_clock::is_zero(set_mtime) ? ceph::real_clock::
            now() : set_mtime;
        ent.meta.etag = etag;
        ...
        if (is_versioned)
            ent.flags =
                rgw_bucket_dir_entry::FLAG_VER | rgw_bucket_dir_entry::
                FLAG_CURRENT;
        ...
        obj.meta.encode(bl);
        ldpp_dout(dpp,
                  20) << __func__ << ": lid=0x" << std::hex << obj.meta.
            layout_id << dendl;
        if (is_versioned) {
        ...
        string bucket_index_iname =
            "motr.rgw.bucket.index." + obj.get_bucket()->get_name();
        rc = store->do_idx_op_by_name(bucket_index_iname, M0_IC_PUT,
                                      obj.get_key().get_oid(), bl);
        if (rc == 0)
            store->get_obj_meta_cache()->put(dpp, obj.get_key().get_oid(), bl);

        if (old_obj.get_bucket()->get_info().versioning_status() !=
            BUCKET_VERSIONED) {
    ...
    int MotrMultipartUpload::delete_parts(const DoutPrefixProvider * dpp) {
        ...
        do {
            rc = this->list_parts(dpp, store->ctx(), max_parts, marker, &marker,
                                  &truncated);
            if (rc == -ENOENT) {
                truncated = false;
                rc = 0;
            ...
            for (auto part_iter = parts.begin(); part_iter != parts.end();
                ...
                string part_obj_name = bucket->get_name() + "." +
                    mp_obj.get_key() + ".part." + std::to_string(part_num);
                std::unique_ptr < rgw::sal::Object > obj;
                obj = this->bucket->get_object(rgw_obj_key(part_obj_name));
                std::unique_ptr < rgw::sal::MotrObject > mobj(static_cast <
                                                              rgw::sal::
        ...
        string obj_part_iname =
            "motr.rgw.object." + bucket->get_name() + "." + oid + ".parts";
        return store->delete_motr_idx_by_name(obj_part_iname);
    }

    int MotrMultipartUpload::abort(const DoutPrefixProvider * dpp,
                                   CephContext * cct) {
        int rc;
        ...
        string bucket_multipart_iname =
            "motr.rgw.bucket." + meta_obj->get_bucket()->get_name() +
            ".multiparts";
        rc = store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_GET,
                                      meta_obj->get_key().to_str(), bl);
        if (rc < 0) {
            ldpp_dout(dpp,
                      0) << __func__ << ": Failed to get multipart upload. rc="
                << rc << dendl;
        ...
        rc = store->do_idx_op_by_name(bucket_multipart_iname,
                                      M0_IC_DEL, meta_obj->get_key().get_oid(),
                                      bl);
        return rc;
    }
    ...
    int MotrMultipartUpload::init(const DoutPrefixProvider * dpp,
                                  optional_yield y, ACLOwner & _owner,
                                  rgw_placement_rule & dest_placement,
                                  rgw::sal::Attrs & attrs) {
        int rc;
        std::string oid = mp_obj.get_key();
        ...
        do {
            ...
            ent.meta.user_data.assign(mpbl.c_str(),
                                      mpbl.c_str() + mpbl.length());
            ent.encode(bl);

            // Insert an entry into bucket multipart index so it is not shown
            // when listing a bucket.
            string bucket_multipart_iname =
                "motr.rgw.bucket." + obj->get_bucket()->get_name() +
                ".multiparts";
            rc = store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_PUT,
                                          obj->get_key().get_oid(), bl);
        ...
        string obj_part_iname =
            "motr.rgw.object." + bucket->get_name() + "." + oid + ".parts";
        ldpp_dout(dpp,
                  20) << "MotrMultipartUpload::init(): object part index=" <<
        ...
        if (rc < 0)
            ...
            ldpp_dout(dpp,
                      0) << "Failed to create object multipart index  " <<
                obj_part_iname << dendl;

        return rc;
    }

    int MotrMultipartUpload::list_parts(const DoutPrefixProvider * dpp,
        ...
        string obj_part_iname =
            "motr.rgw.object." + bucket->get_name() + "." + oid + ".parts";
        ldpp_dout(dpp,
                  20) << __func__ << ": object part index = " << obj_part_iname
            << dendl;
        key_vec[0].clear();
        key_vec[0] = "part.";
        char buf[32];
      ...
      for (const auto & bl:val_vec) {
            ...
            ldpp_dout(dpp,
                      20) << __func__ << ": meta:oid=[" << meta.oid.
                u_hi << "," << meta.oid.u_lo << "], meta:pvid=[" << meta.pver.
                f_container << "," << meta.pver.
                f_key << "], meta:layout id=" << meta.layout_id << dendl;
            ...
            if ((int)info.num > marker) {
                last_num = info.num;
                parts.emplace(info.num,
                              std::make_unique < MotrMultipartPart > (info,
                                                                      meta));
            }
...
// Heavily copy from rgw_sal_rados.cc
    int MotrMultipartUpload::complete(const DoutPrefixProvider * dpp,
        ...
        do {
            ldpp_dout(dpp,
                      20) << "MotrMultipartUpload::complete(): list_parts()" <<
                dendl;
            ...
            ldpp_dout(dpp,
                      20) << "MotrMultipartUpload::complete(): parts.size()=" <<
                parts.size() << dendl;
            ...
            for (auto obj_iter = parts.begin();
                ...
                ldpp_dout(dpp,
                          20) << "MotrMultipartUpload::complete(): part_size="
                    << part_size << dendl;
                ...
                ldpp_dout(dpp,
                          20) << "MotrMultipartUpload::complete(): calc etag "
                    << dendl;
                ...
                ldpp_dout(dpp,
                          0) << "MotrMultipartUpload::complete(): manifest " <<
                    dendl;
#endif

                bool part_compressed =
                    (part->cs_info.compression_type != "none");
                if ((handled_parts > 0)
                ...
                ldpp_dout(dpp,
                          20) <<
                    "MotrMultipartUpload::complete(): part compression" <<
                    dendl;
                if (part_compressed) {
                    ...
                    if (cs_info.blocks.size() > 0)
                        ...
                        new_ofs = 0;
                  for (const auto & block:part->cs_info.
                         blocks) {
                ...
                ldpp_dout(dpp,
                          20) << "MotrMultipartUpload::complete(): off=" << off
        ...
        string bucket_multipart_iname =
            "motr.rgw.bucket." + meta_obj->get_bucket()->get_name() +
            ".multiparts";
        rc = this->store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_GET,
                                            meta_obj->get_key().get_oid(), bl);
        ldpp_dout(dpp,
                  20) <<
            "MotrMultipartUpload::complete(): read entry from bucket multipart index rc="
            << rc << dendl;
        if (rc < 0)
            return rc;
        rgw_bucket_dir_entry ent;
        ...
        ldpp_dout(dpp,
                  20) << "MotrMultipartUpload::complete(): obj size=" << ent.
            meta.size << " obj accounted size=" << ent.meta.
            accounted_size << dendl;
        ent.meta.mtime = ceph::real_clock::now();
        ent.meta.etag = etag;
        ...
        string bucket_index_iname =
            "motr.rgw.bucket.index." + meta_obj->get_bucket()->get_name();
        ldpp_dout(dpp,
                  20) << "MotrMultipartUpload::complete(): target_obj name=" <<
            target_obj->get_name()
        ...
        store->get_obj_meta_cache()->put(dpp, target_obj->get_name(),
                                         update_bl);
        ...
        ldpp_dout(dpp,
            ...
            << dendl;
        return store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_DEL,
                                        meta_obj->get_key().get_oid(), bl);
    }

    int MotrMultipartUpload::get_info(const DoutPrefixProvider * dpp,
                                      optional_yield y,
        ...
        int rc = this->store->do_idx_op_by_name(bucket_multipart_iname,
                                                M0_IC_GET,
                                                meta_obj->get_key().get_oid(),
                                                bl);
        if (rc < 0) {
            ldpp_dout(dpp,
                      0) << __func__ << ": Failed to get multipart info. rc=" <<
                rc << dendl;
            return rc == -ENOENT ? -ERR_NO_SUCH_UPLOAD : rc;
        }
    ...
    std::unique_ptr < Writer >
        MotrMultipartUpload::get_writer(const DoutPrefixProvider * dpp,
                                        optional_yield y,
                                        rgw::sal::Object * obj,
                                        const rgw_user & owner,
                                        const rgw_placement_rule *
                                        ptail_placement_rule, uint64_t part_num,
                                        const std::string & part_num_str) {
        return std::make_unique < MotrMultipartWriter > (dpp, y, this, obj,
                                                         store, owner,
    ...
    int MotrMultipartWriter::prepare(optional_yield y) {
        string part_obj_name = head_obj->get_bucket()->get_name() + "." +
            head_obj->get_key().get_oid() + ".part." + std::to_string(part_num);
        ldpp_dout(dpp,
                  20) << "bucket=" << head_obj->get_bucket()->
            get_name() << "part_obj_name=" << part_obj_name << dendl;
        part_obj =
            std::make_unique < MotrObject > (this->store,
                                             rgw_obj_key(part_obj_name),
                                             head_obj->get_bucket());
        if (part_obj == nullptr)
            return -ENOMEM;
        ...
        // been created.
        int rc =
            part_obj->create_mobj(dpp, store->cctx->_conf->rgw_max_chunk_size);
        if (rc == -EEXIST) {
    ...
    int MotrMultipartWriter::process(bufferlist && data, uint64_t offset) {
        ...
        if (rc == 0) {
            ...
            ldpp_dout(dpp,
                      20) << " write_mobj(): actual_part_size=" <<
                actual_part_size << dendl;
        }
        return rc;
    }

    int MotrMultipartWriter::complete(size_t accounted_size,
                                      const std::string & etag,
                                      ceph::real_time * mtime,
                                      ceph::real_time set_mtime,
                                      std::map < std::string,
        ...
        bool compressed;
        int rc =
            rgw_compression_info_from_attrset(attrs, compressed, info.cs_info);
        ldpp_dout(dpp,
                  20) << "MotrMultipartWriter::complete(): compression rc=" <<
            rc << dendl;
        if (rc < 0) {
        ...
        p.append(buf);
        string obj_part_iname =
            "motr.rgw.object." + head_obj->get_bucket()->get_name() + "." +
            head_obj->get_key().get_oid() + ".parts";
        ldpp_dout(dpp,
                  20) << "MotrMultipartWriter::complete(): object part index = "
            << obj_part_iname << dendl;
    ...
    std::unique_ptr < MultipartUpload >
        MotrBucket::get_multipart_upload(const std::string & oid,
                                         std::optional < std::string >
                                         upload_id, ACLOwner owner,
                                         ceph::real_time mtime) {
        return std::make_unique < MotrMultipartUpload > (store, this, oid,
                                                         upload_id, owner,
                                                         mtime);
    }

    std::unique_ptr < Writer >
        MotrStore::get_append_writer(const DoutPrefixProvider * dpp,
    ...
    std::unique_ptr < Writer >
        MotrStore::get_atomic_writer(const DoutPrefixProvider * dpp,
                                     optional_yield y, rgw::sal::Object * obj,
                                     const rgw_user & owner,
                                     const rgw_placement_rule *
                                     ptail_placement_rule, uint64_t olh_epoch,
                                     const std::string & unique_tag) {
        return std::make_unique < MotrAtomicWriter > (dpp, y, obj, this, owner,
                                                      ptail_placement_rule,
                                                      olh_epoch, unique_tag);
    }

    const std::string & MotrStore::
    ...
    int MotrStore::get_user_by_access_key(const DoutPrefixProvider * dpp,
                                          const std::string & key,
                                          optional_yield y,
    ...
    int MotrStore::get_user_by_email(const DoutPrefixProvider * dpp,
    ...
    int MotrStore::get_user_by_swift(const DoutPrefixProvider * dpp,
    ...
    int MotrStore::store_access_key(const DoutPrefixProvider * dpp,
                                    optional_yield y,
                                    MotrAccessKey access_key) {
        int rc;
        bufferlist bl;
    ...
    int MotrStore::delete_access_key(const DoutPrefixProvider * dpp,
                                     optional_yield y, std::string access_key) {
        int rc;
        bufferlist bl;
        rc = do_idx_op_by_name(RGW_IAM_MOTR_ACCESS_KEY,
    ...
    int MotrStore::store_email_info(const DoutPrefixProvider * dpp,
                                    optional_yield y,
                                    MotrEmailInfo & email_info) {
        int rc;
        bufferlist bl;
        ...
        if (rc < 0) {
            ldout(cctx,
                  0) << "Failed to store the user by email as key: rc = " << rc
                << dendl;
        }
        return rc;
    }
    ...
    int MotrStore::get_bucket(User * u, const RGWBucketInfo & i,
                              std::unique_ptr < Bucket > *bucket) {
        Bucket *bp;
    ...
    int MotrStore::forward_request_to_master(const DoutPrefixProvider * dpp,
    ...
    int MotrStore::forward_iam_request_to_master(const DoutPrefixProvider * dpp,
    ...
    int MotrStore::get_zonegroup(const std::string & id,
    ...
    std::unique_ptr < Notification > MotrStore::get_notification(Object * obj,
                                                                 Object *
                                                                 src_obj,
                                                                 req_state * s,
                                                                 rgw::notify::
    ...
    std::unique_ptr < Notification >
        MotrStore::get_notification(const DoutPrefixProvider * dpp,
                                    Object * obj, Object * src_obj,
    ...
    int MotrStore::log_usage(const DoutPrefixProvider * dpp,
                             map < rgw_user_bucket,
                             RGWUsageBatch > &usage_info) {
        return 0;
    }

    int MotrStore::log_op(const DoutPrefixProvider * dpp, string & oid,
                          bufferlist & bl) {
        return 0;
    }

    int MotrStore::register_to_service_map(const DoutPrefixProvider * dpp,
    ...
    int MotrStore::set_buckets_enabled(const DoutPrefixProvider * dpp,
    ...
    RGWDataSyncStatusManager *MotrStore::
        get_data_sync_manager(const rgw_zone_id & source_zone) {
        return 0;
    }

    int MotrStore::read_all_usage(const DoutPrefixProvider * dpp,
                                  uint64_t start_epoch, uint64_t end_epoch,
                                  uint32_t max_entries, bool * is_truncated,
    ...
    int MotrStore::trim_all_usage(const DoutPrefixProvider * dpp,
                                  uint64_t start_epoch, uint64_t end_epoch) {
        return 0;
    }
    ...
    int MotrStore::meta_list_keys_init(const DoutPrefixProvider * dpp,
                                       const string & section,
                                       const string & marker, void **phandle) {
        return 0;
    }

    int MotrStore::meta_list_keys_next(const DoutPrefixProvider * dpp,
                                       void *handle, int max,
                                       list < string > &keys,
                                       bool * truncated) {
        return 0;
    ...
    int MotrStore::meta_remove(const DoutPrefixProvider * dpp,
                               string & metadata_key, optional_yield y) {
        return 0;
    }

    int MotrStore::open_idx(struct m0_uint128 *id, bool create,
                            struct m0_idx *idx) {
...
// idx must be opened with open_idx() beforehand
    int MotrStore::do_idx_op(struct m0_idx *idx, enum m0_idx_opcode opcode,
                             vector < uint8_t > &key, vector < uint8_t > &val,
                             bool update) {
        int rc, rc_i;
        ...
        if (opcode == M0_IC_PUT || opcode == M0_IC_GET) {
            ...
            if (m0_bufvec_empty_alloc(&v, 1) != 0) {
                ldout(cctx,
                      0) << "ERROR: failed to allocate value bufvec" << dendl;
                goto out;
            }
        }
        ...
        if (opcode == M0_IC_GET) {
            ...
            memcpy(reinterpret_cast < char *>(val.data()), *v.ov_buf,
                   *v.ov_vec.v_count);
        }
...
// Retrieve a range of key/value pairs starting from keys[0].
    int MotrStore::do_idx_next_op(struct m0_idx *idx,
        ...
        for (i = 0; i < v.ov_vec.v_nr; ++i) {
            ...
            memcpy(reinterpret_cast < char *>(key.data()), k.ov_buf[i],
                   k.ov_vec.v_count[i]);
            memcpy(reinterpret_cast < char *>(val.data()), v.ov_buf[i],
                   v.ov_vec.v_count[i]);
        }
...
// from the marker at key_out[0].
    int MotrStore::next_query_by_name(string idx_name,
        ...
        if (rc != 0) {
            ldout(cctx,
                  0) << "ERROR: next_query_by_name(): failed to open index: rc="
                << rc << dendl;
            goto out;
        }
        ...
        ldout(cctx,
              20) << __func__ << ": next_query_by_name(): index=" << idx_name <<
        ...
        for (i = 0; i < (int)val_out.size(); i += k, k = 0) {
            ...
            for (j = 0, k = 0; j < rc; ++j) {
                ...
                vbl.append(reinterpret_cast < char *>(vals[j].data()),
                           vals[j].size());
                ++k;
            }
            ...
            ldout(cctx,
                  0) << "do_idx_next_op(): next_key=" << next_key << dendl;
...
// bucket instance index is created when creating the bucket.
    ...
    std::string MotrStore::get_cluster_id(const DoutPrefixProvider * dpp,
                                          optional_yield y) {
    ...
    int MotrLuaManager::get_script(const DoutPrefixProvider * dpp,
                                   optional_yield y, const std::string & key,
                                   std::string & script) {
        return -ENOENT;
    }

    int MotrLuaManager::put_script(const DoutPrefixProvider * dpp,
                                   optional_yield y, const std::string & key,
                                   const std::string & script) {
        return -ENOENT;
    }

    int MotrLuaManager::del_script(const DoutPrefixProvider * dpp,
                                   optional_yield y, const std::string & key) {
        return -ENOENT;
    }

    int MotrLuaManager::add_package(const DoutPrefixProvider * dpp,
                                    optional_yield y,
                                    const std::string & package_name) {
        return -ENOENT;
    }

    int MotrLuaManager::remove_package(const DoutPrefixProvider * dpp,
                                       optional_yield y,
                                       const std::string & package_name) {
        return -ENOENT;
    }

    int MotrLuaManager::list_packages(const DoutPrefixProvider * dpp,
                                      optional_yield y,
...
extern "C" {
    ...
    void *newMotrStore(CephContext * cct) {
        ...
        if (store) {
            ...
            const auto & proc_ep =
                g_conf().get_val < std::string > ("motr_my_endpoint");
            const auto & ha_ep =
                g_conf().get_val < std::string > ("motr_ha_endpoint");
            const auto & proc_fid =
                g_conf().get_val < std::string > ("motr_my_fid");
            const auto & profile =
                g_conf().get_val < std::string > ("motr_profile_fid");
            ...
            ldout(cct,
                  0) << "INFO: motr admin endpoint: " << admin_proc_ep << dendl;
            ldout(cct,
                  0) << "INFO: motr admin fid:   " << admin_proc_fid << dendl;
            ...
            ldout(cct,
                  50) << "INFO: motr profile fid:  " << store->conf.
                mc_profile << dendl;
            ldout(cct,
                  50) << "INFO: ha addr:  " << store->conf.mc_ha_addr << dendl;
            ldout(cct,
                  50) << "INFO: process fid:  " << store->conf.
                mc_process_fid << dendl;
            ldout(cct,
                  50) << "INFO: motr endpoint:  " << store->conf.
                mc_local_addr << dendl;
            ...
            if (rc != 0) {
                ldout(cct,
                      0) << "ERROR: m0_client_init() failed: " << rc << dendl;
                goto out;
            }

            m0_container_init(&store->container, nullptr, &M0_UBER_REALM,
                              store->instance);
            rc = store->container.co_realm.re_entity.en_sm.sm_rc;
            if (rc != 0) {
                ldout(cct,
                      0) << "ERROR: m0_container_init() failed: " << rc <<
                    dendl;
                goto out;
            }
            ...
            if (rc != 0) {
                ldout(cct,
                      0) << "ERROR: m0_ufid_init() failed: " << rc << dendl;
                goto out;
            }
