...
// struct for holding the callback and its tag in the callback list
    struct reply_callback_with_tag_t {
        ...
        reply_callback_with_tag_t(uint64_t _tag,
                                  reply_callback_t _cb):tag(_tag), cb(_cb) {
    ...
    struct connection_t {
        ...
        void destroy(int s) {
            ...
            // wait for all remaining acks/nacks
                rd_kafka_flush(producer,
                               5 * 1000 /* wait for max 5 seconds */ );
            // destroy all topics
            std::for_each(topics.begin(), topics.end(),[](auto topic) {
            ...
            std::for_each(callbacks.begin(), callbacks.end(),
                          ...
                          ldout(cct,
                                20) <<
                          "Kafka destroy: invoking callback with tag=" <<
                          cb_tag.tag << dendl;}
            );
            callbacks.clear();
            delivery_tag = 1;
        ...
        // ctor for setting immutable values
            connection_t(CephContext * _cct, const std::string & _broker,
                         bool _use_ssl, bool _verify_ssl,
                         const boost::optional <
                         const std::string & >&_ca_location,
                         const std::string & _user,
                         const std::string & _password,
                         const boost::optional <
                         const std::string & >&_mechanism):cct(_cct),
            broker(_broker), use_ssl(_use_ssl), verify_ssl(_verify_ssl),
            ca_location(_ca_location), user(_user), password(_password),
...
// convert int status to string - including RGW specific values
    ...
    void message_callback(rd_kafka_t * rk, const rd_kafka_message_t * rkmessage,
                          void *opaque) {
        ceph_assert(opaque);
        ...
        if (!rkmessage->_private) {
            ldout(conn->cct,
                  20) << "Kafka run: n/ack received, (no callback) with result="
                << result << dendl;
            return;
        }
    ...
    void log_callback(const rd_kafka_t * rk, int level, const char *fac,
                      const char *buf) {
        ...
        if (level <= 3)
            ...
            ldout(conn->cct,
                  10) << "RDKAFKA-" << level << "-" << fac << ": " <<
                rd_kafka_name(rk) << ": " << buf << dendl;
        else
            ldout(conn->cct,
                  20) << "RDKAFKA-" << level << "-" << fac << ": " <<
                rd_kafka_name(rk) << ": " << buf << dendl;
    }

    void poll_err_callback(rd_kafka_t * rk, int err, const char *reason,
                           void *opaque) {
        const auto conn =
            reinterpret_cast < connection_t * >(rd_kafka_opaque(rk));
        ldout(conn->cct,
              10) << "Kafka run: poll error(" << err << "): " << reason <<
            dendl;
    }
...
// utility function to create a producer, when the connection object already exists
    bool new_producer(connection_t * conn) {
        ...
        if (rd_kafka_conf_set
            (conn->temp_conf, "bootstrap.servers", conn->broker.c_str(), errstr,
        ...
        if (conn->use_ssl) {
            if (!conn->user.empty()) {
                // use SSL+SASL
                if (rd_kafka_conf_set
                    (conn->temp_conf, "security.protocol", "SASL_SSL", errstr,
                     sizeof(errstr)) != RD_KAFKA_CONF_OK
                    || rd_kafka_conf_set(conn->temp_conf, "sasl.username",
                                         conn->user.c_str(), errstr,
                                         sizeof(errstr)) != RD_KAFKA_CONF_OK
                    || rd_kafka_conf_set(conn->temp_conf, "sasl.password",
                                         conn->password.c_str(), errstr,
                                         sizeof(errstr)) != RD_KAFKA_CONF_OK)
                    goto conf_error;
                ldout(conn->cct,
                      20) <<
                ...
                if (conn->mechanism) {
                    if (rd_kafka_conf_set
                        (conn->temp_conf, "sasl.mechanism",
                         conn->mechanism->c_str(), errstr,
                         sizeof(errstr)) != RD_KAFKA_CONF_OK)
                        goto conf_error;
                    ldout(conn->cct,
                          20) <<
                    ...
                    if (rd_kafka_conf_set
                        (conn->temp_conf, "sasl.mechanism", "PLAIN", errstr,
                         sizeof(errstr)) != RD_KAFKA_CONF_OK)
                        goto conf_error;
                    ldout(conn->cct,
                          20) << "Kafka connect: using default SASL mechanism"
                        << dendl;
                }
                ...
                // use only SSL
                if (rd_kafka_conf_set
                    (conn->temp_conf, "security.protocol", "SSL", errstr,
                     sizeof(errstr)) != RD_KAFKA_CONF_OK)
                    goto conf_error;
                ldout(conn->cct,
                      20) <<
            ...
            if (conn->ca_location) {
                if (rd_kafka_conf_set
                    (conn->temp_conf, "ssl.ca.location",
                     conn->ca_location->c_str(), errstr,
                     sizeof(errstr)) != RD_KAFKA_CONF_OK)
                    goto conf_error;
                ldout(conn->cct,
                      20) <<
                ...
                ldout(conn->cct,
                      20) << "Kafka connect: using default CA location" <<
                    dendl;
            }
            ...
            ldout(conn->cct,
                  20) << "Kafka connect: successfully configured security" <<
                dendl;
        }
        else if (!conn->user.empty()) {
            // use SASL+PLAINTEXT
            if (rd_kafka_conf_set
                (conn->temp_conf, "security.protocol", "SASL_PLAINTEXT", errstr,
                 sizeof(errstr)) != RD_KAFKA_CONF_OK
                || rd_kafka_conf_set(conn->temp_conf, "sasl.username",
                                     conn->user.c_str(), errstr,
                                     sizeof(errstr)) != RD_KAFKA_CONF_OK
                ...
                goto conf_error;
            ldout(conn->cct,
                  20) << "Kafka connect: successfully configured SASL_PLAINTEXT"
                << dendl;
        ...
        if (conn->producer) {
            ldout(conn->cct,
                  5) <<
                "Kafka connect: producer already exists. detroying the existing before creating a new one"
                << dendl;
            conn->destroy(STATUS_CONF_REPLCACE);
        }
        conn->producer =
            rd_kafka_new(RD_KAFKA_PRODUCER, conn->temp_conf, errstr,
                         sizeof(errstr));
        if (!conn->producer) {
        ...
        ldout(conn->cct,
              20) << "Kafka connect: successfully created new producer" <<
            dendl;
        {
      ...
      conf_error:
        conn->status = rd_kafka_last_error();
        ldout(conn->cct,
              1) << "Kafka connect: configuration failed: " << errstr << dendl;
        return false;
    }
...
// struct used for holding messages in the message queue
    struct message_wrapper_t {
        ...
        message_wrapper_t(const std::string & _conn_name,
                          ...
                          reply_callback_t _cb):conn_name(_conn_name),
            topic(_topic), message(_message), cb(_cb) {
    }};

    typedef std::unordered_map < std::string, connection_t_ptr > ConnectionList;
    typedef boost::lockfree::queue < message_wrapper_t *,
        boost::lockfree::fixed_sized < true >> MessageQueue;
    ...
    class Manager {
      ...
      private:
        ...
        void publish_internal(message_wrapper_t * message) {
            ...
            if (conn_it == connections.end()) {
                ldout(cct,
                      1) <<
                    "Kafka publish: connection was deleted while message was in the queue. error: "
            ...
            if (!conn->is_ok()) {
                ...
                ldout(conn->cct,
                      1) <<
                    "Kafka publish: producer was closed while message was in the queue. error: "
            ...
            auto topic_it =
                std::find(conn->topics.begin(), conn->topics.end(),
                          message->topic);
            rd_kafka_topic_t *topic = nullptr;
            if (topic_it == conn->topics.end()) {
                topic =
                    rd_kafka_topic_new(conn->producer, message->topic.c_str(),
                                       nullptr);
                if (!topic) {
                    const auto err = rd_kafka_last_error();
                    ldout(conn->cct,
                          1) << "Kafka publish: failed to create topic: " <<
                        message->
                        topic << " error: " << status_to_string(err) << dendl;
                    if (message->cb) {
                        message->cb(err);
                    }
                    conn->destroy(err);
                ...
                ldout(conn->cct,
                      20) << "Kafka publish: successfully created topic: " <<
                    message->topic << dendl;
            }
            else {
                topic = *topic_it;
                ldout(conn->cct,
                      20) << "Kafka publish: reused existing topic: " <<
                    message->topic << dendl;
            }

            const auto tag =
                (message->cb ==
                 nullptr ? nullptr : new uint64_t(conn->delivery_tag++));
            ...
            if (rc == -1) {
                const auto err = rd_kafka_last_error();
                ldout(conn->cct,
                      10) << "Kafka publish: failed to produce: " <<
            ...
            if (tag) {
                ...
                if (q_len < max_inflight) {
                    ...
                    ldout(conn->cct,
                          1) <<
                        "Kafka publish (with callback): failed with error: callback queue full"
                        << dendl;
        ...
        void run() noexcept {
            while (!stopped) {
                ...
                const auto send_count =
                    messages.
                    consume_all(std::
                                bind(&Manager::publish_internal, this,
                                     std::placeholders::_1));
                dequeued += send_count;
                ...
                for (; conn_it != end_it;) {
                    ...
                    if (conn->timestamp.sec() + max_idle_time <
                        ceph_clock_now()) {
                        ldout(conn->cct,
                              20) <<
                            "Time for deleting a connection due to idle behaviour: "
                    ...
                    if (!conn->is_ok()) {
                        ldout(conn->cct,
                              10) << "Kafka run: connection status is: " <<
                            status_to_string(conn->status) << dendl;
                    ...
                    reply_count +=
                        rd_kafka_poll(conn->producer, read_timeout_ms);
      ...
      public:
        Manager(size_t _max_connections,
                ...
                CephContext * _cct):max_connections(_max_connections),
            ...
            messages(max_queue),
            queued(0), dequeued(0), cct(_cct), runner(&Manager::run, this) {
            ...
            const auto rc =
                ceph_pthread_setname(runner.native_handle(), "kafka_manager");
            ceph_assert(rc == 0);
        }
        ...
        bool connect(std::string & broker,
            ...
            if (!user.empty() && !use_ssl
                ...
                ldout(cct,
                      1) <<
                    "Kafka connect: user/password are only allowed over secure connection"
                    << dendl;
                return false;
            }
            ...
            ldout(cct,
                  10) <<
                "Kafka connect: new connection is created. Total connections: "
                << connection_count << dendl;
            auto conn =
                connections.emplace(broker,
                                    std::make_unique < connection_t > (cct,
                                                                       broker,
                                                                       use_ssl,
                                                                       verify_ssl,
                                                                       ca_location,
            ...
            if (!new_producer(conn)) {
                ldout(cct,
                      10) <<
                    "Kafka connect: new connection is created. But producer creation failed. will retry"
                    << dendl;
            }
            return true;
        }
        ...
        int publish(const std::string & conn_name,
            ...
            if (messages.
                push(new message_wrapper_t(conn_name, topic, message, nullptr)))
            {
                ++queued;
                return STATUS_OK;
            }
        ...
        int publish_with_confirm(const std::string & conn_name,
            ...
            if (messages.
                push(new message_wrapper_t(conn_name, topic, message, cb))) {
                ++queued;
                return STATUS_OK;
            }
            return STATUS_QUEUE_FULL;
        ...
        // get the number of in-flight messages size_t get_inflight() const {
            size_t sum = 0;
            std::lock_guard lock(connections_lock);
        ...
        // running counter of the dequeued messages size_t get_dequeued() const {
            return dequeued;
    }};
...
// TODO make the pointer atomic in allocation and deallocation to avoid race conditions
    ...
    bool init(CephContext * cct) {
        ...
        s_manager =
            new Manager(MAX_CONNECTIONS_DEFAULT, MAX_INFLIGHT_DEFAULT,
                        MAX_QUEUE_DEFAULT, READ_TIMEOUT_MS_DEFAULT, cct);
        return true;
    }
    ...
    bool connect(std::string & broker, const std::string & url, bool use_ssl,
                 bool verify_ssl,
                 boost::optional < const std::string & >ca_location,
                 boost::optional < const std::string & >mechanism) {
        if (!s_manager)
