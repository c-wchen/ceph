...
namespace ceph {
    ...
    namespace rbd {
        ...
        } inline void decode(DirectoryState & state,
        ...
        inline void encode(const GroupImageLinkState & state,
                           ceph::buffer::list & bl, uint64_t features = 0) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }

        inline void decode(GroupImageLinkState & state,
                           ceph::buffer::list::const_iterator & it) {
            uint8_t int_state;
            using ceph::decode;
            decode(int_state, it);
        ...
        struct MirrorPeer {
            ...
            inline bool is_valid() const {
                ...
                case MIRROR_PEER_DIRECTION_TX:
                    break;
                    case MIRROR_PEER_DIRECTION_RX:case
                        MIRROR_PEER_DIRECTION_RX_TX:if (client_name.empty()) {
                        return false;
                    } break;
                default:
                    return false;
        ...
        struct MirrorImage {
            MirrorImage() {
            } MirrorImage(MirrorImageMode mode,
        ...
        inline void encode(const MirrorImageStatusState & state,
                           ceph::buffer::list & bl, uint64_t features = 0) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }
        ...
        struct MirrorImageSiteStatus {
            ...
            void decode_meta(uint8_t version,
                             ceph::buffer::list::const_iterator & it);
        ...
        struct MirrorImageSiteStatusOnDisk:cls::rbd::MirrorImageSiteStatus {
            ...
            } MirrorImageSiteStatusOnDisk(const cls::rbd::
                                          MirrorImageSiteStatus & status):cls::
                rbd::MirrorImageSiteStatus(status) {
            }
        ...
        WRITE_CLASS_ENCODER_FEATURES(MirrorImageSiteStatusOnDisk)
            ...
            int get_local_mirror_image_site_status(MirrorImageSiteStatus *
                                                   status) const;
        ...
        struct ParentImageSpec {
            ...
            snapid_t snap_id = CEPH_NOSNAP;
             ...
             ParentImageSpec() {
            } ParentImageSpec(int64_t pool_id,
                              const std::string & pool_namespace,
                              const std::string & image_id, snapid_t snap_id)
            :pool_id(pool_id), pool_namespace(pool_namespace),
                image_id(image_id), snap_id(snap_id) {
            }
        ...
        struct ChildImageSpec {
            int64_t pool_id = -1;
             ...
             ChildImageSpec() {
            } ChildImageSpec(int64_t pool_id,
                             const std::string & pool_namespace,
        ...
        struct GroupImageSpec {
            ...
            static int from_key(const std::string & image_key,
                                GroupImageSpec * spec);
        ...
        struct GroupImageStatus {
            GroupImageStatus() {
                               ...
                               int64_t pool_id, GroupImageLinkState state)
        ...
        struct TrashSnapshotNamespace {
            ...
            } TrashSnapshotNamespace(SnapshotNamespaceType
        ...
        inline void encode(const MirrorSnapshotState & state,
                           ceph::buffer::list & bl, uint64_t features = 0) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }

        inline void decode(MirrorSnapshotState & state,
                           ceph::buffer::list::const_iterator & it) {
            using ceph::decode;
            uint8_t int_state;
            decode(int_state, it);
        ...
        struct MirrorSnapshotNamespace {
            ...
            MirrorSnapshotNamespace(MirrorSnapshotState state,
                                    ...
                                    &mirror_peer_uuids,
                                    const std::string & primary_mirror_uuid,
                                    snapid_t primary_snap_id, bool complete,
            ...
            inline bool operator==(const MirrorSnapshotNamespace & rhs) const {
                ...
                else if (last_copied_object_number !=
                         rhs.last_copied_object_number) {
                    return last_copied_object_number <
                        rhs.last_copied_object_number;
                }
                else {
            ...
            void dump(ceph::Formatter * f) const;
             ...
             template < typename F > decltype(auto) visit(F && f) const & {
                return std::visit(std::forward < F > (f),
                                  static_cast <
                                  const SnapshotNamespaceVariant & >(*this));
            } template < typename F > decltype(auto) visit(F && f) & {
                return std::visit(std::forward < F > (f),
                                  static_cast <
                                  SnapshotNamespaceVariant & >(*this));
            } static void generate_test_instances(std::list <
                                                  SnapshotNamespace * >&o);
        };
        ...
        inline void encode(const GroupSnapshotState & state,
                           ceph::buffer::list & bl, uint64_t features = 0) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }

        inline void decode(GroupSnapshotState & state,
                           ceph::buffer::list::const_iterator & it) {
            using ceph::decode;
            uint8_t int_state;
            decode(int_state, it);
        ...
        struct ImageSnapshotSpec {
            ...
            snapid_t snap_id;
             ...
             ImageSnapshotSpec() {
                ...
                image_id(_image_id), snap_id(_snap_id) {
            }
        ...
        inline void encode(const TrashImageSource & source,
                           ceph::buffer::list & bl, uint64_t features = 0) {
            using ceph::encode;
            encode(static_cast < uint8_t > (source), bl);
        }

        inline void decode(TrashImageSource & source,
                           ceph::buffer::list::const_iterator & it) {
            uint8_t int_source;
            using ceph::decode;
        ...
        inline void encode(const TrashImageState & state,
                           ceph::buffer::list & bl) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }

        inline void decode(TrashImageState & state,
                           ceph::buffer::list::const_iterator & it) {
            uint8_t int_state;
            using ceph::decode;
            decode(int_state, it);
        ...
        inline void encode(const MigrationHeaderType & type,
                           ceph::buffer::list & bl) {
            using ceph::encode;
            encode(static_cast < uint8_t > (type), bl);
        }

        inline void decode(MigrationHeaderType & type,
                           ceph::buffer::list::const_iterator & it) {
            uint8_t int_type;
            using ceph::decode;
            decode(int_type, it);
        ...
        inline void encode(const MigrationState & state,
                           ceph::buffer::list & bl) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }

        inline void decode(MigrationState & state,
                           ceph::buffer::list::const_iterator & it) {
            uint8_t int_state;
            using ceph::decode;
            decode(int_state, it);
        ...
        struct MigrationSpec {
            ...
            MigrationState state = MIGRATION_STATE_ERROR;
             ...
             MigrationSpec() {
                            ...
                            uint64_t overlap, bool mirroring,
                            MirrorImageMode mirror_image_mode, bool flatten,
            ...
            :header_type(header_type), pool_id(pool_id),
                ...
                image_id(image_id), source_spec(source_spec),
                snap_seqs(snap_seqs), overlap(overlap), flatten(flatten),
                mirroring(mirroring), mirror_image_mode(mirror_image_mode),
                state(state), state_description(state_description) {
            }
        ...
        inline void encode(const AssertSnapcSeqState & state,
                           ceph::buffer::list & bl) {
            using ceph::encode;
            encode(static_cast < uint8_t > (state), bl);
        }

        inline void decode(AssertSnapcSeqState & state,
                           ceph::buffer::list::const_iterator & it) {
            uint8_t int_state;
            using ceph::decode;
            decode(int_state, it);
