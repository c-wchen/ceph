...
class RocksDBStore:public KeyValueDB {
  ...
  public:
    struct ColumnFamily {
        ...
        ColumnFamily(const std::string & name, size_t shard_cnt,
                     const std::string & options, uint32_t hash_l,
                     uint32_t hash_h)
        :name(name), shard_cnt(shard_cnt), options(options), hash_l(hash_l),
            hash_h(hash_h) {
    }};
  private:
    ...
    std::unordered_map < std::string,
        rocksdb::BlockBasedTableOptions > cf_bbt_opts;

    void add_column_family(const std::string & cf_name, uint32_t hash_l,
                           uint32_t hash_h, size_t shard_idx,
                           rocksdb::ColumnFamilyHandle * handle);
    bool is_column_family(const std::string & prefix);
    std::string_view get_key_hash_view(const prefix_shards & shards,
                                       const char *key, const size_t keylen);
    rocksdb::ColumnFamilyHandle * get_key_cf(const prefix_shards & shards,
                                             const char *key,
                                             const size_t keylen);
    rocksdb::ColumnFamilyHandle * get_cf_handle(const std::string & prefix,
                                                const std::string & key);
    rocksdb::ColumnFamilyHandle * get_cf_handle(const std::string & prefix,
                                                const char *key, size_t keylen);
    rocksdb::ColumnFamilyHandle *
        check_cf_handle_bounds(const cf_handles_iterator & it,
                               const IteratorBounds & bounds);

    int submit_common(rocksdb::WriteOptions & woptions,
                      KeyValueDB::Transaction t);
    int install_cf_mergeop(const std::string & cf_name,
  ...
  public:
    ...
    static void sharding_def_to_columns(const std::vector < ColumnFamily >
                                        &sharding_def,
    ...
    void compact_range_async(const std::string & start,
  ...
  public:
    ...
    uint64_t get_delete_range_threshold() const {
        return cct->_conf.get_val < uint64_t >
    ...
    int ParseOptionsFromString(const std::string & opt_str,
                               rocksdb::Options & opt);
    static int ParseOptionsFromStringStatic(CephContext * cct,
                                            const std::string & opt_str,
                                            rocksdb::Options & opt,
                                            std::function <
                                            int (const std::string &,
                                                 const std::string &,
                                                 rocksdb::Options &) > interp);
    ...
    void compact_range(const std::string & prefix, const std::string & start,
        ...
        compact_range(combine_strings(prefix, start),
    ...
    RocksDBStore(CephContext * c, const std::string & path,
                 std::map < std::string, std::string > opt, void *p):cct(c),
        logger(NULL), path(path), kv_options(opt), priv(p), db(NULL),
        env(static_cast < rocksdb::Env * >(p)), comparator(nullptr),
        dbstats(NULL), compact_queue_stop(false), compact_thread(this),
        compact_on_mount(false), disableWAL(false) {
    }
    ...
    int open_read_only(std::ostream & out, const std::string & cfs =
                       "") override {
    ...
    void split_stats(const std::string & s, char delim,
    ...
    class RocksDBTransactionImpl:public KeyValueDB::TransactionImpl {
      ...
      private:
        void put_bat(rocksdb::WriteBatch & bat,
                     rocksdb::ColumnFamilyHandle * cf,
                     const std::string & k, const ceph::bufferlist & to_set_bl);
      public:
        void set(const std::string & prefix,
                 const std::string & k, const ceph::bufferlist & bl) override;
        void set(const std::string & prefix,
                 const char *k,
                 size_t keylen, const ceph::bufferlist & bl) override;
        void rmkey(const std::string & prefix, const std::string & k) override;
        void rmkey(const std::string & prefix,
                   const char *k, size_t keylen) override;
        ...
        void rm_range_keys(const std::string & prefix,
                           const std::string & start,
                           const std::string & end) override;
        void merge(const std::string & prefix,
                   const std::string & k, const ceph::bufferlist & bl) override;
    };
    ...
    int get(const std::string & prefix,
            const std::string & key, ceph::bufferlist * out) override;
    int get(const std::string & prefix,
            const char *key, size_t keylen, ceph::bufferlist * out) override;

    class RocksDBWholeSpaceIteratorImpl:public KeyValueDB::
    ...
    Iterator get_iterator(const std::string & prefix, IteratorOpts opts =
                          0, IteratorBounds = IteratorBounds())override;
  private:
  ...
  public:
    /// Utility
    static std::string combine_strings(const std::string & prefix,
    ...
    static int split_key(rocksdb::Slice in, std::string * prefix,
                         std::string * key);
    ...
    int set_merge_operator(const std::string & prefix,
                           std::shared_ptr < KeyValueDB::MergeOperator >
                           mop) override;
    std::string assoc_name;     ///< Name of associative operator

    uint64_t get_estimated_size(std::map < std::string,
                                uint64_t > &extra) override {
  ...
  public:
    ...
    int reshard(const std::string & new_sharding, const resharding_ctrl * ctrl =
                nullptr);
