...
public:
        ...
        driver(_driver), bucket(_bucket)
    {
        ...
        list_params.allow_unordered =
            true; // XXX can be unconditionally true, so long as all versions of one object are assured to be on one shard and always ordered on that shard (true today in RADOS)
...
public:
    ...
    int process(lc_op_ctx& oc) override
    {
        ...
        if (oc.o.meta.category == RGWObjCategory::CloudTiered) {
            ...
            ldpp_dout(oc.dpp, 30) << "Object(key:" << oc.o.key << ") is already cloud tiered to cloud-s3 tier: " <<
        ...
        if (!r && oc.tier->get_tier_type() == "cloud-s3") {
            ...
            if (!oc.o.is_current() &&
                ...
                ldpp_dout(oc.dpp, 10) << "Object(key:" << oc.o.key << ") is locked. Skipping transition to cloud-s3 tier: " <<
    ...
    for (const auto& ri : rule_map) {
        ...
        if (rule_expiration.has_date()) {
            ...
            if (rule_expiration.has_days()) {
                rule_expiration_date =
                    boost::optional < ceph::real_time > (
                        mtime + make_timespan(double(rule_expiration.get_days()) * secs_in_a_day - ceph::real_clock::to_time_t(mtime) %
                                              (secs_in_a_day) + secs_in_a_day));
            }
        }
    ...
    for (const auto& ri : rule_map) {
        ...
        if (mp_expiration.has_days()) {
            rule_abort_date = std::optional < ceph::real_time > (
                                  mtime + make_timespan(mp_expiration.get_days() * secs_in_a_day - ceph::real_clock::to_time_t(mtime) %
                                      (secs_in_a_day) + secs_in_a_day));
        }
