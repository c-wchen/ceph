...
////////////////////////////// ConfLine //////////////////////////////
ConfLine::ConfLine(const std::string & key_, const std::string & val_,
...
bool ConfLine::operator<(const ConfLine & rhs) const const
{
           ...
           std::ostream * warnings)
{
    ...
    if (!fp) {
        ostringstream oss;
        oss << __func__ << ": cannot open " << fname << ": " <<
            cpp_strerror(errno);
    ...
    if (fstat(fileno(fp), &st_buf)) {
        ret = -errno;
        ostringstream oss;
        oss << __func__ << ": failed to fstat '" << fname << "': " <<
            cpp_strerror(ret);
        errors->push_back(oss.str());
        goto done;
    }
    ...
    if (st_buf.st_size > MAX_CONFIG_FILE_SZ) {
        ostringstream oss;
        oss << __func__ << ": config file '" << fname << "' is " << st_buf.
            st_size << " bytes, but the maximum is " << MAX_CONFIG_FILE_SZ;
        errors->push_back(oss.str());
        ret = -EINVAL;
        goto done;
    }
...
int ConfFile::
read(const std::string & section, const std::string & key,
...
ConfFile::const_section_iter_t ConfFile::sections_begin() constconst
{
    return sections.begin();
}

ConfFile::const_section_iter_t ConfFile::sections_end() constconst
{
    return sections.end();
}
                 ...
                 std::ostream * warnings)
{
    ...
    while (1) {
        ...
        if (!end) {
            ostringstream oss;
            oss << "read_conf: ignoring line " << line_no <<
                " because it doesn't " <<
        ...
        if (!csection.empty()) {
            std::map < std::string, ConfSection >::value_type nt(csection,
                                                                 ConfSection());
            ...
            if (cur_section->second.lines.count(*cline)) {
                ...
                if (cline->key.length() && warnings)
                    *warnings << "warning: line " << line_no << ": '" << cline->
                        key << "' in section '" << cur_section->
                        first << "' redefined " << std::endl;
            }
    ...
    while (true) {
        ...
        case ACCEPT_COMMENT_START:
            if (c == '\0') {
                ...
                ostringstream oss;
                oss << "unexpected character at char " << (l -
                                                           line) << " of line "
                    << line_no;
                errors->push_back(oss.str());
                return NULL;
