...
" script ": " function json_echo(input, output) output: append(input:str());
end objclass.register(json_echo) ",
            " handler ": " json_echo "
    ,
            " input ": " omg it works "
    ,
                                                           ...
                                                           size, mtime = objclass.stat()
                                                               ...
                                                               ok, ret, size, mtime = pcall(objclass.stat, o)
                                                                   ...
                                                                   return ret
                                                                       ...
                                                                       --
                                                                       function rv_h() end
                                                                       function rv_h1() return 1;
    end
    function rv_h0() return 0;
    end
    function rv_hn1() return -1;
    end
    function rv_hs1() return '1';
    end
    function rv_hs0() return '0';
    end
    function rv_hsn1() return '-1';
    end
    function rv_hnil() return nil;
    end
    function rv_ht() return {};
    end
    function rv_hstr() return 'asdf';
    end
    ...
    objclass.register(rv_h)
            ...
            -- Create
            --
            function create_c() objclass.create(true);
    end
    function create_cne() objclass.create(false);
    end
    ...
    objclass.register(create_c)
            ...
            -- Pcall
            --
            function pcall_c() objclass.create(true);
    end
                               ...
                               ok, ret = pcall(objclass.create, true)
                                         ...
                                         ok, ret = pcall(objclass.create, true)
                                             ...
                                             return -9999
                                                 ...
                                                 -- Remove
                                                 --
                                                 function remove_c() objclass.create(true);
    end
    function remove_r() objclass.remove();
    end
...
do
    table.insert(arr, n)
         ...
         output_str = " "
                      for i, key in ipairs(arr)
                          do
                  output_str = output_str .. key .. " / " .. xattrs[key]:
                                           str() .. " / "
                                           end
                               output:
                                           append(output_str)
                                                                                              ...
                                                                                              for n in pairs(kvs)
                                                                                                  do
                                                                                                      ...
                                                                                                      output_str = " "
                                                                                                          for i, key in ipairs(arr)
                                                                                                              do
                                                              output_str = output_str .. key .. " / " .. kvs[key]:
                                                                                                                      str() .. " / "
                                                                                                                      end
                                                                  output:
                                                                                                                      append(output_str)
...
protected:
    ...
    void SetUp() override
    {
        ...
        const test_info =::testing::UnitTest::GetInstance()->
                         current_test_info();
...
TEST_F(ClsLua, Stat)
{
    ...
    ASSERT_EQ(-ENOENT,
              __clslua_exec("dne", test_script, NULL, "stat_sdne_pcall"));
}
...
TEST_F(ClsLua, Log)
{
    ...
    ASSERT_EQ(0,
              clslua_exec
              ("s = objclass.log('one', 'two', 'three'); objclass.log(s);"));
}
...
TEST_F(ClsLua, MapSetVals)
{
    ...
    ASSERT_STREQ("a_val",
                 std::string(out_vals["a"].c_str(),
                             out_vals["a"].length()).c_str());
    ...
    ASSERT_STREQ("a_val",
                 std::string(out_vals["a"].c_str(),
                             out_vals["a"].length()).c_str());
    ASSERT_STREQ("b_val",
                 std::string(out_vals["b"].c_str(),
                             out_vals["b"].length()).c_str());
    ...
    ASSERT_STREQ("b_val",
                 std::string(out_vals["b"].c_str(),
                             out_vals["b"].length()).c_str());
    ASSERT_STREQ("c_val",
                 std::string(out_vals["c"].c_str(),
                             out_vals["c"].length()).c_str());
    ...
    ASSERT_EQ(2, (int)out_vals.size());
    ASSERT_STREQ("1_val",
                 std::string(out_vals["1"].c_str(),
                             out_vals["1"].length()).c_str());
    ASSERT_STREQ("2_val",
                 std::string(out_vals["2"].c_str(),
                             out_vals["2"].length()).c_str());
