                      ...
                      const ec_profile_t & ec_profile, DoutPrefixProvider & dpp)
{
    ...
    case pg_pool_t::TYPE_ERASURE:
        return std::make_unique < ECBackend > (pg_shard.shard, coll,
                                               shard_services,
                                               std::move(ec_profile),
                                               pool.stripe_width, dpp);
    default:
...
{
    return interruptor::make_interruptible(store->get_attrs(coll, ghobject_t {
                                                            oid,
                                                            ghobject_t::NO_GEN,
                                                            shard}
                                           )).
        safe_then_interruptible([oid] (auto && attrs)->
                                load_metadata_ertr::future <
                                loaded_object_md_t::ref > {
                                loaded_object_md_t::
                                ref ret(new loaded_object_md_t());
                                if (auto oiiter = attrs.find(OI_ATTR);
                                    oiiter != attrs.end()) {
                                bufferlist bl = std::move(oiiter->second); try {
                                ret->os = ObjectState(object_info_t(bl, oid),
                                                      true);}
                                ...
                                ("load_metadata: object {} present but missing object info",
                                 oid);
                                return crimson::ct_error::object_corrupted::
                                make();}
                                ...
                                if (auto ssiter = attrs.find(SS_ATTR);
                                    ssiter != attrs.end()) {
                                ...
                                loaded_object_md_t::ref > (std::move(ret));}
...
PGBackend::rep_op_fut_t
    PGBackend::mutate_object(std::set < pg_shard_t > pg_shards,
                             ...
                             std::vector < pg_log_entry_t > &&log_entries)
{
    ...
    if (obc->obs.exists) {
        ...
        // object_info_t
        {
            ...
            txn.setattr(coll->get_cid(), ghobject_t {
                        obc->obs.oi.soid}
                        , OI_ATTR, osv);
        }
    ...
    return _submit_transaction(std::move(pg_shards), obc->obs.oi.soid,
                               std::move(txn), std::move(osd_op_p), min_epoch,
                               map_epoch, std::move(log_entries));
}

static inline bool _read_verify_data(const object_info_t & oi,
                    ...
                    object_stat_sum_t & delta_stats)
{
    ...
    return _read(oi.soid, offset, length,
                 op.flags).safe_then_interruptible_tuple([&delta_stats, &oi,
                                                          &osd_op] (auto
                                                                    && bl)->
                                                         read_errorator::
                                                         future <> {
                                                         if (!_read_verify_data
                                                             (oi, bl)) {
                                                         // crc mismatches
                                                         return crimson::
                                                         ct_error::
                           ...
                           object_stat_sum_t & delta_stats)
{
    ...
    return interruptor::make_interruptible(store->fiemap(coll, ghobject_t {
                                                         os.oi.soid}
                                                         ,
                                                         offset,
                                                         adjusted_length)).
        safe_then_interruptible([&delta_stats, &os, &osd_op, this] (auto && m) {
                                return seastar::do_with(interval_set <
                                                        uint64_t > {
                                                        std::move(m)}
                                                        ,
                                                        [&delta_stats, &os,
...
namespace {
    ...
    template < class CSum >
        ...
        Checksummer::calculate < CSum > (init_value, chunk_size, 0,
                                         buf.length(), buf, &csum_data);
    ...
    PGBackend::checksum(const ObjectState & os, OSDOp & osd_op)
{
    ...
    if (checksum.chunk_size > 0) {
        if (checksum.length == 0) {
            logger().warn("{}: length required when chunk size provided",
                          __func__);
            return crimson::ct_error::invarg::make();
        }
    ...
    return _read(os.oi.soid, checksum.offset, checksum.length, osd_op.op.flags)
        .safe_then_interruptible([&osd_op] (auto && read_bl) mutable->
                                 checksum_errorator::future <> {
                                 auto & checksum = osd_op.op.checksum;
...
return do_checksum < Checksummer::xxhash32 > (osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata); case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH64:
return do_checksum < Checksummer::xxhash64 > (osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata); case CEPH_OSD_CHECKSUM_OP_TYPE_CRC32C:
return do_checksum < Checksummer::crc32c > (osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata); default:
                                 logger().
                                 warn("checksum: unknown crc type ({})",
    ...
    PGBackend::cmp_ext(const ObjectState & os, OSDOp & osd_op)
{
    ...
    auto read_ext =
        ll_read_ierrorator::make_ready_future < ceph::bufferlist > ();
    if (ext_len == 0) {
    ...
    return read_ext.safe_then_interruptible([&osd_op] (auto && read_bl)
                                            ...
                                            (index <
                                             read_bl.
                                             length()? read_bl[index] : 0);
                                            if (byte_in_op != byte_from_disk) {
                                            logger().
                                            debug("cmp_ext: mismatch at {}",
                                                  index);
                                            ...
                                            return crimson::ct_error::cmp_fail::
                                            make();}
                                            }
                                            osd_op.rval = 0;
                                            return cmp_ext_errorator::
                                            make_ready_future <> ();}
    ) ;
}

PGBackend::stat_ierrorator::future <>
    PGBackend::stat(const ObjectState & os,
                    OSDOp & osd_op, object_stat_sum_t & delta_stats)
{
    if (os.exists /* TODO: && !os.is_whiteout() */ ) {
        logger().debug("stat os.oi.size={}, os.oi.mtime={}", os.oi.size,
                       os.oi.mtime);
...
PGBackend::write_iertr::future <> PGBackend::_writefull(ObjectState & os,
                                                        off_t truncate_size,
                                                        const bufferlist & bl,
                                                        ceph::os::
                                                        Transaction & txn,
                                                        ...
                                                        unsigned flags)
{
    ...
    if (existing && bl.length() < os.oi.size) {
        ...
        txn.truncate(coll->get_cid(), ghobject_t {
                     os.oi.soid}
                     , bl.length());
        truncate_update_size_and_usage(delta_stats, os.oi, truncate_size);

        osd_op_params.clean_regions.mark_data_region_dirty(bl.length(),
                                                           os.oi.size -
                                                           bl.length());
    }
    if (bl.length()) {
        txn.write(coll->get_cid(), ghobject_t {
                  os.oi.soid}
                  , 0, bl.length(), bl, flags);
...
PGBackend::write_iertr::future <> PGBackend::_truncate(ObjectState & os,
                                                       ceph::os::
                                                       Transaction & txn,
                                                       osd_op_params_t &
                                                       osd_op_params,
                                                       ...
                                                       uint32_t truncate_seq)
{
    ...
    if (os.oi.size != offset) {
        ...
        if (os.oi.size > offset) {
            ...
            osd_op_params.clean_regions.mark_data_region_dirty(offset,
                                                               os.oi.size -
                                                               offset);
        }
        else {
...
bool PGBackend::maybe_create_new_object(ObjectState & os,
                                        ceph::os::Transaction & txn,
                                        object_stat_sum_t & delta_stats)
{
    if (!os.exists) {
        ...
        txn.touch(coll->get_cid(), ghobject_t {
                  os.oi.soid}
        );
...
static bool is_offset_and_length_valid(const std::uint64_t offset,
...
PGBackend::interruptible_future <> PGBackend::set_allochint(ObjectState & os,
                                                            const OSDOp &
                                                            osd_op,
                                                            ceph::os::
                                                            Transaction & txn,
                                                            ...
                                                            delta_stats)
{
    ...
    txn.set_alloc_hint(coll->get_cid(), ghobject_t {
                       os.oi.soid},
...
PGBackend::write_iertr::future <> PGBackend::write(ObjectState & os,
                                                   const OSDOp & osd_op,
                                                   ceph::os::Transaction & txn,
                                                   osd_op_params_t &
                                                   osd_op_params,
    ...
    if (auto seq = os.oi.truncate_seq; seq != 0 && op.extent.truncate_seq < seq) {
        ...
        if (os.exists && !os.oi.is_whiteout()) {
            ...
            if (op.extent.truncate_size != os.oi.size) {
                os.oi.size = length;
                if (op.extent.truncate_size > os.oi.size) {
                    osd_op_params.clean_regions.mark_data_region_dirty(os.oi.
                                                                       size,
                                                                       op.
                                                                       extent.
                                                                       truncate_size
                    ...
                    osd_op_params.clean_regions.mark_data_region_dirty(op.
                                                                       extent.
                                                                       truncate_size,
                                                                       os.oi.
                                                                       size -
            ...
            truncate_update_size_and_usage(delta_stats, os.oi,
                                           op.extent.truncate_size);
        }
...
PGBackend::interruptible_future <> PGBackend::write_same(ObjectState & os,
                                                         const OSDOp & osd_op,
                                                         ceph::os::
                                                         Transaction & txn,
                                                         osd_op_params_t &
                                                         ...
                                                         delta_stats)
{
    ...
    txn.write(coll->get_cid(), ghobject_t {
              os.oi.soid}
              , op.writesame.offset, len, std::move(repeated_indata), op.flags);
    update_size_and_usage(delta_stats, os.oi, op.writesame.offset, len);
    osd_op_params.clean_regions.mark_data_region_dirty(op.writesame.offset,
                                                       len);
    return seastar::now();
}

PGBackend::write_iertr::future <> PGBackend::writefull(ObjectState & os,
                                                       const OSDOp & osd_op,
                                                       ceph::os::
                                                       Transaction & txn,
                                                       osd_op_params_t &
                                                       ...
                                                       delta_stats)
{
    ...
    return _writefull(os,
                      op.extent.truncate_size,
                      osd_op.indata, txn, osd_op_params, delta_stats, op.flags);
}

PGBackend::rollback_iertr::future <> PGBackend::rollback(ObjectState & os,
                                                         const OSDOp & osd_op,
                                                         ceph::os::
                                                         Transaction & txn,
                                                         osd_op_params_t &
                                                         ...
                                                         obc_loader)
{
    ...
    return obc_loader.with_clone_obc_only < RWState::RWWRITE > (head,
                                                                target_coid,
                                                                [this, &os,
                                                                 &txn,
                                                                 &delta_stats,
...
PGBackend::append_ierrorator::future <> PGBackend::append(ObjectState & os,
                                                          OSDOp & osd_op,
                                                          ceph::os::
                                                          Transaction & txn,
                                                          osd_op_params_t &
...
PGBackend::write_iertr::future <> PGBackend::truncate(ObjectState & os,
                                                      const OSDOp & osd_op,
                                                      ceph::os::
                                                      Transaction & txn,
                                                      osd_op_params_t &
                                                      ...
                                                      delta_stats)
{
    ...
    return _truncate(os, txn, osd_op_params, delta_stats,
                     op.extent.offset, op.extent.truncate_size,
                     op.extent.truncate_seq);
}

PGBackend::write_iertr::future <> PGBackend::zero(ObjectState & os,
                                                  const OSDOp & osd_op,
                                                  ceph::os::Transaction & txn,
                                                  osd_op_params_t &
                                                  osd_op_params,
                                                  ...
                                                  delta_stats)
{
    ...
    txn.zero(coll->get_cid(), ghobject_t {
             os.oi.soid}
...
PGBackend::create_iertr::future <> PGBackend::create(ObjectState & os,
                                                     const OSDOp & osd_op,
                                                     ceph::os::
                                                     Transaction & txn,
                                                     object_stat_sum_t &
                                                     delta_stats)
{
    ...
    maybe_create_new_object(os, txn, delta_stats);
    txn.create(coll->get_cid(), ghobject_t {
               os.oi.soid, ghobject_t::NO_GEN, shard}
    );
    return seastar::now();
}
...
{
    // todo: snapset
    txn.remove(coll->get_cid(), ghobject_t {
               os.oi.soid, ghobject_t::NO_GEN, shard}
    );
    os.oi.size = 0;
    os.oi.new_object();
...
PGBackend::interruptible_future < std::tuple < std::vector < hobject_t >,
    hobject_t >> PGBackend::list_objects(const hobject_t & start,
                                         uint64_t limit) constconst
{
    auto gstart =
        start.is_min()? ghobject_t { } : ghobject_t { start, 0, shard };
    return interruptor::make_interruptible(store->
                                           list_objects(coll, gstart,
                                                        ghobject_t::get_max(),
                                                        limit))
        .then_interruptible([](auto ret) {
                            ...
                            boost::copy(gobjects |
                                        ...
                                        boost::adaptors::
                                        transformed([](const ghobject_t & o) {
                                                    return o.hobj;}
                                        ),
                                        std::back_inserter(objects));
                            return seastar::make_ready_future < std::tuple <
                            std::vector < hobject_t >,
                            hobject_t >> (std::make_tuple(objects, next.hobj));}
    ) ;
}

PGBackend::setxattr_ierrorator::future <> PGBackend::setxattr(ObjectState & os,
                                                              const OSDOp &
                                                              osd_op,
                                                              ceph::os::
                                                              Transaction & txn,
                                                              ...
                                                              & delta_stats)
{
    ...
    const auto max_name_len =
        std::min < uint64_t > (store->get_max_attr_name_length(),
                               local_conf()->osd_max_attr_name_len);
...
PGBackend::get_attr_ierrorator::future <> PGBackend::
getxattr(const ObjectState & os, OSDOp & osd_op,
         object_stat_sum_t & delta_stats) constconst
{
    std::string name;
    ceph::bufferlist val;
    {
    ...
    return getxattr(os.oi.soid, std::move(name)).safe_then_interruptible([&delta_stats, &osd_op] (ceph::bufferlist && val) {
                                                                         osd_op.
                                                                         outdata
                                                                         =
                                                                         std::
...
PGBackend::get_attr_ierrorator::future < ceph::bufferlist >
    PGBackend::getxattr(const hobject_t & soid, std::string_view key) constconst
{
    return store->get_attr(coll, ghobject_t {
                           soid}
                           , key);
}

PGBackend::get_attr_ierrorator::future < ceph::bufferlist >
    PGBackend::getxattr(const hobject_t & soid, std::string && key) constconst
{
...
PGBackend::get_attr_ierrorator::future <> PGBackend::
get_xattrs(const ObjectState & os, OSDOp & osd_op,
           object_stat_sum_t & delta_stats) constconst
{
    return store->get_attrs(coll, ghobject_t {
                            os.oi.soid}
    ).safe_then([&delta_stats, &osd_op] (auto && attrs) {
      std::vector < std::pair < std::string, bufferlist >> user_xattrs; ceph::bufferlist bl; for (auto &[key, val]:attrs)
                {
...
static int do_xattr_cmp_u64(int op, uint64_t lhs, bufferlist & rhs_xattr)
{
    ...
    if (rhs_xattr.length() > 0) {
        ...
        if (auto[p, ec] =
            std::from_chars(first, first + rhs_xattr.length(), rhs);
            ec != std::errc()) {
            return -EINVAL;
        }
    }
...
PGBackend::cmp_xattr_ierrorator::future <> PGBackend::
cmp_xattr(const ObjectState & os, OSDOp & osd_op,
          object_stat_sum_t & delta_stats) constconst
{
    std::string name {
    "_"};
    ...
    return getxattr(os.oi.soid, std::move(name)).safe_then_interruptible([&delta_stats, &osd_op] (auto && xattr)->cmp_xattr_ierrorator::future <> {
                                                                         delta_stats.
                                                                         num_rd++;
                                                                         delta_stats.
                                                                         num_rd_kb
...
case CEPH_OSD_CMPXATTR_MODE_STRING:
                                                                         ...
                                                                         ("cmpxattr lhs={}, rhs={}",
                                                                          lhs,
                                                                          rhs);}
break; case CEPH_OSD_CMPXATTR_MODE_U64:
                                                                         {
                                                                         uint64_t
                                                                         lhs;
                                                                         try {
                                                                         ...
                                                                         ().
                                                                         info
                                                                         ("cmp_xattr: buffer error expection");
                                                                         result
                                                                         =
                                                                         -EINVAL;
                                                                         break;}
...
break; default:
                                                                         ...
                                                                         ().
                                                                         info
                                                                         ("cmp_xattr: comparison returned false");
                                                                         return
                                                                         crimson::
                                                                         ct_error::
                                                                         ecanceled::
        ...
        .handle_error_interruptible(crimson::ct_error::enodata::
                                    handle([&delta_stats, &osd_op] ()
                                           ->cmp_xattr_errorator::future <> {
                                           delta_stats.num_rd++;
                                           delta_stats.num_rd_kb +=
                                           shift_round_up(osd_op.op.xattr.
                                                          value_len, 10);
                                           return crimson::ct_error::ecanceled::
                                           make();}
...
PGBackend::rm_xattr_iertr::future <>
    PGBackend::rm_xattr(ObjectState & os,
                        const OSDOp & osd_op, ceph::os::Transaction & txn)
{
    if (!os.exists || os.oi.is_whiteout()) {
        logger().debug("{}: {} DNE", __func__, os.oi.soid);
        return crimson::ct_error::enoent::make();
    ...
    txn.rmattr(coll->get_cid(), ghobject_t {
               os.oi.soid}
               , attr_name);
...
void PGBackend::clone(
                         ...
                         const ObjectState & d_os, ceph::os::Transaction & txn)
{
    // See OpsExecutor::execute_clone documentation
    txn.clone(coll->get_cid(), ghobject_t {
              os.oi.soid}, ghobject_t {
              d_os.oi.soid});
    {
        ceph::bufferlist bv;
        snap_oi.encode_no_oid(bv, CEPH_FEATURES_ALL);
        txn.setattr(coll->get_cid(), ghobject_t {
                    d_os.oi.soid}, OI_ATTR, bv);
    }
    txn.rmattr(coll->get_cid(), ghobject_t {
               d_os.oi.soid}, SS_ATTR);
}

using get_omap_ertr =
    crimson::os::FuturizedStore::Shard::read_errorator::extend <
    crimson::ct_error::enodata >;
using get_omap_iertr =::crimson::interruptible::interruptible_errorator <::
    crimson::osd::IOInterruptCondition, get_omap_ertr >;
static get_omap_iertr::future <
    crimson::os::FuturizedStore::Shard::omap_values_t >
maybe_get_omap_vals_by_keys(crimson::os::FuturizedStore::Shard * store,
                            const crimson::os::CollectionRef & coll,
                            const object_info_t & oi,
                            const std::set < std::string > &keys_to_get)
{
    if (oi.is_omap()) {
        return store->omap_get_values(coll, ghobject_t {
                                      oi.soid}
                                      , keys_to_get);
    }
    else {
...
maybe_get_omap_vals(crimson::os::FuturizedStore::Shard * store,
                    const crimson::os::CollectionRef & coll,
                    const object_info_t & oi, const std::string & start_after)
{
    if (oi.is_omap()) {
        return store->omap_get_values(coll, ghobject_t {
                                      oi.soid}
...
PGBackend::ll_read_ierrorator::future < ceph::bufferlist >
    PGBackend::omap_get_header(const crimson::os::CollectionRef & c,
                               const ghobject_t & oid) constconst
{
    return store->omap_get_header(c, oid)
        .handle_error(crimson::ct_error::enodata::handle([] {
                                                         return seastar::
                                                         make_ready_future <
                                                         bufferlist > ();}
...
PGBackend::ll_read_ierrorator::future <>
    PGBackend::omap_get_header(const ObjectState & os,
                               OSDOp & osd_op,
                               object_stat_sum_t & delta_stats) constconst
{
    return omap_get_header(coll, ghobject_t {
                           os.oi.soid}
    ).safe_then_interruptible([&delta_stats, &osd_op] (ceph::bufferlist
                                                       && header) {
                              osd_op.outdata = std::move(header);
                              delta_stats.num_rd_kb +=
                              shift_round_up(osd_op.outdata.length(), 10);
                              delta_stats.num_rd++; return seastar::now();}
    );
}

PGBackend::ll_read_ierrorator::future <>
    PGBackend::omap_get_keys(const ObjectState & os,
                             OSDOp & osd_op,
                             object_stat_sum_t & delta_stats) constconst
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    return maybe_get_omap_vals(store, coll, os.oi,
                               start_after).safe_then_interruptible([ =,
                                                                     &delta_stats,
                                                                     &osd_op]
                                                                    (auto ret) {
      ceph::bufferlist result; bool truncated = false; uint32_t num = 0; for (auto &[key, val]:std::get < 1 > (ret)) {
                                                                    if (num >=
                                                                        max_return
                                                                        ||
                                                                        result.
...
static
 PGBackend::omap_cmp_ertr::future <> do_omap_val_cmp(std::map < std::string,
                                                     bufferlist,
                                                     std::less <>> out,
                                                     std::map < std::string,
                                                     std::pair < bufferlist,
...
PGBackend::omap_cmp_iertr::future <>
    PGBackend::omap_cmp(const ObjectState & os,
                        OSDOp & osd_op,
                        object_stat_sum_t & delta_stats) constconst
{
    if (!os.exists || os.oi.is_whiteout()) {
      ...
      for (auto & i:assertions) {
        ...
        return store->omap_get_values(coll, ghobject_t {
            ...
            .safe_then([ =, &osd_op] (auto && out)->omap_cmp_iertr::future <> {
                       osd_op.rval = 0; return do_omap_val_cmp(out, assertions);}
        );
    }
    else {
        return crimson::ct_error::ecanceled::make();
...
PGBackend::ll_read_ierrorator::future <>
    PGBackend::omap_get_vals(const ObjectState & os,
                             OSDOp & osd_op,
                             object_stat_sum_t & delta_stats) constconst
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    return maybe_get_omap_vals(store, coll, os.oi, start_after)
        .safe_then_interruptible([ =, &osd_op] (auto && ret) {
                                 ...
                                 auto iter =
                                 filter_prefix >
                                 start_after ? vals.lower_bound(filter_prefix)
                                 : std::begin(vals); for (;
                                                          iter !=
                                                          std::end(vals);
                                                          ++iter) {
                                 const auto &[key, value] = *iter;
                                 if (key.substr(0, filter_prefix.size()) !=
                                     filter_prefix) {
                                 break;}
                                 else
                                 if (num >= max_return ||
...
PGBackend::ll_read_ierrorator::future <>
    PGBackend::omap_get_vals_by_keys(const ObjectState & os,
                                     OSDOp & osd_op,
                                     object_stat_sum_t & delta_stats) constconst
{
    if (!os.exists || os.oi.is_whiteout()) {
...
PGBackend::interruptible_future <>
    PGBackend::omap_set_vals(ObjectState & os,
                             const OSDOp & osd_op,
                             ceph::os::Transaction & txn,
                             ...
                             object_stat_sum_t & delta_stats)
{
    ...
    txn.omap_setkeys(coll->get_cid(), ghobject_t {
                     os.oi.soid}, to_set_bl);
...
PGBackend::interruptible_future <>
    PGBackend::omap_set_header(ObjectState & os,
                               const OSDOp & osd_op,
                               ceph::os::Transaction & txn,
                               osd_op_params_t & osd_op_params,
                               object_stat_sum_t & delta_stats)
{
    maybe_create_new_object(os, txn, delta_stats);
    txn.omap_setheader(coll->get_cid(), ghobject_t {
                       os.oi.soid}, osd_op.indata);
    osd_op_params.clean_regions.mark_omap_dirty();
    delta_stats.num_wr++;
    os.oi.set_flag(object_info_t::FLAG_OMAP);
    os.oi.clear_omap_digest();
...
PGBackend::interruptible_future <> PGBackend::
omap_remove_range(ObjectState & os, const OSDOp & osd_op,
                  ceph::os::Transaction & txn, object_stat_sum_t & delta_stats)
{
    ...
    txn.omap_rmkeyrange(coll->get_cid(), ghobject_t {
                        os.oi.soid}, key_begin, key_end);
    delta_stats.num_wr++;
    os.oi.clear_omap_digest();
    return seastar::now();
}

PGBackend::interruptible_future <> PGBackend::omap_remove_key(ObjectState & os,
                                                              const OSDOp &
                                                              osd_op,
                                                              ceph::os::
                                                              Transaction & txn)
{
    ...
    txn.omap_rmkeys(coll->get_cid(), ghobject_t {
                    os.oi.soid}, to_rm_bl);
    // TODO:
...
PGBackend::omap_clear_iertr::future <>
    PGBackend::omap_clear(ObjectState & os,
                          OSDOp & osd_op,
                          ceph::os::Transaction & txn,
                          osd_op_params_t & osd_op_params,
                          object_stat_sum_t & delta_stats)
{
    ...
    txn.omap_clear(coll->get_cid(), ghobject_t {
                   os.oi.soid}
    );
    osd_op_params.clean_regions.mark_omap_dirty();
    delta_stats.num_wr++;
...
PGBackend::interruptible_future < struct stat >PGBackend::stat(CollectionRef c,
                                                               const ghobject_t
                                                               & oid) const
    const
{
...
PGBackend::read_errorator::future < std::map < uint64_t, uint64_t >>
    PGBackend::fiemap(CollectionRef c,
                      const ghobject_t & oid, uint64_t off, uint64_t len)
{
    return store->fiemap(c, oid, off, len);
}

PGBackend::write_iertr::future <> PGBackend::tmapput(ObjectState & os,
                                                     const OSDOp & osd_op,
                                                     ceph::os::
                                                     Transaction & txn,
                                                     object_stat_sum_t &
                                                     ...
                                                     osd_op_params)
{
    ...
    if (!ret.has_value()) {
        logger().debug("PGBackend::tmapup: {}, ret={}", os.oi.soid,
                       ret.error());
        ...
        return _writefull(os,
                          bl.length(),
                          std::move(bl), txn, osd_op_params, delta_stats, 0);
    }
}

PGBackend::tmapup_iertr::future <> PGBackend::tmapup(ObjectState & os,
                                                     const OSDOp & osd_op,
                                                     ceph::os::
                                                     Transaction & txn,
                                                     object_stat_sum_t &
                                                     ...
                                                     osd_op_params)
{
    ...
    return PGBackend::write_iertr::now().si_then([this, &os] {
                                                 return _read(os.oi.soid, 0,
                                                              os.oi.size, 0);}
    ).handle_error_interruptible(crimson::ct_error::enoent::handle([](auto &) {
                                                                   return
                                                                   seastar::
                                                                   make_ready_future
                                                                   <
              ...
              auto result = crimson::common::do_tmap_up(osd_op.indata.cbegin(),
                                                        std::move(bl));
              if (!result.has_value()) {
              ...
              switch (ret) {
case -EEXIST:
return crimson::ct_error::eexist::make(); case -ENOENT:
return crimson::ct_error::enoent::make(); case -EINVAL:
return crimson::ct_error::invarg::make(); default:
              ceph_assert(0 == "impossible error");
              return crimson::ct_error::invarg::make();}
              }

              logger().
              debug("PGBackend::tmapup: {}, result.value.length()={}, ret=0",
                    os.oi.soid, result.value().length());
              return _writefull(os, result.value().length(), result.value(),
                                txn, osd_op_params, delta_stats, 0);}
    ) ;
}

PGBackend::read_ierrorator::future <> PGBackend::tmapget(const ObjectState & os,
                                                         OSDOp & osd_op,
                                                         object_stat_sum_t &
                                                         delta_stats)
{
    ...
    return _read(oi.soid, 0, oi.size, 0).safe_then_interruptible_tuple([&delta_stats, &osd_op] (auto && bl)->read_errorator::future <> {
                                                                       logger().
                                                                       debug
                                                                       ("PGBackend::tmapget: data length: {}",
                                                                        bl.
                                                                        length
                                                                        ());
                                                                       osd_op.
