...
void JournalingObjectStore::SubmitManager::op_submit_finish(uint64_t op)
{
    ...
    if (op != op_submitted + 1) {
        dout(0) << "op_submit_finish " << op << " expected " << (op_submitted +
                                                                 1)
            << ", OUT OF ORDER" << dendl;
...
bool JournalingObjectStore::ApplyManager::commit_start()
{
...

    {
        ...
        assert(open_ops == 0);
        dout(10) << "commit_start blocked, all open_ops have completed" <<
            dendl;
        {
            Mutex::Locker l(com_lock);
...
void JournalingObjectStore::ApplyManager::commit_finish()
{
    ...
    map < version_t, vector < Context * > >::iterator p =
        commit_waiters.begin();
                                                     ...
                                                     TrackedOpRef osd_op)
{
    if (osd_op.get())
        dout(10) << "op_journal_transactions " << op << " reqid_t "
            << (static_cast <
                OpRequest * >(osd_op.get()))->get_reqid() << dendl;
    else
