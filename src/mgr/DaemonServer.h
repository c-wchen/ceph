...
class DaemonServer:public Dispatcher, public md_config_obs_t {
  protected:
    ...
    static const MonCommand *_get_mgrcommand(const string & cmd_prefix,
                                             ...
                                             &commands);
    bool _allowed_command(MgrSession * s, const string & module,
                          const string & prefix, const map < string,
                          cmd_vartype > &cmdmap, const map < string,
                          string > &param_str_map, const MonCommand * this_cmd);
