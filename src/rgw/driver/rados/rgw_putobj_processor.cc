 ...
 * in the head object.
 */
void read_cloudtier_info_from_attrs(rgw::sal::Attrs &attrs,
...
int HeadObjectProcessor::process(bufferlist
                                 && data, uint64_t logical_offset)
{
...
static int process_completed(const AioResultList &completed,
                             RawObjSet *written)
{
...
int RadosWriter::process(bufferlist && bl, uint64_t offset)
{
    ...
    auto c =
        aio->get(stripe_obj, Aio::librados_op(std::move(op), y), cost, id);
...
RadosWriter::~RadosWriter()
{
    ...
    for (const auto &obj : written) {
        if (raw_head && obj == *raw_head) {
            ldpp_dout(dpp,
                      5) << "NOTE: we should not process the head object ("
                         << obj << ") here" << dendl;
    ...
    if (need_to_remove_head) {
        ...
        ldpp_dout(dpp,
                  5) << "NOTE: we are going to process the head obj (" <<
                     *raw_head << ")" << dendl;
        int r =
            store->delete_obj(dpp, obj_ctx, bucket_info, head_obj, 0, 0);
        if (r < 0 && r != -ENOENT) {
            ldpp_dout(dpp,
...
int AtomicObjectProcessor::prepare(optional_yield y)
{
    ...
    if (!store->
        get_obj_data_pool(bucket_info.placement_rule, head_obj,
                          &head_pool)) {
        return -EIO;
    }

    int r =
        store->get_max_chunk_size(head_pool, &max_head_chunk_size, dpp,
                                  &alignment);
    if (r < 0) {
        return r;
    }
    ...
    if (bucket_info.placement_rule != tail_placement_rule) {
        rgw_pool tail_pool;
        if (!store->
            get_obj_data_pool(tail_placement_rule, head_obj, &tail_pool)) {
            return -EIO;
        }
    ...
    if (same_pool) {
        ...
        if (!store->svc.zone->get_zone_params().
            get_placement(bucket_info.placement_rule.name, &placement_info)
    ...
    const uint64_t default_stripe_size =
        store->ctx()->_conf->rgw_obj_stripe_size;

    store->get_max_aligned_size(default_stripe_size, alignment,
                                &stripe_size);
...
int MultipartObjectProcessor::prepare_head()
{
    ...
    int r =
        store->get_max_chunk_size(tail_placement_rule, target_obj,
                                  &chunk_size, dpp, &alignment);
    if (r < 0) {
        ldpp_dout(dpp,
                  0) <<
                     "ERROR: unexpected: get_max_chunk_size(): placement_rule=" <<
                     tail_placement_rule.
                     to_str() << " obj=" << target_obj << " returned r=" << r <<
                     dendl;
        return r;
    }
    store->get_max_aligned_size(default_stripe_size, alignment,
    ...
    rgw_raw_obj stripe_obj = manifest_gen.get_cur_obj(store);
    RGWSI_Tier_RADOS::raw_obj_to_obj(head_obj.bucket, stripe_obj,
                                     &head_obj);
                                       ...
                                       bool *pcanceled, optional_yield y)
{
    ...
    rgw_obj meta_obj;
    meta_obj.init_ns(bucket_info.bucket, mp.get_meta(),
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  -1) <<
                      "ERROR: failed to get obj ref of meta obj with ret=" << r <<
                      dendl;
        return r;
    }
    ...
    r = rgw_rados_operate(dpp, meta_obj_ref.pool.ioctx(),
                          meta_obj_ref.obj.oid, &op, y);
    ldpp_dout(dpp,
              20) << "Update meta: " << meta_obj_ref.obj.
                  oid << " part " << p << " prefix " << info.manifest.
                  get_prefix() << " return " << r << dendl;
...
int AppendObjectProcessor::process_first_chunk(bufferlist
        && data,
...
int AppendObjectProcessor::prepare(optional_yield y)
{
    ...
    *cur_accounted_size = astate->accounted_size;
    if (!astate->exists) {
        if (position != 0) {
            ldpp_dout(dpp,
                      5) << "ERROR: Append position should be zero" <<
                         dendl;
        ...
        if (position != *cur_accounted_size) {
            ldpp_dout(dpp,
                      5) <<
                         "ERROR: Append position should be equal to the obj size" <<
                         dendl;
            return -ERR_POSITION_NOT_EQUAL_TO_LENGTH;
        }
        try {
    ...
    manifest.set_multipart_part_rule(store->ctx()->_conf->
                                     rgw_obj_stripe_size, cur_part_num);

    r = manifest_gen.create_begin(store->ctx(), &manifest,
                                  bucket_info.placement_rule,
                                  &tail_placement_rule, head_obj.bucket,
                                  head_obj);
    if (r < 0) {
        return r;
    }
...
int AppendObjectProcessor::complete(size_t accounted_size,
                                    const string &etag,
                                    ceph::real_time *mtime,
                                    ceph::real_time set_mtime,
                                    rgw::sal::Attrs &attrs,
                                    ...
                                    bool *pcanceled, optional_yield y)
{
    ...
    r = obj_op.write_meta(dpp, actual_size + cur_size,
                          accounted_size + *cur_accounted_size, attrs, y);
    if (r < 0) {
        return r;
    }
    if (!obj_op.meta.canceled) {
