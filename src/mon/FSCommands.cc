  ...
  public:
    ...
    :FileSystemCommandHandler("fs flag set") {
        ...
        if (flag_name == "enable_multiple") {
            ...
            bool jewel =
                mon->get_quorum_con_features() & CEPH_FEATURE_SERVER_JEWEL;
            if (flag_bool && !jewel) {
                ss <<
    ...
    int handle(Monitor * mon,
      ...
      for (auto fs:fsmap.get_filesystems()) {
        ...
        if (mon->osdmon()->osdmap.require_osd_release >= CEPH_RELEASE_LUMINOUS
            || mon->osdmon()->pending_inc.new_require_osd_release >=
...
class SetHandler:public FileSystemCommandHandler {
  public:
    ...
    :FileSystemCommandHandler("fs set") {
        ...
        string var;
        if (!cmd_getval_throws(g_ceph_context, cmdmap, "var", var)
            || var.empty()) {
            ss << "Invalid variable";
            return -EINVAL;
        }
        ...
        if (var == "max_mds") {
            ...
            if (enable_inline) {
                ...
                if (!cmd_getval_throws
                    (g_ceph_context, cmdmap, "confirm", confirm)
 ...
 * its command map.
 */
template < typename T > class LegacyHandler:public T {
  ...
  :    T(std::forward < Args > (args) ...) {
    ...
    std::string const &get_prefix() override {
    ...
    >FileSystemCommandHandler::load(Paxos * paxos)
{
    ...
    handlers.push_back(std::make_shared < LegacyHandler < SetHandler >
                       >("mds set"));
    ...
    handlers.push_back(std::make_shared < LegacyHandler <
                       RemoveDataPoolHandler > >("mds remove_data_pool"));
    handlers.push_back(std::make_shared < LegacyHandler <
                       RemoveDataPoolHandler > >("mds rm_data_pool"));
...
int FileSystemCommandHandler::parse_bool(const std::string & bool_str,
...
int FileSystemCommandHandler::_check_pool(OSDMap & osd_map,
                                          const int64_t pool_id,
                                          bool metadata,
                                          bool force,
                                          std::stringstream * ss) const const
{
    assert(ss != NULL);
    ...
    if (!force && !pool->application_metadata.empty() &&
        pool->application_metadata.count(pg_pool_t::APPLICATION_NAME_CEPHFS) ==
        0) {
        *ss << " pool '" << pool_name << "' (id '" << pool_id <<
