...
template <> struct fmt::formatter < z_op >:fmt::formatter < std::string_view > {
    template < typename FormatContext > auto format(z_op s, FormatContext & ctx) {
        ...
        switch (s) {
            using enum z_op;
            case OPEN:name = "BLKOPENZONE";
            break;
            case FINISH:name = "BLKFINISHZONE";
            break;
            case CLOSE:name = "BLKCLOSEZONE";
            break;
            case RESET:name = "BLKRESETZONE";
            break;
...
namespace crimson::os::seastore::segment_manager::zns {
    ...
    static open_device_ret open_device(const std::string & path,
        ...
        return seastar::file_stat(path,
                                  seastar::follow_symlink::yes).then([FNAME,
                                                                      mode,
                                                                      &path]
                                                                     (auto stat)
                                                                     mutable {
                                                                     ...
                                                                     then([ =]
                                                                          ...
                                                                          {
                                                                          DEBUG
                                                                          ("open of device {} successful, size {}",
                                                                           path,
                                                                           stat.
                                                                           size);
                                                                          return
    ...
    static zns_sm_metadata_t make_metadata(uint64_t total_size,
        ...
        INFO("device size {}, available_size {}, block_size {}, allocated_size {}," " total zones {}, zone_size {}, zone_capacity {}," " total segments {}, zones per segment {}, segment size {}", total_size, available_size, data.block_size, data.allocated_size, num_zones, zone_size, zone_capacity, segments, zones_per_segment, zone_capacity * zones_per_segment);
    ...
    struct ZoneReport {
        ...
        :hdr((blk_zone_report *)
             malloc(sizeof(struct blk_zone_report) +
    ...
    static seastar::future < size_t > get_blk_dev_size(seastar::file & device) {
        return seastar::do_with((uint64_t) 0,[&](auto & size_sects) {
                                return device.ioctl(BLKGETSIZE,
                                                    (void *)&size_sects).
                                then([&](int ret) {
...
// zone_size should be in 512B sectors
    static seastar::future <> reset_device(seastar::file & device,
        ...
        return seastar::do_with(blk_zone_range {
                                ...
                                return device.ioctl(BLKRESETZONE,
                                                    &range).then([&](int ret) {
                                                                 return
                                                                 seastar::
                                                                 now();}
    ...
    static seastar::future < size_t > get_zone_capacity(seastar::file & device,
                                                        uint32_t nr_zones) {
        return seastar::do_with(ZoneReport(nr_zones),[&](auto & zr) {
                                zr.hdr->sector = 0;
                                zr.hdr->nr_zones = nr_zones;
                                return device.ioctl(BLKREPORTZONE,
                                                    zr.hdr).then([&](int ret) {
                                                                 return
                                                                 seastar::
                                                                 make_ready_future
    ...
    static write_ertr::future <> do_write(seastar::file & device,
    ...
    static write_ertr::future <> do_writev(seastar::file & device,
                                           uint64_t offset,
                                           bufferlist && bl,
                                           size_t block_size) {
        LOG_PREFIX(ZNSSegmentManager::do_writev);
        DEBUG("offset {} len {}", offset, bl.length());
        ...
        bl.prepare_iov(&iov);
        return device.dma_write(offset, std::move(iov)
            ).handle_exception([FNAME] (auto e)->write_ertr::future < size_t > {
                               ERROR("dma_write got error {}",
                                     e);
                               return crimson::ct_error::input_output_error::
                ...
                std::
                move(bl) /* hold the buf until the end of io */ ](size_t
                                                                  written)
               ->write_ertr::future <> {
    ...
    static ZNSSegmentManager::access_ertr::future <>
        ...
        return seastar::
            do_with(bufferptr(ceph::buffer::create_page_aligned(sb.block_size)),
                    ...
                    DEBUG("block_size {}", sb.block_size); bufferlist bl;
                    encode(sb, bl); auto iter = bl.begin();
                    assert(bl.length() < sb.block_size);
                    DEBUG("buffer length {}", bl.length());
                    iter.copy(bl.length(), bp.c_str()); DEBUG("doing writeout");
                    return do_write(device, 0, bp);}
        );
    }

    static read_ertr::future <> do_read(seastar::file & device,
                                        uint64_t offset,
                                        size_t len, bufferptr & bptr) {
        LOG_PREFIX(ZNSSegmentManager::do_read);
        assert(len <= bptr.length());
        DEBUG("offset {} len {}", offset, len);
        return device.dma_read(offset,
                               bptr.c_str(),
                               len).handle_exception([FNAME] (auto e)->
                                                     read_ertr::future <
                                                     size_t > {
                                                     ERROR
                                                     ("dma_read got error {}",
     ...
     ZNSSegmentManager::access_ertr::future < zns_sm_metadata_t >
        ...
        return seastar::
            do_with(bufferptr(ceph::buffer::create_page_aligned(sd.block_size)),
                    [ =, &device] (auto & bp) {
                    return do_read(device, 0, bp.length(),
                                   bp).safe_then([ =, &bp] {
                                                 bufferlist bl;
                                                 bl.push_back(bp);
    ...
    ZNSSegmentManager::mount_ret ZNSSegmentManager::shard_mount() {
        return open_device(device_path,
                           seastar::open_flags::rw).safe_then([ =,
                                                               this] (auto p) {
                                                              device =
                                                              std::move(p.
                                                                        first);
                    ...
                    metadata = meta; return mount_ertr::now();}
        );
    }

    ZNSSegmentManager::mkfs_ret ZNSSegmentManager::mkfs(device_config_t config) {
    ...
    ZNSSegmentManager::mkfs_ret ZNSSegmentManager::
        primary_mkfs(device_config_t config) {
        ...
        return seastar::do_with(seastar::file {
                                ...
                                [ =, this] (auto & device, auto & stat,
                                            auto & sb, auto & zone_size_sects,
                                            auto & nr_zones, auto & size) {
                                return open_device(device_path,
                                                   seastar::open_flags::rw).
                                safe_then([ =, this, &device, &stat, &sb,
                                           &zone_size_sects, &nr_zones,
                                           &size] (auto p) {
                                          device = p.first; stat = p.second;
                                          return device.ioctl(BLKGETNRZONES,
                                                              (void *)
                                                              &nr_zones).
                                          then([&](int ret) {
                                               if (nr_zones == 0) {
                                          ...
                                          then([&](int ret) {
                                               ...
                                               return reset_device(device,
                                                 ...
                                                 return
                                                 get_blk_dev_size(device);}
                                          ).then([&](auto devsize) {
                                                 size = devsize;
                                                 return
                                                 get_zone_capacity(device,
                                                                   nr_zones);}
                                          ).
                                          then([&, FNAME,
                                                config] (auto
                                               ...
                                               ("zone_size in sectors {}, zone_capacity in sectors {}",
                                                ...
                                                zone_capacity_sects);
                                               sb =
                                               make_metadata(size, config.meta,
                                                             stat,
                                                             zone_size_sects,
                                                             zone_capacity_sects,
                                                             nr_zones);
    ...
    ZNSSegmentManager::mkfs_ret ZNSSegmentManager::shard_mkfs() {
        ...
        return open_device(device_path,
                    ...
                    shard_info = meta.shard_infos[seastar::this_shard_id()];
                    metadata = meta; return device.close();}
        ).safe_then([FNAME] {
...
// Return range of sectors to operate on.
    struct blk_zone_range make_range(segment_id_t id,
                                     size_t segment_size,
    ...
    blk_zone_op_ret blk_zone_op(seastar::file & device,
                                blk_zone_range & range, zone_op op) {
        ...
        return device.ioctl(ioctl_op,
                            &range).then_wrapped([ =] (auto f)->
                                                 blk_zone_op_ret {
                                                 if (f.failed()) {
                                                 ERROR("{} ioctl failed", op);
                                                 return crimson::ct_error::
                                                 input_output_error::make();}
                                                 else {
                                                 int ret = f.get();
                                                 if (ret == 0) {
                                                 return seastar::now();}
    ...
    ZNSSegmentManager::open_ertr::future < SegmentRef >
        ...
        return seastar::do_with(blk_zone_range {
                                ...
                                return blk_zone_op(device, range,
                                                   zone_op::OPEN);}
        ).safe_then([ =, this] {
    ...
    ZNSSegmentManager::release_ertr::future <> ZNSSegmentManager::
    ...
    SegmentManager::read_ertr::future <> ZNSSegmentManager::read(paddr_t addr,
                                                                 size_t len,
                                                                 ceph::
                                                                 bufferptr &
                                                                 out) {
    ...
    Segment::close_ertr::future <> ZNSSegmentManager::
    ...
    Segment::write_ertr::future <> ZNSSegmentManager::
        ...
        DEBUG("write to segment {} at offset {}, physical offset {}, len {}",
              ...
              seg_addr.get_segment_off(), get_offset(addr), bl.length());
        stats.data_write.increment(bl.length());
        return do_writev(device,
                         get_offset(addr), std::move(bl), metadata.block_size);
    }
    ...
    Segment::write_ertr::future <> ZNSSegment::write(segment_off_t offset,
                                                     ceph::bufferlist bl) {
        LOG_PREFIX(ZNSSegment::write);
        if (offset != write_pointer
            || offset % manager.metadata.block_size != 0) {
            ERROR("Segment offset and zone write pointer mismatch. "
                  "segment {} segment-offset {} write pointer {}", id, offset,
                  write_pointer);
            return crimson::ct_error::invarg::make();
        }
    ...
    Segment::write_ertr::future <> ZNSSegment::
        ...
        return crimson::repeat([FNAME, padding_bytes, this] ()mutable {
                               ...
                               bufferptr bp(ceph::buffer::
                                            create_page_aligned(bufsize));
                               bp.zero(); bufferlist padd_bl;
                               padd_bl.append(bp);
                               return write(write_pointer,
                                            padd_bl).safe_then([FNAME,
                                                                padding_bytes,
                                                                this] () {
                                                               if (padding_bytes
                                                                   == 0) {
...
// Advance write pointer, to given offset.
    Segment::write_ertr::future <> ZNSSegment::advance_wp(segment_off_t offset) {
