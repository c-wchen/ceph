...
auth_cluster_required(cct,
                      ...
                      auth_supported), auth_service_required(cct,
                                                             ...
                                                             : cct->_conf->
                                                             auth_supported),
mgr_messenger(mgr_m), mgr_client(cct_, mgr_m), pgservice(nullptr), store(s),
state(STATE_PROBING), elector(this), required_features(0), leader(0),
...
admin_hook(NULL), routed_request_tid(0), op_tracker(cct, true, 1)
{
                               ...
                               ostream & ss)
{
    ...
    string args;
    for (cmdmap_t::iterator p = cmdmap.begin(); p != cmdmap.end(); ++p) {
        if (p->first == "prefix")
            continue;
        if (!args.empty())
            args += ", ";
...
void Monitor::update_log_clients()
{
    ...
    clog->update_config(log_to_monitors, log_to_syslog,
                        log_channel, log_prio, log_to_graylog,
                        log_to_graylog_host, log_to_graylog_port, fsid, host);
...
int Monitor::preinit()
{
    ...
    assert(!cluster_logger);
    {
        ...
        pcb.add_u64(l_cluster_osd_bytes, "osd_bytes",
                    "Total capacity of cluster", NULL, 0, unit_t(BYTES));
        ...
        pcb.add_u64(l_cluster_num_mds_in, "num_mds_in",
                    "MDS in state \"in\" (they are in cluster)");
    ...
    r = admin_socket->register_command("sessions",
                                       "sessions",
                                       admin_hook, "list existing sessions");
    assert(r == 0);
            ...
            messenger->
                send_message(new
                             MMonProbe(monmap->fsid, MMonProbe::OP_PROBE, name,
                                       has_ever_joined), monmap->get_inst(i));
    }
...
bool Monitor::_add_bootstrap_peer_hint(string cmd, cmdmap_t & cmdmap,
                                       ostream & ss)
{
    string addrstr;
    ...
    if (!addr.parse(addrstr.c_str(), &end)) {
        ss << "failed to parse addr '" << addrstr <<
...
set < string > Monitor::get_sync_targets_names()
{
    ...
    ConfigKeyService *config_key_service_ptr =
        dynamic_cast < ConfigKeyService * >(config_key_service);
...
void Monitor::sync_start(entity_inst_t & other, bool full)
{
    ...
    assert(state == STATE_PROBING || state == STATE_SYNCHRONIZING);
    ...
    if (sync_full) {
        ...
        sync_last_committed_floor =
            MAX(sync_last_committed_floor, paxos->get_version());
        dout(10) << __func__ <<
...
void Monitor::sync_trim_providers()
{
    ...
    while (p != sync_providers.end()) {
        if (now > p->second.timeout) {
            dout(10) << __func__ << " expiring cookie " << p->second.
                cookie << " for " << p->second.entity << dendl;
...
void Monitor::handle_probe_reply(MonOpRequestRef op)
{
    ...
    if (monmap->get_epoch() == 0 &&
        monmap->contains(m->name) && monmap->get_addr(m->name).is_blank_ip()) {
        dout(1) << " learned initial mon " << m->name << " addr " << m->
    ...
    if (m->quorum.size()) {
        ...
        if (monmap->contains(name) && !monmap->get_addr(name).is_blank_ip()) {
            ...
            dout(10) <<
                " ready to join, but i'm not in the monmap or my addr is blank, trying to join"
                << dendl;
            messenger->
                send_message(new
                             MMonJoin(monmap->fsid, name,
                                      messenger->get_myaddr()),
...
void Monitor::win_standalone_election()
{
    ...
    win_election(elector.get_epoch(), q,
                 CEPH_FEATURES_ALL,
                 ceph::features::mon::get_supported(), metadata);
}
                           ...
                           const map < int, Metadata > &metadata)
{
    dout(10) << __func__ << " epoch " << epoch << " quorum " << active
        << " features " << features
        << " mon_features " << mon_features << dendl;
    assert(is_electing());
    state = STATE_LEADER;
                            ...
                            const mon_feature_t & mon_features)
{
    ...
    dout(10) << "lose_election, epoch " << epoch << " leader is mon" << leader
        ...
        << " mon_features are " << quorum_mon_features << dendl;
...
void Monitor::finish_election()
{
    ...
    if (cur_name != name) {
        dout(10) << " renaming myself from " << cur_name << " -> " << name <<
            dendl;
        messenger->
            send_message(new
                         MMonJoin(monmap->fsid, name, messenger->get_myaddr()),
...
void Monitor::apply_monmap_to_compatset_features()
{
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_KRAKEN)) {
        assert(ceph::features::mon::get_persistent().
...
void Monitor::_quorum_status(Formatter * f, ostream & ss)
{
    ...
    f->dump_string("quorum_leader_name",
                   quorum.empty()? string() : monmap->get_name(*quorum.
                                                               begin()));
...
health_status_t Monitor::get_health_status(bool want_detail,
                                           Formatter * f,
                                           std::string * plain,
                                           const char *sep1, const char *sep2)
{
    ...
    bool compat_warn =
        g_conf->get_val < bool > ("mon_health_preluminous_compat_warning");
    if (f) {
  ...
  for (auto & svc:paxos_service) {
        r = std::min(r,
                     svc->get_health_checks().dump_summary(f, psummary, sep2,
                                                           want_detail));
    }
...
health_status_t Monitor::get_health(list < string > &status,
                                    bufferlist * detailbl, Formatter * f)
{
    ...
    if (!timecheck_skews.empty()) {
        list < string > warns;
        for (map < entity_inst_t, double >::iterator i =
             timecheck_skews.begin(); i != timecheck_skews.end(); ++i) {
        ...
        if (!warns.empty()) {
            ...
            summary.
                push_back(make_pair
                          (HEALTH_WARN, "Monitor clock skew detected "));
        }
    }
...
void Monitor::get_cluster_status(stringstream & ss, Formatter * f)
{
    ...
    if (f) {
        ...
        if (osdmon()->osdmap.require_osd_release >= CEPH_RELEASE_LUMINOUS) {
            ...
            health = joinify(ls.begin(), ls.end(), string("\n            "));
        }
        ss << "    health: " << health << "\n";
...
bool Monitor::is_keyring_required()
{
    ...
    return auth_service_required == "cephx" || auth_cluster_required == "cephx";
}
...
void Monitor::handle_command(MonOpRequestRef op)
{
    ...
    MonSession *session =
        static_cast < MonSession * >(m->get_connection()->get_priv());
    if (!session) {
    ...
    if (prefix == "get_command_descriptions") {
        ...
        if (quorum_mon_features.
            contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
            commands =
                static_cast <
                MgrMonitor * >(paxos_service[PAXOS_MGR])->get_command_descs();
        }
    ...
    if (prefix == "compact" || prefix == "mon compact") {
        ...
        dout(1) << "finished manual compaction in " << end << " seconds" <<
            dendl;
        ostringstream oss;
        oss << "compacted " << g_conf->get_val < std::string >
            ("mon_keyvaluedb") << " in " << end << " seconds";
        rs = oss.str();
        r = 0;
    }
        ...
        if (!injected_args.empty()) {
            dout(0) << "parsing injected options '" << injected_args << "'" <<
                dendl;
            ostringstream oss;
        ...
        if (prefix == "status") {
            ...
            if (osdmon()->osdmap.require_osd_release >= CEPH_RELEASE_LUMINOUS) {
                string plain;
                get_health_status(detail == "detail", f.get(),
                                  f ? nullptr : &plain);
                if (f) {
                    f->flush(rdata);
                }
                ...
                list < string > health_str;
                get_health(health_str, detail == "detail" ? &rdata : NULL,
                           f.get());
                if (f) {
                    f->flush(ds);
                    ds << '\n';
                    ...
                    if (!health_str.empty()) {
                        ds << ' ';
                        ds << joinify(health_str.begin(), health_str.end(),
                                      string("; "));
                    }
                }
                bufferlist comb;
    ...
    else if (prefix == "sync force" || prefix == "mon sync force") {
...
void Monitor::forward_request_leader(MonOpRequestRef op)
{
    ...
    if (req->get_source().is_mon()
        && req->get_source_addr() != messenger->get_myaddr()) {
    ...
    if (m->session_mon_tid) {
        if (routed_requests.count(m->session_mon_tid)) {
            ...
            if (m->send_osdmap_first) {
                dout(10) << " sending osdmaps from " << m->
                    send_osdmap_first << dendl;
                osdmon()->send_incremental(m->send_osdmap_first, rr->session,
                                           true, MonOpRequestRef());
            }
            assert(rr->tid == m->session_mon_tid
                   && rr->session->routed_request_tids.count(m->
                                                             session_mon_tid));
...
void Monitor::timecheck_reset_event()
{
    ...
    timecheck_event = timer.add_event_after(delay,
...
void Monitor::timecheck_report()
{
    ...
    for (set < int >::iterator q = quorum.begin(); q != quorum.end(); ++q) {
        ...
        for (map < entity_inst_t, double >::iterator it =
             timecheck_skews.begin(); it != timecheck_skews.end(); ++it) {
    ...
    for (map < string, ceph_mon_subscribe_item >::iterator p = m->what.begin();
        ...
        if (p->first.compare(0, 6, "mdsmap") == 0
            ...
            if ((int)s->is_capable("osd", MON_CAP_W)) {
                if (monmap->get_required_features().
                    contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
                    osdmon()->check_pg_creates_sub(s->
                                                   sub_map["osd_pg_creates"]);
                }
                else {
...
void Monitor::scrub_event_start()
{
    ...
    scrub_event = timer.add_event_after(cct->_conf->mon_scrub_interval,
                                        new C_MonContext(this,[this] (int) {
                                                         scrub_start();}));
}
...
{
    timer.add_event_after(g_conf->mon_tick_interval,
                          new C_MonContext(this,[this] (int) {
                                           tick();}));
}
                                ...
                                bool force_new)
{
    ...
    if (service_id == CEPH_ENTITY_TYPE_MON) {
        ...
        ret =
            key_server.build_session_auth_info(service_id, auth_ticket_info,
                                               info, secret, (uint64_t) - 1);
        if (ret < 0) {
    ...
    if (peer_type == CEPH_ENTITY_TYPE_MON &&
        ...
        if (protocol == CEPH_AUTH_CEPHX) {
            ...
            if (authorizer_data.length()) {
                bool ret =
                    cephx_verify_authorizer(g_ceph_context, &keyring, iter,
                                            auth_ticket_info, challenge,
                                            authorizer_reply);
                if (ret) {
                    ...
                    dout(0) << "ms_verify_authorizer bad authorizer from mon "
            ...
            dout(0) <<
                "ms_verify_authorizer cephx enabled, but no authorizer (required for mon)"
                << dendl;
        }
    }
    else {
