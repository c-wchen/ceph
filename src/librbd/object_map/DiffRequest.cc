...
template < typename I > void DiffRequest < I >::send()
{
    ...
    if (m_snap_id_start == CEPH_NOSNAP
        || m_snap_id_start > m_snap_id_end) {
        lderr(cct) << "invalid start/end snap ids: " << "snap_id_start="
                   << m_snap_id_start << ", " << "snap_id_end=" <<
                   m_snap_id_end << dendl;
        finish(-EINVAL);
        return;
    ...
    auto snap_info_it =
        m_image_ctx->snap_info.upper_bound(m_snap_id_start);
    auto snap_info_it_end =
        m_image_ctx->snap_info.lower_bound(m_snap_id_end);
...
template < typename I >
void DiffRequest < I >::load_object_map(std::shared_lock <
                                        ceph::shared_mutex >
                                        *image_locker)
{
    ...
    if (r < 0) {
        ...
        lderr(cct) << "failed to retrieve image flags: " <<
                   cpp_strerror(r)
                   << dendl;
        finish(r);
        return;
    ...
    if ((flags & RBD_FLAG_FAST_DIFF_INVALID) != 0) {
        ldout(cct,
              1) << "cannot perform fast diff on invalid object map" <<
                 dendl;
        finish(-EINVAL);
        return;
    }
    ...
    auto aio_comp = create_rados_callback <
                    DiffRequest < I >,
                    &DiffRequest < I >::handle_load_object_map > (this);
...
void DiffRequest < I >::handle_load_object_map(int r)
{
    ...
    if (r == -ENOENT && m_ignore_enoent) {
        ldout(cct,
              10) << "object map " << oid << " does not exist" << dendl;
    ...
    uint64_t overlap =
        std::min(m_object_map.size(), prev_object_diff_state_size);
