    ...
    while (left > 0)
    {
        ...
        if (p == data.end())
        {
            ...
            if (complete)
            {
                ...
                ldout(oc->cct,
                      20)
                    << "map_read miss+complete+zero " << left << " left, "
                    << *n << dendl;
            }
            else
            {
                ...
                ldout(oc->cct,
                      20)
                    << "map_read miss " << left << " left, " << *n << dendl;
            }
            cur += left;
...
void ObjectCacher::Object::audit_buffers()
{
         ...
         it != data.end(); ++it)
    {
        ...
        if (it->first < offset)
        {
            lderr(oc->cct) << "AUDIT FAILURE: " << it->first << " " << *it->second << " overlaps with previous bh " << *((--it)->second)
                           << dendl;
    ...
    while (left > 0)
    {
        ...
        if (p == data.end())
        {
            if (final == NULL)
            {
                ...
                ldout(oc->cct,
                      10)
                    << "map_write adding trailing bh " << *final << dendl;
            }
            else
            {
...
ObjectCacher::~ObjectCacher()
{
    ...
    perf_stop();
    // we should be empty.
    for (vector<ceph::unordered_map<sobject_t, Object *>>::iterator i = objects.begin(); i != objects.end(); ++i)
        assert(i->empty());
    ...
    else
    {
        ...
        while (true)
        {
            ...
            if (opos >= start + (loff_t)length)
            {
                ldout(cct,
                      20)
                    << "break due to opos " << opos << " >= start+length "
                    << start << "+" << length << "=" << start + (loff_t)length << dendl;
                break;
            }
            ...
            // finishers?
            for (map<loff_t, list<Context *>>::iterator it = bh->waitfor_read.begin(); it != bh->waitfor_read.end(); ++it)
                                   ...
                                   ceph_tid_t tid, int r)
{
         ...
         p != ranges.end(); ++p)
    {
        ...
        if (!ob->exists)
        {
                                                    ...
                                                    ob->get_snap()))
            {
                ldout(cct,
                      10)
                    << "bh_write_commit may copy on write, clearing "
                       "complete on "
                    << *ob << dendl;
                ob->complete = false;
            }
             ...
             p != ob->data.end(); ++p)
        {
            ...
            if (!bh->is_tx())
            {
                ldout(cct,
                      10)
                    << "bh_write_commit skipping non-tx " << *bh << dendl;
                continue;
            }
    ...
    if (flush_set_callback && was_dirty_or_tx > 0 && oset->dirty_or_tx == 0)
    { // nothing dirty/tx
                         ...
                         bool external_call, ZTracer::Trace *trace)
{
        ...
        if (oset->return_enoent && !o->exists)
        {
                                                    ...
                                                    ex_it->length, soid.snap))
            {
                ...
                if (wait)
                {
                    ldout(cct,
                          10)
                        << "readx  waiting on tid " << o->last_write_tid << " on " << *o << dendl;
        ...
        if (!missing.empty() || !rx.empty())
        {
                 ...
                 bh_it != rx.end(); ++bh_it)
            {
                ...
                if (success && onfinish)
                {
                    ldout(cct,
                          10)
                        << "readx missed, waiting on " << *bh_it->second << " off " << bh_it->first << dendl;
        ...
        else
        {
            ...
            if (!error)
            {
                ...
                while (1)
                {
                    ...
                    ldout(cct,
                          10)
                        << "readx rmap opos " << opos << ": " << *bh << " +" << bhoff << " frag " << f_it->first << "~" << f_it->second << " +" << foff << "~" << len << dendl;
    ...
    if (!success)
    {
        ...
        else
        {
            ldout(cct,
                  20)
                << "readx drop " << rd << " (no complete, but no waiter)"
                << dendl;
            delete rd;
        }
        return 0; // wait!
...
void ObjectCacher::verify_stats() const const
{
    ...
    for (vector<ceph::unordered_map<sobject_t,
                                    Object *>>::const_iterator i = objects.begin();
         i != objects.end();
         ++i)
    {
