...
namespace cephfs {
    namespace mirror {
        ...
        ClusterWatcher::ClusterWatcher(CephContext * cct, MonClient * monc,
                                       ServiceDaemon * service_daemon,
                                       Listener & listener)
        :Dispatcher(cct), m_monc(monc), m_service_daemon(service_daemon),
            m_listener(listener) {
        } ClusterWatcher::~ClusterWatcher() {
        } bool ClusterWatcher::ms_can_fast_dispatch2(const cref_t < Message >
                                                     &m)const {
        ...
        bool ClusterWatcher::ms_dispatch2(const ref_t < Message > &m) {
            if (m->get_type() == CEPH_MSG_FS_MAP) {
                if (m->get_connection()->get_peer_type() ==
        ...
        void ClusterWatcher::handle_fsmap(const cref_t < MFSMap > &m) {
            ...
            std::map < Filesystem, uint64_t > fs_metadata_pools;
            {
                ...
                for (auto it = m_filesystem_peers.begin();
                     it != m_filesystem_peers.end();) {
              ...
              for (auto & filesystem:filesystems) {
                    ...
                    if (!mirror_info.is_mirrored()) {
                        ...
                        auto[fspeersit, enabled] =
                            m_filesystem_peers.emplace(fs, Peers {
                                                       }
                        );
          ...
          for (auto & fs:mirroring_enabled) {
                ...
                m_listener.
                    handle_mirroring_enabled(FilesystemSpec
                                             (fs, fs_metadata_pools.at(fs)));
            }
          for (auto & fs:mirroring_disabled) {
            ...
            dout(5) << ": peers added=" << peers_added << ", peers removed=" <<
                peers_removed << dendl;
