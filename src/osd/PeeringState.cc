                   ...
                   PeeringCtxWrapper &ctx)
{
    ...
    if (found_missing && oinfo.last_update != eversion_t()) {
        ...
        ctx.send_info(from.osd, spg_t(info.pgid.pgid, from.shard), get_osdmap_epoch(), // fixme: use lower epoch?
                      get_osdmap_epoch
                      (),
                      tinfo);
    }
        ...
        for (auto i = acting_recovery_backfill.begin();
            ...
            if (pi.last_update == info.last_update) {
                ...
                if (!pi.is_empty()) {
                    ...
                    ctx.send_info(peer.osd, spg_t(info.pgid.pgid, peer.shard), get_osdmap_epoch(),    // fixme: use lower epoch?
                                  get_osdmap_epoch
                                  (),
                                  info,
                                  get_lease
                              ...
                              last_pg_merge_meta)
{
    ...
    PGLog::LogEntryHandlerRef handler {
        pl->get_log_handler(rctx.transaction)};
...
void PeeringState::update_calc_stats()
{
    ...
    if ((is_remapped() || is_undersized() || !is_clean())
        ...
        if (pool.info.is_replicated()) {
            ...
            unsigned needed =
                target - missing_target_objects.size();
            if (needed)
                               ...
                               Transaction &t)
{
    ...
    } else if (pg_log.get_log().complete_to !=
...
template < typename T >
set < pg_shard_t > unique_osd_shard_set(const pg_shard_t &skip,
                                        const T &in)
{
    set < int >osds_found;
    set < pg_shard_t > out;
