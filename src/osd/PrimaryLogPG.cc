            ...
            ObjectContextRef obc)
{
    ...
    mop->new_manifest.calc_refs_to_inc_on_set(obc_l ? & (obc_l->obs.oi.manifest) : nullptr,
...
boost::intrusive_ptr <
PrimaryLogPG::RepGather > PrimaryLogPG::new_repop(eversion_t version,
        int r,
        ObcLockManager
        && manager,
        OpRequestRef
...
populate_obc_watchers(ObjectContextRef obc)
{
    ...
    ceph_assert((recovering.count(obc->obs.oi.soid) || !is_missing_object(obc->obs.oi.soid)) ||
        ...
        if (is_primary()) {
            if (is_degraded_or_backfilling_object(soid)) {
                dout(20) << __func__ <<
                         ...
                         << dendl;
                return -EAGAIN;
            } else if (is_degraded_on_async_recovery_target
                       (soid)) {
                dout(20) << __func__ <<
                           ...
                           work_started)
{
    ...
    ceph_assert(!recovering.count(soid));
    recovering.insert(make_pair(soid, obc));
    int r = pgbackend->recover_object(soid, v, ObjectContextRef(), obc, // has snapset context
                                      h);
    if (r < 0) {
                 ...
                 bool *work_started)
{
    ...
    replicas_by_num_missing.insert(replicas_by_num_missing.end(), async_by_num_missing.begin(), async_by_num_missing.end());
                              ...
                              agent_flush_quota)
{
    ...
    for (vector < hobject_t >::iterator p =
        ...
        if (agent_state->evict_mode !=
            ...
            ++ started;
        } else if (agent_state->flush_mode !=
