...
class ActivePyModule:public PyModuleRunner {
  ...
  public:
    ...
    :PyModuleRunner(py_module_, clog_),
        fin_thread_name(std::string("m-fin-" + py_module->get_name()).
                        substr(0, 15)), finisher(g_ceph_context, thread_name,
                                                 fin_thread_name)
    {
    ...
    PyObject *dispatch_remote(const std::string & method,
                              PyObject * args,
    ...
    } bool is_authorized(const std::map < std::string,
