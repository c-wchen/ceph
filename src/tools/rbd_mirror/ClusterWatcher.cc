...
void ClusterWatcher::read_pool_peers(PoolPeers *pool_peers)
{
    ...
    if (r < 0) {
        derr << "error waiting for OSD map: " << cpp_strerror(r) <<
             dendl;
        return;
    }
    ...
    for (auto &kv : pools) {
        ...
        if (r == -ENOENT) {
            dout(10) << "pool " << pool_name << " no longer exists" <<
                     dendl;
            continue;
        } else if (r < 0) {
            derr << "Error retrieving base tier for pool " << pool_name
                 << dendl;
            continue;
        }
        ...
        if (r == -ENOENT) {
            dout(10) << "pool " << pool_id << " no longer exists" <<
                     dendl;
            continue;
        } else if (r < 0) {
            derr << "Error accessing pool " << pool_name <<
                 cpp_strerror(r) << dendl;
            continue;
        }

        cls::rbd::MirrorMode mirror_mode_internal;
        r = librbd::cls_client::mirror_mode_get(&ioctx,
                                                &mirror_mode_internal);
        if (r == 0
            && mirror_mode_internal == cls::rbd::MIRROR_MODE_DISABLED) {
            dout(10) << "mirroring is disabled for pool " << pool_name
                     << dendl;
            continue;
        }
        ...
        if (r == -EPERM) {
            dout(10) << "access denied querying pool " << pool_name <<
                     dendl;
            m_service_pools[pool_id] =
                m_service_daemon->add_or_update_callout(pool_id,
                    m_service_pools
                    [pool_id],
                    service_daemon::
        ...
        if (r < 0) {
            derr << "error reading mirroring config for pool " <<
                 pool_name << cpp_strerror(r) << dendl;
            m_service_pools[pool_id] =
                m_service_daemon->add_or_update_callout(pool_id,
                    m_service_pools
        ...
        for (auto &peer : peers) {
            r = resolve_peer_site_config_keys(pool_id, pool_name,
                                              &peer);
            if (r < 0) {
                break;
            }
        ...
        if (m_service_pools[pool_id] != service_daemon::CALLOUT_ID_NONE) {
            m_service_daemon->remove_callout(pool_id,
                                             m_service_pools[pool_id]);
            m_service_pools[pool_id] = service_daemon::CALLOUT_ID_NONE;
        }

        pool_peers->emplace(pool_id, Peers {
            peers.begin(), peers.end()}
                           );
    }

    for (auto it = m_service_pools.begin();
         it != m_service_pools.end();) {
        auto current_it(it++);
        if (service_pool_ids.find(current_it->first) ==
            service_pool_ids.end()) {
        ...
        PeerSpec *peer)
{
    dout(10) << "retrieving config-key: pool_id=" << pool_id << ", " <<
             "pool_name=" << pool_name << ", " << "peer_uuid=" << peer->
             uuid << dendl;
    ...
    if (r == -ENOENT || out_bl.length() == 0) {
        ...
        derr << "error reading mirroring peer config for pool " <<
             pool_name << ": " << cpp_strerror(r) << dendl;
        m_service_pools[pool_id] =
            m_service_daemon->add_or_update_callout(pool_id,
                m_service_pools
    ...
    if (!json_valid) {
        derr << "error parsing mirroring peer config for pool " <<
             pool_name << ", " << "peer " << peer->uuid << dendl;
        m_service_pools[pool_id] =
            m_service_daemon->add_or_update_callout(pool_id,
                m_service_pools
                [pool_id],
