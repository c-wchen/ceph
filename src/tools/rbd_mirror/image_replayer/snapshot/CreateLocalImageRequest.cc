...
void CreateLocalImageRequest < I >::disable_mirror_image()
{
    ...
    librbd::cls_client::mirror_image_set(&op,
                                         m_state_builder->
                                         local_image_id,
                                         mirror_image);
    ...
    auto aio_comp = create_rados_callback <
                    ...
                    I >::handle_disable_mirror_image > (this);
    int r =
        m_local_io_ctx.aio_operate(RBD_MIRRORING, aio_comp,
                                   &op);
    ceph_assert(r == 0);
    aio_comp->release();
} template < typename I >
void CreateLocalImageRequest <
I >::handle_disable_mirror_image(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "failed to disable mirror image " <<
...
void CreateLocalImageRequest < I >::remove_mirror_image()
{
    ...
    librados::ObjectWriteOperation op;
    librbd::cls_client::mirror_image_remove(&op,
                                            m_state_builder->
                                            local_image_id);
    ...
    auto aio_comp = create_rados_callback <
                    CreateLocalImageRequest < I >,
                    &CreateLocalImageRequest <
                    I >::handle_remove_mirror_image > (this);
    int r =
        m_local_io_ctx.aio_operate(RBD_MIRRORING, aio_comp,
                                   &op);
    ceph_assert(r == 0);
...
template < typename I >
void CreateLocalImageRequest <
I >::handle_remove_mirror_image(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "failed to remove mirror image " <<
             m_global_image_id << ": " << cpp_strerror(r) <<
             dendl;
        finish(r);
        return;
...
void CreateLocalImageRequest < I >::add_mirror_image()
{
    ...
    dout(10) << "local_image_id=" << m_state_builder->
             local_image_id << dendl;
    ...
    auto aio_comp = create_rados_callback <
                    CreateLocalImageRequest < I >,
                    &CreateLocalImageRequest <
                    I >::handle_add_mirror_image > (this);
    int r =
        m_local_io_ctx.aio_operate(RBD_MIRRORING, aio_comp,
                                   &op);
    ceph_assert(r == 0);
...
template < typename I >
void CreateLocalImageRequest <
I >::handle_add_mirror_image(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "failed to register mirror image " <<
             m_global_image_id << ": " << cpp_strerror(r) <<
             dendl;
        this->finish(r);
        return;
...
void CreateLocalImageRequest < I >::create_local_image()
{
    ...
    auto ctx = create_context_callback <
               CreateLocalImageRequest < I >,
               &CreateLocalImageRequest <
               I >::handle_create_local_image > (this);
    auto request =
        CreateImageRequest < I >::create(m_threads,
                                         m_local_io_ctx,
                                         m_global_image_id,
                                         m_state_builder->
                                         remote_mirror_uuid,
...
template < typename I >
void CreateLocalImageRequest <
I >::handle_create_local_image(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r == -EBADF) {
        dout(5) << "image id " << m_state_builder->
                local_image_id << " " << "already in-use" << dendl;
        disable_mirror_image();
        return;
    } else if (r < 0) {
        if (r == -ENOENT) {
