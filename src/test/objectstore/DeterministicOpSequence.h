...
class DeterministicOpSequence:public TestObjectStoreState {
  public:
    DeterministicOpSequence(ObjectStore * store, std::string status =
                            std::string());
  ...
  protected:
    ...
    virtual void _do_clone(coll_entry_t * entry, hobject_t & orig_obj,
                           hobject_t & new_obj);
    virtual void _do_clone_range(coll_entry_t * entry, hobject_t & orig_obj,
                                 hobject_t & new_obj, uint64_t srcoff,
                                 uint64_t srclen, uint64_t dstoff);
    virtual void _do_write_and_clone_range(coll_entry_t * entry,
                                           hobject_t & orig_obj,
                                           hobject_t & new_obj, uint64_t srcoff,
                                           uint64_t srclen, uint64_t dstoff,
                                           bufferlist & bl);
    virtual void _do_coll_move(coll_entry_t * entry, hobject_t & orig_obj,
                               hobject_t & new_obj);
    virtual void _do_coll_create(coll_entry_t * entry, uint32_t pg_num,
                                 uint64_t num_objs);
  ...
  private:
    bool _prepare_clone(rngen_t & gen,
                        coll_entry_t ** entry_ret,
                        int *orig_obj_id,
                        hobject_t * orig_obj_ret,
                        int *new_obj_id, hobject_t * new_obj_ret);
};
