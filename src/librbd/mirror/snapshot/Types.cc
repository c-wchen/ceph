...
} void SnapState::dump(Formatter *f) const
{
    ...
    f->dump_unsigned("protection_status", protection_status);
                           ...
                           const SnapState &snap_state)
{
    os << "[" << "namespace=" << snap_state.
       snap_namespace << ", " << "name=" << snap_state.
       name << ", " << "protection=" << static_cast <
       int >(snap_state.protection_status)
       << "]";
    return os;
...
{
    os << "[" << "name=" << image_state.
       name << ", " << "features=" << image_state.
       features << ", " << "snap_limit=" << image_state.
       snap_limit << ", " << "snaps=" << image_state.
       snapshots << ", " << "metadata_count=" << image_state.
       metadata.size()
       << "]";
    return os;
}
