...
class RGWMetadataHandler {
  ...
  public:
    ...
    virtual int get(RGWRados * store, string & entry,
                    RGWMetadataObject ** obj) = 0;
    virtual int put(RGWRados * store, string & entry,
                    RGWObjVersionTracker & objv_tracker, real_time mtime,
                    JSONObj * obj, sync_type_t type) = 0;
    virtual int remove(RGWRados * store, string & entry,
                       RGWObjVersionTracker & objv_tracker) = 0;

    virtual int list_keys_init(RGWRados * store, const string & marker,
                               void **phandle) = 0;
    virtual int list_keys_next(void *handle, int max, list < string > &keys,
                               bool * truncated) = 0;
    ...
    virtual void get_hash_key(const string & section, const string & key,
                              string & hash_key) {
   ...
   */
    bool check_versions(const obj_version & ondisk,
                        const real_time & ondisk_time,
                        const obj_version & incoming,
                        const real_time & incoming_time,
                        sync_type_t sync_mode) {
        switch (sync_mode) {
        case APPLY_UPDATES:
            if ((ondisk.tag != incoming.tag) || (ondisk.ver >= incoming.ver))
                return false;
            break;
        case APPLY_NEWER:
...
class RGWMetadataLogInfoCompletion:public RefCountedObject {
  public:
    using info_callback_t =
        std::function < void (int, const cls_log_header &) >;
  private:
...
class RGWMetadataLog {
  ...
  public:
    RGWMetadataLog(CephContext * _cct, RGWRados * _store,
                   const std::string & period)
    :cct(_cct), store(_store), prefix(make_prefix(period)),
    ...
    void get_shard_oid(int id, string & oid) const {
        char buf[16];
         ...
         oid = prefix + buf;
    } int add_entry(RGWMetadataHandler * handler, const string & section,
                    const string & key, bufferlist & bl);
    int store_entries_in_shard(list < cls_log_entry > &entries, int shard_id,
                               librados::AioCompletion * completion);
    ...
    void init_list_entries(int shard_id, const real_time & from_time,
                           const real_time & end_time, string & marker,
                           void **handle);
    ...
    int trim(int shard_id, const real_time & from_time,
...
class RGWMetadataManager {
    ...
    void parse_metadata_key(const string & metadata_key, string & type,
                            string & entry);

    int find_handler(const string & metadata_key, RGWMetadataHandler ** handler,
                     string & entry);
    int pre_modify(RGWMetadataHandler * handler, string & section,
                   const string & key, RGWMetadataLogData & log_data,
                   RGWObjVersionTracker * objv_tracker, RGWMDLogStatus op_type);
    int post_modify(RGWMetadataHandler * handler, const string & section,
                    const string & key, RGWMetadataLogData & log_data,
                    RGWObjVersionTracker * objv_tracker, int ret);

    string heap_oid(RGWMetadataHandler * handler, const string & key,
                    const obj_version & objv);
    int store_in_heap(RGWMetadataHandler * handler, const string & key,
                      bufferlist & bl, RGWObjVersionTracker * objv_tracker,
                      real_time mtime, map < string, bufferlist > *pattrs);
    int remove_from_heap(RGWMetadataHandler * handler, const string & key,
  ...
  public:
    ...
    /// period history
     RGWPeriodHistory::Cursor read_oldest_log_period() const const;
    ...
    RGWCoroutine *trim_log_period_cr(RGWPeriodHistory::Cursor period,
                                     RGWObjVersionTracker * objv) const const;
    ...
    int put_entry(RGWMetadataHandler * handler, const string & key,
                  bufferlist & bl, bool exclusive,
                  RGWObjVersionTracker * objv_tracker, real_time mtime,
                  map < string, bufferlist > *pattrs = NULL);
    int remove_entry(RGWMetadataHandler * handler, const string & key,
                     RGWObjVersionTracker * objv_tracker);
    ...
    int list_keys_init(const string & section, const string & marker,
                       void **phandle);
    ...
    int lock_exclusive(string & metadata_key, timespan duration,
                       string & owner_id);
    int unlock(string & metadata_key, string & owner_id);

    int get_log_shard_id(const string & section, const string & key,
                         int *shard_id);
};
