                ...
                bool & processing_queue)
{
    ...
    for (; *index < max_objs && result.size() < max;
                    ...
                    if (!expired_only) {
                        ...
                        ret =
                            cls_rgw_gc_list(store->gc_pool_ctx,
                                            obj_names[*index], marker, 1, false,
                                            non_expired_entries, truncated,
                                            next_marker);
        ...
        if (transitioned_objects_cache[*index] || check_queue
            ...
            ret =
                cls_rgw_gc_queue_list_entries(store->gc_pool_ctx,
                                              obj_names[*index], marker,
                                              (max - result.size()) -
                                              entries.size(), expired_only,
                                              queue_entries, truncated,
                                              next_marker);
            if (ret < 0) {
                return ret;
            }
                   ...
                   RGWGCIOManager & io_manager)
{
    ...
    do {
        ...
        if (!transitioned_objects_cache[index]) {
            ret =
                cls_rgw_gc_list(store->gc_pool_ctx, obj_names[index], marker,
                                max, expired_only, entries, &truncated,
                                next_marker);
            ldpp_dout(this,
                      20) <<
            ...
            if ((objv.ver == 1) && entries.size() == 0) {
                std::list < cls_rgw_gc_obj_info > non_expired_entries;
                ret =
                    cls_rgw_gc_list(store->gc_pool_ctx, obj_names[index],
                                    marker, 1, false, non_expired_entries,
                                    &truncated, next_marker);
                if (non_expired_entries.size() == 0) {
                    transitioned_objects_cache[index] = true;
                    marker.clear();
            ...
            if (!chain.objs.empty()) {
                for (liter = chain.objs.begin(); liter != chain.objs.end();
                    ...
                    if (obj.pool != last_pool) {
                        ...
                        ctx = new IoCtx;
                        ret =
                            rgw_init_ioctx(this, store->get_rados_handle(),
                                           obj.pool, *ctx);
                        if (ret < 0) {
                            if (transitioned_objects_cache[index]) {
                                goto done;
...
unsigned RGWGC::get_subsys() const const
{
    return dout_subsys;
}
