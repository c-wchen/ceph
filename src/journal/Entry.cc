...
} // anonymous namespace uint32_t Entry::get_fixed_size() {
...
void Entry::encode(bufferlist &bl) const
{
    ...
    encode(crc, bl);
    ceph_assert(get_fixed_size() + m_data.length() + bl_offset ==
                bl.length());
}
void Entry::decode(bufferlist::const_iterator &iter)
{
    ...
    decode(version, iter);
    if (version != 1) {
        throw buffer::malformed_input("unknown version: " +
                                      stringify(version));
    }
    ...
    bufferlist data_bl;
    data_bl.substr_of(iter.get_bl(), start_offset,
...
bool Entry::is_readable(bufferlist::const_iterator iter,
                        uint32_t *bytes_needed)
{
    ...
    if (iter.get_remaining() < HEADER_FIXED_SIZE) {
        bufferlist sub_bl;
        sub_bl.substr_of(iter.get_bl(), iter.get_off(),
                         iter.get_remaining());
...
bool Entry::operator==(const Entry &rhs) const
{
    return (m_tag_tid == rhs.m_tag_tid && m_entry_tid == rhs.m_entry_tid &&
            const_cast < bufferlist & >(m_data).contents_equal(const_cast <
                    bufferlist &
                    >(rhs.
                      m_data)));
}
