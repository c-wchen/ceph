 ...
 */
    ...
    CompletionData::CompletionData
        (libradosstriper::RadosStriperImpl * striper,
         ...
         int n):RefCountedObject(striper->cct(), n),
 ...
 */
    struct ReadCompletionData:CompletionData {
        ...
        ReadCompletionData(libradosstriper::RadosStriperImpl * striper,
                           ...
                           std::vector < bufferlist > *resultbl, int n);
        /// destructor
    ...
    ReadCompletionData::ReadCompletionData
        (libradosstriper::RadosStriperImpl * striper,
         ...
         int n):CompletionData(striper, soid, lockCookie, userCompletion, n),
 ...
 */
    struct RadosReadCompletionData:RefCountedObject {
        /// constructor
      RadosReadCompletionData(MultiAioCompletionImplPtr multiAioCompl, uint64_t expectedBytes, bufferlist * bl, CephContext * context, int n = 1):
 ...
 */
    struct BasicStatCompletionData:CompletionData {
        /// constructor
      BasicStatCompletionData(libradosstriper::RadosStriperImpl * striper, const std::string & soid, librados::AioCompletionImpl * userCompletion, libradosstriper::MultiAioCompletionImpl * multiCompletion, uint64_t * psize, int n = 1):
 ...
 */
    template < class TimeType >
        ...
        /// constructor
      StatCompletionData(libradosstriper::RadosStriperImpl * striper, const std::string & soid, librados::AioCompletionImpl * userCompletion, libradosstriper::MultiAioCompletionImpl * multiCompletion, uint64_t * psize, TimeType * pmtime, int n = 1):
        BasicStatCompletionData(striper, soid, userCompletion, multiCompletion,
                                psize, n), m_pmtime(pmtime) {
        };
 ...
 */
    struct RadosRemoveCompletionData:RefCountedObject {
        ...
        RadosRemoveCompletionData(MultiAioCompletionImplPtr multiAioCompl,
                                  CephContext *
                                  context):RefCountedObject(context, 2),
...
m_refCnt(0), lock("RadosStriper Refcont", false, false), m_radosCluster(ioctx),
m_ioCtx(ioctx), m_ioCtxImpl(ioctx_impl), m_layout(default_file_layout)
{
}
...
static void rados_req_read_complete(rados_completion_t c, void *arg)
{
    ...
    if (rc >= 0 && (((uint64_t) rc) < data->m_expectedBytes)) {
        ...
        unsigned int existingDataToZero =
            min(data->m_bl->length() - rc, lenOfZeros);
                                                ...
                                                size_t len, uint64_t off)
{
    ...
    ReadCompletionData *cdata =
        new ReadCompletionData(this, soid, lockCookie, c,
    ...
    for (vector < ObjectExtent >::iterator p = extents->begin();
        ...
        // will release one
        RadosReadCompletionData *data =
            new RadosReadCompletionData(nc, p->length, oid_bl, cct(), 2);
...
template < class TimeType >
    ...
    StatCompletionData < TimeType > *cdata =
        new StatCompletionData < TimeType > (this, firstObjOid, c,
                                             multi_completion.get(), psize,
                                             pmtime, 4);
    multi_completion->set_complete_callback(cdata,
                                                  ...
                                                  c, int flags)
{
    ...
    RemoveCompletionData *cdata =
        new RemoveCompletionData(this, soid, lockCookie, c, flags);
    MultiAioCompletionImplPtr multi_completion {
    new libradosstriper::MultiAioCompletionImpl, false};
    multi_completion->set_complete_callback(cdata,
                                             ...
                                             uint64_t size)
{
    ...
    utime_t dur = utime_t();
    rados::cls::lock::lock(&op, RADOS_LOCK_NAME, LOCK_EXCLUSIVE, lockCookie, "",
                           "", dur, 0);
                                                            ...
                                                            uint64_t off)
{
    ...
    WriteCompletionData *cdata =
        new WriteCompletionData(this, soid, lockCookie, 0, 3);
    cdata->get();               // local ref
                                                                ...
                                                                uint64_t off)
{
    ...
    // striper_write_aio_req_safe will release one
    WriteCompletionData *cdata =
        new WriteCompletionData(this, soid, lockCookie, c, 3);
    cdata->get();               // local ref
    m_ioCtxImpl->get();
    c->io = m_ioCtxImpl;
    // create a completion object for the unlocking of the striped object at the end of the write
                                                                ...
                                                                lockCookie)
{
    ...
    *lockCookie = getUUID();
    utime_t dur = utime_t();
    rados::cls::lock::lock(&op, RADOS_LOCK_NAME, LOCK_SHARED, *lockCookie,
                           "Tag", "", dur, 0);
    std::string firstObjOid = getObjectId(soid, 0);
    int rc = m_ioCtx.operate(firstObjOid, &op);
    if (rc) {
...
int libradosstriper::RadosStriperImpl::truncate(const std::string & soid,
                                                ...
                                                ceph_file_layout & layout)
{
    TruncateCompletionData *cdata =
        new TruncateCompletionData(this, soid, size);
