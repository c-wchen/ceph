...
struct RecoveryMessages {
    ...
    void read(ECBackend * ec,
              ...
              const set < pg_shard_t > &need, bool attrs) {
    ...
    if (attrs) {
        ...
        if (op.obc->obs.oi.size > 0) {
            ...
            bufferlist::iterator bp =
                op.xattrs[ECUtil::get_hinfo_key()].begin();
            ::decode(hinfo, bp);
        }
...
bool ECBackend::_handle_message(OpRequestRef _op)
{
    ...
    case MSG_OSD_EC_READ:{
            const MOSDECSubOpRead *op =
                static_cast < const MOSDECSubOpRead * >(_op->get_req());
            ...
            get_parent()->send_message_osd_cluster(op->op.from.osd, reply,
    ...
    case MSG_OSD_PG_PUSH:{
            const MOSDPGPush *op =
                static_cast < const MOSDPGPush * >(_op->get_req());
            RecoveryMessages rm;
...
void ECBackend::sub_write_applied(ceph_tid_t tid, eversion_t version,
                                  const ZTracer::Trace & trace)
{
                                 ...
                                 Context * on_local_applied_sync)
{
    ...
    get_parent()->log_operation(op.log_entries,
                                op.updated_hit_set_history,
                                op.trim_to,
                                op.roll_forward_to, !op.backfill, localt);
    ...
    if (on_local_applied_sync) {
        dout(10) << "Queueing onreadable_sync: " << on_local_applied_sync <<
            dendl;
    ...
    localt.register_on_commit(get_parent()->
                              bless_context(new
                                            SubWriteCommitted(this, msg, op.tid,
                                                              ...
                                                              last_complete,
                                                              trace)));
    localt.register_on_applied(get_parent()->
                               bless_context(new
                                             SubWriteApplied(this, msg, op.tid,
                                                             op.at_version,
                                                             trace)));
                                ...
                                const ZTracer::Trace & trace)
{
            ...
            if (!get_parent()->get_pool().allows_ecoverwrites()) {
                ...
                if (!get_parent()->get_pool().allows_ecoverwrites()) {
                    ...
                    if (!hinfo) {
                        r = -EIO;
                        get_parent()->
                            clog_error() << "Corruption detected: object " <<
    ...
    for (set < hobject_t >::iterator i = op.attrs_to_read.begin();
        ...
        int r = store->getattrs(ch,
                                ghobject_t(*i, ghobject_t::NO_GEN,
                                           get_parent()->whoami_shard().shard),
                                reply->attrs_read[*i]);
        if (r < 0) {
                                       ...
                                       const ZTracer::Trace & trace)
{
    ...
    if (i->second.pending_apply.empty() && i->second.on_all_applied) {
        dout(10) << __func__ << " Calling on_all_applied on " << i->
            second << dendl;
...
void ECBackend::submit_transaction(const hobject_t & hoid,
                                   ...
                                   Context * on_all_commit, ceph_tid_t tid,
                                   osd_reqid_t reqid, OpRequestRef client_op)
{
...
void ECBackend::do_read_op(ReadOp & op)
{
    ...
    for (map < hobject_t, read_request_t >::iterator i = op.to_read.begin();
        ...
        for (list < boost::tuple < uint64_t, uint64_t,
            ...
            for (set < pg_shard_t >::const_iterator k = i->second.need.begin();
                 k != i->second.need.end(); ++k) {
                messages[*k].to_read[i->first].
                    push_back(boost::
    ...
    for (map < pg_shard_t, ECSubRead >::iterator i = messages.begin();
        ...
        get_parent()->send_message_osd_cluster(i->first.osd,
                                               msg, get_parent()->get_epoch());
    }
    dout(10) << __func__ << ": started " << op << dendl;
}

ECUtil::HashInfoRef ECBackend::get_hash_info(const hobject_t & hoid,
                                             bool checks, const map < string,
                                             bufferptr > *attrs)
{
    ...
    if (!ref) {
        ...
        if (r >= 0) {
            ...
            if (bl.length() > 0) {
                ...
                if (checks
                    && hinfo.get_total_chunk_size() != (uint64_t) st.st_size) {
...
bool ECBackend::try_reads_to_commit()
{
    ...
    if (op->plan.t) {
        ECTransaction::generate_transactions(op->plan,
                                             ...
                                             (get_osdmap()->
                                              require_osd_release <
                                              CEPH_RELEASE_KRAKEN), sinfo,
                                             op->remote_read_result,
                                             op->log_entries, &written, &trans,
                                             &(op->temp_added),
                                             &(op->temp_cleared),
                                             get_parent()->get_dpp());
    }
  ...
  for (auto && i:written) {
    ...
    for (set < pg_shard_t >::const_iterator i =
        ...
        if (*i == get_parent()->whoami_shard()) {
            ...
            r->min_epoch = get_parent()->get_interval_start_epoch();
            r->trace = trace;
            get_parent()->send_message_osd_cluster(i->osd, r,
                                                   get_parent()->get_epoch());
        }
    }
    if (should_write_local) {
        handle_sub_write(get_parent()->whoami_shard(),
                         op->client_op,
                         local_write_op, op->trace, op->on_local_applied_sync);
