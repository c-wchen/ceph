    ...
    namespace keystone {
        ...
        ApiVersion CephCtxConfig::get_api_version() const noexcept {
            ...
            case 3:
                ...
                default:dout(0) << "ERROR: wrong Keystone API version: "
            ...
            static const std::string url =
                g_ceph_context->_conf->rgw_keystone_url;
...
/* secrets */ const std::string CephCtxConfig::empty {
        ""};

        static inline std::string read_secret(const std::string & file_path) {
            using namespace std;

            constexpr int16_t size {
            1024};
            char buf[size];
            string s;
        ...
        std::string CephCtxConfig::get_admin_password()const noexcept {
            auto & apv =
                g_ceph_context->_conf->rgw_keystone_admin_password_path;
            if (!apv.empty()) {
        ...
        int TokenEnvelope::parse(const DoutPrefixProvider * dpp,
            ...
            if (!parser.parse(bl.c_str(), bl.length())) {
                ldpp_dout(dpp,
                          0) << "Keystone token parse error: malformed json" <<
                    dendl;
                return -EINVAL;
            }
        ...
        bool TokenCache::find_service(const std::string & token_id,
            ...
            std::lock_guard l {
            lock};
            return find_locked(token_id, token, service_tokens,
                               service_tokens_lru);
        }
        ...
        void TokenCache::add_barbican(const rgw::keystone::
                                      TokenEnvelope & token) {
            std::lock_guard l {
            lock};
        ...
        void TokenCache::invalidate(const DoutPrefixProvider * dpp,
