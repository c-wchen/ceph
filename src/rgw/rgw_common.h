...
const char *rgw_conf_get(const std::map < std::string, std::string, ltstr_nocase > & conf_map, const char *name,
                         const char *def_val);
boost::optional < const std::string & > rgw_conf_get_optional(const std::map < std::string, std::string,
        ltstr_nocase > & conf_map, const std::string& name);
int rgw_conf_get_int(const std::map < std::string, std::string, ltstr_nocase > & conf_map, const char *name,
                     int def_val);
...
template < size_t N > static inline std::array < char, N * 2 + 1 > buf_to_hex(const std::array < unsigned char,
        N > & buf)
{
...
static inline sha256_digest_t calc_hmac_sha256(const sha256_digest_t &key,
...
struct AioCompletionDeleter {
    void operator()(librados::AioCompletion* c)
    {
        c->release();
    }
};
