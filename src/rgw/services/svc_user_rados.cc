                                     ...
                                     const DoutPrefixProvider *dpp)
{
    ...
    try {
        ...
        if (user_id.user_id != user) {
            ldpp_dout(dpp, -1)  << "ERROR: rgw_get_user_info_by_uid(): user id mismatch: " << user_id.user_id << " != " << user <<
                                dendl;
            return -EIO;
        }
        if (!iter.end()) {
                           ...
                           const DoutPrefixProvider *dpp)
    {
        ...
        if (!old_info.user_id.empty() &&
            ...
            if (old_info.user_id.tenant != new_info.user_id.tenant) {
                ldpp_dout(dpp, 0) << "ERROR: tenant mismatch: " << old_info.user_id.tenant << " != " << new_info.user_id.tenant <<
                                  dendl;
                return -EINVAL;
            }
                                       ...
                                       const DoutPrefixProvider *dpp)
{
    ...
    for (; kiter != info.access_keys.end(); ++kiter) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: could not remove " << kiter->first << " (access key object), should be fixed (err=" << ret
                              << ")" << dendl;
            return ret;
        }
    }
    ...
    for (; siter != info.swift_keys.end(); ++siter) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: could not remove " << k.id << " (swift name object), should be fixed (err=" << ret << ")"
                              << dendl;
            return ret;
        }
    }
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 0) << "ERROR: could not remove " << info.user_id << ":" << uid_bucks << ", should be fixed (err=" << ret
                          << ")" << dendl;
        return ret;
    }
                                       ...
                                       optional_yield y, const DoutPrefixProvider *dpp)
{
    ...
    if (ret < 0 && ret != -ENOENT && ret  != -ECANCELED) {
        ...
        ldpp_dout(dpp, 0) << "ERROR: could not remove " << user_info.user_id << ":" << uid_obj << ", should be fixed (err=" <<
                          ret << ")" << dendl;
        return ret;
    }
