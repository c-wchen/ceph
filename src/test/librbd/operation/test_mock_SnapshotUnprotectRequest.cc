...
{
public:
    typedef SnapshotUnprotectRequest < MockImageCtx >
    MockSnapshotUnprotectRequest;
    ...
    {
        auto &expect =
            EXPECT_CALL(mock_image_ctx, is_snap_unprotected(_, _));
        if (r < 0) {
            expect.WillOnce(Return(r));
        } else {
            expect.
            WillOnce(DoAll
                     (SetArgPointee < 1 > (is_unprotected),
                      Return(0)));
        }
    } void expect_set_protection_status(MockImageCtx &mock_image_ctx,
                                        uint64_t snap_id,
                                        uint8_t status, int r)
    {
        bufferlist bl;
        encode(snap_id, bl);
        encode(status, bl);
    ...
    void expect_get_children(MockImageCtx &mock_image_ctx,
                             size_t pools, int r)
    {
        bufferlist bl;
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, Success)
{
    ...
    MockSnapshotUnprotectRequest *req =
        new MockSnapshotUnprotectRequest(mock_image_ctx, &cond_ctx,
                                         cls::rbd::
