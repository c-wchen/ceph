...
template < typename node_key_t > struct FixedKVNode: ChildableCachedExtent {
    ...
    bool is_linked()
    {
        ...
        : ChildableCachedExtent(std::move(ptr)),
          children(capacity, nullptr), capacity(capacity)
    {
    } FixedKVNode(const FixedKVNode &rhs)
        : ChildableCachedExtent(rhs),
          range(rhs.range),
          children(rhs.capacity, nullptr), capacity(rhs.capacity)
    {
    }
    ...
    void insert_child_ptr(iter_t iter, ChildableCachedExtent *child)
    {
        ...
        std::memmove(&raw_children[offset + 1],
                     &raw_children[offset],
                     (get_node_size() -
                      offset) * sizeof(ChildableCachedExtent *));
        if (child) {
    ...
    template < typename iter_t > void remove_child_ptr(iter_t iter)
    {
        ...
        SUBTRACE(seastore_fixedkv_tree,
                 ...
                 this->pending_for_transaction, offset, get_node_size(),
    ...
    static void push_copy_sources(FixedKVNode &dest, FixedKVNode &src)
    {
        ...
        if (!src.is_pending()) {
            ...
            dest.copy_sources.emplace(src.get_prior_instance()->
                                      template cast < FixedKVNode > ());
        } else {
            ceph_assert(src.is_initial_pending());
            dest.copy_sources.insert(src.copy_sources.begin(),
    ...
    static void move_child_ptrs(FixedKVNode &dest,
                                FixedKVNode &src,
                                size_t dest_start,
                                size_t src_start, size_t src_end)
    {
        std::memmove(dest.children.data() + dest_start,
                     ...
                     (src_end -
                      src_start) * sizeof(ChildableCachedExtent *));
    ...
    get_child_ret_t < T > get_child(op_context_t < node_key_t > c,
                                    iter_t iter)
    {
        ...
        if (is_valid_child_ptr(child)) {
            ...
            return c.cache.template get_extent_viewable_by_trans < T >
                   (c.trans, (T *) child);
        } else if (is_pending()) {
    ...
    void merge_child_ptrs(FixedKVNode &left, FixedKVNode &right)
    {
        ...
        if (right.is_pending()) {
            move_child_ptrs(*this, right, left.get_node_size(), 0,
                            right.get_node_size());
            right.my_tracker = nullptr;
        }
    }

    static void balance_child_ptrs(FixedKVNode &left,
                                   FixedKVNode &right,
                                   bool prefer_left,
                                   ...
                                   FixedKVNode &replacement_right)
    {
        ...
        if (pivot_idx < l_size) {
            ...
            if (left.is_pending()) {
                ...
                move_child_ptrs(replacement_right, left, 0, pivot_idx,
                                l_size);
                left.my_tracker = nullptr;
            }
            ...
            if (right.is_pending()) {
                move_child_ptrs(replacement_right, right,
                                l_size - pivot_idx, 0, r_size);
                right.my_tracker = nullptr;
            }
        } else {
            // deal with left
            ...
            push_copy_sources(replacement_right, right);
            if (right.is_pending()) {
                move_child_ptrs(replacement_left, right, l_size, 0,
                                pivot_idx - l_size);
                move_child_ptrs(replacement_right, right, 0,
                                pivot_idx - l_size, r_size);
                right.my_tracker = nullptr;
            }
        }
    }
    ...
    bool is_children_empty() const
    {
        for (auto it = children.begin();
             it != children.begin() + get_node_size(); it++) {
    ...
    template < typename iter_t >
    uint16_t copy_children_from_stable_source(FixedKVNode &source,
    ...
    void copy_children_from_stable_sources(Func && get_iter)
    {
        if (!copy_sources.empty()) {
            ...
            auto &cs = *it;
            uint16_t start_pos =
                cs->lower_bound_offset(get_node_meta().begin);
            ...
            for (; it != copy_sources.end(); it++) {
                ...
                if (copy_source->get_node_meta().
                    is_in_range(get_node_meta().end)) {
                    end_pos =
                        copy_source->upper_bound_offset(get_node_meta().
                                                        end);
                }
                ...
                local_next_pos =
                    copy_children_from_stable_source(*copy_source,
                                                     foreign_start_iter,
                                                     foreign_end_iter,
                                                     local_start_iter);
...
template <
    ...
    size_t node_size, typename node_type_t > struct FixedKVInternalNode
          ...
          node_layout_t(this->get_bptr().c_str())
    {
        ...
        : FixedKVNode < NODE_KEY > (rhs), node_layout_t(this->get_bptr().c_str())
    {
    }
    ...
    void prepare_write() final
    {
        if (this->is_initial_pending()) {
            ...
            this->copy_children_from_stable_sources([this]
                                                    (base_t &node,
            uint16_t pos) {
                ceph_assert(node.
                            get_type()
    ...
    get_child_ret_t < LogicalCachedExtent > get_logical_child(op_context_t < NODE_KEY >, uint16_t pos) final
    {
        ...
        return get_child_ret_t < LogicalCachedExtent >
    ...
    virtual ~ FixedKVInternalNode()
    {
        if (this->is_valid() && !this->is_pending()) {
            if (this->range.is_root()) {
                ...
                auto parent =
                    this->template get_parent_node < FixedKVNode <
                            NODE_KEY >> ();
                auto off =
                    parent->lower_bound_offset(this->get_meta().begin);
                assert(parent->get_key_from_idx(off) ==
                       this->get_meta().begin);
    ...
    void on_replace_prior(Transaction &) final
    {
        ...
        auto copied = this->copy_children_from_stable_source(prior,
                      prior.begin(),
                      prior.end(),
                      this->begin());
    ...
    void update(internal_const_iterator_t iter,
                paddr_t addr, FixedKVNode < NODE_KEY > *nextent)
    {
        LOG_PREFIX(FixedKVInternalNode::update);
        SUBTRACE(seastore_fixedkv_tree, "trans.{}, pos {}, {}",
                 this->pending_for_transaction,
                 iter.get_offset(), *nextent);
        this->update_child_ptr(iter, nextent);
        return this->journal_update(iter,
                                    this->maybe_generate_relative(addr),
                                    maybe_get_delta_buffer());
    }

    void insert(internal_const_iterator_t iter,
                NODE_KEY pivot,
                paddr_t addr, FixedKVNode < NODE_KEY > *nextent)
    {
        ...
        return this->journal_insert(iter,
                                    pivot,
                                    this->maybe_generate_relative(addr),
                                    maybe_get_delta_buffer());
    }
                 ...
                 paddr_t addr, FixedKVNode < NODE_KEY > *nextent)
    {
        ...
        SUBTRACE(seastore_fixedkv_tree,
                 ...
                 this->pending_for_transaction, iter.get_offset(),
                 iter.get_key(), pivot, *nextent);
        this->update_child_ptr(iter, nextent);
        return this->journal_replace(iter,
                                     pivot,
                                     this->maybe_generate_relative(addr),
                                     maybe_get_delta_buffer());
    }
    ...
    {
        auto left =
            c.cache.template alloc_new_extent < node_type_t > (c.trans,
                node_size,
                placement_hint_t::
                HOT,
                INIT_GENERATION);
    ...
    Ref make_full_merge(op_context_t < NODE_KEY > c, Ref &right)
    {
        ...
        replacement->merge_from(*this,
                                *right->template cast < node_type_t > ());
        replacement->range = replacement->get_meta();
        return replacement;
    }

    std::tuple < Ref, Ref, NODE_KEY > make_balanced(op_context_t < NODE_KEY > c,
            Ref &_right, bool prefer_left)
    {
        ...
        auto pivot = this->balance_into_new_nodes(*this,
                     right,
                     prefer_left,
                     *replacement_left,
                     *replacement_right);
        this->balance_child_ptrs(*this,
                                 right,
                                 prefer_left,
    ...
    void resolve_relative_addrs(paddr_t base)
    {
        ...
        for (auto i : *this) {
            if (i->get_val().is_relative()) {
                ...
                SUBTRACE(seastore_fixedkv_tree, "{} -> {}", i->get_val(),
                         updated);
                i->set_val(updated);
            }
        }
        ...
        if (this->is_initial_pending()) {
            for (auto i = from; i != to; ++i) {
                if (i->get_val().is_relative()) {
                    ...
                    i->set_val(this->get_paddr().
                               add_relative(i->get_val()));
                }
            }
        }
    } void node_unresolve_vals(internal_iterator_t from,
        ...
        if (this->is_initial_pending()) {
            for (auto i = from; i != to; ++i) {
                if (i->get_val().is_relative()) {
                    ...
                    i->set_val(i->get_val().
                               block_relative_to(this->get_paddr()));
                }
            }
        }
    } std::ostream &_print_detail(std::ostream &out) const
    {
        ...
        if (this->my_tracker) {
            out << ", my_tracker->parent=" << (void *)this->my_tracker->
                get_parent().get();
        }
...
template <
    ...
    typename VAL_LE,
    size_t node_size,
    typename node_type_t, bool has_children > struct FixedKVLeafNode
    : FixedKVNode < NODE_KEY >,
      common::FixedKVNodeLayout <
      CAPACITY,
      fixed_kv_node_meta_t < NODE_KEY >,
    ...
    using internal_const_iterator_t =
        typename node_layout_t::const_iterator;
    using this_type_t =
        FixedKVLeafNode < CAPACITY, NODE_KEY, NODE_KEY_LE, VAL, VAL_LE,
        node_size, node_type_t, has_children >;
    using base_t = FixedKVNode < NODE_KEY >;
    ...
    virtual void update(internal_const_iterator_t iter,
                        VAL val, LogicalCachedExtent *nextent) = 0;
    virtual internal_const_iterator_t insert(internal_const_iterator_t iter,
            NODE_KEY addr,
            VAL val,
            LogicalCachedExtent *
            nextent) = 0;
