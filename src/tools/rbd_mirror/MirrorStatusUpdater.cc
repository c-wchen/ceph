...
template < typename I >
MirrorStatusUpdater <
I >::MirrorStatusUpdater(librados::IoCtx &io_ctx,
                         Threads < I > *threads,
                         const std::string &local_mirror_uuid)
    : m_io_ctx(io_ctx), m_threads(threads),
      m_local_mirror_uuid(local_mirror_uuid),
      m_lock(ceph::
             make_mutex("rbd::mirror::MirrorStatusUpdater " +
                        stringify(m_io_ctx.get_id())))
{
    dout(10) << "local_mirror_uuid=" << local_mirror_uuid << ", " <<
             "pool_id=" << m_io_ctx.get_id() << dendl;
} template < typename I > MirrorStatusUpdater <
I >::~MirrorStatusUpdater()
{
...
template < typename I >
void MirrorStatusUpdater <
I >::init_mirror_status_watcher(Context *on_finish)
{
    dout(10) << dendl;

    auto ctx = new LambdaContext([this, on_finish](int r) {
        handle_init_mirror_status_watcher(r,
                                          on_finish);
    });
    m_mirror_status_watcher =
        MirrorStatusWatcher < I >::create(m_io_ctx,
...
template < typename I >
void MirrorStatusUpdater <
I >::handle_init_mirror_status_watcher(int r, Context *on_finish)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "failed to init mirror status watcher: " <<
             cpp_strerror(r)
             << dendl;
...
template < typename I >
void MirrorStatusUpdater <
I >::shut_down_mirror_status_watcher(Context *on_finish)
{
    ...
    auto ctx = new LambdaContext([this, on_finish](int r) {
        handle_shut_down_mirror_status_watcher
        (r, on_finish);
    });
...
template < typename I >
void MirrorStatusUpdater <
I >::handle_shut_down_mirror_status_watcher(int r,
        Context *on_finish)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "failed to shut down mirror status watcher: " <<
             cpp_strerror(r)
             << dendl;
    }
...
template < typename I >
void MirrorStatusUpdater < I >::finalize_shutdown(int r,
        Context *
        on_finish)
{
    dout(10) << dendl;
...
template < typename I >
bool MirrorStatusUpdater <
I >::exists(const std::string &global_image_id)
{
...
template < typename I >
void MirrorStatusUpdater <
I >::set_mirror_image_status(const std::string &global_image_id,
                             const cls::rbd::
                             ...
                             bool immediate_update)
{
    dout(15) << "global_image_id=" << global_image_id << ", " <<
             "mirror_image_site_status=" << mirror_image_site_status <<
             dendl;
...
template < typename I >
void MirrorStatusUpdater <
I >::remove_refresh_mirror_image_status(const std::
                                        string &global_image_id,
                                        Context *on_finish)
{
    if (try_remove_mirror_image_status
        (global_image_id, false, false, on_finish)) {
        m_threads->work_queue->queue(on_finish, 0);
    }
}
...
template < typename I >
void MirrorStatusUpdater <
I >::remove_mirror_image_status(const std::string &global_image_id,
                                bool immediate_update,
                                Context *on_finish)
{
    if (try_remove_mirror_image_status
...
template < typename I >
bool MirrorStatusUpdater <
I >::try_remove_mirror_image_status(const std::
                                    string &global_image_id,
                                    bool queue_update,
                                    bool immediate_update,
                                    Context *on_finish)
{
    dout(15) << "global_image_id=" << global_image_id << ", " <<
             "queue_update=" << queue_update << ", " << "immediate_update="
             << immediate_update << dendl;
    ...
    if ((m_update_in_flight &&
        ...
        on_finish = new LambdaContext([this, global_image_id,
                                       queue_update, immediate_update,
        on_finish](int r) {
            if (try_remove_mirror_image_status
                (global_image_id,
                 queue_update,
...
void MirrorStatusUpdater < I >::schedule_timer_task()
{
    ...
    m_threads->timer->add_event_after(UPDATE_INTERVAL_SECONDS,
                                      m_timer_task);
}
...
template < typename I >
void MirrorStatusUpdater < I >::queue_update_task(std::unique_lock <
        ceph::mutex >
        &&locker)
{
    if (!m_initialized) {
    ...
    if (m_update_in_progress) {
        if (m_update_in_flight) {
            dout(10) << "deferring update due to in-flight ops" <<
                     dendl;
    ...
    while (it != updating_global_image_ids.end()) {
        ...
        while (it != updating_global_image_ids.end() &&
            ...
            if (status_it == global_image_status.end()) {
                librbd::cls_client::mirror_image_status_remove(&op,
                        global_image_id);
                ++op_count;
                continue;
            }
            ...
            librbd::cls_client::mirror_image_status_set(&op,
                    global_image_id,
                    status_it->
                    second);
            ++op_count;
...
void MirrorStatusUpdater < I >::handle_update_task(int r)
{
    dout(10) << dendl;
    if (r < 0) {
        derr << "failed to update mirror image statuses: " <<
             cpp_strerror(r)
             << dendl;
    }
