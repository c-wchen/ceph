...
void cephx_calc_client_server_challenge(CephContext * cct, CryptoKey & secret,
                                     ...
                                     CephXTicketBlob & blob)
{
    ...
    ldout(cct,
          10) << "build_service_ticket service " << ceph_entity_type_name(info.
                                                                          service_id)
        << " secret_id " << info.
        secret_id << " ticket_info.ticket.name=" << ticket_info.ticket.name.
        to_str() << dendl;
                                      ...
                                      bufferlist & reply)
{
    ...
    for (vector < CephXSessionAuthInfo >::iterator ticket_iter =
         ticket_info_vec.begin(); ticket_iter != ticket_info_vec.end();
         ++ticket_iter) {
        ...
        *_dout << dendl;
        ...
        if (should_encrypt_ticket) {
            if (encode_encrypt
                (cct, service_ticket_bl, ticket_enc_key, reply, error)) {
                ldout(cct,
                                                     ...
                                                     iterator & indata)
{
    ...
    if (decode_decrypt(cct, msg_a, secret, indata, error)) {
        ldout(cct,
              0) <<
            "verify_service_ticket_reply: failed decode_decrypt, error is: " <<
            error << dendl;
        return false;
    }
    ...
    ldout(cct,
          10) << "verify_service_ticket_reply service " <<
        ceph_entity_type_name(service_id)
        << " secret_id " << ticket.secret_id << " session_key " << msg_a.
        session_key << " validity=" << msg_a.validity << dendl;
    ...
    if (!msg_a.validity.is_zero()) {
        ...
        ldout(cct,
              10) << "ticket expires=" << expires << " renew_after=" <<
            renew_after << dendl;
    }
...
bool CephXTicketHandler::need_key() const const
{
    if (have_key_flag) {
...
bool CephXTicketManager::need_key(uint32_t service_id) const const
{
                                                     ...
                                                     iterator & indata)
{
    ...
    ::decode(num, indata);
    ldout(cct,
          10) << "verify_service_ticket_reply got " << num << " keys" << dendl;
 ...
 */
CephXAuthorizer *CephXTicketHandler::build_authorizer(uint64_t global_id) const const
{
 ...
 * ticket, {timestamp}^session_key
 */
CephXAuthorizer *CephXTicketManager::build_authorizer(uint32_t service_id) const const
{
    map < uint32_t, CephXTicketHandler >::const_iterator iter =
        tickets_map.find(service_id);
    if (iter == tickets_map.end()) {
                         ...
                         CephXServiceTicketInfo & ticket_info)
{
    ...
    if (secret_id == (uint64_t) - 1) {
        if (!keys->get_secret(cct->_conf->name, service_secret)) {
            ldout(cct,
                  0) <<
                "ceph_decode_ticket could not get general service secret for service_id="
                             ...
                             *challenge, bufferlist & reply_bl)
{
    ...
    if (ticket.secret_id == (uint64_t) - 1) {
        ...
        if (!keys->get_secret(name, service_secret)) {
            ldout(cct,
                  0) <<
                "verify_authorizer could not get general service secret for service "
                << ceph_entity_type_name(service_id) << " secret_id=" << ticket.
                secret_id << dendl;
            return false;
        }
    }
        ...
        if (!keys->
            get_service_secret(service_id, ticket.secret_id, service_secret)) {
            ldout(cct,
                  0) <<
    ...
    if (ticket_info.ticket.global_id != global_id) {
        ldout(cct,
              0) << "verify_authorizer global_id mismatch: declared id=" <<
            global_id << " ticket_id=" << ticket_info.ticket.global_id << dendl;
        return false;
    }
    ...
    if (challenge) {
        ...
        if (!auth_msg.have_challenge || !c) {
            ...
            challenge->reset(c);
            get_random_bytes((char *)&c->server_challenge,
                             sizeof(c->server_challenge));
            ldout(cct,
                  10) << __func__ << " adding server_challenge " << c->
                server_challenge << dendl;

            encode_encrypt_enc_bl(cct, *c, ticket_info.session_key, reply_bl,
                                  error);
            if (!error.empty()) {
                ldout(cct,
                      10) << "verify_authorizer: encode_encrypt error: " <<
                    error << dendl;
                return false;
            }
            return false;
    ...
    ldout(cct,
          10) << "verify_authorizer ok nonce " << hex << auth_msg.
        nonce << dec << " reply_bl.length()=" << reply_bl.length() << dendl;
    return true;
}
...
bool CephXAuthorizer::verify_reply(bufferlist::iterator & indata)
{
    ...
    if (expect != reply.nonce_plus_one) {
        ldout(cct,
            ...
            nonce_plus_one << " expected " << expect << " sent " << nonce <<
            dendl;
        return false;
    }
    return true;
...
bool CephXAuthorizer::add_challenge(CephContext * cct, bufferlist & challenge)
{
    ...
    if (encode_encrypt(cct, msg, session_key, bl, error)) {
        ldout(cct,
              0) << __func__ << " failed to encrypt authorizer: " << error <<
            dendl;
        return false;
    }
    return true;
