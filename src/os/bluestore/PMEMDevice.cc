...
:BlockDevice(cct),
fd(-1), addr(0),
size(0), block_size(0), debug_lock("PMEMDevice::debug_lock"), injecting_crash(0)
{
}
...
int PMEMDevice::open(const string & p)
{
    ...
    size_t map_len;
    addr =
        (char *)pmem_map_file(path.c_str(), size, PMEM_FILE_EXCL, O_RDWR,
                              &map_len, NULL);
    if (addr == NULL) {
...
int PMEMDevice::collect_metadata(string prefix, map < string, string > *pm) const const
{
