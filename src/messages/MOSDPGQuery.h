...
public:
    ...
    MOSDPGQuery(epoch_t e, map<spg_t,
                               pg_query_t> &ls) : Message(MSG_OSD_PG_QUERY, HEAD_VERSION,
                                                          COMPAT_VERSION),
                                                  epoch(e)
    {
        pg_list.swap(ls);
...
public:
    ...
    void decode_payload() override
    {
        ...
        if (header.version < 4)
        {
            ...
            vector<pair<pg_t, pg_query_t>> _pg_list;
            ::decode(_pg_list, p);
            vector<shard_id_t> _shard_list(_pg_list.size(),
                                           shard_id_t::NO_SHARD);
            _shard_list.clear();
            ...
            {
                pg_list.insert(make_pair(spg_t(_pg_list[i].first, _shard_list[i]),
                                         _pg_list[i].second));
            }
            return;
        }
