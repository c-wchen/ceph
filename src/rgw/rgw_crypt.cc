...
class AES_256_CTR:public BlockCrypt {
  ...
  public:
    ...
    bool encrypt(bufferlist & input, off_t in_ofs, size_t size,
                 bufferlist & output, off_t stream_offset) {
        byte iv[AES_256_IVSIZE];
        ldout(cct, 25)
            << "Encrypt in_ofs " << in_ofs
            << " size=" << size << " stream_offset=" << stream_offset << dendl;
        if (input.length() < in_ofs + size) {
            return false;
        }

        output.clear();
        buffer::ptr buf((size + AES_256_KEYSIZE - 1) / AES_256_KEYSIZE *
                        AES_256_KEYSIZE);
        /*create CTR mask */
        ...
        while (iter != input.buffers().end()) {
            off_t cnt =
                std::min < off_t > (iter->length() - plaintext_pos,
                                    size - crypt_pos);
    ...
    bool encrypt(bufferlist & input, off_t in_ofs, size_t size,
        ...
        if (slot) {
            ...
            symkey =
                PK11_ImportSymKey_FIPS(slot, CKM_AES_CTR, PK11_OriginUnwrap,
                                       CKA_UNWRAP, &keyItem, NULL);
            if (symkey) {
                static_assert(sizeof(ctr_params.cb) >= AES_256_IVSIZE,
                              "Must fit counter");
                ctr_params.ulCounterBits = 128;
                prepare_iv(reinterpret_cast < unsigned char *>(&ctr_params.cb),
                           stream_offset);
                ...
                if (param) {
                    ectx =
                        PK11_CreateContextBySymKey(CKM_AES_CTR, CKA_ENCRYPT,
                                                   symkey, param);
                    if (ectx) {
                        buffer::ptr buf((size + AES_256_KEYSIZE - 1) /
                                        AES_256_KEYSIZE * AES_256_KEYSIZE);
                        ret =
                            PK11_CipherOp(ectx, (unsigned char *)buf.c_str(),
                                          &written, buf.length(),
        ...
        if (result == false) {
            ldout(cct,
                  5) << "Failed to perform AES-CTR encryption: " <<
                PR_GetError() << dendl;
        }
        return result;
    }
...
#error Must define USE_CRYPTOPP or USE_NSS
    ...
    bool decrypt(bufferlist & input, off_t in_ofs, size_t size,
                 bufferlist & output, off_t stream_offset) {
...
CryptoAccelRef get_crypto_accel(CephContext * cct)
{
    ...
    CryptoPlugin *factory =
        dynamic_cast <
        CryptoPlugin * >(reg->get_with_load("crypto", crypto_accel_type));
    if (factory == nullptr) {
        lderr(cct) << __func__ << " cannot load crypto accelerator of type " <<
  ...
  public:
    ...
    bool cbc_transform(unsigned char *out,
        ...
        if (slot) {
            ...
            keyItem.len = AES_256_KEYSIZE;
            symkey =
                PK11_ImportSymKey_FIPS(slot, CKM_AES_CBC, PK11_OriginUnwrap,
                                       CKA_UNWRAP, &keyItem, NULL);
            if (symkey) {
                ...
                if (param) {
                    ectx =
                        PK11_CreateContextBySymKey(CKM_AES_CBC,
                                                   encrypt ? CKA_ENCRYPT :
                                                   CKA_DECRYPT, symkey, param);
                    if (ectx) {
                        ret = PK11_CipherOp(ectx,
                                            out, &written, size, in, size);
        ...
        if (result == false) {
            ldout(cct,
                  5) << "Failed to perform AES-CBC encryption: " <<
                PR_GetError() << dendl;
        }
        return result;
    }
...
#error Must define USE_CRYPTOPP or USE_NSS
    ...
    bool cbc_transform(unsigned char *out,
        ...
        for (size_t offset = 0; result && (offset < size); offset += CHUNK_SIZE) {
            size_t process_size =
                offset + CHUNK_SIZE <= size ? CHUNK_SIZE : size - offset;
            prepare_iv(iv, stream_offset + offset);
            if (crypto_accel != nullptr) {
                if (encrypt) {
                    result =
                        crypto_accel->cbc_encrypt(out + offset, in + offset,
                                                  process_size, iv, key);
                }
                else {
                    result =
                        crypto_accel->cbc_decrypt(out + offset, in + offset,
                                                  process_size, iv, key);
                }
            }
            else {
                result = cbc_transform(out + offset, in + offset, process_size,
                                       iv, key, encrypt);
            }
        }
        return result;
    ...
    bool encrypt(bufferlist & input,
                 off_t in_ofs,
                 size_t size, bufferlist & output, off_t stream_offset) {
        bool result = false;
        ...
        unsigned char *buf_raw =
            reinterpret_cast < unsigned char *>(buf.c_str());
        ...
        result = cbc_transform(buf_raw,
                               input_raw + in_ofs,
                               aligned_size, stream_offset, key, true);
        if (result && (unaligned_rest_size > 0)) {
            ...
            if (aligned_size % CHUNK_SIZE > 0) {
                ...
                result = cbc_transform(buf_raw + aligned_size,
                                       data, AES_256_IVSIZE, iv, key, true);
            }
            if (result) {
    ...
    bool decrypt(bufferlist & input,
        ...
        unsigned char *input_raw =
            reinterpret_cast < unsigned char *>(input.c_str());

        /* decrypt main bulk of data */
        result = cbc_transform(buf_raw,
                               input_raw + in_ofs,
                               aligned_size, stream_offset, key, false);
        if (result && unaligned_rest_size > 0) {
            ...
            if (aligned_size % CHUNK_SIZE > 0) {
                ...
                unsigned char iv[AES_256_IVSIZE] = { 0 };
                result = cbc_transform(buf_raw + aligned_size,
                                       input_raw + in_ofs + aligned_size -
                                       AES_256_IVSIZE, AES_256_IVSIZE, iv, key,
                                       true);
            }
            else {
                /* 0 full blocks in current chunk, use IV as base for unaligned part */
...
std::unique_ptr < BlockCrypt > AES_256_CBC_create(CephContext * cct,
                                                  const uint8_t * key,
                                                  size_t len)
{
                         ...
                         uint8_t * data_out, size_t data_size)
{
    ...
    if (key_size == AES_256_KEYSIZE) {
        ...
        if (slot) {
            ...
            if (param) {
                symkey =
                    PK11_ImportSymKey_FIPS(slot, CKM_AES_ECB, PK11_OriginUnwrap,
                                           CKA_UNWRAP, &keyItem, NULL);
                if (symkey) {
                    ectx =
                        PK11_CreateContextBySymKey(CKM_AES_ECB, CKA_ENCRYPT,
                                                   symkey, param);
                    if (ectx) {
                        ret = PK11_CipherOp(ectx,
        ...
        if (result == false) {
            ldout(cct,
                  5) << "Failed to perform AES-ECB encryption: " <<
                PR_GetError() << dendl;
        }
    }
    else {
...
RGWGetObj_BlockDecrypt::RGWGetObj_BlockDecrypt(CephContext * cct, RGWGetDataCB * next, std::unique_ptr < BlockCrypt > crypt):
RGWGetObj_Filter(next),
cct(cct), crypt(std::move(crypt)), enc_begin_skip(0), ofs(0), end(0), cache()
{
    ...
    if (manifest_bl.length()) {
        ...
        if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 20)) {
            for (size_t i = 0; i < parts_len.size(); i++) {
                ldout(cct,
                      20) << "Manifest part " << i << ", size=" << parts_len[i]
                    << dendl;
            }
        }
    }
...
int RGWGetObj_BlockDecrypt::process(bufferlist & in, size_t part_ofs,
                                    size_t size)
{
    bufferlist data;
...
int RGWGetObj_BlockDecrypt::handle_data(bufferlist & bl, off_t bl_ofs,
                                        off_t bl_len)
{
...
{
    ldout(cct,
          25) << "Decrypt flushing " << cache.length() << " bytes" << dendl;
    int res = 0;
    size_t part_ofs = ofs;
  for (size_t part:parts_len) {
        if (part_ofs >= part) {
...
RGWPutObj_BlockEncrypt::RGWPutObj_BlockEncrypt(CephContext * cct, RGWPutObjDataProcessor * next, std::unique_ptr < BlockCrypt > crypt):
RGWPutObj_Filter(next),
cct(cct), crypt(std::move(crypt)), ofs(0), cache()
{
    block_size = this->crypt->get_block_size();
}
...
static int get_barbican_url(CephContext * const cct, std::string & url)
{
                                   ...
                                   std::string & actual_key)
{
    ...
    if (it != str_map.end()) {
        ...
        try {
            ...
            ldout(cct,
                  5) <<
                "ERROR: get_actual_key_from_kms invalid encryption key id " <<
        ...
        if (master_key.length() == AES_256_KEYSIZE) {
            ...
            if (AES_256_ECB_encrypt(cct,
                                    reinterpret_cast <
                                    const uint8_t * >(master_key.c_str()),
                                    AES_256_KEYSIZE,
                                    reinterpret_cast <
                                    const uint8_t * >(key_selector.data()),
                                    _actual_key, AES_256_KEYSIZE)) {
                actual_key =
                    std::string((char *)&_actual_key[0], AES_256_KEYSIZE);
            }
            else {
                res = -EIO;
            }
        ...
        res =
            request_key_from_barbican(cct, key_id, key_selector, token,
                                      actual_key);
        if (res != 0) {
            ldout(cct,
...
static inline void set_attr(map < string, bufferlist > &attrs,
                            const char *key, boost::string_view value)
{
    bufferlist bl;
...
static boost::string_view get_crypt_attribute(const RGWEnv * env,
                                              std::map < std::string,
                                              RGWPostObj_ObjStore::
                                              post_form_part,
...
{
    static_assert(X_AMZ_SERVER_SIDE_ENCRYPTION_LAST ==
    ...
    if (parts != nullptr) {
        ...
        boost::string_view str =
            boost::string_view(data.c_str(), data.length());
        return rgw_trim_whitespace(str);
    }
    else {
        const char *hdr =
            env->get(crypt_options[option].http_header_name, nullptr);
        if (hdr != nullptr) {
                           ...
                           std::string > &crypt_http_responses)
{
    ...
    crypt_http_responses.clear();
    {
        ...
        if (!req_sse_ca.empty()) {
            ...
            try {
                key_bin =
                    from_base64(get_crypt_attribute
                                (s->info.env, parts,
                                 X_AMZ_SERVER_SIDE_ENCRYPTION_CUSTOMER_KEY));
            }
            catch( ...) {
            ...
            key_hash.Update(reinterpret_cast < const byte * >(key_bin.c_str()),
                            key_bin.size());
            key_hash.Final(key_hash_res);

            if (memcmp
                (key_hash_res, keymd5_bin.c_str(),
            ...
            if (block_crypt) {
                auto aes =
                    std::unique_ptr < AES_256_CBC > (new AES_256_CBC(s->cct));
                aes->set_key(reinterpret_cast <
                             const uint8_t * >(key_bin.c_str()),
                             AES_256_KEYSIZE);
        ...
        if (!req_sse.empty()) {
            ...
            std::string actual_key;
            res =
                get_actual_key_from_kms(s->cct, key_id, key_selector,
                                        actual_key);
            if (res != 0) {
                ldout(s->cct,
                      5) << "ERROR: failed to retrieve actual key from key_id: "
                    << key_id << dendl;
                s->err.message =
                    "Failed to retrieve the actual key, kms-keyid: " +
                    key_id.to_string();
                return res;
            }
            ...
            if (block_crypt) {
                auto aes =
                    std::unique_ptr < AES_256_CBC > (new AES_256_CBC(s->cct));
                aes->set_key(reinterpret_cast <
                             const uint8_t * >(actual_key.c_str()),
                             AES_256_KEYSIZE);
                *block_crypt = std::move(aes);
            }
            actual_key.replace(0, actual_key.length(), actual_key.length(),
                               '\000');
            ...
            if (!key_id.empty()) {
                ldout(s->cct,
                      5) <<
                    "ERROR: SSE-KMS encryption request is missing the header "
        ...
        if (s->cct->_conf->rgw_crypt_default_encryption_key != "") {
            ...
            if (master_encryption_key.size() != 256 / 8) {
                ldout(s->cct,
                      0) <<
                    "ERROR: failed to decode 'rgw crypt default encryption key' to 256 bit string"
                    << dendl;
            ...
            if (AES_256_ECB_encrypt(s->cct,
                                    ...
                                    AES_256_KEYSIZE,
                                    reinterpret_cast <
                                    const uint8_t * >(key_selector.c_str()),
            ...
            if (block_crypt) {
                auto aes =
                    std::unique_ptr < AES_256_CBC > (new AES_256_CBC(s->cct));
                aes->set_key(reinterpret_cast < const uint8_t * >(actual_key),
                             AES_256_KEYSIZE);
                *block_crypt = std::move(aes);
            }
            memset(actual_key, 0, sizeof(actual_key));
                           ...
                           std::string > &crypt_http_responses)
{
    ...
    if (stored_mode == "SSE-C-AES256") {
        if (s->cct->_conf->rgw_crypt_require_ssl &&
            !rgw_transport_is_secure(s->cct, *s->info.env)) {
            ldout(s->cct,
                  5) << "ERROR: Insecure request, rgw_crypt_require_ssl is set"
                << dendl;
            return -ERR_INVALID_REQUEST;
        }
        ...
        if (nullptr == req_cust_alg) {
            ...
            ldout(s->cct,
                  5) <<
                "ERROR: The requested encryption algorithm is not valid, must be AES256."
                << dendl;
            s->err.message =
                "The requested encryption algorithm is not valid, must be AES256.";
            return -ERR_INVALID_ENCRYPTION_ALGORITHM;
        ...
        if ((memcmp
             (key_hash_res, keymd5_bin.c_str(),
              CEPH_CRYPTO_MD5_DIGESTSIZE) != 0)
            || (get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYMD5) !=
                keymd5_bin)) {
            s->err.message =
                "The calculated MD5 hash of the key did not match the hash that was provided.";
            return -EINVAL;
        ...
        aes->set_key(reinterpret_cast < const uint8_t * >(key_bin.c_str()),
    ...
    if (stored_mode == "SSE-KMS") {
        ...
        if (res != 0) {
            ldout(s->cct,
                  10) << "ERROR: failed to retrieve actual key from key_id: " <<
                key_id << dendl;
            s->err.message =
    ...
    if (stored_mode == "RGW-AUTO") {
        ...
        if (AES_256_ECB_encrypt(s->cct,
                                ...
                                AES_256_KEYSIZE,
                                reinterpret_cast <
                                const uint8_t * >(attr_key_selector.c_str()),
                                actual_key, AES_256_KEYSIZE) != true) {
            memset(actual_key, 0, sizeof(actual_key));
            return -EIO;
        }
