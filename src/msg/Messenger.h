...
public:
     ...
     */ static Messenger *create(CephContext *cct,
                                 ...
                                 entity_name_t name,
                                 std::string lname, uint64_t nonce);
...
public:
     ...
     */
    virtual int rebind(const std::set < int > &avoid_ports)
    {
        return -EOPNOTSUPP;
    }
    /**
     ...
     */
    virtual int send_to(Message *m,
                        int type, const entity_addrvec_t &addr) = 0;
    int send_to_mon(Message *m, const entity_addrvec_t &addrs)
    {
        return send_to(m, CEPH_ENTITY_TYPE_MON, addrs);
    }
    int send_to_mds(Message *m, const entity_addrvec_t &addrs)
    {
     ...
     */
    virtual ConnectionRef connect_to(int type, const entity_addrvec_t &dest,
                                     bool anon = false, bool not_local_dest =
                                         false) = 0;
    ConnectionRef connect_to_mon(const entity_addrvec_t &dest, bool anon =
                                     false, bool not_local_dest = false)
    {
