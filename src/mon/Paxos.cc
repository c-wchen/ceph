...
{
    PerfCountersBuilder pcb(g_ceph_context, "paxos", l_paxos_first,
                            l_paxos_last);
    ...
    pcb.add_u64_avg(l_paxos_begin_bytes, "begin_bytes",
                    "Data in transaction on begin", NULL, 0, unit_t(BYTES));
    ...
    pcb.add_u64_avg(l_paxos_commit_bytes, "commit_bytes",
                    "Data in transaction on commit", NULL, 0, unit_t(BYTES));
    pcb.add_time_avg(l_paxos_commit_latency, "commit_latency", "Commit latency",
                     "clat");
    ...
    pcb.add_u64_avg(l_paxos_collect_bytes, "collect_bytes",
                    "Data in transaction on peon collect", NULL, 0,
                    unit_t(BYTES));
    ...
    pcb.add_u64_avg(l_paxos_store_state_bytes, "store_state_bytes",
                    "Data in transaction in stored state", NULL, 0,
                    unit_t(BYTES));
    ...
    pcb.add_u64_avg(l_paxos_share_state_bytes, "share_state_bytes",
                    "Data in shared state", NULL, 0, unit_t(BYTES));
...
void Paxos::collect(version_t oldpn)
{
    ...
    collect_timeout_event =
        mon->timer.add_event_after(g_conf->mon_accept_timeout_factor *
                                   g_conf->mon_lease, new C_MonContext(mon,
                                                                       [this]
                                                                       (int r) {
                                                                       if (r ==
                                                                           -ECANCELED)
...
void Paxos::handle_last(MonOpRequestRef op)
{
    ...
    for (map < int, version_t >::iterator p = peer_last_committed.begin();
        ...
        if (p->second < last_committed) {
            ...
            mon->messenger->send_message(commit,
                                         mon->monmap->get_inst(p->first));
        }
    }
...
void Paxos::commit_finish()
{
    ...
    for (set < int >::const_iterator p = mon->get_quorum().begin();
        ...
        MMonPaxos *commit =
            new MMonPaxos(mon->get_epoch(), MMonPaxos::OP_COMMIT,
                          ceph_clock_now());
...
void Paxos::trim()
{
    ...
    if (g_conf->mon_compact_on_trim) {
        ...
        t->compact_range(get_name(), stringify(first_committed - 1),
                         stringify(end));
    }
