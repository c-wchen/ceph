...
namespace librbd {
    namespace cache {
        ...
        struct ObjectCacherObjectDispatch <I >::C_InvalidateCache:public Context {
            ...
            :dispatcher(dispatcher), purge_on_error(purge_on_error),
                ...
                if (r == -EBLOCKLISTED) {
                    ...
                    dispatcher->m_object_cacher->purge_set(dispatcher->
                                                           m_object_set);
                }
                    ...
                    lderr(cct) << "failed to invalidate cache: " <<
                        cpp_strerror(r) << dendl;
                }

                auto unclean =
                    dispatcher->m_object_cacher->release_set(dispatcher->
                                                             m_object_set);
                if (unclean == 0) {
                    r = 0;
                }
        ...
        template < typename I >
            ...
            I >::ObjectCacherObjectDispatch(I * image_ctx, size_t max_dirty,
      ...
      :    m_image_ctx(image_ctx), m_max_dirty(max_dirty),
        ...
        template < typename I > void ObjectCacherObjectDispatch < I >::init() {
            ...
            m_writeback_handler =
                new ObjectCacherWriteback(m_image_ctx, m_cache_lock);
            ...
            m_object_cacher = new ObjectCacher(cct, m_image_ctx->perfcounter->get_name(), *m_writeback_handler, m_cache_lock, nullptr, nullptr, cache_size, 10, /* reset this in init */
        ...
        template < typename I >
            void ObjectCacherObjectDispatch <
        ...
        template < typename I >
            bool ObjectCacherObjectDispatch < I >::read(uint64_t object_no,
                                                        io::ReadExtents *
                                                        extents,
            ...
            auto cct = m_image_ctx->cct;
            ldout(cct,
                  20) << "object_no=" << object_no << " " << *extents << dendl;
            ...
            op_flags = ((op_flags & ~ObjectCacherWriteback::READ_FLAGS_MASK) |
                        ((read_flags << ObjectCacherWriteback::
                          READ_FLAGS_SHIFT) & ObjectCacherWriteback::
                         READ_FLAGS_MASK));

            ceph::bufferlist * bl;
            if (extents->size() > 1) {
                auto req =
                    new io::ReadResult::C_ObjectReadMergedExtents(cct, extents,
                                                                  on_dispatched);
                on_dispatched = req;
                bl = &req->bl;
            }
            ...
            auto rd =
                m_object_cacher->prepare_read(io_context->read_snap().
                                              value_or(CEPH_NOSNAP), bl,
                                              op_flags);
          ...
          for (auto & read_extent:*extents) {
            ...
            *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
            ...
            m_cache_lock.lock();
            int r =
                m_object_cacher->readx(rd, m_object_set, on_dispatched, &trace);
            m_cache_lock.unlock();
            if (r != 0) {
        ...
        template < typename I >
            bool ObjectCacherObjectDispatch < I >::discard(uint64_t object_no,
                                                           uint64_t object_off,
                                                           uint64_t object_len,
                                                           IOContext io_context,
                                                           int discard_flags,
            ...
            auto cct = m_image_ctx->cct;
            ldout(cct,
                  20) << "object_no=" << object_no << " " << object_off << "~"
                << object_len << dendl;

            ObjectExtents object_extents;
            object_extents.
                emplace_back(data_object_name(m_image_ctx, object_no),
        ...
        template < typename I >
            bool ObjectCacherObjectDispatch < I >::write(uint64_t object_no,
                                                         uint64_t object_off,
                                                         ceph::bufferlist
                                                         && data,
                                                         IOContext io_context,
            ...
            ObjectCacher::OSDWrite * wr =
                m_object_cacher->prepare_write(snapc, data,
        ...
        template < typename I >
            bool ObjectCacherObjectDispatch <
            I >::write_same(uint64_t object_no, uint64_t object_off,
                            uint64_t object_len, io::LightweightBufferExtents
                            && buffer_extents, ceph::bufferlist
            ...
            return write(object_no, object_off, std::move(ws_data), io_context,
                         op_flags, 0, std::nullopt, parent_trace,
                         object_dispatch_flags, journal_tid, dispatch_result,
        ...
        template < typename I >
            ...
            if (flush_source == io::FLUSH_SOURCE_USER && !m_user_flushed) {
                ...
                if (m_writethrough_until_flush && m_max_dirty > 0) {
                    ...
                    ldout(cct,
                          5) << "saw first user flush, enabling writeback" <<
                        dendl;
                }
            }
        ...
        template < typename I >
            bool ObjectCacherObjectDispatch <
            I >::invalidate_cache(Context * on_finish) {
            auto cct = m_image_ctx->cct;
            ldout(cct, 5) << dendl;
        ...
        template < typename I >
            bool ObjectCacherObjectDispatch <
            I >::reset_existence_cache(Context * on_finish) {
            auto cct = m_image_ctx->cct;
            ldout(cct, 5) << dendl;
