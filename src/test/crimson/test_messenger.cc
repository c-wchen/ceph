...
namespace {
    ...
    static seastar::future <> test_echo(unsigned rounds, double keepalive_ratio) {
        struct test_state {
            struct Server final:public crimson::net::Dispatcher {
                ...
                std::optional <
                    seastar::future <>> ms_dispatch(crimson::net::
                                                    ConnectionRef c,
                    ...
                    // reply with a pong
                        std::ignore =
                        c->send(crimson::make_message < MPing > ());
                    return {
                    seastar::now()};
                }
                ...
                seastar::future <> init(const entity_name_t & name,
                    ...
                    msgr->
                        set_default_policy(crimson::net::SocketPolicy::
                                           stateless_server(0));
                    msgr->set_auth_client(&dummy_auth);
                    msgr->set_auth_server(&dummy_auth);
                    return msgr->bind(entity_addrvec_t {
                                      addr}
                    ).safe_then([this] {
                                return msgr->start( {
                                                   this}
                                ...
                                all_same_way([addr] (const std::error_code & e)
                                             {
                                             logger().
                                             error("test_echo(): "
            ...
            struct Client final:public crimson::net::Dispatcher {
                struct PingSession:public seastar::enable_shared_from_this <
                    PingSession > {
                    unsigned count = 0u;
                ...
                std::map < crimson::net::ConnectionRef,
                    seastar::promise <>> pending_conns;
                std::map < crimson::net::ConnectionRef,
                    PingSessionRef > sessions;
                crimson::auth::DummyAuthClientServer dummy_auth;

                Client(unsigned rounds, double keepalive_ratio)
                :rounds(rounds), keepalive_dist(std::bernoulli_distribution {
                                                keepalive_ratio}) {
                }
                ...
                void ms_handle_connect(crimson::net::
                                       ConnectionRef conn) override {
                ...
                std::optional <
                    ...
                    if (session->count == rounds) {
                        logger().info("{}: finished receiving {} pongs", *c,
                                      session->count);
                ...
                seastar::
                    future <> dispatch_pingpong(const entity_addr_t & peer_addr)
                {
                    mono_time start_time = mono_clock::now();
                    auto conn =
                        msgr->connect(peer_addr, entity_name_t::TYPE_OSD);
                    return seastar::futurize_invoke([this, conn] {
                           ...
                           logger().info("{}: handshake {}, pingpong {}", *conn,
                                         dur_handshake.count(),
                                         dur_pingpong.count());});
                }

              private:
                seastar::future <> do_dispatch_pingpong(crimson::net::
                                                        ConnectionRef conn) {
                    auto[i, added] =
                        pending_conns.emplace(conn, seastar::promise <> ());
                    std::ignore = i;
                    ceph_assert(added);
                    return seastar::do_with(0u, 0u,
                                            [this, conn] (auto & count_ping,
                                                          auto &
                                                          count_keepalive) {
                                            return seastar::
                                            do_until([this, conn, &count_ping,
                                                      &count_keepalive] {
                                                     bool stop =
                                                     (count_ping == rounds);
                                                     if (stop) {
                                                     logger().
                                                     info
                                                     ("{}: finished sending {} pings with {} keepalives",
                                                      *conn, count_ping,
                                                      count_keepalive);}
                                                     return stop;}
                                                     ,
                                                     [this, conn, &count_ping,
                                                      &count_keepalive] {
                                                     return seastar::
                                                     repeat([this, conn,
                                                             &count_ping,
        ...
        auto client1 =
            seastar::make_shared < test_state::Client > (rounds,
                                                         keepalive_ratio);
        auto client2 =
            seastar::make_shared < test_state::Client > (rounds,
                                                         keepalive_ratio);
        ...
        return seastar::when_all_succeed(server1->
                                         init(entity_name_t::OSD(0), "server1",
                                              1, addr1),
                                         server2->init(entity_name_t::OSD(1),
                                                       "server2", 2, addr2),
                                         client1->init(entity_name_t::OSD(2),
                                                       "client1", 3),
                                         client2->init(entity_name_t::OSD(3),
                                                       "client2", 4)
                                         // dispatch pingpoing
               ...
               logger().info("client1 shutdown..."); return client1->shutdown();}
        ).then([client2] {
               logger().info("client2 shutdown..."); return client2->shutdown();}
        ).then([server1] {
               logger().info("server1 shutdown..."); return server1->shutdown();}
        ).then([server2] {
               logger().info("server2 shutdown..."); return server2->shutdown();}
        ).then([] {
               logger().info("test_echo() done!\n");}
        ).handle_exception([server1, server2, client1, client2] (auto eptr) {
                           logger().
                           error("test_echo() failed: got exception {}", eptr);
                           throw;}
        );
    }
    ...
    static seastar::future <> test_concurrent_dispatch() {
            ...
            struct Server final:public crimson::net::Dispatcher {
                ...
                std::optional <
                    ...
                    case 1:
                        ...
                        std::ignore = on_second.get_future().then([this] {
                                                                  on_done.
                                                                  set_value();
                                                                  }
                        );
                        break;
                        case 2:on_second.set_value();
                        break;
        ...
        return seastar::when_all_succeed(server->
                                         init(entity_name_t::OSD(4), "server3",
                                              5, addr),
                                         client->init(entity_name_t::OSD(5),
                                                      "client3", 6)
            ).then_unpack([server, client] {
                          auto conn =
                          client->msgr->connect(server->msgr->get_myaddr(),
               ...
               client->msgr->stop(); return client->msgr->shutdown();}
        ).then([server] {
               logger().info("server shutdown...");
               server->msgr->stop(); return server->msgr->shutdown();}
        ).then([] {
               logger().info("test_concurrent_dispatch() done!\n");}
        ).handle_exception([server, client] (auto eptr) {
                           logger().
                           error
                           ("test_concurrent_dispatch() failed: got exception {}",
                            eptr); throw;}
        );
    }
    ...
    seastar::future <> test_preemptive_shutdown() {
            ...
            class Client final:public crimson::net::Dispatcher {
              ...
              public:
                ...
                void send_pings(const entity_addr_t & addr) {
                    ...
                    (void)seastar::do_until([this] {
                                            return stop_send;
                                            }
                                            ,[conn] {
                                            return conn->
                                            send(crimson::make_message < MPing >
                                                 ()).then([] {
                                                          return seastar::
                                                          sleep(0 ms);}
                                            );}
        ...
        addr.set_family(AF_INET);
        return seastar::when_all_succeed(server->
                                         init(entity_name_t::OSD(6), "server4",
                                              7, addr),
                                         client->init(entity_name_t::OSD(7),
                                                      "client4", 8)
            ).then_unpack([server, client] {
                          client->send_pings(server->get_addr());
                          return seastar::sleep(100 ms);}
        ).then([client] {
               logger().info("client shutdown..."); return client->shutdown();}
        ).then([server] {
               logger().info("server shutdown..."); return server->shutdown();}
        ).then([] {
               logger().info("test_preemptive_shutdown() done!\n");}
        ).handle_exception([server, client] (auto eptr) {
                           logger().
                           error
                           ("test_preemptive_shutdown() failed: got exception {}",
                            eptr); throw;}
        );
    }
  ...
  _assert_eq(#actual, actual, #expected, expected)
        void assert_state_at(conn_state_t expected) const {
            ASSERT_EQUAL(state, expected);
                              ...
                              unsigned connects,
                              unsigned reconnects, unsigned dispatched) const {
            ...
            ASSERT_EQUAL(cnt_connect_dispatched, dispatched);
                             ...
                             unsigned accepts,
                             unsigned reaccepts, unsigned dispatched) const {
            ...
            logger().info("\nResult({}):\n"
                          ...
                          server_reconnect_attempts,
                          cnt_accept_dispatched,
                          cnt_reset_dispatched, cnt_remote_reset_dispatched);
    }};
    ...
    struct TestInterceptor:public Interceptor {
        ...
        seastar::future <> wait() {
            ...
            return seastar::sleep_abortable(10 s, *signal).then([] {
                                                                throw std::
                                                                runtime_error
                                                                ("Timeout (10s) in TestInterceptor::wait()");}
      ...
      private:
        ...
        void register_conn_closed(SocketConnection & conn) override {
            auto result =
                find_result(conn.get_local_shared_foreign_from_this());
            if (result == nullptr) {
            ...
            notify();
            logger().info("[{}] {} closed({})", result->index, conn,
                          result->state);
        }
        ...
        bp_action_t intercept(SocketConnection & conn, Breakpoint bp) override {
            ...
            if (bp == custom_bp_t::SOCKET_CONNECTING) {
                ...
                logger().info("[Test] connect_attempts={}",
                              result->connect_attempts);
            }
            else if (bp == tag_bp_t {
                ...
                logger().info("[Test] client_connect_attempts={}",
                              result->client_connect_attempts);
            }
            else if (bp == tag_bp_t {
                ...
                logger().info("[Test] client_reconnect_attempts={}",
                              result->client_reconnect_attempts);
            }
            else if (bp == custom_bp_t::SOCKET_ACCEPTED) {
                ++result->accept_attempts;
                logger().info("[Test] accept_attempts={}",
                              result->accept_attempts);
            }
            else if (bp == tag_bp_t {
                ...
                logger().info("[Test] server_connect_attemps={}",
                              result->server_connect_attempts);
            }
            else if (bp == tag_bp_t {
                ...
                logger().info("[Test] server_reconnect_attempts={}",
                              result->server_reconnect_attempts);
            }
            ...
            if (it_bp != breakpoints.end()) {
                auto it_cnt =
                    it_bp->second.find(breakpoints_counter[bp].counter);
    ...
    class FailoverSuite:public Dispatcher {
        ...
        std::optional < seastar::future <>> ms_dispatch(ConnectionRef c,
                                                        MessageRef m) override {
        ...
        void ms_handle_accept(ConnectionRef conn) override {
            ...
            if (tracked_conn &&
                !tracked_conn->is_closed() && tracked_conn != conn) {
                logger().
                    error
                    ("[{}] {} got accepted, but there's already traced_conn [{}] {}",
        ...
        void ms_handle_connect(ConnectionRef conn) override {
            ...
            if (tracked_conn != conn) {
                logger().
                    error
                    ("[{}] {} got connected, but doesn't match tracked_conn [{}] {}",
                     result->index, *conn, tracked_index, *tracked_conn);
                ceph_abort();
            }
            ceph_assert(result->index == tracked_index);
        ...
        void ms_handle_remote_reset(ConnectionRef conn) override {
            auto result = interceptor.find_result(conn);
            if (result == nullptr) {
                logger().error("Untracked remotely reset connection: {}",
                               *conn);
                ceph_abort();
            }
            ...
            if (tracked_conn != conn) {
                logger().
                    error
                    ("[{}] {} got remotely reset, but doesn't match tracked_conn [{}] {}",
                     result->index, *conn, tracked_index, *tracked_conn);
                ceph_abort();
            }
            ceph_assert(result->index == tracked_index);
            ...
            logger().
                info
                ("[Test] got remote reset (cnt_remote_reset_dispatched={}) -- [{}] {}",
      ...
      private:
        seastar::future <> init(entity_addr_t test_addr, SocketPolicy policy) {
            ...
            return test_msgr->bind(entity_addrvec_t {
                                   test_addr}
            ).safe_then([this] {
                        return test_msgr->start( {
                                                this}
                        );}
                        ,
                        Messenger::bind_ertr::
                        all_same_way([test_addr] (const std::error_code & e) {
                                     logger().
                                     error("FailoverSuite: "
                                           "there is another instance running at {}",
                                           test_addr); ceph_abort();}));
        }
        ...
        seastar::future <> send_op(bool expect_reply = true) {
            ...
            spg_t spgid(pgid);
            return tracked_conn->send(crimson::make_message < MOSDOp >
                                      (0, 0, hobj, spgid, 0, 0, 0));
        }
        ...
        seastar::future <> flush_pending_send() {
            ...
            return seastar::do_until([this] {
                                     return pending_send == 0;
                                     }
                                     ,[this] {
                                     --pending_send; return send_op();}
            );
        }
        ...
        seastar::future <> wait_ready(unsigned num_ready_conns,
          ...
          for (auto & result:interceptor.results) {
            ...
            if (num_ready_conns > 0) {
                ...
                else if (interceptor.results.size() < num_ready_conns
                         || pending_conns > 0) {
                    logger().
                        info("[Test] wait_ready(): wait for connections,"
            ...
            if (wait_received &&
                ...
                if (pending_conns || pending_establish) {
                    logger().
                        info("[Test] wait_ready(): wait for pending_send={},"
                             " pending_peer_receive={}, pending_receive={},"
                             " pending {}/{} ready/establish connections ...",
                             pending_send, pending_peer_receive,
                             pending_receive, pending_conns, pending_establish);
                    do_wait = true;
                }
            }
            if (num_replaced > 0) {
                ...
                if (replaced_conns < num_replaced) {
                    logger().
                        ...
                        ("[Test] wait_ready(): wait for {} replaced connections,"
                         " currently {} ...", num_replaced, replaced_conns);
                    do_wait = true;
                }
            }
            ...
            if (do_wait) {
                return interceptor.wait().
                    then([this, num_ready_conns, num_replaced, wait_received] {
                         return wait_ready(num_ready_conns, num_replaced,
                                           wait_received);}
                );
            }
            else {
      ...
      :    
        test_msgr(test_msgr),
        test_peer_addr(test_peer_addr), interceptor(interceptor) {
        }
        ...
        void dump_results() const {
            ...
            create(entity_addr_t test_addr,
                   ...
                   const TestInterceptor & interceptor) {
            auto suite =
                std::make_unique < FailoverSuite >
                (Messenger::
                 create(entity_name_t::OSD(TEST_OSD), "Test", TEST_NONCE),
                 test_peer_addr, interceptor);
            return suite->init(test_addr,
                               test_policy).then([suite =
                                                  std::move(suite)] ()mutable {
      ...
      public:
        seastar::future <> connect_peer() {
            ...
            auto conn =
                test_msgr->connect(test_peer_addr, entity_name_t::TYPE_OSD);
            auto result = interceptor.find_result(conn);
            ...
            if (tracked_conn) {
                if (tracked_conn->is_closed()) {
                    ...
                    logger().
                        info
                        ("[Test] this is a new session replacing an closed one");
                }
                else {
        ...
        seastar::future <> markdown() {
            ...
            return seastar::sleep(100 ms).then([this] {
    ...
    class FailoverTest:public Dispatcher {
      ...
      private:
        seastar::future <> prepare_cmd(cmd_t cmd,
        ...
        seastar::future <> init(entity_addr_t cmd_peer_addr) {
            ...
            return cmd_msgr->start( {
                                   this}
            ).then([this, cmd_peer_addr] {
                   logger().info("CmdCli connect to CmdSrv({}) ...",
                                 cmd_peer_addr);
                   cmd_conn =
                   cmd_msgr->connect(cmd_peer_addr, entity_name_t::TYPE_OSD);
                   return pingpong();}
            );
        }
      ...
      public:
        FailoverTest(MessengerRef cmd_msgr,
                     entity_addr_t test_addr, entity_addr_t test_peer_addr)
      :    
        cmd_msgr(cmd_msgr),
        test_addr(test_addr), test_peer_addr(test_peer_addr) {
        }
        ...
        seastar::future <> shutdown() {
            ...
            return cmd_conn->send(std::move(m)).then([] {
                   ...
                   cmd_msgr->stop(); return cmd_msgr->shutdown();}
            );
        }

        static seastar::future < seastar::lw_shared_ptr < FailoverTest >>
            create(entity_addr_t test_addr,
                   entity_addr_t cmd_peer_addr, entity_addr_t test_peer_addr) {
            auto test =
                seastar::make_lw_shared < FailoverTest >
                (Messenger::
                 create(entity_name_t::OSD(CMD_CLI_OSD), "CmdCli",
                        CMD_CLI_NONCE),
        ...
        // called by tests
      public:
        seastar::future <> run_suite(std::string name,
                                     const TestInterceptor & interceptor,
                                     policy_t test_policy,
                                     policy_t peer_policy,
                                     std::function <
                                     seastar::future <> (FailoverSuite &) >
                                     &&f) {
            logger().info("\n\n[{}]", name);
            ceph_assert(!test_suite);
            SocketPolicy test_policy_ = to_socket_policy(test_policy);
            return FailoverSuite::create(test_addr, test_policy_,
                                         test_peer_addr,
                                         interceptor).then([this, peer_policy,
                                                            f =
                                                            std::
                                                            move(f)] (auto
                                                                      suite)
        ...
        seastar::future <> markdown_peer() {
            ...
            // sleep to propagate potential remaining acks
            return seastar::sleep(50 ms).then([this] {
                                              return prepare_cmd(cmd_t::
                                                                 suite_markdown);}
            ).then([] {
    ...
    class FailoverSuitePeer:public Dispatcher {
      ...
      private:
        seastar::future <> init(entity_addr_t test_peer_addr,
                                SocketPolicy policy) {
            peer_msgr->set_default_policy(policy);
            peer_msgr->set_auth_client(&dummy_auth);
            peer_msgr->set_auth_server(&dummy_auth);
            return peer_msgr->bind(entity_addrvec_t {
                                   test_peer_addr}
            ).safe_then([this] {
                        return peer_msgr->start( {
                                                this}
                        );}
                        ,
                        Messenger::bind_ertr::
                        all_same_way([test_peer_addr]
                                     (const std::error_code & e) {
                                     logger().
                                     error("FailoverSuitePeer: "
                                           "there is another instance running at {}",
                                           test_peer_addr); ceph_abort();}));
        }
      ...
      :    peer_msgr(peer_msgr), op_callback(op_callback) {
        ...
        seastar::future <> connect_peer(entity_addr_t test_addr_decoded) {
            ...
            auto new_tracked_conn =
                peer_msgr->connect(test_addr_decoded, entity_name_t::TYPE_OSD);
            if (tracked_conn) {
                if (tracked_conn->is_closed()) {
        ...
        seastar::future <> send_peer() {
            if (tracked_conn) {
                ...
                ++pending_send;
                logger().info("[TestPeer] send_peer() (pending {})",
                              pending_send);
                return seastar::now();
            }
        }
        ...
        static seastar::future < std::unique_ptr < FailoverSuitePeer >>
            create(entity_addr_t test_peer_addr, const SocketPolicy & policy,
                   cb_t op_callback) {
            auto suite =
                std::make_unique < FailoverSuitePeer >
                (Messenger::create(entity_name_t::OSD(TEST_PEER_OSD),
                                   "TestPeer",
                                   TEST_PEER_NONCE),
                 op_callback);
            return suite->init(test_peer_addr,
                               policy).then([suite =
                                             std::move(suite)] ()mutable {
                                            return std::move(suite);}
            );
    ...
    class FailoverTestPeer:public Dispatcher {
        ...
        std::optional < seastar::future <>> ms_dispatch(ConnectionRef c,
            ...
            default:
                logger().error("{} got unexpected msg from cmd client: {}", *c,
                               *m);
                ceph_abort();
            }
            return {
      ...
      private:
        ...
        seastar::future <> init(entity_addr_t cmd_peer_addr) {
            ...
            cmd_msgr->set_auth_client(&dummy_auth);
            cmd_msgr->set_auth_server(&dummy_auth);
            return cmd_msgr->bind(entity_addrvec_t {
                                  cmd_peer_addr}
            ).safe_then([this] {
                        return cmd_msgr->start( {
                                               this}
                        ...
                        ,
                        Messenger::bind_ertr::
                        all_same_way([cmd_peer_addr] (const std::error_code & e)
                                     {
                                     logger().
                                     error("FailoverTestPeer: "
                                           "there is another instance running at {}",
                                           cmd_peer_addr); ceph_abort();}));
        }
      ...
      :    cmd_msgr(cmd_msgr), test_peer_addr(test_peer_addr) {
        ...
        static seastar::future < std::unique_ptr < FailoverTestPeer >>
            ...
            auto test_peer =
                std::make_unique < FailoverTestPeer >
                (Messenger::
                 create(entity_name_t::OSD(CMD_SRV_OSD), "CmdSrv",
                        CMD_SRV_NONCE),
                 test_peer_addr);
            return test_peer->init(cmd_peer_addr).
                then([test_peer = std::move(test_peer)] ()mutable {
                     logger().info("CmdSrv ready"); return std::move(test_peer);}
            );
        }
    };
    ...
    seastar::future <> test_v2_lossy_early_connect_fault(FailoverTest & test) {
        return seastar::do_with(std::vector < Breakpoint > {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            [&test] (auto bp) {
                                                            TestInterceptor
                                                            interceptor;
                                                            interceptor.
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_lossy_early_connect_fault -- {}",
                                                                       bp),
                                                                      interceptor,
                                                                      policy_t::
                                                                      lossy_client,
    ...
    seastar::future <> test_v2_lossy_early_accept_fault(FailoverTest & test) {
        return seastar::do_with(std::vector < Breakpoint > {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_lossy_early_accept_fault -- {}",
                                                                       bp),
                                                                      interceptor,
                                                                      policy_t::
                                                                      stateless_server,
    ...
    seastar::future <> test_v2_lossy_accept_fault(FailoverTest & test) {
        ...
        return test.
            run_suite(fmt::format("test_v2_lossy_accept_fault -- {}", bp),
                      interceptor, policy_t::stateless_server,
                      ...
                      return seastar::futurize_invoke([&test] {
                             ...
                             results[1].
                             assert_state_at(conn_state_t::established);
    ...
    seastar::future <> test_v2_lossy_establishing_fault(FailoverTest & test) {
        ...
        interceptor.make_fault(bp);
        return test.
            run_suite(fmt::format("test_v2_lossy_establishing_fault -- {}", bp),
                      interceptor, policy_t::stateless_server,
                      policy_t::lossy_client,[&test] (FailoverSuite & suite) {
                      return seastar::futurize_invoke([&test] {
                                                      return test.
    ...
    seastar::future <> test_v2_lossless_connect_fault(FailoverTest & test) {
        return seastar::do_with(std::vector < Breakpoint > {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_lossless_connect_fault -- {}",
                                                                       bp),
                                                                      interceptor,
                                                                      policy_t::
                                                                      lossless_client,
    ...
    seastar::future <> test_v2_lossless_connected_fault2(FailoverTest & test) {
        return seastar::do_with(std::vector < Breakpoint > {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_lossless_connected_fault2 -- {}",
                                                                       bp),
                                                                      interceptor,
                                                                      policy_t::
                                                                      lossless_client,
    ...
    seastar::future <> test_v2_lossless_reconnect_fault(FailoverTest & test) {
        return seastar::do_with(std::vector < std::pair < Breakpoint,
                                Breakpoint >> {
                                { {
                                Tag::MESSAGE, bp_type_t::WRITE}
                                , {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_lossless_reconnect_fault -- {}, {}",
                                                                       bp_pair.
                                                                       first,
                                                                       bp_pair.
                                                                       second),
    ...
    seastar::future <> test_v2_lossless_accept_fault(FailoverTest & test) {
        ...
        interceptor.make_fault(bp);
        return test.
            run_suite(fmt::format("test_v2_lossless_accept_fault -- {}", bp),
                      interceptor, policy_t::stateful_server,
    ...
    seastar::future <> test_v2_lossless_establishing_fault(FailoverTest & test) {
        ...
        return test.
            run_suite(fmt::
                      format("test_v2_lossless_establishing_fault -- {}", bp),
                      interceptor, policy_t::stateful_server,
                      policy_t::lossless_client,
                      [&test] (FailoverSuite & suite) {
                      return seastar::futurize_invoke([&test] {
    ...
    seastar::future <> test_v2_lossless_reaccept_fault(FailoverTest & test) {
        return seastar::do_with(std::vector < std::pair < Breakpoint,
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_lossless_reaccept_fault -- {}, {}",
                                                                       bp_pair.
                                                                       first,
                                                                       bp_pair.
                                                                       second),
    ...
    seastar::future <> test_v2_peer_accept_fault(FailoverTest & test) {
        ...
        interceptor.make_fault(bp);
        return test.
            run_suite(fmt::format("test_v2_peer_accept_fault -- {}", bp),
                      interceptor, policy_t::lossless_peer,
                      policy_t::lossless_peer,[&test] (FailoverSuite & suite) {
                      return seastar::futurize_invoke([&test] {
                                                      return test.
                                                      peer_send_me();}
                      ).then([&test] {
    ...
    seastar::future <> test_v2_peer_establishing_fault(FailoverTest & test) {
        ...
        interceptor.make_fault(bp);
        return test.
            run_suite(fmt::format("test_v2_peer_establishing_fault -- {}", bp),
                      interceptor, policy_t::lossless_peer,
                      policy_t::lossless_peer,[&test] (FailoverSuite & suite) {
                      return seastar::futurize_invoke([&test] {
                                                      return test.
    ...
    seastar::future <>
        ...
        return test.
            run_suite(fmt::
                      format("test_v2_peer_connected_fault_reconnect -- {}",
                             bp), interceptor, policy_t::lossless_peer,
                      policy_t::lossless_peer,[](FailoverSuite & suite) {
    ...
    seastar::future <>
        ...
        return test.
            run_suite(fmt::
                      format("test_v2_peer_connected_fault_reaccept -- {}", bp),
                      interceptor, policy_t::lossless_peer,
                      policy_t::lossless_peer,[&test] (FailoverSuite & suite) {
                      return seastar::futurize_invoke([&test] {
                                                      return test.
    ...
    seastar::future <>
        ...
        return seastar::do_with(std::vector < std::pair < unsigned,
                                Breakpoint >> {
                                {
                                1, {
                                Tag::SESSION_RECONNECT, bp_type_t::READ}}, {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_racing_reconnect_acceptor_lose -- {}({})",
                                                                       bp.
                                                                       second,
                                                                       bp.
                                                                       first),
    ...
    seastar::future <>
        ...
        return seastar::do_with(std::vector < std::pair < unsigned,
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_racing_reconnect_acceptor_win -- {}({})",
                                                                       bp.
                                                                       second,
                                                                       bp.
                                                                       first),
    ...
    seastar::future <> test_v2_racing_connect_acceptor_lose(FailoverTest & test) {
        return seastar::do_with(std::vector < Breakpoint > {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_racing_connect_acceptor_lose -- {}",
                                                                       bp),
                                                                      interceptor,
                                                                      policy_t::
                                                                      lossless_peer,
    ...
    seastar::future <> test_v2_racing_connect_acceptor_win(FailoverTest & test) {
        return seastar::do_with(std::vector < Breakpoint > {
                                ...
                                return seastar::do_for_each(failure_cases,
                                                            ...
                                                            run_suite(fmt::
                                                                      format
                                                                      ("test_v2_racing_connect_acceptor_win -- {}",
                                                                       bp),
                                                                      interceptor,
                                                                      policy_t::
                                                                      lossless_peer,
    ...
    seastar::future <>
        ...
        TestInterceptor interceptor;
        interceptor.make_fault( {
                               Tag::SERVER_IDENT, bp_type_t::READ}
        );
        interceptor.make_block( {
                               Tag::CLIENT_IDENT, bp_type_t::WRITE}
                               , 2);
        return test.run_suite("test_v2_racing_connect_reconnect_lose",
                              ...
                              return seastar::futurize_invoke([&suite] {
                                     ...
                                     results[0].assert_reset(0, 0);
                                     results[1].
                                     assert_state_at(conn_state_t::replaced);
                                     results[1].assert_connect(0, 0, 0, 0);
    ...
    seastar::future <> test_v2_racing_connect_reconnect_win(FailoverTest & test) {
        ...
        return test.run_suite("test_v2_racing_connect_reconnect_win",
                              ...
                              return seastar::futurize_invoke([&test] {
                                     ...
                                     results[0].assert_reset(0, 0);
                                     results[1].
                                     assert_state_at(conn_state_t::closed);
                                     results[1].assert_connect(0, 0, 0, 0);
                                     results[1].assert_accept(1, 0, 1, 0);
                                     results[1].assert_reset(0, 0);}
                              );}
    ...
    seastar::future <> test_v2_stale_establishing(FailoverTest & test) {
        ...
        interceptor.make_stall(bp);
        return test.
            run_suite(fmt::format("test_v2_stale_establishing -- {}", bp),
                      interceptor, policy_t::lossless_peer,
                      policy_t::lossless_peer,[&test] (FailoverSuite & suite) {
                      return seastar::futurize_invoke([&test] {
                                                      return test.
                             ...
                             suite.unblock(); return suite.wait_results(2);}
                      ).then([](ConnResults & results) {
                             results[0].
                             assert_state_at(conn_state_t::established);
                             results[0].assert_connect(0, 0, 0, 0);
    ...
    seastar::future <> test_v2_stale_reaccept(FailoverTest & test) {
        ...
        return test.run_suite(fmt::format("test_v2_stale_reaccept -- {}", bp),
                              ...
                              return seastar::futurize_invoke([&test] {
                                     ...
                                     logger().
                                     info
                                     ("[Test] block the broken REPLACING for 210ms...");
                                     return seastar::sleep(210 ms);}
                              ).then([&suite] {
                                     suite.unblock();
    ...
    seastar::future <> test_v2_peer_reuse_connector(FailoverTest & test) {
        return test.run_suite("test_v2_peer_reuse_connector",
                              ...
                              return seastar::futurize_invoke([&suite] {
                                     ...
                                     logger().info("-- 2 --");
                                     logger().
                                     info("[Test] acceptor markdown...");
                                     return test.markdown_peer();}
                              ).then([&suite] {
                                     ceph_assert(suite.is_standby());
                                     logger().info("-- 3 --");
                                     logger().
                                     info("[Test] connector reconnect...");
                                     return suite.connect_peer();}
                              ).then([&suite] {
    ...
    seastar::future <>
        ...
        if (test_peer_islocal) {
            ...
            logger().info("test_v2_protocol: start local TestPeer at {}...",
                          cmd_peer_addr);
            return FailoverTestPeer::create(cmd_peer_addr,
                                            test_peer_addr).then([test_addr,
                                                                  cmd_peer_addr,
                                                                  test_peer_addr,
                                                                  peer_wins]
                                                                 (auto peer) {
                               ...
                               logger().
                               error
                               ("FailoverTestPeer failed: got exception {}",
                                eptr); throw;}
            );
        }

        return FailoverTest::create(test_addr, cmd_peer_addr,
                                    test_peer_addr).
            then([peer_wins] (auto test) {
                 return seastar::futurize_invoke([test] {
                        ...
                        return seastar::futurize_invoke([test] {
                               ...
                               return
                               test_v2_racing_reconnect_acceptor_win(*test);}
                        );}
                        else {
                        return seastar::futurize_invoke([test] {
                                                        return
                               ...
                               return
                               test_v2_racing_reconnect_acceptor_lose(*test);}
                        );}
                        }
                 ).then([test] {
            ...
            .handle_exception([](auto eptr) {
                              logger().
                              error("FailoverTest failed: got exception {}",
                                    eptr); throw;}
        );
    }
...
seastar::future < int >do_test(seastar::app_template & app)
{
    ...
    return crimson::common::sharded_conf().start(init_params.name, cluster)
        .then([conf_file_list] {
           ...
           ceph_assert(test_addr.
                       parse(config["test-addr"].as < std::string > ().c_str(),
                             nullptr)); test_addr.set_nonce(TEST_NONCE);
           entity_addr_t cmd_peer_addr;
           ceph_assert(cmd_peer_addr.
                       parse(config["testpeer-addr"].as < std::string >
                             ().c_str(), nullptr));
           ...
           ("test configuration: verbose={}, rounds={}, keepalive_ratio={}, "
            ...
            "testpeer_islocal={}, peer_wins={}", verbose, rounds,
            keepalive_ratio, test_addr, cmd_peer_addr, test_peer_addr,
           ...
           then([test_addr, cmd_peer_addr, test_peer_addr, testpeer_islocal,
                 peer_wins] {
                return test_v2_protocol(test_addr, cmd_peer_addr,
                                        test_peer_addr, testpeer_islocal,
                                        peer_wins);}
           ).then([] {
...
int main(int argc, char **argv)
{
    ...
    return app.run(argc, argv,[&app] {
                   ...
                   return seastar::with_timeout(seastar::lowres_clock::now() +
                                                300 s, do_test(app))
