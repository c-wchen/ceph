...
// class can be added to handle cache and 'real' ops.
    class MotrMetaCache {
      ...
      public:
        ...
        int get(const DoutPrefixProvider * dpp, const std::string & name,
                bufferlist & data);
        ...
        int watch_cb(const DoutPrefixProvider * dpp,
                     uint64_t notify_id,
                     uint64_t cookie, uint64_t notifier_id, bufferlist & bl);
    ...
    class MotrNotification:public StoreNotification {
      public:
        MotrNotification(Object * _obj, Object * _src_obj,
                         rgw::notify::EventType _type): StoreNotification(_obj,
                                                                          _src_obj,
                                                                          _type)
        {
        ...
        virtual int publish_commit(const DoutPrefixProvider * dpp,
                                   uint64_t size, const ceph::real_time & mtime,
    ...
    class MotrUser:public StoreUser {
      ...
      public:
        ...
        MotrUser(MotrStore * _st, const RGWUserInfo & _i):StoreUser(_i),
            store(_st) {
        }
      MotrUser(MotrStore * _st):store(_st) {
        ...
        virtual int read_stats(const DoutPrefixProvider * dpp, optional_yield y,
                               ...
                               nullptr, ceph::real_time * last_stats_update =
                               nullptr) override;
        ...
        int load_user_from_idx(const DoutPrefixProvider * dpp,
                               MotrStore * store, RGWUserInfo & info,
    ...
    class MotrBucket:public StoreBucket {
      ...
      :    StoreBucket(_u), store(_st), acls() {
        ...
        virtual int read_stats(const DoutPrefixProvider * dpp,
                               ...
                               nullptr, bool * syncstopped = nullptr) override;
        virtual int read_stats_async(const DoutPrefixProvider * dpp,
                                     const bucket_index_layout_generation &
                                     idx_layout, int shard_id,
        ...
        virtual int check_index(const DoutPrefixProvider * dpp,
                                ...
                                std::map < RGWObjCategory,
                                RGWStorageStats > &calculated_stats) override;
    ...
    class MotrPlacementTier:public StorePlacementTier {
      ...
      public:
        MotrPlacementTier(MotrStore * _store,
  ...
  /** Get the endpoint from zonegroup, or from master zone if not set */
    ...
    class MotrZone:public StoreZone {
      ...
      public:
        ...
        } MotrZone(MotrStore * _store, MotrZoneGroup _zg):store(_store),
            zonegroup(_zg) {
            realm = new RGWRealm();
        ...
        virtual const std::string & get_realm_name() {
            return realm->get_name();
        }
        virtual const std::string & get_realm_id() {
            return realm->get_id();
        }
        virtual const std::string_view get_tier_type() {
            return "rgw";
        }
    ...
    class MotrObject:public StoreObject {
      ...
      public:
        ...
        virtual int dump_obj_layout(const DoutPrefixProvider * dpp,
                                    optional_yield y, Formatter * f) override;
      ...
      public:
        ...
        void close_mobj();
        int write_mobj(const DoutPrefixProvider * dpp, bufferlist
                       && data, uint64_t offset);
        int read_mobj(const DoutPrefixProvider * dpp, int64_t off, int64_t end,
                      RGWGetDataCB * cb);
        ...
        int open_part_objs(const DoutPrefixProvider * dpp, std::map < int,
                           std::unique_ptr < MotrObject >> &part_objs);
        int read_multipart_obj(const DoutPrefixProvider * dpp, int64_t off,
                               int64_t end, RGWGetDataCB * cb, std::map < int,
                               std::unique_ptr < MotrObject >> &part_objs);
        int delete_part_objs(const DoutPrefixProvider * dpp);
        void set_category(RGWObjCategory _category) {
            category = _category;
        }
        int get_bucket_dir_ent(const DoutPrefixProvider * dpp,
...
// TODO: implement it using Motr object locks.
    class MPMotrSerializer:public StoreMPSerializer {
      ...
      public:
        MPMotrSerializer(const DoutPrefixProvider * dpp, MotrStore * store,
                         MotrObject * obj, const std::string & lock_name) {
    ...
    class MotrMultipartWriter:public StoreWriter {
      ...
      public:
        MotrMultipartWriter(const DoutPrefixProvider * dpp,
                            ...
                            const rgw_placement_rule * ptail_placement_rule,
                            uint64_t _part_num,
                            const std::string & part_num_str):StoreWriter(dpp,
                                                                          y),
            store(_store), head_obj(obj), part_num(_part_num),
...
//
    ...
    class MotrMultipartPart:public StoreMultipartPart {
      ...
      public:
        ...
        MotrMultipartPart(RGWUploadPartInfo _info,
                          MotrObject::Meta _meta):info(_info), meta(_meta) {
    ...
    class MotrMultipartUpload:public StoreMultipartUpload {
      ...
      public:
        MotrMultipartUpload(MotrStore * _store, Bucket * _bucket,
                            const std::string & oid,
                            std::optional < std::string > upload_id,
                            ACLOwner _owner,
                            ceph::
                            real_time _mtime):StoreMultipartUpload(_bucket),
            store(_store), mp_obj(oid, upload_id), owner(_owner),
            mtime(_mtime) {
    ...
    class MotrStore:public StoreDriver {
      ...
      MotrStore(CephContext * c):zone(this), cctx(c) {
        ...
        virtual std::unique_ptr < Notification >
            get_notification(rgw::sal::Object * obj, rgw::sal::Object * src_obj,
        ...
        virtual int get_oidc_providers(const DoutPrefixProvider * dpp,
                                       const std::string & tenant,
                                       std::vector < std::unique_ptr <
                                       RGWOIDCProvider >> &providers) override;
        virtual std::unique_ptr < Writer >
            get_append_writer(const DoutPrefixProvider * dpp, optional_yield y,
                              rgw::sal::Object * obj, const rgw_user & owner,
                              const rgw_placement_rule * ptail_placement_rule,
                              const std::string & unique_tag, uint64_t position,
                              uint64_t * cur_accounted_size) override;
        virtual std::unique_ptr < Writer >
            get_atomic_writer(const DoutPrefixProvider * dpp, optional_yield y,
                              rgw::sal::Object * obj, const rgw_user & owner,
                              const rgw_placement_rule * ptail_placement_rule,
                              uint64_t olh_epoch,
        ...
        int store_access_key(const DoutPrefixProvider * dpp, optional_yield y,
                             MotrAccessKey access_key);
        int delete_access_key(const DoutPrefixProvider * dpp, optional_yield y,
                              std::string access_key);
        int store_email_info(const DoutPrefixProvider * dpp, optional_yield y,
                             MotrEmailInfo & email_info);

        int init_metadata_cache(const DoutPrefixProvider * dpp,
                                CephContext * cct);
        MotrMetaCache *get_obj_meta_cache() {
            return obj_meta_cache;
        }
        MotrMetaCache *get_user_cache() {
            return user_cache;
        }
        MotrMetaCache *get_bucket_inst_cache() {
    ...
    struct obj_time_weight {
        ...
        void init(const real_time & _mtime, uint32_t _short_id,
                  uint64_t _pg_ver) {
            mtime = _mtime;
