    ...
    namespace action {
        namespace info {
            ...
            static int do_show_info(librados::IoCtx & io_ctx,
                ...
                std::string parent_pool, parent_name, parent_id,
                    parent_snapname;
                if ((image.
                     parent_info2(&parent_pool, &parent_name, &parent_id,
                                  &parent_snapname) == 0)
                    && parent_name.length() > 0) {

                    librbd::trash_image_info_t trash_image_info;
                    librbd::RBD rbd;
                    r = rbd.trash_get(io_ctx, parent_id.c_str(),
                                      &trash_image_info);
                    ...
                    if (f) {
                        ...
                        std::
                            cout << "\tparent: " << parent_pool << "/" <<
                            parent_name << "@" << parent_snapname;
                        if (trash_image_info_valid) {
            ...
            int execute(const po::variables_map & vm) {
                ...
                bool has_image_spec =
                    utils::check_if_image_spec_present(vm,
                                                       at::
                ...
                librbd::Image image;
                r = utils::init_and_open_image(pool_name, image_name, image_id,
