...
public:
        ...
        inotablev(0), fin(fin_)
    {
                           ...
                           const interval_set < inodeno_t > &to_free, version_t iv, const interval_set < inodeno_t > &to_purge, LogSegment *_ls,
...
void Server::dispatch_client_request(MDRequestRef &mdr)
{
    ...
    if (is_full) {
        ...
        if (req->get_op() == CEPH_MDS_OP_SETLAYOUT || req->get_op() == CEPH_MDS_OP_SETDIRLAYOUT ||
            ...
            req->get_op() == CEPH_MDS_OP_MKSNAP || ((req->get_op() == CEPH_MDS_OP_LINK || req->get_op() == CEPH_MDS_OP_RENAME) &&
...
xattr_handlers[] = {
    {
xattr_name:
Server::DEFAULT_HANDLER, description: "default xattr handler"
, validate:
&Server::default_xattr_validate, setxattr:
&Server::default_setxattr_handler, removexattr:
...
, validate:
&Server::mirror_info_xattr_validate, setxattr:
&Server::mirror_info_setxattr_handler, removexattr:
...
handle_client_getvxattr(MDRequestRef &mdr)
{
    ...
    CInode *cur =
        rdlock_path_pin_ref(mdr, true, false);
    if (!cur) {
        return;
    }
...
handle_client_mkdir(MDRequestRef &mdr)
{
    ...
    mdr->disable_lock_cache();
    CDentry *dn =
        rdlock_path_xlock_dentry(mdr, true);
    if (!dn) {
        return;
    }
    if (is_unlink_pending(dn)) {
                               ...
                               MDRequestRef &mdr)
{
    ...
    ceph_assert(mdr || mds->is_resolve());
    CDir *dir =
        mdcache->get_dirfrag(rollback.src_dir);
    if (!dir)
        dir =
...
handle_client_rename(MDRequestRef &mdr)
{
    ...
    if (destpath.get_ino() != srcpath.get_ino() && !(req->get_source().is_mds() &&
                             ...
                             CDentry *straydn)
{
    ...
    if (oldin && oldin->is_dir() && straydn
        && !straydn->is_auth())
        force_journal_stray =
            _need_force_journal(oldin, true);
    if (linkmerge)
        dout(10) <<
...
handle_peer_rename_prep(MDRequestRef &mdr)
{
    ...
    // journal.
    mdr->ls = mdlog->get_current_segment();
    EPeerUpdate *le = new EPeerUpdate(mdlog, "peer_rename_prep", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_PREPARE,
                                      EPeerUpdate::RENAME);
    mdlog->start_entry(le);
    le->rollback = mdr->more()->rollback_bl;
                    ...
                    CDentry *straydn)
{
    ...
    if (srcdn->is_auth() && srcdnl->is_primary()) {
        ...
        if (reply) {
            ...
            bufferlist inodebl;
            mdcache->migrator->encode_export_inode(srcdnl->get_inode(), inodebl, exported_client_map, exported_client_metadata_map);
