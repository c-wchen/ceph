...
class DeviceList
{
    ...
    Device **devices;
public:
    explicit DeviceList(CephContext *cct): device_list(nullptr),
        device_context_list(nullptr), num(0), devices(nullptr)
    {
...
public:
    ...
    public:
        ...
        {
        public:
            Chunk(ibv_mr *m, uint32_t bytes, char *buffer, uint32_t offset =
                      0, uint32_t bound = 0, uint32_t lkey = 0, QueuePair *qp =
                      nullptr);
            ~Chunk();
        ...
        bool is_tx_buffer(const char *c)
        {
            return send->is_my_buffer(c);
        }
        bool is_valid_chunk(const Chunk *c)
        {
...
public:
    ...
    public:
        ...
        {
            return channel->fd;
        } ibv_comp_channel *get_channel()
        {
            return channel;
        }
        void bind_cq(ibv_cq *c)
        {
            cq = c;
        }
        void ack_events();
    ...
    public:
        ...
        CompletionChannel *get_cc() const
        {
            return channel;
        } private:
        CephContext *cct;
    ...
    public:
        ...
        QueuePair(CephContext *c, Infiniband &infiniband, ibv_qp_type type,
                  ...
                  struct rdma_cm_id *cid, uint32_t q_key = 0);
        ~QueuePair();
         ...
         */
        uint32_t get_initial_psn() const
        {
            return initial_psn;
        };
        /**
         ...
         */
        uint32_t get_local_qp_number() const
        {
            return qp->qp_num;
        };
        /**
        ...
        {
            return dead;
        } ib_cm_meta_t &get_peer_cm_meta()
        {
            return peer_cm_meta;
        } ib_cm_meta_t &get_local_cm_meta()
        {
...
public:
    ...
    typedef MemoryManager::Chunk Chunk;
    QueuePair *create_queue_pair(CephContext *c, CompletionQueue *,
                                 CompletionQueue *, ibv_qp_type type,
                                 struct rdma_cm_id *cm_id);
    ...
    CompletionQueue *create_comp_queue(CephContext *c, CompletionChannel *cc =
                                           NULL);
    ...
    uint32_t get_rx_queue_len() const
    {
        return rx_queue_len;
    }
};
