...
int RGWPeriod::get_zonegroup(RGWZoneGroup &zonegroup,
                             const string &zonegroup_id) const const
{
...
int RGWPeriod::get_latest_epoch(const DoutPrefixProvider *dpp,
                                epoch_t &latest_epoch, optional_yield y)
{
...
int RGWPeriod::add_zonegroup(const DoutPrefixProvider *dpp,
...
int RGWPeriod::update(const DoutPrefixProvider *dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  0) << "ERROR: failed to list zonegroups: " <<
                     cpp_strerror(-ret) << dendl;
        return ret;
    }
...
static int read_sync_status(const DoutPrefixProvider *dpp,
                            ...
                            rgw_meta_sync_status *sync_status)
{
    ...
    RGWMetaSyncStatusManager mgr(rados_store,
                                 rados_store->svc()->rados->
                                 get_async_processor());
    int r = mgr.init(dpp);
    if (r < 0) {
        return r;
                      ...
                      bool force_if_stale)
{
    ...
    if (master_zone != current_period.get_master_zone()) {
        ...
        int r =
            update_sync_status(dpp, driver, current_period, error_stream,
                               force_if_stale);
        if (r < 0) {
