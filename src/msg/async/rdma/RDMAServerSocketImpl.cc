...
RDMAServerSocketImpl::RDMAServerSocketImpl(CephContext * cct, Infiniband * i,
                                           RDMADispatcher * s, RDMAWorker * w,
                                           entity_addr_t & a)
:  cct(cct), net(cct), server_setup_socket(-1), infiniband(i), dispatcher(s),
worker(w), sa(a)
{
}
...
int RDMAServerSocketImpl::listen(entity_addr_t & sa, const SocketOptions & opt)
{
    ...
    rc = net.set_socket_options(server_setup_socket, opt.nodelay,
                                opt.rcbuf_size);
    if (rc < 0) {
        goto err;
    }
                                 ...
                                 Worker * w)
{
    ...
    server =
        new RDMAConnectedSocketImpl(cct, infiniband, dispatcher,
                                    dynamic_cast < RDMAWorker * >(w));
