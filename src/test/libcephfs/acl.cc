...
static int check_acl_and_mode(const void *buf, size_t size, mode_t mode)
{
    ...
    const acl_ea_header *header =
        reinterpret_cast < const acl_ea_header * >(buf);
...
TEST(ACL, SetACL)
{
    ...
    ASSERT_EQ(ceph_fsetxattr
              (cmount, fd, ACL_EA_DEFAULT, acl_buf, acl_buf_size, 0),
              -CEPHFS_EACCES);
    ASSERT_EQ(ceph_fsetxattr
              (cmount, fd, ACL_EA_ACCESS, acl_buf, acl_buf_size, 0), 0);
    ...
    ASSERT_EQ(ceph_fsetxattr
              (cmount, fd, ACL_EA_ACCESS, acl_buf, acl_buf_size, 0), 0);
    // ACL was deleted
    ASSERT_EQ(ceph_fgetxattr(cmount, fd, ACL_EA_ACCESS, NULL, 0),
              -CEPHFS_ENODATA);
...
TEST(ACL, Chmod)
{
    ...
    ASSERT_EQ(ceph_fgetxattr(cmount, fd, ACL_EA_ACCESS, acl_buf, acl_buf_size),
              acl_buf_size);
    ASSERT_EQ(check_acl_and_mode(acl_buf, acl_buf_size, stx.stx_mode), 0);
...
TEST(ACL, DefaultACL)
{
    ...
    // inherit default acl
    ASSERT_EQ(ceph_getxattr
              (cmount, test_dir2, ACL_EA_DEFAULT, acl2_buf, acl_buf_size),
              acl_buf_size);
    ASSERT_EQ(memcmp(acl1_buf, acl2_buf, acl_buf_size), 0);

    // mode and ACL are updated
    ASSERT_EQ(ceph_getxattr
              (cmount, test_dir2, ACL_EA_ACCESS, acl2_buf, acl_buf_size),
              acl_buf_size);
    {
        struct ceph_statx stx;
    ...
    // no default acl
    ASSERT_EQ(ceph_fgetxattr(cmount, fd, ACL_EA_DEFAULT, NULL, 0),
              -CEPHFS_ENODATA);

    // mode and ACL are updated
    ASSERT_EQ(ceph_fgetxattr(cmount, fd, ACL_EA_ACCESS, acl2_buf, acl_buf_size),
              acl_buf_size);
    {
        struct ceph_statx stx;
...
TEST(ACL, Disabled)
{
    ...
    ASSERT_EQ(ceph_setxattr
              (cmount, test_dir, ACL_EA_DEFAULT, acl_buf, acl_buf_size, 0),
              -CEPHFS_EOPNOTSUPP);
    ASSERT_EQ(ceph_setxattr
              (cmount, test_dir, ACL_EA_ACCESS, acl_buf, acl_buf_size, 0),
              -CEPHFS_EOPNOTSUPP);
    ASSERT_EQ(ceph_getxattr
              (cmount, test_dir, ACL_EA_DEFAULT, acl_buf, acl_buf_size),
              -CEPHFS_EOPNOTSUPP);
    ASSERT_EQ(ceph_getxattr
              (cmount, test_dir, ACL_EA_ACCESS, acl_buf, acl_buf_size),
              -CEPHFS_EOPNOTSUPP);
...
TEST(ACL, SnapdirACL)
{
    ...
    // inherit default acl
    ASSERT_EQ(ceph_getxattr
              (cmount, test_dir2_snapdir, ACL_EA_DEFAULT, acl3_buf,
               acl_buf_size), acl_buf_size);
    ...
    ASSERT_EQ(ceph_getxattr
              ...
              acl_buf_size);
    ASSERT_EQ(ceph_getxattr
              (cmount, test_dir2_snapdir, ACL_EA_ACCESS, acl3_buf,
               acl_buf_size), acl_buf_size);
    ASSERT_EQ(memcmp(acl2_buf, acl3_buf, acl_buf_size), 0);
