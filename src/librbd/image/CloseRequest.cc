...
namespace librbd {
    namespace image {
            ...
            m_image_ctx->
                flush(create_async_context_callback
                      (*m_image_ctx,
                       ...
                       &CloseRequest < I >::handle_flush > (this)));
        }
        ...
        template < typename I > void CloseRequest < I >::send_shut_down_cache() {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << this << " " << __func__ << dendl;
        ...
        template < typename I >
            ...
            save_result(r);
            if (r < 0) {
                lderr(cct) << "failed to shut down cache: " << cpp_strerror(r)
                    << dendl;
            }
