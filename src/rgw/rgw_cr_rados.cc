...
RGWAsyncRadosProcessor::RGWAsyncRadosProcessor(RGWRados * _store,
                                               int num_threads)
:store(_store), m_tp(store->ctx(), "RGWAsyncRadosProcessor::m_tp",
                     "rados_async", num_threads), req_throttle(store->ctx(),
                                                               "rgw_async_rados_ops",
                                                               num_threads * 2),
...
RGWAsyncGetSystemObj::RGWAsyncGetSystemObj(RGWCoroutine * caller,
                                           RGWAioCompletionNotifier * cn,
                                           RGWRados * _store,
                                           RGWObjVersionTracker * _objv_tracker,
                                           const rgw_raw_obj & _obj, off_t _ofs,
                                           off_t _end, bool want_attrs)
:  RGWAsyncRadosRequest(caller, cn), store(_store), obj_ctx(_store), obj(_obj),
ofs(_ofs), end(_end), want_attrs(want_attrs)
{
    if (_objv_tracker) {
...
RGWAsyncPutSystemObj::RGWAsyncPutSystemObj(RGWCoroutine * caller,
                                           RGWAioCompletionNotifier * cn,
                                           RGWRados * _store,
                                           RGWObjVersionTracker * _objv_tracker,
                                           const rgw_raw_obj & _obj,
                                           bool _exclusive, bufferlist _bl)
:  RGWAsyncRadosRequest(caller, cn), store(_store), obj(_obj),
...
{
    return store->system_obj_set_attrs(nullptr, obj, attrs, nullptr,
                                       &objv_tracker);
}

RGWAsyncPutSystemObjAttrs::RGWAsyncPutSystemObjAttrs(RGWCoroutine * caller,
...
int RGWAsyncLockSystemObj::_send_request()
{
    ...
    if (r < 0) {
        lderr(store->
              ctx()) << "ERROR: failed to get ref for (" << obj << ") ret=" << r
            << dendl;
        return r;
    }
...
RGWAsyncLockSystemObj::RGWAsyncLockSystemObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn, RGWRados * _store, RGWObjVersionTracker * _objv_tracker, const rgw_raw_obj & _obj, const string & _name, const string & _cookie, uint32_t _duration_secs):RGWAsyncRadosRequest(caller, cn), store(_store),
obj(_obj),
...
RGWAsyncUnlockSystemObj::RGWAsyncUnlockSystemObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn, RGWRados * _store, RGWObjVersionTracker * _objv_tracker, const rgw_raw_obj & _obj, const string & _name, const string & _cookie):RGWAsyncRadosRequest(caller, cn), store(_store),
obj(_obj),
...
RGWRadosGetOmapKeysCR::RGWRadosGetOmapKeysCR(RGWRados * _store, const rgw_raw_obj & _obj, const string & _marker, std::set < std::string > *_entries, int _max_entries):RGWSimpleCoroutine(_store->ctx()),
store(_store),
marker(_marker),
entries(_entries), max_entries(_max_entries), obj(_obj), cn(NULL)
{
...
RGWRadosRemoveOmapKeysCR::RGWRadosRemoveOmapKeysCR(RGWRados * _store, const rgw_raw_obj & _obj, const set < string > &_keys):RGWSimpleCoroutine(_store->ctx()),
store(_store),
...
RGWSimpleRadosLockCR::RGWSimpleRadosLockCR(RGWAsyncRadosProcessor * _async_rados, RGWRados * _store, const rgw_raw_obj & _obj, const string & _lock_name, const string & _cookie, uint32_t _duration):RGWSimpleCoroutine(_store->ctx()),
...
RGWSimpleRadosUnlockCR::RGWSimpleRadosUnlockCR(RGWAsyncRadosProcessor * _async_rados, RGWRados * _store, const rgw_raw_obj & _obj, const string & _lock_name, const string & _cookie):RGWSimpleCoroutine(_store->ctx()),
async_rados(_async_rados),
...
int RGWAsyncGetBucketInstanceInfo::_send_request()
{
    ...
    int r =
        store->get_bucket_instance_from_oid(obj_ctx, oid, bucket_info, NULL,
                                            NULL);
    if (r < 0) {
        ldout(store->ctx(),
              0) << "ERROR: failed to get bucket instance info for " << oid <<
            dendl;
        return r;
    }
...
RGWRadosTimelogAddCR::RGWRadosTimelogAddCR(RGWRados * _store, const string & _oid, const cls_log_entry & entry):RGWSimpleCoroutine(_store->ctx()),
store(_store),
oid(_oid), cn(NULL)
{
    stringstream & s = set_description();
...
:  RGWRadosTimelogTrimCR(store, oid, real_time
                          {
                          }, real_time {
                          }, std::string {
                          }, to_marker),
...
RGWStatObjCR::RGWStatObjCR(RGWAsyncRadosProcessor * async_rados,
...
bucket_info(_bucket_info), obj(obj), psize(psize), pmtime(pmtime),
pepoch(pepoch), objv_tracker(objv_tracker)
{
}
