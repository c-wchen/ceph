...
public:
    ...
    std::map < std::pair < int64_t, std::string >,
        librados::TestIoCtxImpl * >io_ctxs;
    ...
    } librados::TestIoCtxImpl *get_io_ctx(const IOContext &ioc)
    {
        ...
        for (auto &pool : pools) {
            if (pool.first == pool_id) {
                auto io_ctx =
                    test_rados_client->create_ioctx(pool_id,
                                                    pool.second);
...
librados::AioCompletionImpl *create_aio_completion(std::unique_ptr <
        Op::Completion >
        &&c)
{
...
int save_operation_ec(int result, boost::system::error_code *ec)
{
    if (ec != NULL) {
        *ec = {
            std::abs(result), bs::system_category()
        };
    }
    return result;
}
...
Op::Op()
{
    static_assert(Op::impl_size >=
                  sizeof(librados::TestObjectOperationImpl *));
    auto &o =
        *reinterpret_cast < librados::TestObjectOperationImpl ** >(&impl);
...
void Op::assert_exists()
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::assert_exists, _1, _2,
                   _4));
}
...
void Op::assert_version(uint64_t ver)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::assert_version, _1, _2,
                   ver));
}
...
void Op::cmpext(uint64_t off, ceph::buffer::list && cmp_bl, std::size_t *s)
{
    ...
    librados::ObjectOperationTestImpl op =
        std::bind(&librados::TestIoCtxImpl::cmpext, _1, _2, off, cmp_bl,
                  _4);
    if (s != nullptr) {
        op = std::bind(save_operation_size,
                       std::bind(op, _1, _2, _3, _4, _5, _6), s);
    }
    o->ops.push_back(op);
}
...
std::size_t Op::size()const
{
    auto o =
        *reinterpret_cast <
        librados::TestObjectOperationImpl * const * >(&impl);
    return o->ops.size();
...
void Op::exec(std::string_view cls, std::string_view method,
              ...
              ceph::buffer::list *out, boost::system::error_code *ec)
{
    auto o =
        *reinterpret_cast < librados::TestObjectOperationImpl ** >(&impl);
    ...
    librados::ObjectOperationTestImpl op =
        [cls_handler, cls, method, inbl =
             const_cast < bufferlist & >(inbl), out]
    ...
    uint64_t *)mutable->int {
        return io_ctx->exec(oid, cls_handler, std::string(cls).c_str(),
              ...
              boost::system::error_code *ec)
{
    ...
    uint64_t *)mutable->int {
        return io_ctx->exec(oid, cls_handler, std::string(cls).c_str(),
                            std::string(method).c_str(), inbl, outbl,
                            snap_id, snapc);
    };
    if (ec != NULL) {
        op = std::bind(save_operation_ec,
                  ...
                  boost::system::error_code *ec)
{
    ...
    librados::ObjectOperationTestImpl op;
    if (out != nullptr) {
        op = std::bind(&librados::TestIoCtxImpl::read, _1, _2, len, off,
                       out, _4, _6);
    } else {
        op = std::bind(&librados::TestIoCtxImpl::read, _1, _2, len, off, _3,
                       _4, _6);
    }
                         ...
                         boost::system::error_code *ec)
{
    auto o =
        *reinterpret_cast < librados::TestObjectOperationImpl ** >(&impl);
    librados::ObjectOperationTestImpl op = [off, len, out, extents]
                                           (librados::TestIoCtxImpl * io_ctx, const std::string & oid,
                                            bufferlist * outbl, uint64_t snap_id, const SnapContext & snapc,
    uint64_t *)mutable->int {
        std::map < uint64_t, uint64_t > m;
        int r =
        io_ctx->sparse_read(oid, off, len, &m,
                            (out != nullptr ? out : outbl), snap_id);
...
void ReadOp::list_snaps(SnapSet *snaps, bs::error_code *ec)
{
    auto o =
        *reinterpret_cast < librados::TestObjectOperationImpl ** >(&impl);
    librados::ObjectOperationTestImpl op = [snaps]
                                           (librados::TestIoCtxImpl * io_ctx, const std::string & oid,
                                            bufferlist *, uint64_t, const SnapContext &,
    uint64_t *)mutable->int {
...
void WriteOp::create(bool exclusive)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::create, _1, _2, exclusive,
                   _5));
}
...
void WriteOp::write(uint64_t off, ceph::buffer::list && bl)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::write, _1, _2, bl,
                   bl.length(), off, _5));
}
...
void WriteOp::write_full(ceph::buffer::list && bl)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::write_full, _1, _2, bl,
                   _5));
}
...
void WriteOp::remove()
{
    auto o =
        *reinterpret_cast < librados::TestObjectOperationImpl ** >(&impl);
    o->ops.
    push_back(std::bind(&librados::TestIoCtxImpl::remove, _1, _2, _5));
}
...
void WriteOp::truncate(uint64_t off)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::truncate, _1, _2, off,
                   _5));
}
...
void WriteOp::zero(uint64_t off, uint64_t len)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::zero, _1, _2, off, len,
                   _5));
}
                        ...
                        ceph::buffer::list && bl)
{
    ...
    o->ops.
    push_back(std::
              bind(&librados::TestIoCtxImpl::writesame, _1, _2, bl,
                   write_len, off, _5));
}
...
RADOS RADOS::make_with_librados(librados::Rados &rados)
{
    ...
    return RADOS {
        std::make_unique < detail::Client > (test_rados_client)};
}
                      ...
                      const blkin_trace_info *trace_info)
{
    ...
    auto ops =
        *reinterpret_cast <
        librados::TestObjectOperationImpl ** >(&op.impl);
    ...
    auto r = io_ctx->aio_operate_read(std::string { o }
                                      , *ops, completion, 0U, bl,
                                      snap_id, objver);
    ceph_assert(r == 0);
}
                    ...
                    const blkin_trace_info *trace_info)
{
    ...
    if (opt_snapc) {
        ...
        snapc.snaps.assign(opt_snapc->second.begin(),
                           opt_snapc->second.end());
    }

    auto completion = create_aio_completion(std::move(c));
    auto r = io_ctx->aio_operate(std::string { o }
                                 , *ops, completion, &snapc, nullptr, 0U);
    ceph_assert(r == 0);
}
                        ...
                        std::unique_ptr < Op::Completion > c)
{
    ...
    c->post(std::move(c),
            (r <
             0 ? bs::error_code(-r, osd_category()) : bs::error_code()));
}
...
{
    auto r =
        impl->test_rados_client->blocklist_add(std::string(client_address),
            expire.value_or(0 s).
            count());
    c->post(std::move(c),
            (r <
             0 ? bs::error_code(-r, mon_category()) : bs::error_code()));
}
...
{
    auto &impl =
        *reinterpret_cast < std::unique_ptr < neorados::detail::Client >
        *>(&rados);
...
} MockTestMemRadosClient &get_mock_rados_client(neorados::RADOS &rados)
{
    ...
    return *reinterpret_cast <
           MockTestMemRadosClient * >(impl->test_rados_client);
}
