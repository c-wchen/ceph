...
namespace rbd {
    namespace mirror {
        namespace image_sync {
            ...
            template < typename I >
                SnapshotCopyRequest <
                I >::SnapshotCopyRequest(I * local_image_ctx,
                                         I * remote_image_ctx,
                                         SnapMap * snap_map,
                                         Journaler * journaler,
                                         librbd::journal::MirrorPeerClientMeta *
                                         meta, ContextWQ * work_queue,
                                         Context * on_finish)
          :    BaseRequest("rbd::mirror::image_sync::SnapshotCopyRequest",
                ...
                m_journaler(journaler), m_client_meta(meta),
            ...
            template < typename I > void SnapshotCopyRequest < I >::send() {
                ...
                int r =
                    validate_parent(m_remote_image_ctx, &remote_parent_spec);
                if (r < 0) {
            ...
            template < typename I >
                ...
                for (; snap_id_it != m_local_snap_ids.end(); ++snap_id_it) {
                    ...
                    int r =
                        m_local_image_ctx->is_snap_unprotected(local_snap_id,
                                                               &local_unprotected);
                    if (r < 0) {
                        derr <<
                    ...
                    if (snap_seq_it != m_snap_seqs.end()) {
                        ...
                        r = m_remote_image_ctx->
                            is_snap_unprotected(snap_seq_it->first,
                                                &remote_unprotected);
                        if (r < 0) {
                            derr <<
                ...
                m_snap_name =
                    get_snapshot_name(m_local_image_ctx, m_prev_snap_id);
            ...
            template < typename I >
                ...
                for (; snap_id_it != m_local_snap_ids.end(); ++snap_id_it) {
                    ...
                    m_local_image_ctx->snap_lock.put_read();
                    if (r < 0) {
                        derr << ": failed to retrieve local snap namespace: " <<
                            m_snap_name << dendl;
                        finish(r);
                        return;
                    }
            ...
            template < typename I >
                ...
                for (; snap_id_it != m_remote_snap_ids.end(); ++snap_id_it) {
                    ...
                    m_remote_image_ctx->snap_lock.get_read();
                    int r =
                        m_remote_image_ctx->get_snap_namespace(remote_snap_id,
                                                               &snap_namespace);
                    m_remote_image_ctx->snap_lock.put_read();
                    if (r < 0) {
                        derr << ": failed to retrieve remote snap namespace: "
                            << m_snap_name << dendl;
                        finish(r);
                        return;
                    }
                    ...
                    if (m_snap_seqs.find(remote_snap_id) == m_snap_seqs.end() &&
                        boost::get < cls::rbd::UserSnapshotNamespace >
                        (&snap_namespace) != nullptr) {
                        break;
                    }
                }
                ...
                m_prev_snap_id = *snap_id_it;
                m_snap_name =
                    get_snapshot_name(m_remote_image_ctx, m_prev_snap_id);

                m_remote_image_ctx->snap_lock.get_read();
                auto snap_info_it =
                    m_remote_image_ctx->snap_info.find(m_prev_snap_id);
                if (snap_info_it == m_remote_image_ctx->snap_info.end()) {
                    m_remote_image_ctx->snap_lock.put_read();
                    derr << ": failed to retrieve remote snap info: " <<
                        m_snap_name << dendl;
                    finish(-ENOENT);
                    return;
                }
                ...
                SnapshotCreateRequest < I > *req =
                    SnapshotCreateRequest < I >::create(m_local_image_ctx,
                                                        m_snap_name,
            ...
            template < typename I >
                ...
                dout(20) << ": mapping remote snap id " << m_prev_snap_id <<
                    " to " << local_snap_id << dendl;
            ...
            template < typename I >
                ...
                for (; snap_id_it != m_remote_snap_ids.end(); ++snap_id_it) {
                    ...
                    int r =
                        m_remote_image_ctx->is_snap_protected(remote_snap_id,
                                                              &remote_protected);
                    if (r < 0) {
                        derr <<
                    ...
                    r = m_local_image_ctx->is_snap_protected(snap_seq_it->
                                                             second,
                                                             &local_protected);
                    if (r < 0) {
                        derr <<
            ...
            template < typename I >
                ...
                Context *ctx = create_context_callback <
                    ...
                    &SnapshotCopyRequest < I >::handle_update_client > (this);
            ...
            template < typename I >
              ...
              for (auto & pair:m_snap_seqs) {
                    ...
                    m_snap_map->
                        insert(std::make_pair(pair.first, local_snap_ids));
                }
            }
