                                        ...
                                        const DoutPrefixProvider *dpp)
{
    ...
    if (entry_point.has_bucket_info) {
        *info = entry_point.old_bucket_info;
        ...
        ldpp_dout(dpp, 20) << "rgw_get_bucket_info: old bucket info, bucket=" << info->bucket << " owner " << info->owner <<
                           dendl;
        return 0;
    }
        ...
        const DoutPrefixProvider *dpp)
{
    ...
    if (!orig_info && !exclusive) {
        ...
        if (r < 0) {
            if (r != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): read_bucket_instance_info() of key=" << key << " returned r=" << r <<
                                  dendl;
                return r;
            }
        } else {
    ...
    if (orig_info && *orig_info && !exclusive) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): svc.bi->handle_overwrite() of key=" << key << " returned r=" << r <<
                              dendl;
            return r;
        }
    }
