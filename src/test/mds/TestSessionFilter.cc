...
TEST(MDSSessionFilter, IdEquality)
{
    ...
    ASSERT_TRUE(filter.match(*a,[](client_t c)->bool {
                             return false;
                             }
                ));
    ASSERT_FALSE(filter.match(*b,[](client_t c)->bool {
                              return false;
                              }
                 ));
    a->put();
...
TEST(MDSSessionFilter, MetadataEquality)
{
    ...
    ASSERT_EQ(r, 0);
    Session *a = new Session(nullptr);
    a->set_client_metadata(std::map < std::string, std::string > ( { {
                                                                  "root",
                                                                  "/rhubarb"}}));
    Session *b = new Session(nullptr);
    b->set_client_metadata(std::map < std::string, std::string > ( { {
                                                                  "root",
                                                                  "/custard"}}));
...
TEST(MDSSessionFilter, ReconnectingEquality)
{
    ...
    ASSERT_FALSE(filter.match(*a,[](client_t c)->bool {
                              return false;
                              }
                 ));
    a->put();
