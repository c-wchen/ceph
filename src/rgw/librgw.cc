...
#define MIN_EXPIRE_S 120
    ...
    int RGWLibProcess::process_request(RGWLibRequest * req, RGWLibIO * io) {
        ...
        if (ret < 0) {
            if (s->system_request) {
                ...
                dout(2) << "overriding permissions due to admin operation" <<
                    dendl;
            }
            else {
                abort_req(s, op, ret);
    ...
    int RGWLib::init(vector < const char *>&args) {
        ...
        const string & ldap_searchfilter =
            store->ctx()->_conf->rgw_ldap_searchfilter;
        ...
        if (r < 0) {
            derr << "ERROR: failed to register to service map: " <<
    ...
    int RGWLibRequest::read_permissions(RGWOp * op) {
        ...
        int ret = rgw_build_bucket_policies(rgwlib.get_store(), get_state());
        if (ret < 0) {
            ldout(get_state()->cct,
                  10) << "read_permissions (bucket policy) on " << get_state()->
                bucket << ":" << get_state()->
                object << " only_bucket=" << only_bucket()
                << " ret=" << ret << dendl;
            if (ret == -ENODATA)
                ret = -EACCES;
        }
            ...
            if (ret < 0) {
                ldout(get_state()->cct,
                    ...
                    get_state()->bucket << ":" << get_state()->
                    object << " ret=" << ret << dendl;
                if (ret == -ENODATA)
                    ret = -EACCES;
            }
        }
