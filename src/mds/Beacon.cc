...
Dispatcher(cct),
beacon_interval(g_conf->mds_beacon_interval), monc(monc), name(name)
{
}
...
void Beacon::init(const MDSMap * mdsmap)
{
    ...
    sender = std::thread([this] () {
                         std::unique_lock < std::mutex > lock(mutex); std::condition_variable c;    // no one wakes us
                         while (!finished) {
                         auto now = clock::now();
                         auto since =
                         std::chrono::duration <
...
bool Beacon::ms_can_fast_dispatch(const Message * m) const const
{
...
bool Beacon::_send()
{
    ...
    MMDSBeacon *beacon =
        new MMDSBeacon(monc->get_fsid(), mds_gid_t(monc->get_global_id()),
                       name,
                       epoch,
                       want_state,
                       last_seq,
...
bool Beacon::is_laggy()
{
    ...
    if (since > g_conf->mds_beacon_grace) {
        ...
        laggy = true;
        auto last_reconnect =
            std::chrono::duration < double >(now - last_mon_reconnect).count();
        if (since > (g_conf->mds_beacon_grace * 2)
            && last_reconnect > g_conf->mds_beacon_interval) {
            // maybe it's not us?
            dout(1) <<
...
void Beacon::set_want_state(const MDSMap * mdsmap,
                            MDSMap::DaemonState const newstate)
{
    std::unique_lock < std::mutex > lock(mutex);
...
void Beacon::notify_health(MDSRank const *mds)
{
    ...
    // Arbitrary factor of 2, indicates MDS is not trimming promptly
    {
        if (mds->mdlog->get_num_segments() >
            (size_t) (g_conf->mds_log_max_segments * 2)) {
    ...
    // Detect clients failing to advance their old_client_tid
    {
        ...
        const auto recall_warning_threshold =
            g_conf->get_val < uint64_t > ("mds_recall_warning_threshold");
      ...
      for (auto & session:sessions) {
            ...
            if ((session->get_num_trim_requests_warnings() > 0 &&
                ...
                m.metadata["client_id"] =
                    stringify(session->info.inst.name.num());
        ...
        if (large_completed_requests_metrics.size() <=
            (size_t) g_conf->mds_health_summarize_threshold) {
            health.metrics.splice(health.metrics.end(),
    ...
    // Detect MDS_HEALTH_SLOW_REQUEST condition
    {
        ...
        if (slow) {
            ...
            std::ostringstream oss;
            oss << slow << " slow requests are blocked > " << g_conf->
                mds_op_complaint_time << " sec";
...

    {
        ...
        if (MDSIOContextBase::check_ios_in_flight(cutoff, count, oldest)) {
            ...
            std::ostringstream oss;
            oss << count << " slow metadata IOs are blocked > " <<
    ...
    if (mds->mdcache->cache_overfull()) {
        std::ostringstream oss;
        oss << "MDS cache is too large (" << bytes2str(mds->mdcache->
                                                       cache_size())
