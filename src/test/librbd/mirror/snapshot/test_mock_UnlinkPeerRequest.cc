...
public:
    ...
    void expect_is_refresh_required(MockTestImageCtx &
                                    mock_image_ctx,
                            ...
                            const std::string &peer_uuid, int r)
    {
        ...
        .
        WillOnce(Invoke
                 ([&mock_image_ctx, snap_id, peer_uuid, r](auto
                         &&
                         ...
                         args)->
        int {
    ...
    void expect_notify_update(MockTestImageCtx &mock_image_ctx,
                              int r)
    {
        ...
        Context * on_finish) {
            if (r == 0) {
                auto it =
                    mock_image_ctx.snap_info.find(snap_id);
                EXPECT_NE(it, mock_image_ctx.snap_info.end());
                EXPECT_EQ(it->second.snap_namespace,
                          snap_namespace);
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, Success)
{
    ...
    C_SaferCond ctx;
    auto req =
        new MockUnlinkPeerRequest(&mock_image_ctx, snap_id,
                                  "peer1_uuid",
                                  true, &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest,
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, SnapshotDNE)
{
    ...
    C_SaferCond ctx;
    auto req =
        new MockUnlinkPeerRequest(&mock_image_ctx, 123, "peer_uuid",
                                  true, &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, UnlinkError)
{
    ...
    expect_is_refresh_required(mock_image_ctx, false);
    expect_unlink_peer(mock_image_ctx, snap_id, "peer1_uuid",
                       -EINVAL);
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, UnlinkErrorRestart)
{
    ...
    expect_is_refresh_required(mock_image_ctx, true);
    expect_refresh_image(mock_image_ctx, 0);
    expect_unlink_peer(mock_image_ctx, snap_id, "peer_uuid",
                       -ERESTART);
    expect_refresh_image(mock_image_ctx, 0);
    expect_remove_snapshot(mock_image_ctx, snap_id, 0);
