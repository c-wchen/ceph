    ...
    namespace mirror {
        namespace image_map {
            ...
            static const uint32_t MAX_UPDATE = 256;
             ...
             template < typename I >
                UpdateRequest < I >::UpdateRequest(librados::IoCtx & ioctx,
                                                   ...
                                                   Context * on_finish)
            :m_ioctx(ioctx), m_update_mapping(update_mapping),
                ...
                if (m_update_mapping.empty()
                    && m_remove_global_image_ids.empty()) {
                    finish(0);
                    return;
                ...
                while (it1 != m_update_mapping.end()
                       && nr_updates++ < MAX_UPDATE) {
                    librbd::cls_client::mirror_image_map_update(&op, it1->first,
                                                                it1->second);
                    it1 = m_update_mapping.erase(it1);
                }

                auto it2 = m_remove_global_image_ids.begin();
                while (it2 != m_remove_global_image_ids.end()
                       && nr_updates++ < MAX_UPDATE) {
                ...
                librados::AioCompletion * aio_comp = create_rados_callback <
                    UpdateRequest,
                    &UpdateRequest::handle_update_image_map > (this);
            ...
            template < typename I >
                ...
                if (r < 0) {
                    derr << ": failed to update image map: " << cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
                }
