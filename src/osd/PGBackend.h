...
public:
    ...
    public:
         ...
         */
        virtual void on_failed_pull(const std::set < pg_shard_t > &from,
        ...
        ThreadPool::TPHandle & > *bless_gencontext(GenContext <
                ...
                TPHandle & > *c) = 0;
        virtual GenContext <
        ThreadPool::TPHandle & > *bless_unlocked_gencontext(GenContext <
                ThreadPool::
                TPHandle & > *c) =
                    0;
        ...
        virtual void queue_transactions(std::vector < ObjectStore::Transaction >
        ...
        virtual const std::set < pg_shard_t >
        ...
        virtual const std::map < hobject_t,
                std::set < pg_shard_t >> &get_missing_loc_shards()
                const = 0;
        ...
        const = 0;
        virtual const pg_missing_const_i *maybe_get_shard_missing(pg_shard_t
                peer) const
        {
        ...
        pg_info_t > &get_shard_info() const = 0;
        ...
        virtual void log_operation(std::vector < pg_log_entry_t > &&logv,
                                   ...
                                   ObjectStore::Transaction &t, bool async =
                                       false) = 0;
        ...
        virtual void send_message_osd_cluster(std::vector < std::pair < int,
                                              Message * >> &messages,
                                              epoch_t from_epoch) = 0;
    ...
    Listener *get_parent() const
    {
        return parent;
                ...
                const coll_t &coll,
                ObjectStore::CollectionHandle &ch): cct(cct), store(store),
        coll(coll), ch(ch), parent(l)
    {
    }
     ...
     */ struct RecoveryHandle {
        ...
        std::map < pg_shard_t, std::vector < std::pair < hobject_t,
            eversion_t > >>deletes;
    ...
    virtual int get_ec_data_chunk_count() const
    {
        return 0;
    };
    virtual int get_ec_stripe_chunk_size() const
    {
        return 0;
    };
...
public:
    ...
    virtual void submit_transaction(const hobject_t &hoid,  ///< [in] object
                                    ...
                                    std::optional < pg_hit_set_history_t > &hset_history, Context *on_all_commit,   ///< [in] called when all commit
...
public:
    ...
    int objects_list_partial(const hobject_t &begin,
                             int min,
                             int max,
                             std::vector < hobject_t > *ls, hobject_t *next);
    ...
    int objects_get_attr(const hobject_t &hoid,
                         const std::string &attr, ceph::buffer::list *out);
    ...
    virtual int objects_readv_sync(const hobject_t &hoid,
    ...
    virtual void objects_read_async(const hobject_t &hoid,
                                    const std::list < std::pair < boost::tuple <
