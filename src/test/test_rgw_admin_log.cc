      ...
      public:
         test_helper():resp_data(NULL) {
            ...
            curl_global_cleanup();
                           ...
                           size_t(*function) (void *, size_t, size_t, void *) =
                           0, void *ud = 0, size_t length = 0);
...
#define ERR_CHECK_NEXT_PARAM(o) \
  if(((int)loop + 1) >= argc)return -1;		\
        ...
        if (host.length() <= 0 || rgw_admin_path.length() <= 0)
            return -1;
        return 0;
    }
...
/* destination should be CEPH_CRYPTO_HMACSHA1_DIGESTSIZE bytes long */
    {
        ...
        admin_log::buf_to_hex((unsigned char *)dest,
                              CEPH_CRYPTO_HMACSHA1_DIGESTSIZE, hex_str);
    }

    static int get_s3_auth(string method, string creds, string date, string res,
                           string & out) {
        ...
        if (off != string::npos) {
            ...
            admin_log::calc_hmac_sha1(secret.c_str(), secret.length(),
                                      auth_hdr.c_str(), auth_hdr.length(),
                                      hmac_sha1);
    ...
    int test_helper::send_request(string method, string res,
        ...
        if (curl_inst) {
            ...
            curl_easy_setopt(curl_inst, CURLOPT_HEADERFUNCTION,
                             admin_log::write_header);
            curl_easy_setopt(curl_inst, CURLOPT_WRITEHEADER, (void *)this);
            curl_easy_setopt(curl_inst, CURLOPT_WRITEFUNCTION,
                             admin_log::write_data);
            curl_easy_setopt(curl_inst, CURLOPT_WRITEDATA, (void *)this);
            if (read_function) {
                curl_easy_setopt(curl_inst, CURLOPT_READFUNCTION,
                                 read_function);
                curl_easy_setopt(curl_inst, CURLOPT_READDATA, (void *)ud);
                curl_easy_setopt(curl_inst, CURLOPT_UPLOAD, 1L);
                curl_easy_setopt(curl_inst, CURLOPT_INFILESIZE_LARGE,
                                 (curl_off_t) length);
            }
...
static int put_bucket_obj(const char *obj_name, char *data, unsigned len)
{
    ...
    if (g_test->get_resp_code() != 200U) {
        cout << "Errror sending object to the bucket, http_code " << g_test->
            get_resp_code();
        return -1;
    }
    return 0;
...
static int delete_obj(const char *obj_name)
{
    ...
    if (g_test->get_resp_code() != 204U) {
        cout << "Errror deleting object from bucket, http_code " << g_test->
            get_resp_code();
        return -1;
    }
    return 0;
...
{
    uint32_t r =
        ceph_str_hash_linux(bucket_name, strlen(bucket_name)) % max_shards;
    return (int)r;
}
...
TEST(TestRGWAdmin, datalog_list)
{
    ...
    string rest_req;
    unsigned shard_id =
        get_datalog_shard_id(TEST_BUCKET_NAME,
                             g_ceph_context->_conf->rgw_data_log_num_shards);
    stringstream ss;
    ...
    EXPECT_EQ(put_bucket_obj
              (TEST_BUCKET_OBJECT, bucket_obj, TEST_BUCKET_OBJECT_SIZE), 0);
    sleep(1);
    ss << "/admin/log?type=data&id=" << shard_id << "&start-time=" <<
        start_time;
    rest_req = ss.str();
    g_test->send_request(string("GET"), rest_req);
    EXPECT_EQ(200U, g_test->get_resp_code());
...
TEST(TestRGWAdmin, datalog_lock_unlock)
{
    ...
    g_test->send_request(string("POST"), rest_req, read_dummy_post, NULL,
                         sizeof(int));
    ...
    rest_req =
        "/admin/log?type=data&lock&id=1&length=3&locker-id=ceph&zone-id=1";
    g_test->send_request(string("POST"), rest_req, read_dummy_post, NULL,
                         sizeof(int));
    EXPECT_EQ(200U, g_test->get_resp_code());
...
TEST(TestRGWAdmin, mdlog_list)
{
    ...
    ss.str("");
    ss << "/admin/log?type=metadata&id=" << shard_id << "&start-time=" <<
        start_time;
    rest_req = ss.str();
    g_test->send_request(string("GET"), rest_req);
    EXPECT_EQ(200U, g_test->get_resp_code());
...
TEST(TestRGWAdmin, mdlog_lock_unlock)
{
    ...
    rest_req =
        "/admin/log?type=metadata&lock&length=3&locker-id=ceph&zone-id=1";
    g_test->send_request(string("POST"), rest_req, read_dummy_post, NULL,
                         sizeof(int));
    EXPECT_EQ(400U, g_test->get_resp_code());   /*Bad request */
...
TEST(TestRGWAdmin, bilog_list)
{
    ...
    ASSERT_TRUE(bucket_obj != NULL);
    EXPECT_EQ(put_bucket_obj
              (TEST_BUCKET_OBJECT_1, bucket_obj, TEST_BUCKET_OBJECT_SIZE), 0);
    free(bucket_obj);
