...
class ObjectCache {
    ...
    void touch_lru(const string & name, ObjectCacheEntry & entry,
                   std::list < string >::iterator & lru_iter);
    void remove_lru(const string & name,
                    std::list < string >::iterator & lru_iter);
  ...
  public:
    ...
    } int get(const std::string & name, ObjectCacheInfo & bl, uint32_t mask,
              rgw_cache_entry_info * cache_info);
    boost::optional < ObjectCacheInfo > get(const std::string & name) {
        boost::optional < ObjectCacheInfo > info {
        boost::in_place_init};
    ...
    template < typename F > void for_each(const F & f) {
        ...
        if (enabled) {
          ...
          for (const auto & kv:cache_map) {
                if (expiry.count()
                    && ((now - kv.second.info.time_added) < expiry)) {
    ...
    bool chain_cache_entry(list < rgw_cache_entry_info * >&cache_info_entries,
...
template < class T > class RGWCache:public T {
    ObjectCache cache;
    ...
    string normal_name(rgw_pool & pool, const std::string & oid) {
        ...
        buf.append(pool.name).append("+").append(pool.ns).append("+").
            append(oid);
        return buf;
    }
    ...
    int watch_cb(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id,
  ...
  public:
    ...
    int get_system_obj(RGWObjectCtx & obj_ctx,
                       ...
                       bufferlist & bl, off_t ofs, off_t end, map < string,
                       bufferlist > *attrs, rgw_cache_entry_info * cache_info,
...
template < class T >
    void RGWCache < T >::normalize_pool_and_obj(rgw_pool & src_pool,
...
template < class T >
    int RGWCache < T >::delete_system_obj(rgw_raw_obj & obj,
                                          RGWObjVersionTracker * objv_tracker)
{
    rgw_pool pool;
    string oid;
...
template < class T >
    int RGWCache < T >::get_system_obj(RGWObjectCtx & obj_ctx,
                                       RGWRados::SystemObject::Read::
                                       GetObjState & read_state,
                                       RGWObjVersionTracker * objv_tracker,
                                       rgw_raw_obj & obj, bufferlist & obl,
                                       off_t ofs, off_t end, map < string,
                                       bufferlist > *attrs,
                                       rgw_cache_entry_info * cache_info,
                                       ...
                                       refresh_version)
{
    ...
    int r =
        T::get_system_obj(obj_ctx, read_state, objv_tracker, obj, obl, ofs, end,
                          attrs, cache_info);
    if (r < 0) {
                                             ...
                                             objv_tracker)
{
    ...
    if (ret >= 0) {
        ...
        if (r < 0)
            mydout(0) << "ERROR: failed to distribute cache for " << obj <<
                dendl;
    }
    else {
        cache.remove(name);
...
template < class T >
    int RGWCache < T >::put_system_obj_impl(rgw_raw_obj & obj, uint64_t size,
                                            real_time * mtime,
                                            map < std::string,
                                            ...
                                            real_time set_mtime)
{
    ...
    int ret =
        T::put_system_obj_impl(obj, size, &result_mtime, attrs, flags, data,
...
template < class T >
    int RGWCache < T >::put_system_obj_data(void *ctx, rgw_raw_obj & obj,
                                            ...
                                            RGWObjVersionTracker * objv_tracker)
{
    ...
    int ret =
        T::put_system_obj_data(ctx, obj, data, ofs, exclusive, objv_tracker);
    if (cacheable) {
        string name = normal_name(pool, oid);
        if (ret >= 0) {
            cache.put(name, info, NULL);
...
template < class T >
    int RGWCache < T >::distribute_cache(const string & normal_name,
                                         rgw_raw_obj & obj,
                                         ObjectCacheInfo & obj_info, int op)
{
                                 ...
                                 uint64_t notifier_id, bufferlist & bl)
{
    ...
    case REMOVE_OBJ:
        ...
        break;
    default:
        mydout(0) << "WARNING: got unknown notification op: " << info.
            op << dendl;
        return -EINVAL;
    }
...
template < typename T >
    void RGWCache < T >::call_list(const boost::optional < std::string >
                                   &filter, Formatter * f)
{
    cache.for_each([this, &filter, f] (const string & name,
                                       const ObjectCacheEntry & entry) {
...
template < typename T > void RGWCache < T >::call_zap()
{
