...
Notify::Notify(ConnectionRef client,
               uint64_t client_gid,
               bufferlist & payload,
               uint32_t timeout,
               uint64_t cookie,
...
timeout(timeout),
cookie(cookie),
notify_id(notify_id), version(version), osd(osd), cb(NULL), lock("Notify::lock")
{
}
...
{
    NotifyRef ret(new Notify(client, client_gid,
                             payload, timeout,
                             cookie, notify_id, version, osd));
    ret->set_self(ret);
    return ret;
}
...
void Notify::maybe_complete_notify()
{
    ...
    if (watchers.empty() || timed_out) {
        ...
        for (set < WatchRef >::iterator p = watchers.begin();
             p != watchers.end(); ++p) {
            missed.
                push_back(make_pair
                          ((*p)->get_watcher_gid(), (*p)->get_cookie()));
        }
        ::encode(missed, bl);
...
class HandleDelayedWatchTimeout:public CancelableContext {
  ...
  public:
    bool canceled;
    explicit HandleDelayedWatchTimeout(WatchRef watch):watch(watch),
        canceled(false) {
    } void cancel() override {
        canceled = true;
    }
...
{
    stringstream ss;
    ss << pg->gen_prefix() << " -- Watch(" << make_pair(cookie, entity) << ") ";
    return ss.str();
}

Watch::Watch(PrimaryLogPG * pg,
             OSDService * osd,
...
obc(obc),
timeout(timeout),
cookie(cookie), addr(addr), will_ping(false), entity(entity), discarded(false)
{
    dout(10) << "Watch()" << dendl;
}
...
{
    if (conn == con) {
        dout(10) << __func__ << " con " << con << " - already connected" <<
            dendl;
        return;
    }
    ...
    if (sessionref) {
        ...
        sessionref->put();
        for (map < uint64_t, NotifyRef >::iterator i =
             in_progress_notifies.begin(); i != in_progress_notifies.end();
             ++i) {
...
bool Watch::is_discarded() const const
{
    return discarded;
}
...
void Watch::notify_ack(uint64_t notify_id, bufferlist & reply_bl)
{
    ...
    map < uint64_t, NotifyRef >::iterator i =
        in_progress_notifies.find(notify_id);
    ...
    for (set < WatchRef >::iterator i = _watches.begin();
        ...
        if (!(*i)->is_discarded()) {
            if ((*i)->is_connected(con)) {
                ...
                lgeneric_derr(cct) << __func__ << " not still connected to " <<
                    (*i) << dendl;
            }
        }
        pg->unlock();
