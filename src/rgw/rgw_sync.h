...
class RGWSyncErrorLogger {
    ...
    0};
  public:
    RGWSyncErrorLogger(RGWRados * _store, const string & oid_prefix,
                       int _num_shards);
    RGWCoroutine *log_error_cr(const string & source_zone,
                               const string & section, const string & name,
                               uint32_t error_code, const string & message);
...
struct rgw_sync_error_info {
    ...
    } rgw_sync_error_info(const string & _source_zone, uint32_t _error_code,
                          const string & _message):source_zone(_source_zone),
...
class RGWSyncBackoff {
  ...
  public:
   RGWSyncBackoff(int _max_secs = DEFAULT_BACKOFF_MAX):cur_wait(0),
        max_secs
        (_max_secs)
{
} void backoff_sleep();
...
class RGWBackoffControlCR:public RGWCoroutine {
  ...
  public:
  RGWBackoffControlCR(CephContext * _cct, bool _exit_on_error):RGWCoroutine(_cct), cr(NULL),
...
struct RGWMetaSyncEnv {
    ...
    RGWSyncErrorLogger *error_logger;
     ...
     RGWMetaSyncEnv():cct(NULL), store(NULL), conn(NULL), async_rados(NULL),
        http_manager(NULL), error_logger(NULL) {
...
class RGWRemoteMetaLog:public RGWCoroutinesManager {
  ...
  :    
    ...
    int read_master_log_shards_info(const string & master_period, map < int,
                                    RGWMetadataLogInfo > *shards_info);
    int read_master_log_shards_next(const string & period, map < int,
                                    string > shard_markers, map < int,
...
class RGWMetaSyncStatusManager {
  ...
  public:
    RGWMetaSyncStatusManager(RGWRados * _store,
                             RGWAsyncRadosProcessor * async_rados)
  :    store(_store), master_log(store, async_rados, this),
    ...
    int read_master_log_shards_info(const string & master_period, map < int,
                                    RGWMetadataLogInfo > *shards_info) {
        return master_log.read_master_log_shards_info(master_period,
                                                      shards_info);
    }
    int read_master_log_shards_next(const string & period, map < int,
                                    string > shard_markers, map < int,
                                    rgw_mdlog_shard_data > *result) {
        return master_log.read_master_log_shards_next(period, shard_markers,
                                                      result);
    }
...
class RGWMetaSyncSingleEntryCR:public RGWCoroutine {
  ...
  public:
     RGWMetaSyncSingleEntryCR(RGWMetaSyncEnv * _sync_env,
                              ...
                              _marker_tracker):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env), raw_key(_raw_key), entry_marker(_entry_marker),
        op_status(_op_status), pos(0), sync_status(0),
...
class RGWShardCollectCR:public RGWCoroutine {
  ...
  public:
     RGWShardCollectCR(CephContext * _cct,
                       int _max_concurrent):RGWCoroutine(_cct),
        current_running(0), max_concurrent(_max_concurrent), status(0) {
