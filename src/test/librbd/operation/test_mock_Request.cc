...
struct MockRequest: public Request < librbd::MockTestImageCtx > {
    MockRequest(librbd::MockTestImageCtx &image_ctx,
                Context *on_finish, uint64_t journal_op_tid)
        : Request < librbd::MockTestImageCtx > (image_ctx, on_finish,
                                                journal_op_tid)
    {
    } void complete(int r)
    {
...
struct TestMockOperationRequest: public TestMockFixture {
    void expect_can_affect_io(MockRequest &mock_request,
                              bool can_affect)
    {
...
TEST_F(TestMockOperationRequest, SendJournalDisabled)
{
    ...
    MockRequest *mock_request =
        new MockRequest(mock_image_ctx, &ctx, 0);
