    ...
    for (;;) {
        ...
        if (!bus_dev) {
            if (strcmp(udev_device_get_subsystem(dev), "rbd") == 0) {
                const char *this_pool =
                    udev_device_get_sysattr_value(dev, "pool");
                const char *this_image =
            ...
            if (strcmp(udev_device_get_subsystem(dev), "block") == 0) {
                const char *major =
                    udev_device_get_sysattr_value(bus_dev, "major");
                const char *minor =
                    udev_device_get_sysattr_value(bus_dev, "minor");
                const char *this_major =
                    udev_device_get_property_value(dev, "MAJOR");
                const char *this_minor =
                ...
                if (strcmp(this_major, major) == 0 &&
                    ...
                    assert(strcmp(udev_device_get_devnode(dev), name.c_str()) ==
                           0);
                    *pname = name;
                                     ...
                                     dev_t * pdevno, string * pid)
{
    ...
    if (have_minor_attr()) {
        min =
            strict_strtoll(udev_device_get_sysattr_value(dev, "minor"), 10,
                           &err);
        if (!err.empty()) {
                       ...
                       const char *options)
{
    ...
    if (r < 0) {
        if (r == -ENOENT) {
            cerr << "rbd: '" << devnode << "' is not an rbd device" << std::
                endl;
            r = -EINVAL;
        }
        return r;
...
static int unmap_image(struct krbd_ctx *ctx, const char *pool,
                       const char *image, const char *snap, const char *options)
{
    dev_t devno = 0;
    string id;
    int r;
