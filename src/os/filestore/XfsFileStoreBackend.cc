...
int XfsFileStoreBackend::detect_features()
{
    ...
    // extsize?
    int fd =::openat(get_basedir_fd(), "extsize_test", O_CREAT | O_WRONLY,
                     0600);
    if (fd < 0) {
        ret = -errno;
        dout(0) <<
            "detect_feature: failed to create test file for extsize attr: " <<
    ...
    if (::unlinkat(get_basedir_fd(), "extsize_test", 0) < 0) {
        ret = -errno;
        dout(0) <<
            "detect_feature: failed to unlink test file for extsize attr: " <<
            cpp_strerror(ret) << dendl;
        goto out_close;
    }
    ...
    if (cct()->_conf->filestore_xfs_extsize) {
        ...
        if (ret) {
            ret = 0;
            dout(0) <<
                "detect_feature: failed to set test file extsize, assuming extsize is NOT supported"
                << dendl;
            goto out_close;
        }
        ...
        if (ver == 0) {
            ...
            dout(0) << __func__ <<
                ": disabling extsize, your kernel < 3.5 and has buggy extsize ioctl"
                << dendl;
            m_has_extsize = false;
        }
        else {
