...
struct MAuthReply : public Message
{
    ...
    MAuthReply(__u32 p, bufferlist *bl = NULL, int r = 0, uint64_t gid = 0, const char *msg = "") : Message(CEPH_MSG_AUTH_REPLY),
                                                                                                    protocol(p), result(r), global_id(gid), result_msg(msg)
    {
        if (bl)
            result_bl = *bl;
    }
