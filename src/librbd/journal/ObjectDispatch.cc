...
template < typename I > struct C_CommitIOEvent: public Context {
    ...
    C_CommitIOEvent(I *image_ctx, Journal < I > *journal,
                    uint64_t object_no, uint64_t object_off,
                    uint64_t object_len, uint64_t journal_tid,
                    int object_dispatch_flags, Context *on_finish)
        : image_ctx(image_ctx), journal(journal), object_no(object_no),
          object_off(object_off), object_len(object_len),
    ...
    } void finish(int r) override
    {
        ...
        if (r >= 0 ||
            ...
            for (const auto &extent : image_extents) {
                journal->commit_io_event_extent(journal_tid,
                                                extent.first,
                                                extent.second, r);
            }
        }
...
template < typename I >
bool ObjectDispatch < I >::discard(uint64_t object_no,
                                   uint64_t object_off,
                                   uint64_t object_len,
                                   ...
                                   Context *on_dispatched)
{
    ...
    *on_finish =
        new C_CommitIOEvent < I > (m_image_ctx, m_journal, object_no,
                                   object_off, object_len, *journal_tid,
                                   *object_dispatch_flags, *on_finish);
    *on_finish =
        create_context_callback < Context,
        &Context::complete > (*on_finish, m_journal);

    *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
    wait_or_flush_event(*journal_tid, *object_dispatch_flags,
                        on_dispatched);
    return true;
}

template < typename I >
bool ObjectDispatch < I >::write(uint64_t object_no,
                                 uint64_t object_off,
                                 ceph::bufferlist
...
template < typename I >
bool ObjectDispatch < I >::write_same(uint64_t object_no,
                                      uint64_t object_off,
                                      uint64_t object_len,
                                      io::LightweightBufferExtents
                                      && buffer_extents,
                                      ceph::bufferlist
                                      && data, IOContext io_context,
                                      int op_flags,
...
template < typename I >
bool ObjectDispatch < I >::compare_and_write(uint64_t object_no,
        uint64_t object_off,
        ceph::bufferlist
        && cmp_data,
        ceph::bufferlist
...
template < typename I >
bool ObjectDispatch < I >::flush(io::FlushSource flush_source,
                                 const ZTracer::
                                 Trace &parent_trace,
                                 uint64_t *journal_tid,
                                 io::DispatchResult *
                                 ...
                                 Context *on_dispatched)
{
    ...
    auto ctx = *on_finish;
    *on_finish = new LambdaContext([image_ctx =
                                        m_image_ctx, ctx, journal_tid =
    *journal_tid](int r) {
        image_ctx->journal->
        commit_io_event(journal_tid, r);
        ctx->complete(r);
    });
...
template < typename I >
void ObjectDispatch < I >::extent_overwritten(uint64_t object_no,
        uint64_t object_off,
        uint64_t object_len,
        uint64_t journal_tid,
        uint64_t
...
template < typename I >
void ObjectDispatch < I >::wait_or_flush_event(uint64_t journal_tid,
        int
        object_dispatch_flags,
        Context *
        on_dispatched)
