...
namespace librbd {
    namespace cache {
        namespace pwl {
            ...
            template < typename T >
                C_BlockIORequest < T >::C_BlockIORequest(T & pwl,
                                                         ...
                                                         fadvise_flags,
                                                         Context * user_req)
            :pwl(pwl), image_extents(std::move(extents)), bl(std::move(bl)),
                fadvise_flags(fadvise_flags), user_req(user_req),
                image_extents_summary(image_extents), m_arrived_time(arrived) {
                ...
                ldout(pwl.get_context(),
                      20) << this << " cell=" << cell << dendl;
                ceph_assert(cell);
                ceph_assert(!m_cell);
                m_cell = cell;
            }
            ...
            template < typename T > void C_BlockIORequest < T >::release_cell() {
                ...
                if (m_cell_released.compare_exchange_strong(initial, true)) {
                    ...
                    ldout(pwl.get_context(),
                          5) << "cell " << m_cell << " already released for " <<
                        this << dendl;
                }
            }
            ...
            template < typename T > void C_BlockIORequest < T >::finish(int r) {
                ...
                if (m_finish_called.compare_exchange_strong(initial, true)) {
                    ldout(pwl.get_context(),
                          15) << this << " finishing" << dendl;
                    finish_req(0);
                }
                else {
                    ldout(pwl.get_context(),
                          20) << this << " already finished" << dendl;
                    ceph_assert(0);
                }
            }
            ...
            template < typename T >
                C_WriteRequest < T >::C_WriteRequest(T & pwl,
                                                     const utime_t arrived,
                                                     io::Extents
                                                     && image_extents,
                                                     bufferlist
                                                     ...
                                                     Context * user_req)
            :C_BlockIORequest < T > (pwl, arrived, std::move(image_extents),
            ...
            template < typename T >
                void C_WriteRequest <
                T >::
                blockguard_acquired(GuardedRequestFunctionContext & guard_ctx) {
                ldout(pwl.get_context(),
                      20) << __func__ << " write_req=" << this << " cell=" <<
                    guard_ctx.cell << dendl;
            ...
            template < typename T >
                std::shared_ptr < WriteLogOperation > C_WriteRequest <
                T >::create_operation(uint64_t offset, uint64_t len) {
                return pwl.m_builder->create_write_log_operation(*op_set,
                                                                 offset, len,
                                                                 pwl.
                                                                 get_context(),
                                                                 pwl.m_builder->
            ...
            template < typename T >
                void C_WriteRequest <
                T >::setup_log_operations(DeferredContexts & on_exit) {
                GenericWriteLogEntries log_entries;
                {
                    std::lock_guard locker(m_lock);
                    std::shared_ptr < SyncPoint > current_sync_point =
                        pwl.get_current_sync_point();
                    if ((!pwl.get_persist_on_flush()
                         && current_sync_point->log_entry->writes_completed)
                        || (current_sync_point->log_entry->writes >
                            MAX_WRITES_PER_SYNC_POINT)
                        || (current_sync_point->log_entry->bytes >
                    ...
                    op_set =
                        make_unique < WriteLogOperationSet >
                        (this->m_dispatched_time, m_perfcounter,
                         current_sync_point, pwl.get_persist_on_flush(),
                         pwl.get_context(), this);
                    ldout(pwl.get_context(),
                          20) << "write_req=[" << *this << "], op_set=" <<
                        op_set.get() << dendl;
                    ceph_assert(m_resources.allocated);
                  ...
                  for (auto & extent:this->image_extents) {
                        ...
                        auto operation =
                            this->create_operation(extent.first, extent.second);
            ...
            template < typename T >
                bool C_WriteRequest <
                T >::append_write_request(std::shared_ptr < SyncPoint >
                                          sync_point) {
                std::lock_guard locker(m_lock);
                auto write_req_sp = this;
                if (sync_point->earlier_sync_point) {
                    Context *schedule_append_ctx =
                        new LambdaContext([write_req_sp] (int r){
                                          write_req_sp->schedule_append();});
                    sync_point->earlier_sync_point->on_sync_point_appending.
            ...
            template < typename T > void C_WriteRequest < T >::schedule_append() {
                ...
                pwl.setup_schedule_append(this->op_set->operations,
                                          m_do_early_flush, this);
            }
 ...
 */
            ...
            template < typename T >
                C_FlushRequest < T >::C_FlushRequest(T & pwl,
                                                     const utime_t arrived,
                                                     io::Extents
                                                     && image_extents,
                                                     bufferlist
            ...
            template < typename T > void C_FlushRequest < T >::finish_req(int r) {
                ...
                utime_t now = ceph_clock_now();
                m_perfcounter->tinc(l_librbd_pwl_aio_flush_latency,
                                    now - this->m_arrived_time);
            }
            ...
            template < typename T >
                C_DiscardRequest < T >::C_DiscardRequest(T & pwl,
                                                         const utime_t arrived,
                                                         io::Extents
                                                         && image_extents,
                                                         uint32_t
          ...
          :    C_BlockIORequest < T > (pwl, arrived, std::move(image_extents),
            ...
            template < typename T >
                ...
                GenericWriteLogEntries log_entries;
              for (auto & extent:this->image_extents) {
                    op = pwl.m_builder->create_discard_log_operation(pwl.
                ...
                Context *on_write_persist = new LambdaContext([this,
                                                               discard_req] (int
                                                                             r){
                                                              ldout(pwl.
                                                                    get_context
                                                              ...
                                                              discard_req->
                                                              release_cell();});
                op->init_op(current_sync_gen, persist_on_flush,
                            pwl.get_last_op_sequence_num(), on_write_persist,
                            on_write_append);
                pwl.add_into_log_map(log_entries, this);
            }
            ...
            template < typename T >
                C_WriteSameRequest < T >::C_WriteSameRequest(T & pwl,
                                                             const utime_t
                                                             arrived,
                                                             io::Extents
                                                             && image_extents,
                                                             ...
                                                             perfcounter,
                                                             Context * user_req)
          :    C_WriteRequest < T > (pwl, arrived, std::move(image_extents),
                                      std::move(bl), fadvise_flags, lock,
            ...
            template < typename T >
                ...
                this->m_perfcounter->tinc(l_librbd_pwl_ws_latency,
                                          comp_latency);
            }

            template < typename T >
                std::shared_ptr < WriteLogOperation > C_WriteSameRequest <
                T >::create_operation(uint64_t offset, uint64_t len) {
                ceph_assert(this->image_extents.size() == 1);
                WriteLogOperationSet & set = *this->op_set.get();
                return pwl.m_builder->create_write_log_operation(*this->op_set.
                                                                 get(), offset,
                                                                 len,
                                                                 this->bl.
                                                                 length(),
            ...
            template < typename T >
                ...
                if (is_comp_and_write) {
                    ...
                    utime_t comp_latency = now - this->m_arrived_time;
                    this->m_perfcounter->tinc(l_librbd_pwl_cmp_latency,
                                              comp_latency);
                }
            }
