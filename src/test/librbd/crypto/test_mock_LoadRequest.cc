...
} // namespace util } // namespace crypto }    // namespace librbd
...
struct TestMockCryptoLoadRequest: public TestMockFixture {
    ...
    void SetUp() override
    {
        ...
        mock_load_request =
            MockLoadRequest::create(mock_image_ctx, std::move(formats),
                                    on_finish);
    ...
    } void expect_encryption_load(MockEncryptionFormat *
                                  encryption_format,
                                  MockTestImageCtx *ictx,
                                  std::string detected_format =
                                      "SOMEFORMAT")
    {
        EXPECT_CALL(*encryption_format,
                    load(ictx, _, _)).WillOnce(WithArgs < 1,
                                               2 >
                                               (Invoke
                                                ([this,
                                                    detected_format]
    ...
    void expect_encryption_format_clone(MockEncryptionFormat *
                                        encryption_format)
    {
        cloned_encryption_format = new MockEncryptionFormat();
        EXPECT_CALL(*encryption_format,
        clone()).WillOnce(Invoke([this]() {
...
TEST_F(TestMockCryptoLoadRequest, LoadParentFail)
{
    ...
    mock_load_request->send();
    ASSERT_EQ(ETIMEDOUT, finished_cond.wait_for(0));
    expect_encryption_load(mock_parent_encryption_format,
                           mock_parent_image_ctx);
    load_context->complete(0);
    ASSERT_EQ(ETIMEDOUT, finished_cond.wait_for(0));
    load_context->complete(-EINVAL);
