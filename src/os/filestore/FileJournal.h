...
class FileJournal:public Journal, public md_config_obs_t {
  public:
    ...
    struct write_item {
        ...
        TrackedOpRef tracked_op;
        ZTracer::Trace trace;
        write_item(uint64_t s, bufferlist & b, int ol,
                   TrackedOpRef opref):seq(s), orig_len(ol), tracked_op(opref) {
    ...
    int prepare_entry(vector < ObjectStore::Transaction > &tls,
                      bufferlist * tbl) override;
  ...
  private:
    ...
    int prepare_multi_write(bufferlist & bl, uint64_t & orig_ops,
                            uint64_t & orig_bytee);
    int prepare_single_write(write_item & next_write, bufferlist & bl,
    ...
    void wrap_read_bl(off64_t in_pos,   ///< [in] start position
  ...
  public:
  FileJournal(CephContext * cct, uuid_d fsid, Finisher * fin, Cond * sync_cond, const char *f, bool dio = false, bool ai = true, bool faio = false):
    Journal(cct, fsid, fin, sync_cond),
        ...
        completions_lock("FileJournal::completions_lock", false, true, false,
                         cct), fn(f), zero_buf(NULL), max_size(0),
        block_size(0), directio(dio), aio(ai), force_aio(faio),
        must_write_header(false), write_pos(0), read_pos(0), discard(false),
#ifdef HAVE_LIBAIO
        aio_lock("FileJournal::aio_lock"),
        aio_ctx(0),
        aio_num(0), aio_bytes(0),
        aio_write_queue_ops(0), aio_write_queue_bytes(0),
#endif
        ...
        aio_stop(true),
        write_thread(this),
        write_finish_thread(this), trace_endpoint("0.0.0.0", 0, "FileJournal") {

        if (aio && !directio) {
            lderr(cct) <<
                "FileJournal::_open_any: aio not supported without directio; disabling aio"
                << dendl;
            aio = false;
        }
#ifndef HAVE_LIBAIO
   ...
   */
    read_entry_result do_read_entry(off64_t pos,    ///< [in] position to read
    ...
    bool read_entry(bufferlist & bl, uint64_t & last_seq, bool * corrupt);

    bool read_entry(bufferlist & bl, uint64_t & last_seq) override {
