...
void RGWOp_BILog_List::execute(optional_yield y)
{
    ...
    do {
        ...
        int ret = static_cast<rgw::sal::RadosStore *>(driver)->svc()->bilog_rados->log_list(s, bucket->get_info(), log_layout,
                  shard_id,
...
void RGWOp_BILog_Status::execute(optional_yield y)
{
    ...
    if (!merge) {
        ...
        op_ret = rgw_read_bucket_full_sync_status(
                     ...
                     s->yield);
        if (op_ret < 0) {
            ldpp_dout(this, -1) << "ERROR: rgw_read_bucket_full_sync_status() on pipe=" << pipe << " returned ret=" << op_ret <<
                                dendl;
            return;
        }
        ...
        op_ret = rgw_read_bucket_inc_sync_status(
                     ...
                     &status.inc_status);
        if (op_ret < 0) {
            ldpp_dout(this, -1) << "ERROR: rgw_read_bucket_inc_sync_status() on pipe=" << pipe << " returned ret=" << op_ret <<
                                dendl;
        }
        return;
    }
    ...
    for (auto &entry : local_dests) {
        ...
        if (!pipe.dest.bucket) {
            ...
            ldpp_dout(this, 20) << "ERROR: RGWOp_BILog_Status::execute(optional_yield y): BUG: pipe.dest.bucket was not initialized"
                                << pipe << dendl;
            op_ret = -EIO;
            return;
        }
