...
struct Subscription {
    ...
    bool incremental_onetime;   // has CEPH_FEATURE_INCSUBOSDMAP
     ...
     Subscription(MonSession * s, const string & t):session(s), type(t),
        type_item(this), next(0), onetime(false), incremental_onetime(false) {
}};
...
struct MonSession:public RefCountedObject {
    ...
    uint64_t proxy_tid;
     ...
     MonSession(const entity_inst_t & i,
                Connection * c):RefCountedObject(g_ceph_context), con(c),
        con_type(c->get_peer_type()), con_features(0), inst(i), closed(false),
        item(this), auid(0), global_id(0), osd_epoch(0), auth_handler(NULL),
...
struct MonSessionMap {
    ...
    unsigned get_size() const {
        ...
        assert(!s->closed);
        for (map < string, Subscription * >::iterator p = s->sub_map.begin();
             p != s->sub_map.end(); ++p) {
        ...
        if (s->inst.name.is_osd()) {
            for (multimap < int, MonSession * >::iterator p =
                 by_osd.find(s->inst.name.num());
                 p->first == s->inst.name.num(); ++p)
                if (p->second == s) {
                    by_osd.erase(p);
                    break;
                }
        ...
        while (backward || forward) {
            if (backward) {
                if (osdmap->is_up(b->first) &&
                    osdmap->get_addr(b->first) ==
                    b->second->con->get_peer_addr()) {
                    s = b->second;
                    break;
                }
                if (b != by_osd.begin())
