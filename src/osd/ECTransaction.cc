                                          ...
                                          require_osd_release)
{
  ...
  for (auto && i:entries) {
    ...
    t.safe_create_traverse([&]
                           ...
                           map < string,
                           std::optional < bufferlist > >xattr_rollback;
                           ceph_assert(hinfo); bufferlist old_hinfo;
                           ...
                           ceph_assert(op.truncate->first ==
                                       op.truncate->second); ceph_assert(entry);
                           ceph_assert(obc);
          ...
          for (auto && st:      *transactions) {
                                 ...
                                 [&](const PGTransaction::ObjectOperation::
                                     Init::Rename & op) {
          ceph_assert(op.source.is_temp()); for (auto && st:*transactions)
                                 {
                                 st.second.
          ...
          map < string, bufferlist, less <>> to_set; for (auto && j:op.attr_updates)
                           {
                           if (j.second) {
          ...
          uint64_t object_size = sinfo.logical_to_next_chunk_offset(op.alloc_hint->expected_object_size); uint64_t write_size = sinfo.logical_to_next_chunk_offset(op.alloc_hint->expected_write_size); for (auto && st:*transactions)
                           ...
                           ceph_assert(!op.is_fresh_object()); new_size = sinfo.logical_to_next_stripe_offset(op.truncate->first); ldpp_dout(dpp, 20) << __func__ << ": new_size truncate down " << new_size << dendl; if (new_size != op.truncate->first) {    // 0 the unaligned part
                           bufferlist bl;
      ...
      uint32_t fadvise_flags = 0; for (auto && extent:op.buffer_updates)
                           ...
                           << dendl; ceph_assert(len > 0); if (off > new_size) {
      ...
      auto to_overwrite = to_write.intersect(0, append_after); ldpp_dout(dpp, 20) << __func__ << ": to_overwrite: " << to_overwrite << dendl; for (auto && extent:to_overwrite)
                           {
                           ceph_assert(extent.get_off() + extent.get_len() <=
                                       append_after);
                           ceph_assert(sinfo.
      ...
      auto to_append = to_write.intersect(append_after, std::numeric_limits < uint64_t >::max() - append_after); ldpp_dout(dpp, 20) << __func__ << ": to_append: " << to_append << dendl; for (auto && extent:to_append)
                           ...
                           ceph_assert(hinfo->get_total_logical_size(sinfo) ==
                                       new_size);}
