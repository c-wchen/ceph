...
struct LevelDBStoreStats {
    ...
    LevelDBStoreStats(): bytes_total(0),
        bytes_sst(0), bytes_log(0), bytes_misc(0)
    {
    } void dump(ceph::Formatter *f) const
    {
        ceph_assert(f != NULL);
...
{
    return out << "ScrubResult(keys " << r.prefix_keys << " crc " << r.
           prefix_crc << ")";
}
...
public:
    ...
    explicit constexpr mon_feature_t(const uint64_t f): features(f)
    {
    } mon_feature_t(): features(0)
    {
    }

    constexpr mon_feature_t(const mon_feature_t &o): features(o.features)
    {
    }
...
constexpr mon_feature_t get_supported()
{
    return (FEATURE_KRAKEN |
            ...
            FEATURE_OCTOPUS |
            FEATURE_PACIFIC |
            FEATURE_PINGING | FEATURE_QUINCY | FEATURE_REEF | FEATURE_NONE);
}
/**
...
static inline ceph_release_t infer_ceph_release_from_mon_features(mon_feature_t
        f)
{
...
inline mon_feature_t ceph::features::mon::get_feature_by_name(const std::
        string &n)
{
