  ...
  public:
    ...
    void _respond(int r) override {
      ...
      for (auto & m:batch_reqs) {
        ...
        batch_reqs.clear();
        server->reply_client_request(mdr,
                                     make_message < MClientReply >
                                     (*mdr->client_request, r));
    }
...
{
    PerfCountersBuilder plb(g_ceph_context, "mds_server", l_mdss_first,
                            l_mdss_last);
    ...
    plb.add_time_avg(l_mdss_req_lookupparent_latency,
    ...
    plb.add_time_avg(l_mdss_req_setdirlayout_latency,
...
inject_rename_corrupt_dentry_first(g_conf().get_val <
                                   double
...
*/
    ...
    if (m->get_type() == CEPH_MSG_CLIENT_REQUEST && !mds->is_active()) {
        ...
        if (mds->is_reconnect()
            ...
            if (!session || (!session->is_open() && !sessionclosed_isok)) {
                dout(5) << "session is closed, dropping " << req->
                    get_reqid() << dendl;
                return;
            }
            ...
            if (req->is_replay() || req->is_async()) {
                ...
                queue_replay = true;
                if (req->head.ino &&
                    !session->have_completed_request(req->get_reqid().tid,
                                                     nullptr)) {
    ...
    case MSG_MDS_PEER_REQUEST:
        ...
        return;
    default:
        derr << "Server unknown message " << m->
            get_type() << " from peer type " << m->get_connection()->
            get_peer_type() << dendl;
        ceph_abort_msg("server unknown message  " + to_string(m->get_type()) +
                       " from peer type " +
...
class C_MDS_session_finish:public ServerLogContext {
  ...
  } C_MDS_session_finish(Server * srv, Session * se, uint64_t sseq, bool s, version_t mv, const interval_set < inodeno_t > &to_free, version_t iv, const interval_set < inodeno_t > &to_purge, LogSegment * _ls, Context * fin_ = nullptr):
    ServerLogContext(srv), session(se), state_seq(sseq), open(s), cmapv(mv),
        inos_to_free(to_free), inotablev(iv), inos_to_purge(to_purge), ls(_ls),
        fin(fin_) {
    }
    void finish(int r) override {
        ceph_assert(r == 0);
        server->_session_logged(session, state_seq, open, cmapv, inos_to_free,
                                inotablev, inos_to_purge, ls);
        if (fin) {
            fin->complete(r);
        }
    }
...
void Server::reclaim_session(Session * session,
                             const cref_t < MClientReclaim > &m)
{
    ...
    if (target) {
        if (session->info.auth_name != target->info.auth_name) {
            dout(10) << __func__ << " session auth_name " << session->info.
                auth_name << " != target auth_name " << target->info.
                auth_name << dendl;
    ...
    if (target) {
        ...
        if (reply) {
            ...
            send_reply = new LambdaContext([this, session_id, reply] (int r) {
                                           ...
                                           Session * session =
                                           mds->sessionmap.
                                           get_session(entity_name_t::
                                                       CLIENT(session_id));
                                           if (!session) {
                                           return;}
                                           auto epoch =
                                           mds->objecter->
                                           with_osdmap([](const OSDMap & map) {
                                                       return map.get_epoch();}
                                           );
                                           reply->set_epoch(epoch);
                                           mds->send_message_client(reply,
                                                                    session);}
            ) ;
        }
        else {
        ...
        bool blocklisted =
            mds->objecter->with_osdmap([target] (const OSDMap & map){
                                       return map.is_blocklisted(target->info.
                                                                 inst.addr);}
        );
        ...
        if (blocklisted || !g_conf()->mds_session_blocklist_on_evict) {
            ...
            mds->evict_client(target->get_client().v, false, true, *css,
                              send_reply);
        }
    }
    else if (reply) {
...
void Server::handle_client_session(const cref_t < MClientSession > &m)
{
    ...
    dout(3) << "handle_client_session " << *m << " from " << m->
        get_source() << dendl;
    ceph_assert(m->is_a_client());  // should _not_ come from an mds!
    ...
    case CEPH_SESSION_REQUEST_OPEN:
        ...
        if (session->is_opening() ||
            ...
            if (m->supported_features.test(CEPHFS_FEATURE_NOTIFY_SESSION_STATE)) {
                if (session->is_open() && !mds->is_stopping()) {
                    ...
                    auto reply =
                        make_message < MClientSession > (CEPH_SESSION_OPEN,
                                                         session->
                                                         get_push_seq());
...

        {
            ...
            session->
                set_client_metadata(client_metadata_t
                                    (m->metadata, m->supported_features,
                                     m->metric_spec));
            auto & client_metadata = session->info.client_metadata;

            auto log_session_status =
                [this, m, session] (std::string_view status,
                                    std::string_view err) {
                auto now = ceph_clock_now();
                auto throttle_elapsed =
                    m->get_recv_complete_stamp() - m->get_throttle_stamp();
            ...
            auto send_reject_message =
                [this, &session, &log_session_status] (std::string_view err_str,
                                                       unsigned flags = 0){
                auto m =
                    make_message < MClientSession > (CEPH_SESSION_REJECT, 0,
                                                     flags);
                if (session->info.has_feature(CEPHFS_FEATURE_MIMIC))
            ...
            bool blocklisted = mds->objecter->with_osdmap([&addr]
                                                          (const OSDMap &
                                                           osd_map)->bool {
                                                          return osd_map.
                                                          is_blocklisted(addr);}
            ...
            dout(20) << " features: '" << client_metadata.
                features << "'" << dendl;
            dout(20) << " metric specification: [" << client_metadata.
                metric_spec << "]" << dendl;
          for (const auto & p:client_metadata) {
            ...
            if (auto it = client_metadata.find("root");
                it != client_metadata.end()) {
                ...
                else if (!session->auth_caps.
                         path_capable(claimed_root.substr(1))) {
                    denied = true;
            ...
            if (auto it = client_metadata.find("uuid");
                it != client_metadata.end()) {
                if (find_session_by_uuid(it->second)) {
                    send_reject_message("duplicated session uuid");
                    mds->clog->
                        warn() <<
                        "client session with duplicated session uuid '" << it->
                        second << "' denied (" << session->info.inst << ")";
                    session->clear();
                    break;
                }
            }
            ...
            auto fin =
                new
                LambdaContext([log_session_status =
                               std::move(log_session_status)] (int r){
                              ceph_assert(r == 0);
                              log_session_status("ACCEPTED", "");});
            mdlog->
                start_submit_entry(new
                                   ESession(m->get_source_inst(), true, pv,
                                            client_metadata),
                                   new C_MDS_session_finish(this, session, sseq,
                                                            true, pv, fin));
            mdlog->flush();
        }
        break;
    ...
    case CEPH_SESSION_REQUEST_RENEWCAPS:
        if (session->is_open() || session->is_stale()) {
            ...
            auto reply =
                make_message < MClientSession > (CEPH_SESSION_RENEWCAPS,
                                                 m->get_seq());
            mds->send_message_client(reply, session);
        }
        else {
    ...
    case CEPH_SESSION_REQUEST_CLOSE:
        {
            if (session->is_closed() ||
                ...
                dout(10) << "already closed|closing|killing, dropping this req"
                    << dendl;
                return;
            }
            ...
            ceph_assert(session->is_open() ||
                        session->is_stale() || session->is_opening());
            if (m->get_seq() < session->get_push_seq()) {
                dout(10) << "old push seq " << m->
                    get_seq() << " < " << session->get_push_seq()
                    << ", dropping" << dendl;
                return;
            }
            ...
            //
            if (m->get_seq() != session->get_push_seq()) {
                dout(0) << "old push seq " << m->
                    get_seq() << " != " << session->get_push_seq()
                    << ", BUGGY!" << dendl;
                mds->clog->warn() << "incorrect push seq " << m->
                    get_seq() << " != " << session->
                    get_push_seq() << ", dropping" << " from client : " <<
                    session->get_human_name();
                return;
            }
    ...
    default:
        ...
        mds->send_message_client(m, session);
        derr << "Server received unknown message " << m->
            get_type() << ", closing session and blocklisting the client " <<
...
void Server::flush_session(Session * session, MDSGatherBuilder & gather)
{
    ...
    mds->send_message_client(make_message < MClientSession >
                             (CEPH_SESSION_FLUSHMSG, seq), session);
}

void Server::flush_client_sessions(set < client_t > &client_set,
...
void Server::_session_logged(Session * session, uint64_t state_seq, bool open,
                             version_t pv,
                             ...
                             LogSegment * ls)
{
    ...
    if (!open) {
        if (inos_to_purge.size()) {
            ...
            if (mds->is_clientreplay() || mds->is_active()
                || mds->is_stopping())
        ...
        if (inos_to_free.size()) {
            ceph_assert(piv);
            ceph_assert(session->is_closing() || session->is_killing() || session->is_opening());   // re-open closing session
    ...
    if (session->get_state_seq() != state_seq) {
        ...
        while (!session->caps.empty()) {
            ...
            dout(20) << " killing capability " << ccap_string(cap->
                                                              issued()) <<
                " on " << *in << dendl;
        ...
        if (client_reconnect_gather.erase(session->info.get_client())) {
            ...
            if (client_reconnect_gather.empty()) {
                dout(7) << " client " << session->info.
                    inst << " was last reconnect, finishing" << dendl;
        ...
        if (client_reclaim_gather.erase(session->info.get_client())) {
            ...
            if (client_reclaim_gather.empty()) {
                dout(7) << " client " << session->info.
                    inst << " was last reclaimed, finishing" << dendl;
 ...
 */
version_t Server::prepare_force_open_sessions(map < client_t,
                                              entity_inst_t > &cm,
                                              map < client_t,
                                              client_metadata_t > &cmm,
                                              map < client_t, pair < Session *,
                                              uint64_t > >&smap)
{
    ...
    mds->objecter->with_osdmap([this, &cm, &cmm] (const OSDMap & osd_map) {
                               ...
                               dout(10) << " ignoring blocklisted client." <<
                               p->first << " (" << p->second.
                               addr << ")" << dendl; cmm.erase(p->first);
                               cm.erase(p++);}
                               else {
                               ++p;}
                               }
    ...
    for (map < client_t, entity_inst_t >::iterator p = cm.begin();
         p != cm.end(); ++p) {
        ...
        if (session->is_closed() ||
            ...
            ceph_assert(session->is_open() ||
                        session->is_opening() || session->is_stale());
            sseq = 0;
        }
...
void Server::finish_force_open_sessions(const map < client_t, pair < Session *,
                                        uint64_t > >&smap, bool dec_import)
{
    /*
  ...
  for (auto & it:smap) {
        ...
        if (sseq > 0) {
            if (session->get_state_seq() != sseq) {
                dout(10) << "force_open_sessions skipping changed " << session->
                    info.inst << dendl;
            }
            else {
                dout(10) << "force_open_sessions opened " << session->info.
                    inst << dendl;
                mds->sessionmap.set_state(session, Session::STATE_OPEN);
                mds->sessionmap.touch_session(session);
                metrics_handler->add_session(session);
    ...
    dout(10) << __func__ << ": final v " << mds->sessionmap.
        get_version() << dendl;
}
...
void Server::terminate_sessions()
{
    ...
    for (set < Session * >::const_iterator p = sessions.begin();
        ...
        Session *session = *p;
        if (session->is_closing() ||
            session->is_killing() || session->is_closed())
            continue;
        journal_close_session(session, Session::STATE_CLOSING, NULL);
    }
...
void Server::find_idle_sessions()
{
    ...
    if (last_cleared_laggy < cutoff) {
        dout(10) << " last cleared laggy " << last_cleared_laggy <<
            "s ago (< cutoff " << cutoff << "), not marking any client stale" <<
            dendl;
        return;
    }
    ...
    bool defer_session_stale =
        g_conf().get_val < bool > ("mds_defer_session_stale");
    ...
    if (sessions_p1 != mds->sessionmap.by_state.end()
      ...
      for (auto session:*(sessions_p1->second)) {
            auto last_cap_renew_span =
                std::chrono::duration <
                double >(now - session->last_cap_renew).count();
            ...
            if (session->last_seen > session->last_cap_renew) {
                last_cap_renew_span =
                    std::chrono::duration <
                    double >(now - session->last_seen).count();
                if (last_cap_renew_span < cutoff) {
                    dout(20) << "laggiest active session is " << session->info.
                        inst << " and renewed caps recently (" <<
            ...
            if (last_cap_renew_span >= mds->mdsmap->get_session_autoclose()) {
                dout(20) << "evicting session " << session->info.
                    inst << " since autoclose " "has arrived" << dendl;
            ...
            if (defer_session_stale &&
                ...
                !mds->locker->is_revoking_any_caps_from(session->
                                                        get_client())) {
                dout(20) << "deferring marking session " << session->info.
                    inst << " stale " "since it holds no caps" << dendl;
                continue;
            }
    ...
    const auto sessions_p2 =
        mds->sessionmap.by_state.find(Session::STATE_STALE);
  ...
  for (auto session:to_evict) {
        if (session->is_importing()) {
            dout(10) << "skipping session " << session->info.
                inst << ", it's being imported" << dendl;
            continue;
        }
        ...
        auto last_cap_renew_span =
            ...
            double >(now - session->last_cap_renew).count();
        mds->clog->
            warn() << "evicting unresponsive client " << *session << ", after "
            << last_cap_renew_span << " seconds";
        dout(10) << "autoclosing stale session " << session->info.
            inst << " last renewed caps " << last_cap_renew_span << "s ago" <<
            dendl;
...
void Server::handle_conf_change(const std::set < std::string > &changed)
{
    ...
    if (changed.count("mds_max_snaps_per_dir")) {
        max_snaps_per_dir =
            g_conf().get_val < uint64_t > ("mds_max_snaps_per_dir");
...
void Server::kill_session(Session * session, Context * on_safe)
{
    ...
    if ((session->is_opening() ||
         session->is_open() ||
         session->is_stale()) && !session->is_importing()) {
        dout(10) << "kill_session " << session << dendl;
        journal_close_session(session, Session::STATE_KILLING, on_safe);
    }
    else {
        dout(10) << "kill_session importing or already closing/killing " <<
            session << dendl;
        if (session->is_closing() || session->is_killing()) {
            if (on_safe)
                mdlog->
                    wait_for_safe(new MDSInternalContextWrapper(mds, on_safe));
        }
        else {
                                   ...
                                   Context * on_safe)
{
    ...
    auto le =
        new ESession(session->info.inst, false, pv, inos_to_free, piv,
                     session->delegated_inos);
    auto fin =
        new C_MDS_session_finish(this, session, sseq, false, pv, inos_to_free,
                                 piv,
                                 session->delegated_inos,
                                 mdlog->get_current_segment(), on_safe);
...
void Server::reconnect_clients(MDSContext * reconnect_done_)
{
  ...
  for (auto session:sessions) {
    ...
    reconnect_start = now;
    dout(1) << "reconnect_clients -- " << client_reconnect_gather.
        size() << " sessions" << dendl;
    mds->sessionmap.dump();
}
...
void Server::handle_client_reconnect(const cref_t < MClientReconnect > &m)
{
    ...
    bool reconnect_all_deny =
        g_conf().get_val < bool > ("mds_deny_all_reconnect");
    ...
    auto delay =
        std::chrono::duration <
        double >(clock::now() - reconnect_start).count();
    ...
    if (reconnect_all_deny || !mds->is_reconnect()
        ...
        if (reconnect_all_deny) {
            dout(1) <<
                "mds_deny_all_reconnect was set to speed up reboot phase, ignoring reconnect, sending close"
                << dendl;
        }
        else {
            dout(1) <<
        ...
        mds->clog->info() << "denied reconnect attempt (mds is "
            ...
            << " after " << delay << " (allowed interval " << g_conf()->
        ...
        if (!session->is_open()) {
            ...
            if (session->info.client_metadata.features.empty())
                infer_supported_features(session,
        ...
        if (!error_str.empty()) {
            ...
            dout(1) << " " << error_str << ", ignoring reconnect, sending close"
                << dendl;
            mds->clog->info() << "denied reconnect attempt from " << m->
    ...
    if (!m->has_more()) {
        ...
        mds->send_message_client(reply, session);
        mds->clog->debug() << "reconnect by " << session->info.
            inst << " after " << delay;
    }
  ...
  for (const auto & r:m->realms) {
        ...
        if (in) {
            if (in->snaprealm) {
                ...
                dout(15) << "open snaprealm (null snaprealm) on " << *in <<
                    dendl;
            }
            mdcache->add_reconnected_snaprealm(from, inodeno_t(r.realm.ino),
                                               snapid_t(r.realm.seq));
        }
        else {
            dout(15) << "open snaprealm (w/o inode) on " << inodeno_t(r.realm.
                                                                      ino)
  ...
  for (const auto & p:m->caps) {
        ...
        if (in && in->is_auth()) {
            ...
            dout(15) << "open cap realm " << inodeno_t(p.second.capinfo.
                                                       snaprealm)
                << " on " << *in << dendl;
        ...
        if (in && !in->is_auth()) {
            // not mine.
            dout(10) << "non-auth " << *in << ", will pass off to authority" <<
                dendl;
            ...
            dout(10) << "missing ino " << p.
                first << ", will load later" << dendl;
    ...
    if (!m->has_more()) {
        mdcache->rejoin_recovered_client(session->get_client(),
                                         session->info.inst);
...
void Server::infer_supported_features(Session * session,
                                      client_metadata_t & client_metadata)
{
    ...
    if (it != client_metadata.end()) {
        ...
        else if (session->get_connection()->
                 has_feature(CEPH_FEATURE_FS_CHANGE_ATTR))
        ...
        if (it != client_metadata.end()) {
            // kernel client
            if (session->get_connection()->
                has_feature(CEPH_FEATURE_NEW_OSDOP_ENCODING))
                supported = CEPHFS_FEATURE_LUMINOUS;
        }
    }
    if (supported == -1 &&
    ...
    if (supported >= 0) {
        ...
        dout(10) << __func__ << " got '" << client_metadata.
            features << "'" << dendl;
    }
}
...
void Server::update_required_client_features()
{
    ...
    dout(7) << "required_client_features: " << required_client_features <<
        dendl;
    ...
    if (mds->get_state() >= MDSMap::STATE_RECONNECT) {
      ...
      for (auto session:sessions) {
            ...
            missing_features -= session->info.client_metadata.features;
            if (!missing_features.empty()) {
                bool blocklisted = mds->objecter->with_osdmap([session]
                                                              (const OSDMap &
                                                               osd_map)->bool {
                                                              return osd_map.
                                                              is_blocklisted
...
void Server::reconnect_tick()
{
    ...
    if (reconnect_all_deny
        && (client_reconnect_gather == client_reconnect_denied))
        reject_all_reconnect = true;

    auto now = clock::now();
    auto elapse1 =
        std::chrono::duration < double >(now - reconnect_start).count();
  ...
  for (auto c:client_reconnect_gather) {
        Session *session =
            mds->sessionmap.get_session(entity_name_t::CLIENT(c.v));
        ceph_assert(session);
  ...
  for (auto session:remaining_sessions) {
        ...
        mds->clog->warn() << "evicting unresponsive client " << *session
            << ", after waiting " << elapse1 << " seconds during MDS startup";
    ...
    if (gather.has_subs()) {
        ...
        gather.
            set_finisher(new
                         MDSInternalContextWrapper(mds,
                                                   new
                                                   LambdaContext([this] (int r)
                                                                 {
                                                                 reconnect_gather_finish
                                                                 ();
                                                                 })));
...
void Server::recover_filelocks(CInode * in, bufferlist locks, int64_t client)
{
    ...
    for (int i = 0; i < numlocks; ++i) {
        decode(lock, p);
        lock.client = client;
        in->get_fcntl_lock_state()->held_locks.insert(pair < uint64_t,
                                                      ceph_filelock >
                                                      (lock.start, lock));
    ...
    for (int i = 0; i < numlocks; ++i) {
        decode(lock, p);
        lock.client = client;
        in->get_flock_lock_state()->held_locks.insert(pair < uint64_t,
                                                      ceph_filelock >
                                                      (lock.start, lock));
 ...
 */
std::pair < bool,
    uint64_t > Server::recall_client_state(MDSGatherBuilder * gather,
                                           RecallFlags flags)
{
    ...
    auto f =
        [&caps_session, enforce_max, enforce_liveness, trim,
        ...
        if (trim || (enforce_max && num_caps > max_caps_per_client)
            ...
            caps_session.emplace(std::piecewise_construct,
  ...
  for (const auto &[num_caps, session]:boost::adaptors::
         reverse(caps_session)) {
        ...
        dout(10) << __func__ << ":"
            ...
            << " caps " << num_caps << ", leases " << session->leases.size()
            << dendl;
        ...
        if (num_caps > newlim) {
            ...
            uint64_t recall =
                std::min < uint64_t > (recall_max_caps, num_caps - newlim);
            ...
            if (session_recall_throttle + recall > recall_max_decay_threshold) {
                dout(15) << "  session recall threshold (" <<
                    recall_max_decay_threshold << ") hit at " <<
                    session_recall_throttle << "; skipping!" << dendl;
                throttled = true;
                continue;
            }
            else if (session_recall_throttle2o + recall > recall_max_caps * 2) {
                dout(15) << "  session recall 2nd-order threshold (" << 2 *
                    recall_max_caps << ") hit at " << session_recall_throttle2o
                    << "; skipping!" << dendl;
                throttled = true;
                continue;
            }
            else if (global_recall_throttle + recall >
            ...
            if (steady) {
                ...
                else if (recall < recall_max_caps
            ...
            auto m =
                make_message < MClientSession > (CEPH_SESSION_RECALL_STATE);
 ...
 */
void Server::journal_and_reply(MDRequestRef & mdr, CInode * in, CDentry * dn,
...
void Server::submit_mdlog_entry(LogEvent * le, MDSLogContextBase * fin,
...
void Server::perf_gather_op_latency(const cref_t < MClientRequest > &req,
                                    utime_t lat)
{
...
void Server::early_reply(MDRequestRef & mdr, CInode * tracei, CDentry * tracedn)
{
    ...
    mds->locker->set_xlocks_done(mdr.get(),
 ...
 * Clean up mdr
 */
void Server::reply_client_request(MDRequestRef & mdr,
                                  const ref_t < MClientReply > &reply)
{
    ...
    if (req->may_write() && req->get_op() != CEPH_MDS_OP_SETFILELOCK &&
        ...
        inodeno_t created =
            mdr->alloc_ino ? mdr->alloc_ino : mdr->used_prealloc_ino;
    ...
    if (req->is_queued_for_replay() &&
        ...
        if (reply->get_result() < 0) {
            ...
            mds->clog->warn() << "failed to replay " << req->
                get_reqid() << " error " << r;
        }
                            ...
                            CInode * in, CDentry * dn, MDRequestRef & mdr)
{
    ...
    if (snapid == CEPH_NOSNAP) {
        ...
        dout(10) << "set_trace_dist snaprealm " << *realm << " len=" << reply->
            snapbl.length() << dendl;
    }
...
void Server::handle_client_request(const cref_t < MClientRequest > &req)
{
    ...
    if (req->is_replay() || req->get_retry_attempt()) {
        ...
        if (session->have_completed_request(req->get_reqid().tid, &created)) {
            ...
            if (req->get_op() != CEPH_MDS_OP_OPEN &&
                req->get_op() != CEPH_MDS_OP_CREATE) {
                dout(10) << " completed request which created new inode " <<
                    created << ", convert it to lookup request" << dendl;
                req->head.op =
                    req->
    ...
    if (req->get_oldest_client_tid() > 0) {
        ...
        if (session->trim_completed_requests(req->get_oldest_client_tid())) {
            ...
            mdlog->get_current_segment()->touched_sessions.insert(session->info.
                                                                  inst.name);
    ...
    if (!req->releases.empty() && req->is_a_client() && !req->is_replay()) {
      ...
      for (const auto & r:req->releases) {
            mds->locker->process_request_cap_release(mdr, client, r.item,
                                                     r.dname);
        }
        req->releases.clear();
    }
...
void Server::handle_osd_map()
{
    ...
    mds->objecter->with_osdmap([this] (const OSDMap & o) {
                               auto pi =
                               o.get_pg_pool(mds->get_metadata_pool());
                               is_full = pi
...
void Server::dispatch_client_request(MDRequestRef & mdr)
{
    ...
    if (is_full) {
        ...
        if (req->get_op() == CEPH_MDS_OP_SETLAYOUT || req->get_op() == CEPH_MDS_OP_SETDIRLAYOUT || req->get_op() == CEPH_MDS_OP_SETLAYOUT || req->get_op() == CEPH_MDS_OP_RMXATTR || req->get_op() == CEPH_MDS_OP_SETXATTR || req->get_op() == CEPH_MDS_OP_CREATE || req->get_op() == CEPH_MDS_OP_SYMLINK || req->get_op() == CEPH_MDS_OP_MKSNAP || ((req->get_op() == CEPH_MDS_OP_LINK || req->get_op() == CEPH_MDS_OP_RENAME) && (!mdr->has_more() || mdr->more()->witnessed.empty()))    // haven't started peer request
            ) {
            ...
            if (check_access(mdr, cur, MAY_FULL)) {
                ...
                dout(20) << __func__ <<
                    ": full, responding CEPHFS_ENOSPC to op " <<
                    ceph_mds_op_name(req->get_op()) << dendl;
...
{
    dout(4) << "handle_peer_request " << m->get_reqid() << " from " << m->
        get_source() << dendl;
    ...
    if (m->get_op() == MMDSPeerRequest::OP_RENAMENOTIFY) {
        auto reply =
            make_message < MMDSPeerRequest > (m->get_reqid(), m->get_attempt(),
                                              MMDSPeerRequest::
                                              OP_RENAMENOTIFYACK);
        mds->send_message(reply, m->get_connection());
        return;
    ...
    if (mdcache->have_request(m->get_reqid())) {
        ...
        if (mdr->attempt > m->get_attempt()) {
            dout(10) << "local request " << *mdr << " attempt " << mdr->
                attempt << " > " << m->get_attempt()
                << ", dropping " << *m << dendl;
            return;
        }
...
void Server::handle_peer_request_reply(const cref_t < MMDSPeerRequest > &m)
{
    ...
    if (!mds->is_clientreplay() && !mds->is_active() && !mds->is_stopping()) {
        ...
        if (!mdcache->have_uncommitted_leader(r, from)) {
            dout(10) <<
                "handle_peer_request_reply ignoring peer reply from mds." <<
                from << " reqid " << r << dendl;
            return;
        }
        dout(3) << "not clientreplay|active yet, waiting" << dendl;
    ...
    if (m->get_attempt() != mdr->attempt) {
        dout(10) << "handle_peer_request_reply " << *mdr <<
            " ignoring reply from other attempt " << m->get_attempt() << dendl;
        return;
    }
    ...
    case MMDSPeerRequest::OP_XLOCKACK:
        {
            ...
            dout(10) << "got remote xlock on " << *lock << " on " << *lock->
                get_parent() << dendl;
    ...
    case MMDSPeerRequest::OP_WRLOCKACK:
        {
            ...
            SimpleLock *lock = mds->locker->get_lock(m->get_lock_type(),
                                                     m->get_object_info());
            mdr->more()->peers.insert(from);
            dout(10) << "got remote wrlock on " << *lock << " on " << *lock->
                get_parent() << dendl;
            auto it =
                mdr->emplace_lock(lock, MutationImpl::LockOp::REMOTE_WRLOCK,
                                  from);
...
{
    dout(7) << "dispatch_peer_request " << *mdr << " " << *mdr->
        peer_request << dendl;
    ...
    case MMDSPeerRequest::OP_WRLOCK:
        {
            // identify object
            SimpleLock *lock =
                mds->locker->get_lock(mdr->peer_request->get_lock_type(),
            ...
            if (op == MMDSPeerRequest::OP_XLOCK
              ...
              for (const auto & p:mdr->locks) {
                ...
                // ack
                auto r =
                    make_message < MMDSPeerRequest > (mdr->reqid, mdr->attempt,
                                                      replycode);
    ...
    case MMDSPeerRequest::OP_UNWRLOCK:
        {
            ...
            if (need_issue)
                mds->locker->issue_caps(static_cast <
    ...
    if (!fail) {
      for (const auto & obj:objects) {
            ...
            if (!mdr->can_auth_pin(obj)) {
                if (nonblocking) {
                    dout(10) << " can't auth_pin (freezing?) " << *obj <<
                        " nonblocking" << dendl;
                    fail = true;
                    wouldblock = true;
                    break;
                ...
                obj->add_waiter(CDir::WAIT_UNFREEZE,
    ...
    if (!fail) {
        ...
        if (auth_pin_freeze) {
            ...
            if (!mdr->freeze_auth_pin(auth_pin_freeze)) {
                auth_pin_freeze->add_waiter(CInode::WAIT_FROZEN,
...
void Server::handle_peer_auth_pin_ack(MDRequestRef & mdr,
                                      const cref_t < MMDSPeerRequest > &ack)
{
  ...
  for (auto & p:mdr->object_states) {
    ...
    if (mdr->more()->waiting_on_peer.empty())
        ...
        dout(10) << "still waiting on peers " << mdr->more()->
            waiting_on_peer << dendl;
}
...
{
    if (mdr->session) {
        int r = mdr->session->check_access(in, mask,
                                           mdr->client_request->
                                           get_caller_uid(),
                                           mdr->client_request->
                                           get_caller_gid(),
...
bool Server::check_fragment_space(MDRequestRef & mdr, CDir * dir)
{
    ...
    if (size >= max) {
        dout(10) << "fragment " << *dir << " size exceeds " << max <<
            " (CEPHFS_ENOSPC)" << dendl;
        respond_to_request(mdr, -CEPHFS_ENOSPC);
        return false;
    }
...
bool Server::check_dir_max_entries(MDRequestRef & mdr, CDir * in)
{
    ...
    if (dir_max_entries && size >= dir_max_entries) {
        dout(10) << "entries per dir " << *in << " size exceeds " <<
            dir_max_entries << " (ENOSPC)" << dendl;
...
CDentry *Server::prepare_stray_dentry(MDRequestRef & mdr, CInode * in)
{
    ...
    if (!straydn) {
        if (straydir->is_frozen_dir()) {
            ...
            mds->locker->drop_locks(mdr.get());
            mdr->drop_local_auth_pins();
            straydir->add_waiter(CInode::WAIT_UNFREEZE,
                                 new C_MDS_RetryRequest(mdcache, mdr));
            return nullptr;
        }
 ...
 */
CInode *Server::prepare_new_inode(MDRequestRef & mdr, CDir * dir,
                                  inodeno_t useino, unsigned mode,
                                  const file_layout_t * layout)
{
    ...
    do {
        if (allow_prealloc_inos
            && (mdr->used_prealloc_ino = _inode->ino =
                mdr->session->take_ino(_useino))) {
            if (mdcache->test_and_clear_taken_inos(_inode->ino)) {
                _inode->ino = 0;
                dout(10) << "prepare_new_inode used_prealloc " << mdr->
                    used_prealloc_ino << " (" << mdr->session->info.
                    prealloc_inos.
                    size() << " left)" << " but has been taken, will try again!"
                    << dendl;
            }
            else {
            ...
            if (mdcache->test_and_clear_taken_inos(_inode->ino)) {
                ...
                dout(10) << "prepare_new_inode alloc " << mdr->
                    alloc_ino << dendl;
            }
        }
        _useino = 0;
    ...
    if (allow_prealloc_inos &&
        ...
        mds->sessionmap.mark_projected(mdr->session);
        dout(10) << "prepare_new_inode prealloc " << mdr->
            prealloc_inos << dendl;
    }
    ...
    dout(10) << oct << " dir mode 0" << pip->
        mode << " new mode 0" << mode << dec << dendl;
    ...
    dout(10) << "copying fscrypt_auth len " << req->fscrypt_auth.
        size() << dendl;
...
{
    dout(20) << "journal_allocated_inos sessionmapv " << mds->sessionmap.
        get_projected()
...
struct C_MDS_TryOpenInode:public ServerContext {
    MDRequestRef mdr;
    inodeno_t ino;
    C_MDS_TryOpenInode(Server * s, MDRequestRef & r,
                       inodeno_t i):ServerContext(s), mdr(r), ino(i) {
    } void finish(int r) override {
...
{
    dout(10) << "_try_open_ino " << mdr.
        get() << " ino " << ino << " r=" << r << dendl;
...
class C_MDS_TryFindInode:public ServerContext {
    ...
    inodeno_t ino;
  public:
    C_MDS_TryFindInode(Server * s, MDRequestRef & r, MDCache * m,
                       inodeno_t i):ServerContext(s), mdr(r), mdcache(m),
        ino(i) {
    } void finish(int r) override {
        if (r == -CEPHFS_ESTALE) {  // :( find_ino_peers failed
            ...
            if (in && in->state_test(CInode::STATE_PURGING))
                ...
                mdcache->open_ino(ino, (int64_t) - 1,
                                  new C_MDS_TryOpenInode(server, mdr, ino));
        }
        else {
...
CInode *Server::rdlock_path_pin_ref(MDRequestRef & mdr,
                                    bool want_auth, bool no_want_auth)
{
    ...
    int r =
        mdcache->path_traverse(mdr, cf, refpath, flags, &mdr->dn[0],
                               &mdr->in[0]);
    if (r > 0)
    ...
    if (r < 0) {                // error
        if (r == -CEPHFS_ENOENT && !mdr->dn[0].empty()) {
            ...
            dout(10) << "FAIL on CEPHFS_ESTALE but attempting recovery" <<
                dendl;
            inodeno_t ino = refpath.get_ino();
            mdcache->find_ino_peers(ino,
                                    new C_MDS_TryFindInode(this, mdr, mdcache,
                                                           ino));
        }
        else {
...
CDentry *Server::rdlock_path_xlock_dentry(MDRequestRef & mdr,
                                          bool create, bool okexist,
                                          bool authexist, bool want_layout)
{
    const filepath & refpath = mdr->get_filepath();
 ...
 */
std::pair < CDentry *,
    CDentry * >Server::rdlock_two_paths_xlock_destdn(MDRequestRef & mdr,
                                                     bool xlock_srcdn)
{
    ...
    CF_MDS_RetryRequestFactory cf(mdcache, mdr, true);
    int flags =
        MDS_TRAVERSE_RDLOCK_SNAP | MDS_TRAVERSE_WANT_DENTRY |
        MDS_TRAVERSE_WANT_AUTH;
    int r = mdcache->path_traverse(mdr, cf, refpath, flags, &mdr->dn[0]);
    if (r != 0) {
        if (r == -CEPHFS_ESTALE) {
    ...
    flags =
        MDS_TRAVERSE_RDLOCK_SNAP2 | MDS_TRAVERSE_WANT_DENTRY |
    ...
    if (!mdr->reqid.name.is_mds()) {
        if ((srcdir->get_inode()->is_system()
             && !srcdir->get_inode()->is_root())
 ...
 */
CDir *Server::try_open_auth_dirfrag(CInode * diri, frag_t fg,
                                    MDRequestRef & mdr)
{
    ...
    if (dir) {
        ...
        if (!diri->is_auth()) {
            ...
            dout(7) <<
                "try_open_auth_dirfrag: not open, not inode auth, fw to mds." <<
                inauth << dendl;
...
void Server::handle_client_getattr(MDRequestRef & mdr, bool is_lookup)
{
    ...
    if (!mdr->is_batch_head() && mdr->can_batch()) {
        ...
        if (r < 0) {
            ...
            mdr->pin(dn);
            auto em =
                dn->batch_ops.emplace(std::piecewise_construct,
                                      std::forward_as_tuple(mask),
                                      std::forward_as_tuple());
            if (em.second) {
                em.first->second =
                    std::make_unique < Batch_Getattr_Lookup > (this, mdr);
            }
            else {
                dout(20) << __func__ <<
            ...
            mdr->pin(in);
            auto em =
                in->batch_ops.emplace(std::piecewise_construct,
                                      std::forward_as_tuple(mask),
                                      std::forward_as_tuple());
            if (em.second) {
                em.first->second =
                                      ...
                                      bool want_parent, bool want_dentry)
{
    ...
    if (!in) {
        mdcache->open_ino(ino, (int64_t) - 1, new C_MDS_LookupIno2(this, mdr),
                          false);
        return;
    }
...
void Server::_lookup_snap_ino(MDRequestRef & mdr)
{
    ...
    dout(7) << "lookup_snap_ino " << vino << " parent " << parent_ino <<
        " hash " << hash << dendl;
    ...
    if (parent_ino) {
        ...
        if (!diri) {
            mdcache->open_ino(parent_ino, mds->get_metadata_pool(),
        ...
        mdcache->open_ino(vino.ino, mds->get_metadata_pool(),
...
void Server::handle_client_open(MDRequestRef & mdr)
{
    ...
    dout(10) << "open flags = " << flags
        << ", filemode = " << cmode << ", need_auth = " << need_auth << dendl;
    ...
    if ((flags & CEPH_O_TRUNC) && !mdr->has_completed) {
        ...
        if (pi->is_truncating()) {
            dout(10) << " waiting for pending truncate from " << pi->
                truncate_from << " to " << pi->
                truncate_size << " to complete on " << *cur << dendl;
            mds->locker->drop_locks(mdr.get());
            mdr->drop_local_auth_pins();
            cur->add_waiter(CInode::WAIT_TRUNC,
                            new C_MDS_RetryRequest(mdcache, mdr));
            return;
        }
    ...
    if (cur->is_file() || cur->is_dir()) {
        if (mdr->snapid == CEPH_NOSNAP) {
            // register new cap
            Capability *cap =
                mds->locker->issue_new_caps(cur, cmode, mdr, nullptr);
            if (cap)
...
class C_MDS_openc_finish:public ServerLogContext {
    ...
    CInode *newi;
  public:
    C_MDS_openc_finish(Server * s, MDRequestRef & r, CDentry * d,
                       CInode * ni):ServerLogContext(s, r), dn(d), newi(ni) {
    } void finish(int r) override {
        ceph_assert(r == 0);
...
void Server::handle_client_openc(MDRequestRef & mdr)
{
    ...
    if (req->get_alternate_name().size() > alternate_name_max) {
        dout(10) << " alternate_name longer than " << alternate_name_max <<
            dendl;
    ...
    if (req->get_connection()->has_feature(CEPH_FEATURE_CREATEPOOLID) &&
        ...
        if (req->get_mdsmap_epoch() > mds->mdsmap->get_epoch()) {
            mds->wait_for_mdsmap(req->get_mdsmap_epoch(),
                                 new C_MDS_RetryRequest(mdcache, mdr));
            return;
        }
    }
    ...
    // create inode.
    CInode *newi =
        prepare_new_inode(mdr, dn->get_dir(), inodeno_t(req->head.ino),
    ...
    if (mdr->session->info.has_feature(CEPHFS_FEATURE_DELEG_INO)) {
        ...
        if (delegate_inos_pct && !req->is_queued_for_replay()) {
            ...
            if (mdr->session->delegated_inos.size() <
                (unsigned)g_conf()->mds_client_prealloc_inos / frac / 2)
                mdr->session->delegate_inos(g_conf()->mds_client_prealloc_inos /
                                            frac, ocresp.delegated_inos);
        }

        encode(ocresp, mdr->reply_extra_bl);
    }
    else if (mdr->client_request->get_connection()->
             has_feature(CEPH_FEATURE_REPLY_CREATE_INODE)) {
        dout(10) << "adding ino to reply to indicate inode was created" <<
            dendl;
...
void Server::handle_client_readdir(MDRequestRef & mdr)
{
    ...
    if (num_caps > static_cast < uint64_t >
        (max_caps_per_client * max_caps_throttle_ratio)
        ...
        mds->timer.add_event_after(caps_throttle_retry_request_timeout,
                                   new C_MDS_RetryRequest(mdcache, mdr));
        return;
    }
    ...
    if (diri->dirfragtree[fg.value()] != fg) {
        ...
        dout(10) << " adjust frag " << fg << " -> " << newfg << " " << diri->
            dirfragtree << dendl;
        fg = newfg;
    }
    ...
    for (; !end && numfiles < max; end = (it == dir->end())) {
        ...
        CDentry::linkage_t * dnl =
            dnp ? dn->get_projected_linkage() : dn->get_linkage();
            ...
            if (in) {
              ...
              for (auto & p:*dir) {
                ...
                if (dnbl.length() > 0) {
                    ...
                    dout(10) <<
                        " open remote dentry after caps were issued, stopping at "
                ...
                mds->locker->drop_locks(mdr.get());
                mdr->drop_local_auth_pins();
                mdcache->open_remote_dentry(dn, dnp,
                                            new C_MDS_RetryRequest(mdcache,
                                                                   mdr));
                return;
            }
        ...
        if ((int)
            (dnbl.length() + dn->get_name().length() + sizeof(__u32) +
             sizeof(LeaseStat)) > bytes_left) {
            dout(10) << " ran out of room, stopping at " << dnbl.
                length() << " < " << bytes_left << dendl;
            break;
        }
        ...
        int r =
            in->encode_inodestat(dnbl, mdr->session, realm, snapid,
...
class C_MDS_inode_update_finish:public ServerLogContext {
  ...
  C_MDS_inode_update_finish(Server * s, MDRequestRef & r, CInode * i, bool sm = false, bool cr = false, bool ar = false):
    ServerLogContext(s, r), in(i),
        ...
        int snap_op =
            (in->snaprealm ? CEPH_SNAP_OP_UPDATE : CEPH_SNAP_OP_SPLIT);
    ...
    if (CEPH_LOCK_UNLOCK == set_lock.type) {
        ...
        if (mdr->more()->flock_was_waiting && !lock_state->is_waiting(set_lock)) {
            ...
            respond_to_request(mdr, -CEPHFS_EINTR);
                 ...
                 add_lock(set_lock, will_wait, mdr->more()->flock_was_waiting,
                          &deadlock)) {
            ...
            if (deadlock) {
                ...
                mdr->mark_nowarn();
                cur->add_waiter(CInode::WAIT_FLOCK,
                                new C_MDS_RetryRequest(mdcache, mdr));
            }
        }
        else
...
void Server::handle_client_setattr(MDRequestRef & mdr)
{
    ...
    if (is_full && req->head.args.setattr.size > old_size) {
        dout(20) << __func__ <<
            ": full, responding CEPHFS_ENOSPC to setattr with larger size" <<
            dendl;
        respond_to_request(mdr, -CEPHFS_ENOSPC);
        return;
    }
    ...
    if (mask & CEPH_SETATTR_SIZE) {
        if (req->get_data().length() >
            ...
            fscrypt_last_block_max_size) {
            dout(10) << __func__ << ": the last block size is too large" <<
                dendl;
            respond_to_request(mdr, -CEPHFS_EINVAL);
            return;
        }
        ...
        if (truncating_smaller && pip->is_truncating()) {
            dout(10) << " waiting for pending truncate from " << pip->
                truncate_from << " to " << pip->
                truncate_size << " to complete on " << *cur << dendl;
            mds->locker->drop_locks(mdr.get());
            mdr->drop_local_auth_pins();
            cur->add_waiter(CInode::WAIT_TRUNC,
        ...
        if (truncating_smaller && req->get_data().length()) {
            ...
            if (header.change_attr != pip->change_attr) {
                dout(5) << __func__ << ": header.change_attr:" << header.
                    change_attr << " != current change_attr:" << pip->
                    change_attr << ", let client retry it!" << dendl;
    ...
    if (mask & CEPH_SETATTR_MODE)
        pi.inode->mode =
            (pi.inode->mode & ~07777) | (req->head.args.setattr.mode & 07777);
    else if ((mask &
              (CEPH_SETATTR_UID | CEPH_SETATTR_GID | CEPH_SETATTR_KILL_SGUID |
    ...
    if (mask & CEPH_SETATTR_SIZE) {
        ...
        if (mds->locker->calc_new_client_ranges(cur, pi.inode->size)) {
            dout(10) << " client_ranges " << cur->
                get_previous_projected_inode()->client_ranges << " -> " << pi.
                inode->client_ranges << dendl;
            changed_ranges = true;
        }
    }
    ...
    journal_and_reply(mdr, cur, 0, le,
                      new C_MDS_inode_update_finish(this, mdr, cur,
                                                    truncating_smaller,
                                                    changed_ranges));
...
void Server::do_open_truncate(MDRequestRef & mdr, int cmode)
{
    ...
    uint64_t old_size =
        std::max < uint64_t > (pi.inode->size,
                               mdr->client_request->head.args.open.old_size);
    if (old_size > 0) {
    ...
    le->metablob.add_client_req(mdr->reqid,
                                mdr->client_request->get_oldest_client_tid());

    mdcache->predirty_journal_parents(mdr, &le->metablob, in, 0,
                                      PREDIRTY_PRIMARY);
    ...
    journal_and_reply(mdr, in, dn, le,
                      new C_MDS_inode_update_finish(this, mdr, in, old_size > 0,
                                                    changed_ranges));
...
bool Server::xlock_policylock(MDRequestRef & mdr, CInode * in, bool want_layout,
                              bool xlock_snaplock)
{
    ...
    if (CDentry * pdn = in->get_projected_parent_dn(); pdn) {
        if (!mds->locker->
            try_rdlock_snap_layout(pdn->get_dir()->get_inode(), mdr, 0,
                                   want_layout))
            return false;
    }
...
// XATTRS
int Server::parse_layout_vxattr_json(string name, string value,
                                     const OSDMap & osdmap,
                                     file_layout_t * layout)
{
    auto parse_pool =[&](std::string pool_name, int64_t pool_id)->int64_t {
        if (pool_name != "") {
                              ...
                              if (_pool_id < 0) {
                              dout(10) << __func__ << ": unknown pool name:" <<
                              pool_name << dendl; return -CEPHFS_EINVAL;}
                              return _pool_id;}
                              else if (pool_id >= 0) {
                              const auto pools = osdmap.get_pools();
                              if (pools.find(pool_id) == pools.end()) {
                              dout(10) << __func__ << ": unknown pool id:" <<
                              pool_id << dendl; return -CEPHFS_EINVAL;}
                              return pool_id;}
                              else {
                              ...
                              if (json_parser.
                                  parse(value.c_str(),
                                        value.length())and json_parser.
                                  is_object()) {
                              std::string field; try {
                              field = "object_size";
                              JSONDecoder::decode_json("object_size",
                                                       layout->object_size,
                                                       &json_parser, true);
                              field = "stripe_unit";
                              JSONDecoder::decode_json("stripe_unit",
                                                       layout->stripe_unit,
                                                       &json_parser, true);
                              field = "stripe_count";
                              JSONDecoder::decode_json("stripe_count",
                                                       layout->stripe_count,
                                                       &json_parser, true);
                              field = "pool_namespace";
                              JSONDecoder::decode_json("pool_namespace",
                                                       layout->pool_ns,
                                                       &json_parser, false);
                              field = "pool_id"; int64_t pool_id = 0;
                              JSONDecoder::decode_json("pool_id", pool_id,
                                                       &json_parser, false);
                              field = "pool_name"; std::string pool_name;
                              JSONDecoder::decode_json("pool_name", pool_name,
                                                       &json_parser, false);
                              pool_id = parse_pool(pool_name, pool_id);
                              if (pool_id < 0) {
                              return (int)pool_id;}
                              layout->pool_id =
                              pool_id;} catch(JSONDecoder::err &) {
                              dout(10) << __func__ <<
                              ": json is missing a mandatory field named " <<
                              field << dendl; return -CEPHFS_EINVAL;}
                              }
                              else {
                              ...
                              dout(10) << __func__ << ": unknown layout vxattr " << name << dendl; return -CEPHFS_ENODATA;  // no such attribute
                              }
                              }
                              catch(boost::bad_lexical_cast const &) {
                              dout(10) << __func__ << ": bad vxattr value:" <<
                              value << ", unable to parse for xattr:" << name <<
                              dendl; return -CEPHFS_EINVAL;}
                              return 0;}

// parse old style layout string
                              int Server::
                              parse_layout_vxattr_string(string name,
                                                         string value,
                                                         const OSDMap & osdmap,
                                                         file_layout_t *
                                                         layout) {
                              ...
                              if (name == "layout.object_size") {
                              layout->object_size =
                              boost::lexical_cast < unsigned >(value);}
                              else
                              if (name == "layout.stripe_unit") {
                              layout->stripe_unit =
                              boost::lexical_cast < unsigned >(value);}
                              else
                              if (name == "layout.stripe_count") {
                              layout->stripe_count =
                              boost::lexical_cast < unsigned >(value);}
                              else
                              if (name == "layout.pool") {
                              ...
                              layout->pool_id =
                              boost::lexical_cast <
                              unsigned >(value);} catch(boost::
                                                        bad_lexical_cast const
                                                        &) {
                              int64_t pool = osdmap.lookup_pg_pool_name(value);
                              if (pool < 0) {
                              dout(10) << __func__ << ": unknown pool " << value
                              << dendl; return -CEPHFS_ENOENT;}
                              layout->pool_id = pool;}
                              }
                              else
                              if (name == "layout.pool_id") {
                              layout->pool_id =
                              boost::lexical_cast < int64_t > (value);}
                              else
                              if (name == "layout.pool_name") {
                              layout->pool_id =
                              osdmap.lookup_pg_pool_name(value);
                              if (layout->pool_id < 0) {
                              dout(10) << __func__ << ": unknown pool " << value
                              << dendl; return -CEPHFS_EINVAL;}
                              }
                              else
                              if (name == "layout.pool_namespace") {
                              ...
                              name << dendl; return -CEPHFS_EINVAL;}
                              return 0;}

                              int Server::parse_layout_vxattr(string name,
                                                              string value,
                                                              const OSDMap &
                                                              osdmap,
                                                              file_layout_t *
                                                              layout,
                                                              bool validate) {
                              dout(20) << __func__ << ": name:" << name <<
                              " value:'" << value << "'" << dendl; int r;
                              if (name == "layout.json") {
                              r =
                              parse_layout_vxattr_json(name, value, osdmap,
                                                       layout);}
                              else {
                              r =
                              parse_layout_vxattr_string(name, value, osdmap,
                                                         layout);}
                              if (r < 0) {
                              return r;}
                              ...
                              return -CEPHFS_EINVAL;}
                              if (!mds->mdsmap->is_data_pool(layout->pool_id)) {
                              dout(10) << __func__ << ": invalid data pool " <<
                              layout->pool_id << dendl; return -CEPHFS_EINVAL;}
                              return 0;}

                              int Server::parse_quota_vxattr(string name,
                                                             string value,
                                                             quota_info_t *
                                                             quota) {
                              dout(20) << "parse_quota_vxattr name " << name <<
                              " value '" << value << "'" << dendl; try {
                              if (name == "quota") {
                              ...
                              return -CEPHFS_EINVAL;}
                              string left(begin, end);
                              dout(10) << " parsed " << m << " left '" << left
                              << "'" << dendl; if (begin != end)
                              return -CEPHFS_EINVAL;
                              for (map < string, string >::iterator q =
                                   m.begin(); q != m.end(); ++q) {
                              int r =
                              parse_quota_vxattr(string("quota.") + q->first,
                                                 q->second, quota); if (r < 0)
                              return r;}
                              }
                              else
                              if (name == "quota.max_bytes") {
                              int64_t q =
                              boost::lexical_cast < int64_t > (value);
                              if (q < 0)
                              return -CEPHFS_EINVAL; quota->max_bytes = q;}
                              else
                              if (name == "quota.max_files") {
                              int64_t q =
                              boost::lexical_cast < int64_t > (value);
                              if (q < 0)
                              return -CEPHFS_EINVAL; quota->max_files = q;}
                              else {
                              dout(10) << " unknown quota vxattr " << name <<
                              dendl; return -CEPHFS_EINVAL;}
                              }
                              catch(boost::bad_lexical_cast const &) {
                              dout(10) <<
 ...
 */
                              int Server::
                              check_layout_vxattr(MDRequestRef & mdr,
                                                  string name, string value,
                                                  file_layout_t * layout) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request; epoch_t epoch; int r;
                              mds->objecter->
                              with_osdmap([&](const OSDMap & osdmap) {
                                          r =
                                          parse_layout_vxattr(name, value,
                                                              osdmap, layout);
                              ...
                              new C_IO_Wrapper(mds,
                                               new C_MDS_RetryRequest(mdcache,
                                                                      mdr));
                              mds->objecter->wait_for_map(req_epoch,
                                                          lambdafy(fin));
                              return r;}
                              else
                              ...
                              if (r == -CEPHFS_ENOENT)
                              r = -CEPHFS_EINVAL;
                              respond_to_request(mdr, r); return r;}

                              // all is well
                              return 0;}

                              void Server::handle_set_vxattr(MDRequestRef & mdr,
                                                             CInode * cur) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request; MutationImpl::LockOpVec lov;
                              ...
                              if (!cur->is_dir()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              if (check_layout_vxattr(mdr, rest, value, &layout)
                                  < 0)
                              return; auto pi = cur->project_inode(mdr);
                              pi.inode->layout = layout;
                              mdr->no_early_reply = true; pip = pi.inode.get();}
                              else
                              if (name.compare(0, 16, "ceph.file.layout") == 0) {
                              if (!cur->is_file()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              if (cur->get_projected_inode()->size ||
                              ...
                              return;}
                              file_layout_t layout =
                              cur->get_projected_inode()->layout;
                              rest = name.substr(name.find("layout"));
                              if (check_layout_vxattr(mdr, rest, value, &layout)
                                  < 0)
                              return; lov.add_xlock(&cur->filelock);
                              if (!mds->locker->acquire_locks(mdr, lov))
                              return;
                              ...
                              pi.inode->layout = layout; pip = pi.inode.get();}
                              else
                              if (name.compare(0, 10, "ceph.quota") == 0) {
                              if (!cur->is_dir()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}

                              quota_info_t quota =
                              cur->get_projected_inode()->quota;
                              rest = name.substr(name.find("quota"));
                              int r = parse_quota_vxattr(rest, value, &quota);
                              if (r < 0) {
                              respond_to_request(mdr, r); return;}

                              if (quota.is_enabled()
                                  && !cur->get_projected_srnode())
                              adjust_realm = true;
                              if (!xlock_policylock
                                  (mdr, cur, false, adjust_realm))
                              return;
                              if (cur->get_projected_inode()->quota == quota) {
                              respond_to_request(mdr, 0); return;}

                              auto pi =
                              cur->project_inode(mdr, false, adjust_realm);
                              pi.inode->quota = quota; if (adjust_realm)
                              pi.snapnode->created = pi.snapnode->seq =
                              cur->find_snaprealm()->get_newest_seq();
                              mdr->no_early_reply = true; pip = pi.inode.get();
                              client_t exclude_ct = mdr->get_client();
                              mdcache->broadcast_quota_to_client(cur,
                                                                 exclude_ct,
                                                                 true);}
                              else
                              if (name == "ceph.dir.subvolume" sv) {
                              if (!cur->is_dir()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              dout(10) <<
                              "bad vxattr value, unable to parse bool for " <<
                              name << dendl;
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
     ...
     */
                              ...
                              if (!
                                  (mdr->
                                   locking_state & MutationImpl::ALL_LOCKED)) {
    ...
    /* repeat rdonly checks in case changed between rdlock -> xlock */
                              ...
                              if (!srnode)
                              pi.snapnode->created = pi.snapnode->seq =
                              realm->get_newest_seq(); if (val)
                              pi.snapnode->mark_subvolume();
                              else
                              pi.snapnode->clear_subvolume();
                              mdr->no_early_reply = true;
                              pip = pi.inode.get(); adjust_realm = true;}
                              else
                              ...
                              if (rank >= MAX_MDS) {
                              respond_to_request(mdr, -CEPHFS_EDOM); return;}
                              }
                              catch(boost::bad_lexical_cast const &) {
                              dout(10) <<
                              "bad vxattr value, unable to parse int for " <<
                              ...
                              return;
                              auto pi = cur->project_inode(mdr);
                              cur->set_export_pin(rank); pip = pi.inode.get();}
                              else
                              if (name == "ceph.dir.pin.random" sv) {
                              if (!cur->is_dir() || cur->is_root()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              val =
                              boost::lexical_cast <
                              double >(value);} catch(boost::
                                                      bad_lexical_cast const &)
                              {
                              dout(10) <<
                              "bad vxattr value, unable to parse float for " <<
                              name << dendl;
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              if (mdcache->export_ephemeral_random_max < val) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              if (name == "ceph.dir.pin.distributed" sv) {
                              if (!cur->is_dir() || cur->is_root()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              mdr->ls = mdlog->get_current_segment();
                              EUpdate * le =
                              new EUpdate(mdlog, "set vxattr layout");
                              mdlog->start_entry(le);
                              le->metablob.add_client_req(req->get_reqid(),
                                                          req->
                                                          get_oldest_client_tid
                                                          ());
                              mdcache->predirty_journal_parents(mdr,
                                                                &le->metablob,
                                                                cur, 0,
                                                                PREDIRTY_PRIMARY);
                              mdcache->journal_dirty_inode(mdr.get(),
                                                           &le->metablob, cur);
                              journal_and_reply(mdr, cur, 0, le,
                                                new
                                                C_MDS_inode_update_finish(this,
                                                                          mdr,
                                                                          cur,
                                                                          false,
                                                                          false,
                              ...
                              void Server::
                              handle_remove_vxattr(MDRequestRef & mdr,
                                                   CInode * cur) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              string name(req->get_path2());
                              dout(10) << __func__ << " " << name << " on " <<
                              *cur << dendl; if (name == "ceph.dir.layout") {
                              if (!cur->is_dir()) {
                              respond_to_request(mdr, -CEPHFS_ENODATA); return;}
                              if (cur->is_root()) {
                              dout(10) <<
                              "can't remove layout policy on the root directory"
                              << dendl; respond_to_request(mdr, -CEPHFS_EINVAL);
                              return;}
                              ...
                              mdr->ls = mdlog->get_current_segment();
                              EUpdate * le =
                              new EUpdate(mdlog, "remove dir layout vxattr");
                              mdlog->start_entry(le);
                              le->metablob.add_client_req(req->get_reqid(),
                                                          req->
                                                          get_oldest_client_tid
                              ...
                              if (name == "ceph.dir.layout.pool_namespace"
                                  || name ==
                              ...
                              handle_set_vxattr(mdr, cur); return;}

                              respond_to_request(mdr, -CEPHFS_ENODATA);}

                              const Server::XattrHandler Server::
                              xattr_handlers[] = {
                              {
  xattr_name: Server::DEFAULT_HANDLER, description: "default xattr handler", validate: &Server::default_xattr_validate, setxattr: &Server::default_setxattr_handler, removexattr:&Server::default_removexattr_handler,},
                              {
xattr_name: "ceph.mirror.info", description: "mirror info xattr handler", validate: &Server::mirror_info_xattr_validate, setxattr: &Server::mirror_info_setxattr_handler, removexattr:&Server::mirror_info_removexattr_handler},};
                              const Server::XattrHandler *
                              Server::get_xattr_or_default_handler(std::
                                                                   string_view
                                                                   xattr_name) {
  const XattrHandler * default_xattr_handler = nullptr; for (auto & handler:xattr_handlers)
                              {
                              ...
                              dout(20) << "handler=" << handler.
                              description << dendl; return &handler;}
                              }

                              ceph_assert(default_xattr_handler != nullptr);
                              dout(20) << "handler=" << default_xattr_handler->
                              description << dendl;
                              return default_xattr_handler;}

                              int Server::xattr_validate(CInode * cur,
                              ...
                              dendl; return -CEPHFS_EEXIST;}
                              }
                              if ((flags & CEPH_XATTR_REPLACE)
                                  && !(xattrs
                                       && xattrs->
                                       count(mempool::mds_co::
                                             string(xattr_name)))) {
                              dout(10) << "setxattr '" << xattr_name <<
                              "' XATTR_REPLACE and CEPHFS_ENODATA on " << *cur
                              << dendl; return -CEPHFS_ENODATA;}
                              ...
                              if (!xattrs
                                  ...
                                  count(mempool::mds_co::string(xattr_name)) ==
                                  0) {
                              dout(10) << "removexattr '" << xattr_name <<
                              "' and CEPHFS_ENODATA on " << *cur << dendl;
                              return -CEPHFS_ENODATA;}

                              return 0;}

                              derr << ": unhandled validation for: " <<
                              xattr_name << dendl; return -CEPHFS_EINVAL;}
                              ...
                              bufferptr b = buffer::create(len); if (len) {
                              xattr_value.begin().copy(len, b.c_str());}
                              auto em =
                              xattrs->emplace(std::piecewise_construct,
                                              std::forward_as_tuple(mempool::
                                                                    mds_co::
                                                                    string
                                                                    (xattr_name)),
                              ...
                              void Server::xattr_rm(InodeStoreBase::
                                                    xattr_map_ptr xattrs,
                                                    const std::
                                                    string & xattr_name) {
                              xattrs->
                              ...
                              int Server::default_xattr_validate(CInode * cur,
                                                                 const
                                                                 InodeStoreBase::
                                                                 xattr_map_const_ptr
                                                                 xattrs,
                                                                 XattrOp *
                                                                 xattr_op) {
                              return xattr_validate(cur, xattrs,
                                                    xattr_op->xattr_name,
                                                    xattr_op->op,
                                                    xattr_op->flags);}

                              void Server::default_setxattr_handler(CInode *
                                                                    cur,
                                                                    InodeStoreBase::
                                                                    xattr_map_ptr
                                                                    xattrs,
                                                                    ...
                                                                    XattrOp &
                                                                    xattr_op) {
                              xattr_set(xattrs, xattr_op.xattr_name,
                                        xattr_op.xattr_value);}

                              void Server::default_removexattr_handler(CInode *
                                                                       cur,
                                                                       InodeStoreBase::
                                                                       xattr_map_ptr
                                                                       xattrs,
...
// mirror info xattr handlers
                              const std::string Server::MirrorXattrInfo::
                              MIRROR_INFO_REGEX =
                              "^cluster_id=([a-f0-9]{8}-"
                              "[a-f0-9]{4}-[a-f0-9]{4}-"
                              "[a-f0-9]{4}-[a-f0-9]{12})" " fs_id=(\\d+)$";
                              const std::string Server::MirrorXattrInfo::
                              CLUSTER_ID = "ceph.mirror.info.cluster_id";
                              const std::string Server::MirrorXattrInfo::FS_ID =
                              "ceph.mirror.info.fs_id";
                              int Server::parse_mirror_info_xattr(const std::
                                                                  string & name,
                                                                  const std::
                                                                  string &
                                                                  value,
                                                                  ...
                                                                  string &
                                                                  fs_id) {
                              dout(20) << "parsing name=" << name << ", value="
                              << value << dendl;
                              static const std::regex regex(Server::
                                                            MirrorXattrInfo::
                                                            MIRROR_INFO_REGEX);
                              std::smatch match;
                              ...
                              cluster_id = match[1];
                              fs_id = match[2];
                              dout(20) << " parsed cluster_id=" << cluster_id <<
                              ", fs_id=" << fs_id << dendl; return 0;}

                              int Server::mirror_info_xattr_validate(CInode *
                                                                     cur,
                                                                     const
                                                                     InodeStoreBase::
                                                                     xattr_map_const_ptr
                              ...
                              int v1 =
                              xattr_validate(cur, xattrs,
                                             Server::MirrorXattrInfo::
                                             CLUSTER_ID, xattr_op->op,
                                             xattr_op->flags);
                              int v2 =
                              xattr_validate(cur, xattrs,
                                             Server::MirrorXattrInfo::FS_ID,
                                             xattr_op->op, xattr_op->flags);
                              if (v1 != v2) {
                              derr << "inconsistent mirror info state (" << v1
                              << "," << v2 << ")" << dendl;
                              return -CEPHFS_EINVAL;}
                              ...
                              std::string fs_id;
                              int r =
                              parse_mirror_info_xattr(xattr_op->xattr_name,
                                                      xattr_op->xattr_value.
                                                      to_str(), cluster_id,
                                                      fs_id); if (r < 0) {
                              return r;}
                              ...
                              xattr_op->xinfo =
                              std::make_unique < MirrorXattrInfo > (cluster_id,
                                                                    fs_id);
                              return 0;}

                              void Server::mirror_info_setxattr_handler(CInode *
                                                                        cur,
                                                                        InodeStoreBase::
                                                                        xattr_map_ptr
                                                                        xattrs,
                              ...
                              auto mirror_info =
                              dynamic_cast <
                              MirrorXattrInfo & >(*(xattr_op.xinfo));
                              bufferlist bl;
                              bl.append(mirror_info.cluster_id.c_str(),
                                        mirror_info.cluster_id.length());
                              xattr_set(xattrs,
                                        Server::MirrorXattrInfo::CLUSTER_ID,
                                        bl); bl.clear();
                              bl.append(mirror_info.fs_id.c_str(),
                                        mirror_info.fs_id.length());
                              xattr_set(xattrs, Server::MirrorXattrInfo::FS_ID,
                                        bl);}

                              void Server::
                              mirror_info_removexattr_handler(CInode * cur,
                                                              InodeStoreBase::
                                                              xattr_map_ptr
                                                              xattrs,
                                                              const XattrOp &
                              ...
                              void Server::
                              handle_client_setxattr(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              string name(req->get_path2());
                              ...
                              try_get_auth_inode(mdr,
                                                 req->get_filepath().get_ino());
                              if (!cur)
                              return; handle_set_vxattr(mdr, cur); return;}
                              ...
                              CInode * cur = rdlock_path_pin_ref(mdr, true);
                              if (!cur)
                              return; if (mdr->snapid != CEPH_NOSNAP) {
                              respond_to_request(mdr, -CEPHFS_EROFS); return;}

                              int flags = req->head.args.setxattr.flags;
                              MutationImpl::LockOpVec lov;
                              lov.add_xlock(&cur->xattrlock);
                              if (!mds->locker->acquire_locks(mdr, lov))
                              return; if (!check_access(mdr, cur, MAY_WRITE))
                              return;
                              ...
                              const auto & pxattrs =
                              cur->get_projected_xattrs(); if (pxattrs) {
                              // check xattrs kv pairs size
      size_t cur_xattrs_size = 0; for (const auto & p:*pxattrs)
                              {
                              if ((flags & CEPH_XATTR_REPLACE)
                                  && name.compare(p.first) == 0) {
                              continue;}
                              cur_xattrs_size +=
                              p.first.length() + p.second.length();}

                              if (((cur_xattrs_size + inc) >
                                   g_conf()->mds_max_xattr_pairs_size)) {
                              dout(10) <<
                              "xattr kv pairs size too big. cur_xattrs_size " <<
                              cur_xattrs_size << ", inc " << inc << dendl;
                              respond_to_request(mdr, -CEPHFS_ENOSPC); return;}
                              }

                              XattrOp xattr_op(CEPH_MDS_OP_SETXATTR, name,
                                               req->get_data(), flags);
                              int r =
                              std::invoke(handler->validate, this, cur, pxattrs,
                                          &xattr_op); if (r < 0) {
                              respond_to_request(mdr, r); return;}

                              dout(10) << "setxattr '" << name << "' len " <<
                              len << " on " << *cur << dendl;
                              // project update
                              ...
                              std::invoke(handler->removexattr, this, cur,
                                          pi.xattrs, xattr_op);}
                              else {
                              std::invoke(handler->setxattr, this, cur,
                                          pi.xattrs, xattr_op);}
                              ...
                              void Server::
                              handle_client_removexattr(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              ...
                              try_get_auth_inode(mdr,
                                                 req->get_filepath().get_ino());
                              if (!cur)
                              return; handle_remove_vxattr(mdr, cur); return;}
                              ...
                              Server::get_xattr_or_default_handler(name);
                              bufferlist bl;
                              XattrOp xattr_op(CEPH_MDS_OP_RMXATTR, name, bl,
                                               0);
                              const auto & pxattrs =
                              cur->get_projected_xattrs();
                              int r =
                              std::invoke(handler->validate, this, cur, pxattrs,
                                          &xattr_op); if (r < 0) {
                              respond_to_request(mdr, r); return;}

                              dout(10) << "removexattr '" << name << "' on " <<
                              *cur << dendl;
                              // project update
                              auto pi = cur->project_inode(mdr, true);
                              pi.inode->version = cur->pre_dirty();
                              ...
                              void Server::
                              handle_client_getvxattr(MDRequestRef & mdr) {
                              ...
                              CInode * cur =
                              rdlock_path_pin_ref(mdr, true, false); if (!cur) {
                              return;}
                              ...
                              if (is_ceph_file_vxattr(xattr_name)) {
                              if (cur->is_dir()) {
                              respond_to_request(mdr, -CEPHFS_ENODATA); return;}
                              }

                              CachedStackStringStream css;
                              int r = 0; ceph::bufferlist bl;
                              // handle these vxattrs
                              if ((xattr_name.substr(0, 15) ==
                                   "ceph.dir.layout" sv)
                                  || (xattr_name.substr(0, 16) ==
                                      "ceph.file.layout" sv)) {
      ...
      enum class InheritanceStatus:uint32_t {
                              ...
                              layout_xattr_info_t(const file_layout_t & l,
                                                  InheritanceStatus inh)
                              :layout(l), status(inh) {
                              }

                              static std::
                              string status_to_string(InheritanceStatus status)
                              {
                              switch (status) {
case InheritanceStatus::DEFAULT:
return "default" s; case InheritanceStatus::SET:
return "set" s; case InheritanceStatus::INHERITED:
return "inherited" s; default:
                              return "unknown" s;}
                              }
                              };
                              auto is_default_layout =
                              [&](const file_layout_t & layout)->bool {
                              return (layout == mdcache->default_file_layout);};
                              auto get_inherited_layout =
                              [&](CInode * cur)->layout_xattr_info_t {
                              auto orig_in = cur; while (cur) {
                              if (cur->get_projected_inode()->has_layout()) {
                              auto & curr_layout =
                              cur->get_projected_inode()->layout;
                              ...
                              return {
                              curr_layout,
                              layout_xattr_info_t::InheritanceStatus::
                              INHERITED};}
                              }
                              ...
                              cur =
                              cur->get_projected_parent_dir()->get_inode();}
                              mds->clog->
                              error() << "no layout found at root dir!";
                              ceph_abort
                              ("no layout found at root dir! something is really messed up with layouts!");};
                              ...
                              *css << "{\"stripe_unit\": " << lxi.layout.
                              stripe_unit << ", \"stripe_count\": " << lxi.
                              layout.
                              stripe_count << ", \"object_size\": " << lxi.
                              layout.object_size << ", \"pool_name\": ";
                              mds->objecter->
                              with_osdmap([lxi, &css] (const OSDMap & o) {
                                          *css << "\"";
                                          if (o.
                                              have_pg_pool(lxi.layout.
                                                           pool_id)) {
                                          *css << o.get_pool_name(lxi.layout.
                                                                  pool_id);}
                                          *css << "\"";}
                              );
                              *css << ", \"pool_id\": " << (uint64_t) lxi.
                              layout.pool_id;
                              *css << ", \"pool_namespace\": \"" << lxi.layout.
                              pool_ns << "\"";
                              *css << ", \"inheritance\": \"@" <<
                              layout_xattr_info_t::status_to_string(lxi.
                                                                    status) <<
                              "\"}";}
                              else
                              if ((xattr_name == "ceph.dir.layout.pool_name" sv)
                                  || (xattr_name ==
                              ...
                              if ((xattr_name == "ceph.dir.layout.pool_id" sv)
                                  || (xattr_name ==
      ...
      // fetch layout only for valid xattr_name
                              ...
                              *css << (uint64_t) lxi.layout.pool_id;}
                              ...
                              if (xattr_name.substr(0, 12) == "ceph.dir.pin" sv) {
                              if (xattr_name == "ceph.dir.pin" sv) {
                              *css << cur->get_projected_inode()->export_pin;}
                              else
                              if (xattr_name == "ceph.dir.pin.random" sv) {
                              *css << cur->get_projected_inode()->
                              export_ephemeral_random_pin;}
                              else
                              if (xattr_name == "ceph.dir.pin.distributed" sv) {
                              *css << cur->get_projected_inode()->
    ...
    // since we only handle ceph vxattrs here
                              ...
                              ENCODE_START(1, 1, bl);
                              encode(css->strv(), bl);
                              ENCODE_FINISH(bl); mdr->reply_extra_bl = bl;}
...
struct C_WaitUnlinkToFinish: public MDSContext {
  protected:
                              MDCache * mdcache;
                              CDentry * dn;
                              MDSContext * fin; MDSRank * get_mds()override {
                              ceph_assert(mdcache != NULL); return mdcache->mds;}
  ...
  C_WaitUnlinkToFinish(MDCache * m, CDentry * d, MDSContext * f):
                              ...
                              void finish(int r) override {
                              fin->complete(r); dn->put(CDentry::PIN_PURGING);}
                              }; bool Server::is_unlink_pending(CDentry * dn) {

                              CDentry::linkage_t * dnl =
                              dn->get_projected_linkage();
                              if (!dnl->is_null()
                                  && dn->state_test(CDentry::STATE_UNLINKING)) {
                              return true;}
                              return false;}

                              void Server::wait_for_pending_unlink(CDentry * dn,
                                                                   MDRequestRef
                                                                   & mdr) {
                              ...
                              dn->add_waiter(CDentry::WAIT_UNLINK_FINISH,
...
struct C_WaitReintegrateToFinish:public MDSContext {
  ...
  C_WaitReintegrateToFinish(MDCache * m, CDentry * d, MDSContext * f):
                              ...
                              void Server::
                              wait_for_pending_reintegrate(CDentry * dn,
                                                           MDRequestRef & mdr) {
                              dout(20) << __func__ << " dn " << *dn << dendl;
                              mds->locker->drop_locks(mdr.get());
                              auto fin = new C_MDS_RetryRequest(mdcache, mdr);
                              dn->get(CDentry::PIN_PURGING);
                              dn->add_waiter(CDentry::WAIT_REINTEGRATE_FINISH,
                                             new
                                             C_WaitReintegrateToFinish(mdcache,
                                                                       dn,
                                                                       fin));}
...
class C_MDS_mknod_finish:    public ServerLogContext {
  ...
  C_MDS_mknod_finish(Server * s, MDRequestRef & r, CDentry * d, CInode * ni):
                              ...
                              ceph_assert(dir);
                              dir->mark_dirty(mdr->ls); dir->mark_new(mdr->ls);}

                              mdr->apply();
                              MDRequestRef null_ref;
                              get_mds()->mdcache->send_dentry_link(dn,
                                                                   null_ref);
                              if (newi->is_file()) {
                              get_mds()->locker->share_inode_max_size(newi);}
                              else
                              ...
                              // hit pop
                              get_mds()->balancer->hit_inode(newi,
                                                             META_POP_IWR);
                              // reply
                              server->respond_to_request(mdr, 0);}
                              };
                              void Server::
                              handle_client_mknod(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              client_t client = mdr->get_client();
                              unsigned mode = req->head.args.mknod.mode;
                              if ((mode & S_IFMT) == 0)
                              mode |= S_IFREG; mdr->disable_lock_cache();
                              CDentry * dn =
                              rdlock_path_xlock_dentry(mdr, true, false, false,
                                                       S_ISREG(mode)); if (!dn)
                              return; if (is_unlink_pending(dn)) {
                              wait_for_pending_unlink(dn, mdr); return;}

                              CDir * dir = dn->get_dir();
                              CInode * diri = dir->get_inode();
                              if (!check_access(mdr, diri, MAY_WRITE))
                              return; if (!check_fragment_space(mdr, dir))
                              return; if (!check_dir_max_entries(mdr, dir))
                              return;
                              ceph_assert(dn->get_projected_linkage()->
                                          is_null());
                              if (req->get_alternate_name().size() >
                                  alternate_name_max) {
                              dout(10) << " alternate_name longer than " <<
                              alternate_name_max << dendl;
                              respond_to_request(mdr, -CEPHFS_ENAMETOOLONG);
                              return;}
                              dn->set_alternate_name(req->get_alternate_name());
                              // set layout
                              ...
                              layout = mdcache->default_file_layout;
                              CInode * newi =
                              prepare_new_inode(mdr, dn->get_dir(),
                                                inodeno_t(req->head.ino), mode,
                                                &layout); ceph_assert(newi);
                              dn->push_projected_linkage(newi);
                              auto _inode = newi->_get_inode();
                              _inode->version = dn->pre_dirty();
                              ...
                              _inode->accounted_rstat = _inode->rstat;
                              if (layout.pool_id !=
                                  mdcache->default_file_layout.pool_id)
                              _inode->add_old_pool(mdcache->default_file_layout.
                                                   pool_id);
                              _inode->update_backtrace();
                              snapid_t follows =
                              mdcache->get_global_snaprealm()->get_newest_seq();
                              SnapRealm * realm =
                              dn->get_dir()->inode->find_snaprealm();
                              ceph_assert(follows >= realm->get_newest_seq());
                              ...
                              Capability * cap =
                              mds->locker->issue_new_caps(newi, cmode, mdr,
                                                          realm); if (cap) {
                              cap->set_wanted(0);
                              ...
                              newi->xattrlock.set_state(LOCK_EXCL);
                              dout(15) <<
                              " setting a client_range too, since this is a regular file"
                              << dendl;
                              _inode->client_ranges[client].range.first = 0;
                              _inode->client_ranges[client].range.last =
                              _inode->layout.stripe_unit;
                              _inode->client_ranges[client].follows = follows;
                              newi->mark_clientwriteable();
                              cap->mark_clientwriteable();}
                              }
                              ...
                              ceph_assert(dn->first == follows + 1);
                              newi->first = dn->first;
                              dout(10) << "mknod mode " << _inode->
                              mode << " rdev " << _inode->rdev << dendl;
                              // prepare finisher
                              mdr->ls = mdlog->get_current_segment();
                              ...
                              le->metablob.add_primary_dentry(dn, newi, true,
                                                              true, true);
                              journal_and_reply(mdr, newi, dn, le,
                                                new C_MDS_mknod_finish(this,
                                                                       mdr, dn,
                                                                       newi));
                              mds->balancer->maybe_fragment(dn->get_dir(),
                                                            false);}

// MKDIR
/* This function takes responsibility for the passed mdr*/
                              void Server::
                              handle_client_mkdir(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request; mdr->disable_lock_cache();
                              CDentry * dn =
                              rdlock_path_xlock_dentry(mdr, true); if (!dn)
                              return; if (is_unlink_pending(dn)) {
                              wait_for_pending_unlink(dn, mdr); return;}
...
// SYMLINK
                              ...
                              void Server::
                              handle_client_symlink(MDRequestRef & mdr) {
                              const auto & req = mdr->client_request;
                              mdr->disable_lock_cache();
                              CDentry * dn =
                              rdlock_path_xlock_dentry(mdr, true); if (!dn)
...
// LINK
                              ...
                              void Server::
                              handle_client_link(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              dout(7) << "handle_client_link " << req->
                              get_filepath()
                              ...
                              targeti =
                              mdcache->get_inode(req->get_filepath2().
                                                 get_ino()); if (!targeti) {
                              dout(10) <<
                              "CEPHFS_ESTALE on path2, attempting recovery" <<
                              dendl;
                              inodeno_t ino = req->get_filepath2().get_ino();
                              mdcache->find_ino_peers(ino,
                                                      new
                                                      C_MDS_TryFindInode(this,
                                                                         mdr,
                                                                         mdcache,
                                                                         ino));
                              return;}
                              ...
                              if (!
                                  (mdr->
                                   locking_state & MutationImpl::
                                   SNAP2_LOCKED)) {
                              CDentry * pdn =
                              targeti->get_projected_parent_dn(); if (!pdn) {
                              dout(7) << "target has no parent dn, failing..."
                              << dendl; respond_to_request(mdr, -CEPHFS_EINVAL);
                              return;}
                              if (!mds->locker->
                                  try_rdlock_snap_layout(pdn->get_dir()->
                                                         get_inode(), mdr, 1))
                              return;
                              ...
                              auto ret =
                              rdlock_two_paths_xlock_destdn(mdr, false);
                              destdn = ret.first; if (!destdn)
                              return;
                              if (!destdn->get_projected_linkage()->is_null()) {
                              respond_to_request(mdr, -CEPHFS_EEXIST); return;}

                              targeti =
                              ret.second->get_projected_linkage()->get_inode();}

                              if (is_unlink_pending(destdn)) {
                              wait_for_pending_unlink(destdn, mdr); return;}

                              ceph_assert(destdn->get_projected_linkage()->
                                          is_null());
                              if (req->get_alternate_name().size() >
                                  alternate_name_max) {
                              dout(10) << " alternate_name longer than " <<
                              ...
                              dout(7) << "handle_client_link link " << destdn->
                              get_name() << " in " << *dir << dendl;
                              ...
                              dout(7) << "target has no link, failing..." <<
                              dendl; respond_to_request(mdr, -CEPHFS_ENOENT);
                              return;}
                              ...
                              return;
                              if (!check_access
                                  (mdr, dir->get_inode(), MAY_WRITE))
                              return; if (!check_fragment_space(mdr, dir))
                              return; if (!check_dir_max_entries(mdr, dir))
                              return;}

                              CInode * target_pin =
                              targeti->get_projected_parent_dir()->inode;
                              SnapRealm * target_realm =
                              target_pin->find_snaprealm();
                              if (target_pin != dir->inode
                                  && target_realm->get_subvolume_ino() !=
                                  dir->inode->find_snaprealm()->
                                  get_subvolume_ino()) {
                              if (target_pin->is_stray()) {
                              mds->locker->drop_locks(mdr.get());
                              targeti->add_waiter(CInode::WAIT_UNLINK,
                                                  new
                                                  C_MDS_RetryRequest(mdcache,
                                                                     mdr));
                              mdlog->flush(); return;}
                              dout(7) <<
                              "target is in different subvolume, failing..." <<
                              dendl; respond_to_request(mdr, -CEPHFS_EXDEV);
                              return;}
...
class C_MDS_link_local_finish:public ServerLogContext
                              {
  CDentry * dn; CInode * targeti; version_t dnpv; version_t tipv; bool adjust_realm; public:
  C_MDS_link_local_finish(Server * s, MDRequestRef & r, CDentry * d, CInode * ti, version_t dnpv_, version_t tipv_, bool ar):
                              ServerLogContext(s, r), dn(d), targeti(ti),
                              dnpv(dnpv_), tipv(tipv_), adjust_realm(ar) {
                              }
                              void finish(int r) override {
                              ceph_assert(r == 0);
                              server->_link_local_finish(mdr, dn, targeti, dnpv,
                                                         tipv, adjust_realm);}
                              };
                              void Server::_link_local(MDRequestRef & mdr,
                                                       CDentry * dn,
                                                       CInode * targeti,
                                                       SnapRealm *
                                                       target_realm) {
                              dout(10) << "_link_local " << *dn << " to " <<
                              *targeti << dendl;
                              mdr->ls = mdlog->get_current_segment();
                              // predirty NEW dentry
                              ...
                              targeti->record_snaprealm_parent_dentry(newsnap,
                                                                      target_realm,
                                                                      targeti->
                              ...
                              // log + wait
                              EUpdate * le = new EUpdate(mdlog, "link_local"); mdlog->start_entry(le); le->metablob.add_client_req(mdr->reqid, mdr->client_request->get_oldest_client_tid()); mdcache->predirty_journal_parents(mdr, &le->metablob, targeti, dn->get_dir(), PREDIRTY_DIR, 1);   // new dn
                              ...
                              dn->push_projected_linkage(targeti->ino(),
                                                         targeti->d_type());
                              journal_and_reply(mdr, targeti, dn, le,
                                                new
                                                C_MDS_link_local_finish(this,
                                                                        mdr, dn,
                                                                        targeti,
                                                                        dnpv,
                                                                        tipv,
                              ...
                              void Server::
                              _link_local_finish(MDRequestRef & mdr,
                                                 CDentry * dn, CInode * targeti,
                                                 version_t dnpv, version_t tipv,
                                                 bool adjust_realm) {
                              dout(10) << "_link_local_finish " << *dn << " to "
                              << *targeti << dendl;
                              ...
                              mds->balancer->hit_dir(dn->get_dir(),
                                                     META_POP_IWR);
                              // reply
                              respond_to_request(mdr, 0);}
...
class C_MDS_link_remote_finish:public ServerLogContext
                              {
  bool inc; CDentry * dn; CInode * targeti; version_t dpv; public:
  C_MDS_link_remote_finish(Server * s, MDRequestRef & r, bool i, CDentry * d, CInode * ti):
                              ServerLogContext(s, r), inc(i), dn(d),
                              targeti(ti), dpv(d->get_projected_version()) {
                              }
                              void finish(int r) override {
                              ceph_assert(r == 0);
                              server->_link_remote_finish(mdr, inc, dn, targeti,
                                                          dpv);}
                              };
                              void Server::_link_remote(MDRequestRef & mdr,
                                                        bool inc, CDentry * dn,
                                                        CInode * targeti) {
                              dout(10) << "_link_remote " << (inc ? "link " :
                                                              "unlink ")
                              << *dn << " to " << *targeti << dendl;
                              ...
                              if (mds->is_cluster_degraded() &&
                                  ...
                                  (linkauth)) {
                              dout(10) << " targeti auth mds." << linkauth <<
                              " is not active" << dendl;
                              if (mdr->more()->waiting_on_peer.empty())
                              mds->wait_for_active_peer(linkauth,
                                                        new
                                                        C_MDS_RetryRequest
                                                        (mdcache, mdr));
                              return;}
                              ...
                              dout(20) << " noting uncommitted_peers " << mdr->
                              more()->witnessed << dendl;
                              le->reqid = mdr->reqid; le->had_peers = true;
                              mdcache->add_uncommitted_leader(mdr->reqid,
                                                              mdr->ls,
                                                              mdr->more()->
                                                              witnessed);}

                              if (inc) {
                              dn->pre_dirty(); mdcache->predirty_journal_parents(mdr, &le->metablob, targeti, dn->get_dir(), PREDIRTY_DIR, 1); le->metablob.add_remote_dentry(dn, true, targeti->ino(), targeti->d_type()); // new remote
                              dn->push_projected_linkage(targeti->ino(),
                                                         targeti->d_type());}
                              else {
                              dn->pre_dirty();
                              mdcache->predirty_journal_parents(mdr,
                                                                ...
                                                                PREDIRTY_DIR,
                                                                -1);
                              mdcache->journal_cow_dentry(mdr.get(),
                                                          &le->metablob, dn);
                              le->metablob.add_null_dentry(dn, true);
                              dn->push_projected_linkage();}

                              journal_and_reply(mdr, (inc ? targeti : nullptr),
                                                dn, le,
                                                new
                                                C_MDS_link_remote_finish(this,
                                                                         mdr,
                                                                         inc,
                                                                         dn,
                                                                         targeti));}
                              ...
                              void Server::
                              _link_remote_finish(MDRequestRef & mdr, bool inc,
                                                  CDentry * dn,
                                                  CInode * targeti,
                                                  version_t dpv) {
                              dout(10) << "_link_remote_finish " << (inc ?
                                                                     "link " :
                                                                     "unlink ")
                              << *dn << " to " << *targeti << dendl;
                              ...
                              // unlink main dentry
                              dn->get_dir()->unlink_inode(dn); dn->pop_projected_linkage(); dn->mark_dirty(dn->get_projected_version(), mdr->ls);   // dirty old dentry
                              }

                              mdr->apply(); MDRequestRef null_ref; if (inc) {
                              mdcache->send_dentry_link(dn, null_ref);}
                              else {
                              dn->state_clear(CDentry::STATE_UNLINKING);
                              mdcache->send_dentry_unlink(dn, NULL, null_ref);
                              MDSContext::vec finished;
                              dn->take_waiting(CDentry::WAIT_UNLINK_FINISH,
                                               finished);
                              ...
                              mds->balancer->hit_dir(dn->get_dir(),
                                                     META_POP_IWR);
                              // reply
                              respond_to_request(mdr, 0); if (!inc)
                              // removing a new dn?
...
class C_MDS_PeerLinkPrep:    public ServerLogContext {
  CInode * targeti; bool adjust_realm; public:
  C_MDS_PeerLinkPrep(Server * s, MDRequestRef & r, CInode * t, bool ar):
                              ...
                              void finish(int r) override {
                              ceph_assert(r == 0);
                              server->_logged_peer_link(mdr, targeti,
                                                        adjust_realm);}
}; class C_MDS_PeerLinkCommit:public ServerContext {

  MDRequestRef mdr; CInode * targeti; public:
  C_MDS_PeerLinkCommit(Server * s, MDRequestRef & r, CInode * t):
                              ...
                              mdr->ls = mdlog->get_current_segment();
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_link_prep",
                                              mdr->reqid, mdr->peer_to_mds,
                                              EPeerUpdate::OP_PREPARE,
                                              EPeerUpdate::LINK);
                              mdlog->start_entry(le);
                              ...
                              if (mdr->peer_request->get_op() ==
                                  MMDSPeerRequest::OP_LINKPREP) {
                              inc = true; pi.inode->nlink++;
                              CDentry * target_pdn =
                              targeti->get_projected_parent_dn();
                              SnapRealm * target_realm =
                              target_pdn->get_dir()->inode->find_snaprealm();
                              if (!target_realm->get_subvolume_ino()
                                  && !targeti->
                                  is_projected_snaprealm_global()) {
                              sr_t * newsnap = targeti->project_snaprealm();
                              ...
                              targeti->record_snaprealm_parent_dentry(newsnap,
                                                                      ...
                                                                      target_pdn,
                                                                      true);
                              adjust_realm = true; realm_projected = true;}
                              }
                              else {
                              inc = false;
                              pi.inode->nlink--;
                              if (targeti->is_projected_snaprealm_global()) {
                              ceph_assert(mdr->peer_request->desti_snapbl.
                                          length());
                              auto p = mdr->peer_request->desti_snapbl.cbegin();
                              sr_t * newsnap = targeti->project_snaprealm();
                              decode(*newsnap, p); if (pi.inode->nlink == 0)
                              ...
                              link_rollback rollback; rollback.reqid = mdr->reqid; rollback.ino = targeti->ino(); rollback.old_ctime = targeti->get_inode()->ctime; // we hold versionlock xlock; no concorrent projections
                              const auto & pf =
                              targeti->get_parent_dn()->get_dir()->
                              get_projected_fnode();
                              rollback.old_dir_mtime = pf->fragstat.mtime;
                              rollback.old_dir_rctime = pf->rstat.rctime;
                              rollback.was_inc = inc; if (realm_projected) {
                              ...
                              dout(10) << " projected inode " << pi.inode->
                              ino << " v " << pi.inode->version << dendl;
                              // commit case
                              mdcache->predirty_journal_parents(mdr,
                                                                &le->commit,
                                                                dnl->
                              ...
                              mdcache->journal_dirty_inode(mdr.get(),
                                                           &le->commit,
                                                           targeti);
                              mdcache->add_uncommitted_peer(mdr->reqid, mdr->ls,
                                                            mdr->peer_to_mds);
                              // set up commit waiter
                              mdr->more()->peer_commit =
                              new C_MDS_PeerLinkCommit(this, mdr, targeti);
                              mdr->more()->peer_update_journaled = true;
                              submit_mdlog_entry(le,
                                                 new C_MDS_PeerLinkPrep(this,
                                                                        mdr,
                                                                        targeti,
                                                                        adjust_realm),
                                                 mdr, __func__);
                              ...
                              void Server::_logged_peer_link(MDRequestRef & mdr,
                                                             CInode * targeti,
                                                             bool adjust_realm)
                              {
                              ...
                              *targeti << dendl;
                              ...
                              mds->balancer->hit_inode(targeti, META_POP_IWR);
                              // done.
                              mdr->reset_peer_request(); if (adjust_realm) {
                              int op = CEPH_SNAP_OP_SPLIT;
                              mds->mdcache->send_snap_update(targeti, 0, op);
                              mds->mdcache->
                              do_realm_invalidate_and_update_notify(targeti,
...
struct C_MDS_CommittedPeer:  public ServerLogContext {
  C_MDS_CommittedPeer(Server * s, MDRequestRef & m):ServerLogContext(s,
                                               m)
                              {
                              }
                              void finish(int r) override {
                              ...
                              void Server::_commit_peer_link(MDRequestRef & mdr,
                                                             int r,
                                                             CInode * targeti) {
                              dout(10) << "_commit_peer_link " << *mdr << " r="
                              << r << " " << *targeti << dendl;
                              ...
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_link_commit",
                                              mdr->reqid, mdr->peer_to_mds,
                                              EPeerUpdate::OP_COMMIT,
                                              EPeerUpdate::LINK);
                              mdlog->start_entry(le);
                              submit_mdlog_entry(le,
                                                 new C_MDS_CommittedPeer(this,
                                                                         mdr),
                                                 mdr, __func__);
                              mdlog->flush();}
                              else {
                              do_link_rollback(mdr->more()->rollback_bl,
                                               mdr->peer_to_mds, mdr);}
                              }
                              ...
                              bool assert_exist =
                              mdr->more()->peer_update_journaled;
                              mdcache->finish_uncommitted_peer(mdr->reqid,
                                                               assert_exist);
                              auto req =
                              make_message < MMDSPeerRequest > (mdr->reqid,
                                                                mdr->attempt,
...
struct C_MDS_LoggedLinkRollback:public ServerLogContext
                              {
  MutationRef mut; map < client_t, ref_t < MClientSnap >> splits; C_MDS_LoggedLinkRollback(Server * s, MutationRef & m, MDRequestRef & r, map < client_t, ref_t < MClientSnap >> &&_splits):
                              ...
                              void Server::do_link_rollback(bufferlist & rbl,
                                                            mds_rank_t leader,
                                                            MDRequestRef & mdr)
                              {
                              link_rollback rollback; auto p = rbl.cbegin();
                              decode(rollback, p);
                              dout(10) << "do_link_rollback on " << rollback.
                              reqid << (rollback.was_inc ? " inc" : " dec")
                              << " ino " << rollback.ino << dendl; ceph_assert(g_conf()->mds_kill_link_at != 9); mdcache->add_rollback(rollback.reqid, leader); // need to finish this update before resolve finishes
                              ceph_assert(mdr || mds->is_resolve()); MutationRef mut(new MutationImpl(nullptr, utime_t(), rollback.reqid)); mut->ls = mds->mdlog->get_current_segment(); CInode * in = mdcache->get_inode(rollback.ino); ceph_assert(in); dout(10) << " target is " << *in << dendl; ceph_assert(!in->is_projected());  // live peer request hold versionlock xlock.
                              auto pi = in->project_inode(mut);
                              pi.inode->version = in->pre_dirty();
                              // parent dir rctime
                              CDir * parent =
                              in->get_projected_parent_dn()->get_dir();
                              ...
                              mut->add_updated_lock(&parent->get_inode()->
                                                    filelock);
                              mut->add_updated_lock(&parent->get_inode()->
                                                    nestlock);}
                              ...
                              SnapRealm * realm =
                              parent->get_inode()->find_snaprealm();
                              if (!mds->is_resolve())
                              mdcache->prepare_realm_merge(in->snaprealm, realm,
                                                           splits);
                              ...
                              // journal it
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_link_rollback",
                                              rollback.reqid, leader,
                                              EPeerUpdate::OP_ROLLBACK,
                                              EPeerUpdate::LINK);
                              mdlog->start_entry(le);
                              le->commit.add_dir_context(parent);
                              le->commit.add_dir(parent, true);
                              le->commit.add_primary_dentry(in->
                                                            get_projected_parent_dn
                                                            (), 0, true);
                              submit_mdlog_entry(le,
                                                 new
                                                 C_MDS_LoggedLinkRollback(this,
                                                                          mut,
                                                                          mdr,
                                                                          std::
                                                                          move
                              ...
                              void Server::
                              _link_rollback_finish(MutationRef & mut,
                                                    MDRequestRef & mdr,
                                                    map < client_t,
                                                    ref_t <
                                                    MClientSnap >> &splits) {
                              ...
                              mut->apply(); if (!mds->is_resolve())
                              mdcache->send_snaps(splits); if (mdr)
                              mdcache->request_finish(mdr);
                              mdcache->finish_rollback(mut->reqid, mdr);
                              mut->cleanup();}

                              void Server::
                              handle_peer_link_prep_ack(MDRequestRef & mdr,
                                                        const cref_t <
                                                        MMDSPeerRequest > &m) {
                              dout(10) << "handle_peer_link_prep_ack " << *mdr
                              << " " << *m << dendl;
                              mds_rank_t from =
                              mds_rank_t(m->get_source().num());
                              ceph_assert(g_conf()->mds_kill_link_at != 11);
                              // note peer
                              mdr->more()->peers.insert(from);
                              // witnessed!
                              ceph_assert(mdr->more()->witnessed.count(from) ==
                                          0);
                              mdr->more()->witnessed.insert(from);
                              ceph_assert(!m->is_not_journaled());
                              mdr->more()->has_journaled_peers = true;
                              // remove from waiting list
                              ceph_assert(mdr->more()->waiting_on_peer.count(from)); mdr->more()->waiting_on_peer.erase(from); ceph_assert(mdr->more()->waiting_on_peer.empty()); dispatch_client_request(mdr); // go again!
                              }

// UNLINK

                              void Server::
                              handle_client_unlink(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              client_t client = mdr->get_client();
                              // rmdir or unlink?
                              ...
                              mdr->disable_lock_cache();
                              CDentry * dn =
                              rdlock_path_xlock_dentry(mdr, false, true);
                              if (!dn)
                              return; if (is_reintegrate_pending(dn)) {
                              wait_for_pending_reintegrate(dn, mdr); return;}
                              ...
                              mdcache->send_dentry_unlink(dn, nullptr, mdr,
                                                          true);
                              ...
                              CDentry::linkage_t * dnl =
                              dn->get_linkage(client, mdr);
                              ceph_assert(!dnl->is_null());
                              CInode * in = dnl->get_inode(); if (rmdir) {
                              dout(7) << "handle_client_rmdir on " << *dn <<
                              dendl;}
                              else {
                              dout(7) << "handle_client_unlink on " << *dn <<
                              dendl;}
                              ...
                              dn->state_clear(CDentry::STATE_UNLINKING);
                              respond_to_request(mdr, -CEPHFS_EISDIR); return;}
                              }
                              else {
                              if (rmdir) {
                              // unlink
                              dout(7) << "handle_client_rmdir on non-dir " <<
                              *in << ", returning error" << dendl;
                              dn->state_clear(CDentry::STATE_UNLINKING);
                              respond_to_request(mdr, -CEPHFS_ENOTDIR); return;}
                              }
                              ...
                              if ((!mdr->has_more()
                                   || mdr->more()->witnessed.empty())) {
                              if (!check_access(mdr, diri, MAY_WRITE)) {
                              dn->state_clear(CDentry::STATE_UNLINKING); return;}
                              }

                              // -- create stray dentry? --
                              CDentry * straydn = NULL; if (dnl->is_primary()) {
                              straydn =
                              prepare_stray_dentry(mdr, dnl->get_inode());
                              if (!straydn)
                              return;
                              dout(10) << " straydn is " << *straydn << dendl;}
                              else
                              if (mdr->straydn) {
                              mdr->unpin(mdr->straydn); mdr->straydn = NULL;}
  ...
  // lock
                              ...
                              lov.add_xlock(&in->snaplock); if (in->is_dir())
                              lov.add_rdlock(&in->filelock);    // to verify it's empty
                              if (straydn) {
                              lov.add_wrlock(&straydn->get_dir()->inode->
                                             filelock);
                              lov.add_wrlock(&straydn->get_dir()->inode->
                                             nestlock);
                              ...
                              if (nlink == 1 ||
                                  (nlink == 2 && !dnl->is_primary() &&
                                   !in->get_projected_parent_dir()->inode->
                                   is_stray()))
                              ...
                              if (in->snaprealm
                                  || follows + 1 > in->get_oldest_snap()) {
                              sr_t * new_srnode =
                              in->prepare_new_srnode(follows);
                              in->record_snaprealm_past_parent(new_srnode,
                                                               straydn->
                                                               get_dir()->
                                                               inode->
                                                               find_snaprealm
  ...
  // yay!
                              ...
                              dout(10) << " witnesses " << witnesses <<
                              ", have " << mdr->more()->witnessed << dendl;
                              for (set < mds_rank_t >::iterator p =
                                   witnesses.begin(); p != witnesses.end();
                                   ++p) {
                              if (mdr->more()->witnessed.count(*p)) {
                              dout(10) << " already witnessed by mds." << *p <<
                              dendl;}
                              else
                              if (mdr->more()->waiting_on_peer.count(*p)) {
                              dout(10) << " already waiting on witness mds." <<
                              *p << dendl;}
                              else {
                              if (!_rmdir_prepare_witness
                                  (mdr, *p, mdr->dn[0], straydn))
                              return;}
                              }
                              if (!mdr->more()->waiting_on_peer.empty())
                              ...
                              if (!rmdir && dnl->is_primary()
                                  && mdr->dn[0].size() == 1)
                              mds->locker->create_lock_cache(mdr, diri);
                              // ok!
                              if (dnl->is_remote()
                                  && !dnl->get_inode()->is_auth())
...
class C_MDS_unlink_local_finish:public ServerLogContext
                              {
                              CDentry * dn; CDentry * straydn; version_t dnpv;  // deleted dentry
  public:
  C_MDS_unlink_local_finish(Server * s, MDRequestRef & r, CDentry * d, CDentry * sd):
                              ...
                              void finish(int r) override {
                              ceph_assert(r == 0);
                              server->_unlink_local_finish(mdr, dn, straydn,
                                                           dnpv);}
                              };
                              void Server::_unlink_local(MDRequestRef & mdr,
                                                         CDentry * dn,
                                                         CDentry * straydn) {
                              ...
                              // the unlinked dentry
                              dn->pre_dirty(); auto pi = in->project_inode(mdr); {
                              std::string t;
                              ...
                              pi.inode->rstat.rctime = mdr->get_op_stamp();
                              pi.inode->change_attr++;
                              pi.inode->nlink--; if (pi.inode->nlink == 0)
                              ...
                              le->metablob.add_null_dentry(dn, true);
                              if (in->is_dir()) {
                              dout(10) << " noting renamed (unlinked) dir ino "
                              << in->ino() << " in metablob" << dendl;
                              le->metablob.renamed_dirino = in->ino();}

                              dn->push_projected_linkage(); if (straydn) {
                              ...
                              journal_and_reply(mdr, 0, dn, le,
                                                new
                                                C_MDS_unlink_local_finish(this,
                                                                          mdr,
                                                                          dn,
                                                                          straydn));}

                              void Server::
                              _unlink_local_finish(MDRequestRef & mdr,
                                                   CDentry * dn,
                                                   CDentry * straydn,
                                                   version_t dnpv) {
                              dout(10) << "_unlink_local_finish " << *dn <<
                              dendl; if (!mdr->more()->witnessed.empty())
                              mdcache->logged_leader_update(mdr->reqid);
                              CInode * strayin = NULL; bool hadrealm = false;
                              if (straydn) {
                              ...
                              // update subtree map?
                              if (strayin->is_dir())
                              mdcache->adjust_subtree_after_rename(strayin,
                                                                   dn->
                                                                   get_dir(),
                                                                   true);
                              ...
                              bool Server::
                              _rmdir_prepare_witness(MDRequestRef & mdr,
                                                     mds_rank_t who,
                              ...
                              dout(10) << "_rmdir_prepare_witness mds." << who
                              << " is not active" << dendl;
                              if (mdr->more()->waiting_on_peer.empty())
                              mds->wait_for_active_peer(who,
                                                        new
                                                        C_MDS_RetryRequest
                                                        (mdcache, mdr));
                              return false;}

  dout(10) << "_rmdir_prepare_witness mds." << who << dendl; auto req = make_message < MMDSPeerRequest > (mdr->reqid, mdr->attempt, MMDSPeerRequest::OP_RMDIRPREP); req->srcdnpath = filepath(trace.front()->get_dir()->ino()); for (auto dn:trace)
                              req->srcdnpath.push_dentry(dn->get_name());
                              mdcache->encode_replica_stray(straydn, who,
                                                            req->straybl);
                              if (mdr->more()->desti_srnode)
                              encode(*mdr->more()->desti_srnode,
                                     req->desti_snapbl);
                              req->op_stamp = mdr->get_op_stamp();
...
struct C_MDS_PeerRmdirPrep:  public ServerLogContext {
                              CDentry * dn, *straydn;
                              C_MDS_PeerRmdirPrep(Server * s, MDRequestRef & r,
                                                  CDentry * d, CDentry * st)
  :                          ServerLogContext(s, r), dn(d), straydn(st) {
                              ...
                              {
                              MDRequestRef mdr; CDentry * straydn;
                              C_MDS_PeerRmdirCommit(Server * s,
                                                    MDRequestRef & r,
                                                    CDentry * sd)
  :                          ServerContext(s), mdr(r), straydn(sd) {
                              ...
                              void Server::
                              handle_peer_rmdir_prep(MDRequestRef & mdr) {
                              dout(10) << "handle_peer_rmdir_prep " << *mdr <<
                              " " << mdr->peer_request->
                              srcdnpath << " to " << mdr->peer_request->
                              destdnpath << dendl; vector < CDentry * >trace;
                              filepath srcpath(mdr->peer_request->srcdnpath);
                              dout(10) << " src " << srcpath << dendl;
                              CInode * in;
                              CF_MDS_RetryRequestFactory cf(mdcache, mdr,
                                                            false);
                              int r =
                              mdcache->path_traverse(mdr, cf, srcpath,
                                                     MDS_TRAVERSE_DISCOVER |
                                                     MDS_TRAVERSE_PATH_LOCKED,
                                                     &trace, &in);
                              if (r > 0) return; if (r == -CEPHFS_ESTALE) {
                              mdcache->find_ino_peers(srcpath.get_ino(),
                                                      new
                                                      C_MDS_RetryRequest
                                                      (mdcache, mdr),
                                                      mdr->peer_to_mds, true);
                              return;}
                              ceph_assert(r == 0); CDentry * dn = trace.back();
                              dout(10) << " dn " << *dn << dendl; mdr->pin(dn);
                              ...
                              dout(20) << " rollback is " << mdr->more()->
                              rollback_bl.length() << " bytes" << dendl;
                              // set up commit waiter
                              mdr->more()->peer_commit =
                              new C_MDS_PeerRmdirCommit(this, mdr, straydn);
                              straydn->push_projected_linkage(in);
                              dn->push_projected_linkage();
                              ceph_assert(straydn->first >= in->first);
                              in->first = straydn->first;
                              if (!in->
                                  has_subtree_root_dirfrag(mds->get_nodeid())) {
                              dout(10) << " no auth subtree in " << *in <<
                              ", skipping journal" << dendl;
                              _logged_peer_rmdir(mdr, dn, straydn); return;}

                              mdr->ls = mdlog->get_current_segment();
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_rmdir", mdr->reqid,
                                              mdr->peer_to_mds,
                                              EPeerUpdate::OP_PREPARE,
                                              EPeerUpdate::RMDIR);
                              mdlog->start_entry(le);
                              ...
                              void Server::
                              _logged_peer_rmdir(MDRequestRef & mdr,
                                                 CDentry * dn,
                                                 CDentry * straydn) {
                              dout(10) << "_logged_peer_rmdir " << *mdr <<
                              " on " << *dn << dendl;
                              CInode * in = dn->get_linkage()->get_inode();
                              bool new_realm;
                              if (mdr->peer_request->desti_snapbl.length()) {
                              new_realm = !in->snaprealm;
                              in->decode_snap_blob(mdr->peer_request->
                                                   desti_snapbl);
                              ...
                              mdcache->adjust_subtree_after_rename(in,
                                                                   ...
                                                                   get_dir(),
                                                                   mdr->more()->
                                                                   peer_update_journaled);
                              if (new_realm)
                              mdcache->do_realm_invalidate_and_update_notify(in,
                                                                             CEPH_SNAP_OP_SPLIT,
                                                                             false);
                              // done.
                              mdr->reset_peer_request();
                              mdr->straydn = 0; if (!mdr->aborted) {
                              auto reply =
                              make_message < MMDSPeerRequest > (mdr->reqid,
                                                                mdr->attempt,
                                                                MMDSPeerRequest::
                              ...
                              void Server::
                              handle_peer_rmdir_prep_ack(MDRequestRef & mdr,
                                                         const cref_t <
                                                         MMDSPeerRequest >
                                                         &ack) {
                              ...
                              dout(10) << "still waiting on peers " << mdr->
                              more()->waiting_on_peer << dendl;}

                              void Server::
                              _commit_peer_rmdir(MDRequestRef & mdr, int r,
                                                 CDentry * straydn) {
                              dout(10) << "_commit_peer_rmdir " << *mdr << " r="
                              << r << dendl; if (r == 0) {
                              if (mdr->more()->peer_update_journaled) {
                              CInode * strayin =
                              straydn->get_projected_linkage()->get_inode();
                              ...
                              // write a commit to the journal
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_rmdir_commit",
                                              mdr->reqid, mdr->peer_to_mds,
                                              EPeerUpdate::OP_COMMIT,
                                              EPeerUpdate::RMDIR);
                              mdlog->start_entry(le);
                              ...
                              // abort
                              do_rmdir_rollback(mdr->more()->rollback_bl,
                                                mdr->peer_to_mds, mdr);}
                              }

struct C_MDS_LoggedRmdirRollback:public ServerLogContext
                              {
                              metareqid_t reqid; CDentry * dn;
                              CDentry * straydn;
                              C_MDS_LoggedRmdirRollback(Server * s,
                                                        MDRequestRef & m,
                                                        metareqid_t mr,
                                                        CDentry * d,
                                                        CDentry * st)
  :                          ServerLogContext(s, m), reqid(mr), dn(d),
                              straydn(st) {
                              }
                              void finish(int r) override {
                              ...
                              void Server::do_rmdir_rollback(bufferlist & rbl,
                                                             mds_rank_t leader,
                                                             MDRequestRef & mdr)
                              {
                              ...
                              rmdir_rollback rollback; auto p = rbl.cbegin(); decode(rollback, p); dout(10) << "do_rmdir_rollback on " << rollback.reqid << dendl; mdcache->add_rollback(rollback.reqid, leader);   // need to finish this update before resolve finishes
                              ceph_assert(mdr || mds->is_resolve());
                              CDir * dir =
                              mdcache->get_dirfrag(rollback.src_dir); if (!dir)
                              dir =
                              mdcache->get_dirfrag(rollback.src_dir.ino,
                                                   rollback.src_dname);
                              ceph_assert(dir);
                              ...
                              dout(10) << " dn " << *dn << dendl;
                              CDir * straydir =
                              mdcache->get_dirfrag(rollback.dest_dir);
                              ceph_assert(straydir);
                              CDentry * straydn =
                              straydir->lookup(rollback.dest_dname);
                              ceph_assert(straydn);
                              dout(10) << " straydn " << *straydn << dendl;
                              ...
                              if (rollback.snapbl.length() && in->snaprealm) {
                              bool hadrealm; auto p = rollback.snapbl.cbegin();
                              decode(hadrealm, p); if (hadrealm) {
                              decode(in->snaprealm->srnode, p);}
                              else {
                              in->snaprealm->merge_to(dir->get_inode()->
                                                      find_snaprealm());}
                              }
                              ...
                              ceph_assert(!in->
                                          has_subtree_root_dirfrag(mds->
                                                                   get_nodeid
                                                                   ()));
                              _rmdir_rollback_finish(mdr, rollback.reqid, dn,
                                                     straydn); return;}

                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_rmdir_rollback",
                                              rollback.reqid, leader,
                                              EPeerUpdate::OP_ROLLBACK,
                                              EPeerUpdate::RMDIR);
                              mdlog->start_entry(le);
                              ...
                              << in->ino() << " in metablob" << dendl;
                              le->commit.renamed_dirino = in->ino();
                              mdcache->project_subtree_rename(in,
                                                              straydn->
                                                              get_dir(),
                                                              dn->get_dir());
                              submit_mdlog_entry(le,
                                                 new
                                                 C_MDS_LoggedRmdirRollback(this,
                                                                           mdr,
                                                                           rollback.
                                                                           reqid,
                                                                           dn,
                              ...
                              void Server::
                              _rmdir_rollback_finish(MDRequestRef & mdr,
                                                     metareqid_t reqid,
                                                     CDentry * dn,
                                                     CDentry * straydn) {
                              dout(10) << "_rmdir_rollback_finish " << reqid <<
                              dendl; straydn->get_dir()->unlink_inode(straydn);
                              dn->pop_projected_linkage();
                              straydn->pop_projected_linkage();
                              CInode * in = dn->get_linkage()->get_inode();
                              mdcache->adjust_subtree_after_rename(in,
                              ...
                              if (mds->is_resolve()) {
                              CDir * root =
                              mdcache->get_subtree_root(straydn->get_dir());
 ...
 */
                              bool Server::
                              _dir_is_nonempty_unlocked(MDRequestRef & mdr,
                                                        CInode * in) {
                              dout(10) << "dir_is_nonempty_unlocked " << *in <<
                              dendl; ceph_assert(in->is_auth());
                              if (in->filelock.is_cached())
                              return false; // there can be pending async create/unlink. don't know.
                              if (in->snaprealm
                                  && in->snaprealm->srnode.snaps.size())
                              return true;  // in a snapshot!
  auto && ls = in->get_dirfrags(); for (const auto & dir:ls)
                              {
                              // is the frag obviously non-empty?
                              if (dir->is_auth()) {
                              if (dir->get_projected_fnode()->fragstat.size()) {
                              dout(10) <<
                              "dir_is_nonempty_unlocked dirstat has " << dir->
                              get_projected_fnode()->fragstat.
                              size() << " items " << *dir << dendl;
                              return true;}
                              }
                              }
                              ...
                              bool Server::_dir_is_nonempty(MDRequestRef & mdr,
                                                            CInode * in) {
  dout(10) << "dir_is_nonempty " << *in << dendl; ceph_assert(in->is_auth()); ceph_assert(in->filelock.can_read(mdr->get_client())); frag_info_t dirstat; version_t dirstat_version = in->get_projected_inode()->dirstat.version; auto && ls = in->get_dirfrags(); for (const auto & dir:ls)
                              {
                              const auto & pf = dir->get_projected_fnode();
                              if (pf->fragstat.size()) {
                              dout(10) << "dir_is_nonempty dirstat has " << pf->
                              fragstat.size() << " items " << *dir << dendl;
                              return true;}

                              if (pf->accounted_fragstat.version ==
                                  dirstat_version)
...
class C_MDS_rename_finish:   public ServerLogContext {
  CDentry * srcdn; CDentry * destdn; CDentry * straydn; public:
 ...
 */
                              void Server::
                              handle_client_rename(MDRequestRef & mdr) {
                              const auto & req = mdr->client_request;
                              dout(7) << "handle_client_rename " << *req <<
                              dendl; filepath destpath = req->get_filepath();
                              filepath srcpath = req->get_filepath2();
                              if (srcpath.is_last_dot_or_dotdot()
                                  || destpath.is_last_dot_or_dotdot()) {
                              respond_to_request(mdr, -CEPHFS_EBUSY); return;}
                              ...
                              auto[destdn, srcdn] =
                              rdlock_two_paths_xlock_destdn(mdr, true);
                              if (!destdn)
                              return; if (is_unlink_pending(destdn)) {
                              wait_for_pending_unlink(destdn, mdr); return;}

                              if (is_unlink_pending(srcdn)) {
                              wait_for_pending_unlink(srcdn, mdr); return;}

                              dout(10) << " destdn " << *destdn << dendl;
                              CDir * destdir = destdn->get_dir();
                              ceph_assert(destdir->is_auth());
                              CDentry::linkage_t * destdnl =
                              destdn->get_projected_linkage();
                              dout(10) << " srcdn " << *srcdn << dendl;
                              CDir * srcdir = srcdn->get_dir();
                              CDentry::linkage_t * srcdnl =
                              srcdn->get_projected_linkage();
                              ...
                              // e.g. mv /usr /usr/foo
                              if (srci->is_dir()
                                  && srci->is_projected_ancestor_of(destdir->
                                                                    get_inode
                                                                    ())) {
                              dout(7) <<
                              "cannot rename item to be a child of itself" <<
                              dendl; respond_to_request(mdr, -CEPHFS_EINVAL);
                              return;}
                              ...
                              return;}
                              if (destdn->get_alternate_name() !=
                                  req->get_alternate_name()) {
                              ...
                              if (destpath.get_ino() != srcpath.get_ino() && !(req->get_source().is_mds() && MDS_INO_IS_STRAY(srcpath.get_ino()))) {    // <-- mds 'rename' out of stray dir is ok!
                              CInode * srcbase =
                              srctrace[0]->get_dir()->get_inode();
                              CInode * destbase =
                              desttrace[0]->get_dir()->get_inode();
                              ...
                              dout(10) << "rename prepending srctrace with " <<
                              *pdn << dendl;
                              ...
                              dout(10) << "rename prepending desttrace with " <<
                              *pdn << dendl;
                              destbase = pdn->get_dir()->get_inode();}
                              dout(10) <<
                              "rename src and dest traces now share common ancestor "
                              << *destbase << dendl;}

                              bool linkmerge =
                              srcdnl->get_inode() == destdnl->get_inode();
                              if (linkmerge)
                              ...
                              straydn =
                              prepare_stray_dentry(mdr, destdnl->get_inode());
                              if (!straydn)
                              return;
                              dout(10) << " straydn is " << *straydn << dendl;}
                              else
  ...
  // -- locks --
                              ...
                              lov.add_xlock(&srci->snaplock); if (oldin) {
                              // xlock oldin (for nlink--)
                              lov.add_xlock(&oldin->linklock);
                              lov.add_xlock(&oldin->snaplock);
                              if (oldin->is_dir()) {
                              ceph_assert(srci->is_dir()); lov.add_rdlock(&oldin->filelock);    // to verify it's empty
                              // adjust locking order?
                              int cmp = mdr->compare_paths();
                              if (cmp < 0
                                  || (cmp == 0 && oldin->ino() < srci->ino()))
                              ...
                              CInode * auth_pin_freeze = !srcdn->is_auth()
                              && srcdnl->is_primary()? srci : nullptr;
                              if (!mds->locker->
                                  acquire_locks(mdr, lov, auth_pin_freeze))
                              return;
                              mdr->locking_state |= MutationImpl::ALL_LOCKED;}
                              ...
                              if ((!mdr->has_more()
                                   || mdr->more()->witnessed.empty())) {
                              if (!check_access
                                  (mdr, srcdir->get_inode(), MAY_WRITE))
                              return;
                              if (!check_access
                                  (mdr, destdn->get_dir()->get_inode(),
                                   MAY_WRITE))
                              return;
                              if (!linkmerge
                              ...
                              if (!linkmerge
                                  && !check_dir_max_entries(mdr,
                                                            destdn->get_dir()))
                              return; if (!check_access(mdr, srci, MAY_WRITE))
                              return;}
   ...
   */
                              ...
                              SnapRealm * src_realm = nullptr; if (!linkmerge) {
                              ...
                              if (src_realm != dest_realm &&
                                  src_realm->get_subvolume_ino() !=
                                  dest_realm->get_subvolume_ino()) {
                              respond_to_request(mdr, -CEPHFS_EXDEV); return;}
                              }
                              ...
                              if (srcdnl->is_primary() &&
                                  !srcdn->is_auth() && srci->is_dir()) {
  dout(10) << "srci is remote dir, setting stickydirs and opening all frags" << dendl; mdr->set_stickydirs(srci); frag_vec_t leaves; srci->dirfragtree.get_leaves(leaves); for (const auto & leaf:leaves)
                              {
                              CDir * dir = srci->get_dirfrag(leaf); if (!dir) {
                              dout(10) << " opening " << leaf << " under " <<
                              *srci << dendl;
                              mdcache->open_remote_dirfrag(srci, leaf,
                                                           new
                                                           C_MDS_RetryRequest
                                                           (mdcache, mdr));
                              return;}
  ...
  // -- prepare snaprealm ---
                              ...
                              srci->record_snaprealm_parent_dentry(new_srnode,
                                                                   nullptr,
                                                                   destdn,
                                                                   false);
                              ...
                              oldin->record_snaprealm_parent_dentry(new_srnode,
                                                                    dest_realm,
                                                                    destdn,
                                                                    destdnl->
                                                                    is_primary
                              ...
                              if (nlink == 1 ||
                                  (nlink == 2 && !destdnl->is_primary() &&
                                   !oldin->get_projected_parent_dir()->inode->
                                   is_stray()))
                              ...
                              if (oldin->snaprealm
                                  || follows + 1 > oldin->get_oldest_snap()) {
                              sr_t * new_srnode =
                              oldin->prepare_new_srnode(follows);
                              oldin->record_snaprealm_past_parent(new_srnode,
                                                                  straydn->
                                                                  get_dir()->
                                                                  inode->
                                                                  find_snaprealm
                              ...
                              if (src_realm != dest_realm &&
                                  (srci->snaprealm
                                   || follows + 1 > srci->get_oldest_snap())) {
                              sr_t * new_srnode =
                              srci->prepare_new_srnode(follows);
                              srci->record_snaprealm_past_parent(new_srnode,
                                                                 dest_realm);
                              mdr->more()->srci_srnode = new_srnode;}
                              }
                              }
  ...
  bool need_flush = false; for (auto & dn:srctrace)
                              {
                              if (dn->is_projected()) {
                              ...
                              CInode * diri = dn->get_dir()->get_inode();
                              dn =
                              diri->get_projected_parent_dn();} while (dn);}
                              if (need_flush) {
                              mdlog->
                              wait_for_safe(new
                                            MDSInternalContextWrapper(mds,
                              ...
                              dout(10) << " preparing ambiguous auth for srci"
                              << dendl;
                              ceph_assert(mdr->more()->
                                          is_remote_frozen_authpin);
                              ceph_assert(mdr->more()->rename_inode == srci);
                              _rename_prepare_witness(mdr, last, witnesses,
                                                      srctrace, desttrace,
                                                      straydn); return;}
                              }
                              ...
                              *p << dendl;}
                              ...
                              if (!_rename_prepare_witness
                                  (mdr, *p, witnesses, srctrace, desttrace,
                                   straydn))
                              return;}
                              }
                              if (!mdr->more()->waiting_on_peer.empty())
                              return;   // we're waiting for a witness.
                              if (last != MDS_RANK_NONE
                                  && mdr->more()->witnessed.count(last) == 0) {
                              dout(10) << " preparing last witness (srcdn auth)"
                              << dendl;
                              ceph_assert(mdr->more()->waiting_on_peer.
                                          count(last) == 0);
                              _rename_prepare_witness(mdr, last, witnesses,
                              ...
                              _rename_prepare(mdr, &le->metablob,
                                              &le->client_map, srcdn, destdn,
                                              req->get_alternate_name(),
                                              straydn);
                              ...
                              new C_MDS_rename_finish(this, mdr, srcdn, destdn,
                                                      straydn);
                              ...
                              mdlog->flush();}
                              mds->balancer->maybe_fragment(destdn->get_dir(),
                                                            false);}

                              void Server::_rename_finish(MDRequestRef & mdr,
                                                          CDentry * srcdn,
                                                          CDentry * destdn,
                                                          CDentry * straydn) {
                              ...
                              // bump popularity
                              mds->balancer->hit_dir(srcdn->get_dir(),
                                                     META_POP_IWR);
                              ...
                              // reply
                              respond_to_request(mdr, 0); if (need_eval)
...
// helpers
                              ...
                              bool Server::
                              _rename_prepare_witness(MDRequestRef & mdr,
                                                      mds_rank_t who,
                                                      set < mds_rank_t >
                                                      &witnesse,
                                                      vector <
                              ...
                              if (mds->is_cluster_degraded()
                                  && !mds->mdsmap->
                                  is_clientreplay_or_active_or_stopping(who)) {
                              dout(10) << "_rename_prepare_witness mds." << who
                              << " is not active" << dendl;
                              if (mdr->more()->waiting_on_peer.empty())
                              mds->wait_for_active_peer(who,
                                                        new
          ...
          dout(10) << "_rename_prepare_witness mds." << who << dendl; auto req = make_message < MMDSPeerRequest > (mdr->reqid, mdr->attempt, MMDSPeerRequest::OP_RENAMEPREP); req->srcdnpath = filepath(srctrace.front()->get_dir()->ino()); for (auto dn:srctrace)
          req->srcdnpath.push_dentry(dn->get_name()); req->destdnpath = filepath(dsttrace.front()->get_dir()->ino()); for (auto dn:dsttrace)
                              ...
                              version_t Server::
                              _rename_prepare_import(MDRequestRef & mdr,
                                                     CDentry * srcdn,
                                                     bufferlist *
                                                     client_map_bl) {
                              ...
                              map < client_t,
                              client_metadata_t > client_metadata_map;
                              ...
                              encode(client_map, *client_map_bl,
                              ...
                              srcdnl->get_inode()->mark_clean(); return oldpv;}

                              bool Server::_need_force_journal(CInode * diri,
                                                               bool empty) {
                              auto && dirs = diri->get_dirfrags();
                              bool force_journal = false; if (empty) {
              for (const auto & dir:dirs) {
                              if (dir->is_subtree_root()
                                  && dir->get_dir_auth().first ==
                                  mds->get_nodeid()) {
                              dout(10) << " frag " << dir->
                              get_frag() <<
                              " is auth subtree dirfrag, will force journal" <<
                              dendl; force_journal = true; break;}
                              else
                              dout(20) << " frag " << dir->
                              get_frag() << " is not auth subtree dirfrag" <<
                              dendl;}
                              }
                              else {
                              // see if any children of our frags are auth subtrees.
              std::vector < CDir * >subtrees; mdcache->get_subtrees(subtrees); dout(10) << " subtrees " << subtrees << " frags " << dirs << dendl; for (const auto & dir:dirs)
                              {
                  for (const auto & subtree:subtrees)
                              {
                              if (dir->contains(subtree)) {
                              if (subtree->get_dir_auth().first ==
                                  mds->get_nodeid()) {
                              dout(10) << " frag " << dir->
                              get_frag() <<
                              " contains (maybe) auth subtree, will force journal "
                              << *subtree << dendl; force_journal = true;
                              break;}
                              else
                              dout(20) << " frag " << dir->
                              get_frag() << " contains but isn't auth for " <<
                              *subtree << dendl;}
                              else
                              dout(20) << " frag " << dir->
                              ...
                              void Server::_rename_prepare(MDRequestRef & mdr,
                                                           ...
                                                           alternate_name,
                                                           CDentry * straydn) {
                              dout(10) << "_rename_prepare " << *mdr << " " <<
                              *srcdn << " " << *destdn << dendl; if (straydn)
                              dout(10) << " straydn " << *straydn << dendl;
                              CDentry::linkage_t * srcdnl =
                              srcdn->get_projected_linkage();
                              CDentry::linkage_t * destdnl =
                              ...
                              bool linkmerge = (srci == oldin); if (linkmerge)
                              ceph_assert(srcdnl->is_primary()
                                          && destdnl->is_remote());
                              ...
                              dout(10) <<
                              " we are exporting srci, will force journal destdn"
                              << dendl; force_journal_dest = true;}
                              else
                              force_journal_dest =
                              _need_force_journal(srci, false);}

                              bool force_journal_stray = false;
                              if (oldin && oldin->is_dir() && straydn
                                  && !straydn->is_auth())
                              force_journal_stray =
                              _need_force_journal(oldin, true); if (linkmerge)
                              dout(10) <<
                              " merging remote and primary links to the same inode"
                              << dendl; if (silent)
                              dout(10) <<
                              " reintegrating stray; will avoid changing nlink or dir mtime"
                              << dendl; if (force_journal_dest)
                              dout(10) <<
                              " forcing journal destdn because we (will) have auth subtrees nested beneath it"
                              << dendl; if (force_journal_stray)
                              dout(10) <<
                              " forcing journal straydn because we (will) have auth subtrees nested beneath it"
                              << dendl;
                              if (srci->is_dir()
                                  && (destdn->is_auth()
                                      || force_journal_dest)) {
                              dout(10) << " noting renamed dir ino " << srci->
                              ino() << " in metablob" << dendl;
                              metablob->renamed_dirino = srci->ino();}
                              else
                              if (oldin && oldin->is_dir()
                                  && force_journal_stray) {
                              dout(10) << " noting rename target dir " <<
                              oldin->ino() << " in metablob" << dendl;
                              ...
                              pi.inode->version =
                              straydn->pre_dirty(pi.inode->version);
                              pi.inode->update_backtrace();
                              tpi = pi.inode.get();}
                              ...
                              destdn->push_projected_linkage(srcdnl->
                                                             get_remote_ino(),
                                                             srcdnl->
                                                             get_remote_d_type
                                                             ());
                              ...
                              dout(10) << " will merge remote onto primary link"
                              << dendl; if (destdn->is_auth()) {
                              auto pi = oldin->project_inode(mdr);
                              ...
                              if (destdn->is_auth()) {
                              version_t oldpv; if (srcdn->is_auth())
                              oldpv = srci->get_projected_version();
                              else {
                              oldpv =
                              _rename_prepare_import(mdr, srcdn, client_map_bl);
                              // note which dirfrags have child subtrees in the journal
                              // event, so that we can open those (as bounds) during replay.
                              if (srci->is_dir()) {
                      auto && ls = srci->get_dirfrags(); for (const auto & dir:ls)
                              {
                              if (!dir->is_auth())
                              metablob->renamed_dir_frags.push_back(dir->
                                                                    get_frag
                                                                    ());}
                              ...
                              // src
                              if (srcdn->is_auth())
                              mdr->more()->pvmap[srcdn] = srcdn->pre_dirty(); srcdn->push_projected_linkage();  // push null linkage
                              if (!silent) {
                              if (spi) {
                              spi->ctime = mdr->get_op_stamp();
                              if (mdr->get_op_stamp() > spi->rstat.rctime)
                              spi->rstat.rctime = mdr->get_op_stamp();
                              spi->change_attr++; if (linkmerge)
                              spi->nlink--;}
                              if (tpi) {
                              ...
                              if (destdnl->is_primary()) {
                              ceph_assert(straydn); if (straydn->is_auth()) {
                              metablob->add_dir_context(straydn->get_dir());
                              metablob->add_dir(straydn->get_dir(), true);}
                              }

                              if (!linkmerge && destdnl->is_remote()
                                  && oldin->is_auth()) {
                              CDir * oldin_dir =
                              oldin->get_projected_parent_dir();
                              if (oldin_dir != srcdn->get_dir()
                                  && oldin_dir != destdn->get_dir())
                              mdcache->predirty_journal_parents(mdr, metablob,
                                                                oldin,
                                                                oldin_dir,
                                                                PREDIRTY_PRIMARY);}
                              ...
                              if (srcdnl->is_remote() && srci->is_auth()) {
                              CDir * srci_dir =
                              srci->get_projected_parent_dir();
                              if (srci_dir != srcdn->get_dir()
                                  && srci_dir != destdn->get_dir())
                              mdcache->predirty_journal_parents(mdr, metablob,
                                                                srci, srci_dir,
                                                                PREDIRTY_PRIMARY);}
                              ...
                              if (!linkmerge) {
                              if (destdnl->is_primary()) {
                              ceph_assert(straydn); if (destdn->is_auth()) {
                              ...
                              dout(10) << " forced journaling straydn " <<
                              *straydn << dendl;
                              metablob->add_dir_context(straydn->get_dir());
                              metablob->add_primary_dentry(straydn, oldin,
                                                           true);}
                              }
                              else
                              if (destdnl->is_remote()) {
                              if (oldin->is_auth()) {
                              sr_t * new_srnode = NULL; if (mdr->peer_request) {
                              ...
                              if (auto & desti_srnode =
                                  mdr->more()->desti_srnode) {
                              new_srnode = desti_srnode; desti_srnode = NULL;}
                              if (new_srnode) {
                              ...
                              // auth for targeti
                              CDentry * oldin_pdn =
                              oldin->get_projected_parent_dn();
                              mdcache->journal_cow_dentry(mdr.get(), metablob,
                                                          oldin_pdn);
                              metablob->add_primary_dentry(oldin_pdn, oldin,
                                                           true);}
                              }
                              }
                              ...
                              destdn->first =
                              mdcache->get_global_snaprealm()->
                              get_newest_seq() + 1; if (destdn->is_auth())
                              metablob->add_remote_dentry(destdn, true, srcdnl->get_remote_ino(), srcdnl->get_remote_d_type()); if (srci->is_auth()) {  // it's remote
                              if (mdr->peer_request) {
                              ...
                              CDentry * srci_pdn =
                              srci->get_projected_parent_dn();
                              mdcache->journal_cow_dentry(mdr.get(), metablob,
                                                          srci_pdn);
                              metablob->add_primary_dentry(srci_pdn, srci,
                                                           true);}
                              }
                              else
                              if (srcdnl->is_primary()) {
                              ...
                              dout(10) << " forced journaling destdn " <<
                              *destdn << dendl;
          ...
          auto && ls = srci->get_dirfrags(); for (const auto & dir:ls)
                              ...
                              // src
                              if (srcdn->is_auth()) {
                              dout(10) << " journaling srcdn " << *srcdn <<
                              dendl;
                              mdcache->journal_cow_dentry(mdr.get(), metablob,
                                                          srcdn, CEPH_NOSNAP, 0,
                                                          srcdnl);
                              ...
                              dout(10) << " NOT journaling srcdn " << *srcdn <<
                              dendl;
                              ...
                              if (oldin && oldin->is_dir()) {
                              ceph_assert(straydn);
                              mdcache->project_subtree_rename(oldin,
                                                              destdn->get_dir(),
                                                              straydn->
                                                              get_dir());}
                              if (srci->is_dir())
                              mdcache->project_subtree_rename(srci,
                                                              srcdn->get_dir(),
                                                              destdn->
                                                              get_dir());}

                              void Server::_rename_apply(MDRequestRef & mdr,
                                                         CDentry * srcdn,
                                                         CDentry * destdn,
                                                         CDentry * straydn) {
                              dout(10) << "_rename_apply " << *mdr << " " <<
                              *srcdn << " " << *destdn << dendl;
                              dout(10) << " pvs " << mdr->more()->
                              pvmap << dendl;
                              CDentry::linkage_t * srcdnl =
                              srcdn->get_linkage();
                              CDentry::linkage_t * destdnl =
                              ...
                              if (mdr->peer_request->desti_snapbl.length()) {
                              new_oldin_snaprealm = !oldin->snaprealm;
                              oldin->decode_snap_blob(mdr->peer_request->
                                                      desti_snapbl);
                              ...
                              // nlink-- targeti
                              if (destdn->is_auth())
                              oldin->pop_and_dirty_projected_inode(mdr->ls, mdr); mdcache->touch_dentry_bottom(straydn);    // drop dn as quickly as possible.
                              }
                              else
                              if (destdnl->is_remote()) {
                              destdn->get_dir()->unlink_inode(destdn, false);
                              ...
                              if (destdn->is_auth())
                              destdn->mark_dirty(mdr->more()->pvmap[destdn],
                                                 mdr->ls);
                              // in
                              if (in->is_auth()) {
                              ...
                              if (auto & srci_srnode = mdr->more()->srci_srnode) {
                              delete srci_srnode; srci_srnode = NULL;}
                              }
                              else {
                              dout(10) << "merging remote onto primary link" <<
                              dendl;
                              oldin->pop_and_dirty_projected_inode(mdr->ls,
                                                                   mdr);}
                              }
                              ...
                              if (linkmerge) {
                              dout(10) << "merging primary onto remote link" <<
                              dendl;
                              destdn->get_dir()->unlink_inode(destdn, false);}
                              destdnl = destdn->pop_projected_linkage();
                              if (mdr->is_peer()
                              ...
                              ceph_assert(mdr->more()->inode_import.length() >
                                          0);
                              map < client_t,
                              Capability::Import > imported_caps;
                              // finish cap imports
                              finish_force_open_sessions(mdr->more()->
                                                         imported_session_map);
                              if (mdr->more()->cap_imports.
                                  count(destdnl->get_inode())) {
                              mdcache->migrator->
                              finish_import_inode_caps(destdnl->get_inode(),
                                                       mdr->more()->
                                                       srcdn_auth_mds, true,
                                                       mdr->more()->
                                                       imported_session_map,
                              ...
                              for (auto i =
                                   mdr->locks.lower_bound(&destdnl->
                                                          get_inode()->
                                                          versionlock);
                                   i != mdr->locks.end(); ++i) {
                              SimpleLock * lock = i->lock;
                              if (lock->get_parent() != destdnl->get_inode())
                              break; if (i->is_xlock() && !lock->is_locallock())
                              ...
                              // src
                              if (srcdn->is_auth())
                              srcdn->mark_dirty(mdr->more()->pvmap[srcdn],
                                                mdr->ls);
                              ...
                              mdcache->adjust_subtree_after_rename(oldin,
                                                                   destdn->
                                                                   get_dir(),
                                                                   true);
...
// PEER
      ...
      class C_MDS_PeerRenamePrep:public ServerLogContext
                              {
          CDentry * srcdn, *destdn, *straydn; public:
          C_MDS_PeerRenamePrep(Server * s, MDRequestRef & m, CDentry * sr, CDentry * de, CDentry * st):
                              ServerLogContext(s, m), srcdn(sr), destdn(de),
                              straydn(st) {
                              }
                              void finish(int r) override {
                              server->_logged_peer_rename(mdr, srcdn, destdn,
                                                          straydn);}
      }; class C_MDS_PeerRenameCommit:public ServerContext
                              {
          MDRequestRef mdr; CDentry * srcdn, *destdn, *straydn; public:
          C_MDS_PeerRenameCommit(Server * s, MDRequestRef & m, CDentry * sr, CDentry * de, CDentry * st):
                              ServerContext(s), mdr(m), srcdn(sr), destdn(de),
                              straydn(st) {
                              }
                              void finish(int r) override {
                              server->_commit_peer_rename(mdr, r, srcdn, destdn,
                                                          straydn);}
      }; class C_MDS_PeerRenameSessionsFlushed:public ServerContext
                              {
          MDRequestRef mdr; public:
          C_MDS_PeerRenameSessionsFlushed(Server * s, MDRequestRef & r):
                              ...
                              void Server::
                              handle_peer_rename_prep(MDRequestRef & mdr) {
                              ...
                              dout(10) <<
                              " peer request interrupted, sending noop reply" <<
                              dendl;
                              auto reply =
                              make_message < MMDSPeerRequest > (mdr->reqid,
                                                                mdr->attempt,
                              ...
                              CF_MDS_RetryRequestFactory cf(mdcache, mdr,
                                                            false);
                              int r =
                              mdcache->path_traverse(mdr, cf, destpath,
                                                     MDS_TRAVERSE_DISCOVER |
                                                     MDS_TRAVERSE_PATH_LOCKED |
                                                     MDS_TRAVERSE_WANT_DENTRY,
                                                     &trace); if (r > 0) return;
                              if (r == -CEPHFS_ESTALE) {
                              mdcache->find_ino_peers(destpath.get_ino(),
                                                      new
                                                      C_MDS_RetryRequest
                                                      (mdcache, mdr),
                                                      mdr->peer_to_mds, true);
                              ...
                              r = mdcache->path_traverse(mdr, cf, srcpath,
                                                         ...
                                                         MDS_TRAVERSE_PATH_LOCKED,
                                                         &trace, &srci);
                              if (r > 0) return; ceph_assert(r == 0);
                              CDentry * srcdn = trace.back();
                              CDentry::linkage_t * srcdnl =
                              srcdn->get_projected_linkage();
                              dout(10) << " srcdn " << *srcdn << dendl;
                              mdr->pin(srcdn); mdr->pin(srci);
                              // stray?
                              bool linkmerge =
                              srcdnl->get_inode() == destdnl->get_inode();
                              if (linkmerge)
                              ...
                              mdr->more()->peer_commit =
                              new C_MDS_PeerRenameCommit(this, mdr, srcdn,
                                                         destdn, straydn);
                              // am i srcdn auth?
                              if (srcdn->is_auth()) {
                              ...
                              mdr->unfreeze_auth_pin(); if (!frozen_inode) {
                              srcdnl->get_inode()->
                              ...
                              dout(10) <<
                              " set srci ambiguous auth; providing srcdn replica list"
                              << dendl; reply_witness = true;}

                              // make sure bystanders have received all lock related messages
                              for (set < mds_rank_t >::iterator p =
                                   srcdnrep.begin(); p != srcdnrep.end(); ++p) {
                              ...
                              mdcache->migrator->get_export_client_set(srcdnl->
                                                                       get_inode
                                                                       (),
                                                                       export_client_set);
                              MDSGatherBuilder gather(g_ceph_context);
                              ...
                              if (gather.has_subs()) {
                              mdr->more()->waiting_on_peer.
                              insert(MDS_RANK_NONE);
                              gather.
                              set_finisher(new
                                           C_MDS_PeerRenameSessionsFlushed(this,
                                                                           mdr));
                              gather.activate();}
                              }
                              ...
                              for (set < mds_rank_t >::iterator p =
                                   srcdnrep.begin(); p != srcdnrep.end(); ++p) {
                              if (*p == mdr->peer_to_mds
                                  || mdr->peer_request->witnesses.
                                  count(*p)) continue;
                              dout(10) <<
                              " witness list insufficient; providing srcdn replica list"
                              << dendl; reply_witness = true; break;}
                              ...
                              mdr->reset_peer_request(); return;}
                              dout(10) <<
                              " witness list sufficient: includes all srcdn replicas"
                              << dendl;
                              if (!mdr->more()->waiting_on_peer.empty()) {
                              dout(10) <<
                              " still waiting for rename notify acks from " <<
                              mdr->more()->waiting_on_peer << dendl; return;}
                              }
                              else
                              if (srcdnl->is_primary()
                                  && srcdn->authority() !=
                              ...
                              rollback.reqid = mdr->reqid;
                              rollback.orig_src.dirfrag =
                              srcdn->get_dir()->dirfrag();
                              rollback.orig_src.dirfrag_old_mtime =
                              srcdn->get_dir()->get_projected_fnode()->fragstat.
                              mtime;
                              rollback.orig_src.dirfrag_old_rctime =
                              srcdn->get_dir()->get_projected_fnode()->rstat.
                              rctime;
                              rollback.orig_src.dname = srcdn->get_name();
                              if (srcdnl->is_primary())
                              rollback.orig_src.ino =
                              srcdnl->get_inode()->ino();
                              else {
                              ceph_assert(srcdnl->is_remote());
                              rollback.orig_src.remote_ino =
                              srcdnl->get_remote_ino();
                              rollback.orig_src.remote_d_type =
                              srcdnl->get_remote_d_type();}

                              rollback.orig_dest.dirfrag =
                              destdn->get_dir()->dirfrag();
                              rollback.orig_dest.dirfrag_old_mtime =
                              destdn->get_dir()->get_projected_fnode()->
                              fragstat.mtime;
                              rollback.orig_dest.dirfrag_old_rctime =
                              destdn->get_dir()->get_projected_fnode()->rstat.
                              rctime;
                              rollback.orig_dest.dname = destdn->get_name();
                              if (destdnl->is_primary())
                              rollback.orig_dest.ino =
                              destdnl->get_inode()->ino();
                              else
                              if (destdnl->is_remote()) {
                              rollback.orig_dest.remote_ino =
                              destdnl->get_remote_ino();
                              rollback.orig_dest.remote_d_type =
                              destdnl->get_remote_d_type();}

                              if (straydn) {
                              rollback.stray.dirfrag =
                              straydn->get_dir()->dirfrag();
                              rollback.stray.dirfrag_old_mtime =
                              straydn->get_dir()->get_projected_fnode()->
                              fragstat.mtime;
                              rollback.stray.dirfrag_old_rctime =
                              straydn->get_dir()->get_projected_fnode()->rstat.
                              rctime;
                              ...
                              rollback_bl.length() << " bytes" << dendl;
                              // journal.
                              mdr->ls = mdlog->get_current_segment(); EPeerUpdate * le = new EPeerUpdate(mdlog, "peer_rename_prep", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_PREPARE, EPeerUpdate::RENAME); mdlog->start_entry(le); le->rollback = mdr->more()->rollback_bl; bufferlist blah;  // inode import data... obviously not used if we're the peer
                              _rename_prepare(mdr, &le->commit, &blah, srcdn,
                                              destdn,
                                              mdr->peer_request->alternate_name,
                                              straydn);
                              if (le->commit.empty()) {
                              dout(10) << " empty metablob, skipping journal" <<
                              dendl; mdlog->cancel_entry(le); mdr->ls = NULL;
                              ...
                              void Server::
                              _logged_peer_rename(MDRequestRef & mdr,
                                                  CDentry * srcdn,
                                                  CDentry * destdn,
                                                  CDentry * straydn) {
                              dout(10) << "_logged_peer_rename " << *mdr <<
                              dendl;
                              // prepare ack
                              ref_t < MMDSPeerRequest > reply;
                              if (!mdr->aborted) {
                      ...
                      srcdnl->get_inode()->get_dirfrags(bounds); for (const auto & bound:bounds)
                              {
                              bound->state_set(CDir::STATE_EXPORTBOUND);}
                              }

                  map < client_t, entity_inst_t > exported_client_map; map < client_t, client_metadata_t > exported_client_metadata_map; bufferlist inodebl; mdcache->migrator->encode_export_inode(srcdnl->get_inode(), inodebl, exported_client_map, exported_client_metadata_map); for (const auto & bound:bounds)
                              {
                              bound->state_clear(CDir::STATE_EXPORTBOUND);}

                              encode(exported_client_map, reply->inode_export,
                                     mds->mdsmap->get_up_features());
                              encode(exported_client_metadata_map,
                                     reply->inode_export);
                              reply->inode_export.claim_append(inodebl);
                              reply->inode_export_v =
                              srcdnl->get_inode()->get_version();}
                              ...
                              mds->balancer->hit_dir(srcdn->get_dir(),
                                                     META_POP_IWR);
                              if (destdnl->get_inode()
                                  && destdnl->get_inode()->is_auth())
                              mds->balancer->hit_inode(destdnl->get_inode(),
                                                       META_POP_IWR);
                              // done.
                              mdr->reset_peer_request();
                              mdr->straydn = 0; if (reply) {
                              mds->send_message_mds(reply, mdr->peer_to_mds);}
                              else {
                              ...
                              void Server::
                              _commit_peer_rename(MDRequestRef & mdr, int r,
                                                  CDentry * srcdn,
                                                  CDentry * destdn,
                                                  CDentry * straydn) {
                              dout(10) << "_commit_peer_rename " << *mdr <<
                              " r=" << r << dendl;
                              CInode * in = destdn->get_linkage()->get_inode();
                              inodeno_t migrated_stray;
                              if (srcdn->is_auth()
                                  && srcdn->get_dir()->inode->is_stray())
                              migrated_stray = in->ino();
                              MDSContext::vec finished; if (r == 0) {
                              ...
                              for (auto i =
                                   mdr->locks.lower_bound(&in->versionlock);
                                   i != mdr->locks.end();) {
                              SimpleLock * lock = i->lock;
                              ...
                              map < client_t, Capability::Import > peer_imported; auto bp = mdr->more()->inode_import.cbegin(); decode(peer_imported, bp); dout(10) << " finishing inode export on " << *in << dendl; mdcache->migrator->finish_export_inode(in, mdr->peer_to_mds, peer_imported, finished); mds->queue_waiters(finished);  // this includes SINGLEAUTH waiters.
                              // unfreeze
                              ...
                              // write a commit to the journal
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_rename_commit",
                                              mdr->reqid, mdr->peer_to_mds,
                                              EPeerUpdate::OP_COMMIT,
                                              EPeerUpdate::RENAME);
                              mdlog->start_entry(le);
                              ...
                              if (mdr->more()->is_inode_exporter) {
                              dout(10) << " reversing inode export of " << *in
                              << dendl; in->abort_export();}
                              if (mdcache->
                                  is_ambiguous_peer_update(mdr->reqid,
                                                           mdr->peer_to_mds)) {
                              mdcache->remove_ambiguous_peer_update(mdr->reqid,
                                                                    mdr->
                                                                    peer_to_mds);
                              // rollback but preserve the peer request
                              do_rename_rollback(mdr->more()->rollback_bl,
                                                 mdr->peer_to_mds, mdr, false);
                              mdr->more()->rollback_bl.clear();}
                              else
                              do_rename_rollback(mdr->more()->rollback_bl,
                                                 mdr->peer_to_mds, mdr, true);}
                              else {
                              dout(10) <<
                              " rollback_bl empty, not rollback back rename (leader failed after getting extra witnesses?)"
                              << dendl;
                              // singleauth
                              if (mdr->more()->is_ambiguous_auth) {
                              if (srcdn->is_auth())
                              ...
                              static void _rollback_repair_dir(MutationRef &
                                                               mut, CDir * dir,
                                                               rename_rollback::
                                                               drec & r,
                                                               utime_t ctime,
                              ...
                              pf->version = dir->pre_dirty(); if (isdir) {
                              ...
                              mut->add_updated_lock(&dir->get_inode()->
                                                    filelock);
                              mut->add_updated_lock(&dir->get_inode()->
                                                    nestlock);}
      ...
      struct C_MDS_LoggedRenameRollback:public ServerLogContext
                              {
          MutationRef mut; CDentry * srcdn; version_t srcdnpv; CDentry * destdn; CDentry * straydn; map < client_t, ref_t < MClientSnap >> splits[2]; bool finish_mdr; C_MDS_LoggedRenameRollback(Server * s, MutationRef & m, MDRequestRef & r, CDentry * sd, version_t pv, CDentry * dd, CDentry * st, map < client_t, ref_t < MClientSnap >> _splits[2], bool f):
                              ServerLogContext(s, r), mut(m), srcdn(sd),
                              srcdnpv(pv), destdn(dd), straydn(st),
                              finish_mdr(f) {
                              splits[0].swap(_splits[0]);
                              splits[1].swap(_splits[1]);}
                              void finish(int r) override {
                              server->_rename_rollback_finish(mut, mdr, srcdn,
                                                              srcdnpv, destdn,
                                                              straydn, splits,
                                                              finish_mdr);}
                              };
                              void Server::do_rename_rollback(bufferlist & rbl,
                                                              mds_rank_t leader,
                                                              MDRequestRef &
                                                              mdr,
                                                              bool finish_mdr) {
                              rename_rollback rollback; auto p = rbl.cbegin();
                              decode(rollback, p);
                              dout(10) << "do_rename_rollback on " << rollback.
                              reqid << dendl;
                              // need to finish this update before sending resolve to claim the subtree
                              mdcache->add_rollback(rollback.reqid, leader);
                              MutationRef mut(new
                                              MutationImpl(nullptr, utime_t(),
                                                           rollback.reqid));
                              mut->ls = mds->mdlog->get_current_segment();
                              CDentry * srcdn = NULL;
                              CDir * srcdir =
                              mdcache->get_dirfrag(rollback.orig_src.dirfrag);
                              if (!srcdir)
                              srcdir =
                              mdcache->get_dirfrag(rollback.orig_src.dirfrag.
                                                   ino,
                                                   rollback.orig_src.dname);
                              if (srcdir) {
                              ...
                              CDentry * destdn = NULL;
                              CDir * destdir =
                              mdcache->get_dirfrag(rollback.orig_dest.dirfrag);
                              if (!destdir)
                              destdir =
                              mdcache->get_dirfrag(rollback.orig_dest.dirfrag.
                                                   ino,
                                                   rollback.orig_dest.dname);
                              if (destdir) {
                              dout(10) << " destdir " << *destdir << dendl;
                              destdn =
                              destdir->lookup(rollback.orig_dest.dname);
                              if (destdn)
                              dout(10) << "  destdn " << *destdn << dendl;
                              else
                              ...
                              CInode * in = NULL; if (rollback.orig_src.ino) {
                              in = mdcache->get_inode(rollback.orig_src.ino);
                              if (in && in->is_dir())
                              ceph_assert(srcdn && destdn);}
                              else
                              in =
                              mdcache->get_inode(rollback.orig_src.remote_ino);
                              CDir * straydir = NULL; CDentry * straydn = NULL;
                              if (rollback.stray.dirfrag.ino) {
                              straydir =
                              mdcache->get_dirfrag(rollback.stray.dirfrag);
                              if (straydir) {
                              dout(10) << "straydir " << *straydir << dendl;
                              straydn = straydir->lookup(rollback.stray.dname);
                              if (straydn) {
                              dout(10) << " straydn " << *straydn << dendl;
                              ceph_assert(straydn->get_linkage()->
                                          is_primary());}
                              else
                              ...
                              target =
                              mdcache->get_inode(rollback.orig_dest.ino);
                              if (target)
                              ceph_assert(destdn && straydn);}
                              else
                              if (rollback.orig_dest.remote_ino)
                              target =
                              mdcache->get_inode(rollback.orig_dest.remote_ino);
                              // can't use is_auth() in the resolve stage
                              mds_rank_t whoami = mds->get_nodeid();
                              // peer
                              ceph_assert(!destdn
                                          || destdn->authority().first !=
                                          whoami);
                              ceph_assert(!straydn
                                          || straydn->authority().first !=
                                          whoami);
                              ...
                              force_journal_src =
                              _need_force_journal(in, false);
                              if (in && target && target->is_dir())
                              force_journal_dest =
                              _need_force_journal(in, true);
                              version_t srcdnpv = 0;
                              // repair src
                              if (srcdn) {
                              ...
                              srcdn->push_projected_linkage(rollback.orig_src.
                                                            remote_ino,
                                                            rollback.orig_src.
                                                            remote_d_type);}
                              ...
                              SnapRealm * realm; if (rollback.orig_src.ino) {
                              ceph_assert(srcdir);
                              ...
                              if (rollback.orig_dest.remote_ino) {
                              destdn->push_projected_linkage(rollback.orig_dest.
                                                             remote_ino,
                                                             rollback.orig_dest.
                                                             remote_d_type);}
                              else {
                              ...
                              if (straydn)
                              straydn->push_projected_linkage(); if (target) {
                              bool projected;
                              CInode::inode_ptr ti;
                              ...
                              if (MDS_INO_IS_STRAY
                                  (rollback.orig_src.dirfrag.ino)) {
                              if (MDS_INO_IS_STRAY
                                  (rollback.orig_dest.dirfrag.ino))
                              ...
                              ti->nlink++; if (!projected)
                              target->reset_inode(ti);
                              if (rollback.desti_snapbl.length()
                                  && target->snaprealm) {
                              bool hadrealm;
                              ...
                              SnapRealm * realm; if (rollback.orig_dest.ino) {
                              ceph_assert(destdir);
                              realm = destdir->get_inode()->find_snaprealm();}
                              else {
                              realm = target->snaprealm->parent;}
                              if (!mds->is_resolve())
                              mdcache->prepare_realm_merge(target->snaprealm,
                                                           realm, splits[1]);
                              if (projected)
                              ...
                              nest_info_t blah;
                              _rollback_repair_dir(mut, srcdir,
                                                   rollback.orig_src,
                                                   rollback.ctime, in
                                                   && in->is_dir(),
                                                   pip ? pip->
                                                   accounted_rstat : blah);}
                              ...
                              // journal it
                              EPeerUpdate * le =
                              new EPeerUpdate(mdlog, "peer_rename_rollback",
                                              rollback.reqid, leader,
                                              EPeerUpdate::OP_ROLLBACK,
                                              EPeerUpdate::RENAME);
                              mdlog->start_entry(le);
                              ...
                              if (target && target != in
                                  && target->authority().first == whoami) {
                              ceph_assert(rollback.orig_dest.remote_ino);
                              le->commit.add_dir_context(target->
                                                         get_projected_parent_dir
                                                         ());
                              le->commit.add_primary_dentry(target->
                                                            get_projected_parent_dn
                                                            (), target, true);}
                              ...
                              if (in && in->is_dir()
                                  && (srcdn->authority().first == whoami
                                      || force_journal_src)) {
                              dout(10) << " noting renamed dir ino " << in->
                              ino() << " in metablob" << dendl;
                              le->commit.renamed_dirino = in->ino();
                              if (srcdn->authority().first == whoami) {
              auto && ls = in->get_dirfrags(); for (const auto & dir:ls)
                              {
                              if (!dir->is_auth())
                              le->commit.renamed_dir_frags.push_back(dir->
                                                                     get_frag
                                                                     ());}
                              dout(10) << " noting renamed dir open frags " <<
                              ...
                              if (force_journal_dest) {
                              dout(10) << " noting rename target ino " <<
                              target->ino() << " in metablob" << dendl;
                              le->commit.renamed_dirino = target->ino();}

                              if (target && target->is_dir()) {
                              ceph_assert(destdn);
                              mdcache->project_subtree_rename(target, straydir,
                                                              destdir);}

                              if (in && in->is_dir()) {
                              ceph_assert(srcdn);
                              mdcache->project_subtree_rename(in, destdir,
                                                              srcdir);}
                              ...
                              mut->ls = NULL;
                              _rename_rollback_finish(mut, mdr, srcdn, srcdnpv,
                                                      destdn, straydn, splits,
                                                      finish_mdr);}
                              else {
                              ceph_assert(!le->commit.empty()); if (mdr)
                              mdr->more()->peer_update_journaled = false;
                              MDSLogContextBase * fin =
                              new C_MDS_LoggedRenameRollback(this, mut, mdr,
                                                             srcdn, srcdnpv,
                                                             destdn, straydn,
                                                             splits,
                                                             finish_mdr);
                              ...
                              void Server::
                              _rename_rollback_finish(MutationRef & mut,
                                                      MDRequestRef & mdr,
                                                      CDentry * srcdn,
                                                      version_t srcdnpv,
                                                      CDentry * destdn,
                                                      ...
                                                      map < client_t,
                                                      ref_t <
                                                      MClientSnap >> splits[2],
                                                      bool finish_mdr) {
                              dout(10) << "_rename_rollback_finish " << mut->
                              reqid << dendl; if (straydn) {
                              ...
                              if (mds->is_resolve()) {
                              CDir * root = NULL; if (straydn)
                              root =
                              mdcache->get_subtree_root(straydn->get_dir());
                              else
                              if (destdn)
                              root =
                              mdcache->get_subtree_root(destdn->get_dir());
                              if (root)
                              mdcache->try_trim_non_auth_subtree(root);}
                              else {
                              ...
                              void Server::
                              handle_peer_rename_prep_ack(MDRequestRef & mdr,
                                                          const cref_t <
                                                          MMDSPeerRequest >
                                                          &ack) {
                              ...
                              if (mdr->more()->srcdn_auth_mds == from &&
                                  ...
                                  !mdr->more()->is_ambiguous_auth) {
                              mdr->set_ambiguous_auth(mdr->more()->
                                                      rename_inode);}

                              // witnessed?  or add extra witnesses?
                              ceph_assert(mdr->more()->witnessed.count(from) ==
                                          0); if (ack->is_interrupted()) {
                              dout(10) << " peer request interrupted, noop" <<
                              dendl;}
                              else
                              ...
                              dout(10) << " extra witnesses (srcdn replicas) are " << ack->witnesses << dendl; mdr->more()->extra_witnesses = ack->witnesses; mdr->more()->extra_witnesses.erase(mds->get_nodeid());    // not me!
                              }

                              // srci import?
                              if (ack->inode_export.length()) {
                              dout(10) << " got srci import" << dendl;
                              mdr->more()->inode_import.share(ack->
                                                              inode_export);
                              mdr->more()->inode_import_v =
                              ack->inode_export_v;}
                              ...
                              void Server::
                              handle_peer_rename_notify_ack(MDRequestRef & mdr,
                                                            const cref_t <
                                                            MMDSPeerRequest >
                                                            &ack) {
                              dout(10) << "handle_peer_rename_notify_ack " <<
                              *mdr << " from mds." << ack->
                              get_source() << dendl;
                              ...
                              void Server::
                              _peer_rename_sessions_flushed(MDRequestRef & mdr)
                              {
                              dout(10) << "_peer_rename_sessions_flushed " <<
                              *mdr << dendl;
                              if (mdr->more()->waiting_on_peer.
                                  count(MDS_RANK_NONE)) {
...
/* This function takes responsibility for the passed mdr*/
                              void Server::
                              handle_client_lssnap(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              // traverse to path
                              CInode * diri =
                              try_get_auth_inode(mdr,
                                                 req->get_filepath().get_ino());
                              if (!diri)
                              return; if (!diri->is_dir()) {
                              respond_to_request(mdr, -CEPHFS_ENOTDIR); return;}
                              dout(10) << "lssnap on " << *diri << dendl;
                              // lock snap
                              if (!mds->locker->
                                  try_rdlock_snap_layout(diri, mdr))
                              return; if (!check_access(mdr, diri, MAY_READ))
                              return;
                              SnapRealm * realm = diri->find_snaprealm();
                              map < snapid_t, const SnapInfo * >infomap;
                              realm->get_snap_info(infomap,
                                                   diri->get_oldest_snap());
                              unsigned max_entries =
                              req->head.args.readdir.max_entries;
                              if (!max_entries)
                              ...
                              if (!offset_str.empty())
                              last_snapid =
                              realm->resolve_snapname(offset_str, diri->ino());
                              //Empty DirStat
                              bufferlist dirbl;
                              static DirStat empty;
                              CDir::encode_dirstat(dirbl, mdr->session->info,
                                                   empty);
                              max_bytes -=
                              dirbl.length() - sizeof(__u32) + sizeof(__u8) * 2;
                              __u32 num = 0; bufferlist dnbl;
                              auto p = infomap.upper_bound(last_snapid); for (;
                                                                              p
                                                                              !=
                                                                              infomap.
                                                                              end
                              ...
                              unsigned start_len = dnbl.length();
                              if (int
                                  (start_len + snap_name.length() +
                                   sizeof(__u32) + sizeof(LeaseStat)) >
                                  max_bytes)
                              break; encode(snap_name, dnbl);
                              //infinite lease
                              LeaseStat e(CEPH_LEASE_VALID, -1, 0);
                              mds->locker->encode_lease(dnbl,
                                                        mdr->session->info, e);
                              dout(20) << "encode_infinite_lease" << dendl;
                              int r =
                              diri->encode_inodestat(dnbl, mdr->session, realm,
                                                     p->first,
                                                     max_bytes -
                                                     (int)dnbl.length());
                              if (r < 0) {
                              bufferlist keep;
                              keep.substr_of(dnbl, 0, start_len);
                              dnbl.swap(keep); break;}
                              ++num;}

                              encode(num, dirbl);
                              __u16 flags = 0; if (p == infomap.end()) {
                              flags = CEPH_READDIR_FRAG_END;
                              if (last_snapid == 0)
                              ...
                              dirbl.claim_append(dnbl);
                              mdr->reply_extra_bl = dirbl;
                              mdr->tracei = diri; respond_to_request(mdr, 0);}

// MKSNAP

      struct C_MDS_mksnap_finish:public ServerLogContext
                              {
          CInode * diri; SnapInfo info; C_MDS_mksnap_finish(Server * s, MDRequestRef & r, CInode * di, SnapInfo & i):
                              ServerLogContext(s, r), diri(di),
                              info(i) {
                              }
                              void finish(int r) override {
...
/* This function takes responsibility for the passed mdr*/
                              void Server::
                              handle_client_mksnap(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              // make sure we have as new a map as the client
                              if (req->get_mdsmap_epoch() >
                                  mds->mdsmap->get_epoch()) {
                              mds->wait_for_mdsmap(req->get_mdsmap_epoch(),
                                                   new
                                                   C_MDS_RetryRequest(mdcache,
                                                                      mdr));
                              ...
                              dout(5) <<
                              "new snapshots are disabled for this fs" << dendl;
                              respond_to_request(mdr, -CEPHFS_EPERM); return;}
                              ...
                              req->get_filepath().last_dentry();
                              if (mdr->client_request->get_caller_uid() <
                                  g_conf()->mds_snap_min_uid
                                  || mdr->client_request->get_caller_uid() >
                                  g_conf()->mds_snap_max_uid) {
                              dout(20) << "mksnap " << snapname << " on " <<
                              *diri << " denied to uid " << mdr->
                              client_request->get_caller_uid() << dendl;
                              respond_to_request(mdr, -CEPHFS_EPERM); return;}

                              dout(10) << "mksnap " << snapname << " on " <<
                              *diri << dendl;
                              // lock snap
                              if (!
                                  (mdr->
                              ...
                              return;
                              if (CDentry * pdn =
                                  diri->get_projected_parent_dn(); pdn) {
                              if (!mds->locker->
                                  try_rdlock_snap_layout(pdn->get_dir()->
                                                         get_inode(), mdr))
                              return;}
                              ...
                              if (!check_access
                                  (mdr, diri, MAY_WRITE | MAY_SNAPSHOT))
                              return;
                              if (inodeno_t subvol_ino =
                                  diri->find_snaprealm()->get_subvolume_ino();
                                  (subvol_ino && subvol_ino != diri->ino())) {
                              dout(5) << "is a descendent of a subvolume dir" <<
                              dendl; respond_to_request(mdr, -CEPHFS_EPERM);
                              return;}

                              // check if we can create any more snapshots
                              // we don't allow any more if we are already at or beyond the limit
                              if (diri->snaprealm &&
                                  diri->snaprealm->get_snaps().size() >=
                                  max_snaps_per_dir) {
                              respond_to_request(mdr, -CEPHFS_EMLINK); return;}
                              ...
                              decode(snapid, p);
                              dout(10) << " stid " << stid << " snapid " <<
                              snapid << dendl;
                              ceph_assert(mds->snapclient->
                              ...
                              auto em =
                              newsnap.snaps.emplace(std::piecewise_construct,
                                                    std::
                                                    forward_as_tuple(snapid),
                                                    std::
                                                    forward_as_tuple(info));
                              if (!em.second)
                              em.first->second = info; newsnap.seq = snapid;
                              ...
                              le->metablob.add_client_req(req->get_reqid(),
                                                          ...
                                                          get_oldest_client_tid
                                                          ());
                              le->metablob.add_table_transaction(TABLE_SNAP,
                                                                 stid);
                              mdcache->predirty_journal_parents(mdr,
                                                                &le->metablob,
                                                                diri, 0,
                              ...
                              void Server::_mksnap_finish(MDRequestRef & mdr,
                                                          CInode * diri,
                                                          SnapInfo & info) {
                              dout(10) << "_mksnap_finish " << *mdr << " " <<
                              info << dendl;
                              int op =
                              (diri->
                               snaprealm ? CEPH_SNAP_OP_CREATE :
                               CEPH_SNAP_OP_SPLIT); mdr->apply();
                              mds->snapclient->commit(mdr->more()->stid,
                                                      mdr->ls);
                              // create snap
                              dout(10) << "snaprealm now " << *diri->
                              snaprealm << dendl;
                              // notify other mds
                              mdcache->send_snap_update(diri, mdr->more()->stid,
                                                        op);
                              mdcache->
                              do_realm_invalidate_and_update_notify(diri, op);
                              // yay
                              mdr->in[0] = diri;
...
// RMSNAP
      ...
      struct C_MDS_rmsnap_finish:public ServerLogContext
                              {
          CInode * diri; snapid_t snapid; C_MDS_rmsnap_finish(Server * s, MDRequestRef & r, CInode * di, snapid_t sn):
                              ServerLogContext(s, r), diri(di),
                              snapid(sn) {
                              }
                              void finish(int r) override {
...
/* This function takes responsibility for the passed mdr*/
                              void Server::
                              handle_client_rmsnap(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              CInode * diri =
                              try_get_auth_inode(mdr,
                              ...
                              if (mdr->client_request->get_caller_uid() <
                                  ...
                                  || mdr->client_request->get_caller_uid() >
                                  g_conf()->mds_snap_max_uid) {
                              dout(20) << "rmsnap " << snapname << " on " <<
                              *diri << " denied to uid " << mdr->
                              client_request->get_caller_uid() << dendl;
                              respond_to_request(mdr, -CEPHFS_EPERM); return;}

                              dout(10) << "rmsnap " << snapname << " on " <<
                              *diri << dendl;
                              // does snap exist?
                              ...
                              if (!diri->snaprealm
                                  || !diri->snaprealm->exists(snapname)) {
                              respond_to_request(mdr, -CEPHFS_ENOENT); return;}
                              snapid_t snapid =
                              diri->snaprealm->resolve_snapname(snapname,
                                                                diri->ino());
                              dout(10) << " snapname " << snapname << " is " <<
                              snapid << dendl;
                              if (!
                                  (mdr->
                                   locking_state & MutationImpl::ALL_LOCKED)) {
                              ...
                              // prepare
                              if (!mdr->more()->stid) {
                              mds->snapclient->prepare_destroy(diri->ino(),
                                                               snapid,
                                                               &mdr->more()->
                                                               stid,
                                                               &mdr->more()->
                              ...
                              version_t stid = mdr->more()->stid;
                              auto p = mdr->more()->snapidbl.cbegin();
                              snapid_t seq; decode(seq, p);
                              dout(10) << " stid is " << stid << ", seq is " <<
                              seq << dendl;
                              ceph_assert(mds->snapclient->
                                          get_cached_version() >= stid);
                              // journal
                              ...
                              void Server::_rmsnap_finish(MDRequestRef & mdr,
                                                          CInode * diri,
                                                          snapid_t snapid) {
                              dout(10) << "_rmsnap_finish " << *mdr << " " <<
                              snapid << dendl;
                              snapid_t stid = mdr->more()->stid; mdr->apply();
                              ...
                              mdr->in[0] = diri;
                              mdr->tracei = diri;
                              mdr->snapid = snapid; respond_to_request(mdr, 0);
                              // purge snapshot data
                              diri->purge_stale_snap_data(diri->snaprealm->
                                                          get_snaps());}

      struct C_MDS_renamesnap_finish:public ServerLogContext
                              {
          CInode * diri; snapid_t snapid; C_MDS_renamesnap_finish(Server * s, MDRequestRef & r, CInode * di, snapid_t sn):
                              ServerLogContext(s, r), diri(di),
                              snapid(sn) {
                              }
                              void finish(int r) override {
...
/* This function takes responsibility for the passed mdr*/
                              void Server::
                              handle_client_renamesnap(MDRequestRef & mdr) {
                              const cref_t < MClientRequest > &req =
                              mdr->client_request;
                              if (req->get_filepath().get_ino() !=
                                  req->get_filepath2().get_ino()) {
                              respond_to_request(mdr, -CEPHFS_EINVAL); return;}
                              ...
                              try_get_auth_inode(mdr,
                                                 req->get_filepath().get_ino());
                              if (!diri)
                              return; if (!diri->is_dir()) {    // dir only
                              respond_to_request(mdr, -CEPHFS_ENOTDIR); return;}
                              ...
                              req->get_filepath2().last_dentry();
                              dout(10) << "renamesnap " << srcname << "->" <<
                              dstname << " on " << *diri << dendl;
                              if (srcname.length() == 0 || srcname[0] == '_') {
                              respond_to_request(mdr, -CEPHFS_EINVAL);  // can't rename a parent snap.
                              return;}
                              if (!diri->snaprealm
                                  || !diri->snaprealm->exists(srcname)) {
                              respond_to_request(mdr, -CEPHFS_ENOENT); return;}
                              ...
                              snapid_t snapid =
                              diri->snaprealm->resolve_snapname(srcname,
                                                                diri->ino());
                              dout(10) << " snapname " << srcname << " is " <<
                              snapid << dendl;
                              // lock snap
                              if (!
                                  (mdr->
                              ...
                              // prepare
                              if (!mdr->more()->stid) {
                              mds->snapclient->prepare_update(diri->ino(),
                                                              snapid, dstname,
                                                              utime_t(),
                                                              &mdr->more()->
                                                              stid,
                              ...
                              void Server::
                              _renamesnap_finish(MDRequestRef & mdr,
                                                 CInode * diri,
                                                 snapid_t snapid) {
                              dout(10) << "_renamesnap_finish " << *mdr << " "
                              << snapid << dendl; mdr->apply();
                              mds->snapclient->commit(mdr->more()->stid,
                                                      mdr->ls);
                              dout(10) << "snaprealm now " << *diri->
                              snaprealm << dendl;
 ...
 */
                              bool Server::
                              waiting_for_reconnect(client_t c) const {
                              return client_reconnect_gather.count(c) > 0;}

                              void Server::dump_reconnect_status(Formatter *
                                                                 f) const {
                              f->open_object_section("reconnect_status");
                              f->
                              dump_stream("client_reconnect_gather") <<
                              client_reconnect_gather; f->close_section();}

                              const bufferlist & Server::
                              get_snap_trace(Session * session,
                                             SnapRealm * realm) const {
                              ceph_assert(session); ceph_assert(realm);
                              if (session->info.
                                  has_feature
                              ...
                              Session * session =
                              mds->sessionmap.
                              get_session(entity_name_t::CLIENT(client.v));
