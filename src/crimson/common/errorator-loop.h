...
class parallel_for_each_state final: private seastar::
    continuation_base <>
{
    using future_t =
        typename errorator < AllowedErrors ... >::template future <>;
                                     ...
                                     && func) noexcept
{
    ...
    for (; first != last; ++first) {
        auto f =
            seastar::futurize_invoke(std::forward < Func > (func), *first);
        if (!f.available() || f.failed()) {
            if (!s) {
                using itraits = std::iterator_traits < Iterator >;
                auto n =
                    (seastar::internal::
                     iterator_range_estimate_vector_capacity(first, last,
                         typename
                         itraits::
                         iterator_category
                         ()) + 1);
