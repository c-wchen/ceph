...
struct ipv4_traits {
    ...
    using packet_provider_type =
        std::function < std::optional < l4packet > () >;
    static void tcp_pseudo_header_checksum(checksummer & csum, ipv4_address src,
                                           ipv4_address dst, uint16_t len) {
        csum.sum_many(src.ip, dst.ip, uint8_t(0), uint8_t(ip_protocol_num::tcp),
                      len);
...
class ipv4_tcp final:public ip_protocol {
  ...
  public:
    ...
    ~ipv4_tcp();
    virtual void received(Packet p, ipv4_address from,
                          ipv4_address to) override;
    virtual bool forward(forward_hash & out_hash_data, Packet & p,
                         size_t off) override;
    friend class ipv4;
};
  ...
  public:
    ...
    :cct(c), _inet(inet), _queue_space(c, "DPDK::icmp::_queue_space", 212992) {
        _inet.register_packet_provider([this] {
                                       std::optional < ipv4_traits::l4packet >
                                       l4p; if (!_packetq.empty()) {
                                       l4p = std::move(_packetq.front());
                                       _packetq.pop_front();
                                       _queue_space.put(l4p->p.
                                                        len());} return l4p;}
        ) ;
    }
...
class ipv4_icmp final:public ip_protocol {
    ...
    icmp _icmp;
  public:
     ipv4_icmp(CephContext * c, ipv4 & inet):cct(c), _inet_l4(inet), _icmp(c,
                                                                           _inet_l4)
    {
    } virtual void received(Packet p, ipv4_address from,
                            ipv4_address to) override {
...
struct ip_packet_filter {
    ...
    virtual void handle(Packet & p, ip_hdr * iph, ethernet_address from,
                        bool & handled) = 0;
};
...
class ipv4 {
  ...
  private:
    ...
    static constexpr uint32_t _frag_low_thresh {
    3 *1024 * 1024};
    static constexpr uint32_t _frag_high_thresh {
    4 *1024 * 1024};
  ...
  private:
    ...
    void frag_arm(utime_t now) {
        ...
        frag_timefd =
            center->create_time_event(tp.to_nsec() / 1000, frag_handler);
    }
    void frag_arm() {
        auto now = ceph_clock_now();
        frag_timefd =
            center->create_time_event(now.to_nsec() / 1000, frag_handler);
    }
  ...
  public:
    ...
    // TODO or something. Should perhaps truly be a list// of filters. With ordering. And blackjack. Etc.// But for now, a simple single raw pointer suffices
        void set_packet_filter(ip_packet_filter * f) {
        _packet_filter = f;
    }
    ip_packet_filter *packet_filter() const {
        return _packet_filter;
    } void send(ipv4_address to, ip_protocol_num proto_num, Packet p,
                ethernet_address e_dst);
    ...
    static bool needs_frag(Packet & p, ip_protocol_num proto_num,
...
template < ip_protocol_num ProtoNum >
    inline void ipv4_l4 <
    ProtoNum >::register_packet_provider(ipv4_traits::packet_provider_type func)
{
    _inet.register_packet_provider([func] {
                                   auto l4p = func(); if (l4p) {
...
template < ip_protocol_num ProtoNum >
    inline void ipv4_l4 < ProtoNum >::wait_l2_dst_address(ipv4_address to,
                                                          Packet p,
                                                          resolution_cb cb)
{
...
struct ip_hdr {
    ...
    uint16_t id;
    uint16_t frag;
    enum class frag_bits:uint8_t { mf = 13, df = 14, reserved =
            15, offset_shift = 3 };
    uint8_t ttl;
    uint8_t ip_proto;
    uint16_t csum;
