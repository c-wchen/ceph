...
// per bucket-shard state cached by DataSyncShardCR
    struct State {
        ...
        State(const std::pair < rgw_bucket_shard,
              std::optional < uint64_t >> &key) noexcept:key(key) {
        } State(const rgw_bucket_shard & shard,
                std::optional < uint64_t > gen) noexcept:key(shard, gen) {
    }};
    ...
    using lru_config = ceph::common::intrusive_lru_config <
        std::pair < rgw_bucket_shard, std::optional < uint64_t >>, Entry,
        EntryToKey >;
...
// a recyclable cache entry
    ...
    struct EntryToKey {
        ...
        const type & operator() (const Entry & e) {
            return e.key;
    }};
...
// a state cache for entries within a single datalog shard
    class Cache:public thread_unsafe_ref_counter < Cache > {
      ...
      protected:
        ...
        Handle get(const rgw_bucket_shard & shard,
...
// a State handle that keeps the Cache referenced
    class Handle {
      ...
      public:
        ...
        Handle(boost::intrusive_ptr < Cache > cache,
               boost::intrusive_ptr < Entry >
               entry) noexcept:cache(std::move(cache)),
    ...
    inline Handle Cache::get(const rgw_bucket_shard & shard,
                             std::optional < uint64_t > gen) {
