...
{
    return store->getRados()->get_bucket_stats(dpp, info, idx_layout, shard_id, bucket_ver, master_ver, stats, max_marker,
            syncstopped);
}
...
int RadosBucket::check_index(const DoutPrefixProvider *dpp, std::map < RGWObjCategory,
                             RGWStorageStats > & existing_stats, std::map < RGWObjCategory, RGWStorageStats > & calculated_stats)
{
...
{
    int max_shards = (info.layout.current_index.layout.normal.num_shards > 0 ?
...
std::unique_ptr < Notification > RadosStore::get_notification(
    rgw::sal::Object* obj, rgw::sal::Object* src_obj, req_state* s, rgw::notify::EventType event_type, optional_yield y,
...
{
    return rgw_stat_system_obj(dpp, svc()->sysobj, svc()->zone->get_zone_params().log_pool, topics_oid(tenant), nullptr,
                               nullptr, y, nullptr);
}
...
{
    return std::make_unique < RadosRole > (this, name, tenant, std::move(account_id), path, trust_policy,
                                    ...
                                    bool exclusive)
{
    ...
    using ceph::encode;
    encode(info, bl);
    return rgw_put_system_obj(dpp, sysobj, svc()->zone->get_zone_params().oidc_pool, oid, bl, exclusive, nullptr,
                              real_time(), y);
}
...
{
    int ret = store->getRados()->get_obj_state(dpp, rados_ctx, bucket->get_info(), get_obj(), pstate, &manifest, follow_olh,
              y);
    if (ret < 0) {
        return ret;
    }
                                     ...
                                     optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to transfer object(" << o.key << ") to the cloud endpoint(" << endpoint << ") ret="
                          << ret << dendl;
        return ret;
    }
                                   ...
                                   prefix_map_t &processed_prefixes)
{
    ...
    do {
        ...
        for (auto obj_iter = parts.begin(); etags_iter != part_etags.end() &&
             obj_iter != parts.end(); ++etags_iter, ++obj_iter, ++handled_parts) {
...
void RadosLuaManager::handle_reload_notify(const DoutPrefixProvider* dpp, optional_yield y, uint64_t notify_id,
        uint64_t cookie)
{
...
void RadosLuaManager::PackagesWatcher::handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id,
        bufferlist &bl)
{
...
int RadosRole::store_name(const DoutPrefixProvider *dpp, bool exclusive, optional_yield y)
{
    ...
    return rgw_put_system_obj(dpp, sysobj, store->svc()->zone->get_zone_params().roles_pool, oid, bl, exclusive,
                       ...
                       std::string& role_id, optional_yield y)
{
    ...
    int ret = rgw_get_system_obj(sysobj, store->svc()->zone->get_zone_params().roles_pool, oid, bl, nullptr, nullptr, y,
                                 dpp);
    if (ret < 0) {
        return ret;
    }
...
int RadosRole::read_info(const DoutPrefixProvider *dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed reading role info from Role pool: " << info.id << ": " << cpp_strerror(
                              -ret) << dendl;
        return ret;
    }
