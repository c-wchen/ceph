...
namespace librbd {
    ...
    static bool operator==(const trash_image_info_t & lhs,
        ...
        return (lhs.id == rhs.id &&
                lhs.name == rhs.name && lhs.source == rhs.source);
    ...
    TEST_F(TestTrash, UserRemovingSource) {
        ...
        auto compare_lambda =[](const trash_image_info_t & lhs,
            ...
            if (lhs.id != rhs.id) {
                                   ...
                                   typedef std::set < trash_image_info_t,
                                   decltype(compare_lambda) > TrashEntries;
                                   librbd::RBD rbd; librbd::Image image;
                                   auto image_name1 = m_image_name;
                                   std::string image_id1;
                                   ASSERT_EQ(0,
                                             rbd.open(m_ioctx, image,
                                                      image_name1.c_str()));
                                   ASSERT_EQ(0, image.get_id(&image_id1));
                                   ASSERT_EQ(0, image.close());
                                   auto image_name2 = get_temp_image_name();
                                   ASSERT_EQ(0,
                                             create_image_pp(m_rbd, m_ioctx,
                                                             image_name2,
                                                             m_image_size));
                                   ...
                                   ASSERT_EQ(0,
                                             api::Trash <>::move(m_ioctx,
                                                                 ...
                                                                 image_name2,
                                                                 image_id2, 0));
                                   TrashEntries trash_entries {
                                   compare_lambda};
                                   TrashEntries expected_trash_entries {
                                   compare_lambda};
                                   std::vector < trash_image_info_t > entries;
                                   ASSERT_EQ(0,
                                             api::Trash <>::list(m_ioctx,
                                                                 entries,
                                                                 true));
                                   trash_entries.insert(entries.begin(),
                                                        entries.end());
                                   expected_trash_entries = {
                                   {
                                   .id = image_id1,.name = image_name1,.source =
                                   RBD_TRASH_IMAGE_SOURCE_USER},};
                                   ASSERT_EQ(expected_trash_entries,
                                             trash_entries);
                                   std::vector < image_spec_t >
                                   expected_images = {
                                   {
