...
static PyObject *osdmap_map_pool_pgs_up(BasePyOSDMap *self, PyObject *args)
{
    ...
    for (unsigned ps = 0; ps < pi->get_pg_num(); ++ps) {
        pg_t pgid(ps, poolid);
        self->osdmap->pg_to_up_acting_osds(pgid, &pm[pgid], nullptr, nullptr,
                                           nullptr);
    }
    PyFormatter f;
    for (auto p : pm) {
...
static void BasePyOSDMap_dealloc(BasePyOSDMap *self)
{
    if (self->osdmap) {
        ...
        derr << "Destroying improperly initialized BasePyOSDMap " << self <<
             dendl;
    }
    Py_TYPE(self)->tp_free(self);
}

static PyObject *osdmap_pg_to_up_acting_osds(BasePyOSDMap *self,
        PyObject *args)
{
    int pool_id = 0;
    int ps = 0;
...
static PyObject *osdmap_build_simple(PyObject *cls, PyObject *args,
                                     PyObject *kwargs)
{
    ...
    auto osdmap = without_gil([&] {
        ...
        osdmap->build_simple(g_ceph_context, epoch, uuid,
                             num_osd); return osdmap;
    }
                             );
    return construct_with_capsule("mgr_module", "OSDMap",
                                  reinterpret_cast < void *>(osdmap));
}
...
PyMethodDef BasePyOSDMap_methods[] = {
    ...
    {
        "_dump", (PyCFunction) osdmap_dump, METH_NOARGS,
...
static int BasePyOSDMapIncremental_init(BasePyOSDMapIncremental *self,
                                        PyObject *args, PyObject *kwds)
{
    ...
    self->inc =
        (OSDMap::Incremental *) PyCapsule_GetPointer(inc_capsule, nullptr);
...
static PyObject
*osdmap_inc_set_compat_weight_set_weights(BasePyOSDMapIncremental *self,
        PyObject *weightobj)
{
    map < int, double >wm;
    if (get_int_float_map(weightobj, &wm) < 0) {
    ...
    for (auto i : wm) {
        crush.choose_args_adjust_item_weightf(g_ceph_context,
                                              crush.
                                              choose_args_get(CrushWrapper::
                                                      DEFAULT_CHOOSE_ARGS),
        i.first, {
            i.second
        }
...
static PyObject *crush_get_take_weight_osd_map(BasePyCRUSH *self,
        PyObject *args)
{
    int root;
