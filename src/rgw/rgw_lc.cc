...
public:
    ...
    int delete_tier_obj(lc_op_ctx &oc)
    {
        ...
        if (oc.bucket->versioned() && oc.o.is_current() && !oc.o.is_delete_marker()) {
            ...
            ldpp_dout(oc.dpp, 20) << "delete_tier_obj Object(key:" << oc.o.key << ") not current " << "versioned_epoch:  " <<
    ...
    for (const auto &ri : rule_map) {
        ...
        if (rule_expiration.has_date()) {
            ...
            if (rule_expiration.has_days()) {
                rule_expiration_date =
                    boost::optional<ceph::real_time>(
                        mtime + make_timespan(double(rule_expiration.get_days()) * 24 * 60 * 60 - ceph::real_clock::to_time_t(mtime) %
                                              (24 * 60 * 60) + 24 * 60 * 60));
            }
        }
    ...
    for (const auto &ri : rule_map) {
        ...
        if (mp_expiration.has_days()) {
            rule_abort_date = std::optional<ceph::real_time>(
                                  mtime + make_timespan(mp_expiration.get_days() * 24 * 60 * 60 - ceph::real_clock::to_time_t(mtime) %
                                      (24 * 60 * 60) + 24 * 60 * 60));
        }
