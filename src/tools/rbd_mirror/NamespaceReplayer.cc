...
const std::
string SERVICE_DAEMON_LOCAL_COUNT_KEY("image_local_count");
const std::
string SERVICE_DAEMON_REMOTE_COUNT_KEY("image_remote_count");

} // anonymous namespace template < typename I >
NamespaceReplayer < I >::NamespaceReplayer(const std::string &name,
        librados::
        IoCtx &local_io_ctx,
        librados::
        IoCtx &remote_io_ctx,
        ...
        *service_daemon,
        ...
        PoolMetaCache *
        pool_meta_cache): m_namespace_name
    (name), m_local_mirror_uuid(local_mirror_uuid),
    m_local_mirror_peer_uuid(local_mirror_peer_uuid),
    m_remote_pool_meta(remote_pool_meta), m_threads(threads),
    ...
    m_lock(ceph::
           make_mutex(librbd::util::
                      unique_lock_name("rbd::mirror::NamespaceReplayer "
                                       + name, this))),
...
template < typename I >
void NamespaceReplayer <
I >::handle_update(const std::string &mirror_uuid, ImageIds
                   && added_image_ids, ImageIds
                   && removed_image_ids)
{
    std::lock_guard locker {
        m_lock};
    ...
    m_service_daemon->add_or_update_namespace_attribute(m_local_io_ctx.
            get_id(),
            m_local_io_ctx.
            get_namespace(),
    ...
    if (m_remote_pool_watcher) {
        m_service_daemon->
        add_or_update_namespace_attribute(m_local_io_ctx.get_id(),
                                          m_local_io_ctx.
                                          get_namespace(),
...
template < typename I >
void NamespaceReplayer <
I >::handle_acquire_leader(Context *on_finish)
{
    dout(10) << dendl;
...
template < typename I >
void NamespaceReplayer <
I >::handle_release_leader(Context *on_finish)
{
    dout(10) << dendl;
...
template < typename I >
void NamespaceReplayer <
I >::handle_instances_added(const std::vector < std::string >
                            &instance_ids)
{
...
template < typename I >
void NamespaceReplayer <
I >::handle_instances_removed(const std::vector < std::string >
                              &instance_ids)
{
    dout(10) << "instance_ids=" << instance_ids << dendl;
...
void NamespaceReplayer < I >::init_local_status_updater()
{
    ...
    m_local_status_updater.reset(MirrorStatusUpdater <
                                 I >::create(m_local_io_ctx, m_threads,
                                         ""));
    auto ctx =
        create_context_callback < NamespaceReplayer < I >,
        &NamespaceReplayer < I >::handle_init_local_status_updater >
        (this);
...
template < typename I >
void NamespaceReplayer <
I >::handle_init_local_status_updater(int r)
{
    dout(10) << "r=" << r << dendl;
...
void NamespaceReplayer < I >::init_remote_status_updater()
{
    ...
    m_remote_status_updater.reset(MirrorStatusUpdater <
                                  I >::create(m_remote_io_ctx,
                                          m_threads,
                                          m_local_mirror_uuid));
    auto ctx =
        create_context_callback < NamespaceReplayer < I >,
        &NamespaceReplayer < I >::handle_init_remote_status_updater >
        (this);
...
template < typename I >
void NamespaceReplayer <
I >::handle_init_remote_status_updater(int r)
{
    dout(10) << "r=" << r << dendl;
...
void NamespaceReplayer < I >::init_instance_replayer()
{
    ...
    m_instance_replayer.reset(InstanceReplayer <
                              I >::create(m_local_io_ctx,
                                          m_local_mirror_uuid,
                                          m_threads, m_service_daemon,
                                          m_local_status_updater.get(),
                                          m_cache_manager_handler,
                                          m_pool_meta_cache));
    auto ctx =
        create_context_callback < NamespaceReplayer < I >,
        &NamespaceReplayer < I >::handle_init_instance_replayer >
        (this);
...
void NamespaceReplayer < I >::handle_init_instance_replayer(int r)
{
    ...
    if (r < 0) {
        derr << "error initializing instance replayer: " <<
             cpp_strerror(r)
             << dendl;
    ...
    m_instance_replayer->add_peer({
        m_local_mirror_peer_uuid,
        m_remote_io_ctx, m_remote_pool_meta,
...
void NamespaceReplayer < I >::init_instance_watcher()
{
    ...
    m_instance_watcher.reset(InstanceWatcher <
                             I >::create(m_local_io_ctx,
...
void NamespaceReplayer < I >::handle_init_instance_watcher(int r)
{
    ...
    if (r < 0) {
        derr << "error initializing instance watcher: " <<
             cpp_strerror(r)
             << dendl;
...
void NamespaceReplayer < I >::handle_stop_instance_replayer(int r)
{
    ...
    if (r < 0) {
        derr << "error stopping instance replayer: " << cpp_strerror(r)
             << dendl;
    }
...
template < typename I >
void NamespaceReplayer <
I >::handle_shut_down_instance_watcher(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "error shutting instance watcher down: " <<
             cpp_strerror(r)
             << dendl;
    }
...
template < typename I >
void NamespaceReplayer <
I >::handle_shut_down_instance_replayer(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "error shutting instance replayer down: " <<
             cpp_strerror(r)
             << dendl;
    }
...
template < typename I >
void NamespaceReplayer <
I >::handle_shut_down_remote_status_updater(int r)
{
    dout(10) << "r=" << r << dendl;
...
template < typename I >
void NamespaceReplayer <
I >::handle_shut_down_local_status_updater(int r)
{
    dout(10) << "r=" << r << dendl;
...
void NamespaceReplayer < I >::init_image_map(Context *on_finish)
{
    ...
    auto ctx = new LambdaContext([this, image_map, on_finish](int r) {
        handle_init_image_map(r, image_map,
                              on_finish);
    });
    image_map->
    init(create_async_context_callback(m_threads->work_queue, ctx));
}

template < typename I >
void NamespaceReplayer < I >::handle_init_image_map(int r,
        ImageMap < I >
        *image_map,
        Context *
        on_finish)
{
    dout(10) << "r=" << r << dendl;
    if (r < 0) {
        derr << "failed to init image map: " << cpp_strerror(r) <<
             dendl;
...
template < typename I >
void NamespaceReplayer <
I >::init_local_pool_watcher(Context *on_finish)
{
    dout(10) << dendl;
    ...
    std::lock_guard locker {
        m_lock};
    ceph_assert(!m_local_pool_watcher);
    m_local_pool_watcher.reset(PoolWatcher <
                               I >::create(m_threads, m_local_io_ctx,
                                           m_local_mirror_uuid,
    ...
    auto ctx = new LambdaContext([this, on_finish](int r) {
        handle_init_local_pool_watcher(r,
                                       on_finish);
    });
    m_local_pool_watcher->
    init(create_async_context_callback(m_threads->work_queue, ctx));
...
template < typename I >
void NamespaceReplayer < I >::handle_init_local_pool_watcher(int r,
        Context
        *
        on_finish)
{
    dout(10) << "r=" << r << dendl;
    if (r < 0) {
        derr << "failed to retrieve local images: " << cpp_strerror(r)
             << dendl;
...
template < typename I >
void NamespaceReplayer <
I >::init_remote_pool_watcher(Context *on_finish)
{
    dout(10) << dendl;
    ...
    std::lock_guard locker {
        m_lock};
    ceph_assert(!m_remote_pool_watcher);
    m_remote_pool_watcher.reset(PoolWatcher <
                                I >::create(m_threads, m_remote_io_ctx,
                                            m_remote_pool_meta.
                                            mirror_uuid,
                                            m_remote_pool_watcher_listener));

    auto ctx = new LambdaContext([this, on_finish](int r) {
        handle_init_remote_pool_watcher(r,
                                        on_finish);
    });
    m_remote_pool_watcher->
    init(create_async_context_callback(m_threads->work_queue, ctx));
...
template < typename I >
void NamespaceReplayer < I >::handle_init_remote_pool_watcher(int r,
        Context
        *
        on_finish)
{
    ...
    if (r == -ENOENT) {
        ...
        dout(0) << "remote peer does not have mirroring configured" <<
                dendl;
    } else if (r < 0) {
        derr << "failed to retrieve remote images: " << cpp_strerror(r)
             << dendl;
        on_finish = new LambdaContext([on_finish, r](int) {
            on_finish->complete(r);
        });
...
template < typename I >
void NamespaceReplayer <
I >::init_image_deleter(Context *on_finish)
{
    dout(10) << dendl;
    ...
    on_finish = new LambdaContext([this, on_finish](int r) {
        handle_init_image_deleter(r,
                                  on_finish);
    });
    m_image_deleter.reset(ImageDeleter <
                          I >::create(m_local_io_ctx, m_threads,
...
template < typename I >
void NamespaceReplayer < I >::handle_init_image_deleter(int r,
        Context *
        on_finish)
{
    dout(10) << "r=" << r << dendl;
    if (r < 0) {
        derr << "failed to init image deleter: " << cpp_strerror(r) <<
             dendl;
        on_finish = new LambdaContext([on_finish, r](int) {
            on_finish->complete(r);
        });
...
template < typename I >
void NamespaceReplayer <
I >::shut_down_image_deleter(Context *on_finish)
{
    dout(10) << dendl;
    {
        std::lock_guard locker {
...
template < typename I >
void NamespaceReplayer < I >::handle_shut_down_image_deleter(int r,
        Context
        *
        on_finish)
{
...
template < typename I >
void NamespaceReplayer <
I >::shut_down_pool_watchers(Context *on_finish)
{
    dout(10) << dendl;
...
template < typename I >
void NamespaceReplayer < I >::handle_shut_down_pool_watchers(int r,
        Context
        *
        on_finish)
{
...
template < typename I >
void NamespaceReplayer <
I >::shut_down_image_map(Context *on_finish)
{
    dout(5) << dendl;
...
template < typename I >
void NamespaceReplayer < I >::handle_shut_down_image_map(int r,
        Context *
        on_finish)
{
    dout(5) << "r=" << r << dendl;
    if (r < 0 && r != -EBLOCKLISTED) {
        derr << "failed to shut down image map: " << cpp_strerror(r) <<
             dendl;
    }
...
template < typename I >
void NamespaceReplayer <
I >::handle_acquire_image(const std::string &global_image_id,
                          ...
                          Context *on_finish)
{
    ...
    m_instance_watcher->notify_image_acquire(instance_id,
            global_image_id,
            on_finish);
}
...
template < typename I >
void NamespaceReplayer <
I >::handle_release_image(const std::string &global_image_id,
                          const std::string &instance_id,
                          Context *on_finish)
{
    dout(5) << "global_image_id=" << global_image_id << ", " <<
    ...
    m_instance_watcher->notify_image_release(instance_id,
            global_image_id,
            on_finish);
}
...
template < typename I >
void NamespaceReplayer <
I >::handle_remove_image(const std::string &mirror_uuid,
