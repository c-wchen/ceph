...
#define ERR_CHECK_NEXT_PARAM(o) \
  if((loop + 1) >= argc)return -1; \
    ...
    if (host.length() <= 0 || creds.length() <= 0)
        return -1;
    return 0;
}
                       ...
                       string & out)
{
    ...
    if (off != string::npos) {
        ...
        calc_hmac_sha1(secret.c_str(), secret.length(), auth_hdr.c_str(),
                       auth_hdr.length(), hmac_sha1);
...
void send_cors(set < string > o, set < string > h,
               list < string > e, uint8_t flags, unsigned max_age)
{
    if (g_test->get_key_type() == KEY_TYPE_S3) {
        ...
        stringstream ss;
        ss << "1";
        g_test->send_request(string("POST"),
                             string("/swift/v1/" SWIFT_BUCKET_NAME),
                             cors_read_xml, (void *)&ss, 1);
    }
}
...
TEST(TestCORS, putcors_firsttime)
{
    ...
    EXPECT_EQ(((g_test->get_key_type() == KEY_TYPE_SWIFT) ? 202U : 200U),
...
TEST(TestCORS, optionscors_test_options_2)
{
    ...
    origins.insert(origins.end(), "*.example.com");
    uint8_t flags =
        RGW_CORS_GET | RGW_CORS_PUT | RGW_CORS_DELETE | RGW_CORS_HEAD;
