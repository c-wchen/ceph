...
namespace librbd {
    namespace mirror {
        namespace snapshot {
             ...
             template < typename I > void PromoteRequest < I >::send() {
                ...
                if (!util::can_create_primary_snapshot(m_image_ctx, false, true,
                    ...
                    return;
                        ...
                        15) << "requires_orphan=" << requires_orphan << ", " <<
                ...
                auto ctx = create_context_callback <
                    PromoteRequest < I >,
                    &PromoteRequest < I >::handle_create_orphan_snapshot >
                    (this);

                auto req =
                    CreateNonPrimaryRequest < I >::create(m_image_ctx, false,
                                                          "", CEPH_NOSNAP, { },
                                                          { }, nullptr, ctx);
                req->send();
            }
            ...
            template < typename I >
                void PromoteRequest <
                I >::handle_create_orphan_snapshot(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;

                if (r < 0) {
                    lderr(cct) << "failed to create orphan snapshot: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I > void PromoteRequest < I >::list_watchers() {
                ...
                auto req =
                    ...
                    I >::create(*m_image_ctx, flags, &m_watchers, ctx);
                req->send();
            }
            ...
            template < typename I >
                ...
                *m_timer_lock};
                ...
                if (r < 0) {
                    lderr(cct) << "failed to register update watcher: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I >
                void PromoteRequest <
                I >::scheduler_unregister_update_watcher() {
                ceph_assert(ceph_mutex_is_locked(*m_timer_lock));

                CephContext *cct = m_image_ctx->cct;
                ldout(cct,
                      15) << "scheduler_ticks=" << m_scheduler_ticks << dendl;
            ...
            template < typename I >
                ...
                auto ctx = create_context_callback <
                    PromoteRequest < I >,
                    &PromoteRequest < I >::handle_unregister_update_watcher >
                    (this);

                m_image_ctx->state->
                    unregister_update_watcher(m_update_watcher_handle, ctx);
            }

            template < typename I >
                void PromoteRequest <
                I >::handle_unregister_update_watcher(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;

                if (r < 0) {
                    lderr(cct) << "failed to unregister update watcher: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I >
                void PromoteRequest <
                I >::handle_acquire_exclusive_lock(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;
            ...
            template < typename I > void PromoteRequest < I >::rollback() {
                ...
                m_image_ctx->operations->execute_snap_rollback(snap_namespace,
                                                               snap_name,
                                                               m_progress_ctx,
                                                               ctx);
            }
            ...
            template < typename I >
                ...
                if (r < 0) {
                    lderr(cct) << "failed to rollback: " << cpp_strerror(r) <<
                        dendl;
                    finish(r);
                    return;
                }
            ...
            template < typename I >
                ...
                auto ctx = create_context_callback <
                    PromoteRequest < I >,
                    &PromoteRequest < I >::handle_create_promote_snapshot >
                    (this);

                auto req =
                    CreatePrimaryRequest < I >::create(m_image_ctx,
                                                       m_global_image_id,
                                                       CEPH_NOSNAP,
            ...
            template < typename I >
                void PromoteRequest <
                I >::handle_create_promote_snapshot(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;

                if (r < 0) {
                    lderr(cct) << "failed to create promote snapshot: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I >
                ...
                auto aio_comp = create_rados_callback <
                    PromoteRequest < I >,
                    &PromoteRequest < I >::handle_disable_non_primary_feature >
                    (this);
                int r =
            ...
            template < typename I >
                void PromoteRequest <
                I >::handle_disable_non_primary_feature(int r) {
                CephContext *cct = m_image_ctx->cct;
                ...
                if (m_lock_acquired) {
                    ...
                    if (m_image_ctx->exclusive_lock != nullptr) {
                        ...
                        auto ctx = create_context_callback <
                            PromoteRequest < I >,
                            &PromoteRequest <
                            I >::handle_release_exclusive_lock > (this);
            ...
            template < typename I >
                void PromoteRequest <
                I >::handle_release_exclusive_lock(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;
