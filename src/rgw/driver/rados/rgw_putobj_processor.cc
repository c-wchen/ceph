                                       ...
                                       uint32_t flags)
{
    ...
    ldpp_dout(rctx.dpp, 20) << "Update meta: " << meta_obj_ref.obj.oid << " part " << p << " prefix " <<
...
int AppendObjectProcessor::prepare(optional_yield y)
{
    ...
    *cur_accounted_size = astate->accounted_size;
    ...
    r = manifest_gen.create_begin(store->ctx(), &manifest, bucket_info.placement_rule, &tail_placement_rule,
                                  head_obj.bucket, head_obj);
    if (r < 0) {
        return r;
    }
