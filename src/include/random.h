...
using larger_of = typename std::conditional <
                  sizeof(T0) >= sizeof(T1), T0, T1 >::type;
...
using has_compatible_numeric_types =
    ...
    std::conjunction < std::is_integral < NumberT0 >,
    std::is_integral < NumberT1 > > >;

// Select the larger of type compatible numeric types:
template < typename NumberT0, typename NumberT1 >
using select_number_t =
    std::enable_if_t < detail::has_compatible_numeric_types <
    NumberT0, NumberT1 >::value, detail::larger_of < NumberT0,
    NumberT1 >>;
...
using default_distribution = typename
                             select_distribution < NumberT,
                             std::is_integral < NumberT >::value >::type;
...
template < typename NumberT,
           ...
           typename EngineT >
NumberT generate_random_number(const NumberT min,
                               const NumberT max, EngineT &e)
{
    DistributionT d {
        min, max};

    using param_type = typename DistributionT::param_type;
    return d(e, param_type {
        min, max}
            );
}
...
NumberT generate_random_number(const NumberT min,
                               const NumberT max)
{
    return detail::generate_random_number < NumberT, DistributionT,
           EngineT > (min, max, detail::engine < EngineT > ());
}
...
template < typename MutexT,
           ...
           detail::default_distribution <
               NumberT >> NumberT generate_random_number(MutexT &m,
                       EngineT &e)
{
    return detail::generate_random_number < NumberT, MutexT,
           DistributionT, EngineT > (0,
                                     std::numeric_limits <
                                     NumberT >::max(), m, e);
}

template < typename NumberT, typename MutexT, typename EngineT >
NumberT generate_random_number(const NumberT max, MutexT &m,
                               EngineT &e)
{
...
{
    return detail::generate_random_number < NumberT, DistributionT,
           EngineT > (0, std::numeric_limits < NumberT >::max());
}

template < typename NumberT0, typename NumberT1,
           typename NumberT = detail::select_number_t < NumberT0, NumberT1 >
           >NumberT generate_random_number(const NumberT0 min,
                   const NumberT1 max)
{
    return detail::generate_random_number < NumberT,
           detail::default_distribution < NumberT >,
           std::default_random_engine > (static_cast < NumberT >(min),
                                         static_cast < NumberT >(max));
}
...
{
    return detail::generate_random_number < NumberT, DistributionT,
           EngineT > (static_cast < NumberT >(min),
                      static_cast < NumberT >(max), e);
}
...
public:
    random_device_type &random_device()noexcept
    {
        return rd;
    }
    random_engine_type &random_engine()noexcept
    {
        return e;
    }
...
public:
    ...
    {
        return detail::generate_random_number < NumberT > (min, max, l,
                e);
    }
...
template < typename NumberT > random_number_generator(const NumberT max)->
random_number_generator < NumberT >;
