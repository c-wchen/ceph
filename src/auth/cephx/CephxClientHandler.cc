...
int CephxClientHandler::build_request(bufferlist & bl) const const
{
    ...
    if (need & CEPH_ENTITY_TYPE_AUTH) {
        ...
        get_random_bytes((char *)&req.client_challenge,
                         sizeof(req.client_challenge));
        std::string error;
...
bool CephxClientHandler::_need_tickets() const const
{
    ...
    case CEPHX_GET_AUTH_SESSION_KEY:
        {
            ...
            if (!got) {
                ldout(cct,
                      0) << "key not found for " << cct->_conf->name << dendl;
                return -ENOENT;
            }
...
AuthAuthorizer *CephxClientHandler::build_authorizer(uint32_t service_id) const const
{
    RWLock::RLocker l(lock);
    ldout(cct,
