 ...
 */
    OMapInnerNode::make_split_insert_ret
        OMapInnerNode::make_split_insert(omap_context_t oc,
        ...
        return make_split_children(oc).si_then([ =] (auto tuple) {
                                               ...
                                               if (pivot > key) {
                                               auto liter =
                                               left->iter_idx(iter.get_index());
                                               left->journal_inner_insert(liter,
                                                                          laddr,
                                                                          key,
                                                                          left->
                                               ...
                                               journal_inner_insert(riter,
                                                                    ...
                                                                    maybe_get_delta_buffer
                                                                    ());}
                                               ++(oc.t.get_omap_tree_stats().
                                                  extents_num_delta);
                                               return
                                               make_split_insert_ret
                                               (interruptible::
                                                ready_future_marker {
                                                }
                                                ,
                                                mutation_result_t
                                                (mutation_status_t::WAS_SPLIT,
                                                 tuple, std::nullopt));}
        ) ;
    ...
    OMapInnerNode::handle_split_ret
        OMapInnerNode::handle_split(omap_context_t oc,
                                    internal_iterator_t iter,
        ...
        if (!is_mutable()) {
            auto mut =
                oc.tm.get_mutable_extent(oc.t, this)->cast < OMapInnerNode > ();
            ...
            return make_split_insert(oc, iter + 1, pivot, right->get_laddr())
                .si_then([this, oc] (auto m_result) {
                         ...
                         .si_then([m_result = std::move(m_result)] {
                                  return insert_ret(interruptible::
                                                    ready_future_marker {
                                                    }
                                                    , m_result);}
                         );}
    ...
    OMapInnerNode::get_value_ret
        ...
        return omap_load_extent(oc, laddr, get_meta().depth - 1).si_then([oc,
                                                                          &key]
                                                                         (auto
                                                                          extent)
                                                                         {
    ...
    OMapInnerNode::insert_ret
        OMapInnerNode::insert(omap_context_t oc,
        ...
        return omap_load_extent(oc, laddr, get_meta().depth - 1).si_then([oc,
                  ...
                  return insert_iertr::make_ready_future < mutation_result_t >
                  (mresult);}
                  else
                  ...
                  return insert_ret(interruptible::ready_future_marker {
                                    ...
                                    ,
                                    mutation_result_t(mutation_status_t::
                                                      SUCCESS, std::nullopt,
                                                      std::nullopt));}
                  }
        ) ;
    ...
    OMapInnerNode::rm_key_ret
        ...
        assert(child_pt != iter_cend());
        auto laddr = child_pt->get_val();
        return omap_load_extent(oc, laddr, get_meta().depth - 1).si_then([this,
                                                                          oc,
                                                                          &key,
                                                                          child_pt]
                                                                         (auto
...
return rm_key_iertr::make_ready_future < mutation_result_t > (mresult); case mutation_status_t::NEED_MERGE:{
                                                                          if
                                                                          (get_node_size
                                                                           () >
                                                                           1)
...
return handle_split(oc, child_pt, mresult); default:
                                                                          return
                                                                          rm_key_iertr::
                                                                          make_ready_future
                                                                          <
    ...
    OMapInnerNode::list_ret
        ...
        return seastar::do_with(first_iter,
                                last_iter,
                                iter_t(first_iter), list_bare_ret(false, {
                                                                  }
                                ),
                                [this, &first, &last, oc, config] (auto & fiter,
                                                                   auto & liter,
                                                                   auto & iter,
                                                                   auto & ret) {
                                auto & complete = std::get < 0 > (ret);
                                auto & result = std::get < 1 > (ret);
                                return trans_intr::repeat([&, config, oc,
                                                           this] ()->
                                                          list_iertr::future <
                                                          seastar::
                                                          stop_iteration > {
    ...
    OMapInnerNode::clear_ret OMapInnerNode::clear(omap_context_t oc) {
        ...
        return trans_intr::do_for_each(iter_begin(), iter_end(),
                                       ...
                                       if (ndepth > 1) {
                                       return omap_load_extent(oc, laddr,
                                                               ndepth).
    ...
    OMapInnerNode::split_children_ret
        ...
        return oc.tm.alloc_extents < OMapInnerNode > (oc.t, oc.hint,
            ...
            .si_then([this, oc] (auto && ext_pair) {
                     ...
                     DEBUGT("this: {}, split into: l {} r {}", oc.t, *this,
                            *left, *right);
                     return split_children_ret(interruptible::
                                               ready_future_marker {
                                               }
                                               ,
        ...
        return oc.tm.alloc_extent < OMapInnerNode > (oc.t, oc.hint,
            ...
            .si_then([this, right] (auto && replacement) {
                     replacement->merge_from(*this,
                                             *right->cast < OMapInnerNode > ());
                     return full_merge_ret(interruptible::ready_future_marker {
                                           }
    ...
    OMapInnerNode::make_balanced_ret
        ...
        return oc.tm.alloc_extents < OMapInnerNode > (oc.t, oc.hint,
            ...
            .si_then([this, _right] (auto && replacement_pair) {
                     ...
                     return make_balanced_ret(interruptible::
                                              ready_future_marker {
                                              }
                                              ,
    ...
    OMapInnerNode::merge_entry_ret
        OMapInnerNode::merge_entry(omap_context_t oc,
                                   internal_iterator_t iter,
                                   OMapNodeRef entry) {
        ...
        return omap_load_extent(oc, donor_iter->get_val(),
                                get_meta().depth - 1).si_then([ =, this] (auto
                                                                          &&
                                                                          donor)
                                                              mutable {
                                                              LOG_PREFIX
    ...
    std::ostream & OMapLeafNode::print_detail_l(std::ostream & out) const {
        ...
        if (ite != iter_end()) {
            ...
            return get_value_ret(interruptible::ready_future_marker {
                                 }
                                 , value);
        }
        else {
    ...
    OMapLeafNode::insert_ret
        OMapLeafNode::insert(omap_context_t oc,
                             const std::string & key,
                             const ceph::bufferlist & value) {
        ...
        if (!overflow) {
            if (!is_mutable()) {
                auto mut =
                    oc.tm.get_mutable_extent(oc.t,
                                             this)->cast < OMapLeafNode > ();
            ...
            if (replace_pt != iter_end()) {
                ...
                journal_leaf_update(replace_pt, key, value,
                                    maybe_get_delta_buffer());
            }
            else {
                ++(oc.t.get_omap_tree_stats().num_inserts);
                auto insert_pt = string_lower_bound(key);
                journal_leaf_insert(insert_pt, key, value,
                                    maybe_get_delta_buffer());

                DEBUGT("inserted {}, this: {}", oc.t, insert_pt.get_key(),
                       *this);
            }
            return insert_ret(interruptible::ready_future_marker {
                              }
            ...
            return make_split_children(oc).
                si_then([this, oc, &key, &value] (auto tuple) {
                        auto[left, right, pivot] = tuple;
                        auto replace_pt = find_string_key(key);
                        if (replace_pt != iter_end()) {
                        ++(oc.t.get_omap_tree_stats().num_updates); if (key < pivot) {  //left
                        auto mut_iter = left->iter_idx(replace_pt->get_index());
                        left->journal_leaf_update(mut_iter, key, value,
                                                  left->
                                                  maybe_get_delta_buffer());}
                        else
                        if (key >= pivot) { //right
                        auto mut_iter =
                        right->iter_idx(replace_pt->get_index() -
                                        left->get_node_size());
                        right->journal_leaf_update(mut_iter, key, value,
                                                   right->
                                                   maybe_get_delta_buffer());}
                        }
                        else {
                        ++(oc.t.get_omap_tree_stats().num_inserts); auto insert_pt = string_lower_bound(key); if (key < pivot) {    //left
                        auto mut_iter = left->iter_idx(insert_pt->get_index());
                        left->journal_leaf_insert(mut_iter, key, value,
                                                  left->
                                                  maybe_get_delta_buffer());}
                        else {
                        auto mut_iter =
                        right->iter_idx(insert_pt->get_index() -
                                        left->get_node_size());
                        right->journal_leaf_insert(mut_iter, key, value,
                                                   right->
                                                   maybe_get_delta_buffer());}
                        }
    ...
    OMapLeafNode::rm_key_ret
        ...
        if (!is_mutable() && rm_pt != iter_end()) {
            auto mut =
                oc.tm.get_mutable_extent(oc.t, this)->cast < OMapLeafNode > ();
    ...
    last_inclusive {}, this: {}", oc.t, first ? first->c_str() : "", last ? last->c_str() : "", config.max_result_size, config.first_inclusive, config.last_inclusive, *this);
        auto ret = list_bare_ret(false, { });
        auto &[complete, result] = ret;
        auto iter = first ?
            (config.first_inclusive ?
             string_lower_bound(*first) :
             string_upper_bound(*first)) : iter_begin();
        auto liter = last ?
            (config.last_inclusive ?
             string_upper_bound(*last) :
             string_lower_bound(*last)) : iter_end();
    ...
    OMapLeafNode::split_children_ret
        ...
        DEBUGT("this: {}", oc.t, *this);
        return oc.tm.alloc_extents < OMapLeafNode > (oc.t, oc.hint,
                                                     OMAP_LEAF_BLOCK_SIZE, 2)
    ...
    OMapLeafNode::full_merge_ret
        ...
        DEBUGT("this: {}", oc.t, *this);
        return oc.tm.alloc_extent < OMapLeafNode > (oc.t, oc.hint,
                                                    OMAP_LEAF_BLOCK_SIZE)
            .si_then([this, right] (auto && replacement) {
                     replacement->merge_from(*this,
                                             *right->cast < OMapLeafNode > ());
                     return full_merge_ret(interruptible::ready_future_marker {
                                           }
                                           , std::move(replacement));}
        );
        ...
        if (depth > 1) {
            return oc.tm.read_extent < OMapInnerNode > (oc.t, laddr,
                ...
                .handle_error_interruptible(omap_load_extent_iertr::pass_further {
                                            ...
                                            , crimson::ct_error::assert_all {
                                            "Invalid error in omap_load_extent"}
            ).si_then([](auto && e) {
            ...
            return oc.tm.read_extent < OMapLeafNode > (oc.t, laddr,
                                                       OMAP_LEAF_BLOCK_SIZE).
                handle_error_interruptible(omap_load_extent_iertr::
                                           pass_further {
                                           }
                                           , crimson::ct_error::assert_all {
                                           "Invalid error in omap_load_extent"}
