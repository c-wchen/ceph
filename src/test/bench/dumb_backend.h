...
class DumbBackend:public Backend {
    ...
    struct write_item {
        ...
        Context *on_applied;
        Context *on_commit;
         write_item(const string & oid,
                    const bufferlist & bl,
                    uint64_t offset,
                    Context * on_applied,
                    Context * on_commit):oid(oid), bl(bl), offset(offset),
            on_applied(on_applied), on_commit(on_commit) {
    }};
    ...
    class WriteQueue:public ThreadPool::WorkQueue < write_item > {
      ...
      public:
         WriteQueue(DumbBackend * _backend,
                    time_t ti,
                    ThreadPool * tp):
            ThreadPool::WorkQueue < write_item > ("DumbBackend::queue", ti,
                                                  ti * 10, tp),
            backend(_backend) {
        ...
        void _dequeue(write_item *) override {
            ceph_abort();
        }
        ...
        void _process(write_item * item, ThreadPool::TPHandle &) override {
            return backend->_write(item->oid,
                                   item->offset,
                                   item->bl, item->on_applied, item->on_commit);
        }
    ...
    void _write(const string & oid,
                uint64_t offset,
                const bufferlist & bl,
                Context * on_applied, Context * on_commit);

  public:
    DumbBackend(const string & path,
                bool do_fsync,
                bool do_sync_file_range,
                bool do_fadvise,
                unsigned sync_interval,
                int sync_fd, unsigned worker_threads, CephContext * cct)
    ...
    void write(const string & oid,
        ...
        sem.Get();
        queue.queue(new write_item(oid, bl, offset, on_applied, on_commit));
    }
