        ...
        namespace bench {
            ...
            namespace {
                ...
                io_type_t get_io_type(std::string io_type_string) {
                    ...
                    else if (io_type_string == "readwrite"
                             || io_type_string == "rw")
                        return IO_TYPE_RW;
                    else
                        return IO_TYPE_NUM;
            ...
            struct rbd_bencher {
                ...
                explicit rbd_bencher(librbd::Image * i, io_type_t io_type,
                                     uint64_t io_size)
                :image(i), in_flight(0), io_type(io_type), io_size(io_size) {
                ...
                }} void start_io(int max, uint64_t off, uint64_t len,
                                 int op_flags, bool read_flag) {
                    {
                        std::lock_guard l {
                        lock};
                        in_flight++;
                    ...
                    if (read_flag) {
                        ...
                        c = new librbd::RBD::
                            AioCompletion((void *)(new
                                                   bencher_completer(this,
                                                                     read_bl)),
                ...
                int wait_for(int max, bool interrupt_on_terminating) {
                    ...
                    lock};
                    while (in_flight > max
                           && !(terminating && interrupt_on_terminating)) {
                        cond.wait_for(l, 200 ms);
                    }
            ...
            void rbd_bencher_completion(void *vc, void *pc) {
                ...
                if (b->io_type == IO_TYPE_WRITE && ret != 0) {
                    std::cout << "write error: " << cpp_strerror(ret) << std::
                        endl;
                    exit(ret < 0 ? -ret : ret);
                }
                else if (b->io_type == IO_TYPE_READ
                         && (unsigned int)ret != b->io_size) {
                    std::cout << "read error: " << cpp_strerror(ret) << std::
                        endl;
                    exit(ret < 0 ? -ret : ret);
                }
                b->lock.lock();
            ...
            int do_bench(librbd::Image & image, io_type_t io_type,
                ...
                if (io_size > size) {
                    std::cerr << "rbd: io-size " << byte_u_t(io_size) << " "
                        << "larger than image size " << byte_u_t(size) << std::
                        endl;
                    return -EINVAL;
                }

                if (io_size > std::numeric_limits < uint32_t >::max()) {
                    std::cerr << "rbd: io-size should be less than 4G" << std::
                        endl;
                    return -EINVAL;
                }
                ...
                if (r < 0 && (r != -EROFS || io_type != IO_TYPE_READ)) {
                    std::
                        cerr << "rbd: failed to flush: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                ...
                std::cout << "bench "
                    << " type " << (io_type == IO_TYPE_READ ? "read" :
                                    io_type ==
                                    IO_TYPE_WRITE ? "write" : "readwrite")
                    << (io_type ==
                        IO_TYPE_RW ? " read:write=" +
                ...
                uint64_t i;
                uint64_t seq_chunk_length =
                    (size / io_size / io_threads) * io_size;;
                ...
                RollingSum time_acc(boost::accumulators::tag::rolling_window::
                                    window_size = WINDOW_SIZE);
                RollingSum ios_acc(boost::accumulators::tag::rolling_window::
                                   window_size = WINDOW_SIZE);
                RollingSum off_acc(boost::accumulators::tag::rolling_window::
                                   window_size = WINDOW_SIZE);
                uint64_t cur_ios = 0;
                uint64_t cur_off = 0;
                ...
                for (off = 0; off < io_bytes;) {
                    ...
                    while (i < io_threads && off < io_bytes) {
                        ...
                        b.start_io(io_threads, thread_offset[i], io_size,
                                   op_flags, read_flag);
                    ...
                    if (last == std::chrono::duration < double >::zero()) {
                        ...
                        std::cout.width(10);
                        std::cout << boost::accumulators::rolling_sum(ios_acc) /
                            time_sum;
                        std::cout.width(10);
                        std::cout << byte_u_t(boost::accumulators::
                                              rolling_sum(off_acc) /
                                              time_sum) << "/s" << std::endl;
                        last = elapsed;
                    }
                ...
                if (io_type != IO_TYPE_READ) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to flush at the end: " <<
                            cpp_strerror(r)
                            << std::endl;
                    }
                }
                ...
                std::cout << "elapsed: " << (int)elapsed.count() << "   "
                    ...
                    << "bytes/sec: " << byte_u_t((double)off /
                                                 elapsed.
                                                 count()) << "/s" << std::endl;

                if (io_type == IO_TYPE_RW) {
                    std::cout << "read_ops: " << read_ops << "   "
                        << "read_ops/sec: " << (double)read_ops /
                        elapsed.
                        count() << "   " << "read_bytes/sec: " <<
                        byte_u_t((double)read_ops * io_size /
                                 elapsed.count()) << "/s" << std::endl;

                    std::cout << "write_ops: " << write_ops << "   "
                        << "write_ops/sec: " << (double)write_ops /
                        elapsed.
                        count() << "   " << "write_bytes/sec: " <<
                        byte_u_t((double)write_ops * io_size /
                                 elapsed.count()) << "/s" << std::endl;
            ...
            void add_bench_common_options(po::options_description * positional,
                ...
                options->add_options()
                    ("io-size", po::value < Size > (),
                     "IO size (in B/K/M/G) (< 4G) [default: 4K]")
                    ("io-threads", po::value < uint32_t > (),
                     "ios in flight [default: 16]")
            ...
            int bench_execute(const po::variables_map & vm,
                ...
                utils::SnapshotPresence snap_presence =
                ...
                if (bench_io_size == 0) {
                    std::
                        cerr << "rbd: --io-size should be greater than zero." <<
                        std::endl;
                    return -EINVAL;
                }
                ...
                if (bench_io_threads == 0) {
                    std::
                        cerr << "rbd: --io-threads should be greater than zero."
                        << std::endl;
                    return -EINVAL;
                }
                ...
                if (bench_io_type == IO_TYPE_READ) {
                    ...
                    if (bench_read_proportion > 100) {
                        std::
                            cerr <<
                            "rbd: --rw-mix-read should not be larger than 100."
                            << std::endl;
                        return -EINVAL;
                    }
                }
                ...
                r = do_bench(image, bench_io_type, bench_io_size,
                             bench_io_threads, bench_bytes, bench_pattern,
                             bench_read_proportion);
                ...
                if (r < 0) {
                    std::cerr << "bench failed: " << cpp_strerror(r) << std::
                        endl;
                    return r;
                }
                return 0;
            ...
            int execute_for_bench(const po::variables_map & vm,
                ...
                if (vm.count("io-type")) {
                    ...
                    std::cerr << "rbd: --io-type must be specified." << std::
                        endl;
                    return -EINVAL;
                }
