        ...
        namespace group {
            ...
            void add_group_option(po::options_description * opt,
                ...
                std::string description =
                    at::get_description_prefix(modifier) + "group name";
                switch (modifier) {
                case at::ARGUMENT_MODIFIER_NONE:
                case at::ARGUMENT_MODIFIER_SOURCE:
                    break;
                    case at::ARGUMENT_MODIFIER_DEST:name = DEST_GROUP_NAME;
                    break;
                }
            ...
            int execute_create(const po::variables_map & vm,
                ...
                int r =
                    utils::get_pool_generic_snapshot_names(vm,
                                                           at::
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: create error: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
            ...
            int execute_remove(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: remove error: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
            ...
            int execute_rename(const po::variables_map & vm,
                ...
                if (pool_name != dest_pool_name) {
                    std::
                        cerr << "rbd: group rename across pools not supported"
                        << std::
                        endl << "source pool: " << pool_name << ", dest pool: "
                    ...
                    std::
                        cerr <<
                        "rbd: group rename across namespaces not supported" <<
                        std::
                        endl << "source namespace: " << namespace_name <<
                        ", dest namespace: " << dest_namespace_name << std::
                        endl;
                    return -EINVAL;
                }
            ...
            int execute_add(const po::variables_map & vm,
                ...
                if (group_namespace_name != image_namespace_name) {
                    std::
                        cerr << "rbd: group and image namespace must match." <<
                        std::endl;
                    return -EINVAL;
                }
                ...
                r = utils::init(group_pool_name, group_namespace_name, &rados,
                                &cg_io_ctx);
                if (r < 0) {
                    return r;
                }
                ...
                r = utils::init(image_pool_name, group_namespace_name, &rados,
                                &image_io_ctx);
                if (r < 0) {
                    return r;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: add image error: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
            ...
            int execute_remove_image(const po::variables_map & vm,
                ...
                if (image_id.empty()) {
                    ...
                    r = rbd.group_image_remove_by_id(cg_io_ctx,
                                                     group_name.c_str(),
                                                     image_io_ctx,
                                                     image_id.c_str());
                }
                if (r < 0) {
                    std::
                        cerr << "rbd: remove image error: " << cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_list_images(const po::variables_map & vm,
              ...
              for (auto image:images) {
                    ...
                    if (r < 0) {
                        pool_name =
                            "<missing image pool " + stringify(image.pool) +
                            ">";
                    }
                    else {
                    ...
                    if (f) {
                        ...
                        std::cout << image_name << " " << state_string << std::
                            endl;
                    }
                }
            ...
            int execute_group_snap_create(const po::variables_map & vm,
                ...
                librbd::RBD rbd;
                r = rbd.group_snap_create2(io_ctx, group_name.c_str(),
                                           snap_name.c_str(), flags);
                if (r < 0) {
                    return r;
                }
            ...
            int execute_group_snap_remove(const po::variables_map & vm,
                ...
                librbd::RBD rbd;
                r = rbd.group_snap_remove(io_ctx, group_name.c_str(),
                                          snap_name.c_str());
                if (r < 0) {
            ...
            int execute_group_snap_rename(const po::variables_map & vm,
                ...
                if (vm.count(at::DEST_SNAPSHOT_NAME)) {
                    dest_snap_name =
                        vm[at::DEST_SNAPSHOT_NAME].as < std::string > ();
                }
                ...
                if (dest_snap_name.empty()) {
                    std::
                        cerr <<
                        "rbd: destination snapshot name was not specified" <<
                        std::endl;
                    return -EINVAL;
                }

                r = utils::validate_snapshot_name(at::ARGUMENT_MODIFIER_DEST,
                                                  dest_snap_name,
                                                  utils::
                                                  SNAPSHOT_PRESENCE_REQUIRED,
            ...
            int execute_group_snap_list(const po::variables_map & vm,
                ...
                if (f) {
                    ...
                    t.define_column("STATUS", TextTable::LEFT,
                                    TextTable::RIGHT);
                }
            ...
            int execute_group_snap_rollback(const po::variables_map & vm,
                ...
                r = rbd.group_snap_rollback_with_progress(io_ctx,
                                                          group_name.c_str(),
                                                          snap_name.c_str(),
                                                          pc);
                if (r < 0) {
            ...
            void get_create_arguments(po::options_description * positional,
                ...
                add_group_spec_options(positional, options,
                                       at::ARGUMENT_MODIFIER_NONE, false);
            }
            ...
            void get_rename_arguments(po::options_description * positional,
                                      po::options_description * options) {
                add_group_spec_options(positional, options,
                                       at::ARGUMENT_MODIFIER_SOURCE, false);
                add_group_spec_options(positional, options,
                                       at::ARGUMENT_MODIFIER_DEST, false);
            }
            ...
            void get_remove_image_arguments(po::options_description *
                                            positional,
                                            po::options_description * options) {
                positional->add_options()
                    (GROUP_SPEC.c_str(),
            ...
            void get_group_snap_create_arguments(po::options_description *
                                                 positional,
                                                 po::options_description *
                                                 options) {
                add_group_spec_options(positional, options,
                                       at::ARGUMENT_MODIFIER_NONE, true);
                at::add_snap_create_options(options);
            }

            void get_group_snap_remove_arguments(po::options_description *
                                                 positional,
                                                 po::options_description *
                                                 options) {
                add_group_spec_options(positional, options,
            ...
            void get_group_snap_rename_arguments(po::options_description *
                                                 positional,
                                                 po::options_description *
                                                 options) {
                add_group_spec_options(positional, options,
            ...
            void get_group_snap_list_arguments(po::options_description *
                                               positional,
                                               po::options_description *
                                               options) {
                at::add_format_options(options);
            ...
            void get_group_snap_rollback_arguments(po::options_description *
                                                   positional,
                                                   po::options_description *
                                                   options) {
            ...
            Shell::Action action_remove( {
                                        "group", "remove"}, {
                                        "group", "rm"}, "Delete a group.",
                                        "", &get_remove_arguments,
                                        &execute_remove);
            Shell::Action action_list( {
                                      "group", "list"}, {
                                      "group", "ls"}, "List rbd groups.",
