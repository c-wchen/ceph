...
namespace librbd {
    namespace io {
        ...
        using librbd::util::data_object_name;
         ...
         namespace {
          ...
          template < typename I > class C_UpdateObjectMap:public C_AsyncObjectThrottle < I >
            {
              public:
                C_UpdateObjectMap(AsyncObjectThrottle < I > &throttle,
                                  I * image_ctx, uint64_t object_no,
              ...
              :    C_AsyncObjectThrottle < I > (throttle, *image_ctx),
                ...
                int update_head() {
                    ...
                    bool sent =
                        image_ctx.object_map->template aio_update < Context >
                        (CEPH_NOSNAP, m_object_no, m_head_object_map_state, { },
                         m_trace, false,
                         this);
                    return (sent ? 0 : 1);
                }
        ...
        :m_image_ctx(ictx), m_object_no(objectno),
            ...
            m_trace(librbd::util::
                    create_trace(*m_image_ctx, "copy-up", parent_trace)) {
        ...
        template < typename I >
            void CopyupRequest <
            I >::append_request(AbstractObjectWriteRequest < I > *req,
        ...
        template < typename I > void CopyupRequest < I >::read_from_parent() {
            ...
            auto comp = AioCompletion::create_and_start <
                CopyupRequest < I >,
                &CopyupRequest < I >::handle_read_from_parent > (this,
                                                                 librbd::util::
                                                                 get_image_ctx
                                                                 (m_image_ctx->
                                                                  parent),
            ...
            auto req =
                io::ImageDispatchSpec::create_read(*m_image_ctx->parent,
                                                   io::
        ...
        template < typename I >
            ...
            if (r < 0 && r != -ENOENT) {
                ...
                lderr(cct) << "error reading from parent: " << cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
            }
        ...
        template < typename I > void CopyupRequest < I >::deep_copy() {
            ...
            m_deep_copied = true;
            m_flatten =
                is_copyup_required()? true : m_image_ctx->migration_info.
                flatten;
            m_lock.unlock();
            ...
            auto ctx = librbd::util::create_context_callback <
                CopyupRequest < I >,
                &CopyupRequest < I >::handle_deep_copy > (this);
            auto req =
                deep_copy::ObjectCopyRequest < I >::create(m_image_ctx->parent,
                                                           m_image_ctx, 0, 0,
                                                           m_image_ctx->
                                                           migration_info.
                                                           snap_map,
        ...
        template < typename I >
            ...
            if (r < 0 && r != -ENOENT) {
                ...
                lderr(cct) << "error encountered during deep-copy: " <<
                    cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
        ...
        template < typename I > void CopyupRequest < I >::update_object_maps() {
            ...
            if (r_it != m_pending_requests.rend()) {
                ...
                head_object_map_state =
                    (*r_it)->get_pre_write_object_map_state();
            }

            if ((*m_image_ctx->object_map)[m_object_no] !=
                head_object_map_state) {
            ...
            typename AsyncObjectThrottle <
                I >::ContextFactory context_factory(boost::lambda::
                                                    bind(boost::lambda::
                                                         ...
                                                         boost::lambda::_2));
            auto ctx =
                librbd::util::create_context_callback < CopyupRequest < I >,
                &CopyupRequest < I >::handle_update_object_maps > (this);
            auto throttle =
                new AsyncObjectThrottle < I > (nullptr, *m_image_ctx,
                                               context_factory, ctx, nullptr, 0,
                                               m_snap_ids.size());
            throttle->start_ops(m_image_ctx->config.template get_val <
        ...
        template < typename I > void CopyupRequest < I >::copyup() {
            ...
            auto object =
                neorados::Object { data_object_name(m_image_ctx, m_object_no) };
            if (copyup_op.size() > 0) {
                ...
                m_image_ctx->rados_api.execute(object, copyup_io_context,
                                               std::move(copyup_op),
                                               librbd::asio::util::
            ...
            if (write_op.size() > 0) {
                ...
                ldout(cct, 20) << (!deep_copyup && write_op.size() > 2 ?
                                   "copyup + ops" : !deep_copyup ? "copyup" :
                                   "ops")
                    << " with current snapshot context" << dendl;

                m_image_ctx->rados_api.execute(object, *io_context,
                                               std::move(write_op),
                                               librbd::asio::util::
                                               get_callback_adapter([this]
                                                                    (int r) {
        ...
        template < typename I > void CopyupRequest < I >::handle_copyup(int r) {
            ...
            if (pending_copyups == 0) {
                if (copyup_ret_val < 0 && copyup_ret_val != -ENOENT) {
                    lderr(cct) << "failed to copyup object: " <<
                        cpp_strerror(copyup_ret_val)
                        << dendl;
        ...
        template < typename I >
            void CopyupRequest <
            I >::complete_requests(bool override_restart_retval, int r) {
            auto cct = m_image_ctx->cct;
            remove_from_list();
        ...
        template < typename I >
          ...
          for (auto & it:m_image_ctx->migration_info.snap_map) {
            ...
            ldout(m_image_ctx->cct,
                  15) << "deep_copied=" << deep_copied << dendl;

            std::copy_if(m_image_ctx->snaps.rbegin(), m_image_ctx->snaps.rend(),
                         std::back_inserter(m_snap_ids),
                         [this, cct =
                          m_image_ctx->cct, &deep_copied] (uint64_t snap_id) {
                         if (deep_copied.count(snap_id)) {
                         m_first_snap_is_clean = true; return false;}

                         uint64_t raw_overlap = 0;
                         uint64_t object_overlap = 0;
                         int r =
                         m_image_ctx->get_parent_overlap(snap_id, &raw_overlap);
                         if (r < 0) {
                         ldout(cct,
                               5) <<
                         ...
                         util::object_to_area_extents(m_image_ctx, m_object_no,
                                                      { {
                                                      0,
                                                      m_image_ctx->layout.
                                                      object_size}
                         ...
                         object_overlap =
                         m_image_ctx->prune_parent_extents(parent_extents, area,
                                                           raw_overlap, false);}
        ...
        template < typename I >
          ...
          for (auto[image_offset, image_length]:image_extent_map) {
                ...
                util::area_to_object_extents(m_image_ctx, image_offset,
                                             image_length, m_image_area, 0,
                                             &object_extents);
              for (auto & object_extent:object_extents) {
                    m_copyup_extent_map.emplace_back(object_extent.offset,
        ...
        template < typename I > int CopyupRequest < I >::prepare_copyup_data() {
            ...
            if (copy_on_read || maybe_deep_copyup) {
              ...
              for (auto[object_offset, object_length]:m_copyup_extent_map)
                {
                    bufferlist sub_bl;
                    sub_bl.substr_of(m_copyup_data, buffer_offset,
                                     object_length);
                    buffer_offset += object_length;

                    sparse_bufferlist.insert(object_offset, object_length, {
                                             SPARSE_EXTENT_STATE_DATA,
                                             object_length, std::move(sub_bl)}
                    );
                }
            }
            else {
              ...
              for (auto[object_offset, object_length]:m_copyup_extent_map)
                {
                  ...
                  for (auto[copyup_offset, copyup_length]:copyup_object_extents)
                    {
                        bufferlist sub_bl;
                        sub_bl.substr_of(m_copyup_data,
                                         buffer_offset + (copyup_offset -
                                                          object_offset),
                                         copyup_length);
                        ceph_assert(sub_bl.length() == copyup_length);

                        sparse_bufferlist.insert(copyup_offset, copyup_length, {
                                                 SPARSE_EXTENT_STATE_DATA,
                                                 copyup_length,
                                                 std::move(sub_bl)}
                        );
            ...
            auto r =
                m_image_ctx->io_object_dispatcher->prepare_copyup(m_object_no,
          ...
          for (auto & extent:sparse_bufferlist) {
                ...
                if (sbe.state == SPARSE_EXTENT_STATE_DATA) {
                    m_copyup_extent_map.emplace_back(extent.get_off(),
                                                     extent.get_len());
