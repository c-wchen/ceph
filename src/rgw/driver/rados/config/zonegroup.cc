...
static std::string default_zonegroup_oid(const ceph::common::
        ConfigProxy &conf,
        std::string_view realm_id)
{
    const auto prefix =
        name_or_default(conf->rgw_default_zonegroup_info_oid,
                        default_zonegroup_info_oid);
    return fmt::format("{}.{}", prefix, realm_id);
}

int RadosConfigStore::write_default_zonegroup_id(const DoutPrefixProvider *
...
int RadosConfigStore::read_default_zonegroup_id(const DoutPrefixProvider *
        dpp, optional_yield y,
        std::string_view realm_id,
        std::
        string &zonegroup_id)
...
int RadosConfigStore::delete_default_zonegroup_id(const DoutPrefixProvider *
        dpp, optional_yield y,
        std::
...
public:
                ...
                const RGWZoneGroup &info) override
    {
        ...
        return impl->write(dpp, y, pool, info_oid, Create::MustExist, info,
                           &objv);
    }
               ...
               RGWZoneGroup &info, std::string_view new_name) override
    {
        ...
        info.set_name(std::string {
            new_name}
                     );
        r = impl->write(dpp, y, pool, info_oid, Create::MustExist, info,
                        &objv);
        if (r < 0) {
                                       ...
                                       sal::ZoneGroupWriter > *writer)
{
    ...
    if (writer) {
        *writer =
            std::make_unique < RadosZoneGroupWriter > (impl.get(),
                std::move(objv),
                info.get_id(),
                info.get_name());
    }
