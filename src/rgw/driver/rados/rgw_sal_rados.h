...
namespace rgw {
    namespace sal {
        ...
        class RadosPlacementTier:public StorePlacementTier {
          ...
          public:
             RadosPlacementTier(RadosStore * _store,
                                const RGWZoneGroupPlacementTier &
                                _tier):store(_store), tier(_tier) {
            } virtual ~ RadosPlacementTier() = default;

            virtual const std::string & get_tier_type() {
                return tier.tier_type;
            } virtual const std::string & get_storage_class() {
                return tier.storage_class;
            }
            virtual bool retain_head_object() {
                return tier.retain_head_object;
            }
            RGWZoneGroupPlacementTier & get_rt() {
                return tier;
            }
        };
        ...
        class RadosZoneGroup:public StoreZoneGroup {
            ...
            const RGWZoneGroup group;
             std::string empty;
          public:
             RadosZoneGroup(RadosStore * _store,
                            const RGWZoneGroup & _group):store(_store),
                group(_group) {
  ...
  /** Get the endpoint from zonegroup, or from master zone if not set */
            ...
            virtual int get_zone_by_name(const std::string & name,
                                         std::unique_ptr < Zone >
                                         *zone) override;
            virtual int list_zones(std::list < std::string >
                                   &zone_ids) override;
            ...
            const RGWZoneGroup & get_group() const {
                return group;
        }};
        ...
        class RadosZone:public StoreZone {
          protected:
            ...
            false};
          public:
             RadosZone(RadosStore * _store,
                       std::unique_ptr < ZoneGroup > _zg):store(_store),
                group(std::move(_zg)), local_zone(true) {
            } RadosZone(RadosStore * _store, std::unique_ptr < ZoneGroup > _zg,
                        RGWZone & z):store(_store), group(std::move(_zg)),
                rgw_zone(z) {
            }
            ~RadosZone() = default;
        ...
        class RadosStore:public StoreDriver {
          ...
          public:
            ...
            int delete_raw_obj(const DoutPrefixProvider * dpp,
                               const rgw_raw_obj & obj);
            int delete_raw_obj_aio(const DoutPrefixProvider * dpp,
                                   const rgw_raw_obj & obj, Completions * aio);
            void get_raw_obj(const rgw_placement_rule & placement_rule,
                             const rgw_obj & obj, rgw_raw_obj * raw_obj);
            int get_raw_chunk_size(const DoutPrefixProvider * dpp,
                                   const rgw_raw_obj & obj,
                                   uint64_t * chunk_size);

            void setRados(RGWRados * st) {
                rados = st;
            }
            RGWRados *getRados(void) {
                return rados;
            }
            ...
            const RGWServices *svc() const {
                return &rados->svc;
            } RGWCtl *ctl() {
                return &rados->ctl;
            }
            const RGWCtl *ctl() const {
                return &rados->ctl;
            } void setUserCtl(RGWUserCtl * _ctl) {
                user_ctl = _ctl;
            }
        };
        ...
        class RadosUser:public StoreUser {
          ...
          public:
            RadosUser(RadosStore * _st, const rgw_user & _u):StoreUser(_u),
                store(_st) {
            } RadosUser(RadosStore * _st, const RGWUserInfo & _i):StoreUser(_i),
                store(_st) {
            }
        ...
        class RadosObject:public StoreObject {
          ...
          private:
            int read_attrs(const DoutPrefixProvider * dpp,
                           RGWRados::Object::Read & read_op, optional_yield y,
        ...
        class RadosBucket:public StoreBucket {
          ...
          private:
            int link(const DoutPrefixProvider * dpp, User * new_user,
                     optional_yield y, bool update_entrypoint =
                     true, RGWObjVersionTracker * objv = nullptr);
            int unlink(const DoutPrefixProvider * dpp, User * new_user,
                       optional_yield y, bool update_entrypoint = true);
        ...
        class RadosMultipartPart:public StoreMultipartPart {
          ...
          public:
            ...
            virtual uint32_t get_num() {
                return info.num;
            } virtual uint64_t get_size() {
                return info.accounted_size;
            }
            virtual const std::string & get_etag() {
                return info.etag;
            }
            virtual ceph::real_time & get_mtime() {
            ...
            const std::set < std::string > &get_past_prefixes() const {
        ...
        class RadosMultipartUpload:public StoreMultipartUpload {
          ...
          public:
             RadosMultipartUpload(RadosStore * _store, Bucket * _bucket,
                                  const std::string & oid,
                                  std::optional < std::string > upload_id,
                                  ACLOwner owner, ceph::real_time _mtime)
            :StoreMultipartUpload(_bucket), store(_store), mp_obj(oid,
                                                                  upload_id),
        ...
        class MPRadosSerializer:public StoreMPSerializer {
          ...
          public:
            MPRadosSerializer(const DoutPrefixProvider * dpp,
        ...
        class LCRadosSerializer:public StoreLCSerializer {
          ...
          public:
            LCRadosSerializer(RadosStore * store, const std::string & oid,
                              const std::string & lock_name,
        ...
        class RadosNotification:public StoreNotification {
          ...
          public:
             RadosNotification(const DoutPrefixProvider * _dpp,
                               RadosStore * _store, Object * _obj,
                               Object * _src_obj, req_state * _s,
                               rgw::notify::EventType _type, optional_yield y,
                               const std::string *
                               object_name):StoreNotification(_obj, _src_obj,
                                                              _type),
                store(_store), res(_dpp, _store, _s, _obj, _src_obj,
                                   object_name, y) {
            } RadosNotification(const DoutPrefixProvider * _dpp,
                                RadosStore * _store, Object * _obj,
                                Object * _src_obj, rgw::notify::EventType _type,
                                rgw::sal::Bucket * _bucket,
                                std::string & _user_id,
                                std::string & _user_tenant,
                                std::string & _req_id,
                                optional_yield y):StoreNotification(_obj,
                                                                    _src_obj,
                                                                    _type),
                store(_store), res(_dpp, _store, _obj, _src_obj, _bucket,
        ...
        class RadosAtomicWriter:public StoreWriter {
          ...
          public:
            RadosAtomicWriter(const DoutPrefixProvider * dpp,
                              ...
                              const rgw_placement_rule * ptail_placement_rule,
                              uint64_t olh_epoch,
                              const std::string & unique_tag):StoreWriter(dpp,
                                                                          y),
                store(_store), aio(std::move(_aio)), obj_ctx(obj_ctx),
                processor(&*aio, store->getRados(), bucket_info,
                          ptail_placement_rule, owner, obj_ctx, obj, olh_epoch,
                          unique_tag, dpp, y) {
        ...
        class RadosAppendWriter:public StoreWriter {
          ...
          public:
            RadosAppendWriter(const DoutPrefixProvider * dpp,
                              ...
                              const std::string & unique_tag,
                              uint64_t position,
                              uint64_t * cur_accounted_size):StoreWriter(dpp,
                                                                         y),
                store(_store), aio(std::move(_aio)), obj_ctx(obj_ctx),
                processor(&*aio, store->getRados(), bucket_info,
                          ptail_placement_rule, owner, obj_ctx, obj, unique_tag,
        ...
        class RadosMultipartWriter:public StoreWriter {
          ...
          public:
            RadosMultipartWriter(const DoutPrefixProvider * dpp,
                                 ...
                                 RGWBucketInfo & bucket_info,
                                 RGWObjectCtx & obj_ctx, const rgw_obj & obj,
                                 RadosStore * _store,
                                 ...
                                 const std::
                                 string & part_num_str):StoreWriter(dpp, y),
                store(_store), aio(std::move(_aio)), obj_ctx(obj_ctx),
                processor(&*aio, store->getRados(), bucket_info,
                          ptail_placement_rule, owner, obj_ctx, obj, upload_id,
        ...
        class RadosOIDCProvider:public RGWOIDCProvider {
          ...
          public:
            ...
            virtual int store_url(const DoutPrefixProvider * dpp,
                                  const std::string & url, bool exclusive,
                                  optional_yield y) override;
            virtual int read_url(const DoutPrefixProvider * dpp,
        ...
        class RadosRole:public RGWRole {
          ...
          public:
             RadosRole(RadosStore * _store, std::string name,
                       ...
                       std::string > tags):RGWRole(name, tenant, path,
                                                   trust_policy,
                                                   max_session_duration, tags),
                store(_store) {
            } RadosRole(RadosStore * _store, std::string id):RGWRole(id),
                store(_store) {
            }
            RadosRole(RadosStore * _store,
                      const RGWRoleInfo & info):RGWRole(info), store(_store) {
            }
          RadosRole(RadosStore * _store):store(_store) {
            }
            ~RadosRole() = default;

            virtual int store_info(const DoutPrefixProvider * dpp,
                                   bool exclusive, optional_yield y) override;
            virtual int store_name(const DoutPrefixProvider * dpp,
                                   bool exclusive, optional_yield y) override;
            virtual int store_path(const DoutPrefixProvider * dpp,
                                   bool exclusive, optional_yield y) override;
            virtual int read_id(const DoutPrefixProvider * dpp,
                                const std::string & role_name,
                                const std::string & tenant,
                                std::string & role_id,
                                optional_yield y) override;
            virtual int read_name(const DoutPrefixProvider * dpp,
                                  optional_yield y) override;
            virtual int read_info(const DoutPrefixProvider * dpp,
                                  optional_yield y) override;
            virtual int create(const DoutPrefixProvider * dpp, bool exclusive,
