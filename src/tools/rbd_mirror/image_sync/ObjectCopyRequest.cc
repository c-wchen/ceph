...
namespace librados {
    ...
    bool operator==(const clone_info_t & rhs, const clone_info_t & lhs) {
        return (rhs.cloneid == lhs.cloneid &&
                rhs.snaps == lhs.snaps &&
                rhs.overlap == lhs.overlap && rhs.size == lhs.size);
    } bool operator==(const snap_set_t & rhs, const snap_set_t & lhs) {
        return (rhs.clones == lhs.clones && rhs.seq == lhs.seq);
    }
    ...
    namespace mirror {
        namespace image_sync {
            ...
            using librbd::util::create_rados_callback;
             ...
             template < typename I >
                ObjectCopyRequest < I >::ObjectCopyRequest(I * local_image_ctx,
                                                           I * remote_image_ctx,
                                                           const SnapMap *
                                                           snap_map,
                                                           uint64_t
            ...
            :m_local_image_ctx(local_image_ctx),
                ...
                m_remote_oid =
                    m_remote_image_ctx->get_object_name(object_number);
                ...
                librados::AioCompletion * rados_completion =
                    create_rados_callback < ObjectCopyRequest < I >,
                    &ObjectCopyRequest < I >::handle_list_snaps > (this);
                ...
                int r =
                    m_remote_io_ctx.aio_operate(m_remote_oid, rados_completion,
                                                &op,
                                                nullptr);
                assert(r == 0);
            ...
            template < typename I >
                ...
                if (r < 0) {
                    derr << ": failed to list snaps: " << cpp_strerror(r) <<
                        dendl;
                    finish(r);
                    return;
                }
              ...
              for (auto & sync_op:sync_ops) {
                    ...
                    case SYNC_OP_TYPE_WRITE:
                        if (!read_required) {
                            ...
                            dout(20) << ": remote_snap_seq=" << remote_snap_seq
                                << dendl;
                            read_required = true;
                        }
                        dout(20) << ": read op: " << sync_op.
                            offset << "~" << sync_op.length << dendl;
                        op.sparse_read(sync_op.offset, sync_op.length,
                                       &sync_op.extent_map, &sync_op.out_bl,
                                       nullptr);
                ...
                librados::AioCompletion * comp = create_rados_callback <
                    ObjectCopyRequest < I >,
                    &ObjectCopyRequest < I >::handle_read_object > (this);
                int r =
                    m_remote_io_ctx.aio_operate(m_remote_oid, comp, &op,
                                                nullptr);
                assert(r == 0);
                comp->release();
            }
            ...
            template < typename I >
                ...
                if (r == -ENOENT && m_read_whole_object[snap_seq]) {
                    dout(5) <<
                        ": object missing when forced to read whole object" <<
                        dendl;
                    r = 0;
                }
                ...
                if (r == -ENOENT) {
                    ...
                    dout(5) <<
                        ": object missing potentially due to removed snapshot"
                        << dendl;
                    send_list_snaps();
                    return;
                }
                ...
                if (r < 0) {
                    derr << ": failed to read from remote object: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I >
                ...
                librados::snap_t remote_snap_seq =
                    m_snap_sync_ops.begin()->first.first;
                ...
                Context *finish_op_ctx;
                {
                    ...
                    finish_op_ctx =
                        start_local_op(m_local_image_ctx->owner_lock, &r);
                }
              ...
              for (auto & sync_op:sync_ops) {
                    ...
                    case SYNC_OP_TYPE_WRITE:
                      ...
                      for (auto it:sync_op.extent_map) {
                            if (object_offset < it.first) {
                                ...
                                op.zero(object_offset,
                                        it.first - object_offset);
                            }
                            dout(20) << ": write op: " << it.first << "~" << it.
                                second << dendl;
                            bufferlist tmpbl;
                            tmpbl.substr_of(sync_op.out_bl, buffer_offset,
                                            it.second);
                        ...
                        if (object_offset < sync_op.offset + sync_op.length) {
                            uint64_t sync_op_end =
                                sync_op.offset + sync_op.length;
                            assert(sync_op_end <=
                                   m_snap_object_sizes[remote_snap_seq]);
                            if (sync_op_end ==
                                m_snap_object_sizes[remote_snap_seq]) {
                                dout(20) << ": trunc op: " << object_offset <<
                                    dendl;
                ...
                r = m_local_io_ctx.aio_operate(m_local_oid, comp, &op,
                                               local_snap_seq, local_snap_ids);
                assert(r == 0);
                comp->release();
            }
            ...
            template < typename I >
                ...
                if (r < 0) {
                    derr << ": failed to write to local object: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I >
                ...
                RWLock::WLocker object_map_locker(m_local_image_ctx->
                                                  object_map_lock);
                bool sent =
                    m_local_image_ctx->object_map->template aio_update <
                    Context, &Context::complete > (snap_object_state.first,
                                                   m_object_number,
                                                   snap_object_state.second, {
                                                   }, {
                                                   }, false, ctx);
            ...
            template < typename I >
                ...
                I >::start_local_op(RWLock & owner_lock, int *r) {
            ...
            template < typename I >
                ...
                librados::snap_t remote_sync_pont_snap_id =
                    m_snap_map->rbegin()->first;
              ...
              for (auto & pair:*m_snap_map) {
                    ...
                    calc_snap_set_diff(cct, m_snap_set, start_remote_snap_id,
                                       end_remote_snap_id, &diff, &end_size,
                                       &exists, &clone_end_snap_id,
                                       &read_whole_object);
                    ...
                    if (exists) {
                        ...
                        m_read_whole_object[clone_end_snap_id] =
                            read_whole_object;
                        ...
                        for (auto it = diff.begin(); it != diff.end(); ++it) {
                            dout(20) << ": read/write op: " << it.
                                get_start() << "~" << it.get_len() << dendl;
                            m_snap_sync_ops[ {
                                            end_remote_snap_id,
                                            clone_end_snap_id}
                            ].emplace_back(SYNC_OP_TYPE_WRITE, it.get_start(),
                                           it.get_len());
                        }
                        if (end_size < prev_end_size) {
                            ...
                            m_snap_sync_ops[ {
                                            end_remote_snap_id,
                                            clone_end_snap_id}
                            ].emplace_back(SYNC_OP_TYPE_TRUNC, end_size, 0U);
                        }
                        ...
                        if (prev_exists) {
                            ...
                            m_snap_sync_ops[ {
                                            end_remote_snap_id,
                                            end_remote_snap_id}
                            ].emplace_back(SYNC_OP_TYPE_REMOVE, 0U, 0U);
                        }
                    }
