...
class MonMap {
   ...
   */
    ...
    // upgrade gate ceph_release_t min_mon_release {
   ...
   */ bool contains(const entity_addr_t & a, std::string * name = nullptr)const {
    ...
    std::string get_name(unsigned n)const {
        ...
        std::map < entity_addr_t, std::string >::const_iterator p =
            addr_mons.find(a);
        if (p == addr_mons.end())
            return std::string();
        else
    ...
    const entity_addrvec_t & get_addrs(const std::string & n) const {
        ...
        std::map < std::string, mon_info_t >::const_iterator p =
            mon_info.find(n);
   ...
   */
#ifdef WITH_SEASTAR
    seastar::future <> build_initial(const crimson::common::ConfigProxy & conf,
                                     bool for_mkfs);
#else
   ...
   */
    ...
    seastar::future <> build_monmap(const crimson::common::ConfigProxy & conf,
                                    bool for_mkfs);
    /// initialize monmap by resolving given service name
    seastar::future <> init_with_dns_srv(bool for_mkfs,
