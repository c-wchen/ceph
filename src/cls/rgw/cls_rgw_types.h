...
struct rgw_zone_set_entry {
    ...
    rgw_zone_set_entry(const std::string &_zone, std::optional < std::string > _location_key): zone(_zone),
...
struct rgw_zone_set {
    ...
    void insert(const std::string &zone,
                std::optional < std::string > location_key);
...
void encode_json(const char *name, const rgw_zone_set &zs,
                 ceph::Formatter *f);
...
struct rgw_bucket_dir_entry_meta {
    ...
    rgw_bucket_dir_entry_meta(): category(RGWObjCategory::None), size(0),
        accounted_size(0), appendable(false)
    {
    } void encode(ceph::buffer::list &bl) const
    {
        ENCODE_START(7, 3, bl);
...
struct rgw_bucket_olh_log_entry {
    ...
    rgw_bucket_olh_log_entry(): epoch(0), op(CLS_RGW_OLH_OP_UNKNOWN),
        delete_marker(false)
    {
    } void encode(ceph::buffer::list &bl) const
    {
...
struct rgw_bucket_olh_entry {
    ...
    uint64_t epoch;
    std::map < uint64_t,
        std::vector < struct rgw_bucket_olh_log_entry >>pending_log;
    std::string tag;
    bool exists;
...
struct rgw_bi_log_entry {
    ...
    rgw_bi_log_entry(): op(CLS_RGW_OP_UNKNOWN),
        state(CLS_RGW_STATE_PENDING_MODIFY), index_ver(0), bilog_flags(0)
    {
    } void encode(ceph::buffer::list &bl) const
    {
        ENCODE_START(4, 1, bl);
...
struct rgw_bucket_category_stats {
    ...
    rgw_bucket_category_stats(): total_size(0), total_size_rounded(0),
        num_entries(0)
    {
    } void encode(ceph::buffer::list &bl) const
    {
...
struct cls_rgw_bucket_instance_entry {
    ...
    void dump(ceph::Formatter *f) const;
    static void generate_test_instances(std::list <
                                        cls_rgw_bucket_instance_entry * > &o);
    ...
    } bool resharding_in_progress() const
    {
        return reshard_status == RESHARD_STATUS::IN_PROGRESS;
                                      ...
                                      const cls_rgw_bucket_instance_entry &v)
    {
...
using rgw_bucket_dir_stats =
    std::map < RGWObjCategory, rgw_bucket_category_stats >;
...
struct rgw_usage_data {
    ...
    rgw_usage_data(): bytes_sent(0), bytes_received(0), ops(0),
        successful_ops(0)
    {
    } rgw_usage_data(uint64_t sent, uint64_t received): bytes_sent(sent),
        bytes_received(received), ops(0), successful_ops(0)
    {
    }
...
struct rgw_usage_log_entry {
    ...
    {
    } rgw_usage_log_entry(std::string &o, std::string &b): owner(o), bucket(b),
        epoch(0)
    {
    }
    rgw_usage_log_entry(std::string &o, std::string &p, std::string &b): owner(o), payer(p), bucket(b),
        epoch(0)
    {
    }
...
struct rgw_user_bucket {
    ...
    rgw_user_bucket()
    {
    } rgw_user_bucket(const std::string &u, const std::string &b): user(u),
        bucket(b)
    {
    }
...
struct cls_rgw_obj {
    ...
    size_t estimate_encoded_size() const
    {
        ...
        return start_overhead +
               ...
               string_overhead + loc.size() + key.estimate_encoded_size();
    }
};
...
struct cls_rgw_obj_chain {
    ...
    cls_rgw_obj_chain()
    {
    } void push_obj(const std::string &pool, const cls_rgw_obj_key &key,
    ...
    void dump(ceph::Formatter *f) const
    {
        ...
        for (std::list < cls_rgw_obj >::const_iterator p = objs.begin();
             p != objs.end(); ++p) {
    ...
    size_t estimate_encoded_size() const
    {
        constexpr size_t start_overhead =
            sizeof(__u8) + sizeof(__u8) + sizeof(ceph_le32);
...
struct cls_rgw_reshard_entry {
    ...
    void dump(ceph::Formatter *f) const;
    static void generate_test_instances(std::list <
                                        cls_rgw_reshard_entry * > &o);

    static void generate_key(const std::string &tenant,
