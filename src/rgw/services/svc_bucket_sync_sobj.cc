  ...
  public:
    ...
    template < typename C1, typename C2 >
        int update_hints(const DoutPrefixProvider * dpp,
                         ...
                         C2 & removed_dests,
                         C1 & added_sources,
                         C2 & removed_sources, optional_yield y);
};

RGWSI_Bucket_Sync_SObj::RGWSI_Bucket_Sync_SObj(CephContext * cct):RGWSI_Bucket_Sync
    (cct)
{
}
                                  ...
                                  RGWSI_Bucket_SObj * bucket_sobj_svc)
{
    ...
    hint_index_mgr.
        reset(new
              RGWSI_Bucket_Sync_SObj_HintIndexManager(svc.zone, svc.sysobj));
}

int RGWSI_Bucket_Sync_SObj::do_start(optional_yield,
                                     const DoutPrefixProvider * dpp)
{
    sync_policy_cache.reset(new RGWChainedCacheImpl <
...
void RGWSI_Bucket_Sync_SObj::get_hint_entities(RGWSI_Bucket_X_Ctx & ctx,
                                               ...
                                               *hint_entities, optional_yield y,
                                               const DoutPrefixProvider * dpp)
{
                                                 ...
                                                 const DoutPrefixProvider * dpp)
{
    ...
    get_hint_entities(ctx, source_zones, handler->get_source_hints(),
                      &hint_entities, y, dpp);
    get_hint_entities(ctx, target_zones, handler->get_target_hints(),
                      &hint_entities, y, dpp);
  ...
  for (auto & hint_entity:hint_entities) {
        ...
        if (iter != temp_map.end()) {
            ...
            int r =
                do_get_policy_handler(ctx, zid, hint_bucket, temp_map,
                                                  ...
                                                  dpp)
{
    ...
    if (bucket.bucket_id.empty()) {
        ...
        if (ret < 0) {
            if (ret != -ENOENT) {
                ldout(cct,
                      0) << "ERROR: svc.bucket->read_bucket_info(bucket=" <<
                    bucket << ") returned r=" << ret << dendl;
            }
            return ret;
        }
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            ldpp_dout(dpp,
                      0) << "ERROR: svc.bucket->read_bucket_instance_info(key="
    ...
    if (!zone_policy_handler) {
        ldpp_dout(dpp,
                  20) << "ERROR: could not find policy handler for zone=" <<
            zone << dendl;
        return -ENOENT;
    }

    e.handler.reset(zone_policy_handler->
                    alloc_child(bucket_info, std::move(attrs)));

    r = e.handler->init(dpp, y);
    if (r < 0) {
        ldpp_dout(dpp,
                  20) << "ERROR: failed to init bucket sync policy handler: r="
            << r << dendl;
        return r;
    }

    temp_map.emplace(optional_zone_bucket {
                     zone, bucket}
                     , e.handler);

    rgw_sync_bucket_entity self_entity(zone.value_or(svc.zone->zone_id()),
                                       bucket);

    r = resolve_policy_hints(ctx, self_entity,
                             e.handler, zone_policy_handler, temp_map, y, dpp);
    if (r < 0) {
        ldpp_dout(dpp,
                  20) << "ERROR: failed to resolve policy hints: bucket_key=" <<
            bucket_key << ", r=" << r << dendl;
        return r;
    }

    if (!sync_policy_cache->put(dpp, svc.cache, cache_key, &e, {
                                &cache_info}
        )) {
        ldpp_dout(dpp,
                  20) <<
            "couldn't put bucket_sync_policy cache entry, might have raced with data changes"
            << dendl;
    }
...
class RGWSI_BS_SObj_HintIndexObj {
  ...
  RGWSI_BS_SObj_HintIndexObj(RGWSI_SysObj * _sysobj_svc, const rgw_raw_obj & _obj):cct(_sysobj_svc->ctx()),
        obj(_obj),
    ...
    template < typename C1, typename C2 >
        int update(const DoutPrefixProvider * dpp,
                   ...
                   C1 * add, C2 * remove, optional_yield y);
  ...
  private:
    template < typename C1, typename C2 >
        void update_entries(const rgw_bucket & info_source,
                            const obj_version & info_source_ver,
                            C1 * add,
                            C2 * remove, single_instance_info * instance);
...
#define MAX_RETRIES 25
    ...
    for (int i = 0; i < MAX_RETRIES; ++i) {
        ...
        update_entries(info_source.bucket,
                       info_source_ver, add, remove, &instance);
    ...
    ldpp_dout(dpp,
              0) << "ERROR: failed to flush hint index: too many retries (obj="
        << obj << "), likely a bug" << dendl;

    return -EIO;
}

template < typename C1, typename C2 >
    void RGWSI_BS_SObj_HintIndexObj::
update_entries(const rgw_bucket & info_source,
               const obj_version & info_source_ver, C1 * add, C2 * remove,
...
int RGWSI_BS_SObj_HintIndexObj::read(const DoutPrefixProvider * dpp,
                                     optional_yield y)
{
    ...
    if (r < 0 && r != -ENOENT) {
        ldpp_dout(dpp,
                  0) << "ERROR: failed reading data (obj=" << obj << "), r=" <<
            r << dendl;
        return r;
    }
...
int RGWSI_BS_SObj_HintIndexObj::flush(const DoutPrefixProvider * dpp,
                                      optional_yield y)
{
    int r;
...
rgw_raw_obj RGWSI_Bucket_Sync_SObj_HintIndexManager::
get_sources_obj(const rgw_bucket & bucket) constconst
{
    rgw_bucket b = bucket;
    b.bucket_id.clear();
...
rgw_raw_obj RGWSI_Bucket_Sync_SObj_HintIndexManager::
get_dests_obj(const rgw_bucket & bucket) constconst
{
    rgw_bucket b = bucket;
    b.bucket_id.clear();
    return rgw_raw_obj(svc.zone->get_zone_params().log_pool,
...
update_hints(const DoutPrefixProvider * dpp, const RGWBucketInfo & bucket_info,
             C1 & added_dests, C2 & removed_dests, C1 & added_sources,
             C2 & removed_sources, optional_yield y)
{
    ...
    if (!added_dests.empty() || !removed_dests.empty()) {
        ...
        if (r < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to update targets index for bucket="
      ...
      for (auto & dest_bucket:added_dests) {
            ...
            if (r < 0) {
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: failed to update targets index for bucket=" <<
                                              ...
                                              optional_yield y)
{
    ...
    if (bucket_info.sync_policy) {
        bucket_info.sync_policy->get_potential_related_buckets(bucket_info.
                                                               bucket,
                                                               &sources_set,
                                                               &dests_set);
    }
  ...
  for (auto & e:dests_set) {
    ...
    return hint_index_mgr->update_hints(dpp, bucket_info,
                                        added_dests,
                                        removed_dests,
                                        added_sources, removed_sources, y);
}
                                             ...
                                             optional_yield y)
{
    ...
    if (orig_bucket_info && orig_bucket_info->sync_policy) {
        orig_bucket_info->sync_policy->
            get_potential_related_buckets(bucket_info.bucket, &orig_sources,
                                          &orig_dests);
    }
    ...
    ldpp_dout(dpp,
              20) << __func__ << "(): bucket=" << bucket_info.
        bucket << ": orig_sources=" << orig_sources << " new_sources=" <<
        sources << dendl;
    ldpp_dout(dpp,
              20) << __func__ << "(): bucket=" << bucket_info.
        bucket << ":  potential sources added=" << added_sources << " removed="
        << removed_sources << dendl;
    ...
    ldpp_dout(dpp,
              20) << __func__ << "(): bucket=" << bucket_info.
        bucket << ": orig_dests=" << orig_dests << " new_dests=" << dests <<
        dendl;
    ldpp_dout(dpp,
              20) << __func__ << "(): bucket=" << bucket_info.
        bucket << ":  potential dests added=" << added_dests << " removed=" <<
        removed_dests << dendl;
    ...
    return hint_index_mgr->update_hints(dpp, bucket_info, dests,    /* set all dests, not just the ones that were added */
                                        removed_dests, sources, /* set all sources, not just that the ones that were added */
                                        removed_sources, y);
}

int RGWSI_Bucket_Sync_SObj::get_bucket_sync_hints(const DoutPrefixProvider *
                                                  dpp,
                                                  const rgw_bucket & bucket,
                                                  std::set < rgw_bucket >
                                                  *sources,
                                                  std::set < rgw_bucket >
                                                  *dests, optional_yield y)
{
    if (!sources && !dests) {
        return 0;
    }
    ...
    if (sources) {
        ...
        if (r < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to update sources index for bucket="
                << bucket << " r=" << r << dendl;
            return r;
        }
    ...
    if (dests) {
        ...
        if (r < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to read targets index for bucket=" <<
                bucket << " r=" << r << dendl;
            return r;
        }
