...
class FileStoreTracker {
  ...
  public:
    ...
    :store(store), db(db), lock("Tracker Lock"), restart_seq(0) {
    } class Transaction {
        class Op {
          public:
        ...
        class Write:public Op {
          public:
            coll_t coll;
            string oid;
             Write(const coll_t & coll, const string & oid)
            :coll(coll), oid(oid) {
        ...
        class Clone:public Op {
          public:
            ...
            string from;
            string to;
             Clone(const coll_t & coll, const string & from, const string & to)
            :coll(coll), from(from), to(to) {
            } void operator() (FileStoreTracker * harness,
                               OutTransaction * out) override {
                harness->clone(make_pair(coll, from), make_pair(coll, to), out);
            }
        };
        class Remove:public Op {
          public:
            coll_t coll;
            string obj;
             Remove(const coll_t & coll, const string & obj)
            :coll(coll), obj(obj) {
            } void operator() (FileStoreTracker * harness,
                               OutTransaction * out) override {
                harness->remove(make_pair(coll, obj), out);
            }
        };
      public:
        ...
        void clone(const coll_t & coll, const string & from, const string & to) {
    ...
    void verify(const coll_t & coll,
                const string & from, bool on_start = false);

  private:
    ObjectContents get_current_content(const pair < coll_t, string > &obj);
    pair < uint64_t, uint64_t > get_valid_reads(const pair < coll_t,
                                                string > &obj);
    ObjectContents get_content(const pair < coll_t, string > &obj,
                               uint64_t version);

    void committed(const pair < coll_t, string > &obj, uint64_t seq);
    void applied(const pair < coll_t, string > &obj, uint64_t seq);
    uint64_t set_content(const pair < coll_t, string > &obj,
    ...
    void clone_range(const pair < coll_t, string > &from,
                     const pair < coll_t, string > &to, OutTransaction * out);
