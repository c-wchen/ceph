...
namespace journal {
    ...
    struct MockJournaler {
        ...
        MOCK_METHOD3(get_client,
                     void (const std::string &, cls::journal::Client *,
                           Context *));
        MOCK_METHOD2(get_cached_client,
                     int (const std::string &, cls::journal::Client *));
        ...
        MOCK_METHOD3(get_tags,
                     void (uint64_t, journal::Journaler::Tags *, Context *));
        MOCK_METHOD4(get_tags,
                     void (uint64_t, uint64_t, journal::Journaler::Tags *,
                           Context *));

        MOCK_METHOD1(start_replay,
                     void (::journal::ReplayHandler * replay_handler));
        ...
        MOCK_METHOD3(start_append,
                     void (int flush_interval, uint64_t flush_bytes,
                           double flush_age));
    ...
    struct MockJournalerProxy {
        ...
        template < typename WorkQueue, typename Timer >
            MockJournalerProxy(WorkQueue * work_queue, Timer * timer,
                               Mutex * timer_lock,
        ...
        } void create(uint8_t order, uint8_t splay_width, int64_t pool_id,
                      Context * on_finish) {
            on_finish->complete(-EINVAL);
        }
        ...
        bool is_initialized() const {
            return MockJournaler::get_instance().is_initialized();
                            ...
                            int64_t * pool_id) {
            MockJournaler::get_instance().get_metadata(order, splay_width,
                                                       pool_id);
        }
        ...
        void get_mutable_metadata(uint64_t * min, uint64_t * active,
                                  ...
                                  Context * on_finish) {
            MockJournaler::get_instance().get_mutable_metadata(min, active,
                                                               clients,
                                                               on_finish);
        }
        ...
        void get_client(const std::string & client_id,
                        cls::journal::Client * client, Context * on_finish) {
            MockJournaler::get_instance().get_client(client_id, client,
                                                     on_finish);
        }

        int get_cached_client(const std::string & client_id,
                              cls::journal::Client * client) {
            return MockJournaler::get_instance().get_cached_client(client_id,
                                                                   client);
        }
        ...
        void get_tag(uint64_t tag_tid, cls::journal::Tag * tag,
                     Context * on_finish) {
        ...
        void get_tags(uint64_t start_after_tag_tid, uint64_t tag_class,
            ...
            MockJournaler::get_instance().get_tags(start_after_tag_tid,
        ...
        void start_append(int flush_interval, uint64_t flush_bytes,
                          double flush_age) {
            MockJournaler::get_instance().start_append(flush_interval,
