...
{
    while (arrived.size() &&
           (now - arrived.begin()->first > (double)(history_duration))) {
    ...
    f->dump_int("duration", history_duration);
    {
        ...
        if (arrived.size()) {
            ...
            for (set < pair < utime_t, TrackedOpRef > >::const_iterator i =
                ...
                if (!i->second->filter_out(filters))
                    continue;
                durationvec.push_back(pair < double,
                                      TrackedOpRef > (i->second->get_duration(),
                                                      i->second));
            }
...
struct ShardedTrackingData {
    ...
    explicit ShardedTrackingData(string
                                 lock_name):ops_in_flight_lock_sharded
        (lock_name.c_str()) {
}};
...
tracking_enabled(tracking), lock("OpTracker::lock"), cct(cct_)
{
...
void TrackedOp::mark_event_string(const string & event, utime_t stamp)
{
    ...
    dout(6) << " seq: " << seq
        << ", time: " << stamp << ", event: " << event << ", op: " << get_desc()
        << dendl;
    _event_marked();
}
...
void TrackedOp::dump(utime_t now, Formatter * f) const const
{
