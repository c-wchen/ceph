...
bool Client::is_reserved_vino(vinodeno_t & vino)
{
    if (MDS_IS_PRIVATE_INO(vino.ino)) {
        ldout(cct,
              -1) << __func__ << " attempt to access reserved inode number " <<
            vino << dendl;
        return true;
    }
    return false;
...
int Client::CommandHook::call(std::string_view command,
...
void Client::_assign_faked_root(Inode * in)
{
    ...
    ldout(cct,
          10) << "inode_num " << inode_num << "inode_num & 0x3ff=" << (inode_num
                                                                       & 0x3ff)
        << dendl;
...
initialize_state(CLIENT_NEW, "Client::initstate_lock"), cct_deleter
{
    m->cct,[](CephContext * p) {
        p->put();
    }
...
objecter_finisher(m->cct), m_command_hook(this), fscid(0)
{
    _reset_faked_inos();
    ...
    caps_release_delay =
        cct->_conf.get_val < std::chrono::seconds >
    ...
    writeback_handler.reset(new ObjecterWriteback(objecter, &objecter_finisher,
                                                  &client_lock));
    objectcacher.reset(new ObjectCacher(cct, "libcephfs", *writeback_handler, client_lock, client_flush_set_callback,   // all commit callback
                                        (void *)this,
...
void Client::tear_down_cache()
{
  ...
  for (auto &[fd, fh]:fd_map) {
        ldout(cct,
              1) << __func__ << " forcing close of fh " << fd << " ino " << fh->
            inode->ino << dendl;
        _release_fh(fh);
    }
    fd_map.clear();
    ...
    while (!opened_dirs.empty()) {
        ...
        ldout(cct,
              1) << __func__ << " forcing close of dir " << dirp << " ino " <<
                        ...
                        bool disconnected)
{
    ...
    ldout(cct, 1) << "dump_inode: " << (disconnected ? "DISCONNECTED " : "")
        << "inode " << in->ino << " " << path << " ref " << in->get_nref()
        << " " << *in << dendl;
    ...
    if (in->dir) {
        ldout(cct,
              1) << "  dir " << in->dir << " size " << in->dir->dentries.
            size() << dendl;
        for (ceph::unordered_map < string, Dentry * >::iterator it =
             in->dir->dentries.begin(); it != in->dir->dentries.end(); ++it) {
            ldout(cct,
                  1) << "   " << in->ino << " dn " << it->first << " " << it->
                second << " ref " << it->second->ref << dendl;
            if (f) {
...
void Client::dump_cache(Formatter * f)
{
    ...
    for (ceph::unordered_map < vinodeno_t, Inode * >::iterator it =
         inode_map.begin(); it != inode_map.end(); ++it) {
...
void Client::dump_status(Formatter * f)
{
    ...
    if (f) {
      ...
      for (const auto & kv:metadata)
        ...
        entity_inst_t inst(messenger->get_myname(),
                           messenger->get_myaddr_legacy());
        f->dump_object("inst", inst);
        f->dump_object("addr", inst.addr);
        f->dump_stream("inst_str") << inst.name << " " << inst.addr.
            get_legacy_str();
...
{
    {
        ...
        plb.add_time_avg(l_c_reply, "reply",
                         "Latency of receiving a reply on metadata request");
        plb.add_time_avg(l_c_lat, "lat",
                         "Latency of processing a metadata request");
        plb.add_time_avg(l_c_wrlat, "wrlat",
                         "Latency of a file data write operation");
        plb.add_time_avg(l_c_read, "rdlat",
                         "Latency of a file data read operation");
        plb.add_time_avg(l_c_fsync, "fsync",
                         "Latency of a file sync operation");
        // average, standard deviation mds/r/w/ latencies
        plb.add_time(l_c_md_avg, "mdavg",
                     "Average latency for processing metadata requests");
        plb.add_u64(l_c_md_sqsum, "mdsqsum",
                    "Sum of squares (to calculate variability/stdev) for metadata requests");
        plb.add_u64(l_c_md_ops, "mdops", "Total metadata IO operations");
        plb.add_time(l_c_rd_avg, "readavg",
                     "Average latency for processing read requests");
        plb.add_u64(l_c_rd_sqsum, "readsqsum",
                    "Sum of squares ((to calculate variability/stdev) for read requests");
        plb.add_u64(l_c_rd_ops, "rdops", "Total read IO operations");
        plb.add_time(l_c_wr_avg, "writeavg",
                     "Average latency for processing write requests");
        plb.add_u64(l_c_wr_sqsum, "writesqsum",
...
void Client::trim_cache_for_reconnect(MetaSession * s)
{
    ...
    for (list < Dentry * >::iterator p = skipped.begin(); p != skipped.end();
         ++p)
...
void Client::update_inode_file_time(Inode * in, int issued,
                                    ...
                                    utime_t mtime, utime_t atime)
{
    ...
    if (issued & (CEPH_CAP_FILE_EXCL |
        ...
        ldout(cct,
              30) << "Don't have enough caps, just taking mds' time values" <<
            dendl;
        if (time_warp_seq >= in->time_warp_seq) {
            in->ctime = ctime;
            in->mtime = mtime;
                                ...
                                const UserPerm & request_perms)
{
    ...
    if (inode_map.count(st->vino)) {
        ...
        ldout(cct,
              12) << __func__ << " had " << *in << " caps " << ccap_string(st->
                                                                           cap.
                                                                           caps)
            << dendl;
    }
    ...
    ldout(cct,
          12) << __func__ << " client inode change_attr: " << in->
        change_attr << " , mds inodestat change_attr:  " << st->
        change_attr << dendl;
    ...
    if (in->snapid == CEPH_NOSNAP) {
        add_update_cap(in, session, st->cap.cap_id, st->cap.caps,
                       st->cap.wanted, st->cap.seq, st->cap.mseq,
                       inodeno_t(st->cap.realm), st->cap.flags, request_perms);
        ...
        if (in->is_dir() &&
            ...
            ldout(cct,
                  10) << " marking (I_COMPLETE|I_DIR_ORDERED) on empty dir " <<
                *in << dendl;
            in->flags |= I_COMPLETE | I_DIR_ORDERED;
            if (in->dir) {
                ldout(cct,
                      10) << " dir is open on empty dir " << in->
                    ino << " with " << in->dir->dentries.
...
Dentry *Client::insert_dentry_inode(Dir * dir, const string & dname,
                                    LeaseStat * dlease, Inode * in,
                                    utime_t from, MetaSession * session,
                                    Dentry * old_dentry)
{
    Dentry *dn = NULL;
    ...
    ldout(cct, 12) << __func__ << " '" << dname << "' vino " << in->vino()
        << " in dir " << dir->parent_inode->vino() << " dn " << dn << dendl;
                                 ...
                                 MetaSession * session)
{
    ...
    ldout(cct,
          15) << __func__ << " " << *dn << " " << *dlease << " from " << from <<
        dendl;
...
void Client::update_dir_dist(Inode * in, DirStat * dst, mds_rank_t from)
{
    // auth
    ldout(cct,
          20) << "got dirfrag map for " << in->ino << " frag " << dst->
        frag << " to mds " << dst->auth << dendl;
    if (dst->auth >= 0) {
    ...
    if (from == dst->auth) {
        ...
        if (!dst->dist.empty())
            in->frag_repmap[dst->frag].assign(dst->dist.begin(),
                                              dst->dist.end());
        else
    ...
    if (diri->flags & I_COMPLETE) {
        if (complete) {
            ...
            if (diri->flags & I_DIR_ORDERED) {
                ldout(cct,
                      10) << " clearing I_DIR_ORDERED on " << *diri << dendl;
 ...
 */
void Client::insert_readdir_results(MetaRequest * request,
                                    MetaSession * session, Inode * diri)
{
    ...
    if (!p.end()) {
        ...
        if (hash_order) {
            if (!readdir_start.empty()) {
                last_hash =
                    ceph_frag_value(diri->hash_dentry_name(readdir_start));
            }
        ...
        if (fg != dst.frag) {
            ldout(cct,
                  10) << "insert_trace got new frag " << fg << " -> " << dst.
                frag << dendl;
            fg = dst.frag;
            if (!hash_order) {
                readdir_offset = 2;
        ...
        ldout(cct,
              10) << __func__ << " " << numdn << " readdir items, end=" << end
            << ", hash_order=" << hash_order << ", readdir_start " <<
            readdir_start << ", last_hash " << last_hash << ", next_offset " <<
        ...
        for (unsigned i = 0; i < numdn; i++) {
            ...
            if (hash_order) {
                ...
                last_hash = hash;
                dn->offset =
                    dir_result_t::make_fpos(hash, readdir_offset++, true);
            }
            else {
                dn->offset =
                    dir_result_t::make_fpos(fg, readdir_offset++, false);
            ...
            if (dirp->release_count == diri->dir_release_count &&
                ...
                if (dirp->cache_index == dir->readdir_cache.size()) {
                    ...
                    if (dirp->inode->is_complete_and_ordered())
                        ceph_assert(dir->readdir_cache[dirp->cache_index] ==
                                    dn);
                    else
            ...
            dirp->buffer.
                push_back(dir_result_t::
                          dentry(dn->offset, dname, dn->alternate_name, in));
            ldout(cct,
                  15) << __func__ << "  " << hex << dn->
                offset << dec << ": '" << dname << "' -> " << in->ino << dendl;
        }
...
Inode *Client::insert_trace(MetaRequest * request, MetaSession * session)
{
    ...
    ldout(cct,
        ...
        sent_stamp << " mds." << session->
        mds_num << " is_target=" << (int)reply->head.
        is_target << " is_dentry=" << (int)reply->head.is_dentry << dendl;
...
mds_rank_t Client::choose_target_mds(MetaRequest * req, Inode ** phash_diri)
{
    ...
    if (req->resend_mds >= 0) {
        ...
        req->resend_mds = -1;
        ldout(cct,
              10) << __func__ << " resend_mds specified as mds." << mds <<
            dendl;
        goto out;
    }
    ...
    if (in) {
        ldout(cct,
              20) << __func__ << " starting with req->inode " << *in << dendl;
        if (req->path.depth()) {
            hash = in->hash_dentry_name(req->path[0]);
            ldout(cct,
                  20) << __func__ << " inode dir hash is " << (int)in->
                dir_layout.dl_dir_hash << " on " << req->path[0]
                << " => " << hash << dendl;
            is_hash = true;
        }
    }
        ...
        if (de->inode) {
            in = de->inode.get();
            ldout(cct,
                  20) << __func__ << " starting with req->dentry inode " << *in
                << dendl;
        }
        else {
            ...
            ldout(cct,
                  20) << __func__ << " dentry dir hash is " << (int)in->
                dir_layout.dl_dir_hash << " on " << de->
                name << " => " << hash << dendl;
            is_hash = true;
        }
    }
        ...
        if (in->snapid != CEPH_NOSNAP) {
            ...
            while (in->snapid != CEPH_NOSNAP) {
                if (in->snapid == CEPH_SNAPDIR)
                    ...
                    ldout(cct,
                          10) << "got unlinked inode, can't look at parent" <<
                        dendl;
                    break;
                }
            }
        ...
        if (is_hash && S_ISDIR(in->mode)
            ...
            if (!req->auth_is_best(issued)) {
                ...
                if (repmapit != in->frag_repmap.end()) {
                    ...
                    auto r =
                        ceph::util::generate_random_number < uint64_t > (0,
                                                                         repmap.
                                                                         size()
                                                                         - 1);
                    mds = repmap.at(r);
                               ...
                               const UserPerm & perms)
{
    ...
    if (extra_bl.length() >= 8) {
        if (session->mds_features.test(CEPHFS_FEATURE_DELEG_INO)) {
             ...
             */
            ldout(cct,
                  10) << "delegated_inos: " << ocres.delegated_inos << dendl;
        }
        else {
    ...
    if (request->target) {
        *ptarget = request->target;
        ...
        if (got_created_ino
            && (p =
                inode_map.find(vinodeno_t(created_ino, CEPH_NOSNAP))) !=
            inode_map.end()) {
            (*ptarget) = p->second;
            ldout(cct,
            ...
            if (d) {
                if (d->dir) {
                    ldout(cct,
                          10) <<
                        "make_request got traceless reply, looking up #" << d->
                        dir->parent_inode->ino << "/" << d->
                        name << " got_ino " << got_created_ino << " ino " <<
                        created_ino << dendl;
...
int Client::make_request(MetaRequest * request,
                         ...
                         InodeRef * ptarget, bool * pcreated,
                         mds_rank_t use_mds,
                         bufferlist * pdirbl, size_t feature_needed)
{
    int r = 0;
    ...
    if (cct->_conf->client_inject_fixed_oldest_tid) {
        ldout(cct,
              20) << __func__ << " injecting fixed oldest_client_tid(1)" <<
            dendl;
    ...
    while (1) {
        ...
        int mds_state =
            (mds ==
             MDS_RANK_NONE) ? MDSMap::STATE_NULL : mdsmap->get_state(mds);
        ...
        if (!have_open_session(mds)) {
            ...
            if (session->state == MetaSession::STATE_OPENING) {
                ldout(cct,
                      10) << "waiting for session to mds." << mds << " to open"
                    << dendl;
        ...
        // wait for signal
        ldout(cct,
              20) << "awaiting reply|forward|kick on " << &caller_cond << dendl;
        request->kick = false;
        std::unique_lock l {
        client_lock, std::adopt_lock};
    ...
    ldout(cct,
          20) << "sendrecv kickback on tid " << tid << " " << request->
        dispatch_cond << dendl;
    request->dispatch_cond = 0;

    if (r >= 0 && ptarget)
        r = verify_reply_trace(r, session.get(), request, reply, ptarget,
                               pcreated, perms);
    ...
    if (request->_put()) {
        ...
        if (other_in &&
            (op == CEPH_MDS_OP_RMDIR ||
             op == CEPH_MDS_OP_RENAME || op == CEPH_MDS_OP_RMSNAP)) {
                                 ...
                                 int unless, int force)
{
    ldout(cct, 20) << __func__ << " enter(in:" << *in << ", req:" << req
        << " mds:" << mds << ", drop:" << ccap_string(drop) << ", unless:" <<
        ccap_string(unless)
        << ", force:" << force << ")" << dendl;
    int released = 0;
...
void Client::encode_cap_releases(MetaRequest * req, mds_rank_t mds)
{
    ...
    if (req->inode_drop && req->inode())
        encode_inode_release(req->inode(), req,
                             mds, req->inode_drop, req->inode_unless);

    if (req->old_inode_drop && req->old_inode())
        encode_inode_release(req->old_inode(), req,
                             mds, req->old_inode_drop, req->old_inode_unless);
    ...
    if (req->dentry_drop && req->dentry())
        encode_dentry_release(req->dentry(), req,
                              mds, req->dentry_drop, req->dentry_unless);
...
void Client::populate_metadata(const std::string & mount_root)
{
    ...
    if (r >= 0) {
        ...
        ldout(cct,
              20) << __func__ << " read hostname '" << u.
            nodename << "'" << dendl;
    }
    else {
        ldout(cct,
              1) << __func__ << " failed to read hostname (" << cpp_strerror(r)
            << ")" << dendl;
    }
#endif
  ...
  for (const auto & i:tokens) {
        ...
        if (eqpos == 0 || eqpos == std::string::npos || eqpos == i.size()) {
            lderr(cct) << "Invalid metadata keyval pair: '" << i << "'" <<
                dendl;
            continue;
        }
...
void Client::update_metadata(std::string const &k, std::string const &v)
{
    ...
    if (it != metadata.end()) {
        ldout(cct,
              1) << __func__ << " warning, overriding metadata field '" << k <<
...
{
    ldout(cct,
          2) << __func__ << " mds." << s->mds_num << " seq " << s->seq << dendl;
    s->state = MetaSession::STATE_CLOSING;
    s->con->send_message2(make_message < MClientSession >
                          (CEPH_SESSION_REQUEST_CLOSE, s->seq));
}

void Client::_closed_mds_session(MetaSession * s, int err, bool rejected)
{
    ldout(cct,
          5) << __func__ << " mds." << s->mds_num << " seq " << s->seq << dendl;
...
void Client::handle_client_session(const MConstRef < MClientSession > &m)
{
    ...
    case CEPH_SESSION_FLUSHMSG:
        ...
        session->con->send_message2(make_message < MClientSession >
                                    (CEPH_SESSION_FLUSHMSG_ACK, m->get_seq()));
        break;
...
bool Client::_any_stale_sessions() const const
{
    ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
                          ...
                          bool drop_cap_releases)
{
    ...
    if (request->mds == -1) {
        ...
        ldout(cct,
              20) << __func__ << " set sent_stamp to " << request->
            sent_stamp << dendl;
    }
    request->mds = mds;
                                                      ...
                                                      mds_rank_t mds)
{
    ...
    if (request->retry_attempt) {
        int old_max_retry =
            sizeof(((struct ceph_mds_request_head *) 0)->num_retry);
...
void Client::handle_client_request_forward(const MConstRef <
                                           MClientRequestForward > &fwd)
{
    ...
    if (mds_requests.count(tid) == 0) {
        ldout(cct,
              10) << __func__ << " no pending request on tid " << tid << dendl;
        return;
    }
    ...
    if (num_fwd <= request->num_fwd || (uint32_t) num_fwd >= UINT32_MAX) {
        ...
        ldout(cct, 0) << __func__ << " request tid " << tid << " new num_fwd "
            << num_fwd << " old num_fwd " << request->
            num_fwd << ", fwd seq overflow" << ", abort it" << dendl;
        return;
    }
...
void Client::handle_client_reply(const MConstRef < MClientReply > &reply)
{
    ...
    if (!is_safe || !request->got_unsafe) {
        ...
        // wake up waiter
        ldout(cct,
              20) << __func__ << " signalling caller " << (void *)request->
            caller_cond << dendl;
        request->caller_cond->notify_all();
...
void Client::_handle_full_flag(int64_t pool)
{
    ...
    for (unordered_map < vinodeno_t, Inode * >::iterator i = inode_map.begin();
        ...
        if (inode->oset.dirty_or_tx
            ...
            ldout(cct,
                  4) << __func__ << ": FULL: inode 0x" << std::hex << i->
                first << std::
                dec << " has dirty objects, purging and setting ENOSPC" <<
                dendl;
...
void Client::cancel_commands(const MDSMap & newmap)
{
  ...
  for (const auto &[tid, op]:commands) {
        ...
        if (newmap.is_dne_gid(op_mds_gid) || newmap.is_laggy_gid(op_mds_gid)) {
            ldout(cct,
                  1) << __func__ << ": cancelling command op " << tid << dendl;
...
void Client::send_reconnect(MetaSession * session)
{
    ...
    for (ceph::unordered_map < vinodeno_t, Inode * >::iterator p =
         inode_map.begin(); p != inode_map.end(); ++p) {
        Inode *in = p->second;
        auto it = in->caps.find(mds);
        if (it != in->caps.end()) {
            if (allow_multi &&
            ...
            m->add_cap(p->first.ino, cap.cap_id, path.get_ino(), path.get_path(),   // ino
...
{
    for (xlist < MetaRequest * >::iterator iter =
         session->unsafe_requests.begin(); !iter.end(); ++iter)
...
void Client::kick_requests_closed(MetaSession * session)
{
    ...
    for (map < ceph_tid_t, MetaRequest * >::iterator p = mds_requests.begin();
        ...
        if (req->mds == session->mds_num) {
            ...
            if (req->got_unsafe) {
                lderr(cct) << __func__ << " removing unsafe request " << req->
                    get_tid() << dendl;
...
void Client::handle_lease(const MConstRef < MClientLease > &m)
{
    ...
    if (m->get_mask() & CEPH_LEASE_VALID) {
        if (!in->dir || in->dir->dentries.count(m->dname) == 0) {
            ldout(cct,
                  10) << " don't have dir|dentry " << m->get_ino() << "/" << m->
                dname << dendl;
            goto revoke;
        }
...
Dentry *Client::link(Dir * dir, const string & name, Inode * in, Dentry * dn)
{
    if (!dn) {
        ...
        ldout(cct,
              15) << "link dir " << dir->
            parent_inode << " '" << name << "' to inode " << in << " dn " << dn
            << " (new dn)" << dendl;
    }
    else {
    ...
    if (in) {                   // link to inode
        ...
        inc_dentry_nr();
        ldout(cct,
              20) << "link  inode " << in << " parents now " << in->
            dentries << dendl;
    }
...
void Client::unlink(Dentry * dn, bool keepdir, bool keepdentry)
{
    ...
    ldout(cct,
          15) << "unlink dir " << dn->dir->parent_inode << " '" << dn->
        name << "' dn " << dn << " inode " << dn->inode << dendl;
    ...
    if (dn->inode) {
        ...
        dec_dentry_nr();
        ldout(cct,
              20) << "unlink  inode " << in << " parents now " << in->
            dentries << dendl;
    }
    ...
    if (keepdentry) {
        ...
        ldout(cct,
              15) << "unlink  removing '" << dn->name << "' dn " << dn << dendl;
...
void Client::get_cap_ref(Inode * in, int cap)
{
    if ((cap & CEPH_CAP_FILE_BUFFER) && in->cap_refs[CEPH_CAP_FILE_BUFFER] == 0) {
        ldout(cct,
              5) << __func__ << " got first FILE_BUFFER ref on " << *in <<
            dendl;
        in->iget();
    }
    if ((cap & CEPH_CAP_FILE_CACHE) && in->cap_refs[CEPH_CAP_FILE_CACHE] == 0) {
        ldout(cct,
              5) << __func__ << " got first FILE_CACHE ref on " << *in << dendl;
        in->iget();
    }
    in->get_cap_ref(cap);
}
    ...
    if (last) {
        ...
        if (in->snapid == CEPH_NOSNAP) {
            if ((last & (CEPH_CAP_FILE_WR | CEPH_CAP_FILE_BUFFER)) &&
                ...
                ldout(cct,
                      10) << __func__ << " finishing pending cap_snap on " <<
                    *in << dendl;
                in->cap_snaps.rbegin()->second.writing = 0;
                finish_cap_snap(in, in->cap_snaps.rbegin()->second,
                                get_caps_used(in));
            ...
            if (last & CEPH_CAP_FILE_BUFFER) {
              for (auto & p:in->cap_snaps)
                ...
                ldout(cct,
                      5) << __func__ << " dropped last FILE_BUFFER ref on " <<
                    *in << dendl;
                ++put_nref;
            }
        }
        if (last & CEPH_CAP_FILE_CACHE) {
            ldout(cct,
                  5) << __func__ << " dropped last FILE_CACHE ref on " << *in <<
                dendl;
            ++put_nref;
        }
        if (drop)
...
int Client::get_caps(Fh * fh, int need, int want, int *phave, loff_t endoff)
{
    ...
    while (1) {
        ...
        if ((file_wanted & need) != need) {
            ldout(cct,
                  10) << "get_caps " << *in << " need " << ccap_string(need)
        ...
        if (have & need & CEPH_CAP_FILE_WR) {
            ...
            if (!in->cap_snaps.empty()) {
                if (in->cap_snaps.rbegin()->second.writing) {
                    ldout(cct,
                          10) << "waiting on cap_snap write to complete" <<
                        dendl;
                    waitfor_caps = true;
                }
              for (auto & p:in->cap_snaps) {
        ...
        if (!waitfor_caps && !waitfor_commit) {
            if ((have & need) == need) {
                ...
                ldout(cct,
                      10) << "get_caps " << *in << " have " << ccap_string(have)
            ...
            ldout(cct,
                  10) << "waiting for caps " << *in << " need " <<
                ccap_string(need) << " want " << ccap_string(want) << dendl;
            waitfor_caps = true;
        }
                      ...
                      int flush, ceph_tid_t flush_tid)
{
    ...
    if (cct->_conf->client_inject_release_failure && revoking) {
        ...
        const int would_have_implemented =
        ...
        ldout(cct,
              20) << __func__ << " injecting failure to release caps" << dendl;
        ...
        ldout(cct,
              20) << __func__ << " issued " << ccap_string(cap->
                                                           issued) << " vs " <<
            ccap_string(would_have_issued) << dendl;
        ldout(cct,
              20) << __func__ << " implemented " << ccap_string(cap->
                                                                implemented) <<
            " vs " << ccap_string(would_have_implemented) << dendl;
    }
    else {
        // Normal behaviour
        cap->issued &= retain;
    ...
    if (!(flags & MClientCaps::FLAG_PENDING_CAPSNAP) &&
        !in->cap_snaps.empty() && in->cap_snaps.rbegin()->second.flush_tid == 0)
    ...
    if (cap == in->auth_cap) {
        if (want & CEPH_CAP_ANY_FILE_WR) {
            ...
            ldout(cct,
                ...
                requested_max_size << dendl;
        }
        else {
            in->requested_max_size = 0;
            ldout(cct,
                  15) <<
                "auth cap, reset requested_max_size due to not wanting any file write cap"
                << dendl;
        }
    }
...
void Client::check_caps(Inode * in, unsigned flags)
{
    ...
    if (!is_unmounting() && in->nlink > 0) {
        if (wanted) {
            retain |= CEPH_CAP_ANY;
                 ...
                 (issued & CEPH_CAP_FILE_SHARED) && (in->flags & I_COMPLETE)) {
  ...
  for (auto &[mds, cap]:in->caps) {
        ...
        if ((cap.issued & CEPH_CAP_FILE_WR) &&
            &cap == in->auth_cap && is_max_size_approaching(in)) {
            ldout(cct,
                  10) << "size " << in->size << " approaching max_size " << in->
                max_size << ", reported " << in->reported_size << dendl;
            goto ack;
        }
...
void Client::queue_cap_snap(Inode * in, SnapContext & old_snapc)
{
    ...
    ldout(cct,
          10) << __func__ << " " << *in << " snapc " << old_snapc << " used " <<
        ccap_string(used) << dendl;

    if (in->cap_snaps.size() && in->cap_snaps.rbegin()->second.writing) {
        ldout(cct,
              10) << __func__ << " already have pending cap_snap on " << *in <<
            dendl;
        return;
    }
    else if (in->caps_dirty() ||
             (used & CEPH_CAP_FILE_WR) || (dirty & CEPH_CAP_ANY_WR)) {
        const auto & capsnapem =
            in->cap_snaps.emplace(std::piecewise_construct,
                                  std::make_tuple(old_snapc.seq),
                                  std::make_tuple(in));
...
{
    ldout(cct,
          10) << __func__ << " " << *in << " capsnap " << (void *)&capsnap <<
    ...
    if (used & CEPH_CAP_FILE_BUFFER) {
        capsnap.writing = 1;
        ldout(cct,
              10) << __func__ << " " << *in << " cap_snap " << &capsnap <<
            " used " << used << " WRBUFFER, delaying" << dendl;
    }
    else {
...
class C_Client_CacheInvalidate:public Context {
  ...
  public:
    C_Client_CacheInvalidate(Client * c, Inode * in, int64_t off,
                             int64_t len):client(c), offset(off), length(len) {
...
void Client::_schedule_invalidate_callback(Inode * in, int64_t off, int64_t len)
{
    ...
    if (ino_invalidate_cb)
        ...
        async_ino_invalidator.
            queue(new C_Client_CacheInvalidate(this, in, off, len));
}
...
{
    ldout(cct,
          10) << __func__ << " " << *in << " " << off << "~" << len << dendl;

    // invalidate our userspace inode cache
    if (cct->_conf->client_oc) {
        vector < ObjectExtent > ls;
        Striper::file_to_extents(cct, in->ino, &in->layout, off, len,
                                 in->truncate_size, ls);
...
void Client::_flush_range(Inode * in, int64_t offset, uint64_t size)
{
    ...
    bool ret =
        objectcacher->file_flush(&in->oset, &in->layout,
...
void Client::add_update_cap(Inode * in, MetaSession * mds_session,
                            uint64_t cap_id, unsigned issued, unsigned wanted,
                            unsigned seq, unsigned mseq, inodeno_t realm,
                            int flags, const UserPerm & cap_perms)
{
    if (!in->is_any_caps()) {
        ...
        ldout(cct,
              15) << __func__ << " first one, opened snaprealm " << in->
            snaprealm << dendl;
    }
    else {
    ...
    const auto & capem =
        in->caps.emplace(std::piecewise_construct, std::forward_as_tuple(mds),
    ...
    if (!capem.second) {
        ...
        if (ceph_seq_cmp(seq, cap.seq) <= 0) {
            if (&cap != in->auth_cap)
                ldout(cct,
                      0) << "WARNING: " << "inode " << *in << " caps on mds." <<
                    mds << " != auth_cap." << dendl;
        ...
        if (in->auth_cap != &cap &&
            ...
            if (in->auth_cap && in->flushing_cap_item.is_on_list()) {
                ldout(cct, 10) << __func__ << " changing auth cap: "
                    ...
                    dendl;
                adjust_session_flushing_caps(in, in->auth_cap->session,
                                             mds_session);
            }
            in->auth_cap = &cap;
        }
    ...
    ldout(cct,
          10) << __func__ << " issued " << ccap_string(old_caps) << " -> " <<
        ccap_string(cap.issued)
...
void Client::remove_cap(Cap * cap, bool queue_release)
{
    ...
    if (queue_release) {
        session->enqueue_cap_release(in.ino,
                                     cap->cap_id,
                                     cap->issue_seq,
                                     cap->mseq, cap_epoch_barrier);
    }
    else {
        dec_pinned_icaps();
    }
    ...
    if (!in.is_any_caps()) {
        ldout(cct,
              15) << __func__ << " last one, closing snaprealm " << in.
            snaprealm << dendl;
        in.snaprealm_item.remove_myself();
    ...
    while (s->caps.size()) {
        ...
        if (caps && !in->caps_issued_mask(caps, true)) {
            if (err == -CEPHFS_EBLOCKLISTED) {
                if (in->oset.dirty_or_tx) {
                    lderr(cct) << __func__ << " still has dirty data on " << *in
                        << dendl;
    ...
    if (r == 0) {
        ...
        bool should_abort =
            (cct->_conf.get_val < bool > ("client_die_on_failed_remount") ||
             ...
             ("client_die_on_failed_dentry_invalidate")) && !(retry_on_error
                                                              &&
                                                              (++retries_on_invalidate
                                                               < max_retries));
        if (should_abort && !is_unmounting()) {
            lderr(cct) <<
                "failed to remount for kernel dentry trimming; quitting!" <<
                dendl;
...
void Client::_invalidate_kernel_dcache()
{
    ...
    if (can_invalidate_dentries) {
        if (dentry_invalidate_cb && root->dir) {
            for (ceph::unordered_map < string, Dentry * >::iterator p =
                 root->dir->dentries.begin(); p != root->dir->dentries.end();
                 ++p) {
                if (p->second->inode)
...
class C_Client_CacheRelease:public Context {
  private:
    ...
    vinodeno_t ino;
  public:
    C_Client_CacheRelease(Client * c, Inode * in):client(c) {
        if (client->use_faked_inos())
            ino = vinodeno_t(in->faked_ino, CEPH_NOSNAP);
        else
            ino = in->vino();
    ...
    while ((caps_size - trimmed) > max && !p.end()) {
        ...
        if (in->caps.size() > 1 && cap != in->auth_cap) {
            ...
            if (!(get_caps_used(in.get()) & ~oissued & mine)) {
                ldout(cct,
                      20) << " removing unused, unneeded non-auth cap on " <<
                    *in << dendl;
            ...
            if (all && in->ino != CEPH_INO_ROOT) {
                ldout(cct,
                      20) << __func__ << " counting as trimmed: " << *in <<
                    dendl;
                trimmed++;
            }
        }
...
int Client::mark_caps_flushing(Inode * in, ceph_tid_t * ptid)
{
    ...
    if (!in->flushing_caps) {
        ldout(cct,
              10) << __func__ << " " << ccap_string(flushing) << " " << *in <<
            dendl;
        num_flushing_caps++;
    }
    else {
        ldout(cct,
              10) << __func__ << " (more) " << ccap_string(flushing) << " " <<
            *in << dendl;
    }
...
void Client::adjust_session_flushing_caps(Inode * in, MetaSession * old_s,
                                          MetaSession * new_s)
{
  for (auto & p:in->cap_snaps) {
        CapSnap & capsnap = p.second;
...
void Client::wait_sync_caps(ceph_tid_t want)
{
  retry:
    ldout(cct,
          10) << __func__ << " want " << want << " (last is " << last_flush_tid
  ...
  for (auto & p:mds_sessions) {
        ...
        if (oldest_tid <= want) {
            ldout(cct,
                  10) << " waiting on mds." << p.
                first << " tid " << oldest_tid << " (want " << want << ")" <<
                dendl;
            std::unique_lock l {
            client_lock, std::adopt_lock};
            sync_cond.wait(l);
...
void Client::kick_flushing_caps(Inode * in, MetaSession * session)
{
  ...
  for (auto & p:in->flushing_cap_tids) {
        if (p.second) {
            int msg_flags =
                p.first <
                last_snap_flush ? MClientCaps::FLAG_PENDING_CAPSNAP : 0;
            send_cap(in, session, cap, msg_flags, used, wanted,
...
void Client::kick_flushing_caps(MetaSession * session)
{
    ...
    for (xlist < Inode * >::iterator p = session->flushing_caps.begin();
        ...
        if (in->flags & I_KICK_FLUSH) {
            ldout(cct,
                  20) << " reflushing caps on " << *in << " to mds." << mds <<
                dendl;
...
SnapRealm *Client::get_snap_realm_maybe(inodeno_t r)
{
    ...
    SnapRealm *realm = snap_realms[r];
    ldout(cct,
          20) << __func__ << " " << r << " " << realm << " " << realm->
        nref << " -> " << (realm->nref + 1) << dendl;
    realm->nref++;
    return realm;
}
...
struct SnapRealmInfoMeta {
    ...
    :last_modified(last_modified), change_attr(change_attr) {
    } utime_t last_modified;
    uint64_t change_attr;
};

static std::pair < SnapRealmInfo,
    std::optional <
    SnapRealmInfoMeta >> get_snap_realm_info(MetaSession * session,
                                             bufferlist::const_iterator & p)
{
    if (session->mds_features.test(CEPHFS_FEATURE_NEW_SNAPREALM_INFO)) {
        SnapRealmInfoNew ninfo;
        decode(ninfo, p);
        return std::make_pair(ninfo.info,
                              SnapRealmInfoMeta(ninfo.last_modified,
                                                ninfo.change_attr));
    }
    else {
        SnapRealmInfo info;
...
void Client::update_snap_trace(MetaSession * session, const bufferlist & bl,
                               SnapRealm ** realm_ret, bool flush)
{
    ...
    while (!p.end()) {
        ...
        if (info.seq() > realm->seq ||
            ...
            if (flush) {
                ...
                while (!q.empty()) {
                    ...
                    for (set < SnapRealm * >::iterator p =
                         realm->pchildren.begin(); p != realm->pchildren.end();
                         ++p)
                        q.push_back(*p);
        ...
        if (invalidate) {
            ...
            ldout(cct,
                  15) << __func__ << " " << *realm << " self|parent updated" <<
                dendl;
            ...
            ldout(cct, 10) << __func__ << " " << *realm << " seq " << info.seq()
                << " <= " << realm->
                seq << " and same parent, SKIPPING" << dendl;
        }
...
void Client::handle_snap(const MConstRef < MClientSnap > &m)
{
    ...
    if (m->head.op == CEPH_SNAP_OP_SPLIT) {
      ...
      for (auto & ino:m->split_inos) {
            ...
            if (inode_map.count(vino)) {
                ...
                if (in->snaprealm->created > info.created()) {
                    ldout(cct,
                          10) << " NOT moving " << *in << " from _newer_ realm "
                        << *in->snaprealm << dendl;
                    continue;
                }
                ldout(cct,
                      10) << " moving " << *in << " from " << *in->
                    snaprealm << dendl;
      ...
      for (auto & child_realm:m->split_realms) {
            ldout(cct,
                  10) << "adjusting snaprealm " << child_realm << " parent" <<
                dendl;
    ...
    update_snap_trace(session.get(), m->bl, NULL,
                      m->head.op != CEPH_SNAP_OP_DESTROY);
...
void Client::handle_caps(const MConstRef < MClientCaps > &m)
{
    ...
    if (auto it = inode_map.find(vino); it != inode_map.end()) {
        ...
        if (m->get_op() == CEPH_CAP_OP_IMPORT) {
            ldout(cct,
                  5) << __func__ << " don't have vino " << vino <<
                " on IMPORT, immediately releasing" << dendl;
            session->enqueue_cap_release(m->get_ino(), m->get_cap_id(),
                                         m->get_seq(), m->get_mseq(),
                                         cap_epoch_barrier);
        }
        else {
            ldout(cct,
                  5) << __func__ << " don't have vino " << vino << ", dropping"
                << dendl;
        }
    ...
    if (auto it = in->caps.find(mds); it != in->caps.end()) {
        ...
        ldout(cct,
              5) << __func__ << " don't have " << *in << " cap on mds." << mds
            << dendl;
        return;
    }
}

void Client::handle_cap_import(MetaSession * session, Inode * in,
                               const MConstRef < MClientCaps > &m)
{
    mds_rank_t mds = session->mds_num;

    ldout(cct,
          5) << __func__ << " ino " << m->get_ino() << " mseq " << m->get_mseq()
...
void Client::handle_cap_export(MetaSession * session, Inode * in,
                               const MConstRef < MClientCaps > &m)
{
    mds_rank_t mds = session->mds_num;
                ...
                if (it != in->caps.end()) {
                    ...
                    if (tcap.cap_id == m->peer.cap_id &&
                        ...
                        if (&cap == in->auth_cap)
                            in->auth_cap = &tcap;
                        if (in->auth_cap == &tcap
                            && in->flushing_cap_item.is_on_list())
                            adjust_session_flushing_caps(in, session,
                                                         tsession.get());
                    }
                }
                else {
                    add_update_cap(in, tsession.get(), m->peer.cap_id,
                                   cap.issued, 0, m->peer.seq - 1, m->peer.mseq,
                                   (uint64_t) - 1,
                                   &cap ==
                                   in->auth_cap ? CEPH_CAP_FLAG_AUTH : 0,
                                   cap.latest_perms);
                }
            }
            else {
...
void Client::handle_cap_trunc(MetaSession * session, Inode * in,
                              const MConstRef < MClientCaps > &m)
{
    mds_rank_t mds = session->mds_num;
                                  ...
                                  const MConstRef < MClientCaps > &m)
{
    ...
    if (!cleaned) {
        ...
        if (in->flushing_caps) {
            ldout(cct,
                  5) << "  flushing_caps " << ccap_string(in->flushing_caps)
                                      ...
                                      const MConstRef < MClientCaps > &m)
{
    ...
    if (auto it = in->cap_snaps.find(follows); it != in->cap_snaps.end()) {
        ...
        if (flush_ack_tid != capsnap.flush_tid) {
            ldout(cct,
                  10) << " tid " << flush_ack_tid << " != " << capsnap.
                flush_tid << dendl;
        }
        else {
            InodeRef tmp_ref(in);
            ldout(cct,
                  5) << __func__ << " mds." << mds << " flushed snap follows "
                << follows << " on " << *in << dendl;
        ...
        ldout(cct,
              5) << __func__ << " DUP(?) mds." << mds <<
            " flushed snap follows " << follows << " on " << *in << dendl;
...
class C_Client_DentryInvalidate:public Context {
  private:
    ...
    string name;
  public:
    C_Client_DentryInvalidate(Client * c, Dentry * dn, bool del):client(c),
        name(dn->name) {
        if (client->use_faked_inos()) {
...
void Client::_async_dentry_invalidate(vinodeno_t dirino, vinodeno_t ino,
                                      string & name)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
    ...
    ldout(cct, 10) << __func__ << " '" << name << "' ino " << ino
        << " in dir " << dirino << dendl;
    dentry_invalidate_cb(callback_handle, dirino, ino, name.c_str(),
                         name.length());
}
...
void Client::_schedule_invalidate_dentry_callback(Dentry * dn, bool del)
{
    if (dentry_invalidate_cb && dn->inode->ll_ref > 0)
        async_dentry_invalidator.
            queue(new C_Client_DentryInvalidate(this, dn, del));
}
                              ...
                              const MConstRef < MClientCaps > &m)
{
    ...
    if (!(issued & CEPH_CAP_XATTR_EXCL) &&
        m->xattrbl.length() && m->head.xattr_version > in->xattr_version) {
    ...
    if (revoked) {
        ...
        ldout(cct,
              10) << "  grant, new caps are " << ccap_string(new_caps & ~cap->
                                                             issued) << dendl;
        cap->issued = new_caps;
        cap->implemented |= new_caps;
                        ...
                        const UserPerm & perms)
{
    ...
    if (mask & CEPH_SETATTR_UID) {
        if (perms.uid() != 0
            && (perms.uid() != in->uid || stx->stx_uid != in->uid))
            goto out;
    }
    if (mask & CEPH_SETATTR_GID) {
        if (perms.uid() != 0 && (perms.uid() != in->uid ||
                                 (!perms.gid_in_groups(stx->stx_gid)
                                  && stx->stx_gid != in->gid)))
            goto out;
    }
 ...
 * one MDS.
 */
int Client::resolve_mds(const std::string & mds_spec,
                        std::vector < mds_gid_t > *targets)
{
    ...
    if (role_r == 0) {
        ...
        auto & info =
            fsmap->get_filesystem(role.fscid)->mds_map.get_info(role.rank);
        ldout(cct,
              10) << __func__ << ": resolved " << mds_spec << " to role '" <<
    ...
    if (strtol_err.empty()) {
        ...
        if (fsmap->gid_exists(mds_gid)) {
            ...
            ldout(cct,
                  10) << __func__ << ": validated gid " << mds_gid << " aka " <<
        ...
        ldout(cct,
              10) << __func__ << ": resolving `*' to all MDS daemons" << dendl;
        if (mds_info.empty()) {
      ...
      for (const auto &[gid, info]:mds_info) {
        ...
        if (mds_gid == mds_gid_t {
            0}
        ) {
            lderr(cct) << __func__ << ": no MDS daemons found by name `" <<
                mds_spec << "'" << dendl;
            lderr(cct) << "FSMap: " << *fsmap << dendl;
            return -CEPHFS_ENOENT;
        }
...
int Client::authenticate()
{
    ...
    client_lock.unlock();
    int r =
        monclient->authenticate(std::chrono::duration <
                                double >(mount_timeout).count());
    client_lock.lock();
    if (r < 0) {
        return r;
    }
...
int Client::fetch_fsmap(bool user)
{
    ...
    ldout(cct,
          10) << __func__ << " learned FSMap version " << fsmap_latest << dendl;

    if (user) {
        if (!fsmap_user || fsmap_user->get_epoch() < fsmap_latest) {
            monclient->sub_want("fsmap.user", fsmap_latest,
 ...
 *
 */
int Client::mds_command(const std::string & mds_spec,
...
void Client::handle_command_reply(const MConstRef < MCommandReply > &m)
{
    ...
    if (!command_table.exists(tid)) {
        ldout(cct,
              1) << __func__ << ": unknown tid " << tid << ", dropping" <<
            dendl;
        return;
    }
                  ...
                  bool require_mds, const std::string & fs_name)
{
    ...
    if (r < 0) {
        lderr(cct) << "mdsmap subscription failed: " << cpp_strerror(r) <<
            dendl;
        return r;
    }
    ...
    if (!cct->_conf->client_trace.empty()) {
        ...
        if (traceout.is_open()) {
            ldout(cct,
                  1) << "opened trace file '" << cct->_conf->
                client_trace << "'" << dendl;
        }
        else {
            ldout(cct,
                  1) << "FAILED to open trace file '" << cct->_conf->
                client_trace << "'" << dendl;
        }
    }
    ...
    while (!mds_sessions.empty()) {
      ...
      for (auto & p:mds_sessions) {
        ...
        ldout(cct,
            ...
            size() << " mds session(s) to close (timeout: " << timo << "s)" <<
            dendl;
        std::unique_lock l {
        client_lock, std::adopt_lock};
        if (!timo) {
...
void Client::flush_mdlog(MetaSession * session)
{
    ...
    if (HAVE_FEATURE(features, SERVER_LUMINOUS)) {
        auto m =
            make_message < MClientSession > (CEPH_SESSION_REQUEST_FLUSH_MDLOG);
        session->con->send_message2(std::move(m));
    }
}
...
void Client::_unmount(bool abort)
{
    ...
    if (abort || blocklisted) {
        ldout(cct,
              2) << "unmounting (" << (abort ? "abort)" : "blocklisted)") <<
            dendl;
    }
    else {
    ...
    while (!fd_map.empty()) {
        ...
        ldout(cct,
              0) << " destroyed lost open file " << fh << " on " << *fh->
            inode << dendl;
        _release_fh(fh);
    }
    ...
    if (cct->_conf->client_oc) {
      ...
      for (auto & p:inode_map) {
            ...
            if (!in) {
                ldout(cct,
                      0) << "null inode_map entry ino " << p.first << dendl;
                ceph_assert(in);
            }
    ...
    if (!cct->_conf->client_trace.empty()) {
        ldout(cct,
              1) << "closing trace file '" << cct->_conf->
            client_trace << "'" << dendl;
        traceout.close();
    }
...
void Client::start_tick_thread()
{
    upkeeper = std::thread([this] () {
                           ...
                           auto t_interval =
                           clock::duration(cct->_conf.get_val < sec >
                           ...
                           tick(); last_tick = clock::now();}
                           else {
                           interval -= since;}

                           ldout(cct,
                                 20) << "upkeep thread waiting interval " <<
                           interval << dendl; if (!tick_thread_stopped)
...
void Client::collect_and_send_global_metrics()
{
    ...
    if (!have_open_session((mds_rank_t) 0)) {
        ldout(cct,
              5) << __func__ << ": no session with rank=0 -- not sending metric"
            << dendl;
        return;
    }
    ...
    if (_collect_and_send_global_metrics ||
        ...
        metric =
            ClientMetricMessage(WriteLatencyPayload
                                (logger->tget(l_c_wrlat),
    ...
    if (_collect_and_send_global_metrics ||
        ...
        metric =
            ClientMetricMessage(MetadataLatencyPayload
                                (logger->tget(l_c_lat),
    ...
    if (_collect_and_send_global_metrics ||
        ...
        metric =
            ClientMetricMessage(DentryLeasePayload
                                (dlease_hits, dlease_misses, nr));
        message.push_back(metric);
    }
    ...
    if (_collect_and_send_global_metrics ||
        ...
        metric =
            ClientMetricMessage(OpenedFilesPayload(opened_files, total_inodes));
        message.push_back(metric);
    }
    ...
    if (_collect_and_send_global_metrics ||
        ...
        metric =
            ClientMetricMessage(PinnedIcapsPayload(pinned_icaps, total_inodes));
        message.push_back(metric);
    }
    ...
    if (_collect_and_send_global_metrics ||
        ...
        metric =
            ClientMetricMessage(OpenedInodesPayload
                                (opened_inodes, total_inodes));
        message.push_back(metric);
    }
    ...
    session->con->send_message2(make_message < MClientMetrics >
                                (std::move(message)));
}
...
void Client::renew_caps(MetaSession * session)
{
    ...
    session->con->send_message2(make_message < MClientSession >
                                (CEPH_SESSION_REQUEST_RENEWCAPS, seq));
}
                       ...
                       InodeRef * target, const UserPerm & perms)
{
    int op =
        dir->snapid ==
        CEPH_SNAPDIR ? CEPH_MDS_OP_LOOKUPSNAP : CEPH_MDS_OP_LOOKUP;
...
bool Client::_dentry_valid(const Dentry * dn)
{
    ...
    if (dn->lease_mds >= 0 && dn->lease_ttl > now &&
        ...
        ldout(cct,
              20) << " bad lease, cap_ttl " << s->cap_ttl << ", cap_gen " << s->
            cap_gen << " vs lease_gen " << dn->lease_gen << dendl;
    }
                    ...
                    std::string * alternate_name)
{
    ...
    if (dname == "..") {
        if (dir->dentries.empty()) {
            ...
            InodeRef tmptarget;
            int r =
                make_request(req, perms, &tmptarget, NULL,
                             rand() % mdsmap->get_num_in_mds());

            if (r == 0) {
                *target = std::move(tmptarget);
                ldout(cct,
                      8) << __func__ << " found target " << (*target)->
                    ino << dendl;
            }
            else {
                *target = dir;
  ...
  relookup:
    if (dir->dir && dir->dir->dentries.count(dname)) {
        ...
        ldout(cct,
              20) << __func__ << " have " << *dn << " from mds." << dn->
            lease_mds << " ttl " << dn->lease_ttl << " seq " << dn->
            lease_seq << dendl;
        ...
        if (!dn->inode || dn->inode->caps_issued_mask(mask, true)) {
            ...
            if (dir->caps_issued_mask(CEPH_CAP_FILE_SHARED, true)) {
                ...
                if (!dn->inode && (dir->flags & I_COMPLETE)) {
                    ldout(cct,
                          10) << __func__ << " concluded ENOENT locally for " <<
                        *dir << " dn '" << dname << "'" << dendl;
                    return -CEPHFS_ENOENT;
                }
            }
        ...
        if (dir->caps_issued_mask(CEPH_CAP_FILE_SHARED, true) &&
            ...
            ldout(cct,
                  10) << __func__ << " concluded ENOENT locally for " << *dir <<
                " dn '" << dname << "'" << dendl;
            return -CEPHFS_ENOENT;
        }
    }
  ...
  hit_dn:
    ...
    if (r < 0)
        ldout(cct,
              10) << __func__ << " " << *dir << " " << dname << " = " << r <<
            dendl;
    else
        ldout(cct,
              10) << __func__ << " " << *dir << " " << dname << " = " <<
            **target << dendl;
    return r;
}
...
int Client::walk(std::string_view path, walk_dentry_result * wdr,
...
int Client::path_walk(const filepath & origpath, walk_dentry_result * result,
                      const UserPerm & perms, bool followsym, int mask,
                      InodeRef dirinode)
{
    ...
    while (i < path.depth() && cur) {
        ...
        if (next && next->is_symlink()) {
            symlinks++;
            ldout(cct,
                  20) << " symlink count " << symlinks << ", value is '" <<
                next->symlink << "'" << dendl;
...
int Client::link(const char *relexisting, const char *relpath,
                 const UserPerm & perm, std::string alternate_name)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
        return -CEPHFS_ENOTCONN;
...
int Client::unlinkat(int dirfd, const char *relpath, int flags,
                     const UserPerm & perm)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
...
int Client::mkdirat(int dirfd, const char *relpath, mode_t mode,
                    const UserPerm & perm, std::string alternate_name)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
    ...
    return _mkdir(dir.get(), name.c_str(), mode, perm, 0, {
                  }
                  , std::move(alternate_name));
}
...
int Client::mkdirs(const char *relpath, mode_t mode, const UserPerm & perms)
{
    ...
    if (r != -CEPHFS_ENOENT)
        return r;
    ldout(cct,
          20) << __func__ << " got through " << i << " directories on path " <<
        relpath << dendl;
    ...
    for (; i < path.depth(); ++i) {
        ...
        if (-CEPHFS_EEXIST == r && i < path.depth() - 1) {
            r = _lookup(cur.get(), path[i].c_str(), CEPH_CAP_AUTH_SHARED, &next,
                        perms);
        }
        if (r < 0)
            return r;
...
int Client::mknod(const char *relpath, mode_t mode, const UserPerm & perms,
                  dev_t rdev)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
...
int Client::symlinkat(const char *target, int dirfd, const char *relpath,
                      const UserPerm & perms, std::string alternate_name)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
...
int Client::readlinkat(int dirfd, const char *relpath, char *buf, loff_t size,
                       const UserPerm & perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
...
int Client::_getattr(Inode * in, int mask, const UserPerm & perms, bool force)
{
    ...
    ldout(cct,
          10) << __func__ << " mask " << ccap_string(mask) << " issued=" << yes
        << dendl;
    if (yes && !force)
        return 0;
...
int Client::_getvxattr(Inode * in,
                       const UserPerm & perms,
                        ...
                        std::vector < uint8_t > *aux)
{
    ...
    if (!aux
        && (mask & (CEPH_SETATTR_FSCRYPT_AUTH | CEPH_SETATTR_FSCRYPT_FILE)))
        return -CEPHFS_EINVAL;
    ...
    if ((in->cap_dirtier_uid >= 0 && perms.uid() != in->cap_dirtier_uid) ||
        ...
        ldout(cct,
              10) << __func__ << " caller " << perms.uid() << ":" << perms.gid()
            << " != cap dirtier " << in->cap_dirtier_uid << ":" << in->
            cap_dirtier_gid << ", forcing sync setattr" << dendl;
        /*
    ...
    if (mask & CEPH_SETATTR_FSCRYPT_FILE) {
        ...
        if (in->caps_issued_mask(CEPH_CAP_FILE_EXCL)) {
            ...
            mask &= ~CEPH_SETATTR_FSCRYPT_FILE;
                 ...
                 in->fscrypt_file != *aux) {
            inode_drop |=
                CEPH_CAP_FILE_SHARED | CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR;
        }
        else {
            mask &= ~CEPH_SETATTR_FSCRYPT_FILE;
        }
...
int Client::fsetattrx(int fd, struct ceph_statx *stx, int mask,
                      const UserPerm & perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
                 ...
                 const UserPerm & perms, frag_info_t * dirstat, int mask)
{
    ...
    ldout(cct,
          3) << __func__ << " enter (relpath " << relpath << " mask " << mask <<
        ")" << dendl;
    ...
    ldout(cct,
          3) << __func__ << " exit (relpath " << relpath << " mask " << mask <<
        ")" << dendl;
    return r;
}
...
unsigned Client::statx_to_mask(unsigned int flags, unsigned int want)
{
    ...
    if (want &
        (CEPH_STATX_MODE | CEPH_STATX_UID | CEPH_STATX_GID | CEPH_STATX_BTIME |
         CEPH_STATX_CTIME | CEPH_STATX_VERSION))
        mask |= CEPH_CAP_AUTH_SHARED;
    if (want & (CEPH_STATX_NLINK | CEPH_STATX_CTIME | CEPH_STATX_VERSION))
        mask |= CEPH_CAP_LINK_SHARED;
    if (want &
        (CEPH_STATX_NLINK | CEPH_STATX_ATIME | CEPH_STATX_MTIME |
...
void Client::fill_statx(Inode * in, unsigned int mask, struct ceph_statx *stx)
{
    ldout(cct, 10) << __func__ << " on " << in->ino << " snap/dev" << in->snapid
        << " mode 0" << oct << in->mode << dec
        << " mtime " << in->mtime << " ctime " << in->
        ctime << " change_attr " << in->change_attr << dendl;
                    ...
                    const UserPerm & perms)
{
    ...
    r = path_walk(path, &in, perms, !(flags & AT_SYMLINK_NOFOLLOW), 0,
                  dirinode);
    if (r < 0) {
        return r;
    }
...
int Client::lchown(const char *relpath, uid_t new_uid, gid_t new_gid,
                   const UserPerm & perms)
{
    return chownat(CEPHFS_AT_FDCWD, relpath, new_uid, new_gid,
                   AT_SYMLINK_NOFOLLOW, perms);
}

int Client::chownat(int dirfd, const char *relpath, uid_t new_uid,
                    gid_t new_gid, int flags, const UserPerm & perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
        return -CEPHFS_ENOTCONN;
...
int Client::futimens(int fd, struct timespec times[2], const UserPerm & perms)
{
    ...
    return _setattrx(f->inode, &attr, CEPH_SETATTR_MTIME | CEPH_SETATTR_ATIME,
                     perms);
}

int Client::utimensat(int dirfd, const char *relpath, struct timespec times[2],
...
void Client::_closedir(dir_result_t * dirp)
{
    ...
    if (dirp->inode) {
        ldout(cct,
              10) << __func__ << " detaching inode " << dirp->inode << dendl;
        dirp->inode.reset();
    }
    _readdir_drop_dirp_buffer(dirp);
                         ...
                         uint64_t ino, loff_t next_off)
{
    ...
    ldout(cct,
          10) << __func__ << " '" << de->d_name << "' -> " << inodeno_t(de->
                                                                        d_ino)
        << " type " << (int)de->
        d_type << " w/ next_off " << hex << next_off << dec << dendl;
#endif
}
...
void Client::_readdir_next_frag(dir_result_t * dirp)
{
    ...
    if (fg.is_rightmost()) {
        ldout(cct,
              10) << __func__ << " advance from " << fg << " to END" << dendl;
        dirp->set_end();
        return;
    }
    ...
    fg = fg.next();
    ldout(cct,
          10) << __func__ << " advance from " << dirp->
        buffer_frag << " to " << fg << dendl;
...
void Client::_readdir_rechoose_frag(dir_result_t * dirp)
{
    ...
    if (fg != cur) {
        ldout(cct,
              10) << __func__ << " frag " << cur << " maps to " << fg << dendl;
        dirp->offset = dir_result_t::make_fpos(fg, 2, false);
        dirp->last_name.clear();
        dirp->next_offset = 2;
    }
...
int Client::_readdir_get_frag(dir_result_t * dirp)
{
    ...
    ldout(cct,
          10) << __func__ << " " << dirp << " on " << dirp->inode->
        ino << " fg " << fg << " offset " << hex << dirp->
        offset << dec << dendl;
    ...
    if (res == 0) {
        ldout(cct,
              10) << __func__ << " " << dirp << " got frag " << dirp->
            buffer_frag << " size " << dirp->buffer.size() << dendl;
    }
    else {
        ldout(cct,
              10) << __func__ << " got error " << res << ", setting end flag" <<
            dendl;
        dirp->set_end();
    }
                              ...
                              int caps, bool getref)
{
    ...
    ldout(cct, 10) << __func__ << " " << dirp << " on " << dirp->inode->ino
        << " last_name " << dirp->last_name << " offset " << hex << dirp->
        offset << dec << dendl;
    ...
    vector < Dentry * >::iterator pd =
        std::lower_bound(dir->readdir_cache.begin(), dir->readdir_cache.end(),
                         ...
                         unsigned want, unsigned flags, bool getref)
{
    ...
    ldout(cct,
          10) << __func__ << " " << *dirp->inode << " offset " << hex << dirp->
        offset << dec << " at_end=" << dirp->at_end()
    ...
    while (1) {
        ...
        ldout(cct,
              10) << "frag " << fg << " buffer size " << dirp->buffer.size()
            << " offset " << hex << dirp->offset << dendl;

        for (auto it =
             std::lower_bound(dirp->buffer.begin(), dirp->buffer.end(),
            ...
            fill_dirent(&de, entry.name.c_str(), stx.stx_mode, stx.stx_ino,
                        next_off);
            ...
            ldout(cct,
                  15) << " de " << de.d_name << " off " << hex << next_off -
                1 << dec << " = " << r << dendl;
            if (r < 0)
                return r;
        ...
        if (diri->shared_gen == dirp->start_shared_gen &&
            ...
            if (diri->dir_ordered_count == dirp->ordered_count) {
                ldout(cct,
                    ...
                    << dendl;
                if (diri->dir) {
                    ceph_assert(diri->dir->readdir_cache.size() >=
                                dirp->cache_index);
                          ...
                          unsigned flags, Inode ** out)
{
    ...
    // call, and returns -1 the second time around.
    int r =
        readdir_r_cb(d, _readdir_single_dirent_cb, (void *)&sr, want, flags,
                     out);
    if (r < -1)
        return r;
    if (out)
...
static int _getdir_cb(void *p, struct dirent *de, struct ceph_statx *stx,
                      off_t off, Inode * in)
{
...
int Client::create_and_open(int dirfd, const char *relpath, int flags,
                            const UserPerm & perms, mode_t mode,
                            ...
                            const char *data_pool, std::string alternate_name)
{
    ...
    bool followsym = !((flags & O_NOFOLLOW)
    ...
    if (r == -CEPHFS_ENOENT && (flags & O_CREAT)) {
        ...
        r = _create(dir.get(), dname.c_str(), flags, mode, &in, &fh,
                    stripe_unit, stripe_count, object_size, data_pool, &created,
                    perms, std::move(alternate_name));
    }
    if (r < 0)
        goto out;
...
int Client::openat(int dirfd, const char *relpath, int flags,
                   ...
                   std::string alternate_name)
{
    ...
    std::scoped_lock locker(client_lock);
    int r =
        create_and_open(dirfd, relpath, flags, perms, mode, stripe_unit,
                        stripe_count,
...
{
    ldout(cct,
          3) << __func__ << " enter(" << ino << ", #" << dirino << "/" << name
        << ")" << dendl;
    ...
    int r = make_request(req, perms, NULL, NULL,
                         rand() % mdsmap->get_num_in_mds());
    ldout(cct,
          3) << __func__ << " exit(" << ino << ", #" << dirino << "/" << name <<
        ") = " << r << dendl;
    return r;
}
 ...
 */
int Client::_lookup_vino(vinodeno_t vino, const UserPerm & perms,
                         Inode ** inode)
{
    ...
    if (r == 0 && inode != NULL) {
        unordered_map < vinodeno_t, Inode * >::iterator p =
            inode_map.find(vino);
...
int Client::_lookup_parent(Inode * ino, const UserPerm & perms, Inode ** parent)
{
    ...
    InodeRef target;
    int r =
        make_request(req, perms, &target, NULL,
                     rand() % mdsmap->get_num_in_mds());
    ...
    if (parent != NULL) {
        if (r == 0) {
            *parent = target.get();
            _ll_get(*parent);
            ldout(cct,
                  8) << __func__ << " found parent " << (*parent)->ino << dendl;
        }
        else {
            *parent = NULL;
        }
...
Fh *Client::_create_fh(Inode * in, int flags, int cmode, const UserPerm & perms)
{
    ...
    if (in->snapid != CEPH_NOSNAP) {
        ...
        ldout(cct,
            ...
            " combined IMMUTABLE SNAP caps " << ccap_string(in->
                                                            caps_issued()) <<
            dendl;
    }
    ...
    if (conf->client_readahead_max_bytes) {
        max_readahead =
            std::min(max_readahead,
                     (uint64_t) conf->client_readahead_max_bytes);
    }
    if (conf->client_readahead_max_periods) {
        max_readahead =
            std::min(max_readahead,
                     in->layout.get_period() *
                     (uint64_t) conf->client_readahead_max_periods);
    }
...
int Client::_release_fh(Fh * f)
{
    ...
    if (err != 0) {
        ldout(cct,
              1) << __func__ << " " << f << " on inode " << *in <<
            " caught async_err = " << cpp_strerror(err) << dendl;
    }
    else {
        ldout(cct,
              10) << __func__ << " " << f << " on inode " << *in <<
...
loff_t Client::_lseek(Fh * f, loff_t offset, int whence)
{
    ...
    default:
        ldout(cct,
              1) << __func__ << ": invalid whence value " << whence << dendl;
        return -CEPHFS_EINVAL;
    }
...
int Client::uninline_data(Inode * in, Context * onfinish)
{
    ...
    char oid_buf[32];
    snprintf(oid_buf, sizeof(oid_buf), "%llx.00000000",
             (long long unsigned)in->ino);
    ...
    uninline_ops.cmpxattr("inline_version",
                          ...
                          CEPH_OSD_CMPXATTR_MODE_U64, inline_version_bl);
    ...
    objecter->mutate(oid,
                     ...
                     uninline_ops,
                     in->snaprealm->get_snap_context(),
                     ceph::real_clock::now(), 0, onfinish);
...
{
    lgeneric_subdout(client->cct, client,
                     20) << "client." << client->
        get_nodeid() << " " << "C_Readahead on " << f->inode << dendl;
...
int Client::_read_async(Fh * f, uint64_t off, uint64_t len, bufferlist * bl)
{
    ...
    if (f->readahead.get_min_readahead_size() > 0) {
        pair < uint64_t, uint64_t > readahead_extent =
            f->readahead.update(off, len, in->size);
        if (readahead_extent.second > 0) {
            ldout(cct,
                  20) << "readahead " << readahead_extent.
                first << "~" << readahead_extent.
                second << " (caller wants " << off << "~" << len << ")" <<
                dendl;
            ...
            if (r2 == 0) {
                ldout(cct,
                      20) << "readahead initiated, c " << onfinish2 << dendl;
                       ...
                       bool * checkeof)
{
    ...
    auto wait_and_copy =
        [&](C_SaferCond & onfinish, bufferlist & tbl, int wanted){
        int r = onfinish.wait();
    ...
    while (left > 0) {
        ...
        filer->read_trunc(in->ino, &in->layout, in->snapid,
                          pos, left, &tbl, 0,
                          in->truncate_size, in->truncate_seq, &onfinish);
        client_lock.unlock();
                                       ...
                                       bool write, bool clamp_to_int)
{
    ...
    if (write) {
        ...
        for (unsigned j = 0, resid = r; j < iovcnt && resid > 0; j++) {
             ...
             */
            const auto round_size =
                std::min < unsigned >(resid, iov[j].iov_len);
                       ...
                       const struct iovec * iov, int iovcnt)
{
    ...
    int r =
        get_caps(f, CEPH_CAP_FILE_WR | CEPH_CAP_AUTH_SHARED, want, &have,
                 endoff);
    if (r < 0)
        return r;
    ...
    if (cct->_conf->client_oc &&
        ...
        filer->write_trunc(in->ino, &in->layout,
                           in->snaprealm->get_snap_context(), offset, size, bl,
                           ceph::real_clock::now(), 0, in->truncate_size,
  ...
  success:
    ...
    if (totalwritten + offset > in->size) {
        ...
        ldout(cct,
            ...
            offset << ", extending file size" << dendl;
    }
    else {
        ldout(cct,
              7) << "wrote to " << totalwritten +
            offset << ", leaving file size at " << in->size << dendl;
    }
...
int Client::_fsync(Inode * in, bool syncdataonly)
{
    ...
    ldout(cct,
          8) << "_fsync on " << *in << " " << (syncdataonly ? "(dataonly)" :
                                               "(data+metadata)") << dendl;
    ...
    if (nullptr != object_cacher_completion) {  // wait on a real reply instead of guessing
        ...
        while (in->cap_refs[CEPH_CAP_FILE_BUFFER] > 0) {
            ldout(cct,
                  10) << "ino " << in->ino << " has " << in->
                cap_refs[CEPH_CAP_FILE_BUFFER]
    ...
    if (!r) {
        ...
        ldout(cct,
              10) << "ino " << in->ino << " has no uncommitted writes" << dendl;
    }
    else {
...
{
    ldout(cct,
          8) << "_fsync(" << f << ", " << (syncdataonly ? "dataonly)" :
                                           "data+metadata)") << dendl;
                    ...
                    unsigned int want, unsigned int flags)
{
    ...
    tout(cct) << __func__ << " flags " << hex << flags << " want " << want <<
        dec << std::endl;
    tout(cct) << dirfd << std::endl;
    tout(cct) << relpath << std::endl;
    ...
    InodeRef in;
    filepath path(relpath);
    r = path_walk(path, &in, perms, !(flags & AT_SYMLINK_NOFOLLOW), mask,
                  dirinode);
    if (r < 0) {
        return r;
    }
...
void Client::_getcwd(string & dir, const UserPerm & perms)
{
    ...
    while (in != root.get()) {
        ...
        if (!dn) {
            // look it up
            ldout(cct,
                  10) << __func__ << " looking up parent for " << *in << dendl;
            MetaRequest *req = new MetaRequest(CEPH_MDS_OP_LOOKUPNAME);
                   ...
                   const UserPerm & perms)
{
    ...
    InodeRef quota_root =
        root->quota.is_enabled(QUOTA_MAX_BYTES) ? root : get_quota_root(root.
                                                                        get(),
                                                                        perms,
                                                                        QUOTA_MAX_BYTES);
    ...
    // enabled
    ceph_assert(cct->_conf.get_val < bool > ("client_quota") == false
                || quota_root != nullptr);
    ...
    if (quota_root && cct->_conf->client_quota_df
        && quota_root->quota.max_bytes) {
        ...
        const fsblkcnt_t total =
            quota_root->quota.max_bytes >> CEPH_BLOCK_SHIFT;
    ...
    if (ret == 0) {
        if (op == CEPH_MDS_OP_GETFILELOCK) {
            ...
            if (lock_type == CEPH_LOCK_FCNTL) {
                if (!in->fcntl_locks)
                    in->fcntl_locks.
                        reset(new ceph_lock_state_t(cct, CEPH_LOCK_FCNTL));
                lock_state = in->fcntl_locks.get();
            }
            else if (lock_type == CEPH_LOCK_FLOCK) {
                if (!in->flock_locks)
                    in->flock_locks.
                        reset(new ceph_lock_state_t(cct, CEPH_LOCK_FLOCK));
...
void Client::_encode_filelocks(Inode * in, bufferlist & bl)
{
    ...
    unsigned nr_fcntl_locks =
        in->fcntl_locks ? in->fcntl_locks->held_locks.size() : 0;
    ...
    unsigned nr_flock_locks =
        in->flock_locks ? in->flock_locks->held_locks.size() : 0;
    ...
    if (fh->fcntl_locks) {
        ...
        for (auto p = lock_state->held_locks.begin();
            ...
            if (in->flags & I_ERROR_FILELOCK) {
                ...
                to_release.push_back(pair < int,
                                     ceph_filelock > (CEPH_LOCK_FCNTL,
                                                      q->second));
            }
        }
        lock_state.reset();
...
int Client::_getlk(Fh * fh, struct flock *fl, uint64_t owner)
{
    ...
    int ret =
        _do_filelock(in, fh, CEPH_LOCK_FCNTL, CEPH_MDS_OP_GETFILELOCK, 0, fl,
                     owner);
    return ret;
}
...
int Client::get_snap_info(const char *path, const UserPerm & perms,
                          SnapInfo * snap_info)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
...
int Client::mksnap(const char *relpath, const char *name, const UserPerm & perm,
                   mode_t mode, const std::map < std::string,
                   std::string > &metadata)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
...
int Client::rmsnap(const char *relpath, const char *name,
                   const UserPerm & perms, bool check_perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
        return -CEPHFS_ENOTCONN;
...
Inode *Client::open_snapdir(Inode * diri)
{
    ...
    if (!inode_map.count(vino)) {
        ...
        if (use_faked_inos())
            _assign_faked_ino(in);
        ldout(cct,
              10) << "open_snapdir created snapshot inode " << *in << dendl;
    }
    else {
        in = inode_map[vino];
...
int Client::ll_lookup_vino(vinodeno_t vino,
                           const UserPerm & perms, Inode ** inode)
{
...
int Client::ll_lookup_inode(struct inodeno_t ino,
                            const UserPerm & perms, Inode ** inode)
{
    vinodeno_t vino(ino, CEPH_NOSNAP);
...
void Client::_ll_drop_pins()
{
    ...
    for (ceph::unordered_map < vinodeno_t, Inode * >::iterator it =
         inode_map.begin(); it != inode_map.end(); it = next) {
        Inode *in = it->second;
        next = it;
        ++next;
        if (in->ll_ref) {
                        ...
                        size_t size, const UserPerm & perms)
{
    ...
    ldout(cct,
          3) << __func__ << " " << vino << " " << name << " size " << size <<
        dendl;
    tout(cct) << __func__ << std::endl;
    tout(cct) << vino.ino.val << std::endl;
    tout(cct) << name << std::endl;
        ...
        if (!strcmp(name, ACL_EA_ACCESS)) {
            ...
            if (value) {
                ...
                if (new_mode != in->mode) {
                    ...
                    ret =
                        _do_setattr(in, &stx, CEPH_SETATTR_MODE, perms,
                                    nullptr);
                    if (ret < 0)
                        return ret;
                }
...
int Client::_setxattr_check_data_pool(string & name, string & value,
                                      const OSDMap * osdmap)
{
    string tmp;
    if (name == "layout") {
...
void Client::_setxattr_maybe_wait_for_osdmap(const char *name,
                                             const void *value, size_t size)
{
    ...
    if (strcmp(name, "ceph.file.layout.pool") == 0
        ...
        int r = objecter->with_osdmap([&](const OSDMap & o){
                                      return _setxattr_check_data_pool(rest, v,
                                                                       &o);}
        );
        ...
        if (r == -CEPHFS_ENOENT) {
            bs::error_code ec;
            ldout(cct,
                  20) << __func__ << ": waiting for latest osdmap" << dendl;
                                       ...
                                       const UserPerm & perms)
{
    ...
    return _do_setattr(in, &stx, CEPH_SETATTR_FSCRYPT_AUTH, perms, nullptr,
                       &aux);
}
                                       ...
                                       const UserPerm & perms)
{
    ...
    return _do_setattr(in, &stx, CEPH_SETATTR_FSCRYPT_FILE, perms, nullptr,
                       &aux);
}
...
size_t Client::_vxattrcb_layout(Inode * in, char *val, size_t size)
{
    ...
    objecter->with_osdmap([&](const OSDMap & o) {
                          ...
                          r += snprintf(val + r, size - r, "%s",
                                        o.get_pool_name(in->layout.pool_id).
                                        c_str());
                          else
...
size_t Client::_vxattrcb_layout_pool(Inode * in, char *val, size_t size)
{
    ...
    objecter->with_osdmap([&](const OSDMap & o) {
                          ...
                          r =
                          snprintf(val, size, "%" PRIu64,
                                   (uint64_t) in->layout.pool_id);}
    ) ;
    return r;
}

size_t Client::_vxattrcb_layout_pool_namespace(Inode * in, char *val,
                                               size_t size)
{
...
{
    return snprintf(val, size, "%llu",
                    (unsigned long long)(in->dirstat.nfiles +
                                         in->dirstat.nsubdirs));
}
...
{
    return snprintf(val, size, "%llu",
                    (unsigned long long)in->dirstat.nsubdirs);
}

size_t Client::_vxattrcb_dir_rentries(Inode * in, char *val, size_t size)
{
    return snprintf(val, size, "%llu",
                    (unsigned long long)(in->rstat.rfiles +
                                         in->rstat.rsubdirs));
}
...
int Client::ll_readlink(Inode * in, char *buf, size_t buflen,
                        const UserPerm & perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
                   ...
                   const UserPerm & perms, InodeRef * inp)
{
    ...
    ldout(cct,
          8) << "mknod(" << path << ", 0" << oct << mode << dec << ") = " << res
        << dendl;
    return res;
}
...
int Client::_create(Inode * dir, const char *name, int flags, mode_t mode,
                    InodeRef * inp, Fh ** fhp, int stripe_unit,
                    int stripe_count, int object_size, const char *data_pool,
                    bool * created, const UserPerm & perms,
                    std::string alternate_name)
{
...
int Client::_mkdir(Inode * dir, const char *name, mode_t mode,
                   const UserPerm & perm, InodeRef * inp,
                     ...
                     InodeRef * inp)
{
    ldout(cct, 8) << "_symlink(" << dir->ino << " " << name << ", " << target
        << ", uid " << perms.uid() << ", gid " << perms.gid() << ")" << dendl;
...
int Client::_rmdir(Inode * dir, const char *name, const UserPerm & perms)
{
    ...
    int op =
        dir->snapid == CEPH_SNAPDIR ? CEPH_MDS_OP_RMSNAP : CEPH_MDS_OP_RMDIR;
    MetaRequest *req = new MetaRequest(op);
    filepath path;
    dir->make_nosnap_relative_path(path);
    path.push_dentry(name);
...
int Client::_rename(Inode * fromdir, const char *fromname, Inode * todir,
                    const char *toname, const UserPerm & perm,
                    std::string alternate_name)
{
    ...
    if (cct->_conf.get_val < bool > ("client_quota") && fromdir != todir) {
        Inode *fromdir_root =
            fromdir->quota.
            is_enabled(QUOTA_MAX_FILES) ? fromdir : get_quota_root(fromdir,
                                                                   perm,
                                                                   QUOTA_MAX_FILES);
        Inode *todir_root =
            todir->quota.
            is_enabled(QUOTA_MAX_FILES) ? todir : get_quota_root(todir, perm,
                                                                 QUOTA_MAX_FILES);
                  ...
                  InodeRef * inp)
{
    ...
    trim_cache();
    ldout(cct,
          8) << "link(" << existing << ", " << path << ") = " << res << dendl;
    return res;
}
...
int Client::ll_open(Inode * in, int flags, Fh ** fhp, const UserPerm & perms)
{
    ...
    ldout(cct,
          3) << "ll_open " << vino << " " << ceph_flags_sys2wire(flags) <<
        dendl;
                       ...
                       const UserPerm & perms)
{
    *fhp = NULL;
    ...
    ldout(cct, 8) << "_ll_create " << vparent << " " << name << " 0" << oct <<
        mode << dec << " " << ceph_flags_sys2wire(flags) << ", uid " << perms.
        uid()
        << ", gid " << perms.gid() << dendl;
                          ...
                          uint64_t length, file_layout_t * layout)
{
    ...
    objecter->read(oid,
                   ...
                   offset,
                   length, vino.snapid, &bl, CEPH_OSD_FLAG_READ, &onfinish);
                           ...
                           uint64_t snapseq, uint32_t sync)
{
    ...
    objecter->write(oid,
                    ...
                    offset,
                    length,
                    fakesnap, bl, ceph::real_clock::now(), 0, onsafe.get());
...
int64_t Client::ll_writev(struct Fh * fh, const struct iovec * iov, int iovcnt,
                          int64_t off)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
...
int64_t Client::ll_readv(struct Fh * fh, const struct iovec * iov, int iovcnt,
                         int64_t off)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
        ...
        if (in->inline_version < CEPH_INLINE_NONE &&
            ...
            if (in->inline_version < CEPH_INLINE_NONE) {
                onuninline.
                    reset(new
                          C_SaferCond
                          ("Client::_fallocate_uninline_data flock"));
                uninline_data(in, onuninline.get());
            }
            ...
            filer->zero(in->ino, &in->layout,
                        in->snaprealm->get_snap_context(),
                        offset, length,
                        ceph::real_clock::now(), 0, true, &onfinish);
            in->mtime = in->ctime = ceph_clock_now();
            in->change_attr++;
            in->mark_caps_dirty(CEPH_CAP_FILE_WR);
...
int Client::ll_fallocate(Fh * fh, int mode, int64_t offset, int64_t length)
{
    ...
    ldout(cct,
          3) << __func__ << " " << fh << " " << fh->inode->ino << " " << dendl;
...
int Client::ll_setlk(Fh * fh, struct flock *fl, uint64_t owner, int sleep)
{
    ...
    ldout(cct,
          3) << __func__ << "  (fh) " << fh << " " << fh->inode->ino << dendl;
...
string Client::get_pool_name(int64_t pool)
{
    ...
    return objecter->with_osdmap([pool] (const OSDMap & o) {
                                 return o.have_pg_pool(pool) ? o.
                                 get_pool_name(pool) : string();}
    );
}
                                 ...
                                 vector < int >&osds)
{
    ...
    Striper::file_to_extents(cct, in->ino, &in->layout, off, 1,
                             in->truncate_size, extents);
    ceph_assert(extents.size() == 1);

    objecter->with_osdmap([&](const OSDMap & o) {
                          pg_t pg =
                          o.object_locator_to_pg(extents[0].oid,
                                                 extents[0].oloc);
...
int Client::get_osd_crush_location(int id, vector < pair < string,
                                   string > >&path)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied())
    ...
    if (id < 0)
        return -CEPHFS_EINVAL;
    return objecter->with_osdmap([&](const OSDMap & o) {
                                 return o.crush->get_full_location_ordered(id,
                                                                           path);}
    );
}
                                    ...
                                    vector < entity_addr_t > &address)
{
    ...
    return objecter->with_osdmap([&](const OSDMap & o) {
                                 ...
                                 o.pg_to_acting_osds(pg, osds);
                                 if (osds.empty())
                                 return -CEPHFS_EINVAL; for (unsigned i = 0;
                                                             i < osds.size();
                                                             i++) {
                                 entity_addr_t addr =
                                 o.get_addrs(osds[i]).front();
...
int Client::get_osd_addr(int osd, entity_addr_t & addr)
{
    ...
    return objecter->with_osdmap([&](const OSDMap & o) {
                                 if (!o.exists(osd))
                                 return -CEPHFS_ENOENT;
                                 addr = o.get_addrs(osd).front(); return 0;}
    ) ;
}
                             ...
                             loff_t length, loff_t offset)
{
    ...
    Striper::file_to_extents(cct, in->ino, &in->layout, offset, length,
                             in->truncate_size, result);

    ldout(cct,
          3) << __func__ << "(" << fd << ", " << length << ", " << offset <<
        ") = 0" << dendl;
    return 0;
}
...
int Client::get_local_osd()
{
    ...
    objecter->with_osdmap([this] (const OSDMap & o) {
                          ...
                          local_osd =
                          o.find_osd_on_ip(messenger->get_myaddrs().front());
          ...
          for (auto & p:mds_sessions) {
            ...
            if (mds >= 0) {
                ...
                case MetaSession::STATE_OPENING:
                    {
                        ldout(cct,
                              1) << "reset from mds we were opening; retrying"
                            << dendl;
                ...
                case MetaSession::STATE_OPEN:
                    {
                        ...
                        if (cct->_conf.get_val < bool >
                            ...
                            ldout(cct,
                                  1) <<
                                "reset from mds we were open; mark session as stale"
                                << dendl;
...
bool Client::check_quota_condition(Inode * in, const UserPerm & perms,
                                   std::function < bool(const Inode & in) >
                                   test)
{
    if (!cct->_conf.get_val < bool > ("client_quota"))
        return false;
...
bool Client::is_quota_files_exceeded(Inode * in, const UserPerm & perms)
{
    return check_quota_condition(in, perms,[](const Inode & in) {
                                 return in.quota.max_files
                                 && in.rstat.rsize() >= in.quota.max_files;}
    );
}
...
bool Client::is_quota_bytes_approaching(Inode * in, const UserPerm & perms)
{
    ...
    return check_quota_condition(in, perms,[&size] (const Inode & in) {
                                 ...
                                 in.quota.max_bytes - in.rstat.rbytes;
                                 return (space >> 4) < size;}
                                 else {
                                 return false;}
                                 }
...
int Client::check_pool_perm(Inode * in, int need)
{
    ...
    if (!have) {
        ...
        if (rd_ret == 0 || rd_ret == -CEPHFS_ENOENT)
            ...
            ldout(cct,
                  10) << __func__ << " on pool " << pool_id << " ns " << pool_ns
...
int Client::_posix_acl_permission(Inode * in, const UserPerm & perms,
                                  unsigned want)
{
    if (acl_type == POSIX_ACL) {
...
int Client::_posix_acl_chmod(Inode * in, mode_t mode, const UserPerm & perms)
{
    ...
    if (acl_type == POSIX_ACL) {
        if (in->xattrs.count(ACL_EA_ACCESS)) {
            ...
            if (r < 0)
                goto out;
            r = _do_setxattr(in, ACL_EA_ACCESS, acl.c_str(), acl.length(), 0,
                             perms);
        }
        else {
            r = 0;
  ...
  out:
    ldout(cct,
          10) << __func__ << " ino " << in->ino << " result=" << r << dendl;
    return r;
}

int Client::_posix_acl_create(Inode * dir, mode_t * mode,
                              bufferlist & xattrs_bl, const UserPerm & perms)
{
    if (acl_type == NO_ACL)
        return 0;
                          ...
                          const std::string & fs_name)
{
    ...
    for (unsigned mds = 0; mds < mdsmap->get_num_in_mds();) {
        if (!mdsmap->is_up(mds)) {
            ldout(cct,
                  10) << "mds." << mds << " not active, waiting for new mdsmap"
                << dendl;
            wait_on_list(waiting_for_mdsmap);
            continue;
        }
    ...
    ldout(cct,
          10) << __func__ << ": waiting for OSD epoch " << reclaim_osd_epoch <<
        dendl;
    bs::error_code ec;
    l.unlock();
    ...
    bool blocklisted = objecter->with_osdmap([this] (const OSDMap & osd_map)->
                                             bool {
                                             return osd_map.
                                             is_blocklisted
                                             (reclaim_target_addrs);}
    );
    if (blocklisted)
        return -CEPHFS_ENOTRECOVERABLE;
...
void Client::finish_reclaim()
{
  ...
  for (auto & p:mds_sessions) {
        p.second->reclaim_state = MetaSession::RECLAIM_NULL;
        auto m =
            make_message < MClientReclaim > ("", MClientReclaim::FLAG_FINISH);
...
void Client::handle_client_reclaim_reply(const MConstRef < MClientReclaimReply >
                                         &reply)
{
    mds_rank_t from = mds_rank_t(reply->get_source().num());
    ldout(cct,
          10) << __func__ << " " << *reply << " from mds." << from << dendl;
...
const char **Client::get_tracked_conf_keys() const const
{
...
mds_rank_t Client::_get_random_up_mds() const const
{
    ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
