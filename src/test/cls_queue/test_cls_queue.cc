...
class TestClsQueue:public::testing::Test {
  protected:
    ...
    void test_enqueue(const std::string & queue_name,
                      int number_of_ops,
                      int number_of_elements, int expected_rc) {
...
TEST_F(TestClsQueue, List)
{
    ...
    do {
        ...
        const auto ret =
            cls_queue_list_entries(ioctx, queue_name, marker, max_elements,
...
TEST_F(TestClsQueue, Dequeue)
{
    ...
    std::vector < cls_queue_entry > entries;
    const auto ret =
        cls_queue_list_entries(ioctx, queue_name, marker, remove_elements,
                               entries, &truncated, end_marker);
    ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, ListEmpty)
{
    ...
    std::vector < cls_queue_entry > entries;
    const auto ret =
        cls_queue_list_entries(ioctx, queue_name, marker, max_elements, entries,
...
TEST_F(TestClsQueue, ListAll)
{
    ...
    std::vector < cls_queue_entry > entries;
    const auto ret =
        cls_queue_list_entries(ioctx, queue_name, marker, total_elements,
                               entries, &truncated, next_marker);
    ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, DeleteAll)
{
    ...
    ret =
        cls_queue_list_entries(ioctx, queue_name, marker, 10, entries,
                               &truncated, end_marker);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(truncated, false);
    ASSERT_EQ(entries.size(), 0);
...
TEST_F(TestClsQueue, EnqueueDequeue)
{
    ...
    std::thread producer([this, &queue_name, &done] {
                         test_enqueue(queue_name, number_of_ops,
                                      number_of_elements, 0); done = true;});
    ...
    std::thread consumer([this, &queue_name, &consume_count, &done] {
                         ...
                         const auto ret =
                         cls_queue_list_entries(ioctx, queue_name, marker,
...
TEST_F(TestClsQueue, MultiProducer)
{
  ...
  for (auto & p:producers) {
        p = std::thread([this, &queue_name, &producer_count] {
                        test_enqueue(queue_name, number_of_ops,
                                     number_of_elements, 0); --producer_count;}
        );
    }
...
TEST_F(TestClsQueue, MultiConsumer)
{
  ...
  for (auto & c:consumers) {
        c = std::
            thread([this, &queue_name, &consume_count, &done,
                    &list_and_remove_lock] {
                   librados::ObjectWriteOperation op;
                   const auto max_elements = 42; const std::string marker;
                   bool truncated = false; std::string end_marker;
                   std::vector < cls_queue_entry > entries;
                   while (!done || truncated) {
...
TEST_F(TestClsQueue, WrapAround)
{
    ...
    for (auto i = 0U; i < number_of_entries; ++i) {
        const auto entry_size =
            rand() % (max_entry_size - min_entry_size + 1) + min_entry_size;
