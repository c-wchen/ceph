...
namespace rbd {
    namespace ggate {
        ...
        Server::IOContext * Server::wait_io_finish() {
            ...
            m_lock};
            m_cond.wait(locker,[this] {
                        return !m_io_finished.empty() || m_stopping;
                        }
            );
        ...
        void Server::handle_aio(IOContext * ctx, int r) {
            ...
            if (r < 0) {
                ...
                int pad_byte_count =
                    static_cast < int >(ctx->req->get_length()) - r;
        ...
        void Server::reader_entry() {
            ...
            while (!m_stopping) {
                ...
                case rbd::ggate::Request::Write:
                    m_image.aio_write(pctx->req->get_offset(),
                                      pctx->req->get_length(), pctx->req->bl,
                                      c);
                    break;
                case rbd::ggate::Request::Read:
                    m_image.aio_read(pctx->req->get_offset(),
                                     pctx->req->get_length(), pctx->req->bl, c);
                    break;
                case rbd::ggate::Request::Flush:
                    m_image.aio_flush(c);
                    break;
                case rbd::ggate::Request::Discard:
                    m_image.aio_discard(pctx->req->get_offset(),
                                        pctx->req->get_length(), c);
                    break;
                default:
                    derr << pctx << ": invalid request command: " << pctx->req->
                        get_cmd()
                        << dendl;
                    c->release();
                    std::lock_guard locker {
