...
//    Request.Log()
//
    constexpr const char *RequestLogAction {
    "Log"};

    int RequestLog(lua_State * L) {
        const auto rest =
            reinterpret_cast <
            RGWREST * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
        const auto olog =
            reinterpret_cast <
            OpsLogSink * >(lua_touserdata(L, lua_upvalueindex(SECOND_UPVAL)));
        const auto s =
            reinterpret_cast <
            req_state * >(lua_touserdata(L, lua_upvalueindex(THIRD_UPVAL)));
        const auto op(reinterpret_cast <
                      RGWOp *
        ...
        if (s) {
            ...
            ldpp_dout(s,
                      1) <<
                "Lua ERROR: missing request state, cannot use ops log" << dendl;
    ...
    int SetAttribute(lua_State * L) {
        auto s =
            reinterpret_cast <
            req_state * >(lua_touserdata(L, lua_upvalueindex(1)));
    ...
    struct ResponseMetaTable:public EmptyMetaTable {
        ...
        static int NewIndexClosure(lua_State * L) {
            auto err =
                reinterpret_cast <
                rgw_err * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
    ...
    struct TraceMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            const auto s =
                reinterpret_cast <
                req_state * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
    ...
    struct OwnerMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            const auto owner =
                reinterpret_cast <
                ACLOwner * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
    ...
    struct BucketMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            ...
            if (rgw::sal::Bucket::empty(bucket)) {
                ...
                create_metatable < QuotaMetaTable > (L, false,
                                                     &(bucket->get_info().
                                                       quota));
            }
            else if (strcasecmp(index, "PlacementRule") == 0) {
                create_metatable < PlacementRuleMetaTable > (L, false,
                                                             &(bucket->
                                                               get_info().
                                                               placement_rule));
            }
    ...
    struct GrantMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            const auto grant =
                reinterpret_cast <
                ACLGrant * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
    ...
    struct ACLMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            const auto acl =
                reinterpret_cast <
                Type * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));

            const char *index = luaL_checkstring(L, 2);

            if (strcasecmp(index, "Owner") == 0) {
                create_metatable < OwnerMetaTable > (L, false,
                                                     &(acl->get_owner()));
            }
            else if (strcasecmp(index, "Grants") == 0) {
                create_metatable < GrantsMetaTable > (L, false,
                                                      &(acl->get_acl().
                                                        get_grant_map()));
            }
            else {
    ...
    struct StatementsMetaTable:public EmptyMetaTable {
        ...
        static std::string statement_to_string(const rgw::IAM::
                                               Statement & statement) {
            std::stringstream ss;
            ss << statement;
            return ss.str();
    ...
    struct PolicyMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            ...
            if (strcasecmp(index, "Text") == 0) {
                ...
                create_metatable < StatementsMetaTable > (L, false,
                                                          &(policy->
                                                            statements));
            }
            else {
    ...
    struct PoliciesMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            ...
            if (index >= (int)policies->size() || index < 0) {
                ...
                create_metatable < PolicyMetaTable > (L, false,
                                                      &((*policies)[index]));
            }
            return ONE_RETURNVAL;
        }
    ...
    struct HTTPMetaTable:public EmptyMetaTable {
        ...
        static int IndexClosure(lua_State * L) {
            const auto info =
                reinterpret_cast <
                req_info * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
            ...
            if (strcasecmp(index, "Parameters") == 0) {
                ...
                create_metatable < StringMapMetaTable < meta_map_t,
                    StringMapWriteableNewIndex < meta_map_t >>> (L, false,
                                                                 &(info->
                                                                   x_meta_map));
            }
        ...
        static int IndexClosure(lua_State * L) {
            ...
            if (strcasecmp(index, "RGWOp") == 0) {
                ...
                create_metatable < StringMapMetaTable <
                    rgw::IAM::Environment >> (L, false, &(s->env));
            }
                ...
                create_metatable < PoliciesMetaTable > (L, false,
                                                        &(s->
                                                          iam_user_policies));
            }
    ...
    void create_top_metatable(lua_State * L, req_state * s, const char *op_name) {
        create_metatable < RequestMetaTable > (L, true, s,
                                               const_cast < char *>(op_name));
        lua_getglobal(L, RequestMetaTable::TableName().c_str());
        ceph_assert(lua_istable(L, -1));
    }

    int execute(rgw::sal::Driver * driver,
                RGWREST * rest,
                OpsLogSink * olog,
                req_state * s, RGWOp * op, const std::string & script) {
        ...
        if (perfcounter) {
            perfcounter->
                inc((rc == -1 ? l_rgw_lua_script_fail : l_rgw_lua_script_ok),
                    1);
        }
