 ...
 */
int get_stripe_unit_count(cls_method_context_t hctx, bufferlist * in,
                          bufferlist * out)
{
    int r = check_exists(hctx);
    if (r < 0)
 ...
 * @returns 0 on success
 */
int set_stripe_unit_count(cls_method_context_t hctx, bufferlist * in,
                          bufferlist * out)
{
    ...
    if (r < 0) {
        CLS_ERR("error writing stripe_unit metadata: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    if (r < 0) {
        CLS_ERR("error writing stripe_count metadata: %s",
                cpp_strerror(r).c_str());
        return r;
    }
...
int get_create_timestamp(cls_method_context_t hctx, bufferlist * in,
                         bufferlist * out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("error reading create_timestamp: %s",
                    cpp_strerror(r).c_str());
            return r;
        }
    }
...
int get_flags(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (snap_id == CEPH_NOSNAP) {
        ...
        if (r < 0 && r != -ENOENT) {
            CLS_ERR("failed to read flags off disk: %s",
                    cpp_strerror(r).c_str());
            return r;
        }
    }
...
int set_parent(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    CLS_LOG(20, "set_parent pool=%llu id=%s snapid=%llu size=%llu",
            (unsigned long long)pool, id.c_str(),
            (unsigned long long)snapid.val, (unsigned long long)size);
    ...
    if (r == 0) {
        CLS_LOG(20, "set_parent existing parent pool=%llu id=%s snapid=%llu"
                "overlap=%llu", (unsigned long long)parent.pool,
                parent.id.c_str(), (unsigned long long)parent.snapid.val,
    ...
    if ((features & RBD_FEATURE_DEEP_FLATTEN) != 0) {
        ...
        do {
            ...
            for (std::set < string >::const_iterator it = keys.begin();
                ...
                if (r < 0) {
                    CLS_ERR("Could not update snapshot: snap_id=%" PRIu64
                            ": %s", snap_id, cpp_strerror(r).c_str());
                    return r;
                }
            }
...
int add_child(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (r < 0)
        CLS_LOG(20, "add_child: omap write failed: %s",
                cpp_strerror(r).c_str());
    return r;
}
...
int remove_child(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    r = read_key(hctx, key, &children);
    if (r < 0) {
        CLS_LOG(20, "remove_child: read omap failed: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    if (children.empty()) {
        ...
        if (r < 0)
            CLS_LOG(20, "remove_child: remove key failed: %s",
                    cpp_strerror(r).c_str());
    }
    else {
        ...
        r = cls_cxx_map_set_val(hctx, key, &childbl);
        if (r < 0)
            CLS_LOG(20, "remove_child: write omap failed: %s",
                    cpp_strerror(r).c_str());
    }
    return r;
}
...
int get_children(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT)
            CLS_LOG(20, "get_children: read omap failed: %s",
                    cpp_strerror(r).c_str());
        return r;
    }
    ::encode(children, *out);
 ...
 * @returns 0 on success, negative error code on failure
 */
int get_snapcontext(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    ...
    if (r < 0) {
        CLS_ERR("could not read the image's snap_seq off disk: %s",
                cpp_strerror(r).c_str());
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int get_object_prefix(cls_method_context_t hctx, bufferlist * in,
                      bufferlist * out)
{
...
int get_data_pool(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (r == -ENOENT) {
        ...
        CLS_ERR("error reading image data pool id: %s",
                cpp_strerror(r).c_str());
        return r;
    }
...
int get_snapshot_name(cls_method_context_t hctx, bufferlist * in,
                      bufferlist * out)
{
    uint64_t snap_id;
...
int get_snapshot_timestamp(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    uint64_t snap_id;
    ...
    CLS_LOG(20, "get_snapshot_timestamp snap_id=%llu",
 ...
 * @returns 0 on success, negative error code on failure.
 */
int get_snapshot_namespace(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    uint64_t snap_id;
...
int snapshot_add(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (r < 0) {
        CLS_ERR("Could not read image's snap_seq off disk: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    if (r == -ENOENT) {
        ...
        CLS_ERR("Could not read snapshot limit off disk: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    do {
        ...
        for (map < string, bufferlist >::iterator it = vals.begin();
            ...
            if ((snap_meta.name == old_meta.name &&
                ...
                CLS_LOG(20,
                        "snap_name %s or snap_id %llu matches existing snap %s %llu",
                        snap_meta.name.c_str(),
                        (unsigned long long)snap_meta.id.val,
                        old_meta.name.c_str(),
                        (unsigned long long)old_meta.id.val);
                return -EEXIST;
            }
        }
 ...
 * @returns 0 on success, negative error code on failure.
 */
int snapshot_rename(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    bufferlist snap_namebl, snap_idbl;
    ...
    CLS_LOG(20, "snapshot_rename id=%llu dst_name=%s",
            (unsigned long long)src_snap_id.val, dst_snap_name.c_str());
    ...
    do {
        ...
        for (map < string, bufferlist >::iterator it = vals.begin();
            ...
            if (dst_snap_name == snap_meta.name) {
                CLS_LOG(20,
                        "snap_name %s  matches existing snap with snap id = %llu ",
                        dst_snap_name.c_str(),
                        (unsigned long long)snap_meta.id.val);
                return -EEXIST;
            }
    ...
    if (r == -ENOENT) {
        CLS_LOG(20, "cannot find existing snap with snap id = %llu ",
 ...
 * @returns 0 on success, negative error code on failure
 */
int snapshot_remove(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    snapid_t snap_id;
 ...
 */
int get_all_features(cls_method_context_t hctx, bufferlist * in,
                     bufferlist * out)
{
                                   ...
                                   const string & name, const string & id)
{
    ...
    if (r < 0) {
        if (r != -ENOENT)
            CLS_ERR("error reading name to id mapping: %s",
                    cpp_strerror(r).c_str());
        return r;
    }
    r = read_key(hctx, id_key, &stored_name);
    if (r < 0) {
        CLS_ERR("error reading id to name mapping: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    if (stored_name != name || stored_id != id) {
        CLS_ERR("stored name '%s' and id '%s' do not match args '%s' and '%s'",
                stored_name.c_str(), stored_id.c_str(), name.c_str(),
                id.c_str());
        return -ESTALE;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int dir_rename_image(cls_method_context_t hctx, bufferlist * in,
                     bufferlist * out)
{
    string src, dest, id;
    try {
...
int dir_get_id(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT)
            CLS_ERR("error reading id for name '%s': %s", name.c_str(),
                    cpp_strerror(r).c_str());
        return r;
    }
    ::encode(id, *out);
...
int dir_list(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    while (more && images.size() < max_return) {
        ...
        int r = cls_cxx_map_get_vals(hctx, last_read, RBD_DIR_NAME_KEY_PREFIX,
                                     max_read, &vals, &more);
        if (r < 0) {
            CLS_ERR("error reading directory by name: %s",
                    cpp_strerror(r).c_str());
            return r;
        }
        ...
        for (map < string, bufferlist >::iterator it = vals.begin();
            ...
            CLS_LOG(20, "adding '%s' -> '%s'",
                    dir_name_from_key(it->first).c_str(), id.c_str());
 ...
 * @returns 0 on success, negative error code on failure
 */
int object_map_load(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int object_map_save(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    BitVector < 2 > object_map;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int object_map_resize(cls_method_context_t hctx, bufferlist * in,
                      bufferlist * out)
{
    ...
    if (object_count < orig_object_map_size) {
        for (uint64_t i = object_count + 1; i < orig_object_map_size; ++i) {
            if (object_map[i] != default_state) {
                CLS_ERR("object map indicates object still exists: %" PRIu64,
                        i);
                return -ESTALE;
            }
        }
 ...
 * @returns 0 on success, negative error code on failure
 */
int object_map_update(cls_method_context_t hctx, bufferlist * in,
                      bufferlist * out)
{
    ...
    if (updated) {
        ...
        r = cls_cxx_write2(hctx, object_map.get_header_length() + byte_offset,
                           ...
                           CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        if (r < 0) {
            CLS_ERR("failed to write object map header: %s",
                    cpp_strerror(r).c_str());
            return r;
        }
        ...
        r = cls_cxx_write2(hctx, object_map.get_footer_offset(),
                           footer_bl.length(), &footer_bl,
                           CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        if (r < 0) {
            CLS_ERR("failed to write object map footer: %s",
                    cpp_strerror(r).c_str());
            return r;
        }
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int metadata_remove(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    string key;
...
int old_snapshots_list(cls_method_context_t hctx, bufferlist * in,
                       bufferlist * out)
{
    bufferlist bl;
    struct rbd_obj_header_ondisk *header;
...
int old_snapshot_add(cls_method_context_t hctx, bufferlist * in,
                     bufferlist * out)
{
    bufferlist bl;
    struct rbd_obj_header_ondisk *header;
    ...
    memcpy(new_snap_names + snap_name_len + 1, snap_names,
           header->snap_names_len);

    /* append new snap id */
    new_snaps = (struct rbd_obj_snap_ondisk *)new_snaps_bp.c_str();
    memcpy(new_snaps + 1, header->snaps,
           sizeof(*new_snaps) * header->snap_count);
...
int old_snapshot_remove(cls_method_context_t hctx, bufferlist * in,
                        bufferlist * out)
{
    bufferlist bl;
    struct rbd_obj_header_ondisk *header;
    ...
    int snaps_id_ofs = sizeof(*header);
    int names_ofs =
        snaps_id_ofs + sizeof(struct rbd_obj_snap_ondisk) * header->snap_count;
    const char *snap_name;
    const char *snap_names = ((char *)header) + names_ofs;
    ...
    if (header->snap_count) {
        ...
        if (i < header->snap_count) {
            ...
            memcpy(new_names_bp.c_str() + names_len, snap_names,
                   end - snap_names);
        }
        newbl.push_back(new_snaps_bp);
        newbl.push_back(new_names_bp);
 ...
 * @returns 0 on success, negative error code on failure.
*/
int old_snapshot_rename(cls_method_context_t hctx, bufferlist * in,
                        bufferlist * out)
{
    bufferlist bl;
    struct rbd_obj_header_ondisk *header;
    ...
    int snaps_id_ofs = sizeof(*header);
    int names_ofs =
        snaps_id_ofs + sizeof(rbd_obj_snap_ondisk) * header->snap_count;
    const char *snap_names = ((char *)header) + names_ofs;
    const char *orig_names = snap_names;
    const char *end = snap_names + header->snap_names_len;
    bufferlist::iterator iter = in->begin();
    ...
    CLS_LOG(20, "rename snap with snap id %llu to dest name %s",
            (unsigned long long)src_snap_id.val, dst_snap_name);
    header->snap_names_len =
        header->snap_names_len - strlen(snap_names) + dst.length();
    ...
    if (header->snap_count) {
        ...
        memcpy(new_snaps_bp.c_str(), header->snaps,
...
namespace mirror {
    ...
    int uuid_get(cls_method_context_t hctx, std::string * mirror_uuid) {
        ...
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR("error reading mirror uuid: %s",
                        cpp_strerror(r).c_str());
            }
            return r;
        }
        ...
        *mirror_uuid =
            std::string(mirror_uuid_bl.c_str(), mirror_uuid_bl.length());
        return 0;
    }
    ...
    int read_peers(cls_method_context_t hctx,
        ...
        while (more) {
            ...
            int r =
                cls_cxx_map_get_vals(hctx, last_read, PEER_KEY_PREFIX.c_str(),
                                     max_read, &vals, &more);
            if (r < 0) {
    ...
    int image_get(cls_method_context_t hctx, const string & image_id,
        ...
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR("error reading mirrored image '%s': '%s'",
    ...
    int image_set(cls_method_context_t hctx, const string & image_id,
        ...
        if (r == -ENOENT) {
            ...
            std::string global_id_key =
                global_key(mirror_image.global_image_id);
            std::string image_id;
            ...
            if (r >= 0) {
                ...
                CLS_ERR("error reading global image id: '%s': '%s'",
                        image_id.c_str(), cpp_strerror(r).c_str());
                return r;
            }
        ...
        r = cls_cxx_map_set_val(hctx, global_key(mirror_image.global_image_id),
                                &image_id_bl);
        if (r < 0) {
            CLS_ERR("error adding global id for image '%s': %s",
                    image_id.c_str(), cpp_strerror(r).c_str());
            return r;
        }
        return 0;
    ...
    int image_remove(cls_method_context_t hctx, const string & image_id) {
        ...
        r = cls_cxx_map_remove_key(hctx,
                                   global_key(mirror_image.global_image_id));
        if (r < 0 && r != -ENOENT) {
            CLS_ERR("error removing global id for image '%s': %s",
                    image_id.c_str(), cpp_strerror(r).c_str());
            return r;
        }

        r = cls_cxx_map_remove_key(hctx,
                                   status_global_key(mirror_image.
                                                     global_image_id));
        if (r < 0 && r != -ENOENT) {
            CLS_ERR("error removing global status for image '%s': %s",
                    image_id.c_str(), cpp_strerror(r).c_str());
            return r;
        }
    ...
    struct MirrorImageStatusOnDisk:cls::rbd::MirrorImageStatus {
        ...
        } MirrorImageStatusOnDisk(const cls::rbd::
                                  MirrorImageStatus & status):cls::rbd::
            MirrorImageStatus(status) {
        }
    ...
    int image_status_set(cls_method_context_t hctx,
        ...
        if (r < 0) {
            CLS_ERR
                ("error setting status for mirrored image, global id '%s': %s",
    ...
    int image_status_remove(cls_method_context_t hctx,
        ...
        int r =
            cls_cxx_map_remove_key(hctx, status_global_key(global_image_id));
        if (r < 0) {
            CLS_ERR
                ("error removing status for mirrored image, global id '%s': %s",
                 global_image_id.c_str(), cpp_strerror(r).c_str());
            return r;
        }
        return 0;
    ...
    int image_status_get(cls_method_context_t hctx,
                         const string & global_image_id,
        ...
        bufferlist bl;
        int r =
            cls_cxx_map_get_val(hctx, status_global_key(global_image_id), &bl);
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR
    ...
    int image_status_list(cls_method_context_t hctx,
                          ...
                          map < std::string,
                          cls::rbd::MirrorImage > *mirror_images,
                          map < std::string,
                          cls::rbd::MirrorImageStatus > *mirror_statuses) {
        ...
        while (more && mirror_images->size() < max_return) {
            ...
            for (auto it = vals.begin(); it != vals.end() &&
                ...
                const std::string & image_id =
                    it->first.substr(IMAGE_KEY_PREFIX.size());
                cls::rbd::MirrorImage mirror_image;
                bufferlist::iterator iter = it->second.begin();
                try {
                ...
                cls::rbd::MirrorImageStatus status;
                int r1 =
                    image_status_get(hctx, mirror_image.global_image_id,
                                     watchers,
                                     &status);
                if (r1 < 0) {
                    continue;
    ...
    int image_status_get_summary(cls_method_context_t hctx,
                                 std::map < cls::rbd::MirrorImageStatusState,
                                 int >*states) {
        std::set < entity_inst_t > watchers;
        int r = list_watchers(hctx, &watchers);
        if (r < 0) {
        ...
        while (more) {
            ...
            r = cls_cxx_map_get_vals(hctx, last_read, IMAGE_KEY_PREFIX,
                                     max_read, &vals, &more);
            if (r < 0) {
                CLS_ERR("error reading mirrored images: %s",
                        cpp_strerror(r).c_str());
                return r;
            }
          ...
          for (auto & list_it:vals) {
                ...
                if (0 !=
                    key.compare(0, IMAGE_KEY_PREFIX.size(), IMAGE_KEY_PREFIX)) {
                    break;
                }
                ...
                cls::rbd::MirrorImageStatus status;
                image_status_get(hctx, mirror_image.global_image_id, watchers,
                                 &status);

                cls::rbd::MirrorImageStatusState state =
                    status.up ? status.
    ...
    int instances_list(cls_method_context_t hctx,
        ...
        while (more) {
            std::map < std::string, bufferlist > vals;
            int r =
                cls_cxx_map_get_vals(hctx, last_read,
                                     INSTANCE_KEY_PREFIX.c_str(),
                                     max_read, &vals, &more);
            if (r < 0) {
                if (r != -ENOENT) {
                    CLS_ERR("error reading mirror instances: %s",
                            cpp_strerror(r).c_str());
                }
                return r;
            }
          ...
          for (auto & it:vals) {
                instance_ids->push_back(it.first.
                                        substr(INSTANCE_KEY_PREFIX.size()));
            }
    ...
    int instances_remove(cls_method_context_t hctx, const string & instance_id) {
        ...
        if (r < 0) {
            CLS_ERR("error removing mirror instance %s: %s",
                    instance_id.c_str(), cpp_strerror(r).c_str());
            return r;
        }
        return 0;
    }
                    ...
                    bufferlist * out)
{
  ...
  for (auto const &peer:peers) {
    ...
    r = cls_cxx_map_set_val(hctx, mirror::peer_key(mirror_peer.uuid), &bl);
    if (r < 0) {
    ...
    while (more && groups.size() < max_return) {
      ...
      for (pair < string, bufferlist > val:vals) {
            ...
            try {
                ...
                CLS_ERR("could not decode id of consistency group '%s'",
                        val.first.c_str());
                return -EIO;
            }
            CLS_LOG(20, "adding '%s' -> '%s'",
                    dir_name_from_key(val.first).c_str(), id.c_str());
 ...
 * @return 0 on success, negative error code on failure
 */
int group_dir_remove(cls_method_context_t hctx, bufferlist * in,
                     bufferlist * out)
{
    string name, id;
    try {
 ...
 * @return 0 on success, negative error code on failure
 */
int group_image_set(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    ...
    do {
        ...
        for (map < string, bufferlist >::iterator it = vals.begin();
            ...
            try {
                ...
                CLS_ERR("error decoding state for image: %s",
                        it->first.c_str());
                return -EIO;
            }
            cls::rbd::GroupImageSpec spec;
            ...
            CLS_LOG(20, "Discovered image %s %" PRId64 " %d",
                    spec.image_id.c_str(), spec.pool_id, (int)state);
...
int trash_remove(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("error reading entry key %s: %s", key.c_str(),
                    cpp_strerror(r).c_str());
        }
        return r;
    }
...
CLS_INIT(rbd)
{
    ...
    cls_register_cxx_method(h_class, "copyup",
                            CLS_METHOD_RD | CLS_METHOD_WR, copyup, &h_copyup);
    cls_register_cxx_method(h_class, "get_parent",
                            CLS_METHOD_RD, get_parent, &h_get_parent);
    ...
    cls_register_cxx_method(h_class, "metadata_get",
                            CLS_METHOD_RD, metadata_get, &h_metadata_get);
    ...
    cls_register_cxx_method(h_class, "object_map_load",
                            CLS_METHOD_RD, object_map_load, &h_object_map_load);
    ...
    cls_register_cxx_method(h_class, "mirror_image_set",
                            CLS_METHOD_RD | CLS_METHOD_WR, mirror_image_set,
                            &h_mirror_image_set);
    cls_register_cxx_method(h_class, "mirror_image_remove",
                            CLS_METHOD_RD | CLS_METHOD_WR, mirror_image_remove,
    ...
    cls_register_cxx_method(h_class, "group_dir_list",
                            CLS_METHOD_RD, group_dir_list, &h_group_dir_list);
    ...
    cls_register_cxx_method(h_class, "image_get_group",
                            CLS_METHOD_RD, image_get_group, &h_image_get_group);
    ...
    cls_register_cxx_method(h_class, "trash_list",
                            CLS_METHOD_RD, trash_list, &h_trash_list);
    cls_register_cxx_method(h_class, "trash_get",
                            CLS_METHOD_RD, trash_get, &h_trash_get);
