...
public:
    DencDumper(const char *name,
        ...
        name}, appender {
        appender}, bl_offset {
        appender.bl.length()}, space_offset {
        space_size()}, start {
        appender.get_pos()}
    {
    }
...
private:
    ...
    } void dump() const
    {
        ...
        ::snprintf(fn, sizeof(fn),
                   ...
                   getpid(), i++);
        int fd =::open(fn, O_WRONLY | O_TRUNC | O_CREAT | O_CLOEXEC | O_BINARY,
                       0644);
        if (fd < 0) {
            return;
        }
        ...
        if (auto bl_delta = appender.bl.length() - bl_offset; bl_delta > 0) {
            ...
            appender.bl.begin(bl_offset + space_offset).copy(bl_delta -
                    space_offset,
                    dump_bl);
...
} template < class It > concept is_const_iterator = requires(It &it, size_t n)
{
    {
        it.get_pos_add(n)
    }
...
template < typename T, class It > requires(!is_const_iterator < It >)
T &get_pos_add(It &i)
{
...
> struct denc_traits <T > {
    ...
    {
        p += sizeof(T);
    } template < class It > requires(!is_const_iterator < It >)
...
template < typename T >
requires _denc::is_any_of < T, int16_t, uint16_t > struct ExtType <T > {
    using type = ceph_le16;
};

template < typename T >
requires _denc::is_any_of < T, int32_t, uint32_t > struct ExtType <T > {
    using type = ceph_le32;
};

template < typename T >
requires _denc::is_any_of < T, int64_t, uint64_t > struct ExtType <T > {
...
} // namespace _denc template < typename T >
...
inline void denc_signed_varint_lowz(int64_t v, It &p)
{
    ...
    unsigned lowznib =
        v ? (std::countr_zero(std::bit_cast < uint64_t > (v)) / 4) : 0u;
    if (lowznib > 3) {
        lowznib = 3;
    }
    v >>= lowznib * 4;
...
inline std::enable_if_t < traits::supported > denc(const T &o,
        size_t &p,
        uint64_t f = 0)
{
    if constexpr
...
template < typename T, is_const_iterator It, typename traits =
           denc_traits < T >> requires traits::supported inline void denc(T &o,
                   It &p,
                   uint64_t
                   features = 0)
{
...
has_legacy_denc::value > denc(T &o, ceph::buffer::list::const_iterator &p)
{
...
//
template < typename A >
struct denc_traits <std::basic_string < char, std::char_traits < char >,
           A >> {
private:
...
public:
    ...
    static void encode(const value_type &s, It &p, uint64_t f = 0)
    {
        denc((uint32_t) s.size(), p);
        memcpy(p.get_pos_add(s.size()), s.data(), s.size());
    }
    template < class It >
    static void decode(value_type &s, It &p, uint64_t f = 0)
    {
        uint32_t len;
        denc(len, p);
...
template <> struct denc_traits <ceph::buffer::ptr > {
    ...
    static constexpr bool bounded = false;
    static constexpr bool need_contiguous = false;
    static void bound_encode(const ceph::buffer::ptr &v, size_t &p,
                             uint64_t f = 0)
    {
    ...
    static void decode(ceph::buffer::ptr &v,
                       ceph::buffer::list::const_iterator &p)
    {
        uint32_t len;
        denc(len, p);
...
template <> struct denc_traits <ceph::buffer::list > {
    ...
    static constexpr bool bounded = false;
    static constexpr bool need_contiguous = false;
    static void bound_encode(const ceph::buffer::list &v, size_t &p,
                             uint64_t f = 0)
    {
        p += sizeof(uint32_t) + v.length();
    } static void encode(const ceph::buffer::list &v,
                         ceph::buffer::list::contiguous_appender &p,
                         uint64_t f = 0)
    {
        denc((uint32_t) v.length(), p);
    ...
    static void decode(ceph::buffer::list &v,
...
struct denc_traits < std::pair < A, B >,
           std::enable_if_t < denc_supported < std::remove_const_t <
           A >> &&denc_supported < B >>> {
    ...
    static void decode(std::pair < A, B > &v,
                       ceph::buffer::ptr::const_iterator &p, uint64_t f = 0)
    {
    ...
    !need_contiguous > decode(std::pair < A, B > &v, ceph::buffer::list::const_iterator &p,
                              uint64_t f = 0)
    {
...
namespace _denc
{
template < template < class ... >class C, typename Details,
           typename ... Ts > struct container_base {
private:
...
public:
    ...
    static void bound_encode(const container &s, size_t &p,
                             uint64_t f = 0)
    {
        p += sizeof(uint32_t);
...
template < typename T > class container_has_reserve
{
    ...
    template < typename U >
    static std::true_type test(SFINAE_match <
...
public:
    static constexpr bool value =
        decltype(test < denc_traits < T >> (0))::value;
};
template < typename T >
...
template < typename Container > struct container_details_base {
    ...
    {
        if constexpr
        (container_has_reserve_v < Container >) {
            c.reserve(s);
        }
    }
};
...
struct denc_traits <std::list < T, Ts ... >,
           typename std::enable_if_t < denc_traits <
           T >::supported >>: public _denc::container_base < std::list,
           _denc::pushback_details < std::list < T, Ts ... >>, T, Ts ... > {
};

template < typename T, typename ... Ts >
struct denc_traits <std::vector < T, Ts ... >,
           typename std::enable_if_t < denc_traits <
           T >::supported >>: public _denc::container_base < std::vector,
           _denc::pushback_details < std::vector < T, Ts ... >>, T, Ts ... > {
};

template < typename T, std::size_t N, typename ... Ts >
struct denc_traits <boost::container::small_vector < T, N, Ts ... >,
...
public:
    ...
    template < typename U = T >
    static void bound_encode(const container &s, size_t &p, uint64_t f =
                                 0)
    {
        p += sizeof(uint32_t);
        if constexpr(traits::bounded) {
...
template < typename T, typename ... Ts >
struct denc_traits <std::set < T, Ts ... >,
           std::enable_if_t < denc_traits <
           T >::supported >>: public _denc::container_base < std::set,
           _denc::setlike_details < std::set < T, Ts ... >>, T, Ts ... > {
};

template < typename T, typename ... Ts >
struct denc_traits <boost::container::flat_set < T, Ts ... >,
           std::enable_if_t < denc_traits <
           T >::supported >>: public _denc::container_base < boost::container::flat_set,
           _denc::setlike_details < boost::container::flat_set < T, Ts ... >>, T,
           Ts ... > {
};
...
struct denc_traits < std::map < A, B, Ts ... >,
           std::enable_if_t < denc_traits < A >::supported &&
           denc_traits < B >::supported >> : public _denc::container_base < std::map,
           _denc::maplike_details < std::map < A, B, Ts ... >>, A, B, Ts ... > {
};

template < typename A, typename B, typename ... Ts >
struct denc_traits < boost::container::flat_map < A, B, Ts ... >,
           std::enable_if_t < denc_traits < A >::supported &&
           denc_traits < B >::supported >> : public _denc::container_base <
...
struct denc_traits <std::array < T, N >,
...
public:
    ...
    !need_contiguous > decode(container &s, ceph::buffer::list::const_iterator &p)
    {
        for (auto &e : s) {
            denc(e, p);
        }
...
struct denc_traits < std::tuple < Ts ... >,
           ...
           static std::enable_if_t < denc_traits < U >::featured > bound_encode(const container &s, size_t &p, uint64_t f)
{
    ceph::for_each(s, [&p, f](const auto & e) {
        if constexpr(denc_traits < std::decay_t <
                     decltype(e) >>::featured) {
            denc(e, p, f);
        } else {
            denc(e, p);
...
//
template < typename T >
struct denc_traits <boost::optional < T >,
           std::enable_if_t < denc_traits < T >::supported >> {
    using traits = denc_traits < T >;
    ...
    static void decode(boost::optional < T > &v,
                       ceph::buffer::ptr::const_iterator &p, uint64_t f = 0)
    {
        bool x;
        denc(x, p, f);
...
//
template < typename T >
struct denc_traits <std::optional < T >,
           std::enable_if_t < denc_traits < T >::supported >> {
    using traits = denc_traits < T >;
    ...
    static void decode(std::optional < T > &v,
                       ceph::buffer::ptr::const_iterator &p, uint64_t f = 0)
    {
        bool x;
        denc(x, p, f);
    ...
    template < typename U = T >
    static std::enable_if_t < !!sizeof(U) &&
    !need_contiguous > decode(std::optional < T > &v, ceph::buffer::list::const_iterator &p)
    {
        bool x;
        denc(x, p);
        if (x) {
...
template < typename T, typename traits = denc_traits < T >>
constexpr std::enable_if_t < traits::supported
&& traits::bounded, size_t > encoded_sizeof_bounded()
{
    size_t p = 0;
...
template < typename T, typename traits = denc_traits < T >>
inline std::enable_if_t < traits::supported
&& !traits::featured > encode(const T &o, ceph::buffer::list &bl,
...
template < typename T,
           typename traits = denc_traits < T >>
inline std::enable_if_t < traits::supported
&& !traits::need_contiguous > decode(T &o, ::ceph::buffer::list::const_iterator &p)
{
    if (p.end()) {
...
template < typename T,
           typename traits = denc_traits < T >>
inline std::enable_if_t < traits::supported
&& traits::need_contiguous > decode(T &o,
                                    ceph::buffer::list::
                                    const_iterator &p)
{
    if (p.end()) {
...
template < typename T, typename traits = denc_traits < T >>
inline std::enable_if_t < traits::supported &&
!traits::featured > encode_nohead(const T &o,
                                  ceph::buffer::list &bl)
{
    size_t len = 0;
    traits::bound_encode(o, len);
...
template < typename T, typename traits = denc_traits < T >>
inline std::enable_if_t < traits::supported
&& !traits::featured > decode_nohead(size_t num, T &o,
                                     ceph::buffer::list::
                                     const_iterator &p)
{
    if (!num) {
    ...
    if constexpr
    (traits::need_contiguous) {
        ceph::buffer::ptr tmp;
        auto t = p;
        if constexpr
        (denc_traits < typename T::value_type >::bounded) {
            size_t element_size = 0;
            typename T::value_type v;
            denc_traits < typename T::value_type >::bound_encode(v,
                    element_size);
