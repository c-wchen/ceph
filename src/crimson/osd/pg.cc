...
PG::interruptible_future < > PG::BackgroundProcessLock::lock() noexcept
{
    return interruptor::make_interruptible(mutex.lock());
}

template < class Ret, class SuccessFunc, class FailureFunc >
PG::do_osd_ops_iertr::future < PG::pg_rep_op_fut_t < Ret>> PG::do_osd_ops_execute(
...
PG::do_osd_ops_iertr::future < PG::pg_rep_op_fut_t < MURef<MOSDOpReply>>> PG::do_osd_ops(
    Ref < MOSDOp > m,
...
PG::do_osd_ops_iertr::future < MURef < MOSDOpReply>> PG::log_reply(
    Ref < MOSDOp > m,
    const std::error_code& e)
{
...
PG::do_osd_ops_iertr::future < PG::pg_rep_op_fut_t < >> PG::do_osd_ops(
    ObjectContextRef obc,
    std::vector < OSDOp > & ops,
    const OpInfo &op_info,
