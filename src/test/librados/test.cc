                            ...
                            uint32_t pg_num)
{
    ...
    if (ret < 0) {
        rados_shutdown(*cluster);
        std::ostringstream oss;
        oss << "rados_ioctx_create(" << pool_name << ") failed with error " <<
            ret;
        return oss.str();
    }
...
int destroy_ruleset(rados_t * cluster, std::string ruleset, std::ostream & oss)
{
    char *cmd[2];
    ...
    int ret =
        rados_mon_command(*cluster, (const char **)cmd, 1, "", 0, NULL, 0, NULL,
                          0);
    if (ret)
        oss << "rados_mon_command: osd crush rule rm " + ruleset +
            " failed with error " << ret;
    return ret;
}

int destroy_ec_profile_and_ruleset(rados_t * cluster,
                                   std::string ruleset, std::ostream & oss)
{
    int ret;
                               ...
                               std::string > &config)
{
    ...
    if (ret) {
        cluster.shutdown();
        std::ostringstream oss;
        oss << "cluster.pool_create(" << pool_name << ") failed with error " <<
            ret;
        return oss.str();
    }
...
int destroy_ruleset_pp(Rados & cluster, std::string ruleset, std::ostream & oss)
{
    bufferlist inbl;
    int ret =
        cluster.mon_command("{\"prefix\": \"osd crush rule rm\", \"name\":\"" +
                            ruleset + "\"}", inbl, NULL, NULL);
    if (ret)
        oss << "mon_command: osd crush rule rm " + ruleset +
                          ...
                          std::ostream & oss)
{
    ...
    int ret =
        ...
        ("{\"prefix\": \"osd erasure-code-profile rm\", \"name\": \"testprofile-"
         ...
         inbl, NULL, NULL);
    if (ret)
        oss << "mon_command: osd erasure-code-profile rm testprofile-" <<
            pool_name << " failed with error " << ret << std::endl;
    return ret;
}
                                  ...
                                  Rados & cluster)
{
    ...
    if (ret) {
        cluster.shutdown();
        oss << "mon_command erasure-code-profile set name:testprofile-" <<
            pool_name << " failed with error " << ret;
        return oss.str();
    }

    ret =
        cluster.mon_command("{\"prefix\": \"osd pool create\", \"pool\": \"" +
                            pool_name +
                            "\", \"pool_type\":\"erasure\", \"pg_num\":8, \"pgp_num\":8, \"erasure_code_profile\":\"testprofile-"
