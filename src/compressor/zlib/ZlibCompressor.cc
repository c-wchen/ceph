...
int ZlibCompressor::zlib_compress(const bufferlist & in, bufferlist & out)
{
    ...
    strm.opaque = Z_NULL;
    ret =
        deflateInit2(&strm, cct->_conf->compressor_zlib_level, Z_DEFLATED,
                     ZLIB_DEFAULT_WIN_SIZE, ZLIB_MEMORY_LEVEL,
                     Z_DEFAULT_STRATEGY);
    if (ret != Z_OK) {
...
int ZlibCompressor::decompress(bufferlist::iterator & p, size_t compressed_size,
                               bufferlist & out)
{
    int ret;
    unsigned have;
