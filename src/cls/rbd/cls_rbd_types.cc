...
namespace cls {
    namespace rbd {
        ...
        void MirrorPeer::encode(bufferlist & bl) const {
            ...
            o.push_back(new
                        MirrorPeer("uuid-123", "cluster name", "client name",
                                   123));
        } bool MirrorPeer::operator==(const MirrorPeer & rhs) const {
            return (uuid == rhs.uuid &&
                    cluster_name == rhs.cluster_name &&
                    client_name == rhs.client_name && pool_id == rhs.pool_id);
        ...
        void MirrorImage::dump(Formatter * f) const {
            ...
            o.push_back(new
                        MirrorImage("uuid-123", MIRROR_IMAGE_STATE_ENABLED));
            o.push_back(new
                        MirrorImage("uuid-abc", MIRROR_IMAGE_STATE_DISABLING));
        }
        ...
        std::ostream & operator<<(std::ostream & os,
            ...
            os << "[" << "global_image_id=" << mirror_image.
        ...
        void MirrorImageStatus::dump(Formatter * f) const {
            ...
            o.push_back(new
                        MirrorImageStatus(MIRROR_IMAGE_STATUS_STATE_REPLAYING));
            o.push_back(new
                        MirrorImageStatus(MIRROR_IMAGE_STATUS_STATE_ERROR,
                                          "error"));
        }
        ...
        class GetTypeVisitor:public boost::static_visitor <
        ...
        SnapshotNamespaceType SnapshotNamespaceOnDisk::get_namespace_type() const {
            return static_cast < SnapshotNamespaceType >
                (boost::apply_visitor(GetTypeVisitor(), snapshot_namespace));
        } void SnapshotNamespaceOnDisk::encode(bufferlist & bl) const {
            ENCODE_START(1, 1, bl);
            boost::apply_visitor(EncodeSnapshotNamespaceVisitor(bl),
                                 snapshot_namespace);
            ENCODE_FINISH(bl);
            ...
            boost::apply_visitor(DecodeSnapshotNamespaceVisitor(p),
                                 snapshot_namespace);
            DECODE_FINISH(p);
        }
        ...
        void SnapshotNamespaceOnDisk::dump(Formatter * f) const {
            boost::
                apply_visitor(DumpSnapshotNamespaceVisitor
                              (f, "snapshot_namespace_type"),
                              snapshot_namespace);
        } void SnapshotNamespaceOnDisk::generate_test_instances(std::list <
        ...
        void TrashImageSpec::dump(Formatter * f) const {
            ...
            case TRASH_IMAGE_SOURCE_USER:
                ...
                break;
                case TRASH_IMAGE_SOURCE_MIRRORING:f->dump_string("source",
                                                                 "rbd_mirror");
