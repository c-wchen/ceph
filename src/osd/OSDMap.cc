...
class OSDTreeFormattingDumper: public CrushTreeDumper::
    FormattingDumper
{
public:
    ...
    const
{
    ...
    for (unsigned ps = 0;
        ...
        tmp_osd_map.pg_to_up_acting_osds(pg,
                                         ...
                                         &acting_prim);
        if (cct != nullptr) {
            ldout(cct, 20) << __func__ << " " << pg << " up " << up << " primary " << primary << " acting_primary " << acting_prim
                           << dendl;
        }
        if (!up.empty()) {
            // up can be empty is test generated files
...
float OSDMap::calc_deviations(CephContext *cct, const map < int, set < pg_t >> &pgs_by_osd,
                              ...
                              multimap < float, int > &deviation_osd, float &stddev)  // return current max deviation
{
    //
...
public:
        ...
        max_var(-1), stddev(0), sum(0)
    {
        ...
        } else if (osdmap->crush->
                   class_exists(filter)) {
            // filter by device class
            class_id =
                osdmap->crush->get_class_id(filter);
        } else if (auto pool_id =
                       osdmap->
...
class OSDUtilizationPlainDumper: public OSDUtilizationDumper <
    TextTable >
{
public:
