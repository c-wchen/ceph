...
TEST(ReadAttrs, Unfiltered)
{
    ...
    auto r =
        run(new RGWSimpleRadosReadAttrsCR(dpp(), store, { pool, oid }, &attrs,
                                          true));
    ASSERT_EQ(0, r);
...
TEST(Read, Dne)
{
    ...
    std::string result;
    auto r =
        run(new RGWSimpleRadosReadCR(dpp(), store, { pool, "doesnotexist" },
                                     &result, false));
    ASSERT_EQ(-ENOENT, r);
}
...
int main(int argc, const char **argv)
{
    ...
    store =
        ...
        rgw::sal::RadosStore *
        >(DriverManager::
          get_storage(dpp(), g_ceph_context, cfg, false, false, false, false,
                      false, true, false));
    if (!store) {
        std::cerr << "couldn't init storage provider" << std::endl;
        return 5;               //EIO
    }
    StoreDestructor store_destructor(static_cast <
