...
template < class Key, class Value, bool Ordered > class SimpleLRU {
    ...
    using map_type =
        map_t < Key, std::pair < Value, typename list_type::iterator >>;
    list_type lru;
    map_type cache;
  ...
  :    cache(size), max_size(size) {
    ...
    std::optional < std::enable_if < Ordered,
        Value >> lower_bound(const Key & key);
...
template < class Key, class Value, bool Ordered >
    std::optional < Value > SimpleLRU < Key, Value,
    Ordered >::find(const Key & key)
{
...
template < class Key, class Value, bool Ordered >
    Value SimpleLRU < Key, Value, Ordered >::_lru_add(typename SimpleLRU < Key,
                                                      Value,
                                                      Ordered >::map_type::
                                                      iterator found)
{
