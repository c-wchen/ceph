...
namespace recovery {
    ...
    class PG:public boost::intrusive_ref_counter <
        ...
        PeeringState::PeeringListener, DoutPrefixProvider {
      ...
      public:
        ...
        // EpochSource epoch_t get_osdmap_epoch() const final {
        ...
        // PeeringListener void prepare_write(
                                                 pg_info_t & info,
        ...
        } void send_cluster_message(int osd, MessageURef m,
        ...
        std::vector < pg_shard_t > get_replica_recovery_order()const final {
            ...
            std::ignore = shard_services.local_request_reservation(pgid,
                                                                   on_grant ?
                                                                   make_lambda_context
                                                                   ([this,
                                                                     on_grant =
                                                                     ...
                                                                     (on_grant)]
                                                                    (int) {
          start_peering_event_operation(std::move(*on_grant));}):  nullptr,
                                                                   priority,
                                                                   on_preempt ?
                                                                   make_lambda_context
                                                                   ([this,
        ...
        void request_remote_recovery_reservation(unsigned priority,
            ...
            // TODO -- we probably want to add a mechanism for blocking on this
            // after handling the peering event
            std::ignore = shard_services.remote_request_reservation(pgid,
                                                                    on_grant ?
                                                                    make_lambda_context
                                                                    ([this,
                                                                      on_grant =
        ...
        void schedule_event_on_commit(ceph::os::Transaction & t,
                                      PGPeeringEventRef on_commit) final {
            t.register_on_commit(make_lambda_context([this, on_commit =
                                                      std::
                                                      move(on_commit)] (int) {
        ...
        PGLog::LogEntryHandlerRef get_log_handler(ceph::os::
                                                  Transaction & t) final {
            return std::make_unique < PG::PGLogEntryHandler > (this, &t);
        }

        void rebuild_missing_set_with_deletes(PGLog & pglog) final {
            pglog.rebuild_missing_set_with_deletes_crimson(shard_services.
                                                           get_store(),
                                                           coll_ref,
                                                           peering_state.
                                                           get_info()).get();
        ...
        /// initialize created PG void init(
                                               int role,
        ...
        seastar::future <> read_state(crimson::os::FuturizedStore::Shard *
                                      store);

        interruptible_future <> do_peering_event(PGPeeringEvent & evt,
                                                 PeeringCtx & rctx);
        ...
        using load_obc_ertr = crimson::errorator <
            crimson::ct_error::enoent, crimson::ct_error::object_corrupted >;
        using load_obc_iertr =::crimson::interruptible::
            interruptible_errorator <::crimson::osd::IOInterruptCondition,
            load_obc_ertr >;
        using interruptor =::crimson::interruptible::interruptor <::crimson::
            osd::IOInterruptCondition >;

      public:
        using with_obc_func_t =
            std::function < load_obc_iertr::future <> (ObjectContextRef) >;

        load_obc_iertr::future <> with_locked_obc(const hobject_t & hobj,
        ...
        void replica_clear_repop_obc(const std::vector < pg_log_entry_t >
                                     &logv);
        void handle_rep_op_reply(const MOSDRepOpReply & m);
        interruptible_future <> do_update_log_missing(Ref <
                                                      MOSDPGUpdateLogMissing >
                                                      m,
                                                      crimson::net::
                                                      ConnectionRef conn);
        interruptible_future <> do_update_log_missing_reply(Ref <
                                                            MOSDPGUpdateLogMissingReply
                                                            > m);

        void print(std::ostream & os) const;
        void dump_primary(Formatter *);
        seastar::future <> submit_error_log(Ref < MOSDOp > m,
                                            const OpInfo & op_info,
      ...
      private:
        ...
        using do_osd_ops_iertr =::crimson::interruptible::
            interruptible_errorator <::crimson::osd::IOInterruptCondition,::
            crimson::errorator < crimson::ct_error::eagain >>;
        template < typename Ret = void > using pg_rep_op_fut_t =
            std::tuple < interruptible_future <>,
            do_osd_ops_iertr::future < Ret >>;
        do_osd_ops_iertr::future < pg_rep_op_fut_t < MURef <
            MOSDOpReply >>> do_osd_ops(Ref < MOSDOp > m,
        ...
        using do_osd_ops_failure_func_t =
            std::function <
            do_osd_ops_iertr::future <> (const std::error_code &)>;
        struct do_osd_ops_params_t;
        do_osd_ops_iertr::future <
            pg_rep_op_fut_t <>> do_osd_ops(ObjectContextRef obc,
                                           std::vector < OSDOp > &ops,
                                           const OpInfo & op_info,
                                           const do_osd_ops_params_t
                                           && params,
                                           do_osd_ops_success_func_t
                                           success_func,
                                           do_osd_ops_failure_func_t
                                           failure_func);
        template < class Ret, class SuccessFunc,
            class FailureFunc > do_osd_ops_iertr::future < pg_rep_op_fut_t <
            Ret >> do_osd_ops_execute(seastar::lw_shared_ptr < OpsExecuter > ox,
                                      std::vector < OSDOp > &ops, SuccessFunc
                                      && success_func, FailureFunc
                                      && failure_func);
        interruptible_future < MURef < MOSDOpReply >> do_pg_ops(Ref < MOSDOp >
                                                                m);
        std::tuple < interruptible_future <>,
            interruptible_future <>> submit_transaction(ObjectContextRef
                                                        && obc,
                                                        ceph::os::Transaction
                                                        && txn, osd_op_params_t
                                                        && oop,
                                                        ...
                                                        pg_log_entry_t >
                                                        &&log_entries);
        interruptible_future <> repair_object(const hobject_t & oid,
                                              eversion_t & v);
      ...
      public:
        ...
        int get_recovery_op_priority() const {
            ...
            return pri >
                0 ? pri : crimson::common::local_conf()->
      ...
      private:
        ...
        bool can_discard_replica_op(const Message & m,
                                    epoch_t m_map_epoch)const;
        bool can_discard_op(const MOSDOp & m) const;
        void context_registry_on_change();
        bool is_missing_object(const hobject_t & soid) const {
            return peering_state.get_pg_log().get_missing().get_items().
                count(soid);
        } bool is_unreadable_object(const hobject_t & oid, eversion_t * v =
                                    0)const final {
            return is_missing_object(oid)
                || !peering_state.get_missing_loc().readable_with_acting(oid,
                                                                         get_actingset
                                                                         (), v);
    ...
    struct PG::do_osd_ops_params_t {
        ...
        // Only used by ExecutableMessagePimpl entity_name_t get_source() const {
