...
namespace librbd {
    ...
    template < typename I >
        ManagedLock < I >::ManagedLock(librados::IoCtx & ioctx,
                                       AsioEngine & asio_engine,
                                       const string & oid, Watcher * watcher,
                                       Mode mode, bool blocklist_on_break_lock,
                                       uint32_t blocklist_expire_seconds)
    :m_lock(ceph::
            make_mutex(unique_lock_name
                       ("librbd::ManagedLock<I>::m_lock", this))),
        m_ioctx(ioctx), m_cct(reinterpret_cast < CephContext * >(ioctx.cct())),
        m_asio_engine(asio_engine), m_work_queue(asio_engine.get_work_queue()),
        m_oid(oid), m_watcher(watcher), m_mode(mode),
    ...
    template < typename I >
        ...
        int r;
        {
            ...
            if (is_state_shutdown()) {
                ...
                auto req =
                    managed_lock::GetLockerRequest < I >::create(m_ioctx, m_oid,
                                                                 m_mode ==
                                                                 EXCLUSIVE,
                                                                 locker,
                                                                 on_finish);
    ...
    template < typename I >
        ...
        int r;
        {
            ...
            if (is_state_shutdown()) {
                ...
                on_finish = new C_Tracked(m_async_op_tracker, on_finish);
                auto req =
                    managed_lock::BreakRequest < I >::create(m_ioctx,
                                                             m_asio_engine,
                                                             m_oid, locker,
                                                             m_mode ==
                                                             EXCLUSIVE,
    ...
    template < typename I > int ManagedLock < I >::assert_header_locked() {
        ...
        if (r < 0) {
            if (r == -EBLOCKLISTED) {
                ldout(m_cct,
                      5) << "client is not lock owner -- client blocklisted" <<
                    dendl;
            }
            else if (r == -ENOENT) {
                ldout(m_cct,
                ...
                lderr(m_cct) << "failed to verify lock ownership: " <<
                    cpp_strerror(r)
                    << dendl;
            }
    ...
    template < typename I >
        void ManagedLock < I >::post_acquire_lock_handler(int r,
                                                          Context * on_finish) {
        on_finish->complete(r);
    }
    ...
    template < typename I >
        void ManagedLock < I >::post_release_lock_handler(bool shutting_down,
                                                          int r,
                                                          Context * on_finish) {
        on_finish->complete(r);
    }
    ...
    template < typename I >
        void ManagedLock < I >::post_reacquire_lock_handler(int r,
                                                            Context *
                                                            on_finish) {
        on_finish->complete(r);
    }
    ...
    template < typename I > bool ManagedLock < I >::is_transition_state()const {
        ...
        case STATE_PRE_RELEASING:
            return true;
            case STATE_UNLOCKED:case STATE_LOCKED:case STATE_SHUTDOWN:case
                STATE_UNINITIALIZED:break;
        } return false;
    }
    ...
    template < typename I >
      ...
      for (auto & action_ctxs:m_actions_contexts) {
        ...
        m_actions_contexts.push_back( {
                                     action, std::move(contexts)}
        );
    }
    ...
    template < typename I >
        typename ManagedLock < I >::Action ManagedLock <
        I >::get_active_action()const {
        ceph_assert(ceph_mutex_is_locked(m_lock));
        ceph_assert(!m_actions_contexts.empty());
        return m_actions_contexts.front().first;
    } template < typename I >
        void ManagedLock < I >::complete_active_action(State next_state,
                                                       int r) {
        ceph_assert(ceph_mutex_is_locked(m_lock));
        ceph_assert(!m_actions_contexts.empty());
    ...
    template < typename I > void ManagedLock < I >::send_acquire_lock() {
        ...
        if (watch_handle == 0) {
            if (m_watcher->is_blocklisted()) {
                lderr(m_cct) << "watcher not registered - client blocklisted" <<
                    dendl;
                complete_active_action(STATE_UNLOCKED, -EBLOCKLISTED);
            }
            else {
                lderr(m_cct) << "watcher not registered - delaying request" <<
                    dendl;
    ...
    template < typename I >
        ...
        AcquireRequest < I > *req =
            AcquireRequest < I >::create(m_ioctx, m_watcher, m_asio_engine,
                                         m_oid, m_cookie, m_mode == EXCLUSIVE,
                                         m_blocklist_on_break_lock,
                                         m_blocklist_expire_seconds,
                                         create_context_callback < ManagedLock <
                                         I >,
                                         &ManagedLock <
                                         I >::handle_acquire_lock > (this));
        m_work_queue->queue(new C_SendLockRequest < AcquireRequest < I >> (req),
                            0);
    }
    ...
    template < typename I > void ManagedLock < I >::handle_acquire_lock(int r) {
        ...
        if (r == -EBUSY || r == -EAGAIN || r == -EROFS) {
            ...
            lderr(m_cct) << "failed to acquire exclusive lock: " <<
                cpp_strerror(r)
                << dendl;
        }
        else {
    ...
    template < typename I >
        ...
        ReleaseRequest < I > *req =
            ReleaseRequest < I >::create(m_ioctx, m_watcher, m_work_queue,
                                         m_oid, m_cookie,
                                         new LambdaContext([this, r] (int ret) {
                                                           ...
                                                           (STATE_UNLOCKED,
                                                            r);}));
        m_work_queue->queue(new C_SendLockRequest < ReleaseRequest <
                            I >> (req));
    }
    ...
    template < typename I > void ManagedLock < I >::send_reacquire_lock() {
        ...
        if (m_cookie == m_new_cookie && m_blocklist_on_break_lock) {
            ...
            auto ctx = create_context_callback <
                ManagedLock,
                &ManagedLock < I >::handle_no_op_reacquire_lock > (this);
        ...
        ReacquireRequest < I > *req =
            ReacquireRequest < I >::create(m_ioctx, m_oid, m_cookie,
                                           m_new_cookie, m_mode == EXCLUSIVE,
                                           ctx);
        m_work_queue->queue(new C_SendLockRequest < ReacquireRequest <
                            I >> (req));
    }
    ...
    template < typename I > void ManagedLock < I >::handle_reacquire_lock(int r) {
        ...
        if (r < 0) {
            if (r == -EOPNOTSUPP) {
                ...
                lderr(m_cct) << "failed to update lock cookie: " <<
                    cpp_strerror(r)
                    << dendl;
            }
    ...
    template < typename I > void ManagedLock < I >::send_release_lock() {
        ...
        m_work_queue->queue(new LambdaContext([this] (int r) {
                                              pre_release_lock_handler(false,
                                                                       create_context_callback
                                                                       <
                                                                       ManagedLock
                                                                       < I >,
    ...
    template < typename I > void ManagedLock < I >::handle_release_lock(int r) {
        ...
        m_work_queue->queue(new LambdaContext([this, r] (int ret) {
                                              post_release_lock_handler(false,
                                                                        r,
                                                                        create_context_callback
                                                                        <
                                                                        ManagedLock
