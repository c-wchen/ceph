...
enum {
    ...
    RBD_IO_OPERATIONS_ALL = (RBD_IO_OPERATION_READ |
...
typedef interval_map < uint64_t,
        SparseExtent, SparseExtentSplitMerge > SparseExtents;

typedef std::vector < uint64_t > SnapIds;

typedef std::pair < librados::snap_t,
        librados::snap_t > WriteReadSnapIds;
...
struct SparseBufferlistExtent: public SparseExtent {
    ...
    } bool operator==(const SparseBufferlistExtent &rhs) const
    {
        return (state == rhs.state &&
                length == rhs.length && bl.contents_equal(rhs.bl));
    }
};
...
struct SparseBufferlistExtentSplitMerge {
                                 ...
                                 SparseBufferlistExtent &sbe) const
    {
        ...
        return SparseBufferlistExtent(sbe.state, length,
                                      std::move(bl));
                                   ...
                                   && right) const
    {
        if (left.state == SPARSE_EXTENT_STATE_DATA) {
            ...
            return SparseBufferlistExtent(left.state,
...
typedef std::map < uint64_t,
        SparseBufferlist > SnapshotSparseBufferlist;
...
struct ReadExtent {
    ...
    ReadExtent(uint64_t offset,
        ...
        length(length),
        buffer_extents(buffer_extents), bl(bl), extent_map(extent_map)
    {
    };
