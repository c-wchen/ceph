...
/// label for block device struct bluestore_bdev_label_t {
...
WRITE_CLASS_ENCODER(bluestore_bdev_label_t)
    ...
    std::ostream & operator<<(std::ostream & out,
                              const bluestore_bdev_label_t & l);
...
struct bluestore_pextent_t:public bluestore_interval_t < uint64_t, uint32_t > {
    ...
    bluestore_pextent_t(const bluestore_interval_t &
                        ext):bluestore_interval_t(ext.offset, ext.length) {
    }
...
struct bluestore_blob_use_tracker_t {
    ...
    } bluestore_blob_use_tracker_t(const bluestore_blob_use_tracker_t &
                                   tracker);
    bluestore_blob_use_tracker_t & operator=(const bluestore_blob_use_tracker_t
                                             & rhs);
    ~bluestore_blob_use_tracker_t() {
        clear();
    }
...
bool can_split() const {
    return !has_flag(FLAG_SHARED) && !has_flag(FLAG_COMPRESSED) && !has_flag(FLAG_HAS_UNUSED);  // splitting unused set is complex
} bool can_split_at(uint32_t blob_offset) const {
...
bool is_mutable() const {
  ...
  /// return chunk (i.e. min readable block) size for the blob
    ...
    return has_csum() ?
        std::max < uint64_t > (dev_block_size,
...
bool is_allocated(uint64_t b_off, uint64_t b_len) const {
  ...
  /// return true if the entire range is unallocated/// (not mapped to extents on disk)
...
template < class F, typename std::enable_if < std::is_invocable_r_v <
    int,
    F, uint64_t, uint64_t >>::type * = nullptr > int map_f_invoke(uint64_t lo,
                                                                  const
                                                                  bluestore_pextent_t
                                                                  & p,
                                                                  uint64_t o,
    ...
    F,
    uint64_t,
    uint64_t, uint64_t >>::type * = nullptr > int map_f_invoke(uint64_t lo,
                                                               const
                                                               bluestore_pextent_t
                                                               & p,
                                                               uint64_t o,
...
void add_tail(uint32_t new_len) {
    ...
    if (has_csum()) {
        ...
        t.swap(csum_data);
        csum_data =
            ceph::buffer::create(get_csum_value_size() * logical_length /
                                 get_csum_chunk_size());
...
bool release_extents(bool all,
                     const PExtentVector & logical, PExtentVector * r);
};
...
WRITE_CLASS_DENC(bluestore_shared_blob_t)
    ...
    std::ostream & operator<<(std::ostream & out,
                              const bluestore_shared_blob_t & o);
...
struct bluestore_onode_t {
    ...
    // mempool to be assigned to buffer::ptr manually
     std::map < mempool::bluestore_cache_meta::string,
        ceph::buffer::ptr > attrs;
         ...
         DENC(shard_info, v, p)
{
    ...
    bool is_pgmeta_omap() const {
        ...
        set_flag(FLAG_OMAP |
                 (legacy ? 0 : (FLAG_PERPOOL_OMAP | FLAG_PERPG_OMAP)));
    }
    ...
    void clear_omap_flag() {
        clear_flag(FLAG_OMAP |
                   FLAG_PGMETA_OMAP | FLAG_PERPOOL_OMAP | FLAG_PERPG_OMAP);
    }
...
void dump(ceph::Formatter * f) const;
static void generate_test_instances(std::list <
                                    bluestore_compression_header_t * >&o);
};
    ...
    class ref_counter_2hash_tracker_t
{
  ...
  public:
    ...
    size_t get_num_buckets() const {
        ...
        buckets1[h] += n;
        h = ceph_str_hash_linux((const char *)hash_val,
                                hash_val_len) % num_buckets;
        if (buckets2[h] == 0 && n) {
            ++num_non_zero;
        }
    ...
    bool test_hash_conflict(const char *hash_val1,
                            const char *hash_val2, size_t hash_val_len) const {
...
class shared_blob_2hash_tracker_t:public ref_counter_2hash_tracker_t <
    ...
    } inline hash_input_t build_hash_input(uint64_t sbid,
  ...
  :    ref_counter_2hash_tracker_t(mem_cap) {
    ...
    bool test_hash_conflict(uint64_t sbid,
                            uint64_t offset,
                            uint64_t sbid2, uint64_t offset2) const;
    bool test_all_zero(uint64_t sbid, uint64_t offset) const;
    bool test_all_zero_range(uint64_t sbid,
                             uint64_t offset, uint32_t len) const;
};
...
struct sb_info_space_efficient_map_t {
    ...
    auto find(uint64_t id) {
        if (items.size() != 0) {
            ...
            if (aux_items.size() != 0) {
                auto it = std::lower_bound(aux_items.begin(),
                                           aux_items.end(),
                                           id,
                                           [](const sb_info_t & a,
                                              const uint64_t & b){
  ...
  private:
    sb_info_t & _add(int64_t id) {
        ...
        it = std::upper_bound(aux_items.begin(),
                              aux_items.end(),
                              n_id,[](const uint64_t & a, const sb_info_t & b) {
                              return a < b.get_sbid();}
        );
