...
std::string LogEvent::get_type_str() constconst
{
    switch (_type) {
...
LogEvent *LogEvent::decode_event(bufferlist & bl, bufferlist::iterator & p,
                                 LogEvent::EventType type)
{
    ...
    default:
        generic_dout(0) << "uh oh, unknown log event type " << type <<
            " length " << length << dendl;
        return NULL;
    }
