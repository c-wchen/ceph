...
:epoch(rhs.epoch),
    ...
    standby_daemons(rhs.standby_daemons), standby_epochs(rhs.standby_epochs) {
    filesystems.clear();
...
void reset_filesystem(fs_cluster_id_t fscid);
   ...
   */
void modify_filesystem(const fs_cluster_id_t fscid,
                       std::function < void (std::shared_ptr < Filesystem >) >
                       fn) {
   ...
   */
void modify_daemon(mds_gid_t who,
...
} const std::shared_ptr < Filesystem > &get_filesystem(fs_cluster_id_t fscid) {
...
} std::shared_ptr < const Filesystem > get_filesystem(void) const {
...
int parse_filesystem(boost::string_view ns_str,
                     std::shared_ptr < const Filesystem > *result) const;

int parse_role(boost::string_view role_str,
