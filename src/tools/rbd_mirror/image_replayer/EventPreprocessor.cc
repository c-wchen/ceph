    ...
    namespace mirror {
        namespace image_replayer {
            ...
            :m_local_image_ctx(local_image_ctx),
                ...
                I >::preprocess(EventEntry * event_entry, Context * on_finish) {
                assert(!m_in_progress);
                m_in_progress = true;
            ...
            template < typename I >
                ...
                if (snap_id_it == m_local_image_ctx.snap_ids.end()) {
                    dout(20) << ": cannot map remote snapshot '" << event.
                        src_snap_name << "' " << "to local snapshot" << dendl;
            ...
            template < typename I >
                ...
                for (auto it = snap_seqs->begin(); it != snap_seqs->end();) {
                    auto current_it(it++);
                    if (m_local_image_ctx.snap_info.count(current_it->second) ==
                        0) {
...
template class rbd::mirror::image_replayer::EventPreprocessor <
    librbd::ImageCtx >;
