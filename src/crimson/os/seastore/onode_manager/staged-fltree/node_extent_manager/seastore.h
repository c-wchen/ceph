...
public:
    ...
    constexpr static extent_types_t TYPE =
...
{
public:
    TransactionManagerHandle(TransactionManager &tm): tm {
        tm}
    {
    }
    TransactionManager &tm;
...
public:
    SeastoreNodeExtentManager(TransactionManager &tm, laddr_t min,
                              double p_eagain)
        : TransactionManagerHandle(tm), addr_min {
        min}, p_eagain {
        p_eagain}
    {
        if constexpr
...
protected:
        ...
        (INJECT_EAGAIN) {
            if (trigger_eagain()) {
                SUBDEBUGT(seastore_onode,
                          "reading at {:#x}: trigger eagain", t, addr);
                t.test_set_conflict();
                return read_iertr::make_ready_future < NodeExtentRef > ();
            }
        }
        return tm.read_extent < SeastoreNodeExtent > (t,
                addr).si_then([addr,
                               &t]
                              (auto
                               && e)->
    ...
    override
    {
        ...
        (INJECT_EAGAIN) {
            if (trigger_eagain()) {
                SUBDEBUGT(seastore_onode, "allocating {}B: trigger eagain",
                          t, len);
                t.test_set_conflict();
                return alloc_iertr::make_ready_future < NodeExtentRef > ();
            }
        }
        return tm.alloc_extent < SeastoreNodeExtent > (t, hint,
                len).si_then([len,
                              &t]
                             (auto
        extent) {
            ...
            tracker) override
    {
        ...
        return tm.read_onode_root(t).
        si_then([this, &t, &tracker](auto root_addr) {
            SUBTRACET(seastore_onode, "got root {:#x}", t,
                      root_addr);
            return Super::
                   URef(new SeastoreSuper(t, tracker, root_addr, tm));
        }
                      );
    }
...
template <> struct fmt::formatter <
    crimson::os::seastore::onode::SeastoreNodeExtent > : fmt::ostream_formatter {
};
#endif
