...
#define LARGE_ENOUGH 2048
    ...
    // all chunks are available
    {
        ...
        EXPECT_EQ(0,
                  clay._decode(set < int >(want_to_decode, want_to_decode + 2),
                               encoded, &decoded));
    ...
    for (int i = 1; i < 4; i++) {
        for (int j = 0; j < i; j++) {
            ...
            EXPECT_EQ(0,
                      memcmp(decoded[j].c_str(), encoded[j].c_str(), length));
            EXPECT_EQ(0,
                      memcmp(decoded[i].c_str(), encoded[i].c_str(), length));
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (map < int, vector < pair < int, int >>>::iterator h =
            ...
            for (vector < pair < int, int >>::iterator ind = h->second.begin();
                ...
                bufferlist temp;
                temp.substr_of(encoded[h->first], ind->first * sc_size,
                               ind->second * sc_size);
...
#define LARGE_ENOUGH 2048
    ...
    // all chunks are available
    {
        ...
        map < int, bufferlist > decoded;
        EXPECT_EQ(0,
                  clay._decode(set < int >(want_to_decode, want_to_decode + 3),
                               encoded, &decoded));
    ...
    for (int i = 2; i < 6; i++) {
        for (int j = 1; j < i; j++) {
            for (int k = 0; k < j; k++) {
                ...
                EXPECT_EQ(length, decoded[j].length());
                EXPECT_EQ(0,
                          memcmp(decoded[k].c_str(), encoded[k].c_str(),
                                 length));
                EXPECT_EQ(0,
                          memcmp(decoded[j].c_str(), encoded[j].c_str(),
                                 length));
                EXPECT_EQ(0,
                          memcmp(decoded[i].c_str(), encoded[i].c_str(),
                                 length));
            }
        }
    }
...
TEST(ErasureCodeClay, encode_decode_shortening_case)
{
    ...
    if (in.length() < length) {
        ...
        EXPECT_EQ(0, memcmp(encoded[1].c_str(), in.c_str() + length, length));
        EXPECT_EQ(0,
                  memcmp(encoded[2].c_str(), in.c_str() + 2 * length, length));
        EXPECT_EQ(0,
                  memcmp(encoded[3].c_str(), in.c_str() + 3 * length,
                         in.length() - 3 * length));
    }
    ...
    // all chunks are available
    {
        ...
        map < int, bufferlist > decoded;
        EXPECT_EQ(0,
                  clay._decode(set < int >(want_to_decode, want_to_decode + 4),
                               encoded, &decoded));
        EXPECT_EQ(4u, decoded.size());
        EXPECT_EQ(length, decoded[0].length());
        EXPECT_EQ(0, memcmp(decoded[0].c_str(), encoded[0].c_str(), length));
...
TEST(ErasureCodeClay, encode)
{
    ...
    unsigned aligned_object_size = clay.get_chunk_size(1) * 2 * 2;
    {
        ...
        EXPECT_EQ(0,
                  clay.encode(set < int >(want_to_encode, want_to_encode + 4),
                              in, &encoded));
        EXPECT_EQ(4u, encoded.size());
...
TEST(ErasureCodeClay, create_rule)
{
    ...
    for (int h = 0; h < num_host; ++h) {
        ...
        for (int o = 0; o < num_osd; ++o, ++osd) {
            c->insert_item(g_ceph_context, osd, 1.0,
    ...
    {
        stringstream ss;
        ErasureCodeClay clay(g_conf().get_val < std::string >
                             ("erasure_code_dir"));
        ErasureCodeProfile profile;
        profile["k"] = "2";
        profile["m"] = "2";
