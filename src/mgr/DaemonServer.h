...
class DaemonServer:public Dispatcher, public md_config_obs_t {
  protected:
    ...
    static const MonCommand *_get_mgrcommand(const std::string & cmd_prefix,
                                             ...
                                             &commands);
    bool _allowed_command(MgrSession * s, const std::string & service,
  ...
  private:
    ...
    void _check_offlines_pgs(const std::set < int >&osds,
                             const OSDMap & osdmap,
                             const PGMap & pgmap, offline_pg_report * report);
    void _maximize_ok_to_stop_set(const set < int >&orig_osds,
                                  unsigned max,
                                  const OSDMap & osdmap,
                                  const PGMap & pgmap,
  ...
  public:
    ...
    DaemonServer(MonClient * monc_,
                 ...
                 LogChannelRef cl, LogChannelRef auditcl);
    ...
    void fetch_missing_metadata(const DaemonKey & key,
