...
struct btree_test_base:public seastar_test_suite_t, SegmentProvider,
    JournalTrimmer {
    ...
    bool try_reserve_inline_usage(std::size_t) final {
        return true;
    }
    ...
    segment_id_t allocate_segment(segment_seq_t seq,
                                  segment_type_t type,
    ...
    seastar::future <> submit_transaction(TransactionRef t) {
        auto record =
            cache->prepare_record(*t, JOURNAL_SEQ_NULL, JOURNAL_SEQ_NULL);
        return journal->submit_record(std::move(record),
                                      t->get_handle()).safe_then([this, t =
                                                                  std::
                                                                  move(t)] (auto
                                                                            submit_result)
                                                                 mutable {
    ...
    seastar::future <> set_up_fut()final {
        ...
        return segment_manager->init().safe_then([this] {
                    ...
                    next = segment_id_t {
                    segment_manager->get_device_id(), 0};
                    ...
                    0,
                    paddr_t::
                    make_seg_paddr(segment_id_t
                                   (segment_manager->get_device_id(), 0), 0)};
                    return epm->open_for_write();}
        ).safe_then([this] {
                    return seastar::do_with(cache->
                                            ...
                                            return with_trans_intr(*ref_t,
                                                                   ...
                                                                   si_then([this, &t] {
                                                                           return
                                                                           test_structure_setup
                                                                           (t);}
                                                                   );}
                                            ).safe_then([this, &ref_t] {
                                                        return
                                                        submit_transaction(std::
                                                                           move
                                                                           (ref_t));}
                                            );}
                    );}
    ...
    seastar::future <> tear_down_fut()final {
        return cache->close().safe_then([this] {
                    ...
                    sms.reset(); journal.reset(); epm.reset(); cache.reset();}
        ).handle_error(crimson::ct_error::all_same_way([] {
                                                       ASSERT_FALSE
                                                       ("Unable to close");}
                       )
...
struct lba_btree_test:btree_test_base {
    ...
    template < typename F > auto lba_btree_update(F && f) {
        auto tref =
            cache->create_transaction(Transaction::src_t::MUTATE,
                                      "test_btree_update", false);
        auto & t = *tref;
        with_trans_intr(t,
                        [this, tref = std::move(tref), f =
                         std::forward < F > (f)] (auto & t) mutable {
                        return cache->get_root(t).
                        si_then([f = std::move(f), &t] (RootBlockRef croot) {
                                return seastar::do_with(LBABtree(croot),
                                                        [f =
                                                         std::move(f),
                                                         &t] (auto & btree)
                                                        mutable {
                                                        return std::invoke(std::
                                                                           move
                                                                           (f),
                                                                           btree,
                                                                           t);}
    ...
    void insert(laddr_t addr, extent_len_t len) {
        ...
        lba_btree_update([ =, this] (auto & btree, auto & t) {
                         auto extent = cache->alloc_new_extent < TestBlock > (t,
                                                                              TestBlock::
                                                                              SIZE,
                                                                              placement_hint_t::
                                                                              HOT,
                                                                              ...
                                                                              get_paddr
                                                                              ());
                         return btree.insert(get_op_context(t), addr,
                                             get_map_val(len), extent.get()
                         ).si_then([addr, extent] (auto p) {
                                   auto &[iter, inserted] = p;
                                   assert(inserted); extent->set_laddr(addr);}
                         );}
        );
    }
    ...
    void remove(laddr_t addr) {
        ...
        check.erase(iter++);
        lba_btree_update([ =, this] (auto & btree, auto & t) {
                         return btree.lower_bound(get_op_context(t),
                                                  addr).si_then([this, len,
                                                                 addr, &btree,
                                                                 &t] (auto iter)
                                                                {
                                                                EXPECT_FALSE
...
struct btree_lba_manager_test:              btree_test_base
                                             ...
                                             void complete_commit(Transaction &
                                                                  t) final {
                                             }

                                             LBAManager::
                                             mkfs_ret
                                             test_structure_setup(Transaction &
                                                                  t) final {
                                             lba_manager.
                                             reset(new BtreeLBAManager(*cache));
                                             ...
                                             cache->alloc_new_extent <
                                             TestBlockPhysical > (*t.t,
                                                                  TestBlockPhysical::
                                                                  SIZE,
                                                                  placement_hint_t::
                                                                  HOT, 0);};
                                             ...
                                             create_transaction(Transaction::
                                                                ...
                                                                "test_read_weak",
                                                                true),
                                             test_lba_mappings}; return t;}

                                             void
                                             submit_test_transaction
                                             (test_transaction_t t) {
                                             submit_transaction(std::move(t.t)).
                                             get();
                                             test_lba_mappings.swap(t.
                                                                    mappings);}

                                             auto get_overlap(test_transaction_t
                                                              & t, laddr_t addr,
                                                              size_t len) {
                                             auto bottom =
                                             t.mappings.upper_bound(addr);
                                             if (bottom != t.mappings.begin())
                                             -- bottom;
                                             if (bottom != t.mappings.end()
                                                 && bottom->first +
                                                 bottom->second.len <= addr)
                                             ++ bottom;
                                             auto top =
                                             t.mappings.lower_bound(addr + len);
                                             return std::make_pair(bottom,
                                                                   top);}
                                             ...
                                             auto
                                             alloc_mapping(test_transaction_t &
                                                           t, laddr_t hint,
                                                           size_t len) {
                                             auto ret =
                                             with_trans_intr(*t.t,
                                             ...
                                             logger().debug("alloc'd: {}",
                                                            *ret);
                                             EXPECT_EQ(len, ret->get_length());
                                             auto[b, e] =
                                             get_overlap(t, ret->get_key(),
                                                         len); EXPECT_EQ(b, e);
                                             t.mappings.
                                             emplace(std::
                                                     make_pair(ret->get_key(),
                                                               test_extent_t {
                                                               ret->get_val(),
                                                               ret->
                                                               get_length(), 1}
                                             ...
                                             auto
                                             decref_mapping(test_transaction_t &
                                                            t, laddr_t addr) {
                                             return decref_mapping(t,
                                                                   t.mappings.
                                                                   find(addr));}
                                             ...
                                             ceph_assert(target !=
                                                         t.mappings.end());
                                             ceph_assert(target->second.
                                                         refcount > 0);
                                             target->second.refcount--;
                                             (void)with_trans_intr(*t.t,
                                                                   [ =,
                                                                    this] (auto
                                                                           & t)
                                                                   {
                                             ...
                                             auto
                                             incref_mapping(test_transaction_t &
                                                            t, laddr_t addr) {
                                             return incref_mapping(t,
                                                                   t.mappings.
                                                                   find(addr));}
      ...
      std::vector < laddr_t > addresses; addresses.reserve(test_lba_mappings.size()); for (auto & i:test_lba_mappings)
                                             {
                                             ...
                                             get_mapped_addresses
                                             (test_transaction_t & t) {
      std::vector < laddr_t > addresses; addresses.reserve(t.mappings.size()); for (auto & i:t.mappings)
                                             {
                                             addresses.push_back(i.first);}
                                             return addresses;}
                                             ...
                                             void
                                             check_mappings(test_transaction_t &
                                                            t) {
                                             (void)with_trans_intr(*t.t,
                                                                   [ =,
                                                                    this] (auto
                                                                   ...
                                                                   return
                                                                   lba_manager->
                                                                   check_child_trackers
                                                                   (t);}
      ).unsafe_get0(); for (auto && i:      t.mappings)
                                             {
                                             auto laddr = i.first;
                                             ...
                                             TEST_F(btree_lba_manager_test,
                                                    basic) {
                                             run_async([this] {
                                                       laddr_t laddr =
                                                       0x12345678 * block_size;
                                                       {
                                                       // write initial mapping
                                                       auto t = create_transaction(); check_mappings(t);    // check in progress transaction sees mapping
                                                       check_mappings();    // check concurrent does not
                                                       auto ret =
                                                       alloc_mapping(t, laddr,
                                                                     block_size);
                                                       submit_test_transaction
                                                       (std::move(t));}
                                             ...
                                             TEST_F(btree_lba_manager_test,
                                                    single_transaction_split_merge)
                                             {
                                             run_async([this] {
                                                       {
                                                       auto t =
