...
class DecoratedApplier: public rgw::auth::IdentityApplier
{
    ...
    DecorateeT >::type DerefedDecorateeT;
    ...
    } bool is_identity(const boost::container::flat_set < Principal >
                       &ids)const override
    {
...
public:
    ...
    template < typename U > ThirdPartyAccountApplier(rgw::sal::Driver *driver,
            const rgw_user &acct_user_override,
            U && decoratee)
        : DecoratedApplier < T > (std::move(decoratee)),
          driver(driver), acct_user_override(acct_user_override)
    {
    }
...
template < typename T >
void ThirdPartyAccountApplier <
T >::to_str(std::ostream &out) const
{
    out << "rgw::auth::ThirdPartyAccountApplier(" +
        acct_user_override.to_str() + ")" << " -> ";
    DecoratedApplier < T >::to_str(out);
} template < typename T >
void ThirdPartyAccountApplier <
T >::load_acct_info(const DoutPrefixProvider *dpp,
                    RGWUserInfo &user_info) const
{
    if (UNKNOWN_ACCT == acct_user_override) {
        ...
        if (acct_user_override.tenant.empty()) {
            const rgw_user tenanted_uid(acct_user_override.id,
                                        acct_user_override.id);
...
ThirdPartyAccountApplier < T > add_3rdparty(rgw::sal::Driver *driver,
        const rgw_user &acct_user_override, T && t)
{
...
public:
    template < typename U > SysReqApplier(CephContext *const cct,
                                          rgw::sal::Driver *driver,
                                          const req_state *const s, U && decoratee)
        : DecoratedApplier < T > (std::forward < T > (decoratee)),
          cct(cct),
          driver(driver),
          args(s->info.args), is_system(boost::logic::indeterminate)
    {
    }
                    ...
                    RGWUserInfo &user_info) const
{
    ...
    if (is_system) {
        ...
        rgw_user effective_uid(args.
                               sys_get(RGW_SYS_PARAM_PREFIX "uid"));
...
template < typename T >
void SysReqApplier <
T >::modify_request_state(const DoutPrefixProvider *dpp,
