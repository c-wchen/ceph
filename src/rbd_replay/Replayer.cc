...
void Worker::start()
{
    m_thread =
        ...
        (new boost::thread(boost::bind(&Worker::run, this)));
}
...
bool Worker::readonly() const const
{
    return m_replayer.readonly();
}

rbd_loc Worker::map_image_name(string image_name, string snap_name) const const
{
...
void Replayer::wait_for_actions(const action::Dependencies & deps)
{
    ...
    if (release_time > boost::get_system_time()) {
        dout(SLEEP_LEVEL) << "Sleeping for " << (release_time -
                                                 boost::get_system_time()).
            total_microseconds() << " microseconds" << dendl;
...
bool Replayer::readonly() const const
{
    return m_readonly;
}
...
string Replayer::pool_name() const const
{
    return m_pool_name;
}
