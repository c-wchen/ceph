...
namespace librbd {
    ...
    template <> struct Journal <librbd::MockTestImageCtx > {
    ...
    Journal < librbd::MockTestImageCtx > *Journal <
        librbd::MockTestImageCtx >::s_instance = nullptr;

    template <> struct MirroringWatcher <librbd::MockTestImageCtx > {
    ...
    MirroringWatcher < librbd::MockTestImageCtx > *MirroringWatcher <
        librbd::MockTestImageCtx >::s_instance = nullptr;
        ...
        class TestMockMirrorDisableRequest:public TestMockFixture {
          public:
            ...
            void expect_get_mirror_image(MockTestImageCtx & mock_image_ctx,
                                         const cls::rbd::
                                         MirrorImage & mirror_image, int r) {
                bufferlist bl;
                    ...
                    .WillOnce(DoAll(SetArgPointee < 0 > (is_primary),
                                    ...
                                    (CompleteContext
                                     (r,
                                      ...
                                      op_work_queue))));
            } void expect_set_mirror_image(MockTestImageCtx & mock_image_ctx,
                                           int r) {
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                            exec(RBD_MIRRORING, _, StrEq("rbd"),
                                 StrEq("mirror_image_set"), _, _, _))
                    .WillOnce(Return(r));
            } void expect_remove_mirror_image(MockTestImageCtx & mock_image_ctx,
                                              int r) {
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                            exec(RBD_MIRRORING, _, StrEq("rbd"),
                                 StrEq("mirror_image_remove"), _, _, _))
                    .WillOnce(Return(r));
            }
            ...
            void expect_notify_image_updated(MockTestImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(mock_mirroring_watcher,
                            notify_image_updated(state, mock_image_ctx.id,
                                                 global_id, _))
                    .WillOnce(WithArg < 3 >
                              (CompleteContext
