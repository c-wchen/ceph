...
namespace {
    ...
    TracepointProvider::Traits tracepoint_traits("librados_tp.so",
                                                 "rados_tracing");
...
class ObjectOpCompletionCtx:public Context {
    ...
    bufferlist bl;
  public:
    explicit ObjectOpCompletionCtx(librados::ObjectOperationCompletion *
                                   c):completion(c) {
...
void librados::ObjectReadOperation::stat2(uint64_t * psize,
                                          struct timespec *pts, int *prval)
{
    ::ObjectOperation * o = &impl->o;
...
void librados::ObjectReadOperation::omap_get_vals(const std::
                                                  string & start_after,
                                                  const std::
...
void librados::ObjectReadOperation::omap_get_vals2(const std::
                                                   string & start_after,
                                                   const std::
                                                   string & filter_prefix,
                                                   uint64_t max_return,
...
void librados::ObjectReadOperation::omap_get_keys(const std::
                                                  string & start_after,
                                                  uint64_t max_return,
                                                  std::set < std::string >
                                                  *out_keys, int *prval)
...
void librados::ObjectReadOperation::omap_get_keys2(const std::
                                                   string & start_after,
                                                   uint64_t max_return,
                                                   std::set < std::string >
                                                   *out_keys, bool * pmore,
...
void librados::ObjectReadOperation::omap_get_vals_by_keys(const std::set <
                                                          std::string > &keys,
                                                          std::map <
                                                          std::string,
                                                          bufferlist > *map,
...
void librados::ObjectOperation::omap_cmp(const std::map < std::string,
...
void librados::ObjectReadOperation::list_watchers(list < obj_watch_t >
                                                  *out_watchers, int *prval)
{
    ::ObjectOperation * o = &impl->o;
    o->list_watchers(out_watchers, prval);
}

void librados::ObjectReadOperation::list_snaps(snap_set_t * out_snaps,
                                               int *prval)
{
    ::ObjectOperation * o = &impl->o;
...
int librados::IoCtx::omap_get_vals2(const std::string & oid,
                                    const std::string & start_after,
                                    const std::string & filter_prefix,
                                    uint64_t max_return,
                                    std::map < std::string,
                                    bufferlist > *out_vals, bool * pmore)
{
    ObjectReadOperation op;
    int r;
    op.omap_get_vals2(start_after, filter_prefix, max_return, out_vals, pmore,
                      &r);
    bufferlist bl;
    int ret = operate(oid, &op, &bl);
    if (ret < 0)
...
void librados::ObjectReadOperation::getxattrs(map < string,
                                              bufferlist > *pattrs, int *prval)
{
    ::ObjectOperation * o = &impl->o;
...
void librados::ObjectWriteOperation::create(bool exclusive, const std::string & category)   // unused
{
    ::ObjectOperation * o = &impl->o;
    o->create(exclusive);
}
...
void librados::ObjectWriteOperation::omap_set(const map < string,
                                              bufferlist > &map)
{
    ::ObjectOperation * o = &impl->o;
    o->omap_set(map);
...
void librados::ObjectWriteOperation::omap_rm_keys(const std::set < std::string >
                                                  &to_rm)
{
    ::ObjectOperation * o = &impl->o;
...
set_alloc_hint(uint64_t expected_object_size, uint64_t expected_write_size)
{
    ::ObjectOperation * o = &impl->o;
...
void librados::ObjectWriteOperation::
set_alloc_hint2(uint64_t expected_object_size, uint64_t expected_write_size,
                uint32_t flags)
{
    ::ObjectOperation * o = &impl->o;
...
librados::NObjectIteratorImpl & librados::NObjectIteratorImpl::
operator=(const librados::NObjectIteratorImpl & rhs)
{
    if (&rhs == this)
        return *this;
    if (rhs.ctx.get() == NULL) {
    ...
    Objecter::NListContext * list_ctx =
        new Objecter::NListContext(*rhs.ctx->nlc);
    ctx.reset(new ObjListCtx(rhs.ctx->ctx, list_ctx));
    cur_obj = rhs.cur_obj;
    return *this;
}

bool librados::NObjectIteratorImpl::operator==(const librados::
                                               NObjectIteratorImpl & rhs) const
    const
{
...
const librados::ListObject & librados::NObjectIteratorImpl::operator*() const const
{
    return cur_obj;
}
...
{
    uint32_t r =
        rados_nobjects_list_seek_cursor(ctx.get(),
...
uint32_t librados::NObjectIteratorImpl::get_pg_hash_position() const const
{
...
librados::NObjectIterator & librados::NObjectIterator::
operator=(const librados::NObjectIterator & rhs)
{
    if (rhs.impl == NULL) {
        delete impl;
        impl = NULL;
...
bool librados::NObjectIterator::operator==(const librados::
                                           NObjectIterator & rhs) constconst
{
    if (impl && rhs.impl) {
...
const librados::ListObject & librados::NObjectIterator::operator*() const const
{
    assert(impl);
...
uint32_t librados::NObjectIterator::get_pg_hash_position() const const
{
    assert(impl);
    return impl->get_pg_hash_position();
}

const librados::NObjectIterator librados::NObjectIterator::
...
///////////////////////////// AioCompletion //////////////////////////////
int librados::AioCompletion::AioCompletion::set_complete_callback(void *cb_arg,
                                                                  rados_callback_t
                                                                  cb)
{
...
int librados::AioCompletion::AioCompletion::set_safe_callback(void *cb_arg,
                                                              rados_callback_t
                                                              cb)
{
    AioCompletionImpl *c = (AioCompletionImpl *) pc;
...
std::string librados::IoCtx::get_pool_name() constconst
{
    return io_ctx_impl->get_cached_pool_name();
}

uint64_t librados::IoCtx::get_instance_id() const const
{
...
int librados::IoCtx::create(const std::string & oid, bool exclusive, const std::string & category)  // unused
{
    object_t obj(oid);
    return io_ctx_impl->create(obj, exclusive);
}
...
int librados::IoCtx::sparse_read(const std::string & oid, std::map < uint64_t,
                                 uint64_t > &m, bufferlist & bl, size_t len,
                                 uint64_t off)
{
    object_t obj(oid);
...
int librados::IoCtx::getxattrs(const std::string & oid, map < std::string,
                               bufferlist > &attrset)
{
    object_t obj(oid);
...
int librados::IoCtx::stat2(const std::string & oid, uint64_t * psize,
                           struct timespec *pts)
{
    object_t obj(oid);
...
int librados::IoCtx::omap_get_keys2(const std::string & oid,
                                    const std::string & start_after,
                                    uint64_t max_return,
                                    std::set < std::string > *out_keys,
                                    bool * pmore)
...
{
    object_t obj(oid);
    return io_ctx_impl->operate(obj, &o->impl->o,
                                 ...
                                 ObjectWriteOperation * o, int flags)
{
    object_t obj(oid);
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc,
                                    io_ctx_impl->snapc, translate_flags(flags));
}
...
int librados::IoCtx::selfmanaged_snap_set_write_ctx(snap_t seq,
                                                    vector < snap_t > &snaps)
{
    vector < snapid_t > snv;
    snv.resize(snaps.size());
...
int librados::IoCtx::snap_rollback(const std::string & oid,
                                   const char *snapname)
{
...
int librados::IoCtx::selfmanaged_snap_rollback(const std::string & oid,
                                               uint64_t snapid)
{
...
int librados::IoCtx::aio_unlock(const std::string & oid,
                                const std::string & name,
...
int librados::IoCtx::list_lockers(const std::string & oid,
                                  const std::string & name, int *exclusive,
                                  std::string * tag,
                                  std::list < librados::locker_t > *lockers)
{
    std::list < librados::locker_t > tmp_lockers;
    map < rados::cls::lock::locker_id_t,
        rados::cls::lock::locker_info_t > rados_lockers;
    std::string tmp_tag;
    ClsLockType tmp_type;
    int r =
        rados::cls::lock::get_lock_info(this, oid, name, &rados_lockers,
                                        &tmp_type, &tmp_tag);
    if (r < 0)
        return r;
...
librados::NObjectIterator librados::IoCtx::
nobjects_begin(const bufferlist & filter)
{
...
librados::NObjectIterator librados::IoCtx::
nobjects_begin(const ObjectCursor & cursor)
{
    bufferlist bl;
...
librados::NObjectIterator librados::IoCtx::
nobjects_begin(const ObjectCursor & cursor, const bufferlist & filter)
{
    rados_list_ctx_t listh;
    rados_nobjects_list_open(io_ctx_impl, &listh);
    NObjectIterator iter((ObjListCtx *) listh);
...
int librados::IoCtx::aio_read(const std::string & oid,
                              librados::AioCompletion * c, bufferlist * pbl,
...
int librados::IoCtx::aio_sparse_read(const std::string & oid,
                                     librados::AioCompletion * c,
                                     std::map < uint64_t, uint64_t > *m,
...
int librados::IoCtx::aio_write(const std::string & oid,
                               librados::AioCompletion * c,
...
int librados::IoCtx::aio_write_full(const std::string & oid,
                                    librados::AioCompletion * c,
                                    const bufferlist & bl)
{
    object_t obj(oid);
...
int librados::IoCtx::aio_writesame(const std::string & oid,
                                   librados::AioCompletion * c,
...
int librados::IoCtx::aio_remove(const std::string & oid,
                                librados::AioCompletion * c)
{
...
int librados::IoCtx::aio_remove(const std::string & oid,
                                librados::AioCompletion * c, int flags)
{
...
struct AioGetxattrDataPP {
    AioGetxattrDataPP(librados::AioCompletionImpl * c,
                      bufferlist * _bl):bl(_bl), completion(c) {
    } bufferlist *bl;
...
int librados::IoCtx::aio_getxattr(const std::string & oid,
                                  librados::AioCompletion * c, const char *name,
                                  bufferlist & bl)
{
...
int librados::IoCtx::aio_stat(const std::string & oid,
                              librados::AioCompletion * c, uint64_t * psize,
                              time_t * pmtime)
{
    object_t obj(oid);
...
int librados::IoCtx::aio_watch(const string & oid, AioCompletion * c,
                               uint64_t * cookie, librados::WatchCtx2 * ctx2)
{
    object_t obj(oid);
...
int librados::IoCtx::get_object_hash_position2(const std::string & oid,
                                               uint32_t * hash_position)
{
    return io_ctx_impl->get_object_hash_position(oid, hash_position);
}

int librados::IoCtx::get_object_pg_hash_position2(const std::string & oid,
...
int librados::Rados::conf_read_file(const char *const path) const const
{
    return rados_conf_read_file((rados_t) client, path);
}

int librados::Rados::conf_parse_argv(int argc, const char **argv) const const
{
...
int librados::Rados::conf_parse_argv_remainder(int argc, const char **argv,
                                               const char **remargv) const const
{
    return rados_conf_parse_argv_remainder((rados_t) client, argc, argv,
                                           remargv);
}

int librados::Rados::conf_parse_env(const char *name) const const
{
...
int librados::Rados::service_daemon_register(const std::string & service,   ///< service name (e.g., 'rgw')
...
int librados::Rados::service_daemon_update_status(const std::map < std::string,
                                                  std::string > &status)
{
...
int librados::Rados::pool_create_async(const char *name,
...
int librados::Rados::pool_create_async(const char *name, uint64_t auid,
                                       PoolAsyncCompletion * c)
{
    string str(name);
...
int librados::Rados::pool_create_async(const char *name, uint64_t auid,
                                       __u8 crush_rule, PoolAsyncCompletion * c)
{
    string str(name);
...
{
    tracepoint(librados, rados_pool_get_base_tier_enter, (rados_t) client,
               pool_id);
...
int librados::Rados::pool_delete_async(const char *name,
                                       PoolAsyncCompletion * c)
{
...
int librados::Rados::pool_list2(std::list < std::pair < int64_t,
                                std::string > >&v)
{
...
int librados::Rados::osd_command(int osdid, std::string cmd,
...
int librados::Rados::pg_command(const char *pgstr, std::string cmd,
                                const bufferlist & inbl, bufferlist * outbl,
                                std::string * outs)
{
    vector < string > cmdvec;
...
int librados::Rados::get_pool_stats(std::list < string > &v, string & category, // unused
                                    std::map < string, stats_map > &result)
{
    return -EOPNOTSUPP;
}

bool librados::Rados::get_pool_is_selfmanaged_snaps_mode(const std::
                                                         string & pool)
{
...
namespace {
    ...
    int get_inconsistent_pgs(librados::RadosClient & client,
        ...
        "{\"prefix\": \"pg ls\","
                ...
                "\"states\": [\"inconsistent\"]," "\"format\": \"json\"}"};
...
int librados::Rados::get_inconsistent_objects(const PlacementGroup & pg,
                                              ...
                                              unsigned max_return,
                                              AioCompletion * c,
                                              std::vector < inconsistent_obj_t >
                                              *objects, uint32_t * interval)
{
    IoCtx ioctx;
                                                                 ...
                                                                 cb_safe)
{
    AioCompletionImpl *c;
    int r =
        rados_aio_create_completion(cb_arg, cb_complete, cb_safe, (void **)&c);
    assert(r == 0);
    return new AioCompletion(c);
}
...
extern "C" int rados_cluster_fsid(rados_t cluster, char *buf, size_t maxlen)
{
...
extern "C" int rados_ioctx_pool_stat(rados_ioctx_t io,
...
extern "C" int rados_pool_create_with_crush_rule(rados_t cluster,
                                                 const char *name,
                                                 __u8 crush_rule_num)
{
...
extern "C" int rados_pool_get_base_tier(rados_t cluster, int64_t pool_id,
                                        int64_t * base_tier)
{
...
extern "C" int rados_ioctx_pool_get_auid(rados_ioctx_t io, uint64_t * auid)
{
    ...
    librados::IoCtxImpl * ctx = (librados::IoCtxImpl *) io;
    int retval =
        ctx->client->pool_get_auid(ctx->get_id(), (unsigned long long *)auid);
...
extern "C" int rados_ioctx_get_pool_name(rados_ioctx_t io, char *s,
                                         unsigned maxlen)
{
...
extern "C" int rados_ioctx_snap_get_stamp(rados_ioctx_t io, rados_snap_t id,
                                          time_t * t)
{
...
extern "C" int rados_setxattr(rados_ioctx_t io, const char *o, const char *name,
                              const char *buf, size_t len)
{
...
extern "C" int rados_object_list_is_end(rados_ioctx_t io,
...
extern "C" void rados_object_list_cursor_free(rados_ioctx_t io,
                                              rados_object_list_cursor cur)
{
    hobject_t *hobj = (hobject_t *) cur;
    delete hobj;
...
extern "C" int rados_object_list_cursor_cmp(rados_ioctx_t io,
                                 ...
                                 rados_object_list_cursor * next)
{
    ...
    C_SaferCond cond;
    ctx->objecter->enumerate_objects(ctx->poolid,
                                     ctx->oloc.nspace,
                                     *((hobject_t *) start),
                                     *((hobject_t *) finish),
                                     result_item_count,
                                     filter_bl, &result, &next_hash, &cond);
...
extern "C" void rados_object_list_free(const size_t result_size,
...
extern "C" uint32_t rados_nobjects_list_get_pg_hash_position(rados_list_ctx_t
                                                             listctx)
{
    librados::ObjListCtx * lh = (librados::ObjListCtx *) listctx;
                                        ...
                                        const char **nspace)
{
    ...
    if (h->list.empty()) {
        ...
        if (h->list.empty()) {
            tracepoint(librados, rados_nobjects_list_next_exit, -ENOENT, NULL,
                       NULL, NULL);
            return -ENOENT;
        }
    }
 ...
 */
extern "C" int rados_objects_list_open(rados_ioctx_t io, rados_list_ctx_t * ctx)
{
    return -ENOTSUP;
}

extern "C" uint32_t rados_objects_list_get_pg_hash_position(rados_list_ctx_t
                                                            ctx)
{
    return 0;
}

extern "C" uint32_t rados_objects_list_seek(rados_list_ctx_t ctx, uint32_t pos)
{
    return 0;
}

extern "C" int rados_objects_list_next(rados_list_ctx_t ctx,
                                       const char **entry, const char **key)
{
    return -ENOTSUP;
}

extern "C" void rados_objects_list_close(rados_list_ctx_t ctx)
{
}
...
extern "C" int rados_aio_watch(rados_ioctx_t io, const char *o,
                               ...
                               rados_watcherrcb_t watcherrcb, void *arg)
{
    return rados_aio_watch2(io, o, completion, handle, watchcb, watcherrcb, 0,
                            arg);
}
...
extern "C" int rados_aio_watch_flush(rados_t cluster,
                                     rados_completion_t completion)
{
...
extern "C" int rados_aio_unlock(rados_ioctx_t io, const char *o,
                                const char *name, const char *cookie,
                                rados_completion_t completion)
{
...
extern "C" void rados_write_op_assert_version(rados_write_op_t write_op,
                                              uint64_t ver)
{
...
extern "C" void rados_write_op_assert_exists(rados_write_op_t write_op)
{
    ...
    ((::ObjectOperation *) write_op)->stat(NULL, (ceph::real_time *) NULL,
                                           NULL);
...
extern "C" void rados_write_op_create(rados_write_op_t write_op, int exclusive, const char *category)   // unused
{
                                          ...
                                          time_t * mtime, int flags)
{
    ...
    librados::AioCompletionImpl * c =
        (librados::AioCompletionImpl *) completion;
    int retval =
        ctx->aio_operate(obj, oo, c, ctx->snapc, translate_flags(flags));
...
extern "C" void rados_read_op_assert_version(rados_read_op_t read_op,
                                             uint64_t ver)
{
...
extern "C" void rados_read_op_getxattrs(rados_read_op_t read_op,
                                        rados_xattrs_iter_t * iter, int *prval)
{
...
const std::string & librados::ListObject::get_nspace() const const
{
    return impl->get_nspace();
}

const std::string & librados::ListObject::get_oid() const const
{
    return impl->get_oid();
}

const std::string & librados::ListObject::get_locator() const const
{
...
librados::ObjectCursor & librados::ObjectCursor::operator=(const librados::
                                                           ObjectCursor & rhs)
{
...
bool librados::ObjectCursor::operator<(const librados::ObjectCursor & rhs) constconst
{
    const hobject_t lhs_hobj =
        (c_cursor == nullptr) ? hobject_t() : *((hobject_t *) c_cursor);
    const hobject_t rhs_hobj =
        (rhs.c_cursor ==
         nullptr) ? hobject_t() : *((hobject_t *) (rhs.c_cursor));
...
{
    hobject_t *h =
        new hobject_t(io_ctx_impl->objecter->enumerate_objects_begin());
    ObjectCursor oc;
...
{
    hobject_t *h =
        new hobject_t(io_ctx_impl->objecter->enumerate_objects_end());
    librados::ObjectCursor oc;
    oc.set((rados_object_list_cursor) h);
    return oc;
}
...
string librados::ObjectCursor::to_str() const const
{
    stringstream ss;
...
CEPH_RADOS_API std::ostream & librados::operator<<(std::ostream & os,
                                                   const librados::
                                                   ObjectCursor & oc)
{
    if (oc.c_cursor) {
                                 ...
                                 ObjectCursor * next)
{
    ...
    io_ctx_impl->objecter->enumerate_objects(io_ctx_impl->poolid,
                                             io_ctx_impl->oloc.nspace,
                                             *((hobject_t *) start.c_cursor),
                                             *((hobject_t *) finish.c_cursor),
                                             result_item_count,
                                             filter,
                                             &obj_result, &next_hash, &cond);
    ...
    for (std::list < librados::ListObjectImpl >::iterator i =
         obj_result.begin(); i != obj_result.end(); ++i) {
        ObjectItem oi;
        oi.oid = i->oid;
        oi.nspace = i->nspace;
...
void librados::IoCtx::object_list_slice(const ObjectCursor start,
                                        const ObjectCursor finish,
                                        const size_t n,
                                        const size_t m,
