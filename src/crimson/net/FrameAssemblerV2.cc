...
void FrameAssemblerV2::intercept_frame(Tag tag, bool is_write)
{
    ...
    if (conn.interceptor) {
        ...
        auto action =
            conn.interceptor->intercept(conn, Breakpoint { tag, type }
                                       );
...
void FrameAssemblerV2::
create_session_stream_handlers(const AuthConnectionMeta &auth_meta,
                               bool crossed)
{
...
seastar::future <> FrameAssemblerV2::replace_by(FrameAssemblerV2::mover_t
        && mover)
{
    record_io = false;
    rxbuf.clear();
...
FrameAssemblerV2::record_bufs_t FrameAssemblerV2::stop_recording()
{
    ...
    record_io = false;
    return record_bufs_t {
        std::move(rxbuf), std::move(txbuf)};
}
...
void FrameAssemblerV2::
learn_socket_ephemeral_port_as_connector(uint16_t port)
{
    assert(has_socket());
...
seastar::future <> FrameAssemblerV2::replace_shutdown_socket(SocketRef
        && new_socket)
{
    assert(has_socket());
...
seastar::future < Socket::tmp_buf > FrameAssemblerV2::read_exactly(std::size_t bytes)
{
    assert(has_socket());
...
seastar::future < ceph::bufferlist > FrameAssemblerV2::read(std::size_t bytes)
{
    assert(has_socket());
    if (unlikely(record_io)) {
...
seastar::future <> FrameAssemblerV2::write(ceph::bufferlist && buf)
{
    assert(has_socket());
    if (unlikely(record_io)) {
        txbuf.append(buf);
...
seastar::future <> FrameAssemblerV2::flush()
{
    assert(has_socket());
    return socket->flush();
}

seastar::future <> FrameAssemblerV2::write_flush(ceph::bufferlist && buf)
{
    assert(has_socket());
    if (unlikely(record_io)) {
        txbuf.append(buf);
...
seastar::future < FrameAssemblerV2::read_main_t > FrameAssemblerV2::read_main_preamble()
{
    rx_preamble.clear();
    return read_exactly(rx_frame_asm.get_preamble_onwire_len()
        ...
        try {
            ...
            const Tag tag =
                rx_frame_asm.disassemble_preamble(rx_preamble);
#ifdef UNIT_TESTS_BUILT
...
> FrameAssemblerV2::read_frame_payload()
{
    ...
    , [this] {
        ...
        if (uint16_t alignment =
            rx_frame_asm.get_segment_align(seg_idx);
            alignment !=
            segment_t::DEFAULT_ALIGNMENT) {
            logger().
            trace
            ("{} cannot allocate {} aligned buffer at segment desc index {}",
