...
// Single cache shard interface.
    class CacheShard {
      public:
        ...
        virtual bool Release(rocksdb::Cache::Handle * handle, bool force_erase =
                             false) = 0;
        ...
        virtual void ApplyToAllCacheEntries(const std::function <
                                            void (const rocksdb::Slice & key,
        ...
        virtual std::string GetPrintableOptions() const {
            return "";
        } virtual DeleterFn GetDeleter(rocksdb::Cache::Handle * handle) const =
            0;
    };
...
// Keys are sharded by the highest num_shard_bits bits of hash value.
    class ShardedCache:public rocksdb::Cache, public PriorityCache::PriCache {
      public:
        ...
        virtual rocksdb::Status Insert(const rocksdb::Slice & key, void *value,
                                       size_t charge, DeleterFn,
                                       rocksdb::Cache::Handle ** handle,
...
#if (ROCKSDB_MAJOR >= 7 || (ROCKSDB_MAJOR == 6 && ROCKSDB_MINOR >= 22))
        virtual void ApplyToAllEntries(const std::function <
                                       void (const rocksdb::Slice & key,
                                             void *value, size_t charge,
