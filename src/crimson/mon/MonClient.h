...
namespace crimson::mon {
    ...
    class Client:public crimson::net::Dispatcher,
        public crimson::auth::AuthClient, public crimson::auth::AuthServer {
        ...
        // commands
        using get_version_t =
            seastar::future < std::tuple < version_t, version_t >>;

        ceph_tid_t last_version_req_id = 0;
        std::map < ceph_tid_t,
            typename get_version_t::promise_type > version_reqs;

        ceph_tid_t last_mon_command_id = 0;
        using command_result_t =
            seastar::future < std::tuple < std::int32_t, std::string,
            ceph::bufferlist >>;
      ...
      public:
        ...
        bool sub_want(const std::string & what, version_t start,
                      unsigned flags);
        void sub_got(const std::string & what, version_t have);
        void sub_unwant(const std::string & what);
        bool sub_want_increment(const std::string & what, version_t start,
                                unsigned flags);
      ...
      private:
        ...
        int handle_auth_request(crimson::net::Connection & conn,
                                AuthConnectionMeta & auth_meta, bool more,
                                uint32_t auth_method,
      ...
      private:
        ...
        seastar::future <> handle_get_version_reply(Ref < MMonGetVersionReply >
                                                    m);
...
template <> struct fmt::formatter <
    crimson::mon::Client >:fmt::ostream_formatter {
};
#endif
