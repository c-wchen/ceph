...
{
    {
        ...
        if (!m_image_ctx.snap_info.empty()) {
            m_parent_spec =
                m_image_ctx.snap_info.begin()->second.parent.spec;
        } else {
...
void DetachChildRequest < I >::clone_v2_child_detach()
{
    ...
    auto aio_comp = create_rados_callback <
                    DetachChildRequest < I >,
                    &DetachChildRequest < I >::handle_clone_v2_child_detach >
                    (this);
    r = m_parent_io_ctx.aio_operate(m_parent_header_name, aio_comp,
                                    &op);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
void DetachChildRequest < I >::handle_clone_v2_child_detach(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "error detaching child from parent: " <<
                   cpp_strerror(r)
                   << dendl;
        finish(r);
        return;
...
void DetachChildRequest < I >::clone_v2_get_snapshot()
{
    ...
    auto aio_comp = create_rados_callback <
                    DetachChildRequest < I >,
                    &DetachChildRequest < I >::handle_clone_v2_get_snapshot >
                    (this);
    int r =
        m_parent_io_ctx.aio_operate(m_parent_header_name, aio_comp, &op,
                                    &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
void DetachChildRequest < I >::handle_clone_v2_get_snapshot(int r)
{
    ...
    if (r == 0) {
        ...
        if (r == 0) {
            ...
            if (cls::rbd::
                get_snap_namespace_type(m_parent_snap_namespace) ==
...
void DetachChildRequest < I >::clone_v2_open_parent()
{
    ...
    m_parent_image_ctx->read_only_mask &=
...
void DetachChildRequest < I >::clone_v2_remove_snapshot()
{
    ...
    auto ctx = create_context_callback <
               DetachChildRequest < I >,
               &DetachChildRequest < I >::handle_clone_v2_remove_snapshot >
               (this);
...
template < typename I >
void DetachChildRequest <
I >::handle_clone_v2_remove_snapshot(int r)
{
    auto cct = m_image_ctx.cct;
    ldout(cct, 5) << "r=" << r << dendl;
...
void DetachChildRequest < I >::clone_v2_get_parent_trash_entry()
{
    ...
    auto aio_comp = create_rados_callback <
                    DetachChildRequest < I >,
                    &DetachChildRequest <
                    I >::handle_clone_v2_get_parent_trash_entry > (this);
    int r =
        m_parent_io_ctx.aio_operate(RBD_TRASH, aio_comp, &op,
                                    &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
template < typename I >
void DetachChildRequest <
I >::handle_clone_v2_get_parent_trash_entry(int r)
{
    auto cct = m_image_ctx.cct;
    ldout(cct, 5) << "r=" << r << dendl;
    ...
    if (r < 0 && r != -ENOENT) {
        ldout(cct,
              5) << "failed to get parent trash entry: " <<
                 cpp_strerror(r)
                 << dendl;
        clone_v2_close_parent();
        return;
...
void DetachChildRequest < I >::clone_v2_remove_parent_from_trash()
{
    ...
    auto ctx = create_context_callback <
               DetachChildRequest < I >,
               &DetachChildRequest <
               I >::handle_clone_v2_remove_parent_from_trash > (this);
    auto req =
        librbd::trash::RemoveRequest < I >::create(m_parent_io_ctx,
            m_parent_image_ctx,
            m_image_ctx.
            op_work_queue, false,
            m_no_op, ctx);
...
template < typename I >
void DetachChildRequest <
I >::handle_clone_v2_remove_parent_from_trash(int r)
{
    auto cct = m_image_ctx.cct;
    ldout(cct, 5) << "r=" << r << dendl;
    ...
    if (r < 0) {
        ldout(cct,
              5) << "failed to remove parent image:" << cpp_strerror(r)
                 << dendl;
    }
...
void DetachChildRequest < I >::clone_v2_close_parent()
{
    ...
    auto ctx = create_context_callback <
               DetachChildRequest < I >,
               &DetachChildRequest < I >::handle_clone_v2_close_parent >
               (this);
...
void DetachChildRequest < I >::clone_v1_remove_child()
{
    ...
    librados::ObjectWriteOperation op;
    librbd::cls_client::remove_child(&op, m_parent_spec,
                                     m_image_ctx.id);

    auto aio_comp = create_rados_callback <
                    DetachChildRequest < I >,
                    &DetachChildRequest < I >::handle_clone_v1_remove_child >
                    (this);
