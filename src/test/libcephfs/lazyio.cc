...
TEST(LibCephFS, LazyIOOneWriterMulipleReaders)
{
    ...
    /* Client a issues a write and propagates/flushes the buffer */
    ASSERT_EQ((int)sizeof(out_buf),
              ceph_write(ca, fda, out_buf, sizeof(out_buf), 10));
    ASSERT_EQ(0, ceph_lazyio_propagate(ca, fda, 0, 0));
    ...
    ASSERT_EQ(ceph_read(cb, fdb, in_buf, sizeof(in_buf), 0),
              2 * strlen(out_buf) + 1);
    ...
    ASSERT_EQ(ceph_read(ca, fda, in_buf, sizeof(in_buf), 0),
              2 * strlen(out_buf) + 1);
    ASSERT_STREQ(in_buf, "fooooooooofooooooooo");
...
TEST(LibCephFS, LazyIOMultipleWritersMulipleReaders)
{
    ...
    ASSERT_EQ((int)sizeof(wait_out_buf),
...
TEST(LibCephFS, LazyIOSynchronizeFlush)
{
    ...
    ASSERT_EQ((int)sizeof(out_buf),
              ceph_write(cb, fdb, out_buf, sizeof(out_buf), 10));
    ASSERT_EQ((int)sizeof(out_buf),
              ceph_write(cb, fdb, out_buf, sizeof(out_buf), 20));
...
TEST(LibCephFS, WithoutandWithLazyIO)
{
    ...
    ASSERT_EQ((int)sizeof(out_buf_w),
    ...
    char in_buf_w[30];
    ASSERT_EQ(ceph_read(ca, fda, in_buf_w, sizeof(in_buf_w), 0),
    ...
    /* Client a issues a write and propagates/flushes the buffer */
    ASSERT_EQ((int)sizeof(out_buf),
              ceph_write(ca, fda, out_buf, sizeof(out_buf), 20));
    ASSERT_EQ(0, ceph_lazyio_propagate(ca, fda, 0, 0));

    /* Client b issues a write and propagates/flushes the buffer */
    ASSERT_EQ((int)sizeof(out_buf),
              ceph_write(cb, fdb, out_buf, sizeof(out_buf), 30));
    ASSERT_EQ(0, ceph_lazyio_propagate(cb, fdb, 0, 0));
