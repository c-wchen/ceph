...
int RGWRestRole::parse_tags()
{
    ...
    if (keys.size() != vals.size()) {
        ldout(s->cct,
              0) << "No. of keys doesn't match with no. of values in tags" <<
                 dendl;
        return -EINVAL;
    }
    for (size_t i = 0; i < keys.size(); i++) {
        ...
        ldout(s->cct,
              0) << "Tag Key: " << keys[i] << " Tag Value is: " << vals[i] <<
                 dendl;
    }
    return 0;
}
...
int RGWCreateRole::get_params()
{
    ...
    if (role_name.empty() || trust_policy.empty()) {
        ldpp_dout(this,
                  20) <<
                      "ERROR: one of role name or assume role policy document is empty" <<
                      dendl;
        return -EINVAL;
    }
...
void RGWCreateRole::execute(optional_yield y)
{
    ...
    if (!user_tenant.empty() && role->get_tenant() != user_tenant) {
        ldpp_dout(this,
                  20) <<
                      "ERROR: the tenant provided in the role name does not match with the tenant of the user creating the role"
                      << dendl;
        op_ret = -EINVAL;
        return;
    }
    ...
    if (!driver->is_meta_master()) {
        ...
        if (!parser.init()) {
            ldpp_dout(this,
                      0) << "ERROR: failed to initialize xml parser" << dendl;
            op_ret = -EINVAL;
            return;
        }
        ...
        op_ret =
            driver->forward_iam_request_to_master(s, key, nullptr, bl_post_body,
                &parser, s->info, y);
        if (op_ret < 0) {
            ldpp_dout(this,
                      20) <<
        ...
        if (!create_role_resp_obj) {
            ldpp_dout(this,
                      5) << "ERROR: unexpected xml: CreateRoleResponse" <<
                         dendl;
            op_ret = -EINVAL;
            return;
        }
        ...
        if (!create_role_res_obj) {
            ldpp_dout(this,
                      5) << "ERROR: unexpected xml: CreateRoleResult" << dendl;
            op_ret = -EINVAL;
            return;
        }
        ...
        ldpp_dout(this,
                  0) << "role_id decoded from master zonegroup response is" <<
                     role_id << dendl;
    }
...
void RGWDeleteRole::execute(optional_yield y)
{
    ...
    if (!driver->is_meta_master()) {
        ...
        if (master_op_ret < 0) {
            ...
            ldpp_dout(this,
                      0) << "forward_iam_request_to_master returned ret=" <<
                         op_ret << dendl;
            return;
        }
    }
...
int RGWModifyRoleTrustPolicy::get_params()
{
    ...
    if (role_name.empty() || trust_policy.empty()) {
        ldpp_dout(this,
                  20) << "ERROR: One of role name or trust policy is empty" <<
                      dendl;
        return -EINVAL;
    }
    JSONParser p;
    if (!p.parse(trust_policy.c_str(), trust_policy.length())) {
        ldpp_dout(this,
                  20) << "ERROR: failed to parse assume role policy doc" <<
                      dendl;
        return -ERR_MALFORMED_DOC;
    }
...
void RGWListRoles::execute(optional_yield y)
{
    ...
    op_ret =
        driver->get_roles(s, y, path_prefix, s->user->get_tenant(), result);
...
int RGWPutRolePolicy::get_params()
{
    ...
    if (role_name.empty() || policy_name.empty() || perm_policy.empty()) {
        ldpp_dout(this,
                  20) <<
                      "ERROR: One of role name, policy name or perm policy is empty" <<
                      dendl;
        return -EINVAL;
    }
...
int RGWGetRolePolicy::get_params()
{
    ...
    if (role_name.empty() || policy_name.empty()) {
        ldpp_dout(this,
                  20) << "ERROR: One of role name or policy name is empty" <<
                      dendl;
        return -EINVAL;
    }
    return 0;
