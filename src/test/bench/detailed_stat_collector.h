...
class DetailedStatCollector:public StatCollector {
  ...
  private:
    struct Op {
        ...
        uint64_t seq;
         Op(string type,
            utime_t start, double latency, uint64_t size, uint64_t seq)
        :type(type), start(start), latency(latency), size(size), seq(seq) {
    ...
    void dump(const string & type,
              boost::tuple < utime_t, utime_t, uint64_t, uint64_t > stuff);
  public:
    DetailedStatCollector(double bin_size,
                          Formatter * formatter,
                          ostream * out,
                          ostream * summary_out,
