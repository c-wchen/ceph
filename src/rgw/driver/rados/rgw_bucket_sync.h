...
struct rgw_sync_group_pipe_map {
    ...
    rgw_sync_policy_group::Status status {
    ...
    std::set < rgw_zone_id > *pall_zones {
        nullptr};
    ...
    void try_add_to_pipe_map(const rgw_zone_id &source_zone,
                             ...
                             const std::vector < rgw_sync_bucket_pipes >
                             &pipes, zb_pipe_map_t *pipe_map,
                             CB1 filter_cb, CB2 call_filter_cb);
    ...
    std::pair < zb_pipe_map_t::const_iterator,
        zb_pipe_map_t::const_iterator > find_pipes(const zb_pipe_map_t &m,
     ...
     */
    std::vector < rgw_sync_bucket_pipe > find_dest_pipes(std::optional <
            rgw_bucket >
            source_bucket,
            const rgw_zone_id &
            dest_zone,
...
public:
    ...
    public:
        using prefix_map_t =
            std::multimap < std::string, rgw_sync_bucket_pipe * >;
        ...
        } prefix_map_t::const_iterator prefix_search(const std::
                string &s)const;
    ...
    struct pipe_handler: public endpoints_pair {
        ...
        pipe_handler()
        {
                       ...
                       const rgw_sync_bucket_pipe &_pipe): endpoints_pair(_pipe), rules(_rules)
        {
        }
        bool specific() const
        {
                                            ...
                                            bool *need_more_info)const
        {
            ...
            return rules->find_basic_info_without_tags(key, user,
                    acl_translation,
                    storage_class, mode,
                    need_more_info);
        }
...
private:
    ...
    const RGWBucketSyncFlowManager *parent {
        nullptr};
...
public:
    ...
    void reflect(const DoutPrefixProvider *dpp,
                 std::optional < rgw_bucket > effective_bucket,
                 pipe_set *flow_by_source, pipe_set *flow_by_dest,
                 bool only_enabled) const;

};

static inline std::ostream &operator<<(std::ostream &os,
                                       const RGWBucketSyncFlowManager::
                                       endpoints_pair &e)
{
    os << e.dest << " -> " << e.source;
    return os;
...
class RGWBucketSyncPolicyHandler
{
    bool legacy_config {
        false};
    const RGWBucketSyncPolicyHandler *parent {
        nullptr};
    RGWSI_Zone *zone_svc;
...
public:
    ...
    void reflect(const DoutPrefixProvider *dpp,
                 ...
                 std::map < rgw_zone_id,
                 RGWBucketSyncFlowManager::pipe_set > *psources,
                 std::map < rgw_zone_id,
                 RGWBucketSyncFlowManager::pipe_set > *ptargets,
                 std::set < rgw_zone_id > *psource_zones,
                 std::set < rgw_zone_id > *ptarget_zones,
                 bool only_enabled) const;

    void set_resolved_hints(std::set < rgw_sync_bucket_pipe >
                            &&_resolved_sources,
    ...
    {
        return target_zones;
    } const std::map < rgw_zone_id,
    RGWBucketSyncFlowManager::pipe_set > &get_sources()
    {
        return sources;
    }
    ...
    std::multimap < rgw_zone_id,
        rgw_sync_bucket_pipe > get_all_dests_in_zone(const rgw_zone_id &zone_id) const;

    const std::map < rgw_zone_id,
          RGWBucketSyncFlowManager::pipe_set > &get_targets()
    {
        return targets;
    }
    ...
    bufferlist > > &get_bucket_attrs() const
    {
        return bucket_attrs;
    } void get_pipes(RGWBucketSyncFlowManager::pipe_set **_sources,
                     RGWBucketSyncFlowManager::pipe_set **_targets)      /* return raw pipes (with zone name) */
    {
        *_sources = &source_pipes;
        *_targets = &target_pipes;
    }
    void get_pipes(std::set < rgw_sync_bucket_pipe > *sources,
