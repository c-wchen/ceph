        ...
        namespace rename {
            ...
            static int do_rename(librbd::RBD & rbd, librados::IoCtx & io_ctx,
                ...
                if (pool_name != dst_pool_name) {
                    std::
                        cerr << "rbd: mv/rename across pools not supported" <<
                        std::
                        endl << "source pool: " << pool_name << " dest pool: "
                        << dst_pool_name << std::endl;
                    return -EINVAL;
                }
                else if (namespace_name != dst_namespace_name) {
                    std::
                        cerr << "rbd: mv/rename across namespaces not supported"
                        << std::
                ...
                librbd::RBD rbd;
                r = do_rename(rbd, io_ctx, image_name.c_str(),
                              dst_image_name.c_str());
                if (r < 0) {
                    std::
                        cerr << "rbd: rename error: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
