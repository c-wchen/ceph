...
template < typename I >
RefreshRequest < I >::RefreshRequest(I &image_ctx,
                                     bool acquiring_lock,
...
void RefreshRequest < I >::send_get_migration_header()
{
    ...
    librados::AioCompletion *comp =
        ...
        &klass::handle_get_migration_header > (this);
    m_out_bl.clear();
...
I >::handle_get_migration_header(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
    ...
    if (*result >= 0) {
        ...
        *result =
            cls_client::migration_get_finish(&it, &m_migration_spec);
    } else if (*result == -ENOENT) {
        ldout(cct,
              5) << this << " " << __func__ <<
                 ": no migration header found" << ", retrying" << dendl;
        send();
        return nullptr;
    }
        ...
        case cls::rbd::MIGRATION_HEADER_TYPE_DST:
            ...
            switch (m_migration_spec.state) {
                case cls::rbd::MIGRATION_STATE_PREPARING:
                    ldout(cct,
                          5) << this << " " << __func__ <<
                             ": current migration state: " << m_migration_spec.
                             state << ", retrying" << dendl;
                    send();
                    return nullptr;
                ...
                case cls::rbd::MIGRATION_STATE_ABORTING:
                    if (!m_read_only) {
                        lderr(cct) << this << " " << __func__ <<
                                   ": migration is being aborted" << dendl;
                        *result = -EROFS;
                        return m_on_finish;
                    }
                    break;
                default:
                    lderr(cct) << this << " " << __func__ <<
                               ": migration is in an " << "unexpected state" << dendl;
                    *result = -EINVAL;
                    return m_on_finish;
            }
            break;
...
template < typename I > void RefreshRequest < I >::send_v1_read_header()
{
    ...
    m_out_bl.clear();
    int r =
        m_image_ctx.md_ctx.aio_operate(m_image_ctx.header_oid, comp,
                                       &op,
                                       &m_out_bl);
    ceph_assert(r == 0);
    comp->release();
...
template < typename I >
Context *RefreshRequest <
I >::handle_v1_get_snapshots(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": " << "r=" << *result <<
    ...
    if (*result < 0) {
        lderr(cct) << "failed to retrieve v1 snapshots: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
Context *RefreshRequest < I >::handle_v1_get_locks(int *result)
{
    ...
    if (*result >= 0) {
        ...
        *result =
            rados::cls::lock::get_lock_info_finish(&it, &m_lockers,
                &lock_type,
                &m_lock_tag);
        if (*result >= 0) {
    ...
    if (*result < 0) {
        lderr(cct) << "failed to retrieve locks: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
void RefreshRequest < I >::send_v2_get_mutable_metadata()
{
    ...
    bool read_only = (((m_read_only_flags &
                        ~IMAGE_READ_ONLY_FLAG_NON_PRIMARY) != 0)
                      || (snap_id != CEPH_NOSNAP));
    librados::ObjectReadOperation op;
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_get_mutable_metadata(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct, 10) << this << " " << __func__ << ": "
                   << "r=" << *result << dendl;
    ...
    if (unsupported != 0ULL) {
        lderr(cct) << "Image uses unsupported features: " << unsupported
                   << dendl;
        *result = -ENOSYS;
        return m_on_finish;
    }
    ...
    if (((m_incompatible_features & RBD_FEATURE_NON_PRIMARY) != 0U) &&
        ...
        &&
        ((m_image_ctx.
          read_only_mask & IMAGE_READ_ONLY_FLAG_NON_PRIMARY) != 0U)) {
        ...
        m_read_only_flags |= IMAGE_READ_ONLY_FLAG_NON_PRIMARY;
                ...
                ||
                ((m_image_ctx.
                  read_only_mask & IMAGE_READ_ONLY_FLAG_NON_PRIMARY) ==
                 0U))
               && ((m_read_only_flags & IMAGE_READ_ONLY_FLAG_NON_PRIMARY)
                   != 0U)) {
        ldout(cct,
              5) << "removing non-primary read-only image flag" <<
                 dendl;
        m_read_only_flags &= ~IMAGE_READ_ONLY_FLAG_NON_PRIMARY;
    }
...
template < typename I > void RefreshRequest < I >::send_v2_get_parent()
{
    ...
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": legacy=" <<
              m_legacy_parent << dendl;
    ...
    auto aio_comp = create_rados_callback <
                    RefreshRequest < I >,
                    &RefreshRequest < I >::handle_v2_get_parent > (this);
    m_out_bl.clear();
    m_image_ctx.md_ctx.aio_operate(m_image_ctx.header_oid, aio_comp,
                                   &op, &m_out_bl);
    aio_comp->release();
}
...
Context *RefreshRequest < I >::handle_v2_get_parent(int *result)
{
    ...
    if (!m_legacy_parent) {
        if (*result >= 0) {
            *result =
                cls_client::parent_get_finish(&it, &m_parent_md.spec);
        }
    ...
    if (*result == -EOPNOTSUPP && !m_legacy_parent) {
        ...
        lderr(cct) << "failed to retrieve parent: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
void RefreshRequest < I >::send_v2_get_metadata()
{
    ...
    auto ctx = create_context_callback <
               RefreshRequest < I >,
               &RefreshRequest < I >::handle_v2_get_metadata > (this);
    m_metadata.clear();
    auto req =
        GetMetadataRequest < I >::create(m_image_ctx.md_ctx,
                                         m_image_ctx.header_oid, true,
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_get_metadata(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to retrieve metadata: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
void RefreshRequest < I >::send_v2_get_pool_metadata()
{
    ...
    auto ctx = create_context_callback <
               RefreshRequest < I >,
               &RefreshRequest < I >::handle_v2_get_pool_metadata > (this);
    auto req =
        GetMetadataRequest < I >::create(m_pool_metadata_io_ctx,
                                         RBD_INFO, true,
                                         ImageCtx::METADATA_CONF_PREFIX,
                                         ImageCtx::METADATA_CONF_PREFIX,
                                         0U, &m_metadata, ctx);
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_get_pool_metadata(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to retrieve pool metadata: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
void RefreshRequest < I >::send_v2_get_op_features()
{
    ...
    librados::AioCompletion *comp = create_rados_callback <
                                    RefreshRequest < I >,
                                    &RefreshRequest < I >::handle_v2_get_op_features > (this);
    m_out_bl.clear();
    int r =
        m_image_ctx.md_ctx.aio_operate(m_image_ctx.header_oid, comp,
                                       &op,
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_get_op_features(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct, 10) << this << " " << __func__ << ": "
                   << "r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to retrieve op features: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
Context *RefreshRequest < I >::handle_v2_get_group(int *result)
{
    ...
    if (*result == -EOPNOTSUPP) {
        ...
        lderr(cct) << "failed to retrieve group: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_get_snapshots(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": " << "r=" << *result <<
    ...
    for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
        if (m_legacy_snapshot != LEGACY_SNAPSHOT_DISABLED) {
            ...
            if (*result >= 0) {
                uint8_t order;
                *result =
                    cls_client::get_size_finish(&it, &snap_size,
                                                &order);
            }
            ...
            *result =
                cls_client::snapshot_get_finish(&it, &m_snap_infos[i]);
        }
        ...
        if (*result >= 0) {
            *result =
                cls_client::get_flags_finish(&it, &m_snap_flags[i]);
        }

        if (*result >= 0) {
            *result =
                cls_client::get_protection_status_finish(&it,
                    &m_snap_protection
                    [i]);
        }
    ...
    if (*result == -ENOENT && m_enoent_retries++ < MAX_ENOENT_RETRIES) {
        ldout(cct,
              10) << "out-of-sync snapshot state detected, retrying" <<
                  dendl;
        send_v2_get_mutable_metadata();
        return nullptr;
        ...
        return nullptr;
               ...
               *result == -EOPNOTSUPP) {
        ldout(cct,
              10) << "retrying using legacy snapshot methods (jewel)" <<
                  dendl;
        m_legacy_snapshot = LEGACY_SNAPSHOT_ENABLED_NO_TIMESTAMP;
        send_v2_get_snapshots();
        return nullptr;
    } else if (*result < 0) {
        lderr(cct) << "failed to retrieve snapshots: " <<
                   cpp_strerror(*result)
                   << dendl;
        return m_on_finish;
    }
...
{
    {
        ...
        int r =
            get_parent_info(m_image_ctx.snap_id, &parent_md,
                            &migration_info);
        if (!m_skip_open_parent_image
            && (r < 0
                || RefreshParentRequest <
                I >::is_refresh_required(m_image_ctx, parent_md,
                                         migration_info))) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 10) << this << " " << __func__ << dendl;
            ...
            m_refresh_parent =
                RefreshParentRequest < I >::create(m_image_ctx,
                                                   parent_md,
                                                   migration_info, ctx);
        }
    }
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_refresh_parent(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result == -ENOENT && m_enoent_retries++ < MAX_ENOENT_RETRIES) {
        ldout(cct,
              10) << "out-of-sync parent info detected, retrying" <<
                  dendl;
        ceph_assert(m_refresh_parent != nullptr);
        ...
        lderr(cct) << "failed to refresh parent image: " <<
                   cpp_strerror(*result)
                   << dendl;
        save_result(result);
        send_v2_apply();
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_init_exclusive_lock(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
...
{
    bool journal_disabled = ((m_features & RBD_FEATURE_JOURNALING) == 0
                             || m_read_only
    ...
    bool journal_disabled_by_policy;
    {
        std::shared_lock image_locker {
            m_image_ctx.image_lock};
        journal_disabled_by_policy = (!journal_disabled &&
                                      m_image_ctx.get_journal_policy()->
                                      journal_disabled());
    }
    ...
    if (journal_disabled || journal_disabled_by_policy) {
        ...
        if ((m_features & RBD_FEATURE_JOURNALING) != 0 &&
            ...
            m_image_ctx.exclusive_lock->set_require_lock(true,
                       librbd::io::
                       DIRECTION_BOTH,
                       ctx);
            return;
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_open_journal(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to initialize journal: " <<
                   cpp_strerror(*result)
                   << dendl;
        save_result(result);
    }
...
void RefreshRequest < I >::send_v2_block_writes()
{
    ...
    Context *ctx = create_context_callback <
                   RefreshRequest < I >,
                   &RefreshRequest < I >::handle_v2_block_writes > (this);
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_block_writes(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to block writes: " <<
                   cpp_strerror(*result)
                   << dendl;
        save_result(result);
    }
...
void RefreshRequest < I >::send_v2_open_object_map()
{
    ...
    if (m_image_ctx.snap_name.empty()) {
        ...
        for (size_t snap_idx = 0; snap_idx < m_snap_infos.size();
             ++snap_idx) {
            if (m_snap_infos[snap_idx].name == m_image_ctx.snap_name) {
                m_object_map =
                    m_image_ctx.create_object_map(m_snapc.
                                                  snaps[snap_idx].val);
                break;
            }
        }
        ...
        if (m_object_map == nullptr) {
            lderr(cct) << "failed to locate snapshot: " << m_image_ctx.
                       snap_name << dendl;
            send_v2_open_journal();
            return;
        }
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_open_object_map(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to open object map: " <<
                   cpp_strerror(*result)
                   << dendl;
        m_object_map->put();
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_finalize_refresh_parent(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_shut_down_exclusive_lock(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_close_journal(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        save_result(result);
        lderr(cct) << "failed to close journal: " <<
                   cpp_strerror(*result)
                   << dendl;
    }
...
template < typename I >
Context *RefreshRequest <
I >::handle_v2_close_object_map(int *result)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct,
          10) << this << " " << __func__ << ": r=" << *result << dendl;
    ...
    if (*result < 0) {
        lderr(cct) << "failed to close object map: " <<
                   cpp_strerror(*result)
                   << dendl;
    }
...
template < typename I > Context *RefreshRequest < I >::send_flush_aio()
{
    ...
    if (m_flush_aio) {
        ...
        auto ctx = create_context_callback <
                   RefreshRequest < I >,
                   &RefreshRequest < I >::handle_flush_aio > (this);
        auto aio_comp =
            io::AioCompletion::create_and_start(ctx,
                                                ...
                                                io::AIO_TYPE_FLUSH);
        auto req =
            io::ImageDispatchSpec::create_flush(m_image_ctx,
                                                io::
        ...
        Context *ctx = create_context_callback <
                       RefreshRequest < I >,
                       &RefreshRequest < I >::handle_error > (this);
        m_image_ctx.op_work_queue->queue(ctx, 0);
        return nullptr;
    }
...
Context *RefreshRequest < I >::handle_flush_aio(int *result)
{
    ...
    if (*result < 0) {
        lderr(cct) << "failed to flush pending AIO: " <<
                   cpp_strerror(*result)
                   << dendl;
    }
...
template < typename I > void RefreshRequest < I >::apply()
{
    ...
    std::scoped_lock locker {
        m_image_ctx.owner_lock, m_image_ctx.image_lock};
    ...
    if (m_image_ctx.old_format) {
        ...
        m_image_ctx.operations_disabled = ((m_op_features &
        ...
        if (migration_info_valid) {
            for (auto it : m_image_ctx.migration_info.
                 snap_map) {
                migration_reverse_snap_seq[it.second.front()] =
                    it.first;
            }
        } else {
    ...
    for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
        std::vector < librados::snap_t >::const_iterator it =
            std::find(m_image_ctx.snaps.begin(),
                      m_image_ctx.snaps.end(), m_snapc.snaps[i].val);
    ...
    for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
        ...
        uint8_t protection_status = m_image_ctx.old_format ?
                                    static_cast < uint8_t >
                                    (RBD_PROTECTION_STATUS_UNPROTECTED) : m_snap_protection[i];
        ParentImageInfo parent;
        if (!m_image_ctx.old_format) {
            if (!m_image_ctx.migration_info.empty()) {
                parent = m_image_ctx.parent_md;
                auto it =
                    migration_reverse_snap_seq.find(m_snapc.snaps[i].
                                                    val);
                if (it != migration_reverse_snap_seq.end()) {
                    ...
                    overlap =
                        std::min(overlap, m_snap_infos[i].image_size);
    ...
    if (m_image_ctx.data_ctx.is_valid()) {
        m_image_ctx.data_ctx.selfmanaged_snap_set_write_ctx(m_image_ctx.
                   snapc.seq,
                   m_image_ctx.
                   snaps);
    ...
    if (m_image_ctx.exclusive_lock != nullptr &&
        ...
        if (!m_image_ctx.test_features(RBD_FEATURE_JOURNALING,
            ...
            if (!m_image_ctx.clone_copy_on_read
                && m_image_ctx.journal != nullptr) {
                m_image_ctx.exclusive_lock->
                unset_require_lock(io::DIRECTION_READ);
            }
        ...
        migration_info)
{
    bool migration_info_valid;
    int r =
        get_migration_info(parent_md, migration_info,
                           &migration_info_valid);
    if (r < 0) {
        return r;
    }
...
template < typename I >
int RefreshRequest <
I >::get_migration_info(ParentImageInfo *parent_md,
                        MigrationInfo *migration_info,
                        bool *migration_info_valid)
{
    CephContext *cct = m_image_ctx.cct;
    if (m_migration_spec.header_type !=
        cls::rbd::MIGRATION_HEADER_TYPE_DST
        || (m_migration_spec.state != cls::rbd::MIGRATION_STATE_PREPARED
            && m_migration_spec.state !=
            cls::rbd::MIGRATION_STATE_EXECUTING
            && m_migration_spec.state !=
            cls::rbd::MIGRATION_STATE_ABORTING)) {
        if (m_migration_spec.header_type !=
    ...
    // these snapshots.
    snapid_t snap_id =
        snap_seqs.empty() ? 0 : snap_seqs.rbegin()->second;
    auto it =
        std::upper_bound(m_snapc.snaps.rbegin(), m_snapc.snaps.rend(),
                         snap_id);
