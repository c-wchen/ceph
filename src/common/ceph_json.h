...
class JSONObj {
  ...
  public:
    ...
    friend ostream & operator<<(ostream & out, const JSONObj & obj);    // does not work, FIXME
...
template < class T > void decode_json_obj(list < T > &l, JSONObj * obj)
{
    l.clear();
...
template < class T > void decode_json_obj(deque < T > &l, JSONObj * obj)
{
    l.clear();
...
template < class T > void decode_json_obj(set < T > &l, JSONObj * obj)
{
    l.clear();
...
template < class T > void decode_json_obj(vector < T > &l, JSONObj * obj)
{
    l.clear();
...
template < class K, class V, class C =
    std::less < K > >void decode_json_obj(map < K, V, C > &m, JSONObj * obj)
{
    m.clear();
...
{
    f->open_array_section(name);
    for (typename std::deque < T >::const_iterator iter = l.begin();
         iter != l.end(); ++iter) {
        encode_json("obj", *iter, f);
    }
    f->close_section();
...
template < class T >
    static void encode_json(const char *name, const std::set < T > &l,
                            ceph::Formatter * f)
{
    f->open_array_section(name);
    for (typename std::set < T >::const_iterator iter = l.begin();
         iter != l.end(); ++iter) {
        encode_json("obj", *iter, f);
    }
    f->close_section();
...
{
    f->open_array_section(name);
    for (typename std::map < K, V, C >::const_iterator i = m.begin();
         i != m.end(); ++i) {
...
{
    f->open_array_section(name);
    for (typename std::multimap < K, V >::const_iterator i = m.begin();
         i != m.end(); ++i) {
        f->open_object_section("entry");
        encode_json("key", i->first, f);
        encode_json("val", i->second, f);
...
template < class K, class V >
    void encode_json_map(const char *name, const map < K, V > &m,
                         ceph::Formatter * f)
{
    f->open_array_section(name);
...
template < class K, class V >
    void encode_json_map(const char *name, const char *index_name,
                         const char *value_name, const map < K, V > &m,
                         ceph::Formatter * f)
{
