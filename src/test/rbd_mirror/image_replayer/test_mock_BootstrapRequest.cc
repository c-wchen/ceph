    ...
    namespace mirror {
        ...
        class ProgressContext;
         ...
         template <> struct ImageSync <librbd::MockTestImageCtx > {
            ...
            static ImageSync *create(Threads < librbd::MockTestImageCtx >
                                     *threads,
        ...
        namespace image_replayer {
            ...
            PrepareRemoteImageRequest < librbd::MockTestImageCtx >
                *PrepareRemoteImageRequest <
                librbd::MockTestImageCtx >::s_instance = nullptr;
            StateBuilder < librbd::MockTestImageCtx > *StateBuilder <
                librbd::MockTestImageCtx >::s_instance = nullptr;
        ...
        namespace image_replayer {
              ...
              public:
                ...
                void SetUp() override {
                    ...
                    EXPECT_CALL(mock_request, send())
                        .
                        WillOnce(Invoke
                                 ([&mock_request, &mock_state_builder,
                                   local_image_id, local_image_name, r] () {
                                  if (r == 0) {
                                  *mock_request.state_builder =
                                  &mock_state_builder;
                                  mock_state_builder.local_image_id =
                                  local_image_id;
                                  *mock_request.local_image_name =
                                  local_image_name;} mock_request.on_finish->
                                  complete(r);}
                                 )) ;
                }
                ...
                void expect_send(MockPrepareRemoteImageRequest & mock_request,
                                 ...
                                 const std::string & remote_image_id, int r) {
                    EXPECT_CALL(mock_request, send())
                        .
                        WillOnce(Invoke
                                 ([&mock_request, &mock_state_builder,
                                  ...
                                  *mock_request.state_builder =
                                  ...
                                  mock_state_builder.remote_mirror_uuid =
                                  remote_mirror_uuid;
                                  mock_request.on_finish->complete(r);}
                                 )) ;
                }

                void expect_is_local_primary(MockStateBuilder &
                                             mock_state_builder,
                                             bool is_primary) {
                ...
                void expect_is_remote_primary(MockStateBuilder &
                                              mock_state_builder,
                                              bool is_primary) {
                ...
                void expect_is_linked(MockStateBuilder & mock_state_builder,
                                      bool is_linked) {
                    EXPECT_CALL(mock_state_builder, is_linked())
                        .WillOnce(Return(is_linked));
                }

                void expect_is_disconnected(MockStateBuilder &
                                            mock_state_builder,
                ...
                void expect_replay_requires_remote_image(MockStateBuilder &
                                                         mock_state_builder,
                                                         bool requires_image) {
                ...
                void expect_close_remote_image(MockStateBuilder &
                                               mock_state_builder, int r) {
                    EXPECT_CALL(mock_state_builder, close_remote_image(_))
                        .WillOnce(Invoke([&mock_state_builder, r]
                                         (Context * on_finish) {
                                         mock_state_builder.remote_image_ctx =
                                         nullptr; on_finish->complete(r);}));
                }

                void expect_create_local_image(MockStateBuilder &
                                               mock_state_builder,
                                               const std::
                                               string & local_image_id, int r) {
                    EXPECT_CALL(mock_state_builder,
                        ...
                        .WillOnce(WithArg < 5 >
                                  (Invoke
                                   ([&mock_state_builder, local_image_id,
                                    ...
                                    mock_state_builder.local_image_id =
                                    local_image_id;}
                                    mock_state_builder.mock_base_request.
                                    on_finish = ctx;
                                    return &mock_state_builder.
                                    mock_base_request;}
                                   ))) ;
                    EXPECT_CALL(mock_state_builder.mock_base_request, send())
                        .WillOnce(Invoke([this, &mock_state_builder, r] () {
                                         m_threads->work_queue->
                                         queue(mock_state_builder.
                                               mock_base_request.on_finish, r);}
                                  ));
                }

                void expect_prepare_replay(MockStateBuilder &
                                           mock_state_builder,
                                           bool resync_requested, bool syncing,
                                           int r) {
                    EXPECT_CALL(mock_state_builder,
                        ...
                        .WillOnce(WithArgs < 2, 3,
                                  4 >
                                  (Invoke
                                   ([&mock_state_builder, resync_requested,
                                     syncing, r]
                                    (bool * resync, bool * sync, Context * ctx) {
                                    if (r >= 0) {
                                    *resync = resync_requested; *sync = syncing;}
                                    mock_state_builder.mock_base_request.
                                    on_finish = ctx;
                                    return &mock_state_builder.
                                    mock_base_request;}
            ...
            TEST_F(TestMockImageReplayerBootstrapRequest, Success) {
                ...
                expect_send(mock_prepare_local_image_request,
                            mock_state_builder, m_local_image_ctx->id,
                            m_local_image_ctx->name, 0);

                // prepare remote image
                MockPrepareRemoteImageRequest mock_prepare_remote_image_request;
                expect_send(mock_prepare_remote_image_request,
                            mock_state_builder, "remote mirror uuid",
                ...
                MockBootstrapRequest *request =
                    create_request(&mock_threads, &mock_instance_watcher,
                                   "global image id",
            ...
            TEST_F(TestMockImageReplayerBootstrapRequest, OpenLocalImageDNE) {
                ...
                // create local image
                expect_create_local_image(mock_state_builder, "local image id",
                                          0);

                // re-open the local image
                expect_open_local_image(mock_open_local_image_request,
                                        m_local_io_ctx, "local image id",
                                        &mock_local_image_ctx, 0);
            ...
            TEST_F(TestMockImageReplayerBootstrapRequest, PrepareReplayError) {
                ...
                // prepare replay
                expect_prepare_replay(mock_state_builder, false, false,
                                      -EINVAL);
