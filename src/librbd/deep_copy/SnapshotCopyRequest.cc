...
template < typename I >
const std::string &get_snapshot_name(I *image_ctx,
                                     librados::
                                     snap_t snap_id)
{
...
} // anonymous namespace using librbd::util::create_context_callback;
...
SnapshotCopyRequest < I >::SnapshotCopyRequest(I *src_image_ctx,
    ...
    :    RefCountedObject(dst_image_ctx->cct),
         ...
         m_dst_snap_id_start(dst_snap_id_start), m_flatten(flatten),
         m_work_queue(work_queue), m_snap_seqs_result(snap_seqs),
         m_snap_seqs(*snap_seqs), m_on_finish(on_finish),
         m_cct(dst_image_ctx->cct),
         m_lock(ceph::
                make_mutex(unique_lock_name
                           ("SnapshotCopyRequest::m_lock", this)))
{
    ...
    if (m_src_snap_id_end != CEPH_NOSNAP) {
        m_src_snap_ids.erase(m_src_snap_ids.
                             upper_bound(m_src_snap_id_end),
                             m_src_snap_ids.end());
    }
}
...
template < typename I > void SnapshotCopyRequest < I >::send()
{
    ...
    if (r < 0) {
        lderr(m_cct) << "destination image parent spec mismatch" <<
                     dendl;
        error(r);
        return;
    }
...
void SnapshotCopyRequest < I >::send_snap_unprotect()
{
    ...
    for (; snap_id_it != m_dst_snap_ids.end(); ++snap_id_it) {
        ...
        int r =
            m_dst_image_ctx->is_snap_unprotected(dst_snap_id,
                &dst_unprotected);
        if (r < 0) {
            lderr(m_cct) <<
                         "failed to retrieve destination snap unprotect status: "
        ...
        auto snap_seq_it =
            std::find_if(m_snap_seqs.begin(), m_snap_seqs.end(),
                         [dst_snap_id](const SnapSeqs::
        value_type & pair) {
        ...
        if (snap_seq_it != m_snap_seqs.end()) {
            ...
            if (r < 0) {
                lderr(m_cct) <<
                             "failed to retrieve source snap unprotect status: "
                             << cpp_strerror(r) << dendl;
                m_src_image_ctx->image_lock.unlock_shared();
                finish(r);
                return;
...
void SnapshotCopyRequest < I >::handle_snap_unprotect(int r)
{
...

    {
        ...
        auto snap_info_it =
            m_dst_image_ctx->snap_info.find(m_prev_snap_id);
...
void SnapshotCopyRequest < I >::send_snap_remove()
{
    ...
    for (; snap_id_it != m_dst_snap_ids.end(); ++snap_id_it) {
        ...
        m_dst_image_ctx->image_lock.lock_shared();
        int r =
            m_dst_image_ctx->get_snap_namespace(dst_snap_id,
                                                &snap_namespace);
        m_dst_image_ctx->image_lock.unlock_shared();
        if (r < 0) {
            lderr(m_cct) <<
                         "failed to retrieve destination snap namespace: " <<
                         m_snap_name << dendl;
            finish(r);
            return;
        }
...
void SnapshotCopyRequest < I >::send_snap_create()
{
    ...
    for (; snap_id_it != m_src_snap_ids.end(); ++snap_id_it) {
        ...
        m_src_image_ctx->image_lock.lock_shared();
        int r =
            m_src_image_ctx->get_snap_namespace(src_snap_id,
                                                &snap_namespace);
        m_src_image_ctx->image_lock.unlock_shared();
        if (r < 0) {
        ...
        if (m_snap_seqs.find(src_snap_id) == m_snap_seqs.end()) {
            ...
            if (std::holds_alternative <
                cls::rbd::UserSnapshotNamespace > (snap_namespace)) {
    ...
    if (snap_info_it == m_src_image_ctx->snap_info.end()) {
        m_src_image_ctx->image_lock.unlock_shared();
        lderr(m_cct) << "failed to retrieve source snap info: " <<
                     m_snap_name << dendl;
        finish(-ENOENT);
        return;
    }
    ...
    SnapshotCreateRequest < I > *req =
        SnapshotCreateRequest < I >::create(m_dst_image_ctx,
                                            m_snap_name,
                                            m_snap_namespace, size,
                                            parent_spec, parent_overlap,
                                            ctx);
    req->send();
}
...
void SnapshotCopyRequest < I >::handle_snap_create(int r)
{
    ...
    auto snap_it = m_dst_image_ctx->snap_ids.find({ cls::rbd::
    ...
    ldout(m_cct,
          20) << "mapping source snap id " << m_prev_snap_id << " to "
              << dst_snap_id << dendl;
...
void SnapshotCopyRequest < I >::send_snap_protect()
{
    ...
    for (; snap_id_it != m_src_snap_ids.end(); ++snap_id_it) {
        ...
        bool src_protected;
        int r =
            m_src_image_ctx->is_snap_protected(src_snap_id,
                                               &src_protected);
        if (r < 0) {
            lderr(m_cct) <<
                         "failed to retrieve source snap protect status: " <<
                         cpp_strerror(r) << dendl;
            m_src_image_ctx->image_lock.unlock_shared();
            finish(r);
            return;
        ...
        m_dst_image_ctx->image_lock.lock_shared();
        bool dst_protected;
        r = m_dst_image_ctx->is_snap_protected(snap_seq_it->second,
                                               &dst_protected);
        if (r < 0) {
            lderr(m_cct) <<
                         "failed to retrieve destination snap protect status: "
                         << cpp_strerror(r) << dendl;
            m_dst_image_ctx->image_lock.unlock_shared();
            finish(r);
            return;
...
template < typename I > void SnapshotCopyRequest < I >::send_set_head()
{
    ...
    auto ctx = create_context_callback <
               ...
               &SnapshotCopyRequest < I >::handle_set_head > (this);
    auto req =
        SetHeadRequest < I >::create(m_dst_image_ctx, size, parent_spec,
                                     parent_overlap, ctx);
    req->send();
}
...
void SnapshotCopyRequest < I >::handle_set_head(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to set head: " << cpp_strerror(r) <<
                     dendl;
        finish(r);
        return;
    }
...
void SnapshotCopyRequest < I >::send_resize_object_map()
{
    ...
    if (m_dst_image_ctx->test_features(RBD_FEATURE_OBJECT_MAP)) {
        ...
        if (m_dst_image_ctx->object_map != nullptr &&
            ...
            auto finish_op_ctx =
                start_lock_op(m_dst_image_ctx->owner_lock, &r);
            if (finish_op_ctx != nullptr) {
                auto ctx =
                new LambdaContext([this, finish_op_ctx](int r) {
                    handle_resize_object_map(r);
                    finish_op_ctx->complete(0);
                });

                m_dst_image_ctx->object_map->
                aio_resize(m_dst_image_ctx->size,
...
void SnapshotCopyRequest < I >::handle_resize_object_map(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to resize object map: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
template < typename I > void SnapshotCopyRequest < I >::error(int r)
{
    ...
    m_work_queue->queue(new LambdaContext([this, r](int r1) {
        finish(r);
    }));
}
