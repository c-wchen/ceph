...
#include "crimson/os/seastore/logging.h"
    ...
    RBMDevice::mkfs_ret RBMDevice::do_mkfs(device_config_t config) {
        ...
        return stat_device().handle_error(mkfs_ertr::pass_further {
                                          }
                    ...
                    return open(get_device_path(),
                                seastar::open_flags::rw | seastar::open_flags::
                                dsync).handle_error(mkfs_ertr::pass_further {
                                                    }
                                                    ,
                                                    crimson::ct_error::
                                                    assert_all {
    ...
    read_ertr::future < rbm_metadata_header_t >
        RBMDevice::read_rbm_header(rbm_abs_addr addr) {
        LOG_PREFIX(RBMDevice::read_rbm_header);
        assert(super.block_size > 0);
        return seastar::
            do_with(bufferptr
                    (ceph::buffer::create_page_aligned(super.block_size)),[this,
                                                                           addr,
                                                                           FNAME]
                    (auto & bptr) {
                    return read(addr,
                                bptr).safe_then([length =
                                                 bptr.length(), this, bptr,
                                                 FNAME] ()
                                                ->read_ertr::future <
                                                ...
                                                DEBUG
                                                ("read_rbm_header: unable to decode rbm super block {}",
                                                 e.what());
                                                ...
                                                ceph_assert_always(crc ==
                                                                   (checksum_t)
                                                                   - 1);}
                                                super_block.crc = crc;
                                                super = super_block;
                                                DEBUG("got {} ", super);
                                                return read_ertr::future <
                                                rbm_metadata_header_t >
                                                (read_ertr::
                                                 ready_future_marker {
                                                 }
    ...
    RBMDevice::mount_ret RBMDevice::do_mount() {
        return open(get_device_path(),
            ...
            safe_then([this] {
                      return stat_device().handle_error(mount_ertr::pass_further {
                                                        }
                                                        ,
                                                        crimson::ct_error::
                                                        assert_all {
    ...
    EphemeralRBMDeviceRef create_test_ephemeral(uint64_t journal_size,
                                                uint64_t data_size) {
        return EphemeralRBMDeviceRef(new
                                     EphemeralRBMDevice(journal_size +
                                                        data_size +
                                                        random_block_device::
                                                        RBMDevice::
                                                        get_journal_start(),
    ...
    open_ertr::future <> EphemeralRBMDevice::open(const std::string & in_path,
    ...
    write_ertr::future <> EphemeralRBMDevice::write(uint64_t offset,
                                                    bufferptr && bptr,
                                                    uint16_t stream) {
        LOG_PREFIX(EphemeralRBMDevice::write);
        ceph_assert(buf);
        DEBUG("EphemeralRBMDevice: write offset {} len {}",
              offset, bptr.length());
    ...
    read_ertr::future <> EphemeralRBMDevice::read(uint64_t offset,
                                                  bufferptr & bptr) {
        LOG_PREFIX(EphemeralRBMDevice::read);
        ceph_assert(buf);
        DEBUG("EphemeralRBMDevice: read offset {} len {}",
              offset, bptr.length());
    ...
    write_ertr::future <> EphemeralRBMDevice::writev(uint64_t offset,
                                                     ceph::bufferlist bl,
                                                     uint16_t stream) {
