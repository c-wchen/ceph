...
struct MockManagedLockImageCtx: public MockImageCtx {
    explicit MockManagedLockImageCtx(ImageCtx &
...
template <> struct Traits <MockManagedLockImageCtx > {
    ...
    MockMockManagedLock(librados::IoCtx &ioctx, AsioEngine &asio_engine,
        ...
        : ManagedLock < MockManagedLockImageCtx > (ioctx, asio_engine, oid,
                watcher,
                librbd::managed_lock::
                EXCLUSIVE, true, 0)
    {
...
struct AcquireRequest <MockManagedLockImageCtx >: public BaseRequest
    ...
    static AcquireRequest *create(librados::IoCtx &ioctx,
                                  ...
                                  Context *on_finish)
    {
        return BaseRequest::create(ioctx, watcher, oid, cookie,
                                   on_finish);
...
public:
    ...
    {
        EXPECT_CALL(watcher,
                    is_blocklisted()).WillOnce(Return(blocklisted));
    }
...
TEST_F(TestMockManagedLock, AcquireLockBlocklist)
{
    ...
    expect_acquire_lock(*mock_image_ctx.image_watcher, ictx->op_work_queue,
                        request_lock_acquire, -EBLOCKLISTED);
...
TEST_F(TestMockManagedLock, ReleaseLockBlocklist)
{
    ...
    expect_post_release_lock_handler(managed_lock, false, -EBLOCKLISTED,
                                     -EBLOCKLISTED);
