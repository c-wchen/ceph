...
{
    reinterpret_cast <
    T * >(arg)->complete(rados_aio_get_return_value(c));
} template < typename T,
             void (T::*MF)(int)>void rados_callback(rados_completion_t c,
                     void *arg)
{
    ...
    if (on_finish != nullptr) {
        ...
        if (destroy) {
            delete obj;
             ...
             void (T::*MF)(int)>class C_CallbackAdapter: public Context
{
    T *obj;
public:
...
template < typename T,
           void (T::*MF)(int)>class C_RefCallbackAdapter: public Context
{
...
{
    return librados::Rados::aio_create_completion(obj,
            &detail::
            rados_callback < T >);
}

template < typename T,
           void (T::*MF)(int)>librados::AioCompletion *create_rados_callback(T *obj)
{
    return librados::Rados::aio_create_completion(obj,
            &detail::
            rados_callback < T,
...
template < typename T,
           Context * (T::*MF)(int *)>Context *create_context_callback(T *
                   obj,
                   RefCountedPtr
                   refptr)
{
    return new detail::C_RefStateCallbackAdapter < T, MF > (obj,
            refptr);
}
...
typename std::enable_if < not std::is_base_of < RefCountedPtr,
         R >::value, Context * >::type create_context_callback(T *obj,
                 R *refptr)
{
    return new detail::C_CallbackAdapter < T, MF > (obj);
}

template < typename T, Context * (T::*MF)(int *), typename R,
           bool destroy =
           true > typename std::enable_if < not std::is_base_of <
RefCountedPtr, R >::value,
...
template < typename I >
Context *create_async_context_callback(I &image_ctx,
                                       Context *on_finish)
{
    // use async callback to acquire a clean lock context
    return new detail::C_AsyncCallback <
           typename std::decay <
           decltype(*image_ctx.op_work_queue) >::type >
           (image_ctx.op_work_queue, on_finish);
}

template < typename WQ >
Context *create_async_context_callback(WQ *work_queue,
                                       Context *on_finish)
{
    // use async callback to acquire a clean lock context
...
template < typename I >
inline ZTracer::Trace create_trace(const I &image_ctx,
                                   const char *trace_name,
                                   const ZTracer::
                                   Trace &parent_trace)
{
    if (parent_trace.valid()) {
        return ZTracer::Trace(trace_name, &image_ctx.trace_endpoint,
                              &parent_trace);
    }
