...
COMMAND("pg force-recovery name=pgid,type=CephPgid,n=N",
        "force recovery of <pgid> first", "pg", "rw")
COMMAND("pg force-backfill name=pgid,type=CephPgid,n=N",
        "force backfill of <pgid> first", "pg", "rw")
COMMAND("pg cancel-force-recovery name=pgid,type=CephPgid,n=N",
        "restore normal recovery priority of <pgid>", "pg", "rw")
COMMAND("pg cancel-force-backfill name=pgid,type=CephPgid,n=N",
        "restore normal backfill priority of <pgid>", "pg", "rw")
...
COMMAND("osd blocked-by",
        "print histogram of which OSDs are blocking their peers", "osd", "r")
...
COMMAND("osd destroy "
        "name=id,type=CephOsdName " "name=force,type=CephBool,req=false "
        ...
        "rendering data permanently unreadable.", "osd", "rw")
COMMAND("osd purge "
        "name=id,type=CephOsdName " "name=force,type=CephBool,req=false "
        // backward compat synonym for --force
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "purge all osd data from the monitors including the OSD id "
        "and CRUSH position", "osd", "rw")
...
COMMAND("device info name=devid,type=CephString",
        "Show information about a device", "mgr", "r")
COMMAND("device ls-by-daemon name=who,type=CephString",
        "Show devices associated with a daemon", "mgr", "r")
COMMAND("device ls-by-host name=host,type=CephString",
        "Show devices on a host", "mgr", "r")
...
COMMAND("device rm-life-expectancy name=devid,type=CephString",
        "Clear predicted device life expectancy", "mgr", "rw")
