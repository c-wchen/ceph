...
namespace librbd {
    namespace watcher {
        ...
        const uint64_t Notifier::NOTIFY_TIMEOUT = 5000;
         ...
         Notifier::C_AioNotify::C_AioNotify(Notifier * notifier,
        ...
        Notifier::Notifier(ContextWQ * work_queue, IoCtx & ioctx,
                           const std::string & oid)
        :m_work_queue(work_queue), m_ioctx(ioctx), m_oid(oid),
            m_aio_notify_lock(util::
                              unique_lock_name
