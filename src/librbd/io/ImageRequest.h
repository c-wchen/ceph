...
public:
    virtual ~ ImageRequest()
    {
        m_trace.event("finish");
                           ...
                           ReadResult
                           && read_result, IOContext io_context,
    ...
    static void aio_write(ImageCtxT *ictx, AioCompletion *c, Extents
                          ...
                          && bl, int op_flags,
                          const ZTracer::Trace &parent_trace);
    static void aio_discard(ImageCtxT *ictx, AioCompletion *c, Extents
...
protected:
    ...
    virtual int prune_object_extents(LightweightObjectExtents *
                                     object_extents) const
    {
        return 0;
    } void send_object_requests(const LightweightObjectExtents &
                                object_extents, IOContext io_context,
                                uint64_t journal_tid);
    virtual ObjectDispatchSpec *create_object_request(const
...
protected:
    ...
    {
        return "aio_write";
    } void assemble_extent(const LightweightObjectExtent &
                           object_extent, bufferlist *bl);

    ObjectDispatchSpec *create_object_request(const
            LightweightObjectExtent &
            object_extent,
...
protected:
    ...
    int prune_object_extents(LightweightObjectExtents *
                             object_extents) const override;
...
template < typename ImageCtxT = ImageCtx > class ImageCompareAndWriteRequest: public AbstractImageWriteRequest
    < ImageCtxT >
{
public:
    using typename ImageRequest < ImageCtxT >::ObjectRequests;
...
{
public:
    ImageListSnapsRequest(ImageCtxT &image_ctx,
                          AioCompletion *aio_comp, Extents
