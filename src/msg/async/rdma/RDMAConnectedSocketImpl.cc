...
RDMAConnectedSocketImpl::RDMAConnectedSocketImpl(CephContext * cct,
                                                 Infiniband * ib,
                                                 RDMADispatcher * s,
                                                 RDMAWorker * w)
:  cct(cct), connected(0), error(0), infiniband(ib), dispatcher(s), worker(w),
lock("RDMAConnectedSocketImpl::lock"), is_server(false),
con_handler(new C_handle_connection(this)), active(false), pending(false)
{
...
int RDMAConnectedSocketImpl::activate()
{
    ...
    ldout(cct,
          20) << __func__ << " QueuePair: " << qp << " with qp:" << qp->
        get_qp() << dendl;

    if (!is_server) {
        connected = 1;          //indicate successfully
        ldout(cct,
              20) << __func__ << " handle fake send, wake it up. QP: " <<
            my_msg.qpn << dendl;
        submit(false);
    }
    active = true;
...
int RDMAConnectedSocketImpl::try_connect(const entity_addr_t & peer_addr,
...
{
    ldout(cct,
          20) << __func__ << " QP: " << my_msg.
        qpn << " tcp_fd: " << tcp_fd << " notify_fd: " << notify_fd << dendl;
    ...
    if (!is_server) {           // syn + ack from server
        ...
        ldout(cct,
              20) << __func__ << " peer msg :  < " << peer_msg.
            qpn << ", " << peer_msg.psn << ", " << peer_msg.
            lid << ", " << peer_msg.peer_qpn << "> " << dendl;
        if (!connected) {
            r = activate();
            assert(!r);
        }
        ...
        if (peer_msg.peer_qpn == 0) {   // syn from client
            if (active) {
                ldout(cct,
                      10) << __func__ << " server is already active." << dendl;
                return;
            }
            r = infiniband->send_msg(cct, tcp_fd, my_msg);
            if (r < 0) {
...
ssize_t RDMAConnectedSocketImpl::read(char *buf, size_t len)
{
    ...
    ldout(cct,
          20) << __func__ << " notify_fd : " << i << " in " << my_msg.
        qpn << " r = " << r << dendl;
    ssize_t read = 0;
    if (!buffers.empty())
    ...
    ldout(cct,
          20) << __func__ << " poll queue got " << cqe.
        size() << " responses. QP: " << my_msg.qpn << dendl;
    for (size_t i = 0; i < cqe.size(); ++i) {
        ibv_wc *response = &cqe[i];
        assert(response->status == IBV_WC_SUCCESS);
        Chunk *chunk = reinterpret_cast < Chunk * >(response->wr_id);
        ldout(cct,
              25) << __func__ << " chunk length: " << response->
            byte_len << " bytes." << chunk << dendl;
        ...
        if (response->byte_len == 0) {
            ...
            if (connected) {
                error = ECONNRESET;
                ldout(cct,
                      20) << __func__ << " got remote close msg..." << dendl;
            }
            ...
            if (read == (ssize_t) len) {
                ...
                ldout(cct,
                      25) << __func__ << " buffers add a chunk: " << response->
                    byte_len << dendl;
            }
                ...
                buffers.push_back(chunk);
                ldout(cct,
                      25) << __func__ << " buffers add a chunk: " << chunk->
                    get_offset() << ":" << chunk->get_bound() << dendl;
            }
            else {
    ...
    if (is_server && connected == 0) {
        ldout(cct,
              20) << __func__ << " we do not need last handshake, QP: " <<
            my_msg.qpn << " peer QP: " << peer_msg.qpn << dendl;
...
ssize_t RDMAConnectedSocketImpl::read_buffers(char *buf, size_t len)
{
    ...
    for (; c != buffers.end(); ++c) {
        ...
        read += tmp;
        ldout(cct,
              25) << __func__ << " this iter read: " << tmp << " bytes." <<
            " offset: " << (*c)->get_offset() << " ,bound: " << (*c)->
            get_bound() << ". Chunk:" << *c << dendl;
        if ((*c)->over()) {
    ...
    ldout(cct,
          25) << __func__ << " got " << read << " bytes, buffers size: " <<
        buffers.size() << dendl;
    return read;
}
...
ssize_t RDMAConnectedSocketImpl::zero_copy_read(bufferptr & data)
{
    ...
    ldout(cct,
          20) << __func__ << " pool completion queue got " << cqe.
...
ssize_t RDMAConnectedSocketImpl::send(bufferlist & bl, bool more)
{
        ...
        return 0;
    {
        ...
        if (!connected) {
            ldout(cct,
                  20) << __func__ << " fake send to upper, QP: " << my_msg.
                qpn << dendl;
            return bytes;
        }
    }
...
ssize_t RDMAConnectedSocketImpl::submit(bool more)
{
    ...
    auto fill_tx_via_copy =
        [this] (std::vector < Chunk * >&tx_buffers, unsigned bytes,
        ...
        if (ret == 0) {
            ldout(cct,
                  1) << __func__ << " no enough buffers in worker " << worker <<
                dendl;
        ...
        while (start != end) {
            const uintptr_t addr =
                reinterpret_cast < const uintptr_t > (start->c_str());
            unsigned copied = 0;
            while (copied < start->length()) {
                uint32_t r =
                    current_chunk->write((char *)addr + copied,
                                         start->length() - copied);
                copied += r;
                total_copied += r;
                bytes -= r;
    ...
    while (it != pending_bl.buffers().end()) {
        if (infiniband->is_tx_buffer(it->raw_c_str())) {
            if (need_reserve_bytes) {
                unsigned copied =
                    fill_tx_via_copy(tx_buffers, need_reserve_bytes, copy_it,
                                     it);
                total += copied;
            ...
            assert(copy_it == it);
            tx_buffers.push_back(infiniband->
                                 get_tx_chunk_by_buffer(it->raw_c_str()));
            total += it->length();
            ++copy_it;
        }
        else {
  ...
  sending:
    ...
    ldout(cct,
          20) << __func__ << " left bytes: " << pending_bl.
        length() << " in buffers " << pending_bl.buffers().
        size() << " tx chunks " << tx_buffers.size() << dendl;
    ...
    ldout(cct,
          20) << __func__ << " finished sending " << bytes << " bytes." <<
        dendl;
    return pending_bl.length()? -EAGAIN : 0;
}

int RDMAConnectedSocketImpl::post_work_request(std::vector <
                                               Chunk * >&tx_buffers)
{
    ldout(cct,
          20) << __func__ << " QP: " << my_msg.
        qpn << " " << tx_buffers[0] << dendl;
    ...
    while (current_buffer != tx_buffers.end()) {
        ...
        ldout(cct,
              25) << __func__ << " sending buffer: " << *current_buffer <<
            " length: " << isge[current_sge].length << dendl;

        iswr[current_swr].wr_id =
        ...
        worker->perf_logger->inc(l_msgr_rdma_tx_bytes,
                                 isge[current_sge].length);
        if (pre_wr)
    ...
    ldout(cct,
          20) << __func__ << " qp state is " << Infiniband::qp_state_string(qp->
                                                                            get_state
                                                                            ())
        << dendl;
    return 0;
...
void RDMAConnectedSocketImpl::cleanup()
{
    if (con_handler && tcp_fd >= 0) {
        ...
        worker->center.submit_to(worker->center.get_id(),[this] () {
                                 worker->center.delete_file_event(tcp_fd,
                                                                  EVENT_READABLE);}
                                 , false);
        delete con_handler;
        con_handler = nullptr;
...
void RDMAConnectedSocketImpl::set_accept_fd(int sd)
{
    ...
    is_server = true;
    worker->center.submit_to(worker->center.get_id(),[this] () {
                             worker->center.create_file_event(tcp_fd,
                                                              EVENT_READABLE,
                                                              con_handler);},
                             true);
}
