...
struct rbm_test_t: public seastar_test_suite_t {
    ...
    seastar::future <> set_up_fut()final
    {
        ...
        rbm_manager.
        reset(new BlockRBManager(device.get(), std::string(), false));
    ...
    {
        paddr_t paddr = convert_abs_addr_to_paddr(addr,
...
TEST_F(rbm_test_t, mkfs_test)
{
    run_async([this] {
        ...
        ASSERT_TRUE(super.block_size == block_size &&
                    super.size == size);
        config.spec.id = DEVICE_ID_NULL;
        mkfs();
        super = read_rbm_header();
        ASSERT_TRUE(super.config.spec.id == DEVICE_ID_NULL &&
                    super.size == size);
    }
             );
}
    ...
    run_async([this] {
        auto content = generate_extent(1); {
            write(block_size,
                  content);
            auto bp =
            bufferptr(ceph::buffer::create_page_aligned(block_size));
            read(block_size, bp); bufferlist bl; bufferlist block;
            bl.append(bp); block.append(content);
