...
template < typename PtrType > class local_shared_foreign_ptr
{
    ...
    friend local_shared_foreign_ptr < T > make_local_shared_foreign(seastar::foreign_ptr < T > &&);
...
public:
    ...
    local_shared_foreign_ptr(const local_shared_foreign_ptr &other) =
        default;
    ...
    seastar::future < seastar::foreign_ptr <
    PtrType >> get_foreign()const noexcept
    {
        assert(!ptr || (ptr && *ptr));
        return ptr ? ptr->copy() :
               seastar::make_ready_future < seastar::foreign_ptr <
               PtrType >> (nullptr);
    }
    ...
    local_shared_foreign_ptr &operator=(const local_shared_foreign_ptr &
                                        other)noexcept
    {
        ptr = other.ptr;
        return *this;
...
template < typename T >
local_shared_foreign_ptr < T > make_local_shared_foreign(seastar::foreign_ptr < T > &&ptr)
{
...
inline bool operator==(std::nullptr_t,
                       const local_shared_foreign_ptr < T > &y)
{
...
template < typename T >
struct hash <crimson::local_shared_foreign_ptr < T >>: private hash <
            typename std::pointer_traits < T >::element_type * > {
    size_t operator()(const crimson::local_shared_foreign_ptr < T > &p)const
    {
        return hash < typename std::pointer_traits <
               T >::element_type * >::operator()(p.get());
    }
};
...
template < typename T >
struct is_smart_ptr <crimson::local_shared_foreign_ptr <
    T >>: std::true_type {
};
