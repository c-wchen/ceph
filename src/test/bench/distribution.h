...
template < typename T, typename U, typename V, typename W > class FourTupleDist:public Distribution < boost::tuple < T, U, V,
    W >
    > {
  ...
  public:
    FourTupleDist(Distribution < T > *t,
                  Distribution < U > *u,
                  Distribution < V > *v, Distribution < W > *w)
  :    t(t), u(u), v(v), w(w) {
    }
...
template < typename T > class RandomDist:public Distribution < T > {
    rngen_t rng;
...
template < typename T > class WeightedDist:public Distribution < T > {
    rngen_t rng;
    double total;
    std::map < double, T > contents;
  public:
    WeightedDist(const rngen_t & rng, const std::set < std::pair < double,
                 T > >&initial)
    :rng(rng), total(0) {
        for (typename std::set < std::pair < double, T > >::const_iterator i =
             initial.begin(); i != initial.end(); ++i) {
            total += i->first;
            contents.insert(std::make_pair(total, i->second));
        }
    }
    T operator() () override {
        return contents.lower_bound(boost::uniform_real <> (0, total) (rng))->
            second;
    }
};

template < typename T, typename U > class SequentialDist:public Distribution < T >
{
    rngen_t rng;
...
class UniformRandom:public Distribution < uint64_t > {
    ...
    uint64_t max;
  public:
    UniformRandom(const rngen_t & rng, uint64_t min, uint64_t max):rng(rng),
        min(min), max(max) {
...
class Align:public Distribution < uint64_t > {
    ...
    uint64_t align;
  public:
    Align(Distribution < uint64_t > *dist, uint64_t align):dist(dist),
        align(align) {
    } uint64_t operator() () override {
        uint64_t ret = (*dist) ();
