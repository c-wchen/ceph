...
} void SnapshotRemoveRequest::load_map()
{
    ...
    auto rados_completion = librbd::util::create_rados_callback <
                            ...
                            &SnapshotRemoveRequest::handle_load_map > (this);
    int r =
...
void SnapshotRemoveRequest::remove_snapshot()
{
    ...
    CephContext *cct = m_image_ctx.cct;
    std::string oid(ObjectMap <>::
                    object_map_name(m_image_ctx.id, m_next_snap_id));
...
void SnapshotRemoveRequest::remove_map()
{
    ...
    auto rados_completion = librbd::util::create_rados_callback <
                            SnapshotRemoveRequest,
                            &SnapshotRemoveRequest::handle_remove_map > (this);
    int r = m_image_ctx.md_ctx.aio_operate(oid, rados_completion, &op);
    ceph_assert(r == 0);
    rados_completion->release();
}
...
void SnapshotRemoveRequest::update_object_map()
{
    ...
    std::unique_lock object_map_locker {
        *m_object_map_lock};
    if (m_next_snap_id == m_image_ctx.snap_id
        && m_next_snap_id == CEPH_NOSNAP) {
        CephContext *cct = m_image_ctx.cct;
        ldout(cct, 5) << dendl;
