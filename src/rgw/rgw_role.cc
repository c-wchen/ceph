...
int RGWRole::store_info(bool exclusive)
{
    ...
    return rgw_put_system_obj(store, store->get_zone_params().roles_pool, oid,
                              bl.c_str(), bl.length(), exclusive, NULL,
                              real_time(), NULL);
}
...
{
    string oid =
        tenant + get_path_oid_prefix() + path + get_info_oid_prefix() + id;
...
int RGWRole::create(bool exclusive)
{
    ...
    if (exclusive && ret == 0) {
        ldout(cct,
              0) << "ERROR: name " << name << " already in use for role id " <<
            id << dendl;
        return -EEXIST;
    }
    ...
    if (ret < 0) {
        ldout(cct,
            ...
            name << ": " << id << ": " << cpp_strerror(-ret) << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldout(cct,
            ...
            name << ": " << name << ": " << cpp_strerror(-ret) << dendl;
        ...
        if (info_ret < 0) {
            ldout(cct,
                ...
                name << ": " << id << ": " << cpp_strerror(-info_ret) << dendl;
        }
        return ret;
    }
    ...
    if (ret < 0) {
        ldout(cct,
            ...
            name << ": " << path << ": " << cpp_strerror(-ret) << dendl;
        //Delete the role info that was stored in the previous call
        string oid = get_info_oid_prefix() + id;
        int info_ret = rgw_delete_system_obj(store, pool, oid, NULL);
        if (info_ret < 0) {
        ...
        if (name_ret < 0) {
            ldout(cct,
                ...
                name << ": " << name << ": " << cpp_strerror(-name_ret) <<
                dendl;
        }
        return ret;
    }
...
int RGWRole::get_role_policy(const string & policy_name, string & perm_policy)
{
    ...
    if (it == perm_policy_map.end()) {
        ldout(cct,
              0) << "ERROR: Policy name: " << policy_name << " not found" <<
            dendl;
        return -EINVAL;
    }
    else {
...
void RGWRole::dump(Formatter * f) const const
{
    encode_json("id", id, f);
...
int RGWRole::read_id(const string & role_name, const string & tenant,
                     string & role_id)
{
    auto & pool = store->get_zone_params().roles_pool;
                                      ...
                                      vector < RGWRole > &roles)
{
    ...
    do {
        list < string > oids;
        int r =
            store->list_raw_objects(pool, prefix, 1000, ctx, oids,
                                    &is_truncated);
        if (r < 0) {
            ldout(cct,
