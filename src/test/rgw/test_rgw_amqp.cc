...
TEST_F(TestAMQP, InvalidPort)
{
    const auto connection_number = amqp::get_connection_count();
    conn =
        amqp::connect("amqp://localhost:1234", "ex1", false, false,
                      boost::none);
    EXPECT_TRUE(conn);
    EXPECT_EQ(amqp::get_connection_count(), connection_number + 1);
    auto rc = amqp::publish(conn, "topic", "message");
...
TEST_F(TestAMQP, InvalidVhost)
{
    const auto connection_number = amqp::get_connection_count();
    conn =
        amqp::connect("amqp://localhost/kaboom", "ex1", false, false,
                      boost::none);
    EXPECT_TRUE(conn);
    EXPECT_EQ(amqp::get_connection_count(), connection_number + 1);
    auto rc = amqp::publish(conn, "topic", "message");
...
TEST_F(TestAMQP, UserPassword)
{
    amqp_mock::set_valid_host("127.0.0.1");
    {
        const auto connection_number = amqp::get_connection_count();
        conn =
            amqp::connect("amqp://foo:bar@127.0.0.1", "ex1", false, false,
                          boost::none);
        EXPECT_TRUE(conn);
        EXPECT_EQ(amqp::get_connection_count(), connection_number + 1);
        auto rc = amqp::publish(conn, "topic", "message");
    ...
    amqp_mock::set_valid_host("127.0.0.2");
    {
        const auto connection_number = amqp::get_connection_count();
        conn =
            amqp::connect("amqp://guest:guest@127.0.0.2", "ex1", false, false,
                          boost::none);
        EXPECT_TRUE(conn);
        EXPECT_EQ(amqp::get_connection_count(), connection_number + 1);
        auto rc = amqp::publish(conn, "topic", "message");
...
TEST_F(TestAMQP, MaxConnections)
{
    ...
    while (remaining_connections > 0) {
        ...
        amqp::connection_ptr_t conn =
            amqp::connect("amqp://" + host, "ex1", false, false, boost::none);
        EXPECT_TRUE(conn);
        auto rc = amqp::publish(conn, "topic", "message");
        EXPECT_EQ(rc, 0);
...
TEST_F(TestAMQP, ReceiveAck)
{
    ...
    EXPECT_TRUE(conn);
    auto rc =
        publish_with_confirm(conn, "topic", "message", my_callback_expect_ack);
    EXPECT_EQ(rc, 0);
    wait_until_drained();
...
TEST_F(TestAMQP, ImplicitConnectionClose)
{
    ...
    for (auto i = 0; i < NUMBER_OF_CALLS; ++i) {
        auto rc =
            publish_with_confirm(conn, "topic", "message",
                                 my_callback_expect_close_or_ack);
        EXPECT_EQ(rc, 0);
    }
    wait_until_drained();
...
TEST_F(TestAMQP, ReceiveMultipleAck)
{
    ...
    for (auto i = 0; i < NUMBER_OF_CALLS; ++i) {
        auto rc =
            publish_with_confirm(conn, "topic", "message",
                                 my_callback_expect_multiple_acks);
        EXPECT_EQ(rc, 0);
    }
    wait_until_drained();
...
TEST_F(TestAMQP, ReceiveNack)
{
    ...
    EXPECT_TRUE(conn);
    auto rc =
        publish_with_confirm(conn, "topic", "message", my_callback_expect_nack);
    EXPECT_EQ(rc, 0);
    wait_until_drained();
    EXPECT_TRUE(callback_invoked);
...
TEST_F(TestAMQP, RetryInvalidPort)
{
    ...
    const auto connection_number = amqp::get_connection_count();
    conn =
        amqp::connect("amqp://localhost:" + std::to_string(port), "ex1", false,
                      false, boost::none);
    EXPECT_TRUE(conn);
    EXPECT_EQ(amqp::get_connection_count(), connection_number + 1);
    auto rc = amqp::publish(conn, "topic", "message");
...
TEST_F(TestAMQP, RetryFailWrite)
{
    ...
    EXPECT_TRUE(callback_invoked);
    callback_invoked = false;
    rc = publish_with_confirm(conn, "topic", "message",
                              my_callback_expect_nack);
    EXPECT_LT(rc, 0);
