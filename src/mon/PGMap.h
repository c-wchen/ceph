...
namespace ceph {
   ...
   */
    mempool::pgmap::unordered_map < uint64_t, pair < pool_stat_t,
        utime_t > >per_pool_sum_delta;
   ...
   */
    ...
    // kill me post-mimic or -nautilus bool definitely_converted_snapsets() const {
        // false negative is okay; false positive is not!
        return
            num_pg &&
            num_pg_unknown == 0 && pg_sum.stats.sum.num_legacy_snapsets == 0;
    }
    // kill me post-luminous: virtual float get_fallback_full_ratio() const {
        return .95;
...
class PGMap:public PGMapDigest {
  public:
    ...
    class Incremental {
      ...
      public:
        ...
        const mempool::pgmap::map < int32_t,
            osd_stat_t > &get_osd_stat_updates() const {
            ...
            assert(osd_epochs.size() == osd_stat_updates.size());
        } void stat_osd_down_up(int32_t osd, epoch_t epoch,
                                const PGMap & pg_map) {
  ...
  public:
    ...
    mempool::pgmap::map < int, map < epoch_t,
        set < pg_t > >>creating_pgs_by_osd_epoch;
  ...
  :    
    version(0),
    last_osdmap_epoch(0), last_pg_scan(0), full_ratio(0), nearfull_ratio(0) {
    }
    ...
    void stat_pg_sub(const pg_t & pgid, const pg_stat_t & s,
                     bool sameosds = false);
    void stat_pg_update(const pg_t pgid, pg_stat_t & prev,
    ...
    int dump_stuck_pg_stats(stringstream & ds,
                            Formatter * f,
                            int threshold, vector < string > &args) const;
    ...
    void get_health_checks(CephContext * cct,
                           const OSDMap & osdmap,
...
int process_pg_map_command(const string & prefix,
                           const map < string, cmd_vartype > &cmdmap,
                           const PGMap & pg_map,
                           const OSDMap & osdmap,
                           Formatter * f,
                           stringstream * ss, bufferlist * odata);

class PGMapUpdater {
  public:
    static void check_osd_map(const OSDMap::Incremental & osd_inc,
                              std::set < int >*need_check_down_pg_osds,
                              std::map < int, utime_t > *last_osd_report,
                              PGMap * pg_map, PGMap::Incremental * pending_inc);

    static void check_osd_map(CephContext * cct,
                              const OSDMap & osdmap,
                              const PGMap & pg_map,
   ...
   */
    static void register_new_pgs(const OSDMap & osd_map,
                                 const PGMap & pg_map,
                                 PGMap::Incremental * pending_inc);
   ...
   */
    static void update_creating_pgs(const OSDMap & osd_map,
                                    const PGMap & pg_map,
                                    PGMap::Incremental * pending_inc);

    static void register_pg(const OSDMap & osd_map,
    ...
    static void check_down_pgs(const OSDMap & osd_map,
                               const PGMap & pg_map,
                               bool check_all,
 ...
 */
    int by_utilization(const OSDMap & osd_map,
                       ...
                       std::stringstream * ss,
                       std::string * out_str, Formatter * f);
  ...
  public:
    ...
    const osd_stat_t & get_osd_sum() const override {
    ...
    const mempool::pgmap::unordered_map < int32_t,
        osd_stat_t > &get_osd_stat() const override {
        return pgmap.osd_stat;
    } float get_full_ratio() const override {
        return pgmap.full_ratio;
    } float get_nearfull_ratio() const override {
        return pgmap.nearfull_ratio;
    ...
    } bool have_full_osds() const override {
        return !pgmap.full_osds.empty();
    ...
    } void dump_pool_stats(const OSDMap & osdm, stringstream * ss,
                           Formatter * f, bool verbose) const override {
