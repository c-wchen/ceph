...
public:
    C_ImportDiff(ImportDiffContext *idiffctx, bufferlist data,
        ...
        : m_idiffctx(idiffctx), m_data(data), m_offset(offset),
          m_length(length), m_write_zeroes(write_zeroes)
    {
    ...
    int send()
    {
        ...
        int r;
        if (m_write_zeroes) {
            r = m_idiffctx->image->aio_write_zeroes(m_offset,
                                                    m_length,
                                                    aio_completion,
                                                    0U,
...
static int do_image_io(ImportDiffContext *idiffctx,
                       bool write_zeroes, size_t sparse_size)
{
    int r;
    char buf[16];
    r = safe_read_exact(idiffctx->fd, buf, sizeof(buf));
                        ...
                        size_t sparse_size)
{
    ...
    if (from_stdin) {
        ...
        throttle.
        reset(new
              SimpleThrottle(g_conf().get_val < uint64_t >
