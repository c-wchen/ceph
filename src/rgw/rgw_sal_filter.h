...
namespace rgw {
    namespace sal {
        ...
        class FilterCompletions:public Completions {
          ...
          public:
            FilterCompletions(std::unique_ptr < Completions >
                              _next):next(std::move(_next)) {
        ...
        class FilterPlacementTier:public PlacementTier {
          ...
          public:
            FilterPlacementTier(std::unique_ptr < PlacementTier >
                                _next):next(std::move(_next)) {
            ...
            virtual const std::string & get_storage_class() override {
                return next->get_storage_class();
            }
            virtual bool retain_head_object() override {
                return next->retain_head_object();
            }

            /* Internal to Filters */
            PlacementTier *get_next() {
                return next.get();
            }
        };
        ...
        class FilterZoneGroup:public ZoneGroup {
          ...
          public:
            FilterZoneGroup(std::unique_ptr < ZoneGroup >
                            _next):next(std::move(_next)) {
            ...
            } virtual int get_placement_target_names(std::set < std::string >
                                                     &names) const override {
                return next->get_placement_target_names(names);
            } virtual const std::
                string & get_default_placement_name() const override {
                return next->get_default_placement_name();
            } virtual int get_hostnames(std::list < std::string >
                                        &names) const override {
                return next->get_hostnames(names);
            } virtual int get_s3website_hostnames(std::list < std::string >
                                                  &names) const override {
        ...
        class FilterZone:public Zone {
          ...
          public:
            FilterZone(std::unique_ptr < Zone > _next):next(std::move(_next)) {
                group =
                    std::make_unique < FilterZoneGroup >
                    (next->get_zonegroup().clone());
            ...
            virtual bool has_zonegroup_api(const std::
                                           string & api)const override {
            ...
            virtual RGWBucketSyncPolicyHandlerRef get_sync_policy_handler()
                override {
        ...
        class FilterDriver:public Driver {
          ...
          public:
            ...
            virtual std::unique_ptr < User >
                get_user(const rgw_user & u) override;
            ...
            virtual std::unique_ptr < Object >
                get_object(const rgw_obj_key & k) override;
            ...
            virtual int forward_request_to_master(const DoutPrefixProvider *
                                                  dpp, User * user,
                                                  obj_version * objv,
                                                  bufferlist & in_data,
                                                  JSONParser * jp,
                                                  req_info & info,
                                                  optional_yield y) override;
            virtual int forward_iam_request_to_master(const DoutPrefixProvider *
                                                      dpp,
            ...
            virtual std::
                string zone_unique_trans_id(const uint64_t unique_num)override;
            virtual int get_zonegroup(const std::string & id,
                                      std::unique_ptr < ZoneGroup >
                                      *zonegroup) override;
            ...
            virtual std::unique_ptr < Notification >
                get_notification(rgw::sal::Object * obj,
                                 ...
                                 nullptr) override;
            virtual std::unique_ptr < Notification >
                get_notification(const DoutPrefixProvider * dpp,
                                 rgw::sal::Object * obj,
                                 rgw::sal::Object * src_obj,
                                 rgw::notify::EventType event_type,
            ...
            int read_topics(const std::string & tenant,
            ...
            int write_topics(const std::string & tenant,
            ...
            int remove_topics(const std::string & tenant,
                              RGWObjVersionTracker * objv_tracker,
                              optional_yield y,
                              const DoutPrefixProvider * dpp) override {
            ...
            virtual int log_op(const DoutPrefixProvider * dpp,
                               std::string & oid, bufferlist & bl) override;
            ...
            virtual int get_sync_policy_handler(const DoutPrefixProvider * dpp,
                                                ...
                                                phandler,
                                                optional_yield y) override;
            virtual RGWDataSyncStatusManager *get_data_sync_manager(const
                                                                    rgw_zone_id
                                                                    &
                                                                    source_zone)
                override;
            virtual void wakeup_meta_sync_shards(std::set <
                                                 int >&shard_ids) override;
            ...
            virtual std::unique_ptr < RGWRole >
                get_role(std::string id) override;
            virtual std::unique_ptr < RGWRole >
                get_role(const RGWRoleInfo & info)override;
            virtual int get_roles(const DoutPrefixProvider * dpp,
                                  optional_yield y,
                                  const std::string & path_prefix,
                                  const std::string & tenant,
                                  std::vector < std::unique_ptr <
                                  RGWRole >> &roles) override;
            ...
            virtual int get_oidc_providers(const DoutPrefixProvider * dpp,
                ...
                override;
            virtual std::unique_ptr < Writer >
                get_append_writer(const DoutPrefixProvider * dpp,
            ...
            virtual std::unique_ptr < Writer >
                get_atomic_writer(const DoutPrefixProvider * dpp,
                                  optional_yield y, rgw::sal::Object * obj,
                                  const rgw_user & owner,
                                  const rgw_placement_rule *
            ...
            virtual const std::
                string & get_compression_type(const rgw_placement_rule & rule)
                override;
            virtual bool valid_placement(const rgw_placement_rule & rule)
                override;
        ...
        class FilterUser:public User {
          ...
          public:
            ...
            virtual int create_bucket(const DoutPrefixProvider * dpp,
                                      ...
                                      Attrs & attrs, RGWBucketInfo & info,
                                      obj_version & ep_objv, bool exclusive,
                                      bool obj_lock_enabled, bool * existed,
                                      req_info & req_info,
                                      std::unique_ptr < Bucket > *bucket,
                                      optional_yield y) override;
            ...
            virtual const std::string & get_tenant() override {
                return next->get_tenant();
            }
            virtual void set_tenant(std::string & _t) override {
                next->set_tenant(_t);
            }
            virtual const std::string & get_ns() override {
                return next->get_ns();
            }
            virtual void set_ns(std::string & _ns) override {
                next->set_ns(_ns);
            }
            virtual void clear_ns() override {
                next->clear_ns();
            }
            virtual const rgw_user & get_id() const override {
                return next->get_id();
            } virtual uint32_t get_type() const override {
                return next->get_type();
            } virtual int32_t get_max_buckets() const override {
                return next->get_max_buckets();
            } virtual const RGWUserCaps & get_caps() const override {
            ...
            virtual void set_attrs(Attrs & _attrs) override {
            ...
            virtual int complete_flush_stats(const DoutPrefixProvider * dpp,
                                             optional_yield y) override;
            virtual int read_usage(const DoutPrefixProvider * dpp,
                                   uint64_t start_epoch, uint64_t end_epoch,
                                   uint32_t max_entries, bool * is_truncated,
                                   RGWUsageIter & usage_iter,
                                   std::map < rgw_user_bucket,
                                   rgw_usage_log_entry > &usage) override;
            virtual int trim_usage(const DoutPrefixProvider * dpp,
                                   uint64_t start_epoch,
                                   uint64_t end_epoch) override;

            virtual int load_user(const DoutPrefixProvider * dpp,
                                  optional_yield y) override;
            virtual int store_user(const DoutPrefixProvider * dpp,
            ...
            /* Internal to Filters */ User *get_next() {
                return next.get();
            }
        };
        ...
        class FilterBucket:public Bucket {
          ...
          public:
            ...
            FilterBucket(std::unique_ptr < Bucket > _next,
                         User * _user):next(std::move(_next)), user(_user) {
            } virtual ~ FilterBucket() = default;

            virtual std::unique_ptr < Object >
                get_object(const rgw_obj_key & key) override;
            ...
            virtual int sync_user_stats(const DoutPrefixProvider * dpp,
                                        optional_yield y) override;
            virtual int update_container_stats(const DoutPrefixProvider *
                                               dpp) override;
            virtual int check_bucket_shards(const DoutPrefixProvider *
                                            dpp) override;
            ...
            virtual User *get_owner(void) override {
                return user;
            }
            virtual ACLOwner get_acl_owner(void)override {
                return next->get_acl_owner();
            }
            ...
            virtual bool empty() const override {
                ...
                return next->get_name();
            } virtual const std::string & get_tenant() const override {
                return next->get_tenant();
            } virtual const std::string & get_marker() const override {
                return next->get_marker();
            } virtual const std::string & get_bucket_id() const override {
                return next->get_bucket_id();
            } virtual size_t get_size() const override {
                return next->get_size();
            } virtual size_t get_size_rounded() const override {
                return next->get_size_rounded();
            } virtual uint64_t get_count() const override {
                return next->get_count();
            } virtual rgw_placement_rule & get_placement_rule() override {
                return next->get_placement_rule();
            }
            virtual ceph::real_time & get_creation_time() override {
                return next->get_creation_time();
            }
            virtual ceph::real_time & get_modification_time() override {
                return next->get_modification_time();
            }
            virtual obj_version & get_version() override {
                return next->get_version();
            }
            virtual void set_version(obj_version & ver) override {
                next->set_version(ver);
            }
            virtual bool versioned() override {
                return next->versioned();
            }
            virtual bool versioning_enabled() override {
            ...
            virtual std::unique_ptr < MultipartUpload >
                get_multipart_upload(const std::string & oid,
                                     std::optional < std::string > upload_id =
                                     std::nullopt, ACLOwner owner = {
            ...
            int read_topics(rgw_pubsub_bucket_topics & notifications,
                            RGWObjVersionTracker * objv_tracker,
                            optional_yield y,
                            const DoutPrefixProvider * dpp) override {
            ...
            virtual rgw_bucket & get_key() override {
                return next->get_key();
            }
        ...
        class FilterObject:public Object {
          ...
          public:
            ...
            struct FilterReadOp:ReadOp {
                ...
                FilterReadOp(std::unique_ptr < ReadOp >
                             _next):next(std::move(_next)) {
                ...
                virtual int iterate(const DoutPrefixProvider * dpp, int64_t ofs,
                                    ...
                                    optional_yield y) override;
                virtual int get_attr(const DoutPrefixProvider * dpp,
            ...
            struct FilterDeleteOp:DeleteOp {
                ...
                FilterDeleteOp(std::unique_ptr < DeleteOp >
                               _next):next(std::move(_next)) {
          ...
          FilterObject(std::unique_ptr < Object > _next):next(std::
                 move(_next))
            {
            }
          FilterObject(std::unique_ptr < Object > _next, Bucket * _bucket):
            ...
            virtual int delete_obj_aio(const DoutPrefixProvider * dpp,
                                       RGWObjState * astate, Completions * aio,
                                       bool keep_index_consistent,
                                       optional_yield y) override;
            virtual int copy_object(User * user, req_info * info,
                                    ...
                                    bool copy_if_newer, Attrs & attrs,
                                    RGWObjCategory category, uint64_t olh_epoch,
                                    boost::optional < ceph::real_time >
                                    delete_at, std::string * version_id,
                                    std::string * tag, std::string * etag,
            ...
            virtual int set_obj_attrs(const DoutPrefixProvider * dpp,
                                      ...
                                      optional_yield y) override;
            virtual int get_obj_attrs(optional_yield y,
                                      const DoutPrefixProvider * dpp,
                                      rgw_obj * target_obj = NULL) override;
            virtual int modify_obj_attrs(const char *attr_name,
                                         bufferlist & attr_val,
                                         optional_yield y,
            ...
            virtual std::unique_ptr < MPSerializer >
                get_serializer(const DoutPrefixProvider * dpp,
            ...
            virtual bool placement_rules_match(rgw_placement_rule & r1,
                ...
                override;
            virtual int dump_obj_layout(const DoutPrefixProvider * dpp,
                                        optional_yield y,
            ...
            virtual ceph::real_time get_mtime(void)const override {
                return next->get_mtime();
            };
            virtual uint64_t get_obj_size(void)const override {
                return next->get_obj_size();
            };
            virtual Bucket *get_bucket(void)const override {
                return bucket;
            };
            virtual void set_bucket(Bucket * b) override;
            virtual std::string get_hash_source(void) override {
                return next->get_hash_source();
            };
            virtual void set_hash_source(std::string s) override {
                return next->set_hash_source(s);
            };
            virtual std::string get_oid(void)const override {
                return next->get_oid();
            };
            virtual bool get_delete_marker(void)override {
                return next->get_delete_marker();
            };
            virtual bool get_in_extra_data(void)override {
                return next->get_in_extra_data();
            };
            virtual void set_in_extra_data(bool i) override {
            ...
            virtual void set_obj_size(uint64_t s) override {
                return next->set_obj_size(s);
            };
            virtual void set_name(const std::string & n) override {
                return next->set_name(n);
            };
            virtual void set_key(const rgw_obj_key & k) override {
                return next->set_key(k);
            };
            virtual rgw_obj get_obj(void)const override {
                return next->get_obj();
            };
            virtual rgw_obj_key & get_key() override {
                return next->get_key();
            }
            virtual void set_instance(const std::string & i) override {
                return next->set_instance(i);
            }
            virtual const std::string & get_instance() const override {
                return next->get_instance();
            } virtual bool have_instance(void)override {
                return next->have_instance();
            }
            virtual void clear_instance() override {
            ...
            virtual int omap_get_vals(const DoutPrefixProvider * dpp,
                                      const std::string & marker,
                                      uint64_t count, std::map < std::string,
                                      bufferlist > *m, bool * pmore,
                                      optional_yield y) override;
            ...
            /* Internal to Filters */ Object *get_next() {
                return next.get();
            }
        };
        ...
        class FilterMultipartPart:public MultipartPart {
          ...
          public:
            FilterMultipartPart(std::unique_ptr < MultipartPart >
                                _next):next(std::move(_next)) {
        ...
        class FilterMultipartUpload:public MultipartUpload {
          ...
          public:
             FilterMultipartUpload(std::unique_ptr < MultipartUpload > _next,
                                   Bucket * _b):next(std::move(_next)),
                bucket(_b) {
            } virtual ~ FilterMultipartUpload() = default;

            virtual const std::string & get_meta() const override {
                return next->get_meta();
            } virtual const std::string & get_key() const override {
                return next->get_key();
            ...
            virtual std::map < uint32_t,
                std::unique_ptr < MultipartPart >> &get_parts() override {
                return parts;
            }

            virtual const jspan_context & get_trace() override {
                return next->get_trace();
            }

            virtual std::unique_ptr < rgw::sal::Object >
            ...
            virtual int list_parts(const DoutPrefixProvider * dpp,
                                   CephContext * cct, int num_parts, int marker,
                                   int *next_marker, bool * truncated,
                                   bool assume_unsorted = false) override;
            virtual int abort(const DoutPrefixProvider * dpp,
                              CephContext * cct) override;
            ...
            virtual std::unique_ptr < Writer >
                get_writer(const DoutPrefixProvider * dpp, optional_yield y,
        ...
        class FilterMPSerializer:public MPSerializer {
          ...
          public:
            FilterMPSerializer(std::unique_ptr < MPSerializer >
                               _next):next(std::move(_next)) {
        ...
        class FilterLCSerializer:public LCSerializer {
          ...
          public:
            FilterLCSerializer(std::unique_ptr < LCSerializer >
                               _next):next(std::move(_next)) {
        ...
        class FilterLifecycle:public Lifecycle {
          ...
          public:
            struct FilterLCHead:LCHead {
                ...
                FilterLCHead(std::unique_ptr < LCHead >
                             _next):next(std::move(_next)) {
                } virtual ~ FilterLCHead() = default;

                virtual time_t & get_start_date() override {
                    return next->get_start_date();
                } virtual void set_start_date(time_t t) override {
                    next->set_start_date(t);
                }
                virtual std::string & get_marker() override {
                    return next->get_marker();
                }
                virtual void set_marker(const std::string & m) override {
                    next->set_marker(m);
                }
                virtual time_t & get_shard_rollover_date() override {
                    return next->get_shard_rollover_date();
                }
                virtual void set_shard_rollover_date(time_t t) override {
                    next->set_shard_rollover_date(t);
                }
            };
            ...
            struct FilterLCEntry:LCEntry {
                ...
                FilterLCEntry(std::unique_ptr < LCEntry >
                              _next):next(std::move(_next)) {
                } virtual ~ FilterLCEntry() = default;

                virtual std::string & get_bucket() override {
                    return next->get_bucket();
                }
                virtual void set_bucket(const std::string & b) override {
                    next->set_bucket(b);
                }
                virtual std::string & get_oid() override {
                    return next->get_oid();
                }
                virtual void set_oid(const std::string & o) override {
                    next->set_oid(o);
                }
                virtual uint64_t get_start_time() override {
                    return next->get_start_time();
                }
                virtual void set_start_time(uint64_t t) override {
                    next->set_start_time(t);
                }
                virtual uint32_t get_status() override {
                    return next->get_status();
                }
                virtual void set_status(uint32_t s) override {
                    next->set_status(s);
                }
                virtual void print(std::ostream & out) const override {
                    return next->print(out);
          ...
          FilterLifecycle(std::unique_ptr < Lifecycle > _next):next(std::
                 move(_next))
            {
            }
            ...
            virtual int list_entries(const std::string & oid,
                                     ...
                                     uint32_t max_entries,
                                     std::vector < std::unique_ptr <
                                     LCEntry >> &entries) override;
            virtual int rm_entry(const std::string & oid,
                                 LCEntry & entry) override;
            virtual int get_head(const std::string & oid,
                                 std::unique_ptr < LCHead > *head) override;
            virtual int put_head(const std::string & oid,
                                 LCHead & head) override;
            virtual std::unique_ptr < LCSerializer >
                get_serializer(const std::string & lock_name,
                               const std::string & oid,
        ...
        class FilterNotification:public Notification {
          ...
          public:
            FilterNotification(std::unique_ptr < Notification >
                               _next):next(std::move(_next)) {
            ...
            virtual int publish_reserve(const DoutPrefixProvider * dpp,
                                        RGWObjTags * obj_tags =
                                        nullptr) override;
            virtual int publish_commit(const DoutPrefixProvider * dpp,
                                       uint64_t size,
        ...
        class FilterWriter:public Writer {
          protected:
            ...
            Object *obj;
          public:
             FilterWriter(std::unique_ptr < Writer > _next,
                          Object * _obj):next(std::move(_next)), obj(_obj) {
        ...
        class FilterLuaManager:public LuaManager {
          ...
          public:
            FilterLuaManager(std::unique_ptr < LuaManager >
                             _next):next(std::move(_next)) {
            ...
            virtual int put_script(const DoutPrefixProvider * dpp,
                                   optional_yield y, const std::string & key,
                                   const std::string & script) override;
            virtual int del_script(const DoutPrefixProvider * dpp,
                                   optional_yield y,
                                   const std::string & key) override;
            virtual int add_package(const DoutPrefixProvider * dpp,
                                    optional_yield y,
                                    const std::string & package_name) override;
            virtual int remove_package(const DoutPrefixProvider * dpp,
                                       optional_yield y,
                                       const std::
                                       string & package_name) override;
            virtual int list_packages(const DoutPrefixProvider * dpp,
                                      optional_yield y,
