...
class CrushWrapper {
  ...
  private:
    ...
    void build_rmaps() const {
        ...
        have_rmaps = true;
    } void build_rmap(const std::map < int, std::string > &f,
    ...
    // default bucket types unsigned get_default_bucket_alg() const {
    ...
    // bucket types int get_num_type_names() const {
    ...
    int can_rename_item(const std::string & srcname,
                        const std::string & dstname, std::ostream * ss) const;
    int rename_item(const std::string & srcname,
                    const std::string & dstname, std::ostream * ss);
   ...
   */
    std::pair < std::string, std::string > get_immediate_parent(int id,
                                                                int *ret =
                                                                NULL)const;
   ...
   */
    int get_full_location(const std::string & name,
     ...
     */
    int get_full_location_ordered(int id,
 ...
 /**
   ...
   */
    int verify_upmap(CephContext * cct,
                     int rule_id, int pool_size, const std::vector < int >&up);
   ...
   * @return 0 for success, negative on error
   */
    int move_bucket(CephContext * cct, int id, const std::map < std::string,
                    std::string > &loc);
   ...
   */
  private:
    bool _maybe_remove_last_instance(CephContext * cct, int id,
                                     bool unlink_only);
   ...
   */
    ...
    float get_item_weightf(int id) const {
        return (float)get_item_weight(id) / (float)0x10000;
                                 ...
                                 std::string > &loc);
    float get_item_weightf_in_loc(int id, const std::map < std::string,
                                  std::string > &loc) {
    ...
    int validate_weightf(float weight) {
        ...
        if (iweight > static_cast < uint64_t >
    ...
    int adjust_item_weight_in_bucket(CephContext * cct, int id, int weight,
                                     int bucket_id, bool update_weight_sets);
    int adjust_item_weight_in_loc(CephContext * cct, int id, int weight,
                                  const std::map < std::string,
                                  std::string > &loc, bool update_weight_sets =
                                  true);
    int adjust_item_weightf_in_loc(CephContext * cct, int id, float weight,
        ...
        return adjust_item_weight_in_loc(cct, id,
                                         (int)(weight * (float)0x10000), loc,
                                         update_weight_sets);
    }
  ...
  /*** rules ***/ private:
    ...
    float _get_take_weight_osd_map(int root, std::map < int,
                                   float >*pmap) const;
   ...
   */
    int get_rule_weight_osd_map(unsigned ruleno, std::map < int,
                                float >*pmap) const;
   ...
   */
    ...
    int set_rule_step(unsigned ruleno, unsigned step, int op, int arg1,
                      int arg2) {
        if (!crush)
            return -ENOENT;
        crush_rule *n = get_rule(ruleno);
    ...
    int set_rule_step_set_choose_local_tries(unsigned ruleno, unsigned step,
                                             int val) {
        return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSE_LOCAL_TRIES,
                             val, 0);
    }
    int set_rule_step_set_choose_local_fallback_tries(unsigned ruleno,
                                                      unsigned step, int val) {
        return set_rule_step(ruleno, step,
                             CRUSH_RULE_SET_CHOOSE_LOCAL_FALLBACK_TRIES, val,
                             0);
    }
    int set_rule_step_set_chooseleaf_tries(unsigned ruleno, unsigned step,
                                           int val) {
        return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSELEAF_TRIES, val,
                             0);
    }
    int set_rule_step_set_chooseleaf_vary_r(unsigned ruleno, unsigned step,
                                            int val) {
        return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSELEAF_VARY_R,
                             val, 0);
    }
    int set_rule_step_set_chooseleaf_stable(unsigned ruleno, unsigned step,
                                            int val) {
        return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSELEAF_STABLE,
                             val, 0);
    }
    int set_rule_step_choose_firstn(unsigned ruleno, unsigned step, int val,
                                    int type) {
        return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSE_FIRSTN, val, type);
    }
    int set_rule_step_choose_indep(unsigned ruleno, unsigned step, int val,
                                   int type) {
        return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSE_INDEP, val, type);
    }
    int set_rule_step_choose_leaf_firstn(unsigned ruleno, unsigned step,
                                         int val, int type) {
        return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSELEAF_FIRSTN, val,
                             type);
    }
    int set_rule_step_choose_leaf_indep(unsigned ruleno, unsigned step, int val,
                                        int type) {
        return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSELEAF_INDEP, val,
                             type);
    }
    ...
    int add_simple_rule(std::string name, std::string root_name,
                        std::string failure_domain_type,
                        std::string device_class, std::string mode,
                        int rule_type, std::ostream * err = 0);

  /**
   * @param rno rule[set] id to use, -1 to pick the lowest available
   */
    int add_simple_rule_at(std::string name, std::string root_name,
                           std::string failure_domain_type,
                           std::string device_class, std::string mode,
  ...
  public:
    ...
    int bucket_adjust_item_weight(CephContext * cct,
                                  struct crush_bucket *bucket, int item,
    ...
    int device_class_clone(int original, int device_class,
                           const std::map < int32_t, std::map < int32_t,
                           int32_t >> &old_class_bucket,
                           const std::set < int32_t > &used_ids, int *clone,
                           std::map < int, std::map < int,
                           std::vector < int >>>*cmap_item_weight);
    ...
    int populate_classes(const std::map < int32_t, std::map < int32_t,
                         int32_t >> &old_class_bucket);
    int get_rules_by_class(const std::string & class_name,
                           std::set < int >*rules);
    ...
    int reclassify(CephContext * cct,
                   std::ostream & out,
                   const std::map < std::string, std::string > &classify_root,
                   const std::map < std::string, std::pair < std::string,
                   std::string >> &classify_bucket);

    int set_subtree_class(const std::string & name,
   ...
   */
    ...
    } crush_choose_arg_map choose_args_get_with_fallback(int64_t
                                                         choose_args_index)
        const {
    ...
    bool create_choose_args(int64_t id, int positions) {
        ...
        for (int bidx = 0; bidx < crush->max_buckets; ++bidx) {
            ...
            if (b && b->alg == CRUSH_BUCKET_STRAW2) {
                crush_bucket_straw2 *sb =
                    reinterpret_cast < crush_bucket_straw2 * >(b);
                carg.weight_set_positions = positions;
                carg.weight_set =
                    static_cast <
                    crush_weight_set *
                    >(calloc
                      (sizeof(crush_weight_set), carg.weight_set_positions));
    ...
    int _choose_args_adjust_item_weight_in_bucket(CephContext * cct,
                                                  ...
                                                  int >&weight,
                                                  std::ostream * ss);
    int choose_args_adjust_item_weight(CephContext * cct,
                                       crush_choose_arg_map cmap, int id,
                                       const std::vector < int >&weight,
                                       std::ostream * ss);
    int choose_args_adjust_item_weightf(CephContext * cct,
                                        crush_choose_arg_map cmap, int id,
    ...
    template < typename WeightVector >
        ...
        for (int i = 0; i < numrep; i++)
             out[i] = rawout[i];
    } int _choose_type_stack(CephContext * cct,
    ...
    int try_remap_rule(CephContext * cct,
                       int rule,
                       int maxout,
                       const std::set < int >&overfull,
                       const std::vector < int >&underfull,
