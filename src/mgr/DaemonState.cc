...
string DeviceState::get_life_expectancy_str(utime_t now) const const
{
...
void DeviceState::dump(Formatter *f) const const
{
...
void DeviceState::print(ostream &out) const const
{
    out << "device " << devid << "\n";
    for (auto &i : attachments) {
        out << "attachment " << std::get < 0 > (i) << " " << std::get < 1 >
            (i) << " " << std::get < 2 > (i) << "\n";
        out << "\n";
    }
...
void DaemonState::set_metadata(const std::map < std::string, std::string > &m)
{
    ...
    if (auto found = m.find("device_ids"); found != m.end()) {
        ...
        for_each_pair(device_ids, ",; ",
                      [&paths, this](std::string_view devname,
        std::string_view id) {
            // skip blank ids
            if (id.empty()) {
                return;
            ...
            if (auto path = paths.find(std::string(id));
                path != paths.end()) {
                // id -> path
...
void DaemonStateIndex::_insert(DaemonStatePtr dm)
{
    ...
    for (auto &i : dm->devices) {
        ...
        if (p != dm->devices_bypath.end()) {
            d->attachments.
            insert(std::make_tuple(dm->hostname, i.second, p->second));
        } else {
...
void DaemonStateIndex::_erase(const DaemonKey &dmk)
{
    ...
    for (auto &i : dm->devices) {
        ...
        if (p != dm->devices_bypath.end()) {
            ...
            d->attachments.
            erase(make_tuple(dm->hostname, i.second, std::string()));
        }
        if (d->empty()) {
            _erase_device(d);
        }
...
DaemonStateCollection DaemonStateIndex::get_by_service(const std::string &svc) const const
{
    std::shared_lock l {
        lock};
...
DaemonStateCollection DaemonStateIndex::get_by_server(const std::
...
bool DaemonStateIndex::exists(const DaemonKey &key) constconst {
    std::shared_lock l {
        lock};
...
void DaemonStateIndex::cull_services(const std::set < std::string >
                                     &types_exist)
{
