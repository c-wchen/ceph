        ...
        class mutex_debugging_base {
          protected:
            ...
            void _will_unlock();    // about to unlock
             ...
             mutex_debugging_base(const std::string & n =
                                  std::string(), bool bt =
                                  false, CephContext * cct = nullptr);
            ~mutex_debugging_base();

             ceph::mono_time before_lock_blocks();
            void after_lock_blocks(ceph::mono_time start, bool no_lockdep);
         ...
         template < typename Mutex >
          ...
          public:
           mutex_debugging(const std::string & n = std::string(), bool bt = false, CephContext * cct = nullptr):
            mutex_debugging_base(n, bt, cct),
                impl(static_cast < Mutex * >(this)) {
...
// pthread error check mutex.
      template < bool Recursive > class mutex_debug_impl:public mutex_debugging < mutex_debug_impl < Recursive >
            > {
          private:
            pthread_mutex_t m;
          public:
          ...
          mutex_debug_impl(const std::string & n = std::string(), bool bt = false, CephContext * cct = nullptr):
