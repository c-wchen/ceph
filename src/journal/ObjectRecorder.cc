...
namespace journal {
    ...
    :RefCountedObject(NULL, 0), m_oid(oid), m_object_number(object_number),
        ...
        m_flush_handler(this), m_lock(lock), m_append_tid(0),
        m_pending_bytes(0), m_size(0), m_overflowed(false),
    ...
    void ObjectRecorder::schedule_append_task() {
        ...
        if (m_append_task == nullptr && m_flush_age > 0) {
            m_append_task =
                m_timer.add_event_after(m_flush_age,
    ...
    bool ObjectRecorder::flush_appends(bool force) {
        ...
        if (m_append_buffers.empty() ||
            (!force &&
             m_size + m_pending_bytes < m_soft_max_size &&
             (m_flush_interval > 0
              && m_append_buffers.size() < m_flush_interval)
    ...
    void ObjectRecorder::send_appends_aio() {
        ...
        C_Gather *gather_ctx;
        {
            ...
            ldout(m_cct,
                  10) << __func__ << ": " << m_oid << " flushing journal tid="
                << append_tid << dendl;

            gather_ctx =
                new C_Gather(m_cct, new C_AppendFlush(this, append_tid));
            auto append_buffers = &m_in_flight_appends[append_tid];

            for (auto it = m_pending_buffers.begin();
                 it != m_pending_buffers.end();) {
                ldout(m_cct,
                      20) << __func__ << ": flushing " << *it->first << dendl;
                op.append(it->second);
        ...
        librados::AioCompletion * rados_completion =
            librados::Rados::aio_create_completion(gather_ctx->new_sub(),
                                                   nullptr,
