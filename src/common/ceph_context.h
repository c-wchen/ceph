    ...
    class CephContext {
      public:
        ...
        CephContext(uint32_t,
                    code_environment_t = CODE_ENVIRONMENT_UTILITY, int = 0)
      :    CephContext {
        } {
 ...
 */
    class CephContext {
      public:
        ...
        struct create_options {
            ...
            std::function <
                ceph::logging::Log *
                (const ceph::logging::SubsystemMap *)>create_log;
        };
   ...
   */
        int do_command(std::string_view command, const cmdmap_t & cmdmap,
                       Formatter * f,
                       std::ostream & errss, ceph::bufferlist * out);
   ...
   */
        ...
        /// check if experimental feature is enable, and emit appropriate warnings
            bool check_experimental_feature_enabled(const std::
                                                    string & feature);
        bool check_experimental_feature_enabled(const std::string & feature,
                                                std::ostream * message);
   ...
   */
        ...
        void set_mon_addrs(const std::vector < entity_addrvec_t > &in) {
            auto ptr =
                std::make_shared < std::vector < entity_addrvec_t >> (in);
            atomic_store_explicit(&_mon_addrs, std::move(ptr),
                                  std::memory_order_relaxed);
        }
        std::shared_ptr < std::vector < entity_addrvec_t >> get_mon_addrs()const {
            auto ptr =
                atomic_load_explicit(&_mon_addrs, std::memory_order_relaxed);
            return ptr;
      } private:
        ...
        struct associated_objs_cmp {
            ...
            template < typename T, typename U >
                bool operator () (const std::pair < T, std::type_index > &l,
                                  const std::pair < U,
                                  std::type_index > &r)const noexcept {
      ...
      private:
        ...
        std::vector < std::string > _mempool_perf_names,
