...
static std::basic_ostream < CharT, Traits > &operator<<(std::basic_ostream < CharT, Traits > & os,
...
protected:
    ...
    void configure_cluster(std::vector < QuiesceInterface::PeerId > leader_and_replicas = { mds_gid_t(1), mds_gid_t(2), mds_gid_t(3) })
    {
                ...
                [epoch = this->epoch, this, leader, me = this_peer](auto recipient, auto listing) {
                    ...
                    if (epoch == this->epoch) {
                        if (this->managers.contains(recipient)) {
                            ...
                            dout(10) << "listing from " << me << " (leader=" << leader << ") to " << recipient << " for version " <<
                                     listing.db_version << " with " << listing.sets.size() << " sets" << dendl;
                ...
                [epoch = this->epoch, this, leader, me = this_peer](auto diff_map)
                {
                    ...
                    if (epoch == this->epoch) {
                        if (this->managers.contains(leader)) {
                            std::queue < std::promise < void>> done_hooks;
                            dout(10) << "ack from " << me << " to the leader (" << leader << ") for version " << diff_map.db_version << " with " <<
        ...
        dout(6) << "\n === configured cluster with the following members, starting with the leader: " << leader_and_replicas <<
                dendl;
    }
    ...
    {
        dout(15) << "QUIESCING_AGENT_CB: notified with " << quiesce_map.roots.size() << " roots for version " <<
    ...
    {
        dout(15) << "FAILING_AGENT_CB: notified with " << quiesce_map.roots.size() << " roots for version " <<
                 quiesce_map.db_version << dendl;
        for (auto it = quiesce_map.roots.begin(); it != quiesce_map.roots.end();) {
            switch (it->second.state) {
                case QS_QUIESCING:
    ...
    {
        dout(15) << "SILENT_AGENT_CB: nacking quiesce map version " << quiesce_map.db_version << " with " <<
                 quiesce_map.roots.size() << " roots" << dendl;
        return false;
    };

    GenericVerboseErrorCode run_request(std::invocable < QuiesceDbRequest & > auto const& c)
    {
    ...
    GenericVerboseErrorCode run_request_for(double seconds, std::invocable < QuiesceDbRequest & > auto const& c)
    {
        last_request.reset(new TestRequestContext(*this));
        last_request->start(c);
...
TEST_F(QuiesceDbTest, Timeouts)
{
    ...
    ASSERT_EQ(QuiesceState::QS_TIMEDOUT,
              db(mds_gid_t(1)).sets.at("set3").rstate.state);  // reset can be used to resurrect a set from a terminal state
    ASSERT_EQ(OK(), run_request([](auto & r) {
        r.set_id = "set3";
        r.reset_roots({ "root1" });
    }));
...
TEST_F(QuiesceDbTest, ReleaseAwait)
{
    ...
    EXPECT_TRUE(timed_run(sec(0.1), [&]() {
        root1_state.wait(QS__INVALID);
    }));
