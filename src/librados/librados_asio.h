            ...
            Result result;
             template < typename Completion >
                void dispatch(Completion
                              && completion, boost::system::error_code ec) {
...
// specialization for Result=void
        ...
        template < typename Result > struct AsyncOp:Invoker < Result > {
            ...
            using Completion =
                ceph::async::Completion < Signature, AsyncOp < Result >>;
            ...
            template < typename Executor1, typename CompletionHandler >
                ...
                p->user_data.aio_completion.
                    reset(Rados::aio_create_completion(p.get(), aio_dispatch));
                return p;
            }
        };
...
/// given handler with signature (boost::system::error_code, bufferlist).
    template < typename ExecutionContext, typename CompletionToken >
        auto async_read(ExecutionContext & ctx, IoCtx & io,
...
/// given handler with signature (boost::system::error_code).
    template < typename ExecutionContext, typename CompletionToken >
        auto async_write(ExecutionContext & ctx, IoCtx & io,
...
/// given handler with signature (boost::system::error_code, bufferlist).
    template < typename ExecutionContext, typename CompletionToken >
        auto async_operate(ExecutionContext & ctx, IoCtx & io,
...
/// given handler with signature (boost::system::error_code, bufferlist).
    template < typename ExecutionContext, typename CompletionToken >
        auto async_notify(ExecutionContext & ctx, IoCtx & io,
                          const std::string & oid, bufferlist & bl,
                          uint64_t timeout_ms, CompletionToken && token) {
        using Op = detail::AsyncOp < bufferlist >;
        using Signature = typename Op::Signature;
        boost::asio::async_completion < CompletionToken,
            Signature > init(token);
