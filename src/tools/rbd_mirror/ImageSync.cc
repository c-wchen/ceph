...
namespace rbd {
    namespace mirror {
        ...
        using librbd::util::unique_lock_name;
         ...
         template < typename I >
            class ImageSync <
            I >::ImageCopyProgressHandler:public librbd::deep_copy::
            NoOpHandler {
          public:
          ImageCopyProgressHandler(ImageSync * image_sync):image_sync(image_sync)
            {
                ...
                image_sync->handle_copy_image_update_progress(object_no,
                                                              object_count);
                return 0;
        ...
        template < typename I >
            ImageSync < I >::ImageSync(Threads < I > *threads,
                                       I * local_image_ctx,
                                       I * remote_image_ctx,
      ...
      :CancelableRequest("rbd::mirror::ImageSync", local_image_ctx->cct,
            ...
            m_lock(ceph::
                       make_mutex(unique_lock_name
                                      ("ImageSync::m_lock", this))),
            m_update_sync_point_interval(m_local_image_ctx->cct->_conf.
                                             template get_val <
                                             double
                                             >
        ...
        template < typename I > void ImageSync < I >::send_copy_image() {
...

            {
                ...
                if (snap_id_end == CEPH_NOSNAP) {
                    derr << ": failed to locate snapshot: " << sync_point.
                        snap_name << dendl;
                    r = -ENOENT;
                }
            ...
            m_image_copy_request =
                librbd::DeepCopyRequest < I >::create(m_remote_image_ctx,
                                                      m_local_image_ctx,
                                                      snap_id_start,
                                                      snap_id_end, 0, false,
                                                      object_number,
        ...
        template < typename I >
            void ImageSync <
            I >::handle_copy_image_update_progress(uint64_t object_no,
        ...
        template < typename I > void ImageSync < I >::send_update_sync_point() {
            ...
            if (sync_point->object_number &&
                (m_image_copy_object_no - 1) ==
                sync_point->object_number.get()) {
            ...
            auto ctx = create_context_callback <
                ImageSync < I >,
                &ImageSync < I >::handle_update_sync_point > (this);
            m_sync_point_handler->update_sync_points(m_snap_seqs_copy,
                                                     m_sync_points_copy, false,
                                                     ctx);
        }
        ...
        template < typename I > void ImageSync < I >::send_flush_sync_point() {
            ...
            auto ctx = create_context_callback <
                ImageSync < I >,
                &ImageSync < I >::handle_flush_sync_point > (this);
            m_sync_point_handler->update_sync_points(m_snap_seqs_copy,
                                                     m_sync_points_copy, false,
                                                     ctx);
        }
