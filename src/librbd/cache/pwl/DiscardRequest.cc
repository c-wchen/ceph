    ...
    namespace cache {
        namespace pwl {
            ...
            template < typename I >
                ...
                I >::create(I & image_ctx, plugin::Api < I > &plugin_api,
                            Context * on_finish) {
                return new DiscardRequest(image_ctx, plugin_api, on_finish);
            } template < typename I >
                DiscardRequest < I >::DiscardRequest(I & image_ctx,
                                                     plugin::Api < I >
                                                     &plugin_api,
                                                     Context * on_finish)
            :m_image_ctx(image_ctx), m_plugin_api(plugin_api),
                ...
                m_cache_state =
                    ImageCacheState < I >::get_image_cache_state(&m_image_ctx,
                                                                 m_plugin_api);
                if (!m_cache_state) {
                    remove_feature_bit();
                    return;
                ...
                if (m_cache_state->present &&
                    ...
                    if (ec) {
                        lderr(cct) << "failed to remove persistent cache file: "
                            << ec.message()
                            << dendl;
                        // not fatal
                    }
