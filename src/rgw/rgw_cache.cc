...
int ObjectCache::get(const DoutPrefixProvider *dpp, const string &name,
                     ObjectCacheInfo &info, uint32_t mask,
                     rgw_cache_entry_info *cache_info)
{
    ...
    std::shared_lock rl {
        lock};
    std::unique_lock wl {
        lock, std::defer_lock};     // may be promoted to write lock
    if (!enabled) {
        return -ENOENT;
    }
    ...
    if (expiry.count() &&
        ...
        expiry) {
        ldpp_dout(dpp,
                  10) << "cache get: name=" << name << " : expiry miss" <<
                      dendl;
        rl.unlock();
    ...
    if (src.status == -ENOENT) {
        ldpp_dout(dpp,
                  10) << "cache get: name=" << name << " : hit (negative entry)"
                      << dendl;
        if (perfcounter) {
    ...
    if ((src.flags & mask) != mask) {
        ldpp_dout(dpp,
                  10) << "cache get: name=" << name <<
                      " : type miss (requested=0x" << std::
                      hex << mask << ", cached=0x" << src.
                      flags << std::dec << ")" << dendl;
        if (perfcounter) {
            perfcounter->inc(l_rgw_cache_miss);
        }
    ...
    for (auto cache_info : cache_info_entries) {
        ...
        if (entry->gen != cache_info->gen) {
            ldpp_dout(dpp, 20) << "chain_cache_entry: entry.gen (" << entry->gen
                               << ") != cache_info.gen (" << cache_info->gen << ")" << dendl;
            return false;
        }
...
void ObjectCache::put(const DoutPrefixProvider *dpp, const string &name,
                      ObjectCacheInfo &info, rgw_cache_entry_info *cache_info)
{
    std::unique_lock l {
        lock};
    ...
    if (info.flags & CACHE_FLAG_XATTRS) {
        ...
        for (iter = info.rm_xattrs.begin(); iter != info.rm_xattrs.end();
             ++iter) {
            ldpp_dout(dpp,
                      10) << "removing xattr: name=" << iter->first << dendl;
...
bool ObjectCache::invalidate_remove(const DoutPrefixProvider *dpp,
                                    const string &name)
{
    std::unique_lock l {
        lock};
...
void ObjectCache::touch_lru(const DoutPrefixProvider *dpp, const string &name,
...
void RGWCacheNotifyInfo::generate_test_instances(list <
        RGWCacheNotifyInfo * > &o)
{
