        ...
        std::unique_ptr < rgw::auth::Identity >
            ...
            class DummyIdentityApplier:public rgw::auth::Identity {
              ...
              public:
                ...
                :cct(cct), id(auth_id), perm_mask(perm_mask), is_admin(is_admin) {
                } uint32_t get_perms_from_aclspec(const aclspec_t & aclspec)
                    const override {
            ...
            return std::unique_ptr < rgw::auth::Identity >
                (new
                 DummyIdentityApplier(s->cct, s->user->user_id, s->perm_mask,
...
rgw::auth::Engine::result_t
    rgw::auth::Strategy::authenticate(const req_state * const s) const const
{
...
int rgw::auth::Strategy::apply(const rgw::auth::Strategy & auth_strategy,
...
get_perms_from_aclspec(const aclspec_t & aclspec) const const
{
    uint32_t perm = 0;

    /* For backward compatibility with ACLOwner. */
    perm |= rgw_perms_from_aclspec_default_strategy(info.acct_user, aclspec);
...
{
    std::string s =
        c->get_val < std::string > ("rgw_keystone_implicit_tenants");
    int v = 0;
...
const char **rgw::auth::ImplicitTenants::get_tracked_conf_keys() const const
{
...
void rgw::auth::RemoteApplier::create_account(const rgw_user & acct_user, bool implicit_tenant, RGWUserInfo & user_info) const const    /* out */
{
...
void rgw::auth::RemoteApplier::load_acct_info(RGWUserInfo & user_info) const const  /* out */
{
    ...
    if (split_mode && !implicit_tenant) ;   /* suppress lookup for id used by "other" protocol */
...
void rgw::auth::LocalApplier::load_acct_info(RGWUserInfo & user_info) const const   /* out */
{
...
rgw::auth::Engine::result_t
    rgw::auth::AnonymousEngine::authenticate(const req_state *
                                             const s) const const
{
    if (!is_applicable(s)) {
