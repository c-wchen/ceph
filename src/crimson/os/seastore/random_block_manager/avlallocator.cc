...
namespace crimson::os::seastore {
    ...
    void AvlAllocator::_remove_from_tree(rbm_abs_addr start, rbm_abs_addr size) {
        ...
        auto rs =
            extent_tree.find(extent_range_t { start, end },
    ...
    rbm_abs_addr AvlAllocator::find_block(size_t size) {
        ...
        auto iter =
            extent_size_tree.lower_bound(extent_range_t { base_addr,
                                         base_addr + size }, comp);
    ...
    void AvlAllocator::_add_to_tree(rbm_abs_addr start, rbm_abs_addr size) {
        ...
        bool merge_before = (rs_before != extent_tree.end()
                             && rs_before->end == start);
        bool merge_after = (rs_after != extent_tree.end()
                            && rs_after->start == end);
    ...
    std::optional < interval_set <
        rbm_abs_addr >> AvlAllocator::alloc_extent(size_t size) {
    ...
    bool AvlAllocator::is_free_extent(rbm_abs_addr start, size_t size) {
        ...
        auto rs = extent_tree.find(extent_range_t { start, end }
                                   , extent_tree.key_comp());
