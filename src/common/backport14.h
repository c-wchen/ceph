...
namespace ceph {
    ...
    namespace _backport14 {
        template < typename T > struct uniquity {
            using datum = std::unique_ptr < T >;
        };

         template < typename T > struct uniquity <T[] > {
            using array = std::unique_ptr < T[] >;
        };

         template < typename T, std::size_t N > struct uniquity <T[N] > {
            using verboten = void;
        };

         template < typename T, typename ... Args >
            inline typename uniquity < T >::datum make_unique(Args
                                                              && ... args) {
            return std::unique_ptr < T >
                (new T(std::forward < Args > (args) ...));
        } template < typename T > inline typename uniquity <
            T >::array make_unique(std::size_t n) {
            return std::unique_ptr < T > (new remove_extent_t < T >[n] ());
        } template < typename T, class ... Args >
            typename uniquity < T >::verboten make_unique(Args && ...) = delete;
...
// invoke_result_t, and so may not behave correctly when SFINAE is required
        template < typename F > class not_fn_result {
          ...
          explicit not_fn_result(F && f):fn(std::forward < F > (f)) {
            ...
            template < class ... Args >
                auto operator()(Args
                                && ... args) const &->decltype(!std::declval <
                                                               result_of_t <
                                                               DecayF const
                                                               &(Args ...) >>
                                                               ()) {
            ...
            template < class ... Args >
                ...
                &&->decltype(!std::declval < result_of_t <
                             DecayF(Args ...) >> ()) {
        ...
        template < typename F > not_fn_result < F > not_fn(F && fn) {
