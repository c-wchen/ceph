...
int rgw_put_system_obj(RGWRados * rgwstore, const rgw_pool & pool,
                       ...
                       real_time set_mtime, map < string, bufferlist > *pattrs)
{
    ...
    int ret =
        rgwstore->put_system_obj(NULL, obj, data, size, exclusive, NULL,
                                 *pattrs, objv_tracker, set_mtime);
...
int rgw_get_system_obj(RGWRados * rgwstore, RGWObjectCtx & obj_ctx,
...
int rgw_delete_system_obj(RGWRados * rgwstore, const rgw_pool & pool,
                          const string & oid,
