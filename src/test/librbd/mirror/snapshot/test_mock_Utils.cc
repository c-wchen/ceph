...
can_create_primary_snapshot(librbd::MockTestImageCtx *image_ctx, bool demoted,
...
TEST_F(TestMockMirrorSnapshotUtils, CanCreatePrimarySnapshot)
{
    ...
    uint64_t rollback_snap_id;
    ASSERT_TRUE(util::
                can_create_primary_snapshot(&mock_image_ctx, false,
                                            false, &requires_orphan,
                                            &rollback_snap_id));
    ...
    ASSERT_FALSE(util::
                 can_create_primary_snapshot(&mock_image_ctx, true,
                         true, nullptr,
                         nullptr));
...
TEST_F(TestMockMirrorSnapshotUtils, CanCreateNonPrimarySnapshot)
{
    ...
    // no previous mirror snapshots found
    ASSERT_TRUE(util::
                can_create_non_primary_snapshot(&mock_image_ctx));
