...
{
    return *_dout << "-- " << async_msgr->get_myinst().
        addr << " >> " << peer_addr << " conn(" << this << " :" << port << " s="
        << get_state_name(state)
        << " pgs=" << peer_global_seq << " cs=" << connect_seq << " l=" <<
        policy.lossy << ").";
}
...
AsyncConnection::AsyncConnection(CephContext * cct, AsyncMessenger * m,
                                 DispatchQueue * q, Worker * w)
:  Connection(cct, m), delay_state(NULL), async_msgr(m), conn_id(q->get_id()),
logger(w->get_perf_counter()), global_seq(0), connect_seq(0),
peer_global_seq(0), state(STATE_NONE), state_after_send(STATE_NONE), port(-1),
dispatch_queue(q), can_write(WriteStatus::NOWRITE), keepalive(false),
recv_buf(NULL),
recv_max_prefetch(MAX
                  (msgr->cct->_conf->ms_tcp_prefetch_max_size,
                   TCP_PREFETCH_MIN_SIZE)), recv_start(0), recv_end(0),
last_active(ceph::coarse_mono_clock::now()),
inactive_timeout_us(cct->_conf->ms_tcp_read_timeout * 1000 * 1000),
authorizer(NULL), replacing(false), is_reset_from_peer(false),
once_ready(false), state_buffer(NULL), state_offset(0), worker(w),
center(&w->center)
{
...
void AsyncConnection::maybe_start_delay_thread()
{
    if (!delay_state) {
        auto pos =
            async_msgr->cct->_conf->get_val < std::string >
            ("ms_inject_delay_type").find(ceph_entity_type_name(peer_type));
        if (pos != string::npos) {
            ldout(msgr->cct,
                  1) << __func__ << " setting up a delay queue" << dendl;
            delay_state =
                new DelayedDelivery(async_msgr, center, dispatch_queue,
                                    conn_id);
        }
    }
}
  ...
  again:
    ...
    if (nread < 0) {
        if (nread == -EAGAIN) {
            ...
            ldout(async_msgr->cct,
                  1) << __func__ << " reading from fd=" << cs.fd()
...
ssize_t AsyncConnection::_try_send(bool more)
{
    if (async_msgr->cct->_conf->ms_inject_socket_failures && cs) {
        if (rand() % async_msgr->cct->_conf->ms_inject_socket_failures == 0) {
            ldout(async_msgr->cct,
                  0) << __func__ << " injecting socket failure" << dendl;
            cs.shutdown();
        }
    }
    ...
    if (r < 0) {
        ldout(async_msgr->cct,
              1) << __func__ << " send error: " << cpp_strerror(r) << dendl;
        return r;
    }
...
{
    ldout(async_msgr->cct,
          25) << __func__ << " len is " << len << " state_offset is " <<
        state_offset << dendl;
    ...
    if (recv_end > recv_start) {
        ...
        left -= to_read;
        ldout(async_msgr->cct,
              25) << __func__ << " got " << to_read << " in buffer " <<
            " left is " << left << " buffer still has " << recv_end -
            recv_start << dendl;
        if (left == 0) {
            return 0;
        }
    ...
    if (len > recv_max_prefetch) {
        ...
        do {
            ...
            ldout(async_msgr->cct,
                  25) << __func__ << " read_bulk left is " << left << " got " <<
                r << dendl;
            if (r < 0) {
                ldout(async_msgr->cct,
                      1) << __func__ << " read failed" << dendl;
                return -1;
            }
        ...
        do {
            ...
            ldout(async_msgr->cct,
                  25) << __func__ << " read_bulk recv_end is " << recv_end <<
    ...
    ldout(async_msgr->cct,
          25) << __func__ << " need len " << len << " remaining " << len -
        state_offset << " bytes" << dendl;
...
void AsyncConnection::process()
{
    ...
    do {
        ldout(async_msgr->cct,
              20) << __func__ << " prev state is " << get_state_name(prev_state)
            << dendl;
        prev_state = state;
        switch (state) {
        case STATE_OPEN:
            {
                ...
                if (r < 0) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " read tag failed" << dendl;
                    goto fail;
                }
                else if (r > 0) {
                    break;
                ...
                if (tag == CEPH_MSGR_TAG_KEEPALIVE) {
                    ldout(async_msgr->cct,
                          20) << __func__ << " got KEEPALIVE" << dendl;
                    ...
                    ldout(async_msgr->cct,
                          0) << __func__ << " bad tag " << (int)tag << dendl;
                    goto fail;
                }
        ...
        case STATE_OPEN_KEEPALIVE2:
            {
                ...
                if (r < 0) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " read keeplive timespec failed" <<
                        dendl;
                    goto fail;
                }
                else if (r > 0) {
                ...
                ldout(async_msgr->cct,
                      30) << __func__ << " got KEEPALIVE2 tag ..." << dendl;
                ...
                write_lock.unlock();
                ldout(async_msgr->cct,
                      20) << __func__ << " got KEEPALIVE2 " << kp_t << dendl;
                set_last_keepalive(ceph_clock_now());
        ...
        case STATE_OPEN_TAG_ACK:
            {
                ...
                if (r < 0) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " read ack seq failed" << dendl;
                    goto fail;
                }
                else if (r > 0) {
                    break;
        ...
        case STATE_OPEN_MESSAGE_HEADER:
            {
                ...
                if (r < 0) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " read message header failed" <<
                        dendl;
                    goto fail;
                }
                else if (r > 0) {
                ...
                ldout(async_msgr->cct,
                      20) << __func__ << " got MSG header" << dendl;
                    ...
                    if (msgr->crcflags & MSG_CRC_HEADER) {
                        ...
                        header_crc =
                            ceph_crc32c(0, (unsigned char *)&oldheader,
                                        sizeof(oldheader) -
                                        sizeof(oldheader.crc));
                    }
                }

                ldout(async_msgr->cct,
                      20) << __func__ << " got envelope type=" << header.
                    type << " src " << entity_name_t(header.src)
                    << " front=" << header.front_len << " data=" << header.
                    data_len << " off " << header.data_off << dendl;

                // verify header crc
                if (msgr->crcflags & MSG_CRC_HEADER && header_crc != header.crc) {
                    ldout(async_msgr->cct,
                          0) << __func__ << " got bad header crc " << header_crc
                        << " != " << header.crc << dendl;
                    goto fail;
                }
        ...
        case STATE_OPEN_MESSAGE_THROTTLE_MESSAGE:
            {
                if (policy.throttler_messages) {
                    ldout(async_msgr->cct,
                          10) << __func__ << " wants " << 1 <<
                        " message from policy throttler " << policy.
                        throttler_messages->get_current() << "/" << policy.
                        throttler_messages->get_max() << dendl;
                    if (!policy.throttler_messages->get_or_fail()) {
                        ldout(async_msgr->cct,
                              10) << __func__ <<
                            " wants 1 message from policy throttle " << policy.
                            throttler_messages->get_current() << "/" << policy.
                            throttler_messages->
        ...
        case STATE_OPEN_MESSAGE_THROTTLE_BYTES:
            {
                ...
                if (cur_msg_size) {
                    if (policy.throttler_bytes) {
                        ldout(async_msgr->cct,
                              10) << __func__ << " wants " << cur_msg_size <<
                            " bytes from policy throttler " << policy.
                            throttler_bytes->get_current() << "/" << policy.
        ...
        case STATE_OPEN_MESSAGE_THROTTLE_DISPATCH_QUEUE:
            {
                if (cur_msg_size) {
                    if (!dispatch_queue->dispatch_throttler.
                        ...
                        ldout(async_msgr->cct,
                              10) << __func__ << " wants " << cur_msg_size <<
                            " bytes from dispatch throttle " << dispatch_queue->
                            dispatch_throttler.
                            get_current() << "/" << dispatch_queue->
                            dispatch_throttler.
                            get_max() << " failed, just wait." << dendl;
                        // following thread pool deal with th full message queue isn't a
                        // short time, so we can wait a ms.
        ...
        case STATE_OPEN_MESSAGE_READ_FRONT:
            {
                ...
                if (front_len) {
                    ...
                    if (r < 0) {
                        ldout(async_msgr->cct,
                              1) << __func__ << " read message front failed" <<
                            dendl;
                        goto fail;
                    }
                    else if (r > 0) {
        ...
        case STATE_OPEN_MESSAGE_READ_MIDDLE:
            {
                ...
                if (middle_len) {
                    ...
                    if (r < 0) {
                        ldout(async_msgr->cct,
                              1) << __func__ << " read message middle failed" <<
                            dendl;
                        goto fail;
                    }
                    else if (r > 0) {
        ...
        case STATE_OPEN_MESSAGE_READ_DATA_PREPARE:
            {
                ...
                if (data_len) {
                    ...
                    map < ceph_tid_t, pair < bufferlist, int >>::iterator p =
                        rx_buffers.find(current_header.tid);
                    if (p != rx_buffers.end()) {
                        ldout(async_msgr->cct,
                              10) << __func__ << " seleting rx buffer v " << p->
                            second.
                            second << " at offset " << data_off << " len " <<
                        ...
                        if (data_buf.length() < data_len)
                            data_buf.
                                push_back(buffer::
                                          create(data_len - data_buf.length()));
        ...
        case STATE_OPEN_MESSAGE_READ_DATA:
            {
                while (msg_left > 0) {
                    ...
                    if (r < 0) {
                        ldout(async_msgr->cct,
                              1) << __func__ << " read data error " << dendl;
                        goto fail;
                    }
                    else if (r > 0) {
                        break;
        ...
        case STATE_OPEN_MESSAGE_READ_FOOTER_AND_DISPATCH:
            {
                ...
                if (r < 0) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " read footer data error " << dendl;
                    goto fail;
                }
                else if (r > 0) {
                    break;
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " aborted = " << aborted << dendl;
                if (aborted) {
                    ldout(async_msgr->cct,
                          0) << __func__ << " got " << front.
                        length() << " + " << middle.length() << " + " << data.
                        length()
                ...
                ldout(async_msgr->cct,
                      20) << __func__ << " got " << front.
                    length() << " + " << middle.length()
                    << " + " << data.length() << " byte message" << dendl;
                Message *message =
                    decode_message(async_msgr->cct, async_msgr->crcflags,
                                   current_header, footer,
                                   front, middle, data, this);
                if (!message) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " decode message failed " << dendl;
                    goto fail;
                }
                ...
                if (session_security.get() == NULL) {
                    ldout(async_msgr->cct,
                          10) << __func__ << " no session security set" <<
                        dendl;
                }
                else {
                    if (session_security->check_message_signature(message)) {
                        ldout(async_msgr->cct,
                              0) << __func__ << " Signature check failed" <<
                            dendl;
                        message->put();
                        goto fail;
                    }
                ...
                if (message->get_seq() <= cur_seq) {
                    ldout(async_msgr->cct, 0) << __func__ << " got old message "
                        << message->
                        get_seq() << " <= " << cur_seq << " " << message << " "
                        << *message << ", discarding" << dendl;
                    message->put();
                    if (has_feature(CEPH_FEATURE_RECONNECT_SEQ)
                        && async_msgr->cct->_conf->ms_die_on_old_message)
                ...
                if (message->get_seq() > cur_seq + 1) {
                    ldout(async_msgr->cct,
                          0) << __func__ <<
                        " missed message?  skipped from seq " << cur_seq <<
                        " to " << message->get_seq() << dendl;
                ...
                ldout(async_msgr->cct,
                      5) << " rx " << message->
                    get_source() << " seq " << message->
                    get_seq() << " " << message << " " << *message << dendl;
                ...
                logger->inc(l_msgr_recv_bytes,
                            cur_msg_size + sizeof(ceph_msg_header) +
                ...
                if (delay_state) {
                    ...
                    if (rand() % 10000 <
                        async_msgr->cct->_conf->ms_inject_delay_probability *
                        10000.0) {
                        delay_period =
                            async_msgr->cct->_conf->ms_inject_delay_max *
                            (double)(rand() % 10000) / 10000.0;
                        release += delay_period;
                        ldout(async_msgr->cct,
                              1) << "queue_received will delay until " <<
                            release << " on " << message << " " << *message <<
                            dendl;
                    }
                    ...
                    dispatch_queue->enqueue(message, message->get_priority(),
                                            conn_id);
                }
        ...
        case STATE_STANDBY:
            {
                ldout(async_msgr->cct,
                      20) << __func__ << " enter STANDY" << dendl;
        ...
        case STATE_NONE:
            {
                ldout(async_msgr->cct,
                      20) << __func__ << " enter none state" << dendl;
                break;
            }
        ...
        case STATE_CLOSED:
            {
                ldout(async_msgr->cct,
                      20) << __func__ << " socket closed" << dendl;
                break;
            }
        ...
        case STATE_WAIT:
            {
                ldout(async_msgr->cct,
                      1) << __func__ << " enter wait state, failing" << dendl;
                goto fail;
            }
...
ssize_t AsyncConnection::_process_connection()
{
    ...
    case STATE_CONNECTING:
        {
            ...
            if (cs) {
                center->delete_file_event(cs.fd(),
    ...
    case STATE_CONNECTING_RE:
        {
            ...
            if (r < 0) {
                ldout(async_msgr->cct,
                      1) << __func__ << " reconnect failed " << dendl;
                if (r == -ECONNREFUSED) {
                    ldout(async_msgr->cct,
                          2) << __func__ << " connection refused!" << dendl;
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " nonblock connect inprogress" <<
                    dendl;
                if (async_msgr->get_stack()->
                    nonblock_connect_need_writable_event())
            ...
            if (r == 0) {
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " connect write banner done: " <<
                    get_peer_addr() << dendl;
            }
            else if (r > 0) {
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " connect wait for write banner: " <<
                    get_peer_addr() << dendl;
            }
            else {
                goto fail;
    ...
    case STATE_CONNECTING_WAIT_BANNER_AND_IDENTIFY:
        {
            ...
            try {
                ...
                lderr(async_msgr->
                      cct) << __func__ << " decode peer addr failed " << dendl;
                goto fail;
            }
            ...
            if (peer_addr != paddr) {
                if (paddr.is_blank_ip()
                    ...
                    ldout(async_msgr->cct,
                          0) << __func__ << " connect claims to be " << paddr <<
                        " not " << peer_addr <<
                        " - presumably this is the same node!" << dendl;
                }
                else {
                    ldout(async_msgr->cct,
                          10) << __func__ << " connect claims to be " << paddr
                        << " not " << peer_addr << dendl;
                    goto fail;
                }
            }
            ...
            ldout(async_msgr->cct,
                  20) << __func__ << " connect peer addr for me is " <<
            ...
            if (async_msgr->cct->_conf->ms_inject_internal_delays
                ...
                if (rand() %
                    async_msgr->cct->_conf->ms_inject_socket_failures == 0) {
                    ldout(msgr->cct,
                          10) << __func__ << " sleep for " << async_msgr->cct->
                        _conf->ms_inject_internal_delays << dendl;
                    utime_t t;
                    t.set_from_double(async_msgr->cct->_conf->
            ...
            if (r == 0) {
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " connect sent my addr " <<
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " connect send my addr done: " <<
                    async_msgr->get_myaddr() << dendl;
            }
            else {
                ldout(async_msgr->cct,
                      2) << __func__ << " connect couldn't write my addr, " <<
    ...
    case STATE_CONNECTING_SEND_CONNECT_MSG:
        {
            ...
            connect_msg.protocol_version =
                async_msgr->get_proto_version(peer_type, true);
            connect_msg.authorizer_protocol =
                authorizer ? authorizer->protocol : 0;
            connect_msg.authorizer_len =
                authorizer ? authorizer->bl.length() : 0;
            if (authorizer)
                ldout(async_msgr->cct,
                      10) << __func__ << " connect_msg.authorizer_len=" <<
                    connect_msg.authorizer_len << " protocol=" << connect_msg.
                    authorizer_protocol << dendl;
            connect_msg.flags = 0;
            if (policy.lossy)
            ...
            ldout(async_msgr->cct,
                  10) << __func__ << " connect sending gseq=" << global_seq <<
                " cseq=" << connect_seq << " proto=" << connect_msg.
                protocol_version << dendl;
            ...
            if (r == 0) {
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " continue send reply " << dendl;
            }
            else {
                ldout(async_msgr->cct,
                      2) << __func__ << " connect couldn't send reply " <<
                    cpp_strerror(r) << dendl;
                goto fail;
            }
    ...
    case STATE_CONNECTING_WAIT_CONNECT_REPLY:
        {
            ...
            if (r < 0) {
                ldout(async_msgr->cct,
                      1) << __func__ << " read connect reply failed" << dendl;
                goto fail;
            }
            else if (r > 0) {
                break;
            ...
            ldout(async_msgr->cct,
                  20) << __func__ << " connect got reply tag " << (int)
                connect_reply.tag << " connect_seq " << connect_reply.
                connect_seq << " global_seq " << connect_reply.
                global_seq << " proto " << connect_reply.
                protocol_version << " flags " << (int)connect_reply.
                flags << " features " << connect_reply.features << dendl;
    ...
    case STATE_CONNECTING_WAIT_CONNECT_REPLY_AUTH:
        {
            ...
            if (connect_reply.authorizer_len) {
                ldout(async_msgr->cct,
                      10) << __func__ << " reply.authorizer_len=" <<
                    connect_reply.authorizer_len << dendl;
                ...
                authorizer_reply.append(state_buffer,
                                        connect_reply.authorizer_len);

                if (connect_reply.tag == CEPH_MSGR_TAG_CHALLENGE_AUTHORIZER) {
                    ldout(async_msgr->cct,
                          10) << __func__ << " connect got auth challenge" <<
                        dendl;
                    authorizer->add_challenge(async_msgr->cct,
                                              authorizer_reply);
                    state = STATE_CONNECTING_SEND_CONNECT_MSG;
                    break;
                }
                ...
                if (authorizer && !authorizer->verify_reply(iter)) {
                    ldout(async_msgr->cct,
                          0) << __func__ << " failed verifying authorize reply"
                        << dendl;
                    goto fail;
                }
            }
    ...
    case STATE_CONNECTING_WAIT_ACK_SEQ:
        {
            ...
            if (r < 0) {
                ldout(async_msgr->cct,
                      1) << __func__ << " read connect ack seq failed" << dendl;
                goto fail;
            }
            else if (r > 0) {
                break;
            ...
            ldout(async_msgr->cct,
                  2) << __func__ << " got newly_acked_seq " << newly_acked_seq
            ...
            if (r == 0) {
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " send in_seq done " << dendl;
            }
            else if (r > 0) {
                state_after_send = STATE_CONNECTING_READY;
                state = STATE_WAIT_SEND;
                ldout(async_msgr->cct,
                      10) << __func__ << " continue send in_seq " << dendl;
            }
            else {
                goto fail;
            }
    ...
    case STATE_CONNECTING_READY:
        {
            ...
            set_features((uint64_t) connect_reply.
                         features & (uint64_t) connect_msg.features);
            ldout(async_msgr->cct,
                  10) << __func__ << " connect success " << connect_seq <<
                ", lossy = " << policy.
                lossy << ", features " << get_features() << dendl;
    ...
    case STATE_ACCEPTING:
        {
            ...
            ldout(async_msgr->cct,
                  1) << __func__ << " sd=" << cs.
                fd() << " " << socket_addr << dendl;

            r = try_send(bl);
            if (r == 0) {
                state = STATE_ACCEPTING_WAIT_BANNER_ADDR;
                ldout(async_msgr->cct,
                      10) << __func__ << " write banner and addr done: " <<
                    get_peer_addr() << dendl;
            }
            else if (r > 0) {
                state = STATE_WAIT_SEND;
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " wait for write banner and addr: " <<
                    get_peer_addr() << dendl;
            }
            else {
                goto fail;
    ...
    case STATE_ACCEPTING_WAIT_BANNER_ADDR:
        {
            ...
            r = read_until(strlen(CEPH_BANNER) + sizeof(ceph_entity_addr),
                           state_buffer);
            if (r < 0) {
                ldout(async_msgr->cct,
                      1) << __func__ << " read peer banner and addr failed" <<
                    dendl;
                goto fail;
            }
            else if (r > 0) {
            ...
            if (memcmp(state_buffer, CEPH_BANNER, strlen(CEPH_BANNER))) {
                ldout(async_msgr->cct,
                      1) << __func__ << " accept peer sent bad banner '" <<
                    state_buffer << "' (should be '" << CEPH_BANNER << "')" <<
                    dendl;
                goto fail;
            }

            addr_bl.append(state_buffer + strlen(CEPH_BANNER),
            ...
            try {
                ...
                lderr(async_msgr->
                      cct) << __func__ << " decode peer_addr failed " << dendl;
                goto fail;
            }

            ldout(async_msgr->cct,
                  10) << __func__ << " accept peer addr is " << peer_addr <<
                dendl;
            if (peer_addr.is_blank_ip()) {
                ...
                ldout(async_msgr->cct,
                      0) << __func__ << " accept peer addr is really " <<
                    peer_addr << " (socket is " << socket_addr << ")" << dendl;
            }
    ...
    case STATE_ACCEPTING_WAIT_CONNECT_MSG:
        {
            ...
            if (r < 0) {
                ldout(async_msgr->cct,
                      1) << __func__ << " read connect msg failed" << dendl;
                goto fail;
            }
            else if (r > 0) {
                break;
    ...
    case STATE_ACCEPTING_WAIT_CONNECT_MSG_AUTH:
        {
            ...
            if (connect_msg.authorizer_len) {
                if (!authorizer_buf.length())
                    authorizer_buf.
                        push_back(buffer::create(connect_msg.authorizer_len));

                r = read_until(connect_msg.authorizer_len,
                               authorizer_buf.c_str());
                if (r < 0) {
                    ldout(async_msgr->cct,
                          1) << __func__ << " read connect authorizer failed" <<
                        dendl;
                    goto fail;
                }
                else if (r > 0) {
            ...
            ldout(async_msgr->cct,
                  20) << __func__ << " accept got peer connect_seq " <<
                connect_msg.connect_seq << " global_seq " << connect_msg.
                global_seq << dendl;
            set_peer_type(connect_msg.host_type);
            policy = async_msgr->get_policy(connect_msg.host_type);
            ldout(async_msgr->cct,
                  10) << __func__ << " accept of host_type " << connect_msg.
                host_type << ", policy.lossy=" << policy.
                lossy << " policy.server=" << policy.
                server << " policy.standby=" << policy.
                standby << " policy.resetcheck=" << policy.resetcheck << dendl;
    ...
    case STATE_ACCEPTING_WAIT_SEQ:
        {
            ...
            newly_acked_seq = *((uint64_t *) state_buffer);
            ldout(async_msgr->cct,
                  2) << __func__ << " accept get newly_acked_seq " <<
                newly_acked_seq << dendl;
            discard_requeued_up_to(newly_acked_seq);
    ...
    default:
        {
            lderr(async_msgr->
                  cct) << __func__ << " bad state: " << state << dendl;
            ceph_abort();
        }
    }
...
int AsyncConnection::handle_connect_reply(ceph_msg_connect & connect,
                                          ceph_msg_connect_reply & reply)
{
    uint64_t feat_missing;
    if (reply.tag == CEPH_MSGR_TAG_FEATURES) {
        ldout(async_msgr->cct,
              0) << __func__ << " connect protocol feature mismatch, my " <<
            std::hex << connect.features << " < peer " << reply.
            features << " missing " << (reply.features & ~policy.
                                        features_supported)
            << std::dec << dendl;
        goto fail;
    }
    ...
    if (reply.tag == CEPH_MSGR_TAG_BADPROTOVER) {
        ldout(async_msgr->cct,
              0) << __func__ << " connect protocol version mismatch, my " <<
            connect.protocol_version << " != " << reply.
            protocol_version << dendl;
        goto fail;
    }
    ...
    if (reply.tag == CEPH_MSGR_TAG_BADAUTHORIZER) {
        ldout(async_msgr->cct,
              0) << __func__ << " connect got BADAUTHORIZER" << dendl;
        goto fail;
    }
    if (reply.tag == CEPH_MSGR_TAG_RESETSESSION) {
        ldout(async_msgr->cct,
              0) << __func__ << " connect got RESETSESSION" << dendl;
        was_session_reset();
    ...
    if (reply.tag == CEPH_MSGR_TAG_RETRY_GLOBAL) {
        ...
        ldout(async_msgr->cct, 5) << __func__ << " connect got RETRY_GLOBAL "
            << reply.global_seq << " chose new " << global_seq << dendl;
        state = STATE_CONNECTING_SEND_CONNECT_MSG;
    }
    if (reply.tag == CEPH_MSGR_TAG_RETRY_SESSION) {
        ...
        ldout(async_msgr->cct, 5) << __func__ << " connect got RETRY_SESSION "
            << connect_seq << " -> " << reply.connect_seq << dendl;
        connect_seq = reply.connect_seq;
        state = STATE_CONNECTING_SEND_CONNECT_MSG;
    }
    if (reply.tag == CEPH_MSGR_TAG_WAIT) {
        ldout(async_msgr->cct,
              1) << __func__ << " connect got WAIT (connection race)" << dendl;
        state = STATE_WAIT;
    }

    feat_missing =
        policy.features_required & ~(uint64_t) connect_reply.features;
    if (feat_missing) {
        ldout(async_msgr->cct,
              1) << __func__ << " missing required features " << std::
    ...
    if (reply.tag == CEPH_MSGR_TAG_READY) {
        ldout(async_msgr->cct,
              10) << __func__ << " got CEPH_MSGR_TAG_READY " << dendl;
        state = STATE_CONNECTING_READY;
    }
...
ssize_t AsyncConnection::handle_connect_msg(ceph_msg_connect & connect,
                                            bufferlist & authorizer_bl,
                                            bufferlist & authorizer_reply)
{
    ssize_t r = 0;
    ceph_msg_connect_reply reply;
    ...
    // mismatch?
    ldout(async_msgr->cct,
          10) << __func__ << " accept my proto " << reply.
        protocol_version << ", their proto " << connect.
        protocol_version << dendl;
    if (connect.protocol_version != reply.protocol_version) {
        return _reply_accept(CEPH_MSGR_TAG_BADPROTOVER, connect, reply,
                             authorizer_reply);
    }
    ...
    uint64_t feat_missing =
        policy.features_required & ~(uint64_t) connect.features;
    if (feat_missing) {
        ldout(async_msgr->cct,
              1) << __func__ << " peer missing required features " << std::
            hex << feat_missing << std::dec << dendl;
        return _reply_accept(CEPH_MSGR_TAG_FEATURES, connect, reply,
                             authorizer_reply);
    }
    ...
    if (!async_msgr->
        verify_authorizer(this, peer_type, connect.authorizer_protocol,
                          authorizer_bl, authorizer_reply, authorizer_valid,
                          session_key,
        ...
        if (need_challenge && !had_challenge && authorizer_challenge) {
            ...
            ldout(async_msgr->cct,
                  0) << __func__ << ": got bad authorizer" << dendl;
    ...
    ldout(async_msgr->cct,
          10) << __func__ << " accept setting up session_security." << dendl;
    ...
    if (existing) {
        ...
        if (existing->state == STATE_CLOSED) {
            ldout(async_msgr->cct,
                  1) << __func__ << " existing already closed." << dendl;
        ...
        if (connect.global_seq < existing->peer_global_seq) {
            ldout(async_msgr->cct,
                  10) << __func__ << " accept existing " << existing << ".gseq "
                << existing->peer_global_seq << " > " << connect.
                global_seq << ", RETRY_GLOBAL" << dendl;
        ...
        ldout(async_msgr->cct,
              0) << __func__ << " accept connect_seq " << connect.
            connect_seq << " vs existing csq=" << existing->
            connect_seq << " existing_state=" << get_state_name(existing->
                                                                state) << dendl;
        ...
        if (connect.connect_seq < existing->connect_seq) {
            ...
            ldout(async_msgr->cct,
                  10) << __func__ << " accept existing " << existing << ".cseq "
                << existing->connect_seq << " > " << connect.
                connect_seq << ", RETRY_SESSION" << dendl;
        ...
        if (connect.connect_seq == existing->connect_seq) {
            ...
            if (existing->state == STATE_OPEN ||
                ...
                ldout(async_msgr->cct,
                      10) << __func__ << " accept connection race, existing " <<
                    existing << ".cseq " << existing->
                    connect_seq << " == " << connect.
                    connect_seq << ", OPEN|STANDBY, RETRY_SESSION" << dendl;
                reply.connect_seq = existing->connect_seq + 1;
                existing->lock.unlock();
                return _reply_accept(CEPH_MSGR_TAG_RETRY_SESSION, connect,
            ...
            if (peer_addr < async_msgr->get_myaddr() || existing->policy.server) {
                ...
                ldout(async_msgr->cct,
                    ...
                    existing << ".cseq " << existing->
                    connect_seq << " == " << connect.
                    connect_seq << ", or we are server, replacing my attempt" <<
                    dendl;
                goto replace;
            }
            else {
        ...
        if (policy.resetcheck &&    // RESETSESSION only used by servers; peers do not reset each other
            ...
            ldout(async_msgr->cct,
                  0) << __func__ << " accept we reset (peer sent cseq " <<
                connect.
                connect_seq << ", " << existing << ".cseq = " << existing->
        ...
        // reconnect
        ldout(async_msgr->cct,
              10) << __func__ << " accept peer sent cseq " << connect.
            connect_seq << " > " << existing->connect_seq << dendl;
        goto replace;
    }                           // existing
    else if (!replacing && connect.connect_seq > 0) {
        // we reset, and they are opening a new session
        ldout(async_msgr->cct,
              0) << __func__ << " accept we reset (peer sent cseq " << connect.
            connect_seq << "), sending RESETSESSION" << dendl;
        return _reply_accept(CEPH_MSGR_TAG_RESETSESSION, connect, reply,
                             authorizer_reply);
    }
        ...
        // new session
        ldout(async_msgr->cct,
              10) << __func__ << " accept new session" << dendl;
        existing = NULL;
        goto open;
    }
    ceph_abort();
  ...
  replace:
    ldout(async_msgr->cct,
          10) << __func__ << " accept replacing " << existing << dendl;

    inject_delay();
    if (existing->policy.lossy) {
        // disconnect from the Connection
        ldout(async_msgr->cct,
              1) << __func__ << " replacing on lossy channel, failing existing"
            << dendl;
        existing->_stop();
        ...
        ldout(async_msgr->cct,
              1) << __func__ << " stop myself to swap existing" << dendl;
        ...
        auto deactivate_existing = std::bind([existing, new_worker, new_center,
                                              connect, reply,
                                              authorizer_reply] (ConnectedSocket
                                                                 & cs) mutable {
                                             // we need to delete time event in original thread
                                             {
                                              ...
                                              existing->center->
                                              submit_to(existing->center->
                                                        get_id(),
                                                        std::
                                                        move(transfer_existing),
                                                        true);}
                                              , std::move(temp_cs));
                                             existing->center->
                                             submit_to(existing->center->
  ...
  existing->lock.unlock(); open:
                                             connect_seq =
                                             connect.connect_seq + 1;
                                             peer_global_seq =
                                             connect.global_seq;
                                             ldout(async_msgr->cct,
                                                   10) << __func__ <<
                                             ...
                                             << connect_seq << " in_seq=" <<
                                             in_seq << ", sending READY" <<
                                             dendl; int next_state;
                                             ...
                                             reply.global_seq =
                                             async_msgr->get_global_seq();
                                             reply.connect_seq = connect_seq;
                                             reply.flags = 0;
                                             reply.authorizer_len =
                                             ...
                                             reset(get_auth_session_handler
                                                   (async_msgr->cct,
                                                    ...
                                                    session_key,
                                                    get_features()));
                                             reply_bl.append((char *)&reply,
                                                             sizeof(reply));
                                             if (reply.authorizer_len)
                                             reply_bl.append(authorizer_reply.
                                                             c_str(),
                                                             authorizer_reply.
                                                             length());
                                             if (reply.tag ==
                                             ...
                                             inject_delay();
                                             lock.lock();
                                             replacing = false; if (r < 0) {
                                             ldout(async_msgr->cct,
                                                   1) << __func__ <<
                                             " existing race replacing process for addr="
                                             << peer_addr <<
                                             " just fail later one(this)" <<
                                             dendl; goto fail_registered;}
                                             if (state !=
                                             ...
                                             r = try_send(reply_bl); if (r < 0)
                                             goto fail_registered;
                                             // notify
                                             dispatch_queue->queue_accept(this);
                                             async_msgr->
                                             ms_deliver_handle_fast_accept
                                             (this); once_ready = true;
                                             if (r == 0) {
                                             state = next_state;
                                             ldout(async_msgr->cct,
                                                   2) << __func__ <<
                                             " accept write reply msg done" <<
                                             dendl;}
                                             else {
                                             state = STATE_WAIT_SEND;
                                             state_after_send = next_state;}

  return 0; fail_registered:
  ldout(async_msgr->cct, 10) << __func__ << " accept fault after register" << dendl; inject_delay(); fail:
                                             ldout(async_msgr->cct,
                                                   10) << __func__ <<
                                             ...
                                             FUNCTRACE();
                                             lgeneric_subdout(async_msgr->cct,
                                                              ms,
                                                              1) << "-- " <<
                                             async_msgr->
                                             get_myaddr() << " --> " <<
                                             ...
                                             if (!m->get_priority())
                                             m->set_priority(async_msgr->
                                                             get_default_send_priority
                                                             ());
                                             m->get_header().src =
                                             async_msgr->get_myname();
                                             m->set_connection(this);
                                             if (m->get_type() ==
                                                 CEPH_MSG_OSD_OP)
                                             OID_EVENT_TRACE_WITH_MSG(m,
                                                                      "SEND_MSG_OSD_OP_BEGIN",
                                                                      true);
                                             else
                                             if (m->get_type() ==
                                                 CEPH_MSG_OSD_OPREPLY)
                                             OID_EVENT_TRACE_WITH_MSG(m, "SEND_MSG_OSD_OPREPLY_BEGIN", true); if (async_msgr->get_myaddr() == get_peer_addr()) {    //loopback connection
                                             ldout(async_msgr->cct,
                                                   20) << __func__ << " " << *m
                                             << " local" << dendl;
                                             std::lock_guard < std::mutex >
                                             ...
                                             if (can_write !=
                                                 WriteStatus::CLOSED) {
                                             dispatch_queue->local_delivery(m,
                                                                            m->
                                                                            get_priority
                                                                            ());}
                                             else {
                                             ...
                                             out_q[m->get_priority()].
                                             emplace_back(std::move(bl), m);
                                             ldout(async_msgr->cct,
                                                   15) << __func__ <<
                                             " inline write is denied, reschedule m="
                                             << m << dendl;
                                             if (can_write !=
                                             ...
                                             if (sent.empty())
                                             return;
                                             list < pair < bufferlist,
                                             Message * > >&rq =
                                             out_q[CEPH_MSG_PRIO_HIGHEST];
                                             while (!sent.empty()) {
                                             Message * m = sent.back();
                                             sent.pop_back();
                                             ldout(async_msgr->cct,
                                             ...
                                             get_seq() << ")" << dendl;
                                             rq.
                                             push_front(make_pair
                                                        (bufferlist(), m));
                                             out_seq--;}
                                             }

                                             void AsyncConnection::
                                             discard_requeued_up_to(uint64_t
                                                                    seq) {
                                             ldout(async_msgr->cct,
                                                   10) << __func__ << " " << seq
                                             << dendl;
                                             ...
                                             out_q[CEPH_MSG_PRIO_HIGHEST];
                                             while (!rq.empty()) {
                                             pair < bufferlist, Message * >p =
                                             rq.front();
                                             if (p.second->get_seq() == 0
                                                 || p.second->get_seq() > seq)
                                             break;
                                             ldout(async_msgr->cct,
                                                   10) << __func__ << " " <<
                                             *(p.
                                               second) << " for resend seq " <<
                                             p.second->get_seq()
                                             << " <= " << seq << ", discarding"
                                             << dendl; p.second->put();
                                             rq.pop_front(); out_seq++;}
                                             if (rq.empty())
 ...
 */
                                             ...
                                             for (map < int,
                                                  ...
                                                  out_q.begin();
                                                  p != out_q.end(); ++p)
                                             for (list < pair < bufferlist,
                                                  Message * > >::iterator r =
                                                  p->second.begin();
                                                  r != p->second.end(); ++r) {
                                             ldout(async_msgr->cct,
                                                   20) << __func__ <<
                                             " discard " << r->second << dendl;
                                             r->second->put();}
                                             out_q.clear();}
                                             ...
                                             get_random_bytes((char *)&rand_seq,
                                                              sizeof(rand_seq));
                                             rand_seq &= SEQ_MASK;
                                             lsubdout(async_msgr->cct, ms,
                                                      10) << __func__ <<
                                             " randomize_out_seq " << rand_seq
                                             << dendl; out_seq = rand_seq;
                                             return seq_error;}
                                             else {
                                             ...
                                             << " accept state just closed" <<
                                             dendl; write_lock.unlock();
                                             _stop();
                                             dispatch_queue->queue_reset(this);
                                             return;}
                                             reset_recv_state();
                                             ...
                                             << dendl; state = STATE_STANDBY;
                                             write_lock.unlock(); return;}

                                             write_lock.unlock(); if (!(state >= STATE_CONNECTING && state < STATE_CONNECTING_READY) && state != STATE_WAIT) {  // STATE_WAIT is coming from STATE_CONNECTING_*
                                             ...
                                             if (state == STATE_WAIT) {
                                             backoff.
                                             set_from_double(async_msgr->cct->
                                                             _conf->
                                                             ms_max_backoff);}
                                             else
                                             if (backoff == utime_t()) {
                                             ...
                                             ldout(async_msgr->cct,
                                                   15) << __func__ <<
                                             " could not get random bytes to set seq number for session reset; set seq number to "
                                             << out_seq << dendl;}

                                             in_seq = 0; connect_seq = 0;
                                             ...
                                             if (state == STATE_CLOSED)
                                             return; if (delay_state)
                                             delay_state->flush();
                                             ldout(async_msgr->cct,
                                                   2) << __func__ << dendl;
                                             std::lock_guard < std::mutex >
                                             l(write_lock); reset_recv_state();
                                             dispatch_queue->
                                             discard_queue(conn_id);
                                             discard_out_queue();
                                             async_msgr->unregister_conn(this);
                                             ...
                                             dispatch_event_external
                                             (EventCallbackRef
                                              (new C_clean_handler(this)));}

                                             void AsyncConnection::
                                             prepare_send_message(uint64_t
                                                                  features,
                                                                  Message * m,
                                                                  bufferlist &
                                                                  bl) {
                                             ...
                                             << " front=" << header.
                                             front_len << " data=" << header.
                                             data_len << " off " << header.
                                             data_off << dendl;
                                             if ((bl.length() <=
                                                  ASYNC_COALESCE_THRESHOLD)
                                                 && (bl.buffers().size() > 1)) {
      for (const auto & pb:                 bl.buffers())
                                             {
                                             outcoming_bl.append((char *)pb.
                                                                 c_str(),
                                                                 pb.length());}
                                             }
                                             else {
                                             ...
                                             if (msgr->
                                                 crcflags & MSG_CRC_HEADER) {
                                             old_footer.front_crc =
                                             footer.front_crc;
                                             old_footer.middle_crc =
                                             footer.middle_crc;
                                             old_footer.data_crc =
                                             footer.data_crc;}
                                             else {
                                             old_footer.front_crc =
                                             old_footer.middle_crc = 0;}
                                             old_footer.data_crc =
                                             msgr->
                                             crcflags & MSG_CRC_DATA ? footer.
                                             data_crc : 0;
                                             ...
                                             dispatch_throttler.
                                             get_current() << "/" <<
                                             dispatch_queue->dispatch_throttler.
                                             get_max() << dendl;
                                             dispatch_queue->
                                             ...
                                             void AsyncConnection::
                                             handle_ack(uint64_t seq) {
                                             ldout(async_msgr->cct,
                                                   15) << __func__ <<
                                             " got ack seq " << seq << dendl;
                                             // trim sent list
                                             std::lock_guard < std::mutex >
                                             l(write_lock);
                                             while (!sent.empty()
                                             ...
                                             *m << dendl; m->put();}
                                             ...
                                             if (stop_dispatch)
                                             return; if (delay_queue.empty())
                                             return;
                                             utime_t release =
                                             delay_queue.front().first;
                                             m = delay_queue.front().second;
                                             string delay_msg_type =
                                             msgr->cct->_conf->
                                             ms_inject_delay_msg_type;
                                             utime_t now = ceph_clock_now();
                                             if ((release > now
                                                  && (delay_msg_type.empty()
                                                      || m->get_type_name() ==
                                                      delay_msg_type))) {
      ...
      for (auto i:                                            register_time_events)
                                             ...
                                             void AsyncConnection::
                                             _append_keepalive_or_ack(bool ack,
                                                                      utime_t *
                                                                      tp) {
                                             ldout(async_msgr->cct,
                                                   10) << __func__ << dendl;
                                             ...
                                             auto start =
                                             ceph::mono_clock::now(); bool more;
                                             do {
                                             bufferlist data;
                                             Message * m =
                                             _get_next_outgoing(&data); if (!m)
                                             break; if (!policy.lossy) {
                                             // put on sent list
                                             sent.push_back(m); m->get();}
                                             more = _has_next_outgoing();
                                             write_lock.unlock();
                                             // send_message or requeue messages may not encode message
                                             if (!data.length())
                                             prepare_send_message(get_features
                                                                  (), m, data);
                                             ...
                                             goto fail;}
                                             write_lock.lock(); if (r > 0)
                                             break;} while (can_write ==
                                                            WriteStatus::
                                                            CANWRITE);
                                             write_lock.unlock();
                                             uint64_t left = ack_left;
                                             if (left) {
                                             ceph_le64 s; s = in_seq;
                                             outcoming_bl.
                                             append(CEPH_MSGR_TAG_ACK);
                                             outcoming_bl.append((char *)&s,
                                                                 sizeof(s));
                                             ldout(async_msgr->cct,
                                                   10) << __func__ <<
                                             " try send msg ack, acked " << left
                                             << " messages" << dendl;
                                             ack_left -= left; left = ack_left;
                                             r = _try_send(left);}
                                             else
                                             if (is_queued()) {
                                             r = _try_send();}
                                             ...
                                             ldout(async_msgr->cct,
                                                   1) << __func__ <<
                                             " send outcoming bl failed" <<
                                             dendl; write_lock.unlock();
                                             fault(); lock.unlock(); return;}
                                             }
                                             write_lock.unlock(); lock.unlock();}
  ...
  return; fail:
                                             ...
                                             dendl;
                                             std::lock_guard < std::mutex >
                                             l(lock); last_tick_id = 0;
                                             auto idle_period =
                                             std::chrono::duration_cast <
                                             std::chrono::microseconds >
                                             (now - last_active).count();
                                             if (inactive_timeout_us <
                                                 (uint64_t) idle_period) {
                                             ldout(async_msgr->cct,
                                                   1) << __func__ << " idle(" <<
                                             idle_period << ") more than " <<
                                             inactive_timeout_us <<
