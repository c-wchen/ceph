...
namespace ceph {
    namespace mutex_debug_detail {
        enum {
         ...
         mutex_debugging_base::mutex_debugging_base(const std::string & n,
                                                    bool bt,
                                                    CephContext * cct):id(-1),
            backtrace(bt), nlock(0), locked_by(thread::id()), cct(cct),
            logger(0) {
            if (n.empty()) {
                uuid_d uu;
                 uu.generate_random();
        ...
        void mutex_debugging_base::after_lock_blocks(ceph::mono_time start,
                                                     bool no_lockdep) {
            if (logger && cct && cct->_conf->mutex_perf_counter)
                logger->tinc(l_mutex_wait, ceph::mono_clock::now() - start);
