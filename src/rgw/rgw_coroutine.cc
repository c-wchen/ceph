...
RGWAioCompletionNotifier::RGWAioCompletionNotifier(RGWCompletionManager * _mgr,
    ...
    (_mgr), user_data(_user_data), lock("RGWAioCompletionNotifier"),
registered(true)
{
  ...
  for (auto & st:stacks) {
    ...
    for (list < RGWCoroutinesStack * >::iterator iter =
        ...
        while (completion_mgr->try_get_next((void **)&blocked_stack)) {
            handle_unblocked_stack(context_stacks, scheduled_stacks,
                                   blocked_stack, &blocked_count);
        }
...
create_completion_notifier(RGWCoroutinesStack * stack)
{
    RGWAioCompletionNotifier *cn =
        new RGWAioCompletionNotifier(completion_mgr, (void *)stack);
...
bool RGWCoroutinesManagerRegistry::call(std::string command, cmdmap_t & cmdmap,
