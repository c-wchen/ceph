    ...
    namespace crypto {
        ...
        using librbd::util::data_object_name;
         ...
         template < typename I >
            uint64_t get_file_offset(I * image_ctx, uint64_t object_no,
                                     uint64_t object_off) {
            auto off =
                io::util::raw_to_area_offset(*image_ctx,
                                             Striper::
                                             get_file_offset(image_ctx->cct,
                                                             &image_ctx->layout,
                                                             object_no,
                                                             object_off));
            ...
            bool disable_read_from_parent;
             ...
             C_AlignedObjectReadRequest(I * image_ctx, CryptoInterface * crypto,
                                        uint64_t object_no,
                                        io::ReadExtents * extents,
                                        ...
                                        Context *
                                        on_dispatched):image_ctx(image_ctx),
                crypto(crypto), object_no(object_no), extents(extents),
                io_context(io_context), parent_trace(parent_trace),
                ...
                auto ctx = create_context_callback <
                 ...
                 req =
                    io::ObjectDispatchSpec::create_read(image_ctx,
                                                        io::
                                                        OBJECT_DISPATCH_LAYER_CRYPTO,
                                                        object_no, extents,
                                                        io_context, op_flags,
                                                        read_flags,
                                                        parent_trace, version,
                                                        ctx);
            } void send() {
                req->send();
            ...
            void handle_read(int r) {
                ...
                if (r >= 0) {
                  ...
                  for (auto & extent:*extents) {
                        auto crypto_ret =
                            crypto->decrypt_aligned_extent(extent,
                                                           get_file_offset
                                                           (image_ctx,
                                                            object_no,
                                                            extent.offset));
        ...
        template < typename I >
            ...
            Context *on_finish;
             ...
             C_UnalignedObjectReadRequest(I * image_ctx,
                                          CryptoInterface * crypto,
                                          uint64_t object_no,
                                          io::ReadExtents * extents,
                                          IOContext io_context, int op_flags,
                                          ...
                                          int *object_dispatch_flags,
                                          Context *
                                          on_dispatched):cct(image_ctx->cct),
                extents(extents), on_finish(on_dispatched) {
            ...
            void remove_alignment_data() {
                for (uint64_t i = 0; i < extents->size(); ++i) {
                    ...
                    if (aligned_extent.extent_map.empty()) {
                        uint64_t cut_offset =
                            extent.offset - aligned_extent.offset;
                        int64_t padding_count =
                            cut_offset + extent.length -
                            aligned_extent.bl.length();
                        if (padding_count > 0) {
                            aligned_extent.bl.append_zero(padding_count);
                        }
                        aligned_extent.bl.splice(cut_offset, extent.length,
                                                 &extent.bl);
                    }
                    else {
                      for (auto[off, len]:aligned_extent.
                             extent_map)
                        {
                            ceph::bufferlist tmp;
                            ...
                            len =
                                std::min(len,
                                         extent.offset + extent.length - off);
                            if (len == 0) {
                                continue;
                            }
        ...
        template < typename I >
            ...
            bool object_exists;
             ...
             C_UnalignedObjectWriteRequest(I * image_ctx,
                                           CryptoInterface * crypto,
                                           uint64_t object_no,
                                           uint64_t object_off, ceph::bufferlist
                                           && data, ceph::bufferlist
                                           && cmp_data,
                                           uint64_t * mismatch_offset,
                                           IOContext io_context, int op_flags,
                                           int write_flags,
                                           ...
                                           bool
                                           may_copyup):image_ctx(image_ctx),
                crypto(crypto), object_no(object_no), object_off(object_off),
                data(data), cmp_data(cmp_data),
                ...
                if (pre_align != 0) {
                    ...
                    extents.emplace_back(object_off + data.length(),
                                         post_align);
                }
                ...
                read_req =
                    new C_UnalignedObjectReadRequest < I > (image_ctx, crypto,
                                                            object_no, &extents,
                                                            io_context, 0,
                                                            io::
            ...
            bool check_create_exclusive() {
                bool exclusive =
                    ((write_flags & io::OBJECT_WRITE_FLAG_CREATE_EXCLUSIVE) !=
                     0);
            ...
            void build_aligned_data() {
                ...
                if (pre_align != 0) {
                    ...
                    io::util::unsparsify(image_ctx->cct, &extent.bl,
                                         extent.extent_map, extent.offset,
                                         extent.length);
            ...
            void handle_read(int r) {
                ...
                if (r == -ENOENT) {
                    if (may_copyup) {
                        auto ctx = create_context_callback <
                            ...
                            I >::handle_copyup > (this);
                        if (io::util::
                            trigger_copyup(image_ctx, object_no, io_context,
                                           ctx)) {
                            return;
                        }
                        delete ctx;
                ...
                if (!check_create_exclusive() || !check_version()
                    || !check_cmp_data()) {
                    return;
                }
                ...
                auto aligned_off =
                    crypto->align(object_off, data.length()).first;
                ...
                auto write_req = io::ObjectDispatchSpec::create_write(image_ctx,
                                                                      io::util::
                                                                      get_previous_layer
                                                                      (io::
                                                                       OBJECT_DISPATCH_LAYER_CRYPTO),
            ...
            void restart_request(bool may_copyup) {
                auto req =
                    new C_UnalignedObjectWriteRequest < I > (image_ctx, crypto,
                                                             object_no,
                                                             object_off,
                                                             std::move(data),
                                                             std::
        ...
        template < typename I >
            CryptoObjectDispatch < I >::CryptoObjectDispatch(I * image_ctx,
                                                             CryptoInterface *
                                                             crypto)
      :    m_image_ctx(image_ctx), m_crypto(crypto) {
            m_data_offset_object_no =
                Striper::get_num_objects(image_ctx->layout,
        ...
        template < typename I >
            bool CryptoObjectDispatch < I >::read(uint64_t object_no,
                                                  io::ReadExtents * extents,
                                                  IOContext io_context,
            ...
            *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
            if (m_crypto->is_aligned(*extents)) {
                auto req =
                    new C_AlignedObjectReadRequest < I > (m_image_ctx, m_crypto,
                                                          object_no, extents,
                                                          io_context,
                                                          op_flags, read_flags,
                                                          parent_trace, version,
                ...
                auto req =
                    new C_UnalignedObjectReadRequest < I > (m_image_ctx,
                                                            m_crypto, object_no,
                                                            extents, io_context,
                                                            op_flags,
                                                            read_flags,
        ...
        template < typename I >
            bool CryptoObjectDispatch < I >::write(uint64_t object_no,
                                                   uint64_t object_off,
                                                   ceph::bufferlist
                                                   && data,
                                                   IOContext io_context,
            ...
            if (m_crypto->is_aligned(object_off, data.length())) {
                auto r =
                    m_crypto->encrypt(&data,
                                      get_file_offset(m_image_ctx, object_no,
                                                      object_off));
                *dispatch_result =
                    r ==
                    0 ? io::DISPATCH_RESULT_CONTINUE : io::
                ...
                *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
                auto req =
                    new C_UnalignedObjectWriteRequest < I > (m_image_ctx,
                                                             m_crypto,
                                                             object_no,
                                                             object_off,
                                                             std::move(data),
        ...
        template < typename I >
            bool CryptoObjectDispatch < I >::write_same(uint64_t object_no,
                                                        uint64_t object_off,
                                                        uint64_t object_len,
                                                        io::
                                                        LightweightBufferExtents
                                                        && buffer_extents,
                                                        ceph::bufferlist
                                                        && data,
                                                        IOContext io_context,
            ...
            io::LightweightObjectExtent extent(object_no, object_off,
                                               object_len, 0);
        ...
        template < typename I >
            bool CryptoObjectDispatch <
            I >::compare_and_write(uint64_t object_no, uint64_t object_off,
                                   ceph::bufferlist
            ...
            *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
            auto req =
                new C_UnalignedObjectWriteRequest < I > (m_image_ctx, m_crypto,
        ...
        template < typename I >
            bool CryptoObjectDispatch < I >::discard(uint64_t object_no,
                                                     uint64_t object_off,
                                                     uint64_t object_len,
                                                     IOContext io_context,
                                                     int discard_flags,
          ...
          for (auto &[key, extent_map]:*snapshot_sparse_bufferlist) {
              ...
              for (auto & extent:extent_map) {
                    ...
                    if (sbe.state == io::SPARSE_EXTENT_STATE_DATA) {
                        current_bl.begin(extent.get_off()).copy_in(extent.
                                                                   get_len(),
                                                                   sbe.bl);
                    }
              ...
              for (auto & extent:extent_map) {
                    ...
                    auto[image_extents, _] =
                        io::util::object_to_area_extents(m_image_ctx, object_no,
                                                         { {
                                                         aligned_off,
                                                         aligned_len}
                                                         }
                  ...
                  for (auto[image_offset, image_length]:image_extents)
                    {
                        ...
                        aligned_bl.substr_of(current_bl, aligned_off + position,
                                             image_length);
