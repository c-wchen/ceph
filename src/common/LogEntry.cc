...
void LogEntryKey::dump(Formatter *f) const const
{
...
void LogEntryKey::generate_test_instances(list < LogEntryKey * > &o)
{
    ...
    o.push_back(new
                LogEntryKey(entity_name_t::CLIENT(1234), utime_t(1, 2), 34));
}
...
clog_type string_to_clog_type(const string &s)
{
    ...
    if (boost::iequals(s, "warning") ||
        boost::iequals(s, "warn") || boost::iequals(s, "wrn")) {
        return CLOG_WARN;
    }
...
int string_to_syslog_level(string s)
{
    ...
    if (boost::iequals(s, "crit") ||
        boost::iequals(s, "critical") || boost::iequals(s, "emerg")) {
        return LOG_CRIT;
    }
...
void LogEntry::log_to_syslog(string level, string facility) const const
{
    ...
    if (l <= min) {
        ...
        syslog(l | f, "%s %s %llu : %s",
               name.to_cstr(),
               stringify(rank).c_str(), (long long unsigned)seq, msg.c_str());
    }
}

void LogEntry::encode(bufferlist &bl, uint64_t features) const const
{
...
void LogEntry::dump(Formatter *f) const const
{
...
void LogSummary::build_ordered_tail_legacy(list < LogEntry > *tail) const const
{
    tail->clear();
    // channel -> (begin, end)
...
void LogSummary::encode(bufferlist &bl, uint64_t features) const const
{
...
void LogSummary::dump(Formatter *f) const const
{
