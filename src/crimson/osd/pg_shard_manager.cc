...
namespace crimson::osd {
    ...
    seastar::future <> PGShardManager::start(const int whoami,
                                             crimson::net::
                                             Messenger & cluster_msgr,
                                             crimson::net::
                                             Messenger & public_msgr,
                                             crimson::mon::Client & monc,
                                             crimson::mgr::Client & mgrc,
                                             crimson::os::
                                             FuturizedStore & store) {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return osd_singleton_state.start_single(whoami, std::ref(cluster_msgr),
                                                std::ref(public_msgr),
    ...
    seastar::future <> PGShardManager::stop() {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return shard_services.stop().then([this] {
                                          return osd_singleton_state.stop();}
        );
    }

    seastar::future <> PGShardManager::load_pgs(crimson::os::
                                                FuturizedStore & store) {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return store.list_collections().then([this] (auto colls_cores) {
                                             return seastar::
                                             parallel_for_each(colls_cores,
                                                               [this] (auto
                                                                       coll_core)
                                                               {
                                                               auto[coll,
                                                               ...
                                                               ("found temp collection on crimson osd, should be impossible: {}",
                                                                coll);
                                                               ceph_assert(0 ==
                                                                           "temp collection on crimson osd, should be impossible");
                                                               return seastar::
                                                               now();}
                                                               else {
                                                               logger().
                                                               warn
                                                               ("ignoring unrecognized collection: {}",
                                                                coll);
                                                               return seastar::
                                                               now();}
                                                               }
    ...
    seastar::future <> PGShardManager::stop_pgs() {
        ...
        return shard_services.invoke_on_all([](auto & local_service) {
                                            return local_service.local_state.
                                            stop_pgs();}
        );
    }
    ...
    seastar::future < std::map < pg_t, pg_stat_t >>
        ...
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return shard_services.map_reduce0([](auto & local) {
    ...
    seastar::future <> PGShardManager::broadcast_map_to_pgs(epoch_t epoch) {
        ...
        return shard_services.invoke_on_all([epoch] (auto & local_service) {
                                            return local_service.local_state.
                                            broadcast_map_to_pgs(local_service,
                                                                 epoch);}
        ).then([this, epoch] {
    ...
    seastar::future <> PGShardManager::set_up_epoch(epoch_t e) {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return shard_services.invoke_on_all(seastar::smp_submit_to_options {
                                            }
