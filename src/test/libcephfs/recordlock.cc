...
TEST(LibCephFS, ConcurrentRecordLocking)
{
    ...
    s.sem_init(0);
    ASSERT_EQ(0,
              pthread_create(&thread, NULL, thread_ConcurrentRecordLocking_,
                             &s));
...
TEST(LibCephFS, ThreesomeRecordLocking)
{
    ...
    s.sem_init(0);
    ASSERT_EQ(0,
              pthread_create(&thread[0], NULL, thread_ConcurrentRecordLocking_,
                             &s));
    ASSERT_EQ(0,
              pthread_create(&thread[1], NULL, thread_ConcurrentRecordLocking_,
                             &s));
    // Synchronization point with thread (failure: thread is dead)
    ...
    TWICE(WAIT_WORKER(6);       // (6)
          ...
          lock1.l_pid = getpid();
          ASSERT_EQ(-CEPHFS_EAGAIN,
                    ceph_ll_setlk(cmount, fh, &lock1, ceph_pthread_self(),
                                  false)); lock1.l_type = F_RDLCK;
