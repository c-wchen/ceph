...
namespace librados {
      ...
      for (auto & watch_handle_pair:watcher->watch_handles) {
        ...
        FunctionContext *ctx =
            new FunctionContext(boost::
                                bind(&TestWatchNotify::execute_notify, this,
                                     rados_client, oid, bl,
                                     notify_id));
    ...
    int TestWatchNotify::unwatch(TestRadosClient * rados_client,
        ...
        for (FileWatchers::iterator it = m_file_watchers.begin();
            ...
            if (w_it != watcher->watch_handles.end()) {
                ...
                if (watcher->watch_handles.empty()
                    && watcher->notify_handles.empty()) {
    ...
    TestWatchNotify::SharedWatcher TestWatchNotify::get_watcher(const std::
                                                                string & oid) {
        ...
        for (WatcherIDs::iterator w_id_it = watcher_ids.begin();
            ...
            if (w_it == watch_handles.end()) {
                ...
                watch_handle.rados_client->get_aio_finisher()->
                    queue(new
                          FunctionContext([this, oid, bl, notify_id,
