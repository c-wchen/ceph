...
namespace rocksdb_cache {
    ...
    BinnedLRUHandleTable::BinnedLRUHandleTable():list_(nullptr), length_(0),
        elems_(0) {
        Resize();
    ...
    BinnedLRUHandle *BinnedLRUHandleTable::Lookup(const rocksdb::Slice & key,
                                                  uint32_t hash) {
    ...
    BinnedLRUHandle *BinnedLRUHandleTable::Remove(const rocksdb::Slice & key,
                                                  uint32_t hash) {
    ...
    BinnedLRUHandle **BinnedLRUHandleTable::FindPointer(const rocksdb::
                                                        Slice & key,
                                                        uint32_t hash) {
    ...
    BinnedLRUCacheShard::BinnedLRUCacheShard(CephContext * c, size_t capacity,
                                             bool strict_capacity_limit,
                                             double high_pri_pool_ratio)
    :cct(c), capacity_(0), high_pri_pool_usage_(0),
        strict_capacity_limit_(strict_capacity_limit),
        high_pri_pool_ratio_(high_pri_pool_ratio), high_pri_pool_capacity_(0),
        usage_(0), lru_usage_(0), age_bins(1) {
        shift_bins();
...
// Call deleter and free
    ...
    void BinnedLRUCacheShard::ApplyToAllCacheEntries(const std::function <
                                                     void (const rocksdb::
                                                           Slice & key,
                                                           void *value,
                                                           size_t charge,
        ...
        table_.ApplyToAllCacheEntries([callback] (BinnedLRUHandle * h) {
                                      callback(h->key(), h->value, h->charge,
                                               h->deleter);}
        );
        if (thread_safe) {
            mutex_.unlock();
    ...
    void BinnedLRUCacheShard::TEST_GetLRUList(BinnedLRUHandle ** lru,
    ...
    bool BinnedLRUCacheShard::Release(rocksdb::Cache::Handle * handle,
                                      bool force_erase) {
        if (handle == nullptr) {
            return false;
        }
    ...
    rocksdb::Status BinnedLRUCacheShard::Insert(const rocksdb::Slice & key,
    ...
    :ShardedCache(capacity, num_shard_bits, strict_capacity_limit), cct(c) {
        ...
        for (int i = 0; i < num_shards_; i++) {
            new(&shards_[i])
                BinnedLRUCacheShard(c, per_shard, strict_capacity_limit,
                                    high_pri_pool_ratio);
        }
    }
...
// PriCache
    ...
    int64_t BinnedLRUCache::request_cache_bytes(PriorityCache::Priority pri,
        ...
        case PriorityCache::Priority::PRI0:
            {
                ...
                // priorities.
                request =
                    PriorityCache::get_chunk(GetHighPriPoolUsage(),
                                             total_cache);
                break;
        ...
        ldout(cct,
              10) << __func__ << " Priority: " << static_cast < uint32_t > (pri)
    ...
    int64_t BinnedLRUCache::commit_cache_size(uint64_t total_bytes) {
        ...
        int64_t new_bytes =
            PriorityCache::get_chunk(get_cache_bytes(), total_bytes);
        ldout(cct,
              10) << __func__ << " old: " << old_bytes << " new: " << new_bytes
            << dendl;
        ...
        ldout(cct,
              5) << __func__ << " High Pri Pool Ratio set to " << ratio <<
            dendl;
    ...
    std::shared_ptr < rocksdb::Cache > NewBinnedLRUCache(CephContext * c,
                                                         size_t capacity,
                                                         int num_shard_bits,
                                                         bool
