...
namespace ceph {
    namespace logging {
        ...
        :m_subs(s),
            ...
            m_formatter =
                std::unique_ptr < Formatter > (Formatter::create("json"));
            m_formatter_section =
                std::unique_ptr < Formatter > (Formatter::create("json"));
        ...
        void Graylog::set_destination(const std::string & host, int port) {
            try {
                ...
                cerr << "Error resolving graylog destination: " << e.
                    what() << std::endl;
                m_log_dst_valid = false;
            }
        }
        ...
        void Graylog::log_entry(Entry const *const e) {
            if (m_log_dst_valid) {
                ...
                m_formatter->dump_float("timestamp",
                                        e->m_stamp.sec() +
                                        (e->m_stamp.usec() / 1000000.0));
                m_formatter->dump_unsigned("_thread", (uint64_t) e->m_thread);
                m_formatter->dump_int("_level", e->m_prio);
                if (m_subs != NULL)
                    m_formatter->dump_string("_subsys_name",
                                             m_subs->get_name(e->m_subsys));
                ...
                try {
                    ...
                    socket.
                        send_to(boost::asio::buffer(m_ostream_compressed.str()),
                                m_endpoint);
                }
                catch(boost::system::system_error const &e) {
                    cerr << "Error sending graylog message: " << e.
                        what() << std::endl;
                }
            }
        }
