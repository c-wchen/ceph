...
void test_stats(librados::IoCtx & ioctx, string & oid, int category,
...
void index_prepare(OpMgr & mgr, librados::IoCtx & ioctx, string & oid,
                   RGWModifyOp index_op, string & tag, string & obj,
                   string & loc, uint16_t bi_flags = 0)
{
    ObjectWriteOperation *op = mgr.write_op();
    cls_rgw_obj_key key(obj, string());
    rgw_zone_set zones_trace;
    cls_rgw_bucket_prepare_op(*op, index_op, tag, key, loc, true, bi_flags,
                              zones_trace);
    ASSERT_EQ(0, ioctx.operate(oid, op));
}

void index_complete(OpMgr & mgr, librados::IoCtx & ioctx, string & oid,
                    RGWModifyOp index_op, string & tag, int epoch, string & obj,
...
#define NUM_OBJS 10
    for (int i = 0; i < NUM_OBJS; i++) {
        ...
        meta.category = 0;
        meta.size = obj_size;
        index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, epoch, obj,
                       meta);
    }
...
TEST(cls_rgw, index_multiple_obj_writers)
{
    ...
    for (int i = NUM_OBJS; i > 0; i--) {
        ...
        index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, i, obj,
                       meta);
...
TEST(cls_rgw, index_remove_object)
{
    ...
    for (int i = 0; i < NUM_OBJS; i++) {
        ...
        index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, ++epoch,
                       obj, meta);
    ...
    index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_DEL, tag_remove, ++epoch,
                   obj, meta);
    ...
    index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_ADD, tag_modify, ++epoch,
                   obj, meta);
...
TEST(cls_rgw, index_suggest)
{
    ...
    for (int i = num_objs / 2; i < num_objs; i++) {
        ...
        rgw_bucket_dir_entry_meta meta;
        index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_DEL, tag, ++epoch,
                       obj, meta);
...
TEST(cls_rgw, index_list)
{
    ...
    int r =
        CLSRGWIssueBucketList(ioctx, start_key, "", 1000, true, oids,
                              list_results, 1) ();
...
TEST(cls_rgw, bi_list)
{
    ...
    for (uint64_t i = 0; i < num_objs; i++) {
        ...
        meta.size = obj_size;
        index_complete(mgr, ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, epoch, obj,
                       meta, RGW_BILOG_FLAG_VERSIONED_OP);
    }

    ret =
        cls_rgw_bi_list(ioctx, bucket_oid, name, marker, num_objs + 10,
    ...
    while (is_truncated) {
        ...
        if (is_truncated) {
            ASSERT_EQ(entries.size(),
                      std::min(max,
                               cct->_conf->osd_max_omap_entries_per_request));
        }
        else {
    ...
    if (cct->_conf->osd_max_omap_entries_per_request < 15) {
        ...
        while (is_truncated) {
            ret =
                cls_rgw_bi_list(ioctx, bucket_oid, name, marker, max, &entries,
                                &is_truncated);
            ASSERT_EQ(ret, 0);
            if (is_truncated) {
                ASSERT_EQ(entries.size(),
...
TEST(cls_rgw, usage_basic)
{
    ...
    ASSERT_EQ(0,
              cls_rgw_usage_log_trim(ioctx, oid, user, start_epoch, end_epoch));
