...
OSDService::OSDService(OSD *osd, ceph::async::io_context_pool &poolctx):
    ...
    osd_skip_data_digest(cct->_conf, "osd_skip_data_digest"), publish_lock {
...
cur_state(NONE),
cur_ratio(0), physical_ratio(0), boot_epoch(0), up_epoch(0), bind_epoch(0)
{
    objecter->init();
    ...
    for (int i = 0; i < m_objecter_finishers; i++) {
        ...
        auto fin =
            make_unique < Finisher > (osd->client_messenger->cct, str.str(),
                                      "finisher");
...
ceph::signedspan OSDService::get_mnow() constconst {
    return ceph::mono_clock::now() - osd->startup_time;
}

void OSDService::identify_splits_and_merges(OSDMapRef old_map,
        OSDMapRef new_map,
        spg_t pgid,
        set < pair < spg_t,
        epoch_t >> *split_children,
        set < pair < spg_t,
        epoch_t >> *merge_pgs)
{
                ...
                if (cur.ps() >= q->second) {
                    if (cur.ps() < pgnum) {
                        ...
                        if (cur.is_merge_source(pgnum, q->second, &parent)) {
                            ...
                            dout(20) << __func__ << " " << cur << " e" << q->
                                     first << " pg_num " << pgnum << " -> " << q->
                                     second << " is merge source, target " << parent
...
void OSDService::queue_renew_lease(epoch_t epoch, spg_t spgid)
{
    osd->enqueue_peering_evt(spgid,
                             PGPeeringEventRef(std::make_shared <
                                     PGPeeringEvent > (epoch, epoch,
                                             RenewLease
                                             ())));
}
...
bool OSDService::_check_inject_full(DoutPrefixProvider *dpp, s_names type) const const
{
    if (injectfull && injectfull_state >= type) {
        ...
        ldpp_dout(dpp,
                  10) << __func__ << " Injected " << get_full_state_name(type)
                      << " OSD (" << (injectfull <
                                      0 ? "set" : std::
                                      to_string(injectfull)) << ")" << dendl;
        return true;
    }
    return false;
}

bool OSDService::_check_full(DoutPrefixProvider *dpp, s_names type) const const
{
    std::lock_guard l(full_status_lock);
...
bool OSDService::_tentative_full(DoutPrefixProvider *dpp, s_names type,
                                 uint64_t adjust_used, osd_stat_t adjusted_stat)
{
    ldpp_dout(dpp,
              20) << __func__ << " type " << get_full_state_name(type) <<
                  " adjust_used " << (adjust_used >> 10) << "KiB" << dendl;
    {
        std::lock_guard l(full_status_lock);
        if (_check_inject_full(dpp, type)) {
            return true;
    ...
    if (tentative_state >= type)
        ldpp_dout(dpp,
                  10) << __func__ << " tentative usage is " << ratio << dendl;

    return tentative_state >= type;
}

bool OSDService::check_failsafe_full(DoutPrefixProvider *dpp) const const
{
    return _check_full(dpp, FAILSAFE);
}

bool OSDService::check_full(DoutPrefixProvider *dpp) const const
{
    return _check_full(dpp, FULL);
}

bool OSDService::tentative_backfill_full(DoutPrefixProvider *dpp,
        uint64_t adjust_used, osd_stat_t stats)
{
    return _tentative_full(dpp, BACKFILLFULL, adjust_used, stats);
}

bool OSDService::check_backfill_full(DoutPrefixProvider *dpp) const const
{
    return _check_full(dpp, BACKFILLFULL);
}

bool OSDService::check_nearfull(DoutPrefixProvider *dpp) const const
{
                            ...
                            osd_alert_list_t &alerts)
{
    ...
    if (cct->_conf->fake_statfs_for_testing) {
        ...
        dout(0) << __func__ << " fake total " << cct->_conf->
                fake_statfs_for_testing << " adjust available " << avail << dendl;
        used = bytes - avail;
    }
...
osd_stat_t OSDService::set_osd_stat(vector < int > &hb_peers, int num_pgs)
{
    utime_t now = ceph_clock_now();
    auto stale_time =
        g_conf().get_val < int64_t > ("osd_mon_heartbeat_stat_stale");
    std::lock_guard l(stat_lock);
...
{
    *pratio =
        ((float)new_stat.statfs.get_used_raw()) /
        ((float)new_stat.statfs.total);

    if (adjust_used) {
        dout(20) << __func__ << " Before kb_used() " << new_stat.statfs.
                 kb_used() << dendl;
        ...
        dout(20) << __func__ << " After kb_used() " << new_stat.statfs.
                 kb_used() << dendl;
    }
        ...
        epoch_t from_epoch)
{
    ...
    if (peer == whoami) {
        ...
        peer_con =
            osd->cluster_messenger->connect_to_osd(next_map->
...
void OSDService::send_message_osd_cluster(std::vector < std::pair < int,
        Message * >> &messages,
        epoch_t from_epoch)
{
        ...
        from_epoch)
{
    ...
    ret.first =
        osd->hb_back_client_messenger->connect_to_osd(next_map->
            get_hb_back_addrs(peer));
    ret.second =
        osd->hb_front_client_messenger->connect_to_osd(next_map->
            get_hb_front_addrs
            (peer));
    release_map(next_map);
    return ret;
...
entity_name_t OSDService::get_cluster_msgr_name() const const
{
...
bool OSDService::prepare_to_stop()
{
    ...
    if (osdmap && osdmap->is_up(whoami)) {
        ...
        monc->send_mon_message(new MOSDMarkMeDown(monc->get_fsid(), whoami, osdmap->get_addrs(whoami), osdmap->get_epoch(),
                               true,   // request ack
        ...
        OSDSuperblock &sblock)
{
    ...
    if (since < m->cluster_osdmap_trim_lower_bound) {
        ...
        dout(10) << __func__ << " cluster osdmap lower bound "
                 ...
                 << " > since " << since << ", starting with full map" << dendl;
...
panic:
    ...
    if (get_inc_map_bl(m->newest_map, bl)) {
        ...
        derr << __func__ << " unable to load latest map " << m->
             newest_map << dendl;
        if (!get_map_bl(m->newest_map, bl)) {
            derr << __func__ << " unable to load latest full map " << m->
                 newest_map << dendl;
            ceph_abort();
        }
...
void OSDService::handle_misdirected_op(PG *pg, OpRequestRef op)
{
    ...
    clog->warn() << m->get_source_inst() << " misdirected " << m->get_reqid()
                 << " pg " << m->get_raw_pg()
                 << " to osd." << whoami << " not " << pg->get_acting()
...
()
{
    dout(20) << __func__ << " ready_to_merge_source " << ready_to_merge_source << " not_ready_to_merge_source " <<
             not_ready_to_merge_source << " ready_to_merge_target " << ready_to_merge_target << " not_ready_to_merge_target " <<
             not_ready_to_merge_target << " sent_ready_to_merge_source " << sent_ready_to_merge_source << dendl;
     ...
     osdspec_affinity)
{
    ...
    if (ch) {
        ...
        if (ret < 0) {
            derr <<
                 "OSD::mkfs: have meta collection but no superblock"
                 << dendl;
            return ret;
        }
...
OSD::OSD(CephContext *cct_, std::unique_ptr < ObjectStore > store_, int id, Messenger *internal_messenger,
         Messenger *external_messenger, Messenger *hb_client_front, Messenger *hb_client_back, Messenger *hb_front_serverm,
         Messenger *hb_back_serverm, Messenger *osdc_messenger, MonClient *mc, const std::string &dev, const std::string &jdev,
    ...
    if (r >= 0
        ...
        if (r >= 0
            ...
            if (front_node ==
                ...
                store_node) {
                dout(1) <<
                        " objectstore and network numa nodes all match"
                        << dendl;
                if (g_conf().
                    get_val <
                    bool >
            ...
            dout(1) << __func__
                    ...
                    " cluster network "
                    << back_iface <<
                    " ports numa nodes do not match"
                    << dendl;
        } else {
            derr << __func__ <<
                 " unable to identify cluster interface '"
                 << back_iface <<
                 "' numa node: " <<
                 cpp_strerror(r) <<
                 dendl;
        ...
        derr << __func__ <<
             " unable to identify public interface '"
             << front_iface <<
             "' numa node: " <<
             cpp_strerror(r) <<
             dendl;
...
will start to track new ops received afterwards."; 
        ...
        (!ceph_heap_set_numeric_property
         (property.c_str(),
          (size_t) value)) {
            error =
                "invalid property";
        ...
        (!ceph_heap_get_numeric_property
         (property.c_str(),
          &value)) {
            error =
                "invalid property";
        ...
        if (ret < 0) {
            ss <<
               "Error flushing objectstore cache: "
               <<
               cpp_strerror(ret);
            goto out;
        }
...
class TestOpsSocketHook:                                    public
    AdminSocketHook
{
    OSDService *service;
    ObjectStore *store;
public:
    TestOpsSocketHook(OSDService *s, ObjectStore *st):      service(s),
        store(st)
    {
    }
    int call(std::
    ...
    void test_ops(OSDService *service, ObjectStore *store, std::string_view command, const cmdmap_t &cmdmap, ostream &ss);
};
...
int OSD::init()
{
    ...
    // sanity check long object name handling
    {
        ...
        if (r < 0) {
            derr << "backend ("
                 << store->
                 get_type() <<
                 ") is unable to support max "
                 <<
                 "object name[space] len"
                 << dendl;
            derr <<
            ...
            dout(20) <<
                     "configured osd_max_object_name[space]_len looks ok"
                     << dendl;
        }
    }
        ...
        if (osd_compat.
            ...
            CompatSet diff =
                ...
                (superblock.
                 compat_features);
            derr <<
                 "Cannot write to disk! Missing features: "
                 << diff << dendl;
            r = -EOPNOTSUPP;
            goto out;
        }
    ...
    if (get_osdmap()) {
        derr <<
             "OSD::init: unable to read current osdmap"
             << dendl;
        r = -EINVAL;
        goto out;
    }
            ...
            if (c.is_pg(&pgid)
                ...
                if (!store->
                    ...
                    derr << __func__ <<
                         ...
                         <<
                         "pg deletion to complete before upgrading"
                         << dendl;
                    ceph_abort();
                }
            }
    ...
    if (superblock.
        ...
        if (diff.incompat.
            contains
            (CEPH_OSD_FEATURE_INCOMPAT_SNAPMAPPER2)) {
            dout(1) << __func__
                    <<
    ...
    if (!store->
        exists(service.
               ...
               ())) {
        dout(10) <<
                 "init creating/touching purged_snaps object"
                 << dendl;
        ObjectStore::
        Transaction t;
        t.
    ...
    if (cct->_conf.
        ...
        dout(2) <<
                "compacting object store's omap"
                << dendl;
        store->compact();
    }
    ...
    monc->
    set_handle_authentication_dispatcher
    (this);
    monc->
    set_want_keys
        ...
        (rotating_auth_attempts
         ...
         max_rotating_auth_attempts) {
            derr << __func__ <<
                 ...
                 <<
                 " -- maybe I have a clock skew against the monitors?"
                 << dendl;
            exit(1);
        }
    }
    ...
    check_config();
    dout(10) <<
             "ensuring pgs have consumed prior maps"
             << dendl;
    consume_map();
    dout(0) <<
...
final_init()
{
    ...
    r =
        admin_socket->
        register_command
        ("dump_osd_network name=value,type=CephInt,req=false",
         asok_hook,
    ...
    r =
        admin_socket->
        register_command
        ("dump_pool_statfs name=poolid,type=CephInt,req=true",
         asok_hook,
...
int OSD::shutdown()
{
    ...
    dout(0) <<
            "Fast Shutdown: - cct->_conf->osd_fast_shutdown = "
            << cct->_conf->
            osd_fast_shutdown
            << ", null-fm = "
            << store->
    ...
    if (cct->_conf->
        ...
        dout(0) <<
                "Fast Shutdown duration total     :"
                << end_time -
                start_time_func <<
                " seconds" <<
                dendl;
        dout(0) <<
                "Fast Shutdown duration osd_drain :"
                <<
                start_time_umount -
                start_time_osd_drain
                << " seconds" <<
                dendl;
        dout(0) <<
                "Fast Shutdown duration umount    :"
                << end_time -
                start_time_umount
                << " seconds" <<
                dendl;
        dout(0) <<
                "Fast Shutdown duration timer     :"
                <<
                start_time_osd_drain
                -
                start_time_timer <<
...
()
{
    ...
    if (device_class.
        ...
        dout(20) <<
                 __func__ <<
                 " no device class stored locally"
                 << dendl;
        return 0;
    }
    ...
    for (vector <
        ...
        if (it->
            ...
            dout(10) <<
                     "load_pgs " << *it
                     <<
                     " removing, legacy or flagged for removal pg"
                     << dendl;
            recursive_remove_collection
            (cct, store.get(),
             pgid, *it);
        ...
        if (!it->
            is_pg(&pgid)) {
            dout(10) <<
                     "load_pgs ignoring unrecognized "
                     << *it << dendl;
            continue;
        }
 ...
 * info)
{
    ...
    if (info->by_mon) {
        ...
        if (osdmap->
            ...
            dout(20) <<
                     ...
                     "  dropping " <<
                     pgid <<
                     "create, pool does not have CREATING flag set"
                     << dendl;
            return nullptr;
        }
    }
    ...
    if (pg->
        ...
        lock_guard locker {
            m_perf_queries_lock};
        pg->
        set_dynamic_perf_stats_queries
        (m_perf_queries);
    }
...
(int p)
{
    ...
    if (i ==
        ...
        set_priv(sf);
        dout(10) <<
                 "_add_heartbeat_peer: new peer osd."
                 << p << " " << hi->
                 con_back->
                 get_peer_addr()
                 << " " << hi->
...
()
{
    ...
    l(heartbeat_lock);
    dout(10) <<
             "maybe_update_heartbeat_peers updating"
             << dendl;
            ...
            if (i !=
                ...
                if (acked !=
                    ...
                    if (con ==
                        ...
                        con_back) {
                        dout(25) <<
                                 "handle_osd_ping got reply from osd."
                                 << from <<
                                 " first_tx " << i->
                                 second.
                                 first_tx <<
                    ...
                    if (unacknowledged
                        ...
                        dout(25) <<
                                 "handle_osd_ping got all replies from osd."
                                 << from <<
                                 " , erase pending ping(sent at "
                                 << m->
                                 ping_stamp << ")"
                                 <<
...
#define ROUND_S_TO_USEC(sec) (uint32_t)((sec) * 1000 * 1000 + 0.5)
                    ...
                    if (i->second.
                        ...
                        (failure_queue_entry
                         ...
                         failure_queue.
                            ...
                            dout(10) <<
                                     "handle_osd_ping canceling queued "
                                     <<
                                     "failure report for osd."
                                     << from << dendl;
                            failure_queue.
                            erase
                            (failure_queue_entry);
                        ...
                        (failure_pending_entry
                         ...
                         failure_pending.
                            ...
                            dout(10) <<
                                     "handle_osd_ping canceling in-flight "
                                     <<
                                     "failure report for osd."
                                     << from << dendl;
                            send_still_alive
                    ...
                    dout(10) <<
                             "handle_osd_ping no pending ping(sent at "
                             << m->
                             ping_stamp <<
...
heartbeat_check()
{
    ...
    for (map < int,
        ...
        if (p->second.
            ...
            utime_t()) {
            dout(25) <<
                     "heartbeat_check we haven't sent ping to osd."
                     << p->
                     first <<
                     " yet, skipping" <<
                     dendl;
 ...
 con)
{
    ...
    if (s) {
        ...
        if (p !=
            ...
            dout(10) <<
                     "heartbeat_reset closing (old) failed hb con "
                     << con << dendl;
        }
    }
    return true;
 ...
 con)
{
    if (con->
        ...
        if (auto s =
            ...
            con->set_priv(s);
            dout(10) <<
                     "new session (incoming)"
                     << s << " con=" <<
                     con << " addr=" <<
                     con->
                     get_peer_addr()
                     <<
                     " must have raced with connect"
                     << dendl;
            ceph_assert(con->
                        get_peer_type
                        () ==
         ...
         newest)
{
    ...
    if (osdmap->
        ...
        0) {
        derr <<
             "waiting for initial osdmap"
             << dendl;
    } else if (osdmap->
               is_destroyed
               (whoami)) {
        derr <<
             "osdmap says I am destroyed"
             << dendl;
        ...
        derr <<
             ...
             << dendl;
               ...
               ()) {
        derr <<
             "osdmap fullness state needs update"
             << dendl;
        send_full_update
        ();
    } else if (monmap.
...
_is_healthy()
{
    if (!cct->
        ...
        is_healthy()) {
        dout(1) <<
                "is_healthy false -- internal heartbeat failed"
                << dendl;
        return false;
    }
...
_send_boot()
{
    ...
    (cluster_messenger->
     ...
     (client_addrs)) {
        dout(10) <<
                 " assuming cluster_addrs match client_addrs "
                 << client_addrs <<
                 dendl;
        cluster_addrs =
            cluster_messenger->
    ...
    (hb_back_server_messenger->
     ...
     (cluster_addrs)) {
        dout(10) <<
                 " assuming hb_back_addrs match cluster_addrs "
                 << cluster_addrs <<
                 dendl;
        hb_back_addrs =
            hb_back_server_messenger->
    ...
    (hb_front_server_messenger->
     ...
     (client_addrs)) {
        dout(10) <<
                 " assuming hb_front_addrs match client_addrs "
                 << client_addrs <<
                 dendl;
        hb_front_addrs =
            hb_front_server_messenger->
...
()
{
    ...
    dout(10) <<
             __func__ <<
             " done queueing pgs, updating superblock"
             << dendl;
    ObjectStore::
    Transaction t;
    superblock.
    ...
    for (auto dev :             devnames) {
        ...
        (block_device_get_metrics
         (dev,
          ...
          &smart_json)) {
            dout(10) <<
                     "block_device_get_metrics failed for /dev/"
                     << dev << dendl;
            continue;
        }
        json_map[devid] =
 ...
 OSDMapRef osdmap)
{
    ...
    if (session->
        ...
        register_session_waiting_on_map
        (session);
    }
}
...
sched_scrub()
{
    ...
    (scrub_scheduler.
     ...
     ()) {
        dout(20) <<
                 __func__ <<
                 ": scrub resources reservation in progress"
                 << dendl;
        return;
    }
...
(MOSDMap *m)
{
    ...
    for (epoch_t e =
        ...
        if (p !=
            ...
            dout(10) <<
                     "handle_osd_map  got full map for epoch "
                     << e << dendl;
            OSDMap *o =
                new OSDMap;
            bufferlist &bl =
        ...
        if (p !=
            ...
            dout(10) <<
                     "handle_osd_map  got inc map for epoch "
                     << e << dendl;
            bufferlist &bl =
                p->second;
            ghobject_t oid =
        ...
        ceph_abort_msg
        ("MOSDMap lied about what maps it had?");
    }
    ...
    for (auto &i :                         added_maps) {
        if (!lastmap) {
            if (!
                ...
                dout(10) <<
                         __func__ <<
                         " can't get previous map "
                         << i.first -
                         1 <<
                         " probably first start of this osd"
                         << dendl;
                continue;
            }
        }
    ...
    if (superblock.
        ...
        dout(10) <<
                 ...
                 purged_snaps_last
                 <<
                 ", not recording new purged_snaps"
                 << dendl;
    }
 ...
 MOSDMap *m)
{
    ...
    if (osdmap->
        ...
        if (!osdmap->
            ...
            // everything paused
                   ...
                   (whoami)) {
            derr <<
                 "map says i am stopped by admin. shutting down."
                 << dendl;
            do_shutdown =
                true;
        } else if (!osdmap->
            ...
            if (!service.
                ...
                if (r != 0) {
                    ...
                    derr << __func__ <<
                         " marked down:" <<
                         " rebind cluster_messenger failed"
                         << dendl;
                }
    ...
    if (do_shutdown) {
        ...
        queue_async_signal
        (SIGINT);
    } else if (m->newest_map
               && m->
               newest_map >
               last) {
        dout(10) <<
                 " msg say newest map is "
                 << m->
                 newest_map <<
                 ", requesting more"
                 << dendl;
    ...
    }
    {
        ...
        if ((p.
            ...
            dout(0) <<
                    ...
                    features_required
                    <<
                    ", adjusting msgr requires for mons"
                    << dendl;
            p.
            features_required =
                (p.
        ...
        for (auto pg =
            ...
            if (osdmap->
                ...
                dout(10) <<
                         ...
                         " doesn't map here, "
                         <<
                         "discarding pending_create_from_osd"
                         << dendl;
                pg =
                    pending_creates_from_osd.
                    erase(pg);
...
activate_map()
{
    ...
    if (osdmap->
        ...
        if (!service.
            ...
            ()) {
            dout(1) <<
                    "pausing recovery (NORECOVER flag set)"
                    << dendl;
            service.
            pause_recovery();
        }
 ...
 m)
{
    if (!m->
        ...
        peer_is_mgr()) {
        dout(0) <<
                "require_mon_or_mgr_peer received from non-mon, non-mgr "
                << m->
                get_connection()->
                get_peer_addr()
                << " " << *m <<
        ...
        ();
    {
        ...
        if (recovery_sleep
            ...
            recovery_requeue_callback
                =
                    new
            LambdaContext([this, pgref, queued, reserved_pushes, priority](int r) {
                dout
                (20)
                        <<
                        "do_recovery wake up at "
                        <<
                        ceph_clock_now
                        ()
                        <<
             ...
             *merge_pgs)
{
    ...
    while (p !=
        ...
        if (slot->pg) {
            ...
            dout(20) <<
                     __func__ <<
                     "  creating empty merge participant "
                     << pgid <<
                     " for merge in " <<
                     epoch << dendl;
        ...
        if (!slot->
            ...
            if (!qi.
                ...
                if (create_info) {
                    ...
                    dout(20) <<
                             __func__ << " " <<
                             token <<
                             " no pg, peering, !create, discarding "
                             << qi << dendl;
                }
            } else {
                dout(20) <<
                         __func__ << " " <<
                         token <<
                         " no pg, peering, doesn't map here e"
                         << osdmap->
                         get_epoch()
                         << ", discarding "
                         << qi << dendl;
            ...
            if (std::optional <
                ...
                _op =
                    ...
                    maybe_get_op()) {
                osd->service.
                maybe_share_map((*_op)->get_req()->get_connection().get(), sdata->shard_osdmap, (*_op)->sent_epoch);
            }
            unsigned
            pushes_to_free =
                qi.
         ...
         erros)
{
    ...
    if (!cmd_getval
        (cmdmap,
         ...
         cmd)) {
        erros <<
              "unable to get value for command \""
              << cmd << "\"";
        return -EINVAL;
    }
