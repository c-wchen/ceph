        ...
        namespace snapshot {
            ...
            using librbd::util::create_rados_callback;
             ...
             template < typename I > void RemoveImageStateRequest < I >::send() {
                ...
                auto oid =
                    util::image_state_object_name(m_image_ctx, m_snap_id, 0);
                 ...
                 librados::AioCompletion * comp = create_rados_callback <
                    ...
                    &RemoveImageStateRequest < I >::handle_get_object_count >
                    (this);
                int r =
                ...
                void RemoveImageStateRequest <
                I >::handle_get_object_count(int r) {
                CephContext *cct = m_image_ctx->cct;
                ...
                try {
                    ...
                    lderr(cct) << "failed to decode image state object header"
                        << dendl;
                    // still try to remove it
                }

                m_object_count =
                    header.object_count > 0 ? header.object_count : 1;
            ...
            template < typename I >
                ...
                librados::AioCompletion * comp = create_rados_callback <
                    RemoveImageStateRequest < I >,
                    &RemoveImageStateRequest < I >::handle_remove_object >
                    (this);
            ...
            template < typename I >
                void RemoveImageStateRequest <
                I >::handle_remove_object(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;

                if (r < 0 && r != -ENOENT) {
                    lderr(cct) << "failed to remove image state object: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
...
template class librbd::mirror::snapshot::RemoveImageStateRequest <
    librbd::ImageCtx >;
