...
TEST(CommonIPAddr, TestV6_PrefixTooLong)
{
    ...
    ASSERT_FALSE(matches_ipv6_in_subnet
                 (one, (struct sockaddr_in6 *)&net, 9000));
}
...
TEST(CommonIPAddr, ParseNetwork_Bad_IPv6Slash)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/", &network,
                       &prefix_len);
    ASSERT_EQ(ok, false);
}
...
TEST(CommonIPAddr, ParseNetwork_Bad_IPv6SlashNegative)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/-3", &network,
                       &prefix_len);
    ASSERT_EQ(ok, false);
}
...
TEST(CommonIPAddr, ParseNetwork_Bad_IPv6SlashJunk)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/foo", &network,
                       &prefix_len);
    ASSERT_EQ(ok, false);
}
...
TEST(CommonIPAddr, ParseNetwork_IPv6_0)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/0", &net_storage,
                       &prefix_len);
    ...
    ASSERT_EQ(0,
              memcmp(want.sin6_addr.s6_addr, network.sin6_addr.s6_addr,
...
TEST(CommonIPAddr, ParseNetwork_IPv6_67)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/67", &net_storage,
                       &prefix_len);
    network = *(struct sockaddr_in6 *)&net_storage;
    ASSERT_EQ(ok, true);
...
TEST(CommonIPAddr, ParseNetwork_IPv6_128)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/128", &net_storage,
                       &prefix_len);
    network = *(struct sockaddr_in6 *)&net_storage;
    ASSERT_EQ(ok, true);
...
TEST(CommonIPAddr, ParseNetwork_IPv6_9000)
{
    ...
    ok = parse_network("2001:1234:5678:90ab::dead:beef/9000", &net_storage,
                       &prefix_len);
    network = *(struct sockaddr_in6 *)&net_storage;
    ASSERT_EQ(ok, true);
...
TEST(pick_address, find_ip_in_subnet_list)
{
    ...
    // match by network
    result = find_ip_in_subnet_list(cct.get(),
                                    &one,
...
TEST(pick_address, filtering)
{
    ...
    entity_addrvec_t av;
    {
        int r = pick_addresses(cct.get(),
                               ...
                               CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_MSGR1,
                               &one, &av);
        ASSERT_EQ(0, r);
    ...
    }
    {
        ...
        int r = pick_addresses(cct.get(),
                               CEPH_PICK_ADDRESS_PUBLIC |
                               CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_MSGR2,
                               &one, &av);
        ASSERT_EQ(0, r);
        ASSERT_EQ(1u, av.v.size());
...

    {
        ...
        ASSERT_EQ(0, r);
        ASSERT_EQ(1u, av.v.size());
        ASSERT_EQ(string("v2:[2001:1234:5678:90ab::cdef]:0/0"),
                  stringify(av.v[0]));
        cct->_conf.set_val("public_network", "");
    }
    {
    ...
    }
    {
        ...
        ASSERT_EQ(2u, av.v.size());
        ASSERT_EQ(string("v1:10.2.1.123:0/0"), stringify(av.v[0]));
        ASSERT_EQ(string("v1:[2001:1234:5678:90ab::cdef]:0/0"),
                  stringify(av.v[1]));
        cct->_conf.set_val("public_network", "");
        cct->_conf.set_val("public_network_interface", "");
    }
