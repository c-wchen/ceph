...
namespace rbd {
    namespace mirror {
        namespace image_deleter {
                ...
                m_lock(ceph::
                       make_mutex(librbd::util::
                                  ...
                                  ("rbd::mirror::image_deleter::TrashWatcher",
                                   this))) {
            } template < typename I > void TrashWatcher <
                I >::init(Context * on_finish) {
                dout(5) << dendl;
            ...
            template < typename I >
                void TrashWatcher <
                I >::handle_image_added(const std::string & image_id,
            ...
            template < typename I >
                void TrashWatcher <
                I >::handle_image_removed(const std::string & image_id) {
            ...
            template < typename I > void TrashWatcher < I >::create_trash() {
                ...
                auto aio_comp = create_rados_callback <
                    TrashWatcher < I >,
                    &TrashWatcher < I >::handle_create_trash > (this);
            ...
            template < typename I >
                ...
                if (r == -EBLOCKLISTED || r == -ENOENT) {
                    ...
                    derr << "failed to create trash object: " << cpp_strerror(r)
                        << dendl;
                    {
                        std::lock_guard locker {
                        m_lock};
            ...
            template < typename I > void TrashWatcher < I >::register_watcher() {
                ...
                Context *ctx = create_context_callback <
                    TrashWatcher,
                    &TrashWatcher < I >::handle_register_watcher > (this);
            ...
            template < typename I >
                void TrashWatcher <
                I >::unregister_watcher(Context * on_finish) {
                dout(5) << dendl;

                m_async_op_tracker.start_op();
                Context *ctx = new LambdaContext([this, on_finish] (int r){
                                                 handle_unregister_watcher(r,
                                                                           on_finish);});
                this->unregister_watch(ctx);
            }

            template < typename I >
                void TrashWatcher < I >::handle_unregister_watcher(int r,
                                                                   Context *
                                                                   on_finish) {
            ...
            template < typename I >
                ...
                librbd::cls_client::trash_list_start(&op, m_last_image_id,
                                                     MAX_RETURN);

                librados::AioCompletion * aio_comp = create_rados_callback <
                    TrashWatcher < I >,
                    &TrashWatcher < I >::handle_trash_list > (this);
                m_out_bl.clear();
                int r =
                    m_io_ctx.aio_operate(RBD_TRASH, aio_comp, &op, &m_out_bl);
                ceph_assert(r == 0);
                aio_comp->release();
            }
            ...
            template < typename I >
                ...
                Context *on_init_finish = nullptr;
                {
                    ...
                    if (r == -EBLOCKLISTED) {
                        dout(0) <<
                            "detected client is blocklisted during trash refresh"
                            << dendl;
                        m_trash_list_in_progress = false;
                        std::swap(on_init_finish, m_on_init_finish);
                    }
                ...
                if (r >= 0 && images.size() == MAX_RETURN) {
                    ...
                    derr << "failed to retrieve trash directory: " <<
                        cpp_strerror(r) << dendl;
                    schedule_trash_list(10);
                }
            ...
            template < typename I >
                ...
                std::scoped_lock locker {
                m_threads->timer_lock, m_lock};
                if (m_shutting_down || m_trash_list_in_progress
                    || m_timer_ctx != nullptr) {
            ...
            template < typename I >
                ...
                auto ctx =
                    new
                    LambdaContext([this, image_id, deferment_end_time] (int r){
