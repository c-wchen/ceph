 ...
 */
    struct extent_to_write_t {
        ...
        bool is_data() const {
            ...
            return addr + len;
        } static extent_to_write_t create_data(laddr_t addr,
                                               bufferlist to_write) {
            return extent_to_write_t(addr, to_write);
        } static extent_to_write_t create_zero(laddr_t addr, extent_len_t len) {
            return extent_to_write_t(addr, len);
        } static extent_to_write_t create_existing(laddr_t addr,
      ...
      :    type(type_t::ZERO), addr(addr), len(len) {
        ...
        extent_to_write_t(laddr_t addr, paddr_t existing_paddr,
                          extent_len_t len)
 ...
 * combining them.
 */
    void append_extent_to_write(extent_to_write_list_t & to_write,
                                extent_to_write_t && to_append) {
 ...
 * combining them.
 */
    void splice_extent_to_write(extent_to_write_list_t & to_write,
                                extent_to_write_list_t && to_splice) {
        if (!to_splice.empty()) {
...
/// Removes extents/mappings in pins
    ObjectDataHandler::write_ret do_removals(context_t ctx,
                                             lba_pin_list_t & pins) {
        return trans_intr::do_for_each(pins,[ctx] (auto & pin) {
                                       LOG_PREFIX(object_data_handler.
                                                  cc::do_removals);
                                       DEBUGT("decreasing ref: {}", ctx.t,
                                              pin->get_key());
                                       return ctx.tm.dec_ref(ctx.t,
                                                             pin->get_key()
                                       ).si_then([](auto) {
                                                 }
                                                 ,
...
/// Creates zero/data extents in to_write
    ObjectDataHandler::write_ret do_insertions(context_t ctx,
                                               extent_to_write_list_t &
                                               to_write) {
        return trans_intr::do_for_each(to_write,[ctx] (auto & region) {
                                       LOG_PREFIX(object_data_handler.
                                                  cc::do_insertions);
                                       if (region.is_data()) {
                                       assert_aligned(region.addr);
                                       assert_aligned(region.len);
                                       ceph_assert(region.len ==
                                                   region.to_write->length());
                                       DEBUGT("allocating extent: {}~{}", ctx.t,
                                              region.addr, region.len);
                                       return ctx.tm.alloc_extent <
                                       ObjectDataBlock > (ctx.t, region.addr,
                                                          region.len).
                                       si_then([&region] (auto extent) {
                                               ...
                                               ceph_assert(extent->
                                                           ...
                                                           region.len);
                                               auto iter =
                                               region.to_write->cbegin();
                                               iter.copy(region.len,
                                                         extent->get_bptr().
                                                         c_str());
                                               return ObjectDataHandler::
                                               write_iertr::now();}
                                       );}
                                       else
                                       if (region.is_zero()) {
                                       DEBUGT("reserving: {}~{}",
                                              ...
                                              region.addr,
                                              region.len);
                                       return ctx.tm.reserve_region(ctx.t,
                                                                    region.addr,
                                                                    region.len).
                                       si_then([FNAME, ctx,
                                                &region] (auto pin) {
                                               ceph_assert(pin->get_length() ==
                                                           region.len);
                                               if (pin->get_key() !=
                                                   region.addr) {
                                               ERRORT
                                       ...
                                       DEBUGT
                                       ("map existing extent: laddr {} len {} {}",
                                        ctx.t, region.addr, region.len,
                                        *region.existing_paddr);
                                       return ctx.tm.map_existing_extent <
                                       ObjectDataBlock > (ctx.t, region.addr,
                                                          *region.
                                                          existing_paddr,
                                                          region.len).
    ...
    struct overwrite_plan_t {
      ...
      public:
        extent_len_t get_left_size() const {
            ...
            return pin_end - pin_begin;
                                           ...
                                           overwrite_plan) {
            return out << "overwrite_plan_t(" << "pin_begin=" << overwrite_plan.
                pin_begin << ", pin_end=" << overwrite_plan.
                pin_end << ", left_paddr=" << overwrite_plan.
                left_paddr << ", right_paddr=" << overwrite_plan.
                right_paddr << ", data_begin=" << overwrite_plan.
                data_begin << ", data_end=" << overwrite_plan.
                data_end << ", aligned_data_begin=" << overwrite_plan.
                aligned_data_begin << ", aligned_data_end=" << overwrite_plan.
                aligned_data_end << ", left_operation=" << overwrite_plan.
                left_operation << ", right_operation=" << overwrite_plan.
                right_operation << ", block_size=" << overwrite_plan.
                block_size << ")";
        } overwrite_plan_t(laddr_t offset, extent_len_t len,
                           const lba_pin_list_t & pins,
                           extent_len_t block_size):pin_begin(pins.front()->
                                                              get_key()),
            pin_end(pins.back()->get_key() + pins.back()->get_length()),
            left_paddr(pins.front()->get_val()),
            right_paddr(pins.back()->get_val()), data_begin(offset),
            data_end(offset + len),
            aligned_data_begin(p2align
                               ((uint64_t) data_begin, (uint64_t) block_size)),
            aligned_data_end(p2roundup
                             ((uint64_t) data_end, (uint64_t) block_size)),
      ...
      private:
         ...
         */ void evaluate_operations() {
            ...
            else if (left_paddr.is_relative() || left_paddr.is_delayed()) {
            ...
            else if (right_paddr.is_relative() || right_paddr.is_delayed()) {
...
template <> struct fmt::formatter <
    crimson::os::seastore::overwrite_plan_t >:fmt::ostream_formatter {
};
#endif
 ...
 */
    using operate_ret_bare = std::pair <
        std::optional < extent_to_write_t >, std::optional < bufferptr >>;
    using operate_ret = get_iertr::future < operate_ret_bare >;
    operate_ret operate_left(context_t ctx, LBAMappingRef & pin,
        ...
        if (overwrite_plan.get_left_size() == 0) {
        ...
        } if (overwrite_plan.left_operation ==
            ...
            return get_iertr::make_ready_future < operate_ret_bare > ((zero_extent_len == 0 ? std::nullopt : std::make_optional(extent_to_write_t::create_zero(overwrite_plan.pin_begin, zero_extent_len))), (zero_prepend_len == 0 ? std::nullopt : std::make_optional(bufferptr(ceph::buffer::create(zero_prepend_len, 0))))
                );
        }
        else if (overwrite_plan.left_operation ==
            ...
            if (prepend_len == 0) {
                ...
                return ctx.tm.read_pin < ObjectDataBlock > (ctx.t,
                                                            pin->duplicate()
                    ).si_then([prepend_len] (auto left_extent) {
                              return get_iertr::make_ready_future <
                              operate_ret_bare > (std::nullopt,
                                                  std::
                                                  make_optional(bufferptr
                                                                (left_extent->
                                                                 get_bptr(), 0,
                                                                 prepend_len)));}
                );
            ...
            assert(overwrite_plan.left_operation ==
            ...
            std::optional < extent_to_write_t > left_to_write_extent =
                std::make_optional(extent_to_write_t::
                                   create_existing(overwrite_plan.pin_begin,
            ...
            if (prepend_len == 0) {
                return get_iertr::make_ready_future < operate_ret_bare >
                    (left_to_write_extent, std::nullopt);
            }
            else {
                return ctx.tm.read_pin < ObjectDataBlock > (ctx.t,
                                                            pin->duplicate()
                    ).
                    si_then([prepend_offset =
                             extent_len, prepend_len, left_to_write_extent =
                             std::move(left_to_write_extent)]
                            (auto left_extent) mutable {
                            return get_iertr::make_ready_future <
                            operate_ret_bare > (left_to_write_extent,
                                                std::
                                                make_optional(bufferptr
                                                              (left_extent->
                                                               get_bptr(),
 ...
 */
    operate_ret operate_right(context_t ctx, LBAMappingRef & pin,
                              const overwrite_plan_t & overwrite_plan) {
        ...
        if (overwrite_plan.right_operation ==
            overwrite_operation_t::OVERWRITE_ZERO) {
            assert(pin->get_val().is_zero());
            auto zero_suffix_len = overwrite_plan.get_right_alignment_size();
            auto zero_extent_len = overwrite_plan.get_right_extent_size();
            assert_aligned(zero_extent_len);
            return get_iertr::make_ready_future < operate_ret_bare > ((zero_extent_len == 0 ? std::nullopt : std::make_optional(extent_to_write_t::create_zero(overwrite_plan.aligned_data_end, zero_extent_len))), (zero_suffix_len == 0 ? std::nullopt : std::make_optional(bufferptr(ceph::buffer::create(zero_suffix_len, 0))))
                );
        }
        else if (overwrite_plan.right_operation ==
                 overwrite_operation_t::MERGE_EXISTING) {
            ...
            assert(overwrite_plan.right_operation ==
                   overwrite_operation_t::SPLIT_EXISTING);
            ...
            std::optional < extent_to_write_t > right_to_write_extent =
                std::make_optional(extent_to_write_t::
                                   create_existing(overwrite_plan.
                                                   aligned_data_end,
                                                   overwrite_plan.right_paddr.
            ...
            if (append_len == 0) {
                return get_iertr::make_ready_future < operate_ret_bare >
                    (right_to_write_extent, std::nullopt);
            }
            else {
                auto append_offset = overwrite_plan.data_end - right_pin_begin;
                return ctx.tm.read_pin < ObjectDataBlock > (ctx.t,
                    ...
                    si_then([append_offset, append_len, right_to_write_extent =
                             std::move(right_to_write_extent)]
                            (auto right_extent) mutable {
                            return get_iertr::make_ready_future <
                            operate_ret_bare > (right_to_write_extent,
                                                std::
                                                make_optional(bufferptr
                                                              (right_extent->
                                                               get_bptr(),
    ...
    template < typename F >
        auto with_object_data(ObjectDataHandler::context_t ctx, F && f) {
        return seastar::do_with(ctx.onode.get_layout().object_data.get(),
                                std::forward < F > (f),
                                [ctx] (auto & object_data, auto & f) {
                                return std::invoke(f,
                                                   object_data).si_then([ctx,
                                                                         &object_data]
                                                                        {
                                                                        if
                                                                        (object_data.
    ...
    ObjectDataHandler::write_ret ObjectDataHandler::
        prepare_data_reservation(context_t ctx, object_data_t & object_data,
                                 extent_len_t size) {
        ...
        if (!object_data.is_null()) {
            ...
            DEBUGT("reserving: {}~{}",
                   ctx.t, ctx.onode.get_data_hint(), max_object_size);
            return ctx.tm.reserve_region(ctx.t,
                                         ctx.onode.get_data_hint(),
                                         max_object_size).
                si_then([max_object_size =
                         max_object_size, &object_data] (auto pin) {
                        ceph_assert(pin->get_length() == max_object_size);
                        object_data.update_reserved(pin->get_key(),
                                                    pin->get_length());
                        return write_iertr::now();}
            );
        }
    ...
    ObjectDataHandler::clear_ret ObjectDataHandler::
        trim_data_reservation(context_t ctx, object_data_t & object_data,
                              extent_len_t size) {
        ceph_assert(!object_data.is_null());
        ceph_assert(size <= object_data.get_reserved_data_len());
        return seastar::do_with(lba_pin_list_t(),
                                extent_to_write_list_t(),
                                [ctx, size, &object_data] (auto & pins,
                                                           auto & to_write) {
                                LOG_PREFIX(ObjectDataHandler::
                                           trim_data_reservation);
                                DEBUGT("object_data: {}~{}", ctx.t,
                                       object_data.get_reserved_data_base(),
                                       object_data.get_reserved_data_len());
                                return ctx.tm.get_pins(ctx.t,
                                                       object_data.
                                                       get_reserved_data_base()
                                                       + size,
                                                       object_data.
                                                       get_reserved_data_len() -
                                                       size).si_then([ctx, size,
                                                                      &pins,
                                                                      &object_data,
                                                                      &to_write]
                                                                     (auto
                                                                     ...
                                                                     *pins.
                                                                     ...
                                                                     si_then([ctx, size, pin_offset, &pin, &object_data, &to_write] (auto extent) {
                                                                             bufferlist
                                                                             bl;
                                                                             bl.
                                                                             append
 ...
 */
    extent_to_write_list_t get_to_writes_with_zero_buffer(const extent_len_t
                                                          block_size,
                                                          laddr_t offset,
                                                          extent_len_t len,
                                                          std::optional <
        ...
        auto right = tailptr ?
            (offset + len + tailptr->length()) : (offset + len);

        assert((headptr && ((zero_left - left) ==
                            p2roundup(headptr->length(), block_size))) ^
               (!headptr && (zero_left == left)));
        assert((tailptr && ((right - zero_right) ==
        ...
        if (zero_right <= zero_left) {
            ...
            bl.append_zero(right - left - bl.length() -
                           (tailptr ? tailptr->length() : 0));
            if (tailptr) {
                bl.append(*tailptr);
            }
            ...
            return {
            extent_to_write_t::create_data(left, bl)};
        }
        else {
            ...
            ret.push_back(extent_to_write_t::
                          create_zero(zero_left, zero_right - zero_left));
 ...
 */
    ...
    ObjectDataHandler::write_ret ObjectDataHandler::overwrite(context_t ctx,
                                                              laddr_t offset,
                                                              extent_len_t len,
                                                              std::optional <
                                                              bufferlist > &&bl,
        ...
        overwrite_plan_t overwrite_plan(offset, len, _pins,
                                        ctx.tm.get_block_size());
        return seastar::do_with(std::move(_pins), extent_to_write_list_t(),
                                [ctx, len, offset, overwrite_plan, bl =
                                 std::move(bl)]
                                ...
                                DEBUGT("overwrite: split overwrite_plan {}",
                                       ctx.t, overwrite_plan);
                                return operate_left(ctx, pins.front(),
                                                    overwrite_plan).
                                si_then([ctx, len, offset, overwrite_plan, bl =
                                         std::move(bl), &to_write,
                                         &pins] (auto p) mutable {
                                        auto &[left_extent, headptr] = p;
                                        if (left_extent) {
                                        ceph_assert(left_extent->addr ==
                                                    overwrite_plan.pin_begin);
                                        append_extent_to_write(to_write,
                                                               std::
                                                               move
                                                               (*left_extent));}
                                        if (headptr) {
                                        assert(headptr->length() > 0);}
                                        return operate_right(ctx,
                                                             pins.back(),
                                                             overwrite_plan).
                                        si_then([ctx, len, offset, pin_begin =
                                                 overwrite_plan.pin_begin,
                                                 pin_end =
                                                 overwrite_plan.pin_end, bl =
                                                 std::move(bl), headptr =
                                                 std::move(headptr), &to_write,
                                                 &pins] (auto p) mutable {
                                                auto &[right_extent, tailptr] =
                                                p; if (bl.has_value()) {
                                                ...
                                                assert_aligned(write_bl.
                                                               length());}
                                                splice_extent_to_write(to_write,
                                                                       get_to_writes
                                                                       (write_offset,
                                                                        write_bl));}
                                                else {
    ...
    ObjectDataHandler::zero_ret ObjectDataHandler::zero(context_t ctx,
                                                        objaddr_t offset,
                                                        extent_len_t len) {
        return with_object_data(ctx,
                                [this, ctx, offset, len] (auto & object_data) {
                                LOG_PREFIX(ObjectDataHandler::zero);
                                DEBUGT
                                ("zero to {}~{}, object_data: {}~{}, is_null {}",
                                 ctx.t, offset, len,
                                 object_data.get_reserved_data_base(),
                                 object_data.get_reserved_data_len(),
                                 object_data.is_null());
                                return prepare_data_reservation(ctx,
                                                                object_data,
                                                                p2roundup(offset
                                                                          + len,
                                                                          ctx.
                                          ...
                                          return ctx.tm.get_pins(ctx.t,
                                                                 logical_offset,
                                                                 len).
                                          si_then([this, ctx, logical_offset,
                                                   len] (auto pins) {
                                                  return overwrite(ctx,
                                                                   logical_offset,
                                                                   len,
                                                                   std::nullopt,
                                                                   std::
    ...
    ObjectDataHandler::write_ret ObjectDataHandler::write(context_t ctx,
                                                          objaddr_t offset,
                                                          const bufferlist & bl)
    {
        return with_object_data(ctx,
                                ...
                                DEBUGT
                                ("writing to {}~{}, object_data: {}~{}, is_null {}",
                                 ctx.t, offset, bl.length(),
                                 object_data.get_reserved_data_base(),
                                 object_data.get_reserved_data_len(),
                                 object_data.is_null());
                                return prepare_data_reservation(ctx,
    ...
    ObjectDataHandler::read_ret ObjectDataHandler::read(context_t ctx,
        ...
        return seastar::do_with(bufferlist(),[ctx, obj_offset, len] (auto & ret) {
                                return with_object_data(ctx,
                                                        ...
                                                        si_then([ctx, loffset,
                                                                ...
                                                                (_pins.size() >=
                                                                 1);
                                                                ceph_assert((*_pins.begin())->get_key() <= loffset); return seastar::do_with(std::move(_pins), loffset,[ctx, loffset, len, &ret] (auto & pins, auto & current) {
                                                                                                                                             return
                                                                                                                                             trans_intr::
                                                                                                                                             do_for_each
                                                                                                                                             (pins,
                                                                                                                                              ...
                                                                                                                                              (current
                                                                                                                                               ...
                                                                                                                                               (loffset
                                                                                                                                                ...
                                                                                                                                                len));
                                                                                                                                              ceph_assert
                                                                                                                                              ((loffset + len) > pin->get_key()); laddr_t end = std::min(pin->get_key() + pin->get_length(), loffset + len); if (pin->get_val().is_zero()) {
                                                                                                                                              ...
                                                                                                                                              si_then
                                                                                                                                              ([&ret, &current, end] (auto extent) {
                                                                                                                                               ceph_assert
                                                                                                                                               ((extent->get_laddr() + extent->get_length()) >= end); ceph_assert(end > current); ret.append(bufferptr(extent->get_bptr(), current - extent->get_laddr(), end - current)); current = end; return seastar::now();}
                                                                                                                                              ).
                                                                                                                                              handle_error_interruptible
                                                                                                                                              (read_iertr::
                                                                                                                                               pass_further
    ...
    ObjectDataHandler::fiemap_ret ObjectDataHandler::fiemap(context_t ctx,
                                                            objaddr_t
                                                            obj_offset,
                                                            extent_len_t len) {
        return seastar::do_with(std::map < uint64_t, uint64_t > (),
                                ...
                                return with_object_data(ctx,
                                                        ...
                                                        si_then([loffset, len,
                                                                 ...
                                                                 &ret] (auto
                                                                        && pins)
                                                                {
                  ceph_assert(pins.size() >= 1); ceph_assert((*pins.begin())->get_key() <= loffset); for (auto && i:pins)
                                                                {
                                                                if (!
                                                                    (i->
                                                                     get_val().
    ...
    ObjectDataHandler::truncate_ret ObjectDataHandler::truncate(context_t ctx,
                                                                objaddr_t
                                                                offset) {
        return with_object_data(ctx,[this, ctx, offset] (auto & object_data) {
                                ...
                                if (offset <
                                    object_data.get_reserved_data_len()) {
                                return trim_data_reservation(ctx, object_data,
                                                             offset);}
                                else
                                if (offset >
                                    object_data.get_reserved_data_len()) {
