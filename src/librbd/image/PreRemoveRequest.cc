...
void PreRemoveRequest < I >::acquire_exclusive_lock()
{
    ...
    // the image
    m_image_ctx->
    set_exclusive_lock_policy(new exclusive_lock::StandardPolicy <
                              I > (m_image_ctx));
    ...
    auto ctx = create_context_callback <
               PreRemoveRequest < I >,
               &PreRemoveRequest < I >::handle_exclusive_lock > (this,
                   m_exclusive_lock);
...
void PreRemoveRequest < I >::shut_down_exclusive_lock()
{
    ...
    auto ctx = create_context_callback <
               PreRemoveRequest < I >,
               &PreRemoveRequest < I >::handle_shut_down_exclusive_lock >
               (this);
...
template < typename I >
void PreRemoveRequest <
I >::handle_shut_down_exclusive_lock(int r)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 5) << "r=" << r << dendl;
...
void PreRemoveRequest < I >::validate_image_removal()
{
    ...
    if (!m_image_ctx->ignore_migrating &&
        ...
        lderr(cct) << "image in migration state - not removing" <<
                   dendl;
        finish(-EBUSY);
        return;
    }
...
void PreRemoveRequest < I >::list_image_watchers()
{
    ...
    auto req =
        ListWatchersRequest < I >::create(*m_image_ctx, flags,
                                          &m_watchers,
                                          ctx);
    req->send();
}
...
void PreRemoveRequest < I >::handle_list_image_watchers(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "error listing image watchers: " <<
...
template < typename I > void PreRemoveRequest < I >::check_group()
{
    ...
    auto rados_completion = create_rados_callback <
                            PreRemoveRequest < I >,
                            &PreRemoveRequest < I >::handle_check_group > (this);
    m_out_bl.clear();
...
void PreRemoveRequest < I >::handle_check_group(int r)
{
    ...
    if (r < 0 && r != -EOPNOTSUPP) {
        lderr(cct) << "error fetching group for image: " <<
                   cpp_strerror(r)
                   << dendl;
        finish(r);
        return;
...
template < typename I > void PreRemoveRequest < I >::remove_snapshot()
{
    ...
    auto ctx = create_context_callback <
               PreRemoveRequest < I >,
               &PreRemoveRequest < I >::handle_remove_snapshot > (this);
    auto req =
        librbd::operation::SnapshotRemoveRequest <
        I >::create(*m_image_ctx, snap_info.snap_namespace,
                    snap_info.name,
                    snap_id, ctx);
    req->send();
