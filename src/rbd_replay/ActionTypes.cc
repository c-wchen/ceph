...
bool byte_swap_required(__u8 version)
{
    ...
    return false;
#endif
} void decode_big_endian_string(std::string &str,
...
public:
    ...
    {
        using ceph::encode;
        encode(static_cast < uint8_t >(Action::ACTION_TYPE),
               m_bl);
        action.encode(m_bl);
    } private:
    bufferlist &m_bl;
...
{
public:
    explicit DumpVisitor(Formatter *
    ...
    inline void operator()(const Action &action)const
    {
        ...
        m_formatter->dump_string("action_type",
...
} void ImageActionBase::decode(__u8 version,
                               bufferlist::const_iterator &it)
{
    using ceph::decode;
...
} void IoActionBase::encode(bufferlist &bl) const
{
    ...
    encode(offset, bl);
    encode(length, bl);
} void IoActionBase::decode(__u8 version,
                            bufferlist::const_iterator &it)
{
    using ceph::decode;
...
} void OpenImageAction::encode(bufferlist &bl) const
{
    ...
    encode(snap_name, bl);
    encode(read_only, bl);
} void OpenImageAction::decode(__u8 version,
                               bufferlist::const_iterator &it)
{
    using ceph::decode;
    ImageActionBase::decode(version, it);
...
} void AioOpenImageAction::encode(bufferlist &bl) const
{
    ...
    encode(snap_name, bl);
    encode(read_only, bl);
} void AioOpenImageAction::decode(__u8 version,
                                  bufferlist::const_iterator &it)
{
    using ceph::decode;
    ImageActionBase::decode(version, it);
...
{
    ceph_abort();
} void UnknownAction::decode(__u8 version,
                             bufferlist::const_iterator &it)
{
}
...
void ActionEntry::decode_versioned(__u8 version,
                                   bufferlist::const_iterator &it)
{
    using ceph::decode;
    uint8_t action_type;
...
} void ActionEntry::generate_test_instances(std::list <
        ActionEntry * > &o)
{
