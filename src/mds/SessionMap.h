...
class Session:public RefCountedObject {
  ...
  public:
    ...
    enum {
     ...
     Session() = delete;
     Session(ConnectionRef con):item_session_list(this),
        requests(member_offset(MDRequestImpl, item_session_request)),
        recall_caps(g_conf().get_val <
                    double >("mds_recall_warning_decay_rate")),
        release_caps(g_conf().get_val <
                     double >("mds_recall_warning_decay_rate")),
        recall_caps_throttle(g_conf().get_val <
                             double >("mds_recall_max_decay_rate")),
        recall_caps_throttle2o(0.5),
        session_cache_liveness(g_conf().get_val <
                               double
                               >("mds_session_cache_liveness_decay_rate")),
        cap_acquisition(g_conf().get_val <
    ...
    const std::string & get_human_name() const {
        return human_name;
    ...
    void delegate_inos(int want, interval_set < inodeno_t > &inos) {
        ...
        for (auto it = free_prealloc_inos.begin();
             it != free_prealloc_inos.end();) {
    ...
    bool trim_completed_requests(ceph_tid_t mintid) {
        ...
        while (!info.completed_requests.empty() &&
               (mintid == 0
                || info.completed_requests.begin()->first < mintid)) {
    ...
    int check_access(CInode * in, unsigned mask, int caller_uid, int caller_gid,
                     const std::vector < uint64_t > *gid_list, int new_uid,
                     int new_gid);
...
class SessionFilter {
  public:
    ...
    } bool match(const Session & session,
...
class SessionMapStore {
  public:
    ...
    virtual void decode_values(std::map < std::string,
                               ceph::buffer::list > &session_vals);
...
class SessionMap:public SessionMapStore {
  public:
    ...
    const Session *get_session(entity_name_t w) const {
        ceph::unordered_map < entity_name_t, Session * >::const_iterator p =
            session_map.find(w);
    ...
    void _load_finish(int operation_r,
                      int header_r,
                      int values_r,
                      bool first,
  ...
  private:
    ...
    void update_average_birth_time(const Session & s, bool added = true) {
        ...
        if (added) {
            avg_birth_time = clock::time_point(((avg_birth_time -
                                                 clock::zero()) / sessions) *
                                               (sessions - 1) + (birth_time -
                                                                 clock::
                                                                 zero()) /
                                               sessions);
        }
