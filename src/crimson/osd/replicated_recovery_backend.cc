...
RecoveryBackend::interruptible_future <> ReplicatedRecoveryBackend::push_delete(const hobject_t &soid,
        eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
...
RecoveryBackend::interruptible_future <> ReplicatedRecoveryBackend::handle_recovery_delete(Ref <
...
RecoveryBackend::interruptible_future <> ReplicatedRecoveryBackend::on_local_recover_persist(const hobject_t &soid,
...
RecoveryBackend::interruptible_future <> ReplicatedRecoveryBackend::read_omap_for_push_op(const hobject_t &oid,
        const
        ObjectRecoveryProgress &
        progress,
        ObjectRecoveryProgress &
...
RecoveryBackend::interruptible_future <> ReplicatedRecoveryBackend::_handle_push(pg_shard_t from,
        PushOp &push_op,
...
std::pair < interval_set < uint64_t >,
    bufferlist > ReplicatedRecoveryBackend::trim_pushed_data(const interval_set < uint64_t >
            &copy_subset,
    ...
    mutable {
        ...
        (!data_zeros.
         empty()) {
            ...
            logger().debug("submit_push_data recovering object {} copy_subset: {} " "intervals_included: {} data_zeros: {}",
                           recovery_info.soid, recovery_info.copy_subset, intervals_included, data_zeros);
...
RecoveryBackend::interruptible_future <> ReplicatedRecoveryBackend::handle_recovery_delete_reply(Ref <
