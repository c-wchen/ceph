...
namespace rbd {
    ...
    namespace action {
        namespace list {
            ...
            int list_process_image(librados::Rados * rados, WorkerEntry * w,
                                   bool lflag, Formatter * f, TextTable & tbl) {
                int r = 0;
                ...
                if (f) {
                    ...
                    if (!lockers.empty())
                        f->dump_string("lock_type",
                                       exclusive ? "exclusive" : "shared");
                    f->close_section();
                }
                else {
                    tbl << w->name << stringify(byte_u_t(info.size))
                        << parent << ((old_format) ? '1' : '2')
                ...
                if (w->img.snap_list(snaplist) >= 0 && !snaplist.empty()) {
                    ...
                    for (std::vector < librbd::snap_info_t >::iterator s =
                         snaplist.begin(); s != snaplist.end(); ++s) {
                        bool is_protected;
                        bool has_parent = false;
                        parent.clear();
                        w->img.snap_set(s->name.c_str());
                        r = w->img.snap_is_protected(s->name.c_str(),
                                                     &is_protected);
                        if (r < 0)
                            return r;
                        if (w->img.
                        ...
                        if (f) {
                            ...
                            if (has_parent) {
                                f->open_object_section("parent");
                                f->dump_string("pool",
                                               parent_image_spec.pool_name);
                                f->dump_string("pool_namespace",
                                               parent_image_spec.
                                               pool_namespace);
                                f->dump_string("image",
                                               parent_image_spec.image_name);
                                f->dump_string("snapshot",
                                               parent_snap_spec.name);
                                f->close_section();
                            }
                            f->dump_int("format", old_format ? 1 : 2);
            ...
            int do_list(const std::string & pool_name,
                        const std::string & namespace_name, bool lflag,
                        Formatter * f) {
                ...
                if (f) {
                    ...
                    tbl.define_column("SIZE", TextTable::LEFT,
                                      TextTable::RIGHT);
                ...
                for (size_t left = 0;
                     left < std::min < size_t > (threads, images.size());
                     left++) {
                ...
                while (true) {
                  ...
                  for (auto comp:workers) {
                        ...
                        case STATE_IDLE:
                            ...
                            comp->id = i->id;
                            comp->completion =
                                new librbd::RBD::AioCompletion(nullptr,
                                                               nullptr);
                            r = rbd.aio_open_read_only(ioctx, comp->img,
                                                       i->name.c_str(), nullptr,
                                                       comp->completion);
                            i++;
                        ...
                        case STATE_OPENED:
                            ...
                            comp->completion->release();
                            if (r < 0) {
                                std::cerr << "rbd: error opening " << comp->
                                    name << ": " << cpp_strerror(r) << std::
                                    endl;
                            ...
                            if (r < 0) {
                                std::
                                    cerr << "rbd: error processing image " <<
                                    comp->
                                    name << ": " << cpp_strerror(r) << std::
                                    endl;
                            }
            ...
            int execute(const po::variables_map & vm,
                ...
                r = do_list(pool_name, namespace_name,
                            vm["long"].as < bool > (), formatter.get());
                if (r < 0) {
                    std::
                        cerr << "rbd: listing images failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
