    ...
    MessengerTest():server_msgr(NULL), client_msgr(NULL) {
        ...
        server_msgr =
            Messenger::create(g_ceph_context, string(GetParam()),
                              entity_name_t::OSD(0), "server", getpid(), 0);
        client_msgr =
            Messenger::create(g_ceph_context, string(GetParam()),
...
class FakeDispatcher:public Dispatcher {
  public:
    ...
    explicit FakeDispatcher(bool s):Dispatcher(g_ceph_context),
        lock("FakeDispatcher::lock"), is_server(s), got_new(false),
        got_remote_reset(false), got_connect(false), loopback(false) {
    }
    bool ms_can_fast_dispatch_any() const override {
        ...
        if (!s) {
            s = new Session(con);
            con->set_priv(s->get());
            lderr(g_ceph_context) << __func__ << " con: " << con << " count: "
                << s->count << dendl;
        }
        s->put();
        got_connect = true;
...
TEST_P(MessengerTest, SimpleTest)
{
    ...
    ASSERT_TRUE((static_cast < Session * >(conn->get_priv()))->get_count() ==
                1);
...
TEST_P(MessengerTest, NameAddrTest)
{
    ...
    ConnectionRef server_conn =
        server_msgr->get_connection(client_msgr->get_myinst());
...
TEST_P(MessengerTest, StatefulTest)
{
    ...
    ASSERT_TRUE(static_cast <
                Session * >(server_conn->get_priv())->get_count() == 1);
    ...
    server_conn = server_msgr->get_connection(client_msgr->get_myinst());
    ASSERT_EQ(1U,
              static_cast < Session * >(server_conn->get_priv())->get_count());
    cli_dispatcher.got_remote_reset = false;
...
class SyntheticDispatcher:public Dispatcher {
  public:
    ...
    SyntheticDispatcher(bool s,
        ...
        lock("SyntheticDispatcher::lock"), is_server(s), got_new(false),
...
class SyntheticWorkload {
  ...
  public:
    ...
    SyntheticWorkload(int servers, int clients, string type, int random_num,
                      ...
                      Messenger::
                      Policy cli_policy):lock("SyntheticWorkload::lock"),
        dispatcher(false, this), rng(time(NULL)) {
        Messenger *msgr;
        int base_port = 16800;
        entity_addr_t bind_addr;
        char addr[64];
    ...
    void generate_connection() {
        ...
        pair < Messenger *, Messenger * >p;
        {
            ...
            if (server->get_default_policy().server) {
                ...
                ConnectionRef conn =
                    client->get_connection(server->get_myinst());
    ...
    void drop_connection() {
        ...
        if (!p.first->get_default_policy().server
            ...
            ConnectionRef peer =
                p.second->get_connection(p.first->get_myinst());
            peer->mark_down();
...
class MarkdownDispatcher:public Dispatcher {
  ...
  public:
    ...
    explicit MarkdownDispatcher(bool s):Dispatcher(g_ceph_context),
        lock("MarkdownDispatcher::lock"), last_mark(false) {
    }
    bool ms_can_fast_dispatch_any() const override {
        return false;
    } bool ms_can_fast_dispatch(const Message * m)const override {
...
TEST_P(MessengerTest, MarkdownTest)
{
    ...
    while (i--) {
        ConnectionRef conn1 =
            client_msgr->get_connection(server_msgr->get_myinst());
        ConnectionRef conn2 =
            client_msgr->get_connection(server_msgr2->get_myinst());
        MPing *m = new MPing();
...
INSTANTIATE_TEST_CASE_P(Messenger,
                        MessengerTest,::testing::Values("async+posix", "simple")
    );
