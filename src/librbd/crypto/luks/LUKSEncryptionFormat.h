...
class EncryptionFormat: public crypto::EncryptionFormat <
    ImageCtxT >
{
public:
...
public:
    ...
    ImageCtxT >> clone() const override
    {
        return std::make_unique < LUKSEncryptionFormat >
               (m_passphrase);
...
{
public:
    LUKS1EncryptionFormat(encryption_algorithm_t alg,
    ...
    ImageCtxT >> clone()const override
    {
        return std::make_unique < LUKS1EncryptionFormat > (m_alg,
                m_passphrase);
    } void format(ImageCtxT *ictx, Context *on_finish) override;
    void load(ImageCtxT *ictx, std::string *detected_format_name,
              Context *on_finish) override;
...
{
public:
    LUKS2EncryptionFormat(encryption_algorithm_t alg,
                          std::string_view passphrase)
        :    m_alg(alg), m_passphrase(passphrase)
    {
    }
    ...
    ImageCtxT >> clone()const override
    {
        return std::make_unique < LUKS2EncryptionFormat > (m_alg,
                m_passphrase);
    } void format(ImageCtxT *ictx, Context *on_finish) override;
    void load(ImageCtxT *ictx, std::string *detected_format_name,
              Context *on_finish) override;
