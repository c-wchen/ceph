...
using NodeExtentRef =
    crimson::os::seastore::TCachedExtentRef < NodeExtent >;
...
// we support up to 64 KiB tree nodes using node_offset_t = uint16_t;
...
{
    return (node_size > 0 &&
            node_size <= MAX_NODE_SIZE && node_size % DISK_BLOCK_SIZE == 0);
}
...
struct tree_stats_t {
    ...
    {
        return (double)(size_filled() - size_value()) / (size_logical() -
                size_value());
...
inline std::ostream &operator<<(std::ostream &os,
                                const tree_stats_t &stats)
{
    os << "Tree stats:" << "\n  height = " << stats.
       height << "\n  num values = " << stats.
       num_kvs_leaf << "\n  num nodes  = " << stats.num_nodes()
       << " (leaf=" << stats.num_nodes_leaf << ", internal=" << stats.
       num_nodes_internal << ")" << "\n  size persistent = " << stats.
       size_persistent() << "B" << "\n  size filled     = " << stats.
       size_filled() << "B" << " (value=" << stats.
       size_value_leaf << "B" << ", rest=" << stats.size_filled() -
       stats.size_value_leaf << "B)" << "\n  size logical    = " << stats.
       size_logical() << "B" << "\n  size overhead   = " << stats.
       size_overhead() << "B" << "\n  ratio fullness  = " << stats.
       ratio_fullness()
               ...
               const char *new_base, extent_len_t node_size)
{
    ...
    ptr = reinterpret_cast < PtrType >((const char *)ptr - origin_base +
                                       new_base);
}
...
struct fmt::formatter <
    crimson::os::seastore::onode::tree_stats_t > : fmt::ostream_formatter {
};
#endif
