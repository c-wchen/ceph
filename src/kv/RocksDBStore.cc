...
std::shared_ptr < rocksdb::Cache > RocksDBStore::create_block_cache(const std::string &cache_type,
...
rocksdb::ColumnFamilyHandle *RocksDBStore::get_key_cf(const prefix_shards &shards, const char *key,
        const size_t keylen)
{
...
rocksdb::ColumnFamilyHandle *RocksDBStore::get_cf_handle(const std::string &prefix,
...
rocksdb::ColumnFamilyHandle *RocksDBStore::get_cf_handle(const std::string &prefix, const char *key,
        size_t keylen)
{
    auto iter = cf_handles.find(prefix);
    if (iter == cf_handles.end()) {
...
int RocksDBStore::_test_init(const string &dir)
{
    ...
    rocksdb::DB *db;
    rocksdb::Status status =
        rocksdb::DB::Open(options, dir, &db);
    delete db;
    db = nullptr;
