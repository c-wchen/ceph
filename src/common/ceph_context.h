 ...
 */ class CephContext {
   ...
   */
    ...
    template < typename T >
        void lookup_or_create_singleton_object(T * &p,
        ...
        if (!_associated_objs.count(name)) {
            ...
            TypedSingletonWrapper < T > *wrapper =
                dynamic_cast < TypedSingletonWrapper < T >
                *>(_associated_objs[name]);
            assert(wrapper != NULL);
            p = wrapper->singleton;
        }
  ...
  private:
    ...
    template < typename T > struct TypedSingletonWrapper:public SingletonWrapper {
