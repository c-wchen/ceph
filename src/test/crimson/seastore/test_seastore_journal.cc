...
struct record_validator_t {
    ...
    template < typename ... T >
        record_validator_t(T
                           && ... record):record(std::forward < T >
                                                 (record) ...) {
      ...
      for (auto && block:record.extents) {
            ...
            ASSERT_EQ(bl.begin().crc32c(bl.length(), 1),
                      block.bl.begin().crc32c(block.bl.length(), 1));
        }
    }

    auto get_replay_handler() {
        auto checker =[this, iter = record.deltas.begin()](paddr_t base,
                                                           const delta_info_t &
                                                           di)mutable {
...
struct journal_test_t:seastar_test_suite_t, SegmentProvider, JournalTrimmer {
    ...
    seastar::future <> tear_down_fut()final {
        return journal->close().safe_then([this] {
                                          segment_manager.reset();
                                          sms.reset(); journal.reset();}
    ...
    template < typename T > auto replay(T && f) {
        return journal->close().safe_then([this, f = std::move(f)] ()mutable {
                                          journal =
                                          journal::make_segmented(*this, *this);
                                          journal->
                                          set_write_pipeline(&pipeline);
                                          return journal->replay(std::forward <
                                                                 T >
                                                                 (std::
                                                                  move(f)));}
        ).safe_then([this] {
    ...
    auto replay_and_check() {
        ...
        replay([&advance, &delta_checker]
               (const auto & offsets,
                const auto & di,
                const journal_seq_t &, const journal_seq_t &, auto t)mutable {
               if (!delta_checker) {
    ...
    template < typename ... T > auto submit_record(T && ... _record) {
        auto record {
        std::forward < T > (_record) ...};
        records.push_back(record);
        OrderingHandle handle = get_dummy_ordering_handle();
        auto[addr, _] = journal->submit_record(std::move(record),
                                               handle).unsafe_get0();
    ...
    delta_info_t generate_delta(size_t bytes) {
        ...
        , L_ADDR_NULL,
                0, 0, block_size, 1, MAX_SEG_SEQ, segment_type_t::NULL_SEG, bl};
    }
};
...
TEST_F(journal_test_t, replay_twice)
{
    run_async([this] {
              ...
              ); replay_and_check(); submit_record(record_t {
                                                   {
                                                   generate_extent(2),
                                                   generate_extent(5)}
                                                   , {
...
TEST_F(journal_test_t, roll_journal_and_replay)
{
    run_async([this] {
              ...
              unsigned so_far = 0;
              while (current.as_seg_paddr().get_segment_id() ==
                     starting_segment) {
