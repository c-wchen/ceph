...
namespace ECUtil {
    ...
    class stripe_info_t {
      ...
      public:
        ...
        :stripe_width(stripe_width), chunk_size(stripe_width / stripe_size) {
            ...
            return ((offset % stripe_width) ?
                    (offset - (offset % stripe_width) + stripe_width) : offset);
            ...
            uint64_t > aligned_offset_len_to_chunk(std::pair < uint64_t,
                                                   uint64_t > in) const {
            return std::
                make_pair(aligned_logical_offset_to_chunk_offset(in.first),
                          aligned_logical_offset_to_chunk_offset(in.second));
        } std::pair < uint64_t,
            uint64_t > offset_len_to_stripe_bounds(std::pair < uint64_t,
                                                   uint64_t > in) const {
            uint64_t off = logical_to_prev_stripe_offset(in.first);
            uint64_t len = logical_to_next_stripe_offset((in.first - off) +
                                                         in.second);
             return std::make_pair(off, len);
    }};

    int decode(const stripe_info_t & sinfo,
               ErasureCodeInterfaceRef & ec_impl,
               std::map < int, bufferlist > &to_decode, bufferlist * out);
    ...
    int encode(const stripe_info_t & sinfo,
               ErasureCodeInterfaceRef & ec_impl,
               bufferlist & in,
               const std::set < int >&want, std::map < int, bufferlist > *out);
    ...
    class HashInfo {
      ...
      public:
         HashInfo() {
        } explicit HashInfo(unsigned
                            num_chunks):cumulative_shard_hashes(num_chunks,
                                                                -1) {
        }
        ...
        uint32_t get_chunk_hash(int shard) const {
            ...
            return get_projected_total_chunk_size() *
                (sinfo.get_stripe_width() / sinfo.get_chunk_size());
        } void set_projected_total_logical_size(const stripe_info_t & sinfo,
