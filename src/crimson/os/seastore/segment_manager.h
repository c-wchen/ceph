...
struct block_sm_superblock_t {
    ...
    } void validate() const
    {
        ...
        for (unsigned int i = 0; i < seastar::smp::count; i++) {
            ...
            ceph_assert_always(shard_infos[i].segments <=
...
class Segment: public boost::intrusive_ref_counter <
    Segment, boost::thread_unsafe_counter >
{
public:
    ...
    */
    using write_ertr = crimson::errorator < crimson::ct_error::input_output_error,  // media error or corruption
          ...
          >;
    virtual write_ertr::future <> write(segment_off_t offset,
...
template <> struct fmt::formatter <
    crimson::os::seastore::block_shard_info_t > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::block_sm_superblock_t > : fmt::ostream_formatter {
};
#endif
