      ...
      mgrclient(cct.get(), nullptr, &monclient.monmap)
{
    ...
    messenger.
    reset(Messenger::
          create_client_messenger(cct.get(), "radosclient"));
    if (!messenger) {
        throw std::bad_alloc();
    }
    ...
    objecter =
        std::make_unique < Objecter > (cct.get(), messenger.get(),
                                       &monclient, ioctx);
    ...
    messenger->
    set_myname(entity_name_t::CLIENT(monclient.get_global_id()));
