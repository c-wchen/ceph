...
rgw_pubsub_s3_notification::rgw_pubsub_s3_notification(const rgw_pubsub_topic_filter& topic_filter) :
    id(topic_filter.s3_id), events(topic_filter.events), topic_arn(topic_filter.topic.arn),
...
void rgw_pubsub_dest::dump(Formatter *f) const
{
    ...
    encode_json("retry_sleep_duration", retry_sleep_duration != DEFAULT_GLOBAL_VALUE ? std::to_string(
                    retry_sleep_duration) : DEFAULT_CONFIG, f);
}
...
void rgw_pubsub_dest::dump_xml(Formatter *f) const
{
    ...
    encode_xml("RetrySleepDuration", retry_sleep_duration != DEFAULT_GLOBAL_VALUE ? std::to_string(
                   retry_sleep_duration) : DEFAULT_CONFIG, f);
}
...
std::string rgw_pubsub_dest::to_json_str() const
{
    ...
    encode_json("RetrySleepDuration", retry_sleep_duration != DEFAULT_GLOBAL_VALUE ? std::to_string(
                    retry_sleep_duration) : DEFAULT_CONFIG, &f);
    f.close_section();
    std::stringstream ss;
    f.flush(ss);
                                    ...
                                    optional_yield y) const
{
    if (ps.use_notification_v2) {
        if (const auto ret = ps.driver->stat_topics_v1(bucket->get_tenant(), y, dpp); ret != -ENOENT) {
            ldpp_dout(dpp, 1) << "WARNING: " << (ret == 0 ? "topic migration in process" :
                                                 "cannot determine topic migration status. ret = " + std::to_string(ret))
...
int RGWPubSub::Bucket::create_notification(const DoutPrefixProvider *dpp, const std::string& topic_name,
        const rgw::notify::EventTypeList& events, OptionalFilter s3_filter, const std::string& notif_name,
        optional_yield y) const
{
...
int RGWPubSub::Bucket::remove_notifications(const DoutPrefixProvider *dpp, optional_yield y) const
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 1) << "ERROR: failed to get list of topics from bucket '" << bucket->get_name() << "', ret=" << ret <<
                          dendl;
        return ret ;
    }
