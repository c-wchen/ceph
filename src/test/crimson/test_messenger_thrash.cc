...
namespace
{
    ...
    class SyntheticDispatcher final:public crimson::net::Dispatcher {
      public:
        std::map < crimson::net::Connection *,
            std::deque < payload_seq_t > >conn_sent;
        ...
        SyntheticDispatcher(bool s, SyntheticWorkload * wl):
            ...
            seastar::future <>> ms_dispatch(crimson::net::ConnectionRef con,
                                            MessageRef m) {
            if (verbose) {
        ...
        std::optional < seastar::future <>> reply_message(const MessageRef m,
                                                          crimson::net::
                                                          ConnectionRef con,
                                                          Payload & pl) {
            pl.who = Payload::PONG;
        ...
        void clear_pending(crimson::net::ConnectionRef con) {
            for (std::deque < uint64_t >::iterator it =
                 conn_sent[&*con].begin(); it != conn_sent[&*con].end(); ++it)
                sent.erase(*it);
            conn_sent.erase(&*con);
        }
    ...
    class SyntheticWorkload {
        ...
        seastar::future < crimson::net::ConnectionRef > get_random_connection() {
            return seastar::do_until([this] {
                                     return dispatcher.get_num_pending_msgs() <=
                                     max_in_flight;
                                     }
                                     ,[] {
                   ...
                   boost::uniform_int <> choose(0,
                                                available_connections.size() -
                                                1); int index = choose(rng);
                   std::map < crimson::net::ConnectionRef,
                   std::pair < crimson::net::MessengerRef,
                   crimson::net::MessengerRef >>::iterator i =
                   available_connections.begin(); for (; index > 0;
                                                       --index, ++i);
                   return seastar::make_ready_future <
                   crimson::net::ConnectionRef > (i->first);}
            ) ;
        }
      ...
      public:
        ...
        :server_policy(srv_policy),
            ...
            dispatcher(false, this), servers(servers), clients(clients) {
        ...
        seastar::future <> maybe_generate_connection() {
                ...
                connected_pair;
            {
                crimson::net::ConnectionRef conn =
                    client->connect(server->get_myaddr(),
        ...
        seastar::future <> send_message() {
            return get_random_connection()
                .then([this] (crimson::net::ConnectionRef conn) {
                      ...
                      int val = true_false(rng); if (val >= 95) {
                      uuid_d uuid;
                      ...
                      m->cmd = cmds;
                      m->set_priority(200); return conn->send(std::move(m));}
                      else {
                      boost::uniform_int <> u(0, rand_data.size() - 1);
                      return dispatcher.send_message_wrap(conn,
                                                          rand_data[u(rng)]);}
                      }
            ) ;
        }
        ...
        seastar::future <> drop_connection() {
            ...
            return get_random_connection()
                .then([this] (crimson::net::ConnectionRef conn) {
                      ...
                      p.second->connect(p.first->get_myaddr(),
                                        p.first->get_mytype());
                      peer->mark_down(); dispatcher.clear_pending(peer);
        ...
        seastar::future <> wait_for_done() {
            ...
            return seastar::do_until([this] {
                ...
                .then([this] {
                      return seastar::do_for_each(available_servers,
                                                  [](auto server) {
                                                  if (verbose) {
                                                  logger().
                ...
                .then([this] {
                      return seastar::do_for_each(available_clients,
                                                  [](auto client) {
                                                  if (verbose) {
                                                  logger().
    ...
    seastar::future <> reset_conf() {
        return seastar::when_all_succeed(local_conf().
                                         set_val("ms_inject_socket_failures",
                                                 "0"),
                                         local_conf().
                                         set_val("ms_inject_internal_delays",
                                                 "0"),
                                         local_conf().
                                         set_val("ms_inject_delay_probability",
                                                 "0"),
                                         local_conf().
    ...
    seastar::future <> test_stress(thrash_params_t tp) {
        ...
        return seastar::do_with(test_msg,[tp]
                                ...
                                return seastar::do_until([&test_msg] {
                                       ...
                                       return test_msg.generate_connections(tp.
                                                                            connections);}
                                ).then([&test_msg, tp] {
...
seastar::future < int >do_test(seastar::app_template & app)
{
    ...
    return crimson::common::sharded_conf().start(init_params.name, cluster)
        .then([conf_file_list] {
           ...
           return test_stress(thrash_params_t {
                              8, 32, 50, 120}
           )
           .then([] {
                 return test_injection(thrash_params_t {
                                       16, 32, 50, 120}
                 );}
           ).then([] {
