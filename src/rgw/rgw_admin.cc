...
int check_obj_tail_locator_underscore(RGWBucketInfo &bucket_info, rgw_obj_key &key, bool fix, Formatter *f)
{
    ...
    int ret = static_cast<rgw::sal::RadosStore *>(driver)->getRados()->fix_tail_obj_locator(dpp(), bucket_info, key, fix,
              &needs_fixing, null_yield);
    if (ret < 0) {
...
static void get_data_sync_status(const rgw_zone_id &source_zone, list<string> &status, int tab)
{
    ...
    RGWDataSyncStatusManager sync(static_cast<rgw::sal::RadosStore *>(driver),
                                  static_cast<rgw::sal::RadosStore *>(driver)->svc()->rados->get_async_processor(), source_zone, nullptr);
...
#define MAX_LC_LIST_ENTRIES 100
    ...
    if (opt_cmd == OPT::METADATA_GET) {
        int ret = static_cast<rgw::sal::RadosStore *>(driver)->ctl()->meta.mgr->get(metadata_key, formatter.get(), null_yield,
                  dpp());
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATA_SYNC_RUN) {
        ...
        RGWDataSyncStatusManager sync(static_cast<rgw::sal::RadosStore *>(driver),
                                      static_cast<rgw::sal::RadosStore *>(driver)->svc()->rados->get_async_processor(), source_zone, nullptr, sync_module);
    ...
    if (opt_cmd == OPT::BILOG_LIST) {
        ...
        do {
            ...
            ret = static_cast<rgw::sal::RadosStore *>(driver)->svc()->bilog_rados->log_list(dpp(), bucket->get_info(), log_layout,
                  shard_id, marker, max_entries - count, entries, &truncated);
            if (ret < 0) {
