...
class RocksDBStore:public KeyValueDB {
  ...
  public:
    ...
    int tryInterpret(const string & key, const string & val,
                     rocksdb::Options & opt);
    ...
    void compact_range_async(const string & prefix, const string & start,
                             const string & end) override {
        compact_range_async(combine_strings(prefix, start),
                            combine_strings(prefix, end));
    }

    RocksDBStore(CephContext * c, const string & path, void *p):cct(c),
        logger(NULL),
        path(path),
        priv(p),
        db(NULL),
    ...
    class RocksDBTransactionImpl:public KeyValueDB::TransactionImpl {
      public:
        ...
        void set(const string & prefix,
                 const string & k, const bufferlist & bl) override;
        void set(const string & prefix,
                 const char *k, size_t keylen, const bufferlist & bl) override;
        void rmkey(const string & prefix, const string & k) override;
        void rmkey(const string & prefix,
                   const char *k, size_t keylen) override;
        void rm_single_key(const string & prefix, const string & k) override;
        void rmkeys_by_prefix(const string & prefix) override;
        void rm_range_keys(const string & prefix,
                           const string & start, const string & end) override;
        void merge(const string & prefix,
                   const string & k, const bufferlist & bl) override;
    };
    ...
    int get(const string & prefix,
            const string & key, bufferlist * out) override;
    int get(const string & prefix,
            const char *key, size_t keylen, bufferlist * out) override;
    ...
    class RocksDBWholeSpaceIteratorImpl:public KeyValueDB::
      ...
      public:
        explicit RocksDBWholeSpaceIteratorImpl(rocksdb::Iterator *
                                               iter):dbiter(iter) {
        }
    ...
    static void combine_strings(const string & prefix,
                                const char *key, size_t keylen, string * out) {
    ...
    virtual int64_t request_cache_bytes(PriorityCache::Priority pri,
                                        uint64_t cache_bytes) const override;
