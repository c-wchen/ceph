...
namespace crimson::os::seastore::lba_manager::btree {
    ...
    using get_phy_tree_root_node_ret =
        std::pair < bool,::crimson::interruptible::interruptible_future <
        typename trans_intr::condition, CachedExtentRef >>;

    template < typename T, typename key_t >
        const get_phy_tree_root_node_ret get_phy_tree_root_node(const
                                                                RootBlockRef &
                                                                root_block,
                                                                op_context_t <
                                                                key_t > c);
    ...
    template < typename ROOT_T >
        void link_phy_tree_root_node(RootBlockRef & root_block,
                                     ROOT_T * root_node);
    ...
    template <
        ...
        using self_type = FixedKVBtree <
            node_key_t,
            node_val_t,
            internal_node_t, leaf_node_t, pin_t, node_size, leaf_has_children >;
      public:
        ...
        class iterator {
          public:
            iterator(const iterator & rhs) noexcept:internal(rhs.internal),
                leaf(rhs.leaf) {
            } iterator(iterator
                       && rhs) noexcept:internal(std::move(rhs.internal)),
                leaf(std::move(rhs.leaf)) {
            } iterator & operator=(const iterator &) = default;
            iterator & operator=(iterator &&) = default;

            iterator_fut next(op_context_t < node_key_t > c,
                              mapped_space_visitor_t * visitor = nullptr) const
            {
                ...
                ret.leaf.pos++;
                if (ret.at_boundary()) {
                    return seastar::do_with(ret,
                                            [c, visitor] (auto & ret) mutable {
                                            return ret.handle_boundary(c,
                                                                       visitor).
                                            si_then([&ret] {
                                                    return std::move(ret);}
                                            );}
                    ...
                    return iterator_fut(interruptible::ready_future_marker {
                                        }
                                        , ret);
                }
            ...
            iterator_fut prev(op_context_t < node_key_t > c) const {
                ...
                return seastar::do_with(std::move(ret),
                                        ...
                                        ,
                                        [c, depth_with_space] (auto & ret,
                                                               auto & li,
                                                               auto & ll) {
                                        for (depth_t depth = 2;
                                             depth < depth_with_space;
                                             ++depth) {
                                        ret.get_internal(depth).reset();}
                                        ret.leaf.reset();
                                        ret.get_internal(depth_with_space).
                                        pos--;
                                        // note, cannot result in at_boundary() by construction
                                        return lookup_depth_range(c, ret,
                                                                  depth_with_space
                                                                  - 1, 0, li,
                                                                  ll,
                                                                  nullptr).
            ...
            const auto & get_internal(depth_t depth) const {
                ...
                if constexpr(std::is_same_v <
                             ...
                             lba_map_val_t, node_val_t >) {
                    ret.paddr =
                        ret.paddr.maybe_relative_to(leaf.node->get_paddr());
                } return ret;
            }
            ...
            PhysicalNodeMappingRef < node_key_t, typename pin_t::val_type >
                ...
                auto val = get_val();
                auto key = get_key();
                return std::make_unique < pin_t > (ctx,
                                                   leaf.node,
                                                   leaf.pos,
                                                   val,
          ...
          iterator(depth_t depth) noexcept:internal(depth - 1) {
            ...
            handle_boundary_ret handle_boundary(op_context_t < node_key_t > c,
                                                mapped_space_visitor_t *
                                                visitor) {
                assert(at_boundary());
                depth_t depth_with_space = 2;
                for (; depth_with_space <= get_depth(); ++depth_with_space) {
                ...
                if (depth_with_space <= get_depth()) {
                    return seastar::do_with([](const internal_node_t & internal) {
                                            ...
                                            pos++;
                                            // note, cannot result in at_boundary() by construction
                                            return lookup_depth_range(c, *this,
                                                                      depth_with_space
                                                                      - 1, 0,
                                                                      li, ll,
                                                                      visitor);}
            ...
            depth_t check_split() const {
                if (!leaf.node->at_max_capacity()) {
                    return 0;
                } for (depth_t split_from = 1; split_from < get_depth();
                       ++split_from) {
            ...
            depth_t check_merge() const {
                if (!leaf.node->below_min_capacity()) {
                    return 0;
                } for (depth_t merge_from = 1; merge_from < get_depth();
                       ++merge_from) {
                    if (!get_internal(merge_from + 1).node->
                        below_min_capacity())
                        return merge_from;
                }
                return get_depth();
      ...
      FixedKVBtree(RootBlockRef & root_block):root_block(root_block) {
        ...
        auto & get_root()const {
            ...
            static_assert(std::is_base_of_v < typename internal_node_t::base_t,
                          T >);
        ...
        /// mkfs using mkfs_ret = phy_tree_root_t;
        static mkfs_ret mkfs(RootBlockRef & root_block,
                             op_context_t < node_key_t > c) {
            assert(root_block->is_mutation_pending());
            auto root_leaf =
                c.cache.template alloc_new_extent < leaf_node_t > (c.trans,
                                                                   node_size,
                                                                   placement_hint_t::
                                                                   HOT,
                                                                   INIT_GENERATION);
            ...
            return phy_tree_root_t {
            root_leaf->get_paddr(), 1u};
        }
   ...
   */
        iterator_fut lower_bound(op_context_t < node_key_t > c,
                                 node_key_t addr,
                                 mapped_space_visitor_t * visitor = nullptr,
                                 depth_t min_depth = 1) const {
            LOG_PREFIX(FixedKVBtree::lower_bound);
            return lookup(c,[addr] (const internal_node_t & internal) {
                          ...
                          SUBTRACET(seastore_fixedkv_tree,
                                    ...
                                    ret.get_offset(),
                                    leaf.get_size(),
                                    ret == leaf.end()); return ret;}
                          ,
                          min_depth,
                          visitor).si_then([FNAME, c, min_depth] (auto && ret) {
                                           SUBTRACET(seastore_fixedkv_tree,
                                                     "ret.leaf.pos {}",
                                                     c.trans,
                                                     ret.leaf.pos);
   ...
   */
        iterator_fut upper_bound_right(op_context_t < node_key_t > c,
        ...
        iterator_fut begin(op_context_t < node_key_t > c) const {
            ...
            void check_node(op_context_t < node_key_t > c,
          ...
          for (auto i:*node) {
                ...
                if constexpr
                    (std::is_base_of_v < typename internal_node_t::base_t,
                     child_node_t >) {
                    ret =
                        c.trans.get_extent(i->get_val().
                                           maybe_relative_to(node->get_paddr()),
                                           &child_node);
                    }
                else {
                    if constexpr
                        (leaf_has_children) {
                        ret =
                            c.trans.get_extent(i->get_val().paddr.
                                               maybe_relative_to(node->
                                                                 get_paddr()),
                                               &child_node);
                        }
                }
                if (ret == Transaction::get_extent_ret::PRESENT) {
                    if (child_node->is_mutation_pending()) {
                        auto & prior =
                            (child_node_t &) * child_node->prior_instance;
                        ...
                        auto cnode =
                            child_node->template cast < child_node_t > ();
                        ...
                        if (node->is_pending()) {
                            ...
                            assert(cnode->get_parent_node().get() ==
                                   node.get());
        ...
        check_child_trackers_ret check_child_trackers(op_context_t <
                                                      node_key_t > c) {
            mapped_space_visitor_t checker =[c, this] (paddr_t,
                                                       node_key_t,
                                                       extent_len_t,
                                                       depth_t depth,
                                                       extent_types_t,
   ...
   */
                                                           ...
                                                           assert(n_fixed_kv_extent->get_paddr().is_record_relative()); n_fixed_kv_extent->resolve_relative_addrs(make_record_relative_paddr(0).block_relative_to(n_fixed_kv_extent->get_paddr()));}    // else: backend_type_t::RANDOM_BLOCK
      ...
      private:
                                                           ...
                                                           T >;
                                                           using
                                                           get_internal_node_iertr
                                                           = base_iertr;
                                                           using
                                                           get_internal_node_ret
                                                           =
                                                           get_internal_node_iertr::
                                                           future <
                                                           InternalNodeRef >;
                                                           ...
                                                           now();}; if (found) {
                                                           return fut.
                                                           then_interruptible([this, c, on_found_internal = std::move(on_found_internal), on_found_leaf = std::move(on_found_leaf)] (auto root) {
                                                                              LOG_PREFIX
                                                                              (FixedKVBtree::
                                                                               lookup_root);
                                                                              ceph_assert
                                                           ...
                                                           using
                                                           lookup_internal_level_iertr
                                                           = base_iertr;
                                                           using
                                                           lookup_internal_level_ret
                                                           =
                                                           lookup_internal_level_iertr::
                                                           future <>;
                                                           template <
   ...
   * lf FixedKVBtree.
   */
                                                           using lookup_depth_range_iertr = base_iertr; using lookup_depth_range_ret = lookup_depth_range_iertr::future <>; template < typename LI, typename LL > static lookup_depth_range_ret lookup_depth_range(op_context_t < node_key_t > c,   ///< [in] context
   ...
   */
                                                           ...
                                                           do_with(depth_t {
                                                                   ...
                                                                   return
                                                                   trans_intr::
                                                                   repeat([FNAME, this, c, &iter, &to_merge] {
                                                                          SUBTRACET
                                                                          (seastore_fixedkv_tree,
                                                                           "merging depth {}",
                                                                           c.
  ...
  }; template < typename T > struct is_fixed_kv_tree:     std::
                                                           false_type
                                                           {
  }; template < typename node_key_t, typename node_val_t, typename internal_node_t, typename leaf_node_t, typename pin_t, size_t node_size, bool leaf_has_children > struct is_fixed_kv_tree < FixedKVBtree < node_key_t, node_val_t, internal_node_t, leaf_node_t, pin_t, node_size, leaf_has_children >>:std::
                                                           true_type
                                                           {
                                                           };
                                                           template <
