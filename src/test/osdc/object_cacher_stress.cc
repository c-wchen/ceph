                ...
                uint64_t max_op_len, float percent_reads)
{
    ...
    for (uint64_t i = 0; i < num_ops; ++i) {
        ...
        uint64_t length =
            random() % (std::max < uint64_t > (max_len - 1, 1)) + 1;
...
int correctness_test(uint64_t delay_ns)
{
    ...
    ObjectCacher obc(g_ceph_context, "test", writeback, lock, NULL, NULL, 1 << 21,  // max cache size, 2MB
    ...
    for (int i = 1 << 18; i < 1 << 22; i += 1 << 18) {
        ...
        ceph_assert(0 ==
                    memcmp(ones_maybe.c_str(), ones_bl.c_str(),
                           ones_bl.length()));
    }
    bufferlist ones_maybe;
    ones_maybe.substr_of(readbl, (1 << 18) + (1 << 16), ones_bl.length());
    ceph_assert(0 ==
                memcmp(ones_maybe.c_str(), ones_bl.c_str(), ones_bl.length()));
...
int main(int argc, const char **argv)
{
    ...
    if (stress) {
        srandom(seed);
        return stress_test(num_ops, num_objs, obj_bytes, delay_ns, max_len,
                           percent_reads);
    }
    if (correctness) {
