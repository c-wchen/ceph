 ...
 */
    ...
    COMMAND("auth del "
        "name=entity,type=CephString",
        "delete all caps for <name>", "auth", "rwx", "cli,rest")
    COMMAND("auth rm "
        "name=entity,type=CephString",
        "remove all caps for <name>", "auth", "rwx", "cli,rest")

/*
 * Monitor commands (Monitor.cc)
 */
    COMMAND_WITH_FLAG("compact", "cause compaction of monitor's leveldb storage",
                  "mon", "rw", "cli,rest", FLAG(NOFORWARD) | FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("scrub", "scrub the monitor stores",
                  "mon", "rw", "cli,rest", FLAG(DEPRECATED))
    ...
    COMMAND("log last "
        ...
        "print last few lines of the cluster log", "mon", "r", "cli,rest")
    ...
    COMMAND("tell " "name=target,type=CephName " "name=args,type=CephString,n=N",
 ...
 */
    ...
    COMMAND("mon metadata name=id,type=CephString,req=false",
        "fetch metadata for mon <id>", "mon", "r", "cli,rest")
    COMMAND("mon count-metadata name=property,type=CephString",
        "count mons by metadata field property", "mon", "r", "cli,rest")
    COMMAND("mon versions",
        "check running versions of monitors", "mon", "r", "cli,rest")
    COMMAND("versions",
        "check running versions of ceph daemons", "mon", "r", "cli,rest")
 ...
 */
    ...
    COMMAND("mds metadata name=who,type=CephString,req=false",
        "fetch metadata for mds <who>", "mds", "r", "cli,rest")
    COMMAND("mds count-metadata name=property,type=CephString",
        "count MDSs by metadata field property", "mds", "r", "cli,rest")
    COMMAND("mds versions", "check running versions of MDSs", "mds", "r",
        "cli,rest")
    COMMAND_WITH_FLAG("mds tell " "name=who,type=CephString "
                  "name=args,type=CephString,n=N", "send command to particular mds",
                  "mds", "rw", "cli,rest", FLAG(OBSOLETE))
    COMMAND("mds compat show", "show mds compatibility settings", "mds", "r",
        "cli,rest")
    COMMAND_WITH_FLAG("mds stop name=who,type=CephString", "stop mds", "mds", "rw",
                  "cli,rest", FLAG(DEPRECATED))
    COMMAND("mds deactivate name=who,type=CephString",
        "clean up specified MDS rank (use with `set max_mds` to shrink cluster)",
        "mds", "rw", "cli,rest")
    COMMAND_WITH_FLAG("mds set_max_mds " "name=maxmds,type=CephInt,range=0",
...
// #include "include/ceph_fs.h"
    ...
    COMMAND
    ("mds rmfailed name=who,type=CephString name=confirm,type=CephString,req=false",
 "remove failed mds", "mds", "rw", "cli,rest")
    COMMAND_WITH_FLAG("mds cluster_down", "take MDS cluster down", "mds", "rw",
                  "cli,rest", FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("mds cluster_up", "bring MDS cluster up", "mds", "rw",
                  "cli,rest", FLAG(DEPRECATED))
    COMMAND("mds compat rm_compat " "name=feature,type=CephInt,range=0",
        "remove compatible feature", "mds", "rw", "cli,rest")
    COMMAND("mds compat rm_incompat " "name=feature,type=CephInt,range=0",
        "remove incompatible feature", "mds", "rw", "cli,rest")
    COMMAND_WITH_FLAG("mds add_data_pool " "name=pool,type=CephString",
                  "add data pool <pool>", "mds", "rw", "cli,rest",
                  FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("mds remove_data_pool " "name=pool,type=CephString",
                  "remove data pool <pool>", "mds", "rw", "cli,rest",
                  FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("mds rm_data_pool " "name=pool,type=CephString",
                  "remove data pool <pool>", "mds", "rw", "cli,rest",
                  FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("mds newfs " "name=metadata,type=CephInt,range=0 "
                  "name=data,type=CephInt,range=0 "
                  "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
                  "make new filesystem using pools <metadata> and <data>", "mds",
                  "rw", "cli,rest", FLAG(OBSOLETE))
    COMMAND("fs new " "name=fs_name,type=CephString "
        "name=metadata,type=CephString " "name=data,type=CephString "
        ...
        "cli,rest")
    COMMAND("fs rm " "name=fs_name,type=CephString "
        "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
        "disable the named filesystem", "fs", "rw", "cli,rest")
    COMMAND("fs reset " "name=fs_name,type=CephString "
        "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
        "disaster recovery only: reset to a single-MDS map", "fs", "rw",
        "cli,rest")
    COMMAND("fs ls ", "list filesystems", "fs", "r", "cli,rest")
    COMMAND("fs get name=fs_name,type=CephString",
        "get info about one filesystem", "fs", "r", "cli,rest")
    COMMAND("fs set "
    ...
    COMMAND("fs flag set name=flag_name,type=CephChoices,strings=enable_multiple "
        ...
        "Set a global CephFS flag", "fs", "rw", "cli,rest")
    ...
    COMMAND_WITH_FLAG("fs set_default name=fs_name,type=CephString",
                  ...
                  "fs", "rw", "cli,rest", FLAG(DEPRECATED))
    COMMAND("fs set-default name=fs_name,type=CephString",
        "set the default to the named filesystem", "fs", "rw", "cli,rest")
 ...
 */
    COMMAND("mon dump "
        ...
        "dump formatted monmap (optionally from epoch)", "mon", "r", "cli,rest")
    ...
    COMMAND("mon feature set "
        ...
        "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
        "set provided feature on mon map", "mon", "rw", "cli,rest")
 ...
 */
    ...
    COMMAND("osd metadata "
        ...
        "fetch metadata for osd {id} (default all)", "osd", "r", "cli,rest")
    COMMAND("osd count-metadata name=property,type=CephString",
        "count OSDs by metadata field property", "osd", "r", "cli,rest")
    COMMAND("osd versions",
        "check running versions of OSDs", "osd", "r", "cli,rest")
    COMMAND("osd map "
        ...
        "cli,rest")
    COMMAND("osd lspools " "name=auid,type=CephInt,req=false", "list pools", "osd",
        "r", "cli,rest")
    ...
    COMMAND("osd crush rename-bucket "
        ...
        "rename bucket <srcname> to <dstname>", "osd", "rw", "cli,rest")
    ...
    COMMAND("osd crush set-device-class "
        ...
        "or use <all|any|*> to set all.", "osd", "rw", "cli,rest")
    COMMAND("osd crush rm-device-class "
        "name=ids,type=CephString,n=N",
        "remove class of the osd(s) <id> [<id>...],"
        "or use <all|any|*> to remove all.", "osd", "rw", "cli,rest")
    ...
    COMMAND("osd crush set-tunable "
        ...
        "set crush tunable <tunable> to <value>", "osd", "rw", "cli,rest")
    COMMAND("osd crush get-tunable "
        "name=tunable,type=CephChoices,strings=straw_calc_version",
        "get crush tunable <tunable>", "osd", "r", "cli,rest")
    ...
    COMMAND("osd crush rule rename "
        "name=srcname,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=dstname,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "rename crush rule <srcname> to <dstname>", "osd", "rw", "cli,rest")
    COMMAND("osd crush tree "
        "name=shadow,type=CephChoices,strings=--show-shadow,req=false",
        "dump crush buckets and items in a tree view", "osd", "r", "cli,rest")
    COMMAND
    ("osd crush ls name=node,type=CephString,goodchars=goodchars=[A-Za-z0-9-_.]",
 "list items beneath a node in the CRUSH tree", "osd", "r", "cli,rest")
    ...
    COMMAND("osd crush weight-set create-compat",
        "create a default backward-compatible weight-set", "osd", "rw",
        "cli,rest")
    COMMAND("osd crush weight-set create " "name=pool,type=CephPoolname "
        "name=mode,type=CephChoices,strings=flat|positional",
        "create a weight-set for a given pool", "osd", "rw", "cli,rest")
    COMMAND("osd crush weight-set rm name=pool,type=CephPoolname",
        "remove the weight-set for a given pool", "osd", "rw", "cli,rest")
    COMMAND("osd crush weight-set rm-compat",
        "remove the backward-compatible weight-set", "osd", "rw", "cli,rest")
    COMMAND("osd crush weight-set reweight " "name=pool,type=CephPoolname "
        "name=item,type=CephString " "name=weight,type=CephFloat,range=0.0,n=N",
        "set weight for an item (bucket or osd) in a pool's weight-set", "osd",
        "rw", "cli,rest")
    COMMAND("osd crush weight-set reweight-compat " "name=item,type=CephString "
        "name=weight,type=CephFloat,range=0.0,n=N",
        "set weight for an item (bucket or osd) in the backward-compatible weight-set",
        "osd", "rw", "cli,rest")
    COMMAND("osd setmaxosd " "name=newmax,type=CephInt,range=0",
        "set new maximum osd value", "osd", "rw", "cli,rest")
    COMMAND("osd set-full-ratio " "name=ratio,type=CephFloat,range=0.0|1.0",
        "set usage ratio at which OSDs are marked full", "osd", "rw",
        "cli,rest")
    COMMAND("osd set-backfillfull-ratio " "name=ratio,type=CephFloat,range=0.0|1.0",
        "set usage ratio at which OSDs are marked too full to backfill", "osd",
        "rw", "cli,rest")
    COMMAND("osd set-nearfull-ratio " "name=ratio,type=CephFloat,range=0.0|1.0",
        "set usage ratio at which OSDs are marked near-full", "osd", "rw",
        "cli,rest")
    COMMAND("osd set-require-min-compat-client " "name=version,type=CephString "
        "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
        "set the minimum client version we will maintain compatibility with", "osd",
        "rw", "cli,rest")
    ...
    COMMAND("osd erasure-code-profile get "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "get erasure code profile <name>", "osd", "r", "cli,rest")
    COMMAND("osd erasure-code-profile rm "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "remove erasure code profile <name>", "osd", "rw", "cli,rest")
    COMMAND("osd erasure-code-profile ls",
        "list all erasure code profiles", "osd", "r", "cli,rest")
    COMMAND("osd set "
    ...
    COMMAND("osd require-osd-release "
        ...
        "osd", "rw", "cli,rest")
    COMMAND("osd cluster_snap", "take cluster snapshot (disabled)", "osd", "r", "")
    COMMAND("osd down "
        "type=CephString,name=ids,n=N",
        "set osd(s) <id> [<id>...] down, "
        "or use <any|all|*> to set all osds down", "osd", "rw", "cli,rest")
    COMMAND("osd out "
        "name=ids,type=CephString,n=N",
        "set osd(s) <id> [<id>...] out, "
        "or use <any|all|*> to set all osds out", "osd", "rw", "cli,rest")
    COMMAND("osd in "
        "name=ids,type=CephString,n=N",
        "set osd(s) <id> [<id>...] in, "
    ...
    COMMAND("osd rm "
        "name=ids,type=CephString,n=N",
        "remove osd(s) <id> [<id>...], "
        "or use <any|all|*> to remove all osds", "osd", "rw", "cli,rest")
    COMMAND("osd add-noup "
        "name=ids,type=CephString,n=N",
        "mark osd(s) <id> [<id>...] as noup, "
        "or use <all|any|*> to mark all osds as noup", "osd", "rw", "cli,rest")
    ...
    COMMAND("osd add-noin "
        ...
        "or use <all|any|*> to mark all osds as noin", "osd", "rw", "cli,rest")
    COMMAND("osd add-noout "
        "name=ids,type=CephString,n=N",
        "mark osd(s) <id> [<id>...] as noout, "
        "or use <all|any|*> to mark all osds as noout", "osd", "rw", "cli,rest")
    COMMAND("osd rm-noup "
        "name=ids,type=CephString,n=N",
    ...
    COMMAND("osd reweightn "
        ...
        "reweight osds with {<id>: <weight>,...})", "osd", "rw", "cli,rest")
    COMMAND("osd force-create-pg "
        "name=pgid,type=CephPgid ",
        "force creation of pg <pgid>", "osd", "rw", "cli,rest")
    COMMAND("osd pg-temp "
        "name=pgid,type=CephPgid "
    ...
    COMMAND("osd destroy "
        ...
        "rendering data permanently unreadable.", "osd", "rw", "cli,rest")
    COMMAND("osd purge "
        "name=id,type=CephOsdName "
        "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
        "purge all osd data from the monitors. Combines `osd destroy`, "
        "`osd rm`, and `osd crush rm`.", "osd", "rw", "cli,rest")
    COMMAND("osd lost "
        "name=id,type=CephOsdName "
        "name=sure,type=CephChoices,strings=--yes-i-really-mean-it,req=false",
    ...
    COMMAND_WITH_FLAG("osd create "
                  ...
                  "cli,rest", FLAG(DEPRECATED))
    COMMAND("osd new " "name=uuid,type=CephUUID,req=true "
        "name=id,type=CephOsdName,req=false",
        "Create a new OSD. If supplied, the `id` to be replaced needs to "
        "exist and have been previously destroyed. "
        "Reads secrets from JSON file via `-i <file>` (see man page).", "osd", "rw",
        "cli,rest")
    COMMAND("osd blacklist " "name=blacklistop,type=CephChoices,strings=add|rm "
    ...
    COMMAND("osd pool delete "
        ...
        "delete pool", "osd", "rw", "cli,rest")
    COMMAND("osd pool rm "
        "name=pool,type=CephPoolname "
        "name=pool2,type=CephPoolname,req=false "
        "name=sure,type=CephString,req=false",
        "remove pool", "osd", "rw", "cli,rest")
...
// with units.
    ...
    COMMAND("osd pool get-quota "
        "name=pool,type=CephPoolname ",
        "obtain object or byte limits for pool", "osd", "r", "cli,rest")
    ...
    COMMAND("osd utilization",
        "get basic pg distribution stats", "osd", "r", "cli,rest")
// tiering
    ...
    COMMAND("osd tier cache-mode "
        ...
        "cli,rest")
    COMMAND("osd tier set-overlay " "name=pool,type=CephPoolname "
        "name=overlaypool,type=CephPoolname",
        "set the overlay pool for base pool <pool> to be <overlaypool>", "osd",
        "rw", "cli,rest")
    COMMAND("osd tier remove-overlay " "name=pool,type=CephPoolname ",
        "remove the overlay pool for base pool <pool>", "osd", "rw", "cli,rest")
    COMMAND("osd tier rm-overlay " "name=pool,type=CephPoolname ",
        "remove the overlay pool for base pool <pool>", "osd", "rw", "cli,rest")
 ...
 */
    COMMAND("config-key get "
        "name=key,type=CephString", "get <key>", "config-key", "r", "cli,rest")
    ...
    COMMAND("config-key rm "
        "name=key,type=CephString", "rm <key>", "config-key", "rw", "cli,rest")
    ...
    COMMAND("config-key dump", "dump keys and values", "config-key", "r",
        "cli,rest")
 ...
 */
    COMMAND("mgr dump "
        "name=epoch,type=CephInt,range=0,req=false",
        "dump the latest MgrMap", "mgr", "r", "cli,rest")
    COMMAND("mgr fail name=who,type=CephString",
        "treat the named manager daemon as failed", "mgr", "rw", "cli,rest")
    COMMAND("mgr module ls", "list active mgr modules", "mgr", "r", "cli,rest")
    ...
    COMMAND("mgr versions",
        "check running versions of ceph-mgr daemons", "mgr", "r", "cli,rest")
