...
namespace librbd {
    namespace mirror {
        ...
        template < typename I >
            ...
            I >::handle_get_mirror_info(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ...
            if (*result < 0) {
                if (*result == -ENOENT) {
                    ldout(cct,
                          20) << "mirroring is not enabled for this image" <<
                        dendl;
                    *result = 0;
                }
                else {
                    lderr(cct) << "failed to get mirroring info: " <<
                        cpp_strerror(*result)
                        << dendl;
                }
                return m_on_finish;
        ...
        template < typename I >
            ...
            auto req =
                ImageStateUpdateRequest < I >::create(m_image_ctx->md_ctx,
                                                      m_image_ctx->id,
                                                      cls::rbd::
        ...
        template < typename I >
            Context * DisableRequest <
            I >::handle_image_state_update(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << "r=" << *result << dendl;
        ...
        template < typename I > void DisableRequest < I >::send_promote_image() {
            ...
            auto ctx = util::create_context_callback <
                DisableRequest < I >,
                &DisableRequest < I >::handle_promote_image > (this);
            if (m_mirror_image.mode == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
                ...
                auto req =
                    journal::PromoteRequest < I >::create(m_image_ctx, true,
                                                          ctx);
                req->send();
            }
            ...
            bool removing_snapshots = false;
            {
              ...
              for (auto & it:m_image_ctx->snap_info) {
                    ...
                    if (type == cls::rbd::SNAPSHOT_NAMESPACE_TYPE_MIRROR) {
                        send_remove_snap("", snap_info.snap_namespace,
                                         snap_info.name);
        ...
        template < typename I >
            ...
            ctx =
                new LambdaContext([this, snap_namespace, snap_name, ctx] (int r)
                                  {
        ...
        template < typename I >
            ...
            auto req =
                ImageRemoveRequest < I >::create(m_image_ctx->md_ctx,
        ...
        template < typename I >
            ...
            ldout(cct,
                  20) << "removed image state from rbd_mirroring object" <<
                dendl;
            return m_on_finish;
        }
