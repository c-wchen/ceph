...
lock("RDMADispatcher::lock"), w_lock("RDMADispatcher::for worker pending list"),
stack(s)
{
    PerfCountersBuilder plb(cct, "AsyncMessenger::RDMADispatcher",
                            l_msgr_rdma_dispatcher_first,
                            l_msgr_rdma_dispatcher_last);

    plb.add_u64_counter(l_msgr_rdma_polling, "polling",
                        "Whether dispatcher thread is polling");
    plb.add_u64_counter(l_msgr_rdma_inflight_tx_chunks, "inflight_tx_chunks",
                        "The number of inflight tx chunks");
    plb.add_u64_counter(l_msgr_rdma_inqueue_rx_chunks, "inqueue_rx_chunks",
                        "The number of inqueue rx chunks");

    plb.add_u64_counter(l_msgr_rdma_tx_total_wc, "tx_total_wc",
                        "The number of tx work comletions");
    plb.add_u64_counter(l_msgr_rdma_tx_total_wc_errors, "tx_total_wc_errors",
                        "The number of tx errors");
    plb.add_u64_counter(l_msgr_rdma_tx_wc_retry_errors, "tx_retry_errors",
                        "The number of tx retry errors");
    plb.add_u64_counter(l_msgr_rdma_tx_wc_wr_flush_errors, "tx_wr_flush_errors",
                        "The number of tx work request flush errors");

    plb.add_u64_counter(l_msgr_rdma_rx_total_wc, "rx_total_wc",
                        "The number of total rx work completion");
    plb.add_u64_counter(l_msgr_rdma_rx_total_wc_errors, "rx_total_wc_errors",
                        "The number of total rx error work completion");
    plb.add_u64_counter(l_msgr_rdma_rx_fin, "rx_fin",
                        "The number of rx finish work request");

    plb.add_u64_counter(l_msgr_rdma_total_async_events, "total_async_events",
                        "The number of async events");
    plb.add_u64_counter(l_msgr_rdma_async_last_wqe_events,
                        "async_last_wqe_events",
                        "The number of last wqe events");

    plb.add_u64_counter(l_msgr_rdma_handshake_errors, "handshake_errors",
                        "The number of handshake errors");

    plb.add_u64_counter(l_msgr_rdma_created_queue_pair, "created_queue_pair",
                        "Active queue pair number");
    plb.add_u64_counter(l_msgr_rdma_active_queue_pair, "active_queue_pair",
...
void RDMADispatcher::handle_async_event()
{
    ...
    while (1) {
        ...
        if (ibv_get_async_event
            (global_infiniband->get_device()->ctxt, &async_event)) {
            if (errno != EAGAIN)
                lderr(cct) << __func__ << " ibv_get_async_event failed. (errno="
        ...
        if (async_event.event_type == IBV_EVENT_QP_LAST_WQE_REACHED) {
            ...
            ldout(cct,
                  10) << __func__ << " event associated qp=" << async_event.
                element.qp << " evt: " << ibv_event_type_str(async_event.
                                                             event_type) <<
                dendl;
            Mutex::Locker l(lock);
            ...
            ldout(cct,
                  1) << __func__ << " ibv_get_async_event: dev=" <<
                global_infiniband->get_device()->
                ctxt << " evt: " << ibv_event_type_str(async_event.event_type)
                << dendl;
        }
...
void RDMADispatcher::polling()
{
    ...
    ldout(cct,
          20) << __func__ << " going to poll tx cq: " << tx_cq << " rx cq: " <<
        rx_cq << dendl;
    ...
    while (true) {
        ...
        if (rx_ret > 0) {
            ...
            for (int i = 0; i < rx_ret; ++i) {
                ...
                ldout(cct,
                      25) << __func__ << " got chunk=" << chunk << " bytes:" <<
                    response->byte_len << " opcode:" << response->
                    opcode << dendl;
                ...
                if (response->status == IBV_WC_SUCCESS) {
                    ...
                    if (!conn) {
                        ...
                        ldout(cct,
                              1) << __func__ << " csi with qpn " << response->
                            qp_num << " may be dead. chunk " << chunk <<
                            " will be back ? " << r << dendl;
                        assert(r == 0);
                    }
                    else {
                    ...
                    ldout(cct,
                        ...
                        ") status(" << response->
                        status << ":" << global_infiniband->
                        wc_status_to_string(response->status) << ")" << dendl;
                    assert(global_infiniband->is_rx_buffer(chunk->buffer));
                    r = global_infiniband->post_chunk(chunk);
                    if (r) {
                        ldout(cct,
                              0) << __func__ << " post chunk failed, error: " <<
          ...
          for (auto && i:polled) {
                perf_logger->inc(l_msgr_rdma_inqueue_rx_chunks,
                                 i.second.size());
        ...
        if (!tx_ret && !rx_ret) {
            ...
            if (num_dead_queue_pair) {
                ...
                while (!dead_queue_pairs.empty()) {
                    ldout(cct,
                          10) << __func__ << " finally delete qp=" <<
                        dead_queue_pairs.back() << dendl;
            ...
            if ((ceph_clock_now() - last_inactive).to_nsec() / 1000 >
...
void RDMADispatcher::handle_tx_event(ibv_wc * cqe, int n)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        ldout(cct, 25) << __func__ << " QP: " << response->qp_num
            ...
            << " " << global_infiniband->wc_status_to_string(response->
                                                             status) << dendl;
        ...
        if (response->status != IBV_WC_SUCCESS) {
            ...
            if (response->status == IBV_WC_RETRY_EXC_ERR) {
                ...
                ldout(cct,
                    ...
                    response->
                    qp_num << " should be down while this WR=" << response->
                    wr_id << " still in flight." << dendl;
                ...
                ldout(cct,
                    ...
                    response->wr_id << ") status(" << response->
                    status << "): " << global_infiniband->
                    wc_status_to_string(response->status) << dendl;
            }
            ...
            if (conn && conn->is_connected()) {
                ...
                ldout(cct,
                      1) << __func__ << " missing qp_num=" << response->
                    qp_num << " discard event" << dendl;
            }
        }
        ...
        if (global_infiniband->get_memory_manager()->
            is_tx_buffer(chunk->buffer)) {
            tx_chunks.push_back(chunk);
        }
        else if (reinterpret_cast <
                 QueuePair * >(response->wr_id)->get_local_qp_number() ==
                 response->qp_num) {
            ldout(cct,
                  1) << __func__ << " sending of the disconnect msg completed"
                << dendl;
        }
        else {
            ldout(cct,
                  1) << __func__ << " not tx buffer, chunk " << chunk << dendl;
            ceph_abort();
        }
    }
...
tx_handler(new C_handle_cq_tx(this)), lock("RDMAWorker::lock")
{
    ...
    plb.add_u64_counter(l_msgr_rdma_tx_no_mem, "tx_no_mem",
                        "The count of no tx buffer");
    plb.add_u64_counter(l_msgr_rdma_tx_parital_mem, "tx_parital_mem",
                        "The count of parital tx buffer");
    plb.add_u64_counter(l_msgr_rdma_tx_failed, "tx_failed_post",
                        "The number of tx failed posted");
    plb.add_u64_counter(l_msgr_rdma_rx_no_registered_mem,
                        "rx_no_registered_mem",
                        "The count of no registered buffer when receiving");

    plb.add_u64_counter(l_msgr_rdma_tx_chunks, "tx_chunks",
                        "The number of tx chunks transmitted");
    plb.add_u64_counter(l_msgr_rdma_tx_bytes, "tx_bytes",
                        "The bytes of tx chunks transmitted", NULL, 0,
                        unit_t(BYTES));
    plb.add_u64_counter(l_msgr_rdma_rx_chunks, "rx_chunks",
                        "The number of rx chunks transmitted");
    plb.add_u64_counter(l_msgr_rdma_rx_bytes, "rx_bytes",
                        "The bytes of rx chunks transmitted", NULL, 0,
                        unit_t(BYTES));
    plb.add_u64_counter(l_msgr_rdma_pending_sent_conns, "pending_sent_conns",
...
int RDMAWorker::listen(entity_addr_t & sa, const SocketOptions & opt,
                       ServerSocket * sock)
{
    global_infiniband->init();

    auto p =
        new RDMAServerSocketImpl(cct, global_infiniband.get(),
...
int RDMAWorker::connect(const entity_addr_t & addr, const SocketOptions & opts,
                        ConnectedSocket * socket)
{
    global_infiniband->init();

    RDMAConnectedSocketImpl *p =
        new RDMAConnectedSocketImpl(cct, global_infiniband.get(),
                                    get_stack()->get_dispatcher(), this);
...
int RDMAWorker::get_reged_mem(RDMAConnectedSocketImpl * o,
                              std::vector < Chunk * >&c, size_t bytes)
{
    assert(center.in_thread());
    int r = global_infiniband->get_tx_buffers(c, bytes);
    assert(r >= 0);
    size_t got =
        global_infiniband->get_memory_manager()->get_tx_buffer_size() * r;
    ldout(cct,
          30) << __func__ << " need " << bytes << " bytes, reserve " << got <<
...
{
    ldout(cct,
          20) << __func__ << " pending conns " << pending_sent_conns.
        size() << dendl;
    while (!pending_sent_conns.empty()) {
        ...
        ldout(cct,
              20) << __func__ << " sent pending bl socket=" << o << " r=" << r
            << dendl;
        if (r < 0) {
            if (r == -EAGAIN) {
                pending_sent_conns.push_back(o);
...
RDMAStack::RDMAStack(CephContext * cct, const string & t):NetworkStack(cct, t)
{
    ...
    ldout(cct,
          1) << __func__ << " ms_async_rdma_enable_hugepage value is: " << cct->
        _conf->ms_async_rdma_enable_hugepage << dendl;
    if (cct->_conf->ms_async_rdma_enable_hugepage) {
        rc = setenv("RDMAV_HUGEPAGES_SAFE", "1", 1);
        ldout(cct,
              1) << __func__ << " RDMAV_HUGEPAGES_SAFE is set as: " <<
    ...
    if (!global_infiniband)
        global_infiniband.construct(cct, cct->_conf->ms_async_rdma_device_name,
    ...
    ldout(cct,
          20) << " creating RDMAStack:" << this << " with dispatcher:" <<
        dispatcher << dendl;
}
