...
} template < typename I > void RemoveRequest < I >::set_state()
{
    ...
    auto aio_comp = create_rados_callback <
                    RemoveRequest < I >,
                    &RemoveRequest < I >::handle_set_state > (this);
...
void RemoveRequest < I >::handle_set_state(int r)
{
    ...
    if (r < 0 && r != -EOPNOTSUPP) {
        lderr(m_cct) << "error setting trash image state: " <<
                     cpp_strerror(r)
                     << dendl;
        if (m_ret_val == 0) {
            m_ret_val = r;
...
template < typename I > void RemoveRequest < I >::close_image()
{
    ...
    auto ctx = create_context_callback <
               RemoveRequest < I >,
               &RemoveRequest < I >::handle_close_image > (this);
...
template < typename I > void RemoveRequest < I >::remove_image()
{
    ...
    auto ctx = create_context_callback <
               RemoveRequest < I >,
               &RemoveRequest < I >::handle_remove_image > (this);
    if (m_image_ctx != nullptr) {
        auto req =
            librbd::image::RemoveRequest < I >::create(m_io_ctx,
                m_image_ctx,
                m_force, true,
                m_prog_ctx,
                m_op_work_queue,
...
template < typename I > void RemoveRequest < I >::remove_trash_entry()
{
    ...
    auto aio_comp = create_rados_callback <
                    RemoveRequest < I >,
                    &RemoveRequest < I >::handle_remove_trash_entry > (this);
    int r = m_io_ctx.aio_operate(RBD_TRASH, aio_comp, &op);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
void RemoveRequest < I >::handle_remove_trash_entry(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(m_cct) << "error removing trash entry: " <<
