...
private:
    ...
    CephContext *cct;
public:
    canonical_char_sorter(const DoutPrefixProvider *dpp,
                          CephContext *cct): dpp(dpp), cct(cct)
    {
        UErrorCode status = U_ZERO_ERROR;
        normalizer = icu::Normalizer2::getNFCInstance(status);
        if (U_FAILURE(status)) {
            ldpp_dout(this->dpp,
                      -1) << "ERROR: can't get nfc instance, error = " << status
                          << dendl;
            normalizer = 0;
        }
    }
...
template < typename M >
bool canonical_char_sorter < M >::compare_helper(const M *a, const M *b)
{
    UErrorCode status = U_ZERO_ERROR;
    const std::string as {
        a->name.GetString(), a->name.GetStringLength()}, bs {
        b->name.GetString(), b->name.GetStringLength()};
    icu::UnicodeString aw {
        icu::UnicodeString::fromUTF8(as)}, bw {
...
template < typename M >
bool
canonical_char_sorter < M >::make_string_canonical(rapidjson::Value &v,
        rapidjson::Document::
        AllocatorType &a)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    const icu::UnicodeString aw {
        icu::UnicodeString::fromUTF8(as)};
    icu::UnicodeString an {
        normalizer->normalize(aw, status)};
    if (U_FAILURE(status)) {
...
rapidjson::GenericMember < rapidjson::UTF8 <>,
          rapidjson::MemoryPoolAllocator <> > MyMember;

template < typename H >
bool sort_and_write(rapidjson::Value &d, H &writer,
...
mec_error make_everything_canonical(rapidjson::Value &d,
    ...
    switch (d.GetType()) {
        case rapidjson::kObjectType:
            for (auto &m : d.GetObject()) {
                ...
                if ((r =
                         make_everything_canonical(m.value, a, ccs,
                                                   f)) != mec_error::success) {
                    goto Error;
                }
            }
        ...
        case rapidjson::kArrayType:
            for (auto &v : d.GetArray()) {
                if ((r =
                         make_everything_canonical(v, a, ccs, f)) != mec_error::success) {
                    goto Error;
                }
            }
            break;
...
int make_canonical_context(req_state *s,
                           std::string_view &context, std::string &cooked_context)
{
...
//       d.ParseStream<rapidjson::kParseNumbersAsStringsFlag>(isw);
    ...
    if (!add_object_to_context(obj, d)) {
        ldpp_dout(s,
                  -1) << "ERROR: can't add default value to context" << dendl;
        s->err.message = "context: internal error adding defaults";
        return -ERR_INVALID_REQUEST;
    }
    b = make_everything_canonical(d, allocator, ccs,
                                  options) == mec_error::success;
    if (!b) {
...
CryptoAccelRef get_crypto_accel(const DoutPrefixProvider *dpp,
                                CephContext *cct)
{
    ...
    if (factory == nullptr) {
        ldpp_dout(dpp,
                  -1) << __func__ << " cannot load crypto accelerator of type "
                       ...
                       const unsigned char *const key, const bool encrypt)
{
    ...
    // operate!
    int written = 0;
    ceph_assert(size <= static_cast < size_t >
...
public:
    explicit AES_256_CBC(const DoutPrefixProvider *dpp,
                         CephContext *cct): dpp(dpp), cct(cct)
    {
    } ~AES_256_CBC()
    {
    ...
    bool cbc_transform(unsigned char *out,
                       ...
                       bool encrypt)
    {
        return evp_sym_transform < AES_256_KEYSIZE, AES_256_IVSIZE > (dpp, cct,
                EVP_aes_256_cbc
                (), out,
                in, size,
                iv, key,
...
std::unique_ptr < BlockCrypt > AES_256_CBC_create(const DoutPrefixProvider *dpp, CephContext *cct,
...
int RGWGetObj_BlockDecrypt::read_manifest(const DoutPrefixProvider *dpp,
...
RGWPutObj_BlockEncrypt::RGWPutObj_BlockEncrypt(const DoutPrefixProvider *dpp,
        ...
        std::unique_ptr < BlockCrypt >
        crypt)
    :  Pipe(next), dpp(dpp), cct(cct), crypt(std::move(crypt)),
       block_size(this->crypt->get_block_size())
{
}
...
{
    ldpp_dout(this->dpp,
              25) << "Encrypt " << data.length() << " bytes" << dendl;
...
struct CryptAttributes {
    ...
    } std::string_view get(crypt_option_e option)
    {
        ...
        auto hdr {
            x_meta_map.find(crypt_options[option].post_part_name)};
...
static int get_sse_s3_bucket_key(req_state *s, std::string &key_id)
{
    ...
    if (key_id == cant_expand_key) {
        ldpp_dout(s, 5) << "ERROR: unable to expand key_id " <<
                        s->cct->_conf->
                        rgw_crypt_sse_s3_key_template << " on bucket" << dendl;
    ...
    if (saved_key != key_id) {
        ...
        if (res != 0) {
            ldpp_dout(s,
                      5) << "ERROR: unable to save new key_id on bucket" <<
                         dendl;
...
{
    int res = 0;
    CryptAttributes crypt_attributes {
        s};
...

    {
        ...
        if (!req_sse_ca.empty()) {
            ...
            key_hash.Update(reinterpret_cast <
                            const unsigned char * >(key_bin.c_str()),
                            key_bin.size());
            ...
            if (block_crypt) {
                auto aes =
                    std::unique_ptr < AES_256_CBC >
                    (new AES_256_CBC(s, s->cct));
        ...
        if (!req_sse.empty()) {
            ...
            if (req_sse == "aws:kms") {
                ...
                if (key_id.empty()) {
                    ldpp_dout(s,
                              5) << "ERROR: not provide a valid key id" <<
                                 dendl;
                    s->err.message =
                ...
                if (res != 0) {
                    ldpp_dout(s,
                              5) <<
                                 "ERROR: failed to retrieve actual key from key_id: " <<
                                 key_id << dendl;
                    s->err.message =
                ...
                crypt_http_responses["x-amz-server-side-encryption"] =
                    "aws:kms";
                crypt_http_responses
                ["x-amz-server-side-encryption-aws-kms-key-id"] =
                    std::string(key_id);
        ...
        if (s->cct->_conf->rgw_crypt_sse_s3_backend == "vault") {
            ...
            if (actual_key.size() != AES_256_KEYSIZE) {
                ldpp_dout(s, 5) << "ERROR: key obtained from key_id:" <<
                                key_id << " is not 256 bit size" << dendl;
                s->err.message =
                    "SSE-S3 provided an invalid key for the given keyid.";
                return -EINVAL;
            }
        ...
        if (s->cct->_conf->rgw_crypt_default_encryption_key != "") {
            ...
            if (AES_256_ECB_encrypt(s, s->cct,
                ...
                ::ceph::crypto::zeroize_for_security(actual_key,
                                                     sizeof(actual_key));
                return -EIO;
            }
            if (block_crypt) {
                           ...
                           std::string > &crypt_http_responses)
{
    ...
    if (stored_mode == "SSE-C-AES256") {
        ...
        try {
            ...
            ldpp_dout(s,
                      5) <<
                         "ERROR: rgw_s3_prepare_decrypt invalid encryption key " <<
...
int rgw_remove_sse_s3_bucket_key(req_state *s)
{
    int res;
    auto key_id {
        expand_key_name(s, s->cct->_conf->rgw_crypt_sse_s3_key_template)};
    auto saved_key {
        fetch_bucket_key_id(s)};
    size_t i;
    ...
    if (saved_key == "") {
        ...
        ldpp_dout(s,
                  5) << "Found but will not delete strange KEK ID: " <<
                     saved_key << dendl;
        return 0;
    }
    ...
    if (res != 0) {
        ldpp_dout(s,
                  0) << "ERROR: Unable to remove KEK ID: " << saved_key <<
                     " got " << res << dendl;
    }
    return res;
}
