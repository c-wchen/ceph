...
namespace crimson::auth {
    ...
    seastar::future < KeyRing * >load_from_keyring(KeyRing * keyring) {
        ...
        return seastar::map_reduce(paths,[](auto path) {
                                   return seastar::engine().file_exists(path).
                                   then([path] (bool file_exists) {
                                        return std::make_pair(file_exists,
                                                              path);}
                                   );}
                                   , std::move(found),[](auto found,
                                                         auto
                                                         file_exists_and_path) {
                                   if (!found.first
                                       && file_exists_and_path.first) {
                                   found = std::move(file_exists_and_path);}
                                   return found;}
        )
            .then([keyring] (auto file_exists_and_path) {
                  const auto &[exists, path] = file_exists_and_path; if (exists) {
