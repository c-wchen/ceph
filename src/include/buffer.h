    ...
    namespace buffer CEPH_BUFFER_API {
        ...
        struct malformed_input:public error {
            explicit malformed_input(const std::string & w) {
                snprintf(buf, sizeof(buf), "buffer::malformed_input: %s",
                         w.c_str());
            }
            const char *what() const throw() override;
          private:
        ...
        raw *create_aligned_in_mempool(unsigned len, unsigned align,
                                       int mempool);
        ...
        class CEPH_BUFFER_API ptr {
          ...
          public:
             class iterator {
                ...
                :bp(p),
                    ...
                    pos(start), end_ptr(p->end_c_str()), deep(d) {
                } friend class ptr;
          ...
          ptr():_raw(0), _off(0), _len(0) {
            ...
            // misc bool at_buffer_head() const { return _off == 0;
        }
    ...
    class CEPH_BUFFER_API list {
      ...
      private:
         template < bool is_const >
            class CEPH_BUFFER_API iterator_impl:public std::iterator <
            std::forward_iterator_tag, char > {
          protected:
            typedef typename std::conditional < is_const,
                const list, list >::type bl_t;
            typedef typename std::conditional < is_const,
                const std::list < ptr >, std::list < ptr > >::type list_t;
            typedef typename std::conditional < is_const,
          ...
          public:
            ...
            /// true if iterator is at the end of the buffer::list bool end() const {
                return p == ls->end();
        ...
        class contiguous_appender {
            ...
            void flush_and_continue() {
                if (bp.have_raw()) {
                    ...
                    if (l) {
                        pbl->append_buffer.set_length(pbl->
                                                      append_buffer.length() +
                                                      l);
                        pbl->append(pbl->append_buffer,
                                    pbl->append_buffer.end() - l, l);
                        pos = pbl->append_buffer.end_c_str();
                    }
                }
            }
          ...
          public:
            ...
            size_t get_logical_offset() {
                if (bp.have_raw()) {
                    ...
                    return out_of_band_offset + (pos -
                                                 pbl->
                                                 append_buffer.end_c_str());
                }
            }
        };

        contiguous_appender get_contiguous_appender(size_t len, bool deep =
                                                    false) {
          ...
          public:
            ...
            void append(const char *buf, size_t len) {
                while (len > 0) {
                    if (!pos) {
                        size_t alloc =
                            (len + CEPH_PAGE_SIZE - 1) & CEPH_PAGE_MASK;
                        if (alloc < min_alloc) {
                            alloc = min_alloc;
                        }
      ...
      list():_len(0), _memcopy_count(0), last_p(this) {
        ...
        // crope lookalikes.// **** WARNING: this are horribly inefficient for large bufferlists. ****
