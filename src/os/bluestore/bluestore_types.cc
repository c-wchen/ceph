...
void bluestore_bdev_label_t::encode(bufferlist & bl) const const
{
...
void bluestore_bdev_label_t::dump(Formatter * f) const const
{
...
void bluestore_bdev_label_t::generate_test_instances(list <
                                                     bluestore_bdev_label_t *
                                                     >&o)
{
...
void bluestore_cnode_t::dump(Formatter * f) const const
{
...
void bluestore_extent_ref_map_t::_check() const const
{
    uint64_t pos = 0;
    unsigned refs = 0;
...
void bluestore_extent_ref_map_t::_maybe_merge_left(map < uint64_t,
                                                   record_t >::iterator & p)
{
    if (p == ref_map.begin())
        return;
...
void bluestore_extent_ref_map_t::get(uint64_t offset, uint32_t length)
{
    ...
    while (length > 0) {
        if (p == ref_map.end()) {
            ...
            p = ref_map.insert(map < uint64_t,
                               record_t >::value_type(offset,
                                                      record_t(length,
                                                               1))).first;
            break;
        }
        if (p->first > offset) {
            ...
            p = ref_map.insert(map < uint64_t, record_t >::value_type(offset,
                                                                      record_t
                                                                      (newlen,
                                                                       1))).
                first;
                                     ...
                                     bool * maybe_unshared)
{
    ...
    while (length > 0) {
        ...
        if (p->second.refs > 1) {
            ...
            if (release)
                release->
                    push_back(bluestore_pextent_t(p->first, p->second.length));
            ref_map.erase(p++);
        }
    }
    if (p != ref_map.end())
...
bool bluestore_extent_ref_map_t::contains(uint64_t offset, uint32_t length) const const
{
    auto p = ref_map.lower_bound(offset);
    if (p == ref_map.end() || p->first > offset) {
        if (p == ref_map.begin()) {
...
bool bluestore_extent_ref_map_t::intersects(uint64_t offset, uint32_t length) const const
{
    auto p = ref_map.lower_bound(offset);
    if (p != ref_map.begin()) {
        --p;
...
void bluestore_extent_ref_map_t::dump(Formatter * f) const const
{
...
void bluestore_extent_ref_map_t::generate_test_instances(list <
                                                         bluestore_extent_ref_map_t
                                                         * >&o)
{
...
ostream & operator<<(ostream & out, const bluestore_extent_ref_map_t & m)
{
    ...
    for (auto p = m.ref_map.begin(); p != m.ref_map.end(); ++p) {
        if (p != m.ref_map.begin())
            out << ",";
        out << std::hex << "0x" << p->first << "~" << p->second.
            length << std::dec << "=" << p->second.refs;
    }
    out << ")";
    return out;
}
...
:  au_size
{
tracker.au_size}, num_au(0), alloc_au(0), bytes_per_au {
nullptr}
...
void bluestore_blob_use_tracker_t::allocate(uint32_t au_count)
{
    ...
    mempool::get_pool(mempool::
                      pool_index_t(mempool::
                                   mempool_bluestore_cache_other)).adjust_count
        (alloc_au, sizeof(uint32_t) * alloc_au);
...
bool bluestore_blob_use_tracker_t::can_split() const const
{
    return num_au > 0;
}

bool bluestore_blob_use_tracker_t::can_split_at(uint32_t blob_offset) const const
{
    ceph_assert(au_size);
...
bool bluestore_blob_use_tracker_t::
equal(const bluestore_blob_use_tracker_t & other) const const
{
...
void bluestore_blob_use_tracker_t::dump(Formatter * f) const const
{
...
void bluestore_blob_use_tracker_t::generate_test_instances(list <
                                                           bluestore_blob_use_tracker_t
                                                           * >&o)
{
...
void bluestore_pextent_t::dump(Formatter * f) const const
{
...
{
    if (o.is_valid())
        return out << "0x" << std::hex << o.offset << "~" << o.
            length << std::dec;
    else
        return out << "!~" << std::hex << o.length << std::dec;
}

void bluestore_pextent_t::generate_test_instances(list <
                                                  bluestore_pextent_t * >&ls)
{
...
size_t bluestore_blob_t::get_csum_value_size() const const
{
    return Checksummer::get_csum_value_size(csum_type);
}

void bluestore_blob_t::dump(Formatter * f) const const
{
...
void bluestore_blob_t::generate_test_instances(list < bluestore_blob_t * >&ls)
{
    ...
    ls.back()->
        allocated_test(bluestore_pextent_t
...
ostream & operator<<(ostream & out, const bluestore_blob_t & o)
{
    ...
    if (o.is_compressed()) {
        out << " clen 0x" << std::hex << o.get_logical_length()
            << " -> 0x" << o.get_compressed_payload_length()
            << std::dec;
    }
    if (o.flags) {
...
void bluestore_blob_t::calc_csum(uint64_t b_off, const bufferlist & bl)
{
    ...
    case Checksummer::CSUM_XXHASH32:
        Checksummer::calculate < Checksummer::xxhash32 > (get_csum_chunk_size(),
                                                          b_off, bl.length(),
                                                          bl, &csum_data);
        break;
    case Checksummer::CSUM_XXHASH64:
        Checksummer::calculate < Checksummer::xxhash64 > (get_csum_chunk_size(),
                                                          b_off, bl.length(),
                                                          bl, &csum_data);
        break;;
    case Checksummer::CSUM_CRC32C:
        Checksummer::calculate < Checksummer::crc32c > (get_csum_chunk_size(),
                                                        b_off, bl.length(), bl,
                                                        &csum_data);
        break;
    ...
    case Checksummer::CSUM_CRC32C_8:
        Checksummer::calculate < Checksummer::crc32c_8 > (get_csum_chunk_size(),
                                                          b_off, bl.length(),
                                                          bl, &csum_data);
        break;
    }
                                  ...
                                  uint64_t * bad_csum) const const
{
    ...
    *b_bad_off = -1;
    ...
    case Checksummer::CSUM_XXHASH32:
        *b_bad_off =
            Checksummer::verify < Checksummer::xxhash32 >
            (get_csum_chunk_size(), b_off, bl.length(), bl, csum_data,
             bad_csum);
        break;
    case Checksummer::CSUM_XXHASH64:
        *b_bad_off =
            Checksummer::verify < Checksummer::xxhash64 >
            (get_csum_chunk_size(), b_off, bl.length(), bl, csum_data,
             bad_csum);
        break;
    case Checksummer::CSUM_CRC32C:
        *b_bad_off =
            Checksummer::verify < Checksummer::crc32c > (get_csum_chunk_size(),
                                                         b_off, bl.length(), bl,
                                                         csum_data, bad_csum);
        break;
    case Checksummer::CSUM_CRC32C_16:
        *b_bad_off =
            Checksummer::verify < Checksummer::crc32c_16 >
            (get_csum_chunk_size(), b_off, bl.length(), bl, csum_data,
             bad_csum);
        break;
    case Checksummer::CSUM_CRC32C_8:
        *b_bad_off =
            Checksummer::verify < Checksummer::crc32c_8 >
            (get_csum_chunk_size(), b_off, bl.length(), bl, csum_data,
             bad_csum);
        break;
    default:
    ...
    if (extents.size() == 0) {
        ...
        if (b_off) {
            extents.
                emplace_back(bluestore_pextent_t
                             (bluestore_pextent_t::INVALID_OFFSET, b_off));
      ...
      for (auto & a:allocs) {
        ...
        extents.insert(start_it,
                       count,
                       bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET,
                                           0));
...
struct vecbuilder {
    ...
    void add_invalid(uint64_t length) {
        ...
        if (invalid) {
            v.emplace_back(bluestore_pextent_t
                           (bluestore_pextent_t::INVALID_OFFSET, invalid));
...
void bluestore_shared_blob_t::dump(Formatter * f) const const
{
    f->dump_int("sbid", sbid);
    f->dump_object("ref_map", ref_map);
}

void bluestore_shared_blob_t::generate_test_instances(list <
                                                      bluestore_shared_blob_t *
                                                      >&ls)
{
...
void bluestore_onode_t::shard_info::dump(Formatter * f) const const
{
    f->dump_unsigned("offset", offset);
...
void bluestore_onode_t::dump(Formatter * f) const const
{
...
void bluestore_deferred_op_t::dump(Formatter * f) const const
{
...
void bluestore_deferred_op_t::generate_test_instances(list <
                                                      bluestore_deferred_op_t *
                                                      >&o)
{
...
void bluestore_deferred_transaction_t::dump(Formatter * f) const const
{
...
void bluestore_deferred_transaction_t::generate_test_instances(list <
...
void bluestore_compression_header_t::dump(Formatter * f) const const
{
...
void bluestore_compression_header_t::generate_test_instances(list <
...
void shared_blob_2hash_tracker_t::inc_range(uint64_t sbid,
                                            uint64_t offset,
                                            uint32_t len, int n)
{
    ...
    const
{
    ...
    return ref_counter_2hash_tracker_t::test_hash_conflict((char *)hash_input1.
                                                           data(),
                                                           (char *)hash_input2.
                                                           data(),
                                                           get_hash_input_size
...
bool shared_blob_2hash_tracker_t::test_all_zero(uint64_t sbid, uint64_t offset) const const
{
    auto hash_input = build_hash_input(sbid, offset);
    return
        ref_counter_2hash_tracker_t::test_all_zero((char *)hash_input.data(),
                                                   get_hash_input_size());
}

bool shared_blob_2hash_tracker_t::test_all_zero_range(uint64_t sbid,
                                                      uint64_t offset,
                                                      uint32_t len) const const
{
    uint32_t alloc_unit = 1 << au_void_bits;
    int64_t l = len;
    while (l > 0) {
