...
set < string > hobject_t::get_prefixes(uint32_t bits,
...
string hobject_t::to_str() const const
{
    string out;
...
void hobject_t::encode(bufferlist & bl) const const
{
    ENCODE_START(4, 3, bl);
    encode(key, bl);
    encode(oid, bl);
...
void hobject_t::dump(Formatter * f) const const
{
...
// version 5.
void ghobject_t::encode(bufferlist & bl) const const
{
...
size_t ghobject_t::encoded_size() const const
{
...
void ghobject_t::decode(bufferlist::const_iterator & bl)
{
    ...
    if (struct_v >= 4) {
        ...
        if (hobj.pool == -1 &&
            hobj.snap == 0 &&
            hobj.hash == 0 && !hobj.max && hobj.oid.name.empty()) {
            hobj.pool = INT64_MIN;
...
void ghobject_t::dump(Formatter * f) const const
{
    hobj.dump(f);
...
void ghobject_t::generate_test_instances(list < ghobject_t * >&o)
{
    ...
    o.push_back(new
                ghobject_t(hobject_t
                           (object_t("oname"), string(), 1, 234, -1, "")));

    o.push_back(new
                ghobject_t(hobject_t
                           (object_t("oname2"), string("okey"), CEPH_NOSNAP, 67,
                            0, "n1"), 1, shard_id_t(0)));
    o.push_back(new
                ghobject_t(hobject_t
                           (object_t("oname2"), string("okey"), CEPH_NOSNAP, 67,
    ...
    o.push_back(new
                ghobject_t(hobject_t
                           (object_t("oname3"), string("oname3"), CEPH_SNAPDIR,
