...
PGRecovery::start_recovery_ops(RecoveryBackend::RecoveryBlockingEvent::
                               TriggerI &trigger, size_t max_to_start)
{
    ...
    if (max_to_start > 0) {
        max_to_start -=
            start_replica_recovery_ops(trigger, max_to_start, &started);
    }
    using interruptor =
        crimson::interruptible::interruptor <
        crimson::osd::IOInterruptCondition >;
    return interruptor::parallel_for_each(started, [](auto && ifut) {
        ...
        if (done) {
            logger().
            debug
            ("start_recovery_ops: AllReplicasRecovered for pg: {}",
             pg->get_pgid());
            using LocalPeeringEvent =
            ...
            if (!pg->get_peering_state().needs_backfill()) {
                ...
                ("start_recovery_ops: AllReplicasRecovered for pg: {}",
                 pg->get_pgid());
                (void)pg->get_shard_services().start_operation <
                LocalPeeringEvent > (static_cast <
                                     crimson::osd::PG * >(pg),
                                     pg->get_pg_whoami(),
                                     pg->get_pgid(),
                                     pg->get_osdmap_epoch(),
                ...
                logger().
                debug("start_recovery_ops: RequestBackfill for pg: {}",
                      pg->get_pgid());
                (void)pg->get_shard_services().start_operation <
                LocalPeeringEvent > (static_cast <
                                     crimson::osd::PG * >(pg),
...
size_t PGRecovery::start_primary_recovery_ops(RecoveryBackend::
        RecoveryBlockingEvent::
        TriggerI &trigger,
        size_t max_to_start,
        std::vector <
        PGRecovery::
        interruptible_future < >> *out)
{
    if (!pg->is_recovering()) {
        return 0;
    }
    ...
    map < version_t, hobject_t >::const_iterator p =
        missing.get_rmissing().lower_bound(pg->get_peering_state().get_pg_log().
    ...
    while (started < max_to_start && p != missing.get_rmissing().end()) {
        ...
        auto it_objects =
            pg->get_peering_state().get_pg_log().get_log().objects.find(p->
                second);
        if (it_objects !=
        ...
        logger().info("{} {} item.need {} {} {} {} {}",
                      ...
                      is_recovering(soid) ? " (recovering)" : "",
                      pg->get_recovery_backend()->
                      is_recovering(head) ? " (recovering head)" : "");

        // TODO: handle lost/unfound
        if (pg->get_recovery_backend()->is_recovering(soid)) {
            auto &recovery_waiter =
                pg->get_recovery_backend()->get_recovering(soid);
...
size_t PGRecovery::start_replica_recovery_ops(RecoveryBackend::
        RecoveryBlockingEvent::
        TriggerI &trigger,
        size_t max_to_start,
        std::vector <
        ...
        interruptible_future < >> *out)
{
    ...
    for (auto &peer : recovery_order) {
        ...
        for (auto p = pm.get_rmissing().begin();
            ...
            if (soid > pi.last_backfill) {
                if (!pg->get_recovery_backend()->is_recovering(soid)) {
                    ...
                    ("{}: object {} in missing set for backfill (last_backfill {})"
                     " but not in recovering", __func__, soid,
                     pi.last_backfill);
                    ceph_abort();
                }
                continue;
            ...
            if (pg->get_recovery_backend()->is_recovering(soid)) {
                logger().debug("{}: already recovering object {}", __func__,
                               soid);
                auto &recovery_waiter =
                    pg->get_recovery_backend()->get_recovering(soid);
                out->emplace_back(recovery_waiter.wait_for_recovered(trigger));
            ...
            if (pg->get_peering_state().get_missing_loc().is_deleted(soid)) {
                logger().debug("{}: soid {} is a delete, removing", __func__,
                               soid);
                map < hobject_t, pg_missing_item >::const_iterator r =
                    pm.get_items().find(soid);
                started++;
                out->
                emplace_back(prep_object_replica_deletes
                             (trigger, soid, r->second.need));
                continue;
            }
            ...
            if (pg->get_peering_state().get_pg_log().get_missing().
                is_missing(soid)) {
                logger().debug("{}: soid {} still missing on primary", __func__,
                               soid);
                continue;
            }
            ...
            started++;
            out->
            emplace_back(prep_object_replica_pushes
                         (trigger, soid, r->second.need));
        }
    }
...
PGRecovery::recover_missing(RecoveryBackend::RecoveryBlockingEvent::
                            TriggerI &trigger, const hobject_t &soid,
                            eversion_t need)
{
    if (pg->get_peering_state().get_missing_loc().is_deleted(soid)) {
        return pg->get_recovery_backend()->add_recovering(soid).
               wait_track_blocking(trigger,
                                   pg->get_recovery_backend()->recover_delete(soid,
                                           need));
    } else {
        return pg->get_recovery_backend()->add_recovering(soid).
               wait_track_blocking(trigger,
                                   pg->get_recovery_backend()->recover_object(soid,
                                           need)
                                   .
                                   handle_exception_interruptible([ =, this, soid =
                                               std::
                                               move(soid)]
        (auto e) {
            on_failed_recover({
...
RecoveryBackend::interruptible_future <> PGRecovery::
prep_object_replica_deletes(RecoveryBackend::RecoveryBlockingEvent::
                            TriggerI &trigger, const hobject_t &soid,
                            eversion_t need)
{
    return pg->get_recovery_backend()->add_recovering(soid).
           wait_track_blocking(trigger,
                               pg->get_recovery_backend()->push_delete(soid,
                                       need).
    then_interruptible([ =, this] {
...
RecoveryBackend::interruptible_future <> PGRecovery::
prep_object_replica_pushes(RecoveryBackend::RecoveryBlockingEvent::
                           TriggerI &trigger, const hobject_t &soid,
                           eversion_t need)
{
    return pg->get_recovery_backend()->add_recovering(soid).
...
void PGRecovery::on_local_recover(const hobject_t &soid,
                                  ...
                                  ceph::os::Transaction &t)
{
    if (const auto &log = pg->get_peering_state().get_pg_log();
        ...
        if (const auto *latest =
            ...
            ceph_abort
            ("mark_unfound_lost (LOST_REVERT) is not implemented yet");
        }
    }
...
void PGRecovery::on_global_recover(const hobject_t &soid,
...
void PGRecovery::on_failed_recover(const set < pg_shard_t > &from,
...
void PGRecovery::on_peer_recover(pg_shard_t peer,
                                 const hobject_t &oid,
                                 const ObjectRecoveryInfo &recovery_info)
{
    crimson::get_logger(ceph_subsys_osd).debug("{}: {}, {} on {}", __func__,
            oid, recovery_info.version,
            peer);
        ...
        eversion_t last_complete)
{
    if (!pg->has_reset_since(epoch)) {
        ...
        debug("{} pg has changed, not touching last_complete_ondisk",
              __func__);
    }
}
...
void PGRecovery::start_backfill_recovery(const EventT &evt)
{
    ...
    std::ignore =
        ...
        (static_cast < crimson::osd::PG * >(pg), pg->get_shard_services(),
         pg->get_osdmap_epoch(), evt);
}

void PGRecovery::request_replica_scan(const pg_shard_t &target,
                                      const hobject_t &begin,
                                      const hobject_t &end)
{
    logger().debug("{}: target.osd={}", __func__, target.osd);
    auto msg =
        crimson::make_message < MOSDPGScan > (MOSDPGScan::OP_SCAN_GET_DIGEST,
            pg->get_pg_whoami(),
            pg->get_osdmap_epoch(),
            pg->get_last_peering_reset(),
            spg_t(pg->get_pgid().pgid,
                  target.shard),
            begin,
            end);
    std::ignore = pg->get_shard_services().send_to_osd(target.osd,
                  std::move(msg),
                  pg->get_osdmap_epoch());
}
...
void PGRecovery::request_primary_scan(const hobject_t &begin)
{
    ...
    std::ignore = pg->get_recovery_backend()->scan_for_backfill(begin,
                  local_conf()->
    ...
    then_interruptible([this](BackfillInterval bi) {
        ...
        start_backfill_recovery(BackfillState::
        PrimaryScanned {
            std::move(bi)});
    });
}
...
void PGRecovery::enqueue_push(const hobject_t &obj, const eversion_t &v)
{
    ...
    std::ignore =
        pg->get_recovery_backend()->recover_object(obj,
            v).handle_exception_interruptible
    ([](auto) {
        ...
        using BackfillState = crimson::osd::BackfillState;
        start_backfill_recovery(BackfillState::
                                ObjectPushed(std::move(obj)));
    }
                            );
}

void PGRecovery::enqueue_drop(const pg_shard_t &target,
                              const hobject_t &obj, const eversion_t &v)
{
    // allocate a pair if target is seen for the first time
    auto &req = backfill_drop_requests[target];
    if (!req) {
        req =
            crimson::make_message < MOSDPGBackfillRemove >
...
void PGRecovery::update_peers_last_backfill(const hobject_t &new_last_backfill)
{
    ...
    for (const auto &bt : pg->get_peering_state().get_backfill_targets()) {
        if (const pg_info_t &pinfo = pg->get_peering_state().get_peer_info(bt);
            ...
            pg->get_peering_state().update_peer_last_backfill(bt,
                new_last_backfill);
            auto m =
                crimson::make_message < MOSDPGBackfill >
                (pinfo.last_backfill.
                 is_max() ? MOSDPGBackfill::OP_BACKFILL_FINISH : MOSDPGBackfill::
            ...
            m->stats = pinfo.stats;
            std::ignore =
                pg->get_shard_services().send_to_osd(bt.osd, std::move(m),
                    pg->get_osdmap_epoch());
            logger().info("{}: peer {} num_objects now {} / {}", __func__, bt,
...
bool PGRecovery::budget_available() const const
{
    // TODO: the limits!
    return true;
}
...
void PGRecovery::dispatch_backfill_event(boost::intrusive_ptr <
...
void PGRecovery::on_backfill_reserved()
{
    ...
    using BackfillState = crimson::osd::BackfillState;
    backfill_state = std::make_unique < BackfillState > (*this,
                     std::make_unique <
                     crimson::osd::
                     PeeringFacade >
                     (pg->
