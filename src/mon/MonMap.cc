...
void mon_info_t::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(2, 1, bl);
    ::encode(name, bl);
...
{
    out << "mon." << name
        << " public " << public_addr << " priority " << priority;
}
...
void MonMap::calc_ranks()
{
    ...
    unsigned i = 0;
    for (set < mon_info_t >::iterator p = tmp.begin(); p != tmp.end(); ++p, ++i) {
        ranks[i] = p->name;
    }
}

void MonMap::encode(bufferlist & blist, uint64_t con_features) const const
{
...
int MonMap::build_from_host_list(std::string hostlist, std::string prefix)
{
    ...
    for (unsigned i = 0; i < addrs.size(); i++) {
        ...
        string name = prefix;
        name += n;
        if (!contains(addrs[i]) && !contains(name))
            add(name, addrs[i]);
    }
    return 0;
}
                                 ...
                                 set < entity_addr_t > *removed)
{
    ...
    while (i < size()) {
        ...
        if (std::find(initial_members.begin(), initial_members.end(), n) !=
            initial_members.end()) {
            lgeneric_dout(cct,
                          1) << " keeping " << n << " " << get_addr(i) << dendl;
            i++;
            continue;
        }

        lgeneric_dout(cct,
                      1) << " removing " << get_name(i) << " " << get_addr(i) <<
            dendl;
        if (removed)
    ...
    for (list < string >::iterator p = initial_members.begin();
         p != initial_members.end(); ++p) {
        if (!contains(*p)) {
            if (*p == my_name) {
                lgeneric_dout(cct,
                              1) << " adding self " << *p << " " << my_addr <<
                    dendl;
                add(*p, my_addr);
            }
            else {
