...
void add_config_entity_option(boost::program_options::
                              options_description *
                              positional)
{
    positional->add_options()
    ("config-entity",
     "config entity (global, client, client.<id>)");
} void add_pool_option(boost::program_options::
                       options_description *positional)
{
    positional->add_options()
    ("pool-name", "pool name");
} void add_key_option(po::options_description *positional)
{
    positional->add_options()
    ("key", "config key");
} int get_config_entity(const po::variables_map &vm,
                        std::string *config_entity)
{
    *config_entity = utils::get_positional_argument(vm, 0);
    ...
    if (*config_entity != "global" && *config_entity != "client"
        ...
        std::
        cerr << "rbd: invalid config entity: " <<
             *config_entity <<
             " (must be global, client or client.<id>)" << std::
             endl;
        return -EINVAL;
    }
...
int get_pool(const po::variables_map &vm,
             std::string *pool_name)
{
    *pool_name = utils::get_positional_argument(vm, 0);
    if (pool_name->empty()) {
        std::cerr << "rbd: pool name was not specified" << std::
                  endl;
        return -EINVAL;
    }
            ...
            std::string *key)
{
    *key = utils::get_positional_argument(vm, *arg_index);
    if (key->empty()) {
        std::
        cerr << "rbd: config key was not specified" << std::
             endl;
        return -EINVAL;
    } else {
        ++(*arg_index);
    ...
    if (!boost::starts_with(*key, "rbd_")) {
        std::cerr << "rbd: not rbd option: " << *key << std::
                  endl;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        std::
        cerr << "rbd: invalid config key: " << *key << std::
             endl;
        return -EINVAL;
    }
...
int config_global_list(librados::Rados &rados,
                       ...
                       std::string >> *options)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: error reading config: " << ss << std::
             endl;
        return r;
    }
    ...
    if (!json_spirit::read(out_bl.to_str(), json_root)) {
        std::cerr << "rbd: error parsing config dump" << std::
                  endl;
        return -EINVAL;
    }
    ...
    try {
        ...
        for (auto &e : json_array) {
            ...
            if (section != "global" && section != "client" &&
                (!client_id_config_entity
                 || section != config_entity)) {
                continue;
            }
            if (config_entity == "global"
                && section != "global") {
                continue;
            }
        ...
        std::cerr << "rbd: error parsing config dump: " << e.
                  what() << std::endl;
        return -EINVAL;
    }
...
int execute_global_remove(const po::variables_map &vm,
                          const std::vector < std::string >
                          &ceph_global_init_args)
{
    std::string config_entity;
    ...
    if (r < 0) {
        std::
        cerr << "rbd: error removing " << key << ": " << ss <<
             std::endl;
        return r;
    }
                        ...
                        &ceph_global_init_args)
{
    ...
    if (f) {
        ...
        tbl.define_column("Value", TextTable::LEFT,
                          TextTable::LEFT);
        tbl.define_column("Section", TextTable::LEFT,
                          TextTable::LEFT);
    }
                     ...
                     &ceph_global_init_args)
{
    ...
    r = rbd.pool_metadata_get(io_ctx, METADATA_CONF_PREFIX + key,
                              &value);
    if (r < 0) {
        if (r == -ENOENT) {
            std::cerr << "rbd: " << key << " is not set" << std::
                      endl;
        } else {
            std::
            cerr << "rbd: failed to get " << key << ": " <<
                 cpp_strerror(r)
                 << std::endl;
        }
        return r;
                     ...
                     &ceph_global_init_args)
{
    ...
    librbd::RBD rbd;
    r = rbd.pool_metadata_set(io_ctx, METADATA_CONF_PREFIX + key,
                              value);
    if (r < 0) {
        std::
        cerr << "rbd: failed to set " << key << ": " <<
             cpp_strerror(r)
             << std::endl;
        return r;
    }
                        ...
                        &ceph_global_init_args)
{
    ...
    librbd::RBD rbd;
    r = rbd.pool_metadata_remove(io_ctx,
                                 METADATA_CONF_PREFIX + key);
    if (r < 0) {
        std::
        cerr << "rbd: failed to remove " << key << ": " <<
             cpp_strerror(r)
             << std::endl;
        return r;
    }
                      ...
                      &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: failed to list config: " <<
    ...
    if (f) {
        ...
        tbl.define_column("Value", TextTable::LEFT,
                          TextTable::LEFT);
        tbl.define_column("Source", TextTable::LEFT,
                          TextTable::LEFT);
    }
    ...
    for (auto &option : options) {
        if (f) {
            ...
            tbl << option.name << option.value << source.
                      ...
                      &ceph_global_init_args)
{
    ...
    if (value.empty()) {
        std::
        cerr << "rbd: image config value was not specified" <<
             std::endl;
        return -EINVAL;
    }
...
void get_image_remove_arguments(po::options_description *
                                positional,
                                po::options_description *options)
{
    at::add_image_spec_options(positional, options,
...
int execute_image_remove(const po::variables_map &vm,
                         const std::vector < std::string >
                         &ceph_global_init_args)
{
    size_t arg_index = 0;
                       ...
                       &ceph_global_init_args)
{
    ...
    if (f == nullptr) {
        ...
        std::
        cout << "There " << (single ? "is" : "are") << " " <<
             options.size()
             << " " << (single ? "value" : "values") << ":" << std::
             endl;
    }
