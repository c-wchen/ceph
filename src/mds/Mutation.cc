...
// auth pins
bool MutationImpl::is_auth_pinned(MDSCacheObject * object) const const
{
...
{
    pin(dn);
    dirty_cow_dentries.push_back(pair < CDentry *,
                                 version_t > (dn, dn->get_projected_version()));
}
...
void MutationImpl::apply()
{
    ...
    for (list < CInode * >::iterator p = dirty_cow_inodes.begin();
         p != dirty_cow_inodes.end(); ++p)
        (*p)->_mark_dirty(ls);
    for (list < pair < CDentry *, version_t > >::iterator p =
         dirty_cow_dentries.begin(); p != dirty_cow_dentries.end(); ++p)
        p->first->mark_dirty(p->second, ls);

    for (list < ScatterLock * >::iterator p = updated_locks.begin();
         p != updated_locks.end(); ++p)
        (*p)->mark_dirty();
}
...
void MutationImpl::_dump_op_descriptor_unlocked(ostream & stream) const const
{
    stream << "Mutation";
}
...
bool MDRequestImpl::has_more() const const
{
...
bool MDRequestImpl::did_ino_allocation() const const
{
...
bool MDRequestImpl::can_auth_pin(MDSCacheObject * object)
{
    return object->can_auth_pin() ||
        (is_auth_pinned(object) && has_more() &&
         more()->is_freeze_authpin && more()->rename_inode == object);
}
...
bool MDRequestImpl::is_queued_for_replay() const const
{
...
void MDRequestImpl::print(ostream & out) const const
{
    out << "request(" << reqid;
    //if (request) out << " " << *request;
    if (is_slave())
        out << " slave_to mds." << slave_to_mds;
    if (client_request)
        out << " cr=" << client_request;
    if (slave_request)
        out << " sr=" << slave_request;
    out << ")";
}

void MDRequestImpl::dump(Formatter * f) const const
{
    _dump(f);
}

void MDRequestImpl::_dump(Formatter * f) const const
{
    ...
    f->dump_stream("reqid") << reqid;
    {
        ...
        if (_client_request) {
            ...
            f->dump_string("op_type",
                           MMDSSlaveRequest::get_opname(_slave_request->
                                                        get_op()));
...
void MDRequestImpl::_dump_op_descriptor_unlocked(ostream & stream) const const
{
    msg_lock.lock();
    boost::intrusive_ptr < MClientRequest > _client_request(client_request);
    boost::intrusive_ptr < MMDSSlaveRequest > _slave_request(slave_request);
    ...
    if (_client_request) {
        ...
        stream << "internal op " << ceph_mds_op_name(internal_op) << ":" <<
            reqid;
    }
    else {
