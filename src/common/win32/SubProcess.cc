...
int SubProcess::get_stdin() const const
{
    ceph_assert(is_spawned());
...
int SubProcess::get_stdout() const const
{
    ceph_assert(is_spawned());
...
int SubProcess::get_stderr() const const
{
    ceph_assert(is_spawned());
...
const std::string SubProcess::err() const const
{
    return errstr.str();
}
...
void SubProcess::kill(int signo) const const
{
    ceph_assert(is_spawned());
...
int SubProcess::spawn()
{
  ...
  for (auto & arg:cmd_args) {
    ...
    si.hStdOutput =
        stdout_op == KEEP ? GetStdHandle(STD_OUTPUT_HANDLE) : stdout_w;
    si.hStdError =
        stderr_op == KEEP ? GetStdHandle(STD_ERROR_HANDLE) : stderr_w;
    si.dwFlags |= STARTF_USESTDHANDLES;

    stdin_pipe_out_fd =
        stdin_op == PIPE ? _open_osfhandle((intptr_t) stdin_w, 0) : -1;
    stdout_pipe_in_fd =
        stdout_op == PIPE ? _open_osfhandle((intptr_t) stdout_r,
                                            _O_RDONLY) : -1;
    stderr_pipe_in_fd =
        stderr_op == PIPE ? _open_osfhandle((intptr_t) stderr_r,
                                            _O_RDONLY) : -1;
    ...
    if (!CreateProcess(NULL, const_cast < char *>(cmdline.str().c_str()), NULL, NULL,   /* No special security attributes */
