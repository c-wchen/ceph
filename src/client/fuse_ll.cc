...
static void fuse_ll_ioctl(fuse_req_t req, fuse_ino_t ino, int cmd, void *arg,
...
CephFuse::Handle::Handle(Client * c, int fd):fd_on_success(fd),
client(c),
ch(NULL),
se(NULL), mountpoint(NULL), stag_lock("fuse_ll.cc stag_lock"), last_stag(0)
{
...
int CephFuse::Handle::start()
{
    ...
    struct client_callback_args args = {
      handle:this,
      ino_cb:client->cct->_conf->
            fuse_use_invalidate_cb ? ino_invalidate_cb : NULL,
