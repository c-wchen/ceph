...
template class rbd::mirror::image_sync::ObjectCopyRequest <
    ...
    namespace mirror {
        namespace image_sync {
            ...
            namespace {
                ...
                void scribble(librbd::ImageCtx * image_ctx, int num_ops,
                    ...
                    for (int i = 0; i < num_ops; i++) {
                        ...
                        int r =
                            image_ctx->io_work_queue->write(off, len,
                                                            std::move(bl), 0);
              ...
              public:
                                       ...
                                       MockExclusiveLock & mock_exclusive_lock)
                {
                    EXPECT_CALL(mock_exclusive_lock,
                                start_op(_)).WillOnce(ReturnNew <
                                                      FunctionContext >
                                                      ([](int) {
                                                       }));
                }

                void expect_list_snaps(librbd::
                    ...
                    EXPECT_CALL(mock_io_ctx,
                                list_snaps(mock_image_ctx.image_ctx->
                                           get_object_name(0), _))
                        .
                        WillOnce(DoAll
                                 (WithArg < 1 >
                                  (Invoke
                                   ([&snap_set]
                                    (librados::snap_set_t * out_snap_set) {
                                    *out_snap_set = snap_set;})), Return(0)));
                }
                ...
                void expect_set_snap_read(librados::
                                          MockTestMemIoCtxImpl & mock_io_ctx,
                                          uint64_t snap_id) {
                    EXPECT_CALL(mock_io_ctx, set_snap_read(snap_id));
                }

                void expect_sparse_read(librados::
                                        MockTestMemIoCtxImpl & mock_io_ctx,
                    ...
                    auto & expect =
                        EXPECT_CALL(mock_io_ctx,
                                    sparse_read(_, offset, length, _, _));
                    if (r < 0) {
                        expect.WillOnce(Return(r));
                    }
                    else {
                ...
                void expect_sparse_read(librados::
                  ...
                  for (auto extent:extents) {
                        expect_sparse_read(mock_io_ctx, extent.first,
                                           extent.second, r);
                        if (r < 0) {
                            break;
                        }
                                          ...
                                          (Invoke
                                           ([&mock_image_ctx, snap_id, state,
                                            ...
                                            assert(mock_image_ctx.image_ctx->
                                                   ...
                                                   is_wlocked());
                                            mock_image_ctx.image_ctx->
                                            object_map->aio_update < Context >
                                            (snap_id, 0, 1, state, boost::none,
                                             {
                                             }
                                             , false, ctx);}
                                           )), Return(true)));
                ...
                int create_snap(const char *snap_name) {
                    ...
                    MockObjectCopyRequest::SnapIds local_snap_ids( {
                                                                  local_snap_id}
                    );
                    if (!m_snap_map.empty()) {
                ...
                int compare_objects() {
                    ...
                    while (!snap_map.empty()) {
                        librados::snap_t remote_snap_id =
                            snap_map.begin()->first;
                        librados::snap_t local_snap_id =
                            *snap_map.begin()->second.begin();
                        snap_map.erase(snap_map.begin());

                        std::string snap_name =
                            get_snap_name(m_remote_image_ctx, remote_snap_id);
                        if (snap_name.empty()) {
                            return -ENOENT;
                        }
                        ...
                        std::
                            ...
                            remote_snap_id << " to " << local_snap_id << ")" <<
                            std::endl;
                        ...
                        r = m_remote_image_ctx->io_work_queue->read(0,
                                                                    object_size,
                                                                    librbd::io::
                                                                    ReadResult {
                                                                    &remote_bl}
                        ...
                        r = m_local_image_ctx->io_work_queue->read(0,
                                                                   object_size,
                                                                   librbd::io::
                                                                   ReadResult {
                                                                   &local_bl}
            ...
            TEST_F(TestMockImageSyncObjectCopyRequest, DNE) {
                ...
                InSequence seq;
                expect_list_snaps(mock_remote_image_ctx, mock_remote_io_ctx,
                                  -ENOENT);
            ...
            TEST_F(TestMockImageSyncObjectCopyRequest, ReadMissingStaleSnapSet) {
                ...
                expect_write(mock_local_io_ctx, 0, one.range_end(), {
                             m_local_snap_ids[1], {
                             m_local_snap_ids[1], m_local_snap_ids[0]}
                             }
                             , 0);
                expect_start_op(mock_exclusive_lock);
                expect_update_object_map(mock_local_image_ctx, mock_object_map,
            ...
            TEST_F(TestMockImageSyncObjectCopyRequest,
                ...
                expect_list_snaps(mock_remote_image_ctx, mock_remote_io_ctx, 0);
                expect_set_snap_read(mock_remote_io_ctx, m_remote_snap_ids[0]);
                expect_sparse_read(mock_remote_io_ctx, 0, one.range_end(),
                                   -ENOENT);
                expect_list_snaps(mock_remote_image_ctx, mock_remote_io_ctx, 0);
            ...
            TEST_F(TestMockImageSyncObjectCopyRequest, Trim) {
                ASSERT_EQ(0,
                          ...
                          metadata_set("conf_rbd_skip_partial_discard",
                                       "false"));
                // scribble some data
                interval_set < uint64_t > one;
                scribble(m_remote_image_ctx, 10, 102400, &one);
                ...
                ASSERT_LE(0,
                          m_remote_image_ctx->io_work_queue->
                          discard(trim_offset, one.range_end() - trim_offset,
                                  m_remote_image_ctx->skip_partial_discard));
                ASSERT_EQ(0, create_snap("sync"));
            ...
            TEST_F(TestMockImageSyncObjectCopyRequest, Remove) {
                ...
                uint64_t object_size = 1 << m_remote_image_ctx->order;
                ASSERT_LE(0,
                          m_remote_image_ctx->io_work_queue->discard(0,
                                                                     object_size,
                                                                     m_remote_image_ctx->
