    ...
    namespace journal {
        ...
        template <> class Replay < MockJournalImageCtx > {
          public:
            ...
            int decode(bufferlist::const_iterator * it,
            ...
            void process(const EventEntry & event_entry, Context * on_ready,
                         Context * on_commit) {
                MockReplay::get_instance().process(event_entry, on_ready,
                                                   on_commit);
            }
        ...
        template <> class OpenRequest < MockJournalImageCtx > {
          public:
            ...
            static OpenRequest *create(MockJournalImageCtx *
                                       image_ctx,::journal::MockJournalerProxy *
                                       journaler, ceph::mutex * lock,
        ...
        OpenRequest < MockJournalImageCtx > *OpenRequest <
            MockJournalImageCtx >::s_instance = nullptr;

        template <> class PromoteRequest < MockJournalImageCtx > {
          public:
            static PromoteRequest s_instance;
            static PromoteRequest *create(MockJournalImageCtx * image_ctx,
        ...
        PromoteRequest < MockJournalImageCtx > PromoteRequest <
            MockJournalImageCtx >::s_instance;

        template <>
            struct ObjectDispatch <MockJournalImageCtx >:public io::
            MockObjectDispatch {
            static ObjectDispatch *s_instance;

            static ObjectDispatch *create(MockJournalImageCtx * image_ctx,
                                          Journal < MockJournalImageCtx >
                                          *journal) {
                ceph_assert(s_instance != nullptr);
                return s_instance;
            } ObjectDispatch() {
        ...
        ObjectDispatch < MockJournalImageCtx > *ObjectDispatch <
            MockJournalImageCtx >::s_instance = nullptr;
...
namespace librbd {
    ...
    class TestMockJournal:public TestMockFixture {
      public:
        ...
        typedef journal::OpenRequest < MockJournalImageCtx >
            MockJournalOpenRequest;
        typedef journal::ObjectDispatch < MockJournalImageCtx >
            MockObjectDispatch;
        typedef std::function < void (::journal::ReplayHandler *) >
            ReplayAction;
        ...
        void expect_construct_journaler(::journal::
                                        MockJournaler & mock_journaler) {
            EXPECT_CALL(mock_journaler, construct());
        }

        void expect_open_journaler(MockImageCtx & mock_image_ctx,::journal::
                                   MockJournaler & mock_journaler,
            ...
            EXPECT_CALL(mock_open_request, send())
                .WillOnce(DoAll(Invoke([&mock_open_request, primary] () {
                                       if (!primary) {
                                       mock_open_request.tag_data->mirror_uuid =
                                       "remote mirror uuid";}
                                       }
                                ),
        ...
        void expect_shut_down_dispatch(MockImageCtx & mock_image_ctx) {
            EXPECT_CALL(*mock_image_ctx.io_object_dispatcher,
                        shut_down_dispatch(io::OBJECT_DISPATCH_LAYER_JOURNAL,
                                           _))
                .WillOnce(WithArg < 1 >
                          (CompleteContext
        ...
        void expect_get_max_append_size(::journal::
                                        MockJournaler & mock_journaler,
                                        uint32_t max_size) {
        ...
        void expect_get_journaler_tags(MockImageCtx & mock_image_ctx,::journal::
                                       MockJournaler & mock_journaler,
                                       uint64_t start_after_tag_tid,::journal::
                                       Journaler::Tags && tags, int r) {
        ...
        void expect_start_replay(MockJournalImageCtx & mock_image_ctx,::
            ...
            EXPECT_CALL(mock_journaler, start_replay(_))
                .WillOnce(DoAll(SaveArg < 0 > (&m_replay_handler),
                                StartReplay(mock_image_ctx.image_ctx->
                                            op_work_queue,
                                            new
                                            C_ReplayAction(&m_replay_handler,
                                                           action))));
        }
        ...
        void expect_shut_down_replay(MockJournalImageCtx & mock_image_ctx,
                                     ...
                                     int r, bool cancel_ops = false) {
            EXPECT_CALL(mock_journal_replay, shut_down(cancel_ops, _))
                .WillOnce(WithArg < 1 >
                          (Invoke
                           ([this, &mock_image_ctx, r] (Context * on_flush) {
        ...
        void expect_try_pop_front(MockJournalImageCtx & mock_image_ctx,::
                                  journal::MockJournaler & mock_journaler,
                                  bool entries_available,::journal::
            ...
            EXPECT_CALL(mock_journal_replay, process(_, _, _))
                ...
                WillOnce(DoAll
                         (WithArg < 1 >
                          ...
                          WithArg < 2 >
                          (Invoke
                           (this, &TestMockJournal::save_commit_context))));
        }
        ...
        void expect_set_append_batch_options(MockJournalImageCtx &
                                             mock_image_ctx,::journal::
                                             MockJournaler & mock_journaler,
                                             bool user_flushed) {
        ...
        void expect_stop_append(::journal::MockJournaler & mock_journaler,
                                int r) {
            EXPECT_CALL(mock_journaler, stop_append(_))
                .
                WillOnce(CompleteContext
                         (r, static_cast < asio::ContextWQ * >(NULL)));
        }
        ...
        void expect_committed(::journal::MockJournaler & mock_journaler,
            ...
            EXPECT_CALL(mock_journaler,
                        committed(MatcherCast <
                                  const::journal::MockReplayEntryProxy & >(_)))
            .Times(events);
        }
        ...
        void expect_future_committed(::journal::MockJournaler & mock_journaler) {
            EXPECT_CALL(mock_journaler,
                        committed(MatcherCast <
                                  const::journal::MockFutureProxy & >(_)));
        }
        ...
        uint64_t when_append_write_event(MockJournalImageCtx & mock_image_ctx,
            ...
            mock_image_ctx.owner_lock};
            return mock_journal->append_write_event(0, length, bl, false);
        }

        uint64_t when_append_compare_and_write_event(MockJournalImageCtx &
                                                     mock_image_ctx,
                                                     MockJournal * mock_journal,
                                                     uint64_t length) {
            bufferlist cmp_bl;
            ...
            std::shared_lock owner_locker {
            mock_image_ctx.owner_lock};
            return mock_journal->append_compare_and_write_event(0, length,
                                                                cmp_bl,
                                                                write_bl,
                                                                false);
        }
        ...
        uint64_t when_append_io_event(MockJournalImageCtx & mock_image_ctx,
            ...
            std::shared_lock owner_locker {
            mock_image_ctx.owner_lock};
            return mock_journal->append_io_event(journal::EventEntry {
                                                 journal::AioFlushEvent {
                                                 }
                                                 }
                                                 , 0, 0, false, filter_ret_val);
        }
        ...
        void commit_replay(MockJournalImageCtx & mock_image_ctx,
        ...
        void open_journal(MockJournalImageCtx & mock_image_ctx,
                          MockJournal * mock_journal,
                          MockObjectDispatch & mock_object_dispatch,::journal::
                          MockJournaler & mock_journaler,
                          MockJournalOpenRequest & mock_open_request,
                          bool primary = true) {
            ...
            expect_open_journaler(mock_image_ctx, mock_journaler,
                                  mock_open_request, primary, 0);
            expect_get_max_append_size(mock_journaler, 1 << 16);
            expect_start_replay(mock_image_ctx, mock_journaler,
            ...
            expect_set_append_batch_options(mock_image_ctx, mock_journaler,
                                            false);
        ...
        static void invoke_replay_complete(::journal::ReplayHandler * handler,
                                           int r) {
    ...
    TEST_F(TestMockJournal, StateTransitions) {
        ...
        BOOST_SCOPE_EXIT(&mock_journal) {
            mock_journal->put();
        } BOOST_SCOPE_EXIT_END InSequence seq;
        ...
        MockJournalReplay mock_journal_replay;
        expect_try_pop_front(mock_image_ctx, mock_journaler, true,
                             mock_replay_entry);
        ...
        expect_try_pop_front(mock_image_ctx, mock_journaler, false,
                             mock_replay_entry, std::bind(&invoke_replay_ready,
                                                          _1));
        expect_try_pop_front(mock_image_ctx, mock_journaler, true,
                             mock_replay_entry);
        expect_replay_process(mock_journal_replay);
    ...
    TEST_F(TestMockJournal, CorruptEntry) {
        ...
        expect_try_pop_front(mock_image_ctx, mock_journaler, true,
                             mock_replay_entry);
        EXPECT_CALL(mock_journal_replay,
                    decode(_, _)).WillOnce(Return(-EBADMSG));
        expect_stop_replay(mock_journaler);
        expect_shut_down_replay(mock_image_ctx, mock_journal_replay, 0, true);
        expect_flush_commit_position(mock_journaler);
        expect_shut_down_journaler(mock_journaler);
    ...
    TEST_F(TestMockJournal, ReplayOnDiskPreFlushError) {
        ...
        // wait for the process callback
        {
            ...
            m_cond.wait(locker,[this] {
                        return !m_commit_contexts.empty();
                        }
            );
        }
        on_ready->complete(0);
    ...
    TEST_F(TestMockJournal, AppendWriteEvent) {
        ...
        ASSERT_EQ(1U,
                  when_append_write_event(mock_image_ctx, mock_journal,
                                          1 << 17));
        mock_journal->get_work_queue()->drain();
    ...
    TEST_F(TestMockJournal, IOCommitErrorFiltered) {
        ...
        expect_wait_future(mock_future, &on_journal_safe);
        ASSERT_EQ(1U,
                  when_append_io_event(mock_image_ctx, mock_journal, -EILSEQ));
        mock_journal->get_work_queue()->drain();
    ...
    TEST_F(TestMockJournal, AppendDisabled) {
        ...
        mock_image_ctx.image_lock};
        EXPECT_CALL(mock_image_ctx,
                    get_journal_policy()).WillOnce(Return(ictx->
                                                          get_journal_policy
                                                          ()));
    ...
    TEST_F(TestMockJournal, CloseListenerEvent) {
        ...
        BOOST_SCOPE_EXIT_END MockObjectDispatch mock_object_dispatch;
        ::journal::MockJournaler mock_journaler;
        MockJournalOpenRequest mock_open_request;
        open_journal(mock_image_ctx, mock_journal, mock_object_dispatch,
                     mock_journaler, mock_open_request);
    ...
    TEST_F(TestMockJournal, ResyncRequested) {
        ...
        expect_get_journaler_cached_client(mock_journaler, image_client_meta,
                                           0);
        expect_shut_down_journaler(mock_journaler);
