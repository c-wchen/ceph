...
class C_Drop_Cache:public MDSInternalContext {
  ...
  class C_ContextTimeout:public MDSInternalContext {
      ...
      :    
        MDSInternalContext(mds),
        timeout(timeout),
        lock("mds::context::timeout", false, true), on_finish(on_finish) {
        }
        ~C_ContextTimeout() {
    ...
    void recall_client_state() {
        ...
        auto result =
            server->recall_client_state(gather, Server::RecallFlags::STEADY);
...
MDSRank::MDSRank(mds_rank_t whoami_,
                 Mutex & mds_lock_,
                 LogChannelRef & clog_,
                 SafeTimer & timer_,
                 Beacon & beacon_,
                 ...
                 Messenger * msgr,
                 MonClient * monc_,
                 Context * respawn_hook_, Context * suicide_hook_)
:
...
mdsmap_->get_metadata_pool(), objecter, new FunctionContext([this] (int r)
                                           {
    ...
    , progress_thread(this), dispatch_depth(0),
        ...
        standby_replaying(false), starttime(mono_clock::now()) {
...
void MDSRank::update_targets(utime_t now)
{
    ...
    while (it != export_targets.end()) {
        ...
        dout(20) << "export target mds." << rank << " value is " << val << " @ "
            << now << dendl;
    ...
    if (send) {
        dout(15) << "updating export_targets, now " << new_map_targets.
            size() << " ranks are targets" << dendl;
        MMDSLoadTargets *m =
            new MMDSLoadTargets(mds_gid_t(monc->get_global_id()),
                                new_map_targets);
...
void MDSRank::hit_export_target(utime_t now, mds_rank_t rank, double amount)
{
    ...
    auto em =
        export_targets.emplace(std::piecewise_construct,
                               ...
                               std::forward_as_tuple(now, DecayRate(rate)));
    if (em.second) {
        dout(15) << "hit export target (new) " << amount << " @ " << now <<
            dendl;
    }
    else {
...
bool MDSRank::is_stale_message(Message * m) const const
{
    // from bad mds?
        ...
        if (session->is_closed()) {
            ...
            if (imported_session && imported_session != session) {
                ...
                while (!session->preopen_out_queue.empty()) {
                    imported_session->connection->send_message(session->
                                                               preopen_out_queue.
                                                               front());
...
void MDSRank::forward_message_mds(Message * m, mds_rank_t mds)
{
    ...
    if (m->get_type() == CEPH_MSG_CLIENT_REQUEST &&
        ...
        messenger->
            send_message(new
                         MClientRequestForward(creq->get_tid(), mds,
                                               creq->get_num_fwd(),
                                               client_must_resend),
...
void MDSRank::replay_done()
{
    ...
    if (is_standby_replay()) {
        ...
        assert(mdlog->get_journaler()->get_read_pos() ==
...
void MDSRank::reconnect_start()
{
    ...
    objecter->with_osdmap([this, &blacklist, &epoch] (const OSDMap & o) {
                          o.get_blacklist(&blacklist); epoch = o.get_epoch();});
...
void MDSRank::stopping_start()
{
    ...
    if (mdsmap->get_num_in_mds() == 1 && !sessionmap.empty()) {
        ...
        dout(0) <<
            "we are the last MDS, and have mounted clients: we cannot flush our journal.  suicide!"
            << dendl;
        suicide();
    }
...
void MDSRankDispatcher::handle_mds_map(MMDSMap * m, MDSMap * oldmap)
{
    ...
    {
        map < epoch_t, list < MDSInternalContextBase * > >::iterator p =
    ...
    if (is_active()) {
        ...
        for (map < mds_gid_t, MDSMap::mds_info_t >::const_iterator p =
             mdsmap->get_mds_info().begin(); p != mdsmap->get_mds_info().end();
             ++p) {
            if (p->second.state == MDSMap::STATE_STANDBY_REPLAY
                && (p->second.standby_for_rank == whoami
                    || (info.name.length()
                        && p->second.standby_for_name == info.name))) {
                found = true;
                break;
            }
            if (found)
                                            ...
                                            std::ostream & ss)
{
	...
	Please enable \"osd_enable_op_tracker\", and the tracker will start to track new ops received afterwards.";
	  ...
	  please enable \"osd_enable_op_tracker\", and the tracker will start to track new ops received afterwards.";
        ...
        bool got_val =
            cmd_getval(g_ceph_context, cmdmap, "target_epoch", target_epoch);
        ...
        if (!already_got) {
            dout(4) << __func__ << ": waiting for OSD epoch " << target_epoch <<
                dendl;
            cond.wait();
        }
    }
...
class C_MDS_Send_Command_Reply:public MDSInternalContext {
  protected:
    MCommand * m;
  public:
    C_MDS_Send_Command_Reply(MDSRank * _mds,
                             MCommand * _m):MDSInternalContext(_mds), m(_m) {
        m->get();
                                      ...
                                      Formatter * f) const const
{
    ...
    const ceph::unordered_map < entity_name_t, Session * >session_map =
        sessionmap.get_sessions();
    for (ceph::unordered_map < entity_name_t, Session * >::const_iterator p =
         session_map.begin(); p != session_map.end(); ++p) {
        if (!p->first.is_client()) {
            continue;
        }
...
void MDSRank::command_scrub_path(Formatter * f, boost::string_view path,
                                 vector < string > &scrubop_vec)
{
    bool force = false;
    bool recursive = false;
    bool repair = false;
    for (vector < string >::iterator i = scrubop_vec.begin();
         i != scrubop_vec.end(); ++i) {
        if (*i == "force")
            force = true;
        else if (*i == "recursive")
...
void MDSRank::command_flush_journal(Formatter * f)
{
...

    {
        ...
        C_Flush_Journal *flush_journal =
            new C_Flush_Journal(mdcache, mdlog, this, &ss, &cond);
        flush_journal->send();
    }
    int r = cond.wait();
...
void MDSRank::command_get_subtrees(Formatter * f)
{
    ...
    for (std::list < CDir * >::iterator i = subtrees.begin();
         i != subtrees.end(); ++i) {
        const CDir *dir = *i;
...
void MDSRank::command_export_dir(Formatter * f,
                                 boost::string_view path, mds_rank_t target)
{
...
int MDSRank::_command_export_dir(boost::string_view path, mds_rank_t target)
{
    Mutex::Locker l(mds_lock);
    filepath fp(path);
...
bool MDSRank::evict_client(int64_t session_id,
                           bool wait, bool blacklist,
                           std::stringstream & err_ss, Context * on_killed)
{
    assert(mds_lock.is_locked_by_me());
    ...
    auto kill_client_session =[this, session_id, wait, on_killed] (){
        ...
        if (session) {
            ...
            dout(1) << "session " << session_id <<
                " was removed while we waited " "for blacklist" << dendl;
...
:                                                       MDSRank(whoami_,
                                                         ...
                                                         *r =
                                                         filter.
                                                         parse(filter_args, ss);
                                                         if (*r != 0) {
                                                         return true;}

                                                         JSONFormatter f(true);
                                                         dump_sessions(filter,
                                                                       &f);
                                                         f.flush(*ds);
                                                         return true;}
                                                         else
                                                         ...
                                                         *run_later =
                                                         new C_OnFinisher(new
                                                                          FunctionContext
                                                                          ([this, timeout, f, on_finish] (int _) {
                                                                           command_cache_drop
                                                                           ((uint64_t) timeout, f, on_finish);}), finisher); return true;}
                                                         else {
                                                         return false;}
                                                         }
