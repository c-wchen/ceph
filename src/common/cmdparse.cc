...
void dump_cmd_and_help_to_json(Formatter *jf,
                               uint64_t features,
                               const string &secname,
                               const string &cmdsig, const string &helptext)
{
...
void dump_cmddesc_to_json(Formatter *jf,
                          uint64_t features,
                          const string &secname,
                          const string &cmdsig,
                          const string &helptext,
                          const string &module,
                          const string &perm, uint64_t flags)
{
    jf->open_object_section(secname);
    jf->open_array_section("sig");
    dump_cmd_to_json(jf, features, cmdsig);
...
bool cmdmap_from_json(const vector < string > &cmd, cmdmap_t *mapp,
                      std::ostream &ss)
{
    ...
    try {
        ...
        for (auto it = o.begin(); it != o.end(); ++it) {
            ...
            switch (it->second.type()) {
                ...
                default:
                    throw std::runtime_error("JSON array/object not allowed " +
                                             fullcmd);
                    break;
                ...
                case json_spirit::array_type: {
                    ...
                    const vector < json_spirit::mValue > &spvals =
                        it->second.get_array();
                    if (spvals.empty()) {
                        ...
                        for (const auto &sv : spvals) {
                            if (sv.type() != json_spirit::str_type) {
                                throw std::
                                runtime_error
                                ("Can't handle arrays of multiple types");
                            }
                        ...
                        (*mapp)[it->first] = std::move(outv);
                    } else if (spvals.front().type() ==
                               json_spirit::real_type) {
                        vector < double >outv;
                        for (const auto &sv : spvals) {
                            if (spvals.front().type() !=
                                json_spirit::real_type) {
...
bool find_first_in(std::string_view s, const char *delims, Func
                   && f)
{
...
template < typename T >
bool arg_in_range(T value, const arg_desc_t &desc,
                  std::ostream &os)
{
...
bool validate_str_arg(std::string_view value,
                      ...
                      const arg_desc_t &desc, std::ostream &os)
{
    if (type == "CephIPAddr") {
        entity_addr_t addr;
        if (addr.parse(value)) {
                  ...
                  const std::string_view type, std::ostream &os)
{
    ...
    auto validate = [&](const T & value) {
        if constexpr
        (is_same_v < std::string, T >) {
            return
                validate_str_arg(value,
                                 type, desc,
                                 os);
                            ...
                            < double,
                            T >) {
            return arg_in_range(value,
                                desc,
                                os);
        }
    };
    if constexpr(is_vector) {
        return find_if_not(begin(v),
                           end(v),
                           validate)
               == end(v);
    } else {
