...
namespace librbd {
    ...
    namespace api {
        ...
        Namespace < librbd::MockTestImageCtx > *Namespace <
...
namespace rbd {
    namespace mirror {
        ...
        template <> struct NamespaceReplayer <librbd::MockTestImageCtx > {
            ...
            MOCK_METHOD1(handle_instances_added,
                         void (const std::vector < std::string > &));
            MOCK_METHOD1(handle_instances_removed,
                         void (const std::vector < std::string > &));
        ...
        std::map < std::string,
            NamespaceReplayer < librbd::MockTestImageCtx > *>NamespaceReplayer <
        ...
        template <> struct LeaderWatcher <librbd::MockTestImageCtx > {
            ...
            static LeaderWatcher *create(Threads < librbd::MockTestImageCtx >
                                         *threads, librados::IoCtx & ioctx,
        ...
        LeaderWatcher < librbd::MockTestImageCtx > *LeaderWatcher <
            librbd::MockTestImageCtx >::s_instance = nullptr;
        ...
        template <> struct RemotePoolPoller <librbd::MockTestImageCtx > {
            ...
            static RemotePoolPoller *create(Threads < librbd::MockTestImageCtx >
                                            *threads,
...
namespace rbd {
    namespace mirror {
        ...
        class TestMockPoolReplayer:public TestMockFixture {
          public:
            typedef librbd::api::Namespace < librbd::MockTestImageCtx >
                MockNamespace;
            typedef PoolReplayer < librbd::MockTestImageCtx > MockPoolReplayer;
            typedef Throttler < librbd::MockTestImageCtx > MockThrottler;
            typedef NamespaceReplayer < librbd::MockTestImageCtx >
                MockNamespaceReplayer;
            typedef RemotePoolPoller < librbd::MockTestImageCtx >
                MockRemotePoolPoller;
            typedef LeaderWatcher < librbd::MockTestImageCtx >
                MockLeaderWatcher;
            typedef ServiceDaemon < librbd::MockTestImageCtx >
                MockServiceDaemon;
            ...
            void expect_work_queue(MockThreads & mock_threads) {
                ...
                EXPECT_CALL(mock_cluster, create_rados_client(_))
                    .
                    WillOnce(Invoke
                             ([cluster_name, mock_rados_client,
                               cct_ref] (CephContext * cct) {
                              EXPECT_EQ(cluster_name, cct->_conf->cluster);
                              if (cct_ref != nullptr) {
                              cct->get();
                              *cct_ref = cct;} return mock_rados_client;}
                             )) ;
            }

            void expect_create_ioctx(librados::MockTestMemRadosClient *
                                     mock_rados_client,
                                     librados::MockTestMemIoCtxImpl *
                                     mock_io_ctx_impl) {
            ...
            void expect_mirror_uuid_get(librados::MockTestMemIoCtxImpl *
                                        io_ctx_impl, const std::string & uuid,
                                        int r) {
                bufferlist out_bl;
                encode(uuid, out_bl);
                ...
                EXPECT_CALL(*io_ctx_impl,
                    ...
                    .
                    WillOnce(DoAll
                             (WithArg < 5 > (Invoke([out_bl] (bufferlist * bl) {
                                                    *bl =
                                                    out_bl;})), Return(r)));
            }

            void expect_mirror_mode_get(librados::MockTestMemIoCtxImpl *
                                        io_ctx_impl,
            ...
            void expect_mirror_mode_get(librados::MockTestMemIoCtxImpl *
                ...
                EXPECT_CALL(*io_ctx_impl,
                    ...
                    WillRepeatedly(DoAll
                                   (WithArg < 5 > (Invoke([](bufferlist * bl) {
                                                          encode(cls::rbd::
                                                                 MIRROR_MODE_POOL,
                                                                 *bl);})),
                                    Return(0)));
            ...
            void expect_leader_watcher_init(MockLeaderWatcher &
                                            mock_leader_watcher, int r) {
                EXPECT_CALL(mock_leader_watcher, init())
                    .WillOnce(Return(r));
            }

            void expect_leader_watcher_shut_down(MockLeaderWatcher &
                                                 mock_leader_watcher) {
                EXPECT_CALL(mock_leader_watcher, shut_down());
            }

            void expect_leader_watcher_get_leader_instance_id(MockLeaderWatcher
                                                              &
                                                              mock_leader_watcher)
            {
            ...
            void expect_leader_watcher_list_instances(MockLeaderWatcher &
                                                      mock_leader_watcher) {
                EXPECT_CALL(mock_leader_watcher, list_instances(_))
                    .Times(AtLeast(0));
            }

            void expect_remote_pool_poller_init(MockRemotePoolPoller &
                                                mock_remote_pool_poller,
                                                const RemotePoolMeta &
                                                remote_pool_meta, int r) {
                EXPECT_CALL(mock_remote_pool_poller, init(_))
                    .WillOnce(Invoke([this, &mock_remote_pool_poller,
                                      remote_pool_meta, r]
                                     (Context * ctx) {
                                     if (r >= 0) {
                                     mock_remote_pool_poller.listener->
                                     handle_updated(remote_pool_meta);}

                                     m_threads->work_queue->queue(ctx, r);}
                              )) ;
            }

            void expect_remote_pool_poller_shut_down(MockRemotePoolPoller &
                                                     mock_remote_pool_poller,
                                                     int r) {
            ...
            void expect_leader_watcher_is_blocklisted(MockLeaderWatcher &
                                                      mock_leader_watcher,
                                                      bool blocklisted) {
            ...
            void expect_namespace_replayer_is_blocklisted(MockNamespaceReplayer
                                                          &
            ...
            void expect_namespace_replayer_get_instance_id(MockNamespaceReplayer
                                                           &
                                                           mock_namespace_replayer,
                                                           const std::
                                                           string & instance_id)
            ...
            void expect_namespace_replayer_init(MockNamespaceReplayer &
                                                mock_namespace_replayer, int r,
            ...
            void expect_namespace_replayer_shut_down(MockNamespaceReplayer &
                                                     mock_namespace_replayer,
                                                     Context * on_shut_down =
                                                     nullptr) {
            ...
            void expect_namespace_replayer_handle_acquire_leader
                ...
                EXPECT_CALL(mock_namespace_replayer, handle_acquire_leader(_))
                    .WillOnce(Invoke([this, r, on_acquire] (Context * ctx) {
                                     ...
                                     m_threads->work_queue->queue(on_acquire,
                                                                  r);}
                                     }
                              )) ;
            }
            ...
            void expect_namespace_replayer_handle_release_leader
                ...
                EXPECT_CALL(mock_namespace_replayer, handle_release_leader(_))
                    .WillOnce(Invoke([this, r, on_release] (Context * ctx) {
                                     ...
                                     m_threads->work_queue->queue(on_release,
                                                                  r);}
                                     }
                              )) ;
            }
            ...
            void expect_namespace_replayer_handle_update_leader
                (MockNamespaceReplayer & mock_namespace_replayer,
                 const std::string & leader_instance_id, Context * on_update =
                 nullptr) {
            ...
            void expect_service_daemon_add_namespace(MockServiceDaemon &
                                                     mock_service_daemon,
                                                     const std::
                                                     string & namespace_name) {
                EXPECT_CALL(mock_service_daemon,
                            add_namespace(m_local_io_ctx.get_id(),
                                          namespace_name));
            }

            void expect_service_daemon_remove_namespace(MockServiceDaemon &
                                                        mock_service_daemon,
                                                        const std::
                                                        string & namespace_name)
            {
                EXPECT_CALL(mock_service_daemon,
                            remove_namespace(m_local_io_ctx.get_id(),
                                             namespace_name));
            }

            void expect_service_daemon_add_or_update_attribute(MockServiceDaemon
                                                               &
                                                               mock_service_daemon,
                                                               const std::
                                                               string & key,
                                                               ...
                                                               value) {
                EXPECT_CALL(mock_service_daemon,
                            add_or_update_attribute(_, key, value));
            }

            void expect_service_daemon_remove_attribute(MockServiceDaemon &
                                                        mock_service_daemon,
                                                        const std::
                                                        string & key) {
        ...
        TEST_F(TestMockPoolReplayer, ConfigKeyOverride) {
            ...
            expect_connect(mock_cluster, mock_local_rados_client, "ceph",
                           nullptr);

            auto mock_remote_rados_client =
                mock_cluster.do_create_rados_client(g_ceph_context);
            CephContext *remote_cct = nullptr;
            expect_connect(mock_cluster, mock_remote_rados_client,
                           "cluster name", &remote_cct);

            auto mock_local_io_ctx =
                mock_local_rados_client->do_create_ioctx(m_local_io_ctx.
                                                         get_id(),
                                                         m_local_io_ctx.
                                                         get_pool_name());
            ...
            std::string instance_id =
                stringify(mock_local_io_ctx->get_instance_id());
            ...
            ASSERT_EQ("123",
                      remote_cct->_conf.get_val < std::string > ("mon_host"));
        ...
        TEST_F(TestMockPoolReplayer, Namespaces) {
            ...
            g_ceph_context->_conf.
                set_val("rbd_mirror_pool_replayers_refresh_interval", "1");
            ...
            expect_namespace_replayer_is_blocklisted
                (*mock_ns1_namespace_replayer, false);

            auto mock_ns2_namespace_replayer = new MockNamespaceReplayer("ns2");
            expect_namespace_replayer_is_blocklisted
                (*mock_ns2_namespace_replayer, false);
        ...
        TEST_F(TestMockPoolReplayer, NamespacesError) {
            ...
            C_SaferCond on_ns1_init;
            Context *ctx = new LambdaContext([&mock_namespace,
                                              &on_ns1_init] (int r){
                                             mock_namespace.remove("ns1");
                                             on_ns1_init.complete(r);});
            expect_namespace_replayer_init(*mock_ns1_namespace_replayer,
                                           -EINVAL, ctx);
            mock_namespace.add("ns1");
            ...
            ctx = new LambdaContext([&mock_namespace] (int) {
                                    mock_namespace.remove("ns2");});
            expect_service_daemon_remove_namespace(mock_service_daemon, "ns2");
            expect_namespace_replayer_shut_down(*mock_ns2_namespace_replayer,
                                                ctx);
            ...
            expect_namespace_replayer_shut_down(*mock_ns3_namespace_replayer,
                                                ctx);
