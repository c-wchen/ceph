...
ostream & Pipe::_pipe_prefix(std::ostream & out) const const
{
    return out << "-- " << msgr->get_myinst().
        addr << " >> " << peer_addr << " pipe(" << this << " sd=" << sd << " :"
        << port << " s=" << state << " pgs=" << peer_global_seq << " cs=" <<
        connect_seq << " l=" << policy.
        lossy << " c=" << connection_state << ").";
}
...
class Pipe::DelayedDelivery:public Thread {
  ...
  public:
    ...
    :pipe(p),
        ...
        delay_dispatching(false), stop_fast_dispatching_flag(false) {
...
connect_seq(0), peer_global_seq(0), out_seq(0), in_seq(0), in_seq_acked(0)
{
    ...
    if (randomize_out_seq()) {
        lsubdout(msgr->cct, ms,
                 15) <<
            "Pipe(): Could not get random bytes to set seq number for session reset; set seq number to "
            << out_seq << dendl;
    }
...
void Pipe::start_reader()
{
    ...
    reader_running = true;
    reader_thread.create("ms_pipe_read",
                         msgr->cct->_conf->ms_rwthread_stack_bytes);
}
...
void Pipe::maybe_start_delay_thread()
{
    if (!delay_thread) {
        auto pos =
            msgr->cct->_conf->get_val < std::string >
...
void Pipe::start_writer()
{
    ...
    writer_running = true;
    writer_thread.create("ms_pipe_write",
                         msgr->cct->_conf->ms_rwthread_stack_bytes);
}
...
{
    lgeneric_subdout(pipe->msgr->cct, ms,
                     20) << *pipe << "DelayedDelivery::discard" << dendl;
    Mutex::Locker l(delay_lock);
...
{
    lgeneric_subdout(pipe->msgr->cct, ms,
                     20) << *pipe << "DelayedDelivery::flush" << dendl;
    Mutex::Locker l(delay_lock);
...
void *Pipe::DelayedDelivery::entry()
{
    ...
    lgeneric_subdout(pipe->msgr->cct, ms,
                     20) << *pipe << "DelayedDelivery::entry start" << dendl;

    while (!stop_delayed_delivery) {
        if (delay_queue.empty()) {
            lgeneric_subdout(pipe->msgr->cct, ms,
                             30) << *pipe <<
                "DelayedDelivery::entry sleeping on delay_cond because delay queue is empty"
                << dendl;
            delay_cond.Wait(delay_lock);
            continue;
        }
        ...
        string delay_msg_type =
            pipe->msgr->cct->_conf->ms_inject_delay_msg_type;
        if (!flush_count
            && (release > ceph_clock_now()
                && (delay_msg_type.empty()
                    || m->get_type_name() == delay_msg_type))) {
            lgeneric_subdout(pipe->msgr->cct, ms,
                             10) << *pipe <<
                "DelayedDelivery::entry sleeping on delay_cond until " <<
                release << dendl;
        ...
        lgeneric_subdout(pipe->msgr->cct, ms,
            ...
            " for delivery, past " << release << dendl;
        delay_queue.pop_front();
        if (flush_count > 0) {
            --flush_count;
            active_flush = true;
    ...
    lgeneric_subdout(pipe->msgr->cct, ms,
                     20) << *pipe << "DelayedDelivery::entry stop" << dendl;
    return NULL;
}
...
int Pipe::accept()
{
    ...
    try {
        ...
        ldout(msgr->cct,
              2) << __func__ << " decode peer_addr failed: " << e.what()
            << dendl;
        goto fail_unlocked;
    }
    ...
    while (1) {
        ...
        ldout(msgr->cct,
            ...
            connect_seq << " global_seq " << connect.global_seq << dendl;
        ...
        if (feat_missing) {
            ldout(msgr->cct,
                  1) << "peer missing required features " << std::
        ...
        authorizer_reply.clear();
        if (!msgr->
            verify_authorizer(connection_state.get(), peer_type,
	 ...
	 */
            ...
            if (connect.global_seq < existing->peer_global_seq) {
                ldout(msgr->cct,
                      10) << "accept existing " << existing << ".gseq " <<
            ...
            if (connect.connect_seq < existing->connect_seq) {
                ...
                ldout(msgr->cct,
                      10) << "accept existing " << existing << ".cseq " <<
            ...
            if (connect.connect_seq == existing->connect_seq) {
                ...
                // connection race?
                if (peer_addr < msgr->my_inst.addr || existing->policy.server) {
                    // incoming wins
                    ldout(msgr->cct,
                          10) << "accept connection race, existing " << existing
                        << ".cseq " << existing->
      ...
      reply:
        assert(pipe_lock.is_locked());
        reply.features =
            ((uint64_t) connect.features & policy.features_supported) | policy.
            features_required;
  ...
  replace:
    ...
    if (existing->policy.lossy) {
        ...
        ldout(msgr->cct,
              10) << "accept re-queuing on out_seq " << out_seq << " in_seq " <<
            in_seq << dendl;
        for (map < int, list < Message * > >::iterator p =
             existing->out_q.begin(); p != existing->out_q.end(); ++p)
  ...
  open:
    ...
    connection_state->set_features((uint64_t) reply.
                                   features & (uint64_t) connect.features);
    ldout(msgr->cct,
          10) << "accept features " << connection_state->
        get_features() << dendl;

    session_security.reset(get_auth_session_handler(msgr->cct,
    ...
    if (state != STATE_CLOSED) {
        ldout(msgr->cct,
              10) << "accept starting writer, state " << get_state_name() <<
            dendl;
        start_writer();
    }
  ...
  fail_registered:
    ...
    if (msgr->cct->_conf->ms_inject_internal_delays) {
        ldout(msgr->cct,
              10) << " sleep for " << msgr->cct->_conf->
            ms_inject_internal_delays << dendl;
        utime_t t;
...
void Pipe::set_socket_options()
{
    ...
    if (msgr->cct->_conf->ms_tcp_nodelay) {
        int flag = 1;
        int r =::setsockopt(sd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag,
                            sizeof(flag));
        if (r < 0) {
            r = -errno;
    ...
    if (msgr->cct->_conf->ms_tcp_rcvbuf) {
        ...
        int r =::setsockopt(sd, SOL_SOCKET, SO_RCVBUF, (void *)&size,
                            sizeof(size));
        if (r < 0) {
            r = -errno;
    ...
    int val = 1;
    int r =::setsockopt(sd, SOL_SOCKET, SO_NOSIGPIPE, (void *)&val,
                        sizeof(val));
    if (r) {
        r = -errno;
    ...
    if (prio >= 0) {
        ...
        case AF_INET:
            ...
            break;
        case AF_INET6:
            r =::setsockopt(sd, IPPROTO_IPV6, IPV6_TCLASS, &iptos,
                            sizeof(iptos));
            break;
        default:
            lderr(msgr->cct) << "couldn't set ToS of unknown family ("
                << addr_family << ")" << " to " << iptos << dendl;
            return;
        }
        if (r < 0) {
            r = -errno;
...
int Pipe::connect()
{
    ...
    if (sd < 0) {
        int e = errno;
        lderr(msgr->
              cct) << "connect couldn't create socket " << cpp_strerror(e) <<
            dendl;
        rc = -e;
        goto fail;
    }
...

    {
        ...
        if (msgr->cct->_conf->ms_bind_before_connect
            && (!addr2bind.is_blank_ip())) {
            addr2bind.set_port(0);
            int r =::bind(sd, addr2bind.get_sockaddr(),
                          addr2bind.get_sockaddr_len());
            if (r < 0) {
                ldout(msgr->cct,
    ...
    try {
        ...
        ldout(msgr->cct,
              2) << "connect couldn't decode peer addrs: " << e.what()
            << dendl;
        goto fail;
    }
    ...
    ldout(msgr->cct,
          20) << "connect read peer addr " << paddr << " on socket " << sd <<
        dendl;
    if (peer_addr != paddr) {
    ...
    ldout(msgr->cct,
          20) << "connect peer addr for me is " << peer_addr_for_me << dendl;
    ...
    ldout(msgr->cct,
          10) << "connect sent my addr " << msgr->my_inst.addr << dendl;
    ...
    while (1) {
        ...
        if (authorizer)
            ldout(msgr->cct,
                ...
                authorizer_len << " protocol=" << connect.
        ...
        ldout(msgr->cct,
              10) << "connect sending gseq=" << gseq << " cseq=" << cseq <<
            " proto=" << connect.protocol_version << dendl;
        rc = do_sendmsg(&msg, msglen);
        if (rc < 0) {
            ldout(msgr->cct,
                  2) << "connect couldn't write gseq, cseq, " <<
        ...
        if (rc < 0) {
            ldout(msgr->cct,
                  2) << "connect read reply " << cpp_strerror(rc) << dendl;
            goto fail;
        }
        ...
        if (state != STATE_CONNECTING) {
            ldout(msgr->cct,
                  0) << "connect got RESETSESSION but no longer connecting" <<
                dendl;
            goto stop_locked;
        }
        ...
        if (reply.tag == CEPH_MSGR_TAG_FEATURES) {
            ldout(msgr->cct,
                  0) << "connect protocol feature mismatch, my " << std::
                hex << connect.features << " < peer " << reply.
        ...
        if (reply.tag == CEPH_MSGR_TAG_BADPROTOVER) {
            ldout(msgr->cct,
                ...
                protocol_version << " != " << reply.protocol_version << dendl;
            goto fail_locked;
        }
        ...
        if (reply.tag == CEPH_MSGR_TAG_RETRY_GLOBAL) {
            ...
            ldout(msgr->cct,
                ...
                global_seq << " chose new " << gseq << dendl;
            pipe_lock.Unlock();
            continue;
        }
        ...
        if (reply.tag == CEPH_MSGR_TAG_READY || reply.tag == CEPH_MSGR_TAG_SEQ) {
            uint64_t feat_missing =
                policy.features_required & ~(uint64_t) reply.features;
            if (feat_missing) {
                ldout(msgr->cct,
                      1) << "missing required features " << std::
                    hex << feat_missing << std::dec << dendl;
                goto fail_locked;
            }
            ...
            if (reply.tag == CEPH_MSGR_TAG_SEQ) {
                ldout(msgr->cct,
                    ...
                    << dendl;
                uint64_t newly_acked_seq = 0;
                rc = tcp_read((char *)&newly_acked_seq,
                              sizeof(newly_acked_seq));
                if (rc < 0) {
                    ldout(msgr->cct,
                          2) << "connect read error on newly_acked_seq" <<
                        cpp_strerror(rc) << dendl;
                    goto fail_locked;
                }
                ldout(msgr->cct,
                ...
                while (newly_acked_seq > out_seq) {
                    ...
                    assert(m);
                    ldout(msgr->cct,
                          2) << " discarding previously sent " << m->get_seq()
                        << " " << *m << dendl;
            ...
            backoff = utime_t();
            connection_state->set_features((uint64_t) reply.
                                           features & (uint64_t) connect.
                                           features);
            ldout(msgr->cct,
                  10) << "connect success " << connect_seq << ", lossy = " <<
                policy.lossy << ", features " << connection_state->
                get_features() << dendl;
...
{
    assert(msgr->lock.is_locked());
    ceph::unordered_map < entity_addr_t, Pipe * >::iterator p =
        msgr->rank_pipe.find(peer_addr);
...
void Pipe::requeue_sent()
{
    ...
    while (!sent.empty()) {
        ...
        sent.pop_back();
        ldout(msgr->cct,
              10) << "requeue_sent " << *m << " for resend seq " << out_seq <<
            " (" << m->get_seq() << ")" << dendl;
        rq.push_front(m);
        out_seq--;
    }
...
void Pipe::discard_requeued_up_to(uint64_t seq)
{
    ...
    while (!rq.empty()) {
        ...
        if (m->get_seq() == 0 || m->get_seq() > seq)
            break;
        ldout(msgr->cct,
              10) << "discard_requeued_up_to " << *m << " for resend seq " <<
            out_seq << " <= " << seq << ", discarding" << dendl;
        m->put();
        rq.pop_front();
        out_seq++;
    }
...
void Pipe::discard_out_queue()
{
    ...
    sent.clear();
    for (map < int, list < Message * > >::iterator p = out_q.begin();
         p != out_q.end(); ++p)
        for (list < Message * >::iterator r = p->second.begin();
             r != p->second.end(); ++r) {
...
void Pipe::was_session_reset()
{
    ...
    if (randomize_out_seq()) {
        lsubdout(msgr->cct, ms,
                 15) <<
            "was_session_reset(): Could not get random bytes to set seq number for session reset; set seq number to "
            << out_seq << dendl;
    }
...
void Pipe::stop_and_wait()
{
    ...
    while (reader_running && reader_dispatching)
        cond.Wait(pipe_lock);
}
...
void Pipe::reader()
{
    ...
    while (state != STATE_CLOSED && state != STATE_CONNECTING) {
        ...
        if (tag == CEPH_MSGR_TAG_ACK) {
            ...
            if (rc < 0) {
                ldout(msgr->cct,
                      2) << "reader couldn't read ack seq, " <<
                    cpp_strerror(errno) << dendl;
                fault(true);
            }
            ...
            if (state == STATE_CLOSED || state == STATE_CONNECTING) {
                in_q->dispatch_throttle_release(m->
            ...
            if (delay_thread) {
                utime_t release;
                if (rand() % 10000 <
                    msgr->cct->_conf->ms_inject_delay_probability * 10000.0) {
                ...
                if (in_q->can_fast_dispatch(m)) {
                    ...
                    if (state == STATE_CLOSED || notify_on_dispatch_done) { // there might be somebody waiting
    ...
    while (state != STATE_CLOSED) { // && state != STATE_WAIT) {
        ...
        if (state != STATE_CONNECTING && state != STATE_WAIT
            ...
            if (send_keepalive_ack) {
                ...
                if (rc < 0) {
                    ldout(msgr->cct,
                          2) << "writer couldn't write keepalive_ack, " <<
                        cpp_strerror(errno) << dendl;
                    fault();
                    continue;
                }
...
int Pipe::read_message(Message ** pm, AuthSessionHandler * auth_handler)
{
    ...
    ldout(msgr->cct, 20) << "reader got envelope type=" << header.type
        ...
        << " data=" << header.data_len << " off " << header.data_off << dendl;
    ...
    if (policy.throttler_messages) {
        ldout(msgr->cct,
            ...
            << policy.throttler_messages->get_current() << "/" << policy.
    ...
    if (message_size) {
        if (policy.throttler_bytes) {
            ldout(msgr->cct,
                ...
                " bytes from policy throttler " << policy.throttler_bytes->
                get_current() << "/" << policy.throttler_bytes->
                get_max() << dendl;
        ...
        ldout(msgr->cct,
              10) << "reader wants " << message_size <<
            " from dispatch throttler " << in_q->dispatch_throttler.
            get_current() << "/" << in_q->dispatch_throttler.get_max() << dendl;
    ...
    if (middle_len) {
        ...
        ldout(msgr->cct,
              20) << "reader got middle " << middle.length() << dendl;
    }
        ...
        while (left > 0) {
            ...
            if (p != connection_state->rx_buffers.end()) {
                if (rxbuf.length() == 0 || p->second.second != rxbuf_version) {
                    ldout(msgr->cct,
                          10) << "reader seleting rx buffer v " << p->second.
                        second << " at offset " << offset << " len " << p->
                        second.first.length() << dendl;
            ...
            ldout(msgr->cct,
                ...
                c_str() << " len " << bp.length() << dendl;
            ssize_t got = tcp_read_nonblocking(bp.c_str(), read);
            ldout(msgr->cct,
                  30) << "reader read " << got << " of " << read << dendl;
    ...
    if (aborted) {
        ldout(msgr->cct,
              0) << "reader got " << front.length() << " + " << middle.
            length() << " + " << data.length()
    ...
    ldout(msgr->cct,
          20) << "reader got " << front.length() << " + " << middle.
        length() << " + " << data.length()
        << " byte message" << dendl;
    ...
    if (message_size) {
        if (policy.throttler_bytes) {
            ldout(msgr->cct,
                ...
                " bytes to policy throttler " << policy.throttler_bytes->
                get_current() << "/" << policy.throttler_bytes->
                get_max() << dendl;
...
int Pipe::do_sendmsg(struct msghdr *msg, unsigned len, bool more)
{
    ...
    while (len > 0) {
        ...
        if (r < 0) {
            r = -errno;
            ldout(msgr->cct,
                  1) << "do_sendmsg error " << cpp_strerror(r) << dendl;
            return r;
        }
        if (state == STATE_CLOSED) {
            ldout(msgr->cct,
                  10) << "do_sendmsg oh look, state == CLOSED, giving up" <<
                dendl;
        ...
        ldout(msgr->cct,
              20) << "do_sendmsg short write did " << r << ", still have " <<
            len << dendl;
        while (r > 0) {
...
{
    ldout(msgr->cct,
          10) << "write_keepalive2 " << (int)tag << " " << t << dendl;
...
int Pipe::write_message(const ceph_msg_header & header,
                        const ceph_msg_footer & footer, bufferlist & blist)
{
    ...
    while (left > 0) {
        ...
        if (donow == 0) {
            ldout(msgr->cct,
                  0) << "donow = " << donow << " left " << left <<
                " pb->length " << pb->length()
                << " b_off " << b_off << dendl;
        }
        assert(donow > 0);
        ldout(msgr->cct, 30) << " bl_pos " << bl_pos << " b_off " << b_off
            << " leftinchunk " << left << " buffer len " << pb->length()
            << " writing " << donow << dendl;
    ...
    if (connection_state->has_feature(CEPH_FEATURE_MSG_AUTH)) {
        ...
        old_footer.data_crc =
            msgr->crcflags & MSG_CRC_DATA ? footer.data_crc : 0;
