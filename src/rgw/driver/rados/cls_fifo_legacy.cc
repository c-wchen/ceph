...
} int get_meta(const DoutPrefixProvider *dpp, lr::IoCtx &ioctx,
               const std::string &oid, std::optional < fifo::objv > objv,
               fifo::info *info, std::uint32_t *part_header_size,
               std::uint32_t *part_entry_overhead, uint64_t tid,
               optional_yield y, bool probe)
{
    ...
    if (r >= 0)
        ...
        ldpp_dout(dpp, -1)
                << __PRETTY_FUNCTION__ << ":" << __LINE__
                << " fifo::op::GET_META failed r=" << r << " tid=" << tid << dendl;
    }
    return r;
};
...
int push_part(const DoutPrefixProvider *dpp, lr::IoCtx &ioctx,
              ...
              optional_yield y)
{
    ...
    auto r =
        rgw_rados_operate(dpp, ioctx, oid, &op, y,
...
int list_part(const DoutPrefixProvider *dpp, lr::IoCtx &ioctx,
              const std::string &oid, std::uint64_t ofs,
...
struct list_entry_completion: public lr::ObjectOperationCompletion {
    ...
    list_entry_completion(CephContext *cct, int *r_out,
                          ...
                          *entries, bool *more, bool *full_part,
                          std::uint64_t tid)
                                  ...
                                  bool *full_part, std::uint64_t tid)
{
    ...
    op.exec(fifo::op::CLASS, fifo::op::LIST_PART, in,
            new list_entry_completion(cct, r_out, entries, more,
                                      full_part, tid));
    return op;
}

int get_part_info(const DoutPrefixProvider *dpp, lr::IoCtx &ioctx,
                  const std::string &oid, fifo::part_header *header,
                  std::uint64_t tid, optional_yield y)
{
    ...
    if (r >= 0)
        ...
        ldpp_dout(dpp, -1)
                << __PRETTY_FUNCTION__ << ":" << __LINE__
                << " fifo::op::GET_PART_INFO failed r=" << r << " tid=" <<
                tid << dendl;
    }
    return r;
}
...
struct partinfo_completion: public lr::ObjectOperationCompletion {
    ...
    fifo::part_header *h;
    std::uint64_t tid;
    partinfo_completion(CephContext *cct, int *rp,
                        fifo::part_header *h,
                        std::uint64_t tid): cct(cct), rp(rp), h(h),
        tid(tid)
    {
...
int FIFO::_update_meta(const DoutPrefixProvider *dpp,
...
struct Updater: public Completion < Updater > {
    ...
    std::uint64_t tid;
    Updater(const DoutPrefixProvider *dpp, FIFO *fifo,
            lr::AioCompletion *super, const fifo::update &update,
    ...
    {
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           << " handling async update_meta: tid=" << tid << dendl;
        if (r < 0 && r != -ECANCELED) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
        ...
        if (!canceled) {
            int r =
                fifo->apply_update(dpp, &fifo->info, version, update, tid);
            if (r < 0) {
                ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
    ...
    {
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           << " handling async read_meta: tid=" << tid << dendl;
        if (r < 0 && pcanceled) {
            *pcanceled = false;
...
void FIFO::_update_meta(const DoutPrefixProvider *dpp,
                        const fifo::update &update, fifo::objv version,
                        bool *pcanceled, std::uint64_t tid,
                        lr::AioCompletion *c)
{
    ...
    auto updater =
        std::make_unique < Updater > (dpp, this, c, update, version,
                                      pcanceled,
                                      tid);
...
int FIFO::create_part(const DoutPrefixProvider *dpp, int64_t part_num,
                      std::uint64_t tid, optional_yield y)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           << " part_init failed: r=" << r << " tid=" << tid << dendl;
    }
    return r;
}

int FIFO::remove_part(const DoutPrefixProvider *dpp, int64_t part_num,
                      std::uint64_t tid, optional_yield y)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           << " remove failed: r=" << r << " tid=" << tid << dendl;
    }
    return r;
}

int FIFO::process_journal(const DoutPrefixProvider *dpp, std::uint64_t tid,
                          optional_yield y)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
    ...
    for (auto i = 0; canceled && i < MAX_RACE_RETRIES; ++i) {
        ...
        if (new_head > info.head_part_num) {
            head_part_num = new_head;
        }
        if (new_max > info.max_push_part_num) {
                            ...
                            std::uint64_t tid, optional_yield y)
{
    ...
    if (info.journal.contains({
    create, new_part_num}
) && (!is_head || info.journal.contains({
    set_head, new_part_num}
                                                                ))) {
        l.unlock();
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                               << " process_journal failed: r=" << r << " tid=" << tid <<
                               dendl;
        }
        return r;
    }
    ...
    for (auto i = 0; canceled && i < MAX_RACE_RETRIES; ++i) {
        ...
        if (r >= 0 && canceled) {
            ...
            auto found = (info.journal.contains({ create, new_head_part_num }
                          ...
                          info.journal.contains({ set_head,
                                                  new_head_part_num }
                                               ));
            version = info.version;
...
struct NewPartPreparer: public Completion < NewPartPreparer > {
    ...
    NewPartPreparer(const DoutPrefixProvider *dpp, FIFO *f,
                    lr::AioCompletion *super,
                    std::vector < fifo::journal_entry > jentries,
                    std::int64_t new_part_num, std::uint64_t tid)
    ...
    } void handle(const DoutPrefixProvider *dpp, Ptr && p, int r)
    {
        ...
        if (canceled) {
            ...
            auto found = (f->info.journal.contains({ create, new_part_num })
                          || f->info.journal.contains({ set_head,
                                  new_part_num }));
...
void FIFO::_prepare_new_part(const DoutPrefixProvider *dpp,
                             std::int64_t new_part_num, bool is_head,
                             std::uint64_t tid, lr::AioCompletion *c)
{
    ...
    auto n = std::make_unique < NewPartPreparer > (dpp, this, c, jentries,
             new_part_num, tid);
    auto np = n.get();
    _update_meta(dpp, fifo::update {
    }
    .journal_entries_add(jentries), version,
...
struct NewHeadPreparer: public Completion < NewHeadPreparer > {
    ...
    NewHeadPreparer(const DoutPrefixProvider *dpp, FIFO *f,
                    lr::AioCompletion *super, bool newpart,
...
void FIFO::_prepare_new_head(const DoutPrefixProvider *dpp,
                             std::int64_t new_head_part_num,
                             std::uint64_t tid, lr::AioCompletion *c)
{
    ldpp_dout(dpp,
    ...
    if (max_push_part_num < new_head_part_num) {
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           << " need new part: tid=" << tid << dendl;
        auto n =
            std::make_unique < NewHeadPreparer > (dpp, this, c, true,
                new_head_part_num,
                tid);
        ...
        auto n =
            std::make_unique < NewHeadPreparer > (dpp, this, c, false,
                new_head_part_num,
                tid);
        auto np = n.get();
        using enum fifo::journal_entry::Op;
...
int FIFO::push_entries(const DoutPrefixProvider *dpp,
                       const std::deque < cb::list > &data_bufs,
                       std::uint64_t tid, optional_yield y)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
...
int FIFO::trim_part(const DoutPrefixProvider *dpp, int64_t part_num,
                    uint64_t ofs, bool exclusive, std::uint64_t tid,
                    optional_yield y)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
...
void FIFO::trim_part(const DoutPrefixProvider *dpp, int64_t part_num,
                     uint64_t ofs, bool exclusive, std::uint64_t tid,
                     lr::AioCompletion *c)
{
    ldpp_dout(dpp,
...
int FIFO::open(const DoutPrefixProvider *dpp, lr::IoCtx ioctx,
               std::string oid, std::unique_ptr < FIFO > *fifo,
               optional_yield y, std::optional < fifo::objv > objv,
               bool probe)
{
    ldpp_dout(dpp, 20)
            << __PRETTY_FUNCTION__ << ":" << __LINE__ << " entering" << dendl;
    fifo::info info;
    std::uint32_t size;
    std::uint32_t over;
    int r =
        get_meta(dpp, ioctx, std::move(oid), objv, &info, &size, &over, 0,
                 y,
                 probe);
    if (r < 0) {
...
int FIFO::create(const DoutPrefixProvider *dpp, lr::IoCtx ioctx,
                 std::string oid, std::unique_ptr < FIFO > *fifo,
                 optional_yield y, std::optional < fifo::objv > objv,
                 std::optional < std::string_view > oid_prefix,
...
int FIFO::read_meta(const DoutPrefixProvider *dpp, std::uint64_t tid,
                    optional_yield y)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
    ...
    auto r =
        get_meta(dpp, ioctx, oid, std::nullopt, &_info, &_phs, &_peo, tid,
                 y);
    if (r < 0) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
...
struct Reader: public Completion < Reader > {
    ...
    cb::list bl;
    std::uint64_t tid;
    Reader(const DoutPrefixProvider *dpp, FIFO *fifo,
           lr::AioCompletion *super, std::uint64_t tid)
    ...
    } void handle(const DoutPrefixProvider *dpp, Ptr && p, int r)
    {
        ...
        if (r >= 0)
            ...
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                               << " read_meta failed r=" << r << " tid=" << tid << dendl;
        }
        complete(std::move(p), r);
    }
};

void FIFO::read_meta(const DoutPrefixProvider *dpp, std::uint64_t tid,
                     lr::AioCompletion *c)
{
    ldpp_dout(dpp,
              20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
    ...
    auto rp = reader.get();
    auto r =
        ioctx.aio_exec(oid, Reader::call(std::move(reader)),
                       fifo::op::CLASS,
...
int FIFO::push(const DoutPrefixProvider *dpp, const cb::list &bl,
               optional_yield y)
{
    return push(dpp, std::vector {
        bl}
    , y);
}

void FIFO::push(const DoutPrefixProvider *dpp, const cb::list &bl,
                lr::AioCompletion *c)
{
    push(dpp, std::vector {
        bl}, c);
...
int FIFO::push(const DoutPrefixProvider *dpp,
               const std::vector < cb::list > &data_bufs,
               optional_yield y)
{
    std::unique_lock l(m);
    auto tid = ++next_tid;
    ...
    while ((!remaining.empty() || !batch.empty()) &&
        ...
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           ...
                           << " batch=" << batch.size() << " retries=" << retries
                           << " batch_len=" << batch_len << " tid=" << tid << dendl;
...
struct Pusher: public Completion < Pusher > {
    ...
    void prep_then_push(const DoutPrefixProvider *dpp, Ptr
                        && p, const unsigned successes)
    {
        std::unique_lock l(f->m);
    ...
    {
        state = new_heading;
        f->_prepare_new_head(dpp, head_part_num + 1, tid,
                             call(std::move(p)));
    }
        ...
        switch (state) {
            case pushing:
                ...
                if (r == -ENOENT) {
                    if (i > MAX_RACE_RETRIES) {
                        ldpp_dout(dpp,
                                  -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
    ...
    Pusher(const DoutPrefixProvider *dpp, FIFO *f,
           std::deque < cb::list > &&remaining, std::int64_t head_part_num,
...
void FIFO::push(const DoutPrefixProvider *dpp,
                const std::vector < cb::list > &data_bufs,
                lr::AioCompletion *c)
{
    std::unique_lock l(m);
    ...
    ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                       << " entering: tid=" << tid << dendl;
    auto p =
        std::make_unique < Pusher > (dpp, this,
                                     std::deque < cb::list >
                                     (data_bufs.begin(), data_bufs.end()),
                                     head_part_num, tid, c);
    // Validate sizes
    for (const auto &bl : data_bufs) {
        if (bl.length() > max_entry_size) {
...
int FIFO::trim(const DoutPrefixProvider *dpp, std::string_view markstr,
               bool exclusive, optional_yield y)
{
    ...
    while ((tail_part_num < part_num) &&
           canceled && (retries <= MAX_RACE_RETRIES)) {
        r = _update_meta(dpp, fifo::update {
        }
        .tail_part_num(part_num), objv, &canceled, tid, y);
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                               << " _update_meta failed: r=" << r
                               << " tid= " << tid << dendl;
...
struct Trimmer: public Completion < Trimmer > {
    ...
    Trimmer(const DoutPrefixProvider *dpp, FIFO *fifo,
            std::int64_t part_num, std::uint64_t ofs, std::int64_t pn,
            bool exclusive, lr::AioCompletion *super, std::uint64_t tid)
        : Completion(dpp, super), fifo(fifo), part_num(part_num), ofs(ofs),
          pn(pn), exclusive(exclusive), tid(tid)
    {
    } void handle(const DoutPrefixProvider *dpp, Ptr && p, int r)
    {
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
...
void FIFO::trim(const DoutPrefixProvider *dpp, std::string_view markstr,
                bool exclusive, lr::AioCompletion *c)
{
    auto marker = to_marker(markstr);
    ...
    ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                       << " entering: tid=" << tid << dendl;
    auto trimmer =
        std::make_unique < Trimmer > (dpp, this, realmark.num, realmark.ofs,
                                      pn, exclusive, c, tid);
    if (!marker) {
    ...
    trim_part(dpp, pn, ofs, exclusive, tid,
                        ...
                        fifo::part_header *header, optional_yield y)
{
    ...
    l.unlock();
    auto r =
        rgw::cls::fifo::get_part_info(dpp, ioctx, part_oid, header, tid, y);
    if (r < 0) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                           << " get_part_info failed: r=" << r << " tid=" << tid << dendl;
    }
    return r;
}
...
struct InfoGetter: Completion < InfoGetter > {
    ...
    InfoGetter(const DoutPrefixProvider *dpp, FIFO *fifo,
               fu2::function < void (int r, fifo::part_header
                                     &&) > f, std::uint64_t tid,
               lr::AioCompletion *super)
    ...
    } void handle(const DoutPrefixProvider *dpp, Ptr && p, int r)
    {
        if (!headerread) {
            ...
            headerread = true;
            auto op =
                rgw::cls::fifo::get_part_info(fifo->cct, &header, tid);
            std::unique_lock l(fifo->m);
...
void FIFO::get_head_info(const DoutPrefixProvider *dpp,
                         fu2::unique_function < void (int r,
                                 fifo::part_header
                                 &&) > f,
                         lr::AioCompletion *c)
{
    ...
    l.unlock();
    auto ig =
        std::make_unique < InfoGetter > (dpp, this, std::move(f), tid, c);
...
private:
    ...
    void create_part(const DoutPrefixProvider *dpp, Ptr
                     && p, int64_t part_num)
    {
        ldpp_dout(dpp,
                  20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
        ...
        ceph_assert(r >= 0);
        return;
    } void remove_part(const DoutPrefixProvider *dpp, Ptr
                       && p, int64_t part_num)
    {
        ldpp_dout(dpp,
                  20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
...
public:
    ...
    JournalProcessor(const DoutPrefixProvider *dpp, FIFO *fifo,
                     std::uint64_t tid, lr::AioCompletion *super)
        :    Completion(dpp, super), fifo(fifo), tid(tid)
    {
        std::unique_lock l(fifo->m);
    ...
    void pp_run(const DoutPrefixProvider *dpp, Ptr
                && p, int r, bool canceled)
    {
        ldpp_dout(dpp,
                  20) << __PRETTY_FUNCTION__ << ":" << __LINE__ <<
                      " entering: tid=" << tid << dendl;
...
void FIFO::process_journal(const DoutPrefixProvider *dpp,
                           std::uint64_t tid, lr::AioCompletion *c)
{
...
public:
    Lister(const DoutPrefixProvider *dpp, FIFO *f, std::int64_t part_num,
           std::uint64_t ofs, int max_entries,
           std::vector < list_entry > *entries_out, bool *more_out,
           std::uint64_t tid, lr::AioCompletion *super)
        : Completion(dpp, super), f(f), part_num(part_num), ofs(ofs),
          max_entries(max_entries), entries_out(entries_out),
          more_out(more_out), tid(tid)
    {
        result.reserve(max_entries);
    }
    ...
    void list(Ptr && p)
    {
        if (max_entries > 0) {
            ...
            f->ioctx.aio_operate(part_oid, call(std::move(p)), &op,
                                 nullptr);
        } else {
            complete(std::move(p), 0);
        }
                ...
                bool *more, lr::AioCompletion *c)
{
    ...
    auto ls =
        std::make_unique < Lister > (dpp, this, part_num, ofs, max_entries,
                                     out,
                                     more, tid, c);
    if (markstr && !marker) {
        auto l = ls.get();
