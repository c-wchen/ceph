...
namespace librbd {
    namespace journal {
        namespace util {
            ...
            int C_DecodeTag::decode(bufferlist::const_iterator * it,
                                    TagData * tag_data) {
                try {
                    using ceph::decode;
                     decode(*tag_data, *it);
                ...
                *tag_tid = tag.tid;
                ...
                ldout(cct,
                      20) << "C_DecodeTag: " << this << " " << __func__ << ": "
                    << "allocated journal tag: " << "tid=" << tag.
                    tid << ", " << "data=" << *tag_data << dendl;
                return 0;
            }
            ...
            int C_DecodeTags::process(int r) {
                if (r < 0) {
                    lderr(cct) << "C_DecodeTags: " << this << " " << __func__ <<
                        ": " << "failed to retrieve journal tags: " <<
                        cpp_strerror(r)
                        << dendl;
                    return r;
                }
