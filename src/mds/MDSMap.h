...
class MDSMap {
  public:
    ...
    bool get_inline_data_enabled() const {
    ...
    uint64_t get_max_filesize() const {
    ...
    std::string_view get_fs_name()const {
        return fs_name;
    } void set_fs_name(std::string new_fs_name) {
    ...
    } bool was_snaps_ever_allowed() const {
    ...
    void clear_standby_replay_allowed() {
        clear_flag(CEPH_MDSMAP_ALLOW_STANDBY_REPLAY);
    }
    bool allows_standby_replay() const {
        return test_flag(CEPH_MDSMAP_ALLOW_STANDBY_REPLAY);
    } bool was_standby_replay_ever_allowed() const {
    ...
    void clear_multimds_snaps_allowed() {
        clear_flag(CEPH_MDSMAP_ALLOW_MULTIMDS_SNAPS);
    }
    bool allows_multimds_snaps() const {
    ...
    void set_old_max_mds() {
        old_max_mds = max_mds;
    }
    mds_rank_t get_old_max_mds() const {
        return old_max_mds;
    ...
    const bool check_special_bal_rank_mask(std::string val,
    ...
    mds_rank_t get_tableserver() const {
        ...
        return root;
    } const std::vector < int64_t > &get_data_pools() const {
        return data_pools;
        ...
        return get_enabled() && (is_data_pool(poolid)
                                 || metadata_pool == poolid);
    } const auto & get_mds_info() const {
        return mds_info;
    ...
    // features uint64_t get_up_features();
   ...
   */
    ...
    void get_mds_set_lower_bound(std::set < mds_rank_t > &s,
                                 DaemonState first) const;
    void get_mds_set(std::set < mds_rank_t > &s, DaemonState state) const;

    void get_health(std::list < std::pair < health_status_t,
                    std::string > >&summary,
                    std::list < std::pair < health_status_t,
   ...
   */
    ...
    // mds states bool is_down(mds_rank_t m) const {
        return up.count(m) == 0;
   ...
   */ auto get_state(mds_rank_t m) const {
    ...
    // degraded = some recovery in process.  fixes active membership and// recovery_set.
        bool is_degraded() const;
    bool is_any_failed() const {
        ...
        return
            ...
            get_num_mds(STATE_REPLAY) == 0 && failed.empty() && damaged.empty();
