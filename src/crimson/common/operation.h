...
void dump_time_event(const char *name,
                     const utime_t &timestamp, ceph::Formatter *f);
void dump_blocking_event(const char *name,
                         const utime_t &timestamp,
                         const Blocker *blocker, ceph::Formatter *f);
}                           // namespace detail
/**
...
template < class T > struct Event {
    ...
    void trigger(OpT && op, Args && ... args)
    {
        ...
        EventBackendRegistry < std::decay_t <
        OpT >>::get_backends());
    }
};
...
template < class T > struct TimeEvent: Event < T > {
    ...
    void dump(ceph::Formatter *f) const
    {
        ...
        detail::dump_time_event(demangled_name.c_str(),
...
public:
    struct BlockingEvent: Event < typename T::BlockingEvent > {
        ...
        struct Backend {
            ...
            // basing on the type-of-event.
            virtual void handle(typename T::BlockingEvent &,
                                const Operation &, const T &) = 0;
        };

        struct InternalBackend: Backend {
            void handle(typename T::BlockingEvent &,
                        const Operation &, const T &blocker) override
            {
        ...
        struct TriggerI {
            ...
            auto maybe_record_blocking(FutureT
                                       && fut, const T &blocker)
            {
                if (!fut.available()) {
                    ...
                    return std::forward < FutureT > (fut).finally([&event =
                                this->
                                event,
                            &blocker]
                    ()mutable {
        ...
        protected:
            ...
            static void record_unblocking(BlockingEvent &event,
                                          const T &blocker)
            {
        ...
        template < class OpT > struct Trigger: TriggerI {
            Trigger(BlockingEvent &event, const OpT &op): TriggerI(event),
                op(op)
            {
            } template < class FutureT > auto maybe_record_blocking(FutureT
                    && fut,
                    const T
                    &
                    blocker)
        ...
        {
            auto demangled_name = boost::core::demangle(typeid(T).name());
            detail::dump_blocking_event(demangled_name.c_str(),
    ...
    template < class TriggerT, class ... Args > decltype(auto) track_blocking(TriggerT && trigger, Args
            && ... args)
    {
        return std::forward < TriggerT >
               (trigger).maybe_record_blocking(std::forward < Args >
                                               (args) ...,
                                               static_cast <
                                               const T & >(*this));
    }
...
template < class T > struct AggregateBlockingEvent {
    ...
    template < class OpT > struct Trigger final: TriggerI {
        ...
        } class TriggerContainer final: public TriggerI::TriggerContainerI
        {
            ...
            TriggerContainer(AggregateBlockingEvent &_event,
                             const OpT &op): event(_event),
...
class OperationRegistryI
{
    using op_list_member_option = boost::intrusive::member_hook <
                                  Operation, registry_hook_t, &Operation::registry_hook >;
...
public:
    ...
    {
        boost::intrusive_ptr < T > op =
            new T(std::forward < Args > (args) ...);
        do_register(&*op);
        return op;
    } seastar::future <> stop()
    {
        crimson::get_logger(ceph_subsys_osd).info("OperationRegistryI::{}",
                __func__);
        do_stop();
        ...
        arm_periodic(std::chrono::
                     milliseconds(100 /*TODO: use option instead */));
...
protected:
    ...
    {
        static_assert(REGISTRY_INDEX < std::tuple_size <
                      decltype(registries) >::value);
...
public:
    ...
    /// Removes op from registry void remove_from_registry(Operation & op) {
    const auto op_type = op.get_type();
...
public:
    core_id_t get_core()const
    {
        return core;
    } template < class ... Args > decltype(auto) enter(Args && ... args)
    {
        return static_cast < T * >(this)->enter(std::forward < Args >
                                                (args) ...);
    }
};
    ...
    public:
        ...
        void exit() final
        {
            if (phase) {
                ...
                auto id = op_id;
                phase = nullptr;
                std::ignore = seastar::smp::submit_to(p->get_core(), [p, id] {
                    p->exit(id);
                }
                                                     );
            }
...
private:
    ...
    class ExitBarrier final: public PipelineExitBarrierI
    {
        ...
        TriggerT trigger;
    public:
        ExitBarrier(OrderedConcurrentPhaseT *phase, seastar::future <> &&barrier, TriggerT &trigger): phase(phase),
        ...
        void exit() final
        {
            if (barrier) {
                static_cast <
                void >(std::move(*barrier).then([phase = this->phase] {
                    phase->mutex.unlock();
                }));
                barrier = std::nullopt;
                phase = nullptr;
            ...
            if (phase) {
                std::ignore = seastar::smp::submit_to(phase->get_core(),
                [this] {
                    phase->mutex.unlock();
                    phase = nullptr;
                }
