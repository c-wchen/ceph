...
TEST(BlueFS, test_tracker_50965)
{
    ...
    BlueFS fs(g_ceph_context);
    ASSERT_EQ(0,
              fs.add_block_device(BlueFS::BDEV_WAL, bdev_wal.path, false, 0));
    ...
    bool h_slow_dev_dirty =
        fs.debug_get_is_dev_dirty(h_slow, BlueFS::BDEV_SLOW);

    //problem if allocations are stable in log but slow device is not flushed yet
    ASSERT_FALSE(h_slow_dirty_seq_1 != 0 &&
                 h_slow_dirty_seq_2 == 0 && h_slow_dev_dirty == true);
        ...
        const std::string content(content_size, 'x');
        {
            ...
            }
            {
                ...
                ASSERT_EQ(content.length(),
                          fs.read(h, 0, read_size, &bl, NULL));
                ASSERT_EQ(0,
                          strncmp(content.c_str(), bl.c_str(),
                                  content.length()));
                delete h;
            }
            {
...
TEST(BlueFS, test_shared_alloc)
{
    ...
    ASSERT_EQ(0,
              fs.add_block_device(BlueFS::BDEV_DB, bdev_db.path, false,
                                  size_db - 0x1000));
    ASSERT_EQ(0,
              fs.add_block_device(BlueFS::BDEV_SLOW, bdev_slow.path, false, 0,
                                  &shared_alloc));
