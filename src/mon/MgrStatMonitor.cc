...
MgrStatMonitor::MgrStatMonitor(Monitor * mn, Paxos * p,
                               const string & service_name)
:  PaxosService(mn, p, service_name), pgservice(new MgrPGStatService(digest))
{
}
...
void MgrStatMonitor::update_logger()
{
  ...
  for (auto i:digest.num_pg_by_pool) {
    ...
    mon->cluster_logger->set(l_cluster_num_object,
                             digest.pg_sum.stats.sum.num_objects);
    ...
    mon->cluster_logger->set(l_cluster_num_object_unfound,
                             digest.pg_sum.stats.sum.num_objects_unfound);
    mon->cluster_logger->set(l_cluster_num_bytes,
                             digest.pg_sum.stats.sum.num_bytes);
...
void MgrStatMonitor::create_pending()
{
    ...
    pending_service_map_bl.clear();
    ::encode(service_map, pending_service_map_bl,
...
void MgrStatMonitor::get_health(list < pair < health_status_t,
                                string > >&summary,
...
void MgrStatMonitor::print_summary(Formatter * f, std::ostream * ss) const const
{
...
bool MgrStatMonitor::preprocess_getpoolstats(MonOpRequestRef op)
{
  ...
  for (const auto & pool_name:m->pools) {
        const auto pool_id =
            mon->osdmon()->osdmap.lookup_pg_pool_name(pool_name);
        if (pool_id == -ENOENT)
            continue;
