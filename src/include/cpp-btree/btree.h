...
namespace btree::internal {
    ...
    template < typename T > inline constexpr bool has_compare_to =
...
// default.
    ...
    template < typename K >
        struct key_compare_to_adapter <std::less < K >,
        std::enable_if_t < has_compare_to < K >>> {
        struct type {
    ...
    template < typename K >
        struct key_compare_to_adapter <std::less < K >,
        std::enable_if_t < std::is_signed_v < K >>> {
        struct type {
    ...
    template < typename Key, typename Compare, typename Alloc,
        int TargetNodeSize, int ValueSize, bool Multi > struct common_params {
...
// Compare and Alloc should be nothrow copy-constructible.
    template < typename Key, typename Data, typename Compare, typename Alloc,
        ...
        static const Key & key(const value_type & value) {
            return value.first;
        }
        ...
        template < class ... Args >
            static void construct(Alloc * alloc, slot_type * slot, Args
                                  && ... args) {
        ...
        static void construct(Alloc * alloc, slot_type * slot,
                              slot_type * other) {
            emplace(slot);
...
// A parameters structure for holding the type parameters for a btree_set.
    template < typename Key, typename Compare, typename Alloc,
        int TargetNodeSize, bool Multi > struct set_params
    :public common_params < Key, Compare, Alloc, TargetNodeSize, sizeof(Key),
        Multi > {
...
// that the children array is only valid in internal nodes.
    template < typename Params > class btree_node {
      ...
      public:
        ...
        using use_linear_search = std::integral_constant <
            ...
            (std::is_same_v < std::less < key_type >,
             key_compare > ||std::is_same_v < std::greater < key_type >,
             key_compare >) >;
      ...
      private:
        ...
        constexpr static size_type NodeTargetValues(const int begin,
                                                    const int end) {
            return begin ==
                end ? begin : SizeWithNValues((begin + end) / 2 + 1) >
                params_type::kTargetNodeSize ? NodeTargetValues(begin,
                                                                (begin +
                                                                 end) / 2)
        ...
        constexpr static int kNodeTargetValues =
            NodeTargetValues(0, kTargetNodeSize);
        ...
        template < auto MemPtr > auto & GetField() {
            return reinterpret_cast < internal_fields * >(this)->*MemPtr;
        }

        template < auto MemPtr > auto & GetField()const {
      ...
      public:
        ...
        bool leaf() const {
            return GetField < &base_fields::max_count > () !=
        ...
        // Getter for the number of values stored in this node.
            ...
            const field_type max_count =
                GetField < &base_fields::max_count > ();
            return max_count == field_type {
            kInternalNodeMaxCount} ? field_type {
            kNodeValues}
            : max_count;
        }
        ...
        template < typename K >
            SearchResult < int,
            is_key_compare_to::value > lower_bound(const K & k,
        ...
        int upper_bound(const K & k, const key_compare & comp) const {
            ...
            return use_linear_search::value ? linear_search(k,
                                                            upper_compare).
                value : binary_search(k, upper_compare).value;
        } template < typename K, typename Compare > SearchResult < int,
            btree_is_key_compare_to < Compare,
            key_type >::value > linear_search(const K & k,
            ...
            btree_is_key_compare_to < Compare,
            key_type >::value > binary_search(const K & k,
                                              const Compare & comp) const {
            return binary_search_impl(k, 0, count(), comp,
                                      btree_is_key_compare_to < Compare,
                                      key_type > ());
        }
        // Returns the position of the first value whose key is not less than k using// linear search performed using plain compare.
            template < typename K, typename Compare >
        SearchResult < int, false > linear_search_impl(const K & k, int s,
                                                       const int e,
                                                       const Compare & comp,
                                                       std::
                                                       false_type
        ...
        template < typename K, typename Compare >
            SearchResult < int, true > linear_search_impl(const K & k, int s,
                                                          const int e,
                                                          const Compare & comp,
                                                          std::
                                                          true_type
        ...
        template < typename K, typename Compare >
            SearchResult < int, false > binary_search_impl(const K & k, int s,
                                                           int e,
                                                           const Compare & comp,
                                                           std::
                                                           false_type
        ...
        template < typename K, typename CompareTo >
            SearchResult < int, true > binary_search_impl(const K & k, int s,
                                                          int e,
                                                          const CompareTo &
                                                          comp,
                                                          std::
        ...
        template < typename ... Args >
            void emplace_value(size_type i, allocator_type * alloc, Args
                               && ... args);
        ...
        void split(const int insert_position, btree_node * dest,
                   allocator_type * alloc);
      ...
      private:
        template < typename ... Args >
            void value_init(const size_type i, allocator_type * alloc, Args
                            && ... args) {
            params_type::construct(alloc, slot(i),
    ...
    template < typename Node, typename Reference, typename Pointer > struct btree_iterator {
      ...
      public:
        ...
        // NOTE: this SFINAE allows for implicit conversions from iterator to// const_iterator, but it specifically avoids defining copy constructors so// that btree_iterator can be trivially copyable. This is for performance and// binary size reasons.
            template < typename N, typename R, typename P,
        std::enable_if_t <
            std::is_same_v < btree_iterator < N, R, P >,
            iterator > &&std::is_same_v < btree_iterator, const_iterator >,
            int > = 0 > btree_iterator(const btree_iterator < N, R, P > &x)
      ...
      private:
        ...
        template < typename N, typename R, typename P,
            std::enable_if_t <
            std::is_same_v < btree_iterator < N, R, P >,
            const_iterator > &&std::is_same_v < btree_iterator, iterator >,
            int > =
            0 > explicit btree_iterator(const btree_iterator < N, R, P > &x)
    ...
    template < typename Params > class btree {
        ...
        static node_type *EmptyNode() {
            static constexpr EmptyNodeType empty_node(const_cast <
                                                      EmptyNodeType *
                                                      >(&empty_node));
        ...
        template < typename Base1, typename Base2, typename Data >
            ...
            empty_base_handle(const Base1 & b1, const Base2 & b2,
                              const Data & d)
      ...
      btree(btree && x) noexcept:root_(std::move(x.root_)),
        ...
        ~btree() {
            ...
            // is complete.
            static_assert(static_assert_validation(),
        ...
        // Finds the range of values which compare equal to key. The first member of// the returned pair is equal to lower_bound(key). The second member pair of// the pair is equal to upper_bound(key).
            template < typename K >
        ...
        template < typename K >
            ...
            const_iterator > equal_range(const K & key)const {
            return {
            lower_bound(key), upper_bound(key)};
        }
        ...
        template < typename ValueType > iterator insert_multi(ValueType && v) {
            return insert_multi(params_type::key(v),
        ...
        // Verifies the structure of the btree. void verify() const;
        ...
        // The total number of bytes used by the btree. size_type bytes_used() const {
            node_stats stats = internal_stats(root());
        ...
        // The overhead of the btree structure in bytes per node. Computed as the// total number of bytes used by the btree minus the number of bytes used for// storing elements divided by the number of elements.// Returns 0 for empty trees.
        ...
        // The allocator used by the btree. allocator_type get_allocator() const {
            return allocator();
      } private:
        ...
        // Emplaces a value into the btree immediately before iter. Requires that// key(v) <= iter.key() and (--iter).key() <= key(v).
            template < typename ... Args >
        ...
        template < typename K >
            ...
            is_key_compare_to::value > internal_locate(const K & key) const;

        template < typename K >
            SearchResult < iterator, false > internal_locate_impl(const K & key,
                                                                  std::
                                                                  false_type
                                                                  /* IsCompareTo */
                                                                  ) const;
        ...
        template < typename K >
            SearchResult < iterator, true > internal_locate_impl(const K & key,
                                                                 std::
                                                                 true_type
                                                                 /* IsCompareTo */
                                                                 ) const;
...
// btree_node methods
    ...
    template < typename P >
        inline void btree_node < P >::remove_value(const int i,
                                                   allocator_type * alloc) {
        if (!leaf() && count() > i + 1) {
    ...
    template < typename P >
        inline void btree_node < P >::remove_values_ignore_children(const int i,
                                                                    const int
                                                                    to_erase,
                                                                    allocator_type
                                                                    * alloc) {
    ...
    template < typename P >
        ...
        if (right->count() >= to_move) {
            ...
            // 1) Shift existing values in the right node to their correct positions.
            right->uninitialized_move_n(right->count(), 0, to_move, right,
                                        alloc);
            ...
            // 3) Move the (to_move - 1) values from the left node to the right node.
            const size_type uninitialized_remaining =
    ...
    template < typename P >
        void btree_node < P >::split(const int insert_position,
                                     btree_node * dest,
                                     allocator_type * alloc) {
        assert(dest->count() == 0);
    ...
    template < typename P >
        ...
        larger->uninitialized_move_n(to_move, smaller->count(),
                                     smaller->count(), smaller, alloc);
...
// btree methods
    template < typename P >
        ...
        static_assert(std::is_same_v < btree,
                      Btree > ||std::is_same_v < const btree, Btree >,
    ...
    template < typename P >
        ...
        static_assert(std::is_same_v < compare_result_type,
                      bool > ||std::is_signed_v < compare_result_type >,
    ...
    template < typename P >
        btree < P >::btree(const key_compare & comp,
  ...
  :    root_(comp, alloc, EmptyNode()), rightmost_(EmptyNode()), size_(0) {
    ...
    ->std::pair < iterator, bool > {
        ...
        return {
        internal_emplace(iter, std::forward < Args > (args) ...), true};
    }

    template < typename P >
        template < typename ... Args >
        inline auto btree < P >::insert_hint_unique(iterator position,
                                                    const key_type & key, Args
                                                    && ... args)
        ->std::pair < iterator, bool > {
        if (!empty()) {
    ...
    template < typename P >
        template < typename InputIterator >
        void btree < P >::insert_iterator_unique(InputIterator b,
                                                 InputIterator e) {
        for (; b != e; ++b) {
    ...
    template < typename P >
        template < typename ValueType >
        auto btree < P >::insert_multi(const key_type & key, ValueType
                                       && v)->iterator {
        if (empty()) {
            mutable_root() = rightmost_ = new_leaf_root_node(1);
        }
    ...
    template < typename P >
        template < typename ValueType >
        auto btree < P >::insert_hint_multi(iterator position, ValueType
                                            && v)->iterator {
        if (!empty()) {
            ...
            if (position == end() || !compare_keys(position.key(), key)) {
                ...
                if (position == begin() || !compare_keys(key, (--prev).key())) {
                    // prev.key() <= key <= position.key()
                    return internal_emplace(position,
                                            std::forward < ValueType > (v));
                }
            }
            else {
    ...
    template < typename P >
        template < typename InputIterator >
        void btree < P >::insert_iterator_multi(InputIterator b,
                                                InputIterator e) {
        for (; b != e; ++b) {
    ...
    template < typename P >
        ...
        if (begin.node == end.node) {
            ...
            size_ -= count;
            return {
            count, rebalance_after_delete(begin)};
        }
        ...
        while (size_ > target_size) {
            if (begin.node->leaf()) {
                ...
                const size_type remaining_in_node =
                    begin.node->count() - begin.position;
                begin =
                    erase_from_leaf_node(begin,
                                         std::min(remaining_to_erase,
                                                  remaining_in_node));
            }
            else {
                begin = erase(begin);
    ...
    template < typename P >
        ...
        if (!node->leaf()) {
            ...
            for (size_type i = begin.position + to_erase + 1;
                 i <= node->count(); ++i) {
    ...
    template < typename P >
        auto btree < P >::erase_from_leaf_node(iterator begin,
                                               size_type to_erase)
        ->iterator {
        node_type *node = begin.node;
        assert(node->leaf());
    ...
    template < typename P >
        template < typename ... Args >
        inline auto btree < P >::internal_emplace(iterator iter, Args
                                                  && ... args)
        ->iterator {
        if (!iter.node->leaf()) {
    ...
    template < typename P >
        int btree < P >::internal_verify(const node_type * node,
                                         const key_type * lo,
