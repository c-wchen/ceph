...
struct frag_info_t:public scatter_info_t {
    ...
    // *this += cur - acc;
    void add_delta(const frag_info_t & cur, const frag_info_t & acc,
                   bool * touched_mtime = 0, bool * touched_chattr = 0) {
        if (cur.mtime > mtime) {
            mtime = cur.mtime;
            if (touched_mtime)
                *touched_mtime = true;
...
struct nest_info_t:public scatter_info_t {
    ...
    bool same_sums(const nest_info_t & o) const {
        return rctime <= o.rctime &&
            rbytes == o.rbytes &&
            rfiles == o.rfiles &&
            rsubdirs == o.rsubdirs && rsnaprealms == o.rsnaprealms;
    } void encode(bufferlist & bl) const;
    void decode(bufferlist::iterator & bl);
    void dump(Formatter * f) const;
...
struct client_writeable_range_t {
    ...
    static void generate_test_instances(std::list <
                                        client_writeable_range_t * >&ls);
};

inline void decode(client_writeable_range_t::byte_range_t & range,
  ...
  inline_data_t(const inline_data_t & o):version(o.version) {
    ...
    bool operator==(const inline_data_t & o)const {
        return length() == o.length() &&
            (length() == 0 ||
             (*const_cast < bufferlist * >(blp.get()) == *const_cast <
              bufferlist * >(o.blp.get())));
    } bool operator!=(const inline_data_t & o)const {
        return !(*this == o);
    } void encode(bufferlist & bl) const;
    void decode(bufferlist::iterator & bl);
...
template < template < typename > class Allocator = std::allocator > struct inode_t {
   ...
   */
    ...
    compact_set < int64_t, std::less < int64_t >,
        Allocator < int64_t >> old_pools;
    ...
    using client_range_map =
        std::map < client_t, client_writeable_range_t, std::less < client_t >,
    ...
    inode_t()
{
  ...
  // file type bool is_symlink() const {
...
uint64_t get_layout_size_increment() const {
    ...
    for (std::map < client_t, client_writeable_range_t >::const_iterator p =
         client_ranges.begin(); p != client_ranges.end(); ++p)
...
template < template < typename > class Allocator >
    void inode_t < Allocator >::encode(bufferlist & bl,
    ...
    void inode_t < Allocator >::decode(bufferlist::iterator & p)
{
    ...
    if (struct_v >= 12) {
        ...
        stray_prior_path =
            ...
            Allocator < char >>(boost::string_view(tmp));
    }
...
template < template < typename > class Allocator >
    void inode_t < Allocator >::dump(Formatter * f) const const
{
...
template < template < typename > class Allocator >
    int inode_t < Allocator >::compare(const inode_t < Allocator > &other,
                                       bool * divergent) const const
{
...
template < template < typename > class Allocator >
    bool inode_t < Allocator >::older_is_consistent(const inode_t < Allocator >
                                                    &other) constconst
{
...
template < template < typename > class Allocator >
    inline void encode(const inode_t < Allocator > &c,::ceph::bufferlist & bl,
                       uint64_t features)
{
    ENCODE_DUMP_PRE();
...
template < template < typename > class Allocator >
    inline void decode(inode_t < Allocator >
                       &c,::ceph::bufferlist::iterator & p)
{
    c.decode(p);
}
...
template < template < typename > class Allocator > using xattr_map = compact_map < alloc_string < Allocator >, bufferptr, std::less < alloc_string < Allocator >>, Allocator < std::pair < const alloc_string < Allocator >, bufferptr >>>; // FIXME bufferptr not in mempool
...
// These methods may be moved back to mdstypes.cc when we have pmr
template < template < typename > class Allocator >
    void old_inode_t < Allocator >::encode(bufferlist & bl,
                                           uint64_t features) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(first, bl);
...
template < template < typename > class Allocator >
    void old_inode_t < Allocator >::dump(Formatter * f) const const
{
...
template < template < typename > class Allocator >
    void old_inode_t < Allocator >::generate_test_instances(std::list <
                                                            old_inode_t <
                                                            Allocator > *>&ls)
{
...
template < template < typename > class Allocator >
    inline void encode(const old_inode_t < Allocator >
                       &c,::ceph::bufferlist & bl, uint64_t features)
{
    ENCODE_DUMP_PRE();
    c.encode(bl, features);
    ENCODE_DUMP_POST(cl);
...
template < template < typename > class Allocator >
    inline void decode(old_inode_t < Allocator >
                       &c,::ceph::bufferlist::iterator & p)
{
    c.decode(p);
}
...
{
    return out << "old_rstat(first " << o.first << " " << o.rstat << " " << o.
        accounted_rstat << ")";
}
...
struct session_info_t {
    ...
    } const entity_name_t & get_source() const {
        return inst.name;
    } void clear_meta() {
        prealloc_inos.clear();
        used_inos.clear();
...
// cap info for client reconnect struct cap_reconnect_t {
    string path;
    ...
    cap_reconnect_t() {
        ...
        snap_follows = 0;
    } cap_reconnect_t(uint64_t cap_id, inodeno_t pino, boost::string_view p,
                      int w, int i, inodeno_t sr, snapid_t sf,
                      bufferlist & lb):path(p) {
...
class inode_load_vec_t {
  ...
  public:
    ...
    :vec {
    DecayCounter(now), DecayCounter(now)} {
    }
...
inline void decode(inode_load_vec_t & c, const utime_t & t,
...
class dirfrag_load_vec_t {
  public:
    ...
    :vec {
        DecayCounter(now),
            DecayCounter(now),
            DecayCounter(now), DecayCounter(now), DecayCounter(now)
    } {
    }
    // for dencoder infrastructure
    dirfrag_load_vec_t() {
...
inline void decode(dirfrag_load_vec_t & c, const utime_t & t,
                   bufferlist::iterator & p)
{
    c.decode(t, p);
}
...
struct mds_load_t {
    ...
    // mostly for the dencoder infrastructure mds_load_t():auth(), all() {
    }
...
inline std::ostream & operator<<(std::ostream & out, const mds_load_t & load)
{
    return out << "mdsload<" << load.auth << "/" << load.all
        ...
        << ", qlen " << load.queue_len << ", cpu " << load.cpu_load_avg << ">";
}
...
inline std::ostream & operator<<(std::ostream & out,
                                 const MDSCacheObjectInfo & info)
{
    if (info.ino)
        return out << info.ino << "." << info.snapid;
    if (info.dname.length())
        return out << info.dirfrag << "/" << info.dname
            << " snap " << info.snapid;
...
template < typename Iterator > struct keys_and_values
