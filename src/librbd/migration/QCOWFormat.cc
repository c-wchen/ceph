        ...
        namespace qcow_format {
            ...
            struct ClusterExtent {
                ...
                :cluster_offset(cluster_offset), cluster_length(cluster_length),
                    ...
                    image_offset(image_offset), buffer_offset(buffer_offset) {
            }};
            ...
            void LookupTable::init() {
                if (cluster_offsets == nullptr) {
                    cluster_offsets =
                        reinterpret_cast < uint64_t * >(bl.c_str());
            ...
            void populate_cluster_extents(CephContext * cct,
              ...
              for (auto[image_offset, image_length]:image_extents)
                {
                    while (image_length > 0) {
                        auto intra_cluster_offset =
                            image_offset & (cluster_size - 1);
                        auto intra_cluster_length =
                            cluster_size - intra_cluster_offset;
                        auto cluster_length =
                            std::min(image_length, intra_cluster_length);
                        ...
                        cluster_extents->emplace_back(0, cluster_length,
...
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::ClusterCache: " \
        ...
        template < typename I > class QCOWFormat < I >::ClusterCache {
          ...
          :    
            ...
            void get_cluster(uint64_t cluster_offset, uint64_t cluster_length,
                ...
                boost::asio::dispatch(m_strand,
                                      [this, cluster_offset, cluster_length,
                                       intra_cluster_offset, bl, on_finish] () {
                                      execute_get_cluster(cluster_offset,
                                                          cluster_length,
                                                          intra_cluster_offset,
                                                          bl, on_finish);});
            }
          ...
          private:
            typedef std::tuple < uint64_t, uint64_t, bufferlist *,
                Context * >Completion;
            ...
            void execute_get_cluster(uint64_t cluster_offset,
                                     uint64_t cluster_length,
                                     uint64_t intra_cluster_offset,
                ...
                if (cluster && cluster->cluster_offset == cluster_offset) {
                    ...
                    bl->substr_of(cluster->cluster_data_bl,
                                  intra_cluster_offset, cluster_length);
                ...
                bool new_request =
                    (cluster_completions.count(cluster_offset) == 0);
                cluster_completions[cluster_offset].
                    emplace_back(intra_cluster_offset, cluster_length, bl,
                                 on_finish);
                if (new_request) {
            ...
            void read_cluster(std::shared_ptr < Cluster > cluster) {
                ...
                ldout(cct,
                    ...
                    cluster_offset << ", " << "stream_offset=" << stream_offset
                ...
                auto ctx = new LambdaContext([this, cluster] (int r){
                                             ...
                                             void handle_read_cluster(int r,
                                                                      std::
                                                                      shared_ptr
                                                                      <
                                                                      Cluster >
                                                                      cluster) {
                                             auto cct =
                                             qcow_format->m_image_ctx->cct;
                                             ldout(cct,
                                                   20) << "r=" << r << ", " <<
                                             "cluster_offset=" << cluster->
                                             cluster_offset << dendl;
                                             auto completions =
                                             std::
                                             move(cluster_completions
                                                  [cluster->cluster_offset]);
                                             cluster_completions.erase(cluster->
                                                                       cluster_offset);
                                             if (r < 0) {
                                             lderr(cct) <<
                                             "failed to read cluster offset " <<
                                             cluster->
                                             cluster_offset << ": " <<
...
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::L2TableCache: " \
                           << this << " " << __func__ << ": "
                                             ...
                                             I >::L2TableCache {
          public:
                                             L2TableCache(QCOWFormat *
                                                          qcow_format)
          :                                 qcow_format(qcow_format),
                                             m_strand(*qcow_format->
                                                      m_image_ctx->asio_engine),
                                             l2_cache_entries
                                             (QCOW_L2_CACHE_SIZE) {
                                             }

                                             void get_l2_table(const LookupTable
                                                               * l1_table,
                                                               uint64_t
                                                               l2_table_offset,
                                                               std::shared_ptr <
                                             ...
                                             ldout(cct,
                                                   20) << "l2_table_offset=" <<
                                             l2_table_offset << dendl;
                                             // cache state machine runs in a single strand thread
                                             Request request {
                                             l1_table, l2_table_offset,
                                             l2_table, on_finish};
                                             boost::asio::dispatch(m_strand,
                                                                   [this,
                                                                    request =
                                             ...
                                             image_offset >> qcow_format->
                                             m_l1_shift;
                                             uint64_t l2_table_offset =
                                             l1_table->
                                             cluster_offsets[std::min <
                                                             uint32_t >
                                                             (l1_table_index,
                                                              l1_table->size -
                                                              1)] &
          ...
          private:
                                             ...
                                             strand m_strand; struct Request {
                                             ...
                                             std::shared_ptr <
                                             const LookupTable > *l2_table;
                                             Context * on_finish;
                                             Request(const LookupTable *
                                                     l1_table,
                                                     uint64_t l2_table_offset,
                                                     uint32_t l2_table_index,
                                                     uint64_t * cluster_offset,
                                             ...
                                             typedef std::deque < Request >
                                             Requests; struct L2Cache {
                                             uint64_t l2_offset = 0;
                                             std::shared_ptr < LookupTable >
                                             l2_table; utime_t timestamp;
                                             uint32_t count = 0;
                                             ...
                                             if (request.cluster_offset !=
                                                 nullptr) {
                                             auto cluster_offset =
                                             l2_table->cluster_offsets[request.
                                                                       l2_table_index];
                                             if (!l2_table->decoded) {
                                             ...
                                             *request.cluster_offset =
                                             cluster_offset & qcow_format->
                                             m_cluster_mask;
                                             if (*request.cluster_offset ==
                                                 QCOW_OFLAG_ZERO) {
                                             ldout(cct,
                                             ...
                                             "l2_table_index=" << request.
                                             l2_table_index << ", " <<
                                             "cluster_offset=" << *request.
                                             cluster_offset << dendl;}
                                             }
                                             else
                                             if (request.l2_table != nullptr) {
                                             ...
                                             *request.l2_table = l2_table;}
                                             ...
                                             for (auto idx = 0U;
                                                  idx < l2_cache_entries.size();
                                                  ++idx) {
                                             auto & l2_cache =
                                             l2_cache_entries[idx];
                                             if (l2_cache.l2_offset ==
                                                 l2_offset) {
                          ...
                          for (auto & entry:l2_cache_entries)
                                             ...
                                             *l2_table = l2_cache.l2_table;
                                             ...
                                             ldout(cct,
                                                   20) << "l2_offset=" <<
                                             l2_offset << ", " << "index=" <<
                                             min_idx << " (loading)" << dendl;
                                             auto & l2_cache =
                                             l2_cache_entries[min_idx];
                                             l2_cache.l2_table =
                                             std::make_shared < LookupTable >
                                             (qcow_format->m_l2_size);
                                             ...
                                             void handle_l2_table_lookup(int r,
                                                                         uint32_t
                                                                         index,
                                                                         uint64_t
                                                                         l2_offset)
...
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::ReadRequest: " \
          ...
          private:
              ...
              handle_get_cluster_offsets(r);}); auto gather_ctx = new C_Gather(cct, ctx); for (auto & cluster_extent:cluster_extents)
                                             {
                                             auto sub_ctx =
                                             new
                                             LambdaContext([this,
                                                           ...
                                                           (r, cluster_extent,
                                                            on_finish);});
                                             qcow_format->m_l2_table_cache->
                                             get_cluster_offset(l1_table,
                                                                cluster_extent.
                                                                image_offset,
                                                                &cluster_extent.
                                                                cluster_offset,
                                             ...
                                             void handle_get_cluster_offset(int
                                                                            r,
                                                                            const
                                                                            ClusterExtent
                                                                            &
                                             ...
                                             void handle_get_cluster_offsets(int
                                                                             r)
                                             {
                                             auto cct =
                                             qcow_format->m_image_ctx->cct;
                                             ...
                                             if (r < 0) {
                                             lderr(cct) <<
                                             "failed to retrieve cluster extents: "
                                             << cpp_strerror(r)
                                             << dendl; aio_comp->fail(r);
                                             delete this; return;}

                                             read_clusters();}

                                             void read_clusters() {
              auto cct = qcow_format->m_image_ctx->cct; ldout(cct, 20) << dendl; aio_comp->set_request_count(cluster_extents.size()); for (auto & cluster_extent:cluster_extents)
                                             {
                                             auto read_ctx =
                                             new io::ReadResult::
                                             C_ImageReadRequest(aio_comp,
                                                                cluster_extent.
                                                                buffer_offset,
                                                                { {
                                                                cluster_extent.
                                             ...
                                             auto log_ctx = new LambdaContext([this, cct = qcow_format->m_image_ctx->cct, image_offset = cluster_extent.image_offset, image_length = cluster_extent.cluster_length, ctx = read_ctx] (int r) {
                                                                              handle_read_cluster
                                                                              (cct,
                                                                               r,
                                                                               image_offset,
                                             ...
                                             qcow_format->m_cluster_cache->
                                             get_cluster(cluster_extent.
                                                         cluster_offset,
                                                         cluster_extent.
                                                         cluster_length,
                                                         cluster_extent.
                                             ...
                                             void
                                             handle_read_cluster(CephContext *
                                                                 cct, int r,
                                                                 uint64_t
                                                                 image_offset,
                                                                 uint64_t
...
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::" \
          ...
          private:
                                             ...
                                             if (l1_table_index >=
                                                 l1_table->size
                                                 || l1_table->
                                                 cluster_offsets[l1_table_index]
                                                 == 0) {
                                             ldout(cct,
                                                   20) << "l1_table_index=" <<
                                             l1_table_index << ", " <<
                                             "snap_id=" << snap_id << ": DNE" <<
                                             dendl; ctx->complete(-ENOENT);
                                             return;}
                                             ...
                                             void handle_get_l2_table(int r,
                                                                      uint64_t
                                                                      snap_id) {
                                             ceph_assert(qcow_format->m_strand.
                                             ...
                                             if (r < 0) {
                                             lderr(cct) <<
                                             "failed to retrieve L2 table for snapshot "
                                             << snap_id << ": " <<
                                             cpp_strerror(r) << dendl;
                                             finish(r); return;}
                                             ...
                                             cluster_offset =
                                             l2_table->
                                             cluster_offsets[l2_table_index] &
                                             qcow_format->
                                             m_cluster_offset_mask;}

                                             std::optional < uint64_t >
                                             prev_cluster_offset;
                                             if (previous_l2_table
                                                 && l2_table_index <
                                                 previous_l2_table->size) {
                                             prev_cluster_offset =
                                             previous_l2_table->
                                             cluster_offsets[l2_table_index] &
                                             qcow_format->
                                             ...
                                             sparse_extents.
                                             insert(cluster_extent.image_offset,
                                                    cluster_extent.
                                                    cluster_length, {
                                                    state,
                                                    cluster_extent.
...
#define dout_prefix *_dout << "librbd::migration::QCOWFormat: " << this \
                           << " " << __func__ << ": "
                                             ...
                                             if (r < 0) {
                                             lderr(cct) <<
                                             "failed to build migration stream handler"
                                             << cpp_strerror(r)
                                             << dendl; on_finish->complete(r);
                                             return;}
                                             ...
                                             auto header_probe =
                                             *reinterpret_cast <
                                             QCowHeaderProbe * >(m_bl.c_str());
                                             header_probe.magic =
                                             big_to_native(header_probe.magic);
                                             header_probe.version =
                                             big_to_native(header_probe.
                                                           version);
                                             if (header_probe.magic !=
                                                 QCOW_MAGIC) {
                                             lderr(cct) <<
                                             "invalid QCOW header magic" <<
                                             dendl;
                                             ...
                                             if (header_probe.version >= 2
                                                 && header_probe.version <= 3) {
                                             read_v2_header(on_finish); return;}
                                             else {
                                             lderr(cct) <<
                                             "invalid QCOW header version " <<
                                             header_probe.version << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}
                                             }
                                             ...
                                             if (r < 0) {
                                             lderr(cct) <<
                                             "failed to read QCOW header: " <<
                                             cpp_strerror(r) << dendl;
                                             on_finish->complete(r); return;}

                                             auto header =
                                             *reinterpret_cast <
                                             QCowHeaderV1 * >(m_bl.c_str());
                                             // byte-swap important fields
                                             header.magic =
                                             big_to_native(header.magic);
                                             header.version =
                                             big_to_native(header.version);
                                             ...
                                             big_to_native(header.
                                                           backing_file_size);
                                             header.size =
                                             big_to_native(header.size);
                                             header.crypt_method =
                                             big_to_native(header.crypt_method);
                                             ...
                                             lderr(cct) << "header is not QCOW"
                                             << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}

                                             if (header.cluster_bits <
                                             ...
                                             if (header.l2_bits <
                                                 (QCOW_MIN_CLUSTER_BITS - 3)
                                                 || header.l2_bits >
                                                 (QCOW_MAX_CLUSTER_BITS - 3)) {
                                             lderr(cct) << "invalid L2 bits: "
                                             << header.l2_bits << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}
                                             ...
                                             if (header.crypt_method !=
                                                 QCOW_CRYPT_NONE) {
                                             lderr(cct) <<
                                             "invalid or unsupported encryption method"
                                             << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}
                                             ...
                                             header.cluster_bits;
                                             m_cluster_size =
                                             1UL << header.cluster_bits;
                                             m_cluster_offset_mask =
                                             (1ULL <<
                                              (63 - header.cluster_bits)) - 1;
                                             m_cluster_mask =
                                             ~QCOW_OFLAG_COMPRESSED;
                                             m_l2_bits = header.l2_bits;
                                             m_l2_size = (1UL << m_l2_bits);
                                             m_l1_shift =
                                             m_cluster_bits + m_l2_bits;
                                             m_l1_table.size =
                                             (m_size + (1LL << m_l1_shift) -
                                              1) >> m_l1_shift;
                                             m_l1_table_offset =
                                             header.l1_table_offset;
                                             if (m_size >
                                             ...
                                             m_l2_table_cache =
                                             std::make_unique < L2TableCache >
                                             (this);
                                             m_cluster_cache =
                                             std::make_unique < ClusterCache >
                                             (this); read_l1_table(on_finish);}

#endif // WITH_RBD_MIGRATION_FORMAT_QCOW_V1

                                             template < typename I >
                                             void QCOWFormat <
                                             I >::read_v2_header(Context *
                                                                 on_finish) {
                                             auto cct = m_image_ctx->cct;
                                             ldout(cct, 10) << dendl;
                                             auto ctx =
                                             ...
                                             if (r < 0) {
                                             lderr(cct) <<
                                             "failed to read QCOW2 header: " <<
                                             cpp_strerror(r) << dendl;
                                             on_finish->complete(r); return;}

                                             auto header =
                                             *reinterpret_cast <
                                             QCowHeader * >(m_bl.c_str());
                                             // byte-swap important fields
                                             header.magic =
                                             big_to_native(header.magic);
                                             header.version =
                                             ...
                                             big_to_native(header.
                                                           backing_file_size);
                                             header.cluster_bits =
                                             big_to_native(header.cluster_bits);
                                             header.size =
                                             big_to_native(header.size);
                                             header.crypt_method =
                                             big_to_native(header.crypt_method);
                                             header.l1_size =
                                             big_to_native(header.l1_size);
                                             header.l1_table_offset =
                                             big_to_native(header.
                                                           l1_table_offset);
                                             header.nb_snapshots =
                                             big_to_native(header.nb_snapshots);
                                             ...
                                             if ((header.compression_type != 0)
                                                 ...
                                                 ((header.
                                                  ...
                                                  != 0)) {
                                             lderr(cct) <<
                                             "invalid or unsupported compression type"
                                             << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}
                                             ...
                                             if ((header.
                                                 ...
                                                 0) {
                                             lderr(cct) <<
                                             "external data file feature not supported"
                                             << dendl;
                                             on_finish->complete(-ENOTSUP);}

                                             if ((header.
                                                  incompatible_features &
                                                  QCOW2_INCOMPAT_EXTL2) != 0) {
                                             lderr(cct) <<
                                             "extended L2 table feature not supported"
                                             << dendl;
                                             on_finish->complete(-ENOTSUP);
                                             return;}
                                             ...
                                             if (header.incompatible_features !=
                                                 0) {
                                             lderr(cct) <<
                                             "unknown incompatible feature enabled"
                                             << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}
                                             ...
                                             if (m_l1_table.size >
                                                 header.l1_size) {
                                             lderr(cct) <<
                                             "invalid L1 table size in header ("
                                             << header.
                                             l1_size << " < " << m_l1_table.
                                             size << ")" << dendl;
                                             on_finish->complete(-EINVAL);
                                             ...
                                             m_cluster_cache =
                                             std::make_unique < ClusterCache >
                                             (this); read_snapshot(on_finish);}

                                             template < typename I >
                                             void QCOWFormat <
                                             I >::read_snapshot(Context *
                                                                on_finish) {
                                             if (m_snapshots_offset == 0
                                                 || m_snapshots.size() ==
                                                 m_snapshot_count) {
                                             read_l1_table(on_finish); return;}
                                             ...
                                             size() << dendl; if (r < 0) {
                                             lderr(cct) <<
                                             "failed to read QCOW2 snapshot header: "
                                             << cpp_strerror(r)
                                             << dendl; on_finish->complete(r);
                                             return;}
                                             ...
                                             *reinterpret_cast <
                                             ...
                                             >(m_bl.c_str());
                                             auto & snapshot =
                                             m_snapshots[m_snapshots.size() +
                                                         1];
                                             snapshot.id.
                                             resize(big_to_native
                                                    (header.id_str_size));
                                             snapshot.name.
                                             resize(big_to_native
                                                    (header.name_size));
                                             ...
                                             big_to_native(header.
                                                           l1_table_offset);
                                             snapshot.l1_table.size =
                                             big_to_native(header.l1_size);
                                             snapshot.timestamp.sec_ref() =
                                             big_to_native(header.date_sec);
                                             snapshot.timestamp.nsec_ref() =
                                             big_to_native(header.date_nsec);
                                             snapshot.extra_data_size =
                                             big_to_native(header.
                                                           extra_data_size);
                                             ldout(cct,
                                                   10) << "snap_id=" <<
                                             ...
                                             << snapshot.name.
                                             size() << ", " <<
                                             "l1_table_offset=" << snapshot.
                                             l1_table_offset << ", " <<
                                             "l1_size=" << snapshot.l1_table.
                                             size << ", " << "extra_data_size="
                                             << snapshot.
                                             extra_data_size << dendl;
                                             read_snapshot_extra(on_finish);}

                                             template < typename I >
                                             void QCOWFormat <
                                             I >::read_snapshot_extra(Context *
                                                                      on_finish)
                                             {
                                             ceph_assert(!m_snapshots.empty());
                                             auto & snapshot =
                                             m_snapshots.rbegin()->second;
                                             uint32_t length =
                                             snapshot.extra_data_size +
                                             snapshot.id.size() +
                                             snapshot.name.size();
                                             ...
                                             snapshot.name =
                                             uuid_gen.to_string();
                                             read_snapshot(on_finish); return;}
                                             ...
                                             if (snapshot.extra_data_size >=
                                                 offsetof(QCowSnapshotExtraData,
                                                          disk_size) +
                                                 sizeof(uint64_t)) {
                                             auto extra =
                                             reinterpret_cast <
                                             const QCowSnapshotExtraData *
                                             >(m_bl.c_str());
                                             snapshot.size =
                                             big_to_native(extra->disk_size);}
                                             else {
                                             snapshot.size = m_size;}

                                             auto data =
                                             reinterpret_cast <
                                             const char *>(m_bl.c_str());
                                             ...
                                             size() << ", " <<
                                             "l1_table_offset=" << snapshot.
                                             l1_table_offset << dendl;
                                             auto ctx =
                                             new
                                             LambdaContext([this,
                                                            on_finish] (int r) {
                                                           handle_read_snapshot_l1_table
                                                           (r, on_finish);});
                                             m_stream->read( { {
                                                            snapshot.
                                                            l1_table_offset,
                                             ...
                                             void QCOWFormat <
                                             I >::
                                             handle_read_snapshot_l1_table(int
                                                                           r,
                                                                           Context
                                                                           *
                                                                           on_finish)
                                             ...
                                             size() << dendl; if (r < 0) {
                                             lderr(cct) <<
                                             "failed to read snapshot L1 table: "
                                             << cpp_strerror(r)
                                             << dendl; on_finish->complete(r);
                                             return;}
                                             ...
                                             template < typename I >
                                             void QCOWFormat <
                                             I >::read_backing_file(Context *
                                                                    on_finish) {
                                             if (m_backing_file_offset == 0
                                                 || m_backing_file_size == 0) {
                                             // all data is within the specified file
                                             on_finish->complete(0); return;}
                                             ...
                                             template < typename I >
                                             void QCOWFormat <
                                             I >::get_snapshots(SnapInfos *
                                                                snap_infos,
                                                                Context *
                                                                on_finish) {
          auto cct = m_image_ctx->cct; ldout(cct, 10) << dendl; snap_infos->clear(); for (auto &[snap_id, snapshot]:m_snapshots)
                                             {
                                             SnapInfo snap_info(snapshot.name,
                                                                cls::rbd::
                                             ...
                                             *size = snapshot.size;}
                                             ...
                                             new ReadRequest(this, aio_comp,
                                                             ...
                                                             move
                                                             (image_extents));
                                             read_request->send(); return true;}

                                             template < typename I >
                                             void QCOWFormat <
                                             I >::list_snaps(io::Extents
                                                             && image_extents,
                                                             io::SnapIds
                                                             && snap_ids,
                                                             int
          ...
          std::map < uint64_t, ClusterExtents > l1_cluster_extents; for (auto & cluster_extent:cluster_extents)
                                             {
                                             uint32_t l1_table_index =
                                             cluster_extent.
                                             image_offset >> m_l1_shift;
                                             auto & l1_cluster_extent =
                                             l1_cluster_extents[l1_table_index];
                                             l1_cluster_extent.
                                             reserve(cluster_extents.size());
                                             l1_cluster_extent.
                                             push_back(cluster_extent);}

          std::map < uint64_t, const LookupTable * >snap_id_to_l1_table; for (auto &[snap_id, snapshot]:m_snapshots)
                                             {
                                             snap_id_to_l1_table[snap_id] =
                                             &snapshot.l1_table;}
                                             snap_id_to_l1_table[CEPH_NOSNAP] =
                                             &m_l1_table;
                                             on_finish =
                                             new
                                             LambdaContext([this, image_extents,
                                                            snap_ids =
                                                            std::move(snap_ids),
                                                            snapshot_delta,
                                                            on_finish] (int r)
                                                           mutable {
          handle_list_snaps(r, std::move(image_extents), std::move(snap_ids), snapshot_delta, on_finish);}); auto gather_ctx = new C_Gather(cct, on_finish); for (auto &[l1_table_index, cluster_extents]:l1_cluster_extents)
                                             {
                                             auto list_snaps_request =
                                             new ListSnapsRequest(this,
                                                                  l1_table_index,
                                                                  std::
                                                                  move
                                                                  (cluster_extents),
                                             ...
                                             I >::handle_list_snaps(int r,
                                                                    ...
                                                                    *
                                                                    ...
                                                                    Context *
                                                                    on_finish) {
          auto cct = m_image_ctx->cct; ldout(cct, 20) << "r=" << r << ", " << "snapshot_delta=" << *snapshot_delta << dendl; std::optional < uint64_t > previous_size = std::nullopt; for (auto &[snap_id, snapshot]:m_snapshots)
                                             {
                                             auto sparse_extents =
                                             &(*snapshot_delta)[ {
                                                                snap_id, snap_id}
                                             ];
                                             util::zero_shrunk_snapshot(cct,
                                                                        image_extents,
                                                                        snap_id,
                                                                        snapshot.
                                                                        size,
                                             ...
                                             template class librbd::migration::
                                             QCOWFormat < librbd::ImageCtx >;
