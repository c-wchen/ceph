...
class RGWSI_MBSObj_Handler_Module:public RGWSI_MetaBackend::Module {
  ...
  public:
    ...
    } virtual void get_pool_and_oid(const std::string & key, rgw_pool * pool,
                                    std::string * oid) = 0;
...
struct RGWSI_MBSObj_GetParams:public RGWSI_MetaBackend::GetParams {
  ...
  RGWSI_MBSObj_GetParams(bufferlist * _pbl, std::map < std::string, bufferlist > *_pattrs, ceph::real_time * _pmtime):RGWSI_MetaBackend::GetParams(_pmtime),
        pbl(_pbl),
        pattrs(_pattrs) {
    }
    ...
    RGWSI_MBSObj_GetParams & set_refresh_version(boost::optional < obj_version >
                                                 &_refresh_version) {
...
struct RGWSI_MBSObj_PutParams:public RGWSI_MetaBackend::PutParams {
    ...
    RGWSI_MBSObj_PutParams(std::map < std::string, bufferlist > *_pattrs,
                           const ceph::real_time & _mtime):RGWSI_MetaBackend::
        PutParams(_mtime), pattrs(_pattrs) {
    }
    RGWSI_MBSObj_PutParams(bufferlist & _bl,
                           std::map < std::string, bufferlist > *_pattrs,
                           const ceph::real_time & _mtime,
                           bool _exclusive):RGWSI_MetaBackend::
        PutParams(_mtime), bl(_bl), pattrs(_pattrs), exclusive(_exclusive) {
    }
};
...
class RGWSI_MetaBackend_SObj:public RGWSI_MetaBackend {
  ...
  public:
    ...
    int call_with_get_params(ceph::real_time * pmtime,
                             std::function <
...
class RGWSI_MetaBackend_Handler_SObj:public RGWSI_MetaBackend_Handler {
  ...
  public:
    RGWSI_MetaBackend_Handler_SObj(RGWSI_MetaBackend *
                                   be):RGWSI_MetaBackend_Handler(be) {
