...
namespace librbd {
    namespace operation {
        ...
        namespace {
            ...
            template < typename I >
                ...
                case RenameRequest < I >::STATE_READ_DIRECTORY:
                    ...
                    case RenameRequest <
                        I >::
                        STATE_READ_SOURCE_HEADER:os << "READ_SOURCE_HEADER";
                    break;
                    case RenameRequest <
                        I >::STATE_WRITE_DEST_HEADER:os << "WRITE_DEST_HEADER";
                    break;
                    case RenameRequest <
                        I >::STATE_UPDATE_DIRECTORY:os << "UPDATE_DIRECTORY";
                    break;
                    case RenameRequest <
                        I >::
                        STATE_REMOVE_SOURCE_HEADER:os << "REMOVE_SOURCE_HEADER";
                    break;
        ...
        :Request < I > (image_ctx, on_finish), m_dest_name(dest_name),
            m_source_oid(image_ctx.
                         old_format ? util::old_header_name(image_ctx.
                                                            name) : util::
                         id_obj_name(image_ctx.name)),
            m_dest_oid(image_ctx.
                       old_format ? util::old_header_name(dest_name) : util::
        ...
        template < typename I > bool RenameRequest < I >::should_complete(int r) {
            ...
            if (m_state == STATE_READ_DIRECTORY) {
                ...
                if (r < 0) {
                    lderr(cct) << "could not read directory: " <<
        ...
        template < typename I >
            ...
            int r =
                image_ctx.md_ctx.aio_operate(m_source_oid, rados_completion,
                                             &op,
                                             &m_header_bl);
            ceph_assert(r == 0);
        ...
        template < typename I >
            ...
            librados::AioCompletion * rados_completion =
                this->create_callback_completion();
            int r =
                image_ctx.md_ctx.aio_operate(m_dest_oid, rados_completion, &op);
            ceph_assert(r == 0);
            rados_completion->release();
        }
        ...
        template < typename I >
            ...
            librados::AioCompletion * rados_completion =
                this->create_callback_completion();
            int r =
                image_ctx.md_ctx.aio_operate(RBD_DIRECTORY, rados_completion,
                                             &op);
            ceph_assert(r == 0);
            rados_completion->release();
        }
