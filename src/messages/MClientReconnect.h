  ...
  public:
    const char *get_type_name() const override {
        ...
        out << "client_reconnect(" << caps.size() << " caps)";
                   ...
                   const string & path, int wanted, int issued, inodeno_t sr,
                   snapid_t sf, bufferlist & lb) {
        caps[ino] =
    ...
    void encode_payload(uint64_t features) override {
        ...
        if (features & CEPH_FEATURE_MDSENC) {
            ...
            __u32 n = caps.size();
            ::encode(n, data);
            for (map < inodeno_t, cap_reconnect_t >::iterator p = caps.begin();
                 p != caps.end(); ++p) {
                ::encode(p->first, data);
    ...
    void decode_payload() override {
        ...
        if (header.version >= 3) {
            ...
            map < inodeno_t, old_cap_reconnect_t > ocaps;
            ::decode(ocaps, p);
            for (map < inodeno_t, old_cap_reconnect_t >::iterator q =
                 ocaps.begin(); q != ocaps.end(); q++)
