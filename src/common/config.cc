...
:  cluster(""), lock("md_config_t", true, false)
{
    init_subsys();
...
int md_config_t::parse_config_files_impl(const std::list < std::string >
                                         &conf_files, std::ostream * warnings)
{
  ...
  for (const auto & i:schema) {
        ...
        if (ret == 0) {
            ...
            if (warnings != nullptr && (r != 0 || !error_message.empty())) {
                *warnings << "parse error setting '" << opt.
                    name << "' to '" << val << "'";
    ...
    for (size_t o = 0; o < subsys.get_num(); o++) {
        ...
        std::string val;
        int ret =
            _get_val_from_conf_file(my_sections, as_option.c_str(), val, false);
        if (ret == 0) {
            int log, gather;
    ...
    if (!old_style_section_names.empty()) {
        ...
        string sep;
        for (std::deque < std::string >::const_iterator os =
             old_style_section_names.begin();
    ...
    if (show_config_value) {
        ...
        if (r < 0) {
            if (r == -ENOENT)
                ...
                std::cerr << "failed to get config option '" <<
                    show_config_value_arg << "': " << cpp_strerror(r) << std::
                    endl;
            _exit(1);
        }
        string s = buf;
  ...
  for (const auto & opt_iter:schema) {
        ...
        if (opt.type == Option::TYPE_BOOL) {
            ...
            if (oss && ((!opt.is_safe()) &&
                ...
                *oss << "You cannot change " << opt.
                    name << " using injectargs.\n";
                return -ENOSYS;
            }
...
void md_config_t::apply_changes(std::ostream * oss)
{
    rev_obs_map_t rev_obs;
    {
        ...
        if (cluster.size()) {
            for_each_change(oss,
                            [this, &rev_obs] (md_config_obs_t * obs,
...
void md_config_t::set_val_or_die(const std::string & key,
                                 const std::string & val, bool meta)
{
...
int md_config_t::get_val(const std::string & key, char **buf, int len) const const
{
    Mutex::Locker l(lock);
    return _get_val(key, buf, len);
}

Option::value_t md_config_t::get_val_generic(const std::string & key) constconst
{
    Mutex::Locker l(lock);
    return _get_val(key);
}

Option::value_t md_config_t::_get_val(const std::string & key) constconst
{
    assert(lock.is_locked());
...
int md_config_t::_get_val(const std::string & key, std::string * value) const const
{
    assert(lock.is_locked());
...
int md_config_t::_get_val(const std::string & key, char **buf, int len) const const
{
    assert(lock.is_locked());
    ...
    for (size_t o = 0; o < subsys.get_num(); o++) {
        ...
        if (k == as_option) {
            ...
            int l =
                snprintf(*buf, len, "%d/%d", subsys.get_log_level(o),
                         subsys.get_gather_level(o));
            return (l == len) ? -ENAMETOOLONG : 0;
        }
    }
...
void md_config_t::get_all_keys(std::vector < std::string > *keys) const const
{
 ...
 */
void md_config_t::get_my_sections(std::vector < std::string > &sections) const const
{
    Mutex::Locker l(lock);
    _get_my_sections(sections);
}

void md_config_t::_get_my_sections(std::vector < std::string > &sections) const const
{
    assert(lock.is_locked());
...
int md_config_t::get_val_from_conf_file(const std::vector < std::string >
                                        &sections, const std::string & key,
                                        std::string & out,
...
class assign_visitor:public boost::static_visitor <> {
  ...
  public:
    ...
    :conf(conf_), val(val_) {
    } template < typename T > void operator() (T md_config_t::*ptr) const {
        T *member =
            const_cast <
            T * >(&(conf->*(boost::get < const T md_config_t::*>(ptr))));
...
void md_config_t::expand_all_meta()
{
  ...
  for (const auto & i:schema) {
        ...
        if (opt.type == Option::TYPE_STR) {
            list < const Option *>stack;
            std::string * str =
                boost::get < std::string > (&(values.at(opt.name)));
...
bool md_config_t::expand_meta(std::string & val, std::ostream * oss) const const
{
    list < const Option *>stack;
                              ...
                              std::ostream * oss) constconst
{
    ...
    for (string::size_type s = 0; s < val.size();) {
        ...
        if (val[s + 1] == '{') {
            ...
            if (endpos != std::string::npos && val[endpos] == '}') {
            ...
            if (!expanded) {
                ...
                if (other_opt_iter != schema.end()) {
                    ...
                    if (other_opt.type == Option::TYPE_STR) {
                        ...
                        // before inserting.
                        Option::value_t * other_val_ptr =
                            const_cast <
                            Option::value_t * >(&(values.at(other_opt.name)));
                        std::string * other_opt_val =
                            boost::get < std::string > (other_val_ptr);
...
void md_config_t::diff(const md_config_t * other,
...
void md_config_t::diff_helper(const md_config_t * other,
                              map < string, pair < string, string > >*diff,
                              set < string > *unknown, const string & setting)
{
    Mutex::Locker l(lock);
...
void md_config_t::map_observer_changes(md_config_obs_t * obs,
                                       const std::string & key,
                                       rev_obs_map_t * rev_obs)
{
