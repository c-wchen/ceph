...
namespace crimson::os::seastore {
    ...
    std::ostream & operator<<(std::ostream & out, const segment_id_t & segment) {
        if (segment == NULL_SEG_ID) {
            ...
            return out << "Seg[" << device_id_printer_t {
            segment.device_id()}
    ...
    journal_seq_t journal_seq_t::add_offset(journal_type_t type,
                                            device_off_t off,
        ...
        jseq, make_block_relative_paddr(joff)};
    }

    device_off_t journal_seq_t::relative_to(journal_type_t type,
        ...
        device_off_t ret =
            static_cast < device_off_t > (segment_seq) - r.segment_seq;
        ret *= roll_size;
        if (type == journal_type_t::SEGMENTED) {
            ret +=
                (static_cast < device_off_t >
                 (offset.as_seg_paddr().get_segment_off()) - static_cast <
                 device_off_t > (r.offset.as_seg_paddr().get_segment_off()));
        }
        else {
            assert(type == journal_type_t::RANDOM_BLOCK);
    ...
    std::ostream & operator<<(std::ostream & out, const journal_seq_t & seq) {
        if (seq == JOURNAL_SEQ_NULL) {
            ...
            return out << "jseq(" << segment_seq_printer_t {
            seq.segment_seq}
            <<", " << seq.offset << ")";
        }
    }
    ...
    std::ostream & operator<<(std::ostream & out, mod_time_point_printer_t tp) {
        ...
        return out << "mod_" << sea_time_point_printer_t {
        time};
    }
    ...
    std::ostream & operator<<(std::ostream & out,
                              const journal_tail_delta_t & delta) {
        return out << "journal_tail_delta_t(" << "alloc_tail=" << delta.
            alloc_tail << ", dirty_tail=" << delta.dirty_tail << ")";
    }

    std::ostream & operator<<(std::ostream & out, const extent_info_t & info) {
        return out << "extent_info_t("
            << "type: " << info.type
            << ", addr: " << info.addr << ", len: " << info.len << ")";
    }

    std::ostream & operator<<(std::ostream & out,
                              const segment_header_t & header) {
        return out << "segment_header_t(" << header.
            physical_segment_id << " " << header.
            type << " " << segment_seq_printer_t {
        header.segment_seq}
        <<" " << header.category << " " << rewrite_gen_printer_t {
        header.generation}
    ...
    std::ostream & operator<<(std::ostream & out, const segment_tail_t & tail) {
        return out << "segment_tail_t("
            ...
            << " " << tail.type << " " << segment_seq_printer_t {
        tail.segment_seq}
        <<", segment_nonce=" << tail.segment_nonce
            << ", modify_time=" << mod_time_point_printer_t {
        tail.modify_time}
    ...
    std::ostream & operator<<(std::ostream & out, const record_size_t & rsize) {
        return out << "record_size_t(" << "raw_md=" << rsize.get_raw_mdlength()
            << ", data=" << rsize.dlength << ")";
    }

    std::ostream & operator<<(std::ostream & out, const record_t & r) {
        return out << "record_t("
            << "type=" << r.type << ", num_extents=" << r.extents.size()
    ...
    std::ostream & operator<<(std::ostream & out,
                              const record_group_header_t & h) {
        return out << "record_group_header_t(" << "num_records=" << h.
            records << ", mdlength=" << h.mdlength << ", dlength=" << h.
            dlength << ", nonce=" << h.segment_nonce << ", committed_to=" << h.
            committed_to << ", data_crc=" << h.data_crc << ")";
    }
    ...
    } void record_group_size_t::account(const record_size_t & rsize,
    ...
    std::ostream & operator<<(std::ostream & out,
        ...
        return out << "record_group_size_t(" << "raw_md=" << size.
            get_raw_mdlength()
            << ", data=" << size.dlength << ", block_size=" << size.
            block_size << ", fullness=" << size.get_fullness()
            << ")";
    }
    ...
    ceph::bufferlist encode_record(record_t && record,
        ...
        return encode_records(record_group,
                              committed_to, current_segment_nonce);
    }

    ceph::bufferlist encode_records(record_group_t & record_group,
                                    const journal_seq_t & committed_to,
                                    segment_nonce_t current_segment_nonce) {
      ...
      for (auto & r:record_group.records) {
        ...
        record_group_header_t header {
            static_cast < extent_len_t > (record_group.records.size()),
                ...
                current_segment_nonce, committed_to, data_bl.crc32c(-1)
        };
        encode(header, bl);
      ...
      for (auto & r:record_group.records) {
        ...
        auto metadata_crc =
            bliter.crc32c(ceph::encoded_sizeof_bounded < record_group_header_t >
                          (),
                          -1);
        ...
        metadata_crc_filler.copy_in(sizeof(checksum_t),
                                    reinterpret_cast <
                                    const char *>(&metadata_crc_le));
    ...
    std::optional < record_group_header_t >
        try_decode_records_header(const ceph::bufferlist & header_bl,
        ...
        if (header.segment_nonce != expected_nonce) {
            journal_logger().
                ...
                ("try_decode_records_header: failed, record_group_header nonce mismatch, "
                 "read {}, expected {}!", header.segment_nonce, expected_nonce);
            return std::nullopt;
        }
        return header;
    }
    ...
    bool validate_records_data(const record_group_header_t & header,
                               const ceph::bufferlist & data_bl) {
        bool success = (data_bl.crc32c(-1) == header.data_crc);
        if (!success) {
            journal_logger().
                debug("validate_records_data: failed, data crc mismatch!");
        }
        return success;
    }

    std::optional < std::vector < record_header_t >>
        try_decode_record_headers(const record_group_header_t & header,
    ...
    std::optional < std::vector < record_extent_infos_t >
        >try_decode_extent_infos(const record_group_header_t & header,
                                 const ceph::bufferlist & md_bl) {
        ...
        std::vector < record_extent_infos_t >
            record_extent_infos(maybe_headers->size());
    ...
    std::optional < std::vector < record_deltas_t >
        >try_decode_deltas(const record_group_header_t & header,
                           const ceph::bufferlist & md_bl,
      ...
      for (auto & r:*maybe_record_extent_infos) {
        ...
        std::vector < record_deltas_t >
            record_deltas(maybe_record_extent_infos->size());
    ...
    void scan_valid_records_cursor::
        ...
        last_committed = new_committed_to;
        pending_record_groups.emplace_back(seq.offset,
                                           header, std::move(md_bl));
    ...
    std::ostream & operator<<(std::ostream & out,
                              const scan_valid_records_cursor & c) {
        return out << "cursor(last_valid_header_found=" << c.
            last_valid_header_found << ", seq=" << c.
            seq << ", last_committed=" << c.
            last_committed << ", pending_record_groups=" << c.
            pending_record_groups.size()
