...
class OpHistory {
  ...
  public:
    ...
    void set_slow_op_size_and_threshold(uint32_t new_size,
...
class OpTracker {
  ...
  public:
    ...
    bool dump_ops_in_flight(Formatter * f, bool print_only_blocked =
                            false, set < string > filters = {
                            ""});
    bool dump_historic_ops(Formatter * f, bool by_duration =
                           false, set < string > filters = {
                           ""});
...
class TrackedOp:public boost::intrusive::list_base_hook <> {
  ...
  protected:
    ...
    virtual bool filter_out(const set < string > &filters) {
        return true;
    }
  ...
  private:
    ...
    void mark_event(const char *event, utime_t stamp = ceph_clock_now());
