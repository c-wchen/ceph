...
namespace journal {
    ...
    ObjectRecorder::ObjectRecorder(librados::IoCtx & ioctx,
                                   std::string_view oid, uint64_t object_number,
                                   ceph::mutex * lock, ContextWQ * work_queue,
                                   Handler * handler, uint8_t order,
                                   int32_t max_in_flight_appends)
    :m_oid(oid), m_object_number(object_number), m_op_work_queue(work_queue),
        m_handler(handler), m_order(order), m_soft_max_size(1 << m_order),
        m_max_in_flight_appends(max_in_flight_appends), m_lock(lock) {
        m_ioctx.dup(ioctx);
    ...
    void ObjectRecorder::flush(Context * on_safe) {
            ...
            *m_lock};
            ...
            if (!m_pending_buffers.empty()) {
                ...
                AppendBuffers & append_buffers =
                    m_in_flight_appends.rbegin()->second;
        ...
        if (future.is_valid()) {
            ...
            m_op_work_queue->queue(new LambdaContext([future,
                                                      on_safe] (int r)mutable {
    ...
    bool ObjectRecorder::send_appends(bool force,
                                      ceph::ref_t < FutureImpl > flush_future) {
        ldout(m_cct, 20) << dendl;
        ...
        if (!force &&
            ((m_flush_interval > 0
              && m_pending_buffers.size() >= m_flush_interval)
        ...
        if (!force && max_in_flight_appends != 0 &&
            static_cast < int32_t > (m_in_flight_tids.size()) >=
        ...
        for (auto it = m_pending_buffers.begin();
             it != m_pending_buffers.end();) {
            auto & future = it->first;
            auto & bl = it->second;
            auto size =
                m_object_bytes + m_in_flight_bytes + append_bytes + bl.length();
            if (size == m_soft_max_size) {
                ldout(m_cct,
                      10) << "object at capacity (" << size << ") " << *future
                    << dendl;
                m_overflowed = true;
            }
            ...
            bool flush_break = (force && flush_future
                                && flush_future == future);
            ldout(m_cct, 20) << "flushing " << *future << dendl;
        ...
        if (append_bytes > 0) {
            ...
            auto rados_completion =
                librados::Rados::
                aio_create_completion(new C_AppendFlush(this, append_tid),
    ...
    void ObjectRecorder::notify_handler_unlock(std::unique_lock < ceph::mutex >
                                               &locker,
