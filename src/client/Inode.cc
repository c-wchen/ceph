...
{
    out << in.vino() << "("
        << "faked_ino=" << in.faked_ino << " nref=" << in.get_nref()
        << " ll_ref=" << in.ll_ref
...
bool Inode::cap_is_valid(const Cap &cap) constconst {
...
bool Inode::caps_issued_mask(unsigned mask, bool allow_impl)
{
    ...
    // prefer auth cap
    if (auth_cap &&
        cap_is_valid(*auth_cap) && (auth_cap->issued & mask) == mask) {
        auth_cap->touch();
        client->cap_hit();
        return true;
    }
    ...
    if (is_dir()) {
        ...
        if (dir_replicated) {
            ...
            for (const auto &frag : frag_repmap) {
                ...
                *css << std::hex << frag.first.
                     value() << "/" << std::dec << frag.first.bits();
