...
namespace crimson::os::seastore::journal {
    ...
    RecordBatch::add_pending_ret
        RecordBatch::add_pending(const std::string & name,
                                 record_t && record, extent_len_t block_size) {
        ...
        TRACE("{} batches={}, write_size={}, dlength_offset={} ...",
              name,
              pending.get_size() + 1,
              new_size.get_encoded_length(), dlength_offset);
        assert(state != state_t::SUBMITTING);
        assert(evaluate_submit(record.size, block_size).submit_size ==
               new_size);
        ...
        return io_promise->get_shared_future().
            then([dlength_offset, FNAME,
                  &name] (auto maybe_promise_result)->add_pending_ret {
                 ...
                 write_result.start_seq.offset.add_offset(maybe_promise_result->
                                                          mdlength +
                                                          dlength_offset),
                 write_result};
                 TRACE("{} write finish with {}", name, submit_result);
                 return add_pending_ret(add_pending_ertr::ready_future_marker {
                                        }
                                        , submit_result);}
        ) ;
    }
    ...
    std::pair < ceph::bufferlist, record_group_size_t >
        RecordBatch::encode_batch(const journal_seq_t & committed_to,
    ...
    void RecordBatch::set_result(maybe_result_t maybe_write_result) {
        ...
        if (maybe_write_result.has_value()) {
            ...
            *maybe_write_result, submitting_mdlength};
        }
        assert(state == state_t::SUBMITTING);
        assert(io_promise.has_value());
    ...
    std::pair < ceph::bufferlist, record_group_size_t >
        RecordBatch::submit_pending_fast(record_t && record,
    ...
    RecordSubmitter::RecordSubmitter(std::size_t io_depth,
                                     std::size_t batch_capacity,
                                     std::size_t batch_flush_size,
                                     double preferred_fullness,
                                     JournalAllocator & ja)
  :    io_depth_limit {
    io_depth}, preferred_fullness {
    preferred_fullness}, journal_allocator {
    ja}, batches(new RecordBatch[io_depth + 1]) {
    ...
    RecordSubmitter::action_t
        ...
        assert(is_available());
        auto eval =
            p_current_batch->evaluate_submit(rsize,
                                             journal_allocator.
                                             get_block_size());
    ...
    RecordSubmitter::roll_segment_ertr::future <>
        ...
        ().then_wrapped([FNAME, this] (auto fut) {
                        if (fut.failed()) {
                        ERROR("{} rolling is skipped unexpectedly, available",
                              get_name()); has_io_error = true;
                        ...
                        std::ignore =
                        journal_allocator.roll().safe_then([FNAME, this] {
                                                           // good
                                                           DEBUG
                                                           ("{} rolling done, available",
                                                            get_name());
                                                           assert
                                                           (!has_io_error);
                                                           wait_available_promise->
                                                           set_value();
                                                           wait_available_promise.
                                                           reset();}
                        ).
                        handle_error(crimson::ct_error::
                                     all_same_way([FNAME, this] (auto e) {
                                                  ERROR
                                                  ("{} got error {}, available",
                                                   get_name(), e);
                                                  has_io_error = true;
                                           ...
                                           ERROR
                                           ("{} got exception {}, available",
                                            get_name(), e); has_io_error = true;
                                           wait_available_promise->set_value();
                                           wait_available_promise.reset();}
                        );
    ...
    RecordSubmitter::submit_ret
        ...
        auto eval =
            p_current_batch->evaluate_submit(record.size,
                                             journal_allocator.
                                             get_block_size());
        bool needs_flush = (state == state_t::IDLE
                            || eval.submit_size.get_fullness() >
                            preferred_fullness ||
        ...
        if (p_current_batch->is_empty() &&
            needs_flush && state != state_t::FULL) {
            // fast path with direct write
            increment_io();
            auto[to_write, sizes] =
                p_current_batch->submit_pending_fast(std::move(record),
                                                     journal_allocator.
                                                     get_block_size(),
                                                     get_committed_to(),
                                                     journal_allocator.
        ...
        auto write_fut = p_current_batch->add_pending(get_name(),
                                                      std::move(record),
                                                      journal_allocator.
                                                      get_block_size());
        if (needs_flush) {
            if (state == state_t::FULL) {
                ...
                DEBUG
                    ("{} added with {} pending, outstanding_io={}, unavailable, wait flush ...",
                     get_name(), p_current_batch->get_num_records(),
                     num_outstanding_io);
                if (with_atomic_roll_segment) {
                    ...
                    std::ignore =
                        wait_unfull_flush_promise->get_future().
                        finally([FNAME, this] {
    ...
    RecordSubmitter::open_ret RecordSubmitter::open(bool is_mkfs) {
        return journal_allocator.open(is_mkfs).
            safe_then([this] (journal_seq_t ret) {
                      ...
                      DEBUG("{} register metrics", get_name()); stats = {
                      }; namespace sm = seastar::metrics;
                      std::vector < sm::label_instance > label_instances;
                      label_instances.
                      push_back(sm::label_instance("submitter", get_name()));
                      metrics.add_group("journal", {
                                        ...
                                        sm::make_counter("record_batch_num",
                                                         ...
                                                         sm::
                                                         description
                                                         ("total number of records batched"),
                                                         label_instances),
                                        sm::make_counter("io_num",
                                                         stats.io_depth_stats.
                                                         num_io,
                                                         sm::
                                                         description
                                        ...
                                        ("record_group_metadata_bytes",
                                         ...
                                         label_instances),
                                        sm::
                                        make_counter("record_group_data_bytes",
                                                     stats.
                                                     record_group_data_bytes,
                                                     sm::
                                                     description
                                                     ("bytes of data when write record groups"),
                                                     label_instances),}
                      ); return ret;}
        );
    }
    ...
    void RecordSubmitter::decrement_io_with_flush() {
        ...
        if (prv_state == state_t::FULL) {
            if (wait_unfull_flush_promise.has_value()) {
                DEBUG("{} flush, resolve wait_unfull_flush_promise",
                      get_name());
                assert(!p_current_batch->is_empty());
        ...
        auto needs_flush = (!p_current_batch->is_empty()
                            && (state == state_t::IDLE
                                || p_current_batch->get_submit_size().
                                get_fullness() > preferred_fullness
    ...
    void RecordSubmitter::account_submission(std::size_t num,
    ...
    void RecordSubmitter::finish_submit_batch(RecordBatch * p_batch,
    ...
    void RecordSubmitter::flush_current_batch() {
        ...
        auto[to_write, sizes] =
            p_batch->encode_batch(get_committed_to(),
        ...
        std::ignore = journal_allocator.write(std::move(to_write)
            ...
            safe_then([this, p_batch, FNAME, num, sizes =
                       sizes] (auto write_result) {
                      TRACE("{} {} records, {}, write done with {}", get_name(),
                            num, sizes, write_result);
                      finish_submit_batch(p_batch,
                                          write_result);}).
            handle_error(crimson::ct_error::
                         all_same_way([this, p_batch, FNAME, num, sizes =
                                       sizes] (auto e) {
            ...
            handle_exception([this, p_batch, FNAME, num, sizes =
                              sizes] (auto e) {
