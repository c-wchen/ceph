    ...
    namespace io {
        ...
        :public boost::static_visitor < ObjectDispatchSpec::DiscardRequest * > {
            ObjectDispatchSpec::DiscardRequest *
                operator() (ObjectDispatchSpec::
                            DiscardRequest & discard) const {
                return &discard;
            } template < typename T >
                ObjectDispatchSpec::DiscardRequest * operator() (T & t) const {
                return nullptr;
        }};
...
namespace librbd {
    namespace operation {
        ...
        class TestMockOperationTrimRequest:public TestMockFixture {
          public:
            ...
            void expect_object_map_update(MockTestImageCtx & mock_image_ctx,
                                          ...
                                          uint8_t current_state, bool updated,
                                          int ret_val) {
            ...
            void expect_reduce_parent_overlap(MockTestImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(mock_image_ctx,
                            reduce_parent_overlap(overlap, false))
                    .
                    WillOnce(Return
                             (std::make_pair(overlap, io::ImageArea::DATA)));
            }

            void expect_get_area_size(MockTestImageCtx & mock_image_ctx) {
                EXPECT_CALL(mock_image_ctx,
                            get_area_size(io::ImageArea::CRYPTO_HEADER))
                    .WillOnce(Return(0));
            }
                ...
                EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
                    ...
                    WillOnce(Invoke
                             ([&mock_image_ctx, offset, length,
                              ...
                              mock_image_ctx.op_work_queue->queue(&spec->
                                                                  dispatcher_ctx,
                                                                  r);}
                             )) ;
            }
        ...
        TEST_F(TestMockOperationTrimRequest, SuccessRemove) {
            ...
            MockTrimRequest *req =
                new MockTrimRequest(mock_image_ctx, &cond_ctx, m_image_size, 0,
                                    progress_ctx);
            {
        ...
        TEST_F(TestMockOperationTrimRequest, SuccessCopyUp) {
            ...
            ASSERT_EQ(0,
                      librbd::clone(m_ioctx, m_image_name.c_str(), "snap1",
                                    m_ioctx, clone_name.c_str(), features,
                                    &order, 0, 0));
            ...
            // pre
            expect_object_map_update(mock_image_ctx, 0, 2, OBJECT_PENDING,
                                     OBJECT_EXISTS, true, 0);
            ...
            expect_reduce_parent_overlap(mock_image_ctx,
                                         ictx->get_object_size());
            expect_get_object_name(mock_image_ctx, 0, "object0");
            ...
            librbd::NoOpProgressContext progress_ctx;
            MockTrimRequest *req =
                new MockTrimRequest(mock_image_ctx, &cond_ctx,
                                    2 * ictx->get_object_size(), 0,
                                    progress_ctx);
            {
                std::shared_lock owner_locker {
                mock_image_ctx.owner_lock};
        ...
        TEST_F(TestMockOperationTrimRequest, SuccessBoundary) {
            ...
            MockTrimRequest *req =
                new MockTrimRequest(mock_image_ctx, &cond_ctx,
                                    ictx->get_object_size(), 1, progress_ctx);
            {
                std::shared_lock owner_locker {
                mock_image_ctx.owner_lock};
                req->send();
