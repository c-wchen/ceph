...
} PG::do_osd_ops_params_t WatchTimeoutRequest::get_do_osd_ops_params() const
{
    ...
    PG::do_osd_ops_params_t params {
        watch->conn,
        reqid,
        ceph_clock_now(), get_pg().get_osdmap_epoch(), entity_inst_t {
            watch->entity_name, watch->winfo.addr}, 0};
...
Watch::~Watch()
{
    logger().debug("{} gid={} cookie={}", __func__, get_watcher_gid(),
                   get_cookie());
}
...
seastar::future <> Watch::connect(crimson::net::ConnectionRef conn, bool)
{
    ...
    timeout_timer.arm(std::chrono::seconds {
        winfo.timeout_seconds}
                     );
...
{
    logger().info("{} for notify(id={})", __func__,
                  notify->ninfo.notify_id);
    return conn->send(crimson::make_message < MWatchNotify >
                      (winfo.cookie, notify->user_version,
                       notify->ninfo.notify_id, CEPH_WATCH_EVENT_NOTIFY,
                       notify->ninfo.bl, notify->client_gid));
}

seastar::future <> Watch::start_notify(NotifyRef notify)
{
    logger().info("{} adding notify(id={})", __func__,
                  notify->ninfo.notify_id);
...
seastar::future <> Watch::notify_ack(const uint64_t notify_id,
                                     const ceph::bufferlist &reply_bl)
{
    ...
    return seastar::do_for_each(in_progress_notifies,
                                [this_shared =
                                     shared_from_this(),
    reply_bl](auto notify) {
        return notify->complete_watcher(this_shared,
                                        reply_bl);
    }
    ).then([this] {
        in_progress_notifies.clear(); return seastar::now();
    }
                                     );
}
...
seastar::future <> Watch::remove()
{
    ...
    shared_from_this()](auto notify) {
        ...
        discard_state(); return seastar::now();
    }
                                     );
}
...
Notify::Notify(crimson::net::ConnectionRef conn,
    ...
    :
    ninfo(ninfo),
    conn(std::move(conn)), client_gid(client_gid), user_version(user_version)
{
}
...
seastar::future <> Notify::complete_watcher(WatchRef watch,
        const ceph::
        bufferlist &reply_bl)
{
    if (discarded || complete) {
    ...
    notify_replies.emplace(notify_reply_t {
        ...
        watch->get_cookie(), reply_bl}
                          );
    return remove_watcher(std::move(watch));
}

seastar::future <> Notify::send_completion(std::set < WatchRef >
        timedout_watchers)
{
    logger().info("{} -- {} in progress watchers, timedout watchers {}",
                  __func__, watchers.size(), timedout_watchers.size());
    logger().debug("{} sending notify replies: {}", __func__,
                   notify_replies);

    ceph::bufferlist empty;
    auto reply = crimson::make_message < MWatchNotify > (ninfo.cookie,
                 user_version,
                 ninfo.notify_id,
    ...
    ceph::bufferlist reply_bl;
    {
        ...
        boost::insert(missed, std::begin(missed),
                      timedout_watchers | boost::adaptors::
        transformed([](auto w) {
...
void Notify::do_notify_timeout()
{
    ...
    for (auto &watcher : watchers) {
        logger().debug("canceling watcher cookie={} gid={} use_count={}",
                       watcher->get_cookie(),
                       watcher->get_watcher_gid(), watcher->use_count());
...
template <> struct fmt::formatter <
    crimson::osd::WatchTimeoutRequest > : fmt::ostream_formatter {
};
#endif
