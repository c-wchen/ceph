...
unsigned int ErasureCodeIsa::get_chunk_size(unsigned int object_size) const const
{
...
void ErasureCodeIsaDefault::isa_encode(char **data,
                                       char **coding, int blocksize)
{

    if (m == 1)
        // single parity stripe
        region_xor((unsigned char **)data, (unsigned char *)coding[0], k,
                   blocksize);
    else
...
int ErasureCodeIsaDefault::isa_decode(int *erasures,
                                      char **data, char **coding, int blocksize)
{
    int nerrs = 0;
    int i, r, s;
    ...
    // ---------------------------------------------
    if (!tcache.
        getDecodingTableFromCache(erasure_signature, p_tbls, matrixtype, k,
                                  m)) {
        int j;
        ...
        tcache.putDecodingTableToCache(erasure_signature, p_tbls, matrixtype, k,
                                       m);
    }
...
unsigned ErasureCodeIsaDefault::get_alignment() const const
{
...
int ErasureCodeIsaDefault::parse(ErasureCodeProfile & profile, ostream * ss)
{
    ...
    if (matrixtype == kVandermonde) {
        ...
        if (k > 32) {
            *ss << "Vandermonde: m=" << m
                << " should be less/equal than 32 : revert to k=32" << std::
                endl;
            k = 32;
            err = -EINVAL;
        }
