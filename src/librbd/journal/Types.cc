...
{
public:
    template < typename T > inline E operator()(const T &)const
    {
        return T::TYPE;
    }
};
...
} void AioDiscardEvent::decode(__u8 version,
...
void AioDiscardEvent::dump(Formatter *f) const
{
    ...
    f->dump_unsigned("discard_granularity_bytes",
                     discard_granularity_bytes);
...
void AioWriteEvent::encode(bufferlist &bl) const
{
    ...
    encode(length, bl);
    encode(data, bl);
} void AioWriteEvent::decode(__u8 version,
                             bufferlist::const_iterator &it)
{
    using ceph::decode;
    decode(offset, it);
...
} void AioWriteSameEvent::encode(bufferlist &bl) const
{
    ...
    encode(length, bl);
    encode(data, bl);
} void AioWriteSameEvent::decode(__u8 version,
                                 bufferlist::const_iterator &it)
{
    using ceph::decode;
    decode(offset, it);
...
} void AioCompareAndWriteEvent::decode(__u8 version,
                                       bufferlist::
                                       const_iterator &it)
{
    using ceph::decode;
...
{
} void AioFlushEvent::decode(__u8 version,
                             bufferlist::const_iterator &it)
{
}
...
} void OpEventBase::encode(bufferlist &bl) const
{
    using ceph::encode;
    encode(op_tid, bl);
} void OpEventBase::decode(__u8 version,
                           bufferlist::const_iterator &it)
{
    using ceph::decode;
    decode(op_tid, it);
...
} void OpFinishEvent::encode(bufferlist &bl) const
{
    ...
    encode(op_tid, bl);
    encode(r, bl);
} void OpFinishEvent::decode(__u8 version,
                             bufferlist::const_iterator &it)
{
...
} void SnapEventBase::decode(__u8 version,
                             bufferlist::const_iterator &it)
{
    using ceph::decode;
    OpEventBase::decode(version, it);
...
} void SnapCreateEvent::decode(__u8 version,
                               bufferlist::const_iterator &it)
{
    using ceph::decode;
...
} void SnapLimitEvent::encode(bufferlist &bl) const
{
    ...
    using ceph::encode;
    encode(limit, bl);
} void SnapLimitEvent::decode(__u8 version,
                              bufferlist::const_iterator &it)
{
    OpEventBase::decode(version, it);
    using ceph::decode;
...
} void SnapRenameEvent::decode(__u8 version,
                               bufferlist::const_iterator &it)
{
    using ceph::decode;
    OpEventBase::decode(version, it);
...
} void RenameEvent::decode(__u8 version,
                           bufferlist::const_iterator &it)
{
    OpEventBase::decode(version, it);
    using ceph::decode;
...
} void ResizeEvent::encode(bufferlist &bl) const
{
    ...
    using ceph::encode;
    encode(size, bl);
} void ResizeEvent::decode(__u8 version,
                           bufferlist::const_iterator &it)
{
    OpEventBase::decode(version, it);
    using ceph::decode;
...
{
} void DemotePromoteEvent::decode(__u8 version,
                                  bufferlist::const_iterator &it)
{
}
...
} void UpdateFeaturesEvent::encode(bufferlist &bl) const
{
    ...
    encode(features, bl);
    encode(enabled, bl);
} void UpdateFeaturesEvent::decode(__u8 version,
                                   bufferlist::const_iterator &it)
{
    OpEventBase::decode(version, it);
    using ceph::decode;
...
} void MetadataSetEvent::encode(bufferlist &bl) const
{
    ...
    encode(key, bl);
    encode(value, bl);
} void MetadataSetEvent::decode(__u8 version,
                                bufferlist::const_iterator &it)
{
    OpEventBase::decode(version, it);
    using ceph::decode;
...
} void MetadataRemoveEvent::encode(bufferlist &bl) const
{
    ...
    using ceph::encode;
    encode(key, bl);
} void MetadataRemoveEvent::decode(__u8 version,
                                   bufferlist::const_iterator &it)
{
    OpEventBase::decode(version, it);
    using ceph::decode;
...
{
    ceph_abort();
} void UnknownEvent::decode(__u8 version,
                            bufferlist::const_iterator &it)
{
}
...
void EventEntry::generate_test_instances(std::list < EventEntry * > &o)
{
    ...
    o.push_back(new
                EventEntry(AioDiscardEvent(123, 345, 4096),
                           utime_t(1, 1)));
    ...
    o.push_back(new
                EventEntry(AioWriteEvent(123, 456, bl), utime_t(1, 1)));
    ...
    o.push_back(new
                EventEntry(SnapCreateEvent
                           (234, cls::rbd::UserSnapshotNamespace(),
                            "snap"), utime_t(1, 1)));

    o.push_back(new EventEntry(SnapRemoveEvent()));
    o.push_back(new
                EventEntry(SnapRemoveEvent
                           (345, cls::rbd::UserSnapshotNamespace(),
                            "snap"), utime_t(1, 1)));

    o.push_back(new EventEntry(SnapRenameEvent()));
    o.push_back(new
                EventEntry(SnapRenameEvent
                           (456, 1, "src snap", "dest snap"), utime_t(1,
                                   1)));

    o.push_back(new EventEntry(SnapProtectEvent()));
    o.push_back(new
                EventEntry(SnapProtectEvent
                           (567, cls::rbd::UserSnapshotNamespace(),
                            "snap"), utime_t(1, 1)));

    o.push_back(new EventEntry(SnapUnprotectEvent()));
    o.push_back(new
                EventEntry(SnapUnprotectEvent
                           (678, cls::rbd::UserSnapshotNamespace(),
                            "snap"), utime_t(1, 1)));

    o.push_back(new EventEntry(SnapRollbackEvent()));
    o.push_back(new
                EventEntry(SnapRollbackEvent
                           (789, cls::rbd::UserSnapshotNamespace(),
                            "snap"), utime_t(1, 1)));

    o.push_back(new EventEntry(RenameEvent()));
    o.push_back(new
                EventEntry(RenameEvent(890, "image name"),
                           utime_t(1, 1)));
    ...
    o.push_back(new
                EventEntry(UpdateFeaturesEvent(123, 127, true),
                           utime_t(1, 1)));

    o.push_back(new EventEntry(MetadataSetEvent()));
    o.push_back(new
                EventEntry(MetadataSetEvent(123, "key", "value"),
                           utime_t(1, 1)));

    o.push_back(new EventEntry(MetadataRemoveEvent()));
    o.push_back(new
                EventEntry(MetadataRemoveEvent(123, "key"),
                           utime_t(1, 1)));
}
...
} void ImageClientMeta::decode(__u8 version,
                               bufferlist::const_iterator &it)
{
    using ceph::decode;
    decode(tag_class, it);
...
} void MirrorPeerSyncPoint::encode(bufferlist &bl) const
{
    ...
    encode(snap_namespace, bl);
} void MirrorPeerSyncPoint::decode(__u8 version,
                                   bufferlist::const_iterator &it)
{
    using ceph::decode;
    decode(snap_name, it);
...
void MirrorPeerClientMeta::decode(__u8 version,
                                  bufferlist::const_iterator &it)
{
    using ceph::decode;
    decode(image_id, it);
...
{
} void CliClientMeta::decode(__u8 version,
                             bufferlist::const_iterator &it)
{
}
...
{
    ceph_abort();
} void UnknownClientMeta::decode(__u8 version,
                                 bufferlist::const_iterator &it)
{
}
...
{
    return boost::apply_visitor(GetTypeVisitor < ClientMetaType > (),
                                client_meta);
...
{
    boost::apply_visitor(DumpVisitor(f, "client_meta_type"),
                         client_meta);
...
} void TagData::generate_test_instances(std::list < TagData * > &o)
{
    ...
    o.push_back(new
                TagData("mirror-uuid", "remote-mirror-uuid", true, 123,
                        234));
}
...
std::ostream &operator<<(std::ostream &out,
                         const MirrorPeerSyncPoint &sync)
{
    out << "[snap_name=" << sync.
        snap_name << ", " << "from_snap_name=" << sync.from_snap_name;
...
{
    out << "[image_id=" << meta.image_id << ", " << "state=" << meta.
        state << ", " << "sync_object_count=" << meta.
        sync_object_count << ", " << "sync_points=[";
...
std::ostream &operator<<(std::ostream &out,
                         const TagPredecessor &predecessor)
{
