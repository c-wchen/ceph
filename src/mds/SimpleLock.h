...
class SimpleLock {
  ...
  SimpleLock(MDSCacheObject * o, LockType * lt):
    ...
    void put_xlock() {
        ceph_assert(state == LOCK_XLOCK || state == LOCK_XLOCKDONE || state == LOCK_XLOCKSNAP || state == LOCK_LOCK_XLOCK || state == LOCK_LOCK ||  /* if we are a leader of a peer */
                    is_locallock());
        --more()->num_xlock;
        parent->put(MDSCacheObject::PIN_LOCK);
        if (more()->num_xlock == 0) {
    ...
    void encode_state_for_replica(ceph::buffer::list & bl) const {
        ...
        using ceph::encode;
        encode(s, bl);
    } void decode_state(ceph::buffer::list::const_iterator & p, bool is_new =
                        true) {
        using ceph::decode;
        __s16 s;
        decode(s, p);
