...
static void get_time_key(real_time & ut, string * key)
{
    ...
    snprintf(buf, 32, "%011llu.%09u", (unsigned long long)ts.tv_sec,
             (unsigned int)ts.tv_nsec);
    *key = buf;
}

static void get_index_ver_key(cls_method_context_t hctx, uint64_t index_ver,
                              string * key)
{
    char buf[48];
...
static void bi_log_index_key(cls_method_context_t hctx, string & key,
                             string & id, uint64_t index_ver)
{
    bi_log_prefix(key);
...
static int log_index_operation(cls_method_context_t hctx,
                               cls_rgw_obj_key & obj_key, RGWModifyOp op,
                               string & tag, real_time & timestamp,
                               rgw_bucket_entry_ver & ver,
                               RGWPendingState state, uint64_t index_ver,
                               string & max_marker, uint16_t bilog_flags,
                               string * owner, string * owner_display_name,
...
static int get_obj_vals(cls_method_context_t hctx, const string & start,
                        const string & filter_prefix, int num_entries,
                        map < string, bufferlist > *pkeys, bool * pmore)
{
    int ret =
        cls_cxx_map_get_vals(hctx, start, filter_prefix, num_entries, pkeys,
                             pmore);
    if (ret < 0)
        return ret;
    ...
    auto comp =
        [](const pair < string, bufferlist > &l,
           const string & r){ return l.first < r;
    };
    string new_start = { static_cast < char >(BI_PREFIX_CHAR + 1) };

    auto lower =
        pkeys->lower_bound(string { static_cast < char >(BI_PREFIX_CHAR) });
    ...
    ret =
        cls_cxx_map_get_vals(hctx, new_start, filter_prefix,
...
static void get_list_index_key(struct rgw_bucket_dir_entry &entry,
                               string * index_key)
{
...
static void encode_obj_index_key(const cls_rgw_obj_key & key,
                                 string * index_key)
{
...
template < class T >
    static int read_index_entry(cls_method_context_t hctx, string & name,
                                T * entry);

static int encode_list_index_key(cls_method_context_t hctx,
                                 ...
                                 string * index_key)
{
    ...
    if (ret < 0) {
        CLS_LOG(1,
                "ERROR: encode_list_index_key(): cls_cxx_map_get_val returned %d\n",
                ret);
        return ret;
    }
 ...
 */
static int decode_list_index_key(const string & index_key,
                                 cls_rgw_obj_key * key, uint64_t * ver)
{
    ...
    *ver = 0;
    ...
    if (iter == vals.end()) {
        CLS_LOG(0, "ERROR: %s(): bad index_key (%s): no vals", __func__,
...
int rgw_bucket_list(cls_method_context_t hctx, bufferlist * in,
                    bufferlist * out)
{
    ...
    do {
        rc = get_obj_vals(hctx, start_key, op.filter_prefix, left_to_read,
                          &keys, &more);
        if (rc < 0)
            return rc;
        ...
        for (kiter = keys.begin(); kiter != keys.end(); ++kiter) {
            ...
            CLS_LOG(20, "start_key=%s len=%zu", start_key.c_str(),
                    start_key.size());
...
#define CHECK_CHUNK_SIZE 1000
    ...
    do {
        rc = get_obj_vals(hctx, start_obj, filter_prefix, CHECK_CHUNK_SIZE,
                          &keys, &more);
        if (rc < 0)
            return rc;
...
int rgw_bucket_check_index(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
...
static int write_bucket_header(cls_method_context_t hctx,
                               struct rgw_bucket_dir_header *header)
{
    header->ver++;
...
int rgw_bucket_rebuild_index(cls_method_context_t hctx, bufferlist * in,
                             bufferlist * out)
{
    struct rgw_bucket_dir_header existing_header;
    struct rgw_bucket_dir_header calc_header;
...
int rgw_bucket_update_stats(cls_method_context_t hctx, bufferlist * in,
                            bufferlist * out)
{
    // decode request
...
int rgw_bucket_init_index(cls_method_context_t hctx, bufferlist * in,
                          bufferlist * out)
{
...
int rgw_bucket_set_tag_timeout(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    // decode request
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: rgw_bucket_set_tag_timeout(): failed to decode request\n");
        return -EINVAL;
    }
    ...
    if (rc < 0) {
        CLS_LOG(1,
                "ERROR: rgw_bucket_set_tag_timeout(): failed to read header\n");
        return rc;
    }
...
static int read_key_entry(cls_method_context_t hctx, cls_rgw_obj_key & key,
                          string * idx, struct rgw_bucket_dir_entry *entry,
                          bool special_delete_marker_name = false);

int rgw_bucket_prepare_op(cls_method_context_t hctx, bufferlist * in,
                          bufferlist * out)
{
    // decode request
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: rgw_bucket_prepare_op(): failed to decode request\n");
        return -EINVAL;
    }
    ...
    info.op = op.op;
    entry.pending_map.insert(pair < string,
                             rgw_bucket_pending_info > (op.tag, info));
    ...
    if (op.log_op && !header.syncstopped) {
        rc = log_index_operation(hctx, op.key, op.op, op.tag, entry.meta.mtime,
                                 entry.ver, info.state, header.ver,
...
static void unaccount_entry(struct rgw_bucket_dir_header &header,
                            struct rgw_bucket_dir_entry &entry)
{
...
static void log_entry(const char *func, const char *str,
                      struct rgw_bucket_dir_entry *entry)
{
    CLS_LOG(1, "%s(): %s: ver=%ld:%llu name=%s instance=%s locator=%s\n", func,
            str, (long)entry->ver.pool, (unsigned long long)entry->ver.epoch,
            entry->key.name.c_str(), entry->key.instance.c_str(),
            entry->locator.c_str());
}

static void log_entry(const char *func, const char *str,
                      struct rgw_bucket_olh_entry *entry)
{
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    // decode request
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: rgw_bucket_complete_op(): failed to decode request\n");
        return -EINVAL;
    }
    CLS_LOG(1,
    ...
    if (op.tag.size()) {
        map < string, struct rgw_bucket_pending_info >::iterator pinter =
    ...
    if (cancel) {
        if (op.log_op && !header.syncstopped) {
            rc = log_index_operation(hctx, op.key, op.op, op.tag,
                                     entry.meta.mtime, entry.ver,
    ...
    for (remove_iter = op.remove_objs.begin();
         remove_iter != op.remove_objs.end(); ++remove_iter) {
        cls_rgw_obj_key & remove_key = *remove_iter;
        CLS_LOG(1,
                "rgw_bucket_complete_op(): removing entries, read_index_entry name=%s instance=%s\n",
        ...
        if (ret < 0) {
            CLS_LOG(1,
                    "rgw_bucket_complete_op(): removing entries, read_index_entry name=%s instance=%s ret=%d\n",
        ...
        CLS_LOG(0,
                ...
                remove_entry.key.instance.c_str(), remove_entry.meta.category);
        ...
        if (ret < 0) {
            CLS_LOG(1,
                    "rgw_bucket_complete_op(): cls_cxx_map_remove_key, failed to remove entry, name=%s instance=%s read_index_entry ret=%d\n",
...
template < class T >
    static int write_entry(cls_method_context_t hctx, T & entry,
                           const string & key)
{
    bufferlist bl;
    ::encode(entry, bl);
...
static int read_olh(cls_method_context_t hctx, cls_rgw_obj_key & obj_key,
                    ...
                    string * index_key, bool * found)
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        CLS_LOG(0, "ERROR: read_index_entry() olh_key=%s ret=%d",
                olh_key.name.c_str(), ret);
        return ret;
    }
    if (found) {
        *found = (ret != -ENOENT);
...
static void update_olh_log(struct rgw_bucket_olh_entry &olh_data_entry,
                           OLHLogOp op, const string & op_tag,
                           cls_rgw_obj_key & key, bool delete_marker,
                           uint64_t epoch)
{
    vector < rgw_bucket_olh_log_entry > &log =
        olh_data_entry.pending_log[olh_data_entry.epoch];
...
static int write_obj_instance_entry(cls_method_context_t hctx,
                                    struct rgw_bucket_dir_entry &instance_entry,
                                    const string & instance_idx)
{
    CLS_LOG(20, "write_entry() instance=%s idx=%s flags=%d",
    ...
    if (ret < 0) {
        CLS_LOG(0, "ERROR: write_entry() instance_key=%s ret=%d",
 ...
 */
static int write_obj_entries(cls_method_context_t hctx,
                             struct rgw_bucket_dir_entry &instance_entry,
                             const string & instance_idx)
{
    ...
    if (instance_idx != instance_list_idx) {
        CLS_LOG(20, "write_entry() idx=%s flags=%d",
...
class BIVerObjEntry {
  ...
  public:
     BIVerObjEntry(cls_method_context_t & _hctx,
                   const cls_rgw_obj_key & _key):hctx(_hctx), key(_key),
        initialized(false) {
        ...
        if (ret < 0) {
            CLS_LOG(0, "ERROR: read_key_entry() idx=%s ret=%d",
        ...
        CLS_LOG(20, "read instance_entry key.name=%s key.instance=%s flags=%d",
                ...
                instance_entry.key.instance.c_str(), instance_entry.flags);
        return 0;
    }
    ...
    int unlink_list_entry() {
        ...
        CLS_LOG(20, "unlink_list_entry() list_idx=%s",
    ...
    int unlink() {
        ...
        if (ret < 0) {
            CLS_LOG(0, "ERROR: cls_cxx_map_remove_key() instance_idx=%s ret=%d",
                    instance_idx.c_str(), ret);
            return ret;
        }
        return 0;
    ...
    int write_entries(uint64_t flags_set, uint64_t flags_reset) {
        ...
        bool special_delete_marker_key = (instance_entry.is_delete_marker()
                                          && instance_entry.key.instance.
                                          empty());
        ...
        if (ret < 0) {
            CLS_LOG(0, "ERROR: write_obj_entries() instance_idx=%s ret=%d",
                    instance_idx.c_str(), ret);
            return ret;
        }
    ...
    int write(uint64_t epoch, bool current) {
        if (instance_entry.versioned_epoch > 0) {
            CLS_LOG(20, "%s(): instance_entry.versioned_epoch=%d epoch=%d",
                    __func__, (int)instance_entry.versioned_epoch, (int)epoch);
            /* this instance has a previous list entry, remove that entry */
    ...
    int find_next_key(cls_rgw_obj_key * next_key, bool * found) {
        ...
        try {
            ...
            CLS_LOG(0, "ERROR; failed to decode entry: %s",
                    last->first.c_str());
            return -EIO;
        }
...
class BIOLHEntry {
    ...
    bool initialized;
  public:
     BIOLHEntry(cls_method_context_t & _hctx,
                const cls_rgw_obj_key & _key):hctx(_hctx), key(_key),
        initialized(false) {
    ...
    int write() {
        ...
        if (ret < 0) {
            CLS_LOG(0, "ERROR: write_entry() olh_key=%s ret=%d",
                    olh_data_idx.c_str(), ret);
            return ret;
        }
    ...
    void update_log(OLHLogOp op, const string & op_tag, cls_rgw_obj_key & key,
...
static int write_version_marker(cls_method_context_t hctx,
                                cls_rgw_obj_key & key)
{
    struct rgw_bucket_dir_entry entry;
    entry.key = key;
 ...
 */
static int rgw_bucket_link_olh(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    string olh_data_idx;
    string instance_idx;
    ...
    try {
        ...
        CLS_LOG(0,
                "ERROR: rgw_bucket_link_olh_op(): failed to decode request\n");
        return -EINVAL;
    }
    ...
    if (!olh.start_modify(op.olh_epoch)) {
        ...
        if (removing) {
            olh.update_log(CLS_RGW_OLH_OP_REMOVE_INSTANCE, op.op_tag, op.key,
                           false, op.olh_epoch);
        }
        return 0;
    }
    ...
    if (olh_found) {
        ...
        if (olh.exists()) {
            ...
            if (!(olh_entry.key == op.key)) {
                ...
                if (ret < 0) {
                    CLS_LOG(0,
                            "ERROR: could not demote current on previous key ret=%d",
                            ret);
                    return ret;
                }
            }
    ...
    if (ret < 0) {
        CLS_LOG(1,
                "ERROR: rgw_bucket_unlink_instance(): failed to read header\n");
        return ret;
    }
    ...
    if (op.log_op && !header.syncstopped) {
        ...
        ret =
            log_index_operation(hctx, op.key, operation, op.op_tag,
                                entry.meta.mtime, ver, CLS_RGW_STATE_COMPLETE,
...
static int rgw_bucket_unlink_instance(cls_method_context_t hctx,
                                      bufferlist * in, bufferlist * out)
{
    ...
    if (olh_key == dest_key) {
        ...
        if (found) {
            ...
            olh.update_log(CLS_RGW_OLH_OP_LINK_OLH, op.op_tag, next_key,
                           next.is_delete_marker());
        }
        else {
            /* next_key is empty */
            olh.update(next_key, false);
            olh.update_log(CLS_RGW_OLH_OP_UNLINK_OLH, op.op_tag, next_key,
                           false);
    ...
    if (op.log_op && !header.syncstopped) {
        ...
        ret =
            log_index_operation(hctx, op.key, CLS_RGW_OP_UNLINK_INSTANCE,
                                op.op_tag, mtime, ver, CLS_RGW_STATE_COMPLETE,
                                header.ver, header.max_marker,
                                op.bilog_flags | RGW_BILOG_FLAG_VERSIONED_OP,
                                   ...
                                   bufferlist * out)
{
    ...
    try {
        ...
        CLS_LOG(0,
                "ERROR: rgw_bucket_read_olh_log(): failed to decode request\n");
        return -EINVAL;
    }
...
#define MAX_OLH_LOG_ENTRIES 1000
    ...
    if (log.begin()->first > op.ver_marker && log.size() <= MAX_OLH_LOG_ENTRIES) {
        ...
        for (int i = 0; i < MAX_OLH_LOG_ENTRIES && iter != log.end();
             ++i, ++iter) {
...
static int rgw_bucket_trim_olh_log(cls_method_context_t hctx, bufferlist * in,
                                   bufferlist * out)
{
    // decode request
    ...
    try {
        ...
        CLS_LOG(0,
                "ERROR: rgw_bucket_trim_olh_log(): failed to decode request\n");
        return -EINVAL;
    }
    ...
    map < uint64_t, vector < rgw_bucket_olh_log_entry > >&log =
        olh_data_entry.pending_log;
    map < uint64_t, vector < rgw_bucket_olh_log_entry > >::iterator liter =
        log.begin();
    while (liter != log.end() && liter->first <= op.ver) {
        map < uint64_t,
            vector < rgw_bucket_olh_log_entry > >::iterator rm_iter = liter;
        ++liter;
        log.erase(rm_iter);
    }
...
static int rgw_bucket_clear_olh(cls_method_context_t hctx, bufferlist * in,
                                bufferlist * out)
{
    // decode request
    ...
    if (ret < 0) {
        CLS_LOG(1, "NOTICE: %s(): can't remove key %s ret=%d", __func__,
                olh_data_key.c_str(), ret);
        return ret;
    }
    ...
    if (ret < 0) {
        CLS_LOG(0, "ERROR: read_index_entry key=%s ret=%d", op.key.name.c_str(),
                ret);
        return ret;
    }
                            ...
                            bufferlist * in, bufferlist * out)
{
    ...
    timespan tag_timeout(std::chrono::
                         seconds(header.tag_timeout ? header.
                                 tag_timeout : CEPH_RGW_TAG_TIMEOUT));
    ...
    while (!in_iter.end()) {
        ...
        try {
            ...
            CLS_LOG(1,
                    "ERROR: rgw_dir_suggest_changes(): failed to decode request\n");
            return -EINVAL;
        }
        ...
        if (cur_disk_bl.length()) {
            ...
            try {
                ...
                CLS_LOG(1,
                        "ERROR: rgw_dir_suggest_changes(): failed to decode cur_disk\n");
                return -EINVAL;
            }
            ...
            while (iter != cur_disk.pending_map.end()) {
                map < string,
                    struct rgw_bucket_pending_info >::iterator cur_iter =
                    iter++;
                if (cur_time >
                    (cur_iter->second.timestamp + timespan(tag_timeout))) {
        ...
        if (cur_disk.pending_map.empty()) {
            if (cur_disk.exists) {
                struct rgw_bucket_category_stats &old_stats =
            ...
            case CEPH_RGW_REMOVE:
                ...
                if (log_op && cur_disk.exists && !header.syncstopped) {
                    ret =
                        log_index_operation(hctx, cur_disk.key, CLS_RGW_OP_DEL,
                                            cur_disk.tag, cur_disk.meta.mtime,
                                            cur_disk.ver,
                                            CLS_RGW_STATE_COMPLETE, header.ver,
                                            header.max_marker, 0, NULL, NULL,
                                            NULL);
                    if (ret < 0) {
                        CLS_LOG(0,
                                "ERROR: %s(): failed to log operation ret=%d",
                                __func__, ret);
                        return ret;
                    }
                }
            ...
            case CEPH_RGW_UPDATE:
                CLS_LOG(10,
                        "CEPH_RGW_UPDATE name=%s instance=%s total_entries: %"
                        PRId64 " -> %" PRId64 "\n", cur_change.key.name.c_str(),
                        cur_change.key.instance.c_str(), stats.num_entries,
                        stats.num_entries + 1);
                ...
                if (log_op && !header.syncstopped) {
                    ret =
                        log_index_operation(hctx, cur_change.key,
                                            CLS_RGW_OP_ADD, cur_change.tag,
...
static int rgw_obj_remove(cls_method_context_t hctx, bufferlist * in,
                          bufferlist * out)
{
    // decode request
    ...
    if (ret < 0 && ret != -ENOENT) {
        CLS_LOG(0, "ERROR: %s(): cls_cxx_getxattrs() returned %d", __func__,
                ret);
        return ret;
    }
    ...
    for (list < string >::iterator iter = op.keep_attr_prefixes.begin();
        ...
        for (map < string, bufferlist >::iterator aiter =
             attrset.lower_bound(check_prefix); aiter != attrset.end();
             ++aiter) {
    ...
    for (map < string, bufferlist >::iterator aiter = new_attrs.begin();
        ...
        if (ret < 0) {
            CLS_LOG(0, "ERROR: %s(): cls_cxx_setxattr (attr=%s) returned %d",
                    __func__, attr.c_str(), ret);
            return ret;
        }
    }
...
static int rgw_obj_store_pg_ver(cls_method_context_t hctx, bufferlist * in,
                                bufferlist * out)
{
    // decode request
...
static int rgw_obj_check_attrs_prefix(cls_method_context_t hctx,
                                      bufferlist * in, bufferlist * out)
{
    // decode request
    ...
    for (map < string, bufferlist >::iterator aiter =
         attrset.lower_bound(op.check_prefix); aiter != attrset.end();
         ++aiter) {
        const string & attr = aiter->first;
...
static int rgw_obj_check_mtime(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    // decode request
...
static int rgw_bi_get_op(cls_method_context_t hctx, bufferlist * in,
                         bufferlist * out)
{
    // decode request
...
static int rgw_bi_put_op(cls_method_context_t hctx, bufferlist * in,
                         bufferlist * out)
{
    // decode request
    ...
    if (r < 0) {
        CLS_LOG(0, "ERROR: %s(): cls_cxx_map_set_val() returned r=%d", __func__,
                r);
    }
...
static int list_plain_entries(cls_method_context_t hctx, const string & name,
...
static int list_instance_entries(cls_method_context_t hctx, const string & name,
                                 const string & marker, uint32_t max,
                                 ...
                                 bool * pmore)
{
    ...
    if (max > 0) {
        ret =
            cls_cxx_map_get_vals(hctx, start_key, string(), max, &keys, pmore);
        CLS_LOG(20, "%s(): start_key=%s first_instance_idx=%s keys.size()=%d",
                __func__, escape_str(start_key).c_str(),
    ...
    for (iter = keys.begin(); iter != keys.end(); ++iter) {
        ...
        CLS_LOG(20, "%s(): entry.idx=%s", __func__,
...
static int list_olh_entries(cls_method_context_t hctx, const string & name,
                            const string & marker, uint32_t max,
                            list < rgw_cls_bi_entry > *entries, bool * pmore)
{
    cls_rgw_obj_key key(name);
...
static int rgw_bi_list_op(cls_method_context_t hctx, bufferlist * in,
                          bufferlist * out)
{
    // decode request
...
#define MAX_BI_LIST_ENTRIES 1000
    ...
    bool more;
    int ret =
        list_plain_entries(hctx, op.name, op.marker, max, &op_ret.entries,
                           &more);
    if (ret < 0) {
        CLS_LOG(0, "ERROR: %s(): list_plain_entries retured ret=%d", __func__,
                ret);
        return ret;
    }
    int count = ret;
    ...
    if (!more) {
        ret =
            list_instance_entries(hctx, op.name, op.marker, max - count,
                                  &op_ret.entries, &more);
        if (ret < 0) {
            CLS_LOG(0, "ERROR: %s(): list_instance_entries retured ret=%d",
                    __func__, ret);
            return ret;
        }
    ...
    if (!more) {
        ret =
            list_olh_entries(hctx, op.name, op.marker, max - count,
                             &op_ret.entries, &more);
        if (ret < 0) {
            CLS_LOG(0, "ERROR: %s(): list_olh_entries retured ret=%d", __func__,
                    ret);
            return ret;
        }
...
static int bi_log_iterate_entries(cls_method_context_t hctx,
                                  const string & marker,
                                  const string & end_marker, string & key_iter,
                                  uint32_t max_entries, bool * truncated,
                                  int (*cb) (cls_method_context_t,
                                             const string &, rgw_bi_log_entry &,
                                             void *), void *param)
{
    ...
    int ret =
        cls_cxx_map_get_vals(hctx, start_key, filter, max_entries, &keys,
                             truncated);
    if (ret < 0)
        return ret;
...
static int bi_log_list_cb(cls_method_context_t hctx, const string & key,
                          rgw_bi_log_entry & info, void *param)
{
...
static int bi_log_list_entries(cls_method_context_t hctx, const string & marker,
                               uint32_t max, list < rgw_bi_log_entry > &entries,
                               bool * truncated)
{
    string key_iter;
    string end_marker;
...
static int rgw_bi_log_list(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
    ...
    int ret =
        bi_log_list_entries(hctx, op.marker, op.max, op_ret.entries,
                            &op_ret.truncated);
    if (ret < 0)
        return ret;
...
static int bi_log_list_trim_cb(cls_method_context_t hctx, const string & key,
                               rgw_bi_log_entry & info, void *param)
{
...
static int bi_log_remove_entry(cls_method_context_t hctx,
...
static int rgw_bi_log_trim(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
#define MAX_TRIM_ENTRIES 1000   /* don't do more than that in a single operation */
    bool truncated;
    int ret =
        bi_log_list_trim_entries(hctx, op.start_marker, op.end_marker, entries,
                                 &truncated);
    if (ret < 0)
        return ret;
...
static int rgw_bi_log_resync(cls_method_context_t hctx, bufferlist * in,
                             bufferlist * out)
{
    struct rgw_bucket_dir_header header;
    int rc = read_bucket_header(hctx, &header);
...
static int rgw_bi_log_stop(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    struct rgw_bucket_dir_header header;
    int rc = read_bucket_header(hctx, &header);
...
static void usage_record_prefix_by_user(string & user, uint64_t epoch,
                                        string & key)
{
    char buf[user.size() + 32];
    snprintf(buf, sizeof(buf), "%s_%011llu_", user.c_str(),
             (long long unsigned)epoch);
    key = buf;
}

static void usage_record_name_by_time(uint64_t epoch, const string & user,
                                      string & bucket, string & key)
{
    char buf[32 + user.size() + bucket.size()];
    snprintf(buf, sizeof(buf), "%011llu_%s_%s", (long long unsigned)epoch,
             user.c_str(), bucket.c_str());
    key = buf;
}

static void usage_record_name_by_user(const string & user, uint64_t epoch,
                                      string & bucket, string & key)
{
    char buf[32 + user.size() + bucket.size()];
    snprintf(buf, sizeof(buf), "%s_%011llu_%s", user.c_str(),
...
static int usage_record_decode(bufferlist & record_bl, rgw_usage_log_entry & e)
{
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: usage_record_decode(): failed to decode record_bl\n");
        return -EINVAL;
    }
                           ...
                           bufferlist * out)
{
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: rgw_user_usage_log_add(): failed to decode request\n");
        return -EINVAL;
    }
    ...
    for (iter = info.entries.begin(); iter != info.entries.end(); ++iter) {
        ...
        usage_record_name_by_time(entry.epoch, puser->to_str(), entry.bucket,
                                  key_by_time);
        ...
        string key_by_user;
        usage_record_name_by_user(puser->to_str(), entry.epoch, entry.bucket,
                                  key_by_user);
...
static int usage_iterate_range(cls_method_context_t hctx, uint64_t start,
                               uint64_t end, string & user, string & key_iter,
                               uint32_t max_entries, bool * truncated,
...
#define NUM_KEYS 32
    ...
    int ret =
        cls_cxx_map_get_vals(hctx, start_key, filter_prefix, max_entries, &keys,
                             &truncated_status);
    if (ret < 0)
        return ret;
    ...
    *truncated = truncated_status;
    ...
    for (; iter != keys.end(); ++iter, ++i) {
        ...
        if (!by_user && key.compare(end_key) >= 0) {
            CLS_LOG(20, "usage_iterate_range reached key=%s, done",
                    key.c_str());
            *truncated = false;
            key_iter = key;
            return 0;
                            ...
                            bufferlist * out)
{
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: rgw_user_usage_log_read(): failed to decode request\n");
        return -EINVAL;
    }
...
#define MAX_ENTRIES 1000
    ...
    int ret =
        usage_iterate_range(hctx, op.start_epoch, op.end_epoch, op.owner, iter,
                            max_entries, &ret_info.truncated, usage_log_read_cb,
                            (void *)usage);
    if (ret < 0)
        return ret;
                            ...
                            bufferlist * out)
{
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: rgw_user_log_usage_log_trim(): failed to decode request\n");
        return -EINVAL;
    }
...
#define MAX_USAGE_TRIM_ENTRIES 128
    ret =
        usage_iterate_range(hctx, op.start_epoch, op.end_epoch, op.user, iter,
                            MAX_USAGE_TRIM_ENTRIES, &more, usage_log_trim_cb,
                            (void *)&found);
    if (ret < 0)
        return ret;
...
static int gc_omap_get(cls_method_context_t hctx, int type, const string & key,
...
static int gc_omap_set(cls_method_context_t hctx, int type, const string & key,
...
static int gc_omap_remove(cls_method_context_t hctx, int type,
                          const string & key)
{
    string index = gc_index_prefixes[type];
    index.append(key);
...
static int rgw_cls_gc_set_entry(cls_method_context_t hctx, bufferlist * in,
                                bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_cls_gc_defer_entry(cls_method_context_t hctx, bufferlist * in,
                                  bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int gc_iterate_entries(cls_method_context_t hctx, const string & marker,
                              bool expired_only, string & key_iter,
                              uint32_t max_entries, bool * truncated,
                              int (*cb) (cls_method_context_t, const string &,
                                         cls_rgw_gc_obj_info &, void *),
                              void *param)
{
...
static int gc_list_cb(cls_method_context_t hctx, const string & key,
                      cls_rgw_gc_obj_info & info, void *param)
{
...
static int rgw_cls_gc_list(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
#define GC_LIST_ENTRIES_DEFAULT 128
    int ret =
        gc_list_entries(hctx, op.marker,
...
static int rgw_cls_gc_remove(cls_method_context_t hctx, bufferlist * in,
                             bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_cls_lc_set_entry(cls_method_context_t hctx, bufferlist * in,
                                bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_cls_lc_rm_entry(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_cls_lc_get_next_entry(cls_method_context_t hctx, bufferlist * in,
                                     bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
    ...
    if (!vals.empty()) {
        ...
        try {
            ...
            CLS_LOG(1,
                    "ERROR: rgw_cls_lc_get_next_entry(): failed to decode entry\n");
            return -EIO;
        }
    }
    op_ret.entry = entry;
                                   ...
                                   bufferlist * out)
{
    ...
    string filter_prefix;
    int ret =
        cls_cxx_map_get_vals(hctx, op.marker, filter_prefix, op.max_entries,
    ...
    for (it = vals.begin(); it != vals.end(); ++it) {
        ...
        try {
            ...
            CLS_LOG(1,
                    "ERROR: rgw_cls_lc_list_entries(): failed to decode entry\n");
            return -EIO;
        }
...
static int rgw_cls_lc_put_head(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_cls_lc_get_head(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    bufferlist bl;
    ...
    if (bl.length() != 0) {
        ...
        try {
            ...
            CLS_LOG(0,
                    "ERROR: rgw_cls_lc_get_head(): failed to decode entry %s\n",
                    err.what());
            return -EINVAL;
        }
    }
...
static int rgw_reshard_add(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_reshard_list(cls_method_context_t hctx, bufferlist * in,
                            bufferlist * out)
{
...
#define MAX_RESHARD_LIST_ENTRIES 1000
    ...
    int32_t max = (op.max
                   && (op.max <
                       MAX_RESHARD_LIST_ENTRIES) ? op.
                   max : MAX_RESHARD_LIST_ENTRIES);
    int ret =
        cls_cxx_map_get_vals(hctx, op.marker, filter_prefix, max, &vals,
                             &op_ret.is_truncated);
    if (ret < 0)
        return ret;
    map < string, bufferlist >::iterator it;
    ...
    for (it = vals.begin(); i < (int)op.max && it != vals.end(); ++it, ++i) {
        ...
        try {
            ...
            CLS_LOG(1,
                    "ERROR: rgw_cls_rehard_list(): failed to decode entry\n");
            return -EIO;
        }
...
static int get_reshard_entry(cls_method_context_t hctx, const string & key,
                             cls_rgw_reshard_entry * entry)
{
    bufferlist bl;
...
static int rgw_reshard_get(cls_method_context_t hctx, bufferlist * in,
                           bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int rgw_reshard_remove(cls_method_context_t hctx, bufferlist * in,
                              bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
    ...
    if (ret < 0) {
        CLS_LOG(0, "ERROR: failed to remove key: key=%s ret=%d", key.c_str(),
                ret);
        return 0;
    }
    return ret;
...
static int rgw_set_bucket_resharding(cls_method_context_t hctx, bufferlist * in,
                                     bufferlist * out)
{
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: cls_rgw_set_bucket_resharding: failed to decode entry\n");
        return -EINVAL;
    }
    ...
    header.new_instance.set_status(op.entry.new_bucket_instance_id,
                                   op.entry.num_shards,
                                   op.entry.reshard_status);

    return write_bucket_header(hctx, &header);
}

static int rgw_clear_bucket_resharding(cls_method_context_t hctx,
                                       bufferlist * in, bufferlist * out)
{
    ...
    try {
        ...
        CLS_LOG(1,
                "ERROR: cls_rgw_clear_bucket_resharding: failed to decode entry\n");
        return -EINVAL;
    }
...
static int rgw_guard_bucket_resharding(cls_method_context_t hctx,
                                       bufferlist * in, bufferlist * out)
{
...
static int rgw_get_bucket_resharding(cls_method_context_t hctx, bufferlist * in,
                                     bufferlist * out)
{
...
CLS_INIT(rgw)
{
    ...
    /* bucket index */
    cls_register_cxx_method(h_class, RGW_BUCKET_INIT_INDEX,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_init_index, &h_rgw_bucket_init_index);
    cls_register_cxx_method(h_class, RGW_BUCKET_SET_TAG_TIMEOUT,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_set_tag_timeout,
                            &h_rgw_bucket_set_tag_timeout);
    cls_register_cxx_method(h_class, RGW_BUCKET_LIST, CLS_METHOD_RD,
                            rgw_bucket_list, &h_rgw_bucket_list);
    cls_register_cxx_method(h_class, RGW_BUCKET_CHECK_INDEX, CLS_METHOD_RD,
                            rgw_bucket_check_index, &h_rgw_bucket_check_index);
    cls_register_cxx_method(h_class, RGW_BUCKET_REBUILD_INDEX,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_rebuild_index,
                            &h_rgw_bucket_rebuild_index);
    cls_register_cxx_method(h_class, RGW_BUCKET_UPDATE_STATS,
                            CLS_METHOD_RD | CLS_METHOD_WR,
    ...
    cls_register_cxx_method(h_class, RGW_BUCKET_COMPLETE_OP,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_complete_op, &h_rgw_bucket_complete_op);
    cls_register_cxx_method(h_class, RGW_BUCKET_LINK_OLH,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_link_olh,
                            &h_rgw_bucket_link_olh);
    cls_register_cxx_method(h_class, RGW_BUCKET_UNLINK_INSTANCE,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_unlink_instance,
                            &h_rgw_bucket_unlink_instance_op);
    cls_register_cxx_method(h_class, RGW_BUCKET_READ_OLH_LOG, CLS_METHOD_RD,
                            rgw_bucket_read_olh_log,
                            &h_rgw_bucket_read_olh_log);
    cls_register_cxx_method(h_class, RGW_BUCKET_TRIM_OLH_LOG,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_trim_olh_log,
                            &h_rgw_bucket_trim_olh_log);
    cls_register_cxx_method(h_class, RGW_BUCKET_CLEAR_OLH,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_clear_olh,
                            &h_rgw_bucket_clear_olh);

    cls_register_cxx_method(h_class, RGW_OBJ_REMOVE,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_obj_remove,
                            &h_rgw_obj_remove);
    cls_register_cxx_method(h_class, RGW_OBJ_STORE_PG_VER, CLS_METHOD_WR,
                            rgw_obj_store_pg_ver, &h_rgw_obj_store_pg_ver);
    cls_register_cxx_method(h_class, RGW_OBJ_CHECK_ATTRS_PREFIX, CLS_METHOD_RD,
                            rgw_obj_check_attrs_prefix,
                            &h_rgw_obj_check_attrs_prefix);
    cls_register_cxx_method(h_class, RGW_OBJ_CHECK_MTIME, CLS_METHOD_RD,
                            rgw_obj_check_mtime, &h_rgw_obj_check_mtime);

    cls_register_cxx_method(h_class, RGW_BI_GET, CLS_METHOD_RD, rgw_bi_get_op,
                            &h_rgw_bi_get_op);
    cls_register_cxx_method(h_class, RGW_BI_PUT, CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bi_put_op, &h_rgw_bi_put_op);
    cls_register_cxx_method(h_class, RGW_BI_LIST, CLS_METHOD_RD, rgw_bi_list_op,
                            &h_rgw_bi_list_op);

    cls_register_cxx_method(h_class, RGW_BI_LOG_LIST, CLS_METHOD_RD,
                            rgw_bi_log_list, &h_rgw_bi_log_list_op);
    cls_register_cxx_method(h_class, RGW_BI_LOG_TRIM,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_trim,
                            &h_rgw_bi_log_list_op);
    cls_register_cxx_method(h_class, RGW_DIR_SUGGEST_CHANGES,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_dir_suggest_changes,
                            &h_rgw_dir_suggest_changes);

    cls_register_cxx_method(h_class, "bi_log_resync",
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_resync,
                            &h_rgw_bi_log_resync_op);
    cls_register_cxx_method(h_class, "bi_log_stop",
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_stop,
                            &h_rgw_bi_log_stop_op);

    /* usage logging */
    cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_ADD,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_user_usage_log_add, &h_rgw_user_usage_log_add);
    cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_READ, CLS_METHOD_RD,
                            rgw_user_usage_log_read,
                            &h_rgw_user_usage_log_read);
    cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_TRIM,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_user_usage_log_trim,
                            &h_rgw_user_usage_log_trim);

    /* garbage collection */
    cls_register_cxx_method(h_class, RGW_GC_SET_ENTRY,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_gc_set_entry,
                            &h_rgw_gc_set_entry);
    cls_register_cxx_method(h_class, RGW_GC_DEFER_ENTRY,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_cls_gc_defer_entry, &h_rgw_gc_set_entry);
    cls_register_cxx_method(h_class, RGW_GC_LIST, CLS_METHOD_RD,
                            rgw_cls_gc_list, &h_rgw_gc_list);
    cls_register_cxx_method(h_class, RGW_GC_REMOVE,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_gc_remove,
                            &h_rgw_gc_remove);

    /* lifecycle bucket list */
    cls_register_cxx_method(h_class, RGW_LC_SET_ENTRY,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_set_entry,
                            &h_rgw_lc_set_entry);
    cls_register_cxx_method(h_class, RGW_LC_RM_ENTRY,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_rm_entry,
                            &h_rgw_lc_rm_entry);
    cls_register_cxx_method(h_class, RGW_LC_GET_NEXT_ENTRY, CLS_METHOD_RD,
                            rgw_cls_lc_get_next_entry,
                            &h_rgw_lc_get_next_entry);
    cls_register_cxx_method(h_class, RGW_LC_PUT_HEAD,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_put_head,
                            &h_rgw_lc_put_head);
    cls_register_cxx_method(h_class, RGW_LC_GET_HEAD, CLS_METHOD_RD,
                            rgw_cls_lc_get_head, &h_rgw_lc_get_head);
    cls_register_cxx_method(h_class, RGW_LC_LIST_ENTRIES, CLS_METHOD_RD,
                            rgw_cls_lc_list_entries, &h_rgw_lc_list_entries);
    cls_register_cxx_method(h_class, "reshard_add",
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_reshard_add,
                            &h_rgw_reshard_add);
    cls_register_cxx_method(h_class, "reshard_list", CLS_METHOD_RD,
                            rgw_reshard_list, &h_rgw_reshard_list);
    cls_register_cxx_method(h_class, "reshard_get", CLS_METHOD_RD,
                            rgw_reshard_get, &h_rgw_reshard_get);
    cls_register_cxx_method(h_class, "reshard_remove",
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_reshard_remove,
                            &h_rgw_reshard_remove);
    cls_register_cxx_method(h_class, "set_bucket_resharding",
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_set_bucket_resharding,
                            &h_rgw_set_bucket_resharding);
    cls_register_cxx_method(h_class, "clear_bucket_resharding",
                            CLS_METHOD_RD | CLS_METHOD_WR,
