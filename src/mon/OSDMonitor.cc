...
creating_pgs_t OSDMonitor::update_pending_pgs(const OSDMap::Incremental &inc,
...
public:
    ...
    } void _finish(int r) override
    {
        if (r == 0) {
            ...
            osdmon->mon.send_reply(op, new MOSDMarkMeDown(m->fsid, m->target_osd, m->target_addrs, m->get_epoch(),
...
std::set < osd_pool_get_choices > subtract_second_from_first(const std::set < osd_pool_get_choices >
        &first,
                     ...
                     const cmdmap_t &cmdmap)
{
    ...
    if (prefix == "osd setcrushmap" ||
        ...
        err = prepare_new_pool(poolstr, -1,  // default crush rule
                               rule_name,
                               pg_num,
                               pgp_num,
                               pg_num_min,
