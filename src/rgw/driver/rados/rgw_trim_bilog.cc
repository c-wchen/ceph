...
struct TrimNotifyHandler {
    ...
    virtual void handle(bufferlist::const_iterator & input,
                        bufferlist & output) = 0;
};
...
struct TrimCounters {
    ...
    class Handler:public TrimNotifyHandler {
      ...
      public:
        ...
        } void handle(bufferlist::const_iterator & input,
                      bufferlist & output) override;
    };
};

std::ostream & operator<<(std::ostream & out,
                          const TrimCounters::BucketCounter & rhs)
{
    return out << rhs.bucket << ":" << rhs.count;
}

void TrimCounters::BucketCounter::encode(bufferlist & bl) const const
{
    using ceph::encode;
...
void TrimCounters::Request::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void TrimCounters::Response::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void TrimComplete::Request::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ENCODE_FINISH(bl);
}
...
void TrimComplete::Response::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ENCODE_FINISH(bl);
}
...
class BucketTrimWatcher:public librados::WatchCtx2 {
  ...
  public:
    ...
    void handle_notify(uint64_t notify_id, uint64_t cookie,
        ...
        try {
            ...
            if (handler != handlers.end()) {
                ...
                lderr(store->
                      ctx()) << "no handler for notify type " << type << dendl;
            }
        }
        catch(const buffer::error & e) {
            lderr(store->ctx()) << "Failed to decode notification: " << e.
                what() << dendl;
        }
    ...
    void handle_error(uint64_t cookie, int err) override {
        ...
        if (err == -ENOTCONN) {
            ldout(store->ctx(),
                  4) << "Disconnected watch on " << ref.obj << dendl;
            restart();
        }
    }
};
...
class BucketTrimShardCollectCR:public RGWShardCollectCR {
    ...
    int handle_result(int r) override {
        if (r == -ENOENT) {     // ENOENT is not a fatal error
            ...
            ldout(cct,
                  4) << "failed to trim bilog shard: " << cpp_strerror(r) <<
                dendl;
        }
        return r;
    }
  public:
    BucketTrimShardCollectCR(const DoutPrefixProvider * dpp,
                             ...
                             const RGWBucketInfo & bucket_info,
                             const rgw::
                             bucket_index_layout_generation & generation,
                             const std::vector < std::string > &markers)
  :    RGWShardCollectCR(store->ctx(), MAX_CONCURRENT_SHARDS), dpp(dpp),
        store(store), bucket_info(bucket_info), generation(generation),
        markers(markers) {
    }
    ...
    while (i < markers.size()) {
        ...
        if (!marker.empty()) {
            ldpp_dout(dpp, 10) << "trimming bilog shard " << shard_id
                << " of " << bucket_info.
                bucket << " at marker " << marker << dendl;
            spawn(new
                  RGWRadosBILogTrimCR(dpp, store, bucket_info, shard_id,
...
class BucketCleanIndexCollectCR:public RGWShardCollectCR {
  ...
  :    
    ...
    dpp(dpp), store(store), bucket_info(bucket_info), index(index) {
    }
    bool spawn_next() override {
        if (shard < num_shards) {
            ...
            spawn(new
                  RGWRadosRemoveOidCR(store, std::move(bs.bucket_obj), nullptr),
                  false);
            ++shard;
            return true;
        }
...
class BucketTrimInstanceCR:public RGWCoroutine {
  ...
  :    
    ...
    zone_id(store->svc()->zone->get_zone().id), dpp(dpp) {
...
/// populate the status with the minimum stable marker of each shard
    int take_min_status(CephContext * cct,
                        const uint64_t min_generation,
                        std::vector <
                        BucketTrimInstanceCR::StatusShards >::
...
int BucketTrimInstanceCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ldpp_dout(dpp,
                  4) << "starting trim on bucket=" << bucket_instance << dendl;

        get_policy_params.zone = zone_id;
        get_policy_params.bucket = bucket;
        yield call(new
                   RGWBucketGetSyncPolicyHandlerCR(store->svc()->rados->
        ...
        if (retcode < 0) {
            if (retcode != -ENOENT) {
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: failed to fetch policy handler for bucket=" <<
                    bucket << dendl;
            }
        ...
        if (auto & opt_bucket_info =
            source_policy->policy_handler->get_bucket_info(); opt_bucket_info) {
        ...
        yield {
            const auto & all_dests =
                source_policy->policy_handler->get_all_dests();
          ...
          for (auto & zid:zids) {
                ...
                rgw_http_param_pair params[] = {
                    ...
                    ,           /* equal to source-bucket when `options==merge` and source-bucket
                ...
                if (ziter == zone_conn_map.end()) {
                    ldpp_dout(dpp,
                        ...
                        ", can't trim bucket: " << bucket << dendl;
                    return set_cr_error(-ECANCELED);
                }

                using StatusCR = RGWReadRESTResourceCR < StatusShards >;
                spawn(new
                      StatusCR(cct, ziter->second, http, "/admin/log/", params,
                               &*p), false);
                ++p;
            }
        }
        ...
        if (clean_info) {
            ...
            yield call(new
                       BucketCleanIndexCollectCR(dpp, store, clean_info->first,
                                                 clean_info->second.layout.
                                                 in_index));
            if (retcode < 0) {
            ...
            while (clean_info && retries < MAX_RETRIES) {
                yield call(new
                           RGWPutBucketInstanceInfoCR(store->svc()->rados->
                                                      get_async_processor(),
                                                      store, clean_info->first,
                                                      false, {
                                                      }
                                                      ,
                                                      no_change_attrs(), dpp));
                ...
                if (retcode == -ECANCELED) {
                    yield call(new
                               RGWGetBucketInstanceInfoCR(store->svc()->rados->
                                                          get_async_processor(),
                                                          store,
                                                          clean_info->first.
                                                          bucket,
                    ...
                    if (clean_info->first.layout.logs.front().gen ==
                        ...
                        clean_info->first.layout.logs.erase(clean_info->first.
                                                            layout.logs.
                                                            begin());
                        ++retries;
                        continue;
            ...
            min_markers.
                assign(std::max(1u, rgw::num_shards(totrim.layout.in_index)),
            ...
            if (retcode < 0) {
                ldpp_dout(dpp,
                          4) <<
                    "failed to correlate bucket sync status from peers" <<
                    dendl;
                return set_cr_error(retcode);
            }
            ...
            ldpp_dout(dpp,
                ...
                bucket << " markers=" << min_markers << ", shards=" <<
                min_markers.size() << dendl;
            set_status("trimming bilog shards");
            yield call(new
                       BucketTrimShardCollectCR(dpp, store, *pbucket_info,
                                                totrim.layout.in_index,
                                                min_markers));
...
class BucketTrimInstanceCollectCR:public RGWShardCollectCR {
    ...
    int handle_result(int r) override {
        if (r == -ENOENT) {     // ENOENT is not a fatal error
            ...
            ldout(cct,
                  4) << "failed to trim bucket instance: " << cpp_strerror(r) <<
                dendl;
        }
        return r;
    }
  public:
    BucketTrimInstanceCollectCR(rgw::sal::RadosStore * store,
                                RGWHTTPManager * http,
                                BucketTrimObserver * observer,
                                const std::vector < std::string > &buckets,
                                int max_concurrent,
                                const DoutPrefixProvider * dpp)
  :    RGWShardCollectCR(store->ctx(), max_concurrent), store(store),
        http(http), observer(observer), bucket(buckets.begin()),
        end(buckets.end()), dpp(dpp) {
    }
    bool spawn_next() override;
};
...
class AsyncMetadataList:public RGWAsyncRadosRequest {
  ...
  public:
    ...
    :RGWAsyncRadosRequest(caller, cn), cct(cct), mgr(mgr), section(section),
        start_marker(start_marker), callback(callback)
{
}};
...
class MetadataListCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    int send_request(const DoutPrefixProvider * dpp) override {
        req =
            new AsyncMetadataList(cct, this,
                                  stack->create_completion_notifier(), mgr,
                                  section, start_marker, callback);
...
const std::string BucketTrimCR::section {
"bucket.instance"};
    ...
    reenter(this) {
        ...
        if (config.buckets_per_interval) {
            ...
            yield {
                ...
                TrimCounters::Request request {
                32};
                bufferlist bl;
                encode(type, bl);
                encode(request, bl);
                call(new
                     RGWRadosNotifyCR(store, obj, bl, config.notify_timeout_ms,
                                      &notify_replies));
            }
            if (retcode < 0) {
                ldpp_dout(dpp,
                          10) << "failed to fetch peer bucket counters" <<
                    dendl;
                return set_cr_error(retcode);
            }
        ...
        if (buckets.size() < config.buckets_per_interval) {
            ...
            yield {
                ...
                auto cb =[this, ref] (std::string && bucket, std::string
                                      && marker) {
                    ...
                    if (observer->trimmed_recently(bucket)) {
                                                             ...
                                                             ldout(cct,
                                                                   4) <<
                                                             "failed to list bucket instance metadata: "
                                                             <<
                                                             cpp_strerror
                                                             (retcode) << dendl;
                                                             return
                                                             ...
                                                             ldpp_dout(dpp,
                                                                       4) <<
                                                             "failed to write updated trim status: "
                                                             <<
                                                             cpp_strerror
                                                             (retcode) << dendl;
                                                             return
                                                             ...
                                                             ldout(cct,
                                                                   10) <<
                                                             "failed to notify peers of trim completion"
                                                             << dendl;
                                                             return
                                                             set_cr_error
                                                             (retcode);}
                                                             ...
                                                             ldpp_dout(dpp,
                                                                       4) <<
                                                             "bucket index log processing completed in "
                                                             << ceph::
                                                             mono_clock::now() -
                                                             start_time <<
                                                             dendl;
...
class BucketTrimPollCR:                                     public RGWCoroutine
                                                             ...
                                                             * const observer;
                                                             ...
                                                             string name {
                                                             "trim"};   //< lock name
  const std::string cookie; const DoutPrefixProvider * dpp; public:
                                                             BucketTrimPollCR
                                                             (rgw::sal::
                                                              RadosStore *
                                                              store,
...
// read bucket trim configuration from ceph context
  ...
  class BucketTrimManager::Impl:                            public
                                                             TrimCounters::
                                                             Server,
                                                             public
                                                             BucketTrimObserver
  ...
  :                                                         impl(new
                                                             ...
                                                             *log;
                                                             ...
                                                             << ":" << __LINE__
                                                             <<
                                                             "ERROR: bilog_rados->log_trim returned r="
                                                             << r << dendl;}
                                                             return r;}
