...
class TokenEngine: public rgw::auth::Engine
{
    ...
    boost::optional < token_envelope_t > get_from_keystone(const DoutPrefixProvider *dpp,
            const std::string &token,
            bool allow_expired) const;

    acl_strategy_t get_acl_strategy(const token_envelope_t &token)
    const;
...
public:
    TokenEngine(CephContext *const cct,
                const rgw::auth::TokenExtractor *
                const auth_token_extractor,
                const rgw::auth::TokenExtractor *
                const service_token_extractor,
                const rgw::auth::RemoteApplier::Factory *
                const apl_factory, rgw::keystone::Config &config,
                rgw::keystone::TokenCache &token_cache)
        : cct(cct), auth_token_extractor(auth_token_extractor),
          service_token_extractor(service_token_extractor),
          apl_factory(apl_factory), config(config),
                            ...
                            optional_yield y)const override
    {
        return authenticate(dpp, auth_token_extractor->get_token(s),
                            service_token_extractor->get_token(s),
                            s);
    }
};                 /* class TokenEngine */
...
class SecretCache
{
    ...
    static SecretCache &get_instance()
    {
        ...
        return instance;
    } bool find(const std::string &token_id,
                token_envelope_t &token, std::string &secret);
    boost::optional < boost::tuple < token_envelope_t,
          std::string >> find(const std::string &token_id)
    {
    ...
    void add(const std::string &token_id,
...
class EC2Engine: public rgw::auth::s3::AWSEngine
{
    ...
    noexcept;
    std::pair < boost::optional < token_envelope_t >,
        int > get_from_keystone(const DoutPrefixProvider *dpp,
                                const std::
    ...
    access_token_result get_access_token(const DoutPrefixProvider *dpp,
...
public:
    EC2Engine(CephContext *const cct,
              const rgw::auth::s3::AWSEngine::VersionAbstractor *
        ...
        : AWSEngine(cct, *ver_abstractor),
          ...
          config(config),
          token_cache(token_cache), secret_cache(secret_cache)
    {
