...
struct FakeStore {
    ...
    template < class Func >
        ...
        for (auto max = std::numeric_limits < std::uint64_t >::max();
            ...
            per_entry(*it);
        } return it != std::end(objs) ? static_cast <
            const hobject_t & >(it->first)
        : hobject_t::get_max();
    }
...
class BackfillFixture:public crimson::osd::BackfillState::BackfillListener {
    ...
    // BackfillListener { void request_replica_scan(
                                                       const pg_shard_t &
                                                       target,
    ...
    void enqueue_drop(const pg_shard_t & target,
                      const hobject_t & obj, const eversion_t & v) override;
  ...
  public:
    ...
    bool all_stores_look_like(const FakeStore & reference) const {
        const bool all_replica_match =
            std::all_of(std::begin(backfill_targets),
...
struct BackfillFixture::PeeringFacade:public crimson::osd::BackfillState::
    PeeringFacade {
    FakePrimary & backfill_source;
    std::map < pg_shard_t, FakeReplica > &backfill_targets;
    ...
    :backfill_source(backfill_source), backfill_targets(backfill_targets) {
        std::transform(std::begin(backfill_targets), std::end(backfill_targets),
                       std::inserter(backfill_targets_as_set,
                                     std::end(backfill_targets_as_set)),
                       [](auto pair) {
                       return pair.first;}
        );
    }
...
BackfillFixture::BackfillFixture(FakePrimary && backfill_source,
                                 std::map < pg_shard_t,
                                 FakeReplica > &&backfill_targets)
:  backfill_source(std::move(backfill_source)),
backfill_targets(std::move(backfill_targets)), backfill_state(*this,
                                                              std::make_unique <
                                                              PeeringFacade >
                                                              (this->
                                                               backfill_source,
                                                              ...
                                                              (this->
                                                               backfill_source))
{
    backfill_state.process_event(crimson::osd::BackfillState::Triggered {
                                 }
                                 .intrusive_from_this());
}

void BackfillFixture::request_replica_scan(const pg_shard_t & target,
                                           const hobject_t & begin,
                                           const hobject_t & end)
{
    BackfillInterval bi;
    ...
    schedule_event(crimson::osd::BackfillState::ReplicaScanned {
                   target, std::move(bi)});
}
...
void BackfillFixture::request_primary_scan(const hobject_t & begin)
{
    ...
    schedule_event(crimson::osd::BackfillState::PrimaryScanned {
                   std::move(bi)});
}
...
void BackfillFixture::enqueue_push(const hobject_t & obj, const eversion_t & v)
{
  for (auto &[_, bt]:backfill_targets) {
    ...
    schedule_event(crimson::osd::BackfillState::ObjectPushed {
                   obj}
    );
}

void BackfillFixture::enqueue_drop(const pg_shard_t & target,
...
void BackfillFixture::
update_peers_last_backfill(const hobject_t & new_last_backfill)
{
}

bool BackfillFixture::budget_available() const const
{
    return true;
}
...
struct BackfillFixtureBuilder {
    ...
    BackfillFixtureBuilder && add_target(FakeStore::objs_t objs) && {
        ...
        const auto[_, inserted] =
            backfill_targets.emplace(new_osd_num,
                                     FakeReplica { FakeStore {std::
                                                              move(objs)} });
...
TEST(backfill, same_primary_same_replica)
{
    ...
    auto cluster_fixture =
        ...
        add_target(reference_store.objs).get_result();
...
namespace StoreRandomizer {
    ...
    FakeStore mutate(const FakeStore & source_store) {
        ...
        source_store.list(hobject_t {
                          },[&](const auto & kv) {
                          const auto & oid = kv.first; const auto & version = kv.second; execute_random([] {    /* just drop the entry */
                                                                                                        },
                                                                                                        [&]
                                                                                                        {
                                                                                                        mutated_store.
...
TEST(backfill, one_pseudorandomized_replica)
{
    ...
    auto cluster_fixture =
        BackfillFixtureBuilder::add_source(reference_store.objs).
        add_target(StoreRandomizer::mutate(reference_store).objs).get_result();
