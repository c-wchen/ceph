 ...
 */
    template < typename iterator, typename const_iterator >
        static void copy_from_foreign(iterator tgt,
        ...
        auto to_copy =
            from_src->get_right_ptr_end() - to_src->get_right_ptr_end();
        assert(to_copy > 0);
        memcpy(tgt->get_right_ptr_end() - to_copy,
               to_src->get_right_ptr_end(), to_copy);
        memcpy(tgt->get_node_key_ptr(),
               from_src->get_node_key_ptr(),
               to_src->get_node_key_ptr() - from_src->get_node_key_ptr());

        auto offset_diff =
            tgt->get_right_offset_end() - from_src->get_right_offset_end();
 ...
 */ template < typename iterator >
    static void copy_from_local(unsigned len,
                                iterator tgt,
                                iterator from_src, iterator to_src) {
        assert(tgt->node == from_src->node);
        assert(to_src->node == from_src->node);
...
{
    buffer.push_back(delta_inner_t {
                     delta_inner_t::op_t::INSERT, key, addr});
} void update(const std::string & key, laddr_t addr) {
            buffer.push_back(delta_inner_t {
                             delta_inner_t::op_t::UPDATE, key, addr});
}
        void remove(const std::string & key) {
            buffer.push_back(delta_inner_t {
                             delta_inner_t::op_t::REMOVE, key, L_ADDR_NULL});
        }
...
{
    buffer.push_back(delta_leaf_t {
                     delta_leaf_t::op_t::INSERT, key, val});
} void update(const std::string & key, const ceph::bufferlist & val) {
            buffer.push_back(delta_leaf_t {
                             delta_leaf_t::op_t::UPDATE, key, val});
}
        void remove(const std::string & key) {
            buffer.push_back(delta_leaf_t {
                             delta_leaf_t::op_t::REMOVE, key, bufferlist()
                             });
        }
 ...
 */
    class StringKVInnerNodeLayout {
        ...
        using L =
            absl::container_internal::Layout < ceph_le32, omap_node_meta_le_t,
            omap_inner_key_le_t >;
        static constexpr L layout {
        ...
        template < bool is_const > class iter_t
{
    ...
    using parent_t =
        typename crimson::common::maybe_const_t < StringKVInnerNodeLayout,
        is_const >::type;

    mutable parent_t node;
    uint16_t index;

    iter_t(parent_t parent, uint16_t index):node(parent), index(index) {
  } public:
  ...
  private:
    omap_inner_key_t get_node_key()const {
        ...
        return reinterpret_cast <
            typename crimson::common::maybe_const_t < char,
            is_const >::type > (node->get_node_key_ptr() + index);
...
public:
void journal_inner_insert(const_iterator _iter,
                          const laddr_t laddr,
...
void journal_inner_update(const_iterator _iter,
                          const laddr_t laddr,
                          delta_inner_buffer_t * recorder) {
    auto iter = iterator(this, _iter.index);
...
const_iterator iter_idx(uint16_t off) const {
    ...
    auto it = std::lower_bound(boost::make_counting_iterator < uint16_t > (0),
                               boost::make_counting_iterator < uint16_t >
                               (get_size()),
                               str,
...
uint32_t free_space() const {
   ...
   */
std::string split_into(StringKVInnerNodeLayout & left,
   ...
   */
static std::string balance_into_new_nodes(const StringKVInnerNodeLayout & left,
                                          const StringKVInnerNodeLayout & right,
                                          StringKVInnerNodeLayout &
                                          replacement_left,
                                          StringKVInnerNodeLayout &
    ...
    uint32_t right_size =
        omap_inner_key_t(right.get_node_key_ptr()[right.get_size() - 1]).
        key_off;
    ...
    if (pivot_size < left_size) {
        copy_from_foreign(replacement_left.iter_end(),
                          left.iter_begin(), left.iter_idx(pivot_idx));
        replacement_left.set_size(pivot_idx);

        copy_from_foreign(replacement_right.iter_end(),
                          left.iter_idx(pivot_idx), left.iter_end());
        replacement_right.set_size(left.get_size() - pivot_idx);

        copy_from_foreign(replacement_right.iter_end(),
                          right.iter_begin(), right.iter_end());
        replacement_right.set_size(right.get_size() + left.get_size() -
                                   pivot_idx);
    }
    else {
        copy_from_foreign(replacement_left.iter_end(),
                          left.iter_begin(), left.iter_end());
...
void inner_remove(iterator iter) {
    assert(iter != iter_end());
    if ((iter + 1) != iter_end())
        copy_from_local(iter->get_node_key().key_len, iter, iter + 1,
                        iter_end());
    set_size(get_size() - 1);
}
 ...
 */
    class StringKVLeafNodeLayout {
      ...
      public:
        template < bool is_const > class iter_t {
            ...
            using parent_t =
                typename crimson::common::maybe_const_t <
                StringKVLeafNodeLayout, is_const >::type;

            template < typename iterator, typename const_iterator >
                friend void copy_from_foreign(iterator, const_iterator,
                                              const_iterator);
            template < typename iterator > friend void copy_from_local(unsigned,
                                                                       iterator,
                                                                       iterator,
                                                                       iterator);
            ...
            omap_leaf_key_t get_node_key() const {
                ...
                return node->buf + OMAP_LEAF_BLOCK_SIZE -
            ...
            void set_node_key(omap_leaf_key_t _lb) const {
                ...
                lb = _lb;
                node->get_node_key_ptr()[index] = lb;
            } void set_node_val(const std::string & key,
          ...
          public:
            uint16_t get_index()const {
                ...
                auto node_key = get_node_key();
                return std::string(get_node_val_ptr() + node_key.key_len,
                                   get_node_key().val_len);
            } ceph::bufferlist get_val() const {
                auto node_key = get_node_key();
                ceph::bufferlist bl;
                ceph::bufferptr bptr(get_node_val_ptr() + node_key.key_len,
                                     get_node_key().val_len);
                bl.append(bptr);
                return bl;
        }};
      ...
      public:
        void journal_leaf_insert(const_iterator _iter,
                                 const std::string & key,
                                 const ceph::bufferlist & val,
        ...
        void journal_leaf_update(const_iterator _iter,
                                 const std::string & key,
                                 const ceph::bufferlist & val,
                                 delta_leaf_buffer_t * recorder) {
            auto iter = iterator(this, _iter.index);
        ...
        void journal_leaf_remove(const_iterator _iter,
                                 delta_leaf_buffer_t * recorder) {
            auto iter = iterator(this, _iter.index);
            if (recorder) {
                recorder->remove(iter->get_key());
        ...
        StringKVLeafNodeLayout(char *buf):buf(buf) {
        }
   ...
   */
        omap_node_meta_t get_meta() const {
            omap_node_meta_le_t & metaint =
                *layout.template Pointer < 1 > (buf);
            return omap_node_meta_t(metaint);
        } void set_meta(const omap_node_meta_t & meta) {
            *layout.template Pointer < 1 > (buf) = omap_node_meta_le_t(meta);
        }
        ...
        uint32_t used_space() const {
            uint32_t count = get_size();
            if (count) {
                omap_leaf_key_t last_key =
                    omap_leaf_key_t(get_node_key_ptr()[count - 1]);
        ...
        uint32_t free_space() const {
            ...
            return OMAP_LEAF_BLOCK_SIZE
                - (reinterpret_cast <
                   char *>(layout.template Pointer < 2 > (buf))
                   - reinterpret_cast < char *>(layout.template Pointer < 0 >
                                                (buf)));
   ...
   * Takes *this and splits its contents into left and right.
   */
        std::string split_into(StringKVLeafNodeLayout & left,
   ...
   */ void merge_from(
            ...
            copy_from_foreign(iter_end(), right.iter_begin(), right.iter_end());
            set_size(left.get_size() + right.get_size());
            set_meta(omap_node_meta_t::
                     merge_from(left.get_meta(), right.get_meta()));
        }
   ...
   */
        static std::
            string balance_into_new_nodes(const StringKVLeafNodeLayout & left,
                                          const StringKVLeafNodeLayout & right,
                                          StringKVLeafNodeLayout &
                                          replacement_left,
                                          StringKVLeafNodeLayout &
                                          replacement_right) {
            uint32_t left_size =
                omap_leaf_key_t(left.get_node_key_ptr()[left.get_size() - 1]).
                key_off;
            uint32_t right_size =
                omap_leaf_key_t(right.get_node_key_ptr()[right.get_size() - 1]).
                key_off;
            uint32_t total = left_size + right_size;
            uint32_t pivot_size = total / 2;
            uint32_t pivot_idx = 0;
            if (pivot_size < left_size) {
                ...
                uint32_t more_size = pivot_size - left_size;
                uint32_t size = 0;
                for (auto ite = right.iter_begin(); ite < right.iter_end();
                     ite++) {
                    auto node_key = ite->get_node_key();
                    size += node_key.key_len + node_key.val_len;
                    if (size >= more_size) {
      ...
      private:
        void leaf_insert(iterator iter,
            ...
            if (get_size() != 0 && iter != iter_end())
                copy_from_local(node_key.key_len + node_key.val_len, iter + 1,
                                iter, iter_end());
        ...
        void leaf_update(iterator iter,
                         const std::string & key,
                         const ceph::bufferlist & val) {
            assert(iter != iter_end());
            leaf_remove(iter);
        ...
        void leaf_remove(iterator iter) {
            ...
            if ((iter + 1) != iter_end()) {
                ...
                copy_from_local(key.key_len + key.val_len, iter, iter + 1,
                                iter_end());
            }
            set_size(get_size() - 1);
        }
