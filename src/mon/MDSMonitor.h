...
class MDSMonitor:public PaxosService, public PaxosFSMap {
   ...
   */
    ...
    void modify_legacy_filesystem(FSMap & fsmap,
                                  std::function < void (std::shared_ptr <
                                                        Filesystem >) > fn);
    int legacy_filesystem_command(FSMap & fsmap, MonOpRequestRef op,
                                  std::string const &prefix, map < string,
                                  cmd_vartype > &cmdmap,
                                  std::stringstream & ss);
    int filesystem_command(FSMap & fsmap, MonOpRequestRef op,
                           std::string const &prefix, map < string,
                           cmd_vartype > &cmdmap, std::stringstream & ss);
  ...
  protected:
    ...
    mds_gid_t gid_from_arg(const FSMap & fsmap, const std::string & arg,
                           std::ostream & err);
