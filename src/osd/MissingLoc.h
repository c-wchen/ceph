...
class MissingLoc {
  public:
    ...
    using missing_by_count_t =
        std::map < shard_id_t, std::map < loc_count_t, int >>;
  private:
    ...
    void pgs_by_shard_id(const std::set < pg_shard_t > &s,
                         std::map < shard_id_t,
                         std::set < pg_shard_t > >&pgsbs) {
    ...
    void _dec_count(const std::set < pg_shard_t > &s) {
      ...
      for (auto shard:pgsbs) {
            auto p =
                missing_by_count[shard.first].find(_get_count(shard.second));
  ...
  public:
    ...
    explicit MissingLoc(spg_t pgid,
                        MappingInfo * mapping_info,
                        DoutPrefixProvider * dpp, CephContext * cct)
        ...
        for (std::map < hobject_t, pg_missing_item >::const_iterator i =
            ...
            if (j == needs_recovery_map.end()) {
                ...
                if (i->second.need != j->second.need) {
                    lgeneric_dout(cct,
                                  0) << this << " " << pgid <<
                        " unexpected need for " << i->first << " have " << j->
    ...
    void add_batch_sources_info(const std::set < pg_shard_t > &sources, ///< [in] a std::set of resources which can be used for all objects
                                HBHandle * handle   ///< [in] ThreadPool handle
        );
