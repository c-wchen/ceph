     ...
     namespace {
        ...
        std::ostream & operator<<(std::ostream & out, const Operation & op) {
            ...
            case OPERATION_CHECK_OBJECT_MAP:
                ...
                break;
                case OPERATION_FLATTEN:out << "flatten";
                break;
                case OPERATION_METADATA_UPDATE:out << "metadata update";
                break;
                case OPERATION_MIGRATE:out << "migrate";
                break;
                case OPERATION_REBUILD_OBJECT_MAP:out << "rebuild object map";
                break;
                case OPERATION_RENAME:out << "rename";
                break;
                case OPERATION_RESIZE:out << "resize";
                break;
                case OPERATION_SNAP_CREATE:out << "snap create";
                break;
                case OPERATION_SNAP_PROTECT:out << "snap protect";
                break;
                case OPERATION_SNAP_REMOVE:out << "snap remove";
                break;
                case OPERATION_SNAP_RENAME:out << "snap rename";
                break;
                case OPERATION_SNAP_ROLLBACK:out << "snap rollback";
                break;
                case OPERATION_SNAP_UNPROTECT:out << "snap unprotect";
                break;
                case OPERATION_SPARSIFY:out << "sparsify";
                break;
                case OPERATION_UPDATE_FEATURES:out << "update features";
                break;
                default:ceph_abort();
                break;
            } return out;
            ...
            :image_ctx(image_ctx), on_finish(on_finish) {
                ...
                if (notified) {
                    if (r == -ETIMEDOUT) {
                        ...
                        ldout(cct,
                              5) << "update notification on missing header" <<
                            dendl;
                        r = 0;
                    }
                    else if (r < 0) {
                        lderr(cct) << "update notification failed: " <<
                            cpp_strerror(r)
                            << dendl;
                    }
                    Context::complete(r);
   ...
   */
            ...
            bool request_lock = false;
             ...
             C_InvokeAsyncRequest(I & image_ctx, Operation operation,
                                  ...
                                  bool permit_snapshot,
                                  const boost::function < void (Context *) >
                                  &local,
                                  const boost::function < void (Context *) >
                                  &remote,
                                  const std::set < int >&filter_error_codes,
                                  Context * on_finish)
            :image_ctx(image_ctx), operation(operation),
                request_type(request_type), permit_snapshot(permit_snapshot),
            ...
            void handle_remote_request(int r) {
                ...
                if (r == -EOPNOTSUPP) {
                    ldout(cct,
                          5) << operation <<
                        " not supported by current lock owner" << dendl;
                    request_lock = true;
                    send_refresh_image();
                    return;
                }
                ...
                ldout(cct,
                      5) << operation << " timed out notifying lock owner" <<
                    dendl;
                send_refresh_image();
            }
            ...
            void send_local_request() {
                auto ctx = new LambdaContext([this] (int r){
                                             ...
                                             image_ctx.operations->
                                             start_op(operation, ctx);}

                                             void execute_local_request() {
                                             std::shared_lock owner_locker {
                                             image_ctx.owner_lock};
                                             CephContext * cct = image_ctx.cct;
                                             ldout(cct,
                                                   20) << __func__ << dendl;
                                             Context * ctx =
                                             ...
                                             r << dendl;
                                             image_ctx.operations->
                                             finish_op(operation, r);
                                             if (r == -ERESTART) {
                                             send_refresh_image(); return;}
                                             complete(r);}

                                             void finish(int r) override {
                                             if (filter_error_codes.count(r) !=
                                                 0) {
                                             r = 0;}
                                             on_finish->complete(r);}
                                             };
                                             template < typename I >
                                             bool needs_invalidate(I &
                                                                   image_ctx,
                                                                   uint64_t
                                                                   object_no,
                                                                   uint8_t
                                             ...
                                             template < typename I >
                                             Operations <
                                             I >::Operations(I & image_ctx)
  :                                         m_image_ctx(image_ctx),
                                             m_queue_lock(ceph::
                                                          make_mutex(util::
                                                                     unique_lock_name
                                                                     ("librbd::Operations::m_queue_lock",
                                                                      this))) {
                                             }

                                             template < typename I >
                                             void Operations <
                                             I >::start_op(Operation op,
                                                           Context * ctx) {
                                             CephContext * cct =
                                             m_image_ctx.cct;
                                             ldout(cct,
                                             ...
                                             if (queue.empty()) {
                                             m_in_flight_ops.erase(op); return;}
                                             ...
                                             int Operations <
                                             I >::
                                             flatten(ProgressContext & prog_ctx)
                                             {
                                             CephContext * cct =
                                             m_image_ctx.cct;
                                             ldout(cct,
                                             ...
                                             if (m_image_ctx.parent_md.spec.
                                                 pool_id == -1) {
                                             lderr(cct) << "image has no parent"
                                             << dendl; return -EINVAL;}
                                             }
                                             ...
                                             (OPERATION_FLATTEN,
                                              ...
                                              false,
                                              boost::bind(&Operations <
                                                          I >::execute_flatten,
                                                          this,
                                                          boost::ref(prog_ctx),
                                                          _1),
                                              boost::bind(&ImageWatcher <
                                                          I >::notify_flatten,
                                                          m_image_ctx.
                                                          image_watcher,
                                                          request_id,
                                             ...
                                             Striper::
                                             get_num_objects(m_image_ctx.layout,
                                                             m_image_ctx.
                                                             get_area_size(io::
                                                                           ImageArea::
                                                                           CRYPTO_HEADER));
                                             ...
                                             operation::FlattenRequest < I >
                                             *req =
                                             new operation::FlattenRequest < I >
                                             (m_image_ctx,
                                              new C_NotifyUpdate < I >
                                              (m_image_ctx, on_finish),
                                              crypto_header_objects,
                                              data_overlap_objects, prog_ctx);
                                             req->send();}
                                             ...
                                             lderr(cct) <<
                                             "image must support object-map feature"
                                             << dendl;
                                             on_finish->complete(-EINVAL);
                                             return;}

                                             operation::
                                             RebuildObjectMapRequest < I >
                                             *req =
                                             new operation::
                                             RebuildObjectMapRequest < I >
                                             (m_image_ctx,
                                             ...
                                             operation::
                                             ObjectMapIterateRequest < I >
                                             *req =
                                             new operation::
                                             ObjectMapIterateRequest < I >
                                             (m_image_ctx, on_finish, prog_ctx,
                                              handle_mismatch); req->send();}
                                             ...
                                             check_object_map(ProgressContext &
                                                              ...
                                                              Context *
                                                              on_finish) {
                                             object_map_iterate(prog_ctx,
                                                                needs_invalidate,
                                                                on_finish);}

                                             template < typename I >
                                             int Operations <
                                             I >::rename(const char *dstname) {
                                             CephContext * cct =
                                             m_image_ctx.cct;
                                             ldout(cct,
                                                   5) << this << " " << __func__
                                             ...
                                             m_image_ctx.
                                             set_image_name(dstname); return 0;}

                                             template < typename I >
                                             void Operations <
                                             I >::execute_rename(const std::
                                                                 string &
                                                                 dest_name,
                                                                 Context *
                                                                 on_finish) {
                                             ...
                                             return;}
                                             operation::RenameRequest < I >
                                             *req =
                                             new operation::RenameRequest < I >
                                             (m_image_ctx, on_finish,
                                              dest_name); req->send();}

                                             template < typename I >
                                             int Operations <
                                             I >::resize(uint64_t size,
                                                         bool allow_shrink,
                                                         ProgressContext &
                                                         prog_ctx) {
                                             CephContext * cct =
                                             ...
                                             if (m_image_ctx.
                                                 ...
                                                 is_compatible(m_image_ctx.
                                                               ...
                                                               raw_size)) {
                                             lderr(cct) <<
                                             "New size not compatible with object map"
                                             << dendl; return -EINVAL;}
                                             ...
                                             template < typename I >
                                             void Operations <
                                             I >::execute_resize(uint64_t size,
                                                                 bool
                                                                 allow_shrink,
                                                                 ProgressContext
                                                                 & prog_ctx,
                                             ...
                                             m_image_ctx.image_lock.
                                             unlock_shared();
                                             operation::ResizeRequest < I >
                                             *req =
                                             new operation::ResizeRequest < I >
                                             (m_image_ctx,
                                              new C_NotifyUpdate < I >
                                              (m_image_ctx, on_finish),
                                              raw_size, allow_shrink, prog_ctx,
                                              journal_op_tid, false);
                                             req->send();}

                                             template < typename I >
                                             int Operations <
                                             I >::snap_create(const cls::rbd::
                                                              SnapshotNamespace
                                                              & snap_namespace,
                                                              const std::
                                                              string &
                                             ...
                                             ldout(cct,
                                                   5) << this << " " << __func__
                                             << ": snap_name=" << snap_name <<
                                             dendl; if (m_image_ctx.read_only) {
                                             on_finish->complete(-EROFS);
                                             return;}
                                             ...
                                             uint64_t request_id =
                                             util::reserve_async_request_id();
                                             C_InvokeAsyncRequest < I > *req =
                                             new C_InvokeAsyncRequest < I >
                                             (m_image_ctx,
                                             ...
                                             (m_image_ctx,
                                              ...
                                              journal_op_tid, flags, prog_ctx);
                                             req->send();}

                                             template < typename I >
                                             int Operations <
                                             I >::snap_rollback(const cls::rbd::
                                                                SnapshotNamespace
                                                                &
                                                                snap_namespace,
                                                                const std::
                                             ...
                                             start_op(OPERATION_SNAP_ROLLBACK,
                                                      ctx);}

                                             r = cond_ctx.wait(); if (r < 0) {
                                             return r;}
                                             ...
                                             void Operations <
                                             I >::
                                             execute_snap_rollback(const cls::
                                                                   rbd::
                                                                   SnapshotNamespace
                                                                   &
                                                                   snap_namespace,
                                             ...
                                             *request =
                                             ...
                                             ((m_image_ctx.
                                              ...
                                              || (m_image_ctx.
                                                  features &
                                                  RBD_FEATURE_JOURNALING) != 0);
                                             m_image_ctx.image_lock.
                                             unlock_shared(); if (proxy_op) {
                                             uint64_t request_id =
                                             util::reserve_async_request_id();
                                             auto request_type =
                                             exclusive_lock::
                                             ...
                                             std::shared_lock owner_lock {
                                             m_image_ctx.owner_lock};
                                             execute_snap_remove(snap_namespace,
                                                                 snap_name,
                                                                 on_finish);}
                                             }
                                             ...
                                             m_image_ctx.image_lock.
                                             unlock_shared();
                                             operation::SnapshotRemoveRequest <
                                             I > *req =
                                             new operation::
                                             ...
                                             << ": " << "snap_name=" << srcname
                                             << ", " << "new_snap_name=" <<
                                             dstname << dendl; snapid_t snap_id;
                                             if (m_image_ctx.read_only) {
                                             return -EROFS;}
                                             ...
                                             void Operations <
                                             I >::
                                             execute_snap_rename(const uint64_t
                                                                 src_snap_id,
                                                                 const std::
                                                                 string &
                                                                 dest_snap_name,
                                             ...
                                             << ": " << "snap_id=" <<
                                             src_snap_id << ", " <<
                                             "new_snap_name=" << dest_snap_name
                                             << dendl;
                                             operation::SnapshotRenameRequest <
                                             I > *req =
                                             new operation::
                                             SnapshotRenameRequest < I >
                                             (m_image_ctx,
                                              new C_NotifyUpdate < I >
                                              (m_image_ctx, on_finish),
                                             ...
                                             void Operations <
                                             I >::
                                             execute_snap_protect(const cls::
                                                                  rbd::
                                                                  SnapshotNamespace
                                                                  &
                                                                  snap_namespace,
                                             ...
                                             void Operations <
                                             I >::
                                             execute_snap_unprotect(const cls::
                                                                    rbd::
                                                                    SnapshotNamespace
                                                                    &
                                                                    snap_namespace,
                                             ...
                                             m_image_ctx.image_lock.
                                             lock_shared(); bool is_unprotected;
                                             int r =
                                             m_image_ctx.
                                             is_snap_unprotected(m_image_ctx.
                                                                 get_snap_id
                                             ...
                                             *request =
                                             ...
                                             execute_snap_set_limit(limit,
                                                                    &limit_ctx);}

                                             r = limit_ctx.wait(); return r;}

                                             template < typename I >
                                             void Operations <
                                             I >::
                                             execute_snap_set_limit(const
                                                                    uint64_t
                                                                    limit,
                                                                    Context *
                                                                    on_finish) {
                                             ...
                                             ldout(cct,
                                                   5) << this << " " << __func__
                                             << ": limit=" << limit << dendl;
                                             operation::SnapshotLimitRequest <
                                             I > *request =
                                             new operation::
                                             SnapshotLimitRequest < I >
                                             (m_image_ctx, on_finish, limit);
                                             request->send();}
                                             ...
                                             if (m_image_ctx.old_format) {
                                             lderr(cct) <<
                                             "old-format images do not support features"
                                             << dendl; return -EINVAL;}
                                             ...
                                             if ((enabled
                                                 ...
                                                 ||
                                                 ((features &
                                                   ~RBD_FEATURES_MUTABLE_INTERNAL)
                                                  != features)) {
                                             lderr(cct) <<
                                             "cannot update immutable features"
                                             << dendl; return -EINVAL;}
                                             ...
                                             bool exist_fast_diff =
                                             (m_image_ctx.
                                              features & RBD_FEATURE_FAST_DIFF)
                                             != 0;
                                             bool exist_object_map =
                                             (m_image_ctx.
                                              features & RBD_FEATURE_OBJECT_MAP)
                                             != 0;
                                             if ((enabled
                                             ...
                                             if (features == 0) {
                                             lderr(cct) <<
                                             "update requires at least one feature"
                                             << dendl; return -EINVAL;}
                                             {
                                             std::shared_lock image_locker {
                                             m_image_ctx.image_lock};
                                             if (enabled
                                                 && (features & m_image_ctx.
                                                     features) != 0) {
                                             lderr(cct) <<
                                             ...
                                             m_image_ctx.image_lock};
                                             m_image_ctx.
                                             set_journal_policy(new journal::
                                                                DisabledPolicy
                                                                ());
                                             ...
                                             ldout(cct,
                                                   2) <<
                                             "update_features finished" <<
                                             dendl; return r;}

                                             template < typename I >
                                             void Operations <
                                             I >::
                                             execute_update_features(uint64_t
                                                                     features,
                                                                     bool
                                                                     enabled,
                                                                     Context *
                                             ...
                                             if (enabled) {
                                             operation::EnableFeaturesRequest <
                                             I > *req =
                                             new operation::
                                             ...
                                             operation::DisableFeaturesRequest <
                                             I > *req =
                                             new operation::
                                             DisableFeaturesRequest < I >
                                             (m_image_ctx, on_finish,
                                              journal_op_tid, features, false);
                                             req->send();}
                                             }

                                             template < typename I >
                                             int Operations <
                                             I >::metadata_set(const std::
                                                               string & key,
                                                               const std::
                                                               string & value) {
                                             CephContext * cct =
                                             ...
                                             util::
                                             is_metadata_config_override(key,
                                                                         &config_key);
                                             if (config_override) {
                                             // validate config setting
                                             if (!librbd::api::Config <
                                                 I >::
                                                 is_option_name(&m_image_ctx,
                                                                config_key)) {
                                             lderr(cct) << "validation for " <<
                                             key <<
                                             " failed: not allowed image level override"
                                             << dendl; return -EINVAL;}
                                             int r = ConfigProxy {
                                             false}.set_val(config_key.c_str(),
                                                            value); if (r < 0) {
                                             return r;}
                                             }
                                             ...
                                             ldout(cct,
                                                   20) <<
                                             "metadata_set finished" << dendl;
                                             return r;}

                                             template < typename I >
                                             void Operations <
                                             I >::
                                             execute_metadata_set(const std::
                                                                  string & key,
                                                                  const std::
                                                                  string &
                                                                  value,
                                             ...
                                             operation::MetadataSetRequest < I >
                                             *request =
                                             new operation::MetadataSetRequest <
                                             I > (m_image_ctx,
                                                  new C_NotifyUpdate < I >
                                                  (m_image_ctx, on_finish), key,
                                                  value); request->send();}

                                             template < typename I >
                                             int Operations <
                                             I >::metadata_remove(const std::
                                                                  string & key)
                                             {
                                             CephContext * cct =
                                             m_image_ctx.cct;
                                             ...
                                             r =
                                             cls_client::
                                             metadata_get(&m_image_ctx.md_ctx,
                                                          m_image_ctx.
                                                          header_oid, key,
                                                          &value); if (r < 0)
                                             return r;
                                             ...
                                             void Operations <
                                             I >::
                                             execute_metadata_remove(const std::
                                                                     string &
                                                                     key,
                                                                     Context *
                                                                     on_finish)
                                             ...
                                             int Operations <
                                             I >::
                                             migrate(ProgressContext & prog_ctx)
                                             {
                                             CephContext * cct =
                                             m_image_ctx.cct;
                                             ldout(cct,
                                             ...
                                             m_image_ctx.image_lock.
                                             unlock_shared();
                                             operation::MigrateRequest < I >
                                             *req =
                                             new operation::MigrateRequest < I >
                                             (m_image_ctx,
                                              new C_NotifyUpdate < I >
                                              (m_image_ctx, on_finish),
                                              prog_ctx); req->send();}
                                             ...
                                             <<
                                             " and not larger image object size"
                                             << dendl; return -EINVAL;}
                                             ...
                                             return r;}
                                             ldout(cct,
                                                   20) << "resparsify finished"
                                             << dendl; return 0;}

                                             template < typename I >
                                             void Operations <
                                             I >::
                                             execute_sparsify(size_t
                                                              sparse_size,
                                                              ProgressContext &
                                                              prog_ctx,
                                                              Context *
                                             ...
                                             (exclusive_lock::
                                              OperationRequestType request_type,
                                              bool request_lock) {
                                             ceph_assert(ceph_mutex_is_rlocked
                                                         (m_image_ctx.
                                                          owner_lock));
                                             ...
                                             int Operations <
                                             I >::
                                             invoke_async_request(Operation op,
                                                                  exclusive_lock::
                                             ...
                                             C_InvokeAsyncRequest < I > *req =
                                             new C_InvokeAsyncRequest < I >
                                             (m_image_ctx, op, request_type,
                                              permit_snapshot, local_request,
                                              remote_request, {
                                              }, &ctx); req->send();
                                             return ctx.wait();}
                                             ...
                                             template class librbd::Operations <
                                             librbd::ImageCtx >;
