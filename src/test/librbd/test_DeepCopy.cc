...
namespace librbd {
    ...
    struct TestDeepCopy:public TestFixture {
        ...
        void deep_copy() {
            ...
            EXPECT_EQ(0,
                      librbd::api::Image <>::deep_copy(m_src_ictx,
                                                       m_src_ictx->md_ctx,
        ...
        void compare() {
            ...
            for (size_t i = 0; i <= src_snaps.size(); i++) {
                ...
                EXPECT_EQ(0,
                          librbd::api::Image <>::snap_set(m_src_ictx,
                                                          cls::rbd::
                                                          UserSnapshotNamespace
                                                          (), src_snap_name));
                EXPECT_EQ(0,
                          librbd::api::Image <>::snap_set(m_dst_ictx,
                                                          cls::rbd::
                                                          UserSnapshotNamespace
                                                          (), dst_snap_name));
                uint64_t src_size, dst_size;
                {
                    ...
                    m_src_ictx->image_lock};
                    std::shared_lock dst_locker {
                    m_dst_ictx->image_lock};
                ...
                while (offset < src_size) {
                    ...
                    &src_bl};
                    EXPECT_EQ(read_size,
                              api::Io <>::read(*m_src_ictx, offset, read_size,
                    ...
                    &dst_bl};
                    EXPECT_EQ(read_size,
                              api::Io <>::read(*m_dst_ictx, offset, read_size,
                                               librbd::io::ReadResult {
                                               dst_result}
                                               , 0));
                    ...
                    if (!src_bl.contents_equal(dst_bl)) {
                        std::
                            cout << "snap: " << (src_snap_name ? src_snap_name :
                                                 "null")
        ...
        void test_no_snaps() {
            ...
            ASSERT_EQ(static_cast < ssize_t > (bl.length()),
                      api::Io <>::write(*m_src_ictx, 0 * bl.length(),
                                        bl.length(), bufferlist {
                                        bl}, 0));
            ASSERT_EQ(static_cast < ssize_t > (bl.length()),
                      api::Io <>::write(*m_src_ictx, 2 * bl.length(),
                                        bl.length(), bufferlist {
                                        bl}, 0));
        }
        ...
        void test_snaps() {
            ...
            ASSERT_EQ(static_cast < ssize_t > (bl.length()),
                      api::Io <>::write(*m_src_ictx, 1 * bl.length(),
                                        bl.length(), bufferlist {
                                        bl}, 0));
            bufferlist bl1;
        ...
        void test_clone_discard() {
            ...
            ASSERT_EQ(0,
                      librbd::clone(m_ioctx, m_src_ictx->name.c_str(), "snap",
        ...
        void test_clone_hide_parent() {
            ...
            ASSERT_EQ(static_cast < ssize_t > (bl.length()),
                      api::Io <>::discard(*m_src_ictx, object_size, bl.length(),
                                          false));
            ASSERT_EQ(0, api::Io <>::flush(*m_src_ictx));
            ...
            librbd::NoOpProgressContext no_op;
            ASSERT_EQ(0,
                      m_src_ictx->operations->resize(object_size, true, no_op));

            ASSERT_EQ(0, snap_create(*m_src_ictx, "snap3"));

            ASSERT_EQ(0,
                      m_src_ictx->operations->resize(2 * object_size, true,
                                                     no_op));
        }
        ...
        void test_stress() {
            ...
            for (int i = 0; i < nsnaps; i++) {
                for (int j = 0; j < nwrites; j++) {
                    ...
                    ASSERT_EQ(static_cast < ssize_t > (len),
                              api::Io <>::discard(*m_src_ictx, off, len,
                                                  false));
                }
                ...
                if (m_src_ictx->test_features(RBD_FEATURE_LAYERING)
                    && rand() % 4) {
                    ...
                    std::cout << "clone " << m_src_ictx->
                        name << " -> " << clone_name << std::endl;
                    ASSERT_EQ(0,
                              librbd::clone(m_ioctx, m_src_ictx->name.c_str(),
                                            snap_name.c_str(), m_ioctx,
                                            clone_name.c_str(), features,
                                            &order, m_src_ictx->stripe_unit,
                ...
                if (rand() % 2) {
                    ...
                    ASSERT_EQ(0,
                              m_src_ictx->operations->resize(new_size, true,
                                                             no_op));
                    {
                        std::shared_lock src_locker {
                        m_src_ictx->image_lock};
