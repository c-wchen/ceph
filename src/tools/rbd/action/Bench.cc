        ...
        namespace bench {
            ...
            struct rbd_bencher {
                ...
                explicit rbd_bencher(librbd::Image * i, io_type_t io_type,
                                     uint64_t io_size)
                :image(i), lock("rbd_bencher::lock"), in_flight(0),
                    io_type(io_type), io_size(io_size) {
                    if (io_type == IO_TYPE_WRITE) {
                        bufferptr bp(io_size);
                         memset(bp.c_str(), rand() & 0xff, io_size);
                         write_bl.push_back(bp);
                }} bool start_io(int max, uint64_t off, uint64_t len,
                                 int op_flags) {
                    {
                        Mutex::Locker l(lock);
                        if (in_flight >= max)
            ...
            int do_bench(librbd::Image & image, io_type_t io_type,
                ...
                for (off = 0; off < io_bytes;) {
                    ...
                    while (i < io_threads && off < io_bytes) {
                        if (!b.
                            start_io(io_threads, thread_offset[i], io_size,
                                     op_flags)) {
                            break;
                        }
                        if (random) {
                    ...
                    if (last.is_zero()) {
                        ...
                        printf("%5d  %8d  %8.2lf  %8.2lf\n", (int)elapsed,
                               ...
                               time_sum,
                               boost::accumulators::rolling_sum(off_acc) /
                               time_sum);
                        last = elapsed;
                    }
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "Error flushing data at the end: " <<
                        cpp_strerror(r)
                        << std::endl;
                }
                ...
                printf
                    ("elapsed: %5d  ops: %8d  ops/sec: %8.2lf  bytes/sec: %8.2lf\n",
                     (int)elapsed, ios, (double)ios / elapsed,
                     (double)off / elapsed);
            ...
            int bench_execute(const po::variables_map & vm,
                ...
                r = do_bench(image, bench_io_type, bench_io_size,
                             bench_io_threads, bench_bytes, bench_random);
                if (r < 0) {
