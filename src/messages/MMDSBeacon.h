...
inline const char *mds_metric_summary(mds_metric_t m)
{
    ...
    case MDS_HEALTH_CLIENT_RECALL_MANY:
        return
            "%num% MDSs have many clients failing to respond to cache pressure";
    case MDS_HEALTH_CLIENT_LATE_RELEASE_MANY:
    ...
    case MDS_HEALTH_CLIENT_OLDEST_TID_MANY:
        return
            "%num% MDSs have clients failing to advance oldest client/flush tid";
    case MDS_HEALTH_DAMAGE:
...
struct MDSHealthMetric {
    ...
    void encode(ceph::buffer::list & bl) const {
        ...
        return (type == other.type && sev == other.sev
                && message == other.message);
    } MDSHealthMetric():type(MDS_HEALTH_NULL), sev(HEALTH_OK) {
    }
    MDSHealthMetric(mds_metric_t type_, health_status_t sev_,
                                      ...
                                      COMPAT_VERSION)
{
    set_priority(CEPH_MSG_PRIO_HIGH);
             ...
             MDSMap::DaemonState st, version_t se,
             uint64_t feat):PaxosServiceMessage(MSG_MDS_BEACON, les,
                                                HEAD_VERSION, COMPAT_VERSION),
    fsid(f), global_id(g), name(n), state(st), seq(se), mds_features(feat) {
    set_priority(CEPH_MSG_PRIO_HIGH);
}
~MMDSBeacon()final {
}
...
const uuid_d & get_fsid() const {
    return fsid;
} mds_gid_t get_global_id() const {
    return global_id;
} const std::string & get_name() const {
    return name;
} epoch_t get_last_epoch_seen() const {
    return version;
...
const std::string & get_fs() const {
    return fs;
} void set_fs(std::string_view s) {
    fs = s;
}

const std::map < std::string, std::string > &get_sys_info() const {
    return sys_info;
} void set_sys_info(const std::map < std::string, std::string > &i) {
    sys_info = i;
}
