    ...
    namespace io {
        ...
        struct ReadResult::AssembleResultVisitor:public boost::static_visitor < void > {
            CephContext *cct;
             ...
             AssembleResultVisitor(CephContext * cct,
                                   Striper::StripedReadResult & destriper)
            :cct(cct), destriper(destriper) {
                ...
                bufferlist bl;
                 ...
                 ldout(cct,
                    ...
                    length() << " bytes to iovec " << reinterpret_cast <
                    const void *>(vector.iov) << dendl;
                ...
                for (; offset < length && idx < vector.iov_count; idx++) {
                    size_t len =
                        std::min(vector.iov[idx].iov_len, length - offset);
                     it.copy(len,
                             static_cast < char *>(vector.iov[idx].iov_base));
                     offset += len;
                ...
                ldout(cct,
                      20) << "moved resulting " << bufferlist.bl->
                    length() << " " << "bytes to bl " << reinterpret_cast <
                    void *>(bufferlist.bl)
                << dendl;
                ...
                auto buffer_extents_length =
                    destriper.assemble_result(cct, &buffer_extent_map,
                 ...
                 sparse_bufferlist.extent_map->reserve(buffer_extent_map.
                                                       size());
                ...
                for (auto[image_offset, image_length]:sparse_bufferlist.
                     image_extents) {
                    while (bem_it != buffer_extent_map.end()) {
                        auto[buffer_extent_offset, buffer_extent_length] =
                            *bem_it;

                        if (buffer_offset + image_length <=
                        ...
                        auto image_extent_offset =
                            image_offset + (buffer_extent_offset -
                                            buffer_offset);
                        ldout(cct,
                              20) << "mapping buffer extent " <<
                            buffer_extent_offset << "~" << buffer_extent_length
                            << " to image extent " << image_extent_offset << "~"
                            << buffer_extent_length << dendl;
                        sparse_bufferlist.extent_map->
                            emplace_back(image_extent_offset,
                                         buffer_extent_length);
                        ++bem_it;
                    }
                ...
                ldout(cct,
                    ...
                    extent_map << " extents of total " << sparse_bufferlist.bl->
                    length()
                << " bytes to bl " << reinterpret_cast <
                    void *>(sparse_bufferlist.bl) << dendl;
            }
        };

        ReadResult::C_ImageReadRequest::C_ImageReadRequest(AioCompletion *
                                                           aio_completion,
                                                           uint64_t
                                                           buffer_offset,
                                                           const Extents
        ...
        void ReadResult::C_ImageReadRequest::finish(int r) {
            ...
            if (r >= 0 || (ignore_enoent && r == -ENOENT)) {
              ...
              for (auto & image_extent:image_extents) {
                    buffer_extents.emplace_back(buffer_offset + length,
                                                image_extent.second);
                ...
                aio_completion->read_result.m_destriper.add_partial_result(cct,
                                                                           std::
                                                                           move
                                                                           (bl),
                                                                           buffer_extents);
        ...
        ReadResult::C_ObjectReadRequest::C_ObjectReadRequest(AioCompletion *
                                                             aio_completion,
                                                             ReadExtents
                                                             && extents)
      :    aio_completion(aio_completion), extents(std::move(extents)) {
            ...
            if (r >= 0) {
              ...
              for (auto & extent:extents) {
                    ...
                    aio_completion->read_result.m_destriper.
                        add_partial_sparse_result(cct, std::move(extent.bl),
                                                  extent.extent_map,
                                                  extent.offset,
        ...
        ReadResult::C_ObjectReadMergedExtents::
            ...
            if (r >= 0) {
              for (auto & extent:*extents) {
                    if (bl.length() < extent.length) {
                        lderr(cct) <<
                            "Merged extents length is less than expected" <<
                            dendl;
                        r = -EIO;
                        break;
                    }
                ...
                if (bl.length() != 0) {
                    lderr(cct) <<
                        "Merged extents length is greater than expected" <<
                        dendl;
                    r = -EIO;
                }
            }
      ...
      :    m_buffer(SparseBufferlist(extent_map, bl)) {
        ...
        void ReadResult::set_image_extents(const Extents & image_extents) {
            boost::apply_visitor(SetImageExtentsVisitor(image_extents),
                                 m_buffer);
        }

        void ReadResult::assemble_result(CephContext * cct) {
            boost::apply_visitor(AssembleResultVisitor(cct, m_destriper),
                                 m_buffer);
        }
