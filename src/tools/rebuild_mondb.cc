...
static int check_fsid(const uuid_d & fsid, MonitorDBStore & ms)
{
    ...
    if (fsid != existing) {
        cerr << "error: cluster_uuid " << existing << " != " << fsid << std::
            endl;
        return -EEXIST;
    }
    return 0;
