...
public:
    ...
    RGWPeriodHistory::Cursor find_oldest_period(const DoutPrefixProvider *dpp,
            optional_yield y);
    ...
    /// period history
    RGWPeriodHistory::Cursor read_oldest_log_period(optional_yield y,
            const DoutPrefixProvider *
            dpp) const;
    ...
    RGWCoroutine *trim_log_period_cr(const DoutPrefixProvider *dpp,
                                     RGWPeriodHistory::Cursor period,
                                     RGWObjVersionTracker *objv) const;
    int read_history(RGWMetadataLogHistory *state,
    ...
    int add_entry(const DoutPrefixProvider *dpp, const std::string &hash_key,
    ...
    } int pull_period(const DoutPrefixProvider *dpp,
