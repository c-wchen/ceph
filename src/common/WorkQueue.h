...
class ThreadPool:public md_config_obs_t {
  ...
  public:
     class TPHandle {
      ...
      public:
         TPHandle(CephContext * cct,
                  heartbeat_handle_d * hb, time_t grace, time_t suicide_grace)
  ...
  template < class T > class BatchWorkQueue:public WorkQueue_ {
        ThreadPool *pool;
  ...
  template < typename T, typename U = T > class WorkQueueVal:public WorkQueue_
    {
      ...
      :    
        WorkQueue_(std::move(n), ti, sti), _lock("WorkQueueVal::lock"),
        pool(p) {
            pool->add_work_queue(this);
        }
...
class GenContextWQ:public ThreadPool::WorkQueueVal < GenContext <
    ThreadPool::TPHandle & >*> {
...
class ShardedThreadPool {
    ...
    false};
    std::atomic < bool > pause_threads = {
    false};
    std::atomic < bool > drain_threads = {
    false};
  ...
  public:
    ...
    class BaseShardedWQ {
      ...
      BaseShardedWQ(time_t ti, time_t sti):timeout_interval(ti),
        ...
        virtual void _process(uint32_t thread_index, heartbeat_handle_d * hb) =
            0;
  ...
  private:
    ...
    struct WorkThreadSharded:public Thread {
        ...
        WorkThreadSharded(ShardedThreadPool * p,
                          uint32_t pthread_index):pool(p),
  ...
  public:
    ...
    ShardedThreadPool(CephContext * cct_, string nm, string tn,
