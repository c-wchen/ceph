...
void Elector::defer(int who)
{
    ...
    MMonElection *m =
        new MMonElection(MMonElection::OP_ACK, epoch, mon->monmap);
    m->mon_features = ceph::features::mon::get_supported();
    ...
    m->sharing_bl =
        mon->get_local_commands_bl(mon->get_required_mon_features());
...
void Elector::expire()
{
    ...
    // did i win?
    if (electing_me && acked_me.size() > (unsigned)(mon->monmap->size() / 2)) {
        // i win
        victory();
    }
    else {
