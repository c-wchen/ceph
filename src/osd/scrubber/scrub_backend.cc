...
, m_incomplete_clones_allowed {
m_pool.info.allow_incomplete_clones()}

, m_conf {
m_scrubber.get_pg_cct()->_conf}

, clog {
m_scrubber.get_logger()}
...
ScrubBackend::ScrubBackend(ScrubBeListener & scrubber,
                           PgScrubBeListener & pg,
                           pg_shard_t i_am,
                           bool repair, scrub_level_t shallow_or_deep)
:  m_scrubber
{
scrubber}
...
uint64_t ScrubBackend::logical_to_ondisk_size(uint64_t logical_size) const const
{
    ...
    if (g_conf()->subsys.should_gather < ceph_subsys_osd, 15 > ()) {
      for (const auto & rpl:m_acting_but_me) {
            dout(15) << fmt::format("{}: replica {} has {} items",
                                    __func__,
                                    rpl,
                                    this_chunk->received_maps[rpl].objects.
                                    size())
                << dendl;
        }
    }
...
std::vector < snap_mapper_fix_t >
    ScrubBackend::replica_clean_meta(ScrubMap & repl_map, bool max_reached,
                                     const hobject_t & start,
                                     SnapMapReaderI & snaps_getter)
{
    dout(15) << __func__ << ": REPL META # " << m_cleaned_meta_map.objects.
        size()
        << " objects" << dendl;
...
{
    const bool needs_omap_check = std::any_of(this_chunk->received_maps.begin(),
                                              this_chunk->received_maps.end(),
                                              [](const auto & m)->bool {
                                              return m.second.
                                              has_large_omap_object_errors
                                              || m.second.has_omap_keys;}
    );
  ...
  for (const auto & ho:this_chunk->authoritative_set) {
      ...
      for (const auto &[srd, smap]:this_chunk->received_maps) {
            ...
            if (smap_obj.large_omap_object_found) {
                ...
                pg_t pg;
                osdmap->map_to_pg(ho.pool, ho.oid.name, ho.get_key(), ho.nspace,
                                  &pg);
                pg_t mpg = osdmap->raw_pg_to_pg(pg);
                m_omap_stats.large_omap_objects++;
                wss << "Large omap object found. Object: " << ho << " PG: " <<
                    pg << " (" << mpg << ")" << " Key count: " << smap_obj.
                    large_omap_object_key_count << " Size (bytes): " <<
...
void ScrubBackend::update_authoritative()
{
  ...
  for (auto &[obj, peers]:this_chunk->authoritative) {
      ...
      for (auto & peer:peers) {
            good_peers.emplace_back(this_chunk->received_maps[peer].
                                    objects[obj], peer);
        }
...
int ScrubBackend::scrub_process_inconsistent()
{
    dout(20) << fmt::format("{}: {} (m_repair:{}) good peers tbl #: {}",
                            __func__,
                            m_mode_desc, m_repair, m_auth_peers.size())
        << dendl;
                                 ...
                                 const set < pg_shard_t > &bad_peers)
{
    if (g_conf()->subsys.should_gather < ceph_subsys_osd, 20 > ()) {
      ...
      for (const auto & peer:ok_peers) {
        ...
        dout(10) << fmt::
            format("repair_object {} bad_peers osd.{{{}}}, ok_peers osd.{{{}}}",
                   soid, bad_peers, ok_shards)
            << dendl;
    }
    ...
    m_pg.force_object_missing(ScrubberPasskey {
                              }
                              , bad_peers, soid, oi.version);
}
                                                  ...
                                                  stringstream & errstream)
{
  ...
  for (auto & l:shards) {
        ...
        if (auth_version != eversion_t() &&
            ...
            dout(10) << fmt::
                format("{}: digest_match = false, {} data_digest 0x{:x} != "
                       "data_digest 0x{:x}", __func__, ho,
        ...
        dout(20)
            << fmt::format("{}: {} shard {} got:{:D}", __func__, ho, l,
                           shard_ret)
            << dendl;
        ...
        if (shard_ret.possible_auth == shard_as_auth_t::usable_t::not_usable) {
            ...
            errstream << m_pg_id.
                pgid << " shard " << l << " soid " << ho << " : " << shard_ret.
                error_text << "\n";
            ...
            dout(15) << fmt::format("{}: {} not found on shard {}", __func__,
                                    ho, l)
                << dendl;
        }
        else {
            ...
            dout(30) << fmt::format("{}: consider using {} srv: {} oi soid: {}",
                                    __func__,
                                    l, shard_ret.oi.version, shard_ret.oi.soid)
                << dendl;
            ...
            if (auth_version == eversion_t()
                ...
                dout(20) << fmt::
                    format("{}: using {} moved auth oi {:p} <-> {:p}", __func__,
                           l, (void *)&ret_auth.auth_oi, (void *)&shard_ret.oi)
                    << dendl;
                                                  ...
                                                  shard_info_map_t & shard_map)
{
    ...
    dup_error_cond(err,
                   ...
                   "candidate had a read error" sv, errstream);
    dup_error_cond(err,
                   true,
    ...
    if (obj.is_head()) {
        ...
        if (dup_error_cond(err,
            ...
            // debug@dev only
            dout(30) << fmt::
                format("{} missing snap addr: {:p} shard_info: {:p} er: {:x}",
    ...
    if (test_error_cond(smap_obj.size != logical_to_ondisk_size(oi.size),
        ...
        errstream << sep(err) << "candidate size " << smap_obj.
            size << " info size " << logical_to_ondisk_size(oi.
                                                            size) <<
            " mismatch";
    }
...
void ScrubBackend::compare_smaps()
{
    ...
    std::for_each(this_chunk->authoritative_set.begin(),
                  this_chunk->authoritative_set.end(),[this] (const auto & ho) {
    ...
    ScrubBackend::compare_obj_in_maps(const hobject_t & ho)
{
    ...
    if (opt_ers.has_value()) {
        ...
        inconsistents(ho,
                      auth_object,
                      auth_res.auth_oi, std::move(*opt_ers), errstream);
    }
    else {
                                 ...
                                 stringstream & errstream)
{
    ...
    this_chunk->cur_inconsistent.insert(object_errors.begin(), object_errors.end());    // merge?

    dout(15) << fmt::
        format("{}: object errors #: {}  auth list #: {}  cur_missing #: {}  "
               "cur_incon #: {}", __func__, object_errors.size(),
               auth_list.size(), this_chunk->cur_missing.size(),
    ...
    if (this_chunk->fix_digest) {
        ...
        this_chunk->missing_digest.
            push_back(make_pair(ho, make_pair(data_digest, omap_digest)));
    }
    ...
    if (!this_chunk->cur_inconsistent.empty() ||
        ...
        case digest_fixing_t::if_aged:{
                ...
                const auto age_limit =
                    m_conf->osd_deep_scrub_update_digest_min_age;
...
ScrubBackend::digest_fixing_t ScrubBackend::
should_fix_digest(const hobject_t & ho, const ScrubMap::object & auth_object,
                  const object_info_t & auth_oi, bool repair_flag,
                  stringstream & errstream)
{
    digest_fixing_t update {
    digest_fixing_t::no};
...
ScrubBackend::auth_and_obj_errs_t ScrubBackend::
match_in_shards(const hobject_t & ho, auth_selection_t & auth_sel,
                inconsistent_obj_wrapper & obj_result, stringstream & errstream)
{
  ...
  for (auto &[srd, smap]:this_chunk->received_maps) {
        ...
        if (smap.objects.count(ho)) {
            ...
            dout(20) << fmt::format("{}: {}{} <{}:{}> shards: {} {} {}",
                                    __func__, (m_repair ? "repair " : ""),
            ...
            if (discrep_found) {
                dout(10) << fmt::
                    format("{}: <{}> auth:{} ({}/{}) vs {} ({}/{}) {}",
                           __func__, ho, auth_sel.auth_shard,
            ...
            this_chunk->m_error_counts.shallow_errors++;
            errstream << m_pg_id << " shard " << srd << " " << ho <<
                " : missing\n";
        }
        obj_result.add_shard(srd, auth_sel.shard_map[srd]);

        dout(20) << __func__ << ": (debug) soid " << ho << " : " << errstream.
            str()
            << dendl;
    }
    ...
    dout(15) << fmt::format("{}: auth_list: {} #: {}; obj-errs#: {}",
                            __func__,
                            auth_list, auth_list.size(), object_errors.size())
        << dendl;
    return {
    auth_list, object_errors};
}
                                       ...
                                       bool has_snapset)
{
    ...
    if (m_is_replicated) {
        ...
        if (auth_oi.is_omap_digest() && candidate.omap_digest_present &&
            ...
            fmt::format_to(std::back_inserter(out),
                           ...
                           sep(error),
                           candidate.omap_digest, auth_oi.omap_digest, auth_oi);
    ...
    if (!shard_result.has_info_missing() && !shard_result.has_info_corrupted()) {
        ...
        if (!can_bl.contents_equal(auth_bl)) {
            fmt::format_to(std::back_inserter(out),
                           "{}object info inconsistent ", sep(error));
    ...
    if (has_snapset) {
        if (!shard_result.has_snapset_missing() &&
            ...
            if (!can_bl.contents_equal(auth_bl)) {
                fmt::format_to(std::back_inserter(out),
                               "{}snapset inconsistent ", sep(error));
  ...
  for (const auto &[k, v]:auth.attrs) {
        ...
        if (cand == candidate.attrs.end()) {
            fmt::format_to(std::back_inserter(out),
                           "{}attr name mismatch '{}'", sep(error), k);
            obj_result.set_attr_name_mismatch();
        }
        else if (cand->second.cmp(v)) {
            fmt::format_to(std::back_inserter(out),
                           "{}attr value mismatch '{}'", sep(error), k);
...
static inline bool doing_clones(const std::optional < SnapSet > &snapset,
                                const vector <
                                snapid_t >::reverse_iterator & curclone)
{
...
{
    dout(10) << __func__ << " num stat obj " << m_pg.get_pg_info(ScrubberPasskey {
                                                                 }).stats.stats.
        sum.num_objects << dendl;
    ...
    for (auto p = map.objects.rbegin(); p != map.objects.rend(); ++p) {
        ...
        if (soid.has_snapset()) {
            ...
            if (head && (head_error.errors || soid_error_count)) {
                this_chunk->m_inconsistent_objs.
                    push_back(std::move(head_error));
            }
            ...
            if (snapset) {
                ...
                if (!snapset->clones.empty()) {
                    ...
                    if (snapset->seq == 0) {
                        clog.
                            error() << m_mode_desc << " " << m_pg_id << " " <<
                            soid << " : snaps.seq not set";
                        this_chunk->m_error_counts.shallow_errors++;
            ...
            dout(20) << __func__ << " " << m_mode_desc << " matched clone " <<
                soid << dendl;
            ...
            if (snapset->clone_size.count(soid.snap) == 0) {
                ...
                if (snapset->clone_overlap.count(soid.snap) == 0) {
                    ...
                    if (bad_interval_set) {
                        clog.
                            error() << m_mode_desc << " " << m_pg_id << " " <<
                            soid << " : bad interval_set in clone_overlap";
                        this_chunk->m_error_counts.shallow_errors++;
                        soid_error.set_size_mismatch();
                    }
                    else {
                                                           ...
                                                           snaps_getter)
{
    ...
    for (auto i = smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
        ...
        if (hoid.snap < CEPH_MAXSNAP) {
            ...
            auto maybe_fix_order =
                scan_object_snaps(hoid, snapset, snaps_getter);
            if (maybe_fix_order) {
...
std::optional < snap_mapper_fix_t >
    ScrubBackend::scan_object_snaps(const hobject_t & hoid,
                                    ...
                                    SnapMapReaderI & snaps_getter)
{
    ...
    if (!cur_snaps) {
        ...
        case result_t::code_t::not_found:
            ...
            return snap_mapper_fix_t {
                snap_mapper_op_t::add, hoid, obj_snaps, {
                }
            };
        case result_t::code_t::inconsistent:
            dout(10) << __func__ << ": inconsistent snapmapper data for " <<
    ...
    dout(20) << fmt::format("{}: obj {}: was: {} updating to: {}", __func__,
                            hoid, *cur_snaps, obj_snaps)
        << dendl;
    return snap_mapper_fix_t {
