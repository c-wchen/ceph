    ...
    : sealed(false)
{
    switch (params.get_type()) {
        ...
        case TYPE_EXPLICIT_HASH:
            impl.
            reset(new
                  ExplicitHashHitSet(static_cast <
                                     ExplicitHashHitSet::Params *
                                     >(params.impl.get())));
            break;
        ...
        case TYPE_EXPLICIT_OBJECT:
            impl.
            reset(new
                  ExplicitObjectHitSet(static_cast <
                                       ExplicitObjectHitSet::Params *
                                       >(params.impl.get())));
            break;
...
void HitSet::encode(ceph::buffer::list &bl) const const
{
    ENCODE_START(1, 1, bl);
    encode(sealed, bl);
    if (impl) {
...
void HitSet::dump(Formatter *f) const const
{
...
void HitSet::Params::encode(ceph::buffer::list &bl) const const
{
    ENCODE_START(1, 1, bl);
    if (impl) {
        encode((__u8) impl->get_type(), bl);
...
void HitSet::Params::dump(Formatter *f) const const
{
...
void ExplicitHashHitSet::dump(Formatter *f) const const
{
...
void ExplicitObjectHitSet::dump(Formatter *f) const const
{
    f->dump_unsigned("insert_count", count);
...
void BloomHitSet::Params::dump(Formatter *f) const const
{
...
void BloomHitSet::dump(Formatter *f) const const
{
