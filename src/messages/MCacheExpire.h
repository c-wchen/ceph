...
public:
    ...
    struct realm
    {
        ...
        void merge(realm &o)
        {
            ...
            for (map<dirfrag_t, map<pair<string, snapid_t>,
                                    uint32_t>>::iterator p = o.dentries.begin();
                 p != o.dentries.end(); ++p)
            {
                if (dentries.count(p->first) == 0)
                    dentries[p->first] = p->second;
                else
                    dentries[p->first].insert(p->second.begin(),
                                              p->second.end());
            }
        }
    ...
    MCacheExpire(int f) : Message(MSG_MDS_CACHEEXPIRE), from(f)
    {
    }
...
public:
    ...
    void add_dentry(dirfrag_t r, dirfrag_t df, boost::string_view dn,
                    snapid_t last, unsigned nonce)
    {
        realms[r].dentries[df][pair<string,
                                    snapid_t>(std::string(dn), last)] = nonce;
    }
