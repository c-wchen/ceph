...
class ObjBencher {
  ...
  protected:
    ...
    int write_bench(int secondsToRun, int concurrentios,
                    const string & run_name_meta, unsigned max_objects);
    int seq_read_bench(int secondsToRun, int num_objects, int concurrentios,
                       int writePid, bool no_verify = false);
  ...
  explicit ObjBencher(CephContext * cct_):show_time(false), cct(cct_),
    ...
    int aio_bench(int operation, int secondsToRun,
                  ...
                  const std::string & run_name, bool no_verify = false);
