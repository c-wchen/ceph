...
std::string to_string(const rgw_bucket_shard & bs,
    ...
    spawn(new CR(env->dpp, env->driver,
                 rgw_raw_obj(env->svc->zone->get_zone_params().log_pool,
                             RGWDataSyncStatusManager::shard_obj_name(sc->
                                                                      source_zone,
                                                                      shard_id)),
                 &markers[shard_id], true, &objvs[shard_id]), false);
    shard_id++;
    return true;
}
...
class RGWReadDataSyncRecoveringShardsCR:public RGWShardCollectCR {
  ...
  public:
    RGWReadDataSyncRecoveringShardsCR(RGWDataSyncCtx * sc,
                                      uint64_t _max_entries, int _num_shards,
                                      std::vector <
                                      RGWRadosGetOmapKeysCR::ResultPtr >
                                      &omapkeys)
  :    RGWShardCollectCR(sc->cct, MAX_CONCURRENT_SHARDS), sc(sc), env(sc->env),
        max_entries(_max_entries), num_shards(_num_shards),
        omapkeys(omapkeys) {
    }
    bool spawn_next() override;
};
...
bool RGWReadDataSyncRecoveringShardsCR::spawn_next()
{
    ...
    spawn(new
          RGWRadosGetOmapKeysCR(env->driver,
                                rgw_raw_obj(env->svc->zone->get_zone_params().
                                            log_pool, error_oid), marker,
                                max_entries, shard_keys), false);
    ...
    reenter(this) {
        ...
        yield {
            ...
            call(new ReadInfoCR(dpp, sync_env->driver,
                                rgw_raw_obj(sync_env->svc->zone->
                                            get_zone_params().log_pool,
                                            RGWDataSyncStatusManager::
                                            sync_status_oid(sc->source_zone)),
...
class RGWReadRemoteDataLogShardInfoCR:public RGWCoroutine {
  ...
  public:
    RGWReadRemoteDataLogShardInfoCR(RGWDataSyncCtx * _sc,
                                    ...
                                    _shard_info):RGWCoroutine(_sc->cct),
        sc(_sc), sync_env(_sc->env), http_op(NULL), shard_id(_shard_id),
    ...
    int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            yield {
                ...
                http_op =
                    new RGWRESTReadResource(sc->conn, p, pairs, NULL,
                ...
                if (ret < 0) {
                    ldpp_dout(dpp,
                              0) << "ERROR: failed to read from " << p << dendl;
...
class RGWReadRemoteDataLogShardCR:public RGWCoroutine {
  ...
  public:
    RGWReadRemoteDataLogShardCR(RGWDataSyncCtx * _sc, int _shard_id,
                                ...
                                bool * _truncated)
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), shard_id(_shard_id),
        marker(marker), pnext_marker(pnext_marker), entries(_entries),
...
#define READ_DATALOG_MAX_CONCURRENT 10
  ...
  public:
  RGWReadRemoteDataLogInfoCR(RGWDataSyncCtx * _sc, int _num_shards, map < int, RGWDataChangesLogInfo > *_datalog_info):RGWShardCollectCR(_sc->cct, READ_DATALOG_MAX_CONCURRENT),
...
bool RGWReadRemoteDataLogInfoCR::spawn_next()
{
    ...
    spawn(new
          RGWReadRemoteDataLogShardInfoCR(sc, shard_id,
                                          &(*datalog_info)[shard_id]), false);
    shard_id++;
    return true;
}
...
class RGWListRemoteDataLogShardCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    :RGWSimpleCoroutine(sc->cct), sc(sc), sync_env(sc->env), http_op(NULL),
        ...
        snprintf(max_entries_buf, sizeof(max_entries_buf), "%d",
                 (int)max_entries);
        ...
        http_op =
            new RGWRESTReadResource(conn, p, pairs, NULL,
                                    sync_env->http_manager);
        init_new_io(http_op);
        ...
        if (ret < 0) {
            ...
            log_error() << "failed to send http operation: " << http_op->
                to_str() << " ret=" << ret << std::endl;
            http_op->put();
            return ret;
        }
    ...
    int request_complete() override {
        ...
        http_op->put();
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(sync_env->dpp,
                      0) << "ERROR: failed to list remote datalog shard, ret="
                << ret << dendl;
            return ret;
        }
        return 0;
...
#define READ_DATALOG_MAX_CONCURRENT 10
  ...
  public:
  RGWListRemoteDataLogCR(RGWDataSyncCtx * _sc, map < int, string > &_shards, int _max_entries_per_shard, map < int, rgw_datalog_shard_data > *_result):RGWShardCollectCR(_sc->cct, READ_DATALOG_MAX_CONCURRENT),
        sc(_sc), sync_env(_sc->env),
        max_entries_per_shard(_max_entries_per_shard), result(_result) {
        shards.swap(_shards);
        iter = shards.begin();
    }
    bool spawn_next() override;
...
bool RGWListRemoteDataLogCR::spawn_next()
{
    ...
    spawn(new
          RGWListRemoteDataLogShardCR(sc, iter->first, iter->second,
...
class RGWInitDataSyncStatusCoroutine:public RGWCoroutine {
    static constexpr auto lock_name {
    "sync_lock" sv};
  ...
  public:
    RGWInitDataSyncStatusCoroutine(RGWDataSyncCtx * _sc, uint32_t num_shards,
                                   ...
                                   boost::intrusive_ptr < RGWContinuousLeaseCR >
                                   lease_cr,
                                   RGWObjVersionTracker & objv_tracker,
                                   std::vector < RGWObjVersionTracker > &objvs)
    :RGWCoroutine(_sc->cct), sc(_sc), num_shards(num_shards), status(status),
        ...
        return new RGWContinuousLeaseCR(sc->env->async_rados, sc->env->driver, {
                                        sc->env->svc->zone->get_zone_params().
                                        log_pool,
                                        RGWDataSyncStatusManager::
                                        sync_status_oid(sc->source_zone)}
                                        , string(lock_name), lock_duration,
                                        caller, &sc->lcc);
    }
    ...
    int operate(const DoutPrefixProvider * dpp) override {
        ...
        reenter(this) {
            ...
            yield call(new WriteInfoCR(dpp, sync_env->driver, rgw_raw_obj {
                                       pool, sync_status_oid}
                                       , status->sync_info, &objv_tracker));
            if (retcode < 0) {
                tn->log(0,
                        SSTR("ERROR: failed to write sync status info with " <<
                             retcode));
                return set_cr_error(retcode);
            }
            ...
            yield {
                RGWRESTConn *conn =
                    sync_env->svc->zone->get_zone_conn(sc->source_zone);
                if (!conn) {
                    tn->log(0,
                            SSTR("ERROR: connection to zone " << sc->
                                 source_zone << " does not exist!"));
                    return set_cr_error(-EIO);
                }
                for (uint32_t i = 0; i < num_shards; i++) {
                    spawn(new
                          RGWReadRemoteDataLogShardInfoCR(sc, i,
                                                          &shards_info[i]),
                          true);
                }
            }
            while (collect(&ret, NULL)) {
                if (ret < 0) {
                    tn->log(0,
                            SSTR
                            ("ERROR: failed to read remote data log shards"));
            ...
            yield {
                ...
                for (uint32_t i = 0; i < num_shards; i++) {
                    ...
                    using WriteMarkerCR =
                        RGWSimpleRadosWriteCR < rgw_data_sync_marker >;
                    spawn(new WriteMarkerCR(dpp, sync_env->driver, rgw_raw_obj {
                                            pool, oid}, marker, &objv), true);
                }
            }
            while (collect(&ret, NULL)) {
                if (ret < 0) {
                    tn->log(0,
                            SSTR
                            ("ERROR: failed to write data sync status markers"));
                    return set_state(RGWCoroutine_Error);
                }
                yield;
            }

            status->sync_info.state =
                                    ...
                                    rgw_datalog_info * log_info)
{
    ...
    int ret =
        sc.conn->get_json_resource(dpp, "/admin/log", pairs, null_yield,
                                   *log_info);
    if (ret < 0) {
...
int RGWRemoteDataLog::read_source_log_shards_info(const DoutPrefixProvider *
                                                  dpp, map < int,
                                                  RGWDataChangesLogInfo >
                                                  *shards_info)
{
    ...
    return run(dpp,
               new RGWReadRemoteDataLogInfoCR(&sc, log_info.num_shards,
                                              shards_info));
}

int RGWRemoteDataLog::read_source_log_shards_next(const DoutPrefixProvider *
                                                  dpp, map < int,
                                                  string > shard_markers,
                                                  map < int,
                                                  rgw_datalog_shard_data >
                                                  *result)
{
    return run(dpp, new RGWListRemoteDataLogCR(&sc, shard_markers, 1, result));
}

int RGWRemoteDataLog::init(const rgw_zone_id & _source_zone,
                           RGWRESTConn * _conn,
                           ...
                           PerfCounters * counters)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  0) << "failed in http_manager.start() ret=" << ret << dendl;
        return ret;
    }

    tn = sync_env.sync_tracer->add_node(sync_env.sync_tracer->root_node,
                                        "data");
                                       ...
                                       rgw_data_sync_status * sync_status)
{
    ...
    ret =
        crs.run(dpp,
                new RGWReadDataSyncStatusCoroutine(&sc_local, sync_status,
                                                   &objv, shard_objvs));
    http_manager.stop();
    return ret;
}
                                             ...
                                             set < int >&recovering_shards)
{
    ...
    ret =
        crs.run(dpp,
                new RGWReadDataSyncRecoveringShardsCR(&sc_local, max_entries,
                                                      num_shards, omapkeys));
    http_manager.stop();
...
namespace RGWRDL {
    class DataSyncInitCR:public RGWCoroutine {
      ...
      public:
        ...
        DataSyncInitCR(RGWDataSyncCtx * sc, uint32_t num_shards,
                       uint64_t instance_id, const RGWSyncTraceNodeRef & tn,
                       rgw_data_sync_status * sync_status,
                       std::vector < RGWObjVersionTracker > &objvs)
        :RGWCoroutine(sc->cct), sc(sc), num_shards(num_shards),
            ...
            reenter(this) {
                lease_cr.
                    reset(RGWInitDataSyncStatusCoroutine::
                          continuous_lease_cr(sc, this));

                yield spawn(lease_cr.get(), false);
                while (!lease_cr->is_locked()) {
                    if (lease_cr->is_done()) {
                        tn->log(5,
                                "ERROR: failed to take data sync status lease");
                ...
                yield call(new
                           RGWInitDataSyncStatusCoroutine(sc, num_shards,
                                                          instance_id, tn,
                                                          sync_status, lease_cr,
                                                          objv_tracker, objvs));
                lease_cr->go_down();
                lease_cr.reset();
                drain_all();
                                             ...
                                             int shard_id)
{
    ...
    snprintf(buf, sizeof(buf), "%s.%s.%d",
             datalog_sync_full_sync_index_prefix.c_str(),
        ...
        reenter(this) {
            yield {
                ...
                call(new RGWReadRESTResourceCR < rgw_bucket_index_marker_info >
                     (sync_env->cct, sc->conn, sync_env->http_manager, p, pairs,
                      info));
            }
            if (retcode < 0) {
                return set_cr_error(retcode);
...
class RGWListBucketIndexesCR:public RGWCoroutine {
  ...
  public:
    ...
    int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            entries_index =
                std::make_unique < RGWShardedOmapCRManager >
            ...
            do {
                ...
                for (iter = result.keys.begin(); iter != result.keys.end();
                     ++iter) {
                    ldpp_dout(dpp,
                              20) <<
                    ...
                    yield {
                        ...
                        call(new RGWReadRESTResourceCR <
                             bucket_instance_meta_info > (sync_env->cct,
                                                          sc->conn,
                                                          sync_env->
                                                          http_manager, path,
                                                          pairs, &meta_info));
                    ...
                    if (retcode < 0) {
                        ldpp_dout(dpp,
                                  0) <<
                            "ERROR: failed to fetch metadata for key: " << key
                            << dendl;
                        return set_cr_error(retcode);
                    }
                    ...
                    // shard count.
                    yield entries_index->append(fmt::format("{}:{}", key, 0),
                                                sync_env->svc->datalog_rados->
                                                get_log_shard_id(meta_info.data.
                                                                 get_bucket_info
                                                                 ().bucket, 0));
                }
            ...
            if (!failed) {
                for (auto iter = sync_status->sync_markers.begin();
                    ...
                    marker.total_entries =
                        entries_index->get_total_entries(shard_id);
                    spawn(new RGWSimpleRadosWriteCR < rgw_data_sync_marker >
                          (dpp, sync_env->driver,
                           rgw_raw_obj(sync_env->svc->zone->get_zone_params().
                                       log_pool,
                                       RGWDataSyncStatusManager::
                                       shard_obj_name(sc->source_zone,
                                                      shard_id)), marker,
                           &objvs[shard_id]), true);
                }
            }
            else {
                yield call(sync_env->error_logger->
                           log_error_cr(dpp, sc->conn->get_remote_id(),
                                        "data.init", "", EIO,
                                        string
                                        ("failed to build bucket instances map")));
            }
            while (collect(&ret, NULL)) {
                if (ret < 0) {
                    yield call(sync_env->error_logger->
...
class RGWDataSyncShardMarkerTrack:public RGWSyncShardMarkerTrack < string,
  ...
  public:
    RGWDataSyncShardMarkerTrack(RGWDataSyncCtx * _sc,
                                ...
                                objv):RGWSyncShardMarkerTrack
        (DATA_SYNC_UPDATE_MARKER_WINDOW), sc(_sc), sync_env(_sc->env),
        marker_oid(_marker_oid), sync_marker(_marker), tn(_tn), objv(objv) {
        ...
        tn->log(20,
                SSTR("updating marker marker_oid=" << marker_oid << " marker="
                     << new_marker));
...
struct rgw_sync_pipe_info_entity {
  ...
  public:
    ...
    void update_empty_bucket_info(const std::map < rgw_bucket,
                                  all_bucket_info > &buckets_info) {
        if (_has_bucket_info) {
            return;
        }
...
struct rgw_sync_pipe_handler_info {
    ...
    } rgw_sync_pipe_handler_info(const RGWBucketSyncFlowManager::
                                 pipe_handler & _handler,
                                 std::optional < all_bucket_info >
                                 source_bucket_info,
                                 std::optional < all_bucket_info >
                                 target_bucket_info):handler(_handler),
        source(handler.source, source_bucket_info), target(handler.dest,
                                                           target_bucket_info) {
    }
...
class RGWRunBucketSourcesSyncCR:public RGWCoroutine {
  ...
  public:
     RGWRunBucketSourcesSyncCR(RGWDataSyncCtx * _sc,
                               boost::intrusive_ptr <
                               const RGWContinuousLeaseCR > lease_cr,
...
class RGWDataSyncSingleEntryCR:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        ...
        set_description() << "data sync single entry (source_zone=" << sc->
            source_zone << ") " << obligation;
        tn = sync_env->sync_tracer->add_node(_tn_parent, "entry",
                                             to_string(obligation.bs,
                                                       obligation.gen));
    } int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            if (state->obligation) {
                ...
                if (state->obligation->timestamp < obligation.timestamp) {
                    ...
                    tn->log(10,
                            SSTR("canceling existing obligation " << *state->
                                 obligation));
                    ...
                    *state->obligation = std::move(obligation);
                    ...
                    tn->log(10,
                            SSTR("canceling new obligation " << obligation));
                ...
                while ((state->obligation->timestamp == ceph::real_time() ||
                        ...
                        state->obligation->timestamp)
                       && obligation_counter != state->counter) {
                    obligation_counter = state->counter;
                    progress = ceph::real_time {
                    };

                    ldout(cct, 4) << "starting sync on " << bucket_shard_str {
                    state->key.first}
                    <<' ' << *state->
                        obligation << " progress timestamp " << state->
                        progress_timestamp << " progress " << progress << dendl;
                    yield call(new
                               RGWRunBucketSourcesSyncCR(sc, lease_cr,
                                                         state->key.first, tn,
                ...
                tn->log(10, SSTR("sync finished on " << bucket_shard_str {
                                 state->key.first}
            ...
            if (sync_status < 0) {
                ...
                if (sync_status != -EBUSY && sync_status != -EAGAIN) {
                    yield call(sync_env->error_logger->
                               log_error_cr(dpp, sc->conn->get_remote_id(),
                                            "data", to_string(complete->bs,
                                                              complete->gen),
                                            -sync_status,
                                            string
                                            ("failed to sync bucket instance: ")
                                            + cpp_strerror(-sync_status)));
                    if (retcode < 0) {
                        tn->log(0,
                                SSTR
                                ("ERROR: failed to log sync failure: retcode="
                                 << retcode));
                    }
                }
                if (complete->timestamp != ceph::real_time {
                    ...
                    tn->log(10,
                            SSTR("writing " << *complete <<
                                 " to error repo for retry"));
                    yield call(rgw::error_repo::
                               write_cr(sync_env->driver->svc()->rados,
                                        error_repo,
                                        rgw::error_repo::encode_key(complete->
                                                                    bs,
                                                                    complete->
                                                                    gen),
                                        complete->timestamp));
                ...
                yield call(rgw::error_repo::
                           remove_cr(sync_env->driver->svc()->rados, error_repo,
                                     rgw::error_repo::encode_key(complete->bs,
                                                                 complete->gen),
                                     complete->timestamp));
                if (retcode < 0) {
                    tn->log(0,
...
class RGWDataIncrementalSyncFullObligationCR:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), source_bs(_source_bs),
        error_repo(error_repo), error_marker(_error_marker),
        ...
        reenter(this) {
            yield call(new
                       RGWReadRemoteBucketIndexLogInfoCR(sc, source_bs.bucket,
                                                         &remote_info));
            if (retcode < 0) {
                return set_cr_error(retcode);
            }
            ...
            for (; each != remote_info.generations.cend(); each++) {
                for (sid = 0; sid < each->num_shards; sid++) {
                    ...
                    error_repo =
                        datalog_oid_for_error_repo(sc, sync_env->driver, pool,
                                                   source_bs);
                    tn->log(10,
                            SSTR("writing shard_id " << sid << " of gen " <<
                                 each->gen << " to error repo for retry"));
                    yield_spawn_window(rgw::error_repo::
                                       write_cr(sync_env->driver->svc()->rados,
                                                error_repo,
                                                rgw::error_repo::encode_key(bs,
                                                                            each->
                                                                            gen),
                                                timestamp),
                                       sc->lcc.adj_concurrency(cct->_conf->
            ...
            drain_all_cb([&](uint64_t stack_id, int ret) {
                         if (ret < 0) {
                         tn->log(10,
                                 SSTR("writing to error repo returned error: "
                                      << ret));}
                         return ret;}
            ) ;
...
RGWCoroutine *data_sync_single_entry(RGWDataSyncCtx * sc,
                                     ...
                                     marker_tracker, rgw_raw_obj error_repo,
...
class RGWDataFullSyncSingleEntryCR:public RGWCoroutine {
  ...
  public:
    RGWDataFullSyncSingleEntryCR(RGWDataSyncCtx * _sc, const rgw_pool & _pool,
                                 ...
                                 ceph::real_time _timestamp,
                                 boost::intrusive_ptr <
                                 const RGWContinuousLeaseCR > _lease_cr,
                                 boost::intrusive_ptr <
                                 rgw::bucket_sync::Cache > _bucket_shard_cache,
                                 RGWDataSyncShardMarkerTrack * _marker_tracker,
                                 RGWSyncTraceNodeRef & _tn)
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), pool(_pool),
        ...
        error_inject =
            (sync_env->cct->_conf->rgw_sync_data_full_inject_err_probability >
             0);
    } int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            if (error_inject &&
                ...
                10000.0) {
                tn->log(0,
                        SSTR("injecting read bilog info error on key=" << key));
                retcode = -ENOENT;
            }
            else {
                tn->log(0, SSTR("read bilog info key=" << key));
                yield call(new
                           RGWReadRemoteBucketIndexLogInfoCR(sc,
                                                             source_bs.bucket,
                                                             &remote_info));
            }
            ...
            if (retcode < 0) {
                ...
                yield call(rgw::error_repo::
                           write_cr(sync_env->driver->svc()->rados, error_repo,
                                    rgw::error_repo::encode_key(source_bs,
                                                                std::nullopt),
                                    timestamp));
                if (retcode < 0) {
                    tn->log(0,
                            SSTR("ERROR: failed to log " << source_bs.
                                 shard_id << " in error repo: retcode=" <<
                                 retcode));
                }
                ...
                for (sid = 0; sid < each->num_shards; sid++) {
                    ...
                    error_repo =
                        datalog_oid_for_error_repo(sc, sync_env->driver, pool,
                                                   source_bs);
                    timestamp =
                        timestamp_for_bucket_shard(sync_env->driver,
                                                   sync_status, source_bs);
                    if (retcode < 0) {
                        tn->log(10,
                                SSTR("Write " << source_bs.
                                     shard_id << " to error repo for retry"));
                        yield_spawn_window(rgw::error_repo::
                                           write_cr(sync_env->driver->svc()->
                                                    rados, error_repo,
                                                    rgw::error_repo::
                                                    encode_key(source_bs,
                                                               each->gen),
                                                    timestamp),
                                           sc->lcc.adj_concurrency(cct->_conf->
                                                                   rgw_data_sync_spawn_window),
                        ...
                        shard_cr =
                            data_sync_single_entry(sc, source_bs, each->gen,
                                                   key, timestamp, lease_cr,
                                                   bucket_shard_cache, nullptr,
                                                   error_repo, tn, false);
                        tn->log(10,
                                SSTR("full sync: syncing shard_id " << sid <<
                                     " of gen " << each->gen));
                        if (first_shard) {
                            yield call(shard_cr);
                            first_shard = false;
...
class RGWDataBaseSyncShardCR:public RGWCoroutine {
  protected:
    ...
    RGWDataBaseSyncShardCR(RGWDataSyncCtx * const _sc, const rgw_pool & pool,
                           const uint32_t shard_id,
                           rgw_data_sync_marker & sync_marker,
                           RGWSyncTraceNodeRef tn, const string & status_oid,
                           const rgw_raw_obj & error_repo,
                           boost::intrusive_ptr < const RGWContinuousLeaseCR >
                           lease_cr, const rgw_data_sync_status & sync_status,
                           RGWObjVersionTracker & objv,
                           const boost::intrusive_ptr <
                           rgw::bucket_sync::Cache > &bucket_shard_cache)
...
class RGWDataFullSyncShardCR:public RGWDataBaseSyncShardCR {
  ...
  public:
    ...
    RGWDataFullSyncShardCR(RGWDataSyncCtx * const sc, const rgw_pool & pool,
                           const uint32_t shard_id,
                           rgw_data_sync_marker & sync_marker,
                           RGWSyncTraceNodeRef tn, const string & status_oid,
                           const rgw_raw_obj & error_repo,
    ...
    :RGWDataBaseSyncShardCR(sc, pool, shard_id, sync_marker, tn, status_oid,
                            error_repo, std::move(lease_cr), sync_status, objv,
                            bucket_shard_cache) {
    } int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            ...
            do {
                ...
                omapvals =
                    std::make_shared < RGWRadosGetOmapValsCR::Result > ();
                yield call(new
                           RGWRadosGetOmapValsCR(sc->env->driver,
                ...
                for (; iter != entries.end(); ++iter) {
                    ...
                    if (retcode < 0) {
                        tn->log(1,
                                SSTR("failed to parse bucket shard: " << iter->
                                     first));
                    ...
                    if (!marker_tracker->start(iter->first, total_entries,
                                               entry_timestamp)) {
                        tn->log(0,
                                SSTR("ERROR: cannot start syncing " << iter->
                                     first << ". Duplicate entry?"));
                    }
                    else {
                        tn->log(10,
                                SSTR("timestamp for " << iter->
                                     first << " is :" << entry_timestamp));
                        yield_spawn_window(new
                                           RGWDataFullSyncSingleEntryCR(sc,
                                                                        pool,
                                                                        source_bs,
                                                                        iter->
                                                                        first,
            ...
            if (retcode < 0) {
                tn->log(0,
                        SSTR("ERROR: failed to set sync marker: retcode=" <<
                             retcode));
                return set_cr_error(retcode);
            }

            // clean up full sync index, ignoring errors
            yield call(new RGWRadosRemoveCR(sc->env->driver, {
                                            pool, oid}
                       ));
...
class RGWDataIncSyncShardCR:public RGWDataBaseSyncShardCR {
  ...
  public:
    ...
    RGWDataIncSyncShardCR(RGWDataSyncCtx * const sc, const rgw_pool & pool,
                          const uint32_t shard_id,
                          rgw_data_sync_marker & sync_marker,
                          RGWSyncTraceNodeRef tn, const string & status_oid,
                          const rgw_raw_obj & error_repo,
                          ...
                          lease_cr, const rgw_data_sync_status & sync_status,
                          RGWObjVersionTracker & objv,
                          const boost::intrusive_ptr < rgw::bucket_sync::Cache >
                          &bucket_shard_cache, ceph::mutex & inc_lock,
                          bc::flat_set < rgw_data_notify_entry >
                          &modified_shards)
  :    RGWDataBaseSyncShardCR(sc, pool, shard_id, sync_marker, tn, status_oid,
                               error_repo, std::move(lease_cr), sync_status,
                               objv, bucket_shard_cache), inc_lock(inc_lock),
        ...
        reenter(this) {
            ...
            do {
                ...
                for (modified_iter = current_modified.begin();
                    ...
                    if (!lease_cr->is_locked()) {
                        ...
                        if (retcode < 0) {
                            tn->log(0,
                                    SSTR
                                    ("ERROR: data sync marker_tracker.flush() returned retcode="
                                     << retcode));
                            return set_cr_error(retcode);
                        }
                        return set_cr_error(-ECANCELED);
                    ...
                    tn->log(20, SSTR("received async update notification: "
                                     << modified_iter->key));
                    spawn(data_sync_single_entry
                          (sc, source_bs, modified_iter->gen, {
                           }
                           , ceph::real_time {
                           }
                           , lease_cr,
                ...
                if (error_retry_time <= ceph::coarse_real_clock::now()) {
                    ...
                    tn->log(20,
                            SSTR("read error repo, got " << error_entries.size()
                                 << " entries"));
                    ...
                    for (; iter != error_entries.end(); ++iter) {
                        ...
                        entry_timestamp =
                            rgw::error_repo::decode_value(iter->second);
                        retcode =
                            rgw::error_repo::decode_key(iter->first, source_bs,
                                                        gen);
                        if (retcode == -EINVAL) {
                        ...
                        if (retcode < 0) {
                            tn->log(1,
                                    SSTR("failed to parse bucket shard: " <<
                                         error_marker));
                            spawn(rgw::error_repo::
                                  remove_cr(sc->env->driver->svc()->rados,
                                            error_repo, error_marker,
                        ...
                        if (!gen) {
                            ...
                            spawn(new
                                  RGWDataIncrementalSyncFullObligationCR(sc,
                                                                         source_bs,
                                                                         error_repo,
                                                                         error_marker,
                                                                         entry_timestamp,
                            ...
                            tn->log(20, SSTR("handle error entry key="
                                             ...
                                             entry_timestamp));
                            spawn(data_sync_single_entry
                                  (sc, source_bs, gen, "", entry_timestamp,
                                   lease_cr, bucket_shard_cache,
                                   &*marker_tracker, error_repo, tn, true),
                                  false);
                        }
                    }
                    if (!omapvals->more) {
                ...
                for (log_iter = log_entries.begin();
                     log_iter != log_entries.end(); ++log_iter) {
                    if (!lease_cr->is_locked()) {
                        drain_all();
                        yield call(marker_tracker->flush());
                        if (retcode < 0) {
                    ...
                    tn->log(20,
                            SSTR("shard_id=" << shard_id << " log_entry: " <<
                                 log_iter->log_id << ":" << log_iter->
                                 log_timestamp << ":" << log_iter->entry.key));
                    retcode = parse_bucket_key(log_iter->entry.key, source_bs);
                    if (retcode < 0) {
                        tn->log(1, SSTR("failed to parse bucket shard: "
                                        << log_iter->entry.key));
                        marker_tracker->try_update_high_marker(log_iter->log_id,
                                                               0,
                                                               log_iter->
                                                               log_timestamp);
                        continue;
                    ...
                    if (!marker_tracker->start(log_iter->log_id, 0,
                        ...
                        tn->log(0,
                                SSTR("ERROR: cannot start syncing " <<
                                     log_iter->log_id << ". Duplicate entry?"));
                    }
                    else {
                        tn->log(1,
                                SSTR("incremental sync on " << log_iter->entry.
                                     key << "shard: " << shard_id << "on gen "
                                     << log_iter->entry.gen));
                        yield_spawn_window(data_sync_single_entry
                                           (sc, source_bs, log_iter->entry.gen,
                                            log_iter->log_id,
                                            log_iter->log_timestamp, lease_cr,
                                            bucket_shard_cache,
                                           ...
                                           tn->log(10,
                                                   SSTR
                                                   ("data_sync_single_entry returned error: "
                                                    << ret)); cbret = ret;}
                                           return 0;}
                        ) ;
                    }
...
class RGWDataSyncShardCR:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(_sc->cct), sc(_sc), pool(pool), shard_id(shard_id),
        ...
        set_description() << "data sync shard source_zone=" << sc->
            source_zone << " shard_id=" << shard_id;
    }
        ...
        reenter(this) {
            ...
            *reset_backoff = true;
            ...
            yield call(new RGWSimpleRadosReadCR < rgw_data_sync_marker >
                       (sync_env->dpp, sync_env->driver,
                        rgw_raw_obj(pool, status_oid), &sync_marker, true,
                        &objv));
            if (retcode < 0) {
                lease_cr->go_down();
                drain_all();
            ...
            while (true) {
                if (sync_marker.state == rgw_data_sync_marker::FullSync) {
                    ...
                    if (retcode < 0) {
                        if (retcode != -EBUSY) {
                            tn->log(10,
                                    SSTR("full sync failed (retcode=" << retcode
                                         << ")"));
                        }
                        lease_cr->go_down();
                        drain_all();
                ...
                else if (sync_marker.state ==
                         rgw_data_sync_marker::IncrementalSync) {
                    yield call(new
                               RGWDataIncSyncShardCR(sc, pool, shard_id,
                                                     sync_marker, tn,
                    ...
                    if (retcode < 0) {
                        if (retcode != -EBUSY) {
                            tn->log(10,
                                    SSTR("incremental sync failed (retcode=" <<
                                         retcode << ")"));
                        }
                        lease_cr->go_down();
                        drain_all();
...
class RGWDataSyncShardControlCR:public RGWBackoffControlCR {
  ...
  public:
    ...
    :RGWBackoffControlCR(_sc->cct, false), sc(_sc), sync_env(_sc->env),
        ...
        return new RGWDataSyncShardCR(sc, pool, shard_id, sync_marker,
                                      sync_status, tn, objv, backoff_ptr());
    }

    RGWCoroutine *alloc_finisher_cr() override {
        return new RGWSimpleRadosReadCR < rgw_data_sync_marker > (sync_env->dpp,
                                                                  sync_env->
                                                                  driver,
                                                                  rgw_raw_obj
                                                                  (sync_env->
...
class RGWDataSyncCR:public RGWCoroutine {
  ...
  public:
    RGWDataSyncCR(RGWDataSyncCtx * _sc, uint32_t _num_shards,
                  RGWSyncTraceNodeRef & _tn,
                  bool * _reset_backoff):RGWCoroutine(_sc->cct), sc(_sc),
    ...
    int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            ...
            if (retcode < 0 && retcode != -ENOENT) {
                tn->log(0,
                        SSTR("ERROR: failed to fetch sync status, retcode=" <<
                             retcode));
                return set_cr_error(retcode);
            }
            ...
            if ((rgw_data_sync_info::SyncState) sync_status.sync_info.state !=
                ...
                obj_version.clear();
                yield call(new
                           RGWReadDataSyncStatusCoroutine(sc, &sync_status,
                                                          &obj_version, objvs));
                if (retcode < 0) {
                    tn->log(0,
                            SSTR("ERROR: failed to fetch sync status, retcode="
            ...
            if ((rgw_data_sync_info::SyncState) sync_status.sync_info.state ==
                ...
                uint64_t instance_id;
                instance_id =
                    ceph::util::generate_random_number < uint64_t > ();
                yield call(new
                           RGWInitDataSyncStatusCoroutine(sc, num_shards,
                                                          instance_id, tn,
                                                          &sync_status,
                ...
                if (retcode < 0) {
                    tn->log(0,
                            SSTR("ERROR: failed to init sync, retcode=" <<
                                 retcode));
                    init_lease->go_down();
                    drain_all();
                    return set_cr_error(retcode);
            ...
            if ((rgw_data_sync_info::SyncState) sync_status.sync_info.state ==
                ...
                if (retcode < 0) {
                    tn->log(0,
                            SSTR
                            ("ERROR: sync module init_sync() failed, retcode="
                             << retcode));
                    return set_cr_error(retcode);
                }
                ...
                if (retcode < 0) {
                    tn->log(0,
                            SSTR
                            ("ERROR: failed to build full sync maps, retcode="
                             << retcode));
                    return set_cr_error(retcode);
                }
                ...
                if (retcode < 0) {
                    tn->log(0,
                            SSTR("ERROR: failed to write sync status, retcode="
                                 << retcode));
                    return set_cr_error(retcode);
                }
            ...
            if (retcode < 0) {
                tn->log(0,
                        SSTR("ERROR: failed to start sync, retcode=" <<
                             retcode));
                return set_cr_error(retcode);
            }
            ...
            if ((rgw_data_sync_info::SyncState) sync_status.sync_info.state ==
                ...
                yield {
                    tn->log(10,
                            SSTR("spawning " << num_shards << " shards sync"));
    ...
    RGWCoroutine *set_sync_info_cr() {
        return new RGWSimpleRadosWriteCR < rgw_data_sync_info > (sync_env->dpp,
                                                                 sync_env->
                                                                 driver,
                                                                 rgw_raw_obj
                                                                 (sync_env->
...
class RGWUserPermHandler {
    ...
    struct Init:public RGWGenericAsyncCR::Action {
        ...
        Init(RGWUserPermHandler * handler):sync_env(handler->sync_env),
            uid(handler->uid), info(handler->info) {
        } int operate() override {
            auto user_ctl = sync_env->driver->getRados()->ctl.user;

            ret =
                user_ctl->get_info_by_uid(sync_env->dpp, uid, &info->user_info,
                                          null_yield);
            if (ret < 0) {
                return ret;
            } info->identity = rgw::auth::transform_old_authinfo(sync_env->cct, uid, RGW_PERM_FULL_CONTROL, false,  /* system_request? */
                                                                 TYPE_RGW);

            map < string, bufferlist > uattrs;

            ret =
                user_ctl->get_attrs_by_uid(sync_env->dpp, uid, &uattrs,
                                           null_yield);
            if (ret == 0) {
                ret =
                    RGWUserPermHandler::policy_from_attrs(sync_env->cct, uattrs,
                                                          &info->user_acl);
            }
            if (ret == -ENOENT) {
                info->user_acl.create_default(uid,
  ...
  public:
  RGWUserPermHandler(RGWDataSyncEnv * _sync_env, const rgw_user & _uid):sync_env(_sync_env),
        uid(_uid)
    {
    }
    ...
    RGWCoroutine *init_cr() {
        ...
        return new RGWGenericAsyncCR(sync_env->cct,
                                     sync_env->async_rados, init_action);
    }
    ...
    class Bucket {
      ...
      public:
        ...
        bool verify_object_permission(const map < string,
                                      bufferlist > &obj_attrs, int perm);
    };
...
int RGWUserPermHandler::Bucket::init(RGWUserPermHandler * handler,
                                     const RGWBucketInfo & bucket_info,
                                     const map < string,
                                     bufferlist > &bucket_attrs)
{
    sync_env = handler->sync_env;
    info = handler->info;

    int r =
        RGWUserPermHandler::policy_from_attrs(sync_env->cct, bucket_attrs,
                                              &bucket_acl);
    if (r < 0) {
        return r;
    }

    ps.emplace(sync_env->cct, info->env, info->identity.get(), bucket_info, info->identity->get_perm_mask(), false, /* defer to bucket acls */
               nullptr,         /* referer */
               false);          /* request_payer */
...
bool RGWUserPermHandler::Bucket::verify_object_permission(const map < string,
                                                          bufferlist >
                                                          &obj_attrs, int perm)
{
...
class RGWFetchObjFilter_Sync:public RGWFetchObjFilter_Default {
  ...
  public:
    RGWFetchObjFilter_Sync(rgw_bucket_sync_pipe & _sync_pipe,
                           std::shared_ptr < RGWUserPermHandler::Bucket >
                           &_bucket_perms,
                           std::optional < rgw_sync_pipe_dest_params >
                           &&_verify_dest_params,
                           std::shared_ptr < bool >
                           &_need_retry):sync_pipe(_sync_pipe),
                                   ...
                                   const rgw_placement_rule ** prule)
{
    ...
    return RGWFetchObjFilter_Default::filter(cct,
                                             ...
                                             dest_bucket_info,
                                             dest_placement_rule,
                                             obj_attrs, poverride_owner, prule);
}
...
#define MAX_RACE_RETRIES_OBJ_FETCH 10
            for (try_num = 0; try_num < MAX_RACE_RETRIES_OBJ_FETCH; ++try_num) {

                {
                    ...
                    if (!sync_pipe.info.handler.
                        find_basic_info_without_tags(key, &param_user,
                ...
                if (need_more_info) {
                    ldout(cct,
                          20) <<
                        "Could not determine exact policy rule for obj=" << key
                ...
                if (param_mode == rgw_sync_pipe_params::MODE_USER) {
                    ...
                    if (r < 0) {
                        ldout(cct,
                            ...
                            *param_user << " bucket=" << sync_pipe.
                            source_bucket_info.bucket.get_key() << dendl;
                        return set_cr_error(retcode);
                    }

                    if (!dest_bucket_perms.
                        verify_bucket_permission(RGW_PERM_WRITE)) {
                        ldout(cct,
                              0) << "ERROR: " << __func__ <<
                            ": permission check failed: user not allowed to write into bucket (bucket="
                            << sync_pipe.info.dest_bucket.
                            get_key() << ")" << dendl;
                        return -EPERM;
                    }
                    ...
                    source_bucket_perms =
                        make_shared < RGWUserPermHandler::Bucket > ();
                ...
                yield {
                    ...
                    auto filter =
                        make_shared < RGWFetchObjFilter_Sync > (sync_pipe,
                                                                source_bucket_perms,
                                                                std::
                                                                move
                                                                (dest_params),
                    ...
                    call(new
                         RGWFetchRemoteObjCR(sync_env->async_rados,
                                             sync_env->driver, sc->source_zone,
                                             nullopt,
                                             sync_pipe.info.source_bs.bucket,
                                             std::nullopt,
                                             sync_pipe.dest_bucket_info, key,
                                             dest_key, versioned_epoch, true,
...
RGWCoroutine *RGWDefaultDataSyncModule::sync_object(const DoutPrefixProvider *
                                                    dpp, RGWDataSyncCtx * sc,
                                                    rgw_bucket_sync_pipe &
                                                    sync_pipe,
                                                    rgw_obj_key & key,
                                                    std::optional < uint64_t >
                                                    versioned_epoch,
                                                    const rgw_zone_set_entry &
                                                    source_trace_entry,
                                                    rgw_zone_set * zones_trace)
{
...
RGWCoroutine *RGWDefaultDataSyncModule::remove_object(const DoutPrefixProvider *
                                                      dpp, RGWDataSyncCtx * sc,
                                                      rgw_bucket_sync_pipe &
                                                      sync_pipe,
                                                      rgw_obj_key & key,
                                                      ...
                                                      zones_trace)
{
    ...
    return new RGWRemoveObjCR(sync_env->dpp, sync_env->async_rados,
                              sync_env->driver, sc->source_zone,
                              sync_pipe.dest_bucket_info, key, versioned,
                              versioned_epoch, NULL, NULL, false, &mtime,
                              zones_trace);
}

RGWCoroutine *RGWDefaultDataSyncModule::
create_delete_marker(const DoutPrefixProvider * dpp, RGWDataSyncCtx * sc,
                     ...
                     rgw_zone_set * zones_trace)
{
    ...
    return new RGWRemoveObjCR(sync_env->dpp, sync_env->async_rados,
                              sync_env->driver, sc->source_zone,
                              sync_pipe.dest_bucket_info, key, versioned,
                              versioned_epoch, &owner.id, &owner.display_name,
                              true, &mtime, zones_trace);
}
...
RGWCoroutine *RGWArchiveDataSyncModule::sync_object(const DoutPrefixProvider *
                                                    dpp, RGWDataSyncCtx * sc,
                                                    rgw_bucket_sync_pipe &
                                                    sync_pipe,
                                                    rgw_obj_key & key,
                                                    ...
                                                    rgw_zone_set * zones_trace)
{
    auto sync_env = sc->env;
    ldout(sc->cct,
          5) << "SYNC_ARCHIVE: sync_object: b=" << sync_pipe.info.source_bs.
    ...
    if (!sync_pipe.dest_bucket_info.versioned()
        ...
        ldout(sc->cct,
              0) <<
            "SYNC_ARCHIVE: sync_object: enabling object versioning for archive bucket"
            << dendl;
        sync_pipe.dest_bucket_info.flags =
            (sync_pipe.dest_bucket_info.
             flags & ~BUCKET_VERSIONS_SUSPENDED) | BUCKET_VERSIONED;
        int op_ret =
            sync_env->driver->getRados()->put_bucket_instance_info(sync_pipe.
                                                                   dest_bucket_info,
                                                                   false,
                                                                   real_time(),
                                                                   NULL,
        ...
        if (op_ret < 0) {
            ldpp_dout(sync_env->dpp,
                      0) <<
                "SYNC_ARCHIVE: sync_object: error versioning archive bucket" <<
                dendl;
            return NULL;
        }
    }
...
RGWCoroutine *RGWArchiveDataSyncModule::remove_object(const DoutPrefixProvider *
                                                      dpp, RGWDataSyncCtx * sc,
                                                      rgw_bucket_sync_pipe &
                                                      sync_pipe,
                                                      rgw_obj_key & key,
...
{
    ldout(sc->cct,
          0) << "SYNC_ARCHIVE: remove_object: b=" << sync_pipe.info.source_bs.
        bucket << " k=" << key << " versioned_epoch=" << versioned_epoch <<
        dendl;
    return NULL;
}
...
{
    ldout(sc->cct,
          0) << "SYNC_ARCHIVE: create_delete_marker: b=" << sync_pipe.info.
        source_bs.
...
class RGWDataSyncControlCR:public RGWBackoffControlCR {
  ...
  public:
    RGWDataSyncControlCR(RGWDataSyncCtx * _sc, uint32_t _num_shards,
                         RGWSyncTraceNodeRef &
                         _tn_parent):RGWBackoffControlCR(_sc->cct,
                                                         exit_on_error),
        sc(_sc), sync_env(_sc->env), num_shards(_num_shards) {
...
CephContext *RGWDataSyncStatusManager::get_cct() const const
{
    return driver->ctx();
}
...
int RGWDataSyncStatusManager::init(const DoutPrefixProvider * dpp)
{
    ...
    if (!(zone_def = driver->svc()->zone->find_zone(source_zone))) {
        ldpp_dout(this,
                  0) << "ERROR: failed to find zone config info for zone=" <<
            source_zone << dendl;
        return -EIO;
    }

    if (!driver->svc()->sync_modules->get_manager()->
    ...
    if (!conn) {
        ldpp_dout(this,
                  0) << "connection object to zone " << source_zone <<
            " does not exist" << dendl;
        return -EINVAL;
    }

    error_logger =
        new RGWSyncErrorLogger(driver, RGW_SYNC_ERROR_LOG_SHARD_PREFIX,
                               ERROR_LOGGER_SHARDS);

    int r =
        source_log.init(source_zone, conn, error_logger,
                        driver->getRados()->get_sync_tracer(),
                        sync_module, counters);
    if (r < 0) {
        ldpp_dout(this,
                  0) << "ERROR: failed to init remote log, r=" << r << dendl;
        finalize();
        return r;
    }
    ...
    if (r < 0) {
        ldpp_dout(this,
                  5) << "ERROR: master.read_log_info() returned r=" << r <<
            dendl;
        finalize();
        return r;
    }
...
unsigned RGWDataSyncStatusManager::get_subsys() const const
{
    return dout_subsys;
}

std::ostream & RGWDataSyncStatusManager::gen_prefix(std::ostream & out) const const
{
    auto zone = std::string_view { source_zone.id };
    return out << "data sync zone:" << zone.substr(0, 8) << ' ';
}

string RGWDataSyncStatusManager::
sync_status_oid(const rgw_zone_id & source_zone)
{
    char buf[datalog_sync_status_oid_prefix.size() + source_zone.id.size() +
             16];
...
string RGWDataSyncStatusManager::shard_obj_name(const rgw_zone_id & source_zone,
                                                int shard_id)
{
    char buf[datalog_sync_status_shard_prefix.size() + source_zone.id.size() +
             16];
    snprintf(buf, sizeof(buf), "%s.%s.%d",
...
class RGWInitBucketShardSyncStatusCoroutine:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), sync_pair(_sync_pair),
        sync_status_oid(RGWBucketPipeSyncStatusManager::
                        inc_status_oid(sc->source_zone, _sync_pair, gen)),
        ...
        reenter(this) {
            ...
            if (retcode < 0) {
                ldout(cct,
                      20) << "ERROR: init marker position failed. error: " <<
                    retcode << dendl;
                return set_cr_error(retcode);
            }
            ldout(cct,
                  20) << "init marker position: " << status.inc_marker.
                position << ". written to shard status object: " <<
                sync_status_oid << dendl;
            return set_cr_done();
        }
        return 0;
    }
...
template < class T >
    static bool decode_attr(CephContext * cct, map < string,
                            bufferlist > &attrs, const string & attr_name,
                            T * val)
{
    ...
    try {
        ...
        ldout(cct,
              0) << "ERROR: failed to decode attribute: " << attr_name << dendl;
        return false;
    }
    return true;
}
...
void rgw_bucket_shard_sync_info::encode_all_attrs(map < string,
                                                  bufferlist > &attrs)
{
...
void rgw_bucket_shard_sync_info::encode_state_attr(map < string,
                                                   bufferlist > &attrs)
{
    using ceph::encode;
...
void rgw_bucket_shard_full_sync_marker::encode_attr(map < string,
                                                    bufferlist > &attrs)
{
    using ceph::encode;
...
void rgw_bucket_shard_inc_sync_marker::encode_attr(map < string,
                                                   bufferlist > &attrs)
{
    using ceph::encode;
...
class RGWReadBucketPipeSyncStatusCoroutine:public RGWCoroutine {
  ...
  public:
    RGWReadBucketPipeSyncStatusCoroutine(RGWDataSyncCtx * _sc,
                                         ...
                                         uint64_t gen)
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        oid(RGWBucketPipeSyncStatusManager::
            inc_status_oid(sc->source_zone, sync_pair, gen)), status(_status),
        objv_tracker(objv_tracker) {
    } int operate(const DoutPrefixProvider * dpp) override;
};

int RGWReadBucketPipeSyncStatusCoroutine::operate(const DoutPrefixProvider *
                                                  dpp)
{
    reenter(this) {
        yield call(new RGWSimpleRadosReadAttrsCR(dpp, sync_env->driver,
                                                 rgw_raw_obj(sync_env->svc->
                                                             zone->
                                                             get_zone_params().
                                                             log_pool, oid),
        ...
        if (retcode < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to call fetch bucket shard info oid="
...
class CheckBucketShardStatusIsIncremental:public
  ...
  public:
    ...
    :RGWCoroutine(sc->cct), sc(sc), pair(pair), gen(gen), marker_mgr(marker_mgr) {
        ...
        reenter(this) {
            ...
            objv.generate_new_write_ver(cct);
            yield call(new
                       RGWInitBucketShardSyncStatusCoroutine(sc, pair, status,
                                                             gen, marker_mgr,
                                                             objv, false));
            if (retcode < 0) {
                return set_cr_error(retcode);
...
class InitBucketShardStatusCollectCR:public RGWShardCollectCR {
  ...
  :    
    ...
    sc(sc), sync_pair(sync_pair), gen(gen), marker_mgr(marker_mgr),
    ...
    bool spawn_next() override {
        ...
        sync_pair.source_bs.shard_id = shard++;
        spawn(new InitBucketShardStatusCR(sc, sync_pair, gen, marker_mgr),
              false);
        return true;
    }
};
  ...
  public:
    ...
    :RGWCoroutine(sc->cct), sc(sc), sync_env(sc->env), sync_pair(sync_pair),
        ...
        reenter(this) {
            ...
            if (retcode < 0 && retcode != -ENOENT) {
                ldout(cct,
                      20) << "ERROR: failed to remove bucket shard status for: "
...
class RemoveBucketShardStatusCollectCR:public RGWShardCollectCR {
  ...
  public:
    RemoveBucketShardStatusCollectCR(RGWDataSyncCtx * sc,
                                     const rgw_bucket_sync_pair_info &
                                     sync_pair, uint64_t gen, int num_shards)
        ...
        reenter(this) {
            ...
            if (info.oldest_gen == 0) {
                if (check_compat) {
                    ...
                    if (source_info.layout.logs.empty() ||
                        ...
                        ldpp_dout(dpp,
                                  20) <<
                            "no generation zero when checking compatibility" <<
                            dendl;
                        no_zero = true;
                    }
                    ...
                    if (!no_zero) {
                        yield {
                            const int num_shards0 =
                                rgw::num_shards(source_info.layout.logs.front().
                                                layout.in_index.layout);
                            call(new
                                 CheckAllBucketShardStatusIsIncremental(sc,
                                                                        sync_pair,
                                                                        num_shards0,
                                                                        &all_incremental));
                        }
            ...
            if (status.state != BucketSyncState::Incremental) {
                ...
                yield {
                    ...
                    call(new
                         InitBucketShardStatusCollectCR(sc, sync_pair,
                                                        info.latest_gen,
                                                        marker_mgr,
                                                        num_shards));
                }
            ...
            ldout(cct,
                  20) << "writing bucket sync status during init. state=" <<
                status.state << ". marker=" << status.full.position << dendl;
...
int RGWReadRecoveringBucketShardsCoroutine::operate(const DoutPrefixProvider *
                                                    dpp)
{
    reenter(this) {
        ...
        do {
            ...
            yield call(new
                       RGWRadosGetOmapKeysCR(driver,
                                             rgw_raw_obj(sync_env->svc->zone->
                                                         get_zone_params().
                                                         log_pool, error_oid),
                                             marker, max_omap_entries,
                                             omapkeys));
...
class RGWReadPendingBucketShardsCoroutine:public RGWCoroutine {
  ...
  public:
    RGWReadPendingBucketShardsCoroutine(RGWDataSyncCtx * _sc,
                                        const int _shard_id,
...
int RGWReadPendingBucketShardsCoroutine::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        yield call(new CR(dpp, sync_env->driver,
                          rgw_raw_obj(sync_env->svc->zone->get_zone_params().
                                      log_pool, status_oid), sync_marker));
        if (retcode < 0) {
                                        ...
                                        const int max_entries)
{
    ...
    recovering_stack->
        call(new
             RGWReadRecoveringBucketShardsCoroutine(&sc_local, shard_id,
                                                    recovering_buckets,
                                                    max_entries));
    ...
    pending_stack->
        call(new
             RGWReadPendingBucketShardsCoroutine(&sc_local, shard_id,
                                                 pending_buckets, sync_marker,
                                                 max_entries));
...
CephContext *RGWBucketPipeSyncStatusManager::get_cct() const const
{
    return driver->ctx();
}
...
class RGWBucketFullSyncMarkerTrack:public RGWSyncShardMarkerTrack < rgw_obj_key,
    rgw_obj_key > {
    RGWDataSyncCtx *sc;
    RGWDataSyncEnv *sync_env;
  ...
  public:
        ...
        sync_status(sync_status), tn(std::move(tn)), objv_tracker(objv_tracker)
    {
        ...
        tn->log(20,
                SSTR("updating marker oid=" << status_obj.
                     oid << " marker=" << new_marker));
...
class RGWWriteBucketShardIncSyncStatus:public RGWCoroutine {
  ...
  public:
    RGWWriteBucketShardIncSyncStatus(RGWDataSyncEnv * sync_env,
                                     ...
                                     const rgw_bucket_shard_inc_sync_marker &
                                     sync_marker,
                                     ceph::real_time * stable_timestamp,
                                     RGWObjVersionTracker & objv_tracker)
    :RGWCoroutine(sync_env->cct), sync_env(sync_env), obj(obj),
        ...
        reenter(this) {
            ...
            yield call(new
                       RGWSimpleRadosWriteAttrsCR(sync_env->dpp,
                                                  sync_env->driver, obj, attrs,
                                                  &objv_tracker));
            if (retcode < 0) {
                return set_cr_error(retcode);
            }
...
class RGWBucketIncSyncShardMarkerTrack:public RGWSyncShardMarkerTrack < string,
  ...
  public:
    RGWBucketIncSyncShardMarkerTrack(RGWDataSyncCtx * _sc,
                                     const string & _marker_oid,
                                     const rgw_bucket_shard_inc_sync_marker &
                                     _marker, RGWSyncTraceNodeRef tn,
                                     RGWObjVersionTracker & objv_tracker,
                                     ceph::real_time * stable_timestamp)
    :RGWSyncShardMarkerTrack(BUCKET_SYNC_UPDATE_MARKER_WINDOW), sc(_sc),
        sync_env(_sc->env), obj(sync_env->svc->zone->get_zone_params().log_pool,
                                _marker_oid), sync_marker(_marker),
        tn(std::move(tn)), objv_tracker(objv_tracker),
        stable_timestamp(stable_timestamp) {
    }

    const rgw_raw_obj & get_obj() const {
        return obj;
    } RGWCoroutine *store_marker(const string & new_marker, uint64_t index_pos,
                                 const real_time & timestamp)override {
        ...
        tn->log(20,
                SSTR("updating marker marker_oid=" << obj.
                     oid << " marker=" << new_marker << " timestamp=" <<
                     timestamp));
    ...
    bool can_do_op(const rgw_obj_key & key, bool is_olh) {
        ...
        if (is_olh && pending_olh.count(key.name)) {
            tn->log(20,
                    SSTR("sync of " << key << " waiting for pending olh op"));
            return false;
        }
...
template < class T, class K > class RGWBucketSyncSingleEntryCR:public RGWCoroutine
{
  ...
  public:
    RGWBucketSyncSingleEntryCR(RGWDataSyncCtx * _sc,
                               ...
                               RGWSyncShardMarkerTrack < T,
                               K > *_marker_tracker,
                               rgw_zone_set & _zones_trace,
                               RGWSyncTraceNodeRef &
                               _tn_parent):RGWCoroutine(_sc->cct), sc(_sc),
        sync_env(_sc->env), sync_pipe(_sync_pipe),
        bs(_sync_pipe.info.source_bs), key(_key), versioned(_versioned),
        ...
        stringstream ss;
        ss << bucket_shard_str {
        bs} <<"/" << key << "[" << versioned_epoch.value_or(0) << "]";
        set_description() << "bucket sync single entry (source_zone=" << sc->
            source_zone << ") b=" << ss.
            str() << " log_entry=" << entry_marker << " op=" << (int)op <<
        ...
        tn->log(20,
                SSTR("bucket sync single entry (source_zone=" << sc->
                     source_zone << ") b=" << ss.
                     str() << " log_entry=" << entry_marker << " op=" << (int)op
                     << " op_state=" << (int)op_state));
        error_injection =
        ...
        reenter(this) {
            ...
            do {
                yield {
                    ...
                    if (error_injection &&
                        rand() % 10000 <
                        cct->_conf->rgw_sync_data_inject_err_probability *
                        10000.0) {
                        tn->log(0,
                                SSTR(": injecting data sync error on key=" <<
                                     key.name));
                        retcode = -EIO;
                    }
                        ...
                        if (versioned_epoch) {
                            pretty_print(sc->env,
                                         ...
                                         bs.bucket.name, key, *versioned_epoch,
                                         zone_name);
                        }
                        else {
                            pretty_print(sc->env,
                        ...
                        call(data_sync_module->
                             sync_object(dpp, sc, sync_pipe, key,
                                         versioned_epoch, source_trace_entry,
                                         &zones_trace));
                    }
                        ...
                        tn->log(10,
                                SSTR("removing obj: " << sc->
                                     source_zone << "/" << bs.
                                     bucket << "/" << key << "[" <<
                                     versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->
                             remove_object(dpp, sc, sync_pipe, key, timestamp,
                                           versioned,
                                           versioned_epoch.value_or(0),
                                           &zones_trace));
                        ...
                        tn->log(10,
                                SSTR("creating delete marker: obj: " << sc->
                                     source_zone << "/" << bs.
                                     bucket << "/" << key << "[" <<
                                     versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->
                             create_delete_marker(dpp, sc, sync_pipe, key,
                                                  timestamp, owner, versioned,
                                                  versioned_epoch.value_or(0),
                                                  &zones_trace));
                    }
                    tn->set_resource_name(SSTR
                                          (bucket_str_noinstance(bs.bucket) <<
                                           "/" << key));
                }
                if (retcode == -ERR_PRECONDITION_FAILED) {
                    pretty_print(sc->env,
                                 ...
                                 bs.bucket.name, key, zone_name);
                    set_status
                        ("Skipping object sync: precondition failed (object contains newer change or policy doesn't allow sync)");
                    tn->log(0,
                            "Skipping object sync: precondition failed (object contains newer change or policy doesn't allow sync)");
                    retcode = 0;
                }
            ...
            if (!error_ss.str().empty()) {
                yield call(sync_env->error_logger->
                           log_error_cr(dpp, sc->conn->get_remote_id(), "data",
                                        error_ss.str(), -retcode,
                                        string("failed to sync object") +
                                        cpp_strerror(-sync_status)));
            }
          done:
            if (sync_status == 0) {
                /* update marker */
                set_status() << "calling marker_tracker->finish(" <<
                    entry_marker << ")";
...
class RGWBucketFullSyncCR:public RGWCoroutine {
  ...
  public:
    RGWBucketFullSyncCR(RGWDataSyncCtx * _sc,
                        ...
                        const rgw_raw_obj & status_obj,
                        boost::intrusive_ptr < const RGWContinuousLeaseCR >
                        lease_cr, rgw_bucket_sync_status & sync_status,
  ...
  :    RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        ...
        tn(sync_env->sync_tracer->
           add_node(tn_parent, "full_sync", SSTR(bucket_shard_str {
                                                 bs}))), marker_tracker(sc,
                                                                        status_obj,
                                                                        sync_status,
                                                                        tn,
                                                                        objv_tracker)
...
int RGWBucketFullSyncCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        do {
            ...
            yield call(new
                       RGWListRemoteBucketCR(sc, bs, list_marker,
                                             &list_result));
            if (retcode < 0 && retcode != -ENOENT) {
            ...
            for (; entries_iter != list_result.entries.end(); ++entries_iter) {
                ...
                if (!prefix_handler.check_key_handled(entries_iter->key)) {
                    set_status() << "skipping entry due to policy rules: " <<
                        entries_iter->key;
                    tn->log(20,
                            SSTR("skipping entry due to policy rules: " <<
                                 entries_iter->key));
                    continue;
                }
                total_entries++;
                if (!marker_tracker.
                    start(entry->key, total_entries, real_time())) {
                    tn->log(0,
                            SSTR("ERROR: cannot start syncing " << entry->
                                 key << ". Duplicate entry?"));
                }
                else {
                    using SyncCR =
                        RGWBucketSyncSingleEntryCR < rgw_obj_key, rgw_obj_key >;
                    yield spawn(new SyncCR(sc, sync_pipe, entry->key, false,    /* versioned, only matters for object removal */
                                           entry->versioned_epoch, entry->mtime,
                                           entry->owner, entry->get_modify_op(),
                                           CLS_RGW_STATE_COMPLETE, entry->key,
                                           &marker_tracker, zones_trace, tn),
                                false);
                }
                drain_with_cb(sc->lcc.
                              adj_concurrency(cct->_conf->
...
class RGWBucketShardIsDoneCR:public RGWCoroutine {
  ...
  public:
    RGWBucketShardIsDoneCR(RGWDataSyncCtx * _sc,
                           ...
                           int _shard_id, const next_bilog_result & _next_log,
                           const uint64_t _gen)
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        ...
        reenter(this) {
            do {
                ...
                yield call(new ReadCR(dpp, sync_env->driver,
                                      bucket_status_obj, &bucket_status, false,
                                      &objv_tracker));
                if (retcode < 0) {
                ...
                if (bucket_status.state != BucketSyncState::Incremental) {
                    ...
                    ldpp_dout(dpp,
                              20) <<
                        "RGWBucketShardIsDoneCR found sync state = " <<
                ...
                if (bucket_status.incremental_gen != generation) {
                    ...
                    // retried in error repo if we lost a race
                    ldpp_dout(dpp,
                              20) << "RGWBucketShardIsDoneCR expected gen: " <<
                        generation << ", got: " << bucket_status.
                        incremental_gen << dendl;
                    return set_cr_done();
                }
                ...
                yield {
                    ...
                    ldpp_dout(dpp,
                              20) << "bucket status incremental gen is " <<
                        bucket_status.incremental_gen << dendl;
                    using WriteCR =
                        RGWSimpleRadosWriteCR < rgw_bucket_sync_status >;
                    call(new
                         WriteCR(dpp, sync_env->driver, bucket_status_obj,
                                 bucket_status, &objv_tracker, false));
                }
                if (retcode < 0 && retcode != -ECANCELED) {
                    ldpp_dout(dpp,
                              20) << "failed to write bucket sync status: " <<
                        cpp_strerror(retcode) << dendl;
                    return set_cr_error(retcode);
                }
                else if (retcode >= 0) {
...
class RGWBucketShardIncrementalSyncCR:public RGWCoroutine {
  ...
  public:
    RGWBucketShardIncrementalSyncCR(RGWDataSyncCtx * _sc,
                                    ...
                                    RGWObjVersionTracker & objv_tracker,
                                    ceph::real_time * stable_timestamp)
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), sync_pipe(_sync_pipe),
        bs(_sync_pipe.info.source_bs), bucket_status_obj(_bucket_status_obj),
        lease_cr(std::move(lease_cr)), sync_info(sync_info),
        generation(generation), zone_id(sync_env->svc->zone->get_zone().id),
        tn(sync_env->sync_tracer->
           add_node(_tn_parent, "inc_sync", SSTR(bucket_shard_str {
                                                 bs}))), marker_tracker(sc,
                                                                        shard_status_oid,
                                                                        sync_info.
                                                                        inc_marker,
...
int RGWBucketShardIncrementalSyncCR::operate(const DoutPrefixProvider * dpp)
{
    ...
    reenter(this) {
        do {
            ...
            tn->log(20,
                    SSTR("listing bilog for incremental sync; position=" <<
                         sync_info.inc_marker.position));
            set_status() << "listing bilog; position=" << sync_info.inc_marker.
                position;
            yield call(new
                       RGWListBucketIndexLogCR(sc, bs,
                                               sync_info.inc_marker.position,
                                               generation, &extended_result));
            if (retcode < 0 && retcode != -ENOENT) {
                /* wait for all operations to complete */
                drain_all();
                yield spawn(marker_tracker.flush(), true);
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                auto & squash_entry =
                    squash_map[make_pair(e.object, e.instance)];
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP
                    ...
                    ldpp_dout(dpp,
                              20) << "detected syncstop or resync on " <<
                        entries_iter->timestamp << ", skipping entry" << dendl;
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                }

                if (!key.set(rgw_obj_index_key {
                             entry->object, entry->instance}
                    )) {
                    set_status() << "parse_raw_oid() on " << entry->
                        object << " returned false, skipping entry";
                    tn->log(20,
                            SSTR("parse_raw_oid() on " << entry->
                                 object << " returned false, skipping entry"));
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                ...
                tn->log(20,
                        SSTR("parsed entry: id=" << cur_id << " iter->object="
                             << entry->object << " iter->instance=" << entry->
                             instance << " name=" << key.
                             name << " instance=" << key.
                             instance << " ns=" << key.ns));

                if (!key.ns.empty()) {
                    set_status() << "skipping entry in namespace: " << entry->
                        object;
                    tn->log(20,
                            SSTR("skipping entry in namespace: " << entry->
                                 object));
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                ...
                if (entry->op == CLS_RGW_OP_CANCEL) {
                    ...
                    tn->log(20, SSTR("skipping object: " << bucket_shard_str {
                                     bs}
                                     <<"/" << key << ": canceled operation"));
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                }
                ...
                if (make_pair <> (entry->timestamp, entry->op) !=
                    ...
                    tn->log(20, SSTR("skipping object: " << bucket_shard_str {
                                     bs}
                                     <<"/" << key << ": squashed operation"));
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                }
                tn->set_flag(RGW_SNS_FLAG_ACTIVE);
                tn->log(20, SSTR("syncing object: " << bucket_shard_str {
                                 bs}
                                 <<"/" << key));
                ...
                while (!marker_tracker.can_do_op(key, has_olh_epoch(entry->op))) {
                    if (!updated_status) {
                        set_status() <<
                            "can't do op, conflicting inflight operation";
                        updated_status = true;
                    }
                    tn->log(5,
                            SSTR("can't do op on key=" << key <<
                ...
                if (!marker_tracker.
                    index_key_to_marker(key, cur_id,
                                        has_olh_epoch(entry->op))) {
                    set_status() <<
                        "can't do op, sync already in progress for object";
                    tn->log(20,
                            SSTR("skipping sync of entry: " << cur_id << ":" <<
                                 key <<
                ...
                if (!marker_tracker.start(cur_id, 0, entry->timestamp)) {
                    tn->log(0,
                            SSTR("ERROR: cannot start syncing " << cur_id <<
                                 ". Duplicate entry?"));
                }
                else {
                    std::optional < uint64_t > versioned_epoch;
                    rgw_bucket_entry_owner owner(entry->owner,
                    ...
                    using SyncCR =
                        RGWBucketSyncSingleEntryCR < string, rgw_obj_key >;
                    spawn(new
                          SyncCR(sc, sync_pipe, key, entry->is_versioned(),
                                 versioned_epoch, entry->timestamp, owner,
                                 entry->op, entry->state, cur_id,
                                 &marker_tracker, entry->zones_trace, tn),
                          false);
                }
                // }
                drain_with_cb(sc->lcc.
        ...
        if (!truncated && extended_result.next_log) {
            yield call(new
                       RGWBucketShardIsDoneCR(sc, bucket_status_obj,
                                              bs.shard_id,
            ...
            yield {
                ...
                auto status_obj =
                    sync_env->svc->rados->obj(marker_tracker.get_obj());
                retcode = status_obj.open(dpp);
                if (retcode < 0) {
                    return set_cr_error(retcode);
                }
                call(new
                     RGWRadosRemoveOidCR(sync_env->driver,
                                         std::move(status_obj)));
                if (retcode < 0) {
                    ldpp_dout(dpp,
                              20) << "failed to remove shard status object: " <<
                        cpp_strerror(retcode) << dendl;
                    return set_cr_error(retcode);
                }
            }
...
class RGWGetBucketPeersCR:public RGWCoroutine {
    ...
    static pair < pipe_const_iter,
        pipe_const_iter > get_pipe_iters(const map < rgw_zone_id,
    ...
    void filter_sources(std::optional < rgw_zone_id > source_zone,
                        std::optional < rgw_bucket > source_bucket,
                        const map < rgw_zone_id,
                        RGWBucketSyncFlowManager::pipe_set > &all_sources,
                        rgw_sync_pipe_info_set * result) {
        ldpp_dout(sync_env->dpp,
                  20) << __func__ << ": source_zone=" << source_zone.
            value_or(rgw_zone_id("*")).id << " source_bucket=" << source_bucket.
            value_or(rgw_bucket())
        ...
        for (auto i = iters.first; i != iters.second; ++i) {
          for (auto & handler:i->second) {
                if (!handler.specific()) {
                    ldpp_dout(sync_env->dpp,
                              20) << __func__ << ": pipe_handler=" << handler <<
                        ": skipping" << dendl;
                    continue;
                }
                if (source_bucket &&
    ...
    void filter_targets(std::optional < rgw_zone_id > target_zone,
                        std::optional < rgw_bucket > target_bucket,
                        const map < rgw_zone_id,
                        RGWBucketSyncFlowManager::pipe_set > &all_targets,
                        rgw_sync_pipe_info_set * result) {
        ldpp_dout(sync_env->dpp,
                  20) << __func__ << ": target_zone=" << source_zone.
            value_or(rgw_zone_id("*")).id << " target_bucket=" << source_bucket.
            value_or(rgw_bucket())
    ...
    struct GetHintTargets:public RGWGenericAsyncCR::Action {
        ...
        GetHintTargets(RGWDataSyncEnv * _sync_env,
            ...
            int r =
                sync_env->svc->bucket_sync->get_bucket_sync_hints(sync_env->dpp,
                                                                  source_bucket,
                                                                  nullptr,
                                                                  &targets,
                                                                  null_yield);
...
static RGWCoroutine *sync_bucket_shard_cr(RGWDataSyncCtx * sc,
                                          boost::intrusive_ptr <
                                          const RGWContinuousLeaseCR > lease,
                                          const rgw_bucket_sync_pair_info &
                                          sync_pair,
                                          std::optional < uint64_t > gen,
                                          const RGWSyncTraceNodeRef & tn,
...
lease_cr(std::move(lease_cr)),
tn(sync_env->sync_tracer->
   add_node(_tn_parent, "bucket_sync_sources",
            SSTR("source=" << source_bs << ":source_zone=" << sc->
                 source_zone))), progress(progress), gen(gen)
{
...
int RGWRunBucketSourcesSyncCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        yield call(new
                   RGWGetBucketPeersCR(sync_env, std::nullopt, sc->source_zone,
                                       sync_pair.source_bs.bucket, &pipes, tn));
        if (retcode < 0 && retcode != -ENOENT) {
            tn->log(0,
                    SSTR("ERROR: failed to read sync status for bucket. error: "
                         << retcode));
            return set_cr_error(retcode);
        }

        ldpp_dout(dpp,
                  20) << __func__ << "(): requested source_bs=" << sync_pair.
            source_bs << dendl;

        if (pipes.empty()) {
            ldpp_dout(dpp,
                      20) << __func__ << "(): no relevant sync pipes found" <<
                dendl;
            return set_cr_done();
        }
        ...
        drain_all_cb([&](uint64_t stack_id, int ret) {
                     if (ret < 0) {
                     tn->log(10,
                             SSTR("a sync operation returned error: " << ret));}
                     return ret;}
        ) ;
        if (progress) {
            *progress =
...
int RGWSyncGetBucketInfoCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        yield call(new
                   RGWGetBucketInstanceInfoCR(sync_env->async_rados,
        ...
        if (retcode == -ENOENT) {
            ...
            if (retcode < 0) {
                tn->log(0,
                        SSTR("ERROR: failed to fetch bucket instance info for "
                             << bucket_str {
                             bucket}
                        ));
                return set_cr_error(retcode);
        ...
        if (retcode < 0) {
            tn->log(0,
                    SSTR("ERROR: failed to retrieve bucket info for bucket=" <<
                         bucket_str {
                         bucket}
                    ));
            return set_cr_error(retcode);
...
void RGWGetBucketPeersCR::update_from_source_bucket_policy()
{
    ...
    filter_targets(sync_env->svc->zone->get_zone().id,
                   target_bucket, handler->get_targets(), pipes);
  ...
  public:
    ...
    :RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bucket(_bucket),
        ...
        reenter(this) {
            for (i = 0; i < 2; ++i) {
                yield call(new
                           RGWBucketGetSyncPolicyHandlerCR(sync_env->
                                                           async_rados,
                                                           sync_env->driver,
                                                           get_policy_params,
                                                           policy, dpp));
...
int RGWGetBucketPeersCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        if (target_bucket) {
            target_policy =
                make_shared < rgw_bucket_get_sync_policy_result > ();
            yield call(new
                       RGWSyncGetBucketSyncPolicyHandlerCR(sync_env, nullopt,
                                                           *target_bucket,
                                                           target_policy, tn));
            if (retcode < 0 && retcode != -ENOENT) {
                return set_cr_error(retcode);
        ...
        if (source_bucket && source_zone) {
            source_policy =
                make_shared < rgw_bucket_get_sync_policy_result > ();
            yield call(new
                       RGWSyncGetBucketSyncPolicyHandlerCR(sync_env,
                                                           source_zone,
                                                           *source_bucket,
                                                           source_policy, tn));
            if (retcode < 0 && retcode != -ENOENT) {
            ...
            if (source_policy->policy_handler) {
                ...
                auto & opt_attrs =
                    source_policy->policy_handler->get_bucket_attrs();
            ...
            if (!target_bucket) {
                ...
                for (hiter = get_hint_targets_action->targets.begin();
                    ...
                    ldpp_dout(dpp,
                              20) << "Got sync hint for bucket=" <<
                        *source_bucket << ": " << hiter->get_key() << dendl;
...
class RGWSyncBucketShardCR:public RGWCoroutine {
  ...
  public:
    RGWSyncBucketShardCR(RGWDataSyncCtx * _sc,
                         boost::intrusive_ptr < const RGWContinuousLeaseCR >
                         lease_cr, const rgw_bucket_sync_pair_info & _sync_pair,
                         rgw_bucket_sync_pipe & sync_pipe,
                         bool & bucket_stopped, uint64_t generation,
                         const RGWSyncTraceNodeRef & tn,
                         ceph::real_time * progress)
    :RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        ...
        shard_status_oid(RGWBucketPipeSyncStatusManager::
                         inc_status_oid(sc->source_zone, sync_pair,
                                        generation)),
        bucket_status_obj(sc->env->svc->zone->get_zone_params().log_pool,
                          RGWBucketPipeSyncStatusManager::full_status_oid(sc->
                                                                          source_zone,
                                                                          sync_pair.
                                                                          source_bs.
                                                                          bucket,
...
int RGWSyncBucketShardCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        objv_tracker.clear();
        yield call(new
                   RGWReadBucketPipeSyncStatusCoroutine(sc, sync_pair,
                                                        &sync_status,
                                                        &objv_tracker,
                                                        generation));
        if (retcode < 0 && retcode != -ENOENT) {
        ...
        tn->log(20,
                SSTR("sync status for source bucket shard: " << sync_status.
                     state));
        ...
        if (retcode < 0) {
            tn->log(5,
                    SSTR("incremental sync on bucket failed, retcode=" <<
                         retcode));
            return set_cr_error(retcode);
        }
    ...
    :RGWCoroutine(_sc->cct), sc(_sc), env(_sc->env),
        ...
        status_obj(env->svc->zone->get_zone_params().log_pool,
                   RGWBucketPipeSyncStatusManager::full_status_oid(sc->
                                                                   ...
                                                                   sync_pair.
                                                                   dest_bucket)),
        tn(env->sync_tracer->add_node(_tn_parent, "bucket", SSTR(bucket_str {
                                                                 _sync_pair.
                                                                 dest_bucket}
                                                                 <<"<-" <<
                                                                 bucket_shard_str
...
int RGWSyncBucketCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        yield call(new
                   RGWSyncGetBucketInfoCR(env, sync_pair.source_bs.bucket,
        ...
        yield call(new
                   RGWSyncGetBucketInfoCR(env, sync_pair.dest_bucket,
                                          &sync_pipe.dest_bucket_info,
        ...
        if (retcode == -ENOENT) {
            ...
            objv.generate_new_write_ver(cct);
            yield call(new
                       WriteCR(dpp, env->driver, status_obj, bucket_status,
                               &objv, true));
            tn->log(20,
                    "bucket status object does not exist, create a new one");
            if (retcode == -EEXIST) {
                // raced with another create, read its status
                tn->log(20, "raced with another create, read its status");
                objv.clear();
                yield call(new ReadCR(dpp, env->driver,
                                      status_obj, &bucket_status, false,
                                      &objv));
            }
        }
        if (retcode < 0) {
            tn->log(20,
                    SSTR("ERROR: failed to read bucket status object. error: "
                         << retcode));
            return set_cr_error(retcode);
        }
        ...
        do {
            tn->log(20,
                    SSTR("sync status for source bucket: " << bucket_status.
                         state << ". lease is: " << (bucket_lease_cr ? "taken" :
                                                     "not taken") <<
            ...
            if (bucket_status.state != BucketSyncState::Incremental ||
                ...
                if (!bucket_lease_cr) {
                    bucket_lease_cr.
                        reset(new
                              RGWContinuousLeaseCR(env->async_rados,
                                                   env->driver, status_obj,
                    ...
                    while (!bucket_lease_cr->is_locked()) {
                        if (bucket_lease_cr->is_done()) {
                            ...
                            set_status("lease lock failed, early abort");
                            drain_all();
                            return set_cr_error(bucket_lease_cr->
                                                get_ret_status());
                        }
                ...
                if (info.syncstopped) {
                    ...
                    if (bucket_status.state == BucketSyncState::Incremental) {
                        yield {
                            ...
                            call(new
                                 RemoveBucketShardStatusCollectCR(sc, sync_pair,
                                                                  gen,
                                                                  num_shards));
                        }
                    }
                    ...
                    if (retcode < 0) {
                        tn->log(20,
                                SSTR
                                ("ERROR: failed to read status before writing 'stopped'. error: "
                                 << retcode));
                        RELEASE_LOCK(bucket_lease_cr);
                        return set_cr_error(retcode);
                    }
                    if (bucket_status.state != BucketSyncState::Stopped) {
                        ...
                        yield call(new
                                   WriteCR(dpp, env->driver, status_obj,
                                           bucket_status, &objv, false));
                        if (retcode < 0) {
                            tn->log(20,
                                    SSTR
                ...
                if (bucket_stopped) {
                    tn->log(20,
                            SSTR
                            ("ERROR: switched from 'stop' to 'start' sync. while state is: "
                             << bucket_status.state));
            ...
            if (bucket_status.state != BucketSyncState::Incremental) {
                ...
                if (retcode < 0) {
                    ...
                    tn->log(20,
                            SSTR
                            ("ERROR: reading the status after acquiring the lock failed. error: "
                             << retcode));
                    return set_cr_error(retcode);
                }
                tn->log(20,
                        SSTR("status after acquiring the lock is: " <<
                             bucket_status.state));

                yield call(new
                           InitBucketFullSyncStatusCR(sc, sync_pair, status_obj,
                                                      bucket_status, objv,
                                                      sync_pipe.
                                                      source_bucket_info,
                ...
                if (retcode < 0) {
                    tn->log(20,
                            SSTR("ERROR: init full sync failed. error: " <<
                                 retcode));
                    RELEASE_LOCK(bucket_lease_cr);
                    return set_cr_error(retcode);
                }
            ...
            if (bucket_status.state == BucketSyncState::Full) {
                ...
                if (retcode < 0) {
                    tn->log(20,
                            SSTR("ERROR: full sync failed. error: " <<
                                 retcode));
                    RELEASE_LOCK(bucket_lease_cr);
                    return set_cr_error(retcode);
                }
            ...
            if (bucket_status.state == BucketSyncState::Incremental) {
                ...
                if (gen) {
                    ...
                    if (*gen > current_gen) {
                        ...
                        if ((static_cast < std::size_t > (source_bs.shard_id) <
                             bucket_status.shards_done_with_gen.size())
                            && !bucket_status.shards_done_with_gen[source_bs.
                                                                   shard_id]) {
                            // use the error repo and sync status timestamp from the datalog shard corresponding to source_bs
                            error_repo =
                                datalog_oid_for_error_repo(sc, sc->env->driver,
                                                           pool, source_bs);
                            yield call(rgw::error_repo::
                                       write_cr(sc->env->driver->svc()->rados,
                                                error_repo,
                                                rgw::error_repo::
                                                encode_key(source_bs,
                                                           current_gen),
                            ...
                            if (retcode < 0) {
                                tn->log(0,
                                        ...
                                        ("ERROR: failed to log prev gen entry (bucket="
                                         << source_bs.
                                         bucket << ", shard_id=" << source_bs.
                                         shard_id << ", gen=" << current_gen <<
                                         " in error repo: retcode=" <<
                                         retcode));
                            }
                            else {
                                tn->log(20,
                ...
                if (static_cast < std::size_t >
                    ...
                    bucket_status.shards_done_with_gen.size()) {
                    tn->log(1,
                            SSTR("bucket shard " << sync_pair.
                                 source_bs << " index out of bounds"));
                    return set_cr_done();   // return success so we don't retry
                }
                if (bucket_status.
                    shards_done_with_gen[sync_pair.source_bs.shard_id]) {
                    tn->log(10,
                            SSTR("bucket shard " << sync_pair.
                                 source_bs << " of gen " << gen <<
                                 " already synced."));
                    return set_cr_done();
                }

                yield call(new
                           RGWSyncBucketShardCR(sc, data_lease_cr, sync_pair,
                                                sync_pipe, bucket_stopped,
                                                bucket_status.incremental_gen,
                                                tn, progress));
                if (retcode < 0) {
                    tn->log(20,
                            SSTR("ERROR: incremental sync failed. error: " <<
                                 retcode));
                    return set_cr_error(retcode);
                }
            }
                                            ...
                                            std::ostream * ostr)
{
    ...
    if (ret < 0) {
        ldpp_dout(this,
                  0) << "failed to get bucket source peers info: (ret=" << ret
    ...
    if (pipes.empty()) {
        ldpp_dout(this,
                  0) << "No peers. This is not a valid multisite configuration."
            << dendl;
        return -EINVAL;
    }
  ...
  for (auto & pipe:pipes) {
        ...
        if (!conn) {
            ldpp_dout(this,
                      0) << "connection object to zone " << szone <<
                " does not exist" << dendl;
            return -EINVAL;
        }
        ...
        if (!(z = driver->svc()->zone->find_zone(szone))) {
            ldpp_dout(this,
                      0) << "zone " << szone << " does not exist" << dendl;
            return -EINVAL;
        }
...
tl::expected < std::unique_ptr < RGWBucketPipeSyncStatusManager >, int >
    RGWBucketPipeSyncStatusManager::construct(const DoutPrefixProvider * dpp,
                                              rgw::sal::RadosStore * driver,
...
int RGWBucketPipeSyncStatusManager::init_sync_status(const DoutPrefixProvider *
                                                     dpp)
{
  ...
  for (auto & source:sources) {
        ...
        RGWCoroutinesStack *stack =
            new RGWCoroutinesStack(driver->ctx(), &cr_mgr);
...
tl::expected < std::map < int, rgw_bucket_shard_sync_info >, int >
    RGWBucketPipeSyncStatusManager::read_sync_status(const DoutPrefixProvider *
                                                     dpp)
{
    ...
    for (auto shard = 0u; shard < num_shards; ++shard) {
        ...
        stack->
            call(new
                 RGWReadBucketPipeSyncStatusCoroutine(&sz->sc, pair,
                                                      &sync_status[shard],
                                                      nullptr, latest_gen));
    }
            ...
            reenter(this) {
                ...
                for (retries = 0u, retcode = -EDOM;
                    ...
                    if (retcode == -ECANCELED) {
                        ...
                        ldpp_dout(dpp,
                                  5) << "WARNING: Got error, retcode=" <<
                            retcode << " for " << pair.
                            source_bs << "on retry " << retries +
                            1 << " of " << allowed_retries << " allowed" <<
                            dendl;
                ...
                if (retcode < 0) {
                    ldpp_dout(dpp, -1) << "ERROR: Exhausted retries for "
                        << pair.source_bs << " retcode=" << retcode << dendl;
                    drain_all();
                    return set_cr_error(retcode);
                }
...
// Loop over calls to ShardCR with limited concurrency
    class GenCR:public RGWShardCollectCR {
      ...
      public:
        GenCR(RGWDataSyncCtx & sc, const rgw_bucket & source,
              const rgw_bucket & dest, const uint64_t gen,
        ...
        :RGWCoroutine(sc.cct), sc(sc), info(info), dest(dest), handler(handler),
            ...
            reenter(this) {
                ...
                if (status.state == BucketSyncState::Stopped) {
                    ...
                    ldpp_dout(dpp,
                              5) <<
                        "SourceCR: Bucket is in state Stopped, returning." <<
                        dendl;
                    drain_all();
                    return set_cr_done();
                }
                ...
                do {
                    ...
                    if (state != BucketSyncState::Incremental) {
                        ...
                        yield call(new
                                   GenCR(sc, info.bucket, dest, gen, 1,
                                         handler));
                    }
                    else {
                        pretty_print(sc.env,
                        ...
                        ldpp_dout(dpp,
                            ...
                            dendl;
                        yield call(new
                                   GenCR(sc, info.bucket, dest, gen, num_shards,
                                         handler));
                    }
                    if (retcode < 0) {
                    ...
                    yield call(new RGWSimpleRadosReadCR <
                               rgw_bucket_sync_status > (dpp, sc.env->driver,
                                                         status_obj, &status));
                    if (retcode < 0) {
                        ldpp_dout(dpp,
                                  -1) <<
                            "ERROR: Unable to fetch status for zone=" << sc.
                            source_zone << " retcode=" << retcode << dendl;
                        drain_all();
                        return set_cr_error(retcode);
                    }
                    ...
                    ldpp_dout(dpp, 5) << "SourceCR: "
                        ...
                        << ", status.shards_done_with_gen.size()=" << status.
                        shards_done_with_gen.size()
                        << dendl;
...
int RGWBucketPipeSyncStatusManager::run(const DoutPrefixProvider * dpp)
{
  ...
  for (auto & source:sources) {
        auto stack = new RGWCoroutinesStack(driver->ctx(), &cr_mgr);
        stack->
            call(new rgw::bucket_sync_run::
                 SourceCR(source.sc, source.info, source.dest, source.handler,
                          source.zone_name));
        stacks.push_back(stack);
    }
...
unsigned RGWBucketPipeSyncStatusManager::get_subsys() const const
{
    return dout_subsys;
}

std::ostream & RGWBucketPipeSyncStatusManager::gen_prefix(std::ostream & out) const const
{
...
string RGWBucketPipeSyncStatusManager::
full_status_oid(const rgw_zone_id & source_zone,
...
string RGWBucketPipeSyncStatusManager::
inc_status_oid(const rgw_zone_id & source_zone,
...
string RGWBucketPipeSyncStatusManager::
obj_status_oid(const rgw_bucket_sync_pipe & sync_pipe,
               const rgw_zone_id & source_zone, const rgw_obj & obj)
{
    string prefix =
        object_status_oid_prefix + "." + source_zone.id + ":" +
        obj.bucket.get_key();
                               ...
                               optional_yield y)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  -1) << "failed to fetch remote log markers: " <<
            cpp_strerror(r) << dendl;
        return r;
    }
    // parse shard markers
...
class RGWCollectBucketSyncStatusCR:public RGWShardCollectCR {
  ...
  public:
    RGWCollectBucketSyncStatusCR(rgw::sal::RadosStore * driver,
                                 RGWDataSyncCtx * sc,
                                 const rgw_bucket_sync_pair_info & sync_pair,
  ...
  :    RGWShardCollectCR(sc->cct, max_concurrent_shards), driver(driver),
    ...
    bool spawn_next() override {
        ...
        spawn(new
              RGWReadBucketPipeSyncStatusCoroutine(sc, sync_pair, &*i, nullptr,
                                                   gen), false);
        ++i;
        ++sync_pair.source_bs.shard_id;
        return true;
                                     ...
                                     optional_yield y)
{
    ...
    try {
        ...
        *status = result;
        ...
        lderr(svc->ctx()) << "error decoding " << obj << ": " << err.
            what() << dendl;
        return -EIO;
    }
}
...
int rgw_read_bucket_inc_sync_status(const DoutPrefixProvider * dpp,
                                    ...
                                    const rgw_sync_bucket_pipe & pipe,
                                    uint64_t gen,
                                    std::vector < rgw_bucket_shard_sync_info >
                                    *status)
{
    if (!pipe.source.zone ||
        !pipe.source.bucket || !pipe.dest.zone || !pipe.dest.bucket) {
        return -EINVAL;
    }
    ...
    env.init(dpp, driver->ctx(), driver, driver->svc(),
             driver->svc()->rados->get_async_processor(), nullptr, nullptr,
    ...
    RGWCoroutinesManager crs(driver->ctx(),
                             driver->getRados()->get_cr_registry());
    return crs.run(dpp,
                   new RGWCollectBucketSyncStatusCR(driver, &sc, sync_pair, gen,
                                                    status));
}

void rgw_data_sync_info::generate_test_instances(list <
                                                 rgw_data_sync_info * >&o)
{
...
void rgw_data_sync_marker::generate_test_instances(list <
                                                   rgw_data_sync_marker * >&o)
{
...
void rgw_data_sync_status::generate_test_instances(list <
                                                   rgw_data_sync_status * >&o)
{
    o.push_back(new rgw_data_sync_status);
}

void rgw_bucket_shard_full_sync_marker::dump(Formatter * f) const const
{
...
void rgw_bucket_shard_inc_sync_marker::dump(Formatter * f) const const
{
    encode_json("position", position, f);
...
void rgw_bucket_shard_sync_info::dump(Formatter * f) const const
{
    const char *s {
    nullptr};
...
void rgw_bucket_full_sync_status::dump(Formatter * f) const const
{
    encode_json("position", position, f);
    encode_json("count", count, f);
}
...
void rgw_bucket_sync_status::dump(Formatter * f) const const
{
...
void bilog_status_v2::dump(Formatter * f) const const
{
