...
void ImageCacheState < I >::init_from_config()
{
    ...
    mode =
        config.get_val < std::string >
        ("rbd_persistent_cache_mode");
    size = 0;
} template < typename I >
bool ImageCacheState <
I >::init_from_metadata(json_spirit::mValue &json_root)
{
    ldout(m_image_ctx->cct, 20) << dendl;
    ...
    try {
        ...
        lderr(m_image_ctx->
              cct) << "failed to parse cache state: " << e.what()
                   << dendl;
        return false;
    }
    return true;
                             ...
                             &locker, Context *on_finish)
{
    ...
    m_plugin_api.execute_image_metadata_set(m_image_ctx,
...
template < typename I >
void ImageCacheState <
I >::clear_image_cache_state(Context *on_finish)
{
    std::shared_lock owner_lock {
        m_image_ctx->owner_lock};
    ldout(m_image_ctx->cct,
          20) << __func__ << " Remove state: " << dendl;
    m_plugin_api.execute_image_metadata_remove(m_image_ctx,
                PERSISTENT_CACHE_STATE,
                on_finish);
}
...
I >::create_image_cache_state(I *image_ctx,
                              ...
                              int &r)
{
    ...
    r = 0;
    bool dirty_cache =
        plugin_api.test_image_features(image_ctx,
                                       RBD_FEATURE_DIRTY_CACHE);
    if (dirty_cache) {
        cls_client::metadata_get(&image_ctx->md_ctx,
    ...
    ldout(image_ctx->cct,
          20) << "image_cache_state: " << cache_state_str << dendl;
    ...
    if (!dirty_cache && !cache_desired) {
        ...
        lderr(image_ctx->
              cct) <<
                   ...
                   dendl;
        r = -EINVAL;
    } else if ((!dirty_cache || cache_state_str.empty())
               && cache_desired) {
        cache_state =
            new ImageCacheState < I > (image_ctx, plugin_api);
        ...
        if (!json_spirit::read(cache_state_str.c_str(), json_root)) {
            lderr(image_ctx->
                  cct) << "failed to parse cache state" << dendl;
            r = -EINVAL;
            return nullptr;
        }
        cache_state =
...
template < typename I >
ImageCacheState < I > *ImageCacheState <
I >::get_image_cache_state(I *image_ctx,
                           plugin::Api < I > &plugin_api)
{
    ImageCacheState < I > *cache_state = nullptr;
...
template < typename I > bool ImageCacheState < I >::is_valid()
{
    if (this->present &&
        ...
        lderr(m_image_ctx->
              cct) <<
                   "An image cache (RWL) remains on another host " << host
