...
Mgr::Mgr(MonClient * monc_, const MgrMap & mgrmap,
         ...
         Client * client_, LogChannelRef clog_,
         LogChannelRef audit_clog_):monc(monc_), objecter(objecter_),
client(client_), client_messenger(clientm_), finisher(g_ceph_context, "Mgr",
                                                      "mgr-fin"),
digest_received(false), py_module_registry(py_module_registry_),
cluster_state(monc, nullptr, mgrmap), server(monc, finisher, daemon_state,
                                             cluster_state, *py_module_registry,
                                             clog_, audit_clog_), clog(clog_),
audit_clog(audit_clog_), initialized(false), initializing(false)
{
    ...
    if (r == 0) {
        if (key.type == "mds" || key.type == "osd" ||
            ...
            if (daemon_meta.count("hostname") == 0) {
                dout(1) << "Skipping incomplete metadata entry for " << key <<
                    dendl;
                return;
            }
...
void Mgr::init()
{
    ...
    if (!mon_allows_kv_sub) {
        ...
        lock.unlock();
        dout(10) <<
            "waiting a bit for the pre-pacific mon to process our beacon" <<
            dendl;
        sleep(g_conf().get_val < std::chrono::seconds >
              ("mgr_tick_period").count() * 3);
        lock.lock();
    }
    ...
    fs_map_cond.wait(l,[this] {
                     return cluster_state.have_fsmap();
                     }
    );

    // Wait for MgrDigest...
    dout(4) << "waiting for MgrDigest..." << dendl;
    digest_cond.wait(l,[this] {
                     return digest_received;
                     }
    );
    ...
    for (auto p = pre_init_store.lower_bound("device/");
        ...
        if (r < 0) {
            derr << __func__ << " failed to parse " << p->second << ": " << ss.
                str()
                << dendl;
        }
        else {
...
void Mgr::load_all_metadata()
{
  ...
  for (auto & metadata_val:mds_cmd.json_result.get_array()) {
        ...
        DaemonStatePtr dm =
            std::make_shared < DaemonState > (daemon_state.types);
        dm->key = DaemonKey {
        "mds", daemon_meta.at("name").get_str()};
  ...
  for (auto & metadata_val:mon_cmd.json_result.get_array()) {
        ...
        DaemonStatePtr dm =
            std::make_shared < DaemonState > (daemon_state.types);
        dm->key = DaemonKey {
        "mon", daemon_meta.at("name").get_str()};
        dm->hostname = daemon_meta.at("hostname").get_str();
  ...
  for (auto & osd_metadata_val:osd_cmd.json_result.get_array()) {
        ...
        DaemonStatePtr dm =
            std::make_shared < DaemonState > (daemon_state.types);
        dm->key = DaemonKey {
        "osd", stringify(osd_metadata.at("id").get_int())};
...
void Mgr::handle_osd_map()
{
   ...
   */
    cluster_state.
        with_osdmap_and_pgmap([this, &names_exist] (const OSDMap & osd_map,
                                                    const PGMap & pg_map) {
                              for (int osd_id = 0;
                                   osd_id < osd_map.get_max_osd(); ++osd_id) {
                              ...
                              const auto k = DaemonKey {
                              "osd", std::to_string(osd_id)};
                              if (daemon_state.is_updating(k)) {
                              continue;}

                              bool update_meta = false;
                              if (daemon_state.exists(k)) {
                              if (osd_map.get_up_from(osd_id) ==
                                  osd_map.get_epoch()) {
                              dout(4) << "Mgr::handle_osd_map: osd." << osd_id
                              << " joined cluster at " << "e" << osd_map.
                              get_epoch()
                              << dendl; update_meta = true;}
                              }
                              else {
                              update_meta = true;}
                              if (update_meta) {
    ...
    case MSG_KV_DATA:
        {
            ...
            if (!msg->data.empty()) {
                if (initialized) {
                    py_module_registry->update_kv_data(msg->prefix,
                                                       msg->incremental,
                                                       msg->data);
                }
                else {
                    ...
                    if (!msg->incremental) {
                        ...
                        while (p != pre_init_store.end()
                               && p->first.find(msg->prefix) == 0) {
                            dout(20) << " rm prior " << p->first << dendl;
                            p = pre_init_store.erase(p);
                        }
                    }
                    else {
                        dout(10) << "incremental update on " << msg->
                            prefix << dendl;
                    }
                  for (auto & i:msg->data) {
                        if (i.second) {
                            dout(20) << " set " << i.first << " = " << i.
                                second->to_str() << dendl;
  ...
  for (const auto & i:mds_info) {
        ...
        if (daemon_state.exists(k)) {
            ...
            if (metadata->metadata.empty() ||
                ...
                if (update) {
                    dout(4) << "MDS[" << info.
                        name << "] addr change " << metadata_addrs << " != " <<
...
std::map < std::string, std::string > Mgr::get_services() constconst
{
    std::lock_guard l(lock);
...
int Mgr::call(std::string_view admin_command,
              const cmdmap_t & cmdmap,
              const bufferlist &,
              Formatter * f, std::ostream & errss, bufferlist & out)
{
    try {
        if (admin_command == "mgr_status") {
            f->open_object_section("mgr_status");
            cluster_state.with_mgrmap([f] (const MgrMap & mm) {
                                      f->dump_unsigned("mgrmap_epoch",
                                                       mm.get_epoch());});
