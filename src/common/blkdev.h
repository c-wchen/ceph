...
extern int get_device_by_path(const char *path, char *partition, char *device,
                              size_t max);
...
extern void get_device_metadata(const std::set < std::string > &devnames,
                                std::map < std::string, std::string > *pm,
                                std::map < std::string, std::string > *errs);

extern void get_dm_parents(const std::string &dev,
...
public:
    ...
    // from an fd int discard(int64_t offset, int64_t len) const;
