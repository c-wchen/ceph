...
int queue_write_head(cls_method_context_t hctx, cls_queue_head & head)
{
    ...
    int ret =
        cls_cxx_write2(hctx, 0, bl.length(), &bl,
...
int queue_read_head(cls_method_context_t hctx, cls_queue_head & head)
{
    ...
    if (encoded_len > (chunk_size - QUEUE_ENTRY_OVERHEAD)) {
        ...
        const auto ret =
            cls_cxx_read2(hctx, start_offset, chunk_size, &bl_remaining_head,
                          CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL);
        if (ret < 0) {
            CLS_LOG(5,
                    "ERROR: queue_read_head: failed to read remaining part of head");
            return ret;
        }
...
int queue_init(cls_method_context_t hctx, const cls_queue_init_op & op)
{
    ...
    CLS_LOG(20, "INFO: init_queue_op queue front offset %s",
            head.front.to_str().c_str());
    CLS_LOG(20, "INFO: init_queue_op queue max urgent data size %lu",
                  ...
                  cls_queue_head & head)
{
  ...
  for (auto & bl_data:op.bl_data_vec) {
        ...
        CLS_LOG(10,
                "INFO: queue_enqueue(): Total size to be written is %u and data size is %lu",
                bl.length(), data_size);
        ...
        if (head.tail.offset >= head.front.offset) {
            ...
            if ((head.tail.offset + bl.length()) <= head.queue_size) {
                ...
                auto ret =
                    cls_cxx_write2(hctx, head.tail.offset, bl.length(), &bl,
                                   CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL);
                if (ret < 0) {
                    return ret;
                }
                ...
                if (bl.length() <= free_space_available) {
                    ...
                    auto ret =
                        cls_cxx_write2(hctx, head.tail.offset,
                                       bl_data_before_wrap.length(),
                                       &bl_data_before_wrap,
                                       CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL);
                    if (ret < 0) {
                       ...
                       cls_queue_list_ret & op_ret, cls_queue_head & head)
{
    ...
    do {
        ...
        if (size_to_read == 0) {
            ...
            op_ret.is_truncated = false;
            CLS_LOG(20,
                    "INFO: queue_list_entries(): size_to_read is 0, hence breaking out!\n");
            break;
        }
        ...
        do {
            CLS_LOG(10,
                    "INFO: queue_list_entries(): index: %u, size_to_process: %lu",
            ...
            if (!entry_start_processed) {
                if (size_to_process >= QUEUE_ENTRY_OVERHEAD) {
                    ...
                    if (entry_start != QUEUE_ENTRY_START) {
                        CLS_LOG(5,
                                "ERROR: queue_list_entries: invalid entry start %u",
                                entry_start);
                        return -EINVAL;
                    }
            ...
            if (data_size <= size_to_process) {
                ...
                last_marker = entry.marker;
                CLS_LOG(10,
                        "INFO: queue_list_entries(): not enough data to read data, breaking out!");
                break;
            }
                         ...
                         const cls_queue_remove_op & op, cls_queue_head & head)
{
    ...
    if (end_marker.offset > head.front.offset
        ...
        if (len > 0) {
            ...
            if (ret < 0) {
                CLS_LOG(5,
                        "INFO: queue_remove_entries: Failed to zero out entries");
