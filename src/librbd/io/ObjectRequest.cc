    ...
    namespace io {
        ...
        using librbd::util::create_trace;
         ...
         namespace {
            ...
            template < typename I >
                inline bool is_copy_on_read(I * ictx,
                                            const IOContext & io_context) {
                std::shared_lock image_locker {
                ictx->image_lock};
                 return (ictx->clone_copy_on_read && !ictx->read_only &&
                         io_context->read_snap().value_or(CEPH_NOSNAP) ==
                         CEPH_NOSNAP && (ictx->exclusive_lock == nullptr
                                         || ictx->exclusive_lock->
                                         is_lock_owner()));
            } template < typename S,
                typename D > void convert_snap_set(const S & src_snap_set,
                                                   D * dst_snap_set) {
        ...
        template < typename I >
            ObjectRequest < I > *ObjectRequest < I >::create_write(I * ictx,
                                                                   uint64_t
                                                                   object_no,
                                                                   uint64_t
                                                                   object_off,
            ...
            return new ObjectWriteRequest < I > (ictx, object_no, object_off,
                                                 ...
                                                 op_flags, write_flags,
                                                 assert_version, parent_trace,
                                                 completion);
        }

        template < typename I >
            ObjectRequest < I > *ObjectRequest < I >::create_discard(I * ictx,
                                                                     uint64_t
                                                                     object_no,
                                                                     uint64_t
                                                                     object_off,
        ...
        template < typename I >
            ...
            I >::create_write_same(I * ictx, uint64_t object_no,
                                   uint64_t object_off, uint64_t object_len,
                                   ceph::bufferlist
                                   && data, IOContext io_context, int op_flags,
                                   const ZTracer::Trace & parent_trace,
                                   Context * completion) {
            return new ObjectWriteSameRequest < I > (ictx, object_no,
        ...
        template < typename I >
            ObjectRequest < I > *ObjectRequest <
            I >::create_compare_and_write(I * ictx, uint64_t object_no,
                                          uint64_t object_off, ceph::bufferlist
                                          && cmp_data, ceph::bufferlist
                                          && write_data, IOContext io_context,
                                          uint64_t * mismatch_offset,
                                          int op_flags,
                                          const ZTracer::Trace & parent_trace,
                                          Context * completion) {
            return new ObjectCompareAndWriteRequest < I > (ictx, object_no,
                                                           object_off,
                                                           std::move(cmp_data),
                                                           std::
                                                           move(write_data),
      ...
      :    m_ictx(ictx), m_object_no(objectno), m_io_context(io_context),
        ...
        template < typename I >
            bool ObjectRequest <
            I >::compute_parent_extents(Extents * parent_extents,
                                        ImageArea * area, bool read_request) {
            ...
            *area = ImageArea::DATA;
            ...
            int r =
                m_ictx->get_parent_overlap(m_io_context->read_snap().
                                           value_or(CEPH_NOSNAP), &raw_overlap);
            if (r < 0) {
            ...
            bool migration_write = !read_request
                && !m_ictx->migration_info.empty();
            if (migration_write) {
            ...
            std::tie(*parent_extents, *area) =
                io::util::object_to_area_extents(m_ictx, m_object_no, { {
                                                 0, m_ictx->layout.object_size}
                                                 }
            );
            uint64_t object_overlap =
                m_ictx->prune_parent_extents(*parent_extents, *area,
                                             raw_overlap, migration_write);
            if (object_overlap > 0) {
                m_has_parent = true;
                return true;
            }
        ...
        template < typename I > void ObjectRequest < I >::async_finish(int r) {
            ...
            m_ictx->asio_engine->post([this, r] () {
                                      finish(r);
                                      });
        }
...
/** read **/
        ...
        template < typename I >
            ObjectReadRequest < I >::ObjectReadRequest(I * ictx,
                                                       uint64_t objectno,
                                                       ReadExtents * extents,
                                                       IOContext io_context,
                                                       int op_flags,
        ...
        :ObjectRequest < I > (ictx, objectno, io_context, "read", parent_trace,
                              completion), m_extents(extents),
            m_op_flags(op_flags), m_read_flags(read_flags), m_version(version) {
        }
        ...
        template < typename I > void ObjectReadRequest < I >::read_object() {
            ...
            if (read_snap_id == image_ctx->snap_id
                ...
                && !image_ctx->object_map->object_may_exist(this->
                                                            m_object_no)) {
          ...
          for (auto & extent:*this->m_extents) {
                if (extent.length >= image_ctx->sparse_read_threshold_bytes) {
                    read_op.sparse_read(extent.offset, extent.length,
                                        &extent.bl, &extent.extent_map);
                }
                else {
                    read_op.read(extent.offset, extent.length, &extent.bl);
                }
            }
            util::apply_op_flags(m_op_flags,
                                 image_ctx->get_read_flags(read_snap_id),
                                 &read_op);
            ...
            image_ctx->rados_api.execute( {
                                         ...
                                         librbd::asio::util::
                                         get_callback_adapter([this] (int r) {
                                                              handle_read_object
                                                              (r);
                                                              }), m_version,
                                         (this->m_trace.valid()? this->m_trace.
                                          get_info() : nullptr));
        }
        ...
        template < typename I > void ObjectReadRequest < I >::read_parent() {
            ...
            io::util::read_parent < I > (image_ctx, this->m_object_no,
                                         this->m_extents,
                                         this->m_io_context->read_snap().
                                         value_or(CEPH_NOSNAP), this->m_trace,
                                         ctx);
        }
        ...
        template < typename I > void ObjectReadRequest < I >::copyup() {
            ...
            if (it == image_ctx->copyup_list.end()) {
                ...
                auto new_req =
                    CopyupRequest < I >::create(image_ctx, this->m_object_no,
                                                std::move(parent_extents), area,
                                                this->m_trace);
...
/** write **/
        ...
        template < typename I >
            ...
            I >::AbstractObjectWriteRequest(I * ictx, uint64_t object_no,
                                            uint64_t object_off, uint64_t len,
                                            IOContext io_context,
                                            const char *trace_name,
                                            const ZTracer::Trace & parent_trace,
                                            Context * completion)
        :ObjectRequest < I > (ictx, object_no, io_context, trace_name,
                              parent_trace, completion),
        ...
        template < typename I >
            ...
            this->compute_parent_extents(&m_parent_extents, &m_image_area,
                                         false);
        ...
        template < typename I >
            ...
            I >::add_write_hint(neorados::WriteOp * wr) {
            I *image_ctx = this->m_ictx;
            std::shared_lock image_locker {
            image_ctx->image_lock};
        ...
        template < typename I > void AbstractObjectWriteRequest < I >::send() {
            ...
            if (!m_object_may_exist && is_no_op_for_nonexistent_object()) {
                ldout(image_ctx->cct,
                      20) << "skipping no-op on nonexistent object" << dendl;
                this->async_finish(0);
                return;
            }
        ...
        template < typename I >
            void AbstractObjectWriteRequest <
            I >::pre_write_object_map_update() {
            I *image_ctx = this->m_ictx;

            image_ctx->image_lock.lock_shared();
            if (image_ctx->object_map == nullptr
                || !is_object_map_update_enabled()) {
                image_ctx->image_lock.unlock_shared();
                write_object();
                return;
            }
            ...
            ldout(image_ctx->cct,
                  20) << this->m_object_off << "~" << this->
                m_object_len << dendl;

            if (image_ctx->object_map->template aio_update <
                AbstractObjectWriteRequest < I >,
                &AbstractObjectWriteRequest <
                I >::handle_pre_write_object_map_update > (CEPH_NOSNAP,
                                                           this->m_object_no,
                                                           new_state, {
                                                           }
                                                           , this->m_trace,
        ...
        template < typename I >
            void AbstractObjectWriteRequest <
            I >::handle_pre_write_object_map_update(int r) {
            I *image_ctx = this->m_ictx;
            ldout(image_ctx->cct, 20) << "r=" << r << dendl;
            if (r < 0) {
        ...
        template < typename I >
            ...
            if (m_copyup_enabled) {
                if (m_guarding_migration_write) {
                    ...
                    cls_client::assert_snapc_seq(&write_op, snap_seq,
                                                 cls::rbd::
                                                 ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ);
                }
                else {
        ...
        template < typename I >
            ...
            if (r == -ENOENT) {
                ...
                lderr(image_ctx->
                      cct) << "failed to write object: " << cpp_strerror(r)
                    << dendl;
                this->finish(r);
                return;
            }
        ...
        template < typename I > void AbstractObjectWriteRequest < I >::copyup() {
            ...
            if (it == image_ctx->copyup_list.end()) {
                ...
                new_req->append_request(this,
                                        std::
                                        move(get_copyup_overwrite_extents()));
                image_ctx->copyup_list[this->m_object_no] = new_req;
        ...
        template < typename I >
            ...
            if (r < 0 && r != -ERESTART) {
                lderr(image_ctx->
                      cct) << "failed to copyup object: " << cpp_strerror(r)
                    << dendl;
                this->finish(r);
                return;
            }
        ...
        template < typename I >
            void AbstractObjectWriteRequest <
            I >::post_write_object_map_update() {
            I *image_ctx = this->m_ictx;
            ...
            if (image_ctx->object_map->template aio_update <
                AbstractObjectWriteRequest < I >,
                &AbstractObjectWriteRequest <
                I >::handle_post_write_object_map_update > (CEPH_NOSNAP,
                                                            this->m_object_no,
                                                            OBJECT_NONEXISTENT,
                                                            OBJECT_PENDING,
                                                            this->m_trace,
        ...
        template < typename I >
            void AbstractObjectWriteRequest <
            I >::handle_post_write_object_map_update(int r) {
            I *image_ctx = this->m_ictx;
            ldout(image_ctx->cct, 20) << "r=" << r << dendl;
            if (r < 0) {
                lderr(image_ctx->cct) << "failed to update object map: "
        ...
        template < typename I >
            void ObjectWriteRequest <
            I >::add_write_ops(neorados::WriteOp * wr) {
            if (this->m_full_object) {
                wr->write_full(bufferlist {
                               m_write_data}
                );
            }
            else {
                wr->write(this->m_object_off, bufferlist {
                          m_write_data}
                );
            }
        ...
        template < typename I >
            void ObjectWriteSameRequest <
            I >::add_write_ops(neorados::WriteOp * wr) {
            wr->writesame(this->m_object_off, this->m_object_len, bufferlist {
                          m_write_data});
            util::apply_op_flags(m_op_flags, 0U, wr);
        }
        ...
        template < typename I >
            ...
            I >::add_write_ops(neorados::WriteOp * wr) {
            wr->cmpext(this->m_object_off, bufferlist {
                       m_cmp_bl}, nullptr);
        ...
        template < typename I >
            int ObjectCompareAndWriteRequest <
            I >::filter_write_result(int r) const {
            if (r <= -MAX_ERRNO) {
                I *image_ctx = this->m_ictx;
        ...
        template < typename I >
            ObjectListSnapsRequest < I >::ObjectListSnapsRequest(I * ictx,
                                                                 uint64_t
                                                                 objectno,
                                                                 Extents
                                                                 &&
                                                                 ...
                                                                 Context *
                                                                 completion)
      :    ObjectRequest < I > (ictx, objectno,
                                 ictx->duplicate_data_io_context(), "snap_list",
                                 parent_trace, completion),
        ...
        template < typename I > void ObjectListSnapsRequest < I >::send() {
            ...
            if (m_snap_ids.size() < 2) {
                lderr(image_ctx->
                      cct) << "invalid snap ids: " << m_snap_ids << dendl;
        ...
        template < typename I >
            ...
            if (r == -ENOENT) {
                ...
                lderr(cct) << "failed to retrieve object snapshot list: " <<
                    cpp_strerror(r)
                    << dendl;
                this->finish(r);
                return;
          ...
          for (auto & object_extent:m_object_extents) {
                object_interval.insert(object_extent.first,
          ...
          for (auto end_snap_id:m_snap_ids) {
                ...
                if (read_whole_object ||
                    (!diff.empty() &&
                     ((m_list_snaps_flags & LIST_SNAPS_FLAG_WHOLE_OBJECT) !=
                      0))) {
                ...
                if (end_size < prev_end_size) {
                    ...
                    trunc_interval.intersection_of(zero_interval,
                                                   diff_interval);
                    if (!trunc_interval.empty()) {
                        diff_interval.subtract(trunc_interval);
                        ldout(cct,
                              20) << "clearing truncate diff: " <<
                            trunc_interval << dendl;
                    }
                }
                ...
                if (exists) {
                  for (auto & interval:diff_interval) {
                        snapshot_delta[ {
                                       end_snap_id, clone_end_snap_id}
                        ].insert(interval.first, interval.second,
                                 SparseExtent(SPARSE_EXTENT_STATE_DATA,
                                              interval.second));
                    }
                }
                else {
                ...
                if ((m_list_snaps_flags & LIST_SNAPS_FLAG_IGNORE_ZEROED_EXTENTS)
                    == 0) {
                  for (auto & interval:zero_interval) {
                        snapshot_delta[ {
                                       end_snap_id, end_snap_id}
                        ].insert(interval.first, interval.second,
                                 SparseExtent(SPARSE_EXTENT_STATE_ZEROED,
                                              interval.second));
                    }
                }
            }
        ...
        template < typename I >
            ...
            if ((snap_id_start > 0) || (image_ctx->parent == nullptr) ||
                ((m_list_snaps_flags & LIST_SNAPS_FLAG_DISABLE_LIST_FROM_PARENT)
                 != 0)) {
                image_locker.unlock();
            ...
            if (raw_overlap > 0) {
                ...
                std::tie(parent_extents, m_image_area) =
                    io::util::object_to_area_extents(image_ctx,
                                                     ...
                                                     m_object_extents);
                object_overlap =
                    image_ctx->prune_parent_extents(parent_extents,
                                                    m_image_area, raw_overlap,
                                                    false);
            }
            if (object_overlap == 0) {
                image_locker.unlock();
            ...
            auto aio_comp =
                AioCompletion::create_and_start(ctx,
                                                librbd::util::
                                                get_image_ctx(image_ctx->
                                                              parent),
                                                AIO_TYPE_GENERIC);
            ldout(cct,
                  20) << "completion=" << aio_comp << " parent_extents=" <<
                parent_extents << " area=" << m_image_area << dendl;

            auto list_snaps_flags =
                (m_list_snaps_flags | LIST_SNAPS_FLAG_IGNORE_ZEROED_EXTENTS);

            ImageListSnapsRequest < I > req(*image_ctx->parent, aio_comp,
        ...
        template < typename I >
          ...
          for (auto &[key, image_extents]:m_parent_snapshot_delta) {
              for (auto image_extent:image_extents) {
                    ...
                    io::util::area_to_object_extents(image_ctx,
                                                     image_extent.get_off(),
                                                     image_extent.get_len(),
                                                     m_image_area, 0,
                                                     &object_extents);
                  for (auto & object_extent:object_extents) {
                        ceph_assert(object_extent.object_no ==
                                    this->m_object_no);
                        intervals.insert(object_extent.offset,
        ...
        template < typename I >
            void ObjectListSnapsRequest < I >::zero_extent(uint64_t snap_id,
                                                           bool dne) {
            I *image_ctx = this->m_ictx;
            auto cct = image_ctx->cct;
            ...
            if ((m_list_snaps_flags & LIST_SNAPS_FLAG_IGNORE_ZEROED_EXTENTS) ==
              ...
              for (auto[offset, length]:interval) {
                    ...
                    (*m_snapshot_delta)[ {
                             ...
                             SparseExtent((dne ? SPARSE_EXTENT_STATE_DNE :
