...
public:
    uint64_t end;
    C_ReProbe(Journaler *l, C_OnFinisher *onfinish_) : ls(l),
                                                       onfinish(onfinish_), end(0)
    {
    }
    void finish(int r) override
    {
...
void Journaler::_read_head(Context *on_finish, bufferlist *bl)
{
    ...
    objecter->read_full(oid, oloc, CEPH_NOSNAP, bl, 0,
                        wrap_finisher(on_finish));
}
...
void Journaler::_finish_read_head(int r, bufferlist &bl)
{
    ...
    ldout(cct, 1) << "_finish_read_head " << h
                  << ".  probing for end of log (from " << write_pos << ")..." << dendl;
...
void Journaler::_finish_probe_end(int r, uint64_t end)
{
    ...
    if (((int64_t)end) == -1)
    {
        ...
        ldout(cct,
              1)
            << "_finish_probe_end write_pos = " << end << " (header had "
    ...
    else
    {
        ...
        ldout(cct, 1) << "_finish_probe_end write_pos = " << end
                      << " (header had " << write_pos << "). recovered." << dendl;
    }
...
public:
    C_RereadHeadProbe(Journaler *l, C_OnFinisher *finish) : ls(l),
                                                            final_finish(finish)
    {
    }
    void finish(int r) override
...
uint64_t Journaler::append_entry(bufferlist &bl)
{
    ...
    if (write_obj != flush_obj)
    {
        ...
        if (write_buf.length() > 0 && write_buf.length() <= wrote)
...
void Journaler::_do_flush(unsigned amount)
{
    ...
    if (flush_pos + len + 2 * period > prezero_pos)
    {
        ...
        if (newlen <= 0)
        {
            ldout(cct,
                  10)
                << "_do_flush wanted to do " << flush_pos << "~" << len << " already too close to prezero_pos " << prezero_pos << ", zeroing first" << dendl;
        ...
        if (static_cast<uint64_t>(newlen) < len)
        {
            ldout(cct,
                  10)
                << "_do_flush wanted to do " << flush_pos << "~" << len << " but hit prezero_pos " << prezero_pos << ", will do " << flush_pos << "~" << newlen << dendl;
            waiting_for_zero_pos = flush_pos + len;
            len = newlen;
        }
    }
    ...
    filer.write(ino, &layout, snapc,
                ...
                0, wrap_finisher(onsafe), write_iohint);
...
{
    return last_wrote_head + seconds(cct->_conf->journaler_write_head_interval) < ceph::real_clock::now();
}
...
void Journaler::_issue_prezero()
{
    ...
    if (prezeroing_pos >= to)
    {
        ldout(cct,
              20)
            << "_issue_prezero target " << to << " <= prezeroing_pos " << prezeroing_pos << dendl;
        return;
    }
                             ...
                             bufferlist &bl)
{
    ...
    else
    {
        ...
        if (bl.length() < length)
        {
            ldout(cct,
                  0)
                << "_finish_read got less than expected (" << length << ")"
                << dendl;
            error = -EINVAL;
        }
    }
...
void Journaler::_assimilate_prefetch()
{
    ...
    if (got_any)
    {
        ldout(cct,
              10)
            << "_assimilate_prefetch read_buf now " << read_pos << "~" << read_buf.length() << ", read pointers " << read_pos << "/" << received_pos << "/" << requested_pos << dendl;
    ...
    if ((got_any && !was_readable && readable) || read_pos == write_pos)
    {
        ...
        ldout(cct,
              10)
            << "_finish_read now readable (or at journal end) readable="
            << readable << " read_pos=" << read_pos << " write_pos=" << write_pos << dendl;
        if (on_readable)
        {
            C_OnFinisher *f = on_readable;
            on_readable = 0;
...
void Journaler::_prefetch()
{
    ...
    if (temp_fetch_len)
    {
        ldout(cct,
              10)
            << "_prefetch temp_fetch_len " << temp_fetch_len << dendl;
        pf = temp_fetch_len;
        temp_fetch_len = 0;
    }
    else
    ...
    if (requested_pos < target)
    {
        ...
        ldout(cct,
              10)
            << "_prefetch " << pf << " requested_pos " << requested_pos << " < target " << target << " (" << raw_target << "), prefetching " << len << dendl;
...
bool Journaler::_is_readable()
{
    ...
    if (need > fetch_len)
    {
        ...
        ldout(cct,
              10)
            << "_is_readable noting temp_fetch_len " << temp_fetch_len << dendl;
    }
...
void Journaler::erase(Context *completion)
{
    ...
    filer.purge_range(ino, &layout, SnapContext(), first, num,
                      ...
                      wrap_finisher(new C_EraseFinish(this,
...
bool Journaler::have_waiter() const const
{
...
void Journaler::_trim()
{
    ...
    ldout(cct, 10) << "trim trimming to " << trim_to
                   ...
                   << trimmed_pos << "/" << trimming_pos << "/" << expire_pos << dendl;
...
void Journaler::_finish_trim(int r, uint64_t to)
{
    ...
    ldout(cct, 10) << "_finish_trim trimmed_pos was " << trimmed_pos
                   ...
                   << to << "/" << trimming_pos << "/" << expire_pos << dendl;
    if (r < 0 && r != -ENOENT)
    {
 ...
 */
bool JournalStream::readable(bufferlist &read_buf, uint64_t *need) const const
{
    assert(need != NULL);
