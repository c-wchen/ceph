...
namespace librados {
    ...
    TestMemIoCtxImpl::TestMemIoCtxImpl(TestMemRadosClient * client,
                                       int64_t pool_id,
    ...
    int TestMemIoCtxImpl::aio_remove(const std::string & oid,
                                     AioCompletionImpl * c, int flags) {
        m_client->add_aio_operation(oid, true,
                                    std::bind(&TestMemIoCtxImpl::remove, this,
                                              oid, get_snap_context()), c);
        return 0;
    }
    ...
    int TestMemIoCtxImpl::append(const std::string & oid, const bufferlist & bl,
        ...
        ldout(cct,
              20) << "length=" << bl.length() << ", snapc=" << snapc << dendl;
    ...
    int TestMemIoCtxImpl::list_snaps(const std::string & oid,
    ...
    int TestMemIoCtxImpl::omap_get_vals(const std::string & oid,
                                        ...
                                        std::map < std::string,
                                        bufferlist > *out_vals) {
        return omap_get_vals2(oid, start_after, filter_prefix, max_return,
                              out_vals, nullptr);
    }
    ...
    int TestMemIoCtxImpl::omap_set(const std::string & oid,
        ...
        std::unique_lock l {
        file->lock};
        for (std::map < std::string, bufferlist >::const_iterator it =
    ...
    int TestMemIoCtxImpl::read(const std::string & oid, size_t len,
                               uint64_t off, bufferlist * bl, uint64_t snap_id,
                               uint64_t * objver) {
        if (m_client->is_blocklisted()) {
            return -EBLOCKLISTED;
        }
    ...
    int TestMemIoCtxImpl::remove(const std::string & oid,
        ...
        m_pool->file_lock};
        TestMemCluster::SharedFile file =
            get_file(oid, false, CEPH_NOSNAP, snapc);
        if (file == NULL) {
            return -ENOENT;
        }
        file = get_file(oid, true, CEPH_NOSNAP, snapc);
    ...
    int TestMemIoCtxImpl::selfmanaged_snap_rollback(const std::string & oid,
        ...
        size_t versions = 0;
        for (TestMemCluster::FileSnapshots::reverse_iterator it =
             snaps.rbegin(); it != snaps.rend(); ++it) {
            ...
            if (file->snap_id < get_snap_read()) {
                if (versions == 0) {
                    ...
                    if (versions == 1) {
                        ...
                        file =
                            TestMemCluster::SharedFile(new TestMemCluster::
                                                       File(**it));
    ...
    int TestMemIoCtxImpl::write(const std::string & oid, bufferlist & bl,
    ...
    int TestMemIoCtxImpl::xattr_set(const std::string & oid,
                                    const std::string & name, bufferlist & bl) {
        if (m_client->is_blocklisted()) {
            return -EBLOCKLISTED;
        }
    ...
    int TestMemIoCtxImpl::zero(const std::string & oid, uint64_t off,
    ...
    TestMemCluster::SharedFile TestMemIoCtxImpl::get_file(const std::
                                                          string & oid,
                                                          bool write,
                                                          uint64_t snap_id,
                                                          const SnapContext &
        ...
        if (write) {
            ...
            if (!file || !file->exists) {
                ...
                if (!snapc.snaps.empty() && file->snap_id < snapc.seq) {
                    for (std::vector <
                         snapid_t >::const_reverse_iterator seq_it =
                         snapc.snaps.rbegin(); seq_it != snapc.snaps.rend();
                         ++seq_it) {
