                                     ...
                                     std::string &upload_id)
{
    ...
    ldpp_dout(dpp, 20) << "init multipart result: bucket=" << result.bucket << " key=" << result.key << " upload_id=" <<
                       result.upload_id << dendl;
        ...
        const std::string &upload_id)
{
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to abort multipart upload dest obj=" << dest_obj << " upload_id=" <<
                                   upload_id << " ret=" << ret << dendl;
...
#define MULTIPART_MAX_PARTS 10000
    ...
    for (; (uint32_t)cur_part <= num_parts; ++cur_part) {
        ldpp_dout(tier_ctx.dpp, 20) << "cur_part = " << cur_part << ", info.ofs = " << cur_ofs << ", info.size = " << part_size
                                    << ", obj size = " << obj_size << ", num_parts:" << num_parts << dendl;
        ...
        if (ret < 0) {
            ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to send multipart part of obj=" << tier_ctx.obj <<
                                       ", sync via multipart upload, upload_id=" << status.upload_id << " part number " << cur_part << " (error: " <<
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to complete multipart upload of obj=" << tier_ctx.obj << " (error: " <<
                                   cpp_strerror(-ret) << ")" << dendl;
        cloud_tier_abort_multipart_upload(tier_ctx, dest_obj, status_obj, status.upload_id);
        return ret;
    }
    ...
    ret = delete_upload_status(tier_ctx.dpp, tier_ctx.driver, &status_obj);
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to abort multipart upload obj=" << tier_ctx.obj << " upload_id=" <<
                                   status.upload_id << " part number " << cur_part << " (" << cpp_strerror(-ret) << ")" << dendl;
        // ignore error, best effort
    }
    return 0;
}
...
static int cloud_tier_check_object(RGWLCCloudTierCtx &tier_ctx, bool &already_tiered)
{
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to fetch HEAD from cloud for obj=" << tier_ctx.obj << " , ret = " << ret <<
                                   dendl;
        return ret;
    }
...
static int cloud_tier_create_bucket(RGWLCCloudTierCtx &tier_ctx)
{
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "create target bucket : " << tier_ctx.target_bucket_name << " returned ret:" << ret <<
                                   dendl;
    }
    if (out_bl.length() > 0) {
        RGWXMLDecoder::XMLParser parser;
        if (!parser.init()) {
            ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to initialize xml parser for parsing create_bucket response from server" <<
                                       dendl;
            return -EIO;
        }
