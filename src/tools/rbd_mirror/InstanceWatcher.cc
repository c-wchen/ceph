...
struct C_GetInstances: public Context {
    ...
    C_GetInstances(std::vector < std::string > *instance_ids,
                   Context *on_finish)
        : instance_ids(instance_ids), on_finish(on_finish)
    {
    } void finish(int r) override
    {
        dout(10) << "C_GetInstances: " << this << " " << __func__ <<
                 ": r=" << r << dendl;

        if (r == 0) {
            auto it = out_bl.cbegin();
            r = librbd::cls_client::
                mirror_instances_list_finish(&it, instance_ids);
        } else if (r == -ENOENT) {
            r = 0;
        }
...
struct C_RemoveInstanceRequest: public Context {
    ...
    C_RemoveInstanceRequest(librados::IoCtx &io_ctx,
                            ...
                            Context *on_finish)
        : instance_watcher(io_ctx, asio_engine, nullptr, nullptr,
                           instance_id), on_finish(on_finish)
    {
    } void send()
    {
        dout(10) << "C_RemoveInstanceRequest: " << this << " " <<
                 __func__ << dendl;
...
template < typename I >
struct InstanceWatcher <I >::
    C_NotifyInstanceRequest: public Context {
    InstanceWatcher < I > *instance_watcher;
    ...
    C_NotifyInstanceRequest(InstanceWatcher < I > *instance_watcher,
                            const std::string &instance_id,
                            uint64_t request_id, bufferlist
    ...
    {
        dout(10) << "C_NotifyInstanceRequest: " << this << " " <<
                 __func__ << ": instance_watcher=" << instance_watcher <<
                 ", instance_id=" << instance_id << ", request_id=" <<
                 request_id << dendl;
        ...
        if (!send_to_leader) {
            ...
            notifier.
            reset(new librbd::watcher::
                  Notifier(instance_watcher->m_work_queue,
        ...
        auto result =
            instance_watcher->m_notify_ops.
            insert(std::make_pair(instance_id, this)).second;
        ceph_assert(result);
    }
    ...
    void send()
    {
        ...
        if (send_to_leader) {
            ...
            if (instance_watcher->m_leader_instance_id != instance_id) {
                auto count =
                    instance_watcher->m_notify_ops.
                    erase(std::make_pair(instance_id, this));
                ceph_assert(count > 0);
        ...
        dout(10) << "C_NotifyInstanceRequest: " << this << " " <<
                 __func__ << ": sending to " << instance_id << dendl;
        ...
        if (r == 0 || r == -ETIMEDOUT) {
            ...
            for (auto &it : response.acks) {
                ...
                if (it.second.length() == 0) {
                    dout(5) << "C_NotifyInstanceRequest: " << this <<
                            " " << __func__ <<
                ...
                try {
                    ...
                    decode(ack, iter);
                    if (ack.instance_id !=
                        instance_watcher->get_instance_id()) {
                        derr << "C_NotifyInstanceRequest: " << this <<
                             " " << __func__ << ": ack instance_id (" <<
                             ack.
                             instance_id << ") " <<
            ...
            if (!found) {
                if (r == -ETIMEDOUT) {
                    derr << "C_NotifyInstanceRequest: " << this << " "
                         << __func__ << ": resending after timeout" <<
                         dendl;
                    std::lock_guard locker {
                        instance_watcher->m_lock};
                    send();
                    return;
                } else {
                    r = -EINVAL;
        ...
        Context *on_finish)
{
    ...
    int r =
        io_ctx.aio_operate(RBD_MIRROR_LEADER, aio_comp, &op,
                           &ctx->out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
I >::remove_instance(librados::IoCtx &io_ctx,
                     librbd::AsioEngine &asio_engine,
                     const std::string &instance_id,
                     Context *on_finish)
{
    auto req =
        new C_RemoveInstanceRequest < I > (io_ctx, asio_engine,
                                           instance_id,
                                           on_finish);
    req->send();
}
...
I >::create(librados::IoCtx &io_ctx,
            librbd::AsioEngine &asio_engine,
            InstanceReplayer < I > *instance_replayer,
            Throttler < I > *image_sync_throttler)
{
    return new InstanceWatcher < I > (io_ctx, asio_engine,
                                      instance_replayer,
...
InstanceWatcher < I >::InstanceWatcher(librados::IoCtx &io_ctx,
    ...
    :    Watcher(io_ctx, asio_engine.get_work_queue(),
         ...
         m_lock(ceph::
                make_mutex(unique_lock_name
                           ("rbd::mirror::InstanceWatcher::m_lock", this))),
         m_instance_lock(librbd::ManagedLock <
                         I >::create(m_ioctx, asio_engine, m_oid, this,
...
template < typename I >
void InstanceWatcher <
I >::notify_image_acquire(const std::string &instance_id,
                          ...
                          Context *on_notify_ack)
{
    ...
    encode(NotifyMessage {
        ImageAcquirePayload {
            request_id, global_image_id}}, bl);
    auto req =
        new C_NotifyInstanceRequest(this, instance_id, request_id,
...
template < typename I >
void InstanceWatcher <
I >::notify_image_release(const std::string &instance_id,
                          const std::string &global_image_id,
                          Context *on_notify_ack)
{
    dout(10) << "instance_id=" << instance_id << ", global_image_id=" <<
...
template < typename I >
void InstanceWatcher <
I >::notify_peer_image_removed(const std::string &instance_id,
                               const std::string &global_image_id,
                               ...
                               Context *on_notify_ack)
{
    ...
    encode(NotifyMessage {
        PeerImageRemovedPayload {
            request_id, global_image_id, peer_mirror_uuid}}, bl);
    auto req =
        new C_NotifyInstanceRequest(this, instance_id, request_id,
                                    std::move(bl), on_notify_ack);
    req->send();
...
template < typename I >
void InstanceWatcher <
I >::notify_sync_request(const std::string &sync_id,
...
template < typename I >
bool InstanceWatcher <
I >::cancel_sync_request(const std::string &sync_id)
{
    dout(10) << "sync_id=" << sync_id << dendl;
...
template < typename I >
void InstanceWatcher <
I >::notify_sync_start(const std::string &instance_id,
                       const std::string &sync_id)
{
    ...
    auto ctx = new LambdaContext([this, sync_id](int r) {
        dout(10) << "finish: sync_id=" <<
                 sync_id << ", r=" << r << dendl;
        std::lock_guard locker {
            m_lock};
        if (r != -ESTALE && is_leader()) {
            m_image_sync_throttler->
            finish_op(m_ioctx.get_namespace(),
                      sync_id);
        }
    }
                                ) ;
...
template < typename I >
void InstanceWatcher <
I >::notify_sync_complete(const std::string &sync_id)
{
    std::lock_guard locker {
        m_lock};
...
template < typename I >
void InstanceWatcher <
I >::notify_sync_complete(const ceph::mutex &,
                          const std::string &sync_id)
{
    dout(10) << "sync_id=" << sync_id << dendl;
...
template < typename I >
void InstanceWatcher <
I >::handle_notify_sync_request(C_SyncRequest *sync_ctx, int r)
{
...
template < typename I >
void InstanceWatcher <
I >::handle_notify_sync_complete(C_SyncRequest *sync_ctx, int r)
{
    dout(10) << "sync_id=" << sync_ctx->sync_id << ", r=" << r << dendl;
...
template < typename I >
void InstanceWatcher <
I >::handle_update_leader(const std::string &leader_instance_id)
{
...
template < typename I >
void InstanceWatcher <
I >::cancel_notify_requests(const std::string &instance_id)
{
...
template < typename I > void InstanceWatcher < I >::register_instance()
{
    ...
    librados::AioCompletion *aio_comp = create_rados_callback <
                                        InstanceWatcher < I >,
                                        &InstanceWatcher < I >::handle_register_instance > (this);
...
void InstanceWatcher < I >::handle_register_instance(int r)
{
    ...
    Context *on_finish = nullptr;
    {
        ...
        derr << "error registering instance: " << cpp_strerror(r) <<
             dendl;
...
void InstanceWatcher < I >::handle_create_instance_object(int r)
{
    ...
    if (r < 0) {
        derr << "error creating " << m_oid << " object: " <<
             cpp_strerror(r)
             << dendl;
...
void InstanceWatcher < I >::handle_acquire_lock(int r)
{
    ...
    Context *on_finish = nullptr;
    {
        ...
        if (r < 0) {
            ...
            derr << "error acquiring instance lock: " << cpp_strerror(r)
                 << dendl;
...
void InstanceWatcher < I >::handle_release_lock(int r)
{
    ...
    if (r < 0) {
        derr << "error releasing instance lock: " << cpp_strerror(r) <<
             dendl;
    }
...
void InstanceWatcher < I >::handle_remove_instance_object(int r)
{
    ...
    if (r < 0) {
        derr << "error removing " << m_oid << " object: " <<
             cpp_strerror(r)
             << dendl;
    }
...
void InstanceWatcher < I >::unregister_instance()
{
    ...
    librados::AioCompletion *aio_comp = create_rados_callback <
                                        InstanceWatcher < I >,
                                        &InstanceWatcher < I >::handle_unregister_instance > (this);
...
void InstanceWatcher < I >::handle_unregister_instance(int r)
{
    ...
    if (r < 0) {
        derr << "error unregistering instance: " << cpp_strerror(r) <<
             dendl;
    }
...
void InstanceWatcher < I >::handle_get_instance_locker(int r)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            derr << "error retrieving instance locker: " <<
...
void InstanceWatcher < I >::handle_break_instance_lock(int r)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            derr << "error breaking instance lock: " << cpp_strerror(r)
                 << dendl;
        }
        remove_instance_object();
        return;
...
template < typename I >
void InstanceWatcher <
I >::suspend_notify_request(C_NotifyInstanceRequest *req)
{
    dout(10) << req << dendl;
...
template < typename I >
bool InstanceWatcher <
I >::unsuspend_notify_request(C_NotifyInstanceRequest *req)
{
    dout(10) << req << dendl;
...
template < typename I >
Context *InstanceWatcher <
I >::prepare_request(const std::string &instance_id,
                     uint64_t request_id,
                     C_NotifyAck *on_notify_ack)
{
    dout(10) << "instance_id=" << instance_id << ", request_id=" <<
             request_id << dendl;
...
template < typename I >
void InstanceWatcher <
I >::complete_request(const std::string &instance_id,
...
template < typename I >
void InstanceWatcher < I >::handle_notify(uint64_t notify_id,
        uint64_t handle,
        ...
        bufferlist &bl)
{
    ...
    try {
        ...
        derr << "error decoding image notification: " << err.
             what() << dendl;
        ctx->complete(0);
        return;
    }
    ...
    apply_visitor(HandlePayloadVisitor
                  (this, stringify(notifier_id), ctx),
                  notify_message.payload);
}

template < typename I >
void InstanceWatcher <
I >::handle_image_acquire(const std::string &global_image_id,
                          Context *on_finish)
{
    dout(10) << "global_image_id=" << global_image_id << dendl;

    auto ctx = new LambdaContext([this, global_image_id,
    on_finish](int r) {
        m_instance_replayer->
...
template < typename I >
void InstanceWatcher <
I >::handle_image_release(const std::string &global_image_id,
                          Context *on_finish)
{
    dout(10) << "global_image_id=" << global_image_id << dendl;
...
template < typename I >
void InstanceWatcher <
I >::handle_peer_image_removed(const std::string &global_image_id,
                               const std::string &peer_mirror_uuid,
                               Context *on_finish)
{
    ...
    auto ctx = new LambdaContext([this, peer_mirror_uuid,
    global_image_id, on_finish](int r) {
        m_instance_replayer->
...
template < typename I >
void InstanceWatcher <
I >::handle_sync_request(const std::string &instance_id,
                         const std::string &sync_id,
                         Context *on_finish)
{
    dout(10) << "instance_id=" << instance_id << ", sync_id=" << sync_id
             << dendl;
    ...
    Context *on_start =
        create_async_context_callback(m_work_queue,
                                      new
                                      LambdaContext([this, instance_id,
                                          sync_id,
    on_finish](int r) {
        dout(10) <<
                 "handle_sync_request: finish: instance_id="
                 << instance_id <<
                 ", sync_id=" <<
                 sync_id << ", r=" <<
                 r << dendl;
    ...
    m_image_sync_throttler->start_op(m_ioctx.get_namespace(), sync_id,
                                     on_start);
}

template < typename I >
void InstanceWatcher <
I >::handle_sync_start(const std::string &instance_id,
                       const std::string &sync_id,
                       Context *on_finish)
{
    dout(10) << "instance_id=" << instance_id << ", sync_id=" << sync_id
...
template < typename I >
void InstanceWatcher <
I >::handle_payload(const std::string &instance_id,
                    ...
                    C_NotifyAck *on_notify_ack)
{
    ...
    auto on_finish = prepare_request(instance_id, payload.request_id,
                                     on_notify_ack);
    if (on_finish != nullptr) {
        handle_peer_image_removed(payload.global_image_id,
                                  payload.peer_mirror_uuid, on_finish);
    }
}
