...
struct Subscription {
    ...
    Subscription(MonSession *s, const std::string &t): session(s), type(t),
...
struct MonSession: public RefCountedObject {
    ...
    bool is_capable(std::string service, int mask)
    {
        ...
        return caps.is_capable(g_ceph_context,
                               ...
                               mask & MON_CAP_X, get_peer_socket_addr());
    }
    ...
    void dump(ceph::Formatter *f) const
    {
        ...
        f->dump_stream("con_features_hex") << std::hex << con_features << std::
                                           dec;
...
struct MonSessionMap {
    ...
    } void remove_session(MonSession *s)
    {
        ...
        for (std::map < std::string, Subscription * >::iterator p =
                 s->sub_map.begin(); p != s->sub_map.end(); ++p) {
    ...
    MonSession *new_session(const entity_name_t &n,
                            const entity_addrvec_t &av, Connection *c)
    {
        ...
        while (backward || forward) {
            if (backward) {
                if (osdmap->is_up(b->first) &&
                    osdmap->get_addrs(b->first) ==
                    b->second->con->get_peer_addrs()) {
                    s = b->second;
                    break;
                }
                if (b != by_osd.begin()) {
    ...
    void add_update_sub(MonSession *s, const std::string &what,
                        version_t start, bool onetime,
