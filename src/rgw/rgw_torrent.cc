                           ...
                           ceph::bufferlist & bl_data, rgw_obj & obj)
{
    ...
    if (r < 0) {
        ldout(s->cct,
              0) << "ERROR: omap_get_vals_by_keys failed: " << r << dendl;
        return r;
    }
    if (m.size() != 1) {
        ldout(s->cct,
              0) << "ERROR: omap key " RGW_OBJ_TORRENT " not found" << dendl;
        return -EINVAL;
    }
...
int seed::complete()
{
    ...
    sha_len =
        (info.len / info.piece_length +
         remain_len) * CEPH_CRYPTO_SHA1_DIGESTSIZE;
...
int seed::save_torrent_file()
{
    ...
    if (op_ret < 0) {
        ldout(s->cct,
              0) << "ERROR: failed to omap_set() op_ret = " << op_ret << dendl;
        return op_ret;
    }
