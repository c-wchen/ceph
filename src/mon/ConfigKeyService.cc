...
void ConfigKeyService::store_put(const string & key, bufferlist & bl,
                                 Context * cb)
{
...
void ConfigKeyService::store_delete(MonitorDBStore::TransactionRef t,
                                    const string & key)
{
...
{
    KeyValueDB::Iterator iter = mon->store->get_iterator(STORE_PREFIX);
...
void ConfigKeyService::store_delete_prefix(MonitorDBStore::TransactionRef t,
                                           const string & prefix)
{
    KeyValueDB::Iterator iter = mon->store->get_iterator(STORE_PREFIX);
...
int ConfigKeyService::validate_osd_destroy(const int32_t id,
                                           const uuid_d & uuid)
{
    string dmcrypt_prefix = _get_dmcrypt_prefix(uuid, "");
    string daemon_prefix = "daemon-private/osd." + stringify(id) + "/";

    if (!store_has_prefix(dmcrypt_prefix) && !store_has_prefix(daemon_prefix)) {
        return -ENOENT;
    }
    return 0;
}
...
int ConfigKeyService::validate_osd_new(const uuid_d & uuid,
...
void ConfigKeyService::do_osd_new(const uuid_d & uuid,
                                  const string & dmcrypt_key)
{
