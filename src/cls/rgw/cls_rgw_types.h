...
template < class T > void encode_packed_val(T val, bufferlist & bl)
{
...
template < class T > void decode_packed_val(T & val, bufferlist::iterator & bl)
{
    unsigned char c;
    ::decode(c, bl);
    if (c < 0x80) {
...
struct rgw_bucket_dir_entry {
    ...
    rgw_bucket_dir_entry():exists(false), index_ver(0), flags(0),
        versioned_epoch(0)
{
} void encode(bufferlist & bl) const {
    ENCODE_START(8, 3, bl);
...
bool is_current() {
    int test_flags =
        RGW_BUCKET_DIRENT_FLAG_VER | RGW_BUCKET_DIRENT_FLAG_CURRENT;
...
bool get_info(cls_rgw_obj_key * key, uint8_t * category,
...
struct cls_rgw_bucket_instance_entry {
    ...
    static void generate_test_instances(list <
    ...
    void set_status(const string & new_instance_id, int32_t new_num_shards,
...
rgw_usage_log_entry(string & o, string & p, string & b):owner(o), payer(p), bucket(b),
    epoch(0) {
}
...
void aggregate(const rgw_usage_log_entry & e, map < string, bool > *categories =
               NULL) {
    if (owner.empty()) {
        owner = e.owner;
        bucket = e.bucket;
...
void sum(rgw_usage_data & usage, map < string, bool > &categories) const {
    ...
    for (map < string, rgw_usage_data >::const_iterator iter =
         usage_map.begin(); iter != usage_map.end(); ++iter) {
...
struct cls_rgw_obj_chain {
    ...
    cls_rgw_obj_chain()
{
} void push_obj(const string & pool, const cls_rgw_obj_key & key,
                const string & loc) {
    cls_rgw_obj obj;
    obj.pool = pool;
    obj.key = key;
...
void dump(Formatter * f) const {
    ...
    for (list < cls_rgw_obj >::const_iterator p = objs.begin(); p != objs.end();
         ++p) {
...
static void generate_key(const string & tenant, const string & bucket_name,
                         string * key);
