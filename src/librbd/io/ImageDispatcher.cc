...
namespace librbd {
    namespace io {
        ...
        template < typename I >
            struct ImageDispatcher <I >::SendVisitor:public boost::
            static_visitor < bool > {
            ...
            :image_dispatch(image_dispatch),
                ...
                return image_dispatch->write_same(image_dispatch_spec->aio_comp,
                               ...
                               CompareAndWrite & compare_and_write) const {
                return image_dispatch->compare_and_write(image_dispatch_spec->
                                                         aio_comp,
                                                         std::
                                                         move
                                                         (image_dispatch_spec->
         ...
         template < typename I >
            struct ImageDispatcher <I >::PreprocessVisitor:public boost::
            static_visitor < bool > {
            ...
            :image_dispatcher(image_dispatcher),
                ...
                auto area = (image_dispatch_spec->image_dispatch_flags &
                             IMAGE_DISPATCH_FLAG_CRYPTO_HEADER ? ImageArea::
                             CRYPTO_HEADER : ImageArea::DATA);
            ...
            bool operator() (ImageDispatchSpec::Flush &) const {
                ...
                image_dispatcher->m_image_ctx->image_lock};
      ...
      :    Dispatcher < I, ImageDispatcherInterface > (image_ctx) {
            ...
            m_write_block_dispatch =
                new WriteBlockImageDispatch < I > (image_ctx);
        ...
        template < typename I >
            ...
            auto ctx =
                new C_InvalidateCache(this, IMAGE_DISPATCH_LAYER_NONE,
                                      on_finish);
            ctx->complete(0);
        }
        ...
        template < typename I >
            void ImageDispatcher <
            I >::apply_qos_schedule_tick_min(uint64_t tick) {
            m_qos_image_dispatch->apply_qos_schedule_tick_min(tick);
        }

        template < typename I >
            void ImageDispatcher < I >::apply_qos_limit(uint64_t flag,
                                                        uint64_t limit,
                                                        uint64_t burst,
        ...
        template < typename I >
            void ImageDispatcher <
            I >::apply_qos_exclude_ops(uint64_t exclude_ops) {
        ...
        template < typename I >
            void ImageDispatcher <
            I >::wait_on_writes_unblocked(Context * on_unblocked) {
            m_write_block_dispatch->wait_on_writes_unblocked(on_unblocked);
        }

        template < typename I >
            void ImageDispatcher <
            I >::remap_to_physical(Extents & image_extents, ImageArea area) {
            ...
            auto crypto_image_dispatch =
                static_cast <
                crypto::CryptoImageDispatch * >(it->second.dispatch);
            crypto_image_dispatch->remap_to_physical(image_extents, area);
        }

        template < typename I >
            ImageArea ImageDispatcher <
        ...
        template < typename I >
            ...
            I >::send_dispatch(ImageDispatchInterface * image_dispatch,
                               ImageDispatchSpec * image_dispatch_spec) {
            ...
            return boost::apply_visitor(SendVisitor {
                                        image_dispatch, image_dispatch_spec}
                                        , image_dispatch_spec->request);
        }

        template < typename I >
            bool ImageDispatcher <
            I >::preprocess(ImageDispatchSpec * image_dispatch_spec) {
            return boost::apply_visitor(PreprocessVisitor {
                                        this, image_dispatch_spec}
                                        , image_dispatch_spec->request);
        }
