...
//  * raw pointer (doesn't embed ownership enforcement - std::move).
    template < class T >
        struct unique_leakable_ptr:public std::unique_ptr < T,
        ceph::nop_delete < T >> {
...
/// Actual definitions in common/error_code.h
             ...
             * named constructors
             */
             ceph::unique_leakable_ptr < raw > copy(const char *c,
                                                    unsigned len);
             ceph::unique_leakable_ptr < raw > create(unsigned len);
             ceph::unique_leakable_ptr < raw > create(unsigned len, char c);
             ceph::unique_leakable_ptr < raw > create_in_mempool(unsigned len,
                                                                 int mempool);
             ceph::unique_leakable_ptr < raw > claim_char(unsigned len,
                                                          char *buf);
             ceph::unique_leakable_ptr < raw > create_malloc(unsigned len);
             ceph::unique_leakable_ptr < raw > claim_malloc(unsigned len,
                                                            char *buf);
             ceph::unique_leakable_ptr < raw > create_static(unsigned len,
                                                             char *buf);
             ceph::unique_leakable_ptr < raw > create_aligned(unsigned len,
                                                              unsigned align);
             ceph::unique_leakable_ptr < raw >
                create_aligned_in_mempool(unsigned len, unsigned align,
                                          int mempool);
             ceph::unique_leakable_ptr < raw >
                create_page_aligned(unsigned len);
             ceph::unique_leakable_ptr < raw >
                create_small_page_aligned(unsigned len);
             ceph::unique_leakable_ptr < raw > claim_buffer(unsigned len,
                                                            char *buf,
                                                            deleter del);
            ...
            /// make it safe to share between cpus
             ceph::unique_leakable_ptr < buffer::raw >
                create(seastar::temporary_buffer < char >&&buf);
            ...
            /// destructed on this cpu
             ceph::unique_leakable_ptr < buffer::raw >
                create_local(seastar::temporary_buffer < char >&&buf);
#endif
            ...
            class CEPH_BUFFER_API ptr {
                ...
                void release();
                 ...
                 template < bool is_const > class iterator_impl {
                    ...
                    const bool deep;    ///< if true, do not allow shallow ptr copies
                     ...
                     iterator_impl(typename std::conditional < is_const,
                                   const ptr *, ptr * >::type p, size_t offset,
                                   bool d)
                    :bp(p), start(p->c_str() + offset), pos(start),
                        end_ptr(p->end_c_str()), deep(d) {
                    } friend class ptr;

                  public:
                     using pointer =
                        typename std::conditional < is_const, const char *,
                        char *>::type;
              ...
              ptr():_raw(nullptr), _off(0), _len(0) {
                ...
                // misc bool is_aligned(unsigned align)const {
        ...
        class ptr_node:public ptr_hook, public ptr {
          public:
            ...
            struct disposer {
                void operator() (ptr_node * const delete_this) {
                    if (!__builtin_expect
                        (dispose_if_hypercombined(delete_this), 0)) {
                        delete delete_this;
                    }
                }
            };
            ...
            template < class ... Args >
                static std::unique_ptr < ptr_node, disposer >
                create(Args && ... args) {
                return std::unique_ptr < ptr_node,
                    disposer > (new ptr_node(std::forward < Args > (args) ...));
            }
          ...
          template < class ... Args > ptr_node(Args && ... args):ptr(std::forward < Args >
                (args) ...)
            {
            }
            ...
            static std::unique_ptr < ptr_node,
                disposer > create_hypercombined(ceph::unique_leakable_ptr <
                                                raw > r);
        };
        /*
         * list - the useful bit!
        ...
        class CEPH_BUFFER_API list {
            ...
            class buffers_t {
              ...
              public:
                 template < class T > class buffers_iterator {
                    typename std::conditional <
                        std::is_const < T >::value, const ptr_hook *,
                        ptr_hook * >::type cur;
                  ...
                  public:
                    ...
                    using iterator_category = std::forward_iterator_tag;
                     ...
                     template < class U > buffers_iterator(U * const p)
                    :cur(p) {
                    }
                    // copy constructor
                        buffers_iterator(const buffers_iterator < T >
                                         &other)
                    :cur(other.cur) {
                    }
                    ...
                    template < class U, typename std::enable_if <
                        ...
                        && !std::is_const < U >::value, int >::type =
                        0 > buffers_iterator(const buffers_iterator < U >
                                             &other)
                        :cur(other.cur) {
                    }
                    ...
                    template < class U >
                        buffers_iterator & operator=(buffers_iterator < U >
                                                     &other) {
                        cur = other.cur;
                        return *this;
                    }
              ...
              :
                ...
                _root(other._root.next ==
                      &other._root ? &_root : other._root.next),
                ...
                iterator erase_after(const_iterator it) {
                    ...
                    _root.next =
                        _root.next == to_erase ? to_erase->next : _root.next;
                ...
                void swap(buffers_t & other) {
                    ...
                    _root.next =
                        other._root.next ==
                        &other._root ? &this->_root : other._root.next;
                    other._root.next =
                        copy_root.next ==
                        &_root ? &other._root : copy_root.next;

                    const auto copy_tail = _tail;
                    _tail =
                        other._tail ==
                        &other._root ? &this->_root : other._tail;
                    other._tail =
                        copy_tail == &_root ? &other._root : copy_tail;
            ...
            template < bool is_const > class CEPH_BUFFER_API iterator_impl {
              protected:
                typedef typename std::conditional < is_const,
                    const list, list >::type bl_t;
                typedef typename std::conditional < is_const,
                    const buffers_t, buffers_t >::type list_t;
                typedef typename std::conditional < is_const,
              ...
              :    bl(0), ls(0), off(0), p_off(0) {
                ...
                // copy data out.// note that these all _append_ to dest!
          ...
          public:
            ...
            class page_aligned_appender {
                ...
                template < class Func >
                    ...
                    const auto free_in_last =
                        bl.get_append_buffer_unused_tail_length();
              ...
              public:
                ...
                void append(const char *buf, size_t entire_len) {
                    _append_common(entire_len,
                                   ...
                                   bl.append(buf, chunk_len); buf += chunk_len;
                                   });
                }
            ...
            page_aligned_appender get_page_aligned_appender(unsigned
                                                            min_pages = 1) {
          ...
          list(list && other) noexcept:_buffers(std::move(other._buffers)),
            ...
            unsigned length() const {
                ...
                // DEBUG: verify _len
                unsigned len = 0;
                for (std::list < ptr >::const_iterator it =
                     _buffers.begin(); it != _buffers.end(); it++) {
                    len += (*it).length();
                }
#ifdef __CEPH__
            ...
            [[deprecated("in favor of operator=(list&&)")]] void claim(list
                                                                       & bl) {
                *this = std::move(bl);
            }
            ...
            // funky modifer
            void splice(unsigned off, unsigned len, list * claim_by =
            ...
            void hexdump(std::ostream & out, bool trailing_newline =
                         true) const;
            ssize_t pread_file(const char *fn, uint64_t off, uint64_t len,
                               std::string * error);
    ...
    class hash {
      ...
      public:
        ...
        // cppcheck-suppress noExplicitConstructor hash(uint32_t init):crc(init) {
        }
