...
namespace librbd {
    ...
    namespace image_watcher {
        ...
        :m_image_ctx(image_ctx), m_notifier(notifier), m_bl(std::move(bl)),
            ...
            m_notifier.notify(m_bl, &m_notify_response,
                              create_context_callback < NotifyLockOwner,
                              &NotifyLockOwner::handle_notify > (this));
        }
        ...
        void NotifyLockOwner::handle_notify(int r) {
            ...
            if (r < 0 && r != -ETIMEDOUT) {
                lderr(cct) << ": lock owner notification failed: " <<
                    cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
          ...
          for (auto & it:m_notify_response.acks) {
                if (it.second.length() > 0) {
                    if (lock_owner_responded) {
                        lderr(cct) << ": duplicate lock owners detected" <<
                            dendl;
                        finish(-EINVAL);
                        return;
                    }
