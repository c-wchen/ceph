...
namespace librbd {
    namespace image {
        ...
        :m_p_imctx(p_imctx), m_ioctx(c_ioctx), m_name(c_name), m_id(c_id),
            m_opts(c_options), m_pspec(m_p_imctx->md_ctx.get_id(),
                                       m_p_imctx->id, m_p_imctx->snap_id),
            ...
            if (!default_format_set) {
                m_opts.set(RBD_IMAGE_OPTION_FORMAT,
                           static_cast < uint64_t > (2));
            } ldout(m_cct,
                    20) << "clone " << &m_p_imctx->
                md_ctx << " name " << m_p_imctx->name << " snap " << m_p_imctx->
                snap_name << " to child " << &m_ioctx << " name " << m_name <<
                " opts = " << &m_opts << dendl;
             return;
        ...
        template < typename I > void CloneRequest < I >::validate_options() {
            ...
            if (m_opts.get(RBD_IMAGE_OPTION_FEATURES, &m_features) == 0) {
                if (m_features & ~RBD_FEATURES_ALL) {
                    lderr(m_cct) << "librbd does not support requested features"
                        << dendl;
        ...
        template < typename I > void CloneRequest < I >::send_validate_parent() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;

            if (m_p_imctx->snap_id == CEPH_NOSNAP) {
                lderr(m_cct) << "image to be cloned must be a snapshot" <<
                    dendl;
                return complete(-EINVAL);
            }
            ...
            r = m_p_imctx->is_snap_protected(m_p_imctx->snap_id,
                                             &snap_protected);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "failed to determine tag ownership: " <<
                    cpp_strerror(r)
                    << dendl;
                return complete(r);
            }
            ...
            if ((m_p_features & RBD_FEATURE_JOURNALING) != 0) {
                if (!m_is_primary && !m_force_non_primary) {
                    lderr(m_cct) << "parent is non-primary mirrored image" <<
                        dendl;
                    return complete(-EINVAL);
                }
            }
        ...
        template < typename I > void CloneRequest < I >::send_validate_child() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;

            using klass = CloneRequest < I >;
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_validate_child > (this);

            librados::ObjectReadOperation op;
            op.stat(NULL, NULL, NULL);

            int r =
                m_ioctx.aio_operate(util::old_header_name(m_name), comp, &op,
                                    &m_out_bl);
            assert(r == 0);
            comp->release();
        }
        ...
        template < typename I >
            ...
            if (r != -ENOENT) {
                lderr(m_cct) << "rbd image " << m_name << " already exists" <<
                    dendl;
                return complete(r);
            }
        ...
        template < typename I > void CloneRequest < I >::send_create() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            using klass = CloneRequest < I >;
            Context *ctx =
                create_context_callback < klass, &klass::handle_create > (this);

            RWLock::RLocker snap_locker(m_p_imctx->snap_lock);
            CreateRequest < I > *req =
                CreateRequest < I >::create(m_ioctx, m_name, m_id, m_size,
                                            m_opts,
                                            m_non_primary_global_image_id,
                                            m_primary_mirror_uuid, true,
                                            m_op_work_queue, ctx);
            req->send();
        }

        template < typename I > void CloneRequest < I >::handle_create(int r) {
            ldout(m_cct, 20) << this << " " << __func__ << " r=" << r << dendl;

            if (r < 0) {
                lderr(m_cct) << "error creating child: " << cpp_strerror(r) <<
                    dendl;
                return complete(r);
            }
            send_open();
        ...
        template < typename I > void CloneRequest < I >::send_open() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            using klass = CloneRequest < I >;
            Context *ctx =
                create_context_callback < klass, &klass::handle_open > (this);

            m_imctx->state->open(true, ctx);
        }

        template < typename I > void CloneRequest < I >::handle_open(int r) {
            ldout(m_cct, 20) << this << " " << __func__ << " r=" << r << dendl;

            if (r < 0) {
                lderr(m_cct) << "Error opening new image: " << cpp_strerror(r)
                    << dendl;
                m_r_saved = r;
                return send_remove();
            }
        ...
        template < typename I > void CloneRequest < I >::send_set_parent() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "couldn't set parent: " << cpp_strerror(r) <<
                    dendl;
                m_r_saved = r;
                return send_close();
            }
        ...
        template < typename I > void CloneRequest < I >::send_add_child() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_add_child > (this);
            int r = m_ioctx.aio_operate(RBD_CHILDREN, comp, &op);
            assert(r == 0);
            comp->release();
        }

        template < typename I > void CloneRequest < I >::handle_add_child(int r) {
            ldout(m_cct, 20) << this << " " << __func__ << " r=" << r << dendl;

            if (r < 0) {
                lderr(m_cct) << "couldn't add child: " << cpp_strerror(r) <<
                    dendl;
                m_r_saved = r;
                return send_close();
            }
        ...
        template < typename I > void CloneRequest < I >::send_refresh() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;

            using klass = CloneRequest < I >;
            RefreshRequest < I > *req =
                RefreshRequest < I >::create(*m_imctx, false, false,
                                             create_context_callback < klass,
                                             &klass::handle_refresh > (this));
            req->send();
        }

        template < typename I > void CloneRequest < I >::handle_refresh(int r) {
            ldout(m_cct, 20) << this << " " << __func__ << " r=" << r << dendl;
        ...
        template < typename I > void CloneRequest < I >::send_metadata_list() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_metadata_list > (this);
            m_out_bl.clear();
        ...
        template < typename I >
            ...
            if (r < 0) {
                if (r == -EOPNOTSUPP || r == -EIO) {
                    ldout(m_cct,
                          10) << "config metadata not supported by OSD" <<
                        dendl;
                    get_mirror_mode();
                }
                else {
                    lderr(m_cct) << "couldn't list metadata: " <<
        ...
        template < typename I > void CloneRequest < I >::send_metadata_set() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_metadata_set > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "couldn't set metadata: " << cpp_strerror(r) <<
                    dendl;
                m_r_saved = r;
                send_remove_child();
            }
        ...
        template < typename I > void CloneRequest < I >::get_mirror_mode() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_get_mirror_mode > (this);
            m_out_bl.clear();
        ...
        template < typename I > void CloneRequest < I >::send_enable_mirror() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;

            using klass = CloneRequest < I >;
            Context *ctx =
                create_context_callback < klass,
                &klass::handle_enable_mirror > (this);

            mirror::EnableRequest < I > *req =
                mirror::EnableRequest < I >::create(m_imctx->md_ctx, m_id,
                                                    m_non_primary_global_image_id,
        ...
        template < typename I > void CloneRequest < I >::send_close() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            using klass = CloneRequest < I >;
            Context *ctx =
                create_async_context_callback(*m_imctx,
                                              create_context_callback < klass,
                                              &klass::handle_close > (this));
            m_imctx->state->close(ctx);
        }

        template < typename I > void CloneRequest < I >::handle_close(int r) {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            if (r < 0) {
                lderr(m_cct) << "couldn't close image: " << cpp_strerror(r) <<
                    dendl;
                return complete(r);
            }
        ...
        template < typename I > void CloneRequest < I >::send_remove_child() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_remove_child > (this);
        ...
        template < typename I > void CloneRequest < I >::send_remove() {
            ldout(m_cct, 20) << this << " " << __func__ << dendl;

            using klass = CloneRequest < I >;
            Context *ctx =
                create_context_callback < klass, &klass::handle_remove > (this);

            librbd::image::RemoveRequest <> *req =
                librbd::image::RemoveRequest <>::create(m_ioctx, m_name, m_id,
                                                        false, false, m_no_op,
                                                        m_op_work_queue, ctx);
            req->send();
        }

        template < typename I > void CloneRequest < I >::handle_remove(int r) {
            ldout(m_cct, 20) << this << " " << __func__ << " r=" << r << dendl;
