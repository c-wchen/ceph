...
class LogEvent {
  public:
    ...
    static std::unique_ptr < LogEvent >
        decode_event(bufferlist::const_iterator);
  ...
  protected:
    LogSegment * get_segment() {
        return _segment;
    }
    LogSegment const *get_segment() const {
        return _segment;
    } utime_t stamp;
  ...
  private:
    ...
    static std::unique_ptr < LogEvent >
        decode_event(bufferlist::const_iterator &, EventType);
