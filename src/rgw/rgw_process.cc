                              ...
                              const bool skip_retarget)
{
    ...
    {
        auto span =
            tracing::rgw::tracer.add_span("verify_permission", s->trace);
                    ...
                    ceph::coarse_real_clock::duration *latency, int *http_ret)
{
    ...
    }
    {
        ...
        auto rc =
            rgw::lua::read_script(s, penv.lua.manager.get(), s->bucket_tenant,
                                  s->yield, rgw::lua::context::preRequest,
                                  script);
        if (rc == -ENOENT) {
    ...
    try {
        ...
        s->trace =
            tracing::rgw::tracer.start_trace(trace_name, s->trace_enabled);
    ...
    if (op) {
        if (s->trace) {
            ...
            if (!rgw::sal::User::empty(s->user)) {
                s->trace->SetAttribute(tracing::rgw::USER_ID,
                                       s->user->get_id().id);
            }
        ...
        auto rc =
            rgw::lua::read_script(s, penv.lua.manager.get(), s->bucket_tenant,
                                  s->yield, rgw::lua::context::postRequest,
                                  script);
        if (rc == -ENOENT) {
            // no script, nothing to do
        } else if (rc < 0) {
