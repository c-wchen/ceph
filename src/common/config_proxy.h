...
namespace ceph::common {
   ...
   * hold this lock when calling configuration observers.  */
        ...
        void map_observer_changes(md_config_obs_t * obs,
      ...
      :    config {
        values, obs_mgr, is_daemon}
        {
        }
        ConfigProxy(const ConfigProxy & config_proxy)
        :values(config_proxy.get_config_values()), config {
        values, obs_mgr, config_proxy.config.is_daemon}
        {
        }
        ...
        template < typename T, typename Callback, typename ... Args >
            auto with_val(const std::string_view key, Callback && cb, Args
                          && ... args)const {
            std::lock_guard l {
            lock};
        ...
        } void diff(ceph::Formatter * f, const std::string & name = {
                    }) const {
            std::lock_guard l {
            lock};
        ...
        int get_val_from_conf_file(const std::vector < std::string > &sections,
                                   ...
                                   std::string & out, bool emeta) const {
            std::lock_guard l {
            lock};
        ...
        void call_all_observers() {
            std::unique_lock locker(lock);
            rev_obs_map_t rev_obs;
            obs_mgr.for_each_observer([this, &rev_obs] (md_config_obs_t * obs,
                                                        const std::
                                                        string & key) {
                                      map_observer_changes(obs, key,
                                                           &rev_obs);});
        ...
        void _gather_changes(std::set < std::string > &changes,
            ...
            obs_mgr.for_each_change(changes, *this,
        ...
        void set_val_default(const std::string_view key,
        ...
        int parse_argv(std::vector < const char *>&args, int level =
                       CONF_CMDLINE) {
            std::lock_guard l {
            lock};
