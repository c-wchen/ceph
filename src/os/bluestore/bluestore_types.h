...
size_t get_csum_count() const {
    ...
    case 0:
        ...
        case 2:return reinterpret_cast < const __le16 *>(p)[i];
        case 4:return reinterpret_cast < const __le32 *>(p)[i];
        case 8:return reinterpret_cast < const __le64 *>(p)[i];
        default:assert(0 == "unrecognized csum word size");
...
void add_tail(uint32_t new_len) {
    ...
    if (has_csum()) {
        ...
        t.swap(csum_data);
        csum_data =
            buffer::create(get_csum_value_size() * logical_length /
                           get_csum_chunk_size());
