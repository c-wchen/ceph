    ...
    class Thread_get:public Thread {
      public:
        ...
        Thread_get(Throttle & _throttle, int64_t _count):
            throttle(_throttle), count(_count), waited(false) {
        } void *entry() override {
            usleep(5);
...
TEST_F(ThrottleTest, destructor)
{
    EXPECT_DEATH( {
                 int64_t throttle_max = 10;
                 auto throttle =
                 ceph::make_unique < Throttle > (g_ceph_context, "throttle",
                                                 throttle_max);
                 ASSERT_FALSE(throttle->get(5));
                 unique_ptr < Thread_get > t =
                 ceph::make_unique < Thread_get > (*throttle, 7);
                 t->create("t_throttle"); bool blocked; useconds_t delay = 1;
                 do {
                 usleep(delay); if (throttle->get_or_fail(1)) {
                 throttle->put(1); blocked = false;}
                 else {
                 blocked = true;}
                 delay *= 2;} while (!blocked);}, ".*");
    }
    ...
    std::pair < double,
        ...
        bool valid = throttle.set_params(low_threshhold,
                                         high_threshhold,
                                         expected_throughput,
                                         high_multiple,
                                         max_multiple,
      ...
      for (auto && i:pts)
        ...
        return make_pair(((double)total_observed_total) /
                         ((double)total_observations), wait_time / waits);
    }

    TEST(BackoffThrottle, destruct) {
        EXPECT_DEATH( {
                     auto throttle =
                     ceph::make_unique < BackoffThrottle > (g_ceph_context,
                                                            "destructor test",
                                                            10);
                     ASSERT_TRUE(throttle->
                                 set_params(0.4, 0.6, 1000, 2, 10, 6, nullptr));
                     throttle->get(5); {
                     auto & t = *throttle; std::thread([&t] () {
                                                       usleep(5); t.get(6);});}
    ...
    TEST(OrderedThrottle, destruct) {
        EXPECT_DEATH( {
                     auto throttle =
                     ceph::make_unique < OrderedThrottle > (1, false);
