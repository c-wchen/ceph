...
namespace crimson::osd {
  ...
  :    OperationRegistryT(seastar::this_shard_id()) {
    ...
    void OSDOperationRegistry::put_historic(const ClientRequest & op) {
        ...
        if (num_slow_ops > local_conf()->osd_op_history_slow_op_size) {
            ...
            const auto fastest_historic_iter =
                std::min_element(std::cbegin(historic_registry),
                                 last_of_recents,
                                 [](const auto & lop, const auto & rop){
                                 const auto & lclient_request =
                                 static_cast < const ClientRequest & >(lop);
                                 const auto & rclient_request =
                                 static_cast < const ClientRequest & >(rop);
                                 return get_duration(lclient_request) <
                                 get_duration(rclient_request);}
            );
    ...
    size_t OSDOperationRegistry::dump_historic_client_requests(ceph::Formatter *
                                                               f) const {
        const auto & historic_client_registry = get_registry < static_cast < size_t > (OperationTypeCode::historic_client_request) > ();    //ClientRequest::type)>();
    ...
    size_t OSDOperationRegistry::dump_slowest_historic_client_requests(ceph::
                                                                       Formatter
                                                                       *
                                                                       f) const
    {
        ...
        std::multimap < utime_t,
            const ClientRequest *, std::greater < utime_t >> sorted_slowest_ops;
        ...
        std::transform(std::begin(historic_client_registry),
                       ...
                       std::inserter(sorted_slowest_ops,
                                     std::end(sorted_slowest_ops)),
                       [](const Operation & op) {
                       const auto & cop =
                       static_cast < const ClientRequest & >(op);
        ...
        for (auto it = std::begin(sorted_slowest_ops);
             ...
             && it != std::end(sorted_slowest_ops); ++it, ++ops_count) {
            it->second->dump(f);
        }
        f->close_section();
        return ops_count;
  ...
  :    scheduler(crimson::osd::scheduler::make_scheduler(conf)) {
    ...
    seastar::future <> OperationThrottler::acquire_throttle(crimson::osd::
                                                            scheduler::
                                                            params_t params) {
        crimson::osd::scheduler::item_t item {
        params, seastar::promise <> ()};
    ...
    void OperationThrottler::update_from_config(const ConfigProxy & conf) {
        max_in_progress =
            conf.get_val < uint64_t > ("crimson_osd_scheduler_concurrency");
        wake();
    }
    ...
    void OperationThrottler::handle_conf_change(const ConfigProxy & conf,
