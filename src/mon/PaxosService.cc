...
void PaxosService::maybe_trim()
{
    ...
    if (g_conf->paxos_service_trim_min > 0 &&
        ...
        dout(10) << __func__ << " trim_to " << trim_to << " would only trim " <<
            to_remove << " < paxos_service_trim_min " << g_conf->
            paxos_service_trim_min << dendl;
        return;
    }
    ...
    if (g_conf->paxos_service_trim_max > 0 &&
        ...
        dout(10) << __func__ << " trim_to " << trim_to << " would only trim " <<
            to_remove << " > paxos_service_trim_max, limiting to " << g_conf->
                        ...
                        version_t from, version_t to)
{
    ...
    if (g_conf->mon_compact_on_trim) {
        ...
        t->compact_range(get_service_name(), stringify(from - 1),
                         stringify(to));
        t->compact_range(get_service_name(),
                         mon->store->combine_strings(full_prefix_name,
                                                     from - 1),
