        ...
        namespace image_meta {
             ...
             namespace {
                ...
                void add_key_option(po::options_description * positional) {
                    ...
                    *key = utils::get_positional_argument(vm, 1);
                    if (key->empty()) {
                        std::
                            cerr << "rbd: metadata key was not specified" <<
                            std::endl;
                        return -EINVAL;
                    } return 0;
            ...
            static int do_metadata_list(librbd::Image & image, Formatter * f) {
                ...
                while (more_results) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "failed to list metadata of image : " <<
                            cpp_strerror(r)
                            << std::endl;
                        return r;
                    }
                    ...
                    if (!pairs.empty()) {
                        if (count == 0) {
                            if (f) {
                                ...
                                tbl.define_column("Key", TextTable::LEFT,
                                                  TextTable::LEFT);
                ...
                if (f == nullptr) {
                    ...
                    std::
                        ...
                        count << " " << (single ? "metadatum" : "metadata") <<
                        " on this image" << (count ==
                                             0 ? "." : ":") << std::endl;
                }
            ...
            static int do_metadata_set(librbd::Image & image, const char *key,
                ...
                if (r < 0) {
                    std::
                        cerr << "failed to set metadata " << key <<
                        " of image : " << cpp_strerror(r) << std::endl;
                }
                return r;
            }

            static int do_metadata_remove(librbd::Image & image,
                                          const char *key) {
                int r = image.metadata_remove(key);
                if (r == -ENOENT) {
                    std::
                        cerr << "rbd: no existing metadata key " << key <<
                        " of image : " << cpp_strerror(r) << std::endl;
                }
                else if (r < 0) {
                    std::
                        cerr << "failed to remove metadata " << key <<
                        " of image : " << cpp_strerror(r) << std::endl;
                }
                return r;
            }
            ...
            static int do_metadata_get(librbd::Image & image, const char *key) {
                ...
                if (r < 0) {
                    std::
                        cerr << "failed to get metadata " << key <<
                        " of image : " << cpp_strerror(r) << std::endl;
                    return r;
                }
                std::cout << s << std::endl;
            ...
            int execute_list(const po::variables_map & vm) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: listing metadata failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_get(const po::variables_map & vm) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: getting metadata failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_set(const po::variables_map & vm) {
                ...
                if (value.empty()) {
                    std::
                        cerr << "rbd: metadata value was not specified" << std::
                        endl;
                    return -EINVAL;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: setting metadata failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_remove(const po::variables_map & vm) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: removing metadata failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
