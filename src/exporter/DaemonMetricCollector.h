...
private:
    ...
    void dump_asok_metric(boost::json::object perf_info,
                          ...
                          labels_t labels);
    std::pair < labels_t, std::string > get_labels_and_metric_name(std::string daemon_name,
            std::string metric_name);
    std::pair < labels_t,
        std::string > add_fixed_name_metrics(std::string metric_name);
    void get_process_metrics(std::vector < std::pair < std::string,
                             int >>daemon_pids);
...
public:
    ...
    virtual void add(std::string value, std::string name,
