...
PK11SymKey *ceph::crypto::PK11_ImportSymKey_FIPS(PK11SlotInfo * const slot,
                                                 const CK_MECHANISM_TYPE type,
                                                 const PK11Origin origin,
                                                 const CK_ATTRIBUTE_TYPE
                                                 operation,
                                                 ...
                                                 void *const wincx)
{
    if (PK11_IsFIPS() == PR_FALSE) {
        ...
        ret_key =
            PK11_ImportSymKey(slot, type, origin, operation, raw_key, wincx);
    ...
    PK11SymKey *const wrapping_key = PK11_KeyGen(slot,
                                                 wrap_mechanism,
                                                 nullptr,
                                                 PK11_GetBestKeyLength(slot,
                                                                       wrap_mechanism),
                                                 nullptr);
    ...
    PK11Context *const wrap_key_crypt_context =
    ...
    auto wkcc_guard = make_scope_guard([wrap_key_crypt_context] {
                                       PK11_DestroyContext
                                       (wrap_key_crypt_context, PR_TRUE);}
    );
    ...
    // be aligned to cipher's block size. For 3DES it's 64 bits.
    {
        ...
        auto rka_guard = make_scope_guard([raw_key_aligned] {
                                          SECITEM_FreeItem(raw_key_aligned,
                                                           PR_TRUE);}
        );
        ...
        int ret = PK11_CipherOp(wrap_key_crypt_context,
                                wrapped_key.data(),
                                &out_len,
    ...
    return PK11_UnwrapSymKey(wrapping_key,
                             wrap_mechanism,
                             &tmp_sec_item,
...
void ceph::crypto::init(CephContext * cct)
{
    ...
    if (++crypto_refs == 1) {
        ...
        crypto_context =
            NSS_InitContext(cct->_conf->nss_db_path.c_str(), "", "", SECMOD_DB,
                            &init_params, flags);
    }
