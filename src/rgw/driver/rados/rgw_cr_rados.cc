...
RGWSimpleRadosLockCR::RGWSimpleRadosLockCR(RGWAsyncRadosProcessor *_async_rados, rgw::sal::RadosStore *_store,
    ...
    req(nullptr)
{
    set_description() << "rados lock dest=" << obj << " lock=" << lock_name << " cookie=" << cookie << " duration=" <<
                      duration;
}
    ...
    if (r < 0) {
        ...
        if (bytes_transferred) {
            ...
            if (ret < 0) {
                ...
                ret = rgw::notify::publish_commit(&dest_obj, *bytes_transferred, ceph::real_clock::now(), etag, dest_obj.get_instance(),
                                                  rgw::notify::ObjectSyncedCreate, notify_res, dpp);
                if (ret < 0) {
...
{
    stringstream &s = set_description();
    s << "timelog add entry oid=" <<  oid << "entry={id=" << entry.id << ", section=" << entry.section << ", name=" <<
      entry.name << "}";
