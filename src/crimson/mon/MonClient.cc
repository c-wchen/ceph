...
namespace crimson::mon {
    ...
    class Connection:public seastar::enable_shared_from_this < Connection > {
      public:
        Connection(const AuthRegistry & auth_registry,
                   crimson::net::ConnectionRef conn, KeyRing * keyring);
        ...
        auth::AuthClient::auth_request_t
            get_auth_request(const EntityName & name, uint32_t want_keys);
        ...
        int handle_auth_bad_method(uint32_t old_auth_method,
                                   ...
                                   const std::vector < uint32_t >
                                   &allowed_modes);
        tuple < CryptoKey, secret_t,
            int > handle_auth_done(uint64_t new_global_id,
      ...
      private:
        std::unique_ptr < AuthClientHandler >
            create_auth(crimson::auth::method_t, uint64_t global_id,
                        const EntityName & name, uint32_t want_keys);
        enum class request_t {
            rotating,
            general,
        };
        seastar::future < std::optional <
            auth_result_t >> do_auth_single(request_t);
    ...
    :auth_registry {
    auth_registry}, conn {
    conn}, rotating_keyring {
    std::make_unique < RotatingKeyRing > (nullptr,
                                              CEPH_ENTITY_TYPE_OSD, keyring)}
    {
    }
    ...
    seastar::future <> Connection::renew_tickets() {
        if (auth->need_tickets()) {
            ...
            return do_auth(request_t::general).then([](const auth_result_t r) {
                                                    ...
                                                    logger().
                                                    info
                                                    ("renew_tickets: ignoring failed auth reply");}
                                                    }
            ) ;
        }
        else {
    ...
    seastar::future <> Connection::renew_rotating_keyring() {
        ...
        auto ttl = std::chrono::seconds {
            static_cast <
                long >(crimson::common::local_conf()->auth_service_ticket_ttl)
        };
        auto cutoff =
            utime_t { now - std::min(std::chrono::seconds {30}, ttl / 4) };
        ...
        if ((now > last_rotating_renew_sent) &&
            (now - last_rotating_renew_sent < std::chrono::seconds {
             1}
            )) {
        ...
        return do_auth(request_t::rotating).then([](const auth_result_t r) {
                                                 ...
                                                 logger().
                                                 info
                                                 ("renew_rotating_keyring: ignoring failed auth reply");}
                                                 }
        ) ;
    }
    ...
    seastar::future < std::optional < Connection::auth_result_t >>
        ...
        return conn->send(std::move(m)).then([this] {
               ...
               switch (ret) {
case -EAGAIN:
auth_result = std::nullopt; break; case 0:
auth_result = auth_result_t::success; break; default:
               auth_result = auth_result_t::failure;
               logger().error("do_auth_single: got error {} on mon {}",
                              ret, conn->get_peer_addr()); break;}
               return auth_result;}
        ) ;
    }
    ...
    seastar::future < Connection::auth_result_t >
        ...
        return seastar::repeat_until_value([this, life_extender =
                                            shared_from_this(), what] () {
    ...
    auth::AuthClient::auth_request_t
        ...
        auth_method =[&] {
            ...
            auth_registry.get_supported_methods(conn->get_peer_type(),
                                                &methods);
            if (methods.empty()) {
    ...
    tuple < CryptoKey, Connection::secret_t, bufferlist >
        ...
        if (r == -EAGAIN) {
            ...
            return {
            session_key, connection_secret, reply};
        }
        else if (r < 0) {
    ...
    tuple < CryptoKey, Connection::secret_t, int >
        ...
        return {
        session_key, connection_secret, r};
    }
  ...
  :    want_keys {
    ...
    seastar::future <> Client::start() {
        ...
        return load_keyring().then([this] {
                                   return monmap.
                                   build_initial(crimson::common::local_conf(),
                                                 false);}
        ).then([this] {
               return authenticate();}
        ).then([this] {
    ...
    seastar::future <> Client::load_keyring() {
        if (!auth_registry.
            is_supported_method(msgr.get_mytype(), CEPH_AUTH_CEPHX)) {
            return seastar::now();
        }
        else {
    ...
    void Client::tick() {
        gate.dispatch_in_background(__func__, *this,[this] {
                                    if (active_con) {
                                    return seastar::
                                    when_all_succeed(wait_for_send_log(),
                                                     active_con->get_conn()->
                                                     send_keepalive(),
                                                     active_con->
                                                     renew_tickets(),
                                    ...
                                    assert(is_hunting());
                                    logger().info("{} continuing the hunt",
                                                  __func__);
                                    return authenticate();}
                                    }
        ) ;
    ...
    bool Client::is_hunting() const {
        ...
        gate.dispatch_in_background(__func__, *this,
                                    [this, conn, &m, &dispatched] {
                                    // we only care about these message types
                                    switch (m->get_type()) {
case CEPH_MSG_MON_MAP:
return handle_monmap(*conn, boost::static_pointer_cast < MMonMap > (m)); case CEPH_MSG_AUTH_REPLY:
return handle_auth_reply(*conn, boost::static_pointer_cast < MAuthReply > (m)); case CEPH_MSG_MON_SUBSCRIBE_ACK:
return handle_subscribe_ack(boost::static_pointer_cast < MMonSubscribeAck > (m)); case CEPH_MSG_MON_GET_VERSION_REPLY:
return handle_get_version_reply(boost::static_pointer_cast < MMonGetVersionReply > (m)); case MSG_MON_COMMAND_ACK:
return handle_mon_command_ack(boost::static_pointer_cast < MMonCommandAck > (m)); case MSG_LOGACK:
return handle_log_ack(boost::static_pointer_cast < MLogAck > (m)); case MSG_CONFIG:
return handle_config(boost::static_pointer_cast < MConfig > (m)); default:
                                    dispatched = false; return seastar::now();}
                                    }
        ) ;
        return (dispatched ? std::make_optional(seastar::now()) : std::nullopt);
    }

    void Client::ms_handle_reset(crimson::net::ConnectionRef conn,
                                 bool /* is_replace */ ) {
        gate.dispatch_in_background(__func__, *this,[this, conn] {
                                    auto found =
                                    std::find_if(pending_conns.begin(),
                                                 pending_conns.end(),
                                                 [peer_addr =
                                                  conn->
                                                  get_peer_addr()](auto & mc) {
                                                 return mc->
                                                 is_my_peer(peer_addr);}
                                    ); if (found != pending_conns.end()) {
                                    logger().warn("pending conn reset by {}",
                                                  conn->get_peer_addr());
                                    (*found)->close();
                                    ...
                                    if (active_con
                                        && active_con->is_my_peer(conn->
                                                                  get_peer_addr
                                                                  ())) {
                                    logger().warn("active conn reset {}",
                                                  conn->get_peer_addr());
    ...
    int Client::handle_auth_request(crimson::net::Connection & conn,
        ...
        AuthAuthorizeHandler *ah =
            get_auth_authorize_handler(conn.get_peer_type(),
                                       auth_method);
        if (!ah) {
    ...
    auth::AuthClient::auth_request_t
        ...
        if (conn.get_peer_type() == CEPH_ENTITY_TYPE_MON) {
            auto found =
                std::find_if(pending_conns.begin(), pending_conns.end(),
                             [peer_addr = conn.get_peer_addr()](auto & mc) {
                             return mc->is_my_peer(peer_addr);}
            );
            if (found == pending_conns.end()) {
                throw crimson::auth::error {
                "unknown connection"};
            }
            ...
            auth_registry.get_supported_modes(conn.get_peer_type(),
                                              auth_meta.auth_method, &modes);
            return {
            authorizer->protocol, modes, authorizer->bl};
        }
    }

    ceph::bufferlist Client::handle_auth_reply_more(crimson::net::
                                                    Connection & conn,
                                                    AuthConnectionMeta &
                                                    auth_meta,
                                                    const bufferlist & bl) {
    ...
    int Client::handle_auth_done(crimson::net::Connection & conn,
                                 AuthConnectionMeta & auth_meta,
                                 uint64_t global_id, uint32_t /*con_mode */ ,
                                 const bufferlist & bl) {
        if (conn.get_peer_type() == CEPH_ENTITY_TYPE_MON) {
            auto found =
                std::find_if(pending_conns.begin(), pending_conns.end(),
        ...
        if (monmap.get_addr_name(peer_addr, cur_mon)) {
            if (active_con) {
                ...
                return seastar::when_all_succeed(active_con->renew_tickets(),
                    ...
                    then_unpack([] {
                                logger().
                                info("handle_mon_map: renewed tickets");}
                );
            }
            else {
                return seastar::now();
    ...
    seastar::future <> Client::handle_auth_reply(crimson::net::
                                                 Connection & conn,
                                                 Ref < MAuthReply > m) {
        logger().info("handle_auth_reply {} returns {}: {}", conn, *m,
                      m->result);
    ...
    seastar::future <> Client::handle_mon_command_ack(Ref < MMonCommandAck > m) {
        ...
        if (auto found = std::find_if(mon_commands.begin(),
            ...
            command.result.
                set_value(std::
                          make_tuple(m->r, m->rs, std::move(m->get_data())));
    ...
    static entity_addr_t choose_client_addr(const entity_addrvec_t & my_addrs,
    ...
    seastar::future < bool > Client::reopen_session(int rank) {
        ...
        return seastar::parallel_for_each(mons,[this] (auto rank) {
                                          auto peer =
                                          choose_client_addr(msgr.get_myaddrs(),
                                                             monmap.
                                                             get_addrs(rank));
                                          ...
                                          logger().
                                          warn
                                          ("mon.{} does not have an addr compatible with me",
                                           rank); return seastar::now();}
                                          logger().info("connecting to mon.{}",
                                                        rank);
                                          auto conn =
                                          msgr.connect(peer,
                                          ...
                                          auto & mc =
                                          pending_conns.
                                          emplace_back(seastar::make_shared <
                                                       Connection >
                                                       (auth_registry, conn,
                                                        &keyring));
                                          assert(conn->get_peer_addr().
                                                 is_msgr2());
                                          return mc->authenticate_v2().
                                          then([peer, this] (auto result) {
                                               if (result ==
                                                   Connection::auth_result_t::
                                                   success) {
                                               _finish_auth(peer);}
                                               logger().
                                               debug
                                               ("reopen_session mon connection attempts complete");}
                                                             ...
                                                             logger().
                                                             error
                                                             ("mon connections failed with ep {}",
                                                              ep);
                                                             return seastar::
    ...
    Client::command_result_t
        ...
        auto & command =
            mon_commands.emplace_back(crimson::make_message < MMonCommand >
                                      (*m));
    ...
    seastar::future <> Client::on_session_opened() {
        return active_con->renew_rotating_keyring().then([this] {
                                                         ...
                                                         logger().
                                                         info
                                                         ("on_session_opened {}: connection closed",
                                                          __LINE__);
                                                         return seastar::now();}
          for (auto & m:                                pending_messages)
            ...
            .then([this] {
                  if (!active_con) {
                  logger().info("on_session_opened {}: connection closed",
                                __LINE__); return seastar::now();}
                  return seastar::parallel_for_each(mon_commands,
                                                    [this] (auto & command) {
                                                    return
                                                    send_message(crimson::
                                                                 make_message <
                                                                 MMonCommand >
                                                                 (*command.
                                                                  req));}
    ...
    bool Client::sub_want(const std::string & what, version_t start,
                          unsigned flags) {
    ...
    bool Client::sub_want_increment(const std::string & what,
                                    version_t start, unsigned flags) {
