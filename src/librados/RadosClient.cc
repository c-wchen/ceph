...
:Dispatcher(cct_->get()), cct_deleter
{
    cct,[](CephContext * p) {
        p->put();
}}
...
int librados::RadosClient::pool_requires_alignment2(int64_t pool_id, bool * req)
{
    ...
    return objecter->with_osdmap([req, pool_id] (const OSDMap & o) {
                                 ...
                                 return -ENOENT;}
                                 *req =
                                 o.get_pg_pool(pool_id)->
                                 requires_aligned_append(); return 0;}
    ) ;
}
                                                    ...
                                                    uint64_t * alignment)
{
    ...
    return objecter->with_osdmap([alignment, pool_id] (const OSDMap & o) {
                                 ...
                                 return -ENOENT;}
                                 *alignment =
                                 o.get_pg_pool(pool_id)->required_alignment();
                                 return 0;}
    ) ;
}
...
int librados::RadosClient::connect()
{
    ...
    poolctx.start(cct->_conf.get_val < std::uint64_t >
    ...
    messenger->
        set_default_policy(Messenger::Policy::
                           lossy_client(CEPH_FEATURE_OSDREPLYMUX));
    ...
    if (err) {
        ldout(cct,
              0) << conf->
            name << " initialization error " << cpp_strerror(-err) << dendl;
        shutdown();
        goto out;
    }
    ...
    err =
        monclient.authenticate(std::chrono::duration <
                               double >(conf.get_val < std::chrono::seconds >
                                        ("client_mount_timeout")).count());
    if (err) {
        ldout(cct,
              0) << conf->
            name << " authentication error " << cpp_strerror(-err) << dendl;
        shutdown();
        goto out;
    }
    ...
    mgrclient.set_mgr_optional(!get_required_monitor_features().
...
struct CB_aio_watch_flush_Complete {
    ...
    CB_aio_watch_flush_Complete(librados::RadosClient * _client,
                                librados::AioCompletionImpl * _c)
    :client(_client), c(_c) {
        c->get();
    } CB_aio_watch_flush_Complete(const CB_aio_watch_flush_Complete &) = delete;
    CB_aio_watch_flush_Complete operator =(const CB_aio_watch_flush_Complete &)
        = delete;
...
int librados::RadosClient::get_min_compatible_osd(int8_t * require_osd_release)
{
    ...
    objecter->with_osdmap([require_osd_release] (const OSDMap & o) {
                          *require_osd_release =
                          to_integer < int8_t > (o.require_osd_release);});
    return 0;
}
                                                     ...
                                                     require_min_compat_client)
{
    ...
    objecter->with_osdmap([min_compat_client,
                          ...
                          *min_compat_client =
                          to_integer < int8_t > (o.get_min_compat_client());
                          *require_min_compat_client =
                          to_integer < int8_t >
...
int librados::RadosClient::pool_list(std::list < std::pair < int64_t,
                                     string > >&v)
{
    int r = wait_for_osdmap();
    if (r < 0)
...
bool librados::RadosClient::get_pool_is_selfmanaged_snaps_mode(const std::
                                                               string & pool)
{
    bool ret = false;
...
int librados::RadosClient::pool_get_base_tier(int64_t pool_id,
                                              int64_t * base_tier)
{
    int r = wait_for_osdmap();
    if (r < 0) {
...
int librados::RadosClient::pool_delete_async(const char *name,
                                             PoolAsyncCompletionImpl * c)
{
    int r = wait_for_osdmap();
    if (r < 0)
...
std::string librados::RadosClient::get_addrs() constconst
{
                                              ...
                                              Context * on_finish)
{
    ...
    monclient.start_mon_command(cmd, inbl,
                                ...
                                (bs::error_code e, std::string
                                 && s, ceph::bufferlist && b) mutable {
                                if (outs)
                                * outs = std::move(s); if (outbl)
                                * outbl = std::move(b); if (on_finish)
                                on_finish.release()->
...
int librados::RadosClient::mgr_command(const string & name,
...
int librados::RadosClient::mon_command(string name,
                                       const vector < string > &cmd,
                                       const bufferlist & inbl,
                                       bufferlist * outbl, string * outs)
{
    ...
    if (log_last_version < m->version) {
        ...
        if (log_cb || log_cb2) {
            for (std::deque < LogEntry >::iterator it = m->entries.begin();
                ...
                ldout(cct,
                      20) << __func__ << " delivering " << ss.str() << dendl;
                if (log_cb)
                                                   ...
                                                   std::string > &metadata)
{
    ...
    if (service == "osd" ||
        service == "mds" ||
        service == "client" || service == "mon" || service == "mgr") {
...
int librados::RadosClient::service_daemon_update_status(std::map < std::string,
                                                        std::string > &&status)
{
    if (state != CONNECTED) {
        return -ENOTCONN;
...
mon_feature_t librados::RadosClient::get_required_monitor_features() const const
{
...
const char **librados::RadosClient::get_tracked_conf_keys() const const
{
