...
namespace librbd {
    ...
    MirroringWatcher < MockTestImageCtx > *MirroringWatcher <
    ...
    namespace mirror {
        ...
        namespace pool_watcher {
            ...
            RefreshImagesRequest < librbd::MockTestImageCtx >
                *RefreshImagesRequest < librbd::MockTestImageCtx >::s_instance =
                nullptr;
    ...
    namespace mirror {
        ...
        class TestMockPoolWatcher:public TestMockFixture {
          public:
            ...
            typedef pool_watcher::RefreshImagesRequest <
                librbd::MockTestImageCtx > MockRefreshImagesRequest;
            typedef librbd::MockMirroringWatcher MockMirroringWatcher;
            typedef librbd::MirroringWatcher < librbd::MockTestImageCtx >
                MirroringWatcher;
            ...
            struct MockListener:MockPoolWatcher::Listener {
                ...
                } MOCK_METHOD3(mock_handle_update,
                               void (const std::string &, const ImageIds &,
                                     const ImageIds &));
                void handle_update(const std::string & mirror_uuid, ImageIds
                                   && added_image_ids, ImageIds
                                   && removed_image_ids) override {
                    mock_handle_update(mirror_uuid, added_image_ids,
                                       removed_image_ids);
            }};
            ...
            void expect_mirroring_watcher_is_unregistered(MockMirroringWatcher &
            ...
            void expect_mirroring_watcher_register(MockMirroringWatcher &
                                                   mock_mirroring_watcher,
                                                   int r) {
            ...
            void expect_mirroring_watcher_unregister(MockMirroringWatcher &
                                                     mock_mirroring_watcher,
                                                     int r) {
            ...
            void expect_mirror_uuid_get(librados::IoCtx & io_ctx,
                ...
                EXPECT_CALL(get_mock_io_ctx(io_ctx),
                            exec(RBD_MIRRORING, _, StrEq("rbd"),
                                 StrEq("mirror_uuid_get"), _, _, _))
                    .
                    WillOnce(DoAll
                             (WithArg < 5 >
                              (Invoke([this, out_bl] (bufferlist * bl) {
                                      *bl = out_bl;})), Return(r)));
            }
        ...
        TEST_F(TestMockPoolWatcher, EmptyPool) {
            ...
            expect_mirroring_watcher_is_unregistered(mock_mirroring_watcher,
                                                     true);
            expect_mirroring_watcher_register(mock_mirroring_watcher, 0);

            MockRefreshImagesRequest mock_refresh_images_request;
            expect_refresh_images(mock_refresh_images_request, {
                                  }
                                  , 0);
        ...
        TEST_F(TestMockPoolWatcher, NotifyDuringRefresh) {
            ...
            EXPECT_CALL(mock_refresh_images_request, send())
                .
                WillOnce(Invoke
                         ([this, &mock_refresh_images_request, &image_ids,
                           &refresh_sent] () {
                          *mock_refresh_images_request.image_ids = image_ids;
                          Mutex::Locker locker(m_lock); refresh_sent = true;
                          m_cond.Signal();}
                         ));
            ...
            MirroringWatcher::get_instance().
                handle_image_updated(cls::rbd::MIRROR_IMAGE_STATE_DISABLING,
        ...
        TEST_F(TestMockPoolWatcher, Notify) {
            ...
            MirroringWatcher::get_instance().
                handle_image_updated(cls::rbd::MIRROR_IMAGE_STATE_DISABLING,
                                     "remote id 2", "global id 2");
            MirroringWatcher::get_instance().
                handle_image_updated(cls::rbd::MIRROR_IMAGE_STATE_DISABLED,
                                     "remote id 2", "global id 2");
            MirroringWatcher::get_instance().
                handle_image_updated(cls::rbd::MIRROR_IMAGE_STATE_ENABLED,
                                     "remote id 1a", "global id 1");
        ...
        TEST_F(TestMockPoolWatcher, RegisterWatcherBlacklist) {
            ...
            expect_mirroring_watcher_is_unregistered(mock_mirroring_watcher,
                                                     true);
            expect_mirroring_watcher_register(mock_mirroring_watcher,
                                              -EBLACKLISTED);
        ...
        TEST_F(TestMockPoolWatcher, GetMirrorUuidBlacklist) {
            ...
            expect_mirror_uuid_get(m_remote_io_ctx, "remote uuid",
                                   -EBLACKLISTED);
        ...
        TEST_F(TestMockPoolWatcher, RewatchBlacklist) {
            ...
            MirroringWatcher::get_instance().
                handle_rewatch_complete(-EBLACKLISTED);
            ASSERT_TRUE(mock_pool_watcher.is_blacklisted());
            ...
            EXPECT_CALL(mock_refresh_images_request, send())
                .WillOnce(Invoke([&mock_refresh_images_request] () {
                                 *mock_refresh_images_request.image_ids = {
                                 ...
                                 MirroringWatcher::get_instance().
                                 handle_rewatch_complete(0);
                                 mock_refresh_images_request.on_finish->
                                 complete(0);}));
            expect_mirror_uuid_get(m_remote_io_ctx, "remote uuid", 0);
            expect_timer_add_event(mock_threads);
        ...
        TEST_F(TestMockPoolWatcher, MirrorUuidUpdated) {
            ...
            expect_mirroring_watcher_is_unregistered(mock_mirroring_watcher,
                                                     false);
            expect_refresh_images(mock_refresh_images_request, new_image_ids,
                                  0);
            expect_mirror_uuid_get(m_remote_io_ctx, "updated uuid", 0);
            expect_listener_handle_update(mock_listener, "remote uuid", {
                                          }, image_ids);
            expect_listener_handle_update(mock_listener, "updated uuid",
                                          new_image_ids, {
                                          });
