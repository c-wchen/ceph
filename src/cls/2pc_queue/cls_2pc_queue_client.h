...
void cls_2pc_queue_init(librados::ObjectWriteOperation & op,
...
int cls_2pc_queue_get_capacity(librados::IoCtx & io_ctx,
                               const std::string & queue_name, uint64_t & size);
...
int cls_2pc_queue_reserve(librados::IoCtx & io_ctx,
                          const std::string & queue_name, uint64_t res_size,
                          uint32_t entries, cls_2pc_reservation::id_t & res_id);

// incremental listing of all entries in the queue
int cls_2pc_queue_list_entries(librados::IoCtx & io_ctx,
...
int cls_2pc_queue_list_reservations(librados::IoCtx & io_ctx,
                                    const std::string & queue_name,
...
void cls_2pc_queue_get_capacity(librados::ObjectReadOperation & op,
...
void cls_2pc_queue_reserve(librados::ObjectWriteOperation & op,
...
void cls_2pc_queue_commit(librados::ObjectWriteOperation & op,
...
void cls_2pc_queue_list_entries(librados::ObjectReadOperation & op,
                                const std::string & marker, uint32_t max,
                                bufferlist * obl, int *prval);
...
void cls_2pc_queue_list_reservations(librados::ObjectReadOperation & op,
                                     bufferlist * obl, int *prval);

int cls_2pc_queue_list_reservations_result(const librados::bufferlist & bl,
                                           cls_2pc_reservations & reservations);
...
void cls_2pc_queue_remove_entries(librados::ObjectWriteOperation & op,
