...
(_req)
{
    io_id =
        req->
        get_io_id(RGWHTTPClient::HTTPCLIENT_IO_READ | RGWHTTPClient::
                  HTTPCLIENT_IO_CONTROL);
    req->set_in_cb(this);
}
...
void RGWStreamWriteHTTPResourceCRF::write_drain_notify(uint64_t pending_size)
{
    ...
    if (is_blocked && (pending_size < PENDING_WRITES_WINDOW / 2)) {
        env->manager->io_complete(caller,
                                  req->
                                  get_io_id(RGWHTTPClient::
                                            HTTPCLIENT_IO_WRITE |
                                            RGWHTTPClient::
                                            HTTPCLIENT_IO_CONTROL));
        is_blocked = false;
...
void RGWStreamWriteHTTPResourceCRF::WriteDrainNotify::
notify(uint64_t pending_size)
{
