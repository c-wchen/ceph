...
TEST(MonCap, AllowAll)
{
    ...
    ASSERT_TRUE(cap.is_capable(NULL, CEPH_ENTITY_TYPE_MON, EntityName(),
                               "foo", "asdf", map < string, string > (), true,
                               true, true));
...
TEST(MonCap, ProfileOSD)
{
    ...
    ca.clear();
    ASSERT_FALSE(cap.is_capable(NULL, CEPH_ENTITY_TYPE_MON,
                                name, "", "config-key get", ca, true, true,
                                true));
    ...
    ASSERT_TRUE(cap.is_capable(NULL, CEPH_ENTITY_TYPE_MON,
                               ...
                               true));
    ASSERT_TRUE(cap.
                is_capable(NULL, CEPH_ENTITY_TYPE_MON, name, "",
                           "config-key get", ca, true, true, true));
    ASSERT_TRUE(cap.
                is_capable(NULL, CEPH_ENTITY_TYPE_MON, name, "",
                           "config-key get", ca, true, true, true));
    ...
    ASSERT_TRUE(cap.
                is_capable(NULL, CEPH_ENTITY_TYPE_MON, name, "",
                           "config-key put", ca, true, true, true));
    ASSERT_TRUE(cap.
                is_capable(NULL, CEPH_ENTITY_TYPE_MON, name, "",
                           "config-key set", ca, true, true, true));
    ASSERT_TRUE(cap.
                is_capable(NULL, CEPH_ENTITY_TYPE_MON, name, "",
                           "config-key exists", ca, true, true, true));
    ASSERT_TRUE(cap.
                is_capable(NULL, CEPH_ENTITY_TYPE_MON, name, "",
                           "config-key delete", ca, true, true, true));
}
...
TEST(MonCap, CommandRegEx)
{
    ...
    EntityName name;
    name.from_str("osd.123");
    ASSERT_TRUE(cap.is_capable(nullptr, CEPH_ENTITY_TYPE_OSD, name, "", "abc", { {
                               "arg", "12345abcde"}}, true, true, true));
    ASSERT_FALSE(cap.is_capable(nullptr, CEPH_ENTITY_TYPE_OSD, name, "", "abc", { {
                                "arg", "~!@#$"}}, true, true, true));
...
TEST(MonCap, ProfileBootstrapRBD)
{
    ...
    ASSERT_TRUE(cap.is_capable(nullptr, CEPH_ENTITY_TYPE_MON, name, "",
                               ...
                               true, true, true));
    ASSERT_FALSE(cap.
                 is_capable(nullptr, CEPH_ENTITY_TYPE_MON, name, "",
                            "auth get-or-create", {
                            {
                            "entity", "client.rbd"}, {
                            "caps_mon", "allow *"}, {
