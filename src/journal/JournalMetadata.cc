...
namespace journal {
     ...
     namespace {
        ...
        struct C_GetClient:public Context {
            ...
            bufferlist out_bl;
             ...
             C_GetClient(CephContext * cct, librados::IoCtx & ioctx,
                         const std::string & oid,
                         AsyncOpTracker & async_op_tracker,
                         const std::string & client_id,
                         cls::journal::Client * client, Context * on_finish)
            :cct(cct), ioctx(ioctx), oid(oid),
                async_op_tracker(async_op_tracker), client_id(client_id),
                client(client), on_finish(on_finish) {
            ...
            void send_get_client() {
                ...
                librados::AioCompletion * comp =
                    librados::Rados::aio_create_completion(this, nullptr,
                                                           &utils::
                                                           rados_state_callback
                                                           < C_GetClient,
                                                           &C_GetClient::
                                                           handle_get_client >);
        ...
        struct C_GetTag:public Context {
            ...
            bufferlist out_bl;
             ...
             C_GetTag(CephContext * cct, librados::IoCtx & ioctx,
                      const std::string & oid,
                      AsyncOpTracker & async_op_tracker, uint64_t tag_tid,
                      JournalMetadata::Tag * tag, Context * on_finish)
            :cct(cct), ioctx(ioctx), oid(oid),
                async_op_tracker(async_op_tracker), tag_tid(tag_tid), tag(tag),
                on_finish(on_finish) {
                async_op_tracker.start_op();
            } ~C_GetTag() override {
                async_op_tracker.finish_op();
        ...
        struct C_GetTags:public Context {
            ...
            bufferlist out_bl;
             ...
             C_GetTags(CephContext * cct, librados::IoCtx & ioctx,
            ...
            void send_tag_list() {
                librados::ObjectReadOperation op;
                client::tag_list_start(&op, start_after_tag_tid, MAX_RETURN,
                                       client_id, tag_class);
        ...
        struct C_FlushCommitPosition:public Context {
            ...
            Context *on_finish;
             ...
             C_FlushCommitPosition(Context * commit_position_ctx,
                                   Context * on_finish)
        ...
        struct C_AssertActiveTag:public Context {
            ...
            :cct(cct), ioctx(ioctx), oid(oid),
                async_op_tracker(async_op_tracker), client_id(client_id),
                tag_tid(tag_tid), on_finish(on_finish) {
                async_op_tracker.start_op();
            ...
            void handle_send(int r) {
                ldout(cct,
                      20) << "C_AssertActiveTag: " << __func__ << ": r=" << r <<
                    dendl;
    ...
    :RefCountedObject(NULL, 0), m_cct(NULL), m_oid(oid), m_client_id(client_id),
        m_settings(settings), m_order(0), m_splay_width(0), m_pool_id(-1),
        m_initialized(false), m_work_queue(work_queue), m_timer(timer),
        m_timer_lock(timer_lock), m_lock("JournalMetadata::m_lock"),
        m_commit_tid(0), m_watch_ctx(this), m_watch_handle(0), m_minimum_set(0),
        m_active_set(0), m_update_notifications(0), m_commit_position_ctx(NULL),
    ...
    void JournalMetadata::init(Context * on_finish) {
        ...
        on_finish = new FunctionContext([this, on_finish] (int r) {
                                        ...
                                        get_immutable_metadata(&m_order,
                                                               &m_splay_width,
                                                               &m_pool_id,
                                                               on_finish);}
        ) ;
        ...
        librados::AioCompletion * comp =
            librados::Rados::aio_create_completion(on_finish, nullptr,
    ...
    void JournalMetadata::shut_down(Context * on_finish) {
        ...
        on_finish = new FunctionContext([this, on_finish] (int r) {
                                        ldout(m_cct,
                                              20) <<
                                        "shut_down: waiting for ops" << dendl;
                                        m_async_op_tracker.
                                        wait_for_ops(on_finish);});
        on_finish = new FunctionContext([this, on_finish] (int r) {
                                        ldout(m_cct,
                                              20) << "shut_down: flushing watch"
                                        << dendl;
                                        librados::Rados rados(m_ioctx);
                                        librados::AioCompletion * comp =
                                        librados::Rados::
                                        aio_create_completion(on_finish,
                                                              nullptr,
                                                              utils::
                                                              rados_ctx_callback);
                                        r = rados.aio_watch_flush(comp);
                                        assert(r == 0); comp->release();});
        on_finish = new FunctionContext([this, on_finish] (int r) {
    ...
    void JournalMetadata::get_immutable_metadata(uint8_t * order,
                                                 ...
                                                 int64_t * pool_id,
                                                 Context * on_finish) {
        client::get_immutable_metadata(m_ioctx, m_oid, order, splay_width,
                                       pool_id, on_finish);
    }
    ...
    void JournalMetadata::allocate_tag(uint64_t tag_class,
                                       const bufferlist & data, Tag * tag,
                                       Context * on_finish) {
    ...
    void JournalMetadata::get_client(const std::string & client_id,
                                     ...
                                     Context * on_finish) {
        C_GetClient *ctx =
            new C_GetClient(m_cct, m_ioctx, m_oid, m_async_op_tracker,
                            client_id, client, on_finish);
        ctx->send();
    }

    void JournalMetadata::get_tag(uint64_t tag_tid, Tag * tag,
                                  Context * on_finish) {
    ...
    void JournalMetadata::assert_active_tag(uint64_t tag_tid,
                                            Context * on_finish) {
        Mutex::Locker locker(m_lock);
    ...
    void JournalMetadata::reserve_entry_tid(uint64_t tag_tid,
                                            uint64_t entry_tid) {
        Mutex::Locker locker(m_lock);
    ...
    bool JournalMetadata::get_last_allocated_entry_tid(uint64_t tag_tid,
        ...
        AllocatedEntryTids::const_iterator it =
            m_allocated_entry_tids.find(tag_tid);
    ...
    void JournalMetadata::handle_refresh_complete(C_Refresh * refresh, int r) {
        ...
        if (r == 0) {
            ...
            RegisteredClients::iterator it =
                refresh->registered_clients.find(client);
            if (it != refresh->registered_clients.end()) {
                ...
                lderr(m_cct) << "failed to locate client: " << m_client_id <<
                    dendl;
                r = -ENOENT;
            }
        }
    ...
    void JournalMetadata::handle_commit_position_task() {
        ...
        Context *ctx = new FunctionContext([this, commit_position_ctx] (int r){
                                           ...
                                           m_lock.Lock();
                                           assert(m_flush_commits_in_progress >
                                                  0);
                                           --m_flush_commits_in_progress;
                                           if (m_flush_commits_in_progress ==
                                               0) {
                                           std::swap(flush_commit_position_ctxs,
                                                     m_flush_commit_position_ctxs);}
    ...
    void JournalMetadata::handle_watch_reset() {
        ...
        if (r < 0) {
            if (r == -ENOENT) {
                ldout(m_cct,
                      5) << __func__ << ": journal header not found" << dendl;
            }
    ...
    void JournalMetadata::handle_watch_notify(uint64_t notify_id,
                                              uint64_t cookie) {
    ...
    void JournalMetadata::handle_watch_error(int err) {
        if (err == -ENOTCONN) {
            ...
            lderr(m_cct) << "journal watch error: " << cpp_strerror(err) <<
                dendl;
        }
    ...
    uint64_t JournalMetadata::allocate_commit_tid(uint64_t object_num,
        ...
        ldout(m_cct,
            ...
            << "object_num=" << object_num << ", " << "tag_tid=" << tag_tid <<
    ...
    void JournalMetadata::committed(uint64_t commit_tid,
        ...
        Context *stale_ctx = nullptr;
        {
            ...
            while (!m_pending_commit_tids.empty()) {
                ...
                commit_position.object_positions.emplace_front(commit_entry.
                                                               object_num,
                                                               commit_entry.
                                                               tag_tid,
                                                               commit_entry.
            ...
            ObjectPositions::iterator ob_it =
    ...
    void JournalMetadata::schedule_laggy_clients_disconnect(Context * on_finish) {
        ...
        Context *ctx = on_finish;
        {
          ...
          for (auto & c:m_registered_clients) {
                ...
                if (m_active_set >
                    object_set + m_settings.max_concurrent_object_sets) {
                    ldout(m_cct,
                          1) << __func__ << ": " << client_id <<
                    ...
                    ctx = new FunctionContext([this, client_id, ctx] (int r1) {
                                              ldout(m_cct,
                                                    10) << __func__ << ": " <<
                                              client_id <<
                                              ": flagging disconnected" <<
                                              dendl;
                                              librados::ObjectWriteOperation op;
                                              client::client_update_state(&op,
                                                                          client_id,
                                                                          cls::
                                                                          journal::
                                              ...
                                              auto comp =
                                              librados::Rados::
                                              aio_create_completion(ctx,
                                                                    nullptr,
                                                                    utils::
                                                                    rados_ctx_callback);
                                              int r =
                                              m_ioctx.aio_operate(m_oid, comp,
                                                                  &op);
                                              assert(r == 0);
                                              comp->release();});
                }
        ...
        if (ctx == on_finish) {
            ldout(m_cct,
                  20) << __func__ << ": no laggy clients to disconnect" <<
                dendl;
        }
        ctx->complete(0);
    }
    ...
    std::ostream & operator<<(std::ostream & os,
                              ...
                              RegisteredClients & clients) {
        os << "[";
        for (JournalMetadata::RegisteredClients::const_iterator c =
             clients.begin(); c != clients.end(); ++c) {
