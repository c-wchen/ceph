...
int verify_bucket_owner_or_policy(req_state *const s,
                                  const uint64_t op)
{
    auto identity_policy_res = eval_identity_or_session_policies(s, s->iam_user_policies, s->env, op,
        ...
        const uint64_t op)
{
    return (s->defer_to_bucket_acls == deferred_check \
            && verify_bucket_permission(dpp, s, bucket, user_acl, bucket_acl, bucket_policy, identity_policies, session_policies,
                                        op));
}
                                        ...
                                        const int perm)
{
    if (check_deferred_bucket_only_acl(dpp, s, user_acl, bucket_acl, RGW_DEFER_TO_BUCKET_ACLS_RECURSE, perm) ||
        check_deferred_bucket_only_acl(dpp, s, user_acl, bucket_acl, RGW_DEFER_TO_BUCKET_ACLS_FULL_CONTROL,
