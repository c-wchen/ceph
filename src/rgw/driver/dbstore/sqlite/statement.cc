                           ...
                           sqlite3 *db, std::string_view sql)
{
    ...
    int result =::sqlite3_prepare_v2(db, sql.data(), sql.size(), &stmt,
                                     nullptr);
...
void execute(const DoutPrefixProvider *dpp, sqlite3 *db,
             const char *query, sqlite3_callback callback, void *arg)
{
    char *errmsg = nullptr;
    ...
    if (ec != sqlite::errc::ok) {
        ldpp_dout(dpp,
                  1) << "query execution failed: " << errmsg << " (" << ec
                     << ")\nquery: " << query << dendl;
        throw sqlite::error(errmsg, ec);
    }
