 ...
 */
    ...
    OpSchedulerRef make_scheduler(CephContext * cct, int whoami,
  ...
  template < typename T > class ClassedOpQueueScheduler final:public OpScheduler
    {
      ...
      public:
      template < typename ... Args > ClassedOpQueueScheduler(CephContext * cct, Args && ... args):
        ...
        void enqueue(OpSchedulerItem && item) final {
            ...
            if (priority >= cutoff)
                queue.enqueue_strict(item.get_owner(), priority,
                                     std::move(item));
            else
                queue.enqueue(item.get_owner(), priority, cost,
                              std::move(item));
        }
        ...
        void enqueue_front(OpSchedulerItem && item) final {
            ...
            unsigned cost = item.get_cost();
            if (priority >= cutoff)
                queue.enqueue_strict_front(item.get_owner(),
                                           priority, std::move(item));
            else
                queue.enqueue_front(item.get_owner(),
