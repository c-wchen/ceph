...
int ClassHandler::open_all_classes()
{
    ...
    while ((pde =::readdir(dir))) {
        if (pde->d_name[0] == '.')
            continue;
        if (strlen(pde->d_name) >
            sizeof(CLS_PREFIX) - 1 + sizeof(CLS_SUFFIX) - 1
            && strncmp(pde->d_name, CLS_PREFIX, sizeof(CLS_PREFIX) - 1) == 0
            && strcmp(pde->d_name + strlen(pde->d_name) -
                      (sizeof(CLS_SUFFIX) - 1), CLS_SUFFIX) == 0) {
            char cname[PATH_MAX + 1];
            strncpy(cname, pde->d_name + sizeof(CLS_PREFIX) - 1,
                    sizeof(cname) - 1);
                                                   ...
                                                   bool check_allowed)
{
    ...
    if (iter != classes.end()) {
        ...
        if (check_allowed
            ...
            ldout(cct,
                  0) << "_get_class not permitted to load " << cname << dendl;
            return NULL;
        }
        cls = &classes[cname];
        ldout(cct,
              10) << "_get_class adding new class name " << cname << " " << cls
            << dendl;
        cls->name = cname;
        cls->handler = this;
        cls->whitelisted =
            in_class_list(cname, cct->_conf->osd_class_default_list);
    }
    return cls;
}
...
int ClassHandler::_load_class(ClassData * cls)
{
    ...
    while (p != cls->missing_dependencies.end()) {
        ...
        ldout(cct,
              10) << "_load_class " << cls->
            name << " satisfied dependency " << dc->name << dendl;
...
ClassHandler::ClassData * ClassHandler::register_class(const char *cname)
{
    ...
    ClassData *cls = _get_class(cname, false);
    ldout(cct,
          10) << "register_class " << cname << " status " << cls->
        status << dendl;
                                             ...
                                             cls_method_call_t func)
{
    ...
    ldout(handler->cct,
          10) << "register_method " << name << "." << mname << " flags " <<
        flags << " " << (void *)func << dendl;
                                                 ...
                                                 cls_method_cxx_call_t func)
{
    /* no need for locking, called under the class_init mutex */
    ldout(handler->cct,
          10) << "register_cxx_method " << name << "." << mname << " flags " <<
        flags << " " << (void *)func << dendl;
    ClassMethod & method = methods_map[mname];
...
ClassHandler::ClassFilter *
    ClassHandler::ClassData::register_cxx_filter(const std::
                                                 string & filter_name,
...
ClassHandler::ClassMethod *
    ClassHandler::ClassData::_get_method(const char *mname)
{
    map < string, ClassHandler::ClassMethod >::iterator iter =
        methods_map.find(mname);
...
void ClassHandler::ClassData::unregister_method(ClassHandler::ClassMethod *
                                                method)
{
    /* no need for locking, called under the class_init mutex */
...
void ClassHandler::ClassData::unregister_filter(ClassHandler::ClassFilter *
                                                filter)
{
    /* no need for locking, called under the class_init mutex */
