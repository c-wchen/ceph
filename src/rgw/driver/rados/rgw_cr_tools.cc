...
int RGWUserCreateCR::Request::_send_request(const DoutPrefixProvider *dpp)
{
    ...
    if (params.apply_quota) {
        ...
        if (cct->_conf->rgw_user_default_quota_max_size >= 0) {
            quota.user_quota.max_size =
                cct->_conf->rgw_user_default_quota_max_size;
            quota.user_quota.enabled = true;
        }
    ...
    return RGWUserAdminOp_User::create(dpp, store, op_state, flusher,
                                       null_yield);
}
...
_send_request(const DoutPrefixProvider *dpp)
{
    ...
    int ret =
        store->getRados()->get_bucket_info(store->svc(), user.tenant,
                                           bucket_name,
    ...
    ret = store->getRados()->create_bucket(*user_info, bucket, zonegroup_id,
                                           ...
                                           pquota_info, attrs, info, nullptr,
                                           &ep_objv, creation_time,
    ...
    ret =
        store->ctl()->bucket->link_bucket(user, bucket, info.creation_time,
                                          null_yield, dpp, false);
    if (ret && !existed && ret != -EEXIST) {
        /* if it exists (or previously existed), don't remove it! */
        int r =
            store->ctl()->bucket->unlink_bucket(user, bucket, null_yield, dpp);
        if (r < 0) {
            ldpp_dout(dpp,
