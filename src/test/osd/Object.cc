                                     ...
                                     uint64_t > &out)
{
    ...
    while (pos < limit) {
        uint64_t segment_length =
            round_up(rand() % (max_append_size - min_append_size),
                                     ...
                                     uint64_t > &out)
{
    ...
    while (pos < limit) {
        uint64_t segment_length =
            (rand() % (max_stride_size - min_stride_size)) + min_stride_size;
...
void ObjectDesc::iterator::adjust_stack()
{
    ...
    while (current != layers.end() && !current->covers(pos)) {
        ...
        if (next < cur_valid_till) {
            stack.push_front(make_pair(current, StackState {
                                       next, size}
                             )
                );
            cur_valid_till = next;
...
void ObjectDesc::update(ContentsGenerator * gen, const ContDesc & next)
{
    layers.push_front(std::pair < ceph::shared_ptr < ContentsGenerator >,
                      ContDesc > (ceph::shared_ptr < ContentsGenerator > (gen),
                                  next));
    return;
}
