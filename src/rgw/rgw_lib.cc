...
#define MIN_EXPIRE_S 120
    ...
    void RGWLibProcess::run() {
        ...
        while (!shutdown) {
            ...
            auto delay_s =
                std::max(int64_t(1),
                         std::min(int64_t(MIN_EXPIRE_S), expire_s / 2));
    ...
    void RGWLibProcess::handle_request(const DoutPrefixProvider * dpp,
                                       RGWRequest * r) {
        /*
    ...
    int RGWLibProcess::process_request(RGWLibRequest * req, RGWLibIO * io) {
        ...
        if (!op) {
            ldpp_dout(op,
                      1) << "failed to derive cognate RGWOp (invalid op?)" <<
                dendl;
            return -EINVAL;
        }
    ...
    int RGWLibFrontend::init() {
        ...
        pprocess = new RGWLibProcess(g_ceph_context, env,
                                     g_conf()->rgw_thread_pool_size, uri_prefix,
                                     conf);
        return 0;
    }
    ...
    int RGWLib::init(vector < const char *>&args) {
        ...
        init_timer.init();
        mutex.lock();
        init_timer.add_event_after(g_conf()->rgw_init_timeout,
                                   new C_InitTimeout);
        mutex.unlock();
    ...
    int RGWLib::stop() {
        ...
        const auto finalize_async_signals =[](){
            unregister_async_signal_handler(SIGUSR1,
    ...
    int RGWLibRequest::read_permissions(RGWOp * op, optional_yield y) {
        ...
        int ret =
            rgw_build_bucket_policies(op, g_rgwlib->get_driver(), get_state(),
                                      y);
        if (ret < 0) {
            ldpp_dout(op, 10) << "read_permissions (bucket policy) on "
                << get_state()->bucket << ":"
                << get_state()->object << " only_bucket=" << only_bucket()
                << " ret=" << ret << dendl;
            if (ret == -ENODATA)
                ret = -EACCES;
        }
            ...
            /* object ops */
            ret =
                rgw_build_object_policies(op, g_rgwlib->get_driver(),
                                          get_state(), op->prefetch_data(), y);
            if (ret < 0) {
