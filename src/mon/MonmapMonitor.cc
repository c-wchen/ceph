...
void MonmapMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    assert(mon->monmap->epoch + 1 == pending_map.epoch || pending_map.epoch == 1);  // special case mkfs!
    bufferlist bl;
...
class C_ApplyFeatures:public Context {
  ...
  public:
    C_ApplyFeatures(MonmapMonitor * s, const mon_feature_t & f):svc(s),
        features(f) {
...
void MonmapMonitor::apply_mon_features(const mon_feature_t & features)
{
    ...
    if (mon->get_quorum().size() < mon->monmap->size()) {
        dout(1) << __func__ << " new features " << new_features
            ...
            << new_features << " -- do not enable them!" << dendl;
        return;
    }
...
return prepare_join(op); default:
                    ...
                    stringstream ss; string rs; int err = -EINVAL;
                    map < string, cmd_vartype > cmdmap;
                    if (!cmdmap_from_json(m->cmd, &cmdmap, ss)) {
                    string rs = ss.str();
                    ...
                    string prefix;
                    cmd_getval_throws(g_ceph_context, cmdmap, "prefix", prefix);
                    MonSession * session = m->get_session(); if (!session) {
                    ...
                    assert(mon->monmap); MonMap & monmap = *mon->monmap;
                    /* Please note:
                     *
                    ...
                    entity_addr_t addr;
                    bufferlist rdata; if (!addr.parse(addrstr.c_str())) {
                    err = -EINVAL;
     ...
     */
                    ...
                    // go ahead and add
                    break;}
                    err = -EEXIST; goto reply;} while (false);
                    ...
                    if (!cmd_getval_throws
                        (g_ceph_context, cmdmap, "feature_name",
                         feature_name)) {
                    ss << "missing required feature name"; err = -EINVAL;
                    goto reply;}
                    ...
                    "' in the monmap."; err = -EPERM; goto reply;}
...
reply:
                    ...
                    MMonJoin * join = static_cast < MMonJoin * >(op->get_req());
                    dout(0) << "adding/updating " << join->
                    name << " at " << join->
                    addr << " to monitor cluster" << dendl;
                    if (pending_map.contains(join->name))
                    ...
                    void MonmapMonitor::get_health(list < pair <
                                                   health_status_t,
                                                   string > >&summary,
                                                   list < pair <
                                                   health_status_t,
                    ...
                    int actual = mon->get_quorum().size(); if (actual < max) {
                    ostringstream ss;
                    ss << (max -
                           actual) << " mons down, quorum " << mon->
                    get_quorum() << " " << mon->get_quorum_names();
                    summary.push_back(make_pair(HEALTH_WARN, ss.str()));
                    if (detail) {
                    set < int >q = mon->get_quorum(); for (int i = 0; i < max;
                                                           i++) {
                    if (q.count(i) == 0) {
                    ostringstream ss;
      ...
      return; for (auto sub:        *subs->second) {
                    ...
                    if (sub->onetime) {
                    mon->
                    with_session_map([this, sub] (MonSessionMap & session_map) {
