...
class C_DelayRead:public Context {
  ...
  public:
     C_DelayRead(MemWriteback * mwb, CephContext * cct, Context * c,
                 ceph::mutex * lock, const object_t & oid, uint64_t off,
                 uint64_t len, bufferlist * pbl, uint64_t delay_ns = 0)
  :    wb(mwb), m_cct(cct), m_con(c),
        m_delay(delay_ns * std::chrono::nanoseconds(1)), m_lock(lock),
        m_oid(oid), m_off(off), m_len(len), m_bl(pbl) {
...
class C_DelayWrite:public Context {
  ...
  public:
     C_DelayWrite(MemWriteback * mwb, CephContext * cct, Context * c,
                  ceph::mutex * lock, const object_t & oid, uint64_t off,
                  uint64_t len, const bufferlist & bl, uint64_t delay_ns = 0)
  :    wb(mwb), m_cct(cct), m_con(c),
        m_delay(delay_ns * std::chrono::nanoseconds(1)), m_lock(lock),
        m_oid(oid), m_off(off), m_len(len), m_bl(bl) {
    } void finish(int r) override {
        std::this_thread::sleep_for(m_delay);
        std::lock_guard locker {
        *m_lock};
...
MemWriteback::MemWriteback(CephContext * cct, ceph::mutex * lock,
                           uint64_t delay_ns)
...
void MemWriteback::read(const object_t & oid, uint64_t object_no,
                        ...
                        const ZTracer::Trace & parent_trace, Context * onfinish)
{
...
void MemWriteback::write_object_data(const object_t & oid, uint64_t off,
                                     uint64_t len, const bufferlist & data_bl)
{
...
int MemWriteback::read_object_data(const object_t & oid, uint64_t off,
                                   uint64_t len, bufferlist * data_bl)
{
    ...
    dout(1) << "reading " << oid << " from total size " << obj_bl.
        length() << dendl;
