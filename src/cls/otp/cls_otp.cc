...
bool otp_instance::verify(const ceph::real_time & timestamp, const string & val)
{
    ...
    if (index <= last_success) {    /* already used value */
        CLS_LOG(20, "otp, use of old token: index=%lld last_success=%lld",
                              ...
                              const otp_instance & instance)
{
    ...
    if (r < 0) {
        CLS_ERR("ERROR: %s(): failed to store key (otp id=%s, r=%d)", __func__,
...
static int remove_otp_instance(cls_method_context_t hctx, const string & id)
{
    ...
    if (r < 0) {
        CLS_ERR("ERROR: %s(): failed to remove key (otp id=%s, r=%d)", __func__,
                id.c_str(), r);
        return r;
    }
                      ...
                      bufferlist * in, bufferlist * out)
{
  ...
  for (auto entry:op.entries) {
        ...
        r = parse_seed(instance.otp.seed, instance.otp.seed_type,
                       &instance.otp.seed_bin);
        if (r < 0) {
            return r;
        }
