...
namespace cephfs {
    namespace mirror {
        ...
        :m_cct(cct),
            m_rados(rados), m_timer(new SafeTimer(cct, m_timer_lock, true)) {
            m_timer->init();
        }
        ...
        ServiceDaemon::~ServiceDaemon() {
            dout(10) << dendl;
            {
                ...
                if (m_timer_ctx != nullptr) {
                    dout(5) << ": canceling timer task=" << m_timer_ctx <<
                        dendl;
        ...
        int ServiceDaemon::init() {
            ...
            int r =
                m_rados->service_daemon_register("cephfs-mirror", instance_id,
                                                 service_metadata);
            if (r < 0) {
                return r;
            }
        ...
        void ServiceDaemon::add_filesystem(fs_cluster_id_t fscid,
        ...
        void ServiceDaemon::remove_peer(fs_cluster_id_t fscid,
                                        const Peer & peer) {
            dout(10) << ": peer=" << peer << dendl;
        ...
        void ServiceDaemon::add_or_update_fs_attribute(fs_cluster_id_t fscid,
                                                       std::string_view key,
        ...
        void ServiceDaemon::add_or_update_peer_attribute(fs_cluster_id_t fscid,
                                                         const Peer & peer,
                                                         std::string_view key,
                                                         AttributeValue value) {
            dout(10) << ": fscid=" << fscid << dendl;
        ...
        void ServiceDaemon::update_status() {
            dout(20) << ": " << m_filesystems.
                size() << " filesystem(s)" << dendl;
            ...
            ceph::JSONFormatter f;
            {
              ...
              for (auto &[fscid, filesystem]:m_filesystems) {
                  ...
                  for (auto &[attr_name, attr_value]:filesystem.
                         fs_attributes)
                    {
                  ...
                  for (auto &[peer, attributes]:filesystem.
                         peer_attributes)
                    {
            ...
            int r =
                m_rados->
                service_daemon_update_status({ {"status_json", ss.str()} });
            if (r < 0) {
