...
void MDBalancer::send_heartbeat()
{
    ...
    auto em = mds_load.emplace(std::piecewise_construct, std::forward_as_tuple(mds->get_nodeid()),
...
void MDBalancer::queue_split(const CDir *dir, bool fast)
{
    ...
    auto callback = [this, df](int r) {
        ...
        if (mds->is_stopping()) {
            ...
            dout(5) << "ignoring the " << _func_ << " callback because the MDS state is '" << ceph_mds_state_name(
                        mds->get_state()) << "'" << dendl;
            return;
        }
