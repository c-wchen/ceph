...
namespace journal {
    ...
    } template < typename ImageCtxT = ImageCtx > class Journal {
      public:
  /**
   * @verbatim
   *
   ...
   */
        ...
        static int reset(librados::IoCtx & io_ctx,
                         const std::string & image_id);

        static void is_tag_owner(ImageCtxT * image_ctx, bool * is_tag_owner,
                                 Context * on_finish);
        static void is_tag_owner(librados::IoCtx & io_ctx,
                                 std::string & image_id, bool * is_tag_owner,
        ...
        void commit_io_event_extent(uint64_t tid, uint64_t offset,
                                    uint64_t length, int r);

        void append_op_event(uint64_t op_tid, journal::EventEntry
                             && event_entry, Context * on_safe);
        ...
        uint64_t allocate_op_tid() {
            uint64_t op_tid = ++m_op_tid;
             assert(op_tid != 0);
             return op_tid;
        } void start_external_replay(journal::Replay < ImageCtxT >
                                     **journal_replay, Context * on_start);
      ...
      private:
        ...
        struct Event {
            ...
            int ret_val = 0;
             ...
             Event() {
            } Event(const Futures & _futures,
                    const IOObjectRequests & _requests, uint64_t offset,
                    size_t length, int filter_ret_val)
        ...
        struct C_OpEventSafe:public Context {
            ...
            Context *on_safe;
             ...
             C_OpEventSafe(Journal * journal, uint64_t tid,
            ...
            :journal(journal), tid(tid), op_start_future(op_start_future),
                ...
                journal->handle_op_event_safe(r, tid, op_start_future,
        ...
        void handle_op_event_safe(int r, uint64_t tid,
                                  const Future & op_start_future,
        ...
        void handle_refresh_metadata(uint64_t refresh_sequence,
                                     uint64_t tag_tid,
