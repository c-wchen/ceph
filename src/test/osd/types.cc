...
// iterate through all 4 combinations
    for (unsigned i = 0; i < 4; ++i) {
        ...
        //
        {
            ...
            past_intervals.update_type(ec_pool, compact);
  ...
  protected:
    ...
    class Thread_read_lock:public Thread {
      public:
        ...
        explicit Thread_read_lock(ObjectContext & _obc): obc(_obc) {
        } void *entry() override {
    ...
    class Thread_write_lock:public Thread {
      public:
        ...
        explicit Thread_write_lock(ObjectContext & _obc):obc(_obc) {
        } void *entry() override {
...
struct MapPredicate {
    ...
    MapPredicate(vector < pair < int, pair < PastIntervals::osd_state_t,
                 epoch_t >>> _states)
...
struct PITest:::testing::Test {
    PITest() {
      ...
      for (auto && i:intervals) {
        ...
        PI::PriorSet simple_ps = simple.get_prior_set(ec_pool,
                                                      last_epoch_started,
                                                      new
                                                      RequiredPredicate
                                                      (rec_pred), map_pred, up,
                                                      acting, nullptr);
        PI::PriorSet compact_ps =
            compact.get_prior_set(ec_pool, last_epoch_started,
                                  new RequiredPredicate(rec_pred), map_pred, up,
                                  acting, nullptr);
