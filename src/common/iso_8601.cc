    ...
    optional < real_time > from_iso_8601(const string_view s,
        ...
        auto read_digit =[end] (sriter & c) mutable {
            if (c == end) {
                           ...
                           return f;};
                           auto read_digits =
                           [&read_digit] (sriter & c, std::size_t n) {
                           auto v = 0ULL; for (auto i = 0U; i < n; ++i) {
                           auto d = read_digit(c); v = (10ULL * v) + d;}
                           return v;};
                           auto partial_date =
                           [end, ws_terminates] (sriter & c) {
                           return (c == end
                                   || (ws_terminates && std::isspace(*c)));};
                           ...
                           if (c == end || *c != q) {
                           throw std::
                           invalid_argument("Expected delimiter not found.");}
                           else {
                           ++c;}
                           }; tm t = {
                           0,   // tm_sec
                           ...
                           auto c = s.cbegin(); {
                           auto y = read_digits(c, 4); if (y < 1970) {
                           return none;}
                           t.tm_year = y - 1900;}
                           if (partial_date(c)) {
                           ...
                           return calculate(t);}
                           consume_delimiter(c, '-');
                           t.tm_mday = read_digits(c, 2); if (partial_date(c)) {
                           return calculate(t);}
                           consume_delimiter(c, 'T');
                           t.tm_hour = read_digits(c, 2); if (time_end(c)) {
                           return calculate(t);}
                           consume_delimiter(c, ':');
                           t.tm_min = read_digits(c, 2); if (time_end(c)) {
                           return calculate(t);}
                           consume_delimiter(c, ':');
                           t.tm_sec = read_digits(c, 2); if (time_end(c)) {
                           return calculate(t);}
                           consume_delimiter(c, '.');
                           auto n = 0UL;
                           ...
                           struct tm bt; gmtime_r(&sec, &bt); out.fill('0');
                           out << 1900 + bt.tm_year;
