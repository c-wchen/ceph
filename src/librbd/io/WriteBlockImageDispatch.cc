...
template < typename I >
struct WriteBlockImageDispatch <I >::
    C_BlockedWrites: public Context {
...
I >::WriteBlockImageDispatch(I *image_ctx)
...
template < typename I >
void WriteBlockImageDispatch <
I >::block_writes(Context *on_blocked)
{
    ...
    on_blocked =
        util::create_async_context_callback(*m_image_ctx, on_blocked);

    {
        ...
        if (!m_write_blocker_contexts.empty() || m_in_flight_writes > 0) {
            ldout(cct,
                  5) << "waiting for in-flight writes to complete: " <<
...
I >::wait_on_writes_unblocked(Context *on_unblocked)
{
...

    {
        ...
        if (!m_unblocked_write_waiter_contexts.empty()
            || m_write_blockers > 0) {
...
template < typename I >
bool WriteBlockImageDispatch < I >::write(AioCompletion *aio_comp,
        Extents
        && image_extents,
        bufferlist
        && bl, int op_flags,
...
template < typename I >
bool WriteBlockImageDispatch <
I >::discard(AioCompletion *aio_comp, Extents
...
template < typename I >
bool WriteBlockImageDispatch <
I >::write_same(AioCompletion *aio_comp, Extents
...
template < typename I >
bool WriteBlockImageDispatch < I >::flush(AioCompletion *aio_comp,
...
template < typename I >
void WriteBlockImageDispatch < I >::handle_finished(int r,
        uint64_t tid)
{
    auto cct = m_image_ctx->cct;
    ...
    if (m_write_blockers > 0 && m_in_flight_writes == 0) {
        ldout(cct,
              10) << "flushing all in-flight IO for blocked writes" <<
                  dendl;
        writes_blocked = true;
    }
    locker.unlock();
...
template < typename I >
bool WriteBlockImageDispatch < I >::process_io(uint64_t tid,
        DispatchResult *
        dispatch_result,
        Context **on_finish,
        Context *
        on_dispatched)
{
    ...
    ++m_in_flight_writes;
    *on_finish =
    new LambdaContext([this, tid, on_finish = *on_finish](int r) {
...
void WriteBlockImageDispatch < I >::flush_io(Context *on_finish)
{
    ...
    auto req =
        ImageDispatchSpec::create_flush(*m_image_ctx,
