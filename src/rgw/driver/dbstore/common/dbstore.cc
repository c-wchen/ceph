...
namespace rgw {
    namespace store {
        ...
        std::shared_ptr < class DBOp >
            DB::getDBOp(const DoutPrefixProvider * dpp, std::string_view Op,
        ...
        int DB::objectmapInsert(const DoutPrefixProvider * dpp, string bucket,
                                class ObjectOp * ptr) {
            map < string, class ObjectOp * >::iterator iter;
            class ObjectOp *Ob;
            ...
            if (iter != DB::objectmap.end()) {
                ...
                ldpp_dout(dpp,
                          30) << "Objectmap entry already exists for bucket(" <<
        ...
        int DB::objectmapDelete(const DoutPrefixProvider * dpp, string bucket) {
            ...
            if (iter == DB::objectmap.end()) {
                ...
                ldpp_dout(dpp,
                          20) << "Objectmap entry for bucket(" << bucket << ") "
        ...
        int DB::InitializeParams(const DoutPrefixProvider * dpp,
                                 DBOpParams * params) {
            int ret = -1;
        ...
        int DB::ProcessOp(const DoutPrefixProvider * dpp, std::string_view Op,
                          DBOpParams * params) {
            int ret = -1;
            ...
            if (!db_op) {
                ldpp_dout(dpp,
                          0) << "No db_op found for Op(" << Op << ")" << dendl;
                return ret;
            }
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In Process op Execute failed for fop(" << Op <<
                    ")" << dendl;
            }
            else {
                ldpp_dout(dpp,
                          20) << "Successfully processed fop(" << Op << ")" <<
                    dendl;
            }
        ...
        int DB::get_user(const DoutPrefixProvider * dpp,
                         ...
                         const std::string & query_str_val, RGWUserInfo & uinfo,
            ...
            if (query_str.empty() || query_str_val.empty()) {
                ldpp_dout(dpp,
                          0) << "In GetUser - Invalid query(" << query_str <<
            ...
            if (query_str == "username") {
                ...
                ldpp_dout(dpp,
                          0) << "In GetUser Invalid query string :" <<
            ...
            if (params.op.user.uinfo.access_keys.empty() ||
                ...
                ldpp_dout(dpp,
                          0) << "In GetUser - No user with query(" << query_str.
                    c_str() << "), user_id(" << uinfo.
                    user_id << ") found" << dendl;
                return -ENOENT;
            }
        ...
        int DB::store_user(const DoutPrefixProvider * dpp,
                           RGWUserInfo & uinfo, bool exclusive, map < string,
                           bufferlist > *pattrs, RGWObjVersionTracker * pobjv,
            ...
            ret =
                get_user(dpp, string("user_id"), uinfo.user_id.id, orig_info,
            ...
            if (!ret && obj_ver.ver) {
                ...
                if (pobjv && (pobjv->read_version.ver != obj_ver.ver)) {
                    ...
                    ret = -ECANCELED;
                    ldpp_dout(dpp,
                              0) << "User Read version mismatch err:(" << ret <<
                        ") " << dendl;
                    return ret;
                }
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "store_user failed with err:(" << ret << ") " <<
                    dendl;
                goto out;
            }
            ldpp_dout(dpp,
                      20) << "User creation successful - userid:(" << uinfo.
                user_id << ") " << dendl;
        ...
        int DB::remove_user(const DoutPrefixProvider * dpp,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "remove_user failed with err:(" << ret << ") "
                    << dendl;
                goto out;
            }
        ...
        int DB::get_bucket_info(const DoutPrefixProvider * dpp,
                                const std::string & query_str,
                                const std::string & query_str_val,
                                RGWBucketInfo & info, rgw::sal::Attrs * pattrs,
            ...
            if (query_str == "name") {
                ...
                ldpp_dout(dpp,
                          0) << "In GetBucket Invalid query string :" <<
                    query_str.c_str() << ") " << dendl;
                return -1;
            }
        ...
        int DB::create_bucket(const DoutPrefixProvider * dpp,
            ...
            ret =
                get_bucket_info(dpp, string("name"), "", orig_info, nullptr,
                                nullptr, nullptr);
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "create_bucket failed with err:(" << ret << ") "
                    << dendl;
                goto out;
            }
        ...
        int DB::remove_bucket(const DoutPrefixProvider * dpp,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In RemoveBucket failed err:(" << ret << ") " <<
                    dendl;
                goto out;
            }
        ...
        int DB::list_buckets(const DoutPrefixProvider * dpp,
                             const std::string & query_str, rgw_user & user,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In ListUserBuckets failed err:(" << ret << ") "
                    << dendl;
                goto out;
            }
        ...
        int DB::update_bucket(const DoutPrefixProvider * dpp,
                              const std::string & query_str,
                              RGWBucketInfo & info, bool exclusive,
                              const rgw_user * powner_id, map < std::string,
                              bufferlist > *pattrs, ceph::real_time * pmtime,
            ...
            ret =
                get_bucket_info(dpp, string("name"), "", orig_info, nullptr,
                                nullptr, &bucket_version);

            if (ret) {
                ldpp_dout(dpp,
                          0) << "Failed to read bucket info err:(" << ret <<
                    ") " << dendl;
                goto out;
            }
            ...
            if (pobjv) {
                if (pobjv->read_version.ver != bucket_version.ver) {
                    ldpp_dout(dpp,
                              0) << "Read version mismatch err:(" << ret << ") "
                        << dendl;
                    ret = -ECANCELED;
                    goto out;
                }
            ...
            if (query_str == "attrs") {
                ...
                ret = -1;
                ldpp_dout(dpp,
                          0) << "In UpdateBucket Invalid query_str : " <<
                    query_str << dendl;
                goto out;
            }
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In UpdateBucket failed err:(" << ret << ") " <<
                    dendl;
                goto out;
            }
 ...
 * max, then truncated.
 */
        int DB::Bucket::List::list_objects(const DoutPrefixProvider * dpp,
                                           int64_t max,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In ListBucketObjects failed err:(" << ret <<
                    ") " << dendl;
                goto out;
            }
          ...
          for (auto & entry:db_params.op.obj.list_entries) {
                ...
                if (!params.delim.empty()) {
                    ...
                    const int delim_pos =
                        objname.find(params.delim, params.prefix.size());
                    if (delim_pos >= 0) {
        ...
        int DB::raw_obj::InitializeParamsfromRawObj(const DoutPrefixProvider *
                                                    dpp, DBOpParams * params) {
            int ret = 0;
        ...
        int DB::Object::InitializeParamsfromObject(const DoutPrefixProvider *
                                                   dpp, DBOpParams * params) {
            int ret = 0;
        ...
        int DB::Object::get_object_impl(const DoutPrefixProvider * dpp,
            ...
            if (!ret && !params.op.obj.state.exists) {
                ldpp_dout(dpp,
                          0) << "Object(bucket:" << bucket_info.bucket.
                    name << ", Object:" << obj.key.
                    name << ") doesn't exist" << dendl;
                ret = -ENOENT;
            }
        ...
        int DB::Object::obj_omap_set_val_by_key(const DoutPrefixProvider * dpp,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "get_object_impl failed err:(" << ret << ")" <<
                    dendl;
                goto out;
            }
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In UpdateObject failed err:(" << ret << ") " <<
                    dendl;
                goto out;
            }
        ...
        int DB::Object::obj_omap_get_vals_by_keys(const DoutPrefixProvider *
                                                  dpp, const std::string & oid,
                                                  const std::set < std::string >
                                                  &keys, std::map < std::string,
                                                  bufferlist > *vals) {
            int ret = 0;
            DBOpParams params = { };
...
#define OBJ_INSTANCE_LEN 32
        ...
        int DB::Object::set_attrs(const DoutPrefixProvider * dpp,
            ...
            if (ret && !state->exists) {
                ldpp_dout(dpp,
                          0) << "get_state failed err:(" << ret << ")" << dendl;
                goto out;
            }
        ...
        int DB::raw_obj::read(const DoutPrefixProvider * dpp, int64_t ofs,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In GetObjectData failed err:(" << ret << ")" <<
                    dendl;
                return ret;
            }
        ...
        int DB::raw_obj::write(const DoutPrefixProvider * dpp, int64_t ofs,
                               int64_t write_ofs, uint64_t len,
                               bufferlist & bl) {
            int ret = 0;
            DBOpParams params = { };
            ...
            unsigned write_len =
                std::min((uint64_t) bl.length() - write_ofs, len);
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In PutObjectData failed err:(" << ret << ")" <<
                    dendl;
                return ret;
            }
        ...
        int DB::Object::list_versioned_objects(const DoutPrefixProvider * dpp,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In ListVersionedObjects failed err:(" << ret <<
                    ") " << dendl;
            }
            else {
        ...
        int DB::Object::get_obj_state(const DoutPrefixProvider * dpp,
                                      ...
                                      const rgw_obj & obj, bool follow_olh,
                                      RGWObjState ** state) {
            int ret = 0;
            ...
            if (!obj.key.instance.empty()) {
                ...
                if (params.op.obj.list_entries.size() != 0) {
                    ...
                    if (ret) {
                        ldpp_dout(dpp,
                                  0) <<
                            "get_object_impl of versioned object failed err:("
                            << ret << ")" << dendl;
                        goto out;
                    }
                }
        ...
        int DB::Object::get_state(const DoutPrefixProvider * dpp,
                                  RGWObjState ** pstate, bool follow_olh) {
            return get_obj_state(dpp, bucket_info, obj, follow_olh, pstate);
        }

        int DB::Object::Read::get_attr(const DoutPrefixProvider * dpp,
        ...
        int DB::Object::Read::prepare(const DoutPrefixProvider * dpp) {
            ...
            if (params.attrs) {
                *params.attrs = astate->attrset;
                if (cct->_conf->subsys.should_gather < ceph_subsys_rgw, 20 > ()) {
                    for (iter = params.attrs->begin();
                         iter != params.attrs->end(); ++iter) {
                        ldpp_dout(dpp,
            ...
            if (conds.if_match || conds.if_nomatch) {
                ...
                if (conds.if_match) {
                    ...
                    if (if_match_str.
                        compare(0, etag.length(), etag.c_str(),
                                etag.length()) != 0) {
                ...
                if (conds.if_nomatch) {
                    string if_nomatch_str =
                        rgw_string_unquote(conds.if_nomatch);
                    ldpp_dout(dpp, 10) << "ETag: " << string(etag.c_str(),
                                                             etag.
                                                             length()) << " " <<
        ...
        int DB::Object::Read::range_to_ofs(uint64_t obj_size, int64_t & ofs,
                                           int64_t & end) {
            if (ofs < 0) {
                ofs += obj_size;
                if (ofs < 0)
        ...
        int DB::Object::Read::read(int64_t ofs, int64_t end, bufferlist & bl,
            ...
            if (reading_from_head) {
                if (astate) {   // && astate->prefetch_data)?
                    ...
                    if (ofs < astate->data.length()) {
                        unsigned copy_len =
                            std::min((uint64_t) head_data_size - ofs, len);
            ...
            raw_obj read_obj(store, source->get_bucket_info().bucket.name,
                             astate->obj.key.name, astate->obj.key.instance,
                             astate->obj.key.ns, source->obj_id, "0.0",
                             part_num);

            read_len = len;

            ldpp_dout(dpp,
                      20) << "dbstore->read obj-ofs=" << ofs << " read_ofs=" <<
        ...
        static int _get_obj_iterate_cb(const DoutPrefixProvider * dpp,
                                       ...
                                       off_t obj_ofs, off_t len,
                                       bool is_head_obj, RGWObjState * astate,
                                       void *arg) {
            struct db_get_obj_data *d =
                static_cast < struct db_get_obj_data *>(arg);
        ...
        int DB::get_obj_iterate_cb(const DoutPrefixProvider * dpp,
            ...
            while (read_ofs < bl.length()) {
                ...
                read_len += chunk_len;
                ldpp_dout(dpp,
                          20) << "dbstore->get_obj_iterate_cb  obj-ofs=" <<
                    obj_ofs << " len=" << len << " chunk_len = " << chunk_len <<
        ...
        int DB::Object::Read::iterate(const DoutPrefixProvider * dpp,
            ...
            int r =
                source->iterate_obj(dpp, source->get_bucket_info(), state.obj,
                                    ofs, end, chunk_size, _get_obj_iterate_cb,
                                    &data);
            if (r < 0) {
        ...
        int DB::Object::iterate_obj(const DoutPrefixProvider * dpp,
                                    ...
                                    uint64_t max_chunk_size, iterate_obj_cb cb,
                                    void *arg) {
            DB *store = get_store();
            uint64_t len;
            RGWObjState *astate;
            ...
            while (ofs <= end && (uint64_t) ofs < astate->size) {
                ...
                /* XXX: Handle multipart_str */
                raw_obj read_obj(store, get_bucket_info().bucket.name,
                                 astate->obj.key.name, astate->obj.key.instance,
                                 astate->obj.key.ns, obj_id, "0.0", part_num);
                bool reading_from_head = (ofs < head_data_size);

                r = cb(dpp, read_obj, ofs, read_len, reading_from_head, astate,
                       arg);
                if (r <= 0) {
                    return r;
                }
...
/* writes tail objects */
        int DB::Object::Write::write_data(const DoutPrefixProvider * dpp,
            ...
            while (write_ofs < end) {
                ...
                /* XXX: Handle multipart_str */
                raw_obj write_obj(store, target->get_bucket_info().bucket.name,
                                  obj_state.obj.key.name,
                                  obj_state.obj.key.instance,
                                  obj_state.obj.key.ns, target->obj_id,
                                  mp_part_str, part_num);

                ldpp_dout(dpp,
                          20) << "dbstore->write obj-ofs=" << ofs <<
                    " write_len=" << len << dendl;
...
/* Write metadata & head object data */
        int DB::Object::Write::_do_write_meta(const DoutPrefixProvider * dpp,
            ...
            if (target->bucket_info.obj_lock_enabled()
                ...
                if (iter == attrs.end()) {
                    real_time lock_until_date =
                        target->bucket_info.obj_lock.get_lock_until_date(meta.
                                                                         set_mtime);
            ...
            if (meta.rmattrs) {
                for (iter = meta.rmattrs->begin(); iter != meta.rmattrs->end();
                     ++iter) {
            ...
            if (meta.manifest) {
                storage_class =
                    meta.manifest->get_tail_placement().placement_rule.
                    storage_class;
          ...
          out:
            if (ret < 0) {
                ldpp_dout(dpp,
                          0) << "ERROR: do_write_meta returned ret=" << ret <<
                    dendl;
            }
        ...
        int DB::Object::Write::write_meta(const DoutPrefixProvider * dpp,
                                          uint64_t size,
                                          uint64_t accounted_size, map < string,
                                          bufferlist > &attrs) {
            bool assume_noent = false;
            /* handle assume_noent */
            int r =
                _do_write_meta(dpp, size, accounted_size, attrs, assume_noent,
                               meta.modify_tail);
            return r;
        }
        ...
        int DB::Object::Delete::delete_obj(const DoutPrefixProvider * dpp) {
            ...
            bool versioning_enabled =
                ((params.versioning_status & BUCKET_VERSIONED) ==
                 BUCKET_VERSIONED);
            ...
            if (ret < 0 && ret != -ENOENT) {
                ldpp_dout(dpp,
                          0) << "GetObject during delete failed err:(" << ret <<
                    ")" << dendl;
                return ret;
            }
            ...
            if (!ret) {
                ...
                ret =
                    target->list_versioned_objects(dpp,
                                                   del_params.op.obj.
                                                   list_entries);
                if (ret) {
                    ldpp_dout(dpp,
                              0) << "ListVersionedObjects failed err:(" << ret
                        << ")" << dendl;
                    return ret;
                }
        ...
        int DB::Object::Delete::delete_obj_impl(const DoutPrefixProvider * dpp,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In DeleteObject failed err:(" << ret << ")" <<
                    dendl;
                return ret;
            }
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "Updating tail objects mtime failed err:(" <<
                    ret << ")" << dendl;
            }
            return ret;
        }
 ...
 */
        int DB::Object::Delete::create_dm(const DoutPrefixProvider * dpp,
            ...
            if (versioning_suspended) {
                ...
                store->gen_rand_obj_instance_name(&dm_params.op.obj.state.obj.
                                                  key);
                dm_params.op.obj.obj_id =
                    dm_params.op.obj.state.obj.key.instance;
            }

            dm_params.op.obj.flags |=
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) <<
                    "delete_olh: failed to create delete marker - err:(" << ret
                    << ")" << dendl;
                return ret;
            }
        ...
        int DB::get_entry(const std::string & oid, const std::string & marker,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In GetLCEntry failed err:(" << ret << ") " <<
                    dendl;
                goto out;
            }
            ...
            if (!params.op.lc_entry.entry.get_start_time() == 0) {  //ensure entry found
                ...
                e = new rgw::sal::StoreLifecycle::StoreLCEntry(params.op.
                                                               lc_entry.entry);
                if (!e) {
                    ret = -ENOMEM;
                    goto out;
        ...
        int DB::get_next_entry(const std::string & oid,
                               const std::string & marker,
                               std::unique_ptr < rgw::sal::Lifecycle::LCEntry >
                               *entry) {
            int ret = 0;
        ...
        int DB::set_entry(const std::string & oid,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In InsertLCEntry failed err:(" << ret << ") "
                    << dendl;
                goto out;
            }
        ...
        int DB::list_entries(const std::string & oid,
                             const std::string & marker, uint32_t max_entries,
                             std::vector < std::unique_ptr <
                             rgw::sal::Lifecycle::LCEntry >> &entries) {
            int ret = 0;
            const DoutPrefixProvider *dpp = get_def_dpp();
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In ListLCEntries failed err:(" << ret << ") "
                    << dendl;
                goto out;
            }
          ...
          for (auto & entry:params.op.lc_entry.list_entries) {
                entries.push_back(std::make_unique <
                                  rgw::sal::StoreLifecycle::StoreLCEntry >
                                  (std::move(entry)));
            }
        ...
        int DB::rm_entry(const std::string & oid,
                         rgw::sal::Lifecycle::LCEntry & entry) {
            int ret = 0;
            const DoutPrefixProvider *dpp = get_def_dpp();
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In RemoveLCEntry failed err:(" << ret << ") "
                    << dendl;
                goto out;
            }
        ...
        int DB::get_head(const std::string & oid,
                         std::unique_ptr < rgw::sal::Lifecycle::LCHead >
                         *head) {
            int ret = 0;
            const DoutPrefixProvider *dpp = get_def_dpp();
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In GetLCHead failed err:(" << ret << ") " <<
                    dendl;
                goto out;
            }

            *head =
                std::make_unique < rgw::sal::StoreLifecycle::StoreLCHead >
        ...
        int DB::put_head(const std::string & oid,
            ...
            if (ret) {
                ldpp_dout(dpp,
                          0) << "In InsertLCHead failed err:(" << ret << ") " <<
                    dendl;
                goto out;
            }
        ...
        int DB::delete_stale_objs(const DoutPrefixProvider * dpp,
                  ...
                  for (const auto & ent:buckets.
                         get_buckets())
                    {
                        ...
                        if (r < 0) {    //do nothing? skip to next entry?
                            ldpp_dout(dpp,
                                      2) <<
                                " delete_stale_objs failed for bucket( " <<
                                bname << ")" << dendl;
                        }
                        bucket_marker = bname;
                        user_marker = user.id;
