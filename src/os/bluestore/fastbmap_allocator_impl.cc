    ...
    for (auto pos = pos_start / d; pos < pos_end / d; ++pos) {
        ...
        for (auto c = 0; c < d; c++) {
            ...
            case L1_ENTRY_PARTIAL:
                ...
                longest =
                    _get_longest_from_l0(l1_pos * l0_w, (l1_pos + 1) * l0_w,
                                                          ...
                                                          uint64_t pos_end)
{
    ...
    if (unlikely(length <= l0_granularity)) {
        ...
        search_ctx_t ctx;
        _analyze_partials(pos_start, pos_end, length, min_length, NO_STOP,
                          &ctx);
        ceph_assert(ctx.fully_processed);
        // check partially free slot sets first (including neighboring),
        // full length match required.
        ...
        if (ctx.free_count) {
            ...
            if (aligned_extent.length > 0) {
                ...
                auto pos_end =
                    (aligned_extent.offset +
                     aligned_extent.length) / l0_granularity;
                                         ...
                                         interval_vector_t * res)
{
    ...
    if (min_length != l0_granularity) {
        ...
        while (length > *allocated && has_space) {
            interval_t i =
                _allocate_l1_contiguous(length - *allocated, min_length,
                                        max_length,
        ...
        for (auto idx = l1_pos_start / d1;
            ...
            if (slot_val == all_slot_clear) {
                ...
                *allocated += to_alloc;
                ...
                _fragment_and_emplace(max_length, idx * d1 * l1_granularity,
                                      to_alloc, res);
            ...
            do {
                ...
                empty = _allocate_l0(length, max_length,
                                     (idx * d1 +
                                      free_pos / L1_ENTRY_WIDTH) * l0_w,
                                     (idx * d1 + free_pos / L1_ENTRY_WIDTH +
                                      1) * l0_w, allocated, res);
                ...
                free_pos =
                    find_next_set_bit(slot_val, free_pos + L1_ENTRY_WIDTH);
...
void AllocatorLevel01Loose::collect_stats(std::map < size_t,
                                          size_t > &bins_overall)
{
...
void AllocatorLevel01Loose::foreach_internal(std::function <
        ...
        for (size_t j = 0; j < L1_ENTRIES_PER_SLOT * L1_ENTRY_WIDTH;
            ...
            case L1_ENTRY_FREE:
                if (len == 0)
                    off =
                        ((bits_per_slot * i +
                          j) / L1_ENTRY_WIDTH) * slots_per_slotset *
                        bits_per_slot;
                ...
                for (size_t t = 0; t < slots_per_slotset; t++) {
                    ...
                    while (p < bits_per_slot) {
                        if (len == 0) {
                            ...
                            ssize_t alloc_count =
                                count_0s(allocation_pattern, p);
                            p += alloc_count;
                            //now we are switched to expecting free space
                            if (p < bits_per_slot) {
                                //now @p are 1s
                                ssize_t free_count =
                                    count_1s(allocation_pattern, p);
                                assert(free_count > 0);
                                len = free_count;
