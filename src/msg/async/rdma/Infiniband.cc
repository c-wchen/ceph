...
Port::Port(CephContext * cct, struct ibv_context *ictxt,
           uint8_t ipn):ctxt(ictxt), port_num(ipn), port_attr(new ibv_port_attr)
{
#ifdef HAVE_IBV_EXP
    union ibv_gid cgid;
...
Device::Device(CephContext * cct, ibv_device * d):device(d), device_attr(new ibv_device_attr),
active_port(nullptr)
{
    if (device == NULL) {
        lderr(cct) << __func__ << " device == NULL" << cpp_strerror(errno) <<
            dendl;
        ceph_abort();
    }
    ...
    if (ctxt == NULL) {
        lderr(cct) << __func__ << " open rdma device failed. " <<
            cpp_strerror(errno) << dendl;
        ceph_abort();
    }
...
void Device::binding_port(CephContext * cct, int port_num)
{
    ...
    for (uint8_t i = 0; i < port_cnt; ++i) {
        ...
        if (i + 1 == port_num
            ...
            active_port = port;
            ldout(cct,
                  1) << __func__ << " found active port " << i + 1 << dendl;
            break;
        }
        else {
            ldout(cct,
                  10) << __func__ << " port " << i +
                1 << " is not what we want. state: " << port->get_port_attr()->
                state << ")" << dendl;
        }
        delete port;
    }
...
ib_physical_port(port), pd(infiniband.pd->pd), srq(srq), qp(NULL), txcq(txcq),
rxcq(rxcq), initial_psn(0), max_send_wr(max_send_wr), max_recv_wr(max_recv_wr),
q_key(q_key), dead(false)
{
 ...
 */
bool Infiniband::QueuePair::is_error() const const
{
    ibv_qp_attr qpa;
    ibv_qp_init_attr qpia;
...
Infiniband::MemoryManager::MemoryManager(Device * d, ProtectionDomain * p,
                                         bool hugepage)
:  device(d), pd(p)
{
...
void Infiniband::MemoryManager::register_rx_tx(uint32_t size, uint32_t rx_num,
                                               uint32_t tx_num)
{
    assert(device);
    assert(pd);
...
int Infiniband::MemoryManager::get_channel_buffers(std::vector <
                                                   Chunk * >&chunks,
                                                   size_t bytes)
{
...
Infiniband::Infiniband(CephContext * cct, const std::string & device_name,
                       uint8_t port_num)
...
void Infiniband::init()
{
    ...
    if (max_recv_wr > cct->_conf->ms_async_rdma_receive_buffers) {
        ...
        ldout(cct,
              1) << __func__ << " assigning: " << max_recv_wr <<
            " receive buffers" << dendl;
    }
    else {
        ldout(cct,
              1) << __func__ << " using the max allowed receive buffers: " <<
            max_recv_wr << dendl;
    }
    ...
    if (max_send_wr > cct->_conf->ms_async_rdma_send_buffers) {
        ...
        ldout(cct,
              1) << __func__ << " assigning: " << max_send_wr << " send buffers"
            << dendl;
    }
    else {
        ldout(cct,
              1) << __func__ << " using the max allowed send buffers: " <<
            max_send_wr << dendl;
    }

    ldout(cct,
          1) << __func__ << " device allow " << device->device_attr->
        max_cqe << " completion entries" << dendl;

    memory_manager = new MemoryManager(device, pd,
                                       cct->_conf->
                                       ms_async_rdma_enable_hugepage);
    memory_manager->register_rx_tx(cct->_conf->ms_async_rdma_buffer_size,
...
int Infiniband::post_channel_cluster()
{
    ...
    assert(r > 0);
    for (vector < Chunk * >::iterator iter = free_chunks.begin();
         iter != free_chunks.end(); ++iter) {
        r = post_chunk(*iter);
        assert(r == 0);
    }
    return 0;
...
int Infiniband::recv_msg(CephContext * cct, int sd, IBSYNMsg & im)
{
    ...
    if (r < 0) {
        ...
        sscanf(msg, "%hu:%x:%x:%x:%s", &(im.lid), &(im.qpn), &(im.psn),
               &(im.peer_qpn), gid);
        wire_gid_to_gid(gid, &(im.gid));
        ldout(cct,
              5) << __func__ << " recevd: " << im.lid << ", " << im.
            qpn << ", " << im.psn << ", " << im.
            peer_qpn << ", " << gid << dendl;
    }
    return r;
...
int Infiniband::send_msg(CephContext * cct, int sd, IBSYNMsg & im)
{
  ...
  retry:
    ...
    sprintf(msg, "%04x:%08x:%08x:%08x:%s", im.lid, im.qpn, im.psn, im.peer_qpn,
            gid);
    ldout(cct,
          10) << __func__ << " sending: " << im.lid << ", " << im.
        qpn << ", " << im.psn << ", " << im.peer_qpn << ", " << gid << dendl;
