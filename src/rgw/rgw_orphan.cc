...
int RGWOrphanStore::store_entries(const string & oid, const map < string,
                                  bufferlist > &entries)
{
    ...
    if (ret < 0) {
        lderr(store->
              ctx()) << "ERROR: " << __func__ << "(" << oid << ") returned ret="
            << ret << dendl;
    }
...
int RGWOrphanSearch::init(const string & job_name, RGWOrphanSearchInfo * info)
{
    ...
    if (r == 0) {
        ...
        if (r < 0) {
            lderr(store->
                  ctx()) << "ERROR: failed to write state ret=" << r << dendl;
            return r;
        }
    }
    else {
...
int RGWOrphanSearch::log_oids(map < int, string > &log_shards, map < int,
                              list < string > >&oids)
{
...
int RGWOrphanSearch::build_all_oids_index()
{
    ...
    int ret =
        rgw_init_ioctx(store->get_rados_handle(), search_info.pool, ioctx);
    if (ret < 0) {
        lderr(store->
              ctx()) << __func__ << ": rgw_init_ioctx() returned ret=" << ret <<
            dendl;
        return ret;
    }
...
int RGWOrphanSearch::handle_stat_result(map < int, list < string > >&oids,
...
int RGWOrphanSearch::pop_and_handle_stat_op(map < int, list < string > >&oids,
                                            std::deque <
                                            RGWRados::Object::Stat > &ops)
{
    ...
    if (ret < 0) {
        lderr(store->
              ctx()) << "ERROR: handle_stat_response() returned error: " <<
            cpp_strerror(-ret) << dendl;
    }
  done:
    ops.pop_front();
...
int RGWOrphanSearch::
build_linked_oids_for_bucket(const string & bucket_instance_id, map < int,
                             list < string > >&oids)
{
    ldout(store->ctx(),
    ...
    RGWObjectCtx obj_ctx(store);
    int ret =
        store->get_bucket_instance_info(obj_ctx, bucket_instance_id,
...
#define MAX_LIST_OBJS_ENTRIES 100
        ret =
            list_op.list_objects(MAX_LIST_OBJS_ENTRIES, &result, NULL,
                                 &truncated);
        if (ret < 0) {
        ...
        for (vector < rgw_bucket_dir_entry >::iterator iter = result.begin();
            ...
            if (entry.key.instance.empty()) {
                ldout(store->ctx(),
                      20) << "obj entry: " << entry.key.name << dendl;
            }
            else {
                ldout(store->ctx(),
                      20) << "obj entry: " << entry.key.name << " [" << entry.
                    key.instance << "]" << dendl;
            }
...
int RGWOrphanSearch::build_linked_oids_index()
{
    ...
    for (; iter != buckets_instance_index.end(); ++iter) {
        ...
        do {
            ...
            if (ret < 0) {
                lderr(store->
                      ctx()) << __func__ << ": ERROR: read_entries() oid=" <<
...
int RGWOrphanSearch::run()
{
    ...
    case ORPHAN_SEARCH_STAGE_INIT:
        ...
        if (r < 0) {
            lderr(store->
                  ctx()) << __func__ << ": ERROR: failed to save state, ret=" <<
                r << dendl;
            return r;
        }
        // fall through
