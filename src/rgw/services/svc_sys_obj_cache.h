...
class RGWSI_SysObj_Cache:public RGWSI_SysObj_Core {
    ...
    RGWSI_Notify *notify_svc {
    nullptr};
    ObjectCache cache;

     std::shared_ptr < RGWSI_SysObj_Cache_CB > cb;

    void normalize_pool_and_obj(const rgw_pool & src_pool,
                                const std::string & src_obj,
                                rgw_pool & dst_pool, std::string & dst_obj);
  protected:
    void init(RGWSI_RADOS * _rados_svc,
              RGWSI_Zone * _zone_svc, RGWSI_Notify * _notify_svc) {
    ...
    int read(const DoutPrefixProvider * dpp,
             ...
             boost::optional < obj_version >, optional_yield y) override;
    ...
    int remove(const DoutPrefixProvider * dpp,
               RGWObjVersionTracker * objv_tracker,
               const rgw_raw_obj & obj, optional_yield y) override;
    ...
    int distribute_cache(const DoutPrefixProvider * dpp,
                         ...
                         const rgw_raw_obj & obj, ObjectCacheInfo & obj_info,
                         int op, optional_yield y);
  ...
  public:
    RGWSI_SysObj_Cache(const DoutPrefixProvider * dpp,
                       CephContext * cct):RGWSI_SysObj_Core(cct), asocket(dpp,
                                                                          this)
    {
        cache.set_ctx(cct);
    }
    ...
    class ASocketHandler {
      ...
      public:
         ASocketHandler(const DoutPrefixProvider * dpp,
...
template < class T > class RGWChainedCacheImpl:public RGWChainedCache {
    ...
    std::unordered_map < std::string, std::pair < T,
        ceph::coarse_mono_time >> entries;
  ...
  RGWChainedCacheImpl():lock("RGWChainedCacheImpl::lock") {
    ...
    void init(RGWSI_SysObj_Cache * _svc) {
        ...
        expiry =
            std::chrono::seconds(svc->ctx()->_conf.get_val < uint64_t >
    ...
    bool put(const DoutPrefixProvider * dpp, RGWSI_SysObj_Cache * svc,
             const std::string & key, T * entry,
             std::initializer_list <
             rgw_cache_entry_info * >cache_info_entries) {
        if (!svc) {
            return false;
