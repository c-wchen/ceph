...
void MgrMonitor::create_pending()
{
    ...
    if (map.get_epoch() == 1 &&
        command_descs.empty() && pending_command_descs.empty()) {
...
void MgrMonitor::get_health(list < pair < health_status_t, string > >&summary,
...
void MgrMonitor::tick()
{
    ...
    const auto mgr_beacon_grace =
        std::chrono::seconds(g_conf->get_val < int64_t >
    ...
    const auto mgr_tick_period =
        std::chrono::seconds(g_conf->get_val < int64_t > ("mgr_tick_period"));
...
bool MgrMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mgr dump") {
        int64_t epoch = 0;
        cmd_getval_throws(g_ceph_context, cmdmap, "epoch", epoch,
                          (int64_t) map.get_epoch());
