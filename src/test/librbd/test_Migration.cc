...
namespace librbd {
    ...
    struct TestMigration:public TestFixture {
        static void SetUpTestCase() {
            ...
            ASSERT_EQ(0,
                      create_image_pp(m_rbd, m_ioctx, ref_image_name,
                                      m_ictx->size));
        ...
        void compare(const std::string & description = "") {
            ...
            for (size_t i = 0; i <= src_snaps.size(); i++) {
                ...
                EXPECT_EQ(0,
                          librbd::api::Image <>::snap_set(m_ref_ictx,
                                                          cls::rbd::
                                                          UserSnapshotNamespace
                                                          (), src_snap_name));
                EXPECT_EQ(0,
                          librbd::api::Image <>::snap_set(m_ictx,
                                                          cls::rbd::
                                                          UserSnapshotNamespace
                                                          (), dst_snap_name));
                compare_snaps(description + " snap: " +
                              (src_snap_name ? src_snap_name : "null"),
                              m_ref_ictx, m_ictx);
            }
        }

        void compare_snaps(const std::string & description,
            ...
            while (offset < src_size) {
                read_size =
                    std::min(read_size,
                             static_cast < ssize_t > (src_size - offset));

                bufferptr src_ptr(read_size);
                bufferlist src_bl;
                src_bl.push_back(src_ptr);
                librbd::io::ReadResult src_result {
                &src_bl};
                EXPECT_EQ(read_size,
                          api::Io <>::read(*src_ictx, offset, read_size,
                ...
                librbd::io::ReadResult dst_result {
                &dst_bl};
                EXPECT_EQ(read_size,
                          api::Io <>::read(*dst_ictx, offset, read_size,
                                           librbd::io::ReadResult {
                                           dst_result}
                                           , 0));
                ...
                if (!src_bl.contents_equal(dst_bl)) {
                    std::cout << description
                        << ", block " << offset << "~" << read_size <<
                        " differs" << std::endl;
                    std::cout << "src block: " << src_ictx->
                        id << ": " << std::endl;
                    src_bl.hexdump(std::cout);
                    std::cout << "dst block: " << dst_ictx->
                        id << ": " << std::endl;
        ...
        void migration_prepare(librados::IoCtx & dst_io_ctx,
            ...
            EXPECT_EQ(r,
                      librbd::api::Migration <>::prepare(m_ioctx, m_image_name,
                                                         dst_io_ctx, dst_name,
                                                         m_opts));
            if (r == 0) {
        ...
        void migration_execute(librados::IoCtx & io_ctx,
                               const std::string & name, int r = 0) {
            std::cout << __func__ << std::endl;

            librbd::NoOpProgressContext no_op;
            EXPECT_EQ(r,
                      librbd::api::Migration <>::execute(io_ctx, name, no_op));
        }
        ...
        void migration_commit(librados::IoCtx & io_ctx,
                              const std::string & name) {
            std::cout << __func__ << std::endl;

            librbd::NoOpProgressContext no_op;
            EXPECT_EQ(0,
                      librbd::api::Migration <>::commit(io_ctx, name, no_op));
        ...
        void migration_status(librbd::image_migration_state_t state) {
            ...
            EXPECT_EQ(0,
                      librbd::api::Migration <>::status(m_ioctx, m_image_name,
                                                        &status));
            ...
            EXPECT_EQ(status.dest_pool_namespace,
        ...
        void clone(const std::string & snap_name) {
            ...
            ASSERT_EQ(0,
                      librbd::clone(m_ref_ictx->md_ctx,
                                    m_ref_ictx->name.c_str(), snap_name.c_str(),
                                    m_ref_ioctx, ref_clone_name.c_str(),
                                    features, &order, m_ref_ictx->stripe_unit,
        ...
        template < typename L >
            ...
            BOOST_SCOPE_EXIT_TPL(&prev_clone_format) {
                _rados.conf_set("rbd_default_clone_format",
                                prev_clone_format.c_str());
            ...
            std::string clone_name = get_temp_image_name();
            ASSERT_EQ(0,
                      librbd::clone(m_ictx->md_ctx, m_ictx->name.c_str(),
                                    "snap1", m_ioctx, clone_name.c_str(),
                                    features, &order, m_ictx->stripe_unit,
                                    m_ictx->stripe_count));
            ...
            bufferptr ptr(10);
            bl.push_back(ptr);
            librbd::io::ReadResult result {
            &bl};
            ASSERT_EQ(10,
                      api::Io <>::read(*child_ictx, 0, 10,
                                       librbd::io::ReadResult {
                                       result}, 0));
            bufferlist ref_bl;
        ...
        void test_stress2(bool concurrent) {
            ...
            std::thread user([this] () {
                             ...
                             write(off, len, '1' + i);
                             off += len / 4; len /= 2; discard(off, len);}
                             flush();}) ;
    ...
    TEST_F(TestMigration, OtherNamespace) {
        ASSERT_EQ(0,
                  librbd::api::Namespace <>::create(_other_pool_ioctx, "ns1"));
        _other_pool_ioctx.set_namespace("ns1");

        migrate(_other_pool_ioctx, m_image_name);

        ASSERT_EQ(_other_pool_ioctx.get_id(), m_ictx->md_ctx.get_id());
        ASSERT_EQ(_other_pool_ioctx.get_namespace(),
                  m_ictx->md_ctx.get_namespace());
    ...
    TEST_F(TestMigration, OtherNamespaceAbortAfterExecute) {
        ASSERT_EQ(0,
                  librbd::api::Namespace <>::create(_other_pool_ioctx, "ns2"));
        ...
        _other_pool_ioctx.set_namespace("");
        ASSERT_EQ(0,
                  librbd::api::Namespace <>::remove(_other_pool_ioctx, "ns2"));
    }
    ...
    TEST_F(TestMigration, MirroringSamePool) {
        ...
        ASSERT_EQ(0,
                  librbd::api::Mirror <>::mode_set(m_ioctx,
                                                   RBD_MIRROR_MODE_IMAGE));

        ASSERT_EQ(0,
                  librbd::api::Mirror <>::image_enable(m_ictx,
    ...
    TEST_F(TestMigration, MirroringOtherPoolEnabled) {
        ...
        ASSERT_EQ(0,
                  librbd::api::Mirror <>::mode_set(m_ioctx,
                                                   RBD_MIRROR_MODE_IMAGE));
        ASSERT_EQ(0,
                  librbd::api::Mirror <>::mode_set(_other_pool_ioctx,
                                                   RBD_MIRROR_MODE_IMAGE));
    ...
    TEST_F(TestMigration, Group) {
        ...
        ASSERT_EQ(0,
                  librbd::api::Group <>::image_remove(m_ioctx, "123", m_ioctx,
                                                      name.c_str()));
    ...
    TEST_F(TestMigration, SnapsSnapAfterPrepare) {
        ...
        auto ictx =
            new librbd::ImageCtx(m_ictx->name.c_str(), "", "snap3", m_ioctx,
                                 false);
    ...
    TEST_F(TestMigration, CloneParent) {
        ...
        auto it =
            m_ictx->snap_ids.find({ cls::rbd::UserSnapshotNamespace {},
                                  "snap" });
    ...
    TEST_F(TestMigration, AbortInUseImage) {
        ...
        librbd::NoOpProgressContext no_op;
        EXPECT_EQ(-EBUSY,
                  librbd::api::Migration <>::abort(m_ioctx, m_ictx->name,
                                                   no_op));
    }
    ...
    TEST_F(TestMigration, CloneV1ParentAbortFixIncompleteChildReattach) {
        ...
        test_migrate_parent(CLONE_FORMAT,[this] (librbd::ImageCtx * child_ictx) {
                            ...
                            open_image(m_ioctx, "", src_image_id, false,
                                       librbd::OPEN_FLAG_IGNORE_MIGRATING,
                                       &src_ictx); C_SaferCond cond;
                            auto req =
                            librbd::image::AttachChildRequest <>::
                            create(child_ictx, src_ictx, src_ictx->snaps[0],
                                   nullptr, 0, CLONE_FORMAT, &cond);
                            req->send(); ASSERT_EQ(0, cond.wait());
    ...
    TEST_F(TestMigration, CloneV1ParentAbortFixParentReattach) {
        ...
        test_migrate_parent(CLONE_FORMAT,[this] (librbd::ImageCtx * child_ictx) {
                            ...
                            librbd::image::AttachChildRequest <>::
                            create(child_ictx, src_ictx, src_ictx->snaps[0],
                                   m_ictx, m_ictx->snaps[0], CLONE_FORMAT,
                                   &cond); req->send();
                            ASSERT_EQ(0, cond.wait()); close_image(src_ictx);
                            migration_abort(m_ioctx, m_image_name);});
    }
    ...
    TEST_F(TestMigration, CloneV1ParentAbortRelinkNotNeeded) {
        ...
        test_migrate_parent(CLONE_FORMAT,[this] (librbd::ImageCtx * child_ictx) {
                            ...
                            create(*child_ictx, parent_spec, parent_overlap,
                                   true, &cond); req->send();
                            ASSERT_EQ(0, cond.wait());
                            librbd::ImageCtx * src_ictx;
                            open_image(m_ioctx, "", src_image_id, false,
                                       librbd::OPEN_FLAG_IGNORE_MIGRATING,
                                       &src_ictx); C_SaferCond cond1;
                            auto req1 =
                            librbd::image::AttachChildRequest <>::
                            create(child_ictx, src_ictx, src_ictx->snaps[0],
                                   m_ictx, m_ictx->snaps[0], CLONE_FORMAT,
                                   &cond1); req1->send();
                            ASSERT_EQ(0, cond1.wait()); close_image(src_ictx);
                            migration_abort(m_ioctx, m_image_name);});
    }
