...
int KineticStore::_test_init(CephContext * c)
{
    ...
    kinetic::Status status =
        conn_factory.NewThreadsafeBlockingConnection(options, kinetic_conn, 10);
    kinetic_conn.reset();
    if (!status.ok())
        derr << __func__ << "Unable to connect to kinetic store " << options.
            host << ":" << options.port << " : " << status.ToString() << dendl;
...
int KineticStore::do_open(ostream & out, bool create_if_missing)
{
    ...
    PerfCountersBuilder plb(g_ceph_context, "kinetic", l_kinetic_first,
                            l_kinetic_last);
    ...
    for (vector < KineticOp >::iterator it = _t->ops.begin();
        ...
        if (it->type == KINETIC_OP_WRITE) {
            ...
            kinetic::KineticRecord record(data, "", "",
                                          com::seagate::kinetic::client::proto::
                                          Message_Algorithm_SHA1);
            dout(30) << "kinetic before put of " << it->key << " (" << data.
                length() << " bytes)" << dendl;
            status =
                kinetic_conn->Put(it->key, "",
                                  kinetic::WriteMode::IGNORE_VERSION, record);
...
void KineticStore::KineticTransactionImpl::set(const string & prefix,
                                               const string & k,
...
void KineticStore::KineticTransactionImpl::
...
void KineticStore::KineticTransactionImpl::rm_range_keys(const string & prefix,
                                                         const string & start,
                                                         const string & end)
{
    KeyValueDB::Iterator it = db->get_iterator(prefix);
    ...
    while (it->valid()) {
        ...
        ops.push_back(KineticOp
                      (KINETIC_OP_DELETE, combine_strings(prefix, it->key())));
        it->next();
    }
}

int KineticStore::get(const string & prefix,
...
string KineticStore::combine_strings(const string & prefix,
                                     const string & value)
{
    string out = prefix;
    out.push_back(1);
               ...
               "")
{
    ...
    while (it != kinetic::KeyRangeEnd()) {
        try {
            ...
            kinetic_status =
                kinetic::KineticStatus(kinetic::StatusCode::
                                       CLIENT_INTERNAL_ERROR, e.what());
            return;
        }
        ++it;
    }
...
int KineticStore::KineticWholeSpaceIteratorImpl::
...
bool KineticStore::KineticWholeSpaceIteratorImpl::
