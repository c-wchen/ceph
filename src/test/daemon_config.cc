...
TEST(DaemonConfig, Substitution)
{
    int ret;
    ret =
        g_ceph_context->_conf->set_val("internal_safe_to_start_threads",
                                       "false");
    ASSERT_EQ(0, ret);
    ret = g_ceph_context->_conf->set_val("host", "foo");
    ASSERT_EQ(0, ret);
    ret =
        g_ceph_context->_conf->set_val("public_network", "bar$host.baz", false);
    ASSERT_EQ(0, ret);
    g_ceph_context->_conf->apply_changes(NULL);
    char buf[128];
    memset(buf, 0, sizeof(buf));
...
TEST(DaemonConfig, SubstitutionBraces)
{
    ...
    ASSERT_EQ(0, ret);
    ret =
        g_ceph_context->_conf->set_val("public_network", "bar${host}baz",
                                       false);
    ASSERT_EQ(0, ret);
    g_ceph_context->_conf->apply_changes(NULL);
    char buf[128];
...
TEST(DaemonConfig, SubstitutionMultiple)
{
    ...
    ASSERT_EQ(0, ret);
    ret =
        g_ceph_context->_conf->set_val("keyring",
                                       "$mon_host/$cluster.keyring,$mon_host/$cluster.mon.keyring",
                                       false);
    ASSERT_EQ(0, ret);
    g_ceph_context->_conf->apply_changes(NULL);
    char buf[512];
...
TEST(DaemonConfig, InjectArgs)
{
    ...
    memset(buf, 0, sizeof(buf));
    ret =
        g_ceph_context->_conf->get_val("leveldb_max_open_files", &tmp,
                                       sizeof(buf));
    ASSERT_EQ(0, ret);
...
{
    {
        ...
        int ret =
            g_ceph_context->_conf->set_val("log_stop_at_utilization", str);
        ASSERT_EQ(-EINVAL, ret);
    }
    {
        double bad_value = 2 * (double)std::numeric_limits < float >::max();
    ...
    {
        int ret =
            g_ceph_context->_conf->set_val("log_stop_at_utilization",
                                           "not a float");
        ASSERT_EQ(-EINVAL, ret);
    }
}
