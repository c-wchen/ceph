...
void UnlinkPeerRequest < I >::refresh_image()
{
    ...
    auto ctx = create_context_callback <
               UnlinkPeerRequest < I >,
               &UnlinkPeerRequest < I >::handle_refresh_image > (this);
...
template < typename I > void UnlinkPeerRequest < I >::unlink_peer()
{
    ...
    for (auto snap_it = m_image_ctx->snap_info.find(m_snap_id);
        ...
        if (snap_it->first == m_snap_id) {
            ...
            ldout(cct,
                  15) << "located newer mirror snapshot" << dendl;
    ...
    if (r == -ENOENT) {
        ldout(cct,
              15) << "missing snapshot: snap_id=" << m_snap_id <<
                  dendl;
    ...
    if (mirror_ns == nullptr) {
        lderr(cct) << "not mirror snapshot (snap_id=" << m_snap_id
                   << ")" << dendl;
        m_image_ctx->image_lock.unlock_shared();
        finish(-EINVAL);
        return;
    ...
    if ((mirror_ns->mirror_peer_uuids.empty() ||
         (mirror_ns->mirror_peer_uuids.size() == 1 &&
          mirror_ns->mirror_peer_uuids.count(m_mirror_peer_uuid) !=
          0)) && have_newer_mirror_snapshot) {
        if (m_allow_remove) {
            m_image_ctx->image_lock.unlock_shared();
            ...
            ldout(cct,
                  15) << "skipping removal of snapshot: snap_id=" <<
                      m_snap_id << ", mirror_peer_uuid=" <<
                      m_mirror_peer_uuid << ", mirror_peer_uuids=" <<
                      mirror_ns->mirror_peer_uuids << dendl;
        }
    }
    ...
    if (mirror_ns->mirror_peer_uuids.count(m_mirror_peer_uuid) == 0) {
        ldout(cct, 15) << "no peer to unlink: snap_id=" << m_snap_id
                       ...
                       << ", mirror_peer_uuids=" << mirror_ns->
                       mirror_peer_uuids << dendl;
        m_image_ctx->image_lock.unlock_shared();
        finish(0);
        return;
    }
    ...
    librbd::cls_client::mirror_image_snapshot_unlink_peer(&op,
            m_snap_id,
            m_mirror_peer_uuid);
    auto aio_comp =
        create_rados_callback < UnlinkPeerRequest < I >,
        &UnlinkPeerRequest < I >::handle_unlink_peer > (this);
    r = m_image_ctx->md_ctx.aio_operate(m_image_ctx->header_oid,
                                        aio_comp, &op);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
void UnlinkPeerRequest < I >::handle_unlink_peer(int r)
{
    ...
    if (r == -ERESTART || r == -ENOENT) {
        if (r == -ERESTART) {
            ldout(cct,
                  15) << "unlinking last peer not supported" <<
                      dendl;
            m_allow_remove = true;
        }
        refresh_image();
    ...
    if (r < 0) {
        lderr(cct) << "failed to unlink peer: " << cpp_strerror(r)
                   << dendl;
        finish(r);
        return;
    }
...
void UnlinkPeerRequest < I >::notify_update()
{
    ...
    auto ctx = create_context_callback <
               UnlinkPeerRequest < I >,
               &UnlinkPeerRequest < I >::handle_notify_update > (this);
...
void UnlinkPeerRequest < I >::handle_notify_update(int r)
{
    ...
    if (r == -ENOENT || r == -ETIMEDOUT) {
        // non-fatel errors
        lderr(cct) << "failed to notify update: " << cpp_strerror(r)
                   << dendl;
    } else if (r < 0) {
        lderr(cct) << "failed to notify update: " << cpp_strerror(r)
                   << dendl;
...
template < typename I >
void UnlinkPeerRequest <
I >::remove_snapshot(const cls::rbd::
                     ...
                     const std::string &snap_name)
{
    ...
    auto ctx = create_context_callback <
               UnlinkPeerRequest < I >,
               &UnlinkPeerRequest < I >::handle_remove_snapshot > (this);
    m_image_ctx->operations->snap_remove(snap_namespace, snap_name,
                                         ctx);
}
