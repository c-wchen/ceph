...
TEST_F(TestClient, AlternateNameSymlink)
{
    ...
    ASSERT_EQ(0,
              client->symlink(file.c_str(), file2.c_str(), myperm, altname2));
...
TEST_F(TestClient, AlternateNameRenameExistMatch)
{
    ...
    fd = client->open(file2.c_str(), O_CREAT | O_WRONLY, myperm, 0777,
                      altname2);
    ASSERT_LE(0, fd);
    ASSERT_EQ(3, client->write(fd, "baz", 3, 0));
    ASSERT_EQ(0, client->close(fd));
...
TEST_F(TestClient, AlternateNameRenameExistMisMatch)
{
    ...
    ASSERT_EQ(-EINVAL,
              client->rename(file.c_str(), file2.c_str(), myperm, altname2));
