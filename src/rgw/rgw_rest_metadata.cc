...
void RGWOp_Metadata_Get::execute(optional_yield y)
{
    ...
    auto meta_mgr =
        static_cast < rgw::sal::RadosStore * >(driver)->ctl()->meta.mgr;
...
void RGWOp_Metadata_List::execute(optional_yield y)
{
    ...
    do {
        ...
        op_ret =
            driver->meta_list_keys_next(this, handle, left, keys, &truncated);
        if (op_ret < 0) {
            ldpp_dout(this,
                      5) << "ERROR: lists_keys_next(): " << cpp_strerror(op_ret)
                         << dendl;
            return;
        }
...
void RGWOp_Metadata_Put::execute(optional_yield y)
{
    ...
    op_ret =
        static_cast <
        rgw::sal::RadosStore * >(driver)->ctl()->meta.mgr->put(metadata_key, bl,
            s->yield, s,
            sync_type, false,
            &ondisk_version);
    if (op_ret < 0) {
...
void RGWOp_Metadata_Delete::execute(optional_yield y)
{
    ...
    op_ret =
        static_cast <
        rgw::sal::RadosStore * >(driver)->ctl()->meta.mgr->remove(metadata_key,
            s->yield, s);
    if (op_ret < 0) {
        ldpp_dout(s,
