...
int RGWPutObj_Compress::handle_data(bufferlist & bl, off_t ofs, void **phandle,
...
RGWGetObj_Decompress::RGWGetObj_Decompress(CephContext * cct_, RGWCompressionInfo * cs_info_, bool partial_content_, RGWGetDataCB * next):RGWGetObj_Filter(next),
cct(cct_),
cs_info(cs_info_),
                                      ...
                                      off_t bl_len)
{
    ...
    while (first_block <= last_block) {
        ...
        while (out_bl.length() - q_ofs >= cct->_conf->rgw_max_chunk_size) {
            ...
            if (r < 0) {
                lderr(cct) << "handle_data failed with exit code " << r <<
                    dendl;
                return r;
            }
