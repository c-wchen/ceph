...
class Locker {
  ...
  public:
    ...
    void eval_gather(SimpleLock * lock, bool first = false, bool * need_issue =
                     0, list < MDSInternalContextBase * >*pfinishers = 0);
    void eval(SimpleLock * lock, bool * need_issue);
    void eval_any(SimpleLock * lock, bool * need_issue,
                  list < MDSInternalContextBase * >*pfinishers = 0, bool first =
                  false) {
        if (!lock->is_stable())
    ...
    bool rdlock_start(SimpleLock * lock, MDRequestRef & mut, bool as_anon =
                      false);
    void rdlock_finish(SimpleLock * lock, MutationImpl * mut,
                       bool * pneed_issue);
    ...
    bool wrlock_start(SimpleLock * lock, MDRequestRef & mut, bool nowait =
                      false);
    void wrlock_finish(SimpleLock * lock, MutationImpl * mut,
                       bool * pneed_issue);

    void remote_wrlock_start(SimpleLock * lock, mds_rank_t target,
                             MDRequestRef & mut);
    void remote_wrlock_finish(SimpleLock * lock, mds_rank_t target,
                              MutationImpl * mut);

    bool xlock_start(SimpleLock * lock, MDRequestRef & mut);
    void _finish_xlock(SimpleLock * lock, client_t xlocker, bool * pneed_issue);
    void xlock_finish(SimpleLock * lock, MutationImpl * mut,
                      bool * pneed_issue);
  ...
  public:
    ...
    void scatter_tick();
    void scatter_nudge(ScatterLock * lock, MDSInternalContextBase * c,
  ...
  public:
    ...
    void get_late_revoking_clients(std::list < client_t > *result,
  ...
  protected:
    ...
    void _update_cap_fields(CInode * in, int dirty, MClientCaps * m,
    ...
    bool _do_cap_update(CInode * in, Capability * cap, int dirty,
                        snapid_t follows, MClientCaps * m, MClientCaps * ack =
                        0, bool * need_flush = NULL);
  ...
  public:
    ...
    Capability *issue_new_caps(CInode * in, int mode, Session * session,
                               SnapRealm * conrealm, bool is_replay);
  ...
  protected:
    ...
    void file_update_finish(CInode * in, MutationRef & mut, bool share_max,
  ...
  public:
    ...
    bool check_inode_max_size(CInode * in, bool force_wrlock =
                              false, uint64_t newmax = 0, uint64_t newsize =
                              0, utime_t mtime = utime_t());
