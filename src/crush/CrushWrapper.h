...
class CrushWrapper {
  ...
  private:
    ...
    void build_rmaps() const {
        ...
        r.clear();
        for (std::map < int, string >::const_iterator p = f.begin();
             p != f.end(); ++p)
            r[p->second] = p->first;
  } public:
   ...
   */
    bool check_item_loc(CephContext * cct, int item, const map < string,
                        string > &loc, int *iweight);
    bool check_item_loc(CephContext * cct, int item, const map < string,
                        string > &loc, float *weight) {
        int iweight;
   ...
   */
     ...
     */
    int get_full_location_ordered(int id, vector < pair < string,
                                  string > >&path);
   ...
   */
    int verify_upmap(CephContext * cct,
                     int rule_id, int pool_size, const vector < int >&up);
   ...
   * @return 0 for success, negative on error
   */
    int move_bucket(CephContext * cct, int id, const map < string,
                    string > &loc);
   ...
   * @return 0 for success, negative on error
   */
    int link_bucket(CephContext * cct, int id, const map < string,
                    string > &loc);
   ...
   */
    int update_item(CephContext * cct, int id, float weight, string name,
                    const map < string, string > &loc);
   ...
   */
    int create_or_move_item(CephContext * cct, int item, float weight,
                            string name, const map < string, string > &loc);
   ...
   */
    ...
    int add_simple_rule(string name, string root_name,
                        string failure_domain_type, string device_class,
                        string mode, int rule_type, ostream * err = 0);

  /**
   * @param rno rule[set] id to use, -1 to pick the lowest available
   */
    int add_simple_rule_at(string name, string root_name,
                           string failure_domain_type, string device_class,
  ...
  public:
    ...
    int update_device_class(int id, const string & class_name,
    ...
    int populate_classes(const std::map < int32_t, map < int32_t,
                         int32_t >> &old_class_bucket);
   ...
   */
    ...
    void create_choose_args(int64_t id, int positions) {
        ...
        for (int bidx = 0; bidx < crush->max_buckets; ++bidx) {
            ...
            if (b && b->alg == CRUSH_BUCKET_STRAW2) {
                ...
                carg.weight_set =
                    (crush_weight_set *) calloc(sizeof(crush_weight_set),
    ...
    template < typename WeightVector >
        ...
        for (int i = 0; i < numrep; i++)
             out[i] = rawout[i];
                             ...
                             set < int >&used,
                             vector < int >*pw, int root_bucket) const;
    ...
    void dump_tree(Formatter * f, const CrushTreeDumper::name_map_t & ws) const;
