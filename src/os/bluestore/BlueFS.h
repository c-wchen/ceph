...
class BlueFSVolumeSelector {
  public:
    ...
    virtual BlueFSVolumeSelector *clone_empty() const {
        return nullptr;
    } virtual bool compare(BlueFSVolumeSelector * other) {
        return true;
    };
};
...
class BlueFS {
  public:
    ...
    struct FileWriter {
      ...
      public:
        ...
        } ceph::bufferlist flush_buffer(CephContext * cct,
                                        const bool partial,
      ...
      public:
        ...
        :file(std::move(f)),
            buffer_appender(buffer.
                            get_page_aligned_appender(g_conf()->
                                                      bluefs_alloc_size /
                                                      CEPH_PAGE_SIZE)) {
            ++file->num_writers;
            iocv.fill(nullptr);
        ...
        // NOTE: caller must call BlueFS::close_writer() ~FileWriter() {
            --file->num_writers;
        }
        ...
        void append(ceph::buffer::list & bl) {
            uint64_t l0 = get_buffer_length();
            ceph_assert(l0 + bl.length() <= std::numeric_limits <
                        unsigned >::max());
  ...
  private:
    ...
    std::atomic < uint64_t > inject_read_zeros {
    0};
    ...
    void _release_pending_allocations(std::vector < interval_set <
                                      uint64_t >> &to_release);
    ...
    void _compact_log_dump_metadata_NF(uint64_t start_seq,
                                       bluefs_transaction_t * t,
                                       int flags, uint64_t capture_before_seq);
    ...
    int _check_allocations(const bluefs_fnode_t & fnode, boost::dynamic_bitset < uint64_t > *used_blocks, bool is_alloc,    //true when allocating, false when deallocating
                           const char *op_name);
  ...
  public:
    ...
    int open_for_write(std::string_view dir,
                       std::string_view file, FileWriter ** h, bool overwrite);

    int open_for_read(std::string_view dir,
...
class OriginalVolumeSelector:public BlueFSVolumeSelector {
  ...
  public:
     OriginalVolumeSelector(uint64_t _wal_total,
                            uint64_t _db_total, uint64_t _slow_total)
...
class FitToFastVolumeSelector:public OriginalVolumeSelector {
  public:
    FitToFastVolumeSelector(uint64_t _wal_total,
                            uint64_t _db_total, uint64_t _slow_total)
