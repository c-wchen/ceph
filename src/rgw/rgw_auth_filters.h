      ...
      template < typename T > class ThirdPartyAccountApplier:public DecoratedApplier < T >
        {
          ...
          public:
            ...
            :DecoratedApplier < T > (std::move(decoratee)),
                store(store), acct_user_override(acct_user_override) {
            }
 ...
 * of the default construction. */
        ...
        template < typename T >
            ...
            void ThirdPartyAccountApplier <
            T >::load_acct_info(RGWUserInfo & user_info) const {
            if (UNKNOWN_ACCT == acct_user_override) {
                ...
                const int ret =
                    rgw_get_user_info_by_uid(store, acct_user_override,
                                             user_info);
                if (ret < 0) {
      ...
      template < typename T > class SysReqApplier:public DecoratedApplier < T >
        {
          ...
          public:
            template < typename U > SysReqApplier(CephContext * const cct,
                                                  ...
                                                  const req_state * const s, U
                                                  && decoratee)
            :DecoratedApplier < T > (std::forward < T > (decoratee)), cct(cct),
                store(store), args(s->info.args),
            ...
            if (is_system) {
                ...
                if (!effective_uid.empty()) {
                    ...
                    if (rgw_get_user_info_by_uid
                        (store, effective_uid, euser_info) < 0) {
        ...
        template < typename T >
            void SysReqApplier <
            T >::modify_request_state(req_state * const s) const {
