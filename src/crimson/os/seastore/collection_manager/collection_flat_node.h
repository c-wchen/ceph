...
struct coll_map_t: base_coll_map_t {
    ...
    {
        return emplace(std::make_pair(denc_coll_t {
            coll}
        , bits)
                      );
...
public:
    ...
    } void insert(coll_t coll, uint32_t bits)
    {
        buffer.push_back(delta_t {
            delta_t::op_t::INSERT, denc_coll_t(coll), bits});
    } void update(coll_t coll, uint32_t bits)
    {
        buffer.push_back(delta_t {
            delta_t::op_t::UPDATE, denc_coll_t(coll), bits});
    }
    void remove(coll_t coll)
    {
        buffer.push_back(delta_t {
            delta_t::op_t::REMOVE, denc_coll_t(coll), 0});
    }
...
struct CollectionNode
    ...
    template < typename ... T > CollectionNode(T && ... t)
...
template <> struct fmt::formatter <
    crimson::os::seastore::collection_manager::CollectionNode > : fmt::
    ostream_formatter {
};
#endif
