    ...
    class RGWFileHandle:public cohort::lru::Object {
        ...
        struct directory {
            ...
            struct timespec last_readdir;
             ...
             directory():flags(FLAG_NONE), last_readdir {
            0, 0} {
            }
        };
...
#define CREATE_FLAGS(x) \
    ((x) & ~(RGWFileHandle::FLAG_CREATE|RGWFileHandle::FLAG_LOCK))
        ...
        static constexpr uint32_t RCB_MASK =
            RGW_SETATTR_MTIME | RGW_SETATTR_CTIME | RGW_SETATTR_ATIME |
            RGW_SETATTR_SIZE;
      ...
      :    
        ...
        variant_type {
        directory()}, depth(0), flags(FLAG_NONE) {
            fh.fh_hk.bucket = 0;
            fh.fh_hk.object = 0;
            /* root */
        ...
        friend void encode(const RGWFileHandle & c,::ceph::buffer::list & bl,
                           uint64_t features);
        friend void decode(RGWFileHandle & c,::ceph::bufferlist::
                           const_iterator & p);
      public:
        ...
        uint32_t get_owner_uid() const {
            ...
            return state.mtime;
        } const ceph::buffer::list & get_etag() const {
            return etag;
        } const ceph::buffer::list & get_acls() const {
            return acls;
        ...
        int offset_of(const std::string & name, int64_t * offset,
                      uint32_t flags) {
            if (unlikely(!is_dir())) {
                return -EINVAL;
            }
        ...
        void encode_attrs(ceph::buffer::list & ux_key1,
                          ceph::buffer::list & ux_attrs1, bool inc_ov = true);
        ...
        struct FhLT {
            ...
            bool operator() (const RGWFileHandle & lhs,
                             const RGWFileHandle & rhs)const {
                return (lhs.get_key() < rhs.get_key());
            }
            // for external search by fh_key
                bool operator() (const fh_key & k,
                                 const RGWFileHandle & fh)const {
                return k < fh.get_key();
...
#if defined(FHCACHE_AVL)
        typedef bi::avltree < RGWFileHandle, bi::compare < FhLT >,
            FhHook > FHTree;
#else
        typedef bi::rbtree < RGWFileHandle, bi::compare < FhLT >,
            FhHook > FhTree;
#endif
    ...
    class RGWLibFS {
      ...
      public:
        ...
        int authorize(const DoutPrefixProvider * dpp, rgw::sal::Driver * driver) {
            int ret =
                driver->get_user_by_access_key(dpp, key.id, null_yield, &user);
            if (ret == 0) {
                    ...
                    if (user->load_user(dpp, null_yield) < 0) {
                        ...
                        if (ret < 0) {
                            lsubdout(get_context(), rgw, 10)
                                <<
                                "NOTICE: failed to driver new user's info: ret="
                                << ret << dendl;
                        }
                    }
                }               /* auth success */
        ...
        LookupFHResult lookup_fh(RGWFileHandle * parent, const char *name,
          ...
          retry:
            ...
            if (fh) {
                ...
                fh = static_cast < RGWFileHandle * >(fh_lru.insert(&prototype,
                                                                   cohort::lru::
                                                                   Edge::MRU,
                                                                   iflags));
                if (fh) {
                    ...
                    if (likely(!(iflags & cohort::lru::FLAG_RECYCLE))) {
                        ...
                        fh_cache.insert_latched(fh, lat,
                                                RGWFileHandle::FHCache::
                                                FLAG_UNLOCK);
                    }
                    else {
                        /* recycle step invalidates Latch */
                        fh_cache.insert(fhk.fh_hk.object, fh,
        ...
        int setxattrs(RGWFileHandle * rgw_fh, rgw_xattrlist * attrs,
                      uint32_t flags);

        int rmxattrs(RGWFileHandle * rgw_fh, rgw_xattrlist * attrs,
                     uint32_t flags);

        void update_fh(RGWFileHandle * rgw_fh);

        LookupFHResult stat_bucket(RGWFileHandle * parent, const char *path,
                                   RGWLibFS::BucketStats & bs, uint32_t flags);
        ...
        MkObjResult create(RGWFileHandle * parent, const char *name,
        ...
        MkObjResult mkdir(RGWFileHandle * parent, const char *name,
                          struct stat *st, uint32_t mask, uint32_t flags);
        ...
        void update_user(const DoutPrefixProvider * dpp) {
            (void)g_rgwlib->get_driver()->get_user_by_access_key(dpp, key.id,
                                                                 null_yield,
                                                                 &user);
        }
    ...
    class RGWListBucketsRequest:public RGWLibRequest, public RGWListBuckets /* RGWOp */
    {
      public:
        ...
        bool rcb_eof;           // caller forced early stop in readdir cycle
         ...
         RGWListBucketsRequest(CephContext * _cct,
                               std::unique_ptr < rgw::sal::User > _user,
                               RGWFileHandle * _rgw_fh, rgw_readdir_cb _rcb,
                               void *_cb_arg,
                               RGWFileHandle::readdir_offset & _offset)
        :RGWLibRequest(_cct, std::move(_user)), rgw_fh(_rgw_fh),
            offset(_offset), cb_arg(_cb_arg), rcb(_rcb), ioff(nullptr), ix(0),
            d_count(0), rcb_eof(false) {
        ...
        int operator() (const std::string_view & name,
            ...
            rgw_fh->add_marker(off, rgw_obj_key {
                               marker.data(), ""}
                               , RGW_FS_TYPE_DIRECTORY);
            ++d_count;
            return rcb(name.data(), cb_arg, off, nullptr, 0,
                       RGW_LOOKUP_FLAG_DIR);
        }
    ...
    class RGWReaddirRequest:public RGWLibRequest, public RGWListBucket  /* RGWOp */
    {
      public:
        ...
        bool rcb_eof;           // caller forced early stop in readdir cycle
         ...
         RGWReaddirRequest(CephContext * _cct,
                           std::unique_ptr < rgw::sal::User > _user,
                           RGWFileHandle * _rgw_fh, rgw_readdir_cb _rcb,
                           void *_cb_arg,
                           RGWFileHandle::readdir_offset & _offset)
        ...
        int operator() (const std::string_view name, const rgw_obj_key & marker,
                        const ceph::real_time & t, const uint64_t fsz,
                        uint8_t type) {
        ...
        void send_response() override {
            ...
            class DirIterator {
              ...
              public:
                ...
                void parse_cp() {
                    if (is_cp()) {
                        ...
                        if (cp_iter->first.back() == '/')
                            const_cast <
                                std::string & >(cp_iter->first).pop_back();
                ...
                bool entry_is_obj() {
                    return (is_obj() &&
                            ((!is_cp()) || (obj_sref.get() < cp_sref.get())));
                }
            ...
            for (;;) {
                ...
                if (di.entry_is_obj()) {
                    ...
                    if (sref.empty()) {
                        ...
                        if (!this->
                            operator()(sref, next_marker, obj_entry.meta.mtime,
    ...
    class RGWRMdirCheck:public RGWLibRequest, public RGWListBucket  /* RGWOp */
    {
      public:
        ...
        bool has_children;
         ...
         RGWRMdirCheck(CephContext * _cct,
                       std::unique_ptr < rgw::sal::User > _user,
    ...
    class RGWCreateBucketRequest:public RGWLibRequest, public RGWCreateBucket   /* RGWOp */
    {
      public:
        const std::string & bucket_name;
         ...
         RGWCreateBucketRequest(CephContext * _cct,
                                std::unique_ptr < rgw::sal::User > _user,
                                std::string & _bname)
        ...
        int get_params(optional_yield) override {
            ...
            int ret =
                s3policy.create_canned(state->owner, state->bucket_owner,
                                       state->canned_acl);
            policy = s3policy;
            return ret;
        }
    ...
    class RGWDeleteBucketRequest:public RGWLibRequest, public RGWDeleteBucket   /* RGWOp */
    {
      public:
        const std::string & bucket_name;
         ...
         RGWDeleteBucketRequest(CephContext * _cct,
                                std::unique_ptr < rgw::sal::User > _user,
                                std::string & _bname)
        :RGWLibRequest(_cct, std::move(_user)), bucket_name(_bname) {
            op = this;
    ...
    class RGWPutObjRequest:public RGWLibRequest, public RGWPutObj   /* RGWOp */
    {
      public:
        ...
        size_t bytes_written;
         ...
         RGWPutObjRequest(CephContext * _cct,
                          std::unique_ptr < rgw::sal::User > _user,
                          const std::string & _bname,
                          const std::string & _oname, buffer::list & _bl)
        :RGWLibRequest(_cct, std::move(_user)), bucket_name(_bname),
            obj_name(_oname), bl(_bl), bytes_written(0) {
            op = this;
        } bool only_bucket() override {
            return true;
        }
    ...
    class RGWReadRequest:public RGWLibRequest, public RGWGetObj /* RGWOp */
    {
      public:
        ...
        bool do_hexdump = false;
         ...
         RGWReadRequest(CephContext * _cct,
                        std::unique_ptr < rgw::sal::User > _user,
                        RGWFileHandle * _rgw_fh, uint64_t off, uint64_t len,
                        void *_ulp_buffer)
        :RGWLibRequest(_cct, std::move(_user)), rgw_fh(_rgw_fh),
            ulp_buffer(_ulp_buffer), nread(0), read_resid(len) {
            op = this;
        ...
        int send_response_data(ceph::buffer::list & bl, off_t bl_off,
          ...
          for (auto & bp:bl.buffers()) {
                ...
                memcpy(static_cast < char *>(ulp_buffer) + nread,
                       bp.c_str() + bl_off, bytes);
...
*/
    ...
    class RGWDeleteObjRequest:public RGWLibRequest, public RGWDeleteObj /* RGWOp */
    {
      public:
        ...
        const std::string & obj_name;
         ...
         RGWDeleteObjRequest(CephContext * _cct,
                             std::unique_ptr < rgw::sal::User > _user,
                             const std::string & _bname,
    ...
    class RGWStatObjRequest:public RGWLibRequest, public RGWGetObj  /* RGWOp */
    {
      public:
        ...
        static constexpr uint32_t FLAG_NONE = 0x000;
         ...
         RGWStatObjRequest(CephContext * _cct,
                           std::unique_ptr < rgw::sal::User > _user,
                           const std::string & _bname,
    ...
    class RGWStatBucketRequest:public RGWLibRequest, public RGWStatBucket   /* RGWOp */
    {
      public:
        ...
        RGWStatBucketRequest(CephContext * _cct,
                             std::unique_ptr < rgw::sal::User > _user,
        ...
        void send_response() override {
            bucket->get_creation_time() =
                get_state()->bucket->get_info().creation_time;
    ...
    class RGWStatLeafRequest:public RGWLibRequest, public RGWListBucket /* RGWOp */
    {
      public:
        ...
        bool exact_matched;
         ...
         RGWStatLeafRequest(CephContext * _cct,
                            std::unique_ptr < rgw::sal::User > _user,
    ...
    class RGWCopyObjRequest:public RGWLibRequest, public RGWCopyObj /* RGWOp */
    {
      public:
        ...
        const std::string & dst_name;
         ...
         RGWCopyObjRequest(CephContext * _cct,
                           std::unique_ptr < rgw::sal::User > _user,
        ...
        int header_init() override {
            ...
            std::string dest_obj_name =
                dst_parent->format_child_name(dst_name, false);
            ...
            state->object =
                RGWHandler::driver->get_object(rgw_obj_key(dest_obj_name));
        ...
        int get_params(optional_yield) override {
            ...
            /* we don't have (any) headers, so just create canned ACLs */
            int ret =
                s3policy.create_canned(s->owner, s->bucket_owner,
                                       s->canned_acl);
    ...
    class RGWGetAttrsRequest:public RGWLibRequest, public RGWGetAttrs   /* RGWOp */
    {
      public:
        ...
        :RGWLibRequest(_cct, std::move(_user)), RGWGetAttrs(),
            ...
            std::optional < buffer::list >> &get_attrs() {
            return attrs;
        }
    ...
    class RGWSetAttrsRequest:public RGWLibRequest, public RGWSetAttrs   /* RGWOp */
    {
      public:
        ...
        const std::string & obj_name;
         ...
         RGWSetAttrsRequest(CephContext * _cct,
                            std::unique_ptr < rgw::sal::User > _user,
                            const std::string & _bname,
                            const std::string & _oname)
        :RGWLibRequest(_cct, std::move(_user)), bucket_name(_bname),
    ...
    class RGWGetClusterStatReq:public RGWLibRequest, public RGWGetClusterStat {
      public:
        struct rados_cluster_stat_t &stats_req;
         RGWGetClusterStatReq(CephContext * _cct,
                              std::unique_ptr < rgw::sal::User > _user,
                              rados_cluster_stat_t & _stats):RGWLibRequest(_cct,
                                                                           std::
                                                                           move
                                                                           (_user)),
            stats_req(_stats) {
