...
int rgw_compression_info_from_attrset(const std::map < std::string,
                                      bufferlist > &attrs,
                                      bool & need_decompress,
                                      RGWCompressionInfo & cs_info);
...
class RGWGetObj_Decompress:public RGWGetObj_Filter {
  ...
  public:
     RGWGetObj_Decompress(CephContext * cct_,
                          ...
                          bool partial_content_, RGWGetObj_Filter * next);
...
class RGWPutObj_Compress:public rgw::putobj::Pipe {
  ...
  public:
    ...
    std::vector < compression_block > &get_compression_blocks() {
        return blocks;
    }
    std::optional < int32_t > get_compressor_message() {
