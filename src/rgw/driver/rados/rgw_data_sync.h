...
struct rgw_bucket_entry_owner {
    ...
    rgw_bucket_entry_owner(const std::string &_id, const std::string &_display_name) : id(_id),
...
public:
    ...
    int read_shard_status(const DoutPrefixProvider *dpp, int shard_id, std::set<std::string> &lagging_buckets,
                          std::set<std::string> &recovering_buckets, rgw_data_sync_marker *sync_marker, const int max_entries);
...
class RGWBucketPipeSyncStatusManager : public DoutPrefixProvider
{
    ...
    int remote_info(const DoutPrefixProvider *dpp, source &s,
                    ...
                    uint64_t *num_shards);
public:
    static tl::expected<std::unique_ptr<RGWBucketPipeSyncStatusManager>, int> construct(const DoutPrefixProvider *dpp,
            rgw::sal::RadosStore *driver,
            std::optional<rgw_zone_id> source_zone,
            std::optional<rgw_bucket> source_bucket,
