...
bool InoTable::is_marked_free(inodeno_t id) const const
{
    return free.contains(id) || projected_free.contains(id);
}

bool InoTable::intersects_free(const interval_set < inodeno_t > &other,
...
bool InoTable::repair(inodeno_t id)
{
    ...
    dout(10) << "repair: before status. ino = 0x" << std::
        hex << id << " pver =" << projected_version << " ver= " << version <<
        dendl;
    free.erase(id);
    projected_free.erase(id);
    projected_version = ++version;
    dout(10) << "repair: after status. ino = 0x" << std::
        hex << id << " pver =" << projected_version << " ver= " << version <<
        dendl;
    return true;
}
