...
class MDSMap {
  ...
  public:
    ...
    boost::string_view get_fs_name()const {
        return fs_name;
    ...
    // data pools void add_data_pool(int64_t poolid) {
    ...
    // features uint64_t get_up_features() {
        if (!cached_up_features) {
            bool first = true;
            for (std::map < mds_rank_t, mds_gid_t >::const_iterator p =
                 up.begin(); p != up.end(); ++p) {
   ...
   */
    void get_down_mds_set(std::set < mds_rank_t > *s) const {
        ...
        for (const auto & p:mds_info)
            if (p.second.state >= STATE_REPLAY
   ...
   */ DaemonState get_state_gid(mds_gid_t gid) const {
        std::map < mds_gid_t, mds_info_t >::const_iterator i =
            mds_info.find(gid);
        if (i == mds_info.end())
            return STATE_NULL;
        ...
        return (is_resolve(m) ||
                is_replay(m) ||
                is_rejoin(m) ||
                is_clientreplay(m) || is_active(m) || is_stopping(m));
    } bool is_laggy_gid(mds_gid_t gid) const {
        if (!mds_info.count(gid))
            return false;
        std::map < mds_gid_t, mds_info_t >::const_iterator p =
            mds_info.find(gid);
    ...
    // degraded = some recovery in process.  fixes active membership and// recovery_set.
        ...
        for (std::map < mds_gid_t, mds_info_t >::const_iterator p =
             mds_info.begin(); p != mds_info.end(); ++p)
            if (p->second.state >= STATE_REPLAY
                && p->second.state <= STATE_CLIENTREPLAY)
                return true;
        return false;
