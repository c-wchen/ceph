...
public:
    ...
    public:
        ...
        PhysicalNodeMappingRef < node_key_t, typename pin_t::val_type > get_pin(op_context_t < node_key_t > ctx) const
        {
            assert(!is_end());
            auto val = get_val();
            auto key = get_key();
    ...
    using insert_ret =
        ...
        future < std::pair <
        iterator, bool >>;
    insert_ret insert(op_context_t <
                      node_key_t >
                      c,
                      iterator iter,
                      node_key_t
    ...
    using update_ret =
        update_iertr::
        future < iterator >;
    update_ret update(op_context_t <
                      node_key_t >
                      c,
                      iterator iter,
                      node_val_t
    ...
    using remove_ret =
        remove_iertr::
        future <>;
    remove_ret remove(op_context_t <
                      node_key_t >
                      c,
                      iterator iter)
    {
        ...
        fixed_kv_extent) {
            ...
            (!n_fixed_kv_extent->
             ...
             is_absolute()) {
                ...
                n_fixed_kv_extent->resolve_relative_addrs(make_record_relative_paddr(0).block_relative_to(
...
private:
     ...
     * visitor) const
    {
        ...
        if (found) {
            return fut.
                   then_interruptible([this, c, on_found_internal = std::move(on_found_internal),
    ...
    template < typename LI, typename LL > static lookup_depth_range_ret lookup_depth_range(
    ...
    template <
        typename LI,
        typename LL >
    lookup_ret lookup(op_context_t <
                      node_key_t >
                      c, LI
                      &&
                      lookup_internal,
...
template < typename node_key_t, typename node_val_t, typename internal_node_t, typename leaf_node_t, typename pin_t, size_t node_size, bool leaf_has_children >
struct is_fixed_kv_tree
    < FixedKVBtree < node_key_t, node_val_t, internal_node_t, leaf_node_t, pin_t, node_size, leaf_has_children >>: std::
            true_type {
};
template <
