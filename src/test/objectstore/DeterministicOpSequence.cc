...
DeterministicOpSequence::DeterministicOpSequence(ObjectStore * store,
                                                 std::string status)
:  TestObjectStoreState(store), txn(0), m_osr("OSR")
{
...
void DeterministicOpSequence::_do_coll_create(coll_entry_t * entry,
                                              uint32_t pg_num,
                                              uint64_t num_objs)
{
    ...
    t.collection_hint(entry->m_coll,
                      ObjectStore::Transaction::COLL_HINT_EXPECTED_NUM_OBJECTS,
                      hint);
    dout(0) << "Give collection: " << entry->
        m_coll << " a hint, pg_num is: " << pg_num << ", num_objs is: " <<
        num_objs << dendl;
...
void DeterministicOpSequence::_do_clone(coll_entry_t * entry,
                                        hobject_t & orig_obj,
                                        hobject_t & new_obj)
{
    ObjectStore::Transaction t;
