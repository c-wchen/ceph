...
void RGWOp_OBJLog_SetBounds::execute()
{
    ...
    if (id_str.empty() || marker.empty() || time.empty() || daemon_id.empty()) {
    ...
    if (!err.empty()) {
        dout(5) << "Error parsing id parameter - " << id_str << ", err " << err
            << dendl;
        http_ret = -EINVAL;
        return;
    }
    ...
    if ((http_ret =
         rgw_rest_get_json_input(store->ctx(), s, markers,
                                 REPLICA_INPUT_MAX_LEN, NULL)) < 0) {
...
void RGWOp_OBJLog_GetBounds::execute()
{
    ...
    if (!err.empty()) {
        dout(5) << "Error parsing id parameter - " << id << ", err " << err <<
            dendl;
        http_ret = -EINVAL;
        return;
    }
...
void RGWOp_OBJLog_DeleteBounds::execute()
{
    ...
    if (id.empty() || (!purge_all && daemon_id.empty())) {
        dout(5) << "Error - invalid parameter list" << dendl;
        http_ret = -EINVAL;
        return;
    }
...
static int bucket_instance_to_bucket(RGWRados * store,
                                     ...
                                     rgw_bucket & bucket)
{
    ...
    int r =
        store->get_bucket_instance_info(obj_ctx, bucket_instance, bucket_info,
                                        &mtime, NULL);
    if (r < 0) {
        dout(5) << "could not get bucket instance info for bucket=" <<
            bucket_instance << ": " << cpp_strerror(r) << dendl;
        if (r == -ENOENT)
            return r;
        return -EINVAL;
    }
...
void RGWOp_BILog_SetBounds::execute()
{
    ...
    if (marker.empty() || time.empty() || daemon_id.empty()) {
        dout(5) << "Error - invalid parameter list" << dendl;
        http_ret = -EINVAL;
        return;
    }
    ...
    if ((http_ret =
         bucket_instance_to_bucket(store, bucket_instance, bucket)) < 0) {
        return;
    }
    ...
    http_ret =
        rl.update_bound(bucket, shard_id, daemon_id, marker, ut, &markers);
}
...
RGWOp *RGWHandler_ReplicaLog::op_get()
{
    ...
    if (type.compare("metadata") == 0) {
        ...
        return new RGWOp_OBJLog_GetBounds(DATA_REPLICA_LOG_OBJ_PREFIX,
                                          "datalog");
    }
    return NULL;
}
...
RGWOp *RGWHandler_ReplicaLog::op_delete()
{
    ...
    if (type.compare("metadata") == 0)
        return new RGWOp_OBJLog_DeleteBounds(META_REPLICA_LOG_OBJ_PREFIX,
                                             "mdlog");
    else if (type.compare("bucket-index") == 0)
        return new RGWOp_BILog_DeleteBounds;
    else if (type.compare("data") == 0)
        return new RGWOp_OBJLog_DeleteBounds(DATA_REPLICA_LOG_OBJ_PREFIX,
                                             "datalog");
...
RGWOp *RGWHandler_ReplicaLog::op_post()
{
    ...
    if (type.compare("metadata") == 0) {
        ...
        return new RGWOp_OBJLog_SetBounds(DATA_REPLICA_LOG_OBJ_PREFIX,
                                          "datalog");
    }
    return NULL;
}
