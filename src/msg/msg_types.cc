...
void entity_name_t::dump(Formatter * f) const const
{
    f->dump_string("type", type_str());
    f->dump_unsigned("num", num());
}

void entity_addr_t::dump(Formatter * f) const const
{
    f->dump_unsigned("nonce", nonce);
    f->dump_stream("addr") << get_sockaddr();
}

void entity_inst_t::dump(Formatter * f) const const
{
...
void entity_addrvec_t::encode(bufferlist & bl, uint64_t features) const const
{
...
void entity_addrvec_t::dump(Formatter * f) const const
{
