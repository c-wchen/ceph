...
public:
    ...
    RGWRealm(const std::string &_name, CephContext *_cct, RGWSI_SysObj *_sysobj_svc): RGWSystemMetaObj(_name, _cct,
                _sysobj_svc) {}
...
public:
    ...
    int init(const DoutPrefixProvider *dpp, CephContext *_cct, RGWSI_SysObj *_sysobj_svc,
             const std::string &period_realm_id, optional_yield y,
...
public:
    ...
    const std::optional<RGWRealm> &get_realm() const
    {
        return realm;
    }
    /// Return the current period configuration, if a period is present.
    const std::optional<RGWPeriod> &get_period() const
    {
        return period;
    }
