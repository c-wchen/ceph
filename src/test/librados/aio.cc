...
TEST(LibRadosAio, FlushAsync)
{
    ...
    ASSERT_EQ(0,
              rados_aio_create_completion2(nullptr, nullptr,
                                           &flush_completion));
    char buf[128];
    memset(buf, 0xee, sizeof(buf));
    ASSERT_EQ(0, rados_aio_write(test_data.m_ioctx, "foo",
...
TEST(LibRadosAio, OperateMtime)
{
    ...
    time_t set_mtime = 1457129052;
    {
        ...
        ASSERT_EQ(0,
                  rados_aio_create_completion2(nullptr, nullptr, &completion));
        ASSERT_EQ(0,
                  rados_aio_write_op_operate(op, test_data.m_ioctx, completion,
                                             "foo", &set_mtime, 0));
        {
            TestAlarm alarm;
...
TEST(LibRadosAio, Operate2Mtime)
{
    ...
    1457129052, 123456789};
    {
        ...
        ASSERT_EQ(0,
                  rados_aio_create_completion2(nullptr, nullptr, &completion));
        ASSERT_EQ(0,
                  rados_aio_write_op_operate2(op, test_data.m_ioctx, completion,
                                              "foo", &set_mtime, 0));
        {
            TestAlarm alarm;
            ASSERT_EQ(0, rados_aio_wait_for_complete(completion));
