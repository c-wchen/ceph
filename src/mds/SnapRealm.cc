...
static std::ostream & _prefix(std::ostream * _dout, int whoami,
                              const CInode * inode, uint64_t seq,
 ...
 */
void SnapRealm::build_snap_set() const const
{
...
void SnapRealm::get_snap_info(map < snapid_t, const SnapInfo * >&infomap,
                              snapid_t first, snapid_t last)
{
    ...
    if (srnode.current_parent_since <= last && parent)
        parent->get_snap_info(infomap,
                              std::max(first, srnode.current_parent_since),
                              last);
}
...
std::string_view SnapRealm::get_snapname(snapid_t snapid, inodeno_t atino)
{
    ...
    if (!srnode.past_parent_snaps.empty()) {
        if (srnode.past_parent_snaps.count(snapid)) {
            const SnapInfo *sinfo =
                mdcache->mds->snapclient->get_snap_info(snapid);
            if (sinfo) {
                if (atino == sinfo->ino)
                    return sinfo->name;
                else
...
snapid_t SnapRealm::resolve_snapname(std::string_view n, inodeno_t atino,
                                     snapid_t first, snapid_t last)
{
    // first try me
...
void SnapRealm::merge_to(SnapRealm * newparent)
{
    if (!newparent)
        newparent = parent;
    dout(10) << "merge to " << *newparent << " on " << *newparent->
        inode << dendl;

    dout(10) << " open_children are " << open_children << dendl;
  for (auto realm:open_children) {
        dout(20) << " child realm " << *realm << " on " << *realm->
            inode << dendl;
...
const bufferlist & SnapRealm::get_snap_trace() const const
{
    check_cache();
    return cached_snap_trace;
}

const bufferlist & SnapRealm::get_snap_trace_new() const const
{
    check_cache();
    return cached_snap_trace_new;
}

void SnapRealm::build_snap_trace() const const
{
    ...
    if (parent) {
        ...
        if (!past.empty()) {
            ...
            dout(10) << "build_snap_trace prior_parent_snaps from [1," << *past.
                rbegin() << "] " << info.prior_parent_snaps << dendl;
        }
    }
