...
namespace Scrub {
    ...
    class Store {
      public:
        ...
        void add_snap_error(int64_t pool,
      ...
      private:
         Store(const coll_t & coll, const ghobject_t & oid,
               ObjectStore * store);
         std::vector < ceph::buffer::list >
            get_errors(const std::string & start, const std::string & end,
      ...
      private:
        ...
        // scrubbing
        OSDriver driver;
        mutable MapCacher::MapCacher < std::string,
            ceph::buffer::list > backend;
