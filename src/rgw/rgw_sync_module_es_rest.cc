...
void RGWMetadataSearchOp::execute()
{
  ...
  for (auto & i:s->bucket_info.mdsearch_config) {
    ...
    if (!valid) {
        ldout(s->cct,
              10) << "invalid query, failed generating request json" << dendl;
        op_ret = -EINVAL;
        return;
    }
    ...
    ldout(s->cct, 20) << "response: " << string(out.c_str(),
                                                out.length()) << dendl;

    JSONParser jparser;
    if (!jparser.parse(out.c_str(), out.length())) {
        ldout(s->cct,
              0) << "ERROR: failed to parse elasticsearch response" << dendl;
        op_ret = -EINVAL;
        return;
    }
...
class RGWMetadataSearch_ObjStore_S3:public RGWMetadataSearchOp {
  public:
    RGWMetadataSearch_ObjStore_S3(const RGWSyncModuleInstanceRef &
