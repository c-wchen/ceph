...
struct connection_t {
    ...
    connection_t(CephContext *_cct, const amqp_connection_info &info, bool _verify_ssl,
        ...
        cct(_cct), use_ssl(info.ssl), user(info.user), password(info.password), verify_ssl(_verify_ssl),
    ...
    void publish_internal(message_wrapper_t *message)
    {
        ...
        if (rc == AMQP_STATUS_OK) {
            ...
            if (q_len < max_inflight) {
                ldout(cct, 20) << "AMQP publish (with callback, tag=" << conn->delivery_tag << "): OK. Queue has: " << q_len <<
                               " callbacks" << dendl;
...
public:
                 ...
                 boost::optional<const std::string &> ca_location)
    {
        ...
        ++connection_count;
        auto conn = connections.emplace(tmp_id, std::make_unique<connection_t>(cct, info, verify_ssl,
                                        ca_location)).first->second.get();
