...
OPTION(host, OPT_STR)           // "" means that ceph will use short hostname
    ...
    SAFE_OPTION(erasure_code_dir, OPT_STR)
...
//    "default all to 'daemon', override 'audit' with 'local0'
    ...
    OPTION(xio_trace_mempool, OPT_BOOL)
                                    // mempool allocation counters
    OPTION(xio_trace_msgcnt, OPT_BOOL)
                                    // incoming/outgoing msg counters
    ...
    OPTION(xio_mp_max_64, OPT_INT)
                                // max 64-byte chunks (buffer is 40)
    OPTION(xio_mp_max_256, OPT_INT)
                                // max 256-byte chunks
    OPTION(xio_mp_max_1k, OPT_INT)
                                // max 1K chunks
    OPTION(xio_mp_max_page, OPT_INT)// max 1K chunks
    OPTION(xio_mp_max_hint, OPT_INT)// max size-hint chunks
    OPTION(xio_portal_threads, OPT_INT)
                                    // xio portal threads per messenger
    OPTION(xio_max_conns_per_portal, OPT_INT)
                                            // max xio_connections per portal/ctx
    OPTION(xio_transport_type, OPT_STR)
                                    // xio transport type: {rdma or tcp}
    ...
    OPTION(ms_tcp_prefetch_max_size, OPT_INT)
    ...
    SAFE_OPTION(ms_inject_delay_type, OPT_STR)
    ...
    OPTION(ms_inject_delay_max, OPT_DOUBLE)
                                        // seconds
    OPTION(ms_inject_delay_probability, OPT_DOUBLE)
                                                // range [0, 1]
    OPTION(ms_inject_internal_delays, OPT_DOUBLE)
                                                // seconds
...
// just do ceph_abort() . Here we make it configurable.
    ...
    SAFE_OPTION(ms_dpdk_coremask, OPT_STR)
...
// Weighing of a hardware network queue relative to a software queue (0=no work, 1=     equal share)")
    ...
    OPTION(mon_subscribe_interval, OPT_DOUBLE)
    ...
    OPTION(mon_osd_down_out_interval, OPT_INT)
                                            // seconds
    OPTION(mon_osd_down_out_subtree_limit, OPT_STR)
                                                // smallest crush unit/type that we will not automatically mark out
    ...
    OPTION(mon_osd_warn_op_age, OPT_DOUBLE)
    ...
    OPTION(mon_osd_pool_ec_fast_read, OPT_BOOL)
    ...
    OPTION(mon_force_standby_active, OPT_BOOL)
    ...
    OPTION(mon_data_size_warn, OPT_U64)
    ...
    OPTION(mon_scrub_interval, OPT_INT)
                                    // once a day
...
// UNSAFE -- TESTING ONLY! Allows addition of a cache tier with preexisting snaps
    ...
    OPTION(paxos_propose_interval, OPT_DOUBLE)
                                            // gather updates for this long before proposing a map update
    OPTION(paxos_min_wait, OPT_DOUBLE)
    ...
    OPTION(paxos_service_trim_min, OPT_INT)
                                        // minimum amount of versions to trigger a trim (0 disables it)
    OPTION(paxos_service_trim_max, OPT_INT)
    ...
    OPTION(mon_client_ping_timeout, OPT_DOUBLE)
    ...
    OPTION(mon_client_hunt_interval_max_multiple, OPT_DOUBLE)
    ...
    OPTION(mon_pool_quota_warn_threshold, OPT_INT)
                                                // percent of quota at which to issue warnings
    OPTION(mon_pool_quota_crit_threshold, OPT_INT)
    ...
    OPTION(client_debug_inject_tick_delay, OPT_INT)
...
// note: the max amount of "in flight" dirty data is roughly (max - target)
    ...
    OPTION(fuse_require_active_mds, OPT_BOOL)
                                            // if ceph_fuse requires active mds server
    ...
    OPTION(objecter_inflight_op_bytes, OPT_U64)
                                            // max in-flight data (both directions)
    OPTION(objecter_inflight_ops, OPT_U64)
                                        // max in-flight ios
    OPTION(objecter_completion_locks_per_session, OPT_U64)
...
// Max number of truncate at once in a single Filer::truncate call
    ...
    OPTION(journaler_prefetch_periods, OPT_INT)
                                            // * journal object size
    OPTION(journaler_prezero_periods, OPT_INT)
                                            // * journal object size
...
// max xattr kv pairs size for each dir/file
    ...
    OPTION(mds_session_blacklist_on_timeout, OPT_BOOL)
    ...
    OPTION(mds_log_segment_size, OPT_INT)
    ...
    OPTION(mds_bal_sample_interval, OPT_DOUBLE)
                                            // every 3 seconds
...
// this amount below the threshold to disable.
    ...
    OPTION(osd_journal_size, OPT_INT)
                                    // in mb
...
// and we still want to bring the osd daemon back normally, etc.
    ...
    OPTION(osd_pool_erasure_code_stripe_unit, OPT_U32)
                                                    // in bytes
    OPTION(osd_pool_default_size, OPT_INT)
    OPTION(osd_pool_default_min_size, OPT_INT)
                                            // 0 means no specific default; ceph will use size-size/2
    OPTION(osd_pool_default_pg_num, OPT_INT)// number of PGs for new pools. Configure in global or mon section of ceph.conf
    OPTION(osd_pool_default_pgp_num, OPT_INT)
                                            // number of PGs for placement purposes. Should be equal to pg_num
...
// conservative default throttling values
    ...
    OPTION(osd_tier_default_cache_min_write_recency_for_promote, OPT_INT)
                                                                        // number of recent HitSets the object must appear in to be promoted (on write)
...
// shutdown the OSD if stuatus flipping more than max_markdown_count times in recent max_markdown_period seconds
    ...
    OPTION(osd_disk_thread_ioprio_class, OPT_STR)
                                                // rt realtime be best effort idle
...
// This setting is read by the MONs and OSDs and has to be set to a equal value in both settings of the configuration
    ...
    OPTION(osd_heartbeat_min_size, OPT_INT)
...
// back up after being wrongly marked down.
    ...
    OPTION(osd_mon_ack_timeout, OPT_DOUBLE)
    ...
    OPTION(osd_recovery_max_chunk, OPT_U64)
    ...
    OPTION(osd_copyfrom_max_chunk, OPT_U64)
                                        // max size of a COPYFROM chunk
    OPTION(osd_push_per_object_cost, OPT_U64)
                                            // push cost per object
    OPTION(osd_max_push_cost, OPT_U64)
                                    // max size of push message
    OPTION(osd_max_push_objects, OPT_U64)
                                        // max objects in single push op
    OPTION(osd_recovery_forget_lost_objects, OPT_BOOL)
                                                    // off for now
    ...
    OPTION(osd_scrub_backoff_ratio, OPT_DOUBLE)
...
// Bounds how infrequently a new map epoch will be persisted for a pg
    ...
    OPTION(osd_op_log_threshold, OPT_INT)
                                        // how many op log messages to show in one go
    OPTION(osd_verify_sparse_read_holes, OPT_BOOL)
    ...
    OPTION(osd_backoff_on_down, OPT_BOOL)
                                        // pg in down/incomplete state
    ...
    OPTION(osd_delete_sleep, OPT_FLOAT)
                                    // seconds to sleep between removal transactions
...
// default wait time for an empty queue before pinging the hb timeout
    ...
    OPTION(leveldb_write_buffer_size, OPT_U64)
                                            // leveldb write buffer size
    OPTION(leveldb_cache_size, OPT_U64)
                                    // leveldb cache size
    OPTION(leveldb_block_size, OPT_U64)
                                    // leveldb block size
    ...
    OPTION(kinetic_host, OPT_STR)
                                // hostname or ip address of a kinetic drive to use
    OPTION(kinetic_port, OPT_INT)
                                // port number of the kinetic drive
    OPTION(kinetic_user_id, OPT_INT)// kinetic user to authenticate as
    OPTION(kinetic_hmac_key, OPT_STR)
                                    // kinetic key to authenticate with
    OPTION(kinetic_use_ssl, OPT_BOOL)
                                    // whether to secure kinetic traffic with TLS
    OPTION(rocksdb_separate_wal_dir, OPT_BOOL)
                                            // use $path.wal for wal
    SAFE_OPTION(rocksdb_db_paths, OPT_STR)
                                        // path,size( path,size)*
    OPTION(rocksdb_log_to_ceph_log, OPT_BOOL)
                                            // log to ceph log
    OPTION(rocksdb_cache_size, OPT_U64)
    ...
    OPTION(rocksdb_cache_type, OPT_STR)
                                    // 'lru' or 'clock'
    OPTION(rocksdb_block_size, OPT_INT)
 ...
 */
    ...
    OPTION(osd_snap_trim_cost, OPT_U32)
...
// Max time to wait between notifying mon of shutdown and shutting down
    ...
    OPTION(osd_bench_max_block_size, OPT_U64)
...
// NVMe driver is loaded while osd is running.
    ...
    OPTION(bdev_nvme_retry_count, OPT_INT)
    ...
    OPTION(bluefs_min_log_runway, OPT_U64)
                                        // alloc when we get this low
    OPTION(bluefs_max_log_runway, OPT_U64)
    ...
    OPTION(bluestore_bluefs_min, OPT_U64)
                                        // 1gb
    OPTION(bluestore_bluefs_min_ratio, OPT_FLOAT)
                                                // min fs free / total free
    OPTION(bluestore_bluefs_max_ratio, OPT_FLOAT)
                                                // max fs free / total free
    OPTION(bluestore_bluefs_gift_ratio, OPT_FLOAT)
                                                // how much to add at a time
    OPTION(bluestore_bluefs_reclaim_ratio, OPT_FLOAT)
...
// Default value 0 means that let SPDK nvme library determine the value.
    ...
    OPTION(bluestore_block_size, OPT_U64)
                                        // 10gb for testing
    ...
    OPTION(bluestore_block_wal_size, OPT_U64)
                                            // rocksdb wal
 ...
 */
    ...
    OPTION(bluestore_2q_cache_kout_ratio, OPT_DOUBLE)
...
// max xattr value size
    OPTION(filestore_max_xattr_value_size, OPT_U32)
                                                //Override
...
// to work around ext4's total xattr limit.
    ...
    OPTION(filestore_max_alloc_hint_size, OPT_U64)
                                                // bytes
    OPTION(filestore_max_sync_interval, OPT_DOUBLE)
                                                // seconds
    OPTION(filestore_min_sync_interval, OPT_DOUBLE)
                                                // seconds
...
/// Multiple over expected at max.  Defaults to 0 (disabled).
    ...
    OPTION(journal_align_min_size, OPT_INT)
    ...
    OPTION(rgw_max_put_param_size, OPT_U64)
 ...
 */
    ...
    OPTION(rgw_keystone_revocation_interval, OPT_INT)
                                                    // seconds between tokens revocation check
 ...
 */
    ...
    OPTION(rgw_ops_log_data_backlog, OPT_INT)
    ...
    OPTION(rgw_get_obj_max_req_size, OPT_INT)
    ...
    OPTION(rgw_opstate_ratelimit_sec, OPT_INT)
                                            // min time between opstate updates on a single upload (0 for disabling ratelimit)
    ...
    OPTION(rgw_bucket_quota_soft_threshold, OPT_DOUBLE)
    ...
    OPTION(rgw_bucket_default_quota_max_size, OPT_LONGLONG)
    ...
    OPTION(rgw_user_default_quota_max_objects, OPT_INT)
                                                    // number of objects allowed
    OPTION(rgw_user_default_quota_max_size, OPT_LONGLONG)
                                                        // Max size of object in bytes
    ...
    OPTION(rgw_safe_max_objects_per_shard, OPT_INT)
                                                // safe max loading
    OPTION(rgw_shard_warning_threshold, OPT_DOUBLE)
                                                // pct of safe max
    // at which to warn
...
/* The following are tunables for torrent data */
    ...
    OPTION(rgw_torrent_origin, OPT_STR)
                                    // torrent origin
    OPTION(rgw_torrent_sha_unit, OPT_INT)
...
/* resharding tunables */
    ...
    OPTION(rgw_reshard_bucket_lock_duration, OPT_INT)
                                                    // duration of lock on bucket obj during resharding
