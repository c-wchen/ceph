...
namespace librbd {
    namespace io {
        ...
        class LatencyStats {
          private:
            accumulator_set < uint64_t, stats < tag::rolling_count,
                tag::rolling_sum >> m_acc;
        ...
        template < typename I >
            bool SimpleSchedulerObjectDispatch <
            I >::ObjectRequests::try_delay_request(uint64_t object_off,
                                                   ceph::bufferlist
                                                   && data,
            ...
            if (!m_delayed_requests.empty()) {
                ...
                if (iter != m_delayed_requests.end()) {
                    auto new_iter = m_delayed_requests.insert({ object_off, {}
                                                              }
                    ).first;
                    new_iter->second.data = std::move(data);
                    new_iter->second.data.append(std::move(iter->second.data));
                    new_iter->second.requests =
                        std::move(iter->second.requests);
                ...
                if (iter != m_delayed_requests.begin() &&
                    (iter == m_delayed_requests.end()
                     || iter->first > object_off)) {
                    iter--;
                }
                if (iter != m_delayed_requests.end() &&
            ...
            auto iter = m_delayed_requests.insert({ object_off, {}
                                                  }
            ).first;
        ...
        template < typename I >
            void SimpleSchedulerObjectDispatch <
            I >::ObjectRequests::try_merge_delayed_requests(typename std::map <
                                                            uint64_t,
                                                            MergedRequests >::
                                                            iterator & iter1,
                                                            typename std::map <
        ...
        template < typename I >
            void SimpleSchedulerObjectDispatch <
            I >::ObjectRequests::dispatch_delayed_requests(I * image_ctx,
                                                           LatencyStats *
                                                           latency_stats,
                                                           ceph::mutex *
                                                           latency_stats_lock) {
          for (auto & it:m_delayed_requests) {
                ...
                auto ctx = new LambdaContext([requests =
                                              std::move(merged_requests.
                                                        requests),
                                              latency_stats,
                                              latency_stats_lock, start_time =
                                              ceph_clock_now()](int r){
                                             if (latency_stats) {
                                                                 std::
                                                                 lock_guard
                                                                 locker
                  ...
                  for (auto on_dispatched:                      requests)
                                             ...
                                             auto req =
                                             ObjectDispatchSpec::
                                             create_write(image_ctx,
                                                          OBJECT_DISPATCH_LAYER_SCHEDULER,
                                                          m_object_no, offset,
                                                          std::
                                                          move(merged_requests.
                                                               data),
                                                          m_io_context,
                                                          m_op_flags, 0,
                                                          std::nullopt, 0, {
                                             ...
                                             template < typename I >
                                             SimpleSchedulerObjectDispatch <
                                             I >::
                                             SimpleSchedulerObjectDispatch(I *
                                                                           image_ctx)
      :                                     m_image_ctx(image_ctx),
          ...
          m_lock}; for (auto & extent:      *extents)
                                             {
                                             if (intersects
                                                 (object_no, extent.offset,
                                                  extent.length)) {
                                             dispatch_delayed_requests
                                             (object_no); break;}
                                             }
                                             ...
                                             (object_no, {
                                              }, on_finish); return false;}

                                             template < typename I >
                                             bool SimpleSchedulerObjectDispatch
                                             < I >::write(uint64_t object_no,
                                                          uint64_t object_off,
                                                          ceph::bufferlist
                                                          && data,
                                                          IOContext io_context,
                                             ...
                                             ++m_dispatch_seq;
                                             m_flush_tracker->
                                             start_io(dispatch_seq);
                                             *on_finish =
                                             new
                                             LambdaContext([this, dispatch_seq,
                                                            ctx =
                                                            *on_finish] (int r)
                                                           {
                                                           ctx->complete(r);
                                             ...
                                             *dispatch_result =
                                             ...
                                             (object_no);
                                             register_in_flight_request
                                             (object_no, ceph_clock_now(),
                                              on_finish); return false;}

                                             template < typename I >
                                             bool SimpleSchedulerObjectDispatch
                                             <
                                             I >::write_same(uint64_t object_no,
                                                             uint64_t
                                                             object_off,
                                                             uint64_t
                                                             object_len,
                                             ...
                                             *dispatch_result =
                                             ...
                                             m_flush_tracker->
                                             flush(on_dispatched); return true;}

                                             template < typename I >
                                             bool SimpleSchedulerObjectDispatch
                                             <
                                             I >::intersects(uint64_t object_no,
                                                             uint64_t
                                                             object_off,
                                                             uint64_t len) const
                                             {
                                             ...
                                             auto cct = m_image_ctx->cct;
                                             auto it =
                                             m_requests.find(object_no);
                                             bool intersects =
                                             (it != m_requests.end())
                                             && it->second->
                                             intersects(object_off, len);
                                             ldout(cct,
                                                   20) << intersects << dendl;
                                             return intersects;}
                                             ...
                                             <
                                             I >::
                                             try_delay_write(uint64_t object_no,
                                                             uint64_t
                                                             object_off,
                                                             ceph::bufferlist
                                                             && data,
                                             ...
                                             ldout(cct,
                                                   20) <<
                                             "latency stats not collected yet"
                                             << dendl; return false;}
                                             ...
                                             ldout(cct,
                                                   20) << "no pending requests"
                                             << dendl; return false;}

                                             auto & object_requests =
                                             it->second;
                                             bool delayed =
                                             object_requests->
                                             try_delay_request(object_off,
                                                               std::move(data),
                                                               io_context,
                                                               op_flags,
                                                               object_dispatch_flags,
                                             ...
                                             dispatch_time +=
                                             std::chrono::
                                             nanoseconds(m_latency_stats->
                                                         avg() / 2);}
                                             else {
                                             dispatch_time +=
                                             std::chrono::
                                             milliseconds(m_max_delay);}
                                             object_requests->
                                             set_scheduled_dispatch
                                             (dispatch_time);
                                             m_dispatch_queue.
                                             push_back(object_requests);
                                             if (m_dispatch_queue.front() ==
                                                 object_requests) {
                                             ...
                                             <
                                             I >::
                                             register_in_flight_request(uint64_t
                                                                        object_no,
                                                                        const
                                                                        utime_t
                                                                        &
                                             ...
                                             start_io(dispatch_seq);
                                             it->second->
                                             set_dispatch_seq(dispatch_seq);
                                             *on_finish =
                                             new
                                             LambdaContext([this, object_no,
                                                            dispatch_seq,
                                                            start_time, ctx =
                                                            *on_finish] (int r)
                                                           {
                                             ...
                                             <
                                             I >::
                                             dispatch_delayed_requests(uint64_t
                                                                       object_no)
                                             {
                                             ceph_assert(ceph_mutex_is_locked
                                                         (m_lock));
                                             ...
                                             dispatch_delayed_requests
                                             (m_image_ctx,
                                              m_latency_stats.get(), &m_lock);
                                             ceph_assert(!m_dispatch_queue.
                                                         empty());
                                             if (m_dispatch_queue.front() ==
                                                 object_requests) {
                                             ...
                                             *m_timer_lock};
                                             ...
                                             ldout(cct,
                                                   20) << "nothing to schedule"
                                             << dendl; return;}

                                             auto object_requests =
                                             m_dispatch_queue.front().get();
                                             while (!object_requests->
                                                    is_scheduled_dispatch()) {
                                             ldout(cct,
                                                   20) << "garbage collecting "
                                             << object_requests << dendl;
                                             m_dispatch_queue.pop_front();
                                             if (m_dispatch_queue.empty()) {
                                             ldout(cct,
                                                   20) << "nothing to schedule"
                                             ...
                                             m_timer_task << " at " <<
                                             object_requests->
                                             get_dispatch_time() << dendl;
                                             m_timer->
                                             add_event_at(object_requests->
                                                          get_dispatch_time(),
                                                          m_timer_task);}
