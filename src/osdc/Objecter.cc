...
class Objecter::RequestStateHook:public AdminSocketHook {
  ...
  public:
    ...
    int call(std::string_view command, const cmdmap_t & cmdmap,
             const bufferlist &,
             Formatter * f, std::ostream & ss, cb::list & out) override;
};
...
std::unique_lock < std::mutex > Objecter::OSDSession::get_lock(object_t & oid)
{
    ...
    return {
    completion_locks[h % num_locks], std::defer_lock};
}

const char **Objecter::get_tracked_conf_keys() const const
{
                                  ...
                                  const std::set < std::string > &changed)
{
    ...
    if (changed.count("rados_osd_op_timeout")) {
        osd_timeout =
            conf.get_val < std::chrono::seconds > ("rados_osd_op_timeout");
    }
}
...
void Objecter::init()
{
    ...
    if (!logger) {
        ...
        pcb.add_u64_avg(l_osdc_oplen_avg, "oplen_avg",
                        "Average length of operation vector");
...
void Objecter::_linger_ping(LingerOp * info, bs::error_code ec,
    ...
    bs::error_code > Objecter::linger_check(LingerOp * info)
{
    ...
    if (!info->watch_pending_async.empty())
        stamp =
            std::min(info->watch_valid_thru, info->watch_pending_async.front());
    auto age = ceph::coarse_mono_clock::now() - stamp;

    ldout(cct, 10) << __func__ << " " << info->linger_id
        << " err " << info->last_error << " age " << age << dendl;
    if (info->last_error)
                                               ...
                                               int flags)
{
    ...
    ldout(cct, 10) << __func__ << " info " << info
        << " linger_id " << info->linger_id << " cookie " << info->get_cookie()
        << dendl;
...
ceph_tid_t Objecter::linger_notify(LingerOp * info,
                                   ...
                                   decltype(LingerOp::on_reg_commit)
                                   && onfinish, version_t * objver)
{
    info->snap = snap;
                                ...
                                boost::intrusive_ptr < MWatchNotify > m)
{
    ...
    case CEPH_WATCH_EVENT_NOTIFY:
        info->handle( {
                     ...
                     , m->notify_id, m->cookie, m->notifier_gid,
                     std::move(m->bl));
        break;
    }
                              ...
                              ceph::shunique_lock < ceph::shared_mutex > &sul)
{
    ...
    while (p != s->ops.end()) {
        ...
        case RECALC_OP_TARGET_NO_ACTION:
            if (!skipped_map
                && !(force_resend_writes && op->target.respects_full()))
                break;
            // -- fall-thru --
        case RECALC_OP_TARGET_NEED_RESEND:
        ...
        if (osdmap->get_epoch()) {
            ...
            for (epoch_t e = osdmap->get_epoch() + 1; e <= m->get_last(); e++) {
                ...
                if (osdmap->get_epoch() == e - 1 &&
                    ...
                    ldout(cct,
                          3) << "handle_osd_map decoding incremental epoch " <<
                        e << dendl;
              ...
              for (auto & i:need_resend) {
                ...
                for (auto p = osd_sessions.begin(); p != osd_sessions.end();) {
                    ...
                    if (!osdmap->is_up(s->osd) ||
                        (s->con &&
                         s->con->get_peer_addrs() !=
                         osdmap->get_addrs(s->osd))) {
                        close_session(s);
                    }
                }
                                     ...
                                     const OSDMap & new_osd_map)
{
    ...
    std::set_difference(new_set.begin(), new_set.end(), old_set.begin(),
                        old_set.end(), std::inserter(delta_set,
                                                     delta_set.begin()));
    std::set_difference(new_range_set.begin(), new_range_set.end(),
...
int Objecter::pool_snap_get_info(int64_t poolid, snapid_t snap,
                                 pool_snap_info_t * info) const const
{
    shared_lock rl(rwlock);
...
// sl may be unlocked.
void Objecter::_check_op_pool_eio(Op * op,
                                  std::unique_lock < std::shared_mutex > *sl)
{
    // rwlock is locked unique
...
void Objecter::_check_command_map_dne(CommandOp * c)
{
    ...
    ldout(cct, 10) << "_check_command_map_dne tid " << c->tid
        << " current " << osdmap->get_epoch()
        << " map_dne_bound " << c->map_dne_bound << dendl;
    if (c->map_dne_bound > 0) {
...
void Objecter::wait_for_osd_map(epoch_t e)
{
    ...
    waiting_for_map[e].emplace_back(OpCompletion::create(service.get_executor(),
                                                         w.ref()),
                                    bs::error_code {
                                    }
    );
                                   ...
                                   std::unique_lock < ceph::shared_mutex > &&l)
{
    ...
    if (osdmap->get_epoch() >= newest) {
        ldout(cct,
              10) << __func__ << " latest " << newest << ", have it" << dendl;
        l.unlock();
        ...
        ldout(cct,
              10) << __func__ << " latest " << newest << ", waiting" << dendl;
...
void Objecter::_wait_for_new_map(std::unique_ptr < OpCompletion > c,
                                 epoch_t epoch, bs::error_code ec)
{
    // rwlock is locked unique
...
void Objecter::tick()
{
    ...
    for (auto siter = osd_sessions.begin();
        ...
        for (auto p = s->ops.begin(); p != s->ops.end(); ++p) {
            ...
            if (op->stamp < cutoff) {
                ldout(cct,
                      2) << " tid " << p->first << " on osd." << op->session->
                    osd << " is laggy" << dendl;
                found = true;
                ++laggy_ops;
            }
        ...
        for (auto p = s->linger_ops.begin(); p != s->linger_ops.end(); ++p) {
            ...
            ldout(cct,
                ...
                first << " (osd." << op->session->osd << ")" << dendl;
            found = true;
    ...
    if (initialized) {
        tick_event =
            timer.
            reschedule_me(ceph::
                          make_timespan(cct->_conf->objecter_tick_interval));
    }
}
...
void Objecter::resend_mon_ops()
{
    ...
    for (auto p = check_latest_map_lingers.begin();
        ...
        monc->get_version("osdmap",
                          CB_Linger_Map_Latest(this, p->second->linger_id));
    }

    for (auto p = check_latest_map_commands.begin();
         p != check_latest_map_commands.end(); ++p) {
        monc->get_version("osdmap",
                          CB_Command_Map_Latest(this, p->second->tid));
    }
}
...
void Objecter::_op_submit(Op * op, shunique_lock < ceph::shared_mutex > &sul,
                          ceph_tid_t * ptid)
{
    // rwlock is locked
    ...
    if (r == -EAGAIN ||
        ...
        if (orig_epoch != osdmap->get_epoch()) {
            ...
            ldout(cct,
                  10) << __func__ << " relock raced with osdmap, recalc target"
                << dendl;
    ...
    for (auto siter = osd_sessions.begin();
        ...
        for (auto op_i = s->ops.begin(); op_i != s->ops.end(); ++op_i) {
            if (op_i->second->target.flags & CEPH_OSD_FLAG_WRITE
                && (pool == -1
                    || op_i->second->target.target_oloc.pool == pool)) {
...
bool Objecter::is_pg_changed(int oldprimary,
                             const vector < int >&oldacting,
                             int newprimary,
                             const vector < int >&newacting, bool any_change)
{
    if (OSDMap::primary_changed_broken( // https://tracker.ceph.com/issues/43213
                                          oldprimary,
                                          oldacting, newprimary, newacting))
        return true;
 ...
 */
bool Objecter::osdmap_full_flag() const const
{
    shared_lock rl(rwlock);
...
bool Objecter::osdmap_pool_full(const int64_t pool_id) constconst
{
    shared_lock rl(rwlock);
...
bool Objecter::_osdmap_pool_full(const int64_t pool_id) constconst
{
...
bool Objecter::_osdmap_has_pool_full() const const
{
    for (auto it = osdmap->get_pools().begin();
         it != osdmap->get_pools().end(); ++it) {
 ...
 */
bool Objecter::_osdmap_full_flag() const const
{
...
void Objecter::update_pool_full_map(map < int64_t, bool > &pool_full_map)
{
    for (map < int64_t, pg_pool_t >::const_iterator it
         = osdmap->get_pools().begin(); it != osdmap->get_pools().end(); ++it) {
...
void Objecter::_prune_snapc(const mempool::osdmap::map < int64_t,
      ...
      for (auto s:op->snapc.snaps) {
        ...
        if (match) {
          ...
          for (auto s:op->snapc.snaps) {
            ...
            ldout(cct,
                  10) << __func__ << " op " << op->tid << " snapc " << op->
                snapc << " (was " << new_snaps << ")" << dendl;
        }
    }
}
...
int Objecter::_calc_target(op_target_t * t, Connection * con, bool any_change)
{
    ...
    ldout(cct, 20) << __func__ << " epoch " << t->epoch
        ...
        << " pgid " << t->base_pgid << (is_read ? " is_read" : "")
        << (is_write ? " is_write" : "")
        << dendl;
        ...
        if ((t->flags & (CEPH_OSD_FLAG_BALANCE_READS |
            ...
            if (t->flags & CEPH_OSD_FLAG_BALANCE_READS) {
                ...
                for (unsigned i = 0; i < t->acting.size(); ++i) {
                    int locality =
                        osdmap->crush->get_common_ancestor_distance(cct,
                                                                    t->
                                                                    acting[i],
                                                                    crush_location);
                    ldout(cct,
                          20) << __func__ << " localize: rank " << i << " osd."
                        << t->acting[i]
...
int Objecter::_recalc_linger_op_target(LingerOp * linger_op,
                                       shunique_lock < ceph::shared_mutex >
                                       &sul)
{
    // rwlock is locked unique
...
void Objecter::_finish_op(Op * op, int r)
{
    ...
    ceph_assert(check_latest_map_ops.find(op->tid) ==
                check_latest_map_ops.end());
...
void Objecter::_send_op(Op * op)
{
    ...
    if (p != op->session->backoffs.end()) {
        ...
        if (q != p->second.end()) {
            ldout(cct,
                  20) << __func__ << " ? " << q->first << " [" << q->second.
                begin << "," << q->second.end << ")" << dendl;
            int r = cmp(hoid, q->second.begin);
            if (r == 0 || (r > 0 && hoid < q->second.end)) {
                ldout(cct,
                      10) << __func__ << " backoff " << op->target.
                    actual_pgid << " id " << q->second.
                    id << " on " << hoid << ", queuing " << op << " tid " <<
                    op->tid << dendl;
                return;
            }
        }
    ...
    ldout(cct, 15) << "_send_op " << op->tid << " to "
        << op->target.actual_pgid << " on osd." << op->session->osd << dendl;
    ...
    if (op->outbl && op->ontimeout == 0 &&  // only post rx_buffer if no timeout; see #9582
        op->outbl->length()) {
...
void Objecter::handle_osd_op_reply(MOSDOpReply * m)
{
    ...
    if (op->outbl) {
        ...
        if (op->outbl->length() == bl.length() && bl.get_num_buffers() <= 1) {
            ...
            ldout(cct, 10) << __func__ << " copying resulting " << bl.length()
                << " into existing ceph::buffer of length " << op->outbl->
                length()
                << dendl;
            cb::list t;
            t = std::move(*op->outbl);
    ...
    logger->tinc(l_osdc_op_latency,
                 ceph::coarse_mono_time::clock::now() - op->stamp);
...
void Objecter::handle_osd_backoff(MOSDBackoff * m)
{
    ...
    case CEPH_OSD_BACKOFF_OP_UNBLOCK:
        {
            ...
            if (p != s->backoffs_by_id.end()) {
                ...
                if (b->begin != m->begin && b->end != m->end) {
                    lderr(cct) << __func__ << " got " << m->
                        pgid << " id " << m->id << " unblock on [" << m->
                        begin << "," << m->end << ") but backoff is [" << b->
                        begin << "," << b->end << ")" << dendl;
    ...
    default:
        ldout(cct,
              10) << __func__ << " unrecognized op " << (int)m->op << dendl;
    }
                                        ...
                                        hobject_t * cursor)
{
    ...
    if (list_context->list.empty()) {
        *cursor = list_context->pos;
        ...
        const string *key =
            (entry.locator.empty()? &entry.oid : &entry.locator);
        uint32_t h =
            osdmap->get_pg_pool(list_context->pool_id)->hash_key(*key,
                                                                 entry.nspace);
        *cursor =
            hobject_t(entry.oid, entry.locator, list_context->pool_snap_seq, h,
...
void Objecter::list_nobjects(NListContext * list_context, Context * onfinish)
{
    ...
    if (list_context->starting_pg_num != pg_num) {
        if (!sort_bitwise) {
            ...
            ldout(cct,
                  10) << " pg_num changed; restarting with " << pg_num << dendl;
    ...
    list_context->current_pg =
        pool->raw_hash_to_pg(list_context->pos.get_hash());
    rl.unlock();
                                               ...
                                               snapid_t) >> onfinish)
{
    ...
    op->tid = ++last_tid;
    op->pool = pool;
    op->onfinish = PoolOp::OpComp::create(service.get_executor(),
                                          CB_SelfmanagedSnap(std::
                                                             move(onfinish)));
...
void Objecter::delete_pool_snap(int64_t pool, std::string_view snap_name,
                                decltype(PoolOp::onfinish) && onfinish)
{
    unique_lock wl(rwlock);
        ...
        if (osdmap->get_epoch() < m->epoch) {
            ...
            if (osdmap->get_epoch() < m->epoch) {
                ldout(cct,
                    ...
                    epoch << " before calling back" << dendl;
                _wait_for_new_map(OpCompletion::
                                  create(service.get_executor(),
                                         [o = std::move(op->onfinish), bl =
                                          std::move(bl)] (bs::
...
void Objecter::get_pool_stats(const std::vector < std::string > &pools,
...
int Objecter::pool_stat_op_cancel(ceph_tid_t tid, int r)
{
    ...
    if (op->onfinish)
        op->onfinish->defer(std::move(op->onfinish), osdcode(r),
                            bc::flat_map < std::string, pool_stat_t > {
                            }
                            , false);
...
void Objecter::_fs_stats_submit(StatfsOp * op)
{
    ...
    monc->send_mon_message(new MStatfs(monc->get_fsid(), op->tid,
                                       op->data_pool, last_seen_pgmap_version));
    op->last_submit = ceph::coarse_mono_clock::now();
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_OSD) {
        ...
        if (session) {
            ...
            if (!(initialized && osdmap->is_up(session->osd))) {
                ldout(cct,
                      1) << "ms_handle_reset aborted,initialized=" <<
                    initialized << dendl;
                wl.unlock();
                return false;
            }
...
void Objecter::op_target_t::dump(Formatter * f) const const
{
...
void Objecter::_dump_ops(const OSDSession * s, Formatter * fmt)
{
    for (auto p = s->ops.begin(); p != s->ops.end(); ++p) {
        ...
        auto age =
            std::chrono::duration <
            double >(ceph::coarse_mono_clock::now() - op->stamp);
...
void Objecter::dump_pool_ops(Formatter * fmt) const const
{
...
void Objecter::dump_pool_stat_ops(Formatter * fmt) const const
{
...
void Objecter::dump_statfs_ops(Formatter * fmt) const const
{
...
linger_id(linger_id),
watch_lock(ceph::
           make_shared_mutex(fmt::
                             format("LingerOp::watch_lock #{}", linger_id)))
{
}
                               ...
                               string && rs, cb::list && bl)
{
    ...
    if (c->onfinish)
        c->onfinish->defer(std::move(c->onfinish), ec, std::move(rs),
                           std::move(bl));
...
template < typename T > struct EnumerationContext {
  ...
  private:
    fu2::unique_function < void (bs::error_code,
                                 std::vector < T >, hobject_t) && >on_finish;
  public:
    epoch_t epoch = 0;
    int budget = -1;
...
template < typename T > struct CB_EnumerateReply {
    ...
    CB_EnumerateReply(Objecter * objecter,
                      ...
                      T >> &&ctx):objecter(objecter), ctx(std::move(ctx)) {
...
template < typename T >
    void Objecter::enumerate_objects(int64_t pool_id,
                                     std::string_view ns,
                                     hobject_t start,
                                     hobject_t end,
                                     const uint32_t max,
                                     ...
                                     fu2::unique_function <
                                     void (bs::error_code, std::vector < T >,
                                           hobject_t) && >on_finish)
{
    if (!end.is_max() && start > end) {
        lderr(cct) << __func__ << ": start " << start << " > end " << end <<
            dendl;
    ...
    _issue_enumerate(start,
                     std::make_unique < EnumerationContext < T >> (this,
                                                                   std::
                                                                   move(end),
                                                                   filter_bl,
                                                                   max,
...
template
    ...
    (int64_t pool_id, std::string_view ns, hobject_t start, hobject_t end,
...
template
    void Objecter::enumerate_objects < neorados::Entry > (int64_t pool_id,
                                                          std::string_view ns,
                                                          hobject_t start,
                                                          hobject_t end,
                                                          const uint32_t max,
                                    ...
                                    T >> ctx)
{
    ...
    auto on_ack =
        std::make_unique < CB_EnumerateReply < T >> (this, std::move(ctx));
    ...
    pg_read(start.get_hash(),
            c->oloc, op, pbl, 0,
            Op::OpComp::create(service.get_executor(),[c = std::move(on_ack)]
...
template < typename T >
    void Objecter::_enumerate_reply(cb::list && bl,
                                    bs::error_code ec,
                                    std::unique_ptr < EnumerationContext <
                                    T >> &&ctx)
{
    ...
    if (response.entries.size() <= ctx->max) {
        ...
        next = hobject_t {
        i->oid, i->locator,
                CEPH_NOSNAP, hash, ctx->oloc.get_pool(), i->nspace};
    }
...
template
    void Objecter::_enumerate_reply < librados::ListObjectImpl > (cb::list
                                                                  && bl,
                                                                  bs::
                                                                  error_code ec,
                                                                  std::
...
template
    void Objecter::_enumerate_reply < neorados::Entry > (cb::list && bl,
                                                         bs::error_code ec,
                                                         std::unique_ptr <
                                                         EnumerationContext <
                                                         neorados::
...
namespace {
    ...
    struct C_ObjectOperation_scrub_ls:public Context {
        ...
        :interval(interval), objects(objects), rval(rval) {
                                     ...
                                     std::vector < inconsistent_snapset_t >
                                     *snapsets, int *rval)
...
void::ObjectOperation::scrub_ls(const librados::object_id_t & start_after,
                                uint64_t max_to_get,
                                std::vector < librados::inconsistent_obj_t >
                                *objects, uint32_t * interval, int *rval)
{
...
void::ObjectOperation::scrub_ls(const librados::object_id_t & start_after,
                                uint64_t max_to_get,
                                std::vector < librados::inconsistent_snapset_t >
                                *snapsets, uint32_t * interval, int *rval)
{
