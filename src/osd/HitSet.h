...
class HitSet {
  public:
    ...
    /// abstract interface for a HitSet implementation class Impl {
      public:
  ...
  HitSet():impl(NULL), sealed(false) {
    ...
    /// insert a hash into the set void insert(const hobject_t & o) {
        impl->insert(o);
    }
...
ExplicitHashHitSet(const ExplicitHashHitSet & o):count(o.count), hits(o.hits) {
}
...
class BloomHitSet:public HitSet::Impl {
  ...
  public:
    HitSet::impl_type_t get_type() const const override
{
