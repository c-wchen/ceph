...
/// returns the connection to its pool on the handle's destruction.
    template < typename Connection > class ConnectionHandle {
      ...
      ConnectionHandle(ConnectionPoolBase < Connection > *pool, std::unique_ptr < Connection > conn) noexcept:pool(pool),
            conn(std::
                 move(conn)) {
        }
...
//   F(const DoutPrefixProvider*) -> std::unique_ptr<T>
    template < typename F, typename T >
        concept factory_of =
        requires(F factory, const DoutPrefixProvider * dpp) {
        {
...
/// Generic database connection pool that enforces a limit on open connections.
  template < typename Connection, factory_of < Connection > Factory > class ConnectionPool:public ConnectionPoolBase < Connection >
    {
      public:
      ...
      :    
        ...
        ->ConnectionHandle < Connection > {
            ...
            if (!this->connections.empty()) {
                ...
                ldpp_dout(dpp,
                          4) << "ConnectionPool waiting on a connection" <<
                    dendl;
                this->cond.wait(lock,[&] {
