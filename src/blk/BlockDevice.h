...
private:
    ...
    static block_device_t device_type_from_name(const std::
            string &blk_dev_name);
...
public:
    ...
    {
        return block_size;
    } uint64_t get_optimal_io_size() const
    {
    ...
    virtual int read_random(uint64_t off,
                            uint64_t len, char *buf, bool buffered) = 0;
    virtual int write(uint64_t off,
                      ceph::buffer::list &bl,
                      bool buffered, int write_hint = WRITE_LIFE_NOT_SET) = 0;

    virtual int aio_read(uint64_t off,
                         uint64_t len,
                         ceph::buffer::list *pbl, IOContext *ioc) = 0;
    virtual int aio_write(uint64_t off,
                          ceph::buffer::list &bl,
                          IOContext *ioc,
                          bool buffered,
                          int write_hint = WRITE_LIFE_NOT_SET) = 0;
    virtual int flush() = 0;
    virtual bool try_discard(interval_set < uint64_t > &to_release, bool async =
                                 true)
    {
        return false;
    }
    virtual void discard_drain()
    {
        return;
    }
