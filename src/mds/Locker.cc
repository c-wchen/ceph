        ...
        if (want_layout && !found_layout) {
            if (!mdr->is_rdlocked(&t->policylock)) {
                if (!t->policylock.can_rdlock(client)) {
                    t->policylock.add_waiter(SimpleLock::WAIT_RD,
                                             new C_MDS_RetryRequest(mdcache,
                                                                    mdr));
                    goto failed;
                }
                t->policylock.get_rdlock();
                mdr->locks.emplace(&t->policylock,
                                   MutationImpl::LockOp::RDLOCK);
                dout(20) << " got rdlock on " << t->
                    policylock << " " << *t << dendl;
            }
...
struct MarkEventOnDestruct {
    ...
    MarkEventOnDestruct(MDRequestRef & _mdr,
                        std::string_view _message):mdr(_mdr), message(_message),
        mark_event(true) {
    } ~MarkEventOnDestruct() {
        if (mark_event)
...
bool Locker::acquire_locks(MDRequestRef & mdr,
                           ...
                           CInode * auth_pin_freeze, bool auth_pin_nonblocking)
{
    ...
    for (size_t i = 0; i < lov.size(); ++i) {
        ...
        if (p.is_xlock()) {
            if ((lock->get_type() == CEPH_LOCK_ISNAP ||
                ...
                mdr->is_leader() && !mdr->is_queued_for_replay()) {
                ...
                if (wait) {
                    ...
                    mdr->drop_local_auth_pins();
                    mds->
                        wait_for_cluster_recovered(new
            ...
            if (lock->get_type() >= CEPH_LOCK_IFIRST
                && lock->get_type() != CEPH_LOCK_IVERSION) {
                // inode version lock?
            ...
            client_t _client =
                p.is_state_pin()? lock->get_excl_client() : client;
            if (object->is_auth()) {
                mustpin.insert(object);
            }
  ...
  for (const auto & p:mustpin) {
        ...
        if (mdr->is_auth_pinned(object)) {
            ...
            if (mdr->more()->is_remote_frozen_authpin) {
                ...
                mustpin_remote[mdr->more()->rename_inode->authority().first].
                    size();
            }
        }
        ...
        if (!object->is_auth()) {
            ...
            if (object->is_ambiguous_auth()) {
                // wait
                dout(10) << " ambiguous auth, waiting to authpin " << *object <<
                    dendl;
                ...
                object->add_waiter(MDSCacheObject::WAIT_SINGLEAUTH,
                                   new C_MDS_RetryRequest(mdcache, mdr));
                return false;
            }
        ...
        if (!object->can_auth_pin(&err)) {
            ...
            if (auth_pin_nonblocking) {
                dout(10) << " can't auth_pin (freezing?) " << *object <<
                    ", nonblocking" << dendl;
                mdr->aborted = true;
                return false;
            }
            ...
            dout(10) << " can't auth_pin (freezing?), waiting to authpin " <<
                *object << dendl;
    ...
    if (!mustpin_remote.empty()) {
      ...
      for (const auto & p:mdr->object_states) {
            if (p.second.remote_auth_pinned == MDS_RANK_NONE)
                continue;
            ceph_assert(p.second.remote_auth_pinned ==
      ...
      for (auto & p:mustpin_remote) {
            dout(10) << "requesting remote auth_pins from mds." << p.
                first << dendl;
            ...
            if (mds->is_cluster_degraded() &&
                ...
                if (mdr->more()->waiting_on_peer.empty())
                    mds->wait_for_active_peer(p.first,
                                              new C_MDS_RetryRequest(mdcache,
                                                                     mdr));
                return false;
            }
  ...
  for (const auto & p:lov) {
        ...
        if (p.is_xlock()) {
            if (mdr->is_xlocked(lock)) {
                dout(10) << " already xlocked " << *lock << " " << *lock->
                    get_parent() << dendl;
                continue;
            }
            ...
            dout(10) << " got xlock on " << *lock << " " << *lock->
                get_parent() << dendl;
        }
            ...
            if (p.is_wrlock()) {
                if (it != mdr->locks.end() && it->is_wrlock()) {
                    dout(10) << " already wrlocked " << *lock << " " << *lock->
                        get_parent() << dendl;
                    continue;
                }
                client_t _client =
                ...
                dout(10) << " got wrlock on " << *lock << " " << *lock->
                    get_parent() << dendl;
            }
        }
        else {
            if (mdr->is_rdlocked(lock)) {
                dout(10) << " already rdlocked " << *lock << " " << *lock->
                    get_parent() << dendl;
                continue;
            }
            ...
            dout(10) << " got rdlock on " << *lock << " " << *lock->
                get_parent() << dendl;
        }
    }
    ...
    for (set < mds_rank_t >::iterator p = peers.begin(); p != peers.end(); ++p) {
        if (!mds->is_cluster_degraded() ||
            ...
            dout(10) << "_drop_non_rdlocks dropping remote locks on mds." << *p
                << dendl;
            auto peerreq =
                make_message < MMDSPeerRequest > (mut->reqid, mut->attempt,
                                                  MMDSPeerRequest::
                                                  OP_DROPLOCKS);
...
class C_MDL_DropCache:public LockerContext {
  ...
  public:
    C_MDL_DropCache(Locker * l, MDLockCache * lc):LockerContext(l),
        lock_cache(lc) {
...
{
    dout(10) << "invalidate_lock_caches " << *lock << " on " << *lock->
        get_parent() << dendl;
    if (lock->is_cached()) {
                               ...
                               file_layout_t * dir_layout)
{
    ...
    dout(10) << "create_lock_cache for client." << client << "/" <<
    ...
    if (mdr->has_more() && !mdr->more()->peers.empty()) {
        dout(10) << " there are peers requests for " << *mdr << ", noop" <<
            dendl;
        return;
    }
    ...
    if (!cap) {
        dout(10) << " there is no cap for client." << client << ", noop" <<
            dendl;
        return;
    }
    ...
    for (auto p = cap->lock_caches.begin(); !p.end(); ++p) {
        if ((*p)->opcode == opcode) {
            dout(10) << " lock cache already exists for " <<
                ceph_mds_op_name(opcode) << ", noop" << dendl;
            return;
        }
    }
  ...
  for (auto & p:mdr->object_states) {
        ...
        if (stat.auth_pinned) {
            if (!p.first->can_auth_pin()) {
                dout(10) << " can't auth_pin(freezing?) lock parent " << *p.
                    first << ", noop" << dendl;
                return;
            }
            if (CInode * in = dynamic_cast < CInode * >(p.first);
                ...
                if (!dir->can_auth_pin()) {
                    dout(10) << " can't auth_pin(!auth|freezing?) dirfrag " <<
                        *dir << ", noop" << dendl;
                    return;
                }
            }
  ...
  for (auto dir:dfv) {
        ...
        if (dir->is_any_freezing_or_frozen_inode()) {
            dout(10) << " there is freezing/frozen inode in " << *dir <<
                ", noop" << dendl;
            return;
        }
    }
...
bool Locker::find_and_attach_lock_cache(MDRequestRef & mdr, CInode * diri)
{
    ...
    for (auto p = cap->lock_caches.begin(); !p.end(); ++p) {
        ...
        if (lock_cache->opcode == opcode) {
            dout(10) << "found lock cache for " << ceph_mds_op_name(opcode) <<
                " on " << *diri << dendl;
            mdr->lock_cache = lock_cache;
            mdr->lock_cache->ref++;
            return true;
...
{
    dout(10) << "eval_gather " << *lock << " on " << *lock->
        get_parent() << dendl;
    ...
    if (caps && in->is_head()) {
        ...
        if (first && ((~lock->gcaps_allowed(CAP_ANY, next) & other_issued) ||
                      ...
                      (~lock->
                       gcaps_allowed(CAP_XLOCKER, next) & xlocker_issued)))
            need_issue = true;
    }

#define IS_TRUE_AND_LT_AUTH(x, auth) (x && ((auth && x <= AUTH) || (!auth && x < AUTH)))
    bool auth = lock->get_parent()->is_auth();
    if (!lock->is_gathering() && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_rdlock, auth) || !lock->is_rdlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_wrlock, auth) || !lock->is_wrlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_xlock, auth) || !lock->is_xlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_lease, auth) || !lock->is_leased()) && !(lock->get_parent()->is_auth() && lock->is_flushing()) &&  // i.e. wait for scatter_writebehind!
        (!caps || ((~lock->gcaps_allowed(CAP_ANY, next) & other_issued) == 0 && (~lock->gcaps_allowed(CAP_LONER, next) & loner_issued) == 0 && (~lock->gcaps_allowed(CAP_XLOCKER, next) & xlocker_issued) == 0)) && lock->get_state() != LOCK_SYNC_MIX2 &&  // these states need an explicit trigger from the auth mds
        ...
        if (lock->get_sm() == &sm_filelock) {
            ...
            if (in->state_test(CInode::STATE_RECOVERING)) {
                dout(7) << "eval_gather finished gather, but still recovering"
                    << dendl;
                return;
            }
            else if (in->state_test(CInode::STATE_NEEDSRECOVER)) {
                dout(7) << "eval_gather finished gather, but need to recover" <<
                    dendl;
        ...
        if (!lock->get_parent()->is_auth()) {
            ...
            if (!mds->is_cluster_degraded() ||
                ...
                case LOCK_SYNC_LOCK:
                    mds->send_message_mds(make_message < MLock >
                                          (lock, LOCK_AC_LOCKACK,
                                           mds->get_nodeid()), auth);
                    break;
                ...
                case LOCK_MIX_SYNC:
                    {
                        auto reply =
                            make_message < MLock > (lock, LOCK_AC_SYNCACK,
                                                    mds->get_nodeid());
                ...
                case LOCK_SYNC_MIX:
                    {
                        auto reply =
                            make_message < MLock > (lock, LOCK_AC_MIXACK,
                                                    mds->get_nodeid());
                        mds->send_message_mds(reply, auth);
                        next = LOCK_SYNC_MIX2;
                    }
            ...
            if (lock->get_state() == LOCK_MIX_LOCK &&
                ...
                dout(10) <<
                    " finished (local) gather for mix->lock, now gathering from replicas"
                    << dendl;
        ...
        if (pfinishers)
            lock->
                take_waiting(SimpleLock::WAIT_STABLE | SimpleLock::
                             WAIT_WR | SimpleLock::WAIT_RD | SimpleLock::
                             WAIT_XLOCK, *pfinishers);
        else
            lock->
                finish_waiters(SimpleLock::WAIT_STABLE | SimpleLock::
                               WAIT_WR | SimpleLock::WAIT_RD | SimpleLock::
                               WAIT_XLOCK);
...
bool Locker::eval(CInode * in, int mask, bool caps_imported)
{
    ...
    if (in->is_auth() && in->is_head()) {
        ...
        if (in->choose_ideal_loner()) {
            ...
            dout(10) << "eval want loner: client." << in->
    ...
    if (in->is_auth() && in->is_head()
        ...
        if (in->try_drop_loner()) {
            ...
            if (in->get_wanted_loner() >= 0) {
                dout(10) << "eval end set loner to client." << in->
...
class C_Locker_Eval:public LockerContext {
    ...
    int mask;
  public:
    C_Locker_Eval(Locker * l, MDSCacheObject * pp, int m):LockerContext(l),
        p(pp), mask(m) {
...
void Locker::try_eval(MDSCacheObject * p, int mask)
{
    ...
    if (p->is_ambiguous_auth()) {
        ...
        p->add_waiter(MDSCacheObject::WAIT_SINGLEAUTH,
                      new C_Locker_Eval(this, p, mask));
        return;
    }

    if (p->is_auth() && p->is_frozen()) {
        dout(7) << "try_eval frozen, waiting on " << *p << dendl;
        p->add_waiter(MDSCacheObject::WAIT_UNFREEZE,
                      new C_Locker_Eval(this, p, mask));
        return;
    }
...
void Locker::try_eval(SimpleLock * lock, bool * pneed_issue)
{
    ...
    if (p->is_frozen()) {
        dout(7) << "try_eval " << *lock << " frozen, waiting on " << *p <<
            dendl;
        p->add_waiter(MDSCacheObject::WAIT_UNFREEZE,
                      new C_Locker_Eval(this, p, lock->get_type()));
        return;
    ...
    if (lock->get_type() != CEPH_LOCK_DN &&
        ...
        dout(7) << "try_eval " << *lock << " freezing, waiting on " << *p <<
            dendl;
        p->add_waiter(MDSCacheObject::WAIT_UNFREEZE,
                      new C_Locker_Eval(this, p, lock->get_type()));
        return;
    ...
    if (lock->is_stable()) {
        if (lock->get_parent()->is_auth()) {
            if (lock->get_sm() == &sm_scatterlock) {
                ...
                if (lock->get_state() == LOCK_EXCL && in->get_target_loner() >= 0 && !in->is_dir() && !as_anon) // as_anon => caller wants SYNC, not XSYN
                    file_xsyn(lock);
                else
                    simple_sync(lock);
            }
...
{
    dout(7) << "rdlock_try on " << *lock << " on " << *lock->
        get_parent() << dendl;
...
{
    dout(7) << "rdlock_start  on " << *lock << " on " << *lock->
        get_parent() << dendl;
    ...
    dout(7) << "rdlock_start waiting on " << *lock << " on " << *lock->
        get_parent() << dendl;
...
void Locker::rdlock_finish(const MutationImpl::lock_iterator & it,
                           MutationImpl * mut, bool * pneed_issue)
{
    ceph_assert(it->is_rdlock());
    SimpleLock *lock = it->lock;
    // drop ref
    ...
    dout(7) << "rdlock_finish on " << *lock << " on " << *lock->
        get_parent() << dendl;
...
bool Locker::rdlock_try_set(MutationImpl::LockOpVec & lov, MDRequestRef & mdr)
{
  ...
  for (const auto & p:lov) {
        ...
        dout(20) << " got rdlock on " << *lock << " " << *lock->
            get_parent() << dendl;
    }
...
bool Locker::wrlock_try(SimpleLock * lock, const MutationRef & mut,
                        client_t client)
{
    dout(10) << "wrlock_try " << *lock << " on " << *lock->
        get_parent() << dendl;
    if (client == -1)
...
bool Locker::wrlock_start(const MutationImpl::LockOp & op, MDRequestRef & mut)
{
    ...
    dout(10) << "wrlock_start " << *lock << " on " << *lock->
        get_parent() << dendl;

    CInode *in = static_cast < CInode * >(lock->get_parent());
    client_t client =
        op.is_state_pin()? lock->get_excl_client() : mut->get_client();
    ...
    dout(7) << "wrlock_start waiting on " << *lock << " on " << *lock->
        get_parent() << dendl;
    lock->add_waiter(SimpleLock::WAIT_STABLE,
...
void Locker::wrlock_finish(const MutationImpl::lock_iterator & it,
                           MutationImpl * mut, bool * pneed_issue)
{
    ...
    dout(7) << "wrlock_finish on " << *lock << " on " << *lock->
        get_parent() << dendl;
    lock->put_wrlock();
...
void Locker::remote_wrlock_start(SimpleLock * lock, mds_rank_t target,
                                 MDRequestRef & mut)
{
    ...
    if (mds->is_cluster_degraded() &&
        ...
        if (mut->more()->waiting_on_peer.empty())
            mds->wait_for_active_peer(target,
                                      new C_MDS_RetryRequest(mdcache, mut));
        return;
    }
...
void Locker::remote_wrlock_finish(const MutationImpl::lock_iterator & it,
                                  MutationImpl * mut)
{
...
bool Locker::xlock_start(SimpleLock * lock, MDRequestRef & mut)
{
    ...
    dout(7) << "xlock_start on " << *lock << " on " << *lock->
        get_parent() << dendl;
    ...
    if (lock->get_parent()->is_auth()) {
        ...
        while (1) {
            if (mut->locking && // started xlock (not preempt other request)
                lock->can_xlock(client) && !(lock->get_state() == LOCK_LOCK_XLOCK &&    // client is not xlocker or
...
void Locker::_finish_xlock(SimpleLock * lock, client_t xlocker,
                           bool * pneed_issue)
{
    ceph_assert(!lock->is_stable());
    if (lock->get_type() != CEPH_LOCK_DN &&
...
void Locker::xlock_finish(const MutationImpl::lock_iterator & it,
                          MutationImpl * mut, bool * pneed_issue)
{
    ...
    dout(10) << "xlock_finish on " << *lock << " " << *lock->
        get_parent() << dendl;
    ...
    if (!lock->get_parent()->is_auth()) {
        ...
        // tell auth
        dout(7) << "xlock_finish releasing remote xlock on " << *lock->
            get_parent() << dendl;
        mds_rank_t auth = lock->get_parent()->authority().first;
        if (!mds->is_cluster_degraded() ||
            mds->mdsmap->get_state(auth) >= MDSMap::STATE_REJOIN) {
        ...
        lock->finish_waiters(SimpleLock::WAIT_STABLE |
                             SimpleLock::WAIT_WR | SimpleLock::WAIT_RD, 0);
    }
    else {
...
void Locker::xlock_export(const MutationImpl::lock_iterator & it,
                          MutationImpl * mut)
{
    ceph_assert(it->is_xlock());
    SimpleLock *lock = it->lock;
    dout(10) << "xlock_export on " << *lock << " " << *lock->
        get_parent() << dendl;
...
{
    dout(10) << "xlock_import on " << *lock << " " << *lock->
        get_parent() << dendl;
...
{
    dout(10) << "xlock_downgrade on " << *lock << " " << *lock->
        get_parent() << dendl;
...
class C_Locker_FileUpdate_finish:public LockerLogContext {
  ...
  public:
    C_Locker_FileUpdate_finish(Locker * l, CInode * i, MutationRef & m,
                               unsigned f, const ref_t < MClientCaps > &ack,
                               client_t c = -1)
                                ...
                                const ref_t < MClientCaps > &ack)
{
    ...
    if (ack) {
        ...
        if (session && !session->is_closed()) {
            ...
            dout(10) << " no session for client." << client << " " << *ack <<
                dendl;
        }
    }
    ...
    if (in->is_head()) {
        ...
        if ((flags & UPDATE_SHAREMAX) && in->is_auth() &&
            (in->filelock.
             gcaps_allowed(CAP_LONER) & (CEPH_CAP_GWR | CEPH_CAP_GBUFFER)))
...
Capability *Locker::issue_new_caps(CInode * in,
                                   int mode,
                                   MDRequestRef & mdr, SnapRealm * realm)
{
...
class C_Locker_RevokeStaleCap:public LockerContext {
    ...
    client_t client;
  public:
    C_Locker_RevokeStaleCap(Locker * l, CInode * i,
                            client_t c):LockerContext(l), in(i), client(c) {
        in->get(CInode::PIN_PTRWAITER);
    } void finish(int r) override {
    ...
    for (; it != in->client_caps.end(); ++it) {
        ...
        if (!(pending & ~allowed)) {
            ...
            if (cap->is_new() || cap->is_suppress() || cap->is_stale()) {
                dout(20) <<
                    "  !revoke and new|suppressed|stale, skipping client." <<
                    it->first << dendl;
                continue;
            }
        }
            ...
            if (cap->is_stale()) {
                dout(20) << "  revoke stale cap from client." << it->
                    first << dendl;
                ceph_assert(!cap->is_valid());
                cap->issue(allowed & pending, false);
                mds->
                    queue_waiter_front(new
                                       C_Locker_RevokeStaleCap(this, in,
                                                               it->first));
                continue;
            }
            ...
            if (!cap->is_valid() && (pending & ~CEPH_CAP_PIN)) {
                ...
                dout(7) << "   sending MClientCaps to client." << it->first
                    << " seq " << seq << " re-issue " << ccap_string(pending) <<
                    dendl;
                ...
                auto m =
                    make_message < MClientCaps > (CEPH_CAP_OP_GRANT, in->ino(),
                                                  in->find_snaprealm()->inode->
                                                  ino(),
                                                  cap->get_cap_id(),
                                                  cap->get_last_seq(),
                                                  pending, wanted, 0,
        ...
        if ((pending & ~allowed) || // need to revoke ~allowed caps.
            ...
            if (op == CEPH_CAP_OP_REVOKE) {
                ...
                revoking_caps_by_client[cap->get_client()].push_back(&cap->
...
void Locker::revoke_stale_cap(CInode * in, client_t client)
{
    ...
    if (cap->revoking() & CEPH_CAP_ANY_WR) {
        ...
        mds->evict_client(client.v, false,
                          g_conf()->mds_session_blocklist_on_timeout, *css,
                          nullptr);
        return;
    }
...
bool Locker::revoke_stale_caps(Session * session)
{
    ...
    for (auto p = session->caps.begin(); !p.end();) {
        ...
        dout(10) << " revoking " << ccap_string(issued) << " on " << *in <<
            dendl;
...
void Locker::request_inode_file_caps(CInode * in)
{
    ...
    int wanted =
        in->get_caps_wanted() & in->get_caps_allowed_ever() & ~CEPH_CAP_PIN;
    if (wanted != in->replica_caps_wanted) {
        ...
        if (!mds->is_cluster_degraded() ||
            mds->mdsmap->is_clientreplay_or_active_or_stopping(auth))
            mds->send_message_mds(make_message < MInodeFileCaps >
                                  (in->ino(), in->replica_caps_wanted), auth);
    }
}
...
class C_MDL_CheckMaxSize:public LockerContext {
  ...
  public:
    C_MDL_CheckMaxSize(Locker * l, CInode * i, uint64_t _new_max_size,
                       uint64_t _newsize, utime_t _mtime):LockerContext(l),
        in(i), new_max_size(_new_max_size), newsize(_newsize), mtime(_mtime) {
        in->get(CInode::PIN_PTRWAITER);
    } void finish(int r) override {
        if (in->is_auth())
            locker->check_inode_max_size(in, false, new_max_size, newsize,
                                         mtime);
        in->put(CInode::PIN_PTRWAITER);
    }
};

uint64_t Locker::calc_new_max_size(const CInode::inode_const_ptr & pi,
                                   uint64_t size)
{
...
bool Locker::calc_new_client_ranges(CInode * in, uint64_t size,
                                    bool * max_increased)
{
    const auto & latest = in->get_projected_inode();
    uint64_t ms;
  ...
  for (auto & p:in->client_caps) {
        if ((p.second.issued() | p.second.wanted()) & CEPH_CAP_ANY_FILE_WR) {
            ...
            if (it != pi->client_ranges.end() && it->first == p.first) {
                ...
                it = pi->client_ranges.emplace_hint(it,
                                  ...
                                  utime_t new_mtime)
{
    ...
    if (new_ranges &&
        calc_new_client_ranges(in, std::max(new_max_size, size),
                               &max_increased)) {
        dout(10) << "check_inode_max_size client_ranges " << in->
            get_previous_projected_inode()->client_ranges << " -> " << pi.
            inode->client_ranges << dendl;
    }

    if (update_size) {
        dout(10) << "check_inode_max_size size " << pi.inode->
            size << " -> " << new_size << dendl;
        pi.inode->size = new_size;
        pi.inode->rstat.rbytes = new_size;
        dout(10) << "check_inode_max_size mtime " << pi.inode->
            mtime << " -> " << new_mtime << dendl;
...
bool Locker::_need_flush_mdlog(CInode * in, int wanted, bool lock_state_any)
{
    ...
    if (((wanted &
          (CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR | CEPH_CAP_FILE_SHARED |
           CEPH_CAP_FILE_EXCL))
         && (lock_state_any ? in->filelock.is_locked() : in->filelock.
             is_unstable_and_locked()))
        || ((wanted & (CEPH_CAP_AUTH_SHARED | CEPH_CAP_AUTH_EXCL))
            && (lock_state_any ? in->authlock.is_locked() : in->authlock.
                is_unstable_and_locked()))
        || ((wanted & (CEPH_CAP_LINK_SHARED | CEPH_CAP_LINK_EXCL))
            && (lock_state_any ? in->linklock.is_locked() : in->linklock.
                is_unstable_and_locked()))
        || ((wanted & (CEPH_CAP_XATTR_SHARED | CEPH_CAP_XATTR_EXCL))
            && (lock_state_any ? in->xattrlock.is_locked() : in->xattrlock.
                is_unstable_and_locked())))
        return true;
    return false;
}
                                ...
                                snapid_t last)
{
    ...
    for (auto p = head_in->client_need_snapflush.begin();
        ...
        if (clients.count(client)) {
            dout(10) << " doing async NULL snapflush on " << snapid <<
                " from client." << client << dendl;
            CInode *sin = mdcache->pick_inode_snap(head_in, snapid - 1);
            ceph_assert(sin);
            ceph_assert(sin->first <= snapid);
            _do_snap_update(sin, snapid, 0, sin->first - 1, client,
                            ref_t < MClientCaps > (), ref_t < MClientCaps > ());
...
void Locker::handle_client_caps(const cref_t < MClientCaps > &m)
{
    ...
    if (!mds->is_clientreplay() && !mds->is_active() && !mds->is_stopping()) {
        ...
        if (session->is_closed() ||
            ...
            dout(7) << " session closed|closing|killing, dropping " << *m <<
                dendl;
            return;
        }
        if ((mds->is_reconnect()
             || mds->get_want_state() == MDSMap::STATE_RECONNECT) && dirty
            && m->get_client_tid() > 0
    ...
    if (m->get_client_tid() > 0 && session &&
        session->have_completed_flush(m->get_client_tid())) {
        dout(7) << "handle_client_caps already flushed tid " << m->
            get_client_tid()
        ...
        if (op == CEPH_CAP_OP_FLUSHSNAP) {
            ...
            ack =
                make_message < MClientCaps > (CEPH_CAP_OP_FLUSHSNAP_ACK,
                                              m->get_ino(), 0, 0, 0, 0, 0,
                                              dirty, 0,
                                              mds->get_osd_epoch_barrier());
        }
            ...
            ack =
                make_message < MClientCaps > (CEPH_CAP_OP_FLUSH_ACK,
                                              m->get_ino(), 0, m->get_cap_id(),
                                              m->get_seq(), m->get_caps(), 0,
                                              dirty, 0,
                                              mds->get_osd_epoch_barrier());
        }
    ...
    if (m->get_oldest_flush_tid() > 0 && session) {
        if (session->trim_completed_flushes(m->get_oldest_flush_tid())) {
            mds->mdlog->get_current_segment()->touched_sessions.insert(session->
                                                                       info.
                                                                       inst.
                                                                       name);
    ...
    if (!head_in) {
        if (mds->is_clientreplay()) {
            ...
            mdcache->wait_replay_cap_reconnect(m->get_ino(),
         ...
         */
        dout(7) << "handle_client_caps on unknown ino " << m->
    ...
    if (should_defer_client_cap_frozen(head_in)) {
        dout(7) << "handle_client_caps freezing|frozen on " << *head_in <<
            dendl;
        head_in->add_waiter(CInode::WAIT_UNFREEZE,
                            new C_MDS_RetryMessage(mds, m));
        return;
    }
    if (ceph_seq_cmp(m->get_mseq(), cap->get_mseq()) < 0) {
        dout(7) << "handle_client_caps mseq " << m->get_mseq() << " < " << cap->
            get_mseq()
            << ", dropping" << dendl;
        return;
    }
    ...
    if (op == CEPH_CAP_OP_FLUSHSNAP) {
        ...
        if (in == head_in ||
            ...
            if (in == head_in)
                cap->client_follows =
                    snap < CEPH_NOSNAP ? snap : realm->get_newest_seq();
    ...
    if (cap->get_cap_id() != m->get_cap_id()) {
        dout(7) << " ignoring client capid " << m->
            get_cap_id() << " != my " << cap->get_cap_id() << dendl;
    }
    else {
        CInode *in = head_in;
        if (follows > 0) {
            ...
            while (in != head_in) {
                ceph_assert(in->last != CEPH_NOSNAP);
                if (in->is_auth() && dirty) {
                    dout(10) << " updating intermediate snapped inode " << *in
                        << dendl;
                    _do_cap_update(in, NULL, dirty, follows, m,
                                   ref_t < MClientCaps > ());
                }
        ...
        int caps = m->get_caps();
        if (caps & ~cap->issued()) {
            dout(10) << " confirming not issued caps " << ccap_string(caps &
                                                                      ~cap->
                                                                      issued())
                << dendl;
            caps &= cap->issued();
        ...
        if (!head_in->client_need_snapflush.empty()) {
            if (!(cap->issued() & CEPH_CAP_ANY_FILE_WR) &&
                ...
                dout(10) <<
                    " revocation in progress, not making any conclusions about null snapflushes"
                    << dendl;
            }
        }
        ...
        if (dirty && in->is_auth()) {
            ...
            ack =
                make_message < MClientCaps > (CEPH_CAP_OP_FLUSH_ACK, in->ino(),
                                              0, cap->get_cap_id(),
                                              m->get_seq(), m->get_caps(), 0,
                                              dirty, 0,
                                              mds->get_osd_epoch_barrier());
        ...
        if (in->is_auth() &&
            ...
            if (!need_flush && (cap->wanted() & ~cap->pending()))
                need_flush =
                    _need_flush_mdlog(in, cap->wanted() & ~cap->pending());
        }
        else {
            // no update, ack now.
            if (ack) {
...
class C_Locker_RetryRequestCapRelease:public LockerContext {
  ...
  public:
    C_Locker_RetryRequestCapRelease(Locker * l, client_t c,
                                    const ceph_mds_request_release &
                                    it):LockerContext(l), client(c), item(it) {
    } void finish(int r) override {
        string dname;
                                         ...
                                         std::string_view dname)
{
    ...
    if (dname.length()) {
        ...
        if (dir) {
            ...
            if (dn) {
                ...
                if (l) {
                    dout(10) << __func__ << " removing lease on " << *dn <<
                        dendl;
                ...
                dout(7) << __func__ << " client." << client <<
                    " released lease on dn " << dir->
                    dirfrag() << "/" << dname << " which dne" << dendl;
            }
        }
    }
    ...
    if (caps & ~cap->issued()) {
        dout(10) << " confirming not issued caps " << ccap_string(caps & ~cap->
                                                                  issued()) <<
            dendl;
        caps &= cap->issued();
    }
...
class C_Locker_RetryKickIssueCaps:public LockerContext {
    ...
    ceph_seq_t seq;
  public:
    C_Locker_RetryKickIssueCaps(Locker * l, CInode * i, client_t c,
                                ceph_seq_t s):LockerContext(l), in(i),
        client(c), seq(s) {
        in->get(CInode::PIN_PTRWAITER);
    } void finish(int r) override {
 ...
 */
void Locker::_do_snap_update(CInode * in, snapid_t snap, int dirty,
                             snapid_t follows, client_t client,
                             const cref_t < MClientCaps > &m,
                             const ref_t < MClientCaps > &ack)
{
    dout(10) << "_do_snap_update dirty " << ccap_string(dirty)
        << " follows " << follows << " snap " << snap << " on " << *in << dendl;
    ...
    // xattr
    if (xattrs) {
        dout(7) << " xattrs v" << i->xattr_version << " -> " << m->head.
            xattr_version << " len " << m->xattrbl.length() << dendl;
    ...
    mdcache->predirty_journal_parents(mut, &le->metablob, in, 0,
                                      PREDIRTY_PRIMARY, 0, follows);
    mdcache->journal_dirty_inode(mut.get(), &le->metablob, in, follows);

    // "oldest flush tid" > 0 means client uses unique TID for each flush
    if (ack && ack->get_oldest_flush_tid() > 0)
        le->metablob.
            add_client_flush(metareqid_t
                             (m->get_source(), ack->get_client_tid()),
                             ack->get_oldest_flush_tid());

    mds->mdlog->submit_entry(le,
                             new C_Locker_FileUpdate_finish(this, in, mut,
                                                            UPDATE_SNAPFLUSH,
                                                            ack, client));
}
                                ...
                                CInode::mempool_inode * pi)
{
    ...
    if ((features & CEPH_FEATURE_FS_CHANGE_ATTR) &&
        ...
        dout(7) << "  change_attr " << pi->change_attr << " -> " << m->
            get_change_attr()
            << " for " << *in << dendl;
    ...
    if (dirty & (CEPH_CAP_FILE_EXCL | CEPH_CAP_FILE_WR)) {
        ...
        if ((dirty & CEPH_CAP_FILE_EXCL) &&
            ...
            dout(7) << "  time_warp_seq " << pi->time_warp_seq << " -> " << m->
                get_time_warp_seq()
                << " for " << *in << dendl;
                            ...
                            const ref_t < MClientCaps > &ack, bool * need_flush)
{
    ...
    if (session->check_access(in, MAY_WRITE,
        ...
        dout(10) << "check_access failed, dropping cap update on " << *in <<
            dendl;
        return false;
    }
    ...
    // xattrs update?
    if (xattr) {
        dout(7) << " xattrs v" << pi.inode->xattr_version << " -> " << m->head.
            xattr_version << dendl;
    ...
    mds->mdlog->submit_entry(le,
                             new C_Locker_FileUpdate_finish(this, in, mut,
                                                            update_flags, ack,
                                                            client));
    if (need_flush && !*need_flush && ((change_max && new_max) ||   // max INCREASE
...
void Locker::handle_client_cap_release(const cref_t < MClientCapRelease > &m)
{
  ...
  for (const auto & cap:m->caps) {
        _do_cap_release(client, inodeno_t((uint64_t) cap.ino), cap.cap_id,
                        cap.migrate_seq, cap.seq);
    }
...
class C_Locker_RetryCapRelease:public LockerContext {
  ...
  public:
    C_Locker_RetryCapRelease(Locker * l, client_t c, inodeno_t i, uint64_t id,
                             ceph_seq_t mseq, ceph_seq_t seq):LockerContext(l),
                             ...
                             ceph_seq_t mseq, ceph_seq_t seq)
{
    ...
    Capability *cap = in->get_client_cap(client);
    if (!cap) {
        dout(7) << "_do_cap_release no cap for client" << client << " on " <<
            *in << dendl;
        return;
    }

    dout(7) << "_do_cap_release for client." << client << " on " << *in <<
        dendl;
    if (cap->get_cap_id() != cap_id) {
        dout(7) << " capid " << cap_id << " != " << cap->
    ...
    if (should_defer_client_cap_frozen(in)) {
        ...
        in->add_waiter(CInode::WAIT_UNFREEZE,
                       new C_Locker_RetryCapRelease(this, client, ino, cap_id,
                                                    mseq, seq));
        return;
    }
    if (seq != cap->get_last_issue()) {
        dout(7) << " issue_seq " << seq << " != " << cap->
...
void Locker::remove_client_cap(CInode * in, Capability * cap, bool kill)
{
    ...
    if (in->is_auth()) {
        ...
        if (in->get_projected_inode()->client_ranges.count(client) && !(in->get_inode()->nlink == 0 && !in->is_any_caps())) {   // unless it's unlink + stray
            if (kill)
                in->state_set(CInode::STATE_NEEDSRECOVER);
            else
                check_inode_max_size(in);
...
std::set < client_t > Locker::get_late_revoking_clients(double timeout) constconst
{
...
void Locker::caps_tick()
{
    ...
    dout(20) << __func__ << " " << revoking_caps.
        size() << " revoking caps" << dendl;

    now = ceph_clock_now();
    int n = 0;
    for (xlist < Capability * >::iterator p = revoking_caps.begin(); !p.end();
         ++p) {
        Capability *cap = *p;

        utime_t age = now - cap->get_last_revoke_stamp();
        dout(20) << __func__ << " age = " << age << " client." << cap->
            get_client() << "." << cap->get_inode()->ino() << dendl;
        if (age <= mds->mdsmap->get_session_timeout()) {
            dout(20) << __func__ << " age below timeout " << mds->mdsmap->
        ...
        if (age >
            ...
            *css << "client." << cap->
                ...
                << cap->get_inode()->ino() << " pending " << ccap_string(cap->
                                                                         pending
                                                                         ())
            ...
            dout(20) << __func__ << " silencing log message (backoff) for " <<
                "client." << cap->get_client() << "." << cap->get_inode()->
                ino() << dendl;
        }
    }
}
...
void Locker::handle_client_lease(const cref_t < MClientLease > &m)
{
    ...
    if (!in) {
        dout(7) << "handle_client_lease don't have ino " << m->
            get_ino() << "." << m->get_last() << dendl;
        return;
    }
    CDentry *dn = 0;
    ...
    if (!dn) {
        dout(7) << "handle_client_lease don't have dn " << m->
            get_ino() << " " << m->dname << dendl;
        return;
    }
    dout(10) << " on " << *dn << dendl;
    ...
    case CEPH_MDS_LEASE_RELEASE:
        if (l->seq != m->get_seq()) {
            dout(7) << "handle_client_lease release - seq " << l->
                seq << " != provided " << m->get_seq() << dendl;
        }
        else {
    ...
    case CEPH_MDS_LEASE_RENEW:
        {
            dout(7) << "handle_client_lease client." << client << " renew on "
                << *dn << (!dn->lock.
                           can_lease(client) ? ", revoking lease" : "") <<
                dendl;
            if (dn->lock.can_lease(client)) {
                auto reply = make_message < MClientLease > (*m);
                int pool = 1;   // fixme.. do something smart!
                reply->h.duration_ms =
                    (int)(1000 * mdcache->client_lease_durations[pool]);
                reply->h.seq = ++l->seq;
                reply->clear_payload();
...
void Locker::issue_client_lease(CDentry * dn, CInode * in, MDRequestRef & mdr,
                                utime_t now, bufferlist & bl)
{
    ...
    if (mdr->snapid == CEPH_NOSNAP && dn->lock.can_lease(client) && !diri->is_stray() &&    // do not issue dn leases in stray dir!
        ...
        !(diri->
          get_client_cap_pending(client) & (CEPH_CAP_FILE_SHARED |
                                            CEPH_CAP_FILE_EXCL))) {
        int mask = 0;
        ...
        lstat.duration_ms =
            (uint32_t) (1000 * mdcache->client_lease_durations[pool]);
        lstat.seq = ++l->seq;
        lstat.alternate_name = std::string(dn->alternate_name);
        encode_lease(bl, session->info, lstat);
        dout(20) << "issue_client_lease seq " << lstat.seq << " dur " << lstat.
            duration_ms << "ms " << " on " << *dn << dendl;
    }
    else {
        // null lease
        LeaseStat lstat;
...
void Locker::revoke_client_leases(SimpleLock * lock)
{
    ...
    for (map < client_t, ClientLease * >::iterator p =
         dn->client_lease_map.begin(); p != dn->client_lease_map.end(); ++p) {
        ...
        CInode *diri = dn->get_dir()->get_inode();
        auto lease =
            make_message < MClientLease > (CEPH_MDS_LEASE_REVOKE, l->seq, mask,
                                           diri->ino(), diri->first,
    ...
    case CEPH_LOCK_DN:
        {
            ...
            if (!dn) {
                dout(7) << "get_lock don't have dn " << info.dirfrag.
                    ino << " " << info.dname << dendl;
                return 0;
            }
            return &dn->lock;
        }
...
{
    // nobody should be talking to us during recovery.
    ceph_assert(mds->is_rejoin() || mds->is_clientreplay() || mds->is_active()
                || mds->is_stopping());

    SimpleLock *lock = get_lock(m->get_lock_type(), m->get_object_info());
    if (!lock) {
        dout(10) << "don't have object " << m->
...
void Locker::handle_reqrdlock(SimpleLock * lock, const cref_t < MLock > &m)
{
    ...
    if (parent->is_auth() &&
        lock->get_state() != LOCK_SYNC && !parent->is_frozen()) {
        ...
        if (lock->is_stable()) {
            ...
            dout(7) << "handle_reqrdlock delaying request until lock is stable"
                << dendl;
            lock->
                add_waiter(SimpleLock::WAIT_STABLE | MDSCacheObject::
                           WAIT_UNFREEZE, new C_MDS_RetryMessage(mds, m));
        }
    }
    else {
...
void Locker::handle_simple_lock(SimpleLock * lock, const cref_t < MLock > &m)
{
    ...
    if (mds->is_rejoin()) {
        if (lock->get_parent()->is_rejoining()) {
            dout(7) << "handle_simple_lock still rejoining " << *lock->
                get_parent()
                << ", dropping " << *m << dendl;
            return;
        }
    ...
    case LOCK_AC_LOCKACK:
        ...
        if (lock->is_gathering()) {
            dout(7) << "handle_simple_lock " << *lock << " on " << *lock->
                get_parent() << " from " << from << ", still gathering " <<
                lock->get_gather_set() << dendl;
        }
        else {
            dout(7) << "handle_simple_lock " << *lock << " on " << *lock->
                get_parent() << " from " << from << ", last one" << dendl;
            eval_gather(lock);
        }
        break;
...
{
    dout(10) << "simple_eval " << *lock << " on " << *lock->
        get_parent() << dendl;
    ...
    if (lock->get_state() != LOCK_EXCL &&
        in && in->get_target_loner() >= 0 && (wanted & CEPH_CAP_GEXCL)) {
    ...
    // stable -> sync?
             ...
             ((!(wanted & CEPH_CAP_GEXCL)
              ...
              || (lock->get_state() == LOCK_EXCL && in
                  && in->get_target_loner() < 0))) {
        dout(7) << "simple_eval stable, syncing " << *lock << " on " << *lock->
            get_parent() << dendl;
        simple_sync(lock, need_issue);
    }
}
...
{
    dout(7) << "simple_sync on " << *lock << " on " << *lock->
        get_parent() << dendl;
    ceph_assert(lock->get_parent()->is_auth());
    ceph_assert(lock->is_stable());
...
{
    dout(7) << "simple_excl on " << *lock << " on " << *lock->
        get_parent() << dendl;
    ceph_assert(lock->get_parent()->is_auth());
    ceph_assert(lock->is_stable());
...
{
    dout(7) << "simple_lock on " << *lock << " on " << *lock->
        get_parent() << dendl;
    ceph_assert(lock->get_parent()->is_auth());
    ceph_assert(lock->is_stable());
    ceph_assert(lock->get_state() != LOCK_LOCK);
    ...
    if (lock->get_parent()->is_replicated() &&
        ...
        dout(10) <<
            " doing local stage of mix->lock gather before gathering from replicas"
            << dendl;
    }
    else {
        ...
        if (lock->get_parent()->is_replicated() && lock->get_sm()->states[old_state].replica_state != LOCK_LOCK) {  // replica may already be LOCK
            gather++;
            send_lock_message(lock, LOCK_AC_LOCK);
            lock->init_gather();
        }
    ...
    if (gather) {
        ...
        lock->set_state(LOCK_LOCK);
        lock->
            finish_waiters(ScatterLock::WAIT_XLOCK | ScatterLock::
                           WAIT_WR | ScatterLock::WAIT_STABLE);
    }
}

void Locker::simple_xlock(SimpleLock * lock)
{
    dout(7) << "simple_xlock on " << *lock << " on " << *lock->
        get_parent() << dendl;
    ceph_assert(lock->get_parent()->is_auth());
...
class C_Locker_ScatterWB:public LockerLogContext {
    ...
    MutationRef mut;
  public:
    C_Locker_ScatterWB(Locker * l, ScatterLock * sl,
                       MutationRef & m):LockerLogContext(l), lock(sl), mut(m) {
    } void finish(int r) override {
...
{
    CInode *in = static_cast < CInode * >(lock->get_parent());
    dout(10) << "scatter_writebehind " << in->get_inode()->
        mtime << " on " << *lock << " on " << *in << dendl;
...
{
    dout(10) << "scatter_eval " << *lock << " on " << *lock->
        get_parent() << dendl;
    ...
    if (!lock->is_rdlocked() &&
        lock->get_state() != LOCK_MIX && lock->get_scatter_wanted()) {
 ...
 * inode.dirstat.
 */
void Locker::scatter_nudge(ScatterLock * lock, MDSContext * c,
    ...
    if (p->is_auth()) {
        ...
        while (true) {
            if (lock->is_stable()) {
                ...
                if (mdcache->is_readonly()) {
                    if (lock->get_state() != LOCK_SYNC) {
                        dout(10) << "scatter_nudge auth, read-only FS, syncing "
                            << *lock << " on " << *p << dendl;
                ...
                if (lock->is_stable() && count == 2) {
                    dout(10) << "scatter_nudge oh, stable after two cycles." <<
                        dendl;
                ...
                dout(10) << "scatter_nudge auth, waiting for stable " << *lock
                    << " on " << *p << dendl;
                if (c)
                    lock->add_waiter(SimpleLock::WAIT_STABLE, c);
                return;
        ...
        if (!mds->is_cluster_degraded()
            || mds->mdsmap->is_clientreplay_or_active_or_stopping(auth)) {
            mds->send_message_mds(make_message < MLock >
                                  (lock, LOCK_AC_NUDGE, mds->get_nodeid()),
                                  auth);
        }
...
void Locker::scatter_tempsync(ScatterLock * lock, bool * need_issue)
{
    ...
    if (lock->get_cap_shift() &&
        in->is_head() && in->issued_caps_need_gather(lock)) {
        if (need_issue)
            *need_issue = true;
        else
            issue_caps(in);
...
void Locker::local_wrlock_finish(const MutationImpl::lock_iterator & it,
                                 MutationImpl * mut)
{
    ceph_assert(it->is_wrlock());
    ...
    if (lock->get_num_wrlocks() == 0) {
        lock->finish_waiters(SimpleLock::WAIT_STABLE |
                             SimpleLock::WAIT_WR | SimpleLock::WAIT_RD);
    }
}
...
void Locker::local_xlock_finish(const MutationImpl::lock_iterator & it,
                                MutationImpl * mut)
{
    ceph_assert(it->is_xlock());
    LocalLockC *lock = static_cast < LocalLockC * >(it->lock);
...
void Locker::file_eval(ScatterLock * lock, bool * need_issue)
{
    ...
    if (lock->get_state() == LOCK_EXCL) {
        ...
        dout(7) << "file_eval loner_issued=" << gcap_string(loner_issued)
            ...
            << dendl;
        if (!((loner_wanted | loner_issued) & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GBUFFER)) || (other_wanted & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GRD)) || (in->is_dir() && in->multiple_nonstale_caps())) {   // FIXME.. :/
    ...
    // * -> sync?
             ...
             !lock->is_waiter_for(SimpleLock::WAIT_WR) && !(wanted & CEPH_CAP_GWR) && !((lock->get_state() == LOCK_MIX) && in->is_dir() && in->has_subtree_or_exporting_dirfrag())  // if we are a delegation point, stay where we are
             //((wanted & CEPH_CAP_RD) || 
             //in->is_replicated() || 
             //lock->is_leased() ||
...
{
    dout(7) << "scatter_mix " << *lock << " on " << *lock->
        get_parent() << dendl;
...
void Locker::file_excl(ScatterLock * lock, bool * need_issue)
{
    ...
    ceph_assert((in->get_loner() >= 0 && in->get_mds_caps_wanted().empty()) || (lock->get_state() == LOCK_XSYN));   // must do xsyn -> excl -> <anything else>
    ...
    if (in->is_replicated() && lock->get_state() != LOCK_LOCK_EXCL && lock->get_state() != LOCK_XSYN_EXCL) {    // if we were lock, replicas are already lock.
        send_lock_message(lock, LOCK_AC_LOCK);
        lock->init_gather();
        gather++;
    }
...
{
    dout(7) << "file_xsyn on " << *lock << " on " << *lock->
        get_parent() << dendl;
    CInode *in = static_cast < CInode * >(lock->get_parent());
    ceph_assert(in->is_auth());
...
void Locker::handle_file_lock(ScatterLock * lock, const cref_t < MLock > &m)
{
    ...
    dout(7) << "handle_file_lock a=" << lock->get_lock_action_name(m->
                                                                   get_action())
        << " on " << *lock << " from mds." << from << " " << *in << dendl;
    ...
    case LOCK_AC_REQSCATTER:
        if (lock->is_stable()) {
            ...
            dout(7) <<
                "handle_file_lock got scatter request, !stable, marking scatter_wanted on "
                << *lock << " on " << *lock->get_parent() << dendl;
    ...
    case LOCK_AC_REQUNSCATTER:
        if (lock->is_stable()) {
            ...
            dout(7) << "handle_file_lock ignoring unscatter request on " <<
                *lock << " on " << *lock->get_parent() << dendl;
    ...
    case LOCK_AC_NUDGE:
        if (!lock->get_parent()->is_auth()) {
            ...
            dout(7) << "handle_file_lock IGNORING nudge on non-replicated " <<
                *lock << " on " << *lock->get_parent() << dendl;
        }
        else {
