...
void aws_response_handler::push_header(const char *header_name,
...
int aws_response_handler::create_header_records()
{
    ...
    //1
    push_header(header_name_str[IDX(header_name_En::EVENT_TYPE)],
    ...
    //3
    push_header(header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
...
int aws_response_handler::create_error_header_records(const char *error_message)
{
    ...
    //[header-name-byte-length:1][header-name:variable-length][header-value-type:1][header-value:variable-length]
    //1
    push_header(header_name_str[IDX(header_name_En::ERROR_CODE)],
                header_value_str[IDX(header_value_En::ENGINE_ERROR)]);
    //2
    push_header(header_name_str[IDX(header_name_En::ERROR_MESSAGE)],
                error_message);
    //3
    push_header(header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
...
void aws_response_handler::send_progress_response()
{
    std::string progress_payload =
        fmt::
        format
        ("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Progress><BytesScanned>{}</BytesScanned><BytesProcessed>{}</BytesProcessed><BytesReturned>{}</BytesReturned></Progress>",
         get_processed_size(), get_processed_size(),
...
void aws_response_handler::send_stats_response()
{
    std::string stats_payload =
        fmt::
        format
        ("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Stats><BytesScanned>{}</BytesScanned><BytesProcessed>{}</BytesProcessed><BytesReturned>{}</BytesReturned></Stats>",
         get_processed_size(), get_processed_size(),
         get_total_bytes_returned());
                                                  ...
                                                  size_t input_length)
{
    ...
    m_s3_csv_object.set_result_formatters(fp_s3select_result_format,
...
#define APOS "&apos;"
    ...
    m_s3select_input =
        m_s3select_query.substr(_qi + input_tag.size() + 2,
                                _qe - (_qi + input_tag.size() + 2));
    ...
    m_s3select_output =
        m_s3select_query.substr(_qo + output_tag.size() + 2,
                                _qs - (_qo + output_tag.size() + 2));
    extract_by_tag(m_s3select_output, "FieldDelimiter",
                   output_column_delimiter);
    extract_by_tag(m_s3select_output, "QuoteCharacter", output_quot);
    extract_by_tag(m_s3select_output, "QuoteEscapeCharacter",
                   output_escape_char);
                                            ...
                                            void *buff, optional_yield y)
{
    ...
    range_req_str =
        "bytes=" + std::to_string(ofs) + "-" + std::to_string(ofs + len - 1);
                                              ...
                                              off_t len)
{
    ...
    if (s->obj_size == 0 || m_object_size_for_processing == 0) {
      ...
      for (auto & it:bl.buffers()) {
            ...
            status =
                run_s3select_on_json(m_sql_query.c_str(), &(it)[0] + ofs, len);
            if (status < 0) {
                status = -EINVAL;
                break;
            }
