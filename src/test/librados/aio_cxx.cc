...
TEST(LibRadosAio, TooBigPP)
{
    ...
    ASSERT_EQ(-E2BIG,
              test_data.m_ioctx.aio_write(test_data.m_oid, aio_completion.get(),
                                          bl, UINT_MAX, 0));
    ASSERT_EQ(-E2BIG,
              test_data.m_ioctx.aio_append(test_data.m_oid,
                                           aio_completion.get(), bl, UINT_MAX));
...
TEST(LibRadosAio, WaitForSafePP)
{
    ...
    bl1.append(buf, sizeof(buf));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_write(test_data.m_oid, my_completion.get(),
                                          bl1, sizeof(buf), 0));
    TestAlarm alarm;
    ASSERT_EQ(0, my_completion->wait_for_complete());
    ASSERT_EQ(0, my_completion->get_return_value());
...
TEST(LibRadosAio, RoundTripPP)
{
    ...
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_read(test_data.m_oid, my_completion2.get(),
                                         &bl2, sizeof(buf), 0));
    {
        TestAlarm alarm;
...
TEST(LibRadosAioPP, RmXattrPP)
{
    ...
    auto my_completion2 =
        std::unique_ptr < AioCompletion > { Rados::aio_create_completion() };
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_rmxattr(test_data.m_oid,
                                            my_completion2.get(), attr1));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
...
TEST(LibRadosAio, RoundTripWriteFullPP)
{
    ...
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_read(test_data.m_oid, my_completion3.get(),
                                         &bl3, sizeof(buf), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
...
TEST(LibRadosAio, SimpleStatPPNS)
{
    ...
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_stat(test_data.m_oid, my_completion2.get(),
                                         &psize, &pmtime));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
...
TEST(LibRadosAio, OperateMtime)
{
    ...
    time_t set_mtime = 1457129052;
    {
        ...
        op.create(false);
        ASSERT_EQ(0,
                  test_data.m_ioctx.aio_operate(test_data.m_oid, c.get(), &op));
        {
            TestAlarm alarm;
...
TEST(LibRadosAio, StatRemovePP)
{
    ...
    ASSERT_TRUE(my_completion3);
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_remove(test_data.m_oid,
                                           my_completion3.get()));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
    ...
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_stat(test_data.m_oid, my_completion4.get(),
                                         &psize2, &pmtime2));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion4->wait_for_complete());
...
TEST(LibRadosAio, ExecuteClassPP)
{
    ...
    bufferlist in, out;
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_exec(test_data.m_oid, my_completion2.get(),
...
TEST(LibRadosAio, MultiWritePP)
{
    ...
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_write(test_data.m_oid, my_completion2.get(),
...
TEST(LibRadosAio, AioUnlockPP)
{
    ...
    auto my_completion =
        std::unique_ptr < AioCompletion > { Rados::aio_create_completion() };
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_unlock(test_data.m_oid, "TestLock",
                                           "Cookie", my_completion.get()));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    }
...
TEST(LibRadosAio, RoundTripAppendPP)
{
    ...
    bl1.append(buf, sizeof(buf));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_append(test_data.m_oid, my_completion.get(),
                                           bl1, sizeof(buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
...
TEST(LibRadosAioPP, RemoveTestPP)
{
    ...
    bufferlist bl2;
    ASSERT_EQ(-ENOENT,
              test_data.m_ioctx.read(test_data.m_oid, bl2, sizeof(buf), 0));
}
...
TEST(LibRadosAioEC, RoundTripAppendPP)
{
    ...
    ASSERT_TRUE(my_completion4);
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_read(test_data.m_oid, my_completion4.get(),
                                         &bl3, bsize * 3, 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion4->wait_for_complete());
...
TEST(LibRadosAio, RoundTripCmpExtPP)
{
    ...
    auto my_completion2 =
        std::unique_ptr < AioCompletion > { Rados::aio_create_completion() };
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_cmpext(test_data.m_oid,
                                           my_completion2.get(), 0, cbl));
...
TEST(LibRadosAio, PoolEIOFlag)
{
    ...
    for (; min_failed == 0 && i <= timeout; ++i) {
        ...
        my_lock.unlock();
        int r =
            test_data.m_ioctx.aio_write(test_data.m_oid, info->c, bl,
                                        bl.length(), 0);
...
TEST(LibRadosAio, MultiReads)
{
    ...
    std::deque < std::pair < bufferlist,
        std::unique_ptr < AioCompletion >>> reads;
    for (int i = 0; i < 100; i++) {
