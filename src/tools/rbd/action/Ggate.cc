...
{
#if !defined(__FreeBSD__)
    std::cerr << "rbd: ggate is only supported on FreeBSD" << std::
              endl;
    return -EOPNOTSUPP;
#else
                   ...
                   &ceph_global_init_args)
{
    ...
    std::cerr << "rbd: ggate is only supported on FreeBSD" << std::
              endl;
#else
    std::cerr << "rbd: ggate attach command not supported" << std::
              endl;
#endif
    return -EOPNOTSUPP;
}
                   ...
                   &ceph_global_init_args)
{
    ...
    std::cerr << "rbd: ggate is only supported on FreeBSD" << std::
              endl;
#else
    std::cerr << "rbd: ggate detach command not supported" << std::
              endl;
#endif
    return -EOPNOTSUPP;
}
