...
namespace librbd {
    namespace api {
        ...
        namespace {
            ...
            class GetGroupVisitor {
              public:
                ...
                explicit GetGroupVisitor(CephContext * cct,
                ...
                template < typename T > inline int operator() (const T &)const {
                    ...
                    return -EINVAL;
                } inline int operator() (const cls::rbd::
                                         GroupSnapshotNamespace &
                                         snap_namespace) {
                    IoCtx group_ioctx;
                    int r =
                        util::create_ioctx(*image_ioctx, "group",
                                           snap_namespace.group_pool,
                                           { }, &group_ioctx);
                    if (r < 0) {
                        return r;
                    ...
                    r = cls_client::dir_get_name(&group_ioctx,
                                                 RBD_GROUP_DIRECTORY,
                                                 snap_namespace.group_id,
                                                 &group_name);
                    if (r < 0) {
                        lderr(cct) << "failed to retrieve group name: " <<
                            cpp_strerror(r)
                            << dendl;
                        return r;
                    }
              ...
              public:
                ...
                :mirror_snap(mirror_snap) {
                    ...
                    return -EINVAL;
                                         ...
                                         snap_namespace) {
                    mirror_snap->state =
                        static_cast < snap_mirror_state_t >
                        (snap_namespace.state);
                    mirror_snap->complete = snap_namespace.complete;
                    mirror_snap->mirror_peer_uuids =
                        snap_namespace.mirror_peer_uuids;
                    mirror_snap->primary_mirror_uuid =
                        snap_namespace.primary_mirror_uuid;
                    mirror_snap->primary_snap_id =
                        snap_namespace.primary_snap_id;
        ...
        template < typename I >
            ...
            GetGroupVisitor ggv =
                GetGroupVisitor(ictx->cct, &ictx->md_ctx, group_snap);
        ...
        template < typename I >
            int Snapshot < I >::get_mirror_namespace(I * ictx, uint64_t snap_id,
        ...
        template < typename I >
            ...
            ldout(ictx->cct,
                  20) << "snap_remove " << ictx << " " << snap_id << dendl;
            ...
            C_SaferCond ctx;
            ictx->operations->snap_remove(snapshot_namespace, snapshot_name,
                                          &ctx);
            r = ctx.wait();
            return r;
        }
        ...
        template < typename I >
            int Snapshot < I >::get_name(I * ictx, uint64_t snap_id,
                                         std::string * snap_name) {
            ldout(ictx->cct,
                  20) << "snap_get_name " << ictx << " " << snap_id << dendl;
        ...
        template < typename I >
            int Snapshot < I >::get_id(I * ictx, const std::string & snap_name,
                                       uint64_t * snap_id) {
            ldout(ictx->cct,
                  20) << "snap_get_id " << ictx << " " << snap_name << dendl;
        ...
        template < typename I >
            ...
            ictx->image_lock};
            *exists =
                ictx->get_snap_id(snap_namespace, snap_name) != CEPH_NOSNAP;
            return 0;
        }

        template < typename I >
            int Snapshot < I >::create(I * ictx, const char *snap_name,
                                       uint32_t flags, ProgressContext & pctx) {
            ldout(ictx->cct,
                  20) << "snap_create " << ictx << " " << snap_name <<
                " flags: " << flags << dendl;
            ...
            return ictx->operations->
                snap_create(cls::rbd::UserSnapshotNamespace(), snap_name,
                            internal_flags, pctx);
        }

        template < typename I >
            int Snapshot < I >::remove(I * ictx, const char *snap_name,
                                       uint32_t flags, ProgressContext & pctx) {
            ldout(ictx->cct,
                  20) << "snap_remove " << ictx << " " << snap_name <<
                " flags: " << flags << dendl;
            ...
            if (protect && flags & RBD_SNAP_REMOVE_UNPROTECT) {
                ...
                if (protect) {
                    lderr(ictx->
                          cct) <<
                        "snapshot is still protected after unprotection" <<
                        dendl;
                    ceph_abort();
                }
            }
        ...
        template < typename I >
            int Snapshot < I >::get_timestamp(I * ictx, uint64_t snap_id,
        ...
        template < typename I >
            ...
            int r =
                cls_client::snapshot_get_limit(&ictx->md_ctx, ictx->header_oid,
                                               limit);
            if (r == -EOPNOTSUPP) {
                *limit = UINT64_MAX;
                r = 0;
        ...
        template < typename I >
            int Snapshot < I >::is_protected(I * ictx, const char *snap_name,
                                             bool * protect) {
            ldout(ictx->cct,
                  20) << "snap_is_protected " << ictx << " " << snap_name <<
                dendl;
        ...
        template < typename I >
            int Snapshot < I >::get_namespace(I * ictx, const char *snap_name,
                                              ...
                                              snap_namespace) {
            ldout(ictx->cct,
                  20) << "get_snap_namespace " << ictx << " " << snap_name <<
                dendl;
