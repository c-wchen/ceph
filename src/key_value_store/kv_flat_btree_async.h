...
struct object_data {
    ...
    object_data(key_data min, key_data kdat, std::string the_name)
        :
        min_kdata(min),
        max_kdata(kdat), name(the_name), unwritable(false), version(0), size(0)
    {
    }
    ...
    object_data(key_data min, key_data kdat, std::string the_name,
        ...
        min_kdata(min),
        max_kdata(kdat),
        name(the_name), omap(the_omap), unwritable(false), version(0), size(0)
    {
    }
    ...
    object_data(key_data min, key_data kdat, std::string the_name, int the_version)
        : min_kdata(min),
          max_kdata(kdat),
          name(the_name), unwritable(false), version(the_version), size(0)
    {
    }
...
public:
     ...
     */
    int get(const std::string &key, index_data *idata,
...
protected:
     ...
     */
    void set_up_prefix_index(const std::vector < object_data > &to_create,
     ...
     */
    void set_up_ops(const std::vector < object_data > &create_vector,
     ...
     * unwritable to "0"
     */
    void set_up_make_object(const std::map < std::string, bufferlist > &to_set,
                            librados::ObjectWriteOperation *owo);
     ...
     */
    void set_up_unwrite_object(const int &ver,
                               librados::ObjectWriteOperation *owo);
     ...
     * until success.
     */
    int remove_op(const std::string &key, index_data &idata,
...
public:
    ...
    KvFlatBtreeAsync(int k_val, std::string name, int cache, double cache_r,
        ...
        : k(k_val),
          ...
          timeout(100000, 0),
          cache_size(cache),
          cache_refresh(cache_r), verbose(verb), client_index(0), icache(cache)
    {
    }
    /**
    ...
    void aio_set(const std::string &key, const bufferlist &val,
                 bool exclusive, callback cb, void *cb_args, int *err) override;
    void aio_remove(const std::string &key, callback cb, void *cb_args,
                    int *err) override;
    ...
    int get_all_keys_and_values(std::map < std::string,
                                bufferlist > *kv_map) override;
