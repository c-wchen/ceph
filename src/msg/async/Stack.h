...
class ServerSocket {
  ...
  public:
    ...
    int accept(ConnectedSocket * sock, const SocketOptions & opt,
...
class Worker {
  ...
  public:
    ...
    :cct(c), perf_logger(NULL), id(i), references(0), center(c) {
        ...
        PerfCountersBuilder plb(cct, name, l_msgr_first, l_msgr_last);
         ...
         plb.add_u64_counter(l_msgr_recv_messages, "msgr_recv_messages",
                             "Network received messages");
         plb.add_u64_counter(l_msgr_send_messages, "msgr_send_messages",
                             "Network sent messages");
         plb.add_u64_counter(l_msgr_recv_bytes, "msgr_recv_bytes",
                             "Network received bytes", NULL, 0, unit_t(BYTES));
         plb.add_u64_counter(l_msgr_send_bytes, "msgr_send_bytes",
                             "Network sent bytes", NULL, 0, unit_t(BYTES));
         plb.add_u64_counter(l_msgr_active_connections,
                             "msgr_active_connections",
                             "Active connection number");
         plb.add_u64_counter(l_msgr_created_connections,
                             "msgr_created_connections",
                             "Created connection number");

         plb.add_time(l_msgr_running_total_time, "msgr_running_total_time",
                      "The total time of thread running");
         plb.add_time(l_msgr_running_send_time, "msgr_running_send_time",
                      "The total time of message sending");
         plb.add_time(l_msgr_running_recv_time, "msgr_running_recv_time",
                      "The total time of message receiving");
         plb.add_time(l_msgr_running_fast_dispatch_time,
    ...
    virtual int connect(const entity_addr_t & addr,
                        const SocketOptions & opts, ConnectedSocket * socket) =
        0;
...
class NetworkStack:public CephContext::ForkWatcher {
  ...
  public:
    ...
    ~NetworkStack() override {
      for (auto && w:workers)
            delete w;
    } static std::shared_ptr < NetworkStack > create(CephContext * c,
                                                     const string & type);

    static Worker *create_worker(CephContext * c, const string & t, unsigned i);
    // backend need to override this method if supports zero copy read
    virtual bool support_zero_copy_read() const {
        return false;
    }
    // backend need to override this method if backend doesn't support shared// listen table.// For example, posix backend has in kernel global listen table. If one// thread bind a port, other threads also aware this.// But for dpdk backend, we maintain listen table in each thread. So we// need to let each thread do binding port.
        virtual bool support_local_listen_table() const {
        return false;
    } virtual bool nonblock_connect_need_writable_event() const {
        return true;
    } void start();
    void stop();
