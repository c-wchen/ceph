...
bool Cursor::has_prev() const const
{
    std::lock_guard < std::mutex > lock(*mutex);
    return epoch > history->get_oldest_epoch();
}

bool Cursor::has_next() const const
{
    std::lock_guard < std::mutex > lock(*mutex);
...
Cursor RGWPeriodHistory::Impl::attach(RGWPeriod && period)
{
    ...
    for (;;) {
        ...
        if (predecessor_id.empty()) {
            lderr(cct) << "reached a period with an empty predecessor id" <<
                dendl;
            return Cursor {
            -EINVAL};
        }
...
Cursor RGWPeriodHistory::get_current() const const
{
