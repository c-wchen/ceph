...
public:
    JSONObjIter();
    ~JSONObjIter();
    void set(const JSONObjIter::map_iter_t &_cur,
...
public:
    ...
    friend std::ostream &operator<<(std::ostream &out, const JSONObj &obj);     // does not work, FIXME
...
void encode_json(const char *name, const JSONObj::data_val &v,
                 ceph::Formatter *f);
...
public:
    ...
    template < class T >
    static void decode_json(const char *name, T &val,
                            const T &default_val, JSONObj *obj);

    template < class T >
    static bool decode_json(const char *name, boost::optional < T > &val,
                            JSONObj *obj, bool mandatory = false);

    template < class T >
    static bool decode_json(const char *name, std::optional < T > &val,
                            JSONObj *obj, bool mandatory = false);
...
void decode_json_obj(boost::container::flat_set < T, Compare, Alloc > &l,
                     JSONObj *obj)
{
    l.clear();
...
template < class K, class V, class C =
           std::less < K > >void decode_json_obj(std::map < K, V, C > &m,
                   JSONObj *obj)
{
    m.clear();
...
template < class K, class V, class C =
           std::less < K > >void decode_json_obj(boost::container::flat_map < K, V,
                   C > &m, JSONObj *obj)
{
    m.clear();
...
template < class C >
void decode_json_obj(C &container, void (*cb)(C &, JSONObj *obj),
                     JSONObj *obj)
{
    container.clear();
...
template < class T >
bool JSONDecoder::decode_json(const char *name, T &val, JSONObj *obj,
                              bool mandatory)
{
...
template < class C >
bool JSONDecoder::decode_json(const char *name, C &container,
                              void (*cb)(C &, JSONObj *), JSONObj *obj,
                              bool mandatory)
{
    container.clear();
...
template < class T >
void JSONDecoder::decode_json(const char *name, T &val,
                              const T &default_val, JSONObj *obj)
{
    JSONObjIter iter = obj->find_first(name);
    if (iter.end()) {
...
template < class T >
bool JSONDecoder::decode_json(const char *name, boost::optional < T > &val,
                              JSONObj *obj, bool mandatory)
{
    JSONObjIter iter = obj->find_first(name);
    if (iter.end()) {
        if (mandatory) {
...
template < class T >
bool JSONDecoder::decode_json(const char *name, std::optional < T > &val,
                              JSONObj *obj, bool mandatory)
{
    JSONObjIter iter = obj->find_first(name);
    if (iter.end()) {
...
public:
    ...
    public:
        ...
        virtual void encode_json(const char *name, const void *pval,
...
template < class T >
static void encode_json_impl(const char *name, const T &val,
                             ceph::Formatter *f)
{
...
template < class T >
static void encode_json(const char *name, const T &val,
                        ceph::Formatter *f)
{
    JSONEncodeFilter *filter =
        static_cast <
        JSONEncodeFilter *
...
void encode_json(const char *name, const std::string &val,
                 ceph::Formatter *f);
...
void encode_json(const char *name, const ceph::buffer::list &bl,
                 ceph::Formatter *f);
...
template < class T >
static void encode_json(const char *name, const std::list < T > &l,
                        ceph::Formatter *f)
{
...
template < class T >
static void encode_json(const char *name, const std::deque < T > &l,
                        ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
...
template < class T, class Compare =
           std::less < T > >static void encode_json(const char *name,
                   const std::set < T, Compare > &l,
                   ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
...
static void encode_json(const char *name,
                        const boost::container::flat_set < T, Compare,
                        Alloc > &l, ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
        encode_json("obj", *iter, f);
...
template < class T >
static void encode_json(const char *name, const std::vector < T > &l,
                        ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
...
static void encode_json(const char *name, const std::map < K, V, C > &m,
                        ceph::Formatter *f)
{
    f->open_array_section(name);
...
template < class K, class V, class C =
           std::less < K > >static void encode_json(const char *name,
                   const boost::container::flat_map <
                   K, V, C > &m, ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto i = m.cbegin(); i != m.cend(); ++i) {
        f->open_object_section("entry");
...
template < class K, class V >
static void encode_json(const char *name, const std::multimap < K, V > &m,
                        ceph::Formatter *f)
{
    f->open_array_section(name);
...
template < class K, class V >
void encode_json_map(const char *name, const std::map < K, V > &m,
                     ceph::Formatter *f)
{
    f->open_array_section(name);
...
void encode_json_map(const char *name, const char *index_name,
                     ...
                     void (*cb)(const char *, const V &, ceph::Formatter *,
                                void *), void *parent, const std::map < K,
                     V > &m, ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto iter = m.cbegin(); iter != m.cend(); ++iter) {
        if (index_name) {
...
{
    encode_json_map < K, V > (name, index_name, object_name, value_name, NULL,
                              NULL, m, f);
}

template < class K, class V >
void encode_json_map(const char *name, const char *index_name,
                     const char *value_name, const std::map < K, V > &m,
                     ceph::Formatter *f)
{
    encode_json_map < K, V > (name, index_name, NULL, value_name, NULL, NULL, m,
                              f);
}

template < class T >
static void encode_json(const char *name, const std::optional < T > &o,
                        ceph::Formatter *f)
{
    if (!o) {
        return;
...
template < class K, class V >
void encode_json_map(const char *name, const boost::container::flat_map < K,
                     V > &m, ceph::Formatter *f)
{
    f->open_array_section(name);
    for (auto iter = m.cbegin(); iter != m.cend(); ++iter) {
...
protected:
    bool handle_value(std::string_view name, std::string_view s,
                      bool quoted) override;
    bool handle_open_section(std::string_view name, const char *ns,
...
public:
    ...
    {
        return val_bool();
    } template < class T > T operator[](const std::string &name)const
    {
...
void encode_json(const char *name, const JSONFormattable &v,
                 ceph::Formatter *f);
