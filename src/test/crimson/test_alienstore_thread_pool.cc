...
seastar::future <> test_accumulate(ThreadPool & tp)
{
    ...
    auto slow_plus =[&tp] (int i){
        return tp.submit(::rand() % 2,[ =] {
                                            std::this_thread::sleep_for(10 ns);
                                            return i + M;}
                         );};
                         return seastar::map_reduce(boost::irange(0, N),
                                                    slow_plus, 0, std::plus {
                                                    }
                         ).then([](int sum) {
                                ...
                                throw std::
                                runtime_error("test_accumulate failed");}
                                }
                         );}
                         ...
                         seastar::app_template app; return app.run(argc, argv,[] {
                                                                   std::vector <
                                                                   const char
                                                                   *>args;
                                                                   std::
