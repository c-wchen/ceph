        ...
        namespace openssl {
            ...
            struct TestCryptoOpensslDataCryptor:public TestFixture {
                ...
                void SetUp() override {
                    TestFixture::SetUp();
                    cryptor =
                        new DataCryptor(reinterpret_cast <
                                        CephContext * >(m_ioctx.cct()));
                    ASSERT_EQ(0,
                              cryptor->init(TEST_CIPHER_NAME, TEST_KEY,
                                            sizeof(TEST_KEY)));
            ...
            TEST_F(TestCryptoOpensslDataCryptor, InvalidCipherName) {
                EXPECT_EQ(-EINVAL,
                          cryptor->init(nullptr, TEST_KEY, sizeof(TEST_KEY)));
                EXPECT_EQ(-EINVAL,
                          cryptor->init("", TEST_KEY, sizeof(TEST_KEY)));
                EXPECT_EQ(-EINVAL,
                          cryptor->init("Invalid", TEST_KEY, sizeof(TEST_KEY)));
            }
            ...
            TEST_F(TestCryptoOpensslDataCryptor, InvalidKey) {
                ...
                EXPECT_EQ(-EINVAL, cryptor->init(TEST_CIPHER_NAME, nullptr,
                                                 sizeof(TEST_KEY)));
                EXPECT_EQ(-EINVAL,
                          cryptor->init(TEST_CIPHER_NAME, TEST_KEY, 1));
            }

            TEST_F(TestCryptoOpensslDataCryptor, GetContextInvalidMode) {
                EXPECT_EQ(nullptr,
                          cryptor->get_context(static_cast < CipherMode >
                                               (-1)));
            }

            TEST_F(TestCryptoOpensslDataCryptor, ReturnNullContext) {
                cryptor->return_context(nullptr,
                                        static_cast < CipherMode > (-1));
            }
            ...
            TEST_F(TestCryptoOpensslDataCryptor, EncryptDecrypt) {
                ...
                ASSERT_EQ(sizeof(TEST_DATA),
                          cryptor->update_context(ctx, TEST_DATA, out,
                                                  sizeof(TEST_DATA)));
                ...
                ASSERT_EQ(0,
                          cryptor->init_context(ctx, TEST_IV, sizeof(TEST_IV)));
                ASSERT_EQ(sizeof(TEST_DATA),
                          cryptor->update_context(ctx, out, out,
                                                  sizeof(TEST_DATA)));
            ...
            TEST_F(TestCryptoOpensslDataCryptor, ReuseContext) {
                ...
                ASSERT_EQ(0,
                          cryptor->init_context(ctx, TEST_IV_2,
                                                sizeof(TEST_IV_2)));
                ASSERT_EQ(sizeof(TEST_DATA),
                          cryptor->update_context(ctx, TEST_DATA, out,
                                                  sizeof(TEST_DATA)));
                ...
                ASSERT_EQ(0,
                          cryptor->init_context(ctx2, TEST_IV_2,
                                                sizeof(TEST_IV_2)));
                unsigned char out2[sizeof(TEST_DATA)];
                ASSERT_EQ(sizeof(TEST_DATA),
                          cryptor->update_context(ctx2, TEST_DATA, out2,
                                                  sizeof(TEST_DATA)));
