...
namespace crimson::osd::scheduler {
    ...
    mClockScheduler::mClockScheduler(ConfigProxy & conf):scheduler(std::
                                                                   bind
                                                                   (&mClockScheduler::
                                                                    ClientRegistry::
                                                                    get_info,
                                                                   ...
                                                                   ("osd_mclock_scheduler_anticipation_timeout"))
    {
    ...
    } void mClockScheduler::ClientRegistry::
        update_from_config(const ConfigProxy & conf) {
        default_external_client_info.update(conf.get_val <
                                            double
                                            >
        ...
        internal_client_infos[static_cast < size_t >
            ...
            update(conf.get_val <
                   double >("osd_mclock_scheduler_background_best_effort_res"),
                   conf.get_val < uint64_t >
                   ("osd_mclock_scheduler_background_best_effort_wgt"),
                   conf.get_val <
                   double >("osd_mclock_scheduler_background_best_effort_lim"));
    }

    const dmc::ClientInfo *
        mClockScheduler::ClientRegistry::
        get_external_client(const client_profile_id_t & client) const {
        ...
        case scheduler_class_t::immediate:
            ...
            case scheduler_class_t::repop:case scheduler_class_t::
                client:return get_external_client(id.client_profile_id);
            default:ceph_assert(static_cast < size_t > (id.class_id) <
