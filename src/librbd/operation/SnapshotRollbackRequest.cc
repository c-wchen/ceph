...
{
public:
    C_RollbackObject(AsyncObjectThrottle < I > &throttle,
                     I *image_ctx, uint64_t snap_id,
                     uint64_t object_num, uint64_t head_num_objects,
                     decltype(I::object_map) snap_object_map)
        :    C_AsyncObjectThrottle < I > (throttle, *image_ctx),
             m_snap_id(snap_id), m_object_num(object_num),
...
I >::SnapshotRollbackRequest(I &image_ctx, Context *on_finish,
                             const cls::rbd::
...
void SnapshotRollbackRequest < I >::send_block_writes()
{
    ...
    block_writes(create_context_callback < SnapshotRollbackRequest <
                 I >,
                 &SnapshotRollbackRequest <
...
I >::handle_block_writes(int *result)
{
    ...
    CephContext *cct = image_ctx.cct;
    ldout(cct,
          5) << this << " " << __func__ << ": r=" << *result << dendl;
...
void SnapshotRollbackRequest < I >::send_resize_image()
{
    ...
    m_head_num_objects =
        Striper::get_num_objects(image_ctx.layout, current_size);
    ...
    ResizeRequest < I > *req =
        ResizeRequest < I >::create(image_ctx, ctx, m_snap_size, true,
...
template < typename I >
Context *SnapshotRollbackRequest <
I >::handle_resize_image(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
void SnapshotRollbackRequest < I >::send_get_snap_object_map()
{
    ...
    if (object_map_enabled &&
        ...
        lderr(cct) << "warning: object-map is invalid for snapshot" <<
                   dendl;
        object_map_enabled = false;
    }
    if (!object_map_enabled) {
    ...
    Context *ctx = create_context_callback <
                   SnapshotRollbackRequest < I >,
                   &SnapshotRollbackRequest < I >::handle_get_snap_object_map >
                   (this);
...
template < typename I >
Context *SnapshotRollbackRequest <
I >::handle_get_snap_object_map(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...

    {
        ...
        if (image_ctx.object_map != nullptr) {
            ...
            Context *ctx = create_context_callback <
                           SnapshotRollbackRequest < I >,
                           &SnapshotRollbackRequest <
                           I >::handle_rollback_object_map > (this);
...
template < typename I >
Context *SnapshotRollbackRequest <
I >::handle_rollback_object_map(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
    ...
    if (*result < 0) {
        lderr(cct) << this << " " << __func__ <<
                   ": failed to roll back object " << "map: " <<
                   cpp_strerror(*result) << dendl;
...
void SnapshotRollbackRequest < I >::send_rollback_objects()
{
    ...
    Context *ctx = create_context_callback <
                   SnapshotRollbackRequest < I >,
                   &SnapshotRollbackRequest < I >::handle_rollback_objects >
                   (this);
...
template < typename I >
Context *SnapshotRollbackRequest <
I >::handle_rollback_objects(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
    ...
    if (*result == -ERESTART) {
        ldout(cct,
              5) << "snapshot rollback operation interrupted" << dendl;
        return this->create_context_finisher(*result);
    } else if (*result < 0) {
        lderr(cct) << "failed to rollback objects: " <<
                   cpp_strerror(*result)
                   << dendl;
        return this->create_context_finisher(*result);
    }
...
Context *SnapshotRollbackRequest < I >::send_refresh_object_map()
{
    ...
    Context *ctx = create_context_callback <
                   SnapshotRollbackRequest < I >,
                   &SnapshotRollbackRequest < I >::handle_refresh_object_map >
                   (this);
...
template < typename I >
Context *SnapshotRollbackRequest <
I >::handle_refresh_object_map(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
template < typename I >
Context *SnapshotRollbackRequest <
I >::handle_invalidate_cache(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
