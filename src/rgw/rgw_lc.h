...
class LCExpiration {
  ...
  public:
    LCExpiration() {
    } LCExpiration(const std::string & _days,
                   const std::string & _date):days(_days), date(_date) {
    }
...
//  static void generate_test_instances(list<ACLOwner*>& o);
    void set_days(const std::string & _days) {
        days = _days;
    }
...
class LCTransition {
  ...
  public:
    int get_days() const {
        return atoi(days.c_str());
    } std::string get_date() const const
{
    return date;
...
class LCFilter {
  ...
  LCFilter():flags(make_flag(LCFlagType::none)) {
    ...
    // Determine if we need AND tag when creating xml bool has_multi_condition() const {
...
bool add_transition(const LCTransition & _transition) {
    auto ret =
        transitions.emplace(_transition.get_storage_class(), _transition);
    return ret.second;
}

bool add_noncur_transition(const LCTransition & _noncur_transition) {
    auto ret =
        noncur_transitions.emplace(_noncur_transition.get_storage_class(),
                                   _noncur_transition);
    return ret.second;
}
...
void init_simple_days_rule(std::string_view _id, std::string_view _prefix,
                           int num_days);
};
...
//  int get_group_perm(ACLGroupTypeEnum group, int perm_mask);
    ...
    std::multimap < std::string, LCRule > &get_rule_map() {
        return rule_map;
    }
    std::multimap < std::string, lc_op > &get_prefix_map() {
        return prefix_map;
    }
/*
...
class LCWorker:public Thread {
  ...
  public:
    ...
    std::set < std::string > &get_cloud_targets() {
...
int list_lc_progress(std::string & marker, uint32_t max_entries,
                     ...
                     rgw::sal::Lifecycle::LCEntry >> &, int &index);
...
CephContext *get_cct() const override {
    return cct;
} rgw::sal::Lifecycle * get_lc() const {
    return sal_lc.get();
