 ...
 */
    ...
    void Formatter::dump_format_ns(const char *name, const char *ns,
                                   const char *fmt, ...) {
        va_list ap;
        va_start(ap, fmt);
...
// -----------------------
  ...
  :    m_pretty(p), m_is_pending_string(false) {
    ...
    void JSONFormatter::open_array_section_in_ns(const char *name,
                                                 const char *ns) {
    ...
    void JSONFormatter::open_object_section_in_ns(const char *name,
                                                  const char *ns) {
        std::ostringstream oss;
        oss << name << " " << ns;
    ...
    void JSONFormatter::dump_format_va(const char *name, const char *ns,
  ...
  :    
    ...
    void XMLFormatter::open_object_section_with_attrs(const char *name,
                                                      const FormatterAttrs &
                                                      attrs) {
        open_section_in_ns(name, NULL, &attrs);
    }

    void XMLFormatter::open_object_section_in_ns(const char *name,
                                                 const char *ns) {
    ...
    void XMLFormatter::open_array_section_with_attrs(const char *name,
                                                     const FormatterAttrs &
                                                     attrs) {
        open_section_in_ns(name, NULL, &attrs);
    }

    void XMLFormatter::open_array_section_in_ns(const char *name,
                                                const char *ns) {
        open_section_in_ns(name, ns, NULL);
    }
    ...
    void XMLFormatter::dump_string_with_attrs(const char *name,
                                              boost::string_view s,
        ...
        print_spaces();
        m_ss << "<" << e << attrs_str << ">" << escape_xml_str(s.
                                                               data()) << "</"
            << e << ">";
        if (m_pretty)
            m_ss << "\n";
    ...
    void XMLFormatter::dump_format_va(const char *name, const char *ns,
                                      bool quoted, const char *fmt,
                                      va_list ap) {
        char buf[LARGE_SIZE];
        vsnprintf(buf, LARGE_SIZE, fmt, ap);
    ...
    void XMLFormatter::get_attrs_str(const FormatterAttrs * attrs,
        ...
        for (std::list < std::pair < std::string,
             std::string > >::const_iterator iter = attrs->attrs.begin();
  ...
  TableFormatter::TableFormatter(bool keyval):m_keyval(keyval) {
                ...
                if (need_header_set.count(i)) {
                    ...
                    if (!m_keyval) {
                        ...
                        for (size_t j = 0; j < m_vec[i].size(); j++) {
                            os << " ";
                            std::stringstream fs;
                            fs << boost::format("%%-%is") % (m_column_size[j] +
                                                             2);
    ...
    void TableFormatter::open_object_section_with_attrs(const char *name,
                                                        const FormatterAttrs &
                                                        attrs) {
        open_section_in_ns(name, NULL, NULL);
    }

    void TableFormatter::open_object_section_in_ns(const char *name,
                                                   const char *ns) {
        open_section_in_ns(name, NULL, NULL);
    }
    ...
    void TableFormatter::open_array_section_with_attrs(const char *name,
                                                       const FormatterAttrs &
                                                       attrs) {
        open_section_in_ns(name, NULL, NULL);
    }

    void TableFormatter::open_array_section_in_ns(const char *name,
                                                  const char *ns) {
        open_section_in_ns(name, NULL, NULL);
    }

    void TableFormatter::open_section_in_ns(const char *name, const char *ns,
                                            const FormatterAttrs * attrs) {
    ...
    void TableFormatter::dump_string_with_attrs(const char *name,
                                                boost::string_view s,
                                                const FormatterAttrs & attrs) {
        finish_pending_string();
        size_t i = m_vec_index(name);
    ...
    void TableFormatter::dump_format_va(const char *name, const char *ns,
                                        bool quoted, const char *fmt,
                                        va_list ap) {
        finish_pending_string();
        char buf[LARGE_SIZE];
