...
template < bool TRACK > class PerfTree:public TMTestState {
  public:
  PerfTree(bool is_dummy):is_dummy {
    is_dummy}
    {
    }
    ...
    seastar::future <> run(const bpo::variables_map & config)
{
    return seastar::async([&config] {
                          auto backend =
                          config["backend"].as < std::string > ();
                          bool is_dummy; if (backend == "dummy") {
                          is_dummy = true;}
                          else
                          if (backend == "seastore") {
                          is_dummy = false;}
                          ...
                          auto ns_sizes =
                          config["ns-sizes"].as < std::vector < size_t >> ();
                          auto oid_sizes =
                          config["oid-sizes"].as < std::vector < size_t >> ();
                          auto onode_sizes =
                          config["onode-sizes"].as < std::vector < size_t >> ();
                          auto range2 =
                          config["range2"].as < std::vector < int >>();
                          ceph_assert(range2.size() == 2);
                          auto range1 =
                          config["range1"].as < std::vector < unsigned >>();
                          ceph_assert(range1.size() == 2);
                          auto range0 =
                          config["range0"].as < std::vector < unsigned >>();
                          ceph_assert(range0.size() == 2);
                          auto erase_ratio =
                          config["erase-ratio"].as < double >();
                          ...
                          seastar::engine().at_exit([] {
                                                    return sharded_perf_coll().
                                                    stop();}
                          );
                          auto kvs =
                          KVPool < test_item_t >::create_raw_range(ns_sizes,
                                                                   oid_sizes,
                                                                   onode_sizes,
                                                                   {
                                                                   range2[0],
                          ...
                          is_dummy}; perf.run(kvs, erase_ratio).get0();}
    ) ;
}
