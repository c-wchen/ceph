...
public:
    ...
    uint64_t get_last_change() const
    {
        return last_change;
...
{
    return crimson::common::sharded_conf().start(EntityName {
    }
    , "ceph" sv).then([] {
        std::vector <
        const char
        * > args;
        ...
        return conf.set_val(test_uint_option,
                            std::to_string(EXPECTED_VALUE));
    }
    ).then([] {
        return crimson::common::sharded_conf().
        invoke_on_all([](Config & config) {
            if (config.get_val < uint64_t > (test_uint_option) !=
                EXPECTED_VALUE) {
                throw std::runtime_error("configurations don't match");
            }
            if (sharded_cobs.local().get_last_change() !=
                EXPECTED_VALUE) {
                throw std::
                runtime_error
            ...
            if (sharded_cobs.local().get_num_changes() != 1) {
                throw std::
                runtime_error
                ("num changes don't match actual changes");
            }
        }
                     );
    }
