...
int FileJournal::check()
{
    ...
    if (header.fsid != fsid) {
        derr << "check: ondisk fsid " << header.
            fsid << " doesn't match expected " << fsid <<
...
int FileJournal::open(uint64_t fs_op_seq)
{
    ...
    dout(10) << "open header.fsid = " << header.fsid
        ...
        << dendl;
    if (header.fsid != fsid) {
        derr << "FileJournal::open: ondisk fsid " << header.
            fsid << " doesn't match expected " << fsid <<
            ", invalid (someone else's?) journal" << dendl;
        err = -EINVAL;
        goto out;
    ...
    if (header.max_size > max_size) {
        dout(2) << "open journal size " << header.
            max_size << " > current " << max_size << dendl;
        err = -EINVAL;
        goto out;
    }
    if (header.block_size != block_size) {
        dout(2) << "open journal block size " << header.
            block_size << " != current " << block_size << dendl;
        err = -EINVAL;
        goto out;
    }
    ...
    if (header.alignment != block_size && directio) {
        dout(0) << "open journal alignment " << header.
            alignment << " does not match block size " << block_size <<
...
void FileJournal::_close(int fd) const const
{
    VOID_TEMP_FAILURE_RETRY(::close(fd));
}
...
int FileJournal::_fdump(Formatter & f, bool simple)
{
    ...
    while (1) {
        ...
        stringstream ss;
        read_entry_result result = do_read_entry(pos,
                                                 &next_pos,
                                                 &bl,
                                                 &seq,
                                                 &ss);
        if (result != SUCCESS) {
            if (seq < header.committed_up_to) {
                dout(2) << "Unable to read past sequence " << seq
                    ...
                    << header.
                    committed_up_to << ", journal is corrupt" << dendl;
                err = -EINVAL;
            }
            dout(25) << ss.str() << dendl;
            dout(25) <<
                "No further valid entries found, journal is most likely valid"
                << dendl;
            break;
        }
...
void FileJournal::print_header(const header_t & header) const const
{
    dout(10) << "header: block_size " << header.block_size
        << " alignment " << header.alignment
        << " max_size " << header.max_size << dendl;
    dout(10) << "header: start " << header.start << dendl;
    dout(10) << " write_pos " << write_pos << dendl;
}

int FileJournal::read_header(header_t * hdr) const const
{
     ...
     */
    if (hdr->flags > 3) {
        derr << "read_header appears to have gibberish flags; assuming 0" <<
            dendl;
        hdr->flags = 0;
    }
...
int FileJournal::check_for_full(uint64_t seq, off64_t pos, off64_t size)
{
    ...
    if (room >= size) {
        dout(10) << "check_for_full at " << pos << " : " << size << " < " <<
            room << dendl;
                                     ...
                                     uint64_t & orig_bytes)
{
    ...
    while (!writeq_empty()) {
        ...
        while (it != items.end()) {
            ...
            int r =
                prepare_single_write(*it, bl, queue_pos, orig_ops, orig_bytes);
            ...
            if (r == -ENOSPC) {
                ...
                if (wait_on_full) {
                    dout(20) <<
                        "prepare_multi_write full on first entry, need to wait"
                        << dendl;
                }
                else {
                    dout(20) <<
  ...
  out:
    ...
    assert((write_pos + bl.length() == queue_pos) ||
           (write_pos + bl.length() - header.max_size + get_top() ==
            queue_pos));
    return 0;
}
                                      ...
                                      uint64_t & orig_bytes)
{
    ...
    dout(15) << "prepare_single_write " << orig_ops << " will write " <<
        queue_pos << " : seq " << seq << " len " << orig_len << " -> " << size
        << dendl;
    ...
    footerptr.copy_in(post_offset + seq_offset, sizeof(uint64_t),
                      (char *)&_seq);
...
void FileJournal::check_align(off64_t pos, bufferlist & bl)
{
    ...
    if (directio
        && !bl.is_aligned_size_and_memory(block_size,
                                          CEPH_DIRECTIO_ALIGNMENT)) {
...
void FileJournal::do_write(bufferlist & bl)
{
    ...
    if (pos + bl.length() > header.max_size) {
        ...
        dout(10) << "do_write wrapping, first bit at " << pos << " len " <<
            first.length()
            << " second bit len " << second.length() << " (orig len " << bl.
            length() << ")" << dendl;
...
#endif
        if (ret < 0) {
            derr << __func__ << " fsync/fdatasync failed: " <<
...
void FileJournal::write_thread_entry()
{
    ...
    while (1) {
        ...
        if (aio) {
            ...
            while (aio_num > 0) {
                ...
                dout(20) << "write_thread_entry aio throttle: aio num " <<
                    aio_num << " bytes " << aio_bytes << " ... exp " << exp <<
                    " min_new " << min_new << " ... pending " << cur << dendl;
                if (cur >= min_new)
                    break;
                dout(20) <<
                    "write_thread_entry deferring until more aios complete: " <<
        ...
        if (r == -ENOSPC) {
            if (write_stop) {
                dout(20) <<
                    "write_thread_entry full and stopping, throw out queue and finish up"
                    << dendl;
                while (!writeq_empty()) {
                    complete_write(1, peek_write().orig_len);
                    pop_write();
...
{
    dout(20) << "write_aio_bl " << pos << "~" << bl.
        length() << " seq " << seq << dendl;
    ...
    if (completed_something) {
        ...
        if (full_state != FULL_NOTFULL) {
            dout(10) << "check_aio_completion NOT queueing finisher seq " <<
                journaled_seq << ", full_commit_seq|full_restart_seq" << dendl;
        }
        else {
            if (plug_journal_completions) {
                dout(20) <<
                    "check_aio_completion NOT queueing finishers through seq "
                    << journaled_seq << " due to completion plug" << dendl;
            }
            else {
                dout(20) <<
                    "check_aio_completion queueing finishers through seq " <<
                    journaled_seq << dendl;
                queue_completions_thru(journaled_seq);
            }
        }
                               ...
                               bufferlist * tbl)
{
    ...
    for (vector < ObjectStore::Transaction >::iterator p = tls.begin();
        ...
        if ((int)(*p).get_data_length() > data_len) {
            ...
            data_align =
                ((*p).get_data_alignment() - bl.length()) & ~CEPH_PAGE_MASK;
        }
        ::encode(*p, bl);
    }
    if (tbl->length()) {
    ...
    if (data_align >= 0)
        h.pre_pad =
            ((unsigned int)data_align -
             (unsigned int)head_size) & ~CEPH_PAGE_MASK;
    ...
    dout(10) << " len " << bl.length() << " -> " << size
        ...
        << " bl " << bl.
        length() << " post_pad " << post_pad << " tail " << head_size << ")" <<
                               ...
                               Context * oncommit, TrackedOpRef osd_op)
{
    ...
    }
    {
        ...
        completions.
            push_back(completion_item(seq, oncommit, ceph_clock_now(), osd_op));
        if (writeq.empty())
            writeq_cond.Signal();
...
void FileJournal::commit_start(uint64_t seq)
{
    ...
    case FULL_WAIT:
        dout(1) <<
            " FULL_WAIT -> FULL_NOTFULL.  journal now active, setting completion plug."
            << dendl;
...
void FileJournal::do_discard(int64_t offset, int64_t end)
{
    ...
    if (offset < end)
        if (block_device_discard(fd, offset, end - offset) < 0)
            dout(1) << __func__ << "ioctl(BLKDISCARD) error:" <<
                cpp_strerror(errno) << dendl;
}
...
void FileJournal::committed_thru(uint64_t seq)
{
    ...
    // completions!
    {
        ...
        if (plug_journal_completions && seq >= header.start_seq) {
            dout(10) <<
                " removing completion plug, queuing completions thru journaled_seq "
                << journaled_seq << dendl;
    ...
    if (discard) {
        dout(10) << __func__ << " will trim (" << old_start << ", " << header.
            start << ")" << dendl;
...
const char **FileJournal::get_tracked_conf_keys() const const
{
...
void FileJournal::wrap_read_bl(off64_t pos,
                               int64_t olen,
                               bufferlist * bl, off64_t * out_pos) const const
{
    while (olen > 0) {
...
bool FileJournal::read_entry(bufferlist & bl,
                             uint64_t & next_seq, bool * corrupt)
{
    if (corrupt)
        *corrupt = false;
    ...
    if (result == SUCCESS) {
        ...
        uint64_t amount_to_take =
            next_pos > pos ?
            next_pos - pos : (header.max_size - pos) + (next_pos - get_top());
...
FileJournal::read_entry_result FileJournal::do_read_entry(off64_t init_pos,
                                                          off64_t * next_pos,
                                                          bufferlist * bl,
                                                          uint64_t * seq,
                                                          ostream * ss,
...
void FileJournal::get_header(uint64_t wanted_seq,
                             off64_t * _pos, entry_header_t * h)
{
...
void FileJournal::corrupt_footer_magic(int wfd, uint64_t seq)
{
    ...
    off64_t corrupt_at =
        ...
        h.len + h.post_pad +
        (reinterpret_cast < char *>(&h.magic2) - reinterpret_cast <
         char *>(&h));
    corrupt(wfd, corrupt_at);
}
