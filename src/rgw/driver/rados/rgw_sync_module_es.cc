...
static constexpr ESVersion ES_V5 {
5, 0};
static constexpr ESVersion ES_V7 {
7, 0};
...
struct ESInfo {
    ...
    std::string get_version_str() {
        return std::to_string(version.first) + "." +
...
struct ElasticConfig {
    uint64_t sync_instance {
    0};
    string id;
    string index_path;
    std::unique_ptr < RGWRESTConn > conn;
    bool explicit_custom_meta {
    true};
    ...
    void init(CephContext * cct, const JSONFormattable & config) {
        ...
        conn.reset(new RGWRESTConn(cct, (rgw::sal::Driver *) nullptr, id, {
                                   elastic_endpoint}, nullopt /* region */ ));
        ...
        if (string user = config["username"], pw = config["password"];
            ...
            default_headers.emplace("AUTHORIZATION",
                                    "Basic " + rgw::to_base64(auth_string));
        }
    ...
    void init_instance(const RGWRealm & realm, uint64_t instance_id) {
        ...
        snprintf(buf, sizeof(buf), "-%08x",
                 (uint32_t) (sync_instance & 0xFFFFFFFF));
    ...
    string get_obj_path(const RGWBucketInfo & bucket_info,
...
struct es_type_v2 {
    ...
    es_type_v2(ESType et):estype(et) {
        ...
        if (is_analyzed) {
            encode_json("index",
                        (is_analyzed.value()? "analyzed" : "not_analyzed"), f);
        }
    }
};
...
template < class T > struct es_index_mappings {
    ...
    es_index_mappings(ESVersion esv):es_version(esv) {
        ...
        return es_type < T > (t);
    } void dump_custom(const char *section, ESType type, const char *format,
                       Formatter * f) const {
        ...
        dump_custom("custom-date", ESType::Date,
...
struct es_index_settings {
    ...
    es_index_settings(uint32_t _replicas,
                      uint32_t _shards):num_replicas(_replicas),
        num_shards(_shards) {
    } void dump(Formatter * f) const {
...
template < class T > struct es_index_config:public es_index_config_base {
    ...
    es_index_config(es_index_settings & _s, ESVersion esv):settings(_s),
        mappings(esv) {
    } void dump(Formatter * f) const {
...
static bool is_sys_attr(const std::string & attr_name)
{
    ...
    return std::find(rgw_sys_attrs.begin(), rgw_sys_attrs.end(),
                     attr_name) != rgw_sys_attrs.end();
}
...
struct es_obj_metadata {
    ...
    es_obj_metadata(CephContext * _cct, ElasticConfigRef _es_conf,
                    const RGWBucketInfo & _bucket_info,
                    const rgw_obj_key & _key, ceph::real_time & _mtime,
                    uint64_t _size, map < string, bufferlist > &_attrs,
                    uint64_t _versioned_epoch):cct(_cct), es_conf(_es_conf),
        bucket_info(_bucket_info), key(_key), mtime(_mtime), size(_size),
        attrs(std::move(_attrs)), versioned_epoch(_versioned_epoch) {
    } void dump(Formatter * f) const {
        ...
        for (auto i:attrs) {
            ...
            if (!boost::algorithm::starts_with(attr_name, RGW_ATTR_PREFIX)) {
                continue;
                  ...
                  starts_with(attr_name, RGW_ATTR_META_PREFIX)) {
                custom_meta.emplace(attr_name.
                                    substr(sizeof(RGW_ATTR_META_PREFIX) - 1),
            ...
            if (attr_name == RGW_ATTR_ACL) {
                try {
                    ...
                    ldpp_dout(dpp,
                              0) << "ERROR: failed to decode acl for " <<
              ...
              for (auto acliter:acl.get_grant_map()) {
                    ...
                    if (grant.get_type().get_type() == ACL_TYPE_CANON_USER &&
                        ((uint32_t) grant.get_permission().
                         get_permissions() & RGW_PERM_READ) != 0) {
                        rgw_user user;
                ...
                try {
                    ...
                    ldpp_dout(dpp,
                              0) <<
                        "ERROR: failed to decode compression attr for " <<
                        bucket_info.bucket << "/" << key << dendl;
                    continue;
                }
                out_attrs.emplace("compression",
                                  std::move(cs_info.compression_type));
            }
            else {
                if (!is_sys_attr(attr_name)) {
                    out_attrs.emplace(attr_name.
                                      substr(sizeof(RGW_ATTR_PREFIX) - 1),
      ...
      for (auto i:custom_meta) {
            ...
            if (config == bucket_info.mdsearch_config.end()) {
                if (!es_conf->explicit_custom_meta) {
                    ...
                    ldpp_dout(dpp,
                        ...
                        first << " not found in bucket mdsearch config: " <<
        ...
        if (!custom_date.empty()) {
          ...
          for (auto i:custom_date) {
                ...
                if (r < 0) {
                    ldpp_dout(dpp,
                              20) << __func__ << "(): failed to parse time (" <<
                        i.
                        second <<
                        "), skipping encoding of custom date attribute" <<
                        dendl;
                    continue;
                }
...
class RGWElasticGetESInfoCBCR:public RGWCoroutine {
  public:
    RGWElasticGetESInfoCBCR(RGWDataSyncCtx * _sc,
        ...
        sc(_sc), sync_env(_sc->env), conf(_conf) {
    } int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            ldpp_dout(dpp,
                      5) << conf->
                id << ": get elasticsearch info for zone: " << sc->
                source_zone << dendl;
            yield call(new RGWReadRESTResourceCR < ESInfo >
                       (sync_env->cct, conf->conn.get(), sync_env->http_manager,
                        "/", nullptr /*params */ ,
                        &(conf->default_headers), &(conf->es_info)));
            if (retcode < 0) {
                ldpp_dout(dpp,
                          5) << conf->
                    id << ": get elasticsearch failed: " << retcode << dendl;
                return set_cr_error(retcode);
            }

            ldpp_dout(dpp,
                      5) << conf->id << ": got elastic version=" << conf->
                es_info.get_version_str() << dendl;
            return set_cr_done();
        }
        return 0;
    }
  ...
  public:
    RGWElasticPutIndexCBCR(RGWDataSyncCtx * _sc,
        ...
        reenter(this) {
            ldpp_dout(dpp,
                      5) << conf->
                id << ": put elasticsearch index for zone: " << sc->
                source_zone << dendl;

            yield {
                string path = conf->get_index_path();
                es_index_settings settings(conf->num_replicas,
                                           conf->num_shards);
                std::unique_ptr < es_index_config_base > index_conf;

                if (conf->es_info.version >= ES_V5) {
                    ldpp_dout(dpp,
                              0) << "elasticsearch: index mapping: version >= 5"
                        << dendl;
                    index_conf.reset(new es_index_config < es_type_v5 >
                                     (settings, conf->es_info.version));
                }
                else {
                    ldpp_dout(dpp,
                              0) << "elasticsearch: index mapping: version < 5"
                        << dendl;
                    index_conf.reset(new es_index_config < es_type_v2 >
                                     (settings, conf->es_info.version));
                }
                call(new RGWPutRESTResourceCR < es_index_config_base, int,
                     _err_response > (sc->cct, conf->conn.get(),
                                      sync_env->http_manager, path,
                                      nullptr /*params */ ,
                                      &(conf->default_headers), *index_conf,
            ...
            if (retcode < 0) {
                ...
                if (err_response.error.type != "index_already_exists_exception"
                    ...
                    ldpp_dout(dpp,
                        ...
                        << err_response.error.
                        type << " response.reason=" << err_response.error.
                        reason << dendl;
                    return set_cr_error(retcode);
                }
                ...
                ldpp_dout(dpp,
                          0) <<
                    "elasticsearch: index already exists, assuming external initialization"
                    << dendl;
            }
            return set_cr_done();
        }
...
class RGWElasticHandleRemoteObjCBCR:public RGWStatRemoteObjCBCR {
  ...
  public:
    RGWElasticHandleRemoteObjCBCR(RGWDataSyncCtx * _sc,
                                  ...
                                  rgw_obj_key & _key, ElasticConfigRef _conf,
                                  uint64_t
                                  _versioned_epoch):RGWStatRemoteObjCBCR(_sc,
                                                                         _sync_pipe.
                                                                         info.
                                                                         source_bs.
                                                                         bucket,
                                                                         _key),
        sync_pipe(_sync_pipe), conf(_conf), versioned_epoch(_versioned_epoch) {
    } int operate(const DoutPrefixProvider * dpp) override {
        reenter(this) {
            ...
            yield {
                string path =
                    conf->get_obj_path(sync_pipe.dest_bucket_info, key);
                es_obj_metadata doc(sync_env->cct, conf,
                                    sync_pipe.dest_bucket_info, key, mtime,
                                    size, attrs, versioned_epoch);

                call(new RGWPutRESTResourceCR < es_obj_metadata,
                     int >(sync_env->cct, conf->conn.get(),
                           sync_env->http_manager, path, nullptr /* params */ ,
                           &(conf->default_headers), doc,
                           nullptr /* result */ ));
...
class RGWElasticHandleRemoteObjCR:public RGWCallStatRemoteObjCR {
  ...
  public:
    ...
    RGWStatRemoteObjCBCR *allocate_callback() override {
        return new RGWElasticHandleRemoteObjCBCR(sc, sync_pipe, key, conf,
                                                 versioned_epoch);
    }
};
...
class RGWElasticRemoveRemoteObjCBCR:public RGWCoroutine {
  ...
  public:
    RGWElasticRemoveRemoteObjCBCR(RGWDataSyncCtx * _sc,
                                  ...
                                  ElasticConfigRef _conf):RGWCoroutine(_sc->
                                                                       cct),
        sc(_sc), sync_env(_sc->env), sync_pipe(_sync_pipe), key(_key),
        ...
        reenter(this) {
            ...
            yield {
                ...
                call(new
                     RGWDeleteRESTResourceCR(sync_env->cct, conf->conn.get(),
                                             sync_env->http_manager, path,
                                             nullptr /* params */ ));
            }
            if (retcode < 0) {
...
class RGWElasticDataSyncModule:public RGWDataSyncModule {
    ElasticConfigRef conf;
  public:
    RGWElasticDataSyncModule(const DoutPrefixProvider * dpp, CephContext * cct,
                             const JSONFormattable & config):conf(std::
                                                                  make_shared <
                                                                  ElasticConfig
                                                                  > ()) {
    ...
    RGWCoroutine *init_sync(const DoutPrefixProvider * dpp,
                            RGWDataSyncCtx * sc) override {
        ldpp_dout(dpp, 5) << conf->id << ": init" << dendl;
        return new RGWElasticInitConfigCBCR(sc, conf);
    }

    RGWCoroutine *start_sync(const DoutPrefixProvider * dpp,
                             RGWDataSyncCtx * sc)override {
    ...
    RGWCoroutine *sync_object(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  10) << conf->id << ": sync_object: b=" << sync_pipe.info.
            source_bs.
        ...
        if (!conf->should_handle_operation(sync_pipe.dest_bucket_info)) {
            ldpp_dout(dpp,
                      10) << conf->
                id << ": skipping operation (bucket not approved)" << dendl;
            return nullptr;
        }
        return new RGWElasticHandleRemoteObjCR(sc, sync_pipe, key, conf,
    ...
    RGWCoroutine *remove_object(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  10) << conf->id << ": rm_object: b=" << sync_pipe.info.
            source_bs.
        ...
        return new RGWElasticRemoveRemoteObjCBCR(sc, sync_pipe, key, mtime,
                                                 conf);
    }
    RGWCoroutine *create_delete_marker(const DoutPrefixProvider * dpp,
                                       ...
                                       rgw_zone_set * zones_trace)override {
        ldpp_dout(dpp,
                  10) << conf->id << ": create_delete_marker: b=" << sync_pipe.
            info.source_bs.
            bucket << " k=" << key << " mtime=" << mtime << " versioned=" <<
            versioned << " versioned_epoch=" << versioned_epoch << dendl;
        ldpp_dout(dpp,
                  10) << conf->
            id << ": skipping operation (not handled)" << dendl;
        return NULL;
    }
...
RGWElasticSyncModuleInstance(const DoutPrefixProvider * dpp, CephContext * cct,
                             const JSONFormattable & config)
{
    data_handler =
        std::unique_ptr < RGWElasticDataSyncModule >
...
RGWRESTMgr *RGWElasticSyncModuleInstance::get_rest_filter(int dialect,
                                                          RGWRESTMgr * orig)
{
...
int RGWElasticSyncModule::create_instance(const DoutPrefixProvider * dpp,
                                          CephContext * cct,
                                          const JSONFormattable & config,
