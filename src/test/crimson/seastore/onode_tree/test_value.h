...
template <> struct fmt::formatter <
    crimson::os::seastore::onode::delta_op_t >:fmt::ostream_formatter {
};
#endif
...
namespace crimson::os::seastore::onode {
    ...
    template < value_magic_t MAGIC,
        ...
        bool DO_SPLIT_CHECK > class TestValue final:public Value {
      public:
      ...
      private:
        ...
        struct Replayable {
            static void set_id(NodeExtentMutable & payload_mut, id_t id) {
                ...
                p_payload->id = id;
            } static void set_tail_magic(NodeExtentMutable & payload_mut,
                                         magic_t magic) {
          ...
          private:
            static payload_t *get_write(NodeExtentMutable & payload_mut) {
                return reinterpret_cast <
                    payload_t * >(payload_mut.get_write());
            }
        };
      ...
      class Recorder final:public ValueDeltaRecorder {
            ...
            value_magic_t get_header_magic()const override {
                ...
                try {
                    ...
                    case delta_op_t::UPDATE_ID:{
                            logger().
                                debug
                                ("OTree::TestValue::Replay: decoding UPDATE_ID ...");
                            id_t id;
                            ceph::decode(id, delta);
                            logger().
                                debug
                                ("OTree::TestValue::Replay: apply id={} ...",
                                 id);
                    ...
                    case delta_op_t::UPDATE_TAIL_MAGIC:{
                            ...
                            logger().
                                debug
                                ("OTree::TestValue::Replay: apply magic={} ...",
                                 magic);
                    ...
                    default:
                        logger().
                            error
                            ("OTree::TestValue::Replay: got unknown op {} when replay {:#x}+{:#x}",
                    ...
                    logger().
                        error
                        ("OTree::TestValue::Replay: got decode error {} when replay {:#x}+{:#x}",
        ...
        TestValue(NodeExtentManager & nm, const ValueBuilder & vb,
      ...
      :    Value(nm, vb, p_cursor) {
        ...
        id_t get_id() const {
            ...
            auto value_mutable =
                prepare_mutate_payload < payload_t, Recorder > (t);
        ...
        magic_t get_tail_magic() const {
            ...
            auto p_magic =
                reinterpret_cast < const char *>(p_payload) + offset_magic;
        ...
        void initialize(Transaction & t, const item_t & item) {
            ceph_assert(get_payload_size() + sizeof(value_header_t) ==
                        item.size);
...
template <>
    ...
    crimson::os::seastore::onode::test_item_t >:fmt::ostream_formatter {
};
#endif
