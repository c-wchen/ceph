...
class MDCache {
  ...
  public:
    ...
    double cache_toofull_ratio(void) const {
        ...
        double memory_reserve = cache_memory_limit * (1.0 - cache_reservation);
         return fmax(0.0,
                     fmax((cache_size() - memory_reserve) / memory_reserve,
                          cache_inode_limit ==
                          0 ? 0.0 : (CInode::count() -
        ...
        return (cache_inode_limit > 0
                && CInode::count() > cache_inode_limit * cache_health_threshold)
  ...
  public:
    ...
    struct discover_info_t {
        ...
        bool want_xlocked;
         ...
         discover_info_t():tid(0), mds(-1), snap(CEPH_NOSNAP), basei(NULL),
    ...
    // waiters
    map < int, map < inodeno_t,
        list < MDSInternalContextBase * >>>waiting_for_base_ino;

    void discover_base_ino(inodeno_t want_ino,
                           MDSInternalContextBase * onfinish, mds_rank_t from =
                           MDS_RANK_NONE);
    void discover_dir_frag(CInode * base, frag_t approx_fg,
                           MDSInternalContextBase * onfinish, mds_rank_t from =
                           MDS_RANK_NONE);
    void discover_path(CInode * base, snapid_t snap, filepath want_path,
                       MDSInternalContextBase * onfinish, bool want_xlocked =
  ...
  public:
    ...
    void adjust_bounded_subtree_auth(CDir * dir, set < CDir * >&bounds,
    ...
    void adjust_bounded_subtree_auth(CDir * dir, vector < dirfrag_t > &bounds,
                                     mds_authority_t auth);
    void adjust_bounded_subtree_auth(CDir * dir, vector < dirfrag_t > &bounds,
                                     mds_rank_t a) {
        adjust_bounded_subtree_auth(dir, bounds,
  ...
  public:
    ...
    MDRequestRef request_start_slave(metareqid_t rid, __u32 attempt,
                                     Message * m);
    ...
    void journal_cow_inode(MutationRef & mut, EMetaBlob * metablob, CInode * in,
                           snapid_t follows =
                           CEPH_NOSNAP, CInode ** pcow_inode = 0);
    void journal_dirty_inode(MutationImpl * mut, EMetaBlob * metablob,
                             CInode * in, snapid_t follows = CEPH_NOSNAP);

    void project_rstat_inode_to_frag(CInode * cur, CDir * parent,
                                     snapid_t first, int linkunlink,
                                     SnapRealm * prealm);
    void _project_rstat_inode_to_frag(CInode::mempool_inode & inode,
                                      snapid_t ofirst, snapid_t last,
                                      CDir * parent, int linkunlink,
                                      bool update_inode);
    void project_rstat_frag_to_inode(nest_info_t & rstat,
    ...
    // slaves
    void add_uncommitted_master(metareqid_t reqid, LogSegment * ls,
                                set < mds_rank_t > &slaves, bool safe = false) {
        uncommitted_masters[reqid].ls = ls;
        uncommitted_masters[reqid].slaves = slaves;
        uncommitted_masters[reqid].safe = safe;
    }
    void wait_for_uncommitted_master(metareqid_t reqid,
    ...
    bool have_uncommitted_master(metareqid_t reqid, mds_rank_t from) {
        ...
        return p != uncommitted_masters.end()
            && p->second.slaves.count(from) > 0;
    }
  ...
  protected:
    ...
    // from MMDSResolves
    map < mds_rank_t, map < dirfrag_t,
        vector < dirfrag_t > > >other_ambiguous_imports;
    ...
    void add_uncommitted_slave_update(metareqid_t reqid, mds_rank_t master,
                                      MDSlaveUpdate *);
    void finish_uncommitted_slave_update(metareqid_t reqid, mds_rank_t master);
    MDSlaveUpdate *get_uncommitted_slave_update(metareqid_t reqid,
                                                mds_rank_t master);
  public:
  ...
  protected:
    ...
    map < mds_rank_t, map < inodeno_t, map < client_t,
        Capability::Import > > >rejoin_imported_caps;
    map < inodeno_t, pair < mds_rank_t, map < client_t,
        Capability::Export > > >rejoin_slave_exports;
  ...
  public:
    ...
    void rejoin_export_caps(inodeno_t ino, client_t client,
                            const cap_reconnect_t & icr, int target = -1) {
    ...
    struct reconnected_cap_info_t {
        ...
        int dirty_caps;
         reconnected_cap_info_t(): realm_ino(0), snap_follows(0), dirty_caps(0) {
    }};
    ...
    void rejoin_open_sessions_finish(map < client_t, pair < Session *,
  ...
  private:
    std::pair < bool, uint64_t > trim_lru(uint64_t count, map < mds_rank_t,
                                          MCacheExpire * >&expiremap);
    bool trim_dentry(CDentry * dn, map < mds_rank_t,
                     MCacheExpire * >&expiremap);
    void trim_dirfrag(CDir * dir, CDir * con, map < mds_rank_t,
                      MCacheExpire * >&expiremap);
    bool trim_inode(CDentry * dn, CInode * in, CDir * con, map < mds_rank_t,
                    class MCacheExpire * >&expiremap);
   ...
   */
    bool expire_recursive(CInode * in,
  ...
  public:
    ...
    void _create_system_file(CDir * dir, const char *name, CInode * in,
    ...
    MDSInternalContextBase *_get_waiter(MDRequestRef & mdr, Message * req,
                                        MDSInternalContextBase * fin);
   ...
   */
    int path_traverse(MDRequestRef & mdr, Message * req,
                      MDSInternalContextBase * fin, const filepath & path,
                      vector < CDentry * >*pdnvec, CInode ** pin, int onfail);

    CInode *cache_traverse(const filepath & path);

    void open_remote_dirfrag(CInode * diri, frag_t fg,
                             MDSInternalContextBase * fin);
    CInode *get_dentry_inode(CDentry * dn, MDRequestRef & mdr, bool projected =
                             false);

    bool parallel_fetch(map < inodeno_t, filepath > &pathmap,
    ...
    void open_remote_dentry(CDentry * dn, bool projected,
                            MDSInternalContextBase * fin, bool want_xlocked =
                            false);
  ...
  protected:
    ...
    int open_ino_traverse_dir(inodeno_t ino, MMDSOpenIno * m,
                              vector < inode_backpointer_t > &ancestors,
                              bool discover, bool want_xlocked,
                              mds_rank_t * hint);
  ...
  public:
    ...
    struct find_ino_peer_info_t {
        ...
        mds_rank_t checking;
         ...
         find_ino_peer_info_t():tid(0), fin(NULL), hint(MDS_RANK_NONE),
    ...
    void find_ino_peers(inodeno_t ino, MDSInternalContextBase * c,
  ...
  public:
    ...
    void _snaprealm_create_finish(MDRequestRef & mdr, MutationRef & mut,
                                  CInode * in);
  ...
  private:
    ...
    void adjust_dir_fragments(CInode * diri, frag_t basefrag, int bits,
                              ...
                              list < MDSInternalContextBase * >&waiters,
                              bool replay);
    void adjust_dir_fragments(CInode * diri, list < CDir * >&srcfrags,
    ...
    void add_uncommitted_fragment(dirfrag_t basedirfrag, int bits,
                                  list < frag_t > &old_frag, LogSegment * ls,
  ...
  public:
    void wait_for_uncommitted_fragment(dirfrag_t dirfrag,
                                       MDSInternalContextBase * c) {
  ...
  protected:
    int dump_cache(boost::string_view fn, Formatter * f,
                   boost::string_view dump_root = "", int depth = -1);
  public:
    int dump_cache() {
        return dump_cache(NULL, NULL);
    }
