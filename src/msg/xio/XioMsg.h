...
class XioSend:public XioSubmit {
  public:
    ...
    const struct xio_msg *get_xio_msg() const {
        return &req_0.msg;
    } struct xio_msg *get_xio_msg() {
        return &req_0.msg;
    } virtual size_t get_msg_count() const {
        return 1;
...
class XioCommand:public XioSend {
  public:
    XioCommand(XioConnection * _xcon, struct xio_reg_mem &_mp):XioSend(_xcon,
                                                                       _mp) {
    } buffer::list & get_bl_ref() {
        return bl;
    };
...
struct XioMsg:public XioSend {
  ...
  public:
    XioMsg(Message * _m, XioConnection * _xcon, struct xio_reg_mem &_mp,
           int _ex_cnt, uint64_t _features):XioSend(_xcon, _mp, _ex_cnt),
        m(_m), hdr(m->get_header(), m->get_footer(), _features), req_arr(NULL) {
    ...
    size_t get_msg_count() const override {
        ...
        req_arr =
            static_cast < xio_msg_ex * >(malloc(cnt * sizeof(xio_msg_ex)));
...
class XioDispatchHook:public Message::CompletionHook {
  ...
  public:
    ...
    XioDispatchHook(XioConnection * _xcon, Message * _m, XioInSeq & _msg_seq,
                    struct xio_reg_mem &_mp):CompletionHook(_m),
        xcon(_xcon->get()),
        msg_seq(_msg_seq),
        rsp_pool(xio_msgr_noreg_mpool), cl_flag(false), mp_this(_mp) {
...
class XioMarkDownHook:public Message::CompletionHook {
  ...
  public:
    ...
    XioMarkDownHook(XioConnection * _xcon, Message * _m,
                    struct xio_reg_mem &_mp):CompletionHook(_m),
        xcon(_xcon->get()), mp_this(_mp) {
