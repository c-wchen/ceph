...
void AllocTest::do_fill(uint64_t high_mark,
                            ...
                            uint32_t iterations, double leak_factor)
{
    ...
    r.fragments_count +=
        (fragmented != 0 ? double (fragments) / fragmented : 2);
...
TEST_P(AllocTest, test_alloc_triangle_0_8M_16M)
{
  ...
  for (auto & s:scenarios) {
        ...
        doAgingTest(size_generator, allocator_name, s.capacity * _1G,
                    s.alloc_unit, s.high_mark * s.capacity * _1G,
                    s.low_mark * s.capacity * _1G, s.repeats, s.leakness);
    }
}
    ...
    for (size_t i = 0; i < capacity / (1024 * 1024); i++) {
        ...
        if (tmp.size() > 1) {
          ...
          for (auto & t:tmp) {
            ...
            release_set.insert(offset + length - 2 * alloc_unit,
                               2 * alloc_unit);
            alloc->release(release_set);
            release_set.clear();
        }
