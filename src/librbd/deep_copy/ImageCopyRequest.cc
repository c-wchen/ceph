...
template < typename I >
ImageCopyRequest < I >::ImageCopyRequest(I *src_image_ctx,
        I *dst_image_ctx,
        librados::
...
} template < typename I > void ImageCopyRequest < I >::send()
{
    ...
    util::compute_snap_map(m_dst_image_ctx->cct, m_src_snap_id_start,
                           m_src_snap_id_end, m_dst_image_ctx->snaps,
                           m_snap_seqs, &m_snap_map);
    m_dst_image_ctx->image_lock.unlock_shared();

    if (m_snap_map.empty()) {
        lderr(m_cct) << "failed to map snapshots within boundary" <<
                     dendl;
        finish(-EINVAL);
        return;
    }
        ...
        *src_objects)
{
    ...
    Striper::extent_to_file(m_cct, &m_dst_image_ctx->layout, dst_object,
                            0, m_dst_image_ctx->layout.object_size,
                            image_extents);

    for (auto &e : image_extents) {
        std::map < object_t,
            std::vector < ObjectExtent >> src_object_extents;
        Striper::file_to_extents(m_cct, m_src_image_ctx->format_string,
                                 &m_src_image_ctx->layout, e.first,
                                 e.second, 0, src_object_extents);
...
template < typename I > void ImageCopyRequest < I >::compute_diff()
{
    ...
    auto ctx = create_context_callback <
               ImageCopyRequest < I >,
               &ImageCopyRequest < I >::handle_compute_diff > (this);
    auto req =
        object_map::DiffRequest < I >::create(m_src_image_ctx,
            m_src_snap_id_start,
            m_src_snap_id_end,
            &m_object_diff_state,
            ctx);
...
void ImageCopyRequest < I >::send_object_copies()
{
    ...
    uint64_t size;
    {
        ...
        for (auto snap_id : m_src_image_ctx->snaps) {
            size =
                std::max(size,
                         m_src_image_ctx->get_image_size(snap_id));
        }
    }
    m_end_object_no =
        Striper::get_num_objects(m_dst_image_ctx->layout, size);
    ...
    bool complete;
    {
        std::lock_guard locker {
            m_lock};
        auto max_ops =
            m_src_image_ctx->config.template get_val < uint64_t >
...
void ImageCopyRequest < I >::send_next_object_copy()
{
    ...
    if (m_object_diff_state.size() > 0) {
        ...
        if (object_diff_state == object_map::DIFF_STATE_HOLE) {
            ldout(m_cct,
                  20) << "skipping non-existent object " << ono <<
                      dendl;
    ...
    auto req =
        ObjectCopyRequest < I >::create(m_src_image_ctx,
                                        m_dst_image_ctx,
                                        m_src_snap_id_start,
                                        m_dst_snap_id_start,
                                        m_snap_map, ono, flags,
                                        m_handler, ctx);
    req->send();
}

template < typename I >
void ImageCopyRequest < I >::handle_object_copy(uint64_t object_no,
        int r)
{
    ldout(m_cct,
          20) << "object_no=" << object_no << ", r=" << r << dendl;
    ...
    bool complete;
    {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(m_cct) << "object copy failed: " << cpp_strerror(r) <<
                         dendl;
            if (m_ret_val == 0) {
                m_ret_val = r;
            }
            ...
            while (!m_updating_progress && !m_copied_objects.empty() &&
                ...
                m_lock.unlock();
                m_handler->update_progress(progress_object_no,
                                           m_end_object_no);
                m_lock.lock();
