...
namespace journal {
    ...
    namespace {
        ...
        struct C_Flush:public Context {
            ...
            Context *on_finish;
            std::atomic < int64_t > pending_flushes {
            0};
            int ret_val = 0;

            C_Flush(ceph::ref_t < JournalMetadata > _journal_metadata,
                    Context * _on_finish, size_t _pending_flushes)
            :journal_metadata(std::move(_journal_metadata)),
                on_finish(_on_finish), pending_flushes(_pending_flushes) {
  ...
  :    m_object_oid_prefix(object_oid_prefix),
        ...
        m_current_set(m_journal_metadata->get_active_set()), m_object_locks {
        ceph::make_lock_container < ceph::mutex >
    ...
    )}
    {
        ...
        for (uint8_t splay_offset = 0; splay_offset < splay_width;
             ++splay_offset) {
            uint64_t object_number =
                splay_offset + (m_current_set * splay_width);
            std::lock_guard locker {
            m_object_locks[splay_offset]};
            m_object_ptrs[splay_offset] =
                create_object_recorder(object_number,
                                       &m_object_locks[splay_offset]);
        }
    ...
    void JournalRecorder::shut_down(Context * on_safe) {
        on_safe = new LambdaContext([this, on_safe] (int r) {
                                    Context * ctx = nullptr; {
                                    std::lock_guard locker {
                                    m_lock}; if (m_in_flight_advance_sets != 0) {
                                    ceph_assert(m_on_object_set_advanced ==
                                                nullptr);
                                    m_on_object_set_advanced =
                                    new LambdaContext([on_safe, r] (int) {
                                                      on_safe->complete(r);});}
                                    else {
                                    ctx = on_safe;}
                                    }
    ...
    void JournalRecorder::set_append_batch_options(int flush_interval,
        ...
        for (uint8_t splay_offset = 0; splay_offset < splay_width;
             ++splay_offset) {
            std::lock_guard object_locker {
            m_object_locks[splay_offset]};
            auto object_recorder = get_object(splay_offset);
            object_recorder->set_append_batch_options(flush_interval,
                                                      flush_bytes, flush_age);
        }
    }
    ...
    Future JournalRecorder::append(uint64_t tag_tid,
        ...
        uint64_t commit_tid =
            m_journal_metadata->allocate_commit_tid(object_ptr->
                                                    get_object_number(),
                                                    tag_tid, entry_tid);
        auto future =
            ceph::make_ref < FutureImpl > (tag_tid, entry_tid, commit_tid);
        ...
        bufferlist entry_bl;
        encode(Entry
               (future->get_tag_tid(), future->get_entry_tid(), payload_bl),
               entry_bl);
        ...
        if (object_full) {
            ldout(m_cct,
                ...
                get_oid() << " now full" << dendl;
            std::lock_guard l {
            m_lock};
            close_and_advance_object_set(object_ptr->get_object_number() /
                                         splay_width);
        }
    ...
    void JournalRecorder::flush(Context * on_safe) {
        ...
        C_Flush *ctx;
        {
            ...
            ctx =
                new C_Flush(m_journal_metadata, on_safe,
                            m_object_ptrs.size() + 1);
    ...
    void JournalRecorder::advance_object_set() {
        ...
        m_journal_metadata->set_active_set(m_current_set,
                                           new C_AdvanceObjectSet(this));
    }
    ...
    void JournalRecorder::handle_advance_object_set(int r) {
        Context *on_object_set_advanced = nullptr;
        {
            ...
            if (r < 0 && r != -ESTALE) {
                lderr(m_cct) << "failed to advance object set: " <<
                    cpp_strerror(r)
                    << dendl;
            }
    ...
    void JournalRecorder::open_object_set() {
      ...
      for (const auto & p:m_object_ptrs) {
        ...
        if (overflowed) {
            ldout(m_cct,
                  10) << "object set " << m_current_set << " now full" << dendl;
    ...
    bool JournalRecorder::close_object_set(uint64_t active_set) {
      ...
      for (const auto & p:m_object_ptrs) {
            const auto & object_recorder = p.second;
            if (object_recorder->get_object_number() / splay_width !=
                active_set) {
                ldout(m_cct,
                      10) << "closing object " << object_recorder->get_oid()
                    << dendl;
                // flush out all queued appends and hold future appends
                if (!object_recorder->close()) {
                    ++m_in_flight_object_closes;
                    ldout(m_cct,
                          10) << "object " << object_recorder->
                        get_oid() << " " << "close in-progress" << dendl;
                }
                else {
                    ldout(m_cct,
                          10) << "object " << object_recorder->
    ...
    ceph::ref_t < ObjectRecorder >
        JournalRecorder::create_object_recorder(uint64_t object_number,
                                                ceph::mutex * lock) {
        ldout(m_cct, 10) << "object_number=" << object_number << dendl;
        auto object_recorder =
            ceph::make_ref < ObjectRecorder > (m_ioctx,
                                               utils::
                                               get_object_name
                                               (m_object_oid_prefix,
                                                object_number),
        ...
        object_recorder->set_append_batch_options(m_flush_interval,
                                                  m_flush_bytes, m_flush_age);
        return object_recorder;
    }

    bool JournalRecorder::create_next_object_recorder(ceph::ref_t <
                                                      ObjectRecorder >
                                                      object_recorder) {
        ceph_assert(ceph_mutex_is_locked(m_lock));
        ...
        auto new_object_recorder = create_object_recorder((m_current_set *
                                                           splay_width) +
                                                          splay_offset,
                                                          &m_object_locks
                                                          [splay_offset]);
      ...
      for (auto & append_buffer:append_buffers) {
            m_journal_metadata->overflow_commit_tid(append_buffer.first->
                                                    get_commit_tid(),
                                                    new_object_recorder->
                                                    get_object_number());
        }
    ...
    void JournalRecorder::handle_update() {
        ...
        if (m_current_set < active_set) {
            ...
            if (m_in_flight_advance_sets == 0 && m_in_flight_object_closes == 0) {
                ldout(m_cct,
                      10) << "closing current object set " << current_set <<
                    dendl;
    ...
    void JournalRecorder::handle_closed(ObjectRecorder * object_recorder) {
        ...
        ceph_assert(active_object_recorder->get_object_number() ==
                    object_number);
