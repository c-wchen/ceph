...
class XMLObjIter {
  public:
    ...
    typedef std::map < std::string, XMLObj * >::iterator const_map_iter_t;
     ...
     XMLObjIter();
     virtual ~ XMLObjIter();
    void set(const XMLObjIter::const_map_iter_t & _cur,
...
class RGWXMLParser:public XMLObj {
  private:
    ...
    static void call_xml_start(void *user_data, const char *el,
                               const char **attr);
...
namespace RGWXMLDecoder {
    ...
    typedef RGWXMLParser XMLParser;
     ...
     template < class T >
        bool decode_xml(const char *name, T & val, XMLObj * obj,
                        bool mandatory = false);

     template < class T >
        bool decode_xml(const char *name, std::vector < T > &v, XMLObj * obj,
                        bool mandatory = false);

     template < class C >
        bool decode_xml(const char *name, C & container,
                        void (*cb) (C &, XMLObj * obj), XMLObj * obj,
                        bool mandatory = false);

     template < class T >
        void decode_xml(const char *name, T & val, T & default_val,
                        XMLObj * obj);
} static inline std::ostream & operator<<(std::ostream & out,
                                          RGWXMLDecoder::err & err)
{
    return out << err.what();
}
...
template < class T >
    void do_decode_xml_obj(std::list < T > &l, const std::string & name,
                           XMLObj * obj)
{
    l.clear();
...
template < class T >
    bool RGWXMLDecoder::decode_xml(const char *name, T & val, XMLObj * obj,
                                   bool mandatory)
{
    XMLObjIter iter = obj->find(name);
...
template < class T >
    bool RGWXMLDecoder::decode_xml(const char *name, std::vector < T > &v,
                                   XMLObj * obj, bool mandatory)
{
    XMLObjIter iter = obj->find(name);
    XMLObj *o = iter.get_next();
...
template < class C >
    bool RGWXMLDecoder::decode_xml(const char *name, C & container,
                                   void (*cb) (C &, XMLObj *), XMLObj * obj,
                                   bool mandatory)
{
    container.clear();
...
template < class T >
    void RGWXMLDecoder::decode_xml(const char *name, T & val, T & default_val,
                                   XMLObj * obj)
{
    XMLObjIter iter = obj->find(name);
    XMLObj *o = iter.get_next();
...
template < class T >
    static void encode_xml(const char *name, const char *ns, const T & val,
                           ceph::Formatter * f)
{
...
template < class T >
    static void do_encode_xml(const char *name, const std::list < T > &l,
...
template < class T >
    static void encode_xml(const char *name, const std::vector < T > &l,
                           ceph::Formatter * f)
{
...
template < class T >
    static void encode_xml(const char *name, const std::optional < T > &o,
                           ceph::Formatter * f)
{
    if (!o) {
        return;
