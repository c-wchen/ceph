...
ConfigProxy::ConfigProxy(const EntityName &name, std::string_view cluster)
{
    ...
    local_config.reset(new md_config_t {
        *values, obs_mgr, true}
                      );
    if (name.is_mds()) {
...
} seastar::future <> ConfigProxy::parse_config_files(const std::
        string &conf_files)
{
    auto conffile_paths = get_config().get_conffile_paths(*values,
                          conf_files.
                          empty() ? nullptr :
                          conf_files.
                          c_str(),
    ...
    return seastar::do_with(std::move(conffile_paths), [this](auto & paths) {
        return seastar::
               repeat([path = paths.begin(), e =
        paths.end(), this]()mutable {
            if (path == e) {
