  ...
  protected:
    ...
    size_t get_num_sorted()const {
        using const_iterator = typename view_type::const_iterator;
         return std::distance < const_iterator > (sorted.begin(),
                                                  sorted_position);
  } public:
    ...
    /// return the maximum number of keys size_t capacity() const noexcept {
        return bound;
    }
    /// increment a counter for the given key and return its value. if the key was/// not present, insert it. if the map is full, return 0
    ...
    template < typename Callback >
        ...
        if (sorted.empty()) {
            ...
            sorted.assign(const_pointer_iterator {
                          counters.cbegin()}
