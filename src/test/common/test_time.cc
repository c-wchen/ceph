...
{
    ceph::real_clock::time_point tp(seconds(1556122013) +
                                    nanoseconds(39923122));
...
void float_format_eq(string_view lhs, string_view rhs, unsigned precision)
{
    const float TOLERANCE =
        10.0F / pow(10.0F, static_cast < float >(precision));
...
TEST(TimeDurations, print)
{
    float_format_eq("0.123456700s",
                    to_string(std::chrono::duration_cast < ceph::timespan >
                              (0.1234567 s)), 9);
    float_format_eq("-0.123456700s",
                    to_string(std::chrono::duration_cast < ceph::signedspan >
                              (-0.1234567 s)), 9);
    EXPECT_EQ("42s", to_string(42 s));
    float_format_eq("0.123000000s", to_string(123 ms), 9);
}
