...
#define CREATE_FLAGS(x) \
    ((x) & ~(RGWFileHandle::FLAG_CREATE|RGWFileHandle::FLAG_LOCK))
      ...
      :    fs(_fs), bucket(nullptr), parent(nullptr), variant_type {
    ...
    class RGWLibFS {
      ...
      public:
            ...
            if (ret == 0) {
                ...
                if (token.valid() && (ldh->auth(token.id, token.key) == 0)) {
                    ...
                    if (rgw_get_user_info_by_uid(store, token.id, user) < 0) {
                        int ret =
                            rgw_store_user_info(store, user, NULL, NULL,
                                                real_time(),
                                                true);
                        if (ret < 0) {
        ...
        void update_user() {
            ...
            int ret =
                rgw_get_user_info_by_access_key(rgwlib.get_store(), key.id,
                                                user);
            if (ret != 0)
                user = _user;
        }
...
*/
    ...
    class RGWReaddirRequest:public RGWLibRequest, public RGWListBucket  /* RGWOp */
    {
      public:
        ...
        int operator() (const boost::string_ref name,
                        const rgw_obj_key & marker, uint8_t type) {
        ...
        void send_response() override {
          ...
          for (auto & iter:common_prefixes) {
                ...
                lsubdout(cct, rgw,
                    ...
                    " iter first: " << iter.first << " iter second: " << iter.
                    second << dendl;
