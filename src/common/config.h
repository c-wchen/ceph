...
struct md_config_t {
  public:
    typedef std::variant < int64_t ConfigValues::*,
        ...
        entity_addrvec_t ConfigValues::*, uuid_d ConfigValues::*>member_ptr_t;

    // For use when intercepting configuration updates
    typedef std::function < bool(const std::string & k,
                                 const std::string & v) > config_callback;
   ...
   */
    ...
    explicit md_config_t(ConfigValues & values,
                         const ConfigTracker & tracker, bool is_daemon = false);
    ~md_config_t();
    ...
    int parse_buffer(ConfigValues & values, const ConfigTracker & tracker,
                     const char *buf, size_t len, std::ostream * warnings);
    ...
    int set_mon_vals(CephContext * cct,
                     ...
                     std::less <>> &kv, config_callback config_cb);
    ...
    void get_config_bl(const ConfigValues & values,
                       ...
                       ceph::buffer::list * bl, uint64_t * got_version);
    ...
    int get_val(const ConfigValues & values, const std::string_view key,
                char **buf, int len) const;
    int get_val(const ConfigValues & values, const std::string_view key,
                std::string * val) const;
    template < typename T > const T get_val(const ConfigValues & values,
                                            const std::string_view key) const;
    template < typename T, typename Callback,
        typename ... Args > auto with_val(const ConfigValues & values,
                                          const std::string_view key, Callback
                                          && cb, Args
                                          && ... args) const-> std::
        result_of_t < Callback(const T &, Args ...) > {
        return std::forward < Callback > (cb) (std::get < T >
                                               (this->
                                                get_val_generic(values, key)),
                                               std::forward < Args >
                                               (args) ...);
    }
    ...
    std::vector < std::string >
        get_my_sections(const ConfigValues & values) const;
    ...
    void diff(const ConfigValues & values,
              ceph::Formatter * f, std::string name = {
              }) const;
  ...
  private:
    ...
    typedef boost::container::small_vector < std::pair < const Option *,
        const Option::value_t * >, 4 > expand_stack_t;
    ...
    int parse_option(ConfigValues & values,
                     ...
                     std::ostream * oss, int level);
    int parse_injectargs(ConfigValues & values,
                         const ConfigTracker & tracker,
                         std::vector < const char *>&args, std::ostream * oss);

    // @returns negative number for an error, otherwise a
    //          @c ConfigValues::set_value_result_t is returned.
    int _set_val(ConfigValues & values, const ConfigTracker & tracker, const std::string & val, const Option & opt, int level,  // CONF_*
    ...
    void update_legacy_val(ConfigValues & values,
                           const Option & opt, member_ptr_t member);

    Option::value_t _expand_meta(const ConfigValues & values,
  ...
  public:                      // for global_init
    void early_expand_meta(const ConfigValues & values,
                           std::string & val, std::ostream * oss) const;
  ...
  public:
    unsigned get_osd_pool_default_min_size(const ConfigValues & values,
                                           uint8_t size) const {
        uint8_t min_size =
            get_val < uint64_t > (values, "osd_pool_default_min_size");
...
template < typename T >
    const T md_config_t::get_val(const ConfigValues & values,
                                 const std::string_view key) const const
{
