...
public:
    ...
    struct Export {
        Export()
        {
                 ...
                 ceph_seq_t s, ceph_seq_t m, utime_t lis,
                 unsigned st): cap_id(id), wanted(w), issued(i), pending(p),
            client_follows(cf), seq(s), mseq(m), last_issue_stamp(lis),
            state(st)
        {
    ...
    struct Import {
        Import()
        {
        } Import(int64_t i, ceph_seq_t s, ceph_seq_t m): cap_id(i), issue_seq(s),
            mseq(m)
        {
        }
        void encode(ceph::buffer::list &bl) const;
    ...
    struct revoke_info {
        revoke_info()
        {
        } revoke_info(__u32 b, ceph_seq_t s, ceph_seq_t li): before(b), seq(s),
            last_issue(li)
        {
        }
        void encode(ceph::buffer::list &bl) const;
    ...
    const static unsigned MASK_STATE_EXPORTED =
        (STATE_CLIENTWRITEABLE | STATE_NOINLINE | STATE_NOPOOLNS |
         STATE_NOQUOTA);
    ...
    utime_t get_last_issue_stamp() const
    {
        return last_issue_stamp;
    } utime_t get_last_revoke_stamp() const
    {
    ...
    unsigned get_num_revoke_warnings() const
    {
    ...
    //ceph_seq_t get_last_issue() { return last_issue; } bool is_suppress() const {
    return suppress > 0;
} void inc_suppress()
{
    suppress++;
...
void mark_needsnapflush()
{
    state |= STATE_NEEDSNAPFLUSH;
}
void clear_needsnapflush()
{
    state &= ~STATE_NEEDSNAPFLUSH;
}
...
bool is_nopoolns() const
{
    return state & STATE_NOPOOLNS;
}
bool is_noquota() const
{
    return state & STATE_NOQUOTA;
}
CInode *get_inode() const
{
    return inode;
}
...
ceph_seq_t get_last_issue() const
{
    return last_issue;
}
void reset_seq()
...
{
    return Export(cap_id, wanted(), issued(), pending(), client_follows,
                  get_last_seq(), mseq + 1, last_issue_stamp, state);
}
...
int get_lock_cache_allowed() const
{
    return lock_cache_allowed;
}
void set_lock_cache_allowed(int c)
{
    lock_cache_allowed |= c;
}
void clear_lock_cache_allowed(int c)
{
    lock_cache_allowed &= ~c;
}
