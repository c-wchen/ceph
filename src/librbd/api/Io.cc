    ...
    namespace api {
        ...
        namespace {
         ...
         ssize_t Io < I >::read(I & image_ctx, uint64_t off, uint64_t len,
            ...
            aio_read(image_ctx, aio_comp, off, len, std::move(read_result),
                     op_flags, false);
            return ctx.wait();
        }

        template < typename I >
            ssize_t Io < I >::write(I & image_ctx, uint64_t off, uint64_t len,
            ...
            if (r < 0) {
                lderr(cct) << "invalid IO request: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
            ...
            C_SaferCond ctx;
            auto aio_comp = io::AioCompletion::create(&ctx);
            aio_write(image_ctx, aio_comp, off, len, std::move(bl), op_flags,
                      false);
        ...
        template < typename I >
            ...
            C_SaferCond ctx;
            auto aio_comp = io::AioCompletion::create(&ctx);
            aio_discard(image_ctx, aio_comp, off, len,
                        discard_granularity_bytes, false);
        ...
        template < typename I >
            ssize_t Io < I >::write_same(I & image_ctx, uint64_t off,
            ...
            C_SaferCond ctx;
            auto aio_comp = io::AioCompletion::create(&ctx);
            aio_write_same(image_ctx, aio_comp, off, len, std::move(bl),
                           op_flags, false);
        ...
        template < typename I >
            ssize_t Io < I >::write_zeroes(I & image_ctx, uint64_t off,
            ...
            C_SaferCond ctx;
            auto aio_comp = io::AioCompletion::create(&ctx);
            aio_write_zeroes(image_ctx, aio_comp, off, len, zero_flags,
                             op_flags, false);
        ...
        template < typename I >
            ssize_t Io < I >::compare_and_write(I & image_ctx, uint64_t off,
                                                uint64_t len, bufferlist
                                                && cmp_bl, bufferlist
                                                && bl, uint64_t * mismatch_off,
                                                int op_flags) {
            auto cct = image_ctx.cct;
            ldout(cct,
                  20) << "compare_and_write ictx=" << &image_ctx << ", off=" <<
                off << ", " << "len = " << len << dendl;
            ...
            C_SaferCond ctx;
            auto aio_comp = io::AioCompletion::create(&ctx);
            aio_compare_and_write(image_ctx, aio_comp, off, len,
        ...
        template < typename I >
            void Io < I >::aio_read(I & image_ctx, io::AioCompletion * aio_comp,
                                    uint64_t off, uint64_t len, io::ReadResult
            ...
            aio_comp->init_time(util::get_image_ctx(&image_ctx),
                                io::AIO_TYPE_READ);
            ldout(cct,
                  20) << "ictx=" << &image_ctx << ", " << "completion=" <<
                aio_comp << ", off=" << off << ", " << "len=" << len << ", " <<
            ...
            auto req =
                io::ImageDispatchSpec::create_read(image_ctx,
                                                   io::
                                                   IMAGE_DISPATCH_LAYER_API_START,
                                                   aio_comp,
                                                   { {off, len}
                                                   }
                                                   , io::ImageArea::DATA,
        ...
        template < typename I >
            void Io < I >::aio_write(I & image_ctx,
                                     io::AioCompletion * aio_comp, uint64_t off,
                                     uint64_t len, bufferlist
                                     && bl, int op_flags, bool native_async) {
            auto cct = image_ctx.cct;
            FUNCTRACE(cct);
            ZTracer::Trace trace;
            if (image_ctx.blkin_trace_all) {
            ...
            auto req =
                io::ImageDispatchSpec::create_write(image_ctx,
                                                    io::
                                                    IMAGE_DISPATCH_LAYER_API_START,
                                                    aio_comp,
                                                    { {off, len}
        ...
        template < typename I >
            void Io < I >::aio_discard(I & image_ctx,
            ...
            ldout(cct,
                  20) << "ictx=" << &image_ctx << ", " << "completion=" <<
                aio_comp << ", off=" << off << ", " << "len=" << len << dendl;
            ...
            auto req =
                io::ImageDispatchSpec::create_discard(image_ctx,
                                                      io::
                                                      IMAGE_DISPATCH_LAYER_API_START,
                                                      aio_comp,
                                                      { {off, len}
        ...
        template < typename I >
            void Io < I >::aio_write_same(I & image_ctx,
                                          io::AioCompletion * aio_comp,
            ...
            auto req =
                io::ImageDispatchSpec::create_write_same(image_ctx,
                                                         io::
                                                         IMAGE_DISPATCH_LAYER_API_START,
                                                         aio_comp,
                                                         { {off, len}
        ...
        template < typename I >
            void Io < I >::aio_write_zeroes(I & image_ctx,
                                            io::AioCompletion * aio_comp,
                                            uint64_t off, uint64_t len,
            ...
            if (io_type == io::AIO_TYPE_WRITESAME) {
                ...
                uint64_t write_same_offset_end =
                    p2align(off + len, data_length);
                uint64_t write_same_length = 0;
                if (write_same_offset_end > write_same_offset) {
                    write_same_length =
                        write_same_offset_end - write_same_offset;
                }
                ...
                uint64_t append_length =
                    len - prepend_length - write_same_length;
                ldout(cct,
                      20) << "prepend_offset=" << prepend_offset << ", " <<
                ...
                ceph_assert(prepend_length + write_same_length +
                            append_length == len);
                ...
                if (prepend_length > 0) {
                    ...
                    auto prepend_aio_comp =
                        io::AioCompletion::create_and_start(prepend_ctx,
                                                            &image_ctx,
                                                            io::AIO_TYPE_WRITE);
                    auto prepend_req =
                        io::ImageDispatchSpec::create_write(image_ctx,
                ...
                if (append_length > 0) {
                    ...
                    auto append_aio_comp =
                        io::AioCompletion::create_and_start(append_ctx,
                                                            &image_ctx,
                                                            io::AIO_TYPE_WRITE);
                    auto append_req =
                        io::ImageDispatchSpec::create_write(image_ctx,
                ...
                auto write_same_aio_comp =
                    io::AioCompletion::create_and_start(write_same_ctx,
                                                        &image_ctx,
                                                        io::AIO_TYPE_WRITESAME);
                auto req =
                    io::ImageDispatchSpec::create_write_same(image_ctx,
        ...
        template < typename I >
            void Io < I >::aio_compare_and_write(I & image_ctx,
                                                 io::AioCompletion * aio_comp,
                                                 uint64_t off, uint64_t len,
                                                 bufferlist
                                                 && cmp_bl, bufferlist
            ...
            auto req =
                io::ImageDispatchSpec::create_compare_and_write(image_ctx,
                                                                io::
                                                                IMAGE_DISPATCH_LAYER_API_START,
                                                                aio_comp,
                                                                { {off, len}
        ...
        template < typename I >
            void Io < I >::aio_flush(I & image_ctx,
                                     io::AioCompletion * aio_comp,
                                     bool native_async) {
            auto cct = image_ctx.cct;
            FUNCTRACE(cct);
            ...
            auto req =
                io::ImageDispatchSpec::create_flush(image_ctx,
                                                    io::
                                                    IMAGE_DISPATCH_LAYER_API_START,
                                                    aio_comp,
