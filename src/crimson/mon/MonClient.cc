...
public:
    ...
    auth::AuthClient::auth_request_t get_auth_request(const EntityName& name,
            uint32_t want_keys);
    ...
    tuple < CryptoKey, secret_t, int > handle_auth_done(uint64_t new_global_id,
...
tuple < CryptoKey, Connection::secret_t,
bufferlist > Connection::handle_auth_reply_more(const ceph::buffer::list& payload)
{
...
Client::command_result_t Client::run_command(std::string&& cmd,
        bufferlist&& bl)
{
