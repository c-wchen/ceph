                               ...
                               bool snapshot = false)
{
    ...
    int r =
        image.mirror_image_get_info(&mirror_image,
                                    sizeof(mirror_image));
    if (r < 0) {
    ...
    if (mirror_image.state != RBD_MIRROR_IMAGE_ENABLED) {
        std::
        cerr << "rbd: mirroring not enabled on the image" <<
             std::endl;
        return -EINVAL;
    }
    if (snapshot) {
...
int execute_enable_disable(const po::variables_map &vm,
                           bool enable, bool force)
{
    ...
    if (enable) {
        ...
        if (mode_arg == "journal") {
            ...
            std::
            cerr << "rbd: invalid mode name: " << mode_arg <<
                 std::endl;
            return -EINVAL;
        }
                    ...
                    &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: error promoting image to primary" << std::
             endl;
        return r;
    }
                   ...
                   &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: error demoting image to non-primary" <<
             std::endl;
        return r;
    }
                   ...
                   &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::cerr << "rbd: error flagging image resync" << std::
                  endl;
        return r;
    }

    std::cout << "Flagged image for resync from primary" << std::
              endl;
    return 0;
}
                   ...
                   &ceph_global_init_args)
{
    ...
    r = image.mirror_image_get_global_status(&status,
        sizeof(status));
    if (r < 0) {
        std::
        cerr << "rbd: failed to get status for image " <<
             image_name << ": " << cpp_strerror(r) << std::endl;
        return r;
    }

    utils::populate_unknown_mirror_image_site_statuses(mirror_peers,
            &status);
    ...
    status.site_statuses.
    erase(std::
          remove_if(status.site_statuses.begin(),
    ...
    if (status.info.primary
        && status.info.state == RBD_MIRROR_IMAGE_ENABLED) {
        librbd::mirror_image_mode_t mode =
            RBD_MIRROR_IMAGE_MODE_JOURNAL;
        r = image.mirror_image_get_mode(&mode);
        if (r < 0) {
        ...
        if (mode == RBD_MIRROR_IMAGE_MODE_SNAPSHOT) {
            ...
            snap_info_t &snap) {
                librbd::
                snap_namespace_type_t type;
                int r =
                    image.
                    snap_get_namespace_type(snap.id,
                                            &type);
                if (r < 0) {
                    return false;
                }
    ...
    auto mirror_service =
        daemon_service_info.get_by_instance_id(instance_id);
    ...
    if (formatter != nullptr) {
        ...
        if (local_site_r >= 0) {
            formatter->dump_string("state",
                                   utils::
                                   mirror_image_site_status_state
                                   (local_status));
            formatter->dump_string("description",
                                   local_status.description);
            if (mirror_service != nullptr) {
                mirror_service->dump_image(formatter);
            }
            formatter->dump_string("last_update",
                                   utils::timestr(local_status.
                                                  last_update));
        }
        if (!status.site_statuses.empty()) {
            ...
            for (auto &status : status.
                ...
                auto name_it =
                    peer_mirror_uuids_to_name.find(status.
                                                   mirror_uuid);
                formatter->dump_string("site_name",
                                       (name_it !=
                                        ...
                                        second : ""));
                formatter->dump_string("mirror_uuids",
                                       status.mirror_uuid);
        ...
        if (!snaps.empty()) {
            ...
            for (auto &snap : snaps) {
                ...
                r = image.snap_get_mirror_namespace(snap.id, &info,
                                                    sizeof(info));
                if (r < 0
        ...
        if (local_site_r >= 0) {
            ...
            std::cout << "  last_update: " << utils::
                      timestr(local_status.last_update) << std::endl;
        }
        if (!status.site_statuses.empty()) {
            std::cout << "  peer_sites:" << std::endl;

            bool first_site = true;
            for (auto &site : status.
                 site_statuses) {
                if (!first_site) {
                    std::cout << std::endl;
                }
                ...
                auto name_it =
                    peer_mirror_uuids_to_name.find(site.
                                                   mirror_uuid);
                std::cout << "    name: " << (name_it !=
                          ...
                          mirror_image_site_status_state(site) << std::
                     ...
                     &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: error creating snapshot: " <<
             cpp_strerror(r)
             << std::endl;
        return r;
    }
