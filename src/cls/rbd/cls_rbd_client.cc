...
namespace librbd {
    namespace cls_client {
        ...
        int get_features_finish(bufferlist::const_iterator * it,
                                uint64_t * features,
        ...
        int get_data_pool_finish(bufferlist::const_iterator * it,
                                 int64_t * data_pool_id) {
            try {
        ...
        int get_snapcontext_finish(bufferlist::const_iterator *
        ...
        template < typename O, typename E >
            void sparse_copyup(O * op, const E & extent_map,
        ...
        void sparse_copyup(librados::ObjectWriteOperation * op,
                           ...
                           bufferlist data) {
            sparse_copyup < librados::ObjectWriteOperation > (op, extent_map,
                                                              data);
        }
        ...
        int snapshot_get_limit_finish(bufferlist::const_iterator * it,
                                      uint64_t * limit) {
            try {
                decode(*limit, *it);
            } catch(const ceph::buffer::error & err) {
...
/******************** rbd_directory object methods ********************/
        ...
        int dir_get_id_finish(bufferlist::const_iterator * iter,
                              std::string * image_id) {
            try {
        ...
        int dir_get_name_finish(bufferlist::const_iterator * it,
                                std::string * name) {
            try {
                decode(*name, *it);
            } catch(const ceph::buffer::error & err) {
        ...
        int dir_list_finish(bufferlist::const_iterator * it, map < string,
                            string > *images) {
            try {
                decode(*images, *it);
            } catch(const ceph::buffer::error & err) {
        ...
        void assert_snapc_seq(librados::ObjectWriteOperation * op,
                              uint64_t snapc_seq,
                              cls::rbd::AssertSnapcSeqState state) {
            assert_snapc_seq < librados::ObjectWriteOperation > (op, snapc_seq,
                                                                 state);
        }
        ...
        int mirror_mode_get_finish(bufferlist::const_iterator * it,
            ...
            try {
                ...
                *mirror_mode =
                    static_cast < cls::rbd::MirrorMode > (mirror_mode_decode);
            } catch(const ceph::buffer::error & err) {
                return -EBADMSG;
            }
        ...
        void mirror_peer_ping(librados::ObjectWriteOperation * op,
            ...
            encode(fsid, in_bl);
            encode(static_cast < uint8_t > (cls::rbd::MIRROR_PEER_DIRECTION_TX),
                   in_bl);
        ...
        int mirror_peer_remove(librados::IoCtx * ioctx,
            ...
            bufferlist out_bl;
            int r =
                ioctx->exec(RBD_MIRRORING, "rbd", "mirror_peer_remove", in_bl,
                            out_bl);
            if (r < 0) {
                return r;
            }
                                      ...
                                      MirrorPeerDirection mirror_peer_direction)
        {
            ...
            bufferlist out_bl;
            int r =
                ioctx->exec(RBD_MIRRORING, "rbd", "mirror_peer_set_direction",
                            in_bl, out_bl);
            if (r < 0) {
                return r;
            }
        ...
        int mirror_image_status_get_summary_finish(bufferlist::const_iterator *
                                                   iter,
                                                   std::map <
                                                   cls::rbd::
...
// Groups functions
        ...
        int group_snap_get_by_id(librados::IoCtx * ioctx,
            ...
            encode(snap_id, inbl);
            int r =
                ioctx->exec(oid, "rbd", "group_snap_get_by_id", inbl, outbl);
            if (r < 0) {
                return r;
            }
