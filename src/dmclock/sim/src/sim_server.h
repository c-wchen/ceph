        ...
        template < typename Q, typename ReqPm, typename RespPm, typename Accum >
            ...
            struct QueueItem {
                ...
                RespPm additional;
                 ...
                 QueueItem(const ClientId & _client,
                           ...
                           const RespPm & _additional):client(_client),
                    request(std::move(_request)), additional(_additional) {
                    // empty
            }};                 // QueueItem
          ...
          public:
            ...
            struct InternalStats {
                ...
                uint32_t request_complete_count;
                 ...
                 InternalStats():add_request_time(0),
                    request_complete_time(0),
                    add_request_count(0), request_complete_count(0) {
                    // empty
            }};
          ...
          public:
            ...
            using HandleRequestFunc =
                std::function < void (const ClientId &,
                                      std::unique_ptr < TestRequest >,
                                      const RespPm &) >;
            using CreateQueueF =
            ...
            SimulatedServer(ServerId _id,
                            ...
                            CreateQueueF _create_queue_f):id(_id),
                priority_queue(_create_queue_f
                               (std::
                                bind(&SimulatedServer::has_avail_thread, this),
                                std::bind(&SimulatedServer::inner_post, this,
                                          std::placeholders::_1,
                                          std::placeholders::_2,
                                          std::placeholders::_3))),
                client_resp_f(_client_resp_f), iops(_iops),
                thread_pool_size(_thread_pool_size), finishing(false),
                accum_f(_accum_f) {
                op_time =
                    std::chrono::
                    microseconds((int)
                                 (0.5 + thread_pool_size * 1000000.0 / iops));
                std::chrono::milliseconds delay(1000);
                threads = new std::thread[thread_pool_size];
                for (size_t i = 0; i < thread_pool_size; ++i) {
                    threads[i] =
                        std::thread(&SimulatedServer::run, this, delay);
                }
            }
            ...
            void post(TestRequest && request,
                      const ClientId & client_id, const ReqPm & req_params) {
            ...
            const Accum & get_accumulator() const {
                return accumulator;
            } const Q & get_priority_queue() const {
            ...
            void run(std::chrono::milliseconds check_period) {
                ...
                while (true) {
                    ...
                    if (!inner_queue.empty()) {
                        ...
                        client_resp_f(client, TestResponse {
                                      req->epoch}
                                      , id, additional);
