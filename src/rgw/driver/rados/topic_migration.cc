                           ...
                           std::string& marker, const DoutPrefixProvider* dpp)
{
    ...
    if (pos == std::string::npos) {
        ldpp_dout(dpp, 1) << "ERROR: bucket_topics_oid:" << bucket_topics_oid << " doesn't contain " <<
    ...
    if (pos == std::string::npos) {
        ldpp_dout(dpp, 1) << "ERROR: bucket_topics_oid:" << bucket_topics_oid << " doesn't contain / after bucket name!" <<
                          dendl;
        return -EINVAL;
    }
                   ...
                   const rgw_raw_obj& topics_obj)
{
    ...
    if (r == -ENOENT) {
        ldpp_dout(dpp, 20) << "INFO: v1 topics object: " << topics_obj.to_str() << " does not exists. already migrated" <<
                           dendl;
        return 0; // ok, someone else already migrated
    }
    if (r < 0) {
    ...
    for (const auto& [name, topic] : topics.topics) {
        ...
        if (r == -EEXIST) {
            ldpp_dout(dpp, 20) << "INFO: v1 topics object: " << topics_obj.to_str() <<
                                  " already migrated. no need to write v2 object" << dendl;
    ...
    if (r == -ECANCELED || r == -ENOENT) {
        ldpp_dout(dpp, 20) << "INFO: v1 topics object: " << topics_obj.to_str() << " already migrated. no need to remove" <<
                           dendl;
        return 0; // ok, someone else already migrated
    }
    if (r < 0) {
