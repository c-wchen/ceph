                         ...
                         const overwrite_plan_t &overwrite_plan)
{
    ...
    if (overwrite_plan.left_operation ==
        ...
        return get_iertr::make_ready_future < operate_ret_bare > ((zero_extent_len == 0 ? std::nullopt : std::make_optional(
                    extent_to_write_t::create_zero(overwrite_plan.pin_begin, zero_extent_len))),
                (zero_prepend_len == 0 ? std::nullopt : std::make_optional(bufferptr(ceph::buffer::create(zero_prepend_len, 0))))
                                                                 );
                          ...
                          const overwrite_plan_t &overwrite_plan)
{
    ...
    if (overwrite_plan.right_operation ==
        ...
        assert_aligned(zero_extent_len);
        return get_iertr::make_ready_future < operate_ret_bare > ((zero_extent_len == 0 ? std::nullopt : std::make_optional(
                    extent_to_write_t::create_zero(overwrite_plan.aligned_data_end, zero_extent_len))),
                (zero_suffix_len == 0 ? std::nullopt : std::make_optional(bufferptr(ceph::buffer::create(zero_suffix_len, 0))))
                                                                 );
    ...
    return seastar::do_with(bufferlist(), [ctx, obj_offset, len](auto & ret) {
        ...
        object_data) {
            ...
            _pins) {
                ...
                return seastar::do_with(std::move(_pins), loffset, [ctx, loffset, len, &ret](auto & pins, auto & current) {
                    return
                        trans_intr::
                        do_for_each
                        (pins,
                    ...
                    <> {
                        ...
                        (current
                         ...
                         (loffset
                          ...
                          len));
                        ceph_assert
                        ((loffset + len) > pin->get_key()); laddr_t end = std::min(pin->get_key() + pin->get_length(), loffset + len);
                        if (pin->get_val().is_zero()) {
                            ...
                            ([&ret, &current, end](auto extent) {
                                ...
                                ret.append(bufferptr(extent->get_bptr(), current - extent->get_laddr(), end - current));
                                current = end;
                                return seastar::now();
                            }
                            ).
