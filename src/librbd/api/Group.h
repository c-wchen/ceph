...
template < typename ImageCtxT = librbd::ImageCtx > struct Group {
    ...
    static int rename(librados::IoCtx &io_ctx,
                      const char *src_group_name,
                      const char *dest_group_name);

    static int image_add(librados::IoCtx &group_ioctx,
                         const char *group_name,
                         librados::IoCtx &image_ioctx,
                         const char *image_name);
    static int image_remove(librados::IoCtx &group_ioctx,
                            const char *group_name,
                            librados::IoCtx &image_ioctx,
                            const char *image_name);
    static int image_remove_by_id(librados::IoCtx &group_ioctx,
                                  ...
                                  librados::IoCtx &image_ioctx,
                                  const char *image_id);
    static int image_list(librados::IoCtx &group_ioctx,
                          const char *group_name,
                          std::vector < group_image_info_t > *images);

    static int image_get_group(ImageCtxT *ictx,
    ...
    static int snap_rename(librados::IoCtx &group_ioctx,
                           const char *group_name,
                           const char *old_snap_name,
                           const char *new_snap_name);
    static int snap_list(librados::IoCtx &group_ioctx,
                         const char *group_name,
