...
namespace {
    class PG_SendMessageOnConn:public Context {
        ...
        ConnectionRef conn;
      public:
         PG_SendMessageOnConn(PGBackend::Listener * pg,
                              Message * reply,
                              ConnectionRef conn):pg(pg), reply(reply),
            conn(conn) {
    ...
    class PG_RecoveryQueueAsync:public Context {
      ...
      public:
        PG_RecoveryQueueAsync(PGBackend::Listener * pg,
                              GenContext < ThreadPool::TPHandle & >*c):pg(pg),
            c(c) {
        } void finish(int) override {
...
ReplicatedBackend::ReplicatedBackend(PGBackend::Listener * pg, coll_t coll, ObjectStore::CollectionHandle & c, ObjectStore * store, CephContext * cct):
PGBackend(cct, pg, store, coll,
          c)
{
}
...
void ReplicatedBackend::check_recovery_sources(const OSDMapRef & osdmap)
{
    for (map < pg_shard_t, set < hobject_t > >::iterator i =
         pull_from_peer.begin(); i != pull_from_peer.end();) {
        if (osdmap->is_down(i->first.osd)) {
            dout(10) << "check_recovery_sources resetting pulls from osd." <<
                i->first << ", osdmap has it marked down" << dendl;
...
bool ReplicatedBackend::_handle_message(OpRequestRef op)
{
    ...
    case MSG_OSD_SUBOP:{
            const MOSDSubOp *m =
                static_cast < const MOSDSubOp * >(op->get_req());
            if (m->ops.size() == 0) {
                assert(0);
            }
            break;
...
void ReplicatedBackend::on_change()
{
    ...
    for (map < ceph_tid_t, InProgressOp >::iterator i = in_progress_ops.begin();
         i != in_progress_ops.end(); in_progress_ops.erase(i++)) {
...
int ReplicatedBackend::objects_read_sync(const hobject_t & hoid,
                                         uint64_t off,
                                         uint64_t len,
...
void ReplicatedBackend::objects_read_async(const hobject_t & hoid,
                                           const list < pair < boost::tuple <
                                           ...
                                           bool fast_read)
{
    ...
    for (list < pair < boost::tuple < uint64_t, uint64_t, uint32_t >,
         ...
         to_read.begin(); i != to_read.end() && r >= 0; ++i) {
...
class C_OSD_OnOpApplied:public Context {
    ...
    ReplicatedBackend::InProgressOp * op;
  public:
    C_OSD_OnOpApplied(ReplicatedBackend * pg,
                      ReplicatedBackend::InProgressOp * op)
    :pg(pg), op(op) {
    } void finish(int) override {
        pg->op_applied(op);
...
void generate_transaction(PGTransactionUPtr & pgt,
                          const coll_t & coll,
                          ...
                          set < hobject_t > *added, set < hobject_t > *removed)
{
  ...
  for (auto && le:log_entries) {
    ...
    pgt->safe_create_traverse([&]
                              (pair < const hobject_t,
                               PGTransaction::ObjectOperation > &obj_op) {
                              const hobject_t & oid = obj_op.first;
                              const ghobject_t goid =
                              ghobject_t(oid, ghobject_t::NO_GEN,
                                         shard_id_t::NO_SHARD);
                              const PGTransaction::ObjectOperation & op =
                              obj_op.second; if (oid.is_temp()) {
          ...
          map < string, bufferlist > attrs; for (auto && p:op.attr_updates)
                              {
                              if (p.second)
                              ...
                              if (op.clear_omap)
                              t->omap_clear(coll, goid); if (op.omap_header)
      t->omap_setheader(coll, goid, *(op.omap_header)); for (auto && up:op.omap_updates)
                              {
                              using UpdateType =
                              PGTransaction::ObjectOperation::OmapUpdateType;
                              switch (up.first) {
case UpdateType::Remove:
t->omap_rmkeys(coll, goid, up.second); break; case UpdateType::Insert:
                              t->omap_setkeys(coll, goid, up.second); break;}
                              }
      ...
      for (auto && extent:   op.buffer_updates) {
                              ...
                              match(extent.get_val(),
                                    ...
                                    t->write(coll, goid, extent.get_off(),
                                             extent.get_len(), op.buffer);},
                                    [&](const BufferUpdate::Zero & op) {
                                    t->zero(coll, goid, extent.get_off(),
                                            extent.get_len());},
...
void ReplicatedBackend::submit_transaction(const hobject_t & soid,
                                           ...
                                           PGTransactionUPtr
                                           && _t, const eversion_t & trim_to,
                                           const eversion_t & roll_forward_to,
                                           const vector < pg_log_entry_t >
                                           &_log_entries,
                                           boost::optional <
                                           pg_hit_set_history_t > &hset_history,
                                           ...
                                           OpRequestRef orig_op)
{
    ...
    generate_transaction(t,
                         coll,
                         (get_osdmap()->require_osd_release <
                          CEPH_RELEASE_KRAKEN), log_entries, &op_t, &added,
                         &removed);
    assert(added.size() <= 1);
    assert(removed.size() <= 1);

    assert(!in_progress_ops.count(tid));
    InProgressOp & op = in_progress_ops.insert(make_pair(tid,
                                                         InProgressOp(tid,
                                                                      on_all_commit,
                                                                      on_all_acked,
                                                                      orig_op,
    ...
    op.waiting_for_applied.insert(parent->get_actingbackfill_shards().begin(),
                                  parent->get_actingbackfill_shards().end());
    op.waiting_for_commit.insert(parent->get_actingbackfill_shards().begin(),
                                 parent->get_actingbackfill_shards().end());
    ...
    issue_op(soid,
             ...
             log_entries, hset_history, &op, op_t);

    add_temp_objs(added);
    clear_temp_objs(removed);

    parent->log_operation(log_entries,
                          hset_history, trim_to, at_version, true, op_t);

    op_t.register_on_applied_sync(on_local_applied_sync);
    op_t.register_on_applied(parent->
                             bless_context(new C_OSD_OnOpApplied(this, &op)));
    op_t.register_on_commit(parent->
                            bless_context(new C_OSD_OnOpCommit(this, &op)));
...
{
    FUNCTRACE();
    OID_EVENT_TRACE_WITH_MSG((op
                              && op->op) ? op->op->get_req() : NULL,
...
void ReplicatedBackend::do_repop_reply(OpRequestRef op)
{
    ...
    const MOSDRepOpReply *r =
        static_cast < const MOSDRepOpReply * >(op->get_req());
                                     ...
                                     ScrubMap::object & o)
{
    ...
    if (!pos.data_done()) {
        ...
        if (r == cct->_conf->osd_deep_scrub_stride) {
            dout(20) << __func__ << "  " << poid <<
                " more data, digest so far 0x" << std::hex << pos.data_hash.
                digest() << std::dec << dendl;
            return -EINPROGRESS;
        }
        // done with bytes
        pos.data_pos = -1;
    ...
    // omap
    ObjectMap::ObjectMapIterator iter = store->get_omap_iterator(coll,
                                                                 ghobject_t
                                                                 (poid,
                                                                  ghobject_t::
                                                                  NO_GEN,
    ...
    if (pos.omap_keys >
        ...
        dout(25) << __func__ << " " << poid <<
            " large omap object detected. Object has " << pos.
            omap_keys << " keys and size " << pos.
            omap_bytes << " bytes" << dendl;
...
void ReplicatedBackend::_do_push(OpRequestRef op)
{
    ...
    t.register_on_complete(new
                           PG_SendMessageOnConn(get_parent(), reply,
                                                m->get_connection()));
...
void ReplicatedBackend::_do_pull_response(OpRequestRef op)
{
    ...
    if (!to_continue.empty()) {
        ...
        t.register_on_complete(new PG_RecoveryQueueAsync(get_parent(),
                                                         get_parent()->
                                                         bless_gencontext(c)));
    }
...
void ReplicatedBackend::do_push_reply(OpRequestRef op)
{
    const MOSDPGPushReply *m =
        static_cast < const MOSDPGPushReply * >(op->get_req());
...
Message *ReplicatedBackend::generate_subop(const hobject_t & soid,
                                           ...
                                           &log_entries,
                                           boost::optional <
                                           pg_hit_set_history_t > &hset_hist,
                                           ObjectStore::Transaction & op_t,
                                           pg_shard_t peer,
                                           const pg_info_t & pinfo)
{
    ...
    MOSDRepOp *wr = new MOSDRepOp(reqid, parent->whoami_shard(),
    ...
    if (!parent->should_send_op(peer, soid)) {
        dout(10) << "issue_repop shipping empty opt to osd." << peer
            ...
            << ", pinfo.last_backfill " << pinfo.last_backfill << ")" << dendl;
        ObjectStore::Transaction t;
                                 ...
                                 ObjectStore::Transaction & op_t)
{
    ...
    for (set < pg_shard_t >::const_iterator i =
         ...
         i != parent->get_actingbackfill_shards().end(); ++i) {
        ...
        wr = generate_subop(soid,
                            ...
                            new_temp_oid,
                            discard_temp_oid,
                            log_entries, hset_hist, op_t, peer, pinfo);
        if (op->op)
            wr->trace.init("replicated op", nullptr, &op->op->pg_trace);
        get_parent()->send_message_osd_cluster(peer.osd, wr,
                                               get_osdmap()->get_epoch());
    }
}
...
void ReplicatedBackend::do_repop(OpRequestRef op)
{
    ...
    if (m->new_temp_oid != hobject_t()) {
        dout(20) << __func__ << " start tracking temp " << m->
            new_temp_oid << dendl;
        add_temp_obj(m->new_temp_oid);
    }
    if (m->discard_temp_oid != hobject_t()) {
        dout(20) << __func__ << " stop tracking temp " << m->
            discard_temp_oid << dendl;
        if (rm->opt.empty()) {
    ...
    parent->log_operation(log,
                          m->updated_hit_set_history,
                          m->pg_trim_to,
                          m->pg_roll_forward_to, update_snaps, rm->localt);

    rm->opt.register_on_commit(parent->
                               bless_context(new
                                             C_OSD_RepModifyCommit(this, rm)));
    rm->localt.register_on_applied(parent->
                                   bless_context(new
                                                 C_OSD_RepModifyApply(this,
                                                                      rm)));
    vector < ObjectStore::Transaction > tls;
    tls.reserve(2);
...
void ReplicatedBackend::repop_applied(RepModifyRef rm)
{
    ...
    if (!rm->committed) {
        Message *ack = new MOSDRepOpReply(req, parent->whoami_shard(),
                                          0, get_osdmap()->get_epoch(),
                                          req->min_epoch, CEPH_OSD_FLAG_ACK);
        ack->set_priority(CEPH_MSG_PRIO_HIGH);  // this better match commit priority!
        ack->trace = rm->op->pg_trace;
        get_parent()->send_message_osd_cluster(rm->ackerosd, ack,
                                               get_osdmap()->get_epoch());
    }
...
void ReplicatedBackend::repop_commit(RepModifyRef rm)
{
    ...
    dout(10) << __func__ << " on op " << *m
        << ", sending commit to osd." << rm->ackerosd << dendl;
...
void ReplicatedBackend::calc_head_subsets(ObjectContextRef obc,
                                          SnapSet & snapset,
                                          const hobject_t & head,
                                          const pg_missing_t & missing,
                                          const hobject_t & last_backfill,
                                          interval_set < uint64_t >
                                          &data_subset, map < hobject_t,
                                          interval_set <
                                          ...
                                          ObcLockManager & manager)
{
    ...
    for (int j = snapset.clones.size() - 1; j >= 0; j--) {
        ...
        if (!missing.is_missing(c) &&
            c < last_backfill && get_parent()->try_lock_for_read(c, manager)) {
...
void ReplicatedBackend::calc_clone_subsets(SnapSet & snapset,
...
void ReplicatedBackend::prepare_pull(eversion_t v,
                                     const hobject_t & soid,
                                     ObjectContextRef headctx, RPGHandle * h)
{
    assert(get_parent()->get_local_missing().get_items().count(soid));
    eversion_t _v =
        get_parent()->get_local_missing().get_items().find(soid)->second.need;
    assert(_v == v);
    const map < hobject_t,
        set <
        pg_shard_t >> &missing_loc(get_parent()->get_missing_loc_shards());
    const map < pg_shard_t,
        pg_missing_t > &peer_missing(get_parent()->get_shard_missing());
    map < hobject_t, set < pg_shard_t >>::const_iterator q =
        missing_loc.find(soid);
    ...
    dout(7) << "pull " << soid
        << " v " << v
        << " on osds " << q->second << " from osd." << fromshard << dendl;
    ...
    if (pmissing.is_missing(soid, v)) {
        ...
        assert(get_parent()->get_log().get_log().objects.count(soid) &&
               (get_parent()->get_log().get_log().objects.find(soid)->second->
                op == pg_log_entry_t::LOST_REVERT)
               && (get_parent()->get_log().get_log().objects.find(soid)->
    ...
    if (soid.is_snap()) {
        assert(!get_parent()->get_local_missing().is_missing(soid.get_head()) ||
               !get_parent()->get_local_missing().is_missing(soid.
                                                             get_snapdir()));
        assert(headctx);
        // check snapset
                                            ...
                                            bool cache_dont_need)
{
    ...
    if (soid.snap && soid.snap < CEPH_NOSNAP) {
        ...
        calc_head_subsets(obc,
                          ...
                          get_parent()->get_shard_info().find(peer)->second.
                          last_backfill, data_subset, clone_subsets,
                          lock_manager);
    }

    return prep_push(obc,
                     soid,
                     peer,
                     oi.version,
                     data_subset,
                     clone_subsets,
                     pop, cache_dont_need, std::move(lock_manager));
}
...
int ReplicatedBackend::prep_push(ObjectContextRef obc,
                                 ...
                                 interval_set < uint64_t > &data_subset,
                                 map < hobject_t,
                                 interval_set < uint64_t >> &clone_subsets,
                                 PushOp * pop, bool cache_dont_need,
                                 ObcLockManager && lock_manager)
{
...
submit_push_data(const ObjectRecoveryInfo & recovery_info, bool first,
                 bool complete, bool cache_dont_need,
                 const interval_set < uint64_t > &intervals_included,
                 bufferlist data_included, bufferlist omap_header,
                 const map < string, bufferlist > &attrs, const map < string,
                 bufferlist > &omap_entries, ObjectStore::Transaction * t)
{
    ...
    for (interval_set < uint64_t >::const_iterator p =
         intervals_included.begin(); p != intervals_included.end(); ++p) {
        bufferlist bit;
                     ...
                     ObjectStore::Transaction * t)
{
    for (map < hobject_t, interval_set < uint64_t >>::const_iterator p =
        ...
        for (interval_set < uint64_t >::const_iterator q = p->second.begin();
            ...
            t->clone_range(coll, ghobject_t(p->first),
                           ghobject_t(recovery_info.soid), q.get_start(),
                           q.get_len(), q.get_start());
        }
    }
}

ObjectRecoveryInfo ReplicatedBackend::
recalc_subsets(const ObjectRecoveryInfo & recovery_info, SnapSetContext * ssc,
               ObcLockManager & manager)
{
                                             ...
                                             ObjectStore::Transaction * t)
{
    ...
    dout(10) << "handle_pull_response "
        << pop.recovery_info
        << pop.after_progress << " data.size() is " << data.length()
    ...
    if (pi.recovery_info.size == (uint64_t(-1))) {
        ...
        pi.recovery_info.copy_subset.intersection_of(pop.recovery_info.
                                                     copy_subset);
    }
    ...
    dout(10) << "new recovery_info " << pi.recovery_info
        << ", new progress " << pi.recovery_progress << dendl;
    ...
    if (complete) {
        ...
        to_continue->push_back( {
                               hoid, pi.stat}
        );
        get_parent()->on_local_recover(hoid, pi.recovery_info, pi.obc, false,
                                       t);
        return false;
    }
    else {
...
{
    dout(10) << "handle_push "
        << pop.recovery_info << pop.after_progress << dendl;
    bufferlist data;
    data = pop.data;
    ...
    submit_push_data(pop.recovery_info, first, complete, true,  // must be replicate
                     pop.data_included,
                     data, pop.omap_header, pop.attrset, pop.omap_entries, t);

    if (complete)
        get_parent()->on_local_recover(pop.recovery_info.soid, pop.recovery_info, ObjectContextRef(),   // ok, is replica
                                       false, t);
}

void ReplicatedBackend::send_pushes(int prio, map < pg_shard_t,
                                    vector < PushOp > >&pushes)
{
    for (map < pg_shard_t, vector < PushOp > >::iterator i = pushes.begin();
         i != pushes.end(); ++i) {
        ConnectionRef con = get_parent()->get_con_osd_cluster(i->first.osd,
                                                              get_osdmap()->
                                                              get_epoch());
        if (!con)
            continue;
        ...
        while (j != i->second.end()) {
            ...
            for (;
                 (j != i->second.end() &&
                  ...
                  pushes < cct->_conf->osd_max_push_objects); ++j) {
...
void ReplicatedBackend::send_pulls(int prio, map < pg_shard_t,
                                   vector < PullOp > >&pulls)
{
                                     ...
                                     bool cache_dont_need)
{
    ...
    if (progress.first) {
        int r =
            store->omap_get_header(coll, ghobject_t(recovery_info.soid),
                                   &out_op->omap_header);
        if (r < 0) {
            dout(1) << __func__ << " get omap header failed: " <<
                cpp_strerror(-r) << dendl;
            return r;
        }
        r = store->getattrs(ch, ghobject_t(recovery_info.soid),
                            out_op->attrset);
        if (r < 0) {
            dout(1) << __func__ << " getattrs failed: " << cpp_strerror(-r) <<
                dendl;
            return r;
        }
        ...
        try {
            ...
            dout(0) << __func__ << ": bad object_info_t: " << recovery_info.
                soid << dendl;
            return -EINVAL;
        }
    ...
    if (available > 0) {
        if (!recovery_info.copy_subset.empty()) {
            ...
            out_op->data_included.span_of(copy_subset,
            ...
            if (out_op->data_included.empty())  // zero filled section, skip to end!
                ...
                new_progress.data_recovered_to =
                    out_op->data_included.range_end();
        }
    }
    else {
    ...
    for (interval_set < uint64_t >::iterator p = out_op->data_included.begin();
         p != out_op->data_included.end(); ++p) {
        bufferlist bit;
        int r = store->read(ch, ghobject_t(recovery_info.soid),
                            p.get_start(), p.get_len(), bit,
                            cache_dont_need ? CEPH_OSD_OP_FLAG_FADVISE_DONTNEED
                            : 0);
        if (cct->_conf->osd_debug_random_push_read_error
            && (rand() %
                (int)(cct->_conf->osd_debug_random_push_read_error * 100.0)) ==
            0) {
            dout(0) << __func__ << ": inject EIO " << recovery_info.
                soid << dendl;
            r = -EIO;
        }
        if (r < 0) {
                                          ...
                                          PushOp * reply)
{
    ...
    if (pushing.count(soid) == 0) {
        ...
        if (!pi->recovery_progress.data_complete && !error) {
            ...
            if (r < 0) {
                dout(5) << __func__ << ": oid " << soid << " error " << r <<
                    dendl;
            ...
            if (pushing[soid].empty()) {
                ...
                if (error)
                    pushing[soid].begin()->second.recovery_progress.error =
                        true;
                dout(10) << "pushed " << soid <<
                    ", still waiting for push ack from " << pushing[soid].
                    size() << " others" << dendl;
            }
            return false;
        }
 ...
 */
void ReplicatedBackend::trim_pushed_data(const interval_set < uint64_t >
                                         &copy_subset,
                                         ...
                                         bufferlist * data_usable)
{
    ...
    uint64_t off = 0;
    for (interval_set < uint64_t >::const_iterator p =
         intervals_received.begin(); p != intervals_received.end(); ++p) {
...
void ReplicatedBackend::clear_pull_from(map < hobject_t,
                                        PullInfo >::iterator piter)
{
...
int ReplicatedBackend::start_pushes(const hobject_t & soid,
                                    ObjectContextRef obc, RPGHandle * h)
{
