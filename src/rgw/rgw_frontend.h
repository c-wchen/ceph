...
class RGWFrontendConfig {
  ...
  public:
    ...
    bool get_val(const std::string & key,
                 const std::string & def_val, std::string * out);
...
class RGWCivetWebFrontend:public RGWFrontend {
    ...
    void set_conf_default(std::multimap < std::string, std::string > &m,
     ...
     RGWCivetWebFrontend(RGWProcessEnv & env, RGWFrontendConfig * conf)
    :conf(conf), ctx(nullptr), env(env) {
    }
...
class RGWLoadGenFrontend:public RGWProcessFrontend {
  public:
    ...
    :RGWProcessFrontend(pe, _conf) {
        ...
        int num_threads;
        conf->get_val("num_threads", g_conf->rgw_thread_pool_size,
                      &num_threads);
        ...
        map < string, RGWAccessKey >::iterator aiter =
...
class RGWFrontendPauser:public RGWRealmReloader::Pauser {
  ...
  :    
    frontends(frontends),
    pauser(pauser), implicit_tenants(implicit_tenants) {
    } void pause() override {
    ...
    void resume(RGWRados * store) override {
        ...
        auto auth_registry =
            rgw::auth::StrategyRegistry::create(g_ceph_context,
                                                implicit_tenants, store);
