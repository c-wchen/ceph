...
starting_zone_num(first_seq_zone_num), num_zones(size / zone_size)
{
...
int64_t ZonedAllocator::allocate(uint64_t want_size,
                                 uint64_t alloc_unit,
                                 ...
                                 int64_t hint, PExtentVector * extents)
{
    ...
    ldout(cct, 10) << " moving zone 0x" << std::hex
        ...
        << " -> 0x" << offset + want_size << std::dec << dendl;
...
void ZonedAllocator::foreach(std::function <
                             void (uint64_t offset, uint64_t length) > notify)
{
    std::lock_guard l(lock);
}

void ZonedAllocator::init_from_zone_pointers(std::vector < zone_state_t >
                                             &&_zone_states)
{
    ...
    for (size_t i = first_seq_zone_num; i < num_zones; ++i) {
        num_sequential_free +=
            zone_size - (zone_states[i].write_pointer % zone_size);
    }
    ldout(cct, 10) << "free 0x" << std::hex << num_sequential_free
        << " / 0x" << sequential_size << std::dec << dendl;
}
...
int64_t ZonedAllocator::pick_zone_to_clean(float min_score, uint64_t min_saved)
{
    ...
    if (best_score >= min_score) {
        ldout(cct,
              10) << " zone 0x" << std::
            hex << best << " with score " << best_score << ": 0x" <<
            zone_states[best].
            num_dead_bytes << " dead and 0x" << zone_states[best].
            write_pointer -
            zone_states[best].
            num_dead_bytes << " live bytes" << std::dec << dendl;
    }
    else if (best > 0) {
        ldout(cct,
              10) << " zone 0x" << std::
            ...
            write_pointer -
            zone_states[best].
            num_dead_bytes << " live bytes" << std::
            dec << " but below min_score " << min_score << dendl;
        best = -1;
    }
    else {
        ldout(cct,
              10) << " no zones found that are good cleaning candidates" <<
            dendl;
    }
    return best;
}
...
bool ZonedAllocator::low_on_space(void)
{
    std::lock_guard l(lock);
    double free_ratio =
        static_cast < double >(num_sequential_free) / sequential_size;
