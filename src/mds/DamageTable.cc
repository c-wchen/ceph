...
public:
    ...
    DentryDamage(inodeno_t ino_,
                 frag_t frag_, std::string_view dname_, snapid_t snap_id_)
...
bool DamageTable::notify_dentry(inodeno_t ino, frag_t frag,
                                snapid_t snap_id, std::string_view dname,
                                std::string_view path)
{
    ...
    if ((MDS_INO_IS_MDSDIR(ino) && MDS_INO_MDSDIR_OWNER(ino) == rank)
        ...
        derr << "Damage to dentries in fragment " << frag << " of ino " << ino
             << "is fatal because it is a system directory for this rank" <<
             dendl;
        return true;
    }
    ...
    if (auto[it, inserted] =
            df_dentries.try_emplace(DentryIdent(dname, snap_id)); inserted) {
        auto entry =
            std::make_shared < DentryDamage > (ino, frag, dname, snap_id);
        entry->path = path;
        it->second = entry;
...
bool DamageTable::oversized() const const
{
    return by_id.size() > (size_t)(g_conf()->mds_damage_table_max_entries);
}

bool DamageTable::is_dentry_damaged(const CDir *dir_frag,
                                    std::string_view dname,
                                    const snapid_t snap_id) constconst {
...
bool DamageTable::is_dirfrag_damaged(const CDir *dir_frag) constconst {
    return dirfrags.
    count(DirFragIdent(dir_frag->inode->ino(), dir_frag->frag)) > 0;
}

bool DamageTable::is_remote_damaged(const inodeno_t ino) constconst {
    return remotes.count(ino) > 0;
}

void DamageTable::dump(Formatter *f) const const
{
...
void DamageTable::erase(damage_entry_id_t damage_id)
{
    ...
    if (type == DAMAGE_ENTRY_DIRFRAG) {
        ...
        auto backtrace_entry =
            std::static_pointer_cast < BacktraceDamage > (entry);
