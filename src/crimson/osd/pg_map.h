      ...
      public:
        ...
        /// Returns mapping for pgid, creates new one if it doesn't already exist
            ...
            if (!inserted) {
                ...
                if (core == NULL_CORE) {
                    core_iter = std::min_element(core_to_num_pgs.begin(),
                                                 core_to_num_pgs.end(),
                                                 [](const auto & left,
                                                    const auto & right) {
                                                 return left.second <
        ...
        /// Map to cores in [min_core_mapping, core_mapping_limit)
            PGShardMapping(core_id_t min_core_mapping,
 ...
 */
   ...
   */
        ...
        wait_for_pg_ret wait_for_pg(PGCreationBlockingEvent::TriggerI
                                    &&, spg_t pgid);
