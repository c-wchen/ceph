...
public:
    ...
    } virtual interruptible_future <> handle_recovery_op(Ref <
            MOSDFastDispatchOp > m,
            crimson::net::
            ConnectionRef conn);

    virtual interruptible_future <> recover_object(const hobject_t &soid,
            eversion_t need) = 0;
    virtual interruptible_future <> recover_delete(const hobject_t &soid,
            eversion_t need) = 0;
    virtual interruptible_future <> push_delete(const hobject_t &soid,
            eversion_t need) = 0;

    interruptible_future < BackfillInterval > scan_for_backfill(const hobject_t &from, std::int64_t min,
            std::int64_t max);
...
public:
    ...
    public:
        ...
        auto wait_track_blocking(T &trigger, F && fut)
        {
            ...
            return track_blocking(trigger, std::forward < F > (fut)
            ).finally([ref] {
            }
                                          );
        }
    ...
    using RecoveryBlockingEvent =
        crimson::AggregateBlockingEvent <
        WaitForObjectRecovery::BlockingEvent >;
    using WaitForObjectRecoveryRef =
        boost::intrusive_ptr < WaitForObjectRecovery >;
protected:
...
private:
    void handle_backfill_finish(MOSDPGBackfill &m,
    ...
    interruptible_future <> handle_scan_get_digest(MOSDPGScan &m,
            crimson::net::
            ConnectionRef conn);
    interruptible_future <> handle_scan_digest(MOSDPGScan &m);
    interruptible_future <> handle_scan(MOSDPGScan &m,
                                        crimson::net::ConnectionRef conn);
