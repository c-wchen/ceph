...
public:
    ...
    struct MutateParams {
        ...
        MutateParams()
        {
                       ...
                       RGWMDLogStatus _op_type): mtime(_mtime),
            op_type(_op_type)
        {
        }
    ...
    virtual int call_with_get_params(ceph::real_time *pmtime,
                                     std::function <
    ...
    virtual int list_init(const DoutPrefixProvider *dpp,
                          RGWSI_MetaBackend::Context *ctx,
                          const std::string &marker) = 0;
    virtual int list_next(const DoutPrefixProvider *dpp,
                          RGWSI_MetaBackend::Context *ctx, int max,
...
public:
    class Op
    {
        ...
        int mutate(const std::string &key,
                   ...
                   RGWObjVersionTracker *objv_tracker,
                   optional_yield y,
                   std::function < int () > f, const DoutPrefixProvider *dpp)
        {
        ...
        int list_next(const DoutPrefixProvider *dpp, int max,
                      std::list < std::string > *keys, bool *truncated)
        {
