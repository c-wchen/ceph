...
public:
    ...
    } void set_cost(uint64_t c)
    {
        ...
        : MOSDFastDispatchOp {
        MSG_OSD_PG_RECOVERY_DELETE, HEAD_VERSION, COMPAT_VERSION}
    {
    }
    ...
    MOSDPGRecoveryDelete(pg_shard_t from, spg_t pgid, epoch_t map_epoch,
                         epoch_t min_epoch)
        :    MOSDFastDispatchOp {
        MSG_OSD_PG_RECOVERY_DELETE, HEAD_VERSION,
        COMPAT_VERSION},
    from(from), pgid(pgid), map_epoch(map_epoch), min_epoch(min_epoch)
    {
    }
