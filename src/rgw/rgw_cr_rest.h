...
template < class T > class RGWReadRESTResourceCR:public RGWSimpleCoroutine {
    RGWRESTConn *conn;
  ...
  public:
    ...
    int send_request() override {
        auto op =
            ...
            (new
             RGWRESTReadResource(conn, path, params, &extra_headers,
                                 http_manager));
...
template < class S, class T, class E = int >
  ...
  public:
     RGWSendRESTResourceCR(CephContext * _cct, RGWRESTConn * _conn,
                           ...
                           rgw_http_param_pair * _params, map < string,
                           string > *_attrs, S & _input, T * _result,
                           E * _err_result = nullptr)
  :    RGWSimpleCoroutine(_cct), conn(_conn), http_manager(_http_manager),
        method(_method), path(_path), params(make_param_list(_params)),
        headers(make_param_list(_attrs)), result(_result),
    ...
    int send_request() override {
        auto op =
            ...
            (new
             RGWRESTSendResource(conn, method, path, params, &headers,
                                 http_manager));
  ...
  public:
    ...
    int send_request() override {
        auto op =
            ...
            (new
             RGWRESTDeleteResource(conn, path, params, nullptr, http_manager));
