...
namespace librbd {
    ...
    class ImageUpdateWatchers {
      public:
        ...
        explicit ImageUpdateWatchers(CephContext * cct):m_cct(cct),
            m_lock(ceph::
                   make_mutex(util::
                              unique_lock_name
                              ("librbd::ImageUpdateWatchers::m_lock", this))) {
        } ~ImageUpdateWatchers() {
        ...
        void register_watcher(UpdateWatchCtx * watcher, uint64_t * handle) {
            ldout(m_cct,
                  20) << "ImageUpdateWatchers::" << __func__ << ": watcher=" <<
                watcher << dendl;
        ...
        void unregister_watcher(uint64_t handle, Context * on_finish) {
            ldout(m_cct,
                  20) << "ImageUpdateWatchers::" << __func__ << ": handle=" <<
                handle << dendl;
            int r = 0;
            {
                std::lock_guard locker {
                ...
                if (it == m_watchers.end()) {
                    ...
                    if (m_in_flight.find(handle) != m_in_flight.end()) {
                        ceph_assert(m_pending_unregister.find(handle) ==
                                    m_pending_unregister.end());
      ...
      class ThreadPoolSingleton:public ThreadPool {
          ...
          :    
            ThreadPool(cct, "librbd::ImageUpdateWatchers::thread_pool",
                       "tp_librbd", 1) {
                start();
            }
        ...
        void create_work_queue() {
            ...
            m_work_queue =
                new ContextWQ("librbd::ImageUpdateWatchers::work_queue",
                              ceph::make_timespan(m_cct->_conf.get_val <
                                                  uint64_t >
    ...
    class QuiesceWatchers {
      public:
        ...
        void notify_quiesce(Context * on_finish) {
            ...
            if (m_blocked) {
                ldout(m_cct,
                      20) << "QuiesceWatchers::" << __func__ << ": queue" <<
                    dendl;
        ...
        void quiesce_complete(uint64_t handle, int r) {
            Context *on_notify = nullptr;
            {
                ...
                if (r < 0) {
                    ldout(m_cct,
                          10) << "QuiesceWatchers::" << __func__ << ": watcher "
                        << handle << " failed" << dendl;
      ...
      private:
        ...
        void send_notify(uint64_t handle, QuiesceWatchCtx * watcher,
            ...
            auto ctx = new LambdaContext([this, handle, watcher, event_type,
                                         ...
                                         switch (event_type) {
case QUIESCE:
watcher->handle_quiesce(); break; case UNQUIESCE:
                                         {
                                         std::lock_guard locker {
                                         m_lock};
                                         ...
                                         ldout(m_cct,
                                               20) << "QuiesceWatchers::" <<
                                         __func__ << ": skip for failed watcher"
                                         << dendl; break;}
                                         }
watcher->handle_unquiesce(); break; default:
                                         ceph_abort_msgf
    ...
    template < typename I > ImageState < I >::ImageState(I * image_ctx)
  :    
    m_image_ctx(image_ctx), m_state(STATE_UNINITIALIZED),
    m_lock(ceph::
           make_mutex(util::
                      unique_lock_name("librbd::ImageState::m_lock", this))),
    m_last_refresh(0), m_refresh_seq(0),
    m_update_watchers(new ImageUpdateWatchers(image_ctx->cct)),
    m_quiesce_watchers(new
                       QuiesceWatchers(image_ctx->cct,
    ...
    template < typename I > void ImageState < I >::handle_update_notification() {
        ...
        CephContext *cct = m_image_ctx->cct;
        ldout(cct,
              20) << __func__ << ": refresh_seq = " << m_refresh_seq << ", " <<
    ...
    template < typename I > bool ImageState < I >::is_transition_state()const {
        ...
        case STATE_CLOSED:
            return false;
            case STATE_OPENING:case STATE_CLOSING:case STATE_REFRESHING:case
                STATE_SETTING_SNAP:case STATE_PREPARING_LOCK:break;
        } return true;
    }
    ...
    template < typename I > bool ImageState < I >::is_closed()const {
        ...
        return ((m_state == STATE_CLOSED) ||
                (!m_actions_contexts.empty() &&
                 m_actions_contexts.back().first.action_type ==
                 ACTION_TYPE_CLOSE));
    } template < typename I > void ImageState <
        I >::append_context(const Action & action, Context * context) {
        ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    template < typename I >
        ...
        if (next_state == STATE_CLOSED ||
            ...
            auto ctx = new LambdaContext([image_ctx = m_image_ctx, contexts =
                                          std::move(action_contexts.second)]
                                         (int r){
              delete image_ctx; for (auto ctx:contexts)
                                         {
                                         ctx->complete(r);}
                                         }
            ) ;
            TaskFinisherSingleton::get_singleton(m_image_ctx->cct).queue(ctx,
                                                                         r);
        }
        else {
    ...
    template < typename I > void ImageState < I >::send_close_unlock() {
        ...
        image::CloseRequest < I > *req =
            image::CloseRequest < I >::create(m_image_ctx, ctx);
    ...
    template < typename I > void ImageState < I >::handle_close(int r) {
        ...
        if (r < 0) {
            lderr(cct) << "error occurred while closing image: " <<
                cpp_strerror(r)
                << dendl;
        }
    ...
    template < typename I > void ImageState < I >::send_refresh_unlock() {
        ...
        Context *ctx =
            create_async_context_callback(*m_image_ctx,
                                          create_context_callback < ImageState <
                                          I >,
                                          &ImageState < I >::handle_refresh >
                                          (this));
        image::RefreshRequest < I > *req =
            image::RefreshRequest < I >::create(*m_image_ctx, false, false,
                                                ctx);
    ...
    template < typename I > void ImageState < I >::send_set_snap_unlock() {
        ...
        Context *ctx =
            create_async_context_callback(*m_image_ctx,
                                          create_context_callback < ImageState <
                                          I >,
                                          &ImageState < I >::handle_set_snap >
                                          (this));
        image::SetSnapRequest < I > *req =
            image::SetSnapRequest < I >::create(*m_image_ctx,
                                                action_contexts.first.snap_id,
                                                ctx);
    ...
    template < typename I > void ImageState < I >::handle_set_snap(int r) {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(cct) << "failed to set snapshot: " << cpp_strerror(r) <<
                dendl;
        }
    ...
    template < typename I >
        int ImageState <
        I >::register_quiesce_watcher(QuiesceWatchCtx * watcher,
                                      uint64_t * handle) {
        CephContext *cct = m_image_ctx->cct;
        ldout(cct, 20) << __func__ << dendl;
    ...
    template < typename I >
        ...
        CephContext *cct = m_image_ctx->cct;
        ldout(cct,
              20) << __func__ << ": handle=" << handle << " r=" << r << dendl;
