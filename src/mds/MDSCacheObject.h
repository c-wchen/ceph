...
struct ClientLease {
    ...
    utime_t ttl;
     ...
     ClientLease(client_t c, MDSCacheObject * p):client(c), parent(p),
        item_session_lease(this), item_lease(this) {
    } ClientLease() = delete;
};
...
class MDSCacheObject {
  public:
    ...
    // -- state -- const static int STATE_AUTH = (1 << 30);
  ...
  // --------------------------------------------// replication (across mds cluster) protected:
  ...
  public:
    ...
    replica_map_type & get_replicas() {
        return replica_map;
    }
    const replica_map_type & get_replicas() const {
        return replica_map;
    ...
    // waiting
  private:
    alloc_ptr < mempool::mds_co::multimap < uint64_t, std::pair < uint64_t,
        MDSInternalContextBase * >>>waiting;
  ...
  public:
    ...
    virtual void add_waiter(uint64_t mask, MDSInternalContextBase * c) {
        ...
        waiting->insert(pair < uint64_t, pair < uint64_t,
                        MDSInternalContextBase * > >(mask, pair < uint64_t,
...
//                 << dendl;
    ...
    virtual void take_waiting(uint64_t mask,
                              std::list < MDSInternalContextBase * >&ls) {
        if (!waiting || waiting->empty())
            return;
...
//                 << dendl;
        ...
        for (auto it = ordered_waiters.begin(); it != ordered_waiters.end();
             ++it) {
    ...
    virtual void add_lock_waiter(int type, uint64_t mask,
