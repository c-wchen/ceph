...
class KStore:public ObjectStore {
  ...
  public:
    ...
    struct Onode {
        ...
        :cct(cct),
            nref(0),
            oid(o), key(k), dirty(false), exists(false), tail_offset(0) {
        } void flush();
        void get() {
            ++nref;
        } void put() {
    ...
    struct OnodeHashLRU {
        ...
        void clear();
        bool get_next(const ghobject_t & after, pair < ghobject_t,
                      OnodeRef > *next);
        int trim(int max = -1);
    };
    ...
    struct TransContext {
        ...
        :state(STATE_PREPARE),
            ...
            bytes(0),
            oncommit(NULL),
            onreadable(NULL), onreadable_sync(NULL), start(ceph_clock_now()) {
    ...
    class OpSequencer:public Sequencer_impl {
      public:
        ...
        :Sequencer_impl(cct), parent(NULL) {
  ...
  public:
    ...
    int stat(const coll_t & cid, const ghobject_t & oid, struct stat *st, bool allow_eio = false) override; // struct stat?
    ...
    int collection_list(const coll_t & cid, const ghobject_t & start,
                        const ghobject_t & end, int max,
