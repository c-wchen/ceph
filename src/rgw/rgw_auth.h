 ...
 * any operation that comes from an authenticated user. */
        class Identity {
          public:
            ...
            virtual uint32_t get_perms_from_aclspec(const aclspec_t & aclspec)
                const = 0;
 ...
 * policy (ACLs, account's ownership and entitlement). */
        class IdentityApplier:public Identity {
          public:
            ...
            virtual void modify_request_state(req_state * s) const {
        } /* in/out */ };
        ...
        class Strategy:public Engine {
          public:
            ...
            enum class Control {
             ...
             Engine::result_t authenticate(const req_state *
                                           s) const override final;

            bool is_empty() const {
                return auth_stack.empty();
            } static int apply(const Strategy & auth_strategy,
                               req_state * s) noexcept;
 ...
 * data will be used to fill RGWUserInfo structure. */
        class RemoteApplier:public IdentityApplier {
          ...
          protected:
            ...
            virtual void create_account(const rgw_user & acct_user, bool implicit_tenant, RGWUserInfo & user_info) const;   /* out */
          ...
          public:
            static const std::string NO_SUBUSER;
             ...
             LocalApplier(CephContext * const cct,
                          const RGWUserInfo & user_info, std::string subuser)
            :user_info(user_info), subuser(std::move(subuser)) {
            } uint32_t get_perms_from_aclspec(const aclspec_t & aclspec) const
                override;
            bool is_admin_of(const rgw_user & uid) const override;
            bool is_owner_of(const rgw_user & uid) const override;
