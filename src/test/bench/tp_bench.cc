...
class Base:public Queueable {
    ...
    Semaphore *sem;
  public:
    Base(DetailedStatCollector * col, Semaphore * sem):col(col), sem(sem) {
...
class WQWrapper:public Queueable {
  ...
  public:
    WQWrapper(ThreadPool::WorkQueue < unsigned >*wq, ThreadPool * tp):wq(wq),
        tp(tp) {
    } void queue(unsigned *item) override {
        wq->queue(item);
    }
...
class PassAlong:public ThreadPool::WorkQueue < unsigned > {
    ...
    bool _enqueue(unsigned *item) override {
        q.push_back(item);
        return true;
    } void _dequeue(unsigned *item) override {
        ceph_abort();
    }
...
int main(int argc, char **argv)
{
    ...
    desc.add_options()
        ...
        ("num-items", po::value < unsigned >()->default_value(3000000),
         "num items")
        ("layers", po::value < string > ()->default_value(""), "layer desc");
    ...
    try {
        po::parsed_options parsed =
            po::command_line_parser(argc,
                                    argv).options(desc).allow_unregistered().
            run();
        po::store(parsed, vm);
        po::notify(vm);
    ...
    auto cct = global_init(&def_args, ceph_options, CEPH_ENTITY_TYPE_CLIENT,
    ...
    QQ wqs;
    wqs.push_back(new Base(&col, &sem));
    string layers(vm["layers"].as < string > ());
    unsigned num = 0;
    for (string::reverse_iterator i = layers.rbegin(); i != layers.rend(); ++i) {
        stringstream ss;
        ss << "Test " << num;
        if (*i == 'q') {
            ThreadPool *tp =
                new ThreadPool(g_ceph_context, ss.str(), "tp_test",
                               vm["num-threads"].as < unsigned >(), 0);
            wqs.push_back(new WQWrapper(new PassAlong(tp, wqs.back()), tp));
        }
        else if (*i == 'f') {
            wqs.push_back(new FinisherWrapper(g_ceph_context, wqs.back()));
        }
        ++num;
    }

    for (QQ::iterator i = wqs.begin(); i != wqs.end(); ++i) {
        (*i)->start();
    }
