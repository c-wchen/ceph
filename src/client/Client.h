...
struct dir_result_t {
    ...
    struct dentry {
        ...
        } dentry(int64_t o, const string & n, const InodeRef & in):offset(o),
            name(n), inode(in) {
        }
    };
...
class Client:public Dispatcher, public md_config_obs_t {
  ...
  protected:
    ...
    void update_snap_trace(bufferlist & bl, SnapRealm ** realm_ret,
...
#define CHECK_CAPS_SYNCHRONOUS	(0x2)
   ...
   */
    ...
    Dentry *insert_dentry_inode(Dir * dir, const string & dname,
  ...
  private:
    ...
    int _do_lookup(Inode * dir, const string & name, int mask,
    ...
    int _mkdir(Inode * dir, const char *name, mode_t mode,
               const UserPerm & perm, InodeRef * inp = 0);
    ...
    int _setxattr(InodeRef & in, const char *name, const void *value,
                  size_t len, int flags, const UserPerm & perms);
    int _setxattr_check_data_pool(string & name, string & value,
                                  const OSDMap * osdmap);
...
#define VXATTR_RSTAT 0x1
    ...
    int _lookup_ino(inodeno_t ino, const UserPerm & perms, Inode ** inode =
                    NULL);
   ...
   */
    ...
    int getdir(const char *relpath, list < string > &names, const UserPerm & perms);    // get the whole dir at once.
   ...
   */
    ...
    int lookup_ino(inodeno_t ino, const UserPerm & perms, Inode ** inode =
                   NULL);
    int lookup_parent(Inode * in, const UserPerm & perms, Inode ** parent =
                      NULL);
