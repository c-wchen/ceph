...
{
} explicit cls_replica_log_set_marker_op(const cls_replica_log_progress_marker &
                                         m):marker(m) {
}
...
void dump(Formatter * f) const;
static void generate_test_instances(std::list <
                                    cls_replica_log_set_marker_op * >&ls);
};
...
void dump(Formatter * f) const;
static void generate_test_instances(std::list <
                                    cls_replica_log_get_bounds_op * >&ls);
};
     ...
     cls_replica_log_get_bounds_ret()
{
                                 ...
                                 &m):position_marker(pos_marker),
    oldest_time(time), markers(m) {
}
void encode(bufferlist & bl) const {
    ENCODE_START(1, 1, bl);
...
void dump(Formatter * f) const;
static void generate_test_instances(std::list <
                                    cls_replica_log_get_bounds_ret * >&ls);
};
