...
class RGWDataSyncModule {
  public:
    ...
    virtual RGWCoroutine *sync_object(RGWDataSyncEnv * sync_env,
                                      RGWBucketInfo & bucket_info,
                                      rgw_obj_key & key,
                                      boost::optional < uint64_t >
                                      versioned_epoch,
                                      rgw_zone_set * zones_trace) = 0;
    virtual RGWCoroutine *remove_object(RGWDataSyncEnv * sync_env,
                                        RGWBucketInfo & bucket_info,
                                        rgw_obj_key & key, real_time & mtime,
                                        bool versioned,
                                        uint64_t versioned_epoch,
                                        rgw_zone_set * zones_trace) = 0;
    virtual RGWCoroutine *create_delete_marker(RGWDataSyncEnv * sync_env,
                                               RGWBucketInfo & bucket_info,
                                               rgw_obj_key & key,
                                               real_time & mtime,
...
class RGWSyncModule {
  ...
  public:
    ...
    virtual int create_instance(CephContext * cct, map < string, string,
                                ltstr_nocase > &config,
...
class RGWSyncModulesManager {
  ...
  public:
    ...
    int create_instance(CephContext * cct, const string & name, map < string,
