...
I >::WriteAroundObjectDispatch(I *image_ctx, size_t max_dirty,
                               bool writethrough_until_flush)
    : m_image_ctx(image_ctx), m_init_max_dirty(max_dirty),
...
template < typename I >
void WriteAroundObjectDispatch <
I >::shut_down(Context *on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 5) << dendl;
...
template < typename I >
bool WriteAroundObjectDispatch < I >::read(uint64_t object_no,
        io::ReadExtents *
        extents,
        ...
        on_dispatched)
{
    ...
    for (auto &extent : *extents) {
        handled |=
            dispatch_unoptimized_io(object_no, extent.offset,
                                    extent.length, dispatch_result,
                                    on_dispatched);
    }
    return handled;
}
...
template < typename I >
bool WriteAroundObjectDispatch < I >::discard(uint64_t object_no,
        uint64_t object_off,
        uint64_t object_len,
        IOContext io_context,
        int discard_flags,
        ...
        on_dispatched)
{
    ...
    return dispatch_io(object_no, object_off, object_len, 0,
                       dispatch_result, on_finish, on_dispatched);
}

template < typename I >
bool WriteAroundObjectDispatch < I >::write(uint64_t object_no,
        uint64_t object_off,
        ceph::bufferlist
        && data,
        IOContext io_context,
...
template < typename I >
bool WriteAroundObjectDispatch < I >::write_same(uint64_t object_no,
        uint64_t
        object_off,
        uint64_t
        object_len,
...
template < typename I >
bool WriteAroundObjectDispatch <
...
{
    return dispatch_unoptimized_io(object_no, object_off,
                                   cmp_data.length(), dispatch_result,
                                   on_dispatched);
}

template < typename I >
bool WriteAroundObjectDispatch <
I >::flush(io::FlushSource flush_source,
           const ZTracer::Trace &parent_trace,
           uint64_t *journal_tid,
           io::DispatchResult *dispatch_result,
           Context **on_finish, Context *on_dispatched)
{
    ...
    if (flush_source == io::FLUSH_SOURCE_USER && !m_user_flushed) {
        ...
        if (m_max_dirty == 0 && m_init_max_dirty > 0) {
            ldout(cct,
                  5) << "first user flush: enabling write-around" <<
                     dendl;
    ...
    *on_finish = new LambdaContext([this, tid](int r) {
        handle_in_flight_flush_complete(r,
                                        tid);
    });
    ...
    m_queued_flushes.emplace(tid, QueuedFlush {
        ctx, on_dispatched}
                            );
    return true;
}
...
template < typename I >
bool WriteAroundObjectDispatch <
I >::dispatch_unoptimized_io(uint64_t object_no,
                             uint64_t object_off,
                             uint64_t object_len,
                             io::DispatchResult *dispatch_result,
                             Context *on_dispatched)
{
    ...
    if (in_flight_extents_it == m_in_flight_extents.end() ||
        !in_flight_extents_it->second.intersects(object_off,
                object_len)) {
    ...
    *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
    m_blocked_unoptimized_ios[object_no].emplace(tid, BlockedIO {
...
template < typename I >
bool WriteAroundObjectDispatch <
I >::dispatch_io(uint64_t object_no, uint64_t object_off,
                 ...
                 Context **on_finish, Context *on_dispatched)
{
    ...
    *on_finish = new LambdaContext([this, tid, object_no, object_off,
    object_len](int r) {
        handle_in_flight_io_complete(r, tid,
                                     object_no,
                                     object_off,
                                     object_len);
    });
    ...
    if (blocked) {
        ...
        m_blocked_ios[object_no].emplace(tid, BlockedIO {
            object_off, object_len, ctx,
            on_dispatched}
                                        );
        m_lock.unlock();
        ...
        ldout(cct, 20) << "queueing: tid=" << tid << dendl;
        m_queued_or_blocked_io_tids.insert(tid);
        m_queued_ios.emplace(tid, QueuedIO {
            object_len, ctx, on_dispatched}
                            );
        m_lock.unlock();
    }
    return true;
...
template < typename I >
bool WriteAroundObjectDispatch <
I >::block_overlapping_io(InFlightObjectExtents *
...
template < typename I >
void WriteAroundObjectDispatch <
I >::unblock_overlapping_ios(uint64_t object_no,
                             uint64_t object_off,
                             uint64_t object_len,
                             Contexts *unoptimized_io_dispatches)
{
    ...
    auto blocked_unoptimized_ios_it =
        m_blocked_unoptimized_ios.find(object_no);
    if (blocked_unoptimized_ios_it != m_blocked_unoptimized_ios.end()) {
        auto &blocked_unoptimized_object_ios =
            blocked_unoptimized_ios_it->second;
        for (auto it = blocked_unoptimized_object_ios.begin();
            ...
            if (!in_flight_object_extents.intersects(blocked_io.offset,
                    blocked_io.
                    length)) {
                unoptimized_io_dispatches->emplace(it->first,
                                                   blocked_io.
                                                   on_dispatched);
    ...
    if (blocked_io_it != m_blocked_ios.end()) {
        ...
        while (blocked_object_ios_it != blocked_object_ios.end()) {
            ...
            if (blocked_unoptimized_ios.intersects(blocked_io.offset,
                                                   blocked_io.length) ||
                block_overlapping_io(&in_flight_object_extents,
                                     blocked_io.offset,
                                     blocked_io.length)) {
                break;
            }
            ...
            ldout(cct,
                  20) << "queueing unblocked: tid=" << tid << dendl;
...
template < typename I >
bool WriteAroundObjectDispatch < I >::can_dispatch_io(uint64_t tid,
        uint64_t
        length)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));

    if (m_in_flight_bytes == 0
        || m_in_flight_bytes + length <= m_max_dirty) {
...
template < typename I >
void WriteAroundObjectDispatch <
I >::handle_in_flight_io_complete(int r, uint64_t tid,
                                  uint64_t object_no,
                                  uint64_t object_off,
                                  uint64_t object_len)
{
    ...
    for (auto &it : ready_flushes) {
        ldout(cct,
              20) << "dispatching flush: tid=" << it.first << dendl;
        it.second->complete(0);
    }
}
...
template < typename I >
void WriteAroundObjectDispatch <
I >::handle_in_flight_flush_complete(int r, uint64_t tid)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", tid=" << tid << dendl;
...
WriteAroundObjectDispatch < I >::collect_finished_flushes()
{
    ...
    while (true) {
        ...
        if (it == m_pending_flushes.end() ||
            (io_tid_it != m_in_flight_io_tids.end()
             && *io_tid_it < it->first)) {
            break;
        }
