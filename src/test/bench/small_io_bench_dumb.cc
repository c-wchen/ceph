...
int main(int argc, char **argv)
{
    ...
    if (!vm.count("filestore-path")) {
        cout << "Must provide filestore-path" << std::endl << desc << std::endl;
        return 1;
    }
    ...
    for (uint64_t num = 0; num < vm["num-objects"].as < unsigned >(); ++num) {
        ...
        if (num == col_num) {
            ...
            string coll_str(vm["filestore-path"].as < string >
                            () + string("/") + coll.str());
            int r =::mkdir(coll_str.c_str(),
                           0777);
            if (r < 0) {
                std::
                    cerr << "Error " << errno << " creating collection" << std::
                    endl;
                return 1;
            }
        }
    ...
    Bencher bencher(gen,
                    new DetailedStatCollector(1, new JSONFormatter,
                                              detailed_ops, &cout),
                    new DumbBackend(vm["filestore-path"].as < string > (),
                                    vm["fsync"].as < bool > (),
                                    vm["sync-file-range"].as < bool > (),
                                    vm["fadvise"].as < bool > (),
                                    vm["sync-interval"].as < unsigned >(),
                                    sync_fd, 10, g_ceph_context),
