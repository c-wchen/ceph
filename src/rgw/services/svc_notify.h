...
class RGWSI_Notify:public RGWServiceInstance {
  ...
  private:
    ...
    double inject_notify_timeout_probability {
    0};
    ...
    RGWSI_Notify_ShutdownCB *shutdown_cb {
    nullptr};
    ...
    void init(RGWSI_Zone * _zone_svc,
              RGWSI_RADOS * _rados_svc, RGWSI_Finisher * _finisher_svc) {
        zone_svc = _zone_svc;
  ...
  RGWSI_Notify(CephContext * cct):RGWServiceInstance(cct) {
    ...
    int distribute(const DoutPrefixProvider * dpp, const std::string & key,
                   const RGWCacheNotifyInfo & bl, optional_yield y);
