...
template <> struct TrashWatcher <MockTestImageCtx > {
    ...
    static void notify_image_added(librados::IoCtx &,
                                   const std::string &image_id,
...
TrashWatcher < MockTestImageCtx > *TrashWatcher <
...
ResetRequest < MockTestImageCtx > *ResetRequest <
MockTestImageCtx >::s_instance = nullptr;
...
GetInfoRequest < librbd::MockTestImageCtx > *GetInfoRequest <
...
ImageRemoveRequest < librbd::MockTestImageCtx > *ImageRemoveRequest <
librbd::MockTestImageCtx >::s_instance = nullptr;
...
template <> struct MoveRequest <MockTestImageCtx > {
    ...
    static MoveRequest *create(librados::IoCtx &io_ctx,
                               const std::string &image_id,
                               const cls::rbd::
                               TrashImageSpec &trash_image_spec,
                               Context *on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
MoveRequest < MockTestImageCtx > *MoveRequest <
MockTestImageCtx >::s_instance = nullptr;
...
{
public:
    typedef TrashMoveRequest < librbd::MockTestImageCtx >
    MockTrashMoveRequest;
    typedef librbd::journal::ResetRequest <
    librbd::MockTestImageCtx > MockJournalResetRequest;
    typedef librbd::mirror::GetInfoRequest <
    librbd::MockTestImageCtx > MockGetMirrorInfoRequest;
    typedef librbd::mirror::ImageRemoveRequest <
    librbd::MockTestImageCtx > MockImageRemoveRequest;
    typedef librbd::trash::MoveRequest < librbd::MockTestImageCtx >
    MockLibrbdTrashMoveRequest;
    typedef librbd::TrashWatcher < librbd::MockTestImageCtx >
    MockTrashWatcher;
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(0,
                  open_image(m_local_io_ctx, m_image_name,
                             &m_local_image_ctx));
    } void expect_mirror_image_get_image_id(const std::
                                            string &image_id,
                                            int r)
    {
        bufferlist bl;
    ...
    } void expect_get_mirror_info(MockGetMirrorInfoRequest &
                                  ...
                                  int r)
    {
        ...
        .
        WillOnce(Invoke
                 ([this, &mock_get_mirror_info_request,
                   mirror_image, promotion_state,
        primary_mirror_uuid, r]() {
            *mock_get_mirror_info_request.mirror_image =
                mirror_image;
            *mock_get_mirror_info_request.
            ...
            *mock_get_mirror_info_request.
            ...
            queue(mock_get_mirror_info_request.on_finish,
                  r);
        }));
    }

    void expect_set_journal_policy(librbd::
                                   MockTestImageCtx &
                                   mock_image_ctx)
    {
    ...
    void expect_open(librbd::MockTestImageCtx &mock_image_ctx,
                     int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, open(true, _))
        .WillOnce(WithArg < 1 >
                  (Invoke
        ([this, &mock_image_ctx, r](Context * ctx) {
            EXPECT_EQ(0U,
                      mock_image_ctx.
                      read_only_mask & librbd::
                      IMAGE_READ_ONLY_FLAG_NON_PRIMARY);
            m_threads->work_queue->queue(ctx, r);
        })));
    }

    void expect_close(librbd::MockTestImageCtx &mock_image_ctx,
                      int r)
    {
    ...
    void expect_block_requests(librbd::
    ...
    void expect_acquire_lock(librbd::
                             MockTestImageCtx &mock_image_ctx,
                             int r)
    {
                                 ...
                                 int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(m_local_io_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"),
                         StrEq("mirror_image_set"),
                         ContentsEqual(bl), _, _, _))
        .WillOnce(Return(r));
    }

    void expect_mirror_image_remove_request(MockImageRemoveRequest &
                                            mock_image_remove_request,
                                            int r)
    {
        EXPECT_CALL(mock_image_remove_request, send())
        .
        WillOnce(Invoke
        ([this, &mock_image_remove_request, r]() {
            m_threads->work_queue->
            queue(mock_image_remove_request.on_finish,
                  r);
        }));
    }

    void expect_journal_reset(MockJournalResetRequest &
                              mock_journal_reset_request, int r)
    {
        EXPECT_CALL(mock_journal_reset_request, send())
        .
        WillOnce(Invoke
        ([this, &mock_journal_reset_request, r]() {
            m_threads->work_queue->
            queue(mock_journal_reset_request.on_finish,
                  r);
        }));
    }

    void expect_trash_move(MockLibrbdTrashMoveRequest &
                           mock_trash_move_request,
                           ...
                           &delay, int r)
    {
        ...
        TrashImageSpec & spec) {
            ...
            if (delay) {
                ...
                time += *delay;
                ASSERT_TRUE(time ==
                            spec.deferment_end_time);
            } else {
                ASSERT_EQ(spec.deletion_time,
                          spec.deferment_end_time);
            }
        }
                   ))) ;
        ...
        .WillOnce(Invoke([this, &mock_trash_move_request, r]() {
            m_threads->work_queue->
            queue(mock_trash_move_request.
                  on_finish, r);
        }
                        ));
    }

    void expect_notify_image_added(MockTrashWatcher &
                                   mock_trash_watcher,
                                   const std::string &image_id)
    {
        EXPECT_CALL(mock_trash_watcher,
                    notify_image_added(image_id, _, _))
...
TEST_F(TestMockImageDeleterTrashMoveRequest, SuccessJournal)
{
    ...
    expect_trash_move(mock_librbd_trash_move_request, m_image_name,
    "image id", {
    }, 0);
    ...
    C_SaferCond ctx;
    auto req =
        MockTrashMoveRequest::create(m_local_io_ctx,
                                     "global image id",
                                     true,
                                     m_local_image_ctx->
                                     op_work_queue,
...
TEST_F(TestMockImageDeleterTrashMoveRequest,
       GetMirrorInfoLocalPrimary)
{
    InSequence seq;
    expect_mirror_image_get_image_id("image id", 0);
