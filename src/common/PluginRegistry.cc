...
} PluginRegistry::~PluginRegistry()
{
    ...
    for (std::map < std::string, std::map < std::string,
         Plugin * > >::iterator i = plugins.begin(); i != plugins.end();
         ++i) {
        for (std::map < std::string, Plugin * >::iterator j =
...
int PluginRegistry::remove(const std::string &type,
                           const std::string &name)
{
    ceph_assert(ceph_mutex_is_locked(lock));

    std::map < std::string, std::map < std::string,
        Plugin * > >::iterator i = plugins.find(type);
    if (i == plugins.end()) {
        return -ENOENT;
    }
...
int PluginRegistry::load(const std::string &type, const std::string &name)
{
    ...
    if (code_version == NULL) {
        lderr(cct) << __func__ << " code_version == NULL" << dlerror() <<
                   dendl;
        return -EXDEV;
    }
    ...
    int (*code_init)(CephContext *,
                     ...
                     const std::string & name) =
                         (int (*)(CephContext *,
                                  const std::string & type,
                                  const std::string & name))dlsym(library,
