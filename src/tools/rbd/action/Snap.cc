        ...
        namespace snap {
            ...
            int do_list_snaps(librbd::Image & image, Formatter * f) {
                ...
                if (f) {
                    ...
                    t.define_column("SNAPID", TextTable::RIGHT,
                                    TextTable::RIGHT);
                    ...
                    if (timestamp.tv_sec != 0) {
                        ...
                        t << s->id << s->
                            name << stringify(byte_u_t(s->size)) << tt_str <<
                            TextTable::endrow;
                    }
                }
            ...
            int do_purge_snaps(librbd::Image & image, bool no_progress) {
                ...
                if (r < 0) {
                    ...
                    for (size_t i = 0; i < snaps.size(); ++i) {
                        r = image.snap_is_protected(snaps[i].name.c_str(),
                                                    &is_protected);
                        if (r < 0) {
                            pc.fail();
                            return r;
                            ...
                            pc.fail();
                            std::cerr << "\r" << "rbd: snapshot '" << snaps[i].
                                name.c_str()
