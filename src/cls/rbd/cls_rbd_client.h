...
// low-level interface (mainly for testing)
        ...
        void get_features_start(librados::ObjectReadOperation * op,
                                bool read_only);
        int get_features_finish(ceph::buffer::list::const_iterator * it,
                                uint64_t * features,
                                uint64_t * incompatible_features);
        int get_features(librados::IoCtx * ioctx, const std::string & oid,
                         bool read_only, uint64_t * features,
                         uint64_t * incompatible_features);
        void set_features(librados::ObjectWriteOperation * op,
        ...
        int get_data_pool_finish(ceph::buffer::list::const_iterator * it,
                                 int64_t * data_pool_id);
        int get_data_pool(librados::IoCtx * ioctx, const std::string & oid,
                          int64_t * data_pool_id);

        void get_size_start(librados::ObjectReadOperation * op,
                            snapid_t snap_id);
        int get_size_finish(ceph::buffer::list::const_iterator * it,
        ...
        void get_flags_start(librados::ObjectReadOperation * op,
                             snapid_t snap_id);
        int get_flags_finish(ceph::buffer::list::const_iterator * it,
                             uint64_t * flags);
...
// NOTE: deprecate v1 parent APIs after mimic EOLed
        void get_parent_start(librados::ObjectReadOperation * op,
                              snapid_t snap_id);
...
// v2 parent APIs
        ...
        void snapshot_remove(librados::ObjectWriteOperation * op,
                             snapid_t snap_id);
        ...
        int get_snapcontext_finish(ceph::buffer::list::const_iterator *
                                   it,::SnapContext * snapc);
        int get_snapcontext(librados::IoCtx * ioctx,
                            const std::string & oid,::SnapContext * snapc);
...
/// NOTE: remove after Luminous is retired
        void get_snapshot_timestamp_start(librados::ObjectReadOperation * op,
                                          snapid_t snap_id);
        int get_snapshot_timestamp_finish(ceph::buffer::list::const_iterator *
                                          it, utime_t * timestamp);
        int get_snapshot_timestamp(librados::IoCtx * ioctx,
                                   const std::string & oid, snapid_t snap_id,
                                   utime_t * timestamp);
...
/// NOTE: remove protection after clone v1 is retired
        void get_protection_status_start(librados::ObjectReadOperation * op,
                                         snapid_t snap_id);
        int get_protection_status_finish(ceph::buffer::list::const_iterator *
                                         it, uint8_t * protection_status);
        int get_protection_status(librados::IoCtx * ioctx,
                                  const std::string & oid, snapid_t snap_id,
                                  uint8_t * protection_status);

        int set_protection_status(librados::IoCtx * ioctx,
                                  const std::string & oid, snapid_t snap_id,
                                  uint8_t protection_status);
        ...
        int snapshot_get_limit_finish(ceph::buffer::list::const_iterator * it,
                                      uint64_t * limit);
        ...
        int get_stripe_unit_count_finish(ceph::buffer::list::const_iterator *
                                         it, uint64_t * stripe_unit,
                                         uint64_t * stripe_count);
        int get_stripe_unit_count(librados::IoCtx * ioctx,
        ...
        int set_stripe_unit_count(librados::IoCtx * ioctx,
                                  const std::string & oid, uint64_t stripe_unit,
                                  uint64_t stripe_count);
        ...
        int get_create_timestamp_finish(ceph::buffer::list::const_iterator * it,
                                        utime_t * timestamp);
        int get_create_timestamp(librados::IoCtx * ioctx,
                                 const std::string & oid, utime_t * timestamp);
        ...
        int get_access_timestamp_finish(ceph::buffer::list::const_iterator * it,
                                        utime_t * timestamp);
        int get_access_timestamp(librados::IoCtx * ioctx,
                                 const std::string & oid, utime_t * timestamp);

        void set_access_timestamp(librados::ObjectWriteOperation * op);
        int set_access_timestamp(librados::IoCtx * ioctx,
                                 const std::string & oid);
        ...
        int get_modify_timestamp_finish(ceph::buffer::list::const_iterator * it,
                                        utime_t * timestamp);
        int get_modify_timestamp(librados::IoCtx * ioctx,
                                 const std::string & oid, utime_t * timestamp);

        void set_modify_timestamp(librados::ObjectWriteOperation * op);
        int set_modify_timestamp(librados::IoCtx * ioctx,
                                 const std::string & oid);
        ...
        void migration_set(librados::ObjectWriteOperation * op,
                           const cls::rbd::MigrationSpec & migration_spec);
        int migration_set_state(librados::IoCtx * ioctx,
                                const std::string & oid,
...
// operations on rbd_id objects
        ...
        int get_id_finish(ceph::buffer::list::const_iterator * it,
                          std::string * id);
        int get_id(librados::IoCtx * ioctx, const std::string & oid,
                   std::string * id);

        void set_id(librados::ObjectWriteOperation * op,
                    const std::string & id);
        int set_id(librados::IoCtx * ioctx, const std::string & oid,
                   const std::string & id);
...
// operations on rbd_directory objects
        ...
        int dir_get_id_finish(ceph::buffer::list::const_iterator * iter,
                              std::string * image_id);
        void dir_get_name_start(librados::ObjectReadOperation * op,
                                const std::string & id);
        int dir_get_name_finish(ceph::buffer::list::const_iterator * it,
                                std::string * name);
        ...
        void dir_list_start(librados::ObjectReadOperation * op,
                            const std::string & start, uint64_t max_return);
        int dir_list_finish(ceph::buffer::list::const_iterator * it,
...
// operations on the rbd_mirroring object
        ...
        int mirror_peer_set_direction(librados::IoCtx * ioctx,
                                      const std::string & uuid,
                                      cls::rbd::
                                      MirrorPeerDirection
        ...
        int mirror_image_get_image_id_finish(ceph::buffer::list::
                                             const_iterator * it,
                                             std::string * image_id);
        int mirror_image_get_image_id(librados::IoCtx * ioctx,
                                      const std::string & global_image_id,
                                      std::string * image_id);
        int mirror_image_get(librados::IoCtx * ioctx,
        ...
        int mirror_image_set(librados::IoCtx * ioctx,
                             const std::string & image_id,
                             const cls::rbd::MirrorImage & mirror_image);
        ...
        void mirror_image_status_get_start(librados::ObjectReadOperation * op,
                                           const std::string & global_image_id);
        int mirror_image_status_get_finish(ceph::buffer::list::const_iterator *
                                           iter,
        ...
        int mirror_image_status_list(librados::IoCtx * ioctx,
                                     ...
                                     uint64_t max_return,
                                     std::map < std::string,
                                     cls::rbd::MirrorImage > *images,
                                     std::map < std::string,
                                     cls::rbd::MirrorImageStatus > *statuses);
        void mirror_image_status_list_start(librados::ObjectReadOperation * op,
                                            const std::string & start,
                                            uint64_t max_return);
        int mirror_image_status_list_finish(ceph::buffer::list::const_iterator *
                                            iter, std::map < std::string,
                                            cls::rbd::MirrorImage > *images,
                                            std::map < std::string,
                                            cls::rbd::MirrorImageStatus >
                                            *statuses);
        int mirror_image_status_get_summary(librados::IoCtx * ioctx,
                                            ...
                                            int32_t > *states);
        void mirror_image_status_get_summary_start(librados::
                                                   ObjectReadOperation * op,
                                                   const std::vector <
                                                   cls::rbd::MirrorPeer >
                                                   &mirror_peer_sites);
        int mirror_image_status_get_summary_finish(ceph::buffer::list::
                                                   const_iterator * iter,
                                                   std::map <
                                                   cls::rbd::
        ...
        void mirror_image_status_remove_down(librados::ObjectWriteOperation *
                                             op);
        ...
        void mirror_image_instance_get_start(librados::ObjectReadOperation * op,
                                             const std::
                                             string & global_image_id);
        int mirror_image_instance_get_finish(ceph::buffer::list::
                                             const_iterator * iter,
                                             entity_inst_t * instance);
        int mirror_image_instance_list(librados::IoCtx * ioctx,
                                       ...
                                       uint64_t max_return,
                                       std::map < std::string,
                                       entity_inst_t > *instances);
        void mirror_image_instance_list_start(librados::ObjectReadOperation *
                                              op, const std::string & start,
                                              uint64_t max_return);
        int mirror_image_instance_list_finish(ceph::buffer::list::
                                              const_iterator * iter,
                                              std::map < std::string,
                                              entity_inst_t > *instances);

        void mirror_instances_list_start(librados::ObjectReadOperation * op);
        int mirror_instances_list_finish(ceph::buffer::list::const_iterator *
                                         iter,
...
// image mapping related routines
        ...
        int mirror_image_snapshot_unlink_peer(librados::IoCtx * ioctx,
                                              ...
                                              const std::
                                              string & mirror_peer_uuid);
        void mirror_image_snapshot_set_copy_progress(librados::
...
// Groups functions
        ...
        int group_image_list(librados::IoCtx * ioctx, const std::string & oid,
                             ...
                             uint64_t max_return,
                             std::vector < cls::rbd::GroupImageStatus >
                             *images);
        ...
        int group_snap_get_by_id(librados::IoCtx * ioctx,
                                 const std::string & oid,
                                 const std::string & snap_id,
...
// operations on rbd_trash object
        ...
        int trash_list_finish(ceph::buffer::list::const_iterator * it,
                              std::map < std::string,
                              cls::rbd::TrashImageSpec > *entries);
        int trash_list(librados::IoCtx * ioctx, const std::string & start,
                       uint64_t max_return, std::map < std::string,
                       cls::rbd::TrashImageSpec > *entries);
...
// operations on rbd_namespace object
        ...
        int namespace_list(librados::IoCtx * ioctx, const std::string & start,
                           uint64_t max_return,
                           std::list < std::string > *entries);
...
// operations on data objects
        ...
        void copyup(librados::ObjectWriteOperation * op,
                    ceph::buffer::list data);
        ...
        void sparse_copyup(neorados::WriteOp * op,
                           ...
                           uint64_t >> &extent_map, ceph::buffer::list data);
        ...
        void sparsify(librados::ObjectWriteOperation * op, uint64_t sparse_size,
                      bool remove_empty);
        int sparsify(librados::IoCtx * ioctx, const std::string & oid,
                     uint64_t sparse_size, bool remove_empty);

} // namespace cls_client }     // namespace librbd
