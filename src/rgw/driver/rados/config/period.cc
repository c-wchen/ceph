...
static std::string period_oid(std::string_view period_id, uint32_t epoch)
{
    ...
    return fmt::format("{}{}.{}", period_info_oid_prefix, period_id,
                       epoch);
}
...
static int read_latest_epoch(const DoutPrefixProvider *dpp,
                             optional_yield y, ConfigImpl *impl,
                             std::string_view period_id, uint32_t &epoch,
                             RGWObjVersionTracker *objv)
{
    const auto &pool = impl->period_pool;
    const auto latest_oid =
        latest_epoch_oid(dpp->get_cct()->_conf, period_id);
...
static int write_latest_epoch(const DoutPrefixProvider *dpp,
                              optional_yield y, ConfigImpl *impl,
                              bool exclusive, std::string_view period_id,
...
static int delete_latest_epoch(const DoutPrefixProvider *dpp,
                               optional_yield y, ConfigImpl *impl,
                               std::string_view period_id,
                               RGWObjVersionTracker *objv)
{
...
static int update_latest_epoch(const DoutPrefixProvider *dpp,
                               optional_yield y, ConfigImpl *impl,
                               std::string_view period_id, uint32_t epoch)
{
    ...
    for (int i = 0; i < MAX_RETRIES; i++) {
        ...
        // read existing epoch
        int r =
            read_latest_epoch(dpp, y, impl, period_id, existing_epoch,
                              &objv);
        if (r == -ENOENT) {
            ...
            ldpp_dout(dpp,
                      10) << "found existing latest_epoch " <<
                          existing_epoch << " >= given epoch " << epoch <<
                          ", returning r=" << r << dendl;
            return r;
        } else {
            ldpp_dout(dpp,
        ...
        r = write_latest_epoch(dpp, y, impl, exclusive, period_id, epoch,
                               &objv);
        if (r == -EEXIST) {
                                    ...
                                    const RGWPeriod &info)
{
    ...
    (void)update_latest_epoch(dpp, y, impl.get(), info.get_id(),
                              info.get_epoch());
    return 0;
}
                                  ...
                                  RGWPeriod &info)
{
    ...
    if (!epoch) {
        epoch = 0;
        r = read_latest_epoch(dpp, y, impl.get(), period_id, *epoch,
                              nullptr);
        if (r < 0) {
            return r;
        }
