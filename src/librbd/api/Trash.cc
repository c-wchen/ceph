...
namespace librbd {
    namespace api {
        ...
        template < typename I >
            const typename Trash < I >::TrashImageSources Trash <
            I >::ALLOWED_RESTORE_SOURCES {
        cls::rbd::TRASH_IMAGE_SOURCE_USER,
         ...
         namespace {
            ...
            template < typename I > int disable_mirroring(I * ictx) {
                ...
                C_SaferCond ctx;
                auto req =
                    mirror::DisableRequest < I >::create(ictx, false, true,
                                                         &ctx);
                 req->send();
                int r = ctx.wait();
                if (r < 0) {
                    lderr(ictx->
                          cct) << "failed to disable mirroring: " <<
                        cpp_strerror(r)
                        << dendl;
                    return r;
                } return 0;
            } template < typename I >
                int enable_mirroring(IoCtx & io_ctx,
                ...
                int r =
                    cls_client::get_features(&io_ctx,
                                             util::header_name(image_id), true,
                                             &features, &incompatible_features);
                if (r < 0) {
                    lderr(cct) << "failed to retrieve features: " <<
                ...
                C_SaferCond ctx;
                auto req =
                    mirror::EnableRequest < I >::create(io_ctx, image_id,
                                                        cls::rbd::
                ...
                r = ctx.wait();
                if (r < 0) {
                    lderr(cct) << "failed to enable mirroring: " <<
                        cpp_strerror(r)
                        << dendl;
                    return r;
                }
            ...
            int list_trash_image_specs(librados::IoCtx & io_ctx,
                                       std::map < std::string,
                ...
                do {
                    std::map < std::string,
                        cls::rbd::TrashImageSpec > trash_entries;
        ...
        template < typename I >
            int Trash < I >::move(librados::IoCtx & io_ctx,
            ...
            if (r == 0) {
                ...
                int mirror_r =
                    cls_client::mirror_image_get(&ictx->md_ctx, ictx->id,
                                                 &mirror_image);
                if (mirror_r == -ENOENT) {
                    ldout(ictx->cct,
                ...
                if (!ictx->migration_info.empty()) {
                    lderr(cct) << "cannot move migrating image to trash" <<
                        dendl;
            ...
            C_SaferCond ctx;
            auto req =
                trash::MoveRequest < I >::create(io_ctx, image_id,
                                                 trash_image_spec,
                                                 &ctx);
            req->send();
            ...
            TrashWatcher < I >::notify_image_added(io_ctx, image_id,
                                                   trash_image_spec,
                                                   &notify_ctx);
            r = notify_ctx.wait();
            if (r < 0) {
        ...
        template < typename I >
            ...
            int r = cls_client::dir_get_id(&io_ctx, RBD_DIRECTORY, image_name,
                                           &image_id);
            if (r == -ENOENT) {
                r = io_ctx.stat(util::old_header_name(image_name), nullptr,
                                nullptr);
                if (r == 0) {
                ...
                std::map < std::string,
                    cls::rbd::TrashImageSpec > trash_image_specs;
                int r =
                    list_trash_image_specs(io_ctx, &trash_image_specs, true);
                if (r < 0) {
                    return r;
                }
                if (auto found_image =
                    std::find_if(trash_image_specs.begin(),
                ...
                ldout(cct,
                    ...
                    " from existing " << "trash entry" << dendl;
            }
            else if (r < 0) {
            ...
            return Trash < I >::move(io_ctx, source, image_name, image_id,
                                     delay);
        }
        ...
        template < typename I >
            ...
            if (r == -ENOENT) {
                ...
                lderr(cct) << "error retrieving trash entry: " <<
                    cpp_strerror(r)
                    << dendl;
                return r;
            }
        ...
        template < typename I >
            ...
            trash_entries.
                erase(std::
                      remove_if(trash_entries.begin(), trash_entries.end(),
            ...
            if (threshold != -1) {
                ...
                rados.
                    mon_command(R "({" prefix ": " df ", " format ": " json
                                "})", inbl, &outbl, nullptr);
              ...
              for (const auto & entry:trash_entries) {
                    ...
                    r = cls_client::get_data_pool(&io_ctx,
                                                  util::header_name(entry.id),
                                                  &data_pool_id);
                    if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
                        lderr(cct) << "failed to query data pool: " <<
                            cpp_strerror(r) << dendl;
                        return r;
                    }
                ...
                for (uint8_t i = 0; i < arr.size(); ++i) {
                    ...
                    if (img != datapools.end()) {
                        json_spirit::mObject stats =
                            arr[i].get_obj()["stats"].get_obj();
                      ...
                      for (const auto & it:img->second) {
                            ...
                            r = librbd::api::DiffIterate <
                                I >::diff_iterate(ictx,
                                                  cls::rbd::
                                                  UserSnapshotNamespace(),
                                                  nullptr, 0, ictx->size, false,
                                                  true,[](uint64_t offset,
                                                          size_t len,
                                                          int exists,
                                                          void *arg) {
                                                  auto * to_free =
                                                  reinterpret_cast <
                                                  uint64_t * >(arg); if (exists)
                                                  (*to_free) += len; return 0;}
                                                  , &bytes_to_free) ;

                            ictx->state->close();
                            if (r < 0) {
                                lderr(cct) <<
                                    "failed to calculate disk usage for image "
                                    << it << ": " << cpp_strerror(r) << dendl;
                                continue;
                            }
          ...
          for (const auto & entry:trash_entries) {
            ...
            while (!to_be_removed.empty() && remove_err == 1) {
                remove_err = 0;
                for (auto it = to_be_removed.begin();
                     it != to_be_removed.end();) {
        ...
        template < typename I >
            int Trash < I >::remove(IoCtx & io_ctx,
            ...
            C_SaferCond cond;
            auto req =
                librbd::trash::RemoveRequest < I >::create(io_ctx, image_id,
                                                           asio_engine.
                                                           get_work_queue(),
                                                           force, prog_ctx,
                                                           &cond);
            ...
            C_SaferCond notify_ctx;
            TrashWatcher < I >::notify_image_removed(io_ctx, image_id,
                                                     &notify_ctx);
            r = notify_ctx.wait();
            if (r < 0) {
                lderr(cct) << "failed to send update notification: " <<
        ...
        template < typename I >
            int Trash < I >::restore(librados::IoCtx & io_ctx,
                                     const TrashImageSources &
                                     trash_image_sources,
                                     const std::string & image_id,
                                     const std::string & image_new_name) {
            CephContext *cct((CephContext *) io_ctx.cct());
            ldout(cct,
                  20) << "trash_restore " << &io_ctx << " " << image_id << " "
            ...
            if (trash_image_sources.count(trash_spec.source) == 0) {
                lderr(cct) << "Current trash source '" << trash_spec.
            ...
            if (image_name.empty()) {
                ...
                ldout(cct,
                      20) << "restoring image id " << image_id << " with name "
                    << image_name << dendl;
            }
            ...
            if (create_id_obj) {
                ...
                if (r < 0) {
                    lderr(cct) << "error adding id object for image " <<
