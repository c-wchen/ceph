...
public:
    ...
    virtual int create_bucket(const DoutPrefixProvider *dpp,
                              ...
                              sal::Attrs &attrs, RGWBucketInfo &info,
...
TEST(TestRGWLua, EmptyScript)
{
    ...
    const auto rc =
        lua::request::execute(nullptr, nullptr, nullptr, &s, nullptr, script);
    ASSERT_EQ(rc, 0);
}
...
TEST(TestRGWLua, GenericAttributes)
{
    const std::string script = R "(
                               ...
                               for k, v in pairs(Request.GenericAttributes)
                                   do
                                       assert(k)
                                       assert(v)
                                       end
...
TEST(TestRGWLua, Environment)
{
    const std::string script = R "(
                               ...
                               for k, v in pairs(Request.Environment)
                                   do
                                       assert(k)
                                       assert(v)
                                       end
...
TEST(TestRGWLua, Metadata)
{
    const std::string script = R "(
                               ...
                               for k, v in pairs(Request.HTTP.Metadata)
                                   do
                                       assert(k)
                                       assert(v)
                                       end
...
TEST(TestRGWLua, Acl)
{
    const std::string script = R "(
                               ...
                               if (g.User)
                                   ...
                                   for k, v in pairs(Request.UserAcl.Grants)
                                       do
                                           print_grant(v)
                                           if k == " john$doe " then
...
TEST(TestRGWLua, WithLib)
{
    ...
    expected_result = {" my ", " bucket ", " name ", " is ", " fish "}
                               i = 1
                                   for p in string.gmatch(Request.Bucket.Name, " % a + ")
                                       do
...
TEST(TestRGWLua, OpsLog)
{
    ...
    s.auth.identity =
        std::unique_ptr < rgw::auth::Identity > (new FakeIdentity());

    auto rc =
        lua::request::execute(store.get(), nullptr, &olog, &s, nullptr, script);
    EXPECT_EQ(rc, 0);
    EXPECT_FALSE(olog.logged);  // don't log http_ret=200

    s.err.http_ret = 400;
    rc = lua::request::execute(store.get(), nullptr, &olog, &s, nullptr,
                               script);
    EXPECT_EQ(rc, 0);
...
public:
    TestBackground(sal::RadosStore *store, const std::string &script, unsigned read_time = 0):
        rgw::lua::Background(store, g_cct, "", /* luarocks path */
                             1 /* run every second */),
        read_time(read_time)
    {
...
template < typename T >
const T &get_table_value(const TestBackground &b,
                         const std::string &index)
{
    try {
        return std::get < T > (b.get_table_value(index));
    } catch (std::bad_variant_access const &ex) {
        std::cout << "expected RGW[" << index << "] to be: " << typeid(T).
                  name() << std::endl;
        throw (ex);
    }
}
...
TEST(TestRGWLuaBackground, RequestScript)
{
    ...
    const auto rc =
        lua::request::execute(nullptr, nullptr, nullptr, &s, nullptr,
                              request_script);
    ASSERT_EQ(rc, 0);
    EXPECT_EQ(get_table_value < std::string > (lua_background, "hello"),
              "from request");
...
TEST(TestRGWLuaBackground, PauseWhileReading)
{
    ...
    local value = " world "
            ...
            EXPECT_TRUE(value_len + 1 >= get_table_value < std::string >
                        (lua_background, "hello").size());
        }
...
TEST(TestRGWLuaBackground, PauseResume)
{
    ...
    local value = " 1 "
            ...
            EXPECT_GT(get_table_value < std::string > (lua_background, "hello").size(),
                      value_len);
        }
...
TEST(TestRGWLuaBackground, TablePersist)
{
    ...
    rc = lua::request::execute(nullptr, nullptr, nullptr, &s, nullptr,
                               request_script);
    ASSERT_EQ(rc, 0);
    EXPECT_EQ(get_table_value < std::string > (lua_background, "key1"),
              "string value");
    EXPECT_EQ(get_table_value < long long int >(lua_background, "key2"), 42);
    EXPECT_EQ(get_table_value < std::string > (lua_background, "key3"),
              "string value");
