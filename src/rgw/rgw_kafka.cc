...
bool new_producer(connection_t* conn)
{
    ...
    if (rd_kafka_conf_set(conf.get(), "message.timeout.ms",
                          std::to_string(message_timeout).c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        goto conf_error;
    }

    // get list of brokers based on the bootstrap broker
    if (rd_kafka_conf_set(conf.get(), "bootstrap.servers", conn->broker.c_str(), errstr,
    ...
    if (conn->use_ssl) {
        if (!conn->user.empty()) {
            ...
            if (rd_kafka_conf_set(conf.get(), "security.protocol", "SASL_SSL", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK ||
                ...
                rd_kafka_conf_set(conf.get(), "sasl.password", conn->password.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                goto conf_error;
            }
            ldout(conn->cct, 20) << "Kafka connect: successfully configured SSL+SASL security" << dendl;

            if (conn->mechanism) {
                if (rd_kafka_conf_set(conf.get(), "sasl.mechanism", conn->mechanism->c_str(), errstr,
                                      sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                    goto conf_error;
                }
                ldout(conn->cct, 20) << "Kafka connect: successfully configured SASL mechanism" << dendl;
            } else {
                if (rd_kafka_conf_set(conf.get(), "sasl.mechanism", "PLAIN", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                    goto conf_error;
                }
            ...
            // use only SSL
            if (rd_kafka_conf_set(conf.get(), "security.protocol", "SSL", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                goto conf_error;
            }
            ldout(conn->cct, 20) << "Kafka connect: successfully configured SSL security" << dendl;
        }
        if (conn->ca_location) {
            if (rd_kafka_conf_set(conf.get(), "ssl.ca.location", conn->ca_location->c_str(), errstr,
                                  sizeof(errstr)) != RD_KAFKA_CONF_OK) {
                goto conf_error;
            }
...
private:
    ...
    void publish_internal(message_wrapper_t* message)
    {
        ...
        if (conn->status != 0) {
            ...
            ldout(conn->cct, 1) << "Kafka publish: producer was closed while message was in the queue. with status: " <<
...
public:
                 ...
                 boost::optional < const std::string & > mechanism)
    {
        ...
        auto conn = std::make_unique < connection_t > (cct, broker, use_ssl, verify_ssl, ca_location, user, password,
                    mechanism);
