...
#include "rgw_iam_policy_keywords.frag.cc"
      ...
      :    
        ...
        service(Service::s3),
        region(), account(o.bucket.tenant), resource(o.bucket.name) {
            resource.push_back('/');
        ...
        :partition(Partition::aws),
            service(Service::s3),
            region(), account(b.tenant), resource(b.name) {
        }
        ...
        optional < ARN > ARN::parse(const string & s, bool wildcards) {
            static const char str_wild[] =
                "arn:([^:]*):([^:]*):([^:]*):([^:]*):([^:]*)";
            static const regex rx_wild(str_wild, sizeof(str_wild) - 1,
                                       ECMAScript | optimize);
...
// do that sanely would be to use unification rather than matching.
        bool ARN::match(const ARN & candidate)const {
            ...
            if (!match_policy
                (resource, candidate.resource, MATCH_POLICY_RESOURCE)) {
                return false;
            }
...
//
        ...
        bool ParseState::key(const char *s, size_t l) {
            ...
            if (w->id == TokenID::Condition && w->kind == TokenKind::statement) {
                ...
                if (boost::algorithm::ends_with(boost::string_view {
                                                s, l}
                                                , IfExists)) {
                    ifexists = true;
...
// which will make all of this ever so much nicer.
        static optional < Principal > parse_principal(CephContext * cct,
                                                      TokenID t, string && s) {
            // Wildcard!
            if ((t == TokenID::AWS) && (s == "*")) {
                ...
                if (auto a = ARN::parse(s)) {
                    ...
                    if (std::none_of(s.begin(), s.end(),[](const char &c) {
                                     return (c == ':') || (c == '/');}
                        )) {
            ...
            if ((w->id == TokenID::Version) && k &&
              ...
              for (auto & p:actpairs) {
                    if (match_policy( {
                        ...
                        (w->id == TokenID::Action ? t->action : t->notaction) |=
                            p.bit;
                    }
                }
            }
