...
public:
    ...
    int register_to_service_map(const DoutPrefixProvider *dpp, const std::string& daemon_type, const std::map < std::string,
                                std::string > & meta);
        ...
        struct Delete {
            ...
            struct DeleteParams {
                ...
                DeleteParams() : versioning_status(0), olh_epoch(0), bilog_flags(0), remove_objs(NULL), high_precision_time(false),
                    zones_trace(nullptr), abortmp(false), parts_accounted_size(0), check_objv(nullptr) {}
            } params;
...
public:
    ...
    int put_bucket_instance_info(RGWBucketInfo& info, bool exclusive, ceph::real_time mtime, const std::map < std::string,
                                 bufferlist > *pattrs, const DoutPrefixProvider *dpp, optional_yield y);
