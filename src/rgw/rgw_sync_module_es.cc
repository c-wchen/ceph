...
struct ElasticConfig {
    ...
    void init(CephContext * cct, const map < string, string,
              ltstr_nocase > &config) {
        string elastic_endpoint = rgw_conf_get(config, "endpoint", "");
        id = string("elastic:") + elastic_endpoint;
        conn.reset(new RGWRESTConn(cct, nullptr, id, {
                                   elastic_endpoint}));
        explicit_custom_meta =
            rgw_conf_get_bool(config, "explicit_custom_meta", true);
        ...
        num_shards =
            rgw_conf_get_int(config, "num_shards", ES_NUM_SHARDS_DEFAULT);
        if (num_shards < ES_NUM_SHARDS_MIN) {
            num_shards = ES_NUM_SHARDS_MIN;
        }
        num_replicas =
            rgw_conf_get_int(config, "num_replicas", ES_NUM_REPLICAS_DEFAULT);
      ...
      for (auto i:custom_meta) {
        ...
        if (!custom_date.empty()) {
          ...
          for (auto i:custom_date) {
                ...
                if (r < 0) {
                    ldout(cct,
                          20) << __func__ << "(): failed to parse time (" << i.
                        second <<
...
class RGWElasticInitConfigCBCR:public RGWCoroutine {
  ...
  public:
  RGWElasticInitConfigCBCR(RGWDataSyncEnv * _sync_env, ElasticConfigRef _conf):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env),
        conf(_conf) {
    }
        ...
        reenter(this) {
            ...
            yield {
                ...
                call(new RGWPutRESTResourceCR < es_index_config_base, int,
                     _err_response > (sync_env->cct, conf->conn.get(),
...
class RGWElasticHandleRemoteObjCBCR:public RGWStatRemoteObjCBCR {
  ...
  public:
    RGWElasticHandleRemoteObjCBCR(RGWDataSyncEnv * _sync_env,
                                  ...
                                  uint64_t
                                  _versioned_epoch):RGWStatRemoteObjCBCR
        (_sync_env, _bucket_info, _key), conf(_conf),
        versioned_epoch(_versioned_epoch) {
    } int operate() override {
        reenter(this) {
            ldout(sync_env->cct,
                  10) << ": stat of remote obj: z=" << sync_env->
                source_zone << " b=" << bucket_info.
                bucket << " k=" << key << " size=" << size << " mtime=" << mtime
                << dendl;

            yield {
                string path = conf->get_obj_path(bucket_info, key);
                es_obj_metadata doc(sync_env->cct, conf, bucket_info, key,
                                    mtime, size, attrs, versioned_epoch);
...
class RGWElasticHandleRemoteObjCR:public RGWCallStatRemoteObjCR {
  ...
  public:
    RGWElasticHandleRemoteObjCR(RGWDataSyncEnv * _sync_env,
                                ...
                                rgw_obj_key & _key, ElasticConfigRef _conf,
                                uint64_t
                                _versioned_epoch):RGWCallStatRemoteObjCR
        (_sync_env, _bucket_info, _key), conf(_conf),
        versioned_epoch(_versioned_epoch) {
    ...
    RGWStatRemoteObjCBCR *allocate_callback() override {
        return new RGWElasticHandleRemoteObjCBCR(sync_env, bucket_info, key,
                                                 conf, versioned_epoch);
    }
};
...
class RGWElasticRemoveRemoteObjCBCR:public RGWCoroutine {
  ...
  public:
    RGWElasticRemoveRemoteObjCBCR(RGWDataSyncEnv * _sync_env,
                                  ...
                                  ElasticConfigRef
                                  _conf):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env), bucket_info(_bucket_info), key(_key),
...
class RGWElasticDataSyncModule:public RGWDataSyncModule {
    ElasticConfigRef conf;
  public:
    RGWElasticDataSyncModule(CephContext * cct, const map < string, string,
                             ltstr_nocase > &config):conf(std::make_shared <
                                                          ElasticConfig > ()) {
    ...
    RGWCoroutine *sync_object(RGWDataSyncEnv * sync_env,
        ...
        ldout(sync_env->cct,
              10) << conf->id << ": sync_object: b=" << bucket_info.
    ...
    RGWCoroutine *remove_object(RGWDataSyncEnv * sync_env,
        ...
        ldout(sync_env->cct,
              10) << conf->id << ": rm_object: b=" << bucket_info.
        ...
        return new RGWElasticRemoveRemoteObjCBCR(sync_env, bucket_info, key,
                                                 mtime, conf);
    }
    RGWCoroutine *create_delete_marker(RGWDataSyncEnv * sync_env,
                                       ...
                                       rgw_zone_set * zones_trace) override {
        ldout(sync_env->cct,
              10) << conf->id << ": create_delete_marker: b=" << bucket_info.
            bucket << " k=" << key << " mtime=" << mtime << " versioned=" <<
            versioned << " versioned_epoch=" << versioned_epoch << dendl;
        ldout(sync_env->cct,
              10) << conf->id << ": skipping operation (not handled)" << dendl;
        return NULL;
    }
...
RGWElasticSyncModuleInstance::RGWElasticSyncModuleInstance(CephContext * cct,
                                                           const map < string,
                                                           string,
                                                           ltstr_nocase >
                                                           &config)
{
    data_handler =
        ...
        (new RGWElasticDataSyncModule(cct, config));
}
...
int RGWElasticSyncModule::create_instance(CephContext * cct, map < string,
