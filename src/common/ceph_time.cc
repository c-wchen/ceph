                                  ...
                                  struct ceph_timespec &ts)
{
    ...
    ts.tv_nsec = (t.time_since_epoch() % 1 s).count();
...
real_clock::time_point real_clock::
from_ceph_timespec(const struct ceph_timespec &ts)
{
...
struct ceph_timespec coarse_real_clock::
to_ceph_timespec(const time_point &t)
{
    struct ceph_timespec ts;
    to_ceph_timespec(t, ts);
    return ts;
}
coarse_real_clock::time_point coarse_real_clock::
from_ceph_timespec(const struct ceph_timespec &ts)
{
    return time_point(seconds(ts.tv_sec) + nanoseconds(ts.tv_nsec));
}
...
template < typename Clock,
           typename std::enable_if <
               Clock::is_steady >::type * >std::ostream &operator<<(std::ostream &m,
                       const std::
                       chrono::
                       time_point <
                       Clock > &t)
{
    return m << std::fixed << std::chrono::duration <
...
template std::ostream &operator<< < coarse_mono_clock > (std::ostream &m,
                                const coarse_mono_time &t);
template std::ostream &operator<< < coarse_real_clock > (std::ostream &m,
                                const
                                coarse_real_time &
                                t);
...
std::chrono::seconds parse_timespan(const std::string &s)
{
    ...
    while (pos < s.size()) {
        ...
        if (unit_start != pos) {
            ...
            throw std::invalid_argument("unexpected trailing '" s +
                                        s.substr(pos) + "'");
        }
...
{
    if constexpr
    (chrono::treat_as_floating_point_v < Rep > || Period::den > 1) {
...
template ostream &operator<< <::ceph::timespan::rep, ::ceph::timespan::period > (ostream &,
                           const::
                           ceph::
                           timespan
                           &);
...
operator<< < chrono::milliseconds::rep,
             chrono::milliseconds::period > (ostream &,
