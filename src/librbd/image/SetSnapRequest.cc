...
namespace librbd {
    namespace image {
        ...
        using util::create_context_callback;
         ...
         template < typename I >
            SetSnapRequest < I >::SetSnapRequest(I & image_ctx,
                                                 const cls::rbd::
                                                 SnapshotNamespace &
                                                 snap_namespace,
        ...
        :m_image_ctx(image_ctx), m_snap_namespace(snap_namespace),
            ...
            m_snap_id(CEPH_NOSNAP), m_exclusive_lock(nullptr),
            m_object_map(nullptr), m_refresh_parent(nullptr),
        ...
        template < typename I >
            Context * SetSnapRequest <
            I >::handle_init_exclusive_lock(int *result) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
...

            {
                RWLock::RLocker snap_locker(m_image_ctx.snap_lock);
                m_snap_id =
                    m_image_ctx.get_snap_id(m_snap_namespace, m_snap_name);
        ...
        template < typename I >
            Context * SetSnapRequest <
            I >::send_shut_down_exclusive_lock(int *result) {
            {
                RWLock::RLocker snap_locker(m_image_ctx.snap_lock);
        ...
        template < typename I >
            Context * SetSnapRequest <
            I >::handle_shut_down_exclusive_lock(int *result) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            ...
            bool refresh_parent;
            {
                ...
                const ParentInfo *parent_info =
                    m_image_ctx.get_parent_info(m_snap_id);
                if (parent_info == nullptr) {
                    *result = -ENOENT;
                    lderr(cct) << "failed to retrieve snapshot parent info" <<
                        dendl;
                    finalize();
                    return m_on_finish;
                }
            ...
            m_refresh_parent =
                RefreshParentRequest < I >::create(m_image_ctx, parent_md, ctx);
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to refresh snapshot parent: " <<
                    cpp_strerror(*result)
                    << dendl;
                finalize();
                return m_on_finish;
        ...
        template < typename I >
            Context * SetSnapRequest <
            I >::handle_open_object_map(int *result) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            Context * SetSnapRequest <
            I >::send_finalize_refresh_parent(int *result) {
        ...
        template < typename I >
            Context * SetSnapRequest <
            I >::handle_finalize_refresh_parent(int *result) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct,
