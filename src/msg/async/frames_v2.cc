...
// Discards trailing empty segments, unless there is just one segment.// A frame always has at least one (possibly empty) segment.
    ...
    static void check_segment_crc(const bufferlist & segment_bl,
        ...
        if (crc != expected_crc) {
            throw FrameError(fmt::
                             format("bad segment crc calculated={} expected={}",
                                    crc, expected_crc));
        }
    }
...
// it was aborted by the sender and must be dropped.
    ...
    void FrameAssembler::fill_preamble(Tag tag, preamble_block_t & preamble) const {
        ...
        preamble.crc =
            ceph_crc32c(0, reinterpret_cast < const unsigned char *>(&preamble),
    ...
    bufferlist FrameAssembler::asm_crc_rev0(const preamble_block_t & preamble,
        ...
        frame_bl.append(reinterpret_cast < const char *>(&preamble),
                        sizeof(preamble));
        for (size_t i = 0; i < m_descs.size(); i++) {
            ceph_assert(segment_bls[i].length() == m_descs[i].logical_len);
            epilogue.crc_values[i] =
                m_with_data_crc ? segment_bls[i].crc32c(-1) : 0;
            if (segment_bls[i].length() > 0) {
                frame_bl.claim_append(segment_bls[i]);
        }} frame_bl.append(reinterpret_cast < const char *>(&epilogue),
                           sizeof(epilogue));
        return frame_bl;
    }

    bufferlist FrameAssembler::
        asm_secure_rev0(const preamble_block_t & preamble,
                        bufferlist segment_bls[])const {
    ...
    bufferlist FrameAssembler::
        asm_secure_rev1(const preamble_block_t & preamble,
                        bufferlist segment_bls[])const {
        ...
        if (segment_bls[0].length() > FRAME_PREAMBLE_INLINE_SIZE) {
            ...
            uint32_t pad_len =
                FRAME_PREAMBLE_INLINE_SIZE - segment_bls[0].length();
    ...
    Tag FrameAssembler::disassemble_preamble(bufferlist & preamble_bl) {
        if (m_crypto->rx) {
            ...
            if (m_is_rev1) {
                ceph_assert(preamble_bl.length() ==
        ...
        auto preamble =
            reinterpret_cast < const preamble_block_t * >(preamble_bl.c_str());
        ...
        if (preamble->num_segments < 1 ||
            ...
            throw FrameError(fmt::
                             format("bad number of segments num_segments={}",
    ...
    bool FrameAssembler::disasm_all_crc_rev0(bufferlist segment_bls[],
        ...
        auto epilogue =
            reinterpret_cast <
            const epilogue_crc_rev0_block_t * >(epilogue_bl.c_str());
    ...
    bool FrameAssembler::disasm_remaining_crc_rev1(bufferlist segment_bls[],
        ...
        auto epilogue =
            reinterpret_cast <
            const epilogue_crc_rev1_block_t * >(epilogue_bl.c_str());
                                                  ...
                                                  bufferlist & segment_bl) const
    {
        ...
        if (padded_len > FRAME_PREAMBLE_INLINE_SIZE) {
            ...
            bufferlist tmp;
            segment_bl.swap(tmp);
            preamble_bl.splice(sizeof(preamble_block_t),
                               FRAME_PREAMBLE_INLINE_SIZE, &segment_bl);
    ...
    bool FrameAssembler::disasm_remaining_secure_rev1(bufferlist segment_bls[],
                                                      bufferlist & epilogue_bl)
        const {
        m_crypto->rx->reset_rx_handler();
        for (size_t i = 1; i < m_descs.size(); i++) {
        ...
        auto epilogue =
            reinterpret_cast <
            const epilogue_secure_rev1_block_t * >(epilogue_bl.c_str());
        return check_epilogue_late_status(epilogue->late_status);
    }
    ...
    bool FrameAssembler::
        disassemble_remaining_segments(bufferlist segment_bls[],
                                       bufferlist & epilogue_bl) const {
        ceph_assert(!m_descs.empty());
        if (m_is_rev1) {
            if (m_descs.size() == 1) {
    ...
    void FrameAssembler::asm_compress(bufferlist segment_bls[]) {
        ...
        m_compression->tx->reset_handler(m_descs.size(),
