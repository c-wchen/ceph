...
{
    ldout(cct,
          10) << "id=" << notify_id << ", " << "handle=" << handle << dendl;
}
    ...
    : m_ioctx(ioctx), m_work_queue(work_queue), m_oid(oid),
      ...
      m_watch_lock(ceph::
                   make_shared_mutex(util::
                                     unique_lock_name
                                     ("librbd::Watcher::m_watch_lock",
                                      this))), m_watch_handle(0),
      m_notifier(work_queue, ioctx, oid), m_watch_state(WATCH_STATE_IDLE),
      m_watch_ctx(*this)
{
}
...
void Watcher::register_watch(Context *on_finish)
{
    ...
    int r =
        m_ioctx.aio_watch(m_oid, aio_comp, &m_watch_handle, &m_watch_ctx);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
{
    lderr(m_cct) << "handle=" << handle << ": " << cpp_strerror(err) <<
                 dendl;
...
void Watcher::handle_rewatch(int r)
{
    ...
    Context *unregister_watch_ctx = nullptr;
    {
        ...
        if (m_unregister_watch_ctx != nullptr) {
            ...
            lderr(m_cct) << "failed to rewatch: " << cpp_strerror(r) <<
                         dendl;
            watch_error = true;
