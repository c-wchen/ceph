...
TEST(LibCephFS, OpenReadTruncate)
{
    ...
    ASSERT_EQ(ceph_write(cmount, fd, data.c_str(), data.size(), 0),
              (int)data.size());
    ASSERT_EQ(0, ceph_close(cmount, fd));
...
TEST(LibCephFS, OpenLayout)
{
    ...
    int fd =
        ceph_open_layout(cmount, test_layout_file, O_CREAT | O_WRONLY, 0666,
                         (1 << 20), 7, (1 << 20), NULL);
    ASSERT_GT(fd, 0);
    char poolname[80];
    ASSERT_LT(0,
              ceph_get_file_pool_name(cmount, fd, poolname, sizeof(poolname)));
    ...
    ASSERT_EQ(-CEPHFS_ENOTEMPTY,
              ceph_setxattr(cmount, test_layout_file, xattrk, (void *)xattrv, 5,
                            0));

    /* invalid layout */
    sprintf(test_layout_file, "test_layout_%d_c", getpid());
    fd = ceph_open_layout(cmount, test_layout_file, O_CREAT, 0666, (1 << 20), 1,
                          19, NULL);
    ASSERT_EQ(fd, -CEPHFS_EINVAL);

    /* with data pool */
    sprintf(test_layout_file, "test_layout_%d_d", getpid());
    fd = ceph_open_layout(cmount, test_layout_file, O_CREAT, 0666, (1 << 20), 7,
                          (1 << 20), poolname);
    ASSERT_GT(fd, 0);
    ceph_close(cmount, fd);
...
TEST(LibCephFS, DirLs)
{
    ...
    ASSERT_EQ(ceph_statx(cmount, barstr, &stx, 0, AT_SYMLINK_NOFOLLOW),
              -CEPHFS_ENOENT);
...
TEST(LibCephFS, Xattrs)
{
    ...
    ASSERT_EQ(-CEPHFS_ENODATA,
              ceph_removexattr(cmount, test_xattr_file, "user.nosuchxattr"));
    ...
    for (; i < 'a' + 26; ++i) {
        ...
        ASSERT_EQ(ceph_setxattr
                  (cmount, test_xattr_file, xattrk, (void *)xattrv, len,
                   XATTR_CREATE), 0);
    }
    ...
    int len =
        ceph_listxattr(cmount, test_xattr_file, xattrlist, len_needed - 1);
    ...
    while (len > 0) {
        ...
        int alen =
            ceph_getxattr(cmount, test_xattr_file, p, (void *)gxattrv, 128);
        ASSERT_GT(alen, 0);
...
TEST(LibCephFS, Xattrs_ll)
{
    ...
    res =
        ceph_ll_setxattr(cmount, existent_file_handle, valid_name, value,
                         strlen(value), 0, perms);
    ASSERT_EQ(res, 0);

    res =
        ceph_ll_getxattr(cmount, existent_file_handle, valid_name, value_buf,
                         256, perms);
...
TEST(LibCephFS, StatDirNlink)
{
    ...
    ASSERT_GT(fd, 0);
    struct ceph_statx stx;
    ASSERT_EQ(ceph_fstatx
              (cmount, fd, &stx, CEPH_STATX_NLINK, AT_SYMLINK_NOFOLLOW), 0);
    ASSERT_EQ(stx.stx_nlink, 2u);

    {
        char test_dir2[296];
        sprintf(test_dir2, "%s/.", test_dir1);
        ASSERT_EQ(ceph_statx
                  (cmount, test_dir2, &stx, CEPH_STATX_NLINK,
...

    {
        ...
        ASSERT_EQ(stx.stx_nlink, 2u);
        ASSERT_EQ(ceph_statx
                  (cmount, test_dir1, &stx, CEPH_STATX_NLINK,
                   AT_SYMLINK_NOFOLLOW), 0);
...
TEST(LibCephFS, DoubleChmod)
{
    ...
    ASSERT_EQ(ceph_write(cmount, fd, bytes, strlen(bytes), 0),
              (int)strlen(bytes));
    ...
    struct ceph_statx stx;
    ASSERT_EQ(ceph_statx
              (cmount, test_file, &stx, CEPH_STATX_MODE, AT_SYMLINK_NOFOLLOW),
              0);
...
TEST(LibCephFS, Symlinks)
{
    ...
    ASSERT_EQ(ceph_statx(cmount, test_file, &stx_orig, CEPH_STATX_ALL_STATS, 0),
              0);
    // stat the symlink
    struct ceph_statx stx_symlink_orig;
    ASSERT_EQ(ceph_statx
              (cmount, test_symlink, &stx_symlink_orig, CEPH_STATX_ALL_STATS,
               0), 0);
...
TEST(LibCephFS, LoopSyms)
{
    ...
    char test_file[256];
    sprintf(test_file, "/dir1_loopsym_%d/loop_dir/symdir/test_loopsym_file",
            getpid());
    int fd = ceph_open(cmount, test_file, O_CREAT | O_RDWR, 0600);
    ASSERT_EQ(fd, -CEPHFS_ELOOP);
...
TEST(LibCephFS, BadFileDesc)
{
    ...
    ASSERT_EQ(ceph_get_file_stripe_address(cmount, -1, 0, &addr, 1),
              -CEPHFS_EBADF);
    ...
    ASSERT_EQ(ceph_get_file_layout
...
TEST(LibCephFS, PreadvPwritev)
{
    ...
    ASSERT_EQ(0,
              strncmp((const char *)iov_in[0].iov_base,
                      (const char *)iov_out[0].iov_base, iov_out[0].iov_len));
    ASSERT_EQ(0,
              strncmp((const char *)iov_in[1].iov_base,
                      (const char *)iov_out[1].iov_base, iov_out[1].iov_len));
...
TEST(LibCephFS, LlreadvLlwritev)
{
    ...
    ASSERT_EQ(ceph_ll_create(cmount, root, filename, 0666,
                             O_RDWR | O_CREAT | O_TRUNC, &file, &fh, &stx, 0, 0,
                             perms), 0);
...
TEST(LibCephFS, Rename)
{
    ...
    ASSERT_EQ(-CEPHFS_ENOENT,
              ceph_statx(cmount, path_src, &stx, 0, AT_SYMLINK_NOFOLLOW));
...
TEST(LibCephFS, UseUnmounted)
{
    ...
    struct ceph_statx stx;
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_readdirplus_r(cmount, dirp, &rdent, &stx, 0, 0, NULL));
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_getxattr(cmount, "/path", "name", NULL, 0));
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_lgetxattr(cmount, "/path", "name", NULL, 0));
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_setxattr(cmount, "/path", "name", NULL, 0, 0));
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_lsetxattr(cmount, "/path", "name", NULL, 0, 0));
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_open_layout(cmount, "/path", 0, 0, 0, 0, 0, "pool"));
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_get_file_layout(cmount, 0, NULL, NULL, NULL, NULL));
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_get_path_layout(cmount, "/path", NULL, NULL, NULL, NULL));
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_get_path_pool_name(cmount, "/path", NULL, 0));
    EXPECT_EQ(-CEPHFS_ENOTCONN, ceph_get_pool_name(cmount, 0, NULL, 0));
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_get_file_stripe_address(cmount, 0, 0, NULL, 0));
...
TEST(LibCephFS, GetExtentOsds)
{
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_get_file_extent_osds(cmount, 0, 0, NULL, NULL, 0));
    ...
    /* half stripe extent */
    EXPECT_EQ(ret,
              ceph_get_file_extent_osds(cmount, fd, stripe_unit / 2, &len, osds,
                                        ret));
    EXPECT_EQ(len, (int64_t) stripe_unit / 2);

    /* 1.5 stripe unit offset -1 byte */
    EXPECT_EQ(ret,
              ceph_get_file_extent_osds(cmount, fd, 3 * stripe_unit / 2 - 1,
                                        &len, osds, ret));
    ...
    if (ret > 1) {
        EXPECT_EQ(-CEPHFS_ERANGE,
                  ceph_get_file_extent_osds(cmount, fd, 0, NULL, osds, 1));
    }
...
TEST(LibCephFS, GetOsdCrushLocation)
{
    ...
    EXPECT_EQ(-CEPHFS_ENOTCONN,
              ceph_get_osd_crush_location(cmount, 0, NULL, 0));
    ...
    char path[256];
    ASSERT_EQ(ceph_get_osd_crush_location(cmount, 9999999, path, 0),
              -CEPHFS_ENOENT);
    ...
    for (int i = 0; i < ret; i++) {
        int len =
            ceph_get_osd_crush_location(cmount, osds[i], path, sizeof(path));
        ASSERT_GT(len, 0);
        int pos = 0;
        while (pos < len) {
...
TEST(LibCephFS, Nlink)
{
    ...
    ASSERT_EQ(ceph_ll_mkdir
              (cmount, root, dirname, 0755, &dir, &stx, 0, 0, perms), 0);
    ASSERT_EQ(ceph_ll_create
              (cmount, dir, filename, 0666, O_RDWR | O_CREAT | O_EXCL, &file,
               &fh, &stx, CEPH_STATX_NLINK, 0, perms), 0);
    ...
    ASSERT_EQ(ceph_ll_getattr(cmount, file, &stx, CEPH_STATX_NLINK, 0, perms),
              0);
...
TEST(LibCephFS, LazyStatx)
{
    ...
    ASSERT_EQ(ceph_ll_create
              (cmount1, root1, filename, 0666, O_RDWR | O_CREAT | O_EXCL,
    ...
    ASSERT_EQ(ceph_ll_lookup
              (cmount2, root2, filename, &file2, &stx, CEPH_STATX_CTIME, 0,
               perms2), 0);
    ...
    sleep(1);
    stx.stx_mode = 0644;
    ASSERT_EQ(ceph_ll_setattr(cmount1, file1, &stx, CEPH_SETATTR_MODE, perms1),
              0);

    ASSERT_EQ(ceph_ll_getattr
              (cmount2, file2, &stx, CEPH_STATX_CTIME, AT_STATX_DONT_SYNC,
               perms2), 0);
...
TEST(LibCephFS, DirChangeAttrCreateFile)
{
    ...
    ASSERT_EQ(ceph_setxattr
              (cmount, dirpath, "user.name", (void *)"bob", 3, XATTR_CREATE),
              0);
    ASSERT_EQ(ceph_statx(cmount, dirpath, &stx, CEPH_STATX_VERSION, 0), 0);
    ASSERT_TRUE(stx.stx_mask & CEPH_STATX_VERSION);
...
TEST(LibCephFS, OperationsOnDotDot)
{
    ...
    sprintf(c_non_existent_dirs,
            "/%s/../ood_nonexistent1_%d/oodd_nonexistent2_%d", c_dir, mypid,
            mypid);
...
TEST(LibCephFS, Caps_vxattr)
{
    ...
    int alen =
        ceph_getxattr(cmount, test_caps_vxattr_file, "ceph.caps",
                      (void *)gxattrv, xbuflen);
    ASSERT_GT(alen, 0);
    gxattrv[alen] = '\0';
...
TEST(LibCephFS, SnapXattrs)
{
    ...
    int alen =
        ceph_getxattr(cmount, c_temp, "ceph.snap.btime", (void *)gxattrv,
                      xbuflen);
    ...
    *s = '.';                   // restore for later strcmp
    ...
    sprintf(c_temp, "/.snap/test_snap_xattr_snap_%d/%s", mypid,
            test_snap_xattr_file);

    int alen2 =
        ceph_getxattr(cmount, c_temp, "ceph.snap.btime", (void *)gxattrv2,
                      xbuflen);
    ...
    alen =
        ceph_getxattr(cmount, test_snap_xattr_file, "ceph.snap.btime",
                      (void *)gxattrv2, xbuflen);
    ...
    *s = '\0';
    ...
    char xattrlist[512];
    int len =
        ceph_listxattr(cmount, test_snap_xattr_file, xattrlist,
                       sizeof(xattrlist));
...
TEST(LibCephFS, Statxat)
{
    ...
    ASSERT_EQ(ceph_statxat(cmount, fd, rel_file_name_1, &stx, 0, 0),
              -CEPHFS_ENOENT);
    ASSERT_EQ(0, ceph_close(cmount, fd));
...
TEST(LibCephFS, StatxatATFDCWD)
{
    ...
    ASSERT_EQ(ceph_statxat
              (cmount, CEPHFS_AT_FDCWD, rel_file_name_1, &stx, 0, 0), 0);
    ASSERT_EQ(stx.stx_mode & S_IFMT, S_IFREG);
...
TEST(LibCephFS, ReadlinkatATFDCWD)
{
    ...
    char target[target_len + 1];
    ASSERT_EQ(target_len,
              ceph_readlinkat(cmount, CEPHFS_AT_FDCWD, rel_link_path, target,
                              target_len));
    target[target_len] = '\0';
    ASSERT_EQ(0, memcmp(target, rel_file_path, target_len));
...
TEST(LibCephFS, SymlinkatATFDCWD)
{
    ...
    ASSERT_EQ(0, ceph_chdir(cmount, dir_path));
    ASSERT_EQ(0,
              ceph_symlinkat(cmount, rel_file_path, CEPHFS_AT_FDCWD,
                             rel_link_path));
...
TEST(LibCephFS, Unlinkat)
{
    ...
    fd = ceph_open(cmount, dir_path, O_DIRECTORY | O_RDONLY, 0);
    ASSERT_LE(0, fd);
    ASSERT_EQ(-CEPHFS_ENOTDIR,
              ceph_unlinkat(cmount, fd, rel_file_path, AT_REMOVEDIR));
...
TEST(LibCephFS, UnlinkatATFDCWD)
{
    ...
    ASSERT_EQ(0, ceph_chdir(cmount, dir_path));
    ASSERT_EQ(-CEPHFS_ENOTDIR,
              ceph_unlinkat(cmount, CEPHFS_AT_FDCWD, rel_file_path,
                            AT_REMOVEDIR));
    ASSERT_EQ(0, ceph_unlinkat(cmount, CEPHFS_AT_FDCWD, rel_file_path, 0));

    ASSERT_EQ(0, ceph_chdir(cmount, "/"));
    ASSERT_EQ(-CEPHFS_EISDIR,
              ceph_unlinkat(cmount, CEPHFS_AT_FDCWD, dir_name, 0));
    ASSERT_EQ(0,
              ceph_unlinkat(cmount, CEPHFS_AT_FDCWD, dir_name, AT_REMOVEDIR));
...
TEST(LibCephFS, ChownatATFDCWD)
{
    ...
    // change ownership to nobody -- we assume nobody exists and id is always 65534
    ASSERT_EQ(ceph_conf_set(cmount, "client_permissions", "0"), 0);
    ASSERT_EQ(ceph_chownat
              (cmount, CEPHFS_AT_FDCWD, rel_file_path, 65534, 65534, 0), 0);
    ASSERT_EQ(ceph_conf_set(cmount, "client_permissions", "1"), 0);
...
TEST(LibCephFS, UtimensatATFDCWD)
{
    ...
    ASSERT_EQ(0, ceph_chdir(cmount, dir_path));
    EXPECT_EQ(0,
              ceph_utimensat(cmount, CEPHFS_AT_FDCWD, rel_file_path, times, 0));
...
TEST(LibCephFS, FsCrypt)
{
    ...
    ASSERT_EQ(0,
              ceph_fsetxattr(cmount, fd, "ceph.fscrypt.auth", "foo", 3,
                             CEPH_XATTR_CREATE));
    ASSERT_EQ(0,
              ceph_fsetxattr(cmount, fd, "ceph.fscrypt.file", "foo", 3,
                             CEPH_XATTR_CREATE));

    char buf[64];
    ASSERT_EQ(3,
              ceph_fgetxattr(cmount, fd, "ceph.fscrypt.auth", buf,
                             sizeof(buf)));
    ASSERT_EQ(3,
              ceph_fgetxattr(cmount, fd, "ceph.fscrypt.file", buf,
                             sizeof(buf)));
    ASSERT_EQ(0, ceph_close(cmount, fd));
...
TEST(LibCephFS, SnapdirAttrs)
{
    ...
    ASSERT_EQ(ceph_ll_lookup_root(cmount, &root), 0);
    ASSERT_EQ(ceph_ll_mkdir
              (cmount, root, dir_name, 0755, &dir, &stx_dir, 0, 0, perms), 0);
    ...
    ASSERT_EQ(ceph_statx
              (cmount, snap_dir_path, &stx_snap_dir,
               CEPH_STATX_MTIME | CEPH_STATX_ATIME | CEPH_STATX_MODE |
               CEPH_STATX_MODE | CEPH_STATX_GID | CEPH_STATX_VERSION, 0), 0);
    ASSERT_EQ(ceph_statx
    ...
    // case) attrs
    ASSERT_EQ(utime_t(stx_root_snap_dir.stx_mtime),
              utime_t(stx_snap_dir.stx_mtime));
    ASSERT_EQ(utime_t(stx_root_snap_dir.stx_ctime),
              utime_t(stx_snap_dir.stx_ctime));
    ...
    stx_dir.stx_uid++;
    stx_dir.stx_gid++;
    ASSERT_EQ(ceph_ll_setattr
              (cmount, dir, &stx_dir, CEPH_SETATTR_UID | CEPH_SETATTR_GID,
               rootcred), 0);
...
TEST(LibCephFS, SnapdirAttrsOnSnapCreate)
{
    ...
    ASSERT_EQ(ceph_statx
              (cmount, dir_path, &stx_dir,
               CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0), 0);
    ASSERT_EQ(ceph_statx
              (cmount, snap_dir_path, &stx_snap_dir,
               CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0), 0);
    ASSERT_EQ(ceph_statx
    ...
    ASSERT_EQ(ceph_statx
              (cmount, snap_dir_path, &stx_snap_dir_1,
               CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0), 0);
    ASSERT_LT(utime_t(stx_root_snap_dir.stx_mtime),
              utime_t(stx_snap_dir_1.stx_mtime));
    ASSERT_LT(utime_t(stx_root_snap_dir.stx_ctime),
              utime_t(stx_snap_dir_1.stx_ctime));
...
TEST(LibCephFS, SnapdirAttrsOnSnapDelete)
{
    ...
    ASSERT_EQ(ceph_statx
              (cmount, snap_dir_path, &stx_snap_dir_2,
               CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0), 0);
    ASSERT_LT(utime_t(stx_snap_dir_1.stx_mtime),
              utime_t(stx_snap_dir_2.stx_mtime));
    ASSERT_LT(utime_t(stx_snap_dir_1.stx_ctime),
              utime_t(stx_snap_dir_2.stx_ctime));
