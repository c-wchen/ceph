...
#ifdef HAVE_RES_NQUERY
    EXPECT_CALL(*resolvH,
                res_nquery(_, StrEq("not_exists.com"), C_IN, T_A, _, _))
        .WillOnce(Return(0));
#else
...
TEST_F(DNSResolverTest, resolve_srv_hosts_empty_domain)
{
    ...
    DNSResolver::get_instance(resolvH)->resolve_srv_hosts(g_ceph_context,
                                                          "cephmon",
                                                          DNSResolver::
                                                          SRV_Protocol::TCP,
                                                          &records);
...
#ifdef HAVE_RES_NQUERY
        EXPECT_CALL(*resolvH,
                    res_nsearch(_, StrEq("_noservice._tcp"), C_IN, T_SRV, _, _))
            .WillOnce(Return(0));
#else
        EXPECT_CALL(*resolvH,
                    res_search(StrEq("_noservice._tcp"), C_IN, T_SRV, _, _))
            .WillOnce(Return(0));
#endif
    }
