                                 ...
                                 bufferlist * out)
{
    ...
    init_op.max_urgent_data_size =
        g_ceph_context->_conf->rgw_gc_max_deferred_entries_size;
        ...
        if (op_ret.entries.size()) {
          for (auto it:op_ret.entries) {
                ...
                if (!found && urgent_data.num_xattr_urgent_entries > 0) {
                    ...
                    int ret =
                        cls_cxx_getxattr(hctx, "cls_queue_urgent_data",
                                         &bl_xattrs);
                    if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
                        CLS_LOG(0,
                                "ERROR: %s(): cls_cxx_getxattrs() returned %d",
                                __func__, ret);
                        return ret;
                    }
                    if (ret != -ENOENT && ret != -ENODATA) {
                        std::unordered_map < string,
                            ceph::real_time > xattr_urgent_data_map;
            ...
            CLS_LOG(10,
                    "INFO: cls_rgw_gc_queue_list_entries(): num_entries: %u and op.max: %u\n",
                    num_entries, op.max);
                                           ...
                                           bufferlist * in, bufferlist * out)
{
    ...
    do {
        ...
        if (op_ret.entries.size()) {
          for (auto it:op_ret.entries) {
                ...
                CLS_LOG(20,
                        "INFO: cls_rgw_gc_queue_remove_entries(): entry: %s\n",
                        info.tag.c_str());
                total_num_entries++;
                index++;
                bool found = false;
    ...
    CLS_LOG(10,
            "INFO: cls_rgw_gc_queue_remove_entries(): Total number of entries to remove: %d\n",
            total_num_entries);
    ...
    CLS_LOG(5,
            "INFO: cls_rgw_gc_queue_remove_entries(): Urgent data size is %u\n",
                                         ...
                                         bufferlist * in, bufferlist * out)
{
    ...
    if (it != urgent_data.urgent_data_map.end()) {
        ...
        if (ret != -ENOENT && ret != -ENODATA) {
            ...
            if (xattr_iter != xattr_urgent_data_map.end()) {
                ...
                CLS_LOG(20, "%s(): setting attr: %s", __func__,
    ...
    if (!tag_found) {
        ...
        if (bl_urgent_data.length() > head.max_urgent_data_size) {
            ...
            bufferlist bl_xattrs;
            int ret =
                cls_cxx_getxattr(hctx, "cls_queue_urgent_data", &bl_xattrs);
            if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
                CLS_LOG(0, "ERROR: %s(): cls_cxx_getxattrs() returned %d",
                        __func__, ret);
                return ret;
            }
            std::unordered_map < string,
    ...
    if ((urgent_data.num_head_urgent_entries +
        ...
        CLS_LOG(20, "Total num entries %u",
                urgent_data.num_urgent_data_entries);
        CLS_LOG(20, "Num xattr entries %u",
                urgent_data.num_xattr_urgent_entries);
        CLS_LOG(20, "Num head entries %u", urgent_data.num_head_urgent_entries);
        CLS_LOG(0,
                "ERROR: Number of urgent data entries exceeded that requested by user, returning no space!");
        return -ENOSPC;
    }
...
CLS_INIT(rgw_gc)
{
    ...
    /* gc */
    cls_register_cxx_method(h_class, RGW_GC_QUEUE_INIT,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_rgw_gc_queue_init, &h_rgw_gc_queue_init);
    cls_register_cxx_method(h_class, RGW_GC_QUEUE_ENQUEUE,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_rgw_gc_queue_enqueue, &h_rgw_gc_queue_enqueue);
    cls_register_cxx_method(h_class, RGW_GC_QUEUE_LIST_ENTRIES, CLS_METHOD_RD,
                            cls_rgw_gc_queue_list_entries,
                            &h_rgw_gc_queue_list_entries);
    cls_register_cxx_method(h_class, RGW_GC_QUEUE_REMOVE_ENTRIES,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_rgw_gc_queue_remove_entries,
                            &h_rgw_gc_queue_remove_entries);
    cls_register_cxx_method(h_class, RGW_GC_QUEUE_UPDATE_ENTRY,
                            CLS_METHOD_RD | CLS_METHOD_WR,
