...
int FileStore::validate_hobject_key(const hobject_t & obj) const const
{
...
{
    os_commit_latency.consume_next(logger.
                                   get_tavg_ms(l_filestore_journal_latency));
    os_apply_latency.consume_next(logger.
                                  get_tavg_ms(l_filestore_apply_latency));
}
...
int FileStore::lfn_unlink(const coll_t & cid, const ghobject_t & o,
                          const SequencerPosition & spos, bool force_clear_omap)
{
    Index index;
    int r = get_index(cid, &index);
    if (r < 0) {
        dout(25) << __FUNC__ << ": get_index failed " << cpp_strerror(r) <<
            dendl;
        return r;
    }
...

    {
        ...
        if (force_clear_omap) {
            ...
            if (r < 0 && r != -ENOENT) {
                dout(25) << __FUNC__ << ": omap clear failed " <<
                    cpp_strerror(r) << dendl;
                assert(!m_filestore_fail_eio || r != -EIO);
                return r;
            }
...
FileStore::FileStore(CephContext * cct, const std::string & base, const std::string & jdev, osflagbits_t flags, const char *name, bool do_update):
...
m_apply_finisher_num(cct->_conf->filestore_apply_finisher_threads), op_tp(cct,
                                                                          "FileStore::op_tp",
                                                                          "tp_fstore_op",
                                                                          cct->
                                                                          _conf->
...
op_wq(this, cct->_conf->filestore_op_thread_timeout,
      cct->_conf->filestore_op_thread_suicide_timeout, &op_tp), logger(NULL),
...
m_journal_dio(cct->_conf->journal_dio), m_journal_aio(cct->_conf->journal_aio),
...
m_filestore_max_inline_xattrs(0), m_filestore_max_xattr_value_size(0)
{
    ...
    plb.add_time_avg(l_filestore_apply_latency, "apply_latency",
                     "Apply latency");
    plb.add_u64(l_filestore_committing, "committing",
                "Is currently committing");

    plb.add_u64_counter(l_filestore_commitcycle, "commitcycle",
                        "Commit cycles");
...
void FileStore::create_backend(long f_type)
{
    ...
    dout(0) << "backend " << backend->get_name()
        << " (magic 0x" << std::hex << f_type << std::dec << ")" << dendl;
...
int FileStore::mkfs()
{
    ...
    if (basefs.f_type == BTRFS_SUPER_MAGIC &&
        ...
        derr << __FUNC__ << ": deprecated btrfs support is not enabled" <<
            dendl;
        goto close_fsid_fd;
    }
#endif
    ...
    // write initial op_seq
    {
        ...
        if (initial_seq == 0) {
            ...
            if (backend->can_checkpoint()) {
                ...
                if (ret < 0 && ret != -EEXIST) {
                    VOID_TEMP_FAILURE_RETRY(::close(fd));
                    derr << __FUNC__ << ": failed to create snap_1: " <<
                        cpp_strerror(ret) << dendl;
                    goto close_fsid_fd;
                }
            }
    ...
    if (ret < 0) {
        derr << __FUNC__ << ": failed to create " << cct->_conf->
            filestore_omap_backend << dendl;
        goto close_fsid_fd;
    }
    ...
    snprintf(omap_fsid_fn, sizeof(omap_fsid_fn), "%s/osd_uuid",
             omap_dir.c_str());
    ...
    if (read_fsid(omap_fsid_fd, &old_omap_fsid) < 0 || old_omap_fsid.is_zero()) {
        ...
        dout(10) << __FUNC__ << ": write success, fsid:" << fsid_str << ", ret:"
            << ret << dendl;
        ...
        dout(1) << __FUNC__ << ": omap fsid is already set to " << fsid <<
            dendl;
    }

    dout(1) << cct->_conf->
        filestore_omap_backend << " db exists/created" << dendl;
...
int FileStore::mkjournal()
{
    ...
    if (journal) {
        ...
        if (ret < 0) {
            ret = journal->create();
            if (ret)
                derr << __FUNC__ << ": error creating journal on " <<
                    journalpath << ": " << cpp_strerror(ret) << dendl;
            else
                dout(0) << __FUNC__ << ": created journal on " << journalpath <<
                    dendl;
        }
        delete journal;
        journal = 0;
...
int FileStore::_detect_fs()
{
    ...
    if (r < 0) {
        derr << __FUNC__ << ": detect_features error: " << cpp_strerror(r) <<
            dendl;
        return r;
    }
    ...
    if (tmpfd < 0) {
        int ret = -errno;
        derr << __FUNC__ << ": unable to create " << fn << ": " <<
            cpp_strerror(ret) << dendl;
        return ret;
    }
...
int FileStore::_sanity_check_fs()
{
    ...
    if (((int)m_filestore_journal_writeahead +
        ...
        dout(0) <<
            "mount ERROR: more than one of filestore journal {writeahead,parallel,trailing} enabled"
            << dendl;
        cerr << TEXT_RED <<
    ...
    if (!backend->can_checkpoint()) {
        if (!journal || !m_filestore_journal_writeahead) {
            dout(0) <<
                "mount WARNING: no btrfs, and no journal in writeahead mode; data may be lost"
                << dendl;
            cerr << TEXT_RED <<
...
int FileStore::upgrade()
{
    ...
    if (version < 3) {
        derr << "ObjectStore is old at version " << version <<
            ".  Please upgrade to firefly v0.80.x, convert your store, and then upgrade."
            << dendl;
        return -EINVAL;
    }
...
{
    int op_fd =::open(current_op_seq_fn.c_str(), O_CREAT | O_RDWR | O_CLOEXEC,
                      0644);
    if (op_fd < 0) {
        int r = -errno;
        assert(!m_filestore_fail_eio || r != -EIO);
...
int FileStore::mount()
{
    ...
    if (m_osd_rollback_to_cluster_snap.length() &&
        ...
        derr << "rollback to cluster snapshot '" <<
            m_osd_rollback_to_cluster_snap << "': not found" << dendl;
        ret = -ENOENT;
        goto close_basedir_fd;
    }
    ...
    if (backend->can_checkpoint()) {
        if (snaps.empty()) {
            dout(0) << __FUNC__ <<
                ": WARNING: no consistent snaps found, store may be in inconsistent state"
                << dendl;
        }
        else {
            char s[NAME_MAX];
            ...
            if (m_osd_rollback_to_cluster_snap.length()) {
                derr << TEXT_RED
                    << " ** NOTE: rolling back to cluster snapshot " <<
                    m_osd_rollback_to_cluster_snap << " **" << TEXT_NORMAL <<
                    dendl;
                ...
                if (curr_seq)
                    ...
                    dout(10) << " current/ missing entirely (unusual, but okay)"
                        << dendl;

                uint64_t cp = snaps.back();
                dout(10) << " most recent snap from " << snaps << " is " << cp
                    << dendl;
                ...
                if (::stat(nosnapfn, &st) == 0) {
                    if (!m_osd_use_stale_snap) {
                        derr << "ERROR: " << nosnapfn <<
                            ...
                            << dendl;
                        derr <<
                            "Force rollback to old snapshotted version with 'osd use stale snap = true'"
                            << dendl;
                        derr <<
                            "config option for --osd-use-stale-snap startup argument."
                            << dendl;
                        ret = -ENOTSUP;
                        goto close_basedir_fd;
                    }
                    derr <<
                        "WARNING: user forced start with data sequence mismatch: current was "
                ...
                dout(10) << __FUNC__ << ": rolling back to consistent snap " <<
                    cp << dendl;
    ...
    if (::stat(omap_fsid_buf, &omap_fsid_stat) != 0) {
        ...
        if (ret < 0) {
            derr << __FUNC__ << ": error reading omap_fsid_fd"
                << ", omap_fsid = " << omap_fsid << cpp_strerror(ret)
                << dendl;
            goto close_current_fd;
        }
        if (fsid != omap_fsid) {
    ...
    if (!(generic_flags & SKIP_MOUNT_OMAP)) {
        KeyValueDB *omap_store = KeyValueDB::create(cct,
                                                    ...
                                                    omap_dir);
        if (omap_store == NULL) {
            derr << __FUNC__ << ": Error creating " << superblock.
                omap_backend << dendl;
            ret = -1;
            goto close_current_fd;
        }
        ...
        if (ret < 0) {
            derr << __FUNC__ << ": Error initializing omap_store: " <<
                cpp_strerror(ret) << dendl;
            goto close_current_fd;
        }
        ...
        if (omap_store->create_and_open(err)) {
            delete omap_store;
            derr << __FUNC__ << ": Error initializing " << superblock.
                omap_backend << " : " << err.str() << dendl;
            ret = -1;
            goto close_current_fd;
        }
        ...
        if (ret < 0) {
            delete dbomap;
            derr << __FUNC__ << ": Error initializing DBObjectMap: " << ret <<
                dendl;
            goto close_current_fd;
        }
        stringstream err2;
    ...
    if (journal) {
        if (!m_filestore_journal_writeahead &&
            !m_filestore_journal_parallel && !m_filestore_journal_trailing) {
            if (!backend->can_checkpoint()) {
...
void FileStore::queue_op(OpSequencer * osr, Op * o)
{
    ...
    dout(5) << __FUNC__ << ": " << o << " seq " << o->op
        << " " << *osr
        << " " << o->bytes << " bytes"
        << "   (queue has " << throttle_ops.
        get_current() << " ops and " << throttle_bytes.
        get_current() << " bytes)" << dendl;
    op_wq.queue(osr);
}
...
void FileStore::_do_op(OpSequencer * osr, ThreadPool::TPHandle & handle)
{
    ...
    if (cct->_conf->filestore_inject_stall) {
        ...
        dout(5) << __FUNC__ << ": filestore_inject_stall " << orig <<
            ", sleeping" << dendl;
        sleep(orig);
    ...
    dout(5) << __FUNC__ << ": " << o << " seq " << o->
        op << " " << *osr << "/" << osr->parent << " start" << dendl;
...
void FileStore::_finish_op(OpSequencer * osr)
{
    ...
    dout(10) << __FUNC__ << ": " << o << " seq " << o->
        op << " " << *osr << "/" << osr->parent << " lat " << lat << dendl;
...
struct C_JournaledAhead:public Context {
    ...
    C_JournaledAhead(FileStore * f, FileStore::OpSequencer * os,
                     FileStore::Op * o, Context * ondisk):fs(f), osr(os), o(o),
        ondisk(ondisk) {
                                  ...
                                  ThreadPool::TPHandle * handle)
{
    ...
    if (journal && journal->is_writeable() && !m_filestore_journal_trailing) {
        ...
        if (m_filestore_journal_parallel) {
            dout(5) << __FUNC__ << ": (parallel) " << o->op << " " << o->
                tls << dendl;
            ...
            dout(5) << __FUNC__ << ": (writeahead) " << o->op << " " << o->
                tls << dendl;
...
int FileStore::_do_transactions(vector < Transaction > &tls,
                                uint64_t op_seq, ThreadPool::TPHandle * handle)
{
    int trans_num = 0;
                                         ...
                                         const SequencerPosition & spos)
{
    ...
    if (fd < 0) {
        int err = errno;
        derr << __FUNC__ << ": " << cid << " error " << cpp_strerror(err) <<
            dendl;
        assert(0 == "_set_global_replay_guard failed");
    }
    ...
    ::encode(spos, v);
    int r =
        chain_fsetxattr < true, true > (fd, GLOBAL_REPLAY_GUARD_XATTR,
                                        v.c_str(), v.length());
    if (r < 0) {
                                    ...
                                    const ghobject_t * hoid)
{
    ...
    ::encode(in_progress, v);
    int r =
        chain_fsetxattr < true, true > (fd, REPLAY_GUARD_XATTR, v.c_str(),
                                        v.length());
    if (r < 0) {
        derr << "fsetxattr " << REPLAY_GUARD_XATTR << " got " << cpp_strerror(r)
            << dendl;
...
int FileStore::_check_replay_guard(int fd, const SequencerPosition & spos)
{
    ...
    if (opos > spos) {
        dout(10) << __FUNC__ << ": object has " << opos << " > current pos " <<
            spos << ", now or in future, SKIPPING REPLAY" << dendl;
        return -1;
    }
    else if (opos == spos) {
        if (in_progress) {
            dout(10) << __FUNC__ << ": object has " << opos <<
                " == current pos " << spos <<
        ...
        dout(10) << __FUNC__ << ": object has " << opos << " < current pos " <<
            spos << ", in past, will replay" << dendl;
        return 1;
    }
}
...
#ifdef WITH_LTTNG
    const char *osr_name =
        t.get_osr()? static_cast <
        OpSequencer * >(t.get_osr())->get_name().c_str() : "<NULL>";
#endif
        ...
        case Transaction::OP_SETATTR:
            {
                ...
                if (_check_replay_guard(cid, oid, spos) > 0) {
                    ...
                    if (r == -ENOSPC)
                        dout(0) << " ENOSPC on setxattr on " << cid << "/" <<
                            oid << " name " << name << " size " << bl.
                            length() << dendl;
                }
        ...
        case Transaction::OP_SETATTRS:
            {
                ...
                if (r == -ENOSPC)
                    dout(0) << " ENOSPC on setxattrs on " << cid << "/" << oid
                        << dendl;
            }
            break;
        ...
        case Transaction::OP_CLONERANGE2:
            {
                ...
                r = _clone_range(cid, oid, ncid, noid, srcoff, len, dstoff,
                                 spos);
        ...
        case Transaction::OP_COLL_MOVE_RENAME:
            {
                ...
                const coll_t & oldcid =
                    !_need_temp_object_collection(_oldcid,
                                                  oldoid) ? _oldcid : _oldcid.
                    get_temp();
                const coll_t & newcid =
                    !_need_temp_object_collection(_newcid,
                                                  newoid) ? _oldcid : _newcid.
                    get_temp();
        ...
        if (r < 0) {
            ...
            if (replaying && !backend->can_checkpoint()) {
                if (r == -EEXIST && op->op == Transaction::OP_MKCOLL) {
                    dout(10) <<
                        "tolerating EEXIST during journal replay since checkpoint is not enabled"
                        << dendl;
                    ok = true;
                }
            ...
            if (!ok) {
                ...
                derr << " error " << cpp_strerror(r) <<
                    " not handled on operation " << op << " (" << spos <<
                    ", or op " << spos.op << ", counting from 0)" << dendl;
                dout(0) << msg << dendl;
...
int FileStore::stat(const coll_t & _cid, const ghobject_t & oid,
...
int FileStore::set_collection_opts(const coll_t & cid, const pool_opts_t & opts)
{
    return -EOPNOTSUPP;
}

int FileStore::read(const coll_t & _cid,
                    const ghobject_t & oid,
                    uint64_t offset,
                    size_t len, bufferlist & bl, uint32_t op_flags)
{
    ...
    if (got < 0) {
        dout(10) << __FUNC__ << ": (" << cid << "/" << oid << ") pread error: "
    ...
    if (cct->_conf->filestore_debug_inject_read_err && debug_data_eio(oid)) {
        return -EIO;
             ...
             (rand() %
              (int)(cct->_conf->filestore_debug_random_read_err * 100.0)) ==
             0) {
                          ...
                          map < uint64_t, uint64_t > *m)
{
  ...
  more:
    ...
    while (i < fiemap->fm_mapped_extents) {
        ...
        dout(10) << __FUNC__ << ": fm_mapped_extents=" << fiemap->
            fm_mapped_extents << " fe_logical=" << extent->
            fe_logical << " fe_length=" << extent->fe_length << dendl;
...
int FileStore::_zero(const coll_t & cid, const ghobject_t & oid,
                     uint64_t offset, size_t len)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << oid << " " << offset << "~" <<
        len << dendl;
...
int FileStore::_do_clone_range(int from, int to, uint64_t srcoff, uint64_t len,
                               uint64_t dstoff)
{
    dout(20) << __FUNC__ << ": copy " << srcoff << "~" << len << " to " <<
        dstoff << dendl;
                                     ...
                                     uint64_t len, uint64_t dstoff)
{
    ...
    if (backend->has_seek_data_hole()) {
        dout(15) << "seek_data/seek_hole " << from << " " << srcoff << "~" <<
            len << dendl;
        r = _do_seek_hole_data(from, srcoff, len, &exomap);
    }
    else if (backend->has_fiemap()) {
        dout(15) << "fiemap ioctl" << from << " " << srcoff << "~" << len <<
            dendl;
    ...
    for (map < uint64_t, uint64_t >::iterator miter = exomap.begin();
        ...
        if (r < 0) {
            derr << __FUNC__ << ": copy error at " << miter->
                first << "~" << miter->
                second << " to " << it_off << ", " << cpp_strerror(r) << dendl;
            break;
        }
    ...
    if (r >= 0) {
        if (m_filestore_sloppy_crc) {
            int rc =
                backend->_crc_update_clone_range(from, to, srcoff, len, dstoff);
            assert(rc >= 0);
        }
        struct stat st;
        r =::fstat(to, &st);
        if (r < 0) {
            r = -errno;
            derr << __FUNC__ << ": fstat error at " << to << " " <<
                cpp_strerror(r) << dendl;
            goto out;
        }
        if (st.st_size < (int)(dstoff + len)) {
            ...
            if (r < 0) {
                r = -errno;
                derr << __FUNC__ << ": ftruncate error at " << dstoff +
                    len << " " << cpp_strerror(r) << dendl;
                goto out;
            }
        }
        r = written;
                              ...
                              uint64_t dstoff, bool skip_sloppycrc)
{
    ...
    if (backend->has_splice()) {
        ...
        while (pos < end) {
            ...
            dout(10) << "  safe_splice read from " << pos << "~" << l << " got "
                << r << dendl;
            if (r < 0) {
                derr << __FUNC__ << ": safe_splice read error at " << pos << "~"
                    << len << ", " << cpp_strerror(r) << dendl;
                break;
            }
            if (r == 0) {
...
#endif
    {
        ...
        if (actual != (int64_t) srcoff) {
            if (actual < 0)
                ...
                r = -EINVAL;
            derr << "lseek64 to " << srcoff << " got " << cpp_strerror(r) <<
                dendl;
            return r;
        }
        ...
        if (actual != (int64_t) dstoff) {
            if (actual < 0)
                ...
                r = -EINVAL;
            derr << "lseek64 to " << dstoff << " got " << cpp_strerror(r) <<
                dendl;
            return r;
        }
...
void FileStore::sync_entry()
{
    ...
    while (!stop) {
        ...
        if (!force_sync) {
            dout(20) << __FUNC__ << ":  waiting for max_interval " <<
                max_interval << dendl;
      ...
      again:
        ...
        if (apply_manager.commit_start()) {
            ...
            if (backend->can_checkpoint()) {
                ...
                if (err < 0) {
                    derr << "Error during write_op_seq: " << cpp_strerror(err)
                        << dendl;
                ...
                if (err < 0) {
                    int err = errno;
                    derr << "snap create '" << s << "' got error " << err <<
                        dendl;
                    assert(err == 0);
                }
                ...
                if (cid > 0) {
                    dout(20) << " waiting for checkpoint " << cid <<
                        " to complete" << dendl;
                    err = backend->sync_checkpoint(cid);
                    if (err < 0) {
                        derr << "ioctl WAIT_SYNC got " << cpp_strerror(err) <<
                            dendl;
                        assert(0 == "wait_sync got error");
                    }
                    dout(20) << " done waiting for checkpoint " << cid <<
                        " to complete" << dendl;
                }
            }
            else {
                ...
                if (err < 0) {
                    derr << "object_map sync got " << cpp_strerror(err) <<
                        dendl;
                ...
                err =::fsync(op_fd);
                if (err < 0) {
                    derr << "Error during fsync of op_seq: " <<
            ...
            dout(10) << __FUNC__ << ": commit took " << lat << ", interval was "
                << dur << dendl;
            utime_t max_pause_lat =
                logger->tget(l_filestore_sync_pause_max_lat);
...
int FileStore::snapshot(const string & name)
{
    ...
    if (!backend->can_checkpoint()) {
        dout(0) << __FUNC__ << ": " << name << " failed, not supported" <<
            dendl;
        return -EOPNOTSUPP;
    }
    ...
    if (r) {
        derr << __FUNC__ << ": " << name << " failed: " << cpp_strerror(r) <<
            dendl;
    }
...
int FileStore::getattr(const coll_t & _cid, const ghobject_t & oid,
...
int FileStore::getattrs(const coll_t & _cid, const ghobject_t & oid,
                        map < string, bufferptr > &aset)
{
    ...
    }
    {
        ...
        if (r < 0 && r != -ENOENT) {
            dout(10) << __FUNC__ << ": could not get omap_attrs r = " << r <<
                dendl;
            goto out;
        }
                         ...
                         const SequencerPosition & spos)
{
    ...
    if (spill_out && !omap_remove.empty()) {
        ...
        if (r < 0 && r != -ENOENT) {
            dout(10) << __FUNC__ << ": could not remove_xattrs r = " << r <<
                dendl;
            assert(!m_filestore_fail_eio || r != -EIO);
            goto out_close;
        }
                        ...
                        const SequencerPosition & spos)
{
    ...
    if (r >= 0) {
        for (map < string, bufferptr >::iterator p = aset.begin();
            ...
            r = chain_fremovexattr(**fd, n);
            if (r < 0) {
                dout(10) << __FUNC__ << ": could not remove xattr r = " << r <<
                    dendl;
                goto out_close;
            }
        }
...
int FileStore::list_collections(vector < coll_t > &ls, bool include_temp)
{
    ...
    while ((de =::readdir(dir))) {
        if (de->d_type == DT_UNKNOWN) {
            ...
            if (r < 0) {
                r = -errno;
                derr << "stat on " << filename << ": " << cpp_strerror(-r) <<
                    dendl;
                assert(!m_filestore_fail_eio || r != -EIO);
                break;
            }
        ...
        if (!cid.parse(de->d_name)) {
            derr << "ignoring invalid collection '" << de->
                d_name << "'" << dendl;
            continue;
        }
                               ...
                               vector < ghobject_t > *ls, ghobject_t * next)
{
    ...
    if (!c.is_temp() && !c.is_meta()) {
        if (start < sep) {
            ...
            start = sep;
            dout(10) << __FUNC__ <<
                ": fall through to non-temp collection, start " << start <<
                dendl;
        }
        else {
            dout(10) << __FUNC__ << ": start " << start << " >= sep " << sep <<
                dendl;
        }
    }
...
int FileStore::omap_get_header(const coll_t & _c,
                               const ghobject_t & hoid,
...
int FileStore::omap_get_keys(const coll_t & _c, const ghobject_t & hoid,
                             set < string > *keys)
{
...
int FileStore::_collection_hint_expected_num_objs(const coll_t & c,
                                                  uint32_t pg_num,
                                                  ...
                                                  spos)
{
    dout(15) << __FUNC__ << ": collection: " << c << " pg number: "
        << pg_num << " expected number of objects: " << expected_num_objs <<
        dendl;
    ...
    if (!empty && !replaying) {
        dout(0) <<
            ...
            << c << ", only empty collection can take such type of hint. " <<
            dendl;
        return 0;
    }
...
int FileStore::_create_collection(const coll_t & c,
                                  int bits, const SequencerPosition & spos)
{
    char fn[PATH_MAX];
    get_cdir(c, fn, sizeof(fn));
    dout(15) << __FUNC__ << ": " << fn << dendl;
                                       ...
                                       bool allow_enoent)
{
...

    {
        ...
        if (r < 0) {
            ...
            if (replaying) {
                dout(10) << __FUNC__ << ": " << c << "/" << o << " from "
                    << oldcid << "/" << oldoid << " (dne, continue replay) " <<
                    dendl;
            }
                             ...
                             const SequencerPosition & spos)
{
    ...
    }
    {
        ...
        r = lfn_find(hoid, index);
        if (r < 0) {
            dout(20) << __FUNC__ << ": lfn_find got " << cpp_strerror(r) <<
                dendl;
            return r;
        }
    }
                               ...
                               uint64_t expected_write_size)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << oid << " object_size " <<
        expected_object_size << " write_size " << expected_write_size << dendl;
...

    {
        ...
        uint64_t hint =
            MIN(expected_write_size, m_filestore_max_alloc_hint_size);
...
const char **FileStore::get_tracked_conf_keys() const const
{
                                   ...
                                   const std::set < std::string > &changed)
{
    ...
    if (changed.count("filestore_min_sync_interval") ||
        ...
        m_filestore_sloppy_crc_block_size =
            conf->filestore_sloppy_crc_block_size;
...
int FileStore::set_throttle_params()
{
    stringstream ss;
    bool valid =
        throttle_bytes.set_params(cct->_conf->filestore_queue_low_threshhold,
    ...
    valid &= throttle_ops.set_params(cct->_conf->filestore_queue_low_threshhold,
                                     cct->_conf->
                                     filestore_queue_high_threshhold,
                                     cct->_conf->
                                     filestore_expected_throughput_ops,
    ...
    if (!valid) {
        derr << "tried to set invalid params: " << ss.str()
            << dendl;
    }
                                  ...
                                  uint64_t seq, OpSequencer * osr)
{
    ...
    for (vector < ObjectStore::Transaction >::iterator i = ls.begin();
         i != ls.end(); ++i, ++trans_num) {
...
void FileStore::set_xattr_limits_via_conf()
{
    ...
    case XFS_SUPER_MAGIC:
        ...
        fs_xattr_max_value_size =
            cct->_conf->filestore_max_xattr_value_size_xfs;
        break;
    case BTRFS_SUPER_MAGIC:
...
void FSSuperblock::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 1, bl);
...
void FSSuperblock::dump(Formatter * f) const const
{
