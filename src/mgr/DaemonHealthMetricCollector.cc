...
class SlowOps final: public DaemonHealthMetricCollector
{
    ...
    void _summarize(health_check_t &check) const override
    {
        ...
        // Note this message format is used in mgr/prometheus, so any change in format// requires a corresponding change in the mgr/prometheus module.
        ostringstream ss;
        if (daemons.size() > 1) {
            if (daemons.size() > 10) {
                ss << "daemons " << vector < DaemonKey > (daemons.begin(),
                        daemons.begin() +
                        10)
                   << "..." << " have slow ops.";
            } else {
...
class PendingPGs final: public DaemonHealthMetricCollector
{
    ...
    void _summarize(health_check_t &check) const override
    {
        ...
        check.summary =
            fmt::format("{} PGs pending on creation", value.n);
        ostringstream ss;
        if (osds.size() > 1) {
