...
// zone oids
    ...
    int RadosConfigStore::write_default_zone_id(const DoutPrefixProvider * dpp,
        ...
        const auto default_oid =
            default_zone_oid(dpp->get_cct()->_conf, realm_id);
        const auto create = exclusive ? Create::MustNotExist : Create::MayExist;

        RGWDefaultSystemMetaObjInfo default_info;
        default_info.default_id = zone_id;

        return impl->write(dpp, y, pool, default_oid, create, default_info,
                           nullptr);
    }
    ...
    int RadosConfigStore::create_zone(const DoutPrefixProvider * dpp,
        ...
        if (writer) {
            *writer =
                std::make_unique < RadosZoneWriter > (impl.get(),
                                                      std::move(objv),
                                                      info.get_id(),
                                                      info.get_name());
        }
