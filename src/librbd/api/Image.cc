...
namespace librbd {
    namespace api {
        ...
        namespace {
            ...
            template < typename I >
                int pre_remove_image(librados::IoCtx & io_ctx,
                ...
                C_SaferCond ctx;
                auto req =
                    image::PreRemoveRequest < I >::create(image_ctx, false,
                                                          &ctx);
                req->send();
        ...
        template < typename I > int64_t Image < I >::get_data_pool_id(I * ictx) {
            ...
            int64_t pool_id;
            int r =
                cls_client::get_data_pool(&ictx->md_ctx, ictx->header_oid,
                                          &pool_id);
            if (r < 0) {
                CephContext *cct = ictx->cct;
                lderr(cct) << "error getting data pool ID: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
        ...
        template < typename I >
            ...
            if (io_ctx.get_namespace().empty()) {
                ...
                if (r == -ENOENT) {
                    ...
                    lderr(cct) << "error listing v1 images: " << cpp_strerror(r)
                        << dendl;
                    return r;
                }
        ...
        template < typename I >
            int Image < I >::list_images_v2(librados::IoCtx & io_ctx,
            ...
            do {
                ...
                r = cls_client::dir_list(&io_ctx, RBD_DIRECTORY, last_read,
                                         max_read, &images_page);
                if (r < 0 && r != -ENOENT) {
                ...
                for (map < string, string >::const_iterator it =
                     images_page.begin(); it != images_page.end(); ++it) {
                    images->insert(*it);
                }
        ...
        template < typename I >
            ...
            std::shared_lock parent_image_locker {
            parent->image_lock};
            ...
            if (parent->snap_id != CEPH_NOSNAP) {
                ...
                if (snap_info == nullptr) {
                    lderr(cct) << "error finding parent snap name: " <<
                        cpp_strerror(r)
                        << dendl;
                    return -ENOENT;
                }
                ...
                parent_snap->namespace_type =
                    ...
                    (cls::rbd::
                     get_snap_namespace_type(snap_info->snap_namespace));
            ...
            if (r == -ENOENT || r == -EOPNOTSUPP) {
                ...
                lderr(cct) << "error looking up trash status: " <<
                    cpp_strerror(r)
                    << dendl;
                return r;
            }
        ...
        template < typename I >
            int Image < I >::list_descendants(librados::IoCtx & io_ctx,
                                              const std::string & image_id,
        ...
        template < typename I >
            int Image < I >::list_descendants(I * ictx,
                                              const std::optional < size_t >
                                              &max_level,
                                              std::vector <
                                              librbd::linked_image_spec_t >
          ...
          for (auto snap_id:snap_ids) {
                ...
                ictx->md_ctx.get_id(),
                        ictx->md_ctx.get_namespace(), ictx->id, snap_id};
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(cct) << "error listing pools: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
          ...
          for (auto & it:pools) {
                ...
                if (r == -ENOENT) {
                    ldout(cct,
                          1) << "pool " << it.
                        second << " no longer exists" << dendl;
                    continue;
                }
                else if (r < 0) {
                    lderr(cct) << "error retrieving base tier for pool " << it.
                        second << dendl;
                    return r;
                }
              ...
              for (auto & image_id:image_ids) {
                    images->push_back( {
                                      it.first, "",
                                      ictx->md_ctx.get_namespace(), image_id,
                                      "", false}
                    );
                    r = list_descendants(ioctx, image_id, child_max_level,
                                         images);
                    if (r < 0) {
                        return r;
                    }
            ...
            IoCtx parent_io_ctx;
            r = librbd::util::create_ioctx(ictx->md_ctx, "parent image",
                                           parent_spec.pool_id,
            ...
            if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
                lderr(cct) << "error retrieving children: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
          ...
          for (auto & child_image:child_images) {
                ...
                if (!child_max_level || *child_max_level > 0) {
                    ...
                    r = list_descendants(ioctx, child_image.image_id,
                                         child_max_level, images);
                    if (r < 0) {
                        return r;
                    }
                }
          ...
          for (auto & image:*images) {
                if (child_pool_id == -1 || child_pool_id != image.pool_id ||
                    ...
                    if (r < 0) {
                        lderr(cct) << "error listing v2 images: " <<
                            cpp_strerror(r) << dendl;
                        return r;
                    }
                  ...
                  for (auto &[name, id]:image_names_to_ids) {
                    ...
                    if (r < 0 && r != -EOPNOTSUPP) {
                        lderr(cct) << "error listing trash images: " <<
                            cpp_strerror(r)
                            << dendl;
                        return r;
                    }
        ...
        template < typename I >
            ...
            if (format == 1) {
                lderr(cct) << "old format not supported for destination image"
                    << dendl;
                return -EINVAL;
            }
            ...
            if (parent_spec.pool_id == -1) {
                r = create(dest_md_ctx, destname, "", src_size, opts, "", "",
                           false);
            }
            else {
                librados::IoCtx parent_io_ctx;
                r = librbd::util::create_ioctx(src->md_ctx, "parent image",
                                               parent_spec.pool_id,
                                               parent_spec.pool_namespace,
                                               &parent_io_ctx);
                if (r < 0) {
        ...
        template < typename I >
            ...
            C_SaferCond flush_ctx;
            {
                ...
                src->owner_lock};
                auto aio_comp =
                    io::AioCompletion::create_and_start(&flush_ctx, src,
                                                        io::AIO_TYPE_FLUSH);
                auto req =
                    io::ImageDispatchSpec::create_flush(*src,
                                                        io::
            ...
            C_SaferCond cond;
            SnapSeqs snap_seqs;
            deep_copy::ProgressHandler progress_handler {
            &prog_ctx};
            auto req =
                DeepCopyRequest < I >::create(src, dest, snap_id_start,
                                              snap_id_end, 0U, flatten,
                                              boost::none,
                                              asio_engine.get_work_queue(),
                                              &snap_seqs, &progress_handler,
                                              &cond);
            req->send();
            r = cond.wait();
            if (r < 0) {
        ...
        template < typename I >
            ...
            ldout(ictx->cct,
                  20) << "snap_set " << ictx << " snap = " << (snap_name ?
                                                               snap_name :
                                                               "NULL") << dendl;
        ...
        template < typename I >
            ...
            if (r < 0) {
                if (r != -ENOENT) {
                    lderr(ictx->cct) << "failed to " << (snap_id ==
                                                         CEPH_NOSNAP ? "un" :
                                                         "")
        ...
        template < typename I >
            int Image < I >::remove(IoCtx & io_ctx,
            ...
            if (r == -ENOENT) {
              ...
              for (auto & entry:trash_entries) {
                    if (entry.name == image_name &&
                        ...
                        if (r < 0) {
                            lderr(cct) << "error getting image id " << entry.id
                                << " info from trash: " << cpp_strerror(r) <<
                                dendl;
                            return r;
                        }
                        if (spec.state == cls::rbd::TRASH_IMAGE_STATE_MOVING) {
                            r = Trash < I >::move(io_ctx, entry.source,
                                                  entry.name, entry.id, 0);
                            if (r < 0) {
                                return r;
                            }
                        }
                        return Trash < I >::remove(io_ctx, entry.id, true,
                                                   prog_ctx);
                    }
                }
                ...
                lderr(cct) << "failed to retrieve image id: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
            else {
                ...
                if (config.get_val < bool > ("rbd_move_to_trash_on_remove")) {
                    ...
                    if (r == -ECHILD) {
                        if (config.get_val < bool >
                            ...
                            lderr(cct) << "image has snapshots - not removing"
                                << dendl;
                            return -ENOTEMPTY;
                        }
                    }
            ...
            C_SaferCond cond;
            auto req =
                librbd::image::RemoveRequest < I >::create(io_ctx, image_name,
                                                           "", false, false,
                                                           prog_ctx,
                                                           asio_engine.
                                                           get_work_queue(),
        ...
        template < typename I >
            ...
            ictx->image_lock};
            snap_t snap_id =
                ictx->get_snap_id(cls::rbd::UserSnapshotNamespace(),
                                  snap_name);
          ...
          for (auto & child_image:child_images) {
                ...
                ImageCtx *imctx =
                    new ImageCtx("", child_image.image_id, nullptr,
                                 child_io_ctx, false);
                r = imctx->state->open(0);
                if (r < 0) {
                    lderr(cct) << "error opening image: " << cpp_strerror(r) <<
                        dendl;
                    return r;
                }
        ...
        template < typename I >
            ...
            C_SaferCond cond;
            auto req =
                librbd::crypto::FormatRequest < I >::create(ictx,
                                                            std::unique_ptr <
                                                            crypto::
                                                            EncryptionFormat <
                                                            I >>
        ...
        template < typename I >
            int Image < I >::encryption_load(I * ictx,
                                             const encryption_spec_t * specs,
                                             size_t spec_count, bool c_api) {
            std::vector < std::unique_ptr < crypto::EncryptionFormat <
                I >>> formats;

            for (size_t i = 0; i < spec_count; ++i) {
                crypto::EncryptionFormat < I > *result_format;
                auto r =
                    util::create_encryption_format(ictx->cct, specs[i].format,
                                                   specs[i].opts,
                                                   specs[i].opts_size,
                                                   c_api, &result_format);
                if (r != 0) {
            ...
            C_SaferCond cond;
            auto req =
                librbd::crypto::LoadRequest < I >::create(ictx,
                                                          std::move(formats),
                                                          &cond);
            req->send();
            return cond.wait();
