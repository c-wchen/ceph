...
void TempURLApplier::modify_request_state(const DoutPrefixProvider *dpp, req_state *s) const        /* in/out */
{
    ...
    if (inline_exists) {
        ...
        s->content_disp.override =
            "attachment; filename=\"" + fenc + "\"";
    } else {
        std::string fenc;
        url_encode(s->object->get_name(), fenc);
        s->content_disp.fallback =
            "attachment; filename=\"" + fenc + "\"";
    }
    ldpp_dout(dpp,
              20) <<
                  "finished applying changes to req_state for TempURL: " <<
                  " content_disp override " << s->content_disp.
                  override << " content_disp fallback " << s->content_disp.
                  fallback << dendl;
...
/* TempURL: engine */
bool TempURLEngine::is_applicable(const req_state *
                                  const
                                  s) const noexcept
{
    return s->info.args.exists("temp_url_sig") ||
           s->info.args.exists("temp_url_expires");
} void TempURLEngine::get_owner_info(const DoutPrefixProvider *dpp,
                                     ...
                                     optional_yield y) const
{
    ...
    int ret =
        driver->get_bucket(dpp, nullptr, b, &bucket, s->yield);
    if (ret < 0) {
        throw ret;
    }
...
bool TempURLEngine::is_expired(const std::string &expires)const
{
    ...
    const uint64_t expiration =
        (uint64_t) strict_strtoll(expires.c_str(),
                                  10, &err);
    if (!err.empty()) {
        dout(5) << "failed to parse temp_url_expires: " << err <<
                dendl;
        return true;
    }
    if (expiration <= (uint64_t) now.sec()) {
        dout(5) << "temp url expired: " << expiration << " <= " <<
                now.sec() << dendl;
        return true;
    }
...
bool TempURLEngine::
is_disallowed_header_present(const req_info &info)const
{
...
std::string extract_swift_subuser(const std::
                                  string &swift_user_name)
{
...
public:
    ...
    } bool is_equal_to(const std::string &rhs)const
    {
        ...
        return rhs.compare(0 /* pos */, output_size,
                           dest_str) == 0;
    }
...
class TempURLEngine::
    PrefixableSignatureHelper: private TempURLEngine::
    SignatureHelper
{
...
public:
    PrefixableSignatureHelper(const std::string &_decoded_uri,
        ...
        : decoded_uri(_decoded_uri), object_name(object_name),
          prefix(prefix)
    {
                       ...
                       const std::string &expires)
    {
        if (!prefix) {
            ...
            return base_t::calc(key, method, prefixed_path,
                                expires);
        }
    }
    ...
    bool is_equal_to(const std::string &rhs)const
    {
        ...
        if (prefix && is_auth_ok) {
            const auto prefix_uri =
                string_cat_reserve(no_obj_uri, *prefix);
            is_auth_ok =
...
TempURLEngine::result_t TempURLEngine::authenticate(const DoutPrefixProvider *dpp,
        const req_state *const s,
        optional_yield y)const
{
    if (!is_applicable(s)) {
    ...
    const std::string &temp_url_sig =
        s->info.args.get("temp_url_sig");
    ...
    try {
        ...
        ldpp_dout(dpp,
                  5) << "cannot get user_info of account's owner" <<
                     dendl;
    ...
    if (owner_info.temp_url_keys.empty()) {
        ldpp_dout(dpp,
                  5) <<
                     "user does not have temp url key set, aborting" <<
                     dendl;
        return result_t::reject();
    }
    ...
    if (is_disallowed_header_present(s->info)) {
        ldout(cct,
              5) << "temp url rejected due to disallowed header" <<
                 dendl;
    ...
    const size_t pos =
        g_conf()->rgw_swift_url_prefix.find_last_not_of('/') + 1;
    ...
    boost::container::static_vector < std::string_view,
          3 > allowed_methods;
    ...
    PrefixableSignatureHelper sig_helper {
        s->decoded_uri, s->object->get_name(), temp_url_prefix};
        ...
        for (const auto &path : allowed_paths) {
            for (const auto &method : allowed_methods) {
                const char *const local_sig =
                    sig_helper.calc(temp_url_key, method,
                                    path, temp_url_expires);

                ldpp_dout(dpp,
                          20) << "temp url signature [" <<
                              temp_url_key_num << "] (calculated): " <<
                              local_sig << dendl;

                if (sig_helper.is_equal_to(temp_url_sig)) {
                    auto apl =
                        apl_factory->create_apl_turl(cct, s,
                                                     owner_info);
                    return result_t::grant(std::move(apl));
                } else {
                    ldpp_dout(dpp,
                              5) << "temp url signature mismatch: "
                                 << local_sig << " != " << temp_url_sig <<
                                 dendl;
                }
            }
        }
...
/* External token */
bool ExternalTokenEngine::is_applicable(const std::
                                        string &token)const
noexcept
{
    if (token.empty()) {
...
ExternalTokenEngine::result_t ExternalTokenEngine::authenticate(const DoutPrefixProvider *
        dpp,
        ...
        optional_yield y)const
{
    ...
    RGWHTTPHeadersCollector validator(cct, "GET", url_buf, {
        "X-Auth-Groups",
        "X-Auth-Ttl"
    });
    ...
    std::unique_ptr < rgw::sal::User > user;
    ret =
        driver->get_user_by_swift(dpp, swift_user, s->yield, &user);
    if (ret < 0) {
        ldpp_dout(dpp,
    ...
    auto apl =
        apl_factory->create_apl_local(cct, s, user->get_info(),
                                      extract_swift_subuser
                                      (swift_user),
                                      std::nullopt,
                       ...
                       bufferlist &bl)
{
    ...
    calc_hmac_sha1(k, sizeof(k), bl.c_str(), bl.length(),
                   p.c_str());
...
static int encode_token(CephContext *cct, string &swift_user,
                        string &key, bufferlist &bl)
{
    const auto nonce =
...
bool SignedTokenEngine::is_applicable(const std::
...
SignedTokenEngine::result_t SignedTokenEngine::authenticate(const DoutPrefixProvider *dpp,
        const std::string &token,
        const req_state *const s)const
{
    ...
    const std::string etoken =
        token.substr(strlen("AUTH_rgwtk"));
    const size_t etoken_len = etoken.length();

    if (etoken_len & 1) {
        ldpp_dout(dpp,
                  0) <<
                     "NOTICE: failed to verify token: odd token length=" <<
                     etoken_len << dendl;
        throw - EINVAL;
    }
    ...
    if (expiration < now) {
        ldpp_dout(dpp,
                  0) << "NOTICE: old timed out token was used now="
                     << now << " token.expiration=" << expiration << dendl;
        return result_t::deny(-EPERM);
    }
    ...
    ret =
        build_token(swift_user, swift_key.key, nonce, expiration,
                    local_tok_bl);
    if (ret < 0) {
        throw ret;
    }
    ...
    if (memcmp(local_tok_bl.c_str(), tok_bl.c_str(),
        ...
        buf_to_hex(reinterpret_cast <
                   const unsigned char * >(local_tok_bl.c_str()),
...
void RGW_SWIFT_Auth_Get::execute(optional_yield y)
{
    ...
    if (swift_url.size() == 0) {
        ...
        if (!host) {
            dout(0) <<
                    "NOTICE: server is misconfigured, missing rgw_swift_url_prefix or rgw_swift_url, HTTP_HOST is not set"
                    << dendl;
            ret = -EINVAL;
            goto done;
        }
    ...
    if (swift_key->key.compare(key) != 0) {
        dout(0) << "NOTICE: RGW_SWIFT_Auth_Get::execute(): bad swift key" <<
                dendl;
        ret = -EPERM;
        goto done;
    }
