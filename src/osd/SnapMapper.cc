...
tl::expected < std::set < snapid_t >, Scrub::SnapMapReaderI::result_t > SnapMapper::get_snaps(
    const hobject_t &oid) const const
{
    auto snaps = get_snaps_common(oid);
    if (snaps) {
        return snaps->snaps;
...
tl::expected < SnapMapper::object_snaps, Scrub::SnapMapReaderI::result_t > SnapMapper::get_snaps_common(
    const hobject_t  &oid) constconst {
...
tl::expected < std::set < snapid_t >, result_t > SnapMapper::get_snaps_check_consistency(
    const hobject_t &hoid) constconst {
