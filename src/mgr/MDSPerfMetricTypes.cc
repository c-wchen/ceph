...
void MDSPerformanceCounterDescriptor::pack_counter(const PerformanceCounter & c,
                                                   bufferlist * bl) const const
{
    using ceph::encode;
    encode(c.first, *bl);
    encode(c.second, *bl);
...
void MDSPerformanceCounterDescriptor::unpack_counter(bufferlist::
                                                     const_iterator & bl,
                                                     PerformanceCounter *
                                                     c) const const
{
...
{
    return os << "[order_by=" << limit.order_by << ", max_count=" << limit.
        max_count << "]";
}
