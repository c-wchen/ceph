 ...
 */
   ...
   */
        ...
        base_iertr::future < LogicalCachedExtentRef >
            read_pin_by_type(Transaction & t, LBAMappingRef pin,
                             extent_types_t type) {
            auto v = pin->get_logical_extent(t);
            if (v.has_child()) {
   ...
   */
        ...
        template < typename T > alloc_extent_ret < T >
            alloc_extent(Transaction & t, laddr_t laddr_hint, extent_len_t len,
            ...
            return cache->get_root(t).
                si_then([this, &t, &key, &value] (RootBlockRef root) {
                        ...
                        cache->duplicate_for_write(t,
                                                   root)->cast < RootBlock > ();
                        auto meta = root->root.get_meta(); meta[key] = value;
                        root->root.set_meta(meta); return seastar::now();}
            );
        }
