...
namespace crimson::osd {
    ...
    PG::PG(spg_t pgid,
           ...
           std::string && name,
           cached_map_t osdmap,
           ShardServices & shard_services, ec_profile_t profile)
  :    pgid {
    pgid}
    , pg_whoami {
    pg_shard}
    ...
    , osdmap_gate("PG::osdmap_gate"), shard_services {
    shard_services}
    , backend(PGBackend::create(pgid.pgid,
                                pg_shard,
                                pool,
                                coll_ref,
                                shard_services,
                                profile,
                                *this)),
        recovery_backend(std::make_unique < ReplicatedRecoveryBackend >
                         (*this, shard_services, coll_ref, backend.get())),
        recovery_handler(std::make_unique < PGRecovery > (this)),
        peering_state(shard_services.get_cct(), pg_shard, pgid,
                      PGPool(osdmap, pgid.pool(), pool, name), osdmap, this,
                      this), obc_registry {
    local_conf()}
    , obc_loader {
    obc_registry, *backend.get(), *this}
    , osdriver(&shard_services.get_store(),
               coll_ref,
               pgid.make_pgmeta_oid()),
        snap_mapper(this->shard_services.get_cct(),
                    &osdriver,
                    pgid.ps(),
                    pgid.get_split_bits(pool.get_pg_num()),
                    pgid.pool(), pgid.shard), wait_for_active_blocker(this) {
        peering_state.set_backend_predicates(new ReadablePredicate(pg_whoami),
    ...
    void PG::check_blocklisted_watchers() {
        ...
        obc_registry.for_each([this] (ObjectContextRef obc) {
          assert(obc); for (const auto &[key, watch]:obc->watchers)
                              {
                              ...
                              logger().
                              info("watch: Found blocklisted watcher for {}",
                                   ea); watch->do_watch_timeout();}
                              }
                              }
        ) ;
    }
    ...
    bool PG::try_flush_or_schedule_async() {
        ...
        (void)shard_services.get_store().flush(coll_ref).then([this, epoch =
                                                               get_osdmap_epoch
                                                               ()]() {
                                                              return
                                                              shard_services.
    ...
    void PG::publish_stats_to_osd() {
        if (!is_primary())
            return;
        if (auto new_pg_stats =
            peering_state.prepare_stats_for_publish(pg_stats,
    ...
    void PG::queue_check_readable(epoch_t last_peering_reset,
        ...
        logger().debug("{}: PG::queue_check_readable lpr: {}, delay: {}",
        ...
        check_readable_timer.set_callback([last_peering_reset, this] {
                                          ...
                                          (void)shard_services.start_operation <
                                          LocalPeeringEvent > (this, pg_whoami,
                                                               pgid,
                                                               last_peering_reset,
                                                               last_peering_reset,
                                                               PeeringState::
        ...
        check_readable_timer.arm(std::chrono::duration_cast <
                                 seastar::lowres_clock::duration > (delay));
    }
    ...
    void PG::recheck_readable() {
        ...
        if (peering_state.state_test(PG_STATE_LAGGY)) {
            ...
            if (readable_until == readable_until.zero()) {
                logger().
                    info("{}:{} still laggy (mnow {}, readable_until zero)",
                         *this, __func__, mnow);
            }
            else if (mnow >= readable_until) {
                logger().
                    info("{}:{} still laggy (mnow {} >= readable_until {})",
                         *this, __func__, mnow, readable_until);
            }
            else {
                logger().
                    info("{}:{} no longer laggy (mnow {} < readable_until {})",
                         *this, __func__, mnow, readable_until);
    ...
    unsigned PG::get_target_pg_log_entries() const {
        ...
        const unsigned local_target =
            ...
            ("osd_target_pg_log_entries_per_osd") / seastar::smp::count;
        ...
        if (local_num_pgs > 0 && local_target > 0) {
            ...
            return std::clamp(local_target / local_num_pgs,
                              min_pg_log_entries, max_pg_log_entries);
        }
        else {
    ...
    void PG::prepare_write(pg_info_t & info,
                           ...
                           bool dirty_info,
                           bool dirty_big_info,
                           bool need_write_epoch, ceph::os::Transaction & t) {
        ...
        pglog.write_log_and_missing(t, &km, coll_ref->get_cid(), pgmeta_oid,
                                    peering_state.get_pgpool().info.
                                    require_rollback());
        if (!km.empty()) {
    ...
    std::pair < ghobject_t, bool >
        ...
        auto fut =
            interruptor::make_interruptible(shard_services.get_store().
                                            list_objects(coll_ref,
                                                         _next,
                                                         ghobject_t::get_max(),
                                                         local_conf()->
        ...
        if (objs_to_rm.empty()) {
            ...
            (void)shard_services.get_store().do_transaction(coll_ref,
                                                            std::move(t)).
                then([this] {
    ...
    void PG::on_active_actmap() {
        ...
        std::ignore = seastar::do_until([this] {
                                        return snap_trimq.empty()
                                        || peering_state.
                                        ...
                                        publish_stats_to_osd();
                                        const auto to_trim =
                                        snap_trimq.range_start();
                                        snap_trimq.erase(to_trim);
                                        const auto needs_pause =
                                        !snap_trimq.empty();
                                        return seastar::
                                        repeat([to_trim, needs_pause, this] {
                                               logger().
                                               debug
                                               ("{}: going to start SnapTrimEvent, to_trim={}",
                                                *this, to_trim);
                                               return shard_services.
                                               start_operation < SnapTrimEvent >
                                               (this, snap_mapper, to_trim,
                                                needs_pause).second.
                                               handle_error(crimson::ct_error::
                                                            enoent::
                                                            handle([this] {
                                                                   logger().
                                                                   error
                                                                   ("{}: ENOENT saw, trimming stopped",
                                                                    *this);
                                                                   peering_state.
                                                                   state_set
    ...
    void PG::scrub_requested(scrub_level_t scrub_level, scrub_type_t scrub_type) {
        ...
        peering_state.
            update_stats([scrub_level, this] (auto & history, auto & stats) {
    ...
    void PG::log_state_exit(const char *state_name, utime_t enter_time,
        ...
        logger().info("Exiting state: {}, entered at {}, {} spent on {} events",
                      state_name, enter_time, event_dur, events);
    }
    ...
    void PG::schedule_renew_lease(epoch_t last_peering_reset,
                                  ceph::timespan delay) {
        // handle the peering event in the background
        ...
        renew_lease_timer.arm(std::chrono::duration_cast <
                              seastar::lowres_clock::duration > (delay));
    }
    ...
    void PG::init(int role,
        ...
        peering_state.init(role, newup, new_up_primary, newacting,
                           new_acting_primary, history, pi, t);
    }

    seastar::future <> PG::read_state(crimson::os::FuturizedStore::Shard *
                                      store) {
        if (__builtin_expect(stopping, false)) {
            return seastar::make_exception_future <> (crimson::common::
        ...
        return seastar::do_with(PGMeta(*store, pgid),[](auto & pg_meta) {
               ...
               return peering_state.init_from_disk_state(std::move(pg_info),
                                                         std::
                                                         move(past_intervals),
                                                         [this,
                                                          store] (PGLog & pglog)
               ...
               peering_state.get_osdmap()->pg_to_up_acting_osds(pgid.pgid, &up,
                                                                &up_primary,
                                                                &acting,
                                                                &primary);
               peering_state.init_primary_up_acting(up, acting, up_primary,
                                                    primary);
               int rr = OSDMap::calc_pg_role(pg_whoami, acting);
               peering_state.set_role(rr); epoch_t epoch = get_osdmap_epoch();
               (void)shard_services.start_operation < LocalPeeringEvent > (this,
                                                                           pg_whoami,
                                                                           pgid,
                                                                           epoch,
    ...
    PG::interruptible_future <> PG::do_peering_event(PGPeeringEvent & evt,
                                                     PeeringCtx & rctx) {
        if (peering_state.pg_has_reset_since(evt.get_epoch_requested())
            || peering_state.pg_has_reset_since(evt.get_epoch_sent())) {
            logger().debug("{} ignoring {} -- pg has reset", __func__,
                           evt.get_desc());
            return interruptor::now();
        }
        else {
            logger().debug("{} handling {} for pg: {}", __func__,
                           evt.get_desc(), pgid);
            ...
            return interruptor::async([this, &evt, &rctx] {
                                      peering_state.handle_event(evt.
                                                                 get_event(),
                                                                 &rctx);
                                      peering_state.write_if_dirty(rctx.
                                                                   transaction);}
            );
        }
    }
    ...
    seastar::future <> PG::handle_advance_map(cached_map_t next_map,
        ...
        return seastar::async([this, next_map = std::move(next_map), &rctx] {
                              ...
                              next_map->pg_to_up_acting_osds(pgid.pgid,
                                                             &newup,
                                                             &up_primary,
                                                             &newacting,
                                                             &acting_primary);
                              peering_state.advance_map(next_map,
                                                        peering_state.
                                                        get_osdmap(), newup,
                                                        up_primary, newacting,
                                                        acting_primary, rctx);
    ...
    seastar::future <> PG::handle_initialize(PeeringCtx & rctx) {
        return seastar::async([this, &rctx] {
                              peering_state.
                              handle_event(PeeringState::Initialize {
                                           }
                                           , &rctx);}
        );
    }
    ...
    std::tuple < PG::interruptible_future <>,
        ...
        PG::submit_transaction(ObjectContextRef && obc,
                               ceph::os::Transaction && txn,
                               osd_op_params_t && osd_op_p,
                               std::vector < pg_log_entry_t > &&log_entries) {
        if (__builtin_expect(stopping, false)) {
            return {
            seastar::make_exception_future <> (crimson::common::
                                                   system_shutdown_exception
                                                   ()), seastar::now()};
        }
        ...
        peering_state.pre_submit_op(obc->obs.oi.soid, log_entries,
                                    osd_op_p.at_version);
        peering_state.append_log_with_trim_to_updated(std::move(log_entries),
                                                      osd_op_p.at_version, txn,
                                                      true, false);

        auto[submitted, all_completed] =
            backend->mutate_object(peering_state.get_acting_recovery_backfill(),
                                   std::move(obc), std::move(txn),
                                   std::move(osd_op_p),
                                   peering_state.get_last_peering_reset(),
                                   map_epoch, std::move(log_entries));
        return std::make_tuple(std::move(submitted),
                               all_completed.
                               then_interruptible([this, last_complete =
                                                   peering_state.get_info().
                                                   last_complete, at_version =
                                                   osd_op_p.
                                                   at_version] (auto acked) {
    ...
    PG::interruptible_future <> PG::repair_object(const hobject_t & oid,
                                                  eversion_t & v) {
        // see also PrimaryLogPG::rep_repair_primary_object()
        assert(is_primary());
        logger().debug("{}: {} peers osd.{}", __func__, oid,
        ...
        auto[op, fut] =
            get_shard_services().start_operation < UrgentRecovery > (oid, v,
                                                                     this,
                                                                     get_shard_services
                                                                     (),
                                                                     get_osdmap_epoch
    ...
    template < class Ret, class SuccessFunc, class FailureFunc >
        ...
        PG::do_osd_ops_execute(seastar::lw_shared_ptr < OpsExecuter > ox,
        ...
        auto failure_func_ptr =
            seastar::make_lw_shared(std::move(failure_func));
        return interruptor::do_for_each(ops,[ox] (OSDOp & osd_op) {
                                  ...
                                  const auto & m = ox->get_message(); if (m.get_reqid().name.is_mds() ||    // FIXME: ignore MDS for now
                                                                          m.
                                                                          has_flag
                                  ...
                                  (seastar::now(),
                                   OpsExecuter::osd_op_ierrorator::
                                   future <> (crimson::ct_error::enospc::
                                              make()));}
                                  else {
                                  // drop request
                                  logger().
                                  info(" full, dropping request (bad client)");
                                  return interruptor::make_ready_future <
                                  OpsExecuter::rep_op_fut_tuple >
                                  (seastar::now(),
                                   OpsExecuter::osd_op_ierrorator::
                                   future <> (crimson::ct_error::eagain::
                                              make()));}
                                  }
                                  return std::move(*ox).
                                  flush_changes_n_do_ops_effects(ops,
                                                                 snap_mapper,
                                                                 osdriver,
                                                                 [this] (auto
                                                                         && txn,
            ...
            .safe_then_unpack_interruptible([success_func =
                                             std::move(success_func),
                                             rollbacker, this, failure_func_ptr]
                                            (auto submitted_fut,
                                             auto all_completed_fut) mutable {
                                            return PG::do_osd_ops_iertr::
                                            make_ready_future <
                                            pg_rep_op_fut_t <
                                            Ret >> (std::move(submitted_fut),
                                                    all_completed_fut.
                                                    ...
                                                    (std::move(success_func),
                                                     ...
                                                     handle([rollbacker,
                                                            ...
                                                            (e).
                                                            then_interruptible([obc = rollbacker.get_obc(), this] {
                                                                               return
                                                                               repair_object
                                                                               (obc->
                                                                                obs.
    ...
    seastar::future <> PG::submit_error_log(Ref < MOSDOp > m,
        ...
        peering_state.merge_new_log_entries(log_entries, t,
      ...
      for (auto & i:get_acting_recovery_backfill()) {
            ...
            auto log_m =
                crimson::make_message < MOSDPGUpdateLogMissing > (log_entries,
                                                                  spg_t
                                                                  (peering_state.
                                                                   get_info().
                                                                   pgid.pgid,
                                                                  ...
                                                                  get_min_last_complete_ondisk
                                                                  ());
            send_cluster_message(peer.osd, std::move(log_m),
                                 get_osdmap_epoch());
            waiting_on.insert(peer);
        }
        waiting_on.insert(pg_whoami);
        log_entry_update_waiting_on.insert(std::make_pair(rep_tid, log_update_t {
                                                          std::move(waiting_on)}
                                           ));
        return shard_services.get_store().do_transaction(get_collection_ref(),
                                                         std::move(t))
            .then([this] {
                  peering_state.update_trim_to(); return seastar::now();}
        );
    }

    PG::do_osd_ops_iertr::future < PG::pg_rep_op_fut_t < MURef < MOSDOpReply >>>
        PG::do_osd_ops(Ref < MOSDOp > m,
                       crimson::net::ConnectionRef conn,
                       ObjectContextRef obc,
                       const OpInfo & op_info, const SnapContext & snapc) {
        if (__builtin_expect(stopping, false)) {
            throw crimson::common::system_shutdown_exception();
        }
        return do_osd_ops_execute < MURef <
            MOSDOpReply >> (seastar::make_lw_shared < OpsExecuter >
                            (Ref < PG > {
                             this}
                             , obc, op_info, *m, conn, snapc),
                            m->ops,
                            [this, m, obc, may_write = op_info.may_write(),
                             may_read = op_info.may_read(), rvec =
                             op_info.allows_returnvec()] {
                            // TODO: should stop at the first op which returns a negative retval,
                            //       cmpext uses it for returning the index of first unmatched byte
                            int result =
                            m->ops.empty()? 0 : m->ops.back().rval.code;
                            if (may_read && result >= 0) {
              for (auto & osdop:m->ops) {
                            if (osdop.rval < 0
                                && !(osdop.op.
                                     flags & CEPH_OSD_OP_FLAG_FAILOK)) {
                            result = osdop.rval.code; break;}
                            }
                            }
                            else
                            ...
                            if (result < 0 && (m->ops.empty()?
                                               0 : m->ops.back().op.
                                               flags & CEPH_OSD_OP_FLAG_FAILOK))
                            {
                            result = 0;}
                            auto reply =
                            crimson::make_message < MOSDOpReply > (m.get(),
                                                                   result,
                                                                   get_osdmap_epoch
                                                                   (), 0,
                                                                   false);
                            ...
                            logger().
                            debug("do_osd_ops: {} - object {} sending reply",
                                  *m, m->get_hobj()); if (obc->obs.exists) {
                            reply->set_reply_versions(peering_state.get_info().
                                                      last_update,
                                                      obc->obs.oi.
                                                      user_version);}
                            else {
                            reply->set_reply_versions(peering_state.get_info().
                                                      ...
                                                      peering_state.get_info().
                                                      last_user_version);}
                            return do_osd_ops_iertr::make_ready_future < MURef <
                            MOSDOpReply >> (std::move(reply));}
                            ,[m, &op_info, obc,
                              this] (const std::error_code & e) {
                            return seastar::do_with(eversion_t(),
                                                    [m, &op_info, obc, e,
                                                     this] (auto & version) {
                                                    ...
                                                    get_osdmap_epoch();
                                                    ceph_tid_t rep_tid =
                                                    shard_services.get_tid();
                                                    auto last_complete =
                                                    peering_state.get_info().
                                                    last_complete;
                                                    if (op_info.may_write()) {
                                                    fut =
                                                    submit_error_log(m, op_info,
                                                                     obc, e,
                                                                     rep_tid,
                                                                     version);}
                                                    return fut.
                                                    then([m, e, epoch, &op_info,
                                                          rep_tid, &version,
                                                          last_complete, this] {
                                                         auto log_reply =
                                                         [m, e, this] {
                                                         ...
                                                         {
                          assert(!m->ops.empty()); for (auto & osdop:m->ops)
                                                         {
                                                         if (osdop.rval < 0) {
                                                         reply->
                                                         set_result(osdop.rval);
                                                         break;}
                                                         }
                                                         }
                                                         reply->
    ...
    PG::do_osd_ops_iertr::future < PG::pg_rep_op_fut_t <>>
        PG::do_osd_ops(ObjectContextRef obc,
                       std::vector < OSDOp > &ops,
                       const OpInfo & op_info,
        ...
        return seastar::do_with(std::move(msg_params),
                                [ =, this, &ops, &op_info] (auto & msg_params) {
                                return do_osd_ops_execute <
                                void >(seastar::make_lw_shared < OpsExecuter >
                                       (Ref < PG > {
                                        this}, std::move(obc), op_info,
                                        msg_params, msg_params.get_connection(),
                                        SnapContext {
                                        }
                                       ),
                                       ops,
    ...
    PG::interruptible_future < MURef < MOSDOpReply >> PG::do_pg_ops(Ref <
                                                                    MOSDOp >
                                                                    m) {
        if (__builtin_expect(stopping, false)) {
            throw crimson::common::system_shutdown_exception();
        ...
        return interruptor::do_for_each(m->ops,[ox = ox.get()](OSDOp & osd_op) {
                                        logger().
                                        debug("will be handling pg op {}",
                                              ceph_osd_op_name(osd_op.op.op));
                                        return ox->execute_op(osd_op);}
        ).then_interruptible([m, this, ox = std::move(ox)] {
                             auto reply =
                             crimson::make_message < MOSDOpReply > (m.get(), 0,
                                                                    get_osdmap_epoch
                                                                    (),
                                                                    CEPH_OSD_FLAG_ACK
                                                                    |
                             ...
                             reply->set_reply_versions(peering_state.get_info().
                                                       ...
                                                       peering_state.get_info().
                                                       last_user_version);
                             return seastar::make_ready_future < MURef <
                             MOSDOpReply >> (std::move(reply));}
        ).handle_exception_type_interruptible([ =,
                                               this] (const crimson::osd::
                                                      error & e) {
                                              auto reply =
                                              crimson::make_message <
                                              MOSDOpReply > (m.get(),
    ...
    void PG::check_blocklisted_obc_watchers(ObjectContextRef & obc) {
        if (obc->watchers.empty()) {
          for (auto &[src, winfo]:obc->obs.oi.watchers) {
                auto watch =
                    crimson::osd::Watch::create(obc, winfo, src.second, this);
                watch->disconnect();
                auto[it, emplaced] =
    ...
    PG::load_obc_iertr::future <>
        PG::with_locked_obc(const hobject_t & hobj,
                            const OpInfo & op_info, with_obc_func_t && f) {
        if (__builtin_expect(stopping, false)) {
            throw crimson::common::system_shutdown_exception();
        }
        ...
        case RWState::RWREAD:
            return obc_loader.with_obc < RWState::RWREAD > (oid,
                                                            std::move(wrapper));
        case RWState::RWWRITE:
            return obc_loader.with_obc < RWState::RWWRITE > (oid,
                                                             std::
                                                             move(wrapper));
        case RWState::RWEXCL:
            return obc_loader.with_obc < RWState::RWEXCL > (oid,
                                                            std::move(wrapper));
        default:
            ceph_abort();
        };
    ...
    PG::interruptible_future <> PG::handle_rep_op(Ref < MOSDRepOp > req) {
        ...
        log_operation(std::move(log_entries),
                      req->pg_trim_to,
                      req->version,
                      req->min_last_complete_ondisk, !txn.empty(), txn, false);
        logger().debug("PG::handle_rep_op: do_transaction...");
        return interruptor::make_interruptible(shard_services.get_store().
                                               do_transaction(coll_ref,
                                                              std::move(txn))).
            then_interruptible([req, lcod =
                                peering_state.get_info().last_complete, this] {
                               peering_state.update_last_complete_ondisk(lcod);
                               const auto map_epoch = get_osdmap_epoch();
                               auto reply =
                               crimson::make_message < MOSDRepOpReply >
                               (req.get(), pg_whoami, 0, map_epoch,
                                req->get_min_epoch(), CEPH_OSD_FLAG_ONDISK);
                               reply->set_last_complete_ondisk(lcod);
                               return shard_services.send_to_osd(req->from.osd,
                                                                 std::
                                                                 move(reply),
                                                                 map_epoch);}
        );
    ...
    void PG::log_operation(std::vector < pg_log_entry_t > &&logv,
    ...
    void PG::replica_clear_repop_obc(const std::vector < pg_log_entry_t > &logv) {
      ...
      for (auto && e:logv) {
            ...
            obc_registry.clear_range(e.soid.get_object_boundary(),
                                     e.soid.get_head());
        }
    }
    ...
    PG::interruptible_future <> PG::do_update_log_missing(Ref <
        ...
        peering_state.append_log_entries_update_missing(m->entries, t,
                                                        op_trim_to,
                                                        op_roll_forward_to);
        ...
        return interruptor::make_interruptible(shard_services.get_store().
                                               do_transaction(coll_ref,
                                                              std::move(t))).
            then_interruptible([m, conn, lcod =
                                peering_state.get_info().last_complete, this] {
                               if (!peering_state.
                                   pg_has_reset_since(m->get_epoch())) {
                               peering_state.update_last_complete_ondisk(lcod);
                               auto reply =
                               crimson::make_message <
                               MOSDPGUpdateLogMissingReply >
                               (spg_t
                                (peering_state.get_info().pgid.pgid,
                                 get_primary().shard), pg_whoami.shard,
                                m->get_epoch(), m->min_epoch, m->get_tid(),
                                lcod); reply->set_priority(CEPH_MSG_PRIO_HIGH);
                               return conn->send(std::move(reply));}
                               return seastar::now();}
        ) ;
    }

    PG::interruptible_future <> PG::do_update_log_missing_reply(Ref <
                                                                MOSDPGUpdateLogMissingReply
                                                                > m) {
        ...
        if (it != log_entry_update_waiting_on.end()) {
            if (it->second.waiting_on.count(m->get_from())) {
                ...
                if (m->last_complete_ondisk != eversion_t()) {
                    peering_state.update_peer_last_complete_ondisk(m->
                                                                   get_from(),
                                                                   m->
                ...
                logger().
                    error
                    ("{} : {} got reply {} from shard we are not waiting for ",
                     __func__, peering_state.get_info().pgid, *m,
                     m->get_from());
            }
    ...
    bool PG::can_discard_replica_op(const Message & m, epoch_t m_map_epoch)const {
        ...
        // Mostly, this overlaps with the old_peering_msg// condition.  An important exception is pushes// sent by replicas not in the acting set, since// if such a replica goes down it does not cause// a new interval.
    ...
    bool PG::can_discard_op(const MOSDOp & m) const {
        ...
        return __builtin_expect(m.get_map_epoch()
                                < peering_state.get_info().history.
                                same_primary_since, false);
    }
    ...
    bool PG::is_degraded_or_backfilling_object(const hobject_t & soid)const {
        ...
        for (auto & peer:get_acting_recovery_backfill()) {
            if (peer == get_primary())
                continue;
            auto peer_missing_entry =
                peering_state.get_peer_missing().find(peer);
            ...
            // Object is degraded if after last_backfill AND// we are backfilling it
    ...
    PG::interruptible_future < std::optional < PG::complete_op_t >>
        ...
        if (peering_state.get_pg_log().get_log().
            get_request(reqid, &version, &user_version, &ret, &op_returns)) {
            complete_op_t dupinfo {
            user_version, version, ret};
            ...
            return seastar::make_ready_future < std::optional <
                complete_op_t >> (std::nullopt);
        }
    }
