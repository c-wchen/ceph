...
class RGWAsyncRadosRequest:public RefCountedObject {
  ...
  public:
     RGWAsyncRadosRequest(RGWCoroutine * _caller,
...
class RGWAsyncRadosProcessor {
    ...
    struct RGWWQ:public DoutPrefixProvider,
        ...
        RGWAsyncRadosProcessor *processor;
         RGWWQ(RGWAsyncRadosProcessor * p,
               ...
               ThreadPool * tp)
        :ThreadPool::WorkQueue < RGWAsyncRadosRequest > ("RGWWQ", timeout,
                                                         suicide_timeout, tp),
            processor(p) {
  ...
  public:
    ...
    void start();
    void stop();
    void handle_request(const DoutPrefixProvider * dpp,
...
template < class P > class RGWSimpleWriteOnlyAsyncCR:public RGWSimpleCoroutine {
    ...
    class Request:public RGWAsyncRadosRequest {
      ...
      public:
         Request(RGWCoroutine * caller,
                 ...
                 rgw::sal::RadosStore * store,
                 const P & _params,
                 const DoutPrefixProvider * dpp):RGWAsyncRadosRequest(caller,
                                                                      cn),
            store(store), params(_params), dpp(dpp) {
    }} *req {
    nullptr};
  ...
  public:
    RGWSimpleWriteOnlyAsyncCR(RGWAsyncRadosProcessor * _async_rados,
                              ...
                              _dpp):RGWSimpleCoroutine(_store->ctx()),
        async_rados(_async_rados), store(_store), params(_params), dpp(_dpp) {
    }
...
template < class P, class R > class RGWSimpleAsyncCR:public RGWSimpleCoroutine {
    ...
    class Request:public RGWAsyncRadosRequest {
      ...
      public:
         Request(const DoutPrefixProvider * dpp,
                 ...
                 const DoutPrefixProvider * _dpp):RGWAsyncRadosRequest(caller,
                                                                       cn),
            store(_store), params(_params), result(_result), dpp(_dpp) {
    }} *req {
    nullptr};
  ...
  public:
    RGWSimpleAsyncCR(RGWAsyncRadosProcessor * _async_rados,
                     ...
                     const DoutPrefixProvider *
                     _dpp):RGWSimpleCoroutine(_store->ctx()),
        async_rados(_async_rados), store(_store), params(_params),
        result(_result), dpp(_dpp) {
    }
...
class RGWGenericAsyncCR:public RGWSimpleCoroutine {
  ...
  private:
    ...
    class Request:public RGWAsyncRadosRequest {
      ...
      public:
      Request(const DoutPrefixProvider * dpp, RGWCoroutine * caller, RGWAioCompletionNotifier * cn, std::shared_ptr < Action > &_action):RGWAsyncRadosRequest(caller, cn),
            action(_action)
        {
        }
    } *req {
  ...
  public:
  RGWGenericAsyncCR(CephContext * _cct, RGWAsyncRadosProcessor * _async_rados, std::shared_ptr < Action > &_action):RGWSimpleCoroutine(_cct),
        async_rados(_async_rados),
        action(_action) {
    }
  template < typename T > RGWGenericAsyncCR(CephContext * _cct, RGWAsyncRadosProcessor * _async_rados, std::shared_ptr < T > &_action):RGWSimpleCoroutine(_cct),
        async_rados(_async_rados),
    ...
    int send_request(const DoutPrefixProvider * dpp) override {
        req = new Request(dpp, this,
                          stack->create_completion_notifier(), action);
...
class RGWAsyncGetSystemObj:public RGWAsyncRadosRequest {
  ...
  public:
     RGWAsyncGetSystemObj(const DoutPrefixProvider * dpp,
                          ...
                          const rgw_raw_obj & _obj, bool want_attrs,
                          bool raw_attrs);
...
class RGWAsyncPutSystemObjAttrs:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
     RGWAsyncPutSystemObjAttrs(const DoutPrefixProvider * dpp,
                               RGWCoroutine * caller,
                               RGWAioCompletionNotifier * cn,
                               RGWSI_SysObj * _svc,
                               RGWObjVersionTracker * _objv_tracker,
                               const rgw_raw_obj & _obj, std::map < std::string,
                               bufferlist > _attrs, bool exclusive);
...
class RGWAsyncUnlockSystemObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
     RGWAsyncUnlockSystemObj(RGWCoroutine * caller,
                             RGWAioCompletionNotifier * cn,
                             rgw::sal::RadosStore * _store,
                             RGWObjVersionTracker * _objv_tracker,
...
template < class T > class RGWSimpleRadosReadCR:public RGWSimpleCoroutine {
  ...
  :    
    ...
    int send_request(const DoutPrefixProvider * dpp) {
        ...
        if (r < 0) {
            ldpp_dout(dpp,
                      -1) << "ERROR: failed to get ref for (" << obj << ") ret="
                << r << dendl;
            return r;
        }
...
class RGWSimpleRadosWriteAttrsCR:public RGWSimpleCoroutine {
  ...
  :    
    ...
    obj(std::move(obj)), attrs(std::move(attrs)), exclusive(exclusive) {
    } int send_request(const DoutPrefixProvider * dpp) override {
        int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
        if (r < 0) {
            ldpp_dout(dpp,
...
class RGWRadosGetOmapKeysCR:public RGWSimpleCoroutine {
  public:
    ...
    using ResultPtr = std::shared_ptr < Result >;
     ...
     RGWRadosGetOmapKeysCR(rgw::sal::RadosStore * _store,
                           const rgw_raw_obj & _obj,
...
class RGWRadosGetOmapValsCR:public RGWSimpleCoroutine {
  public:
    ...
    using ResultPtr = std::shared_ptr < Result >;
     ...
     RGWRadosGetOmapValsCR(rgw::sal::RadosStore * _store,
                           const rgw_raw_obj & _obj,
                           const std::string & _marker, int _max_entries,
                           ResultPtr result);
...
class RGWSimpleRadosLockCR:public RGWSimpleCoroutine {
  ...
  public:
     RGWSimpleRadosLockCR(RGWAsyncRadosProcessor * _async_rados,
                          ...
                          const std::string & _cookie, uint32_t _duration);
...
class RGWOmapAppend:public RGWConsumerCR < std::string > {
  ...
  public:
    RGWOmapAppend(RGWAsyncRadosProcessor * _async_rados,
                  rgw::sal::RadosStore * _store, const rgw_raw_obj & _obj,
...
class RGWShardedOmapCRManager {
  ...
  public:
    RGWShardedOmapCRManager(RGWAsyncRadosProcessor * _async_rados,
                            rgw::sal::RadosStore * _store, RGWCoroutine * _op,
                            int _num_shards, const rgw_pool & pool,
...
class RGWAsyncGetBucketInstanceInfo:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
    RGWAsyncGetBucketInstanceInfo(RGWCoroutine * caller,
                                  RGWAioCompletionNotifier * cn,
                                  rgw::sal::RadosStore * _store,
...
class RGWGetBucketInstanceInfoCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    RGWGetBucketInstanceInfoCR(RGWAsyncRadosProcessor * _async_rados,
                               rgw::sal::RadosStore * _store,
    ...
    :RGWSimpleCoroutine(_store->ctx()), async_rados(_async_rados),
        store(_store), bucket(_bucket), bucket_info(_bucket_info),
        pattrs(_pattrs), dpp(dpp) {
    ...
    int send_request(const DoutPrefixProvider * dpp) override {
        req =
            new RGWAsyncGetBucketInstanceInfo(this,
                                              stack->
...
class RGWPutBucketInstanceInfoCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    :RGWSimpleCoroutine(store->ctx()), async_rados(async_rados), store(store),
        bucket_info(bucket_info), exclusive(exclusive), mtime(mtime),
        attrs(attrs), dpp(dpp) {
...
class RGWAsyncFetchRemoteObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
    RGWAsyncFetchRemoteObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                           rgw::sal::RadosStore * _store,
                           ...
                           rgw_zone_set * _zones_trace, PerfCounters * counters,
                           const DoutPrefixProvider * dpp)
    :RGWAsyncRadosRequest(caller, cn), store(_store), source_zone(_source_zone),
        user_id(_user_id), src_bucket(_src_bucket),
        dest_placement_rule(_dest_placement_rule),
        dest_bucket_info(_dest_bucket_info), key(_key), dest_key(_dest_key),
        versioned_epoch(_versioned_epoch), copy_if_newer(_if_newer),
        filter(_filter), source_trace_entry(source_trace_entry),
...
class RGWFetchRemoteObjCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWFetchRemoteObjCR(RGWAsyncRadosProcessor * _async_rados,
                        rgw::sal::RadosStore * _store,
                        const rgw_zone_id & _source_zone,
                        std::optional < rgw_user > _user_id,
                        const rgw_bucket & _src_bucket,
    ...
    :RGWSimpleCoroutine(_store->ctx()), cct(_store->ctx()),
        async_rados(_async_rados), store(_store), source_zone(_source_zone),
        user_id(_user_id), src_bucket(_src_bucket),
        dest_placement_rule(_dest_placement_rule),
        dest_bucket_info(_dest_bucket_info), key(_key), dest_key(_dest_key),
        versioned_epoch(_versioned_epoch), copy_if_newer(_if_newer),
        filter(_filter), req(NULL), source_trace_entry(source_trace_entry),
    ...
    int send_request(const DoutPrefixProvider * dpp) override {
        req =
            new RGWAsyncFetchRemoteObj(this,
                                       stack->create_completion_notifier(),
                                       store, source_zone, user_id, src_bucket,
                                       dest_placement_rule, dest_bucket_info,
                                       key, dest_key, versioned_epoch,
                                       copy_if_newer, filter,
...
class RGWAsyncStatRemoteObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
    RGWAsyncStatRemoteObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                          rgw::sal::RadosStore * _store,
                          const rgw_zone_id & _source_zone,
                          rgw_bucket & _src_bucket, const rgw_obj_key & _key,
                          ceph::real_time * _pmtime, uint64_t * _psize,
                          std::string * _petag, std::map < std::string,
                          bufferlist > *_pattrs, std::map < std::string,
                          std::string > *_pheaders):RGWAsyncRadosRequest(caller,
                                                                         cn),
        store(_store), source_zone(_source_zone), src_bucket(_src_bucket),
        key(_key), pmtime(_pmtime), psize(_psize), petag(_petag),
        pattrs(_pattrs), pheaders(_pheaders) {
}};
...
class RGWStatRemoteObjCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWStatRemoteObjCR(RGWAsyncRadosProcessor * _async_rados,
                       rgw::sal::RadosStore * _store,
                       const rgw_zone_id & _source_zone,
                       rgw_bucket & _src_bucket, const rgw_obj_key & _key,
                       ceph::real_time * _pmtime, uint64_t * _psize,
                       ...
                       bufferlist > *_pattrs, std::map < std::string,
                       std::string >
                       *_pheaders):RGWSimpleCoroutine(_store->ctx()),
        cct(_store->ctx()), async_rados(_async_rados), store(_store),
        source_zone(_source_zone), src_bucket(_src_bucket), key(_key),
        pmtime(_pmtime), psize(_psize), petag(_petag), pattrs(_pattrs),
        pheaders(_pheaders), req(NULL) {
    ...
    int send_request(const DoutPrefixProvider * dpp) override {
        req =
            new RGWAsyncStatRemoteObj(this, stack->create_completion_notifier(),
                                      store, source_zone, src_bucket, key,
                                      pmtime, psize, petag, pattrs, pheaders);
        async_rados->queue(req);
        return 0;
    }
...
class RGWAsyncRemoveObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
    RGWAsyncRemoveObj(const DoutPrefixProvider * _dpp, RGWCoroutine * caller,
                      RGWAioCompletionNotifier * cn,
                      rgw::sal::RadosStore * _store,
                      const rgw_zone_id & _source_zone,
                      RGWBucketInfo & _bucket_info, const rgw_obj_key & _key,
                      const std::string & _owner,
                      const std::string & _owner_display_name, bool _versioned,
                      uint64_t _versioned_epoch, bool _delete_marker,
                      bool _if_older, real_time & _timestamp,
                      rgw_zone_set * _zones_trace):RGWAsyncRadosRequest(caller,
                                                                        cn),
        dpp(_dpp), store(_store), source_zone(_source_zone), owner(_owner),
        owner_display_name(_owner_display_name), versioned(_versioned),
        versioned_epoch(_versioned_epoch), del_if_older(_if_older),
...
class RGWRemoveObjCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWRemoveObjCR(const DoutPrefixProvider * _dpp,
                   RGWAsyncRadosProcessor * _async_rados,
                   rgw::sal::RadosStore * _store,
                   const rgw_zone_id & _source_zone,
                   RGWBucketInfo & _bucket_info, const rgw_obj_key & _key,
                   bool _versioned, uint64_t _versioned_epoch,
                   std::string * _owner, std::string * _owner_display_name,
                   bool _delete_marker, real_time * _timestamp,
                   rgw_zone_set *
                   _zones_trace):RGWSimpleCoroutine(_store->ctx()), dpp(_dpp),
        cct(_store->ctx()), async_rados(_async_rados), store(_store),
        source_zone(_source_zone), bucket_info(_bucket_info), key(_key),
        versioned(_versioned), versioned_epoch(_versioned_epoch),
    ...
    int send_request(const DoutPrefixProvider * dpp) override {
        req =
            new RGWAsyncRemoveObj(dpp, this,
                                  stack->create_completion_notifier(), store,
                                  source_zone, bucket_info, key, owner,
                                  owner_display_name, versioned,
                                  versioned_epoch, delete_marker, del_if_older,
                                  timestamp, zones_trace);
        async_rados->queue(req);
        return 0;
    }
...
class LatencyMonitor {
  ...
  public:
    ...
    void add_latency(ceph::timespan latency) {
        ...
        return count == 0 ? 0 s : total / count;
    }
};
...
class RGWRadosTimelogAddCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWRadosTimelogAddCR(const DoutPrefixProvider * dpp,
                         rgw::sal::RadosStore * _store,
                         const std::string & _oid, const cls_log_entry & entry);
...
class RGWAsyncStatObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request(const DoutPrefixProvider * dpp) override;
  public:
    RGWAsyncStatObj(const DoutPrefixProvider * dpp, RGWCoroutine * caller,
                    RGWAioCompletionNotifier * cn, rgw::sal::RadosStore * store,
                    const RGWBucketInfo & _bucket_info, const rgw_obj & obj,
                    uint64_t * psize = nullptr, real_time * pmtime =
                    nullptr, uint64_t * pepoch =
                    nullptr, RGWObjVersionTracker * objv_tracker = nullptr)
  :    RGWAsyncRadosRequest(caller, cn), dpp(dpp), store(store), obj(obj),
        psize(psize), pmtime(pmtime), pepoch(pepoch),
        objv_tracker(objv_tracker) {
}};
...
class RGWStatObjCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWStatObjCR(const DoutPrefixProvider * dpp,
                 RGWAsyncRadosProcessor * async_rados,
                 rgw::sal::RadosStore * store,
                 const RGWBucketInfo & _bucket_info, const rgw_obj & obj,
                 uint64_t * psize = nullptr, real_time * pmtime =
...
class RGWDataPostNotifyCR:public RGWCoroutine {
  ...
  public:
    RGWDataPostNotifyCR(RGWRados * _store, RGWHTTPManager & _http_manager,
                        bc::flat_map < int,
                        bc::flat_set < rgw_data_notify_entry > >&_shards,
                        const char *_zone, RGWRESTConn * _conn)
