...
int __librbd_open(const char *name, struct rbd_ctx *ctx)
{
    ...
    assert(!ctx->name && !ctx->image && !ctx->krbd_name && ctx->krbd_fd < 0);
               ...
               int stripe_count, bool krbd)
{
    ...
    if (krbd) {
        features &= ~(RBD_FEATURE_OBJECT_MAP |
                      ...
                      RBD_FEATURE_DEEP_FLATTEN | RBD_FEATURE_JOURNALING);
    }
    ret = rbd_clone2(ioctx, ctx->name, src_snapname, ioctx,
                     dst_imagename, features, order, stripe_unit, stripe_count);
    if (ret < 0) {
...
void check_clone(int clonenum, bool replay_image)
{
    ...
    ret = posix_memalign((void **)&good_buf,
                         MAX(writebdy, (int)sizeof(void *)), file_info.st_size);
    if (ret > 0) {
    ...
    ret = posix_memalign((void **)&temp_buf,
                         MAX(readbdy, (int)sizeof(void *)), file_info.st_size);
    if (ret > 0) {
