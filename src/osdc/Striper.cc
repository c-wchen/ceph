...
void Striper::file_to_extents(CephContext *cct, const char *object_format,
                              ...
                              uint64_t trunc_size,
                              map<object_t,
                                  vector<ObjectExtent>> &object_extents,
                              uint64_t buffer_offset)
{
    ldout(cct, 10) << "file_to_extents " << offset << "~" << len
                   << " format " << object_format << dendl;
    assert(len > 0);
    ...
    while (left > 0)
    {
        ...
        ldout(cct,
              20)
            << " off " << cur << " blockno " << blockno << " stripeno " << stripeno << " stripepos " << stripepos << " objectsetno " << objectsetno << " objectno " << objectno << " block_start " << block_start << " block_off " << block_off << " " << x_offset << "~"
            << x_len << dendl;
...
void Striper::assimilate_extents(map<object_t,
                                     vector<ObjectExtent>> &object_extents,
                                 vector<ObjectExtent> &extents)
{
    // make final list
    for (map<object_t, vector<ObjectExtent>>::iterator it = object_extents.begin(); it != object_extents.end(); ++it)
    {
                                                        ...
                                                        uint64_t>> &buffer_extents)
{
    ...
    for (vector<pair<uint64_t, uint64_t>>::const_iterator p = buffer_extents.begin(); p != buffer_extents.end(); ++p)
    {
                                                               ...
                                                               uint64_t>> &buffer_extents)
{
    ldout(cct,
          10)
        << "add_partial_sparse_result(" << this << ") " << bl.length()
                                                 ...
                                                 char *buffer, size_t length)
{
    ...
    while (p != partial.rend())
    {
        ...
        ldout(cct,
              20)
            << "assemble_result(" << this << ") " << p->first << "~" << p->second.second << " " << p->second.first.length() << " bytes" << dendl;
        assert(p->first == end - p->second.second);
        end = p->first;
