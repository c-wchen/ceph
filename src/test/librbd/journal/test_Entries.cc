...
namespace librbd {
    namespace journal {
          ...
          public:
            ...
            ::journal::Journaler * create_journaler(librbd::ImageCtx * ictx) {
                ::journal::Journaler * journaler =
                    new::journal::Journaler(ictx->md_ctx, ictx->id,
                                            "dummy client", {
                                            }
                                            , nullptr);
            ...
            bool wait_for_entries_available(librbd::ImageCtx * ictx) {
                ...
                m_replay_handler.lock};
                while (!m_replay_handler.entries_available) {
                    if (m_replay_handler.cond.wait_for(locker, 10 s) ==
        ...
        TEST_F(TestJournalEntries, AioWrite) {
            ...
            c->get();
            api::Io <>::aio_write(*ictx, c, 123, buffer.size(),
                                  std::move(write_bl), 0, true);
            ...
            librbd::journal::AioWriteEvent aio_write_event =
                boost::get < librbd::journal::AioWriteEvent >
                (event_entry.event);
        ...
        TEST_F(TestJournalEntries, AioDiscard) {
            ...
            auto c = librbd::io::AioCompletion::create(&cond_ctx);
            c->get();
            api::Io <>::aio_discard(*ictx, c, 123, 234,
            ...
            librbd::journal::AioDiscardEvent aio_discard_event =
                boost::get < librbd::journal::AioDiscardEvent >
                (event_entry.event);
