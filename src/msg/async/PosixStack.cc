...
class PosixConnectedSocketImpl final:public ConnectedSocketImpl {
  ...
  public:
     explicit PosixConnectedSocketImpl(NetHandler & h, const entity_addr_t & sa,
                        ...
                        ServerSocket * sock)
{
    ...
    *sock =
        ServerSocket(std::unique_ptr < PosixServerSocketImpl >
...
:  NetworkStack(c, t)
{
  ...
  for (auto & corestr:corestrs) {
        ...
        if (err == "")
            ...
            lderr(cct) << __func__ << " failed to parse " << corestr << " in "
                << cct->_conf->ms_async_affinity_cores << dendl;
    }
}
