...
namespace crimson::osd {
    ...
    void usage(const char *prog) {
        ...
        return seastar::async([] {
                              auto auth_handler =
                              std::make_unique < DummyAuthHandler > ();
                              auto msgr =
                              crimson::net::Messenger::create(entity_name_t::
                                                              CLIENT(),
                                                              "temp_mon_client",
                                                              get_nonce());
                              crimson::mon::Client monc {
                              *msgr, *auth_handler};
                              msgr->set_auth_client(&monc); msgr->start( {
                                                                        &monc}
                              ).get(); auto stop_msgr = seastar::defer([&] {
                                                                       msgr->
                                                                       stop();
                                                                       msgr->
                                                                       shutdown
                                                               ...
                                                               <> (ep);}
                              ).get(); auto stop_monc = seastar::defer([&] {
                                                                       monc.
                                                                       stop().
                                                                       get();}
                              ...
                              logger().
                              debug("{}: got config from monitor, fsid {}",
                                    __func__, fsid);}
        );
    }
    ...
    static tl::expected < early_config_t, int >
        ...
        ret.init_params = ceph_argparse_early_args(early_args,
        ...
        int r = app.run(sizeof(bootstrap_args) / sizeof(bootstrap_args[0]),
 ...
 */
                        tl::expected < early_config_t,
                        int >get_early_config(int argc, const char *argv[]) {
                        int pipes[2]; int r = pipe2(pipes, 0); if (r < 0) {
                        std::
                        cerr << "get_early_config: failed to create pipes: " <<
                        -errno << std::endl; return tl::unexpected(-errno);}

                        pid_t worker = fork(); if (worker < 0) {
                        close(pipes[0]);
                        close(pipes[1]);
                        ...
                        if (ret.has_value()) {
                        bufferlist bl;::encode(ret.value(), bl);
                        r = bl.write_fd(pipes[1]); close(pipes[1]); if (r < 0) {
                        std::
                        cerr << "get_early_config: child failed to write_fd: "
                        << r << std::endl; exit(-r);}
                        else {
                        exit(0);}
                        }
                        ...
                        << -ret.error() << std::endl; exit(-ret.error());}
                        ...
                        waitpid(worker, nullptr, 0); if (r < 0) {
                        std::
                        cerr <<
                        "get_early_config: parent failed to read from pipe: " <<
                        r << std::endl; return tl::unexpected(r);}
                        try {
                        auto bliter = bl.cbegin();::decode(ret, bliter);
                        return ret;}
                        catch( ...) {
                        std::
                        cerr << "get_early_config: parent failed to decode" <<
                        std::endl; return tl::unexpected(-EINVAL);}
                        }
                        }
