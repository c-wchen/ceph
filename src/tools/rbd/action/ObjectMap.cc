    ...
    namespace action {
        namespace object_map {
            ...
            static int do_object_map_rebuild(librbd::Image & image,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: rebuilding object map failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            static int do_object_map_check(librbd::Image & image,
                                           bool no_progress) {
            ...
            int execute_check(const po::variables_map & vm) {
                ...
                r = do_object_map_check(image,
                                        vm[at::NO_PROGRESS].as < bool > ());
                if (r < 0) {
                    std::
                        cerr << "rbd: checking object map failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
