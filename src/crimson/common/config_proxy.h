...
// update the settings at runtime.
    class ConfigProxy:public seastar::peering_sharded_service < ConfigProxy > {
        ...
        // apply changes to all shards// @param func a functor which accepts @c "ConfigValues&"
            template < typename Func >
        seastar::future <> do_change(Func && func) {
            return container().invoke_on(values.get_owner_shard(),
                                         [func =
                                          std::
                                          move(func)] (ConfigProxy & owner) {
                                         // apply the changes to a copy of the values
                                         auto new_values =
                                         seastar::make_lw_shared(*owner.values);
                                         ...
                                         ObserverMgr <
                                         ConfigObserver >::rev_obs_map rev_obs;
                                         owner.values.reset(new_values);
                                         owner.obs_mgr.for_each_change(owner.
                                                                       values->
                                                                       changed,
                                                                       owner,
                                                                       [&rev_obs]
                                                                        ...
                                                                        * obs,
                                                                        ...
                                                                        string &
                                                                        key) {
              rev_obs[obs].insert(key);}, nullptr); for (auto &[obs, keys]:rev_obs)
                                         {
                                         obs->handle_conf_change(owner, keys);}

                                         return seastar::
                                         parallel_for_each(boost::
                                                           irange(1u,
                                                                  seastar::smp::
                                                                  count),
                                                           [&owner,
                                                           ...
                                                           invoke_on(cpu,
                                                                     ...
                                                                     (proxy.
                                                                       ...
                                                                       * obs,
                                                                       ...
                                                                       string &
                                                                       key) {
                      rev_obs[obs].insert(key);}, nullptr); for (auto & obs_keys:rev_obs)
                                                                     {
                                                                     obs_keys.
                                                                     first->
                                                                     handle_conf_change
      ...
      public:
        ...
        seastar::future <> set_val(const std::string & key,
            ...
            return do_change([key, val, this] (ConfigValues & values) {
                             ...
                             auto ret =
                             get_config().set_val(values, obs_mgr, key, val,
                                                  &err); if (ret < 0) {
        ...
        int get_val(std::string_view key, std::string * val) const {
        ...
        } int get_val_from_conf_file(const std::vector < std::string >
            ...
            set_mon_vals(const std::map < std::string, std::string,
                         std::less <>> &kv) {
            return do_change([kv, this] (ConfigValues & values) {
                             get_config().set_mon_vals(nullptr, values, obs_mgr,
                                                       kv, nullptr);}
            );
        }
        ...
        seastar::future <> inject_args(const std::string & s) {
            return do_change([s, this] (ConfigValues & values) {
                             std::stringstream err;
                             if (get_config().
                                 injectargs(values, obs_mgr, s, &err)) {
                             throw std::invalid_argument(err.str());}
                             }
            ) ;
        }
