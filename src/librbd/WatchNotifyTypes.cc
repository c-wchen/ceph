    ...
    namespace watch_notify {
        ...
        namespace {
            ...
            class DumpPayloadVisitor:public boost::static_visitor < void > {
              public:
                explicit DumpPayloadVisitor(Formatter *
                                            formatter):m_formatter(formatter) {
                } template < typename Payload >
                    inline void operator() (const Payload & payload)const {
                    NotifyOp notify_op = Payload::NOTIFY_OP;
                     m_formatter->dump_string("notify_op",
                                              stringify(notify_op));
        ...
        } // anonymous namespace void AsyncRequestId::encode(bufferlist & bl) const {
            ::encode(client_id, bl);
        ...
        void UnknownPayload::dump(Formatter * f) const {
            ...
            ENCODE_START(6, 1, bl);
            boost::apply_visitor(watcher::util::EncodePayloadVisitor(bl),
                                 payload);
            ENCODE_FINISH(bl);
            ...
            apply_visitor(watcher::util::DecodePayloadVisitor(struct_v, iter),
                          payload);
            DECODE_FINISH(iter);
        }
        ...
        void NotifyMessage::dump(Formatter * f) const {
            ...
            o.push_back(new
                        NotifyMessage(RequestLockPayload
                                      (ClientId(1, 2), true)));
            o.push_back(new NotifyMessage(HeaderUpdatePayload()));
            o.push_back(new
                        NotifyMessage(AsyncProgressPayload
                                      (AsyncRequestId(ClientId(0, 1), 2), 3,
                                       4)));
            o.push_back(new
                        NotifyMessage(AsyncCompletePayload
                                      (AsyncRequestId(ClientId(0, 1), 2), 3)));
            o.push_back(new
                        NotifyMessage(FlattenPayload
                                      (AsyncRequestId(ClientId(0, 1), 2))));
            o.push_back(new
                        NotifyMessage(ResizePayload
                                      (123, true,
                                       AsyncRequestId(ClientId(0, 1), 2))));
            o.push_back(new
                        NotifyMessage(SnapCreatePayload
                                      (cls::rbd::UserSnapshotNamespace(),
                                       "foo")));
            o.push_back(new
                        NotifyMessage(SnapRemovePayload
                                      (cls::rbd::UserSnapshotNamespace(),
                                       "foo")));
            o.push_back(new
                        NotifyMessage(SnapProtectPayload
                                      (cls::rbd::UserSnapshotNamespace(),
                                       "foo")));
            o.push_back(new
                        NotifyMessage(SnapUnprotectPayload
                                      (cls::rbd::UserSnapshotNamespace(),
                                       "foo")));
            o.push_back(new
                        NotifyMessage(RebuildObjectMapPayload
                                      (AsyncRequestId(ClientId(0, 1), 2))));
