  ...
  public:
      ...
      :    
        m_test_lock(ceph::
                    make_mutex(unique_lock_name
                               ("LeaderWatcher::m_test_lock", this))) {
    ...
    void SetUp() override {
        ...
        if (is_librados_test_stub(*_rados)) {
            ...
            EXPECT_EQ(0,
                      _rados->conf_set("rbd_mirror_leader_heartbeat_interval",
                                       "1"));
        }
    }

    librados::IoCtx & create_connection(bool no_heartbeats = false) {
        m_connections.push_back(std::unique_ptr < Connection >
                                (new Connection()));
        ...
        if (no_heartbeats) {
            EXPECT_EQ(0,
                      c->cluster.
                      conf_set("rbd_mirror_leader_heartbeat_interval", "3600"));
        }
        ...
        EXPECT_EQ(0,
                  c->cluster.ioctx_create(_local_pool_name.c_str(), c->io_ctx));
...
{
    Listener listener1;
    LeaderWatcher <> leader_watcher1(m_threads, create_connection(),
                                     &listener1);
    ...
    Listener listener2;
    LeaderWatcher <> leader_watcher2(m_threads, create_connection(),
                                     &listener2);
