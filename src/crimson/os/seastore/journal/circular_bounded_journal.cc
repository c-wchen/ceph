...
namespace crimson::os::seastore::journal {
    ...
    CircularBoundedJournal::CircularBoundedJournal(JournalTrimmer & trimmer,
                                                   RBMDevice * device,
    ...
    :trimmer(trimmer), path(path),
        ...
        record_submitter(crimson::common::get_conf < uint64_t >
                         ...
                         cjs) {
    } CircularBoundedJournal::open_for_mkfs_ret CircularBoundedJournal::
        open_for_mkfs() {
        return record_submitter.open(true).safe_then([this] (auto ret) {
                                                     record_submitter.
                                                     update_committed_to
                                                     (get_written_to());
                                                     return
    ...
    CircularBoundedJournal::open_for_mount_ret
        ...
        return record_submitter.open(false).safe_then([this] (auto ret) {
                                                      record_submitter.
                                                      update_committed_to
                                                      (get_written_to());
                                                      return
    ...
    CircularBoundedJournal::close_ertr::future <> CircularBoundedJournal::
        close() {
        return record_submitter.close();
    }

    CircularBoundedJournal::submit_record_ret
        CircularBoundedJournal::submit_record(record_t && record,
    ...
    CircularBoundedJournal::submit_record_ret
        CircularBoundedJournal::do_submit_record(record_t && record,
                                                 OrderingHandle & handle) {
        ...
        if (!record_submitter.is_available()) {
            ...
            return record_submitter.wait_available().
                safe_then([this, record =
                           std::move(record), &handle] ()mutable {
                          return do_submit_record(std::move(record), handle);}
            );
        }
            ...
            then([submit_fut = std::move(submit_fut)] ()mutable {
                 return std::move(submit_fut);}
                    ...
                    then([FNAME, this, result, &handle] {
                         ...
                         auto new_committed_to =
                         result.write_result.get_end_seq();
    ...
    Journal::replay_ret CircularBoundedJournal::
        scan_valid_record_delta(cbj_delta_handler_t
                                && delta_handler, journal_seq_t tail) {
        LOG_PREFIX(CircularBoundedJournal::scan_valid_record_delta);
        return seastar::do_with(bool(false),
                                rbm_abs_addr(get_rbm_addr(tail)),
                                std::move(delta_handler),
                                segment_seq_t(NULL_SEG_SEQ),
                                [this, FNAME] (auto & is_rolled,
                                               auto & cursor_addr,
                                               auto & d_handler,
                                               auto & expected_seq) {
                                return crimson::
                                repeat([this, &is_rolled, &cursor_addr,
                                        &d_handler, &expected_seq,
                                        FNAME] ()mutable->replay_ertr::future <
                                       seastar::stop_iteration > {
                                       paddr_t record_paddr =
                                       convert_abs_addr_to_paddr(cursor_addr,
                                                                 get_device_id
                                                                 ());
                                       return read_record(record_paddr,
                                                          expected_seq).
                                       safe_then([this, &is_rolled,
                                                  &cursor_addr, &d_handler,
                                                  &expected_seq,
                                                  FNAME] (auto ret)
                                                 ->replay_ertr::future <
                                                 seastar::stop_iteration > {
                                                 ...
                                                 if (expected_seq ==
                                                     ...
                                                     || is_rolled) {
                                                 DEBUG
                                                 ("no more records, stop replaying");
                                                 return replay_ertr::
                                                 make_ready_future <
                                                 seastar::stop_iteration >
                                                 (seastar::stop_iteration::
                                                 ...
                                                 bufferlist mdbuf;
                                                 mdbuf.substr_of(bl, 0,
                                                                 r_header.
                                                                 mdlength);
                                                 paddr_t record_block_base =
                                                 paddr_t::
                                                 make_blk_paddr(get_device_id(),
                                                                cursor_addr +
                                                                r_header.
                                                                mdlength);
                                                 ...
                                                 ERROR
                                                 ("unable to decode deltas for record {} at {}",
                                                 ...
                                                 (cursor_addr, get_device_id());
                                                 write_result.start_seq.offset =
                                                 addr;
                                                 write_result.start_seq.
    ...
    Journal::replay_ret CircularBoundedJournal::replay(delta_handler_t
                                                       && delta_handler) {
        /*
         * read records from last applied record prior to written_to, and replay
         */
        LOG_PREFIX(CircularBoundedJournal::replay);
        return cjs.read_header().handle_error(open_for_mount_ertr::pass_further {
                                              }
                                              , crimson::ct_error::assert_all {
                                              "Invalid error read_header"}
        ).safe_then([this, FNAME, delta_handler =
                     std::move(delta_handler)] (auto p)
                    mutable {
                    auto &[head, bl] = *p;
                    ...
                    return seastar::do_with(std::move(delta_handler),
                                            ...
                                            if (alloc_delta.op ==
                                                alloc_delta_t::op_types_t::
                                                CLEAR) {
                          for (auto & alloc_blk:alloc_delta.
                                                 alloc_blk_ranges)
                                            {
                                            map[alloc_blk.paddr] =
                                            ...
                                            auto tail =
                                            get_dirty_tail() <=
                                            get_alloc_tail()? get_dirty_tail() :
                                            get_alloc_tail();
                                            set_written_to(tail);
                                            // The first pass to build the paddr->journal_seq_t map 
                                            // from extent allocations
                                            return scan_valid_record_delta(std::
                                                                           move
                                                                           (build_paddr_seq_map),
                                                                           tail).
                                            safe_then([this, &map, &d_handler,
                                                       tail] () {
                                                      auto
                                                      call_d_handler_if_valid =
                                                      [this, &map,
                                                       &d_handler] (const auto &
                                                                    offsets,
                                                                    const auto &
                                                                    e,
                                                                    sea_time_point
                                                      ...
                                                      if (map.find(e.paddr) ==
                                                          map.end()
                                                          || map[e.paddr] <=
                                                          offsets.write_result.
                                                          start_seq) {
                                                      return d_handler(offsets,
                                                                       e,
                                                                       get_dirty_tail
                                                                       (),
                                                                       get_alloc_tail
                                ...
                                record_submitter.
                                update_committed_to(get_written_to());
                                trimmer.update_journal_tails(get_dirty_tail(),
                                                             get_alloc_tail());}
                    );}
        ) ;
    }
    ...
    CircularBoundedJournal::read_record_ret
        ...
        if (validate_records_metadata(md_bl) &&
            ...
            return read_record_ret(read_record_ertr::ready_future_marker {
                                   }
    ...
    CircularBoundedJournal::read_record_ret
        CircularBoundedJournal::read_record(paddr_t off,
                                            segment_seq_t expected_seq) {
        ...
        DEBUG("reading record from abs addr {} read length {}", addr,
              read_length);
        auto bptr = bufferptr(ceph::buffer::create_page_aligned(read_length));
        return cjs.read(addr,
                        bptr).safe_then([this, addr, bptr, expected_seq,
                                         FNAME] ()mutable->read_record_ret {
                                        record_group_header_t h; bufferlist bl;
                                        bl.append(bptr); auto bp = bl.cbegin();
                                        try {
                                        decode(h, bp);}
                                        catch(ceph::buffer::error & e) {
                                        return
                                        read_record_ret(read_record_ertr::
                                                        ready_future_marker {
                                                        }
                                                        , std::nullopt);}
                                        if (h.mdlength < get_block_size() ||
                                            ...
                                            || addr + h.mdlength + h.dlength >
                                            get_journal_end()
                                            || h.committed_to.segment_seq ==
                                            NULL_SEG_SEQ
                                        ...
                                        read_record_ret(read_record_ertr::
                                                        ...
                                                        , std::nullopt);}
                                        auto record_size =
                                        h.mdlength + h.dlength;
                                        if (record_size > get_block_size()) {
                                        auto next_addr =
                                        addr + get_block_size();
                                        auto next_length =
                                        record_size - get_block_size();
                                        auto next_bptr =
                                        bufferptr(ceph::buffer::
                                                  create_page_aligned
                                                  (next_length));
                                        DEBUG
                                        ("reading record part 2 from abs addr {} read length {}",
                                         next_addr, next_length);
                                        return cjs.read(next_addr,
                                                        next_bptr).
                                        safe_then([this, h, next_bptr =
                                                   std::move(next_bptr), bl =
                                                   std::move(bl)] ()mutable {
                                                  bl.append(next_bptr);
    ...
    seastar::future <> CircularBoundedJournal::
        finish_commit(transaction_type_t type) {
        if (is_trim_transaction(type)) {
            return update_journal_tail(trimmer.get_dirty_tail(),
