...
:  key
{
ConfFile::normalize_key_name(key)}, val {

    boost::algorithm::trim_copy_if(val,[](unsigned char c) {
...
bool conf_line_t::operator<(const conf_line_t & rhs) constconst
{
...
int ConfFile::parse_file(const std::string & fname, std::ostream * warnings)
{
    clear();
    try {
        if (auto file_size = fs::file_size(fname);
            file_size > MAX_CONFIG_FILE_SZ) {
    ...
    template < typename Iterator, typename Skipper >
        ...
        struct error_handler_t {
            ...
            template < typename Iter >
                ...
                << " at position " << boost::spirit::get_column(line_start,
                                                                where) << "\n";
                return qi::fail;
        }};
        IniGrammer(Iterator begin, std::ostream & err)
      :    IniGrammer::base_type {
        conf_file}
        , report_error {
            error_handler_t {
            err}
        }
        {
            ...
            text_char %=
                (lit('\\') >> (char_ - eol)) | (char_ - (comment_start | eol));
            ...
            qi::on_error < qi::fail > (conf_file,
                                       report_error(qi::_1, qi::_2, qi::_3,
                                                    qi::_4));
...
bool ConfFile::parse_buffer(std::string_view buf, std::ostream * err)
{
    ...
    IniGrammer < iter_t, skipper_t > grammar {
    first, *err};
    skipper_t skipper = grammar.continue_marker | grammar.comment;
    return qi::phrase_parse(first, iter_t {
                            _buf.end()}
...
void ConfFile::check_old_style_section_names(const std::vector < std::string >
                                             &prefixes, std::ostream & os)
{
