...
template < class T >
std::tuple < int,
    bufferlist > rgw_rest_get_json_input_keep_data(CephContext *cct,
            req_state *s, T &out,
            uint64_t max_len)
{
    int rv = 0;
...
protected:
    ...
    virtual bool is_obj_update_op() const
    {
        return false;
...
protected:
    ...
    virtual RGWHandler_REST *get_handler(rgw::sal::Driver *driver,
                                         req_state *const s,
                                         const rgw::auth::
...
extern void dump_header(req_state *s,
                        const std::string_view &name, long long val);
extern void dump_header(req_state *s,
                        const std::string_view &name, const utime_t &val);
...
extern void dump_etag(req_state *s,
                      const std::string_view &etag, bool quoted = false);
