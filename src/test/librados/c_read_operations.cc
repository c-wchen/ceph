...
protected:
                                    ...
                                    const size_t *lens, size_t len)
    {
        ...
        rados_read_op_t op = rados_create_read_op();
        rados_read_op_omap_get_vals2(op, NULL, NULL, 100, &iter_vals, NULL,
                                     &r_vals);
                            ...
                            size_t len, rados_omap_iter_t iter)
    {
        ...
        ASSERT_EQ(i, len);
        ASSERT_EQ(0,
                  rados_omap_get_next2(iter, &key, &val, &key_len, &val_len));
        ASSERT_EQ((char *)NULL, key);
        ASSERT_EQ((char *)NULL, val);
...
TEST_F(CReadOpsTest, CmpXattr)
{
    ...
    EXPECT_EQ(-ECANCELED,
              cmp_xattr(xattr, buf, sizeof(buf), LIBRADOS_CMPXATTR_OP_NE));
    EXPECT_EQ(-ECANCELED,
    ...
    EXPECT_EQ(1,
              cmp_xattr(xattr, buf, sizeof(buf) - 1, LIBRADOS_CMPXATTR_OP_NE));
    EXPECT_EQ(-ECANCELED,
              cmp_xattr(xattr, buf, sizeof(buf) - 1, LIBRADOS_CMPXATTR_OP_GT));
    EXPECT_EQ(-ECANCELED,
              cmp_xattr(xattr, buf, sizeof(buf) - 1, LIBRADOS_CMPXATTR_OP_GTE));
    EXPECT_EQ(1,
              cmp_xattr(xattr, buf, sizeof(buf) - 1, LIBRADOS_CMPXATTR_OP_LT));
    EXPECT_EQ(1,
              cmp_xattr(xattr, buf, sizeof(buf) - 1, LIBRADOS_CMPXATTR_OP_LTE));

    // > value
    memset(buf, 0xcd, sizeof(buf));
    EXPECT_EQ(-ECANCELED,
              cmp_xattr(xattr, buf, sizeof(buf), LIBRADOS_CMPXATTR_OP_EQ));
    ...
    EXPECT_EQ(-ECANCELED,
              cmp_xattr(xattr, buf, sizeof(buf), LIBRADOS_CMPXATTR_OP_LT));
    EXPECT_EQ(-ECANCELED,
              cmp_xattr(xattr, buf, sizeof(buf), LIBRADOS_CMPXATTR_OP_LTE));
...
TEST_F(CReadOpsTest, Checksum)
{
...

    {
        ...
        rados_read_op_checksum(op, LIBRADOS_CHECKSUM_TYPE_XXHASH32,
                               reinterpret_cast < char *>(&init_value),
                               sizeof(init_value), 0, len, 0, nullptr, 0,
                               &rval);
        ASSERT_EQ(0, rados_read_op_operate(op, ioctx, obj, 0));
        ASSERT_EQ(0, rval);
        rados_release_read_op(op);
