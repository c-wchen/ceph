...
class JournalingObjectStore:public ObjectStore {
  protected:
    ...
    class SubmitManager {
      ...
      public:
         SubmitManager(CephContext * cct):cct(cct),
            lock("JOS::SubmitManager::lock", false, true, false, cct),
      ...
      public:
         ApplyManager(CephContext * cct, Journal * &j, Finisher & f):
            ...
            apply_lock("JOS::ApplyManager::apply_lock", false, true, false,
                       cct), blocked(false), open_ops(0), max_applied_seq(0),
  ...
  protected:
    ...
    void _op_journal_transactions(bufferlist & tls, uint32_t orig_len,
                                  uint64_t op, Context * onjournal,
                                  TrackedOpRef osd_op);

    virtual int do_transactions(vector < ObjectStore::Transaction > &tls,
                                uint64_t op_seq) = 0;
  ...
  :    
    ...
    submit_manager(cct),
    apply_manager(cct, journal, finisher), replaying(false) {
    }
