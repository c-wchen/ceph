...
template < typename T > struct C_Holder:public Context {
    T obj;
    explicit C_Holder(T obj)
    :obj(obj) {
...
struct OnWriteApplied:public Context {
    ...
    uint64_t seq;
     ...
     OnWriteApplied(Bencher * bench, uint64_t seq,
                    ceph::shared_ptr < OnDelete > on_delete):bench(bench),
        seq(seq), on_delete(on_delete) {
    } void finish(int r) override {
...
struct OnWriteCommit:public Context {
    ...
    uint64_t seq;
     ceph::shared_ptr < OnDelete > on_delete;
     OnWriteCommit(Bencher * bench, uint64_t seq,
                   ceph::shared_ptr < OnDelete > on_delete):bench(bench),
        seq(seq), on_delete(on_delete) {
    } void finish(int r) override {
...
struct OnReadComplete:public Context {
    ...
    uint64_t seq;
     ...
     OnReadComplete(Bencher * bench, uint64_t seq,
                    bufferlist * bl):bench(bench), seq(seq), bl(bl) {
    } void finish(int r) override {
...
struct OnFinish {
    ...
    Cond *cond;
    OnFinish(bool * done,
             Mutex * lock, Cond * cond):done(done), lock(lock), cond(cond) {
    } ~OnFinish() {
        Mutex::Locker l(*lock);
        *done = true;
        cond->Signal();
...
void Bencher::init(const set < std::string > &objects,
                   uint64_t size, std::ostream * out)
{
    ...
    bool done = 0;
    {
        ceph::shared_ptr < OnFinish >
            on_finish(new OnFinish(&done, &lock, &cond));
        uint64_t num = 0;
        for (set < std::string >::const_iterator i = objects.begin();
             i != objects.end(); ++i, ++num) {
            if (!(num % 20))
                *out << "Creating " << num << "/" << objects.
                    size() << std::endl;
            backend->write(*i, 0, bl,
                           new C_Holder < ceph::shared_ptr < OnFinish >
                           >(on_finish),
                           new C_Holder < ceph::shared_ptr < OnFinish >
                           >(on_finish)
                );
...
void Bencher::run_bench()
{
    ...
    while ((!max_duration || time(0) < end) && (!max_ops || ops < max_ops)) {
        ...
        switch (op_type) {
        case WRITE:{
                ceph::shared_ptr < OnDelete >
                    on_delete(new OnDelete(new Cleanup(this)));
                stat_collector->start_write(seq, length);
                while (bl.length() < length) {
                    bl.append(rand());
                }
                backend->write(obj_name,
                               offset,
                               bl,
                               new OnWriteApplied(this, seq, on_delete),
                               new OnWriteCommit(this, seq, on_delete)
                    );
                break;
            }
        case READ:{
                ...
                backend->read(obj_name,
                              offset,
                              length,
                              read_bl, new OnReadComplete(this, seq, read_bl)
                    );
                break;
            }
        default:{
