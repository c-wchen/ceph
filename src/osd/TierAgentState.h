...
struct TierAgentState {
    ...
    /// approximate ratio of objects (assuming they are uniformly/// distributed) that i should aim to evict.
  ...
  :    
    ...
    delaying(false),
    hist_age(0),
    flush_mode(FLUSH_MODE_IDLE), evict_mode(EVICT_MODE_IDLE), evict_effort(0) {
    }
    ...
    /// add archived HitSet void add_hit_set(time_t start, HitSetRef hs) {
