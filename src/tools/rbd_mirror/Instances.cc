...
namespace rbd {
    namespace mirror {
        ...
        using librbd::util::create_rados_callback;
         ...
         template < typename I >
            Instances < I >::Instances(Threads < I > *threads,
                                       librados::IoCtx & ioctx,
                                       const std::string & instance_id,
                                       instances::
                                       Listener & listener):m_threads(threads),
            m_ioctx(ioctx), m_instance_id(instance_id), m_listener(listener),
            m_cct(reinterpret_cast < CephContext * >(ioctx.cct())),
            m_lock(ceph::
                   make_mutex("rbd::mirror::Instances " +
                              ioctx.get_pool_name())) {
          ...
          for (auto & pair:m_instances) {
            ...
            if (!added_instance_ids.empty()) {
                m_threads->work_queue->
                    queue(new C_NotifyInstancesAdded(this, added_instance_ids),
                          0);
            }
        }
        ...
        template < typename I >
            void Instances <
            I >::handle_acked(const InstanceIds & instance_ids) {
          ...
          for (auto & instance_id:instance_ids) {
                auto & instance =
                    m_instances.insert(std::make_pair(instance_id, Instance {
                                                      }
                                       )).first->second;
        ...
        template < typename I >
            void Instances <
            I >::notify_instances_added(const InstanceIds & instance_ids) {
          ...
          for (auto & instance_id:instance_ids) {
                ...
                if (it != m_instances.end()
                    && it->second.state == INSTANCE_STATE_ADDING) {
                    added_instance_ids.push_back(instance_id);
                }
            }
        ...
        template < typename I >
            void Instances <
            I >::notify_instances_removed(const InstanceIds & instance_ids) {
            dout(5) << "instance_ids=" << instance_ids << dendl;
        ...
        template < typename I >
            void Instances < I >::list(std::vector < std::string >
                                       *instance_ids) {
            dout(20) << dendl;
        ...
        template < typename I >
            ...
            if (r < 0) {
                derr << "error retrieving instances: " << cpp_strerror(r) <<
                    dendl;
            }
            else {
        ...
        template < typename I >
            void Instances < I >::remove_instances(const Instances <
                                                   I >::clock_t::
                                                   time_point & time) {
            ceph_assert(ceph_mutex_is_locked(m_lock));
          ...
          for (auto & instance_pair:m_instances) {
            ...
            Context *ctx = new LambdaContext([this, instance_ids] (int r){
                                             handle_remove_instances(r,
                                                                     instance_ids);});
            ctx = create_async_context_callback(m_threads->work_queue, ctx);

            auto gather_ctx = new C_Gather(m_cct, ctx);
          for (auto & instance_id:instance_ids) {
                InstanceWatcher < I >::remove_instance(m_ioctx,
        ...
        template < typename I >
            void Instances < I >::handle_remove_instances(int r,
                                                          const InstanceIds &
                                                          instance_ids) {
            std::scoped_lock locker {
            m_threads->timer_lock, m_lock};
            ...
            m_threads->work_queue->
                queue(new C_NotifyInstancesRemoved(this, instance_ids), 0);
        ...
        template < typename I >
            void Instances < I >::schedule_remove_task(const Instances <
                                                       I >::clock_t::
                                                       time_point & time) {
            cancel_remove_task();
            if (m_on_finish != nullptr) {
            ...
            int after =
                ...
                ("rbd_mirror_leader_heartbeat_interval") * (1 +
                                                            m_cct->_conf.
                                                            get_val < uint64_t >
