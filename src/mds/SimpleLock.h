...
struct LockType {
    ...
    explicit LockType(int t):type(t) {
        ...
        case CEPH_LOCK_IPOLICY:
            sm = &sm_simplelock;
            break;
            case CEPH_LOCK_IDFT:case CEPH_LOCK_INEST:sm = &sm_scatterlock;
            break;
            case CEPH_LOCK_IFILE:sm = &sm_filelock;
            break;
            case CEPH_LOCK_DVERSION:case CEPH_LOCK_IVERSION:sm = &sm_locallock;
            break;
            default:sm = 0;
}}};
...
class SimpleLock {
    ...
    const char *get_state_name(int n) const {
        ...
        case LOCK_UNDEF:
            ...
            return 0;
                                                            ...
                                                            // waiting static const uint64_t WAIT_RD = (1 << 0);
                                                            // to read
    ...
    struct unstable_bits_t {
        ...
        bool empty() {
            return
                ...
                num_xlock == 0 &&
                xlock_by.get() == NULL &&
                xlock_by_client == -1 && excl_client == -1;
        } unstable_bits_t() {
        }
    };
  ...
  SimpleLock(MDSCacheObject * o, LockType * lt):
    type(lt), parent(o), state(LOCK_SYNC), state_flags(0), num_rdlock(0) {
    }
    virtual ~ SimpleLock() {
    }
    ...
    // parent MDSCacheObject *get_parent() {
        return parent;
    }
    int get_type() const {
        return type->type;
        ...
        bool operator() (const SimpleLock * l, const SimpleLock * r)const {
            ...
            if (!(l->type->type > CEPH_LOCK_DN)
                && (r->type->type > CEPH_LOCK_DN))
                return true;
            if ((l->type->type > CEPH_LOCK_DN) ==
                (r->type->type > CEPH_LOCK_DN)) {
                // then sort by object
                if (l->parent->is_lt(r->parent))
                    return true;
    ...
    bool is_sync_and_unlocked() const {
        return
            get_state() == LOCK_SYNC &&
            !is_rdlocked() && !is_leased() && !is_wrlocked() && !is_xlocked();
    }
    /*
    ...
    // wrlock void get_wrlock(bool force = false) {
    ...
    // xlock void get_xlock(MutationRef who, client_t client) {
    ...
    void put_xlock() {
        assert(state == LOCK_XLOCK || state == LOCK_XLOCKDONE || state == LOCK_XLOCKSNAP || state == LOCK_LOCK_XLOCK || state == LOCK_LOCK ||   /* if we are a master of a slave */
               is_locallock());
        --more()->num_xlock;
        parent->put(MDSCacheObject::PIN_LOCK);
        if (more()->num_xlock == 0) {
    ...
    bool is_loner_mode() const {
        ...
        return parent->is_auth() ? get_sm()->allowed_ever_auth : get_sm()->
            allowed_ever_replica;
    ...
    int gcaps_careful() const {
        ...
        if (client == get_xlock_by_client())
            return type->type ==
                CEPH_LOCK_IFILE ? 0xf : (CEPH_CAP_GSHARED | CEPH_CAP_GEXCL);
        return 0;
    }
    // simplelock specifics int get_replica_state() const {
