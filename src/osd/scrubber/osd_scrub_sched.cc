...
void ScrubQueue::ScrubJob::update_schedule(const ScrubQueue::
        scrub_schedule_t &adjusted)
{
    schedule = adjusted;
...
void ScrubQueue::remove_from_osd_queue(ScrubJobRef scrub_job)
{
    ...
    qu_state_t expected_state {
        qu_state_t::registered};
                                   ...
                                   const ScrubQueue::sched_params_t &suggested)
{
    ...
    dout(20) << fmt::format("pg[{}] state at entry: <{:.14}>", scrub_job->pgid,
                            state_at_entry)
             << dendl;
    ...
    switch (state_at_entry) {
            ...
            // restore to the to_sched queue
        {
            ...
            if (scrub_job->state == qu_state_t::not_registered) {
                dout(5) << " scrub job state changed to 'not registered'" <<
                        dendl;
                to_scrub.push_back(scrub_job);
            }
            scrub_job->in_queues = true;
    ...
    dout(10) << fmt::
             format("pg[{}] sched-state changed from <{:.14}> to <{:.14}> (@{:s})",
...
ScrubQueue::sched_params_t ScrubQueue::
determine_scrub_time(const requested_scrub_t &request_flags,
                     const pg_info_t &pg_info,
                     const pool_opts_t &pool_conf) const const
{
    ...
    dout(15) << fmt::
             format("suggested: {:s} hist: {:s} v:{}/{} must:{} pool-min:{} {}",
                    res.proposed_time, pg_info.history.last_scrub_stamp,
...
// clang-format off
    ...
    std::string_view ScrubQueue::qu_state_text(qu_state_t st) {
        switch (st) {
            case qu_state_t::not_registered:
                return "not registered w/ OSD"sv;
            case qu_state_t::registered:
                return "registered"sv;
            case qu_state_t::unregistering:
                return "unregistering"sv;
        }
        // g++ (unlike CLANG), requires an extra ' return ' here
        return "(unknown)"sv;
    }
// clang-format on
    ...
    Scrub::schedule_result_t ScrubQueue::select_pg_and_scrub(
        ...
        to_scrub.end(), [](const auto & jobref)->void {
            jobref->updated = false;
        });
...
// must be called under lock
    void ScrubQueue::rm_unregistered_jobs(ScrubQContainer & group) {
        ...
        group.
        erase(std::
              remove_if(group.begin(), group.end(), invalid_state),
              group.end());
    }
    ...
    struct cmp_sched_time_t {
        ...
        {
            return lhs->schedule.scheduled_at <
                   rhs->schedule.scheduled_at;
        }
    };
    }                   // namespace

// called under lock
    ScrubQueue::ScrubQContainer ScrubQueue::
    collect_ripe_jobs(ScrubQContainer & group, utime_t time_now) {
...
// not holding jobs_lock. 'group' is a copy of the actual list.
    Scrub::schedule_result_t ScrubQueue::
    select_from_group(ScrubQContainer & group,
        ...
        for (auto &candidate : group) {
            ...
            dout(20) << "try initiating scrub for " << candidate->
                     pgid << dendl;

            if (preconds.only_deadlined
                && (candidate->schedule.deadline.is_zero()
                    || candidate->schedule.deadline >= now_is)) {
                dout(15) << " not scheduling scrub for " << candidate->
                         pgid << " due to " << (preconds.
                                                time_permit ? "high load" :
                                                "time not permitting")
                         << dendl;
                continue;
            }
            ...
            switch (osd_service.initiate_a_scrub(candidate->pgid,
                ...
                case Scrub::schedule_result_t::scrub_initiated:
                    ...
                    dout(20) << " initiated for " << candidate->
                             pgid << dendl;
                ...
                case Scrub::schedule_result_t::bad_pg_state:
                    ...
                    dout(20) << "failed (state/cond/started) " <<
                             candidate->pgid << dendl;
                    break;

                case Scrub::schedule_result_t::no_such_pg:
                    // The pg is no longer there
                    dout(20) << "failed (no pg) " << candidate->
                             pgid << dendl;
                    break;
                ...
                case Scrub::schedule_result_t::no_local_resources:
                    ...
                    // failure!
                    dout(20) << "failed (local) " << candidate->
                             pgid << dendl;
    ...
    ScrubQueue::scrub_schedule_t ScrubQueue::
    adjust_target_time(const sched_params_t &times)const {
        ...
        if (times.is_must == ScrubQueue::must_scrub_t::not_mandatory) {
            ...
            double scrub_min_interval = times.min_interval > 0
                                        ? times.min_interval : conf()->osd_scrub_min_interval;
            double scrub_max_interval = times.max_interval > 0
                                        ? times.max_interval : conf()->osd_scrub_max_interval;

            sched_n_dead.scheduled_at += scrub_min_interval;
            double r = rand() / (double)RAND_MAX;
            sched_n_dead.scheduled_at +=
                scrub_min_interval *
                conf()->osd_scrub_interval_randomize_ratio * r;
            ...
            dout(20) << fmt::
                     ...
                     ("not-must. Was:{:s} {{min:{}/{} max:{}/{} ratio:{}}} "
                      "Adjusted:{:s} ({:s})", times.proposed_time,
    ...
    double ScrubQueue::scrub_sleep_time(bool must_scrub) const {
        ...
        dout(20) << "w/ extended sleep (" << extended_sleep << ")" <<
                 dendl;
    ...
    bool ScrubQueue::scrub_load_below_threshold()const {
        ...
        double loadavg_per_cpu =
            cpus > 0 ? loadavgs[0] / cpus : loadavgs[0];
        if (loadavg_per_cpu < conf()->osd_scrub_load_threshold) {
            dout(20) << "loadavg per cpu " << loadavg_per_cpu <<
                     " < max " << conf()->
                     osd_scrub_load_threshold << " = yes" << dendl;
            return true;
        }
        ...
        if (loadavgs[0] < daily_loadavg && loadavgs[0] < loadavgs[2]) {
            dout(20) << "loadavg " << loadavgs[0] << " < daily_loadavg "
                     << daily_loadavg << " and < 15m avg " << loadavgs[2] <<
                     " = yes" << dendl;
            return true;
        }
        ...
        dout(20) << "loadavg " << loadavgs[0] << " >= max "
                 << conf()->
                 osd_scrub_load_threshold << " and ( >= daily_loadavg " <<
                 daily_loadavg << " or >= 15m avg " << loadavgs[2] <<
                 ") = no" << dendl;
        return false;
    }
...
// note: called with jobs_lock held
    void ScrubQueue::scan_penalized(bool forgive_all, utime_t time_now) {
        dout(20) << time_now << (forgive_all ? " all " : " - ") <<
                 penalized.size()
                 << dendl;
        ...
        if (forgive_all) {
            ...
            std::copy(penalized.begin(), penalized.end(),
                      std::back_inserter(to_scrub));
            penalized.clear();

        } else {

            auto forgiven_last = std::partition(penalized.begin(),
                                                penalized.end(),
                                                [time_now](const auto &
            e) {
                return (*e).updated
            ...
            std::copy(penalized.begin(), forgiven_last,
                      std::back_inserter(to_scrub));
            penalized.erase(penalized.begin(), forgiven_last);
            dout(20) << "penalized after screening: " << penalized.
                     size() << dendl;
        }
    }
...
// closed.
    static inline bool isbetween_modulo(int64_t from, int64_t till,
                                        int p) {
        // the 1st condition is because we have defined from==till as "always true"
        return (till == from)
               || ((till >= from) ^ (p >= from) ^ (p < till));
    }
    bool ScrubQueue::scrub_time_permit(utime_t now) const {
        ...
        bool day_permit =
            isbetween_modulo(conf()->osd_scrub_begin_week_day,
                             conf()->osd_scrub_end_week_day,
                             bdt.tm_wday);
        if (!day_permit) {
            dout(20) << "should run between week day "
                     << conf()->osd_scrub_begin_week_day << " - "
                     << conf()->osd_scrub_end_week_day << " now " << bdt.
                     tm_wday << " - no" << dendl;
            return false;
        }

        bool time_permit =
            isbetween_modulo(conf()->osd_scrub_begin_hour,
                             conf()->osd_scrub_end_hour, bdt.tm_hour);
        dout(20) << "should run between " << conf()->
                 osd_scrub_begin_hour << " - " << conf()->
                 osd_scrub_end_hour << " now (" << bdt.
                 tm_hour << ") = " << (time_permit ? "yes" : "no") << dendl;
        return time_permit;
    }
    ...
    void ScrubQueue::dump_scrubs(ceph::Formatter * f) const {
        ...
        [&f](const ScrubJobRef & j) {
            j->dump(f);
        });

        std::for_each(penalized.cbegin(),
        penalized.cend(), [&f](const ScrubJobRef & j) {
            j->dump(f);
        });
    ...
    ScrubQueue::ScrubQContainer ScrubQueue::list_registered_jobs()const {
        ...
        std::copy_if(to_scrub.begin(),
                     to_scrub.end(),
                     std::back_inserter(all_jobs), registered_job);
...
// ScrubQueue - scrub resource management
    ...
    bool ScrubQueue::can_inc_scrubs()const {
        ...
        dout(20) << " == false. " << scrubs_local << " local + " <<
                 scrubs_remote << " remote >= max " << conf()->
    ...
    void ScrubQueue::dec_scrubs_local() {
        std::lock_guard lck {
            resource_lock};
        dout(20) << ": " << scrubs_local << " -> " << (scrubs_local -
                 1) << " (max " <<
                 conf()->
                 osd_max_scrubs << ", remote " << scrubs_remote << ")" <<
                 dendl;
    ...
    bool ScrubQueue::inc_scrubs_remote() {
        ...
        if (scrubs_local + scrubs_remote < conf()->osd_max_scrubs) {
            dout(20) << ": " << scrubs_remote << " -> " <<
                     (scrubs_remote + 1)
                     << " (max " << conf()->
                     osd_max_scrubs << ", local " << scrubs_local << ")" <<
                     dendl;
            ++scrubs_remote;
            return true;
        }
    ...
    void ScrubQueue::clear_pg_scrub_blocked(spg_t blocked_pg) {
        dout(5) << fmt::format(": pg {} is unblocked",
                               blocked_pg) << dendl;
        --blocked_scrubs_cnt;
