...
namespace rbd {
    namespace ggate {
        ...
        int Driver::load() {
            ...
            while (size <= 1024 * 1024) {
                devs_ =
                    ...
                    char
                    **>(realloc
                        (static_cast < void *>(devs_), size * sizeof(*devs_)));
        ...
        Driver::Driver(const std::string & devname, size_t sectorsize,
                       size_t mediasize, bool readonly,
                       const std::string & info)
      :    m_devname(devname), m_sectorsize(sectorsize),
            m_mediasize(mediasize), m_readonly(readonly), m_info(info) {
        }
        ...
        int Driver::init() {
            ...
            int r =
                ggate_drv_create(name, namelen, m_sectorsize, m_mediasize,
                                 m_readonly,
                                 m_info.c_str(), &m_drv);
            if (r < 0) {
                return r;
        ...
        int Driver::recv(Request ** req) {
            ...
            *req = new Request(req_);
            ...
            if (ggate_drv_req_cmd(req_) == GGATE_DRV_CMD_WRITE) {
                bufferptr ptr(buffer::claim_malloc(ggate_drv_req_length(req_),
                                                   static_cast <
                                                   char
                                                   *>(ggate_drv_req_release_buf
                                                      (req_))));
        ...
        int Driver::send(Request * req) {
            ...
            if (ggate_drv_req_cmd(req->req) == GGATE_DRV_CMD_READ &&
                ...
                req->bl.copy(0, ggate_drv_req_length(req->req),
                             static_cast <
                             char *>(ggate_drv_req_buf(req->req)));
                dout(20) << "copied resulting " << req->bl.
                    length() << " bytes to " << ggate_drv_req_buf(req->
                                                                  req) << dendl;
            }
