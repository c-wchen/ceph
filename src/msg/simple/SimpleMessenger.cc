...
timeout(0), local_connection(new PipeConnection(cct, this))
{
                                    ...
                                    PipeConnection * con, Message * first)
{
    ...
    ldout(cct,
          10) << "connect_rank to " << addr << ", creating pipe and registering"
        << dendl;
...
{
    return ms_deliver_verify_authorizer(con, peer_type, protocol, authorizer,
...
ConnectionRef SimpleMessenger::get_connection(const entity_inst_t & dest)
{
    ...
    while (true) {
        ...
        if (pipe) {
            ldout(cct,
                  10) << "get_connection " << dest << " existing " << pipe <<
                dendl;
        }
        else {
            pipe = connect_rank(dest.addr, dest.name.type(), NULL, NULL);
            ldout(cct,
                  10) << "get_connection " << dest << " new " << pipe << dendl;
        }
...
void SimpleMessenger::submit_message(Message * m, PipeConnection * con,
                                     ...
                                     int dest_type, bool already_locked)
{
    ...
    if (con) {
        ...
        while (pipe && ok) {
            ...
            if (pipe->state != Pipe::STATE_CLOSED) {
                ldout(cct,
                      20) << "submit_message " << *m << " remote, " << dest_addr
                    << ", have pipe." << dendl;
                pipe->_send(m);
                pipe->pipe_lock.Unlock();
                pipe->put();
    ...
    if (policy.server) {
       ...
       *  Connection, so we can use our existing pointer without doing another lookup. */
            ...
            connect_rank(dest_addr, dest_type,
                         static_cast < PipeConnection * >(con), m);
        }
    }
}
...
int SimpleMessenger::send_keepalive(Connection * con)
{
    ...
    if (pipe) {
        ldout(cct,
              20) << "send_keepalive con " << con << ", have pipe." << dendl;
...
void SimpleMessenger::wait()
{
    ...
    lock.Lock();
    {
        ...
        reaper();
        ldout(cct,
              10) << "wait: waiting for pipes " << pipes << " to close" <<
            dendl;
        while (!pipes.empty()) {
            reaper_cond.Wait(lock);
            reaper();
...
void SimpleMessenger::mark_down_all()
{
    ...
    lock.Lock();
    for (set < Pipe * >::iterator q = accepting_pipes.begin();
         q != accepting_pipes.end(); ++q) {
        Pipe *p = *q;
    ...
    while (!rank_pipe.empty()) {
        ceph::unordered_map < entity_addr_t, Pipe * >::iterator it =
            rank_pipe.begin();
        Pipe *p = it->second;
