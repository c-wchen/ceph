...
struct seastore_test_t:public seastar_test_suite_t,
    SeaStoreTestState,::testing::WithParamInterface < const char *> {
    ...
    seastar::future <> set_up_fut()final {
        ...
        return tm_setup(journal).then([this] {
                                      return sharded_seastore->
    ...
    void do_transaction(CTransaction && t) {
        return sharded_seastore->do_transaction(coll, std::move(t)).get0();
    }
    ...
    std::tuple < int, std::string > get_meta(const std::string & key) {
    ...
    struct object_state_t {
        ...
        void truncate(CTransaction & t, uint64_t off) {
        ...
        std::map < uint64_t, uint64_t > fiemap(SeaStoreShard & sharded_seastore,
        ...
        void set_omap(CTransaction & t,
                      const string & key, const bufferlist & val) {
            omap[key] = val;
        ...
        void set_omap(SeaStoreShard & sharded_seastore,
                      const string & key, const bufferlist & val) {
            CTransaction t;
            set_omap(t, key, val);
            sharded_seastore.do_transaction(coll, std::move(t)).get0();
        ...
        void write(SeaStoreShard & sharded_seastore,
                   CTransaction & t, uint64_t offset, bufferlist bl) {
            bufferlist new_contents;
            if (offset > 0 && contents.length()) {
                new_contents.substr_of(contents,
                                       0,
                                       std::min < size_t > (offset,
                                                            contents.length())
                    );
            }
            ...
            if (contents.length() > tail_offset) {
                bufferlist tail;
                tail.substr_of(contents,
                               tail_offset, contents.length() - tail_offset);
        ...
        void write(SeaStoreShard & sharded_seastore,
                   uint64_t offset, size_t len, char fill) {
        ...
        void zero(SeaStoreShard & sharded_seastore,
                  CTransaction & t, uint64_t offset, size_t len) {
            ceph::buffer::list bl;
            bl.append_zero(len);
            bufferlist new_contents;
            if (offset > 0 && contents.length()) {
        ...
        void read(SeaStoreShard & sharded_seastore,
                  uint64_t offset, uint64_t len) {
            bufferlist to_check;
            to_check.substr_of(contents, offset, len);
            auto ret = sharded_seastore.read(coll,
                                             oid,
                                             offset,
                                             len).unsafe_get0();
        ...
        void check_size(SeaStoreShard & sharded_seastore) {
            auto st = sharded_seastore.stat(coll,
                                            oid).get0();
            EXPECT_EQ(contents.length(), st.st_size);
        }

        void set_attr(SeaStoreShard & sharded_seastore,
                      std::string key, bufferlist & val) {
            CTransaction t;
        ...
        ceph::bufferlist get_attr(SeaStoreShard & sharded_seastore,
        ...
        void check_omap_key(SeaStoreShard & sharded_seastore,
                            const string & key) {
            std::set < string > to_check;
            to_check.insert(key);
            auto result = sharded_seastore.omap_get_values(coll,
                                                           oid,
                                                           to_check).
                unsafe_get0();
            if (result.empty()) {
        ...
        void check_omap(SeaStoreShard & sharded_seastore) {
            ...
            while (true) {
                auto[done, kvs] = sharded_seastore.omap_get_values(coll,
                                                                   oid,
                                                                   start).
                    unsafe_get0();
                auto iter = kvs.begin();
                while (true) {
                    ...
                    if (iter == kvs.end() || refiter->first < iter->first) {
                        logger().debug("check_omap: missing omap key {}",
                                       refiter->first);
                        GTEST_FAIL() << "missing omap key " << refiter->first;
                        ++refiter;
                    }
                    else if (refiter == omap.end()
                             || refiter->first > iter->first) {
                        logger().debug("check_omap: extra omap key {}",
                                       iter->first);
    ...
    object_state_t & get_object(const ghobject_t & oid) {
        return test_objects.emplace(std::make_pair(oid, object_state_t {
                                                   coll_name, coll, oid}
                                    )).first->second;
    }
    ...
    void validate_objects() const {
        ...
        } auto ret = sharded_seastore->list_objects(coll,
                                                    ghobject_t(),
                                                    ghobject_t::get_max(),
                                                    std::numeric_limits <
                                                    uint64_t >::max()).get0();
    ...
    struct bound_t {
        ...
        static bound_t get_min() {
            return bound_t {
            type_t::MIN};
        }
        static bound_t get_max() {
            return bound_t {
            type_t::MAX};
        }
        static bound_t get_temp_end() {
            return bound_t {
            type_t::TEMP_END};
        }
        ...
        ghobject_t get_oid(SeaStore & seastore, CollectionRef & coll) const {
            ...
            case type_t::MIN:
                ...
                case type_t::TEMP_END:return seastore.get_objs_range(coll,
                                                                     0).
                    temp_end;
                case type_t::NORMAL_BEGIN:return seastore.get_objs_range(coll,
                                                                         0).
                    obj_begin;
    ...
    using list_test_cases_t =
        std::list < std::tuple < unsigned, bound_t, bound_t >>;
    ...
    void test_list(unsigned temp_to_create, /// create temp 0..temp_to_create-1
      ...
      for (auto[limit, in_left_bound, in_right_bound]:cases) {
            ...
            auto[listed, next] =
                sharded_seastore->list_objects(coll, left_bound, right_bound,
                                               limit).get0();

            // compute correct answer
            auto correct_begin = std::find_if(objs.begin(), objs.end(),
            ...
            for (; count < limit &&
                 ...
                 *correct_end < right_bound; ++correct_end, ++count) ;
...
TEST_P(seastore_test_t, collection_create_list_remove)
{
    run_async([this] {
              ...
              CTransaction t;
              t.create_collection(test_coll, 4); do_transaction(std::move(t));}
              auto colls_cores = seastore->list_collections().get0();
              std::vector < coll_t > colls;
              colls.resize(colls_cores.size());
              std::transform(colls_cores.begin(), colls_cores.end(),
                             colls.begin(),[](auto p) {
                             return p.first;
                             }
              );
              ...
              {
              CTransaction t;
              t.remove_collection(test_coll); do_transaction(std::move(t));}
              auto colls_cores = seastore->list_collections().get0();
              std::vector < coll_t > colls;
              colls.resize(colls_cores.size());
              std::transform(colls_cores.begin(), colls_cores.end(),
...
TEST_P(seastore_test_t, meta)
{
    run_async([this] {
              ...
              EXPECT_EQ(ret1, 0);
              EXPECT_EQ(ret2, 0);
              EXPECT_EQ(value1, "value1"); EXPECT_EQ(value2, "value2");});
}
...
TEST_P(seastore_test_t, touch_stat_list_remove)
{
    run_async([this] {
              ...
              validate_objects();
              remove_object(test_obj); validate_objects();});
}
...
{
    run_async([this] {
              test_list(5, 0, normal_list_cases);
              });
}
...
{
    run_async([this] {
              test_list(5, 5, normal_list_cases);
              });
}
...
TEST_P(seastore_test_t, omap_test_simple)
{
    run_async([this] {
              auto & test_obj = get_object(make_oid(0));
              test_obj.touch(*sharded_seastore);
              test_obj.set_omap(*sharded_seastore,
                                "asdf",
                                make_bufferlist(128));
...
TEST_P(seastore_test_t, attr)
{
    run_async([this] {
              ...
              test_val2.clear();
              decode(test_val2, bl2); EXPECT_EQ(test_val, test_val2);
              //test rm_attrs
              ...
              char oi_array[onode_layout_t::MAX_OI_LENGTH + 1] = {
              'a'};
              std::string oi_str(&oi_array[0], sizeof(oi_array));
              bl.clear();
              encode(oi_str, bl);
              test_obj.set_attr(*sharded_seastore, OI_ATTR, bl);
              char ss_array[onode_layout_t::MAX_SS_LENGTH + 1] = {
              'b'};
              ...
              bl2.clear();
              bl2 = attrs[SS_ATTR];
              decode(ss2, bl2); EXPECT_EQ(oi, oi2); EXPECT_EQ(ss, ss2);}
              }
    );
}
...
TEST_P(seastore_test_t, omap_test_iterator)
{
    run_async([this] {
              ...
              test_obj.set_omap(*sharded_seastore,
                                make_key(i), make_bufferlist(128));}
...
TEST_P(seastore_test_t, simple_extent_test)
{
    run_async([this] {
              auto & test_obj = get_object(make_oid(0));
              test_obj.write(*sharded_seastore,
                             1024,
                             1024,
                             'a');
              test_obj.read(*sharded_seastore,
                            1024,
                            1024); test_obj.check_size(*sharded_seastore);}
    );
}
...
TEST_P(seastore_test_t, fiemap_empty)
{
    run_async([this] {
              ...
              EXPECT_TRUE(m.empty()); test_obj.remove(*sharded_seastore);}
    );
}
...
TEST_P(seastore_test_t, fiemap_holes)
{
    run_async([this] {
              ...
              auto m =
              test_obj.fiemap(*sharded_seastore, 0,
                              SKIP_STEP * (MAX_EXTENTS - 1) + 3);
              ASSERT_EQ(m.size(), MAX_EXTENTS); for (uint64_t i = 0;
                                                     i < MAX_EXTENTS; i++) {
              ...
              auto m =
              test_obj.fiemap(*sharded_seastore, SKIP_STEP,
                              SKIP_STEP * (MAX_EXTENTS - 3) + 3);
              ASSERT_EQ(m.size(), MAX_EXTENTS - 2); for (uint64_t i = 1;
                                                         i < MAX_EXTENTS - 1;
                                                         i++) {
              ASSERT_TRUE(m.count(SKIP_STEP * i));
              ASSERT_GE(m[SKIP_STEP * i], bl.length());}
...
TEST_P(seastore_test_t, sparse_read)
{
    run_async([this] {
      ...
      interval_set < uint64_t > m; m = interval_set < uint64_t > (test_obj.fiemap(*sharded_seastore, 0, SKIP_STEP * (MAX_EXTENTS - 1) + 3)); ASSERT_TRUE(!m.empty()); uint64_t off = 0; auto rbl = test_obj.readv(*sharded_seastore, m); for (auto && miter:m)
              {
              bufferlist subl;
              subl.substr_of(rbl, off,
                             std::min(miter.second, uint64_t(wbl.length())));
              ASSERT_TRUE(subl.contents_equal(wbl)); off += miter.second;}
              test_obj.remove(*sharded_seastore);}
    ) ;
}
...
TEST_P(seastore_test_t, zero)
{
    run_async([this] {
              auto test_zero =[this] (
                                         ...
                                         uint64_t, uint64_t >> writes,
                                         uint64_t zero_off, uint64_t zero_len) {

              // Test zero within a block
          auto & test_obj = get_object(make_oid(0)); uint64_t size = 0; for (auto &[off, len, repeat]:writes)
              {
              ...
              test_obj.read(*sharded_seastore,
                            ...
                            size);
              test_obj.check_size(*sharded_seastore);
              remove_object(test_obj);}; const uint64_t BS = 4 << 10;
              // Test zero within a block
              test_zero( { {
                        1 << 10, 1 << 10, 1}}, 1124, 200);
              // Multiple writes, partial on left, partial on right.
              test_zero( { {
                        BS, BS, 10}}, BS + 128, BS * 4);
              // Single large write, block boundary on right, partial on left.
              test_zero( { {
                        BS, BS * 10, 1}}, BS + 128, (BS * 4) - 128);
              // Multiple writes, block boundary on left, partial on right.
              test_zero( { {
                        BS, BS, 10}}, BS, (BS * 4) + 128);}) ;
}

INSTANTIATE_TEST_SUITE_P(seastore_test,
                         seastore_test_t,::testing::Values("segmented",
                                                           "circularbounded")
    );
