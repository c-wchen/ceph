     ...
     C_handle_notify(EventCenter * c, CephContext * cc):center(c), cct(cc) {
        ...
        do {
            ...
            if (r < 0) {
                if (errno != EAGAIN)
                    ldout(cct,
                          1) << __func__ << " read notify pipe failed: " <<
...
int EventCenter::init(int n, unsigned i, const std::string & t)
{
    ...
    if (pipe_cloexec(fds) < 0) {
        int e = errno;
        lderr(cct) << __func__ << " can't create notify pipe: " <<
...
void EventCenter::set_owner()
{
    ...
    if (!global_centers) {
        cct->lookup_or_create_singleton_object <
            EventCenter::AssociatedCenters > (global_centers,
                                              "AsyncMessenger::EventCenter::global_center::"
                                              + type);
        ...
        if (driver->need_wakeup()) {
            ...
            int r =
                create_file_event(notify_receive_fd, EVENT_READABLE,
                                  notify_handler);
            assert(r == 0);
        }
    }
...
int EventCenter::create_file_event(int fd, int mask, EventCallbackRef ctxt)
{
    ...
    if (fd >= nevent) {
        ...
        while (fd >= new_size)
            new_size <<= 2;
        ldout(cct,
              20) << __func__ << " event count exceed " << nevent <<
    ...
    ldout(cct,
          20) << __func__ << " create event started fd=" << fd << " mask=" <<
        mask << " original mask is " << event->mask << dendl;
    if (event->mask == mask)
        return 0;
    ...
    ldout(cct,
          20) << __func__ << " create event end fd=" << fd << " mask=" << mask
...
void EventCenter::delete_file_event(int fd, int mask)
{
    ...
    if (fd >= nevent) {
        ldout(cct,
              1) << __func__ << " delete event fd=" << fd <<
            " is equal or greater than nevent=" << nevent << "mask=" << mask <<
            dendl;
        return;
    }
    EventCenter::FileEvent * event = _get_file_event(fd);
    ldout(cct,
          30) << __func__ << " delete event started fd=" << fd << " mask=" <<
        mask << " original mask is " << event->mask << dendl;
    if (!event->mask)
        return;
    ...
    ldout(cct,
          30) << __func__ << " delete event end fd=" << fd << " mask=" << mask
        << " original mask is " << event->mask << dendl;
}

uint64_t EventCenter::create_time_event(uint64_t microseconds,
                                        EventCallbackRef ctxt)
{
    ...
    ldout(cct,
          30) << __func__ << " id=" << id << " trigger after " << microseconds
        << "us" << dendl;
    EventCenter::TimeEvent event;
    clock_type::time_point expire =
        clock_type::now() + std::chrono::microseconds(microseconds);
    event.id = id;
    event.time_cb = ctxt;
    std::multimap < clock_type::time_point,
        TimeEvent >::value_type s_val(expire, event);
...
void EventCenter::wakeup()
{
    ...
    if (n < 0) {
        if (errno != EAGAIN) {
            ldout(cct,
                  1) << __func__ << " write notify pipe failed: " <<
                cpp_strerror(errno) << dendl;
            ceph_abort();
        }
    }
...
int EventCenter::process_time_events()
{
    ...
    while (!time_events.empty()) {
        ...
        if (now >= it->first) {
            ...
            event_map.erase(id);
            ldout(cct,
                  30) << __func__ << " process time event: id=" << id << dendl;
            processed++;
            cb->do_request(id);
        }
        else {
                                ...
                                ceph::timespan * working_dur)
{
    ...
    if (!blocking) {
        ...
        if (it != time_events.end() && shortest >= it->first) {
            ldout(cct,
                  30) << __func__ << " shortest is " << shortest <<
                " it->first is " << it->first << dendl;
            shortest = it->first;
            trigger_time = true;
            if (shortest > now) {
    ...
    ldout(cct,
          30) << __func__ << " wait second " << tv.tv_sec << " usec " << tv.
        tv_usec << dendl;
    ...
    for (int j = 0; j < numevents; j++) {
        ...
        ldout(cct,
              30) << __func__ << " event_wq process is " << fired_events[j].
            fd << " mask is " << fired_events[j].mask << dendl;
    }
