...
struct TestMockTrashMoveRequest: public TestMockFixture {
                          ...
                          const utime_t &end_time, int r)
    {
        ...
        .WillOnce(WithArg < 4 > (Invoke([ = ](bufferlist & in_bl) {
            ...
            decode(id, bl_it);
            decode(trash_image_spec,
                   bl_it);
            EXPECT_EQ(id, image_id);
            EXPECT_EQ(trash_image_spec.
                      source,
                      trash_image_source);
            EXPECT_EQ(trash_image_spec.
                      name, name);
                             ...
                             const std::string &id, int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(StrEq("rbd_directory"), _, StrEq("rbd"),
                         StrEq("dir_remove_image"),
...
TEST_F(TestMockTrashMoveRequest, Success)
{
    ...
    expect_trash_add(mock_image_ctx, "image id",
                     ...
                     delete_time, 0);
    expect_aio_remove(mock_image_ctx, util::id_obj_name("image name"),
                      0);
    ...
    cls::rbd::TrashImageSpec trash_image_spec {
        ...
        delete_time};
    auto req =
        MockMoveRequest::create(mock_image_ctx.md_ctx, "image id",
