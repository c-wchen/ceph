...
rx_frame_asm(&session_stream_handlers, false, cct->_conf->ms_crc_data,
             &session_compression_handlers),
next_tag(static_cast < Tag > (0)), keepalive(false)
{
}
...
void ProtocolV2::requeue_sent()
{
    ...
    while (!sent.empty()) {
        ...
        ldout(cct, 5) << __func__ << " requeueing message m=" << m
            ...
            << *m << dendl;
        m->clear_payload();
        rq.emplace_front(out_queue_entry_t {
                         false, m}
        );
    }
}
...
uint64_t ProtocolV2::discard_requeued_up_to(uint64_t out_seq, uint64_t seq)
{
    ...
    if (rq.empty())
        out_queue.erase(CEPH_MSG_PRIO_HIGHEST);
    return count;
}
...
void ProtocolV2::reset_recv_state()
{
    ...
    if (!connection->center->in_thread()) {
        ...
        connection->center->submit_to(connection->center->get_id(),[this] {
                                      ldout(cct,
                                            5) <<
                                      "reset_recv_state (warped) reseting crypto and compression handlers"
                                      << dendl;
                                      // Possibly unnecessary. See the comment in `deactivate_existing`.
                                      std::lock_guard < std::mutex >
                                      l(connection->lock);
                                      std::lock_guard < std::mutex >
                                      wl(connection->write_lock);
                                      reset_security(); reset_compression();}
                                      , /* always_async = */ true);
    }
    else {
        reset_security();
...
size_t ProtocolV2::get_current_msg_size() const const
{
...
void ProtocolV2::reset_throttle()
{
    ...
    if (state > THROTTLE_DISPATCH_QUEUE && state <= THROTTLE_DONE) {
        ...
        ldout(cct, 10)
            ...
            << connection->dispatch_queue->dispatch_throttler.
...
CtPtr ProtocolV2::_fault()
{
    ...
    if (!(state >= START_CONNECT && state <= SESSION_RECONNECTING) &&
        ...
        // woke up again;
        connection->register_time_events.insert(connection->center->
...
void ProtocolV2::prepare_send_message(uint64_t features, Message * m)
{
    ...
    ldout(cct,
          20) << __func__ << (m->
                              empty_payload()? " encoding features " :
...
void ProtocolV2::send_message(Message * m)
{
    ...
    if (state == CLOSED) {
        ...
        out_queue[m->get_priority()].emplace_back(out_queue_entry_t {
                                                  is_prepared, m}
        );
        ldout(cct,
              15) << __func__ << " inline write is denied, reschedule m=" << m
            << dendl;
        if (((!replacing && can_write) || state == STANDBY)
            && !write_in_progress) {
            write_in_progress = true;
            connection->center->dispatch_event_external(connection->
                                                        write_handler);
        }
    }
}
...
ssize_t ProtocolV2::write_message(Message * m, bool more)
{
    ...
    header.seq, header.tid,
            header.type, header.priority,
            header.version,
            ceph_le32(0), header.data_off,
            ceph_le64(ack_seq),
            footer.flags, header.compat_version, header.reserved};
    ...
    if (rc < 0) {
        ...
        const auto sent_bytes =
            total_send_size - connection->outgoing_bl.length();
...
void ProtocolV2::handle_message_ack(uint64_t seq)
{
    ...
    while (!sent.empty() && sent.front()->get_seq() <= seq && i < max_pending) {
        ...
        ldout(cct, 10) << __func__ << " got ack seq " << seq
            << " >= " << m->get_seq() << " on " << m << " " << *m << dendl;
    }
    connection->write_lock.unlock();
    connection->logger->tinc(l_msgr_handle_ack_lat,
...
void ProtocolV2::write_event()
{
    ...
    if (can_write) {
        ...
        if (r == 0) {
            ...
            if (left) {
                ldout(cct,
                      10) << __func__ << " try send msg ack, acked " << left <<
                    " messages" << dendl;
        ...
        if (state == STANDBY && !connection->policy.server && is_queued()) {
            ...
            if (r < 0) {
                ldout(cct,
                      1) << __func__ << " send outcoming bl failed" << dendl;
                connection->write_lock.unlock();
                fault();
                connection->lock.unlock();
                return;
                       ...
                       rx_buffer_t && buffer)
{
    ...
    ssize_t r = connection->read(len, buf,
                                 ...
                                 ceph_assert(!cct->_conf->ms_die_on_bug ||
                                             pre_auth.rxbuf.length() <
                                             20000000);}
                                 next.r = r; return &next;}

                                 return nullptr;}

                                 template < class F >
                                 CtPtr ProtocolV2::write(const std::
                                                         string & desc,
                                                         CONTINUATION_TYPE <
                                                         ProtocolV2 > &next,
                                                         F & frame) {
                                 ceph::bufferlist bl; try {
                                 bl =
                                 frame.
                                 get_buffer(tx_frame_asm);} catch(ceph::crypto::
                                                                  onwire::
                                                                  TxHandlerError
                                                                  & e) {
                                 ldout(cct,
                                       1) << __func__ << " " << e.
                                 what() << dendl; return _fault();}
                                 ...
                                 ceph_assert(!cct->_conf->ms_die_on_bug ||
                                             pre_auth.txbuf.length() <
                                             20000000);}

                                 ssize_t r =
                                 connection->write(buffer,
                                                   [&next, desc, this] (int r) {
                                                   if (r < 0) {
                                                   ldout(cct,
                                                         1) << __func__ << " "
                                                   << desc << " write failed r="
                                 ...
                                 if (r == 0) {
                                 next.setParams(); return &next;}

                                 return nullptr;}

                                 CtPtr ProtocolV2::
                                 _banner_exchange(CtRef callback) {
                                 ldout(cct, 20) << __func__ << dendl;
                                 ...
                                 encode((uint64_t)
                                        ...
                                        banner_payload, 0);
                                 encode((uint64_t) CEPH_MSGR2_REQUIRED_FEATURES,
                                        banner_payload, 0); ceph::bufferlist bl;
                                 bl.append(CEPH_BANNER_V2_PREFIX,
                                           strlen(CEPH_BANNER_V2_PREFIX));
                                 encode((uint16_t) banner_payload.length(), bl,
                                        0); bl.claim_append(banner_payload);
                                 INTERCEPT(state == BANNER_CONNECTING ? 3 : 4);
                                 return WRITE(bl, "banner",
                                 ...
                                 unsigned banner_prefix_len =
                                 strlen(CEPH_BANNER_V2_PREFIX);
                                 if (memcmp
                                     (buffer->c_str(), CEPH_BANNER_V2_PREFIX,
                                      banner_prefix_len)) {
                                 if (memcmp
                                     (buffer->c_str(), CEPH_BANNER,
                                      strlen(CEPH_BANNER)) == 0) {
                                 lderr(cct) << __func__ << " peer " <<
                                 *connection->
                                 peer_addrs << " is using msgr V1 protocol" <<
                                 dendl; return _fault();}
                                 ldout(cct,
                                       1) << __func__ <<
                                 ...
                                 ldout(cct,
                                       1) << __func__ << " supported=" << std::
                                 hex << peer_supported_features << " required="
                                 << std::hex << peer_required_features << std::
                                 dec << dendl;
                                 ...
                                 << " required=" << std::
                                 hex << required_features << " supported=" <<
                                 std::hex << peer_supported_features << std::
                                 dec << dendl; stop();
                                 connection->dispatch_queue->
                                 queue_reset(connection); return nullptr;}
                                 ...
                                 << " required=" << std::
                                 hex << peer_required_features << " supported="
                                 << supported_features << std::dec << dendl;
                                 stop();
                                 connection->dispatch_queue->
                                 queue_reset(connection); return nullptr;}

                                 this->peer_supported_features =
                                 peer_supported_features;
                                 ...
                                 auto hello =
                                 HelloFrame::Encode(messenger->get_mytype(),
                                                    connection->target_addr);
                                 INTERCEPT(state == HELLO_CONNECTING ? 7 : 8);
                                 return WRITE(hello, "hello frame",
                                              read_frame);}

                                 CtPtr ProtocolV2::handle_hello(ceph::
                                                                bufferlist &
                                                                payload) {
                                 ldout(cct,
                                       20) << __func__ << " payload.length()="
                                 << payload.length() << dendl;
                                 ...
                                 << " peer_addr_for_me=" << hello.
                                 peer_addr() << dendl; sockaddr_storage ss;
                                 socklen_t len = sizeof(ss);
                                 getsockname(connection->cs.fd(),
                                             (sockaddr *) & ss, &len);
                                 ldout(cct,
                                       5) << __func__ <<
                                 " getsockname says I am " << (sockaddr *) & ss
                                 << " when talking to " << connection->
                                 target_addr << dendl;
                                 if (connection->get_peer_type() == -1) {
                                 connection->set_peer_type(hello.entity_type());
                                 ceph_assert(state == HELLO_ACCEPTING);
                                 connection->policy =
                                 messenger->get_policy(hello.entity_type());
                                 ldout(cct,
                                       10) << __func__ <<
                                 " accept of host_type " << (int)hello.
                                 entity_type()
                                 << ", policy.lossy=" << connection->policy.
                                 lossy << " policy.server=" << connection->
                                 policy.
                                 server << " policy.standby=" << connection->
                                 policy.
                                 standby << " policy.resetcheck=" <<
                                 connection->policy.resetcheck << dendl;}
                                 else {
                                 ceph_assert(state == HELLO_CONNECTING);
                                 if (connection->get_peer_type() !=
                                     hello.entity_type()) {
                                 ldout(cct,
                                       1) << __func__ <<
                                 " connection peer type does not match what" <<
                                 " peer advertises " << connection->
                                 get_peer_type()
                                 ...
                                 ldout(cct,
                                       1) << __func__ << " peer " <<
                                 connection->
                                 target_addr << " says I am " << hello.
                                 peer_addr() << " (socket says " << (sockaddr *)
                                 & ss << ")" << dendl; a = hello.peer_addr();}
                                 else {
                                 ldout(cct,
                                       1) << __func__ << " socket to  " <<
                                 connection->
                                 target_addr << " says I am " << (sockaddr *) &
                                 ss << " (peer says " << hello.
                                 peer_addr() << ")" << dendl;
                                 ...
                                 if (rand() %
                                     ...
                                     0) {
                                 ldout(cct,
                                       10) << __func__ << " sleep for " << cct->
                                 _conf->ms_inject_internal_delays << dendl;
                                 utime_t t;
                                 t.set_from_double(cct->_conf->
                                 ...
                                 << " replacing must be happened just now" <<
                                 dendl; return nullptr;}
                                 }

                                 CtPtr callback;
                                 callback = bannerExchangeCallback;
                                 bannerExchangeCallback = nullptr;
                                 ceph_assert(callback); return callback;}
                                 ...
                                 ldout(cct,
                                       1) << __func__ <<
                                 " read frame preamble failed r=" << r << dendl;
                                 return _fault();}
                                 ...
                                 *_dout << dendl; try {
                                 ...
                                 ldout(cct,
                                       1) << __func__ << "bad auth tag" <<
                                 dendl; return _fault();}

                                 ldout(cct,
                                       25) << __func__ <<
                                 " disassembled preamble " << rx_frame_asm <<
                                 dendl; if (session_stream_handlers.rx) {
                                 ldout(cct,
                                       30) << __func__ <<
                                 " preamble after decrypt\n";
                                 rx_preamble.hexdump(*_dout); *_dout << dendl;}
                                 ...
                                 (next_tag) << dendl; switch (next_tag) {
case Tag::HELLO:
...
return handle_frame_payload(); case Tag::MESSAGE:
return handle_message(); default:{
                                 lderr(cct) << __func__
                                 << " received unknown tag=" << static_cast <
                                 uint32_t > (next_tag)
                                 << dendl; return _fault();}
                                 }
                                 ...
                                 << dendl; rx_segments_data.emplace_back();
                                 uint32_t onwire_len =
                                 ...
                                 uint16_t align =
                                 rx_frame_asm.get_segment_align(seg_idx); try {
                                 rx_buffer =
                                 ceph::buffer::ptr_node::create(ceph::buffer::
                                                                create_aligned
                                                                (onwire_len,
                                                                 align));}
                                 ...
                                 ldout(cct,
                                       1) << __func__ <<
                                 " can't allocate aligned rx_buffer" << " len="
                                 << onwire_len << " align=" << align << dendl;
                                 return _fault();}

                                 return READ_RXBUF(std::move(rx_buffer),
                                 ...
                                 *_dout << dendl; switch (next_tag) {
case Tag::HELLO:
return handle_hello(payload); case Tag::AUTH_REQUEST:
return handle_auth_request(payload); case Tag::AUTH_BAD_METHOD:
return handle_auth_bad_method(payload); case Tag::AUTH_REPLY_MORE:
return handle_auth_reply_more(payload); case Tag::AUTH_REQUEST_MORE:
return handle_auth_request_more(payload); case Tag::AUTH_DONE:
return handle_auth_done(payload); case Tag::AUTH_SIGNATURE:
return handle_auth_signature(payload); case Tag::CLIENT_IDENT:
return handle_client_ident(payload); case Tag::SERVER_IDENT:
return handle_server_ident(payload); case Tag::IDENT_MISSING_FEATURES:
return handle_ident_missing_features(payload); case Tag::SESSION_RECONNECT:
return handle_reconnect(payload); case Tag::SESSION_RESET:
return handle_session_reset(payload); case Tag::SESSION_RETRY:
return handle_session_retry(payload); case Tag::SESSION_RETRY_GLOBAL:
return handle_session_retry_global(payload); case Tag::SESSION_RECONNECT_OK:
return handle_reconnect_ok(payload); case Tag::KEEPALIVE2:
return handle_keepalive2(payload); case Tag::KEEPALIVE2_ACK:
return handle_keepalive2_ack(payload); case Tag::ACK:
return handle_message_ack(payload); case Tag::WAIT:
return handle_wait(payload); case Tag::COMPRESSION_REQUEST:
return handle_compression_request(payload); case Tag::COMPRESSION_DONE:
return handle_compression_done(payload); default:
                                 ceph_abort();}
                                 return nullptr;}

                                 CtPtr ProtocolV2::ready() {
                                 ldout(cct, 25) << __func__ << dendl;
                                 reconnecting = false; replacing = false;
                                 // make sure no pending tick timer
                                 if (connection->last_tick_id) {
                                 connection->center->
                                 delete_time_event(connection->last_tick_id);}
                                 connection->last_tick_id =
                                 connection->center->
                                 create_time_event(connection->
                                                   inactive_timeout_us,
                                                   connection->tick_handler); {
                                 std::lock_guard < std::mutex >
                                 l(connection->write_lock); can_write = true;
                                 if (!out_queue.empty()) {
                                 connection->center->
                                 ...
                                 state = READY;
                                 ldout(cct,
                                       1) << __func__ << " entity=" << peer_name
                                 << " client_cookie=" << std::
                                 hex << client_cookie << " server_cookie=" <<
                                 server_cookie << std::
                                 dec << " in_seq=" << in_seq << " out_seq=" <<
                                 out_seq << dendl; INTERCEPT(15);
                                 ...
                                 bool ok = false; try {
                                 ok =
                                 rx_frame_asm.disassemble_segments(rx_preamble,
                                                                   rx_segments_data.
                                                                   data(),
                                                                   rx_epilogue);}
                                 catch(FrameError & e) {
                                 ...
                                 << dendl;
                                 INTERCEPT(16); ceph_msg_header header {
                                 current_header.seq,
                                 current_header.tid,
                                 current_header.type,
                                 ...
                                 ceph_le32(0)}; ceph_msg_footer footer {
                                 ceph_le32(0), ceph_le32(0),
                                 ceph_le32(0), ceph_le64(0),
                                 current_header.flags};
                                 Message * message =
                                 decode_message(cct, 0, header, footer,
                                                msg_frame.front(),
                                                msg_frame.middle(),
                                 ...
                                 INTERCEPT(17);
                                 message->set_byte_throttler(connection->policy.
                                                             throttler_bytes);
                                 message->set_message_throttler(connection->
                                                                policy.
                                                                throttler_messages);
                                 ...
                                 ceph_assert(0 ==
                                             "old msgs despite reconnect_seq feature");}
                                 return nullptr;}
                                 ...
                                 ((double)
                                  (ltt_processed_stamp.to_nsec() -
                                   recv_stamp.to_nsec())) / 1000;
                                 ostringstream buf;
                                 if (message->get_type() == CEPH_MSG_OSD_OP)
                                 OID_ELAPSED_WITH_MSG(message, usecs_elapsed,
                                 ...
                                 *message << dendl; message->put(); goto out;}
                                 ...
                                 connection->logger->
                                 inc(l_msgr_recv_encrypted_bytes,
                                 ...
                                 if (rand() % 10000 <
                                     ...
                                     10000.0) {
                                 delay_period =
                                 cct->_conf->ms_inject_delay_max *
                                 (double)(rand() % 10000) / 10000.0;
                                 ldout(cct,
                                       1) << "queue_received will delay after "
                                 << (ceph_clock_now() +
                                     delay_period) << " on " << message << " "
                                 << *message << dendl;}
                                 connection->delay_state->queue(delay_period,
                                                                message);}
                                 else
                                 if (messenger->ms_can_fast_dispatch(message)) {
                                 connection->lock.unlock();
                                 connection->dispatch_queue->
                                 ...
                                 connection->dispatch_queue->enqueue(message,
                                                                     message->
                                                                     get_priority
                                                                     (),
                                                                     connection->
...
handle_message_ack(current_header.ack_seq); out:
                                 ...
                                 connection->register_time_events.
                                 insert(connection->center->
                                 ...
                                 ldout(cct, 20) << __func__ << dendl;
                                 const size_t cur_msg_size =
                                 get_current_msg_size(); if (cur_msg_size) {
                                 if (connection->policy.throttler_bytes) {
                                 ldout(cct,
                                       10) << __func__ << " wants " <<
                                 cur_msg_size << " bytes from policy throttler "
                                 << connection->policy.throttler_bytes->
                                 get_current() << "/" << connection->policy.
                                 throttler_bytes->get_max() << dendl;
                                 if (!connection->policy.throttler_bytes->
                                 ...
                                 << connection->dispatch_queue->
                                 dispatch_throttler.
                                 get_current() << "/" << connection->
                                 dispatch_queue->dispatch_throttler.get_max()
                                 << " failed, just wait." << dendl;
                                 // following thread pool deal with th full message queue isn't a
                                 // short time, so we can wait a ms.
                                 if (connection->register_time_events.empty()) {
                                 ...
                                 CtPtr ProtocolV2::handle_keepalive2(ceph::
                                                                     bufferlist
                                                                     & payload)
                                 {
                                 ldout(cct,
                                 ...
                                 CtPtr ProtocolV2::handle_keepalive2_ack(ceph::
                                                                         bufferlist
                                                                         &
                                                                         payload)
                                 {
                                 ...
                                 set_last_keepalive_ack(keepalive_ack_frame.
                                                        timestamp());
                                 ldout(cct,
                                       20) << __func__ << " got KEEPALIVE_ACK"
                                 << dendl; return CONTINUE(read_frame);}

                                 CtPtr ProtocolV2::handle_message_ack(ceph::
                                                                      bufferlist
                                                                      & payload)
                                 {
                                 ldout(cct,
...
/* Client Protocol Methods */
                                 ...
                                 ldout(cct, 20) << __func__ << dendl;
                                 INTERCEPT(1); state = BANNER_CONNECTING;
                                 global_seq = messenger->get_global_seq();
                                 return
                                 _banner_exchange(CONTINUATION
                                 ...
                                 CtPtr ProtocolV2::send_auth_request(std::
                                                                     vector <
                                                                     uint32_t >
                                                                     &allowed_methods)
                                 {
                                 ...
                                 connection->
                                 peer_type << " auth_client " << messenger->
                                 auth_client << dendl; ceph::bufferlist bl;
                                 std::vector < uint32_t > preferred_modes;
                                 auto am = auth_meta; connection->lock.unlock();
                                 int r =
                                 messenger->auth_client->
                                 get_auth_request(connection, am.get(),
                                                  &am->auth_method,
                                                  &preferred_modes, &bl);
                                 connection->lock.lock();
                                 if (state != AUTH_CONNECTING) {
                                 ldout(cct,
                                       1) << __func__ << " state changed!" <<
                                 dendl; return _fault();}
                                 if (r < 0) {
                                 ldout(cct,
                                       0) << __func__ <<
                                 ...
                                 AuthRequestFrame::Encode(auth_meta->
                                                          auth_method,
                                                          preferred_modes, bl);
                                 return WRITE(frame, "auth request",
                                              read_frame);}

                                 CtPtr ProtocolV2::handle_auth_bad_method(ceph::
                                                                          bufferlist
                                                                          &
                                                                          payload)
                                 {
                                 ...
                                 ldout(cct,
                                       1) << __func__ << " method=" <<
                                 bad_method.method()
                                 << " result " << cpp_strerror(bad_method.
                                                               result())
                                 << ", allowed methods=" << bad_method.
                                 allowed_methods()
                                 << ", allowed modes=" << bad_method.
                                 allowed_modes()
                                 << dendl; ceph_assert(messenger->auth_client);
                                 auto am = auth_meta; connection->lock.unlock();
                                 int r =
                                 messenger->auth_client->
                                 handle_auth_bad_method(connection, am.get(),
                                                        bad_method.method(),
                                                        bad_method.result(),
                                                        bad_method.
                                                        allowed_methods(),
                                 ...
                                 return _fault();}
                                 return send_auth_request(bad_method.
                                                          allowed_methods());}

                                 CtPtr ProtocolV2::handle_auth_reply_more(ceph::
                                                                          bufferlist
                                                                          &
                                                                          payload)
                                 {
                                 ...
                                 ldout(cct,
                                       5) << __func__ << " auth reply more len="
                                 << auth_more.auth_payload().length()
                                 << dendl; ceph_assert(messenger->auth_client);
                                 ceph::bufferlist reply; auto am = auth_meta;
                                 connection->lock.unlock();
                                 int r =
                                 messenger->auth_client->
                                 handle_auth_reply_more(connection, am.get(),
                                                        auth_more.
                                                        auth_payload(), &reply);
                                 connection->lock.lock();
                                 if (state != AUTH_CONNECTING) {
                                 ...
                                 << r << dendl; return _fault();}
                                 auto more_reply =
                                 AuthRequestMoreFrame::Encode(reply);
                                 return WRITE(more_reply, "auth request more",
                                              read_frame);}

                                 CtPtr ProtocolV2::handle_auth_done(ceph::
                                                                    bufferlist &
                                                                    payload) {
                                 ldout(cct,
                                       20) << __func__ << " payload.length()="
                                 ...
                                 int r =
                                 messenger->auth_client->
                                 handle_auth_done(connection, am.get(),
                                                  auth_done.global_id(),
                                                  auth_done.con_mode(),
                                 ...
                                 create_handler_pair(cct, *auth_meta,
                                                     /*new_nonce_format= */
                                                     is_rev1, /*crossed= */
                                                     false);
                                 ...
                                 empty()? sha256_digest_t() : auth_meta->
                                 session_key.hmac_sha256(cct, pre_auth.rxbuf);
                                 auto sig_frame =
                                 AuthSignatureFrame::Encode(sig);
                                 pre_auth.enabled = false;
                                 pre_auth.rxbuf.clear();
                                 return WRITE(sig_frame, "auth signature",
                                              read_frame);}

                                 CtPtr ProtocolV2::finish_client_auth() {
                                 if (HAVE_MSGR2_FEATURE
                                     (peer_supported_features, COMPRESSION)) {
                                 ...
                                 auto client_ident =
                                 ClientIdentFrame::Encode(messenger->
                                                          get_myaddrs(),
                                                          connection->
                                                          target_addr,
                                                          messenger->
                                                          ...
                                                          connection->policy.
                                                          features_required |
                                                          msgr2_required, flags,
                                                          client_cookie);
                                 ldout(cct,
                                       5) << __func__ <<
                                 " sending identification: " << "addrs=" <<
                                 messenger->get_myaddrs()
                                 << " target=" << connection->
                                 target_addr << " gid=" << messenger->
                                 get_myname().num()
                                 << " global_seq=" << global_seq <<
                                 " features_supported=" << std::
                                 hex << connection->policy.
                                 features_supported << " features_required=" <<
                                 (connection->policy.
                                  features_required | msgr2_required)
                                 << " flags=" << flags << " cookie=" <<
                                 client_cookie << std::dec << dendl;
                                 INTERCEPT(11);
                                 return WRITE(client_ident, "client ident",
                                              read_frame);}
                                 ...
                                 hex << client_cookie << " server_cookie=" <<
                                 server_cookie << std::
                                 dec << " gs=" << global_seq << " cs=" <<
                                 connect_seq << " ms=" << in_seq << dendl;
                                 INTERCEPT(13);
                                 return WRITE(reconnect, "reconnect",
                                              read_frame);}

                                 CtPtr ProtocolV2::
                                 handle_ident_missing_features(ceph::
                                                               bufferlist &
                                                               payload) {
                                 ldout(cct,
                                       20) << __func__ << " payload.length()="
                                 ...
                                 << std::hex << ident_missing.
                                 features() << std::dec << dendl;
                                 return _fault();}

                                 CtPtr ProtocolV2::handle_session_reset(ceph::
                                                                        bufferlist
                                                                        &
                                                                        payload)
                                 {
                                 ...
                                 ldout(cct,
                                       1) << __func__ <<
                                 " received session reset full=" << reset.full()
                                 << dendl; if (reset.full()) {
                                 reset_session();}
                                 else {
                                 server_cookie = 0; connect_seq = 0; in_seq = 0;}

                                 state = SESSION_CONNECTING;
                                 return send_client_ident();}

                                 CtPtr ProtocolV2::handle_session_retry(ceph::
                                                                        bufferlist
                                                                        &
                                                                        payload)
                                 {
                                 ...
                                 ldout(cct, 1) << __func__
                                 << " received session retry connect_seq=" <<
                                 retry.connect_seq()
                                 << ", inc to cs=" << connect_seq << dendl;
                                 return send_reconnect();}

                                 CtPtr ProtocolV2::
                                 handle_session_retry_global(ceph::
                                                             bufferlist &
                                                             payload) {
                                 ldout(cct,
                                       20) << __func__ << " payload.length()="
                                 ...
                                 global_seq =
                                 messenger->get_global_seq(retry.global_seq());
                                 ldout(cct,
                                       1) << __func__ <<
                                 " received session retry global global_seq=" <<
                                 retry.
                                 global_seq() << ", choose new gs=" <<
                                 global_seq << dendl; return send_reconnect();}

                                 CtPtr ProtocolV2::handle_wait(ceph::
                                                               bufferlist &
                                                               payload) {
                                 ldout(cct,
                                       20) << __func__ <<
                                 ...
                                 state = WAIT;
                                 WaitFrame::Decode(payload); return _fault();}

                                 CtPtr ProtocolV2::handle_reconnect_ok(ceph::
                                                                       bufferlist
                                                                       &
                                                                       payload)
                                 {
                                 ...
                                 ldout(cct,
                                       5) << __func__ <<
                                 " reconnect accepted: sms=" << reconnect_ok.
                                 msg_seq()
                                 << dendl;
                                 out_seq =
                                 ...
                                 backoff = utime_t();
                                 ldout(cct,
                                       10) << __func__ << " reconnect success "
                                 << connect_seq << ", lossy = " << connection->
                                 policy.lossy << ", features " << connection->
                                 get_features() << dendl;
                                 if (connection->delay_state) {
                                 ...
                                 CtPtr ProtocolV2::handle_server_ident(ceph::
                                                                       bufferlist
                                                                       &
                                                                       payload)
                                 {
                                 ...
                                 if (!server_ident.addrs().
                                     contains(connection->target_addr)) {
                                 ldout(cct,
                                       1) << __func__ << " peer identifies as "
                                 << server_ident.addrs()
                                 << ", does not include " << connection->
                                 target_addr << dendl; return _fault();}

                                 server_cookie = server_ident.cookie();
                                 connection->set_peer_addrs(server_ident.
                                                            addrs());
                                 peer_name =
                                 entity_name_t(connection->get_peer_type(),
                                               server_ident.gid());
                                 connection->set_features(server_ident.
                                                          supported_features() &
                                                          connection->policy.
                                                          features_supported);
                                 peer_global_seq = server_ident.global_seq();
                                 connection->policy.lossy =
                                 server_ident.flags() & CEPH_MSG_CONNECT_LOSSY;
                                 backoff = utime_t();
                                 ldout(cct,
                                       10) << __func__ << " connect success " <<
                                 connect_seq << ", lossy = " << connection->
                                 policy.lossy << ", features " << connection->
                                 get_features() << dendl;
                                 if (connection->delay_state) {
                                 ...
                                 (messenger->comp_registry.
                                  get_mode(peer_type,
                                           auth_meta->is_mode_secure()));
                                 const auto preferred_methods =
                                 messenger->comp_registry.
                                 get_methods(peer_type);
                                 auto comp_req_frame =
                                 CompressionRequestFrame::Encode(comp_meta.
                                                                 is_compress(),
                                                                 preferred_methods);
                                 INTERCEPT(19);
                                 return WRITE(comp_req_frame,
                                              "compression request",
                                              read_frame);}

                                 CtPtr ProtocolV2::
                                 handle_compression_done(ceph::
                                                         bufferlist & payload) {
                                 if (state != COMPRESSION_CONNECTING) {
                                 lderr(cct) << __func__ << " state changed!" <<
                                 dendl; return _fault();}

                                 auto response =
                                 CompressionDoneFrame::Decode(payload);
                                 ldout(cct,
                                       10) << __func__ <<
                                 " CompressionDoneFrame(is_compress=" <<
                                 ...
                                 session_compression_handlers =
                                 ceph::compression::onwire::rxtx_t::
                                 create_handler_pair(cct, comp_meta,
                                                     messenger->comp_registry.
                                                     get_min_compression_size
                                                     (connection->
                                                      get_peer_type()));
                                 return start_session_connect();}
...
/* Server Protocol Methods */
                                 ...
                                 ldout(cct, 20) << __func__ << dendl;
                                 INTERCEPT(2); state = BANNER_ACCEPTING;
                                 return
                                 _banner_exchange(CONTINUATION
                                 ...
                                 CtPtr ProtocolV2::handle_auth_request(ceph::
                                                                       bufferlist
                                                                       &
                                                                       payload)
                                 {
                                 ...
                                 ldout(cct,
                                       10) << __func__ << " AuthRequest(method="
                                 << request.method()
                                 << ", preferred_modes=" << request.
                                 preferred_modes()
                                 << ", payload_len=" << request.auth_payload().
                                 length() << ")" << dendl;
                                 auth_meta->auth_method = request.method();
                                 auth_meta->con_mode =
                                 messenger->auth_server->
                                 pick_con_mode(connection->get_peer_type(),
                                               auth_meta->auth_method,
                                               request.preferred_modes());
                                 if (auth_meta->con_mode ==
                                     CEPH_CON_MODE_UNKNOWN) {
                                 return _auth_bad_method(-EOPNOTSUPP);}
                                 return _handle_auth_request(request.
                                                             auth_payload(),
                                                             false);}
                                 ...
                                 messenger->auth_server->
                                 get_supported_auth_methods(connection->
                                                            get_peer_type(),
                                                            &allowed_methods,
                                                            &allowed_modes);
                                 ldout(cct,
                                       1) << __func__ << " auth_method " <<
                                 auth_meta->
                                 auth_method << " r " << cpp_strerror(r)
                                 << ", allowed_methods " << allowed_methods <<
                                 ", allowed_modes " << allowed_modes << dendl;
                                 auto bad_method =
                                 AuthBadMethodFrame::Encode(auth_meta->
                                                            auth_method, r,
                                                            allowed_methods,
                                                            allowed_modes);
                                 return WRITE(bad_method, "bad auth method",
                                              read_frame);}

                                 CtPtr ProtocolV2::_handle_auth_request(ceph::
                                                                        bufferlist
                                                                        &
                                                                        auth_payload,
                                                                        bool
                                 ...
                                 int r =
                                 messenger->auth_server->
                                 handle_auth_request(connection, am.get(), more,
                                                     am->auth_method,
                                                     auth_payload, &reply);
                                 connection->lock.lock();
                                 ...
                                 << dendl; ceph_assert(state == CLOSED);
                                 return _fault();}
                                 if (r == 1) {
                                 INTERCEPT(10);
                                 ...
                                 AuthDoneFrame::Encode(connection->
                                                       ...
                                                       auth_meta->con_mode,
                                                       reply);
                                 return WRITE(auth_done, "auth done",
                                              finish_auth);}
                                 else
                                 if (r == 0) {
                                 state = AUTH_ACCEPTING_MORE;
                                 auto more = AuthReplyMoreFrame::Encode(reply);
                                 return WRITE(more, "auth reply more",
                                              read_frame);}
                                 else
                                 if (r == -EBUSY) {
    // kick the client and maybe they'll come back later
                                 ...
                                 return _handle_auth_request(auth_more.
                                                             auth_payload(),
                                                             true);}

                                 CtPtr ProtocolV2::handle_auth_signature(ceph::
                                                                         bufferlist
                                                                         &
                                                                         payload)
                                 {
                                 ...
                                 " sig_frame.signature()=" << sig_frame.
                                 signature()
                                 << dendl; pre_auth.txbuf.clear();}

                                 if (state == AUTH_ACCEPTING_SIGN) {
                                 // this happened on server side
                                 return finish_server_auth();}
                                 else
                                 if (state == AUTH_CONNECTING_SIGN) {
                                 ...
                                 CtPtr ProtocolV2::handle_client_ident(ceph::
                                                                       bufferlist
                                                                       &
                                                                       payload)
                                 {
                                 ...
                                 ldout(cct,
                                       5) << __func__ <<
                                 " received client identification:" << " addrs="
                                 << client_ident.addrs()
                                 ...
                                 if (client_ident.addrs().empty()
                                     || client_ident.addrs().front() ==
                                     entity_addr_t()) {
                                 ldout(cct, 5) << __func__ << " oops, client_ident.addrs() is empty" << dendl; return _fault(); // a v2 peer should never do this
                                 }
                                 if (!messenger->get_myaddrs().
                                     contains(client_ident.target_addr())) {
                                 ldout(cct,
                                       5) << __func__ <<
                                 " peer is trying to reach " << client_ident.
                                 target_addr()
                                 << " which is not us (" << messenger->
                                 get_myaddrs() << ")" << dendl;
                                 return _fault();}

                                 connection->set_peer_addrs(client_ident.
                                                            addrs());
                                 connection->target_addr =
                                 connection->_infer_target_addr(client_ident.
                                                                addrs());
                                 peer_name =
                                 entity_name_t(connection->get_peer_type(),
                                               client_ident.gid());
                                 ...
                                 ldout(cct,
                                       1) << __func__ <<
                                 " peer missing required features " << std::
                                 hex << feat_missing << std::dec << dendl;
                                 auto ident_missing_features =
                                 IdentMissingFeaturesFrame::
                                 Encode(feat_missing);
                                 return WRITE(ident_missing_features,
                                              "ident missing features",
                                              read_frame);}

                                 connection_features =
                                 client_ident.
                                 supported_features() & connection->policy.
                                 features_supported;
                                 ...
                                 messenger->lookup_conn(*connection->
                                                        peer_addrs);
                                 if (existing
                                     && existing->protocol->proto_type != 2) {
                                 ldout(cct,
                                       1) << __func__ << " existing " <<
                                 existing << " proto " << existing->protocol.
                                 get() << " version is " << existing->protocol->
                                 proto_type << ", marking down" << dendl;
                                 existing->mark_down(); existing = nullptr;}
                                 ...
                                 CtPtr ProtocolV2::handle_reconnect(ceph::
                                                                    bufferlist &
                                                                    payload) {
                                 ldout(cct,
                                       20) << __func__ << " payload.length()="
                                 ...
                                 ldout(cct,
                                       5) << __func__ << " received reconnect:"
                                 << " client_cookie=" << std::hex << reconnect.
                                 client_cookie()
                                 << " server_cookie=" << reconnect.
                                 server_cookie() << std::
                                 dec << " gs=" << reconnect.global_seq()
                                 ...
                                 connection->target_addr =
                                 connection->_infer_target_addr(reconnect.
                                                                addrs());
                                 ...
                                 <<
                                 " no existing connection exists, reseting client"
                                 << dendl;
                                 auto reset = ResetFrame::Encode(true);
                                 return WRITE(reset, "session reset",
                                              read_frame);}
                                 ...
                                 ProtocolV2 * exproto =
                                 dynamic_cast <
                                 ProtocolV2 * >(existing->protocol.get());
                                 if (!exproto) {
                                 ldout(cct,
                                       1) << __func__ << " existing=" <<
                                 existing << dendl; ceph_assert(false);}

                                 if (exproto->state == CLOSED) {
                                 ldout(cct,
                                       5) << __func__ << " existing " <<
                                 existing << " already closed. Reseting client"
                                 << dendl;
                                 auto reset = ResetFrame::Encode(true);
                                 return WRITE(reset, "session reset",
                                              read_frame);}
                                 ...
                                 RetryGlobalFrame::Encode(exproto->
                                                          peer_global_seq);
                                 return WRITE(retry, "session retry",
                                              read_frame);}

                                 if (exproto->client_cookie !=
                                 ...
                                 << " cc=" << std::hex << exproto->
                                 client_cookie << " rcc=" << reconnect.
                                 client_cookie()
                                 << ", reseting client." << std::dec << dendl;
                                 auto reset =
                                 ResetFrame::Encode(connection->policy.
                                                    resetcheck);
                                 return WRITE(reset, "session reset",
                                              read_frame);}
                                 else
                                 if (exproto->server_cookie == 0) {
                                 // this happens when:
                                 //   - a connects to b
                                 ...
                                 ldout(cct,
                                       5) << __func__ <<
                                 " stale global_seq: sgs=" << exproto->
                                 peer_global_seq << " cgs=" << reconnect.
                                 global_seq()
                                 ...
                                 ldout(cct,
                                       5) << __func__ <<
                                 " stale connect_seq scs=" << exproto->
                                 connect_seq << " ccs=" << reconnect.
                                 connect_seq()
                                 << " , ask client to retry" << dendl;
                                 auto retry =
                                 RetryFrame::Encode(exproto->connect_seq);
                                 return WRITE(retry, "session retry",
                                              read_frame);}
                                 ...
                                 ldout(cct,
                                       1) << __func__ <<
                                 " reconnect to existing=" << existing << dendl;
                                 reconnecting = true;
                                 ...
                                 CtPtr ProtocolV2::
                                 handle_existing_connection(const
                                                            AsyncConnectionRef &
                                                            existing) {
                                 ldout(cct,
                                       20) << __func__ << " existing=" <<
                                 existing << dendl;
                                 ...
                                 l.unlock(); return send_server_ident();}
                                 ...
                                 << peer_global_seq <<
                                 " existing->peer_global_seq=" << exproto->
                                 peer_global_seq <<
                                 ", stopping this connection." << dendl; stop();
                                 connection->dispatch_queue->
                                 queue_reset(connection); return nullptr;}
                                 ...
                                 << dendl; existing->protocol->stop();
                                 existing->dispatch_queue->queue_reset(existing.
                                                                       get());
                                 l.unlock(); return send_server_ident();}
                                 ...
                                 if (exproto->state == READY
                                     || exproto->state == STANDBY) {
                                 ldout(cct,
                                       1) << __func__ << " existing=" <<
                                 existing << " is READY/STANDBY, lets reuse it"
                                 << dendl;
                                 return reuse_connection(existing, exproto);}
                                 ...
                                 << existing << dendl;
                                 ceph_assert(connection->peer_addrs->
                                             msgr2_addr() >
                                             messenger->get_myaddrs().
                                             msgr2_addr());
                                 ...
                                 ldout(cct,
                                       20) << __func__ << " existing=" <<
                                 existing << " reconnect=" << reconnecting <<
                                 dendl; connection->inject_delay();
                                 std::lock_guard < std::mutex >
                                 l(existing->write_lock);
                                 connection->center->
                                 delete_file_event(connection->cs.fd(),
                                                   EVENT_READABLE |
                                                   EVENT_WRITABLE);
                                 ...
                                 exproto->connection_features =
                                 connection_features;
                                 existing->set_features(connection_features);
                                 exproto->peer_supported_features =
                                 peer_supported_features;}
                                 ...
                                 auto temp_stream_handlers =
                                 std::move(session_stream_handlers);
                                 auto temp_compression_handlers =
                                 std::move(session_compression_handlers);
                                 ...
                                 ceph_assert(connection->recv_start ==
                                             connection->recv_end);
                                 auto deactivate_existing =
                                 std::
                                 bind([existing, new_worker, new_center,
                                       exproto, reconnecting =
                                       reconnecting, tx_is_rev1 =
                                       tx_frame_asm.get_is_rev1(), rx_is_rev1 =
                                       rx_frame_asm.get_is_rev1(),
                                       temp_stream_handlers =
                                       std::move(temp_stream_handlers),
                                       temp_compression_handlers =
                                      ...
                                      exproto->session_stream_handlers =
                                      std::move(temp_stream_handlers);
                                      exproto->session_compression_handlers =
                                      std::move(temp_compression_handlers);
                                      if (!reconnecting) {
                                      exproto->tx_frame_asm.
                                      set_is_rev1(tx_is_rev1);
                                      exproto->rx_frame_asm.
                                      ...
                                      auto back_to_close = std::bind([]
                                                                     ...
                                                                     std::
                                                                     move(cs));
                                      new_center->submit_to(new_center->
                                                            get_id(),
                                                            std::
                                                            move(back_to_close),
                                                            true); return;}
                                      ...
                                      auto transfer_existing =
                                      [existing, exproto] ()mutable {
                                      std::lock_guard < std::mutex >
                                      l(existing->lock);
                                      ...
                                      existing->last_connect_started =
                                      ceph::coarse_mono_clock::now();
                                      existing->last_tick_id =
                                      existing->center->
                                      create_time_event(existing->
                                                        connect_timeout_us,
                                                        existing->tick_handler);
                                      existing->state =
                                      AsyncConnection::
                                      STATE_CONNECTION_ESTABLISHED;
                                      existing->center->
                                      create_file_event(existing->cs.fd(),
                                                        EVENT_READABLE,
                                                        existing->read_handler);
                                      if (!exproto->reconnecting) {
                                      exproto->run_continuation(exproto->
                                                                send_server_ident
                                                                ());}
                                      else {
                                      exproto->run_continuation(exproto->
                                      ...
                                      existing->center->submit_to(existing->
                                                                  center->
                                                                  get_id(),
                                                                  std::
                                                                  move
                                                                  ...
                                                                  true);}
                                      , std::move(temp_cs));
                                 existing->center->submit_to(existing->center->
                                                             get_id(),
                                                             std::
                                                             move
                                                             (deactivate_existing),
                                 ...
                                 in_seq = 0; if (!connection->policy.lossy) {
                                 server_cookie =
                                 ceph::util::generate_random_number < uint64_t >
                                 (1, -1ll);}
                                 ...
                                 auto server_ident =
                                 ServerIdentFrame::Encode(messenger->
                                                          get_myaddrs(),
                                                          messenger->
                                                          get_myname().num(),
                                                          gs,
                                 ...
                                 INTERCEPT(12);
                                 return WRITE(server_ident, "server ident",
                                              server_ready);}
                                 ...
                                 ldout(cct, 20) << __func__ << dendl;
                                 out_seq =
                                 discard_requeued_up_to(out_seq, message_seq);
                                 uint64_t ms = in_seq;
                                 auto reconnect_ok =
                                 ...
                                 ms_deliver_handle_fast_accept(connection);
                                 INTERCEPT(14);
                                 return WRITE(reconnect_ok, "reconnect ok",
                                              server_ready);}

                                 CtPtr ProtocolV2::
                                 handle_compression_request(ceph::
                                                            bufferlist &
                                                            payload) {
                                 ...
                                 auto request =
                                 CompressionRequestFrame::Decode(payload);
                                 ldout(cct,
                                       10) << __func__ <<
                                 " CompressionRequestFrame(is_compress=" <<
                                 request.is_compress()
                                 << ", preferred_methods=" << request.
                                 preferred_methods() << ")" << dendl;
                                 const int peer_type =
                                 connection->get_peer_type();
                                 if (Compressor::CompressionMode mode =
                                     messenger->comp_registry.
                                     get_mode(peer_type,
                                              auth_meta->is_mode_secure());
                                     mode != Compressor::COMP_NONE
                                     && request.is_compress()) {
                                 comp_meta.con_method =
                                 messenger->comp_registry.pick_method(peer_type,
                                                                      request.
                                                                      preferred_methods
                                                                      ());
                                 ldout(cct,
                                       10) << __func__ <<
                                 " Compressor(pick_method=" << Compressor::
                                 get_comp_alg_name(comp_meta.get_method())
                                 << ")" << dendl;
                                 if (comp_meta.con_method !=
                                 ...
                                 auto response =
                                 CompressionDoneFrame::Encode(comp_meta.
                                                              is_compress(),
                                                              comp_meta.
                                                              get_method());
                                 INTERCEPT(20);
                                 return WRITE(response, "compression done",
                                              finish_compression);}
