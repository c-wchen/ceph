    ...
    struct BackfillState {
      ...
      private:
        template < class S > struct StateHelper {
            ...
            PGFacade & pg() {
                return *static_cast < S * >(this)->template context <
                    BackfillMachine > ().pg;
            }
      ...
      public:
        ...
        struct Enqueuing:sc::state < Enqueuing, BackfillMachine >,
            ...
            // completed yet.
            static bool all_enqueued(const PeeringFacade & peering_state,
          ...
          private:
            ...
            hobject_t earliest_peer_backfill(const std::map < pg_shard_t,
                                             BackfillInterval >
                                             &peer_backfill_info) const;
            bool should_rescan_replicas(const std::map < pg_shard_t,
                                        BackfillInterval > &peer_backfill_info,
                                        const BackfillInterval & backfill_info)
                const;
            ...
            bool should_rescan_primary(const std::map < pg_shard_t,
                                       BackfillInterval > &peer_backfill_info,
                                       const BackfillInterval & backfill_info)
                const;
            ...
            void trim_backfilled_object_from_intervals(result_t &&,
                                                       hobject_t &
        ...
        struct ReplicasScanning:sc::state < ReplicasScanning, BackfillMachine >,
            ...
            static bool replica_needs_scan(const BackfillInterval &
...
// conveyed as events; see ObjectPushed as an example.
    struct BackfillState::BackfillListener {
        virtual void request_replica_scan(const pg_shard_t & target,
                                          const hobject_t & begin,
                                          const hobject_t & end) = 0;

        virtual void request_primary_scan(const hobject_t & begin) = 0;

        virtual void enqueue_push(const hobject_t & obj,
                                  const eversion_t & v) = 0;

        virtual void enqueue_drop(const pg_shard_t & target,
                                  const hobject_t & obj,
                                  const eversion_t & v) = 0;

        virtual void maybe_flush() = 0;

        virtual void update_peers_last_backfill(const hobject_t &
                                                new_last_backfill) = 0;
...
// of behaviour that must be provided by a unit test's mock.
    struct BackfillState::PeeringFacade {
        ...
        virtual const hobject_t & get_peer_last_backfill(pg_shard_t peer) const
            = 0;
        ...
        virtual void update_complete_backfill_object_stats(const hobject_t &
                                                           hoid,
                                                           const pg_stat_t &
                                                           stats) = 0;
