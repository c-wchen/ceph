...
class AsyncCompressor {
  private:
    ...
    struct Job {
      ...
      Job(uint64_t i, bool compress):id(i), is_compress(compress) {
        ...
        Job(const Job & j):id(j.id), status(j.status.load()),
            is_compress(j.is_compress), data(j.data) {
        }
    };
    Mutex job_lock;
    ...
    struct CompressWQ:public ThreadPool::WorkQueue < Job > {
        ...
        AsyncCompressor *async_compressor;
         ...
         CompressWQ(AsyncCompressor * ac, time_t timeout,
                    time_t suicide_timeout, ThreadPool * tp)
        :ThreadPool::WorkQueue < Job > ("AsyncCompressor::CompressWQ", timeout,
                                        suicide_timeout, tp),
            async_compressor(ac) {
        ...
        Job *_dequeue() override {
            ...
            while (!job_queue.empty()) {
                ...
                if (item->status.
                    compare_exchange_strong(expected, status_t::WORKING)) {
                    break;
                }
                else {
        ...
        void _process(Job * item, ThreadPool::TPHandle &) override {
            ...
            if (!r) {
                ...
                assert(item->status.
                       compare_exchange_strong(expected, status_t::DONE));
            }
            else {
  ...
  public:
    ...
    int get_compress_data(uint64_t compress_id, bufferlist & data,
                          bool blocking, bool * finished);
    int get_decompress_data(uint64_t decompress_id, bufferlist & data,
                            bool blocking, bool * finished);
};
