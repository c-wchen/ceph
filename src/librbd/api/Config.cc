...
typedef std::map < std::string_view, std::pair < std::string,
        config_source_t >> Parent;
...
struct Options: Parent {
    ...
    Options(librados::IoCtx &io_ctx, bool image_apply_only_options)
    {
        ...
        CephContext *cct =
            reinterpret_cast < CephContext * >(m_io_ctx.cct());
    ...
    int init()
    {
        ...
        while (more_results) {
            ...
            int r =
                librbd::api::PoolMetadata <>::list(m_io_ctx,
                                                   last_key,
                                                   MAX_KEYS,
                                                   &pairs);
            if (r < 0) {
            ...
            for (auto kv : pairs) {
                std::string key;
                if (!util::
                    is_metadata_config_override(kv.first, &key)) {
                    more_results = false;
                    break;
                }
                auto it = find(key);
...
template < typename I >
bool Config < I >::is_option_name(I *image_ctx,
                                  const std::string &name)
{
                       ...
                       std::vector < config_option_t > *options)
{
    ...
    C_SaferCond ctx;
    auto req =
        image::GetMetadataRequest < I >::create(image_ctx->md_ctx,
    ...
    r = ctx.wait();
    if (r < 0) {
        lderr(cct) << "failed reading image metadata: " <<
                   cpp_strerror(r)
                   << dendl;
        return r;
    }
                                        ...
                                        ConfigProxy *config)
{
    ...
    int r = opts.init();
    if (r < 0) {
        lderr(cct) << "failed to read pool config overrides: " <<
                   cpp_strerror(r)
                   << dendl;
        return;
    }
