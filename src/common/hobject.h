...
struct hobject_t {
  ...
  :    
    ...
    pool(pool), nspace(nspace), key(oid.name == key ? string() : key) {
        build_hash_cache();
    }
    ...
    /// @return head version of this hobject_t hobject_t get_head() const {
        hobject_t ret(*this);
         ret.snap = CEPH_NOSNAP;
         return ret;
    }
    /// @return snapdir version of this hobject_t hobject_t get_snapdir() const {
        hobject_t ret(*this);
         ret.snap = CEPH_SNAPDIR;
         return ret;
    }
    /// @return true if object is snapdir bool is_snapdir() const {
        return snap == CEPH_SNAPDIR;
    }
    /// @return true if object is head bool is_head() const {
   ...
   */
    static set < string > get_prefixes(uint32_t bits,
                                       uint32_t mask, int64_t pool);
    ...
    // newer bit-reversed key uint32_t get_bitwise_key_u32() const {
        assert(!max);
        return hash_reverse_bits;
    } uint64_t get_bitwise_key() const {
        return max ? 0x100000000ull : hash_reverse_bits;
    }
    // please remember to update set_bitwise_key_u32() also// once you change build_hash_cache()
...
} // namespace std ostream & operator<<(ostream & out, const hobject_t & o);
...
{
    static_assert(always_false < T >::value::value,
...
struct ghobject_t {
  ...
  public:
    static const gen_t NO_GEN = UINT64_MAX;
     ...
     ghobject_t()
    :generation(NO_GEN), shard_id(shard_id_t::NO_SHARD), max(false) {
    } explicit ghobject_t(const hobject_t & obj)
    :hobj(obj), generation(NO_GEN), shard_id(shard_id_t::NO_SHARD), max(false) {
    }
...
} // namespace std ostream & operator<<(ostream & out, const ghobject_t & o);
