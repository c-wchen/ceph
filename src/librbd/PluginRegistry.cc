...
template < typename I > PluginRegistry < I >::PluginRegistry(I *image_ctx)
    : m_image_ctx(image_ctx),
      m_plugin_api(std::make_unique < plugin::Api < I >> ()),
      m_image_writeback(std::make_unique < cache::ImageWriteback <
                        I >> (*image_ctx))
{
...
template < typename I >
void PluginRegistry < I >::init(const std::string &plugins,
                                Context *on_finish)
{
    ...
    for (auto token : tokenizer) {
        ...
        auto plugin =
            dynamic_cast < plugin::Interface < I >
        ...
        plugin->init(m_image_ctx, *m_plugin_api, *m_image_writeback,
                     m_plugin_hook_points, ctx);
    }
...
template < typename I >
void PluginRegistry <
I >::acquired_exclusive_lock(Context *on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << dendl;
...
template < typename I >
void PluginRegistry <
I >::prerelease_exclusive_lock(Context *on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << dendl;
