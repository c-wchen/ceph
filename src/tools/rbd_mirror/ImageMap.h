        ...
        template < typename > struct Threads;
         ...
         template < typename ImageCtxT = librbd::ImageCtx > class ImageMap {
          public:
            ...
            void update_instances_added(const std::vector < std::string >
                                        &instances);
            void update_instances_removed(const std::vector < std::string >
                                          &instances);
          ...
          private:
            ...
            struct Update {
                ...
                utime_t mapped_time;
                 ...
                 Update(const std::string & global_image_id,
                        const std::string & instance_id, utime_t mapped_time)
                :global_image_id(global_image_id), instance_id(instance_id),
            ...
            void handle_peer_ack_remove(const std::string & global_image_id,
                                        int r);

            void handle_load(const std::map < std::string,
                             cls::rbd::MirrorImageMap > &image_mapping);
            void handle_update_request(const Updates & updates,
                                       const std::set < std::string >
                                       &remove_global_image_ids, int r);

            // continue (retry or resume depending on state machine) processing
            // current action.
            void continue_action(const std::set < std::string >
            ...
            void filter_instance_ids(const std::vector < std::string >
                                     &instance_ids,
