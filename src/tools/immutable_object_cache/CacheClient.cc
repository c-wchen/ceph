...
namespace ceph {
    namespace immutable_obj_cache {
        ...
        CacheClient::CacheClient(const std::string & file,
        ...
        :m_cct(ceph_ctx), m_io_service_work(m_io_service),
            ...
            if (m_worker_thread_num != 0) {
                ...
                for (uint64_t i = 0; i < m_worker_thread_num; i++) {
                    std::thread * thd = new std::thread([this] () {
                                                        m_worker->run();
                                                        }
                    );
        ...
        void CacheClient::connect(Context * on_finish) {
            m_dm_socket.async_connect(m_ep,
                                      boost::bind(&CacheClient::handle_connect,
                                                  this, on_finish,
        ...
        void CacheClient::lookup_object(std::string pool_nspace,
        ...
        void CacheClient::send_message() {
            ...
            boost::asio::async_write(m_dm_socket,
                                     ...
                                     [this,
                                      bl] (const boost::system::
                                           error_code & err, size_t cb) {
                                     if (err || cb != bl.length()) {
                                     fault(ASIO_ERROR_WRITE, err); return;}

                                     ceph_assert(cb == bl.length()); {
                                     std::lock_guard locker {
                                     m_lock}; if (m_outcoming_bl.length() == 0) {
                                     m_writing.store(false); return;}
                                     }
        ...
        void CacheClient::read_reply_header() {
            ...
            boost::asio::async_read(m_dm_socket,
                                    ...
                                    boost::bind(&CacheClient::
                                                handle_reply_header, this,
                                                bp_head,
                                                boost::asio::placeholders::
                                                error,
                                                boost::asio::placeholders::
        ...
        void CacheClient::read_reply_data(bufferptr && bp_head,
            ...
            boost::asio::async_read(m_dm_socket,
                                    ...
                                    boost::bind(&CacheClient::handle_reply_data,
                                                this, std::move(bp_head),
                                                std::move(bp_data), data_len,
                                                boost::asio::placeholders::
                                                error,
                                                boost::asio::placeholders::
                                                bytes_transferred));
