...
namespace librbd {
    namespace operation {
        ...
        template < typename I >
          ...
          public:
            C_CopyupObject(AsyncObjectThrottle < I > &throttle,
                           I * image_ctx,::SnapContext snapc,
                           uint64_t object_no)
          :    C_AsyncObjectThrottle < I > (throttle, *image_ctx),
                ...
                string oid = image_ctx.get_object_name(m_object_no);
                 ldout(image_ctx.cct,
                       10) << "removing (with copyup) " << oid << dendl;

                auto req =
                    io::ObjectRequest < I >::create_discard(&image_ctx, oid,
                                                            m_object_no, 0,
                                                            image_ctx.layout.
                                                            object_size,
                                                            m_snapc,
      ...
      :    AsyncRequest < I > (image_ctx, on_finish), m_new_size(new_size),
            ...
            m_num_objects =
                Striper::get_num_objects(image_ctx.layout, original_size);
        ...
        template < typename I > bool TrimRequest < I >::should_complete(int r) {
            ...
            if (r == -ERESTART) {
                ...
                lderr(cct) << "trim encountered an error: " << cpp_strerror(r)
                    << dendl;
                return true;
            }
        ...
        template < typename I > void TrimRequest < I >::send_pre_trim() {
...

            {
                ...
                if (image_ctx.object_map != nullptr) {
                    ...
                    RWLock::WLocker object_map_locker(image_ctx.
                                                      object_map_lock);
                    if (image_ctx.object_map->template aio_update <
                        AsyncRequest < I > >(CEPH_NOSNAP, m_delete_start_min,
                                             m_num_objects, OBJECT_PENDING,
                                             OBJECT_EXISTS, {
                                             }
                                             , false, this)) {
                        return;
        ...
        template < typename I > void TrimRequest < I >::send_copyup_objects() {
            ...
            uint64_t parent_overlap;
            {
                ...
                int r =
                    image_ctx.get_parent_overlap(CEPH_NOSNAP, &parent_overlap);
                assert(r == 0);
            }
        ...
        template < typename I > void TrimRequest < I >::send_post_trim() {
...

            {
                ...
                if (image_ctx.object_map != nullptr) {
                    ...
                    if (image_ctx.object_map->template aio_update <
                        AsyncRequest < I > >(CEPH_NOSNAP, m_delete_start_min,
                                             m_num_objects, OBJECT_NONEXISTENT,
                                             OBJECT_PENDING, {
                                             }
                                             , false, this)) {
                        return;
        ...
        template < typename I > void TrimRequest < I >::send_clean_boundary() {
            ...
            Striper::file_to_extents(cct, image_ctx.format_string,
                                     &image_ctx.layout, m_new_size, delete_len,
                                     0, extents);

            ContextCompletion *completion =
                new ContextCompletion(this->create_async_callback_context(),
                                      true);
            for (vector < ObjectExtent >::iterator p = extents.begin();
                ...
                auto req =
                    io::ObjectRequest < I >::create_discard(&image_ctx,
                                                            p->oid.name,
                                                            p->objectno,
                                                            p->offset,
                                                            p->length, snapc,
