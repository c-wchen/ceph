    ...
    static std::vector < fio_option > options = {
        make_option([](fio_option & o) {
                    ...
                    make_option([](fio_option & o) {
                                ...
                                o.type = FIO_OPT_STR_STORE;
                                o.help =
                                "Extra ceph arguments (e.g., -d --debug-rgw=16)";
                    ...
                    make_option([](fio_option & o) {
                                ...
                                FIO_OPT_G_INVALID;}),
                    make_option([](fio_option & o) {
                                o.name = "bucket_name"; o.lname = "S3 bucket";
                                o.type = FIO_OPT_STR_STORE;
      ...
      int argc; char *argv[8]; save_args():argc(1) {
                    argv[0] = strdup("librgw"); for (int ix = 1; ix < 8; ++ix) {
                    argv[ix] = nullptr;}
                    }
 ...
 */
                    ...
                    opt_struct & o =
                    *(reinterpret_cast < opt_struct * >(td->eo));
                    librgw_data * data = nullptr; int r = 0;
                    dprint(FD_IO, "fio_librgw_setup\n"); if (!td->io_ops_data) {
                    data = new librgw_data(td);
                    /* init args */
                    std::string sopt; if (o.config) {
                    sopt = fmt::format("--conf={}", o.config);
                    args.push_arg(sopt);}
                    std::cout << o.name << std::endl; if (o.name) {
                    ...
                    r =
                    rgw_mount2(data->rgw_h, o.userid, o.access_key,
                               o.secret_key, "/", &data->fs,
                               RGW_MOUNT_FLAG_NONE); if (! !r) {
                    dprint(FD_IO, "rgw_mount2 failed\n"); return r;}

                    /* go ahead and lookup the bucket as well */
                    r = rgw_lookup(data->fs, data->fs->root_fh, o.bucket_name,
                                   &data->bucket_fh, nullptr, 0,
                                   RGW_LOOKUP_FLAG_NONE);
                    if (!data->bucket_fh) {
                    dprint(FD_IO,
                           "rgw_lookup on bucket %s failed, will create\n",
                           o.bucket_name); struct stat st;
                    st.st_uid = o.owner_uid; st.st_gid = o.owner_gid;
                    st.st_mode = 755;
                    r =
                    rgw_mkdir(data->fs, data->fs->root_fh, o.bucket_name, &st,
                              create_mask, &data->bucket_fh,
                              RGW_MKDIR_FLAG_NONE); if (!data->bucket_fh) {
                    dprint(FD_IO, "rgw_mkdir for bucket %s failed\n",
                           o.bucket_name); return EINVAL;}
                    }

                    td->io_ops_data = data;}

                    td->o.use_thread = 1; if (r != 0) {
                    abort();}
 ...
 */
                    ...
                    int r = 0; dprint(FD_IO, "fio_librgw_cleanup\n");
                    /* cleanup specific data */
                    librgw_data * data =
                    static_cast < librgw_data * >(td->io_ops_data); if (data) {

                    /* release active handles */
                    data->release_handles(); if (data->bucket_fh) {
                    r = rgw_fh_rele(data->fs, data->bucket_fh, 0 /* flags */ );
                    }
                    r = rgw_umount(data->fs, RGW_UMOUNT_FLAG_NONE);
                    librgw_shutdown(data->rgw_h);
                    td->io_ops_data = nullptr; delete data;}
                    }
 ...
 */
                    static int fio_librgw_prep(struct thread_data *td,
                                               struct io_u *io_u) {
                    return 0;}
 ...
 */
                    static struct io_u *fio_librgw_event(struct thread_data *td,
                                                         int event) {
                    return NULL;}
 ...
 * numbers. Required.
 */
                    static int fio_librgw_getevents(struct thread_data *td,
                                                    unsigned int min,
                                                    unsigned int max,
 ...
 */
                    static int fio_librgw_cancel(struct thread_data *td,
                                                 struct io_u *io_u) {
                    return 0;}
 ...
 */
                    static enum fio_q_status fio_librgw_queue(struct thread_data
                                                              *td,
                                                              struct io_u *io_u)
                    {
                    librgw_data * data =
                    static_cast < librgw_data * >(td->io_ops_data);
                    const char *object = io_u->file->file_name;
                    struct rgw_file_handle *object_fh = nullptr; size_t nbytes;
                    int r = 0;
                    /*
                     * Double sanity check to catch errant write on a readonly setup
                     */
                    fio_ro_check(td, io_u); if (io_u->ddir == DDIR_WRITE) {
                    /* Do full write cycle */
                    r =
                    rgw_lookup(data->fs, data->bucket_fh, object, &object_fh,
                               nullptr, 0, RGW_LOOKUP_FLAG_CREATE); if (! !r) {
                    dprint(FD_IO,
                           "rgw_lookup failed to create filehandle for %s\n",
                           object); goto out;}

                    r =
                    rgw_open(data->fs, object_fh, 0 /* posix flags */ ,
                             0 /* flags */ );
                    if (! !r) {
                    dprint(FD_IO,
                           "rgw_open failed to create filehandle for %s\n",
                           object);
                    ...
                    r =
                    rgw_write(data->fs, object_fh, 0, io_u->xfer_buflen,
                              &nbytes, (void *)io_u->xfer_buf,
                              RGW_WRITE_FLAG_NONE); if (! !r) {
                    ...
                    r =
                    rgw_lookup(data->fs, data->bucket_fh, object, &object_fh,
                               nullptr, 0, RGW_LOOKUP_FLAG_NONE); if (! !r) {
                    dprint(FD_IO,
                           "rgw_lookup failed to create filehandle for %s\n",
                           object); goto out;}
                    ...
                    r =
                    rgw_read(data->fs, object_fh, io_u->offset,
                             io_u->xfer_buflen, &nbytes, io_u->xfer_buf,
                             RGW_READ_FLAG_NONE); if (! !r) {
 ...
 */
                    static int fio_librgw_open(struct thread_data *td,
                                               struct fio_file *f) {
 ...
 */
                    static int fio_librgw_close(struct thread_data *td,
                                                struct fio_file *f) {
                    /* for now, let's try to avoid doing open/close in these hooks */
                    return 0;}
...
/* XXX next two probably not needed */
                    ...
                    u->engine_data = nullptr; return 0;}
...
// the exported fio engine interface
                    ...
                    static librgw_ioengine ioengine; *ioengine_ptr = &ioengine;}
                    }           // extern "C"
