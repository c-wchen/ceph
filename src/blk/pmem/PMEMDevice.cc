...
#if defined(HAVE_LIBDML)
    auto result = dml::execute < execution_path > (dml::mem_move, dml::make_view(addr + off, len), dml::make_view(p.c_str(),
                  len));
    ceph_assert(result.status == dml::status_code::ok);
#else
...
#if defined(HAVE_LIBDML)
    auto result = dml::execute < execution_path > (dml::mem_move, dml::make_view(addr + off, len), dml::make_view(buf,
                  len));
    ceph_assert(result.status == dml::status_code::ok);
#else
