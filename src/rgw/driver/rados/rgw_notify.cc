...
private:
    ...
    int read_queue_list(queues_t &queues, optional_yield y)
    {
        ...
        while (more) {
            ...
            const auto ret = rgw_rados_operate(this, rados_store.getRados()->get_notif_pool_ctx(), Q_LIST_OBJECT_NAME, &op, nullptr,
                                               y);
            if (ret == -ENOENT) {
    ...
    void cleanup_queue(const std::string& queue_name, boost::asio::yield_context yield)
    {
        while (!shutdown) {
            ...
            if (ret == -EBUSY) {
                ldpp_dout(this, 5) << "WARNING: queue: " << queue_name << " ownership moved to another daemon. processing will stop" <<
                                   dendl;
                return;
            }
            if (ret < 0) {
        ...
        while (!shutdown) {
                ...
                push_endpoint = push_endpoint.get(), &topic_info](boost::asio::yield_context yield) {
                    ...
                    if (result == EntryProcessingResult::Successful || result == EntryProcessingResult::Expired
                        ...
                        if (set_min_marker(end_marker, entry.marker) < 0) {
                            ldpp_dout(this, 1) << "ERROR: cannot determine minimum between malformed markers: " << end_marker << ", " <<
                                               entry.marker << dendl;
                        } else {
            ...
            if (remove_entries) {
                ...
                if (!entries_to_migrate.empty()) {
                    ...
                    if (ret < 0) {
                        ldpp_dout(this, 1) << "ERROR: failed to reserve migration space on queue: " << queue_name << ". error: " << ret <<
                                           dendl;
                        return;
                    }
    ...
    void process_queues(boost::asio::yield_context yield)
    {
        ...
        while (processed_queue_count > 0) {
            ldpp_dout(this, 5) << "INFO: manager stopped. " << processed_queue_count << " queues are still being processed" <<
                               dendl;
                         ...
                         const std::string& topic_queue, optional_yield y)
{
    if (topic_queue == Q_LIST_OBJECT_NAME) {
        ldpp_dout(dpp, 1) << "ERROR: topic name cannot be: " << Q_LIST_OBJECT_NAME << " (conflict with queue list object name)"
                          << dendl;
        return -EINVAL;
    }
    librados::ObjectWriteOperation op;
...
int remove_persistent_topic(const DoutPrefixProvider* dpp, librados::IoCtx& rados_ioctx, const std::string& topic_queue,
                            optional_yield y)
{
    librados::ObjectWriteOperation op;
    op.remove();
