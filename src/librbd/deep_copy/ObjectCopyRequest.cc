...
ObjectCopyRequest < I >::ObjectCopyRequest(I *src_image_ctx,
    ...
    : m_src_image_ctx(src_image_ctx), m_dst_image_ctx(dst_image_ctx),
      m_cct(dst_image_ctx->cct), m_src_snap_id_start(src_snap_id_start),
...
void ObjectCopyRequest < I >::send_list_snaps()
{
    ...
    std::tie(m_image_extents, m_image_area) =
        io::util::object_to_area_extents(m_dst_image_ctx,
    m_dst_object_number, { {
            0,
            m_dst_image_ctx->layout.
            object_size
        }
    });
    ldout(m_cct,
          20) << "image_extents=" << m_image_extents << " area=" <<
              m_image_area << dendl;

    auto ctx =
        create_async_context_callback(*m_src_image_ctx,
                                      create_context_callback <
                                      ObjectCopyRequest,
                                      &ObjectCopyRequest <
                                      I >::handle_list_snaps > (this));
    ...
    auto list_snaps_flags =
        io::LIST_SNAPS_FLAG_DISABLE_LIST_FROM_PARENT;
    ...
    auto aio_comp =
        io::AioCompletion::create_and_start(ctx,
                                            ...
                                            io::AIO_TYPE_GENERIC);
    auto req =
        io::ImageDispatchSpec::create_list_snaps(*m_src_image_ctx,
            io::
            IMAGE_DISPATCH_LAYER_NONE,
            aio_comp,
            io::
    Extents {
        m_image_extents }
...
void ObjectCopyRequest < I >::handle_list_snaps(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to list snaps: " << cpp_strerror(r) <<
                     dendl;
        finish(r);
        return;
    }
...
template < typename I > void ObjectCopyRequest < I >::send_read()
{
    ...
    io::ReadResult read_result {
        &read_op.image_extent_map, &read_op.out_bl};
    ...
    auto ctx = create_context_callback <
               ObjectCopyRequest < I >,
               &ObjectCopyRequest < I >::handle_read > (this);
    auto aio_comp =
        io::AioCompletion::create_and_start(ctx,
                                            get_image_ctx
                                            (m_src_image_ctx),
    ...
    auto req =
        io::ImageDispatchSpec::create_read(*m_src_image_ctx,
                                           io::
                                           IMAGE_DISPATCH_LAYER_INTERNAL_START,
                                           aio_comp,
...
template < typename I > void ObjectCopyRequest < I >::handle_read(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to read from source object: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
void ObjectCopyRequest < I >::send_update_object_map()
{
    ...
    ldout(m_cct,
          20) << "dst_snap_id=" << dst_snap_id << ", object_state=" <<
    ...
    bool sent = dst_image_ctx->object_map->template aio_update <
                    Context, &Context::complete > (dst_snap_id, m_dst_object_number,
    object_state, {
    }, {
    }, false, ctx);
...
void ObjectCopyRequest < I >::handle_update_object_map(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to update object map: " <<
...
template < typename I > void ObjectCopyRequest < I >::process_copyup()
{
    ...
    auto r =
        m_dst_image_ctx->io_object_dispatcher->
        prepare_copyup(m_dst_object_number,
                       &m_snapshot_sparse_bufferlist);
    if (r < 0) {
        lderr(m_cct) << "failed to prepare copyup data: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
void ObjectCopyRequest < I >::send_write_object()
{
    ...
    auto &sparse_bufferlist =
        m_snapshot_sparse_bufferlist.begin()->second;
    ...
    librados::snap_t src_snap_seq =
        m_snapshot_sparse_bufferlist.begin()->first;
    if (src_snap_seq != 0) {
        ...
        auto dst_may_exist_it =
            m_dst_object_may_exist.find(dst_snap_id);
        ...
        if (!dst_may_exist_it->second && !sparse_bufferlist.empty()) {
            ...
            ldout(m_cct,
                  20) << "object DNE: src_snap_seq=" << src_snap_seq <<
                      dendl;
        ...
        dst_snap_ids = SnapIds {
            dst_snap_ids_it, snap_map_it->second.end()};
    ...
    bool migration =
        ((m_flags & OBJECT_COPY_REQUEST_FLAG_MIGRATION) != 0);
    if (migration) {
        ldout(m_cct,
              20) << "assert_snapc_seq=" << dst_snap_seq << dendl;
        cls_client::assert_snapc_seq(&op, dst_snap_seq,
                                     cls::rbd::
                                     ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ);
    }
...
void ObjectCopyRequest < I >::handle_write_object(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to write to destination object: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
    ...
    m_snapshot_sparse_bufferlist.erase(m_snapshot_sparse_bufferlist.
                                       begin());
    if (!m_snapshot_sparse_bufferlist.empty()) {
        send_write_object();
        return;
...
template < typename I >
Context *ObjectCopyRequest <
I >::start_lock_op(ceph::shared_mutex &owner_lock, int *r)
{
...
template < typename I > void ObjectCopyRequest < I >::compute_read_ops()
{
    ...
    for (auto &[key, image_intervals] : m_snapshot_delta) {
        io::WriteReadSnapIds write_read_snap_ids {
            key};
        ...
        if (write_read_snap_ids.first > m_src_snap_id_start) {
            ...
            auto snap_map_it =
                m_snap_map.find(write_read_snap_ids.first);
            ceph_assert(snap_map_it != m_snap_map.end());
            auto dst_snap_seq = snap_map_it->second.front();

            auto dst_may_exist_it =
                m_dst_object_may_exist.find(dst_snap_seq);
            ceph_assert(dst_may_exist_it !=
                        m_dst_object_may_exist.end());
            if (!dst_may_exist_it->second) {
                ldout(m_cct,
                      20) << "DNE snapshot: " << write_read_snap_ids.
                          first << dendl;
                continue;
            }
        }
        ...
        for (auto &image_interval : image_intervals) {
            ...
            switch (state) {
                case io::SPARSE_EXTENT_STATE_DNE:
                    if (write_read_snap_ids ==
                        io::INITIAL_WRITE_READ_SNAP_IDS
                        && read_from_parent) {
                        ...
                        dne_image_interval.insert(image_interval.get_off(),
                ...
                case io::SPARSE_EXTENT_STATE_DATA:
                    ldout(m_cct, 20) << "read op: "
                                     ...
                                     << image_interval.get_off() << "~"
                                     << image_interval.get_len() << dendl;
                    m_read_ops[write_read_snap_ids].image_interval.
                    union_insert(image_interval.get_off(),
                                 image_interval.get_len());
                    only_dne_extents = false;
                    break;
                default:
    ...
    if (!dne_image_interval.empty() && (!only_dne_extents || flatten)) {
        ...
        WriteReadSnapIds write_read_snap_ids {
        ...
        int r =
            m_src_image_ctx->get_parent_overlap(src_snap_seq,
                                                &raw_overlap);
        if (r < 0) {
            ldout(m_cct,
            ...
            m_src_image_ctx->prune_parent_extents(parent_extents,
                                                  m_image_area,
                                                  raw_overlap, false);
            for (auto[image_offset, image_length] : parent_extents) {
                ldout(m_cct,
                      20) << "parent read op: " << "snap_ids=" <<
                          write_read_snap_ids << " " << image_offset << "~" <<
                          image_length << dendl;
                m_read_ops[write_read_snap_ids].image_interval.
                union_insert(image_offset, image_length);
            }
        }
    }
...
template < typename I > void ObjectCopyRequest < I >::merge_write_ops()
{
    ...
    for (auto &[write_read_snap_ids, read_op] : m_read_ops) {
        ...
        for (auto[image_offset, image_length] : read_op.
             image_extent_map) {
            image_data_interval.union_insert(image_offset,
                                             image_length);
        }
        ...
        intersection.intersection_of(read_op.image_interval,
                                     image_data_interval);
        ...
        for (auto &[image_offset, image_length] : read_op.
            ...
            ldout(m_cct,
                  20) << "src_snap_seq=" << src_snap_seq << ", " <<
                      ...
                      image_length << dendl;
            m_dst_zero_interval[src_snap_seq].union_insert(image_offset,
                    image_length);
        }
        ...
        for (auto[image_offset, image_length] : read_op.
            ...
            io::util::area_to_object_extents(m_dst_image_ctx,
            ...
            for (auto &object_extent : object_extents) {
                ldout(m_cct,
                      20) << "src_snap_seq=" << src_snap_seq << ", " <<
                          "object_offset=" << object_extent.
                          offset << ", " << "object_length=" << object_extent.
                          length << dendl;

                bufferlist sub_bl;
                sub_bl.substr_of(read_op.out_bl, buffer_offset,
                                 object_extent.length);

                m_snapshot_sparse_bufferlist[src_snap_seq].
                insert(object_extent.offset, object_extent.length, {
                    io::SPARSE_EXTENT_STATE_DATA,
                    object_extent.length, std::move(sub_bl)
                }
                      );
...
template < typename I > void ObjectCopyRequest < I >::compute_zero_ops()
{
    ...
    bool object_exists = (m_src_snap_id_start > 0 &&
                          m_snapshot_delta.count({ m_src_snap_id_start,
                              m_src_snap_id_start }
                                                ) == 0);
    bool fast_diff =
        m_dst_image_ctx->test_features(RBD_FEATURE_FAST_DIFF);
    ...
    for (auto &it : m_dst_zero_interval) {
        ...
        if (!dst_may_exist_it->second && object_exists) {
            ldout(m_cct,
                  5) << "object DNE for snap_id: " << dst_snap_seq <<
                     dendl;
            m_snapshot_sparse_bufferlist[src_snap_seq].insert(0,
                    m_dst_image_ctx->
                    layout.
            object_size, {
                io::
        ...
        if (hide_parent) {
            ...
            int r =
                m_dst_image_ctx->get_parent_overlap(dst_snap_seq,
                                                    &raw_overlap);
            if (r < 0) {
                ldout(m_cct,
                      5) <<
                         "failed getting parent overlap for snap_id: " <<
                         dst_snap_seq << ": " << cpp_strerror(r) << dendl;
            } else if (raw_overlap > 0) {
                auto parent_extents = m_image_extents;
                object_overlap =
                    m_dst_image_ctx->
                    prune_parent_extents(parent_extents, m_image_area,
                                         raw_overlap, false);
            }
            if (object_overlap == 0) {
                        ...
                        if (write_read_snap_ids !=
                            ...
                            ldout(m_cct,
                                  20) << "zeroed extent: " <<
                                      "src_snap_seq=" << src_snap_seq << " " <<
                                      image_interval.
                                      get_off() << "~" << image_interval.
                                      get_len() << dendl;
                            zero_interval.union_insert(image_interval.
                                                       get_off(),
                                                       image_interval.
                                                       get_len());
                        } else if (hide_parent &&
        ...
        if (iter != m_snapshot_sparse_bufferlist.end()) {
            for (auto &sparse_bufferlist : iter->
                 second) {
                object_exists = true;
                end_size =
                    std::max(end_size,
                             sparse_bufferlist.get_off() +
                             sparse_bufferlist.get_len());
            }
        }
        ...
        for (auto z = zero_interval.begin(); z != zero_interval.end();
             ++z) {
            // convert image extents back to object extents for the write op
            striper::LightweightObjectExtents object_extents;
            io::util::area_to_object_extents(m_dst_image_ctx,
                                             z.get_start(), z.get_len(),
                                             m_image_area, 0,
                                             &object_extents);
            for (auto &object_extent : object_extents) {
                ceph_assert(object_extent.offset +
                            object_extent.length <=
                            m_dst_image_ctx->layout.object_size);

                if (object_extent.offset + object_extent.length >=
                    end_size) {
                    // zero interval at the object end
                    if ((object_extent.offset == 0 && hide_parent) ||
                        (object_extent.offset < prev_end_size)) {
                        ldout(m_cct,
                              20) << "truncate " << object_extent.
                                  offset << dendl;
                        auto length =
                            m_dst_image_ctx->layout.object_size -
                            object_extent.offset;
                        m_snapshot_sparse_bufferlist[src_snap_seq].
                        insert(object_extent.offset, length, {
                            io::SPARSE_EXTENT_STATE_ZEROED,
                            length
                        }
                              );
                    ...
                    object_exists = (object_extent.offset > 0
                                     || hide_parent);
        ...
        if (object_exists) {
            ...
            if (fast_diff
                && m_snapshot_sparse_bufferlist.count(src_snap_seq) ==
                0) {
...
void ObjectCopyRequest < I >::compute_dst_object_may_exist()
{
    ...
    for (auto snap_id : snap_ids) {
        m_dst_object_may_exist[snap_id] =
            (m_dst_object_number <
             m_dst_image_ctx->get_object_count(snap_id));
    }

    ldout(m_cct,
          20) << "dst_object_may_exist=" << m_dst_object_may_exist <<
              dendl;
}
