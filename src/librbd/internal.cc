...
#define READ_SIZE 4096
    ...
    static_assert(sizeof(RBD_HEADER_TEXT) ==
           ...
           bool skip_mirror_enable)
{
    ...
    if (opts.get(RBD_IMAGE_OPTION_FLATTEN, &option) == 0) {
        lderr(cct) << "create does not support 'flatten' image option" <<
                   dendl;
        return -EINVAL;
    }
    ...
    if (old_format) {
        ...
        image::CreateRequest <> *req =
            image::CreateRequest <>::create(config, io_ctx, image_name, id,
                                            size, opts, create_flags,
                                            static_cast <
          ...
          const std::string &primary_mirror_uuid)
{
    ...
    if (c_opts.get(RBD_IMAGE_OPTION_FLATTEN, &flatten) == 0) {
        lderr(cct) << "clone does not support 'flatten' image option" <<
                   dendl;
        return -EINVAL;
    }
    ...
    ldout(cct, 10) << __func__ << " "
                   << "c_name=" << c_name << ", "
                   << "c_id= " << clone_id << ", " << "c_opts=" << c_opts << dendl;

    ConfigProxy config {
        reinterpret_cast < CephContext * >(c_ioctx.cct())->_conf};
    ...
    C_SaferCond cond;
    auto *req =
        image::CloneRequest <>::create(config, p_ioctx, parent_id,
    p_snap_name, {
        cls::rbd::UserSnapshotNamespace {}
    }
    , CEPH_NOSNAP, c_ioctx, c_name,
    clone_id, c_opts,
...
int get_overlap(ImageCtx *ictx, uint64_t *overlap)
{
    ...
    *overlap =
        (_overlap.second == io::ImageArea::DATA ? _overlap.first : 0);
    return 0;
}
         ...
         size_t sparse_size)
{
    ...
    uint64_t option;
    if (opts.get(RBD_IMAGE_OPTION_FLATTEN, &option) == 0) {
        lderr(cct) << "copy does not support 'flatten' image option" <<
                   dendl;
        return -EINVAL;
    }
    if (opts.get(RBD_IMAGE_OPTION_CLONE_FORMAT, &option) == 0) {
    ...
    ImageCtx *dest =
        new librbd::ImageCtx(destname, "", nullptr, dest_md_ctx,
                             false);
    r = dest->state->open(0);
    if (r < 0) {
    ...
    } void finish(int r) override
    {
        ...
        while (offset < length) {
            if (util::calc_sparse_extent(m_ptr,
                ...
                write_bl->
                push_back(buffer::ptr_node::
                          create(m_ptr, write_offset, write_length));
                Context *ctx =
                ...
                api::Io <>::aio_write(*m_dest, comp,
                                      m_offset + write_offset, write_length,
                                      std::move(*write_bl),
         ...
         size_t sparse_size)
{
    ...
    r = ctx.wait();
    if (r < 0) {
        lderr(cct) << "failed to copy metadata: " << cpp_strerror(r) <<
                   dendl;
        return r;
    }
    ...
    SimpleThrottle throttle(src->config.get_val < uint64_t >
    ...
    for (uint64_t offset = 0; offset < src_size; offset += period) {
        ...
        auto comp =
            io::AioCompletion::create_and_start < Context > (ctx, src,
                ...
                AIO_TYPE_READ);
        auto req =
            io::ImageDispatchSpec::create_read(*src,
                                               io::
         ...
         const string &tag)
{
     ...
     */
    {
        ...
        r = rados::cls::lock::lock(&ictx->md_ctx, ictx->header_oid,
                                   RBD_LOCK_NAME,
                                   exclusive ? ClsLockType::
                                   EXCLUSIVE : ClsLockType::SHARED, cookie,
                                   tag, "", utime_t(), 0);
        if (r < 0) {
            return r;
        }
               ...
               const string &cookie)
{
    ...
    if (ictx->config.get_val < bool > ("rbd_blocklist_on_break_lock")) {
        ...
        r = rados.blocklist_add(client_address,
                                ictx->config.get_val < uint64_t >
                                ("rbd_blocklist_expire_seconds"));
        if (r < 0) {
            lderr(ictx->
                  cct) << "unable to blocklist client: " << cpp_strerror(r)
                       << dendl;
            return r;
        }
    }
                     ...
                     void *arg)
{
    ...
    while (left > 0) {
        ...
        auto c = io::AioCompletion::create_and_start(&ctx, ictx,
                 io::AIO_TYPE_READ);
        auto req =
            io::ImageDispatchSpec::create_read(*ictx,
                                               io::
        IMAGE_DISPATCH_LAYER_NONE, c, {
            {off, read_len}
        }
        , io::ImageArea::DATA,
        io::ReadResult { &bl }
                  ...
                  map < string, bufferlist > *pairs)
{
    ...
    C_SaferCond ctx;
    auto req =
        image::GetMetadataRequest <>::create(ictx->md_ctx, ictx->header_oid,
            false, "", start, max, pairs,
            &ctx);
    req->send();
    ...
    for (auto &i : librbd::IMAGE_OPTIONS_TYPE_MAPPING) {
        if (i.second == librbd::STR) {
            ...
            if (opts.get(i.first, &val) == 0) {
                os << delimiter << librbd::image_option_name(i.
                        first) << "=" <<
                   val;
                delimiter = ", ";
            }
