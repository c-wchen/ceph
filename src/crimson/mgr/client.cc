...
namespace crimson::mgr {
    ...
    std::optional < seastar::future <>>
        ...
        gate.dispatch_in_background(__func__, *this,
                                    ...
                                    switch (m->get_type()) {
case MSG_MGR_MAP:
return handle_mgr_map(conn, boost::static_pointer_cast < MMgrMap > (m)); case MSG_MGR_CONFIGURE:
return handle_mgr_conf(conn, boost::static_pointer_cast < MMgrConfigure > (m)); default:
                                    dispatched = false;
                                    return seastar::now();}
                                    }
        ) ;
    ...
    void Client::ms_handle_reset(crimson::net::ConnectionRef c,
                                 bool /* is_replace */ ) {
        gate.dispatch_in_background(__func__, *this,[this, c] {
                                    if (conn == c) {
                                    report_timer.cancel(); return reconnect();}
                                    else {
                                    return seastar::now();}
                                    }
        ) ;
    ...
    seastar::future <> Client::reconnect() {
        ...
        auto retry_interval =
            std::chrono::duration < double >(local_conf().get_val <
                                             double
        ...
        return seastar::sleep(a_while).then([this] {
                                            ...
                                            logger().
                                            error
                                            ("mgr.{} does not have an addr compatible with me",
                                             mgrmap.get_active_name()); return;}
                                            conn =
                                            msgr.connect(peer,
    ...
    void Client::report() {
        gate.dispatch_in_background(__func__, *this,[this] {
                                    ...
                                    logger().
                                    warn
                                    ("report: no conn available; raport skipped");
                                    return seastar::now();}
