    ...
    namespace mirror {
        ...
        namespace {
            ...
            const std::vector < std::string > UNIQUE_PEER_CONFIG_KEYS {
                {
            "monmap", "mon_host", "mon_dns_srv_name", "key", "keyfile",
                        "keyring"}};

             template < typename I > class PoolReplayerAdminSocketCommand {
              public:
                PoolReplayerAdminSocketCommand(PoolReplayer < I >
                                               *pool_replayer)
          ...
          template < typename I > class StatusCommand:public PoolReplayerAdminSocketCommand < I >
            {
              ...
              :    
                PoolReplayerAdminSocketCommand < I >
                (pool_replayer) {
                }
          ...
          template < typename I > class PoolReplayerAdminSocketHook:public AdminSocketHook
            {
              public:
                PoolReplayerAdminSocketHook(CephContext * cct,
              ...
              :    admin_socket(cct->get_admin_socket()) {
                    ...
                    if (r == 0) {
                        commands[command] =
                            new StatusCommand < I > (pool_replayer);
                    }
                    ...
                    if (r == 0) {
                        commands[command] =
                            new StartCommand < I > (pool_replayer);
                    }
                    ...
                    if (r == 0) {
                        commands[command] =
                            new StopCommand < I > (pool_replayer);
                    }
                    ...
                    if (r == 0) {
                        commands[command] =
                            new RestartCommand < I > (pool_replayer);
                    }
                    ...
                    if (r == 0) {
                        commands[command] =
                            new FlushCommand < I > (pool_replayer);
                    }
                    ...
                    if (r == 0) {
                        commands[command] =
                            new LeaderReleaseCommand < I > (pool_replayer);
                    }
                }
        ...
        template < typename I >
            struct PoolReplayer <I >::
            RemotePoolPollerListener:public remote_pool_poller::Listener {
        ...
        template < typename I >
            PoolReplayer < I >::PoolReplayer(Threads < I > *threads,
                                             ...
                                             *service_daemon,
                                             ...
                                             const std::vector <
                                             const char
                                             *>&args):m_threads(threads),
            m_service_daemon(service_daemon),
            m_cache_manager_handler(cache_manager_handler),
            m_pool_meta_cache(pool_meta_cache), m_local_pool_id(local_pool_id),
            m_peer(peer), m_args(args),
            m_lock(ceph::
                   make_mutex("rbd::mirror::PoolReplayer " + stringify(peer))),
            m_pool_replayer_thread(this), m_leader_listener(this) {
        }
        ...
        template < typename I > bool PoolReplayer < I >::is_running()const {
            ...
            if (r < 0) {
                m_callout_id =
                    m_service_daemon->add_or_update_callout(m_local_pool_id,
                                                            m_callout_id,
                                                            service_daemon::
                                                            CALLOUT_LEVEL_ERROR,
            ...
            librbd::api::Config < I >::apply_pool_overrides(m_local_io_ctx,
                                                            &cct->_conf);
            ...
            if (r < 0) {
                derr << "failed to retrieve local mirror uuid from pool "
                    << m_local_io_ctx.
                    get_pool_name() << ": " << cpp_strerror(r) << dendl;
                m_callout_id =
                    m_service_daemon->add_or_update_callout(m_local_pool_id,
                                                            m_callout_id,
                                                            service_daemon::
            ...
            r = m_remote_rados->ioctx_create(m_local_io_ctx.get_pool_name().
                                             c_str(), m_remote_io_ctx);
            if (r < 0) {
                derr << "error accessing remote pool " << m_local_io_ctx.
                    get_pool_name()
                    << ": " << cpp_strerror(r) << dendl;
                m_callout_id =
                    m_service_daemon->add_or_update_callout(m_local_pool_id,
            ...
            if (r < 0) {
                derr << "failed to initialize remote pool poller: " <<
                    cpp_strerror(r)
                    << dendl;
                m_callout_id =
                    m_service_daemon->add_or_update_callout(m_local_pool_id,
            ...
            m_pool_meta_cache->set_remote_pool_meta(m_remote_io_ctx.get_id(),
                                                    m_remote_pool_meta);
            m_pool_meta_cache->set_local_pool_meta(m_local_io_ctx.get_id(), {
                                                   m_local_mirror_uuid}
            );
            ...
            r = on_init.wait();
            if (r < 0) {
                derr << "error initializing default namespace replayer: " <<
                    cpp_strerror(r)
                    << dendl;
                m_callout_id =
                    m_service_daemon->add_or_update_callout(m_local_pool_id,
            ...
            m_leader_watcher.reset(LeaderWatcher <
                                   I >::create(m_threads, m_local_io_ctx,
                                               &m_leader_listener));
            r = m_leader_watcher->init();
            if (r < 0) {
                derr << "error initializing leader watcher: " << cpp_strerror(r)
                    << dendl;
                m_callout_id =
                    m_service_daemon->add_or_update_callout(m_local_pool_id,
                                                            m_callout_id,
            ...
            m_service_daemon->add_or_update_attribute(m_local_io_ctx.get_id(),
        ...
        template < typename I > void PoolReplayer < I >::shut_down() {
            ...
            if (m_remote_pool_poller) {
                ...
                m_pool_meta_cache->remove_remote_pool_meta(m_remote_io_ctx.
                                                           get_id());
                m_pool_meta_cache->remove_local_pool_meta(m_local_io_ctx.
                                                          get_id());
            }
            m_remote_pool_poller.reset();
        ...
        template < typename I >
            ...
            if (client_name.empty() || !iparams.name.from_str(client_name)) {
                derr << "error initializing cluster handle for " << description
                    << dendl;
                return -EINVAL;
            }
            ...
            if (strip_cluster_overrides) {
              ...
              for (auto & key:UNIQUE_PEER_CONFIG_KEYS) {
                    config_values[key] =
                        cct->_conf.get_val < std::string > (key);
                }
            }
            ...
            if (!m_args.empty()) {
                ...
                r = cct->_conf.parse_argv(args);
                if (r < 0) {
                    derr << "could not parse command line args for " <<
            ...
            if (strip_cluster_overrides) {
              ...
              for (auto & pair:config_values) {
                    auto value =
                        cct->_conf.get_val < std::string > (pair.first);
        ...
        template < typename I > void PoolReplayer < I >::run() {
            ...
            while (true) {
                std::string asok_hook_name =
                    m_local_io_ctx.get_pool_name() + " " + m_peer.cluster_name;
                if (m_asok_hook_name != asok_hook_name
                    || m_asok_hook == nullptr) {
                    m_asok_hook_name = asok_hook_name;
                    delete m_asok_hook;

                    m_asok_hook =
                        new PoolReplayerAdminSocketHook < I > (g_ceph_context,
                                                               m_asok_hook_name,
                                                               this);
                }

                with_namespace_replayers([this] () {
        ...
        template < typename I >
            ...
            for (auto it = m_namespace_replayers.begin();
                ...
                if (iter == mirroring_namespaces.end()) {
                    ...
                    auto on_shut_down = new LambdaContext([namespace_replayer,
                                                          ...
                                                          namespace_replayer;
                                                          ctx->complete(r);});
                    m_service_daemon->remove_namespace(m_local_pool_id,
                                                       it->first);
          ...
          for (auto & name:mirroring_namespaces) {
                auto namespace_replayer =
                    NamespaceReplayer < I >::create(name, m_local_io_ctx,
                                                    m_remote_io_ctx,
                                                    m_local_mirror_uuid,
                                                    m_peer.uuid,
                                                    m_remote_pool_meta,
                ...
                auto on_init =
                    new
                    LambdaContext([this, namespace_replayer, name,
                                   &mirroring_namespaces,
                                   ctx = gather_ctx->new_sub()](int r){
                                  std::lock_guard locker {m_lock}; if (r < 0) {
                                  derr <<
                                  ...
                                  m_namespace_replayers[name] =
                                  namespace_replayer;
                                  m_service_daemon->
                                  add_namespace(m_local_pool_id, name);}
                                  ctx->complete(r);}
                ) ;
            ...
            if (m_leader) {
              ...
              for (auto & name:mirroring_namespaces) {
                    namespace_replayer_acquire_leader(name,
                                                      acquire_gather_ctx->
                                                      new_sub());
                }
        ...
        template < typename I >
            int PoolReplayer < I >::list_mirroring_namespaces(std::set <
                                                              std::string >
                                                              *namespaces) {
            ceph_assert(ceph_mutex_is_locked(m_lock));
            ...
            if (r < 0) {
                derr << "failed to list namespaces: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
          ...
          for (auto & name:names) {
                ...
                int r =
                    librbd::cls_client::mirror_mode_get(&m_local_io_ctx,
                                                        &mirror_mode);
                if (r < 0 && r != -ENOENT) {
                    derr << "failed to get namespace mirror mode: " <<
                        cpp_strerror(r)
                        << dendl;
                    ...
                    dout(10) << "mirroring is disabled for namespace " << name
                        << dendl;
                    continue;
                }
        ...
        template < typename I > void PoolReplayer < I >::reopen_logs() {
            ...
            if (m_local_rados) {
                reinterpret_cast <
                    CephContext * >(m_local_rados->cct())->reopen_logs();
            }
            if (m_remote_rados) {
                reinterpret_cast <
                    CephContext * >(m_remote_rados->cct())->reopen_logs();
            }
        }

        template < typename I >
            void PoolReplayer <
            I >::namespace_replayer_acquire_leader(const std::string & name,
                                                   Context * on_finish) {
            ceph_assert(ceph_mutex_is_locked(m_lock));
            ...
            on_finish = new LambdaContext([this, name, on_finish] (int r) {
                                          ...
                                          auto on_shut_down =
                                          new
                                          LambdaContext([namespace_replayer,
                                                         on_finish] (int r) {
                                                        delete
                                                        namespace_replayer;
                                                        on_finish->
        ...
        template < typename I >
            ...
            if (m_local_io_ctx.is_valid()) {
                ...
                f->dump_stream("instance_id") << m_local_io_ctx.
                    get_instance_id();
            }
            ...
            if (m_local_rados) {
                auto cct =
                    reinterpret_cast < CephContext * >(m_local_rados->cct());
            ...
            if (m_remote_rados) {
                auto cct =
                    reinterpret_cast < CephContext * >(m_remote_rados->cct());
        ...
        template < typename I >
            void PoolReplayer <
            I >::handle_post_acquire_leader(Context * on_finish) {
            dout(20) << dendl;

            with_namespace_replayers([this] (Context * on_finish) {
                                     dout(10) << "handle_post_acquire_leader" <<
                                     dendl;
                                     ceph_assert(ceph_mutex_is_locked(m_lock));
                                     m_service_daemon->
                                     ...
                                     new LambdaContext([this, on_finish] (int r)
                                                       ...
                                                       if (r == 0) {
                                                       std::lock_guard locker {
                                                       m_lock}; m_leader = true;}
                                                       on_finish->complete(r);}
              ); auto cct = reinterpret_cast < CephContext * >(m_local_io_ctx.cct()); auto gather_ctx = new C_Gather(cct, ctx); m_default_namespace_replayer->handle_acquire_leader(gather_ctx->new_sub()); for (auto & it:m_namespace_replayers)
                                     {
                                     namespace_replayer_acquire_leader(it.first,
                                                                       gather_ctx->
                                                                       new_sub
                                                                       ());}
        ...
        template < typename I >
            void PoolReplayer < I >::handle_instances_added(const std::vector <
                                                            std::string >
                                                            &instance_ids) {
        ...
        template < typename I >
            void PoolReplayer <
            I >::
            handle_remote_pool_meta_updated(const RemotePoolMeta &
                                            remote_pool_meta) {
