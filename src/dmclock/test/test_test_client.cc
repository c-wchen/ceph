...
TEST(test_client, full_bore_timing)
{
    ...
    client =
        new test::DmcClient(ClientId(0),
                            ...
                            ++count;
                            client->receive_response(resp, client_id,
                                                     resp_params);},
                            [&](const uint64_t seed)->ServerId & {
                            return server_id;}
                            , test::dmc_client_accumulate_f, 1000,  // ops to run
                            100,    // iops goal
...
TEST(test_client, paused_timing)
{
    ...
    std::thread t([&]() {
                  ...
                  "should have 50 unresponded calls"; auto_respond = true;
