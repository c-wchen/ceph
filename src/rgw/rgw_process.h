...
protected:
    ...
    struct RGWWQ: public DoutPrefixProvider,
        ...
        RGWProcess *process;
        RGWWQ(RGWProcess *p, ceph::timespan timeout,
...
public:
    RGWProcess(CephContext *const cct,
               ...
               std::string uri_prefix, RGWFrontendConfig *const conf)
        :
        cct(cct), env(env),
    ...
    const RGWProcessEnv &get_env() const
    {
        return env;
    } virtual void run() = 0;
    virtual void handle_request(const DoutPrefixProvider *dpp,
                                RGWRequest *req) = 0;
