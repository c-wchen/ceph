...
TEST_F(nvdev_test_t, write_and_verify_test)
{
    run_async([this] {
        device.
        reset(new random_block_device::nvme::NVMeBlockDevice(dev_path));
        local_conf().set_val("seastore_cbjournal_size", "1000000").get();
        device->mkfs(device_config_t {
            true, device_spec_t {
                (magic_t) std::rand(),
        ...
        uint64_t bl_length = 0; {
            ...
            bl_length = bl.length();
            auto write_buf =
            ceph::bufferptr(buffer::create_page_aligned(BLK_SIZE));
        ...
        nvdev_test_block_t read_data; {
            auto read_buf =
            ceph::bufferptr(buffer::create_page_aligned(BLK_SIZE));
            device->read(0, read_buf).unsafe_get(); bufferlist bl;
            bl.push_back(read_buf); auto bliter = bl.cbegin();
            decode(read_data, bliter);
        }

        int ret = memcmp(original_data.data, read_data.data, BUF_SIZE);
        device->close().unsafe_get();
        ASSERT_TRUE(ret == 0); device.reset(nullptr);
    });
}
