...
ObjectContextLoader::load_obc_iertr::future <> ObjectContextLoader::with_head_obc(ObjectContextRef obc,
        bool existed,
        with_obc_func_t && func)
{
    ...
    return obc->with_lock < State, IOInterruptCondition > ([existed =
                existed, obc =
                obc, func =
                std::move(func),
    this] {
...
template < RWState::State State >
ObjectContextLoader::load_obc_iertr::future <> ObjectContextLoader::with_clone_obc(hobject_t oid,
        with_obc_func_t && func)
{
    ...
    return with_obc < RWState::RWREAD > (oid.get_head(),
                                         [FNAME, oid, func =
                                                 std::move(func),
                                          this](auto head) mutable->
    load_obc_iertr::future <> {
        if (!head->obs.exists) {
            ERRORDPP
            ("head doesn't exist for object {}",
             dpp, head->obs.oi.soid);
            return load_obc_iertr::future <> {
                crimson::ct_error::enoent::make()
            };
        }
        return this->with_clone_obc_only <
               State > (std::move(head), oid,
                        std::move(func));
    }
                                        ) ;
}
        ...
        with_obc_func_t && func)
{
    ...
    return clone->template with_lock < State, IOInterruptCondition > ([existed = existed, clone = std::move(clone),
...
template < RWState::State State >
ObjectContextLoader::load_obc_iertr::future <> ObjectContextLoader::with_head_and_clone_obc(hobject_t oid,
        ...
        && func)
{
    ...
    load_obc_iertr::future <> {
        ...
        auto coid =
            resolve_oid(head->get_head_ss(),
                        oid);
        if (!coid)
        {
            ERRORDPP("clone {} not found", dpp,
                     oid);
            return load_obc_iertr::future <> {
                crimson::ct_error::enoent::make()
            };
        ...
        auto[clone, existed] =
            obc_registry.get_cached_obc(*coid);
        return clone->template with_lock <
                   State,
...
ObjectContextLoader::load_obc_iertr::future < ObjectContextRef > ObjectContextLoader::load_obc(ObjectContextRef obc)
{
    ...
    return backend.load_metadata(obc->get_oid())
           .safe_then_interruptible([FNAME, this, obc =
                                         std::move(obc)](auto md)
                                    ->load_obc_ertr::future <
    ObjectContextRef > {
        const hobject_t &oid = md->os.oi.soid;
        DEBUGDPP("loaded obs {} for {}", dpp,
                 md->os.oi, oid);
        if (oid.is_head()) {
            if (!md->ssc) {
        ...
        DEBUGDPP("returning obc {} for {}", dpp,
...
template < RWState::State State >
ObjectContextLoader::load_obc_iertr::future < ObjectContextRef > ObjectContextLoader::get_or_load_obc(
    ObjectContextRef obc,
    bool existed)
{
    ...
    if (existed) {
        ...
        loaded =
            obc->template with_promoted_lock < State,
        IOInterruptCondition > ([obc, this] {
            return load_obc(obc);
        }
                                                                      );
    }
...
ObjectContextLoader::load_obc_iertr::future <> ObjectContextLoader::reload_obc(ObjectContext &obc) const
{
    ...
    assert(obc.is_head());
    return backend.load_metadata(obc.get_oid())
           .safe_then_interruptible < false > ([FNAME, this,
                   &obc](auto md)->
    load_obc_ertr::future <> {
        DEBUGDPP("reloaded obs {} for {}",
                 dpp, md->os.oi,
                 obc.get_oid());
        if (!md->ssc) {
            ERRORDPP
