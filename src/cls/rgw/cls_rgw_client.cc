                                                 ...
                                                 string > *objs)
{
    ...
    for (; iter != completions.end(); ++iter) {
        ...
        if (objs && r == 0) {   /* update list of successfully completed objs */
            map < int, string >::iterator liter =
                completion_objs.find(iter->first);
...
void cls_rgw_bucket_prepare_op(ObjectWriteOperation & o, RGWModifyOp op,
                               string & tag, const cls_rgw_obj_key & key,
...
void cls_rgw_bucket_complete_op(ObjectWriteOperation & o, RGWModifyOp op,
                                string & tag, rgw_bucket_entry_ver & ver,
                                 ...
                                 struct rgw_cls_list_ret *pdata)
{
    ...
    op.exec(RGW_CLASS, RGW_BUCKET_LIST, in,
            new ClsBucketIndexOpCtx < struct rgw_cls_list_ret >(pdata, NULL));
    return manager->aio_operate(io_ctx, oid, &op);
}

int CLSRGWIssueBucketList::issue_op(int shard_id, const string & oid)
{
    return issue_bucket_list_op(io_ctx, oid, start_obj, filter_prefix,
...
int cls_rgw_clear_olh(IoCtx & io_ctx, librados::ObjectWriteOperation & op,
                      string & oid, const cls_rgw_obj_key & olh,
                      const string & olh_tag)
{
    bufferlist in, out;
                                 ...
                                 struct cls_rgw_bi_log_list_ret *pdata)
{
    ...
    op.exec(RGW_CLASS, RGW_BI_LOG_LIST, in,
            new ClsBucketIndexOpCtx < struct cls_rgw_bi_log_list_ret >(pdata,
                                                                       NULL));
    return manager->aio_operate(io_ctx, oid, &op);
}
...
static bool issue_bucket_check_index_op(IoCtx & io_ctx, const string & oid,
                                        BucketIndexAioManager * manager,
                                        struct rgw_cls_check_index_ret *pdata)
{
    ...
    op.exec(RGW_CLASS, RGW_BUCKET_CHECK_INDEX, in,
            new ClsBucketIndexOpCtx < struct rgw_cls_check_index_ret >(pdata,
                                                                       NULL));
    return manager->aio_operate(io_ctx, oid, &op);
}
...
{
    return issue_bucket_check_index_op(io_ctx, oid, &manager,
                                       &result[shard_id]);
}
...
int CLSRGWIssueGetDirHeader::issue_op(int shard_id, const string & oid)
{
    ...
    return issue_bucket_list_op(io_ctx, oid, nokey, "", 0, false, &manager,
                                &result[shard_id]);
}

static bool issue_resync_bi_log(librados::IoCtx & io_ctx, const string & oid,
                                BucketIndexAioManager * manager)
{
    bufferlist in;
    librados::ObjectWriteOperation op;
...
static bool issue_bi_log_stop(librados::IoCtx & io_ctx, const string & oid,
                              BucketIndexAioManager * manager)
{
    bufferlist in;
    librados::ObjectWriteOperation op;
...
int cls_rgw_lc_get_head(IoCtx & io_ctx, string & oid,
...
int cls_rgw_lc_put_head(IoCtx & io_ctx, string & oid,
                        cls_rgw_lc_obj_head & head)
{
    bufferlist in, out;
...
int cls_rgw_lc_get_next_entry(IoCtx & io_ctx, string & oid, string & marker,
...
int cls_rgw_lc_rm_entry(IoCtx & io_ctx, string & oid, pair < string,
                        int >&entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_set_entry(IoCtx & io_ctx, string & oid, pair < string,
                         int >&entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_list(IoCtx & io_ctx, string & oid,
                    const string & marker,
                    uint32_t max_entries, map < string, int >&entries)
{
    bufferlist in, out;
