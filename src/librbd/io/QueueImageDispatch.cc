...
namespace librbd {
    namespace io {
        ...
        template < typename I >
            ...
            return enqueue(true, tid, dispatch_result, on_finish,
                           on_dispatched);
        }
        ...
        template < typename I >
            ...
            return enqueue(false, tid, dispatch_result, on_finish,
                           on_dispatched);
        }

        template < typename I >
            bool QueueImageDispatch < I >::discard(AioCompletion * aio_comp,
                                                   Extents
                                                   && image_extents,
                                                   uint32_t
        ...
        template < typename I >
            bool QueueImageDispatch < I >::write_same(AioCompletion * aio_comp,
                                                      Extents
                                                      && image_extents,
                                                      bufferlist
                                                      && bl, int op_flags,
        ...
        template < typename I >
            void QueueImageDispatch < I >::handle_finished(int r,
                                                           uint64_t tid) {
            auto cct = m_image_ctx->cct;
            ldout(cct, 20) << "tid=" << tid << dendl;
