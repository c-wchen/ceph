...
public:
    ...
    } void shut_down(Context *on_finish) override
    {
        ...
        disconnect(new LambdaContext([this](int r) {
            handle_shut_down(r);
        }));
    }
    ...
    void issue(std::shared_ptr < Work > &&work) override
    {
        ...
        if (is_shutdown()) {
            lderr(cct) <<
                       "cannot issue HTTP request, client is shutdown" <<
                       dendl;
                      ...
                      std::shared_ptr < Work > &&work) override
    {
        ...
        if (is_shutdown()) {
            lderr(cct) << "client shutdown during in-flight request" <<
                       dendl;
            work->complete(-ESHUTDOWN, {
            }
                          );
        ...
        if (ec) {
            if (ec == boost::asio::error::bad_descriptor ||
                ...
                ldout(cct,
                      5) <<
                         "remote peer stream closed, retrying request" <<
                         dendl;
                m_issue_queue.push_front(work);
            } else if (ec == boost::beast::error::timeout) {
                lderr(cct) << "timed-out while issuing request" <<
                           dendl;
                work->complete(-ETIMEDOUT, {
                }
                              );
            } else {
                lderr(cct) << "failed to issue request: " << ec.
                           message() << dendl;
...
protected:
    ...
    virtual void connect(boost::asio::ip::tcp::resolver::
                         results_type results, Context *on_finish) = 0;
    ...
    void close_socket()
    {
        ...
        boost::beast::get_lowest_layer(derived().stream()).socket().
        close(ec);
    }
...
private:
    ...
    std::optional < boost::beast::http::parser < false,
        EmptyBody >> m_header_parser;
    std::optional < boost::beast::http::parser < false,
        StringBody >> m_parser;
    ...
    void resolve_host(Context *on_finish)
    {
        ...
        m_resolver.async_resolve(m_http_client->m_url_spec.host,
                                 m_http_client->m_url_spec.port, [this,
                                         on_finish]
                                 (boost::system::error_code ec,
        auto results) {
            handle_resolve_host(ec, results,
                                on_finish);
        });
    }

    void handle_resolve_host(boost::system::error_code ec,
    ...
    void handle_shut_down(int r)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to disconnect stream: '" <<
                       cpp_strerror(r)
                       << dendl;
        }
    ...
    {
        ceph_assert(m_http_client->m_strand.running_in_this_thread());
        return (m_state == STATE_SHUTTING_DOWN
    ...
    int shutdown_socket()
    {
        if (!boost::beast::get_lowest_layer(derived().stream()).
            socket().is_open()) {
            return 0;
        }
        ...
        if (ec && ec != boost::beast::errc::not_connected) {
            lderr(cct) << "failed to shutdown socket: " << ec.
                       message() << dendl;
            return -ec.value();
        }
    ...
    void receive(std::shared_ptr < Work > &&work)
    {
        ...
        if (work->header_only()) {
            ...
            m_header_parser->body_limit(std::numeric_limits <
                                        uint64_t >::max());

            boost::beast::http::async_read_header(derived().stream(),
                                                  m_buffer,
                                                  *m_header_parser,
                                                  [this, work =
                                                          std::move(work)]
            ...
            boost::beast::http::async_read(derived().stream(), m_buffer,
                                           *m_parser, [this, work =
                                                   std::move(work)]
                        ...
                        std::shared_ptr < Work > &&work)
    {
        ...
        if (is_shutdown()) {
            lderr(cct) << "client shutdown with in-flight request" <<
                       dendl;
            work->complete(-ESHUTDOWN, {
            }
                          );
        ...
        if (result == boost::beast::http::status::not_found) {
            ...
            lderr(cct) <<
                       "permission denied attempting to access resource" <<
                       dendl;
            r = -EACCES;
        } else if (boost::beast::http::to_status_class(result) !=
                   boost::beast::http::status_class::successful) {
            lderr(cct) << "failed to retrieve size: HTTP " << result <<
                       dendl;
            r = -EIO;
        }
        ...
        if (need_eof) {
            ldout(cct,
                  20) << "reset required for non-pipelined response: "
                      << "work=" << work.get() << dendl;
            reset();
    ...
    void complete_work(std::shared_ptr < Work > work, int r, Response
                       && response)
    {
        auto cct = m_http_client->m_cct;
...
public:
    ...
    inline boost::beast::tcp_stream &stream()
    {
        return m_stream;
    }
...
public:
    ...
    inline boost::beast::ssl_stream < boost::beast::tcp_stream > &stream()
    {
        return m_stream;
    }
...
protected:
    ...
    void disconnect(Context *on_finish) override
    {
        ...
        m_stream.
        async_shutdown(asio::util::
                       get_callback_adapter([this,
        on_finish](int r) {
...
private:
    ...
    void handshake(Context *on_finish)
    {
        ...
        m_stream.
        set_verify_callback([host, next =
                                 boost::asio::ssl::
                                 host_name_verification(host),
                             ignore_self_signed =
                                 http_client->m_ignore_self_signed_cert]
                            (bool preverified,
                             boost::asio::ssl::
        verify_context & ctx) {
        ...
        if (!SSL_set_tlsext_host_name(m_stream.native_handle(),
                                      ...
                                      c_str())) {
            int r = -::ERR_get_error();
            lderr(cct) << "failed to initialize SNI hostname: " <<
                       cpp_strerror(r)
                       << dendl;
            on_finish->complete(r);
            return;
        ...
        m_stream.async_handshake(boost::asio::ssl::stream_base::client,
                                 asio::util::get_callback_adapter([this,
                                         on_finish]
                                         (int
        r) {
            handle_handshake
    ...
    void handle_handshake(int r, Context *on_finish)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to complete handshake: " <<
                       cpp_strerror(r)
                       << dendl;
...
template < typename I > void HttpClient < I >::open(Context *on_finish)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to parse url '" << m_url << "': " <<
                     cpp_strerror(r)
                     << dendl;
...
template < typename I >
void HttpClient < I >::get_size(uint64_t *size,
                                Context *on_finish)
{
    ...
    issue(std::move(req),
    [this, size, on_finish](int r, Response && response) {
        handle_get_size(r, std::move(response), size, on_finish);
    });
}

template < typename I >
void HttpClient < I >::handle_get_size(int r, Response
                                       && response, uint64_t *size,
                                       Context *on_finish)
{
    ldout(m_cct, 10) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to retrieve size: " << cpp_strerror(r)
                     << dendl;
        on_finish->complete(r);
        return;
    } else if (!response.has_content_length()) {
        lderr(m_cct) <<
                     "failed to retrieve size: missing content-length" << dendl;
        on_finish->complete(-EINVAL);
        return;
    }
...
template < typename I >
void HttpClient < I >::read(io::Extents
                            && byte_extents, bufferlist *data,
                            Context *on_finish)
{
    ...
    for (auto[byte_offset, byte_length] : byte_extents) {
        ...
        range << "bytes=" << byte_offset << "-" << (byte_offset +
                byte_length - 1);
        req.set(boost::beast::http::field::range, range.str());

        issue(std::move(req),
              [this, byte_offset = byte_offset, byte_length =
                   byte_length, ctx]
        (int r, Response && response) {
            handle_read(r, std::move(response), byte_offset,
                        byte_length, &ctx->bl, ctx);
        });
    }
}
...
{
    ldout(m_cct,
          20) << "bytes=" << byte_offset << "~" << byte_length << ", "
              << "r=" << r << dendl;
