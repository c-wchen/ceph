 ...
 */
void PerfCountersCollectionImpl::dump_formatted_generic(Formatter *f,
        bool schema,
        bool histograms,
        bool dump_labeled,
        const std::
    ...
    if (dump_labeled) {
        ...
        for (auto l = m_loggers.begin(); l != m_loggers.end(); ++l) {
            ...
            if (logger.empty() || (*l)->get_name() == logger) {
                (*l)->dump_formatted_generic(f, schema, histograms, false,
                                             counter);
            }
        }
    }
...
void PerfCountersCollectionImpl::with_counters(std::function <
        void (const
...
PerfCounters::PerfCounters(CephContext *cct, const std::string &name,
    ...
    : m_cct(cct),
      m_lower_bound(lower_bound), m_upper_bound(upper_bound), m_name(name)
...
void PerfCountersBuilder::add_u64_counter_histogram(int idx,
        const char *name,
        PerfHistogramCommon::
        axis_config_d
        x_axis_config,
        ...
        int prio, int unit)
{
    ...
    unit, std::unique_ptr < PerfHistogram <>> {
        new PerfHistogram <> {
...
void PerfCountersBuilder::add_impl(int idx, const char *name,
                                   const char *description,
                                   const char *nick, int prio, int ty,
                                   int unit,
                                   std::unique_ptr <
