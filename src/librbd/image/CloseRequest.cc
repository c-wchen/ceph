...
void CloseRequest < I >::handle_shut_down_update_watchers(int r)
{
    ...
    save_result(r);
    if (r < 0) {
        lderr(cct) << "failed to shut down update watchers: " <<
                   cpp_strerror(r)
                   << dendl;
    }
...
template < typename I > void CloseRequest < I >::send_flush()
{
    ...
    auto aio_comp =
        io::AioCompletion::create_and_start(ctx, m_image_ctx,
                                            io::AIO_TYPE_FLUSH);
    auto req =
...
template < typename I > void CloseRequest < I >::handle_flush(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to flush IO: " << cpp_strerror(r) <<
                   dendl;
    }
...
void CloseRequest < I >::send_shut_down_exclusive_lock()
{
    ...
    m_exclusive_lock->shut_down(create_context_callback <
                                CloseRequest < I >,
                                &CloseRequest <
                                I >::handle_shut_down_exclusive_lock >
                                (this));
}
...
void CloseRequest < I >::send_unregister_image_watcher()
{
    ...
    unregister_watch(create_context_callback < CloseRequest < I >,
                     &CloseRequest <
                     I >::handle_unregister_image_watcher > (this));
}
...
void CloseRequest < I >::handle_unregister_image_watcher(int r)
{
    ...
    save_result(r);
    if (r < 0) {
        lderr(cct) << "failed to unregister image watcher: " <<
                   cpp_strerror(r)
                   << dendl;
    }
...
template < typename I > void CloseRequest < I >::send_flush_readahead()
{
    ...
    wait_for_pending(create_async_context_callback
                     (*m_image_ctx,
                      create_context_callback < CloseRequest < I >,
...
void CloseRequest < I >::send_shut_down_image_dispatcher()
{
    ...
    shut_down(create_context_callback < CloseRequest < I >,
              &CloseRequest <
              I >::handle_shut_down_image_dispatcher > (this));
}
...
void CloseRequest < I >::send_shut_down_object_dispatcher()
{
    ...
    shut_down(create_context_callback < CloseRequest < I >,
              &CloseRequest <
              I >::handle_shut_down_object_dispatcher > (this));
}
...
template < typename I > void CloseRequest < I >::send_close_parent()
{
    ...
    m_image_ctx->parent->state->
    close(create_async_context_callback
          (*m_image_ctx,
           create_context_callback < CloseRequest < I >,
           &CloseRequest < I >::handle_close_parent > (this)));
}
...
void CloseRequest < I >::handle_close_parent(int r)
{
    ...
    save_result(r);
    if (r < 0) {
        lderr(cct) << "error closing parent image: " << cpp_strerror(r)
                   << dendl;
    }
    send_flush_image_watcher();
}
...
void CloseRequest < I >::handle_flush_image_watcher(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "error flushing image watcher: " <<
                   cpp_strerror(r) << dendl;
    }
    save_result(r);
    finish();
