...
TEST_F(TestClsJournal, MinimumSet)
{
    ...
    client::get_mutable_metadata(ioctx, oid, &read_minimum_set,
                                 &read_active_set, &read_clients, &cond);
    ASSERT_EQ(0, cond.wait());
...
TEST_F(TestClsJournal, ClientRegisterDuplicate)
{
    ...
    ASSERT_EQ(0, client::client_register(ioctx, oid, "id1", bufferlist()));
    ASSERT_EQ(-EEXIST,
              client::client_register(ioctx, oid, "id1", bufferlist()));
}
...
TEST_F(TestClsJournal, TagList)
{
    ...
    std::set < Tag > tags;
    ASSERT_EQ(0,
              client::tag_list(ioctx, oid, "id1",
                               boost::optional < uint64_t > (), &tags));
    ASSERT_EQ(expected_all_tags, tags);

    ASSERT_EQ(0,
              client::tag_list(ioctx, oid, "id1",
                               boost::optional < uint64_t > (0), &tags));
