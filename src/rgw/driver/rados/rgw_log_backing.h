...
tl::expected < log_type, bs::error_code > log_backing_type(const DoutPrefixProvider * dpp, librados::IoCtx & ioctx, log_type def, int shards,   //< Total number of shards
...
bs::error_code log_remove(librados::IoCtx & ioctx, int shards,  //< Total number of shards
                          /// A function taking a shard number and
                          /// returning an oid.
                          const fu2::unique_function < std::string(int)
                          const >&get_oid, bool leave_zero, optional_yield y);
...
class logback_generations:public librados::WatchCtx2 {
  ...
  protected:
    ...
    logback_generations(librados::IoCtx & ioctx,
                        std::string oid,
                        fu2::unique_function < std::string(uint64_t,
                                                           int)const >&&get_oid,
                        int shards)noexcept:ioctx(ioctx), oid(oid),
        get_oid(std::move(get_oid)), shards(shards) {
  ...
  private:
    ...
    bs::error_code write(const DoutPrefixProvider * dpp, entries_t
                         && e, std::unique_lock < std::mutex > &&l_,
                         optional_yield y) noexcept;
    bs::error_code setup(const DoutPrefixProvider * dpp, log_type def,
                         optional_yield y) noexcept;
  ...
  public:
    ...
    void handle_notify(uint64_t notify_id,
                       uint64_t cookie,
                       uint64_t notifier_id, bufferlist & bl) override final;
    ...
    template < typename T, typename ... Args >
        ...
        init(const DoutPrefixProvider * dpp, librados::IoCtx & ioctx_,
             std::string oid_, fu2::unique_function < std::string(uint64_t,
                                                                  int) const
             >&&get_oid_, int shards_, log_type def, optional_yield y, Args
             && ... args) noexcept {
        try {
            ...
            return tl::unexpected(bs::
                                  error_code(ENOMEM, bs::system_category()));
        }
    }

    bs::error_code update(const DoutPrefixProvider * dpp,
                          optional_yield y)noexcept;

    entries_t entries() const {
        return entries_;
    } bs::error_code new_backing(const DoutPrefixProvider * dpp, log_type type,
                                 optional_yield y)noexcept;

    bs::error_code empty_to(const DoutPrefixProvider * dpp, uint64_t gen_id,
                            optional_yield y) noexcept;

    bs::error_code remove_empty(const DoutPrefixProvider * dpp,
                                optional_yield y) noexcept;
...
{
    return (gen_id > 0 ?
            fmt::format("G{:0>20}@{}", gen_id, cursor) : std::string(cursor));
}
...
class LazyFIFO {
  ...
  :    ioctx(ioctx), oid(std::move(oid)) {
    ...
    int push(const DoutPrefixProvider * dpp,
             const ceph::buffer::list & bl, optional_yield y) {
        auto r = lazy_init(dpp, y);
        if (r < 0)
            return r;
    ...
    int trim(const DoutPrefixProvider * dpp, std::string_view markstr,
             bool exclusive, librados::AioCompletion * c, optional_yield y) {
        auto r = lazy_init(dpp, y);
        if (r < 0)
            return r;
