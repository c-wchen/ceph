...
struct lba_map_val_t {
    ...
    lba_map_val_t(extent_len_t len,
                  paddr_t paddr, uint32_t refcount, uint32_t checksum)
...
struct LBAInternalNode
    : FixedKVInternalNode <
      ...
      laddr_t, laddr_le_t, LBA_BLOCK_SIZE, LBAInternalNode > {
    using Ref = TCachedExtentRef < LBAInternalNode >;
    using internal_iterator_t = const_iterator;
    template < typename ... T > LBAInternalNode(T
            && ... t): FixedKVInternalNode(std::forward < T >
                                               (t) ...)
    {
...
struct lba_map_val_le_t {
    ...
    explicit lba_map_val_le_t(const lba_map_val_t &val)
        : len(init_extent_len_le(val.len)),
          ...
          refcount(val.refcount), checksum(val.checksum)
    {
    } operator  lba_map_val_t() const
    {
        return lba_map_val_t {
            len, paddr, refcount, checksum};
    }
};
...
struct LBALeafNode
    : FixedKVLeafNode <
      LEAF_NODE_CAPACITY,
      laddr_t, laddr_le_t,
      lba_map_val_t, lba_map_val_le_t, LBA_BLOCK_SIZE, LBALeafNode, true > {
    ...
    template < typename ... T > LBALeafNode(T && ... t): parent_type_t(std::forward < T > (t) ...)
    {
    ...
    bool validate_stable_children() final
    {
        ...
        for (auto i : *this) {
            auto child =
                (LogicalCachedExtent *) this->children[i.get_offset()];
            if (is_valid_child_ptr(child)
                && child->get_laddr() != i.get_key()) {
                SUBERROR(seastore_fixedkv_tree,
                         "stable child not valid: child {}, key {}", *child,
                         i.get_key());
                ceph_abort();
                return false;
            }
    ...
    void update(internal_const_iterator_t iter,
                lba_map_val_t val, LogicalCachedExtent *nextent) final
    {
        ...
        return this->journal_update(iter,
                                    val, this->maybe_get_delta_buffer());
    }

    internal_const_iterator_t insert(internal_const_iterator_t iter,
                                     laddr_t addr,
                                     lba_map_val_t val,
                                     LogicalCachedExtent *nextent) final
    {
        ...
        val.paddr = this->maybe_generate_relative(val.paddr);
        this->journal_insert(iter,
                             addr, val, this->maybe_get_delta_buffer());
        return iter;
    }
    ...
    void node_resolve_vals(internal_iterator_t from,
        ...
        if (this->is_initial_pending()) {
            for (auto i = from; i != to; ++i) {
                ...
                if (val.paddr.is_relative()) {
                    ...
                    val.paddr =
                        val.paddr.block_relative_to(this->get_paddr());
                    i->set_val(val);
                }
            }
        }
...
template <> struct fmt::formatter <
    crimson::os::seastore::lba_manager::btree::lba_node_meta_t > : fmt::
    ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::lba_manager::btree::lba_map_val_t > : fmt::
    ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::lba_manager::btree::LBAInternalNode > : fmt::
    ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::lba_manager::btree::LBALeafNode > : fmt::
    ostream_formatter {
};
#endif
