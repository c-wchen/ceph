...
public:
    struct ModuleOption {
        ...
        } void dump(ceph::Formatter *f) const
        {
            ...
            f->dump_string("type",
                           Option::type_to_str(static_cast < Option::type_t >
                                               (type)));
            f->dump_string("level",
                           Option::level_to_str(static_cast < Option::level_t >
                                                (level)));
    ...
    public:
        ...
        StandbyInfo(uint64_t gid_, const std::string &name_,
                    const std::vector < ModuleInfo > &am, uint64_t feat)
    ...
    } const entity_addrvec_t &get_active_addrs() const
    {
        return active_addrs;
    } uint64_t get_active_gid() const
    {
        return active_gid;
    } bool get_available() const
    {
        return available;
    } const std::string &get_active_name() const
    {
        return active_name;
    } const utime_t &get_active_change() const
    {
        return active_change;
    } int get_num_standby() const
    {
