...
struct object_data_handler_test_t:public seastar_test_suite_t, TMTestState {
    ...
    void read(Transaction & t, objaddr_t offset, extent_len_t len) {
        bufferlist bl = with_trans_intr(t,[&](auto & t) {
                                        ...
                                        read(ObjectDataHandler::context_t {
                                                                           *tm,
                                                                           t,
                                                                           *onode}
                                             ,
    ...
    void read_near(objaddr_t offset, extent_len_t len, extent_len_t fuzz) {
        auto fuzzes =
            std::vector < int32_t > { -1 * (int32_t) fuzz, 0, (int32_t) fuzz };
    ...
    std::list < LBAMappingRef > get_mappings(objaddr_t offset,
                                             extent_len_t length) {
        auto t = create_mutate_transaction();
    ...
    seastar::future <> set_up_fut()final {
        onode = new TestOnode(DEFAULT_OBJECT_DATA_RESERVATION,
...
TEST_F(object_data_handler_test_t, single_write)
{
    run_async([this] {
              write(1 << 20, 8 << 10, 'c');
              read_near(1 << 20, 8 << 10, 1); read_near(1 << 20, 8 << 10, 512);}
    );
}
...
TEST_F(object_data_handler_test_t, overwrite_single)
{
    run_async([this] {
              write((1 << 20), 4 << 10, 'a');
              write((1 << 20), 4 << 10, 'c');
              read_near(1 << 20, 4 << 10, 1); read_near(1 << 20, 4 << 10, 512);}
    );
}
...
TEST_F(object_data_handler_test_t, truncate)
{
    run_async([this] {
              ...
              truncate(base + (12 << 10));
              read(base, 64 << 10);
              truncate(base - (12 << 10)); read(base, 64 << 10);}
    );
}
...
TEST_F(object_data_handler_test_t, no_split)
{
    run_async([this] {
              ...
              EXPECT_EQ(pins.size(), 1); read(0, 8 << 10);}
    );
}
...
TEST_F(object_data_handler_test_t, split_left)
{
    run_async([this] {
              ...
              EXPECT_EQ(pins.size(), 2); size_t res[2] = {
      0, 64 << 10}; auto base = pins.front()->get_key(); int i = 0; for (auto & pin:pins)
              {
              EXPECT_EQ(pin->get_key() - base, res[i]); i++;}
              read(0, 128 << 10);}
    ) ;
}
...
TEST_F(object_data_handler_test_t, split_left_right)
{
    run_async([this] {
              ...
              write(48 << 10, 32 << 10, 'a');
              auto pins = get_mappings(0, 128 << 10);
              EXPECT_EQ(pins.size(), 3); size_t res[3] = {
      0, 48 << 10, 80 << 10}; auto base = pins.front()->get_key(); int i = 0; for (auto & pin:pins)
              {
              EXPECT_EQ(pin->get_key() - base, res[i]); i++;}
              }
    ) ;
...
{
    run_async([this] {
              write(0, 128 << 10, 'x'); auto t = create_mutate_transaction();
              // normal split
              ...
              auto pins = get_mappings(0, 128 << 10);
              EXPECT_EQ(pins.size(), 10); size_t res[10] = {
      0, 4 << 10, 12 << 10, 20 << 10, 32 << 10, 36 << 10, 60 << 10, 96 << 10, 120 << 10, 124 << 10}; auto base = pins.front()->get_key(); int i = 0; for (auto & pin:pins)
              {
              EXPECT_EQ(pin->get_key() - base, res[i]); i++;}
              read(0, 128 << 10);}
    ) ;
