...
class WeightedTestGenerator:public TestOpGenerator {
  public:
    ...
    WeightedTestGenerator(int ops,
                          ...
                          bool ec_pool,
                          bool balance_reads,
                          bool set_redirect):m_nextop(NULL), m_op(0),
    ...
    TestOp *next(RadosTestContext & context) override {
        ...
        if (m_set_redirect) {
            ...
            int set_redirect_end =
                create_objects_end + m_initial_redirected_objects;
            ...
            if (m_op <= create_objects_end) {
                ...
                cout << m_op << ": " << "(create redirect oid) copy_from oid "
                    << oid.str()
                    << " from oid " << oid2.str() << std::endl;
                return new CopyFromOp(m_op, &context, oid.str(), oid2.str(),
                                      m_stats);
            }
                ...
                return new SetRedirectOp(m_op, &context, oid.str(), oid2.str(),
                                         context.pool_name);
            }

            if (!context.oid_redirect_not_in_use.size()
                && m_op > set_redirect_end) {
                int set_size = context.oid_not_in_use.size();
                if (set_size < m_objects + m_redirect_objects) {
                    return NULL;
                }
                for (int t_op = m_objects + 1; t_op <= create_objects_end;
                     t_op++) {
                    stringstream oid;
                    oid << t_op;
                    if (t_op % 2) {
...
int main(int argc, char **argv)
{
    ...
    RadosTestContext context(pool_name,
                             ...
                             max_stride_size,
                             no_omap,
                             no_sparse, pool_snaps, write_fadvise_dontneed, id);
