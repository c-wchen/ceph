...
namespace rgw::dmclock {
    ...
    void AsyncScheduler::cancel() {
        ...
        queue.remove_by_req_filter([&](RequestRef && request) {
                                   ...
                                   auto c =
                                   static_cast <
                                   Completion * >(request.release());
                                   Completion::dispatch(std::unique_ptr <
                                                        Completion > {
                                                        c},
                                                        boost::asio::error::
                                                        operation_aborted,
    ...
    void AsyncScheduler::cancel(const client_id & client) {
        ...
        queue.remove_by_client(client, false,[&](RequestRef && request) {
                               ...
                               auto c =
                               static_cast < Completion * >(request.release());
                               Completion::dispatch(std::unique_ptr <
                                                    Completion > {
                                                    c},
                                                    boost::asio::error::
    ...
    void AsyncScheduler::process(const Time & now) {
        ...
        while (outstanding_requests < max_requests) {
            ...
            Completion::post(std::unique_ptr < Completion > {
                             c}
            ...
            if (auto c = counters(client)) {
                auto lat =
                    Clock::from_double(now) - Clock::from_double(started);
