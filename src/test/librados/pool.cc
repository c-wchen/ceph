...
TEST(LibRadosPools, PoolList)
{
    ...
    ASSERT_LT(rados_pool_list(cluster, buf, POOL_LIST_BUF_SZ),
              POOL_LIST_BUF_SZ);

    // we can pass a null buffer too.
    ASSERT_LT(rados_pool_list(cluster, NULL, POOL_LIST_BUF_SZ),
              POOL_LIST_BUF_SZ);
...
TEST(LibRadosPools, PoolGetBaseTier)
{
    ...
    ASSERT_EQ(0,
              rados_mon_command(cluster, (const char **)cmd, 1, "", 0, NULL, 0,
                                NULL, 0));
    ...
    EXPECT_EQ(-ENOENT,
              rados_pool_get_base_tier(cluster, nonexistent_pool_id,
                                       &base_tier));
