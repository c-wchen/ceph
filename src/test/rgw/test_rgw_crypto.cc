...
class BlockCryptNone:public BlockCrypt {
  ...
  BlockCryptNone(size_t sz):block_size(sz) {
    ...
    bool encrypt(bufferlist & input,
                 off_t in_ofs,
                 size_t size,
                 bufferlist & output, off_t stream_offset) override {
        output.clear();
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity)
{
  ...
  for (unsigned int step:{
        ...
        for (size_t r = 97; r < 123; r++) {
            ...
            off_t end =
                begin + r * r * r * r * r * r * r % (test_range - begin);
            if (r % 3)
            ...
            ASSERT_TRUE(aes->
                        encrypt(input, begin, end - begin, encrypted, offset));
            bufferlist decrypted;
            ASSERT_TRUE(aes->
                        decrypt(encrypted, 0, end - begin, decrypted, offset));
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity_3)
{
  ...
  for (unsigned int step:{
        ...
        for (size_t r = 97; r < 123; r++) {
            ...
            while (pos < end) {
                chunk =
                    block_size + (rr / 3) * (rr + 17) * (rr + 71) * (rr +
                                                                     123) *
                    (rr + 131) % 50000;
            ...
            while (pos < end) {
                chunk =
                    block_size + (rr / 3) * (rr + 97) * (rr + 151) * (rr +
                                                                      213) *
                    (rr + 251) % 50000;
                chunk = chunk - chunk % block_size;
                if (pos + chunk > end)
...
TEST(TestRGWCrypto, verify_RGWGetObj_BlockDecrypt_ranges)
{
    ...
    for (off_t r = 93; r < 150; r++) {
        ...
        //random ranges
        off_t begin =
            (r / 3) * r * (r + 13) * (r + 23) * (r + 53) * (r +
                                                            71) % test_range;
        off_t end =
            begin + (r / 5) * (r + 7) * (r + 13) * (r +
                                                    101) * (r * 103) %
            (test_range - begin) - 1;
        ...
        ASSERT_EQ(decrypted,
                  boost::string_ref(input.c_str() + begin, expected_len));
    }
}
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_simple)
{
    ...
    ASSERT_EQ(fixup_range(&decrypt, 0, part_size - 2),
              range_t(0, part_size - 1));
    ASSERT_EQ(fixup_range(&decrypt, 0, part_size - 1),
              range_t(0, part_size - 1));
    ASSERT_EQ(fixup_range(&decrypt, 0, part_size),
              range_t(0, part_size + 4095));
    ASSERT_EQ(fixup_range(&decrypt, 0, part_size + 1),
              range_t(0, part_size + 4095));
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned_part_size)
{
    ...
    ASSERT_EQ(fixup_range(&decrypt, 0, na_part_size - 2),
              range_t(0, na_part_size - 2));
    // even though na_part_size -1 should not align to a 4095 boundary, the range
    // should not span the next part
    ASSERT_EQ(fixup_range(&decrypt, 0, na_part_size - 1),
              range_t(0, na_part_size - 1));

    ASSERT_EQ(fixup_range(&decrypt, 0, na_part_size),
...
TEST(TestRGWCrypto, verify_RGWPutObj_BlockEncrypt_chunks)
{
    ...
    for (off_t r = 93; r < 150; r++) {
        ...
        off_t test_size =
            (r / 5) * (r + 7) * (r + 13) * (r + 101) * (r * 103) % (test_range -
                                                                    1) + 1;
        off_t pos = 0;
        do {
            off_t size = 2 << ((pos * 17 + pos / 113 + r) % 16);
        ...
        ASSERT_EQ(put_sink.get_sink().length(),
                  static_cast < size_t > (test_size));
...
TEST(TestRGWCrypto, verify_Encrypt_Decrypt)
{
    ...
    do {
        ...
        ut_put_sink put_sink;
        RGWPutObj_BlockEncrypt encrypt(g_ceph_context, &put_sink,
                                       AES_256_CBC_create(g_ceph_context,
                                                          &key[0], 32));
        bufferlist bl;
        ...
        ASSERT_EQ(get_sink.get_sink(),
                  boost::string_ref((char *)test_in, test_size));
    }
