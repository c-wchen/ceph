...
RGWSI_MetaBackend_OTP::RGWSI_MetaBackend_OTP(CephContext *cct): RGWSI_MetaBackend_SObj
    (cct)
{
}
...
{
    RGWSI_MBOTP_GetParams &params =
        static_cast < RGWSI_MBOTP_GetParams & >(_params);

    int r =
        cls_svc->mfa.list_mfa(dpp, key, params.pdevices, objv_tracker,
                              params.pmtime, y);
    if (r < 0) {
        return r;
    }
                                     ...
                                     optional_yield y)
{
    RGWSI_MBOTP_PutParams &params =
        static_cast < RGWSI_MBOTP_PutParams & >(_params);

    return cls_svc->mfa.set_mfa(dpp, key, params.devices, true, objv_tracker,
                                params.mtime, y);
}
