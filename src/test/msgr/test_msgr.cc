...
TEST_P(MessengerTest, ConnectionRaceTest)
{
    ...
    cli_interceptor->proceed(Interceptor::STEP::SEND_CLIENT_IDENTITY,
                             Interceptor::ACTION::CONTINUE);
    srv_interceptor->proceed(Interceptor::STEP::SEND_CLIENT_IDENTITY,
                             Interceptor::ACTION::CONTINUE);
    ...
    ASSERT_EQ(1u,
              static_cast < Session * >(s2c->get_priv().get())->get_count());
    ASSERT_TRUE(s2c->peer_is_client());

    ASSERT_TRUE(c2s->is_connected());
    ASSERT_EQ(1u,
              static_cast < Session * >(c2s->get_priv().get())->get_count());
...
TEST_P(MessengerTest, ConnectionRaceReuseBannerTest)
{
    ...
    srv_interceptor->proceed(Interceptor::STEP::BANNER_EXCHANGE,
                             Interceptor::ACTION::CONTINUE);
    cli_interceptor->proceed(Interceptor::STEP::READY,
                             Interceptor::ACTION::CONTINUE);
    ...
    EXPECT_EQ(cli_interceptor->
              count_step(c2s.get(),
                         Interceptor::STEP::START_CLIENT_BANNER_EXCHANGE), 1u);
    EXPECT_EQ(cli_interceptor->
              count_step(c2s.get(), Interceptor::STEP::SEND_RECONNECT), 0u);
    EXPECT_EQ(cli_interceptor->count_step(c2s.get(), Interceptor::STEP::READY),
              1u);
...
TEST_P(MessengerTest, MissingServerIdenTest)
{
    ...
    srv_interceptor->proceed(Interceptor::STEP::SEND_SERVER_IDENTITY,
    ...
    ASSERT_EQ(1u,
              static_cast <
              Session * >(c2s_accepter->get_priv().get())->get_count());
...
TEST_P(MessengerTest, ReconnectTest)
{
    ...
    // failing client
    cli_interceptor->proceed(Interceptor::STEP::HANDLE_MESSAGE,
                             Interceptor::ACTION::FAIL);
    ...
    ASSERT_EQ(srv_interceptor->
              count_step(c2s_accepter, Interceptor::STEP::SEND_RECONNECT), 0u);
    // c2s_accepter sent 1 reconnect_ok messages
    ASSERT_EQ(srv_interceptor->
              count_step(c2s_accepter, Interceptor::STEP::SEND_RECONNECT_OK),
              1u);
    // c2s sent 1 reconnect messages
    ASSERT_EQ(cli_interceptor->
              count_step(c2s.get(), Interceptor::STEP::SEND_RECONNECT), 1u);
    // c2s sent 0 reconnect_ok messages
    ASSERT_EQ(cli_interceptor->
              count_step(c2s.get(), Interceptor::STEP::SEND_RECONNECT_OK), 0u);
...
TEST_P(MessengerTest, ReconnectRaceTest)
{
    ...
    cli_interceptor->proceed(Interceptor::STEP::SEND_RECONNECT,
                             Interceptor::ACTION::CONTINUE);
    srv_interceptor->proceed(Interceptor::STEP::SEND_RECONNECT,
                             Interceptor::ACTION::CONTINUE);
    ...
    ASSERT_LT(srv_interceptor->
              count_step(c2s_accepter, Interceptor::STEP::SEND_RECONNECT), 3u);
    ASSERT_GT(srv_interceptor->
              count_step(c2s_accepter, Interceptor::STEP::SEND_RECONNECT), 0u);
    ...
    ASSERT_EQ(cli_interceptor->
              count_step(c2s.get(), Interceptor::STEP::SEND_RECONNECT_OK), 1u);

    if (srv_interceptor->
        count_step(c2s_accepter, Interceptor::STEP::SEND_RECONNECT) == 2) {
    ...
    srv_interceptor->proceed(Interceptor::STEP::READY,
                             Interceptor::ACTION::CONTINUE);
...
TEST_P(MessengerTest, SimpleTest)
{
    ...
    ASSERT_EQ(1u,
              static_cast < Session * >(conn->get_priv().get())->get_count());
...
TEST_P(MessengerTest, StatefulTest)
{
    ...
    ASSERT_EQ(1U,
              static_cast <
              Session * >(server_conn->get_priv().get())->get_count());
...
TEST_P(MessengerTest, AnonTest)
{
    ...
    ASSERT_EQ(1U,
              static_cast < Session * >(con_a->get_priv().get())->get_count());
    ...
    ASSERT_EQ(1U,
              static_cast < Session * >(con_b->get_priv().get())->get_count());
...
class SyntheticWorkload {
    ...
    map < ConnectionRef, pair < Messenger *,
        Messenger * >>available_connections;
  ...
  :    client_policy(cli_policy), dispatcher(false, this), rng(time(NULL)),
        ...
        for (int i = 0; i < servers; ++i) {
            msgr =
                Messenger::create(g_ceph_context, type, entity_name_t::OSD(0),
                                  "server", getpid() + i);
...
class MarkdownDispatcher:public Dispatcher {
  ...
  public:
    ...
    bool ms_dispatch(Message * m) override {
        ...
        last_mark = true;
        usleep(rand() % 500);
        for (set < ConnectionRef >::iterator it = conns.begin();
             it != conns.end(); ++it) {
