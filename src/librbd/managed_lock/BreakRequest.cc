...
namespace librbd {
    namespace managed_lock {
         ...
         namespace {
            ...
            struct C_BlacklistClient:public Context {
                ...
                Context *on_finish;
                 ...
                 C_BlacklistClient(librados::IoCtx & ioctx,
        ...
        template < typename I > void BreakRequest < I >::send_blacklist() {
            ...
            Context *ctx =
                create_context_callback < klass,
                &klass::handle_blacklist > (this);
            m_work_queue->
                queue(new
                      C_BlacklistClient(m_ioctx, m_locker.address,
                                        m_blacklist_expire_seconds, ctx), 0);
        }

        template < typename I > void BreakRequest < I >::handle_blacklist(int r) {
            ldout(m_cct, 10) << "r=" << r << dendl;

            if (r < 0) {
                lderr(m_cct) << "failed to blacklist lock owner: " <<
                    cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
