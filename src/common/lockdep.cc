...
namespace std {
    template <> struct hash <pthread_t > {
        size_t operator() (pthread_t __x) const {
...
void lockdep_register_ceph_context(CephContext * cct)
{
    ...
    if (g_lockdep_ceph_ctx == NULL) {
        ANNOTATE_BENIGN_RACE_SIZED(&g_lockdep_ceph_ctx,
                                   sizeof(g_lockdep_ceph_ctx), "lockdep cct");
...
void lockdep_unregister_ceph_context(CephContext * cct)
{
    ...
    if (cct == g_lockdep_ceph_ctx) {
        ...
        memset((void *)&follows_bt[0][0], 0,
               sizeof(BackTrace *) * current_maxid * MAX_LOCKS);
    }
    pthread_mutex_unlock(&lockdep_mutex);
}
...
int lockdep_dump_locks()
{
    ...
    for (ceph::unordered_map < pthread_t, map < int,
         BackTrace * > >::iterator p = held.begin(); p != held.end(); ++p) {
    ...
    for (int i = 0; i < MAX_LOCKS / 8; ++i) {
        if (free_ids[i] != 0) {
            for (int j = 0; j < 8; ++j) {
                if (free_ids[i] & (1 << j)) {
                    free_ids[i] &= 255 - (1 << j);
                    lockdep_dout(1) << "lockdep using id " << i * 8 +
                        j << dendl;
                    return i * 8 + j;
                }
            }
...
void lockdep_unregister(int id)
{
    ...
    if (--refs == 0) {
        if (p != lock_names.end()) {
            ...
            lockdep_dout(10) << "unregistered '" << name << "' from " << id <<
                dendl;
...
static bool does_follow(int a, int b)
{
    ...
    for (unsigned i = 0; i < current_maxid; i++) {
        if ((follows[a][i / 8] & (1 << (i % 8))) && does_follow(i, b)) {
            lockdep_dout(0) << "existing intermediate dependency " <<
                lock_names[a]
                << " (" << a << ") -> " << lock_names[i] << " (" << i <<
                ") at:\n";
            if (follows_bt[a][i]) {
