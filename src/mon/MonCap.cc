...
ostream & operator<<(ostream & out, const MonCapGrant & m)
{
    ...
    if (m.command.length()) {
        ...
        if (!m.command_args.empty()) {
            out << " with";
            for (map < string, StringConstraint >::const_iterator p =
                 m.command_args.begin(); p != m.command_args.end(); ++p) {
...
BOOST_FUSION_ADAPT_STRUCT(MonCapGrant, (std::string, service)
                          (std::string, profile)
                          (std::string, command)
                          (kvmap, command_args)
                          (mon_rwxa_t, allow))
...
BOOST_FUSION_ADAPT_STRUCT(StringConstraint,
                          ...
                          (std::string, value))
// </magic>
void MonCapGrant::expand_profile(int daemon_type, const EntityName & name) const const
{
...
void MonCapGrant::expand_profile_mgr(const EntityName & name) const const
{
}

void MonCapGrant::expand_profile_mon(const EntityName & name) const const
{
    if (profile == "mon") {
    ...
    if (profile == "mgr") {
        ...
        profile_grants.
            push_back(MonCapGrant("config-key", MON_CAP_R | MON_CAP_W));
    ...
    if (profile == "bootstrap-rbd") {
        ...
        profile_grants.back().command_args["caps_mon"] =
            StringConstraint(StringConstraint::MATCH_TYPE_EQUAL, "profile rbd");
        profile_grants.back().command_args["caps_osd"] =
            StringConstraint(StringConstraint::MATCH_TYPE_REGEX,
                             "^([ ,]*profile(=|[ ]+)['\"]?rbd[^ ,'\"]*['\"]?([ ]+pool(=|[ ]+)['\"]?[^,'\"]+['\"]?)?)+$");
    }
...
mon_rwxa_t MonCapGrant::get_allowed(CephContext * cct,
                                    ...
                                    EntityName name,
                                    const std::string & s,
                                    const std::string & c, const map < string,
                                    string > &c_args) const const
{
    if (profile.length()) {
    ...
    if (command.length()) {
        if (command != c)
            return 0;
        for (map < string, StringConstraint >::const_iterator p =
             command_args.begin(); p != command_args.end(); ++p) {
            ...
            case StringConstraint::MATCH_TYPE_REGEX:
                {
                    boost::regex pattern(p->second.value,
                                         boost::regex::extended | boost::regex::
                                         no_except);
                    if (pattern.empty()
                        || !boost::regex_match(q->second, pattern))
                        return 0;
                }
                break;
            default:
...
bool MonCap::is_allow_all() const const
{
...
bool MonCap::is_capable(CephContext * cct,
                        ...
                        const string & command, const map < string,
                        string > &command_args, bool op_may_read,
                        bool op_may_write, bool op_may_exec) const const
{
    if (cct)
        ldout(cct,
              20) << "is_capable service=" << service << " command=" << command
            << (op_may_read ? " read" : "")
            << (op_may_write ? " write" : "")
            << (op_may_exec ? " exec" : "")
            << " on cap " << *this << dendl;
    ...
    for (vector < MonCapGrant >::const_iterator p = grants.begin();
        ...
        if (cct)
            ldout(cct,
                  20) << " allow so far " << allow << ", doing grant " << *p <<
                dendl;
...
void MonCap::encode(bufferlist & bl) const const
{
...
void MonCap::dump(Formatter * f) const const
{
    ...
    struct MonCapParser:qi::grammar < Iterator, MonCap() >
{
  MonCapParser():MonCapParser::base_type(moncap) {
        ...
        command_match =
            ...
            >> str >> -(spaces >> lit("with") >> spaces >> kv_map)
            >> qi::attr(0);
        ...
        rwxa =
            ...
            (eps[_val = 0] >>
             (lit('r')[_val |= MON_CAP_R] ||
              lit('w')[_val |= MON_CAP_W] || lit('x')[_val |= MON_CAP_X]
             )
            );

        // grant := allow ...
        grant =
            -spaces >> (rwxa_match | profile_match | service_match |
                        command_match) >> -spaces;
...
bool MonCap::parse(const string & str, ostream * err)
{
    ...
    if (err) {
        if (iter != end)
            *err << "moncap parse failed, stopped at '" << std::string(iter,
                                                                       end)
                << "' of '" << str << "'\n";
        else
