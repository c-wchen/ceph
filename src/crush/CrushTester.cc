...
void CrushTester::adjust_weights(vector < __u32 > &weight)
{
    ...
    if (mark_down_device_ratio > 0) {
        ...
        int num_buckets_to_visit =
            (int)(mark_down_bucket_ratio * buckets_above_devices.size());
...
bool CrushTester::check_valid_placement(int ruleno, vector < int >in,
                                  ...
                                  vector < __u32 > &weight)
{
    ...
    do {
        ...
        accept_placement =
            check_valid_placement(ruleno, trial_placement, weight);
        attempted_tries++;
...
void CrushTester::check_overlapped_rules() const const
{
    ...
    for (int rule = 0; rule < crush.get_max_rules(); rule++) {
        ...
        Rules & rules = rulesets[ {
                                 crush.get_rule_mask_ruleset(rule),
  ...
  for (auto i:rulesets) {
      ...
      for (auto r:rules) {
            ...
            // print them out.
            if (names.size() > 1) {
                err << "overlapped rules in ruleset " << ruleset_type.
                    first << ": " << boost::join(names, ", ") << "\n";
            }
        }
    }
}
    ...
    for (int r = min_rule; r < crush.get_max_rules() && r <= max_rule; r++) {
        ...
        for (int nr = minr; nr <= maxr; nr++) {
            ...
            float expected_objects =
                min(nr, get_maximum_affected_by_rule(r)) * num_objects;

            // compute each device's proportional weight
            vector < float >proportional_weights(per.size());

            for (unsigned i = 0; i < per.size(); i++)
                proportional_weights[i] =
                    (float)weight[i] / (float)total_weight;

            if (output_data_file) {
                // stage the absolute weight information for post-processing
                for (unsigned i = 0; i < per.size(); i++) {
                    tester_data.absolute_weights[i] =
                        (float)weight[i] / (float)0x10000;
                }

                // stage the proportional weight information for post-processing
                for (unsigned i = 0; i < per.size(); i++) {
                    if (proportional_weights[i] > 0)
                        tester_data.proportional_weights[i] =
                            proportional_weights[i];

                    tester_data.proportional_weights_all[i] =
                        proportional_weights[i];
                }
            ...
            for (unsigned i = 0; i < num_devices; i++)
                num_objects_expected[i] =
                    (proportional_weights[i] * expected_objects);

            for (int current_batch = 0; current_batch < num_batches;
                 current_batch++) {
                ...
                float batch_expected_objects =
                    min(nr,
                        get_maximum_affected_by_rule(r)) * objects_per_batch;
                ...
                for (int x = batch_min; x <= batch_max; x++) {
                    ...
                    if (use_crush) {
                        ...
                        if (pool_id != -1) {
                            real_x =
                                crush_hash32_2(CRUSH_HASH_RJENKINS1, x,
                                               (uint32_t) pool_id);
                        }
                    ...
                    if (output_bad_mappings &&
                        ...
                        err << "bad mapping rule " << r << " x " << x <<
                            " num_rep " << nr << " result " << out << std::endl;
                    }
                }
            ...
            for (map < int, int >::iterator p = sizes.begin(); p != sizes.end();
                ...
                if (output_statistics)
                    err << "rule " << r << " (" << crush.
                        get_rule_name(r) << ") num_rep " << nr <<
                        " result size == " << p->first << ":\t" << p->
                        second << "/" << (max_x - min_x + 1) << std::endl;
            ...
            if (output_csv)
                write_data_set_to_csv(output_data_file_name + rule_tag,
                                      tester_data);
        }
    }
