                   ...
                   &ceph_global_init_args)
{
    ...
    if (r == -EOPNOTSUPP) {
        std::
        cerr << "rbd: luminous or later release required." <<
             std::endl;
    } else if (r == -EPERM) {
        std::
        cerr <<
        ...
        std::
        cerr << "rbd: error registered application: " <<
             cpp_strerror(r)
             << std::endl;
    }
                  ...
                  &ceph_global_init_args)
{
    ...
    pool_stats.add(RBD_POOL_STAT_OPTION_IMAGE_MAX_PROVISIONED_BYTES,
                   &provisioned_bytes);
    pool_stats.add(RBD_POOL_STAT_OPTION_IMAGE_SNAPSHOTS,
                   &snap_count);
    ...
    pool_stats.add(RBD_POOL_STAT_OPTION_TRASH_SNAPSHOTS,
                   &trash_snap_count);

    r = rbd.pool_stats_get(io_ctx, &pool_stats);
    if (r < 0) {
        std::
        cerr << "rbd: failed to query pool stats: " <<
             cpp_strerror(r)
             << std::endl;
        return r;
    }
    ...
    if (formatter) {
        ...
        formatter->dump_unsigned("provisioned_bytes",
                                 provisioned_bytes);
        ...
        std::
        cout << "Provisioned Size: " <<
             byte_u_t(provisioned_bytes);
        if (trash_count > 0) {
            std::
            cout << " (" << byte_u_t(trash_provisioned_bytes) <<
                 " in trash)";
        }
        std::cout << std::endl;
    }
