...
namespace queue_counters {
...
} // namespace queue_counters namespace throttle_counters {
    enum {
        l_first = 437219,
        l_throttle,
        l_outstanding,
    ...
    PerfCountersRef build(CephContext * cct, const std::string & name);
} // namespace throttle namespace rgw::dmclock {

// the last client counter would be for global scheduler stats
    static constexpr auto counter_size =
        static_cast < size_t > (client_id::count) + 1;
/// array of per-client counters to serve as GetClientCounters
    ...
    class ThrottleCounters {
      ...
      public:
        ThrottleCounters(CephContext * const cct,
                         const std::string & name):counters(throttle_counters::
                                                            build(cct, name)) {
    ...
    void on_process(PerfCounters * c, const ClientSum & rsum,
                    const ClientSum & psum);
      ...
      public:
        SchedulerCtx(CephContext * const cct):sched_t(get_scheduler_t(cct)) {
            if (sched_t == scheduler_t::dmclock) {
                ...
                dmc_client_counters =
                    std::make_optional < ClientCounters > (cct);
        }}
        // We need to construct a std::function from a NonCopyable object
            ClientCounters & get_dmc_client_counters() {
            return dmc_client_counters.value();
        }
        ClientConfig *const get_dmc_client_config() const {
            return dmc_client_config.get();
      } private:
        scheduler_t sched_t;
        std::shared_ptr < ClientConfig > dmc_client_config {
        nullptr};
        std::optional < ClientCounters > dmc_client_counters {
        std::nullopt};
    };
