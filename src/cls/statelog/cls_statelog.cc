...
static int write_statelog_entry(cls_method_context_t hctx, const string & index,
                                const cls_statelog_entry & entry)
{
    bufferlist bl;
    ::encode(entry, bl);
...
static void get_index_by_client(const string & client_id, const string & op_id,
                                string & index)
{
...
static void get_index_by_object(const string & object, const string & op_id,
                                string & index)
{
    char buf[16];
...
static int get_existing_entry(cls_method_context_t hctx,
                              const string & client_id, const string & op_id,
                              const string & object, cls_statelog_entry & entry)
{
    ...
    if ((!object.empty() && entry.object != object) ||
        ...
        CLS_LOG(0,
                "data mismatch: object=%s client_id=%s entry: object=%s client_id=%s",
                object.c_str(), client_id.c_str(), entry.object.c_str(),
                entry.client_id.c_str());
        return -EINVAL;
    }
...
static int cls_statelog_add(cls_method_context_t hctx, bufferlist * in,
                            bufferlist * out)
{
...
static int cls_statelog_list(cls_method_context_t hctx, bufferlist * in,
                             bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
#define MAX_ENTRIES 1000
    ...
    CLS_LOG(20, "from_index=%s match_prefix=%s", from_index.c_str(),
            match_prefix.c_str());
    ...
    for (; iter != keys.end(); ++iter) {
        ...
        try {
            ...
            CLS_LOG(0,
                    "ERROR: cls_statelog_list: could not decode entry, index=%s",
                    index.c_str());
        }
    }
...
static int cls_statelog_remove(cls_method_context_t hctx, bufferlist * in,
                               bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
static int cls_statelog_check_state(cls_method_context_t hctx, bufferlist * in,
                                    bufferlist * out)
{
    bufferlist::iterator in_iter = in->begin();
...
CLS_INIT(statelog)
{
    ...
    cls_register_cxx_method(h_class, "add", CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_statelog_add, &h_statelog_add);
    cls_register_cxx_method(h_class, "list", CLS_METHOD_RD, cls_statelog_list,
                            &h_statelog_list);
    cls_register_cxx_method(h_class, "remove", CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_statelog_remove, &h_statelog_remove);
    cls_register_cxx_method(h_class, "check_state", CLS_METHOD_RD,
                            cls_statelog_check_state, &h_statelog_check_state);
