...
TEST_F(TestInternal, Metadata)
{
    ...
    map < string, bool > test_confs =
        boost::assign::map_list_of("aaaaaaa",
                                   false) ("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
...
TEST_F(TestInternal, SnapshotCopyup)
{
    ...
    ASSERT_EQ(256, ictx->io_work_queue->write(0, bl.length(), bufferlist {
                                              bl}, 0));
    ...
    ASSERT_EQ(256, ictx2->io_work_queue->write(256, bl.length(), bufferlist {
                                               bl}, 0));
    ...
    for (std::list < std::string >::iterator it = snaps.begin();
        ...
        ASSERT_EQ(256, ictx2->io_work_queue->read(0, 256, librbd::io::ReadResult {
                                                  read_result}, 0));
        ASSERT_TRUE(bl.contents_equal(read_bl));

        ASSERT_EQ(256,
                  ictx2->io_work_queue->read(256, 256, librbd::io::ReadResult {
                                             read_result}, 0));
        if (snap_name == NULL) {
            ASSERT_TRUE(bl.contents_equal(read_bl));
        }
...
TEST_F(TestInternal, ResizeCopyup)
{
    ...
    for (size_t i = 0; i < m_image_size; i += bl.length()) {
        ASSERT_EQ((ssize_t) bl.length(),
                  ictx->io_work_queue->write(i, bl.length(), bufferlist {
                                             bl}
                                             , 0));
    }
...
TEST_F(TestInternal, ShrinkFlushesCache)
{
    ...
    ictx->io_work_queue->write(0, buffer.size(), bufferlist {
                               write_bl}
                               , 0);
    ...
    c->get();
    ictx->io_work_queue->aio_write(c, 0, buffer.size(), bufferlist {
                                   write_bl}
                                   , 0);
...
TEST_F(TestInternal, WriteFullCopyup)
{
    ...
    ASSERT_EQ((ssize_t) bl.length(),
              ictx->io_work_queue->write(0, bl.length(), bufferlist {
                                         bl}
                                         , 0));
    ...
    ASSERT_EQ(0, open_image(clone_name, &ictx2));
    ASSERT_EQ(0,
              ictx2->operations->snap_create(cls::rbd::UserSnapshotNamespace(),
                                             "snap1"));
...
TEST_F(TestInternal, DiffIterateCloneOverwrite)
{
    ...
    interval_set < uint64_t > diff;
    ASSERT_EQ(0,
              librbd::snap_set(ictx, cls::rbd::UserSnapshotNamespace(), "one"));
    ASSERT_EQ(0,
...
TEST_F(TestInternal, TestCoR)
{
    ...
    ASSERT_EQ(0,
              create_image_full_pp(m_rbd, m_ioctx, m_image_name, m_image_size,
