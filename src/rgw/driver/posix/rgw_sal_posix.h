...
public:
    ...
    void set_stat(struct statx _stx)
    {
        stx = _stx;
        stat_done = true;
    }
    int get_dir_fd(const DoutPrefixProvider *dpp)
    {
        open(dpp);
        return dir_fd;
    }
...
public:
    ...
    bool exists(const DoutPrefixProvider* dpp)
    {
        stat(dpp);
        return state.exists;
    }
...
public:
    MPPOSIXSerializer(const DoutPrefixProvider *dpp, POSIXDriver* driver, POSIXObject* _obj,
                      const std::string& lock_name) : obj(_obj) {}
