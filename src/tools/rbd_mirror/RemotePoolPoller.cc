    ...
    namespace mirror {
        ...
        template < typename I > void RemotePoolPoller < I >::get_mirror_uuid() {
            ...
            auto aio_comp = create_rados_callback <
                RemotePoolPoller < I >,
                &RemotePoolPoller < I >::handle_get_mirror_uuid > (this);
            m_out_bl.clear();
            int r =
        ...
        template < typename I > void RemotePoolPoller < I >::mirror_peer_ping() {
            ...
            librbd::cls_client::mirror_peer_ping(&op, m_site_name,
                                                 m_local_mirror_uuid);

            auto aio_comp = create_rados_callback <
                RemotePoolPoller < I >,
                &RemotePoolPoller < I >::handle_mirror_peer_ping > (this);
        ...
        template < typename I >
            ...
            if (r == -EOPNOTSUPP) {
                ...
                dout(10) <<
                    "remote peer does not support snapshot-based mirroring" <<
                    dendl;
                notify_listener();
                return;
            }
                ...
                derr << "failed to ping remote mirror peer: " << cpp_strerror(r)
                    << dendl;
            }
        ...
        template < typename I > void RemotePoolPoller < I >::mirror_peer_list() {
            ...
            auto aio_comp = create_rados_callback <
                RemotePoolPoller < I >,
                &RemotePoolPoller < I >::handle_mirror_peer_list > (this);
            m_out_bl.clear();
            int r =
                m_remote_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op,
                                            &m_out_bl);
        ...
        template < typename I >
            ...
            if (r < 0) {
                derr << "failed to retrieve mirror peers: " << cpp_strerror(r)
                    << dendl;
            }
          ...
          for (auto & peer:peers) {
            ...
            dout(10) << "remote_mirror_peer_uuid=" << remote_mirror_peer_uuid <<
                dendl;
        ...
        template < typename I >
            ...
            if (m_state == STATE_POLLING) {
                ...
                m_threads->timer->add_event_after(POLL_INTERVAL_SECONDS,
                                                  m_timer_task);
            }
