...
bool RGWReadDataSyncStatusMarkersCR::spawn_next()
{
    ...
    spawn(new CR(env->dpp, env->driver,
                 rgw_raw_obj(env->svc->zone->get_zone_params().log_pool, RGWDataSyncStatusManager::shard_obj_name(sc->source_zone,
                             shard_id)),
...
bool RGWListRemoteDataLogCR::spawn_next()
{
    ...
    spawn(new RGWListRemoteDataLogShardCR(sc, iter->first, iter->second, max_entries_per_shard, &(*result)[iter->first]),
          false);
    ++iter;
    return true;
}
        ...
        reenter(this) {
            ...
            do {
                ...
                for (log_iter = log_entries.begin();
                    ...
                    tn->log(20, SSTR("shard_id=" << shard_id << " log_entry: " << log_iter->log_id << ":" << log_iter->log_timestamp << ":"
                                     << log_iter->entry.key));
                    ...
                    if (!marker_tracker->start(log_iter->log_id, 0,
                        ...
                        tn->log(1, SSTR("incremental sync on " << log_iter->entry.key  << "shard: " << shard_id << "on gen " <<
                                        log_iter->entry.gen));
...
public:
    ...
    RGWCoroutine *alloc_finisher_cr() override
    {
        return new RGWSimpleRadosReadCR<rgw_data_sync_marker>(sync_env->dpp, sync_env->driver,
                rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool, RGWDataSyncStatusManager::shard_obj_name(sc->source_zone,
                            shard_id)),
                &sync_marker, true, &objv);
    }
...
public:
    RGWDataSyncCR(RGWDataSyncCtx *_sc, uint32_t _num_shards, RGWSyncTraceNodeRef &_tn,
...
#define MAX_RACE_RETRIES_OBJ_FETCH 10
            for (try_num = 0; try_num < MAX_RACE_RETRIES_OBJ_FETCH; ++try_num) {
                ...
                if (need_more_info) {
                    ldout(cct, 20) << "Could not determine exact policy rule for obj=" << key << ", will read source object attributes" <<
                                   dendl;
                    /*
                ...
                if (param_mode == rgw_sync_pipe_params::MODE_USER) {
                    ...
                    int r = user_perms->init_bucket(sync_pipe.dest_bucket_info,
                                                    ...
                                                    &dest_bucket_perms);
                    if (r < 0) {
                        ldout(cct, 20) << "ERROR: " << __func__ << ": failed to init bucket perms manager for uid=" << *param_user << " bucket="
                                       << sync_pipe.source_bucket_info.bucket.get_key() << dendl;
                        return set_cr_error(retcode);
                    }

                    if (!dest_bucket_perms.verify_bucket_permission(RGW_PERM_WRITE)) {
                        ldout(cct, 0) << "ERROR: " << __func__ << ": permission check failed: user not allowed to write into bucket (bucket=" <<
            ...
            ldout(cct, 0) << "ERROR: " << __func__ << ": Too many retries trying to fetch object, possibly a bug: bucket=" <<
...
public:
    ...
    int operate(const DoutPrefixProvider *dpp) override
    {
        reenter(this) {
            ...
            ldout(cct, 20) << "writing bucket sync status during init. state=" << status.state << ". marker=" <<
                           status.full.position << dendl;
...
int RGWRemoteDataLog::read_shard_status(const DoutPrefixProvider *dpp, int shard_id, set<string> &pending_buckets,
                                        set<string> &recovering_buckets, rgw_data_sync_marker *sync_marker, const int max_entries)
{
    // cannot run concurrently with run_sync(), so run in a separate manager
    ...
    recovering_stack->call(new RGWReadRecoveringBucketShardsCoroutine(&sc_local, shard_id, recovering_buckets,
                           max_entries));
    stacks.push_back(recovering_stack);
    RGWCoroutinesStack *pending_stack = new RGWCoroutinesStack(driver->ctx(), &crs);
    pending_stack->call(new RGWReadPendingBucketShardsCoroutine(&sc_local, shard_id, pending_buckets, sync_marker,
                        max_entries));
...
public:
        ...
        sync_status(0)
    {
        ...
        set_description() << "bucket sync single entry (source_zone=" << sc->source_zone << ") b=" << ss.str() << " log_entry="
                          << entry_marker << " op=" << (int)op << " op_state=" << (int)op_state;
        set_status("init");

        tn = sync_env->sync_tracer->add_node(_tn_parent, "entry", SSTR(key));

        tn->log(20, SSTR("bucket sync single entry (source_zone=" << sc->source_zone << ") b=" << ss.str() << " log_entry=" <<
                         entry_marker << " op=" << (int)op << " op_state=" << (int)op_state));
    ...
    int operate(const DoutPrefixProvider *dpp) override
    {
        reenter(this) {
            ...
            do {
                yield {
                               ...
                               op == CLS_RGW_OP_LINK_OLH)
                    {
                        ...
                        tn->log(5, SSTR("bucket sync: sync obj: " << sc->source_zone << "/" << bs.bucket << "/" << key << "[" <<
                    ...
                    } else if (op == CLS_RGW_OP_DEL || op == CLS_RGW_OP_UNLINK_INSTANCE)
                    {
                        ...
                        tn->log(10, SSTR("removing obj: " << sc->source_zone << "/" << bs.bucket << "/" << key << "[" <<
                                         versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->remove_object(dpp, sc, sync_pipe, key, timestamp, versioned, versioned_epoch.value_or(0),
                                                             &zones_trace));
                        // our copy of the object is more recent, continue as if it succeeded
                    } else if (op == CLS_RGW_OP_LINK_OLH_DM)
                    {
                        set_status("creating delete marker");
                        tn->log(10, SSTR("creating delete marker: obj: " << sc->source_zone << "/" << bs.bucket << "/" << key << "[" <<
                                         versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->create_delete_marker(dpp, sc, sync_pipe, key, timestamp, owner, versioned,
            ...
            if (!error_ss.str().empty()) {
                yield call(sync_env->error_logger->log_error_cr(dpp, sc->conn->get_remote_id(), "data", error_ss.str(), -retcode,
    ...
    reenter(this) {
        do {
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                tn->log(20, SSTR("parsed entry: id=" << cur_id << " iter->object=" << entry->object << " iter->instance=" <<
                                 entry->instance << " name=" << key.name << " instance=" << key.instance << " ns=" << key.ns));
...
class RGWGetBucketPeersCR : public RGWCoroutine
{
    ...
    static pair<pipe_const_iter, pipe_const_iter> get_pipe_iters(const
            map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set> &m, std::optional<rgw_zone_id> zone)
    {
        if (!zone) {
    ...
    struct GetHintTargets : public RGWGenericAsyncCR::Action {
        ...
        int operate() override
        {
            int r = sync_env->svc->bucket_sync->get_bucket_sync_hints(sync_env->dpp,
                    ...
                    null_yield);
            if (r < 0) {
                ldpp_dout(sync_env->dpp, 0) << "ERROR: " << __func__ << "(): failed to fetch bucket sync hints for bucket=" <<
                                            source_bucket << dendl;
                return r;
            }
                ...
                if (gen) {
                    ...
                    if (*gen > current_gen) {
                        ...
                        if ((static_cast<std::size_t>(source_bs.shard_id) < bucket_status.shards_done_with_gen.size()) &&
                            ...
                            if (retcode < 0) {
                                tn->log(0, SSTR("ERROR: failed to log prev gen entry (bucket=" << source_bs.bucket << ", shard_id=" <<
                                                source_bs.shard_id << ", gen=" << current_gen << " in error repo: retcode=" << retcode));
                            } else {
                                tn->log(20, SSTR("logged prev gen entry (bucket=" << source_bs.bucket << ", shard_id=" << source_bs.shard_id << ", gen="
                                                 << current_gen << " in error repo: retcode=" << retcode));
                            }
                        }
                        retcode = -EAGAIN;
