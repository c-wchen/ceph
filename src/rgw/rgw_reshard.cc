...
class BucketReshardShard {
  ...
  public:
  BucketReshardShard(RGWRados * _store, const RGWBucketInfo & _bucket_info, int _num_shard, deque < librados::AioCompletion * >&_completions):
...
class BucketReshardManager {
  ...
  public:
     BucketReshardManager(RGWRados * _store,
                          ...
                          int _num_target_shards):store(_store),
        ...
        for (int i = 0; i < num_target_shards; ++i) {
            target_shards[i] =
                new BucketReshardShard(store, target_bucket_info, i,
                                       completions);
    ...
    int add_entry(int shard_index,
                  ...
                  const rgw_bucket_category_stats & entry_stats) {
        int ret =
            target_shards[shard_index]->add_entry(entry, account, category,
                                                  entry_stats);
        if (ret < 0) {
    ...
    int finish() {
      ...
      for (auto & shard:target_shards) {
            ...
            if (r < 0) {
                derr << "ERROR: target_shards[" << shard->
                    get_num_shard() << "].flush() returned error: " <<
      ...
      for (auto & shard:target_shards) {
            ...
            if (r < 0) {
                derr << "ERROR: target_shards[" << shard->
                    get_num_shard() << "].wait_all_aio() returned error: " <<
                    cpp_strerror(-r) << dendl;
                ret = r;
            }
            delete shard;
...
RGWBucketReshard::RGWBucketReshard(RGWRados * _store, const RGWBucketInfo & _bucket_info, const map < string, bufferlist > &_bucket_attrs, RGWBucketReshardLock * _outer_reshard_lock):
                                            ...
                                            cls_rgw_reshard_status status)
{
    if (new_instance_id.empty()) {
        ldout(store->ctx(),
              0) << __func__ << " missing new bucket instance id" << dendl;
        return -EINVAL;
    }
...
class BucketInfoReshardUpdate {
    ...
    int set_status(cls_rgw_reshard_status s) {
        ...
        int ret =
            store->put_bucket_instance_info(bucket_info, false, real_time(),
                                            &bucket_attrs);
        if (ret < 0) {
            ldout(store->ctx(),
  ...
  public:
  BucketInfoReshardUpdate(RGWRados * _store, RGWBucketInfo & _bucket_info, map < string, bufferlist > &_bucket_attrs, const string & new_bucket_id):
    store(_store),
        bucket_info(_bucket_info), bucket_attrs(_bucket_attrs) {
    ...
    ~BucketInfoReshardUpdate() {
        if (in_progress) {
            ...
            int ret =
                RGWBucketReshard::clear_index_shard_reshard_status(store,
                                                                   bucket_info);
            if (ret < 0) {
...
RGWBucketReshardLock::RGWBucketReshardLock(RGWRados * _store, const std::string & reshard_lock_oid, bool _ephemeral):
store(_store),
...
int RGWBucketReshardLock::lock()
{
    ...
    if (ret < 0) {
        ldout(store->ctx(), 0) << "RGWReshardLock::" << __func__ <<
            " failed to acquire lock on " << lock_oid << " ret=" << ret <<
            dendl;
        return ret;
    }
...
int RGWBucketReshard::do_reshard(int num_shards,
                                 ...
                                 int max_entries,
                                 bool verbose,
                                 ostream * out, Formatter * formatter)
{
    ...
    if (out) {
        ...
        (*out) << "old bucket instance id: " << bucket_info.bucket.bucket_id <<
            std::endl;
        (*out) << "new bucket instance id: " << new_bucket_info.bucket.
            bucket_id << std::endl;
    }
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              0) << __func__ << ": failed to update bucket info ret=" << ret <<
            dendl;
        return ret;
    }

    int num_target_shards =
        (new_bucket_info.num_shards > 0 ? new_bucket_info.num_shards : 1);

    BucketReshardManager target_shards_mgr(store, new_bucket_info,
                                           num_target_shards);
    ...
    for (int i = 0; i < num_source_shards; ++i) {
        ...
        while (is_truncated) {
            entries.clear();
            ret =
                store->bi_list(bucket, i, string(), marker, max_entries,
            ...
            for (auto iter = entries.begin(); iter != entries.end(); ++iter) {
                ...
                int ret =
                    store->get_target_shard_id(new_bucket_info,
                                               obj.get_hash_object(),
                                               &target_shard_id);
                if (ret < 0) {
                    lderr(store->
                          ctx()) << "ERROR: get_target_shard_id() returned ret="
                        << ret << dendl;
                    return ret;
                }
    ...
    ret =
        rgw_link_bucket(store, new_bucket_info.owner, new_bucket_info.bucket,
                        bucket_info.creation_time);
    if (ret < 0) {
        lderr(store->
              ctx()) << "failed to link new bucket instance (bucket_id=" <<
            new_bucket_info.bucket.
                              ...
                              Formatter * formatter, RGWReshard * reshard_log)
{
    ...
    ret = do_reshard(num_shards,
                     new_bucket_info, max_op_entries, verbose, out, formatter);
    if (ret < 0) {
        goto error_out;
    }
    ...
    ldout(store->ctx(), 1) << __func__ <<
        " INFO: reshard of bucket \"" << bucket_info.bucket.
        name << "\" from \"" << bucket_info.bucket.
        get_key() << "\" to \"" << new_bucket_info.bucket.
  ...
  error_out:
    ...
    if (ret2 < 0) {
        lderr(store->ctx()) << "Error: " << __func__ <<
            ...
            new_bucket_info.bucket.get_key() <<
            "\"created during incomplete resharding with error " << ret2 <<
            dendl;
    }
...
RGWReshard::RGWReshard(RGWRados * _store, bool _verbose, ostream * _out, Formatter * _formatter):
...
int RGWReshard::add(cls_rgw_reshard_entry & entry)
{
    if (!store->can_reshard()) {
        ldout(store->ctx(),
              20) << __func__ << " Resharding is disabled" << dendl;
        return 0;
    }
    ...
    if (ret < 0) {
        lderr(store->
              ctx()) << "ERROR: failed to add entry to reshard log, oid=" <<
                       ...
                       const RGWBucketInfo & new_bucket_info)
{
    ...
    entry.new_instance_id =
        new_bucket_info.bucket.name + ":" + new_bucket_info.bucket.bucket_id;
...
int RGWReshard::list(int logshard_num, string & marker, uint32_t max,
                     ...
                     bool * is_truncated)
{
    ...
    int ret =
        cls_rgw_reshard_list(store->reshard_pool_ctx, logshard_oid, marker, max,
                             entries, is_truncated);
    ...
    if (ret < 0) {
        ...
        lderr(store->
              ctx()) << "ERROR: failed to list reshard log entries, oid=" <<
            logshard_oid << dendl;
        if (ret == -EACCES) {
            lderr(store->ctx()) << "access denied to pool " << store->
                get_zone_params().
                reshard_pool <<
...
int RGWReshard::get(cls_rgw_reshard_entry & entry)
{
    ...
    if (ret < 0) {
        if (ret != -ENOENT) {
            lderr(store->
                  ctx()) << "ERROR: failed to get entry from reshard log, oid="
...
int RGWReshard::remove(cls_rgw_reshard_entry & entry)
{
    ...
    if (ret < 0) {
        lderr(store->
              ctx()) << "ERROR: failed to remove entry from reshard log, oid="
            << logshard_oid << " tenant=" << entry.
            tenant << " bucket=" << entry.bucket_name << dendl;
        return ret;
    }
...
int RGWReshard::clear_bucket_resharding(const string & bucket_instance_oid,
                                        cls_rgw_reshard_entry & entry)
{
    int ret =
        cls_rgw_clear_bucket_resharding(store->reshard_pool_ctx,
                                        bucket_instance_oid);
    if (ret < 0) {
        lderr(store->
              ctx()) <<
                                           ...
                                           const RGWBucketInfo & bucket_info)
{
    ...
    for (int i = 0; i < num_retries; i++) {
        ret =
            cls_rgw_get_bucket_resharding(bs->index_ctx, bs->bucket_obj,
                                          &entry);
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << __func__ << " ERROR: failed to get bucket resharding :"
                << cpp_strerror(-ret) << dendl;
            return ret;
        }
        ...
        ldout(store->ctx(),
              20) << "NOTICE: reshard still in progress; " << (i <
                                                               num_retries -
                                                               1 ? "retrying" :
                                                               "too many retries")
            << dendl;
        ...
        // and we're free to clear the flags.
        {
            ...
            if (ret < 0) {
                ...
                ldout(store->ctx(), 10) << __func__ <<
                    ...
                    bucket_id << dendl;
                ret =
                    RGWBucketReshard::clear_resharding(bs->store, bucket_info);
                if (ret < 0) {
                    reshard_lock.unlock();
                    ldout(store->ctx(), 0) << __func__ <<
    ...
    ldout(store->ctx(),
          0) << __func__ << " ERROR: bucket is still resharding, please retry"
        << dendl;
...
int RGWReshard::process_single_logshard(int logshard_num)
{
    ...
    if (ret == -EBUSY) {        /* already locked by another processor */
        ldout(store->ctx(),
              5) << __func__ << "(): failed to acquire lock on " << logshard_oid
            << dendl;
        return ret;
    }
    ...
    do {
      ...
      for (auto & entry:entries) {
            ...
            if (entry.new_instance_id.empty()) {
                ...
                ret =
                    store->get_bucket_info(obj_ctx, entry.tenant,
                                           entry.bucket_name, bucket_info,
                                           nullptr, &attrs);
                if (ret < 0) {
                    ldout(cct,
                          0) << __func__ << ": Error in get_bucket_info: " <<
                        cpp_strerror(-ret) << dendl;
                    return -ret;
                }
                ...
                ret =
                    br.execute(entry.new_num_shards, max_entries, true, nullptr,
                               formatter, this);
                if (ret < 0) {
                    ldout(store->ctx(), 0) << __func__ <<
                ...
                ldout(store->ctx(),
                      20) << " removing entry" << entry.bucket_name << dendl;
...
int RGWReshard::process_all_logshards()
{
    ...
    for (int i = 0; i < num_logshards; i++) {
        ...
        ldout(store->ctx(),
              20) << "proceeding logshard = " << logshard << dendl;
