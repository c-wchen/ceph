...
struct RGWOrphanSearchStage {
    ...
    {
    } explicit RGWOrphanSearchStage(RGWOrphanSearchStageId
                                    _stage): stage(_stage), shard(0)
    {
    }
    RGWOrphanSearchStage(RGWOrphanSearchStageId _stage, int _shard,
                         const std::string &_marker): stage(_stage),
        shard(_shard), marker(_marker)
    {
    }
...
public:
    explicit RGWOrphanStore(rgw::sal::RadosStore *_store): store(_store),
    ...
    int read_entries(const std::string &oid, const std::string &marker,
...
class RGWOrphanSearch
{
    ...
    int log_oids(const DoutPrefixProvider *dpp, std::map < int,
                 std::string > &log_shards, std::map < int,
                 std::list < std::string > > &oids);

#define RGW_ORPHANSEARCH_HASH_PRIME 7877
    int orphan_shard(const std::string &str)
    {
        return ceph_str_hash_linux(str.c_str(),
                                   str.size()) % RGW_ORPHANSEARCH_HASH_PRIME %
               search_info.num_shards;
    } int handle_stat_result(const DoutPrefixProvider *dpp, std::map < int,
                             std::list < std::string > > &oids,
                             RGWRados::Object::Stat::Result &result);
    int pop_and_handle_stat_op(const DoutPrefixProvider *dpp, std::map < int,
                               std::list < std::string > > &oids,
                               std::deque < RGWRados::Object::Stat > &ops);

    int remove_index(std::map < int, std::string > &index);
public:
    RGWOrphanSearch(rgw::sal::RadosStore *_store, int _max_ios,
                    uint64_t _stale_secs): store(_store), orphan_store(store),
        max_concurrent_ios(_max_ios), stale_secs(_stale_secs)
    {
    }
    ...
    int init(const DoutPrefixProvider *dpp, const std::string &job_name,
             RGWOrphanSearchInfo *info, bool _detailed_mode = false);
...
class RGWRadosList
{
    ...
    struct process_t {
        ...
        process_t(): entire_container(false)
        {
        }
    };
...
public:
    ...
    RGWRadosList(rgw::sal::RadosStore *_store,
                 ...
                 const std::string &_tenant_name): store(_store),
        max_concurrent_ios(_max_ios),
        stale_secs(_stale_secs),
        tenant_name(_tenant_name), include_rgw_obj_name(false)
    {
    }
    ...
    int run(const DoutPrefixProvider *dpp,
            const std::string &bucket_id, const bool silent_indexless = false);
    int run(const DoutPrefixProvider *dpp,
