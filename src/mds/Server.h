...
class Server {
  ...
  public:
    ...
    version_t prepare_force_open_sessions(map < client_t, entity_inst_t > &cm,
                                          ...
                                          uint64_t > >&smap);
    void finish_force_open_sessions(const map < client_t, pair < Session *,
                                    uint64_t > >&smap, bool dec_import = true);
    void flush_client_sessions(set < client_t > &client_set,
    ...
    void set_trace_dist(Session * session, MClientReply * reply, CInode * in,
                        CDentry * dn, snapid_t snapid, int num_dentries_wanted,
                        MDRequestRef & mdr);
    ...
    bool _check_access(Session * session, CInode * in, unsigned mask,
                       ...
                       int setattr_gid);
    CDir *validate_dentry_dir(MDRequestRef & mdr, CInode * diri,
                              boost::string_view dname);
    CDir *traverse_to_auth_dir(MDRequestRef & mdr, vector < CDentry * >&trace,
                               filepath refpath);
    CDentry *prepare_null_dentry(MDRequestRef & mdr, CDir * dir,
    ...
    CInode *rdlock_path_pin_ref(MDRequestRef & mdr, int n,
                                set < SimpleLock * >&rdlocks, bool want_auth,
                                bool no_want_auth =
                                false, file_layout_t ** layout =
                                NULL, bool no_lookup = false);
    ...
    int check_layout_vxattr(MDRequestRef & mdr,
                            string name, string value, file_layout_t * layout);
    ...
    void do_link_rollback(bufferlist & rbl, mds_rank_t master,
                          MDRequestRef & mdr);
    ...
    bool _rmdir_prepare_witness(MDRequestRef & mdr, mds_rank_t who,
                                vector < CDentry * >&trace, CDentry * straydn);
    void handle_slave_rmdir_prep(MDRequestRef & mdr);
    void _logged_slave_rmdir(MDRequestRef & mdr, CDentry * srcdn,
                             CDentry * straydn);
    void _commit_slave_rmdir(MDRequestRef & mdr, int r, CDentry * straydn);
    void handle_slave_rmdir_prep_ack(MDRequestRef & mdr,
                                     MMDSSlaveRequest * ack);
    void do_rmdir_rollback(bufferlist & rbl, mds_rank_t master,
                           MDRequestRef & mdr);
    ...
    void handle_slave_rename_notify_ack(MDRequestRef & mdr,
                                        MMDSSlaveRequest * m);
    void _slave_rename_sessions_flushed(MDRequestRef & mdr);
    void _logged_slave_rename(MDRequestRef & mdr, CDentry * srcdn,
                              CDentry * destdn, CDentry * straydn);
    void _commit_slave_rename(MDRequestRef & mdr, int r, CDentry * srcdn,
                              CDentry * destdn, CDentry * straydn);
    void do_rename_rollback(bufferlist & rbl, mds_rank_t master,
