...
void OSDMapMapping::_update_range(const OSDMap &osdmap,
                                  int64_t pool,
                                  unsigned pg_begin, unsigned pg_end)
{
    ...
    for (unsigned ps = pg_begin; ps < pg_end; ++ps) {
        ...
        osdmap.pg_to_up_acting_osds(pg_t(ps, pool),
...
void ParallelPGMapper::queue(Job *job,
                             ...
                             const vector < pg_t > &input_pgs)
{
    ...
    for (auto &p : job->osdmap->get_pools()) {
        for (unsigned ps = 0; ps < p.second.get_pg_num(); ps += pgs_per_item) {
            unsigned ps_end =
                std::min(ps + pgs_per_item, p.second.get_pg_num());
            job->start_one();
            wq.queue(new Item(job, p.first, ps, ps_end));
            ldout(cct,
                  20) << __func__ << " " << job << " " << p.
                      first << " [" << ps << "," << ps_end << ")" << dendl;
            any = true;
        }
    }
    ceph_assert(any);
