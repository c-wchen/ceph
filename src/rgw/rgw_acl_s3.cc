...
bool ACLGrant_S3::xml_end(const char *el)
{
    ...
    acl_permission =
        static_cast < ACLPermission_S3 * >(find_first("Permission"));
    if (!acl_permission)
        return false;
    ...
    case ACL_TYPE_CANON_USER:
        ...
        acl_name =
            static_cast <
            ACLDisplayName_S3 * >(acl_grantee->find_first("DisplayName"));
        if (acl_name)
    ...
    case ACL_TYPE_EMAIL_USER:
        acl_email =
            static_cast <
            ACLEmail_S3 * >(acl_grantee->find_first("EmailAddress"));
        if (!acl_email)
            return false;
...
static int parse_grantee_str(const DoutPrefixProvider * dpp,
                             rgw::sal::Driver * driver, string & grantee_str,
                             const struct s3_acl_header *perm, ACLGrant & grant)
{
    ...
    if (strcasecmp(id_type.c_str(), "emailAddress") == 0) {
        ...
        std::unique_ptr < rgw::sal::User > user =
            driver->get_user(rgw_user(id_val));
...
static int parse_acl_header(const DoutPrefixProvider * dpp,
                            rgw::sal::Driver * driver, const RGWEnv * env,
                            const struct s3_acl_header *perm,
                            std::list < ACLGrant > &_grants)
{
    ...
    for (list < string >::iterator it = grantees.begin(); it != grantees.end();
         ++it) {
        ACLGrant grant;
...
int RGWAccessControlList_S3::create_canned(ACLOwner & owner,
                                           ...
                                           const string & canned_acl)
{
    ...
    owner_grant.set_canon(owner.get_id(), owner.get_display_name(),
...
int RGWAccessControlList_S3::create_from_grants(std::list < ACLGrant > &grants)
{
    ...
    for (std::list < ACLGrant >::iterator it = grants.begin();
         it != grants.end(); ++it) {
        ACLGrant g = *it;
        add_grant(&g);
    }
...
void RGWAccessControlPolicy_S3::to_xml(ostream & out)
{
    ...
    RGWAccessControlList_S3 & _acl =
        static_cast < RGWAccessControlList_S3 & >(acl);
    _owner.to_xml(out);
    _acl.to_xml(out);
...
int RGWAccessControlPolicy_S3::create_from_headers(const DoutPrefixProvider *
                                                   dpp,
                                       ...
                                       std::string & err_msg)
{
    ...
    ACLOwner *requested_owner =
        static_cast < ACLOwner_S3 * >(find_first("Owner"));
    if (requested_owner) {
    ...
    ldpp_dout(dpp,
              20) << "dest owner id=" << dest.get_owner().get_id() << dendl;
        ...
        case ACL_TYPE_EMAIL_USER:
            {
                ...
                if (driver->get_user_by_email(dpp, email, null_yield, &user) <
                    0) {
                    ldpp_dout(dpp,
                              10) << "grant user email not found or other error"
                        << dendl;
                    err_msg =
        ...
        case ACL_TYPE_CANON_USER:
            {
                if (type.get_type() == ACL_TYPE_CANON_USER) {
                    if (!src_grant.get_id(uid)) {
                        ldpp_dout(dpp,
                                  0) << "ERROR: src_grant.get_id() failed" <<
                            dendl;
                        err_msg = "Invalid id";
                        return -EINVAL;
                    }
                ...
                new_grant.set_canon(uid, grant_user.display_name,
                                    perm.get_permissions());
                grant_ok = true;
                rgw_user new_id;
                new_grant.get_id(new_id);
                ldpp_dout(dpp,
                          10) << "new grant: " << new_id << ":" << grant_user.
                    display_name << dendl;
            }
            break;
        case ACL_TYPE_GROUP:
...
bool RGWAccessControlPolicy_S3::compare_group_name(string & id,
