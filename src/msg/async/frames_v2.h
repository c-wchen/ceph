...
// bit would be a cleaner approach, but those are scarce these days.
    const uint64_t msgr2_frame_assumed =
        msgr2_required | CEPH_FEATUREMASK_SERVER_NAUTILUS;
    ...
    static constexpr uint8_t CRYPTO_BLOCK_SIZE {
    16};
...
// XXX: currently the multi-segment facility is NOT implemented.
    ...
    __attribute__ ((packed));
    static_assert(sizeof(epilogue_secure_rev0_block_t) % CRYPTO_BLOCK_SIZE ==
                  0);
...
// epilogue_secure_rev0_block_t with late_flags changed to late_status
    ...
    __attribute__ ((packed));
    static_assert(sizeof(epilogue_secure_rev1_block_t) % CRYPTO_BLOCK_SIZE ==
                  0);
    ...
    class FrameAssembler {
      public:
        ...
        FrameAssembler(const ceph::crypto::onwire::rxtx_t * crypto,
                       bool is_rev1, bool with_data_crc,
        ...
        // Preamble:////   preamble_block_t//   [preamble inline buffer + auth tag -- only in msgr2.1 secure mode]//// The preamble is generated unconditionally.//// In msgr2.1 secure mode, the first segment is inlined into the// preamble inline buffer, either fully or partially.
            ...
            if (m_crypto->rx) {
                ...
                if (m_is_rev1 && seg_idx == 0) {
                    if (padded_len > FRAME_PREAMBLE_INLINE_SIZE) {
                        return padded_len + get_auth_tag_len() -
      ...
      private:
        ...
        boost::container::static_vector < segment_desc_t,
            MAX_NUM_SEGMENTS > m_descs;
        __u8 m_flags;
...
// Args pack while ControlFrame provides common encode/decode machinery.
  template < class C, typename ... Args > class ControlFrame:public Frame < C,
        segment_t::DEFAULT_ALIGNMENT /* single segment */  >
    {
      protected:
        ...
        template < typename T > inline void _encode_payload_each(T & t) {
            if constexpr
                (std::is_same < T, std::vector < uint32_t > const >()) {
                    encode((uint32_t) t.size(), this->get_payload_segment(),
                           features);
        ...
        template < typename T >
            inline void _decode_payload_each(T & t,
                                             bufferlist::
                                             const_iterator & ti) const {
        ...
        } template < std::size_t N > inline decltype(auto) get_val() {
    ...
    struct HelloFrame:public ControlFrame < HelloFrame, uint8_t,    // entity type
        entity_addr_t > {       // peer address
        static const Tag tag = Tag::HELLO;
        using ControlFrame::Encode;
        using ControlFrame::Decode;

        inline uint8_t & entity_type() {
            return get_val < 0 > ();
        } inline entity_addr_t & peer_addr() {
            return get_val < 1 > ();
        }
    ...
    struct AuthRequestFrame:public ControlFrame < AuthRequestFrame, uint32_t,   // auth method
    std::vector < uint32_t >,   // preferred modes
        ...
        inline uint32_t & method() {
            return get_val < 0 > ();
        } inline std::vector < uint32_t > &preferred_modes() {
            return get_val < 1 > ();
        }
        inline bufferlist & auth_payload() {
            return get_val < 2 > ();
        }
    ...
    struct AuthBadMethodFrame:public ControlFrame < AuthBadMethodFrame, uint32_t,   // method
        int32_t,                // result
    ...
    std::vector < uint32_t >> { // allowed modes
        ...
        inline uint32_t & method() {
            return get_val < 0 > ();
        } inline int32_t & result() {
            return get_val < 1 > ();
        }
        inline std::vector < uint32_t > &allowed_methods() {
            return get_val < 2 > ();
        }
        inline std::vector < uint32_t > &allowed_modes() {
            return get_val < 3 > ();
        }
    ...
    struct AuthReplyMoreFrame:public ControlFrame < AuthReplyMoreFrame, bufferlist > {  // auth payload
    ...
    struct AuthRequestMoreFrame:public ControlFrame < AuthRequestMoreFrame, bufferlist > {  // auth payload
    ...
    struct AuthDoneFrame:public ControlFrame < AuthDoneFrame, uint64_t, // global id
        ...
        inline uint64_t & global_id() {
            return get_val < 0 > ();
        } inline uint32_t & con_mode() {
            return get_val < 1 > ();
        }
        inline bufferlist & auth_payload() {
            return get_val < 2 > ();
    ...
    :public ControlFrame < AuthSignatureFrame, sha256_digest_t > {
        ...
        inline sha256_digest_t & signature() {
            return get_val < 0 > ();
      } protected:
        using ControlFrame::ControlFrame;
    };
    ...
    :public ControlFrame < ClientIdentFrame, entity_addrvec_t,  // my addresses
        ...
        inline entity_addrvec_t & addrs() {
            return get_val < 0 > ();
        } inline entity_addr_t & target_addr() {
            return get_val < 1 > ();
        }
        inline int64_t & gid() {
            return get_val < 2 > ();
        }
        inline uint64_t & global_seq() {
            return get_val < 3 > ();
        }
        inline uint64_t & supported_features() {
            return get_val < 4 > ();
        }
        inline uint64_t & required_features() {
            return get_val < 5 > ();
        }
    ...
    :public ControlFrame < ServerIdentFrame, entity_addrvec_t,  // my addresses
        int64_t,                // global_id
        uint64_t,               // global seq
        uint64_t,               // supported features
        uint64_t,               // required features
    ...
    :public ControlFrame < ReconnectFrame, entity_addrvec_t,    // my addresses
        uint64_t,               // client cookie
        uint64_t,               // server cookie
        ...
        inline entity_addrvec_t & addrs() {
            return get_val < 0 > ();
        } inline uint64_t & client_cookie() {
            return get_val < 1 > ();
        }
        inline uint64_t & server_cookie() {
            return get_val < 2 > ();
        }
        inline uint64_t & global_seq() {
            return get_val < 3 > ();
        }
        inline uint64_t & connect_seq() {
            return get_val < 4 > ();
        }
        inline uint64_t & msg_seq() {
            return get_val < 5 > ();
        }
    ...
    struct ResetFrame:public ControlFrame < ResetFrame, bool > {    // full reset
    ...
    struct RetryFrame:public ControlFrame < RetryFrame, uint64_t > {    // connection seq
    ...
    struct RetryGlobalFrame:public ControlFrame < RetryGlobalFrame, uint64_t > {    // global seq
    ...
    struct ReconnectOkFrame:public ControlFrame < ReconnectOkFrame, uint64_t > {    // message seq
    ...
    :public ControlFrame < IdentMissingFeaturesFrame, uint64_t > {  // missing features mask
        static const Tag tag = Tag::IDENT_MISSING_FEATURES;
        using ControlFrame::Encode;
        using ControlFrame::Decode;

        inline uint64_t & features() {
            return get_val < 0 > ();
      } protected:
        using ControlFrame::ControlFrame;
    };

    struct KeepAliveFrame:public ControlFrame < KeepAliveFrame, utime_t > { // timestamp
    ...
    struct KeepAliveFrameAck:public ControlFrame < KeepAliveFrameAck, utime_t > {   // ack timestamp
    ...
    struct AckFrame:public ControlFrame < AckFrame, uint64_t > {    // message sequence
...
// being the `extra_payload_len` passed to get_buffer().
    ...
    segment_t::DEFAULT_ALIGNMENT,
        ...
        static MessageFrame Encode(const ceph_msg_header2 & msg_header,
            ...
            MessageFrame f;
            f.segments[SegmentIndex::Msg::HEADER].append(reinterpret_cast <
                                                         const char
                                                         *>(&msg_header),
                                                         sizeof(msg_header));
        ...
        inline const ceph_msg_header2 & header() {
            ...
            return reinterpret_cast <
                const ceph_msg_header2 & >(*hdrbl.c_str());
        }
    ...
    struct CompressionRequestFrame:public ControlFrame < CompressionRequestFrame, bool, // is compress
    std::vector < uint32_t >> { // preferred methods
        static const Tag tag = Tag::COMPRESSION_REQUEST;
        using ControlFrame::Encode;
        using ControlFrame::Decode;

        inline bool & is_compress() {
            return get_val < 0 > ();
        } inline std::vector < uint32_t > &preferred_methods() {
            return get_val < 1 > ();
        }
    ...
    struct CompressionDoneFrame:public ControlFrame < CompressionDoneFrame, bool,   // is compress
        uint32_t > {            // method
