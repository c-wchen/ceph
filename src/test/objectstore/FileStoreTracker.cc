  ...
  public:
    ...
    :tracker(tracker), in_flight(in_flight) {
        ...
        for (list < pair < pair < coll_t, string >, uint64_t > >::iterator i =
             in_flight.begin(); i != in_flight.end(); ++i) {
...
void FileStoreTracker::submit_transaction(Transaction & t)
{
    ...
    store->queue_transaction(0, std::move(*out.t),
                                   ...
                                   OutTransaction * out)
{
    ...
    out->t->clone_range(coll_t(from.first),
                        ghobject_t(hobject_t
                                   (sobject_t(from.second, CEPH_NOSNAP))),
                        ghobject_t(hobject_t
                                   (sobject_t(to.second, CEPH_NOSNAP))), offset,
                        len, offset);
                             ...
                             OutTransaction * out)
{
    ...
    if (to_contents.exists())
        out->t->remove(coll_t(to.first),
                       ghobject_t(hobject_t
                                  (sobject_t(to.second, CEPH_NOSNAP))));
...
struct ObjStatus {
    ...
    uint64_t trim_to() const {
        return last_applied < last_committed ? last_applied : last_committed;
}};
...
void set_obj_status(const pair < coll_t, string > &obj,
                    const ObjStatus & status, KeyValueDB::Transaction t)
{
...
void _clean_forward(const pair < coll_t, string > &obj,
                    uint64_t last_valid, KeyValueDB * db)
{
    KeyValueDB::Transaction t = db->get_transaction();
                              ...
                              bool on_start)
{
    ...
    pair < uint64_t, uint64_t > valid_reads =
        get_valid_reads(make_pair(coll, obj));
    ...
    for (uint64_t i = valid_reads.first; i < valid_reads.second; ++i) {
        ...
        std::cerr << "old_contents exists " << old_contents.
            exists() << std::endl;
    ...
    std::cerr << "Verifying " << make_pair(coll,
                                           obj) << " failed " << std::endl;
    ceph_abort();
}

ObjectContents FileStoreTracker::get_current_content(const pair < coll_t,
                                                     string > &obj)
{
...
ObjectContents FileStoreTracker::get_content(const pair < coll_t, string > &obj,
                                             uint64_t version)
{
    set < string > to_get;
    map < string, bufferlist > got;
...
pair < uint64_t,
    uint64_t > FileStoreTracker::get_valid_reads(const pair < coll_t,
                                                 string > &obj)
{
...
void clear_obsolete(const pair < coll_t, string > &obj,
                    ...
                    KeyValueDB * db, KeyValueDB::Transaction t)
{
    KeyValueDB::Iterator iter = db->get_iterator(obj_to_prefix(obj));
    set < string > to_remove;
    iter->seek_to_first();
