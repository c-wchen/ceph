...
bool may_be_trimmed(const std::map < uint64_t,
                    uint64_t > &extent_map, const bufferlist &bl,
                    size_t sparse_size, uint64_t *new_end_ptr)
{
    if (extent_map.empty()) {
        *new_end_ptr = 0;
        return true;
    ...
    uint64_t end =
        extent_map.rbegin()->first + extent_map.rbegin()->second;
    uint64_t new_end = end;
    uint64_t bl_off = bl.length();

    for (auto it = extent_map.rbegin(); it != extent_map.rend();
         it++) {
        auto off = it->first;
        auto len = it->second;
...
public:
    ...
    C_SparsifyObject(AsyncObjectThrottle < I > &throttle, I *image_ctx,
        ...
        m_cct(image_ctx->cct), m_object_no(object_no),
    ...
    int send() override
    {
...

        {
            ...
            int r =
                image_ctx.get_parent_overlap(CEPH_NOSNAP, &raw_overlap);
            ceph_assert(r == 0);
            if (raw_overlap > 0) {
    ...
    void send_sparsify()
    {
        ...
        auto comp = create_rados_callback <
                    C_SparsifyObject,
                    &C_SparsifyObject::handle_sparsify > (this);
    ...
    void handle_sparsify(int r)
    {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to sparsify: " << cpp_strerror(r) <<
                         dendl;
            finish_op(r);
            return;
        }
    ...
    void send_pre_update_object_map()
    {
        ...
        auto ctx = create_context_callback <
                   C_SparsifyObject < I >,
                   &C_SparsifyObject < I >::handle_pre_update_object_map >
                   (this);
    ...
    void send_check_exists()
    {
        ...
        auto comp = create_rados_callback <
                    C_SparsifyObject,
                    &C_SparsifyObject::handle_check_exists > (this);
    ...
    void send_post_update_object_map(bool exists)
    {
        ...
        auto ctx = create_context_callback <
                   C_SparsifyObject < I >,
                   &C_SparsifyObject < I >::handle_post_update_object_map >
                   (this);
        bool sent;
        {
    ...
    void send_read()
    {
        ...
        librados::ObjectReadOperation op;
        m_bl.clear();
        op.sparse_read(0, image_ctx.layout.object_size, &m_extent_map,
                       &m_bl, nullptr);
        auto comp =
            create_rados_callback < C_SparsifyObject,
            &C_SparsifyObject::handle_read > (this);
        int r = image_ctx.data_ctx.aio_operate(m_oid, comp, &op, &m_bl);
        ceph_assert(r == 0);
        comp->release();
    }
    ...
    void handle_read(int r)
    {
        ...
        if (r < 0) {
            if (r == -ENOENT) {
                ...
                lderr(m_cct) << "failed to read object: " <<
        ...
        if (!may_be_trimmed
            (m_extent_map, m_bl, m_sparse_size, &m_new_end)) {
            finish_op(0);
            return;
        }
    ...
    void handle_trim(int r)
    {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(m_cct) << "failed to trim: " << cpp_strerror(r) <<
                         dendl;
            finish_op(r);
            return;
        }
...
void SparsifyRequest < I >::handle_sparsify_objects(int r)
{
    ...
    if (r == -ERESTART) {
        ...
        lderr(cct) << "sparsify encountered an error: " <<
                   cpp_strerror(r) << dendl;
        this->complete(r);
        return;
    }
