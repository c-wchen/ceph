...
namespace librados {
    ...
    class TestMemIoCtxImpl:public TestIoCtxImpl {
      public:
        ...
        int set_alloc_hint(const std::string & oid,
                           uint64_t expected_object_size,
                           uint64_t expected_write_size,
                           const SnapContext & snapc) override;
        int sparse_read(const std::string & oid, uint64_t off, uint64_t len,
                        std::map < uint64_t, uint64_t > *m,
                        bufferlist * data_bl) override;
        int stat(const std::string & oid, uint64_t * psize,
                 time_t * pmtime) override;
        ...
        int writesame(const std::string & oid, bufferlist & bl, size_t len,
                      uint64_t off, const SnapContext & snapc) override;
        int cmpext(const std::string & oid, uint64_t off,
                   bufferlist & cmp_bl) override;
        int xattr_get(const std::string & oid, std::map < std::string,
                      bufferlist > *attrset) override;
      ...
      protected:
        ...
        void ensure_minimum_length(size_t len, bufferlist * bl);
         ...
         TestMemCluster::SharedFile get_file(const std::string & oid,
                                             bool write,
