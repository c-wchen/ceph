...
template < typename T, typename K > class WeightedPriorityQueue:public OpQueue < T, K >
{
  ...
  class ListPair:public bi::list_base_hook <> {
      public:
        unsigned cost;
        T item;
        ListPair(unsigned c, T & i):cost(c), item(i) {
        }
    };
    ...
    class SubQueue:public bi::set_base_hook <> {
        ...
        bool empty() const {
            ...
            if (ret.second) {
                ret.first =
                    klasses.insert_unique_commit(*new Klass(cl), insert_data);
                check_end();
            }
    ...
    class Queue {
      ...
      public:
        unsigned size;
        Queue(): total_prio(0), max_cost(0), size(0) {
        } ~Queue() {
            queues.clear_and_dispose(DelItem < SubQueue > ());
        }
        bool empty() const {
            return !size;
        } void insert(unsigned p, K cl, unsigned cost, T & item, bool front =
                      false) {
            ...
            if (ret.second) {
                ret.first =
                    queues.insert_unique_commit(*new SubQueue(p), insert_data);
                total_prio += p;
            }
