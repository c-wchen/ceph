...
void Journaler::_set_layout(file_layout_t const *l)
{
    ...
    uint64_t periods =
        cct->_conf.get_val < uint64_t > ("journaler_prefetch_periods");
...
void Journaler::_finish_probe_end(int r, uint64_t end)
{
    ...
    if (((int64_t) end) == -1) {
        ...
        ldout(cct,
              1) << "_finish_probe_end write_pos = " << end << " (header had "
...
void Journaler::_do_flush(unsigned amount)
{
    ...
    if (flush_pos + len + 2 * period > prezero_pos) {
        ...
        if (newlen <= 0) {
            ldout(cct,
                  10) << "_do_flush wanted to do " << flush_pos << "~" << len <<
...
void Journaler::_flush(C_OnFinisher * onsafe)
{
    ...
    if (write_pos == flush_pos) {
        ...
        ldout(cct, 10) << "flush nothing to flush, (prezeroing/prezero)/write/"
            ...
            prezero_pos << ")/" << write_pos << "/" << flush_pos << "/" <<
            safe_pos << dendl;
        if (onsafe) {
            onsafe->complete(0);
        }
...
void Journaler::_issue_prezero()
{
    ...
    if (prezeroing_pos >= to) {
        ldout(cct,
              20) << "_issue_prezero target " << to << " <= prezeroing_pos " <<
...
void Journaler::_assimilate_prefetch()
{
    ...
    if (got_any) {
        ldout(cct,
            ...
            " received_pos=" << received_pos << " requested_pos=" <<
            requested_pos << dendl;
...
void Journaler::_issue_read(uint64_t len)
{
    ...
    ldout(cct, 10) << "_issue_read reading " << requested_pos << "~" << len
        << ", read pointers read_pos=" << read_pos << " received_pos=" <<
        received_pos << " requested_pos+len=" << (requested_pos + len) << dendl;
...
void Journaler::_prefetch()
{
    ...
    if (temp_fetch_len) {
        ldout(cct,
              10) << "_prefetch temp_fetch_len " << temp_fetch_len << dendl;
        pf = temp_fetch_len;
        temp_fetch_len = 0;
    }
    else {
    ...
    if (requested_pos < target) {
        ...
        ldout(cct,
              10) << "_prefetch " << pf << " requested_pos " << requested_pos <<
            " < target " << target << " (" << raw_target << "), prefetching " <<
            len << dendl;
...
bool Journaler::_have_next_entry()
{
    ...
    ldout(cct,
          10) << "_have_next_entry read_buf.length() == " << read_buf.length()
...
bool Journaler::try_read_entry(bufferlist & bl)
{
    ...
    try {
        ...
        lderr(cct) << __func__ << ": decode error from _have_next_entry" <<
            dendl;
        error = -EINVAL;
        return false;
    }
...
void Journaler::_trim()
{
    ...
    if (trimming_pos > trimmed_pos) {
        ldout(cct, 10) << "trim already trimming atm, try again later.  "
            "trimmed/trimming is " << trimmed_pos << "/" << trimming_pos <<
            dendl;
        return;
    }
...
void Journaler::handle_write_error(int r)
{
    ...
    if (on_write_error) {
        ...
        lderr(cct) << __func__ <<
            ": multiple write errors, handler already called" << dendl;
    }
    else {
