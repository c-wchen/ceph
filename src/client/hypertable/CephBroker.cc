                        ...
                        int64_t blksz)
{
    ...
    if ((r = ceph_mkdirs(cmount, directory.c_str(), 0644)) < 0
        && r != -CEPHFS_EEXIST) {
        HT_ERRORF("create failed on mkdirs: dname='%s' - %d", directory.c_str(),
                  -r);
        report_error(cb, -r);
        return;
    }
...
void CephBroker::read(ResponseCallbackRead *cb, uint32_t fd, uint32_t amount)
{
    ...
    if ((offset = ceph_lseek(cmount, fdata->fd, 0, SEEK_CUR)) < 0) {
        ...
        HT_ERRORF("lseek failed: fd=%" PRIu32
                  " ceph_fd=%d offset=0 SEEK_CUR - %s", fd, fdata->fd,
                  errs.c_str());
    ...
    if ((nread = ceph_read(cmount, fdata->fd, (char *)buf.base, amount, 0)) < 0) {
        HT_ERRORF("read failed: fd=%" PRIu32 " ceph_fd=%d amount=%d", fd,
                  fdata->fd, amount);
                        ...
                        uint32_t amount, const void *data, bool sync)
{
    ...
    if (sync && ((r = ceph_fsync(cmount, fdata->fd, true)) != 0)) {
        ...
        HT_ERRORF("flush failed: fd=%" PRIu32 " ceph_fd=%d - %s", fd, fdata->fd,
                  errs.c_str());
        report_error(cb, r);
        return;
    }
                       ...
                       uint32_t amount, bool)
{
    ...
    HT_DEBUGF("pread fd=%" PRIu32 " offset=%llu amount=%d", fd, (Llu) offset,
              amount);
...
void CephBroker::mkdirs(ResponseCallback *cb, const char *dname)
{
    ...
    int r;
    if ((r = ceph_mkdirs(cmount, absdir.c_str(), 0644)) < 0
        && r != -CEPHFS_EEXIST) {
