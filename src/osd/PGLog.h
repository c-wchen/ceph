...
constexpr auto PGLOG_INDEXED_ALL = PGLOG_INDEXED_OBJECTS
    ...
    | PGLOG_INDEXED_EXTRA_CALLER_OPS | PGLOG_INDEXED_DUPS;
...
struct PGLog:DoutPrefixProvider {
    ...
    ////////////////////////////// sub classes ////////////////////////////// struct LogEntryHandler {
   ...
   */
    struct IndexedLog:public pg_log_t {
        ...
        mutable ceph::unordered_multimap < osd_reqid_t,
            pg_log_entry_t * >extra_caller_ops;
     ...
     */
         ...
         */
         template < typename F >
            ...
            bool dirty_log = to > can_rollback_to
                || to > rollback_info_trimmed_to;
            if (dirty_log) {
                ...
                --rollback_info_trimmed_to_riter;
                if (rollback_info_trimmed_to_riter->version >
                    rollback_info_trimmed_to) {
                    ++rollback_info_trimmed_to_riter;
                    break;
                }
      ...
      IndexedLog():
        complete_to(log.end()),
            last_requested(0),
            indexed_data(0), rollback_info_trimmed_to_riter(log.rbegin()) {
        }

      template < typename ... Args > explicit IndexedLog(Args && ... args):
        ...
        IndexedLog(const IndexedLog & rhs):pg_log_t(rhs),
            complete_to(log.end()),
        ...
        void trim_rollback_info_to(eversion_t to, LogEntryHandler * h) {
            advance_can_rollback_to(to,[&](pg_log_entry_t & entry) {
                                    h->trim(entry);});
        }
        bool roll_forward_to(eversion_t to, LogEntryHandler * h) {
            return advance_can_rollback_to(to,[&](pg_log_entry_t & entry) {
                                           h->rollforward(entry);}
            );
        }
        ...
        template < typename T > void scan_log_after(const eversion_t & bound,   ///< [in] scan entries > bound
                                                    T && f) const {
    ...
    /****/ void claim_log_and_clear_rollback_info(const pg_log_t & o) {
        ...
        void split_out_child(pg_t child_pgid,
                             unsigned split_bits, IndexedLog * target);
        ...
        bool get_request(const osd_reqid_t & r,
                         eversion_t * version,
                ...
                for (auto i = p->second->extra_reqids.begin();
                    ...
                    if (i->first == r) {
                        ...
                        *op_returns = p->second->op_returns;
                        if (*return_code >= 0) {
                            auto it =
                                p->second->extra_reqid_return_codes.find(idx);
        ...
        void get_object_reqids(const hobject_t & oid, unsigned max,
                               ...
                               osd_reqid_t, version_t > >*pls,
                               mempool::osd_pglog::map < uint32_t,
                               int >*return_codes) const {
            ...
            for (auto i = log.rbegin(); i != log.rend(); ++i) {
                if (i->soid == oid) {
                    if (i->reqid_is_indexed()) {
                        ...
                        pls->
                            push_back(std::
                                      make_pair(i->reqid, i->user_version));
                    }

                    pls->insert(pls->end(), i->extra_reqids.begin(),
                                i->extra_reqids.end());
                    if (pls->size() >= max) {
                        if (pls->size() > max) {
                            pls->resize(max);
        ...
        void index(__u16 to_index = PGLOG_INDEXED_ALL) const {
            ...
            if (to_index & PGLOG_INDEXED_DUPS) {
                ...
                PGLOG_INDEXED_CALLER_OPS | PGLOG_INDEXED_EXTRA_CALLER_OPS;
        ...
        void add(const pg_log_entry_t & e, bool applied = true) {
            ...
            if (indexed_data & PGLOG_INDEXED_EXTRA_CALLER_OPS) {
                for (auto j = e.extra_reqids.begin();
                     j != e.extra_reqids.end(); ++j) {
                    extra_caller_ops.
                        insert(std::make_pair(j->first, &(log.back())));
                }
            }
  ...
  public:
    ...
    static void clear_after(std::set < std::string > *log_keys_debug,
                            const std::string & lb) {
        if (!log_keys_debug)
            return;
        for (auto i = log_keys_debug->lower_bound(lb);
             i != log_keys_debug->end(); log_keys_debug->erase(i++)) ;
    }
    static void clear_up_to(std::set < std::string > *log_keys_debug,
                            const std::string & ub) {
        if (!log_keys_debug)
            return;
  ...
  PGLog(CephContext * cct):
    dirty_from(eversion_t::max()),
        ...
        touched_log(false), dirty_log(false), clear_divergent_priors(false) {
    }
    ...
    const pg_missing_tracker_t & get_missing() const {
        return missing;
    ...
    void trim(eversion_t trim_to,
              pg_info_t & info,
              bool transaction_applied = true, bool async = false);

    void roll_forward_to(eversion_t roll_forward_to, LogEntryHandler * h) {
        if (log.roll_forward_to(roll_forward_to, h))
            dirty_log = true;
    }
    ...
    void merge_from(const std::vector < PGLog * >&sources,
...
#ifdef WITH_SEASTAR
    seastar::future <> rebuild_missing_set_with_deletes_crimson(crimson::os::
                                                                FuturizedStore::
                                                                Shard & store,
                                                                crimson::os::
                                                                CollectionRef
  ...
  protected:
    static void split_by_object(mempool::osd_pglog::list < pg_log_entry_t >
                                &entries, std::map < hobject_t,
                                mempool::osd_pglog::list <
                                pg_log_entry_t >> *out_entries) {
        while (!entries.empty()) {
   ...
   *    necessary.
   */
    template < typename missing_type > static void _merge_object_divergent_entries(const IndexedLog & log,  ///< [in] log to merge against
        ...
        const bool object_not_in_store =
            !missing.is_missing(hoid) && entries.rbegin()->is_delete();
        ...
        if (objiter != log.objects.end() &&
            ...
            if (objiter->second->is_update() ||
                ...
                ceph_assert(missing.is_missing(hoid) &&
                            missing.get_items().at(hoid).need ==
                            objiter->second->version);
            }
            else {
                ceph_assert(!missing.is_missing(hoid));
            }
        ...
        if (can_rollback) {
            /// Case 4)
            for (auto i = entries.rbegin(); i != entries.rend(); ++i) {
                ceph_assert(i->can_rollback()
                            && i->version > olog_can_rollback_to);
                ldpp_dout(dpp,
                          10) << __func__ << ": hoid " << hoid <<
            ...
            ldpp_dout(dpp,
                      10) << __func__ << ": hoid " << hoid <<
                " cannot roll back, " << "removing and adding to missing" <<
                dendl;
            if (rollbacker) {
                if (!object_not_in_store)
                    rollbacker->remove(hoid);
    ...
    template < typename missing_type > static void _merge_divergent_entries(const IndexedLog & log, ///< [in] log to merge against
        ...
        std::map < hobject_t,
            mempool::osd_pglog::list < pg_log_entry_t > >split;
        split_by_object(entries, &split);
        for (auto i = split.begin(); i != split.end(); ++i) {
            _merge_object_divergent_entries(log,
                                            i->first,
                                            i->second,
                                            oinfo,
                                            olog_can_rollback_to,
                                            omissing, rollbacker, dpp);
        }
    }
   ...
   * cases
   */
    void merge_old_entry(ObjectStore::Transaction & t,
  ...
  public:
    ...
    template < typename missing_type >
        static bool append_log_entries_update_missing(const hobject_t &
                                                      last_backfill,
                                                      const mempool::osd_pglog::
                                                      list < pg_log_entry_t >
                                                      &entries,
            ...
            if (p->soid <= last_backfill && !p->is_error()) {
                if (missing.may_include_deletes) {
                    ...
                    if (rollbacker) {
                        ...
                        if (maintain_rollback && p->is_lost_delete()
                            && p->can_rollback()) {
    ...
    bool append_new_log_entries(const hobject_t & last_backfill,
                                const mempool::osd_pglog::list <
                                pg_log_entry_t > &entries,
                                LogEntryHandler * rollbacker) {
        bool invalidate_stats = append_log_entries_update_missing(last_backfill,
                                                                  entries,
                                                                  true,
                                                                  &log,
                                                                  missing,
    ...
    static void write_log_and_missing(ObjectStore::Transaction & t,
                                      std::map < std::string,
                                      ceph::buffer::list > *km, pg_log_t & log,
                                      const coll_t & coll,
                                      const ghobject_t & log_oid,
    ...
    static void _write_log_and_missing_wo_missing(ObjectStore::Transaction & t,
                                                  std::map < std::string,
                                                  ceph::buffer::list > *km,
                                                  pg_log_t & log,
                                                  const coll_t & coll,
    ...
    static void _write_log_and_missing(ObjectStore::Transaction & t,
                                       std::map < std::string,
                                       ceph::buffer::list > *km, pg_log_t & log,
                                       const coll_t & coll,
                                       const ghobject_t & log_oid,
                                       ...
                                       std::set < std::string > &&trimmed_dups,
                                       const pg_missing_tracker_t & missing,
                                       bool touch_log, bool require_rollback,
                                       bool clear_divergent_priors,
                                       eversion_t dirty_to_dups,
                                       eversion_t dirty_from_dups,
                                       eversion_t write_from_dups,
    ...
    void read_log_and_missing(ObjectStore * store,
                              ObjectStore::CollectionHandle & ch,
        ...
        return read_log_and_missing(cct, store, ch, pgmeta_oid, info,
                                    log, missing, oss,
    ...
    template < typename missing_type >
        static void read_log_and_missing(CephContext * cct,
                                         ObjectStore * store,
                                         ObjectStore::CollectionHandle & ch,
                                         ghobject_t pgmeta_oid,
                                         const pg_info_t & info,
                                         ...
                                         *log_keys_debug =
                                         nullptr,
                                         bool debug_verify_stored_missing =
                                         false) {
        ldpp_dout(dpp,
                  10) << "read_log_and_missing coll " << ch->
            cid << " " << pgmeta_oid << dendl;
        size_t total_dups = 0;
        ...
        const auto NUM_DUPS_WARN_THRESHOLD =
            2 * cct->_conf->osd_pg_log_dups_tracked;
        if (p) {
            using ceph::decode;
            for (p->seek_to_first(); p->valid(); p->next()) {
                ...
                if (p->key() == "divergent_priors") {
                    ...
                    decode(item, bp);
                    ldpp_dout(dpp,
                              20) << "read_log_and_missing " << item << dendl;
                    if (item.is_delete()) {
        ...
        log = IndexedLog(info.last_update,
                         ...
                         std::move(entries), std::move(dups));
                ...
                for (auto i = log.log.rbegin(); i != log.log.rend(); ++i) {
                    ...
                    if (r >= 0) {
                        ...
                        if (oi.version < i->version) {
                            ldpp_dout(dpp,
                                      15) << "read_log_and_missing  missing " <<
                                *i << " (have " << oi.
                                version << ")" << " clean_regions " << i->
                                clean_regions << dendl;
                            ...
                            if (debug_verify_stored_missing) {
                                ...
                                // then created again
                                ceph_assert(miter->second.have == oi.version
                                            || miter->second.have ==
                                            eversion_t());
                  ...
                  for (auto && i:missing.get_items()) {
                        ...
                        if (i.first > info.last_backfill) {
                            ldpp_dout(dpp,
                                ...
                                "found before last_backfill: " << i.
                                first << " " << i.
                                second << " last_backfill = " << info.
                                last_backfill << dendl;
                            ceph_abort_msg
                                ("invalid missing std::set entry found");
                        }
                        ceph::buffer::list bv;
                        int r = store->getattr(ch,
                                               ghobject_t(i.first,
                        ...
                        if (r >= 0) {
                            object_info_t oi(bv);
                            ceph_assert(oi.version == i.second.have
                                        || eversion_t() == i.second.have);
                        }
                        else {
                            ceph_assert(i.second.is_delete()
                                        || eversion_t() == i.second.have);
                        }
                    }
                }
...
#ifdef WITH_SEASTAR
    seastar::future <> read_log_and_missing_crimson(crimson::os::
                                                    FuturizedStore::
                                                    Shard & store,
                                                    crimson::os::
                                                    CollectionRef ch,
                                                    const pg_info_t & info,
                                                    ghobject_t pgmeta_oid) {
        return read_log_and_missing_crimson(store, ch, info, log,
                                            (pg_log_debug ? &log_keys_debug :
                                             nullptr), missing, pgmeta_oid,
                                            this);
    }

    static seastar::future <> read_log_and_missing_crimson(crimson::os::
                                                           FuturizedStore::
                                                           Shard & store,
                                                           crimson::os::
                                                           CollectionRef ch,
