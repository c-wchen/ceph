...
void usage()
{
    ...
    cout << "   --with-default-pool     include default pool when creating map"
        << std::endl;
    cout << "   --clear-temp            clear pg_temp and primary_temp" << std::
        endl;
    ...
    cout <<
        ...
        << std::endl;
    cout << "   --tree                  displays a tree of the map" << std::
        endl;
    cout <<
...
void print_inc_upmaps(const OSDMap::Incremental & pending_inc, int fd,
                      bool vstart, std::string cmd = "ceph")
{
    ostringstream ss;
  ...
  for (auto & i:pending_inc.new_pg_upmap_primary) {
        if (vstart)
            ss << prefix;
        ss << cmd +
            " osd pg-upmap-primary " << i.first << " " << i.second << std::endl;
    }
    string s = ss.str();
...
int main(int argc, const char **argv)
{
    ...
    for (std::vector < const char *>::iterator i = args.begin();
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            upmap_cleanup = true;
                 ...
                 (args, i, &upmap_file, "--upmap", (char *)NULL)) {
            upmap_cleanup = true;
            upmap = true;
        }
        else if (ceph_argparse_witharg
                 (args, i, &upmap_file, "--read", (char *)NULL)) {
            read = true;
        }
        else if (ceph_argparse_witharg
            ...
            upmap_p_seed = &upmap_seed;
                 ...
                 (args, i, &val, "--upmap-pool", (char *)NULL)) {
            upmap_pools.insert(val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--read-pool", (char *)NULL)) {
            read_pool = val;
        }
        else if (ceph_argparse_witharg
            ...
            health = true;
                 ...
                 (args, i, "--with-default-pool", (char *)NULL)) {
            createpool = true;
        }
        else if (ceph_argparse_flag
                 (args, i, "--create-from-conf", (char *)NULL)) {
            ...
            mark_up_in = true;
                 ...
                 (args, i, &val, "--mark-out", (char *)NULL)) {
            marked_out = std::stoi(val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--mark-up", (char *)NULL)) {
            marked_up = std::stod(val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--mark-in", (char *)NULL)) {
            ...
            test_map_pgs = true;
                 ...
                 (args, i, "--test-map-pgs-dump", (char *)NULL)) {
            test_map_pgs_dump = true;
        }
        else if (ceph_argparse_flag
                 (args, i, "--test-map-pgs-dump-all", (char *)NULL)) {
            ...
            if (!err.str().empty()) {
                ...
                exit(EXIT_FAILURE);
                 ...
                 (args, i, &val, "--export_crush", (char *)NULL)) {
            export_crush = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--import_crush", (char *)NULL)) {
            import_crush = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--test_map_pg", (char *)NULL)) {
            test_map_pg = val;
        }
        else if (ceph_argparse_witharg
            ...
            test_crush = true;
                 ...
                 (args, i, &val, err, "--pg_num", (char *)NULL)) {
            string interr;
            ...
            if (interr.length() > 0) {
                ...
                exit(EXIT_FAILURE);
                 ...
                 (args, i, &pool, err, "--pool", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                exit(EXIT_FAILURE);
            }
    ...
    if (range_first >= 0 && range_last >= 0) {
        ...
        for (int i = range_first; i <= range_last; i++) {
            ...
            if (r < 0) {
                cerr << "unable to read " << s << ": " << cpp_strerror(r) <<
                    std::endl;
                exit(1);
            }
    ...
    if (!createsimple && !create_from_conf && !clobber) {
        ...
        if (r == 0) {
            try {
                ...
                cerr << me << ": error decoding osdmap '" << fn << "'" << std::
                    endl;
                return -1;
            }
        }
        else {
            cerr << me << ": couldn't open " << fn << ": " << error << std::
                endl;
            return -1;
        }
    }
    else if ((createsimple || create_from_conf) && !clobber
             &&::stat(fn.c_str(), &st) == 0) {
    ...
    if (createsimple || create_from_conf) {
        ...
        uuid_d fsid;
        if (createpool) {
            osdmap.build_simple_with_pool(g_ceph_context, 0, fsid, num_osd,
                                          pg_bits, pgp_bits);
        }
        else {
    ...
    for_each_substr(adjust_crush_weight, ",",[&](auto osd_to_adjust) {
                    ...
                    cerr << me <<
                    ": use ':' as separator of osd id and its weight" << std::
                    endl; usage();}
                    int osd_id =
                    std::stoi(string(osd_to_adjust.substr(0, pos)));
                    float new_weight =
                    std::stof(string(osd_to_adjust.substr(pos + 1)));
                    osdmap.crush->adjust_item_weightf(g_ceph_context, osd_id,
                                                      new_weight);
                    std::
                    cout << "Adjusted osd." << osd_id << " CRUSH weight to " <<
                    new_weight << std::endl; if (save) {
                    OSDMap::Incremental inc; inc.fsid = osdmap.get_fsid();
                    inc.epoch = osdmap.get_epoch() + 1;
                    osdmap.apply_incremental(inc); modified = true;}
                    }
    ) ;
    ...
    if (upmap || upmap_cleanup || read) {
        if (upmap_file != "-") {
            upmap_fd =::open(upmap_file.c_str(), O_CREAT | O_WRONLY | O_TRUNC,
                             0644);
            if (upmap_fd < 0) {
                cerr << "error opening " << upmap_file << ": " <<
                    cpp_strerror(errno)
                    << std::endl;
                exit(1);
            }
            cout << "writing upmap command output to: " << upmap_file << std::
                endl;
        }
    }
    if (upmap_cleanup) {
    ...
    if (read) {
        ...
        pgs_by_osd =
            tmp_osd_map.get_pgs_by_osd(g_ceph_context, pid, &prim_pgs_by_osd,
                                       &acting_prims_by_osd);
        ...
        OSDMap::Incremental pending_inc(osdmap.get_epoch() + 1);
        int num_changes =
            osdmap.balance_primaries(g_ceph_context, pid, &pending_inc,
                                     tmp_osd_map);

        if (num_changes < 0) {
            cerr <<
                "Error balancing primaries. Rerun with at least --debug-osd=10 for more details."
                << std::endl;
            exit(1);
        }
        ...
        pgs_by_osd_2 =
            tmp_osd_map.get_pgs_by_osd(g_ceph_context, pid, &prim_pgs_by_osd_2,
        ...
        if (num_changes > 0) {
          ...
          for (auto &[osd, pgs]:prim_pgs_by_osd) {
                cout << " osd." << osd << " | primary affinity: " <<
                    tmp_osd_map.
                    get_primary_affinityf(osd) << " | number of prims: " << pgs.
                    size() << "\n";
            }
            cout << " \n";
    ...
    if (upmap) {
        cout << "upmap, max-count " << upmap_max
            << ", max deviation " << upmap_deviation << std::endl;
      ...
      for (auto & s:upmap_pools) {
        ...
        if (!pools.empty()) {
            ...
            mempool::osdmap::map < int64_t, pg_pool_t > opools =
                osdmap.get_pools();
          for (auto & i:opools) {
        ...
        do {
            random_device_t rd;
            std::shuffle(pools.begin(), pools.end(), std::mt19937 {
                         rd()}
            );
            cout << "pools ";
          for (auto & i:pools)
          ...
          for (auto & i:pools) {
                ...
                int did = osdmap.calc_pg_upmaps(g_ceph_context, upmap_deviation,
                                                left, one_pool,
            ...
            cout << "prepared " << total_did << "/" << upmap_max << " changes"
                << std::endl;
            float elapsed_time =
                (end.tv_sec - begin.tv_sec) + 1.0e-9 * (end.tv_nsec -
                                                        begin.tv_nsec);
            if (upmap_active)
            ...
            if (total_did > 0) {
                ...
                if (upmap_active) {
                  ...
                  for (auto & i:osdmap.get_pools()) {
                        if (!upmap_pool_nums.empty()
                            && !upmap_pool_nums.count(i.first))
                            continue;
                        for (unsigned ps = 0; ps < i.second.get_pg_num(); ++ps) {
                            pg_t pg(ps, i.first);
                            vector < int >up;
                            osdmap.pg_to_up_acting_osds(pg, &up, nullptr,
                                                        nullptr, nullptr);
                  ...
                  for (auto & i:pgs_by_osd)
                        cout << "osd." << i.first << " pgs " << i.second.
                            size() << std::endl;
                    float elapsed_time =
                        (end.tv_sec - round_start.tv_sec) +
                        1.0e-9 * (end.tv_nsec - round_start.tv_nsec);
                    cout << "Total time elapsed " << elapsed_time << " secs, "
  ...
  skip_upmap:
    ...
    if (!import_crush.empty()) {
        ...
        cout << me << ": imported " << cbl.
            length() << " byte crush map from " << import_crush << std::endl;
        modified = true;
    }
    ...
    if (!test_map_object.empty()) {
        ...
        if (pool == -1) {
            cout << me << ": assuming pool 1 (use --pool to override)" << std::
                endl;
            pool = 1;
        }
    ...
    if (test_map_pgs || test_map_pgs_dump || test_map_pgs_dump_all) {
        ...
        double edev =
            sqrt((double)total / (double)in * (1.0 - (1.0 / (double)in)));
        cout << " in " << in << std::endl;
        cout << " avg " << avg
            << " stddev " << dev
            << " (" << (dev / avg) << "x)"
        ...
        if (min_osd >= 0)
            cout << " min osd." << min_osd << " " << count[min_osd] << std::
                endl;
        if (max_osd >= 0)
            cout << " max osd." << max_osd << " " << count[max_osd] << std::
                endl;
    ...
    if (test_crush) {
        ...
        while (1) {
            ...
            for (map < int64_t, pg_pool_t >::const_iterator p =
                 osdmap.get_pools().begin(); p != osdmap.get_pools().end();
                 ++p) {
