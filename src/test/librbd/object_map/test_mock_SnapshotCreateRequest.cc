...
public:
    void inject_snap_info(librbd::ImageCtx *ictx, uint64_t snap_id)
    {
        std::unique_lock image_locker {
            ictx->image_lock};
        ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "snap name",
                       snap_id, ictx->size, ictx->parent_md,
    ...
    } void expect_read_map(librbd::ImageCtx *ictx, int r)
    {
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        read(ObjectMap <>::
                             object_map_name(ictx->id, CEPH_NOSNAP), 0,
                             0, _, _, _)).WillOnce(Return(r));
        } else {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        read(ObjectMap <>::
                             object_map_name(ictx->id, CEPH_NOSNAP), 0,
                             0, _, _, _)).WillOnce(DoDefault());
        }
    }

    void expect_write_map(librbd::ImageCtx *ictx, uint64_t snap_id,
                          int r)
    {
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        write_full(ObjectMap <>::
    ...
    void expect_add_snapshot(librbd::ImageCtx *ictx, int r)
    {
        std::string oid(ObjectMap <>::
                        object_map_name(ictx->id, CEPH_NOSNAP));
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
            ...
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        exec(oid, _, StrEq("rbd"),
                             StrEq("object_map_snap_add"), _, _, _, _))
            .WillOnce(DoDefault());
        }
    }
...
TEST_F(TestMockObjectMapSnapshotCreateRequest, Success)
{
    ...
    AsyncRequest <> *request =
        new SnapshotCreateRequest(*ictx, &object_map_lock, &object_map,
                                  snap_id, &cond_ctx);
    {
