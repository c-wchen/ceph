...
TEST_F(TestJournalReplay, AioWriteEvent)
{
    ...
    payload_bl.append(payload);
    inject_into_journal(ictx,
                        librbd::journal::AioWriteEvent(0, payload.size(),
                                                       payload_bl));
    close_image(ictx);
    ...
    // verify lock ordering constraints
    aio_comp = new librbd::io::AioCompletion();
    ictx->io_work_queue->aio_write(aio_comp, 0, payload.size(), bufferlist {
                                   payload_bl}
                                   , 0);
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    aio_comp->release();
...
TEST_F(TestJournalReplay, SnapUnprotect)
{
    ...
    ASSERT_EQ(0,
              ictx->operations->
              snap_unprotect(cls::rbd::UserSnapshotNamespace(), "snap2"));
}
...
TEST_F(TestJournalReplay, SnapRename)
{
    ...
    // inject snapshot ops into journal
    inject_into_journal(ictx,
                        librbd::journal::SnapRenameEvent(1, snap_id, "snap",
                                                         "snap2"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
...
TEST_F(TestJournalReplay, MetadataRemove)
{
    ...
    ASSERT_EQ(0,
              ictx->operations->metadata_set("conf_rbd_mirroring_replay_delay",
                                             "9876"));
