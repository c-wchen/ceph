...
class ACLGrant {
  ...
  public:
    ...
    const ACLPermission & get_permission() const {
        ...
        return group;
    } const string & get_referer() const {
        return url_spec;
    } void encode(bufferlist & bl) const {
        ENCODE_START(5, 3, bl);
        ::encode(type, bl);
    ...
    void set_canon(const rgw_user & _id, const string & _name,
                   const uint32_t perm) {
...
const multimap < string, ACLGrant > &get_grant_map() const {
    return grant_map;
...
uint32_t get_perm(const rgw::auth::Identity & auth_identity,
                  uint32_t perm_mask, const char *http_referer);
uint32_t get_group_perm(ACLGroupTypeEnum group, uint32_t perm_mask);
bool verify_permission(const rgw::auth::Identity & auth_identity,
                       uint32_t user_perm_mask,
                       uint32_t perm, const char *http_referer = nullptr);
