...
struct ConstructVisitor: public boost::static_visitor < Action::ptr > {
    ...
    } inline Action::ptr operator()(const action::
                                    StopThreadAction &action)const
    {
    ...
    } inline Action::ptr operator()(const action::OpenImageAction &action)const
    {
        return Action::ptr(new OpenImageAction(action));
                                    ...
                                    CloseImageAction &action)const
    {
        return Action::ptr(new CloseImageAction(action));
    } inline Action::ptr operator()(const action::
                                    AioOpenImageAction &action)const
    {
        return Action::ptr(new AioOpenImageAction(action));
    } inline Action::ptr operator()(const action::
                                    AioCloseImageAction &action)const
    {
...
void AioReadAction::perform(ActionCtx &worker)
{
    ...
    int r =
        image->aio_read(m_action.offset, m_action.length, io->bufferlist(),
                        &io->completion());
...
void AioWriteAction::perform(ActionCtx &worker)
{
    ...
    if (worker.readonly()) {
        ...
        int r =
            image->aio_write(m_action.offset, m_action.length, io->bufferlist(),
                             &io->completion());
        assertf(r >= 0, "id = %d, r = %d", id(), r);
    }
}
...
void WriteAction::perform(ActionCtx &worker)
{
    ...
    if (!worker.readonly()) {
        ssize_t r =
            image->write(m_action.offset, m_action.length, io->bufferlist());
        assertf(r >= 0, "id = %d, r = %d", id(), r);
    }
    worker.remove_pending(io);
}
...
void AioDiscardAction::perform(ActionCtx &worker)
{
    ...
    if (worker.readonly()) {
        ...
        int r =
            image->aio_discard(m_action.offset, m_action.length,
                               &io->completion());
        assertf(r >= 0, "id = %d, r = %d", id(), r);
    }
}
...
void OpenImageAction::perform(ActionCtx &worker)
{
    ...
    if (m_action.read_only || worker.readonly()) {
        r = rbd->open_read_only(*worker.ioctx(), *image, name.image.c_str(),
                                name.snap.c_str());
    } else {
        r = rbd->open(*worker.ioctx(), *image, name.image.c_str(),
                      name.snap.c_str());
    }
    if (r) {
