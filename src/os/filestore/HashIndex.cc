...
int HashIndex::col_split_level(HashIndex & from,
                               HashIndex & to,
    ...
    for (vector < string >::iterator i = subdirs.begin();
        ...
        if (bits < inbits) {
            if (hobject_t::match_hash(hash, bits, match)) {
                r = col_split_level(from,
                                    to, sub_path, inbits, match, mkdirred);
                if (r < 0)
                    return r;
...
int HashIndex::split_dirs(const vector < string > &path, int target_level)
{
    ...
    if (must_split(info, target_level)) {
        ...
        dout(1) << __func__ << " " << path << " split completed in pg " <<
            coll() << "." << dendl;
        if (r < 0) {
...
int HashIndex::read_settings()
{
    ...
    if (r == -ENODATA)
        return 0;
    if (r < 0) {
        derr << __func__ << " error reading settings: " << cpp_strerror(r) <<
            dendl;
        return r;
    }
    bufferlist::iterator it = bl.begin();
    settings.decode(it);
    dout(20) << __func__ << " split_rand_factor = " << settings.
        split_rand_factor << dendl;
    return 0;
}
...
int HashIndex::set_info(const vector < string > &path,
                        const subdir_info_s & info)
{
    bufferlist buf;
    assert(path.size() == (unsigned)info.hash_level);
    info.encode(buf);
...
bool HashIndex::must_split(const subdir_info_s & info, int target_level)
{
    ...
    return (info.hash_level < (unsigned)MAX_HASH_LEVEL &&
            ((target_level > 0 && info.hash_level < (unsigned)target_level) ||
             (info.objs >
              ((unsigned)
               (abs(merge_threshold) * split_multiplier +
                settings.split_rand_factor) * 16))));
}
...
int HashIndex::complete_split(const vector < string > &path, subdir_info_s info)
{
    ...
    for (map < string, map < string, ghobject_t > >::iterator i =
         mapped.begin(); i != mapped.end();) {
        dst[level] = i->first;
...
int HashIndex::get_path_contents_by_hash_bitwise(const vector < string > &path,
                                                 ...
                                                 CmpPairBitwise > *objects)
{
    ...
    for (vector < string >::iterator i = subdirs.begin(); i != subdirs.end();
         ++i) {
                                    ...
                                    vector < ghobject_t > *out)
{
    ...
    for (set < string, CmpHexdigitStringBitwise >::iterator i =
         hash_prefixes.begin(); i != hash_prefixes.end(); ++i) {
        ...
        if (j == objects.end() || j->first != *i) {
            *(next_path.rbegin()) = *(i->rbegin());
            ...
            r = list_by_hash_bitwise(next_path,
                                     end, max_count, &next_recurse, out);
            ...
            while (j != objects.end() && j->first == *i) {
                ...
                if (!next || j->second >= *next) {
                    dout(20) << __func__ << " prefix " << *i << " ob " << j->
                        second << dendl;
