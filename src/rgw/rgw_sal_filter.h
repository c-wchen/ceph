...
public:
    ...
    {
        return next->set_count(_count);
    }
    virtual void set_size(uint64_t _size) override
    {
        return next->set_size(_size);
    }
    virtual bool empty() const override
    {
        return next->empty();
...
public:
    ...
    {
        return next->set_obj_state(_state);
    }
...
public:
    ...
    virtual std::map<uint32_t, std::unique_ptr<MultipartPart>> &get_parts() override
    {
        return parts;
    }
    ...
    virtual int init(const DoutPrefixProvider *dpp, optional_yield y, ACLOwner &owner, rgw_placement_rule &dest_placement,
