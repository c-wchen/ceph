                       ...
                       std::pair < std::string, ceph::buffer::list > *next)
{
    ...
    return interruptor::green_get(os->omap_get_values(ch, hoid, key).
                                  safe_then_unpack([&key, next] (bool,
                                                                 FuturizedStore::
                                                                 Shard::
                                                                 omap_values_t
                                                                 && vals) {
...
int OSDriver::get_next_or_current(const std::string & key,
                                  std::pair < std::string,
                                  ceph::buffer::list > *next_or_current)
{
    CRIMSON_DEBUG("OSDriver::{}:{}", __func__, __LINE__);
    using crimson::os::FuturizedStore;
    // let's try to get current first
    return interruptor::green_get(os->
                                  omap_get_values(ch, hoid,
                                                  FuturizedStore::Shard::
                                                  omap_keys_t {
                                                  key}
                                  ).
                                  safe_then([&key,
                                             next_or_current] (FuturizedStore::
                                                               Shard::
                                                               omap_values_t
                                                               && vals) {
                                            assert(vals.size() == 1);
                                            *next_or_current =
                                            ...
                                            FuturizedStore::Shard::
                                            read_errorator::
                                            all_same_way([next_or_current, &key,
                                                          this] {
                                                         // no current, try next
                                                         return get_next(key,
                                                                         next_or_current);}
...
string SnapMapper::to_raw_key(const pair < snapid_t, hobject_t > &in) constconst
{
    return get_prefix(in.second.pool,
                      in.first) + shard_prefix + in.second.to_str();
}

std::string SnapMapper::to_raw_key(snapid_t snap, const hobject_t & clone) constconst
{
    return get_prefix(clone.pool, snap) + shard_prefix + clone.to_str();
}

pair < string, ceph::buffer::list > SnapMapper::to_raw(const pair < snapid_t,
                                                       hobject_t >
                                                       &in) constconst
{
    ceph::buffer::list bl;
...
pair < snapid_t, hobject_t > SnapMapper::from_raw(const pair < std::string,
...
std::pair < snapid_t,
    hobject_t > SnapMapper::from_raw(const ceph::buffer::list & image)
{
    using ceph::decode;
    Mapping map;
...
string SnapMapper::to_object_key(const hobject_t & hoid) constconst
{
    return OBJECT_PREFIX + shard_prefix + hoid.to_str();
}

void SnapMapper::object_snaps::encode(ceph::buffer::list & bl) const const
{
    ENCODE_START(1, 1, bl);
    encode(oid, bl);
    encode(snaps, bl);
...
bool SnapMapper::check(const hobject_t & hoid) const const
{
    if (hoid.match(mask_bits, match)) {
        return true;
    }
    derr << __func__ << " " << hoid << " mask_bits " << mask_bits
        << " match 0x" << std::hex << match << std::dec << " is false" << dendl;
    return false;
}

int SnapMapper::get_snaps(const hobject_t & oid, object_snaps * out) const const
{
...
tl::expected < std::set < snapid_t >, Scrub::SnapMapReaderI::result_t >
    SnapMapper::get_snaps(const hobject_t & oid) const const
{
    auto snaps = get_snaps_common(oid);
    if (snaps) {
        return snaps->snaps;
...
tl::expected < SnapMapper::object_snaps, Scrub::SnapMapReaderI::result_t >
    SnapMapper::get_snaps_common(const hobject_t & oid) constconst
{
    ...
    if (r < 0) {
        ...
        return tl::unexpected(result_t {
                              code_t::backend_error, r}
        );
    }
    if (got.empty()) {
        dout(10) << __func__ << " " << oid << " got.empty()" << dendl;
        return tl::unexpected(result_t {
                              code_t::not_found, -ENOENT}
        );
    }
...
std::set < std::string > SnapMapper::to_raw_keys(const hobject_t & clone,
                                                 ...
                                                 &snaps) constconst
{
  ...
  for (auto snap:snaps) {
    ...
    dout(20) << fmt::format("{}: clone:{} snaps:{} -> keys: {}", __func__,
                            clone, snaps, keys)
        << dendl;
    return keys;
}
...
tl::expected < std::set < snapid_t >, result_t >
    SnapMapper::get_snaps_check_consistency(const hobject_t & hoid) constconst
{
    ...
    if (r < 0) {
        dout(10) << fmt::format("{}: backend error ({}) for cobject {}",
                                __func__, r, hoid)
            << dendl;
  ...
  for (auto &[k, v]:kvmap) {
        ...
        if (obj != hoid) {
            dout(1) << fmt::
                format("{}: unexpected object ID {} for key{} (expected {})",
                       __func__, obj, k, hoid)
                << dendl;
            return tl::unexpected(result_t {
                                  code_t::inconsistent}
    ...
    if (snaps_from_mapping != *obj_snaps) {
        dout(10) << fmt::
            format("{}: hoid:{} -> mapper internal inconsistency ({} vs {})",
    ...
    dout(10) << fmt::format("{}: snaps for {}: {}", __func__, hoid,
                            snaps_from_mapping)
        << dendl;
    return obj_snaps;
}

void SnapMapper::clear_snaps(const hobject_t & oid,
...
void SnapMapper::set_snaps(const hobject_t & oid,
...
int SnapMapper::update_snaps(const hobject_t & oid,
...
void SnapMapper::add_oid(const hobject_t & oid,
...
int SnapMapper::get_next_objects_to_trim(snapid_t snap,
                                         unsigned max,
...
int SnapMapper::get_snaps(const hobject_t & oid, std::set < snapid_t > *snaps) const const
{
    ceph_assert(check(oid));
    object_snaps out;
    int r = get_snaps(oid, &out);
...
int SnapMapper::_lookup_purged_snap(CephContext * cct,
                                    OSDriver & backend,
...
void SnapMapper::record_purged_snaps(CephContext * cct,
                                     OSDriver & backend,
      ...
      for (auto &[pool, snaps]:bypool) {
            for (auto i = snaps.begin(); i != snaps.end(); ++i) {
                ...
                int b = _lookup_purged_snap(cct, backend,
                                            pool, begin - 1, &before_begin,
                                            &before_end);
                int a = _lookup_purged_snap(cct, backend,
                                            pool, end, &after_begin,
                                            &after_end);
                if (!b && !a) {
                    dout(10) << __func__
                        << " [" << begin << "," << end << ") - joins ["
                    ...
                    make_purged_snap_key_value(pool, before_begin, after_end,
                                               &m);
                }
                else if (!b) {
                    dout(10) << __func__
...
bool SnapMapper::Scrubber::_parse_m()
{
    ...
    dout(20) << __func__ << " mapping pool " << mapping.hoid.pool
        << " snap " << mapping.snap
        << " shard " << shard << " " << mapping.hoid << dendl;
    mapit->next();
    return true;
}
...
void SnapMapper::Scrubber::run()
{
    ...
    while (_parse_m()) {
        ...
        if (pool < 0) {
            dout(10) << __func__ <<
                " passed final purged_snaps interval, rest ok" << dendl;
            break;
        }
        if (mapping.hoid.pool < pool || mapping.snap < begin) {
            ...
            assert(mapping.snap >= begin &&
                   mapping.snap < end && mapping.hoid.pool == pool);
            // invalid
            dout(10) << __func__ << " stray " << mapping.hoid
                ...
                << " shard " << shard
                << " purged_snaps [" << begin << "," << end << ")" << dendl;
            stray.emplace_back(std::tuple < int64_t, snapid_t, uint32_t,
                               shard_id_t > (pool, mapping.snap,
 ...
 */
std::string SnapMapper::convert_legacy_key(const std::string & old_key,
                                           const ceph::buffer::list & value)
{
    auto old = from_raw(make_pair(old_key, value));
    std::string object_suffix =
        old_key.substr(SnapMapper::LEGACY_MAPPING_PREFIX.length());
    return SnapMapper::MAPPING_PREFIX + std::to_string(old.second.pool)
        + "_" + object_suffix;
}

int SnapMapper::convert_legacy(CephContext * cct,
                               ObjectStore * store,
                               ...
                               ghobject_t hoid, unsigned max)
{
    ...
    while (iter->valid()) {
        ...
        if (valid) {
            to_set.emplace(convert_legacy_key(iter->key(), iter->value()),
                           iter->value());
            ++n;
            iter->next();
        }
    ...
    // remove the old keys
    {
        ...
        t.omap_rmkeyrange(ch->cid, hoid,
                          SnapMapper::LEGACY_MAPPING_PREFIX, end);
        int r = store->queue_transaction(ch, std::move(t));
        ceph_assert(r == 0);
    }
    return 0;
