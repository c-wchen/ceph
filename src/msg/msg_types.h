...
class entity_name_t {
  ...
  public:
    ...
    explicit entity_name_t(const ceph_entity_name & n):_type(n.type),
        _num(n.num) {
    }
    ...
    static entity_name_t MGR(int64_t i = NEW) {
...
inline std::ostream & operator<<(std::ostream & out,
                                 const ceph_entity_name & addr)
{
    return out << entity_name_t {
    addr.type, static_cast < int64_t > (addr.num)};
}
...
// define a wire format for sockaddr that matches Linux's. struct ceph_sockaddr_storage {
    ceph_le16 ss_family;
...
struct entity_addr_t {
  ...
  entity_addr_t(__u32 _type, __u32 _nonce):type(_type), nonce(_nonce) {
    ...
    // this isn't a guarantee; some client addrs will match it bool maybe_cidr() const {
    ...
    operator  ceph_entity_addr() const {
        ...
        case AF_INET:
            ...
            case AF_INET6:return memcmp(&u.sin6.sin6_addr, &in6addr_any,
    ...
    bool parse(const char *s, const char **end = 0, int default_type =
               TYPE_DEFAULT);

    void decode_legacy_addr_after_marker(ceph::buffer::list::
                                         const_iterator & bl) {
        using ceph::decode;
        __u8 marker;
        __u16 rest;
    ...
    void decode(ceph::buffer::list::const_iterator & bl) {
        ...
        if (elen) {
            ...
            if (elen < sizeof(ss_family)) {
                throw ceph::buffer::
                    malformed_input("elen smaller than family len");
            }
            decode(ss_family, bl);
#if defined(_WIN32)
            ...
            if (elen > get_sockaddr_len() - sizeof(u.sa.sa_family)) {
                throw ceph::buffer::
                    malformed_input("elen exceeds sockaddr len");
            }
...
namespace std {
...
} // namespace std struct entity_addrvec_t {
    std::vector < entity_addr_t > v;

    entity_addrvec_t() {
    } explicit entity_addrvec_t(const entity_addr_t & a):v( {
                                                           a}) {
    }
    ...
    entity_addr_t pick_addr(uint32_t type) const {
        ...
        case entity_addr_t::TYPE_LEGACY:
            [[fallthrough]];
            case entity_addr_t::TYPE_MSGR2:picked_addr = addr_of_type(type);
            break;
    ...
    bool legacy_equals(const entity_addrvec_t & o) const {
        if (v == o.v) {
            return true;
              ...
              front().is_legacy() && front() == o.legacy_addr()) {
            return true;
        }
        if (o.v.size() == 1 &&
            o.front().is_legacy() && o.front() == legacy_addr()) {
            return true;
        }
        return false;
    }
    ...
    friend bool operator==(const entity_addrvec_t & l,
                           const entity_addrvec_t & r) {
        return l.v == r.v;
    }
    friend bool operator!=(const entity_addrvec_t & l,
                           const entity_addrvec_t & r) {
...
struct entity_inst_t {
    ...
    entity_inst_t(const ceph_entity_name & n,
                  const ceph_entity_addr & a):name(n), addr(a) {
    }
