...
void ECSubWrite::encode(bufferlist & bl) const const
{
    ENCODE_START(4, 1, bl);
    ::encode(from, bl);
    ::encode(tid, bl);
...
void ECSubWrite::dump(Formatter * f) const const
{
...
void ECSubWriteReply::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(from, bl);
    ::encode(tid, bl);
...
void ECSubWriteReply::dump(Formatter * f) const const
{
    f->dump_unsigned("tid", tid);
...
void ECSubRead::encode(bufferlist & bl, uint64_t features) const const
{
    if ((features & CEPH_FEATURE_OSD_FADVISE_FLAGS) == 0) {
        ...
        for (map < hobject_t, list < boost::tuple < uint64_t, uint64_t,
             uint32_t > >>::const_iterator m = to_read.begin();
             m != to_read.end(); ++m) {
            list < pair < uint64_t, uint64_t > >tlist;
            for (list < boost::tuple < uint64_t, uint64_t,
                 uint32_t > >::const_iterator l = m->second.begin();
                 l != m->second.end(); ++l) {
                tlist.
                    push_back(std::make_pair(l->get < 0 > (), l->get < 1 > ()));
            }
            tmp[m->first] = tlist;
        }
        ::encode(tmp, bl);
...
void ECSubRead::decode(bufferlist::iterator & bl)
{
    ...
    if (struct_v == 1) {
        map < hobject_t, list < pair < uint64_t, uint64_t > >>tmp;
        ::decode(tmp, bl);
        for (map < hobject_t, list < pair < uint64_t,
             uint64_t > >>::const_iterator m = tmp.begin(); m != tmp.end();
             ++m) {
            list < boost::tuple < uint64_t, uint64_t, uint32_t > >tlist;
            for (list < pair < uint64_t, uint64_t > >::const_iterator l =
                 m->second.begin(); l != m->second.end(); ++l) {
...
void ECSubRead::dump(Formatter * f) const const
{
    f->dump_stream("from") << from;
    f->dump_unsigned("tid", tid);
    f->open_array_section("objects");
    for (map < hobject_t, list < boost::tuple < uint64_t, uint64_t,
         uint32_t > >>::const_iterator i = to_read.begin(); i != to_read.end();
         ++i) {
        f->open_object_section("object");
        f->dump_stream("oid") << i->first;
        f->open_array_section("extents");
        for (list < boost::tuple < uint64_t, uint64_t,
             uint32_t > >::const_iterator j = i->second.begin();
             j != i->second.end(); ++j) {
...
void ECSubReadReply::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(from, bl);
    ::encode(tid, bl);
...
void ECSubReadReply::dump(Formatter * f) const const
{
    f->dump_stream("from") << from;
    f->dump_unsigned("tid", tid);
    f->open_array_section("buffers_read");
    for (map < hobject_t, list < pair < uint64_t,
         bufferlist > >>::const_iterator i = buffers_read.begin();
         i != buffers_read.end(); ++i) {
        f->open_object_section("object");
        f->dump_stream("oid") << i->first;
    ...
    for (map < hobject_t, map < string, bufferlist >>::const_iterator i =
         attrs_read.begin(); i != attrs_read.end(); ++i) {
