    ...
    namespace image {
        ...
        template < typename I > void OpenRequest < I >::send_v1_detect_header() {
            ...
            librados::AioCompletion * comp =
                ...
                &klass::handle_v1_detect_header > (this);
            m_out_bl.clear();
            m_image_ctx->md_ctx.
                aio_operate(util::old_header_name(m_image_ctx->name), comp, &op,
                            &m_out_bl);
            comp->release();
        }
        ...
        template < typename I >
            ...
            if (*result < 0) {
                if (*result != -ENOENT) {
                    lderr(cct) << "failed to stat image header: " <<
                        cpp_strerror(*result)
                        << dendl;
                }
                ...
                m_image_ctx->header_oid =
                    util::old_header_name(m_image_ctx->name);
        ...
        template < typename I > void OpenRequest < I >::send_v2_detect_header() {
            if (m_image_ctx->id.empty()) {
                ...
                librados::AioCompletion * comp =
                    create_rados_callback < klass,
                    &klass::handle_v2_detect_header > (this);
                m_out_bl.clear();
                m_image_ctx->md_ctx.
                    aio_operate(util::id_obj_name(m_image_ctx->name), comp, &op,
                                &m_out_bl);
                comp->release();
            }
            else {
        ...
        template < typename I >
            ...
            if (*result == -ENOENT) {
                ...
                lderr(cct) << "failed to stat v2 image header: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
            }
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to retrieve image id: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
            }
        ...
        template < typename I > void OpenRequest < I >::send_v2_get_name() {
            ...
            m_out_bl.clear();
            m_image_ctx->md_ctx.aio_operate(RBD_DIRECTORY, comp, &op,
                                            &m_out_bl);
            comp->release();
        }
        ...
        template < typename I >
            Context * OpenRequest <
            I >::handle_v2_get_name_from_trash(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            Context * OpenRequest <
            I >::handle_v2_get_initial_metadata(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
            ...
            if (*result >= 0) {
                uint64_t size;
                *result =
                    cls_client::get_size_finish(&it, &size,
                                                &m_image_ctx->order);
            }
            ...
            if (*result >= 0) {
                ...
                *result =
                    cls_client::get_features_finish(&it, &m_image_ctx->features,
        ...
        template < typename I >
            Context * OpenRequest <
            I >::handle_v2_get_stripe_unit_count(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
            ...
            if (*result == 0) {
                auto it = m_out_bl.cbegin();
                *result =
                    cls_client::get_stripe_unit_count_finish(&it,
                                                             &m_image_ctx->
                                                             stripe_unit,
                                                             &m_image_ctx->
                                                             stripe_count);
            ...
            if (*result < 0) {
                lderr(cct) << "failed to read striping metadata: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
                return nullptr;
        ...
        template < typename I >
            Context * OpenRequest <
            I >::handle_v2_get_create_timestamp(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
        ...
        template < typename I >
            Context * OpenRequest <
            I >::handle_v2_get_access_modify_timestamp(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to read data pool: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
                return nullptr;
            ...
            if (data_pool_id != -1) {
                *result =
                    util::create_ioctx(m_image_ctx->md_ctx, "data pool",
                                       data_pool_id, {
                                       }
        ...
        template < typename I > void OpenRequest < I >::send_refresh() {
            ...
            m_image_ctx->config_watcher =
                ConfigWatcher < I >::create(*m_image_ctx);
            m_image_ctx->config_watcher->init();

            using klass = OpenRequest < I >;
            RefreshRequest < I > *req =
                RefreshRequest < I >::create(*m_image_ctx, false,
                                             m_skip_open_parent_image,
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to refresh image: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
                return nullptr;
        ...
        template < typename I >
            ...
            auto plugins =
                m_image_ctx->config.template get_val < std::string >
                ("rbd_plugins");
            ldout(cct, 10) << __func__ << ": plugins=" << plugins << dendl;

            auto ctx = create_context_callback <
                OpenRequest < I >,
                &OpenRequest < I >::handle_init_plugin_registry > (this);
            m_image_ctx->plugin_registry->init(plugins, ctx);
        }

        template < typename I >
            Context * OpenRequest <
            I >::handle_init_plugin_registry(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            ...
            if (cache_policy == "writearound") {
                auto cache =
                    cache::WriteAroundObjectDispatch < I >::create(m_image_ctx,
                                                                   max_dirty,
            ...
            else if (cache_policy == "writethrough"
                ...
                auto cache =
                    cache::ObjectCacherObjectDispatch < I >::create(m_image_ctx,
                                                                    max_dirty,
                                                                    writethrough_until_flush);
                cache->init();

                // readahead requires the object cacher cache
                m_image_ctx->readahead.set_trigger_requests(m_image_ctx->config.
                                                            template get_val <
                                                            uint64_t >
                                                            ("rbd_readahead_trigger_requests"));
                m_image_ctx->readahead.set_max_readahead_size(m_image_ctx->
                                                              config.
                                                              template get_val <
                                                              Option::size_t >
        ...
        template < typename I >
            ...
            if ((m_image_ctx->read_only_flags & IMAGE_READ_ONLY_FLAG_USER) !=
                0U) {
        ...
        template < typename I >
            ...
            if (*result == -EPERM) {
                ...
                lderr(cct) << "failed to register watch: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
                return nullptr;
        ...
        template < typename I >
            ...
            if (snap_id == CEPH_NOSNAP) {
                lderr(cct) << "failed to find snapshot " << m_image_ctx->
                    snap_name << dendl;
            ...
            SetSnapRequest < I > *req =
                SetSnapRequest < I >::create(*m_image_ctx, snap_id,
                                             create_context_callback < klass,
                                             &klass::handle_set_snap > (this));
            req->send();
            return nullptr;
        }
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to set image snapshot: " <<
                    cpp_strerror(*result)
                    << dendl;
                send_close_image(*result);
                return nullptr;
        ...
        template < typename I > Context * OpenRequest < I >::finalize(int r) {
            if (r == 0) {
                ...
                if (io_scheduler_cfg == "simple" && !m_image_ctx->read_only) {
                    auto io_scheduler =
                        io::SimpleSchedulerObjectDispatch <
                        I >::create(m_image_ctx);
                    io_scheduler->init();
                }
            }
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to close image: " << cpp_strerror(*result)
                    << dendl;
            }
            if (m_error_result < 0) {
