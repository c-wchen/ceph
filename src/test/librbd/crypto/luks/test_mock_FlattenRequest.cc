        ...
        namespace luks {
            ...
            struct TestMockCryptoLuksFlattenRequest:public TestMockFixture {
                ...
                void SetUp() override {
                    ...
                    mock_image_ctx->encryption_format.
                        reset(mock_encryption_format);
                    mock_flatten_request =
                        MockFlattenRequest::create(mock_image_ctx, on_finish);
                } void TearDown() override {
                    delete mock_image_ctx;
                     TestMockFixture::TearDown();
                } void generate_header(const char *type, const char *alg,
                    ...
                    Header header(mock_image_ctx->cct);
                     ...
                     ASSERT_EQ(0,
                               header.format(type, alg, nullptr, key_size,
                                             cipher_mode, sector_size,
                                             OBJECT_SIZE, true));
                     ASSERT_EQ(0,
                               header.add_keyslot(passphrase_cstr,
                                                  strlen(passphrase_cstr)));
                     ASSERT_LT(0, header.read(&header_bl));
                    if (magic_switched) {
                        ASSERT_EQ(0,
                                  Magic::replace_magic(mock_image_ctx->cct,
                                                       header_bl));
                }} void expect_get_crypto() {
                    EXPECT_CALL(*mock_encryption_format,
                                get_crypto()).WillOnce(Return(&mock_crypto));
                }
                ...
                void expect_image_read(uint64_t offset, uint64_t length) {
                    EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, send(_))
                        .WillOnce(Invoke([this, offset,
                                         ...
                                         ASSERT_EQ(1,
                                                   spec->image_extents.size());
                                         ASSERT_EQ(offset,
                                                   spec->image_extents[0].
                                                   first);
                                         ASSERT_EQ(length,
                                                   spec->image_extents[0].
                                                   second);
                                         ...
                                         aio_comp->read_result.
                                         set_image_extents(spec->image_extents);
                                         auto ctx =
                                         new io::ReadResult::
                                         ...
                                         if (header_bl.length() <
                                             offset + length) {
                                         header_bl.append_zero(offset + length -
                                                               header_bl.
                                                               length());}
                                         ctx->bl.substr_of(header_bl, offset,
                                                           length);
                    ...
                    EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, send(_))
                        .WillOnce(Invoke([this] (io::ImageDispatchSpec * spec) {
                                         ...
                                         ASSERT_EQ(0,
                                                   spec->image_extents[0].
                                                   first);
                                         ASSERT_GT(spec->image_extents[0].
                                                   second, 0);
                                         spec->dispatch_result =
                                         io::DISPATCH_RESULT_COMPLETE;
