...
class NetworkWorkerTest:public::testing::TestWithParam < const char *> {
  public:
    ...
    NetworkWorkerTest() {
        ...
        if (strncmp(GetParam(), "dpdk", 4)) {
            ...
            g_ceph_context->_conf->set_val("ms_dpdk_debug_allow_loopback",
                                           "true", false);
            g_ceph_context->_conf->set_val("ms_async_op_threads", "2", false);
            g_ceph_context->_conf->set_val("ms_dpdk_coremask", "0x7", false);
            g_ceph_context->_conf->set_val("ms_dpdk_host_ipv4_addr",
                                           "172.16.218.3", false);
            g_ceph_context->_conf->set_val("ms_dpdk_gateway_ipv4_addr",
                                           "172.16.218.2", false);
            g_ceph_context->_conf->set_val("ms_dpdk_netmask_ipv4_addr",
                                           "255.255.255.0", false);
  ...
  template < typename func > class C_dispatch:public EventCallback {
      ...
      public:
      C_dispatch(Worker * w, func && _f):worker(w), f(std::move(_f)),
            done(false) {
        }
...
TEST_P(NetworkWorkerTest, SimpleTest)
{
    ...
    exec_events([this, accepted_p, bind_addr] (Worker * worker) mutable {
                ...
                ASSERT_EQ(0, r);
                ConnectedSocket cli_socket, srv_socket; if (worker->id == 0) {
                r = worker->connect(bind_addr, options, &cli_socket);
                ASSERT_EQ(0, r);}

                bool is_my_accept = false; if (bind_socket) {
                C_poll cb(center);
                center->create_file_event(bind_socket.fd(), EVENT_READABLE,
                                          &cb); if (cb.poll(500)) {
                *accepted_p = true; is_my_accept = true;}
                ...
                ASSERT_EQ(0, r); ASSERT_TRUE(srv_socket.fd() > 0);}

                if (worker->id == 0) {
                C_poll cb(center);
                center->create_file_event(cli_socket.fd(), EVENT_READABLE, &cb);
                r = cli_socket.is_connected(); if (r == 0) {
                ASSERT_EQ(true, cb.poll(500)); r = cli_socket.is_connected();}
                ASSERT_EQ(1, r);
                center->delete_file_event(cli_socket.fd(), EVENT_READABLE);}

                const char *message = "this is a new message";
                int len = strlen(message);
                bufferlist bl; bl.append(message, len); if (worker->id == 0) {
                r = cli_socket.send(bl, false); ASSERT_EQ(len, r);}

                char buf[1024]; C_poll cb(center); if (is_my_accept) {
                center->create_file_event(srv_socket.fd(), EVENT_READABLE, &cb); {
                r = srv_socket.read(buf, sizeof(buf)); while (r == -EAGAIN) {
                ASSERT_TRUE(cb.poll(500));
                r = srv_socket.read(buf, sizeof(buf)); cb.reset();}
                ASSERT_EQ(len, r); ASSERT_EQ(0, memcmp(buf, message, len));}
                ...
                bl.clear(); bl.append(message, len); if (worker->id == 0) {
                r = cli_socket.send(bl, false); ASSERT_EQ(-EPIPE, r);}
                if (is_my_accept) {
                cb.reset();
                ASSERT_TRUE(cb.poll(500));
                r = srv_socket.read(buf, sizeof(buf)); if (r == -EAGAIN) {
                cb.reset();
...
TEST_P(NetworkWorkerTest, ConnectFailedTest)
{
    ...
    exec_events([this, bind_addr] (Worker * worker) mutable {
                ...
                r = worker->listen(bind_addr, options, &bind_socket);
                ASSERT_EQ(0, r);
                ConnectedSocket cli_socket1, cli_socket2; if (worker->id == 0) {
                ASSERT_TRUE(cli_addr.parse(get_ip_different_port().c_str()));
                r = worker->connect(cli_addr, options, &cli_socket1);
                ASSERT_EQ(0, r);
                C_poll cb(center);
                center->create_file_event(cli_socket1.fd(), EVENT_READABLE,
                                          &cb); r = cli_socket1.is_connected();
                if (r == 0) {
                ASSERT_TRUE(cb.poll(500)); r = cli_socket1.is_connected();}
                ...
                ASSERT_EQ(0, r);
                C_poll cb(center);
                center->create_file_event(cli_socket2.fd(), EVENT_READABLE,
                                          &cb); r = cli_socket2.is_connected();
                if (r == 0) {
                cb.poll(500); r = cli_socket2.is_connected();}
                ASSERT_TRUE(r != 1);
...
TEST_P(NetworkWorkerTest, AcceptAndCloseTest)
{
    ...
    exec_events([this, bind_addr, accepted_p, count_p] (Worker * worker) mutable {
                SocketOptions options;
                EventCenter * center = &worker->center;
                entity_addr_t cli_addr; int r = 0; {
                ServerSocket bind_socket;
                if (stack->support_local_listen_table() || worker->id == 0)
                r = worker->listen(bind_addr, options, &bind_socket);
                ASSERT_EQ(0, r);
                ConnectedSocket srv_socket, cli_socket; if (bind_socket) {
                r = bind_socket.accept(&srv_socket, options, &cli_addr, worker);
                ASSERT_EQ(-EAGAIN, r);}

                C_poll cb(center); if (worker->id == 0) {
                center->create_file_event(bind_socket.fd(), EVENT_READABLE,
                                          &cb);
                r = worker->connect(bind_addr, options, &cli_socket);
                ASSERT_EQ(0, r); ASSERT_TRUE(cb.poll(500));}

                if (bind_socket) {
                cb.reset(); cb.poll(500); ConnectedSocket srv_socket2;
                do {
                r =
                bind_socket.accept(&srv_socket2, options, &cli_addr, worker);
                usleep(100);} while (r == -EAGAIN && !*accepted_p); if (r == 0)
                * accepted_p = true; ASSERT_TRUE(*accepted_p);
                // srv_socket2 closed
                center->delete_file_event(bind_socket.fd(), EVENT_READABLE);}
                ...
                int i = 3; while (!i--) {
                ASSERT_TRUE(cb.poll(500));
                r = cli_socket.read(buf, sizeof(buf)); if (r == 0)
                break;}
                ASSERT_EQ(0, r);
                center->delete_file_event(cli_socket.fd(), EVENT_READABLE);}
                ...
                *accepted_p = false;
                r = worker->connect(bind_addr, options, &cli_socket);
                ASSERT_EQ(0, r); cb.reset(); ASSERT_TRUE(cb.poll(500));
                cli_socket.close();}
                ...
                * accepted_p = true;
                ...
                center->create_file_event(cli_socket.fd(), EVENT_READABLE, &cb);
                r = cli_socket.is_connected(); if (r == 0) {
                ASSERT_TRUE(cb.poll(500)); r = cli_socket.is_connected();}
                ASSERT_TRUE(r == -ECONNREFUSED || r == -ECONNRESET);}
                }) ;
                    }
                    ...
                    TEST_P(NetworkWorkerTest, ComplexTest) {
                        ...
                        exec_events([this, bind_addr, listen_p, accepted_p,
                                    ...
                                    EventCenter * center = &worker->center;
                                    SocketOptions options;
                                    ServerSocket bind_socket; int r = 0;
                                    if (stack->support_local_listen_table()
                                        || worker->id == 0) {
                                    r =
                                    worker->listen(bind_addr, options,
                                                   &bind_socket);
                                    ASSERT_EQ(0, r); *listen_p = true;}
                                    ConnectedSocket cli_socket, srv_socket;
                                    if (worker->id == 1) {
                                    while (!*listen_p) {
                                    usleep(50);
                                    r =
                                    worker->connect(bind_addr, options,
                                                    &cli_socket);
                                    ASSERT_EQ(0, r);}
                                    }
                                    ...
                                    if (bind_socket) {
                                    C_poll cb(center);
                                    center->create_file_event(bind_socket.fd(),
                                                              EVENT_READABLE,
                                                              &cb);
                                    int count = 3; while (count--) {
                                    if (cb.poll(500)) {
                                    r =
                                    bind_socket.accept(&srv_socket, options,
                                                       &cli_addr, worker);
                                    ASSERT_EQ(0, r); *accepted_p = true; break;}
                                    }
                                    ASSERT_TRUE(*accepted_p);
                                    center->delete_file_event(bind_socket.fd(),
                                                              EVENT_READABLE);}

                                    if (worker->id == 1) {
                                    C_poll cb(center);
                                    center->create_file_event(cli_socket.fd(),
                                                              EVENT_WRITABLE,
                                                              &cb);
                                    r = cli_socket.is_connected(); if (r == 0) {
                                    ASSERT_TRUE(cb.poll(500));
                                    ...
                                    ASSERT_EQ(1, r);
                                    center->delete_file_event(cli_socket.fd(),
                                                              EVENT_WRITABLE);}
                                    ...
                                    C_poll cb(center); if (worker->id == 1)
                                    center->create_file_event(cli_socket.fd(),
                                                              EVENT_WRITABLE,
                                                              &cb);
                                    if (srv_socket)
                                    center->create_file_event(srv_socket.fd(),
                                                              EVENT_READABLE,
                                                              &cb);
                                    size_t left = len; len *= 2;
                                    string read_string; int again_count = 0;
                                    int c = 2; bufferlist bl; for (size_t i = 0;
                                                                   i < count;
                                                                   ++i)
                                    bl.
                                    push_back(bufferptr
                                              ((char *)message.data(),
                                               message_size));
                                    while (!*done_p) {
                                    again_count = 0; if (worker->id == 1) {
                                    if (c > 0) {
                                    ssize_t r = 0; usleep(100); if (left > 0) {
                                    ...
                                    if (srv_socket) {
                                    char buf[1000]; if (len > 0) {
                                    r = srv_socket.read(buf, sizeof(buf));
                                    ASSERT_TRUE(r > 0 || r == -EAGAIN);
                                    if (r > 0) {
                                    read_string.append(buf, r); len -= r;}
                                    else
                                    if (r == -EAGAIN) {
                                    ++again_count;}
                                    }
                                    if (len == 0) {
                                    for (size_t i = 0; i < read_string.size();
                                         i += message_size)
                                    ASSERT_EQ(0,
                                              memcmp(read_string.c_str() + i,
                                                     message.c_str(),
                                                     message_size));
                                    *done_p = true;}
                                    }
                                    if (again_count) {
                                    cb.reset(); cb.poll(500);}
                                    }
                                    if (worker->id == 1)
                                    center->delete_file_event(cli_socket.fd(),
                                                              EVENT_WRITABLE);
                                    if (srv_socket)
                                    center->delete_file_event(srv_socket.fd(),
                                                              EVENT_READABLE);
                                    if (bind_socket)
                                    bind_socket.abort_accept(); if (srv_socket)
                                    srv_socket.close(); if (worker->id == 1)
                                    cli_socket.close();}
                        ) ;
                    }
                    ...
                    class StressFactory {
                        ...
                        struct RandomString {
                            ...
                            RandomString(size_t s):slen(s), rng(rd()) {
                                ...
                                strs.reserve(n);
                                std::generate_n(std::back_inserter(strs),
                                                strs.capacity(),[&] {
                                                std::string str;
                                                str.reserve(slen);
                                                std::generate_n(std::
                        ...
                        struct Message {
                            ...
                            explicit Message(RandomString & rs, size_t i,
                                             size_t l):idx(i) {
                                size_t slen = rs.slen;
                                len = std::max(slen, l);
                      ...
                      template < typename T > class C_delete:public EventCallback
                        ...
                        class Client {
                          ...
                          public:
                            Client(StressFactory * f, EventCenter * cen,
                                   ConnectedSocket s, size_t c)
                          :    factory(f), center(cen), socket(std::move(s)),
                                left(c), homeless_message(factory->rs, -1,
                                                          1024),
                                read_ctxt(this), write_ctxt(this) {
                                center->create_file_event(socket.fd(),
                                                          EVENT_READABLE,
                                                          &read_ctxt);
                            ...
                            void close() {
                                ...
                                dead = true;
                                socket.shutdown();
                                center->delete_file_event(socket.fd(),
                                                          EVENT_READABLE);
                                center->dispatch_event_external(new C_delete <
                                                                Client >
                                                                (this));
                            }
                            ...
                            void do_read_request() {
                                ...
                                bool must_no = false;
                                while (true) {
                                    r = socket.read((char *)buffer.data() +
                                                    read_offset,
                                                    m->len - read_offset);
                            ...
                            void do_write_request() {
                                ...
                                while (left > 0
                                       && factory->queue_depth >
                                       writings.size() + acking.size()) {
                                    StressFactory::Message * m =
                                        new StressFactory::Message(factory->rs,
                                                                   ++index,
                                                                   factory->
                                                                   rd() %
                                                                   factory->
                                ...
                                while (!writings.empty()) {
                                    StressFactory::Message * m =
                                        writings.front();
                                    bufferlist bl;
                                    bl.append(m->content.data() + write_offset,
                                              m->content.size() - write_offset);
                                    ssize_t r = socket.send(bl, false);
                                    if (r == 0)
                                        break;
                                    std::
                                        cerr << " client " << this << " send "
                                        << m->
                                        idx << " len " << r << " content: " <<
                                        std::endl;
                                    ASSERT_TRUE(r >= 0);
                                ...
                                if (writings.empty() && write_enabled) {
                                    ...
                                    ASSERT_EQ(0,
                                              center->create_file_event(socket.
                                                                        fd(),
                                                                        EVENT_WRITABLE,
                                                                        &write_ctxt));
                                    write_enabled = true;
                        ...
                        class Server {
                          ...
                          Server(StressFactory * f, EventCenter * c, ConnectedSocket s):
                            factory(f), center(c), socket(std::move(s)),
                                read_ctxt(this), write_ctxt(this) {
                                center->create_file_event(socket.fd(),
                                                          EVENT_READABLE,
                                                          &read_ctxt);
                            ...
                            void do_read_request() {
                                ...
                                while (true) {
                                    ...
                                    if (r == 0) {
                                        ...
                                        break;
                                    if (factory->zero_copy_read) {
                                        buffers.emplace_back(data.c_str(), 0,
                                                             data.length());
                                    }
                                    else {
                        ...
                        class C_accept:public EventCallback {
                          ...
                          public:
                            C_accept(StressFactory * f, ServerSocket s,
                                     ThreadData * data, Worker * w)
                            :factory(f), bind_socket(std::move(s)),
                                t_data(data), worker(w) {
                            } void do_request(int id) override {
                                while (true) {
                                    entity_addr_t cli_addr;
                      ...
                      public:
                        ...
                        RandomString rs;
                        std::random_device rd;
                        const size_t client_num, queue_depth,
                            max_message_length;
                        atomic_int message_count, message_left;
                        entity_addr_t bind_addr;
                        std::atomic_bool already_bind = {
                        false};
                        bool zero_copy_read;
                        SocketOptions options;

                        explicit StressFactory(std::shared_ptr < NetworkStack >
                                               s, const string & addr,
                                               size_t cli, size_t qd, size_t mc,
                                               size_t l, bool zero_copy)
                        :stack(s), rs(128), client_num(cli), queue_depth(qd),
                            max_message_length(l), message_count(mc),
                            message_left(mc), zero_copy_read(zero_copy) {
                        ...
                        void add_client(ThreadData * t_data) {
                            ...
                            int r =
                                t_data->worker->connect(bind_addr, options,
                                                        &sock);
                            std::default_random_engine rng(rd());
                            std::
                                uniform_int_distribution <>
                                dist(min_client_send_messages,
                            ...
                            Client *cb =
                                new Client(this, &t_data->worker->center,
                                           std::move(sock), c);
                        ...
                        void start(Worker * worker) {
                            ...
                            if (stack->support_local_listen_table()
                                || worker->id == 0) {
                                r = worker->listen(bind_addr, options,
                                                   &bind_socket);
                                ASSERT_EQ(0, r);
                                already_bind = true;
                            }
                            ...
                            while (message_count > 0 || !t_data.clients.empty()
                                ...
                                if (message_count > 0
                                    && t_data.clients.size() < client_num
                              ...
                              for (auto && s:t_data.
                                ...
                                if (echo_throttle > message_left) {
                                    std::cerr << " clients " << t_data.clients.
                                        size() << " servers " << t_data.servers.
                                        size()
                                        << " message count " << message_left <<
                                        std::endl;
                                    echo_throttle -= 100;
                                }
                            }
                            if (bind_fd)
                                worker->center.delete_file_event(bind_fd,
                                                                 EVENT_READABLE);
                    ...
                    TEST_P(NetworkWorkerTest, StressTest) {
                        StressFactory factory(stack, get_addr(), 16, 16, 10000,
                                              1024, strncmp(GetParam(), "dpdk",
                                                            4) == 0);
                    ...
                    INSTANTIATE_TEST_CASE_P(NetworkStack,
                                            NetworkWorkerTest,::testing::Values(
#ifdef HAVE_DPDK
                                                                                      "dpdk",
#endif
                                                                                      "posix")
