...
static inline void encode(const std::map < std::string,
                          ceph::buffer::ptr > *attrset, ceph::buffer::list &bl)
{
    using ceph::encode;
    encode(*attrset, bl);
}
...
public:
    ...
    struct CollectionImpl: public RefCountedObject {
        ...
        CollectionImpl(CephContext *cct,
                       const coll_t &c): RefCountedObject(cct), cid(c)
        {
...
public:
    ...
    virtual bool has_null_manager() const
    {
        return false;
    }
    ...
    virtual int get_numa_node(int *numa_node,
                              std::set < int > *nodes,
     ...
     */
    virtual void set_collection_commit_queue(const coll_t &cid,
     ...
     */
    virtual int set_collection_opts(CollectionHandle &c,
    ...
    virtual int read(CollectionHandle &c,
                     ...
                     uint64_t offset,
                     size_t len,
                     ceph::buffer::list &bl, uint32_t op_flags = 0) = 0;
     ...
     * @returns number of bytes read on success, or negative error code on failure.
     */
    virtual int readv(CollectionHandle &c,
                      const ghobject_t &oid,
     ...
     * @returns 0 on success, negative error code on failure.
     */
    virtual int dump_onode(CollectionHandle &c,
                           const ghobject_t &oid,
