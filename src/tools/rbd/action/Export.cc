        ...
        namespace export_full {
            ...
            struct ExportDiffContext {
                ...
                OrderedThrottle throttle;
                 ...
                 ExportDiffContext(librbd::Image * i, int f, uint64_t t,
                                   int max_ops, bool no_progress,
                                   int eformat):image(i), fd(f),
                    export_format(eformat), totalsize(t), pc("Exporting image",
                                                             no_progress),
            ...
            class C_ExportDiff:public Context {
              public:
                C_ExportDiff(ExportDiffContext * edc, uint64_t offset,
                ...
                :m_export_diff_context(edc), m_offset(offset), m_length(length),
                    ...
                    if (m_export_diff_context->throttle.pending_error()) {
                        ...
                        m_export_diff_context->throttle.start_op(this);
                    if (m_exists) {
                        ...
                        int r =
                            m_export_diff_context->image->aio_read2(m_offset,
                                                                    m_length,
                                                                    m_read_data,
                                                                    aio_completion,
                                                                    op_flags);
                ...
                static int export_diff_cb(uint64_t offset, size_t length,
                                          int exists, void *arg) {
                    ExportDiffContext *edc =
                        reinterpret_cast < ExportDiffContext * >(arg);

                    C_ExportDiff *context =
                        new C_ExportDiff(edc, offset, length, exists,
                                         edc->export_format);
                    return context->send();
                }
              ...
              protected:
                void finish(int r) override {
                    if (r >= 0) {
                        ...
                        r = write_extent(m_export_diff_context, m_offset,
                                         m_length, m_exists, m_export_format);
                        if (r == 0 && m_exists) {
              ...
              private:
                ...
                static int write_extent(ExportDiffContext * edc,
            ...
            int do_export_diff_fd(librbd::Image & image,
                ...
                r = image.diff_iterate2(fromsnapname, 0, info.size, true,
                                        whole_object,
                                        &C_ExportDiff::export_diff_cb,
                                        (void *)&edc);
                if (r < 0) {
                    goto out;
                }
            ...
            void get_arguments_diff(po::options_description * positional,
                ...
                options->add_options()
                    (at::FROM_SNAPSHOT_NAME.c_str(),
                     po::value < std::string > (), "snapshot starting point")
            ...
            int execute_diff(const po::variables_map & vm) {
                ...
                if (vm.count(at::FROM_SNAPSHOT_NAME)) {
                    from_snap_name =
                        vm[at::FROM_SNAPSHOT_NAME].as < std::string > ();
                }
                ...
                r = do_export_diff(image,
                                   from_snap_name.
                                   empty()? nullptr : from_snap_name.c_str(),
                                   snap_name.empty()? nullptr : snap_name.
                                   c_str(), vm[at::WHOLE_OBJECT].as < bool > (),
                                   path.c_str(),
                                   vm[at::NO_PROGRESS].as < bool > ());
                if (r < 0) {
                    std::
                        cerr << "rbd: export-diff error: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            class C_Export:public Context {
              public:
                C_Export(SimpleThrottle & simple_throttle,
                         librbd::Image & image, uint64_t fd_offset,
                ...
                :m_aio_completion(new librbd::RBD::
                    ...
                    m_dest_offset(fd_offset), m_offset(offset),
                    ...
                    if (r < 0) {
                        cerr << "rbd: error requesting read from source image"
                            << std::endl;
                ...
                void finish(int r) override {
                    ...
                    if (m_fd != STDOUT_FILENO) {
                        ...
                        uint64_t chkret =
                            lseek64(m_fd, m_dest_offset, SEEK_SET);
            ...
            static int do_export_v2(librbd::Image & image,
                                    librbd::image_info_t & info, int fd,
                ...
                while (more_results) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "failed to retrieve metadata of image : " <<
                            cpp_strerror(r)
                            << std::endl;
                        return r;
                    }
                    ...
                    if (!pairs.empty()) {
                      ...
                      for (auto kv:pairs) {
                            ...
                            std::string val(kv.second.c_str(),
                                            kv.second.length());
                            imagemetas[key] = val;
                        }
                    }
                ...
                for (std::map < std::string, string >::iterator it =
                ...
                for (size_t i = 0; i < snaps.size(); ++i) {
                    ...
                    r = do_export_diff_fd(image, last_snap,
                                          snaps[i].name.c_str(), false, fd,
                                          true, 2);
                    if (r < 0) {
                        return r;
                    }
                ...
                r = do_export_diff_fd(image, last_snap, nullptr, false, fd,
                                      true, 2);
                if (r < 0) {
                    return r;
                }
            ...
            static int do_export_v1(librbd::Image & image,
                                    librbd::image_info_t & info, int fd,
                                    uint64_t period, int max_concurrent_ops,
                                    utils::ProgressContext & pc) {
                int r = 0;
                ...
                for (uint64_t offset = 0; offset < info.size; offset += period) {
                    ...
                    C_Export *ctx =
                        new C_Export(throttle, image, file_size + offset,
                                     offset, length, fd);
                    ctx->send();
            ...
            static int do_export(librbd::Image & image, const char *path,
                ...
                uint64_t period =
                    image.get_stripe_count() * (1ull << info.order);

                if (export_format == 1)
                    r = do_export_v1(image, info, fd, period,
                                     max_concurrent_ops, pc);
                else
                    r = do_export_v2(image, info, fd, period,
                                     max_concurrent_ops, pc);
            ...
            int execute(const po::variables_map & vm) {
                ...
                r = do_export(image, path.c_str(),
                              vm[at::NO_PROGRESS].as < bool > (), format);
                if (r < 0) {
                    std::
                        cerr << "rbd: export error: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            Shell::Action action( {
                                 ...
                                 , "Export image to file.", "", &get_arguments,
                                 &execute);
