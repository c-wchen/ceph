...
void usage()
{
    ...
    cout << "   --test-map-pgs-dump-all [--pool <poolid>] map all pgs to osds"
        << std::endl;
    ...
    for (std::vector < const char *>::iterator i = args.begin();
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            upmap_cleanup = true;
                 ...
                 (args, i, &upmap_file, "--upmap-save", (char *)NULL)) {
            upmap_save = true;
        }
        else if (ceph_argparse_witharg
      ...
      for (auto & s:upmap_pools) {
        ...
        if (!pools.empty())
            cout << " limiting to pools " << upmap_pools << " (" << pools << ")"
                << std::endl;
        int changed = osdmap.calc_pg_upmaps(g_ceph_context, upmap_deviation,
                                            upmap_max, pools,
                                            &pending_inc);
        if (changed) {
            print_inc_upmaps(pending_inc, upmap_fd);
