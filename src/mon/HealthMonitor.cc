...
void HealthMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  for (auto & p:pending_health.checks) {
        p.second.summary = boost::regex_replace(p.second.summary,
                                                ...
                                                1 ? "have" : "has");
        p.second.summary =
            boost::regex_replace(p.second.summary, boost::regex("%names%"),
...
bool HealthMonitor::check_member_health()
{
    ...
    // OSD_NO_DOWN_OUT_INTERVAL
    {
        ...
        if (g_conf->mon_warn_on_osd_down_out_interval_zero &&
            ...
            auto & d =
                next.add("OSD_NO_DOWN_OUT_INTERVAL", HEALTH_WARN, ss.str());
            ds << "mon." << mon->
...
bool HealthMonitor::check_leader_health()
{
    ...
    if (!mon->timecheck_skews.empty()) {
        ...
        for (map < entity_inst_t, double >::iterator i =
             mon->timecheck_skews.begin(); i != mon->timecheck_skews.end();
             ++i) {
