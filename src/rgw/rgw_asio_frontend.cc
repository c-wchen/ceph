...
// use mmap/mprotect to allocate 512k coroutine stacks
  ...
  template < typename Stream > class StreamIO:public rgw::asio::ClientIO {
      ...
      public:
        ...
        boost::system::error_code get_fatal_error_code()const {
            return fatal_ec;
        } size_t write_data(const char *buf, size_t len)override {
            boost::system::error_code ec;
            timeout.start();
            auto bytes =
                boost::asio::async_write(stream, boost::asio::buffer(buf, len),
                                         yield[ec]);
            timeout.cancel();
            if (ec) {
                ...
                if (ec == boost::asio::error::broken_pipe) {
                    ...
                    stream.lowest_layer().shutdown(tcp_socket::shutdown_both,
                                                   ec_ignored);
                }
                if (!fatal_ec) {
                    fatal_ec = ec;
        ...
        size_t recv_body(char *buf, size_t max)override {
            ...
            while (body_remaining.size && !parser.is_done()) {
                ...
                if (ec) {
                    ...
                    throw rgw::io::Exception(ec.value(),
                                             std::system_category());
                }
            }
...
// log fractional seconds in milliseconds
    ...
    std::ostream & operator<<(std::ostream & out, const log_ms_remainder & m) {
        ...
        return out << std::setfill('0') << std::setw(3)
            ...
            (m.t.time_since_epoch()).count() % 1000;
    }
...
// log time in apache format: day/month/year:hour:minute:second zone
    ...
    std::ostream & operator<<(std::ostream & out, const log_apache_time & a) {
        ...
        return out << std::put_time(local, "%d/%b/%Y:%T.") << log_ms_remainder {
        a.t}
    ...
    template < typename Stream >
        ...
        static constexpr size_t body_limit =
        ...
        for (;;) {
            ...
            if (ec == boost::asio::error::connection_reset ||
                ...
                ldout(cct,
                      20) << "failed to read header: " << ec.message() << dendl;
                return;
            }
            auto & message = parser.get();
            if (ec) {
                ldout(cct,
                      1) << "failed to read header: " << ec.message() << dendl;
            ...
            bool expect_continue =
                (message[http::field::expect] == "100-continue");

            {
                ...
                if (ec == boost::asio::error::operation_aborted) {
                    ...
                    ldout(cct,
                          1) << "failed to lock: " << ec.message() << dendl;
                    return;
                }

                // process the request
                RGWRequest req {
                env.driver->get_new_req_id()};
                ...
                cct, stream, timeout, parser, yield, buffer,
                        is_ssl, local_endpoint, remote_endpoint};

                auto real_client_io =
                    rgw::io::add_reordering(rgw::io::add_buffering(cct,
                                                                   rgw::io::
                                                                   add_chunking
                                                                   (rgw::io::
                ...
                if (cct->_conf->subsys.should_gather(ceph_subsys_rgw_access, 1)) {
                    ...
                    lsubdout(cct, rgw_access,
                             1) << "beast: " << std::hex << &req << std::
                        dec << ": " << remote_endpoint.
                        address() << " - " << user << " [" << log_apache_time {
                    started}
                    <<"] \""
                        << message.method_string() << ' ' << message.
                        target() << ' ' << http_version {
                    message.version()}
                    <<"\" " << http_ret << ' '
                        << client.get_bytes_sent() +
                        client.get_bytes_received() << ' ' << log_header {
                    message, http::field::referer, "\""}
                    <<' ' << log_header {
                    message, http::field::user_agent, "\""}
                    <<' ' << log_header {
                    message, http::field::range}
...
// timeout_handler can outlive the coroutine
    struct Connection:boost::intrusive::list_base_hook <>,
        ...
        explicit Connection(tcp_socket
                            && socket) noexcept:socket(std::move(socket)) {
    ...
    class ConnectionList {
      ...
      public:
        class Guard {
            ...
            Connection *conn;
          public:
          Guard(ConnectionList * list, Connection * conn):list(list),
                conn(conn)
            {
            }
            ~Guard() {
    ...
    class AsioFrontend {
        ...
        int get_config_key_val(string name,
                               const string & type, bufferlist * pbl);
    ...
    static int drop_privileges(CephContext * ctx) {
        ...
        if (gid && setgid(gid) != 0) {
            int err = errno;
            ldout(ctx,
                  -1) << "unable to setgid " << gid << ": " << cpp_strerror(err)
                << dendl;
            return -err;
        }
        if (uid && setuid(uid) != 0) {
            int err = errno;
            ldout(ctx,
                  -1) << "unable to setuid " << uid << ": " << cpp_strerror(err)
                << dendl;
            return -err;
        }
        if (uid && gid) {
...
// Setting global timeout
        ...
        if (timeout != config.end()) {
            ...
            if (timeout_number) {
                ...
                lderr(ctx()) <<
                    "WARNING: invalid value for request_timeout_ms: " <<
                    timeout->
                    second << " setting it to the default value: " <<
        ...
        if (max_header_size != config.end()) {
            ...
            if (!limit) {
                ...
                lderr(ctx()) << "WARNING: max_header_size " << max_header_size->
                    second << " capped at maximum value " << header_limit <<
                    dendl;
            }
            else {
                header_limit = *limit;
        ...
        for (auto i = endpoints.first; i != endpoints.second; ++i) {
            ...
            if (ec) {
                lderr(ctx()) << "failed to parse endpoint=" << i->
                    second << dendl;
                return -ec.value();
            }
            listeners.emplace_back(context);
      ...
      for (auto & l:listeners) {
            ...
            if (ec) {
                if (ec == boost::asio::error::address_family_not_supported) {
                    ldout(ctx(),
                          0) << "WARNING: cannot open socket for endpoint=" <<
                        l.endpoint << ", " << ec.message() << dendl;
                    continue;
                }

                lderr(ctx()) << "failed to open socket: " << ec.
                    message() << dendl;
                return -ec.value();
            }
            ...
            if (it != config.end()) {
                ...
                max_connection_backlog =
                    strict_strtol(it->second.c_str(), 10, &err);
                if (!err.empty()) {
                    ldout(ctx(),
                          0) <<
                        "WARNING: invalid value for max_connection_backlog=" <<
                        it->second << dendl;
                    max_connection_backlog =
                        boost::asio::socket_base::max_listen_connections;
                }
    ...
    int AsioFrontend::ssl_set_private_key(const string & name,
        ...
        if (!boost::algorithm::starts_with(name, config_val_prefix)) {
            ...
            ssl_context->
                use_private_key(boost::asio::buffer(bl.c_str(), bl.length()),
                                ssl::context::pem, ec);
        }
    ...
    int AsioFrontend::ssl_set_certificate_chain(const string & name) {
        ...
        if (!boost::algorithm::starts_with(name, config_val_prefix)) {
            ...
            ssl_context->
                use_certificate_chain(boost::asio::
                                      buffer(bl.c_str(), bl.length()), ec);
        }
    ...
    int AsioFrontend::init_ssl() {
        ...
        if (options) {
            if (!cert) {
                lderr(ctx()) << "no ssl_certificate configured for ssl_options"
                    << dendl;
                return -EINVAL;
            }
        }
        ...
        if (ciphers) {
            if (!cert) {
                lderr(ctx()) << "no ssl_certificate configured for ssl_ciphers"
                    << dendl;
                return -EINVAL;
            }
        ...
        for (auto i = ports.first; i != ports.second; ++i) {
            if (!have_cert) {
                lderr(ctx()) << "no ssl_certificate configured for ssl_port" <<
                    dendl;
                return -EINVAL;
            }
            auto port = parse_port(i->second.c_str(), ec);
            if (ec) {
                lderr(ctx()) << "failed to parse ssl_port=" << i->
                    second << dendl;
                return -ec.value();
            }
            listeners.emplace_back(context);
        ...
        for (auto i = endpoints.first; i != endpoints.second; ++i) {
            if (!have_cert) {
                lderr(ctx()) << "no ssl_certificate configured for ssl_endpoint"
                    << dendl;
                return -EINVAL;
            }
            auto endpoint = parse_endpoint(i->second, 443, ec);
            if (ec) {
                lderr(ctx()) << "failed to parse ssl_endpoint=" << i->
                    second << dendl;
                return -ec.value();
            }
            listeners.emplace_back(context);
    ...
    void AsioFrontend::accept(Listener & l, boost::system::error_code ec) {
        ...
        if (l.use_ssl) {
            spawn::spawn(context,
                         [this, s =
                          std::move(stream)] (yield_context yield) mutable {
                         auto conn = boost::intrusive_ptr {
                         new Connection(std::move(s))};
                         auto c = connections.add(*conn);
                         // wrap the tcp stream in an ssl stream
                         boost::asio::ssl::stream < tcp_socket & >stream {
                         conn->socket, *ssl_context};
                         auto timeout = timeout_timer {
                         context.get_executor(), request_timeout, conn};
                         // do ssl handshake
                         boost::system::error_code ec;
                         timeout.start();
                         auto bytes =
                         ...
                         timeout.cancel(); if (ec) {
                         ldout(ctx(),
                               1) << "ssl handshake failed: " << ec.
                         message() << dendl; return;}
                         conn->buffer.consume(bytes);
                         handle_connection(context, env, stream, timeout,
                                           header_limit, conn->buffer, true,
                                           pause_mutex, scheduler.get(),
                                           uri_prefix, ec, yield); if (!ec) {
...
#else
        {
            ...
            spawn::spawn(context,
                         ...
                         context.get_executor(), request_timeout, conn};
                         boost::system::error_code ec;
                         handle_connection(context, env, conn->socket, timeout,
                                           header_limit, conn->buffer, false,
                                           pause_mutex, scheduler.get(),
                                           uri_prefix, ec, yield);
    ...
    int AsioFrontend::run() {
        ...
        ldout(cct,
              4) << "frontend spawning " << thread_count << " threads" << dendl;
    ...
    void AsioFrontend::pause() {
      ...
      for (auto & l:listeners) {
        ...
        if (ec) {
            ldout(ctx(),
                  1) << "frontend failed to pause: " << ec.message() << dendl;
        }
        else {
