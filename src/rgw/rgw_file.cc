...
namespace rgw {
            ...
            case 2:
                {
                    ...
                    path};
                    RGWStatLeafRequest req(cct, get_user(), parent,
                                           object_name);
                    int rc = rgwlib.get_fe()->execute_req(&req);
                    if ((rc == 0) && (req.get_ret() == 0)) {
                        if (req.matched) {
    ...
    int RGWLibFS::unlink(RGWFileHandle * rgw_fh, const char *name,
        ...
        if (parent->is_root()) {
            ...
            if (bs.num_entries > 1) {
                ...
                string oname {
                "/"};
                RGWDeleteObjRequest req(cct, get_user(), bkt_fh->bucket_name(),
                                        oname);
                         ...
                         const char *_src_name, const char *_dst_name)
    {
      ...
      for (int ix:{
            ...
            case 0:
                {
                    RGWCopyObjRequest req(cct, get_user(), src_fh, dst_fh,
                                          src_name, dst_name);
                    int rc = rgwlib.get_fe()->execute_req(&req);
                                ...
                                struct stat * st, uint32_t mask, uint32_t flags)
    {
        ...
        if (parent->is_root()) {
            ...
            RGWPutObjRequest req(get_context(), get_user(),
    ...
    MkObjResult RGWLibFS::create(RGWFileHandle * parent, const char *name,
        ...
        /* create it */
        buffer::list bl;
        RGWPutObjRequest req(cct, get_user(), parent->bucket_name(), obj_name,
                             bl);
        MkObjResult mkr {
        nullptr, -EINVAL};
    ...
    int RGWLibFS::setattr(RGWFileHandle * rgw_fh, struct stat *st,
        ...
        RGWSetAttrsRequest req(cct, get_user(), rgw_fh->bucket_name(),
                               obj_name);
    ...
    int RGWFileHandle::readdir(rgw_readdir_cb rcb, void *cb_arg,
        ...
        if (is_root()) {
            ...
            RGWReaddirRequest req(cct, fs->get_user(), this, rcb, cb_arg,
                                  offset);
            rc = rgwlib.get_fe()->execute_req(&req);
            if (!rc) {
                (void)clock_gettime(CLOCK_MONOTONIC_COARSE, &now);  /* !LOCKED */
        ...
        if (!f->write_req) {
            ...
            if (off != 0) {
                lsubdout(fs->get_context(), rgw, 5)
                    ...
                    << " non-0 initial write position " << off << dendl;
                return -EIO;
            }
    ...
    int RGWWriteRequest::exec_start() {
        ...
        auto compression_type =
            get_store()->get_zone_params().get_compression_type(s->bucket_info.
                                                                placement_rule);
