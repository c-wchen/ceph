...
struct rgw_placement_rule {
    ...
    rgw_placement_rule() {
    } rgw_placement_rule(const std::string & _n,
                         const std::string & _sc):name(_n), storage_class(_sc) {
    }
    rgw_placement_rule(const rgw_placement_rule & _r,
                       const std::string & _sc):name(_r.name) {
        if (!_sc.empty()) {
            storage_class = _sc;
        }
        else {
    ...
    static const std::string & get_canonical_storage_class(const std::
                                                           string &
                                                           storage_class) {
        if (storage_class.empty()) {
