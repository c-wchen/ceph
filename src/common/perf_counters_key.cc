...
public:
    ...
    label_insert_iterator(base_iterator begin): label {
        begin}
    {
        static_assert(std::output_iterator < label_insert_iterator,
                      label_pair >);
    }

    // increments are noops
    label_insert_iterator &operator++()
    {
        return *this;
    }
    label_insert_iterator operator++(int)
    {
        return *this;
    }
                   ...
                   label_pair *begin, label_pair *end)
{
    ...
    std::copy(begin, end, label_insert_iterator {
        out}
             );
...
void label_iterator::advance(std::optional < iterator_state > &s)
{
    ...
    s->label.second = std::string_view {
        s->pos, d};
    s->pos = std::next(d);
}
...
{
    std::optional state = iterator_state {
        begin, end};
    advance(state);
    return state;
}
