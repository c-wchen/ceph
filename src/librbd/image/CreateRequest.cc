...
namespace librbd {
    namespace image {
         ...
         namespace {
            ...
            int validate_features(CephContext * cct, uint64_t features,
                ...
                if (features & ~RBD_FEATURES_ALL) {
                    ...
                    lderr(cct) << "cannot use fast diff without object map" <<
                        dendl;
                    return -EINVAL;
                } if ((features & RBD_FEATURE_OBJECT_MAP) != 0 &&
                      (features & RBD_FEATURE_EXCLUSIVE_LOCK) == 0) {
                    lderr(cct) << "cannot use object map without exclusive lock"
                        << dendl;
                    return -EINVAL;
                }
            ...
            int validate_striping(CephContext * cct, uint8_t order,
            ...
            bool validate_layout(CephContext * cct, uint64_t size,
                                 file_layout_t & layout) {
                if (!librbd::ObjectMap <>::is_compatible(layout, size)) {
                    lderr(cct) << "image size not compatible with object map" <<
                        dendl;
                    return false;
                }
        ...
        template < typename I >
            int CreateRequest < I >::validate_order(CephContext * cct,
                                                    uint8_t order) {
...
#define dout_prefix *_dout << "librbd::image::CreateRequest: " << this << " " \
        ...
        template < typename I >
            CreateRequest < I >::CreateRequest(IoCtx & ioctx,
      ...
      :    m_ioctx(ioctx), m_image_name(image_name), m_image_id(image_id),
            ...
            if (image_options.
                get(RBD_IMAGE_OPTION_STRIPE_UNIT, &m_stripe_unit) != 0
                || m_stripe_unit == 0) {
                m_stripe_unit =
            ...
            if (image_options.
                get(RBD_IMAGE_OPTION_STRIPE_COUNT, &m_stripe_count) != 0
                || m_stripe_count == 0) {
                m_stripe_count =
                    m_cct->_conf->get_val < uint64_t >
            ...
            if (get_image_option
                (image_options, RBD_IMAGE_OPTION_ORDER, &m_order) != 0
                || m_order == 0) {
                m_order =
                    m_cct->_conf->get_val < int64_t > ("rbd_default_order");
            }
            if (get_image_option(image_options, RBD_IMAGE_OPTION_JOURNAL_ORDER,
                                 &m_journal_order) != 0) {
                m_journal_order =
                    m_cct->_conf->get_val < uint64_t > ("rbd_journal_order");
            }
            if (get_image_option
            ...
            if (image_options.
                get(RBD_IMAGE_OPTION_JOURNAL_POOL, &m_journal_pool) != 0) {
                m_journal_pool =
                    m_cct->_conf->get_val < std::string > ("rbd_journal_pool");
            }
            if (image_options.get(RBD_IMAGE_OPTION_DATA_POOL, &m_data_pool) !=
                0) {
                m_data_pool =
                    m_cct->_conf->get_val < std::string >
        ...
        template < typename I > void CreateRequest < I >::send() {
            ...
            r = validate_striping(m_cct, m_order, m_stripe_unit,
                                  m_stripe_count);
            if (r < 0) {
                complete(r);
                return;
        ...
        template < typename I > void CreateRequest < I >::validate_data_pool() {
            ...
            if ((m_features & RBD_FEATURE_DATA_POOL) != 0) {
                ...
                if (r < 0) {
                    lderr(m_cct) << "data pool " << m_data_pool <<
            ...
            librados::AioCompletion * comp =
                ...
                &klass::handle_validate_data_pool > (this);
        ...
        template < typename I >
            ...
            if (r >= 0 && m_outbl.contents_equal(bl)) {
                ...
                lderr(m_cct) << "failed to read RBD info: " << cpp_strerror(r)
                    << dendl;
                complete(r);
                return;
            }
            ...
            ldout(m_cct,
                  10) << "validating self-managed RBD snapshot support" <<
                dendl;
            ...
            if (r < 0) {
                ...
                ldout(m_cct,
                      10) << "failed to release self-managed snapshot " <<
            ...
            r = m_data_io_ctx.write(RBD_INFO, initial_bl, initial_bl.length(),
                                    0);
            if (r >= 0) {
                r = m_data_io_ctx.write(RBD_INFO, bl, bl.length(), 0);
            }
            if (r == -EOPNOTSUPP) {
                lderr(m_cct) << "pool missing required overwrite support" <<
                    dendl;
                complete(-EINVAL);
                return;
            }
            else if (r < 0) {
                lderr(m_cct) << "failed to validate overwrite support: " <<
                    cpp_strerror(r)
                    << dendl;
                complete(r);
                return;
        ...
        template < typename I > void CreateRequest < I >::create_id_object() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_create_id_object > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "error creating RBD id object: " <<
                    cpp_strerror(r)
                    << dendl;
                complete(r);
                return;
        ...
        template < typename I >
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_add_image_to_directory > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "error adding image to directory: " <<
                    cpp_strerror(r)
                    << dendl;
        ...
        template < typename I > void CreateRequest < I >::negotiate_features() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_negotiate_features > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                ...
                ldout(m_cct,
                      10) <<
                    "limiting default features set to server supported: " <<
                    m_features << dendl;
            }
        ...
        template < typename I > void CreateRequest < I >::create_image() {
            ...
            librados::ObjectWriteOperation op;
            op.create(true);
            cls_client::create_image(&op, m_size, m_order, m_features,
                                     oss.str(), m_data_pool_id);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "error writing header: " << cpp_strerror(r) <<
                    dendl;
                m_r_saved = r;
                remove_from_dir();
                return;
        ...
        template < typename I >
            ...
            librados::ObjectWriteOperation op;
            cls_client::set_stripe_unit_count(&op, m_stripe_unit,
                                              m_stripe_count);

            using klass = CreateRequest < I >;
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_set_stripe_unit_count > (this);
            int r = m_ioctx.aio_operate(m_header_obj, comp, &op);
            assert(r == 0);
            comp->release();
        }
        ...
        template < typename I > void CreateRequest < I >::object_map_resize() {
            ...
            librados::ObjectWriteOperation op;
            cls_client::object_map_resize(&op,
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_object_map_resize > (this);
        ...
        template < typename I > void CreateRequest < I >::fetch_mirror_mode() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_fetch_mirror_mode > (this);
            m_outbl.clear();
        ...
        template < typename I >
            ...
            if (r == 0) {
                bufferlist::iterator it = m_outbl.begin();
                r = cls_client::mirror_mode_get_finish(&it,
        ...
        template < typename I > void CreateRequest < I >::journal_create() {
            ...
            Context *ctx =
                ...
                &klass::handle_journal_create > (this);

            librbd::journal::TagData tag_data;
            tag_data.mirror_uuid =
                (m_force_non_primary ? m_primary_mirror_uuid : librbd::Journal <
                 I >::LOCAL_MIRROR_UUID);

            librbd::journal::CreateRequest < I > *req =
                librbd::journal::CreateRequest < I >::create(m_ioctx,
                                                             m_image_id,
                                                             m_journal_order,
                                                             m_journal_splay_width,
                                                             m_journal_pool,
        ...
        template < typename I > void CreateRequest < I >::mirror_image_enable() {
            if (((m_mirror_mode != RBD_MIRROR_MODE_POOL)
                 && !m_force_non_primary) || m_skip_mirror_enable) {
                complete(0);
                return;
            }
            ...
            auto ctx = create_context_callback <
                CreateRequest < I >,
                &CreateRequest < I >::handle_mirror_image_enable > (this);
...
// cleanup
        template < typename I > void CreateRequest < I >::journal_remove() {
            ...
            Context *ctx =
                create_context_callback < klass,
                &klass::handle_journal_remove > (this);

            librbd::journal::RemoveRequest < I > *req =
                librbd::journal::RemoveRequest < I >::create(m_ioctx,
                                                             m_image_id,
                                                             librbd::Journal <
                                                             I >::
                                                             IMAGE_CLIENT_ID,
        ...
        template < typename I > void CreateRequest < I >::remove_header_object() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_remove_header_object > (this);
        ...
        template < typename I > void CreateRequest < I >::remove_from_dir() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_remove_from_dir > (this);
            int r = m_ioctx.aio_operate(RBD_DIRECTORY, comp, &op);
            assert(r == 0);
            comp->release();
        }
        ...
        template < typename I > void CreateRequest < I >::remove_id_object() {
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_remove_id_object > (this);
