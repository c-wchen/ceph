...
namespace crimson::os::seastore::segment_manager {
    ...
    std::ostream & operator<<(std::ostream & lhs, const ephemeral_config_t & c) {
        return lhs << "ephemeral_config_t(size=" << c.
            size << ", block_size=" << c.block_size << ", segment_size=" << c.
            segment_size << ")";
    } EphemeralSegmentManagerRef create_test_ephemeral() {
        return EphemeralSegmentManagerRef(new
                                          EphemeralSegmentManager
                                          (DEFAULT_TEST_EPHEMERAL));
    }

    device_config_t get_ephemeral_device_config(std::size_t index,
        ...
        auto get_sec_dtype =[num_main_devices] (std::size_t idx) {
            if (idx < num_main_devices) {
                                         ...
                                         secondary_devices; if (index == 0) {
                                         is_major_device = true;
                                         for (std::size_t secondary_index =
                                              index + 1;
                                              secondary_index < num_devices;
                                              ++secondary_index) {
                                         device_id_t secondary_id =
                                         ...
                                         device_id_t id =
                                         static_cast < device_id_t > (index);
                                         seastore_meta_t meta = {
                                         }; return {
                                         is_major_device, device_spec_t {
                                         magic, get_sec_dtype(index), id}
                                         , meta, secondary_devices};}
  ...
  :                                     manager(manager), id(id) {
                                         ...
                                         segment_off_t EphemeralSegment::
                                         get_write_capacity()const {
                                         return manager.get_segment_size();}

                                         Segment::close_ertr::
                                         future <> EphemeralSegment::close() {
                                         return manager.segment_close(id).
                                         safe_then([] {
                                                   return seastar::sleep(std::
                                                                         chrono::
                                                                         milliseconds
                                                                         (1));}
                                         );}
                                         ...
                                         Segment::write_ertr::
                                         future <> EphemeralSegment::
                                         write(segment_off_t offset,
                                               ceph::bufferlist bl) {
                                         if (offset < write_pointer
                                             || offset %
                                             manager.config.block_size != 0)
                                         return crimson::ct_error::invarg::
                                         make();
                                         if (offset + bl.length() >
                                             (size_t) manager.
                                             get_segment_size())
                                         return crimson::ct_error::enospc::
                                         make();
                                         return manager.
                                         segment_write(paddr_t::
                                                       make_seg_paddr(id,
                                                                      offset),
                                                       bl);}
                                         ...
                                         EphemeralSegmentManager::
                                         mkfs(device_config_t _config) {
                                         logger().
                                         info
                                         ("Mkfs ephemeral segment manager with {}",
                                          _config); device_config = _config;
                                         return mkfs_ertr::now();}

                                         Segment::write_ertr::
                                         future <> EphemeralSegmentManager::
                                         segment_write(paddr_t addr,
                                                       ceph::bufferlist bl,
                                                       bool ignore_check) {
                                         ...
                                         ("segment_write to segment {} at offset {}, physical offset {}, len {}, crc {}",
                                          ...
                                          get_offset(addr), bl.length(),
                                          bl.crc32c(1));
                                         if (!ignore_check
                                             && segment_state[seg_addr.
                                                              get_segment_id().
                                                              device_segment_id
                                                              ()]
                                             != segment_state_t::OPEN)
                                         ...
                                         EphemeralSegmentManager::init_ertr::
                                         future <> EphemeralSegmentManager::
                                         init() {
                                         logger().
                                         info
                                         ("Initing ephemeral segment manager with config {}",
                                          config);
                                         if (config.block_size % (4 << 10) !=
                                             0) {
                                         return crimson::ct_error::invarg::
                                         make();}
                                         if (config.segment_size %
                                             config.block_size != 0) {
                                         return crimson::ct_error::invarg::
                                         make();}
                                         if (config.size %
                                         ...
                                         void *addr =::mmap(nullptr,
                                                            ...
                                                            MAP_ANONYMOUS, -1,
                                                            0);
                                         segment_state.resize(config.size /
                                                              config.
                                                              segment_size,
                                                              segment_state_t::
                                                              EMPTY);
      ...
      for (auto & i:                    segment_state) {
                                         ...
                                         SegmentManager::open_ertr::future <
                                         SegmentRef >
                                         EphemeralSegmentManager::
                                         ...
                                         logger().
                                         error
                                         ("EphemeralSegmentManager::open: invalid segment {}",
                                          id);
                                         return crimson::ct_error::invarg::
                                         make();}
                                         ...
                                         logger().
                                         error
                                         ("EphemeralSegmentManager::open: segment {} not empty",
                                          id);
                                         return crimson::ct_error::invarg::
                                         make();}
                                         ...
                                         segment_state[s_id] =
                                         segment_state_t::OPEN;
                                         return open_ertr::make_ready_future <
                                         SegmentRef >
                                         (new EphemeralSegment(*this, id));}

                                         SegmentManager::release_ertr::
                                         future <> EphemeralSegmentManager::
                                         release(segment_id_t id) {
                                         auto s_id = id.device_segment_id();
                                         logger().
                                         debug
                                         ("EphemeralSegmentManager::release: {}",
                                          id); if (s_id >= get_num_segments()) {
                                         logger().
                                         error
                                         ...
                                         if (seg_addr.get_segment_off() + len >
                                             config.segment_size) {
                                         logger().
                                         error
                                         ...
                                         out.copy_in(0, len,
                                                     buffer + get_offset(addr));
                                         bufferlist bl; bl.push_back(out);
                                         logger().
                                         debug
