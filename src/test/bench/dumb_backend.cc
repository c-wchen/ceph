...
void DumbBackend::_write(const string & oid,
                         uint64_t offset,
                         const bufferlist & bl,
                         Context * on_applied, Context * on_commit)
{
    string full_path(get_full_path(oid));
    int fd =::open(full_path.c_str(), O_CREAT | O_WRONLY, 0777);
    if (fd < 0) {
...
#warning "sync_file_range not supported!"
    ...
    if (do_fadvise) {
        ...
        if (fa_r) {
            std::cout << "posix_fadvise failed, errno is: " << fa_r << std::
                endl;
        }
    }
#else
...
void DumbBackend::read(const string & oid,
                       uint64_t offset,
                       uint64_t length, bufferlist * bl, Context * on_complete)
{
    string full_path(get_full_path(oid));
    int fd =::open(full_path.c_str(), 0, O_RDONLY);
    if (fd < 0)
        return;
    ...
    while (1) {
...
#endif
        {
            ...
            for (set < Context * >::iterator i = pending_commits.begin();
                 i != pending_commits.end(); pending_commits.erase(i++)) {
                (*i)->complete(0);
            }
        }
        tp.unpause();
