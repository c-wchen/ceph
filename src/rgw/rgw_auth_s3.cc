 ...
 */
void rgw_create_s3_canonical_header(const char *const method,
    ...
    if (qsr) {
        ...
        if (header_time) {
            ...
            if (!parse_rfc2616(req_date, &t)) {
                dout(0) << "NOTICE: failed to parse date for auth header" <<
                    dendl;
                return false;
            }
            if (t.tm_year < 70) {
                dout(0) << "NOTICE: bad date (predates epoch): " << req_date <<
                    dendl;
                return false;
            }
        ...
        namespace s3 {
            ...
            bool is_time_skew_ok(time_t t) {
                ...
                if (req_tp < cur_tp - RGW_AUTH_GRACE ||
                    ...
                    using ceph::operator<<;
                    dout(10) << "req_tp=" << req_tp << ", cur_tp=" << cur_tp <<
                        dendl;
                    return false;
                } return true;
                ...
                struct tm date_t;
                if (!parse_iso8601
                    (sview2cstr(date).data(), &date_t, nullptr, false)) {
                    return -EPERM;
                }
                ...
                if ((exp < 1) || (exp > 7 * 24 * 60 * 60)) {
                    dout(10) << "NOTICE: exp out of range, exp = " << exp <<
                        dendl;
                    return -EPERM;
                }
                ...
                if (now >= req_sec + exp) {
                    dout(10) << "NOTICE: now = " << now << ", req_sec = " <<
                        req_sec << ", exp = " << exp << dendl;
                    return -EPERM;
                }
            ...
            namespace {
                ...
                template < std::size_t ExpectedStrNum >
                    boost::container::small_vector < boost::string_view,
                    ExpectedStrNum > get_str_vec(const boost::string_view & str,
                                                 const char *const delims) {
                    boost::container::small_vector < boost::string_view,
                        ExpectedStrNum > str_vec;
            ...
            static inline int parse_v4_auth_header(const req_info & info,   /* in */
                                                   ...
                                                   boost::string_view & signature,  /* out */
                                                   boost::string_view & date) { /* out */
                boost::string_view input(info.env->
              ...
              for (const auto & s:get_str_vec < 4 > (input,
                    ...
                    if (parsed_pair) {
                        ...
                        dout(10) << "NOTICE: failed to parse auth header (s=" <<
                            s << ")" << dendl;
                        return -EINVAL;
                    }
                }

                static const std::array < boost::string_view,
                    3 > required_keys = {
                    "Credential",
                    "SignedHeaders",
                    "Signature"
              ...
              for (const auto & k:required_keys) {
                    if (kv.find(k) == std::end(kv)) {
                        dout(10) << "NOTICE: auth header missing key: " << k <<
                            dendl;
                        return -EINVAL;
                    }
                }
                ...
                if (!parse_iso8601(d, &t, NULL, false)) {
                    dout(10) << "error reading date via http_x_amz_date" <<
                        dendl;
                    return -EACCES;
                }
                date = d;
            ...
            int parse_v4_credentials(const req_info & info, /* in */
                                     ...
                                     boost::string_view & date, /* out */
                                     bool & using_qs) { /* out */
                const char *const http_auth =
                    info.env->get("HTTP_AUTHORIZATION");
            ...
            static inline std::string aws4_uri_recode(const boost::
                                                      string_view & src) {
                std::string decoded = url_decode(src);
                return aws4_uri_encode(decoded);
            }

            std::string get_v4_canonical_qs(const req_info & info,
                                            const bool using_qs) {
              ...
              for (const auto & s:get_str_vec < 5 > (*params,
                    ...
                    if (key == "X-Amz-Credential") {
                        ...
                        canonical_qs_map[aws4_uri_recode(key)] =
                            aws4_uri_recode(val);
                    }
                }
            ...
            boost::optional < std::string >
              ...
              for (const auto & token:get_str_vec < 5 > (signedheaders,
                                   ";"))
                {
                    ...
                    token_env.reserve(token.length() + std::strlen("HTTP_") +
                                      1);

                    std::transform(std::begin(token), std::end(token),
                                   std::back_inserter(token_env),
                                   [](const int c) {
                    ...
                    if (token_env == "HTTP_CONTENT_MD5" &&
                        !std::all_of(std::begin(token_value),
                                     std::end(token_value),
                                     is_base64_for_content_md5)) {
                        dout(0) <<
                            "NOTICE: bad content-md5 provided (not base64)" <<
                    ...
                    if (force_boto2_compat && using_qs && token == "host") {
                        boost::string_view port =
                            info.env->get("SERVER_PORT", "");
                        boost::string_view secure_port =
                            info.env->get("SERVER_PORT_SECURE", "");

                        if (!secure_port.empty()) {
                            if (secure_port != "443")
                                token_value.append(":", std::strlen(":"))
                                    .append(secure_port.data(),
                                            secure_port.length());
                        }
                        else if (!port.empty()) {
                            if (port != "80")
 ...
 */
            sha256_digest_t
                get_v4_canon_req_hash(CephContext * cct,
                                      ...
                                      const boost::
                                      string_view & request_payload_hash) {
                ldout(cct,
                ...
                ldout(cct, 10) << "canonical request = " << sanitize {
                canonical_req} <<dendl;
 ...
 */
            ...
            static inline std::vector < unsigned char >
                transform_secret_key(const boost::
                                     string_view & secret_access_key) {
                /* TODO(rzarzynski): switch to constexpr when C++14 becomes available. */
                static const std::initializer_list < unsigned char >AWS4 {
                'A', 'W', 'S', '4'};
 ...
 */
                                   ...
                                   const boost::string_view & secret_access_key)
            {
                ...
                std::tie(date, region, service) =
                    parse_cred_scope(credential_scope);

                const auto utfed_sec_key =
 ...
 */
            AWSEngine::VersionAbstractor::server_signature_t
                ...
                ldout(cct,
                      10) << "generated signature = " << signature << dendl;
...
/* AWSv4 completers begin. */
            std::pair < AWSv4ComplMulti::ChunkMeta, size_t /* consumed */  >
                ...
                const size_t data_length =
                    std::strtoull(metabuf, &data_field_end, 16);
                ...
                const auto signature =
                    signature_part.substr(eq_sign_pos + 1,
                                          data_sep_pos - 1 - eq_sign_pos);
                ...
                return std::make_pair(ChunkMeta(data_starts_in_stream,
                                                data_length,
                                                signature), semicolon_pos + 83);
            }

            std::string
                AWSv4ComplMulti::calc_chunk_signature(const std::
                                                      string & payload_hash)
                const {
                const auto string_to_sign = string_join_reserve("\n",
            ...
            size_t AWSv4ComplMulti::recv_body(char *const buf,
                ...
                if (chunk_meta.is_new_chunk_in_stream(stream_pos)) {
                    ...
                    if (stream_pos >= ChunkMeta::META_MAX_SIZE
                        && is_signature_mismatched()) {
                        throw rgw::io::Exception(ERR_SIGNATURE_NO_MATCH,
                                                 std::system_category());
                    }
                    ...
                    do {
                        ...
                        const size_t received =
                            io_base_t::recv_body(parsing_buf.data() + orig_size,
                                                 to_extract);
                        parsing_buf.resize(parsing_buf.size() -
                                           (to_extract - received));
                        if (received == 0) {
                            break;
                        }
                ...
                if (to_extract > 0 && parsing_buf.size() > 0) {
                    const auto data_len =
                        std::min(to_extract, parsing_buf.size());
                    const auto data_end_iter =
                        std::begin(parsing_buf) + data_len;
                ...
                while (to_extract > 0) {
                    const size_t received =
                        io_base_t::recv_body(buf + buf_pos, to_extract);
                    dout(30) << "AWSv4ComplMulti: to_extract=" << to_extract <<
                    ...
                    calc_hash_sha256_update_stream(sha256_hash, buf + buf_pos,
                                                   received);
            ...
            void AWSv4ComplMulti::modify_request_state(req_state * const s_rw) {
                ...
                if (!decoded_length) {
                    ...
                    if (s_rw->content_length < 0) {
                        ldout(cct,
                              10) << "negative AWSv4's content length, aborting"
                            << dendl;
                        throw - EINVAL;
                    }
                }
                ...
                AWS_AUTHv4_IO(s_rw)->add_filter(std::static_pointer_cast <
                                                io_base_t >
                                                (shared_from_this()));
            }
            ...
            rgw::auth::Completer::cmplptr_t
                ...
                const auto signing_key =
                    rgw::auth::s3::get_v4_signing_key(s->cct, credential_scope,
                                                      *secret_key);
            ...
            size_t AWSv4ComplSingle::recv_body(char *const buf,
                                               const size_t max) {
