...
namespace crimson::os {
    ...
    ThreadPool::ThreadPool(size_t n_threads,
                           size_t queue_sz,
                           const std::optional < seastar::resource::cpuset >
                           &cpus)
    :n_threads(n_threads), queue_size {
    round_up_to(queue_sz, seastar::smp::count)}, pending_queues(n_threads) {
        auto queue_max_wait =
        ...
        for (size_t i = 0; i < n_threads; i++) {
            threads.emplace_back([this, cpus, queue_max_wait, i] {
                                 ...
                                 pin(*cpus);}
                                 block_sighup();
                                 (void)pthread_setname_np(pthread_self(),
                                                          "alien-store-tp");
    ...
    seastar::future <> ThreadPool::stop() {
        return submit_queue.stop().then([this] {
          stopping = true; for (auto & q:pending_queues)
                                        {
                                        q.stop();}
                                        }
        ) ;
