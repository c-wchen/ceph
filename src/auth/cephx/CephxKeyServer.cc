                                       ...
                                       CryptoKey & secret) const const
{
    ...
    if (riter == secrets.secrets.end()) {
        ...
        for (map < uint64_t, ExpiringCryptoKey >::const_iterator iter =
             secrets.secrets.begin(); iter != secrets.secrets.end(); ++iter)
            ldout(cct,
                  30) << " id " << iter->first << " " << iter->second << dendl;
        return false;
    }
...
bool KeyServerData::get_auth(const EntityName & name, EntityAuth & auth) constconst
{
...
bool KeyServerData::get_secret(const EntityName & name, CryptoKey & secret) constconst
{
    map < EntityName, EntityAuth >::const_iterator iter = secrets.find(name);
    if (iter != secrets.end()) {
...
bool KeyServerData::get_caps(CephContext * cct, const EntityName & name,
                             const string & type,
                             AuthCapsInfo & caps_info) constconst
{
    ...
    if (iter != secrets.end()) {
        ldout(cct,
              10) << "get_secret: num of caps=" << iter->second.caps.
            size() << dendl;
        map < string, bufferlist >::const_iterator capsiter =
            iter->second.caps.find(type);
...
:  cct(cct_), data(extra_secrets), lock("KeyServer::lock")
{
}
...
void KeyServer::_dump_rotating_secrets()
{
    ...
    for (map < uint32_t, RotatingSecrets >::iterator iter =
         data.rotating_secrets.begin(); iter != data.rotating_secrets.end();
         ++iter) {
        RotatingSecrets & key = iter->second;
        for (map < uint64_t, ExpiringCryptoKey >::iterator mapiter =
             key.secrets.begin(); mapiter != key.secrets.end(); ++mapiter)
...
int KeyServer::_rotate_secret(uint32_t service_id)
{
    ...
    double ttl =
        service_id ==
        CEPH_ENTITY_TYPE_AUTH ? cct->_conf->auth_mon_ticket_ttl : cct->_conf->
    ...
    while (r.need_new_secrets(now)) {
        ...
        ldout(cct,
              10) << "_rotate_secret adding " <<
            ceph_entity_type_name(service_id) << dendl;
        ldout(cct,
              30) << "_rotate_secret adding " <<
            ceph_entity_type_name(service_id)
            << " id " << secret_id << " " << ek << dendl;
        added++;
    }
    return added;
}

bool KeyServer::get_secret(const EntityName & name, CryptoKey & secret) constconst
{
    Mutex::Locker l(lock);
    return data.get_secret(name, secret);
}

bool KeyServer::get_auth(const EntityName & name, EntityAuth & auth) constconst
{
    Mutex::Locker l(lock);
...
bool KeyServer::contains(const EntityName & name) constconst
{
    Mutex::Locker l(lock);
...
int KeyServer::encode_secrets(Formatter * f, stringstream * ds) const const
{
    Mutex::Locker l(lock);
    map < EntityName, EntityAuth >::const_iterator mapiter =
        data.secrets_begin();
