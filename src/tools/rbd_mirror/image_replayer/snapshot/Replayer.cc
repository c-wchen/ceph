            ...
            namespace snapshot {
                ...
                namespace {
                    ...
                    double round_to_two_places(double value) {
                        ...
                        std::pair < uint64_t,
                        librbd::SnapInfo * >get_newest_mirror_snapshot(I *
                                                                       image_ctx)
                    {
                        for (auto snap_info_it = image_ctx->snap_info.rbegin();
                             snap_info_it != image_ctx->snap_info.rend();
                             ++snap_info_it) {
                            const auto & snap_ns =
                                snap_info_it->second.snap_namespace;
                            auto mirror_ns =
                                std::get_if <
                            ...
                            snap_info_it->first, &snap_info_it->second};
                        } return {
                        CEPH_NOSNAP, nullptr};
                    }
                ...
                template < typename I >
                    struct Replayer <I >::C_UpdateWatchCtx:public librbd::
                    UpdateWatchCtx {
                    Replayer < I > *replayer;

                    C_UpdateWatchCtx(Replayer < I >
                ...
                template < typename I >
                    struct Replayer <I >::DeepCopyHandler:public librbd::
                    deep_copy::Handler {
                    Replayer *replayer;
                    ...
                    int update_progress(uint64_t object_number,
                        ...
                        replayer->handle_copy_image_progress(object_number,
                                                             object_count);
                        return 0;
                    }
                };
                ...
                template < typename I >
                    Replayer < I >::Replayer(Threads < I > *threads,
                                             InstanceWatcher < I >
                                             *instance_watcher,
                                             const std::
              ...
              :    m_threads(threads), m_instance_watcher(instance_watcher),
                ...
                template < typename I >
                    ...
                    int r =
                        m_pool_meta_cache->
                        get_remote_pool_meta(m_state_builder->remote_image_ctx->
                                             md_ctx.get_id(),
                                             &remote_pool_meta);
                ...
                template < typename I >
                    ...
                    if (state == STATE_REPLAYING) {
                        ...
                        m_instance_watcher->
                            cancel_sync_request(m_state_builder->
                                                local_image_ctx->id);

                        // TODO interrupt snapshot copy and image copy state machines even if remote
                        // cluster is unreachable
                        dout(10) <<
                            "shut down pending on completion of snapshot replay"
                            << dendl;
                        return;
                    }
                    locker.unlock();
                ...
                template < typename I >
                    bool Replayer <
                    I >::get_replay_status(std::string * description,
                                           Context * on_finish) {
                    dout(10) << dendl;
                    ...
                    auto[local_snap_id, local_snap_info] =
                        get_newest_mirror_snapshot(m_state_builder->
                                                   local_image_ctx);
                    ...
                    if (remote_snap_info == nullptr) {
                        ...
                        derr << "remote image does not contain mirror snapshots"
                            << dendl;
                        on_finish->complete(-EAGAIN);
                        return false;
                    }
                    ...
                    if (m_perf_counters) {
                        m_perf_counters->
                            tset(l_rbd_mirror_snapshot_remote_timestamp,
                                 remote_snap_info->timestamp);
                    }

                    auto matching_remote_snap_id =
                        util::compute_remote_snap_id(m_state_builder->
                                                     local_image_ctx->
                                                     image_lock,
                                                     m_state_builder->
                                                     local_image_ctx->snap_info,
                                                     local_snap_id,
                                                     m_state_builder->
                                                     remote_mirror_uuid);
                    auto matching_remote_snap_it =
                        m_state_builder->remote_image_ctx->snap_info.
                    ...
                    if (matching_remote_snap_id != CEPH_NOSNAP
                        ...
                        if (m_perf_counters) {
                            m_perf_counters->
                                tset(l_rbd_mirror_snapshot_local_timestamp,
                    ...
                    root_obj["bytes_per_second"] =
                        round_to_two_places(bytes_per_second);

                    auto bytes_per_snapshot =
                        boost::accumulators::rolling_mean(m_bytes_per_snapshot);
                    root_obj["bytes_per_snapshot"] =
                        round_to_two_places(bytes_per_snapshot);

                    root_obj["last_snapshot_sync_seconds"] =
                        m_last_snapshot_sync_seconds;
                    root_obj["last_snapshot_bytes"] = m_last_snapshot_bytes;

                    auto pending_bytes =
                        bytes_per_snapshot * m_pending_snapshots;
                    if (bytes_per_second > 0 && m_pending_snapshots > 0) {
                        std::uint64_t seconds_until_synced =
                            round_to_two_places(pending_bytes /
                                                bytes_per_second);
                        if (seconds_until_synced >= std::numeric_limits <
                            uint64_t >::max()) {
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_load_local_image_meta > (this);
                ...
                template < typename I >
                    ...
                    if (r < 0 && r != -ENOENT) {
                        derr << "failed to load local image-meta: " <<
                ...
                template < typename I >
                    ...
                    if (!m_state_builder->local_image_ctx->state->
                        is_refresh_required()) {
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_refresh_local_image > (this);
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        derr << "failed to refresh local image: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I >
                    ...
                    if (!m_state_builder->remote_image_ctx->state->
                        is_refresh_required()) {
                        std::unique_lock locker {
                        m_lock};
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_refresh_remote_image > (this);
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        derr << "failed to refresh remote image: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I >
                    void Replayer <
                    I >::scan_local_mirror_snapshots(std::unique_lock <
                                                     ceph::mutex > *locker) {
                    ...
                    for (auto snap_info_it = local_image_ctx->snap_info.begin();
                         snap_info_it != local_image_ctx->snap_info.end();
                         ++snap_info_it) {
                        const auto & snap_ns =
                            snap_info_it->second.snap_namespace;
                        auto mirror_ns =
                        ...
                        dout(15) << "local mirror snapshot: id=" <<
                            snap_info_it->
                            first << ", " << "mirror_ns=" << *mirror_ns <<
                            dendl;
                        ...
                        if (mirror_ns->is_non_primary()) {
                            ...
                            if (mirror_ns->complete) {
                                ...
                                derr << "incomplete local primary snapshot" <<
                                    dendl;
                            ...
                            derr << "unknown local mirror snapshot state" <<
                                dendl;
                            handle_replay_complete(locker, -EINVAL,
                    ...
                    if (!prune_snap_ids.empty()) {
                        ...
                        dout(5) << "pruning unused non-primary snapshot " <<
                            prune_snap_id << dendl;
                    ...
                    if (m_local_snap_id_start > 0
                        ...
                        dout(10) << "found local mirror snapshot: "
                            ...
                            << ", " << "local_snap_id_end=" <<
                            m_local_snap_id_end << ", " << "local_snap_ns=" <<
                            m_local_mirror_snap_ns << dendl;
                        if (!m_local_mirror_snap_ns.is_primary()
                            ...
                            m_remote_snap_id_start =
                                m_local_mirror_snap_ns.primary_snap_id;
                        }
                    }
                ...
                template < typename I >
                    void Replayer <
                    I >::scan_remote_mirror_snapshots(std::unique_lock <
                                                      ceph::mutex > *locker) {
                    dout(10) << dendl;
                    ...
                    std::shared_lock image_locker {
                    remote_image_ctx->image_lock};
                    for (auto snap_info_it =
                         remote_image_ctx->snap_info.begin();
                         snap_info_it != remote_image_ctx->snap_info.end();
                         ++snap_info_it) {
                        const auto & snap_ns =
                            snap_info_it->second.snap_namespace;
                        auto mirror_ns =
                        ...
                        dout(15) << "remote mirror snapshot: id=" <<
                            snap_info_it->
                            first << ", " << "mirror_ns=" << *mirror_ns <<
                            dendl;
                        ...
                        if (!mirror_ns->is_primary()
                            ...
                            derr << "unknown remote mirror snapshot state" <<
                                dendl;
                            handle_replay_complete(locker, -EINVAL,
                                                   "invalid remote mirror snapshot state");
                            return;
                        }
                        else if (mirror_ns->mirror_peer_uuids.
                                 count(m_remote_mirror_peer_uuid) == 0) {
                            dout(15) <<
                        ...
                        if (m_local_snap_id_start > 0
                            ...
                            if (m_local_mirror_snap_ns.is_non_primary()) {
                                ...
                                ceph_assert(m_local_mirror_snap_ns.
                                            primary_mirror_uuid ==
                                            m_state_builder->
                                            remote_mirror_uuid);
                                ...
                                if (mirror_ns->state ==
                                    ...
                                    && mirror_ns->primary_mirror_uuid ==
                                    m_local_mirror_uuid
                                    && mirror_ns->primary_snap_id ==
                                    m_local_snap_id_start) {
                                    dout(10) <<
                                        "located matching demotion snapshot: "
                                        << "remote_snap_id=" << remote_snap_id
                                        << ", " << "local_snap_id=" <<
                                        m_local_snap_id_start << dendl;
                                    m_remote_snap_id_start = remote_snap_id;
                                    split_brain = false;
                                    continue;
                                }
                                    ...
                                    dout(15) << "skipping remote snapshot " <<
                                        remote_snap_id << " " <<
                            ...
                            dout(15) << "skipping non-primary remote snapshot"
                                << dendl;
                            continue;
                        }
                    ...
                    if (m_remote_snap_id_start != 0 &&
                        ...
                        count(m_remote_snap_id_start) == 0) {
                    ...
                    if (!split_brain) {
                        ...
                        if (!unlink_snap_ids.empty()) {
                            ...
                            dout(10) << "unlinking from remote snapshot " <<
                                remote_snap_id << dendl;
                        ...
                        if (m_remote_snap_id_end != CEPH_NOSNAP) {
                            ...
                            if (m_remote_mirror_snap_ns.complete) {
                                ...
                                if (m_local_snap_id_end != CEPH_NOSNAP &&
                                    ...
                                    dout(10) <<
                                        "local image contains in-progress mirror snapshot"
                                        << dendl;
                    ...
                    if (m_image_updated) {
                        ...
                        dout(10) <<
                            "restarting snapshot scan due to remote update notification"
                            << dendl;
                    ...
                    if (is_replay_interrupted(locker)) {
                        ...
                        derr <<
                            ...
                            "local_snap_id_start=" << m_local_snap_id_start <<
                            ", " << "local_snap_ns=" << m_local_mirror_snap_ns
                            << dendl;
                        ...
                        handle_replay_complete(locker, -EREMOTEIO,
                                               "remote image demoted");
                        return;
                    }

                    dout(10) <<
                        "all remote snapshots synced: idling waiting for new snapshot"
                        << dendl;
                ...
                template < typename I >
                    void Replayer <
                    I >::prune_non_primary_snapshot(uint64_t snap_id) {
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_prune_non_primary_snapshot >
                        (this);
                    local_image_ctx->operations->snap_remove(snap_namespace,
                                                             snap_name, ctx);
                }

                template < typename I >
                    void Replayer <
                    I >::handle_prune_non_primary_snapshot(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0 && r != -ENOENT) {
                        derr << "failed to prune non-primary snapshot: " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I > void Replayer < I >::copy_snapshots() {
                    dout(10) << "remote_snap_id_start=" <<
                        m_remote_snap_id_start << ", " << "remote_snap_id_end="
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_copy_snapshots > (this);
                    auto req =
                        librbd::deep_copy::SnapshotCopyRequest <
                        I >::create(m_state_builder->remote_image_ctx,
                                    m_state_builder->local_image_ctx,
                                    m_remote_snap_id_start,
                                    m_remote_snap_id_end, m_local_snap_id_start,
                                    false, m_threads->work_queue,
                ...
                template < typename I >
                    ...
                    dout(10) << "remote_snap_id_start=" <<
                        ...
                        << m_remote_snap_id_end << ", " <<
                        "local_snap_id_start=" << m_local_snap_id_start << ", "
                        << "snap_seqs=" << m_local_mirror_snap_ns.
                        snap_seqs << dendl;
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_get_remote_image_state > (this);
                    auto req =
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_get_local_image_state > (this);
                    auto req =
                        librbd::mirror::snapshot::GetImageStateRequest <
                        I >::create(m_state_builder->local_image_ctx,
                                    m_local_snap_id_end,
                                    &m_image_state, ctx);
                    req->send();
                }
                ...
                template < typename I >
                    ...
                    if (m_local_snap_id_start > 0) {
                        ...
                        auto local_snap_info_it =
                            local_image_ctx->snap_info.
                            find(m_local_snap_id_start);
                        if (local_snap_info_it ==
                            local_image_ctx->snap_info.end()) {
                            local_image_locker.unlock();

                            derr << "failed to locate local snapshot " <<
                                m_local_snap_id_start << dendl;
                            handle_replay_complete(-ENOENT,
...
remote_image_ctx->snap_info) {
                            ...
                            if (mirror_ns->is_demoted()
                                ...
                                auto local_snap_id_it =
                                    local_image_ctx->snap_ids.
                                    find({ remote_snap_info.snap_namespace,
                                         remote_snap_info.name }
                                );
                                if (local_snap_id_it !=
                                    local_image_ctx->snap_ids.end()) {
                                    local_snap_id = local_snap_id_it->second;
                                }
                            }
                            else {
                                auto snap_seq_it =
                                    mirror_ns->snap_seqs.find(remote_snap_id);
                                if (snap_seq_it != mirror_ns->snap_seqs.end()) {
                                    local_snap_id = snap_seq_it->second;
                                }
                            }

                            if (m_local_mirror_snap_ns.snap_seqs.
                                count(remote_snap_id) == 0
                                && local_snap_id != CEPH_NOSNAP) {
                                dout(15) << "mapping remote snapshot " <<
                                    remote_snap_id << " to " <<
                                    "local snapshot " << local_snap_id << dendl;
                                m_local_mirror_snap_ns.
                                    snap_seqs[remote_snap_id] = local_snap_id;
                            }
                        }
                    }

                    dout(10) << "demoted=" << m_remote_mirror_snap_ns.
                        is_demoted() << ", " << "primary_mirror_uuid=" <<
                        m_state_builder->
                        remote_mirror_uuid << ", " << "primary_snap_id=" <<
                        m_remote_snap_id_end << ", " << "snap_seqs=" <<
                        m_local_mirror_snap_ns.snap_seqs << dendl;

                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_create_non_primary_snapshot >
                        (this);
                    auto req =
                        librbd::mirror::snapshot::CreateNonPrimaryRequest <
                        I >::create(local_image_ctx,
                                    m_remote_mirror_snap_ns.is_demoted(),
                                    m_state_builder->remote_mirror_uuid,
                                    m_remote_snap_id_end,
                                    m_local_mirror_snap_ns.snap_seqs,
                                    m_image_state, &m_local_snap_id_end, ctx);
                    req->send();
                }

                template < typename I >
                    void Replayer <
                    I >::handle_create_non_primary_snapshot(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to create local mirror snapshot: " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(r,
                    ...
                    dout(15) << "local_snap_id_end=" << m_local_snap_id_end <<
                        dendl;
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_update_mirror_image_state >
                        (this);
                    auto req =
                        librbd::mirror::ImageStateUpdateRequest <
                        I >::create(m_state_builder->local_image_ctx->md_ctx,
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_update_mirror_image_state(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to update local mirror image state: " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I > void Replayer < I >::request_sync() {
                    ...
                    auto ctx =
                        create_async_context_callback(m_threads->work_queue,
                                                      create_context_callback <
                                                      Replayer < I >,
                                                      &Replayer <
                                                      I >::handle_request_sync >
                                                      (this));
                    m_instance_watcher->notify_sync_request(m_state_builder->
                                                            local_image_ctx->id,
                                                            ctx);
                }
                ...
                template < typename I >
                    ...
                    if (is_replay_interrupted(&locker)) {
                        ...
                        handle_replay_complete(&locker, r,
                                               "image-sync canceled");
                        return;
                    }
                    else if (r < 0) {
                        derr << "failed to request image-sync: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_complete(&locker, r,
                ...
                template < typename I > void Replayer < I >::copy_image() {
                    dout(10) << "remote_snap_id_start=" <<
                        ...
                        << "last_copied_object_number=" <<
                        m_local_mirror_snap_ns.
                        last_copied_object_number << ", " << "snap_seqs=" <<
                        m_local_mirror_snap_ns.snap_seqs << dendl;
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_copy_image > (this);
                    auto req =
                        ...
                        I >::create(m_state_builder->remote_image_ctx,
                                    ...
                                    (m_local_mirror_snap_ns.
                                     last_copied_object_number >
                                     0 ? librbd::deep_copy::ObjectNumber {
                                     m_local_mirror_snap_ns.
                                     last_copied_object_number
                                     } : librbd::deep_copy::ObjectNumber { }),
                                    m_local_mirror_snap_ns.snap_seqs,
                                    m_deep_copy_handler, ctx);
                    req->send();
                }
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        derr << "failed to copy remote image to local image: "
                            << cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(r,
...

                    {
                        ...
                        utime_t duration =
                            ceph_clock_now() - m_snapshot_replay_start;
                        ...
                        if (g_snapshot_perf_counters) {
                            g_snapshot_perf_counters->
                                inc(l_rbd_mirror_snapshot_sync_bytes,
                                    m_snapshot_bytes);
                            g_snapshot_perf_counters->
                                inc(l_rbd_mirror_snapshot_snapshots);
                            g_snapshot_perf_counters->
                                tinc(l_rbd_mirror_snapshot_sync_time, duration);
                        }
                        if (m_perf_counters) {
                            m_perf_counters->
                                inc(l_rbd_mirror_snapshot_sync_bytes,
                            ...
                            m_perf_counters->
                                tinc(l_rbd_mirror_snapshot_sync_time, duration);
                            m_perf_counters->
                                tset(l_rbd_mirror_snapshot_last_sync_time,
                                     duration);
                            m_perf_counters->
                                set(l_rbd_mirror_snapshot_last_sync_bytes,
                                    m_snapshot_bytes);
                        }
                    }
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_copy_image_progress(uint64_t object_number,
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_copy_image_read(uint64_t bytes_read) {
                ...
                template < typename I > void Replayer < I >::apply_image_state() {
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_apply_image_state > (this);
                    auto req =
                        ApplyImageStateRequest <
                        I >::create(m_local_mirror_uuid,
                                    m_state_builder->remote_mirror_uuid,
                                    m_state_builder->local_image_ctx,
                                    m_state_builder->remote_image_ctx,
                                    m_image_state, ctx);
                ...
                template < typename I >
                    void Replayer <
                    I >::update_non_primary_snapshot(bool complete) {
                    ...
                    librbd::cls_client::
                        mirror_image_snapshot_set_copy_progress(&op,
                                                                m_local_snap_id_end,
                                                                m_local_mirror_snap_ns.
                                                                complete,
                                                                m_local_mirror_snap_ns.
                    ...
                    auto ctx =
                        new C_TrackedOp(m_in_flight_op_tracker,
                                        new
                                        LambdaContext([this, complete] (int r){
                                                      handle_update_non_primary_snapshot
                                                      (complete, r);}));
                    auto aio_comp = create_rados_callback(ctx);
                    int r =
                        m_state_builder->local_image_ctx->md_ctx.
                        aio_operate(m_state_builder->local_image_ctx->
                                    header_oid, aio_comp, &op);
                    ceph_assert(r == 0);
                    aio_comp->release();
                }
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_update_non_primary_snapshot(bool complete,
                                                            int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to update local snapshot progress: " <<
                            cpp_strerror(r)
                            << dendl;
                        if (complete) {
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_notify_image_update > (this);
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        derr << "failed to notify local image update: " <<
                            cpp_strerror(r) << dendl;
                    }
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_unlink_peer > (this);
                    auto req =
                ...
                template < typename I > void Replayer < I >::finish_sync() {
...

                    {
                        ...
                        if (m_sync_in_progress) {
                            ...
                            m_instance_watcher->
                                notify_sync_complete(m_state_builder->
                                                     local_image_ctx->id);
                        }
                    }
                ...
                template < typename I >
                    ...
                    int r =
                        m_state_builder->local_image_ctx->state->
                        register_update_watcher(m_update_watch_ctx,
                                                &m_local_update_watcher_handle);
                    auto ctx =
                        create_context_callback < Replayer < I >,
                        &Replayer < I >::handle_register_local_update_watcher >
                        (this);
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_register_local_update_watcher(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to register local update watcher: " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I >
                    ...
                    int r =
                        m_state_builder->remote_image_ctx->state->
                        register_update_watcher(m_update_watch_ctx,
                                                &m_remote_update_watcher_handle);
                    auto ctx =
                        create_context_callback < Replayer < I >,
                        &Replayer < I >::handle_register_remote_update_watcher >
                        (this);
                    m_threads->work_queue->queue(ctx, r);
                }
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_register_remote_update_watcher(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to register remote update watcher: " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer <
                        I >::handle_unregister_remote_update_watcher > (this);
                    m_state_builder->remote_image_ctx->state->
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_unregister_remote_update_watcher(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to unregister remote update watcher: "
                            << cpp_strerror(r)
                            << dendl;
                    }
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer <
                        I >::handle_unregister_local_update_watcher > (this);
                    m_state_builder->local_image_ctx->state->
                        unregister_update_watcher(m_local_update_watcher_handle,
                                                  ctx);
                }

                template < typename I >
                    void Replayer <
                    I >::handle_unregister_local_update_watcher(int r) {
                    dout(10) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to unregister local update watcher: " <<
                            cpp_strerror(r)
                            << dendl;
                    }
                ...
                template < typename I >
                    ...
                    if (m_state == STATE_REPLAYING) {
                        dout(15) << "flagging snapshot rescan required" <<
                            dendl;
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_replay_complete(std::unique_lock < ceph::mutex >
                                                *locker, int r,
                                                const std::
                                                string & description) {
                    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
                ...
                template < typename I >
                    bool Replayer <
                    I >::is_replay_interrupted(std::unique_lock < ceph::mutex >
                                               *locker) {
                ...
                template < typename I >
                    ...
                    auto cct =
                        static_cast <
                        CephContext * >(m_state_builder->local_image_ctx->cct);
                    auto prio =
                    ...
                    std::string labels =
                        ceph::perf_counters::
                        key_create("rbd_mirror_snapshot_image", { {
                                   "pool",
                                   local_image_ctx->md_ctx.get_pool_name()}, {
                                   "namespace",
                                   local_image_ctx->md_ctx.get_namespace()}, {
                                   "image", local_image_ctx->name}});

                    PerfCountersBuilder plb(g_ceph_context, labels,
                                            l_rbd_mirror_snapshot_first,
                                            l_rbd_mirror_snapshot_last);
                    plb.add_u64_counter(l_rbd_mirror_snapshot_snapshots,
                                        "snapshots",
                                        "Number of snapshots synced", nullptr,
                                        prio);
                    plb.add_time_avg(l_rbd_mirror_snapshot_sync_time,
                                     "sync_time", "Average sync time", nullptr,
                                     prio);
                    plb.add_u64_counter(l_rbd_mirror_snapshot_sync_bytes,
                                        "sync_bytes", "Total bytes synced",
                                        nullptr, prio, unit_t(UNIT_BYTES));
                    plb.add_time(l_rbd_mirror_snapshot_remote_timestamp,
                                 "remote_timestamp",
                                 "Timestamp of the remote snapshot", nullptr,
                                 prio);
                    plb.add_time(l_rbd_mirror_snapshot_local_timestamp,
                                 "local_timestamp",
                                 "Timestamp of the local snapshot", nullptr,
                                 prio);
                    plb.add_time(l_rbd_mirror_snapshot_last_sync_time,
                                 "last_sync_time",
                                 "Time taken to sync the last snapshot",
                                 nullptr, prio);
                    plb.add_u64(l_rbd_mirror_snapshot_last_sync_bytes,
                                "last_sync_bytes",
                                "Bytes synced for the last snapshot", nullptr,
                                prio, unit_t(UNIT_BYTES));
...
template class rbd::mirror::image_replayer::snapshot::Replayer <
    librbd::ImageCtx >;
