...
static int list_user(const DoutPrefixProvider *dpp, DBOpInfo &op,
                     sqlite3_stmt *stmt)
{
    if (!stmt) {
        return -1;
    ...
    op.user.uinfo.user_id.tenant =
        (const char *)sqlite3_column_text(stmt, Tenant);
    ...
    op.user.uinfo.user_email =
        (const char *)sqlite3_column_text(stmt, UserEmail);
    ...
    SQL_DECODE_BLOB_PARAM(dpp, stmt, AccessKeys, op.user.uinfo.access_keys,
                          sdb);
    ...
    SQL_DECODE_BLOB_PARAM(dpp, stmt, PlacementTags,
                          op.user.uinfo.placement_tags, sdb);

    SQL_DECODE_BLOB_PARAM(dpp, stmt, BucketQuota,
                          op.user.uinfo.quota.bucket_quota, sdb);
    SQL_DECODE_BLOB_PARAM(dpp, stmt, TempURLKeys, op.user.uinfo.temp_url_keys,
                          sdb);
    SQL_DECODE_BLOB_PARAM(dpp, stmt, UserQuota, op.user.uinfo.quota.user_quota,
                          sdb);
...
static int list_bucket(const DoutPrefixProvider *dpp, DBOpInfo &op,
                       sqlite3_stmt *stmt)
{
    if (!stmt) {
        return -1;
    ...
    op.bucket.ent.bucket.name =
        (const char *)sqlite3_column_text(stmt, BucketName);
    op.bucket.ent.bucket.tenant =
        (const char *)sqlite3_column_text(stmt, Bucket_Tenant);
    op.bucket.ent.bucket.marker =
        (const char *)sqlite3_column_text(stmt, Marker);
    ...
    SQL_DECODE_BLOB_PARAM(dpp, stmt, CreationTime, op.bucket.ent.creation_time,
                          sdb);
    ...
    if (op.name == "GetBucket") {
        op.bucket.info.owner.ns =
            (const char *)sqlite3_column_text(stmt, Bucket_User_NS);
    }

    op.bucket.info.flags = sqlite3_column_int(stmt, Flags);
    op.bucket.info.zonegroup =
        (const char *)sqlite3_column_text(stmt, Zonegroup);
    ...
    SQL_DECODE_BLOB_PARAM(dpp, stmt, WebsiteConf, op.bucket.info.website_conf,
                          sdb);
    ...
    SQL_DECODE_BLOB_PARAM(dpp, stmt, MdsearchConfig,
                          op.bucket.info.mdsearch_config, sdb);
    ...
    SQL_DECODE_BLOB_PARAM(dpp, stmt, SyncPolicyInfoGroups,
                          op.bucket.info.sync_policy, sdb);
...
static int list_object(const DoutPrefixProvider *dpp, DBOpInfo &op,
                       sqlite3_stmt *stmt)
{
    if (!stmt) {
        return -1;
    ...
    op.obj.state.obj.key.name =
        (const char *)sqlite3_column_text(stmt, ObjName);
    ...
    op.obj.storage_class =
        (const char *)sqlite3_column_text(stmt, StorageClass);
    ...
    op.obj.state.shadow_obj =
        (const char *)sqlite3_column_text(stmt, ShadowObj);
    ...
    op.obj.tail_instance =
        (const char *)sqlite3_column_text(stmt, TailInstance);
...
static int get_objectdata(const DoutPrefixProvider *dpp, DBOpInfo &op,
                          sqlite3_stmt *stmt)
{
    if (!stmt) {
        return -1;
...
static int list_lc_entry(const DoutPrefixProvider *dpp, DBOpInfo &op,
                         sqlite3_stmt *stmt)
{
    if (!stmt) {
        return -1;
    ...
    op.lc_entry.entry.
    set_bucket((const char *)sqlite3_column_text(stmt, LCEntryBucketName));
    op.lc_entry.entry.
    set_start_time(sqlite3_column_int(stmt, LCEntryStartTime));
...
static int list_lc_head(const DoutPrefixProvider *dpp, DBOpInfo &op,
                        sqlite3_stmt *stmt)
{
    if (!stmt) {
        return -1;
    ...
    op.lc_head.head.
    set_marker((const char *)sqlite3_column_text(stmt, LCHeadMarker));
...
int SQLiteDB::InitializeDBOps(const DoutPrefixProvider *dpp)
{
    ...
    dbops.GetUser =
        make_shared < SQLGetUser > (&this->db, this->getDBname(), cct);
    dbops.InsertBucket =
    ...
    dbops.GetBucket =
        make_shared < SQLGetBucket > (&this->db, this->getDBname(), cct);
    ...
    dbops.GetLCHead =
        make_shared < SQLGetLCHead > (&this->db, this->getDBname(), cct);
...
void *SQLiteDB::openDB(const DoutPrefixProvider *dpp)
{
    ...
    if (rc) {
        ...
        ldpp_dout(dpp,
                  0) << "Opened database(" << dbname << ") successfully" <<
                     dendl;
    }
...
int SQLiteDB::Step(const DoutPrefixProvider *dpp, DBOpInfo &op,
                   sqlite3_stmt *stmt,
                   int (*cbk)(const DoutPrefixProvider *dpp, DBOpInfo &op,
                              sqlite3_stmt *stmt))
{
    int ret = -1;
...
int SQLiteDB::createUserTable(const DoutPrefixProvider *dpp,
                              DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::createBucketTable(const DoutPrefixProvider *dpp,
                                DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::createObjectTable(const DoutPrefixProvider *dpp,
                                DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::createObjectTableTrigger(const DoutPrefixProvider *dpp,
                                       DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::createObjectView(const DoutPrefixProvider *dpp,
                               DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::createQuotaTable(const DoutPrefixProvider *dpp,
                               DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::createObjectDataTable(const DoutPrefixProvider *dpp,
                                    DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteUserTable(const DoutPrefixProvider *dpp,
                              DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteBucketTable(const DoutPrefixProvider *dpp,
                                DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteObjectTable(const DoutPrefixProvider *dpp,
                                DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteObjectDataTable(const DoutPrefixProvider *dpp,
                                    DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteQuotaTable(const DoutPrefixProvider *dpp,
                               DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteLCEntryTable(const DoutPrefixProvider *dpp,
                                 DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::DeleteLCHeadTable(const DoutPrefixProvider *dpp,
                                DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::ListAllBuckets(const DoutPrefixProvider *dpp,
                             DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLiteDB::ListAllObjects(const DoutPrefixProvider *dpp,
                             DBOpParams *params)
{
    int ret = -1;
    string schema;
...
int SQLObjectOp::InitializeObjectOps(string db_name,
                                     const DoutPrefixProvider *dpp)
{
    ...
    ListBucketObjects =
        make_shared < SQLListBucketObjects > (sdb, db_name, cct);
    ListVersionedObjects =
        make_shared < SQLListVersionedObjects > (sdb, db_name, cct);
...
int SQLInsertUser::Prepare(const DoutPrefixProvider *dpp,
...
int SQLInsertUser::Bind(const DoutPrefixProvider *dpp,
                        struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.user.uinfo.user_id.tenant.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.user_id, sdb);
    SQL_BIND_TEXT(dpp, stmt, index, params->op.user.uinfo.user_id.id.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.ns, sdb);
    SQL_BIND_TEXT(dpp, stmt, index, params->op.user.uinfo.user_id.ns.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.display_name, sdb);
    SQL_BIND_TEXT(dpp, stmt, index, params->op.user.uinfo.display_name.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.user_email, sdb);
    SQL_BIND_TEXT(dpp, stmt, index, params->op.user.uinfo.user_email.c_str(),
                  sdb);
    ...
    if (!params->op.user.uinfo.access_keys.empty()) {
        ...
        SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.access_keys_secret,
                       sdb);
        SQL_BIND_TEXT(dpp, stmt, index, key.c_str(), sdb);

    }
    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.access_keys, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.user.uinfo.access_keys,
                          sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.swift_keys, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.user.uinfo.swift_keys,
                          sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.subusers, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.user.uinfo.subusers,
                          sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.user.uinfo.default_placement.name.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.placement_storage_class,
                   sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.user.uinfo.default_placement.storage_class.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.placement_tags, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index,
                          params->op.user.uinfo.placement_tags, sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.bucket_quota, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index,
                          params->op.user.uinfo.quota.bucket_quota, sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.temp_url_keys, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.user.uinfo.temp_url_keys,
                          sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.user_quota, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index,
                          params->op.user.uinfo.quota.user_quota, sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.user.user_version.tag.c_str(),
                  sdb);
...
int SQLInsertUser::Execute(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int ret = -1;
...
int SQLRemoveUser::Prepare(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLRemoveUser::Bind(const DoutPrefixProvider *dpp,
                        struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLRemoveUser::Execute(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int ret = -1;
...
int SQLGetUser::Prepare(const DoutPrefixProvider *dpp,
                        struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLGetUser::Bind(const DoutPrefixProvider *dpp, struct DBOpParams *params)
{
    ...
    if (params->op.query_str == "email") {
        SQL_BIND_INDEX(dpp, email_stmt, index, p_params.op.user.user_email,
                       sdb);
        SQL_BIND_TEXT(dpp, email_stmt, index,
                      params->op.user.uinfo.user_email.c_str(), sdb);
    } else if (params->op.query_str == "access_key") {
        if (!params->op.user.uinfo.access_keys.empty()) {
            string access_key;
            map < string, RGWAccessKey >::const_iterator it =
            ...
            SQL_BIND_INDEX(dpp, ak_stmt, index, p_params.op.user.access_keys_id,
                           sdb);
            SQL_BIND_TEXT(dpp, ak_stmt, index, access_key.c_str(), sdb);
        }
    } else if (params->op.query_str == "user_id") {
        SQL_BIND_INDEX(dpp, userid_stmt, index, p_params.op.user.user_id, sdb);
        SQL_BIND_TEXT(dpp, userid_stmt, index,
                      params->op.user.uinfo.user_id.id.c_str(), sdb);
    } else {                    // by default by userid
        SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.user_id, sdb);
        SQL_BIND_TEXT(dpp, stmt, index,
                      params->op.user.uinfo.user_id.id.c_str(), sdb);
...
int SQLGetUser::Execute(const DoutPrefixProvider *dpp,
                        struct DBOpParams *params)
{
    int ret = -1;
...
int SQLInsertBucket::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLInsertBucket::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.bucket.info.bucket.name.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.tenant, sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.bucket.tenant.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.marker, sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.bucket.marker.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.bucket_id, sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.bucket.bucket_id.c_str(), sdb);
    ...
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index,
                          params->op.bucket.info.creation_time, sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.placement_rule.name.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.placement_storage_class,
                   sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.placement_rule.storage_class.c_str(),
                  sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.bucket.info.zonegroup.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.has_instance_obj, sdb);
    SQL_BIND_INT(dpp, stmt, index, params->op.bucket.info.has_instance_obj,
                 sdb);
    ...
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.bucket.info.website_conf,
                          sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.swift_versioning, sdb);
    SQL_BIND_INT(dpp, stmt, index, params->op.bucket.info.swift_versioning,
                 sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.swift_ver_location,
                   sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.swift_ver_location.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.mdsearch_config, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index,
                          params->op.bucket.info.mdsearch_config, sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.new_bucket_instance_id,
                   sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.info.new_bucket_instance_id.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.obj_lock, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.bucket.info.obj_lock,
                          sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.sync_policy_info_groups,
                   sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.bucket.info.sync_policy,
                          sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.bucket_attrs, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index, params->op.bucket.bucket_attrs,
                          sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.bucket.bucket_version.tag.c_str(), sdb);
...
int SQLInsertBucket::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
...
int SQLUpdateBucket::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLUpdateBucket::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    if (params->op.query_str == "attrs") {
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.creation_time,
                       sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index,
                              params->op.bucket.info.creation_time, sdb);
    } else if (params->op.query_str == "info") {
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.placement_name,
                       sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.bucket.info.placement_rule.name.c_str(), sdb);

        SQL_BIND_INDEX(dpp, *stmt, index,
                       p_params.op.bucket.placement_storage_class, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.bucket.info.placement_rule.storage_class.
                      c_str(), sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.zonegroup, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.bucket.info.zonegroup.c_str(), sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.has_instance_obj,
                       sdb);
        SQL_BIND_INT(dpp, *stmt, index, params->op.bucket.info.has_instance_obj,
                     sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.quota, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index, params->op.bucket.info.quota,
                              sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.requester_pays,
                       sdb);
        SQL_BIND_INT(dpp, *stmt, index, params->op.bucket.info.requester_pays,
                     sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.has_website, sdb);
        SQL_BIND_INT(dpp, *stmt, index, params->op.bucket.info.has_website,
                     sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.website_conf, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index,
                              params->op.bucket.info.website_conf, sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.swift_versioning,
                       sdb);
        SQL_BIND_INT(dpp, *stmt, index, params->op.bucket.info.swift_versioning,
                     sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.mdsearch_config,
                       sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index,
                              params->op.bucket.info.mdsearch_config, sdb);

        SQL_BIND_INDEX(dpp, *stmt, index,
                       p_params.op.bucket.new_bucket_instance_id, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.bucket.info.new_bucket_instance_id.c_str(),
                      sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.obj_lock, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index,
                              params->op.bucket.info.obj_lock, sdb);

        SQL_BIND_INDEX(dpp, *stmt, index,
                       p_params.op.bucket.sync_policy_info_groups, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index,
                              params->op.bucket.info.sync_policy, sdb);
    }
...
int SQLUpdateBucket::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    sqlite3_stmt **stmt = NULL; // Prepared statement
...
int SQLRemoveBucket::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLRemoveBucket::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLRemoveBucket::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
...
int SQLGetBucket::Prepare(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLGetBucket::Bind(const DoutPrefixProvider *dpp,
                       struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLGetBucket::Execute(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
    class SQLObjectOp *ObPtr = NULL;
...
int SQLListUserBuckets::Prepare(const DoutPrefixProvider *dpp,
                                struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
    ...
    if (params->op.query_str == "all") {
        SQL_PREPARE(dpp, p_params, sdb, all_stmt, ret,
                    "PrepareListUserBuckets");
    } else {
...
int SQLListUserBuckets::Bind(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    if (params->op.query_str != "all") {
        ...
        SQL_BIND_TEXT(dpp, *pstmt, index,
                      params->op.user.uinfo.user_id.id.c_str(), sdb);
    }

    SQL_BIND_INDEX(dpp, *pstmt, index, p_params.op.bucket.min_marker, sdb);
    SQL_BIND_TEXT(dpp, *pstmt, index, params->op.bucket.min_marker.c_str(),
                  sdb);
...
int SQLListUserBuckets::Execute(const DoutPrefixProvider *dpp,
                                struct DBOpParams *params)
{
    int ret = -1;
...
int SQLPutObject::Prepare(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLPutObject::Bind(const DoutPrefixProvider *dpp,
                       struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.obj.state.obj.key.name.c_str(),
                  sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.state.obj.key.instance.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.obj.obj_ns, sdb);
    SQL_BIND_TEXT(dpp, stmt, index, params->op.obj.state.obj.key.ns.c_str(),
                  sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.obj.owner_display_name.c_str(),
                  sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.state.obj.index_hash_source.c_str(), sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.obj.state.write_tag.c_str(),
                  sdb);
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.obj.state.shadow_obj.c_str(),
                  sdb);
    ...
    SQL_BIND_INT(dpp, stmt, index,
                 params->op.obj.state.objv_tracker.read_version.ver, sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.obj.obj_version_tag, sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.state.objv_tracker.read_version.tag.c_str(),
                  sdb);
    ...
    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.obj.head_placement_rule_name,
                   sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.head_placement_rule.name.c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index,
                   p_params.op.obj.head_placement_storage_class, sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.head_placement_rule.storage_class.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.obj.tail_placement_rule_name,
                   sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.tail_placement.placement_rule.name.c_str(),
                  sdb);

    SQL_BIND_INDEX(dpp, stmt, index,
                   p_params.op.obj.tail_placement_storage_class, sdb);
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj.tail_placement.placement_rule.storage_class.
                  c_str(), sdb);
...
int SQLPutObject::Execute(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
...
int SQLDeleteObject::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLDeleteObject::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLDeleteObject::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
...
int SQLGetObject::Prepare(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLGetObject::Bind(const DoutPrefixProvider *dpp,
                       struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLGetObject::Execute(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
...
int SQLUpdateObject::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLUpdateObject::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    if (params->op.query_str == "attrs") {
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_attrs, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index, params->op.obj.state.attrset,
                              sdb);
    }
    ...
    if (params->op.query_str == "meta") {
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_ns, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.state.obj.key.ns.c_str(), sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_category, sdb);
        SQL_BIND_INT(dpp, *stmt, index, (uint8_t)(params->op.obj.category),
                     sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.owner_display_name,
                       sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.owner_display_name.c_str(), sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.storage_class, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index, params->op.obj.storage_class.c_str(),
                      sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.content_type, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index, params->op.obj.content_type.c_str(),
                      sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.index_hash_source,
                       sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.state.obj.index_hash_source.c_str(), sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.accounted_size, sdb);
        SQL_BIND_INT(dpp, *stmt, index, params->op.obj.state.accounted_size,
                     sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_tag, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index, params->op.obj.state.obj_tag,
                              sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.tail_tag, sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index, params->op.obj.state.tail_tag,
                              sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.shadow_obj, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.state.shadow_obj.c_str(), sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.zone_short_id, sdb);
        SQL_BIND_INT(dpp, *stmt, index, params->op.obj.state.zone_short_id,
                     sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_version_tag, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.state.objv_tracker.read_version.tag.
                      c_str(), sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.tail_instance, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index, params->op.obj.tail_instance.c_str(),
                      sdb);

        SQL_BIND_INDEX(dpp, *stmt, index,
                       p_params.op.obj.head_placement_rule_name, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.head_placement_rule.name.c_str(), sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index,
                       p_params.op.obj.tail_placement_rule_name, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.tail_placement.placement_rule.name.c_str(),
                      sdb);
        ...
        SQL_BIND_INDEX(dpp, *stmt, index,
                       p_params.op.obj.tail_placement_storage_class, sdb);
        SQL_BIND_TEXT(dpp, *stmt, index,
                      params->op.obj.tail_placement.placement_rule.
                      storage_class.c_str(), sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.manifest_part_objs,
                       sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index, params->op.obj.objs, sdb);

        SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.manifest_part_rules,
                       sdb);
        SQL_ENCODE_BLOB_PARAM(dpp, *stmt, index, params->op.obj.rules, sdb);
...
int SQLUpdateObject::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    sqlite3_stmt **stmt = NULL; // Prepared statement
...
int SQLListBucketObjects::Prepare(const DoutPrefixProvider *dpp,
                                  struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLListBucketObjects::Bind(const DoutPrefixProvider *dpp,
                               struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLListBucketObjects::Execute(const DoutPrefixProvider *dpp,
                                  struct DBOpParams *params)
{
    int ret = -1;
...
int SQLListVersionedObjects::Prepare(const DoutPrefixProvider *dpp,
                                     struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLListVersionedObjects::Bind(const DoutPrefixProvider *dpp,
                                  struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLListVersionedObjects::Execute(const DoutPrefixProvider *dpp,
                                     struct DBOpParams *params)
{
    int ret = -1;
...
int SQLPutObjectData::Prepare(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLPutObjectData::Bind(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.obj_data.multipart_part_str,
                   sdb);

    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.obj_data.multipart_part_str.c_str(), sdb);
...
int SQLPutObjectData::Execute(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
...
int SQLUpdateObjectData::Prepare(const DoutPrefixProvider *dpp,
                                 struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLUpdateObjectData::Bind(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLUpdateObjectData::Execute(const DoutPrefixProvider *dpp,
                                 struct DBOpParams *params)
{
    int ret = -1;
...
int SQLGetObjectData::Prepare(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLGetObjectData::Bind(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLGetObjectData::Execute(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
...
int SQLDeleteObjectData::Prepare(const DoutPrefixProvider *dpp,
                                 struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLDeleteObjectData::Bind(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLDeleteObjectData::Execute(const DoutPrefixProvider *dpp,
                                 struct DBOpParams *params)
{
    int ret = -1;
...
int SQLDeleteStaleObjectData::Prepare(const DoutPrefixProvider *dpp,
                                      struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLDeleteStaleObjectData::Bind(const DoutPrefixProvider *dpp,
                                   struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLDeleteStaleObjectData::Execute(const DoutPrefixProvider *dpp,
                                      struct DBOpParams *params)
{
    int ret = -1;
...
int SQLInsertLCEntry::Prepare(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLInsertLCEntry::Bind(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.lc_entry.entry.get_bucket().c_str(), sdb);
    ...
    SQL_BIND_INT(dpp, stmt, index, params->op.lc_entry.entry.get_start_time(),
                 sdb);
...
int SQLInsertLCEntry::Execute(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
...
int SQLRemoveLCEntry::Prepare(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLRemoveLCEntry::Bind(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLRemoveLCEntry::Execute(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
...
int SQLGetLCEntry::Prepare(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int ret = -1;
    sqlite3_stmt **pstmt = NULL;    // Prepared statement
...
int SQLGetLCEntry::Bind(const DoutPrefixProvider *dpp,
                        struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLGetLCEntry::Execute(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int ret = -1;
    sqlite3_stmt **pstmt = NULL;    // Prepared statement
...
int SQLListLCEntries::Prepare(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLListLCEntries::Bind(const DoutPrefixProvider *dpp,
                           struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_TEXT(dpp, stmt, index, params->op.lc_entry.min_marker.c_str(),
                  sdb);
...
int SQLListLCEntries::Execute(const DoutPrefixProvider *dpp,
                              struct DBOpParams *params)
{
    int ret = -1;
...
int SQLInsertLCHead::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLInsertLCHead::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
    ...
    SQL_BIND_TEXT(dpp, stmt, index,
                  params->op.lc_head.head.get_marker().c_str(), sdb);

    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.lc_head.start_date, sdb);
    SQL_ENCODE_BLOB_PARAM(dpp, stmt, index,
                          static_cast < int64_t >
                          (params->op.lc_head.head.start_date), sdb);

out:
    return rc;
}

int SQLInsertLCHead::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
...
int SQLRemoveLCHead::Prepare(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLRemoveLCHead::Bind(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLRemoveLCHead::Execute(const DoutPrefixProvider *dpp,
                             struct DBOpParams *params)
{
    int ret = -1;
...
int SQLGetLCHead::Prepare(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
    struct DBOpPrepareParams p_params = PrepareParams;
...
int SQLGetLCHead::Bind(const DoutPrefixProvider *dpp,
                       struct DBOpParams *params)
{
    int index = -1;
    int rc = 0;
...
int SQLGetLCHead::Execute(const DoutPrefixProvider *dpp,
                          struct DBOpParams *params)
{
    int ret = -1;
