...
void DefaultRetention::decode_xml(XMLObj *obj)
{
    ...
    if ((days_exist && years_exist) || (!days_exist && !years_exist)) {
        throw RGWXMLDecoder::
        err("either Days or Years must be specified, but not both");
    }
}

void DefaultRetention::dump_xml(Formatter *f) const const
{
...
void ObjectLockRule::dump_xml(Formatter *f) const const
{
...
void RGWObjectLock::dump_xml(Formatter *f) const const
{
    if (enabled) {
...
ceph::real_time RGWObjectLock::get_lock_until_date(const ceph::
        real_time &mtime) const
const
{
    if (!rule_exist) {
...
void RGWObjectRetention::dump_xml(Formatter *f) const const
{
    encode_xml("Mode", mode, f);
...
void RGWObjectLegalHold::dump_xml(Formatter *f) const const
{
    encode_xml("Status", status, f);
}

bool RGWObjectLegalHold::is_enabled() const const
{
