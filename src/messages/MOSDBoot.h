...
public:
    ...
    MOSDBoot()
        : PaxosServiceMessage {
        MSG_OSD_BOOT, 0, HEAD_VERSION, COMPAT_VERSION},
    ...
    MOSDBoot(const OSDSuperblock &s, epoch_t e, epoch_t be,
             ...
             const entity_addrvec_t &cluster_addr_ref, uint64_t feat)
        : PaxosServiceMessage {
        MSG_OSD_BOOT, e, HEAD_VERSION, COMPAT_VERSION},
    sb(s),
    hb_back_addrs(hb_back_addr_ref),
    hb_front_addrs(hb_front_addr_ref),
    cluster_addrs(cluster_addr_ref), boot_epoch(be), osd_features(feat)
    {
    }
