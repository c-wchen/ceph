...
namespace cephfs {
    namespace mirror {
        ...
        namespace {
            ...
            struct C_UnwatchAndFlush:public Context {
                ...
                :rados(ioctx), on_finish(on_finish) {
                    ...
                    // ensure our reference to the RadosClient is released prior// to completing the callback to avoid racing an explicit// librados shutdown
        ...
        Watcher::Watcher(librados::IoCtx & ioctx, std::string_view oid,
                         ContextWQ * work_queue)
      :    m_oid(oid), m_ioctx(ioctx), m_work_queue(work_queue),
            m_lock(ceph::make_shared_mutex("cephfs::mirror::snap_watcher")),
            m_state(STATE_IDLE), m_watch_ctx(*this) {
        }
        ...
        void Watcher::register_watch(Context * on_finish) {
            ...
            librados::AioCompletion * aio_comp =
                librados::Rados::aio_create_completion(on_finish,
                                                       ...
                                                       &Context::complete >);
            int r =
                m_ioctx.aio_watch(m_oid, aio_comp, &m_watch_handle,
                                  &m_watch_ctx);
            ceph_assert(r == 0);
            aio_comp->release();
        }
        ...
        void Watcher::handle_register_watch(int r, Context * on_finish) {
            ...
            Context *unregister_watch_ctx = nullptr;
            {
                ...
                m_state = STATE_IDLE;
                if (r < 0) {
                    derr << ": failed to register watch: " << cpp_strerror(r) <<
                        dendl;
                    m_watch_handle = 0;
                }
        ...
        void Watcher::unregister_watch(Context * on_finish) {
...

            {
                ...
                if (m_state != STATE_IDLE) {
                    dout(10) <<
                        ": delaying unregister -- watch register in progress" <<
                        dendl;
                    ...
                    librados::AioCompletion * aio_comp =
                        librados::Rados::
                        aio_create_completion(new
                                              C_UnwatchAndFlush(m_ioctx,
                                                                on_finish),
                                              &rados_callback < Context,
                                              &Context::complete >);
        ...
        void Watcher::handle_rewatch(int r) {
            ...
            Context *unregister_watch_ctx = nullptr;
            {
                ...
                if (m_unregister_watch_ctx != nullptr) {
                    ...
                    dout(5) << ": object " << m_oid << " does not exist" <<
                        dendl;
                }
                else if (r < 0) {
            ...
            Context *ctx =
                new C_CallbackAdapter < Watcher,
                &Watcher::handle_rewatch_callback > (this);
