...
class TestStub:public Dispatcher {
  ...
  :    
    ...
    timer(cct, lock), do_shutdown(false), tick_seconds(0.0) {
    }
};
...
class OSDStub:public TestStub {
  ...
  :    
    TestStub(cct, "osd"),
    auth_handler_registry(new AuthAuthorizeHandlerRegistry(cct,
                                                           cct->_conf->
                                                           ...
                                                           : cct->_conf->
                                                           auth_supported)),
    whoami(_whoami), gen(whoami), mon_osd_rng(STUB_MON_OSD_FIRST,
                                              STUB_MON_OSD_LAST) {
        dout(20) << __func__ << " auth supported: " << cct->_conf->
        ...
        messenger.
            reset(Messenger::
                  create(cct, public_msgr_type, entity_name_t::OSD(whoami),
    ...
    void auto_create_pgs() {
        ...
        for (auto pit = osdmap_pools.begin(); pit != osdmap_pools.end(); ++pit) {
            ...
            dout(20) << __func__ << " pool num pgs " << pool.get_pg_num()
    ...
    void update_osd_stat() {
        ...
        osd_stat.kb_used =
            (stbuf.f_blocks - stbuf.f_bfree) * stbuf.f_bsize / 1024;
    ...
    void send_pg_stats() {
        ...
        utime_t now = ceph_clock_now();
        MPGStats *mstats =
            new MPGStats(monc.get_fsid(), osdmap.get_epoch(), now);
    ...
    void op_alive() {
        ...
        dout(10) << __func__ << "up_thru: " << osdmap.
            get_up_thru(whoami) << dendl;
...
void usage()
{
    ...
    std::cout << "usage: " << our_name
        << " <--stub-id ID> [--stub-id ID...]" << std::endl;
    std::cout << "\n\
Global Options:\n\
...
int main(int argc, const char *argv[])
{
    ...
    for (std::vector < const char *>::iterator i = args.begin();
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            if (get_id_interval(first, last, val) < 0) {
                std::cerr << "** error parsing stub id '" << val << "'" << std::
                    endl;
                exit(1);
            }
    ...
    for (set < int >::iterator i = stub_ids.begin(); i != stub_ids.end(); ++i) {
        ...
        int err = stub->init();
        if (err < 0) {
            std::cerr << "** osd stub error: " << cpp_strerror(-err) << std::
                endl;
            return 1;
        }
    ...
    int err = cstub->init();
    if (err < 0) {
        std::cerr << "** client stub error: " << cpp_strerror(-err) << std::
            endl;
        return 1;
    }
