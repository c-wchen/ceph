...
class Checksummer {
  public:
    ...
    struct crc32c {
        ...
        static value_t calc(state_t state,
                            init_value_t init_value,
                            size_t len, bufferlist::const_iterator & p) {
    ...
    template < class Alg >
        static int calculate(size_t csum_block_size,
                             size_t offset,
                             size_t length,
                             const bufferlist & bl, bufferptr * csum_data) {
        return calculate < Alg > (-1, csum_block_size, offset, length, bl,
                                  csum_data);
    }

    template < class Alg >
        static int calculate(typename Alg::init_value_t init_value,
    ...
    template < class Alg >
        static int verify(size_t csum_block_size,
                          size_t offset,
                          size_t length,
                          const bufferlist & bl,
        ...
        const typename Alg::value_t * pv =
            reinterpret_cast <
            const typename Alg::value_t * >(csum_data.c_str());
        pv += offset / csum_block_size;
        size_t pos = offset;
        while (length > 0) {
