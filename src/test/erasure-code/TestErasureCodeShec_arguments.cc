...
bool search_table_shec432(set < int >want_to_read, set < int >available_chunks)
{
    ...
    for (set < set < int > >::iterator itr = tmp.begin(); itr != tmp.end();
         ++itr) {
        found = true;
        value_count = 0;
        settmp = *itr;
        for (set < int >::iterator setitr = settmp.begin();
...
TEST(ParameterTest, combination_all)
{
    ...
    set < int >want_to_read, available_chunks, minimum_chunks,
        ...
        for (unsigned w2 = 0; w2 <= shec->get_chunk_count(); ++w2) {
            ...
            do {
                do {
                    ...
                    result =
                        shec->minimum_to_decode(want_to_read, available_chunks,
                                                &minimum_chunks);
                    ...
                    if (want_to_read.size() == 0) {
                        ...
                        // want - avail
                        for (itr = want_to_read.begin();
                             itr != want_to_read.end(); ++itr) {
                        ...
                        if (want_to_read_without_avails.size() == 0) {
                            ...
                            for (unsigned int i = 0;
                                 i < shec->get_data_chunk_count(); ++i) {
                                if (array_want_to_read[i]) {
                                    usable.clear();
                                    usable.substr_of(in, c_size * i, c_size);
                                    cmp =
                                        memcmp(decoded[i].c_str(),
                                               usable.c_str(), c_size);
                            ...
                            if (search_table_shec432
                                ...
                                EXPECT_EQ(0, result);
                                EXPECT_LT(0u, minimum_chunks.size());
                                EXPECT_GE(value_count + minimum_count,
                                          minimum_chunks.size());
                                EXPECT_EQ(0, dresult);
                                EXPECT_NE(0u, decoded.size());
                                for (unsigned int i = 0;
                                ...
                                EXPECT_EQ(0u, minimum_chunks.size());
                                EXPECT_EQ(-1, dresult);
                                EXPECT_EQ(shec->get_chunk_count(),
                                          decoded.size());
                                if (result != -EIO || dresult != -1) {
                                    ++unexpected_count;
                                }
                    ...
                    decoded.clear();
                    usable.clear();
                } while (std::prev_permutation(array_want_to_read,
                                               array_want_to_read +
                                               shec->get_chunk_count()));

            } while (std::prev_permutation(array_available_chunks,
