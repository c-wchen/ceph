...
namespace librbd {
    namespace image {
        ...
        class TestMockImageValidatePoolRequest:public TestMockFixture {
          public:
            typedef ValidatePoolRequest < MockTestImageCtx >
            ...
            void SetUp() override {
                ...
                EXPECT_CALL(mock_io_ctx, clone())
                    .WillOnce(Invoke([&mock_io_ctx] () {
                                     mock_io_ctx.get(); return &mock_io_ctx;}
                              ));
            } void expect_read_rbd_info(librados::
                                        MockTestMemIoCtxImpl & mock_io_ctx,
                                        const std::string & data, int r) {
                auto & expect =
                    EXPECT_CALL(mock_io_ctx,
                                read(StrEq(RBD_INFO), 0, 0, _, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
                    expect.WillOnce(WithArg < 3 >
                                    (Invoke([data] (bufferlist * bl) {
                                            bl->append(data); return 0;}
                                     )));
                }
            }

            void expect_write_rbd_info(librados::
                                       MockTestMemIoCtxImpl & mock_io_ctx,
                                       const std::string & data, int r) {
                bufferlist bl;
                bl.append(data);
                ...
                EXPECT_CALL(mock_io_ctx,
                            write(StrEq(RBD_INFO), ContentsEqual(bl),
                                  data.length(), 0, _))
                    .WillOnce(Return(r));
            }

            void expect_allocate_snap_id(librados::
                                         MockTestMemIoCtxImpl & mock_io_ctx,
                                         int r) {
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, selfmanaged_snap_create(_));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_release_snap_id(librados::
                                        MockTestMemIoCtxImpl & mock_io_ctx,
                                        int r) {
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, selfmanaged_snap_remove(_));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
        ...
        TEST_F(TestMockImageValidatePoolRequest, Success) {
            librados::
                MockTestMemIoCtxImpl & mock_io_ctx(get_mock_io_ctx(m_ioctx));
        ...
        TEST_F(TestMockImageValidatePoolRequest, OverwriteError) {
            ...
            expect_write_rbd_info(mock_io_ctx, "validate", 0);
            expect_release_snap_id(mock_io_ctx, 0);
            expect_write_rbd_info(mock_io_ctx, "overwrite validated",
                                  -EOPNOTSUPP);
