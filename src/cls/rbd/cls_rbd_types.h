    ...
    namespace rbd {
        ...
        inline void encode(const GroupImageLinkState & state, bufferlist & bl,
            ...
            inline bool is_valid() const {
                return (!uuid.empty() && !cluster_name.empty()
                        && !client_name.empty());
        ...
        struct MirrorImage {
            MirrorImage() {
            } MirrorImage(const std::string & global_image_id,
        ...
        inline void encode(const MirrorImageStatusState & state,
                           bufferlist & bl, uint64_t features = 0) {
            ::encode(static_cast < uint8_t > (state), bl);
        }
        ...
        struct UserSnapshotNamespace {
            static const uint32_t SNAPSHOT_NAMESPACE_TYPE =
        ...
        struct SnapshotNamespaceOnDisk {
            ...
            SnapshotNamespaceOnDisk():snapshot_namespace
                (UnknownSnapshotNamespace()) {
            } SnapshotNamespaceOnDisk(const SnapshotNamespace &
                                      sn):snapshot_namespace(sn) {
            }
        ...
        struct TrashImageSpec {
            ...
            utime_t deferment_end_time;
             ...
             TrashImageSpec() {
                             ...
                             utime_t deferment_end_time):source(source),
                name(name), deletion_time(deletion_time),
