...
namespace crimson::os::seastore::journal {
    ...
    SegmentedJournal::SegmentedJournal(SegmentProvider & segment_provider,
    ...
    :segment_seq_allocator(new SegmentSeqAllocator(segment_type_t::JOURNAL)),
        ...
        sm_group(*segment_provider.get_segment_manager_group()), trimmer {
    trimmer} {
    }
    ...
    SegmentedJournal::prep_replay_segments_fut
        SegmentedJournal::prep_replay_segments(std::vector < std::pair <
                                               segment_id_t,
                                               segment_header_t >> segments) {
        ...
        std::sort(segments.begin(),
                  segments.end(),[](const auto & lt, const auto & rt) {
                  return lt.second.segment_seq < rt.second.segment_seq;}
        );

        segment_seq_allocator->set_next_segment_seq(segments.rbegin()->second.
                                                    segment_seq + 1);
        std::for_each(segments.begin(), segments.end(),[FNAME] (auto & seg) {
                      ...
                      ERROR("illegal journal segment for replay -- {}",
                            seg.second); ceph_abort();}
                      }
        ) ;

        auto last_segment_id = segments.rbegin()->first;
        auto last_header = segments.rbegin()->second;
        return scan_last_segment(last_segment_id,
                                 last_header).safe_then([this, FNAME, segments =
                                                         std::move(segments)] {
                                                        INFO
    ...
    SegmentedJournal::scan_last_segment_ertr::future <>
        ...
        return seastar::do_with(scan_valid_records_cursor(seq),
                                SegmentManagerGroup::found_record_handler_t([FNAME, this] (record_locator_t locator, const record_group_header_t & record_group_header, const bufferlist & mdbuf)->SegmentManagerGroup::scan_valid_records_ertr::future <> {
                                                                            DEBUG
                                                                            ("decoding {} at {}",
                                                                             record_group_header,
                                                                             locator);
                                                                            ...
                                                                            ERROR
                                                                            ("unable to decode headers from {} at {}",
                                                                             record_group_header,
                                                                             locator);
                                                                            ceph_abort
                                                                            ();}
          for (auto & record_header:                                       *maybe_headers) {
              ...
              for (auto & record_deltas:                                   *maybe_record_deltas_list) {
                  for (auto &[ctime, delta]:                               record_deltas.deltas) {
                                   ...
                                   segment_header.segment_nonce] (auto & cursor,
                                                                  auto &
                                                                  handler) {
    ...
    SegmentedJournal::replay_ertr::future <>
        SegmentedJournal::replay_segment(journal_seq_t seq,
        ...
        return seastar::do_with(scan_valid_records_cursor(seq),
                                SegmentManagerGroup::found_record_handler_t([&handler, this, &stats] (record_locator_t locator, const record_group_header_t & header, const bufferlist & mdbuf)
                                                                            ->
                                                                            SegmentManagerGroup::
                                                                             ...
                                                                             (record_deltas_list,
                                                                              ...
                                                                              (record_deltas.
                                                                               ...
                                                                               (locator,
                                                                                ...
                                                                                modify_time).
                                                                               safe_then
                                                                               ([&stats, delta_type = delta.type] (bool is_applied) {
                                                                                if
                                                                                (is_applied)
                                                                                {
    ...
    SegmentedJournal::replay_ret SegmentedJournal::replay(delta_handler_t
                                                          && delta_handler) {
        ...
        return sm_group.find_journal_segment_headers().
            safe_then([this, FNAME, delta_handler = std::move(delta_handler)]
                      ...
                      return seastar::do_with(std::move(delta_handler),
                                              replay_segments_t(),
                                              replay_stats_t(),
                                              [this, segment_headers =
                                               std::move(segment_headers),
                                               FNAME]
                                              (auto & handler, auto & segments,
                                               auto & stats) mutable->
                                              replay_ret {
                                              return prep_replay_segments(std::
                                                                          move
                                                                          (segment_headers)
                                              ).
                                              safe_then([this, &handler,
                                                         &segments,
                                                         &stats] (auto
                                                                  replay_segs)
                                                        mutable {
    ...
    SegmentedJournal::submit_record_ret
        SegmentedJournal::do_submit_record(record_t && record,
                ...
                then([submit_fut = std::move(submit_fut)] ()mutable {
                     return std::move(submit_fut);}
                        ...
                        then([FNAME, this, result, &handle] {
                             DEBUG("H{} finish with {}", (void *)&handle,
                                   result);
    ...
    SegmentedJournal::submit_record_ret
        SegmentedJournal::submit_record(record_t && record,
                                        OrderingHandle & handle) {
        LOG_PREFIX(SegmentedJournal::submit_record);
        DEBUG("H{} {} start ...", (void *)&handle, record);
        assert(write_pipeline);
        auto expected_size = record_group_size_t(record.size,
                                                 journal_segment_allocator.
                                                 get_block_size()
            ).get_encoded_length();
        auto max_record_length =
