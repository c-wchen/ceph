...
int RGWRESTSimpleRequest::execute(RGWAccessKey & key, const char *method,
                                  const char *resource)
{
    string new_url = url;
    ...
    rgw_create_s3_canonical_header(method, NULL, NULL, date_str.c_str(),
                                   meta_map, meta_map, new_url.c_str(),
...
void RGWRESTSimpleRequest::append_param(string & dest, const string & name,
                                        const string & val)
{
    if (dest.empty()) {
        dest.append("?");
...
void RGWRESTSimpleRequest::get_params_str(map < string, string > &extra_args,
                                          string & dest)
{
...
int RGWRESTSimpleRequest::sign_request(RGWAccessKey & key, RGWEnv & env,
                                       req_info & info)
{
    ...
    ldout(cct,
          10) << "generated canonical header: " << canonical_header << dendl;
...
int RGWRESTStreamWriteRequest::put_obj_init(RGWAccessKey & key, rgw_obj & obj,
                                            uint64_t obj_size, map < string,
                                            bufferlist > &attrs)
{
...
static int parse_rgwx_mtime(CephContext * cct, const string & s,
                            ceph::real_time * rt)
{
    string err;
    vector < string > vec;
    ...
    if (!err.empty()) {
        ldout(cct,
              0) << "ERROR: failed converting mtime (" << s << ") to real_time "
            << dendl;
        return -EINVAL;
    }
...
int RGWRESTStreamRWRequest::send_request(RGWAccessKey & key, map < string,
                                         string > &extra_headers, rgw_obj & obj,
                                         RGWHTTPManager * mgr)
{
                                         ...
                                         RGWHTTPManager * mgr)
{
    ...
    for (param_vec_t::iterator iter = params.begin(); iter != params.end();
         ++iter) {
  ...
  for (const auto & kv:new_env.get_map()) {
    ...
    int r =
        pmanager->add_request(this, new_info.method, new_url.c_str(),
                              send_data_hint);
    if (r < 0)
        return r;
...
int RGWRESTStreamRWRequest::complete_request(string & etag, real_time * mtime,
                                             uint64_t * psize, map < string,
                                             string > &attrs)
{
    set_str_from_headers(out_headers, "ETAG", etag);
    if (status >= 0) {
...
int RGWRESTStreamRWRequest::handle_header(const string & name,
                                          const string & val)
{
