...
#define	RBDFS_BSIZE	4096
    ...
    static int rbdfs_readdir(const char *path, void *buf,
...
// return -errno on error.  fi->fh is not set until open time
    ...
    int rbdfs_utime(const char *path, struct utimbuf *utime) {
 ...
 */
    ...
    int rbdfs_setxattr(const char *path, const char *name, const char *value,
    ...
    int rbdfs_getxattr(const char *path, const char *name, char *value,
                       size_t size
#if defined(DARWIN)
                       , uint32_t position
#endif
