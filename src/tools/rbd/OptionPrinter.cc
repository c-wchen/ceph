...
const std::string OptionPrinter::
...
} void OptionPrinter::print_short(std::ostream &os, size_t initial_offset)
{
    ...
    for (size_t i = 0; i < m_positional.options().size(); ++i) {
        ...
        boost::any v;
        bool required =
            !m_positional.options()[i]->semantic()->apply_default(v);
        if (!required) {
                                   ...
                                   size_t &name_width, std::ostream &os)
{
    ...
    for (size_t i = 0; i < global_opts.options().size(); ++i) {
        ...
        std::cout << ss.str();
        IndentStream indent_stream(name_width, ss.str().size(), LINE_WIDTH,
                                   std::cout);
        indent_stream << global_opts.options()[i]->
...
void OptionPrinter::print_detailed(std::ostream &os)
{
    ...
    if (m_positional.options().size() > 0) {
        ...
        for (size_t i = 0; i < m_positional.options().size(); ++i) {
            std::stringstream ss;
            ss << indent_prefix << "<" << m_positional.options()[i]->
               long_name()
               << ">";
            ...
            IndentStream indent_stream(name_width, ss.str().size(),
                                       LINE_WIDTH, os);
            indent_stream << m_positional.options()[i]->
                          description() << std::endl;
        }
        std::cout << std::endl;
    }
...
size_t OptionPrinter::compute_name_width(size_t indent)
{
    ...
    std::vector < OptionsDescription > descs = {
        m_positional, m_optional
    };
    for (size_t desc_idx = 0; desc_idx < descs.size(); ++desc_idx) {
        ...
        for (size_t opt_idx = 0; opt_idx < desc.options().size(); ++opt_idx) {
            size_t name_width =
                desc.options()[opt_idx]->format_name().size() +
