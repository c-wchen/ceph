...
template <> struct BreakRequest <librbd::MockImageCtx > {
    ...
    {
        CephContext *cct =
            reinterpret_cast < CephContext * >(ioctx.cct());
        EXPECT_EQ(cct->_conf.get_val < bool >
                  ("rbd_blocklist_on_break_lock"), blocklist_locker);
        EXPECT_EQ(cct->_conf.get_val < uint64_t >
...
BreakRequest < librbd::MockImageCtx > *BreakRequest <
librbd::MockImageCtx >::s_instance = nullptr;
GetLockerRequest < librbd::MockImageCtx > *GetLockerRequest <
librbd::MockImageCtx >::s_instance = nullptr;
...
public:
                             ...
                             int r)
    {
        ...
        WillOnce(FinishRequest
                 (&mock_break_request, r, &mock_image_ctx));
    }
};
...
TEST_F(TestMockManagedLockAcquireRequest, SuccessExclusive)
{
    ...
    C_SaferCond ctx;
    MockAcquireRequest *req =
        MockAcquireRequest::create(mock_image_ctx.md_ctx,
