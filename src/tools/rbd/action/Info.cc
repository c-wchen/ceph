...
{
    format_bitmask(f, "feature", at::ImageFeatures::FEATURE_MAPPING,
                   features);
}
...
void format_timestamp(struct timespec timestamp,
                      std::string &timestamp_str)
{
    if (timestamp.tv_sec != 0) {
        ...
        timestamp_str =
            timestamp_str.substr(0, timestamp_str.length() - 1);
    }
}

static int do_show_info(librados::IoCtx &io_ctx,
                        librbd::Image &image,
                        ...
                        Formatter *f)
{
    ...
    librbd::mirror_image_mode_t mirror_mode =
    ...
    if (!snapname.empty()) {
        r = image.snap_is_protected(snapname.c_str(),
                                    &snap_protected);
        if (r < 0) {
            return r;
        }
    ...
    r = image.mirror_image_get_info(&mirror_image,
                                    sizeof(mirror_image));
    if (r < 0) {
        return r;
    }
    ...
    if (f) {
        ...
        std::cout << "rbd image '" << imgname << "':\n"
                  ...
                  << std::endl << "\tsnapshot_count: " << snaps.size()
                  << std::endl;
        if (!imgid.empty()) {
        ...
        std::cout << "\tblock_name_prefix: " << prefix
                  << std::endl << "\tformat: " << (old_format ? "1" : "2")
                  << std::endl;
    }
    ...
    if (!create_timestamp_str.empty()) {
        if (f) {
            f->dump_string("create_timestamp",
                           create_timestamp_str);
        } else {
            std::
            cout << "\tcreate_timestamp: " <<
                 create_timestamp_str << std::endl;
        }
    }

    if (!access_timestamp_str.empty()) {
        if (f) {
            f->dump_string("access_timestamp",
                           access_timestamp_str);
        } else {
            std::
            cout << "\taccess_timestamp: " <<
                 access_timestamp_str << std::endl;
        }
    }

    if (!modify_timestamp_str.empty()) {
        if (f) {
            f->dump_string("modify_timestamp",
                           modify_timestamp_str);
        } else {
            std::
            cout << "\tmodify_timestamp: " <<
                 modify_timestamp_str << std::endl;
        }
    }
    ...
    if (!snapname.empty()) {
        if (f) {
            f->dump_string("protected",
                           snap_protected ? "true" : "false");
        } else {
            std::
            cout << "\tprotected: " << (snap_protected ? "True"
                                        : "False")
                 << std::endl;
        }
    }
    ...
    if (snap_limit < UINT64_MAX) {
        if (f) {
            ...
            std::cout << "\tsnapshot_limit: " << snap_limit << std::
                      endl;
        }
    }
    ...
    if ((image.get_parent(&parent_image_spec, &parent_snap_spec) ==
         0) && (parent_image_spec.image_name.length() > 0)) {
        if (f) {
            f->open_object_section("parent");
            f->dump_string("pool", parent_image_spec.pool_name);
            f->dump_string("pool_namespace",
                           parent_image_spec.pool_namespace);
            ...
            std::cout << "\tparent: " << parent_image_spec.
                      pool_name << "/";
            if (!parent_image_spec.pool_namespace.empty()) {
                std::cout << parent_image_spec.
                          pool_namespace << "/";
            }
            std::cout << parent_image_spec.image_name << "@"
                      << parent_snap_spec.name;
            if (parent_image_spec.trash) {
                std::cout << " (trash " << parent_image_spec.
                          image_id << ")";
            }
            std::cout << std::endl;
            std::cout << "\toverlap: " << byte_u_t(overlap) << std::
                      endl;
        }
    }
    ...
    if (features & RBD_FEATURE_STRIPINGV2) {
        if (f) {
            f->dump_unsigned("stripe_unit",
                             image.get_stripe_unit());
            f->dump_unsigned("stripe_count",
                             image.get_stripe_count());
        } else {
            std::cout << "\tstripe unit: " << byte_u_t(image.
                      get_stripe_unit
                      ())
                      << std::endl << "\tstripe count: " << image.
    ...
    if (features & RBD_FEATURE_JOURNALING) {
        if (f) {
            ...
            std::cout << "\tjournal: " << utils::
                      image_id(image) << std::endl;
        }
    }
    ...
    if (features & RBD_FEATURE_JOURNALING ||
        ...
        if (f) {
            ...
            f->dump_string("state",
                           utils::mirror_image_state(mirror_image.
                                   state));
            if (mirror_image.state != RBD_MIRROR_IMAGE_DISABLED) {
            ...
            if (mirror_image.state != RBD_MIRROR_IMAGE_DISABLED) {
                std::cout << "\tmirroring mode: "
                          << utils::
                          mirror_image_mode(mirror_mode) << std::
                          endl << "\tmirroring global id: " <<
                          mirror_image.
                          global_id << std::
                          endl << "\tmirroring primary: " <<
                          (mirror_image.
                           primary ? "true" : "false") << std::endl;
            }
        }
    }
