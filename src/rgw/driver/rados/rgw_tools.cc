                   ...
                   bool mostly_omap, bool bulk)
{
    ...
    if (r == -ENOENT && create) {
        ...
        if (mostly_omap) {
            ...
            bufferlist inbl;
            float bias =
                g_conf().get_val < double >("rgw_rados_pool_autoscale_bias");
            int r =
                rados->
            ...
            if (r < 0) {
                ldpp_dout(dpp,
                          10) << __func__ <<
                    " warning: failed to set pg_autoscale_bias on " << pool.
                    name << dendl;
            }
            ...
            r = rados->
                mon_command("{\"prefix\": \"osd pool set\", \"pool\": \"" +
                            ...
                            stringify(p) + "\"}", inbl, NULL, NULL);
            if (r < 0) {
                ldpp_dout(dpp,
                          10) << __func__ <<
                    " warning: failed to set recovery_priority on " << pool.
                    name << dendl;
            }
        }
        if (bulk) {
            ...
            if (r < 0) {
                ldpp_dout(dpp,
                          10) << __func__ <<
                    " warning: failed to set 'bulk' on " << pool.name << dendl;
            }
        }
    }
    else if (r < 0) {
...
int rgw_stat_system_obj(const DoutPrefixProvider * dpp,
                        ...
                        optional_yield y, std::map < std::string,
                        bufferlist > *pattrs)
{
...
int rgw_delete_system_obj(const DoutPrefixProvider * dpp,
                          RGWSI_SysObj * sysobj_svc, const rgw_pool & pool,
                          const string & oid,
...
int rgw_rados_operate(const DoutPrefixProvider * dpp, librados::IoCtx & ioctx,
                      ...
                      optional_yield y, int flags)
{
    ...
    if (is_asio_thread) {
        ...
        ldpp_dout(dpp,
                  20) << "BACKTRACE: " << __func__ << ": " << ClibBackTrace(0)
            << dendl;
#endif
    }
...
int rgw_rados_notify(const DoutPrefixProvider * dpp, librados::IoCtx & ioctx,
...
void rgw_filter_attrset(map < string, bufferlist > &unfiltered_attrset,
...
int RGWDataAccess::Bucket::init(const DoutPrefixProvider * dpp,
                                optional_yield y)
{
                               ...
                               const DoutPrefixProvider * dpp, optional_yield y)
{
    ...
    rgw::BlockingAioThrottle aio(driver->ctx()->_conf->
    ...
    do {
        size_t read_len =
            std::min(data.length(),
                     (unsigned int)cct->_conf->rgw_max_chunk_size);
