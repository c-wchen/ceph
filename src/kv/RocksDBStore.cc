...
int RocksDBStore::do_open(ostream & out, bool create_if_missing)
{
    ...
    if (g_conf->get_val < std::string > ("rocksdb_db_paths").length()) {
        list < string > paths;
        get_str_list(g_conf->get_val < std::string > ("rocksdb_db_paths"),
                     "; \t", paths);
      for (auto & p:paths) {
            size_t pos = p.find(',');
    ...
    if (g_conf->rocksdb_cache_type == "binned_lru") {
        bbt_opts.block_cache =
            rocksdb_cache::NewBinnedLRUCache(block_cache_size,
                                             g_conf->rocksdb_cache_shard_bits);
    }
    else if (g_conf->rocksdb_cache_type == "lru") {
        bbt_opts.block_cache = rocksdb::NewLRUCache(block_cache_size,
                                                    g_conf->
                                                    rocksdb_cache_shard_bits);
    }
    else if (g_conf->rocksdb_cache_type == "clock") {
        bbt_opts.block_cache = rocksdb::NewClockCache(block_cache_size,
                                                      g_conf->
                                                      rocksdb_cache_shard_bits);
        ...
        derr << "unrecognized rocksdb_cache_type '" << g_conf->
            rocksdb_cache_type << "'" << dendl;
        return -EINVAL;
    }
    ...
    if (g_conf->get_val < std::string > ("rocksdb_index_type") ==
        "binary_search")
        bbt_opts.index_type =
    ...
    if (g_conf->get_val < uint64_t > ("rocksdb_metadata_block_size") > 0)
        bbt_opts.metadata_block_size =
            g_conf->get_val < uint64_t > ("rocksdb_metadata_block_size");
    ...
    dout(10) << __func__ << " block size " << g_conf->rocksdb_block_size
        ...
        << "; shards " << (1 << g_conf->rocksdb_cache_shard_bits)
        << ", type " << g_conf->rocksdb_cache_type << dendl;
    ...
    *_dout << " Rocksdb transaction: " << bat_txc.seen << dendl;
    ...
    if (g_conf->rocksdb_perf) {
        ...
        write_wal_time.set_from_double(static_cast <
                                       double >(rocksdb::perf_context.
                                                write_wal_time) / 1000000000);
        ...
        write_delay_time.set_from_double(static_cast <
                                         double >(rocksdb::perf_context.
                                                  write_delay_time) /
                                         1000000000);
        write_pre_and_post_process_time.set_from_double(static_cast <
                                                        double >(rocksdb::
                                                                 perf_context.
                      ...
                      std::map < string, bufferlist > *out)
{
    ...
    for (std::set < string >::const_iterator i = keys.begin();
        ...
        auto status =
            db->Get(rocksdb::ReadOptions(), rocksdb::Slice(bound), &value);
        if (status.ok()) {
            (*out)[*i].append(value);
        }
...
int RocksDBStore::get(const string & prefix,
                      const char *key, size_t keylen, bufferlist * out)
{
    assert(out && (out->length() == 0));
    utime_t start = ceph_clock_now();
    int r = 0;
...
int64_t RocksDBStore::request_cache_bytes(PriorityCache::Priority pri,
                                          uint64_t chunk_bytes) const const
{
    ...
    case PriorityCache::Priority::PRI0:
        {
            ...
            if (g_conf->rocksdb_cache_type == "binned_lru") {
                auto binned_cache =
                    std::static_pointer_cast < rocksdb_cache::BinnedLRUCache >
                    (cache);
...
int64_t RocksDBStore::get_cache_usage() const const
{
...
int64_t RocksDBStore::commit_cache_size()
{
    ...
    if (g_conf->rocksdb_cache_type == "binned_lru") {
        ...
        dout(10) << __func__ << " High Pri Pool Ratio set to " << ratio <<
            dendl;
...
int RocksDBStore::RocksDBWholeSpaceIteratorImpl::
upper_bound(const string & prefix, const string & after)
{
...
int RocksDBStore::RocksDBWholeSpaceIteratorImpl::
lower_bound(const string & prefix, const string & to)
{
