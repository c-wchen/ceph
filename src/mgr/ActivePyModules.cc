...
ActivePyModules::ActivePyModules(PyModuleConfig const &config_,
                                 ...
                                 Finisher & f)
:config_cache(config_), daemon_state(ds), cluster_state(cs), monc(mc),
clog(clog_), objecter(objecter_), client(client_), finisher(f),
...
PyObject *ActivePyModules::get_python(const std::string & what)
{
    ...
    if (what == "fs_map") {
        ...
        PyFormatter f;
        cluster_state.with_monmap([&f] (const MonMap & monmap) {
                                  monmap.dump(&f);}
        );
        return f.get();
    }
        ...
        PyFormatter f;
        cluster_state.with_servicemap([&f] (const ServiceMap & service_map) {
                                      service_map.dump(&f);}
        );
        return f.get();
    }
      ...
      for (const auto & i:dmc) {
        ...
        PyFormatter f;
        cluster_state.with_pgmap([&f] (const PGMap & pg_map) {
          std::map < std::string, std::map < std::string, uint32_t > >osds; std::map < std::string, std::map < std::string, uint32_t > >pools; std::map < std::string, uint32_t > all; for (const auto & i:pg_map.pg_stat)
              ...
              pools[stringify(pool)][state]++; for (const auto & osd_id:i.second.acting)
                                 ...
                                 all[state]++;}
          f.open_object_section("by_osd"); for (const auto & i:osds)
                                 {
        ...
        cluster_state.with_pgmap([this, &f] (const PGMap & pg_map) {
                                 cluster_state.with_osdmap([&pg_map,
                                                            &f] (const OSDMap &
                                                                 osd_map) {
                                                           pg_map.
                                                           dump_fs_stats
                               ...
                               const SafeThreadState & pMyThreadState)
{
    ...
    modules[module_name].reset(new ActivePyModule(module_name, pClass,
                                                  pMyThreadState, clog));
    ...
    if (r != 0) {
        ...
        modules[module_name]->thread.create(modules[module_name]->get_name().
                                            c_str());
                                 ...
                                 const std::string & notify_id)
{
  ...
  for (auto & i:modules) {
        ...
        finisher.
            queue(new FunctionContext([module, notify_type, notify_id] (int r) {
                                      module->notify(notify_type,
                                                     notify_id);}));
    }
}
                                             ...
                                             string & prefix) constconst
{
    ...
    for (auto p = config_cache.lower_bound(global_prefix);
         p != config_cache.end() && p->first.find(global_prefix) == 0; ++p) {
...
std::vector < ModuleCommand > ActivePyModules::get_py_commands() constconst
{
    Mutex::Locker l(lock);
...
PyObject *ActivePyModules::get_perf_schema_python(const std::string svc_type,
                                                  const std::string & svc_id)
{
    PyThreadState *tstate = PyEval_SaveThread();
    Mutex::Locker l(lock);
 ...
 */
PyObject *construct_with_capsule(const std::string & module_name,
                                 const std::string & clsname, void *wrapped)
{
