...
int Accepter::bind(const entity_addr_t & bind_addr,
                   const set < int >&avoid_ports)
{
    ...
    for (int i = 0; i < conf->ms_bind_retry_count; i++) {
        ...
        if (listen_addr.get_port()) {
            ...
            int on = 1;
            rc =::setsockopt(listen_sd, SOL_SOCKET, SO_REUSEADDR, &on,
                             sizeof(on));
            if (rc < 0) {
            ...
            if (rc < 0) {
                lderr(msgr->
                    ...
                    ms_bind_port_max << ": " << cpp_strerror(errno)
                    << dendl;
                r = -errno;
    ...
    if (rc < 0) {
        lderr(msgr->
              cct) << __func__ << " unable to create signalling pipe " <<
            listen_addr << ": " << cpp_strerror(rc) << dendl;
        return rc;
    }
...
int Accepter::rebind(const set < int >&avoid_ports)
{
    ...
    ldout(msgr->cct,
          10) << " will try " << addr << " and avoid ports " << new_avoid <<
        dendl;
...
void *Accepter::entry()
{
    ...
    while (!done) {
        ldout(msgr->cct,
              20) << __func__ << " calling poll for sd:" << listen_sd << dendl;
        int r = poll(pfd, 2, -1);
        if (r < 0) {
            if (errno == EINTR) {
                continue;
        ...
        ldout(msgr->cct,
              10) << __func__ << " poll returned oke: " << r << dendl;
        ldout(msgr->cct,
              20) << __func__ << " pfd.revents[0]=" << pfd[0].revents << dendl;
        ldout(msgr->cct,
              20) << __func__ << " pfd.revents[1]=" << pfd[1].revents << dendl;
            ...
            if (::read(shutdown_rd_fd, &ch, 1) == -1) {
                if (errno != EAGAIN)
                    ldout(msgr->cct, 1) << __func__ << " Cannot read selfpipe: "
                        << " errno " << errno << " " << cpp_strerror(errno) <<
                        dendl;
            }
            break;
        }
        ...
        if (sd >= 0) {
            errors = 0;
            ldout(msgr->cct,
                  10) << __func__ << " incoming on sd " << sd << dendl;
            ...
            int e = errno;
            ldout(msgr->cct,
                  0) << __func__ << " no incoming connection?  sd = " << sd <<
                " errno " << e << " " << cpp_strerror(e) << dendl;
            if (++errors > msgr->cct->_conf->ms_max_accept_failures) {
                lderr(msgr->
                      cct) <<
                    "accetper has encoutered enough errors, just do ceph_abort()."
                    << dendl;
                ceph_abort();
            }
        }
...
void Accepter::stop()
{
    ...
    ldout(msgr->cct,
          10) << __func__ << " accept listening on: " << listen_sd << dendl;
