...
namespace librbd {
    namespace mirror {
        namespace snapshot {
            ...
            class TestMockMirrorSnapshotCreatePrimaryRequest:public
                TestMockFixture {
              public:
                typedef CreatePrimaryRequest < MockTestImageCtx >
                    MockCreatePrimaryRequest;
                typedef UnlinkPeerRequest < MockTestImageCtx >
                ...
                void expect_clone_md_ctx(MockTestImageCtx & mock_image_ctx) {
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), clone())
                        .WillOnce(Invoke([&mock_image_ctx] () {
                                         ...
                                         get();
                                         return &get_mock_io_ctx(mock_image_ctx.
                                                                 md_ctx);}
                                  ));
                }
                ...
                void expect_can_create_primary_snapshot(MockUtils & mock_utils,
                    ...
                    EXPECT_CALL(mock_utils,
                                can_create_primary_snapshot(_, demoted, force,
                                                            nullptr))
                ...
                void expect_get_mirror_peers(MockTestImageCtx & mock_image_ctx,
                    ...
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(RBD_MIRRORING, _, StrEq("rbd"),
                                     StrEq("mirror_peer_list"), _, _, _, _))
                        .
                        WillOnce(DoAll
                                 (WithArg < 5 > (CopyInBufferlist(bl)),
                                  Return(r)));
                }

                void expect_create_snapshot(MockTestImageCtx & mock_image_ctx,
                                            int r) {
                    EXPECT_CALL(*mock_image_ctx.operations,
                                snap_create(_, _, _, _, _))
                        .
                        WillOnce(DoAll
                                 (Invoke
                                  ([this, &mock_image_ctx,
                                   ...
                                   snap_create(mock_image_ctx, mirror_ns,
                                               snap_name);}
                                  ),
                                  WithArg < 4 >
                                  (CompleteContext
                ...
                void expect_unlink_peer(MockTestImageCtx & mock_image_ctx,
                                        ...
                                        bool is_linked, bool complete,
                                        bool allow_remove, int r) {
                    EXPECT_CALL(mock_unlink_peer_request, send())
                        .
                        WillOnce(Invoke
                                 ([&mock_image_ctx, &mock_unlink_peer_request,
                                   snap_id, peer_uuid, is_linked, complete,
                                   allow_remove, r] () {
                                  ASSERT_EQ(mock_unlink_peer_request.
                                            mirror_peer_uuid, peer_uuid);
                                  ASSERT_EQ(mock_unlink_peer_request.snap_id,
                                            snap_id);
                                  ASSERT_EQ(mock_unlink_peer_request.
                                  ...
                                  queue(mock_unlink_peer_request.on_finish, r);}
                                 )) ;
                }
            };
            ...
            TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, Success) {
                ...
                expect_can_create_primary_snapshot(mock_utils, false, false,
                                                   true);
                expect_get_mirror_peers(mock_image_ctx, { {
                                        "uuid",
                                        cls::rbd::MIRROR_PEER_DIRECTION_TX,
                                        "ceph", "mirror", "mirror uuid"}
                                        }
                                        , 0);
                ...
                C_SaferCond ctx;
                auto req =
                    new MockCreatePrimaryRequest(&mock_image_ctx, "gid",
                                                 CEPH_NOSNAP,
                                                 0U, 0U, nullptr, &ctx);
                req->send();
            ...
            TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest,
                ...
                librbd::ImageCtx * ictx;
                ASSERT_EQ(0, open_image(m_image_name, &ictx));
                ictx->config.set_val("rbd_mirroring_max_mirroring_snapshots",
                                     "3");
                ...
                expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request,
                                   snap_id, "uuid", true, false, true, 0);
                C_SaferCond ctx;
                auto req =
                    new MockCreatePrimaryRequest(&mock_image_ctx, "gid",
                                                 CEPH_NOSNAP,
            ...
            TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest,
                ...
                auto snap_id = it->first;
                expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request,
                                   snap_id, "uuid", false, true, true, 0);
            ...
            TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest,
                ...
                auto snap_id = it->first;
                expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request,
                                   snap_id, "uuid1", true, true, true, 0);
                expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request,
                                   snap_id, "uuid2", true, true, true, 0);
                C_SaferCond ctx;
                auto req =
                    new MockCreatePrimaryRequest(&mock_image_ctx, "gid",
                                                 CEPH_NOSNAP,
