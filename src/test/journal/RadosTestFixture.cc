...
journal::JournalMetadataPtr RadosTestFixture::create_metadata(const std::
                                                              string & oid,
                                                              const std::
                                                              string &
                                                              client_id,
                                                              ...
                                                              max_concurrent_object_sets)
{
    ...
    journal::JournalMetadataPtr metadata(new journal::
                                         JournalMetadata(m_work_queue, m_timer,
...
bool RadosTestFixture::wait_for_update(journal::JournalMetadataPtr metadata)
{
    ...
    while (m_listener.updates[metadata.get()] == 0) {
        if (m_listener.cond.WaitInterval(m_listener.mutex, utime_t(10, 0)) != 0) {
            return false;
        }
    }
