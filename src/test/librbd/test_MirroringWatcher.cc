...
namespace librbd {
    ...
    namespace {
        ...
        struct MockMirroringWatcher:public MirroringWatcher <> {
            ...
            :MirroringWatcher <> (image_ctx.md_ctx, image_ctx.op_work_queue) {
             ...
             MOCK_METHOD3(handle_image_updated,
                          void (cls::rbd::MirrorImageState, const std::string &,
                                const std::string &));
        };
    ...
    TEST_F(TestMirroringWatcher, ModeUpdated) {
        ...
        C_SaferCond ctx;
        MockMirroringWatcher::notify_mode_updated(m_ioctx,
                                                  cls::rbd::
                                                  MIRROR_MODE_DISABLED, &ctx);
        ASSERT_EQ(0, ctx.wait());
    }
