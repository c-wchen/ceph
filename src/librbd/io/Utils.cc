...
void apply_op_flags(uint32_t op_flags, uint32_t flags,
                    neorados::Op *op)
{
...
} bool assemble_write_same_extent(const LightweightObjectExtent &
                                  object_extent,
...
template < typename I >
void read_parent(I *image_ctx, uint64_t object_no,
                 ...
                 Context *on_finish)
{
    ...
    if (raw_overlap > 0) {
        ...
        object_overlap =
            image_ctx->prune_parent_extents(parent_extents, area,
                                            raw_overlap, false);
    }
    if (object_overlap == 0) {
        image_locker.unlock();
    ...
    if (read_extents->size() > 1) {
        auto parent_comp =
            new ReadResult::C_ObjectReadMergedExtents(cct,
                read_extents,
                on_finish);
    ...
    ldout(cct, 20) << "completion=" << comp
                   ...
                   << " area=" << area << dendl;
    auto req =
        io::ImageDispatchSpec::create_read(*image_ctx->parent,
                                           io::
                ...
                uint64_t out_bl_len)
{
    ...
    destriper.add_partial_sparse_result(cct, std::move(*bl),
    extent_map, bl_off, { {
            0, out_bl_len
        }
    });
...
template < typename I >
bool trigger_copyup(I *image_ctx, uint64_t object_no,
                    IOContext io_context, Context *on_finish)
{
    bufferlist bl;
    auto req =
        new ObjectWriteRequest < I > (image_ctx, object_no, 0,
                                      std::move(bl), io_context, 0,
                                      0,
...
template < typename I >
void area_to_object_extents(I *image_ctx, uint64_t offset,
                            ...
                            object_extents)
{
    ...
    image_ctx->io_image_dispatcher->remap_to_physical(extents,
            area);
    for (auto[off, len] : extents) {
        Striper::file_to_extents(image_ctx->cct, &image_ctx->layout,
                                 off, len, 0, buffer_offset,
                                 object_extents);
    }
}
...
template < typename I >
std::pair < Extents,
    ImageArea > object_to_area_extents(I *image_ctx,
                                       uint64_t object_no,
                                       const Extents &
                                       object_extents)
{
    ...
    return {
        std::move(extents), area};
}

template < typename I >
uint64_t area_to_raw_offset(const I &image_ctx,
                            uint64_t offset, ImageArea area)
{
...
template < typename I >
std::pair < uint64_t,
    ImageArea > raw_to_area_offset(const I &image_ctx,
                                   uint64_t offset)
{
    Extents extents = { {offset, 0} };
    auto area =
...
template void librbd::io::util::read_parent(librbd::ImageCtx *image_ctx,
        uint64_t object_no,
        ReadExtents *extents,
        librados::snap_t snap_id,
        const ZTracer::Trace &trace,
        Context *on_finish);
template int librbd::io::util::clip_request(librbd::ImageCtx *image_ctx,
        Extents *image_extents,
        ImageArea area);
template bool librbd::io::util::trigger_copyup(librbd::ImageCtx *image_ctx,
        uint64_t object_no,
        IOContext io_context,
        Context *on_finish);
template void librbd::io::util::area_to_object_extents(librbd::ImageCtx *
        image_ctx,
        uint64_t offset,
        uint64_t length,
        ImageArea area,
        ...
        * object_extents);
template auto librbd::io::util::object_to_area_extents(librbd::ImageCtx *
        image_ctx,
        uint64_t object_no,
        const Extents &extents)
->std::pair < Extents, ImageArea >;
template uint64_t librbd::io::util::area_to_raw_offset(const librbd::
        ImageCtx &image_ctx,
        uint64_t offset,
        ImageArea area);
template auto librbd::io::util::raw_to_area_offset(const librbd::
        ImageCtx &image_ctx,
        uint64_t offset)
