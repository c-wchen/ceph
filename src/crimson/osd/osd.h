...
namespace ceph::os {
    ...
    class OSD final:public crimson::net::Dispatcher,
        private crimson::common::AuthHandler, private crimson::mgr::WithStats {
        const int whoami;
      ...
      public:
        ...
        static seastar::future < OSDMeta >
            open_or_create_meta_coll(crimson::os::FuturizedStore & store);
        static seastar::future <> mkfs(crimson::os::FuturizedStore & store,
                                       unsigned whoami, uuid_d osd_uuid,
                                       uuid_d cluster_fsid,
      ...
      private:
        static seastar::future <> _write_superblock(crimson::os::
                                                    FuturizedStore & store,
                                                    OSDMeta meta,
                                                    OSDSuperblock superblock);
        static seastar::future <> _write_key_meta(crimson::os::
                                                  FuturizedStore & store);
        seastar::future <> start_boot();
        seastar::future <> _preboot(version_t oldest_osdmap,
        ...
        bool require_mon_peer(crimson::net::Connection * conn,
                              Ref < Message > m);
        ...
        seastar::future <> handle_peering_op(crimson::net::ConnectionRef conn,
                                             Ref < MOSDPeeringOp > m);
        seastar::future <> handle_recovery_subreq(crimson::net::
                                                  ConnectionRef conn,
        ...
        seastar::future <> handle_update_log_missing(crimson::net::
                                                     ConnectionRef conn,
                                                     Ref <
                                                     MOSDPGUpdateLogMissing >
                                                     m);
        seastar::future <> handle_update_log_missing_reply(crimson::net::
                                                           ConnectionRef conn,
                                                           Ref <
