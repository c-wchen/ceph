...
void cls_replica_log_item_marker::dump(Formatter * f) const const
{
...
void cls_replica_log_progress_marker::dump(Formatter * f) const const
{
...
generate_test_instances(std::list < cls_replica_log_progress_marker * >&ls)
{
    ...
    std::list < cls_replica_log_item_marker * >::iterator i =
        test_items.begin();
...
void cls_replica_log_bound::dump(Formatter * f) const const
{
    encode_json("position_marker", position_marker, f);
    encode_json("position_time", position_time, f);
