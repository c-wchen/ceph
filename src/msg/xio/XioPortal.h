...
class XioPortal:public Thread {
  ...
  public:
    explicit XioPortal(Messenger * _msgr, int max_conns):msgr(_msgr), ctx(NULL),
        server(NULL), submit_q(), xio_uri(""), portal_id(NULL),
        _shutdown(false), drained(false), magic(0), special_handling(0) {
        ...
        ctx =
            xio_context_create(&ctx_params, 0 /* poll timeout */ ,
                               -1 /* cpu hint */ );
                ...
                while (q_iter != send_q.end()) {
                    ...
                    case XioSubmit::OUTGOING_MSG:  /* it was an outgoing 1-way */
                        ...
                        if (unlikely(!xcon->conn || !xcon->is_connected()))
                            ...
                            if (unlikely
                                ((xcon->send_ctr + xsend->get_msg_count()) >
                                 xio_qdepth_high)) {
...
class XioPortals {
  ...
  public:
     XioPortals(Messenger * msgr, int _n, int nconns):p_vec(NULL),
        last_unused(0) {
        n = max(_n, 1);
    ...
    int accept(struct xio_session *session,
        ...
        if (pix == 0) {
            ...
            return xio_accept(session,
                              (const char **)&(portals_vec[pix]), 1, NULL, 0);
        }
    }
