...
namespace seastar_pingpong {
    ...
    struct Server {
        ...
        struct ServerDispatcher final:crimson::net::Dispatcher {
            ...
            std::optional <
                seastar::future <>> ms_dispatch(crimson::net::ConnectionRef c,
                                                MessageRef m) final {
    ...
    void run(seastar::app_template & app, int argc, char **argv) {
        app.run(argc, argv,[this] {
                ...
                auto init_params = ceph_argparse_early_args(args,
                                                            ...
                                                            &cluster,
                                                            &conf_file_list);
                return crimson::common::sharded_conf().start(init_params.name,
                                                             cluster)
                .then([conf_file_list] {
                      return local_conf().parse_config_files(conf_file_list);}
                       ...
                       return seastar::do_with(seastar::
                                               pollable_fd(std::move(on_end)),[]
                                               (seastar::
                                                pollable_fd & on_end_fds) {
                                               return on_end_fds.readable().
                                               then([&on_end_fds] {
                                                    eventfd_t result = 0;
                                                    on_end_fds.get_file_desc().
                                                    read(&result,
                                                         sizeof(result));
                                                    return seastar::
                                                    make_ready_future <> ();}
...
seastar_echo(const entity_addr_t addr, echo_role role, unsigned count)
{
    ...
    if (role == echo_role::as_server) {
        return seastar::do_with(seastar_pingpong::Server {
                                ...
                                std::
                                cout << "server listening at " << addr << std::
                                endl;
                                // bind the server
                                server.msgr->
                                ...
                                server.msgr->set_auth_client(&server.
                                                             dummy_auth);
                                server.msgr->set_auth_server(&server.
                                                             dummy_auth);
                                return server.msgr->bind(entity_addrvec_t {
                                                         addr}
                                ).safe_then([&server] {
                                            return server.msgr->start( {
                                                                      &server.
                                                                      dispatcher}
                                            );}
                                            ,
                                ...
                                then([&dispatcher = server.dispatcher, count] {
                                     return dispatcher.on_reply.
                                     wait([&dispatcher, count] {
                                          return dispatcher.count >= count;}
                                     );}
                                ).finally([&server] {
                                          std::
                                          cout << "server shutting down" <<
                                          std::endl; server.msgr->stop();
        ...
        return seastar::do_with(seastar_pingpong::Client {
                                ...
                                std::
                                cout << "client sending to " << addr << std::
                                endl;
                                client.msgr->
                                set_default_policy(crimson::net::SocketPolicy::
                                                   lossy_client(0));
                                ...
                                client.msgr->set_auth_client(&client.
                                                             dummy_auth);
                                client.msgr->set_auth_server(&client.
                                                             dummy_auth);
                                return client.msgr->start( {
                                                          &client.dispatcher}
                                ).then([addr, &client, &disp =
                                        client.dispatcher, count] {
                                       auto conn =
                                       client.msgr->connect(addr,
                                                            entity_name_t::
                                                            TYPE_OSD);
                                       return seastar::do_until([&disp, count] {
                                                                return disp.
                                                                count >= count;
                                                                }
                                                                ,[&disp, conn] {
...
int main(int argc, char **argv)
{
    ...
    auto job = sc.with_seastar([&]{
                               ...
                               std::transform(begin(unrecognized_options),
                                              ...
                                              return const_cast <
                                              char *>(s.c_str());});
                               sc.run(app, av.size(), av.data()); job.join();}
