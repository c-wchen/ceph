                ...
                switch (t) {
                    ...
                    default:
                        ...
                        if (need_quiescelock && (lock->get_cap_shift() > 0)) {
                            dout(20) << "need shared quiesce lock for " << p << " on " << SimpleLock::get_lock_type_name(
                                         t) << " of " << in << dendl;
...
void Locker::handle_inode_file_caps(const cref_t < MInodeFileCaps > &m)
{
    ...
    dout(7) << "handle_inode_file_caps replica mds." << from << " wants caps " << ccap_string(
                m->get_caps()) << " on " << *in << dendl;
...
void Locker::handle_client_caps(const cref_t < MClientCaps > &m)
{
    ...
    if (m->get_client_tid() > 0 && session &&
        ...
        if (op == CEPH_CAP_OP_FLUSHSNAP) {
            ...
            ack = make_message < MClientCaps > (CEPH_CAP_OP_FLUSHSNAP_ACK, m->get_ino(), 0, 0, 0, 0, 0, dirty, 0,
                                                mds->get_osd_epoch_barrier());
        } else {
            if (mds->logger) {
                mds->logger->inc(l_mdss_ceph_cap_op_flush_ack);
            }
            ack = make_message < MClientCaps > (CEPH_CAP_OP_FLUSH_ACK, m->get_ino(), 0, m->get_cap_id(), m->get_seq(),
    ...
    if (op == CEPH_CAP_OP_FLUSHSNAP) {
        ...
        ref_t < MClientCaps > ack;
        if (dirty) {
            ack = make_message < MClientCaps > (CEPH_CAP_OP_FLUSHSNAP_ACK, in->ino(), 0, 0, 0, 0, 0, dirty, 0,
                                                mds->get_osd_epoch_barrier());
            ack->set_snap_follows(follows);
            ack->set_client_tid(m->get_client_tid());
    ...
    for (xlist < Capability * >::iterator p = revoking_caps.begin(); !p.end(); ++p) {
        ...
        if (age > mds->mdsmap->get_session_timeout() * (1 << cap->get_num_revoke_warnings())) {
            ...
            *css << "client." << cap->get_client() << " isn't responding to mclientcaps(revoke), ino "
            ...
            dout(20) << __func__ << " silencing log message (backoff) for " << "client." << cap->get_client() << "." <<
...
void Locker::revoke_client_leases(SimpleLock *lock)
{
    ...
    for (map < client_t, ClientLease * >::iterator p = dn->client_lease_map.begin();
        ...
        CInode *diri = dn->get_dir()->get_inode();
        auto lease = make_message < MClientLease > (CEPH_MDS_LEASE_REVOKE, l->seq, mask, diri->ino(), diri->first, CEPH_NOSNAP,
                     dn->get_name());
