...
class RGWChainedCache {
  public:
    ...
    struct Entry {
        ...
        Entry(RGWChainedCache * _c, const std::string & _k, void *_d):cache(_c),
            key(_k), data(_d) {
    }};
};
...
class ObjectCache {
    ...
    void touch_lru(const DoutPrefixProvider * dpp, const std::string & name,
                   ObjectCacheEntry & entry,
                   std::list < std::string >::iterator & lru_iter);
    void remove_lru(const std::string & name,
                    std::list < std::string >::iterator & lru_iter);
  ...
  public:
    ...
    int get(const DoutPrefixProvider * dpp, const std::string & name,
            ObjectCacheInfo & bl, uint32_t mask,
            rgw_cache_entry_info * cache_info);
    std::optional < ObjectCacheInfo > get(const DoutPrefixProvider * dpp,
                                          const std::string & name) {
        std::optional < ObjectCacheInfo > info {
        std::in_place};
    ...
    void put(const DoutPrefixProvider * dpp, const std::string & name,
             ObjectCacheInfo & bl, rgw_cache_entry_info * cache_info);
    bool invalidate_remove(const DoutPrefixProvider * dpp,
                           const std::string & name);
