...
compress_tp(cct, "AsyncCompressor::compressor_tp", "tp_async_compr",
            cct->_conf->async_compressor_threads, "async_compressor_threads"),
job_lock("AsyncCompressor::job_lock"), compress_wq(this,
                                                   c->_conf->
...
uint64_t AsyncCompressor::async_compress(bufferlist & data)
{
    ...
    ldout(cct,
          10) << __func__ << " insert async compress job id=" << id << dendl;
    return id;
}
...
uint64_t AsyncCompressor::async_decompress(bufferlist & data)
{
    ...
    compress_wq.queue(&it.first->second);
    ldout(cct,
          10) << __func__ << " insert async decompress job id=" << id << dendl;
    return id;
}

int AsyncCompressor::get_compress_data(uint64_t compress_id, bufferlist & data,
                                       bool blocking, bool * finished)
{
    assert(finished);
    Mutex::Locker l(job_lock);
    ...
    if (it == jobs.end() || !it->second.is_compress) {
        ldout(cct,
              10) << __func__ << " missing to get compress job id=" <<
            compress_id << dendl;
        return -ENOENT;
    }
  ...
  retry:
    ...
    if (status == status_t::DONE) {
        ...
        *finished = true;
        ...
        ldout(cct,
              20) << __func__ << " compressed data failed, job id=" <<
            compress_id << dendl;
        jobs.erase(it);
        return -EIO;
    }
        ...
        if (it->second.status.compare_exchange_strong(expected, status_t::DONE)) {
            ldout(cct,
                  10) << __func__ << " compress job id=" << compress_id <<
                " hasn't finished, abort!" << dendl;
            if (compressor->compress(it->second.data, data)) {
                ldout(cct,
                      1) << __func__ << " compress job id=" << compress_id <<
                    " failed!" << dendl;
...
int AsyncCompressor::get_decompress_data(uint64_t decompress_id,
                                         bufferlist & data, bool blocking,
                                         bool * finished)
{
    assert(finished);
    Mutex::Locker l(job_lock);
    ...
    if (it == jobs.end() || it->second.is_compress) {
        ldout(cct,
              10) << __func__ << " missing to get decompress job id=" <<
            decompress_id << dendl;
        return -ENOENT;
    }
  retry:
    ...
    if (status == status_t::DONE) {
        ...
        *finished = true;
        ...
        if (it->second.status.compare_exchange_strong(expected, status_t::DONE)) {
            ldout(cct,
                  10) << __func__ << " decompress job id=" << decompress_id <<
