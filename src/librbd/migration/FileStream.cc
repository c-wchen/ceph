...
#define dout_prefix *_dout << "librbd::migration::FileStream::ReadRequest " \
        ...
        template < typename I > struct FileStream <I >::ReadRequest {
            ...
            :file_stream(file_stream), byte_extents(std::move(byte_extents)),
                ...
                auto ptr =
                    buffer::ptr_node::create(buffer::
                                             create_small_page_aligned
                                             (byte_extent.second));
                auto buffer =
                    boost::asio::mutable_buffer(ptr->c_str(),
                                                byte_extent.second);
                data->push_back(std::move(ptr));

                int r;
                auto offset =
                    lseek64(file_stream->m_file_no, byte_extent.first,
                            SEEK_SET);
                if (offset == -1) {
                    r = -errno;
                    lderr(cct) << "failed to seek file stream: " <<
                ...
                size_t bytes_read =
                    boost::asio::read(*file_stream->m_stream_descriptor,
                                      std::move(buffer), ec);
                r = -ec.value();
                if (r < 0 && r != -ENOENT) {
                    lderr(cct) << "failed to read from file stream: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
                    ...
                    lderr(cct) << "failed to read " << byte_extent.
                        second << " bytes from " << "file stream" << dendl;
                    finish(-ERANGE);
                    return;
                }

                // re-queue the remainder of the read requests
                boost::asio::post(file_stream->m_strand,[this] () {
                                  read();
                                  }
                );
            }
...
#define dout_prefix *_dout << "librbd::migration::FileStream: " << this \
        ...
        template < typename I >
            FileStream < I >::FileStream(I * image_ctx,
                                         const json_spirit::
                                         mObject & json_object)
        ...
        template < typename I > void FileStream < I >::open(Context * on_finish) {
            ...
            if (file_path_value.type() != json_spirit::str_type) {
                lderr(m_cct) << "failed to locate '" << FILE_PATH << "' key" <<
                    dendl;
        ...
        template < typename I >
            void FileStream < I >::get_size(uint64_t * size,
                                            Context * on_finish) {
            ldout(m_cct, 10) << dendl;
            ...
            boost::asio::post(m_strand,[this, size, on_finish] () {
                              ...
                              if (offset == -1) {
                              int r = -errno;
                              lderr(m_cct) << "failed to seek to file end: " <<
                              cpp_strerror(r)
                              << dendl; on_finish->complete(r); return;}

                              ldout(m_cct, 10) << "size=" << offset << dendl;
                              *size = offset; on_finish->complete(0);}
            ) ;
        }

        template < typename I >
            void FileStream < I >::read(io::Extents
                                        && byte_extents, bufferlist * data,
                                        Context * on_finish) {
            ldout(m_cct, 20) << byte_extents << dendl;

            auto ctx =
                new ReadRequest(this, std::move(byte_extents), data, on_finish);

            // execute IO operations in a single strand to prevent seek races
            boost::asio::post(m_strand,[ctx] () {
                              ctx->send();
                              });
        }
