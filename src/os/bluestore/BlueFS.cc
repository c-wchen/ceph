...
:  
cct(cct),
bdev(MAX_BDEV), ioc(MAX_BDEV), block_all(MAX_BDEV), block_total(MAX_BDEV, 0)
{
}
...
int BlueFS::mkfs(uuid_d osd_uuid)
{
    ...
    for (unsigned bdev = 0; bdev < MAX_BDEV; ++bdev) {
        ...
        if (p.empty())
            continue;
        for (interval_set < uint64_t >::iterator q = p.begin(); q != p.end();
             ++q) {
            dout(20) << __func__ << " op_alloc_add " << bdev << " 0x" << std::
                hex << q.get_start() << "~" << q.get_len() << std::dec << dendl;
...
int BlueFS::mount()
{
  ...
  for (auto & p:file_map) {
    ...
    dout(10) << __func__ << " log write pos set to 0x"
        << std::hex << log_writer->pos << std::dec << dendl;
...
int BlueFS::_replay(bool noop)
{
    ...
    while (true) {
        ...
        {
            int r =
                _read(log_reader, &log_reader->buf, read_pos, super.block_size,
                      &bl, NULL);
        ...
        if (more) {
            ...
            bufferlist t;
            int r =
                _read(log_reader, &log_reader->buf, read_pos, more, &t, NULL);
            if (r < (int)more) {
            ...
            case bluefs_transaction_t::OP_JUMP:
                {
                    ...
                    if (skip) {
                        bufferlist junk;
                        int r =
                            _read(log_reader, &log_reader->buf, read_pos, skip,
                                  &junk,
                                  NULL);
                        if (r != (int)skip) {
...
int BlueFS::_read(FileReader * h,   ///< [in] read from here
    ...
    while (len > 0) {
        ...
        if (off < buf->bl_off || off >= buf->get_buf_end()) {
            ...
            dout(20) << __func__ << " fetching 0x"
                ...
                << " of " << *p << dendl;
            int r =
                bdev[p->bdev]->read(p->offset + x_off, l, &buf->bl,
                                    ioc[p->bdev],
                                    cct->_conf->bluefs_buffered_io);
            assert(r == 0);
        }
...
void BlueFS::_compact_log_async(std::unique_lock < std::mutex > &l)
{
    ...
    dout(10) << __func__ << " remove 0x" << std::hex << old_log_jump_to << std::
        dec << " of " << log_file->fnode.extents << dendl;
                                ...
                                uint64_t want_seq, uint64_t jump_to)
{
    ...
    if (want_seq && want_seq <= log_seq_stable) {
        dout(10) << __func__ << " want_seq " << want_seq <<
            " <= log_seq_stable " << log_seq_stable << ", done" << dendl;
        assert(!jump_to);
        return 0;
    }
    ...
    if (seq > log_seq_stable) {
        ...
        while (p != dirty_files.end()) {
            ...
            while (l != p->second.end()) {
                ...
                assert(file->dirty_seq <= log_seq_stable);
                dout(20) << __func__ << " cleaned file " << file->
                    fnode << dendl;
                file->dirty_seq = 0;
                p->second.erase(l++);
            }
    ...
    while (length > 0) {
        ...
        if (tail) {
            ...
            dout(20) << __func__ << " caching tail of 0x"
                << std::hex << tail
                << " and padding block with 0x" << zlen << std::dec << dendl;
...
int BlueFS::_allocate(uint8_t id, uint64_t len, bluefs_fnode_t * node)
{
    ...
    if (alloc_len < (int64_t) left) {
        ...
        if (id != BDEV_SLOW) {
            if (bdev[id]) {
                dout(1) << __func__ << " failed to allocate 0x" << std::
                    hex << left << " on bdev " << (int)id << ", free 0x" <<
                    alloc[id]->get_free()
                    << "; fallback to bdev " << (int)id +
                    1 << std::dec << dendl;
            }
...
int BlueFS::open_for_write(const string & dirname,
...
int BlueFS::open_for_read(const string & dirname,
                          const string & filename, FileReader ** h, bool random)
{
    std::lock_guard < std::mutex > l(lock);
