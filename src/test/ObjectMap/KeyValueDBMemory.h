...
class KeyValueDBMemory:public KeyValueDB {
  public:
    ...
    int get_keys(const string & prefix,
                 const std::set < string > &key, std::set < string > *out);

    int set(const string & prefix, const string & key, const bufferlist & bl);

    int rmkey(const string & prefix, const string & key);

    int rmkeys_by_prefix(const string & prefix);

    int rm_range_keys(const string & prefix,
    ...
    class TransactionImpl_:public TransactionImpl {
      public:
        ...
        void set(const string & prefix, const string & k,
        ...
        struct RmKeysByPrefixOp:public Context {
            KeyValueDBMemory *db;
            string prefix;
            RmKeysByPrefixOp(KeyValueDBMemory * db, const string & prefix)
            :db(db), prefix(prefix) {
            } void finish(int r) override {
        ...
        struct RmRangeKeys:public Context {
            ...
            RmRangeKeys(KeyValueDBMemory * db, const string & prefix,
                        const string & s, const string & e)
        ...
        void rm_range_keys(const string & prefix, const string & start,
                           const string & end) {
        ...
        int complete() {
            for (list < Context * >::iterator i = on_commit.begin();
                 i != on_commit.end(); on_commit.erase(i++)) {
                (*i)->complete(0);
            }
            return 0;
        }
    ...
    uint64_t get_estimated_size(map < string, uint64_t > &extras) override {
        ...
        for (map < pair < string, string >, bufferlist >::iterator p =
             db.begin(); p != db.end(); ++p) {
