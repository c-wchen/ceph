...
namespace rbd {
    namespace action {
        namespace merge_diff {
            ...
            static int parse_diff_header(int fd, __u8 * tag, string * from,
                                         string * to, uint64_t * size) {
                int r;

                {               //header
                    char buf[utils::RBD_DIFF_BANNER.size() + 1];
                     r = safe_read_exact(fd, buf,
                                         utils::RBD_DIFF_BANNER.size());
                    if (r < 0)
                         return r;
                    ...
                    if (strcmp(buf, utils::RBD_DIFF_BANNER.c_str())) {
                        std::
                            cerr << "invalid banner '" << buf << "', expected '"
            ...
            static int parse_diff_body(int fd, __u8 * tag, uint64_t * offset,
                                       uint64_t * length) {
                int r;
 ...
 */
            static int accept_diff_body(int fd, int pd, __u8 tag,
 ...
 */
            static int do_merge_diff(const char *first, const char *second,
                ...
                if (first_stdin) {
                    ...
                    if (fd < 0) {
                        r = -errno;
                        std::cerr << "rbd: error opening " << first << std::
                            endl;
                        goto done;
                    }
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to parse first diff header" <<
                        std::endl;
                    goto done;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to parse second diff header" <<
                        std::endl;
                    goto done;
                }
...

                {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to write merged diff header" <<
                            std::endl;
                        goto done;
                    }
                }
                ...
                while (!f_end || !s_end) {
                    ...
                    if (f_off < s_off && f_len) {
                        ...
                        if (r < 0) {
                            std::
                                cerr << "rbd: failed to merge diff chunk" <<
                                std::endl;
                            goto done;
                        }
                        f_off += delta;
                    ...
                    if (f_off < s_off + s_len && f_len) {
                        ...
                        if (f_tag == RBD_DIFF_WRITE) {
                            ...
                            if (r < 0) {
                                std::
                                    cerr <<
                                    "rbd: failed to skip first diff data" <<
                                    std::endl;
                                goto done;
                            }
                        }
            ...
            int execute(const po::variables_map & vm) {
                ...
                if (first_diff.empty()) {
                    std::cerr << "rbd: first diff was not specified" << std::
                        endl;
                    return -EINVAL;
                }
                ...
                if (second_diff.empty()) {
                    std::cerr << "rbd: second diff was not specified" << std::
                        endl;
                    return -EINVAL;
                }
                ...
                std::string path;
                int r =
                    utils::get_path(vm, utils::get_positional_argument(vm, 2),
                                    &path);
                if (r < 0) {
                    return r;
                }

                r = do_merge_diff(first_diff.c_str(), second_diff.c_str(),
                                  path.c_str(),
