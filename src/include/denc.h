...
namespace _denc {
    template < class T, class ... >struct is_any_of:std::false_type {
     ...
     template < class T, class Head, class ... Tail >
        ...
        std::true_type, is_any_of < T, Tail ... >>::type {
    };
} template < typename T >
    struct denc_traits <T,
...
namespace _denc {
    ...
    template < typename T > struct ExtType <T,
...
struct denc_traits <T,
    typename std::enable_if < !std::is_void < typename _denc::ExtType <
    T >::type >::value >::type > {
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = true;
...
template < typename T > inline void denc_varint(T & v, bufferptr::iterator & p)
{
...
template < typename T, typename traits = denc_traits < T >>
    ...
    !traits::featured >::type denc(const T & o, size_t & p, uint64_t f = 0)
{
...
template < typename T, typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    traits::featured >::type denc(const T & o, size_t & p, uint64_t f)
{
...
template < typename T, typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    !traits::featured >::type denc(const T & o,
                                   buffer::list::contiguous_appender & p,
                                   uint64_t features = 0)
{
    traits::encode(o, p);
...
template < typename T,
    ...
    inline typename std::enable_if < traits::supported &&
    has_legacy_denc::value >::type denc(T & o, buffer::list::iterator & p)
{
...
// bufferptr
//
template <> struct denc_traits <bufferptr > {
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = false;
    static constexpr bool need_contiguous = false;
    ...
    } static void encode(const bufferptr & v,
                         buffer::list::contiguous_appender & p, uint64_t f =
                         0) {
...
template <> struct denc_traits <bufferlist > {
    ...
    static void bound_encode(const bufferlist & v, size_t & p, uint64_t f = 0) {
        p += sizeof(uint32_t) + v.length();
    } static void encode(const bufferlist & v,
                         buffer::list::contiguous_appender & p, uint64_t f =
                         0) {
        ::denc((uint32_t) v.length(), p);
        p.append(v);
    }
    static void decode(bufferlist & v, buffer::ptr::iterator & p, uint64_t f =
                       0) {
        uint32_t len;
        ::denc(len, p);
        v.clear();
...
namespace _denc {
    template < template < class ... >class C, typename Details,
      ...
      public:
        ...
        static void decode(container & s, buffer::ptr::iterator & p,
                           uint64_t f = 0) {
            uint32_t num;
            denc(num, p);
        ...
        template < typename U = T >
            static typename std::enable_if < sizeof(U)
            && !need_contiguous >::type decode(container & s,
        ...
        template < typename U = T >
            static typename std::enable_if < sizeof(U) &&
            !traits::featured >::type
            encode_nohead(const container & s,
                          buffer::list::contiguous_appender & p) {
          for (const T & e:s) {
                denc(e, p);
            }
        ...
        template < typename U = T >
            static typename std::enable_if < sizeof(U)
            && !need_contiguous >::type decode_nohead(size_t num, container & s,
                                                      buffer::list::
                                                      iterator & p) {
            s.clear();
            Details::reserve(s, num);
    ...
    template < typename Container > struct reserve_switch <Container, true > {
        static void reserve(Container & c, size_t s) {
            c.reserve(s);
    }};

    template < typename Container > struct reserve_switch <Container, false > {
        static void reserve(Container & c, size_t s) {
    }};
...
template < typename T, typename ... Ts >
    ...
    typename std::enable_if < denc_traits <
    T >::supported >::type >:public _denc::container_base < std::list,
...
template < typename T, typename ... Ts >
    ...
    typename std::enable_if < denc_traits <
    T >::supported >::type >:public _denc::container_base < std::vector,
...
template < typename T, typename ... Ts >
    ...
    typename std::enable_if < denc_traits <
    T >::supported >::type >:public _denc::container_base < std::set,
...
template < typename T, typename ... Ts >
    ...
    typename std::enable_if < denc_traits <
    T >::supported >::type >:public _denc::container_base <
...
template < typename A, typename B, typename ... Ts >
    ...
    typename std::enable_if < denc_traits < A >::supported &&
    denc_traits < B >::supported >::type >:public _denc::container_base <
    std::map, _denc::maplike_details < std::map < A, B, Ts ... >>, A, B,
    Ts ... > {
};
...
namespace _denc {
    ...
    template < size_t ... S > struct build_indices_helper;
    template < size_t N, size_t ... Is > struct build_indices_helper <N, N, Is ... > {
        using type = indices < Is ... >;
    };
    template < size_t N, size_t I, size_t ... Is >
        struct build_indices_helper <N, I, Is ... > {
        using type =
            typename build_indices_helper < N, I + 1, Is ..., I >::type;
    };

    template < size_t I > struct build_indices {
        using type = typename build_indices_helper < I, 1, 0 >::type;
    };
    template <> struct build_indices <0 > {
        using type = indices <>;
    };
    template <> struct build_indices <1 > {
    ...
    template < typename ... Ts > struct tuple_traits;
    template < typename T, typename ... Ts > struct tuple_traits <T, Ts ... > {
    ...
    typename std::enable_if < _denc::tuple_traits <
    Ts ... >::supported >::type > {
  private:
    ...
    template < typename T, size_t I, size_t J, size_t ... Is >
        static void encode_helper_nf(const T & s,
                                     buffer::list::contiguous_appender & p,
                                     _denc::indices < I, J, Is ... >) {
        denc(std::get < I > (s), p);
    ...
    template < typename T, size_t I, size_t J, size_t ... Is >
        static void encode_helper_f(const T & s,
                                    buffer::list::contiguous_appender & p,
                                    uint64_t f, _denc::indices < I, J,
                                    Is ... >) {
        denc(std::get < I > (s), p, f);
...
template < typename T >
    ...
    template < typename U = T >
        static typename std::enable_if < sizeof(U) && !featured >::type
        encode(const boost::optional < T > &v,
               bufferlist::contiguous_appender & p) {
        denc((bool) v, p);
        if (v)
            denc(*v, p);
    ...
    template < typename U = T >
        static typename std::enable_if < sizeof(U) && featured >::type
        encode(const boost::optional < T > &v,
               bufferlist::contiguous_appender & p, uint64_t f) {
        denc((bool) v, p, f);
        if (v)
            denc(*v, p, f);
    }
...
template < typename T, typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    !traits::featured >::type encode(const T & o,
                                     bufferlist & bl,
...
template < typename T, typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    traits::featured >::type encode(const T & o, bufferlist & bl,
                                    uint64_t features)
{
    size_t len = 0;
...
template < typename T,
    typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    !traits::need_contiguous >::type decode(T & o, bufferlist::iterator & p)
{
    if (p.end())
...
template < typename T, typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    !traits::featured >::type encode_nohead(const T & o, bufferlist & bl)
{
    size_t len = 0;
    traits::bound_encode(o, len);
    auto a = bl.get_contiguous_appender(len);
...
template < typename T, typename traits = denc_traits < T >>
    inline typename std::enable_if < traits::supported &&
    !traits::featured >::type decode_nohead(size_t num,
                                            T & o, bufferlist::iterator & p)
{
    if (!num)
        return;
    if (p.end())
