...
struct HandlePayloadVisitor: public boost::static_visitor <
    void > {
    Watcher *watcher;
    uint64_t notify_id;
    uint64_t handle;
    ...
    inline void operator()(const P &payload)const
    {
        typename Watcher::C_NotifyAck *ctx =
            new typename Watcher::C_NotifyAck(watcher, notify_id,
                                              handle);
...
public:
    ...
    {
    } template < typename P > inline void operator()(P &payload) const
    {
