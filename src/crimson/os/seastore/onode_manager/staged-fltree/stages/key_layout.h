...
// Note: this is the reversed version of the object hash
    ...
    static_assert(sizeof(crush_hash_t) ==
                  sizeof(ghobject_t().hobj.get_bitwise_key_u32()));
    ...
    static laddr_t get_lba_hint(shard_t shard, pool_t pool, crush_hash_t crush) {
        ...
        return ((uint64_t) (shard & 0XFF) << 56 |
                (uint64_t) (pool & 0xFF) << 48 | (uint64_t) (crush) << 16);
 ...
 */
    struct string_key_view_t {
        ...
        static constexpr auto MARKER_MAX =
            std::numeric_limits < string_size_t >::max();
        static constexpr auto MARKER_MIN =
            std::numeric_limits < string_size_t >::max() - 1;
        ...
        static void append_str(NodeExtentMutable &, std::string_view,
                               char *&p_append);
        ...
        static void append_dedup(NodeExtentMutable &, const Type & dedup_type,
                                 char *&p_append);
 ...
 */
    class string_view_masked_t {
      public:
        ...
        static string_view_masked_t decode(std::string & str_storage,
                                           ceph::bufferlist::
    ...
    inline auto operator<=>(const string_view_masked_t & l,
                            const string_view_masked_t & r) {
        using Type = string_view_masked_t::Type;
    ...
    inline std::ostream & operator<<(std::ostream & os,
                                     const string_view_masked_t & masked) {
        using Type = string_view_masked_t::Type;
    ...
    struct ns_oid_view_t {
        ...
        template < typename Key >
            static void append(NodeExtentMutable &,
                               const Key & key, char *&p_append);

        static void append(NodeExtentMutable & mut,
                           const ns_oid_view_t & view, char *&p_append) {
            if (view.type() == Type::STR) {
                string_key_view_t::append_str(mut, view.nspace.to_string_view(),
                                              p_append);
                string_key_view_t::append_str(mut, view.oid.to_string_view(),
                                              p_append);
            }
            else {
    ...
    inline std::ostream & operator<<(std::ostream & os,
                                     const ns_oid_view_t & ns_oid) {
        return os << string_view_masked_t {
        ns_oid.nspace}
        <<"," << string_view_masked_t {
        ns_oid.oid};
    }
    inline auto operator<=>(const ns_oid_view_t & l, const ns_oid_view_t & r) {
        auto ret =
            (string_view_masked_t { l.nspace } <=>string_view_masked_t { r.
             nspace });
        if (ret != 0)
            return ret;
        return string_view_masked_t {
    ...
    class key_hobj_t {
      public:
        ...
        bool is_valid() const {
            ...
            std::string oid;
            [[maybe_unused]] auto oid_masked =
                string_view_masked_t::decode(oid, delta);
            // TODO(cross-node string dedup)
            ...
            return
                key_hobj_t(ghobject_t
                           (shard_id_t(shard), pool, crush, nspace, oid, snap,
                            gen));
        }
 ...
 */
   ...
   */
        bool has_shard_pool() const {
            ...
            return ghobject_t(shard_id_t(shard()), pool(), crush(),
                              std::string(nspace()), std::string(oid()), snap(),
                              gen());
        } void replace(const crush_t & key) {
            p_crush = &key;
        }
        ...
        void replace(const ns_oid_view_t & key) {
            p_ns_oid = key;
        }
        ...
        void replace(const snap_gen_t & key) {
            p_snap_gen = &key;
        }
    ...
    template < typename T >
        concept IsFullKey =
        std::same_as < T, key_hobj_t > ||std::same_as < T, key_view_t >;
...
// Note: this is the reversed version of the object hash
    ...
    struct shard_pool_crush_t {
        ...
        template < IsFullKey Key >
            ...
            return {
            shard_pool_t::from_key(key), crush_t::from_key(key)};
        }
    ...
    __attribute__ ((packed));
    inline std::ostream & operator<<(std::ostream & os,
                                     const shard_pool_crush_t & spc) {
        return os << spc.shard_pool << ",0x" << std::hex << spc.
            crush << std::dec;
    }
    ...
    template < IsFullKey LHS, IsFullKey RHS >
        std::strong_ordering operator<=>(const LHS & lhs,
    ...
    template < IsFullKey T >
        ...
        if (ret != 0)
            return ret;
        return key.oid() <=> string_view_masked_t {
        target.oid};
    }
    ...
    template < typename Key >
        ...
        if constexpr
            ...
            if (key.dedup_type() != Type::STR) {
                ...
                return 2 * sizeof(string_size_t) + key.nspace().size() +
                    key.oid().size();
            }
        }
    }
    ...
    template < typename Key >
        void ns_oid_view_t::append(NodeExtentMutable & mut, const Key & key,
                                   char *&p_append) {
        static_assert(IsFullKey < Key >);
        if (key.dedup_type() == Type::STR) {
...
template <> struct fmt::formatter <
    crimson::os::seastore::onode::key_hobj_t >:fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::onode::key_view_t >:fmt::ostream_formatter {
};
#endif
