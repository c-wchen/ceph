    ...
    namespace crypto {
        ...
        class CryptoInterface {
          ...
          public:
            ...
            virtual int encrypt(ceph::bufferlist * data,
                                uint64_t image_offset) = 0;
            virtual int decrypt(ceph::bufferlist * data,
                                uint64_t image_offset) = 0;
            ...
            inline std::pair < uint64_t,
                uint64_t > get_pre_and_post_align(uint64_t off, uint64_t len) {
                if (len == 0) {
                    return std::make_pair(0, 0);
                } auto block_size = get_block_size();
                 return std::make_pair(p2phase(off, block_size),
                                       p2nphase(off + len, block_size));
            } inline std::pair < uint64_t, uint64_t > align(uint64_t off,
                                                            uint64_t len) {
            ...
            inline void align_extents(const io::ReadExtents & extents,
              ...
              for (const auto & extent:extents) {
                    ...
                    aligned_extents->emplace_back(aligned.first,
                                                  aligned.second);
                }
            }
            ...
            inline int decrypt_aligned_extent(io::ReadExtent & extent,
                ...
                if (extent.extent_map.empty()) {
                    extent.extent_map.emplace_back(extent.offset,
                                                   extent.bl.length());
                }
              ...
              for (auto[off, len]:extent.extent_map) {
                    ...
                    if (aligned_off > curr_block_end_offset) {
                        curr_block_bl.append_zero(curr_block_end_offset -
                                                  curr_offset);
                        auto curr_block_length = curr_block_bl.length();
                        if (curr_block_length > 0) {
                            auto r = decrypt(&curr_block_bl,
                                             image_offset +
                                             curr_block_start_offset -
                                             extent.offset);
                            if (r != 0) {
                                return r;
                            }

                            curr_block_bl.splice(0, curr_block_length,
                                                 &result_bl);
                            result_extent_map.
                                emplace_back(curr_block_start_offset,
                                             curr_block_length);
                        }
