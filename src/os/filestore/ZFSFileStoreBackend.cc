...
int ZFSFileStoreBackend::update_current_zh()
{
    ...
    if (zh) {
        ...
        dout(0) << "update_current_zh: zfs_open '" << path << "' got NULL" <<
            dendl;
        return -ENOENT;
    }
    ...
    if (zh) {
        ...
        zfs.close(zh);
        dout(0) <<
            "update_current_zh: basedir and current/ on the same filesystem" <<
            dendl;
    }
    else {
...
int ZFSFileStoreBackend::list_checkpoints(list < string > &ls)
{
    ...
    list < string > snaps;
    int ret =
        zfs.iter_snapshots_sorted(current_zh, list_checkpoints_callback,
                                  &snaps);
    if (ret < 0) {
        ret = -errno;
...
int ZFSFileStoreBackend::create_checkpoint(const string & name, uint64_t * cid)
{
    ...
    char path[PATH_MAX];
    snprintf(path, sizeof(path), "%s@%s", zfs.get_name(current_zh),
             name.c_str());
...
int ZFSFileStoreBackend::rollback_to(const string & name)
{
    ...
    if (zfs.is_mounted(current_zh, NULL)) {
        ...
        if (ret < 0) {
            ret = -errno;
            dout(0) << "rollback_to: zfs_umount '" << zfs.
                get_name(current_zh) << "' got" << cpp_strerror(ret) << dendl;
        }
    }
    ...
    if (ret < 0) {
        ret = -errno;
        dout(0) << "rollback_to: zfs_rollback '" << zfs.
            get_name(snap_zh) << "' got" << cpp_strerror(ret) << dendl;
    }
    ...
    if (!zfs.is_mounted(current_zh, NULL)) {
        ...
        if (ret < 0) {
            ret = -errno;
            dout(0) << "update_current_zh: zfs_mount '" << zfs.
                get_name(current_zh) << "' got " << cpp_strerror(ret) << dendl;
            return ret;
        }
    }
