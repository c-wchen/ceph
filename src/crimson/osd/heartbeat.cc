...
void Heartbeat::add_reporter_peers(int whoami)
{
    ...
    auto min_down =
        local_conf().get_val < uint64_t > ("mon_osd_min_down_reporters");
    auto subtree =
        local_conf().get_val < string > ("mon_osd_reporter_subtree_level");
...
void Heartbeat::ms_handle_reset(crimson::net::ConnectionRef conn,
                                bool is_replace)
{
...
bool Heartbeat::Connection::matches(crimson::net::ConnectionRef _conn) const const
{
...
void Heartbeat::Peer::send_heartbeat(clock::time_point now,
