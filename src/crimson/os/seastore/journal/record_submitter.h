...
public:
    ...
    evaluation_t evaluate_submit(const record_size_t &rsize,
                                 extent_len_t block_size) const
    {
        assert(!needs_flush());
        auto submit_size =
            pending.size.get_encoded_length_after(rsize, block_size);
    ...
    add_pending_ret add_pending(const std::string &name,
                                record_t &&, extent_len_t block_size);

    // Encode the batched records for write.
    std::pair < ceph::bufferlist,
        record_group_size_t > encode_batch(const journal_seq_t &committed_to,
    ...
    std::pair < ceph::bufferlist,
        record_group_size_t > submit_pending_fast(record_t
                &&,
...
private:
    record_group_size_t get_encoded_length_after(const record_t &record,
            extent_len_t block_size)
    const
    {
        return pending.size.get_encoded_length_after(record.size,
                block_size);
    ...
    std::optional < seastar::shared_promise < maybe_promise_result_t >
    >io_promise;
};
...
public:
    ...
    // whether is available to submit a record bool is_available() const;
