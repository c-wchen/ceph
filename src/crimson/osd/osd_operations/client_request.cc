...
void ClientRequest::Orderer::requeue(ShardServices &shard_services,
                                     Ref < PG > pg)
{
    for (auto &req : list) {
...
ClientRequest::ClientRequest(ShardServices &shard_services,
                             crimson::net::ConnectionRef conn,
                             Ref < MOSDOp > &&m)
    :    put_historic_shard_services(&shard_services), conn(std::move(conn)),
         m(std::move(m)), instance_handle(new instance_handle_t)
{
}
...
seastar::future <> ClientRequest::
with_pg_int(ShardServices &shard_services, Ref < PG > pgref)
{
    epoch_t same_interval_since = pgref->get_interval_start_epoch();
    logger().debug("{} same_interval_since: {}", *this,
                   same_interval_since);
    if (m->finish_decode()) {
        m->clear_payload();
    }
    ...
    return interruptor::with_interruption([this, pgref, this_instance_id,
                                           &ihref,
    &shard_services]()mutable {
        PG &pg = *pgref;
        if (pg.can_discard_op(*m)) {
            return shard_services.
            send_incremental_map(std::
                                 ref(*conn),
                                 m->
                                 get_map_epoch
                                 ()
                                ).
            then([this, this_instance_id,
            pgref] {
                logger().
                debug("{}.{}: discarding",
                      *this,
        ...
        return ihref.enter_stage <
               interruptor > (pp(pg).await_map,
                              *this).
    ...
    , pgref)
    .finally([opref = std::move(opref), pgref = std::move(pgref),
...
seastar::future <> ClientRequest::with_pg(ShardServices &shard_services,
        Ref < PG > pgref)
{
...
ClientRequest::interruptible_future <> ClientRequest::process_pg_op(Ref < PG > &pg)
{
    ...
    std::move(pg)](MURef < MOSDOpReply > reply) {
...
auto ClientRequest::reply_op_error(const Ref < PG > &pg, int err)
{
    ...
    auto reply =
        crimson::make_message < MOSDOpReply > (m.get(), err,
            pg->get_osdmap_epoch(),
            m->
            get_flags() &
            (CEPH_OSD_FLAG_ACK |
...
{
    return ihref.enter_stage < interruptor > (pp(*pg).recover_missing,
    *this).then_interruptible([this, pg]()mutable {
        if
        (pg->
         is_primary
         ()) {
            ...
            ().
            debug
            ("process_op: Skipping do_recover_missing"
             "on non primary pg");
            return
            interruptor::
            now
    ...
    .handle_error_interruptible(PG::load_obc_ertr::
                                all_same_way([this, pg =
                                            std::
                                            move(pg)](const auto &
    code) {
        logger().
        error
        ("ClientRequest saw error code {}",
         code);
        assert(code.value() > 0);
        return reply_op_error(pg,
                              -code.
                              value
                              ());
    }
...
ClientRequest::interruptible_future <> ClientRequest::do_process(instance_handle_t &ihref,
        Ref < PG > &pg,
        crimson::osd::ObjectContextRef obc)
{
    ...
    return pg->do_osd_ops(m, conn, obc, op_info,
                          snapc).safe_then_unpack_interruptible([this, pg,
                                  &ihref]
                                  (auto
                                   submitted,
                                   auto
...
} const SnapContext ClientRequest::get_snapc(Ref < PG > &pg,
        crimson::osd::
        ObjectContextRef obc) const
{
    SnapContext snapc;
    if (op_info.may_write() || op_info.may_cache()) {
        // snap
