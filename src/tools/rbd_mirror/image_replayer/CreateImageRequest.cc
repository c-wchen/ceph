...
namespace rbd {
    namespace mirror {
        namespace image_replayer {
            ...
            template < typename I >
                CreateImageRequest < I >::CreateImageRequest(Threads < I >
                                                             *threads,
                                                             librados::
                                                             IoCtx &
                                                             local_io_ctx,
            ...
            template < typename I >
                ...
                auto & config {
                reinterpret_cast <
                        CephContext * >(m_local_io_ctx.cct())->_conf};

                librbd::ImageOptions image_options;
                populate_image_options(&image_options);

                auto req =
                    librbd::image::CreateRequest < I >::create(config,
                                                               m_local_io_ctx,
                                                               m_local_image_name,
                                                               m_local_image_id,
                                                               m_remote_image_ctx->
            ...
            template < typename I >
                ...
                dout(10) << "r=" << r << dendl;
                if (r == -EBADF) {
                    dout(5) << "image id " << m_local_image_id <<
                        " already in-use" << dendl;
                    finish(r);
                    return;
                }
                else if (r < 0) {
                    derr << "failed to create local image: " << cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
                }
            ...
            template < typename I >
                ...
                if (r == 0) {
                    ...
                    if (r == 0) {
                        ...
                        dout(15) << "parent_global_image_id=" <<
            ...
            template < typename I > void CreateImageRequest < I >::clone_image() {
                ...
                int r =
                    m_pool_meta_cache->
                    get_local_pool_meta(m_local_parent_io_ctx.get_id(),
                                        &local_parent_pool_meta);
                if (r < 0) {
                    derr <<
                        "failed to retrieve local parent mirror uuid for pool "
                ...
                cls::rbd::SnapshotNamespace snap_namespace;
                {
                    auto snap_prefix =
                        image_sync::util::
                        get_snapshot_name_prefix(local_parent_pool_meta.
                                                 mirror_uuid);

                    std::
                        shared_lock
                        remote_image_locker(m_remote_parent_image_ctx->
                                            image_lock);
                  for (auto snap_info:m_remote_parent_image_ctx->
                         snap_info) {
                        if (snap_info.first == m_remote_parent_spec.snap_id) {
                            ...
                            snap_namespace = snap_info.second.snap_namespace;
                                 ...
                                 starts_with(snap_info.second.name,
                                             snap_prefix)) {
                ...
                if (!found_parent_snap) {
                    dout(15) << "remote parent image snapshot not found" <<
                        dendl;
                    m_ret_val = -ENOENT;
                    close_remote_parent_image();
                    return;
                    ...
                    dout(15) << "parent image not synced to local cluster" <<
                        dendl;
                    m_ret_val = -ENOENT;
                    close_remote_parent_image();
                    return;
                ...
                librbd::image::CloneRequest < I > *req =
                    librbd::image::CloneRequest < I >::create(config,
            ...
            template < typename I >
                ...
                if (r == -EBADF) {
                    ...
                    derr << "failed to clone image " << m_parent_pool_name <<
                        "/" << m_remote_parent_spec.
                        image_id << " to " << m_local_image_name << dendl;
                    m_ret_val = r;
                }
            ...
            template < typename I >
              ...
              for (auto & snap_info_pair:m_remote_image_ctx->
                ...
                int r =
                    librbd::util::create_ioctx(m_remote_image_ctx->md_ctx,
                                               "remote parent pool",
                ...
                m_local_parent_io_ctx.set_namespace(m_remote_parent_io_ctx.
                                                    get_namespace());
            ...
            template < typename I >
                void CreateImageRequest <
                I >::populate_image_options(librbd::ImageOptions *
                                            image_options) {
                ...
                if (m_remote_parent_spec.pool_id != -1) {
                    ...
                    if (m_remote_image_ctx->
                        test_op_features(RBD_OPERATION_FEATURE_CLONE_CHILD)) {
                        clone_format = 2;
                    }
                    image_options->set(RBD_IMAGE_OPTION_CLONE_FORMAT,
                                       clone_format);
                }
            }
