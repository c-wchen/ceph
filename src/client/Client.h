...
struct dir_result_t {
    ...
    struct dentry {
        ...
        } dentry(int64_t o, std::string n, std::string an,
                 InodeRef in):offset(o), name(std::move(n)),
            alternate_name(std::move(an)), inode(std::move(in)) {
    }};
...
class Client:public Dispatcher, public md_config_obs_t {
  public:
    ...
    typedef int (*add_dirent_cb_t) (void *p, struct dirent * de,
    ...
    static UserPerm pick_my_perms(CephContext * c) {
        uid_t uid =
            c->_conf->client_mount_uid >= 0 ? c->_conf->client_mount_uid : -1;
        gid_t gid =
            c->_conf->client_mount_gid >= 0 ? c->_conf->client_mount_gid : -1;
    ...
    int mds_command(const std::string & mds_spec,
   ...
   */
    ...
    int readdirplus_r(dir_result_t * dirp, struct dirent *de,
                      struct ceph_statx *stx, unsigned want, unsigned flags,
                      Inode ** out);

    int getdir(const char *relpath, std::list < std::string > &names, const UserPerm & perms);  // get the whole dir at once.
   ...
   */
    ...
    int readlink(const char *path, char *buf, loff_t size,
                 const UserPerm & perms);
    int readlinkat(int dirfd, const char *relpath, char *buf, loff_t size,
                   const UserPerm & perms);

    int symlink(const char *existing, const char *newname,
                const UserPerm & perms, std::string alternate_name = "");
    int symlinkat(const char *target, int dirfd, const char *relpath,
                  const UserPerm & perms, std::string alternate_name = "");

    // path traversal for high-level interface
    int walk(std::string_view path, struct walk_dentry_result *result,
             const UserPerm & perms, bool followsym = true);
    ...
    int fsetattrx(int fd, struct ceph_statx *stx, int mask,
                  const UserPerm & perms);
    int chmod(const char *path, mode_t mode, const UserPerm & perms);
    int fchmod(int fd, mode_t mode, const UserPerm & perms);
    int chmodat(int dirfd, const char *relpath, mode_t mode, int flags,
                const UserPerm & perms);
    ...
    int utimes(const char *relpath, struct timeval times[2],
               const UserPerm & perms);
    int lutimes(const char *relpath, struct timeval times[2],
                const UserPerm & perms);
    ...
    int utimensat(int dirfd, const char *relpath, struct timespec times[2],
                  int flags, const UserPerm & perms);
    int flock(int fd, int operation, uint64_t owner);
    int truncate(const char *path, loff_t size, const UserPerm & perms);

    // file ops
    int mknod(const char *path, mode_t mode, const UserPerm & perms,
              dev_t rdev = 0);

    int create_and_open(int dirfd, const char *relpath, int flags,
    ...
    int open(const char *path, int flags, const UserPerm & perms, mode_t mode =
    ...
    int lookup_hash(inodeno_t ino, inodeno_t dirino, const char *name,
                    const UserPerm & perms);
    int lookup_ino(inodeno_t ino, const UserPerm & perms, Inode ** inode =
                   NULL);
    ...
    int pwritev(int fd, const struct iovec *iov, int iovcnt, loff_t offset =
                -1);
    ...
    int get_snap_info(const char *path, const UserPerm & perms,
                      SnapInfo * snap_info);
    ...
    int get_file_extent_osds(int fd, loff_t off, loff_t * len,
                             std::vector < int >&osds);
    ...
    int get_osd_crush_location(int id, std::vector < std::pair < std::string,
                               std::string > >&path);
    ...
    int mksnap(const char *path, const char *name, const UserPerm & perm,
               mode_t mode =
               0, const std::map < std::string, std::string > &metadata = {
               });
    ...
    int ll_lookup_inode(struct inodeno_t ino, const UserPerm & perms,
                        Inode ** inode);
    ...
    int64_t ll_readv(struct Fh *fh, const struct iovec *iov, int iovcnt,
                     int64_t off);
    int64_t ll_writev(struct Fh *fh, const struct iovec *iov, int iovcnt,
                      int64_t off);
    ...
    void maybe_update_snaprealm(SnapRealm * realm, snapid_t snap_created,
    ...
    void handle_cap_export(MetaSession * session, Inode * in,
                           const MConstRef < MClientCaps > &m);
    void handle_cap_trunc(MetaSession * session, Inode * in,
                          const MConstRef < MClientCaps > &m);
    void handle_cap_flush_ack(MetaSession * session, Inode * in, Cap * cap,
                              const MConstRef < MClientCaps > &m);
    void handle_cap_flushsnap_ack(MetaSession * session, Inode * in,
                                  const MConstRef < MClientCaps > &m);
   ...
   */
    ...
    Dentry *insert_dentry_inode(Dir * dir, const std::string & dname,
  ...
  protected:
    ...
    int resolve_mds(const std::string & mds_spec,
                    std::vector < mds_gid_t > *targets);

    void get_session_metadata(std::map < std::string,
    ...
    void _closed_mds_session(MetaSession * s, int err = 0, bool rejected =
                             false);
    ...
    void handle_client_request_forward(const MConstRef < MClientRequestForward >
                                       &reply);
    ...
    int path_walk(const filepath & fp, struct walk_dentry_result *result,
                  const UserPerm & perms, bool followsym = true, int mask =
                  0, InodeRef dirinode = nullptr);
    int path_walk(const filepath & fp, InodeRef * end, const UserPerm & perms,
                  bool followsym = true, int mask = 0, InodeRef dirinode =
                  nullptr);
    ...
    void update_snap_trace(MetaSession * session, const bufferlist & bl,
                           SnapRealm ** realm_ret, bool must_flush = true);
   ...
   */
    ...
    int fill_stat(Inode * in, struct stat *st, frag_info_t * dirstat =
                  0, nest_info_t * rstat = 0);
    int fill_stat(InodeRef & in, struct stat *st, frag_info_t * dirstat =
                  0, nest_info_t * rstat = 0) {
    ...
    Inode *get_quota_root(Inode * in, const UserPerm & perms, quota_max_t type =
                          QUOTA_ANY);
    ...
    void handle_client_reclaim_reply(const MConstRef < MClientReclaimReply >
                                     &reply);
      ...
      public:
        ...
        /* The state migration check */
            ...
            if (require == Client::CLIENT_MOUNTING &&
                ...
                return false;
        } mount_state_t(state_t state, const char *lockname,
      ...
      public:
        ...
        /* The state migration check */
            ...
            if (require == Client::CLIENT_INITIALIZING &&
                ...
                return false;
        } initialize_state_t(state_t state, const char *lockname,
                             uint64_t reader_cnt = 0)
      :    RWRefState(state, lockname, reader_cnt) {
        }
        ~initialize_state_t() {
  ...
  private:
    ...
    void fill_dirent(struct dirent *de, const char *name, int type,
    ...
    int _do_lookup(Inode * dir, const std::string & name, int mask,
                   InodeRef * target, const UserPerm & perms);

    int _lookup(Inode * dir, const std::string & dname, int mask,
                InodeRef * target, const UserPerm & perm,
                std::string * alternate_name = nullptr);

    int _link(Inode * in, Inode * dir, const char *name, const UserPerm & perm,
              std::string alternate_name, InodeRef * inp = 0);
    int _unlink(Inode * dir, const char *name, const UserPerm & perm);
    int _rename(Inode * olddir, const char *oname, Inode * ndir,
                const char *nname, const UserPerm & perm,
                std::string alternate_name);
    int _mkdir(Inode * dir, const char *name, mode_t mode,
    ...
    int _ll_setattrx(Inode * in, struct ceph_statx *stx, int mask,
                     const UserPerm & perms, InodeRef * inp = 0);
    int _getattr(Inode * in, int mask, const UserPerm & perms, bool force =
                 false);
    int _getattr(InodeRef & in, int mask, const UserPerm & perms, bool force =
                 false) {
    ...
    int _do_setxattr(Inode * in, const char *name, const void *value,
                     size_t len, int flags, const UserPerm & perms);
    int _setxattr(Inode * in, const char *name, const void *value, size_t len,
                  int flags, const UserPerm & perms);
    int _setxattr(InodeRef & in, const char *name, const void *value,
                  size_t len, int flags, const UserPerm & perms);
    int _setxattr_check_data_pool(std::string & name, std::string & value,
                                  const OSDMap * osdmap);
    void _setxattr_maybe_wait_for_osdmap(const char *name, const void *value,
                                         size_t len);
    ...
    int _create(Inode * in, const char *name, int flags, mode_t mode,
    ...
    int _vxattrcb_fscrypt_auth_set(Inode * in, const void *val, size_t size,
                                   const UserPerm & perms);
    bool _vxattrcb_fscrypt_file_exists(Inode * in);
    size_t _vxattrcb_fscrypt_file(Inode * in, char *val, size_t size);
    int _vxattrcb_fscrypt_file_set(Inode * in, const void *val, size_t size,
                                   const UserPerm & perms);
    ...
    void _update_lock_state(struct flock *fl, uint64_t owner,
    ...
    int _posix_acl_permission(Inode * in, const UserPerm & perms,
                              unsigned want);

    mds_rank_t _get_random_up_mds() const;

    int _ll_getattr(Inode * in, int caps, const UserPerm & perms);
    int _lookup_parent(Inode * in, const UserPerm & perms, Inode ** parent =
                       NULL);
    int _lookup_name(Inode * in, Inode * parent, const UserPerm & perms);
    int _lookup_vino(vinodeno_t ino, const UserPerm & perms, Inode ** inode =
                     NULL);
