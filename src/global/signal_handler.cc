                         ...
                         std::map < std::string, std::string > *extra)
{
    if (g_ceph_context && g_ceph_context->_conf->crash_dir.size()) {
        ...
        snprintf(base, PATH_MAX, "%s/%s",
                 g_ceph_context->_conf->crash_dir.c_str(), id.c_str());
...
!handler_tid.compare_exchange_strong(expected, ceph_gettid())) {
    ...
    snprintf(buf, sizeof(buf), "*** Caught signal (%s) **\n "
             ...
             (unsigned long long)pthread_self(), pthread_name);
#else
    snprintf(buf, sizeof(buf), "*** Caught signal (%s) **\n "
             "in thread %llx thread_name:%s\n", sig_str(signum),
             (unsigned long long)pthread_self(), pthread_name);
    ...
    // we got the SEGV.
    if (g_ceph_context &&
        g_ceph_context->_log && !g_ceph_context->_log->is_inside_log_lock()) {
...
void SignalHandler::register_handler(int signum, signal_handler_t handler,
                                     bool oneshot)
{
    int r;
