    ...
    namespace crypto {
        namespace luks {
            ...
            struct TestMockCryptoLuksFormatRequest:public TestMockFixture {
                typedef FormatRequest < librbd::MockImageCtx >
                    MockFormatRequest;
                ...
                void verify_header(const char *expected_format,
                    ...
                    if (magic_switched) {
                        ...
                        ASSERT_EQ(-EINVAL,
                                  non_switched_header.load(expected_format));
                        ASSERT_EQ(0,
                    ...
                    ASSERT_EQ(0,
                              header.read_volume_key(passphrase_cstr,
                    ...
                    ASSERT_EQ(0,
                              std::memcmp(volume_key, crypto->get_key(),
                                          expected_key_length));
                    ASSERT_EQ(expected_sector_size, crypto->get_block_size());
                    ASSERT_EQ(header.get_data_offset(),
                              crypto->get_data_offset());
                }
            };

            TEST_F(TestMockCryptoLuksFormatRequest, LUKS1) {
                auto mock_format_request =
                    MockFormatRequest::create(mock_image_ctx,
                ...
                ASSERT_NO_FATAL_FAILURE(verify_header
                                        (CRYPT_LUKS1, 32, 512, false));
            }
