...
InstanceWatcher::InstanceWatcher(librados::IoCtx &ioctx,
    ...
    : Watcher(ioctx, instance_oid(stringify(ioctx.get_instance_id())),
              work_queue), m_ioctx(ioctx), m_listener(listener),
                                    ...
                                    bufferlist &bl)
{
    ...
    try {
        JSONDecoder jd(bl);
        JSONDecoder::decode_json("dir_path", dir_path, &jd.parser,
                                 true);
    ...
    dout(20) << ": notifier_id=" << notifier_id << ", dir_path=" <<
             dir_path << ", mode=" << mode << dendl;
...
void InstanceWatcher::register_watcher()
{
    ...
    InstanceWatcher,
    &InstanceWatcher::handle_register_watcher > (this);
...
void InstanceWatcher::unregister_watcher()
{
    ...
    Context *on_finish = new C_CallbackAdapter <
    InstanceWatcher,
    &InstanceWatcher::handle_unregister_watcher > (this);
    unregister_watch(new C_AsyncCallback < ContextWQ >
