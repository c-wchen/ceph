...
m_filestore_fsync_flushes_journal_data(cct()->_conf->
...
int GenericFileStoreBackend::detect_features()
{
    ...
    if (!m_filestore_fiemap) {
        ...
        if (r < 0) {
            dout(0) << "detect_features: FIEMAP ioctl is NOT supported" <<
                dendl;
            ioctl_fiemap = false;
        }
        else {
            if (fiemap->fm_mapped_extents == 0) {
                dout(0) <<
                    "detect_features: FIEMAP ioctl is supported, but buggy -- upgrade your kernel"
                    << dendl;
                ioctl_fiemap = false;
            }
            else {
    ...
    if (!m_filestore_seek_data_hole) {
        ...
        if (hole_pos < 0) {
            if (errno == EINVAL) {
                dout(0) <<
                    "detect_features: lseek SEEK_DATA/SEEK_HOLE is NOT supported"
                    << dendl;
                seek_data_hole = false;
            }
            else {
    ...
    if (!m_filestore_splice) {
        ...
        if (pipe_cloexec(pipefd) < 0) {
            int e = errno;
            dout(0) << "detect_features: splice pipe met error " <<
    ...
    if (::syncfs(get_basedir_fd()) == 0) {
        ...
        dout(0) <<
            "detect_features: syncfs(2) syscall supported by glibc BUT NOT the kernel"
            << dendl;
    }
    ...
    if (syscall(SYS_syncfs, get_basedir_fd()) == 0) {
        ...
        dout(0) <<
            "detect_features: syscall(SYS_syncfs, fd) supported by libc BUT NOT the kernel"
            << dendl;
    }
    ...
    if (syscall(__NR_syncfs, get_basedir_fd()) == 0) {
        ...
        dout(0) <<
            "detect_features: syscall(__NR_syncfs, fd) supported by libc BUT NOT the kernel"
            << dendl;
    }
#endif
    if (!have_syncfs) {
        ...
        if (m_filestore_fsync_flushes_journal_data) {
            dout(0) <<
                "detect_features: no syncfs(2), but 'filestore fsync flushes journal data = true', so fsync will suffice."
                << dendl;
        }
        else {
            dout(0) << "detect_features: no syncfs(2), must use sync(2)." <<
                dendl;
            dout(0) <<
                "detect_features: WARNING: multiple ceph-osd daemons on the same host will be slow"
                << dendl;
        }
    }
...
int GenericFileStoreBackend::syncfs()
{
    ...
    if (m_filestore_fsync_flushes_journal_data) {
        ...
        dout(15) << "syncfs: doing a full sync (syncfs(2) if possible)" <<
            dendl;
