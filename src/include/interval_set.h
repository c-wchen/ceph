...
template < typename T, typename Map = std::map < T, T >> class interval_set {
  public:
    ...
    class iterator:public std::iterator < std::forward_iterator_tag, T > {
      public:
        ...
        // For the copy constructor and assignment operator, the compiler-generated functions, which// perform simple bitwise copying, should be fine.
        ...
        // Return the interval length. T get_len() const {
            return _iter->second;
        }
        // Set the interval length. void set_len(T len) {
            _iter->second = len;
        }
    ...
    class const_iterator:public std::iterator < std::forward_iterator_tag, T > {
      public:
        ...
        // Return the interval start. T get_start() const {
            return _iter->first;
        }
        // Return the interval length. T get_len() const {
            return _iter->second;
        }
        // Preincrement const_iterator & operator++() {
            ++_iter;
            return *this;
        }
  ...
  interval_set():_size(0) {
    ...
    typename interval_set < T, Map >::const_iterator begin() const {
        ...
        return typename interval_set < T,
            Map >::const_iterator(find_inc(start));
  ...
  // helpers private:
    ...
    void intersection_size_asym(const interval_set & s, const interval_set & l) {
        ...
        while (1) {
            ...
            assert(en > start);
            typename decltype(m)::value_type i {
            start, en - start};
            mi = m.insert(mi, i);
            _size += i.second;
  ...
  public:
    ...
    // outer range of set bool empty() const {
        return m.empty();
    } T range_start() const {
        assert(!empty());
    ...
    // interval start after p (where p not in set) bool starts_after(T i) const {
        assert(!contains(i));
        typename Map::const_iterator p = find_inc(i);
        if (p == m.end())
            return false;
    ...
    // interval end that contains start T end_after(T start) const {
    ...
    void subtract(const interval_set & a) {
        for (typename Map::const_iterator p = a.m.begin(); p != a.m.end(); p++)
...
template < typename T, typename Map >
    ...
    static constexpr bool need_contiguous =
        denc_traits < T, Map >::need_contiguous;
...
template < class T, typename Map >
    inline std::ostream & operator<<(std::ostream & out, const interval_set < T,
                                     Map > &s)
{
    out << "[";
    const char *prequel = "";
