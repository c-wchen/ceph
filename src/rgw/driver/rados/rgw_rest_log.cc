...
void RGWOp_MDLog_List::execute(optional_yield y)
{
    ...
    if (s->info.args.exists("start-time") || s->info.args.exists("end-time")) {
        ldpp_dout(this,
                  5) << "start-time and end-time are no longer accepted" <<
            dendl;
        op_ret = -EINVAL;
        return;
    }
    ...
    if (!max_entries_str.empty()) {
        ...
        if (!err.empty()) {
            ldpp_dout(this,
                      5) << "Error parsing max-entries " << max_entries_str <<
                dendl;
            op_ret = -EINVAL;
            return;
        }
    ...
    meta_log.init_list_entries(shard_id, {
                               }
                               , {
                               }
                               , marker, &handle);
...
void RGWOp_MDLog_List::send_response()
{
    ...
    s->formatter->dump_bool("truncated", truncated);
    {
        ...
        for (list < cls_log_entry >::iterator iter = entries.begin();
            ...
            static_cast <
                rgw::sal::RadosStore *
                >(driver)->ctl()->meta.mgr->dump_log_entry(entry, s->formatter);
            flusher.flush();
        }
...
void RGWOp_MDLog_Info::execute(optional_yield y)
{
    ...
    period =
        static_cast <
        rgw::sal::RadosStore *
        >(driver)->svc()->mdlog->read_oldest_log_period(y, s);
...
void RGWOp_MDLog_Delete::execute(optional_yield y)
{
    ...
    if (s->info.args.exists("end-marker")) {
        if (!s->info.args.exists("marker")) {
            ...
            ldpp_dout(this,
                      5) << "end-marker and marker cannot both be provided" <<
                dendl;
            op_ret = -EINVAL;
        }
    }
    ...
    op_ret = meta_log.trim(this, shard_id, {
                           }
                           , {
                           }
                           , {
...
void RGWOp_MDLog_Lock::execute(optional_yield y)
{
    ...
    if (period.empty() ||
        shard_id_str.empty() ||
        (duration_str.empty()) || locker_id.empty() || zone_id.empty()) {
    ...
    if (!err.empty()) {
        ldpp_dout(this,
                  5) << "Error parsing shard_id param " << shard_id_str <<
            dendl;
        op_ret = -EINVAL;
        return;
    }
...
void RGWOp_MDLog_Unlock::execute(optional_yield y)
{
    ...
    if (period.empty() ||
        shard_id_str.empty() || locker_id.empty() || zone_id.empty()) {
        ldpp_dout(this, 5) << "Error invalid parameter list" << dendl;
        op_ret = -EINVAL;
        return;
    }
...
#define LARGE_ENOUGH_BUF (128 * 1024)
    ...
    if (driver->ctx()->_conf->subsys.should_gather < ceph_subsys_rgw, 20 > ()) {
        for (set < int >::iterator iter = updated_shards.begin();
            ...
            ldpp_dout(this,
                      20) << __func__ << "(): updated shard=" << *iter << dendl;
        }
    }
...
void RGWOp_BILog_List::execute(optional_yield y)
{
    ...
    if (bucket_name.empty() && bucket_instance.empty()) {
        ldpp_dout(this,
                  5) << "ERROR: neither bucket nor bucket instance specified" <<
            dendl;
        op_ret = -EINVAL;
        return;
    }
    ...
    string bn;
    op_ret =
        rgw_bucket_parse_bucket_instance(bucket_instance, &bn, &bucket_instance,
                                         &shard_id);
    if (op_ret < 0) {
        return;
    }
    ...
    if (logs.empty()) {
        ldpp_dout(s,
                  5) << "ERROR: bucket=" << bucket_name << " has no log layouts"
            << dendl;
        op_ret = -ENOENT;
        return;
    }
    ...
    if (gen) {
        ...
        if (log == logs.end()) {
            ldpp_dout(s,
                      5) << "ERROR: no log layout with gen=" << *gen << dendl;
            op_ret = -ENOENT;
            return;
        }
    }
    ...
    do {
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->bilog_rados->log_list(s,
                                                                           bucket->
                                                                           get_info
                                                                           (),
                                                                           log_layout,
...
void RGWOp_BILog_List::send_response_end()
{
    ...
    if (format_ver >= 2) {
        ...
        if (next_log_layout) {
            ...
            encode_json("num_shards",
                        rgw::num_shards(next_log_layout->layout.in_index.
                                        layout), s->formatter);
...
void RGWOp_BILog_Info::execute(optional_yield y)
{
    ...
    int ret =
        bucket->read_stats(s, index, shard_id, &bucket_ver, &master_ver, stats,
...
void RGWOp_BILog_Delete::execute(optional_yield y)
{
    ...
    if ((bucket_name.empty() && bucket_instance.empty()) || end_marker.empty()) {
        ldpp_dout(this,
                  5) <<
            "ERROR: one of bucket or bucket instance, and also end-marker is mandatory"
            << dendl;
        op_ret = -EINVAL;
        return;
    }
...
void RGWOp_DATALog_List::execute(optional_yield y)
{
    ...
    op_ret =
        static_cast <
        rgw::sal::RadosStore *
        >(driver)->svc()->datalog_rados->list_entries(this, shard_id,
                                                      max_entries, entries,
                                                      marker, &last_marker,
                                                      &truncated, y);
}
...
void RGWOp_DATALog_ShardInfo::execute(optional_yield y)
{
    ...
    op_ret =
        static_cast <
        rgw::sal::RadosStore * >(driver)->svc()->datalog_rados->get_info(this,
                                                                         shard_id,
                                                                         &info,
                                                                         y);
}
...
void RGWOp_DATALog_Delete::execute(optional_yield y)
{
    ...
    op_ret =
        static_cast <
        rgw::sal::RadosStore *
        >(driver)->svc()->datalog_rados->trim_entries(this, shard_id, marker,
                                                      y);
}
...
void RGWOp_MDLog_Status::execute(optional_yield y)
{
    auto sync =
        static_cast <
        rgw::sal::RadosStore * >(driver)->getRados()->get_meta_sync_manager();
    if (sync == nullptr) {
...
void RGWOp_BILog_Status::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  4) << "failed to read bucket info: " << cpp_strerror(op_ret)
            << dendl;
        return;
    }
    ...
    if (source_key.empty() || source_key == key) {
        ...
        op_ret =
            rgw_bucket_parse_bucket_key(s->cct, source_key, &source_bucket,
                                        nullptr);
        if (op_ret < 0) {
            ldpp_dout(this,
                      4) << "invalid 'source-bucket' provided (key=" <<
                source_key << ")" << dendl;
            return;
        }
    }
    ...
    if (!merge) {
        ...
        ldpp_dout(this,
                  20) <<
            "RGWOp_BILog_Status::execute(optional_yield y): getting sync status for pipe="
            << pipe << dendl;

        op_ret = rgw_read_bucket_full_sync_status(this,
                                                  static_cast <
                                                  rgw::sal::RadosStore *
                                                  >(driver), pipe,
                                                  &status.sync_status,
        ...
        status.inc_status.resize(status.sync_status.shards_done_with_gen.
                                 size());
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      -1) << "ERROR: rgw_read_bucket_inc_sync_status() on pipe="
    ...
    op_ret =
        driver->get_sync_policy_handler(s, source_zone_id, source_bucket,
                                        &source_handler, y);
    if (op_ret < 0) {
        ldpp_dout(this,
                  -1) << "could not get bucket sync policy handler (r=" <<
            op_ret << ")" << dendl;
        return;
    }
  ...
  for (auto & entry:local_dests) {
        ...
        if (*pipe.dest.bucket != pinfo->bucket) {
            ...
            op_ret =
                driver->get_bucket(s, nullptr, *pipe.dest.bucket, &dest_bucket,
                                   y);
            if (op_ret < 0) {
                ldpp_dout(this,
                          4) << "failed to read target bucket info (bucket=: "
        ...
        int r =
            rgw_read_bucket_inc_sync_status(this,
                                            static_cast <
        ...
        if (status.inc_status.empty()) {
            ...
            if (current_status.size() != status.inc_status.size()) {
                ...
                ldpp_dout(this,
                    ...
                    "syncing from the same source: status.size()= " << status.
                    inc_status.size()
                    << " current_status.size()=" << current_status.
                    size() << dendl;
                return;
            }
