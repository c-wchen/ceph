    ...
    namespace action {
        namespace lock {
             ...
             namespace {
                ...
                void add_id_option(po::options_description * positional) {
                    ...
                    *id = utils::get_positional_argument(vm, 1);
                    if (id->empty()) {
                        std::cerr << "rbd: lock id was not specified" << std::
                            endl;
                        return -EINVAL;
                    } return 0;
            }} // anonymous namespace
                ...
                if (f) {
                    ...
                    tbl.define_column("Locker", TextTable::LEFT,
                                      TextTable::LEFT);
                    tbl.define_column("ID", TextTable::LEFT, TextTable::LEFT);
                    tbl.define_column("Address", TextTable::LEFT,
                                      TextTable::LEFT);
                }
                ...
                if (lockers.size()) {
                    ...
                    for (std::list < librbd::locker_t >::const_iterator it =
                        ...
                        if (f) {
                            ...
                            tbl << it->client << it->cookie << it->
                                address << TextTable::endrow;
                        }
                    }
                    if (!f)
                        std::cout << tbl;
            ...
            int execute_list(const po::variables_map & vm) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: listing locks failed: " << cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_remove(const po::variables_map & vm) {
                ...
                r = do_lock_remove(image, lock_client.c_str(),
                                   lock_cookie.c_str());
                if (r < 0) {
                    std::
                        cerr << "rbd: releasing lock failed: " <<
                        cpp_strerror(r) << std::endl;
                    return r;
                }
                return 0;
