...
std::map < uint64_t, std::set < pg_t >> get_pgs_by_osd(CephContext *cct, int64_t pid,
...
float calc_deviations(CephContext *cct, const std::map < int, std::set < pg_t >> &pgs_by_osd,
                      const std::map < int, float > &osd_weight, float pgs_per_weight, std::map < int, float > &osd_deviation,
                      std::multimap < float, int > &deviation_osd, float &stddev);  // return current max deviation
