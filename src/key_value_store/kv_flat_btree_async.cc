...
{
    if (verbose)
        cout << "\t\t" << client_name << ": cleaning up after "
             << idata.str()
             << std::endl;
    int err = 0;
            ...
            case -EKEYREJECTED: {
                ...
                do {
                    ...
                    if (err < 0 && err != -ENOENT && err != -EBALANCE) {
                        if (verbose)
                            cerr << "\t" << client_name << ": split failed with "
                                 << err << std::endl;
        ...
        switch (err) {
            ...
            case -EKEYREJECTED: {
                ...
                do {
                    ...
                    if (err < 0 && err != -ENOENT && err != -EBALANCE) {
                        if (verbose)
                            cerr << "\t" << client_name << ": rebalance returned "
                                 << err << std::endl;
...
int KvFlatBtreeAsync::get(const string &key, bufferlist *val)
{
    ...
    if (verbose)
        cout << client_name << ": got " << key << " with " << err
             << std::endl;
...
int KvFlatBtreeAsync::set_many(const std::map<string, bufferlist> &in_map)
{
    ...
    if (verbose)
        cout << "finished making to_create and to_delete. "
             << std::endl;
...
int KvFlatBtreeAsync::remove_all()
{
    ...
    if (err < 0) {
        if (verbose)
            cout << "rm index aioc failed with " << err
                 << std::endl;
        return err;
    }
...
{
    if (verbose)
        cout << client_name << ": getting all keys and values"
             << std::endl;
    int err = 0;
...
bool KvFlatBtreeAsync::is_consistent()
{
    ...
    if (!ret) {
        if (verbose)
            cout << "failed consistency test - see error log"
                 << std::endl;
        cerr << str();
    } else {
        if (verbose) {
