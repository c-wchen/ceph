...
TEST(pick_address, find_ip_in_subnet_list)
{
    ...
    // match by network
    result = find_ip_in_subnet_list(g_ceph_context,
                                    &one, "10.1.0.0/16", "eth0");
    ASSERT_EQ((struct sockaddr *)&a_one, result);

    result = find_ip_in_subnet_list(g_ceph_context,
                                    &one, "10.2.0.0/16", "eth1");
    ASSERT_EQ((struct sockaddr *)&a_two, result);

    // match by eth name
    result = find_ip_in_subnet_list(g_ceph_context, &one, "10.0.0.0/8", "eth0");
    ASSERT_EQ((struct sockaddr *)&a_one, result);

    result = find_ip_in_subnet_list(g_ceph_context, &one, "10.0.0.0/8", "eth1");
    ASSERT_EQ((struct sockaddr *)&a_two, result);
}
