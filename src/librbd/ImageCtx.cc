...
namespace librbd {
    ...
    namespace {
        ...
        struct C_InvalidateCache:public Context {
            ...
            :image_ctx(_image_ctx), purge_on_error(_purge_on_error),
                ...
                if (r == -EBLACKLISTED) {
                    lderr(cct) << "Blacklisted during flush!  Purging cache..."
                        << dendl;
                ...
                loff_t unclean =
                    image_ctx->object_cacher->release_set(image_ctx->
                                                          object_set);
                if (unclean == 0) {
                    r = 0;
                }
    ...
    :cct((CephContext *) p.cct()),
        ...
        journal(NULL),
        owner_lock(util::
                   unique_lock_name("librbd::ImageCtx::owner_lock", this)),
        md_lock(util::unique_lock_name("librbd::ImageCtx::md_lock", this)),
        cache_lock(util::
                   unique_lock_name("librbd::ImageCtx::cache_lock", this)),
        snap_lock(util::unique_lock_name("librbd::ImageCtx::snap_lock", this)),
        parent_lock(util::
                    unique_lock_name("librbd::ImageCtx::parent_lock", this)),
        object_map_lock(util::
                        unique_lock_name("librbd::ImageCtx::object_map_lock",
                                         this)),
        async_ops_lock(util::
                       unique_lock_name("librbd::ImageCtx::async_ops_lock",
                                        this)),
        copyup_list_lock(util::
                         unique_lock_name("librbd::ImageCtx::copyup_list_lock",
                                          this)),
        completed_reqs_lock(util::
                            unique_lock_name
                            ("librbd::ImageCtx::completed_reqs_lock", this)),
        extra_read_flags(0), old_format(true), order(0), size(0), features(0),
        format_string(NULL), id(image_id), parent(NULL), stripe_unit(0),
        stripe_count(0), flags(0), object_cacher(NULL), writeback_handler(NULL),
        object_set(NULL), readahead(), total_bytes_read(0),
        state(new ImageState <> (this)), operations(new Operations <> (*this)),
        exclusive_lock(nullptr), object_map(nullptr), io_work_queue(nullptr),
        op_work_queue(nullptr), asok_hook(nullptr), trace_endpoint("librbd") {
        md_ctx.dup(p);
        data_ctx.dup(p);
        if (snap)
            snap_name = snap;
        ...
        io_work_queue =
            new io::ImageRequestWQ <> (this, "librbd::io_work_queue",
                                       cct->_conf->get_val < int64_t >
                                       ("rbd_op_thread_timeout"), thread_pool);
    ...
    void ImageCtx::init() {
        ...
        if (cache) {
            ...
            ldout(cct, 20) << "Initial cache settings:"
                ...
                << " max_dirty_age=" << cache_max_dirty_age << dendl;

            object_cacher = new ObjectCacher(cct, pname, *writeback_handler, cache_lock, NULL, NULL, cache_size, 10,    /* reset this in init */
                                             init_max_dirty,
                                             cache_target_dirty,
                                             cache_max_dirty_age,
            ...
            if (!obj) {
                obj =
                    MIN(2000,
                        MAX(10,
                            cache_size / 100 / sizeof(ObjectCacher::Object)));
            }
            ...
            object_set =
                new ObjectCacher::ObjectSet(NULL, data_ctx.get_id(), 0);
    ...
    void ImageCtx::perf_start(string name) {
        ...
        plb.add_u64_counter(l_librbd_discard_bytes, "discard_bytes",
                            "Discarded data", NULL, 0, unit_t(BYTES));
        ...
        plb.add_u64_counter(l_librbd_readahead_bytes, "readahead_bytes",
                            "Data size in read ahead", NULL, 0, unit_t(BYTES));
        plb.add_u64_counter(l_librbd_invalidate_cache, "invalidate_cache",
                            "Cache invalidates");
    ...
    int ImageCtx::get_parent_spec(snap_t in_snap_id, ParentSpec * out_pspec) const {
        const SnapInfo *info = get_snap_info(in_snap_id);
        if (info) {
    ...
    void ImageCtx::add_snap(cls::rbd::SnapshotNamespace in_snap_namespace,
        ...
        snap_info.insert( {
                         id, info});
        snap_ids.insert( { {
                        in_snap_namespace, in_snap_name}, id});
    }
    ...
    void ImageCtx::rm_snap(cls::rbd::SnapshotNamespace in_snap_namespace,
        ...
        snap_info.erase(id);
        snap_ids.erase( {
                       in_snap_namespace, in_snap_name});
    }
    ...
    void ImageCtx::aio_read_from_cache(object_t o, uint64_t object_no,
        ...
        ObjectCacher::OSDRead * rd =
            object_cacher->prepare_read(snap_id, bl, fadvise_flags);
        snap_lock.put_read();
    ...
    void ImageCtx::write_to_cache(object_t o, const bufferlist & bl, size_t len,
        ...
        ObjectCacher::OSDWrite * wr =
            object_cacher->prepare_write(snapc, bl, ceph::real_time::min(),
                                         fadvise_flags, journal_tid);
        snap_lock.put_read();
    ...
    void ImageCtx::user_flushed() {
        if (object_cacher && cache_writethrough_until_flush) {
            ...
            if (!flushed_before && max_dirty > 0) {
                ...
                ldout(cct,
                      10) << "saw first user flush, enabling writeback" <<
                    dendl;
                Mutex::Locker l(cache_lock);
    ...
    void ImageCtx::invalidate_cache(bool purge_on_error, Context * on_finish) {
        ...
        flush_cache(new
                    C_InvalidateCache(this, purge_on_error, false, on_finish));
    }
    ...
    uint64_t ImageCtx::prune_parent_extents(vector < pair < uint64_t,
                                            uint64_t > >&objectx,
                                            uint64_t overlap) {
        ...
        if (!objectx.empty()
            && objectx.back().first + objectx.back().second > overlap)
            objectx.back().second = overlap - objectx.back().first;

        uint64_t len = 0;
        for (vector < pair < uint64_t, uint64_t > >::iterator p =
             objectx.begin(); p != objectx.end(); ++p)
            len += p->second;
    ...
    void ImageCtx::cancel_async_requests(Context * on_finish) {
        {
            ...
            if (!async_requests.empty()) {
              ...
              for (auto req:async_requests) {
                    ldout(cct,
                          10) << "canceling async request: " << req << dendl;
                    req->cancel();
                }
    ...
    bool ImageCtx::_filter_metadata_confs(const string & prefix,
                                          ...
                                          const map < string,
                                          bufferlist > &pairs, map < string,
                                          bufferlist > *res) {
        size_t conf_prefix_len = prefix.size();

      for (auto it:pairs) {
            if (it.first.
                compare(0, MIN(conf_prefix_len, it.first.size()), prefix) > 0)
                return false;
            ...
            string key =
                it.first.substr(conf_prefix_len,
                                it.first.size() - conf_prefix_len);
    ...
    void ImageCtx::apply_metadata(const std::map < std::string,
                                  bufferlist > &meta, bool thread_safe) {
        ldout(cct, 20) << __func__ << dendl;
        std::map < string, bool > configs =
            boost::assign::map_list_of("rbd_non_blocking_aio",
                                       false) ("rbd_cache",
                                               false)
            ("rbd_cache_writethrough_until_flush", false) ("rbd_cache_size",
                                                           false)
            ("rbd_cache_max_dirty", false) ("rbd_cache_target_dirty",
                                            false) ("rbd_cache_max_dirty_age",
                                                    false)
            ("rbd_cache_max_dirty_object",
             false) ("rbd_cache_block_writes_upfront",
                     false) ("rbd_concurrent_management_ops",
                             false) ("rbd_balance_snap_reads",
                                     false) ("rbd_localize_snap_reads",
                                             false) ("rbd_balance_parent_reads",
                                                     false)
            ("rbd_localize_parent_reads",
             false) ("rbd_sparse_read_threshold_bytes",
                     false) ("rbd_readahead_trigger_requests",
                             false) ("rbd_readahead_max_bytes",
                                     false)
            ("rbd_readahead_disable_after_bytes",
             false) ("rbd_clone_copy_on_read",
                     false) ("rbd_blacklist_on_break_lock",
                             false) ("rbd_blacklist_expire_seconds",
                                     false) ("rbd_request_timed_out_seconds",
                                             false) ("rbd_journal_order",
                                                     false)
            ("rbd_journal_splay_width", false) ("rbd_journal_commit_age",
                                                false)
            ("rbd_journal_object_flush_interval",
             false) ("rbd_journal_object_flush_bytes",
                     false) ("rbd_journal_object_flush_age",
                             false) ("rbd_journal_pool",
                                     false) ("rbd_journal_max_payload_bytes",
                                             false)
            ("rbd_journal_max_concurrent_object_sets",
             false) ("rbd_mirroring_resync_after_disconnect",
                     false) ("rbd_mirroring_replay_delay",
                             false) ("rbd_skip_partial_discard", false);
