...
struct node_header_t {
    ...
    static_assert(static_cast < uint8_t >(field_type_t::_MAX) <=
                  1u << FIELD_TYPE_BITS);
    ...
    } static void bootstrap_extent(NodeExtentMutable &, field_type_t,
                                   node_type_t, bool, level_t);

    static void update_is_level_tail(NodeExtentMutable &,
                                     const node_header_t &, bool);

bits_t field_type:
    FIELD_TYPE_BITS;
bits_t node_type:
    NODE_TYPE_BITS;
bits_t is_level_tail:
    B_LEVEL_TAIL_BITS;
...
template < typename FixedKeyType, field_type_t _FIELD_TYPE > struct _slot_t {
    ...
    static constexpr node_offset_t OVERHEAD =
        sizeof(_slot_t) - sizeof(key_t);
...
node_range_t fields_free_range_before(const FieldType &node,
                                      index_t index,
...
template < typename SlotType > struct _node_fields_013_t {
    ...
    } node_offset_t get_key_start_offset(index_t index,
                                         extent_len_t node_size) const
    {
        ...
        return offset;
    } node_offset_t get_item_start_offset(index_t index,
                                          extent_len_t node_size) const
    {
        assert(index < num_keys);
    ...
    } extent_len_t get_item_end_offset(index_t index,
                                       extent_len_t node_size) const
    {
        return index == 0 ? node_size : get_item_start_offset(index - 1,
                node_size);
    } template < node_type_t NODE_TYPE >
    node_offset_t free_size_before(index_t index,
                                   extent_len_t node_size) const
    {
        auto range =
            fields_free_range_before < NODE_TYPE > (*this, index,
                node_size);
    ...
    static void insert_at(NodeExtentMutable &, const Key &key,
                          const me_t &node, index_t index,
                          node_offset_t size_right);
    static node_offset_t erase_at(NodeExtentMutable &, const me_t &,
                                  index_t, const char *);
    static void update_size_at(NodeExtentMutable &, const me_t &node,
                               index_t index, int change);
    static void append_key(NodeExtentMutable &, const key_t &key,
                           char *&p_append);
    template < IsFullKey Key >
    static void append_key(NodeExtentMutable &mut, const Key &key,
                           char *&p_append)
    {
        append_key(mut, key_t::from_key(key), p_append);
    }
    static void append_offset(NodeExtentMutable &mut,
...
struct node_fields_2_t {
    ...
    static void insert_at(NodeExtentMutable &mut, const Key &key,
                          const node_fields_2_t &node, index_t index,
                          node_offset_t size_right)
    {
        ceph_abort("not implemented");
    }
    static void update_size_at(NodeExtentMutable &mut,
                               const node_fields_2_t &node, index_t index,
                               int change)
    {
        ceph_abort("not implemented");
    }
    static void append_key(NodeExtentMutable &mut, const key_t &key,
                           char *&p_append)
    {
...
struct internal_fields_3_t {
    ...
    std::enable_if_t < NODE_TYPE == node_type_t::INTERNAL,
        node_offset_t > free_size_before(index_t index,
                                         extent_len_t node_size) const
    {
        assert(index <= num_keys);
    ...
    const laddr_packed_t *get_p_child_addr(index_t index,
                                           extent_len_t node_size) const
    {
#ifndef NDEBUG
        if (is_level_tail()) {
        ...
        auto p_addrs =
            reinterpret_cast <
            const laddr_packed_t * >(&keys[get_num_keys_limit(node_size)]);
