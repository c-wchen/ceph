...
// realm oids
    ...
    static std::string default_realm_oid(const ceph::common::ConfigProxy & conf) {
        return std::string {
        name_or_default(conf->rgw_default_realm_info_oid,
    ...
    int RadosConfigStore::delete_default_realm_id(const DoutPrefixProvider *
    ...
    int RadosConfigStore::create_realm(const DoutPrefixProvider * dpp,
        ...
        if (writer) {
            *writer =
                std::make_unique < RadosRealmWriter > (impl.get(),
                                                       std::move(objv),
                                                       info.get_id(),
                                                       info.get_name());
        }
    ...
    int RadosConfigStore::realm_notify_new_period(const DoutPrefixProvider *
                                                  dpp, optional_yield y,
