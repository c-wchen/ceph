...
TEST(LibCephFS, SnapQuota)
{
    ...
    ASSERT_EQ(0,
              ceph_setxattr(cmount, test_snap_dir_quota_xattr, xattrk,
                            (void *)xattrv, 5, XATTR_CREATE));

    // create subdir and set quota
    sprintf(test_snap_subdir_quota_xattr,
    ...
    ASSERT_EQ(0,
              ceph_setxattr(cmount, test_snap_subdir_quota_xattr, xattrk,
                            (void *)xattrv, 5, XATTR_CREATE));
    ...
    sprintf(c_temp,
            ...
            mypid, mypid);
    int alen =
        ceph_getxattr(cmount, c_temp, "ceph.quota.max_bytes", (void *)gxattrv,
                      xbuflen);
    ASSERT_LT(0, alen);
    ...
    sprintf(c_temp,
            "/.snap/test_snap_dir_quota_xattr_snap_%d/test_snap_dir_quota_xattr_%d/subdir_quota",
            mypid, mypid);
    alen =
        ceph_getxattr(cmount, c_temp, "ceph.quota.max_bytes", (void *)gxattrv,
                      xbuflen);
    ...
    sprintf(c_temp,
            "/.snap/test_snap_dir_quota_xattr_snap_%d/test_snap_dir_quota_xattr_%d/subdir_noquota",
            mypid, mypid);
    EXPECT_EQ(-CEPHFS_ENODATA,
              ceph_getxattr(cmount, c_temp, "ceph.quota.max_bytes",
...
TEST(LibCephFS, QuotaRealm)
{
    ...
    ASSERT_EQ(0,
              ceph_setxattr(cmount, test_quota_realm_pdir, xattrk,
                            (void *)xattrv, 7, XATTR_CREATE));

    // create child directory and set quota file
    sprintf(test_quota_realm_cdir, "%s/test_quota_realm_cdir",
    ...
    ASSERT_EQ(0,
              ceph_setxattr(cmount, test_quota_realm_cdir, xattrk,
                            (void *)xattrv, 4, XATTR_CREATE));
