...
#define dout_prefix *_dout << "librbd::migration::S3Stream: " << this \
        ...
        template < typename I >
            S3Stream < I >::S3Stream(I * image_ctx,
        ...
        template < typename I > void S3Stream < I >::open(Context * on_finish) {
            ...
            if (url_value.type() != json_spirit::str_type) {
                lderr(m_cct) << "failed to locate '" << URL_KEY << "' key" <<
                    dendl;
            ...
            if (access_key.type() != json_spirit::str_type) {
                lderr(m_cct) << "failed to locate '" << ACCESS_KEY << "' key" <<
                    dendl;
                on_finish->complete(-EINVAL);
                return;
            }
            ...
            if (secret_key.type() != json_spirit::str_type) {
                lderr(m_cct) << "failed to locate '" << SECRET_KEY << "' key" <<
                    dendl;
                on_finish->complete(-EINVAL);
                return;
            }
            ...
            if (util::is_config_key_uri(m_access_key)) {
                ...
                if (r < 0) {
                    lderr(m_cct) <<
                        "failed to retrieve access key from config: " <<
            ...
            if (util::is_config_key_uri(m_secret_key)) {
                ...
                if (r < 0) {
                    lderr(m_cct) <<
                        "failed to retrieve secret key from config: " <<
                        cpp_strerror(r) << dendl;
                    on_finish->complete(r);
                    return;
                }
        ...
        template < typename I >
            void S3Stream < I >::get_size(uint64_t * size,
                                          Context * on_finish) {
            ldout(m_cct, 10) << dendl;
        ...
        template < typename I >
            void S3Stream < I >::read(io::Extents
        ...
        template < typename I >
            ...
            std::string date =
                fmt::format("{:%a, %d %b %Y %H:%M:%S %z}", timeInfo);
            ...
            ceph::crypto::HMACSHA1 hmac(reinterpret_cast <
                                        const unsigned char *>(m_secret_key.
                                                               data()),
                                        m_secret_key.size());
            hmac.Update(reinterpret_cast <
                        const unsigned char *>(string_to_sign.data()),
                        string_to_sign.size());
            ...
            std::string signature =
                std::string(std::begin(buf), std::begin(buf) + r);
            std::string authorization =
                fmt::format("AWS {}:{}", m_access_key, signature);
            http_request.set(boost::beast::http::field::authorization,
                             authorization);
