...
class LZ4Compressor:public Compressor {
  public:
    LZ4Compressor():Compressor(COMP_ALG_LZ4, "lz4") {
        ...
        bufferptr outptr =
            buffer::create_page_aligned(LZ4_compressBound(src.length()));
    ...
    int decompress(bufferlist::iterator & p,
                   size_t compressed_len, bufferlist & dst) override {
        uint32_t count;
        ...
        for (unsigned i = 0; i < count; ++i) {
            int r =
                LZ4_decompress_safe_continue(&lz4_stream_decode, c_in, c_out,
                                             compressed_pairs[i].second,
