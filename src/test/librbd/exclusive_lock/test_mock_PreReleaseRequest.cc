...
} // namespace exclusive_lock namespace util {
...
} // anonymous namespace using::testing::_;
...
public:
    ...
    typedef PreReleaseRequest < MockTestImageCtx >
    ...
    } void expect_set_require_lock(MockImageDispatch &
                                   mock_image_dispatch,
                                   bool init_shutdown,
                                   ...
                                   int r)
    {
        ...
        .WillOnce(WithArg < 2 > (Invoke([r](Context * ctx) {
            ctx->complete(r);
        })));
    }
    ...
    {
        expect_test_features(mock_image_ctx, RBD_FEATURE_EXCLUSIVE_LOCK,
                             true);
    ...
    void expect_unset_require_lock(MockImageDispatch &
                                   mock_image_dispatch)
    {
        EXPECT_CALL(mock_image_dispatch,
    ...
    void expect_cancel_op_requests(MockTestImageCtx &mock_image_ctx,
                                   int r)
    {
    ...
    void expect_prerelease_exclusive_lock(MockTestImageCtx &
                                          mock_image_ctx, int r)
    {
        EXPECT_CALL(*mock_image_ctx.plugin_registry,
                    prerelease_exclusive_lock(_))
        .
        WillOnce(CompleteContext
                 (r, mock_image_ctx.image_ctx->op_work_queue));
    }
    ...
    void expect_handle_prepare_lock_complete(MockTestImageCtx &
            mock_image_ctx)
    {
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, Success)
{
    ...
    MockPreReleaseRequest *req =
        MockPreReleaseRequest::create(mock_image_ctx,
                                      &mock_image_dispatch, false,
                                      m_async_op_tracker, &ctx);
    req->send();
