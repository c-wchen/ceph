...
int MonClient::get_monmap_privately()
{
    ...
    if (!messenger) {
        messenger = smessenger =
            Messenger::create_client_messenger(cct, "temp_mon_client");
        if (NULL == messenger) {
            return -1;
        }
    ...
    ldout(cct,
          10) << "have " << monmap.epoch << " fsid " << monmap.fsid << dendl;
...
int MonClient::init()
{
    ...
    ldout(cct,
        ...
        get_supported_set() << " method " << method << dendl;
...
void MonClient::shutdown()
{
    ...
    while (!waiting_for_session.empty()) {
        ldout(cct,
              20) << __func__ << " discarding pending message " <<
...
MonConnection & MonClient::_add_conn(unsigned rank, uint64_t global_id)
{
    ...
    ldout(cct, 10) << "picked mon." << monmap.get_name(rank)
        << " con " << conn << " addr " << conn->get_peer_addr()
        << dendl;
...
bool MonClient::ms_handle_reset(Connection * con)
{
    ...
    if (_hunting()) {
        if (pending_cons.count(con->get_peer_addr())) {
            ldout(cct,
                  10) << __func__ << " hunted mon " << con->
                get_peer_addr() << dendl;
        }
        else {
            ldout(cct,
                  10) << __func__ << " stray mon " << con->
                get_peer_addr() << dendl;
        }
        return true;
    }
...
void MonClient::schedule_tick()
{
    ...
    if (_hunting()) {
        ...
        timer.add_event_after(cct->_conf->mon_client_ping_interval,
                              new C_Tick(this));
}
...
void MonClient::handle_subscribe_ack(MMonSubscribeAck * m)
{
    if (sub_renew_sent != utime_t()) {
        ...
        ldout(cct,
              10) << __func__ << " sent " << sub_renew_sent << " renew after "
            << sub_renew_after << dendl;
        sub_renew_sent = utime_t();
    }
    else {
        ldout(cct,
              10) << __func__ << " sent " << sub_renew_sent << ", ignoring" <<
            dendl;
    }
...
int MonClient::wait_auth_rotating(double timeout)
{
    ...
    while (auth_principal_needs_rotating_keys(entity_name) &&
        ...
        if (now >= until) {
            ldout(cct,
                  0) << __func__ << " timed out after " << timeout << dendl;
            return -ETIMEDOUT;
        }
        ldout(cct,
              10) << __func__ << " waiting (until " << until << ")" << dendl;
...
void MonClient::_send_command(MonCommand * r)
{
    ...
    if (r->target_rank >= 0 && r->target_rank != monmap.get_rank(peer)) {
        ...
        if (r->target_rank >= (int)monmap.size()) {
            ldout(cct,
                  10) << " target " << r->
                target_rank << " >= max mon " << monmap.size() << dendl;
    ...
    if (r->target_name.length() && r->target_name != monmap.get_name(peer)) {
        ...
        if (!monmap.contains(r->target_name)) {
            ldout(cct,
                  10) << " target " << r->
                target_name << " not present in monmap" << dendl;
                                  ...
                                  Context * onfinish)
{
    ...
    if (cct->_conf->rados_mon_op_timeout > 0) {
        class C_CancelMonCommand:public Context {
            ...
            MonClient *monc;
          public:
            C_CancelMonCommand(uint64_t tid, MonClient * monc):tid(tid),
                monc(monc) {
            } void finish(int r) override {
...
{
    assert(monc_lock.is_locked());
    map < ceph_tid_t, version_req_d * >::iterator iter =
        version_requests.find(m->handle);
    if (iter == version_requests.end()) {
        ...
        ldout(cct,
              10) << __func__ << " finishing " << req << " version " << m->
            version << dendl;
...
AuthAuthorizer *MonClient::build_authorizer(int service_id) const const
{
    Mutex::Locker l(monc_lock);
    if (auth) {
