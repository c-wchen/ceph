...
struct MutationImpl:public TrackedOp {
  ...
  public:
    ...
    // keep our default values synced with MDRequestParam's
     MutationImpl():TrackedOp(nullptr, utime_t()) {
                   ...
                   metareqid_t ri, __u32 att = 0, mds_rank_t slave_to =
                   MDS_RANK_NONE)
  :    TrackedOp(tracker, initiated), reqid(ri), attempt(att),
    ...
    // pin items in cache void pin(MDSCacheObject * o);
...
struct MDRequestImpl:public MutationImpl {
    ...
    struct Params {
        ...
        Params():attempt(0), client_req(NULL),
            triggering_slave_req(NULL), slave_to(MDS_RANK_NONE),
            internal_op(-1) {
    }};
    MDRequestImpl(const Params & params,
        ...
        slave_request(NULL), internal_op(params.internal_op),
        internal_op_finish(NULL), internal_op_private(NULL), retry(0),
...
struct MDSlaveUpdate {
    ...
    MDSlaveUpdate(int oo, bufferlist & rbl,
                  elist < MDSlaveUpdate * >&list):origop(oo), item(this),
        waiter(0) {
        rollback.claim(rbl);
        list.push_back(&item);
    } ~MDSlaveUpdate() {
