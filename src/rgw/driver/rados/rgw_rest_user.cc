                                  ...
                                  optional_yield y)
{
    ...
    RGWUserInfo ui;
    int op_ret =
        driver->forward_request_to_master(s, s->user.get(), nullptr, data, &jp,
                                          s->info, y);
    if (op_ret < 0) {
        ldpp_dout(dpp,
...
void RGWOp_User_Create::execute(optional_yield y)
{
    ...
    RESTArgs::get_string(s, "default-placement", default_placement_str,
                         &default_placement_str);
    RESTArgs::get_string(s, "placement-tags", placement_tags_str,
                         &placement_tags_str);
...
void RGWOp_User_Remove::execute(optional_yield y)
{
    ...
    bufferlist data;
    op_ret =
        driver->forward_request_to_master(s, s->user.get(), nullptr, data,
                                          nullptr, s->info, y);
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "forward_request_to_master returned ret=" << op_ret <<
...
void RGWOp_Quota_Set::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  20) << "failed initializing user info: " << op_ret << dendl;
        return;
    }
...
#define QUOTA_INPUT_MAX_LEN 1024
    if (set_all) {
        ...
        if (use_http_params) {
            ...
            if (op_ret < 0) {
                ldpp_dout(this,
                          20) << "failed to get user info: " << op_ret << dendl;
                return;
            }
            ...
            RESTArgs::get_int64(s, "max-objects", old_quota->max_objects,
                                &quota.max_objects);
            RESTArgs::get_int64(s, "max-size", old_quota->max_size,
                                &quota.max_size);
            int64_t max_size_kb;
            bool has_max_size_kb = false;
            RESTArgs::get_int64(s, "max-size-kb", 0, &max_size_kb,
                                &has_max_size_kb);
            if (has_max_size_kb) {
                quota.max_size = max_size_kb * 1024;
            }
            RESTArgs::get_bool(s, "enabled", old_quota->enabled,
                               &quota.enabled);
        }
