...
TEST(chain_xattr, get_and_set)
{
    ...
    {
        const string name =
            user + string(CHAIN_XATTR_MAX_NAME_LEN - user.size(), '@');
        const string x(LARGE_BLOCK_LEN, 'X');

        {
            char y[LARGE_BLOCK_LEN];
            ASSERT_EQ(LARGE_BLOCK_LEN,
                      chain_setxattr(file, name.c_str(), x.c_str(),
                                     LARGE_BLOCK_LEN));
            ASSERT_EQ(LARGE_BLOCK_LEN,
                      chain_getxattr(file, name.c_str(), 0, 0));
            ASSERT_EQ(LARGE_BLOCK_LEN,
                      chain_getxattr(file, name.c_str(), y, LARGE_BLOCK_LEN));
...

        {
            char y[LARGE_BLOCK_LEN];
            ASSERT_EQ(LARGE_BLOCK_LEN,
                      chain_fsetxattr(fd, name.c_str(), x.c_str(),
                                      LARGE_BLOCK_LEN));
            ASSERT_EQ(LARGE_BLOCK_LEN, chain_fgetxattr(fd, name.c_str(), 0, 0));
            ASSERT_EQ(LARGE_BLOCK_LEN,
                      chain_fgetxattr(fd, name.c_str(), y, LARGE_BLOCK_LEN));
    ...
    //
    {
...

        {
            ...
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_getxattr(file, name.c_str(), 0, 0));
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_getxattr(file, name.c_str(), y,
                                     CHAIN_XATTR_MAX_BLOCK_LEN));
            ASSERT_EQ(0, chain_removexattr(file, name.c_str()));
...

        {
            ...
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_fsetxattr(fd, name.c_str(), x.c_str(),
                                      CHAIN_XATTR_MAX_BLOCK_LEN));
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_fgetxattr(fd, name.c_str(), 0, 0));
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_fgetxattr(fd, name.c_str(), y,
                                      CHAIN_XATTR_MAX_BLOCK_LEN));
            ASSERT_EQ(0, chain_fremovexattr(fd, name.c_str()));
            ASSERT_EQ(0, memcmp(x.c_str(), y, CHAIN_XATTR_MAX_BLOCK_LEN));
        }
...

    {
        int x = 0;
        ASSERT_EQ(-ENOENT,
                  chain_setxattr("UNLIKELY_TO_EXIST", "NAME", &x, sizeof(x)));
        ASSERT_EQ(-ENOENT, chain_getxattr("UNLIKELY_TO_EXIST", "NAME", 0, 0));
        ASSERT_EQ(-ENOENT,
                  chain_getxattr("UNLIKELY_TO_EXIST", "NAME", &x, sizeof(x)));
        ASSERT_EQ(-ENOENT, chain_removexattr("UNLIKELY_TO_EXIST", "NAME"));
        int unlikely_to_be_a_valid_fd = 400;
        ASSERT_EQ(-EBADF,
                  chain_fsetxattr(unlikely_to_be_a_valid_fd, "NAME", &x,
                                  sizeof(x)));
        ASSERT_EQ(-EBADF,
                  chain_fgetxattr(unlikely_to_be_a_valid_fd, "NAME", 0, 0));
        ASSERT_EQ(-EBADF,
                  chain_fgetxattr(unlikely_to_be_a_valid_fd, "NAME", &x,
                                  sizeof(x)));
        ASSERT_EQ(-EBADF,
                  chain_fremovexattr(unlikely_to_be_a_valid_fd, "NAME"));
    }
...
TEST(chain_xattr, chunk_aligned)
{
    ...
    for (int len = CHAIN_XATTR_MAX_BLOCK_LEN - 10;
         len < CHAIN_XATTR_MAX_BLOCK_LEN + 10; ++len) {
        cout << len << std::endl;
    ...
    for (int len = CHAIN_XATTR_SHORT_BLOCK_LEN - 10;
         len < CHAIN_XATTR_SHORT_BLOCK_LEN + 10; ++len) {
        cout << len << std::endl;
        const string x(len, 'x');
        char buf[len * 2];
        ASSERT_EQ(len, chain_setxattr(file, name.c_str(), x.c_str(), len));
...

    {
        ...
        ASSERT_EQ((int)sizeof(buf),
                  chain_setxattr(file, aname, buf, sizeof(buf)));
        ASSERT_EQ(-ERANGE,
                  chain_getxattr(file, aname, buf,
    ...
    }
    {
        ...
        memset(buf, 'x', sizeof(buf));
        ASSERT_EQ((int)sizeof(buf),
                  chain_fsetxattr(fd, aname, buf, sizeof(buf)));
        ASSERT_EQ(-ERANGE,
                  chain_fgetxattr(fd, aname, buf,
                                  CHAIN_XATTR_SHORT_BLOCK_LEN * 2));
    }
...
bool listxattr_cmp(char *xattr1, char *xattr2, int size)
{
    ...
    std::set_difference(xattrs1.begin(), xattrs1.end(),
                        xattrs2.begin(), xattrs2.end(), diff.begin());
...
TEST(chain_xattr, listxattr)
{
    ...
    const string user("user.");
    const string name1 =
        user + string(CHAIN_XATTR_MAX_NAME_LEN - user.size(), '1');
    const string name2 =
        user + string(CHAIN_XATTR_MAX_NAME_LEN - user.size(), '@');
    const string x(LARGE_BLOCK_LEN, 'X');
    const int y = 1234;
    ...
    ASSERT_EQ(LARGE_BLOCK_LEN,
              chain_setxattr(file, name1.c_str(), x.c_str(), LARGE_BLOCK_LEN));
    ASSERT_EQ((int)sizeof(y),
              chain_setxattr(file, name2.c_str(), &y, sizeof(y)));
    ...
    ASSERT_GT(0,
              chain_flistxattr(unlikely_to_be_a_valid_fd, actual, buffer_size));
...
TEST(chain_xattr, fskip_chain_cleanup_and_ensure_single_attr)
{
    ...
    {
        std::size_t r =
            chain_fsetxattr < false, true > (fd, name, buf, sizeof(buf));
        ASSERT_EQ(sizeof(buf), r);
...
TEST(chain_xattr, skip_chain_cleanup_and_ensure_single_attr)
{
    ...
    {
        std::size_t r =
            chain_setxattr < false, true > (file, name, buf, sizeof(buf));
        ASSERT_EQ(sizeof(buf), r);
...
int main(int argc, char **argv)
{
    ...
    if ((ret < 0) || (x != y)) {
        cerr <<
            "SKIP all tests because extended attributes don't appear to work in the file system in which the tests are run: "
