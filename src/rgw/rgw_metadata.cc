...
void LogStatusDump::dump(Formatter * f) const const
{
    string s;
    switch (status) {
...
void RGWMetadataLogData::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void RGWMetadataLogData::dump(Formatter * f) const const
{
...
int RGWMetadataLog::add_entry(RGWMetadataHandler * handler,
...
int RGWMetadataLog::store_entries_in_shard(list < cls_log_entry > &entries,
                                           int shard_id,
...
void RGWMetadataLog::init_list_entries(int shard_id,
...
int RGWMetadataLog::list_entries(void *handle,
                                 ...
                                 string * last_marker, bool * truncated)
{
    LogListCtx *ctx = static_cast < LogListCtx * >(handle);
...
int RGWMetadataLog::get_info_async(int shard_id,
...
int RGWMetadataLog::trim(int shard_id, const real_time & from_time,
                         const real_time & end_time,
                         const string & start_marker, const string & end_marker)
{
    ...
    ret =
        store->time_log_trim(oid, from_time, end_time, start_marker,
                             end_marker);
                                   ...
                                   string & zone_id, string & owner_id)
{
    ...
    return store->lock_exclusive(store->get_zone_params().log_pool, oid,
                                 duration, zone_id, owner_id);
}
...
int RGWMetadataLog::unlock(int shard_id, string & zone_id, string & owner_id)
{
    ...
    return store->unlock(store->get_zone_params().log_pool, oid, zone_id,
                         owner_id);
}
...
class RGWMetadataTopHandler:public RGWMetadataHandler {
  ...
  public:
    ...
    int put(RGWRados * store, string & entry,
            ...
            sync_type_t sync_type) override {
        return -ENOTSUP;
    }

    virtual void get_pool_and_oid(RGWRados * store, const string & key,
...
namespace {
    ...
    int read_history(RGWRados * store, RGWMetadataLogHistory * state,
        ...
        bufferlist bl;
        int ret =
            rgw_get_system_obj(store, ctx, pool, oid, bl, objv_tracker,
                               nullptr);
        if (ret < 0) {
            return ret;
        }
        ...
        :RGWCoroutine(store->ctx()), store(store), cursor(cursor),
            ...
            reenter(this) {
                yield {
                    ...
                    using ReadCR =
                        RGWSimpleRadosReadCR < RGWMetadataLogHistory >;
                    call(new
                         ReadCR(store->get_async_rados(), store, obj, &state,
                ...
                *cursor =
                    store->period_history->lookup(state.oldest_realm_epoch);
                if (!*cursor) {
        ...
        :RGWCoroutine(store->ctx()), store(store), cursor(cursor), objv(objv) {
            ...
            reenter(this) {
                ...
                yield {
                    ...
                    using WriteCR =
                        RGWSimpleRadosWriteCR < RGWMetadataLogHistory >;
                    call(new
                         WriteCR(store->get_async_rados(), store, obj, state,
                                 objv));
                }
                if (retcode < 0) {
...
/// update the mdlog history to reflect trimmed logs
    class TrimHistoryCR:public RGWCoroutine {
      ...
      public:
        TrimHistoryCR(RGWRados * store, Cursor cursor,
                      RGWObjVersionTracker * objv)
        :RGWCoroutine(store->ctx()), store(store), cursor(cursor), objv(objv),
            next(cursor) {
            ...
            reenter(this) {
                ...
                if (cursor.get_epoch() < existing.get_epoch()) {
                    ldout(cct,
                        ...
                        get_epoch()
                        << ", rejecting trim at epoch=" << cursor.
                        get_epoch() << dendl;
...
Cursor RGWMetadataManager::init_oldest_log_period()
{
    ...
    if (period.get_realm_epoch() != state.oldest_realm_epoch) {
        ldout(cct, 1) << "inconsistent mdlog history: read period id="
            << period.get_id() << " with realm_epoch=" << period.
            get_realm_epoch()
...
Cursor RGWMetadataManager::read_oldest_log_period() const const
{
    RGWMetadataLogHistory state;
...
void RGWMetadataManager::parse_metadata_key(const string & metadata_key,
...
int RGWMetadataManager::find_handler(const string & metadata_key,
                            ...
                            obj_version * existing_version)
{
    ...
    ret =
        handler->put(store, entry, objv_tracker, mtime.to_real_time(), jo,
                     sync_type);
    if (existing_version) {
...
int RGWMetadataManager::lock_exclusive(string & metadata_key, timespan duration,
                                       string & owner_id)
{
    RGWMetadataHandler *handler;
    string entry;
...
int RGWMetadataManager::list_keys_next(void *handle, int max,
...
void RGWMetadataManager::dump_log_entry(cls_log_entry & entry, Formatter * f)
{
    ...
    try {
        ...
        lderr(cct) << "failed to decode log entry: " << entry.
            section << ":" << entry.name << " ts=" << entry.timestamp << dendl;
    }
    f->close_section();
}
...
int RGWMetadataManager::pre_modify(RGWMetadataHandler * handler,
                                   string & section, const string & key,
                                   RGWMetadataLogData & log_data,
                                   RGWObjVersionTracker * objv_tracker,
...
int RGWMetadataManager::post_modify(RGWMetadataHandler * handler,
                                    const string & section, const string & key,
                                    RGWMetadataLogData & log_data,
                                    RGWObjVersionTracker * objv_tracker,
                                    int ret)
...
string RGWMetadataManager::heap_oid(RGWMetadataHandler * handler,
                                    const string & key,
                                    const obj_version & objv)
{
    char buf[objv.tag.size() + 32];
    snprintf(buf, sizeof(buf), "%s:%lld", objv.tag.c_str(),
             (long long)objv.ver);
    return string(".meta:") + handler->get_type() + ":" + key + ":" + buf;
}

int RGWMetadataManager::store_in_heap(RGWMetadataHandler * handler,
                                      ...
                                      bufferlist > *pattrs)
{
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: rgw_put_system_obj() oid=" << oid <<
...
int RGWMetadataManager::remove_from_heap(RGWMetadataHandler * handler,
                                         const string & key,
                                         RGWObjVersionTracker * objv_tracker)
{
    if (!objv_tracker) {
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: store->delete_system_obj()=" << oid <<
            ") returned ret=" << ret << dendl;
        return ret;
    }
...
int RGWMetadataManager::put_entry(RGWMetadataHandler * handler,
                                  const string & key, bufferlist & bl,
                                  bool exclusive,
                                  RGWObjVersionTracker * objv_tracker,
                                  real_time mtime, map < string,
                                  bufferlist > *pattrs)
{
    ...
    RGWMetadataLogData log_data;
    int ret =
        pre_modify(handler, section, key, log_data, objv_tracker,
                   MDLOG_STATUS_WRITE);
    if (ret < 0)
        return ret;
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: " << __func__ << ": store_in_heap() key=" << key <<
            " returned ret=" << ret << dendl;
        goto done;
    }
    ...
    if (ret < 0) {
        ...
        if (r < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: " << __func__ << ": remove_from_heap() key=" <<
                key << " returned ret=" << r << dendl;
        }
    }
  done:
                                         ...
                                         const string & key, int *shard_id)
{
    ...
    handler->get_hash_key(section, key, hash_key);
    *shard_id =
        store->key_to_shard_id(hash_key, cct->_conf->rgw_md_log_max_shards);
    return 0;
}
