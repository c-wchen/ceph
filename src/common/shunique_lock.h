...
public:
    ...
    // We do not provide a default locking/try_locking constructor that// takes only the mutex, since it is not clear whether to take it// shared or unique. We explicitly require the use of lock_deferred// to prevent Nasty Surprises.
    ...
    template < typename AcquireType > shunique_lock(mutex_type &m, AcquireType at, std::try_to_lock_t)
        :    m(&m), o(ownership::none)
    {
        try_lock(at);
    }
    ...
    template < typename AcquireType, typename Clock, typename Duration > shunique_lock(mutex_type &m, AcquireType at,
    ...
    template < typename AcquireType, typename Rep, typename Period > shunique_lock(mutex_type &m, AcquireType at,
    ...
    bool try_lock_for(const std::chrono::duration < Rep,
                      Period > &dur)
    {
        lockable();
    ...
    template < typename Rep, typename Period >
    bool try_lock_shared_for(const std::chrono::duration < Rep,
                             Period > &dur)
    {
        lockable();
    ...
    void unlock()
    {
        switch (o) {
            case ownership::none:
                throw std::system_error((int)std::errc::
    ...
    // If you want to make sure you have a lock of some sort on the// mutex, just treat as a bool.
