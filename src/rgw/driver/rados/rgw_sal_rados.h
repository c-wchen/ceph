...
private:
    int link(const DoutPrefixProvider* dpp, const rgw_owner& new_owner, optional_yield y, bool update_entrypoint = true,
...
public:
    RadosNotification(const DoutPrefixProvider* _dpp,
        ...
        : StoreNotification(_obj, _src_obj,
    {
        _type
    }),
    store(_store),
...
public:
    ...
    int put_script(const DoutPrefixProvider* dpp, optional_yield y, const std::string& key,
...
public:
    RadosRole(RadosStore* _store, std::string name,
        ...
        : RGWRole(name, tenant, std::move(account_id), path, trust_policy, std::move(description), max_session_duration, tags),
          store(_store) {}
