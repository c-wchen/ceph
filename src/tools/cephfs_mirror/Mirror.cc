    ...
    namespace mirror {
            ...
            class ThreadPoolSingleton:public ThreadPool {
              public:
                ...
                :ThreadPool(cct, "Mirror::thread_pool", "tp_mirror", 1) {
                    work_queue =
                        new ContextWQ("Mirror::work_queue",
                                      ceph::make_timespan(60), this);
        ...
        struct Mirror::C_EnableMirroring:Context {
            ...
            uint64_t pool_id;
             ...
             C_EnableMirroring(Mirror * mirror, const Filesystem & filesystem,
                               uint64_t pool_id)
        ...
        struct Mirror::C_RestartMirroring:Context {
            ...
            :mirror(mirror),
                filesystem(filesystem), pool_id(pool_id), peers(peers) {
            } void finish(int r) override {
                disable_mirroring();
            }
        ...
        Mirror::Mirror(CephContext * cct,
                       const std::vector < const char *>&args, MonClient * monc,
                       Messenger * msgr)
        ...
        int Mirror::init_mon_client() {
            ...
            int r = m_monc->init();
            if (r < 0) {
                derr << ": failed to init mon client: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }

            r = m_monc->authenticate(std::chrono::duration <
                                     double >(m_cct->_conf.get_val <
                                              std::chrono::seconds >
                                              ("client_mount_timeout")).
                                     count());
            if (r < 0) {
                derr << ": failed to authenticate to monitor: " <<
        ...
        int Mirror::init(std::string & reason) {
            ...
            m_service_daemon =
                std::make_unique < ServiceDaemon > (m_cct, m_local);
            r = m_service_daemon->init();
            if (r < 0) {
                derr << ": error registering service daemon: " <<
                    cpp_strerror(r) << dendl;
                return r;
            }
        ...
        void Mirror::handle_enable_mirroring(const Filesystem & filesystem,
            ...
            m_cond.notify_all();
            if (r < 0) {
                derr << ": failed to initialize FSMirror for filesystem=" <<
                    filesystem << ": " << cpp_strerror(r) << dendl;
          ...
          for (auto & peer:peers) {
            ...
            dout(10) << ": Initialized FSMirror for filesystem=" << filesystem
                << dendl;
        }
        ...
        void Mirror::enable_mirroring(const Filesystem & filesystem,
            ...
            dout(10) << ": starting FSMirror: filesystem=" << filesystem <<
                dendl;

            mirror_action.action_in_progress = true;
            mirror_action.fs_mirror =
                std::make_unique < FSMirror > (m_cct, filesystem, local_pool_id,
                                               m_service_daemon.get(), m_args,
                                               m_work_queue);
            mirror_action.fs_mirror->init(new C_AsyncCallback < ContextWQ >
                                          (m_work_queue, on_finish));
        }

        void Mirror::mirroring_enabled(const Filesystem & filesystem,
                                       uint64_t local_pool_id) {
            dout(10) << ": filesystem=" << filesystem << ", pool_id=" <<
                local_pool_id << dendl;
            ...
            auto p =
                m_mirror_actions.emplace(filesystem,
                                         MirrorAction(local_pool_id));
            auto & mirror_action = p.first->second;
            mirror_action.action_ctxs.
                push_back(new
                          C_EnableMirroring(this, filesystem, local_pool_id));
        }

        void Mirror::handle_disable_mirroring(const Filesystem & filesystem,
                                              int r) {
            ...
            if (!m_stopping) {
                ...
                if (mirror_action.action_ctxs.empty()) {
                    dout(10) << ": no pending actions for filesystem=" <<
                        filesystem << dendl;
        ...
        void Mirror::disable_mirroring(const Filesystem & filesystem,
                                       Context * on_finish) {
            ceph_assert(ceph_mutex_is_locked(m_lock));
            ...
            if (mirror_action.fs_mirror->is_init_failed()) {
                dout(10) << ": init failed for filesystem=" << filesystem <<
                    dendl;
            ...
            mirror_action.action_in_progress = true;
            mirror_action.fs_mirror->shutdown(new C_AsyncCallback < ContextWQ >
                                              (m_work_queue, on_finish));
        }
        ...
        void Mirror::peer_added(const Filesystem & filesystem,
                                const Peer & peer) {
            dout(20) << ": filesystem=" << filesystem << ", peer=" << peer <<
                dendl;
            ...
            auto & mirror_action = m_mirror_actions.at(filesystem);
            mirror_action.action_ctxs.
                push_back(new C_PeerUpdate(this, filesystem, peer));
        }

        void Mirror::remove_peer(const Filesystem & filesystem,
                                 const Peer & peer) {
            ceph_assert(ceph_mutex_is_locked(m_lock));
        ...
        void Mirror::peer_removed(const Filesystem & filesystem,
                                  const Peer & peer) {
            dout(20) << ": filesystem=" << filesystem << ", peer=" << peer <<
                dendl;
            ...
            auto & mirror_action = m_mirror_actions.at(filesystem);
            mirror_action.action_ctxs.
                push_back(new C_PeerUpdate(this, filesystem, peer, true));
        }
        ...
        void Mirror::update_fs_mirrors() {
            ...
            bool check_blocklist = blocklist_interval > 0
                && ((now - m_last_blocklist_check) >= blocklist_interval);

            double failed_interval =
                g_ceph_context->_conf.get_val < std::chrono::seconds >
                ("cephfs_mirror_restart_mirror_on_failure_interval").count();
            bool check_failure = failed_interval > 0
                && ((now - m_last_failure_check) >= failed_interval);

            {
                std::scoped_lock locker(m_lock);
              for (auto &[filesystem, mirror_action]:m_mirror_actions)
                {
                    auto failed = mirror_action.fs_mirror
                        && mirror_action.fs_mirror->is_failed();
                    auto blocklisted = mirror_action.fs_mirror
                    ...
                    if (check_failure && !mirror_action.action_in_progress
                        ...
                        auto ctx =
                            new C_RestartMirroring(this, filesystem,
                                                   mirror_action.pool_id,
                                                   peers);
                        ctx->complete(0);
                    }
                    else if (check_blocklist
        ...
        void Mirror::schedule_mirror_update_task() {
            ...
            dout(20) << ": scheduling fs mirror update (" << m_timer_task <<
                ") after " << after << " seconds" << dendl;
        ...
        void Mirror::run() {
            ...
            m_cluster_watcher.
                reset(new
                      ClusterWatcher(m_cct, m_monc, m_service_daemon.get(),
                                     m_listener));
            ...
            locker.unlock();
            {
                ...
                if (m_timer_task != nullptr) {
                    dout(10) << ": canceling timer task=" << m_timer_task <<
                        dendl;
          ...
          for (auto &[filesystem, mirror_action]:m_mirror_actions) {
                dout(10) << ": trying to shutdown filesystem=" << filesystem <<
                    dendl;
                ...
                if (mirror_action.fs_mirror &&
                    ...
                    C_SaferCond cond;
                    mirror_action.fs_mirror->shutdown(new C_AsyncCallback <
                                                      ContextWQ > (m_work_queue,
                                                                   &cond));
                    int r = cond.wait();
                    dout(10) << ": shutdown filesystem=" << filesystem << ", r="
                        << r << dendl;
                }
