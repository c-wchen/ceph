...
KernelDevice::KernelDevice(CephContext *cct, aio_callback_t cb, void *cbpriv,
    ...
    : BlockDevice(cct, cb, cbpriv), aio(false), dio(false), discard_callback(d_cb),
      discard_callback_priv(d_cbpriv), aio_stop(false), discard_started(false),
      discard_stop(false), aio_thread(this), discard_thread(this), injecting_crash(0)
{
    ...
    if (use_ioring && ioring_queue_t::supported()) {
        bool use_ioring_hipri =
            cct->_conf.get_val < bool > ("bdev_ioring_hipri");
        bool use_ioring_sqthread_poll =
            cct->_conf.get_val < bool > ("bdev_ioring_sqthread_poll");
        io_queue =
            std::make_unique < ioring_queue_t > (iodepth, use_ioring_hipri,
...
int KernelDevice::open(const string &p)
{
    ...
    if (i != WRITE_LIFE_MAX) {
        enable_wrt = false;
        dout(0) << "ioctl(F_SET_FILE_RW_HINT) on " << path << " failed: " <<
...
int KernelDevice::get_devices(std::set < std::string > *ls) const const
{
    if (devname.empty()) {
        return 0;
    }
...
int KernelDevice::collect_metadata(const string &prefix, map < string,
...
int KernelDevice::get_ebd_state(ExtBlkDevState &state) const const
{
...
int KernelDevice::choose_fd(bool buffered, int write_hint) const const
{
#if defined(F_SET_FILE_RW_HINT)
    if (!enable_wrt) {
                    ...
                    if (cct->_conf->bdev_debug_aio_suicide_timeout) {
                        ...
                        if (debug_stall_since < cutoff) {
                            derr << __func__ << " stalled aio " << debug_oldest
                                 ...
                                 ", timeout is " << cct->_conf->
                                 bdev_debug_aio_suicide_timeout << "s, suicide"
                                 << dendl;
                            ceph_abort_msg
...
void KernelDevice::_aio_log_start(IOContext *ioc,
...
void KernelDevice::_aio_log_finish(IOContext *ioc,
                                   uint64_t offset, uint64_t length)
{
    ...
    do {
        ...
        if (left) {
            ...
            if (r) {
                ...
                iov[idx].iov_base =
                    static_cast < char *>(iov[idx].iov_base) + r;
                iov[idx].iov_len -= r;
                r = 0;
            }
            ceph_assert(r == 0);
...
int KernelDevice::write(uint64_t off,
                        bufferlist &bl, bool buffered, int write_hint)
{
    uint64_t len = bl.length();
...
int KernelDevice::aio_write(uint64_t off,
                            bufferlist &bl,
    ...
    *_dout << dendl;
    ...
    if (aio && dio && !buffered) {
        if (cct->_conf->bdev_inject_crash &&
            ...
            ioc->pending_aios.
            push_back(aio_t(ioc, choose_fd(false, write_hint)));
            ++ioc->num_pending;
            auto &aio = ioc->pending_aios.back();
            aio.bl.
            push_back(ceph::buffer::ptr_node::
                      create(ceph::buffer::create_small_page_aligned(len)));
...
int KernelDevice::_discard(uint64_t offset, uint64_t len)
{
    ...
    dout(10) << __func__
             << " 0x" << std::hex << offset << "~" << len << std::dec << dendl;
    r = BlkDev {
        fd_directs[WRITE_LIFE_NOT_SET]}
    .discard((int64_t) offset, (int64_t) len);
    return r;
}
...
struct ExplicitHugePagePool {
    ...
    struct mmaped_buffer_raw: public instrumented_raw {
        ...
        mmaped_buffer_raw(void *mmaped_region, ExplicitHugePagePool &parent)
            : instrumented_raw(static_cast < char *>(mmaped_region),
                               parent.buffer_size), region_q(parent.region_q)
        {
        ...
        : buffer_size(buffer_size), region_q(buffers_in_pool)
    {
        while (buffers_in_pool--) {
            void *const mmaped_region =::mmap(nullptr,
                                              buffer_size,
...
HugePagePoolOfPools HugePagePoolOfPools::from_desc(const std::string &desc)
{
    ...
    return HugePagePoolOfPools {
        std::move(conf)};
}

// create a buffer basing on user-configurable. it's intended to make
// our buffers THP-able.
ceph::unique_leakable_ptr < buffer::raw > KernelDevice::create_custom_aligned(const size_t len,
        IOContext *const ioc) constconst {
    ...
    } else
    {
        ...
        HugePagePoolOfPools::from_desc(cct->_conf.get_val < std::string >
...
int KernelDevice::aio_read(uint64_t off,
                           uint64_t len, bufferlist *pbl, IOContext *ioc)
{
    ...
    if (aio && dio) {
        ...
        aio.bl.
        push_back(ceph::buffer::ptr_node::
                  create(create_custom_aligned(len, ioc)));
        aio.bl.prepare_iov(&aio.iov);
        aio.preadv(off, len);
        dout(30) << aio << dendl;
        pbl->append(aio.bl);
...
int KernelDevice::invalidate_cache(uint64_t off, uint64_t len)
{
    ...
    int r =
        posix_fadvise(fd_buffereds[WRITE_LIFE_NOT_SET], off, len,
                      POSIX_FADV_DONTNEED);
    if (r) {
        r = -r;
        derr << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
