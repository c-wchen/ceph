...
class TestWatchContext:public librados::WatchCtx2 {
  ...
  public:
    ...
    bool waiting;
    Mutex lock;
    TestWatchContext():handle(0), waiting(false), lock("watch lock") {
...
class TestOp {
  ...
  :    num(n), context(context), stat(stat), done(false) {
    } virtual ~ TestOp() {
    };
...
class RadosTestContext {
  ...
  RadosTestContext(const string & pool_name, int max_in_flight, uint64_t max_size, uint64_t min_stride_size, uint64_t max_stride_size, bool no_omap, bool no_sparse, bool pool_snaps, bool write_fadvise_dontneed, const char *id = 0):
    state_lock("Context Lock"),
        ...
        write_fadvise_dontneed(write_fadvise_dontneed), snapname_num(0) {
    } int init() {
    ...
    ObjectDesc get_most_recent(const string & oid) {
        ...
        for (map < int, map < string, ObjectDesc > >::reverse_iterator i =
             pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
    ...
    void rm_object_attrs(const string & oid, const set < string > &attrs) {
        ...
        pool_obj_cont[current_snap].insert(pair < string,
                                           ObjectDesc > (oid, new_obj));
    }
    ...
    void update_object_attrs(const string & oid, const map < string,
                             ContDesc > &attrs) {
        ObjectDesc new_obj = get_most_recent(oid);
    ...
    void update_object_redirect_target(const string & oid,
                                       const string & target) {
    ...
    bool object_existed_at(const string & oid, int snap = -1) const {
        ...
        map < int, map < string, ObjectDesc > >::iterator next_iter =
            pool_obj_cont.find(snap);
        assert(next_iter != pool_obj_cont.end());
        map < int, map < string, ObjectDesc > >::iterator current_iter =
            next_iter++;
    ...
    void roll_back(const string & oid, int snap) {
        ...
        pool_obj_cont.rbegin()->second.insert(pair < string,
                                              ObjectDesc > (oid, contents));
    }
};
...
class RemoveAttrsOp:public TestOp {
  public:
    ...
    RemoveAttrsOp(int n, RadosTestContext * context,
                  const string & oid, TestOpStat * stat)
...
class WriteOp:public TestOp {
  ...
  :    
    ...
    last_acked_tid(0), do_append(do_append), do_excl(do_excl) {
    } void _begin() override {
    ...
    void _finish(CallbackInfo * info) override {
        ...
        cout << num << ":  finishing write tid " << tid << " to " << context->
            prefix + oid << std::endl;
        ...
        if (waiting_on == 0) {
            uint64_t version = 0;
            for (set < librados::AioCompletion * >::iterator i =
                 waiting.begin(); i != waiting.end();) {
...
class WriteSameOp:public TestOp {
  public:
    ...
    WriteSameOp(int n,
                RadosTestContext * context,
                const string & oid, TestOpStat * stat = 0)
  :    
    TestOp(n, context, stat),
...
class ReadOp:public TestOp {
  public:
    ...
    ReadOp(int n,
           RadosTestContext * context,
           const string & oid, bool balance_reads, TestOpStat * stat = 0)
  :    
    TestOp(n, context, stat),
    completions(3),
    oid(oid),
    ...
    void _begin() override {
        ...
        assert(!context->io_ctx.
               aio_operate(context->prefix + oid, completions[0], &op, flags,
                           NULL));
        waiting_on++;
    ...
    void _finish(CallbackInfo * info) override {
        ...
        for (vector < librados::AioCompletion * >::iterator it =
             completions.begin(); it != completions.end(); ++it) {
        ...
        if (!retval) {
            ...
            for (map < string, ContDesc >::iterator iter =
                 old_value.attrs.begin(); iter != old_value.attrs.end();
                 ++iter) {
                bufferlist bl = context->attr_gen.gen_bl(iter->second);
                if (!context->no_omap) {
                    map < string, bufferlist >::iterator omap_iter =
                ...
                map < string, bufferlist >::iterator xattr_iter =
                    xattrs.find(iter->first);
...
class SnapCreateOp:public TestOp {
  ...
  :    TestOp(n, context, stat) {
        ...
        if (context->pool_snaps) {
            ...
            int j = 0;
            for (map < int, uint64_t >::reverse_iterator i =
                 context->snaps.rbegin(); i != context->snaps.rend();
                 ++i, ++j) {
                snapset[j] = i->second;
            }
            ...
            int r =
                context->io_ctx.selfmanaged_snap_set_write_ctx(context->seq,
                                                               snapset);
            if (r) {
                cerr << "r is " << r << " snapset is " << snapset << " seq is "
...
class WatchOp:public TestOp {
  ...
  public:
    WatchOp(int n,
            RadosTestContext * context,
            const string & _oid, TestOpStat * stat = 0)
...
class CopyFromOp:public TestOp {
  public:
    ...
    CopyFromOp(int n,
               RadosTestContext * context,
               const string & oid, const string & oid_src, TestOpStat * stat)
    :TestOp(n, context, stat),
        ...
        pair < TestOp *, TestOp::CallbackInfo * >*read_cb_arg =
            new pair < TestOp *, TestOp::CallbackInfo * >(this,
                                                          ...
                                                          CallbackInfo(1));
        comp_racing_read =
            context->rados.aio_create_completion((void *)read_cb_arg, NULL,
                                                 &write_callback);
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (info->id == 0) {
            ...
            cout << num << ":  finishing copy_from to " << context->prefix +
                oid << std::endl;
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    cout << num << ":  got expected ENOENT (src dne)" << std::
                        endl;
                }
                else {
            ...
            cout << num << ":  finishing copy_from racing read to " << context->
                prefix + oid << std::endl;
...
class SetRedirectOp:public TestOp {
  public:
    ...
    SetRedirectOp(int n,
                  ...
                  const string & tgt_pool_name, TestOpStat * stat = 0)
  :    
    TestOp(n, context, stat),
    ...
    comp(NULL), done(0), r(0) {
        ...
        op.set_redirect(context->prefix + oid_tgt, context->io_ctx,
                        tgt_value.version);
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (info->id == 0) {
            assert(comp->is_complete());
            cout << num << ":  finishing set_redirect to oid " << oid << std::
                endl;
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    cout << num << ":  got expected ENOENT (src dne)" << std::
                    ...
                    cerr << "Error: oid " << oid << " set_redirect " << oid_tgt
                        << " returned error code " << r << std::endl;
                    ceph_abort();
                }
            }
...
class CacheFlushOp:public TestOp {
  public:
    ...
    CacheFlushOp(int n,
                 RadosTestContext * context,
                 const string & oid, TestOpStat * stat, bool b)
    :TestOp(n, context, stat),
...
class CacheEvictOp:public TestOp {
  public:
    ...
    :TestOp(n, context, stat), completion(NULL), oid(oid) {
        ...
        cout << num << ": cache_evict oid " << oid << " snap " << snap << std::
            endl;
