...
public:
    ...
    void test_rb_osd_size(const vector < pair < int, int>> &weights, const vector < int > &read_ratios, bool expect_failure,
                          bool reset_rb = false, bool build = true)
    {
        ...
        for (auto rr : read_ratios) {
            ...
            if (verbose) {
                cout << "=== Read ratio: " << rr << " High load before: " << high_load_before << " High load after: " << high_load_after
                     << std::endl;
            }
...
TEST_F(OSDMapTest, rb_osdsize_opt_1small_osd)
{
    ...
    if (is_verbose()) {
        cout << std::endl << " Second iteration - reset between read ratio tests" << std::endl << "====================" <<
             std::endl << std::endl;
    }
