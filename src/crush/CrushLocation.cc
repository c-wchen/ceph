...
} int CrushLocation::_parse(const std::string &s)
{
    ...
    if (r < 0) {
        lderr(cct) << "warning: crush_location '" << cct->_conf->
                   crush_location <<
...
int CrushLocation::update_from_hook()
{
    ...
    ceph_abort_msg
    ("crimson does not support crush_location_hook, it must stay empty");
#else
    if (0 != access(cct->_conf->crush_location_hook.c_str(), R_OK)) {
        lderr(cct) << "the user define crush location hook: " << cct->
                   _conf->
                   crush_location_hook << " may not exist or can not access it" <<
                   dendl;
        return errno;
    }

    SubProcessTimed hook(cct->_conf->crush_location_hook.c_str(),
                         SubProcess::CLOSE, SubProcess::PIPE,
                         SubProcess::PIPE,
                         cct->_conf->crush_location_hook_timeout);
    hook.add_cmd_args("--cluster", cct->_conf->cluster.c_str(), "--id",
                      cct->_conf->name.get_id().c_str(), "--type",
                      cct->_conf->name.get_type_str(), NULL);
    int ret = hook.spawn();
    if (ret != 0) {
        lderr(cct) << "error: failed run " << cct->_conf->
                   crush_location_hook << ": " << hook.err() << dendl;
        return ret;
    }
...
int CrushLocation::init_on_startup()
{
    ...
    std::lock_guard l(lock);
    loc.clear();
    loc.insert(std::make_pair < std::string,
