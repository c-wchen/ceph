    ...
    namespace logging {
        ...
        struct Entry {
            ...
            :m_thread(0), m_prio(0), m_subsys(0),
                ...
                m_buf_len(sizeof(m_static_buf)), m_exp_len(NULL) {
            ...
            m_buf_len(sizeof(m_static_buf)), m_exp_len(NULL) {
                if (msg) {
                    ostream os(&m_streambuf);
                     os << msg;
            }} Entry(utime_t s, pthread_t t, short pr, short sub, char *buf,
                     size_t buf_len, size_t * exp_len, const char *msg = NULL)
          :    m_stamp(s), m_thread(t), m_prio(pr), m_subsys(sub),
                m_next(NULL), m_streambuf(buf, buf_len), m_buf_len(buf_len),
                m_exp_len(exp_len) {
                if (msg) {
                    ostream os(&m_streambuf);
                    os << msg;
          ...
          public:
            ...
            void hint_size() {
                if (m_exp_len != NULL) {
                    ...
                    if (size > __atomic_load_n(m_exp_len, __ATOMIC_RELAXED)) {
                        ...
                        __atomic_store_n(m_exp_len, size + 10,
                                         __ATOMIC_RELAXED);
                    }
                    else {
                        //asymptotically adapt expected size to message size
                        __atomic_store_n(m_exp_len,
                                         (size + 10 + m_buf_len * 31) / 32,
                                         __ATOMIC_RELAXED);
                    }
                }
            }
            ...
            // returns current size of content size_t size() const {
