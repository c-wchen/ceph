...
void ClusterState::final_init()
{
    ...
    int r =
        admin_socket->
        register_command("dump_osd_network name=value,type=CephInt,req=false",
                         asok_hook,
    ...
    if (admin_command == "dump_osd_network") {
      ...
      for (auto i:pg_map.osd_stat) {
          for (auto j:i.second.hb_pingtime) {
                ...
                item.pingtime =
                    std::max(j.second.back_pingtime[0],
                             j.second.back_pingtime[1]);
                item.pingtime =
                    std::max(item.pingtime, j.second.back_pingtime[2]);
                ...
                item.pingtime =
                    std::max(item.pingtime, j.second.front_pingtime[2]);
                if (!value || item.pingtime >= value) {
                    item.from = i.first;
                    item.to = j.first;
      ...
      for (auto & sitem:boost::adaptors::reverse(sorted)) {
            ...
            f->dump_bool("stale",
                         ceph_clock_now().sec() - sitem.last_update > stale);
            ...
            f->dump_format_unquoted("1min", "%s",
                                    fixed_u_to_string(sitem.min[0], 3).c_str());
            f->dump_format_unquoted("5min", "%s",
                                    fixed_u_to_string(sitem.min[1], 3).c_str());
            f->dump_format_unquoted("15min", "%s",
                                    fixed_u_to_string(sitem.min[2], 3).c_str());
            f->close_section(); // min
