...
namespace rgw {
    ...
    static const std::unordered_set <
        ...
        rgw_xattrstr {const_cast < char *>(RGW_ATTR_ETAG),
                      sizeof(RGW_ATTR_ETAG) - 1}
    };
    ...
    LookupFHResult RGWLibFS::stat_leaf(RGWFileHandle * parent,
                                       const char *path,
                                       enum rgw_fh_type type, uint32_t flags) {
            ...
            case 0:
                {
                    ...
                    if ((rc == 0) && (req.get_ret() == 0)) {
                        fhr =
                            lookup_fh(parent, path, RGWFileHandle::FLAG_CREATE);
                        if (get < 0 > (fhr)) {
                            RGWFileHandle *rgw_fh = get < 0 > (fhr);
                            lock_guard guard(rgw_fh->mtx);
    ...
    int RGWLibFS::read(RGWFileHandle * rgw_fh, uint64_t offset, size_t length,
        ...
        RGWReadRequest req(get_context(), user->clone(), rgw_fh, offset, length,
                           buffer);
    ...
    int RGWLibFS::readlink(RGWFileHandle * rgw_fh, uint64_t offset,
    ...
    int RGWLibFS::unlink(RGWFileHandle * rgw_fh, const char *name,
                         uint32_t flags) {
        int rc = 0;
        BucketStats bs;
        ...
        if (unlikely(flags & RGWFileHandle::FLAG_UNLINK_THIS)) {
            ...
            parent = rgw_fh;
            LookupFHResult fhr =
                lookup_fh(parent, name, RGWFileHandle::FLAG_LOCK);
            rgw_fh = get < 0 > (fhr);
            /* LOCKED */
        }
        ...
        if (parent->is_root()) {
            ...
            if (bs.num_entries > 1) {
                ...
                string oname {
                "/"};
                RGWDeleteObjRequest req(cct, user->clone(),
            ...
            RGWDeleteObjRequest req(cct, user->clone(), parent->bucket_name(),
                                    oname);
            rc = g_rgwlib->get_fe()->execute_req(&req);
            if (!rc) {
                rc = req.get_ret();
                         ...
                         const char *_src_name, const char *_dst_name)
    {
        ...
        /* atomicity */
        LookupFHResult fhr =
            lookup_fh(src_fh, _src_name, RGWFileHandle::FLAG_LOCK);
        RGWFileHandle *rgw_fh = get < 0 > (fhr);

        /* should not happen */
        if (!rgw_fh) {
            ldout(get_context(), 0) << __func__
                << " BUG no such src renaming path=" << src_name << dendl;
            goto out;
        }
      ...
      for (int ix:{
            ...
            case 0:
                {
                    RGWCopyObjRequest req(cct, user->clone(), src_fh, dst_fh,
                                          src_name, dst_name);
                    int rc = g_rgwlib->get_fe()->execute_req(&req);
                    if ((rc != 0) || ((rc = req.get_ret()) != 0)) {
                        ldout(get_context(), 1)
                                ...
                                struct stat * st, uint32_t mask, uint32_t flags)
    {
        ...
        if (parent->is_root()) {
            ...
            RGWPutObjRequest req(get_context(), user->clone(),
    ...
    MkObjResult RGWLibFS::create(RGWFileHandle * parent, const char *name,
        ...
        /* create it */
        buffer::list bl;
        RGWPutObjRequest req(cct, user->clone(), parent->bucket_name(),
                             obj_name, bl);
        MkObjResult mkr {
        nullptr, -EINVAL};
        ...
        if ((rc == 0) && (rc2 == 0)) {
            /* XXX atomicity */
            LookupFHResult fhr =
                lookup_fh(parent, name,
                          RGWFileHandle::FLAG_CREATE | RGWFileHandle::
                          FLAG_LOCK);
            RGWFileHandle *rgw_fh = get < 0 > (fhr);
            if (rgw_fh) {
    ...
    MkObjResult RGWLibFS::symlink(RGWFileHandle * parent, const char *name,
                                  const char *link_path, struct stat * st,
        ...
        /* XXXX */
#if 0
        bl.push_back(buffer::create_static(len, static_cast < char *>(buffer)));
#else
    ...
    int RGWLibFS::setattr(RGWFileHandle * rgw_fh, struct stat *st,
                          uint32_t mask, uint32_t flags) {
        int rc, rc2;
        buffer::list ux_key, ux_attrs;
        ...
        RGWSetAttrsRequest req(cct, user->clone(), rgw_fh->bucket_name(),
                               obj_name);
        ...
        if (rc == -ENOENT) {
            ...
            buffer::list bl;
            RGWPutObjRequest req(get_context(), user->clone(),
                                 rgw_fh->bucket_name(), obj_name, bl);
    ...
    static inline std::string_view unprefix_xattr_keystr(const std::
                                                         string & key) {
        std::string_view svk {
        key};
    ...
    int RGWLibFS::getxattrs(RGWFileHandle * rgw_fh, rgw_xattrlist * attrs,
        ...
        int rc, rc2, rc3;
        string obj_name {
        rgw_fh->relative_object_name2()};

        RGWGetAttrsRequest req(cct, user->clone(), rgw_fh->bucket_name(),
                               obj_name);
    ...
    int RGWLibFS::lsxattrs(RGWFileHandle * rgw_fh, rgw_xattrstr * filter_prefix,
                           rgw_getxattr_cb cb, void *cb_arg, uint32_t flags) {
        /* cannot store on fs_root, should not on buckets? */
        if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
            return -EINVAL;
        ...
        if (rc3 == 0) {
          ...
          for (const auto & k:keys) {
                ...
                std::string_view svk = is_exposed_attr(rgw_xattrstr {
                                                       ...
                                                       uint32_t(k.first.
                                                                length())})
                ? k.first : unprefix_xattr_keystr(k.first);
    ...
    int RGWLibFS::rmxattrs(RGWFileHandle * rgw_fh, rgw_xattrlist * attrs,
        ...
        RGWRMAttrsRequest req(cct, user->clone(), rgw_fh->bucket_name(),
                              obj_name);
        ...
        class ObjUnref {
          ...
          public:
             explicit ObjUnref(RGWLibFS * _fs):fs(_fs) {
                ...
                << __PRETTY_FUNCTION__
                    << fh->name << " before ObjUnref refs=" << fh->get_refcnt()
                << dendl;
                fs->unref(fh);
        }};
    ...
    void RGWLibFS::gc() {
        ...
        do {
                ...
                << " now=" << now << " expire_s=" << expire_s << dendl;
            {
                ...
                lsubdout(get_context(), rgw, 15)
                    << "GC: processing" << " count=" << events.size()
                    << " events" << dendl;
                ...
                for (uint32_t ix = 0; (ix < _max_ev) && (events.size() > 0);
                     ++ix) {
                    ...
                    if (rgw_fh) {
                        ...
                        if (d) {
                            ...
                            if (unlikely(ev_ts < d_last_readdir)) {
                                ...
                                lsubdout(get_context(), rgw, 15)
                                    << "GC: expiring " << rgw_fh->object_name()
                                    << dendl;
                                rgw_fh->clear_state();
                                rgw_fh->invalidate();
                            }
    ...
    DecodeAttrsResult RGWFileHandle::decode_attrs(const ceph::buffer::list *
                                                  ux_key1,
    ...
    bool RGWFileHandle::reclaim(const cohort::lru::ObjectFactory * newobj_fac) {
        lsubdout(fs->get_context(), rgw, 17)
            << __func__ << " " << *this << dendl;
        auto factory =
            dynamic_cast < const RGWFileHandle::Factory * >(newobj_fac);
        if (factory == nullptr) {
            return false;
        }
        ...
        if (!fs->fh_cache.
            is_same_partition(factory->fhk.fh_hk.object, fh.fh_hk.object)) {
            return false;
        }
    ...
    bool RGWFileHandle::has_children() const {
        ...
        RGWRMdirCheck req(fs->get_context(),
                          g_rgwlib->get_driver()->get_user(fs->get_user()->
                                                           user_id), this);
        int rc = g_rgwlib->get_fe()->execute_req(&req);
        if (!rc) {
    ...
    int RGWFileHandle::readdir(rgw_readdir_cb rcb, void *cb_arg,
        ...
        lsubdout(cct, rgw, 10)
            << __func__ << " readdir called on " << object_name()
            << dendl;
        ...
        if (is_root()) {
            RGWListBucketsRequest req(cct,
                                      g_rgwlib->get_driver()->get_user(fs->
                                                                       get_user
                                                                       ()->
                                                                       user_id),
    ...
    int RGWFileHandle::write(uint64_t off, size_t len, size_t * bytes_written,
        ...
        if (deleted()) {
            lsubdout(fs->get_context(), rgw, 5)
                << __func__
                << " write attempted on deleted object " << this->object_name()
                << dendl;
        ...
        if (!f->write_req) {
            ...
            const RGWProcessEnv & penv =
                g_rgwlib->get_fe()->get_process()->get_env();
            ...
            if (rc < 0) {
                ...
                if (stateless_open()) {
                    ...
                    f->write_req->timer_id =
                        ...
                        add_event(std::chrono::
                                  seconds(RGWLibFS::
                                          write_completion_interval_s),
        ...
        if (rc == 0) {
            ...
            if (stateless_open()) {
                /* bump write timer */
                RGWLibFS::write_timer.adjust_event(f->write_req->timer_id,
    ...
    int RGWFileHandle::write_finish(uint32_t flags) {
        ...
        if (f && (f->write_req)) {
            lsubdout(fs->get_context(), rgw, 10)
                << __func__ << " finishing write trans on " << object_name()
                << dendl;
    ...
    int RGWWriteRequest::exec_start() {
        ...
        auto compression_type =
            get_driver()->get_compression_type(state->bucket->
                                               get_placement_rule());
        ...
        if (state->object->empty()) {
            ldout(state->cct,
                  0) << __func__ << " called on empty object" << dendl;
            goto done;
        }
        ...
        processor =
            get_driver()->get_atomic_writer(this, state->yield,
                                            state->object.get(),
                                            state->bucket_owner.get_id(),
                                            &state->dest_placement, 0,
                                            state->req_id);
    ...
    int RGWWriteRequest::exec_continue() {
        ...
        op_ret =
            state->bucket->check_quota(this, quota, real_ofs, null_yield, true);
        /* max_size exceed */
        if (op_ret < 0)
            return -EIO;
    ...
    int RGWWriteRequest::exec_finish() {
        ...
        op_ret =
            state->bucket->check_quota(this, quota, state->obj_size, null_yield,
                                       true);
        /* max_size exceed */
        if (op_ret < 0) {
            goto done;
        ...
        for (iter = state->generic_attrs.begin();
             iter != state->generic_attrs.end(); ++iter) {
        ...
        if (unlikely(! !slo_info)) {
            ...
            emplace_attr(RGW_ATTR_SLO_UINDICATOR,
                         std::move(slo_userindicator_bl));
        }

        op_ret =
            processor->complete(state->obj_size, etag, &mtime, real_time(),
...
*/ int rgw_mount(librgw_t rgw, const char *uid, const char *acc_key,
        ...
        RGWLibFS *new_fs =
            new RGWLibFS(static_cast < CephContext * >(rgw), uid, acc_key,
                         sec_key, "/");
        ceph_assert(new_fs);

        const DoutPrefix dp(g_rgwlib->get_driver()->ctx(), dout_subsys,
                            "rgw mount: ");
    ...
    int rgw_mount2(librgw_t rgw, const char *uid, const char *acc_key,
                   const char *sec_key, const char *root,
                   struct rgw_fs **rgw_fs, uint32_t flags) {
        int rc = 0;
 ...
 */
    int rgw_symlink(struct rgw_fs *rgw_fs, struct rgw_file_handle *parent_fh,
                    ...
                    uint32_t mask, struct rgw_file_handle **fh,
    ...
    int rgw_lookup(struct rgw_fs *rgw_fs,
        ...
        if (parent->is_root()) {
            ...
            if (unlikely((strcmp(path, "..") == 0) || (strcmp(path, "/") == 0))) {
                ...
                fhr =
                    fs->stat_bucket(parent, path, bstat,
                                    RGWFileHandle::FLAG_NONE);
                rgw_fh = get < 0 > (fhr);
                if (!rgw_fh)
                    return -ENOENT;
            ...
            if (unlikely((strcmp(path, "..") == 0))) {
                ...
                if ((flags & RGW_LOOKUP_FLAG_RCB) && fast_attrs) {
                    ...
                    fhr =
                        fs->fake_leaf(parent, path, fh_type, st, mask,
                                      sl_flags);
                }
                else {
...
/* project offset of dirent name */
    int rgw_dirent_offset(struct rgw_fs *rgw_fs,
                          struct rgw_file_handle *parent_fh,
                          const char *name, int64_t * offset, uint32_t flags) {
        RGWFileHandle *parent = get_rgwfh(parent_fh);
        if ((!parent)) {
            /* bad parent */
            return -EINVAL;
...
*/
    ...
    int rgw_readv(struct rgw_fs *rgw_fs,
        ...
        RGWGetObjRequest req(cct, fs->get_user(), rgw_fh->bucket_name(),
                             rgw_fh->object_name(), uio->uio_offset,
                             uio->uio_resid, bl);
            ...
            RGWReadV *rdv =
                ...
                >(::
                  operator  new(sizeof(RGWReadV) +
                                (bl.buffers().size() *
                                 sizeof(struct rgw_vio))));

            (void)new(rdv)
                RGWReadV(bl,
                         reinterpret_cast <
                         rgw_vio * >(rdv + sizeof(RGWReadV)));
...
*/
    int rgw_writev(struct rgw_fs *rgw_fs, struct rgw_file_handle *fh,
        ...
        for (unsigned int ix = 0; ix < uio->uio_cnt; ++ix) {
            rgw_vio *vio = &(uio->uio_vio[ix]);
            bl.push_back(buffer::create_static(vio->vio_len,
                                               static_cast <
                                               char *>(vio->vio_base)));
        }
