 ...
 */
     ...
     */
    bool check_valid_placement(int ruleno, vector < int >in,
     ...
     */
    int random_placement(int ruleno, vector < int >&out, int maxout,
                         vector < __u32 > &weight);
    ...
    void write_to_csv(ofstream & csv_file, vector < string > &payload) {
        if (csv_file.good())
            for (vector < string >::iterator it = payload.begin();
                 it != payload.end(); ++it)
                csv_file << (*it);
    } void write_to_csv(ofstream & csv_file, map < int, float >&payload) {
        if (csv_file.good())
            for (map < int, float >::iterator it = payload.begin();
                 it != payload.end(); ++it)
    ...
    void write_data_set_to_csv(string user_tag, tester_data_set & tester_data) {
        ...
        ofstream
            placement_information_file((user_tag +
                                        (string) "-placement_information.csv").
                                       c_str());
        ofstream
            proportional_weights_file((user_tag +
                                       (string) "-proportional_weights.csv").
                                      c_str());
        ofstream
        ...
        ofstream
            absolute_weights_file((user_tag +
                                   (string) "-absolute_weights.csv").c_str());
        ...
        device_utilization_all_file <<
            "Device ID, Number of Objects Stored, Number of Objects Expected" <<
            std::endl;
        proportional_weights_file << "Device ID, Proportional Weight" << std::
            endl;
        ...
        write_to_csv(device_utilization_all_file,
                     tester_data.device_utilization_all);
        write_to_csv(placement_information_file,
                     tester_data.placement_information);
        write_to_csv(proportional_weights_file,
                     tester_data.proportional_weights);
        write_to_csv(proportional_weights_all_file,
                     tester_data.proportional_weights_all);
        ...
        if (num_batches > 1) {
            ...
            write_to_csv(batch_device_utilization_all_file,
                         tester_data.batch_device_utilization_all);
            write_to_csv(batch_device_expected_utilization_all_file,
    ...
    void write_integer_indexed_vector_data_string(vector < string > &dst,
                                                  int index,
                                                  vector < int >vector_data);
    void write_integer_indexed_vector_data_string(vector < string > &dst,
                                                  int index,
                                                  vector < float >vector_data);
    void write_integer_indexed_scalar_data_string(vector < string > &dst,
                                                  int index, int scalar_data);
    void write_integer_indexed_scalar_data_string(vector < string > &dst,
                                                  int index, float scalar_data);
  ...
  :    
    ...
    output_data_file(false), output_csv(false), output_data_file_name("")
    {
    }
