...
namespace crimson::os::seastore {
    ...
    struct device_config_t {
        ...
        DENC(device_config_t, v, p) {
            ...
            DENC_FINISH(p);
        } static device_config_t create_primary(uuid_d new_osd_fsid,
                                                device_id_t id,
                                                device_type_t d_type,
                                                secondary_device_set_t sds) {
            return device_config_t {
                true, device_spec_t {
                (magic_t) std::rand(), d_type, id}
                , seastore_meta_t {
                new_osd_fsid}
            , sds};
        }
        static device_config_t create_secondary(uuid_d new_osd_fsid,
                                                device_id_t id,
                                                device_type_t d_type,
                                                magic_t magic) {
            return device_config_t {
...
// interfaces used by device
      public:
        ...
        using access_ertr = crimson::errorator <
            ...
            crimson::ct_error::permission_denied, crimson::ct_error::enoent >;
        ...
        static seastar::future < DeviceRef >
...
template <> struct fmt::formatter <
    crimson::os::seastore::device_config_t >:fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::device_spec_t >:fmt::ostream_formatter {
};
#endif
