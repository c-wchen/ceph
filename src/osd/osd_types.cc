...
void pg_shard_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(osd, bl);
    ::encode(shard, bl);
...
void object_locator_t::encode(bufferlist & bl) const const
{
...
// -- request_redirect_t --
void request_redirect_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void objectstore_perf_stat_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
// -- osd_stat_t --
void osd_stat_t::dump(Formatter * f) const const
{
...
void osd_stat_t::encode(bufferlist & bl) const const
{
    ENCODE_START(7, 2, bl);
    ::encode(kb, bl);
    ::encode(kb_used, bl);
...
{
    if (preferred() >= 0)
        return snprintf(o, maxlen, "%llu.%xp%d", (unsigned long long)pool(),
                        ps(), preferred());
    else
...
bool pg_t::parse(const char *s)
{
    ...
    int32_t pref;
    int r =
        sscanf(s, "%llu.%xp%d", (long long unsigned *)&ppool, &pseed, &pref);
    if (r < 2)
        return false;
    m_pool = ppool;
    m_seed = pseed;
...
void coll_t::encode(bufferlist & bl) const const
{
...
void pool_snap_info_t::encode(bufferlist & bl, uint64_t features) const const
{
...
{
    for (opt_mapping_t::iterator i = opt_mapping.begin();
         i != opt_mapping.end(); ++i) {
...
void pool_opts_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void pg_pool_t::dump(Formatter * f) const const
{
    ...
    for (set < uint64_t >::const_iterator p = tiers.begin(); p != tiers.end();
         ++p)
 ...
 */
void pg_pool_t::build_removed_snaps(interval_set < snapid_t > &rs) const const
{
    if (is_pool_snaps_mode()) {
        rs.clear();
...
bool pg_pool_t::maybe_updated_removed_snaps(const interval_set < snapid_t >
                                            &cached) const const
{
...
snapid_t pg_pool_t::snap_exists(const char *s) constconst
{
    for (map < snapid_t, pool_snap_info_t >::const_iterator p = snaps.begin();
         p != snaps.end(); ++p)
        if (p->second.name == s)
...
SnapContext pg_pool_t::get_snap_context() const const
{
    ...
    unsigned i = 0;
    for (map < snapid_t, pool_snap_info_t >::const_reverse_iterator p =
         snaps.rbegin(); p != snaps.rend(); ++p)
        s[i++] = p->first;
...
void pg_pool_t::encode(bufferlist & bl, uint64_t features) const const
{
    if ((features & CEPH_FEATURE_PGPOOL3) == 0) {
...
void object_stat_sum_t::encode(bufferlist & bl) const const
{
...
void object_stat_collection_t::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(sum, bl);
    ::encode((__u32) 0, bl);
                                                       ...
                                                       * >&o)
{
    ...
    for (list < object_stat_sum_t * >::iterator p = l.begin(); p != l.end();
         ++p) {
        a.add(**p);
        o.push_back(new object_stat_collection_t(a));
    }
...
void pg_stat_t::dump(Formatter * f) const const
{
    ...
    for (vector < int32_t >::const_iterator p = acting.begin();
         p != acting.end(); ++p)
        f->dump_int("osd", *p);
    f->close_section();
...
void pg_stat_t::encode(bufferlist & bl) const const
{
...
void pool_stat_t::encode(bufferlist & bl, uint64_t features) const const
{
    if ((features & CEPH_FEATURE_OSDENC) == 0) {
        __u8 v = 4;
        ::encode(v, bl);
...
void pg_history_t::encode(bufferlist & bl) const const
{
    ENCODE_START(9, 4, bl);
...
void pg_info_t::encode(bufferlist & bl) const const
{
...
// -- pg_notify_t --
void pg_notify_t::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
...
{
    o.push_back(new
                pg_notify_t(shard_id_t(3), shard_id_t::NO_SHARD, 1, 1,
                            pg_info_t()));
    o.push_back(new
                pg_notify_t(shard_id_t(0), shard_id_t(0), 3, 10, pg_info_t()));
}
...
void PastIntervals::pg_interval_t::encode(bufferlist & bl) const const
{
    ENCODE_START(4, 2, bl);
    ::encode(first, bl);
    ::encode(last, bl);
...
void PastIntervals::pg_interval_t::dump(Formatter * f) const const
{
    ...
    f->close_section();
    f->open_array_section("acting");
    for (vector < int >::const_iterator p = acting.begin(); p != acting.end();
         ++p)
        f->dump_int("osd", *p);
    f->close_section();
...
pair < epoch_t, epoch_t > get_bounds()const override {
    ...
    if (iter != interval_map.end()) {
        ...
        return make_pair(iter->second.first, riter->second.last + 1);
    }
    else {
...
bool is_classic() const override {
    ...
    o.push_back(new pi_simple_rep(true, ivallst {
                                  ival { {
                                  0, 1, 2}, {
    ...
    o.push_back(new pi_simple_rep(false, ivallst {
                                  ival { {
                                  0, 1, 2}, {
                                  0, 1, 2}, 10, 20, true, 0, 0}
                                  , ival { {
...
void iterate_mayberw_back_to(bool ec_pool,
                             epoch_t les,
    ...
    for (auto i = interval_map.rbegin(); i != interval_map.rend(); ++i) {
        ...
        for (unsigned j = 0; j < i->second.acting.size(); ++j) {
            if (i->second.acting[j] == CRUSH_ITEM_NONE)
                continue;
            actingset.insert(pg_shard_t(i->second.acting[j],
                                        ec_pool ? shard_id_t(j) : shard_id_t::
                                        NO_SHARD));
        } f(i->second.first, actingset);
    }
}

bool has_full_intervals() const override {
    return true;
} void iterate_all_intervals(std::function <
    ...
    if (!compact) {
        ...
        if (!past_intervals) {
            ...
            old->iterate_all_intervals([&](const pg_interval_t & i) {
                                       past_intervals->add_interval(ec_pool,
                                                                    i);});
        }
    }
}
                                       ...
                                       std::ostream * out)
{
    ...
    if (is_new_interval(old_acting_primary,
        ...
        for (vector < int >::const_iterator p = i.acting.begin();
             p != i.acting.end(); ++p)
...
void pg_query_t::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    ::encode(type, bl);
    ::encode(since, bl);
...
void ObjectModDesc::encode(bufferlist & _bl) const const
{
...
void pg_log_entry_t::encode_with_checksum(bufferlist & bl) const const
{
...
void pg_log_entry_t::encode(bufferlist & bl) const const
{
...
void pg_log_entry_t::dump(Formatter * f) const const
{
    ...
    for (auto p = extra_reqids.begin(); p != extra_reqids.end(); ++p) {
...
string pg_log_dup_t::get_key_name() const const
{
    return "dup_" + version.get_key_name();
}

void pg_log_dup_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(reqid, bl);
    ::encode(version, bl);
...
void pg_log_t::encode(bufferlist & bl) const const
{
    ENCODE_START(7, 3, bl);
    ::encode(head, bl);
    ::encode(tail, bl);
...
void pg_log_t::copy_range(const pg_log_t & other, eversion_t from,
                          eversion_t to)
{
    can_rollback_to = other.can_rollback_to;
...
void object_copy_cursor_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void object_copy_data_t::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(7, 5, bl);
    ::encode(size, bl);
    ::encode(mtime, bl);
...
void object_copy_data_t::dump(Formatter * f) const const
{
    ...
    for (auto p = reqids.begin(); p != reqids.end(); ++p) {
        f->open_object_section("extra_reqid");
        f->dump_stream("reqid") << p->first;
        f->dump_stream("user_version") << p->second;
        f->close_section();
...
void pg_create_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(created, bl);
    ::encode(parent, bl);
...
void pg_hit_set_info_t::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 1, bl);
    ::encode(begin, bl);
    ::encode(end, bl);
...
void pg_hit_set_history_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
...
void osd_peer_stat_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(stamp, bl);
    ENCODE_FINISH(bl);
...
void osd_peer_stat_t::dump(Formatter * f) const const
{
    f->dump_stream("stamp") << stamp;
}
...
void OSDSuperblock::encode(bufferlist & bl) const const
{
    ENCODE_START(8, 5, bl);
...
void SnapSet::encode(bufferlist & bl) const const
{
    ENCODE_START(3, 2, bl);
    ::encode(seq, bl);
...
void SnapSet::dump(Formatter * f) const const
{
    ...
    for (vector < snapid_t >::const_iterator p = clones.begin();
         p != clones.end(); ++p) {
...
void SnapSet::from_snap_set(const librados::snap_set_t & ss, bool legacy)
{
    ...
    head_exists = false;
    for (vector < librados::clone_info_t >::const_iterator p =
         ss.clones.begin(); p != ss.clones.end(); ++p) {
    ...
    for (set < snapid_t >::iterator p = _clones.begin(); p != _clones.end();
         ++p)
        clones.push_back(*p);
...
void watch_info_t::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(4, 3, bl);
    ::encode(cookie, bl);
...
void object_manifest_t::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(type, bl);
    switch (type) {
...
void object_manifest_t::dump(Formatter * f) const const
{
...
{
    return out << "type:" << om.type << " redirect_target:" << om.
        redirect_target;
}
...
void object_info_t::encode(bufferlist & bl, uint64_t features) const const
{
    object_locator_t myoloc(soid);
    map < entity_name_t, watch_info_t > old_watchers;
    for (map < pair < uint64_t, entity_name_t >,
         watch_info_t >::const_iterator i = watchers.begin();
         i != watchers.end(); ++i) {
...
void object_info_t::decode(bufferlist::iterator & bl)
{
    ...
    if (struct_v >= 11) {
        ...
        for (map < entity_name_t, watch_info_t >::iterator i =
             old_watchers.begin(); i != old_watchers.end(); ++i) {
            watchers.
                insert(make_pair
...
void object_info_t::dump(Formatter * f) const const
{
  ...
  for (auto s:legacy_snaps) {
    ...
    for (map < pair < uint64_t, entity_name_t >,
         watch_info_t >::const_iterator p = watchers.begin();
         p != watchers.end(); ++p) {
        stringstream ss;
        ss << p->first.second;
...
// -- ObjectRecovery --
void ObjectRecoveryProgress::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(first, bl);
    ::encode(data_complete, bl);
...
ostream & ObjectRecoveryProgress::print(ostream & out) const const
{
...
void ObjectRecoveryProgress::dump(Formatter * f) const const
{
...
void ObjectRecoveryInfo::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(2, 1, bl);
    ::encode(soid, bl);
    ::encode(version, bl);
...
void ObjectRecoveryInfo::decode(bufferlist::iterator & bl, int64_t pool)
{
    DECODE_START(2, bl);
    ::decode(soid, bl);
    ::decode(version, bl);
    ...
    if (struct_v < 2) {
        ...
        for (map < hobject_t, interval_set < uint64_t >>::iterator i =
             tmp.begin(); i != tmp.end(); ++i) {
...
void ObjectRecoveryInfo::dump(Formatter * f) const const
{
    f->dump_stream("object") << soid;
...
ostream & ObjectRecoveryInfo::print(ostream & out) const const
{
...
void PushReplyOp::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(soid, bl);
    ENCODE_FINISH(bl);
...
void PushReplyOp::dump(Formatter * f) const const
{
    f->dump_stream("soid") << soid;
}

ostream & PushReplyOp::print(ostream & out) const const
{
...
uint64_t PushReplyOp::cost(CephContext * cct) const const
{
...
void PullOp::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(soid, bl);
...
void PullOp::dump(Formatter * f) const const
{
    f->dump_stream("soid") << soid;
    {
...
ostream & PullOp::print(ostream & out) const const
{
    return out
        << "PullOp(" << soid
...
uint64_t PullOp::cost(CephContext * cct) const const
{
    return cct->_conf->osd_push_per_object_cost +
        cct->_conf->osd_recovery_max_chunk;
}
...
void PushOp::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(soid, bl);
    ::encode(version, bl);
...
void PushOp::dump(Formatter * f) const const
{
    f->dump_stream("soid") << soid;
    f->dump_stream("version") << version;
...
ostream & PushOp::print(ostream & out) const const
{
    return out
        << "PushOp(" << soid
        << ", version: " << version
...
uint64_t PushOp::cost(CephContext * cct) const const
{
    uint64_t cost = data_included.size();
    for (map < string, bufferlist >::const_iterator i =
         omap_entries.begin(); i != omap_entries.end(); ++i) {
...
void ScrubMap::encode(bufferlist & bl) const const
{
    ENCODE_START(3, 2, bl);
    ::encode(objects, bl);
...
void ScrubMap::dump(Formatter * f) const const
{
    f->dump_stream("valid_through") << valid_through;
    f->dump_stream("incremental_since") << incr_since;
    f->open_array_section("objects");
    for (map < hobject_t, object >::const_iterator p = objects.begin();
         p != objects.end(); ++p) {
...
void ScrubMap::object::encode(bufferlist & bl) const const
{
...
void ScrubMap::object::dump(Formatter * f) const const
{
    f->dump_int("size", size);
...
bool store_statfs_t::operator==(const store_statfs_t & other) const const
{
...
ostream & operator<<(ostream & out, const store_statfs_t & s)
{
    out << std::hex
        ...
        << "/0x" << s.compressed_original << std::dec << ")";
    return out;
}
