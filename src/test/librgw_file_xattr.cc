...
TEST(LibRGW, CREATE_BUCKET)
{
    if ((!bucket_fh) && do_create) {
        ...
        int ret =
            rgw_mkdir(fs, fs->root_fh, bucket_name.c_str(), &st, create_mask,
...
TEST(LibRGW, CREATE_PATH)
{
    ...
    for (ix = 0; ix < sz; ++ix) {
        auto & seg = segs[ix];
        struct rgw_file_handle *parent_fh =
            (ix > 0) ? ovec[ix - 1].fh : bucket_fh;
        obj_rec dir {
        seg, nullptr, parent_fh, nullptr};
        if (do_create) {
            ret =
                rgw_mkdir(fs, dir.parent_fh, dir.name.c_str(), &st, create_mask,
...
extern "C" {
    static int getattr_cb(rgw_xattrlist * attrs, void *arg, uint32_t flags) {
        ...
        for (uint32_t ix = 0; ix < attrs->xattr_cnt; ++ix) {
            ...
            string v {
            xattr.val.val, xattr.val.len};
            if (verbose) {
                std::cout << __func__
                    << " attr k: " << k << " v: " << v << std::endl;
            } attrmap.insert(std::map < std::string,
                             std::string >::value_type(k, v));
        }
        return 0;
    }
}
