...
// TODO: replace by// using match_history_t = int8_t;//     left_m, str_m, right_m//  3: GT,//  2: EQ,     GT,//  1: EQ,     EQ,    GT//  0: EQ,     EQ,    EQ// -1: EQ,     EQ,    LT// -2: EQ,     LT,// -3: LT, struct MatchHistory {
...
template < match_stage_t STAGE > void set(MatchKindCMP match)
{
    ...
    assert(!get < STAGE > ().has_value()
           || *get < STAGE > () != MatchKindCMP::EQ);
    const_cast < std::optional < MatchKindCMP > &>(get < STAGE >()) =
        match;
}
...
std::ostream &dump(std::ostream &os) const
{
    ...
    return os;
} std::ostream &dump_each(std::ostream &os,
...
std::ostream &operator<<(std::ostream &os,
                         const staged_position_t < STAGE > &pos)
{
...
staged_position_t < STAGE > &cast_down(search_position_t &pos)
{
    ...
    return const_cast < staged_position_t < STAGE > &>(cast_down < STAGE >
            (_pos));
}
...
template < match_stage_t STAGE, typename =
           std::enable_if_t < STAGE !=
                              STAGE_TOP >> search_position_t normalize(staged_position_t < STAGE >
                                      &&pos)
{
    if (pos.is_end()) {
...
using value_input_type_t =
    typename value_input_type < NODE_TYPE >::type;
...
template < node_type_t NODE_TYPE, match_stage_t STAGE > struct staged_result_t {
    ...
    {
        return {
            staged_position_t < STAGE >::end(), nullptr, MSTAT_END};
    }
    template < typename T = me_t >
    static std::enable_if_t < STAGE != STAGE_BOTTOM,
           T > from_nxt(index_t index, const staged_result_t < NODE_TYPE,
                        STAGE - 1 > &nxt_stage_result)
    {
        return { {
                index, nxt_stage_result.position
            }
            , nxt_stage_result.p_value, nxt_stage_result.mstat};
    }
...
template < node_type_t NODE_TYPE >
lookup_result_t < NODE_TYPE > &&normalize(lookup_result_t < NODE_TYPE >
        &&result)
{
...
lookup_result_t < NODE_TYPE > normalize(staged_result_t < NODE_TYPE,
                                        STAGE > &&result)
{
    // FIXME: assert result.mstat correct
    return {
        normalize(std::move(result.position)), result.p_value, result.mstat};
}
...
struct fmt::formatter < crimson::os::seastore::onode::staged_position_t <
    S >>: fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::onode::MatchHistory > : fmt::ostream_formatter {
};
#endif
