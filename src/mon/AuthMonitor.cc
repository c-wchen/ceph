...
bool AuthMonitor::check_rotate()
{
    ...
    if (!mon->key_server.
        updated_rotating(rot_inc.rotating_bl, last_rotating_ver))
        return false;
...
void AuthMonitor::update_from_paxos(bool * need_bootstrap)
{
    ...
    dout(10) << __func__ << " key server version " << mon->key_server.
        get_ver() << dendl;
...
uint64_t AuthMonitor::assign_global_id(MonOpRequestRef op,
                                       bool should_increase_max)
{
    MAuth *m = static_cast < MAuth * >(op->get_req());
    int total_mon = mon->monmap->size();
    dout(10) << "AuthMonitor::assign_global_id m=" << *m << " mon=" << mon->
        rank << "/" << total_mon << " last_allocated=" << last_allocated_id <<
    ...
    while (mon->is_leader() &&
           (max_global_id < g_conf->mon_globalid_prealloc ||
            next_global_id >=
            max_global_id - g_conf->mon_globalid_prealloc / 2)) {
...
{
    MAuth *m = static_cast < MAuth * >(op->get_req());
    dout(10) << "prep_auth() blob_size=" << m->get_auth_payload().
        length() << dendl;
    ...
    if (!s->global_id) {
        ...
        if (!s->global_id) {
            ...
            if (mon->is_leader() && paxos_writable) {
                dout(10) << "increasing global id, waitlisting message" <<
                    dendl;
            ...
            if (!mon->is_leader()) {
                dout(10) << "not the leader, requesting more ids from leader" <<
                    dendl;
                int leader = mon->get_leader();
                MMonGlobalID *req = new MMonGlobalID();
                req->old_max_id = max_global_id;
                mon->messenger->send_message(req,
                                             mon->monmap->get_inst(leader));
    ...
    try {
        ...
        if (start) {
            ...
            proto =
                s->auth_handler->start_session(entity_name, indata, response_bl,
                                               caps_info);
            ret = 0;
            ...
            // request
            ret =
                s->auth_handler->handle_request(indata, response_bl,
                                                s->global_id, caps_info, &auid);
        }
        if (ret == -EIO) {
            wait_for_active(op, new C_RetryMessage(this, op));
            goto done;
        ...
        if (caps_info.caps.length()) {
            ...
            try {
                ...
                derr << "corrupt cap data for " << entity_name << " in auth db"
                    << dendl;
                str.clear();
            }
    ...
    if (prefix == "auth export") {
        ...
        if (!mon->key_server.get_auth(entity, entity_auth)) {
            ...
            r = 0;
             ...
             prefix == "auth print_key" || prefix == "auth get-key") {
        EntityAuth auth;
...
{
    for (map < EntityName, EntityAuth >::iterator p =
         keyring.get_keys().begin(); p != keyring.get_keys().end(); ++p) {
...
int AuthMonitor::exists_and_matches_entity(const auth_entity_t & entity,
                                           bool has_secret, stringstream & ss)
{
    return exists_and_matches_entity(entity.name, entity.auth,
                                     entity.auth.caps, has_secret, ss);
}

int AuthMonitor::exists_and_matches_entity(const EntityName & name,
                                           const EntityAuth & auth,
                                           const map < string,
                                           bufferlist > &caps, bool has_secret,
                                           stringstream & ss)
{
...
int AuthMonitor::validate_osd_destroy(int32_t id,
                                      const uuid_d & uuid,
...
int AuthMonitor::do_osd_destroy(const EntityName & cephx_entity,
                                const EntityName & lockbox_entity)
{
    ...
    if (err == -ENOENT) {
        dout(10) << __func__ << " " << lockbox_entity << " does not exist" <<
            dendl;
    }
    else {
        removed = true;
...
int _create_auth(EntityAuth & auth,
                 const string & key, const map < string, bufferlist > &caps)
{
    if (key.empty())
        return -EINVAL;
    try {
...
int AuthMonitor::validate_osd_new(int32_t id,
                                  const uuid_d & uuid,
...
int AuthMonitor::do_osd_new(const auth_entity_t & cephx_entity,
...
bool AuthMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "auth import") {
        ...
        if (!has_keyring) {
            dout(10) <<
                "AuthMonitor::prepare_command generating random key for " <<
                auth_inc.name << dendl;
        ...
        return true;
              ...
              prefix == "auth get-or-create") && !entity_name.empty()) {
        ...
        if (mon->key_server.get_auth(entity, entity_auth)) {
          for (const auto & sys_cap:wanted_caps) {
                if (entity_auth.caps.count(sys_cap.first) == 0 ||
                    !entity_auth.caps[sys_cap.first].contents_equal(sys_cap.
                                                                    second)) {
                    ss << "key for " << entity << " exists but cap " << sys_cap.
          ...
          for (const auto & sys_cap:wanted_caps) {
                if (entity_auth.caps.count(sys_cap.first) == 0 ||
                    ...
                    ss << entity <<
                        ...
                        << entity << ", first remove " << entity <<
                        " from configuration files, execute 'ceph auth rm " <<
                        entity << "', then execute this command again.";
                    err = -EINVAL;
                    goto done;
                }
            }
...
void AuthMonitor::upgrade_format()
{
    ...
    if (format_version == 0) {
        ...
        for (p = mon->key_server.secrets_begin();
             p != mon->key_server.secrets_end(); ++p) {
            ...
            if ((p->first.is_osd() || p->first.is_mds()) &&
                mon_caps == "allow rwx") {
                new_caps =
                    string("allow profile ") + string(p->first.get_type_name());
            }
            ...
            if (new_caps.length() > 0) {
                dout(5) << __func__ << " updating " << p->
                    first << " mon cap from " << mon_caps << " to " << new_caps
                    << dendl;
    ...
    if (format_version == 1) {
        ...
        for (p = mon->key_server.secrets_begin();
            ...
            if (n.find("mgr.") == 0 && p->second.caps.count("mon")) {
                ...
                if (oldcaps == "allow *") {
                    dout(5) << " fixing " << n <<
                        " mon cap to 'allow profile mgr'" << dendl;
                    bufferlist bl;
    ...
    if (changed) {
        // note new format
        dout(10) << __func__ << " proposing update from format " <<
