...
static std::
string map_option_read_from_replica_cb(const char *value_char)
{
...
static std::
string map_option_compression_hint_cb(const char *value_char)
{
    if (!strcmp(value_char, "none")
        || !strcmp(value_char, "compressible")
...
static std::string map_option_ms_mode_cb(const char *value_char)
{
    if (!strcmp(value_char, "legacy") || !strcmp(value_char, "crc")
        || !strcmp(value_char, "secure")
        || !strcmp(value_char, "prefer-crc")
...
static void put_map_option(const std::string &key,
...
static int put_map_option_value(const std::string &opt,
                                ...
                                MapOptions *map_options)
{
    if (!value_char || *value_char == '\0') {
        std::
        cerr << "rbd: " << opt << " option requires a value" <<
             std::endl;
        return -EINVAL;
    }
    ...
    for (char *this_char = strtok(options, ", ");
        ...
        if (!strcmp(this_char, "fsid")) {
            if (put_map_option_value
                ("fsid", value_char, map_option_uuid_cb,
                 map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "ip")) {
            if (put_map_option_value
                ("ip", value_char, map_option_ip_cb, map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "share")
                   || !strcmp(this_char, "noshare")) {
            put_map_option("share", this_char, map_options);
        } else if (!strcmp(this_char, "crc")
                   || !strcmp(this_char, "nocrc")) {
            ...
            if (put_map_option_value
                ("mount_timeout", value_char, map_option_int_cb,
                 map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "osd_request_timeout")) {
            if (put_map_option_value
                ("osd_request_timeout", value_char,
                 map_option_int_cb, map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "lock_timeout")) {
            if (put_map_option_value
                ("lock_timeout", value_char, map_option_int_cb,
                 map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "osdkeepalive")) {
            if (put_map_option_value
                ("osdkeepalive", value_char, map_option_int_cb,
                 map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "osd_idle_ttl")) {
            if (put_map_option_value
                ("osd_idle_ttl", value_char, map_option_int_cb,
                 map_options)) {
                return -EINVAL;
            }
        } else if (!strcmp(this_char, "rw")
                   || !strcmp(this_char, "ro")) {
            put_map_option("rw", this_char, map_options);
        } else if (!strcmp(this_char, "queue_depth")) {
            if (put_map_option_value
                ("queue_depth", value_char, map_option_int_cb,
                 map_options)) {
                return -EINVAL;
            }
            ...
            if (put_map_option_value
                ("alloc_size", value_char, map_option_int_cb,
                 map_options)) {
                return -EINVAL;
            }
        ...
        } else if (!strcmp(this_char, "udev")
                   || !strcmp(this_char, "noudev")) {
            put_map_option("udev", this_char, map_options);
        } else {
            std::
            cerr << "rbd: unknown map option '" << this_char <<
                 "'" << std::endl;
            return -EINVAL;
        }
    }
                               ...
                               MapOptions *unmap_options)
{
    ...
    for (char *this_char = strtok(options, ", ");
        ...
        if (!strcmp(this_char, "force")) {
            ...
            std::
            cerr << "rbd: unknown unmap option '" << this_char
                 << "'" << std::endl;
            return -EINVAL;
        }
    }
                                    ...
                                    int maperrno)
{
    ...
    r = utils::init_and_open_image(poolname, nspace_name, imgname,
                                   "", snapname, true, &rados,
                                   &ioctx, &image);
    if (r < 0) {
        goto done;
    ...
    if (!oldformat && (maperrno == -ENXIO)) {
        ...
        if (r == 0
            && (unsupported_features & ~RBD_FEATURES_ALL) == 0) {
            uint64_t immutable =
                RBD_FEATURES_ALL & ~(RBD_FEATURES_MUTABLE |
...
static int do_kernel_map(const char *poolname,
                         const char *nspace_name,
                         ...
                         MapOptions && map_options)
{
    ...
    r = krbd_is_mapped(krbd, poolname, nspace_name, imgname,
                       snapname, &devnode);
    if (r < 0) {
        std::
        cerr <<
             "rbd: warning: can't get image map information: " <<
             cpp_strerror(r) << std::endl;
    } else if (r > 0) {
        std::
        cerr << "rbd: warning: image already mapped as " <<
             devnode << std::endl;
        free(devnode);
    }
    ...
    if (r < 0) {
        print_error_description(poolname, nspace_name, imgname,
                                snapname, r);
        goto out;
    }
...
static int do_kernel_unmap(const char *dev, const char *poolname,
                           const char *nspace_name,
                           const char *imgname,
                           const char *snapname, MapOptions
                           && unmap_options)
{
#if defined(WITH_KRBD)
    struct krbd_ctx *krbd;
    ...
    for (auto it = unmap_options.begin();
         it != unmap_options.end();) {
        if (it->first == "udev") {
            if (it->second == "noudev") {
                flags |= KRBD_CTX_F_NOUDEV;
            }
    ...
    if (dev) {
        ...
        r = krbd_unmap_by_spec(krbd, poolname, nspace_name, imgname,
                               snapname, oss.str().c_str());
                 ...
                 &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: device list failed: " << cpp_strerror(r)
             << std::endl;
        return r;
    }
    return 0;
                ...
                &ceph_global_init_args)
{
    ...
    MapOptions map_options;
    if (vm.count("options")) {
        for (auto &options : vm["options"].as < std::vector <
             std::
             string >> ()) {
            r = parse_map_options(options, &map_options);
            if (r < 0) {
                std::
                cerr << "rbd: couldn't parse map options" <<
                     std::endl;
                return r;
            }
        }
    ...
    if (vm["quiesce"].as < bool > ()) {
        std::
        cerr << "rbd: warning: quiesce is not supported" <<
             std::endl;
    }
    if (vm.count("quiesce-hook")) {
        std::
        cerr << "rbd: warning: quiesce-hook is not supported" <<
             std::endl;
    }
    ...
    for (const auto &option : options) {
        if (option.name == "rbd_default_map_options") {
            r = parse_map_options(option.value,
                                  &default_map_options);
            if (r < 0) {
                std::
                cerr <<
                     "rbd: couldn't parse default map options" <<
                     std::endl;
                return r;
            }
    ...
    r = do_kernel_map(pool_name.c_str(), nspace_name.c_str(),
                      image_name.c_str(), snap_name.c_str(),
                      std::move(map_options));
    if (r < 0) {
        std::cerr << "rbd: map failed: " << cpp_strerror(r) << std::
                  endl;
        return r;
    }
                  ...
                  &ceph_global_init_args)
{
    ...
    if (vm.count("options")) {
        for (auto &options : vm["options"].as < std::vector <
            ...
            if (r < 0) {
                std::
                cerr << "rbd: couldn't parse unmap options" <<
                     std::endl;
                return r;
            }
        }
    ...
    r = do_kernel_unmap(device_name.empty() ? nullptr : device_name.
                        c_str(), pool_name.c_str(),
                        nspace_name.c_str(), image_name.c_str(),
                        snap_name.c_str(),
                        std::move(unmap_options));
    if (r < 0) {
        std::
        cerr << "rbd: unmap failed: " << cpp_strerror(r) <<
             std::endl;
        return r;
    }
    return 0;
