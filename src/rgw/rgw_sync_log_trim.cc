...
struct TrimCounters {
    ...
    class Handler:public TrimNotifyHandler {
      ...
      public:
        ...
        } void handle(bufferlist::iterator & input,
...
class BucketTrimWatcher:public librados::WatchCtx2 {
  ...
  public:
    ...
    void handle_error(uint64_t cookie, int err) override {
        ...
        if (err == -ENOTCONN) {
            ldout(store->ctx(),
                  4) << "Disconnected watch on " << ref.oid << dendl;
            restart();
        }
    }
};
...
struct BucketTrimObserver {
    ...
    virtual bool trimmed_recently(const boost::string_view & bucket_instance) =
        0;
};
...
class BucketTrimShardCollectCR:public RGWShardCollectCR {
  ...
  public:
    BucketTrimShardCollectCR(RGWRados * store,
                             const RGWBucketInfo & bucket_info,
                             const std::vector < std::string > &markers)
    :RGWShardCollectCR(store->ctx(), MAX_CONCURRENT_SHARDS), store(store),
        bucket_info(bucket_info), markers(markers) {
...
bool BucketTrimShardCollectCR::spawn_next()
{
    while (i < markers.size()) {
        ...
        if (!marker.empty()) {
            ...
            spawn(new
                  RGWRadosBILogTrimCR(store, bucket_info, shard_id,
                                      std::string {
                                      }
                                      , marker), false);
            return true;
    ...
    reenter(this) {
        ...
        yield {
          ...
          for (auto & c:store->zone_conn_map) {
            ...
            spawn(new
                  RGWGetBucketInstanceInfoCR(store->get_async_rados(), store,
        ...
        if (retcode < 0) {
            ldout(cct,
                  4) << "failed to correlate bucket sync status from peers" <<
                dendl;
        ...
        ldout(cct, 10) << "trimming bilogs for bucket=" << bucket_info.bucket
            << " markers=" << min_markers << ", shards=" << min_markers.
            size() << dendl;
        set_status("trimming bilog shards");
        yield call(new
                   BucketTrimShardCollectCR(store, bucket_info, min_markers));
...
int AsyncMetadataList::_send_request()
{
    ...
    if (r == -EINVAL) {
        ...
        ldout(cct,
              20) << "starting metadata listing at " << start_marker << dendl;
...
class BucketTrimPollCR:                                     public RGWCoroutine
                                                             ...
                                                             * const observer;
                                                             ...
                                                             string name {
                                                             "trim"};   //< lock name
  const std::string cookie; public:
                                                             BucketTrimPollCR
                                                             (RGWRados * store,
                                                              RGWHTTPManager *
                                                              http,
