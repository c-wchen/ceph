...
class rgw_obj_select {
  ...
  public:
    ...
    explicit rgw_obj_select(const rgw_raw_obj & _raw_obj):raw_obj(_raw_obj),
        is_raw(true) {
    }
...
void convert_to_explicit(const DoutPrefixProvider * dpp,
                         const RGWZoneGroup & zonegroup,
                         const RGWZoneParams & zone_params);
int append_explicit(const DoutPrefixProvider * dpp, RGWObjManifest & m,
                    const RGWZoneGroup & zonegroup,
                    const RGWZoneParams & zone_params);
void append_rules(RGWObjManifest & m, std::map < uint64_t,
...
void set_explicit(uint64_t _size, std::map < uint64_t,
                  RGWObjManifestPart > &_objs) {
    explicit_objs = true;
    objs.swap(_objs);
    set_obj_size(_size);
}
...
void get_implicit_location(uint64_t cur_part_id, uint64_t cur_stripe,
                           uint64_t ofs, std::string * override_prefix,
...
void decode(bufferlist::const_iterator & bl) {
    ...
    if (struct_v >= 3) {
        ...
        explicit_objs = true;
        if (!objs.empty()) {
            std::map < uint64_t, RGWObjManifestPart >::iterator iter =
                objs.begin();
            obj = iter->second.loc;
...
int append(const DoutPrefixProvider * dpp, RGWObjManifest & m,
...
void set_head(const rgw_placement_rule & placement_rule, const rgw_obj & _o,
              uint64_t _s) {
...
const rgw_obj & get_obj() const {
    return obj;
} void set_tail_placement(const rgw_placement_rule & placement_rule,
                          const rgw_bucket & _b) {
...
class obj_iterator {
  ...
  public:
    ...
    explicit obj_iterator(const DoutPrefixProvider * _dpp,
                          const RGWObjManifest * _m)
    :obj_iterator(_dpp, _m, 0) {
    } obj_iterator(const DoutPrefixProvider * _dpp, const RGWObjManifest * _m,
                   uint64_t _ofs):dpp(_dpp), manifest(_m) {
        seek(_ofs);
    }
    ...
    /* start of current stripe */ uint64_t get_stripe_ofs() {
...
obj_iterator obj_begin(const DoutPrefixProvider * dpp) const {
    return obj_iterator {
    dpp, this};
}
obj_iterator obj_end(const DoutPrefixProvider * dpp)const {
    return obj_iterator {
    dpp, this, obj_size};
}
obj_iterator obj_find(const DoutPrefixProvider * dpp, uint64_t ofs)const {
    return obj_iterator {
    dpp, this, std::min(ofs, obj_size)};
}
  ...
  public:
     generator():manifest(NULL), last_ofs(0), cur_part_ofs(0), cur_part_id(0),
        cur_stripe(0), cur_stripe_size(0) {
                       ...
                       const rgw_bucket & bucket, const rgw_obj & obj);
    ...
    rgw_raw_obj get_cur_obj(RGWRados * store) const {
