...
static int read_key(cls_method_context_t hctx, const string &key, T *out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("error reading omap key %s: %s", key.c_str(),
...
template < typename T >
static int write_key(cls_method_context_t hctx, const string &key,
                     const T &t)
{
    bufferlist bl;
    encode(t, bl);
                    ...
                    uint64_t mask)
{
    ...
    if (r < 0) {
        CLS_ERR("failed to read features off disk: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    if (r < 0 && r != -ENOENT) {
        CLS_ERR("Could not read op features off disk: %s",
                cpp_strerror(r).c_str());
        return r;
    }
                  ...
                  bool init)
{
    if (init) {
        ...
        if (r != -ENOENT) {
            ...
            CLS_ERR("failed to read migration off disk: %s",
                    cpp_strerror(r).c_str());
            return r;
        }
        ...
        if (r == -ENOENT) {
            ...
            if (r < 0) {
                CLS_ERR("failed to read v1 header: %s",
                        cpp_strerror(r).c_str());
                return r;
            }
            ...
            if (memcmp(RBD_HEADER_TEXT, header.c_str(), header.length()) !=
                0) {
                if (memcmp
            ...
            if (r < 0) {
                CLS_ERR("error updating v1 header: %s",
                        cpp_strerror(r).c_str());
                return r;
            }
        } else if (r < 0) {
            ...
            r = cls_cxx_map_set_val(hctx, "features", &bl);
            if (r < 0) {
                CLS_ERR("error updating features: %s",
                        cpp_strerror(r).c_str());
                return r;
            }
        }
...
int iterate(cls_method_context_t hctx, L &lambda)
{
    ...
    do {
        ...
        int r =
            cls_cxx_map_get_vals(hctx, last_read, RBD_SNAP_KEY_PREFIX,
                                 max_read, &vals, &more);
        if (r < 0) {
            return r;
        }
        ...
        for (auto &val : vals) {
            ...
            try {
                ...
                CLS_ERR
                ("error decoding snapshot metadata for snap : %s",
                 val.first.c_str());
                return -EIO;
            }
          ...
          cls_rbd_snap && snap)
{
    ...
    if (snap.migrate_parent_format(encode_features)) {
        ...
        if (!on_disk_parent.exists()) {
            ...
            r = write_key(hctx, "parent", on_disk_parent,
                          encode_features);
            if (r < 0) {
                return r;
            }
           ...
           bool reattach)
{
    ...
    if (r < 0) {
        CLS_LOG(20,
                "cls_rbd::image::parent::attach: child doesn't exist");
        return r;
    }
    ...
    CLS_LOG(20,
            ...
            ", ns=%s, id=%s, " "snapid=%" PRIu64 ", size=%" PRIu64,
...
int detach(cls_method_context_t hctx, bool legacy_api)
{
    ...
    auto detach_lambda =
    [hctx, features](const cls_rbd_snap & snap_meta) {
        if (snap_meta.parent.pool_id != -1 || snap_meta.parent_overlap) {
            if
            ((features & RBD_FEATURE_DEEP_FLATTEN) != 0ULL) {
 ...
 out)
{
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("reading object_prefix returned %d",
         r);
        return
            -EEXIST;
    }
    ...
    ["modify_timestamp"]
        =
            timestampbl;
    if
    ((features & RBD_FEATURE_OPERATIONS) != 0ULL) {
        CLS_ERR
        ("Attempting to set internal feature: operations");
        return
            -EINVAL;
    }
    ...
    (features
     ...
     RBD_FEATURE_DATA_POOL) {
        ...
        (data_pool_id
         ...
         -1) {
            CLS_ERR
            ("data pool not provided with feature enabled");
            return
                -EINVAL;
        }
    ...
    (data_pool_id
     ...
     -1) {
        CLS_ERR
        ("data pool provided with feature disabled");
        return
            -EINVAL;
    }
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("failed to read features off disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("Could not read image's features off disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    if
    ((mask & RBD_FEATURES_INTERNAL) != 0ULL) {
        CLS_ERR
        ("Attempting to set internal feature: %"
         PRIu64,
         static_cast
         <
         uint64_t
    ...
    enabled_features
        =
            ...
            mask;
    if
    ((enabled_features & RBD_FEATURES_MUTABLE) != enabled_features) {
        CLS_ERR
        ("Attempting to enable immutable feature: %"
         PRIu64,
         static_cast
         <
         uint64_t
    ...
    if
    ((disabled_features & disable_mask) != disabled_features) {
        CLS_ERR
        ("Attempting to disable immutable feature: %"
         PRIu64,
         enabled_features
         &
         ~disable_mask);
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("failed to read the order off of disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("Could not read image's size off disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    CLS_LOG
    (20,
     "set_size size=%llu orig_size=%llu",
     (unsigned
      long
      long)
     size,
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error writing snapshot metadata: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    try {
        ...
        CLS_LOG
        (20,
         "get_protection_status: invalid decode");
        return
            -EINVAL;
    }
    ...
    CLS_LOG
    (20,
     "get_protection_status snap_id=%llu",
     (unsigned
      long
      long)
     snap_id.
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("could not read key for snapshot id %"
         PRIu64,
         snap_id.
         val);
        return
 ...
 out)
{
    ...
    try {
        ...
        CLS_LOG
        (20,
         "set_protection_status: invalid decode");
        return
            -EINVAL;
    }
    ...
    (r
     ...
     0) {
        CLS_LOG
        (20,
         "image does not support layering");
        return
            r;
    }
    ...
    CLS_LOG
    (20,
     "set_protection_status snapid=%llu status=%u",
     (unsigned
      long
      long)
     snap_id.
 ...
 out)
{
    ...
    try {
        ...
        CLS_LOG
        (20,
         "set_stripe_unit_count: invalid decode");
        return
            -EINVAL;
    }
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error writing stripe_unit metadata: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error writing stripe_count metadata: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error reading create_timestamp: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        try {
            ...
            (const
             ...
             err) {
            CLS_ERR
            ("could not decode create_timestamp");
            return
                -EIO;
        }
    }
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error reading access_timestamp: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        try {
            ...
            (const
             ...
             err) {
            CLS_ERR
            ("could not decode access_timestamp");
            return
                -EIO;
        }
    }
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error reading modify_timestamp: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        try {
            ...
            (const
             ...
             err) {
            CLS_ERR
            ("could not decode modify_timestamp");
            return
                -EIO;
        }
    }
    ...
    (snap_id
     ...
     CEPH_NOSNAP) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("failed to read flags off disk: %s",
             cpp_strerror
             (r).
             c_str
             ());
    ...
    (snap_id
     ...
     CEPH_NOSNAP) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("Could not read image's flags off disk: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        (r
         ...
         0) {
            CLS_ERR
            ("Could not read snapshot: snap_id=%"
             PRIu64
             ": %s",
             snap_id,
             cpp_strerror
 ...
 out)
{
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("failed to read op features off disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    try {
        ...
        CLS_LOG
        (20,
         "cls_rbd::set_parent: invalid decode");
        return
            -EINVAL;
    }
 ...
 out)
{
    ...
    try {
        ...
        CLS_LOG
        (20,
         "cls_rbd::parent_attach: invalid decode");
        return
            -EINVAL;
    }
 ...
 out)
{
    ...
    (children.
     ...
     ()) {
        CLS_LOG
        (20,
         "add_child: child already exists: %s",
         c_image_id.
         c_str
         ());
        return
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_LOG
        (20,
         "remove_child: read omap failed: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    (children.
     ...
     ()) {
        CLS_LOG
        (20,
         "remove_child: child not found: %s",
         c_image_id.
         c_str
         ());
        return
     ...
     ()) {
        ...
        (r
         ...
         0)
            CLS_LOG
            (20,
             "remove_child: remove key failed: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        (r
         ...
         0)
            CLS_LOG
            (20,
             "remove_child: write omap failed: %s",
             cpp_strerror
             (r).
             c_str
             ());
     ...
     0) {
        ...
        (r
         ...
         -ENOENT)
            CLS_LOG
            (20,
             "get_children: read omap failed: %s",
             cpp_strerror
             (r).
             c_str
             ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error reading image data pool id: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    CLS_LOG
    (20,
     "get_snapshot_timestamp snap_id=%llu",
     (unsigned
      long
      long)
     snap_id);
 ...
 out)
{
    ...
    (std::
     ...
     (snap_meta.
      snapshot_namespace)) {
        CLS_ERR
        ("Unknown snapshot namespace provided");
        return
            -EINVAL;
    }
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("Could not read image's snap_seq off disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("Could not read snapshot limit off disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    old_meta) {
        ...
        (total_read
         ...
         snap_limit) {
            CLS_ERR
            ("Attempt to create snapshot over limit of %"
             PRIu64,
             snap_limit);
            return
                -EDQUOT;
        ...
        if
        ((snap_meta.name == old_meta.name && snap_meta.snapshot_namespace == old_meta.snapshot_namespace) ||
         snap_meta.id == old_meta.id) {
            CLS_LOG
            (20,
             "snap_name %s or snap_id %"
             PRIu64
 ...
 out)
{
    ...
    snap_meta) {
        ...
        (cls::
         ...
         dst_snap_name) {
            CLS_LOG
            (20,
             "snap_name %s matches existing snap with snap id %"
             PRIu64,
             dst_snap_name.
             c_str
             (),
    ...
    (r
     ...
     -ENOENT) {
        CLS_LOG
        (20,
         "cannot find existing snap with snap id = %"
         PRIu64,
         src_snap_id.
         val);
        return
 ...
 out)
{
    ...
    (extent_map.
     ...
     ()) {
        CLS_LOG
        (20,
         "sparse_copyup: create empty object");
        r
            =
                cls_cxx_create
                (hctx,
    ...
    for (auto &it :                                                                                  extent_map) {
        ...
        (r
         ...
         0) {
            CLS_ERR
            ("sparse_copyup: error writing extent %"
             PRIu64
             "~%"
             PRIu64
             ": %s",
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error setting access_timestamp");
        return
            r;
    }
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error setting modify_timestamp");
        return
            r;
    }
 ...
 check_for_unique_id)
{
    ...
    (!name.
     ...
     (id)) {
        CLS_ERR
        ("dir_add_image_helper: invalid name '%s' or id '%s'",
         name.
         c_str
         (),
         id.
    ...
    CLS_LOG
    (20,
     "dir_add_image_helper name=%s id=%s",
     name.
     c_str
     (),
     id.
 ...
 id)
{
    CLS_LOG
    (20,
     "dir_remove_image_helper name=%s id=%s",
     name.
     c_str
     (),
     id.
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT)
            CLS_ERR
            ("error reading name to id mapping: %s",
             cpp_strerror
             (r).
             c_str
             ());
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error reading id to name mapping: %s",
         cpp_strerror
         (r).
         c_str
         ());
        ...
        (r
         ...
         0) {
            ...
            (r
             ...
             -ENOENT) {
                CLS_ERR
                ("error reading directory by name: %s",
                 cpp_strerror
                 (r).
                 c_str
                 ());
         ...
         ++it) {
            ...
            try {
                ...
                (const
                 ...
                 err) {
                CLS_ERR
                ("could not decode id of image '%s'",
                 it->
                 first.
                 c_str
                 ());
 ...
 out)
{
    ...
    (object_count
     ...
     cls::
     rbd::
     MAX_OBJECT_MAP_OBJECT_COUNT) {
        CLS_ERR
        ("object map too large: %"
         PRIu64,
         object_count);
        return
            -EINVAL;
        ...
        (;
         ...
         ++i) {
            ...
            (*it
             ...
             default_state) {
                CLS_ERR
                ("object map indicates object still exists: %"
                 PRIu64,
                 i);
                return
                    -ESTALE;
 ...
 out)
{
    ...
    try {
        ...
        (const
         ...
         err) {
        CLS_ERR
        ("failed to decode object map header: %s",
         err.
         what
         ());
        return
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("object map footer read header CRC failed");
        return
            r;
    }
    ...
    try {
        ...
        (const
         ...
         err) {
        CLS_ERR
        ("failed to decode object map header CRC: %s",
         err.
         what
         ());
    }
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("object map footer read data CRCs failed");
        return
            r;
    }
    ...
    try {
        ...
        (const
         ...
         err) {
        CLS_ERR
        ("failed to decode object map data CRCs: %s",
         err.
         what
         ());
    }
     ...
     ++it) {
        ...
        state
            =
                *it;
        if
        ((!current_object_state || state == *current_object_state || (*current_object_state == OBJECT_EXISTS &&
    ...
    (updated) {
        ...
        (r
         ...
         0) {
            CLS_ERR
            ("failed to write object map header: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        (r
         ...
         0) {
            CLS_ERR
            ("failed to write object map footer: %s",
             cpp_strerror
             (r).
             c_str
             ());
        ...
        CLS_LOG
        (20,
         "object_map_update: no update necessary");
    }
        ...
        (r
         ...
         0) {
            ...
            (r
             ...
             -ENOENT) {
                CLS_ERR
                ("failed to read the vals off of disk: %s",
                 cpp_strerror
                 (r).
                 c_str
                 ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error retrieving snapshot limit: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    (new_limit
     ...
     snap_count) {
        ...
        CLS_LOG
        (10,
         "snapshot limit is less than the number of snapshots.\n");
    } else {
        CLS_LOG
        (20,
         "set snapshot limit to %"
         PRIu64
         "\n",
         new_limit);
        bl.
 ...
 out)
{
    ...
    children_key
        =
            image::
            snap_children_key_from_snap_id
            (snap_id);
    cls::
    rbd::
    ChildImageSpecs
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("error reading snapshot children: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error writing snapshot children: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    (snap.
     ...
     ()) {
        ...
        CLS_ERR
        ("children reference count mismatch: %"
         PRIu64,
         snap_id);
        return
            -EINVAL;
 ...
 out)
{
    ...
    *cur_snap_name;
    ...
    CLS_LOG
    (20,
     "rename snap with snap id %llu to dest name %s",
     (unsigned
      long
      long)
     src_snap_id.
 ...
 mirror_uuid)
{
    ...
    (site_it
     ...
     ()) {
        ...
        CLS_LOG
        (10,
         "auto-generating new TX-only peer: %s",
         site_name.
         c_str
         ());
        uuid_d
        ...
        (true) {
            ...
            (r
             ...
             0) {
                CLS_ERR
                ("failed to retrieve mirror peer: %s",
                 cpp_strerror
                 (r).
                 c_str
                 ());
    ...
    (it
     ...
     ()) {
        ...
        (mirror_peer.
         ...
         cls::
         rbd::
         MIRROR_PEER_DIRECTION_RX) {
            CLS_LOG
            (10,
             "switching to RX/TX peer: %s",
             site_name.
             c_str
             ());
            mirror_peer.
    ...
    (!mirror_peer.
     ...
     ()) {
        CLS_ERR
        ("attempting to update invalid peer: %s",
         site_name.
         c_str
         ());
        return
 ...
 mirror_peer)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error retrieving mirroring uuid: %s",
         cpp_strerror
         (r).
         c_str
         ());
    ...
    (mirror_peer.
     ...
     mirror_uuid) {
        CLS_ERR
        ("peer uuid '%s' matches pool mirroring uuid",
         mirror_uuid.
         c_str
         ());
        return
    ...
    (mirror_peer.
     ...
     MIRROR_PEER_DIRECTION_TX) {
        CLS_ERR
        ("peer uuid '%s' cannot use TX-only direction",
         mirror_peer.
         uuid.
         c_str
         ());
    ...
    for (auto const &peer :
        ...
        (peer.
         ...
         site_name) {
            CLS_ERR
            ("peer site name '%s' already exists",
             peer.
             site_name.
             c_str
             ());
        ...
        (!mirror_peer.
         ...
         mirror_uuid) {
            CLS_ERR
            ("peer mirror uuid '%s' already exists",
             peer.
             mirror_uuid.
             c_str
             ());
 ...
 mirror_image)
{
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error reading mirrored image '%s': '%s'",
             image_id.
             c_str
             (),
             cpp_strerror
    ...
    try {
        ...
        (const
         ...
         err) {
        CLS_ERR
        ("could not decode mirrored image '%s'",
         image_id.
         c_str
         ());
        return
 ...
 mirror_image)
{
    ...
    (r
     ...
     -ENOENT) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error reading global image id: '%s': '%s'",
             image_id.
             c_str
             (),
             cpp_strerror
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error adding mirrored image '%s': %s",
         image_id.
         c_str
         (),
         cpp_strerror
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error adding global id for image '%s': %s",
         image_id.
         c_str
         (),
         cpp_strerror
 ...
 image_id)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error removing mirrored image '%s': %s",
         image_id.
         c_str
         (),
         cpp_strerror
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("error removing global id for image '%s': %s",
         image_id.
         c_str
         (),
         cpp_strerror
 ...
 global_image_id)
{
    ...
    for (auto &mirror_uuid :              mirror_uuids) {
        CLS_LOG
        (20,
         "removing status object for mirror_uuid %s",
         mirror_uuid.
         c_str
         ());
        auto
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error removing stale status for key '%s': %s",
             key.
             c_str
             (),
             cpp_strerror
        ...
        (r
         ...
         0) {
            ...
            (r
             ...
             -ENOENT) {
                CLS_ERR
                ("error reading mirrored images: %s",
                 cpp_strerror
                 (r).
                 c_str
                 ());
        ...
        for (auto &list_it :                                                                                   vals) {
            ...
            try {
                ...
                (const
                 ...
                 err) {
                CLS_ERR
                ("could not decode mirror image payload for key '%s'",
                 key.
                 c_str
                 ());
                return
            ...
            for (auto &list_it :                                                                               vals) {
                ...
                (watchers.
                 ...
                 ()) {
                    CLS_LOG
                    (20,
                     "removing stale status object for key %s",
                     key.
                     c_str
                     ());
                    int
        ...
        (r
         ...
         0) {
            ...
            (r
             ...
             -ENOENT) {
                CLS_ERR
                ("error reading mirror instances: %s",
                 cpp_strerror
                 (r).
                 c_str
                 ());
 ...
 instance_id)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error setting mirror instance %s: %s",
         instance_id.
         c_str
         (),
         cpp_strerror
 ...
 instance_id)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error removing mirror instance %s: %s",
         instance_id.
         c_str
         (),
         cpp_strerror
         ...
         ++it) {
            ...
            try {
                ...
                (const
                 ...
                 err) {
                CLS_ERR
                ("could not decode image map payload: %s",
                 cpp_strerror
                 (r).
                 c_str
                 ());
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("Could not read snapshot meta off disk: %s",
             cpp_strerror
             (r).
             c_str
             ());
    ...
    (mirror_ns
     ...
     nullptr) {
        ...
        (5,
         ...
         "not mirroring snapshot snap_id=%"
         PRIu64,
         snap_id);
        return
            -EINVAL;
 ...
 out)
{
    ...
    (r
     ...
     MIRROR_MODE_DISABLED) {
        CLS_ERR
        ("cannot set mirror uuid while mirroring enabled");
        return
            -EINVAL;
    }
 ...
 out)
{
    ...
    (enabled) {
        ...
        (!peers.
         ...
         ()) {
            CLS_ERR
            ("mirroring peers still registered");
            return
                -EBUSY;
        }
    ...
    (r
     ...
     0) {
        ...
        (r
         ...
         -ENOENT) {
            CLS_ERR
            ("error retrieving image id for global id '%s': %s",
             global_id.
             c_str
             (),
             cpp_strerror
 ...
 out)
{
    ...
    CLS_LOG
    (20,
     "mirror_image_snapshot_unlink_peer snap_id=%"
     PRIu64
     " peer_uuid=%s",
     snap_id,
     mirror_peer_uuid.
 ...
 out)
{
    ...
    CLS_LOG
    (20,
     "mirror_image_snapshot_set_copy_progress snap_id=%"
     PRIu64
     " complete=%d last_copied_object_number=%"
     PRIu64,
     snap_id,
     complete,
     last_copied_object_number);
        ...
        for (auto val :                                                                                 vals) {
            ...
            try {
                ...
                (const
                 ...
                 err) {
                CLS_ERR
                ("could not decode id of group '%s'",
                 val.
                 first.
                 c_str
                 ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("could not create group directory: %s",
         cpp_strerror
         (r).
         c_str
         ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("error removing image from group: %s",
         cpp_strerror
         (r).
         c_str
         ());
         ...
         ++it) {
            ...
            try {
                ...
                (const
                 ...
                 err) {
                CLS_ERR
                ("error decoding state for image: %s",
                 it->
                 first.
                 c_str
                 ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        ...
        if
        ((old_group.group_id != new_group.group_id) || (old_group.pool_id != new_group.pool_id)) {
            return
                -EEXIST;
        } else {
 ...
 out)
{
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("could not read key %s entry from trash: %s",
         key.
         c_str
         (),
         cpp_strerror
 ...
 out)
{
    ...
    (r
     ...
     -ENOENT) {
        CLS_ERR
        ("error reading image from trash '%s': '%s'",
         id.
         c_str
         (),
         cpp_strerror
    ...
    (trash_spec.
     ...
     expect_state) {
        ...
        (r
         ...
         0) {
            CLS_ERR
            ("error setting trash image state: %s",
             cpp_strerror
             (r).
             c_str
             ());
 ...
 out)
{
    ...
    (r
     ...
     0) {
        CLS_ERR
        ("failed to read data off of disk: %s",
         cpp_strerror
         (r).
         c_str
         ());
