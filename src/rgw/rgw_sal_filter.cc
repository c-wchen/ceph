...
/* These are helpers for getting 'next' out of an object, handling nullptr */
        ...
        int FilterZoneGroup::get_zone_by_id(const std::string & id,
        ...
        int FilterZoneGroup::get_zone_by_name(const std::string & name,
                                              std::unique_ptr < Zone > *zone) {
            std::unique_ptr < Zone > nz;
        ...
        int FilterDriver::initialize(CephContext * cct,
        ...
        const std::string FilterDriver::get_name() const {
            ...
            return name;
        } std::string FilterDriver::get_cluster_id(const DoutPrefixProvider *
        ...
        int FilterDriver::get_user_by_access_key(const DoutPrefixProvider * dpp,
        ...
        int FilterDriver::get_user_by_email(const DoutPrefixProvider * dpp,
        ...
        int FilterDriver::get_user_by_swift(const DoutPrefixProvider * dpp,
        ...
        int FilterDriver::get_bucket(User * u, const RGWBucketInfo & i,
                                     std::unique_ptr < Bucket > *bucket) {
            std::unique_ptr < Bucket > nb;
            int ret;
            User *nu = nextUser(u);
        ...
        int FilterDriver::forward_request_to_master(const DoutPrefixProvider *
                                                    dpp, User * user,
                                                    obj_version * objv,
                                                    bufferlist & in_data,
                                                    JSONParser * jp,
                                                    req_info & info,
                                                    optional_yield y) {
            return next->forward_request_to_master(dpp, user, objv, in_data, jp,
                                                   info, y);
        }

        int FilterDriver::
            forward_iam_request_to_master(const DoutPrefixProvider * dpp,
                                          const RGWAccessKey & key,
                                          obj_version * objv,
                                          bufferlist & in_data,
                                          RGWXMLDecoder::XMLParser * parser,
                                          req_info & info, optional_yield y) {
            return next->forward_iam_request_to_master(dpp, key, objv, in_data,
                                                       parser, info, y);
        }
        ...
        std::unique_ptr < Notification >
            FilterDriver::get_notification(rgw::sal::Object * obj,
            ...
            std::unique_ptr < Notification > n =
                next->get_notification(nextObject(obj), nextObject(src_obj), s,
                                       event_type, y, object_name);
            return std::make_unique < FilterNotification > (std::move(n));
        }

        std::unique_ptr < Notification >
            FilterDriver::get_notification(const DoutPrefixProvider * dpp,
                                           rgw::sal::Object * obj,
                                           rgw::sal::Object * src_obj,
                                           rgw::notify::EventType event_type,
                                           ...
                                           optional_yield y) {
            std::unique_ptr < Notification > n =
                next->get_notification(dpp, nextObject(obj),
                                       nextObject(src_obj), event_type,
                                       nextBucket(_bucket), _user_id,
        ...
        int FilterDriver::log_usage(const DoutPrefixProvider * dpp,
                                    std::map < rgw_user_bucket,
                                    RGWUsageBatch > &usage_info) {
            return next->log_usage(dpp, usage_info);
        }

        int FilterDriver::log_op(const DoutPrefixProvider * dpp,
                                 std::string & oid, bufferlist & bl) {
            return next->log_op(dpp, oid, bl);
        }

        int FilterDriver::register_to_service_map(const DoutPrefixProvider *
                                                  dpp,
                                                  const std::
                                                  string & daemon_type,
        ...
        void FilterDriver::get_ratelimit(RGWRateLimitInfo & bucket_ratelimit,
            ...
            return next->get_ratelimit(bucket_ratelimit, user_ratelimit,
                                       anon_ratelimit);
        }

        int FilterDriver::set_buckets_enabled(const DoutPrefixProvider * dpp,
                                              std::vector < rgw_bucket >
                                              &buckets, bool enabled) {
        ...
        int FilterDriver::get_sync_policy_handler(const DoutPrefixProvider *
                                                  dpp,
                                                  ...
                                                  * phandler,
                                                  optional_yield y) {
            return next->get_sync_policy_handler(dpp, zone, bucket, phandler,
                                                 y);
        }

        RGWDataSyncStatusManager *FilterDriver::
        ...
        void FilterDriver::wakeup_data_sync_shards(const DoutPrefixProvider *
                                                   dpp,
                                                   const rgw_zone_id &
                                                   source_zone,
        ...
        int FilterDriver::read_all_usage(const DoutPrefixProvider * dpp,
            ...
            return next->read_all_usage(dpp, start_epoch, end_epoch,
                                        max_entries, is_truncated, usage_iter,
                                        usage);
        }

        int FilterDriver::trim_all_usage(const DoutPrefixProvider * dpp,
                                         uint64_t start_epoch,
                                         uint64_t end_epoch) {
        ...
        int FilterDriver::meta_list_keys_next(const DoutPrefixProvider * dpp,
                                              void *handle, int max,
        ...
        int FilterDriver::meta_remove(const DoutPrefixProvider * dpp,
        ...
        std::unique_ptr < RGWRole > FilterDriver::get_role(std::string name,
                                                           ...
                                                           std::string,
                                                           std::string > tags) {
            return next->get_role(name, tenant, path, trust_policy,
        ...
        std::unique_ptr < Writer >
            FilterDriver::get_append_writer(const DoutPrefixProvider * dpp,
                                            optional_yield y,
                                            rgw::sal::Object * obj,
            ...
            std::unique_ptr < Writer > writer =
                next->get_append_writer(dpp, y, nextObject(obj), owner,
                                        ptail_placement_rule, unique_tag,
        ...
        std::unique_ptr < Writer >
            FilterDriver::get_atomic_writer(const DoutPrefixProvider * dpp,
                                            optional_yield y,
                                            rgw::sal::Object * obj,
                                            const rgw_user & owner,
                                            const rgw_placement_rule *
                                            ...
                                            const std::string & unique_tag) {
            std::unique_ptr < Writer > writer =
                next->get_atomic_writer(dpp, y, nextObject(obj), owner,
        ...
        int FilterUser::list_buckets(const DoutPrefixProvider * dpp,
                                     const std::string & marker,
            ...
            buckets.clear();
            ret =
                next->list_buckets(dpp, marker, end_marker, max, need_stats, bl,
                                   y);
            if (ret < 0)
                return ret;
          ...
          for (auto & ent:bl.get_buckets()) {
                buckets.add(std::make_unique < FilterBucket >
                            (std::move(ent.second), this));
            }
        ...
        int FilterUser::create_bucket(const DoutPrefixProvider * dpp,
            ...
            ret =
                next->create_bucket(dpp, b, zonegroup_id, placement_rule,
                                    swift_ver_location, pquota_info, policy,
                                    attrs, info, ep_objv, exclusive,
                                    obj_lock_enabled, existed, req_info, &nb,
                                    y);
            if (ret < 0)
                return ret;
        ...
        int FilterUser::read_attrs(const DoutPrefixProvider * dpp,
                                   optional_yield y) {
        ...
        int FilterUser::read_stats(const DoutPrefixProvider * dpp,
            ...
            return next->read_stats(dpp, y, stats, last_stats_sync,
                                    last_stats_update);
        }

        int FilterUser::read_stats_async(const DoutPrefixProvider * dpp,
                                         RGWGetUserStats_CB * cb) {
            return next->read_stats_async(dpp, cb);
        }

        int FilterUser::complete_flush_stats(const DoutPrefixProvider * dpp,
                                             optional_yield y) {
            return next->complete_flush_stats(dpp, y);
        }

        int FilterUser::read_usage(const DoutPrefixProvider * dpp,
        ...
        int FilterUser::trim_usage(const DoutPrefixProvider * dpp,
                                   uint64_t start_epoch, uint64_t end_epoch) {
            return next->trim_usage(dpp, start_epoch, end_epoch);
        }

        int FilterUser::load_user(const DoutPrefixProvider * dpp,
                                  optional_yield y) {
            return next->load_user(dpp, y);
        }

        int FilterUser::store_user(const DoutPrefixProvider * dpp,
                                   optional_yield y, bool exclusive,
                                   RGWUserInfo * old_info) {
            return next->store_user(dpp, y, exclusive, old_info);
        }

        int FilterUser::remove_user(const DoutPrefixProvider * dpp,
                                    optional_yield y) {
        ...
        int FilterBucket::list(const DoutPrefixProvider * dpp,
        ...
        int FilterBucket::remove_bucket(const DoutPrefixProvider * dpp,
            ...
            return next->remove_bucket(dpp, delete_children, forward_to_master,
                                       req_info, y);
        }
        ...
        int FilterBucket::remove_bucket_bypass_gc(int concurrent_max,
                                                  ...
                                                  dpp) {
            return next->remove_bucket_bypass_gc(concurrent_max,
        ...
        int FilterBucket::load_bucket(const DoutPrefixProvider * dpp,
        ...
        int FilterBucket::read_stats(const DoutPrefixProvider * dpp,
                                     ...
                                     bool * syncstopped) {
            return next->read_stats(dpp, idx_layout, shard_id, bucket_ver,
                                    master_ver, stats, max_marker, syncstopped);
        }
        ...
        int FilterBucket::sync_user_stats(const DoutPrefixProvider * dpp,
                                          optional_yield y) {
        ...
        int FilterBucket::put_info(const DoutPrefixProvider * dpp,
                                   bool exclusive, ceph::real_time _mtime) {
        ...
        int FilterBucket::check_empty(const DoutPrefixProvider * dpp,
                                      optional_yield y) {
            return next->check_empty(dpp, y);
        }

        int FilterBucket::check_quota(const DoutPrefixProvider * dpp,
        ...
        int FilterBucket::read_usage(const DoutPrefixProvider * dpp,
                                     uint64_t start_epoch, uint64_t end_epoch,
                                     uint32_t max_entries, bool * is_truncated,
                                     RGWUsageIter & usage_iter,
                                     std::map < rgw_user_bucket,
        ...
        int FilterBucket::trim_usage(const DoutPrefixProvider * dpp,
                                     uint64_t start_epoch, uint64_t end_epoch) {
            return next->trim_usage(dpp, start_epoch, end_epoch);
        }
        ...
        std::unique_ptr < MultipartUpload >
            FilterBucket::get_multipart_upload(const std::string & oid,
            ...
            return std::make_unique < FilterMultipartUpload > (std::move(nmu),
                                                               this);
        }
        ...
        int FilterBucket::list_multiparts(const DoutPrefixProvider * dpp,
            ...
            ret =
                next->list_multiparts(dpp, prefix, marker, delim, max_uploads,
                                      nup, common_prefixes, is_truncated);
            if (ret < 0)
                return ret;

          for (auto & ent:nup) {
                uploads.emplace_back(std::make_unique < FilterMultipartUpload >
                                     (std::move(ent), this));
            }
        ...
        int FilterBucket::abort_multiparts(const DoutPrefixProvider * dpp,
                                           CephContext * cct) {
        ...
        int FilterObject::delete_obj_aio(const DoutPrefixProvider * dpp,
                                         RGWObjState * astate,
                                         Completions * aio,
                                         bool keep_index_consistent,
                                         optional_yield y) {
            return next->delete_obj_aio(dpp, astate, aio, keep_index_consistent,
                                        y);
        }
        ...
        int FilterObject::copy_object(User * user,
            ...
            return next->copy_object(user, info, source_zone,
                                     ...
                                     nextBucket(src_bucket), dest_placement,
        ...
        int FilterObject::get_obj_state(const DoutPrefixProvider * dpp,
                                        RGWObjState ** pstate, optional_yield y,
                                        bool follow_olh) {
            return next->get_obj_state(dpp, pstate, y, follow_olh);
        }

        int FilterObject::set_obj_attrs(const DoutPrefixProvider * dpp,
                                        Attrs * setattrs, Attrs * delattrs,
                                        optional_yield y) {
            return next->set_obj_attrs(dpp, setattrs, delattrs, y);
        }

        int FilterObject::get_obj_attrs(optional_yield y,
                                        const DoutPrefixProvider * dpp,
                                        rgw_obj * target_obj) {
            return next->get_obj_attrs(y, dpp, target_obj);
        }

        int FilterObject::modify_obj_attrs(const char *attr_name,
                                           bufferlist & attr_val,
                                           optional_yield y,
                                           const DoutPrefixProvider * dpp) {
        ...
        std::unique_ptr < MPSerializer >
            FilterObject::get_serializer(const DoutPrefixProvider * dpp,
                                         const std::string & lock_name) {
            std::unique_ptr < MPSerializer > s =
                next->get_serializer(dpp, lock_name);
        ...
        int FilterObject::transition(Bucket * bucket,
                                     ...
                                     const DoutPrefixProvider * dpp,
                                     optional_yield y) {
            return next->transition(nextBucket(bucket), placement_rule, mtime,
                                    olh_epoch, dpp, y);
        }
        ...
        int FilterObject::transition_to_cloud(Bucket * bucket,
                                              ...
                                              const DoutPrefixProvider * dpp,
                                              optional_yield y) {
            return next->transition_to_cloud(nextBucket(bucket),
                                             nextPlacementTier(tier), o,
                                             cloud_targets, cct, update_object,
                                             dpp, y);
        }

        bool FilterObject::placement_rules_match(rgw_placement_rule & r1,
                                                 rgw_placement_rule & r2) {
            return next->placement_rules_match(r1, r2);
        }

        int FilterObject::dump_obj_layout(const DoutPrefixProvider * dpp,
        ...
        int FilterObject::omap_get_vals(const DoutPrefixProvider * dpp,
                                        const std::string & marker,
        ...
        int FilterObject::chown(User & new_user, const DoutPrefixProvider * dpp,
                                optional_yield y) {
            return next->chown(new_user, dpp, y);
        }

        int FilterObject::FilterReadOp::prepare(optional_yield y,
                                                const DoutPrefixProvider *
                                                dpp) {
        ...
        int FilterObject::FilterReadOp::read(int64_t ofs, int64_t end,
        ...
        int FilterObject::FilterReadOp::get_attr(const DoutPrefixProvider * dpp,
                                                 const char *name,
                                                 bufferlist & dest,
                                                 optional_yield y) {
        ...
        int FilterObject::FilterReadOp::iterate(const DoutPrefixProvider * dpp,
        ...
        int FilterObject::FilterDeleteOp::delete_obj(const DoutPrefixProvider *
                                                     dpp, optional_yield y) {
            /* Copy params into next */
            next->params = params;
        ...
        int FilterMultipartUpload::init(const DoutPrefixProvider * dpp,
        ...
        int FilterMultipartUpload::list_parts(const DoutPrefixProvider * dpp,
            ...
            ret =
                next->list_parts(dpp, cct, num_parts, marker, next_marker,
          ...
          for (auto & ent:next->get_parts()) {
                parts.emplace(ent.first,
                              std::make_unique < FilterMultipartPart >
                              (std::move(ent.second)));
            }
        ...
        int FilterMultipartUpload::abort(const DoutPrefixProvider * dpp,
                                         CephContext * cct) {
        ...
        int FilterMultipartUpload::complete(const DoutPrefixProvider * dpp,
            ...
            return next->complete(dpp, y, cct, part_etags, remove_objs,
                                  accounted_size, compressed, cs_info, ofs, tag,
                                  owner, olh_epoch, nextObject(target_obj));
        }
        ...
        std::unique_ptr < Writer >
            FilterMultipartUpload::get_writer(const DoutPrefixProvider * dpp,
                                              optional_yield y,
                                              rgw::sal::Object * obj,
                                              const rgw_user & owner,
                                              const rgw_placement_rule *
        ...
        int FilterMPSerializer::try_lock(const DoutPrefixProvider * dpp,
                                         utime_t dur, optional_yield y) {
            return next->try_lock(dpp, dur, y);
        }

        int FilterLCSerializer::try_lock(const DoutPrefixProvider * dpp,
                                         utime_t dur, optional_yield y) {
            return next->try_lock(dpp, dur, y);
        }
        ...
        int FilterLifecycle::get_entry(const std::string & oid,
                                       const std::string & marker,
        ...
        int FilterLifecycle::get_next_entry(const std::string & oid,
                                            const std::string & marker,
        ...
        int FilterLifecycle::list_entries(const std::string & oid,
                                          const std::string & marker,
                                          uint32_t max_entries,
                                          std::vector < std::unique_ptr <
                                          LCEntry >> &entries) {
          ...
          for (auto & ent:ne) {
                entries.emplace_back(std::make_unique < FilterLCEntry >
                                     (std::move(ent)));
            }
        ...
        int FilterLifecycle::get_head(const std::string & oid,
        ...
        int FilterLifecycle::put_head(const std::string & oid, LCHead & head) {
            return next->put_head(oid,
                                  *(dynamic_cast <
                                    FilterLCHead & >(head).next.get()));
        }

        std::unique_ptr < LCSerializer >
            FilterLifecycle::get_serializer(const std::string & lock_name,
                                            const std::string & oid,
        ...
        int FilterNotification::publish_commit(const DoutPrefixProvider * dpp,
                                               uint64_t size,
        ...
        int FilterWriter::complete(size_t accounted_size,
        ...
        int FilterLuaManager::put_script(const DoutPrefixProvider * dpp,
                                         optional_yield y,
                                         const std::string & key,
        ...
        int FilterLuaManager::del_script(const DoutPrefixProvider * dpp,
                                         optional_yield y,
                                         const std::string & key) {
            return next->del_script(dpp, y, key);
        }

        int FilterLuaManager::add_package(const DoutPrefixProvider * dpp,
                                          optional_yield y,
                                          const std::string & package_name) {
            return next->add_package(dpp, y, package_name);
        }

        int FilterLuaManager::remove_package(const DoutPrefixProvider * dpp,
                                             optional_yield y,
                                             const std::string & package_name) {
            return next->remove_package(dpp, y, package_name);
        }

        int FilterLuaManager::list_packages(const DoutPrefixProvider * dpp,
                                            optional_yield y,
