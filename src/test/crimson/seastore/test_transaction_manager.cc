...
struct test_extent_record_t {
    ...
    test_extent_record_t(const test_extent_desc_t & desc,
...
template <>
    struct fmt::formatter < test_extent_record_t >:fmt::formatter <
    std::string_view > {
    template < typename FormatContext >
        auto format(const test_extent_record_t & r, FormatContext & ctx) const {
        return fmt::format_to(ctx.out(),
                              "test_extent_record_t({}, refcount={})", r.desc,
                              r.refcount);
}};

struct transaction_manager_test_t:public seastar_test_suite_t,
    TMTestState,::testing::WithParamInterface < const char *> {

    std::random_device rd;
    std::mt19937 gen;

    transaction_manager_test_t(std::size_t num_main_devices,
    ...
    :TMTestState(num_main_devices, num_cold_devices), gen(rd()) {
        ...
        return block_size *
            std::uniform_int_distribution <> (0,
                                              (limit / block_size) - 1) (gen);
    }

    char get_random_contents() {
        return static_cast <
            char >(std::uniform_int_distribution <> (0, 255) (gen));
    }
    ...
    struct test_extents_t:std::map < laddr_t, test_extent_record_t > {
        using delta_t =
            std::map < laddr_t, std::optional < test_extent_record_t >>;
        ...
        struct delta_overlay_t {
            ...
            delta_overlay_t(const test_extents_t & extents,
                            const delta_t & delta)
            :extents(extents), delta(delta) {
                ...
                std::optional < std::pair < laddr_t,
                    test_extent_record_t >> cur;

                iterator(const delta_overlay_t & parent,
                         test_extents_t::const_iterator biter,
                         delta_t::const_iterator oiter)
                :parent(parent), biter(biter), oiter(oiter) {
                } laddr_t get_bkey() {
                    return biter ==
                        parent.extents.end()? L_ADDR_MAX : biter->first;
                } laddr_t get_okey() {
                    return oiter ==
                        parent.delta.end()? L_ADDR_MAX : oiter->first;
                }

                bool is_end() {
                    return oiter == parent.delta.end()
                        && biter == parent.extents.end();
                }
                ...
                auto get_pair() {
                    ...
                    return (bkey < okey ?
                            ...
                            (*biter) : std::make_pair(okey, *(oiter->second)));
                }
            ...
            iterator lower_bound(laddr_t l) {
                auto ret =
                    iterator { *this, extents.lower_bound(l),
                 delta.lower_bound(l) };
                ret.adjust();
                return ret;
            }
            ...
            iterator upper_bound(laddr_t l) {
                auto ret =
                    iterator { *this, extents.upper_bound(l),
                 delta.upper_bound(l) };
                ret.adjust();
                return ret;
            }
      ...
      private:
        ...
        void check_hint(laddr_t hint,
                        laddr_t addr, extent_len_t len, delta_t & delta) const {
            delta_overlay_t overlay(*this, delta);
        ...
        std::optional < test_extent_record_t > &populate_delta(laddr_t addr,
                                                               delta_t & delta,
                                                               const
                                                               test_extent_desc_t
                                                               * desc) const {
            ...
            if (iter == end()) {
                ...
                auto ret =
                    delta.
                    emplace(std::
                            make_pair(addr, test_extent_record_t { *desc, 0 }
                            ));
                assert(ret.second);
      ...
      public:
        ...
        void alloced(laddr_t hint, TestBlock & extent, delta_t & delta) const {
            ...
            return iter->second;
        } void update(laddr_t addr,
                      const test_extent_desc_t & desc, delta_t & delta) const {
    ...
    TestBlockRef alloc_extent(test_transaction_t & t,
                              laddr_t hint, extent_len_t len, char contents) {
        auto extent = with_trans_intr(*(t.t),[&](auto & trans) {
                                      return tm->alloc_extent < TestBlock >
                                      (trans, hint, len);}
        ).unsafe_get0();
        extent->set_contents(contents);
        EXPECT_FALSE(test_mappings.
                     contains(extent->get_laddr(), t.mapping_delta));
    ...
    TestBlockRef get_extent(test_transaction_t & t,
        ...
        auto ext = with_trans_intr(*(t.t),[&](auto & trans) {
                                   return tm->read_extent < TestBlock > (trans,
                                                                         addr,
                                                                         len);}
        ).unsafe_get0();
    ...
    TestBlockRef try_get_extent(test_transaction_t & t, laddr_t addr) {
        ...
        ).handle_error([](const crimson::ct_error::eagain & e) {
    ...
    TestBlockRef try_get_extent(test_transaction_t & t,
                                laddr_t addr, extent_len_t len) {
        ceph_assert(test_mappings.contains(addr, t.mapping_delta));
        ceph_assert(test_mappings.get(addr, t.mapping_delta).desc.len == len);
    ...
    TestBlockRef mutate_extent(test_transaction_t & t, TestBlockRef ref) {
        ...
        ceph_assert(test_mappings.get(ref->get_laddr(), t.mapping_delta).desc.
                    len == ref->get_length());
        ...
        test_mappings.update(ext->get_laddr(), ext->get_desc(),
                             t.mapping_delta);
        return ext;
    }

    TestBlockRef mutate_addr(test_transaction_t & t,
                             laddr_t offset, size_t length) {
    ...
    void check_mappings(test_transaction_t & t) {
      ...
      for (const auto & i:overlay) {
        ...
        with_trans_intr(*t.t,[this, &overlay] (auto & t) {
                        return lba_manager->scan_mappings(t,
                                                          0,
                                                          L_ADDR_MAX,
                                                          [iter =
                                                           overlay.begin(),
    ...
    bool try_submit_transaction(test_transaction_t t) {
        using ertr =
            with_trans_ertr < TransactionManager::submit_transaction_iertr >;
        ...
        bool success =
            ...
            safe_then([&write_seq] (auto seq)->ret {
               ...
               return epm->run_background_work_until_halt().then([ret] {
                                                                 return ret;
                                                                 }
               );}
        ).get0();
    ...
    auto allocate_sequentially(const size_t size, const int num,
                               bool run_clean = true) {
        return repeat_eagain([this, size, num] {
                             return seastar::do_with(create_transaction(),
                                                     [this, size,
                                                      num] (auto & t) {
                                                     return with_trans_intr(*t.
                                                                            t,
                                                                            [&t,
                                                                             this,
                                                                             size,
    ...
    void test_parallel_extent_read() {
        ...
        run_async([this] {
                  ...
                  auto extent = alloc_extent(t,
                                             i * BSIZE,
                                             BSIZE);
                  ASSERT_EQ(i * BSIZE, extent->get_laddr());
                  submit_transaction(std::move(t));}

                  seastar::do_with(create_read_test_transaction(),
                                   [this] (auto & t) {
                                   return with_trans_intr(*(t.t),
                                                          [this] (auto & t) {
                                                          return trans_intr::
                                                          parallel_for_each
                                                          (boost::
    ...
    void test_random_writes_concurrent() {
        ...
        constexpr size_t BLOCKS = TOTAL / BSIZE;
        run_async([this] {
                  std::for_each(boost::make_counting_iterator(0u),
                                boost::make_counting_iterator(WRITE_STREAMS),
                                [&](auto idx) {
                                for (unsigned i = idx; i < BLOCKS;
                                     i += WRITE_STREAMS) {
                                while (true) {
                                auto t = create_transaction();
                                auto extent = alloc_extent(t,
                  ...
                  check();
                  logger().
                  info("random_writes_concurrent: {} suceeded, {} failed",
                       writes, failures);}
        ) ;
    }
    ...
    void test_evict() {
        ...
        run_async([this] {
                  ...
                  double stop_ratio =
                  (double)segment_size / (double)device_size / 2;
                  // 1 segment
                  double default_ratio = stop_ratio * 2;
                  // 1.25 segment
                  double fast_ratio = stop_ratio * 2.5;
                  epm->background_process.eviction_state.init(stop_ratio,
                                                              default_ratio,
                                                              fast_ratio);
                  ...
                  return seastar::repeat([this, size] {
                                         size_t current_size =
                                         epm->background_process.main_cleaner->
                  ...
                  using generation_mapping_t =
                  std::map < rewrite_gen_t, rewrite_gen_t >;
                  std::map < extent_types_t,
                  generation_mapping_t > expected_generations;
          ...
          for (auto t:all_extent_types) {
                  ...
                  for (auto i = INIT_GENERATION + 1; i < REWRITE_GENERATIONS;
                       i++) {
              ...
              for (auto t:all_extent_types) {
                  for (auto gen:all_generations) {
                  auto epm_gen = epm->adjust_generation(get_extent_category(t),
                                                        t,
                                                        placement_hint_t::HOT,
                                                        gen);
                  if (expected_generations[t][gen] != epm_gen) {
                  logger().
                  error("caller: {}, extent type: {}, input generation: {}, "
                  ...
                  EXPECT_TRUE(epm->background_process.eviction_state.
                              is_stop_mode()); test_gen("exceed ratio A");
                  epm->run_background_work_until_halt().get();
                  run_until(ratio_B_size).get();
                  EXPECT_TRUE(epm->background_process.eviction_state.
                              is_stop_mode()); test_gen("exceed ratio B");
                  epm->run_background_work_until_halt().get();
                  ...
                  EXPECT_TRUE(epm->background_process.eviction_state.
                              is_default_mode()); test_gen("exceed ratio C");
                  epm->run_background_work_until_halt().get();
                  // verify that data must go to the cold tier
                  run_until(ratio_D_size).get();
                  update_data_gen_mapping([](rewrite_gen_t gen) {
                                          if (gen >= MIN_REWRITE_GENERATION
                  ...
                  EXPECT_TRUE(epm->background_process.eviction_state.
                              is_fast_mode()); test_gen("exceed ratio D");
                  auto main_size =
                  ...
                  EXPECT_TRUE(epm->background_process.eviction_state.
                              is_default_mode()); test_gen("finish evict");}
        ) ;
    }

    std::optional < TestBlockRef > map_existing_extent(test_transaction_t & t,
                                                       laddr_t hint,
        ...
        auto extent = with_trans_intr(*(t.t),[&](auto & trans) {
                                      return tm->map_existing_extent <
                                      TestBlock > (trans, hint, existing_paddr,
                                                   length);}
        ).handle_error(crimson::ct_error::eagain::handle([] {
                                                         return TCachedExtentRef
                                                         < TestBlock >
                                                         (new TestBlock(0));}
    ...
    void test_map_existing_extent() {
        run_async([this] {
                  ...
                  dec_ref(t, offset);
                  auto extent1 =
                  map_existing_extent(t, offset, base_paddr, 4 << 10);
                  ASSERT_TRUE(extent1.has_value());
                  auto extent2 =
                  map_existing_extent(t, offset + (4 << 10),
                                      base_paddr.add_offset(4 << 10), 4 << 10);
                  ASSERT_TRUE(extent2.has_value());
                  auto extent3 =
                  map_existing_extent(t, offset + (8 << 10),
                                      base_paddr.add_offset(8 << 10), 8 << 10);
                  ...
                  submit_transaction(std::move(t)); check();}
                  replay(); check();});
    }
    ...
    void test_map_existing_extent_concurrent() {
        run_async([this] {
                  ...
                  seastar::parallel_for_each(boost::make_counting_iterator(0u),
                                             boost::
                                             make_counting_iterator(REMAP_NUM),
                                             [&](auto) {
                                             return seastar::async([&] {
                                                                   uint32_t
                                                                   pieces =
                                                                   ...
                                                                   early_exit++;
                                                                   return;}
                  auto paddr = ext0->get_paddr(); dec_ref(t, offset); auto base = 0; ASSERT_TRUE(!split_points.empty()); for (auto off:split_points)
                                                                   {
                                                                   if (off == 0) {
                                                                   continue;}
                  ...
                  logger().
                  info
                  ("test_map_existing_extent_concurrent: early_exit {} conflicted {} success {}",
                   early_exit, conflicted, success); ASSERT_TRUE(success == 1);
...
TEST_P(tm_single_device_test_t, basic)
{
    ...
    run_async([this] {
              ...
              check_mappings(t);
              check(); submit_transaction(std::move(t)); check();}
              });
}
...
TEST_P(tm_single_device_test_t, mutate)
{
    ...
    run_async([this] {
              ...
              check_mappings(t);
              check(); submit_transaction(std::move(t)); check();}
              ASSERT_TRUE(check_usage()); replay(); {
              auto t = create_transaction();
              auto ext = get_extent(t,
                                    ADDR,
                                    SIZE);
              auto mut = mutate_extent(t, ext);
              check_mappings(t);
              check(); submit_transaction(std::move(t)); check();}
              ASSERT_TRUE(check_usage()); replay(); check();});
}
...
TEST_P(tm_single_device_test_t, allocate_lba_conflict)
{
    ...
    run_async([this] {
              ...
              auto t = create_transaction(); auto t2 = create_transaction();
...
TEST_P(tm_single_device_test_t, mutate_lba_conflict)
{
    ...
    run_async([this] {
              {
              auto t = create_transaction(); for (unsigned i = 0; i < 300; ++i) {
              auto extent = alloc_extent(t, laddr_t(i * SIZE), SIZE);}
              check_mappings(t); submit_transaction(std::move(t)); check();}
              ...
              check(); {
              auto t = create_transaction();
              mutate_addr(t, ADDR, SIZE); submit_transaction(std::move(t));}
              check();}) ;
}
...
TEST_P(tm_single_device_test_t, concurrent_mutate_lba_no_conflict)
{
    ...
    run_async([this] {
              {
              auto t = create_transaction(); for (unsigned i = 0; i < NUM; ++i) {
              auto extent = alloc_extent(t, laddr_t(i * SIZE), SIZE);}
              submit_transaction(std::move(t));}
...
TEST_P(tm_single_device_test_t, split_merge_read_same_transaction)
{
    ...
    run_async([this] {
              ...
              check_mappings(t); submit_transaction(std::move(t)); check();}
              {
              auto t = create_transaction(); for (unsigned i = 0; i < 240; ++i) {
...
TEST_P(tm_single_device_test_t, random_writes)
{
    ...
    run_async([this] {
              ...
              auto t = create_transaction(); for (unsigned k = 0; k < 2; ++k) {
              auto ext = get_extent(t,
                                    get_random_laddr(BSIZE, TOTAL),
                                    BSIZE); auto mut = mutate_extent(t, ext);
              // pad out transaction
              auto padding = alloc_extent(t,
...
TEST_P(tm_single_device_test_t, find_hole_assert_trigger)
{
    ...
    run([&, this] {
        return seastar::parallel_for_each(boost::make_counting_iterator(0u),
                                          ...
                                          [&, this] (auto idx) {
                                          return allocate_sequentially(BSIZE,
                                                                       num);}
        );}
    );
}
...
INSTANTIATE_TEST_SUITE_P(transaction_manager_test,
                         tm_single_device_test_t,::testing::Values("segmented",
                                                                   "circularbounded")
    );

INSTANTIATE_TEST_SUITE_P(transaction_manager_test,
                         tm_multi_device_test_t,::testing::Values("segmented")
    );

INSTANTIATE_TEST_SUITE_P(transaction_manager_test,
                         tm_multi_tier_device_test_t,::testing::
                         Values("segmented")
    );
