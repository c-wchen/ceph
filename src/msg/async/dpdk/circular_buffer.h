...
template < typename T, typename Alloc = std::allocator < T >>
  ...
  private:
    ...
    template < typename CB, typename ValueType >
        struct cbiterator:std::iterator < std::random_access_iterator_tag,
        ValueType > {
        ...
        // prefix cbiterator < CB, ValueType > &operator++() {
            idx++;
            return *this;
        }
        // postfix cbiterator < CB, ValueType > operator++(int unused) {
            auto v = *this;
            idx++;
            return v;
        }
        ...
        cbiterator < CB,
            ValueType > operator+(typename super_t::difference_type n) const {
            return cbiterator < CB, ValueType > (cb, idx + n);
        } cbiterator < CB,
            ValueType > operator-(typename super_t::difference_type n) const {
            return cbiterator < CB, ValueType > (cb, idx - n);
        } cbiterator < CB,
            ValueType > &operator+=(typename super_t::difference_type n) {
            idx += n;
            return *this;
        }
        cbiterator < CB,
        ...
        bool operator==(const cbiterator < CB, ValueType > &rhs)const {
            ...
            return idx <= rhs.idx;
        } typename super_t::difference_type operator-(const cbiterator < CB,
                                                      ValueType > &rhs)const {
            return idx - rhs.idx;
      } private:
        CB * cb;
...
template < typename T, typename Alloc >
    inline size_t circular_buffer < T, Alloc >::mask(size_t idx) const const
{
    return idx & (_impl.capacity - 1);
}

template < typename T, typename Alloc >
    inline bool circular_buffer < T, Alloc >::empty() constconst
{
    return _impl.begin == _impl.end;
}

template < typename T, typename Alloc >
    inline size_t circular_buffer < T, Alloc >::size() constconst
{
    return _impl.end - _impl.begin;
}

template < typename T, typename Alloc >
    inline size_t circular_buffer < T, Alloc >::capacity() constconst
{
