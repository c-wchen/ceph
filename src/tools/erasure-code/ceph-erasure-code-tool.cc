...
void usage(const std::string message, ostream & out)
{
    ...
    out << "usage: ceph-erasure-code-tool test-plugin-exists <plugin>" << std::
        endl;
    out <<
    ...
    if (args.size() > 1) {
        ...
        for (size_t i = 1; i < args.size(); i++) {
            if (!valid_params.count(args[i])) {
                usage("invalid display param: " + std::string(args[i]),
                      std::cerr);
                return 1;
            }
...
int do_decode(const std::vector < const char *>&args)
{
  ...
  for (auto &[shard, bl]:encoded_data) {
        ...
        if (r < 0) {
            std::cerr << "failed to read " << name << ": " << error << std::
                endl;
            return 1;
        }
    }
