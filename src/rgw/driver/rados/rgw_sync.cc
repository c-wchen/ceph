...
RGWSyncErrorLogger::RGWSyncErrorLogger(rgw::sal::RadosStore* _store, const string &oid_prefix,
                                       int _num_shards) : store(_store), num_shards(_num_shards)
{
...
RGWCoroutine *RGWSyncErrorLogger::log_error_cr(const DoutPrefixProvider *dpp, const string& source_zone,
        const string& section, const string& name, uint32_t error_code, const string& message)
{
    cls_log_entry entry;
...
int RGWRemoteMetaLog::read_master_log_shards_info(const DoutPrefixProvider *dpp, const string &master_period, map < int,
        RGWMetadataLogInfo > *shards_info)
{
...
int RGWRemoteMetaLog::read_master_log_shards_next(const DoutPrefixProvider *dpp, const string& period, map < int,
        string > shard_markers, map < int, rgw_mdlog_shard_data > *result)
{
    if (store->svc()->zone->is_meta_master()) {
        return 0;
    }
...
int RGWMetaSyncStatusManager::init(const DoutPrefixProvider *dpp)
{
    ...
    int r = rgw_init_ioctx(dpp, store->getRados()->get_rados_handle(), store->svc()->zone->get_zone_params().log_pool,
                           ioctx, true);
    if (r < 0) {
        ldpp_dout(dpp, -1) << "ERROR: failed to open log pool (" << store->svc()->zone->get_zone_params().log_pool << " ret=" <<
                           r << dendl;
        return r;
    }
...
bool RGWListRemoteMDLogCR::spawn_next()
{
    ...
    spawn(new RGWListRemoteMDLogShardCR(sync_env, period, iter->first, iter->second, max_entries_per_shard,
...
public:
    ...
    RGWCoroutine *store_marker(const string& new_marker, uint64_t index_pos, const real_time& timestamp) override
    {
        ...
        ldpp_dout(sync_env->dpp, 20) << __func__ << "(): updating marker marker_oid=" << marker_oid << " marker=" << new_marker
...
#define NUM_TRANSIENT_ERROR_RETRIES 10
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if (sync_status < 0) {
                if (tries < NUM_TRANSIENT_ERROR_RETRIES - 1) {
                    ldpp_dout(dpp, 20) << *this << ": failed to fetch remote metadata entry: " << section << ":" << key << ", will retry" <<
                                       dendl;
                    continue;
                }

                tn->log(10, SSTR("failed to read remote metadata entry: section=" << section << " key=" << key << " status=" <<
                                 sync_status));
                log_error() << "failed to read remote metadata entry: section=" << section << " key=" << key << " status=" <<
                            sync_status << std::endl;
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if ((retcode < 0) && (tries < NUM_TRANSIENT_ERROR_RETRIES - 1)) {
                ldpp_dout(dpp, 20) << *this << ": failed to store metadata entry: " << section << ":" << key << ", got retcode=" <<
                                   retcode << ", will retry" << dendl;
                continue;
            }
            break;
        }
...
public:
    ...
    void collect_children()
    {
        ...
        while (collect_next(&child_ret, &child)) {
            ...
            if (child_ret < 0) {
                ldpp_dout(sync_env->dpp, 0) << *this << ": child operation stack=" << child << " entry=" << pos << " returned " <<
                                            child_ret << dendl;
            ...
            *reset_backoff = true;
            ...
            if (lost_bid) {
                ...
                if (can_adjust_marker) {
                    ...
                    ldpp_dout(sync_env->dpp, 4) << *this << ": saving marker pos=" << temp_marker->marker << " realm_epoch=" << realm_epoch
                                                << dendl;
...
#define INCREMENTAL_MAX_ENTRIES 100
                ldpp_dout(sync_env->dpp, 20) << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id << " mdlog_marker=" <<
                                             mdlog_marker << " sync_marker.marker=" << sync_marker.marker << " period_marker=" << period_marker << " truncated=" <<
                                             truncated << dendl;
                if (!period_marker.empty() && period_marker <= mdlog_marker) {
                    tn->log(10, SSTR("finished syncing current period: mdlog_marker=" << mdlog_marker << " sync_marker=" <<
                                     sync_marker.marker << " period_marker=" << period_marker));
                ...
                *reset_backoff = true; /* if we got to this point, all systems function */
                if (mdlog_marker > max_marker) {
                    ...
                    for (log_iter = log_entries.begin(); log_iter != log_entries.end() && !done_with_period; ++log_iter) {
                        ...
                        if (!mdlog_entry.convert_from(*log_iter)) {
                            tn->log(0, SSTR("ERROR: failed to convert mdlog entry, shard_id=" << shard_id << " log_entry: " << log_iter->id << ":"
                                            << log_iter->section << ":" << log_iter->name << ":" << log_iter->timestamp << " ... skipping entry"));
                            continue;
                        }
                        tn->log(20, SSTR("log_entry: " << log_iter->id << ":" << log_iter->section << ":" << log_iter->name << ":" <<
                                         log_iter->timestamp));
                ...
                collect_children();
                ldpp_dout(sync_env->dpp, 20) << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id << " mdlog_marker=" <<
                                             mdlog_marker << " max_marker=" << max_marker << " sync_marker.marker=" << sync_marker.marker << " period_marker=" <<
                                             period_marker << dendl;
                if (done_with_period) {
...
int RGWRemoteMetaLog::run_sync(const DoutPrefixProvider *dpp, optional_yield y)
{
    ...
    if (num_shards != mdlog_info.num_shards) {
        ldpp_dout(dpp, -1) << "ERROR: can't sync, mismatch between num shards, master num_shards=" << mdlog_info.num_shards <<
                              " local num_shards=" << num_shards << dendl;
        return -EINVAL;
    }
...
int RGWCloneMetaLogCoroutine::state_read_shard_status_complete()
{
    ...
    ldpp_dout(sync_env->dpp, 20) << "shard_id=" << shard_id << " marker=" << shard_info.marker << " last_update=" <<
                                 shard_info.last_update << dendl;
...
int RGWCloneMetaLogCoroutine::state_receive_rest_response()
{
    ...
    ldpp_dout(sync_env->dpp, 20) << "remote mdlog, shard_id=" << shard_id << " num of shard entries: " <<
                                 data.entries.size() << dendl;
