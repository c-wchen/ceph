...
template < typename ImageCtxT = librbd::ImageCtx > struct Snapshot {
    ...
    static int get_mirror_namespace(ImageCtxT *ictx, uint64_t snap_id,
    ...
    static int get_name(ImageCtxT *ictx, uint64_t snap_id,
                        std::string *snap_name);

    static int get_id(ImageCtxT *ictx, const std::string &snap_name,
                      uint64_t *snap_id);

    static int list(ImageCtxT *ictx,
                    std::vector < snap_info_t > &snaps);

    static int exists(ImageCtxT *ictx,
                      const cls::rbd::
                      SnapshotNamespace &snap_namespace,
                      const char *snap_name, bool *exists);

    static int create(ImageCtxT *ictx, const char *snap_name,
                      uint32_t flags, ProgressContext &pctx);

    static int remove(ImageCtxT *ictx, const char *snap_name,
                      uint32_t flags, ProgressContext &pctx);
    ...
    static int get_timestamp(ImageCtxT *ictx, uint64_t snap_id,
                             struct timespec *timestamp);

    static int is_protected(ImageCtxT *ictx, const char *snap_name,
                            bool *protect);
