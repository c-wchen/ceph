...
void flush_evict_all(librados::Rados & cluster, librados::IoCtx & cache_ioctx)
{
    ...
    for (NObjectIterator it = cache_ioctx.nobjects_begin();
        ...
        cache_ioctx.set_namespace(it->get_nspace());
        {
            ...
            cache_ioctx.aio_operate(it->get_oid(), completion, &op,
...
void check_fp_oid_refcount(librados::IoCtx & ioctx, std::string foid,
                           uint64_t count, std::string fp_algo = NULL)
{
    bufferlist t;
...
class LibRadosTwoPoolsPP:public RadosTestPP {
  ...
  protected:
    ...
    void SetUp() override {
        ...
        ASSERT_EQ(0,
                  cluster.ioctx_create(cache_pool_name.c_str(), cache_ioctx));
    ...
    void TearDown() override {
        ...
        ASSERT_EQ(0,
                  ...
                  ("{\"prefix\": \"osd tier remove-overlay\", \"pool\": \"" +
                   pool_name + "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  cluster.
                  mon_command("{\"prefix\": \"osd tier remove\", \"pool\": \"" +
                              pool_name + "\", \"tierpool\": \"" +
                              cache_pool_name + "\"}", inbl, NULL, NULL));
...
TEST_F(LibRadosTwoPoolsPP, Overlay)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command("{\"prefix\": \"osd tier add\", \"pool\": \"" +
                          pool_name + "\", \"tierpool\": \"" + cache_pool_name +
                          "\", \"force_nonempty\": \"--force-nonempty\" }",
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command("{\"prefix\": \"osd tier set-overlay\", \"pool\": \""
                          + pool_name + "\", \"overlaypool\": \"" +
                          cache_pool_name + "\"}", inbl, NULL, NULL));
...
TEST_F(LibRadosTwoPoolsPP, Promote)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command("{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" +
...
TEST_F(LibRadosTwoPoolsPP, PromoteSnapScrub)
{
    ...
    for (unsigned snap = 0; snap < my_snaps.size(); ++snap) {
        cout << "promoting from clones for snap " << my_snaps[snap] << std::
            endl;
...
TEST_F(LibRadosTwoPoolsPP, Evict)
{
    ...
    }
    {
        ...
        op.cache_evict();
        librados::AioCompletion * completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("bar", completion, &op,
                                             librados::OPERATION_IGNORE_CACHE,
                                             NULL));
        completion->wait_for_complete();
...
TEST_F(LibRadosTwoPoolsPP, EvictSnap)
{
    ...
    // evict bam
    {
        ...
        op.cache_evict();
        librados::AioCompletion * completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("bam", completion, &op,
                                             librados::OPERATION_IGNORE_CACHE,
                                             NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
    ...
    for (int i = 0; i < retries; ++i) {
        {
            librados::AioCompletion * completion =
                cluster.aio_create_completion();
            librados::AioCompletion * completion1 =
                cluster.aio_create_completion();
        ...
        ioctx.snap_set_read(my_snaps[0]);
        {
            ...
            librados::AioCompletion * completion =
                cluster.aio_create_completion();
            ASSERT_EQ(0,
                      ioctx.aio_operate("foo", completion, &op,
                                        librados::OPERATION_IGNORE_CACHE,
                                        NULL));
            completion->wait_for_complete();
            ASSERT_EQ(0, completion->get_return_value());
...
TEST_F(LibRadosTwoPoolsPP, TryFlush)
{
    ...
    // evict it
    {
        ...
        librados::AioCompletion * completion = cluster.aio_create_completion();
        ASSERT_EQ(0,
                  cache_ioctx.aio_operate("foo", completion, &op,
                                          librados::OPERATION_IGNORE_CACHE,
                                          NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
...
TEST_F(LibRadosTierPP, FlushWriteRaces)
{
    ...
    do {
        ...
        // try-flush + write
        {
            ...
            ObjectWriteOperation op2;
            op2.write_full(bl);
            librados::AioCompletion * completion2 =
                cluster.aio_create_completion();
            ASSERT_EQ(0, ioctx.aio_operate("foo", completion2, &op2, 0));
...
TEST_F(LibRadosTwoPoolsPP, FlushTryFlushRaces)
{
    ...
    // flush + flush
    {
        ...
        op2.cache_flush();
        librados::AioCompletion * completion2 = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("foo", completion2, &op2,
                                             librados::OPERATION_IGNORE_OVERLAY,
                                             NULL));
    ...
    do {
        ...
        //  (flush will not piggyback on try-flush)
        {
            ...
            librados::AioCompletion * completion2 =
                cluster.aio_create_completion();
            ASSERT_EQ(0,
                      cache_ioctx.aio_operate("foo", completion2, &op2,
                                              librados::
                                              OPERATION_IGNORE_OVERLAY, NULL));
...
TEST_F(LibRadosTwoPoolsPP, HitSetRead)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_count", 2),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_period", 600),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "hit_set_type", "explicit_object"),
                          inbl, NULL, NULL));
...
static int _get_pg_num(Rados & cluster, string pool_name)
{
    ...
    string cmd = string("{\"prefix\": \"osd pool get\",\"pool\":\"")
        + pool_name + string("\",\"var\": \"pg_num\",\"format\": \"json\"}");
    bufferlist outbl;
    int r = cluster.mon_command(cmd, inbl, &outbl, NULL);
    ceph_assert(r >= 0);
    string outstr(outbl.c_str(), outbl.length());
...
int make_hitset(Rados & cluster, librados::IoCtx & cache_ioctx, int num_pg,
                int num, std::map < int, HitSet > &hitsets,
                std::string & cache_pool_name)
{
    ...
    for (int i = 0; i < pg; ++i) {
        ...
        try {
            ...
            std::cout << "failed to decode hit set; bl len is " << bl.
                length() << "\n";
            bl.hexdump(std::cout);
            std::cout << std::endl;
            throw e;
...
TEST_F(LibRadosTwoPoolsPP, HitSetWrite)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_count", 8),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_period", 600),
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "hit_set_type", "explicit_hash"),
                          inbl, NULL, NULL));
    ...
    num_pg =
        make_hitset(cluster, cache_ioctx, num_pg, num, hitsets,
                    cache_pool_name);
...
TEST_F(LibRadosTwoPoolsPP, HitSetTrim)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_count", count),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "hit_set_period", period), inbl,
                          NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "hit_set_type", "bloom"), inbl,
                          NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_fpp", ".01"),
                          inbl, NULL, NULL));
        ...
        if (!ls.empty()) {
            if (!first) {
                ...
                if (ls.front().first != first) {
                    cout << "first now " << ls.front().
                        first << ", trimmed" << std::endl;
                    break;
                }
            }
        }
...
TEST_F(LibRadosTwoPoolsPP, PromoteOn2ndRead)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "min_read_recency_for_promote", 1),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "hit_set_grade_decay_rate", 20),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "hit_set_search_last_n", 1), inbl,
                          NULL, NULL));
...
TEST_F(LibRadosTwoPoolsPP, CachePin)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "target_max_objects", 1), inbl,
                          NULL, NULL));
...
TEST_F(LibRadosTwoPoolsPP, ManifestPromoteRead)
{
    ...
    // set-chunk
    manifest_set_chunk(cluster, cache_ioctx, ioctx, 0, 2, "bar-chunk",
                       "foo-chunk");
...
TEST_F(LibRadosTwoPoolsPP, ManifestDedupRefRead)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (pool_name, "fingerprint_algorithm", "sha1"), inbl,
                          NULL, NULL));
    cluster.wait_for_latest_osdmap();
    string tgt_oid;
...
TEST_F(LibRadosTwoPoolsPP, DedupFlushRead)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "fingerprint_algorithm", "sha1"),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "dedup_tier", pool_name), inbl,
                          NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "dedup_chunk_algorithm", "fastcdc"),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "dedup_cdc_chunk_size", 1024), inbl,
                          NULL, NULL));
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "dedup_cdc_chunk_size", 512), inbl,
                          NULL, NULL));
    cluster.wait_for_latest_osdmap();
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "dedup_cdc_chunk_size", 16384),
                          inbl, NULL, NULL));
    cluster.wait_for_latest_osdmap();
...
TEST_F(LibRadosTwoPoolsPP, TierFlushDuringFlush)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "dedup_tier", temp_pool_name), inbl,
                          NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
...
TEST_F(LibRadosTwoPoolsECPP, PromoteSnap)
{
    ...
    // This test requires cache tier and base tier to have the same pg_num/pgp_num
    {
        for (int tries = 0; tries < 5; ++tries) {
            IoCtx cache_ioctx;
            ASSERT_EQ(0,
                      cluster.ioctx_create(cache_pool_name.c_str(),
                                           cache_ioctx));
            uint32_t hash;
            ASSERT_EQ(0, ioctx.get_object_pg_hash_position2("foo", &hash));
            ostringstream ss;
            ss << "{\"prefix\": \"pg scrub\", \"pgid\": \""
                << cache_ioctx.get_id() << "." << hash << "\"}";
            int r = cluster.mon_command(ss.str(), inbl, NULL, NULL);
...
TEST_F(LibRadosTierECPP, CallForcesPromote)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "min_read_recency_for_promote",
                           "4"), inbl, NULL, NULL));
...
TEST_F(LibRadosTierECPP, HitSetWrite)
{
    ...
    bufferlist inbl;
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(pool_name, "hit_set_count", 8),
                                  inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(pool_name, "hit_set_period", 600), inbl,
                          NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (pool_name, "hit_set_type", "explicit_hash"), inbl,
                          NULL, NULL));
...
TEST_F(LibRadosTwoPoolsECPP, ManifestPromoteRead)
{
    ...
    // set-chunk
    manifest_set_chunk(cluster, ioctx, cache_ioctx, 0, 10, "bar-chunk",
                       "foo-chunk");
    // promote
    {
        ObjectWriteOperation op;
...
TEST_F(LibRadosTwoPoolsECPP, TrySetDedupTier)
{
    ...
    ASSERT_EQ(-EOPNOTSUPP,
              cluster.
              mon_command(set_pool_str
                          (pool_name, "dedup_tier", cache_pool_name), inbl,
                          NULL, NULL));
}
...
TEST_F(LibRadosTwoPoolsPP, PropagateBaseTierError)
{
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_count", 1),
                          inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str(cache_pool_name, "hit_set_period", 600),
    ...
    ASSERT_EQ(0,
              cluster.
              mon_command(set_pool_str
                          (cache_pool_name, "target_max_objects", 250), inbl,
                          NULL, NULL));
    ...
    op2.omap_cmp( { {
                 "somekey", {
                 test_omap_bl, CEPH_OSD_CMPXATTR_OP_EQ}
                 }
                 }
                 , nullptr);
    op2.omap_set( { {
                 "somekey", test_omap_bl}
                 }
    );
