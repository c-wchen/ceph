  ...
  public:
    ...
    void set_peer_config_key(const std::string & pool_name,
        ...
        ASSERT_EQ(0, m_cluster->mon_command("{"
                                            ...
                                            "\"key\": \""
                                            RBD_MIRROR_PEER_CONFIG_KEY_PREFIX +
                                            stringify(pool_id) + "/" +
                                            peer.uuid + "\"," "\"val\": \"" +
                                            json + "\"" + "}", in_bl, nullptr,
                                            nullptr));
        ...
        *cache_pool_name = get_temp_pool_name("test-rbd-mirror-");
        ...
        ASSERT_EQ(0,
                  ...
                  ("{\"prefix\": \"osd tier set-overlay\", \"pool\": \"" +
                   base_pool + "\", \"overlaypool\": \"" + *cache_pool_name +
                   "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  m_cluster->
                  mon_command
                  ("{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" +
                   *cache_pool_name + "\", \"mode\": \"writeback\"}", inbl,
                   NULL, NULL));
        m_cluster->wait_for_latest_osdmap();
    }
    ...
    void remove_cache_pool(const string & base_pool, const string & cache_pool) {
        ...
        ASSERT_EQ(0,
                  ...
                  ("{\"prefix\": \"osd tier remove-overlay\", \"pool\": \"" +
                   base_pool + "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  m_cluster->
                  mon_command("{\"prefix\": \"osd tier remove\", \"pool\": \"" +
                              base_pool + "\", \"tierpool\": \"" + cache_pool +
                              "\"}", inbl, NULL, NULL));
        m_cluster->wait_for_latest_osdmap();
