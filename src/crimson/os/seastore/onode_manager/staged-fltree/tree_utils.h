 ...
 */
    ...
    template < typename CursorType >
        void initialize_cursor_from_item(Transaction & t,
                                         const ghobject_t & key,
                                         const typename decltype(std::declval <
                                                                 CursorType >
                                                                 ().
                                                                 value())::
                                         item_t & item, CursorType & cursor,
                                         bool insert_success) {
    ...
    template < typename ValueItem > class KVPool {
      public:
        ...
        void shuffle() {
            std::shuffle(random_p_kvs.begin(), random_p_kvs.end(),
        ...
        static KVPool create_raw_range(const std::vector < size_t > &ns_sizes,
                                       const std::vector < size_t > &oid_sizes,
                                       const std::vector < size_t >
                                       &value_sizes, const std::pair < index_t,
                                       index_t > &range2,
            ...
            for (index_t i = range2.first; i < range2.second; ++i) {
                for (index_t j = range1.first; j < range1.second; ++j) {
                    ...
                    for (index_t k = range0.first; k < range0.second; ++k) {
                        kvs.emplace_back(kv_t {
                                         make_raw_oid(i, j, k, ns_size,
                                                      oid_size), values.pick()}
                        );
                    }
                }
            }
        ...
        static KVPool create_range(const std::pair < index_t,
                                   index_t > &range_i,
            ...
            for (index_t i = range_i.first; i < range_i.second; ++i) {
                ...
                kvs.emplace_back(kv_t {
                                 make_oid(i), ValueItem::create(value_size, i,
                                                                block_size)}
                );
            }
            return KVPool(std::move(kvs));
      ...
      :    
        kvs(std::move(_kvs)), serial_p_kvs(kvs.size()),
        ...
        static ghobject_t make_raw_oid(index_t index2, index_t index1,
                                       index_t index0, size_t ns_size,
                                       size_t oid_size) {
            assert(index1 < 10);
    ...
    template < bool TRACK, typename ValueImpl > class TreeBuilder {
      ...
      :    kvs {
        ...
        eagain_ifuture < BtreeCursor > insert_one(Transaction & t,
                                                  const iterator_t & iter_rd) {
            auto p_kv = *iter_rd;
            logger().debug("[{}] insert {} -> {}",
                           iter_rd - kvs.random_begin(), key_hobj_t {
                           p_kv->key}, p_kv->value);
            return tree->insert(t, p_kv->key, {
                                p_kv->value.get_payload_size()}
                      ...
                      initialize_cursor_from_item(t, p_kv->key, p_kv->value,
                                                  cursor, success);
#ifndef NDEBUG
                      validate_cursor_from_item(p_kv->key, p_kv->value, cursor);
                      return tree->find(t,
                                        p_kv->key).si_then([cursor,
                                                            p_kv] (auto cursor_)
                                                           mutable {
                                                           assert(!cursor_.
                                                                  is_end());
...
#else
                      return eagain_iertr::make_ready_future < BtreeCursor >
                      (cursor);
#endif
                      }
            ).handle_error_interruptible([]
                                         (const crimson::ct_error::
                                          value_too_large & e) {
        ...
        eagain_ifuture <> insert(Transaction & t) {
            ...
            *ref_kv_iter = kvs.random_begin();
            auto cursors =
                seastar::make_lw_shared < std::vector < BtreeCursor >> ();
            ...
            return trans_intr::repeat([&t, this, cursors, ref_kv_iter,
                ...
                .si_then([&t, this, cursors, ref_kv_iter] {
                         ...
                         *ref_kv_iter = kvs.random_begin();
                         return seastar::do_with(cursors->begin(),
                                                 [&t, this, cursors,
                                                  ref_kv_iter] (auto & c_iter) {
                                                 return trans_intr::
                                                 repeat([&t, this, &c_iter,
                                                         cursors,
                                                         ref_kv_iter] ()
                                                        ->eagain_ifuture <
                                                        seastar::
        ...
        eagain_ifuture <> erase_one(Transaction & t, const iterator_t & iter_rd) {
            ...
            logger().debug("[{}] erase {} -> {}",
                           ...
                           p_kv->key}, p_kv->value);
            return tree->erase(t,
                               p_kv->key).si_then([&t, this, p_kv] (auto size) {
        ...
        eagain_ifuture <> erase(Transaction & t, std::size_t erase_size) {
            ...
            auto ref_kv_iter = seastar::make_lw_shared < iterator_t > ();
            auto cursors =
                seastar::make_lw_shared < std::map < ghobject_t,
                BtreeCursor >> ();
            return eagain_iertr::now().si_then([&t, this, cursors, ref_kv_iter] {
                                               ...
                                               logger().
                                               info
                                               ("Tracking cursors before erase ...");
                                               *ref_kv_iter = kvs.begin();
                                               auto start_time =
                                               mono_clock::now();
                                               return trans_intr::
                                               repeat([&t, this, cursors,
                                                       ref_kv_iter,
                                                       start_time] ()
                                                      ->eagain_ifuture <
                                                      seastar::stop_iteration >
                ...
                .si_then([&t, this, ref_kv_iter, erase_end] {
                         *ref_kv_iter = kvs.random_begin();
                         logger().warn("start erasing {}/{} kvs ...",
                                       erase_end - kvs.random_begin(),
                                       kvs.size());
                         auto start_time = mono_clock::now();
                         return trans_intr::
                         repeat([&t, this, ref_kv_iter, start_time,
                                 erase_end] ()
                                ->eagain_ifuture < seastar::stop_iteration > {
                                if (*ref_kv_iter == erase_end) {
                                std::chrono::duration < double >duration =
                                mono_clock::now() - start_time;
                                logger().warn("Erase done! {}s",
                                              duration.count());
                                return seastar::make_ready_future <
                                seastar::stop_iteration >
                                (seastar::stop_iteration::yes);}
                ...
                .si_then([this, cursors, ref_kv_iter, erase_end] {
                         ...
                         *ref_kv_iter = kvs.random_begin();
                         ...
                         cursors->erase(c_it); ++(*ref_kv_iter);}
                         }
                         kvs.erase_from_random(kvs.random_begin(), erase_end);
                         if constexpr(TRACK) {
                  *ref_kv_iter = kvs.begin(); for (auto &[k, c]:*cursors)
                         {
        ...
        eagain_ifuture <> validate_one(Transaction & t,
                                       const iterator_t & iter_seq) {
            assert(iter_seq != kvs.end());
            auto next_iter = iter_seq + 1;
            auto p_kv = *iter_seq;
            return tree->find(t, p_kv->key).si_then([p_kv, &t] (auto cursor) {
                                                    validate_cursor_from_item
                                                    (p_kv->key, p_kv->value,
                                                     cursor);
                      ...
                      auto p_kv = *next_iter;
                      validate_cursor_from_item(p_kv->key, p_kv->value,
                                                cursor);}
                      }
            ) ;
        }
