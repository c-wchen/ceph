...
template <> struct OpenRequest <MockTestImageCtx > {
    ...
    static OpenRequest *create(MockTestImageCtx *
...
OpenRequest < MockTestImageCtx > *OpenRequest <
...
public:
                                 ...
                                 int r)
    {
        ...
        WillOnce(FinishRequest
                 (&mock_open_request, r, &mock_image_ctx));
    } void expect_allocate_tag(::journal::
                               MockJournaler &mock_journaler,
                               const journal::
                               TagPredecessor &predecessor, int r)
    {
        TagData tag_data;
        ...
        EXPECT_CALL(mock_journaler,
                    allocate_tag(456, ContentsEqual(tag_data_bl), _, _))
        .WillOnce(WithArg < 3 >
                  (CompleteContext
                   (r, static_cast < asio::ContextWQ * >(NULL))));
    } void expect_append_journaler(::journal::
                                   MockJournaler &mock_journaler)
    {
    ...
    void expect_future_committed(::journal::
                                 MockJournaler &mock_journaler)
    {
        EXPECT_CALL(mock_journaler,
                    committed(A <
...
TEST_F(TestMockJournalPromoteRequest, SuccessOrderly)
{
    ...
    expect_allocate_tag(mock_journaler, {
        Journal <>::ORPHAN_MIRROR_UUID, true, 567, 1
    },
    0);
...
TEST_F(TestMockJournalPromoteRequest, SuccessForced)
{
    ...
    expect_open_journaler(mock_image_ctx, mock_open_request, 0);
    expect_allocate_tag(mock_journaler, {
        Journal <>::LOCAL_MIRROR_UUID, true, 567, 0
    },
    0);
