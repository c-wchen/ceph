...
class StoreTool {
  ...
  public:
    ...
    uint32_t traverse(const string & prefix, const bool do_crc, ostream * out) {
    ...
    int copy_store_to(string type, const string & other_path,
        ...
        KeyValueDB *other =
            KeyValueDB::create(g_ceph_context, type, other_path);
        ...
        std::cout << "  total size " << stringify(si_u_t(total_size)) << std::
            endl;
        std::
            cout << "  from '" << store_path << "' to '" << other_path << "'" <<
            std::endl;
