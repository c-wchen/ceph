...
namespace ceph {
   ...
   */
        mempool::pgmap::unordered_map < int64_t,
        mempool::pgmap::list < std::pair < pool_stat_t,
   ...
   */
    mempool::pgmap::unordered_map < int64_t, std::pair < pool_stat_t,
        utime_t > >per_pool_sum_delta;

    pool_stat_t pg_sum_delta;
    utime_t stamp_delta;

    void get_recovery_stats(double *misplaced_ratio,
    ...
    void overall_recovery_rate_summary(ceph::Formatter * f,
   ...
   */
    void overall_client_io_rate_summary(ceph::Formatter * f,
                                        std::ostream * out) const;
  /**
   ...
   * calculated resorting to @p pg_sum_delta and @p stamp_delta.
   */
    void overall_cache_io_rate_summary(ceph::Formatter * f,
                                       std::ostream * out) const;
  /**
   ...
   */
    virtual void dump_pool_stats_full(const OSDMap & osd_map,
                                      std::stringstream * ss,
                                      ceph::Formatter * f, bool verbose) const;
    void dump_cluster_stats(std::stringstream * ss, ceph::Formatter * f,
                            bool verbose) const;
...
class PGMap:public PGMapDigest {
  public:
    ...
    typedef mempool::pgmap::map < std::pair < int64_t, int >,   // <pool, osd>
     store_statfs_t > per_osd_pool_statfs_t;
  ...
  public:
    ...
    mempool::pgmap::map < int, std::map < epoch_t,
  ...
  :    version(0), last_osdmap_epoch(0), last_pg_scan(0) {
    ...
    void dump_filtered_pg_stats(ceph::Formatter * f,
     ...
     */ void dump_pool_stats_and_io_rate(int64_t poolid, const OSDMap & osd_map,
                                         ceph::Formatter * f,
                                         std::stringstream * ss) const;

    static void dump_pg_stats_plain(std::ostream & ss,
    ...
    void dump_filtered_pg_stats(std::ostream & ss,
                                std::set < pg_t > &pgs) const;
...
int process_pg_map_command(const std::string & prefix,
                           const cmdmap_t & cmdmap,
                           const PGMap & pg_map,
                           const OSDMap & osdmap,
                           ceph::Formatter * f,
                           std::stringstream * ss, ceph::buffer::list * odata);
