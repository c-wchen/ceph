...
struct librados::IoCtxImpl {
    ...
    int get_object_hash_position(const std::string & oid,
                                 uint32_t * hash_position);
    int get_object_pg_hash_position(const std::string & oid,
    ...
    int checksum(const object_t & oid, uint8_t type,
                 const bufferlist & init_value, size_t len, uint64_t off,
    ...
    int exec(const object_t & oid, const char *cls, const char *method,
    ...
    int operate(const object_t & oid,::ObjectOperation * o,
                ceph::real_time * pmtime, int flags = 0);
    int operate_read(const object_t & oid,::ObjectOperation * o,
    ...
    int watch(const object_t & oid, uint64_t * cookie, librados::WatchCtx * ctx,
              ...
              false);
    int aio_watch(const object_t & oid, AioCompletionImpl * c,
    ...
    int notify(const object_t & oid, bufferlist & bl, uint64_t timeout_ms,
               bufferlist * preplybl, char **preply_buf,
