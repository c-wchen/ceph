        ...
        namespace snapshot {
            namespace util {
                ...
                namespace {
                    ...
                    const std::string IMAGE_STATE_OBJECT_PREFIX =
                        "rbd_mirror_snapshot.";

                    bool get_rollback_snap_id(std::map < librados::snap_t,
                                              SnapInfo >::reverse_iterator it,
                                              std::map < librados::snap_t,
                                              SnapInfo >::reverse_iterator end,
                                              uint64_t * rollback_snap_id) {

                        for (; it != end; it++) {
                            auto mirror_ns =
                                std::get < cls::rbd::MirrorSnapshotNamespace >
                ...
                template < typename I >
                    bool can_create_primary_snapshot(I * image_ctx,
                    ...
                    for (auto it = image_ctx->snap_info.rbegin();
                        ...
                        ldout(cct,
                              20) << "previous snapshot snap_id=" << it->
                            first << " " << *mirror_ns << dendl;
                        if (mirror_ns->is_demoted() && !force) {
                            lderr(cct) <<
                                "trying to create primary snapshot without force "
                        ...
                        if (mirror_ns->state ==
                            ...
                            if (demoted) {
                                lderr(cct) <<
                                    "trying to create primary demoted snapshot "
                                    << "when previous snapshot is non-primary"
                                    << dendl;
                                return false;
                            }
                    ...
                    ldout(cct,
                          20) << "no previous mirror snapshots found" << dendl;
                    return true;
                }
                ...
                template < typename I >
                    ...
                    for (auto it = image_ctx->snap_info.rbegin();
                        ...
                        if (mirror_ns != nullptr) {
                            ldout(cct,
                                  20) << "previous mirror snapshot snap_id=" <<
                                it->first << " " << *mirror_ns << dendl;

                            if (mirror_ns->state ==
                                cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY) {
                                if (!mirror_ns->complete) {
                                    lderr(cct) <<
                                        "trying to create non-primary snapshot "
                                        <<
                                        "when previous non-primary snapshot is not copied yet"
                                        << dendl;
                                    return false;
                ...
                template < typename I >
                    std::string image_state_object_name(I * image_ctx,
                                                        uint64_t snap_id,
                                                        uint64_t index) {
...
template bool librbd::mirror::snapshot::util::
can_create_primary_snapshot(librbd::ImageCtx * image_ctx, bool demoted,
                            bool force, bool * requires_orphan,
                            uint64_t * rollback_snap_id);

template bool librbd::mirror::snapshot::util::
can_create_non_primary_snapshot(librbd::ImageCtx * image_ctx);

template std::string librbd::mirror::snapshot::util::
image_state_object_name(librbd::ImageCtx * image_ctx, uint64_t snap_id,
                        uint64_t index);
