...
static int log_index_operation(cls_method_context_t hctx,
                               const cls_rgw_obj_key & obj_key, RGWModifyOp op,
                        ...
                        bool * pmore)
{
    ...
    auto comp =
        [](const pair < std::string, bufferlist > &l, const std::string & r){
        return l.first < r;
    };
    std::string new_start = {
    static_cast < char >(BI_PREFIX_CHAR + 1)};
                    ...
                    bufferlist * out)
{
    ...
    for (int attempt = 0;
         ...
         more && !done && name_entry_map.size() < op.num_entries; ++attempt) {
        ...
        CLS_LOG(20,
                "%s: on attempt %d get_obj_vls returned %ld entries, more=%d",
        ...
        for (auto kiter = keys.cbegin(); kiter != keys.cend(); ++kiter) {
            ...
            if (has_delimiter) {
                int delim_pos =
                    key.name.find(op.delimiter, op.filter_prefix.size());
...
static int read_key_entry(cls_method_context_t hctx,
                          const cls_rgw_obj_key & key, string * idx,
                          rgw_bucket_dir_entry * entry,
...
template < class T >
    static int read_omap_entry(cls_method_context_t hctx,
                               const std::string & name, T * entry)
{
                               ...
                               const cls_rgw_obj_key & key, bool log_op)
{
    ...
    if (log_op) {
        ...
        ret =
            log_index_operation(hctx, key, CLS_RGW_OP_DEL, tag,
                                entry.meta.mtime, entry.ver,
                                CLS_RGW_STATE_COMPLETE, header.ver,
                                header.max_marker, 0, nullptr, nullptr,
                                nullptr);
        if (ret < 0) {
            return ret;
        }
...
static int read_olh(cls_method_context_t hctx, cls_rgw_obj_key & obj_key,
                    rgw_bucket_olh_entry * olh_data_entry, string * index_key,
                    bool * found)
{
...
static void update_olh_log(rgw_bucket_olh_entry & olh_data_entry, OLHLogOp op,
                           const string & op_tag, cls_rgw_obj_key & key,
                           bool delete_marker, uint64_t epoch)
{
                               ...
                               bufferlist * out)
{
    ...
    if (olh_found) {
        ...
        ret =
            convert_plain_entry_to_versioned(hctx, key, promote, instance_only);
        if (ret < 0) {
                                      ...
                                      bufferlist * in, bufferlist * out)
{
    ...
    ret =
        log_index_operation(hctx, op.key, CLS_RGW_OP_UNLINK_INSTANCE, op.op_tag,
                            mtime, ver, CLS_RGW_STATE_COMPLETE, header.ver,
                            header.max_marker,
                            op.bilog_flags | RGW_BILOG_FLAG_VERSIONED_OP, NULL,
                            NULL, &op.zones_trace);
    if (ret < 0)
        return ret;
                            ...
                            bufferlist * in, bufferlist * out)
{
    ...
    timespan tag_timeout(std::chrono::seconds(header.tag_timeout ?
                                              header.tag_timeout :
                                              (config_op_expiration ?
                                               config_op_expiration :
                                               CEPH_RGW_DEFAULT_TAG_TIMEOUT)));
    CLS_LOG_BITX(bitx_inst, 10, "INFO: %s: tag_timeout=%ld", __func__,
                 tag_timeout.count());
    ...
    while (!in_iter.end()) {
        ...
        CLS_LOG_BITX(bitx_inst, 10,
                     ...
                     __func__, op, escape_str(cur_change_key).c_str(),
                     cur_change.exists);
        CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: setting map entry at key=%s",
        ...
        if (cur_disk_bl.length()) {
            ...
            try {
                ...
                CLS_LOG_BITX(bitx_inst, 1,
                             "ERROR: %s: failed to decode cur_disk", __func__);
                return -EINVAL;
            }
        ...
        CLS_LOG_BITX(bitx_inst, 20,
                     ...
                     __func__, op, cur_disk.pending_map.empty(),
                     cur_disk.exists, (int)cur_disk.index_ver,
                     cur_change.exists, (int)cur_change.index_ver);
    ...
    if (header_changed) {
        CLS_LOG_BITX(bitx_inst, 10, "INFO: %s: bucket header changed, writing",
                     __func__);
        ...
        if (ret < 0) {
            ...
            CLS_LOG_BITX(bitx_inst, 10, "EXITING %s, returning %d", __func__,
                         ret);
        }
        return ret;
    }
...
static int list_plain_entries_help(cls_method_context_t hctx, const std::string & name_filter, const std::string & start_after_key, // exclusive
                                   const std::string & end_key, // exclusive
                                   uint32_t max,
                                   std::list < rgw_cls_bi_entry > *entries,
                                   bool & end_key_reached, bool & more)
{
    CLS_LOG(10,
            "Entered %s: name_filter=\"%s\", start_after_key=\"%s\", end_key=\"%s\", max=%d",
            __func__, escape_str(name_filter).c_str(),
            escape_str(start_after_key).c_str(), escape_str(end_key).c_str(),
            max);
    int count = 0;
...
static int list_plain_entries(cls_method_context_t hctx,
                              ...
                              std::list < rgw_cls_bi_entry > *entries,
                              bool * pmore,
                              const PlainEntriesRegion region =
                              PlainEntriesRegion::Both)
{
    ...
    if (region <= PlainEntriesRegion::Both && marker < BI_PREFIX_BEGIN) {
        ...
        int r =
            list_plain_entries_help(hctx, name_filter, marker, BI_PREFIX_BEGIN,
                                    max,
    ...
    if (region >= PlainEntriesRegion::Both) {
        ...
        r = list_plain_entries_help(hctx, name_filter, start_after_key, {
                                    }, max, entries, end_key_reached, more);
        CLS_LOG(20,
                                 ...
                                 bool * pmore)
{
    ...
    if (!name.empty()) {
        ...
        start_after_key.
            append(bucket_index_prefixes[BI_BUCKET_OBJ_INSTANCE_INDEX]);
    }
    ...
    for (auto iter = keys.begin(); iter != keys.end(); ++iter) {
        ...
        try {
            ...
            CLS_LOG(0, "ERROR: %s: failed to decode buffer (size=%d)", __func__,
                    entry.data.length());
            return -EIO;
        }
                          ...
                          bufferlist * in, bufferlist * out)
{
    ...
    CLS_LOG(20, "%s: op.marker=\"%s\", op.name_filter=\"%s\", op.max=%u max=%u",
            ...
            escape_str(op.name_filter).c_str(), op.max, max);
    ...
    count = ret;
    CLS_LOG(20, "%s: found %d plain ascii (low) entries, count=%u", __func__,
            ret, count);

    if (!more) {
        ret =
            list_instance_entries(hctx, op.name_filter, op.marker, max - count,
                                  &op_ret.entries, &more);
        if (ret < 0) {
        ...
        count += ret;
        CLS_LOG(20, "%s: found %d instance entries, count=%u", __func__, ret,
                count);
    }

    if (!more) {
        ret =
            list_olh_entries(hctx, op.name_filter, op.marker, max - count,
                             &op_ret.entries, &more);
        if (ret < 0) {
            CLS_LOG(0, "ERROR: %s: list_olh_entries returned ret=%d", __func__,
                    ret);
            return ret;
        }
    ...
    CLS_LOG(20, "%s: returning %lu entries, is_truncated=%d", __func__,
                                             ...
                                             void *), void *param)
{
    ...
    for (; iter != keys.end(); ++iter, ++i) {
        ...
        CLS_LOG(10, "bi_log_iterate_entries key=%s bl.length=%d", key.c_str(),
                           ...
                           bufferlist * out)
{
    ...
    if (key_end < first_key) {
        CLS_LOG(20, "listed key %s past key_end=%s", first_key.c_str(),
                key_end.c_str());
        return -ENODATA;
    }
...
static void usage_record_prefix_by_user(const string & user, uint64_t epoch,
                                        string & key)
{
...
static int rgw_user_usage_log_add(cls_method_context_t hctx, bufferlist * in,
                                  bufferlist * out)
{
    CLS_LOG(10, "entered %s", __func__);
...
static int usage_iterate_range(cls_method_context_t hctx, uint64_t start,
                               uint64_t end, const string & user,
                               const string & bucket, string & key_iter,
...
#define MAX_ENTRIES 1000
    ...
    int ret =
        usage_iterate_range(hctx, op.start_epoch, op.end_epoch, op.owner,
                            op.bucket, iter, max_entries, &ret_info.truncated,
...
#define MAX_USAGE_TRIM_ENTRIES 1000
    ret =
        usage_iterate_range(hctx, op.start_epoch, op.end_epoch, op.user,
                            op.bucket, iter, MAX_USAGE_TRIM_ENTRIES, &more,
                            usage_log_trim_cb, (void *)&found);
    if (ret < 0)
        return ret;
...
int rgw_usage_log_clear(cls_method_context_t hctx, bufferlist * in,
                        bufferlist * out)
{
    CLS_LOG(10, "entered %s", __func__);
...
static int rgw_cls_lc_get_entry(cls_method_context_t hctx, bufferlist * in,
                                bufferlist * out)
{
    CLS_LOG(10, "entered %s", __func__);
    auto in_iter = in->cbegin();
    ...
    for (auto it = vals.begin(); it != vals.end(); ++it) {
        ...
        try {
            ...
            try {
                ...
                decode(oe, iter);
                entry = {
                oe.first, 0 /* start */ , uint32_t(oe.second)};
            } catch(buffer::error & err) {
                CLS_LOG(1,
...
static int rgw_mp_upload_part_info_update(cls_method_context_t hctx,
                                          bufferlist * in, bufferlist * out)
{
    CLS_LOG(10, "entered %s", __func__);
    ...
    CLS_LOG(10, "part info update on key [%s]: %zu past prefixes, ret %d",
...
CLS_INIT(rgw)
{
    ...
    cls_register_cxx_method(h_class, RGW_BI_LOG_RESYNC,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_resync,
                            &h_rgw_bi_log_resync_op);
    cls_register_cxx_method(h_class, RGW_BI_LOG_STOP,
    ...
    cls_register_cxx_method(h_class, RGW_USAGE_LOG_CLEAR, CLS_METHOD_WR,
                            rgw_usage_log_clear, &h_rgw_usage_log_clear);
    ...
    cls_register_cxx_method(h_class, RGW_LC_GET_ENTRY, CLS_METHOD_RD,
                            rgw_cls_lc_get_entry, &h_rgw_lc_get_entry);
    ...
    /* multipart */
    cls_register_cxx_method(h_class, RGW_MP_UPLOAD_PART_INFO_UPDATE,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_mp_upload_part_info_update,
                            &h_rgw_mp_upload_part_info_update);

    /* resharding */
    cls_register_cxx_method(h_class, RGW_RESHARD_ADD,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_reshard_add,
                            &h_rgw_reshard_add);
    cls_register_cxx_method(h_class, RGW_RESHARD_LIST, CLS_METHOD_RD,
                            rgw_reshard_list, &h_rgw_reshard_list);
    cls_register_cxx_method(h_class, RGW_RESHARD_GET, CLS_METHOD_RD,
                            rgw_reshard_get, &h_rgw_reshard_get);
    cls_register_cxx_method(h_class, RGW_RESHARD_REMOVE,
                            CLS_METHOD_RD | CLS_METHOD_WR, rgw_reshard_remove,
                            &h_rgw_reshard_remove);

    /* resharding attribute  */
    cls_register_cxx_method(h_class, RGW_SET_BUCKET_RESHARDING,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_set_bucket_resharding,
                            &h_rgw_set_bucket_resharding);
    cls_register_cxx_method(h_class, RGW_CLEAR_BUCKET_RESHARDING,
                            CLS_METHOD_RD | CLS_METHOD_WR,
