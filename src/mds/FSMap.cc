    ...
    if (f) {
        ...
        if (!by_rank.empty()) {
            if (filesystems.size() > 1) {
              ...
              for (auto i:by_rank) {
                    const auto & fs_name =
                        filesystems.at(i.first.fscid)->mds_map.fs_name;
                    std::ostringstream o;
                       ...
                       string > >*detail) const const
{
  ...
  for (const auto & i:filesystems) {
    ...
    if (standby_count_wanted) {
        ...
        oss << "insufficient standby daemons available: have " <<
            standby_daemons.
...
void FSMap::get_health_checks(health_check_map_t * checks) const const
{
  ...
  for (const auto & i:filesystems) {
      ...
      for (const auto & rank:fs->mds_map.failed) {
            const mds_gid_t replacement = find_replacement_for({ fs->fscid,
                                                               rank }, { },
                                                               g_conf->
...
void FSMap::decode(bufferlist::iterator & p)
{
    ...
    if (struct_v < 6) {
        ...
        ::decode(fs_list, p);
        filesystems.clear();
        for (std::vector < Filesystem >::const_iterator fs = fs_list.begin();
             fs != fs_list.end(); ++fs) {
...
int FSMap::parse_filesystem(boost::string_view ns_str,
...
mds_gid_t FSMap::find_standby_for(mds_role_t role, boost::string_view name) const const
{
  ...
  for (const auto & i:standby_daemons) {
        ...
        else if (info.standby_for_rank < 0
                 && info.standby_for_name.length() == 0
                 && (info.standby_for_fscid == FS_CLUSTER_ID_NONE
...
mds_gid_t FSMap::find_unused_for(mds_role_t role, bool force_standby_active) const const
{
  for (const auto & i:standby_daemons) {
        const auto & gid = i.first;
        const auto & info = i.second;
...
mds_gid_t FSMap::find_replacement_for(mds_role_t role, boost::string_view name,
                                      bool force_standby_active) const const
{
...
void FSMap::sanity() const const
{
  ...
  for (const auto & i:mds_roles) {
        if (i.second == FS_CLUSTER_ID_NONE) {
            ...
            assert(filesystems.at(i.second)->mds_map.mds_info.count(i.first) ==
                   1);
        }
    }
}
                    ...
                    mds_rank_t assigned_rank)
{
    ...
    if (!is_standby_replay) {
        ...
        assert(mds_map.mds_info.at(standby_gid).state ==
 ...
 */
int FSMap::parse_role(boost::string_view role_str,
