...
template < class T > inline void decode_raw(T & t, bufferlist::iterator & p)
{
...
WRITE_RAW_ENCODER(ceph_le32) WRITE_RAW_ENCODER(ceph_le16)
// FIXME: we need to choose some portable floating point encoding here
WRITE_RAW_ENCODER(float) WRITE_RAW_ENCODER(double)
...
inline void encode(const std::string & s, bufferlist & bl, uint64_t features =
                   0)
{
...
// full bl decoder
template < class T > inline void decode(T & o, bufferlist & bl)
{
...
template < class A, class B,
    typename a_traits = denc_traits < A >, typename b_traits =
    denc_traits < B >> inline typename std::enable_if < !a_traits::supported
    || !b_traits::supported >::type encode(const std::pair < A, B > &p,
...
template < class A, class B,
    typename a_traits = denc_traits < A >, typename b_traits =
    denc_traits < B >> inline typename std::enable_if < !a_traits::supported
    || !b_traits::supported >::type decode(std::pair < A, B > &pa,
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type encode(const std::set < T, Comp, Alloc > &s,
                                      bufferlist & bl)
{
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type decode(std::set < T, Comp, Alloc > &s,
                                      bufferlist::iterator & p)
{
    __u32 n;
    decode(n, p);
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type encode_nohead(const std::set < T, Comp,
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type decode_nohead(int len, std::set < T, Comp,
                                             Alloc > &s,
                                             bufferlist::iterator & p)
{
    for (int i = 0; i < len; i++) {
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type encode(const boost::container::flat_set < T,
                                      Comp, Alloc > &s, bufferlist & bl)
{
    __u32 n = (__u32) (s.size());
    encode(n, bl);
  for (const auto & e:s)
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type decode(boost::container::flat_set < T, Comp,
                                      Alloc > &s, bufferlist::iterator & p)
{
    __u32 n;
    decode(n, p);
    s.clear();
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type encode_nohead(const boost::container::flat_set <
                                             T, Comp, Alloc > &s,
                                             bufferlist & bl)
{
  for (const auto & e:s)
...
template < class T, class Comp, class Alloc, typename traits =
    denc_traits < T >> inline typename std::enable_if <
    !traits::supported >::type decode_nohead(int len,
                                             boost::container::flat_set < T,
                                             Comp, Alloc > &s,
                                             bufferlist::iterator & p)
{
...
template < class T, class U, class Comp, class Alloc,
    ...
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type encode(const std::map < T, U, Comp,
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type decode(std::map < T, U, Comp, Alloc > &m,
                                           bufferlist::iterator & p)
{
    __u32 n;
    decode(n, p);
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type encode_nohead(const std::map < T, U, Comp,
                                                  Alloc > &m, bufferlist & bl)
{
    for (auto p = m.begin(); p != m.end(); ++p) {
        encode(p->first, bl);
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type decode_nohead(int n, std::map < T, U, Comp,
                                                  Alloc > &m,
                                                  bufferlist::iterator & p)
{
    m.clear();
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type encode(const boost::container::flat_map < T,
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type decode(boost::container::flat_map < T, U,
                                           Comp, Alloc > &m,
                                           bufferlist::iterator & p)
{
    __u32 n;
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type encode_nohead(const boost::container::
                                                  flat_map < T, U, Comp,
                                                  Alloc > &m, bufferlist & bl)
{
    for (auto p = m.begin(); p != m.end(); ++p) {
...
template < class T, class U, class Comp, class Alloc,
    typename t_traits = denc_traits < T >, typename u_traits =
    denc_traits < U >> inline typename std::enable_if < !t_traits::supported
    || !u_traits::supported >::type decode_nohead(int n,
...
template < class T, class Hash, class Pred, class Alloc >
    inline void encode(const ceph::unordered_set < T, Hash, Pred, Alloc > &m,
                       bufferlist & bl)
{
    __u32 n = (__u32) (m.size());
    encode(n, bl);
