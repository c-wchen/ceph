...
namespace dpdk {
    ...
    static inline std::vector < char >string2vector(std::string str) {
        ...
        return std::bitset < CHAR_BIT * sizeof(n) > {
        n}
        .count();
    }
    ...
    int eal::start() {
        ...
        if (coremaskbit <= 0
            || static_cast < uint64_t > (coremaskbit) <
        ...
        t = std::thread([&]() {
                        ...
                        string2vector("ceph"),
                        string2vector("-c"),
                        string2vector(cct->_conf.get_val < std::string >
                                      ("ms_dpdk_coremask")),
                        string2vector("-n"),
                        ...
                        if (!cct->_conf->ms_dpdk_pmd.empty()) {
                        args.push_back(string2vector("--no-huge"));}

                        for_each_pair(cct->_conf.get_val < std::string >
                                      ("ms_dpdk_devs_allowlist"), " ",
                                      [&args] (std::string_view key,
                                               std::string_view val) {
                                      args.
                                      push_back(string2vector
                                                (std::string(key)));
                                      if (!val.empty()) {
                                      args.
                                      push_back(string2vector
      ...
      ); std::string rte_file_prefix; rte_file_prefix = "rte_"; rte_file_prefix += cct->_conf->name.to_str(); args.push_back(string2vector("--file-prefix")); args.push_back(string2vector(rte_file_prefix)); std::vector < char *>cargs; for (auto && a:args)
                        {
                        ...
                        done = true; cond.notify_all(); return ret;}
                        rte_initialized = true;}

                        std::unique_lock locker(lock);
                        initialized = true;
                        done = true; cond.notify_all(); while (!stopped) {
                        cond.wait(locker,[this] {
                                  return !funcs.empty() || stopped;}
                        ); if (!funcs.empty()) {
                        auto f = std::move(funcs.front());
                        funcs.pop_front(); f(); cond.notify_all();}
                        }
                        }
        ) ;
        std::unique_lock locker(lock);
