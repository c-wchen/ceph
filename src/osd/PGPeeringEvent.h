...
struct PGCreateInfo {
    ...
    PGCreateInfo(spg_t p, epoch_t e,
                 const pg_history_t &h, const PastIntervals &pi, bool mon)
...
public:
    ...
    template < class T > PGPeeringEvent(epoch_t epoch_sent,
                                        epoch_t epoch_requested,
                                        const T &evt_, bool req = true, PGCreateInfo *ci = 0)
        :
        epoch_sent(epoch_sent),
        epoch_requested(epoch_requested),
        evt(evt_.intrusive_from_this()), requires_pg(req), create_info(ci)
    {
...
struct MNotifyRec: boost::statechart::event < MNotifyRec > {
    ...
    {
        *out << "MNotifyRec " << pgid << " from " << from << " notify: " <<
             notify << " features: 0x" << std::hex << features << std::dec;
    }
};
...
struct MQuery: boost::statechart::event < MQuery > {
    ...
    pg_query_t query;
    epoch_t query_epoch;
    MQuery(spg_t p, pg_shard_t from, const pg_query_t &query,
           epoch_t query_epoch)
...
struct MTrim: boost::statechart::event < MTrim > {
    ...
    {
        *out << "MTrim epoch " << epoch << " from " << from << " shard " <<
             shard << " trim_to " << trim_to;
    }
};
...
struct MLease: boost::statechart::event < MLease > {
    ...
    {
        *out << "MLease epoch " << epoch << " from osd." << from << " " <<
             lease;
    }
};
...
struct RequestBackfillPrio: boost::statechart::event < RequestBackfillPrio > {
    ...
    explicit RequestBackfillPrio(unsigned prio, int64_t pbytes,
                                 int64_t lbytes): boost::statechart::event <
        RequestBackfillPrio > (), priority(prio), primary_num_bytes(pbytes),
...
struct RequestRecoveryPrio: boost::statechart::event < RequestRecoveryPrio > {
    unsigned priority;
    explicit RequestRecoveryPrio(unsigned prio): boost::statechart::event <
        RequestRecoveryPrio > (), priority(prio)
    {
    } void print(std::ostream *out) const
    {
