...
RGWAsyncRadosProcessor::RGWAsyncRadosProcessor(CephContext * _cct,
                                               int num_threads)
:cct(_cct), m_tp(cct, "RGWAsyncRadosProcessor::m_tp", "rados_async",
                 num_threads), req_throttle(_cct, "rgw_async_rados_ops",
                                            num_threads * 2), req_wq(this,
                                                                     ceph::
                                                                     make_timespan
                                                                     (g_conf()->
...
void RGWAsyncRadosProcessor::handle_request(const DoutPrefixProvider * dpp,
...
RGWAsyncGetSystemObj::RGWAsyncGetSystemObj(const DoutPrefixProvider * _dpp,
                                           RGWCoroutine * caller,
...
:  RGWAsyncRadosRequest(caller, cn), dpp(_dpp), svc_sysobj(_svc), obj(_obj),
want_attrs(want_attrs), raw_attrs(raw_attrs)
{
    if (_objv_tracker) {
...
:  RGWAsyncRadosRequest(caller, cn), dpp(_dpp), svc(_svc), obj(_obj),
exclusive(_exclusive), bl(std::move(_bl))
{
    if (_objv_tracker) {
        objv_tracker = *_objv_tracker;
    }
...
RGWAsyncPutSystemObjAttrs::RGWAsyncPutSystemObjAttrs(const DoutPrefixProvider *
                                                     _dpp,
                                                     RGWCoroutine * caller,
                                                     RGWAioCompletionNotifier *
                                                     cn, RGWSI_SysObj * _svc,
...
RGWOmapAppend::RGWOmapAppend(RGWAsyncRadosProcessor * _async_rados,
                             rgw::sal::RadosStore * _store,
                             const rgw_raw_obj & _obj, uint64_t _window_size)
:RGWConsumerCR < string > (_store->ctx()), async_rados(_async_rados),
store(_store), obj(_obj), going_down(false), num_pending_entries(0),
window_size(_window_size), total_entries(0)
{
}
...
RGWAsyncLockSystemObj::RGWAsyncLockSystemObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn, rgw::sal::RadosStore * _store, RGWObjVersionTracker * _objv_tracker, const rgw_raw_obj & _obj, const string & _name, const string & _cookie, uint32_t _duration_secs):RGWAsyncRadosRequest(caller, cn), store(_store),
obj(_obj),
lock_name(_name), cookie(_cookie), duration_secs(_duration_secs)
{
}
...
RGWAsyncUnlockSystemObj::RGWAsyncUnlockSystemObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn, rgw::sal::RadosStore * _store, RGWObjVersionTracker * _objv_tracker, const rgw_raw_obj & _obj, const string & _name, const string & _cookie):RGWAsyncRadosRequest(caller, cn), store(_store),
obj(_obj),
...
RGWRadosSetOmapKeysCR::RGWRadosSetOmapKeysCR(rgw::sal::RadosStore * _store,
                                             ...
                                             map < string,
                                             bufferlist >
                                             &_entries):RGWSimpleCoroutine
    (_store->ctx()), store(_store), entries(_entries), obj(_obj), cn(NULL)
{
...
marker(_marker), max_entries(_max_entries), result(std::move(_result))
{
...
int RGWRadosGetOmapKeysCR::send_request(const DoutPrefixProvider * dpp)
{
    ...
    librados::ObjectReadOperation op;
    op.omap_get_keys2(marker, max_entries, &result->entries, &result->more,
                      nullptr);

    cn = stack->create_completion_notifier(result);
    return result->ref.pool.ioctx().aio_operate(result->ref.obj.oid,
                                                cn->completion(), &op, NULL);
}
...
int RGWRadosGetOmapValsCR::send_request(const DoutPrefixProvider * dpp)
{
    ...
    librados::ObjectReadOperation op;
    op.omap_get_vals2(marker, max_entries, &result->entries, &result->more,
                      nullptr);
...
RGWRadosRemoveOmapKeysCR::RGWRadosRemoveOmapKeysCR(rgw::sal::RadosStore * _store, const rgw_raw_obj & _obj, const set < string > &_keys):RGWSimpleCoroutine(_store->ctx()),
store(_store),
keys(_keys), obj(_obj), cn(NULL)
{
...
int RGWRadosRemoveCR::send_request(const DoutPrefixProvider * dpp)
{
    ...
    if (r < 0) {
        lderr(cct) << "ERROR: failed to open pool (" << obj.pool.
            name << ") ret=" << r << dendl;
        return r;
    }
...
oid(obj.get_ref().obj.oid), objv_tracker(objv_tracker)
{
    set_description() << "remove dest=" << oid;
}
...
oid(std::move(obj.get_ref().obj.oid)), objv_tracker(objv_tracker)
{
    set_description() << "remove dest=" << oid;
}
...
RGWSimpleRadosLockCR::RGWSimpleRadosLockCR(RGWAsyncRadosProcessor * _async_rados, rgw::sal::RadosStore * _store, const rgw_raw_obj & _obj, const string & _lock_name, const string & _cookie, uint32_t _duration):RGWSimpleCoroutine(_store->ctx()),
async_rados(_async_rados),
store(_store),
lock_name(_lock_name),
cookie(_cookie), duration(_duration), obj(_obj), req(NULL)
{
...
RGWSimpleRadosUnlockCR::RGWSimpleRadosUnlockCR(RGWAsyncRadosProcessor * _async_rados, rgw::sal::RadosStore * _store, const rgw_raw_obj & _obj, const string & _lock_name, const string & _cookie):RGWSimpleCoroutine(_store->ctx()),
async_rados(_async_rados),
store(_store), lock_name(_lock_name), cookie(_cookie), obj(_obj), req(NULL)
{
...
int RGWAsyncGetBucketInstanceInfo::_send_request(const DoutPrefixProvider * dpp)
{
    ...
    if (!bucket.bucket_id.empty()) {
        r = store->getRados()->get_bucket_instance_info(bucket, bucket_info,
                                                        nullptr, &attrs,
                                                        null_yield, dpp);
    }
    else {
        r = store->ctl()->bucket->read_bucket_info(bucket, &bucket_info,
                                                   null_yield, dpp,
                                                   RGWBucketCtl::
...
int RGWRadosBILogTrimCR::send_request(const DoutPrefixProvider * dpp)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  -1) << "ERROR: bucket shard init failed ret=" << r << dendl;
        return r;
    }
...
int RGWAsyncFetchRemoteObj::_send_request(const DoutPrefixProvider * dpp)
{
    ...
    snprintf(buf, sizeof(buf), ".%lld",
             (long long)store->getRados()->instance_id());
    rgw::sal::Attrs attrs;
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "store->fetch_remote_obj() returned r=" << r << dendl;
        if (counters) {
        ...
        if (bytes_transferred) {
            ...
            if (ret < 0) {
                ...
                if (ret < 0) {
                    ldpp_dout(dpp,
                              1) <<
                        "ERROR: publishing notification failed, with error: " <<
                        ret << dendl;
                }
            }
        }
...
int RGWAsyncStatRemoteObj::_send_request(const DoutPrefixProvider * dpp)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "store->stat_remote_obj() returned r=" << r << dendl;
    }
    return r;
}
...
int RGWAsyncRemoveObj::_send_request(const DoutPrefixProvider * dpp)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  20) << __func__ << "(): get_obj_state() obj=" << obj <<
    ...
    if (del_if_older && (state->mtime > timestamp)) {
        ldpp_dout(dpp,
                  20) << __func__ << "(): skipping object removal obj=" << obj
            << " (obj mtime=" << state->
            mtime << ", request timestamp=" << timestamp << ")" << dendl;
        return 0;
    }
    ...
    map < string, bufferlist >::iterator iter =
        state->attrset.find(RGW_ATTR_ACL);
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  20) << __func__ << "(): delete_obj() obj=" << obj <<
            " returned ret=" << ret << dendl;
    }
    return ret;
}
...
int RGWContinuousLeaseCR::operate(const DoutPrefixProvider * dpp)
{
    ...
    reenter(this) {
        ...
        while (!going_down) {
            ...
            yield call(new
                       RGWSimpleRadosLockCR(async_rados, store, obj, lock_name,
                                            cookie, interval));
            if (latency) {
                latency->add_latency(ceph::coarse_mono_clock::now() -
                                     current_time);
            }
            current_time = ceph::coarse_mono_clock::now();
            if (current_time - last_renew_try_time > interval_tolerance) {
                ...
                ldout(store->ctx(),
                      1) << *this << ": WARNING: did not renew lock " << obj <<
                    ":" << lock_name << ": within 90\% of interval. " <<
                    (current_time -
                     last_renew_try_time) << " > " << interval_tolerance <<
                    dendl;
            }
            ...
            if (retcode < 0) {
                set_locked(false);
                ldout(store->ctx(),
                      20) << *this << ": couldn't lock " << obj << ":" <<
                    lock_name << ": retcode=" << retcode << dendl;
                return set_state(RGWCoroutine_Error, retcode);
            }
            ldout(store->ctx(),
                  20) << *this << ": successfully locked " << obj << ":" <<
                lock_name << dendl;
            set_locked(true);
        ...
        current_time = ceph::coarse_mono_clock::now();
        yield call(new
                   RGWSimpleRadosUnlockCR(async_rados, store, obj, lock_name,
                                          cookie));
        if (latency) {
...
RGWRadosTimelogAddCR::RGWRadosTimelogAddCR(const DoutPrefixProvider * _dpp, rgw::sal::RadosStore * _store, const string & _oid, const cls_log_entry & entry):RGWSimpleCoroutine(_store->ctx()),
dpp(_dpp),
store(_store), oid(_oid), cn(NULL)
{
    stringstream & s = set_description();
    s << "timelog add entry oid=" << oid << "entry={id=" << entry.
        id << ", section=" << entry.section << ", name=" << entry.name << "}";
...
int RGWRadosTimelogAddCR::send_request(const DoutPrefixProvider * dpp)
{
    ...
    cn = stack->create_completion_notifier();
    return store->svc()->cls->timelog.add(dpp, oid, entries, cn->completion(),
                                          true, null_yield);
}
...
int RGWRadosTimelogTrimCR::send_request(const DoutPrefixProvider * dpp)
{
    ...
    cn = stack->create_completion_notifier();
    return store->svc()->cls->timelog.trim(dpp, oid, start_time, end_time,
                                           from_marker, to_marker,
                                           cn->completion(), null_yield);
}
...
:  RGWRadosTimelogTrimCR(dpp, store, oid, real_time
                          {
                          }, real_time {
                          }, std::string {
                          }, to_marker),
...
async_rados(async_rados), bucket_info(_bucket_info), obj(obj), psize(psize),
...
int RGWStatObjCR::send_request(const DoutPrefixProvider * dpp)
{
    req = new RGWAsyncStatObj(dpp, this, stack->create_completion_notifier(),
                              store, bucket_info, obj, psize, pmtime, pepoch,
                              objv_tracker);
    async_rados->queue(req);
    return 0;
}
...
RGWRadosNotifyCR::RGWRadosNotifyCR(rgw::sal::RadosStore * store,
                                   const rgw_raw_obj & obj,
                                   bufferlist & request, uint64_t timeout_ms,
                                   bufferlist * response)
:  RGWSimpleCoroutine(store->ctx()), store(store), obj(obj), request(request),
timeout_ms(timeout_ms), response(response)
{
...
int RGWDataPostNotifyCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        using PostNotify2 =
            RGWPostRESTResourceCR < bc::flat_map < int,
            bc::flat_set < rgw_data_notify_entry >>, int >;
        yield {
            ...
            call(new
                 PostNotify2(store->ctx(), conn, &http_manager, "/admin/log",
                             pairs, shards, nullptr));
        }
        if (retcode == -ERR_METHOD_NOT_ALLOWED) {
            using PostNotify1 =
                RGWPostRESTResourceCR < rgw_data_notify_v1_encoder, int >;
            yield {
                rgw_http_param_pair pairs[] = { {"type", "data"},
                {"notify", NULL},
                {"source-zone", source_zone},
                ...
                call(new
                     PostNotify1(store->ctx(), conn, &http_manager,
                                 "/admin/log", pairs, encoder, nullptr));
            }
        }
        if (retcode < 0) {
