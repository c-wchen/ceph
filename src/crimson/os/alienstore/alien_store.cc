...
public:
    OnCommit(int id,
             ...
             seastar::alien::instance &alien, ceph::os::Transaction &txn)
...
seastar::future <> AlienStore::start()
{
    ...
    CephContext::create_options {
        ...
        [](const ceph::logging::
           SubsystemMap * subsys_map) {
            return new ceph::logging::
            CnLog(subsys_map,
    ...
    auto cpu_cores =
        seastar::resource::parse_cpuset(get_conf < std::string >
    ...
    if (!cpu_cores.has_value()) {
        ...
        std::copy(boost::counting_iterator <
...
seastar::future <> AlienStore::stop()
{
    ...
    return tp->submit([this] {
        for (auto[cid, ch] : coll_map) {
            static_cast <
            AlienCollection * >(ch.get())->collection.reset();
        }
        store.reset(); cct.reset(); g_ceph_context = nullptr;
    }
                     )
    .then([this] {
        return tp->stop();
...
seastar::future < std::tuple < std::vector < ghobject_t >, ghobject_t >> AlienStore::list_objects(CollectionRef ch,
        ...
        const ghobject_t &end, uint64_t limit)const
{
    logger().debug("{}", __func__);
    assert(tp);
    ...
    [ =, this](auto & objects, auto & next) {
        ...
        return tp->submit(ch->get_cid().
                          hash_to_shard(tp->size()), [ =,
                                  this,
                                  &objects,
        &next] {
            auto c =
            static_cast <
            AlienCollection * >(ch.get());
            return store->
            collection_list(c->collection,
                            start, end,
                            store->
                            get_ideal_list_max
                            (), &objects,
            ...
            return seastar::make_ready_future <
                   std::tuple < std::vector < ghobject_t >,
                   ghobject_t >> (std::move(objects),
                                  std::move(next));
        }
                               );
    }
...
seastar::future < std::vector <
coll_core_t >> AlienStore::list_collections()
{
    logger().debug("{}", __func__);
    assert(tp);

    return do_with_op_gate(std::vector < coll_t > {
    }
    , [this](auto & ls) {
...
AlienStore::read_errorator::future < ceph::bufferlist > AlienStore::read(CollectionRef ch,
...
AlienStore::read_errorator::future < ceph::bufferlist > AlienStore::readv(CollectionRef ch,
        const ghobject_t &oid,
        interval_set < uint64_t > &m, uint32_t op_flags)
{
    ...
    , [this, ch, oid, &m, op_flags](auto & bl) {
        return tp->submit(ch->get_cid().
                          hash_to_shard(tp->size()),
                          [this, ch, oid, &m, op_flags,
        &bl] {
            auto c =
            static_cast <
            AlienCollection * >(ch.get());
            return store->readv(c->
                                collection,
                                oid, m, bl,
                                op_flags);
        }
...
AlienStore::get_attr_errorator::future < ceph::bufferlist > AlienStore::get_attr(CollectionRef ch,
        const ghobject_t &oid,
        std::string_view name)const
{
    ...
    , [ =, this](auto & value, const auto & name) {
        ...
        ceph::bufferlist > {
            ...
            {
                return get_attr_errorator::
                make_ready_future < ceph::bufferlist >
                (std::move(value));
            }
        }
                               );
...
AlienStore::get_attrs_ertr::future < AlienStore::attrs_t > AlienStore::get_attrs(CollectionRef ch,
        const ghobject_t &oid)
{
    logger().debug("{}", __func__);
    assert(tp);
    return do_with_op_gate(attrs_t {
    }
    , [ =, this](auto & aset) {
        return tp->submit(ch->get_cid().
                          hash_to_shard(tp->size()), [ =,
...
->read_errorator::future < omap_values_t >
{
    logger().debug("{}", __func__);
    assert(tp);
    return do_with_op_gate(omap_values_t {
    }
    , [ =, this](auto & values) {
        return tp->submit(ch->get_cid().
                          hash_to_shard(tp->size()), [ =,
        ...
        &values] {
            ...
            omap_get_values(c->collection,
                            ...
                            *>(&values));
                                ...
                                ->read_errorator::future < std::tuple <
        bool, omap_values_t >> {
            if (r == -ENOENT) {
                return crimson::ct_error::enoent::make();
            } else if (r < 0) {
                logger().
                error("omap_get_values(start): {}", r);
                return crimson::ct_error::
                input_output_error::make();
            } else
            {
                return read_errorator::make_ready_future <
                std::tuple < bool, omap_values_t >> (true,
                                                     std::
                                                     move
                                                     (values));
            }
...
seastar::future <> AlienStore::do_transaction_no_callbacks(CollectionRef ch,
        ceph::os::
        Transaction
        && txn)
{
    ...
    return do_with_op_gate(std::move(txn),
                           std::move(done),
    [this, ch, id](auto & txn, auto & done) {
        AlienCollection *alien_coll =
            static_cast < AlienCollection * >(ch.get());
        // moving the `ch` is crucial for buildability on newer S* versions.
        return alien_coll->
               with_lock([this, ch =
        std::move(ch), id, &txn, &done] {
            assert(tp);
            return tp->submit(ch->get_cid().
                              hash_to_shard(tp->
                                            size
...
} seastar::future < store_statfs_t > AlienStore::stat() const
{
    logger().info("{}", __func__);
    assert(tp);
    return do_with_op_gate(store_statfs_t {
    }
    , [this](store_statfs_t &st) {
        return tp->submit([this, &st] {
            return store->statfs(&st,
                                 nullptr);
        }
        ).then([&st](int r) {
            assert(r == 0);
            return seastar::make_ready_future <
                   store_statfs_t > (std::move(st));
        }
                               );
    }
                          );
...
unsigned AlienStore::get_max_attr_name_length() const
{
    logger().info("{}", __func__);
    return 256;
} seastar::future < struct stat >AlienStore::stat(CollectionRef ch,
        const ghobject_t &oid)
{
    assert(tp);
    return do_with_op_gate((struct stat) {
    }, [this, ch, oid](auto & st) {
        return tp->submit(ch->get_cid().
                          hash_to_shard(tp->size()),
        [this, ch, oid, &st] {
...
AlienStore::read_errorator::future < std::map < uint64_t,
uint64_t >> AlienStore::fiemap(CollectionRef ch, const ghobject_t &oid,
                               uint64_t off, uint64_t len)
{
    assert(tp);
    return do_with_op_gate(std::map < uint64_t, uint64_t > (),
    [ =, this](auto & destmap) {
        return tp->submit(ch->get_cid().
                          hash_to_shard(tp->size()), [ =,
                              this,
