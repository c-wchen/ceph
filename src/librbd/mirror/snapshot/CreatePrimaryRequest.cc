    ...
    namespace mirror {
        namespace snapshot {
            ...
            using librbd::util::create_rados_callback;
             ...
             template < typename I >
                CreatePrimaryRequest < I >::CreatePrimaryRequest(I * image_ctx,
                                                                 const std::
                                                                 string &
                                                                 global_image_id,
                                                                 uint64_t
            ...
            :m_image_ctx(image_ctx), m_global_image_id(global_image_id),
                ...
                librados::AioCompletion * comp = create_rados_callback <
                    ...
                    &CreatePrimaryRequest < I >::handle_get_mirror_peers >
                    (this);
                m_out_bl.clear();
                int r =
                    m_default_ns_ctx.aio_operate(RBD_MIRRORING, comp, &op,
                                                 &m_out_bl);
                ceph_assert(r == 0);
                comp->release();
            }
            ...
            template < typename I >
              ...
              for (auto & peer:peers) {
                ...
                if (m_mirror_peer_uuids.empty() &&
                    ...
                    lderr(cct) << "no mirror tx peers configured for the pool"
                        << dendl;
                    finish(-EINVAL);
                    return;
                }
...
m_mirror_peer_uuids, "", m_clean_since_snap_id};
                ...
                auto ctx = create_context_callback <
                    CreatePrimaryRequest < I >,
                    &CreatePrimaryRequest < I >::handle_create_snapshot >
                    (this);
                m_image_ctx->operations->snap_create(ns, m_snap_name,
                                                     m_snap_create_flags,
                                                     m_prog_ctx, ctx);
            }
            ...
            template < typename I >
              ...
              for (auto & peer:m_mirror_peer_uuids) {
                ...
                CephContext *cct = m_image_ctx->cct;
                ldout(cct,
                      15) << "peer=" << peer_uuid << ", snap_id=" << snap_id <<
                    dendl;
                ...
                auto req =
                    UnlinkPeerRequest < I >::create(m_image_ctx, snap_id,
                                                    peer_uuid, true,
                                                    ctx);
                req->send();
            }
