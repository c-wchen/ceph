...
struct ReadLatencyMetric {
    ...
    void dump(Formatter * f) const {
        ...
        f->dump_unsigned("count", count);
                                       ...
                                       const ReadLatencyMetric & metric) {
        os << "{latency=" << metric.lat << ", avg_latency=" << metric.
            mean << ", sq_sum=" << metric.sq_sum << ", count=" << metric.
            count << "}";
        return os;
    }
};
...
struct WriteLatencyMetric {
    ...
    void dump(Formatter * f) const {
        ...
        f->dump_unsigned("count", count);
                                       ...
                                       const WriteLatencyMetric & metric) {
        os << "{latency=" << metric.lat << ", avg_latency=" << metric.
            mean << ", sq_sum=" << metric.sq_sum << ", count=" << metric.
            count << "}";
        return os;
...
struct MetadataLatencyMetric {
    ...
    void dump(Formatter * f) const {
        ...
        f->dump_unsigned("count", count);
                                       ...
                                       const MetadataLatencyMetric & metric) {
        os << "{latency=" << metric.lat << ", avg_latency=" << metric.
            mean << ", sq_sum=" << metric.sq_sum << ", count=" << metric.
            count << "}";
        return os;
    ...
    bool updated = false;
     ...
     DENC(DentryLeaseHitMetric, v, p) {
        ...
        f->dump_unsigned("misses", misses);
                                       ...
                                       const DentryLeaseHitMetric & metric) {
        os << "{hits=" << metric.hits << ", misses=" << metric.misses << "}";
        return os;
    }
};
    ...
    bool updated = false;
     ...
     DENC(OpenedFilesMetric, v, p) {
        ...
        f->dump_unsigned("total_inodes", total_inodes);
                                       ...
                                       const OpenedFilesMetric & metric) {
        os << "{opened_files=" << metric.
            opened_files << ", total_inodes=" << metric.total_inodes << "}";
        return os;
    }
};
    ...
    bool updated = false;
     ...
     DENC(PinnedIcapsMetric, v, p) {
        ...
        f->dump_unsigned("total_inodes", total_inodes);
                                       ...
                                       const PinnedIcapsMetric & metric) {
        os << "{pinned_icaps=" << metric.
            pinned_icaps << ", total_inodes=" << metric.total_inodes << "}";
        return os;
    }
};
    ...
    bool updated = false;
     ...
     DENC(OpenedInodesMetric, v, p) {
        ...
        f->dump_unsigned("total_inodes", total_inodes);
                                       ...
                                       const OpenedInodesMetric & metric) {
        os << "{opened_inodes=" << metric.
            opened_inodes << ", total_inodes=" << metric.total_inodes << "}";
        return os;
    }
};
    ...
    bool updated = false;
     ...
     DENC(ReadIoSizesMetric, v, p) {
        ...
        f->dump_unsigned("total_size", total_size);
                                       ...
                                       const ReadIoSizesMetric & metric) {
        os << "{total_ops=" << metric.total_ops << ", total_size=" << metric.
            total_size << "}";
        return os;
    }
};
    ...
    bool updated = false;
     ...
     DENC(WriteIoSizesMetric, v, p) {
        ...
        f->dump_unsigned("total_size", total_size);
                                       ...
                                       const WriteIoSizesMetric & metric) {
        os << "{total_ops=" << metric.total_ops << ", total_size=" << metric.
            total_size << "}";
        return os;
    }
...
void dump(Formatter * f) const {
    ...
    for (auto &[client, metrics]:client_metrics_map) {
        ...
        f->dump_object("metrics", metrics);
                                    ...
                                    const metrics_message_t & metrics_message) {
    os << "[sequence=" << metrics_message.seq << ", rank=" << metrics_message.
        rank << ", metrics=" << metrics_message.client_metrics_map << "]";
    return os;
}
};
