...
template < typename Iterator, typename AsyncAction,
           ...
           std::invoke_result_t < AsyncAction,
                                  typename Iterator::
                                  reference >> inline FutureT do_for_each_impl(Iterator begin,
                                          Iterator end,
                                          AsyncAction action)
{
    while (begin != end) {
        ...
        if (!f.available() || seastar::need_preempt()) {
            ...
            auto *s =
                new crimson::do_for_each_state < Iterator, AsyncAction,
            FutureT > {
                std::move(begin), std::move(end), std::move(action),
                std::move(f)
            };
            return s->get_future();
        }
...
template < typename Iterator, typename AsyncAction >
inline auto do_for_each(Iterator begin, Iterator end,
                        AsyncAction action)
{
...
{
    return::crimson::do_for_each(std::begin(c), std::end(c),
                                 std::move(action));
}
...
template < typename AsyncAction > inline auto repeat(AsyncAction action)
{
    using errorator_t =
        ...
        AsyncAction >>::errorator_type;

    while (true) {
        auto f =::seastar::futurize_invoke(action);
        if (f.failed()) {
            return errorator_t::template make_exception_future2 <> (f.
                    get_exception
                    ()
                                                                   );
        } else if (f.available()) {
            ...
            return std::move(f)._then([action =
                                           std::
            move(action)](auto stop) mutable {
                if (stop ==
                    seastar::stop_iteration::yes) {
                    return errorator_t::
                    template make_ready_future <> ();
                }
                return::crimson::repeat(std::
                                        move
                                        (action));}
                                       ) ;
        }
...
protected:
    ...
    {
        const auto *concrete_error =
            static_cast < const ConcreteErrorT * >(this);
...
struct unthrowable_wrapper: error_t < unthrowable_wrapper < ErrorT,
    ErrorV >> {
    ...
    template < class Func > static auto handle(Func && func)
    {
        return[func = std::forward < Func > (func)
            ...
            if constexpr
            (std::is_invocable_v < Func, ErrorT, decltype(raw_error) >) {
                ...
                return std::invoke(std::forward < Func > (func),
                                   ErrorV, std::move(raw_error));
            } else if constexpr
            (std::is_invocable_v < Func, ErrorT >) {
...
template < class ErrorT, ErrorT ErrorV >
std::exception_ptr unthrowable_wrapper < ErrorT,
    ErrorV >::carrier_instance =
        std::make_exception_ptr < unthrowable_wrapper < ErrorT,
ErrorV >::throwable_carrier > ({
}
                                      );

template < class ErrorT >
struct stateful_error_t: error_t < stateful_error_t < ErrorT >> {
    template < class ... Args > explicit stateful_error_t(Args && ... args)
        : ep(std::make_exception_ptr < ErrorT >
    ...
    } template < class Func > static auto handle(Func && func)
    {
        return[func = std::forward < Func > (func)
            ...
            try {
                ...
                if constexpr
                (std::is_invocable_v < Func, decltype(obj),
                 decltype(e) >) {
...
public:
    ...
    template < class ErrorT > void handle()
    {
        ...
        if (type_info == ErrorT::error_t::get_exception_ptr_type_info()) {
            ...
            if constexpr
            (std::is_assignable_v < decltype(result), return_t >) {
                result =
                    std::invoke(std::forward < ErrorVisitorT > (errfunc),
                                ErrorT::error_t::from_exception_ptr(std::
                                    move
                                    (ep)));
            } else if constexpr
            ...
            (seastar::Future < decltype(result) >) {
                ...
                result =
                    seastar::make_ready_future < return_t >
                    (std::
                     invoke(std::forward < ErrorVisitorT > (errfunc),
                            ErrorT::error_t::from_exception_ptr(std::
                                move(ep))));
            } else {
                result =
                    FuturatorT::type::errorator_type::
                    template make_ready_future < return_t >
                (std::
                 invoke(std::forward < ErrorVisitorT > (errfunc),
                        ErrorT::error_t::from_exception_ptr(std::
                                move(ep))));
...
static constexpr auto composer(FuncHead && head, FuncTail && ... tail)
{
    return[head = std::forward < FuncHead > (head),
        ...
        (sizeof ...(FuncTail) > 0) {
            ...
            auto &&
            next =
                std::apply < next_composer_t > (composer < FuncTail ... >,
                                                std::move(tail));
            return std::invoke(std::move(next),
                               std::forward < decltype(args) > (args) ...);
        } else {
            static_assert(std::is_invocable_v < FuncHead,
                          decltype(args) ... > || (sizeof ...(FuncTail) > 0),
...
template < class T >
static inline constexpr bool is_error_v =
    std::is_base_of_v < error_t < T >, T >;

template < typename ... AllowedErrors > struct errorator;

template < typename Iterator, typename Func, typename ... AllowedErrors >
static inline typename errorator <
AllowedErrors ... >::
template future <> parallel_for_each(Iterator first, Iterator last, Func
                                     && func) noexcept;
...
private:
    ...
    template < class ValueT > class[[nodiscard]] _future <::crimson::errorated_future_marker < ValueT >>: private
                seastar::future < ValueT >
    {
        ...
        friend auto seastar::internal::do_with_impl(T1 && rv1, T2
                && rv2, More
                && ... more);
        ...
        struct make_errorator <errorator < ValueFuncAllowedErrors ... >,
                   ErrorVisitorRetsHeadT, ErrorVisitorRetsTailT ... > {
        private:
            ...
            using _next_errorator = std::conditional_t <
                                    ...
                                    &&!step_errorator::template contains_once_v <
                                        ErrorVisitorRetsHeadT >,
                                                              typename step_errorator::template extend <
                                                                      ErrorVisitorRetsHeadT >, step_errorator >;
            using maybe_head_ertr =
                get_errorator_t < ErrorVisitorRetsHeadT >;
            using next_errorator =
                typename _next_errorator::template extend_ertr <
                    maybe_head_ertr >;
        ...
        template < class ... ValueFuncAllowedErrors >
        struct make_errorator <errorator <
            ValueFuncAllowedErrors ... >> {
            using type =::crimson::errorator < ValueFuncAllowedErrors ... >;
        };
        template < class ... Args >
        using make_errorator_t =
            typename make_errorator < Args ... >::type;
        ...
        {
            maybe_handle_error_t < ErrorVisitor,
                                 Futurator > maybe_handle_error(std::forward < ErrorVisitor >
                                         (errfunc),
                                         std::move(future).
                                         get_exception()
                                                               );
    ...
    public:
        ...
        template < class ... A >[[gnu::always_inline]] _future(ready_future_marker, A && ... a)
            :
            base_t(::seastar::make_ready_future < ValueT >
                   (std::forward < A > (a) ...))
        {
        }
        [[gnu::always_inline]]
        _future(exception_future_marker, ::seastar::future_state_base && state) noexcept:
            base_t(::seastar::futurize <
                   base_t >::make_exception_future(std::move(state)))
        {
        }
        [[gnu::always_inline]]
        _future(exception_future_marker, std::exception_ptr && ep) noexcept:
            base_t(::seastar::futurize <
                   base_t >::make_exception_future(std::move(ep)))
        {
        }
        ...
        template < template < class ... >class ErroratedFuture,
                   class = std::void_t <
                       typename ErroratedFuture <::crimson::errorated_future_marker <
                               ValueT >>::errorator_type >> operator  ErroratedFuture <
        errorated_future_marker < ValueT >> () && {
            using dest_errorator_t =
            typename ErroratedFuture <::crimson::
            errorated_future_marker < ValueT >>::errorator_type;
        ...
        template < class ErrorT,
                   ...
                   class = std::enable_if_t < IsError >> _future(ErrorT && e)
            :
        ...
        auto safe_then(ValueFuncT && valfunc, ErrorVisitorT
                       && errfunc)
        {
            ...
            // Function.
            using value_func_errorator_t =
                get_errorator_t < value_func_result_t >;
            ...
            using futurator_t =
                typename return_errorator_t::template futurize <
                    value_func_result_t >;
            ...
            return this->then_wrapped([valfunc =
                                           std::forward < ValueFuncT >
                                           (valfunc), errfunc =
                                           std::forward < ErrorVisitorT >
                                           (errfunc)
            ](auto && future) mutable noexcept {
                if (__builtin_expect
                    (future.failed(), false)) {
                    return _safe_then_handle_errors <
                    futurator_t > (std::move(future),
                                   std::forward <
                                   ErrorVisitorT >
                                   (errfunc));
                } else {
        ...
        {
            return this->then_wrapped([func =
                                           std::forward < FuncT > (func)](auto
                                               &&
                                               result)
            mutable noexcept {
                if constexpr(seastar::
                       ...
                       ErrorFuncTail && ... error_func_tail)
        {
            ...
            return safe_then(std::forward < ValueFunc > (value_func),
                             composer(std::forward < ErrorFuncHead >
                                      (error_func_head),
                                      std::forward < ErrorFuncTail >
                                      (error_func_tail) ...));
        }
        ...
        {
            return safe_then([value_func = std::move(value_func)](ValueT
                             && tuple)
            mutable {
                assert_moveable(value_func);
                return std::apply(std::move(value_func),
                                  std::move(tuple));}
            ,
        ...
        auto handle_error(ErrorVisitorT && errfunc)
        {
            ...
            using return_errorator_t = make_errorator_t <
                                       ...
                                       AllowedErrors >> ... >;
            using futurator_t =
                typename return_errorator_t::template futurize <::seastar::
                    future < ValueT >>;
            return this->
                   then_wrapped([errfunc =
                ...
                if (__builtin_expect(future.failed(), false)) {
                    ...
                    return typename futurator_t::type {
                        std::move(future)};
                }
            }
                               ) ;
                          ...
                          ErrorFuncTail && ... error_func_tail)
        {
            static_assert(sizeof ...(ErrorFuncTail) > 0);
            return this->
                   handle_error(composer
                                (std::forward < ErrorFuncHead >
                                 (error_func_head),
                                 std::forward < ErrorFuncTail >
                                 (error_func_tail) ...));
        }
    ...
    private:
        ...
        template < class Func > auto _then(Func && func)
        {
            return base_t::then(std::forward < Func > (func));
        }
        template < class T > auto _forward_to(T && pr)
        {
        ...
        template < typename Iterator, typename AsyncAction,
                   typename FutureT > friend class::crimson::do_for_each_state;
        ...
        friend inline auto::seastar::internal::do_with_impl(T
                && rvalue, F
                && f);
        template < typename T1, typename T2, typename T3_or_F,
                   typename ... More >
        friend inline auto::seastar::internal::do_with_impl(T1
                && rv1, T2
                && rv2,
                T3_or_F
                && rv3, More
        ...
        template < typename,
                   typename >
        friend class::crimson::interruptible::
            interruptible_future_detail;
        friend class::crimson::parallel_for_each_state <
            AllowedErrors ... >;
    ...
    template < typename ErrorFunc > struct all_same_way_t {
            ...
            : func(std::forward < ErrorFunc > (error_func))
        {
                     ...
                     EnableIf < ErrorT > ... >decltype(auto) operator()(ErrorT
                             && e)
        {
            using decayed_t = std::decay_t < decltype(e) >;
            auto && handler =
                decayed_t::error_t::handle(std::forward < ErrorFunc >
                                           (func));
            static_assert(std::is_invocable_v < decltype(handler),
                          ErrorT >);
...
public:
    ...
    using future =
        _future <::crimson::errorated_future_marker < ValueT >>;
    ...
    struct discard_all {
        template < class ErrorT,
                   EnableIf < ErrorT > ... >void operator()(ErrorT &&)
        {
            static_assert(contains_once_v < std::decay_t < ErrorT >>,
    ...
    template < typename T >
    static future < T > make_errorator_future(seastar::future < T >
            &&fut)
    {
        return std::move(fut);
    }
    ...
    public:
        template < std::size_t N > assert_all(const char (&msg)[N])
            : msg(msg)
        {
        } assert_all() = default;
    ...
    {
        return all_same_way_t < ErrorFunc > {
            std::forward < ErrorFunc > (error_func)
        };
    };
    ...
    using extend =
        errorator < AllowedErrors ..., NewAllowedErrorsT ... >;
    ...
    public:
        using type =
            typename step_errorator::template unify <
                rest_errorator >::type;
    };
    ...
    {
        return future < T > (ready_future_marker(),
                             std::forward < A > (value) ...);
    }
    ...
    template < typename T = void > static
    future < T > make_exception_future2(seastar::future_state_base
                                        && state) noexcept
    {
        return future < T > (exception_future_marker(), std::move(state));
    }
    template < typename T = void, typename Exception > static
    ...
    static inline auto parallel_for_each(Container && container, Func
                                         && func) noexcept
    {
        return crimson::parallel_for_each < decltype(std::begin(container)),
               Func, AllowedErrors ... >(std::begin(container),
                                         std::end(container),
                                         std::forward < Func > (func));
    }

    template < typename Iterator, typename Func >
    static inline errorator < AllowedErrors ... >::future <> parallel_for_each(Iterator first, Iterator last, Func
            && func) noexcept
    {
        return crimson::parallel_for_each < Iterator, Func,
               AllowedErrors ... >(first, last, std::forward < Func > (func));
    }
private:
    ...
    public:
        using type =
            typename stored_to_future <
            typename vanilla_futurize::value_type >::type;

        template < class Func, class ... Args >
        static type invoke(Func && func, Args && ... args)
        {
            try {
                return vanilla_futurize::invoke(std::forward < Func >
                                                (func),
                                                std::forward < Args >
                                                (args) ...);
            } catch (...) {
        ...
        {
            return vanilla_futurize::make_exception_future(std::forward <
                    Arg > (arg));
        }
    };
    ...
    {
    public:
        using type =::crimson::errorator < AllowedErrors ... >::future <
                    ValueT >;
        ...
        {
            try {
                return::seastar::futurize_invoke(std::forward < Func >
                                                 (func),
                                                 std::forward < Args >
                                                 (args) ...);
            } catch (...) {
        ...
        static type make_exception_future(Arg && arg)
        {
            return::crimson::errorator <
                  AllowedErrors ... >::make_exception_future2 < ValueT >
                  (std::forward < Arg > (arg));
        }
    };
    ...
    public:
        ...
        static type invoke(Func && func, Args && ... args)
        {
            try {
                ...
                return seastar::futurize <::crimson::interruptible::
                       interruptible_future_detail < InterruptCond,
                       FutureType >>::make_exception_future(std::
                               current_exception
                               ());
            }
        }
        ...
        static type make_exception_future(Arg && arg)
        {
            return::seastar::futurize <::crimson::interruptible::
                  interruptible_future_detail < InterruptCond,
                  FutureType >>::make_exception_future(std::forward < Arg >
                          (arg));
        }
    };
...
template < class ErroratorOne,
           class ErroratorTwo,
           class ... FurtherErrators > struct compound_errorator {
private:
...
template < class ... Args >
using compound_errorator_t =
    typename compound_errorator < Args ... >::type;
...
inline std::error_code ec =
    std::error_code(ErrorV, std::generic_category());

template < int ErrorV >
using ct_error_code =
    unthrowable_wrapper < const std::error_code &, ec < ErrorV >>;

namespace ct_error
{
using enoent =
    ct_error_code < static_cast <
    int >(std::errc::no_such_file_or_directory) >;
using enodata =
    ct_error_code < static_cast <
    int >(std::errc::no_message_available) >;
using invarg =
    ct_error_code < static_cast < int >(std::errc::invalid_argument) >;
...
using operation_not_supported =
    ct_error_code < static_cast <
    int >(std::errc::operation_not_supported) >;
using not_connected =
    ct_error_code < static_cast < int >(std::errc::not_connected) >;
using timed_out =
    ct_error_code < static_cast < int >(std::errc::timed_out) >;
using erange =
    ct_error_code < static_cast <
...
using address_not_available =
    ct_error_code < static_cast <
    int >(std::errc::address_not_available) >;
using ecanceled =
    ct_error_code < static_cast <
    int >(std::errc::operation_canceled) >;
using einprogress =
    ct_error_code < static_cast <
    int >(std::errc::operation_in_progress) >;
using enametoolong =
    ct_error_code < static_cast < int >(std::errc::filename_too_long) >;
using eexist =
    ct_error_code < static_cast < int >(std::errc::file_exists) >;
...
struct pass_further_all {
    template < class ErrorT > decltype(auto) operator()(ErrorT && e)
    {
        return std::forward < ErrorT > (e);
    }
};
...
template < template < class ... >class ErroratedFutureT,
           class ValueT >
struct is_errorated_future <ErroratedFutureT <::crimson::
    errorated_future_marker < ValueT >> > {
...
template < template < class > class Container,
           class Value >
struct futurize <Container <::crimson::errorated_future_marker <
    Value >>> {
    using errorator_type =
        typename Container <::crimson::errorated_future_marker <
        Value >>::errorator_type;
    ...
    static type apply(Func
                      && func, std::tuple < FuncArgs ... >&&args) noexcept
    {
        try {
    ...
    template < class Func >[[gnu::always_inline]]
    ...
    template < typename Arg >[[gnu::always_inline]]
    static type make_exception_future(Arg && arg)
    {
...
template < template < class > class Container,
           class Value >
struct continuation_base_from_future <Container <::crimson::
    errorated_future_marker < Value >>> {
