    ...
    if (osdmap.stretch_mode_enabled) {
        ...
        if (osdmap.degraded_stretch_mode) {
            ...
            if (!osdmap.recovering_stretch_mode) {
                ...
                dout(20) << "mon_stretch_cluster_recovery_ratio: " << cct->_conf.get_val <
        ...
        if (!p->is_stretch_pool()) {
            ...
            if (f) {
                ...
                if (osdmap.crush->rule_exists(p->get_crush_rule())) {
                    ...
                    mon.clog->warn() << __func__ << " pool " << poolstr << " crush rule " << stringify(p->get_crush_rule()) <<
                                                    " does not exist";
                }
                                      ...
                                      const cmdmap_t &cmdmap)
{
    ...
    if (prefix == "osd setcrushmap" ||
        ...
        ntp->min_read_recency_for_promote = g_conf().get_val <
                                            uint64_t > ("osd_tier_default_cache_min_read_recency_for_promote");
        ntp->min_write_recency_for_promote = g_conf().get_val <
