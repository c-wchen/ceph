  ...
  private:
    ...
    void read_request() {
        ...
        http::async_read(socket_, buffer_, request_,
                         ...
                         boost::ignore_unused(bytes_transferred); if (ec) {
                         dout(1) << "ERROR: " << ec.message() << dendl; return;}
                         else {
...
void http_server_thread_entrypoint()
{
    try {
        std::string exporter_addr =
            g_conf().get_val < std::string > ("exporter_addr");
        auto const address = net::ip::make_address(exporter_addr);
        unsigned short port =
            g_conf().get_val < int64_t > ("exporter_http_port");
        ...
        ioc};
        http_server(acceptor, socket);
        dout(1) << "Http server running on " << exporter_addr << ":" << port <<
            dendl;
        ioc.run();
