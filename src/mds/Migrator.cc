...
void Migrator::export_try_cancel(CDir * dir, bool notify_peer)
{
    ...
    case EXPORT_LOCKING:
        dout(10) <<
            "export state=locking : dropping locks and removing auth_pin" <<
            dendl;
        num_locking_exports--;
    ...
    case EXPORT_DISCOVERING:
        ...
        dir->auth_unpin(this);
        if (notify_peer && (!mds->is_cluster_degraded() || mds->mdsmap->is_clientreplay_or_active_or_stopping(it->second.peer)))    // tell them.
            mds->
                send_message_mds(new
                                 MExportDirCancel(dir->dirfrag(),
                                                  it->second.tid),
                                 it->second.peer);
        break;
    ...
    case EXPORT_WARNING:
        dout(10) <<
            "export state=warning : unpinning bounds, unfreezing, notifying" <<
            dendl;
    ...
    case EXPORT_EXPORTING:
        dout(10) << "export state=exporting : reversing, and unfreezing" <<
            dendl;
        it->second.state = EXPORT_CANCELLING;
    ...
    case EXPORT_NOTIFYING:
        dout(10) <<
            "export state=loggingfinish|notifying : ignoring dest failure, we were successful."
            << dendl;
        ...
        if ((p->second.peer == who &&
            ...
            if (p->second.notify_ack_waiting.erase(who)) {
                ...
                dout(10) << "faking export_notify_ack from mds." << who
                    << " on " << *dir << " to mds." << p->second.peer << dendl;
        ...
        if (q->second.peer == who) {
            if (dir)
                dout(10) << "cleaning up import state (" << q->second.
                    state << ")" << get_import_statename(q->second.
                                                         state) << " of " <<
                    *dir << dendl;
            else
                dout(10) << "cleaning up import state (" << q->second.
            ...
            case IMPORT_DISCOVERING:
                dout(10) << "import state=discovering : clearing state" <<
                    dendl;
            ...
            case IMPORT_DISCOVERED:
                assert(diri);
                dout(10) << "import state=discovered : unpinning inode " <<
                    *diri << dendl;
            ...
            case IMPORT_PREPPING:
                assert(dir);
                dout(10) << "import state=prepping : unpinning base+bounds " <<
                    *dir << dendl;
            ...
            case IMPORT_PREPPED:
                assert(dir);
                dout(10) <<
                    "import state=prepped : unpinning base+bounds, unfreezing "
                    << *dir << dendl;
                {
                    set < CDir * >bounds;
                    cache->get_subtree_bounds(dir, bounds);
            ...
            case IMPORT_LOGGINGSTART:
                assert(dir);
                dout(10) << "import state=loggingstart : reversing import on "
                    << *dir << dendl;
                import_reverse(dir);
                break;
            ...
            case IMPORT_ACKING:
                ...
                dout(10) << "import state=acking : noting ambiguous import " <<
                    *dir << dendl;
                {
                    set < CDir * >bounds;
                    cache->get_subtree_bounds(dir, bounds);
            ...
            case IMPORT_FINISHING:
                assert(dir);
                dout(10) << "import state=finishing : finishing import on " <<
                    *dir << dendl;
            ...
            case IMPORT_ABORTING:
                assert(dir);
                dout(10) << "import state=aborting : ignoring repeat failure "
                    << *dir << dendl;
                break;
            }
        }
            ...
            if (bystanders_entry != q->second.bystanders.end()) {
                ...
                if (q->second.state == IMPORT_ABORTING) {
                    assert(dir);
                    dout(10) << "faking export_notify_ack from mds." << who
                        << " on aborting import " << *dir << " from mds." << q->
                        second.peer << dendl;
    ...
    for (map < dirfrag_t, import_state_t >::iterator p = import_state.begin();
        ...
        if (dir) {
            dout(10) << " importing from " << p->second.peer
                << ": (" << p->second.state << ") " << get_import_statename(p->
                                                                            second.
                                                                            state)
                << " " << p->first << " " << *dir << dendl;
        }
...
void Migrator::show_exporting()
{
    ...
    for (map < CDir *, export_state_t >::iterator p = export_state.begin();
        ...
        dout(10) << " exporting to " << p->second.peer
            << ": (" << p->second.state << ") " << get_export_statename(p->
                                                                        second.
                                                                        state)
...
void Migrator::export_dir_nicely(CDir * dir, mds_rank_t dest)
{
    ...
    export_queue.push_back(pair < dirfrag_t,
                           mds_rank_t > (dir->dirfrag(), dest));
...
class C_MDC_ExportFreeze:public MigratorContext {
    ...
    uint64_t tid;
  public:
    C_MDC_ExportFreeze(Migrator * m, CDir * e, uint64_t t):MigratorContext(m),
        ex(e), tid(t) {
        assert(ex != NULL);
    } void finish(int r) override {
        if (r >= 0)
...
void Migrator::get_export_lock_set(CDir * dir, set < SimpleLock * >&locks)
{
    ...
    for (set < CDir * >::iterator p = wouldbe_bounds.begin();
         p != wouldbe_bounds.end(); ++p)
...
void Migrator::export_dir(CDir * dir, mds_rank_t dest)
{
    ...
    if (dir->inode->is_system()) {
        dout(7) <<
            "i won't export system dirs (root, mdsdirs, stray, /.ceph, etc.)" <<
            dendl;
        //ceph_abort();
        return;
    }
    ...
    if (dir->is_frozen() || dir->is_freezing()) {
        dout(7) <<
            " can't export, freezing|frozen.  wait for other exports to finish first."
            << dendl;
        return;
    }
    ...
    if (g_conf->mds_thrash_exports) {
        ...
        if (ls.size() > 0) {
            ...
            if (!(bd->is_frozen() || bd->is_freezing())) {
                ...
                dout(0) << "export_dir: create aux subtree " << *bd << " under "
                    << *dir << dendl;
            }
        }
    }
                                  ...
                                  vector < pair < CDir *, size_t > >&results)
{
    ...
    struct LevelData {
        ...
        LevelData(CDir * d):dir(d), iter(d->begin()) {
    }};
    ...
    for (;;) {
        ...
        if (data.complete) {
            ...
            results.insert(results.end(), data.subdirs.begin(),
                           data.subdirs.end());
...
void Migrator::dispatch_export_dir(MDRequestRef & mdr, int count)
{
    ...
    if (!mds->is_export_target(dest)) {
        ...
        if (count > 3) {
            dout(5) <<
                "dest has not been added as export target after three MDSMap epochs, canceling export"
                << dendl;
            export_try_cancel(dir);
            return;
        }
        ...
        mds->wait_for_mdsmap(mds->mdsmap->get_epoch(),
                             new C_M_ExportDirWait(this, mdr, count + 1));
        return;
    }
    ...
    if (!dir->inode->get_parent_dn()) {
        dout(7) << "waiting for dir to become stable before export: " << *dir <<
            dendl;
        dir->add_waiter(CDir::WAIT_CREATED,
                        new C_M_ExportDirWait(this, mdr, 1));
        return;
    }
    ...
    if (!mds->locker->
        acquire_locks(mdr, rdlocks, wrlocks, xlocks, NULL, NULL, true)) {
        if (mdr->aborted)
            export_try_cancel(dir);
        return;
    }
    ...
    if (results.size() == 1 && results.front().first == dir) {
        ...
        MExportDirDiscover *discover =
            new MExportDirDiscover(dir->dirfrag(), path,
        ...
        dir->add_waiter(CDir::WAIT_FROZEN,
    ...
    if (results.empty()) {
        dout(7) <<
            "subtree's children all are under exporting, retry rest parts of parent export "
...
class C_MDC_RestartExportDir:public MigratorContext {
    ...
    uint64_t tid;
  public:
    C_MDC_RestartExportDir(Migrator * m, CDir * d,
                           uint64_t t):MigratorContext(m), dir(d), tid(t) {
    } void finish(int r) override {
...
void Migrator::adjust_export_after_rename(CInode * diri, CDir * olddir)
{
    ...
    assert(stat.state == EXPORT_DISCOVERING || stat.state == EXPORT_FREEZING);

    if (g_conf->mds_thrash_exports) {
        if (rand() % 3 == 0) {
            mds->
                queue_waiter_front(new
                                   C_MDC_RestartExportDir(this, freezing_dir,
                                                          stat.tid));
            return;
        }
    }
...
void Migrator::child_export_finish(std::shared_ptr < export_base_t > &parent,
                                   bool success)
{
    if (success)
        parent->restart = true;
...
void Migrator::handle_export_discover_ack(MExportDirDiscoverAck * m)
{
    ...
    if (it == export_state.end() ||
        it->second.tid != m->get_tid() || it->second.peer != dest) {
        ...
        if (m->is_success()) {
            ...
            MDRequestRef mdr =
                static_cast < MDRequestImpl * >(it->second.mut.get());
            assert(mdr);
            ...
            dout(7) << "peer failed to discover (not active?), canceling" <<
                dendl;
...
void Migrator::export_sessions_flushed(CDir * dir, uint64_t tid)
{
    ...
    map < CDir *, export_state_t >::iterator it = export_state.find(dir);
    if (it == export_state.end() ||
        it->second.state == EXPORT_CANCELLING || it->second.tid != tid) {
        // export must have aborted.
...
void Migrator::export_frozen(CDir * dir, uint64_t tid)
{
    ...
    if ((diri->is_auth() && diri->is_frozen()) ||
        ...
        !diri->filelock.can_wrlock(-1) || !diri->nestlock.can_wrlock(-1)) {
  ...
  for (const auto & p:dir->get_replicas()) {
    ...
    if (gather.has_subs()) {
        ...
        gather.
            set_finisher(new
                         C_M_ExportSessionsFlushed(this, dir, it->second.tid));
        gather.activate();
    }
}
...
void Migrator::handle_export_prep_ack(MExportDirPrepAck * m)
{
  ...
  for (const auto & p:dir->get_replicas()) {
        ...
        MExportDirNotify *notify =
            new MExportDirNotify(dir->dirfrag(), it->second.tid, true,
...
class C_M_ExportGo:public MigratorContext {
    ...
    uint64_t tid;
  public:
    C_M_ExportGo(Migrator * m, CDir * d, uint64_t t):MigratorContext(m), dir(d),
        tid(t) {
        assert(dir != NULL);
    } void finish(int r) override {
...
void Migrator::encode_export_inode(CInode * in, bufferlist & enc_state,
                                   map < client_t,
                                   entity_inst_t > &exported_client_map)
{
...
void Migrator::encode_export_inode_caps(CInode * in, bool auth_cap,
                                        bufferlist & bl, map < client_t,
                                        entity_inst_t > &exported_client_map)
{
    ...
    for (map < client_t, Capability * >::iterator it = in->client_caps.begin();
        ...
        exported_client_map[it->first] =
            mds->sessionmap.get_inst(entity_name_t::CLIENT(it->first.v));
}

void Migrator::finish_export_inode_caps(CInode * in, mds_rank_t peer,
                                        map < client_t,
                                        Capability::Import > &peer_imported)
{
    ...
    for (map < client_t, Capability * >::iterator it = in->client_caps.begin();
        ...
        map < client_t, Capability::Import >::iterator q =
            peer_imported.find(it->first);
                                     ...
                                     utime_t now)
{
    ...
    dout(7) << "encode_export_dir " << *dir << " " << dir->
                                 ...
                                 int *num_dentries)
{
  ...
  for (auto & p:*dir) {
        ...
        // inode?
        if (dn->get_linkage()->is_primary()) {
            finish_export_inode(in, now, peer, peer_imported[in->ino()],
                                finished);
    ...
    // subdirs
    for (list < CDir * >::iterator it = subdirs.begin(); it != subdirs.end();
         ++it)
...
class C_MDS_ExportFinishLogged:public MigratorLogContext {
    CDir *dir;
  public:
    C_MDS_ExportFinishLogged(Migrator * m, CDir * d):MigratorLogContext(m),
        dir(d) {
    } void finish(int r) override {
...
void Migrator::export_notify_abort(CDir * dir, export_state_t & stat,
                                   set < CDir * >&bounds)
{
    ...
    for (set < mds_rank_t >::iterator p = stat.notify_ack_waiting.begin();
        ...
        MExportDirNotify *notify =
            new MExportDirNotify(dir->dirfrag(), stat.tid, true,
                                 pair < int, int >(mds->get_nodeid(),
                                                   stat.peer),
                                 pair < int, int >(mds->get_nodeid(),
                                                   CDIR_AUTH_UNKNOWN));
        for (set < CDir * >::iterator i = bounds.begin(); i != bounds.end();
             ++i)
...
void Migrator::export_logged_finish(CDir * dir)
{
    ...
    if (stat.notify_ack_waiting.empty()) {
        ...
        if (!mds->is_cluster_degraded() ||
            ...
            mds->
                send_message_mds(new
                                 MExportDirFinish(dir->dirfrag(), false,
                                                  stat.tid), stat.peer);
        }
        else {
...
void Migrator::handle_export_notify_ack(MExportDirNotifyAck * m)
{
    ...
    if (export_state_entry != export_state.end()) {
        ...
        else if (stat.state == EXPORT_CANCELLING && m->get_new_auth().second == CDIR_AUTH_UNKNOWN &&    // not warning ack
                 stat.notify_ack_waiting.erase(from)) {
            dout(7) << "handle_export_notify_ack from " << m->get_source()
                << ": cancelling export, processing notify on " << *dir <<
                dendl;
            if (stat.notify_ack_waiting.empty()) {
...
void Migrator::export_finish(CDir * dir)
{
    ...
    if (it == export_state.end()) {
        dout(7) <<
            "target must have failed, not sending final commit message.  export succeeded anyway."
            << dendl;
        return;
    }
    ...
    if (!mds->is_cluster_degraded() ||
        ...
        mds->
            send_message_mds(new
                             MExportDirFinish(dir->dirfrag(), true,
                                              it->second.tid), it->second.peer);
    }
    else {
        dout(7) << "not sending MExportDirFinish last, dest has failed" <<
            dendl;
    }
...
void Migrator::handle_export_discover(MExportDirDiscover * m)
{
    ...
    if (!mds->is_active()) {
        ...
        mds->
            send_message_mds(new MExportDirDiscoverAck(df, m->get_tid(), false),
                             from);
        m->put();
        return;
    }
    ...
    if (!m->started) {
        ...
        if (it == import_state.end() ||
            it->second.peer != from || it->second.tid != m->get_tid()) {
    ...
    if (!in) {
        ...
        int r =
            cache->path_traverse(null_ref, m, NULL, fpath, &trace, NULL,
                                 MDS_TRAVERSE_DISCOVER);
        if (r > 0)
            return;
        if (r < 0) {
            dout(7) << "handle_export_discover failed to discover or not dir "
    ...
    // yay
    dout(7) << "handle_export_discover have " << df << " inode " << *in <<
        dendl;
    ...
    mds->send_message_mds(new MExportDirDiscoverAck(df, m->get_tid()),
                          p_state->peer);
    m->put();
...
void Migrator::handle_export_prep(MExportDirPrep * m)
{
    ...
    if (!m->did_assim()) {
        ...
        if (it == import_state.end() ||
            it->second.peer != oldauth || it->second.tid != m->get_tid()) {
        ...
        assert(dir);
        dout(7) << "handle_export_prep on " << *dir << " (subsequent pass)" <<
            dendl;
        diri = dir->get_inode();
    }
    ...
    if (!m->did_assim()) {
        ...
        for (list < bufferlist >::iterator p = m->traces.begin();
            ...
            char start;
            ::decode(start, q);
            dout(10) << " trace from " << df << " start " << start << " len " <<
                p->length() << dendl;
        ...
        // make bound sticky
        for (map < inodeno_t, fragset_t >::iterator p =
             import_bound_fragset.begin(); p != import_bound_fragset.end();
             ++p) {
    ...
    if (mds->is_active()) {
        ...
        for (map < inodeno_t, fragset_t >::iterator p =
            ...
            dout(10) << " bound inode " << p->first << " fragset " << p->
                second << " maps to " << fglist << dendl;

            for (list < frag_t >::iterator q = fglist.begin();
                 q != fglist.end(); ++q) {
                CDir *bound = cache->get_dirfrag(dirfrag_t(p->first, *q));
                if (!bound) {
                    dout(7) << "  opening bounding dirfrag " << *q << " on " <<
                        *in << dendl;
                ...
                if (!bound->state_test(CDir::STATE_IMPORTBOUND)) {
                    ...
                    dout(7) << "  already pinned import bound " << *bound <<
                        dendl;
                }
        ...
        dout(7) << " all ready, noting auth and freezing import region" <<
            dendl;
    ...
    mds->
        send_message(new
                     MExportDirPrepAck(dir->dirfrag(), success, m->get_tid()),
                     m->get_connection());
...
class C_MDS_ImportDirLoggedStart:public MigratorLogContext {
  ...
  public:
    ...
    C_MDS_ImportDirLoggedStart(Migrator * m, CDir * d,
                               mds_rank_t f):MigratorLogContext(m),
        df(d->dirfrag()), dir(d), from(f) {
    } void finish(int r) override {
...
void Migrator::handle_export_dir(MExportDir * m)
{
    ...
    mds_rank_t oldauth = mds_rank_t(m->get_source().num());
    dout(7) << "handle_export_dir importing " << *dir << " from " << oldauth <<
        dendl;

    assert(!dir->is_auth());

    map < dirfrag_t, import_state_t >::iterator it =
        import_state.find(m->dirfrag);
    assert(it != import_state.end());
    ...
    if (!dir->get_inode()->dirfragtree.is_leaf(dir->get_frag()))
        dir->get_inode()->dirfragtree.force_to_leaf(g_ceph_context,
                                                    dir->get_frag());
    ...
    assert(cmp.end());
    le->cmapv =
        mds->server->prepare_force_open_sessions(client_map,
                                                 onlogged->
    ...
    while (!blp.end()) {
        num_imported_inodes += decode_import_dir(blp, oldauth, dir, // import root
                                                 le,
                                                 mds->mdlog->
                                                 get_current_segment(),
                                                 it->second.peer_exports,
                                                 it->second.
                                                 updated_scatterlocks, now);
    }
...
void Migrator::import_reverse(CDir * dir)
{
    ...
    while (!q.empty()) {
      ...
      for (auto & p:*cur) {
            ...
            if (dn->get_linkage()->is_primary()) {
                ...
                list < CDir * >dfs;
                in->get_dirfrags(dfs);
                for (list < CDir * >::iterator p = dfs.begin(); p != dfs.end();
                     ++p)
                    if (bounds.count(*p) == 0)
                        q.push_back(*p);
            }
    ...
    if (stat.state == IMPORT_ACKING) {
        ...
        for (map < CInode *, map < client_t,
             Capability::Export > >::iterator p = stat.peer_exports.begin();
                                   ...
                                   uint64_t > >&imported_session_map)
{
    ...
    for (map < CInode *, map < client_t, Capability::Export > >::iterator p =
         it->second.peer_exports.begin(); p != it->second.peer_exports.end();
         ++p) {
        // parameter 'peer' is NONE, delay sending cap import messages to client
        finish_import_inode_caps(p->first, MDS_RANK_NONE, true,
                                 imported_session_map, p->second,
    ...
    // send notify's etc.
    dout(7) << "sending ack for " << *dir << " to old auth mds." << from <<
        dendl;
...
void Migrator::handle_export_finish(MExportDirFinish * m)
{
    CDir *dir = cache->get_dirfrag(m->get_dirfrag());
    assert(dir);
    dout(7) << "handle_export_finish on " << *dir << (m->
                                                      is_last()? " last" : "")
        << dendl;
...
void Migrator::import_finish(CDir * dir, bool notify, bool last)
{
    ...
    if (it->second.state == IMPORT_ACKING) {
        for (map < CInode *, map < client_t,
            ...
            for (map < client_t, Capability::Export >::iterator q =
                ...
                cap->clear_importing();
                mds->mdcache->do_cap_import(session, in, cap, q->second.cap_id,
                                            q->second.seq, q->second.mseq - 1,
                                            it->second.peer,
                                            CEPH_CAP_FLAG_AUTH);
            }
            p->second.clear();
    ...
    for (map < CInode *, map < client_t, Capability::Export > >::iterator p =
         peer_exports.begin(); p != peer_exports.end(); ++p) {
        if (p->first->is_auth())
...
void Migrator::decode_import_inode(CDentry * dn, bufferlist::iterator & blp,
                                   ...
                                   map < CInode *, map < client_t,
                                   Capability::Export > >&peer_exports,
...
void Migrator::finish_import_inode_caps(CInode * in, mds_rank_t peer,
                                        bool auth_cap, const map < client_t,
                                        pair < Session *,
                                        uint64_t > >&session_map,
                                        const map < client_t,
                                        Capability::Export > &export_map,
                                        map < client_t,
                                        Capability::Import > &import_map)
{
  for (auto & it:export_map) {
        dout(10) << "finish_import_inode_caps for client." << it.
            first << " on " << *in << dendl;
...
void Migrator::handle_export_notify(MExportDirNotify * m)
{
    ...
    if (m->wants_ack()) {
        mds->
            send_message_mds(new
                             MExportDirNotifyAck(m->get_dirfrag(), m->get_tid(),
                                                 m->get_new_auth()), from);
    }
    else {
        // aborted.  no ack.
    ...
    if (in) {
      ...
      for (auto & it:imported_caps) {
            ...
            MClientCaps *m = new MClientCaps(CEPH_CAP_OP_EXPORT, in->ino(), 0,
                                             cap->get_cap_id(), cap->get_mseq(),
                                             mds->get_osd_epoch_barrier());
            m->set_cap_peer(it.second.cap_id, it.second.issue_seq,
                            it.second.mseq, from, 0);
            mds->send_message_client_counted(m, it.first);
...
class C_M_LoggedImportCaps:public MigratorLogContext {
  ...
  public:
    ...
    C_M_LoggedImportCaps(Migrator * m, CInode * i,
                         mds_rank_t f):MigratorLogContext(m), in(i), from(f) {
    } void finish(int r) override {
...
{
    dout(10) << "handle_export_caps " << *ex << " from " << ex->
        get_source() << dendl;
                                  ...
                                  Capability::Export > >&peer_exports)
{
    ...
    map < client_t, Capability::Import > imported_caps;
    finish_import_inode_caps(in, from, false, imported_session_map, it->second,
                             imported_caps);
                                  ...
                                  const MDSMap & mds_map)
{
    ...
    if (changed.count("mds_inject_migrator_session_race")) {
        inject_session_race =
            conf->get_val < bool > ("mds_inject_migrator_session_race");
    ...
    if (changed.count("mds_inject_migrator_message_loss")) {
        inject_message_loss =
            g_conf->get_val < int64_t > ("mds_inject_migrator_message_loss");
        dout(0) << "mds_inject_migrator_message_loss is " << inject_message_loss
            << dendl;
    }
}
