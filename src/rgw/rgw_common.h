...
public:
    ...
    /** Set the arguments; as received */ void set(const std::string &s)
    {
    ...
    const std::string &get(const std::string &name, bool *exists =
                               NULL) const;
    boost::optional <
    const std::string & > get_optional(const std::string &name) const;
    ...
    std::string sys_get(const std::string &name, bool *exists =
                            nullptr) const;
...
const char *rgw_conf_get(const std::map < std::string, std::string,
                         ltstr_nocase > &conf_map, const char *name,
                         const char *def_val);
int rgw_conf_get_int(const std::map < std::string, std::string,
                     ltstr_nocase > &conf_map, const char *name, int def_val);
bool rgw_conf_get_bool(const std::map < std::string, std::string,
                       ltstr_nocase > &conf_map, const char *name,
                       bool def_val);
...
public:
    ...
    const std::map < std::string, std::string, ltstr_nocase > &get_map() const
    {
        return env_map;
...
void encode_json(const char *name, const rgw_placement_rule &val,
                 ceph::Formatter *f);
...
struct RGWUserInfo {
    ...
    RGWUserInfo()
        : suspended(0),
          ...
          op_mask(RGW_OP_TYPE_ALL), admin(0), system(0), type(TYPE_NONE)
    {
...
struct RGWObjVersionTracker {
    ...
    /// If we have a write version, return a pointer to it. Otherwise/// return null. This is used in `prepare_op_for_write` to treat the/// `write_version` as effectively an `option` type.
...
struct RGWBucketInfo {
    ...
    // resharding
    cls_rgw_reshard_status reshard_status {
...
struct RGWStorageStats {
    ...
    RGWStorageStats(bool _dump_utilized = true)
        ...
        size(0), size_rounded(0), num_objects(0), dump_utilized(_dump_utilized)
    {
    } void dump(Formatter *f) const;
};                              // RGWStorageStats
...
struct req_state: DoutPrefixProvider {
    ...
    req_state(CephContext *_cct, const RGWProcessEnv &penv, RGWEnv *e,
              uint64_t id);
    ~req_state();

    void set_user(std::unique_ptr < rgw::sal::User > &u)
    {
        user.swap(u);
    }
    bool is_err() const
...
struct RGWBucketEnt {
    ...
    explicit RGWBucketEnt(const rgw_user &u, cls_user_bucket_entry && e)
        : bucket(u, std::move(e.bucket)),
          ...
          creation_time(e.creation_time), count(e.count)
    {
    }
...
static inline void buf_to_hex(const unsigned char *const buf,
                              const size_t len, char *const str)
{
    str[0] = '\0';
...
template < size_t N > static inline std::array < char,
         N * 2 + 1 > buf_to_hex(const std::array < unsigned char, N > &buf)
{
...
static inline void append_rand_alpha(CephContext *cct, const std::string &src,
                                     std::string &dest, int len)
{
    dest = src;
    char buf[len + 1];
...
extern void parse_csv_string(const std::string &ival,
                             std::vector < std::string > &ovals);
extern int parse_key_value(std::string &in_str, std::string &key,
                           std::string &val);
extern int parse_key_value(std::string &in_str, const char *delim,
                           std::string &key, std::string &val);

extern boost::optional < std::pair < std::string_view, std::string_view >> parse_key_value(
            const std::string_view &in_str,
...
extern bool parse_iso8601(const char *s, struct tm *t, uint32_t *pns =
                              NULL, bool extended_format = true);
...
struct perm_state: public perm_state_base {
    ...
    perm_state(CephContext *_cct,
        ...
        referer(_referer), request_payer(_request_payer)
    {
...
/** Check if the req_state's user has the necessary permissions
 * to do the requested action */
rgw::IAM::Effect eval_identity_or_session_policies(const DoutPrefixProvider *
        dpp,
        const std::vector <
        rgw::IAM::Policy >
        &user_policies,
        ...
        const uint64_t op,
        const rgw::ARN &arn);
bool verify_user_permission(const DoutPrefixProvider *dpp, req_state *const s,
                            RGWAccessControlPolicy *const user_acl,
                            const std::vector < rgw::IAM::Policy >
                            &user_policies,
                            const std::vector < rgw::IAM::Policy >
                            &session_policies, const rgw::ARN &res,
...
bool verify_user_permission_no_policy(const DoutPrefixProvider *dpp,
                                      req_state *const s, int perm);
bool verify_bucket_permission(const DoutPrefixProvider *dpp,
                              req_state *const s, const rgw_bucket &bucket,
                              RGWAccessControlPolicy *const user_acl,
                              RGWAccessControlPolicy *const bucket_acl,
                              const boost::optional < rgw::IAM::Policy >
                              &bucket_policy,
                              const std::vector < rgw::IAM::Policy >
                              &identity_policies,
                              const std::vector < rgw::IAM::Policy >
                              &session_policies, const uint64_t op);
bool verify_bucket_permission(const DoutPrefixProvider *dpp,
...
extern bool verify_object_permission(const DoutPrefixProvider *dpp,
                                     req_state *const s,
                                     const rgw_obj &obj,
                                     RGWAccessControlPolicy *const user_acl,
                                     RGWAccessControlPolicy *const bucket_acl,
...
extern bool verify_object_permission_no_policy(const DoutPrefixProvider *dpp,
        req_state *const s,
        RGWAccessControlPolicy *
        const user_acl,
        RGWAccessControlPolicy *
        const bucket_acl,
        RGWAccessControlPolicy *
        const object_acl, int perm);
extern bool verify_object_permission_no_policy(const DoutPrefixProvider *dpp,
        req_state *s, int perm);
extern int verify_object_lock(const DoutPrefixProvider *dpp,
...
extern std::string url_encode(const std::string &src, bool encode_slash =
                                  true);
...
static inline sha256_digest_t calc_hmac_sha256(const char *key, const int key_len,
...
static inline sha256_digest_t calc_hmac_sha256(const sha256_digest_t &key, const std::string_view &msg)
{
    ...
    calc_hmac_sha256(reinterpret_cast < const char *>(key.v),
                     sha256_digest_t::SIZE, msg.data(), msg.size(),
                     reinterpret_cast < char *>(dest.v));
    return dest;
}

static inline sha256_digest_t calc_hmac_sha256(const std::vector < unsigned char > &key,
...
extern std::string calc_hash_sha256_restart_stream(ceph::crypto::SHA256 **
        phash);
...
extern int rgw_bucket_parse_bucket_instance(const std::string &bucket_instance,
        std::string *bucket_name,
