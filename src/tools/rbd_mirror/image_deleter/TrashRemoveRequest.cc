...
void TrashRemoveRequest < I >::get_trash_image_spec()
{
    ...
    auto aio_comp = create_rados_callback <
                    ...
                    &TrashRemoveRequest < I >::handle_get_trash_image_spec >
                    (this);
    m_out_bl.clear();
    int r =
...
void TrashRemoveRequest <
I >::handle_get_trash_image_spec(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r == 0) {
        auto bl_it = m_out_bl.cbegin();
        r = librbd::cls_client::trash_get_finish(&bl_it,
            &m_trash_image_spec);
    }
    if (r == -ENOENT
        || (r >= 0
            && m_trash_image_spec.source !=
            cls::rbd::TRASH_IMAGE_SOURCE_MIRRORING)) {
        dout(10) << "image id " << m_image_id <<
    ...
    if (m_trash_image_spec.state !=
        cls::rbd::TRASH_IMAGE_STATE_NORMAL
        && m_trash_image_spec.state !=
        cls::rbd::TRASH_IMAGE_STATE_REMOVING) {
        dout(10) << "image " << m_image_id <<
...
void TrashRemoveRequest < I >::handle_set_trash_state(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        derr << "error setting trash image state for image id " <<
...
void TrashRemoveRequest < I >::get_snap_context()
{
    ...
    m_out_bl.clear();
    int r =
        m_io_ctx.aio_operate(header_oid, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
void TrashRemoveRequest < I >::handle_get_snap_context(int r)
{
    ...
    if (r == 0) {
        auto bl_it = m_out_bl.cbegin();
        r = librbd::cls_client::get_snapcontext_finish(&bl_it,
            &snapc);
    }
    if (r < 0 && r != -ENOENT) {
...
void TrashRemoveRequest < I >::purge_snapshots()
{
    ...
    auto req =
        SnapshotPurgeRequest < I >::create(m_io_ctx, m_image_id,
                                           ctx);
    req->send();
}
...
void TrashRemoveRequest < I >::handle_purge_snapshots(int r)
{
    ...
    if (r == -EBUSY) {
        ...
        *m_error_result = ERROR_RESULT_RETRY_IMMEDIATELY;
        ...
        derr << "failed to purge image snapshots: " <<
...
void TrashRemoveRequest < I >::remove_image()
{
    ...
    auto req =
        librbd::trash::RemoveRequest < I >::create(m_io_ctx,
            m_image_id,
            m_op_work_queue,
            true,
            m_progress_ctx,
...
void TrashRemoveRequest < I >::notify_trash_removed()
{
    ...
    Context *ctx = create_context_callback <
                   TrashRemoveRequest < I >,
                   &TrashRemoveRequest < I >::handle_notify_trash_removed >
                   (this);
    librbd::TrashWatcher < I >::notify_image_removed(m_io_ctx,
            m_image_id,
            ctx);
}
...
template < typename I >
void TrashRemoveRequest <
I >::handle_notify_trash_removed(int r)
{
    dout(10) << "r=" << r << dendl;
...
template class rbd::mirror::image_deleter::TrashRemoveRequest <
    librbd::ImageCtx >;
