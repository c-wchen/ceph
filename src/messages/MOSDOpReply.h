...
class MOSDOpReply final:public Message {
   ...
   */
    ...
    // osdmap epoch_t get_map_epoch() const {
        return osdmap_epoch;
    }
  ...
  */ public:
     MOSDOpReply()
    :Message {
    CEPH_MSG_OSD_OPREPLY, HEAD_VERSION, COMPAT_VERSION}
    , bdata_encode(false) {
        do_redirect = false;
    }
  ...
  :    Message {
    CEPH_MSG_OSD_OPREPLY, HEAD_VERSION, COMPAT_VERSION},
        ...
        flags =
            (req->
             flags & ~(CEPH_OSD_FLAG_ONDISK | CEPH_OSD_FLAG_ONNVRAM |
                       CEPH_OSD_FLAG_ACK)) | acktype;
        osdmap_epoch = e;
        user_version = 0;
