...
public:
    ...
    ObjectContext()
        :
        ssc(NULL),
        destructor_callback(0), blocked(false), requeue_scrub_on_unblock(false)
    {
    }
    ...
    /// in-progress copyfrom ops for this object bool blocked;
...
public:
    ...
    {
        return locks.empty();
    } bool get_lock_type(RWState::State type,
    ...
    bool take_write_lock(const hobject_t &hoid, ObjectContextRef obc)
    {
        ...
        if (obc->rwstate.take_write_lock()) {
            locks.
            insert(std::
                   make_pair(hoid, ObjectLockState(obc, RWState::RWWRITE)));
            return true;
        } else {
            return false;
        }
    ...
    bool try_get_read_lock(const hobject_t &hoid, ObjectContextRef obc)
    {
        ...
        if (obc->try_get_read_lock()) {
            locks.insert(std::make_pair(hoid,
    ...
    void put_locks(std::list < std::pair < ObjectContextRef,
                   std::list < OpRequestRef > > > *to_requeue,
                   bool *requeue_recovery, bool *requeue_snaptrimmer)
    {
        for (auto &p : locks) {
