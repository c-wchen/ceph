...
private:
    ...
    int get_params()
    {
        ...
        s->info.args.get_int("time_to_live", reinterpret_cast < int * > (&dest.time_to_live),
        ...
        s->info.args.get_int("retry_sleep_duration", reinterpret_cast < int * > (&dest.retry_sleep_duration),
                             rgw::notify::DEFAULT_GLOBAL_VALUE);
...
void RGWPSCreateNotifOp::execute_v2(optional_yield y)
{
    if (const auto ret = driver->stat_topics_v1(s->bucket_tenant, y, this); ret != -ENOENT) {
        ldpp_dout(this, 1) << "WARNING: " << (ret == 0 ? "topic migration in process" :
...
{
    if (const auto ret = driver->stat_topics_v1(s->bucket_tenant, y, this); ret != -ENOENT) {
        ldpp_dout(this, 4) << "WARNING: " << (ret == 0 ? "topic migration in process" :
                                              "cannot determine topic migration status. ret = " + std::to_string(ret))
                           << ". please try again later" << dendl;
        op_ret = -ERR_SERVICE_UNAVAILABLE;
        return;
