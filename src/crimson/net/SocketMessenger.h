...
public:
    SocketMessenger(const entity_name_t &myname,
                    const std::string &logic_name, uint32_t nonce);
    ...
    bool owns_connection(Connection &conn) const override
    {
        return this == &static_cast <
               SocketConnection & >(conn).get_messenger();
    }
    // can only wait once seastar::future <> wait() override {
...
void set_policy(entity_type_t peer_type,
                const SocketPolicy &p) override;

void set_policy_throttler(entity_type_t peer_type,
                          Throttle *throttle) override;
...
template <> struct fmt::formatter <
    crimson::net::SocketMessenger > : fmt::ostream_formatter {
};
#endif
