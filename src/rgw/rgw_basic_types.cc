...
rgw_bucket::rgw_bucket(const rgw_user &u,
                       const cls_user_bucket &b): tenant(u.tenant),
    name(b.name), marker(b.marker), bucket_id(b.bucket_id),
...
void rgw_bucket::convert(cls_user_bucket *b) const const
{
    b->name = name;
    b->marker = marker;
...
std::string rgw_bucket::get_key(char tenant_delim, char id_delim,
                                size_t reserve) const const
{
