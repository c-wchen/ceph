        ...
        bool ignore_public_acls)
{
    ...
    if (!ignore_public_acls && ((perm & perm_mask) != perm_mask)) {
        ...
        if (false == auth_identity.is_owner_of(rgw_user(RGW_USER_ANON_ID))) {
            ...
            perm |=
                acl.get_group_perm(dpp, ACL_GROUP_AUTHENTICATED_USERS,
                                   perm_mask);
        }
    }
        ...
        bool ignore_public_acls)
{
    ...
    uint32_t policy_perm =
        get_perm(dpp, auth_identity, test_perm, http_referer,
                 ignore_public_acls);
...
bool RGWAccessControlPolicy::is_public(const DoutPrefixProvider *dpp) constconst {
    ...
    {
        auto p =
            acl.get_group_perm(dpp, g, RGW_PERM_FULL_CONTROL);
