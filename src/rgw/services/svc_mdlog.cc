...
RGWSI_MDLog::RGWSI_MDLog(CephContext * cct, bool _run_sync):RGWServiceInstance(cct),
run_sync(_run_sync)
{
}
...
int RGWSI_MDLog::init(RGWSI_RADOS * _rados_svc, RGWSI_Zone * _zone_svc,
...
int RGWSI_MDLog::read_history(RGWMetadataLogHistory * state,
                              ...
                              optional_yield y,
                              const DoutPrefixProvider * dpp) const const
{
    auto & pool = svc.zone->get_zone_params().log_pool;
    const auto & oid = RGWMetadataLogHistory::oid;
    bufferlist bl;
    int ret =
        rgw_get_system_obj(svc.sysobj, pool, oid, bl, objv_tracker, nullptr, y,
                           dpp);
    if (ret < 0) {
        return ret;
    }
...
namespace mdlog {
    ...
    namespace {
      template < class T > class SysObjReadCR:public RGWSimpleCoroutine {
          ...
          :    RGWSimpleCoroutine(_svc->ctx()), dpp(_dpp),
                async_rados(_async_rados), svc(_svc), obj(_obj),
                result(_result), empty_on_enoent(empty_on_enoent),
                ...
                try {
                    ...
                    ldpp_dout(dpp, 0) << "ERROR: " << __func__ <<
                        ": reference counted object mismatched, \"" << e.
                        what() << "\"" << dendl;
                }
            }
            ...
            int send_request(const DoutPrefixProvider * dpp) {
                req =
                    new RGWAsyncGetSystemObj(dpp, this,
                                             stack->
      ...
      template < class T > class SysObjWriteCR:public RGWSimpleCoroutine
        {
          ...
          public:
            SysObjWriteCR(const DoutPrefixProvider * _dpp,
                          ...
                          nullptr, bool exclusive = false)
          :    RGWSimpleCoroutine(_svc->ctx()), dpp(_dpp),
                async_rados(_async_rados), svc(_svc), obj(_obj),
                objv_tracker(objv_tracker), exclusive(exclusive) {
                encode(_data, bl);
            }
            ...
            int send_request(const DoutPrefixProvider * dpp) override {
                req =
                    new RGWAsyncPutSystemObj(dpp, this,
                                             stack->
                                             create_completion_notifier(), svc,
                                             objv_tracker, obj, exclusive,
                                             std::move(bl));
                async_rados->queue(req);
                return 0;
            }
    ...
    class ReadHistoryCR:public RGWCoroutine {
      ...
      public:
        ReadHistoryCR(const DoutPrefixProvider * dpp,
                      const Svc & svc,
                      Cursor * cursor, RGWObjVersionTracker * objv_tracker)
        :RGWCoroutine(svc.zone->ctx()), dpp(dpp), svc(svc),
            ...
            reenter(this) {
                ...
                *cursor =
                    svc.mdlog->period_history->lookup(state.oldest_realm_epoch);
                if (!*cursor) {
                ...
                ldpp_dout(dpp,
                    ...
                    state.oldest_period_id << " realm_epoch=" << state.
                    oldest_realm_epoch << dendl;
                return set_cr_done();
            }
            return 0;
        }
    ...
    class WriteHistoryCR:public RGWCoroutine {
      ...
      public:
        WriteHistoryCR(const DoutPrefixProvider * dpp,
                       Svc & svc,
                       const Cursor & cursor, RGWObjVersionTracker * objv)
        :RGWCoroutine(svc.zone->ctx()), dpp(dpp), svc(svc),
            ...
            reenter(this) {
                ...
                yield {
                    ...
                    call(new
                         WriteCR(dpp, async_processor, svc.sysobj, obj, state,
                                 objv));
                }
                if (retcode < 0) {
...
/// update the mdlog history to reflect trimmed logs
    class TrimHistoryCR:public RGWCoroutine {
      ...
      public:
        TrimHistoryCR(const DoutPrefixProvider * dpp, const Svc & svc,
                      Cursor cursor, RGWObjVersionTracker * objv)
        :RGWCoroutine(svc.zone->ctx()), dpp(dpp), svc(svc), cursor(cursor),
            objv(objv), next(cursor) {
...
Cursor RGWSI_MDLog::find_oldest_period(const DoutPrefixProvider * dpp,
                                       optional_yield y)
{
                                           ...
                                           const DoutPrefixProvider * dpp)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
            ...
            oldest_period_id << " for mdlog history: " << cpp_strerror(ret) <<
            dendl;
        return Cursor {
        ret};
    }
...
Cursor RGWSI_MDLog::read_oldest_log_period(optional_yield y,
...
RGWCoroutine *RGWSI_MDLog::read_oldest_log_period_cr(const DoutPrefixProvider *
                                                     dpp, Cursor * period,
...
RGWMetadataLog *RGWSI_MDLog::get_log(const std::string & period)
{
    ...
    auto insert = md_logs.emplace(std::piecewise_construct,
                                  ...
                                  std::forward_as_tuple(cct, svc.zone, svc.cls,
                                                        period));
    return &insert.first->second;
}

int RGWSI_MDLog::add_entry(const DoutPrefixProvider * dpp,
                           const string & hash_key, const string & section,
...
int RGWSI_MDLog::pull_period(const DoutPrefixProvider * dpp,
