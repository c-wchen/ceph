...
// prevent the addition of OperationTypeCode-s with no matching OP_NAMES entry:
    static_assert((sizeof(OP_NAMES) / sizeof(OP_NAMES[0])) ==
    ...
    struct InterruptibleOperation:Operation {
        template < typename ValuesT = void >
            ...
            interruptible_future <::crimson::osd::IOInterruptCondition,
            ValuesT >;
    ...
    template < typename T > struct OperationT:InterruptibleOperation {
        static constexpr const char *type_name =
            OP_NAMES[static_cast < int >(T::type)];
  ...
  template < class T > class TrackableOperationT:public OperationT < T > {
        ...
        template < class EventT > decltype(auto) get_event()const {
            return std::get < EventT > (that()->tracking_events);
        ...
        template < class BlockingEventT, class InterruptorT = void, class F >
            ...
            auto ret =
                std::forward < F >
                (f) (typename BlockingEventT::template Trigger < T > {
                     get_event < BlockingEventT > (), *that()
                     });
            if constexpr
                (std::is_same_v < InterruptorT, void >) {
                    return ret;
                }
            else {
        ...
        template < class InterruptorT = void, class StageT >
            ...
            return this->template with_blocking_event <
                ...
                InterruptorT > ([&stage, this] (auto && trigger) {
                                ...
                                return that()->get_handle().template enter < T >
 ...
 */
    class OperationThrottler:public BlockerT < OperationThrottler >,
        ...
        template < typename OperationT, typename F >
            auto with_throttle(OperationT * op,
            ...
            .then([this] (auto x) {
                  release_throttle(); return x;}
            );
        }

        template < typename OperationT, typename F >
            seastar::future <> with_throttle_while(OperationT * op,
                                                   crimson::osd::scheduler::
                                                   params_t params, F && f) {
            return with_throttle(op, params,
                                 f).then([this, params, op, f] (bool cont) {
                                         return cont ? with_throttle_while(op,
                                                                           params,
                                                                           f) :
                                         seastar::now();}
            );
      ...
      public:
        ...
        template < class OpT, class ... Args >
            seastar::future <> with_throttle_while(BlockingEvent::Trigger <
                                                   OpT > &&trigger, Args
                                                   && ... args) {
            return trigger.
                maybe_record_blocking(with_throttle_while
                                      (std::forward < Args > (args) ...),
                                      *this);
        }
      ...
      private:
        ...
        seastar::future <> acquire_throttle(crimson::osd::scheduler::
                                            params_t params);
