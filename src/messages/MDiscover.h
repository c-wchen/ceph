...
class MDiscover:public Message {
  ...
  MDiscover(inodeno_t base_ino_, frag_t base_frag_, snapid_t s, filepath & want_path_, bool want_base_dir_ = true, bool discover_xlocks_ = false):
    Message(MSG_MDS_DISCOVER),
        base_ino(base_ino_),
        base_dir_frag(base_frag_),
        snapid(s),
        want(want_path_),
        want_base_dir(want_base_dir_), want_xlocked(discover_xlocks_) {
    }
  private:
    ~MDiscover()override {
    }
