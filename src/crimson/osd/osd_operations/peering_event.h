...
protected:
    ...
    virtual seastar::
    future <> complete_rctx_no_pg(ShardServices &shard_services)
    {
        return seastar::now();
    }

public:
    template < typename ... Args > PeeringEvent(const pg_shard_t &from, const spg_t &pgid, Args && ... args):
        from(from),
        pgid(pgid), evt(std::forward < Args > (args) ...)
    {
    }
    template < typename ... Args > PeeringEvent(const pg_shard_t &from, const spg_t &pgid,
            float delay, Args && ... args): from(from),
        pgid(pgid), delay(delay), evt(std::forward < Args > (args) ...)
    {
    }
...
protected:
    ...
    PeeringEvent::
    interruptible_future <> complete_rctx(ShardServices &
                                          shard_services,
                                          Ref < PG > pg) override;
    seastar::
    future <> complete_rctx_no_pg(ShardServices &shard_services)
...
public:
    ...
    template < typename ... Args > RemotePeeringEvent(crimson::net::ConnectionRef conn, Args && ... args):
    ...
    CompletionEvent > tracking_events;
...
template <> struct fmt::formatter <
    crimson::osd::LocalPeeringEvent > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::osd::RemotePeeringEvent > : fmt::ostream_formatter {
};
template < class T > struct fmt::formatter < crimson::osd::PeeringEvent <
