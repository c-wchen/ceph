...
class WBThrottle:Thread, public md_config_obs_t {
   ...
   */
    ...
    void remove_object(const ghobject_t & oid) {
        ...
        ceph::unordered_map < ghobject_t,
            list < ghobject_t >::iterator >::iterator iter = rev_lru.find(oid);
        if (iter == rev_lru.end())
            return;
    ...
    bool get_next_should_flush(boost::tuple < ghobject_t, FDRef, PendingWB > *next  ///< [out] next to flush
