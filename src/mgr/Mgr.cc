...
client(client_), client_messenger(clientm_), lock("Mgr::lock"),
timer(g_ceph_context, lock), finisher(g_ceph_context, "Mgr", "mgr-fin"),
...
void Mgr::handle_osd_map()
{
   ...
   */
    objecter->with_osdmap([this, &names_exist] (const OSDMap & osd_map) {
                          ...
                          auto addr_iter =
                          metadata->metadata.find("front_addr");
                          ...
                          dout(4) << "OSD[" << osd_id << "] addr change " <<
                          metadata_addr << " != " << stringify(map_addr) <<
  ...
  for (const auto & i:mds_info) {
        ...
        if (daemon_state.exists(k)) {
            ...
            if (metadata->metadata.empty() ||
                ...
                if (update) {
                    dout(4) << "MDS[" << info.
                        name << "] addr change " << metadata_addr << " != " <<
...
std::vector < MonCommand > Mgr::get_command_set() constconst
{
    Mutex::Locker l(lock);
