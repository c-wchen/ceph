...
struct TestLogEntry {
    ...
    TestLogEntry(const uint64_t image_offset_bytes,
    ...
    {
        os << "referring_map_entries=" << entry.
           referring_map_entries << ", " << "image_offset_bytes="
           << entry.
           image_offset_bytes << ", " << "write_bytes=" << entry.
           write_bytes;
        return os;
    };
};
...
TEST_F(TestWriteLogMap, Simple)
{
    ...
    TestLogMapEntries found0 =
        map.find_map_entries(BlockExtent(0, 100));
    ...
    for (int i = 4; i < 12; i++) {
        TestLogMapEntries found0 =
            map.find_map_entries(BlockExtent(i, i + 1));
        int numfound = found0.size();
        ASSERT_EQ(1, numfound);
        ASSERT_EQ(e1, found0.front().log_entry);
    }
...
TEST_F(TestWriteLogMap, OverlapMiddle)
{
    ...
    TestLogMapEntries found0 =
        map.find_map_entries(BlockExtent(0, 1));
    int numfound = found0.size();
    ASSERT_EQ(1, numfound);
    ASSERT_EQ(e0, found0.front().log_entry);
    TestLogEntries entries =
        map.find_log_entries(BlockExtent(0, 1));
