    ...
    namespace crypto {
        namespace luks {
            ...
            using librbd::util::create_context_callback;
             ...
             template < typename I >
                LoadRequest < I >::LoadRequest(I * image_ctx,
                                               encryption_format_t format,
                                               std::string_view passphrase,
                                               std::unique_ptr <
                                               CryptoInterface > *result_crypto,
                                               std::string *
                                               detected_format_name,
                                               Context *
                                               on_finish):m_image_ctx
                (image_ctx), m_format(format), m_passphrase(passphrase),
                m_on_finish(on_finish), m_result_crypto(result_crypto),
                ...
                I >::set_initial_read_size(uint64_t read_size) {
                m_initial_read_size = read_size;
            } template < typename I > void LoadRequest < I >::send() {
                auto ctx = create_context_callback <
                    LoadRequest < I >,
                    &LoadRequest < I >::handle_read_header > (this);
                 read(m_initial_read_size, ctx);
            } template < typename I >
                void LoadRequest < I >::read(uint64_t end_offset,
                                             Context * on_finish) {
                ...
                ZTracer::Trace trace;
                auto req =
                    io::ImageDispatchSpec::create_read(*m_image_ctx,
                                                       io::
                ...
                if (m_offset == 0) {
                    ...
                    if (m_image_ctx->parent != nullptr
                        ...
                        if (r < 0) {
                            ...
                            auto image_size =
                                m_image_ctx->get_image_size(m_image_ctx->
                                                            snap_id);
                            m_image_ctx->image_lock.unlock_shared();

                            auto max_header_size =
                                std::min(MAXIMUM_HEADER_SIZE, image_size);

                            if (r == -EINVAL && m_bl.length() < max_header_size) {
                                m_bl.clear();
                                auto ctx = create_context_callback <
                                    LoadRequest < I >,
                                    &LoadRequest < I >::handle_read_header >
                                    (this);
                            ...
                            lderr(m_image_ctx->
                                  cct) << "error replacing rbd clone magic: " <<
            ...
            template < typename I >
                ...
                default:
                    lderr(m_image_ctx->
                          cct) << "unsupported format type: " << m_format <<
                        dendl;
                    finish(-EINVAL);
                    return;
                }
                ...
                *m_detected_format_name = m_header.get_format_name();
                ...
                if (strcmp(cipher, "aes") != 0) {
                    lderr(m_image_ctx->
                          cct) << "unsupported cipher: " << cipher << dendl;
                    finish(-ENOTSUP);
                    return;
                }
                ...
                if (strcmp(cipher_mode, "xts-plain64") != 0) {
                    lderr(m_image_ctx->
                          cct) << "unsupported cipher mode: " << cipher_mode <<
                        dendl;
                    finish(-ENOTSUP);
                    return;
                }
                ...
                if (m_header.get_data_offset() > image_size) {
                    lderr(m_image_ctx->
                          cct) << "image is too small, data offset " <<
                ...
                if (m_header.get_data_offset() % stripe_period != 0) {
                    lderr(m_image_ctx->
                          cct) << "incompatible stripe pattern, data offset " <<
                        m_header.get_data_offset() << dendl;
                    finish(-EINVAL);
                    return;
                }
            ...
            template < typename I > void LoadRequest < I >::read_volume_key() {
                ...
                auto r =
                    m_header.read_volume_key(m_passphrase.data(),
                                             m_passphrase.size(),
                                             reinterpret_cast <
                                             char *>(volume_key),
                                             &volume_key_size);
                if (r != 0) {
                    ...
                    if (m_offset < keyslots_end_offset) {
                        ...
                        auto ctx = create_context_callback <
                            LoadRequest < I >,
                            &LoadRequest < I >::handle_read_keyslots > (this);
