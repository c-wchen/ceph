    ...
    namespace crypto {
        namespace openssl {
            ...
            int DataCryptor::init(const char *cipher_name,
                ...
                if (m_cipher == nullptr) {
                    lderr(m_cct) << "EVP_get_cipherbyname failed. Cipher name: "
                        << cipher_name << dendl;
                    log_errors();
                    return -EINVAL;
                }
                ...
                if (expected_key_length != key_length) {
                    lderr(m_cct) << "cipher " << cipher_name <<
                        " expects key of " << expected_key_length <<
            ...
            uint32_t DataCryptor::get_block_size() const {
                ...
                if (1 !=
                    EVP_CipherInit_ex(ctx, m_cipher, nullptr, m_key, nullptr,
                                      enc)) {
            ...
            void DataCryptor::return_context(EVP_CIPHER_CTX * ctx,
                                             CipherMode mode) {
                if (ctx != nullptr) {
            ...
            int DataCryptor::init_context(EVP_CIPHER_CTX * ctx,
                ...
                if (iv_length != m_iv_size) {
                    ...
                    return -EINVAL;
                      ...
                      EVP_CipherInit_ex(ctx, nullptr, nullptr, nullptr, iv,
                                        -1)) {
                    lderr(m_cct) << "EVP_CipherInit_ex failed" << dendl;
                    log_errors();
                    return -EIO;
            ...
            int DataCryptor::update_context(EVP_CIPHER_CTX * ctx,
                                            const unsigned char *in,
                                            unsigned char *out,
                                            uint32_t len) const {
                int out_length;
                if (1 != EVP_CipherUpdate(ctx, out, &out_length, in, len)) {
                    lderr(m_cct) << "EVP_CipherUpdate failed. len=" << len <<
                        dendl;
                    log_errors();
                    return -EIO;
                } return out_length;
