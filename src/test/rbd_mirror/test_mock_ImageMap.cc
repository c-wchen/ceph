...
namespace rbd {
    namespace mirror {
        ...
        namespace image_map {
            ...
            LoadRequest < librbd::MockTestImageCtx > *LoadRequest <
                librbd::MockTestImageCtx >::s_instance = nullptr;
            UpdateRequest < librbd::MockTestImageCtx > *UpdateRequest <
                librbd::MockTestImageCtx >::s_instance = nullptr;
    ...
    namespace mirror {
        ...
        class TestMockImageMap:public TestMockFixture {
          public:
            ...
            typedef UpdateRequest < librbd::MockTestImageCtx >
                MockUpdateRequest;
            ...
            struct MockListener:Listener {
                ...
                } MOCK_METHOD2(mock_acquire_image,
                               void (const std::string &, Context *));
                 MOCK_METHOD2(mock_release_image,
                              void (const std::string &, Context *));
                 MOCK_METHOD3(mock_remove_image,
            ...
            void SetUp() override {
                ...
                EXPECT_EQ(0,
                          ...
                          conf_set("rbd_mirror_image_policy_migration_throttle",
                                   "0"));
                EXPECT_EQ(0,
                          _rados->conf_set("rbd_mirror_image_policy_type",
                                           "simple"));
            }
            ...
            void expect_update_request(MockUpdateRequest & request, int r) {
                EXPECT_CALL(request, send())
                    .WillOnce(Invoke([this, &request, r] () {
                                     request.on_finish->complete(r); if (r == 0) {
                                     std::lock_guard locker {
                                     m_lock};
                                     ++m_map_update_count; m_cond.notify_all();}
                                     }
                              )) ;
            }
            ...
            void expect_listener_acquire_image(MockListener & mock_listener,
                                               ...
                                               std::map < std::string,
                                               Context * >*peer_ack_ctxs) {
                EXPECT_CALL(mock_listener,
                            mock_acquire_image(global_image_id, _))
                    .WillOnce(WithArg < 1 >
                              (Invoke
                               ([this, global_image_id,
                                 peer_ack_ctxs] (Context * ctx) {
                                std::lock_guard locker {
                                m_lock}; peer_ack_ctxs->insert( {
                                                               global_image_id,
                                                               ctx});
            ...
            void expect_listener_release_image(MockListener & mock_listener,
                                               ...
                                               Context * >*peer_ack_ctxs) {
                EXPECT_CALL(mock_listener,
                            mock_release_image(global_image_id, _))
                    .WillOnce(WithArg < 1 >
                              (Invoke
                               ([this, global_image_id,
                                 peer_ack_ctxs] (Context * ctx) {
              ...
              for (auto const &global_image_id:global_image_ids)
                {
                    ...
                    expect_add_event(mock_threads);
                    expect_listener_acquire_image(mock_listener,
                                                  global_image_id,
                                                  peer_ack_ctxs);
                }
            }
            ...
            void listener_release_images(MockListener & mock_listener,
              ...
              for (auto const &global_image_id:global_image_ids)
                {
                    expect_listener_release_image(mock_listener,
                                                  global_image_id,
                                                  peer_ack_ctxs);
                }
            }
            ...
            void listener_remove_images(MockListener & mock_listener,
              ...
              for (auto const &global_image_id:global_image_ids)
                {
                    expect_listener_remove_image(mock_listener, mirror_uuid,
                                                 global_image_id,
                                                 peer_ack_ctxs);
                }
            }
        ...
        TEST_F(TestMockImageMap, SetLocalImages) {
            ...
            MockImageMap::create(m_local_io_ctx, &mock_threads,
                                     m_local_instance_id, mock_listener)};
            ...
            remote_peer_ack_nowait(mock_image_map.get(), global_image_ids_ack,
                                   0, &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, AddRemoveLocalImage) {
            ...
            mock_image_map->update_images("",
                                          std::move(initial_global_image_ids), {
                                          });

            ASSERT_TRUE(wait_for_map_update(1));
            ASSERT_TRUE(wait_for_listener_notify
                        (initial_global_image_ids_ack.size()));

            // remote peer ACKs image acquire request
            remote_peer_ack_nowait(mock_image_map.get(),
                                   initial_global_image_ids_ack, 0,
                                   &peer_ack_ctxs);
            ...
            listener_release_images(mock_listener, remove_global_image_ids,
                                    &peer_ack_ctxs);
            update_map_request(mock_threads, mock_update_request,
                               remove_global_image_ids, 0);

            // remove images
            mock_image_map->update_images("", {
                                          },
                                          std::move(remove_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (remove_global_image_ids_ack.size()));

            remote_peer_ack_wait(mock_image_map.get(),
                                 remove_global_image_ids_ack, 0,
                                 &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, AddRemoveRemoteImage) {
            ...
            // initial image list
            mock_image_map->update_images("uuid1",
                                          std::move(initial_global_image_ids), {
                                          });
            ...
            listener_remove_images(mock_listener, "uuid1",
            ...
            mock_image_map->update_images("uuid1", {
                                          ...
                                          std::move(remove_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (remove_global_image_ids_ack.size() * 2));
        ...
        TEST_F(TestMockImageMap, AddRemoveRemoteImageDuplicateNotification) {
            ...
            mock_image_map->update_images("uuid1",
                                          std::
                                          move(initial_global_image_ids_dup), {
                                          });
            ...
            mock_image_map->update_images("uuid1", {
                                          ...
                                          std::
                                          move(remove_global_image_ids_dup));
        ...
        TEST_F(TestMockImageMap, RemoveRemoteAndLocalImage) {
            ...
            "global id 1"};
            std::set < std::string >
                initial_remote_global_image_ids_ack
            ...
            expect_add_event(mock_threads);
            std::map < std::string, Context * >peer_ack_ctxs;
            listener_acquire_images(mock_listener,
                                    initial_remote_global_image_ids,
                                    &peer_ack_ctxs);
            ...
            mock_image_map->update_images("uuid1",
                                          std::
                                          move(initial_remote_global_image_ids),
                                          {
                                          });

            ASSERT_TRUE(wait_for_map_update(1));
            ASSERT_TRUE(wait_for_listener_notify
                        (initial_remote_global_image_ids_ack.size()));
            ...
            mock_image_map->update_images("",
                                          std::
                                          move(initial_local_global_image_ids),
                                          {
                                          });
            ...
            mock_image_map->update_images("uuid1", {
                                          ...
                                          std::
                                          move(remote_remove_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (remote_remove_global_image_ids_ack.size()));

            // RELEASE+REMOVE_MAPPING
            expect_add_event(mock_threads);
            listener_release_images(mock_listener,
                                    local_remove_global_image_ids,
                                    &peer_ack_ctxs);
            update_map_request(mock_threads, mock_update_request,
                               local_remove_global_image_ids, 0);

            // remove local images
            mock_image_map->update_images("", {
                                          },
                                          std::
                                          move(local_remove_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (local_remove_global_image_ids_ack.size()));

            remote_peer_ack_nowait(mock_image_map.get(),
                                   local_remove_global_image_ids_ack, 0,
                                   &peer_ack_remove_ctxs);
            remote_peer_ack_wait(mock_image_map.get(),
                                 local_remove_global_image_ids_ack, 0,
                                 &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, AddInstance) {
            ...
            mock_image_map->update_instances_added( {
                                                   m_local_instance_id});
            ...
            expect_add_event(mock_threads);
            expect_listener_images_unmapped(mock_listener, 3,
            ...
            wait_for_scheduled_task();
            ASSERT_TRUE(wait_for_listener_notify
                        (shuffled_global_image_ids.size()));

            update_map_and_acquire(mock_threads, mock_update_request,
                                   mock_listener, shuffled_global_image_ids, 0,
                                   &peer_ack_ctxs);
            remote_peer_ack_listener_wait(mock_image_map.get(),
                                          shuffled_global_image_ids, 0,
                                          &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, RemoveInstance) {
            ...
            expect_add_event(mock_threads);
            expect_listener_images_unmapped(mock_listener, 2,
                                            &shuffled_global_image_ids,
                                            &peer_ack_ctxs);

            mock_image_map->update_instances_removed( {
                                                     "9876"});
        ...
        TEST_F(TestMockImageMap, AddInstancePingPongImageTest) {
            ...
            std::set < std::string > global_image_ids {
            "global id 1", "global id 2", "global id 3", "global id 4",
                    "global id 5", "global id 6", "global id 7",
                    "global id 8", "global id 9", "global id 10",
                    "global id 11", "global id 12", "global id 13",
                    "global id 14"};

            std::map < std::string, cls::rbd::MirrorImageMap > image_mapping;
          for (auto & global_image_id:global_image_ids) {
                image_mapping[global_image_id] = {
                    m_local_instance_id, {
                    }
                    , {
                    }
            ...
            // RELEASE+UPDATE_MAPPING+ACQUIRE
            expect_add_event(mock_threads);
            expect_listener_images_unmapped(mock_listener, 7,
                                            &shuffled_global_image_ids,
                                            &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, RemoveInstanceWithRemoveImage) {
            ...
            mock_image_map->update_images("uuid1", {
                                          ...
                                          ,
                                          std::move(shuffled_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (shuffled_global_image_ids_ack.size() * 2));
            ...
            remote_peer_ack_nowait(mock_image_map.get(),
                                   shuffled_global_image_ids, -ENOENT,
                                   &peer_ack_remove_ctxs);
            remote_peer_ack_wait(mock_image_map.get(),
                                 shuffled_global_image_ids, -EBLOCKLISTED,
                                 &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, AddErrorAndRemoveImage) {
            ...
            expect_add_event(mock_threads);
            expect_listener_images_unmapped(mock_listener, 1,
            ...
            remote_peer_ack_nowait(mock_image_map.get(),
                                   shuffled_global_image_ids_ack, 0,
                                   &peer_ack_remove_ctxs);
            remote_peer_ack_wait(mock_image_map.get(),
                                 shuffled_global_image_ids_ack, 0,
                                 &peer_ack_ctxs);
        ...
        TEST_F(TestMockImageMap, MirrorUUIDUpdated) {
            ...
            mock_image_map->update_images("uuid1", {
                                          ...
                                          (remote_removed_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (remote_removed_global_image_ids_ack.size() * 2));
            ...
            mock_image_map->update_images("uuid2",
                                          std::
                                          move(remote_added_global_image_ids), {
                                          }
            );

            ASSERT_TRUE(wait_for_map_update(1));
            ASSERT_TRUE(wait_for_listener_notify
                        (remote_added_global_image_ids_ack.size()));
        ...
        TEST_F(TestMockImageMap, RebalanceImageMap) {
            ...
            // RELEASE+UPDATE_MAPPING+ACQUIRE
            expect_add_event(mock_threads);
            expect_listener_images_unmapped(mock_listener, 5,
                                            &shuffled_global_image_ids,
                                            &peer_ack_ctxs);
            ...
            mock_image_map->update_images("", {
                                          ...
                                          std::move(shuffled_global_image_ids));
            ASSERT_TRUE(wait_for_listener_notify
                        (shuffled_global_image_ids_ack.size()));
            ...
            expect_update_request(mock_update_request, 0);
            expect_add_event(mock_threads);
            listener_acquire_images(mock_listener, new_global_image_ids,
                                    &peer_ack_ctxs);
            ...
            ASSERT_TRUE(wait_for_map_update(1));
            ASSERT_TRUE(wait_for_listener_notify
                        (new_global_image_ids_ack.size()));

            // set rebalance interval
            CephContext *cct =
                reinterpret_cast < CephContext * >(m_local_io_ctx.cct());
            cct->_conf.set_val("rbd_mirror_image_policy_rebalance_timeout",
                               "5");
            remote_peer_ack_nowait(mock_image_map.get(),
                                   new_global_image_ids_ack, 0, &peer_ack_ctxs);
