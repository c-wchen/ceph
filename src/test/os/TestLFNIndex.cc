...
class TestWrapLFNIndex:public LFNIndex {
  public:
    TestWrapLFNIndex(CephContext * cct,
                     coll_t collection,
                     const char *base_path, uint32_t index_version)
    ...
    void test_generate_and_parse(const ghobject_t & hoid,
                                 const std::string & mangled_expected) {
  ...
  protected:
    ...
    int _created(const vector < string > &path,
                 ...
                 const string & mangled_name) override {
        return 0;
    }

    int _remove(const vector < string > &path,
                const ghobject_t & hoid, const string & mangled_name) override {
        return 0;
    }

    int _lookup(const ghobject_t & hoid,
                vector < string > *path,
                string * mangled_name, int *exists) override {
        return 0;
    }
...
TEST_F(TestHASH_INDEX_TAG, generate_and_parse_name)
{
    ...
    test_generate_and_parse(ghobject_t
                            (hobject_t
                             (object_t(".A/B_\\C.D"), key, CEPH_NOSNAP, hash,
                              pool, "")), "\\.A\\sB_\\\\C.D_head_ABABABAB");
    test_generate_and_parse(ghobject_t
                            (hobject_t
                             (object_t("DIR_A"), key, CEPH_NOSNAP, hash, pool,
                              "")), "\\dA_head_ABABABAB");
}
...
TEST_F(TestHASH_INDEX_TAG_2, generate_and_parse_name)
{
...

    {
        ...
        name[1] = '\0';
        ghobject_t
            hoid(hobject_t(object_t(name), key, CEPH_NOSNAP, hash, pool, ""));
...
TEST_F(TestHOBJECT_WITH_POOL, generate_and_parse_name)
{
    ...
    {
        ghobject_t
            hoid(hobject_t(object_t("DIR_A"), key, CEPH_NOSNAP, hash, pool, ""),
                 gen, shard_id);
        hoid.hobj.nspace = "NSPACE";
...
TEST_F(TestLFNIndex, remove_object)
{
    ...
    //
    {
        ...
        EXPECT_EQ(object_name_1.size(),
                  (unsigned)chain_setxattr(pathname.c_str(), LFN_ATTR.c_str(),
                                           object_name_1.c_str(),
                                           object_name_1.size()));
    ...
    //
    {
        ...
        EXPECT_EQ((unsigned)1,
                  (unsigned)chain_setxattr(pathname_1.c_str(), ATTR.c_str(),
                                           "Y", 1));
        ...
        char buffer[1] = { 0, };
        EXPECT_EQ((unsigned)1,
                  (unsigned)chain_getxattr(pathname.c_str(), ATTR.c_str(),
                                           buffer, 1));
...
TEST_F(TestLFNIndex, get_mangled_name)
{
    ...
    //
    {
        ...
        if (getuid() != 0) {
            EXPECT_EQ(-EACCES,
                      get_mangled_name(path, hoid, &mangled_name, &exists));
        }
        ...
        EXPECT_EQ(object_name_same_prefix.size(),
                  (unsigned)chain_setxattr(pathname.c_str(), LFN_ATTR.c_str(),
                                           object_name_same_prefix.c_str(),
                                           object_name_same_prefix.size()));
        std::string mangled_name_same_prefix;
        exists = 666;
        EXPECT_EQ(0,
                  get_mangled_name(path, hoid, &mangled_name_same_prefix,
                                   &exists));
