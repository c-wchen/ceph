...
struct md_config_t {
  private:
    ...
    typedef std::map < md_config_obs_t *,
        std::set < std::string >> rev_obs_map_t;
    typedef std::function < void (md_config_obs_t *,
                                  const std::string &) > config_gather_cb;
  ...
  public:
    typedef boost::variant < int64_t md_config_t::*,
        ...
        double md_config_t::*,
        bool md_config_t::*,
        entity_addr_t md_config_t::*, uuid_d md_config_t::*>member_ptr_t;
   ...
   */
    ...
    int get_val_from_conf_file(const std::vector < std::string > &sections,
                               std::string const &key, std::string & out,
                               bool emeta) const;
  ...
  private:
    ...
    int parse_option(std::vector < const char *>&args,
                     ...
                     std::ostream * oss);
    int parse_injectargs(std::vector < const char *>&args, std::ostream * oss);
    ...
    bool expand_meta(std::string & val, std::ostream * oss) const;
  ...
  public:                      // for global_init
    bool early_expand_meta(std::string & val, std::ostream * oss) const {
        Mutex::Locker l(lock);
...
template < typename T >
    ...
    template < typename U,
        ...
        typename boost::enable_if_c < !boost::is_same < T, U >::value,
        int >::type = 0 > T operator()(U & val) {
        assert("wrong type or option does not exist" == nullptr);
    }
};

template < typename T > T md_config_t::get_val(const std::string & key) const const
{
