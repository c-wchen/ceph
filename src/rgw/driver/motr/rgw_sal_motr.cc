...
int MotrBucket::check_index(const DoutPrefixProvider *dpp, std::map < RGWObjCategory,
...
std::unique_ptr < LuaManager > MotrStore::get_lua_manager(const DoutPrefixProvider *dpp,
...
int MotrObject::delete_part_objs(const DoutPrefixProvider* dpp)
{
    std::unique_ptr < rgw::sal::MultipartUpload > upload;
    upload = this->get_bucket()->get_multipart_upload(this->get_name(), string());
    std::unique_ptr < rgw::sal::MotrMultipartUpload > mupload(static_cast < rgw::sal::MotrMultipartUpload * >
            (upload.release()));
                                  ...
                                  prefix_map_t &processed_prefixes)
{
    ...
    do {
        ...
        for (auto obj_iter = parts.begin();
            ...
            ldpp_dout(dpp, 20) << "MotrMultipartUpload::complete(): off=" << off << ", accounted_size = " << accounted_size <<
                               dendl;
        }
    } while (truncated);
