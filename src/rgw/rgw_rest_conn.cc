...
static void populate_params(param_vec_t & params, const rgw_user * uid,
...
int RGWRESTConn::forward(const rgw_user & uid, req_info & info,
...
int RGWRESTConn::put_obj_init(const rgw_user & uid, rgw_obj & obj,
                              ...
                              RGWRESTStreamWriteRequest ** req)
{
    ...
    param_vec_t params;
    populate_params(params, &uid, self_zone_group);
    RGWRESTStreamWriteRequest *wr =
        new RGWRESTStreamWriteRequest(cct, url, NULL, &params);
...
int RGWRESTConn::complete_request(RGWRESTStreamWriteRequest * req,
                                  string & etag, real_time * mtime)
{
...
static void set_date_header(const real_time * t, map < string,
                            string > &headers, const string & header_name)
{
    if (!t) {
        return;
    }
...
int RGWRESTConn::get_obj(const rgw_user & uid, req_info * info /* optional */ ,
                         rgw_obj & obj,
                         ...
                         RGWGetDataCB * cb, RGWRESTStreamRWRequest ** req)
{
    ...
    if (prepend_metadata) {
        params.
            push_back(param_pair_t
                      (RGW_SYS_PARAM_PREFIX "prepend-metadata",
                       self_zone_group));
    }
    if (rgwx_stat) {
...
{
    int ret =
        req.send_request(&conn->get_key(), headers, resource, nullptr, mgr);
    if (ret < 0) {
        ldout(cct,
              5) << __func__ << ": send_request() resource=" << resource <<
            " returned ret=" << ret << dendl;
