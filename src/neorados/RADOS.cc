...
// Object
    ...
    Object::Object(Object && o) {
        static_assert(impl_size >= sizeof(object_t));
        new(&impl) object_t(std::
                            move(*reinterpret_cast < object_t * >(&o.impl)));
    }
...
// IOContext
  ...
  :    IOContext() {
    ...
    IOContext::IOContext(const IOContext & rhs) {
        static_assert(impl_size >= sizeof(IOContextImpl));
        new(&impl) IOContextImpl(*reinterpret_cast <
                                 const IOContextImpl * >(&rhs.impl));
    }
    ...
    IOContext::IOContext(IOContext && rhs) {
        static_assert(impl_size >= sizeof(IOContextImpl));
        new(&impl) IOContextImpl(std::
                                 move(*reinterpret_cast <
                                      IOContextImpl * >(&rhs.impl)));
    }
    ...
    void IOContext::ns(std::string && _ns) {
        reinterpret_cast < IOContextImpl * >(&impl)->oloc.nspace =
            std::move(_ns);
    }
    ...
    std::optional < std::uint64_t > IOContext::read_snap()const {
        auto & snap_seq =
            reinterpret_cast < const IOContextImpl *>(&impl)->snap_seq;
    ...
    void IOContext::write_snap_context(std::optional < std::pair <
                                       std::uint64_t,
        ...
        if (!_snapc) {
            ...
            SnapContext n(_snapc->first, {
                          _snapc->second.begin(), _snapc->second.end()}
            );
            if (!n.is_valid()) {
    ...
    std::ostream & operator <<(std::ostream & m, const IOContext & o) {
        ...
        return (m << l->oloc.pool << ":" << l->oloc.nspace << ":" << l->oloc.
                key);
    }
...
// Op
    ...
    void Op::set_excl() {
        reinterpret_cast <
            OpImpl * >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_EXCL);
    }
    void Op::set_failok() {
        reinterpret_cast <
            OpImpl * >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_FAILOK);
    }
    void Op::set_fadvise_random() {
        reinterpret_cast <
            OpImpl *
            >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_FADVISE_RANDOM);
    }
    void Op::set_fadvise_sequential() {
        reinterpret_cast <
            OpImpl *
            >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL);
    }
    void Op::set_fadvise_willneed() {
        reinterpret_cast <
            OpImpl *
            >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    }
    void Op::set_fadvise_dontneed() {
        reinterpret_cast <
            OpImpl *
            >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_FADVISE_DONTNEED);
    }
    void Op::set_fadvise_nocache() {
        reinterpret_cast <
            OpImpl *
            >(&impl)->op.set_last_op_flags(CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
    }

    void Op::cmpext(uint64_t off, bufferlist && cmp_bl, std::size_t * s) {
        reinterpret_cast < OpImpl * >(&impl)->op.cmpext(off, std::move(cmp_bl),
                                                        nullptr, s);
    }
    void Op::cmpxattr(std::string_view name, cmpxattr_op op,
                      const bufferlist & val) {
        reinterpret_cast < OpImpl * >(&impl)->op.cmpxattr(name,
                                                          std::uint8_t(op),
    ...
    void Op::assert_exists() {
        reinterpret_cast < OpImpl * >(&impl)->op.stat(nullptr,
                                                      static_cast <
                                                      ceph::real_time *
                                                      >(nullptr),
                                                      static_cast <
    ...
    void Op::exec(std::string_view cls, std::string_view method,
                  ...
                  fu2::unique_function < void (bs::error_code,
                                               const cb::list &) && >f) {
        reinterpret_cast < OpImpl * >(&impl)->op.call(cls, method, inbl,
                                                      std::move(f));
    }
    ...
    void Op::balance_reads() {
        reinterpret_cast < OpImpl * >(&impl)->op.flags |=
...
// ReadOp / WriteOp
    ...
    void ReadOp::sparse_read(uint64_t off, uint64_t len, cb::list * out,
                             ...
                             std::uint64_t >> *extents, bs::error_code * ec) {
        reinterpret_cast < OpImpl * >(&impl)->op.sparse_read(off, len, ec,
                                                             extents, out);
    }
    ...
    void ReadOp::get_omap_keys(std::optional < std::string_view > start_after,
                               ...
                               bool * done, bs::error_code * ec) {
        reinterpret_cast < OpImpl * >(&impl)->op.omap_get_keys(start_after,
                                                               max_return, ec,
                                                               keys, done);
    }

    void ReadOp::get_xattrs(bc::flat_map < std::string,
                            cb::list > *kv, bs::error_code * ec) {
    ...
    void ReadOp::get_omap_vals(std::optional < std::string_view > start_after,
                               ...
                               cb::list > *kv,
                               bool * done, bs::error_code * ec) {
        reinterpret_cast < OpImpl * >(&impl)->op.omap_get_vals(start_after,
                                                               filter_prefix,
                                                               max_return, ec,
                                                               kv, done);
    }

    void ReadOp::get_omap_vals_by_keys(const bc::flat_set < std::string > &keys,
                                       bc::flat_map < std::string,
                                       cb::list > *kv, bs::error_code * ec) {
        reinterpret_cast < OpImpl * >(&impl)->op.omap_get_vals_by_keys(keys, ec,
                                                                       kv);
    }
...
// WriteOp
    ...
    void WriteOp::set_alloc_hint(uint64_t expected_object_size,
        ...
        static_assert(random_write == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_WRITE));
        static_assert(sequential_read == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ));
        static_assert(random_read == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_READ));
        static_assert(append_only == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_APPEND_ONLY));
        static_assert(immutable == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_IMMUTABLE));
        static_assert(shortlived == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_SHORTLIVED));
        static_assert(longlived == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_LONGLIVED));
        static_assert(compressible == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_COMPRESSIBLE));
        static_assert(incompressible == static_cast <
                      int >(CEPH_OSD_ALLOC_HINT_FLAG_INCOMPRESSIBLE));

        reinterpret_cast <
            OpImpl * >(&impl)->op.set_alloc_hint(expected_object_size,
...
// RADOS
    ...
    void RADOS::Builder::build(boost::asio::io_context & ioctx,
        ...
        // pass back.
        {
            ...
            auto r =
                cct->_conf.parse_config_files(conf_files ? conf_files->
                                              data() : nullptr,
                                              &ss, flags);
            if (r < 0)
                c->post(std::move(c), ceph::to_error_code(r), RADOS {
                        nullptr}
            );
        }
      ...
      for (const auto &[n, v]:configs) {
            ...
            if (r < 0)
                c->post(std::move(c), ceph::to_error_code(-EINVAL), RADOS {
                        nullptr}
            );
        }
        ...
        if (!no_mon_conf) {
            ...
            if (err < 0)
                c->post(std::move(c), ceph::to_error_code(err), RADOS {
                        nullptr}
            );
        }
    ...
    void RADOS::make_with_cct(CephContext * cct,
        ...
        try {
            auto r =
                new detail::NeoClient { std::make_unique < detail::RADOS >
               (ioctx, cct) };
            r->objecter->
                wait_for_osd_map([c = std::move(c), r =
                                  std::unique_ptr < detail::Client >
                                  (r)] ()mutable {
                                 c->dispatch(std::move(c), bs::error_code {
                                             }, RADOS {
                                             std::move(r)});});
        } catch(const bs::system_error & err) {
            c->post(std::move(c), err.code(), RADOS {
                    nullptr});
        }
    }
    ...
    RADOS RADOS::make_with_librados(librados::Rados & rados) {
        return RADOS {
        std::make_unique < detail::RadosClient > (rados.client)};
    }
  ...
  :    impl(std::move(impl)) {
    ...
    void RADOS::execute(const Object & o, const IOContext & _ioc, ReadOp && _op,
        ...
        if (trace_info) {
            ...
            trace.init("rados execute", &impl->objecter->trace_endpoint,
                       &parent_trace);
        }

        trace.event("init");
        impl->objecter->read(*oid, ioc->oloc, std::move(op->op), ioc->snap_seq,
                             bl, flags, std::move(c), objver,
                             nullptr /* data_offset */ , 0 /* features */ ,
                             &trace);

        trace.event("submitted");
    }

    void RADOS::execute(const Object & o, const IOContext & _ioc, WriteOp
                        && _op, std::unique_ptr < WriteOp::Completion > c,
                        version_t * objver,
                        const blkin_trace_info * trace_info) {
        auto oid = reinterpret_cast < const object_t * >(&o.impl);
        auto ioc = reinterpret_cast < const IOContextImpl * >(&_ioc.impl);
        ...
        trace.event("init");
        impl->objecter->mutate(*oid, ioc->oloc, std::move(op->op), ioc->snapc,
                               mtime, flags, std::move(c), objver, osd_reqid_t {
                               }
                               , &trace);
        trace.event("submitted");
    }
    ...
    void RADOS::execute(const Object & o, std::int64_t pool, ReadOp && _op,
        ...
        impl->objecter->read(*oid, oloc, std::move(op->op), CEPH_NOSNAP, bl,
                             flags, std::move(c), objver);
    }
    ...
    void RADOS::execute(const Object & o, std::int64_t pool, WriteOp && _op,
        ...
        impl->objecter->mutate(*oid, oloc, std::move(op->op), {
                               }
                               , mtime, flags, std::move(c), objver);
    }
    ...
    boost::uuids::uuid RADOS::get_fsid()const noexcept {
        ...
        int64_t ret =
            ...
            with_osdmap(std::mem_fn(&OSDMap::lookup_pg_pool_name),
                        name);
        if (ret < 0) {
            impl->objecter->wait_for_latest_osdmap([name =
                                                    std::string(name), c =
                                                    std::move(c), objecter =
                                                    impl->objecter]
            ...
            ca::post(std::move(c), bs::error_code {
                     }
                     , ret);
        }
    }
    ...
    std::optional < uint64_t > RADOS::get_pool_alignment(int64_t pool_id) {
        return impl->objecter->with_osdmap([pool_id] (const OSDMap & o)->
                                           std::optional < uint64_t > {
                                           if (!o.have_pg_pool(pool_id)) {
                                           throw bs::system_error(ENOENT,
                                                                  bs::
                                                                  system_category
                                                                  (),
                                                                  "Cannot find pool in OSDMap.");}
                                           else
                                           if (o.get_pg_pool(pool_id)->
                                               requires_aligned_append()) {
                                           return o.get_pg_pool(pool_id)->
                                           required_alignment();}
                                           else {
                                           return std::nullopt;}
                                           }
    ...
    void RADOS::list_pools(std::unique_ptr < LSPoolsComp > c) {
        impl->objecter->with_osdmap([&](OSDMap & o) {
          std::vector < std::pair < std::int64_t, std::string >> v; for (auto p:o.get_pools())
                                    v.
                                    push_back(std::
                                              make_pair(p.first,
                                                        o.get_pool_name(p.
    ...
    void RADOS::create_pool_snap(std::int64_t pool,
        ...
        impl->objecter->create_pool_snap(pool, snapName,
                                         Objecter::PoolOp::OpComp::
                                         create(get_executor(),
                                                [c =
                                                 std::move(c)] (bs::
                                                                error_code e,
    ...
    void RADOS::allocate_selfmanaged_snap(int64_t pool,
        ...
        impl->objecter->allocate_selfmanaged_snap(pool,
                                                  ca::Completion <
                                                  void (bs::error_code,
                                                        snapid_t) >::
                                                  create(get_executor(),
    ...
    void RADOS::delete_pool_snap(std::int64_t pool,
                                 std::string_view snapName,
                                 std::unique_ptr < SimpleOpComp > c) {
        impl->objecter->delete_pool_snap(pool, snapName,
                                         Objecter::PoolOp::OpComp::
                                         create(get_executor(),
                                                [c =
                                                 std::move(c)] (bs::
    ...
    void RADOS::delete_selfmanaged_snap(std::int64_t pool,
                                        std::uint64_t snap,
                                        std::unique_ptr < SimpleOpComp > c) {
        impl->objecter->delete_selfmanaged_snap(pool, snap,
                                                Objecter::PoolOp::OpComp::
                                                create(get_executor(),
                                                       [c =
                                                        std::move(c)] (bs::
    ...
    void RADOS::create_pool(std::string_view name,
                            ...
                            std::unique_ptr < SimpleOpComp > c) {
        impl->objecter->create_pool(name,
                                    Objecter::PoolOp::OpComp::
                                    create(get_executor(),
                                           [c =
                                            std::move(c)] (bs::error_code e,
                                                           const bufferlist &)
                                           mutable {
    ...
    void RADOS::stat_pools(const std::vector < std::string > &pools,
        ...
        impl->objecter->get_pool_stats(pools,[c = std::move(c)]
                                       (bs::error_code ec,
                                        bc::flat_map < std::string,
                                        pool_stat_t > rawresult,
                                        bool per_pool) mutable {
                                       bc::flat_map < std::string,
                                       PoolStats > result;
                                       for (auto p = rawresult.begin();
                                            p != rawresult.end(); ++p) {
                                       ...
                                       uint64_t user_bytes =
                                       pstat.get_user_data_bytes(1.0,
                                                                 per_pool) +
                                       pstat.get_user_omap_bytes(1.0, per_pool);
                                       object_stat_sum_t * sum =
                                       &p->second.stats.sum;
                                       pv.num_kb =
                                       shift_round_up(allocated_bytes, 10);
                                       pv.num_bytes = allocated_bytes;
                                       pv.num_objects = sum->num_objects;
                                       pv.num_object_clones =
                                       sum->num_object_clones;
                                       pv.num_object_copies =
                                       sum->num_object_copies;
                                       pv.num_objects_missing_on_primary =
                                       sum->num_objects_missing_on_primary;
                                       pv.num_objects_unfound =
                                       sum->num_objects_unfound;
                                       pv.num_objects_degraded =
    ...
    void RADOS::stat_fs(std::optional < std::int64_t > _pool,
        ...
        if (_pool)
            pool = *pool;
        impl->objecter->get_fs_stats(pool,
                                     [c =
                                      std::move(c)] (bs::error_code ec,
                                                     const struct ceph_statfs s)
                                     mutable {
                                     FSStats fso {
                                     s.kb, s.kb_used, s.kb_avail,
                                     s.num_objects};
...
// --- Watch/Notify
    ...
    void RADOS::watch(const Object & o, const IOContext & _ioc,
                      std::optional < std::chrono::seconds > timeout, WatchCB
                      && cb, std::unique_ptr < WatchComp > c) {
        auto oid = reinterpret_cast < const object_t * >(&o.impl);
        auto ioc = reinterpret_cast < const IOContextImpl * >(&_ioc.impl);
        ...
        op.watch(cookie, CEPH_OSD_WATCH_OP_WATCH,
                 timeout.value_or(0 s).count());
        bufferlist bl;
        impl->objecter->linger_watch(linger_op, op, ioc->snapc,
                                     ceph::real_clock::now(), bl,
                                     Objecter::LingerOp::OpComp::
                                     create(get_executor(),
                                            [c =
                                             std::move(c),
                                             cookie] (bs::error_code e,
                                                      cb::list) mutable {
                                            ca::dispatch(std::move(c), e,
                                                         cookie);}), nullptr);
    }
    ...
    void RADOS::watch(const Object & o, std::int64_t pool,
        ...
        Objecter::LingerOp * linger_op =
            impl->objecter->linger_register(*oid, oloc, 0);
        uint64_t cookie = linger_op->get_cookie();
        linger_op->handle = std::move(cb);
        op.watch(cookie, CEPH_OSD_WATCH_OP_WATCH,
                 timeout.value_or(0 s).count());
        bufferlist bl;
        impl->objecter->linger_watch(linger_op, op, {
                                     }
                                     , ceph::real_clock::now(), bl,
                                     Objecter::LingerOp::OpComp::
                                     create(get_executor(),
    ...
    tl::expected < ceph::timespan,
        bs::error_code > RADOS::watch_check(uint64_t cookie) {
        Objecter::LingerOp * linger_op =
            reinterpret_cast < Objecter::LingerOp * >(cookie);
    ...
    void RADOS::unwatch(uint64_t cookie, const IOContext & _ioc,
        ...
        impl->objecter->mutate(linger_op->target.base_oid, ioc->oloc,
                               std::move(op), ioc->snapc,
                               ceph::real_clock::now(), ioc->extra_op_flags,
                               Objecter::Op::OpComp::create(get_executor(),
                                                            [objecter =
                                                             impl->objecter,
                                                             linger_op, c =
                                                             std::move(c)]
    ...
    struct NotifyHandler:std::enable_shared_from_this < NotifyHandler > {
        ...
        void operator() (bs::error_code ec, bufferlist && bl) {
            boost::asio::post(strand,[this, ec, p = shared_from_this()]()mutable {
                              finished = true; maybe_cleanup(ec);});
        }
    ...
    void RADOS::notify(const Object & o, const IOContext & _ioc, bufferlist
                       && bl,
        ...
        auto cb =
            std::make_shared < NotifyHandler > (impl->ioctx, impl->objecter,
        ...
        ObjectOperation rd;
        bufferlist inbl;
        rd.notify(linger_op->get_cookie(), 1,
                  timeout ? timeout->count() : impl->cct->_conf->
                  client_notify_timeout, bl, &inbl);

        impl->objecter->linger_notify(linger_op, rd, ioc->snap_seq, inbl,
                                      Objecter::LingerOp::OpComp::
                                      create(get_executor(),
                                             [cb] (bs::error_code ec,
                                                   ceph::
    ...
    void RADOS::notify(const Object & o, std::int64_t pool, bufferlist && bl,
        ...
        impl->objecter->linger_notify(linger_op, rd, CEPH_NOSNAP, inbl,
                                      Objecter::LingerOp::OpComp::
                                      create(get_executor(),
                                             [cb] (bs::error_code ec, bufferlist
                                                   && bl) mutable {
...
// Enumeration
    ...
    Cursor::Cursor(const Cursor & rhs) {
        static_assert(impl_size >= sizeof(hobject_t));
        new(&impl) hobject_t(*reinterpret_cast <
                             const hobject_t * >(&rhs.impl));
    }
    ...
    void RADOS::enumerate_objects(const IOContext & _ioc,
        ...
        impl->objecter->enumerate_objects < Entry > (ioc->oloc.pool,
                                                     ioc->oloc.nspace,
                                                     *reinterpret_cast <
                                                     const hobject_t *
                                                     >(&begin.impl),
    ...
    void RADOS::enumerate_objects(std::int64_t pool,
                                  ...
                                  std::optional < std::string_view > ns,
                                  std::optional < std::string_view > key) {
        impl->objecter->enumerate_objects < Entry > (pool,
                                                     ns ? *ns : std::
                                                     string_view {
                                                     },
                                                     *reinterpret_cast <
    ...
    void RADOS::osd_command(int osd, std::vector < std::string > &&cmd,
                            ceph::bufferlist
                            && in, std::unique_ptr < CommandComp > c) {
        impl->objecter->osd_command(osd, std::move(cmd), std::move(in), nullptr,
                                    [c = std::move(c)]
                                    (bs::error_code ec, std::string
                                     && s, ceph::bufferlist && b) mutable {
                                    ca::dispatch(std::move(c), ec, std::move(s),
                                                 std::move(b));});
    }
    void RADOS::pg_command(PG pg, std::vector < std::string > &&cmd,
                           ceph::bufferlist
                           && in, std::unique_ptr < CommandComp > c) {
        impl->objecter->pg_command(pg_t {
                                   pg.seed, pg.pool}, std::move(cmd),
                                   std::move(in), nullptr,[c = std::move(c)]
                                   (bs::error_code ec, std::string
                                    && s, ceph::bufferlist && b) mutable {
                                   ca::dispatch(std::move(c), ec, std::move(s),
                                                std::move(b));});
    ...
    void RADOS::enable_application(std::string_view pool,
                                   std::string_view app_name, bool force,
                                   std::unique_ptr < SimpleOpComp > c) {
        ...
        if (!impl->get_required_monitor_features().
    ...
    void RADOS::mon_command(std::vector < std::string > command,
        ...
        impl->monclient.start_mon_command(command, bl,
                                          [c =
                                           std::move(c), outs,
                                           outbl] (bs::error_code e,
                                                   std::string s,
                                                   cb::list bl) mutable {
                                          if (outs)
                                          * outs = std::move(s); if (outbl)
                                          * outbl = std::move(bl);
                                          ca::post(std::move(c), e);}
        ) ;
    }
...
#pragma clang diagnostic ignored "-Wnon-virtual-dtor"
    class category:public ceph::converting_category {
      public:
        ...
        const char *message(int ev, char *,
...
#pragma clang diagnostic pop
    ...
    bool category::equivalent(int ev,
...
namespace std {
    size_t hash < neorados::Object >::operator () (const neorados::Object & r)const {
        ...
        neorados::IOContext >::operator () (const neorados::
                                            IOContext & r)const {
        static constexpr const hash < int64_t > H;
        static constexpr const hash < std::string > G;
        const auto l =
            reinterpret_cast < const neorados::IOContextImpl * >(&r.impl);
        return H(l->oloc.
                 pool) ^ (G(l->oloc.nspace) << 1) ^ (G(l->oloc.key) << 2);
}}
