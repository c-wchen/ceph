...
TEST(LibRadosCmd, MonDescribe)
{
    ...
    cmd[0] = (char *)"asdfqwer";
    ASSERT_EQ(-EINVAL,
              rados_mon_command(cluster, (const char **)cmd, 1, "{}", 2, &buf,
                                &buflen, &st, &stlen));
    rados_buffer_free(buf);
    rados_buffer_free(st);
...
TEST(LibRadosCmd, MonDescribePP)
{
    ...
    string outs;
    ASSERT_EQ(0,
              cluster.mon_command("{\"prefix\": \"get_command_descriptions\"}",
                                  inbl, &outbl, &outs));
...
TEST(LibRadosCmd, OSDCmd)
{
    ...
    cmd[0] = (char *)"asdfasdf";
    r = rados_osd_command(cluster, 0, (const char **)cmd, 1, "", 0, &buf,
                          &buflen, &st, &stlen);
    rados_buffer_free(buf);
    rados_buffer_free(st);
...
TEST(LibRadosCmd, OSDCmdPP)
{
    ...
    cmd = "{\"prefix\":\"version\"}";
    r = cluster.osd_command(0, cmd, inbl, &outbl, &outs);
    ASSERT_TRUE((r == 0 && outbl.length() > 0)
                || (r == -ENXIO && outbl.length() == 0));
    cluster.shutdown();
}
...
TEST(LibRadosCmd, PGCmd)
{
    ...
    // note: tolerate NXIO here in case the cluster is thrashing out underneath us.
    int r =
        rados_pg_command(cluster, pgid.c_str(), (const char **)cmd, 1, "", 0,
                         &buf, &buflen, &st, &stlen);
    ASSERT_TRUE(r == -22 || r == -ENXIO);
    rados_buffer_free(buf);
    rados_buffer_free(st);
