  ...
  public:
    ...
    unsigned num_open_past_parents;
     ...
     SnapRealm(MDCache * c, CInode * in):
        srnode(),
        mdcache(c), inode(in),
        open(false), parent(0), num_open_past_parents(0), inodes_with_caps(0) {
    } bool exists(boost::string_view name) const {
        for (map < snapid_t, SnapInfo >::const_iterator p =
             srnode.snaps.begin(); p != srnode.snaps.end(); ++p) {
    ...
    bool _open_parents(MDSInternalContextBase * retryorfinish, snapid_t first =
                       1, snapid_t last = CEPH_NOSNAP);
    ...
    bool have_past_parents_open(snapid_t first = 1, snapid_t last =
                                CEPH_NOSNAP);
    ...
    bool has_past_parents() const {
        return !srnode.past_parents.empty();
                          ...
                          snapid_t last) const;
    void get_snap_info(map < snapid_t, SnapInfo * >&infomap, snapid_t first =
    ...
    snapid_t resolve_snapname(boost::string_view name, inodeno_t atino,
