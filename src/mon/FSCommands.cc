...
class FsNewHandler:public FileSystemCommandHandler {
  public:
    ...
    int handle(Monitor * mon,
        ...
        int64_t metadata =
            mon->osdmon()->osdmap.lookup_pg_pool_name(metadata_name);
        if (metadata < 0) {
      ...
      for (auto & fs:fsmap.get_filesystems()) {
            const std::vector < int64_t > &data_pools =
                fs->mds_map.get_data_pools();
        ...
        pg_pool_t const *metadata_pool =
            mon->osdmon()->osdmap.get_pg_pool(metadata);
        ceph_assert(metadata_pool != NULL); // Checked it existed above

        int r =
            _check_pool(mon->osdmon()->osdmap, data, POOL_DATA_DEFAULT, force,
                        &ss, allow_overlay);
        if (r < 0) {
            return r;
        }

        r = _check_pool(mon->osdmon()->osdmap, metadata, POOL_METADATA, force,
                        &ss, allow_overlay);
        if (r < 0) {
            return r;
        }
        ...
        if (info) {
            mon->clog->info() << info->
                human_name() << " assigned to filesystem " << fs_name <<
                " as rank 0";
...
class SetHandler:public FileSystemCommandHandler {
  public:
    ...
    :FileSystemCommandHandler("fs set") {
        ...
        if (var == "max_mds") {
            ...
            fsmap.modify_filesystem(fs->fscid,
                                    [is_down] (std::shared_ptr < Filesystem >
                                               fs) {
                                    if (is_down) {
                                    if (fs->mds_map.get_max_mds() > 0) {
                                    fs->mds_map.set_old_max_mds(); fs->mds_map.set_max_mds(0);} /* else already down! */
                                    }
                                    else {
                                    mds_rank_t oldmax =
                                    fs->mds_map.get_old_max_mds();
                                    fs->mds_map.
                                    set_max_mds(oldmax ? oldmax : 1);}
                                    }
            ) ;
            ...
            fsmap.modify_filesystem(fs->fscid,
                                    [joinable] (std::shared_ptr < Filesystem >
                                                fs) {
                                    if (joinable) {
                                    fs->mds_map.
                                    clear_flag(CEPH_MDSMAP_NOT_JOINABLE);}
            ...
            auto f =[allow] (auto & fs) {
                if (allow) {
                            ...
                            fsmap.modify_filesystem(fs->fscid,
                                                    ...
                                                    fs->mds_map.
                                                    set_flag
                                                    (CEPH_MDSMAP_REFUSE_CLIENT_SESSION);}
                            );
                            ss <<
                            ...
                            ss << "unknown variable " << var; return -EINVAL;}
  ...
  :                        FileSystemCommandHandler("fs compat") {
                            ...
                            static const std::set < std::string > subops = {
                            "rm_incompat", "rm_compat", "add_incompat",
                            "add_compat"}; std::string fs_name;
                            if (!cmd_getval(cmdmap, "fs_name", fs_name)
                                || fs_name.empty()) {
                            ss << "Missing filesystem name"; return -EINVAL;}
                            auto fs = fsmap.get_filesystem(fs_name);
                            if (fs == nullptr) {
                            ...
                            if (!cmd_getval(cmdmap, "subop", subop)
                                || subops.count(subop) == 0) {
                            ss << "subop `" << subop <<
                            "' not recognized. Must be one of: " << subops;
                            return -EINVAL;}

                            int64_t feature;
                            if (!cmd_getval(cmdmap, "feature", feature)
                                || feature <= 0) {
                            ss << "Invalid feature"; return -EINVAL;}
                            ...
                            ss << "feature with differing name `" << name <<
                            "' exists"; return -EEXIST;}
                            }
                            else {
                            cs.compat.insert(f);
  ...
  :                        FileSystemCommandHandler
                            ...
                            << " is not recognized"; return -EINVAL;}
                            string val;
                            if (!cmd_getval(cmdmap, "val", val)
                                || val.empty()) {
                            ss << "Missing feature id/name"; return -EINVAL;}
                            ...
                            fsmap.modify_filesystem(fs->fscid,
                                                    ...
                                                    return;
                                                    fs->mds_map.
                                                    add_required_client_feature
                                                    (feature); ret = true;}
                            ); if (ret) {
                            ss << "added feature '" <<
                            cephfs_feature_name(feature) <<
                            ...
                            fsmap.modify_filesystem(fs->fscid,
                                                    ...
                                                    return;
                                                    fs->mds_map.
                                                    remove_required_client_feature
                                                    (feature); ret = true;}
                            ); if (ret) {
                            ss << "removed feature '" <<
                            cephfs_feature_name(feature) <<
  ...
  :                        FileSystemCommandHandler("fs add_data_pool"),
                            ...
                            int64_t poolid =
                            mon->osdmon()->osdmap.lookup_pg_pool_name(poolname);
                            if (poolid < 0) {
                            string err;
                            ...
                            int r =
                            _check_pool(mon->osdmon()->osdmap, poolid,
                                        POOL_DATA_EXTRA, false, &ss);
                            if (r != 0) {
                            return r;}
                            ...
                            ss << "data pool " << poolid << " is already on fs "
                            << fs_name; return 0;}
  ...
  private:
Paxos * m_paxos;}; class SetDefaultHandler:public FileSystemCommandHandler
                            {
  public:
                            SetDefaultHandler()
  :                        FileSystemCommandHandler("fs set-default") {
                            ...
                            fsmap.set_legacy_client_fscid(fs->fscid); return 0;}
  ...
  :                        FileSystemCommandHandler("fs rm") {
      ...
      for (const auto & i: fs->mds_map.
                                 get_mds_info()) {
                            ceph_assert(i.second.state ==
  ...
  :                        FileSystemCommandHandler("fs reset") {
                            ...
                            fsmap.reset_filesystem(fs->fscid); return 0;}
  ...
  :                        FileSystemCommandHandler("fs rename"),
                            ...
                            ss << "File system '" << fs_name <<
                            "' does not exist"; return -ENOENT;}
                            }

                            if (new_fs) {
                            ss << "Desired file system name '" << new_fs_name <<
                            "' already in use"; return -EINVAL;}

                            if (fs->mirror_info.mirrored) {
                            ss << "Mirroring is enabled on file system '" <<
                            fs_name << "'. Disable mirroring on the "
      ...
      for (const auto p:   fs->mds_map.
                            ...
                            mon->osdmon()->do_application_enable(fs->mds_map.
                                                                 get_metadata_pool
                                                                 (),
                                                                 pg_pool_t::
                                                                 APPLICATION_NAME_CEPHFS,
                            ...
                            "system name."; return 0;}

  private:
Paxos * m_paxos;}; class RemoveDataPoolHandler:public FileSystemCommandHandler
                            {
  public:
...
template < typename T > class AliasHandler:public T {
  std::string alias_prefix; public:
                            explicit AliasHandler(const std::
                                                  string & new_prefix)
  :                        T() {
                            alias_prefix = new_prefix;}

                            std::string const &get_prefix()const override {
                            return alias_prefix;}
  ...
  :                        FileSystemCommandHandler("fs mirror peer_add") {
                            ...
                            extract_remote_cluster_conf(const std::
                                                        string & spec) {
                            auto pos = spec.find("@");
                            if (pos == std::string_view::npos) {
                            return boost::optional < std::pair < string,
                            string >> ();}
                            ...
                            bool peer_add(FSMap & fsmap, Filesystem::const_ref
                                          && fs, const cmdmap_t & cmdmap,
                                          std::ostream & ss) {
                            string peer_uuid; string remote_spec;
                            string remote_fs_name;
                            cmd_getval(cmdmap, "uuid", peer_uuid);
                            cmd_getval(cmdmap, "remote_cluster_spec",
                                       remote_spec);
                            cmd_getval(cmdmap, "remote_fs_name",
                                       remote_fs_name);
                            ...
                            ss << "peer already exists"; return true;}
                            if (fs->mirror_info.
                                has_peer((*remote_conf).first,
                                         (*remote_conf).second,
                                         remote_fs_name)) {
                            ss << "peer already exists"; return true;}

                            auto f =
                            [peer_uuid, remote_conf, remote_fs_name] (auto
                                                                      && fs) {
                            fs->mirror_info.peer_add(peer_uuid,
                                                     (*remote_conf).first,
                                                     (*remote_conf).second,
                                                     remote_fs_name);};
                            fsmap.modify_filesystem(fs->fscid, std::move(f));
                            ...
                            if (!fs->mirror_info.is_mirrored()) {
                            ss << "Mirroring not enabled for filesystem '" <<
                            fs_name << "'"; return -EINVAL;}

                            auto res =
                            peer_add(fsmap, std::move(fs), cmdmap, ss);
                            if (!res) {
                            return -EINVAL;}
  ...
  :                        FileSystemCommandHandler("fs mirror peer_remove")
                            ...
                            auto res =
                            peer_remove(fsmap, std::move(fs), cmdmap, ss);
                            if (!res) {
                            return -EINVAL;}
                            ...
                            handlers.push_back(std::make_shared <
                                               SetDefaultHandler > ());
                            handlers.push_back(std::make_shared < AliasHandler <
                                               SetDefaultHandler >
                                               >("fs set_default"));
                            handlers.push_back(std::make_shared <
                                               MirrorHandlerEnable > ());
                            ...
                            int FileSystemCommandHandler::
                            _check_pool(OSDMap & osd_map, const int64_t pool_id,
                                        int type, bool force, std::ostream * ss,
                                        bool allow_overlay) const {
                            ceph_assert(ss != NULL);
                            const pg_pool_t * pool =
                            osd_map.get_pg_pool(pool_id); if (!pool) {
                            ...
                            *ss << "pool(" << pool_id <<
                            ...
                            "can't attach pool to fs"; return -EOPNOTSUPP;}

                            const string & pool_name =
                            osd_map.get_pool_name(pool_id);
                            auto app_map = pool->application_metadata;
                            if (!allow_overlay && !force && !app_map.empty()) {
                            auto app =
                            app_map.find(pg_pool_t::APPLICATION_NAME_CEPHFS);
                            if (app != app_map.end()) {
                            ...
                            *ss << "RADOS pool '" << pool_name <<
                            "' is already used by filesystem '" << filesystem <<
                            "' as a '" << type << "' pool for application '" <<
                            app_name << "'"; return -EINVAL;}
                            }
                            else {
                            *ss << "RADOS pool '" << pool_name
                            ...
                            *ss << "pool '" << pool_name << "' (id '" << pool_id
                            ...
                            if (type == POOL_DATA_DEFAULT && !force) {
                            *ss << "pool '" << pool_name << "' (id '" << pool_id
                            ...
                            " Use --force to override."; return -EINVAL;}
                            else
                            ...
                            *ss << "pool '" << pool_name << "' (id '" << pool_id
                            ...
                            const pg_pool_t * write_tier = osd_map.get_pg_pool(pool->write_tier); ceph_assert(write_tier != NULL);  // OSDMonitor shouldn't allow DNE tier
                            if (write_tier->cache_mode ==
                                pg_pool_t::CACHEMODE_FORWARD
                                || write_tier->cache_mode ==
                                pg_pool_t::CACHEMODE_READONLY) {
                            *ss << "EC pool '" << pool_name <<
                            "' has a write tier (" << osd_map.
                            get_pool_name(pool->write_tier)
                            << ") that is configured "
                            "to forward writes.  Use a cache mode such as 'writeback' for "
                            "CephFS"; return -EINVAL;}
                            }
                            }

                            if (pool->is_tier()) {
                            *ss << " pool '" << pool_name << "' (id '" <<
                            pool_id << "') is already in use as a cache tier.";
                            return -EINVAL;}
                            ...
                            *ss << " pool '" << pool_name << "' (id '" <<
                            ...
                            int FileSystemCommandHandler::
                            is_op_allowed(const MonOpRequestRef & op,
                                          const FSMap & fsmap,
                                          const cmdmap_t & cmdmap,
                            ...
                            fsmap_copy.filter(op->get_session()->
                            ...
                            if (!(prefix == "fs rm" || prefix == "fs rename")
