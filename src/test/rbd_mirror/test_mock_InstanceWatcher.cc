...
namespace librbd {
    ...
    template <> struct ManagedLock <MockTestImageCtx > {
        ...
        MOCK_METHOD3(break_lock,
                     void (const managed_lock::Locker &, bool, Context *));
    };

    ManagedLock < MockTestImageCtx > *ManagedLock <
    ...
    namespace mirror {
        ...
        template <> struct Threads <librbd::MockTestImageCtx > {
         ...
         template <> struct InstanceReplayer <librbd::MockTestImageCtx > {
            MOCK_METHOD3(acquire_image,
                         void (InstanceWatcher < librbd::MockTestImageCtx > *,
                               const std::string &, Context *));
             MOCK_METHOD2(release_image, void (const std::string &, Context *));
             MOCK_METHOD3(remove_peer_image,
                          void (const std::string &, const std::string &,
                                Context *));
        };

         template <> struct ImageSyncThrottler <librbd::MockTestImageCtx > {
        ...
        ImageSyncThrottler < librbd::MockTestImageCtx > *ImageSyncThrottler <
...
namespace rbd {
    namespace mirror {
        ...
        class TestMockInstanceWatcher:public TestMockFixture {
          public:
            typedef librbd::ManagedLock < librbd::MockTestImageCtx >
                MockManagedLock;
            ...
            } void expect_register_watch(librados::
                                         MockTestMemIoCtxImpl & mock_io_ctx) {
            ...
            void expect_unregister_watch(librados::
                                         MockTestMemIoCtxImpl & mock_io_ctx) {
                EXPECT_CALL(mock_io_ctx, aio_unwatch(_, _));
            }

            void expect_register_instance(librados::
                                          MockTestMemIoCtxImpl & mock_io_ctx,
                                          int r) {
                EXPECT_CALL(mock_io_ctx,
                            exec(RBD_MIRROR_LEADER, _, StrEq("rbd"),
                                 StrEq("mirror_instances_add"), _, _, _))
                    .WillOnce(Return(r));
            }

            void expect_unregister_instance(librados::
                                            MockTestMemIoCtxImpl & mock_io_ctx,
                                            int r) {
                EXPECT_CALL(mock_io_ctx,
                            exec(RBD_MIRROR_LEADER, _, StrEq("rbd"),
            ...
            void expect_release_lock(MockManagedLock & mock_managed_lock, int r) {
                EXPECT_CALL(mock_managed_lock,
                            shut_down(_)).WillOnce(CompleteContext(r));
            }
            ...
            void expect_get_locker(MockManagedLock & mock_managed_lock,
                ...
                EXPECT_CALL(mock_managed_lock, get_locker(_, _))
                    .
                    WillOnce(Invoke
                             ([r, locker] (librbd::managed_lock::Locker * out,
                                           Context * ctx) {
                              if (r == 0) {
                              *out = locker;}
                              ctx->complete(r);}
        ...
        TEST_F(TestMockInstanceWatcher, InitShutdown) {
            ...
            auto instance_watcher =
                new MockInstanceWatcher(m_local_io_ctx,
                                        m_mock_threads->work_queue, nullptr,
                                        m_instance_id);
            InSequence seq;
        ...
        TEST_F(TestMockInstanceWatcher, RemoveNoent) {
            ...
            expect_get_locker(mock_managed_lock, librbd::managed_lock::Locker(),
                              -ENOENT);
            expect_unregister_instance(mock_io_ctx, 0);
            C_SaferCond on_destroy;
            expect_destroy_lock(mock_managed_lock, &on_destroy);
        ...
        TEST_F(TestMockInstanceWatcher, ImageAcquireRelease) {
            ...
            librados::
                MockTestMemIoCtxImpl & mock_io_ctx1(get_mock_io_ctx(io_ctx1));
            MockInstanceReplayer mock_instance_replayer1;
            auto instance_watcher1 =
                MockInstanceWatcher::create(io_ctx1, m_mock_threads->work_queue,
            ...
            EXPECT_EQ(0,
                      cluster.ioctx_create(_local_pool_name.c_str(), io_ctx2));
            std::string instance_id2 = stringify(io_ctx2.get_instance_id());
            librados::
                MockTestMemIoCtxImpl & mock_io_ctx2(get_mock_io_ctx(io_ctx2));
            MockInstanceReplayer mock_instance_replayer2;
            auto instance_watcher2 =
                MockInstanceWatcher::create(io_ctx2, m_mock_threads->work_queue,
            ...
            EXPECT_CALL(mock_instance_replayer1,
                        acquire_image(instance_watcher1, "gid", _))
                .WillOnce(WithArg < 2 > (CompleteContext(0)));
            C_SaferCond on_acquire1;
            instance_watcher1->notify_image_acquire(instance_id1, "gid",
                                                    &on_acquire1);
            ASSERT_EQ(0, on_acquire1.wait());

            // Acquire Image on the other instance
            EXPECT_CALL(mock_instance_replayer2,
                        acquire_image(instance_watcher2, "gid", _))
                .WillOnce(WithArg < 2 > (CompleteContext(0)));
            C_SaferCond on_acquire2;
            instance_watcher1->notify_image_acquire(instance_id2, "gid",
                                                    &on_acquire2);
            ...
            instance_watcher1->notify_image_release(instance_id1, "gid",
                                                    &on_release1);
            ...
            instance_watcher1->notify_image_release(instance_id2, "gid",
                                                    &on_release2);
        ...
        TEST_F(TestMockInstanceWatcher, PeerImageRemoved) {
            ...
            instance_watcher1->notify_peer_image_removed(instance_id1, "gid",
                                                         "uuid", &on_removed1);
            ...
            instance_watcher1->notify_peer_image_removed(instance_id2, "gid",
                                                         "uuid", &on_removed2);
        ...
        TEST_F(TestMockInstanceWatcher, ImageAcquireReleaseCancel) {
            ...
            EXPECT_CALL(mock_io_ctx, aio_notify(_, _, _, _, _))
                .WillOnce(Invoke([this, instance_watcher,
                                  &mock_io_ctx] (const std::string & o,
                                 ...
                                 auto ctx =
                                 new
                                 FunctionContext([instance_watcher,
                                                  &mock_io_ctx, c,
                                                  pbl] (int r) {
                                                 instance_watcher->
        ...
        class TestMockInstanceWatcher_NotifySync:public TestMockInstanceWatcher {
          public:
            typedef ImageSyncThrottler < librbd::MockTestImageCtx >
            ...
            void SetUp() override {
                ...
                EXPECT_EQ("", connect_cluster_pp(cluster));
                EXPECT_EQ(0,
                          cluster.ioctx_create(_local_pool_name.c_str(),
                                               io_ctx2));
            ...
            void expect_throttler_destroy(std::vector <
                                          Context * >*throttler_queue =
                                          nullptr) {
            ...
            void expect_throttler_start_op(const std::string & sync_id,
                ...
                EXPECT_CALL(mock_image_sync_throttler, start_op(sync_id, _))
                    .
                    WillOnce(Invoke
                             ([on_call, on_start_ctx] (const std::string &,
                                                       Context * ctx) {
                              if (on_call != nullptr) {
                              on_call->complete(0);}
        ...
        TEST_F(TestMockInstanceWatcher_NotifySync,
        ...
        TEST_F(TestMockInstanceWatcher_NotifySync, WaitingOnLeaderReleaseLeader) {
            ...
            std::vector < Context * >throttler_queue = {
            on_start_ctx};
