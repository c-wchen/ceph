...
string RGWRealm::get_predefined_id(CephContext * cct) const const
{
    return cct->_conf.get_val < string > ("rgw_realm_id");
}

const string & RGWRealm::get_predefined_name(CephContext * cct) const const
{
    return cct->_conf->rgw_realm;
}

int RGWRealm::create(const DoutPrefixProvider * dpp, optional_yield y,
                     bool exclusive)
{
    ...
    if (ret < 0 && ret != -EEXIST) {
        ldpp_dout(dpp,
                  0) << "WARNING: failed to set realm as default realm, ret=" <<
            ret << dendl;
    }
...
int RGWRealm::create_control(const DoutPrefixProvider * dpp, bool exclusive,
                             optional_yield y)
{
...
rgw_pool RGWRealm::get_pool(CephContext * cct) const const
{
...
const string RGWRealm::get_default_oid(bool old_format) const const
{
...
const string & RGWRealm::get_names_oid_prefix() const const
{
    return realm_names_oid_prefix;
}

const string & RGWRealm::get_info_oid_prefix(bool old_format) const const
{
    return realm_info_oid_prefix;
}

int RGWRealm::set_current_period(const DoutPrefixProvider * dpp,
                                 RGWPeriod & period, optional_yield y)
{
...
string RGWRealm::get_control_oid() const const
{
    return get_info_oid_prefix() + id + ".control";
}

int RGWRealm::notify_zone(const DoutPrefixProvider * dpp, bufferlist & bl,
                          optional_yield y)
{
    rgw_pool pool {
    get_pool(cct)};
...
int RGWRealm::notify_new_period(const DoutPrefixProvider * dpp,
                                const RGWPeriod & period, optional_yield y)
{
    bufferlist bl;
    using ceph::encode;
...
int RGWRealm::find_zone(const DoutPrefixProvider * dpp,
                        ...
                        bool * pfound, optional_yield y) const const
{
    auto & found = *pfound;
