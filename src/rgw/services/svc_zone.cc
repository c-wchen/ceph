...
std::shared_ptr < RGWBucketSyncPolicyHandler > RGWSI_Zone::get_sync_policy_handler(std::optional < rgw_zone_id > zone)
const
{
...
int RGWSI_Zone::do_start(optional_yield y, const DoutPrefixProvider *dpp)
{
    ...
    if (found_zone &&
        ...
        if (!found_period_conf) {
            ldpp_dout(dpp, 0) << "period (" << current_period->get_id() << " does not have zone " << cur_zone_id << " configured" <<
                              dendl;
        }
    }
    ...
    if (explicit_zg &&
        ...
        ldpp_dout(dpp, 0) << "ERROR: incorrect zonegroup: " << zonegroup_param << " (got: " << zonegroup_param << ", expected: "
    ...
    *zone_public_config = zone_iter->second;
    ...
    ldpp_dout(dpp, 4) << "ZoneGroup: " << std::left << setw(20) << zonegroup->get_name() << " (" << zonegroup->get_id() <<
                         ")" << dendl;
    ldpp_dout(dpp, 4) << "Zone:      " << std::left << setw(20) << zone_params->get_name() << " (" << zone_params->get_id()
                      << ")" << dendl;

    if (init_from_period) {
        ldpp_dout(dpp, 4) << "using period configuration: " << current_period->get_id() << ":" << current_period->get_epoch() <<
                          dendl;
    ...
    for (auto ziter : zonegroup->zones) {
        auto zone_handler = std::make_shared < RGWBucketSyncPolicyHandler > (this, sync_modules_svc, bucket_sync_svc,
                            ziter.second.id);
    ...
    for (const auto& ziter : zonegroup->zones) {
        ...
        RGWRESTConn *conn = new RGWRESTConn(cct, z.id, z.endpoints, zone_params->system_key, zonegroup->get_id(),
                                            zonegroup->api_name);
...
int RGWSI_Zone::init_zg_from_period(const DoutPrefixProvider *dpp, optional_yield y)
{
    ...
    for (iter = current_period->get_map().zonegroups.begin();
        ...
        add_new_connection_to_map(zonegroup_conn_map, zg, new RGWRESTConn(cct, zg.get_id(), endpoints, zone_params->system_key,
                                  zonegroup->get_id(), zg.api_name));
        if (!current_period->get_master_zonegroup().empty() &&
            zg.get_id() == current_period->get_master_zonegroup()) {
            rest_master_conn = new RGWRESTConn(cct, zg.get_id(), endpoints, zone_params->system_key, zonegroup->get_id(),
                                               zg.api_name);
        }
    }
...
int RGWSI_Zone::init_zg_from_local(const DoutPrefixProvider *dpp, optional_yield y)
{
    ...
    if (zonegroup->is_master_zonegroup()) {
        ...
        const auto& endpoints = master->second.endpoints;
        rest_master_conn = new RGWRESTConn(cct, zonegroup->get_id(), endpoints, zone_params->system_key, zonegroup->get_id(),
                                           zonegroup->api_name);
    }
