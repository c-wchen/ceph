...
void SnapshotPurgeRequest < I >::open_image()
{
    ...
    Context *ctx = create_context_callback <
                   ...
                   &SnapshotPurgeRequest < I >::handle_open_image > (this);
...
void SnapshotPurgeRequest < I >::snap_unprotect()
{
    ...
    int r =
        m_image_ctx->get_snap_namespace(snap_id, &m_snap_namespace);
    if (r < 0) {
        m_image_ctx->image_lock.unlock_shared();

        derr << "failed to get snap namespace: " << cpp_strerror(r)
             << dendl;
        m_ret_val = r;
        close_image();
        return;
    ...
    if (r < 0) {
        ...
        derr << "failed to get snap name: " << cpp_strerror(r) <<
             dendl;
        m_ret_val = r;
        close_image();
        return;
    ...
    if (r < 0) {
        ...
        derr << "failed to get snap protection status: " <<
             cpp_strerror(r)
             << dendl;
        m_ret_val = r;
        close_image();
    ...
    execute_snap_unprotect(m_snap_namespace,
...
void SnapshotPurgeRequest < I >::handle_snap_unprotect(int r)
{
    ...
    if (r == -EBUSY) {
        ...
        derr << "failed to unprotect snapshot: " << cpp_strerror(r)
             << dendl;
        m_ret_val = r;
        close_image();
        return;
...
void SnapshotPurgeRequest < I >::snap_remove()
{
    ...
    std::shared_lock owner_locker {
        m_image_ctx->owner_lock};
    m_image_ctx->operations->execute_snap_remove(m_snap_namespace,
            m_snap_name.
            c_str(), ctx);
}
...
void SnapshotPurgeRequest < I >::handle_snap_remove(int r)
{
    ...
    if (r == -EBUSY) {
        ...
        derr << "failed to remove snapshot: " << cpp_strerror(r) <<
             dendl;
        m_ret_val = r;
        close_image();
        return;
...
template class rbd::mirror::image_deleter::SnapshotPurgeRequest <
    librbd::ImageCtx >;
