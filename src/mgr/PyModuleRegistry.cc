...
std::string PyModule::get_site_packages()
{
    ...
    if (site_packages_fn != nullptr) {
        ...
        for (Py_ssize_t i = 0; i < n; ++i) {
            ...
            site_packages <<
                PyString_AsString(PyList_GetItem(site_packages_list, i));
        }
        ...
        bool first = true;
        for (Py_ssize_t i = 0; i < n; ++i) {
            dout(1) << "  " << PyString_AsString(PyList_GetItem(sys_path, i)) <<
                dendl;
            if (first) {
                first = false;
            }
...
void PyModuleRegistry::active_start(PyModuleConfig & config_,
                                    ...
                                    Finisher & f)
{
    ...
    active_modules.
        reset(new
              ActivePyModules(config_, ds, cs, mc, clog_, objecter_, client_,
                              f));
...
static void _list_modules(const std::string path,
