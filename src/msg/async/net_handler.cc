...
namespace ceph {
    ...
    int NetHandler::set_socket_options(int sd, bool nodelay, int size) {
        ...
        if (nodelay) {
            int flag = 1;
            r =::setsockopt(sd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag,
                            sizeof(flag));
            if (r < 0) {
                r = errno;
                ldout(cct,
        ...
        if (size) {
            r =::setsockopt(sd, SOL_SOCKET, SO_RCVBUF, (void *)&size,
                            sizeof(size));
            if (r < 0) {
                r = errno;
                ldout(cct,
        ...
        int val = 1;
        r =::setsockopt(sd, SOL_SOCKET, SO_NOSIGPIPE, (void *)&val,
                        sizeof(val));
        if (r) {
            r = errno;
            ldout(cct,
    ...
    int NetHandler::reconnect(const entity_addr_t & addr, int sd) {
        ...
        if (ret < 0 && errno != EISCONN) {
            r = errno;
            ldout(cct,
                  10) << __func__ << " reconnect: " << strerror(r) << dendl;
