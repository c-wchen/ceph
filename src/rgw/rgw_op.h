...
class RGWOp {
  ...
  public:
    RGWOp()
    :s(nullptr),
        dialect_handler(nullptr), store(nullptr), cors_exist(false), op_ret(0) {
    ...
    bool generate_cors_headers(string & origin, string & method,
...
class RGWGetObjTags:public RGWOp {
  ...
  public:
    ...
    virtual const string name() noexcept override {
        return "get_obj_tags";
    } virtual uint32_t op_mask() {
        return RGW_OP_TYPE_READ;
    } RGWOpType get_type() {
  ...
  public:
    ...
    class Deleter {
      ...
      public:
        ...
        :num_deleted(0), num_unfound(0), store(str), s(s) {
...
class RGWBulkUploadOp:public RGWOp {
  ...
  protected:
    ...
    bool handle_file_verify_permission(RGWBucketInfo & binfo,
                                       ...
                                       ACLOwner & bucket_owner /* out */ );
    int handle_file(boost::string_ref path, size_t size,
  ...
  :    num_created(0) {
    ...
    void init(RGWRados * const store,
              struct req_state *const s, RGWHandler * const h) override {
...
class RGWPutObj:public RGWOp {
  ...
  public:
    RGWPutObj():ofs(0),
        ...
        chunked_upload(0), dlo_manifest(NULL), slo_info(NULL), olh_epoch(0)
{
} ~RGWPutObj() override {
    delete slo_info;
}
...
virtual int get_encrypt_filter(std::unique_ptr < RGWPutObjDataProcessor >
                               *filter, RGWPutObjDataProcessor * cb) {
    *filter = nullptr;
    return 0;
}
...
class RGWPutObj_Filter:public RGWPutObjDataProcessor {
  ...
  public:
    RGWPutObj_Filter(RGWPutObjDataProcessor * next):next(next) {
...
class RGWDeleteObj:public RGWOp {
  ...
  public:
    ...
    :delete_marker(false),
        ...
        no_precondition_error(false), deleter(nullptr) {
    } int verify_permission() override;
    void pre_exec() override;
    void execute() override;
    int handle_slo_manifest(bufferlist & bl);
...
class RGWPutACLs:public RGWOp {
  ...
  public:
    ...
    virtual int get_policy_from_state(RGWRados * store, struct req_state *s,
                                      stringstream & ss) {
        return 0;
    }
    virtual int get_params() = 0;
...
class RGWOptionsCORS:public RGWOp {
  ...
  public:
    ...
    void execute() override;
    void get_response_params(string & allowed_hdrs, string & exp_hdrs,
                             unsigned *max_age);
    void send_response() override = 0;
    const string name() override {
        return "options_cors";
...
class RGWInitMultipart:public RGWOp {
  ...
  public:
    ...
    virtual int prepare_encryption(map < string, bufferlist > &attrs) {
        return 0;
    }
};
    ...
    if (olh_epoch) {
        ...
        if (!epoch_str.empty()) {
            ...
            *olh_epoch = strict_strtol(epoch_str.c_str(), 10, &err);
            if (!err.empty()) {
                lsubdout(s->cct, rgw,
                         0) << "failed to parse versioned-epoch param" << dendl;
                return -EINVAL;
            }
        }
    }
...
static inline void encode_delete_at_attr(boost::optional < ceph::real_time >
                                         delete_at, map < string,
                                         bufferlist > &attrs)
{
...
static inline void encode_obj_tags_attr(RGWObjTags * obj_tags, map < string,
                                        bufferlist > &attrs)
{
    if (obj_tags == nullptr) {
...
static inline int parse_value_and_bound(const string & input,
                                        int &output,
