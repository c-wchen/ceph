...
namespace crimson::os::seastore {
    ...
    void segment_info_t::set_open(segment_seq_t _seq, segment_type_t _type,
    ...
    void segment_info_t::init_closed(segment_seq_t _seq, segment_type_t _type,
                                     data_category_t _category,
                                     rewrite_gen_t _generation,
    ...
    std::ostream & operator<<(std::ostream & out, const segment_info_t & info) {
        ...
        if (info.is_empty()) {
            ...
            out << " " << info.type << " " << segment_seq_printer_t {
            info.seq}
            <<" " << info.category << " " << rewrite_gen_printer_t {
            info.generation}
            <<", modify_time=" << sea_time_point_printer_t {
            info.modify_time}
    ...
    void segments_info_t::init_closed(segment_id_t segment, segment_seq_t seq,
                                      segment_type_t type,
        ...
        DEBUG("initiating {} {} {} {} {}, {}, "
              ...
              segment, type, segment_seq_printer_t {
              seq}, category, rewrite_gen_printer_t {
              generation}, segment_info, num_empty, num_open, num_closed);
        ...
        segment_info.init_closed(seq, type, category, generation,
                                 get_segment_size());
    }

    void segments_info_t::mark_open(segment_id_t segment, segment_seq_t seq,
                                    segment_type_t type,
                                    data_category_t category,
                                    rewrite_gen_t generation) {
        ...
        if (type == segment_type_t::JOURNAL) {
            if (journal_segment_id != NULL_SEG_ID) {
                ...
                ceph_assert(last_journal_segment.type ==
    ...
    void segments_info_t::mark_empty(segment_id_t segment) {
        ...
        INFO("releasing {}, {}, num_segments(empty={}, opened={}, closed={})",
             segment, segment_info, num_empty, num_open, num_closed);
    ...
    std::ostream & operator<<(std::ostream & os, const segments_info_t & infos) {
        return os << "segments(" << "empty=" << infos.get_num_empty()
            ...
            << ", time_bound=" << sea_time_point_printer_t {
        infos.get_time_bound()}
        <<")";
    }
    ...
    void JournalTrimmerImpl::config_t::validate() const {
        ...
        JournalTrimmerImpl::config_t::get_default(std::size_t roll_size,
                                                  journal_type_t type) {
        assert(roll_size);
        ...
        return config_t {
            target_dirty_bytes, target_alloc_bytes, max_journal_bytes, 1 << 17, // rewrite_dirty_bytes_per_cycle
                1 << 24         // rewrite_backref_bytes_per_cycle
        };
    }

    JournalTrimmerImpl::config_t
        JournalTrimmerImpl::config_t::get_test(std::size_t roll_size,
                                               journal_type_t type) {
        assert(roll_size);
        std::size_t target_dirty_bytes = 0;
        std::size_t target_alloc_bytes = 0;
    ...
    JournalTrimmerImpl::JournalTrimmerImpl(BackrefManager & backref_manager,
                                           config_t config,
                                           journal_type_t type,
  ...
  :    
    ...
    config(config),
    journal_type(type),
    roll_start(roll_start), roll_size(roll_size), reserved_usage(0) {
        config.validate();
    ...
    void JournalTrimmerImpl::set_journal_head(journal_seq_t head) {
        ...
        if (journal_head.segment_seq == head.segment_seq) {
            DEBUG("journal_head {} => {}, {}",
                  head, journal_head, stat_printer_t {
                  *this, false}
            );
        }
        else {
            INFO("journal_head {} => {}, {}", head, journal_head, stat_printer_t {
                 *this, false}
            );
        }
    ...
    void JournalTrimmerImpl::update_journal_tails(journal_seq_t dirty_tail,
        ...
        if (dirty_tail != JOURNAL_SEQ_NULL) {
            ...
            if (journal_dirty_tail.segment_seq == dirty_tail.segment_seq) {
                DEBUG("journal_dirty_tail {} => {}, {}",
                      dirty_tail, journal_dirty_tail, stat_printer_t {
                      *this, false}
                );
            }
            else {
        ...
        if (alloc_tail != JOURNAL_SEQ_NULL) {
            ...
            if (journal_alloc_tail.segment_seq == alloc_tail.segment_seq) {
                DEBUG("journal_alloc_tail {} => {}, {}",
                      alloc_tail, journal_alloc_tail, stat_printer_t {
                      *this, false}
                );
            }
            else {
    ...
    journal_seq_t JournalTrimmerImpl::get_tail_limit() const {
        ...
        auto ret = journal_head.add_offset(journal_type,
        ...
        if (!background_callback->is_ready()) {
            return 0;
        } auto ret = journal_head.relative_to(journal_type,
                                              journal_dirty_tail,
                                              roll_start,
                                              roll_size);
        ceph_assert(ret >= 0);
    ...
    JournalTrimmerImpl::trim_ertr::future <> JournalTrimmerImpl::trim_alloc() {
        ...
        return repeat_eagain([this, FNAME] {
                             ...
                             with_transaction_intr(Transaction::src_t::
                                                   TRIM_ALLOC, "trim_alloc",
                                                   [this, FNAME] (auto & t) {
                                                   auto target =
                                                   get_alloc_tail_target();
                                                   DEBUGT
                                                   ("start, alloc_tail={}, target={}",
                                                    t, journal_alloc_tail,
                                                    target);
                                                   return backref_manager.
                                                   merge_cached_backrefs(t,
                                                                         target,
                                                                         config.
                                                   ...
                                                   si_then([this, FNAME,
                                                           ...
                                                           submit_transaction_direct_iertr::
                                                           future <> {
                                                           DEBUGT
                                                           ("trim_alloc_to={}",
                                                            t, trim_alloc_to);
    ...
    JournalTrimmerImpl::trim_ertr::future <> JournalTrimmerImpl::trim_dirty() {
        ...
        return repeat_eagain([this, FNAME] {
                             return extent_callback->
                             with_transaction_intr(Transaction::src_t::
                                                   TRIM_DIRTY, "trim_dirty",
                                                   [this, FNAME] (auto & t) {
                                                   auto target =
                                                   get_dirty_tail_target();
                                                   DEBUGT
                                                   ("start, dirty_tail={}, target={}",
                                                    t, journal_dirty_tail,
                                                    target);
                                                   return extent_callback->
    ...
    void JournalTrimmerImpl::register_metrics() {
        ...
        metrics.add_group("journal_trimmer", {
                          sm::make_counter("dirty_journal_bytes",[this] {
                                           return get_dirty_journal_size();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the size of the journal for dirty extents")),
                          sm::make_counter("alloc_journal_bytes",[this] {
                                           return get_alloc_journal_size();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the size of the journal for alloc info"))
                          }
        );
    }

    std::ostream & operator<<(std::ostream & os,
                              const JournalTrimmerImpl::
                              stat_printer_t & stats) {
        os << "JournalTrimmer(";
        if (stats.trimmer.background_callback->is_ready()) {
            os << "should_block_io_on_trim=" << stats.trimmer.
        ...
        if (stats.detailed) {
            ...
            if (stats.trimmer.background_callback->is_ready()) {
                os << ", alloc_tail_target=" << stats.trimmer.
                    get_alloc_tail_target()
                    << ", dirty_tail_target=" << stats.trimmer.
                    get_dirty_tail_target()
    ...
    bool SpaceTrackerSimple::equals(const SpaceTrackerI & _other)const {
        ...
        for (auto i = live_bytes_by_segment.begin(), j =
             other.live_bytes_by_segment.begin();
    ...
    int64_t SpaceTrackerDetailed::SegmentMap::
        allocate(device_segment_id_t segment, segment_off_t offset,
                 extent_len_t len, const extent_len_t block_size) {
        ...
        for (auto i = b; i < e; ++i) {
            if (bitmap[i]) {
                if (!error) {
                    ERROR("found allocated in {}, {} ~ {}", segment, offset,
                          len);
                    error = true;
                }
    ...
    int64_t SpaceTrackerDetailed::SegmentMap::
        release(device_segment_id_t segment, segment_off_t offset,
                extent_len_t len, const extent_len_t block_size) {
        ...
        for (auto i = b; i < e; ++i) {
            if (!bitmap[i]) {
                if (!error) {
                    ERROR("found unallocated in {}, {} ~ {}", segment, offset,
                          len);
                    error = true;
                }
    ...
    bool SpaceTrackerDetailed::equals(const SpaceTrackerI & _other)const {
        ...
        const auto & other =
            static_cast < const SpaceTrackerDetailed & >(_other);
    ...
    void SpaceTrackerDetailed::SegmentMap::dump_usage(extent_len_t block_size) const {
        ...
        INFO("{}", id);
        segment_usage[id].
            dump_usage(block_size_by_segment_manager[id.device_id()]);
        ...
        INFO("id: {}, live_bytes: {}",
             id, live_bytes_by_segment[id].live_bytes);
                                ...
                                const AsyncCleaner::stat_printer_t & stats) {
        stats.cleaner.print(os, stats.detailed);
        return os;
    }

    SegmentCleaner::SegmentCleaner(config_t config,
                                   SegmentManagerGroupRef && sm_group,
                                   BackrefManager & backref_manager,
                                   SegmentSeqAllocator & segment_seq_allocator,
                                   bool detailed, bool is_cold)
  :    
    detailed(detailed),
    is_cold(is_cold),
    config(config),
    ...
    void SegmentCleaner::register_metrics() {
        ...
        metrics.add_group(prefix, {
                          sm::make_counter("segments_number",[this] {
                          ...
                          sm::make_counter("segment_size",[this] {
                                           return segments.get_segment_size();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the bytes of a segment")),
                          sm::make_counter("segments_in_journal",[this] {
                                           return get_segments_in_journal();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the number of segments in journal")),
                          sm::make_counter("segments_type_journal",[this] {
                                           return segments.
                                           ...
                                           sm::
                                           description
                                           ("the number of segments typed journal")),
                          sm::make_counter("segments_type_ool",[this] {
                                           return segments.get_num_type_ool();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the number of segments typed out-of-line")),
                          sm::make_counter("segments_open",[this] {
                                           return segments.get_num_open();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the number of open segments")),
                          sm::make_counter("segments_empty",[this] {
                                           return segments.get_num_empty();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the number of empty segments")),
                          sm::make_counter("segments_closed",[this] {
                                           return segments.get_num_closed();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the number of closed segments")),
                          sm::make_counter("segments_count_open_journal",
                                           ...
                                           sm::
                                           description
                                           ("the count of open journal segment operations")),
                          sm::make_counter("segments_count_open_ool",[this] {
                                           return segments.get_count_open_ool();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the count of open ool segment operations")),
                          sm::make_counter("segments_count_release_journal",
                                           ...
                                           sm::
                                           description
                                           ("the count of release journal segment operations")),
                          sm::make_counter("segments_count_release_ool",[this] {
                                           return segments.
                                           ...
                                           sm::
                                           description
                                           ("the count of release ool segment operations")),
                          sm::make_counter("segments_count_close_journal",
                                           ...
                                           sm::
                                           description
                                           ("the count of close journal segment operations")),
                          sm::make_counter("segments_count_close_ool",[this] {
                                           return segments.
                                           get_count_close_ool();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the count of close ool segment operations")),
                          ...
                          sm::make_counter("available_bytes",[this] {
                                           return segments.
                                           get_available_bytes();
                                           }
                                           ,
                                           sm::
                                           description
                                           ("the size of the space is available")),
                          sm::make_counter("unavailable_unreclaimable_bytes",
                                           [this] {
                                           return
                                           get_unavailable_unreclaimable_bytes
                                           ();
                                           }
                                           ,
                                           sm::
                          ...
                          sm::make_counter("used_bytes", stats.used_bytes,
                                           sm::
                                           description
                                           ("the size of the space occupied by live extents")),
                          sm::make_counter("unavailable_unused_bytes",[this] {
                                           return
                          ...
                          sm::make_counter("projected_count",
                                           stats.projected_count,
                                           sm::
                                           description
                                           ("the number of projected usage reservations")),
                          sm::make_counter("projected_used_bytes_sum",
                                           stats.projected_used_bytes_sum,
                                           sm::
                                           description
                                           ("the sum of the projected usage in bytes")),
                          sm::make_counter("reclaimed_bytes",
                                           stats.reclaimed_bytes,
                                           sm::
                                           description
                                           ("rewritten bytes due to reclaim")),
                          ...
                          sm::make_counter("closed_journal_used_bytes",
                                           ...
                                           sm::
                                           description
                                           ("used bytes when close a journal segment")),
                          ...
                          sm::make_counter("closed_ool_used_bytes",
                                           ...
                                           sm::
                                           description
                                           ("used bytes when close a ool segment")),
                          sm::make_counter("closed_ool_total_bytes",
                                           stats.closed_ool_total_bytes,
                                           sm::
                                           description
                                           ("total bytes of closed ool segments")),
                          sm::make_gauge("available_ratio",[this] {
                                         return segments.get_available_ratio();
                                         }
                                         ,
                                         sm::
                                         description
                                         ("ratio of available space to total space")),
                          sm::make_gauge("reclaim_ratio",[this] {
                                         return get_reclaim_ratio();
                                         }
                                         ,
                                         sm::
                                         description
                                         ("ratio of reclaimable space to unavailable space")),
                          sm::make_histogram("segment_utilization_distribution",
                                             ...
                                             sm::
                                             description
                                             ("utilization distribution of all segments"))
                          }
        );
    }

    segment_id_t SegmentCleaner::allocate_segment(segment_seq_t seq,
                                                  segment_type_t type,
                                                  data_category_t category,
                                                  rewrite_gen_t generation) {
        ...
        for (auto it = segments.begin(); it != segments.end(); ++it) {
            ...
            if (segment_info.is_empty()) {
                ...
                INFO("opened, {}", stat_printer_t {
                     *this, false}
                );
                return seg_id;
            }
        }
        ERROR("out of space with {} {} {} {}", type, segment_seq_printer_t {
              seq}
              , category, rewrite_gen_printer_t {
              generation}
        );
        ceph_abort();
        return NULL_SEG_ID;
    ...
    void SegmentCleaner::close_segment(segment_id_t segment) {
        ...
        if (seg_info.type == segment_type_t::JOURNAL) {
            stats.closed_journal_used_bytes +=
                space_tracker->get_usage(segment);
        ...
        adjust_segment_util(old_usage, new_usage);
        INFO("closed, {} -- {}", stat_printer_t {
             *this, false}
             , seg_info);
    }

    double SegmentCleaner::calc_gc_benefit_cost(segment_id_t id,
                                                const sea_time_point & now_time,
                                                const sea_time_point &
                                                bound_time) const {
        ...
        if (likely(bound_time != NULL_TIME &&
                   modify_time != NULL_TIME && now_time > modify_time)) {
    ...
    SegmentCleaner::do_reclaim_space_ret
        SegmentCleaner::do_reclaim_space(const std::vector < CachedExtentRef >
                                         &backref_extents,
                                         const backref_pin_list_t & pin_list,
                                         std::size_t & reclaimed,
                                         std::size_t & runs) {
        return
            repeat_eagain([this, &backref_extents, &pin_list, &reclaimed,
                           &runs] {
                          reclaimed = 0; runs++;
                          auto src = Transaction::src_t::CLEANER_MAIN;
                          if (is_cold) {
                          src = Transaction::src_t::CLEANER_COLD;}
                          return extent_callback->with_transaction_intr(src,
                                                                        "clean_reclaim_space",
                                                                        [this,
                                                                         &backref_extents,
                                                                         &pin_list,
                  ...
                  auto cached_backref_entries = backref_manager.get_cached_backref_entries_in_range(reclaim_state->start_pos, reclaim_state->end_pos); backref_entry_query_set_t backref_entries; for (auto & pin:pin_list) {
                                                                         backref_entries.
                                                                         emplace
                                                                         (pin->
                                                                          get_key
                  ...
                  for (auto & cached_backref:                           cached_backref_entries) {
                                                                         ...
                                                                         DEBUGT
                                                                         ("start, backref_entries={}, backref_extents={}",
                                                                          t,
                                                                          backref_entries.
                                                                          size
                                                                          (),
                                  ...
                                  for (auto & e:                           list) {
                                                                         ...
                                                                         si_then
                                                                         ([FNAME, &extents, this, &reclaimed, &t] {
                                                                          DEBUGT
                                                                          ("reclaim {} extents",
                                                                           t,
                                                                           extents.
    ...
    SegmentCleaner::clean_space_ret SegmentCleaner::clean_space() {
        ...
        if (!reclaim_state) {
            ...
            INFO("reclaim {} {} start, usage={}, time_bound={}",
                 ...
                 sea_time_point_printer_t {
                 segments.get_time_bound()}
            );
            ceph_assert(segment_info.is_closed());
            reclaim_state =
                reclaim_state_t::create(seg_id, segment_info.generation,
                                        segments.get_segment_size());
        }
        reclaim_state->advance(config.reclaim_bytes_per_cycle);

        DEBUG("reclaiming {} {}~{}", rewrite_gen_printer_t {
              reclaim_state->generation}
        ...
        return seastar::do_with(std::pair < std::vector < CachedExtentRef >,
                                backref_pin_list_t > (),
            ...
            .safe_then([this, FNAME, pavail_ratio, start] (auto weak_read_ret) {
                       return seastar::do_with(std::move(weak_read_ret.first),
                                               std::move(weak_read_ret.second),
                                               (size_t) 0,
                                               (size_t) 0,
                                               [this, FNAME, pavail_ratio,
                                                start] (auto & backref_extents,
                                                        auto & pin_list,
                                                        auto & reclaimed,
                                                        auto & runs) {
                                               return
                                               do_reclaim_space(backref_extents,
                                                                pin_list,
                                                                reclaimed,
                                                                runs).
                                               safe_then([this, FNAME,
                                                         ...
                                                         now() - start;
                                                         DEBUG
                                                         ("duration: {}, pavail_ratio before: {}, repeats: {}",
                                                          d, pavail_ratio,
                                                          runs);
                                                         if (reclaim_state->
                                                             is_complete()) {
                                                         auto segment_to_release
                                                         =
                                                         reclaim_state->
                                                         get_segment_id();
                                                         INFO
    ...
    SegmentCleaner::mount_ret SegmentCleaner::mount() {
        ...
        space_tracker.reset(detailed ?
                            (SpaceTrackerI *) new SpaceTrackerDetailed(sms) :
      ...
      for (auto sm:sms) {
        ...
        return crimson::do_for_each(segments.begin(),
                                    segments.end(),[this, FNAME] (auto & it) {
                                    auto segment_id = it.first;
                                    return sm_group->
                                    read_segment_header(segment_id).
                                    safe_then([segment_id, this,
                                               FNAME] (auto header) {
                                              DEBUG("segment_id={} -- {}",
                                                    segment_id, header);
                                              ...
                                              ERROR
                                              ("got null segment, segment_id={} -- {}",
                                               segment_id, header);
                                              ceph_abort();}
                                              return sm_group->
                                              read_segment_tail(segment_id).
                                              safe_then([this, FNAME,
                                                         segment_id,
                                                         header] (auto tail)
                                                        ->
                                    ...
                                    handle_error(crimson::ct_error::enoent::
                                                 handle([](auto) {
                                                        return mount_ertr::
                                                        now();}
                                                 ),
    ...
    SegmentCleaner::scan_extents_ret SegmentCleaner::
        scan_no_tail_segment(const segment_header_t & segment_header,
        ...
        return seastar::do_with(scan_valid_records_cursor( {
                                ...
                                SegmentManagerGroup::found_record_handler_t([this, segment_id, segment_header, FNAME] (record_locator_t locator, const record_group_header_t & record_group_header, const bufferlist & mdbuf)mutable->SegmentManagerGroup::scan_valid_records_ertr::future <> {
                                                                            DEBUG
                                                                            ("{} {}, decoding {} records",
                                                                             segment_id,
                                                                             segment_header.
          ...
          for (auto & record_header:                                       *maybe_headers) {
                                                                            ...
                                                                            now
                                                                            ();}
                                ),[this, segment_header] (auto & cursor,
                                                          auto & handler) {
                                return sm_group->scan_valid_records(cursor,
                                                                    segment_header.
                                                                    segment_nonce,
                                                                    segments.
                                                                    get_segment_size
    ...
    bool SegmentCleaner::check_usage() {
        ...
        extent_callback->with_transaction_weak("check_usage",
                                               [this, &tracker] (auto & t) {
                                               return backref_manager.
                                               scan_mapped_space(t,
                                                                 [&tracker]
    ...
    void SegmentCleaner::mark_space_used(paddr_t addr, extent_len_t len) {
        ...
        [[maybe_unused]] auto ret =
            space_tracker->allocate(seg_addr.get_segment_id(),
        ...
        DEBUG("segment {} new len: {}~{}, live_bytes: {}",
              ...
              addr, len, space_tracker->get_usage(seg_addr.get_segment_id()));
    }
    ...
    void SegmentCleaner::mark_space_free(paddr_t addr, extent_len_t len) {
        ...
        auto old_usage = calc_utilization(seg_addr.get_segment_id());
        [[maybe_unused]] auto ret =
            space_tracker->release(seg_addr.get_segment_id(),
                                   seg_addr.get_segment_off(),
                                   len);
        auto new_usage = calc_utilization(seg_addr.get_segment_id());
        adjust_segment_util(old_usage, new_usage);
    ...
    segment_id_t SegmentCleaner::get_next_reclaim_segment() const {
      ...
      for (auto &[_id, segment_info]:segments) {
            if (segment_info.is_closed() &&
                ...
                double benefit_cost =
                    calc_gc_benefit_cost(_id, now_time, bound_time);
    ...
    bool SegmentCleaner::try_reserve_projected_usage(std::
                                                     size_t projected_usage) {
        assert(background_callback->is_ready());
    ...
    void SegmentCleaner::print(std::ostream & os, bool is_detailed) const {
        ...
        if (is_detailed) {
            os << ", unavailable_unreclaimable="
                ...
                << ", alive=" << stats.used_bytes << "B" << ", " << segments;
        }
        os << ")";
    }

    RBMCleaner::RBMCleaner(RBMDeviceGroupRef && rb_group,
                           BackrefManager & backref_manager, bool detailed)
  :    
    detailed(detailed),
    rb_group(std::move(rb_group)), backref_manager(backref_manager) {
    }
    ...
    RBMCleaner::mount_ret RBMCleaner::mount() {
        ...
        return seastar::do_with(rb_group->get_rb_managers(),[](auto & rbs) {
                                return crimson::do_for_each(rbs.begin(),
                                                            rbs.end(),
                                                            [](auto & it) {
                                                            return it->open().
                                                            handle_error
    ...
    bool RBMCleaner::equals(const RBMSpaceTracker & _other)const {
        ...
        for (auto i = other.block_usage.begin();
            ...
            auto state =
                rbm->
                get_extent_state(convert_abs_addr_to_paddr
                                 (addr, rbm->get_device_id()),
                                 rbm->get_block_size());
