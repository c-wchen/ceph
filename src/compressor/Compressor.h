...
class Compressor {
  public:
    ...
    const std::string & get_type_name() const {
        ...
        return alg;
    } virtual int compress(const ceph::bufferlist & in,
                           ceph::bufferlist & out) = 0;
    virtual int decompress(const ceph::bufferlist & in,
                           ceph::bufferlist & out) = 0;
    // this is a bit weird but we need non-const iterator to be in
    // alignment with decode methods
    virtual int decompress(ceph::bufferlist::iterator & p,
                           size_t compressed_len, ceph::bufferlist & out) = 0;
