...
namespace ceph {
    ...
    :m_cct(cct),
        ...
        m_unhealthy_workers(0), m_total_workers(0) {
    ...
    void HeartbeatMap::reset_timeout(heartbeat_handle_d * h, time_t grace,
        ...
        ldout(m_cct,
              20) << "reset_timeout '" << h->
            name << "' grace " << grace << " suicide " << suicide_grace <<
            dendl;
    ...
    bool HeartbeatMap::is_healthy() {
        ...
        if (m_cct->_conf->heartbeat_inject_failure) {
            ...
            m_inject_unhealthy_until =
                now + m_cct->_conf->heartbeat_inject_failure;
        ...
        if (now < m_inject_unhealthy_until) {
            ldout(m_cct,
                ...
                (m_inject_unhealthy_until - now) << " seconds" << dendl;
            healthy = false;
        }
