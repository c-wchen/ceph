...
public:
    ...
    entity_type_t get_mytype() const
    {
    ...
    virtual void set_policy(entity_type_t peer_type,
                            const SocketPolicy &p) = 0;

    virtual void set_policy_throttler(entity_type_t peer_type,
                                      Throttle *throttle) = 0;

    static MessengerRef create(const entity_name_t &name,
                               const std::string &lname, const uint64_t nonce);
...
template <> struct fmt::formatter <
    crimson::net::Messenger > : fmt::ostream_formatter {
};
#endif
