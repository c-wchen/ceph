...
ostream & operator<<(ostream & out, const OSDCapSpec & s)
{
    ...
    if (s.class_name.length())
        return out << "class '" << s.class_name << "' '" << s.
            class_allow << "'";
    return out;
}
...
bool OSDCapMatch::is_match(const string & pn, const string & ns,
                           int64_t pool_auid, const string & object) constconst
{
    if (auid >= 0) {
        if (auid != pool_auid)
            return false;
...
bool OSDCapGrant::is_capable(const string & pool_name, const string & ns,
                             ...
                             const std::vector < OpRequest::ClassInfo >
                             &classes,
            ...
            if (!classes.empty()) {
                ...
                for (size_t i = 0; i < classes.size(); ++i) {
                    ...
                    if (!spec.class_name.empty()
                        && classes[i].name == spec.class_name) {
    ...
    struct OSDCapParser:qi::grammar < Iterator, OSDCap() >
{
  OSDCapParser():OSDCapParser::base_type(osdcap) {
        ...
        grant = (*ascii::blank >>
                 ((lit("allow") >> capspec >> match)[_val =
                                                     phoenix::construct <
                                                     OSDCapGrant > (_2,
                                                                    _1)] |
                  (lit("allow") >> match >> capspec)[_val =
                                                     phoenix::construct <
                                                     OSDCapGrant > (_1,
                                                                    _2)] |
