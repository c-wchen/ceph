...
std::pair < bufferptr,
    std::vector <
    snapid_t >> create_object_snapset(const ScrubGenerator::RealObj & robj,
...
RealObjsConfList ScrubGenerator::make_real_objs_conf(int64_t pool_id,
                                                     const RealObjsConf &
                                                     blueprint,
                                                     std::vector < int32_t >
                                                     active_osds)
{
  ...
  for (auto osd:active_osds) {
        RealObjsConfRef this_osd_fakes =
            std::make_unique < RealObjsConf > (blueprint);
...
ScrubGenerator::SmapEntry ScrubGenerator::make_smobject(const ScrubGenerator::
                                                        RealObj & blueprint,
                                                        int osd_num)
{
    ...
    if (blueprint.snapset_mock_data) {
        ...
        std::cout << fmt::format("{}: ({}) osd:{} snaps:{}",
                                 __func__, ret.ghobj.hobj, osd_num, snaps)
            << std::endl;
    }
  ...
  for (const auto &[at_k, at_v]:blueprint.data.attrs) {
        ret.smobj.attrs[at_k] = ceph::buffer::copy(at_v.c_str(), at_v.size());
        {
            ...
            auto bk =
                ret.smobj.attrs[at_k].begin_deep().get_ptr(ret.smobj.
                                                           attrs[at_k].
                                                           length());
            std::string bkstr {
            bk.raw_c_str(), bk.raw_length()};
...
all_clones_snaps_t ScrubGenerator::all_clones(const ScrubGenerator::
                                              RealObj & head_obj)
{
    std::cout << fmt::format("{}: head_obj.ghobj.hobj:{}",
                             __func__, head_obj.ghobj.hobj)
        << std::endl;
