...
class AsyncConnection:public Connection {
    ...
    void _connect();
    void _stop();
    int handle_connect_reply(ceph_msg_connect & connect,
                             ceph_msg_connect_reply & r);
    ssize_t handle_connect_msg(ceph_msg_connect & m, bufferlist & aubl,
                               bufferlist & bl);
    ...
    void inject_delay();
    ssize_t _reply_accept(char tag, ceph_msg_connect & connect,
        ...
        if (reply.authorizer_len) {
            reply_bl.append(authorizer_reply.c_str(),
    ...
    Message *_get_next_outgoing(bufferlist * bl) {
        Message *m = 0;
        while (!m && !out_q.empty()) {
            map < int, list < pair < bufferlist,
                Message * >>>::reverse_iterator it = out_q.rbegin();
            if (!it->second.empty()) {
                list < pair < bufferlist, Message * > >::iterator p =
                    it->second.begin();
                m = p->second;
                if (bl)
                    bl->swap(p->first);
