...
namespace librbd {
    namespace io {
        ...
        struct TestMockIoSimpleSchedulerObjectDispatch:public TestMockFixture {
            ...
            MockSafeTimer m_mock_timer;
             ceph::mutex m_mock_timer_lock =
                ceph::
                make_mutex("TestMockIoSimpleSchedulerObjectDispatch::Mutex");

             TestMockIoSimpleSchedulerObjectDispatch() {
                MockTestImageCtx::set_timer_instance(&m_mock_timer,
                                                     &m_mock_timer_lock);
                EXPECT_EQ(0,
                          _rados.conf_set("rbd_io_scheduler_simple_max_delay",
                                          "1"));
            ...
            } void expect_dispatch_delayed_requests(MockTestImageCtx &
            ...
            void expect_add_timer_task(Context ** timer_task) {
                EXPECT_CALL(m_mock_timer, add_event_at(_, _))
                    ...
                    WillOnce(Invoke
                             ([timer_task]
                              (ceph::real_clock::time_point, Context * task) {
                              *timer_task = task; return task;}
                             ));
            }

            void expect_schedule_dispatch_delayed_requests(Context *
                                                           current_task,
                                                           Context **
                                                           new_task) {
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, Read) {
            ...
            0, 4096}, {
            8192, 4096}};
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         read(0, &extents, mock_image_ctx.get_data_io_context(),
                              0, 0, {
                              }, nullptr, nullptr, nullptr, &on_finish,
                              nullptr));
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, Write) {
            ...
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         write(0, 0, std::move(data),
                               ...
                               std::nullopt, {
                               }, &object_dispatch_flags, nullptr, nullptr,
                               &on_finish, nullptr));
            ASSERT_NE(on_finish, &cond);
            on_finish->complete(0);
            ASSERT_EQ(0, cond.wait());
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, WriteSame) {
            ...
            Context *on_finish = &cond;
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         write_same(0, 0, 4096, std::move(buffer_extents),
                                    std::move(data),
                                    mock_image_ctx.get_data_io_context(), 0, {
                                    }, nullptr, nullptr, nullptr, &on_finish,
                                    nullptr));
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, CompareAndWrite) {
            ...
            Context *on_finish = &cond;
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         compare_and_write(0, 0, std::move(cmp_data),
                                           std::move(write_data),
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, Flush) {
            ...
            C_SaferCond cond;
            Context *on_finish = &cond;
            ASSERT_TRUE(mock_simple_scheduler_object_dispatch.
                        flush(FLUSH_SOURCE_USER, {
                              }, nullptr, &dispatch_result, &on_finish,
                              nullptr));
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, WriteDelayed) {
            ...
            ASSERT_TRUE(mock_simple_scheduler_object_dispatch.
                        write(0, 0, std::move(data),
                              ...
                              &dispatch_result, &on_finish2, &on_dispatched));
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, WriteDelayedFlush) {
            ...
            ASSERT_TRUE(mock_simple_scheduler_object_dispatch.
                        flush(FLUSH_SOURCE_USER, {
                              }, nullptr, &dispatch_result, &on_finish3,
                              nullptr));
            ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, WriteMerged) {
            ...
            ASSERT_TRUE(mock_simple_scheduler_object_dispatch.
                        write(0, object_off, std::move(data),
                              mock_image_ctx.get_data_io_context(), 0, 0,
                              std::nullopt, {
                              }, &object_dispatch_flags, nullptr,
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, Mixed) {
            ...
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         discard(0, 4096, 4096,
                                 mock_image_ctx.get_data_io_context(), 0, {
                                 }, nullptr, nullptr, nullptr, &on_finish4,
                                 nullptr));
            ASSERT_NE(on_finish4, &cond4);
            ASSERT_EQ(0, on_dispatched2.wait());
            ASSERT_EQ(0, on_dispatched3.wait());
            ...
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         discard(0, 4096, 4096,
                                 mock_image_ctx.get_data_io_context(), 0, {
                                 }, nullptr, nullptr, nullptr, &on_finish6,
                                 nullptr));
            ASSERT_NE(on_finish6, &cond6);
            ASSERT_EQ(0, on_dispatched5.wait());
        ...
        TEST_F(TestMockIoSimpleSchedulerObjectDispatch, DispatchQueue) {
            ...
            Context *on_finish1 = &cond1;
            ASSERT_FALSE(mock_simple_scheduler_object_dispatch.
                         write(object_no, 0, std::move(data),
                               mock_image_ctx.get_data_io_context(), 0, 0,
                               std::nullopt, {
                               }, &object_dispatch_flags, nullptr, nullptr,
                               &on_finish1, nullptr));
