...
SnapshotRemoveRequest < MockTestImageCtx > *SnapshotRemoveRequest <
...
public:
    ...
    typedef ListWatchersRequest < MockTestImageCtx >
    MockListWatchersRequest;
    typedef librbd::operation::SnapshotRemoveRequest <
    MockTestImageCtx > MockSnapshotRemoveRequest;
    ...
    } void expect_set_exclusive_lock_policy(MockTestImageCtx &
                                            mock_image_ctx)
    {
    ...
    void expect_acquire_exclusive_lock(MockTestImageCtx &
                                       mock_image_ctx,
                                       MockExclusiveLock &
    ...
    void expect_shut_down_exclusive_lock(MockTestImageCtx &
                                         mock_image_ctx,
                                         MockExclusiveLock &
                                         mock_exclusive_lock, int r)
    {
    ...
    void expect_is_exclusive_lock_owner(MockTestImageCtx &
                                        mock_image_ctx,
                                        MockExclusiveLock &
                                        mock_exclusive_lock,
                                        bool is_owner)
    ...
    void expect_list_image_watchers(MockTestImageCtx &mock_image_ctx,
                                    MockListWatchersRequest &
                                    mock_list_watchers_request, int r)
    {
        EXPECT_CALL(mock_list_watchers_request, send())
        .
        WillOnce(FinishRequest
                 (&mock_list_watchers_request, r, &mock_image_ctx));
    }
    ...
    void expect_remove_snap(MockTestImageCtx &mock_image_ctx,
                            ...
                            mock_snap_remove_request, int r)
    {
        EXPECT_CALL(mock_snap_remove_request, send())
        .
        WillOnce(FinishRequest
                 (&mock_snap_remove_request, r, &mock_image_ctx));
    }
...
TEST_F(TestMockImagePreRemoveRequest, Success)
{
    ...
    expect_acquire_exclusive_lock(*m_mock_imctx, mock_exclusive_lock,
                                  0);
    expect_is_exclusive_lock_owner(*m_mock_imctx, mock_exclusive_lock,
                                   true);

    MockListWatchersRequest mock_list_watchers_request;
    expect_list_image_watchers(*m_mock_imctx,
                               mock_list_watchers_request, 0);
...
TEST_F(TestMockImagePreRemoveRequest,
       ExclusiveLockTryAcquireNotLockOwner)
{
    REQUIRE_FEATURE(RBD_FEATURE_EXCLUSIVE_LOCK);
...
TEST_F(TestMockImagePreRemoveRequest, Watchers)
{
    ...
    MockListWatchersRequest mock_list_watchers_request;
    expect_list_image_watchers(*m_mock_imctx,
                               mock_list_watchers_request, -EINVAL);
