 ...
 * and recv_body can be mixed. */
        class RestfulClient:public BasicClient {
          ...
          public:
             ...
             * errno. */ virtual size_t complete_header() = 0;
 ...
 * which could be provided both as a pointer-to-object or the object itself. */
         template < typename DecorateeT >
            ...
            typedef typename std::remove_pointer <
             ...
             static_assert(std::is_base_of < RestfulClient,
 ...
 * running on extremely fast network interfaces like the loopback). */
      template < size_t BufferSizeV = 4096 > class StaticOutputBufferer:public std::
            streambuf
        {
            ...
            int sync() override {
                const auto len =
                    static_cast < size_t >
                    (std::streambuf::pptr() - std::streambuf::pbase());
          ...
          :    sink(sink) {
                constexpr size_t len =
                    sizeof(buffer) - sizeof(std::streambuf::char_type);
