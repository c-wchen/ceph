...
inline std::ostream & operator<<(std::ostream & out,
                                 const rgw_data_sync_obligation & o)
{
    out << "key=" << o.bs;
    if (o.gen) {
        out << '[' << *o.gen << ']';
...
inline std::ostream & operator<<(std::ostream & out,
                                 const rgw_bucket_sync_pair_info & p)
{
...
struct rgw_data_sync_info {
    ...
    void dump(Formatter * f) const {
        ...
        case StateInit:
            s = "init";
            break;
            case StateBuildingFullSyncMaps:s = "building-full-sync-maps";
            break;
            case StateSync:s = "sync";
            break;
            default:s = "unknown";
            break;
...
void dump(Formatter * f) const {
    ...
    case FullSync:
        s = "full-sync";
        break;
        case IncrementalSync:s = "incremental-sync";
        break;
        default:s = "unknown";
        break;
    } encode_json("status", s, f);
...
{
} rgw_bucket_entry_owner(const std::string & _id,
                         const std::string & _display_name):id(_id),
...
struct RGWDataSyncEnv {
    ...
    void init(const DoutPrefixProvider * _dpp, CephContext * _cct,
              rgw::sal::RadosStore * _driver, RGWServices * _svc,
    ...
    std::ostream * ostr {
    nullptr};                   // For pretty printing progress
};

// pretty ostream output for `radosgw-admin bucket sync run`
#if FMT_VERSION >= 90000
template < typename ... T >
    void pretty_print(const RGWDataSyncEnv * env,
                      fmt::format_string < T ... >fmt, T && ... t)
{
#else
...
class LatencyConcurrencyControl:public LatencyMonitor {
  ...
  public:
    ...
    /// \brief Lower concurrency when latency rises////// Since we have multiple spawn windows (data sync overall and/// bucket), accept a number of concurrent operations to spawn and,/// if latency is high, cut it in half. If latency is really high,/// cut it to 1.
        int64_t adj_concurrency(int64_t concurrency) {
        using namespace std::literals;
        auto threshold = (cct->_conf->rgw_sync_lease_period * 1 s) / 12;

        if (avg_latency() >= 2 * threshold)
            [[unlikely]] {
            auto now = ceph::coarse_mono_clock::now();
            if (now - last_warning > 5 min) {
                ldout(cct, -1)
                    <<
...
class RGWRemoteDataLog:public RGWCoroutinesManager {
  ...
  public:
    RGWRemoteDataLog(const DoutPrefixProvider * dpp,
                     ...
                     RGWAsyncRadosProcessor * async_rados);
    int init(const rgw_zone_id & _source_zone, RGWRESTConn * _conn,
             RGWSyncErrorLogger * _error_logger,
             RGWSyncTraceManager * _sync_tracer,
             RGWSyncModuleInstanceRef & module, PerfCounters * _counters);
    void finish();

    int read_log_info(const DoutPrefixProvider * dpp,
                      rgw_datalog_info * log_info);
    int read_source_log_shards_info(const DoutPrefixProvider * dpp,
                                    std::map < int,
                                    RGWDataChangesLogInfo > *shards_info);
    int read_source_log_shards_next(const DoutPrefixProvider * dpp,
                                    std::map < int, std::string > shard_markers,
                                    std::map < int,
                                    rgw_datalog_shard_data > *result);
    int read_sync_status(const DoutPrefixProvider * dpp,
                         rgw_data_sync_status * sync_status);
    int read_recovering_shards(const DoutPrefixProvider * dpp,
                               const int num_shards,
                               std::set < int >&recovering_shards);
    int read_shard_status(const DoutPrefixProvider * dpp, int shard_id,
...
class RGWDataSyncStatusManager:public DoutPrefixProvider {
  ...
  public:
    RGWDataSyncStatusManager(rgw::sal::RadosStore * _driver,
                             RGWAsyncRadosProcessor * async_rados,
                             const rgw_zone_id & _source_zone,
                             PerfCounters * counters)
    :driver(_driver), source_zone(_source_zone), conn(NULL), error_logger(NULL),
        sync_module(nullptr), counters(counters), source_log(this, driver,
                                                             async_rados),
        num_shards(0) {
    } RGWDataSyncStatusManager(rgw::sal::RadosStore * _driver,
                               RGWAsyncRadosProcessor * async_rados,
    ...
    :driver(_driver), source_zone(_source_zone), conn(NULL), error_logger(NULL),
        sync_module(_sync_module), counters(counters), source_log(this, driver,
                                                                  async_rados),
        num_shards(0) {
    }
    ...
    static std::string shard_obj_name(const rgw_zone_id & source_zone,
                                      int shard_id);
    ...
    int read_recovering_shards(const DoutPrefixProvider * dpp,
                               const int num_shards,
                               std::set < int >&recovering_shards) {
        return source_log.read_recovering_shards(dpp, num_shards,
                                                 recovering_shards);
    }
    ...
    int read_shard_status(const DoutPrefixProvider * dpp, int shard_id,
                          ...
                          rgw_data_sync_marker * sync_marker,
                          const int max_entries) {
        return source_log.read_shard_status(dpp, shard_id, lagging_buckets,
    ...
    int read_source_log_shards_next(const DoutPrefixProvider * dpp,
                                    ...
                                    std::map < int,
                                    rgw_datalog_shard_data > *result) {
        return source_log.read_source_log_shards_next(dpp, shard_markers,
                                                      result);
    }

    int run(const DoutPrefixProvider * dpp) {
        return source_log.run_sync(dpp, num_shards);
    }
...
struct rgw_bucket_shard_sync_info {
    ...
    void decode_from_attrs(CephContext * cct, std::map < std::string,
                           bufferlist > &attrs);
...
class RGWBucketPipeSyncStatusManager:public DoutPrefixProvider {
    ...
    struct source {
        ...
        source(RGWDataSyncEnv * env, const rgw_zone_id & zone,
    ...
    int remote_info(const DoutPrefixProvider * dpp, source & s,
                    ...
                    uint64_t * num_shards);
  public:
    static tl::expected < std::unique_ptr < RGWBucketPipeSyncStatusManager >,
        int > construct(const DoutPrefixProvider * dpp,
                        rgw::sal::RadosStore * driver,
                        std::optional < rgw_zone_id > source_zone,
                        std::optional < rgw_bucket > source_bucket,
    ...
    int init_sync_status(const DoutPrefixProvider * dpp);
    tl::expected < std::map < int, rgw_bucket_shard_sync_info >,
        int >read_sync_status(const DoutPrefixProvider * dpp);
    int run(const DoutPrefixProvider * dpp);
};
