...
#include "common/config.h"
    ...
    template < typename missing_t >
        ...
        for (auto i = missing.get_items().begin();
            ...
            EXPECT_EQ(tcase.final.get_items().find(i->first)->second.need,
                      i->second.need);
            EXPECT_EQ(tcase.final.get_items().find(i->first)->second.have,
                      i->second.have);
        }
    ...
    void verify_sideeffects(const TestCase & tcase, const LogHandler & handler) {
        ...
        {
            list < pg_log_entry_t >::const_iterator titer =
                tcase.torollback.begin();
            list < pg_log_entry_t >::const_iterator hiter =
    ...
    void test_merge_log(const TestCase & tcase) {
        ...
        merge_log(oinfo, olog, pg_shard_t(1, shard_id_t(0)), info,
...
TEST_F(PGLogTest, merge_log)
{
    ...
    // noop
    {
        ...
        TestHandler h(remove_snap);
        merge_log(oinfo, olog, fromosd, info, &h, dirty_info, dirty_big_info);
...
TEST_F(PGLogTest, proc_replica_log)
{
...

    {
        ...
        EXPECT_EQ(eversion_t(1, 2),
                  omissing.get_items().at(divergent_object).need);
     ...
     */
    {
        ...
        EXPECT_TRUE(omissing.have_missing());
        EXPECT_TRUE(omissing.is_missing(divergent_object));
        EXPECT_EQ(omissing.get_items().at(divergent_object).have,
                  eversion_t(0, 0));
        EXPECT_EQ(omissing.get_items().at(divergent_object).need,
                  eversion_t(1, 1));
        EXPECT_EQ(last_update, oinfo.last_update);
    }
     ...
     */
    {
        ...
        EXPECT_TRUE(omissing.have_missing());
        EXPECT_TRUE(omissing.get_items().begin()->second.need ==
                    eversion_t(1, 1));
        EXPECT_EQ(last_update, oinfo.last_update);
...
{
    {
        ...
        osdmap->build_simple_with_pool(g_ceph_context, epoch, test_uuid,
                                       max_osd, bits, bits);
        ...
        // Some should be removed
        {
            ...
            pg_log_t::filter_log(pgid, *osdmap, hit_set_namespace, log,
                                 filtered, reject);
...
TEST_F(PGLogTest, get_request)
{
  ...
  for (auto & entry:entries) {
        ...
        int return_code = 0;
        bool got =
            log.get_request(entry.reqid, &replay_version, &user_version,
                            &return_code);
        EXPECT_TRUE(got);
...
TEST_F(PGLogTest, ErrorNotIndexedByObject)
{
    ...
    log.add(pg_log_entry_t
            (pg_log_entry_t::ERROR, oid, eversion_t(6, 2), eversion_t(3, 4), 1,
             osd_reqid_t(entity_name_t::CLIENT(777), 8, 1), utime_t(0, 1),
             -ENOENT));
    ...
    log.add(pg_log_entry_t
            (pg_log_entry_t::ERROR, oid, eversion_t(7, 5), eversion_t(7, 4), 3,
             osd_reqid_t(entity_name_t::CLIENT(777), 8, 4), utime_t(20, 1),
             -ENOENT));
  ...
  public:
    ...
    static pg_log_dup_t create_dup_entry(uint a, uint b) {
        ...
        return pg_log_dup_t(eversion_t(a, b),
                            a,
                            osd_reqid_t(entity_name_t::CLIENT(client_id++), 8,
                                        1), 0);
    }
...
TEST_F(PGLogTrimTest, TestPartialTrim)
{
    ...
    log.trim(cct, mk_evt(20, 164), &trimmed2, &trimmed_dups2,
             &write_from_dups2);
