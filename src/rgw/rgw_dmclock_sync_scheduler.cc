...
namespace rgw::dmclock {
    ...
    SyncScheduler::~SyncScheduler() {
        cancel();
    } int SyncScheduler::add_request(const client_id & client,
        ...
        ReqState::Wait};
        auto req =
            SyncRequest { client, time, cost, req_mtx, req_cv, rstate,
         counters };
        ...
        if (r == 0) {
            ...
            // Perform a blocking wait until the request callback is called
            {
                ...
                req_mtx};
                req_cv.wait(lock,[&rstate] {
                            return rstate != ReqState::Wait;
                            }
                );
            }
    ...
    void SyncScheduler::cancel(const client_id & client) {
        ...
        queue.remove_by_client(client, false,[&](RequestRef && request) {
                               sum.count++; sum.cost += request->cost; {
