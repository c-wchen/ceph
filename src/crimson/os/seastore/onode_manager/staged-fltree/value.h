...
inline std::ostream &operator<<(std::ostream &os,
                                const value_config_t &conf)
{
    return os << "ValueConf(" << conf.magic << ", " << conf.
           payload_size << "B)";
}
...
struct value_header_t {
    ...
    } const char *get_payload() const
    {
        return reinterpret_cast <
               const char * >(this) + sizeof(value_header_t);
    ...
    } void initiate(NodeExtentMutable &mut, const value_config_t &config)
    {
        value_header_t header {
            config.magic, config.payload_size};
...
inline std::ostream &operator<<(std::ostream &os,
                                const value_header_t &header)
{
    return os << "Value(" << header.magic << ", " << header.
           payload_size << "B)";
}
...
public:
    ...
    virtual void apply_value_delta(ceph::bufferlist::const_iterator &,
                                   NodeExtentMutable &, laddr_t) = 0;

protected:
    ValueDeltaRecorder(ceph::bufferlist &encoded): encoded {
        encoded}
    {
    }
...
public:
    ...
    Value(NodeExtentManager &, const ValueBuilder &,
          Ref < tree_cursor_t > &);
    ...
    std::pair < NodeExtentMutable &,
        ValueDeltaRecorderT * >prepare_mutate_payload(Transaction &t)
    {
        assert(is_tracked());
        ...
        assert(value_mutable.first.get_write() ==
               const_cast < const Value * >(this)->template read_payload <
                   char > ());
        assert(value_mutable.first.get_length() == get_payload_size());
        return {
            value_mutable.first,
            static_cast <
            ValueDeltaRecorderT * >(value_mutable.second)};
    }
    ...
    template < typename PayloadT > const PayloadT *read_payload() const
    {
        ...
        assert(sizeof(PayloadT) <= get_payload_size());
        return reinterpret_cast <
               const PayloadT * >(read_value_header()->get_payload());
    } private:
    ...
    std::pair < NodeExtentMutable &,
        ValueDeltaRecorder * >do_prepare_mutate_payload(Transaction &);
 ...
 * unavailable.
 */
std::unique_ptr < ValueDeltaRecorder > build_value_recorder_by_type(ceph::bufferlist &encoded,
        const value_magic_t &magic);
...
template <> struct fmt::formatter <
    crimson::os::seastore::onode::value_config_t > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::onode::value_header_t > : fmt::ostream_formatter {
};
#endif
