...
namespace journal {
    ...
    void JournalPlayer::prune_tag(uint64_t tag_tid) {
      ...
      for (auto & player_pair:m_object_players) {
        ...
        if (pruned) {
            ldout(m_cct,
                  15) << __func__ << ": reseting refetch state to immediate" <<
                dendl;
          for (auto & player_pair:m_object_players) {
                ObjectPlayerPtr object_player(player_pair.second);
                object_player->
    ...
    bool JournalPlayer::
        remove_empty_object_player(const ObjectPlayerPtr & player) {
        assert(m_lock.is_locked());
        assert(!m_watch_scheduled);
    ...
    void JournalPlayer::fetch(uint64_t object_num) {
        ...
        ObjectPlayerPtr object_player(new
                                      ObjectPlayer(m_ioctx, m_object_oid_prefix,
                                                   object_num,
                                                   m_journal_metadata->
                                                   get_timer(),
                                                   m_journal_metadata->
