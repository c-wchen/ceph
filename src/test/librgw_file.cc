...
{
    int ret =
        rgw_mount2(h_rgw, uid.c_str(), access_key.c_str(), secret_key.c_str(),
...
TEST(LibRGW, LOOKUP_BUCKETS)
{
  ...
  for (auto & fid_row:bucket_matrix) {
        ...
        ASSERT_EQ(0,
                  rgw_lookup(fs, fs->root_fh, get < 0 > (fid).c_str(), &rgw_fh,
                             nullptr /* stat */ , 0 /* mask */ ,
                             0 /* flags */ ));
        get < 2 > (fid) = rgw_fh;
...
}} TEST(LibRGW, LIST_OBJECTS)
{
  ...
  for (auto & fid_row:bucket_matrix) {
        ...
        ldout(g_ceph_context,
              0) << __func__ << " readdir on bucket " << get < 0 > (fid)
            << dendl;
      ...
      for (auto & obj:obj_vector) {
            std::cout << "fname: " << get < 0 > (obj) << " fid: " << get < 1 >
                (obj)
                << std::endl;
        }
        ASSERT_EQ(ret, 0);
    ...
    if (do_getattr) {
      ...
      for (auto & fid_row:bucket_matrix) {
          ...
          for (auto & obj:obj_vector) {
                ...
                ret =
                    rgw_lookup(fs, bucket_fh, get < 0 > (obj).c_str(), &obj_fh,
                               nullptr /* stat */ , 0 /* mask */ ,
                               0 /* flags */ );
                ASSERT_EQ(ret, 0);
                get < 2 > (obj) = obj_fh;   // stash obj_fh for cleanup
                ASSERT_NE(get < 2 > (obj), nullptr);
                ret = rgw_getattr(fs, obj_fh, &st, RGW_GETATTR_FLAG_NONE);  // now, getattr
                ASSERT_EQ(ret, 0);
            }
        }
    }
