...
public:
    ...
    PGCommand(crimson::osd::OSD &osd,
              ...
              std::string_view desc, std::string_view help)
        : AdminSocketHook {
        prefix, desc, help}
    , osd {
        osd}
                                           ...
                                           && input)const final
    {
        ...
        return osd.get_pg_shard_manager().with_pg(spg_id,
                [this, spg_id,
                 cmdmap =
                     std::move(cmdmap),
                 format =
...
private:
    virtual seastar::future < tell_result_t > do_command(Ref < PG > pg,
...
public:
    // TODO: const correctness of osd
    explicit QueryCommand(crimson::osd::OSD &osd): PGCommand {
        osd, "query", "", "show details of a specific pg"}
    {
    }
private:
...
{
public:
    explicit MarkUnfoundLostCommand(crimson::osd::OSD &osd): PGCommand {
    ...
    seastar::future < tell_result_t > do_command(Ref < PG > pg,
            const cmdmap_t &cmdmap,
            std::string_view, ceph::bufferlist &&)const final
    {
...
make_asok_hook < crimson::admin::pg::QueryCommand >
(crimson::osd::OSD &osd);
