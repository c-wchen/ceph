...
{
public:
    ImageStatRequest(librados::IoCtx &io_ctx,
        ...
        : m_cct(reinterpret_cast < CephContext * >(io_ctx.cct())),
          ...
          m_image_id(image_id), m_scan_snaps(scan_snaps),
          m_bytes(bytes), m_max_bytes(max_bytes), m_snaps(snaps)
    {
        m_throttle.start_op();
    } void send()
    {
...
private:
    ...
    void get_head()
    {
        ...
        auto aio_comp = util::create_rados_callback <
                        ImageStatRequest < I >,
                        &ImageStatRequest < I >::handle_get_head > (this);
        int r =
            m_io_ctx.aio_operate(util::header_name(m_image_id),
                                 aio_comp, &op,
                                 &m_out_bl);
        ceph_assert(r == 0);
        aio_comp->release();
    ...
    void handle_get_head(int r)
    {
        ...
        if (r == 0) {
            uint8_t order;
            r = cls_client::get_size_finish(&it, &m_max_size,
                                            &order);
            if (r == 0) {
                (*m_bytes) += m_max_size;
            }
        ...
        if (r == -ENOENT) {
            ...
            lderr(m_cct) << "failed to stat image: " <<
    ...
    void get_snaps()
    {
        ...
        auto aio_comp = util::create_rados_callback <
                        ImageStatRequest < I >,
                        &ImageStatRequest < I >::handle_get_snaps > (this);
        int r =
            m_io_ctx.aio_operate(util::header_name(m_image_id),
                                 aio_comp, &op,
                                 &m_out_bl);
    ...
    void handle_get_snaps(int r)
    {
        ...
        auto it = m_out_bl.cbegin();
        for ([[maybe_unused]] auto snap_seq : m_snapc.
             snaps) {
            uint64_t size;
            if (r == 0) {
                uint8_t order;
        ...
        if (r == -ENOENT) {
            ...
            lderr(m_cct) << "failed to retrieve snap size: " <<
                         cpp_strerror(r)
                         << dendl;
            finish(r);
        } else {
...
template < typename I >
void get_pool_stat_option_value(typename Pool <
                                I >::StatOptions *stat_options,
...
template < typename I >
int get_pool_stats(librados::IoCtx &io_ctx,
                   ...
                   uint64_t *snapshot_count)
{
    ...
    SimpleThrottle throttle(config.template get_val < uint64_t >
    ...
    for (auto &image_id : image_ids) {
        ...
        auto req =
            new ImageStatRequest < I > (io_ctx, throttle, image_id,
                                        scan_snaps, &bytes,
                                        &max_bytes, &snaps);
        req->send();
    }
...
int Pool < I >::init(librados::IoCtx &io_ctx, bool force)
{
    ...
    int r =
        io_ctx.application_enable(pg_pool_t::APPLICATION_NAME_RBD,
                                  force);
    if (r < 0) {
        return r;
    }
...
template < typename I >
int Pool < I >::get_stats(librados::IoCtx &io_ctx,
                          StatOptions *stat_options)
{
    ...
    get_pool_stat_option_value < I > (stat_options,
    ...
    if (image_count != nullptr || provisioned_bytes != nullptr
        ...
        r = get_pool_stats < I > (io_ctx, config, image_ids,
                                  image_count, provisioned_bytes,
                                  max_provisioned_bytes,
                                  snapshot_count);
        if (r < 0) {
            return r;
