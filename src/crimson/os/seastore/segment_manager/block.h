 ...
 */
    class SegmentStateTracker {
      ...
      public:
        ...
        :bptr(ceph::buffer::
              create_page_aligned(get_raw_size(segments, block_size))),
            layout(bptr.length()) {
            ::memset(bptr.c_str(), static_cast < char >(segment_state_t::EMPTY),
                     bptr.length());
            ...
            return static_cast < segment_state_t > (layout.template Pointer <
                                                    0 > (bptr.c_str())[offset]);
        ...
        write_ertr::future <> write_out(device_id_t device_id,
                                        seastar::file & device,
                                        uint64_t offset);

        read_ertr::future <> read_in(device_id_t device_id,
    ...
    class BlockSegment final:public Segment {
      ...
      public:
        ...
        segment_id_t get_segment_id() const final {
            return id;
        } segment_off_t get_write_capacity() const final;
        segment_off_t get_write_ptr() const final {
            return write_pointer;
        } close_ertr::future <> close() final;
        write_ertr::future <> write(segment_off_t offset,
...
// interfaces used by each shard device
      ...
      :    device_path(path) {
        ...
        Segment::write_ertr::future <> segment_write(paddr_t addr,
                                                     ceph::bufferlist bl,
        ...
        size_t get_offset(paddr_t addr) {
            ...
            return shard_info.first_segment_offset +
                (seg_addr.get_segment_id().device_segment_id() *
                 superblock.segment_size) + seg_addr.get_segment_off();
        }
