...
{
    std::unique_ptr < AdminSocket > asokc(new AdminSocket(g_ceph_context));
...
TEST(AdminSocket, RegisterCommand)
{
    ...
    AdminSocketClient client(get_rand_socket_path());
    ASSERT_EQ(0,
              asoct.m_asokc->register_command("test", "test", new MyTest(),
                                              ""));
    string result;
...
TEST(AdminSocket, RegisterCommandPrefixes)
{
    ...
    AdminSocketClient client(get_rand_socket_path());
    ASSERT_EQ(0,
              asoct.m_asokc->register_command("test",
                                              "test name=args,type=CephString,n=N",
                                              new MyTest(), ""));
    ASSERT_EQ(0,
              asoct.m_asokc->register_command("test command",
                                              "test command name=args,type=CephString,n=N",
                                              new MyTest2(), ""));
    string result;
    ASSERT_EQ("", client.do_request("{\"prefix\":\"test\"}", &result));
    ASSERT_EQ("test|", result);
