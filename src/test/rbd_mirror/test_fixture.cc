    ...
    namespace mirror {
        ...
        uint64_t TestFixture::_image_number = 0;
         ...
         TestFixture::TestFixture() {
            ...
            _rados =
                std::shared_ptr < librados::Rados > (new librados::Rados());
            ...
            ASSERT_EQ(0,
                      _rados->ioctx_create(_local_pool_name.c_str(),
                                           local_ioctx));
            ...
            ASSERT_EQ(0,
                      _rados->ioctx_create(_remote_pool_name.c_str(),
                                           remote_ioctx));
        ...
        void TestFixture::SetUp() {
            ...
            m_threads =
                new rbd::mirror::Threads <> (reinterpret_cast <
                                             CephContext *
                                             >(m_local_io_ctx.cct()));
        }
        ...
        int TestFixture::create_image(librbd::RBD & rbd,
                                      librados::IoCtx & ioctx,
                                      const std::string & name, uint64_t size) {
            int order = 18;
            return rbd.create2(ioctx, name.c_str(), size, RBD_FEATURES_ALL,
                               &order);
        }
        ...
        int TestFixture::open_image(librados::IoCtx & io_ctx,
            ...
            *image_ctx =
                new librbd::ImageCtx(image_name.c_str(), "", NULL, io_ctx,
                                     false);
            m_image_ctxs.insert(*image_ctx);
            return (*image_ctx)->state->open(false);
        }

        int TestFixture::create_snap(librbd::ImageCtx * image_ctx,
