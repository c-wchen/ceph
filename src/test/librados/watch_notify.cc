                                                                 ...
                                                                 int err)
{
    ...
    ceph_assert(thiz);
    thiz->notify_err =
        rados_aio_unwatch(thiz->ioctx, cookie, thiz->notify_comp);
    ASSERT_EQ(0,
              rados_aio_create_completion2(nullptr, nullptr,
                                           &thiz->notify_comp));
    thiz->notify_cookies.erase(cookie); //delete old cookie
    thiz->notify_err =
        rados_aio_watch(thiz->ioctx, thiz->notify_oid, thiz->notify_comp,
                        &cookie, watch_notify2_test_cb,
                        watch_notify2_test_errcb_aio_reconnect, thiz);
...
TEST_F(LibRadosWatchNotify, WatchNotify)
{
    ...
    uint64_t handle;
    ASSERT_EQ(0,
              rados_watch(ioctx, "foo", 0, &handle, watch_notify_test_cb,
                          NULL));
    ...
    // when dne ...
    ASSERT_EQ(-ENOENT,
              rados_watch(ioctx, "dne", 0, &handle, watch_notify_test_cb,
                          NULL));
...
TEST_F(LibRadosWatchNotify, WatchNotify2Timeout)
{
    ...
    char *reply_buf = 0;
    size_t reply_buf_len;
    ASSERT_EQ(-ETIMEDOUT, rados_notify2(ioctx, notify_oid, "notify", 6, 1000,   // 1s
                                        &reply_buf, &reply_buf_len));
    ASSERT_EQ(1u, notify_cookies.size());
    {
        bufferlist reply;
    ...
    notify_sleep = 0;
    notify_cookies.clear();
    ASSERT_EQ(0, rados_notify2(ioctx, notify_oid, "notify", 6, 300000,  // 300s
                               &reply_buf, &reply_buf_len));
    ASSERT_EQ(1u, notify_cookies.size());
    ASSERT_GT(rados_watch_check(ioctx, handle), 0);
...
TEST_F(LibRadosWatchNotify, AioWatchDelete2)
{
    ...
    rados_aio_watch2(ioctx, notify_oid, comp, &handle,
                     watch_notify2_test_cb, watch_notify2_test_errcb, timeout,
                     this);
    ASSERT_EQ(0, rados_aio_wait_for_complete(comp));
    ASSERT_EQ(0, rados_aio_get_return_value(comp));
    rados_aio_release(comp);
