...
:  
FreelistManager(cct),
meta_prefix(meta_prefix), bitmap_prefix(bitmap_prefix), enumerate_bl_pos(0)
{
}
...
int BitmapFreelistManager::_read_cfg(std::function <
                                     int (const std::string &,
                                          std::string *) > cfg_reader)
{
    dout(1) << __func__ << dendl;
...
uint64_t BitmapFreelistManager::size_2_block_count(uint64_t target_size) const const
{
...
void BitmapFreelistManager::get_meta(uint64_t target_size,
                                     std::vector < std::pair < string,
                                     string >> *res) const const
{
    if (target_size == 0) {
