...
public:
    ...
    std::map < EntityName, EntityAuth > &get_keys()
    {
        return keys;
    } // yuck int load(CephContext * cct, const std::string & filename);
    ...
    // modifiers void add(const EntityName & name, const EntityAuth & a) {
    keys[name] = a;
}
...
void set_caps(EntityName &name, std::map < std::string,
