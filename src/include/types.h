...
template < class A, class B, class C >
    inline ostream & operator<<(ostream & out, const boost::tuple < A, B,
                                C > &t);
template < class A, class Alloc > inline ostream & operator<<(ostream & out,
                                                              const list < A,
                                                              Alloc > &ilist);
template < class A, class Comp,
    class Alloc > inline ostream & operator<<(ostream & out, const set < A,
                                              Comp, Alloc > &iset);
template < class A, class Comp,
    class Alloc > inline ostream & operator<<(ostream & out, const multiset < A,
                                              Comp, Alloc > &iset);
template < class A, class B, class Comp,
    class Alloc > inline ostream & operator<<(ostream & out, const map < A, B,
                                              Comp, Alloc > &m);
template < class A, class B, class Comp,
    class Alloc > inline ostream & operator<<(ostream & out, const multimap < A,
                                              B, Comp, Alloc > &m);
...
template < class A, class Comp, class Alloc >
    inline ostream & operator<<(ostream & out, const set < A, Comp,
                                Alloc > &iset)
{
...
template < class A, class Comp, class Alloc >
    inline ostream & operator<<(ostream & out, const multiset < A, Comp,
                                Alloc > &iset)
{
    for (auto it = iset.begin(); it != iset.end(); ++it) {
        if (it != iset.begin())
...
template < class A, class B, class Comp, class Alloc >
    inline ostream & operator<<(ostream & out, const map < A, B, Comp,
                                Alloc > &m)
{
    out << "{";
...
template < class A, class B, class Comp, class Alloc >
    inline ostream & operator<<(ostream & out, const multimap < A, B, Comp,
                                Alloc > &m)
{
    out << "{{";
    for (auto it = m.begin(); it != m.end(); ++it) {
...
WRITE_RAW_ENCODER(ceph_mon_subscribe_item)
 WRITE_RAW_ENCODER(ceph_mon_statfs) WRITE_RAW_ENCODER(ceph_mon_statfs_reply)
// ----------------------
// some basic types
...
{
    return out << i.start << ((i.flags & CEPH_SUBSCRIBE_ONETIME) ? "" : "+");
}
...
inline ostream & operator<<(ostream & out, const weightf_t & w)
{
    if (w.v < -0.01) {
        ...
        return out << std::fixed << std::setprecision(5) << w.
    ...
    errorcode32_t():code(0)
{
  ...
  // cppcheck-suppress noExplicitConstructor errorcode32_t(int32_t i):code(i) {
}
