...
void rgw::AppMain::init_frontends1(bool nfs)
{
    ...
    for (auto &f : frontends) {
        ...
        fe_map.insert(pair < string,
                      RGWFrontendConfig * >(config->get_framework(), config));
    }                           /* for each frontend */
...
void rgw::AppMain::init_storage()
{
    ...
    env.driver =
        DriverManager::get_storage(dpp, dpp->get_cct(), cfg, run_gc, run_lc,
                                   run_quota, run_sync,
                                   g_conf().get_val < bool >
...
void rgw::AppMain::init_ldap()
{
    ...
    ldh.reset(new rgw::LDAPHelper(ldap_uri, ldap_binddn,
                                  ldap_bindpw.c_str(), ldap_searchdn,
...
int rgw::AppMain::init_frontends2(RGWLib *rgwlib)
{
    ...
    env.olog = olog;
    env.auth_registry =
        rgw::auth::StrategyRegistry::create(dpp->get_cct(),
    ...
    if (env.driver->get_name() == "rados") {
        ...
        pusher =
            std::make_unique < RGWPeriodPusher > (dpp, env.driver, null_yield);
...
void rgw::AppMain::shutdown(std::function < void (void) >
                            finalize_async_signals)
{
    if (env.driver->get_name() == "rados") {
