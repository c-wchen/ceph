...
ExternalTokenEngine::result_t ExternalTokenEngine::authenticate(const DoutPrefixProvider *dpp,
...
void RGW_SWIFT_Auth_Get::execute(optional_yield y)
{
    ...
    if (swift_url.size() == 0) {
        ...
        if (!host) {
            dout(0) << "NOTICE: server is misconfigured, missing rgw_swift_url_prefix or rgw_swift_url, HTTP_HOST is not set" <<
                    dendl;
            ret = -EINVAL;
            goto done;
        }
