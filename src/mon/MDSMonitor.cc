...
void MDSMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  for (auto & p:new_checks.checks) {
        p.second.summary = boost::regex_replace(p.second.summary,
                                                boost::regex("%num%"),
                                                stringify(p.second.detail.
                                                          size()));
        p.second.summary =
            boost::regex_replace(p.second.summary, boost::regex("%plurals%"),
                                 p.second.detail.size() > 1 ? "s" : "");
        p.second.summary =
            boost::regex_replace(p.second.summary, boost::regex("%isorare%"),
                                 p.second.detail.size() > 1 ? "are" : "is");
        p.second.summary =
            boost::regex_replace(p.second.summary, boost::regex("%hasorhave%"),
...
bool MDSMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    PaxosServiceMessage *m =
        static_cast < PaxosServiceMessage * >(op->get_req());
...
bool MDSMonitor::preprocess_beacon(MonOpRequestRef op)
{
    ...
    if (info.state != state) {
        ...
        if ((info.state == MDSMap::STATE_STANDBY ||
            ...
            dout(10) << "mds_beacon mds can't activate itself (" <<
        ...
        if ((state == MDSMap::STATE_STANDBY
            ...
            dout(4) <<
                "mds_beacon MDS can't go back into standby after taking rank: "
                "held rank " << info.
                rank << " while requesting state " << ceph_mds_state_name(state)
                << dendl;
            goto reply;
        }
  ...
  reply:
    ...
    mon->send_reply(op,
                    new MMDSBeacon(mon->monmap->fsid, m->get_global_id(),
                                   m->get_name(), effective_epoch, state, seq,
...
bool MDSMonitor::prepare_beacon(MonOpRequestRef op)
{
    ...
    MMDSBeacon *m = static_cast < MMDSBeacon * >(op->get_req());
    // -- this is an update --
    dout(12) << "prepare_beacon " << *m << " from " << m->
        get_orig_source_inst() << dendl;
  ...
  for (const auto & old_metric:old_health) {
        if (new_types.count(old_metric.type) == 0) {
            mon->clog->info() << "MDS health message cleared ("
                << m->get_orig_source_inst().name << "): " << old_metric.
                message;
        }
    }
    ...
    if (state == MDSMap::STATE_BOOT) {
        ...
        if (!info.standby_for_name.empty()) {
            const MDSMap::mds_info_t * leaderinfo =
                pending.find_by_name(info.standby_for_name);
            if (leaderinfo && (leaderinfo->rank >= 0)) {
                const auto & fscid =
                    pending.mds_roles.at(leaderinfo->global_id);

                pending.modify_daemon(gid,
                                      [fscid,
                                       leaderinfo] (MDSMap::mds_info_t * info) {
        ...
        if (!pending.compat.writeable(m->get_compat())) {
            dout(10) << " fsmap " << pending.compat
                ...
                << ", updating fsmap and killing old mds's" << dendl;
        ...
        if (!pending.gid_exists(gid)) {
            ...
            wait_for_finished_proposal(op,
                                       new FunctionContext([op, this] (int r) {
                                                           if (r >= 0) {
                                                           const auto & fsmap =
                                                           get_fsmap();
                                                           MDSMap null_map;
        ...
        if (info.standby_replay != m->get_standby_replay()) {
            pending.modify_daemon(info.global_id,[&m] (MDSMap::mds_info_t * i) {
                                  i->standby_replay = m->get_standby_replay();}
            );
        }

        if (info.state == MDSMap::STATE_STOPPING &&
            state != MDSMap::STATE_STOPPING && state != MDSMap::STATE_STOPPED) {
            // we can't transition to any other states from STOPPING
            dout(0) <<
                "got beacon for MDS in STATE_STOPPING, ignoring requested state change"
                << dendl;
            _note_beacon(m);
            return true;
        }
        ...
        if (info.laggy()) {
            dout(1) << "prepare_beacon clearing laggy flag on " << addr <<
                dendl;
        ...
        if (state == MDSMap::STATE_STOPPED) {
          ...
          for (const auto & erased_gid:erased) {
            ...
            mon->send_reply(op,
                            new MMDSBeacon(mon->monmap->fsid,
                                           m->get_global_id(), m->get_name(),
                                           pending.get_epoch(), state, seq,
                                           CEPH_FEATURES_SUPPORTED_DEFAULT));
        }
  ...
  for (const auto & i:info_map) {
      ...
      for (const auto & metric:health.metrics) {
            ...
            if (detail) {
                ...
                detail->
                    push_back(std::make_pair(metric.sev, detail_message.str()));
            }
        }
    }
}
...
bool MDSMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mds stat") {
        ...
        if (fsmapp->legacy_client_fscid != FS_CLUSTER_ID_NONE) {
            mdsmapp =
                &fsmapp->filesystems.at(fsmapp->legacy_client_fscid)->mds_map;
        }
        else {
            mdsmapp = &blank;
        }
                                   ...
                                   std::stringstream & ss)
{
    ...
    if (prefix == "mds stop" || prefix == "mds deactivate") {
        mds_role_t role;
        ...
        else if (fs->mds_map.get_num_in_mds() <=
                 size_t(fs->mds_map.get_max_mds())) {
            r = -EBUSY;
            ss <<
...
void MDSMonitor::modify_legacy_filesystem(FSMap & fsmap,
                                          ...
                                          void (std::shared_ptr <
                                                Filesystem >) > fn)
{
    fsmap.modify_filesystem(fsmap.legacy_client_fscid, fn);
}
 ...
 */
int MDSMonitor::legacy_filesystem_command(FSMap & fsmap,
                                          MonOpRequestRef op,
                                          std::string const &prefix,
                                          map < string, cmd_vartype > &cmdmap,
                                          std::stringstream & ss)
{
    ...
    if (prefix == "mds set_max_mds") {
        ...
        int64_t maxmds;
        if (!cmd_getval(g_ceph_context, cmdmap, "maxmds", maxmds)
            || maxmds <= 0) {
            return -EINVAL;
        }
        ...
        if (!mdsmap.allows_multimds() &&
            maxmds > mdsmap.get_max_mds() && maxmds > 1) {
            ss <<
        ...
        modify_legacy_filesystem(fsmap,[](std::shared_ptr < Filesystem > fs) {
...
void MDSMonitor::check_subs()
{
  ...
  for (const auto & type:types) {
        if (mon->session_map.subs.count(type) == 0)
            continue;
        xlist < Subscription * >::iterator p =
            mon->session_map.subs[type]->begin();
        while (!p.end()) {
            Subscription *sub = *p;
            ++p;
            check_sub(sub);
    ...
    if (sub->type == "fsmap") {
        ...
        if (sub->next <= fsmap.get_epoch()) {
          ...
          for (const auto & p:fsmap.filesystems) {
            ...
            sub->session->con->
                send_message(new MFSMapUser(mon->monmap->fsid, fsmap_u));
            if (sub->onetime) {
                mon->session_map.remove_sub(sub);
            }
            else {
        ...
        if (is_mds) {
          ...
          for (const auto & p:mds_info) {
            ...
            if (sub->type.find("mdsmap.") == 0) {
                ...
                dout(10) << __func__ << ": namespace_id " << namespace_id_str <<
                    dendl;
                std::string err;
        ...
        dout(10) << __func__ << " selected MDS map epoch " <<
            ...
            << sub->session->inst.name << " who wants epoch " << sub->
            next << dendl;
...
void MDSMonitor::remove_from_metadata(const FSMap & fsmap,
...
void MDSMonitor::count_metadata(const std::string & field, map < string,
                                int >*out)
{
...
bool MDSMonitor::maybe_expand_cluster(FSMap & fsmap, fs_cluster_id_t fscid)
{
    ...
    if (in < max) {
        ...
        mon->clog->info() << new_info.human_name() << " assigned to "
            ...
            << mds << " (now has " << mds_map.get_num_in_mds() + 1 << " ranks)";
                                   ...
                                   bool * mds_propose, bool * osd_propose)
{
  ...
  for (const auto & p:last_beacon) {
    ...
    if (info.rank >= 0 &&
        ...
        (sgid = fsmap.find_replacement_for( {
                                           fscid, info.rank}
                                           , info.name,
        ...
        mon->clog->warn() << info.human_name()
            ...
            << "as rank " << info.rank << " with standby " << si.human_name();
        ...
        *mds_propose = true;
        ...
        dout(1) << " failing and removing " << gid << " " << info.
            addr << " mds." << info.rank << "." << info.
            inc << " " << ceph_mds_state_name(info.state)
            << dendl;
        ...
        *mds_propose = true;
        ...
        dout(1) << " marking " << gid << " " << info.addr << " mds." << info.
            rank << "." << info.inc << " " << ceph_mds_state_name(info.state)
            << " laggy" << dendl;
                                       ...
                                       std::shared_ptr < Filesystem > &fs)
{
    ...
    if (!failed.empty()) {
      ...
      for (const auto & gid:standby_gids) {
             ...
             */
            dout(20) << "gid " << gid << " is standby and following nobody" <<
                dendl;
          ...
          for (const auto & p:fsmap.filesystems) {
              ...
              for (const auto & mds_i:mds_map.mds_info) {
                    ...
                    if (cand_info.rank >= 0
                        && mds_map.is_followable(cand_info.rank)) {
                        if ((info.standby_for_name.length()
                             && info.standby_for_name != cand_info.name)
  ...
  for (auto & p:pending.mds_roles) {
    ...
    for (auto it = last_beacon.begin(); it != last_beacon.end();) {
        ...
        if (since_last.count() >= g_conf->mds_beacon_grace) {
            ...
            if (osdmap_writeable) {
                maybe_replace_gid(pending, gid, info, &do_propose,
                                  &propose_osdmap);
            }
        }
 ...
 */
bool MDSMonitor::try_standby_replay(FSMap & fsmap,
                                    ...
                                    const MDSMap::mds_info_t & ainfo)
{
    ...
    if (leader_fs.has_standby_replay(ainfo.global_id)) {
        ...
        fsmap.assign_standby_replay(finfo.global_id, leader_fs.fscid,
                                    ainfo.rank);
        return true;
    }
}
