...
template < class T >
BackgroundRecoveryT < T >::BackgroundRecoveryT(Ref < PG > pg,
        ShardServices &ss,
        ...
        scheduler_class,
        float delay)
    : pg(pg), epoch_started(epoch_started), delay(delay), ss(ss),
      scheduler_class(scheduler_class)
{
} template < class T > void BackgroundRecoveryT <
T >::print(std::ostream &lhs) const
{
...
template < class T > seastar::future <> BackgroundRecoveryT < T >::start()
{
    ...
    if (delay) {
        maybe_delay =
            seastar::sleep(std::chrono::
                           milliseconds(std::lround(delay * 1000)));
    }
    return maybe_delay.then([ref, this] {
        ...
        ([ref, this](auto && trigger) {
            return ss.
            with_throttle_while(std::move(trigger), this,
                                get_scheduler_params(),
            [this] {
                return T::interruptor::
                with_interruption([this] {
                    return
                    do_recovery
                    ();
                }
...
UrgentRecovery::UrgentRecovery(const hobject_t &soid,
                               const eversion_t &need,
                               Ref < PG > pg,
                               ShardServices &ss, epoch_t epoch_started)
    :    BackgroundRecoveryT {
    pg, ss, epoch_started,
    crimson::osd::scheduler::scheduler_class_t::immediate}, soid {
    soid}, need(need)
{
}
    ...
    if (!pg->has_reset_since(epoch_started)) {
        ...
        interruptor > ([this](auto && trigger) {
            return pg->get_recovery_handler()->
                   recover_missing(trigger, soid, need);
        }
        ).then_interruptible([] {
...
PglogBasedRecovery::PglogBasedRecovery(Ref < PG > pg,
                                       ShardServices &ss,
...
BackfillRecovery::interruptible_future < bool > BackfillRecovery::do_recovery()
{
    ...
    then_interruptible([this] {
        ...
        dispatch_backfill_event(std::move(evt));
        return seastar::make_ready_future < bool >
        (false);
    }
                             );
