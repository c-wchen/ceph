...
int rgw_user_sync_all_stats(RGWRados * store, const rgw_user & user_id)
{
    ...
    do {
        ...
        for (map < string, RGWBucketEnt >::iterator i = buckets.begin();
            ...
            ret =
                store->get_bucket_info(obj_ctx, user_id.tenant,
                                       bucket_ent.bucket.name, bucket_info,
                                       nullptr, nullptr);
            if (ret < 0) {
                ldout(cct,
                      0) << "ERROR: could not read bucket info: bucket=" <<
                    bucket_ent.bucket << " ret=" << ret << dendl;
                continue;
            }
            ...
            ret =
                store->check_bucket_shards(bucket_info, bucket_info.bucket,
                                           bucket_quota);
            if (ret < 0) {
                ldout(cct,
...
int rgw_user_get_all_buckets_stats(RGWRados * store, const rgw_user & user_id,
                                   map < string,
                                   cls_user_bucket_entry > &buckets_usage_map)
{
    CephContext *cct = store->ctx();
    size_t max_entries = cct->_conf->rgw_list_buckets_max_chunk;
    bool done;
                        ...
                        bool exclusive, map < string, bufferlist > *pattrs)
{
    ...
    for (iter = info.swift_keys.begin(); iter != info.swift_keys.end(); ++iter) {
        ...
        if (r >= 0 && inf.user_id.compare(info.user_id) != 0) {
            ldout(store->ctx(),
                ...
                id << ") already mapped to another user (" << info.
                user_id << ")" << dendl;
            return -EEXIST;
        }
    }
    ...
    if (!info.access_keys.empty()) {
        ...
        for (; iter != info.access_keys.end(); ++iter) {
            ...
            if (r >= 0 && inf.user_id.compare(info.user_id) != 0) {
                ldout(store->ctx(),
                      0) <<
                    "WARNING: can't store user info, access key already mapped to another user"
                    << dendl;
                return -EEXIST;
            }
        }
        ...
        if (!old_info || old_info->user_email.compare(info.user_email) != 0) {  /* only if new index changed */
            ret =
                rgw_put_system_obj(store,
                                   store->get_zone_params().user_email_pool,
                                   info.user_email, link_bl.c_str(),
                                   link_bl.length(), exclusive, NULL,
                                   real_time());
            if (ret < 0)
                return ret;
        }
        ...
        for (; iter != info.access_keys.end(); ++iter) {
            ...
            ret =
                rgw_put_system_obj(store,
                                   ...
                                   k.id, link_bl.c_str(), link_bl.length(),
                                   exclusive, NULL, real_time());
            if (ret < 0)
                return ret;
        }
    }
    ...
    for (siter = info.swift_keys.begin(); siter != info.swift_keys.end();
         ++siter) {
        ...
        ret =
            rgw_put_system_obj(store, store->get_zone_params().user_swift_pool,
                               k.id, link_bl.c_str(), link_bl.length(),
                               exclusive, NULL, real_time());
        if (ret < 0)
            return ret;
                                 ...
                                 real_time * const pmtime)
{
    ...
    try {
        ::decode(uid, iter);
        int ret =
            rgw_get_user_info_by_uid(store, uid.user_id, e.info,
                                     &e.objv_tracker, NULL, &cache_info);
        if (ret < 0) {
            return ret;
        }
    }
        ...
        ldout(store->ctx(),
              0) << "ERROR: failed to decode user info, caught buffer::error" <<
            dendl;
        return -EIO;
    }
                             ...
                             map < string, bufferlist > *const pattrs)
{
    ...
    int ret =
        rgw_get_system_obj(store, obj_ctx,
                           store->get_zone_params().user_uid_pool, oid, bl,
                           objv_tracker, pmtime, pattrs, cache_info);
    if (ret < 0) {
        return ret;
    }
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
int rgw_get_user_info_by_email(RGWRados * store, string & email,
                               RGWUserInfo & info,
                               RGWObjVersionTracker * objv_tracker,
                               real_time * pmtime)
{
    return rgw_get_user_info_from_index(store, email,
                                        store->get_zone_params().
                                        user_email_pool, info, objv_tracker,
                                        pmtime);
}
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
extern int rgw_get_user_info_by_swift(RGWRados * const store, const string & swift_name, RGWUserInfo & info,    /* out */
                                      RGWObjVersionTracker * const objv_tracker,
                                      real_time * const pmtime)
{
    return rgw_get_user_info_from_index(store, swift_name,
                                        store->get_zone_params().
                                        user_swift_pool, info, objv_tracker,
                                        pmtime);
}
                    ...
                    RGWObjVersionTracker & objv_tracker)
{
    ...
    for (; kiter != info.access_keys.end(); ++kiter) {
        ldout(store->ctx(),
              10) << "removing key index: " << kiter->first << dendl;
        ret = rgw_remove_key_index(store, kiter->second);
        if (ret < 0 && ret != -ENOENT) {
            ldout(store->ctx(),
                  0) << "ERROR: could not remove " << kiter->
                first << " (access key object), should be fixed (err=" << ret <<
                ")" << dendl;
            return ret;
        }
    }
    ...
    for (; siter != info.swift_keys.end(); ++siter) {
        RGWAccessKey & k = siter->second;
        ldout(store->ctx(),
              10) << "removing swift subuser index: " << k.id << dendl;
        /* check if swift mapping exists */
        ret = rgw_remove_swift_name_index(store, k.id);
        if (ret < 0 && ret != -ENOENT) {
            ldout(store->ctx(),
                  0) << "ERROR: could not remove " << k.
                id << " (swift name object), should be fixed (err=" << ret <<
                ")" << dendl;
            return ret;
        }
    }
    ...
    ldout(store->ctx(),
          10) << "removing email index: " << info.user_email << dendl;
    ret = rgw_remove_email_index(store, info.user_email);
    if (ret < 0 && ret != -ENOENT) {
        ldout(store->ctx(),
              0) << "ERROR: could not remove email index object for " << info.
            user_email << ", should be fixed (err=" << ret << ")" << dendl;
        return ret;
    }

    string buckets_obj_id;
    rgw_get_buckets_obj(info.user_id, buckets_obj_id);
    rgw_raw_obj uid_bucks(store->get_zone_params().user_uid_pool,
                          buckets_obj_id);
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldout(store->ctx(),
            ...
            user_id << ":" << uid_bucks << ", should be fixed (err=" << ret <<
            ")" << dendl;
        return ret;
    }
    ...
    if (ret < 0 && ret != -ENOENT && ret != -ECANCELED) {
        ldout(store->ctx(),
              0) << "ERROR: could not remove " << info.
            user_id << ":" << uid_obj << ", should be fixed (err=" << ret << ")"
            << dendl;
        return ret;
    }
...
static bool remove_old_indexes(RGWRados * store,
                               RGWUserInfo & old_info, RGWUserInfo & new_info,
                               std::string * err_msg)
{
    int ret;
    bool success = true;
    ...
    if (!old_info.user_id.empty() &&
        ...
        if (old_info.user_id.tenant != new_info.user_id.tenant) {
            ldout(store->ctx(),
                  0) << "ERROR: tenant mismatch: " << old_info.user_id.
                tenant << " != " << new_info.user_id.tenant << dendl;
            return false;
        }
    ...
    if (!old_info.user_email.empty() &&
        ...
        if (ret < 0 && ret != -ENOENT) {
            set_err_msg(err_msg,
                        "ERROR: could not remove index for email " +
                        old_info.user_email);
            success = false;
        }
    }
    ...
    for (old_iter = old_info.swift_keys.begin();
         old_iter != old_info.swift_keys.end(); ++old_iter) {
        RGWAccessKey & swift_key = old_iter->second;
        map < string, RGWAccessKey >::iterator new_iter =
            new_info.swift_keys.find(swift_key.id);
        if (new_iter == new_info.swift_keys.end()) {
            ret = rgw_remove_swift_name_index(store, swift_key.id);
            if (ret < 0 && ret != -ENOENT) {
                set_err_msg(err_msg,
                            "ERROR: could not remove index for swift_name " +
                            swift_key.id);
                success = false;
            }
        }
                                   ...
                                   std::string * err_msg)
{
    ...
    if (!gen_secret) {
        ...
        ret =
            gen_rand_alphanumeric_plain(g_ceph_context, secret_key_buf,
...
int RGWAccessKeyPool::modify_key(RGWUserAdminOpState & op_state,
                                 std::string * err_msg)
{
    std::string id;
                                          ...
                                          bool defer_user_update)
{
    ...
    map < std::string, RGWAccessKey >::iterator user_kiter =
        user_info.access_keys.begin();
...
int RGWSubUserPool::remove(RGWUserAdminOpState & op_state,
                           std::string * err_msg)
{
    return remove(op_state, err_msg, false);
}
...
int RGWSubUserPool::execute_modify(RGWUserAdminOpState & op_state,
                                   std::string * err_msg,
                                   bool defer_user_update)
{
    int ret = 0;
...
int RGWSubUserPool::modify(RGWUserAdminOpState & op_state,
                           std::string * err_msg)
{
...
int RGWUserCapPool::remove(RGWUserAdminOpState & op_state,
                           std::string * err_msg)
{
    return remove(op_state, err_msg, false);
}
...
RGWUser::RGWUser():store(NULL), info_stored(false), caps(this), keys(this),
subusers(this)
{
    init_default();
}
...
int RGWUser::init(RGWUserAdminOpState & op_state)
{
    ...
    if (!user_id.empty() && (user_id.compare(RGW_USER_ANON_ID) != 0)) {
        found =
            (rgw_get_user_info_by_uid(store, user_id, user_info, &op_state.objv)
             >= 0);
    ...
    if (!user_email.empty() && !found) {
        found =
            (rgw_get_user_info_by_email
             (store, user_email, user_info, &op_state.objv) >= 0);
        op_state.found_by_email = found;
    }
    if (!swift_user.empty() && !found) {
        found =
            (rgw_get_user_info_by_swift
             (store, swift_user, user_info, &op_state.objv) >= 0);
        op_state.found_by_key = found;
    }
    if (!access_key.empty() && !found) {
        found =
            (rgw_get_user_info_by_access_key
             (store, access_key, user_info, &op_state.objv) >= 0);
        op_state.found_by_key = found;
    }
...
int RGWUser::update(RGWUserAdminOpState & op_state, std::string * err_msg)
{
    ...
    if (is_populated()) {
        ret =
            rgw_store_user_info(store, user_info, &old_info, &op_state.objv,
                                real_time(), false);
        if (ret < 0) {
        ...
        ret =
            rgw_store_user_info(store, user_info, NULL, &op_state.objv,
                                real_time(), false);
        if (ret < 0) {
            set_err_msg(err_msg, "unable to store user info");
            return ret;
...
int RGWUser::execute_add(RGWUserAdminOpState & op_state, std::string * err_msg)
{
    ...
    if (op_state.has_existing_user()) {
        ...
        if (op_state.found_by_email) {
            ...
            set_err_msg(err_msg,
                        "user: " + op_state.user_id.to_str() + " exists");
            ret = -EEXIST;
        }
        return ret;
    }
...
int RGWUser::execute_remove(RGWUserAdminOpState & op_state,
                            std::string * err_msg)
{
    int ret;
...
int RGWUser::execute_modify(RGWUserAdminOpState & op_state,
                            std::string * err_msg)
{
    ...
    if (op_state.has_suspension_op()) {
        ...
        do {
            ret =
                rgw_read_user_buckets(store, user_id, buckets, marker, string(),
                                      max_buckets, false, &is_truncated);
            if (ret < 0) {
                set_err_msg(err_msg,
                            "could not get buckets for uid:  " +
                            user_id.to_str());
                return ret;
            }
...
int RGWUserAdminOp_User::create(RGWRados * store,
...
int RGWUserAdminOp_User::modify(RGWRados * store,
                                RGWUserAdminOpState & op_state,
                                RGWFormatterFlusher & flusher)
{
    RGWUserInfo info;
...
int RGWUserAdminOp_User::remove(RGWRados * store,
                                RGWUserAdminOpState & op_state,
                                RGWFormatterFlusher & flusher)
{
    RGWUserInfo info;
...
int RGWUserAdminOp_Subuser::create(RGWRados * store,
                                   RGWUserAdminOpState & op_state,
                                   RGWFormatterFlusher & flusher)
{
    RGWUserInfo info;
...
int RGWUserAdminOp_Subuser::modify(RGWRados * store,
                                   RGWUserAdminOpState & op_state,
                                   RGWFormatterFlusher & flusher)
{
    RGWUserInfo info;
...
int RGWUserAdminOp_Subuser::remove(RGWRados * store,
                                   RGWUserAdminOpState & op_state,
                                   RGWFormatterFlusher & flusher)
{
    RGWUserInfo info;
...
int RGWUserAdminOp_Caps::remove(RGWRados * store,
                                RGWUserAdminOpState & op_state,
                                RGWFormatterFlusher & flusher)
{
    RGWUserInfo info;
...
class RGWUserMetadataObject:public RGWMetadataObject {
  ...
  public:
    RGWUserMetadataObject(const RGWUserCompleteInfo & _uci, obj_version & v,
                          real_time m)
    :uci(_uci) {
        objv = v;
        mtime = m;
...
class RGWUserMetadataHandler:public RGWMetadataHandler {
  public:
    string get_type() override {
        ...
        RGWUserMetadataObject *mdo =
            new RGWUserMetadataObject(uci, objv_tracker.read_version, mtime);
        *obj = mdo;
    ...
    int put(RGWRados * store, string & entry,
            RGWObjVersionTracker & objv_tracker, real_time mtime, JSONObj * obj,
            sync_type_t sync_mode) override {
        RGWUserCompleteInfo uci;
        ...
        int ret =
            rgw_get_user_info_by_uid(store, uid, old_info, &objv_tracker,
                                     &orig_mtime);
        if (ret < 0 && ret != -ENOENT)
            return ret;
        ...
        ret =
            rgw_store_user_info(store, uci.info, &old_info, &objv_tracker,
                                mtime, false, pattrs);
        if (ret < 0) {
            return ret;
        }
    ...
    void get_pool_and_oid(RGWRados * store, const string & key, rgw_pool & pool,
                          string & oid) override {
        oid = key;
    ...
    int list_keys_init(RGWRados * store, const string & marker,
        ...
        int ret =
            store->list_raw_objects_init(store->get_zone_params().user_uid_pool,
                                         marker,
                                         &info->ctx);
        if (ret < 0) {
            return ret;
