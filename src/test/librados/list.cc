...
static void check_list(std::set < std::string > &myset,
                       rados_list_ctx_t & ctx, std::string check_nspace)
{
...
TEST_F(LibRadosList, ListObjectsCursor)
{
...

    {
        ...
        while (rados_nobjects_list_next(ctx, &entry, NULL, NULL) == 0) {
            string oid = entry;
            ASSERT_EQ(rados_nobjects_list_get_cursor(ctx, &cursor), 0);
            cout << "> oid=" << oid << " cursor=" << ObjectCursor(cursor) <<
                std::endl;
        }
        rados_nobjects_list_seek_cursor(ctx, first_cursor);
        ASSERT_EQ(rados_nobjects_list_next(ctx, &entry, NULL, NULL), 0);
        cout << "FIRST> seek to " << ObjectCursor(first_cursor) << " oid=" <<
            string(entry) << std::endl;
    }
    rados_list_ctx_t ctx;
    ASSERT_EQ(0, rados_nobjects_list_open(ioctx, &ctx));
    ...
    while (rados_nobjects_list_next(ctx, &entry, NULL, NULL) == 0) {
        ...
        ASSERT_EQ(rados_nobjects_list_get_cursor(ctx, &cursor), 0);
        string oid = entry;
        cout << ": oid=" << oid << " cursor=" << ObjectCursor(cursor) << std::
            endl;
        cursor_to_obj[cursor] = oid;
    ...
    while (p != cursor_to_obj.rend()) {
        ...
        ASSERT_EQ(rados_nobjects_list_get_cursor(ctx, &cursor), 0);
        cout << ": cursor()=" << ObjectCursor(cursor) << " expected=" << oid <<
            std::endl;
...
TEST_F(LibRadosListPP, ListObjectsFilterPP)
{
    ...
    ASSERT_EQ(0,
              ioctx.write("has_xattr", obj_content, obj_content.length(), 0));
    ASSERT_EQ(0,
              ioctx.write("has_wrong_xattr", obj_content, obj_content.length(),
                          0));
...
TEST_F(LibRadosList, EnumerateObjects)
{
    ...
    for (unsigned i = 0; i < n_objects; ++i) {
        ASSERT_EQ(0,
                  rados_write(ioctx, stringify(i).c_str(), buf, sizeof(buf),
                              0));
    }
