...
m_version_hook(NULL), m_help_hook(NULL), m_getdescs_hook(NULL)
{
}
...
std::string AdminSocket::bind_and_listen(const std::string & sock_path, int *fd)
{
    ...
    if (sock_path.size() > sizeof(address.sun_path) - 1) {
        ...
        oss << "AdminSocket::bind_and_listen: "
            << "The UNIX domain socket path " << sock_path <<
            " is too long! The " << "maximum length on this system is " <<
    ...
    if (::bind(sock_fd, (struct sockaddr *)&address,
        ...
        if (err == EADDRINUSE) {
            ...
            if (ok) {
                ldout(m_cct,
                      20) << "socket " << sock_path << " is in use" << dendl;
                err = EEXIST;
            }
            else {
...
bool AdminSocket::do_accept()
{
    ...
    while (1) {
        ...
        if (++pos >= sizeof(cmd)) {
            lderr(m_cct) << "AdminSocket: error reading request too long" <<
                dendl;
            VOID_TEMP_FAILURE_RETRY(close(connection_fd));
            return false;
        }
    ...
    bufferlist out;
    if (p == m_hooks.end()) {
        lderr(m_cct) << "AdminSocket: request '" << c << "' not defined" <<
            dendl;
    }
    else {
        string args;
        ...
        if (!success) {
            ldout(m_cct,
                  0) << "AdminSocket: request '" << match << "' args '" << args
                << "' to " << p->second << " failed" << dendl;
            out.append("failed");
        }
        else {
            ldout(m_cct,
                  5) << "AdminSocket: request '" << match << "' '" << args <<
                "' to " << p->second << " returned " << out.
                length() << " bytes" << dendl;
        }
...
int AdminSocket::register_command(std::string command, std::string cmddesc,
                                  AdminSocketHook * hook, std::string help)
{
    ...
    if (m_hooks.count(command)) {
        ldout(m_cct,
              5) << "register_command " << command << " hook " << hook <<
            " EEXIST" << dendl;
        ret = -EEXIST;
    }
    else {
        ldout(m_cct,
              5) << "register_command " << command << " hook " << hook << dendl;
...
int AdminSocket::unregister_command(std::string command)
{
    ...
    if (m_hooks.count(command)) {
        ...
        ldout(m_cct,
              5) << "unregister_command " << command << " ENOENT" << dendl;
        ret = -ENOENT;
    }
    m_lock.Unlock();
    return ret;
...
class HelpHook:public AdminSocketHook {
  ...
  public:
    ...
    } bool call(string command, cmdmap_t & cmdmap, string format,
                bufferlist & out) override {
...
bool AdminSocket::init(const std::string & path)
{
    ...
    register_command("git_version", "git_version", m_version_hook,
                     "get git sha1");
