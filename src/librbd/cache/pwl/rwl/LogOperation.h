...
public:
    WriteLogOperation(WriteLogOperationSet &set,
                      ...
                      write_log_entry)
        : pwl::WriteLogOperation(set, image_offset_bytes,
                                 write_bytes, cct, write_log_entry)
    {
    } WriteLogOperation(WriteLogOperationSet &set,
                        uint64_t image_offset_bytes,
        ...
        : pwl::WriteLogOperation(set, image_offset_bytes,
                                 ...
                                 writesame_log_entry)
    {
    } void copy_bl_to_cache_buffer(std::vector <
...
{
public:
    DiscardLogOperation(std::shared_ptr < SyncPoint >
                        sync_point, uint64_t image_offset_bytes,
                        uint64_t write_bytes,
                        uint32_t discard_granularity_bytes,
                        utime_t dispatch_time,
                        PerfCounters *perfcounter,
                        CephContext *cct)
        : pwl::DiscardLogOperation(sync_point, image_offset_bytes,
                                   write_bytes,
                                   discard_granularity_bytes,
                                   dispatch_time, perfcounter, cct)
    {
    } void init_op(uint64_t current_sync_gen,
...
} // namespace rwl } // namespace pwl } // namespace cache }    // namespace librbd
