...
int wait_for_healthy(rados_t *cluster)
{
    ...
    while (!healthy) {
        ...
        [[maybe_unused]] bool json_parse_success =
            json_spirit::read(out, root);
        ceph_assert(json_parse_success);
        json_spirit::mObject root_obj = root.get_obj();
        json_spirit::mObject pgmap = root_obj["pgmap"].get_obj();
        json_spirit::mArray pgs_by_state =
            pgmap["pgs_by_state"].get_array();
...
int rados_pool_set(rados_t *cluster,
                   ...
                   const std::string &var, const std::string &val)
{
    ...
    cmd[0] = serialized_cmd.c_str();
    int ret =
        rados_mon_command(*cluster, (const char **)cmd, 1, "", 0, NULL,
                          NULL, NULL, NULL);
    return ret;
}
...
struct pool_op_error: std::exception {
    ...
    pool_op_error(const std::string &pool_name,
                  const std::string &func_name, int err)
    {
        std::ostringstream oss;
        oss << func_name << "(" << pool_name << ") failed with error " <<
            err;
        msg = oss.str();
                       ...
                       uint32_t pg_num)
{
    return with_healthy_cluster(cluster, pool_name, [&] {
        // Adjust pg_num
        if (int r =
            rados_pool_set(cluster, pool_name, "pg_num",
                           stringify(pg_num)); r != 0) {
            throw pool_op_error {
                pool_name, "set_pg_num", r};
        }
    }) ;
}

std::string set_pgp_num(rados_t *cluster, const std::string &pool_name,
                        uint32_t pgp_num)
{
    return with_healthy_cluster(cluster, pool_name, [&] {
        // Adjust pgp_num
        if (int r =
            rados_pool_set(cluster, pool_name,
                           "pgp_num",
                           stringify(pgp_num));
            r != 0) {
            throw pool_op_error {
                pool_name, "set_pgp_num", r};
        }
    }) ;
}
