...
int get_encryption_defaults(req_state *s)
{
    ...
    if (iter != s->info.crypt_attribute_map.end()) {
        ldpp_dout(s, 20) << "get_encryption_defaults: found bucket_key_attr " << bucket_key_attr << " = " << iter->second <<
                            ", setting kms_attr_seen" << dendl;
        if (!rest_only) {
        ...
        ldpp_dout(s, 20) << "get_encryption_defaults: no bucket_key_attr, but bucket_key_enabled,  setting kms_attr_seen" <<
                         dendl;
        kms_attr_seen = true;
...
int RGWGetObj_ObjStore_S3Website::send_response_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
    ...
    if (iter != attrs.end()) {
        ...
        s->redirect = bl.c_str();
        s->err.http_ret = 301;
        ldpp_dout(this, 20) << __CEPH_ASSERT_FUNCTION << " redirecting per x-amz-website-redirect-location=" << s->redirect <<
                            dendl;
...
struct ReplicationConfiguration {
                              ...
                              vector < rgw_sync_policy_group > *result) const
    {
        ...
        for (auto& rule : rules) {
            ...
            if (r < 0) {
                ldpp_dout(s, 5) << "NOTICE: failed to convert replication configuration into sync policy pipe (rule.id=" << rule.id <<
...
static void dump_usage_categories_info(Formatter *formatter, const rgw_usage_log_entry& entry, map < string,
                                       bool > *categories)
{
        ...
        optional_yield y)
{
    ...
    if (should_redirect) {
         ...
         */
        new_err_no = RGWHandler_REST_S3Website::serve_errordoc(s, http_error_code, s->bucket->get_info().website_conf.error_doc,
                     y);
        if (new_err_no != -1) {
            err_no = new_err_no;
        }
...
AWSSignerV4::prepare_result_t AWSSignerV4::prepare(const DoutPrefixProvider *dpp,
...
boost::optional < std::string > AWSGeneralBoto2Abstractor::get_v4_canonical_headers(
    const req_info& info,
    const std::string_view& signedheaders,
    const bool using_qs) const
{
...
rgw::auth::RemoteApplier::AuthInfo rgw::auth::s3::STSEngine::get_creds_info(const STS::SessionToken& token) const
noexcept
{
    using acct_privilege_t = \
                             rgw::auth::RemoteApplier::AuthInfo::acct_privilege_t;
