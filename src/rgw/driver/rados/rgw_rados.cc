...
public:
    RGWMetaNotifierManager(RGWRados *_driver) : RGWCoroutinesManager(_driver->ctx(), _driver->get_cr_registry()),
        store(_driver),
    ...
    int notify_all(const DoutPrefixProvider *dpp, map < rgw_zone_id, RGWRESTConn * > & conn_map, set < int > & shards)
    {
        ...
        for (auto iter = conn_map.begin(); iter != conn_map.end(); ++iter) {
            ...
            stack->call(new RGWPostRESTResourceCR < set < int>, int>(store->ctx(), conn, &http_manager, "/admin/log", pairs, shards,
                        NULL));
...
public:
    RGWDataNotifierManager(RGWRados *_driver) : RGWCoroutinesManager(_driver->ctx(), _driver->get_cr_registry()),
        store(_driver),
        http_manager(store->ctx(), completion_mgr)
    {
        http_manager.start();
...
void RGWIndexCompletionManager::process()
{
    ...
    while (!_stop) {
        ...
        for (auto c : comps) {
            ...
            if (r < 0) {
                ldpp_dout(&dpp, 0) << "ERROR: " << __func__ << "(): failed to initialize BucketShard, obj=" << c->obj << " r=" << r <<
                                   dendl;
                /* not much to do */
                continue;
            }
            ...
            if (r < 0) {
                ldpp_dout(&dpp, 0) << "ERROR: " << __func__ << "(): bucket index completion failed, obj=" << c->obj << " r=" << r <<
                                   dendl;
...
int RGWRados::register_to_service_map(const DoutPrefixProvider *dpp, const string& daemon_type, const map < string,
                                      string > & meta)
{
...
int RGWRados::update_service_map(const DoutPrefixProvider *dpp, std::map < std::string, std::string > && status)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: service_daemon_update_status() returned ret=" << ret << ": " << cpp_strerror(
                              -ret) << dendl;
        return ret;
    }
    ...
    for (int i = 0; i < MAX_CREATE_RETRIES; i++) {
        ...
        if (ret == -EEXIST) {
            ...
            if (orig_info.bucket.bucket_id != bucket.bucket_id) {
                ...
                if (r < 0) {
                    ldpp_dout(dpp, 0) << "WARNING: " << __func__ << "(): failed to remove bucket instance info: bucket instance=" <<
...
int RGWRados::fix_head_obj_locator(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, bool copy_obj,
                                   bool remove_bad, rgw_obj_key& key, optional_yield y)
{
    ...
    int process_attrs(void)
    {
        ...
        if (extra_data_bl.length()) {
            ...
            if (!jp.parse(extra_data_bl.c_str(), extra_data_bl.length())) {
                ldpp_dout(dpp, 0) << "failed to parse response extra data. len=" << extra_data_bl.length() << " data=" <<
                               ...
                               std::optional < uint64_t > * bytes_transferred)
{
    ...
    if (copy_if_newer) {
        ...
        ret = get_obj_state(rctx.dpp, &dest_obj_ctx, dest_bucket_info, stat_dest_obj, &dest_state, &manifest, stat_follow_olh,
                            rctx.y);
        if (ret < 0) {
            goto set_err_state;
        }
    ...
    for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
        ...
        if (copy_if_newer && canceled) {
            ...
            if (!dest_state->exists ||
                ...
                ldpp_dout(rctx.dpp, 20) << "retrying writing object mtime=" << set_mtime << " dest_state->mtime=" << dest_state->mtime
                                        << " dest_state->exists=" << dest_state->exists << dendl;
                continue;
            } else {
                ldpp_dout(rctx.dpp, 20) << "not retrying writing object mtime=" << set_mtime << " dest_state->mtime=" <<
                             ...
                             bool log_op)
{
    ...
    if (read_mtime != mtime) {
        /* raced */
        ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to transition obj(" << obj.key << ") read_mtime = " << read_mtime <<
                            ...
                            const DoutPrefixProvider *dpp, bool check_empty)
{
    ...
    int r = svc.bi_rados->open_bucket_index(dpp, bucket_info, std::nullopt, bucket_info.layout.current_index, &index_pool,
                                            &bucket_objs, nullptr);
    if (r < 0) {
        return r;
    }
    ...
    if (objv_tracker.read_version.empty()) {
        ...
        if (r < 0 ||
            ...
            if (r != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: read_bucket_entrypoint_info() bucket=" << bucket_info.bucket << " returned error: r=" << r
                                  << dendl;
                                  ...
                                  optional_yield y)
{
    ...
    for (iter = buckets.begin(); iter != buckets.end(); ++iter) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "NOTICE: get_bucket_info on bucket=" << bucket.name << " returned err=" << r << ", skipping bucket"
                              << dendl;
            ret = r;
            continue;
        }
        ...
        r = put_bucket_instance_info(info, false, real_time(), &attrs, dpp, y);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "NOTICE: put_bucket_info on bucket=" << bucket.name << " returned err=" << r << ", skipping bucket"
                              << dendl;
            ret = r;
            continue;
        }
                                 ...
                                 map < RGWObjCategory, RGWStorageStats > *calculated_stats)
{
    ...
    int ret = svc.bi_rados->open_bucket_index(dpp, bucket_info, std::nullopt, bucket_info.layout.current_index, &index_pool,
              &oids, nullptr);
    if (ret < 0) {
        return ret;
    }
...
int RGWRados::defer_gc(const DoutPrefixProvider *dpp, RGWObjectCtx* octx, RGWBucketInfo& bucket_info,
                       const rgw_obj& obj, optional_yield y)
{
    std::string oid, key;
    get_obj_bucket_and_oid_loc(obj, oid, key);
                               ...
                               const DoutPrefixProvider *dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "() get_bucket_instance_info(bucket=" << obj.bucket << ") returned ret="
                          << ret << dendl;
        return ret;
    }
                                 ...
                                 optional_yield y, bool assume_noent)
{
    ...
    ldpp_dout(dpp, 20) << "get_obj_state: octx=" << (void *)octx << " obj=" << obj << " state=" <<
                       (void *)s << " s->prefetch_data=" << s->prefetch_data << dendl;
    *psm = sm;
    if (s->has_attrs) {
    ...
    if (!assume_noent) {
        r = RGWRados::raw_obj_stat(dpp, raw_obj, &s->size, &s->mtime, &s->epoch, &s->attrset,
    ...
    if (iter = s->attrset.find(RGW_ATTR_PG_VER); iter != s->attrset.end()) {
        ...
        if (pg_ver_bl.length()) {
            ...
            try {
                ...
                ldpp_dout(dpp, 0) << "ERROR: couldn't decode pg ver attr for object " << s->obj << ", non-critical error, ignoring" <<
                                  dendl;
            }
        }
    }
    ...
    if (is_olh(s->attrset)) {
        ...
        ldpp_dout(dpp, 20) << __func__ << ": setting s->olh_tag to " << string(s->olh_tag.c_str(),
                           s->olh_tag.length()) << dendl;
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::set_attr(const DoutPrefixProvider *dpp, RGWObjectCtx* octx, RGWBucketInfo& bucket_info,
                       const rgw_obj& obj, const char *name, bufferlist& bl, optional_yield y)
{
    map < string, bufferlist > attrs;
    attrs[name] = bl;
...
int RGWRados::set_attrs(const DoutPrefixProvider *dpp, RGWObjectCtx* octx, RGWBucketInfo& bucket_info,
                        const rgw_obj& src_obj,
                        map < string, bufferlist > & attrs,
    ...
    if (conds.if_match || conds.if_nomatch) {
        ...
        if (conds.if_nomatch) {
            ...
            ldpp_dout(dpp, 10) << "ETag: " << string(etag.c_str(),
                               etag.length()) << " " << " If-NoMatch: " << if_nomatch_str << dendl;
...
int RGWRados::Bucket::UpdateIndex::guard_reshard(const DoutPrefixProvider *dpp, const rgw_obj& obj_instance,
        BucketShard **pbs, std::function < int(BucketShard *) > call, optional_yield y)
{
    RGWRados *store = target->get_store();
    BucketShard *bs = nullptr;
    int r;
        ...
        if (rgwrados->get_use_datacache()) {
            ...
            if (bl.length() <= g_conf()->rgw_get_obj_max_req_size && !d3n_bypass_cache_write) {
                lsubdout(g_ceph_context, rgw_datacache,
                       ...
                       "(): bl.length <= rgw_get_obj_max_req_size (default 4MB) - write to datacache, bl.length=" << bl.length() << dendl;
                rgwrados->d3n_data_cache->put(bl, bl.length(), oid);
            } else {
                lsubdout(g_ceph_context, rgw_datacache,
                         10) << "D3nDataCache: " << __func__ <<
                       "(): not writing to datacache - bl.length > rgw_get_obj_max_req_size (default 4MB), bl.length=" << bl.length() <<
                                 ...
                                 RGWObjState *astate, void *arg)
{
    ...
    ldpp_dout(dpp, 20) << "rados->get_obj_iterate_cb oid=" << read_obj.oid << " obj-ofs=" << obj_ofs << " read_ofs=" <<
...
int RGWRados::obj_operate(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                          ObjectWriteOperation *op, optional_yield y)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj, &ref);
    if (r < 0) {
...
int RGWRados::obj_operate(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                          ObjectReadOperation *op, optional_yield y)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj, &ref);
    if (r < 0) {
...
int RGWRados::olh_init_modification_impl(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info,
        RGWObjState& state, const rgw_obj& olh_obj, string *op_tag, optional_yield y)
{
    ObjectWriteOperation op;
...
int RGWRados::olh_init_modification(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, RGWObjState& state,
                                    const rgw_obj& obj, string *op_tag, optional_yield y)
{
    int ret;
...
{
    ldpp_dout(dpp, 20) << __func__ << "(): olh_state.olh_tag=" << string(olh_state.olh_tag.c_str(),
                       olh_state.olh_tag.length()) << dendl;
                            ...
                            bool log_op)
{
    ...
    *plast_ver = last_ver;
            ...
            switch (entry.op) {
                ...
                case CLS_RGW_OLH_OP_LINK_OLH:
                    ...
                    if (link_epoch < iter->first || key.instance.empty() ||
                        ...
                        ldpp_dout(dpp, 20) << "apply_olh_log applying key=" << entry.key << " epoch=" << iter->first << " delete_marker=" <<
                                           entry.delete_marker
                        ...
                        ldpp_dout(dpp, 20) << "apply_olh skipping key=" << entry.key << " epoch=" << iter->first << " delete_marker=" <<
                                           entry.delete_marker
    ...
    for (list < cls_rgw_obj_key >::iterator liter = remove_instances.begin();
        ...
        int ret = delete_obj(dpp, obj_ctx, bucket_info, obj_instance, 0, y, RGW_BILOG_FLAG_VERSIONED_OP, ceph::real_time(),
                             zones_trace, log_op);
        if (ret < 0 && ret != -ENOENT) {
    ...
    r = rgw_rados_operate(dpp, ref.ioctx, ref.obj.oid, &op, y);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": could not apply olh update to oid \"" << ref.obj.oid << "\", r=" << r
                          << dendl;
        return r;
    }
...
int RGWRados::update_olh(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWObjState *state,
                         RGWBucketInfo& bucket_info, const rgw_obj& obj, optional_yield y, rgw_zone_set *zones_trace, bool log_op)
{
...
#define MAX_ECANCELED_RETRY 100
    for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 20) << "olh_init_modification() target_obj=" << target_obj << " delete_marker=" <<
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 20) << "bucket_index_link_olh() target_obj=" << target_obj << " delete_marker=" <<
                               (int)delete_marker << " returned " << ret << dendl;
...
int RGWRados::remove_olh_pending_entries(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info,
        RGWObjState& state, const rgw_obj& olh_obj, map < string, bufferlist > & pending_attrs, optional_yield y)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, olh_obj, &ref);
    if (r < 0) {
...
int RGWRados::follow_olh(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info, RGWObjectCtx& obj_ctx,
                         RGWObjState *state, const rgw_obj& olh_obj, rgw_obj *target, optional_yield y)
{
    map < string, bufferlist > pending_entries;
...
int RGWRados::get_bucket_stats_async(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info,
                                     const rgw::bucket_index_layout_generation& idx_layout, int shard_id, boost::intrusive_ptr < rgw::sal::ReadStatsCB > cb)
{
    int num_aio = 0;
    boost::intrusive_ptr headercb = new RGWGetBucketStatsContext(std::move(cb),
...
static int cls_rgw_usage_log_trim_repeat(const DoutPrefixProvider *dpp, rgw_rados_ref ref, const string& user,
        const string& bucket, uint64_t start_epoch, uint64_t end_epoch, optional_yield y)
{
    bool done = false;
    do {
        librados::ObjectWriteOperation op;
...
int RGWRados::cls_bucket_head(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info,
                              const rgw::bucket_index_layout_generation& idx_layout, int shard_id, vector < rgw_bucket_dir_header > & headers,
                              map < int, string > *bucket_instance_ids)
{
    librados::IoCtx index_pool;
    map < int, string > oids;
    map < int, struct rgw_cls_list_ret > list_results;
    int r = svc.bi_rados->open_bucket_index(dpp, bucket_info, shard_id, idx_layout, &index_pool, &oids,
                                            bucket_instance_ids);
    if (r < 0) {
...
int RGWRados::add_bucket_to_reshard(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info,
                                    uint32_t new_num_shards, optional_yield y)
{
    ...
    if (new_num_shards <= num_source_shards) {
        ldpp_dout(dpp, 20) << "not resharding bucket name=" << bucket_info.bucket.name << ", orig_num=" << num_source_shards <<
