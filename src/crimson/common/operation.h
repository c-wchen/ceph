  ...
  template < typename T > class BlockerT:public Blocker {
      public:
        ...
        template < class TriggerT, class ... Args >
            decltype(auto) track_blocking(TriggerT && trigger, Args
                                          && ... args) {
  ...
  template < class T > class OrderedConcurrentPhaseT:public PipelineStageIT < T >
    {
      ...
      private:
        void dump_detail(ceph::Formatter * f) const final {
            ...
            TriggerT trigger;
          public:
          ExitBarrier(OrderedConcurrentPhaseT * phase, seastar::future <> &&barrier, TriggerT & trigger):phase(phase), barrier(std::move(barrier)),
                trigger(trigger)
            {
            }
