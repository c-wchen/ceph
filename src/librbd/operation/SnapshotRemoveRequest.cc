...
} template < typename I > void SnapshotRemoveRequest < I >::send_op()
{
    ...
    ceph_assert(ceph_mutex_is_locked(image_ctx.owner_lock));
    {
        ...
        if (image_ctx.snap_info.find(m_snap_id) ==
            image_ctx.snap_info.end()) {
...
template < typename I > void SnapshotRemoveRequest < I >::trash_snap()
{
    ...
    int r =
        image_ctx.md_ctx.aio_operate(image_ctx.header_oid, aio_comp,
                                     &op);
    ceph_assert(r == 0);
    aio_comp->release();
}
...
void SnapshotRemoveRequest < I >::handle_trash_snap(int r)
{
    ...
    if (r == -EOPNOTSUPP) {
        ...
        lderr(cct) << "failed to move snapshot to trash: " <<
                   cpp_strerror(r)
                   << dendl;
        this->complete(r);
        return;
    ...
    for (auto &child_image : m_child_images) {
        ...
        if (r == -ENOENT) {
            ...
            auto aio_comp = create_rados_callback <
                            ...
                            I >::handle_detach_stale_child > (this);
...
void SnapshotRemoveRequest < I >::handle_detach_stale_child(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "failed to detach stale child: " <<
                   cpp_strerror(r)
                   << dendl;
        this->complete(r);
        return;
...
void SnapshotRemoveRequest < I >::handle_detach_child(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "failed to detach child from parent: " <<
                   cpp_strerror(r)
                   << dendl;
        this->complete(r);
        return;
...
void SnapshotRemoveRequest < I >::remove_object_map()
{
...

    {
        ...
        if (image_ctx.object_map != nullptr) {
            ...
            auto ctx = create_context_callback <
                       SnapshotRemoveRequest < I >,
                       &SnapshotRemoveRequest < I >::handle_remove_object_map >
                       (this);
...
void SnapshotRemoveRequest < I >::handle_remove_object_map(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to remove snapshot object map: " <<
                   cpp_strerror(r)
                   << dendl;
        this->complete(r);
        return;
...
void SnapshotRemoveRequest < I >::remove_image_state()
{
    ...
    auto ctx = create_context_callback <
               SnapshotRemoveRequest < I >,
               &SnapshotRemoveRequest < I >::handle_remove_image_state >
               (this);
    auto req =
        mirror::snapshot::RemoveImageStateRequest <
        I >::create(&image_ctx, m_snap_id, ctx);
    req->send();
}
...
void SnapshotRemoveRequest < I >::handle_remove_image_state(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to remove image state: " <<
                   cpp_strerror(r)
                   << dendl;
        if (r != -ENOENT) {
            this->complete(r);
...
void SnapshotRemoveRequest < I >::handle_release_snap_id(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "failed to release snap id: " << cpp_strerror(r)
                   << dendl;
        this->complete(r);
        return;
    }
...
void SnapshotRemoveRequest < I >::handle_remove_snap(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to remove snapshot: " << cpp_strerror(r)
                   << dendl;
        this->complete(r);
        return;
    }
...
template < typename I >
int SnapshotRemoveRequest <
I >::scan_for_parents(cls::rbd::ParentImageSpec &pspec)
{
    I &image_ctx = this->m_image_ctx;
