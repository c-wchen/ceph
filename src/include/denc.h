...
struct denc_traits<ceph::buffer::ptr> {
    ...
    template <is_const_iterator It>
    static void decode(ceph::buffer::ptr &v, It &p, uint64_t f = 0)
    {
        uint32_t len;
        denc(len, p);
        v = p.get_ptr(len);
...
std::enable_if_t < (denc_traits<Ts>::supported && ...) >> {
    ...
    template<typename U = container>
    static std::enable_if_t<denc_traits<U>::featured> encode(const container &s, ceph::buffer::list::contiguous_appender &p,
            uint64_t f)
    {
        ceph::for_each(s, [&p, f](const auto & e) {
            if constexpr(denc_traits<std::decay_t<decltype(e)>>::featured) {
    ...
    template<typename U = container>
    static std::enable_if_t < !denc_traits<U>::need_contiguous > decode(container &s, ceph::buffer::list::const_iterator &p,
            uint64_t f = 0)
    {
        ceph::for_each(s, [&p](auto & e) {
            denc(e, p);
...
struct denc_traits <
    ...
    template<typename U = T>
    static std::enable_if_t < !!sizeof(U) &&
    !need_contiguous > decode(boost::optional<T> &v, ceph::buffer::list::const_iterator &p)
    {
        bool x;
        denc(x, p);
        if (x) {
