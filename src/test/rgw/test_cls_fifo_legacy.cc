...
namespace {
    int fifo_create(const DoutPrefixProvider * dpp, R::IoCtx & ioctx,
                    ...
                    std::nullopt, bool exclusive =
                    false, std::uint64_t max_part_size =
                    RCf::default_max_part_size, std::uint64_t max_entry_size =
...
TEST_F(LegacyClsFIFO, TestCreate)
{
    ...
    r = fifo_create(&dp, ioctx, fifo_id, fifo_id, null_yield, std::nullopt,
                    std::nullopt, false, 0);
    EXPECT_EQ(-EINVAL, r);
    r = fifo_create(&dp, ioctx, fifo_id, {
                    }, null_yield,
    ...
    EXPECT_EQ(0, r);
    r = fifo_create(&dp, ioctx, fifo_id, {
                    }, null_yield, std::nullopt, std::nullopt, false);
    EXPECT_EQ(-EINVAL, r);
    r = fifo_create(&dp, ioctx, fifo_id, {
                    }, null_yield, std::nullopt, "myprefix" sv, false);
    EXPECT_EQ(-EINVAL, r);
...
TEST_F(LegacyClsFIFO, TestGetInfo)
{
    ...
    r = RCf::get_meta(&dp, ioctx, fifo_id, std::nullopt, &info,
                      &part_header_size, &part_entry_overhead, 0, null_yield);
    EXPECT_EQ(0, r);
    EXPECT_GT(part_header_size, 0);
    EXPECT_GT(part_entry_overhead, 0);
    EXPECT_FALSE(info.version.instance.empty());

    r = RCf::get_meta(&dp, ioctx, fifo_id, info.version, &info,
                      &part_header_size, &part_entry_overhead, 0, null_yield);
    EXPECT_EQ(0, r);
    fifo::objv objv;
    objv.instance = "foo";
...
TEST_F(LegacyFIFO, TestOpenParams)
{
    ...
    auto r =
        RCf::FIFO::create(&dp, ioctx, fifo_id, &f, null_yield, objv, oid_prefix,
...
TEST_F(LegacyFIFO, TestPushListTrim)
{
    ...
    r = f->list(&dp, max_entries * 10, std::nullopt, &result, &more,
                null_yield);
    ASSERT_EQ(0, r);
...
TEST_F(LegacyFIFO, TestPushTooBig)
{
    ...
    std::unique_ptr < RCf::FIFO > f;
    auto r =
        RCf::FIFO::create(&dp, ioctx, fifo_id, &f, null_yield, std::nullopt,
...
TEST_F(LegacyFIFO, TestTwoPushersTrim)
{
    ...
    auto r =
        RCf::FIFO::create(&dp, ioctx, fifo_id, &f1, null_yield, std::nullopt,
                          std::nullopt, false, max_part_size,
                          max_entry_size);
    ASSERT_EQ(0, r);
...
TEST_F(LegacyFIFO, TestAioTrim)
{
    ...
    for (auto i = 0u; i < max_entries; ++i) {
        ...
        marker = result.front().marker;
        std::unique_ptr < R::AioCompletion >
            c(rados.aio_create_completion(nullptr, nullptr));
...
TEST_F(AioLegacyFIFO, TestMultipleParts)
{
...

    {
        ...
        f->get_head_info(&dp,[&](int r, RCf::part_info && p) {
                         ...
                         ASSERT_EQ(0, p.max_index); ASSERT_EQ(ceph::real_time {
                                                              }, p.max_time);},
                         c);
        c->wait_for_complete();
        r = c->get_return_value();
    ...
    c = R::Rados::aio_create_completion();
    f->get_head_info(&dp,[&](int r, RCf::part_info && p) {
                     ASSERT_EQ(next_ofs, p.next_ofs);}, c);
    c->wait_for_complete();
    r = c->get_return_value();
    c->release();
    ASSERT_EQ(0, r);
