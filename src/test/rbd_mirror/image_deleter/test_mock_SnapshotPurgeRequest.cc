...
namespace rbd {
    namespace mirror {
        namespace image_deleter {
            ...
            class TestMockImageDeleterSnapshotPurgeRequest:public
                TestMockFixture {
              public:
                ...
                void expect_snap_unprotect(librbd::
                    ...
                    EXPECT_CALL(*mock_image_ctx.operations,
                                execute_snap_unprotect(ns, name, _))
                        .WillOnce(WithArg < 2 >
                ...
                void expect_start_op(librbd::MockTestImageCtx & mock_image_ctx,
                                     bool success) {
                    EXPECT_CALL(*mock_image_ctx.exclusive_lock, start_op(_))
                        .WillOnce(Invoke([success] (int *r) {
                                         ...
                                         *r = -EROFS;
                                         return static_cast < LambdaContext <
                                         decltype(f) > *>(nullptr);}
            ...
            TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SuccessJournal) {
                {
                    ...
                    m_local_image_ctx->add_snap(cls::rbd::UserSnapshotNamespace {
                                                }
                                                , "snap1", 1, 0, {
                                                }
                                                ,
                ...
                expect_snap_remove(mock_image_ctx,
                                   cls::rbd::UserSnapshotNamespace {
                                   }
                                   , "snap2", 0);
                ...
                expect_start_op(mock_image_ctx, true);
                expect_snap_unprotect(mock_image_ctx,
                                      cls::rbd::UserSnapshotNamespace {
                                      }
                                      , "snap1", 0);
                expect_start_op(mock_image_ctx, true);
                ...
                C_SaferCond ctx;
                auto req =
                    MockSnapshotPurgeRequest::create(m_local_io_ctx,
                                                     mock_image_ctx.id,
                                                     &ctx);
                req->send();
