...
void RGWHTTPManager::register_request(rgw_http_req_data *req_data)
{
    ...
    num_reqs++;
    ldout(cct, 20) << __func__ << " mgr=" << this << " req_data->id=" << req_data->id << ", curl_handle=" <<
                   req_data->curl_handle << dendl;
}
...
{
    ldout(cct, 20) << __func__ << " req_data=" << req_data << " req_data->id=" << req_data->id << ", curl_handle=" <<
                   req_data->curl_handle << dendl;
                ...
                switch (result) {
                    ...
                    case CURLE_OPERATION_TIMEDOUT:
                        dout(0) << "WARNING: curl operation timed out, network average transfer speed less than "
                                << cct->_conf->rgw_curl_low_speed_limit << " Bytes per second during " << cct->_conf->rgw_curl_low_speed_time <<
                                " seconds." << dendl;
                    // coverity[unterminated_case:SUPPRESS]
                    default:
                        dout(20) << "ERROR: msg->data.result=" << result << " req_data->id=" << id << " http_status=" << http_status << dendl;
                        dout(20) << "ERROR: curl error: " << curl_easy_strerror((CURLcode)result) << " req_data->error_buf=" <<
                                 req_data->error_buf << dendl;
                        break;
                }
            }
        }
