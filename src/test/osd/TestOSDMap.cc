...
public:
                                     ...
                                     const string &root, const string &type)
    {
        ...
        string device_class;
        stringstream ss;
        int ruleno = newcrush.add_simple_rule(name, root, type, device_class,
                                              "firstn",
...
TEST_F(OSDMapTest, PGTempRespected)
{
    ...
    *new_acting_osds.rbegin() = first;
    ...
    pgtemp_map.new_pg_temp[pgid] =
        mempool::osdmap::vector < int >(new_acting_osds.begin(),
                                        new_acting_osds.end());
...
TEST_F(OSDMapTest, CleanPGUpmaps)
{
...

    {
        ...
        pending_inc.new_pg_upmap_items[pgid] =
            ...
            int32_t >> (new_pg_upmap_items.begin(), new_pg_upmap_items.end());
        OSDMap nextmap;
...

    {
        ...
        vector < int >up_after_out;
        {
            ...
            osdmap.crush->get_leaves(osdmap.crush->
        ...
        }
        {
            ...
            osdmap.apply_incremental(pending_inc);
            {
                ...
                ASSERT_EQ(new_up.size(), up.size());
                ASSERT_TRUE(std::
                            find(new_up.begin(), new_up.end(),
                                 replaced_by) != new_up.end());
                // and up[1] too
...
TEST_F(OSDMapTest, BUG_38897)
{
    ...
    // ready to go
    {
        ...
        osdmap.calc_pg_upmaps(g_ceph_context, 0,    // so we can force optimizing
                              100, only_pools, &pending_inc);
        osdmap.apply_incremental(pending_inc);
    }
}
        ...
        pg_t rep_pgid2 = tmp.raw_pg_to_pg(rep_pg2);
        {
            ...
            for (auto &from : from_osds) {
                ...
                for (auto &i : new_pg_upmap_items) {
                    ...
                    auto rack_to =
                        tmp.crush->get_parent_of_type(i.second, 3, rno);
...
TEST_P(OSDMapTest, BUG_51842)
{
    ...
    crush_rule_set_step(rule, step++, CRUSH_RULE_CHOOSELEAF_FIRSTN, rep_num,
                        domain);
    crush_rule_set_step(rule, step++, CRUSH_RULE_EMIT, 0, 0);
    ASSERT_TRUE(step == steps);
    auto r = crush_add_rule(crush.get_crush_map(), rule, rno);
                ...
                for (uint i = 0; i < num_osds; i++) {
                    if ((float (i + 1) / float (num_osds)) < fratio) {
                        ...
                        osdmap.set_primary_affinity(osds[i],
                                                    CEPH_OSD_MAX_PRIMARY_AFFINITY);
                        rc = osdmap.calc_read_balance_score(g_ceph_context, pid,
                                                            &rbi);
...
TEST_F(OSDMapTest, read_balance_small_map)
{
    ...
    for (const auto &test : test_cases) {
        ...
        auto rc =
            osdmap.calc_read_balance_score(g_ceph_context, my_rep_pool,
                                           &rb_info);
        ASSERT_TRUE(rc >= 0);
        ...
        if (num_changes > 0) {
            ...
            map < uint64_t, set < pg_t >> prim_pgs_by_osd_4,
