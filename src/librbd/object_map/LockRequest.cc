    ...
    namespace object_map {
        ...
        :m_image_ctx(image_ctx), m_on_finish(on_finish), m_broke_lock(false) {
            ...
            std::string oid(ObjectMap <>::
                            object_map_name(m_image_ctx.id, CEPH_NOSNAP));
            ldout(cct,
        ...
        template < typename I >
            ...
            if (*ret_val == 0) {
                ...
                *ret_val = 0;
                ...
                lderr(cct) << "failed to lock object map: " <<
        ...
        template < typename I > void LockRequest < I >::send_get_lock_info() {
            ...
            librados::AioCompletion * rados_completion =
                ...
                &klass::handle_get_lock_info > (this);
            int r =
        ...
        template < typename I >
            ...
            if (*ret_val < 0) {
                lderr(cct) << "failed to list object map locks: " <<
                    cpp_strerror(*ret_val)
                    << dendl;
                *ret_val = 0;
                return m_on_finish;
        ...
        template < typename I > void LockRequest < I >::send_break_locks() {
            ...
            std::string oid(ObjectMap <>::
                            object_map_name(m_image_ctx.id, CEPH_NOSNAP));
            ldout(cct,
                  10) << this << " " << __func__ << ": oid=" << oid << ", " <<
                "num_lockers=" << m_lockers.size() << dendl;

            librados::ObjectWriteOperation op;
          for (auto & locker:m_lockers) {
                rados::cls::lock::break_lock(&op, RBD_LOCK_NAME,
                                             locker.first.cookie,
                                             locker.first.locker);
            }
        ...
        template < typename I >
            ...
            lderr(cct) << "failed to break object map lock: " <<
                cpp_strerror(*ret_val)
                << dendl;
            *ret_val = 0;
            return m_on_finish;
