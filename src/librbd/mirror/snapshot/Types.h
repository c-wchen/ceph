...
struct ImageStateHeader {
    ...
    ImageStateHeader()
    {
    } ImageStateHeader(uint32_t
                       object_count): object_count(object_count)
    {
...
struct ImageState {
    ...
    ImageState()
    {
    } ImageState(const std::string &name, uint64_t features,
                 uint64_t snap_limit, const std::map < uint64_t,
                 SnapState > &snapshots,
