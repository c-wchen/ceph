...
namespace _mosdop {
      ...
      public:
        ...
        int get_client_inc() const {
            ...
            if (hobj.oid.name.empty())
                return object_locator_t(hobj.pool, hobj.nspace,
                                        hobj.get_hash());
            else
   ...
   */ int get_retry_attempt() const {
        ...
        :MOSDFastDispatchOp(CEPH_MSG_OSD_OP, HEAD_VERSION, COMPAT_VERSION),
            ...
            final_decode_needed(true), bdata_encode(false) {
        }
        MOSDOp(int inc, long tid, const hobject_t & ho, spg_t & _pgid,
               epoch_t _osdmap_epoch, int _flags, uint64_t feat)
      :    
        MOSDFastDispatchOp(CEPH_MSG_OSD_OP, HEAD_VERSION, COMPAT_VERSION),
        client_inc(inc),
        ...
        final_decode_needed(false), features(feat), bdata_encode(false) {
            set_tid(tid);
      ...
      public:
        ...
        bool has_flag(__u32 flag) const {
            return flags & flag;
        };
      ...
      private:
        template < class T, typename ... Args >
            friend boost::intrusive_ptr < T > ceph::make_message(Args
                                                                 && ... args);
    };
}
