    ...
    void make_omap_data(size_t object_count, int64_t poolid, coll_t cid)
    {
        ...
        for (size_t o = 0; o < object_count; o++) {
            ...
            for (size_t i = 0; i < omap_count; i++) {
                std::string name =
                    generate_monotonic_name(omap_count, i,
                                            3.66 + 0.22 * o / object_count,
                                            0.5);
                bufferlist val;
...
TEST_P(StoreTest, FiemapHoles)
{
    ...
    }
    {
        ...
        bufferlist bl;
        store->fiemap(ch, oid, SKIP_STEP, SKIP_STEP * (MAX_EXTENTS - 2) + 3,
                      bl);
        map < uint64_t, uint64_t > m, e;
        auto p = bl.cbegin();
        decode(m, p);
        ...
        if (string(GetParam()) != "kstore") {
            ...
            ASSERT_GT(last->first + last->second,
                      SKIP_STEP * (MAX_EXTENTS - 1));
        }
    }
    {
        ObjectStore::Transaction t;
...
TEST_P(StoreTest, SimpleListTest)
{
    ...
    }
    {
        ...
        while (!next.is_max()) {
            int r =
                collection_list(store, ch, current, ghobject_t::get_max(), 50,
                                &objects, &next);
            ASSERT_EQ(r, 0);
...
TEST_P(StoreTest, List_0xfffffff_Hash_Test_in_meta)
{
    ...
    {
        vector < ghobject_t > objects;
        r = collection_list(store, ch, ghobject_t(), ghobject_t::get_max(),
                            INT_MAX, &objects, nullptr, true);
        ASSERT_EQ(r, 0);
...
TEST_P(StoreTest, ManyObjectTest)
{
    ...
    objects.clear();
    r = collection_list(store,
                        ch,
                        ghobject_t::get_max(),
                        ghobject_t::get_max(), 50, &objects, &next);
    ASSERT_EQ(r, 0);
    ...
    while (1) {
        r = collection_list(store, ch, start, ghobject_t::get_max(), 50,
                            &objects, &next);
        ASSERT_TRUE(sorted(objects));
        ASSERT_EQ(r, 0);
...
public:
    ...
    int clone_range()
    {
        ...
        if (dstdata.length() <= dstoff) {
            ...
            if (value.length() < dstdata.length())
                dstdata.cbegin(value.length()).copy(dstdata.length() -
                                                    value.length(), value);
            value.swap(dstdata);
        }
    ...
    int write()
    {
        ...
        if (data.length() <= offset) {
            ...
            if (value.length() < data.length())
                data.cbegin(value.length()).copy(data.length() - value.length(),
                                                 value);
            value.swap(data);
        }
    ...
    void scan()
    {
        ...
        while (1) {
            ...
            int r =
                collection_list(store, ch, current, ghobject_t::get_max(), 100,
                                &objects, &next);
            ASSERT_EQ(r, 0);
            ASSERT_TRUE(sorted(objects));
...
void StoreTest::doSyntheticTest(int num_ops,
...
TEST_P(StoreTest, HashCollisionTest)
{
    ...
    while (1) {
        r = collection_list(store, ch, current, ghobject_t::get_max(), 60,
                            &objects, &next);
        ASSERT_EQ(r, 0);
        ASSERT_TRUE(sorted(objects));
        for (vector < ghobject_t >::iterator i = objects.begin();
                  ...
                  unsigned common_suffix_size, bool clones)
{
    ...
    small.append("small");
    {
        ...
        for (uint32_t i = 0; i < (2 - (int)clones) * num_objects; ++i) {
            ...
            objname << "obj" << i;
            ghobject_t a(hobject_t(objname.str(),
                                   "",
                                   CEPH_NOSNAP,
            ...
            if (clones) {
                objname << "-clone";
                ghobject_t b(hobject_t(objname.str(),
                                       "",
                                       CEPH_NOSNAP,
                                       i << common_suffix_size, 52, ""));
                t.clone(cid, a, b);
    ...
    objects.clear();
    r = collection_list(store, tch, ghobject_t(), ghobject_t::get_max(),
                        INT_MAX, &objects, 0);
    ASSERT_EQ(r, 0);
    ASSERT_EQ(objects.size(), num_objects);
    for (vector < ghobject_t >::iterator i = objects.begin();
                       ...
                       unsigned anum, unsigned bnum)
{
    ...
    set < ghobject_t > aobjects, bobjects;
    {
        ...
        for (unsigned i = 0; i < 1000; ++i) {
            ...
            ghobject_t o(hobject_t(objname,
                                   "",
                                   CEPH_NOSNAP,
                                   i << (bits + 1) | base, 52, ""));
            aobjects.insert(o);
    ...
    }
    {
        for (auto offset : offsets) {
            for (auto length : lengths) {
                ...
                uint32_t write_size =
                    (last_db - first_db + 1) * t.bdev_block_size;
...
TEST_P(StoreTestSpecificAUSize, DeferredOnBigOverwrite)
{
    ...
    PerfCounters *logger =
        const_cast < PerfCounters * >(store->get_perf_counters());
    ...
    }
    {
        ...
        t.write(cid, hoid2, 0, bl2.length(), bl2,
                CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
...

    {
        ...
        r = store->read(ch, hoid, 0x20000 + 2 * block_size, block_size * 30,
                        bl);
...
TEST_P(StoreTestSpecificAUSize, DeferredDifferentChunks)
{
    ...
    for (size_t expected_write_size = 1024;
         expected_write_size <= prefer_deferred_size;
        ...
        // check whether write will properly use deferred
        {
            ...
            t.write(cid, hoid, large_object_size - 2 * alloc_size - 1,
...
TEST_P(StoreTestSpecificAUSize, ReproBug56488Test)
{
    ...
    }
    {
        ...
        ASSERT_EQ(logger->get(l_bluestore_issued_deferred_writes),
                  issued_dw + 1);
...
TEST_P(StoreTest, FixSMRWritePointer)
{
    ...
    BlockDevice *bdev =
        BlockDevice::create(g_ceph_context, p, nullptr, nullptr, nullptr,
                            nullptr);
    r = bdev->open(p);
    ASSERT_EQ(0, r);
    ...
    uint64_t first_seq_zone =
        bdev->get_conventional_region_size() / bdev->get_zone_size();
                   ...
                   std::function < void (ObjectStore *) > do_check_fn)
{
    ...
    auto r = admin_socket->execute_command({ "{\"prefix\": \"bluefs stats\"}" }
                                           ,
                                           in, err, &out);
    if (r != 0) {
...
TEST_P(StoreTestSpecificAUSize, BluefsWriteInSingleDiskEnvTest)
{
    ...
    BlueStore *bstore = dynamic_cast < BlueStore * >(store.get());
    ceph_assert(bstore);
    bstore->inject_bluefs_file("db.slow", "store_test_injection_slow",
                               1 << 20ul);
