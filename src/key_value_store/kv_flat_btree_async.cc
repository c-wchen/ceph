...
int KvFlatBtreeAsync::split(const index_data & idata)
{
    ...
    map < std::string, bufferlist >::const_iterator it =
    ...
    ops.push_back(make_pair(pair < int, string > (ADD_PREFIX, index_name),
                            &owos[0]));
    for (int i = 1; i < 6; i++) {
                                ...
                                const index_data & next_idata)
{
    ...
    if ((int)args1.odata.size + (int)args2.odata.size <= 2 * k) {
        ...
        to_create.push_back(object_data(args1.odata.min_kdata,
                                        ...
                                        write2_map));
        ops.push_back(make_pair
                      (pair < int, string > (MAKE_OBJECT, o2w), &create[0]));
        ...
        to_create.push_back(object_data(key_data(write1_map.rbegin()->first),
                                        ...
                                        write2_map));
        ops.push_back(make_pair
                      (pair < int, string > (MAKE_OBJECT, o1w), &create[0]));
        ops.push_back(make_pair
                      (pair < int, string > (MAKE_OBJECT, o2w), &create[1]));
    }
                                  ...
                                  const index_data & idata, int *err)
{
    ...
    if (create_vector.size() > 0) {
        for (int i = 0; i < (int)idata.to_delete.size(); ++i) {
            it->first =
                pair < int, string > (UNWRITE_OBJECT, idata.to_delete[i].obj);
    ...
    for (int i = 0; i < (int)idata.to_create.size(); ++i) {
        ...
        if (idata.to_create.size() <= 2) {
            it->first =
                pair < int, string > (MAKE_OBJECT, idata.to_create[i].obj);
        }
        else {
            it->first =
                pair < int, string > (AIO_MAKE_OBJECT, idata.to_create[i].obj);
        }
    ...
    for (int i = 0; i < (int)idata.to_delete.size(); ++i) {
        ...
        it->first =
            pair < int, string > (REMOVE_OBJECT, idata.to_delete[i].obj);
...
void KvFlatBtreeAsync::set_up_make_object(const map < std::string,
                                          bufferlist > &to_set,
                                          librados::ObjectWriteOperation * owo)
{
    bufferlist inbl;
...
int KvFlatBtreeAsync::get_all_keys_and_values(map < std::string,
                                              bufferlist > *kv_map)
{
    if (verbose)
