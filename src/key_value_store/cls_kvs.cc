...
static int omap_insert(cls_method_context_t hctx,
                       const map < string, bufferlist > &omap, int bound,
                       bool exclusive)
{
    ...
    int old_size_int =
        atoi(string(old_size.c_str(), old_size.length()).c_str());

    CLS_LOG(20, "asserting size is less than %d (bound is %d)", assert_bound,
            bound);
...
CLS_INIT(kvs)
{
    ...
    cls_register_cxx_method(h_class, "read_many",
                            CLS_METHOD_RD, read_many_op, &h_read_many);
    ...
    cls_register_cxx_method(h_class, "omap_insert",
                            CLS_METHOD_WR, omap_insert_op, &h_omap_insert);
    ...
    cls_register_cxx_method(h_class, "omap_remove",
                            CLS_METHOD_WR, omap_remove_op, &h_omap_remove);
