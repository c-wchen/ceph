...
inline bool IsDebuggerAttached()
{
    ...
    for (const char *characterPtr =
         tracer_pid_ptr + sizeof(tracerPidString) - 1;
                             ...
                             const char *functionname)
{
    if (ldpp)
        ldpp_dout(ldpp,
                  20) << filename << "(" << linenumber << ") " << functionname
...
#define ldpp_dout(dpp, v)                                                     \
  ...
  dout_impl(pdpp->get_cct(), ceph::dout::need_dynamic(pdpp->get_subsys()), v) \
    ...
    class DaosNotification:public StoreNotification {
      public:
        DaosNotification(Object * _obj, Object * _src_obj,
    ...
    class DaosUser:public StoreUser {
      ...
      public:
        ...
        } DaosUser(DaosStore * _st, const RGWUserInfo & _i):StoreUser(_i),
            store(_st) {
        }
  ...
  /** Read user info without loading it */
        ...
        std::unique_ptr <
            struct ds3_user_info >get_encoded_info(bufferlist & bl,
                                                   obj_version & obj_ver);
...
// into a different index.
  ...
  /** Container ds3b handle */
      ...
      DaosBucket(DaosStore * _st, User * _u):StoreBucket(_u), store(_st),
            acls() {
        }
        ...
        std::unique_ptr <
            struct ds3_bucket_info >get_encoded_info(bufferlist & bl,
                                                     ceph::real_time mtime);
    ...
    class DaosPlacementTier:public StorePlacementTier {
      ...
      public:
        DaosPlacementTier(DaosStore * _store,
                          const RGWZoneGroupPlacementTier & _tier)
  ...
  /** Get the endpoint from zonegroup, or from master zone if not set */
    ...
    class DaosObject:public StoreObject {
      ...
      public:
        ...
        int write(const DoutPrefixProvider * dpp, bufferlist
                  && data, uint64_t offset);
        // Read size bytes from object starting from offset
        int read(const DoutPrefixProvider * dpp, bufferlist & data,
    ...
    class DaosMultipartWriter:public StoreWriter {
      ...
      public:
        DaosMultipartWriter(const DoutPrefixProvider * dpp, optional_yield y,
                            ...
                            uint64_t _part_num,
                            const std::string & part_num_str)
      :    StoreWriter(dpp, y), store(_store), upload(_upload),
            upload_id(_upload->get_upload_id()), part_num(_part_num),
