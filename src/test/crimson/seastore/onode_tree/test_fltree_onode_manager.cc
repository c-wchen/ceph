...
struct onode_item_t {
    ...
    void initialize(Transaction & t, Onode & value) const {
        ...
        ceph_assert(laddr_t(layout.size) == laddr_t {
                    size});
        ceph_assert(layout.omap_root.get(value.get_metadata_hint(block_size)).
                    addr == id);
        ceph_assert(layout.omap_root.get(value.get_metadata_hint(block_size)).
                    depth == cnt_modify);
...
:public seastar_test_suite_t, TMTestState {
    ...
    virtual seastar::future <> _init() final {
        return TMTestState::_init().then([this] {
                                         manager.
                                         reset(new FLTreeOnodeManager(*tm));}
        );
    }
    ...
    virtual FuturizedStore::mkfs_ertr::future <> _mkfs() final {
        return TMTestState::_mkfs().safe_then([this] {
                                              return restart_fut();}
        ).safe_then([this] {
    ...
    void validate_onode(iterator_t & it) {
        with_transaction([this, &it] (auto & t) {
                         ...
                         auto onode = with_trans_intr(t,[&](auto & t) {
                                                      return manager->
                                                      get_onode(t, p_kv->key);}
                         ).unsafe_get0(); p_kv->value.validate(*onode);}
        );
    }
        ...
        with_transaction([this, &it] (auto & t) {
                         ...
                         auto exist = with_trans_intr(t,[&](auto & t) {
                                                      ...
                                                      contains_onode(t,
                                                                     p_kv->
                                                                     key);}
                         ).unsafe_get0(); ceph_assert(exist == false);}
        );
    }

    template < typename F >
        void with_onodes_process(const iterator_t & start,
                                 const iterator_t & end, F && f) {
    ...
    template < typename F >
        void with_onodes_write(const iterator_t & start, const iterator_t & end,
                               F && f) {
        with_onodes_process(start, end,
                            [this, f =
                             std::move(f)] (auto & t, auto & oids,
                                            auto & items) {
                            auto onodes = with_trans_intr(t,[&](auto & t) {
                                                          return manager->
                                                          get_or_create_onodes
                                                          (t, oids);}
          ).unsafe_get0(); for (auto tup:boost::combine(onodes,
                                           items))
                            {
                            OnodeRef onode; onode_item_t * p_item;
    ...
    void validate_onodes(const iterator_t & start, const iterator_t & end) {
        with_onodes_process(start, end,
          ...
          for (auto tup:   boost::combine(oids,
                                           items)) {
                            ghobject_t oid; onode_item_t * p_item;
                            boost::tie(oid, p_item) = tup;
                            auto onode = with_trans_intr(t,[&](auto & t) {
                                                         return manager->
                                                         get_onode(t, oid);}
                            ).unsafe_get0(); p_item->validate(*onode);}
                            }
        ) ;
    }
    ...
    void validate_list_onodes(KVPool < onode_item_t > &pool) {
        with_onodes_process(pool.begin(), pool.end(),
                            ...
                            assert(start < end);
                            assert(start < oids[0]);
                            assert(oids[0] < end); while (start != end) {
                            auto[list_ret, list_end] =
                            with_trans_intr(t,[&](auto & t) {
                                            return manager->list_onodes(t,
                                                                        start,
                                                                        end,
                                                                        LIST_LIMIT);}
                            ).unsafe_get0();
                            listed_oids.insert(listed_oids.end(),
                                               list_ret.begin(),
                                               list_ret.end());
                            start = list_end;}
...
TEST_F(fltree_onode_manager_test_t, 1 _single)
{
    run_async([this] {
...
TEST_F(fltree_onode_manager_test_t, 2 _synthetic)
{
    run_async([this] {
              uint64_t block_size = tm->get_block_size();
              auto pool = KVPool < onode_item_t >::create_range( {
                                                                ...
                                                                256, 512}
                                                                , block_size);
              auto start = pool.begin(); auto end = pool.end();
              ...
              validate_onodes(start, end); validate_list_onodes(pool);}
    );
}
