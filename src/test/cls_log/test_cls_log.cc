...
TEST(cls_rgw, test_log_add_same_time)
{
    ...
    cls_log_list(*rop, start_time, to_time, marker, 1, entries, &marker,
                 &truncated);
...
TEST(cls_rgw, test_log_add_different_time)
{
    ...
    cls_log_list(*rop, next_time, to_time, marker, 0, entries, &marker,
                 &truncated);
    ...
    do {
        ...
        cls_log_list(*rop, start_time, to_time, old_marker, 1, entries, &marker,
                     &truncated);
...
TEST(cls_rgw, test_log_trim)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ASSERT_EQ(0,
                  cls_log_trim(ioctx, oid, zero_time, trim_time, start_marker,
                               end_marker));
