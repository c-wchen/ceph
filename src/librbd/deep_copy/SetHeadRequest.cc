...
} template < typename I > void SetHeadRequest < I >::send_set_size()
{
    ...
    r = m_image_ctx->md_ctx.aio_operate(m_image_ctx->header_oid, comp,
                                        &op);
    ceph_assert(r == 0);
    comp->release();
}
...
void SetHeadRequest < I >::handle_set_size(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to update image size: " <<
...
template < typename I > void SetHeadRequest < I >::send_detach_parent()
{
    ...
    auto req =
        image::DetachParentRequest < I >::create(*m_image_ctx, ctx);
    req->send();
}
...
void SetHeadRequest < I >::handle_detach_parent(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to remove parent: " << cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
    }
...
template < typename I > void SetHeadRequest < I >::send_attach_parent()
{
    ...
    auto req =
        image::AttachParentRequest < I >::create(*m_image_ctx,
            m_parent_spec,
            m_parent_overlap,
            false, ctx);
    req->send();
