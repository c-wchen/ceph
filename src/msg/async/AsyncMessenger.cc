    ...
    for (unsigned k = 0; k < bind_addrs.v.size(); ++k) {
        ...
        for (int i = 0; i < conf->ms_bind_retry_count; i++) {
            if (i > 0) {
                lderr(msgr->
                      cct) << __func__ <<
                           " was unable to bind. Trying again in " << conf->
                           ms_bind_retry_delay << " seconds " << dendl;
            ...
            if (listen_addr.get_port()) {
                ...
                [this, k, &listen_addr, &opts, &r]() {
                    r =
                        worker->listen(listen_addr, k, opts,
                                       &listen_sockets[k]);
                }
                , false);
                if (r < 0) {
                    lderr(msgr->
                          cct) << __func__ << " unable to bind to " <<
                               listen_addr << ": " << cpp_strerror(r) << dendl;
                    continue;
                }
            } else {
                ...
                for (int port = msgr->cct->_conf->ms_bind_port_min;
                    ...
                    worker->center.submit_to(worker->center.get_id(),
                                             [this, k, &listen_addr, &opts,
                    &r]() {
                        r =
                            worker->listen(listen_addr, k,
                                           opts,
                                           &listen_sockets
                                           [k]);
                    }
                ...
                if (r < 0) {
                    lderr(msgr->
                          cct) << __func__ << " unable to bind to " <<
                               listen_addr << " on any port in range " << msgr->cct->
                               _conf->ms_bind_port_min << "-" << msgr->cct->_conf->
        ...
        while (true) {
            ...
            if (r == 0) {
                ...
                msgr->add_accept(w, std::move(cli_socket),
                                 msgr->get_myaddrs().v[listen_socket.
                                                       get_addr_slot()], addr);
                accept_error_num = 0;
                continue;
            } else {
...
AsyncMessenger::AsyncMessenger(CephContext *cct, entity_name_t name,
                               ...
                               uint64_t _nonce)
    : SimplePolicyMessenger(cct, name), dispatch_queue(cct, this, mname),
      nonce(_nonce)
{
    ...
    local_connection =
        ceph::make_ref < AsyncConnection > (cct, this, &dispatch_queue,
                                ...
                                const entity_addr_t &peer_addr)
{
    std::lock_guard l {
        lock};
    auto conn =
        ceph::make_ref < AsyncConnection > (cct, this, &dispatch_queue, w,
                                            listen_addr.is_msgr2(), false);
    conn->accept(std::move(cli_socket), listen_addr, peer_addr);
    accepting_conns.insert(conn);
}

AsyncConnectionRef AsyncMessenger::
create_connect(const entity_addrvec_t &addrs, int type, bool anon)
{
...
entity_addrvec_t AsyncMessenger::_filter_addrs(const entity_addrvec_t &addrs)
{
    if (!should_use_msgr2()) {
        ldout(cct,
              10) << __func__ << " " << addrs << " limiting to v1 ()" << dendl;
        entity_addrvec_t r;
        for (auto &i : addrs.v) {
            if (i.is_msgr2()) {
                continue;
...
int AsyncMessenger::send_to(Message *m, int type,
        ...
        bool anon, bool not_local_dest)
{
    ...
    if (conn) {
        ldout(cct,
              10) << __func__ << " " << av << " existing " << conn << dendl;
    } else {
...
void AsyncMessenger::mark_down_addrs(const entity_addrvec_t &addrs)
{
    ...
    if (conn) {
        ...
        ldout(cct,
              1) << __func__ << " " << addrs << " -- connection dne" << dendl;
    }
}
...
int AsyncMessenger::accept_conn(const AsyncConnectionRef &conn)
{
    ...
    if (conn->policy.server &&
        conn->policy.lossy && !conn->policy.register_lossy_clients) {
        anon_conns.insert(conn);
    ...
    ldout(cct,
          10) << __func__ << " " << conn << " " << *conn->peer_addrs << dendl;
