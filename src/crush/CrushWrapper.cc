...
bool CrushWrapper::has_legacy_rule_ids() const const
{
        ...
        for (unsigned i = 0; i < b->size; ++i) {
            ...
            if (id == item) {
              ...
              for (auto & p:choose_args) {
                    ...
                    vector < int >weightv(get_choose_args_positions(p.second),
                                          0);
                    choose_args_adjust_item_weight(cct, p.second, item, weightv,
                                                   nullptr);
                }
    ...
    for (unsigned i = 0; i < b->size; ++i) {
        ...
        if (id == item) {
          ...
          for (auto & p:choose_args) {
                ...
                _choose_args_adjust_item_weight_in_bucket(cct, p.second, b->id,
                                                          item, weightv,
                                                          nullptr);
            }
            bucket_remove_item(b, item);
...
map < int, string > CrushWrapper::get_parent_hierarchy(int id)
{
    ...
    int high_type = 0;
    for (map < int, string >::iterator it = type_map.begin();
         it != type_map.end(); ++it) {
    ...
    for (auto p = type_map.begin(); p != type_map.end(); ++p) {
        ...
        if (!name_exists(q->second)) {
            ...
            int r = add_bucket(0, 0,
                               CRUSH_HASH_DEFAULT, p->first, 1, &cur, &empty,
                               &newid);
            if (r < 0) {
...
{
    ldout(cct,
          5) << "adjust_item_weight " << id << " weight " << weight << dendl;
    int changed = 0;
    for (int bidx = 0; bidx < crush->max_buckets; bidx++) {
        crush_bucket *b = crush->buckets[bidx];
        if (b == 0)
    ...
    for (auto l = loc.begin(); l != loc.end(); ++l) {
        ...
        for (unsigned int i = 0; i < b->size; i++) {
            if (b->items[i] == id) {
                int diff = bucket_adjust_item_weight(cct, b, id, weight);
                ldout(cct,
                      5) << "adjust_item_weight_in_loc " << id << " diff " <<
                    diff << " in bucket " << bid << dendl;
                             ...
                             string >> &classify_bucket)
{
  ...
  for (auto & i:classify_root) {
        ...
        if (class_exists(new_class)) {
            ...
            for (new_class_id = 1; class_name.count(new_class_id);
                 ++new_class_id) ;
            class_name[new_class_id] = new_class;
            class_rname[new_class] = new_class_id;
                                     ...
                                     vector < int >>>*cmap_item_weight)
{
  ...
  for (auto & w:choose_args) {
        ...
        n.weight_set =
            (crush_weight_set *) calloc(n.weight_set_positions,
    ...
    try {
        ...
        if (!blp.end()) {
            ...
            for (__u32 i = 0; i < choose_args_size; i++) {
                ...
                arg_map.args =
                    (crush_choose_arg *) calloc(arg_map.size,
...
void CrushWrapper::dump_rule(int ruleset, Formatter * f) const const
{
 ...
 */
int CrushWrapper::get_osd_pool_default_crush_replicated_ruleset(CephContext *
                                                                cct)
{
                                 ...
                                 vector < int >*out) const const
{
    ...
    ldout(cct, 10) << __func__ << " ruleno " << ruleno
        ...
        << " underfull " << underfull << " orig " << orig << dendl;
