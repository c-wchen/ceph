    ...
    namespace io {
        ...
        namespace {
            ...
            class UpdateObjectMap:public C_AsyncObjectThrottle <> {
              public:
                UpdateObjectMap(AsyncObjectThrottle <> &throttle,
                                ImageCtx * image_ctx, uint64_t object_no,
                                const std::vector < uint64_t > *snap_ids,
                                const ZTracer::Trace & trace,
                                size_t snap_id_idx)
                :C_AsyncObjectThrottle(throttle, *image_ctx),
                    m_object_no(object_no), m_snap_ids(*snap_ids),
                    m_trace(trace), m_snap_id_idx(snap_id_idx) {
                } int send() override {
                    uint64_t snap_id = m_snap_ids[m_snap_id_idx];
                    if (snap_id == CEPH_NOSNAP) {
                        RWLock::RLocker snap_locker(m_image_ctx.snap_lock);
                        RWLock::WLocker object_map_locker(m_image_ctx.
                                                          object_map_lock);
                        assert(m_image_ctx.exclusive_lock->is_lock_owner());
                        assert(m_image_ctx.object_map != nullptr);
                        bool sent =
                            m_image_ctx.object_map->aio_update < Context >
                            (CEPH_NOSNAP, m_object_no, OBJECT_EXISTS, { }
                             , m_trace, false, this);
                         return (sent ? 0 : 1);
                    ...
                    RWLock::RLocker snap_locker(m_image_ctx.snap_lock);
                    RWLock::RLocker object_map_locker(m_image_ctx.
                                                      object_map_lock);
                    ...
                    bool sent =
                        m_image_ctx.object_map->aio_update < Context > (snap_id,
                                                                        m_object_no,
                                                                        state,
                                                                        { }
                                                                        ,
        ...
        template < typename I >
            CopyupRequest < I >::CopyupRequest(I * ictx,
        ...
        :m_ictx(util::get_image_ctx(ictx)), m_oid(oid), m_object_no(objectno),
            ...
            m_state(STATE_READ_FROM_PARENT), m_lock("CopyupRequest", false,
                                                    false) {
        ...
        template < typename I > bool CopyupRequest < I >::send_copyup() {
            ...
            if (!copy_on_read) {
              ...
              for (auto req:m_pending_requests) {
                ...
                snaps.insert(snaps.end(), snapc.snaps.begin(),
                             snapc.snaps.end());
                librados::AioCompletion * comp =
                    util::create_rados_callback(this);
        ...
        template < typename I > void CopyupRequest < I >::send() {
            ...
            AioCompletion *comp =
                AioCompletion::create_and_start(this, m_ictx, AIO_TYPE_READ);

            ldout(m_ictx->cct, 20) << "completion " << comp
                << ", oid " << m_oid
                << ", extents " << m_image_extents << dendl;
            ImageRequest <>::aio_read(m_ictx->parent, comp,
                                      std::move(m_image_extents), ReadResult {
                                      &m_copyup_data}, 0, m_trace);
        }

        template < typename I > void CopyupRequest < I >::complete(int r) {
        ...
        template < typename I > bool CopyupRequest < I >::should_complete(int r) {
        ...
        template < typename I > bool CopyupRequest < I >::send_object_map_head() {
            CephContext *cct = m_ictx->cct;
            ldout(cct, 20) << dendl;
...

            {
                ...
                if (m_ictx->object_map != nullptr) {
                    ...
                    if (!m_ictx->snaps.empty()) {
                        m_snap_ids.insert(m_snap_ids.end(),
                                          m_ictx->snaps.begin(),
                                          m_ictx->snaps.end());
                    }
                    if (copy_on_read &&
                    ...
                    if (r_it != m_pending_requests.rend()) {
                        ...
                        ldout(cct, 20) << req->get_op_type() << " object no "
                            ...
                            << stringify(static_cast < uint32_t >
                                         (current_state))
                            << " new state " << stringify(static_cast <
                                                          uint32_t >
                                                          (new_state))
                            << dendl;
                        may_update = true;
                    ...
                    if (may_update && (new_state != current_state) &&
                        ...
                        (CEPH_NOSNAP, m_object_no, new_state, current_state,
                         m_trace, false, this)) {
                        return false;
                    }
                }
        ...
        template < typename I > bool CopyupRequest < I >::send_object_map() {
            ...
            if (m_snap_ids.empty()) {
                ...
                RWLock::RLocker owner_locker(m_ictx->owner_lock);
                AsyncObjectThrottle <>::ContextFactory context_factory(boost::
                                                                       lambda::
                                                                       bind
                                                                       (boost::
                                                                        lambda::
                                                                        ...
                                                                        lambda::
                                                                        _2));
                AsyncObjectThrottle <> *throttle =
                    new AsyncObjectThrottle <> (NULL, *m_ictx, context_factory,
                                                util::
                                                create_context_callback(this),
