                  ...
                  const map < string, string > &add_loc)
{
    ...
    if (crush.name_exists(add_name)) {
        cerr << me << " bucket '" << add_name << "' already exists" << std::
            endl;
        return -EEXIST;
    }
    ...
    int r = 0;
    r = crush.add_bucket(0, 0, CRUSH_HASH_DEFAULT, type, 0, nullptr, nullptr,
                         &bucketno);
    if (r < 0) {
    ...
    if (!add_loc.empty()) {
        if (!crush.check_item_loc(cct, bucketno, add_loc, (int *)nullptr)) {
            ...
            if (r < 0) {
                cerr << me << " error moving bucket '" << add_name << "' to " <<
                    add_loc << std::endl;
                return r;
            }
        }
...
int do_move_item(CephContext * cct,
                 const char *me,
                 CrushWrapper & crush,
                 const string & name, const map < string, string > &loc)
{
    ...
    if (loc.empty()) {
        cerr << me << " expecting additional --loc argument to --move" << std::
            endl;
        return -EINVAL;
    }
...
int main(int argc, const char **argv)
{
    ...
    for (std::vector < const char *>::iterator i = args.begin();
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            decompile = true;
                 ...
                 (args, i, &val, "-i", "--infn", (char *)NULL)) {
            infn = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "-o", "--outfn", (char *)NULL)) {
            outfn = val;
        }
            ...
            dump = true;
                 ...
                 (args, i, "--show_utilization", (char *)NULL)) {
            display = true;
            tester.set_output_utilization(true);
        }
        else if (ceph_argparse_flag
                 (args, i, "--show_utilization_all", (char *)NULL)) {
            display = true;
            ...
            tester.set_output_mappings(true);
                 ...
                 (args, i, "--show_bad_mappings", (char *)NULL)) {
            display = true;
            tester.set_output_bad_mappings(true);
        }
        else if (ceph_argparse_flag
                 (args, i, "--show_choose_tries", (char *)NULL)) {
            display = true;
            tester.set_output_choose_tries(true);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "-c", "--compile", (char *)NULL)) {
            srcfn = val;
            compile = true;
        }
        else if (ceph_argparse_witharg
                 (args, i, &max_id, err, "--check", (char *)NULL)) {
            check = true;
        }
            ...
            tester.set_random_placement();
                 ...
                 (args, i, "--enable-unsafe-tunables", (char *)NULL)) {
            ...
            reweight = true;
                 ...
                 (args, i, "--rebuild-class-roots", (char *)NULL)) {
            ...
            if (i == args.end()) {
                cerr << "expecting additional argument to --add-item" << std::
                    endl;
                return EXIT_FAILURE;
            }
            add_weight = atof(*i);
            ...
            if (i == args.end()) {
                cerr << "expecting additional argument to --update-item" <<
                    std::endl;
                return EXIT_FAILURE;
            }
            add_weight = atof(*i);
            ...
            i = args.erase(i);
                 ...
                 (args, i, &val, err, "--add-bucket", (char *)NULL)) {
            add_name.assign(val);
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            ...
            if (i == args.end()) {
                cerr << "expecting additional argument to --add-bucket" << std::
                    endl;
                return EXIT_FAILURE;
            }
            add_type = *i;
            ...
            cout << "--create-simple-rule:"
                << " name=" << rule_name
                << " root=" << rule_root
                << " type=" << rule_type << " mode=" << rule_mode << std::endl;
            add_rule = true;
        }
        else if (ceph_argparse_witharg
            ...
            add_rule = true;
                 ...
                 (args, i, &val, "--device-class", (char *)NULL)) {
            ...
            if (!err.str().empty()) {
                ...
                return EXIT_FAILURE;
                 ...
                 (args, i, &val, "--remove-rule", (char *)NULL)) {
            rule_name.assign(val);
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            ...
            return EXIT_SUCCESS;
                 ...
                 (args, i, &val, "--output-name", (char *)NULL)) {
            std::string name(val);
            if (i == args.end()) {
                cerr << "expecting additional argument to --output-name" <<
                    std::endl;
                return EXIT_FAILURE;
            }
            else {
                tester.set_output_data_file_name(name + "-");
                 ...
                 (args, i, &val, "--remove_item", (char *)NULL)) {
            remove_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--reweight_item", (char *)NULL)) {
            reweight_name = val;
            if (i == args.end()) {
                cerr << "expecting additional argument to --reweight-item" <<
                    std::endl;
                return EXIT_FAILURE;
            }
            ...
            if (!err.str().empty()) {
                ...
                return EXIT_FAILURE;
                 ...
                 (args, i, &x, err, "--num_rep", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            ...
            tester.set_min_x(x);
                 ...
                 (args, i, &z, err, "--pool_id", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            ...
            tester.set_x(x);
                 ...
                 (args, i, &x, err, "--max_rule", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            tester.set_max_rule(x);
                 ...
                 (args, i, &x, err, "--min_rule", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            ...
            tester.set_rule(x);
                 ...
                 (args, i, &x, err, "--ruleset", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            tester.set_ruleset(x);
                 ...
                 (args, i, &x, err, "--batches", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            ...
            tester.set_bucket_down_ratio(y);
                 ...
                 (args, i, &tmp, err, "--weight", (char *)NULL)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            int dev = tmp;
            if (i == args.end()) {
                cerr << "expecting additional argument to --weight" << std::
                    endl;
                return EXIT_FAILURE;
            }
            float f = atof(*i);
    ...
    if (!check && !compile && !decompile && !build && !test && !reweight
        && !adjust && !tree && !dump && add_item < 0 && !add_bucket
        && !move_item && !add_rule && !del_rule && full_location < 0
    ...
    if (build) {
        ...
        for (vector < layer_t >::iterator p = layers.begin(); p != layers.end();
            ...
            dout(2) << "layer " << type
                << "  " << l.name
                << "  bucket type " << l.buckettype << "  " << l.size << dendl;
            ...
            for (int i = 0; bucket_types[i].name; i++)
                if (l.buckettype
                    && strcmp(l.buckettype, bucket_types[i].name) == 0) {
            ...
            while (1) {
                ...
                for (j = 0; j < l.size || l.size == 0; j++) {
                    ...
                    weight += weights[j];
                    lower_pos++;
                    dout(2) << "  item " << items[j] << " weight " << weights[j]
                        << dendl;
                }

                int id;
                int r =
                    crush.add_bucket(0, buckettype, CRUSH_HASH_DEFAULT, type, j,
                                     items, weights, &id);
                if (r < 0) {
                    cerr << " Couldn't add bucket: " << cpp_strerror(r) << std::
                        endl;
                    return r;
                }
        ...
        if (OSDMap::
            build_simple_crush_rules(g_ceph_context, crush, root, &cerr))
            return EXIT_FAILURE;
    ...
    if (!reweight_name.empty()) {
        ...
        if (!crush.name_exists(reweight_name)) {
            ...
            r = crush.adjust_item_weightf(g_ceph_context, item,
                                          reweight_weight);
        }
        if (r >= 0)
            modified = true;
    ...
    if (add_item >= 0) {
        int r;
        if (update_item) {
            r = crush.update_item(g_ceph_context, add_item, add_weight,
                                  add_name.c_str(), add_loc);
        }
        else {
            r = crush.insert_item(g_ceph_context, add_item, add_weight,
                                  add_name.c_str(), add_loc);
        }
        if (r >= 0) {
            modified = true;
    ...
    if (add_bucket) {
        int r =
            do_add_bucket(cct.get(), me, crush, add_name, add_type, add_loc);
        if (!r) {
            modified = true;
        }
        else {
    ...
    if (add_rule) {
        ...
        int r = crush.add_simple_rule(rule_name, rule_root, rule_type,
                                      rule_device_class,
                                      rule_mode, pg_pool_t::TYPE_REPLICATED,
                                      &err);
        if (r < 0) {
            cerr << err.str() << std::endl;
            return EXIT_FAILURE;
  ...
  for (auto & i:set_subtree_class) {
    ...
    if (dump) {
        boost::scoped_ptr < Formatter >
            f(Formatter::create(dump_format, "json-pretty", "json-pretty"));
