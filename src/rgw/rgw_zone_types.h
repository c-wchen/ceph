...
} /* namespace rgw_zone_defaults */ struct RGWNameToId {
    std::string obj_id;
...
public:
    ...
    void set_storage_class(const std::string &sc, const rgw_pool *data_pool,
...
struct RGWZonePlacementInfo {
    ...
    } void decode(bufferlist::const_iterator &bl)
    {
        ...
        if (struct_v >= 7) {
            ...
            storage_classes.set_storage_class(RGW_STORAGE_CLASS_STANDARD,
                                              &standard_data_pool,
...
struct RGWTierACLMapping {
    ...
    RGWTierACLMapping(ACLGranteeTypeEnum t,
                      const std::string &s,
                      const std::string &d): type(t), source_id(s), dest_id(d)
    {
...
struct RGWZoneGroupPlacementTarget {
    ...
    bool user_permitted(const std::list < std::string > &user_tags) const const
    {
        if (tags.empty()) {
            return true;
        }
