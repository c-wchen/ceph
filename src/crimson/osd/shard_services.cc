...
namespace crimson::osd {
    ...
    PerShardState::PerShardState(int whoami,
    ...
    :whoami(whoami),
        ...
        next_tid(static_cast < ceph_tid_t > (seastar::this_shard_id()) <<
    ...
    seastar::future <> PerShardState::
        broadcast_map_to_pgs(ShardServices & shard_services, epoch_t epoch) {
        assert_core();
        auto & pgs = pg_map.get_pgs();
        return seastar::parallel_for_each(pgs.begin(), pgs.end(),
                                          [ =, &shard_services] (auto & pg) {
                                          return shard_services.
                                          start_operation < PGAdvanceMap >
                                          (shard_services, pg.second, epoch,
                                           PeeringCtx {
                                           }
                                           , false).second;}
        );
    ...
    OSDSingletonState::OSDSingletonState(int whoami,
  ...
  :    
    ...
    seastar::future <> OSDSingletonState::send_to_osd(int peer, MessageURef m,
                                                      epoch_t from_epoch) {
        if (osdmap->is_down(peer)) {
            ...
            auto conn =
                cluster_msgr.connect(osdmap->get_cluster_addrs(peer).front(),
                                     CEPH_ENTITY_TYPE_OSD);
            return conn->send(std::move(m));
        }
    }

    seastar::future <> OSDSingletonState::osdmap_subscribe(version_t epoch,
                                                           bool force_request) {
    ...
    void OSDSingletonState::queue_want_pg_temp(pg_t pgid,
    ...
    void OSDSingletonState::requeue_pg_temp() {
        ...
        logger().debug("{}: {} + {} -> {}",
                       __func__,
                       old_wanted, old_pending, pg_temp_wanted.size());
    }
    ...
    seastar::future <> OSDSingletonState::send_pg_created(pg_t pgid) {
        ...
        return monc.send_message(crimson::make_message < MOSDPGCreated >
                                 (pgid));
    }
    ...
    seastar::future <> OSDSingletonState::send_alive(const epoch_t want) {
        logger().info("{} want={} up_thru_wanted={}",
        ...
        if (const epoch_t up_thru = osdmap->get_up_thru(whoami);
            ...
            logger().debug("{} up_thru_wanted={} up_thru={}", __func__, want,
                           up_thru);
            return monc.send_message(crimson::make_message < MOSDAlive >
                                     (osdmap->get_epoch(), want));
        }
        else {
            logger().debug("{} {} <= {}", __func__, want,
                           osdmap->get_up_thru(whoami));
            return seastar::now();
        }
    }
    ...
    void OSDSingletonState::handle_conf_change(const ConfigProxy & conf,
    ...
    seastar::future < OSDSingletonState::local_cached_map_t >
        ...
        if (auto found = osdmaps.find(e); found) {
            ...
            return load_map(e).
                then([e, this] (std::unique_ptr < OSDMap > osdmap) {
                     return seastar::make_ready_future < local_cached_map_t >
    ...
    void OSDSingletonState::store_map_bl(ceph::os::Transaction & t,
    ...
    seastar::future < std::map < epoch_t,
        bufferlist >> OSDSingletonState::load_map_bls(epoch_t first,
                                                      epoch_t last) {
        logger().debug("{} loading maps [{},{}]", __func__, first, last);
        ceph_assert(first <= last);
        return seastar::map_reduce(boost::make_counting_iterator < epoch_t >
                                   (first),
                                   boost::make_counting_iterator < epoch_t >
                                   (last + 1),[this] (epoch_t e) {
    ...
    seastar::future < std::unique_ptr <
        OSDMap >> OSDSingletonState::load_map(epoch_t e) {
        auto o = std::make_unique < OSDMap > ();
        if (e > 0) {
            return load_map_bl(e).
                then([o = std::move(o)] (bufferlist bl) mutable {
                     o->decode(bl);
                     return seastar::make_ready_future < std::unique_ptr <
                     OSDMap >> (std::move(o));}
            );
        }
        else {
    ...
    seastar::future <> OSDSingletonState::store_maps(ceph::os::Transaction & t,
        ...
        return seastar::do_for_each(boost::make_counting_iterator(start),
                                    ...
                                    if (auto p = m->incremental_maps.find(e);
    ...
    seastar::future < Ref <
        PG >> ShardServices::make_pg(OSDMapService::cached_map_t create_map,
        ...
        auto get_pool_info_for_pg =[create_map, pgid, this] {
            if (create_map->have_pg_pool(pgid.pool())){
                                                       ...
                                                       *create_map->
                                                       ...
                                                       ShardServices::
                                                       get_or_create_pg_ret
                                                       ShardServices::
                                                       get_or_create_pg(PGMap::
                                                                        PGCreationBlockingEvent::
                                                                        TriggerI
                                                                        &&
                                                                        trigger,
                                                                        spg_t
                                                       ...
                                                       handle_exception([pgid]
                                                                        (auto
                                                                         ep) {
                                                                        logger
                                                                        ().
                                                                        info
                                                                        ("pg {} saw exception on load {}",
                                                                         pgid,
                                                                         ep);
                                                                        ceph_abort
                                                                        ("Could not load pg"
                                                       ...
                                                       logger().
                                                       debug
                                                       ("ShardServices::dispatch_context_transaction: empty transaction");
                                                       return seastar::now();}

                                                       logger().
                                                       debug
                                                       ("ShardServices::dispatch_context_transaction: do_transaction ...");
                                                       auto ret =
                                                       get_store().
                                                       do_transaction(col,
                                                                      std::
                                                       ...
                                                       parallel_for_each(std::
                                                                         ...
                                                                         ().
                                                                         debug
                                                                         ("dispatch_context_messages sending messages to {}",
                                                                          peer);
                                                                         return
                                                                         seastar::
                                                                         parallel_for_each
