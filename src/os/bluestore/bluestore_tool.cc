...
BlueFS *open_bluefs(CephContext * cct,
                    const string & path, const vector < string > &devs)
{
  ...
  for (auto & i:devs) {
        ...
        if (id >= 0) {
            ...
            if (r < 0) {
                cerr << "unable to open " << i << ": " << cpp_strerror(r) <<
                    std::endl;
                exit(EXIT_FAILURE);
            }
        }
    ...
    if (action == "fsck" || action == "repair") {
                ...
                if (r < 0 && errno == EEXIST) {
                    ...
                    if (r == 0 && S_ISLNK(st.st_mode)) {
                        ...
                        if (r > 0) {
                            ...
                            cerr << "error reading existing link at " << p <<
                                ": " << cpp_strerror(errno)
                                << std::endl;
                        }
                    }
      ...
      for (int devid:{
            ...
            if (!fs->is_device_expandable(devid)) {
                cout << devid
                    << " : can't be expanded. Bypassing..." << std::endl;
                continue;
            }
            ...
            if (end < size) {
                ...
                if (path == nullptr) {
                    cerr << devid << ": can't find device path " << std::endl;
                    continue;
                }
                bluestore_bdev_label_t label;
