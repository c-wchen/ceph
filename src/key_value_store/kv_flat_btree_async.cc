...
bool index_data::is_timed_out(utime_t now, utime_t timeout) const const
{
...
int IndexCache::get(const string &key, index_data *idata) const const
{
    if (cache_size == 0) {
        return -ENODATA;
    }
    ...
    std::map < key_data, std::pair < index_data,
        utime_t > >::const_iterator it = k2itmap.lower_bound(key_data(key));
...
int IndexCache::get(const string &key, index_data *idata,
                    index_data *next_idata) const const
{
    if (cache_size == 0) {
        return -ENODATA;
    }
...
{
    if (verbose)
        cout << "\t\t" << client_name << "-next: finding next of "
             << idata.str()
             << std::endl;
    int err = 0;
    ...
    if (!kvs.empty()) {
        ...
        if (idata.is_timed_out(ceph_clock_now(), timeout)) {
            if (verbose)
                cout << client_name << " THINKS THE OTHER CLIENT DIED."
                     << std::endl;
                                 ...
                                 index_data *next_idata, bool force_update)
{
    ...
    oro.omap_get_vals2(key_data(key).encoded(),
                       (cache_size / cache_refresh >=
                        2 ? cache_size / cache_refresh : 2), &kvmap, nullptr,
                       &err);
    err = io_ctx.operate(index_name, &oro, NULL);
    utime_t mytime = ceph_clock_now();
    if (err < 0) {
        cerr << "\t" << client_name
             << "-read_index: getting keys failed with " << err << std::endl;
    ...
    if (verbose)
        cout << "\t" << client_name << "-read_index: kvmap_size is "
             << kvmap.size()
...
int KvFlatBtreeAsync::split(const index_data &idata)
{
    ...
    if (err < 0) {
        if (verbose)
            cout << "\t\t" << client_name << "-split: read object "
                 << args.odata.name << " got " << err << std::endl;
        return err;
    }

    if (verbose)
        cout << "\t\t" << client_name << "-split: splitting "
             << idata.obj
    ...
    client_index_lock.unlock();
    to_create[1].omap.insert(++args.odata.omap.
                             find(to_create[0].omap.rbegin()->first),
                             args.odata.omap.end());
    ...
    ops.push_back(std::
                  make_pair(std::pair < int, string > (ADD_PREFIX, index_name),
                            &owos[0]));
    for (int i = 1; i < 6; i++) {
                                ...
                                const index_data &next_idata)
{
    ...
    if (idata1.kdata.prefix == "1") {
        ...
        if (err < 0) {
            if (verbose)
                cout << "reading " << idata1.obj << " failed with " << err
                     << std::endl;
            if (err == -ENOENT) {
                return -ECANCELED;
            }
        ...
        if (verbose)
            cout << "\t\t" << client_name << "-rebalance: read "
                 << idata2.obj
                 << ". size: " << args1.odata.size << " version: "
                 << args1.odata.version << std::endl;
    } else {
    ...
    if ((int)args1.odata.size + (int)args2.odata.size <= 2 * k) {
        ...
        to_create.push_back(object_data(args1.odata.min_kdata,
                                        args2.odata.max_kdata, o2w,
                                        write2_map));
        ops.push_back(std::
                      make_pair(std::pair < int,
                                std::string > (MAKE_OBJECT, o2w), &create[0]));
        ...
        if (args1.odata.max_kdata != idata1.kdata) {
            ...
            target_size_1 =
                floor(((int)args1.odata.size + (int)args2.odata.size)
                      / 2.0);
        }
        ...
        if (verbose)
            cout << "\t\t" << client_name
                 ...
                 << write1_map.size() << ", write2_map.size() is " << write2_map.
                 size()
                 << std::endl;
        ...
        ops.push_back(std::
                      make_pair(std::pair < int,
                                std::string > (MAKE_OBJECT, o1w), &create[0]));
        ops.push_back(std::
                      make_pair(std::pair < int,
                                std::string > (MAKE_OBJECT, o2w), &create[1]));
    }
    ...
    push_back(object_data
              (args2.odata.min_kdata, args2.odata.max_kdata,
               args2.odata.name, args2.odata.version));
    for (int i = 1; i < 6; i++) {
...
int KvFlatBtreeAsync::read_object(const string &obj, rebalance_args *args)
{
    ...
    if (err < 0) {
        if (verbose)
            cout << "\t\t" << client_name
                 << "-read_object: reading failed with " << err << std::endl;
        a->release();
        return err;
    }
    auto it = outbl.cbegin();
...
void KvFlatBtreeAsync::set_up_prefix_index(const vector < object_data >
        &to_create,
        const vector < object_data >
        &to_delete,
        librados::ObjectWriteOperation *owo,
        index_data *idata, int *err)
{
...
void KvFlatBtreeAsync::set_up_ops(const vector < object_data > &create_vector,
                                  const vector < object_data > &delete_vector,
                                  vector < std::pair < std::pair < int,
                                  string >,
                                  librados::ObjectWriteOperation * > > *ops,
                                  const index_data &idata, int *err)
{
    ...
    if (create_vector.size() > 0) {
        for (int i = 0; i < (int)idata.to_delete.size(); ++i) {
            it->first =
                std::pair < int, string > (UNWRITE_OBJECT,
                                           idata.to_delete[i].obj);
    ...
    for (int i = 0; i < (int)idata.to_create.size(); ++i) {
        ...
        if (idata.to_create.size() <= 2) {
            it->first =
                std::pair < int, string > (MAKE_OBJECT, idata.to_create[i].obj);
        } else {
            it->first =
                std::pair < int, string > (AIO_MAKE_OBJECT,
                                           idata.to_create[i].obj);
        }
    ...
    for (int i = 0; i < (int)idata.to_delete.size(); ++i) {
        ...
        it->first =
            std::pair < int, string > (REMOVE_OBJECT, idata.to_delete[i].obj);
...
void KvFlatBtreeAsync::set_up_make_object(const std::map < std::string,
        bufferlist > &to_set,
        librados::ObjectWriteOperation *owo)
{
    bufferlist inbl;
...
void KvFlatBtreeAsync::set_up_unwrite_object(const int &ver,
...
void KvFlatBtreeAsync::set_up_restore_object(librados::ObjectWriteOperation *
        owo)
{
...
void KvFlatBtreeAsync::set_up_delete_object(librados::ObjectWriteOperation *
        owo)
{
    owo->cmpxattr("unwritable", CEPH_OSD_CMPXATTR_OP_EQ, to_bl("1"));
    owo->remove();
        ...
        switch (it->first.first) {
            ...
            case UNWRITE_OBJECT:   //marking
                ...
                if (err < 0) {
                    ...
                    if (it->first.second ==
                        (*idata.to_delete.begin()).max.encoded()) {
                ...
                if (verbose)
                    cout << debug_prefix << " marked " << it->first.second
                         << std::endl;
                break;
                ...
                if ((int)idata.to_create.size() == count) {
                    ...
                    for (count -= 1; count >= 0; count--) {
                        ...
                        if (err < 0) {
                            ...
                            if (err == -EEXIST) {
                                //someone thinks we died, so die
                                cerr << client_name << " is suiciding!" << std::
                                     endl;
                                return -ESUICIDE;
                            } else {
                                ceph_abort();
                        ...
                        if (verbose || idata.to_create.size() > 2) {
                            cout << debug_prefix << " completed aio " << aiocs.
                                 size() - count << "/" << aiocs.size() << std::endl;
                        }
                    }
                }
                break;
            case REMOVE_OBJECT:    //deleting
                ...
                if (verbose)
                    cout << debug_prefix << " deleted " << it->first.second
                         << std::endl;
                break;
                ...
                if (err < 0) {
                    if (verbose)
                        cout << debug_prefix
                             ...
                             << ". someone else must have thought we died, so dying"
                             << std::endl;
                    return -ETIMEDOUT;
                }
                if (verbose) {
            ...
            case RESTORE_OBJECT:
                if (verbose)
                    cout << debug_prefix << " restoring " << it->first.second
                         << std::endl;
                err = io_ctx.operate(it->first.second, it->second);
                if (err < 0) {
                    if (verbose)
                ...
                if (verbose)
                    cout << debug_prefix << " restored " << it->first.second
                         << std::endl;
                break;
            default:
                if (verbose)
                    cout << debug_prefix << " performing unknown op on "
                         << it->first.second << std::endl;
                err = io_ctx.operate(index_name, it->second);
                if (err < 0) {
                    if (verbose)
                        cout << debug_prefix << " unknown op on "
                             << it->first.second
                             << " failed with " << err << std::endl;
                    return err;
                }
                if (verbose)
                    cout << debug_prefix << " unknown op on "
                         << it->first.second << " succeeded." << std::endl;
                break;
        }
    }
    ...
    switch (error) {
        case -EFIRSTOBJ: {
            ...
            for (vector < delete_data >::const_iterator it =
                     idata.to_delete.begin(); it != idata.to_delete.end(); ++it) {
                index_data this_entry;
            ...
            if (verbose)
                cout << "\t\t\t" << client_name << "-cleanup: updating index"
                     << std::endl;
            if ((((KeyValueStructure *) this)->*
                 KvFlatBtreeAsync::interrupt)() == 1) {
                return -ESUICIDE;
            }
        ...
        case -ERANGE: {
            ...
            if (err < 0) {
                ...
                if (verbose)
                    cout << "\t\t\t" << client_name << "-cleanup: restoring "
                         << it->obj << " failed with " << err << std::endl;
            } else {
                if (verbose)
                    cout << "\t\t\t" << client_name << "-cleanup: restored "
                         << it->obj << std::endl;
            }
        ...
        case -ENOENT: {
            if (verbose)
                cout << "\t\t" << client_name << "-cleanup: rolling forward"
                     << std::endl;
            //all changes were created except for updating the index and possibly
            //deleting the objects. roll forward.
            vector < std::pair < std::pair < int, string >,
                   librados::ObjectWriteOperation * >>ops;
            vector < librados::ObjectWriteOperation >
            owos(idata.to_delete.size() + 1);
            for (int i = 0; i <= (int)idata.to_delete.size(); ++i) {
                ops.push_back(std::
                              make_pair(std::pair < int, std::string > (0, ""),
                                        &owos[i]));
            }
            ...
            if (verbose)
                cout << "\t\t\t" << client_name << "-cleanup: updated index"
                     << std::endl;
            break;
        }
        default: {
            ...
            if (verbose)
                cout << "\t\t" << client_name << "-cleanup: rolling back"
                     << std::endl;
            std::map < std::string, bufferlist > new_index;
            ...
            for (vector < create_data >::const_reverse_iterator it =
                     idata.to_create.rbegin(); it != idata.to_create.rend(); ++it) {
                ...
                if (verbose)
                    cout << "\t\t\t" << client_name << "-cleanup: marking "
                         << it->obj << std::endl;
                ...
                if (err < 0) {
                    ...
                    if (verbose)
                        cout << "\t\t\t" << client_name << "-cleanup: marked "
                             << it->obj << std::endl;
                }
            }
            ...
            for (vector < delete_data >::const_iterator it =
                ...
                if (err == -ENOENT) {
                    ...
                    if (verbose)
                        cout << "\t\t\t" << client_name
                             << "-cleanup: roll forward instead" << std::endl;
                        ...
                        if (err < 0) {
                            if (verbose)
                                cout << "\t\t\t" << client_name
                                     << "-cleanup: restoring "
                                     << cit->
                                     obj << " failed with " << err << std::endl;
                        }
                        if (verbose)
                            cout << "\t\t\t" << client_name <<
                                 "-cleanup: restored " << cit->obj << std::endl;
                    }
            ...
            for (vector < create_data >::const_reverse_iterator it =
                ...
                if (verbose)
                    cout << "\t\t\t" << client_name << "-cleanup: removing "
                         << it->obj << std::endl;
                err = io_ctx.operate(it->obj, &rm);
                if (err < 0) {
                    if (verbose)
                    ...
                    if (verbose)
                        cout << "\t\t\t" << client_name << "-cleanup: removed "
                             << it->obj << std::endl;
                }
            }
...
int KvFlatBtreeAsync::setup(int argc, const char **argv)
{
    ...
    if (r < 0) {
        if (verbose)
            cout << client_name << ": Making the index failed with code "
                 << r << std::endl;
        return 0;
    }
    if (verbose) {
    ...
    if (r < 0) {
        if (verbose)
            cout << client_name << ": Setting xattr failed with code "
                 << r << std::endl;
    }
                          ...
                          bool update_on_existing)
{
    ...
    if (err < 0) {
        if (verbose)
            cout << "\t" << client_name
                 << ": getting oid failed with code " << err << std::endl;
        return err;
    }
    if (verbose)
    ...
    if (verbose)
        cout << "\t" << client_name << ": finished set with " << err
             << std::endl;
    return err;
}
                             ...
                             bool update_on_existing, index_data &idata)
{
    ...
    if (verbose)
        cout << "\t" << client_name << ": inserting " << key
             << " into object " << idata.obj << std::endl;
    ...
    if (err < 0) {
        switch (err) {
            ...
            case -EKEYREJECTED: {
                ...
                do {
                    if (verbose)
                        cout << "\t" << client_name << ": running split on "
                             << idata.obj << std::endl;
                    ...
                    if (err < 0 && err != -ENOENT && err != -EBALANCE) {
                        if (verbose)
                            cerr << "\t" << client_name <<
                                 ": split failed with " << err << std::endl;
                        int ret =
                            handle_set_rm_errors(err, idata.obj, key, &idata,
                                                 NULL);
                        switch (ret) {
                            case -ESUICIDE:
                                if (verbose)
                                    cout << client_name << " IS SUICIDING!" << std::
                                         endl;
                                return ret;
                            case 1:
            ...
            default:
                if (verbose)
                    cerr << "\t" << client_name << ": writing obj failed with "
                         << err << std::endl;
                if (err == -ENOENT || err == -EACCES) {
                    ...
                    if (verbose)
                        cout << "\t" << client_name << ": index data is "
                             << idata.str()
                             << ", object is " << idata.obj << std::endl;
                    return set_op(key, val, update_on_existing, idata);
                } else {
                                ...
                                index_data &next_idata)
{
    ...
    if (err < 0) {
        if (verbose)
            cout << "\t" << client_name << ": writing obj failed with "
                 << err << std::endl;
        switch (err) {
            case -ENODATA: {
            ...
            case -EKEYREJECTED: {
                ...
                do {
                    if (verbose)
                        cerr << "\t" << client_name << ": running rebalance on "
                             << idata.obj << std::endl;
                    ...
                    if (err < 0 && err != -ENOENT && err != -EBALANCE) {
                        if (verbose)
                            cerr << "\t" << client_name <<
                                 ": rebalance returned " << err << std::endl;
                        int ret =
                            handle_set_rm_errors(err, idata.obj, key, &idata,
                                                 &next_idata);
                        switch (ret) {
                            ...
                            case -EUCLEAN:
                                ...
                                if (verbose)
                                    cout << "\t" << client_name << ": removing " <<
                                         key << " from " << idata.obj << std::endl;
                                int err = io_ctx.operate(idata.obj, &owo);
                                if (err == 0) {
                                    return 0;
                                }
...
void KvFlatBtreeAsync::aio_set(const string &key, const bufferlist &val,
                               bool exclusive, callback cb, void *cb_args,
                               int *err)
{
...
int KvFlatBtreeAsync::set_many(const std::map < string, bufferlist > &in_map)
{
    ...
    if (verbose)
        cout << "finished reading index for objects. there are "
    ...
    for (map < string, bufferlist >::iterator it = imap.begin();
         it != imap.end(); ++it) {
        index_data idata;
        blit = it->second.begin();
        idata.decode(blit);
        to_delete.
        push_back(object_data(idata.min_kdata, idata.kdata, idata.obj));
        err = read_object(idata.obj, &to_delete[to_delete.size() - 1]);
        if (err < 0) {
            if (verbose)
                cout << "reading " << idata.obj << " failed with " << err
                     << std::endl;
    ...
    for (map < string, bufferlist >::iterator it = big_map.begin();
        ...
        if (to_create[to_create.size() - 1].omap.size() == 1.5 * k) {
            ...
            to_create.
            push_back(object_data(to_string(client_name, client_index++)));
    ...
    cout << "finished setting up ops. Starting critical section..." << std::
         endl;
...
int KvFlatBtreeAsync::remove_all()
{
    ...
    if (!index_set.empty()) {
        for (std::map < std::string, bufferlist >::iterator it =
                 index_set.begin(); it != index_set.end(); ++it) {
...
int KvFlatBtreeAsync::get_all_keys_and_values(std::map < std::string,
        bufferlist > *kv_map)
{
    if (verbose) {
...
bool KvFlatBtreeAsync::is_consistent()
{
    ...
    for (map < std::string, bufferlist >::iterator it = index.begin();
        ...
        if (it->first != "") {
            ...
            idata.decode(b);
            if (idata.prefix != "") {
                for (vector < delete_data >::iterator dit =
                         idata.to_delete.begin(); dit != idata.to_delete.end();
                     ++dit) {
                    librados::ObjectReadOperation oro;
                    ...
                    if (ceph_clock_now() - idata.ts > timeout) {
                        if (err < 0) {
                            ...
                            if (err == -ENOENT) {
                                ...
                                cerr << "Not consistent! reading object " <<
                                     dit->obj << "returned " << err << std::endl;
                                ret = false;
                                break;
                            }
                        }
                        if (atoi(string(un.c_str(), un.length()).c_str()) != 1
                            ...
                            cerr << "Not consistent! object " << dit->
                                 obj << " has been " <<
                ...
                for (vector < create_data >::iterator cit =
                         idata.to_create.begin(); cit != idata.to_create.end();
                     ++cit) {
    ...
    for (librados::NObjectIterator oit = io_ctx.nobjects_begin();
        ...
        if (name != index_name && onames.count(name) == 0
            ...
            cerr << "Not consistent! found floating object " << name << std::
                 endl;
            ret = false;
        }
    }
    ...
    for (std::map < std::string, string >::iterator it = parsed_index.begin();
        ...
        for (std::set < std::string >::iterator subit =
                 sub_objs[it->second].begin(); subit != sub_objs[it->second].end();
             ++subit) {
            if ((it->first != "1"
                 && *subit > it->first.substr(1, it->first.length()))
                || *subit <= prev) {
                cerr << "Not consistent! key " << *subit <<
                     " does not belong in " << *it << std::endl;
                cerr << "not last element, i.e. " << it->
                     first << " not equal to 1? " << (it->first !=
                                                      "1") << std::
                     endl << "greater than " << it->first.substr(1,
                             it->first.
                             length())
                     << "? " << (*subit >
                                 it->first.substr(1, it->first.length()))
                     << std::
                     endl << "less than or equal to " << prev << "? " << (*subit
                             <=
                             prev)
                     << std::endl;
                ret = false;
        ...
        for (int i = 0; i < indexer; i++) {
            if (dones[i]) {
                ...
                if (its[i] == all_maps[i].end()) {
                    ...
                    ret << "|" << string((25 -
                                          ((*its[i]).first.length() +
                                           its[i]->second.length() + 3)) / 2,
                                         ' ');
                    ret << (*its[i]).first;
                    ret << " | ";
                    ret << string(its[i]->second.c_str(),
                                  its[i]->second.length());
                    ret <<
                        string((25 -
                                ((*its[i]).first.length() +
