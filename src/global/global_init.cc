    ...
    if (ret == -EDOM) {
        ...
        if (!(flags & CINIT_FLAG_NO_DEFAULT_CONFIG_FILE)) {
            if (conf_file_list.length()) {
                ...
                derr << "did not load config file, using default settings." <<
                    dendl;
            }
        }
    }
...
global_init(std::vector < const char *>*alt_def_args,
            std::vector < const char *>&args,
            uint32_t module_type, code_environment_t code_env,
            int flags, const char *data_dir_option, bool run_pre_init)
{
    ...
    if (g_conf->run_dir.length() &&
        ...
        if (r < 0 && errno != EEXIST) {
            cerr << "warning: unable to create " << g_conf->
                run_dir << ": " << cpp_strerror(errno) << std::endl;
        }
    }
