...
class RGWHTTPClient {
  ...
  protected:
    ...
    int init_request(const char *method,
                     const char *url,
                     rgw_http_req_data * req_data, bool send_data_hint = false);
  ...
  public:
    ...
    :send_len(0),
        ...
        req_data(nullptr),
        user_info(nullptr), verify_ssl(cct->_conf->rgw_verify_ssl), cct(cct) {
    }
...
class RGWHTTPHeadersCollector:public RGWHTTPClient {
  public:
    ...
    :RGWHTTPClient(cct), relevant_headers(relevant_headers) {
