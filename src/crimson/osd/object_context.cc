...
namespace crimson::osd {
    ...
    ObjectContextRegistry::ObjectContextRegistry(crimson::common::
                                                 ConfigProxy & conf) {
        obc_lru.set_target_size(conf.get_val < uint64_t >
    ...
    void ObjectContextRegistry::handle_conf_change(const crimson::common::
                                                   ConfigProxy & conf,
                                                   const std::set <
                                                   std::string > &changed) {
        obc_lru.set_target_size(conf.get_val < uint64_t >
    ...
    std::optional < hobject_t > resolve_oid(const SnapSet & ss,
        ...
        if (oid.snap > ss.seq) {
            ...
            auto clone = std::lower_bound(begin(ss.clones), end(ss.clones),
                                          oid.snap);
            ...
            if (std::find(citer->second.begin(),
                          citer->second.end(),
