...
void node_header_t::bootstrap_extent(NodeExtentMutable &mut,
...
} void node_header_t::update_is_level_tail(NodeExtentMutable &mut,
...
void F013_T::update_size_at(NodeExtentMutable &mut, const me_t &node,
                            index_t index, int change)
{
    ...
    for (const auto *p_slot = &node.slots[index];
        ...
        mut.copy_in_absolute((void *) & (p_slot->right_offset),
...
template < typename SlotType >
void F013_T::append_key(NodeExtentMutable &mut, const key_t &key,
                        char *&p_append)
{
...
template < typename SlotType >
void F013_T::append_offset(NodeExtentMutable &mut,
...
void F013_T::insert_at(NodeExtentMutable &mut, const Key &key,
                       ...
                       node_offset_t size_right)
{
    ...
    mut.shift_absolute(p_insert, p_shift_end - p_insert,
                       estimate_insert_one());
    mut.copy_in_absolute((void *)&node.num_keys,
                         num_keys_t(node.num_keys + 1));
...
template < typename SlotType >
node_offset_t F013_T::erase_at(NodeExtentMutable &mut,
                               const me_t &node, index_t index,
...
void node_fields_2_t::append_offset(NodeExtentMutable &mut,
                                    node_offset_t offset_to_right,
                                    char *&p_append)
{
    mut.copy_in_absolute(p_append, offset_to_right);
