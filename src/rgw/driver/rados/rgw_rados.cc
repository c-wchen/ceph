        ...
        void *_index_op, optional_yield y)
{
    ...
    r = target->prepare_atomic_modification(dpp, op, reset_obj, ptag, meta.if_match, meta.if_nomatch, false, modify_tail,
                                            y);
    if (r < 0) {
        return r;
    }
    ...
    if (versioned_op && meta.olh_epoch) {
        r = store->set_olh(dpp, target->get_ctx(), target->get_bucket_info(), obj, false, NULL, *meta.olh_epoch, real_time(),
    ...
    for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
        ...
        if (copy_if_newer && canceled) {
            ...
            if (!dest_state->exists ||
                ...
                ldpp_dout(dpp, 20) << "retrying writing object mtime=" << set_mtime << " dest_state->mtime=" << dest_state->mtime <<
                                   " dest_state->exists=" << dest_state->exists << dendl;
                continue;
            } else {
                ldpp_dout(dpp, 20) << "not retrying writing object mtime=" << set_mtime << " dest_state->mtime=" << dest_state->mtime <<
                                   " dest_state->exists=" << dest_state->exists << dendl;
            }
        }
        break;
...
int RGWRados::Object::Delete::delete_obj(optional_yield y, const DoutPrefixProvider *dpp)
{
    ...
    if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
        if (instance.empty() || explicit_marker_version) {
            ...
            int r = store->set_olh(dpp, target->get_ctx(), target->get_bucket_info(), marker, true, &meta, params.olh_epoch,
                                   params.unmod_since, params.high_precision_time, y, params.zones_trace);
            if (r < 0) {
                return r;
            }
        } else {
            ...
            r = store->unlink_obj_instance(dpp, target->get_ctx(), target->get_bucket_info(), obj, params.olh_epoch, y,
                                           params.zones_trace);
            if (r < 0) {
                return r;
            }
                                 ...
                                 bool follow_olh, optional_yield y, bool assume_noent)
{
    ...
    *manifest = nullptr;
    ...
    ldpp_dout(dpp, 20) << "get_obj_state: rctx=" << (void *)rctx << " obj=" << obj << " state=" <<
...
int RGWRados::get_obj_state(const DoutPrefixProvider *dpp, RGWObjectCtx *rctx, RGWBucketInfo &bucket_info,
                            const rgw_obj &obj, RGWObjState **state, RGWObjManifest **manifest,
                            bool follow_olh, optional_yield y, bool assume_noent)
{
    int ret;
...
int RGWRados::update_olh(const DoutPrefixProvider *dpp, RGWObjectCtx &obj_ctx, RGWObjState *state,
                         RGWBucketInfo &bucket_info, const rgw_obj &obj, optional_yield y, rgw_zone_set *zones_trace)
{
...
int RGWRados::get_bucket_stats_async(const DoutPrefixProvider *dpp, RGWBucketInfo &bucket_info,
                                     const rgw::bucket_index_layout_generation &idx_layout, int shard_id, RGWGetBucketStats_CB *ctx)
{
    int num_aio = 0;
...
int RGWRados::cls_bucket_head_async(const DoutPrefixProvider *dpp, const RGWBucketInfo &bucket_info,
                                    const rgw::bucket_index_layout_generation &idx_layout, int shard_id, RGWGetDirHeader_CB *ctx, int *num_aio)
{
    RGWSI_RADOS::Pool index_pool;
    map<int, string> bucket_objs;
