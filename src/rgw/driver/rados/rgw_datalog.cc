...
void rgw_data_change::dump(ceph::Formatter * f) const const
{
    std::string type;
    switch (entity_type) {
...
void rgw_data_change_log_entry::dump(Formatter * f) const const
{
...
void rgw_data_notify_entry::dump(Formatter * f) const const
{
    encode_json("key", key, f);
    encode_json("gen", gen, f);
}
...
class RGWDataChangesOmap final:public RGWDataChangesBE {
  ...
  public:
    RGWDataChangesOmap(lr::IoCtx & ioctx,
                       ...
                       uint64_t gen_id, int num_shards)
    ...
    void prepare(ceph::real_time ut, const std::string & key,
        ...
        if (!std::holds_alternative < centries > (out)) {
            ceph_assert(std::visit([](const auto & v) {
                                   return std::empty(v);
                                   }
                                   , out));
            out = centries();
        ...
        cls_log_entry e;
        cls_log_add_prepare_entry(e, utime_t(ut), {
                                  }
                                  , key, entry);
        std::get < centries > (out).push_back(std::move(e));
    }
    int push(const DoutPrefixProvider * dpp, int index, entries
             && items, optional_yield y) override {
    ...
    std::string_view max_marker()const override {
        ...
        for (auto shard = 0u; shard < oids.size(); ++shard) {
            ...
            cls_log_list(op, {
                         ...
                         , 1, log_entries, &out_marker, &truncated);
            auto r =
                rgw_rados_operate(dpp, ioctx, oids[shard], &op, nullptr, y);
            if (r == -ENOENT) {
                continue;
            }
            if (r < 0) {
...
class RGWDataChangesFIFO final:public RGWDataChangesBE {
  ...
  public:
    RGWDataChangesFIFO(lr::IoCtx & ioctx,
                       RGWDataChangesLog & datalog, uint64_t gen_id, int shards)
    :RGWDataChangesBE(ioctx, datalog, gen_id),
    ...
    int list(const DoutPrefixProvider * dpp, int index, int max_entries,
        ...
        bool more = false;
        auto r =
            fifos[index].list(dpp, max_entries, marker, &log_entries, &more,
                              y);
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__
    ...
    int get_info(const DoutPrefixProvider * dpp, int index,
        ...
        info->marker = rgw::cls::fifo::marker {
        p, h.last_ofs}
        .to_string();
...
bs::error_code DataLogBackends::handle_init(entries_t e) noexcept
{
  ...
  for (const auto &[gen_id, gen]:e) {
        ...
        try {
            ...
            case log_type::omap:
                emplace(gen_id,
                        new RGWDataChangesOmap(ioctx, datalog, gen_id, shards));
                break;
            case log_type::fifo:
                emplace(gen_id,
                        new RGWDataChangesFIFO(ioctx, datalog, gen_id, shards));
                break;
            default:
                lderr(datalog.cct)
                    << __PRETTY_FUNCTION__ << ":" << __LINE__
                             ...
                             librados::Rados * lr)
{
    ...
    ceph_assert(zone);
    auto defbacking =
        to_log_type(cct->_conf.get_val < std::string >
    ...
    auto besr =
        logback_generations::init < DataLogBackends > (dpp, ioctx,
                                                       metadata_log_oid(),
...
auto RGWDataChangesLog::_get_change(const rgw_bucket_shard & bs, uint64_t gen)
->ChangeStatusPtr
{
...
void RGWDataChangesLog::register_renew(const rgw_bucket_shard & bs,
                                       const rgw::
                                       bucket_log_layout_generation & gen)
{
    std::scoped_lock l {
    lock};
    cur_cycle.insert( {
...
void RGWDataChangesLog::update_renewed(const rgw_bucket_shard & bs,
                                       uint64_t gen, real_time expiration)
{
    std::unique_lock l {
    lock};
...
std::string RGWDataChangesLog::get_oid(uint64_t gen_id, int i) constconst
{
    return (gen_id > 0 ?
                                 ...
                                 int shard_id, optional_yield y)
{
    ...
    ldpp_dout(dpp,
        ...
        name << " shard_id=" << shard_id << " now=" << now << " cur_expiration="
...
int DataLogBackends::list(const DoutPrefixProvider * dpp, int shard,
                          int max_entries,
                          std::vector < rgw_data_change_log_entry > &entries,
...
int RGWDataChangesLog::list_entries(const DoutPrefixProvider * dpp, int shard,
                                    int max_entries,
                                    std::vector < rgw_data_change_log_entry >
                                    &entries, std::string_view marker,
                                    std::string * out_marker, bool * truncated,
                                    optional_yield y)
{
...
int RGWDataChangesLog::list_entries(const DoutPrefixProvider * dpp,
                                    int max_entries,
                                    std::vector < rgw_data_change_log_entry >
...
int RGWDataChangesLog::trim_entries(const DoutPrefixProvider * dpp,
...
class GenTrim:public rgw::cls::fifo::Completion < GenTrim > {
  public:
    ...
    GenTrim(const DoutPrefixProvider * dpp, DataLogBackends * bes, int shard_id,
            uint64_t target_gen, std::string cursor, uint64_t head_gen,
            uint64_t tail_gen, boost::intrusive_ptr < RGWDataChangesBE > be,
            lr::AioCompletion * super)
    :Completion(dpp, super), bes(bes), shard_id(shard_id),
        target_gen(target_gen), cursor(std::move(cursor)), head_gen(head_gen),
        tail_gen(tail_gen), be(std::move(be)) {
...
void DataLogBackends::trim_entries(const DoutPrefixProvider * dpp, int shard_id,
...
bool RGWDataChangesLog::going_down() const const
{
    return down_flag;
}
                                      ...
                                      uint64_t gen)
{
    ...
    {
        std::shared_lock rl {
        modified_lock};         // read lock to check for existence
        auto shard = modified_shards.find(shard_id);
        if (shard != modified_shards.end() && shard->second.count( {
                                                                  key, gen}
            )) {
            return;
        }
    ...
    std::unique_lock wl {
    modified_lock};             // write lock for insertion
    modified_shards[shard_id].insert(rgw_data_notify_entry {
                                     key, gen}
    );
}

std::string RGWDataChangesLog::max_marker() constconst
{
