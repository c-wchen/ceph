...
namespace crimson::os::seastore {
    ...
    LBAManager::update_mappings_ret
        LBAManager::update_mappings(Transaction & t,
                                    const std::list < LogicalCachedExtentRef >
                                    &extents) {
        return trans_intr::do_for_each(extents,[this, &t] (auto & extent) {
                                       return update_mapping(t, extent->get_laddr(), extent->get_prior_paddr_and_reset(), extent->get_paddr(), nullptr  // all the extents should have already been
