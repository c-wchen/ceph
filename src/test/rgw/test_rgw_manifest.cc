...
static void gen_obj(test_rgw_env &env, uint64_t obj_size, uint64_t head_max_size, uint64_t stripe_size,
                    RGWObjManifest *manifest, const rgw_placement_rule &placement_rule, rgw_bucket *bucket, rgw_obj *head,
...
TEST(TestRGWManifest, head_only_obj)
{
    ...
    gen_obj(env, obj_size, 512 * 1024, 4 * 1024 * 1024, &manifest, env.zonegroup.default_placement, &bucket, &head, &gen,
            &objs);
