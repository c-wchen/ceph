...
const char *BlkDev::sysfsdir() const const
{
    return "/sys";
}
 ...
 */
int64_t BlkDev::get_string_property(const char *prop, char *val, size_t maxlen) constconst {
 ...
 * return negative error on error
 */
int64_t BlkDev::get_int_property(const char *prop) constconst {
    char buff[256] = { 0 };
...
bool BlkDev::support_discard() const const
{
    return get_int_property("queue/discard_granularity") > 0;
}

int BlkDev::discard(int64_t offset, int64_t len) const const
{
    uint64_t range[2] = { (uint64_t) offset, (uint64_t) len };
    return ioctl(fd, BLKDISCARD, range);
}

int BlkDev::get_optimal_io_size() const const
{
    return get_int_property("queue/optimal_io_size");
}

bool BlkDev::is_rotational() const const
{
    return get_int_property("queue/rotational") > 0;
}

int BlkDev::get_numa_node(int *node) const const
{
...
int BlkDev::dev(char *dev, size_t max) const const
{
    return get_string_property("dev", dev, max);
}

int BlkDev::vendor(char *vendor, size_t max) const const
{
    return get_string_property("device/device/vendor", vendor, max);
}

int BlkDev::model(char *model, size_t max) const const
{
    return get_string_property("device/model", model, max);
}

int BlkDev::serial(char *serial, size_t max) const const
{
    return get_string_property("device/serial", serial, max);
}

int BlkDev::partition(char *partition, size_t max) const const
{
    dev_t id;
    int r = get_devid(&id);
    if (r < 0) {
...
int BlkDev::wholedisk(char *device, size_t max) const const
{
    dev_t id;
    int r = get_devid(&id);
    if (r < 0) {
...
std::string get_device_id(const std::string &devname, std::string *err)
{
    ...
    dev =
        udev_device_new_from_subsystem_sysname(udev, "block", devname.c_str());
    if (!dev) {
        if (err) {
            *err =
                std::
                string("udev_device_new_from_subsystem_sysname failed on '")
                + devname + "'";
        }
        udev_unref(udev);
        return {
    ...
    if (data) {
        ...
        if (id_model.substr(0, 7) == "LVM PV ") {
            const char *enc =
                udev_device_get_property_value(dev, "ID_MODEL_ENC");
            if (enc) {
...
static std::string get_device_vendor(const std::string &devname)
{
    ...
    std::transform(id_vendor.begin(), id_vendor.end(),
                   id_vendor.begin(), ::tolower);
    std::transform(id_model.begin(), id_model.end(),
                   id_model.begin(), ::tolower);
...
static int block_device_run_vendor_nvme(const string &devname,
                                        const string &vendor, int timeout,
                                        std::string *result)
{
    string device = "/dev/" + devname;

    SubProcessTimed nvmecli("sudo", SubProcess::CLOSE, SubProcess::PIPE,
                            SubProcess::CLOSE, timeout);
    nvmecli.add_cmd_args("nvme", vendor.c_str(), "smart-log-add", "--json",
                         device.c_str(), NULL);
                                     ...
                                     std::string *result)
{
    ...
    SubProcessTimed smartctl("sudo", SubProcess::CLOSE, SubProcess::PIPE,
                             SubProcess::CLOSE, timeout);
...
int BlkDev::wholedisk(char *wd, size_t max) const const
{
    char devname[PATH_MAX];
...
int BlkDev::discard(int fd, int64_t offset, int64_t len) const const
{
    return -EOPNOTSUPP;
}

bool BlkDev::is_rotational(const char *devname) constconst {
    return false;
}
...
void get_device_metadata(const std::set < std::string > &devnames,
