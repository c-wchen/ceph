...
TEST(Buffer, constructors)
{
    ...
    {
        // no fd
        EXPECT_THROW(buffer::create_zero_copy(len, -1, NULL),
                     buffer::error_code);
        history_alloc_bytes += len;
        history_alloc_num++;
...
TEST(BufferPtr, copy_out)
{
    ...
    }
    {
        ...
        EXPECT_THROW(ptr.copy_out((unsigned)0, strlen(in) + 1, NULL),
                     buffer::end_of_buffer);
        EXPECT_THROW(ptr.copy_out(strlen(in) + 1, (unsigned)0, NULL),
                     buffer::end_of_buffer);
        char out[1] = { 'X' };
...
TEST(BufferList, copy)
{
    ...
    {
        bufferlist bl;
        EXPECT_THROW(bl.copy((unsigned)100, (unsigned)100, (char *)0),
                     buffer::end_of_buffer);
        const char *expected = "ABC";
        bl.append(expected);
    ...
    //
    {
        bufferlist bl;
        bufferlist dest;
        EXPECT_THROW(bl.copy((unsigned)100, (unsigned)100, dest),
                     buffer::end_of_buffer);
        const char *expected = "ABC";
        bl.append(expected);
        bl.copy(1, 2, dest);
...
TEST(BufferList, copy_in)
{
    ...
    //
    {
        bufferlist bl;
        bl.append("XXX");
        EXPECT_THROW(bl.copy_in((unsigned)100, (unsigned)100, (char *)0),
                     buffer::end_of_buffer);
        bl.copy_in(1, 2, "AB");
    ...
    //
    {
        ...
        bufferlist src;
        src.append("ABC");
        EXPECT_THROW(bl.copy_in((unsigned)100, (unsigned)100, src),
                     buffer::end_of_buffer);
        bl.copy_in(1, 2, src);
        EXPECT_EQ(0,::memcmp("XAB", bl.c_str(), 3));
    }
...
TEST(BufferList, hexdump)
{
    ...
    bl.hexdump(stream);
    EXPECT_EQ
        ("00000000  30 31 33 32 34 35 36 37  38 39 30 31 32 33 34 00  |013245678901234.|\n"
...
TEST(BufferList, read_file)
{
    ...
    snprintf(cmd, sizeof(cmd), "echo ABC > %s ; chmod 0 %s", FILENAME,
             FILENAME);
    EXPECT_EQ(0,::system(cmd));
    if (getuid() != 0) {
...
TEST(BufferList, crc32c_zeros)
{
    ...
    for (size_t j = 0; j < 1000; j++) {
        ...
        blb.push_back(a);
        uint32_t crcb =
            ceph_crc32c(111, (unsigned char *)blb.c_str(), blb.length());
...
TEST(BufferList, crc32c_append_perf)
{
    ...
    blb.push_back(b);
    {
        ...
        std::
            cout << "a.crc32c(0) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
    ...
    assert(buffer::get_cached_crc() == 0 + base_cached);
    {
        ...
        float rate = (float)len / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "a.crc32c(0) (again) = " << r << " at " << rate << " MB/sec"
            << std::endl;
        ASSERT_EQ(r, 1138817026u);
    }
...

    {
        ...
        float rate = (float)len / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "a.crc32c(5) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
    ...
    assert(buffer::get_cached_crc_adjusted() == 1 + base_cached_adjusted);
    {
        ...
        float rate = (float)len / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "a.crc32c(5) (again) = " << r << " at " << rate << " MB/sec"
            << std::endl;
        ASSERT_EQ(r, 3239494520u);
    }
    assert(buffer::get_cached_crc() == 1 + base_cached);
    assert(buffer::get_cached_crc_adjusted() == 2 + base_cached_adjusted);
    {
        ...
        float rate = (float)len / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "b.crc32c(0) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
    ...
    assert(buffer::get_cached_crc() == 1 + base_cached);
    {
        ...
        float rate = (float)len / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "b.crc32c(0) (again)= " << r << " at " << rate << " MB/sec"
            << std::endl;
        ASSERT_EQ(r, 2481791210u);
    }
    ...
    ab.push_back(b);
    {
        ...
        utime_t end = ceph_clock_now();
        float rate =
            (float)ab.length() / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "ab.crc32c(0) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
    ...
    ac.push_back(c);
    {
        ...
        utime_t end = ceph_clock_now();
        float rate =
            (float)ac.length() / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "ac.crc32c(0) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
        ASSERT_EQ(r, 2988268779u);
    }
    ...
    ba.push_back(a);
    {
        ...
        utime_t end = ceph_clock_now();
        float rate =
            (float)ba.length() / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "ba.crc32c(0) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
    ...
    assert(buffer::get_cached_crc_adjusted() == 4 + base_cached_adjusted);
    {
        ...
        float rate =
            (float)ba.length() / (float)(1024 * 1024) / (float)(end - start);
        std::
            cout << "ba.crc32c(5) = " << r << " at " << rate << " MB/sec" <<
            std::endl;
    ...
    cout << "crc cache hits (same start) = " << buffer::
        get_cached_crc() << std::endl;
    cout << "crc cache hits (adjusted) = " << buffer::
        get_cached_crc_adjusted() << std::endl;
}
...
TEST(BufferList, TestCopyAll)
{
    ...
    ceph::shared_ptr < unsigned char >big((unsigned char *)malloc(BIG_SZ),
                                          free);
    unsigned char c = 0;
    ...
    ceph::shared_ptr < unsigned char >big2((unsigned char *)malloc(BIG_SZ),
                                           free);
...
TEST(BufferList, InvalidateCrc)
{
    ...
    ceph::shared_ptr < unsigned char >big((unsigned char *)malloc(buffer_size),
                                          free);
    unsigned char c = 0;
