...
void DetailedStatCollector::Op::dump(ostream * out, Formatter * f)
{
    if (!out)
        return;
...
DetailedStatCollector::DetailedStatCollector(double bin_size,
                                             Formatter * formatter,
                                             ostream * out,
                                             ostream * summary_out,
                                             AdditionalPrinting *
                                             details):bin_size(bin_size),
f(formatter), out(out), summary_out(summary_out), details(details),
...
void DetailedStatCollector::write_applied(uint64_t seq)
{
    ...
    Op op("write_applied",
          ...
          cur_time() - not_applied[seq].second, not_applied[seq].first, seq);
    op.dump(out, f.get());
...
void DetailedStatCollector::read_complete(uint64_t seq)
{
    ...
    Op op("read",
          not_read[seq].second,
          cur_time() - not_read[seq].second, not_read[seq].first, seq);
    op.dump(out, f.get());
