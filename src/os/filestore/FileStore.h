...
{
    return out << "sb(" << sb.compat_features << "): " << sb.omap_backend;
}
...
class FileStore:public JournalingObjectStore, public md_config_obs_t {
  ...
  public:
    ...
    bool _need_temp_object_collection(const coll_t & cid,
    ...
    class OpSequencer:public Sequencer_impl {
      ...
      public:
        ...
        /// get_min_uncompleted bool _get_min_uncompleted(
    ...
    std::atomic < int64_t > next_osr_id = {
    0};
  ...
  public:
    ...
    int lfn_open(const coll_t & cid,
                 const ghobject_t & oid,
                 bool create, FDRef * outfd, Index * index = 0);

    void lfn_close(FDRef fd);
    int lfn_link(const coll_t & c, const coll_t & newcid, const ghobject_t & o,
  ...
  public:
    FileStore(CephContext * cct, const std::string & base,
              const std::string & jdev, osflagbits_t flags =
              0, const char *internal_name = "filestore", bool update_to =
              false);
    ~FileStore()override;
    ...
    int _do_transactions(vector < Transaction > &tls, uint64_t op_seq,
   ...
   */
    ...
    void _close_replay_guard(int fd, const SequencerPosition & spos,
                             const ghobject_t * oid = 0);
    void _close_replay_guard(const coll_t & cid,
                             const SequencerPosition & spos);
   ...
   */
    ...
    int _check_replay_guard(const coll_t & cid, const ghobject_t & oid,
    ...
    int _write(const coll_t & cid, const ghobject_t & oid, uint64_t offset,
               size_t len, const bufferlist & bl, uint32_t fadvise_flags = 0);
    ...
    int _clone_range(const coll_t & oldcid, const ghobject_t & oldoid,
    ...
    int getattr(const coll_t & cid, const ghobject_t & oid, const char *name,
                bufferptr & bp) override;
    int getattrs(const coll_t & cid, const ghobject_t & oid, map < string,
                 bufferptr > &aset) override;

    int _setattrs(const coll_t & cid, const ghobject_t & oid, map < string,
                  bufferptr > &aset, const SequencerPosition & spos);
    ...
    int omap_get_header(const coll_t & c,
                        const ghobject_t & oid,
                        bufferlist * out, bool allow_eio = false) override;
    using ObjectStore::omap_get_keys;
    int omap_get_keys(const coll_t & c, const ghobject_t & oid,
                      set < string > *keys) override;
    using ObjectStore::omap_get_values;
    int omap_get_values(const coll_t & c, const ghobject_t & oid,
                        const set < string > &keys, map < string,
                        bufferlist > *out) override;
    using ObjectStore::omap_check_keys;
    int omap_check_keys(const coll_t & c, const ghobject_t & oid,
   ...
   */
    ...
    int _collection_add(const coll_t & c, const coll_t & ocid,
                        const ghobject_t & oid, const SequencerPosition & spos);
    ...
    void dump_stop();
    void dump_transactions(vector < Transaction > &ls, uint64_t seq,
                           OpSequencer * osr);
  ...
  private:
    ...
    int _split_collection_create(const coll_t & cid, uint32_t bits,
                                 uint32_t rem, coll_t dest,
                                 const SequencerPosition & spos);
    ...
    std::atomic < int64_t > m_filestore_kill_at = {
    0};
...
class FileStoreBackend {
  ...
  protected:
    ...
    int _copy_range(int from, int to, uint64_t srcoff, uint64_t len,
        ...
        if (has_fiemap() || has_seek_data_hole()) {
            return filestore->_do_sparse_copy_range(from, to, srcoff, len,
                                                    dstoff);
        }
        else {
  ...
  explicit FileStoreBackend(FileStore * fs):filestore(fs) {
    ...
    virtual int _crc_verify_read(int fd, loff_t off, size_t len,
                                 const bufferlist & bl, ostream * out) = 0;
};
