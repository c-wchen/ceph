...
Worker *NetworkStack::create_worker(CephContext * c, const string & type,
                                    unsigned i)
{
    if (type == "posix")
...
NetworkStack::NetworkStack(CephContext * c, const string & t):type(t), started(false),
cct(c)
{
...
class C_drain:public EventCallback {
  ...
  public:
    ...
    :drain_lock("C_drain::drain_lock"), drain_count(c) {
