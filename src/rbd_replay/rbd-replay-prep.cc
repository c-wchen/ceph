...
static void usage_exit(string prog, string msg)
{
    cerr << msg << endl;
    usage(prog);
    exit(1);
  ...
  public:
    ...
    m_io_count(0), m_anonymize(false), m_verbose(false) {
        ...
        uint64_t start_time_ns =
            bt_trace_handle_get_timestamp_begin(ctx, trace_handle,
                                                BT_CLOCK_REAL);
        ...
        int fd =
            open(output_file_name.c_str(), O_WRONLY | O_CREAT | O_EXCL, 0644);
        ASSERT_EXIT(fd >= 0,
  ...
  private:
    ...
    void process_event(uint64_t ts, struct bt_ctf_event *evt, IO::ptrs * ios) {
        ...
        if (strcmp(event_name, "librbd:open_image_enter") == 0) {
            ...
            IO::ptr io(new
                       OpenImageIO(ionum, ts, threadID, m_recent_completions,
            ...
            action_id_t ionum = next_id();
            IO::ptr io(new
                       CloseImageIO(ionum, ts, threadID, m_recent_completions,
                                    imagectx));
            thread->issued_io(io, &m_latest_ios);
            ...
            pair < string, string > aname(map_image_snap(name, snap_name));
            IO::ptr io(new
                       AioOpenImageIO(ionum, ts, threadID, m_recent_completions,
                                      imagectx, aname.first, aname.second,
                                      readonly));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
            ...
            action_id_t ionum = next_id();
            IO::ptr io(new
                       AioCloseImageIO(ionum, ts, threadID,
                                       m_recent_completions, imagectx));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(thread->latest_io());
            m_pending_ios[completion] = io;
        }
            ...
            action_id_t ionum = next_id();
            IO::ptr io(new
                       ReadIO(ionum, ts, threadID, m_recent_completions,
            ...
            action_id_t ionum = next_id();
            IO::ptr io(new
                       AioDiscardIO(ionum, ts, threadID, m_recent_completions,
                                    imagectx, offset, length));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
            m_pending_ios[completion] = io;
