...
void RGWGC::add_chain(ObjectWriteOperation & op, cls_rgw_obj_chain & chain,
                      const string & tag)
{
                ...
                std::list < cls_rgw_gc_obj_info > &result, bool * truncated)
{
    ...
    for (; *index < max_objs && result.size() < max; (*index)++, marker.clear()) {
        ...
        int ret =
            cls_rgw_gc_list(store->gc_pool_ctx, obj_names[*index], marker,
                            max - result.size(), expired_only, entries,
...
int RGWGC::process(int index, int max_secs)
{
    ...
    do {
        ...
        std::list < cls_rgw_gc_obj_info > entries;
        ret =
            cls_rgw_gc_list(store->gc_pool_ctx, obj_names[index], marker, max,
                            true, entries, &truncated, next_marker);
        if (ret == -ENOENT) {
            ret = 0;
            goto done;
        }
        ...
        for (iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            for (liter = chain.objs.begin(); liter != chain.objs.end(); ++liter) {
                ...
                if (obj.pool != last_pool) {
                    ...
                    ctx = new IoCtx;
                    ret =
                        rgw_init_ioctx(store->get_rados_handle(), obj.pool,
                                       *ctx);
                    if (ret < 0) {
                        last_pool = "";
                        dout(0) << "ERROR: failed to create ioctx pool=" << obj.
                            pool << dendl;
                        continue;
                    }
                    last_pool = obj.pool;
                ...
                dout(5) << "gc::process: removing " << obj.pool << ":" << obj.
                    key.name << dendl;
                ObjectWriteOperation op;
                ...
                if (ret < 0) {
                    remove_tag = false;
                    dout(0) << "failed to remove " << obj.
                        pool << ":" << oid << "@" << obj.loc << dendl;
                }
...
void *RGWGC::GCWorker::entry()
{
    do {
        ...
        int r = gc->process();
        if (r < 0) {
            dout(0) << "ERROR: garbage collection process() returned error r="
                << r << dendl;
        }
