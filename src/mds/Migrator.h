...
class Migrator {
    ...
    static const char *get_export_statename(int s) {
        ...
        case EXPORT_CANCELLING:
            ...
            return 0;
                                                                ...
                                                                // -- imports -- const static int IMPORT_DISCOVERING = 1;
                                                                // waiting for prep
  ...
  protected:
    struct export_base_t {
        ...
        bool restart = false;
        export_base_t(dirfrag_t df, mds_rank_t d, unsigned c,
                      uint64_t g):dirfrag(df), dest(d), pending_children(c),
            export_queue_gen(g) {
    }};
    ...
    void export_notify_abort(CDir * dir, export_state_t & stat,
                             set < CDir * >&bounds);
    ...
    void logged_import_caps(CInode * in,
                            mds_rank_t from,
                            map < client_t, pair < Session *,
                            uint64_t > >&imported_session_map, map < CInode *,
                            map < client_t, Capability::Export > >&cap_imports);
  ...
  public:
    ...
    int get_num_exporting() const {
        return export_state.size();
    } int get_export_queue_size() const {
        return export_queue.size();
    }
    // -- status -- int is_exporting(CDir * dir) const {
        map < CDir *, export_state_t >::const_iterator it =
            export_state.find(dir);
        ...
        map < dirfrag_t, import_state_t >::const_iterator it =
            import_state.find(df);
        ...
        map < dirfrag_t, import_state_t >::const_iterator p =
            import_state.find(df);
    ...
    // this returns true if we are export @dir,// and are not waiting for @who to be// be warned of ambiguous auth.// only returns meaningful results during EXPORT_WARNING state.
    ...
    void child_export_finish(std::shared_ptr < export_base_t > &parent,
                             bool success);
    ...
    void decode_import_inode(CDentry * dn, bufferlist::iterator & blp,
                             ...
                             list < ScatterLock * >&updated_scatterlocks);
    void decode_import_inode_caps(CInode * in, bool auth_cap,
                                  bufferlist::iterator & blp, map < CInode *,
                                  map < client_t,
                                  Capability::Export > >&cap_imports);
    void finish_import_inode_caps(CInode * in, mds_rank_t from, bool auth_cap,
                                  const map < client_t, pair < Session *,
                                  uint64_t > >&smap, const map < client_t,
                                  Capability::Export > &export_map,
                                  map < client_t,
                                  Capability::Import > &import_map);
    int decode_import_dir(bufferlist::iterator & blp, mds_rank_t oldauth,
                          CDir * import_root, EImportStart * le,
                          LogSegment * ls, map < CInode *, map < client_t,
                          Capability::Export > >&cap_imports,
                          list < ScatterLock * >&updated_scatterlocks,
                          utime_t now);
