...
class DeletingState {
  ...
  public:
    ...
    explicit DeletingState(const pair < spg_t,
                           PGRef > &in):lock("DeletingState::lock"),
        status(QUEUED), stop_deleting(false), pgid(in.first),
        old_pg_state(in.second) {
    }
    /// transition status to CLEARING_WAITING bool pause_clearing() {
        Mutex::Locker l(lock);
    ...
    bool start_or_resume_clearing() {
        ...
        assert(status == QUEUED ||
               status == DELETED_DIR || status == CLEARING_WAITING);
        if (stop_deleting) {
            status = CANCELED;
            cond.Signal();
            return false;
...
class OSDService {
  ...
  public:
    ...
    void await_reserved_maps() {
        ...
        assert(next_osdmap);
        while (true) {
            map < epoch_t, unsigned >::const_iterator i =
                map_reservations.cbegin();
            if (i == map_reservations.cend()
  ...
  public:
    ...
    void share_map_peer(int peer, Connection * con, OSDMapRef map =
                        OSDMapRef());
  ...
  public:
    struct ScrubJob {
        ...
        ScrubJob():cct(nullptr) {
                            ...
                            double pool_scrub_max_interval = 0, bool must =
                            true);
    ...
    utime_t reg_pg_scrub(spg_t pgid, utime_t t, double pool_scrub_min_interval,
        ...
        ScrubJob scrub(cct, pgid, t, pool_scrub_min_interval,
                       pool_scrub_max_interval, must);
    ...
    bool next_scrub_stamp(const ScrubJob & next, ScrubJob * out) {
        ...
        if (sched_scrub_pg.empty())
            return false;
        set < ScrubJob >::const_iterator iter =
            sched_scrub_pg.lower_bound(next);
  ...
  private:
    ...
    std::atomic_uint promote_probability_millis {
  ...
  private:
    ...
    void _queue_for_recovery(pair < epoch_t, PGRef > p,
                             uint64_t reserved_pushes) {
        assert(recovery_lock.is_locked_by_me());
        enqueue_back(p.second->info.pgid,
                     PGQueueable(PGRecovery(p.first, reserved_pushes),
                                 cct->_conf->osd_recovery_cost,
                                 cct->_conf->osd_recovery_priority,
                                 ceph_clock_now(), entity_inst_t(), p.first));
    }
  public:
    ...
    void clear_queued_recovery(PG * pg) {
        Mutex::Locker l(recovery_lock);
        for (list < pair < epoch_t, PGRef > >::iterator i =
             awaiting_throttle.begin(); i != awaiting_throttle.end();) {
            if (i->second.get() == pg) {
    ...
    void queue_for_recovery(PG * pg) {
        ...
        if (pg->
            get_state() & (PG_STATE_FORCED_RECOVERY | PG_STATE_FORCED_BACKFILL))
        {
            awaiting_throttle.
                push_front(make_pair(pg->get_osdmap()->get_epoch(), pg));
        }
        else {
            awaiting_throttle.
                push_back(make_pair(pg->get_osdmap()->get_epoch(), pg));
        }
        _maybe_queue_recovery();
    }
    void queue_recovery_after_sleep(PG * pg, epoch_t queued,
                                    uint64_t reserved_pushes) {
        Mutex::Locker l(recovery_lock);
  ...
  public:
    ...
    void expand_pg_num(OSDMapRef old_map, OSDMapRef new_map);
    void _maybe_split_pgid(OSDMapRef old_map, OSDMapRef new_map, spg_t pgid);
...
class OSD:public Dispatcher, public md_config_obs_t {
  ...
  protected:
    ...
    bool asok_command(string admin_command, cmdmap_t & cmdmap, string format,
                      ostream & ss);
  ...
  /** @} monc helpers */
    ...
    epoch_t latest_subscribed_epoch {
    0};
  ...
  private:
    ...
    class ShardedOpWQ:public ShardedThreadPool::ShardedWQ < pair < spg_t,
        PGQueueable >> {
        struct ShardData {
            Mutex sdata_lock;
            Cond sdata_cond;
            ...
            /// priority queue
            std::unique_ptr < OpQueue < pair < spg_t, PGQueueable >,
                entity_inst_t >> pqueue;
            ...
            void _enqueue_front(pair < spg_t, PGQueueable > item,
                ...
                if (priority >= cutoff)
                    ...
                    pqueue->enqueue_front(item.second.get_owner(),
                        ...
                        CephContext * cct, io_queue opqueue)
            :sdata_lock(lock_name.c_str(), false, true, false, cct),
                ...
                if (opqueue == io_queue::weightedpriority) {
                    pqueue = std::unique_ptr
                        < WeightedPriorityQueue < pair < spg_t, PGQueueable >,
                        entity_inst_t >> (new WeightedPriorityQueue < pair <
                                          spg_t, PGQueueable >,
                                          entity_inst_t > (max_tok_per_prio,
                                                           min_cost));
                }
                else if (opqueue == io_queue::prioritized) {
                    pqueue = std::unique_ptr
                        < PrioritizedQueue < pair < spg_t, PGQueueable >,
                        entity_inst_t >> (new PrioritizedQueue < pair < spg_t,
                                          PGQueueable >,
                                          entity_inst_t > (max_tok_per_prio,
                                                           min_cost));
                }
      ...
      public:
        ShardedOpWQ(uint32_t pnum_shards,
                    OSD * o, time_t ti, time_t si, ShardedThreadPool * tp)
      :    
        ShardedThreadPool::ShardedWQ < pair < spg_t, PGQueueable >> (ti, si,
                                                                     tp),
    ...
    struct PeeringWQ:public ThreadPool::BatchWorkQueue < PG > {
        ...
        :ThreadPool::BatchWorkQueue < PG > ("OSD::PeeringWQ", ti, si, tp),
            osd(o) {
  ...
  protected:
    ...
    bool advance_pg(epoch_t advance_to, PG * pg,
                    ThreadPool::TPHandle & handle,
                    PG::RecoveryCtx * rctx, set < PGRef > *split_pgs);
    void consume_map();
    void activate_map();
  ...
  protected:
    ...
    res_result _try_resurrect_pg(OSDMapRef curmap, spg_t pgid,
    ...
    void add_newly_split_pg(PG * pg, PG::RecoveryCtx * rctx);

    int handle_pg_peering_evt(spg_t pgid,
    ...
    void build_initial_pg_history(spg_t pgid,
                                  epoch_t created,
                                  utime_t created_stamp,
                                  pg_history_t * h, PastIntervals * pi);

    /// project pg history from from to now
    bool project_pg_history(spg_t pgid, pg_history_t & h, epoch_t from, const vector < int >&lastup, int lastupprimary, const vector < int >&lastacting, int lastactingprimary);    ///< @return false if there was a map gap between from and now
    ...
    Mutex min_last_epoch_clean_lock {
    ...
    void do_notifies(map < int,
                     ...
                     PastIntervals > > >&notify_list, OSDMapRef map);
    void do_queries(map < int, map < spg_t, pg_query_t > >&query_map,
                    OSDMapRef map);
    void do_infos(map < int, vector < pair < pg_notify_t,
                  PastIntervals > > >&info_map, OSDMapRef map);
   ...
   */
    ...
    struct Command {
        ...
        Command(vector < string > &c, ceph_tid_t t, bufferlist & bl,
                Connection * co)
    ...
    struct CommandWQ:public ThreadPool::WorkQueue < Command > {
        ...
        :ThreadPool::WorkQueue < Command > ("OSD::CommandWQ", ti, si, tp),
            osd(o) {
        } bool _empty() override {
    ...
    void do_command(Connection * con, ceph_tid_t tid, vector < string > &cmd,
                    bufferlist & data);
    ...
    // -- removing --
    struct RemoveWQ:public ThreadPool::WorkQueueVal < pair < PGRef,
        DeletingStateRef > > {
        CephContext *cct;
        ObjectStore *&store;
        ...
        RemoveWQ(CephContext * cct, ObjectStore * &o, time_t ti, time_t si,
                 ThreadPool * tp)
        :ThreadPool::WorkQueueVal < pair < PGRef,
            DeletingStateRef > >("OSD::RemoveWQ", ti, si, tp), cct(cct),
            store(o) {
        } bool _empty() override {
  ...
  private:
    ...
    io_queue get_io_queue() const {
        if (cct->_conf->osd_op_queue == "debug_random") {
            ...
            srand(time(NULL));
            unsigned which =
                rand() % (sizeof(index_lookup) / sizeof(index_lookup[0]));
  ...
  public:
    ...
    static int mkfs(CephContext * cct, ObjectStore * store,
                    const string & dev, uuid_d fsid, int whoami);
