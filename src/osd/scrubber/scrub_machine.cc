...
{
    DECLARE_LOCALS;         // 'scrbr' & 'pg_id' aliases
    dout(10) << "ReservingReplicas::react(const ReservationFailure&)" <<
             dendl;
...
{
    DECLARE_LOCALS;         // 'scrbr' & 'pg_id' aliases
    dout(10) << "WaitLastUpdate::on_new_updates(const UpdatesApplied&)" <<
             dendl;
    ...
    , NamedSimply(context < ScrubMachine > ().m_scrbr, "Act/BuildMap")
{
    ...
    if (scrbr->get_preemptor().was_preempted()) {
        ...
        if (ret == -EINPROGRESS) {
            ...
            dout(10) << "BuildMap::BuildMap() Error! Aborting. Ret: " << ret
                     << dendl;
...
sc::result DrainReplMaps::react(const GotReplicas &)
{
    ...
    dout(15) << "DrainReplMaps::react(const GotReplicas&): still draining "
             "incoming maps: " << scrbr->dump_awaited_maps() << dendl;
    return discard_event();
}
...
sc::result WaitReplicas::react(const GotReplicas &)
{
    ...
    if (!all_maps_already_called && scrbr->are_all_maps_available()) {
        dout(10) << "WaitReplicas::react(const GotReplicas&) got all" <<
                 dendl;
        ...
        if (scrbr->get_preemptor().disable_and_test()) {    // a test&set
            ...
            dout(10) << "WaitReplicas::react(const GotReplicas&) PREEMPTED!"
                     << dendl;
            return transit < PendingTimer > ();
...
WaitDigestUpdate::WaitDigestUpdate(my_context ctx)
    :    my_base(ctx)
    , NamedSimply(context < ScrubMachine > ().m_scrbr,
                  "Act/WaitDigestUpdate")
{
    DECLARE_LOCALS;         // 'scrbr' & 'pg_id' aliases
...
sc::result ActiveReplica::react(const SchedReplica &)
{
    DECLARE_LOCALS;         // 'scrbr' & 'pg_id' aliases
    dout(10) <<
             "ActiveReplica::react(const SchedReplica&). is_preemptable? " <<
