                             ...
                             optional_yield y)
{
    ...
    return static_cast<rgw::sal::RadosStore *>(driver)->ctl()->bucket->unlink_bucket(op_state.get_user_id(),
                           ...
                           optional_yield y, string *err)
{
    ...
    int r = static_cast<rgw::sal::RadosStore *>(driver)->ctl()->bucket->unlink_bucket(owner.get_id(),
            old_bucket->get_info().bucket, y, dpp, false);
    if (r < 0) {
    ...
    r = static_cast<rgw::sal::RadosStore *>(driver)->ctl()->bucket->link_bucket(op_state.get_user_id(),
        loc_bucket->get_info().bucket, loc_bucket->get_info().creation_time, y, dpp, true, &ep_data);
    if (r < 0) {
...
public:
    RGWMetadataHandlerPut_Bucket(RGWBucketMetadataHandler *_handler,
                                 ...
                                 optional_yield y,
                                 RGWMDLogSyncType type, bool from_remote_zone) : RGWMetadataHandlerPut_SObj(_handler, op, entry, obj, objv_tracker, y,
