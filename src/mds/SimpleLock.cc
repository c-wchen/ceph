...
void SimpleLock::dump(ceph::Formatter * f) const const
{
    ceph_assert(f != NULL);
...
int SimpleLock::get_wait_shift() const const
{
    switch (get_type()) {
    case CEPH_LOCK_DN:
        return 8;
...
int SimpleLock::get_cap_shift() const const
{
    switch (get_type()) {
    case CEPH_LOCK_IAUTH:
...
int SimpleLock::get_cap_mask() const const
{
    switch (get_type()) {
    case CEPH_LOCK_IFILE:
