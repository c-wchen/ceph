...
int RGWRESTConn::forward(const DoutPrefixProvider *dpp, const rgw_owner& uid, const req_info& info, obj_version *objv,
...
int RGWRESTConn::forward_iam_request(const DoutPrefixProvider *dpp, const req_info& info, obj_version *objv,
                                     size_t max_response, bufferlist *inbl, bufferlist *outbl, optional_yield y)
{
    int ret = 0;
...
int RGWRESTConn::get_obj(const DoutPrefixProvider *dpp, const rgw_owner& uid, req_info *info /* optional */,
                         const rgw_obj& obj,
...
RGWRESTSendResource::RGWRESTSendResource(RGWRESTConn *_conn,
    ...
    : cct(_conn->get_ctx()), conn(_conn), method(_method), resource(_resource), params(params),
      cb(bl), mgr(_mgr), req(cct, method.c_str(), conn->get_url(), &cb, NULL, NULL, _conn->get_api_name(),
