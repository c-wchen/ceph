...
class LRU {
  public:
    ...
    } uint64_t lru_get_top() const {
        return top.size();
    } uint64_t lru_get_bot() const {
        return bottom.size();
    } uint64_t lru_get_pintail() const {
        return pintail.size();
    } uint64_t lru_get_num_pinned() const {
        return num_pinned;
    } void lru_set_midpoint(double f) {
