...
static int write_log_entry(cls_method_context_t hctx, string &index,
                           cls_log_entry &entry)
{
    bufferlist bl;
    encode(entry, bl);
...
{
    char buf[32];
    snprintf(buf, sizeof(buf), "%010ld.%06ld_", (long)ts.sec(),
             (long)ts.usec());
...
static int cls_log_add(cls_method_context_t hctx, bufferlist *in,
                       bufferlist *out)
{
...
static int cls_log_list(cls_method_context_t hctx, bufferlist *in,
                        bufferlist *out)
{
    auto in_iter = in->cbegin();
    ...
    bool use_time_boundary = (!op.from_time.is_zero()
...
#define MAX_ENTRIES 1000
    ...
    int rc =
        cls_cxx_map_get_vals(hctx, from_index, log_index_prefix, max_entries,
                             &keys, &ret.truncated);
    if (rc < 0) {
        return rc;
    }
...
static int cls_log_trim(cls_method_context_t hctx, bufferlist *in,
                        bufferlist *out)
{
    auto in_iter = in->cbegin();
    ...
    if (to_index < first_key) {
        CLS_LOG(20, "listed key %s past to_index=%s", first_key.c_str(),
                to_index.c_str());
        return -ENODATA;
    }

    CLS_LOG(20, "listed key %s, removing through %s", first_key.c_str(),
            to_index.c_str());
...
static int cls_log_info(cls_method_context_t hctx, bufferlist *in,
                        bufferlist *out)
{
    auto in_iter = in->cbegin();
...
CLS_INIT(log)
{
    ...
    cls_register_cxx_method(h_class, "add", CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_log_add, &h_log_add);
    cls_register_cxx_method(h_class, "list", CLS_METHOD_RD, cls_log_list,
                            &h_log_list);
    cls_register_cxx_method(h_class, "trim", CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_log_trim, &h_log_trim);
    cls_register_cxx_method(h_class, "info", CLS_METHOD_RD, cls_log_info,
                            &h_log_info);
