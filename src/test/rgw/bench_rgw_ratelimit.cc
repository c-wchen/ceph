                      ...
                      std::shared_ptr < RateLimiter > ratelimit)
{
    ...
    auto dout = DoutPrefix(g_ceph_context, ceph_subsys_rgw, "rate limiter: ");
    bool to_fail =
        ratelimit->should_rate_limit(methodop.c_str(), it.tenant, time, &info);
    if (to_fail) {
        it.rejected++;
        it.ops++;
        return true;
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        desc.add_options()
            ...
            ("threads", value < int >()->default_value(512),
             "server's threads count")
        ...
        params.wait_between_retries_ms =
            vm["wait_between_retries_ms"].as < size_t > ();
    ...
    for (int i = 0; i < num_ratelimit_classes; i++) {
        ...
        simulate_clients(context, tenantuser, info, ratelimit->get_active(),
                         params, to_run);
    }
