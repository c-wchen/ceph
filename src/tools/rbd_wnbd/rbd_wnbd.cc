...
bool RegistryDiskIterator::get(Config * cfg)
{
    ...
    *cfg = Config();
    ...
    int err = RegEnumKeyEx(reg_key->hKey, index, subkey_name, &subkey_name_sz,
                           NULL, NULL, NULL, NULL);
...
int restart_registered_mappings(int worker_count,
                                int total_timeout, int image_map_timeout)
{
    Config cfg;
...
class RBDService:public ServiceBase {
  ...
  public:
    ...
    static DWORD handle_connection(HANDLE pipe_handle) {
        ...
        BOOL success = ReadFile(pipe_handle, message, SERVICE_PIPE_BUFFSZ,
                                &bytes_read, NULL);
    ...
    static DWORD accept_pipe_connection() {
        ...
        HANDLE handler_thread =
            CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) handle_connection,
                         pipe_handle, 0, 0);
        if (!handler_thread) {
            err = GetLastError();
    ...
    void monitor_wnbd_adapter() {
        ...
        while (!stop_requsted) {
            ...
            hres = subscription.next(WMI_EVENT_TIMEOUT * 1000, 1,   // we'll process one event at a time
                                     &object, &returned);
    ...
    int run_hook() override {
        ...
        int r =
            restart_registered_mappings(thread_count, service_start_timeout,
                                        image_map_timeout);
        if (r) {
    ...
    int stop_hook() override {
        ...
        int r =
            disconnect_all_mappings(false, hard_disconnect,
                                    soft_disconnect_timeout, thread_count);
...
static int do_map(Config * cfg)
{
    ...
    handler = new WnbdHandler(image, cfg->devpath,
                              ...
                              cfg->io_req_workers, cfg->io_reply_workers);
    r = handler->start();
    if (r) {
    ...
    if (!cfg->parent_pipe.empty()) {
        parent_pipe_handle =
            CreateFile(cfg->parent_pipe.c_str(), GENERIC_WRITE, 0, NULL,
                                 ...
                                 std::string devpath)
{
    ...
    f->dump_int("disk_number",
                conn_info.DiskNumber ? conn_info.DiskNumber : -1);
    f->dump_string("status",
                   cfg.active ? WNBD_STATUS_ACTIVE : WNBD_STATUS_INACTIVE);
