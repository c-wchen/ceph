    ...
    namespace cache {
        namespace pwl {
            namespace rwl {
                ...
                void WriteLogEntry::writeback(librbd::cache::
                    ...
                    cache_bp =
                        buffer::ptr(buffer::
                                    create_static(this->write_bytes(),
                                                  (char *)this->cache_buffer));
                } void WriteLogEntry::init_bl(buffer::ptr & bp,
                                              buffer::list & bl) {
                    if (!is_writesame) {
                        bl.append(bp);
                        return;
                    ...
                    for (uint64_t i = 0;
                         i < ram_entry.write_bytes / ram_entry.ws_datalen;
                         i++) {
                        bl.append(bp);
                    }
                    int trailing_partial =
                        ram_entry.write_bytes % ram_entry.ws_datalen;
                    if (trailing_partial) {
                        bl.append(bp, 0, trailing_partial);
                    }
                }

                void WriteLogEntry::init_cache_buffer(std::vector <
                ...
                void WriteLogEntry::copy_cache_bl(bufferlist * out_bl) {
                    ...
                    buffer::ptr cloned_bp =
                        cache_bp.begin_deep().get_ptr(cache_bp.length());
                    out_bl->clear();
                ...
                void WriteSameLogEntry::writeback(librbd::cache::
                                                  ImageWritebackInterface &
                                                  image_writeback,
                                                  Context * ctx) {
                    bufferlist entry_bl;
