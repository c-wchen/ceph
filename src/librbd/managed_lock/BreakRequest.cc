...
} template < typename I > void BreakRequest < I >::send_get_watchers()
{
    ...
    librados::AioCompletion *rados_completion =
        ...
        &klass::handle_get_watchers > (this);
    m_out_bl.clear();
    int r =
        m_ioctx.aio_operate(m_oid, rados_completion, &op, &m_out_bl);
    ceph_assert(r == 0);
...
void BreakRequest < I >::handle_get_watchers(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to retrieve watchers: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
template < typename I > void BreakRequest < I >::send_blocklist()
{
    ...
    entity_name_t entity_name =
        entity_name_t::CLIENT(m_ioctx.get_instance_id());
    ldout(m_cct,
    ...
    if (!locker_addr.parse(m_locker.address)) {
        lderr(m_cct) << "unable to parse locker address: " << m_locker.
                     address << dendl;
        finish(-EINVAL);
        return;
    }
    ...
    m_asio_engine.get_rados_api().blocklist_add(m_locker.address,
                 expire,
                 librbd::asio::util::
...
template < typename I > void BreakRequest < I >::handle_blocklist(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to blocklist lock owner: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
template < typename I > void BreakRequest < I >::wait_for_osd_map()
{
    ...
    m_asio_engine.get_rados_api().
                 wait_for_latest_osd_map(librbd::asio::util::
    get_callback_adapter([this](int r) {
        handle_wait_for_osd_map
        (r);
    }));
}
...
void BreakRequest < I >::handle_wait_for_osd_map(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to wait for updated OSD map: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
