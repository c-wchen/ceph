...
struct MDSPerfMetricSubKeyDescriptor {
    ...
    MDSPerfMetricSubKeyDescriptor(MDSPerfMetricSubKeyType type,
...
typedef std::vector < MDSPerfMetricSubKeyDescriptor >
MDSPerfMetricKeyDescriptor;
...
struct MDSPerformanceCounterDescriptor {
    MDSPerformanceCounterType type =
        static_cast < MDSPerformanceCounterType >(-1);
    ...
    void pack_counter(const PerformanceCounter &c,
...
template <> struct denc_traits <MDSPerformanceCounterDescriptors > {
    ...
    static constexpr bool featured = false;
    static constexpr bool need_contiguous = true;
    static void bound_encode(const MDSPerformanceCounterDescriptors &v,
                             size_t &p)
    {
        p += sizeof(uint32_t);
        const auto size = v.size();
...
struct MDSPerfMetricLimit {
    ...
    MDSPerfMetricLimit()
    {
    } MDSPerfMetricLimit(const MDSPerformanceCounterDescriptor &order_by,
                         uint64_t max_count)
...
std::ostream &operator<<(std::ostream &os,
                         const MDSPerfMetricLimit &limit);
...
struct MDSPerfMetricQuery {
    ...
    MDSPerfMetricQuery()
    {
                         ...
                         const MDSPerformanceCounterDescriptors &
    ...
    void get_performance_counter_descriptors(MDSPerformanceCounterDescriptors *
            descriptors) const
    {
        *descriptors = performance_counter_descriptors;
    } template < typename L >
    void update_counters(L
                         && update_counter,
    ...
    void pack_counters(const PerformanceCounters &counters,
                       ceph::buffer::list *bl) const;
};
...
struct MDSPerfMetrics {
    MDSPerformanceCounterDescriptors performance_counter_descriptors;
    std::map < MDSPerfMetricKey,
        ceph::buffer::list > group_packed_performance_counters;
