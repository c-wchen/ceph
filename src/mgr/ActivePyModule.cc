...
int ActivePyModule::load_commands()
{
    ...
    if (command_list == nullptr) {
        ...
        derr << "Module " << get_name() << " has missing COMMANDS member" <<
            dendl;
        return -EINVAL;
    }
    if (!PyObject_TypeCheck(command_list, &PyList_Type)) {
        ...
        derr << "Module " << get_name() <<
            " has COMMANDS member of wrong type (" "should be a list)" << dendl;
        return -EINVAL;
    }
...
int ActivePyModule::handle_command(const cmdmap_t & cmdmap,
                                   std::stringstream * ds,
                                   std::stringstream * ss)
{
    assert(ss != nullptr);
    ...
    auto pResult = PyObject_CallMethod(pClassInstance,
                                       const_cast < char *>("handle_command"),
                                       const_cast < char *>("(O)"), py_cmd);
