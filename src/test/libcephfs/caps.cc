...
TEST(Caps, ReadZero)
{
    ...
    for (; i < 30; ++i) {
        ...
        int expect =
            CEPH_CAP_FILE_EXCL | CEPH_CAP_FILE_WR | CEPH_CAP_FILE_BUFFER;
        ...
        int wfd =
            ceph_open(cmount, cw_path, O_CREAT | O_TRUNC | O_WRONLY, 0644);
        ASSERT_LT(0, wfd);
    ...
    for (i = 0; i < 30; ++i) {
        ...
        int expect =
            CEPH_CAP_FILE_RD | CEPH_STAT_CAP_SIZE | CEPH_CAP_FILE_CACHE;
        int caps = ceph_debug_get_fd_caps(cmount, fd);
        ASSERT_EQ(expect, caps & expect);
