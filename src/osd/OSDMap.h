...
template < typename A, typename B > bool vectors_equal(A a, B b)
{
    return
        a.size() == b.size() &&
        (a.empty() ||
...
bl.append((char *)p.second, (*p.second + 1) * sizeof(int32_t));
      ...
      for (auto i:offsets) {
            map.insert(map.end(),
                       make_pair(i.first, (int32_t *) (start + i.second)));
        }
    }
    void rebuild() {
        bufferlist bl;
      ...
      public:
        ...
        int32_t new_max_osd;
         ...
         mempool::osdmap::map < string, map < string,
            string > >new_erasure_code_profiles;
         ...
         mempool::osdmap::map < int32_t, pair < epoch_t,
         ...
         mempool::osdmap::map < pg_t, mempool::osdmap::vector < pair < int32_t,
...
bool pending_osd_state_clear(int osd, unsigned state) {
  ...
  private:
    ...
    struct addrs_s {
        mempool::osdmap::vector < ceph::shared_ptr < entity_addr_t >
            >client_addr;
        mempool::osdmap::vector < ceph::shared_ptr < entity_addr_t >
            >cluster_addr;
        mempool::osdmap::vector < ceph::shared_ptr < entity_addr_t >
  ...
  public:
    ...
    } void get_full_osd_util(const mempool::pgmap::unordered_map < int32_t,
                             osd_stat_t > &osd_stat, map < int, float >*full,
                             map < int, float >*backfill, map < int,
                             float >*nearfull) const;
    void get_full_pools(CephContext * cct, set < int64_t > *full,
  ...
  /***** cluster state *****/
    ...
    } void adjust_osd_weights(const map < int, double >&weights,
    ...
    const map < string, string > &get_erasure_code_profile(const string & name) const {
        ...
        if (i == erasure_code_profiles.end())
            return empty;
             ...
             return i->second;
    } const mempool::osdmap::map < string, map < string,
        string > >&get_erasure_code_profiles() const {
   ...
   */ bool subtree_is_down(int id, set < int >*down_cache) const;
    ...
    bool subtree_type_is_down(CephContext * cct, int id, int subtree_type,
                              set < int >*down_in_osds, set < int >*up_in_osds,
                              set < int >*subtree_up, unordered_map < int,
                              set < int > >*subtree_type_down) const;
    ...
    const entity_addr_t & get_addr(int osd) const {
        assert(exists(osd));
        return osd_addrs->client_addr[osd] ? *osd_addrs->
            client_addr[osd] : osd_addrs->blank;
    } const entity_addr_t & get_cluster_addr(int osd) const {
        assert(exists(osd));
        if (!osd_addrs->cluster_addr[osd]
            || *osd_addrs->cluster_addr[osd] == entity_addr_t())
            return get_addr(osd);
        return *osd_addrs->cluster_addr[osd];
    } const entity_addr_t & get_hb_back_addr(int osd) const {
        assert(exists(osd));
        return osd_addrs->hb_back_addr[osd] ? *osd_addrs->
            hb_back_addr[osd] : osd_addrs->blank;
    } const entity_addr_t & get_hb_front_addr(int osd) const {
        assert(exists(osd));
        return osd_addrs->hb_front_addr[osd] ? *osd_addrs->
            hb_front_addr[osd] : osd_addrs->blank;
  ...
  /****   mapping facilities   ****/
    int map_to_pg(int64_t pool,
                  const string & name,
                  const string & key, const string & nspace, pg_t * pg) const;
  ...
  private:
    ...
    void _pg_to_raw_osds(const pg_pool_t & pool, pg_t pg,
                         vector < int >*osds, ps_t * ppps) const;
   ...
   */
    void _pg_to_up_acting_osds(const pg_t & pg, vector < int >*up,
   ...
   */
        ...
        for (auto & p:pools) {
            if ((int)p.second.get_crush_rule() == rule_id) {
                pool_ids->insert(p.first);
                                 ...
                                 int osd, set < int64_t > *pool_ids) const;
    ...
    static bool primary_changed(int oldprimary,
    ...
    } int clean_pg_upmaps(CephContext * cct, Incremental * pending_inc) const;
    ...
    int calc_pg_upmaps(CephContext * cct, float max_deviation,  ///< max deviation from target (value < 1.0)
  ...
  public:
    ...
    void print_tree(Formatter * f, ostream * out, unsigned dump_flags =
                    0) const;
    ...
    static void dump_erasure_code_profiles(const mempool::osdmap::map < string,
