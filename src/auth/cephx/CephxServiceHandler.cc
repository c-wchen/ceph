...
int CephxServiceHandler::handle_request(bufferlist::iterator & indata,
                                        bufferlist & result_bl,
                                        uint64_t & global_id,
                                        AuthCapsInfo & caps, uint64_t * auid)
{
    int ret = 0;
    ...
    case CEPHX_GET_AUTH_SESSION_KEY:
        {
            ...
            if (!key_server->
                get_service_secret(CEPH_ENTITY_TYPE_AUTH, info.service_secret,
                                   info.secret_id)) {
                ldout(cct,
            ...
            if (!key_server->
                get_service_caps(entity_name, CEPH_ENTITY_TYPE_MON, caps)) {
                ldout(cct,
                ...
                if (!caps_str || !caps_str[0]) {
                    ldout(cct,
                          0) << "mon caps null for " << entity_name << dendl;
                    ret = -EACCES;
                }
            }
        }
    ...
    case CEPHX_GET_PRINCIPAL_SESSION_KEY:
        {
            ...
            if (!cephx_verify_authorizer
                (cct, key_server, indata, auth_ticket_info, nullptr, tmp_bl)) {
                ret = -EPERM;
                break;
            }
            ...
            CryptoKey no_key;
            build_cephx_response_header(cephx_header.request_type, ret,
                                        result_bl);
            cephx_build_service_ticket_reply(cct, auth_ticket_info.session_key,
                                             info_vec, false, no_key,
                                             result_bl);
        }
        break;
