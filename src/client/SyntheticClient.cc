        ...
        case SYNCLIENT_MODE_THRASHLINKS:
            {
                ...
                iargs.pop_front();
                if (run_me()) {
                    dout(2) << "thrashlinks " << sarg1 << " " << iarg1 << " " <<
                        iarg2 << " " << iarg3 << dendl;
        ...
        case SYNCLIENT_MODE_CREATEOBJECTS:
            {
                ...
                iargs.pop_front();
                if (run_me()) {
                    dout(2) << "createobjects " << count << " of " << size <<
                        " bytes" << ", " << inflight << " in flight" << dendl;
        ...
        case SYNCLIENT_MODE_OBJECTRW:
            {
                ...
                iargs.pop_front();
                if (run_me()) {
                    dout(2) << "objectrw " << count << " " << size << " " <<
                        wrpc << " " << overlap << " " << rskew << " " << wskew
                        << dendl;
        ...
        case SYNCLIENT_MODE_TRACE:
            {
                ...
                char realtfile[100];
                snprintf(realtfile, sizeof(realtfile), tfile.c_str(),
                ...
                if (run_me()) {
                    ...
                    for (int i = 0; i < iarg1; i++) {
                        ...
                        dout(0) << " trace " << tfile << " loop " << (i +
                                                                      1) << "/"
                            << iarg1 << " done in " << (double)lat << " seconds"
                            << dendl;
        ...
        case SYNCLIENT_MODE_LOOKUPHASH:
            {
                ...
                sscanf(diname.c_str(), "%llx",
                       (long long unsigned *)&dirino.val);
...
int SyntheticClient::play_trace(Trace & t, string & prefix, bool metadata_only)
{
    ...
    if (prefix.length()) {
        ...
        if (client->
            ll_lookupx(i1, prefix.c_str(), &i2, &stx, CEPH_STATX_INO, 0,
                       perms) == 0) {
    ...
    while (!t.end()) {
        ...
        if (strcmp(op, "link") == 0) {
            ...
            struct stat st;
            const char *a = t.get_string(buf, p);
            if (strcmp(a, p) != 0 && strcmp(a, "/") != 0 && strcmp(a, "/lib") != 0 &&   // or /lib.. that would be a lookup. hack.
        ...
        // low level ops ---------------------
            ...
            if (ll_inos.count(i)) {
                ...
                if (client->
                    ll_lookupx(i1, name, &i2, &stx, CEPH_STATX_INO, 0,
                               perms) == 0)
            ...
            int64_t n = t.get_int();
            if (ll_inos.count(i) &&
                client->ll_forget(client->
                                  ll_get_inode(vinodeno_t
                                               (ll_inos[i], CEPH_NOSNAP)), n))
                ll_inos.erase(i);
        }
            ...
            if (ll_inos.count(i)) {
                Fh *fhp;
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
                if (client->ll_create(i1, n, m, f, &attr, NULL, &fhp, perms) ==
                    0) {
                    ll_inos[ri] = attr.st_ino;
                    ll_files[r] = fhp;
                }
        ...
        // object-level traces
            ...
            dout(0) << (t.get_line() -
                        1) << ": *** trace hit unrecognized symbol '" << op <<
                "' " << dendl;
            ceph_abort();
        }
    }
    ...
    // close open files
    for (ceph::unordered_map < int64_t, int64_t >::iterator fi =
         open_files.begin(); fi != open_files.end(); ++fi) {
        dout(1) << "leftover close " << fi->second << dendl;
        if (fi->second > 0)
            client->close(fi->second);
    }
    for (ceph::unordered_map < int64_t, dir_result_t * >::iterator fi =
         open_dirs.begin(); fi != open_dirs.end(); ++fi) {
    ...
    for (ceph::unordered_map < int64_t, dir_result_t * >::iterator fi =
         ll_dirs.begin(); fi != ll_dirs.end(); ++fi) {
...
int SyntheticClient::full_walk(string & basedir)
{
    ...
    while (!dirq.empty()) {
        ...
        for (list < string >::iterator it = contents.begin();
            ...
            printf("%llx %c%c%c%c%c%c%c%c%c%c %2d %5d %5d %8llu %12s %s\n",
                   ...
                   (long long unsigned)st.st_size, tm, file.c_str());
    ...
    for (ceph::unordered_map < inodeno_t, int >::iterator p = nlink.begin();
         p != nlink.end(); ++p) {
        if (nlink_seen[p->first] != p->second)
            dout(0) << p->first << " nlink " << p->
                second << " != " << nlink_seen[p->first] << "seen" << dendl;
    }
...
int SyntheticClient::make_dirs(const char *basedir, int dirs, int files,
                               int depth)
{
    if (time_to_stop())
        return 0;
...
int SyntheticClient::stat_dirs(const char *basedir, int dirs, int files,
                               int depth)
{
    if (time_to_stop())
        return 0;
...
int SyntheticClient::read_dirs(const char *basedir, int dirs, int files,
                               int depth)
{
    if (time_to_stop())
        return 0;
    ...
    e -= s;
    if (r < 0) {
        dout(0) << "getdir couldn't readdir " << basedir << ", stopping" <<
            dendl;
        return -1;
    }
    ...
    for (int i = 0; i < files; i++) {
        ...
        if (client->lstat(d, &st, perms) < 0) {
            dout(2) << "read_dirs failed stat on " << d << ", stopping" <<
                dendl;
            return -1;
        }
        utime_t e = ceph_clock_now();
...
int SyntheticClient::make_files(int num, int count, int priv, bool more)
{
    ...
    dout(0) << "makefiles time is " << end << " or " << ((double)end /
                                                         (double)num) <<
        " per file" << dendl;
...
int SyntheticClient::check_first_primary(int fh)
{
    ...
    return client->objecter->with_osdmap([&](const OSDMap & o) {
                                         return o.get_pg_acting_primary(o.
                                                                        object_locator_to_pg
                                                                        (extents.
                                                                         begin
                                                                         ()->
...
int SyntheticClient::write_file(string & fn, int size, loff_t wrsize)   // size is in MB, wrsize in bytes
{
    ...
    memset(buf, 7, wrsize);
    int64_t chunks =
        (uint64_t) size * (uint64_t) (1024 * 1024) / (uint64_t) wrsize;
    UserPerm perms = client->pick_my_perms();
                               ...
                               int rdsize, bool ignoreprint)
{
    ...
    memset(buf, 1, rdsize);
    uint64_t chunks =
        (uint64_t) size * (uint64_t) (1024 * 1024) / (uint64_t) rdsize;
    UserPerm perms = client->pick_my_perms();
    ...
    for (unsigned i = 0; i < chunks; i++) {
        ...
        while ((char *)p + 32 < buf + rdsize) {
            uint64_t readoff = *p;
            uint64_t wantoff =
                (uint64_t) i * (uint64_t) rdsize + (uint64_t) ((char *)p - buf);
            p++;
            ...
            if (readoff != wantoff || readclient != client->get_nodeid()) {
                if (!bad && !ignoreprint)
                    dout(0) <<
                        "WARNING: wrong data from OSD, block says fileoffset="
        ...
        if (bad && !ignoreprint)
            dout(0) << " + " << (bad -
                                 1) << " other bad 16-byte bits in this block"
                << dendl;
    }
    ...
    for (int i = start; i < end; i += inc) {
        ...
        }
        {
            ...
            cond.wait(locker,[&unsafe, inflight, this] {
                      if (unsafe > inflight) {
                      dout(20) << "waiting for " << unsafe << " unsafe" <<
                      dendl;}
...
int SyntheticClient::object_rw(int nobj, int osize, int wrpc,
                               int overlappc, double rskew, double wskew)
{
    ...
    for (unsigned i = 0; i < 2000; i++) {
        ...
        if (read) {
            ...
            if (bad)
                dout(0) << " + " << (bad -
                                     1) <<
                    " other bad 16-byte bits in this block" << dendl;
        }
    }
...
int normdist(int min, int max, int stdev)
...
int SyntheticClient::random_walk(int num_req)
{
    ...
    while (left > 0) {
        ...
        // descend?
        if (roll_die(::pow((double).9, (double)cwd.depth()))
            && !subdirs.empty()) {
        ...
        if (r < 0) {
            ...
            dout(DBL) << "r = " << r << ", client may not have " << cwd <<
                ", cd .." << dendl;
            up();
            //}      
        }
...
void SyntheticClient::foo()
{
    ...
    if (1) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            int c = rand() % s;
            char src[80];
            snprintf(src, sizeof(src), "syn.0.0/dir.%d/dir.%d/file.%d", a, b,
                     c);
            //int fd = 
    ...
    if (0) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            snprintf(src, sizeof(src), "syn.0.0/dir.%d/dir.%d/file.%d", a, b,
                     c);
            snprintf(dst, sizeof(dst), "syn.0.0/dir.%d/dir.%d/file.%d", d, e,
                     f);
    ...
    if (1) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            snprintf(src, sizeof(src), "syn.0.0/dir.%d/dir.%d/file.%d", a, b,
                     c);
            snprintf(dst, sizeof(dst), "syn.0.0/dir.%d/dir.%d/newlink.%d", d, e,
                     f);
...
int SyntheticClient::thrash_links(const char *basedir, int dirs, int files,
                                  int depth, int n)
{
...
void SyntheticClient::import_find(const char *base, const char *find, bool data)
{
    ...
    while (!f.eof()) {
        ...
        if (dirnum % num_client != client->get_nodeid()) {
            dout(20) << "skipping leading dir " << dirnum << " " << filename <<
                dendl;
            continue;
        }
        ...
        dout(20) << " mode " << modestring << " to " << oct << mode << dec <<
            dendl;
        ...
        if (S_ISLNK(mode)) {
            ...
            if (S_ISDIR(mode)) {
                ...
                int fd =
                    client->open(f.c_str(), O_WRONLY | O_CREAT, perms,
                                 mode & 0777);
                ceph_assert(fd > 0);
                if (data) {
...
int SyntheticClient::chunk_file(string & filename)
{
    ...
    while (pos < size) {
        ...
        bufferlist bl;
        {
            ...
            client->filer->read(inode.ino, &inode.layout, CEPH_NOSNAP, pos, get,
                                &bl, 0, onfinish);
