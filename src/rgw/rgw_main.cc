...
int main(int argc, const char **argv)
{
    ...
    global_pre_init(&def_args, args, CEPH_ENTITY_TYPE_CLIENT,
    ...
    *g_conf};
    ...
    for (multimap < string, RGWFrontendConfig * >::iterator fiter =
        ...
        if (framework == "civetweb" || framework == "mongoose") {
            ...
            RGWProcessEnv env =
                { store, &rest, olog, 0, uri_prefix, auth_registry };
            ...
            RGWProcessEnv env =
                { store, &rest, olog, port, uri_prefix, auth_registry };
    ...
    for (list < RGWFrontend * >::iterator liter = fes.begin();
         liter != fes.end(); ++liter) {
