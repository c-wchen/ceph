...
namespace librbd {
    ...
    namespace {
        ...
        int validate_pool(IoCtx & io_ctx, CephContext * cct) {
            ...
            if (r == 0) {
                ...
                lderr(cct) << "failed to stat RBD directory: " <<
            ...
            if (r < 0) {
                lderr(cct) << "failed to release self-managed snapshot " <<
    ...
    void trim_image(ImageCtx * ictx, uint64_t newsize,
        ...
        C_SaferCond ctx;
        ictx->snap_lock.get_read();
        operation::TrimRequest <> *req =
            operation::TrimRequest <>::create(*ictx, &ctx, ictx->size, newsize,
                                              prog_ctx);
        ictx->snap_lock.put_read();
        req->send();
...
#define READ_SIZE 4096
    ...
    int list(IoCtx & io_ctx, vector < string > &names) {
        ...
        if (bl.length()) {
            ...
            ::decode(header, p);
            ::decode(m, p);
            for (map < string, bufferlist >::iterator q = m.begin();
                 q != m.end(); ++q) {
        ...
        if (r < 0) {
            lderr(cct) << "error listing v2 images: " << cpp_strerror(r) <<
                dendl;
            return r;
        }
      ...
      for (auto & info:image_info) {
          ...
          for (auto & id_it:info.second) {
                ...
                if ((imctx->features & RBD_FEATURE_DEEP_FLATTEN) == 0 &&
                    !imctx->snaps.empty()) {
                    lderr(cct) << "snapshot in-use by " << pool << "/" <<
                        imctx->name << dendl;
                    imctx->state->close();
                    return -EBUSY;
                }
                ...
                r = imctx->state->close();
                if (r < 0) {
                    lderr(cct) << "failed to close image: " << cpp_strerror(r)
                        << dendl;
                    return r;
                }
            }
    ...
    int list_children(ImageCtx * ictx, set < pair < string, string > >&names) {
      ...
      for (auto & info:image_info) {
            ...
            r = rados.ioctx_create2(info.first.first, ioctx);
            if (r < 0) {
                lderr(cct) << "Error accessing child image pool " << info.first.
                    second << dendl;
                return r;
            }
          ...
          for (auto & id_it:info.second) {
                string name;
                r = cls_client::dir_get_name(&ioctx, RBD_DIRECTORY, id_it,
                                             &name);
                if (r < 0) {
    ...
    int get_snap_namespace(ImageCtx * ictx,
        ...
        ldout(ictx->cct,
              20) << "get_snap_namespace " << ictx << " " << snap_name << dendl;
    ...
    int snap_is_protected(ImageCtx * ictx, const char *snap_name,
                          bool * is_protected) {
        ldout(ictx->cct,
              20) << "snap_is_protected " << ictx << " " << snap_name << dendl;
    ...
    int create(IoCtx & io_ctx, const std::string & image_name,
        ...
        if (r != -ENOENT) {
            if (r) {
                lderr(cct) << "Could not tell if " << image_name <<
                    " already exists" << dendl;
                return r;
            }
            lderr(cct) << "rbd image " << image_name << " already exists" <<
                dendl;
            return -EEXIST;
        }
        ...
        if (old_format) {
            ...
            image::CreateRequest <> *req =
                image::CreateRequest <>::create(io_ctx, image_name, id, size,
                                                opts,
    ...
    int clone(IoCtx & p_ioctx, const char *p_name, const char *p_snap_name,
        ...
        ImageCtx *p_imctx =
            new ImageCtx(p_name, "", p_snap_name, p_ioctx, true);
    ...
    int clone(ImageCtx * p_imctx, IoCtx & c_ioctx, const std::string & c_name,
        ...
        C_SaferCond cond;
        auto *req =
            image::CloneRequest <>::create(p_imctx, c_ioctx, c_name, id, c_opts,
                                           non_primary_global_image_id,
                                           primary_mirror_uuid, op_work_queue,
                                           &cond);
        req->send();
    ...
    int rename(IoCtx & io_ctx, const char *srcname, const char *dstname) {
        ...
        if (r < 0) {
            lderr(cct) << "error opening source image: " << cpp_strerror(r) <<
                dendl;
            return r;
        }
        ...
        BOOST_SCOPE_EXIT_END return ictx->operations->rename(dstname);
    }
    ...
    int get_parent_info(ImageCtx * ictx, string * parent_pool_name,
        ...
        if (parent_pool_name) {
            ...
            if (r < 0) {
                lderr(ictx->
                      cct) << "error looking up pool name: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
        }
    ...
    int set_image_notification(ImageCtx * ictx, int fd, int type) {
        CephContext *cct = ictx->cct;
        ldout(cct,
              20) << __func__ << " " << ictx << " fd " << fd << " type" << type
            << dendl;
    ...
    int lock_acquire(ImageCtx * ictx, rbd_lock_mode_t lock_mode) {
        ...
        C_SaferCond lock_ctx;
        {
            ...
            if (ictx->get_exclusive_lock_policy()->may_auto_request_lock()) {
                ictx->
                    set_exclusive_lock_policy(new exclusive_lock::
                                              StandardPolicy(ictx));
            }
        ...
        int r = lock_ctx.wait();
        if (r < 0) {
            lderr(cct) << "failed to request exclusive lock: " <<
                cpp_strerror(r)
                << dendl;
            return r;
        }
    ...
    int remove(IoCtx & io_ctx, const std::string & image_name,
        ...
        C_SaferCond cond;
        auto req =
            librbd::image::RemoveRequest <>::create(io_ctx, image_name,
                                                    image_id, force,
                                                    from_trash_remove, prog_ctx,
                                                    op_work_queue, &cond);
        req->send();
    ...
    int trash_move(librados::IoCtx & io_ctx, rbd_trash_image_source_t source,
        ...
        if (r < 0) {
            ...
            r = cls_client::dir_get_id(&io_ctx, RBD_DIRECTORY, image_name,
                                       &image_id);
            if (r < 0) {
                if (r != -ENOENT) {
            ...
            if (ictx->exclusive_lock != nullptr) {
                ...
                if (r < 0) {
                    lderr(cct) << "cannot obtain exclusive lock - not removing"
                        << dendl;
    ...
    int trash_list(IoCtx & io_ctx, vector < trash_image_info_t > &entries) {
        ...
        do {
            ...
            int r = cls_client::trash_list(&io_ctx, last_read, max_read,
                                           &trash_entries);
            if (r < 0 && r != -ENOENT) {
                lderr(cct) << "error listing rbd trash entries: " <<
                    cpp_strerror(r)
                    << dendl;
                return r;
            }
          ...
          for (const auto & entry:trash_entries) {
                rbd_trash_image_source_t source =
                    ...
                    (entry.second.source);
                entries.push_back( {
                                  entry.first, entry.second.name, source,
    ...
    int trash_restore(librados::IoCtx & io_ctx, const std::string & image_id,
        ...
        r = cls_client::get_id(&io_ctx, util::id_obj_name(image_name),
                               &existing_id);
        if (r < 0 && r != -ENOENT) {
            ...
            if (existing_id != image_id) {
                ldout(cct,
                      2) << "an image with the same name already exists" <<
                    dendl;
                return -EEXIST;
            }
            create_id_obj = false;
    ...
    int snap_exists(ImageCtx * ictx,
                    const cls::rbd::SnapshotNamespace & snap_namespace,
                    const char *snap_name, bool * exists) {
        ldout(ictx->cct,
              20) << "snap_exists " << ictx << " " << snap_name << dendl;
    ...
    int snap_remove(ImageCtx * ictx, const char *snap_name, uint32_t flags,
                    ProgressContext & pctx) {
        ldout(ictx->cct,
              20) << "snap_remove " << ictx << " " << snap_name << " flags: " <<
            flags << dendl;
        ...
        if (is_protected && flags & RBD_SNAP_REMOVE_UNPROTECT) {
            ...
            if (r < 0) {
                lderr(ictx->
                      cct) << "failed to unprotect snapshot: " << snap_name <<
                    dendl;
                return r;
            }
            ...
            if (is_protected) {
                lderr(ictx->
                      cct) << "snapshot is still protected after unprotection"
                    << dendl;
                ceph_abort();
            }
        }
    ...
    int snap_get_timestamp(ImageCtx * ictx, uint64_t snap_id,
    ...
    int copy(ImageCtx * src, IoCtx & dest_md_ctx, const char *destname,
        ...
        int r =
            create(dest_md_ctx, destname, "", src_size, opts, "", "", false);
        if (r < 0) {
    ...
    class C_CopyRead:public Context {
      public:
        ...
        :m_throttle(throttle), m_dest(dest), m_offset(offset), m_bl(bl),
            ...
            if (r < 0) {
                lderr(m_dest->
                      cct) << "error reading from source image at offset " <<
    ...
    int copy(ImageCtx * src, ImageCtx * dest, ProgressContext & prog_ctx,
        ...
        while (more_results) {
            r = cls_client::metadata_list(&src->md_ctx, src->header_oid,
                                          last_key, 0, &pairs);
            if (r < 0 && r != -EOPNOTSUPP && r != -EIO) {
                lderr(cct) << "couldn't list metadata: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
            else if (r == 0 && !pairs.empty()) {
                r = cls_client::metadata_set(&dest->md_ctx, dest->header_oid,
                                             pairs);
                if (r < 0) {
                    lderr(cct) << "couldn't set metadata: " << cpp_strerror(r)
                        << dendl;
                    return r;
                }
        ...
        for (uint64_t offset = 0; offset < src_size; offset += period) {
            ...
            auto comp =
                io::AioCompletion::create_and_start < Context > (ctx, src,
                                                                 io::
                                                                 AIO_TYPE_READ);

            io::ImageReadRequest <> req(*src, comp, { {
                                        offset, len}
                                        }
                                        , io::ReadResult {
                                        bl}
    ...
    int snap_set(ImageCtx * ictx,
                 ...
                 const char *snap_name) {
        ldout(ictx->cct,
              20) << "snap_set " << ictx << " snap = " << (snap_name ? snap_name
                                                           : "NULL") << dendl;
    ...
    int lock(ImageCtx * ictx, bool exclusive, const string & cookie,
             const string & tag) {
        ldout(ictx->cct,
              20) << "lock image " << ictx << " exclusive=" << exclusive <<
            " cookie='" << cookie << "' tag='" << tag << "'" << dendl;
     ...
     */
        {
            RWLock::RLocker locker(ictx->md_lock);
            r = rados::cls::lock::lock(&ictx->md_ctx, ictx->header_oid,
                                       RBD_LOCK_NAME,
    ...
    int break_lock(ImageCtx * ictx, const string & client,
                   const string & cookie) {
        ldout(ictx->cct,
              20) << "break_lock image " << ictx << " client='" << client <<
            "' cookie='" << cookie << "'" << dendl;
        ...
        if (ictx->blacklist_on_break_lock) {
            ...
            if (r < 0) {
                lderr(ictx->
                      cct) << "unable to retrieve lock info: " <<
                    cpp_strerror(r)
                    << dendl;
                return r;
            }
            ...
            if (r < 0) {
                lderr(ictx->
                      cct) << "unable to blacklist client: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
        }
    ...
    int64_t read_iterate(ImageCtx * ictx, uint64_t off, uint64_t len,
        ...
        while (left > 0) {
            ...
            auto c = io::AioCompletion::create_and_start(&ctx, ictx,
                                                         io::AIO_TYPE_READ);
            io::ImageRequest <>::aio_read(ictx, c, { {
                                          off, read_len}
                                          }
                                          , io::ReadResult {
                                          &bl}
    ...
    int metadata_get(ImageCtx * ictx, const string & key, string * value) {
        ...
        return cls_client::metadata_get(&ictx->md_ctx, ictx->header_oid, key,
                                        value);
    }

    int metadata_list(ImageCtx * ictx, const string & start, uint64_t max,
                      map < string, bufferlist > *pairs) {
        CephContext *cct = ictx->cct;
        ...
        return cls_client::metadata_list(&ictx->md_ctx, ictx->header_oid, start,
                                         max, pairs);
    }
    ...
    struct C_RBD_Readahead:public Context {
        ...
        uint64_t offset;
        uint64_t length;
        C_RBD_Readahead(ImageCtx * ictx, object_t oid, uint64_t offset,
                        uint64_t length)
        :ictx(ictx), oid(oid), offset(offset), length(length) {
        } void finish(int r) override {
            ldout(ictx->cct,
                  20) << "C_RBD_Readahead on " << oid << ": " << offset << "+"
                << length << dendl;
    ...
    void readahead(ImageCtx * ictx,
        ...
        pair < uint64_t, uint64_t > readahead_extent =
            ictx->readahead.update(image_extents, image_size);
        ...
        if (readahead_length > 0) {
            ...
            for (map < object_t, vector < ObjectExtent > >::iterator p =
                ...
                for (vector < ObjectExtent >::iterator q = p->second.begin();
                     q != p->second.end(); ++q) {
                    ldout(ictx->cct,
                          20) << "(readahead) oid " << q->oid << " " << q->
                        offset << "~" << q->length << dendl;

                    Context *req_comp =
                        new C_RBD_Readahead(ictx, q->oid, q->offset, q->length);
