                                ...
                                const ClusterInfo &cluster_info)
{
    out << "{client_name=" << cluster_info.client_name << ", cluster_name="
        << cluster_info.cluster_name << ", fs_name=" << cluster_info.
        fs_name << "}";
    return out;
}
...
struct MirrorInfo {
    ...
    bool has_peer(std::string_view client_name,
                  std::string_view cluster_name, std::string_view fs_name) const
    {
    ...
    bool has_peers() const
    {
        return !peers.empty();
                    ...
                    std::string_view cluster_name, std::string_view fs_name)
    {
        peers.
        emplace(Peer
                (uuid, ClusterInfo(client_name, cluster_name, fs_name)));
    }
...
public:
    ...
    {
        return (mds_map.allows_standby_replay()
                && mds_map.get_num_in_mds() == 0)
...
public:
    ...
    FSMap(): default_compat(MDSMap::get_compat_set_default())
    {
        ...
        : epoch(rhs.epoch),
          ...
          standby_epochs(rhs.standby_epochs), struct_version(rhs.struct_version)
    {
        filesystems.clear();
    ...
    const CompatSet &get_default_compat() const
    {
    ...
    } void filter(const std::vector < std::string > &allowed)
    {
        ...
        erase_if(mds_roles, [&](const auto & r) {
            return std::find(allowed.begin(), allowed.end(),
                             fs_name_from_gid(r.first)) == allowed.end();
        }
                );
    }
     ...
     */
    bool gid_exists(mds_gid_t gid, const std::vector < std::string > &in = {
    }) const
    {
        try {
     ...
     */
    void assign_standby_replay(const mds_gid_t standby_gid,
     ...
     */
    void promote(mds_gid_t standby_gid,
     ...
     */
    Filesystem::ref create_filesystem(std::string_view name, int64_t metadata_pool,
     ...
     */
    void update_export_targets(mds_gid_t who,
                               const std::set < mds_rank_t > &targets)
    {
    ...
    version_t get_struct_version() const
    {
    ...
    } Filesystem::ref get_filesystem(fs_cluster_id_t fscid)
    {
    ...
    {
        return std::const_pointer_cast < const Filesystem >
               (filesystems.begin()->second);
    ...
    int parse_filesystem(std::string_view ns_str,
                         Filesystem::const_ref *result) const;

    int parse_role(std::string_view role_str,
                   mds_role_t *role,
                   std::ostream &ss,
                   const std::vector < std::string > &filter) const;

    int parse_role(std::string_view role_str,
                   mds_role_t *role, std::ostream &ss) const;
