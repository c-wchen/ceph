...
void PGMonitor::update_logger()
{
    ...
    for (ceph::unordered_map < int, int >::iterator p =
         pg_map.num_pg_by_state.begin(); p != pg_map.num_pg_by_state.end();
         ++p) {
    ...
    mon->cluster_logger->set(l_cluster_num_object_degraded,
                             pg_map.pg_sum.stats.sum.num_objects_degraded);
    mon->cluster_logger->set(l_cluster_num_object_misplaced,
                             pg_map.pg_sum.stats.sum.num_objects_misplaced);
    mon->cluster_logger->set(l_cluster_num_object_unfound,
                             pg_map.pg_sum.stats.sum.num_objects_unfound);
...
void PGMonitor::tick()
{
    ...
    if (!pg_map.pg_sum_deltas.empty()) {
        ...
        if (age > 2 * g_conf->mon_delta_reset_interval) {
            dout(10) << " clearing pg_map delta (" << age << " > " << g_conf->
                mon_delta_reset_interval << " seconds old)" << dendl;
            pg_map.clear_delta();
        }
    }
    ...
    if (!pg_map.per_pool_sum_deltas.empty()) {
        ceph::unordered_map < uint64_t, pair < pool_stat_t,
            utime_t > >::iterator it;
...
void PGMonitor::update_from_paxos(bool * need_bootstrap)
{
    ...
    while (version > pg_map.version) {
        ...
        if (r == -ENOENT) {
            dout(10) << __func__ << " failed to read_incremental, read_full" <<
                dendl;
            // reset pg map
            pg_map = PGMap();
            read_pgmap_full();
...
void PGMonitor::read_pgmap_full()
{
    ...
    for (KeyValueDB::Iterator i = mon->store->get_iterator(prefix); i->valid();
         i->next()) {
        string key = i->key();
        pg_t pgid;
...
void PGMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    string prefix = pgmap_meta_prefix;
    if (do_delete) {
        dout(1) << __func__ << " clearing pgmap data at v" << pending_inc.
            version << dendl;
        do_delete = false;
      for (auto key:{
             "version", "stamp", "last_osdmap_epoch",
    ...
    ::encode(pending_inc.stamp, incbl);
    {
        bufferlist dirty;
        string prefix = pgmap_pg_prefix;
        for (map < pg_t, pg_stat_t >::const_iterator p =
             pending_inc.pg_stat_updates.begin();
             p != pending_inc.pg_stat_updates.end(); ++p) {
            ::encode(p->first, dirty);
            bufferlist bl;
            ::encode(p->second, bl, features);
            t->put(prefix, stringify(p->first), bl);
        }
        for (set < pg_t >::const_iterator p = pending_inc.pg_remove.begin();
             p != pending_inc.pg_remove.end(); ++p) {
            ::encode(*p, dirty);
    ...
    }
    {
        ...
        for (map < int32_t, osd_stat_t >::const_iterator p =
             ...
             p != pending_inc.get_osd_stat_updates().end(); ++p) {
            ::encode(p->first, dirty);
            bufferlist bl;
            ::encode(p->second, bl, features);
        ...
        for (set < int32_t >::const_iterator p =
             ...
             p != pending_inc.get_osd_stat_rm().end(); ++p) {
            ::encode(*p, dirty);
            t->erase(prefix, stringify(*p));
        }
        ::encode(dirty, incbl);
...
bool PGMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    case MSG_MON_COMMAND:
        if (mon->osdmon()->osdmap.require_osd_release >= CEPH_RELEASE_LUMINOUS) {
            bufferlist rdata;
            mon->reply_command(op, -EOPNOTSUPP, "this command is obsolete",
                               rdata, get_last_committed());
            return true;
        }
...
bool PGMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    PaxosServiceMessage *m =
        static_cast < PaxosServiceMessage * >(op->get_req());
    dout(10) << "prepare_update " << *m << " from " << m->
        get_orig_source_inst() << dendl;
    switch (m->get_type()) {
    case MSG_PGSTATS:
...
bool PGMonitor::preprocess_pg_stats(MonOpRequestRef op)
{
    ...
    if (!session) {
        dout(10) << "PGMonitor::preprocess_pg_stats: no monitor session!" <<
            dendl;
        return true;
    }
    ...
    if (stats->had_map_for > 30.0 &&
        ...
        stats->epoch < mon->osdmon()->osdmap.get_epoch() && !session->proxy_con)
...
bool PGMonitor::pg_stats_have_changed(int from, const MPGStats * stats) constconst
{
    // any new osd info?
    ceph::unordered_map < int, osd_stat_t >::const_iterator s =
        pg_map.osd_stat.find(from);
    ...
    for (map < pg_t, pg_stat_t >::const_iterator p = stats->pg_stat.begin();
        ...
        ceph::unordered_map < pg_t, pg_stat_t >::const_iterator t =
...
struct PGMonitor::C_Stats:public C_MonOp {
    ...
    entity_inst_t who;
    C_Stats(PGMonitor * p, MonOpRequestRef op, MonOpRequestRef op_ack)
...
bool PGMonitor::prepare_pg_stats(MonOpRequestRef op)
{
    op->mark_pgmon_event(__func__);
    MPGStats *stats = static_cast < MPGStats * >(op->get_req());
    dout(10) << "prepare_pg_stats " << *stats << " from " << stats->
        get_orig_source() << dendl;
    int from = stats->get_orig_source().num();

    if (stats->fsid != mon->monmap->fsid) {
        dout(0) << "prepare_pg_stats on fsid " << stats->fsid << " != " << mon->
            monmap->fsid << dendl;
        return false;
    }
    ...
    if (!pg_stats_have_changed(from, stats)) {
        ...
        for (map < pg_t, pg_stat_t >::const_iterator p = stats->pg_stat.begin();
             p != stats->pg_stat.end(); ++p) {
            ack->pg_stat[p->first] =
                make_pair(p->second.reported_seq, p->second.reported_epoch);
        }
    ...
    if (pg_map.osd_stat.count(from))
        dout(10) << " got osd." << from << " " << stats->
            osd_stat << " (was " << pg_map.osd_stat[from] << ")" << dendl;
    else
        dout(10) << " got osd." << from << " " << stats->
            osd_stat << " (first report)" << dendl;

    // pg stats
    MPGStatsAck *ack = new MPGStatsAck;
    MonOpRequestRef ack_op =
        mon->op_tracker.create_request < MonOpRequest > (ack);
    ack->set_tid(stats->get_tid());
    for (map < pg_t, pg_stat_t >::iterator p = stats->pg_stat.begin();
        ...
        if (pg_map.pg_stat.count(pgid) &&
            pg_map.pg_stat[pgid].get_version_pair() >
            p->second.get_version_pair()) {
            dout(15) << " had " << pgid << " from " << pg_map.pg_stat[pgid].
                reported_epoch << ":" << pg_map.pg_stat[pgid].
                reported_seq << dendl;
            continue;
        }
        if (pending_inc.pg_stat_updates.count(pgid) &&
            pending_inc.pg_stat_updates[pgid].get_version_pair() >
            p->second.get_version_pair()) {
            dout(15) << " had " << pgid << " from " << pending_inc.
                pg_stat_updates[pgid].reported_epoch << ":" << pending_inc.
                pg_stat_updates[pgid].reported_seq << " (pending)" << dendl;
            continue;
        }

        if (pg_map.pg_stat.count(pgid) == 0) {
            dout(15) << " got " << pgid << " reported at " << p->second.
                reported_epoch << ":" << p->second.
                reported_seq << " state " << pg_state_string(p->second.state)
                << " but DNE in pg_map; pool was probably deleted." << dendl;
            continue;
        }

        dout(15) << " got " << pgid
            << " reported at " << p->second.reported_epoch << ":" << p->second.
            reported_seq << " state " << pg_state_string(pg_map.pg_stat[pgid].
                                                         state)
...
void PGMonitor::check_osd_map(epoch_t epoch)
{
    ...
    for (epoch_t e = pg_map.last_osdmap_epoch + 1; e <= epoch; e++) {
        dout(10) << __func__ << " applying osdmap e" << e << " to pg_map" <<
            dendl;
        bufferlist bl;
...
epoch_t PGMonitor::send_pg_creates(int osd, Connection * con, epoch_t next)
{
    ...
    MOSDPGCreate *m = NULL;
    epoch_t last = 0;
    for (map < epoch_t, set < pg_t > >::iterator q =
         p->second.lower_bound(next); q != p->second.end(); ++q) {
        dout(20) << __func__ << " osd." << osd << " from " << next <<
            " : epoch " << q->first << " " << q->second.
            size() << " pgs" << dendl;
        last = q->first;
        for (set < pg_t >::iterator r = q->second.begin(); r != q->second.end();
             ++r) {
            pg_stat_t & st = pg_map.pg_stat[*r];
            if (!m)
                m = new MOSDPGCreate(pg_map.last_osdmap_epoch);
            m->mkpg[*r] = pg_create_t(st.created,
                                      st.parent, st.parent_split_bits);
...
bool PGMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "pg scrub" ||
        ...
        ss << "instructing pg " << pgid << " on osd." << osd << " to " <<
            scrubop;
        r = 0;
    }
    else {
        r = process_pg_map_command(prefix, cmdmap, pg_map,
                                   mon->osdmon()->osdmap, f.get(), &ss, &rdata);
    }
...
bool PGMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "pg force_create_pg") {
        ...
        {
            PGMapUpdater::register_pg(mon->osdmon()->osdmap,
                                      pgid, epoch, true, pg_map, &pending_inc);
        }
    ...
    else if (prefix == "pg set_full_ratio" || prefix == "pg set_nearfull_ratio") {
        if (mon->osdmon()->osdmap.require_osd_release >= CEPH_RELEASE_LUMINOUS) {
  ...
  update:
    ...
    wait_for_finished_proposal(op,
                               new Monitor::C_Command(mon, op, r, rs,
                                                      get_last_committed() +
                                                      1));
    return true;
}
                           ...
                           CephContext * cct) const const
{
    ...
    if (mon->osdmon()->osdmap.require_osd_release < CEPH_RELEASE_LUMINOUS) {
        check_full_osd_health(summary, detail, pg_map.full_osds, "full",
                              HEALTH_ERR);
        check_full_osd_health(summary, detail, pg_map.nearfull_osds,
                              "near full", HEALTH_WARN);
        pg_map.get_health(cct, mon->osdmon()->osdmap, summary, detail);
    }
}

void PGMonitor::check_full_osd_health(list < pair < health_status_t,
                                      string > >&summary,
                                      list < pair < health_status_t,
                                      string > >*detail,
                                      const mempool::pgmap::set < int >&s,
                                      const char *desc,
                                      health_status_t sev) const const
{
    if (!s.empty()) {
        ostringstream ss;
...
void PGMonitor::check_subs()
{
    ...
    mon->with_session_map([this, &type] (const MonSessionMap & session_map) {
                          ...
                          Subscription * sub = *p;
                          ++p;
                          dout(20) << __func__ << " .. " << sub->session->
                          inst << dendl; check_sub(sub);}
                          }
    ) ;
}
...
class PGMonStatService:public MonPGStatService, public PGMapStatService {
  ...
  public:
    ...
    void dump_info(Formatter * f) const override {
        ...
        return process_pg_map_command(prefix, cmdmap, pgmap, osdmap, f, ss,
                                      odata);
    } int reweight_by_utilization(const OSDMap & osd_map, int oload,
                                  double max_changef, int max_osds, bool by_pg,
                                  const set < int64_t > *pools,
                                  bool no_increasing,
                                  mempool::osdmap::map < int32_t,
                                  uint32_t > *new_weights,
                                  std::stringstream * ss, std::string * out_str,
...
pgmap_pg_prefix("pgmap_pg"), pgmap_osd_prefix("pgmap_osd")
{
}
