...
public:
    ...
    GenericLogEntry(uint64_t image_offset_bytes =
                        0, uint64_t write_bytes = 0)
    ...
    {
        return nullptr;
    } virtual void writeback(librbd::cache::
                             ImageWritebackInterface &
                             image_writeback, Context *ctx)
    {
        ceph_assert(false);
    };
    virtual void writeback_bl(librbd::cache::
                              ImageWritebackInterface &
                              image_writeback, Context *ctx,
                              ceph::bufferlist && bl)
    {
...
public:
    ...
    std::atomic < bool > prior_sync_point_flushed = {
        true
    };
    std::shared_ptr < SyncPointLogEntry > next_sync_point_entry =
        nullptr;
    ...
    SyncPointLogEntry &operator=(const SyncPointLogEntry &) =
        delete;
...
public:
    ...
    GenericWriteLogEntry(std::shared_ptr < SyncPointLogEntry >
                         sync_point_entry,
    ...
    {
    } GenericWriteLogEntry(uint64_t image_offset_bytes,
                           uint64_t write_bytes)
        : GenericLogEntry(image_offset_bytes, write_bytes),
    ...
    GenericWriteLogEntry &operator=(const GenericWriteLogEntry &) =
        delete;
...
public:
    ...
    WriteLogEntry(std::shared_ptr < SyncPointLogEntry >
                  sync_point_entry, uint64_t image_offset_bytes,
                  uint64_t write_bytes)
        :    GenericWriteLogEntry(sync_point_entry, image_offset_bytes,
                                  write_bytes),
             m_entry_bl_lock(ceph::
                             make_mutex(pwl::
                                        unique_lock_name
    ...
    WriteLogEntry(uint64_t image_offset_bytes, uint64_t write_bytes,
        ...
        :
        WriteLogEntry(nullptr, image_offset_bytes,
                      write_bytes)
    {
        ram_entry.set_writesame(true);
        ram_entry.ws_datalen = data_length;
    ...
    virtual void init_cache_buffer(std::vector <
                                   WriteBufferAllocation >::
                                   iterator allocation)
    {
    }
    virtual void init_cache_bl(bufferlist &src_bl, uint64_t off,
                               uint64_t len)
    {
    }
    ...
    bool can_retire() const override
    {
        return (this->completed && this->get_flushed()
                && (0 == reader_count()));
...
{
public:
    DiscardLogEntry(std::shared_ptr < SyncPointLogEntry >
                    sync_point_entry, uint64_t image_offset_bytes,
                    uint64_t write_bytes,
    ...
    DiscardLogEntry(uint64_t image_offset_bytes,
                    uint64_t write_bytes)
        :    GenericWriteLogEntry(nullptr, image_offset_bytes,
                                  write_bytes)
    {
        ram_entry.set_discard(true);
    };
