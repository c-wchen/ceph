...
int NetHandler::create_socket(int domain, bool reuse_addr)
{
    ...
    if ((s = socket_cloexec(domain, SOCK_STREAM, 0)) == -1) {
        r = ceph_sock_errno();
        lderr(cct) << __func__ << " couldn't create socket " <<
    ...
    if (reuse_addr) {
        int on = 1;
        if (::
            setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (SOCKOPT_VAL_TYPE) & on,
                       sizeof(on)) == -1) {
            r = ceph_sock_errno();
...
int NetHandler::set_nonblock(int sd)
{
    ...
    if (fcntl(sd, F_SETFL, flags | O_NONBLOCK) < 0) {
        r = ceph_sock_errno();
        lderr(cct) << __func__ << " fcntl(F_SETFL,O_NONBLOCK): " <<
                   cpp_strerror(r) << dendl;
        return -r;
    }
#endif
...
int NetHandler::set_socket_options(int sd, bool nodelay, int size)
{
    ...
    if (nodelay) {
        ...
        r =::setsockopt(sd, IPPROTO_TCP, TCP_NODELAY,
                        (SOCKOPT_VAL_TYPE) & flag, sizeof(flag));
        if (r < 0) {
            r = ceph_sock_errno();
            ldout(cct,
    ...
    if (size) {
        r =::setsockopt(sd, SOL_SOCKET, SO_RCVBUF,
                        (SOCKOPT_VAL_TYPE) & size, sizeof(size));
        if (r < 0) {
            r = ceph_sock_errno();
            ldout(cct,
    ...
    int val = 1;
    r =::setsockopt(sd, SOL_SOCKET, SO_NOSIGPIPE, (SOCKOPT_VAL_TYPE) & val,
                    sizeof(val));
    if (r) {
        r = ceph_sock_errno();
        ldout(cct,
...
void NetHandler::set_priority(int sd, int prio, int domain)
{
    ...
    switch (domain) {
        case AF_INET:
            r =::setsockopt(sd, IPPROTO_IP, IP_TOS, (SOCKOPT_VAL_TYPE) & iptos,
                            sizeof(iptos));
            break;
        case AF_INET6:
            r =::setsockopt(sd, IPPROTO_IPV6, IPV6_TCLASS,
                            (SOCKOPT_VAL_TYPE) & iptos, sizeof(iptos));
            break;
        default:
    ...
    r =::setsockopt(sd, SOL_SOCKET, SO_PRIORITY, (SOCKOPT_VAL_TYPE) & prio,
                    sizeof(prio));
    if (r < 0) {
        r = ceph_sock_errno();
...
int NetHandler::generic_connect(const entity_addr_t &addr,
                                ...
                                bool nonblock)
{
    ...
    set_socket_options(s, cct->_conf->ms_tcp_nodelay,
                       cct->_conf->ms_tcp_rcvbuf);
    ...
    if (ret < 0) {
        ...
        ldout(cct,
              10) << __func__ << " connect: " << cpp_strerror(ret) << dendl;
        compat_closesocket(s);
        return -ret;
    }
