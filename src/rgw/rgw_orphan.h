...
struct RGWOrphanSearchStage {
    ...
    RGWOrphanSearchStage(RGWOrphanSearchStageId _stage, int _shard,
                         const string & _marker):stage(_stage), shard(_shard),
        marker(_marker) {
    }
...
int store_entries(const string & oid, const map < string,
                  bufferlist > &entries);
int read_entries(const string & oid, const string & marker, map < string,
                 bufferlist > *entries, bool * truncated);
};
...
class RGWOrphanSearch {
    ...
    int log_oids(map < int, string > &log_shards, map < int,
                 list < string > >&oids);
...
#define RGW_ORPHANSEARCH_HASH_PRIME 7877
    ...
    } int handle_stat_result(map < int, list < string > >&oids,
  ...
  public:
    RGWOrphanSearch(RGWRados * _store, int _max_ios,
