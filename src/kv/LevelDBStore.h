...
class LevelDBStore:public KeyValueDB {
   ...
   */
    struct options_t {
        ...
        options_t(): write_buffer_size(0),  //< 0 means default
    ...
    LevelDBStore(CephContext * c, const string & path):cct(c),
        logger(NULL), ceph_logger(NULL), path(path), db_cache(NULL),
#ifdef HAVE_LEVELDB_FILTER_POLICY
    filterpolicy(NULL),
#endif
    compact_queue_lock("LevelDBStore::compact_thread_lock"),
        compact_queue_stop(false), compact_thread(this), options() {
    }
    ...
    class LevelDBTransactionImpl:public KeyValueDB::TransactionImpl {
      public:
        ...
        virtual void rm_range_keys(const string & prefix,
                                   const string & start,
                                   const string & end) override;
    ...
    int get(const string & prefix,
            const string & key, bufferlist * value) override;

    using KeyValueDB::get;

    class LevelDBWholeSpaceIteratorImpl:public KeyValueDB::
        WholeSpaceIteratorImpl {
      protected:
        boost::scoped_ptr < leveldb::Iterator > dbiter;
      public:
        explicit LevelDBWholeSpaceIteratorImpl(leveldb::Iterator *
                                               iter):dbiter(iter) {
