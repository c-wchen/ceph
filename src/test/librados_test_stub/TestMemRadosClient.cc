...
int TestMemRadosClient::pool_get_base_tier(int64_t pool_id,
        int64_t *base_tier)
{
    // TODO
    *base_tier = pool_id;
...
int TestMemRadosClient::pool_list(std::list < std::pair < int64_t,
                                  std::string > > &v)
{
...
{
    m_mem_cluster->transaction_start({
        nspace, oid});
}
...
void TestMemRadosClient::transaction_finish(const std::string &nspace,
        const std::string &oid)
{
    m_mem_cluster->transaction_finish({
        nspace, oid});
}
