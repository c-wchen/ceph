...
struct cache_test_t:public seastar_test_suite_t {
    ...
    seastar::future < paddr_t > submit_transaction(TransactionRef t) {
        ...
        if (current.as_seg_paddr().get_segment_off() +
            (segment_off_t) bl.length() > segment_manager->get_segment_size())
            current =
                paddr_t::
                make_seg_paddr(segment_id_t
                               (current.as_seg_paddr().get_segment_id().
                                device_id(),
                                current.as_seg_paddr().get_segment_id().
                                device_segment_id() + 1), 0);

        auto prev = current;
        current.as_seg_paddr().set_segment_off(current.as_seg_paddr().
                                               get_segment_off()
                                               + bl.length());
        return segment_manager->segment_write(prev,
                                              std::move(bl),
                                              true).safe_then([this, prev, t =
                                                               std::
                                                               move(t)]
                                                              ()mutable {
                                                              cache->
    ...
    template < typename T, typename ... Args >
        ...
        return with_trans_intr(t,[this] (auto && ... args) {
    ...
    seastar::future <> set_up_fut()final {
        ...
        return segment_manager->init().safe_then([this] {
                    ...
                    return seastar::do_with(get_transaction(),
                                            [this] (auto & ref_t) {
                                            cache->init();
...
{
    run_async([this] {
              paddr_t addr; int csum = 0; {
              auto t = get_transaction();
              auto extent = cache->alloc_new_extent < TestBlockPhysical > (*t,
                                                                           TestBlockPhysical::
                                                                           SIZE,
                                                                           placement_hint_t::
                                                                           HOT,
                                                                           0);
              extent->set_contents('c'); csum = extent->get_crc32c();
              submit_transaction(std::move(t)).get0();
              addr = extent->get_paddr();}
              {
              auto t = get_transaction();
              auto extent = get_extent < TestBlockPhysical > (*t,
                                                              addr,
                                                              TestBlockPhysical::
                                                              SIZE).
              unsafe_get0(); ASSERT_EQ(extent->get_paddr(), addr);
...
{
    run_async([this] {
              paddr_t addr; int csum = 0; int csum2 = 0; {
              ...
              auto extent = get_extent < TestBlockPhysical > (*t,
                                                              ...
                                                              TestBlockPhysical::
                                                              SIZE).
              unsafe_get0(); ASSERT_TRUE(extent->is_clean());
              ...
              auto extent = get_extent < TestBlockPhysical > (*t,
                                                              ...
                                                              TestBlockPhysical::
                                                              SIZE).
              unsafe_get0(); auto t2 = get_transaction();
              auto extent2 =
              get_extent < TestBlockPhysical > (*t2, addr,
                                                TestBlockPhysical::SIZE).
              unsafe_get0(); ASSERT_EQ(&*extent, &*extent2);}
              {
              ...
              cache->duplicate_for_write(*t,
                                         extent)->cast < TestBlockPhysical > ();
              extent->set_contents('c'); csum2 = extent->get_crc32c();
              ASSERT_EQ(extent->get_paddr(), addr); {
              // test that concurrent read with fresh transaction sees old
              // block
              auto t2 = get_transaction();
              auto extent = get_extent < TestBlockPhysical > (*t2,
                                                              addr,
                                                              TestBlockPhysical::
                                                              SIZE).
              unsafe_get0(); ASSERT_TRUE(extent->is_clean());
              ...
              auto extent = get_extent < TestBlockPhysical > (*t,
                                                              ...
                                                              TestBlockPhysical::
                                                              SIZE).
              unsafe_get0(); ASSERT_TRUE(extent->is_dirty());
              ASSERT_TRUE(extent->is_pending());
              ASSERT_EQ(addr, extent->get_paddr());
