...
Inode::~Inode()
{
    ...
    if (!oset.objects.empty()) {
        lsubdout(client->cct, client,
                 0) << __func__ << ": leftover objects on inode 0x" << std::
            hex << ino << std::dec << dendl;
        assert(oset.objects.empty());
    }

    if (!delegations.empty()) {
        lsubdout(client->cct, client,
                 0) << __func__ << ": leftover delegations on inode 0x" << std::
            hex << ino << std::dec << dendl;
...
ostream & operator<<(ostream & out, const Inode & in)
{
    out << in.vino() << "("
        ...
        << " nlink=" << in.nlink
        << " mtime=" << in.mtime << " caps=" << ccap_string(in.caps_issued());
    if (!in.caps.empty()) {
        out << "(";
...
bool Inode::cap_is_valid(Cap * cap) const const
{
...
int Inode::caps_issued(int *implemented) const const
{
    int c = snap_caps;
    int i = 0;
...
void Inode::get()
{
    _ref++;
    lsubdout(client->cct, client,
             15) << "inode.get on " << this << " " << ino << '.' << snapid <<
        " now " << _ref << dendl;
}
...
int Inode::_put(int n)
{
    _ref -= n;
    lsubdout(client->cct, client,
             15) << "inode.put on " << this << " " << ino << '.' << snapid <<
        " now " << _ref << dendl;
    assert(_ref >= 0);
    return _ref;
}

void Inode::dump(Formatter * f) const const
{
    ...
    for (map < mds_rank_t, Cap * >::const_iterator p = caps.begin();
         p != caps.end(); ++p) {
    ...
    if (flushing_caps) {
        ...
        for (map < ceph_tid_t, int >::const_iterator p =
             flushing_cap_tids.begin(); p != flushing_cap_tids.end(); ++p) {
...
void Cap::dump(Formatter * f) const const
{
...
void CapSnap::dump(Formatter * f) const const
{
...
int Inode::set_deleg(Fh * fh, unsigned type, ceph_deleg_cb_t cb, void *priv)
{
    ...
    if (!caps_issued_mask(need)) {
        lsubdout(client->cct, client, 10) << __func__ << ": cap mismatch, have="
            << ccap_string(caps_issued()) << " need=" << ccap_string(need) <<
            dendl;
        return -EAGAIN;
    }
...
{
    lsubdout(client->cct, client,
             10) << __func__ << " " << *this << " " << ccap_string(dirty_caps)
