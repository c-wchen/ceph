...
namespace journal {
    ...
    JournalRecorder::JournalRecorder(librados::IoCtx & ioctx,
                                     ...
                                     const JournalMetadataPtr &
                                     journal_metadata, uint32_t flush_interval,
    ...
    void JournalRecorder::close_and_advance_object_set(uint64_t object_set) {
        ...
        if (m_current_set != object_set) {
            ldout(m_cct,
                  20) << __func__ << ": close already in-progress" << dendl;
            return;
        }
    ...
    void JournalRecorder::advance_object_set() {
        ...
        assert(m_in_flight_object_closes == 0);
        ldout(m_cct,
              20) << __func__ << ": advance to object set " << m_current_set <<
            dendl;
    ...
    ObjectRecorderPtr JournalRecorder::
        create_object_recorder(uint64_t object_number,
                               shared_ptr < Mutex > lock) {
        ObjectRecorderPtr object_recorder(new
                                          ObjectRecorder(m_ioctx,
                                                         utils::
                                                         get_object_name
                                                         (m_object_oid_prefix,
                                                          object_number),
    ...
    void JournalRecorder::
        create_next_object_recorder_unlock(ObjectRecorderPtr object_recorder) {
        assert(m_lock.is_locked());
        ...
        ObjectRecorderPtr new_object_recorder = create_object_recorder((m_current_set * splay_width) + splay_offset, m_object_locks[splay_offset]);
    ...
    void JournalRecorder::handle_closed(ObjectRecorder * object_recorder) {
        ldout(m_cct,
              10) << __func__ << ": " << object_recorder->get_oid() << dendl;
    ...
    void JournalRecorder::handle_overflow(ObjectRecorder * object_recorder) {
        ...
        ldout(m_cct, 20) << __func__ << ": object "
            << active_object_recorder->get_oid() << " overflowed" << dendl;
