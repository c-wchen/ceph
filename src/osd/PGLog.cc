                             ...
                             pg_shard_t from) const const
{
    ...
    for (map < hobject_t, pg_missing_item >::const_iterator i =
        ...
        dout(20) << " before missing " << i->first << " need " << i->second.
            need << " have " << i->second.have << dendl;
    }
                      ...
                      bool & dirty_info, bool & dirty_big_info)
{
    ...
    for (map < hobject_t, pg_missing_item >::const_iterator i =
         missing.get_items().begin(); i != missing.get_items().end(); ++i) {
                                  ...
                                  bool * rebuilt_missing_with_deletes)
{
    _write_log_and_missing(t, km, log, coll, log_oid,
                           ...
                           eversion_t::max(),
                           eversion_t(),
                           eversion_t(), rebuilt_missing_with_deletes, nullptr);
}
...
// static
void PGLog::_write_log_and_missing(ObjectStore::Transaction & t, map < string, bufferlist > *km, pg_log_t & log, const coll_t & coll, const ghobject_t & log_oid, eversion_t dirty_to, eversion_t dirty_from, eversion_t writeout_from, const set < eversion_t > &trimmed, const set < string > &trimmed_dups, const pg_missing_tracker_t & missing, bool touch_log, bool require_rollback, bool clear_divergent_priors, eversion_t dirty_to_dups, eversion_t dirty_from_dups, eversion_t write_from_dups, bool * rebuilt_missing_with_deletes, // in/out param
                                   set < string > *log_keys_debug)
{
    set < string > to_remove(trimmed_dups);
    for (set < eversion_t >::const_iterator i = trimmed.begin();
  ...
  for (const auto & entry:log.dups) {
    ...
    missing.get_changed([&](const hobject_t & obj) {
                        ...
                        uint64_t features =
                        missing.
                        may_include_deletes ? CEPH_FEATURE_OSD_RECOVERY_DELETES
                        : 0;::encode(make_pair(obj, item), (*km)[key],
                                     features);}
                        }
    ) ;
    if (require_rollback) {
