...
struct MonCapGrant {
  ...
  MonCapGrant(string c, string a, StringConstraint co):command(std::
            move(c)) {
        command_args[a] = co;
    }
   ...
   */
    mon_rwxa_t get_allowed(CephContext * cct, int daemon_type,  ///< CEPH_ENTITY_TYPE_*
                           EntityName name,
    ...
    bool is_allow_all() const {
        return
            allow == MON_CAP_ANY &&
            service.length() == 0 &&
            profile.length() == 0 && command.length() == 0;
}};
...
struct MonCap {
   ...
   */
    bool is_capable(CephContext * cct,
                    ...
                    string > &command_args, bool op_may_read, bool op_may_write,
                    bool op_may_exec) const;
