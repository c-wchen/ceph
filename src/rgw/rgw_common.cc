...
bool rgw_err:: is_clear() const const
{
    return (http_ret == 200);
}

bool rgw_err::is_err() const const
{
...
{
    const static string ABS_PREFIXS[] =
    { "http://", "https://", "ws://", "wss://" };
    bool isAbs = false;
    for (int i = 0; i < 4; ++i) {
...
std::ostream &req_state::gen_prefix(std::ostream &out) const const
{
...
bool search_err(rgw_http_errors &errs, int err_no, int &http_ret,
                string &code)
{
                       ...
                       const int prot_flags)
{
    ...
    if (prot_flags & RGW_REST_SWIFT) {
        if (search_err
            (rgw_http_swift_errors, err_no, err.http_ret, err.err_code)) {
            return;
        }
    }
...
void req_info::init_meta_info(const DoutPrefixProvider *dpp,
                              bool *found_bad_meta)
{
    x_meta_map.clear();
    ...
    for (const auto &kv : env->get_map()) {
        ...
        for (int prefix_num = 0;
             (prefix = meta_prefixes[prefix_num].str) != NULL; prefix_num++) {
            ...
            if (strncmp(p, prefix, len) == 0) {
                ...
                snprintf(name_low, meta_prefixes[0].len - 5 + name_len + 1, "%s%s", meta_prefixes[0].str + 5 /* skip HTTP_ */,
    ...
    for (const auto &kv : x_meta_map) {
        ldpp_dout(dpp,
                  10) << "x>> " << kv.
        first << ":" << rgw::crypt_sanitize::x_meta_map {
            kv.first, kv.second} << dendl;
    }
}

std::ostream &operator<<(std::ostream &oss, const rgw_err &err)
{
    oss << "rgw_err(http_ret=" << err.http_ret << ", err_code='" << err.
        err_code << "') ";
    return oss;
}

void rgw_add_amz_meta_header(meta_map_t &x_meta_map,
...
bool rgw_set_amz_meta_header(meta_map_t &x_meta_map,
                             const std::string &k,
                             const std::string &v, rgw_set_action_if_set a)
{
    auto it {
        x_meta_map.find(k)};
    bool r {
...
int parse_key_value(string &in_str, const char *delim, string &key,
                    string &val)
{
    if (delim == NULL) {
        return -EINVAL;
...
sha256_digest_t calc_hash_sha256(const std::string_view &msg)
{
    ...
    SHA256 hasher;
    hasher.Update(reinterpret_cast < const unsigned char *>(msg.data()),
                  msg.size());
    hasher.Final(hash.v);
...
string calc_hash_sha256_close_stream(SHA256 **phash)
{
    ...
    buf_to_hex((unsigned char *)hash_sha256, CEPH_CRYPTO_SHA256_DIGESTSIZE,
               hex_str);
...
{
    if (name.
        compare(0, sizeof(RGW_SYS_PARAM_PREFIX) - 1,
                RGW_SYS_PARAM_PREFIX) == 0) {
    ...
    if ((name.compare("acl") == 0) ||
        ...
        if ((name.compare("response-content-type") == 0) ||
            ...
            has_resp_modifier = true;
               ...
               (name.compare("list") == 0) ||
               (name.compare("object") == 0) || (name.compare("sync") == 0)) {
...
const string &RGWHTTPArgs::get(const string &name, bool *exists) const const
{
...
boost::optional < const std::string & > RGWHTTPArgs::get_optional(const std::string &name) const const
{
    bool exists;
...
int RGWHTTPArgs::get_bool(const string &name, bool *val, bool *exists) const const
{
...
int RGWHTTPArgs::get_bool(const char *name, bool *val, bool *exists) const const
{
    string s(name);
    return get_bool(s, val, exists);
}

void RGWHTTPArgs::get_bool(const char *name, bool *val, bool def_val) const const
{
    bool exists = false;
...
int RGWHTTPArgs::get_int(const char *name, int *val, int def_val) const const
{
    bool exists = false;
    string val_str;
...
string RGWHTTPArgs::sys_get(const string &name, bool *const exists) constconst {
...
struct perm_state_from_req_state: public perm_state_base {
    ...
    perm_state_from_req_state(req_state *const _s)
        : perm_state_base(_s->cct,
                          ...
                          _s->bucket.get() ? _s->bucket->
                          get_info() : RGWBucketInfo(), _s->perm_mask,
                          _s->defer_to_bucket_acls, _s->bucket_access_conf),
          s(_s)
    {
    } std::optional < bool > get_request_payer() const override
    {
        const char *request_payer =
            s->info.env->get("HTTP_X_AMZ_REQUEST_PAYER");
        if (!request_payer) {
            bool exists;
            request_payer =
                s->info.args.get("x-amz-request-payer", &exists).c_str();
            if (!exists) {
                return false;
            }
        }
...
Effect eval_or_pass(const DoutPrefixProvider *dpp,
                    ...
                    const ARN &resource,
                    boost::optional <
                    rgw::IAM::PolicyPrincipal & > princ_type = boost::none)
{
    if (!policy) {
        return Effect::Pass;
    } else {
...
Effect eval_identity_or_session_policies(const DoutPrefixProvider *dpp,
        ...
        const rgw::IAM::Environment &env,
        const uint64_t op, const ARN &arn)
{
...
bool verify_user_permission(const DoutPrefixProvider *dpp,
                            ...
                            const rgw::ARN &res,
                            const uint64_t op, bool mandatory_policy)
{
    auto identity_policy_res =
        eval_identity_or_session_policies(dpp, user_policies, s->env, op, res);
    ...
    if (!session_policies.empty()) {
        auto session_policy_res =
            eval_identity_or_session_policies(dpp, session_policies, s->env, op,
                                              res);
                            ...
                            const uint64_t op, bool mandatory_policy)
{
    ...
    return verify_user_permission(dpp, &ps, s->user_acl.get(),
                                  s->iam_user_policies, s->session_policies,
                                  res, op, mandatory_policy);
}

bool verify_user_permission_no_policy(const DoutPrefixProvider *dpp,
                                      req_state *const s, const int perm)
{
    perm_state_from_req_state ps(s);
                              ...
                              const uint64_t op)
{
    ...
    auto identity_policy_res =
        eval_identity_or_session_policies(dpp, identity_policies, s->env, op,
                                          ARN(bucket));
    if (identity_policy_res == Effect::Deny) {
        return false;
    }
    ...
    if (!session_policies.empty()) {
        ...
        if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
            ...
            if ((session_policy_res == Effect::Allow
                 && identity_policy_res == Effect::Allow) || r == Effect::Allow) {
                return true;
            }
    ...
    return verify_bucket_permission_no_policy(dpp, s, user_acl, bucket_acl,
            perm);
}
...
bool verify_bucket_permission_no_policy(const DoutPrefixProvider *dpp,
                                        ...
                                        const bucket_acl, const int perm)
{
    if (!bucket_acl) {
        return false;
    }
...
{
    auto identity_policy_res =
        eval_identity_or_session_policies(s, s->iam_user_policies, s->env, op,
    ...
    if (!s->session_policies.empty()) {
        auto session_policy_res =
            eval_identity_or_session_policies(s, s->session_policies, s->env,
                                              op,
                                              ARN(s->bucket->get_key()));
        if (session_policy_res == Effect::Deny) {
            return -EACCES;
        ...
        if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
            ...
            //Intersection of session policy and identity policy plus bucket policy
            if ((session_policy_res == Effect::Allow
                 && identity_policy_res == Effect::Allow) || e == Effect::Allow) {
                return 0;
            }
        } else if (princ_type == rgw::IAM::PolicyPrincipal::Other) { // there was no match in the bucket policy
            if (session_policy_res == Effect::Allow
        ...
        const uint64_t op)
{
    return (s->defer_to_bucket_acls == deferred_check
            && verify_bucket_permission(dpp, s, bucket, user_acl, bucket_acl,
                                        bucket_policy, identity_policies,
                                        session_policies, op));
}

static inline bool check_deferred_bucket_only_acl(const DoutPrefixProvider *
        dpp,
        struct perm_state_base *const
        s,
        RGWAccessControlPolicy *
        const user_acl,
        RGWAccessControlPolicy *
...
{
    return (s->defer_to_bucket_acls == deferred_check
            && verify_bucket_permission_no_policy(dpp, s, user_acl, bucket_acl,
                    perm));
}

bool verify_object_permission(const DoutPrefixProvider *dpp,
                              struct perm_state_base *const s,
                              const rgw_obj &obj,
                              RGWAccessControlPolicy *const user_acl,
                              RGWAccessControlPolicy *const bucket_acl,
                              ...
                              const uint64_t op)
{
    ...
    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    auto r =
        eval_or_pass(dpp, bucket_policy, s->env, *s->identity, op, ARN(obj),
                     princ_type);
    if (r == Effect::Deny) {
        return false;
    }
    ...
    if (check_deferred_bucket_perms
        (dpp, s, obj.bucket, user_acl, bucket_acl, bucket_policy,
         identity_policies, session_policies, RGW_DEFER_TO_BUCKET_ACLS_RECURSE,
         op)
        || check_deferred_bucket_perms(dpp, s, obj.bucket, user_acl, bucket_acl,
                                       bucket_policy, identity_policies,
                                       session_policies,
    ...
    return user_acl->verify_permission(dpp, *s->identity, swift_perm,
                                       swift_perm);
}

bool verify_object_permission(const DoutPrefixProvider *dpp,
                              req_state *const s, const rgw_obj &obj,
                              RGWAccessControlPolicy *const user_acl,
                              RGWAccessControlPolicy *const bucket_acl,
                              RGWAccessControlPolicy *const object_acl,
                              const boost::optional < Policy > &bucket_policy,
...
bool verify_object_permission_no_policy(const DoutPrefixProvider *dpp,
                                        ...
                                        const bucket_acl,
                                        RGWAccessControlPolicy *
                                        const object_acl, const int perm)
{
    if (check_deferred_bucket_only_acl
        (dpp, s, user_acl, bucket_acl, RGW_DEFER_TO_BUCKET_ACLS_RECURSE, perm)
        || check_deferred_bucket_only_acl(dpp, s, user_acl, bucket_acl,
                                          RGW_DEFER_TO_BUCKET_ACLS_FULL_CONTROL,
                                        ...
                                        req_state *s, int perm)
{
    ...
    return verify_object_permission_no_policy(dpp,
            ...
            s->user_acl.get(),
            s->bucket_acl.get(),
            s->object_acl.get(), perm);
}

bool verify_object_permission(const DoutPrefixProvider *dpp, req_state *s,
                              uint64_t op)
{
    perm_state_from_req_state ps(s);
    ...
    return verify_object_permission(dpp,
                                    &ps,
                                    rgw_obj(s->bucket->get_key(),
                                            s->object->get_key()),
                                    s->user_acl.get(), s->bucket_acl.get(),
                                    s->object_acl.get(), s->iam_policy,
                                    s->iam_user_policies, s->session_policies,
                                    op);
}

int verify_object_lock(const DoutPrefixProvider *dpp,
                       const rgw::sal::Attrs &attrs, const bool bypass_perm,
                       const bool bypass_governance_mode)
{
    ...
    if (aiter != attrs.end()) {
        ...
        if (ceph::real_clock::to_time_t(obj_retention.get_retain_until_date()) >
            ceph_clock_now()) {
            if (obj_retention.get_mode().compare("GOVERNANCE") != 0
...
void RGWUserCaps::dump(Formatter *f) const const
{
    dump(f, "caps");
}

void RGWUserCaps::dump(Formatter *f, const char *name) const const
{
...
int RGWUserCaps::check_cap(const string &cap, uint32_t perm) const const
{
...
string rgw_pool::to_str() const const
{
    string esc_name;
                  ...
                  uint32_t flag)
{
    ...
    while (true) {
        auto cur_pos_input =
            colonblocks ? input.find(":", last_pos_input) : npos;
...
void RGWBucketInfo::encode(bufferlist &bl) const const
{
...
void RGWBucketInfo::decode(bufferlist::const_iterator &bl)
{
    ...
    if (layout.logs.empty() &&
        layout.current_index.layout.type == rgw::BucketIndexType::Normal) {
        layout.logs.
        push_back(rgw::log_layout_from_index(0, layout.current_index));
    }
    DECODE_FINISH(bl);
}
...
bool RGWBucketInfo::empty_sync_policy() const const
{
    if (!sync_policy) {
        return true;
    }
...
void RGWBucketEnt::generate_test_instances(list < RGWBucketEnt * > &o)
{
    ...
    init_bucket(&e->bucket, "tenant", "bucket", "pool", ".index_pool", "marker",
                "10");
    e->size = 1024;
    e->size_rounded = 4096;
    e->count = 1;
...
void RGWBucketEnt::dump(Formatter *f) const const
{
...
void rgw_bucket_placement::dump(Formatter *f) const const
{
    encode_json("bucket", bucket, f);
...
void RGWBucketInfo::generate_test_instances(list < RGWBucketInfo * > &o)
{
    ...
    auto gen_layout = [](rgw::BucketLayout & l) {
        ...
        l.logs.push_back(log_layout_from_index(l.current_index.gen,
                                               l.current_index));
    };

    RGWBucketInfo *i = new RGWBucketInfo;
    init_bucket(&i->bucket, "tenant", "bucket", "pool", ".index_pool", "marker",
                "10");
    i->owner = "owner";
...
void RGWBucketInfo::dump(Formatter *f) const const
{
    encode_json("bucket", bucket, f);
    utime_t ut(creation_time);
    ...
    encode_json("bi_shard_hash_type",
...
void RGWBucketInfo::decode_json(JSONObj *obj)
{
    ...
    JSONDecoder::decode_json("num_shards",
...
static void user_info_dump_subuser(const char *name, const RGWSubUser &subuser,
                                   Formatter *f, void *parent)
{
    RGWUserInfo *info = static_cast < RGWUserInfo * >(parent);
    subuser.dump(f, info->user_id.to_str());
}

static void user_info_dump_key(const char *name, const RGWAccessKey &key,
                               Formatter *f, void *parent)
{
    RGWUserInfo *info = static_cast < RGWUserInfo * >(parent);
...
static void user_info_dump_swift_key(const char *name, const RGWAccessKey &key,
                                     Formatter *f, void *parent)
{
    RGWUserInfo *info = static_cast < RGWUserInfo * >(parent);
...
void RGWRateLimitInfo::dump(Formatter *f) const const
{
...
void RGWUserInfo::dump(Formatter *f) const const
{
    ...
    encode_json_map("subusers", NULL, "subuser", NULL, user_info_dump_subuser,
                    (void *)this, subusers, f);
    encode_json_map("keys", NULL, "key", NULL, user_info_dump_key, (void *)this,
                    access_keys, f);
    encode_json_map("swift_keys", NULL, "key", NULL, user_info_dump_swift_key,
                    (void *)this, swift_keys, f);
...
void RGWUserInfo::decode_json(JSONObj *obj)
{
    ...
    JSONDecoder::decode_json("default_storage_class",
                             default_placement.storage_class, obj);
...
void RGWSubUser::dump(Formatter *f) const const
{
...
void RGWSubUser::dump(Formatter *f, const string &user) const const
{
    string s = user;
    s.append(":");
    s.append(name);
...
void RGWAccessKey::dump(Formatter *f) const const
{
...
void RGWAccessKey::dump_plain(Formatter *f) const const
{
    encode_json("access_key", id, f);
    encode_json("secret_key", key, f);
}

void RGWAccessKey::dump(Formatter *f, const string &user, bool swift) const const
{
    string u = user;
    if (!subuser.empty()) {
        u.append(":");
...
void RGWStorageStats::dump(Formatter *f) const const
{
    encode_json("size", size, f);
...
void rgw_obj_key::dump(Formatter *f) const const
{
...
void rgw_raw_obj::dump(Formatter *f) const const
{
...
void rgw_obj::dump(Formatter *f) const const
{
    encode_json("bucket", bucket, f);
...
int rgw_bucket_parse_bucket_instance(const string &bucket_instance,
                                     string *bucket_name, string *bucket_id,
                                     int *shard_id)
{
