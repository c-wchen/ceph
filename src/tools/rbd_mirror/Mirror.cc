...
#define dout_prefix *_dout << "rbd::mirror::PriCache: " << this << " " \
            ...
            struct PriCache:public PriorityCache::PriCache {
                ...
                PriorityCache::Priority m_base_cache_pri =
                    PriorityCache::Priority::PRI10;
                PriorityCache::Priority m_extra_cache_pri =
                    PriorityCache::Priority::PRI10;
                ...
                PriCache(const std::string & name, uint64_t min_size,
                         uint64_t max_size)
                :m_name(name), m_base_cache_max_size(min_size),
                    ...
                    m_base_cache_pri =
                        static_cast < PriorityCache::Priority > (--pri);
                ...
                int64_t get_cache_bytes(PriorityCache::
                                        Priority pri) const override {
                    int64_t cache_bytes = 0;
                ...
                int64_t get_cache_bytes() const override {
                    ...
                    return cache_bytes;
                } void set_cache_bytes(PriorityCache::Priority pri,
                                       int64_t bytes) override {
                    ceph_assert(bytes >= 0);
                    ceph_assert(pri == m_base_cache_pri
                                || pri == m_extra_cache_pri || bytes == 0);

                    dout(30) << "pri=" << pri << " " << bytes << dendl;

                    if (pri == m_base_cache_pri) {
                        m_base_cache_bytes =
                            std::min(m_base_cache_max_size, bytes);
                ...
                void add_cache_bytes(PriorityCache::Priority pri,
                                     int64_t bytes) override {
                    ceph_assert(bytes >= 0);
                    ceph_assert(pri == m_base_cache_pri
                                || pri == m_extra_cache_pri);

                    dout(30) << "pri=" << pri << " " << bytes << dendl;

                    if (pri == m_base_cache_pri) {
                        ceph_assert(m_base_cache_bytes <=
                                    m_base_cache_max_size);

                        auto chunk =
                            std::min(m_base_cache_max_size - m_base_cache_bytes,
                                     bytes);
                ...
                int64_t commit_cache_size(uint64_t total_cache) override {
                    m_committed_bytes =
                        p2roundup < int64_t > (get_cache_bytes(), 4096);
                ...
                void import_bins(const std::vector < uint64_t >
                                 &intervals) override {
                }

                void set_bins(PriorityCache::Priority pri,
                              uint64_t end_interval) override {
                }
...
#define dout_prefix *_dout << "rbd::mirror::Mirror: " << this << " " \
        ...
        class MirrorAdminSocketHook:public AdminSocketHook {
          public:
            MirrorAdminSocketHook(CephContext * cct,
                                  Mirror *
                                  mirror):admin_socket(cct->
                                                       get_admin_socket()) {
                std::string command;
                int r;
          ...
          private:
            typedef std::map < std::string, MirrorAdminSocketCommand *,
                std::less <>> Commands;
        ...
        class CacheManagerHandler:public journal::CacheManagerHandler {
          public:
            ...
            :m_cct(cct) {
                ...
                if (!m_cct->_conf.get_val < bool >
                    ("rbd_mirror_memory_autotune")) {
                    return;
                } uint64_t base =
                    m_cct->_conf.get_val < Option::size_t >
                    ("rbd_mirror_memory_base");
                double fragmentation =
                    m_cct->_conf.get_val <
                    double >("rbd_mirror_memory_expected_fragmentation");
                uint64_t target =
                    m_cct->_conf.get_val < Option::size_t >
                ...
                m_cache_manager =
                    std::make_unique < PriorityCache::Manager > (m_cct, min,
                                                                 max, target,
                                                                 false);
            }
            ...
            void register_cache(const std::string & cache_name,
                ...
                auto p = m_caches.insert({ cache_name,
                                         {cache_name, min_size, max_size,
                                          handler}
                                         }
                );
                ceph_assert(p.second == true);

                m_cache_manager->insert(cache_name, p.first->second.pri_cache,
                                        false);
                m_next_balance = ceph_clock_now();
            }
            ...
            void run_cache_manager() {
                ...
                auto autotune_interval =
                    m_cct->_conf.get_val <
                    double >("rbd_mirror_memory_cache_autotune_interval");
                auto resize_interval =
                    m_cct->_conf.get_val <
                    double >("rbd_mirror_memory_cache_resize_interval");
                ...
                if (autotune_interval > 0 && m_next_balance <= now) {
                  ...
                  for (auto & it:m_caches) {
                        auto pri_cache =
                            static_cast <
                            PriCache * >(it.second.pri_cache.get());
          ...
          private:
            struct Cache {
                ...
                Cache(const std::string name, uint64_t min_size,
                      uint64_t max_size,
                      journal::CacheRebalanceHandler * handler)
        ...
        Mirror::Mirror(CephContext * cct,
                       const std::vector < const char *>&args):m_cct(cct),
            m_args(args), m_local(new librados::Rados()),
        ...
        int Mirror::init() {
            ...
            m_threads = &(m_cct->lookup_or_create_singleton_object <
                          Threads < librbd::ImageCtx >> ("rbd_mirror::threads",
                                                         false, m_local));
            m_service_daemon.
                reset(new ServiceDaemon <> (m_cct, m_local, m_threads));

            r = m_service_daemon->init();
            if (r < 0) {
                derr << "error registering service daemon: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
        ...
        void Mirror::run() {
            ...
            while (!m_stopping) {
                ...
                if (!m_manual_stop) {
                    if (refresh_pools) {
                        update_pool_replayers(m_local_cluster_watcher->
                                              get_pool_peers(),
        ...
        void Mirror::update_pool_replayers(const PoolPeers & pool_peers,
            ...
            for (auto it = m_pool_replayers.begin();
                 it != m_pool_replayers.end();) {
                auto & peer = it->first.second;
                auto pool_peer_it = pool_peers.find(it->first.first);
                if (pool_peer_it == pool_peers.end() ||
                    pool_peer_it->second.find(peer) ==
                    pool_peer_it->second.end()) {
              ...
              for (auto & peer:kv.second) {
                    ...
                    if (pool_replayers_it != m_pool_replayers.end()) {
                        ...
                        if (!m_site_name.empty() && !site_name.empty() &&
                            ...
                            dout(0) << "restarting pool replayer for " << peer
                                << " due to " << "updated site name" << dendl;
                            // TODO: make async
                            ...
                            derr << "restarting failed pool replayer for " <<
                                peer << dendl;
                            // TODO: make async
                            pool_replayer->shut_down();
                            pool_replayer->init(site_name);
                        ...
                        dout(20) << "starting pool replayer for " << peer <<
                            dendl;
                        unique_ptr <
                            PoolReplayer <>> pool_replayer(new PoolReplayer <>
                                                           (m_threads,
                                                            m_service_daemon.
                                                            get(),
                        ...
                        // TODO: make async
                        pool_replayer->init(site_name);
                        m_pool_replayers.emplace(pool_peer,
