...
void DecayCounter::encode(ceph::buffer::list & bl) const const
{
    decay();
    ENCODE_START(5, 4, bl);
    encode(val, bl);
...
void DecayCounter::dump(ceph::Formatter * f) const const
{
    decay();
...
void DecayCounter::decay(double delta) const const
{
    auto now = clock::now();
