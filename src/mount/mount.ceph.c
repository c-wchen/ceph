 ...
 */
static int remove_opt(struct ceph_mount_info *cmi, const char *key,
                      char **value)
{
                         ...
                         int *opt_pos)
{
    ...
    do {
        ...
        if (strcmp(data, "ro") == 0) {
            ...
            ret =
                read_secret_from_file(value, cmi->cmi_secret,
                                      sizeof(cmi->cmi_secret));
            if (ret < 0) {
...
static int parse_arguments(int argc, char *const *const argv,
                           const char **src, const char **node,
                           const char **opts)
{
    int opt = 0;
...
static int call_mount_system_call(const char *rsrc, const char *node,
...
static int mount_new_device_format(const char *node,
                                   struct ceph_mount_info *cmi)
{
    int r;
    char *rsrc = NULL;
...
static int mount_old_device_format(const char *node,
                                   struct ceph_mount_info *cmi)
{
    int r;
    int len = 0;
                    ...
                    struct ceph_mount_info *cmi)
{
    ...
    if (cmi->format == MOUNT_DEV_FORMAT_NEW) {
        ...
        if (retval)
            fallback = (should_fallback() && retval == -EINVAL
                        && cmi->cmi_fsid);
    }
    ...
    if (!retval && !skip_mtab_flag) {
        update_mtab_entry(dev, node, "ceph", cmi->cmi_opts, cmi->cmi_flags, 0,
                          0);
    }
        ...
        if (ret < 0) {
            if (ret == -ENODEV || ret == -ENOSYS) {
                ...
                pos = safe_cat(&cmi->cmi_opts,
                               &cmi->cmi_opts_len, pos, "secret=");
                pos = safe_cat(&cmi->cmi_opts,
                               &cmi->cmi_opts_len, pos, cmi->cmi_secret);
                return 0;
            }
