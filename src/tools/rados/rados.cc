                             ...
                             std::vector < const char *>&nargs)
{
    ...
    if (oloc.size()) {
        if (!pool_name) {
            cerr << "pool name must be specified with --object_locator" << std::
                endl;
            ret = -1;
            goto out;
        }
    ...
    if (strcmp(nargs[0], "lspools") == 0) {
        ...
        if (*endptr) {
            cerr << "Invalid value for new-auid: '" << nargs[1] << "'" << std::
                endl;
            ret = -1;
            goto out;
        }
        ...
        if (ret < 0) {
            cerr << "error changing auid on pool " << io_ctx.
                get_pool_name() << ':' << cpp_strerror(ret) << std::endl;
        }
        else
            cerr << "changed auid on pool " << io_ctx.get_pool_name()
        ...
        if (ret < 0) {
            cerr << "mapext error on " << pool_name << "/" << oid << ": " <<
                cpp_strerror(ret) << std::endl;
            goto out;
        }
        ...
        if (ret < 0) {
            cerr << "error getting " << pool_name << "/" << nargs[1] << ": " <<
                cpp_strerror(ret) << std::endl;
            goto out;
        }
    }
        ...
        if (!pool_name || nargs.size() < 3)
            usage_exit();
        ret =
            do_put(io_ctx, striper, nargs[1], nargs[2], op_size, obj_offset,
                   use_striper);
        if (ret < 0) {
            cerr << "error putting " << pool_name << "/" << nargs[1] << ": " <<
                cpp_strerror(ret) << std::endl;
            goto out;
        }
    }
        ...
        if (!pool_name || nargs.size() < 3)
            usage_exit();
        ret =
            do_append(io_ctx, striper, nargs[1], nargs[2], op_size,
                      use_striper);
        if (ret < 0) {
            cerr << "error appending " << pool_name << "/" << nargs[1] << ": "
                << cpp_strerror(ret) << std::endl;
            goto out;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error setting xattr " << pool_name << "/" << oid << "/" <<
        ...
        if (ret < 0) {
            cerr << "error getting xattr " << pool_name << "/" << oid << "/" <<
                attr_name << ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
        else
        ...
        if (ret < 0) {
            cerr << "error removing xattr " << pool_name << "/" << oid << "/" <<
                attr_name << ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error getting xattr set " << pool_name << "/" << oid <<
                ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
        ...
        if (ret < 0) {
            cerr << "error setting omap value " << pool_name << "/" << oid <<
                "/" << omap_key_pretty << ": " << cpp_strerror(ret) << std::
                endl;
            goto out;
        }
        else {
        ...
        if (ret < 0) {
            cerr << "error creating " << pool_name << "/" << oid << ": " <<
                cpp_strerror(ret) << std::endl;
            goto out;
        }
    }
        ...
        if (strcmp(nargs[1], "dump") == 0) {
            ...
            if (ret < 0) {
                cerr << "error reading " << pool_name << "/" << oid << ": " <<
                    cpp_strerror(ret) << std::endl;
                goto out;
            }
            ...
            try {
                ...
                cerr << "error decoding tmap " << pool_name << "/" << oid <<
                    std::endl;
                ret = -EINVAL;
                goto out;
            }
            ...
            cout << "\n";
            cout << kv.size() << " keys\n";
            for (map < string, bufferlist >::iterator q = kv.begin();
                 q != kv.end(); ++q) {
                cout << "key '" << q->first << "' (" << q->second.
                    length() << " bytes):\n";
            ...
            char c =
                (strcmp(nargs[1], "set") ==
                 0) ? CEPH_OSD_TMAP_SET : CEPH_OSD_TMAP_CREATE;
            ::encode(c, bl);
            ::encode(k, bl);
            ::encode(v, bl);
        ...
        try {
            ...
            cerr << "error decoding tmap " << pool_name << "/" << oid << std::
                endl;
            ret = -EINVAL;
            goto out;
        }
        if (!p.end()) {
            cerr << "error decoding tmap (stray trailing data) in " << pool_name
                << "/" << oid << std::endl;
            ret = -EINVAL;
            goto out;
        }
        ...
        if (r < 0) {
            ret = r;
            cerr << "error writing tmap data as omap on " << pool_name << "/" <<
                oid << ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
        ret = 0;
    }
        ...
        if (nargs.size() > 2) {
            ...
            if (*endptr) {
                cerr << "Invalid value for auid: '" << nargs[2] << "'" << std::
                    endl;
                ret = -EINVAL;
                goto out;
            }
            ...
            if (nargs.size() > 3) {
                ...
                if (*endptr) {
                    cerr << "Invalid value for crush-rule: '" << nargs[3] << "'"
                        << std::endl;
                    ret = -EINVAL;
                    goto out;
                }
        ...
        ret =
            io_ctx.set_alloc_hint(oid, expected_object_size,
                                  expected_write_size);
        if (ret < 0) {
        ...
        if (ret < 0) {
            cerr << "error listing snap shots " << pool_name << "/" << oid <<
                ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
        else
