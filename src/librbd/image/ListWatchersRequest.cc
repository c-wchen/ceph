...
{
    ceph_assert((m_flags & LIST_WATCHERS_FILTER_OUT_MIRROR_INSTANCES) ==
                0
                || (m_flags & LIST_WATCHERS_MIRROR_INSTANCES_ONLY) ==
                0);
...
void ListWatchersRequest < I >::handle_list_image_watchers(int r)
{
    ...
    ldout(m_cct,
          20) << "object_watchers=" << m_object_watchers << dendl;
...
void ListWatchersRequest < I >::handle_list_mirror_watchers(int r)
{
    ...
    ldout(m_cct,
          20) << "mirror_watchers=" << m_mirror_watchers << dendl;
    finish(0);
}
    ...
    if (r == 0) {
        ...
        if (m_object_watchers.size() > 0) {
            ...
            uint64_t watch_handle =
                ...
                nullptr ? m_image_ctx.image_watcher->
                get_watch_handle() : 0;

            for (auto &w : m_object_watchers) {
                if ((m_flags & LIST_WATCHERS_FILTER_OUT_MY_INSTANCE) !=
                    0) {
                    if (w.cookie == watch_handle) {
                        ldout(m_cct,
                              20) << "filtering out my instance: " << w
                                  << dendl;
                        continue;
                    }
                }
                ...
                if ((m_flags &
                     LIST_WATCHERS_FILTER_OUT_MIRROR_INSTANCES) != 0) {
                    if (it != m_mirror_watchers.end()) {
                        ldout(m_cct,
                              20) << "filtering out mirror instance: "
                                  << w << dendl;
                        continue;
                    }
                } else if ((m_flags & LIST_WATCHERS_MIRROR_INSTANCES_ONLY)
                           != 0) {
                    if (it == m_mirror_watchers.end()) {
                        ldout(m_cct,
                              20) <<
                                  "filtering out non-mirror instance: " << w
                                  << dendl;
                        continue;
                    }
                }
