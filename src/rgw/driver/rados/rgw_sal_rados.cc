...
// (use marker to bridge between calls)
    ...
    static int decode_policy(CephContext * cct,
                             bufferlist & bl, RGWAccessControlPolicy * policy) {
        auto iter = bl.cbegin();
        try {
            policy->decode(iter);
    ...
    static int rgw_op_get_bucket_policy_from_attr(const DoutPrefixProvider *
                                                  dpp, RadosStore * store,
                                                  User * user,
                                                  Attrs & bucket_attrs,
                                                  RGWAccessControlPolicy *
                                                  policy, optional_yield y) {
    ...
    int RadosUser::list_buckets(const DoutPrefixProvider * dpp,
        ...
        buckets.clear();
        ret =
            store->ctl()->user->list_buckets(dpp, info.user_id, marker,
                                             end_marker, max, need_stats,
                                             &ulist, &is_truncated, y);
        if (ret < 0)
            return ret;

        buckets.set_truncated(is_truncated);
      for (const auto & ent:ulist.get_buckets()) {
            buckets.add(std::unique_ptr < Bucket >
                        (new RadosBucket(this->store, ent.second, this)));
        }
    ...
    int RadosUser::create_bucket(const DoutPrefixProvider * dpp,
        ...
        if (ret != -ENOENT) {
            ...
            *existed = true;
            ...
            int r =
                rgw_op_get_bucket_policy_from_attr(dpp, store, this,
                                                   bucket->get_attrs(),
                                                   &old_policy, y);
            ...
            bucket =
                std::unique_ptr < Bucket > (new RadosBucket(store, b, this));
            *existed = false;
        ...
        if (!store->svc()->zone->is_meta_master()) {
            JSONParser jp;
            ret =
                store->forward_request_to_master(dpp, this, NULL, in_data, &jp,
                                                 req_info, y);
            if (ret < 0) {
                return ret;
            }
            ...
            ldpp_dout(dpp,
                ...
                tag << " objv.ver=" << objv.ver << dendl;
            std::time_t ctime =
                ceph::real_clock::to_time_t(master_info.creation_time);
            ldpp_dout(dpp,
                      20) << "got creation time: << " << std::put_time(std::
                                                                       localtime
                                                                       (&ctime),
                                                                       "%F %T")
                << dendl;
            ...
            pmaster_num_shards =
                &master_info.layout.current_index.layout.normal.num_shards;
            pobjv = &objv;
        ...
        if (*existed) {
            ...
            ret =
                store->svc()->zone->select_bucket_placement(dpp,
                                                            this->get_info(),
                                                            zid, placement_rule,
            ...
            ret =
                store->getRados()->create_bucket(this->get_info(),
                                                 bucket->get_key(), zid,
                                                 placement_rule,
                                                 swift_ver_location,
                                                 pquota_info, attrs, info,
                                                 pobjv, &ep_objv, creation_time,
                                                 pmaster_bucket,
                                                 pmaster_num_shards, y, dpp,
                                                 exclusive);
            if (ret == -EEXIST) {
                *existed = true;
    ...
    int RadosUser::read_attrs(const DoutPrefixProvider * dpp, optional_yield y) {
        return store->ctl()->user->get_attrs_by_uid(dpp, get_id(), &attrs, y,
                                                    &objv_tracker);
    }

    int RadosUser::merge_and_store_attrs(const DoutPrefixProvider * dpp,
    ...
    int RadosUser::read_stats(const DoutPrefixProvider * dpp,
        ...
        return store->ctl()->user->read_stats(dpp, get_id(), stats, y,
                                              last_stats_sync,
                                              last_stats_update);
    }

    int RadosUser::read_stats_async(const DoutPrefixProvider * dpp,
                                    RGWGetUserStats_CB * cb) {
        return store->svc()->user->read_stats_async(dpp, get_id(), cb);
    }

    int RadosUser::complete_flush_stats(const DoutPrefixProvider * dpp,
                                        optional_yield y) {
        return store->svc()->user->complete_flush_stats(dpp, get_id(), y);
    }

    int RadosUser::read_usage(const DoutPrefixProvider * dpp,
                              uint64_t start_epoch, uint64_t end_epoch,
                              uint32_t max_entries, bool * is_truncated,
                              RGWUsageIter & usage_iter, map < rgw_user_bucket,
                              rgw_usage_log_entry > &usage) {
        std::string bucket_name;
        return store->getRados()->read_usage(dpp, get_id(), bucket_name,
                                             start_epoch, end_epoch,
                                             max_entries, is_truncated,
                                             usage_iter, usage);
    }

    int RadosUser::trim_usage(const DoutPrefixProvider * dpp,
                              uint64_t start_epoch, uint64_t end_epoch) {
        std::string bucket_name;

        return store->getRados()->trim_usage(dpp, get_id(), bucket_name,
                                             start_epoch, end_epoch);
    }
    ...
    int RadosUser::store_user(const DoutPrefixProvider * dpp, optional_yield y,
                              ...
                              const DoutPrefixProvider * dpp, optional_yield y)
    {
        ...
        if (i == info.mfa_ids.end()) {
            ldpp_dout(dpp,
                      5) << "NOTICE: user does not have mfa device with serial="
                << serial << dendl;
            return -EACCES;
        }

        int ret =
            store->svc()->cls->mfa.check_mfa(dpp, info.user_id, serial, pin, y);
        if (ret < 0) {
            ldpp_dout(dpp,
    ...
    int RadosBucket::remove_bucket(const DoutPrefixProvider * dpp,
        ...
        do {
            ...
            if (!results.objs.empty() && !delete_children) {
                ldpp_dout(dpp,
                          -1) << "ERROR: could not remove non-empty bucket " <<
        ...
        ret =
            store->ctl()->bucket->sync_user_stats(dpp, info.owner, info, y,
                                                  nullptr);
        if (ret < 0) {
            ldout(store->ctx(),
                  1) <<
                "WARNING: failed sync user stats before bucket delete. ret=" <<
                ret << dendl;
        }
        ...
        ret =
            store->getRados()->delete_bucket(info, ot, y, dpp,
                                             !delete_children);
        if (ret < 0) {
        ...
        if (ps_ret < 0 && ps_ret != -ENOENT) {
            ldpp_dout(dpp,
                      -1) <<
                "ERROR: unable to remove notifications from bucket. ret=" <<
                ps_ret << dendl;
        }

        ret =
            store->ctl()->bucket->unlink_bucket(info.owner, info.bucket, y, dpp,
                                                false);
        if (ret < 0) {
            ldpp_dout(dpp,
                      -1) << "ERROR: unable to remove user bucket information"
                << dendl;
        }
        ...
        if (forward_to_master) {
            bufferlist in_data;
            ret =
                store->forward_request_to_master(dpp, owner, &ot.read_version,
                                                 in_data, nullptr, *req_info,
                                                 y);
            if (ret < 0) {
                if (ret == -ENOENT) {
    ...
    int RadosBucket::remove_bucket_bypass_gc(int concurrent_max, bool
        ...
        ret =
            read_stats(dpp, index, RGW_NO_SHARD, &bucket_ver, &master_ver,
                       stats, NULL);
        if (ret < 0)
            return ret;
        ...
        while (results.is_truncated) {
            ...
            std::vector < rgw_bucket_dir_entry >::iterator it =
                results.objs.begin();
            for (; it != results.objs.end(); ++it) {
                ...
                ret =
                    store->getRados()->get_obj_state(dpp, &obj_ctx, get_info(),
                                                     obj, &astate, &amanifest,
                                                     false, y);
                if (ret == -ENOENT) {
                    ldpp_dout(dpp,
                              1) << "WARNING: cannot find obj state for obj " <<
                        obj << dendl;
                    continue;
                }
                if (ret < 0) {
                    ldpp_dout(dpp,
                              -1) << "ERROR: get obj state returned with error "
                        << ret << dendl;
                    return ret;
                }
                ...
                if (amanifest) {
                    ...
                    RGWObjManifest::obj_iterator miter =
                        manifest.obj_begin(dpp);
                    const rgw_obj head_obj = manifest.get_obj();
                    rgw_raw_obj raw_head_obj;
                    store->get_raw_obj(manifest.get_head_placement_rule(),
                                       head_obj, &raw_head_obj);
                    ...
                    for (; miter != manifest.obj_end(dpp) && max_aio--; ++miter) {
                        ...
                        ret =
                            store->getRados()->delete_raw_obj_aio(dpp, last_obj,
                                                                  handles);
                        if (ret < 0) {
                            ldpp_dout(dpp,
                                      -1) <<
                                "ERROR: delete obj aio failed with " << ret <<
                                dendl;
                            return ret;
                        }
                    }           // for all shadow objs
                    ...
                    ret =
                        store->getRados()->delete_obj_aio(dpp, head_obj,
                                                          get_info(), astate,
                                                          handles,
                                                          keep_index_consistent,
                                                          y);
                    if (ret < 0) {
                        ldpp_dout(dpp,
                                  -1) << "ERROR: delete obj aio failed with " <<
                            ret << dendl;
                        return ret;
                    }
                }
    ...
    int RadosBucket::load_bucket(const DoutPrefixProvider * dpp,
        ...
        if (info.bucket.bucket_id.empty()) {
            ret =
                store->ctl()->bucket->read_bucket_info(info.bucket, &info, y,
                                                       dpp,
                                                       RGWBucketCtl::
                                                       BucketInstance::
                                                       GetParams()
            ...
            ret =
                store->ctl()->bucket->read_bucket_instance_info(info.bucket,
                                                                &info, y, dpp,
                                                                RGWBucketCtl::
                                                                BucketInstance::
                                                                GetParams()
        ...
        if (get_stats) {
            ret =
                store->ctl()->bucket->read_bucket_stats(info.bucket, &ent, y,
                                                        dpp);
        }
    ...
    int RadosBucket::read_stats(const DoutPrefixProvider * dpp,
                                ...
                                std::map < RGWObjCategory,
                                RGWStorageStats > &stats,
                                std::string * max_marker, bool * syncstopped) {
        return store->getRados()->get_bucket_stats(dpp, info, idx_layout,
                                                   shard_id, bucket_ver,
                                                   master_ver, stats,
    ...
    int RadosBucket::read_stats_async(const DoutPrefixProvider * dpp,
        ...
        return store->getRados()->get_bucket_stats_async(dpp, get_info(),
                                                         idx_layout, shard_id,
                                                         ctx);
    }

    int RadosBucket::sync_user_stats(const DoutPrefixProvider * dpp,
                                     optional_yield y) {
        return store->ctl()->bucket->sync_user_stats(dpp, owner->get_id(), info,
                                                     y, &ent);
    }
    ...
    int RadosBucket::update_container_stats(const DoutPrefixProvider * dpp) {
        ...
        map < std::string, RGWBucketEnt >::iterator iter =
            m.find(info.bucket.name);
        if (iter == m.end())
            return -EINVAL;
    ...
    int RadosBucket::check_bucket_shards(const DoutPrefixProvider * dpp) {
        return store->getRados()->check_bucket_shards(info, info.bucket,
                                                      get_count(), dpp);
    }

    int RadosBucket::link(const DoutPrefixProvider * dpp, User * new_user,
                          optional_yield y, bool update_entrypoint,
        ...
        int r =
            store->ctl()->bucket->link_bucket(new_user->get_id(), info.bucket,
                                              get_creation_time(), y, dpp,
                                              update_entrypoint,
                                              &ep_data);
        if (r < 0)
            return r;
    ...
    int RadosBucket::unlink(const DoutPrefixProvider * dpp, User * new_user,
                            optional_yield y, bool update_entrypoint) {
        return store->ctl()->bucket->unlink_bucket(new_user->get_id(),
                                                   info.bucket, y, dpp,
                                                   update_entrypoint);
    }

    int RadosBucket::chown(const DoutPrefixProvider * dpp, User & new_user,
                           optional_yield y) {
        ...
        if (!owner) {
            ldpp_dout(dpp,
                      0) << __func__ << " Cannot chown without an owner " <<
                dendl;
            return -EINVAL;
        }
    ...
    int RadosBucket::put_info(const DoutPrefixProvider * dpp, bool exclusive,
        ...
        mtime = _mtime;
        return store->getRados()->put_bucket_instance_info(info, exclusive,
                                                           mtime, &attrs, dpp,
                                                           null_yield);
    }
...
/* Make sure to call get_bucket_info() if you need it first */
    ...
    int RadosBucket::check_empty(const DoutPrefixProvider * dpp,
                                 optional_yield y) {
        return store->getRados()->check_bucket_empty(dpp, info, y);
    }

    int RadosBucket::check_quota(const DoutPrefixProvider * dpp,
                                 RGWQuota & quota, uint64_t obj_size,
                                 optional_yield y, bool check_size_only) {
        return store->getRados()->check_quota(dpp, info.owner, get_key(), quota,
                                              obj_size, y, check_size_only);
    }

    int RadosBucket::merge_and_store_attrs(const DoutPrefixProvider * dpp,
                                           Attrs & new_attrs,
                                           optional_yield y) {
      for (auto & it:new_attrs) {
            attrs[it.first] = it.second;
    ...
    int RadosBucket::try_refresh_info(const DoutPrefixProvider * dpp,
                                      ceph::real_time * pmtime) {
        return store->getRados()->try_refresh_bucket_info(info, pmtime, dpp,
                                                          &attrs);
    }

    int RadosBucket::read_usage(const DoutPrefixProvider * dpp,
                                uint64_t start_epoch, uint64_t end_epoch,
                                uint32_t max_entries, bool * is_truncated,
                                RGWUsageIter & usage_iter,
                                map < rgw_user_bucket,
                                rgw_usage_log_entry > &usage) {
        return store->getRados()->read_usage(dpp, owner->get_id(), get_name(),
                                             start_epoch, end_epoch,
                                             max_entries, is_truncated,
                                             usage_iter, usage);
    }

    int RadosBucket::trim_usage(const DoutPrefixProvider * dpp,
                                uint64_t start_epoch, uint64_t end_epoch) {
        return store->getRados()->trim_usage(dpp, owner->get_id(), get_name(),
                                             start_epoch, end_epoch);
    }

    int RadosBucket::remove_objs_from_index(const DoutPrefixProvider * dpp,
                                            std::list < rgw_obj_index_key >
                                            &objs_to_unlink) {
        return store->getRados()->remove_objs_from_index(dpp, info,
                                                         objs_to_unlink);
    }

    int RadosBucket::check_index(const DoutPrefixProvider * dpp,
                                 std::map < RGWObjCategory,
                                 RGWStorageStats > &existing_stats,
                                 std::map < RGWObjCategory,
                                 RGWStorageStats > &calculated_stats) {
        return store->getRados()->bucket_check_index(dpp, info, &existing_stats,
                                                     &calculated_stats);
    }
    ...
    int RadosBucket::set_tag_timeout(const DoutPrefixProvider * dpp,
                                     uint64_t timeout) {
        return store->getRados()->cls_obj_set_bucket_tag_timeout(dpp, info,
                                                                 timeout);
    }
    ...
    int RadosBucket::purge_instance(const DoutPrefixProvider * dpp) {
        int max_shards =
            (info.layout.current_index.layout.normal.num_shards >
             0 ? info.layout.current_index.layout.normal.num_shards : 1);
        for (int i = 0; i < max_shards; i++) {
            ...
            if (ret < 0) {
                cerr << "ERROR: bs.init(bucket=" << info.
                    bucket << ", shard=" << shard_id << "): " <<
    ...
    int RadosBucket::set_acl(const DoutPrefixProvider * dpp,
    ...
    int RadosBucket::list(const DoutPrefixProvider * dpp, ListParams & params,
        ...
        int ret =
            list_op.list_objects(dpp, max, &results.objs,
    ...
    std::unique_ptr < MultipartUpload >
        RadosBucket::get_multipart_upload(const std::string & oid,
                                          std::optional < std::string >
                                          upload_id, ACLOwner owner,
                                          ceph::real_time mtime) {
        return std::make_unique < RadosMultipartUpload > (this->store, this,
                                                          oid, upload_id,
                                                          std::move(owner),
                                                          mtime);
    }
    ...
    int RadosBucket::list_multiparts(const DoutPrefixProvider * dpp,
                                     ...
                                     MultipartUpload >> &uploads, map < string,
    ...
    int RadosBucket::abort_multiparts(const DoutPrefixProvider * dpp,
        ...
        do {
            ...
            ldpp_dout(dpp, 20) << __func__ <<
                ...
                << this << "\"; uploads.size()=" << uploads.
                size() << "; is_truncated=" << is_truncated << dendl;
                    ...
                    if (ret < 0) {
                        ...
                        if (ret != -ENOENT && ret != -ERR_NO_SUCH_UPLOAD) {
                            ...
                            ldpp_dout(dpp, 10) << __func__ <<
                                ...
                                "aborted multipart upload of \"" << upload->
                                get_meta() << "\" for cleaning up" << dendl;
                        }
                    }
                    num_deleted++;
                }
    ...
    std::string RadosBucket::topics_oid()const {
        return pubsub_oid_prefix + get_tenant() + ".bucket." + get_name() +
            "/" + get_marker();
        ...
        try {
            ...
            ldpp_dout(dpp,
                      20) << " failed to decode bucket notifications from oid: "
                << topics_oid() << ". for bucket: " << get_name() << ". error: "
                << err.what() << dendl;
            return -EIO;
        }
    ...
    int RadosBucket::
        write_topics(const rgw_pubsub_bucket_topics & notifications,
        ...
        return rgw_put_system_obj(dpp, store->svc()->sysobj,
                                  ...
                                  log_pool, topics_oid(), bl, false,
                                  objv_tracker, real_time(), y);
    }
    ...
    int RadosBucket::remove_topics(RGWObjVersionTracker * objv_tracker,
        ...
        return rgw_delete_system_obj(dpp, store->svc()->sysobj,
                                     store->svc()->zone->get_zone_params().
                                     log_pool, topics_oid(), objv_tracker, y);
    }
    ...
    std::string RadosStore::get_cluster_id(const DoutPrefixProvider * dpp,
                                           optional_yield y) {
        return getRados()->get_cluster_fsid(dpp, y);
    }

    int RadosStore::get_user_by_access_key(const DoutPrefixProvider * dpp,
        ...
        int r =
            ctl()->user->get_info_by_access_key(dpp, key, &uinfo, y,
                                                RGWUserCtl::GetParams().
                                                set_objv_tracker
                                                (&objv_tracker));
        if (r < 0)
    ...
    int RadosStore::get_user_by_email(const DoutPrefixProvider * dpp,
        ...
        int r =
            ctl()->user->get_info_by_email(dpp, email, &uinfo, y,
                                           RGWUserCtl::GetParams().
                                           set_objv_tracker(&objv_tracker));
        if (r < 0)
            return r;
    ...
    int RadosStore::get_user_by_swift(const DoutPrefixProvider * dpp,
        ...
        int r =
            ctl()->user->get_info_by_swift(dpp, user_str, &uinfo, y,
                                           RGWUserCtl::GetParams().
                                           set_objv_tracker(&objv_tracker));
        if (r < 0)
            return r;
    ...
    int RadosStore::get_bucket(User * u, const RGWBucketInfo & i,
                               std::unique_ptr < Bucket > *bucket) {
        Bucket *bp;
    ...
    int RadosStore::forward_request_to_master(const DoutPrefixProvider * dpp,
...
#define MAX_REST_RESPONSE (128 * 1024)  // we expect a very small response
        int ret =
            svc()->zone->get_master_conn()->forward(dpp, rgw_user(uid_str),
                                                    info,
        ...
        if (jp && !jp->parse(response.c_str(), response.length())) {
            ldpp_dout(dpp,
                      0) << "failed parsing response from master zonegroup" <<
                dendl;
            return -EINVAL;
        }
    ...
    int RadosStore::forward_iam_request_to_master(const DoutPrefixProvider *
                                                  dpp, const RGWAccessKey & key,
                                                  obj_version * objv,
                                                  bufferlist & in_data,
                                                  RGWXMLDecoder::XMLParser *
                                                  parser, req_info & info,
                                                  optional_yield y) {
        if (is_meta_master()) {
            /* We're master, don't forward */
            return 0;
...
#define MAX_REST_RESPONSE (128 * 1024)  // we expect a very small response
        int ret =
            svc()->zone->get_master_conn()->forward_iam_request(dpp, key, info,
                                                                objv,
                                                                MAX_REST_RESPONSE,
                                                                &in_data,
                                                                &response, y);
        ...
        if (parser && !parser->parse(r.c_str(), r.length(), 1)) {
            ldpp_dout(dpp,
                      0) <<
                "ERROR: failed to parse response from master zonegroup" <<
                dendl;
            return -EIO;
        }
    ...
    int RadosStore::list_all_zones(const DoutPrefixProvider * dpp,
    ...
    std::unique_ptr < Notification >
        RadosStore::get_notification(rgw::sal::Object * obj,
                                     rgw::sal::Object * src_obj, req_state * s,
                                     rgw::notify::EventType event_type,
                                     optional_yield y,
                                     const std::string * object_name) {
        return std::make_unique < RadosNotification > (s, this, obj, src_obj, s,
                                                       event_type, y,
                                                       object_name);
    }

    std::unique_ptr < Notification >
        RadosStore::get_notification(const DoutPrefixProvider * dpp,
                                     rgw::sal::Object * obj,
    ...
    std::string RadosStore::topics_oid(const std::string & tenant)const {
        ...
        try {
            ...
            ldpp_dout(dpp,
                      20) << " failed to decode topics from oid: " <<
                topics_oid(tenant) << ". error: " << err.what() << dendl;
            return -EIO;
        }
    ...
    int RadosStore::write_topics(const std::string & tenant,
    ...
    int RadosStore::remove_topics(const std::string & tenant,
                                  RGWObjVersionTracker * objv_tracker,
                                  optional_yield y,
                                  const DoutPrefixProvider * dpp) {
    ...
    int RadosStore::delete_raw_obj(const DoutPrefixProvider * dpp,
                                   const rgw_raw_obj & obj) {
        return rados->delete_raw_obj(dpp, obj);
    }

    int RadosStore::delete_raw_obj_aio(const DoutPrefixProvider * dpp,
                                       const rgw_raw_obj & obj,
                                       Completions * aio) {
    ...
    void RadosStore::get_raw_obj(const rgw_placement_rule & placement_rule,
                                 const rgw_obj & obj, rgw_raw_obj * raw_obj) {
        rados->obj_to_raw(placement_rule, obj, raw_obj);
    }

    int RadosStore::get_raw_chunk_size(const DoutPrefixProvider * dpp,
                                       const rgw_raw_obj & obj,
                                       uint64_t * chunk_size) {
        return rados->get_max_chunk_size(obj.pool, chunk_size, dpp);
    }

    int RadosStore::initialize(CephContext * cct,
                               const DoutPrefixProvider * dpp) {
        std::unique_ptr < ZoneGroup > zg =
            std::make_unique < RadosZoneGroup > (this,
                                                 svc()->zone->get_zonegroup());
        zone = make_unique < RadosZone > (this, std::move(zg));
        return 0;
    }

    int RadosStore::log_usage(const DoutPrefixProvider * dpp,
                              map < rgw_user_bucket,
                              RGWUsageBatch > &usage_info) {
        return rados->log_usage(dpp, usage_info);
    }

    int RadosStore::log_op(const DoutPrefixProvider * dpp, std::string & oid,
                           bufferlist & bl) {
    ...
    int RadosStore::register_to_service_map(const DoutPrefixProvider * dpp,
    ...
    void RadosStore::get_ratelimit(RGWRateLimitInfo & bucket_ratelimit,
                                   RGWRateLimitInfo & user_ratelimit,
                                   RGWRateLimitInfo & anon_ratelimit) {
        bucket_ratelimit =
            svc()->zone->get_current_period().get_config().bucket_ratelimit;
        user_ratelimit =
            svc()->zone->get_current_period().get_config().user_ratelimit;
        anon_ratelimit =
            svc()->zone->get_current_period().get_config().anon_ratelimit;
    }

    int RadosStore::set_buckets_enabled(const DoutPrefixProvider * dpp,
    ...
    int RadosStore::read_all_usage(const DoutPrefixProvider * dpp,
        ...
        return rados->read_usage(dpp, uid, bucket_name, start_epoch, end_epoch,
                                 max_entries, is_truncated, usage_iter, usage);
    }

    int RadosStore::trim_all_usage(const DoutPrefixProvider * dpp,
                                   uint64_t start_epoch, uint64_t end_epoch) {
        rgw_user uid;
        std::string bucket_name;
    ...
    int RadosStore::meta_list_keys_init(const DoutPrefixProvider * dpp,
    ...
    int RadosStore::meta_list_keys_next(const DoutPrefixProvider * dpp,
                                        void *handle, int max,
                                        list < std::string > &keys,
                                        bool * truncated) {
        return ctl()->meta.mgr->list_keys_next(dpp, handle, max, keys,
                                               truncated);
    }
    ...
    int RadosStore::meta_remove(const DoutPrefixProvider * dpp,
                                std::string & metadata_key, optional_yield y) {
    ...
    std::unique_ptr < RGWRole > RadosStore::get_role(std::string name,
                                                     ...
                                                     std::string,
                                                     std::string > tags) {
        return std::make_unique < RadosRole > (this, name, tenant, path,
                                               trust_policy,
                                               max_session_duration_str, tags);
    }
    ...
    int RadosStore::get_roles(const DoutPrefixProvider * dpp,
        ...
        do {
            ...
            int r =
                rados->list_raw_objects(dpp, pool, prefix, 1000, ctx, oids,
                                        &is_truncated);
            if (r < 0) {
          ...
          for (const auto & iter:oids) {
                result.push_back(iter.
                                 substr(RGWRole::role_path_oid_prefix.size()));
            }
        } while (is_truncated);
      ...
      for (const auto & it:result) {
            ...
            if (path_prefix.empty()
                || path.find(path_prefix) != std::string::npos) {
                //Get id from info oid prefix + id
                std::string id =
                    it.substr(pos + RGWRole::role_oid_prefix.length());
    ...
    int RadosStore::get_oidc_providers(const DoutPrefixProvider * dpp,
        ...
        do {
            ...
            if (r < 0) {
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: listing filtered objects failed: OIDC pool: " <<
                    pool.
                    name << ": " << prefix << ": " << cpp_strerror(-r) << dendl;
                return r;
            }
          for (const auto & iter:oids) {
                ...
                r = rgw_get_system_obj(svc()->sysobj, pool, iter, bl, nullptr,
    ...
    std::unique_ptr < Writer >
        RadosStore::get_append_writer(const DoutPrefixProvider * dpp,
        ...
        auto aio =
            rgw::make_throttle(ctx()->_conf->rgw_put_obj_min_window_size, y);
        return std::make_unique < RadosAppendWriter > (dpp, y, bucket_info,
                                                       obj_ctx, obj->get_obj(),
                                                       this, std::move(aio),
                                                       owner,
                                                       ptail_placement_rule,
    ...
    std::unique_ptr < Writer >
        RadosStore::get_atomic_writer(const DoutPrefixProvider * dpp,
                                      optional_yield y, rgw::sal::Object * obj,
                                      const rgw_user & owner,
                                      const rgw_placement_rule *
                                      ptail_placement_rule, uint64_t olh_epoch,
                                      const std::string & unique_tag) {
        RGWBucketInfo & bucket_info = obj->get_bucket()->get_info();
        RGWObjectCtx & obj_ctx = static_cast < RadosObject * >(obj)->get_ctx();
        auto aio =
            rgw::make_throttle(ctx()->_conf->rgw_put_obj_min_window_size, y);
        return std::make_unique < RadosAtomicWriter > (dpp, y, bucket_info,
                                                       obj_ctx, obj->get_obj(),
                                                       this, std::move(aio),
                                                       owner,
                                                       ptail_placement_rule,
    ...
    const std::string & RadosStore::
        get_compression_type(const rgw_placement_rule & rule) {
    ...
    int RadosStore::get_obj_head_ioctx(const DoutPrefixProvider * dpp,
    ...
    int RadosObject::get_obj_state(const DoutPrefixProvider * dpp,
                                   RGWObjState ** pstate, optional_yield y,
                                   bool follow_olh) {
        int ret =
            store->getRados()->get_obj_state(dpp, rados_ctx, bucket->get_info(),
                                             get_obj(), pstate, &manifest,
                                             follow_olh, y);
        if (ret < 0) {
            return ret;
        }
    ...
    int RadosObject::read_attrs(const DoutPrefixProvider * dpp,
    ...
    int RadosObject::set_obj_attrs(const DoutPrefixProvider * dpp,
        ...
        return store->getRados()->set_attrs(dpp, rados_ctx,
                                            ...
                                            delattrs ? delattrs : nullptr, y);
    }

    int RadosObject::get_obj_attrs(optional_yield y,
                                   const DoutPrefixProvider * dpp,
                                   rgw_obj * target_obj) {
        RGWRados::Object op_target(store->getRados(), bucket->get_info(),
                                   *rados_ctx, get_obj());
    ...
    int RadosObject::modify_obj_attrs(const char *attr_name,
    ...
    int RadosObject::delete_obj_attrs(const DoutPrefixProvider * dpp,
    ...
    bool RadosObject::is_expired() {
        ...
        try {
            ...
            ldout(store->ctx(),
                  0) << "ERROR: " << __func__ << ": failed to decode "
                RGW_ATTR_DELETE_AT " attr" << dendl;
            return false;
        }
    ...
    void RadosObject::raw_obj_to_obj(const rgw_raw_obj & raw_obj) {
        ...
        RGWSI_Tier_RADOS::raw_obj_to_obj(get_bucket()->get_key(), raw_obj,
                                         &tobj);
        set_key(tobj.key);
    }

    void RadosObject::get_raw_obj(rgw_raw_obj * raw_obj) {
        store->getRados()->obj_to_raw((bucket->get_info()).placement_rule,
                                      get_obj(), raw_obj);
    }

    int RadosObject::omap_get_vals(const DoutPrefixProvider * dpp,
    ...
    int RadosObject::omap_get_all(const DoutPrefixProvider * dpp,
                                  std::map < std::string, bufferlist > *m,
                                  optional_yield y) {
        rgw_raw_obj raw_obj;
        get_raw_obj(&raw_obj);
    ...
    int RadosObject::omap_get_vals_by_keys(const DoutPrefixProvider * dpp,
        ...
        store->getRados()->obj_to_raw(bucket->get_placement_rule(), obj,
                                      &head_obj);
        ret =
            store->get_obj_head_ioctx(dpp, bucket->get_info(), obj, &cur_ioctx);
        if (ret < 0) {
            return ret;
        }
    ...
    int RadosObject::omap_set_val_by_key(const DoutPrefixProvider * dpp,
                                         const std::string & key,
    ...
    int RadosObject::chown(User & new_user, const DoutPrefixProvider * dpp,
                           optional_yield y) {
        int r = get_obj_attrs(y, dpp);
        if (r < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to read object attrs " << get_name()
        ...
        if (aiter == get_attrs().end()) {
            ldpp_dout(dpp,
                      0) << "ERROR: no acls found for object " << get_name() <<
                dendl;
            return -EINVAL;
        }
        ...
        ACLGrant grant;
        grant.set_canon(new_user.get_id(), new_user.get_display_name(),
    ...
    std::unique_ptr < MPSerializer >
        RadosObject::get_serializer(const DoutPrefixProvider * dpp,
                                    const std::string & lock_name) {
        return std::make_unique < MPRadosSerializer > (dpp, store, this,
                                                       lock_name);
    }
    ...
    int RadosObject::transition(Bucket * bucket,
                                ...
                                const DoutPrefixProvider * dpp,
                                optional_yield y) {
        return store->getRados()->transition_obj(*rados_ctx, bucket->get_info(),
                                                 get_obj(), placement_rule,
                                                 mtime, olh_epoch, dpp, y);
    }
    ...
    int RadosObject::transition_to_cloud(Bucket * bucket,
        ...
        ldpp_dout(dpp,
            ...
            key << ") to the cloud endpoint(" << endpoint << ")" << dendl;
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                ...
                key << ") to the cloud endpoint(" << endpoint << ") ret=" << ret
                << dendl;
            return ret;
        }
        ...
        if (update_object) {
            ...
            if (read_mtime != tier_ctx.o.meta.mtime) {
                ...
                ldpp_dout(dpp,
                          0) << "ERROR: Updating tier object(" << o.
                    key << ") failed ret=" << -ECANCELED << dendl;
                return -ECANCELED;
            }
    ...
    int RadosObject::write_cloud_tier(const DoutPrefixProvider * dpp,
        ...
        pmanifest->set_tail_placement(target_placement,
                                      head_obj->get_obj().bucket);
    ...
    int RadosObject::get_max_chunk_size(const DoutPrefixProvider * dpp,
                                        rgw_placement_rule placement_rule,
                                        uint64_t * max_chunk_size,
                                        uint64_t * alignment) {
        return store->getRados()->get_max_chunk_size(placement_rule, get_obj(),
                                                     max_chunk_size, dpp,
                                                     alignment);
    }
    ...
    bool RadosObject::placement_rules_match(rgw_placement_rule & r1,
    ...
    int RadosObject::dump_obj_layout(const DoutPrefixProvider * dpp,
                                     optional_yield y, Formatter * f) {
        int ret;
        RGWObjManifest *amanifest {
        nullptr};
        rgw_raw_obj head_obj;
        ...
        for (auto miter = amanifest->obj_begin(dpp);
             miter != amanifest->obj_end(dpp); ++miter) {
  ...
  RadosObject::RadosReadOp::RadosReadOp(RadosObject * _source, RGWObjectCtx * _rctx):
    source(_source),
        ...
        op_target(_source->store->getRados(),
                  ...
                  *static_cast < RGWObjectCtx * >(rctx),
                  _source->get_obj()), parent_op(&op_target) {
    }

    int RadosObject::RadosReadOp::prepare(optional_yield y,
                                          const DoutPrefixProvider * dpp) {
        uint64_t obj_size;
    ...
    int RadosObject::RadosReadOp::read(int64_t ofs, int64_t end,
                                       bufferlist & bl, optional_yield y,
                                       const DoutPrefixProvider * dpp) {
        return parent_op.read(ofs, end, bl, y, dpp);
    }

    int RadosObject::RadosReadOp::get_attr(const DoutPrefixProvider * dpp,
  ...
  RadosObject::RadosDeleteOp::RadosDeleteOp(RadosObject * _source):
    ...
    int RadosObject::RadosDeleteOp::delete_obj(const DoutPrefixProvider * dpp,
                                               optional_yield y) {
    ...
    int RadosObject::delete_object(const DoutPrefixProvider * dpp,
        ...
        RGWRados::Object del_target(store->getRados(), bucket->get_info(),
                                    *rados_ctx, get_obj());
    ...
    int RadosObject::delete_obj_aio(const DoutPrefixProvider * dpp,
                                    RGWObjState * astate, Completions * aio,
                                    bool keep_index_consistent,
                                    optional_yield y) {
        RadosCompletions *raio = static_cast < RadosCompletions * >(aio);

        return store->getRados()->delete_obj_aio(dpp, get_obj(),
                                                 bucket->get_info(), astate,
                                                 raio->handles,
                                                 keep_index_consistent, y);
    }
    ...
    int RadosObject::copy_object(User * user,
        ...
        return store->getRados()->copy_obj(*rados_ctx,
                                           ...
                                           if_match,
                                           if_nomatch,
                                           static_cast < RGWRados::AttrsMod >
                                           (attrs_mod), copy_if_newer, attrs,
                                           category, olh_epoch,
                                           (delete_at ? *delete_at :
                                            real_time()), version_id, tag, etag,
                                           progress_cb, progress_data, dpp, y);
    }

    int RadosObject::RadosReadOp::iterate(const DoutPrefixProvider * dpp,
    ...
    int RadosObject::swift_versioning_copy(const DoutPrefixProvider * dpp,
                                           optional_yield y) {
    ...
    int RadosMultipartUpload::cleanup_part_history(const DoutPrefixProvider *
                                                   dpp, optional_yield y,
      ...
      for (auto & ppfx:part->get_past_prefixes()) {
            rgw_obj past_obj;
            past_obj.init_ns(bucket->get_key(),
                             ppfx + "." + std::to_string(part->info.num),
                             mp_ns);
            ...
            for (; miter != manifest.obj_end(dpp); ++miter) {
                ...
                chain.push_obj(raw_part_obj.pool.to_str(), part_key,
                               raw_part_obj.loc);
            }
        }
        if (store->getRados()->get_gc() == nullptr) {
            // Delete objects inline if gc hasn't been initialised (in case when bypass gc is specified)
            store->getRados()->delete_objs_inline(dpp, chain,
                                                  mp_obj.get_upload_id());
        }
        else {
            // use upload id as tag and do it synchronously
            auto[ret, leftover_chain] =
                store->getRados()->send_chain_to_gc(chain,
                                                    mp_obj.get_upload_id());
            if (ret < 0 && leftover_chain) {
                ldpp_dout(dpp,
                          5) << __func__ << ": gc->send_chain() returned " <<
                    ret << dendl;
                if (ret == -ENOENT) {
                    return -ERR_NO_SUCH_UPLOAD;
                }
                // Delete objects inline if send chain to gc fails
                store->getRados()->delete_objs_inline(dpp, *leftover_chain,
                                                      mp_obj.get_upload_id());
            }
        }
        return 0;
    ...
    int RadosMultipartUpload::abort(const DoutPrefixProvider * dpp,
                                    CephContext * cct) {
        ...
        do {
            ...
            if (ret < 0) {
                ldpp_dout(dpp,
                          20) << __func__ <<
                    ": RadosMultipartUpload::list_parts returned " << ret <<
                    dendl;
            ...
            for (auto part_it = parts.begin();
                ...
                RadosMultipartPart *obj_part =
                    dynamic_cast <
                    RadosMultipartPart * >(part_it->second.get());
                if (obj_part->info.manifest.empty()) {
                    std::unique_ptr < rgw::sal::Object > obj =
                        bucket->
                        get_object(rgw_obj_key
                                   (obj_part->oid, std::string(),
                    ...
                    store->getRados()->update_gc_chain(dpp, target,
                                                       obj_part->info.manifest,
                                                       &chain);
                    ...
                    if (oiter != obj_part->info.manifest.obj_end(dpp)) {
                        ...
                        dynamic_cast <
                            rgw::sal::RadosObject *
                            >(head.get())->raw_obj_to_obj(raw_head);

                        rgw_obj_index_key key;
                        head->get_key().get_index_key(&key);
                        remove_objs.push_back(key);

                        cleanup_part_history(dpp, null_yield, obj_part,
                                             remove_objs);
                    }
                }
    ...
    int RadosMultipartUpload::init(const DoutPrefixProvider * dpp,
        ...
        do {
            ...
            obj =
                bucket->get_object(rgw_obj_key(tmp_obj_name, string(), mp_ns));
    ...
    int RadosMultipartUpload::list_parts(const DoutPrefixProvider * dpp,
                                         CephContext * cct, int num_parts,
                                         int marker, int *next_marker,
                                         bool * truncated,
        ...
        for (i = 0, iter = parts_map.begin();
            ...
            try {
                ...
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: could not part info, caught buffer::error" << dendl;
                return -EIO;
            }
            if (sorted_omap) {
                if (part->info.num != expected_next) {
                     ...
                     */
                    return list_parts(dpp, cct, num_parts, marker, next_marker,
                                      truncated, true);
                }
                expected_next++;
            }
        ...
        if (sorted_omap) {
            ...
            std::map < uint32_t,
                std::unique_ptr < MultipartPart >>::iterator piter;
            for (i = 0, piter = parts.begin();
                 i < num_parts && piter != parts.end(); ++i, ++piter) {
    ...
    int RadosMultipartUpload::complete(const DoutPrefixProvider * dpp,
        ...
        do {
            ...
            if (!truncated && total_parts != (int)part_etags.size()) {
                ldpp_dout(dpp,
                          0) << "NOTICE: total parts mismatch: have: " <<
                    total_parts << " expected: " << part_etags.size() << dendl;
            ...
            for (auto obj_iter = parts.begin();
                ...
                RadosMultipartPart *part =
                    dynamic_cast <
                    rgw::sal::RadosMultipartPart * >(obj_iter->second.get());
                ...
                if (etags_iter->first != (int)obj_iter->first) {
                    ldpp_dout(dpp,
                              0) <<
                        "NOTICE: parts num mismatch: next requested: " <<
                        etags_iter->first << " next uploaded: " << obj_iter->
                        first << dendl;
                    ret = -ERR_INVALID_PART;
                    return ret;
                }
                ...
                if (part_etag.compare(part->get_etag()) != 0) {
                    ldpp_dout(dpp,
                              0) << "NOTICE: etag mismatch: part: " <<
                        etags_iter->first << " etag: " << etags_iter->
                        second << dendl;
                    ret = -ERR_INVALID_PART;
                    return ret;
                }
                ...
                if (obj_part.manifest.empty()) {
                    ldpp_dout(dpp,
                              0) <<
                        "ERROR: empty manifest for object part: obj=" << src_obj
                        << dendl;
                    ret = -ERR_INVALID_PART;
                    return ret;
                }
                else {
                    manifest.append(dpp, obj_part.manifest,
                                    store->svc()->zone->get_zonegroup(),
                                    store->svc()->zone->get_zone_params());
                    ...
                    if (not manifest_prefix.empty()) {
                        ...
                        src_obj.init_ns(bucket->get_key(),
                                        manifest_prefix + "." +
                                        std::to_string(part->info.num), mp_ns);
                    }
                }

                bool part_compressed =
                    (obj_part.cs_info.compression_type != "none");
                if ((handled_parts > 0)
                    ...
                    ldpp_dout(dpp,
                        ...
                        << cs_info.compression_type << ">>" << obj_part.cs_info.
                ...
                if (part_compressed) {
                    ...
                    if (cs_info.blocks.size() > 0)
                        new_ofs =
                            ...
                            cs_info.blocks.back().len;
                    else
                        new_ofs = 0;
                  for (const auto & block:obj_part.cs_info.
                         blocks) {
        ...
        buf_to_hex((unsigned char *)final_etag, sizeof(final_etag),
                   final_etag_str);
        ...
        RGWRados::Object op_target(store->getRados(),
                                   ...
                                   dynamic_cast <
                                   RadosObject * >(target_obj)->get_ctx(),
                                   target_obj->get_obj());
        RGWRados::Object::Write obj_op(&op_target);
    ...
    int RadosMultipartUpload::get_info(const DoutPrefixProvider * dpp,
                                       optional_yield y,
        ...
        ret =
            read_op->read(0, store->ctx()->_conf->rgw_max_chunk_size, headbl, y,
                          dpp);
        if (ret < 0) {
            if (ret == -ENOENT) {
                return -ERR_NO_SUCH_UPLOAD;
        ...
        try {
            ...
            ldpp_dout(dpp,
                      0) << "ERROR: failed to decode multipart upload info" <<
                dendl;
            return -EIO;
        }
    ...
    std::unique_ptr < Writer >
        RadosMultipartUpload::get_writer(const DoutPrefixProvider * dpp,
                                         optional_yield y,
                                         rgw::sal::Object * obj,
                                         const rgw_user & owner,
                                         const rgw_placement_rule *
        ...
        RGWObjectCtx & obj_ctx = static_cast < RadosObject * >(obj)->get_ctx();
        auto aio =
            rgw::make_throttle(store->ctx()->_conf->rgw_put_obj_min_window_size,
                               y);
        return std::make_unique < RadosMultipartWriter > (dpp, y,
                                                          get_upload_id(),
                                                          bucket_info, obj_ctx,
                                                          obj->get_obj(), store,
                                                          std::move(aio), owner,
  ...
  MPRadosSerializer::MPRadosSerializer(const DoutPrefixProvider * dpp, RadosStore * store, RadosObject * obj, const std::string & lock_name):
    lock(lock_name)
    {
        ...
        oid = raw_obj.oid;
        store->getRados()->get_obj_data_pool(obj->get_bucket()->
                                             get_placement_rule(),
                                             obj->get_obj(), &meta_pool);
        store->getRados()->open_pool_ctx(dpp, meta_pool, ioctx, true, true);
    }

    int MPRadosSerializer::try_lock(const DoutPrefixProvider * dpp, utime_t dur,
                                    optional_yield y) {
        op.assert_exists();
  ...
  LCRadosSerializer::LCRadosSerializer(RadosStore * store, const std::string & _oid, const std::string & lock_name, const std::string & cookie):
    ...
    int LCRadosSerializer::try_lock(const DoutPrefixProvider * dpp, utime_t dur,
                                    optional_yield y) {
        lock.set_duration(dur);
        return lock.lock_exclusive(ioctx, oid);
    }

    int RadosLifecycle::get_entry(const std::string & oid,
                                  const std::string & marker,
                                  std::unique_ptr < LCEntry > *entry) {
        cls_rgw_lc_entry cls_entry;
        int ret =
            cls_rgw_lc_get_entry(*store->getRados()->get_lc_pool_ctx(), oid,
                                 marker, cls_entry);
        if (ret)
            return ret;

        LCEntry *e;
        e = new StoreLCEntry(cls_entry.bucket, cls_entry.start_time,
                             cls_entry.status);
        if (!e)
            return -ENOMEM;
    ...
    int RadosLifecycle::get_next_entry(const std::string & oid,
                                       const std::string & marker,
                                       std::unique_ptr < LCEntry > *entry) {
        cls_rgw_lc_entry cls_entry;
        int ret =
            cls_rgw_lc_get_next_entry(*store->getRados()->get_lc_pool_ctx(),
                                      oid, marker,
                                      cls_entry);
    ...
    int RadosLifecycle::set_entry(const std::string & oid, LCEntry & entry) {
        ...
        return cls_rgw_lc_set_entry(*store->getRados()->get_lc_pool_ctx(), oid,
                                    cls_entry);
    }

    int RadosLifecycle::list_entries(const std::string & oid,
                                     const std::string & marker,
      ...
      for (auto & entry:cls_entries) {
            entries.push_back(std::make_unique < StoreLCEntry >
                              (entry.bucket, oid, entry.start_time,
                               entry.status));
        }
    ...
    int RadosLifecycle::rm_entry(const std::string & oid, LCEntry & entry) {
        ...
        return cls_rgw_lc_rm_entry(*store->getRados()->get_lc_pool_ctx(), oid,
                                   cls_entry);
    }

    int RadosLifecycle::get_head(const std::string & oid,
                                 std::unique_ptr < LCHead > *head) {
        cls_rgw_lc_obj_head cls_head;
        int ret =
            cls_rgw_lc_get_head(*store->getRados()->get_lc_pool_ctx(), oid,
                                cls_head);
        if (ret)
            return ret;

        LCHead *h;
        h = new StoreLCHead(cls_head.start_date, cls_head.shard_rollover_date,
                            cls_head.marker);
        if (!h)
            return -ENOMEM;
    ...
    int RadosLifecycle::put_head(const std::string & oid, LCHead & head) {
        ...
        return cls_rgw_lc_put_head(*store->getRados()->get_lc_pool_ctx(), oid,
                                   cls_head);
    }

    std::unique_ptr < LCSerializer >
        RadosLifecycle::get_serializer(const std::string & lock_name,
                                       const std::string & oid,
                                       const std::string & cookie) {
        return std::make_unique < LCRadosSerializer > (store, oid, lock_name,
                                                       cookie);
    }

    int RadosNotification::publish_reserve(const DoutPrefixProvider * dpp,
                                           RGWObjTags * obj_tags) {
        return rgw::notify::publish_reserve(dpp, event_type, res, obj_tags);
    }

    int RadosNotification::publish_commit(const DoutPrefixProvider * dpp,
                                          uint64_t size,
                                          const ceph::real_time & mtime,
                                          const std::string & etag,
                                          const std::string & version) {
        return rgw::notify::publish_commit(obj, size, mtime, etag, version,
                                           event_type, res, dpp);
    }
    ...
    int RadosAtomicWriter::complete(size_t accounted_size,
                                    const std::string & etag,
                                    ceph::real_time * mtime,
    ...
    int RadosAppendWriter::complete(size_t accounted_size,
                                    const std::string & etag,
                                    ceph::real_time * mtime,
                                    ceph::real_time set_mtime,
                                    std::map < std::string, bufferlist > &attrs,
    ...
    int RadosMultipartWriter::complete(size_t accounted_size,
                                       const std::string & etag,
                                       ceph::real_time * mtime,
                                       ceph::real_time set_mtime,
                                       std::map < std::string,
    ...
    } int RadosZoneGroup::get_placement_target_names(std::set < std::string >
                                                     &names) const {
    ...
    int RadosZoneGroup::get_placement_tier(const rgw_placement_rule & rule,
                                           std::unique_ptr < PlacementTier >
                                           *tier) {
        std::map < std::string,
            RGWZoneGroupPlacementTarget >::const_iterator titer;
        ...
        std::map < std::string,
            RGWZoneGroupPlacementTier >::const_iterator ttier;
    ...
    int RadosZoneGroup::get_zone_by_id(const std::string & id,
    ...
    int RadosZoneGroup::get_zone_by_name(const std::string & name,
                                         std::unique_ptr < Zone > *zone) {
        rgw_zone_id id;
  ...
  RadosLuaManager::RadosLuaManager(RadosStore * _s):
    store(_s),
        pool((store->svc()
              && store->svc()->zone) ? store->svc()->zone->get_zone_params().
             log_pool : rgw_pool()) {
    }

    int RadosLuaManager::get_script(const DoutPrefixProvider * dpp,
                                    optional_yield y, const std::string & key,
                                    std::string & script) {
        if (pool.empty()) {
            ldpp_dout(dpp,
                      10) << "WARNING: missing pool when reading lua script " <<
                dendl;
            return 0;
        }
        bufferlist bl;
        ...
        int r =
            rgw_get_system_obj(store->svc()->sysobj, pool, key, bl, nullptr,
                               nullptr, y, dpp);
        if (r < 0) {
            return r;
        }
    ...
    int RadosLuaManager::put_script(const DoutPrefixProvider * dpp,
                                    optional_yield y, const std::string & key,
                                    const std::string & script) {
        if (pool.empty()) {
            ldpp_dout(dpp,
                      10) << "WARNING: missing pool when writing lua script " <<
                dendl;
            return 0;
        }
        bufferlist bl;
        ...
        int r =
            rgw_put_system_obj(dpp, store->svc()->sysobj, pool, key, bl, false,
                               nullptr, real_time(), y);
        if (r < 0) {
            return r;
        }
    ...
    int RadosLuaManager::del_script(const DoutPrefixProvider * dpp,
                                    optional_yield y, const std::string & key) {
        if (pool.empty()) {
            ldpp_dout(dpp,
                      10) << "WARNING: missing pool when deleting lua script "
                << dendl;
            return 0;
        }
        int r =
            rgw_delete_system_obj(dpp, store->svc()->sysobj, pool, key, nullptr,
                                  y);
        if (r < 0 && r != -ENOENT) {
            return r;
        }
    ...
    int RadosLuaManager::add_package(const DoutPrefixProvider * dpp,
                                     optional_yield y,
                                     const std::string & package_name) {
        // add package to list
        const bufferlist empty_bl;
        std::map < std::string, bufferlist > new_package { {
        package_name, empty_bl}};
        librados::ObjectWriteOperation op;
        op.omap_set(new_package);
        auto ret =
            rgw_rados_operate(dpp, *(store->getRados()->get_lc_pool_ctx()),
    ...
    int RadosLuaManager::remove_package(const DoutPrefixProvider * dpp,
                                        optional_yield y,
                                        const std::string & package_name) {
        librados::ObjectWriteOperation op;
        ...
        if (pos != package_name.npos) {
            ...
            op.omap_rm_keys(std::set < std::string > ( {
                                                      package_name}
                            ));
            auto ret =
                rgw_rados_operate(dpp, *(store->getRados()->get_lc_pool_ctx()),
      ...
      for (const auto & package:packages) {
            const std::string package_no_version =
                package.substr(0, package.find(" "));
            if (package_no_version.compare(package_name) == 0) {
                ...
                ret =
                    rgw_rados_operate(dpp,
                                      *(store->getRados()->get_lc_pool_ctx()),
                                      PACKAGE_LIST_OBJECT_NAME, &op, y);
                if (ret < 0) {
                    return ret;
                }
    ...
    int RadosLuaManager::list_packages(const DoutPrefixProvider * dpp,
                                       optional_yield y,
                                       rgw::lua::packages_t & packages) {
        ...
        while (more) {
            ...
            op.omap_get_keys2(start_after, max_chunk, &packages_chunk, &more,
                              &rval);
            const auto ret =
                rgw_rados_operate(dpp, *(store->getRados()->get_lc_pool_ctx()),
    ...
    int RadosOIDCProvider::store_url(const DoutPrefixProvider * dpp,
                                     const std::string & url, bool exclusive,
                                     optional_yield y) {
        ...
        using ceph::encode;
        encode(*this, bl);
        return rgw_put_system_obj(dpp, sysobj,
                                  store->svc()->zone->get_zone_params().
                                  oidc_pool, oid, bl, exclusive, nullptr,
                                  real_time(), y);
    }

    int RadosOIDCProvider::read_url(const DoutPrefixProvider * dpp,
        ...
        int ret =
            rgw_get_system_obj(sysobj, pool, oid, bl, nullptr, nullptr,
                               null_yield, dpp);
        if (ret < 0) {
            return ret;
        }
    ...
    int RadosOIDCProvider::delete_obj(const DoutPrefixProvider * dpp,
                                      optional_yield y) {
        auto & pool = store->svc()->zone->get_zone_params().oidc_pool;
        ...
        if (this->tenant != tenant) {
            ldpp_dout(dpp,
                      0) << "ERROR: tenant in arn doesn't match that of user "
                << this->tenant << ", " << tenant << ": " << dendl;
            return -EINVAL;
        }
        ...
        std::string oid = tenant + get_url_oid_prefix() + url;
        ret =
            rgw_delete_system_obj(dpp, store->svc()->sysobj, pool, oid, nullptr,
                                  y);
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: deleting oidc url from pool: " << pool.
                name << ": " << provider_url << ": " << cpp_strerror(-ret) <<
                dendl;
        }
    ...
    int RadosRole::store_info(const DoutPrefixProvider * dpp, bool exclusive,
                              optional_yield y) {
        using ceph::encode;
        std::string oid;
        ...
        if (!this->info.tags.empty()) {
            ...
            std::unique_ptr < RGWSI_MetaBackend::Context >
                ctx(store->svc()->role->svc.meta_be->alloc_ctx());
            ctx->init(store->svc()->role->get_be_handler());
            return store->svc()->role->svc.meta_be->put(ctx.get(), oid, params,
                                                        &info.objv_tracker, y,
                                                        dpp);
        }
        else {
    ...
    int RadosRole::store_name(const DoutPrefixProvider * dpp, bool exclusive,
                              optional_yield y) {
        auto sysobj = store->svc()->sysobj;
        ...
        return rgw_put_system_obj(dpp, sysobj,
                                  store->svc()->zone->get_zone_params().
                                  roles_pool, oid, bl, exclusive,
                                  &info.objv_tracker, real_time(), y);
    }

    int RadosRole::store_path(const DoutPrefixProvider * dpp, bool exclusive,
                              optional_yield y) {
        auto sysobj = store->svc()->sysobj;
        std::string oid =
            info.tenant + get_path_oid_prefix() + info.path +
            get_info_oid_prefix() + info.id;
    ...
    int RadosRole::read_id(const DoutPrefixProvider * dpp,
                           const std::string & role_name,
                           const std::string & tenant, std::string & role_id,
                           optional_yield y) {
        auto sysobj = store->svc()->sysobj;
        ...
        try {
            ...
            ldpp_dout(dpp,
                      0) << "ERROR: failed to decode role from Role pool: " <<
                role_name << dendl;
            return -EIO;
        }
    ...
    int RadosRole::read_name(const DoutPrefixProvider * dpp, optional_yield y) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed reading role name from Role pool: "
        ...
        try {
            ...
            ldpp_dout(dpp,
                      0) << "ERROR: failed to decode role name from Role pool: "
                << info.name << dendl;
            return -EIO;
        }
    ...
    int RadosRole::read_info(const DoutPrefixProvider * dpp, optional_yield y) {
        ...
        ctx->init(store->svc()->role->get_be_handler());
        int ret =
            store->svc()->role->svc.meta_be->get(ctx.get(), oid, params,
                                                 &info.objv_tracker, y, dpp,
                                                 true);
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed reading role info from Role pool: "
        ...
        try {
            ...
            ldpp_dout(dpp,
                      0) << "ERROR: failed to decode role info from Role pool: "
                << info.id << dendl;
            return -EIO;
        }
    ...
    int RadosRole::create(const DoutPrefixProvider * dpp, bool exclusive,
                          const std::string & role_id, optional_yield y) {
        int ret;
        ...
        if (exclusive && ret == 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: name " << info.
                name << " already in use for role id " << info.id << dendl;
            return -EEXIST;
        }
        ...
        //arn
        info.arn =
            role_arn_prefix + info.tenant + ":role" + info.path + info.name;
        ...
        if (ret < 0) {
            ...
            int info_ret =
                rgw_delete_system_obj(dpp, store->svc()->sysobj, pool, oid,
                                      nullptr, y);
            if (info_ret < 0) {
                ldpp_dout(dpp,
