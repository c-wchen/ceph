    ...
    } void finish(int r) override
    {
        ...
        if (m_comp != NULL) {
            if (m_finisher != NULL) {
                ...
                queue(new
                      LambdaContext(std::
                                    bind(&finish_aio_completion, m_comp,
                                         ret)));
            } else {
...
{
    return config_keys;
} void TestRadosClient::handle_conf_change(const ConfigProxy &conf,
        ...
        if ((*j_it)->get_data() == "osd tier add") {
            ...
            if (!op_it.end()
                ...
                auto addr_it =
                    ...
                    find("addr");
                return
                    blocklist_add((*addr_it)->get_data(), expire);
            }
        }
    }
    return -ENOSYS;
