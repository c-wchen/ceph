...
public:
                       ...
                       int r)
    {
        ...
        std::string oid(ObjectMap <>::
                        object_map_name(ictx->id, snap_id));
        if (snap_id == CEPH_NOSNAP) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        exec(oid, _, StrEq("lock"),
                             StrEq("assert_locked"), _, _, _, _))
            .WillOnce(DoDefault());
        }
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        exec(oid, _, StrEq("rbd"),
                             StrEq("object_map_update"),
    ...
    {
        EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                    exec(ictx->header_oid, _, StrEq("rbd"),
                         StrEq("set_flags"), _, _, _, _))
        .WillOnce(DoDefault());
    }
};
...
TEST_F(TestMockObjectMapUpdateRequest, UpdateInMemory)
{
    ...
    ASSERT_EQ(0,
              ictx->operations->resize(4 << ictx->order, true,
                                       no_progress));
    ...
    AsyncRequest <> *req =
        new UpdateRequest <> (*ictx, &object_map_lock, &object_map,
                              CEPH_NOSNAP, 0, object_map.size(),
    OBJECT_NONEXISTENT, OBJECT_EXISTS, {
    }
    , false, &cond_ctx);
    {
...
TEST_F(TestMockObjectMapUpdateRequest, UpdateHeadOnDisk)
{
    ...
    expect_update(ictx, CEPH_NOSNAP, 0, 1, OBJECT_NONEXISTENT,
                  OBJECT_EXISTS, 0);
...
TEST_F(TestMockObjectMapUpdateRequest, UpdateSnapOnDisk)
{
    ...
    expect_update(ictx, snap_id, 0, 1, OBJECT_NONEXISTENT,
                  OBJECT_EXISTS, 0);
...
TEST_F(TestMockObjectMapUpdateRequest, BatchUpdate)
{
    ...
    librbd::NoOpProgressContext no_progress;
    ASSERT_EQ(0,
              ictx->operations->resize(712312 * ictx->get_object_size(),
                                       false, no_progress));
    ASSERT_EQ(0, acquire_exclusive_lock(*ictx));

    expect_unlock_exclusive_lock(*ictx);
    InSequence seq;
    expect_update(ictx, CEPH_NOSNAP, 0, 262144, OBJECT_NONEXISTENT,
                  OBJECT_EXISTS, 0);
