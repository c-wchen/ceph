...
// btree_multimap.
    template < typename Tree > class btree_container {
      ...
      public:
        ...
        // Copy constructor. btree_container(const self_type & x)
        :tree_(x.tree_) {
        }
        ...
        // Lookup routines. iterator lower_bound(const key_type & key) {
  ...
  template < typename Tree > class btree_map_container:public btree_unique_container < Tree >
    {
      ...
      :    super_type(b, e, comp, alloc) {
        ...
        data_type & operator[](const key_type & key) {
            return this->tree_.insert_unique(key,
                                             generate_value(key)).first->second;
        }
    };
