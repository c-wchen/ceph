...
seastar::future <> make_keyring()
{
    ...
    return seastar::file_exists(path).then([path] (bool exists) {
                                           KeyRing keyring; EntityName name {
                                           local_conf()->name};
                                           EntityAuth auth;
                                           if (exists &&
                                               keyring.load(nullptr, path) == 0
                                               && keyring.get_auth(name,
                                                                   auth)) {
                                           fmt::print(std::cerr,
                                           ...
                                           return crimson::write_file(std::
                                                                      move(bl),
                                                                      path,
                                                                      permissions);}
                                           }
    )
        .handle_exception_type([path]
...
static std::ofstream maybe_set_logger()
{
    ...
    if (auto log_file = local_conf()->log_file; !log_file.empty()) {
        ...
        try {
            ...
            ceph_abort_msg(fmt::
                           format("unable to open log file: {}", e.what()));
        }
...
int main(int argc, const char *argv[])
{
    ...
    app.add_options()
        ...
        ("prometheus_prefix",
         bpo::value < std::string > ()->default_value("osd"),
         "Prometheus metrics prefix");

    try {
        return app.run(seastar_n_early_args.size(),
                       ...
                       return seastar::async([&] {
                                             ...
                                             set_all_loggers_level(seastar::
                                                                   log_level::
                                                                   trace);}
                                             sharded_conf().start(early_config.
                                                                  init_params.
                                                                  name,
                                                                  early_config.
                                                                  cluster_name).
                                             get(); local_conf().start().get();
                                             auto stop_conf =
                                             seastar::
                                             deferred_stop(sharded_conf());
                                             sharded_perf_coll().start().get();
                                             auto stop_perf_coll =
                                             seastar::
                                             deferred_stop(sharded_perf_coll());
                                             local_conf().
                                             parse_config_files(early_config.
                                                                conf_file_list).
                                             get();
                                             local_conf().parse_env().get();
                                             local_conf().
                                             parse_argv(config_proxy_args).
                                             get();
                                             auto log_file_stream =
                                             maybe_set_logger();
                                             auto reset_logger =
                                             seastar::defer([] {
                                                            logger().
                                             ...
                                             seastar::httpd::
                                             http_server_control prom_server;
                                             std::any stop_prometheus;
                                             if (uint16_t prom_port =
                                                 config["prometheus_port"].as <
                                                 uint16_t > ();
                                                 prom_port != 0) {
                                             prom_server.start("prometheus").
                                             get();
                                             stop_prometheus =
                                             seastar::make_shared(seastar::
                                                                  deferred_stop
                                                                  (prom_server));
                                             seastar::prometheus::
                                             config prom_config;
                                             prom_config.prefix =
                                             config["prometheus_prefix"].as <
                                             std::string > ();
                                             seastar::prometheus::
                                             start(prom_server,
                                                   prom_config).get();
              ...
              const int whoami = std::stoi(local_conf()->name.get_id()); const auto nonce = crimson::osd::get_nonce(); crimson::net::MessengerRef cluster_msgr, client_msgr; crimson::net::MessengerRef hb_front_msgr, hb_back_msgr; for (auto[msgr, name]:{
                                                                                                                                                                                                                                                                         make_pair
                                                                                                                                                                                                                                                                         (std::
                                                                                                                                                                                                                                                                          ref
                                                                                                                                                                                                                                                                          (cluster_msgr),
                                             ...
                                             create(entity_name_t::OSD(whoami),
                                                    name, nonce);}
                                             auto store =
                                             crimson::os::FuturizedStore::
                                             create(local_conf().get_val <
                                                    std::string >
                                                    ("osd_objectstore"),
                                                    local_conf().get_val <
                                                    std::string > ("osd_data"),
                                             ...
                                             logger().
                                             info
                                             ("bypassing the config fetch due to --no-mon-config");}
                                             else {
                                             crimson::osd::
                                             populate_config_from_mon().get();}
                                             if (config.count("mkfs")) {
                                             auto osd_uuid =
                                             local_conf().get_val < uuid_d >
                                             ("osd_uuid");
                                             if (osd_uuid.is_zero()) {
