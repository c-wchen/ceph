...
ostream & operator<<(ostream & out, const SnapRealm & realm)
{
    ...
    if (realm.srnode.past_parents.size()) {
        out << " past_parents=(";
        for (map < snapid_t, snaplink_t >::const_iterator p =
             realm.srnode.past_parents.begin();
             p != realm.srnode.past_parents.end(); ++p) {
            if (p != realm.srnode.past_parents.begin())
                out << ",";
            out << p->second.first << "-" << p->first << "=" << p->second.ino;
        }
        out << ")";
    }
    out << " " << &realm << ")";
...
struct C_SR_RetryOpenParents:public MDSInternalContextBase {
    ...
    C_SR_RetryOpenParents(SnapRealm * s, snapid_t f, snapid_t l, snapid_t pl,
                          inodeno_t p, MDSInternalContextBase * c):sr(s),
        first(f), last(l), parent_last(pl), parent(p), fin(c) {
...
void SnapRealm::_remove_missing_parent(snapid_t snapid, inodeno_t parent,
                                       int err)
{
...
bool SnapRealm::_open_parents(MDSInternalContextBase * finish, snapid_t first,
                              snapid_t last)
{
    ...
    if (parent) {
        dout(10) << " current parent [" << srnode.
            current_parent_since << ",head] is " << *parent << " on " <<
            *parent->inode << dendl;
        if (last >= srnode.current_parent_since
            && !parent->_open_parents(finish,
                                      MAX(first, srnode.current_parent_since),
                                      last))
            return false;
    }
    ...
    if (srnode.past_parents.size() > num_open_past_parents) {
        for (map < snapid_t, snaplink_t >::iterator p =
             srnode.past_parents.begin(); p != srnode.past_parents.end();) {
            dout(10) << " past_parent [" << p->second.first << "," << p->
                first << "] is " << p->second.ino << dendl;
            CInode *parent = mdcache->get_inode(p->second.ino);
            if (!parent) {
                C_SR_RetryOpenParents *fin =
                    new C_SR_RetryOpenParents(this, first, last, p->first,
                                              p->second.ino, finish);
                mdcache->open_ino(p->second.ino,
                                  mdcache->mds->mdsmap->get_metadata_pool(),
                                  fin);
                return false;
            }
            ...
            if (!parent->snaprealm->
                _open_parents(finish, p->second.first, p->first))
                return false;
...
{
    dout(10) << "have_past_parents_open [" << first << "," << last << "]" <<
        dendl;
    if (open)
        return true;

    for (map < snapid_t, snaplink_t >::iterator p =
         srnode.past_parents.lower_bound(first); p != srnode.past_parents.end();
         ++p) {
        if (p->second.first > last)
            break;
        dout(10) << " past parent [" << p->second.first << "," << p->
            first << "] was " << p->second.ino << dendl;
        if (open_past_parents.count(p->second.ino) == 0) {
            dout(10) << " past parent " << p->second.
                ino << " is not open" << dendl;
            return false;
        }
...
void SnapRealm::build_snap_set(set < snapid_t > &s,
                               ...
                               snapid_t & max_last_destroyed, snapid_t first,
                               snapid_t last) const const
{
    dout(10) << "build_snap_set [" << first << "," << last << "] on " << *this
        << dendl;
    ...
    for (map < snapid_t, snaplink_t >::const_iterator p =
         srnode.past_parents.lower_bound(first);
         p != srnode.past_parents.end() && p->first >= first
         && p->second.first <= last; ++p) {
...
void SnapRealm::check_cache() const const
{
    assert(open);
    ...
    build_snap_set(cached_snaps, cached_seq, cached_last_created,
                   cached_last_destroyed, 0, CEPH_NOSNAP);
    ...
    dout(10) << "check_cache rebuilt " << cached_snaps
        ...
        << " cached_last_destroyed " << cached_last_destroyed << ")" << dendl;
}

const set < snapid_t > &SnapRealm::get_snaps() const const
{
    check_cache();
 ...
 */
const SnapContext & SnapRealm::get_snap_context() const const
{
    check_cache();
...
void SnapRealm::get_snap_info(map < snapid_t, SnapInfo * >&infomap,
                              snapid_t first, snapid_t last)
{
    ...
    if (srnode.current_parent_since <= last && parent)
        parent->get_snap_info(infomap, MAX(first, srnode.current_parent_since),
                              last);
}
...
boost::string_view SnapRealm::get_snapname(snapid_t snapid, inodeno_t atino)
{
    ...
    map < snapid_t, snaplink_t >::iterator p =
        srnode.past_parents.lower_bound(snapid);
...
snapid_t SnapRealm::resolve_snapname(boost::string_view n, inodeno_t atino,
                                     snapid_t first, snapid_t last)
{
    // first try me
    ...
    if (parent && srnode.current_parent_since <= last)
        return parent->resolve_snapname(n, atino,
                                        MAX(first, srnode.current_parent_since),
                                        last);
    return 0;
}
...
{
    SnapRealm *newparent =
        inode->get_parent_dn()->get_dir()->get_inode()->find_snaprealm();
    if (newparent != parent) {
...
void SnapRealm::split_at(SnapRealm * child)
{
    ...
    if (inode->is_mdsdir() || !child->inode->is_dir()) {
        ...
        if (child->inode->containing_realm) {
            ...
            dout(20) << " split no-op, no caps to move on file " << *child->
                inode << dendl;
    ...
    elist < CInode * >::iterator p =
        inodes_with_caps.begin(member_offset(CInode, item_caps));
    while (!p.end()) {
        CInode *in = *p;
        ++p;
...
void SnapRealm::build_snap_trace(bufferlist & snapbl) const const
{
    SnapRealmInfo info(inode->ino(), srnode.created, srnode.seq,
                       srnode.current_parent_since);
    ...
    if (parent) {
        ...
        if (!srnode.past_parents.empty()) {
            ...
            build_snap_set(past, max_seq, max_last_created, max_last_destroyed,
                           0, last);
            info.prior_parent_snaps.reserve(past.size());
            for (set < snapid_t >::reverse_iterator p = past.rbegin();
                 p != past.rend(); ++p)
    ...
    for (map < snapid_t, SnapInfo >::const_reverse_iterator p =
         srnode.snaps.rbegin(); p != srnode.snaps.rend(); ++p)
...
void SnapRealm::prune_past_parents()
{
    ...
    while (p != srnode.past_parents.end()) {
        set < snapid_t >::iterator q =
            cached_snaps.lower_bound(p->second.first);
        if (q == cached_snaps.end() || *q > p->first) {
            dout(10) << "prune_past_parents pruning [" << p->second.
                first << "," << p->first << "] " << p->second.ino << dendl;
            remove_open_past_parent(p->second.ino, p->first);
            srnode.past_parents.erase(p++);
        }
        else {
            dout(10) << "prune_past_parents keeping [" << p->second.
                first << "," << p->first << "] " << p->second.ino << dendl;
            ++p;
        }
    }
