...
public:
    RGWLogStatRemoteObjCBCR(RGWDataSyncCtx *_sc,
                            ...
                            rgw_obj_key &_key): RGWStatRemoteObjCBCR(_sc,
                                        _src_bucket,
                                        _key)
    {
    } int operate(const DoutPrefixProvider *dpp) override
    {
        ldpp_dout(dpp,
                  0) << "SYNC_LOG: stat of remote obj: z=" << sc->
                     source_zone << " b=" << src_bucket << " k=" << key << " size=" <<
                     size << " mtime=" << mtime << " attrs=" << attrs << dendl;
        return set_cr_done();
    }
...
public:
    RGWLogStatRemoteObjCR(RGWDataSyncCtx *_sc,
                          rgw_bucket &_src_bucket,
                          rgw_obj_key &_key): RGWCallStatRemoteObjCR(_sc,
                                      _src_bucket,
                                      _key)
    {
...
public:
    ...
    {
        ldpp_dout(dpp,
                  0) << prefix << ": SYNC_LOG: sync_object: b=" << sync_pipe.
                     info.source_bs.
                     bucket << " k=" << key << " versioned_epoch=" << versioned_epoch.
                     value_or(0) << dendl;
        return new RGWLogStatRemoteObjCR(sc, sync_pipe.info.source_bs.bucket,
                                         key);
    }
    ...
    {
        ldpp_dout(dpp,
                  0) << prefix << ": SYNC_LOG: rm_object: b=" << sync_pipe.info.
                     source_bs.
...
public:
    explicit RGWLogSyncModuleInstance(const string &
                                      prefix): data_handler(prefix)
    {
...
int RGWLogSyncModule::create_instance(const DoutPrefixProvider *dpp,
                                      CephContext *cct,
