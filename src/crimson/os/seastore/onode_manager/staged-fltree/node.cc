...
// create an invalid tree_cursor_t
    ...
    void tree_cursor_t::assert_next_to(const tree_cursor_t & prv,
                                       value_magic_t magic) const {
#ifndef NDEBUG
        assert(!prv.is_end());
        if (is_end()) {
    ...
    std::strong_ordering tree_cursor_t::compare_to(const tree_cursor_t & o,
                                                   value_magic_t magic)const {
    ...
    template < bool VALIDATE >
        void tree_cursor_t::update_track(Ref < LeafNode > node,
                                         const search_position_t & pos) {
    ...
    template void tree_cursor_t::update_track < true > (Ref < LeafNode >,
                                                        const search_position_t
                                                        &);
    template void tree_cursor_t::update_track < false > (Ref < LeafNode >,
                                                         const search_position_t
                                                         &);

    void tree_cursor_t::update_cache_same_node(const key_view_t & key_view,
                                               const value_header_t *
                                               p_value_header) const {
        assert(is_tracked());
        cache.update_all(ref_leaf_node->get_version(), key_view,
                         p_value_header);
 ...
 */
  ...
  :    ref_leaf_node {
    ...
    void tree_cursor_t::Cache::
        update_all(const node_version_t & current_version,
    ...
    void tree_cursor_t::Cache::
        maybe_duplicate(const node_version_t & current_version) {
    ...
    void tree_cursor_t::Cache::make_latest(value_magic_t magic,
                                           const search_position_t & pos) {
    ...
    void tree_cursor_t::Cache::validate_is_latest(const search_position_t & pos) const {
        ...
        if (!value_payload_mut.has_value()) {
            ...
            maybe_duplicate(current_version);
            value_payload_mut =
                p_value_header->get_payload_mutable(value_mutable.first);
            p_value_recorder = value_mutable.second;
            validate_is_latest(pos);
        }
        return {
        *value_payload_mut, p_value_recorder};
    }
 ...
 */
  ...
  Node::Node(NodeImplURef && impl):impl {
    std::move(impl)}
    {
    }
    ...
    level_t Node::level() const {
        return impl->level();
    } eagain_ifuture < Node::search_result_t > Node::lower_bound(context_t c,
                                                                 const
                                                                 key_hobj_t &
                                                                 key) {
    ...
    eagain_ifuture < std::pair < Ref < tree_cursor_t >,
        bool >> Node::insert(context_t c, const key_hobj_t & key,
                             value_config_t vconf, Ref < Node > &&this_ref) {
        return seastar::do_with(MatchHistory(),
                                [this, c, &key, vconf, this_ref =
                                 std::move(this_ref)] (auto & history) mutable {
                                return lower_bound_tracked(c, key,
                                                           history).si_then([c,
                                                                             &key,
                                                                             vconf,
                                                                             &history,
    ...
    eagain_ifuture < std::size_t > Node::erase(context_t c,
                                               const key_hobj_t & key,
                                               Ref < Node > &&this_ref) {
        return lower_bound(c,
                           key).si_then([c, this_ref =
                                         std::
                                         move(this_ref)] (auto result) mutable {
                                        // the cursor in the result should already hold the root node upwards
                                        this_ref.reset();
    ...
    } void Node::test_make_destructable(context_t c, NodeExtentMutable & mut,
                                        Super::URef && _super) {
    ...
    eagain_ifuture <> Node::mkfs(context_t c, RootNodeTracker & root_tracker) {
        ...
        return LeafNode::allocate_root(c,
                                       root_tracker).si_then([c,
                                                              FNAME] (auto ret)
                                                             {
                                                             c.t.
    ...
    eagain_ifuture < Ref < Node >> Node::load_root(context_t c,
                                                   RootNodeTracker &
                                                   root_tracker) {
        ...
        return c.nm.get_super(c.t,
            ...
            handle_error_interruptible(eagain_iertr::pass_further {
                      ...
                      return Node::load(c, root_addr,
                                        true).si_then([c, _super =
                                                       std::move(_super),
                                                       &root_tracker,
                                                       FNAME] (auto root)
                                                      mutable {
                                                      TRACET("loaded {}", c.t, root->get_name()); assert(root->impl->field_type() == field_type_t::N0); root->as_root(std::move(_super)); std::ignore = c;  // as only used in an assert
                                                      std::ignore =
                                                      root_tracker;
                                                      assert(root ==
                                                             root_tracker.
    ...
    void Node::make_root(context_t c, Super::URef && _super) {
        ...
        c.t.get_onode_tree_stats().depth =
    ...
    eagain_ifuture <> Node::upgrade_root(context_t c, laddr_t hint) {
        ...
        return InternalNode::allocate_root(c, hint, impl->level(),
                                           impl->laddr(),
                                           std::move(super_to_move)
    ...
    template < bool VALIDATE >
        void Node::as_child(const search_position_t & pos,
                            Ref < InternalNode > parent_node) {
...
#endif
        _parent_info = parent_info_t {
        pos, parent_node};
        parent_info().ptr->do_track_child < VALIDATE > (*this);
        assert(!is_root());
    }
    template void Node::as_child < true > (const search_position_t &,
                                           Ref < InternalNode >);
    template void Node::as_child < false > (const search_position_t &,
                                            Ref < InternalNode >);
    ...
    eagain_ifuture <> Node::apply_split_to_parent(context_t c,
                                                  Ref < Node > &&this_ref,
        ...
        return parent_info().ptr->apply_child_split(c, std::move(this_ref),
                                                    std::move(split_right),
                                                    update_right_index);
    }
    ...
    template < bool FORCE_MERGE >
        ...
        return parent_info().ptr->get_child_peers(c,
            ...
            si_then([c, this_ref =
                     std::move(this_ref), this, FNAME,
                     update_parent_index] (auto lr_nodes) mutable->
                    eagain_ifuture <> {
                    auto &[lnode, rnode] = lr_nodes;
                    Ref < Node > left_for_merge; Ref < Node > right_for_merge;
                    Ref < Node > *p_this_ref; bool is_left;
                    if (!lnode && !rnode) {
                    ...
                    right_for_merge = std::move(rnode); is_left = true;}
                    else
                    if (!rnode) {
                    left_for_merge = std::move(lnode);
                    right_for_merge = std::move(this_ref);
                    p_this_ref = &right_for_merge; is_left = false;}
                    else {      // lnode && rnode
                    ...
                    left_for_merge->impl->evaluate_merge(*right_for_merge->
                                                         impl);
                    if (merge_size <= left_for_merge->impl->total_size()) {
                    // proceed merge
                    bool update_index_after_merge; if (is_left) {
                    ...
                    si_then([c, update_index_after_merge, left_addr,
                             merge_stage = merge_stage, merge_size =
                             merge_size, left_for_merge =
                             std::move(left_for_merge), right_for_merge =
                             std::
                             move(right_for_merge)] (auto left_mut) mutable {
                            ...
                            *static_cast < LeafNode * >(left_for_merge.get());
                            ...
                            left_for_merge->impl->merge(left_mut,
                                                        *right_for_merge->impl,
                                                        merge_stage,
                                                        merge_size);
                            left_for_merge->track_merge(right_for_merge,
                                                        merge_stage,
                                                        left_last_pos);
                            --(c.t.get_onode_tree_stats().extents_num_delta);
                            return left_for_merge->parent_info().ptr->
                            apply_children_merge(c, std::move(left_for_merge),
                                                 left_addr,
    ...
    template eagain_ifuture <> Node::try_merge_adjacent < true > (context_t,
                                                                  bool,
                                                                  Ref < Node >
                                                                  &&);
    template eagain_ifuture <> Node::try_merge_adjacent < false > (context_t,
                                                                   bool,
                                                                   Ref < Node >
                                                                   &&);
    ...
    template < bool FORCE_MERGE >
        eagain_ifuture <> Node::fix_parent_index(context_t c,
                                                 Ref < Node > &&this_ref,
                                                 bool check_downgrade) {
        assert(!is_root());
        assert(this == this_ref.get());
        return parent_info().ptr->fix_index < FORCE_MERGE > (c,
                                                             std::
                                                             move(this_ref),
                                                             check_downgrade);
    }
    template eagain_ifuture <> Node::fix_parent_index < true > (context_t,
                                                                Ref < Node > &&,
                                                                bool);
    template eagain_ifuture <> Node::fix_parent_index < false > (context_t,
                                                                 Ref < Node >
                                                                 &&, bool);

    eagain_ifuture < Ref < Node >> Node::load(context_t c, laddr_t addr,
                                              bool expect_is_level_tail) {
        LOG_PREFIX(OTree::Node::load);
        return c.nm.read_extent(c.t,
                                addr).
            handle_error_interruptible(eagain_iertr::pass_further {
                                       }
                                       ,
                      ...
                      return eagain_iertr::make_ready_future < Ref <
                      Node >> (new LeafNode(derived_ptr, std::move(impl)));}
                      else
                      ...
                      auto * derived_ptr = impl.get();
                      return eagain_iertr::make_ready_future < Ref <
                      Node >> (new InternalNode(derived_ptr, std::move(impl)));}
                      else {
 ...
 */
    ...
    InternalNode::InternalNode(InternalNodeImpl * impl, NodeImplURef
                               && impl_ref)
  :    Node(std::move(impl_ref)), impl {
    impl}
    {
    }

    eagain_ifuture < Ref < tree_cursor_t >>
        InternalNode::get_next_cursor(context_t c,
                                      const search_position_t & pos) {
        impl->validate_non_empty();
        if (pos.is_end()) {
            assert(impl->is_level_tail());
        ...
        if (next_pos.is_end() && !impl->is_level_tail()) {
            ...
            assert(p_child_addr);
            return get_or_track_child(c, next_pos,
                                      p_child_addr->value).
    ...
    eagain_ifuture <> InternalNode::apply_child_split(context_t c,
                                                      Ref < Node > &&left_child,
                                                      Ref < Node >
                                                      &&right_child,
                                                      bool update_right_index) {
        ...
        return insert_or_split(c, left_pos, left_key, left_child,
                               (update_right_index ? right_child : nullptr)
            ).si_then([this, c,
                       this_ref =
                       std::move(this_ref)] (auto split_right) mutable {
                      if (split_right) {
            ...
            .si_then([c, update_right_index,
                     ...
                     return right_child->fix_parent_index(c,
                                                          std::
                                                          move(right_child),
                                                          false);}
                     else {
    ...
    eagain_ifuture <> InternalNode::erase_child(context_t c,
                                                Ref < Node > &&child_ref) {
        ...
        return eagain_iertr::now().si_then([c, this, fix_tail] {
                                           ...
                                           new_tail_p_addr = nullptr;
                                           impl->get_largest_slot(&new_tail_pos,
                                                                  nullptr,
                                                                  &new_tail_p_addr);
                                           return get_or_track_child(c,
                                                                     new_tail_pos,
                                                                     new_tail_p_addr->
                                                                     value);}
                                           else {
            ...
            .si_then([c, this, child_ref = std::move(child_ref), FNAME]
                     ...
                     assert(new_tail_child->impl->is_level_tail());
                     if (new_tail_child->impl->node_type() ==
                         node_type_t::LEAF) {
                     ...
                     return child_ref->retire(c, std::move(child_ref)
                               ...
                               DEBUGT("{} has one value left, erase ...", c.t,
                                      get_name());
#ifndef NDEBUG
                               ...
                               ceph_abort
                               ("trying to erase the last item from the internal root node");}
                               ...
                               impl->prepare_mutate(c);
                               auto[erase_stage, next_or_last_pos] =
                               impl->erase(child_pos); if (child_pos.is_end()) {
                               ...
                               validate_tracked_children(); if (is_root()) {
                               return try_downgrade_root(c,
                                                         std::move(this_ref));}
                               else {
                               ...
                               true : update_parent_index = false;}
                               return try_merge_adjacent(c, update_parent_index,
                                                         std::move(this_ref));}
                               }
                     ).
                     si_then([c, new_tail_child =
                              std::move(new_tail_child)] ()mutable {
    ...
    template < bool FORCE_MERGE >
        eagain_ifuture <> InternalNode::fix_index(context_t c,
                                                  Ref < Node > &&child,
                                                  bool check_downgrade) {
        ...
        return insert_or_split(c, next_pos, new_key,
                               child).si_then([this, c, update_parent_index,
                                               check_downgrade, this_ref =
                                               std::
                                               move(this_ref)] (auto
                                                                split_right)
                                              mutable {
                                              ...
                                              return apply_split_to_parent(c,
                                                                           std::
                                                                           move
                                                                           (this_ref),
                                                                           std::
    ...
    template < bool FORCE_MERGE >
        eagain_ifuture <> InternalNode::apply_children_merge(context_t c,
                                                             Ref < Node >
                                                             &&left_child,
                                                             laddr_t
                                                             origin_left_addr,
                                                             ...
                                                             bool update_index)
    {
        ...
        DEBUGT("apply {}'s child {} (was {:#x}) at pos({}), "
               ...
               c.t, get_name(), left_child->get_name(), origin_left_addr,
               left_pos, right_child->get_name(), right_pos, update_index);
        ...
        return right_child->retire(c, std::move(right_child)
                      ...
                      validate_tracked_children();
                      Ref < Node > this_ref = this; left_child.reset();
                      // I'm all good but:
                      // - my number of keys is reduced by 1
    ...
    eagain_ifuture < std::pair < Ref < Node >,
        Ref < Node >>> InternalNode::get_child_peers(context_t c,
                                                     const search_position_t &
                                                     pos) {
        // assume I'm already ref counted by caller
        ...
        return eagain_iertr::now().
            si_then([this, c, prev_pos, prev_p_child_addr] {
                    ...
                    return get_or_track_child(c, prev_pos,
                                              prev_p_child_addr->value);}
                    else {
            ...
            .si_then([this, c, next_pos, next_p_child_addr] (Ref < Node > lnode) {
                     ...
                     return get_or_track_child(c, next_pos,
                                               next_p_child_addr->value).
                     si_then([lnode] (Ref < Node > rnode) {
                             return seastar::make_ready_future < std::pair <
                             Ref < Node >, Ref < Node >>> (lnode, rnode);}
                     );}
                     else {
                     return eagain_iertr::make_ready_future < std::pair < Ref <
                     Node >, Ref < Node >>> (lnode, nullptr);}
                     }
        ) ;
    }

    eagain_ifuture < Ref <
        InternalNode >> InternalNode::allocate_root(context_t c, laddr_t hint,
                                                    level_t old_root_level,
                                                    laddr_t old_root_addr,
                                                    Super::URef && super) {
        // support tree height up to 256
        ceph_assert(old_root_level < MAX_LEVEL);
        return InternalNode::allocate(c, hint, field_type_t::N0, true,
                                      old_root_level + 1).si_then([c,
                                                                   old_root_addr,
                                                                   super =
                                                                   std::
                                                                   move(super)]
    ...
    eagain_ifuture < Ref < tree_cursor_t >>
        ...
        return get_or_track_child(c, position,
                                  p_child_addr->value).
            si_then([c] (auto child) {
                    return child->lookup_smallest(c);}
        );
    ...
    eagain_ifuture < Ref < tree_cursor_t >>
        ...
        return get_or_track_child(c, search_position_t::end(),
                                  p_child_addr->value).
            si_then([c] (auto child) {
    ...
    eagain_ifuture < Node::search_result_t >
        InternalNode::lower_bound_tracked(context_t c, const key_hobj_t & key,
                                          MatchHistory & history) {
        auto result = impl->lower_bound(key, history);
        return get_or_track_child(c, result.position,
                                  result.p_value->value).si_then([c, &key,
                                                                  &history]
                                                                 (auto child) {
    ...
    eagain_ifuture <> InternalNode::do_get_tree_stats(context_t c,
                                                      tree_stats_t & stats) {
        impl->validate_non_empty();
        ...
        Ref < Node > this_ref = this;
        return seastar::do_with(search_position_t(),
                                (const laddr_packed_t *)(nullptr),[this,
                                                                   this_ref, c,
                                                                   &stats] (auto
                                                                            &
                                                                            pos,
                                ...
                                return trans_intr::
                                repeat([this, this_ref, c, &stats, &pos,
                                        &p_child_addr] ()
                                       ->eagain_ifuture <
                                       seastar::stop_iteration > {
                                       return get_or_track_child(c, pos,
                                                                 p_child_addr->
                                                                 value).
                                       si_then([c, &stats] (auto child) {
                                               return child->
                                               do_get_tree_stats(c, stats);}
                                       ).
                                       si_then([this, this_ref, &pos,
                                                &p_child_addr] {
                                               if (pos.is_end()) {
                                               return seastar::stop_iteration::
                                               yes;}
                                               else {
                                               impl->get_next_slot(pos, nullptr,
                                                                   &p_child_addr);
                                               if (pos.is_end()) {
                                               if (impl->is_level_tail()) {
                                               p_child_addr =
    ...
    void InternalNode::track_merge(Ref < Node > _right_node,
                                   match_stage_t stage,
                                   search_position_t & left_last_pos) {
        ...
        if (rit != right_tracked_children.end()) {
            ...
            assert(rit->second->parent_info().position ==
    ...
    eagain_ifuture <> InternalNode::test_clone_root(context_t c_other,
                                                    RootNodeTracker &
                                                    tracker_other) const {
        assert(is_root());
        assert(impl->is_level_tail());
        ...
        return InternalNode::allocate(c_other, L_ADDR_MIN, field_type_t::N0,
                  ...
                  handle_error_interruptible(eagain_iertr::pass_further {
                            ...
                            assert(super_other);
                            cloned_root->make_root_new(c_other,
                                                       std::move(super_other));
                            return cloned_root;}
                  );}
                  ...
                  return trans_intr::do_for_each(tracked_child_nodes.begin(),
                                                 ...
                                                 assert(kv.first ==
                                                        ...
                                                        parent_info().position);
                                                 return kv.second->
                                                 test_clone_non_root(c_other,
                                                                     cloned_root);}
                  );}
        );
    }

    eagain_ifuture <> InternalNode::try_downgrade_root(context_t c,
                                                       Ref < Node >
                                                       &&this_ref) {
        ...
        return get_or_track_child(c, search_position_t::end(),
                                  child_addr).si_then([c, this, FNAME,
                                                       this_ref =
                                                       std::
                                                       move(this_ref)] (auto
                                                                        child)
    ...
    eagain_ifuture < Ref <
        InternalNode >> InternalNode::insert_or_split(context_t c,
                                                      const search_position_t &
                                                      pos,
                                                      const key_view_t &
                                                      insert_key,
        ...
        if (free_size >= insert_size) {
            ...
            [[maybe_unused]] auto p_value =
                impl->insert(insert_key, insert_value, insert_pos, insert_stage,
                             insert_size);
        ...
        return (is_root()? upgrade_root(c, left_hint) : eagain_iertr::now()
                      ...
                      return InternalNode::allocate(c, right_hint,
                                                    impl->field_type(),
                                                    impl->is_level_tail(),
                                                    impl->level());}
        ).si_then([this, insert_key, insert_child, insert_pos, insert_stage =
                   insert_stage, insert_size =
                   insert_size, outdated_child, c,
                  ...
                  auto insert_value = insert_child->impl->laddr();
                  auto[split_pos, is_insert_left, p_value] =
                  impl->split_insert(fresh_right.mut, *right_node->impl,
                                     insert_key, insert_value, insert_pos,
                                     insert_stage, insert_size);
                  assert(p_value->value == insert_value);
                  track_split(split_pos, right_node); if (outdated_child) {
                  if (is_insert_left) {
                  track_insert < false > (insert_pos, insert_stage,
                                          insert_child);}
                  else {
                  right_node->template track_insert < false > (insert_pos,
                                                               insert_stage,
                                                               insert_child);}
#ifndef NDEBUG
                  ...
                  right_node->track_insert(insert_pos, insert_stage,
                                           insert_child);}
                  validate_tracked_children();
                  right_node->validate_tracked_children();}
                  ++(c.t.get_onode_tree_stats().extents_num_delta);
    ...
    eagain_ifuture < Ref < Node >> InternalNode::get_or_track_child(context_t c,
                                                                    const
                                                                    search_position_t
                                                                    & position,
                                                                    laddr_t
                                                                    child_addr)
    {
        ...
        return[this, position, child_addr, c, FNAME] {
            ...
            return Node::load(c, child_addr,
                              level_tail).si_then([this, position, c,
                                                   FNAME] (auto child) {
                                                  TRACET
                                                  ("loaded child untracked {}",
                                                   c.t, child->get_name());
                                                  if (child->level() + 1 !=
                                                      level()) {
                                                  ERRORT
                                                  ("loaded child {} error from parent {} at pos({}), level mismatch",
                                                   c.t, child->get_name(),
                                                   get_name(), position);
                                                  ceph_abort("fatal error");}
                                                  child->as_child(position,
                                                                  this);
                                                  return child;}
            ) ;
        }
        ().si_then([this_ref, this, position, child_addr] (auto child) {
                   ...
                   validate_child_tracked(*child); return child;}
        );
    }

    template < bool VALIDATE >
        void InternalNode::track_insert(const search_position_t & insert_pos,
    ...
    template void InternalNode::track_insert < true >
        (const search_position_t &, match_stage_t, Ref < Node >, Ref < Node >);
    template void InternalNode::track_insert < false >
        (const search_position_t &, match_stage_t, Ref < Node >, Ref < Node >);

    void InternalNode::replace_track(Ref < Node > new_child,
                                     Ref < Node > old_child,
    ...
    void InternalNode::track_split(const search_position_t & split_pos,
    ...
    template < bool VALIDATE >
        void InternalNode::track_erase(const search_position_t & erase_pos,
    ...
    template void InternalNode::track_erase < true > (const search_position_t &,
                                                      match_stage_t);
    template void InternalNode::track_erase < false >
        (const search_position_t &, match_stage_t);
    ...
    void InternalNode::validate_child_inconsistent(const Node & child) const {
        ...
        InternalNode::allocate(context_t c, laddr_t hint,
                               field_type_t field_type, bool is_level_tail,
                               level_t level) {
 ...
 */
  ...
  :    Node(std::move(impl_ref)), impl {
    ...
    bool LeafNode::is_level_tail() const {
        ...
        return {
        layout_version, impl->get_extent_state()};
    }
    ...
    eagain_ifuture < Ref < tree_cursor_t >>
        ...
        if (next_pos.is_end()) {
            if (unlikely(is_level_tail())) {
                return eagain_iertr::make_ready_future < Ref <
                    tree_cursor_t >> (tree_cursor_t::create_end(this));
            }
            else {
                return get_next_cursor_from_parent(c);
            }
    ...
    template < bool FORCE_MERGE >
        eagain_ifuture < Ref < tree_cursor_t >>
        LeafNode::erase(context_t c, const search_position_t & pos,
                        bool get_next) {
        ...
        return eagain_iertr::now().si_then([c, &pos, get_next, this] {
            ...
            .si_then([c, &pos, this_ref = std::move(this_ref),
                     ...
                     next_cursor.reset();}
                     return eagain_iertr::now().si_then([c, &pos, this_ref =
                                                         std::move(this_ref),
                                                         this,
                                                         FNAME] ()mutable {
                                                        assert_moveable
                                                        ...
                                                        // track_erase
                                                        DEBUGT
                                                        ("{} has one value left, erase ...",
                                                         c.t, get_name());
                                                        assert(tracked_cursors.
                                                               size() == 1);
                                                        auto iter =
                                                        tracked_cursors.begin();
                                                        assert(iter->first ==
                                                               pos);
                                                        iter->second->
                                                        invalidate();
    ...
    eagain_ifuture <> LeafNode::extend_value(context_t c,
                                             const search_position_t & pos,
    ...
    eagain_ifuture <> LeafNode::trim_value(context_t c,
                                           const search_position_t & pos,
    ...
    eagain_ifuture < Ref < tree_cursor_t >> LeafNode::lookup_smallest(context_t) {
        if (unlikely(impl->is_keys_empty())) {
            assert(is_root());
            return seastar::make_ready_future < Ref <
                tree_cursor_t >> (tree_cursor_t::create_end(this));
        }
    ...
    eagain_ifuture < Node::search_result_t >
        LeafNode::lower_bound_tracked(context_t c, const key_hobj_t & key,
                                      MatchHistory & history) {
        key_view_t index_key;
        ...
        if (result.position.is_end()) {
            ...
            cursor =
                get_or_track_cursor(result.position, index_key, result.p_value);
        }
        search_result_t ret {
        cursor, result.mstat};
    ...
    eagain_ifuture <> LeafNode::do_get_tree_stats(context_t,
                                                  tree_stats_t & stats) {
        auto nstats = impl->get_stats();
    ...
    void LeafNode::track_merge(Ref < Node > _right_node, match_stage_t stage,
                               search_position_t & left_last_pos) {
        assert(level() == _right_node->level());
        // assert(impl->node_type() == _right_node->impl->node_type());
    ...
    eagain_ifuture <> LeafNode::test_clone_root(context_t c_other,
                                                RootNodeTracker & tracker_other)
        const {
        assert(is_root());
        assert(impl->is_level_tail());
        assert(impl->field_type() == field_type_t::N0);
        Ref < const Node > this_ref = this;
        return LeafNode::allocate(c_other, L_ADDR_MIN, field_type_t::N0,
                                  true).si_then([this, c_other,
                                                 &tracker_other] (auto
                                                                  fresh_other) {
                                                impl->test_copy_to(fresh_other.
                                                                   mut);
                                                auto cloned_root =
                                                fresh_other.node;
                                                return c_other.nm.
    ...
    eagain_ifuture < Ref < tree_cursor_t >> LeafNode::insert_value(context_t c,
                                                                   const
                                                                   key_hobj_t &
                                                                   key,
                                                                   value_config_t
        ...
        return (is_root()? upgrade_root(c, left_hint) : eagain_iertr::now()
                      ...
                      return LeafNode::allocate(c, right_hint,
                                                impl->field_type(),
                                                impl->is_level_tail());}
        ).si_then([this_ref =
                   std::move(this_ref), this, c, &key, vconf, FNAME, insert_pos,
                   insert_stage = insert_stage, insert_size =
                   insert_size] (auto fresh_right) mutable {
                  auto right_node = fresh_right.node;
                  DEBUGT("proceed split {} to fresh {} ...", c.t, get_name(),
                         right_node->get_name());
                  // no need to bump version for right node, as it is fresh
                  on_layout_change();
                  impl->prepare_mutate(c);
                  auto[split_pos, is_insert_left, p_value_header] =
                  impl->split_insert(fresh_right.mut, *right_node->impl, key,
                                     vconf, insert_pos, insert_stage,
                                     insert_size);
                  assert(p_value_header->payload_size == vconf.payload_size);
                  track_split(split_pos, right_node); Ref < tree_cursor_t > ret;
                  if (is_insert_left) {
    ...
    eagain_ifuture < Ref < LeafNode >> LeafNode::allocate_root(context_t c,
                                                               RootNodeTracker &
                                                               root_tracker) {
        LOG_PREFIX(OTree::LeafNode::allocate_root);
        return LeafNode::allocate(c, L_ADDR_MIN, field_type_t::N0,
                                  true).si_then([c, &root_tracker,
                                                 FNAME] (auto fresh_node) {
    ...
    Ref < tree_cursor_t >
        LeafNode::get_or_track_cursor(const search_position_t & position,
        ...
        if (found == tracked_cursors.end()) {
            p_cursor =
                tree_cursor_t::create_tracked(this, position, key,
                                              p_value_header);
        }
        else {
    ...
    void LeafNode::validate_cursor(const tree_cursor_t & cursor) const {
        ...
        LeafNode::track_insert(const search_position_t & insert_pos,
                               match_stage_t insert_stage,
                               const value_header_t * p_value_header) {
    ...
    void LeafNode::track_split(const search_position_t & split_pos,
    ...
    void LeafNode::track_erase(const search_position_t & erase_pos,
                               match_stage_t erase_stage) {
    ...
    eagain_ifuture < LeafNode::fresh_node_t > LeafNode::allocate(context_t c,
                                                                 laddr_t hint,
                                                                 field_type_t
                                                                 field_type,
                                                                 bool
                                                                 is_level_tail)
    {
        return LeafNodeImpl::allocate(c, hint, field_type,
                                      is_level_tail).si_then([](auto
                                                                && fresh_impl) {
                                                             auto *
                                                             derived_ptr =
                                                             fresh_impl.impl.
