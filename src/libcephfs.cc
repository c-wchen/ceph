...
std::shared_ptr < ceph::async::io_context_pool > get_icp(CephContext *cct)
{
    ...
    icwp = icp;
    icp->start(cct->_conf.get_val < std::uint64_t >
...
public:
    explicit ceph_mount_info(CephContext *cct_)
        : default_perms(),
          mounted(false),
          inited(false),
          client(nullptr), monclient(nullptr), messenger(nullptr), cct(cct_)
    {
        if (cct_) {
            cct->get();
        }
    } ~ceph_mount_info()
    {
        try {
            ...
            lderr(cct) <<
                       "ceph_mount_info::~ceph_mount_info: caught exception: " << e.
                       what() << dendl;
        } catch (...) {
            // ignore
        }
    ...
    int init()
    {
        ...
        client =
            new StandaloneClient(messenger, monclient, icp->get_io_context());
        if (!client) {
            goto fail;
        }
...
extern "C" int ceph_create_with_context(struct ceph_mount_info **cmount,
                                        CephContext *cct)
{
...
extern "C" int ceph_create(struct ceph_mount_info **cmount,
                           const char *const id)
{
...
extern "C" int ceph_conf_read_file(struct ceph_mount_info *cmount,
                                   const char *path)
{
...
extern "C" int ceph_conf_parse_env(struct ceph_mount_info *cmount,
                                   const char *name)
{
...
extern "C" int ceph_set_mount_timeout(struct ceph_mount_info *cmount,
                                      uint32_t timeout)
{
    if (cmount->is_mounted()) {
        return -CEPHFS_EINVAL;
                                ...
                                char **outsbuf, size_t *outsbuflen)
{
    ...
    C_SaferCond cond;
    int r = cmount->get_client()->mds_command(mds_spec,
            cmdv, inbl,
            &outbl, &outs,
            &cond);
                              ...
                              struct ceph_dir_result **dirpp)
{
    ...
    return cmount->get_client()->fdopendir(dirfd, (dir_result_t **) dirpp,
                                           cmount->default_perms);
}

extern "C" int ceph_closedir(struct ceph_mount_info *cmount,
                             struct ceph_dir_result *dirp)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->closedir(reinterpret_cast <
                                          dir_result_t * >(dirp));
}

extern "C" struct dirent *ceph_readdir(struct ceph_mount_info *cmount,
                                       struct ceph_dir_result *dirp)
{
    if (!cmount->is_mounted()) {
    ...
    return cmount->get_client()->readdir(reinterpret_cast <
                                         dir_result_t * >(dirp));
}

extern "C" int ceph_readdir_r(struct ceph_mount_info *cmount,
                              struct ceph_dir_result *dirp, struct dirent *de)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->readdir_r(reinterpret_cast <
                                           dir_result_t * >(dirp), de);
}

extern "C" int ceph_readdirplus_r(struct ceph_mount_info *cmount,
                                  struct ceph_dir_result *dirp,
                                  ...
                                  struct Inode **out)
{
    ...
    return cmount->get_client()->readdirplus_r(reinterpret_cast <
            dir_result_t * >(dirp), de, stx,
            want, flags, out);
}

extern "C" int ceph_getdents(struct ceph_mount_info *cmount,
                             struct ceph_dir_result *dirp, char *buf,
                             int buflen)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->getdents(reinterpret_cast <
                                          dir_result_t * >(dirp), buf, buflen);
}

extern "C" int ceph_getdnames(struct ceph_mount_info *cmount,
                              struct ceph_dir_result *dirp, char *buf,
                              int buflen)
{
    if (!cmount->is_mounted()) {
    ...
    return cmount->get_client()->getdnames(reinterpret_cast <
                                           dir_result_t * >(dirp), buf, buflen);
}

extern "C" void ceph_rewinddir(struct ceph_mount_info *cmount,
                               struct ceph_dir_result *dirp)
{
    if (!cmount->is_mounted()) {
        return;
...
extern "C" int64_t ceph_telldir(struct ceph_mount_info *cmount,
                                struct ceph_dir_result *dirp)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->telldir(reinterpret_cast <
                                         dir_result_t * >(dirp));
}

extern "C" void ceph_seekdir(struct ceph_mount_info *cmount,
                             struct ceph_dir_result *dirp, int64_t offset)
{
    if (!cmount->is_mounted()) {
        return;
    }
    cmount->get_client()->seekdir(reinterpret_cast < dir_result_t * >(dirp),
                                  offset);
}
...
extern "C" int ceph_unlinkat(struct ceph_mount_info *cmount, int dirfd,
                             const char *relpath, int flags)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->unlinkat(dirfd, relpath, flags,
                                          cmount->default_perms);
}
...
// dirs
extern "C" int ceph_mkdir(struct ceph_mount_info *cmount, const char *path,
                          mode_t mode)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_mkdirat(struct ceph_mount_info *cmount, int dirfd,
                            const char *relpath, mode_t mode)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->mkdirat(dirfd, relpath, mode,
                                         cmount->default_perms);
}

extern "C" int ceph_mksnap(struct ceph_mount_info *cmount, const char *path,
                           ...
                           size_t nr_snap_metadata)
{
    ...
    return cmount->get_client()->mksnap(path, name, cmount->default_perms, mode,
                                        metadata);
}

extern "C" int ceph_rmsnap(struct ceph_mount_info *cmount, const char *path,
                           const char *name)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_mkdirs(struct ceph_mount_info *cmount, const char *path,
                           mode_t mode)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
                               ...
                               const char *relpath, char *buf, int64_t size)
{
    ...
    return cmount->get_client()->readlinkat(dirfd, relpath, buf, size,
                                            cmount->default_perms);
}

extern "C" int ceph_symlink(struct ceph_mount_info *cmount,
...
extern "C" int ceph_symlinkat(struct ceph_mount_info *cmount,
                              const char *existing, int dirfd,
                              const char *newname)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->symlinkat(existing, dirfd, newname,
                                           cmount->default_perms);
}

extern "C" int ceph_fstatx(struct ceph_mount_info *cmount, int fd,
...
extern "C" int ceph_statxat(struct ceph_mount_info *cmount, int dirfd,
                            const char *relpath, struct ceph_statx *stx,
                            unsigned int want, unsigned int flags)
{
    ...
    return cmount->get_client()->statxat(dirfd, relpath, stx,
                                         cmount->default_perms, want, flags);
}
                              ...
                              struct ceph_statx *stx, int mask)
{
    ...
    return cmount->get_client()->fsetattrx(fd, stx, mask,
                                           cmount->default_perms);
}

extern "C" int ceph_setattrx(struct ceph_mount_info *cmount,
                             const char *relpath, struct ceph_statx *stx,
                             int mask, int flags)
{
    if (!cmount->is_mounted()) {
...
extern "C" int ceph_getxattr(struct ceph_mount_info *cmount, const char *path,
...
extern "C" int ceph_lgetxattr(struct ceph_mount_info *cmount, const char *path,
                              const char *name, void *value, size_t size)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_fgetxattr(struct ceph_mount_info *cmount, int fd,
                              const char *name, void *value, size_t size)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_listxattr(struct ceph_mount_info *cmount, const char *path,
...
extern "C" int ceph_llistxattr(struct ceph_mount_info *cmount, const char *path,
                               char *list, size_t size)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_flistxattr(struct ceph_mount_info *cmount, int fd,
                               char *list, size_t size)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_removexattr(struct ceph_mount_info *cmount,
...
extern "C" int ceph_lremovexattr(struct ceph_mount_info *cmount,
                                 const char *path, const char *name)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_fremovexattr(struct ceph_mount_info *cmount, int fd,
                                 const char *name)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_setxattr(struct ceph_mount_info *cmount, const char *path,
...
extern "C" int ceph_lsetxattr(struct ceph_mount_info *cmount, const char *path,
                              const char *name, const void *value, size_t size,
                              int flags)
{
    if (!cmount->is_mounted()) {
...
extern "C" int ceph_fsetxattr(struct ceph_mount_info *cmount, int fd,
                              const char *name, const void *value, size_t size,
                              int flags)
{
    if (!cmount->is_mounted()) {
...
extern "C" int ceph_fstat(struct ceph_mount_info *cmount, int fd,
                          struct stat *stbuf)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern int ceph_lstat(struct ceph_mount_info *cmount, const char *path,
                      struct stat *stbuf)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_chmod(struct ceph_mount_info *cmount, const char *path,
                          mode_t mode)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_lchmod(struct ceph_mount_info *cmount, const char *path,
                           mode_t mode)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_chmodat(struct ceph_mount_info *cmount, int dirfd,
                            const char *relpath, mode_t mode, int flags)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->chmodat(dirfd, relpath, mode, flags,
                                         cmount->default_perms);
}
...
extern "C" int ceph_chownat(struct ceph_mount_info *cmount, int dirfd,
                            const char *relpath, uid_t uid, gid_t gid,
                            int flags)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->chownat(dirfd, relpath, uid, gid, flags,
                                         cmount->default_perms);
}
...
extern "C" int ceph_utimensat(struct ceph_mount_info *cmount, int dirfd,
                              const char *relpath, struct timespec times[2],
                              int flags)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->utimensat(dirfd, relpath, times, flags,
                                           cmount->default_perms);
}
...
extern "C" int ceph_openat(struct ceph_mount_info *cmount, int dirfd,
                           const char *relpath, int flags, mode_t mode)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
    }
    return cmount->get_client()->openat(dirfd, relpath, flags,
                                        cmount->default_perms, mode);
}

extern "C" int ceph_open_layout(struct ceph_mount_info *cmount,
...
extern "C" int ceph_write(struct ceph_mount_info *cmount, int fd,
...
extern "C" int ceph_ftruncate(struct ceph_mount_info *cmount, int fd,
                              int64_t size)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_fsync(struct ceph_mount_info *cmount, int fd,
                          int syncdataonly)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_get_path_stripe_unit(struct ceph_mount_info *cmount,
        const char *path)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_get_file_stripe_count(struct ceph_mount_info *cmount,
        int fh)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_get_path_stripe_count(struct ceph_mount_info *cmount,
        const char *path)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_get_path_object_size(struct ceph_mount_info *cmount,
        const char *path)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_get_path_pool(struct ceph_mount_info *cmount,
                                  const char *path)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_get_file_pool_name(struct ceph_mount_info *cmount, int fh,
                                       char *buf, size_t len)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_get_pool_name(struct ceph_mount_info *cmount, int pool,
                                  char *buf, size_t len)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_get_path_pool_name(struct ceph_mount_info *cmount,
...
extern "C" int ceph_get_default_data_pool_name(struct ceph_mount_info *cmount,
        char *buf, size_t len)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_get_file_layout(struct ceph_mount_info *cmount, int fh,
                                    int *stripe_unit, int *stripe_count,
...
extern "C" int ceph_get_path_layout(struct ceph_mount_info *cmount,
                                    const char *path, int *stripe_unit,
...
extern "C" int ceph_get_path_replication(struct ceph_mount_info *cmount,
        const char *path)
{
    file_layout_t l;
    int r;
...
extern "C" int ceph_set_default_file_stripe_count(struct ceph_mount_info
        *cmount, int count)
{
    // this option no longer exists
    return -CEPHFS_EOPNOTSUPP;
...
extern "C" int ceph_set_default_object_size(struct ceph_mount_info *cmount,
        int size)
{
    // this option no longer exists
    return -CEPHFS_EOPNOTSUPP;
...
extern "C" int ceph_set_default_preferred_pg(struct ceph_mount_info *cmount,
        int osd)
{
    // this option no longer exists
    return -CEPHFS_EOPNOTSUPP;
...
extern "C" int ceph_debug_get_file_caps(struct ceph_mount_info *cmount,
                                        const char *path)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" int ceph_get_pool_id(struct ceph_mount_info *cmount,
...
extern "C" int ceph_ll_lookup_vino(struct ceph_mount_info *cmount,
                                   vinodeno_t vino, Inode **inode)
{
    return (cmount->get_client())->ll_lookup_vino(vino, cmount->default_perms,
            inode);
}
 ...
 * parent dentry.
 */
extern "C" int ceph_ll_lookup_inode(struct ceph_mount_info *cmount,
                                    struct inodeno_t ino, Inode **inode)
{
    return (cmount->get_client())->ll_lookup_inode(ino, cmount->default_perms,
            inode);
}
                               ...
                               const UserPerm *perms)
{
    return (cmount->get_client()->
            ll_opendir(in, O_RDONLY, (dir_result_t **) dirpp, *perms));
}
...
{
    return (cmount->get_client()->
            ll_setxattr(in, name, value, size, flags, *perms));
}
...
extern "C" int ceph_set_deleg_timeout(class ceph_mount_info *cmount,
                                      uint32_t timeout)
{
    if (!cmount->is_mounted()) {
        return -CEPHFS_ENOTCONN;
...
extern "C" void ceph_set_session_timeout(class ceph_mount_info *cmount,
        unsigned timeout)
{
                                  ...
                                  const char *path, struct snap_info *snap_info)
{
    ...
    if (nr_metadata) {
        snap_info->snap_metadata =
            (struct snap_metadata *)calloc(nr_metadata,
