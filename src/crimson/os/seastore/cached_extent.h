...
} template < typename T > class read_set_item_t
{
    ...
    using set_hook_options = boost::intrusive::member_hook <
                             read_set_item_t, set_hook_t, &read_set_item_t::trans_hook >;

public:
    struct cmp_t {
        using is_transparent = paddr_t;
        bool operator()(const read_set_item_t < T > &lhs,
                        const read_set_item_t &rhs)const;
        bool operator()(const paddr_t &lhs,
    ...
    struct trans_cmp_t {
        bool operator()(const read_set_item_t < Transaction > &lhs,
                        const read_set_item_t < Transaction > &rhs)const
        {
            return lhs.t < rhs.t;
        } bool operator()(const Transaction *lhs,
                          const read_set_item_t < Transaction >
                          &rhs)const
        {
            return lhs < rhs.t;
        } bool operator()(const read_set_item_t < Transaction > &lhs,
...
using read_set_t = std::set <
                   read_set_item_t < T >, typename read_set_item_t < T >::cmp_t >;
...
struct trans_spec_view_t {
    ...
    struct cmp_t {
        bool operator()(const trans_spec_view_t &lhs,
                        const trans_spec_view_t &rhs)const
        {
            return lhs.pending_for_transaction <
                   rhs.pending_for_transaction;
        } bool operator()(const transaction_id_t &lhs,
                          const trans_spec_view_t &rhs)const
        {
    ...
    using trans_view_member_options =
        ...
        trans_spec_view_t,
        trans_view_hook_t, &trans_spec_view_t::trans_view_hook >;
...
class CachedExtent: public boost::intrusive_ref_counter <
    CachedExtent, boost::thread_unsafe_counter >, public trans_spec_view_t
{
...
public:
    void init(extent_state_t _state,
              ...
              rewrite_gen_t gen, transaction_id_t trans_id)
    {
    ...
    virtual std::ostream &print_detail(std::ostream &out) const
    {
        return out;
    } std::ostream &print(std::ostream &out) const
    {
        ...
        << ", modify_time=" << sea_time_point_printer_t {
            modify_time} << ", paddr=" << get_paddr()
                     ...
                     << ", refcount=" << use_count()
                     << ", user_hint=" << user_hint
        << ", rewrite_gen=" << rewrite_gen_printer_t {
            rewrite_generation};
     ...
     */
    virtual void apply_delta_and_adjust_crc(paddr_t base,
    ...
    /// Returns true if extent is a fresh extent bool is_initial_pending() const {
    return state == extent_state_t::INITIAL_WRITE_PENDING;
}
...
/// Returns true if extent is dirty (has deltas on disk) bool is_dirty() const {
ceph_assert(is_valid());
return !is_clean();
}
...
{
    return !is_valid() || (is_mutation_pending()
                           && !prior_instance->is_valid());
}
/// Returns true if extent is a plcaeholder bool is_placeholder() const {
...
/// Returns length of extent virtual extent_len_t get_length() const {
return ptr.length();
}
...
/// Returns crc32c of buffer uint32_t get_crc32c() {
return ceph_crc32c(1,
                   reinterpret_cast <
...
/// Compare by paddr
friend bool operator<(const CachedExtent &a,
                      const CachedExtent &b)
{
...
using index =
    boost::intrusive::set < CachedExtent, index_member_options >;
friend class ExtentIndex;
...
using list = boost::intrusive::list <
             CachedExtent, primary_ref_list_member_options >;
...
CachedExtent(const CachedExtent &other)
    ...
    version(other.version), poffset(other.poffset)
{
}
...
CachedExtent(const CachedExtent &other,
             share_buffer_t): state(other.state),
    dirty_from_or_retired_at(other.dirty_from_or_retired_at),
    ptr(other.ptr), version(other.version), poffset(other.poffset)
{
}

struct retired_placeholder_t {
};
CachedExtent(retired_placeholder_t): state(extent_state_t::
            INVALID)
{
}
...
static TCachedExtentRef < T > make_cached_extent_ref(Args
        && ... args)
{
...
template < typename T, typename C > class addr_extent_list_base_t: public std::list < std::pair < T,
    C >>
{
};
...
template < typename T, typename C, typename Cmp > class addr_extent_set_base_t: public std::set < C,
    Cmp >
{
};

using pextent_set_t = addr_extent_set_base_t <
                      paddr_t, CachedExtentRef, ref_paddr_cmp >;

template < typename T >
using t_pextent_list_t =
    addr_extent_list_base_t < paddr_t, TCachedExtentRef < T >>;
...
public:
    auto get_overlap(paddr_t addr, extent_len_t len)
    {
        ...
        auto top =
            extent_index.lower_bound(addr.add_offset(len), paddr_cmp());
    ...
    void erase(CachedExtent &extent)
    {
        ...
        [[maybe_unused]] auto erased =
            extent_index.erase(extent_index.s_iterator_to(extent));
        extent.parent_index = nullptr;
...
template < typename T > struct get_child_ret_t {
    std::variant < child_pos_t,
        seastar::future < TCachedExtentRef < T >>> ret;
...
using PhysicalNodeMappingRef =
    std::unique_ptr < PhysicalNodeMapping < key_t, val_t >>;
...
{
public:
    template < typename ... T > ChildableCachedExtent(T
            && ... t): CachedExtent(std::forward < T >
                                        (t) ...)
    {
...
{
public:
    template < typename ... T > LogicalCachedExtent(T && ... t)
...
template < typename T >
inline bool read_set_item_t <
T >::cmp_t::operator()(const read_set_item_t < T > &lhs,
                       const read_set_item_t < T > &rhs)const
{
...
template < typename T >
inline bool read_set_item_t <
T >::cmp_t::operator()(const paddr_t &lhs,
                       const read_set_item_t < T > &rhs)const
{
...
using lextent_set_t = addr_extent_set_base_t <
                      laddr_t, LogicalCachedExtentRef, ref_laddr_cmp >;
...
template <> struct fmt::formatter <
    crimson::os::seastore::lba_pin_list_t > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::CachedExtent > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::LogicalCachedExtent > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::LBAMapping > : fmt::ostream_formatter {
};
#endif
