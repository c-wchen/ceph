...
template <> struct fmt::formatter <
    crimson::os::seastore::op_type_t >:fmt::formatter < std::string_view > {
    ...
    template < typename FormatContext >
        ...
        case op_type_t::TRANSACTION:
            name = "transaction";
            break;
            case op_type_t::READ:name = "read";
            break;
            case op_type_t::WRITE:name = "write";
            break;
            case op_type_t::GET_ATTR:name = "get_attr";
            break;
            case op_type_t::GET_ATTRS:name = "get_attrs";
            break;
            case op_type_t::STAT:name = "stat";
            break;
            case op_type_t::OMAP_GET_VALUES:name = "omap_get_values";
            break;
            case op_type_t::OMAP_LIST:name = "omap_list";
            break;
            case op_type_t::MAX:name = "unknown";
            break;
...
namespace crimson::os::seastore {
    ...
    class FileMDStore final:public SeaStore::MDStore {
      ...
      public:
        ...
        } write_meta_ret write_meta(const std::string & key,
  ...
  :    root(root), mdstore(std::move(mdstore)) {
    ...
    void SeaStore::Shard::register_metrics() {
        ...
        std::pair < op_type_t, sm::label_instance > labels_by_op_type[] = {
            {
            op_type_t::TRANSACTION, sm::label_instance("latency",
                                                           "TRANSACTION")}, {
            ...
            op_type_t::OMAP_GET_VALUES, sm::label_instance("latency",
                                                               "OMAP_GET_VALUES")},
            {
      ...
      for (auto &[op_type, label]:labels_by_op_type) {
        ...
        metrics.add_group("seastore", {
                          sm::make_gauge("concurrent_transactions",[this] {
                                         return throttler.get_current();}
                                         ,
                                         sm::
                                         description
                                         ("transactions that are running inside seastore")
                          ), sm::make_gauge("pending_transactions",[this] {
                                            return throttler.get_pending();}
                                            ,
                                            sm::
                                            description
                                            ("transactions waiting to get "
    ...
    SeaStore::mount_ertr::future <> SeaStore::mount() {
        ...
        return device->mount().safe_then([this] {
                                         auto sec_devices =
                                         device->get_sharded_device().
                                         get_secondary_devices();
                                         return crimson::
                                         do_for_each(sec_devices,
                                                     [this] (auto &
                                                             device_entry) {
                                                     device_id_t id =
                                                     device_entry.first;
                                                     magic_t magic =
                                                     device_entry.second.magic;
                                                     device_type_t dtype =
                                                     device_entry.second.dtype;
                                                     std::string path =
                                                     fmt::
                                                     format("{}/block.{}.{}",
                                                            root, dtype,
    ...
    seastar::future <> SeaStore::Shard::umount() {
        ...
        ().safe_then([this] {
                    ...
                    collection_manager.reset(); onode_manager.reset();}
        ).handle_error(crimson::ct_error::assert_all {
    ...
    seastar::future <> SeaStore::write_fsid(uuid_d new_osd_fsid) {
        ...
        return read_meta("fsid").then([this, FNAME, new_osd_fsid] (auto tuple) {
                                      auto[ret, fsid] = tuple;
                                      std::string str_fsid =
                                      stringify(new_osd_fsid); if (ret == -1) {
                                      return write_meta("fsid",
                                                        stringify
                                                        (new_osd_fsid));}
                                      else
                                      if (ret == 0 && fsid != str_fsid) {
                                      ERROR("on-disk fsid {} != provided {}",
                                            fsid, stringify(new_osd_fsid));
                                      throw std::
                                      runtime_error("store fsid error");}
                                      else {
                                      return seastar::now();}
                                      }
        ) ;
    ...
    seastar::future <> SeaStore::Shard::mkfs_managers() {
        init_managers();
        return transaction_manager->mkfs().safe_then([this] {
                                                     init_managers();
                                                     return
                                                     transaction_manager->
                                                     mount();}
                    ...
                    return repeat_eagain([this] {
                                         ...
                                         with_transaction_intr(Transaction::
                                                               src_t::MUTATE,
                                                               "mkfs_seastore",
                                                               [this] (auto & t)
                                                               {
    ...
    seastar::future <> SeaStore::set_secondaries() {
        ...
        return shard_stores.invoke_on_all([sec_dev] (auto & local_store) {
                                          local_store.set_secondaries(sec_dev->
                                                                      get_sharded_device
                                                                      ());}
        );
    }
    ...
    seastar::future <> SeaStore::prepare_meta(uuid_d new_osd_fsid) {
        ...
        return write_fsid(new_osd_fsid).then([this] {
                                             return read_meta("type").
                                             then([this] (auto tuple) {
                                                  auto[ret, type] = tuple;
                                                  if (ret == 0
                                                      && type == "seastore") {
                                                  return seastar::now();}
                                                  ...
                                                  LOG_PREFIX(SeaStore::
                                                             prepare_meta);
                                                  ERROR
                                                  ("expected seastore, but type is {}",
                                                   type);
                                                  throw std::
                                                  runtime_error
                                                  ("store type error");}
                                                  else {
                                                  return write_meta("type",
                                                                    "seastore");}
                                                  }
                                             );}
        )
    ...
    SeaStore::mkfs_ertr::future <> SeaStore::mkfs(uuid_d new_osd_fsid) {
        ...
        return read_meta("mkfs_done").then([this, new_osd_fsid] (auto tuple) {
                                           ...
                                           return seastar::
                                           do_with(secondary_device_set_t(),
                                                   [this,
                                                    new_osd_fsid] (auto & sds) {
    ...
    seastar::future < std::vector < coll_core_t >> SeaStore::list_collections() {
        ...
        return shard_stores.map([](auto & local_store) {
          ...
          std::vector < coll_core_t > collections; for (auto & colls:results)
               {
               collections.insert(collections.end(), colls.begin(),
                                  colls.end());}
               return seastar::make_ready_future < std::vector <
               coll_core_t >> (std::move(collections));}
        ) ;
    }
    ...
    store_statfs_t SeaStore::Shard::stat() const {
        ...
        DEBUG("");
        return shard_stores.map_reduce0([](const SeaStore::Shard & local_store) {
                                        return local_store.stat();}
                                        ,
                                        store_statfs_t(),
                                        [](auto && ss, auto && ret) {
                                        ss.add(ret); return std::move(ss);}
        ...
        .si_then([this, ch, &t] (auto coll_root) {
                 return collection_manager->list(coll_root, t);}
                  ...
                  auto it = std::find_if(colls.begin(), colls.end(),
                                         [ch] (const std::pair < coll_t,
                                               coll_info_t > &element) {
                                         return element.first == ch->get_cid();}
                  ); if (it != colls.end()) {
                  return TransactionManager::read_extent_iertr::
                  make_ready_future < std::optional <
                  unsigned >>(it->second.split_bits);}
                  else {
                  return TransactionManager::read_extent_iertr::
                  make_ready_future < std::optional <
                  unsigned >>(std::nullopt);}
                  }) ;
    }
    ...
    seastar::future < std::tuple < std::vector < ghobject_t >, ghobject_t >>
        ...
        return seastar::do_with(RetType(std::vector < ghobject_t > (), start),
                                std::move(limit),
                                [this, ch, start, end] (auto & ret,
                                                        auto & limit) {
                                return
                                repeat_eagain([this, ch, start, end, &limit,
                                               &ret] {
                                              return transaction_manager->
                                              with_transaction_intr
                                              (Transaction::src_t::READ,
                                               "list_objects",[this, ch, start,
                                                               end, &limit,
                                                               &ret] (auto & t)
                                               {
                                               return get_coll_bits(ch,
                                                                    t).
                                               si_then([this, ch, &t, start,
                                                        end, &limit,
                                                        &ret] (auto bits) {
                                                       if (!bits) {
                                                       return list_iertr::
    ...
    seastar::future < CollectionRef >
        ...
        return list_collections().then([cid, this] (auto colls_cores) {
                                       ...
                                       CollectionRef > (_get_collection(cid));}
                                       else {
                                       return seastar::make_ready_future <
                                       CollectionRef > ();}
                                       }
    ...
    seastar::future < std::vector < coll_core_t >>
        ...
        return seastar::do_with(std::vector < coll_core_t > (),
                                [this] (auto & ret) {
    ...
    SeaStore::Shard::read_errorator::future < ceph::bufferlist >
        SeaStore::Shard::read(CollectionRef ch,
    ...
    SeaStore::Shard::read_errorator::future < ceph::bufferlist >
        SeaStore::Shard::readv(CollectionRef ch,
                               const ghobject_t & _oid,
                               interval_set < uint64_t > &m,
                               uint32_t op_flags) {
        return seastar::do_with(_oid, ceph::bufferlist {
                                }
    ...
    SeaStore::Shard::get_attr_errorator::future < ceph::bufferlist >
        SeaStore::Shard::get_attr(CollectionRef ch,
                                  const ghobject_t & oid,
                                  std::string_view name)const {
        auto c = static_cast < SeastoreCollection * >(ch.get());
        LOG_PREFIX(SeaStore::get_attr);
        DEBUG("{} {}", c->get_cid(), oid);
        return repeat_with_onode < ceph::bufferlist > (c,
                                                       oid,
                                                       Transaction::src_t::READ,
                                                       "get_attr",
                                                       op_type_t::GET_ATTR,
    ...
    SeaStore::Shard::get_attrs_ertr::future < SeaStore::Shard::attrs_t >
        SeaStore::Shard::get_attrs(CollectionRef ch, const ghobject_t & oid) {
    ...
    seastar::future < struct stat >SeaStore::Shard::stat(CollectionRef c,
                                                         const ghobject_t & oid)
    {
        LOG_PREFIX(SeaStore::stat);
    ...
    SeaStore::Shard::get_attr_errorator::future < ceph::bufferlist >
        SeaStore::Shard::omap_get_header(CollectionRef ch,
                                         const ghobject_t & oid) {
        return get_attr(ch, oid, OMAP_HEADER_XATTR_KEY);
    }

    SeaStore::Shard::read_errorator::future < SeaStore::Shard::omap_values_t >
        SeaStore::Shard::omap_get_values(CollectionRef ch,
                                         const ghobject_t & oid,
                                         const omap_keys_t & keys) {
        auto c = static_cast < SeastoreCollection * >(ch.get());
        return repeat_with_onode < omap_values_t > (c,
                                                    oid,
                                                    Transaction::src_t::READ,
                                                    "omap_get_values",
    ...
    SeaStore::Shard::_omap_get_value_ret
        SeaStore::Shard::_omap_get_value(Transaction & t,
                                         omap_root_t && root,
        ...
        return seastar::do_with(BtreeOMapManager(*transaction_manager),
                                ...
                                [&t] (auto & manager, auto & root,
                                      auto & key)->_omap_get_value_ret {
                                if (root.is_null()) {
                                return crimson::ct_error::enodata::make();}
                                return manager.omap_get_value(root, t,
                                                              key).
                                si_then([](auto opt)->_omap_get_value_ret {
                                        if (!opt) {
                                        return crimson::ct_error::enodata::
                                        make();}
                                        return seastar::make_ready_future <
                                        ceph::bufferlist > (std::move(*opt));}
                                );}
        ) ;
    }

    SeaStore::Shard::_omap_get_values_ret
        SeaStore::Shard::_omap_get_values(Transaction & t,
                                          omap_root_t && omap_root,
        ...
        if (omap_root.is_null()) {
            return seastar::make_ready_future < omap_values_t > ();
                                  ...
                                  return trans_intr::do_for_each(keys.begin(),
                                                                 keys.end(),
                                                                 [&](auto & key)
                                                                 {
                                                                 return manager.
    ...
    SeaStore::Shard::omap_list_ret
        SeaStore::Shard::omap_list(Onode & onode,
                                   ...
                                   OMapManager::omap_list_config_t config)const
    {
        ...
        if (root.is_null()) {
            return seastar::make_ready_future < omap_list_bare_ret > (true,
                                                                      omap_values_t
                                                                      {
                                                                      });
        }
        return seastar::do_with(BtreeOMapManager(*transaction_manager),
                                ...
                                [&t, config] (auto & manager, auto & root,
                                              auto & start, auto & end) {
                                return manager.omap_list(root, t, start, end,
                                                         config);}
        );
    }
                                         ...
                                         const std::optional < string > &start)
    {
        ...
        return repeat_with_onode < ret_bare_t > (c,
                                                 oid,
                                                 Transaction::src_t::READ,
                                                 "omap_list",
                                                 op_type_t::OMAP_LIST,
    ...
    SeaStore::Shard::_fiemap_ret SeaStore::Shard::_fiemap(Transaction & t,
                                                          Onode & onode,
                                                          uint64_t off,
                                                          uint64_t len) const {
        return seastar::do_with(ObjectDataHandler(max_object_size),
                                ...
                                return objhandler.
                                fiemap(ObjectDataHandler::context_t {
                                       *transaction_manager, t, onode,}
                                       , off, len);}
        );
    }

    SeaStore::Shard::read_errorator::future < std::map < uint64_t, uint64_t >>
        SeaStore::Shard::fiemap(CollectionRef ch,
                                const ghobject_t & oid,
                                uint64_t off, uint64_t len) {
        LOG_PREFIX(SeaStore::fiemap);
        DEBUG("oid: {}, off: {}, len: {} ", oid, off, len);
        return repeat_with_onode < std::map < uint64_t, uint64_t >> (ch,
                                                                     oid,
                                                                     Transaction::
                                                                     src_t::
                                                                     READ,
                                                                     ...
                                                                     if (off >=
                                                                         size) {
                                                                     INFOT
                                                                     ("fiemap offset is over onode size!",
                                                                      t);
                                                                     return
                                                                     seastar::
                                                                     make_ready_future
    ...
    seastar::future <> SeaStore::Shard::
        do_transaction_no_callbacks(CollectionRef _ch, ceph::os::Transaction
                                    && _t) {
        // repeat_with_internal_context ensures ordering via collection lock
        return repeat_with_internal_context(_ch,
                                            std::move(_t),
                                            Transaction::src_t::MUTATE,
                                            "do_transaction",
                                            op_type_t::TRANSACTION,
                                            [this] (auto & ctx) {
                                            return with_trans_intr(*ctx.
                                                                   transaction,
                                                                   [&,
                                                                    this] (auto
                                                                           & t)
                                                                   ...
                                                                   do_with(std::
                                                                           ...
                                                                           trans_intr::
                                                                           repeat
                                                                           ([this, &ctx, &onodes, &d_onodes] ()mutable->tm_iertr::future < seastar::stop_iteration > {
                                                                            if
                                                                            (ctx.
                                                                             iter.
                                                                             have_op
    ...
    seastar::future <> SeaStore::Shard::flush(CollectionRef ch) {
        return seastar::do_with(get_dummy_ordering_handle(),
                                [this, ch] (auto & handle) {
                                return handle.take_collection_lock(static_cast <
                                                                   SeastoreCollection
                                                                   & >(*ch).
                                                                   ordering_lock).
                                then([this, &handle] {
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_do_transaction_step(internal_context_t & ctx,
                                              CollectionRef & col,
        ...
        if (!onodes[op->oid]) {
            if (!create) {
                fut =
                    onode_manager->get_onode(*ctx.transaction,
                                             i.get_oid(op->oid));
            }
            else {
                fut =
                    onode_manager->get_or_create_onode(*ctx.transaction,
                                                       i.get_oid(op->oid));
            }
        }
        return fut.si_then([&, op, this] (auto && get_onode)->tm_ret {
                           LOG_PREFIX(SeaStore::_do_transaction_step);
                           OnodeRef & o = onodes[op->oid]; if (!o) {
                           assert(get_onode);
                           o = get_onode; d_onodes.push_back(get_onode);}
                           try {
                           switch (op->op) {
case Transaction::OP_REMOVE:
                           {
                           TRACET("removing {}", *ctx.transaction,
                                  i.get_oid(op->oid));
...
case Transaction::OP_WRITE:
                           ...
                           ceph::bufferlist bl;
                           i.decode_bl(bl);
                           return _write(ctx, onodes[op->oid], off, len,
...
case Transaction::OP_SETATTR:
                           ...
                           i.decode_bl(bl);
                           return _setattrs(ctx, onodes[op->oid],
                                            std::move(to_set));}
...
case Transaction::OP_OMAP_SETKEYS:
                           ...
                           return _omap_set_values(ctx, onodes[op->oid],
                                                   std::move(aset));}
case Transaction::OP_OMAP_SETHEADER:
                           {
                           ceph::bufferlist bl;
                           i.decode_bl(bl);
                           return _omap_set_header(ctx, onodes[op->oid],
                                                   std::move(bl));}
case Transaction::OP_OMAP_RMKEYS:
                           {
                           omap_keys_t keys;
                           i.decode_keyset(keys);
                           return _omap_rmkeys(ctx, onodes[op->oid],
                                               std::move(keys));}
case Transaction::OP_OMAP_RMKEYRANGE:
                           ...
                           return _omap_rmkeyrange(ctx, onodes[op->oid],
                                                   std::move(first),
                                                   std::move(last));}
...
default:
    ...
    SeaStore::Shard::tm_ret
        ...
        auto omap_root =
            onode->get_layout().omap_root.get(onode->
                                              get_metadata_hint(device->
                                                                get_block_size
                                                                ()));
            ...
            fut = seastar::do_with(BtreeOMapManager(*transaction_manager),
                                   onode->get_layout().omap_root.get(onode->
                                                                     ...
                                                                     (device->
                                                                      get_block_size
                                                                      ())),
                                   [&ctx, onode] (auto & omap_manager,
                                                  auto & omap_root) {
                                   return omap_manager.omap_clear(omap_root,
                                                                  *ctx.
                                                                  transaction);}
            );
        }
        return fut.si_then([this, &ctx, onode] {
                           return seastar::
                           do_with(ObjectDataHandler(max_object_size),
                                   [ =, this, &ctx] (auto & objhandler) {
                                   return objhandler.
                                   clear(ObjectDataHandler::context_t {
                                         *transaction_manager, *ctx.transaction,
                                         *onode,}
                                   );}
                           );}
        ).si_then([this, &ctx, onode] ()mutable {
                  return onode_manager->erase_onode(*ctx.transaction, onode);}
        ).handle_error_interruptible(crimson::ct_error::input_output_error::
                                     pass_further(),
                                     crimson::ct_error::
                                     assert_all
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_touch(internal_context_t & ctx, OnodeRef & onode) {
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_write(internal_context_t & ctx,
                                OnodeRef & onode,
        ...
        {
            auto & object_size =
                onode->get_mutable_layout(*ctx.transaction).size;
            object_size = std::max < uint64_t > (offset + len, object_size);
        }
        return seastar::do_with(std::move(_bl),
                                ObjectDataHandler(max_object_size),
                                [ =, this, &ctx, &onode] (auto & bl,
                                                          auto & objhandler) {
                                return objhandler.
                                write(ObjectDataHandler::context_t {
                                      *transaction_manager, *ctx.transaction,
                                      *onode,}
                                      , offset, bl);}
        );
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_zero(internal_context_t & ctx,
                               OnodeRef & onode,
        ...
        return seastar::do_with(ObjectDataHandler(max_object_size),
                                ...
                                return objhandler.
                                zero(ObjectDataHandler::context_t {
                                     *transaction_manager, *ctx.transaction,
                                     *onode,}
                                     , offset, len);}
        );
    ...
    SeaStore::Shard::omap_set_kvs_ret
        SeaStore::Shard::_omap_set_kvs(OnodeRef & onode,
                                       const omap_root_le_t & omap_root,
                                       Transaction & t,
        ...
        return seastar::do_with(BtreeOMapManager(*transaction_manager),
                                omap_root.get(onode->
                                              get_metadata_hint(device->
                                                                ...
                                                                ())),[&, keys =
                                                                      std::
                                                                      move(kvs)]
                                (auto & omap_manager, auto & root) {
                                tm_iertr::future <> maybe_create_root =
                                !root.is_null()? tm_iertr::now() : omap_manager.
                                initialize_omap(t,
                                                onode->
                                                get_metadata_hint(device->
                                                                  get_block_size
                                ...
                                return maybe_create_root.si_then([&, keys =
                                                                  std::
                                                                  move(keys)]
                                                                 ()mutable {
                                                                 return
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_omap_set_values(internal_context_t & ctx,
                                          OnodeRef & onode,
                                          std::map < std::string,
                                          ceph::bufferlist > &&aset) {
        ...
        return _omap_set_kvs(onode,
                             ...
                             *ctx.transaction,
                             ...
                             omap_root, std::move(aset));
    }

    SeaStore::Shard::tm_ret
        SeaStore::Shard::_omap_set_header(internal_context_t & ctx,
                                          OnodeRef & onode,
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_omap_clear(internal_context_t & ctx,
                                     OnodeRef & onode) {
        ...
        return _xattr_rmattr(ctx, onode, std::string(OMAP_HEADER_XATTR_KEY))
            .si_then([this, &ctx, &onode] ()->tm_ret {
                     ...
                     return seastar::
                     do_with(BtreeOMapManager(*transaction_manager),
                             onode->get_layout().omap_root.get(onode->
                                                               get_metadata_hint
                                                               (device->
                                                                get_block_size
                                                                ())),[&ctx,
                                                                      &onode]
                             (auto & omap_manager, auto & omap_root) {
                             return omap_manager.omap_clear(omap_root,
                                                            *ctx.transaction)
                             .si_then([&] {
                                      if (omap_root.must_update()) {
                                      onode->get_mutable_layout(*ctx.
                                                                transaction).
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_omap_rmkeys(internal_context_t & ctx,
                                      OnodeRef & onode, omap_keys_t && keys) {
            ...
            return seastar::do_with(BtreeOMapManager(*transaction_manager),
                                    onode->get_layout().omap_root.get(onode->
                                                                      ...
                                                                      (device->
                                                                       get_block_size
                                                                       ())),
                                    std::move(first), std::move(last),[&ctx,
                                                                       &onode]
                                    (auto & omap_manager, auto & omap_root,
                                    ...
                                    .without_max();
                                    return omap_manager.
                                    omap_rm_key_range(omap_root,
                                                      *ctx.transaction, first,
                                                      last,
                                                      config).si_then([&] {
                                                                      if
                                                                      (omap_root.
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_truncate(internal_context_t & ctx,
        ...
        return seastar::do_with(ObjectDataHandler(max_object_size),
                                [ =, this, &ctx, &onode] (auto & objhandler) {
                                return objhandler.
                                truncate(ObjectDataHandler::context_t {
                                         *transaction_manager, *ctx.transaction,
                                         *onode}
                                         , size);}
        );
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_setattrs(internal_context_t & ctx,
                                   OnodeRef & onode,
                                   std::map < std::string, bufferlist > &&aset)
    {
        ...
        if (auto it = aset.find(OI_ATTR); it != aset.end()) {
            ...
            if (likely(val.length() <= onode_layout_t::MAX_OI_LENGTH)) {
                maybe_inline_memcpy(&layout.oi[0],
                                    val.c_str(),
                                    val.length(),
        ...
        if (auto it = aset.find(SS_ATTR); it != aset.end()) {
            ...
            if (likely(val.length() <= onode_layout_t::MAX_SS_LENGTH)) {
                maybe_inline_memcpy(&layout.ss[0],
                                    val.c_str(),
                                    val.length(),
        ...
        return fut.si_then([this, onode, &ctx, &layout,
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_rmattr(internal_context_t & ctx,
                                 OnodeRef & onode, std::string name) {
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_xattr_rmattr(internal_context_t & ctx,
                                       OnodeRef & onode, std::string && name) {
        LOG_PREFIX(SeaStore::_xattr_rmattr);
        DEBUGT("onode={}", *ctx.transaction, *onode);
        auto xattr_root =
            onode->get_layout().xattr_root.get(onode->
                                               get_metadata_hint(device->
                                                                 get_block_size
                                                                 ()));
            ...
            return seastar::do_with(BtreeOMapManager(*transaction_manager),
                                    ...
                                    std::move(name),[&ctx,
                                                     &onode] (auto &
                                                              ...
                                                              auto & xattr_root,
                                                              auto & name) {
                                    return omap_manager.omap_rm_key(xattr_root,
                                                                    *ctx.
                                                                    transaction,
                                                                    name)
                                    .si_then([&] {
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_rmattrs(internal_context_t & ctx, OnodeRef & onode) {
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_xattr_clear(internal_context_t & ctx,
                                      OnodeRef & onode) {
        ...
        if (xattr_root.is_null()) {
            ...
            return seastar::do_with(BtreeOMapManager(*transaction_manager),
                                    ...
                                    [&ctx, &onode] (auto & omap_manager,
                                                    auto & xattr_root) {
                                    return omap_manager.omap_clear(xattr_root,
                                                                   *ctx.
                                                                   transaction)
                                    .si_then([&] {
                                             if (xattr_root.must_update()) {
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_create_collection(internal_context_t & ctx,
                                            const coll_t & cid, int bits) {
        return transaction_manager->read_collection_root(*ctx.transaction).
            si_then([ =, this, &ctx] (auto _cmroot) {
                    return seastar::do_with(_cmroot,
                                            [ =, this, &ctx] (auto & cmroot) {
                                            return collection_manager->
                                            create(cmroot, *ctx.transaction,
                                                   cid, bits).si_then([this,
                                                                       &ctx,
                                                                       &cmroot]
                                                                      {
                                                                      if
    ...
    SeaStore::Shard::tm_ret
        SeaStore::Shard::_remove_collection(internal_context_t & ctx,
                                            const coll_t & cid) {
        return transaction_manager->read_collection_root(*ctx.transaction).
            si_then([ =, this, &ctx] (auto _cmroot) {
                    return seastar::do_with(_cmroot,
                                            [ =, this, &ctx] (auto & cmroot) {
                                            return collection_manager->
                                            remove(cmroot, *ctx.transaction,
                                                   cid).si_then([this, &ctx,
                                                                 &cmroot] {
    ...
    boost::intrusive_ptr < SeastoreCollection >
        ...
        return new SeastoreCollection {
        cid};
    }

    seastar::future <> SeaStore::Shard::write_meta(const std::string & key,
                                ...
                                return repeat_eagain([this, FNAME, &key, &value] {
                                                     ...
                                                     (Transaction::src_t::
                                                      ...
                                                      [this, FNAME, &key,
                                                       &value] (auto & t) {
                                                      DEBUGT
                                                      ("Have transaction, key: {}; value: {}",
                                                       t, key, value);
                                                      return
                                                      transaction_manager->
                                                      update_root_meta(t, key,
                                                                       value).
                                                      si_then([this, &t] {
                                                              return
                                                              transaction_manager->
    ...
    uuid_d SeaStore::Shard::get_fsid() const {
        ...
        transaction_manager =
            make_transaction_manager(device, secondaries, is_test);
        collection_manager =
