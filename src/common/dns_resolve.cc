...
namespace ceph {
    ...
    } int ResolvHWrapper::res_nsearch(res_state s, const char *hostname,
                                      int cls, int type, u_char * buf,
                                      int bufsz) {
...
#define MAX_FQDN_SIZE 255
...
#ifdef HAVE_RES_NQUERY
        len =
            resolv_h->res_nquery(res, origname, ns_c_in, ns_t_cname, buf,
                                 sizeof(buf));
#else
        {
...
#endif
            len =
                resolv_h->res_query(origname, ns_c_in, ns_t_cname, buf,
                                    sizeof(buf));
        }
#endif
        if (len < 0) {
    ...
    int DNSResolver::resolve_ip_addr(CephContext * cct, res_state * res,
...
#ifdef HAVE_RES_NQUERY
        len =
            resolv_h->res_nquery(*res, hostname.c_str(), ns_c_in, type, nsbuf,
                                 sizeof(nsbuf));
#else
        {
#ifndef HAVE_THREAD_SAFE_RES_QUERY
...
#endif
            len =
                resolv_h->res_query(hostname.c_str(), ns_c_in, type, nsbuf,
                                    sizeof(nsbuf));
        }
#endif
        if (len < 0) {
            ...
            ldout(cct,
                  20) << "no address found for hostname " << hostname << dendl;
            return -1;
        }
        ...
        if (!addr->parse(addr_buf)) {
            lderr(cct) << "failed to parse address '" << (const char *)
                ns_rr_rdata(rr)
                << "'" << dendl;
            return -1;
        }
    ...
    int DNSResolver::resolve_srv_hosts(CephContext * cct,
        ...
        string query_str =
            "_" + service_name + "._" + proto_str + (domain.empty()? "" : "." +
                                                     domain);
        int len;

#ifdef HAVE_RES_NQUERY
        len =
            resolv_h->res_nsearch(res, query_str.c_str(), ns_c_in, ns_t_srv,
                                  nsbuf, sizeof(nsbuf));
#else
        {
#ifndef HAVE_THREAD_SAFE_RES_QUERY
...
#endif
            len =
                resolv_h->res_search(query_str.c_str(), ns_c_in, ns_t_srv,
                                     nsbuf, sizeof(nsbuf));
        }
#endif
        if (len < 0) {
            ...
            ldout(cct,
                  20) << "No hosts found for service " << query_str << dendl;
            return 0;
        }
        ...
        for (int i = 0; i < num_hosts; i++) {
            ...
            string srv_domain =
                full_srv_name.substr(full_srv_name.find(protocol)
                                     + protocol.length());
