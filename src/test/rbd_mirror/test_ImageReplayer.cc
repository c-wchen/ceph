...
class TestImageReplayer:public::rbd::mirror::TestFixture {
  ...
  :    m_local_cluster(new librados::Rados()), m_watch_handle(0) {
        ...
        int order = 0;
        EXPECT_EQ(0,
                  librbd::create(m_remote_ioctx, m_image_name.c_str(), 1 << 22,
        ...
        m_threads.
            reset(new rbd::mirror::
                  Threads <> (reinterpret_cast <
                              CephContext * >(m_local_ioctx.cct())));

        m_service_daemon.
            reset(new rbd::mirror::
                  ServiceDaemon <> (g_ceph_context, m_local_cluster,
                                    m_threads.get()));
        m_image_deleter.
            reset(new rbd::mirror::
                  ImageDeleter <> (m_threads->work_queue, m_threads->timer,
                                   &m_threads->timer_lock,
                                   m_service_daemon.get()));
        m_instance_watcher =
            rbd::mirror::InstanceWatcher <>::create(m_local_ioctx,
    ...
    ~TestImageReplayer()override {
        ...
        EXPECT_EQ(0,
                  g_ceph_context->_conf->
                  set_val("rbd_mirror_journal_commit_age", "5"));
    }

    template < typename ImageReplayerT =
        rbd::mirror::ImageReplayer <> >void create_replayer() {
        m_replayer =
            new ImageReplayerT(m_threads.get(), m_image_deleter.get(),
                               m_instance_watcher,
                               rbd::mirror::RadosRef(new librados::
                                                     Rados(m_local_ioctx)),
                               m_local_mirror_uuid, m_local_ioctx.get_id(),
                               m_global_image_id);
    ...
    void read_test_data(librbd::ImageCtx * ictx, const char *expected,
        ...
        read = ictx->io_work_queue->read(off, len, librbd::io::ReadResult {
                                         result, len}, 0);
        printf("read: %d\n", (int)read);
...
TEST_F(TestImageReplayer, BootstrapMirrorDisabling)
{
    ...
    ASSERT_EQ(0,
              librbd::cls_client::mirror_image_set(&m_remote_ioctx, ictx->id,
                                                   mirror_image));
    close_image(ictx);
...
TEST_F(TestImageReplayer, Resync)
{
    ...
    m_image_deleter->wait_for_scheduled_deletion(m_local_ioctx.get_id(),
                                                 m_replayer->
                                                 get_global_image_id(),
                                                 &delete_ctx);
...
TEST_F(TestImageReplayer, MultipleReplayFailures_SingleEpoch)
{
    ...
    ASSERT_EQ(0,
              ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(),
                                            "foo"));
    ASSERT_EQ(0,
    ...
    for (uint64_t i = 0; i < 10; ++i) {
        ...
        ictx->journal->append_op_event(i, librbd::journal::EventEntry {
                                       librbd::journal::SnapUnprotectEvent {
                                       i,
...
TEST_F(TestImageReplayer, Disconnect)
{
    ...
    EXPECT_EQ(0,
              m_local_cluster->conf_set("rbd_mirroring_resync_after_disconnect",
                                        "false"));
...
TEST_F(TestImageReplayer, UpdateFeatures)
{
    ...
    ASSERT_EQ(-EINVAL,
              ictx->operations->update_features(RBD_FEATURE_EXCLUSIVE_LOCK,
                                                false));
    generate_test_data();
    for (int i = 0; i < TEST_IO_COUNT; ++i) {
        write_test_data(ictx, m_test_data, TEST_IO_SIZE * i, TEST_IO_SIZE);
