                    ...
                    if (formatter != nullptr) {
                      ...
                      for (auto & peer:peers) {
                        ...
                        if (peers.empty()) {
                            ...
                            TextTable tbl;
                            tbl.define_column("", TextTable::LEFT,
                                              TextTable::LEFT);
                            tbl.define_column("UUID", TextTable::LEFT,
                                              TextTable::LEFT);
                            tbl.define_column("NAME", TextTable::LEFT,
                                              TextTable::LEFT);
                            tbl.define_column("CLIENT", TextTable::LEFT,
                                              TextTable::LEFT);
                          for (auto & peer:peers) {
                                tbl << " "
                                    << peer.uuid
                                    << peer.cluster_name
                                    << peer.client_name << TextTable::endrow;
                            }
   ...
   */
                ...
                class StatusImageRequest:public ImageRequestBase {
                  public:
                    ...
                    void finalize_action() override {
                        ...
                        std::string state =
                            utils::
                            mirror_image_status_state(m_mirror_image_status);
                        std::string last_update =
                            (m_mirror_image_status.last_update ==
                             0 ? "" : utils::timestr(m_mirror_image_status.
                                                     last_update));
                  ...
                  :    m_io_ctx(io_ctx),
                        m_factory(std::
                                  bind(ImageRequestAllocator < RequestT > (),
                                       ...
                                       std::placeholders::_1,
                                       std::forward < Args > (args) ...)),
                        m_throttle(g_conf->get_val < int64_t >
            ...
            int execute_peer_add(const po::variables_map & vm) {
                ...
                if (!mirror_peers.empty()) {
                    std::
                        cerr <<
                        "rbd: multiple peers are not currently supported" <<
                        std::endl;
                    return -EINVAL;
                }

                std::string uuid;
                r = rbd.mirror_peer_add(io_ctx, &uuid, remote_cluster,
                                        remote_client_name);
                if (r < 0) {
            ...
            int execute_peer_set(const po::variables_map & vm) {
                ...
                if (key != "client" && key != "cluster") {
                    std::
                        cerr << "rbd: must specify 'client' or 'cluster' key."
                        << std::endl;
                    return -EINVAL;
                }
                ...
                librbd::RBD rbd;
                if (key == "client") {
                    r = rbd.mirror_peer_set_client(io_ctx, uuid.c_str(),
                                                   value.c_str());
                }
                else {
                    r = rbd.mirror_peer_set_cluster(io_ctx, uuid.c_str(),
                                                    value.c_str());
                }
                if (r < 0) {
                    return r;
            ...
            int execute_enable_disable(const std::string & pool_name,
                ...
                if (current_mirror_mode == next_mirror_mode) {
                    if (mode == "disabled") {
                        std::cout << "mirroring is already " << mode << std::
                            endl;
                    }
                    else {
            ...
            int execute_disable(const po::variables_map & vm) {
                ...
                return execute_enable_disable(pool_name,
