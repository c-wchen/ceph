...
class Filer {
    ...
    struct Probe {
        ...
        bool found_size;
         ...
         Probe(inodeno_t i, const file_layout_t & l, snapid_t sn,
            ...
            found_size(false) {
        } Probe(inodeno_t i, const file_layout_t & l, snapid_t sn, uint64_t f,
  ...
  /*** async file interface.  scatter/gather as needed. ***/
    ...
    void read(inodeno_t ino, const file_layout_t * layout, snapid_t snap, uint64_t offset, uint64_t len, ceph::buffer::list * bl,   // ptr to data
    ...
    void read_trunc(inodeno_t ino, const file_layout_t * layout, snapid_t snap, uint64_t offset, uint64_t len, ceph::buffer::list * bl, // ptr to data
                    int flags,
    ...
    void zero(inodeno_t ino,
        ...
        if (extents.size() == 1) {
            if (extents[0].offset == 0
                && extents[0].length == layout->object_size && (!keep_first
                                                                || extents[0].
                                                                objectno != 0))
