...
namespace rbd {
    namespace action {
        namespace migration {
            ...
            static int do_execute(librados::IoCtx & io_ctx,
                ...
                if (r < 0) {
                    pc.fail();
                    std::
                        cerr << "rbd: migration failed: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                } pc.finish();
                 return 0;
            } static int do_abort(librados::IoCtx & io_ctx,
                                  const std::string & image_name,
                                  bool no_progress) {
                ...
                if (r < 0) {
                    pc.fail();
                    std::
                        cerr << "rbd: aborting migration failed: " <<
                        cpp_strerror(r)
                    << std::endl;
                    return r;
                } pc.finish();
            ...
            static int do_commit(librados::IoCtx & io_ctx,
                                 const std::string & image_name, bool force,
                                 bool no_progress) {
                librbd::image_migration_status_t migration_status;
                int r =
                    librbd::RBD().migration_status(io_ctx, image_name.c_str(),
                                                   &migration_status,
                ...
                r = librados::Rados(io_ctx).ioctx_create2(migration_status.
                                                          dest_pool_id,
                                                          dst_io_ctx);
                if (r < 0) {
                ...
                r = utils::set_namespace(migration_status.dest_pool_namespace,
                                         &dst_io_ctx);
                if (r < 0) {
                    return r;
                }
                ...
                librbd::Image image;
                r = utils::open_image_by_id(dst_io_ctx,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: listing descendants failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                ...
                if (children.size() > 0) {
                    std::cerr << "rbd: the image has "
                        << (children.size() ==
                            1 ? "a descendant" : "descendants") << ": " << std::
                        endl;
                  for (auto & child:children) {
                    ...
                    if (force) {
                        std::
                            cerr << "Proceeding anyway due to force flag set."
                            << std::endl;
                    }
                    else {
                        std::
                            cerr <<
                            "Ensure no descendant images are opened read-only"
                            << " and run again with force flag." << std::endl;
                        return -EBUSY;
                    }
                }

                utils::ProgressContext pc("Commit image migration",
                                          no_progress);
                r = librbd::RBD().migration_commit_with_progress(io_ctx,
                                                                 image_name.
                                                                 c_str(), pc);
                if (r < 0) {
                    pc.fail();
                    std::
                        cerr << "rbd: committing migration failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            void get_prepare_arguments(po::options_description * positional,
                ...
                options->add_options()
                    ("import-only", po::bool_switch(),
                     "only import data from source")
            ...
            int execute_prepare(const po::variables_map & vm,
                ...
                if (vm.count("source-spec") && vm.count("source-spec-path")) {
                    std::
                        cerr <<
                        "rbd: cannot specify both source-spec and source-spec-path"
                        << std::endl;
                    return -EINVAL;
                }
                else if (vm.count("source-spec-path")) {
                    std::string source_spec_path =
                        vm["source-spec-path"].as < std::string > ();
                    ...
                    if (source_spec_path != "-") {
                        ...
                        if (fd < 0) {
                            r = -errno;
                            std::
                                cerr << "rbd: error opening " <<
                                source_spec_path << std::endl;
                            return r;
                        }
                    }

                    source_spec.resize(4096);
                    r = safe_read(fd, source_spec.data(),
                                  source_spec.size() - 1);
                    if (fd != STDIN_FILENO) {
                    ...
                    if (r >= 0) {
                        ...
                        std::
                            cerr << "rbd: error reading source-spec file: " <<
                            cpp_strerror(r)
                            << std::endl;
                        return r;
                    }
                ...
                if (source_spec.empty()) {
                    ...
                    r = utils::init_io_ctx(rados, dst_pool_name,
                                           dst_namespace_name, &dst_io_ctx);
                    if (r < 0) {
                        return r;
                    }
                }
                ...
                if (import_only && source_spec.empty()) {
                    if (snap_name.empty()) {
                        std::
                            cerr << "rbd: snapshot name was not specified" <<
                            std::endl;
                        return -EINVAL;
                    }
                    ...
                    ss << R "({)"
                        ...
                        << R "(" pool_namespace ":")
                        " << io_ctx.get_namespace() << R"(",)" << R "("
                                                          image_name ":")
                        " << image_name << R"(",)" << R "(" snap_name ":")
                        " << snap_name << R"(")" << R "(})";
                                             source_spec = ss.str();
                                             if (dst_image_name.empty()) {
                                             std::
                                             cerr <<
                                             "rbd: destination image name must be provided"
                                             ...
                                             std::
                                             cerr <<
                                             "rbd: --import-only must be used in combination with "
                                             << "source-spec/source-spec-path"
                                             << std::endl; return -EINVAL;}
                                             ...
                                             r =
                                             utils::get_image_options(vm, true,
                                                                      &opts);
                                             if (r < 0) {
                                             return r;}
                                             ...
                                             std::
                                             cerr <<
                                             "rbd: preparing migration failed: "
                                             << cpp_strerror(r)
                                             << std::endl; return r;}
                                             }
                                             else {
                                             ...
                                             (source_spec.c_str(), io_ctx,
                                              image_name.c_str(), opts);
                                             if (r < 0) {
                                             std::
                                             cerr <<
                                             "rbd: preparing import migration failed: "
                                             << cpp_strerror(r)
                                             << std::endl; return r;}
                                             }
                                             ...
                                             {
                                             at::
                                             add_image_spec_options(positional,
                                                                    options,
                                                                    at::
                                                                    ARGUMENT_MODIFIER_NONE);
                                             at::
                                             add_no_progress_option(options);}

                                             int execute_execute(const po::
                                                                 variables_map &
                                                                 vm,
                                                                 const std::
                                                                 vector <
                                             ...
                                             int r =
                                             utils::
                                             get_pool_image_snapshot_names(vm,
                                                                           at::
                                                                           ARGUMENT_MODIFIER_NONE,
                                                                           &arg_index,
                                                                           &pool_name,
                                             ...
                                             utils::init(pool_name,
                                                         namespace_name, &rados,
                                                         &io_ctx); if (r < 0) {
                                             return r;}
                                             io_ctx.set_pool_full_try();
                                             r =
                                             do_execute(io_ctx, image_name,
                                                        vm[at::NO_PROGRESS].as <
                                                        bool > ()); if (r < 0) {
                                             return r;}

                                             return 0;}

                                             void get_abort_arguments(po::
                                                                      options_description
                                                                      *
                                                                      positional,
                                                                      po::
                                             ...
                                             ("force", po::bool_switch(),
                                              "proceed even if the image has children");}

                                             int execute_commit(const po::
                                                                variables_map &
                                                                vm,
                                                                const std::
                                                                vector <
                                             ...
                                             Shell::
                                             SwitchArguments switched_arguments( {
                                                                                "import-only"}
                                             ...
                                             Shell::Action action_commit( {
                                                                         ...
                                                                         , "Commit image migration.", "", &get_commit_arguments, &execute_commit);} // namespace migration
                                             }  // namespace action
                                             }  // namespace rbd
