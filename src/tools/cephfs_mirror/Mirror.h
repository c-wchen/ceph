        ...
        class Mirror {
          ...
          private:
            ...
            struct ClusterListener:ClusterWatcher::Listener {
                ...
                :mirror(mirror) {
                } void handle_mirroring_enabled(const FilesystemSpec & spec)
                    override {
                    mirror->mirroring_enabled(spec.filesystem, spec.pool_id);
                } void handle_mirroring_disabled(const Filesystem & filesystem)
                    override {
                    mirror->mirroring_disabled(filesystem);
                } void handle_peers_added(const Filesystem & filesystem,
                                          const Peer & peer) override {
                    mirror->peer_added(filesystem, peer);
                } void handle_peers_removed(const Filesystem & filesystem,
                                            const Peer & peer) override {
            ...
            struct MirrorAction {
                MirrorAction(uint64_t pool_id):pool_id(pool_id) {
            ...
            // called via listener
            void mirroring_enabled(const Filesystem & filesystem,
            ...
            void enable_mirroring(const Filesystem & filesystem,
                                  uint64_t local_pool_id, Context * on_finish,
                                  bool is_restart = false);
            void handle_enable_mirroring(const Filesystem & filesystem, int r);
            void handle_enable_mirroring(const Filesystem & filesystem,
                                         const Peers & peers, int r);

            // mirror disable callback
            void disable_mirroring(const Filesystem & filesystem,
                                   Context * on_finish);
