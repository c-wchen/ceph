...
RDMAConnectedSocketImpl::RDMAConnectedSocketImpl(CephContext * cct,
                                                 std::shared_ptr < Infiniband >
                                                 &ib,
                                                 std::shared_ptr <
                                                 RDMADispatcher >
                                                 &rdma_dispatcher,
                                                 RDMAWorker * w)
:  cct(cct), connected(0), error(0), ib(ib), dispatcher(rdma_dispatcher),
worker(w), is_server(false), read_handler(new C_handle_connection_read(this)),
established_handler(new C_handle_connection_established(this)), active(false),
pending(false)
{
    if (!cct->_conf->ms_async_rdma_cm) {
        qp = ib->create_queue_pair(cct, dispatcher->get_tx_cq(),
                                   dispatcher->get_rx_cq(), IBV_QPT_RC, NULL);
        if (!qp) {
...
RDMAConnectedSocketImpl::~RDMAConnectedSocketImpl()
{
    ...
    for (unsigned i = 0; i < wc.size(); ++i) {
        dispatcher->post_chunk_to_pool(reinterpret_cast <
                                       Chunk * >(wc[i].wr_id));
    }
...
int RDMAConnectedSocketImpl::handle_connection_established(bool need_set_fault)
{
    ...
    if (r < 0) {
        ldout(cct,
              1) << __func__ << " send handshake msg failed." << r << dendl;
        if (need_set_fault) {
            fault();
        }
        return r;
...
{
    ldout(cct,
          20) << __func__ << " QP: " << local_qpn << " tcp_fd: " << tcp_fd <<
    ...
    if (1 == connected) {
        ldout(cct,
              1) << __func__ << " warnning: logic failed: read len: " << r <<
            dendl;
        fault();
        return;
    }
...
ssize_t RDMAConnectedSocketImpl::read(char *buf, size_t len)
{
    ...
    ldout(cct,
          20) << __func__ << " notify_fd : " << event_val << " in " << local_qpn
        << " r = " << r << dendl;

    if (!active) {
        ldout(cct,
              1) << __func__ << " when ib not active. len: " << len << dendl;
        return -EAGAIN;
    }

    if (0 == connected) {
        ldout(cct,
              1) << __func__ << " when ib not connected. len: " << len << dendl;
        return -EAGAIN;
    }
    ssize_t read = 0;
    ...
    if (is_server && connected == 0) {
        ldout(cct,
            ...
            local_qpn << " peer QP: " << peer_qpn << dendl;
...
ssize_t RDMAConnectedSocketImpl::read_buffers(char *buf, size_t len)
{
    ...
    while (pchunk != buffers.end()) {
        ...
        read_size += tmp;
        ldout(cct,
              25) << __func__ << " read chunk " << *pchunk << " bytes length" <<
            tmp << " offset: " << (*pchunk)->
            get_offset() << " ,bound: " << (*pchunk)->get_bound() << dendl;
    ...
    ldout(cct,
          25) << __func__ << " got " << read_size << " bytes, buffers size: " <<
        buffers.size() << dendl;
...
ssize_t RDMAConnectedSocketImpl::send(ceph::buffer::list & bl, bool more)
{
        ...
        return 0;
    {
        ...
        if (!connected) {
            ldout(cct,
                  20) << __func__ << " fake send to upper, QP: " << local_qpn <<
                dendl;
            return bytes;
        }
    }
...
size_t RDMAConnectedSocketImpl::tx_copy_chunk(std::vector <
                                              Chunk * >&tx_buffers,
                                              size_t req_copy_len,
                                              decltype(std::
                                                       cbegin(pending_bl.
                                              ...
                                              & end)
{
    ...
    while (start != end) {
        ...
        while (slice_write_len < start->length()) {
            size_t real_len =
                current_chunk->write((char *)addr + slice_write_len,
                                     start->length() - slice_write_len);
...
ssize_t RDMAConnectedSocketImpl::submit(bool more)
{
    ...
    while (it != pending_bl.buffers().end()) {
        if (ib->is_tx_buffer(it->raw_c_str())) {
            if (wait_copy_len) {
                size_t copied =
                    tx_copy_chunk(tx_buffers, wait_copy_len, copy_start, it);
                total_copied += copied;
  ...
  sending:
    ...
    if (total_copied < pending_bl.length()) {
        ...
        pending_bl.splice(total_copied, pending_bl.length() - total_copied,
                          &swapped);
    ...
    ldout(cct,
        ...
        length() << " in buffers " << pending_bl.
        get_num_buffers() << " tx chunks " << tx_buffers.size() << dendl;
    ...
    ldout(cct,
          20) << __func__ << " finished sending " << total_copied << " bytes."
        << dendl;
...
{
    ldout(cct,
          20) << __func__ << " QP: " << local_qpn << " " << tx_buffers[0] <<
        dendl;
...
void RDMAConnectedSocketImpl::cleanup()
{
    ...
    if (established_handler) {
        (static_cast <
         C_handle_connection_established * >(established_handler))->close();
...
void RDMAConnectedSocketImpl::update_post_backlog()
{
    if (post_backlog)
        post_backlog -=
            post_backlog - dispatcher->post_chunks_to_rq(post_backlog, qp);
}
