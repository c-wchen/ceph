...
Store *Store::create(ObjectStore *store,
...
Store::Store(const coll_t &coll, const ghobject_t &oid,
             ObjectStore *store)
...
void Store::add_object_error(int64_t pool,
                             const inconsistent_obj_wrapper &e)
{
    bufferlist bl;
    e.encode(bl);
...
std::vector < bufferlist > Store::get_snap_errors(int64_t pool,
...
{
    const string begin = (start.name.empty() ?
                          first_object_key(pool) : to_object_key(pool,
                              start));
    const string end = last_object_key(pool);
    return get_errors(begin, end, max_return);
} std::vector < bufferlist >
Store::get_errors(const string &begin,
                  const string &end, uint64_t max_return)const
{
