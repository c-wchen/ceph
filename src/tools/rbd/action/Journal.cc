        ...
        namespace journal {
            ...
            void add_journal_option(po::options_description * opt,
                ...
                case at::ARGUMENT_MODIFIER_SOURCE:
                    break;
                    case at::ARGUMENT_MODIFIER_DEST:name = DEST_JOURNAL_NAME;
                    break;
                }
                // TODO add validator opt->add_options()
                ...
                pos->add_options()
                    ((get_name_prefix(modifier) + JOURNAL_SPEC).c_str(),
                     (get_description_prefix(modifier) +
                ...
                std::string namespace_key = (mod == at::ARGUMENT_MODIFIER_DEST ?
                                             at::DEST_NAMESPACE_NAME : at::
                                             NAMESPACE_NAME);
                std::string image_key =
                    (mod ==
                     at::ARGUMENT_MODIFIER_DEST ? at::DEST_IMAGE_NAME : at::
                     IMAGE_NAME);
                std::string journal_key =
                    (mod ==
                     at::
                     ARGUMENT_MODIFIER_DEST ? DEST_JOURNAL_NAME : JOURNAL_NAME);
                ...
                if (journal_name != nullptr && !journal_name->empty()) {
                    ...
                    r = extract_spec(journal_name_copy, pool_name,
                                     namespace_name, journal_name, nullptr,
                ...
                if (!image_name.empty()) {
                    ...
                    r = extract_spec(image_name_copy, pool_name, namespace_name,
                                     &image_name, nullptr,
                ...
                if (journal_name != nullptr && spec_arg_index != nullptr &&
                    journal_name->empty()) {
                    std::string spec =
                        utils::get_positional_argument(vm, (*spec_arg_index)++);
                    if (!spec.empty()) {
                        r = extract_spec(spec, pool_name, namespace_name,
                ...
                if (pool_name != nullptr && namespace_name != nullptr &&
                    journal_name != nullptr && journal_name->empty()
                    && !image_name.empty()) {
                    ...
                    librbd::Image image;
                    int r =
                        utils::init_and_open_image(*pool_name, *namespace_name,
                                                   image_name,
                                                   "", "", true, &rados,
                                                   &io_ctx, &image);
                    if (r < 0) {
                        std::cerr << "rbd: failed to open image " << image_name
                            << " to get journal name: " << cpp_strerror(r) <<
                            std::endl;
                        return r;
                    }
                    ...
                    if ((features & RBD_FEATURE_JOURNALING) == 0) {
                        std::
                            cerr << "rbd: journaling is not enabled for image "
                            << image_name << std::endl;
                        return -EINVAL;
                    }
                ...
                if (journal_name != nullptr && journal_name->empty()) {
                    ...
                    std::cerr << "rbd: "
                        << (mod ==
                            at::ARGUMENT_MODIFIER_DEST ? prefix : std::string())
            ...
            static int do_show_journal_info(librados::Rados & rados,
                                            librados::IoCtx & io_ctx,
                ...
                std::string header_oid =::journal::Journaler::
                    header_oid(journal_id);
                std::string object_oid_prefix =::journal::Journaler::
                    object_oid_prefix(io_ctx.get_id(), journal_id);
                uint8_t order;
                uint8_t splay_width;
                int64_t pool_id;

                cls::journal::client::get_immutable_metadata(io_ctx, header_oid,
                                                             &order,
                                                             &splay_width,
                                                             &pool_id, &cond);
                r = cond.wait();
                if (r < 0) {
                    std::
                        cerr << "failed to get journal metadata: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                ...
                if (pool_id >= 0) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "error looking up pool name for pool_id=" <<
                            pool_id << ": " << cpp_strerror(r) << std::endl;
                    }
                }
                ...
                if (f) {
                    ...
                    std::cout << "rbd journal '" << journal_id << "':" << std::
                        endl;
                    std::cout << "\theader_oid: " << header_oid << std::endl;
                    std::
                        cout << "\tobject_oid_prefix: " << object_oid_prefix <<
                        std::endl;
                    std::cout << "\torder: " << static_cast <
                        int >(order) << " (" << byte_u_t(1ull << order) <<
                        " objects)" << std::endl;
                    std::cout << "\tsplay_width: " << static_cast <
                        int >(splay_width) << std::endl;
                    if (!object_pool_name.empty()) {
                        std::
                            cout << "\tobject_pool: " << object_pool_name <<
                            std::endl;
                    }
                }
                return 0;
            ...
            static int do_show_journal_status(librados::IoCtx & io_ctx,
                ...
                cls::journal::client::get_mutable_metadata(io_ctx, oid,
                                                           &minimum_set,
                                                           &active_set,
                                                           &registered_clients,
                                                           &cond);
                ...
                if (r < 0) {
                    std::
                        cerr << "warning: failed to get journal metadata" <<
                        std::endl;
                    return r;
                }
            ...
            static int do_reset_journal(librados::IoCtx & io_ctx,
                ...
                std::string image_name;
                int r =
                    librbd::cls_client::dir_get_name(&io_ctx, RBD_DIRECTORY,
                                                     journal_id,
                                                     &image_name);
                if (r < 0) {
                    std::
                        cerr << "failed to locate journal's image: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "failed to open image: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "failed to disable image journaling: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "failed to re-enable image journaling: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            static int do_disconnect_journal_client(librados::IoCtx & io_ctx,
              ...
              for (auto & c:registered_clients) {
                    if (c.id == IMAGE_CLIENT_ID
                        || (!client_id.empty() && client_id != c.id)) {
                        continue;
                    }
                    r = cls::journal::client::client_update_state(io_ctx, oid,
                                                                  c.id,
                                                                  cls::journal::
                                                                  CLIENT_STATE_DISCONNECTED);
                    if (r < 0) {
                        std::
                            cerr << "warning: failed to disconnect client " <<
                            c.id << ": " << cpp_strerror(r) << std::endl;
                        return r;
                    }
                    std::cout << "client " << c.
                        id << " disconnected" << std::endl;
                    found = true;
                }
                ...
                if (!found) {
                    if (!client_id.empty()) {
                        std::
                            cerr << "warning: client " << client_id <<
                            " is not registered" << std::endl;
                    }
                    else {
                        std::
                            cerr << "no registered clients to disconnect" <<
                            std::endl;
                    }
                    return -ENOENT;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "warning: failed to notify state change:" <<
            ...
            class Journaler:public::journal::Journaler {
              public:
                Journaler(librados::IoCtx & io_ctx,
                          const std::string & journal_id,
                          const std::string & client_id):::journal::
                    Journaler(io_ctx, journal_id, client_id, {
                              }, nullptr) {
                }
                ...
                int init() {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "failed to register client: " <<
                            cpp_strerror(r)
                            << std::endl;
                        return r;
                    }
                ...
                int shut_down() {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to unregister journal client: "
                            << cpp_strerror(r) << std::endl;
                    }
            ...
            class JournalPlayer {
              public:
                JournalPlayer(librados::IoCtx & io_ctx,
                              const std::string & journal_id,
                              const std::string & client_id):m_journaler(io_ctx,
                                                                         journal_id,
                                                                         client_id),
                    m_cond(), m_r(0) {
                ...
                virtual int exec() {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to process journal: " <<
                            cpp_strerror(r)
                            << std::endl;
                        if (m_r == 0) {
                            m_r = r;
              ...
              protected:
                struct ReplayHandler:public::journal::ReplayHandler {
                    ...
                    explicit ReplayHandler(JournalPlayer *
            ...
            static int inspect_entry(bufferlist & data,
                ...
                try {
                    ...
                    std::cerr << "failed to decode event entry: " << err.
                        what() << std::endl;
                    return -EINVAL;
                }
                if (verbose) {
            ...
            class JournalInspector:public JournalPlayer {
              public:
                JournalInspector(librados::IoCtx & io_ctx,
                                 const std::string & journal_id,
                                 bool verbose):JournalPlayer(io_ctx, journal_id,
                                                             "INSPECT"),
                    m_verbose(verbose), m_s() {
                } int exec() override {
              ...
              private:
                ...
                int process_entry(::journal::ReplayEntry replay_entry,
                    ...
                    if (m_verbose) {
                        std::
                            cout << "Entry: tag_id=" << tag_id <<
                            ", commit_tid=" << replay_entry.
            ...
            struct ExportEntry {
                ...
                :tag_id(tag_id), commit_tid(commit_tid), type(type),
                    entry(entry) {
                }
            ...
            class JournalExporter:public JournalPlayer {
              public:
                JournalExporter(librados::IoCtx & io_ctx,
                                const std::string & journal_id, int fd,
                                bool no_error,
                                bool verbose):JournalPlayer(io_ctx, journal_id,
                                                            "EXPORT"),
                    m_journal_id(journal_id), m_fd(fd), m_no_error(no_error),
                    m_verbose(verbose), m_s() {
                } int exec() override {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to write to export file: " <<
                            cpp_strerror(r)
                            << std::endl;
                        return r;
                    }
              ...
              private:
                struct Stats {
                    Stats():total(0), error(0) {
                        ...
                        std::
                            cout << total << " entries processed, " << error <<
                            " errors" << std::endl;
                    }
                ...
                int process_entry(::journal::ReplayEntry replay_entry,
                    ...
                    ExportEntry export_entry(tag_id,
                                             replay_entry.get_commit_tid(),
                                             type, entry);
                    JSONFormatter f;
            ...
            static int do_export_journal(librados::IoCtx & io_ctx,
                ...
                if (to_stdout) {
                    ...
                    fd = open(path.c_str(),
                              O_WRONLY | O_CREAT | O_EXCL | O_BINARY, 0644);
                    if (fd < 0) {
                        r = -errno;
                        std::cerr << "rbd: error creating " << path << std::
                            endl;
                        return r;
                    }
                ...
                JournalExporter exporter(io_ctx, journal_id, fd, no_error,
                                         verbose);
                r = exporter.exec();
            ...
            class JournalImporter {
              public:
                JournalImporter(librados::IoCtx & io_ctx,
                                const std::string & journal_id, int fd,
                                bool no_error, bool verbose):m_journaler(io_ctx,
                                                                         journal_id,
                                                                         "IMPORT"),
                    m_fd(fd), m_no_error(no_error), m_verbose(verbose) {
                    ...
                    for (size_t i = 1; i < sizeof(buf); i++) {
                        ...
                        if (r < 0) {
                            std::
                                cerr << "rbd: error reading import data" <<
                                std::endl;
                            return false;
                        }
                        if (!isdigit(buf[i])) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: error reading from stdin: " <<
                            cpp_strerror(r)
                            << std::endl;
                        return false;
                    }
                ...
                int exec() {
                    ...
                    while (read_entry(bl, r)) {
                        ...
                        if (!p.parse(bl.c_str(), bl.length())) {
                            std::
                                cerr << "rbd: error parsing input (entry " << n
                                << ")" << std::endl;
                            r = -EINVAL;
                            if (m_no_error) {
                                r1 = r;
                        ...
                        try {
                            ...
                            std::
                                cerr <<
                                "rbd: error json decoding import data (entry "
                                << n << "):" << err.what() << std::endl;
                            r = -EINVAL;
                            if (m_no_error) {
                                r1 = r;
                    ...
                    std::
                        cout << n << " entries processed, " << error_count <<
                        " errors" << std::endl;

                    std::
                        cout << "Waiting for journal append to complete..." <<
                        std::endl;
            ...
            static int do_import_journal(librados::IoCtx & io_ctx,
                ...
                JournalImporter importer(io_ctx, journal_id, fd, no_error,
                                         verbose);
                r = importer.exec();
            ...
            int execute_info(const po::variables_map & vm,
                ...
                std::string journal_name;
                int r =
                    get_pool_journal_names(vm, at::ARGUMENT_MODIFIER_NONE,
                                           &arg_index,
                ...
                r = do_show_journal_info(rados, io_ctx, journal_name,
                                         formatter.get());
                if (r < 0) {
                    std::
                        cerr << "rbd: journal info: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_status(const po::variables_map & vm,
                ...
                r = do_show_journal_status(io_ctx, journal_name,
                                           formatter.get());
                if (r < 0) {
                    std::
                        cerr << "rbd: journal status: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_reset(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: journal reset: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_client_disconnect(const po::variables_map & vm,
                ...
                r = do_disconnect_journal_client(io_ctx, journal_name,
                                                 client_id);
                if (r < 0) {
                    std::
                        cerr << "rbd: journal client disconnect: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_inspect(const po::variables_map & vm,
                ...
                r = do_inspect_journal(io_ctx, journal_name,
                                       vm[at::VERBOSE].as < bool > ());
                if (r < 0) {
                    std::
                        cerr << "rbd: journal inspect: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_export(const po::variables_map & vm,
                ...
                std::string journal_name;
                int r =
                    get_pool_journal_names(vm, at::ARGUMENT_MODIFIER_SOURCE,
                                           &arg_index,
                                           &pool_name, &namespace_name,
                                           &journal_name);
                if (r < 0) {
                ...
                r = do_export_journal(io_ctx, journal_name, path,
                                      vm[at::NO_ERR].as < bool > (),
                                      vm[at::VERBOSE].as < bool > ());
                if (r < 0) {
                    std::
                        cerr << "rbd: journal export: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_import(const po::variables_map & vm,
                ...
                r = do_import_journal(io_ctx, journal_name, path,
                                      vm[at::NO_ERR].as < bool > (),
                                      vm[at::VERBOSE].as < bool > ());
                if (r < 0) {
                    std::
                        cerr << "rbd: journal import: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
