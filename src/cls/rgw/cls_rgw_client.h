...
class BucketIndexAioManager {
  ...
  BucketIndexAioManager():next(0), lock("BucketIndexAioManager::lock") {
    ...
    bool wait_for_completions(int valid_ret_code, int *num_completions,
                              int *ret_code, map < int, string > *objs);

  /**
   * Do aio read operation.
   */
    bool aio_operate(librados::IoCtx & io_ctx, const string & oid,
                     librados::ObjectReadOperation * op) {
        Mutex::Locker l(lock);
        BucketIndexAioArg *arg = new BucketIndexAioArg(get_next(), this);
        librados::AioCompletion * c =
            librados::Rados::aio_create_completion((void *)arg, NULL,
...
class CLSRGWConcurrentIO {
  ...
  public:
    ...
    int operator() () {
        ...
        while (manager.
               wait_for_completions(valid_ret_code(), &num_completions, &r,
                                    pobjs)) {
            if (r >= 0 && ret >= 0) {
                for (int i = 0;
                     i < num_completions && iter != objs_container.end();
                     ++i, ++iter) {
            ...
            if (need_multiple_rounds() && iter == objs_container.end()
                && !objs.empty()) {
...
class CLSRGWIssueBucketIndexInit:public CLSRGWConcurrentIO {
  ...
  public:
    CLSRGWIssueBucketIndexInit(librados::IoCtx & ioc, map < int,
                               string > &_bucket_objs,
...
class CLSRGWIssueSetTagTimeout:public CLSRGWConcurrentIO {
  ...
  protected:
    int issue_op(int shard_id, const string & oid) override;
  public:
     CLSRGWIssueSetTagTimeout(librados::IoCtx & ioc, map < int,
                              string > &_bucket_objs, uint32_t _max_aio,
...
void cls_rgw_bucket_update_stats(librados::ObjectWriteOperation & o,
                                 bool absolute, const map < uint8_t,
...
int cls_rgw_bi_put(librados::IoCtx & io_ctx, const string oid,
                   rgw_cls_bi_entry & entry);
void cls_rgw_bi_put(librados::ObjectWriteOperation & op, const string oid,
                    rgw_cls_bi_entry & entry);
...
int cls_rgw_bucket_link_olh(librados::IoCtx & io_ctx,
                            librados::ObjectWriteOperation & op,
                            const string & oid, const cls_rgw_obj_key & key,
...
class CLSRGWIssueBucketList:public CLSRGWConcurrentIO {
  ...
  public:
     CLSRGWIssueBucketList(librados::IoCtx & io_ctx,
                           ...
                           bool _list_versions, map < int, string > &oids,
...
class CLSRGWIssueBILogTrim:public CLSRGWConcurrentIO {
  ...
  protected:
    ...
    void add_object(int shard, const string & oid) override {
  ...
  public:
    CLSRGWIssueBILogTrim(librados::IoCtx & io_ctx,
                         ...
                         BucketIndexShardsManager & _end_marker_mgr, map < int,
                         string > &_bucket_objs,
...
class CLSRGWIssueBucketRebuild:public CLSRGWConcurrentIO {
  protected:
    int issue_op(int shard_id, const string & oid) override;
  public:
     CLSRGWIssueBucketRebuild(librados::IoCtx & io_ctx, map < int,
                              string > &bucket_objs,
                              uint32_t max_aio):CLSRGWConcurrentIO(io_ctx,
                                                                   bucket_objs,
                                                                   max_aio) {
...
class CLSRGWIssueGetDirHeader:public CLSRGWConcurrentIO {
  ...
  protected:
    int issue_op(int shard_id, const string & oid) override;
  public:
     CLSRGWIssueGetDirHeader(librados::IoCtx & io_ctx, map < int,
                             string > &oids, map < int,
...
class CLSRGWIssueSetBucketResharding:public CLSRGWConcurrentIO {
  ...
  protected:
    int issue_op(int shard_id, const string & oid) override;
  public:
     CLSRGWIssueSetBucketResharding(librados::IoCtx & ioc, map < int,
                                    string > &_bucket_objs,
...
class CLSRGWIssueResyncBucketBILog:public CLSRGWConcurrentIO {
  ...
  public:
     CLSRGWIssueResyncBucketBILog(librados::IoCtx & io_ctx, map < int,
                                  string > &_bucket_objs,
                                  uint32_t max_aio):CLSRGWConcurrentIO(io_ctx,
                                                                       _bucket_objs,
                                                                       max_aio)
...
class CLSRGWIssueBucketBILogStop:public CLSRGWConcurrentIO {
  protected:
    int issue_op(int shard_id, const string & oid);
  public:
     CLSRGWIssueBucketBILogStop(librados::IoCtx & io_ctx, map < int,
                                string > &_bucket_objs,
                                uint32_t max_aio):CLSRGWConcurrentIO(io_ctx,
                                                                     _bucket_objs,
                                                                     max_aio) {
...
int cls_rgw_get_dir_header_async(librados::IoCtx & io_ctx, string & oid,
...
/* usage logging */
int cls_rgw_usage_log_read(librados::IoCtx & io_ctx, string & oid,
                           string & user, uint64_t start_epoch,
                           uint64_t end_epoch, uint32_t max_entries,
                           string & read_iter, map < rgw_user_bucket,
                           rgw_usage_log_entry > &usage, bool * is_truncated);

int cls_rgw_usage_log_trim(librados::IoCtx & io_ctx, const string & oid,
                           string & user, uint64_t start_epoch,
                           uint64_t end_epoch);
...
void cls_rgw_gc_defer_entry(librados::ObjectWriteOperation & op,
                            uint32_t expiration_secs, const string & tag);

int cls_rgw_gc_list(librados::IoCtx & io_ctx, string & oid, string & marker,
...
/* lifecycle */
int cls_rgw_lc_get_head(librados::IoCtx & io_ctx, string & oid,
                        cls_rgw_lc_obj_head & head);
int cls_rgw_lc_put_head(librados::IoCtx & io_ctx, string & oid,
                        cls_rgw_lc_obj_head & head);
int cls_rgw_lc_get_next_entry(librados::IoCtx & io_ctx, string & oid,
                              string & marker, pair < string, int >&entry);
int cls_rgw_lc_rm_entry(librados::IoCtx & io_ctx, string & oid, pair < string,
                        int >&entry);
int cls_rgw_lc_set_entry(librados::IoCtx & io_ctx, string & oid, pair < string,
                         int >&entry);
int cls_rgw_lc_list(librados::IoCtx & io_ctx, string & oid,
                    const string & marker, uint32_t max_entries, map < string,
                    int >&entries);

/* resharding */
void cls_rgw_reshard_add(librados::ObjectWriteOperation & op,
                         const cls_rgw_reshard_entry & entry);
int cls_rgw_reshard_list(librados::IoCtx & io_ctx, const string & oid,
                         string & marker, uint32_t max,
                         list < cls_rgw_reshard_entry > &entries,
                         bool * is_truncated);
int cls_rgw_reshard_get(librados::IoCtx & io_ctx, const string & oid,
                        cls_rgw_reshard_entry & entry);
int cls_rgw_reshard_get_head(librados::IoCtx & io_ctx, const string & oid,
                             cls_rgw_reshard_entry & entry);
