...
class utime_t {
  ...
  public:
    ...
    template < class Rep, class Period >
        ...
        tv.tv_sec =
            std::max < common_t > (std::chrono::duration_cast <
                                   std::chrono::seconds > (dur).count(), 0);
        tv.tv_nsec =
            std::max < common_t >
            ((std::chrono::duration_cast < std::chrono::nanoseconds >
              (dur) % std::chrono::seconds(1)).count(), 0);
    }
#if defined(WITH_SEASTAR)
    ...
    // ref accessors/modifiers __u32 & sec_ref() {
        return tv.tv_sec;
    }
    __u32 & nsec_ref() {
        return tv.tv_nsec;
    ...
    uint64_t to_nsec() const {
        ...
        return (uint64_t) tv.tv_nsec / 1000000ull +
    ...
    void padding_check() {
        static_assert(sizeof(utime_t) == sizeof(tv.tv_sec) + sizeof(tv.tv_nsec)
    ...
    // output
        std::ostream & gmtime(std::ostream & out, bool legacy_form =
                              false) const {
    ...
    static int parse_date(const std::string & date, uint64_t * epoch,
                          uint64_t * nsec, std::string * out_date =
                          nullptr, std::string * out_time = nullptr) {
        struct tm tm;
        ...
        if (p) {
            if (*p == ' ' || *p == 'T') {
                ...
                if (nsec && subsec) {
                    ...
                    for (i = 0; (i < sizeof(buf) - 1) && isdigit(*subsec);
                         ++i, ++subsec) {
                        buf[i] = *subsec;
                    }
