...
public:
    ...
    static int diff_iterate(ImageCtxT *ictx,
                            const cls::rbd::
                            SnapshotNamespace &from_snap_namespace,
                            const char *fromsnapname, uint64_t off,
                            uint64_t len, bool include_parent,
                            bool whole_object, int (*cb)(uint64_t,
                                    size_t, int,
                                    void *),
                            void *arg);
...
private:
    ...
    DiffIterate(ImageCtxT &image_ctx,
        ...
        : m_image_ctx(image_ctx), m_from_snap_namespace(from_snap_namespace),
          m_from_snap_name(from_snap_name), m_offset(off), m_length(len),
          m_include_parent(include_parent), m_whole_object(whole_object),
