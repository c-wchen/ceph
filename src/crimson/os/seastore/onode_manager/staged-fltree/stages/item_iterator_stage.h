 ...
 */
    template < node_type_t NODE_TYPE > class item_iterator_t {
      ...
      public:
        ...
        :node_size {
        range.node_size},
            p_items_start(range.range.p_start), p_items_end(range.range.p_end) {
        ...
        } const memory_range_t & get_item_range() const {
            return item_range;
        } node_offset_t get_back_offset() const {
            return back_offset;
        }
        // container type system using key_get_type = const ns_oid_view_t &;
        ...
        node_offset_t size() const {
            size_t ret =
                item_range.p_end - item_range.p_start + sizeof(node_offset_t);
        ...
        node_offset_t size_to_nxt() const {
            ...
            return { {
            item_range.p_start, get_key().p_start()}
            , node_size};
        }
        bool has_next() const {
        ...
        template < IsFullKey Key >
            static memory_range_t insert_prefix(NodeExtentMutable & mut,
    ...
    template < node_type_t NODE_TYPE >
        template < KeyT KT > class item_iterator_t < NODE_TYPE >::Appender {
      public:
