...
void Capability::Export::encode(ceph::buffer::list & bl) const const
{
    ENCODE_START(3, 2, bl);
    encode(cap_id, bl);
    encode(wanted, bl);
...
void Capability::Export::dump(ceph::Formatter * f) const const
{
...
void Capability::Export::generate_test_instances(std::list <
                                                 Capability::Export * >&ls)
{
...
void Capability::Import::encode(ceph::buffer::list & bl) const const
{
    ENCODE_START(1, 1, bl);
    encode(cap_id, bl);
    encode(issue_seq, bl);
...
void Capability::Import::dump(ceph::Formatter * f) const const
{
    f->dump_unsigned("cap_id", cap_id);
...
void Capability::revoke_info::encode(ceph::buffer::list & bl) const const
{
    ENCODE_START(2, 2, bl)
        encode(before, bl);
    encode(seq, bl);
...
void Capability::revoke_info::dump(ceph::Formatter * f) const const
{
...
void Capability::revoke_info::generate_test_instances(std::list <
                                                      Capability::revoke_info *
                                                      >&ls)
{
...
client_t Capability::get_client() const const
{
    return session ? session->get_client() : client_t(-1);
}

bool Capability::is_stale() const const
{
    return session ? session->is_stale() : false;
}

bool Capability::is_valid() const const
{
...
{
    if ((_issued == _pending) &&
        !is_clientwriteable() && !is_wanted_notable(_wanted)) {
...
void Capability::encode(ceph::buffer::list & bl) const const
{
    ENCODE_START(2, 2, bl)
        encode(last_sent, bl);
    encode(last_issue_stamp, bl);
...
void Capability::dump(ceph::Formatter * f) const const
{
    if (inode)
