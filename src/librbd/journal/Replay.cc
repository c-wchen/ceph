        ...
        namespace {
            ...
            static NoOpProgressContext no_op_progress_callback;
             ...
             template < typename I, typename E > struct ExecuteOp:public Context {
                ...
                Context *on_op_complete;
                 ...
                 ExecuteOp(I & image_ctx, const E & event,
                           Context * on_op_complete)
                :image_ctx(image_ctx), event(event),
                    on_op_complete(on_op_complete) {
                } void execute(const journal::SnapCreateEvent & _) {
                    image_ctx.operations->execute_snap_create(event.
                                                              snap_namespace,
                                                              event.snap_name,
                                                              on_op_complete,
                                                              event.op_tid,
                    ...
                    image_ctx.operations->execute_snap_remove(event.
                                                              snap_namespace,
                                                              event.snap_name,
                                                              on_op_complete);
                ...
                void execute(const journal::SnapProtectEvent & _) {
                    image_ctx.operations->execute_snap_protect(event.
                                                               snap_namespace,
                                                               event.snap_name,
                                                               on_op_complete);
                }
                ...
                void execute(const journal::SnapUnprotectEvent & _) {
                    image_ctx.operations->execute_snap_unprotect(event.
                                                                 snap_namespace,
                                                                 event.
                                                                 snap_name,
                                                                 on_op_complete);
                ...
                void execute(const journal::SnapRollbackEvent & _) {
                    image_ctx.operations->execute_snap_rollback(event.
                                                                snap_namespace,
                                                                event.snap_name,
                                                                no_op_progress_callback,
                                                                on_op_complete);
                ...
                void execute(const journal::ResizeEvent & _) {
                    image_ctx.operations->execute_resize(event.size, true,
                                                         no_op_progress_callback,
                                                         on_op_complete,
                                                         event.op_tid);
                }
                ...
                void execute(const journal::SnapLimitEvent & _) {
                    image_ctx.operations->execute_snap_set_limit(event.limit,
                                                                 on_op_complete);
                }

                void execute(const journal::UpdateFeaturesEvent & _) {
                    image_ctx.operations->execute_update_features(event.
                                                                  features,
                                                                  event.enabled,
                                                                  on_op_complete,
                                                                  event.op_tid);
                ...
                void execute(const journal::MetadataSetEvent & _) {
                    image_ctx.operations->execute_metadata_set(event.key,
                                                               event.value,
                                                               on_op_complete);
                }

                void execute(const journal::MetadataRemoveEvent & _) {
                    image_ctx.operations->execute_metadata_remove(event.key,
                                                                  on_op_complete);
                }
                ...
                void finish(int r) override {
                    ...
                    if (r < 0) {
                        lderr(cct) << ": ExecuteOp::" << __func__ << ": r=" << r
                            << dendl;
                    ...
                    if (image_ctx.exclusive_lock == nullptr ||
                        ...
                        ldout(cct,
                              5) << ": lost exclusive lock -- skipping op" <<
                            dendl;
            ...
            template < typename I > struct C_RefreshIfRequired:public Context {
                ...
                void finish(int r) override {
                    ...
                    if (r < 0) {
                        lderr(cct) << ": C_RefreshIfRequired::" << __func__ <<
                            ": r=" << r << dendl;
...
#define dout_prefix *_dout << "librbd::journal::Replay: " << this << " " \
        ...
        template < typename I > Replay < I >::Replay(I & image_ctx)
      :    m_image_ctx(image_ctx) {
        ...
        template < typename I >
            int Replay < I >::decode(bufferlist::const_iterator * it,
        ...
        template < typename I >
            ...
            ldout(cct,
                  20) << ": on_ready=" << on_ready << ", on_safe=" << on_safe <<
                dendl;

            on_ready =
                util::create_async_context_callback(m_image_ctx, on_ready);
              ...
              for (auto & op_event_pair:m_op_events) {
                    ...
                    if (cancel_ops) {
                        ...
                        if (op_event.on_start_ready == nullptr &&
                            ...
                            std::swap(on_op_finish_event,
                                      op_event.on_op_finish_event);
                            m_image_ctx.op_work_queue->queue(on_op_finish_event,
                                                             -ERESTART);
                        }
                    }
        ...
        template < typename I > void Replay < I >::flush(Context * on_finish) {
            io::AioCompletion * aio_comp;
            {
                ...
                m_lock};
                aio_comp =
                    create_aio_flush_completion(util::
        ...
        template < typename I >
            void Replay < I >::replay_op_ready(uint64_t op_tid,
                                               Context * on_resume) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::AioDiscardEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::AioWriteEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::AioFlushEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::AioWriteSameEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::AioCompareAndWriteEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::OpFinishEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
            ...
            Context *on_op_finish_event = nullptr;
            {
                ...
                if (op_it == m_op_events.end()) {
                    ldout(cct,
                          10) <<
                        ": unable to locate associated op: assuming previously "
                        << "committed." << dendl;
                    on_ready->complete(0);
                ...
                filter_ret_val =
                    (op_event.op_finish_error_codes.count(event.r) != 0);
            }
            ...
            if (event.r < 0) {
                if (op_in_progress) {
                    ...
                    handle_op_complete(event.op_tid,
                                       filter_ret_val ? 0 : event.r);
                }
                return;
            }
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapCreateEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
            ...
            OpEvent *op_event;
            Context *on_op_complete =
                create_op_context_callback(event.op_tid, on_ready,
                                           on_safe, &op_event);
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapRemoveEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
            ...
            op_event->on_op_finish_event =
                new C_RefreshIfRequired < I > (m_image_ctx, new ExecuteOp < I,
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapRenameEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapProtectEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapUnprotectEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
            ...
            op_event->op_finish_error_codes = {
            -EBUSY};
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapRollbackEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::DemotePromoteEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::SnapLimitEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::UpdateFeaturesEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::MetadataSetEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
            ...
            on_op_complete =
                new C_RefreshIfRequired < I > (m_image_ctx, on_op_complete);
            op_event->on_op_finish_event =
        ...
        template < typename I >
            void Replay <
            I >::handle_event(const journal::MetadataRemoveEvent & event,
                              Context * on_ready, Context * on_safe) {
            CephContext *cct = m_image_ctx.cct;
        ...
        template < typename I >
            void Replay < I >::handle_aio_modify_complete(Context * on_ready,
                                                          Context * on_safe,
                                                          int r,
                                                          std::set <
                                                          int >&filters) {
            ...
            if (r < 0) {
                lderr(cct) << ": AIO modify op failed: " << cpp_strerror(r) <<
                    dendl;
        ...
        template < typename I >
            void Replay <
            I >::handle_aio_flush_complete(Context * on_flush_safe,
                                           Contexts & on_safe_ctxs, int r) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 20) << ": r=" << r << dendl;

            if (r < 0) {
                lderr(cct) << ": AIO flush failed: " << cpp_strerror(r) <<
                    dendl;
            }
        ...
        template < typename I >
            ...
            if (m_op_events.count(op_tid) != 0) {
                lderr(cct) << ": duplicate op tid detected: " << op_tid <<
                    dendl;
        ...
        template < typename I >
            ...
            ceph_assert(op_event.on_start_ready == nullptr
                        || (r < 0 && r != -ERESTART));
            if (op_event.on_start_ready != nullptr) {
        ...
        template < typename I >
            ...
            *flush_required = (m_aio_modify_unsafe_contexts.size() ==
            ...
            // on_safe callback
            auto aio_comp =
                io::AioCompletion::create_and_start < Context >
                (new
                 C_AioModifyComplete(this, on_ready, on_safe,
                                     std::move(filters)),
        ...
        template < typename I >
            io::AioCompletion * Replay <
            I >::create_aio_flush_completion(Context * on_safe) {
            ceph_assert(ceph_mutex_is_locked(m_lock));
            ...
            auto aio_comp =
                ...
                (new C_AioFlushComplete(this, on_safe,
                                        std::
                                        move(m_aio_modify_unsafe_contexts)),
        ...
        template < typename I >
            bool Replay < I >::clipped_io(uint64_t image_offset,
                                          io::AioCompletion * aio_comp) {
            CephContext *cct = m_image_ctx.cct;
