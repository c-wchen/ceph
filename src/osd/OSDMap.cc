...
void osd_info_t::encode(bufferlist & bl) const const
{
    __u8 struct_v = 1;
    ::encode(struct_v, bl);
...
void osd_xinfo_t::encode(bufferlist & bl) const const
{
    ENCODE_START(3, 1, bl);
...
void OSDMap::Incremental::encode_client_old(bufferlist & bl) const const
{
    __u16 v = 5;
    ::encode(v, bl);
    ::encode(fsid, bl);
...
void OSDMap::Incremental::encode_classic(bufferlist & bl, uint64_t features) const const
{
...
void OSDMap::Incremental::encode(bufferlist & bl, uint64_t features) const const
{
...
void OSDMap::Incremental::dump(Formatter * f) const const
{
  ...
  for (const auto & upclient:new_up_client) {
        ...
        f->dump_stream("cluster_addr") << new_up_cluster.find(upclient.first)->
            second;
        f->dump_stream("heartbeat_back_addr") << new_hb_back_up.find(upclient.
                                                                     first)->
            second;
...
bool OSDMap::is_blacklisted(const entity_addr_t & a) const const
{
    if (blacklist.empty())
        return false;
...
void OSDMap::get_blacklist(list < pair < entity_addr_t, utime_t > >*bl) const const
{
    std::copy(blacklist.begin(), blacklist.end(), std::back_inserter(*bl));
}

void OSDMap::get_blacklist(std::set < entity_addr_t > *bl) const const
{
...
void OSDMap::get_full_pools(CephContext * cct,
                            ...
                            set < int64_t > *nearfull) const const
{
    assert(full);
    assert(backfillfull);
    assert(nearfull);
...
static bool get_osd_utilization(const mempool::pgmap::unordered_map < int32_t,
                                osd_stat_t > &osd_stat, int id, int64_t * kb,
                                int64_t * kb_used, int64_t * kb_avail)
{
...
void OSDMap::get_full_osd_util(const mempool::pgmap::unordered_map < int32_t,
                               osd_stat_t > &osd_stat, map < int, float >*full,
                               map < int, float >*backfill, map < int,
                               float >*nearfull) const const
{
    full->clear();
    backfill->clear();
    nearfull->clear();
...
void OSDMap::get_all_osds(set < int32_t > &ls) const const
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i))
            ls.insert(i);
...
void OSDMap::get_up_osds(set < int32_t > &ls) const const
{
    for (int i = 0; i < max_osd; i++) {
        if (is_up(i))
            ls.insert(i);
...
void OSDMap::get_out_osds(set < int32_t > &ls) const const
{
    for (int i = 0; i < max_osd; i++) {
        if (is_out(i))
            ls.insert(i);
...
int OSDMap::identify_osd(const entity_addr_t & addr) const const
{
    for (int i = 0; i < max_osd; i++)
...
int OSDMap::identify_osd(const uuid_d & u) const const
{
    for (int i = 0; i < max_osd; i++)
...
int OSDMap::identify_osd_on_all_channels(const entity_addr_t & addr) const const
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i) && (get_addr(i) == addr || get_cluster_addr(i) == addr ||
                          get_hb_back_addr(i) == addr
                          || get_hb_front_addr(i) == addr))
            return i;
    return -1;
}

int OSDMap::find_osd_on_ip(const entity_addr_t & ip) const const
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i)
...
uint64_t OSDMap::get_features(int entity_type, uint64_t * pmask) constconst
{
  ...
  for (auto & pool:pools) {
        ...
        if (pool.second.is_erasure() && entity_type != CEPH_ENTITY_TYPE_CLIENT) {   // not for clients
...
uint8_t OSDMap::get_min_compat_client() const const
{
...
uint64_t OSDMap::get_up_osd_features() const const
{
  ...
  for (auto pg:*tmpmap.pg_temp) {
      ...
      for (auto o:pg.second) {
        ...
        if (vectors_equal(raw_up, pg.second)) {
            ldout(cct,
                  10) << __func__ << "  removing pg_temp " << pg.
                first << " " << pg.
                second << " that matches raw_up mapping" << dendl;
            remove = true;
        }
        // oversized pg_temp?
        if (pg.second.size() > tmpmap.get_pg_pool(pg.first.pool())->get_size()) {
            ldout(cct,
                  10) << __func__ << "  removing pg_temp " << pg.
                first << " " << pg.second << " exceeds pool size" << dendl;
            remove = true;
        }
        if (remove) {
  ...
  for (auto & pg:to_check) {
      ...
      for (auto osd:raw_up) {
        ...
        if (it == rule_weight_map.end()) {
            auto r =
                tmpmap.crush->get_rule_weight_osd_map(crush_rule, &weight_map);
            if (r < 0) {
        ...
        {                       // pg_upmap
            ...
            if (osdmap.pg_upmap.count(pg)) {
                ldout(cct, 10) << __func__ << " cancel invalid pg_upmap entry "
                    ...
                    << osdmap.pg_upmap.find(pg)->second << dendl;
        ...
        {                       // pg_upmap_items
            ...
            if (osdmap.pg_upmap_items.count(pg)) {
                ldout(cct, 10) << __func__ << " cancel invalid "
                    ...
                    << osdmap.pg_upmap_items.find(pg)->second << dendl;
...
int OSDMap::apply_incremental(const Incremental & inc)
{
  ...
  for (const auto & client:inc.new_up_client) {
        ...
        osd_addrs->client_addr[client.first].
            reset(new entity_addr_t(client.second));
        if (inc.new_hb_back_up.empty())
            ...
            osd_addrs->hb_back_addr[client.first].
                reset(new
                      entity_addr_t(inc.new_hb_back_up.find(client.first)->
                                    second));
        const auto j = inc.new_hb_front_up.find(client.first);
        if (j != inc.new_hb_front_up.end())
            osd_addrs->hb_front_addr[client.first].
                reset(new entity_addr_t(j->second));
        else
  ...
  for (const auto & cluster:inc.new_up_cluster)
        osd_addrs->cluster_addr[cluster.first].
            reset(new entity_addr_t(cluster.second));
...
int OSDMap::map_to_pg(int64_t poolid,
                      const string & name,
                      const string & key,
                      const string & nspace, pg_t * pg) const const
{
...
void OSDMap::_pg_to_raw_osds(const pg_pool_t & pool, pg_t pg,
                             vector < int >*osds, ps_t * ppps) const const
{
    // map to osds[]
...
int OSDMap::_pick_primary(const vector < int >&osds) const const
{
  for (auto osd:osds) {
        if (osd != CRUSH_ITEM_NONE) {
            return osd;
...
void OSDMap::_apply_upmap(const pg_pool_t & pi, pg_t raw_pg, vector < int >*raw) const const
{
...
void OSDMap::pg_to_raw_osds(pg_t pg, vector < int >*raw, int *primary) const const
{
    *primary = -1;
    raw->clear();
...
void OSDMap::pg_to_raw_upmap(pg_t pg, vector < int >*raw_upmap) const const
{
...
void OSDMap::pg_to_raw_up(pg_t pg, vector < int >*up, int *primary) const const
{
    const pg_pool_t *pool = get_pg_pool(pg.pool());
    if (!pool) {
        if (primary)
...
bool OSDMap::primary_changed(int oldprimary,
...
uint64_t OSDMap::get_encoding_features() const const
{
    ...
    if (require_osd_release < CEPH_RELEASE_JEWEL) {
        f &= ~(CEPH_FEATURE_SERVER_JEWEL |
               CEPH_FEATURE_NEW_OSDOP_ENCODING | CEPH_FEATURE_CRUSH_TUNABLES5);
    }
    return f;
}

// serialize, unserialize
void OSDMap::encode_client_old(bufferlist & bl) const const
{
    __u16 v = 5;
    ::encode(v, bl);
...
void OSDMap::encode_classic(bufferlist & bl, uint64_t features) const const
{
    if ((features & CEPH_FEATURE_PGID64) == 0) {
        encode_client_old(bl);
        return;
...
void OSDMap::encode(bufferlist & bl, uint64_t features) const const
{
    if ((features & CEPH_FEATURE_OSDMAP_ENC) == 0) {
        encode_classic(bl, features);
        return;
...
void OSDMap::dump_erasure_code_profiles(const mempool::osdmap::map < string,
...
void OSDMap::dump(Formatter * f) const const
{
...
string OSDMap::get_flag_string() const const
{
    return get_flag_string(flags);
}

void OSDMap::print_pools(ostream & out) const const
{
  for (const auto & pool:pools) {
        std::string name("<unknown>");
        const auto & pni = pool_name.find(pool.first);
      ...
      for (const auto & snap:pool.second.snaps)
            out << "\tsnap " << snap.second.snapid << " '" << snap.second.
                name << "' " << snap.second.stamp << "\n";
...
void OSDMap::print(ostream & out) const const
{
    out << "epoch " << get_epoch() << "\n"
        << "fsid " << get_fsid() << "\n"
        << "created " << get_created() << "\n"
...
void OSDMap::print_tree(Formatter * f, ostream * out, unsigned filter) const const
{
    if (f) {
...
void OSDMap::print_summary(Formatter * f, ostream & out, const string & prefix) const const
{
    if (f) {
...
void OSDMap::print_oneline_summary(ostream & out) const const
{
    out << "e" << get_epoch() << ": "
        << get_num_osds() << " total, "
        << get_num_up_osds() << " up, " << get_num_in_osds() << " in";
...
bool OSDMap::crush_rule_in_use(int rule_id) const const
{
  for (const auto & pool:pools) {
...
int OSDMap::validate_crush_rules(CrushWrapper * newcrush, ostream * ss) const const
{
  for (auto & i:pools) {
        auto & pool = i.second;
        ...
        if (pool.get_size() < (int)newcrush->get_rule_mask_min_size(ruleno) ||
            ...
            *ss << "pool " << i.first << " size " << pool.
                get_size() << " does not" << " fall within rule " << ruleno <<
    ...
    if (nosd >= 0) {
      ...
      for (auto & section:sections) {
            ...
            if (o > cct->_conf->mon_max_osd) {
                lderr(cct) << "[osd." << o <<
                    "] in config has id > mon_max_osd " << cct->_conf->
                    mon_max_osd << dendl;
                return -ERANGE;
            }
...
int OSDMap::build_simple_crush_rules(CephContext * cct,
                                     CrushWrapper & crush,
  ...
  for (int64_t pool_id:ls) {
        ...
        for (unsigned ps = 0; ps < pi->get_pg_num(); ++ps) {
          ...
          for (int osd:up) {
            ...
            if (newmap) {
                newmap->pg_to_up_acting_osds(pgid, &up2, &up_primary, nullptr,
                                             nullptr);
              for (int osd:up2) {
                    if (osd >= 0 && osd < get_max_osd())
                        ++new_by_osd[osd];
...
int OSDMap::clean_pg_upmaps(CephContext * cct, Incremental * pending_inc) const const
{
    ldout(cct, 10) << __func__ << dendl;
    int changed = 0;
  for (auto & p:pg_upmap) {
...
int OSDMap::calc_pg_upmaps(CephContext * cct,
                           ...
                           OSDMap::Incremental * pending_inc)
{
  ...
  for (auto & i:pools) {
        ...
        ldout(cct, 20) << __func__ << " pool " << i.first
            << " ruleno " << ruleno << " weight-map " << pmap << dendl;
      for (auto p:pmap) {
  ...
  for (auto & i:osd_weight) {
    ...
    if (osd_weight_total == 0) {
        lderr(cct) << __func__ << " abort due to osd_weight_total == 0" <<
            dendl;
        return 0;
    }
  ...
  for (auto & i:pgs_by_osd) {
    ...
    while (max--) {
        ...
        while (overfull.empty()) {
            for (auto i = deviation_osd.rbegin(); i != deviation_osd.rend();
                 i++) {
      ...
      retry:
        ...
        set < pg_t > to_unmap;
        map < pg_t, mempool::osdmap::vector < pair < int32_t,
            int32_t >>> to_upmap;
        ...
        for (auto p = deviation_osd.rbegin(); p != deviation_osd.rend(); ++p) {
          ...
          for (auto pg:pgs) {
                ...
                if (it != tmp.pg_upmap_items.end() &&
                    ...
                    ldout(cct,
                          10) << " " << pg <<
                        " already has full-size pg_upmap_items " << it->
                        second << ", skipping" << dendl;
                    continue;
                }
                else if (it != tmp.pg_upmap_items.end()) {
                    ldout(cct,
                          10) << " " << pg << " already has pg_upmap_items " <<
                        it->second << dendl;
      ...
      for (auto & i:to_upmap) {
            ldout(cct, 10) << " upmap pg " << i.first
                << " new pg_upmap_items " << i.second << dendl;
...
int OSDMap::get_osds_by_bucket_name(const string & name, set < int >*osds) const const
{
...
void OSDMap::get_pool_ids_by_osd(CephContext * cct,
                                 int osd, set < int64_t > *pool_ids) const const
{
    assert(pool_ids);
    set < int >raw_rules;
    int r = crush->get_rules_by_osd(osd, &raw_rules);
    if (r < 0) {
        lderr(cct) << __func__ << " get_rules_by_osd failed: " <<
            cpp_strerror(r)
            << dendl;
        assert(r >= 0);
    }
...
template < typename F > class OSDUtilizationDumper:public CrushTreeDumper::Dumper < F >
{
  public:
    ...
    OSDUtilizationDumper(const CrushWrapper * crush, const OSDMap * osdmap_,
                         const PGStatService * pgs_, bool tree_):Parent(crush,
                                                                        osdmap_->
                                                                        get_pool_names
                                                                        ()),
        osdmap(osdmap_), pgs(pgs_), tree(tree_),
        average_util(average_utilization()), min_var(-1), max_var(-1),
        stddev(0), sum(0) {
    }
  ...
  protected:
    ...
    virtual void dump_item(const CrushTreeDumper::Item & qi,
                           ...
                           int64_t kb_avail,
                           double &util,
                           double &var, const size_t num_pgs, F * f) = 0;
...
class OSDUtilizationPlainDumper:public OSDUtilizationDumper < TextTable > {
  public:
    ...
    OSDUtilizationPlainDumper(const CrushWrapper * crush, const OSDMap * osdmap,
                              const PGStatService * pgs,
                              bool tree):Parent(crush, osdmap, pgs, tree) {
    } void dump(TextTable * tbl) {
        tbl->define_column("ID", TextTable::LEFT, TextTable::RIGHT);
        tbl->define_column("CLASS", TextTable::LEFT, TextTable::RIGHT);
        tbl->define_column("WEIGHT", TextTable::LEFT, TextTable::RIGHT);
        ...
        *tbl << ""
            << "" << "" << "TOTAL" << byte_u_t(pgs->get_osd_sum().kb << 10)
                     ...
                     const OSDUtilizationPlainDumper::lowprecision_t & v)
{
    if (v.v < -0.01) {
        ...
        return out << std::fixed << std::setprecision(2) << v.
...
class OSDUtilizationFormatDumper:public OSDUtilizationDumper < Formatter > {
  public:
    ...
    OSDUtilizationFormatDumper(const CrushWrapper * crush,
                               const OSDMap * osdmap, const PGStatService * pgs,
                               bool tree):Parent(crush, osdmap, pgs, tree) {
    } void dump(Formatter * f) {
        f->open_array_section("nodes");
        Parent::dump(f);
...
void OSDMap::check_health(health_check_map_t * checks) const const
{
        ...
        for (int i = 0; i < get_max_osd(); i++) {
            ...
            if (!is_up(i)) {
                ...
                for (int type = 0; type <= max_type; type++) {
                    ...
                    if (!subtree_type_is_down(g_ceph_context, parent_id, type,
    ...
    if (g_conf->mon_warn_on_crush_straw_calc_version_zero) {
        if (crush->get_straw_calc_version() == 0) {
            ...
            auto & d =
                checks->add("OLD_CRUSH_STRAW_CALC_VERSION", HEALTH_WARN,
                            ss.str());
            d.detail.
                push_back
                ("see http://docs.ceph.com/docs/master/rados/operations/crush-map/#tunables");
    ...
    if (g_conf->mon_warn_on_cache_pools_without_hit_sets) {
        ...
        if (!detail.empty()) {
            ...
            auto & d =
                checks->add("CACHE_POOL_NO_HIT_SET", HEALTH_WARN, ss.str());
            d.detail.swap(detail);
        }
    }
