...
struct function_arg_extractor {
    ...
    private:
        ...
        Function >::type >::type bare_type;
        typedef typename signature_of <
        bare_type >::type normalized_function_type;
        typedef typename boost::function_types::function_arity <
        normalized_function_type >::type arity;
        typedef typename boost::function_types::parameter_types <
        normalized_function_type >::type parameter_types;
        typedef typename boost::function_types::result_type <
        normalized_function_type >::type result_type;

        BOOST_STATIC_ASSERT_MSG((arity::value == 1),
                                "make_visitor called with a non-unary function");

        typedef typename boost::mpl::front <
        parameter_types >::type parameter_type;
    public:
        typedef typename boost::remove_const <
        typename boost::remove_reference <
        parameter_type >::type >::type type;
    };
};
...
template < typename Allowed > struct check_in {
    ...
    private:
        BOOST_STATIC_ASSERT_MSG((boost::mpl::contains < Allowed,
...
public:
    typedef typename boost::mpl::fold < Seq, boost::mpl::map0 <>,
            insert_helper >::type type;
};
...
struct generic_visitor: boost::static_visitor < Result >,
    boost::noncopyable {
private:
    ...
    typedef typename boost::mpl::transform < function_types,
            function_arg_extractor >::type arg_types;
    typedef typename boost::mpl::transform < arg_types,
            boost::mpl::range_c < int, 0,
            boost::mpl::size < arg_types >::value >,
            make_pair >::type pair_list;
    typedef typename as_map < pair_list >::type fmap;

    // Check that the argument types are unique
    BOOST_STATIC_ASSERT_MSG((boost::mpl::size < fmap >::value ==
                             boost::mpl::size < arg_types >::value),
    ...
    typedef typename boost::mpl::fold < fmap, void,
            check_in < typename Variant::types > >::type dummy;
...
struct function_return_extractor {
    ...
    struct apply: boost::function_types::result_type <
        typename signature_of < Function >::type > {
    };
};
...
private:
    ...
    typedef typename boost::mpl::transform < bare_function_types,
            function_return_extractor >::type return_types;
...
private:
    ...
    typedef typename boost::mpl::fold < return_types, result_type,
            check_same >::type dummy;
};
...
{
    return typename detail::get_generic_visitor < Variant,
           Functions ... >::type(boost::forward < Functions > (functions) ...);
}
...
->typename detail::get_generic_visitor < Variant,
Functions ... >::result_type
{
    return boost::apply_visitor(detail::make_visitor < Variant >
                                (boost::forward < Functions > (functions) ...),
                                variant);
}
