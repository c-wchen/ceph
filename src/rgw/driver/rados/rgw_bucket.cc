...
int rgw_remove_object(const DoutPrefixProvider *dpp, rgw::sal::Driver* driver, rgw::sal::Bucket* bucket,
                      rgw_obj_key& key, optional_yield y)
{
...
int RGWBucket::set_quota(RGWBucketAdminOpState& op_state, const DoutPrefixProvider *dpp, optional_yield y,
                         std::string *err_msg)
{
...
int RGWBucket::remove_object(const DoutPrefixProvider *dpp, RGWBucketAdminOpState& op_state, optional_yield y,
                             std::string *err_msg)
{
        ...
        for (iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            if (op_state.will_fix_index()) {
                ...
                if (olh_entry.pending_log.empty()) {
                    ...
                    if (ret < 0) {
                        ldpp_dout(dpp, -1) << "ERROR failed to clear olh for: " << olh_entry.key.name << " clear_olh(): " << cpp_strerror(
                                               -ret) << dendl;
                        continue;
                    }
                } else {
                    ...
                    if (ret < 0) {
                        ldpp_dout(dpp, -1) << "ERROR failed to get state for: " << olh_entry.key.name << " get_obj_state(): " << cpp_strerror(
                                               -ret) << dendl;
                        continue;
                    }
                    ret = store->update_olh(dpp, obj_ctx, state, bucket->get_info(), obj, y);
                    if (ret < 0) {
                        ldpp_dout(dpp, -1) << "ERROR failed to update olh for: " << olh_entry.key.name << " update_olh(): " << cpp_strerror(
                                               -ret) << dendl;
                        continue;
                    }
                }
...
int RGWBucket::sync(RGWBucketAdminOpState& op_state, const DoutPrefixProvider *dpp, optional_yield y,
                    std::string *err_msg)
{
                             ...
                             optional_yield y, string *err)
{
    ...
    return radosdriver->ctl()->bucket->unlink_bucket(*rados, op_state.get_user_id(),
            op_state.get_bucket()->get_info().bucket, y, dpp, true);
}
                           ...
                           optional_yield y, string *err)
{
    ...
    r = radosdriver->ctl()->bucket->link_bucket(*rados, op_state.get_user_id(), loc_bucket->get_info().bucket,
...
int RGWBucketAdminOp::chown(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, const string& marker,
                            const DoutPrefixProvider *dpp, optional_yield y, string *err)
{
    RGWBucket bucket;
...
int RGWBucketAdminOp::sync_bucket(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
                                  const DoutPrefixProvider *dpp, optional_yield y, string *err_msg)
{
    RGWBucket bucket;
...
public:
                  ...
                  optional_yield y, const DoutPrefixProvider *dpp) override
    {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to put new bucket instance info for bucket=" << new_bi.bucket << " ret=" << ret <<
                              dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to put new bucket entrypoint for bucket=" << new_be.bucket << " ret=" << ret <<
                              dendl;
            return ret;
        }
...
public:
               ...
               const DoutPrefixProvider *dpp) override
    {
        ...
        RGWBucketInstanceMetadataObject *mdo = new RGWBucketInstanceMetadataObject(bci, bci.info.objv_tracker.read_version,
            mtime);
...
public:
    RGWMetadataHandlerPut_BucketInstance(CephContext *_cct,
                                         ...
                                         RGWMetadataObject *_obj, RGWObjVersionTracker& objv_tracker,
                                         optional_yield y,
                                         RGWMDLogSyncType type, bool from_remote_zone) : RGWMetadataHandlerPut_SObj(_handler, _op, entry, _obj, objv_tracker, y,
...
public:
    ...
    int do_remove(RGWSI_MetaBackend_Handler::Op *op, string& entry, RGWObjVersionTracker& objv_tracker, optional_yield y,
                  const DoutPrefixProvider *dpp) override
    {
