...
namespace crimson {
    namespace dmclock {
         ...
         */
        static void test_locked(std::mutex & mtx,
            ...
            EXPECT_DEATH_IF_SUPPORTED(pq->add_request(Request {
                                                      }
                                                      , client1, req_params),
        ...
        TEST(dmclock_server, client_idle_erase) {
            ...
            Queue pq(client_info_f,
                     ...
                     std::chrono::seconds(5), std::chrono::seconds(2), false);
        ...
        TEST(dmclock_server, reservation_timing) {
            ...
            // NB? PUSH OR PULL
            using Queue =
                std::unique_ptr < dmc::PriorityQueue < ClientId, Request >>;
            ...
            auto submit_req_f =[&](const ClientId & c,
                                   ...
                                   dmc::PhaseType phase){
                {
                 Guard g(times_mtx); times.emplace_back(dmc::get_time());}
                 std::thread complete([&]() {
                                      pq->request_completed();
                                      }); complete.detach();};
                 // NB? PUSH OR PULL
                 pq =
                 Queue(new dmc::PriorityQueue < ClientId,
                       Request > (client_info_f, server_ready_f, submit_req_f,
                                  false)); Request req;
                 ...
                 pq.remove_by_req_filter([](const MyReq & r)->bool {
                                         return 1 == r.id % 2;}
                 );
                 EXPECT_EQ(5u, pq.request_count());
                 std::list < MyReq > capture;
                 pq.remove_by_req_filter([&capture] (const MyReq & r)->bool {
                                         if (0 == r.id % 2) {
                                         capture.push_front(r); return true;}
                                         else {
                                         return false;}
                                         }
          , true); EXPECT_EQ(0u, pq.request_count()); EXPECT_EQ(5u, capture.size()); int total = 0; for (auto i:capture)
                 {
                 total += i.id;}
                 EXPECT_EQ(146, total) << " sum of captured items should be 146";}  // TEST
                 ...
                 pq.remove_by_req_filter([&capture] (const MyReq & r)->bool {
                                         if (1 == r.id % 2) {
                                         capture.push_back(r); return true;}
                                         else {
                                         return false;}
                                         }
                                         ,
                 ...
                 EXPECT_EQ(1,
                           capture[0].
                           id) << "items should come out in forward order";
                 EXPECT_EQ(3,
                           capture[1].
                           id) << "items should come out in forward order";
                 EXPECT_EQ(5,
                 ...
                 pq.remove_by_req_filter([&capture2] (const MyReq & r)->bool {
                                         if (0 == r.id % 2) {
                                         ...
                                         , false); EXPECT_EQ(0u, pq.request_count()); EXPECT_EQ(3u, capture2.size()); EXPECT_EQ(6, capture2[0].id) << "items should come out in reverse order"; EXPECT_EQ(4, capture2[1].id) << "items should come out in reverse order"; EXPECT_EQ(2, capture2[2].id) << "items should come out in reverse order";}    // TEST
                 ...
                 pq.remove_by_req_filter([&capture2] (const MyReq & r)->bool {
                                         if (0 == r.id % 2) {
                                         capture2.push_back(r); return true;}
                                         else {
                                         return false;}
                                         }
                                         , true); EXPECT_EQ(0u, pq.request_count()); EXPECT_EQ(3u, capture2.size()); EXPECT_EQ(6, capture2[0].id) << "items should come out in reverse order"; EXPECT_EQ(4, capture2[1].id) << "items should come out in reverse order"; EXPECT_EQ(2, capture2[2].id) << "items should come out in reverse order";} // TEST
                 ...
                 if (client2 == c) return info2;
                 else {
                 ADD_FAILURE() <<
                 "client info looked up for non-existant client"; return info1;}
                 }; pq = QueueRef(new Queue(client_info_f, false));
                 ReqParams req_params(1, 1); auto now = dmc::get_time();
                 for (int i = 0; i < 5; ++i) {
                 pq->add_request(Request {
                                 }, client1, req_params);
                 pq->add_request(Request {
                                 }, client2, req_params); now += 0.0001;}

                 int c1_count = 0; int c2_count = 0; for (int i = 0; i < 6; ++i) {
                 ...
                 if (client2 == retn.client)++ c2_count;
                 else
                 ADD_FAILURE() << "got request from neither of two clients";
                 ...
                 for (int i = 0; i < 5; ++i) {
                 pq->add_request_time(Request {
                                      }, client1, req_params, old_time);
                 pq->add_request_time(Request {
                                      }, client2, req_params, old_time);
                 old_time += 0.001;}
                 ...
                 EXPECT_EQ(4, c1_count) << "two-thirds of request should have come from first client"; EXPECT_EQ(2, c2_count) << "one-third of request should have come from second client";}   // dmclock_server_pull.pull_reservation
                 ...
                 for (int i = 0; i < 3; ++i) {
                 pq->add_request_time(Request {
                                      }, client1, req_params, start_time);
                 pq->add_request_time(Request {
                                      }, client2, req_params, start_time);}
                 ...
                 EXPECT_EQ(Queue::NextReqType::returning, pr.type);
                 pr = pq->pull_request(start_time + 2.5);
                 EXPECT_EQ(Queue::NextReqType::none,
                           pr.type) << "no more requests left";}
                 ...
                 auto client_info_f =[&](ClientId c)->dmc::ClientInfo {
                 return info;}; QueueRef pq(new Queue(client_info_f, false));
                 // Request req;
                 ReqParams req_params(1, 1);
                 ...
                 ReqParams req_params(1, 1);
                 // make sure all times are well before now
                 auto now = dmc::get_time(); pq->add_request_time(Request {
                                                                  }
                                                                  , client1,
                                                                  req_params,
                                                                  now + 100);
