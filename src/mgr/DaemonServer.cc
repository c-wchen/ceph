...
audit_clog(audit_clog_), auth_cluster_registry(g_ceph_context,
                                               g_conf->auth_supported.
                                               empty()? g_conf->
                                               auth_cluster_required : g_conf->
                                               auth_supported),
auth_service_registry(g_ceph_context,
                      g_conf->auth_supported.empty()? g_conf->
                      auth_service_required : g_conf->auth_supported),
lock("DaemonServer"), pgmap_ready(false)
{
...
int DaemonServer::init(uint64_t gid, entity_addr_t client_addr)
{
    ...
    msgr = Messenger::create(g_ceph_context, public_msgr_type,
                             entity_name_t::MGR(gid), "mgr", getpid(), 0);
...
entity_addr_t DaemonServer::get_myaddr() const const
{
                                        ...
                                        AuthAuthorizerChallenge > *challenge)
{
    ...
    if (keys) {
        is_valid = handler->verify_authorizer(cct, keys,
                                              authorizer_data,
...
bool DaemonServer::handle_open(MMgrOpen * m)
{
    ...
    if (daemon) {
        dout(20) << "updating existing DaemonState for " << m->
            daemon_name << dendl;
...
bool DaemonServer::handle_report(MMgrReport * m)
{
    ...
    if (daemon_state.exists(key)) {
...

        {
            Mutex::Locker l(lock);
            // kill session
            MgrSessionRef session(static_cast <
                                  MgrSession *
...
void DaemonServer::_generate_command_map(map < string, cmd_vartype > &cmdmap,
...
bool DaemonServer::handle_command(MCommand * m)
{
    ...
    class CommandContext {
      public:
        ...
        void reply(int r, const std::string & rs) {
            ...
            dout(1) << "handle_command " << cpp_strerror(r) << " " << rs <<
                dendl;
            if (con) {
   ...
   */
    ...
    std::shared_ptr < CommandContext > cmdctx =
        std::make_shared < CommandContext > (m);

    MgrSessionRef session(static_cast <
                          MgrSession * >(m->get_connection()->get_priv()));
    if (!session) {
        return true;
    }
    session->put();             // SessionRef takes a ref
    ...
    {
        cmd_getval(g_ceph_context, cmdctx->cmdmap, "format", format,
                   string("plain"));
    ...
    if (!mgr_cmd) {
        ...
        if (!_allowed_command
            (session.get(), py_command.module, prefix, cmdctx->cmdmap,
        ...
        if (!_allowed_command
            (session.get(), mgr_cmd->module, prefix, cmdctx->cmdmap,
    ...
    if (prefix == "pg scrub" ||
      ...
      for (auto osd:osds) {
        ...
        bool dry_run = prefix == "osd test-reweight-by-pg"
            || prefix == "osd test-reweight-by-utilization";
        int64_t oload;
        cmd_getval(g_ceph_context, cmdctx->cmdmap, "oload", oload,
                   int64_t(120));
        set < int64_t > pools;
        ...
        cmd_getval(g_ceph_context, cmdctx->cmdmap, "no_increasing",
                   no_increasing);
        string out_str;
        ...
        cluster_state.with_pgmap([&](const PGMap & pg_map) {
          ...
          int num_active_clean = 0; for (auto & p:pg_map.num_pg_by_state)
              ...
              for (auto osd:                osds) {
                                             ...
                                             auto q =
                                             pg_map.num_pg_by_osd.find(osd);
                                             if (q !=
                                                 pg_map.num_pg_by_osd.end()) {
                                             if (q->second.acting > 0
                                                 || q->second.up > 0) {
                                             active_osds.insert(osd);
                                             affected_pgs +=
                                             q->second.acting + q->second.up;
                                             continue;}
                                             }
                                             if (num_active_clean <
                                                 pg_map.num_pg) {
        ...
        cluster_state.with_pgmap([&](const PGMap & pg_map) {
              ...
              for (auto & p:                pg_delta)
                                             {
                                             auto q =
                                             pg_map.pg_stat.find(p.first);
                                             if (q == pg_map.pg_stat.end()) {
                                             ss << "missing information about "
                                             << p.
                                             first << "; cannot draw" <<
                                             " any conclusions"; r = -EAGAIN;
                                             return;}
                                             if (!
                                                 (q->second.
                                             ...
                                             ++dangerous_pgs; continue;}
                                             const pg_pool_t * pi =
                                             osdmap.get_pg_pool(p.first.pool());
                                             if (!pi) {
                                             ++dangerous_pgs;   // pool is creating or deleting
                                             }
                                             else {
                                             if (q->second.acting.size() +
                                                 p.second < pi->min_size) {
                                             ++dangerous_pgs;}
                                             }
                                             }
                                             }
...
case OFR_RECOVERY:
                                     ...
                                     if (workpg.
                                         state & PG_STATE_FORCED_RECOVERY) {
                                     ss << "pg " << pstr <<
...
break; case OFR_BACKFILL:
                                     ...
                                     if (workpg.
                                         state & PG_STATE_FORCED_BACKFILL) {
                                     ss << "pg " << pstr <<
...
break; case OFR_BACKFILL | OFR_CANCEL:
                                     if ((workpg.
                                         ...
                                         0) {
                                     ss << "pg " << pstr <<
                                     " backfill not forced; "; continue;}
break; case OFR_RECOVERY | OFR_CANCEL:
                                     if ((workpg.
                                          state & PG_STATE_FORCED_RECOVERY) ==
                                         0) {
                                     ss << "pg " << pstr <<
                                     " recovery not forced; "; continue;}
break; default:
                                     assert(0 ==
              ...
              for (auto & pgid:     parsed_pgs) {
                                     ...
                                     if (workit != pg_map.pg_stat.end()) {
                      pg_stat_t workpg = workit->second; set < int32_t > osds(workpg.up.begin(), workpg.up.end()); osds.insert(workpg.acting.begin(), workpg.acting.end()); for (auto i:osds)
                                     {
        ...
        cluster_state.with_osdmap([&](const OSDMap & osdmap) {
          for (auto & i:         osdpgs) {
                                  ...
                                  vector < pg_t >
                                  pgvec(make_move_iterator(i.second.begin()),
                                        make_move_iterator(i.second.end()));
...
void DaemonServer::send_report()
{
    ...
    cluster_state.with_pgmap([&](const PGMap & pg_map) {
                             ...
                             ceph_clock_now();::encode(pending_service_map,
                                                       m->service_map_bl,
                                                       CEPH_FEATURES_ALL);
                             ...
                             with_osdmap([&](const OSDMap & osdmap) {
                                         ...
                                         get_health_checks(g_ceph_context,
                                                           osdmap,
                                                           &m->health_checks);
                                         dout(10) << m->health_checks.checks.
  ...
  for (const auto & osd:osds) {
      ...
      for (const auto & metric:osd.second->osd_health_metrics) {
            ...
            if (acc == accumulated.end()) {
                auto collector =
                    OSDHealthMetricCollector::create(metric.get_type());
                if (!collector) {
