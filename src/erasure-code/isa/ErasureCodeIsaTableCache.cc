...
unsigned char **ErasureCodeIsaTableCache::
getEncodingCoefficientNoLock(int matrix, int k, int m)
{
...
Mutex *ErasureCodeIsaTableCache::getLock()
{
                                                         ...
                                                         int m)
{
    ...
    if (decode_tbls_map->count(signature)) {
        ...
        memcpy(table, (*decode_tbls_map)[signature].second.c_str(),
               k * (m + k) * 32);
        // find item in LRU queue and push back
        dout(12) << "[ cache size   ] = " << decode_tbls_lru->size() << dendl;
        decode_tbls_lru->splice((decode_tbls_lru->begin()), *decode_tbls_lru,
