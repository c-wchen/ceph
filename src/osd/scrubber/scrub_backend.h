...
using digests_fixes_t =
    std::vector < std::pair < hobject_t, data_omap_digests_t >>;
...
struct shard_as_auth_t {
    ...
    :possible_auth {
    usable_t::not_usable}, error_text {
    err_msg}
    , oi {
    }
    , auth_iter {
  ...
  :    possible_auth {
    ...
    shard_as_auth_t(const object_info_t & anoi,
                    ...
                    std::string err_msg, std::optional < uint32_t > data_digest)
    :possible_auth {
    usable_t::usable}
    , error_text {
    err_msg}
...
template <> struct fmt::formatter < shard_as_auth_t > {
    ...
    template < typename FormatContext >
        ...
        if (debug_log) {
            ...
            return fmt::format_to(ctx.out(),
                                  "usable:{} soid:{} {{txt:{}}}",
                                  (as_auth.possible_auth ==
                                   shard_as_auth_t::usable_t::
                                   usable) ? "yes" : "no", as_auth.oi.soid,
                                  as_auth.error_text);
        }
    }
...
struct auth_selection_t {
    ...
    false};                     ///< managed to select an auth' source?
    bool digest_match {
    true};                      ///< do all (existing) digests match?
};
...
struct scrub_chunk_t {
    ...
    explicit scrub_chunk_t(pg_shard_t i_am) {
...
class ScrubBackend {
  public:
    ...
    ScrubBackend(ScrubBeListener & scrubber,
                 PgScrubBeListener & pg,
                 pg_shard_t i_am, bool repair, scrub_level_t shallow_or_deep);
   ...
   */
    ...
    std::vector < Scrub::snap_mapper_fix_t > replica_clean_meta(ScrubMap & smap,
                                                                bool
                                                                max_reached,
                                                                const hobject_t
                                                                & start,
   ...
   */
    ...
    const omap_stat_t & this_scrub_omapstats() const {
        return m_omap_stats;
    } int authoritative_peers_count() const {
        return m_auth_peers.size();
    };
  ...
  private:
    ...
    const PGPool & m_pool;
    bool m_incomplete_clones_allowed {
    false};
  ...
  private:
    ...
    /// shallow/deep error counters
    error_counters_t get_error_counts() const {
   ...
   */ void merge_to_authoritative_set();
    ...
    std::optional < auth_and_obj_errs_t > for_empty_auth_list(std::list <
                                                              pg_shard_t >
                                                              &&auths,
                                                              std::set <
                                                              pg_shard_t >
   ...
   */
    ...
    void inconsistents(const hobject_t & ho, ScrubMap::object & auth_object, object_info_t & auth_oi,   // consider moving to object
   ...
   */
    std::vector < Scrub::snap_mapper_fix_t > scan_snaps(ScrubMap & smap,
                                                        Scrub::
                                                        SnapMapReaderI &
                                                        snaps_getter);
   ...
   */
    std::optional < Scrub::snap_mapper_fix_t >
        scan_object_snaps(const hobject_t & hoid, const SnapSet & snapset,
                          Scrub::SnapMapReaderI & snaps_getter);
