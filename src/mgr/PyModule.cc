...
std::string handle_pyerror(bool crash_dump,
                           std::string module, std::string caller)
{
...
std::pair < int, std::string > PyModuleConfig::set_config(MonClient * monc,
                                                          const std::
                                                          string & module_name,
                                                          const std::
                                                          string & key,
...
std::string PyModule::get_site_packages()
{
    ...
    auto site_packages_fn =
        PyObject_GetAttrString(site_module, "getsitepackages");
    if (site_packages_fn != nullptr) {
        auto site_packages_list =
            PyObject_CallObject(site_packages_fn, nullptr);
        ...
        for (Py_ssize_t i = 0; i < n; ++i) {
            ...
            site_packages <<
                PyUnicode_AsUTF8(PyList_GetItem(site_packages_list, i));
        }
...
int PyModule::load(PyThreadState * pMainThreadState)
{
    ...
    // Configure sub-interpreter
    {
        ...
        if (thread_state == nullptr) {
            derr << "Failed to create python sub-interpreter for '" <<
                module_name << '"' << dendl;
            return -EINVAL;
        }
        else {
            ...
            string paths =
                (g_conf().get_val < std::string >
                 ("mgr_module_path") + ':' + get_site_packages() + ':');
    ...
    // Environment is all good, import the external module
    {
        ...
        if (r) {
            derr << "Class not found in module '" << module_name << "'" <<
                dendl;
            return r;
        }
...
int PyModule::walk_dict_list(const std::string & attr_name,
...
{
    PyObject *pRegCmd = PyObject_CallMethod(cls,
                                            const_cast <
                                            char *>("_register_options"),
                                            const_cast < char *>("(s)"),
                                            module_name.c_str());
    if (pRegCmd != nullptr) {
        Py_DECREF(pRegCmd);
        ...
        derr << handle_pyerror(true, module_name,
                               "PyModule::register_options") << dendl;
    }
    return 0;
}
...
int PyModule::load_notify_types()
{
    ...
    if (!PyObject_TypeCheck(ls, &PyList_Type)) {
        ...
        // as a {} instead of a []
        derr << "Module " << get_name() <<
            " has NOTIFY_TYPES that is not a list" << dendl;
        return -EINVAL;
    }
...
int PyModule::load_options()
{
    int r = walk_dict_list("MODULE_OPTIONS",[this] (PyObject * pOption)->int {
                           ...
                           int t = Option::str_to_type(s); if (t >= 0) {
                           option.type = t;}
                           }
                           ...
                           p = PyDict_GetItemString(pOption, "default"); if (p) {
                           auto q = PyObject_Str(p);
                           option.default_value = PyUnicode_AsUTF8(q);
                           Py_DECREF(q);}
                           p = PyDict_GetItemString(pOption, "min"); if (p) {
                           auto q = PyObject_Str(p);
                           option.min = PyUnicode_AsUTF8(q); Py_DECREF(q);}
                           p = PyDict_GetItemString(pOption, "max"); if (p) {
                           auto q = PyObject_Str(p);
                           option.max = PyUnicode_AsUTF8(q); Py_DECREF(q);}
                           p = PyDict_GetItemString(pOption, "enum_allowed");
                           if (p && PyObject_TypeCheck(p, &PyList_Type)) {
                           for (Py_ssize_t i = 0; i < PyList_Size(p); ++i) {
                           auto q = PyList_GetItem(p, i); if (q) {
                           ...
                           dout(20) << "loaded module option " << option.
                           name << dendl;
                           options[option.name] = std::move(option); return 0;}
    ) ;
...
int PyModule::load_subclass_of(const char *base_class, PyObject ** py_class)
{
    ...
    if (!mgr_module) {
        ...
        derr << handle_pyerror(true, module_name,
                               "PyModule::load_subclass_of") << dendl;
        return -EINVAL;
    }
