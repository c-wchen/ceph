                          ...
                          const std::string & run_name, bool no_verify)
{
    ...
    if (operation != OP_WRITE) {
        ...
        r = fetch_bench_metadata(run_name_meta, &prev_op_size,
                                 &prev_object_size, &num_objects, &prevPid);
        if (r < 0) {
            if (r == -ENOENT)
    ...
    if (OP_WRITE == operation && cleanup) {
        ...
        out(cout) << "Clean up completed and total clean up time :" << runtime
            << std::endl;
...
template < class T > static T vec_stddev(vector < T > &v)
{
    T mean = 0;
                            ...
                            unsigned max_objects)
{
    ...
    while (!secondsToRun || ceph_clock_now() < stopTime) {
        ...
        if (max_objects &&
            data.started >=
            (int)((data.object_size * max_objects + data.op_size -
                   1) / data.op_size))
            break;
    }
    lock.Unlock();
    ...
    double bandwidth;
    bandwidth =
        ((double)data.finished) * ((double)data.op_size) / (double)timePassed;
    ...
    if (!formatter) {
        ...
        formatter->dump_format("stddev_bandwidth", "%f",
                               vec_stddev(data.history.bandwidth));
        ...
        formatter->dump_format("average_iops", "%d",
                               (int)(data.finished / timePassed));
        formatter->dump_format("stddev_iops", "%d",
                               vec_stddev(data.history.iops));
        ...
        formatter->dump_format("stddev_latency", "%f",
                               vec_stddev(data.history.latency));
...
int ObjBencher::seq_read_bench(int seconds_to_run, int num_objects,
                               int concurrentios, int pid, bool no_verify)
{
    ...
    while (data.finished < data.started) {
        ...
        if (!no_verify) {
            ...
            if ((contents[slot]->length() != data.op_size) ||
                (memcmp
                 (data.object_contents, contents[slot]->c_str(),
                  data.op_size) != 0)) {
                cerr << name[slot] << " is not correct!" << std::endl;
                ++errors;
            }
    ...
    double bandwidth;
    bandwidth =
        ((double)data.finished) * ((double)data.op_size) / (double)runtime;
    bandwidth = bandwidth / (1024 * 1024);  // we want it in MB/sec
...
int ObjBencher::rand_read_bench(int seconds_to_run, int num_objects,
                                int concurrentios, int pid, bool no_verify)
{
    lock_cond lc(&lock);
