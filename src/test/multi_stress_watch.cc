...
void test_erasure(Rados & cluster, std::string pool_name, std::string obj_name)
{
    string outs;
    bufferlist inbl;
    int ret;
...
int main(int args, char **argv)
{
    ...
    std::
        cerr << "pool_name, obj_name are " << pool_name << ", " << obj_name <<
        std::endl;
    ...
    if (ret) {
        std::cerr << "Error " << ret << " in cluster.conf_read_file" << std::
            endl;
        return ret;
    }
    ret = cluster.conf_parse_env(NULL);
    if (ret) {
        std::cerr << "Error " << ret << " in cluster.conf_read_env" << std::
            endl;
        return ret;
    }
    ret = cluster.connect();
