...
bool MetricsHandler::ms_can_fast_dispatch2(const cref_t < Message > &m) const const
{
    return m->get_type() == CEPH_MSG_CLIENT_METRICS
...
void MetricsHandler::init()
{
    ...
    updater = std::thread([this] () {
                          std::unique_lock locker(lock); while (!stopping) {
                          double after =
                          g_conf().get_val < std::chrono::seconds >
                          ("mds_metrics_update_interval").count();
                          locker.unlock(); sleep(after); locker.lock();
                          update_rank0();}
                          }) ;
}
...
void MetricsHandler::add_session(Session * session)
{
    ...
    auto p =
        client_metrics_map.emplace(client,
...
void MetricsHandler::remove_session(Session * session)
{
    ...
    if (lus == last_updated_seq) {
        dout(10) << ": metric lus=" << lus << ", last_updated_seq=" <<
            last_updated_seq << dendl;
...
void MetricsHandler::reset_seq()
{
  ...
  for (auto &[client, metrics_v]:client_metrics_map) {
        dout(10) << ": reset last updated seq for client addr=" << client <<
            dendl;
        metrics_v.first = last_updated_seq;
    }
}

void MetricsHandler::handle_payload(Session * session,
                                    const CapInfoPayload & payload)
{
    dout(20) << ": type=" << payload.get_type()
        << ", session=" << session << ", hits=" << payload.
        cap_hits << ", misses=" << payload.cap_misses << dendl;
                                    ...
                                    const DentryLeasePayload & payload)
{
    dout(20) << ": type=" << payload.get_type()
        << ", session=" << session << ", hits=" << payload.
        dlease_hits << ", misses=" << payload.dlease_misses << dendl;
...
{
    dout(20) << ": type=" << payload.get_type()
        << ", session=" << session << ", opened_inodes=" << payload.
        opened_inodes << ", total_inodes=" << payload.total_inodes << dendl;
...
void MetricsHandler::handle_client_metrics(const cref_t < MClientMetrics > &m)
{
  ...
  for (auto & metric:m->updates) {
        boost::apply_visitor(HandlePayloadVisitor(this, session),
                             metric.payload);
    }
}
...
void MetricsHandler::notify_mdsmap(const MDSMap & mdsmap)
{
    ...
    dout(10) << ": rank0 is mds." << mdsmap.get_mds_info((mds_rank_t) 0).
        name << dendl;
...
void MetricsHandler::update_rank0()
{
    ...
    dout(20) << ": sending metric updates for " << update_client_metrics_map.
        size()
        << " clients to rank 0 (address: " << *addr_rank0 <<
        ") with sequence number " << next_seq <<
        ", last updated sequence number " << last_updated_seq << dendl;

    mds->send_message_mds(make_message < MMDSMetrics >
                          (std::move(metrics_message)), *addr_rank0);
}
