...
template class librbd::deep_copy::SnapshotCreateRequest <
...
public:
    ...
    } void expect_test_features(librbd::
    ...
    void expect_object_map_resize(librbd::
                                  MockTestImageCtx &mock_image_ctx,
                                  librados::snap_t snap_id, int r)
    {
        std::string oid(librbd::ObjectMap <>::
    ...
    MockSnapshotCreateRequest *create_request(librbd::
            MockTestImageCtx &
            ...
            uint64_t size,
            const cls::rbd::
            ParentImageSpec &spec,
            uint64_t parent_overlap,
            Context *on_finish)
    {
        return new MockSnapshotCreateRequest(&mock_local_image_ctx,
                                             snap_name, snap_namespace,
                                             size, spec, parent_overlap,
                                             on_finish);
    }
};
