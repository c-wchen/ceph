...
namespace ceph::async::detail {
    ...
    class SharedMutexImpl:public boost::intrusive_ref_counter <
        SharedMutexImpl > {
      public:
        ~SharedMutexImpl();
      ...
      private:
        ...
        static constexpr LockState Exclusive =
            std::numeric_limits < LockState >::max();
  ...
  template < typename Mutex, template < typename > typename Lock > class AsyncRequest:public LockRequest
    {
      ...
      explicit AsyncRequest(Mutex & mutex):mutex(mutex) {
        ...
        void complete(boost::system::error_code ec) override {
            ...
            post(std::unique_ptr < LockCompletion > {
                 r}, ec, ec ? Lock {
                 mutex, std::defer_lock} : Lock {
        ...
        auto SharedMutexImpl::async_lock(Mutex & mtx, CompletionToken && token)
    {
        ...
        auto ex1 = mtx.get_executor();
        {
            ...
            if (state == Unlocked) {
                ...
                auto request =
                    LockCompletion::create(ex1, std::move(handler), mtx);
    ...
    template < typename Mutex, typename CompletionToken >
        auto SharedMutexImpl::async_lock_shared(Mutex & mtx, CompletionToken
                                                && token) {
    ...
    void SharedMutexImpl::complete(RequestList && requests,
        ...
        while (!requests.empty()) {
            ...
            try {
                ...
                requests.clear_and_dispose([](LockRequest * r) {
                                           r->destroy();
                                           }
                );
                throw;
