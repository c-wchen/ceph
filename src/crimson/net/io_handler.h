 ...
 */
    class HandshakeListener {
      public:
        ...
        virtual void notify_out_fault(const char *where, std::exception_ptr) =
            0;
 ...
 */
   ...
   */
        ...
        void set_io_state(const io_state_t & new_state, FrameAssemblerV2Ref fa =
                          nullptr);
      ...
      private:
        ...
        bool is_out_queued() const {
            return (!out_pending_msgs.empty() ||
                    ack_left > 0 ||
                    need_keepalive || next_keepalive_ack.has_value());
        } void reset_out();
        ...
        ceph::bufferlist sweep_out_pending_msgs_to_sent(bool require_keepalive,
                                                        std::optional <
                                                        utime_t >
                                                        maybe_keepalive_ack,
                                                        bool require_ack);
    ...
    inline std::ostream & operator<<(std::ostream & out,
                                     IOHandler::io_stat_printer stat) {
...
template <>
    ...
    crimson::net::IOHandler::io_state_t >:fmt::formatter < std::string_view > {
    template < typename FormatContext >
        auto format(crimson::net::IOHandler::io_state_t state,
                    FormatContext & ctx) {
...
template <> struct fmt::formatter <
    crimson::net::IOHandler::io_stat_printer >:fmt::ostream_formatter {
};
#endif
