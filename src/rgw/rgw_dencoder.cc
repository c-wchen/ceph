...
static void init_bucket(rgw_bucket * b, const char *t, const char *n,
                        const char *dp, const char *ip, const char *m,
                        const char *id)
{
    b->tenant = t;
    b->name = n;
...
void RGWObjManifest::obj_iterator::seek(uint64_t o)
{
    ...
    part_ofs =
        rule.start_ofs + (cur_part_id - rule.start_part_num) * rule.part_size;
    ...
    if (!rule.part_size) {
        ...
        uint64_t next =
            MIN(stripe_ofs + rule.stripe_max_size, part_ofs + rule.part_size);
...
void RGWObjManifest::obj_iterator::update_location()
{
    ...
    manifest->get_implicit_location(cur_part_id, cur_stripe, ofs,
                                    &cur_override_prefix, &location);
}
...
void RGWAccessControlList::generate_test_instances(list <
                                                   RGWAccessControlList * >&o)
{
...
void RGWAccessControlPolicy::generate_test_instances(list <
