...
template < typename M >
static std::string get_canon_amz_hdrs(const M& map)
{
    size_t length = 0;
                                        ...
                                        std::string_view& sessiontoken)  /* out */
{
    ...
    if (now >= req_sec + exp) {
        dout(10) << "ERROR: presigned URL has expired, now = " << now << ", req_sec = " << req_sec << ", exp = " << exp <<
                 dendl;
...
std::pair < AWSv4ComplMulti::ChunkMeta,
size_t /* consumed */ > AWSv4ComplMulti::ChunkMeta::create_next(CephContext* const cct,
        ChunkMeta&& old,
...
std::string AWSv4ComplMulti::calc_v4_trailer_signature(const trailer_map_t &trailer_map,
