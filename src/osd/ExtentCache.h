...
struct bl_split_merge {
    bufferlist split(uint64_t offset, uint64_t length, bufferlist & bl) const {
        bufferlist out;
        ...
        bufferlist bl;
         ...
         return bl;
    } uint64_t length(const bufferlist & b)const {
        return b.length();
}};
    ...
    struct object_extent_set:boost::intrusive::set_base_hook <> {
        ...
        template < typename F >
            ...
            for (auto p = range.first; p != range.second;) {
                ...
                uint64_t extlen = MIN(ext->length - (extoff - ext->offset),
