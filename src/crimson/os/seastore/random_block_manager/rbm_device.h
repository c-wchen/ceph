...
using open_ertr = crimson::errorator <
                  crimson::ct_error::input_output_error,
                  crimson::ct_error::invarg, crimson::ct_error::enoent >;
...
public:
    ...
    read_ertr::future <> read(paddr_t addr,
                              size_t len, ceph::bufferptr &out) final
    {
...
public:
    ...
    } virtual read_ertr::future <> read(uint64_t offset,
                                        bufferptr &bptr) = 0;
     ...
     */
    virtual write_ertr::future <> write(uint64_t offset,
                                        bufferptr && bptr,
                                        uint16_t stream = 0) = 0;

    virtual discard_ertr::future <> discard(uint64_t offset, uint64_t len)
    {
        return seastar::now();
    }

    virtual open_ertr::future <> open(const std::string &path,
                                      seastar::open_flags mode) = 0;

    virtual write_ertr::future <> writev(uint64_t offset,
                                         ceph::bufferlist bl,
                                         uint16_t stream = 0) = 0;
...
public:
    ...
    std::size_t get_available_size()const final
    {
        return size;
    ...
    stat_device_ret stat_device() final
    {
        ...
        stat.size = size;
        return stat_device_ret(read_ertr::ready_future_marker {
        }
        , stat);
    }
