...
extern int rgw_user_get_all_buckets_stats(RGWRados * store,
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
extern int rgw_get_user_info_by_email(RGWRados * store, string & email,
                                      RGWUserInfo & info,
                                      RGWObjVersionTracker * objv_tracker =
                                      NULL, real_time * pmtime = NULL);
/**
 * Given an swift username, finds the user info associated with it.
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
extern int rgw_get_user_info_by_swift(RGWRados * store, const string & swift_name, RGWUserInfo & info,  /* out */
                                      RGWObjVersionTracker * objv_tracker =
                                      nullptr, real_time * pmtime = nullptr);
/**
...
class RGWAccessKeyPool {
  ...
  private:
    int create_key(RGWUserAdminOpState & op_state, std::string * err_msg =
                   NULL);
    int generate_key(RGWUserAdminOpState & op_state, std::string * err_msg =
                     NULL);
    ...
    int execute_remove(RGWUserAdminOpState & op_state, std::string * err_msg,
                       bool defer_save);
    int remove_subuser_keys(RGWUserAdminOpState & op_state,
                            std::string * err_msg, bool defer_save);

    int add(RGWUserAdminOpState & op_state, std::string * err_msg,
            bool defer_save);
...
class RGWSubUserPool {
  ...
  private:
    ...
    int remove(RGWUserAdminOpState & op_state, std::string * err_msg,
               bool defer_save);
    int modify(RGWUserAdminOpState & op_state, std::string * err_msg,
               bool defer_save);
  public:
...
class RGWUser {
  ...
  public:
    ...
    int info(RGWUserAdminOpState & op_state, RGWUserInfo & fetched_info,
