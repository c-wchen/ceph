        ...
        namespace disk_usage {
            ...
            static int do_disk_usage(librbd::RBD & rbd,
                ...
                if (f) {
                    ...
                    tbl.define_column("PROVISIONED", TextTable::RIGHT,
                                      TextTable::RIGHT);
                    tbl.define_column("USED", TextTable::RIGHT,
                                      TextTable::RIGHT);
                }
                ...
                for (std::vector < string >::const_iterator name =
                     names.begin(); name != names.end(); ++name) {
                    ...
                    for (std::vector <
                        ...
                        if (imgname == nullptr || found_from_snap ||
                            (found_from_snap && snapname != nullptr
                             && snap->name == snapname)) {
                            r = compute_image_disk_usage(*name, snap->name,
                                                         last_snap_name,
                                                         snap_image, snap->size,
                                                         tbl, f, &used_size);
                            if (r < 0) {
                                goto out;
                            }
                    ...
                    if (snapname == NULL) {
                        r = compute_image_disk_usage(*name, "", last_snap_name,
                                                     image, info.size, tbl, f,
                                                     &used_size);
                        if (r < 0) {
                            goto out;
                        }
