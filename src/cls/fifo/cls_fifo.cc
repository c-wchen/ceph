        ...
        void encode(ceph::buffer::list & bl) const const
{
    ...
    WRITE_CLASS_ENCODER(entry_header)
        ...
        std::string new_oid_prefix(std::string id,
        ...
        int read_part_header(cls_method_context_t hctx,
            ...
            try {
                ...
                CLS_ERR("ERROR: %s: failed decoding part header",
                        __PRETTY_FUNCTION__);
                return -EIO;
            }
        ...
        int write_part_header(cls_method_context_t hctx,
            ...
            if (bl.length() > CLS_FIFO_MAX_PART_HEADER_SIZE) {
                CLS_ERR("%s: cannot write part header, buffer exceeds max size",
                        __PRETTY_FUNCTION__);
                return -EIO;
            }
        ...
        int read_header(cls_method_context_t hctx,
            ...
            ceph::buffer::list bl;
            r = cls_cxx_read2(hctx, 0, size, &bl,
                              CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
            if (r < 0) {
                CLS_ERR("ERROR: %s: cls_cxx_read2() on obj returned %d",
                        __PRETTY_FUNCTION__, r);
            ...
            if (r == 0) {
                if (get_info) {
                    CLS_LOG(5,
                            "%s: Zero length object, likely probe, returning ENODATA",
                            __PRETTY_FUNCTION__);
                }
                else {
            ...
            try {
                ...
                CLS_ERR("ERROR: %s: failed decoding header",
                        __PRETTY_FUNCTION__);
                return -EIO;
            }
        ...
        int create_meta(cls_method_context_t hctx,
            ...
            try {
                ...
                CLS_ERR("ERROR: %s: failed to decode request: %s",
            ...
            if (r == 0) {
                CLS_LOG(5,
                        "%s: FIFO already exists, reading from disk and comparing.",
                        __PRETTY_FUNCTION__);
                ceph::buffer::list bl;
                r = cls_cxx_read2(hctx, 0, size, &bl,
                                  CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
            ...
            header.params.full_size_threshold =
                op.max_part_size - op.max_entry_size - part_entry_overhead;

            r = write_header(hctx, header);
            if (r < 0) {
                CLS_ERR("%s: failed to write header: r=%d", __PRETTY_FUNCTION__,
                        r);
                return r;
            }
        ...
        int update_meta(cls_method_context_t hctx, ceph::buffer::list * in,
            ...
            try {
                ...
                CLS_ERR("ERROR: %s: failed to decode request",
                        __PRETTY_FUNCTION__);
                return -EINVAL;
            }
            ...
            if (changed) {
                r = write_header(hctx, header);
                if (r < 0) {
                    CLS_ERR("%s: failed to write header: r=%d",
                            __PRETTY_FUNCTION__, r);
                    return r;
                }
            }
        ...
        int init_part(cls_method_context_t hctx, ceph::buffer::list * in,
            ...
            if (r == 0 && size > 0) {
                ...
                if (r < 0) {
                    CLS_ERR("%s: failed to read part header",
                            __PRETTY_FUNCTION__);
                    return r;
                }
        ...
        bool full_part(const part_header & part_header) {
            return (part_header.next_ofs >
                    part_header.params.full_size_threshold);
        }
        ...
        int push_part(cls_method_context_t hctx, ceph::buffer::list * in,
          ...
          for (auto & data:op.data_bufs) {
                ...
                bufferptr pre(reinterpret_cast < char *>(&pre_header),
                              sizeof(pre_header));
                auto entry_write_len =
                    pre.length() + entry_header_bl.length() + data.length();
                all_data.append(pre);
            ...
            if (entries_pushed == 0) {
                CLS_ERR("%s: pushed no entries? Can't happen!",
                        __PRETTY_FUNCTION__);
                return -EFAULT;
            }
        ...
        class EntryReader {
          ...
          public:
            ...
            int get_next_entry(ceph::buffer::list * pbl,
                               std::uint64_t * pofs, ceph::real_time * pmtime);
        };

        int EntryReader::fetch(std::uint64_t num_bytes) {
            CLS_LOG(5, "%s: fetch %d bytes, ofs=%d data.length()=%d",
                    __PRETTY_FUNCTION__, (int)num_bytes, (int)ofs,
                    (int)data.length());
            if (data.length() < num_bytes) {
                ceph::buffer::list bl;
                CLS_LOG(5, "%s: reading % " PRId64 " bytes at ofs=%" PRId64,
                        __PRETTY_FUNCTION__, prefetch_len, ofs + data.length());
                int r =
                    cls_cxx_read2(hctx, ofs + data.length(), prefetch_len, &bl,
                                  CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
                if (r < 0) {
                    CLS_ERR("ERROR: %s: cls_cxx_read2() on obj returned %d",
                            __PRETTY_FUNCTION__, r);
            ...
            if (static_cast < unsigned >(num_bytes) > data.length()) {
                CLS_ERR("%s: requested %" PRId64 " bytes, but only "
                        "%u were available", __PRETTY_FUNCTION__, num_bytes,
                        data.length());
                return -ERANGE;
            }
        ...
        int EntryReader::seek(std::uint64_t num_bytes) {
            ...
            CLS_LOG(5, "%s:%d: num_bytes=%" PRIu64, __PRETTY_FUNCTION__,
                    __LINE__, num_bytes);
        ...
        int EntryReader::peek_pre_header(entry_header_pre * pre_header) {
            ...
            if (r < 0) {
                CLS_ERR("ERROR: %s: peek() size=%zu failed: r=%d",
                        __PRETTY_FUNCTION__, sizeof(pre_header), r);
                return r;
            }

            if (pre_header->magic != part_header.magic) {
                CLS_ERR("ERROR: %s: unexpected pre_header magic",
                        __PRETTY_FUNCTION__);
                return -ERANGE;
            }
        ...
        int EntryReader::get_next_entry(ceph::buffer::list * pbl,
            ...
            if (r < 0) {
                CLS_ERR("ERROR: %s: peek_pre_header() failed: r=%d",
                        __PRETTY_FUNCTION__, r);
                return r;
            }
            ...
            CLS_LOG(5, "%s:%d: pre_header.pre_size=%" PRIu64,
                    __PRETTY_FUNCTION__, __LINE__,
                    uint64_t(pre_header.pre_size));
            r = seek(pre_header.pre_size);
            if (r < 0) {
                CLS_ERR("ERROR: %s: failed to seek: r=%d", __PRETTY_FUNCTION__,
                        r);
                return r;
            }

            ceph::buffer::list header;
            CLS_LOG(5, "%s:%d: pre_header.header_size=%d", __PRETTY_FUNCTION__,
                    __LINE__, (int)pre_header.header_size);
            r = read(pre_header.header_size, &header);
            if (r < 0) {
                CLS_ERR("ERROR: %s: failed to read entry header: r=%d",
                        __PRETTY_FUNCTION__, r);
                return r;
            }
            ...
            try {
                ...
                CLS_ERR("%s: failed decoding entry header",
                        __PRETTY_FUNCTION__);
                return -EIO;
            }
            ...
            if (pbl) {
                ...
                if (r < 0) {
                    CLS_ERR("%s: failed reading data: r=%d",
                            __PRETTY_FUNCTION__, r);
                    return r;
                }
            }
                ...
                if (r < 0) {
                    CLS_ERR("ERROR: %s: failed to seek: r=%d",
                            __PRETTY_FUNCTION__, r);
                    return r;
                }
            }
        ...
        int trim_part(cls_method_context_t hctx,
            ...
            if (op.ofs >= part_header.next_ofs) {
                if (full_part(part_header)) {
                    ...
                    if (r < 0) {
                        CLS_ERR("%s: ERROR: cls_cxx_remove() returned r=%d",
                                __PRETTY_FUNCTION__, r);
                        return r;
                    }
...
CLS_INIT(fifo)
{
    ...
    cls_register_cxx_method(h_class, op::GET_META,
                            CLS_METHOD_RD, get_meta, &h_get_meta);
    ...
    cls_register_cxx_method(h_class, op::LIST_PART,
                            CLS_METHOD_RD, list_part, &h_list_part);

    cls_register_cxx_method(h_class, op::GET_PART_INFO,
                            CLS_METHOD_RD, get_part_info, &h_get_part_info);
