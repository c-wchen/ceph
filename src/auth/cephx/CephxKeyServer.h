...
struct KeyServerData {
    ...
    EntityAuth >::iterator find_name(const EntityName &name)
    {
        return secrets.find(name);
    }
    std::map < EntityName,
    EntityAuth >::const_iterator find_name(const EntityName &name)const
    {
        return secrets.find(name);
    }
    // -- incremental updates -- typedef enum {
    AUTH_INC_NOP,
    AUTH_INC_ADD,
    AUTH_INC_DEL,
...
public:
    ...
    bool get_auth(const EntityName &name, EntityAuth &auth) const;
    bool get_caps(const EntityName &name, const std::string &type,
                  AuthCapsInfo &caps) const const;
    bool get_active_rotating_secret(const EntityName &name,
    ...
    bool get_rotating_encrypted(const EntityName &name,
                                ceph::buffer::list &enc_bl) const;

    ceph::mutex &get_lock()const
    {
        return lock;
