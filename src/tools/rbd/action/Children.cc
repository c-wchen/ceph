...
namespace rbd {
    namespace action {
        namespace children {
            ...
            int do_list_children(librados::IoCtx & io_ctx,
                                 librbd::Image & image, bool all_flag,
                                 bool descendants_flag, Formatter * f) {
            ...
            void get_arguments(po::options_description * positional,
                ...
                options->add_options()
                    ("all,a", po::bool_switch(),
                     "list all children (include trash)");
                options->add_options()
                    ("descendants", po::bool_switch(),
            ...
            int execute(const po::variables_map & vm,
                ...
                if (r == -ENOENT) {
                    ...
                    std::
                        cerr << "rbd: error setting snapshot: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                ...
                r = do_list_children(io_ctx, image, vm["all"].as < bool > (),
                                     vm["descendants"].as < bool > (),
                                     formatter.get());
                if (r < 0) {
                    std::
                        cerr << "rbd: listing children failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
