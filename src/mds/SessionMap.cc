    ...
    class SessionMapIOContext:public MDSIOContextBase {
      protected:
        ...
        MDSRank *get_mds() override {
         ...
         explicit SessionMapIOContext(SessionMap *
                                      sessionmap_):sessionmap(sessionmap_) {
...
{
    dout(10) << "dump" << dendl;
    for (ceph::unordered_map < entity_name_t, Session * >::iterator p =
         session_map.begin(); p != session_map.end(); ++p)
        dout(10) << p->first << " " << p->second << " state " << p->second->
            get_state_name()
            << " completed " << p->second->info.
            completed_requests << " prealloc_inos " << p->second->info.
            prealloc_inos << " used_inos " << p->second->info.
            used_inos << dendl;
}
...
object_t SessionMap::get_object_name() const const
{
    char s[30];
    ...
    class C_IO_SM_Load:public SessionMapIOContext {
      public:
        ...
        :SessionMapIOContext(cm), first(f), header_r(0), values_r(0) {
            ...
            sessionmap->_load_finish(r, header_r, values_r, first, header_bl,
                                     session_vals, more_session_vals);
        }
 ...
 */
void SessionMapStore::decode_values(std::map < std::string,
                                    bufferlist > &session_vals)
{
    for (std::map < std::string, bufferlist >::iterator i =
         session_vals.begin(); i != session_vals.end(); ++i) {

        entity_inst_t inst;

        bool parsed = inst.name.parse(i->first);
        if (!parsed) {
            derr << "Corrupt entity name '" << i->
                first << "' in sessionmap" << dendl;
 ...
 */
void SessionMap::_load_finish(int operation_r,
                              int header_r,
                              int values_r,
                              bool first,
                              bufferlist & header_bl,
                              ...
                              bool more_session_vals)
{
    if (operation_r < 0) {
        ...
        mds->clog->error() << "error reading sessionmap '" << get_object_name()
            << "' " << operation_r << " (" << cpp_strerror(operation_r) << ")";
        mds->damaged();
    ...
    if (first) {
        if (header_r != 0) {
            derr << __func__ << ": header error: " << cpp_strerror(header_r) <<
                dendl;
            mds->clog->
                error() << "error reading sessionmap header " << header_r <<
        ...
        if (header_bl.length() == 0) {
            dout(4) << __func__ << ": header missing, loading legacy..." <<
                dendl;
            load_legacy();
            return;
        }
    ...
    if (more_session_vals) {
        ...
        for (ceph::unordered_map < entity_name_t, Session * >::iterator i =
             session_map.begin(); i != session_map.end(); ++i) {
            Session *s = i->second;
...
namespace {
    class C_IO_SM_Save:public SessionMapIOContext {
        version_t version;
      public:
        C_IO_SM_Save(SessionMap * cm, version_t v):SessionMapIOContext(cm),
            version(v) {
        } void finish(int r) override {
            if (r != 0) {
...
void SessionMap::save(MDSInternalContextBase * onsave, version_t needv)
{
    ...
    for (std::set < entity_name_t >::iterator i = dirty_sessions.begin();
        ...
        if (session->is_open() ||
            session->is_closing() ||
            session->is_stale() || session->is_killing()) {
            dout(20) << "  " << name << dendl;
            // Serialize K
            std::ostringstream k;
            k << name;
    ...
    if (pre == (uint64_t) - 1) {
        ...
        while (n-- && !p.end()) {
            ...
            if (session_map.count(s->info.inst.name)) {
                ...
                dout(10) << " already had session for " << s->info.inst.
                    name << ", recovering" << dendl;
...
void SessionMapStore::dump(Formatter * f) const const
{
...
{
    dout(10) << __func__ << " s=" << s << " name=" << s->info.inst.
        name << dendl;
    ...
    if (by_state_entry == by_state.end())
        by_state_entry =
            by_state.emplace(s->state, new xlist < Session * >).first;
    by_state_entry->second->push_back(&s->item_session_list);
    s->get();
...
{
    dout(10) << __func__ << " s=" << session << " name=" << session->info.inst.
        name << dendl;
...
size_t Session::get_request_count()
{
    ...
    elist < MDRequestImpl * >::iterator p =
        requests.begin(member_offset(MDRequestImpl, item_session_request));
    while (!p.end()) {
        ++result;
        ++p;
    }
                          ...
                          int new_uid, int new_gid)
{
    ...
    if (diri && diri->is_stray()) {
        path =
            std::string(boost::
                        string_view(in->get_projected_inode()->
                                    stray_prior_path));
    ...
    if (!auth_caps.
        is_capable(path, in->inode.uid, in->inode.gid, in->inode.mode,
                   caller_uid, caller_gid, caller_gid_list, mask, new_uid,
                   new_gid)) {
        return -EACCES;
    }
    return 0;
...
void SessionMap::hit_session(Session * session)
{
    ...
    double total_load =
        total_load_avg.hit(ceph_clock_now(), total_load_avg_rate);
                                    ...
                                    const std::set < std::string > &changed)
{
    ...
    if (changed.count("mds_recall_max_decay_rate")) {
        ...
        if (it != by_state.end()) {
          for (const auto & session:*(it->second)) {
                session->recall_caps_throttle =
                    DecayCounter(ceph_clock_now(), d);
            }
        }
        it = by_state.find(Session::STATE_STALE);
        if (it != by_state.end()) {
...
int SessionFilter::parse(const std::vector < std::string > &args,
                         std::stringstream * ss)
{
    assert(ss != NULL);
  ...
  for (const auto & s:args) {
        ...
        dout(20) << __func__ << " parsed k='" << k << "', v='" << v << "'" <<
            dendl;
...
bool SessionFilter::match(const Session & session,
                          std::function < bool(client_t) >
                          is_reconnecting) constconst
{
  for (const auto & m:metadata) {
    ...
    if (reconnecting.first) {
        const bool am_reconnecting =
            is_reconnecting(session.info.inst.name.num());
...
std::ostream & operator<<(std::ostream & out, const Session & s)
{
    if (s.get_human_name() == stringify(s.info.inst.name.num())) {
        ...
        out << s.get_human_name() << " (" << std::dec << s.info.inst.name.
            num() << ")";
    }
    return out;
}
