            ...
            for (tables_it = ttables_it->second.begin();
                ...
                for (tables_it_ = tables_it->second.begin();
                    ...
                    for (tables_it__ = tables_it_->second.begin();
                         tables_it__ != tables_it_->second.end();
                         ++tables_it__) {
                        for (table_it = tables_it__->second.begin();
                             table_it != tables_it__->second.end();
                             ++table_it) {
                            if (table_it->second) {
                                if (*(table_it->second)) {
...
int **ErasureCodeShecTableCache::getEncodingTable(int technique, int k, int m,
                                                  int c, int w)
{
    Mutex::Locker lock(codec_tables_guard);
...
int **ErasureCodeShecTableCache::getEncodingTableNoLock(int technique, int k,
                                                        int m, int c, int w)
{
...
int *ErasureCodeShecTableCache::setEncodingTable(int technique, int k, int m,
                                                 int c, int w, int *ec_in_table)
{
    Mutex::Locker lock(codec_tables_guard);
...
Mutex *ErasureCodeShecTableCache::getLock()
{
    return &codec_tables_guard;
}

uint64_t
    ErasureCodeShecTableCache::getDecodingCacheSignature(int k, int m, int c,
                                                         int w, int *erased,
                                                         int *avails)
{
...
bool ErasureCodeShecTableCache::getDecodingTableFromCache(int *decoding_matrix,
                                                          int *dm_row,
                                                          int *dm_column,
                                                          int *minimum,
                                                          int technique,
                                                          ...
                                                          int *avails)
{
    ...
    memcpy(dm_column,
           decode_tbls_map_it->second.second.dm_column, k * sizeof(int));
    memcpy(minimum,
           decode_tbls_map_it->second.second.minimum, (k + m) * sizeof(int));

    // find item in LRU queue and push back
    decode_tbls_lru->splice(decode_tbls_lru->end(),
                            *decode_tbls_lru, decode_tbls_map_it->second.first);
    return true;
}

void ErasureCodeShecTableCache::putDecodingTableToCache(int *decoding_matrix,
                                                        int *dm_row,
                                                        int *dm_column,
                                                        int *minimum,
                                                        int technique,
                                                        ...
                                                        int *avails)
{
...

    {
        ...
        memcpy(map_value.second.decoding_matrix,
               decoding_matrix, k * k * sizeof(int));
