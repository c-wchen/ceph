...
public:
    Watch(private_ctag_t,
        ...
        : obc(std::move(obc)),
          winfo(winfo),
          entity_name(entity_name), pg(std::move(pg)), timeout_timer([this]
    {
        return
            do_watch_timeout
            ();
...
class Notify: public seastar::enable_shared_from_this < Notify >
{
    ...
    template < class WatchIteratorT > Notify(WatchIteratorT begin,
            WatchIteratorT end,
            crimson::net::ConnectionRef conn,
            const notify_info_t &ninfo,
            const uint64_t client_gid, const uint64_t user_version);
...
public:
    template < class ... Args > Notify(private_ctag_t, Args && ... args): Notify(std::forward < Args >
                (args) ...)
    {
    }

    template < class WatchIteratorT, class ... Args >
    static seastar::future <> create_n_propagate(WatchIteratorT begin,
            WatchIteratorT end,
            Args && ... args);
...
template < class WatchIteratorT > Notify::Notify(WatchIteratorT begin,
        WatchIteratorT end,
        crimson::net::ConnectionRef conn,
        const notify_info_t &ninfo,
        const uint64_t client_gid, const uint64_t user_version)
    : watchers(begin, end),
      ninfo(ninfo),
      conn(std::move(conn)),
      client_gid(client_gid), user_version(user_version)
{
...
template < class WatchIteratorT, class ... Args >
seastar::future <> Notify::create_n_propagate(WatchIteratorT begin,
        WatchIteratorT end,
        Args && ... args)
{
    static_assert(std::is_same_v < typename std::iterator_traits <
                  WatchIteratorT >::value_type, crimson::osd::WatchRef >);
    if (begin == end) {
        auto notify = seastar::make_shared < Notify > (private_ctag_t { }
                      ,
                      std::forward < Args >
                      (args) ...);
...
template <> struct fmt::formatter <
    crimson::osd::notify_reply_t > : fmt::ostream_formatter {
};
#endif
