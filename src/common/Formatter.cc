...
namespace ceph {
    ...
    std::string fixed_u_to_string(uint64_t num, int scale) {
        ...
        t << num;
        int len = t.str().size();
         return t.str().substr(0,
                               len - scale) + "." + t.str().substr(len - scale);
...
// -----------------------
  ...
  :    
    ...
    void XMLFormatter::open_object_section_with_attrs(std::string_view name,
    ...
    void XMLFormatter::open_array_section_with_attrs(std::string_view name,
                                                     const FormatterAttrs &
                                                     attrs) {
        open_section_in_ns(name, NULL, &attrs);
    }
    ...
    void XMLFormatter::dump_format_va(std::string_view name, const char *ns,
        ...
        if (ns) {
            m_ss << "<" << e << " xmlns=\"" << ns << "\">" <<
                xml_stream_escaper(std::
                                   string_view(buf, len)) << "</" << e << ">";
        }
        else {
            m_ss << "<" << e << ">" << xml_stream_escaper(std::
                                                          string_view(buf,
                                                                      len)) <<
                "</" << e << ">";
        }
  ...
  TableFormatter::TableFormatter(bool keyval):m_keyval(keyval) {
    ...
    void TableFormatter::open_object_section_with_attrs(std::string_view name,
                                                        const FormatterAttrs &
                                                        attrs) {
        open_section_in_ns(name, NULL, NULL);
    }
    ...
    void TableFormatter::open_array_section_with_attrs(std::string_view name,
                                                       const FormatterAttrs &
                                                       attrs) {
        open_section_in_ns(name, NULL, NULL);
    }
    ...
    void TableFormatter::open_section_in_ns(std::string_view name,
                                            const char *ns,
                                            const FormatterAttrs * attrs) {
