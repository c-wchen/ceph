    ...
    : m_image_ctx(image_ctx),
      m_lock(ceph::
             make_shared_mutex(util::
                               unique_lock_name
                               ("librbd::exclusve_lock::ImageDispatch::m_lock",
                                this)))
{
} template < typename I > void ImageDispatch <
...
template < typename I >
void ImageDispatch <
I >::unset_require_lock(io::Direction direction)
{
    set_require_lock(direction, false);
}

template < typename I >
bool ImageDispatch < I >::set_require_lock(io::Direction direction,
        bool enabled)
{
    auto cct = m_image_ctx->cct;
    ldout(cct,
          20) << "direction=" << direction << ", enabled=" << enabled <<
              dendl;

    std::unique_lock locker {
        m_lock};
    auto prev_require_lock = (m_require_lock_on_read
    ...
    bool require_lock = (m_require_lock_on_read
                         || m_require_lock_on_write);
                                ...
                                Context *on_dispatched)
{
    ...
    if (needs_exclusive_lock
        (false, tid, dispatch_result, on_dispatched)) {
        return true;
    }
...
template < typename I >
bool ImageDispatch <
I >::compare_and_write(io::AioCompletion *aio_comp, io::Extents
                       && image_extents, bufferlist
                       && cmp_bl, bufferlist
        ...
        on_dispatched)
{
    ...
    if (lock_required) {
        ...
        std::unique_lock locker {
            m_lock};
        bool retesting_lock = (!m_on_dispatches.empty()
                               && m_on_dispatches.front() ==
                               on_dispatched);
...
void ImageDispatch < I >::handle_acquire_lock(int r)
{
    ...
    if (r == -ERESTART) {
        ldout(cct,
              5) << "IO raced with exclusive lock shutdown" << dendl;
    } else if (r < 0) {
