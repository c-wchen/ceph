...
namespace librbd {
    namespace operation {
             ...
             template < typename I >
                ...
                case SnapshotUnprotectRequest < I >::STATE_UNPROTECT_SNAP_START:
                    ...
                    case SnapshotUnprotectRequest <
                        I >::
                        STATE_SCAN_POOL_CHILDREN:os << "SCAN_POOL_CHILDREN";
                    break;
                    case SnapshotUnprotectRequest <
                        I >::
                        STATE_UNPROTECT_SNAP_FINISH:os <<
                        "UNPROTECT_SNAP_FINISH";
                    break;
                    case SnapshotUnprotectRequest <
                        I >::
                        STATE_UNPROTECT_SNAP_ROLLBACK:os <<
                        "UNPROTECT_SNAP_ROLLBACK";
                    break;
                    default:os << "UNKNOWN (" << static_cast < uint32_t >
                        (state) << ")";
                    break;
                } return os;
            } template < typename I >
              ...
              public:
                C_ScanPoolChildren(AsyncObjectThrottle < I > &throttle,
                                   I * image_ctx, const ParentSpec & pspec,
                                   const Pools & pools, size_t pool_idx)
              :    C_AsyncObjectThrottle < I > (throttle, *image_ctx),
                    m_pspec(pspec), m_pool(pools[pool_idx]) {
                }
                ...
                int send() override {
                    ...
                    if (r == -ENOENT) {
                        ...
                        lderr(cct) << "can't create ioctx for pool '" << m_pool.
                            second << "'" << dendl;
                        return r;
                    }
                    ...
                    r = m_pool_ioctx.aio_operate(RBD_CHILDREN, rados_completion,
                                                 &op, &m_children_bl);
                    assert(r == 0);
              ...
              protected:
                void finish(int r) override {
                    ...
                    if (r == -ENOENT) {
                        ...
                        lderr(cct) << "cannot get children for pool '" <<
                            m_pool.second << "'" << dendl;
                    }
                    else {
                        lderr(cct) << "cannot unprotect: at least " <<
                            m_children.
                            size() << " " << "child(ren) [" <<
                            joinify(m_children.begin(), m_children.end(),
                                    std::
                                    string(",")) << "] " << "in pool '" <<
                            m_pool.second << "'" << dendl;
                        r = -EBUSY;
        ...
        template < typename I >
            SnapshotUnprotectRequest <
            I >::SnapshotUnprotectRequest(I & image_ctx, Context * on_finish,
                                          const cls::rbd::
        ...
        template < typename I >
            ...
            ParentSpec pspec(image_ctx.md_ctx.get_id(), image_ctx.id,
                             m_snap_id);
            ...
            AsyncObjectThrottle < I > *throttle =
                new AsyncObjectThrottle < I > (nullptr, image_ctx,
                                               context_factory, ctx, NULL, 0,
                                               pools.size());
        ...
        template < typename I >
            ...
            I >::send_unprotect_snap_rollback() {
            I & image_ctx = this->m_image_ctx;
            assert(image_ctx.owner_lock.is_locked());
        ...
        template < typename I >
            int SnapshotUnprotectRequest <
            I >::verify_and_send_unprotect_snap_start() {
            I & image_ctx = this->m_image_ctx;
