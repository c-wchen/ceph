...
public:
    ...
    struct realm {
        ...
        std::map < dirfrag_t, std::map < std::pair < std::string, snapid_t >,
            uint32_t > >dentries;
        ...
        void merge(const realm &o)
        {
            ...
            for (const auto &p : o.dentries) {
                auto em =
                    dentries.emplace(std::piecewise_construct,
...
public:
    ...
    void add_dentry(dirfrag_t r, dirfrag_t df, std::string_view dn,
                    snapid_t last, unsigned nonce)
    {
        realms[r].dentries[df][std::pair < std::string, snapid_t > (dn, last)] =
            nonce;
    }
    ...
    {
        auto em =
            realms.emplace(std::piecewise_construct, std::forward_as_tuple(df),
