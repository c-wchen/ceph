...
namespace STS {
    ...
    void Credentials::dump(Formatter * f) const {
        ...
        encode_json("SessionToken", sessionToken, f);
                                           ...
                                           const boost::optional < std::string >
                                           &role_session,
                                           const boost::optional < std::vector <
                                           std::string >> &token_claims,
                                           const boost::optional < std::vector <
                                           std::pair < std::string,
                                           std::string >>> &session_princ_tags,
                                           boost::optional < rgw_user > user,
                                           rgw::auth::Identity * identity) {
        uuid_d accessKey, secretKey;
        char accessKeyId_str[MAX_ACCESS_KEY_LEN],
            secretAccessKey_str[MAX_SECRET_KEY_LEN];

        //AccessKeyId
         gen_rand_alphanumeric_plain(cct, accessKeyId_str,
        ...
        if (ret = cryptohandler->validate_secret(secret); ret < 0) {
            ldpp_dout(dpp,
                      0) <<
                "ERROR: Invalid rgw sts key, please ensure its length is 16" <<
                dendl;
            return ret;
        }
        string error;
        ...
        if (ret = keyhandler->encrypt(input, enc_output, &error); ret < 0) {
            ldpp_dout(dpp,
                      0) <<
                "ERROR: Encrypting session token returned an error !" << dendl;
            return ret;
        }
  ...
  :    
    cct(cct), iamPolicy(iamPolicy), roleArn(roleArn),
    roleSessionName(roleSessionName) {
        ...
        if (duration.empty()) {
            ...
            this->duration =
                strict_strtoll(duration.c_str(), 10, &this->err_msg);
        }
    }
    ...
    int AssumeRoleRequestBase::validate_input(const DoutPrefixProvider * dpp) const {
        ...
        if (!iamPolicy.empty() &&
            (iamPolicy.size() < MIN_POLICY_SIZE
        ...
        if (!roleArn.empty() &&
            (roleArn.size() < MIN_ROLE_ARN_SIZE
        ...
        if (!roleSessionName.empty()) {
            if (roleSessionName.size() < MIN_ROLE_SESSION_SIZE
                ...
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: Either role session name is empty or role session size is incorrect: "
            ...
            if (!std::regex_match(roleSessionName, regex_roleSession)) {
                ldpp_dout(dpp,
                          0) << "ERROR: Role session name is incorrect: " <<
    ...
    int AssumeRoleWithWebIdentityRequest::
        validate_input(const DoutPrefixProvider * dpp) const {
        if (!providerId.empty()) {
            if (providerId.length() < MIN_PROVIDER_ID_LEN ||
                providerId.length() > MAX_PROVIDER_ID_LEN) {
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: Either provider id is empty or provider id length is incorrect: "
    ...
    int AssumeRoleRequest::validate_input(const DoutPrefixProvider * dpp) const {
        if (!externalId.empty()) {
            if (externalId.length() < MIN_EXTERNAL_ID_LEN ||
                ...
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: Either external id is empty or external id length is incorrect: "
        ...
        if (!serialNumber.empty()) {
            if (serialNumber.size() < MIN_SERIAL_NUMBER_SIZE
                ...
                ldpp_dout(dpp,
                          0) <<
                    "Either serial number is empty or serial number length is incorrect: "
    ...
    std::tuple < int,
        rgw::sal::RGWRole * >STSService::getRoleInfo(const DoutPrefixProvider *
                                                     dpp, const string & arn,
                                                     optional_yield y) {
        if (auto r_arn = rgw::ARN::parse(arn); r_arn) {
            ...
            std::unique_ptr < rgw::sal::RGWRole > role =
                driver->get_role(roleName, r_arn->account);
            if (int ret = role->get(dpp, y); ret < 0) {
                if (ret == -ENOENT) {
                    ldpp_dout(dpp,
                              0) << "Role doesn't exist: " << roleName << dendl;
                ...
                if (path_pos == pos) {
                    ...
                    path =
                        r_arn->resource.substr(path_pos,
                                               ((pos - path_pos) + 1));
                }
                ...
                if (path != r_path) {
                    ldpp_dout(dpp,
                              0) <<
                        "Invalid Role ARN: Path in ARN does not match with the role path: "
                        << path << " " << r_path << dendl;
    ...
    AssumeRoleWithWebIdentityResponse STSService::
        assumeRoleWithWebIdentity(const DoutPrefixProvider * dpp,
        ...
        response.assumeRoleResp.retCode =
            response.assumeRoleResp.user.generateAssumedRoleUser(cct, driver,
                                                                 roleId,
                                                                 r_arn.get(),
                                                                 req.
                                                                 getRoleSessionName
        ...
        response.assumeRoleResp.retCode =
            response.assumeRoleResp.creds.generateCredentials(dpp, cct,
                                                              req.getDuration(),
                                                              req.getPolicy(),
                                                              roleId,
                                                              req.
    ...
    AssumeRoleResponse STSService::assumeRole(const DoutPrefixProvider * dpp,
        ...
        //Generate Assumed Role User
        response.retCode =
            response.user.generateAssumedRoleUser(cct, driver, roleId,
                                                  r_arn.get(),
        ...
        //Role and Policy provide the authorization info, user id and applier info are not needed
        response.retCode =
            response.creds.generateCredentials(dpp, cct, req.getDuration(),
    ...
    GetSessionTokenRequest::GetSessionTokenRequest(const string & duration,
    ...
    GetSessionTokenResponse STSService::
        getSessionToken(const DoutPrefixProvider * dpp,
