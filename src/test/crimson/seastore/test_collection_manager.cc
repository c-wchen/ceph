...
struct collection_manager_test_t:public seastar_test_suite_t, TMTestState {
    ...
    collection_manager_test_t() {
        ...
        return tm_setup().then([this] {
                               collection_manager =
                               collection_manager::create_coll_manager(*tm);
                               return seastar::now();}
        );
    }
    ...
    auto get_root() {
        ...
        auto coll_root = with_trans_intr(*tref,
                                         [this] (auto & t) {
    ...
    void checking_mappings(coll_root_t & coll_root, Transaction & t) {
      ...
      for (std::pair < coll_t, coll_info_t > p:test_coll_mappings) {
            EXPECT_NE(std::find(coll_list.begin(), coll_list.end(), p),
                      coll_list.end());
        }
    }
...
TEST_F(collection_manager_test_t, basic)
{
    run_async([this] {
              ...
              auto t = create_mutate_transaction(); for (int i = 0; i < 20; i++) {
              ...
              replay(); checking_mappings(coll_root); {
...
TEST_F(collection_manager_test_t, update)
{
    run_async([this] {
              coll_root_t coll_root = get_root(); {
              auto t = create_mutate_transaction(); for (int i = 0; i < 2; i++) {
