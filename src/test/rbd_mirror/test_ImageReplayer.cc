...
namespace rbd {
    namespace mirror {
        ...
        template < typename T > class TestImageReplayer:public TestFixture {
          public:
            ...
            struct C_WatchCtx:public librados::WatchCtx2 {
                ...
                :test(test), oid(oid), notified(false) {
                    ...
                    bufferlist bl;
                     test->m_remote_ioctx.notify_ack(oid, notify_id, cookie,
                                                     bl);
          ...
          :    
            m_local_cluster(new librados::Rados()), m_watch_handle(0)
            {
                ...
                EXPECT_EQ(0,
                          m_local_cluster->
                          conf_set("rbd_mirror_journal_commit_age", "0.1"));
                m_local_pool_name = get_temp_pool_name();
                EXPECT_EQ(0,
                          m_local_cluster->pool_create(m_local_pool_name.
                                                       c_str()));
                EXPECT_EQ(0,
                          m_local_cluster->ioctx_create(m_local_pool_name.
                                                        c_str(),
                                                        m_local_ioctx));
                ...
                EXPECT_EQ(0,
                          m_remote_cluster.pool_create(m_remote_pool_name.
                                                       c_str()));
                m_remote_pool_id =
                ...
                EXPECT_EQ(0,
                          m_remote_cluster.ioctx_create(m_remote_pool_name.
                                                        c_str(),
                                                        m_remote_ioctx));
                ...
                if (MIRROR_IMAGE_MODE == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
                    EXPECT_EQ(0,
                              librbd::api::Mirror <>::mode_set(m_remote_ioctx,
                                                               RBD_MIRROR_MODE_POOL));
                    EXPECT_EQ(0,
                              librbd::api::Mirror <>::mode_set(m_local_ioctx,
                                                               RBD_MIRROR_MODE_POOL));
                }
                else {
                    EXPECT_EQ(0,
                    ...
                    EXPECT_EQ(0,
                              librbd::cls_client::
                              mirror_peer_add(&m_remote_ioctx, {
                                              remote_peer_uuid,
                                              cls::rbd::
                    ...
                    m_pool_meta_cache.set_remote_pool_meta(m_remote_ioctx.
                                                           get_id(), {
                ...
                int order = 0;
                EXPECT_EQ(0,
                          librbd::create(m_remote_ioctx, m_image_name.c_str(),
                                         1 << 22, false, features, &order, 0,
                                         0));
                ...
                if (MIRROR_IMAGE_MODE != cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
                    ...
                    EXPECT_EQ(0,
                              librbd::api::Mirror <>::
                              image_enable(remote_image_ctx,
                                           static_cast <
                                           rbd_mirror_image_mode_t >
                                           (MIRROR_IMAGE_MODE), false));
                    close_image(remote_image_ctx);
                }

                m_remote_image_id = get_image_id(m_remote_ioctx, m_image_name);
                m_global_image_id =
                    get_global_image_id(m_remote_ioctx, m_remote_image_id);

                auto cct =
                    reinterpret_cast < CephContext * >(m_local_ioctx.cct());
                ...
                m_instance_watcher =
                    InstanceWatcher <>::create(m_local_ioctx,
                                               *m_threads->asio_engine, nullptr,
                ...
                m_local_status_updater =
                    MirrorStatusUpdater <>::create(m_local_ioctx,
                                                   m_threads.get(), "");
            ...
            ~TestImageReplayer()override {
                ...
                EXPECT_EQ(0,
                          m_remote_cluster.pool_delete(m_remote_pool_name.
                                                       c_str()));
                EXPECT_EQ(0,
                          m_local_cluster->pool_delete(m_local_pool_name.
                                                       c_str()));
            }

            void create_replayer() {
                m_replayer =
                    new ImageReplayer <> (m_local_ioctx, m_local_mirror_uuid,
                                          m_global_image_id, m_threads.get(),
                                          m_instance_watcher,
                                          m_local_status_updater, nullptr,
                                          &m_pool_meta_cache);
                m_replayer->add_peer( {
                                     "peer uuid", m_remote_ioctx, {
                                     m_remote_mirror_uuid,
                                     "remote mirror peer uuid"}, nullptr});
            }
            ...
            void create_watch_ctx() {
                ...
                ASSERT_EQ(0,
                          m_remote_ioctx.watch2(oid, &m_watch_handle,
                                                m_watch_ctx));
            }
            ...
            std::string get_image_id(librados::IoCtx & ioctx,
            ...
            std::string get_global_image_id(librados::IoCtx & io_ctx,
                ...
                EXPECT_EQ(0,
                          librbd::cls_client::mirror_image_get(&io_ctx,
                                                               image_id,
                                                               &mirror_image));
                return mirror_image.global_image_id;
            }

            void open_image(librados::IoCtx & ioctx,
                            const std::string & image_name, bool readonly,
                            librbd::ImageCtx ** ictxp) {
                librbd::ImageCtx * ictx =
                    new librbd::ImageCtx(image_name.c_str(), "", "", ioctx,
                                         readonly);
            ...
            void get_commit_positions(cls::journal::ObjectPosition *
                                      master_position,
                ...
                *mirror_position = cls::journal::ObjectPosition();
                ...
                for (c = registered_clients.begin();
                    ...
                    if (p != object_positions.end()) {
                        if (c->id == master_client_id) {
                            ASSERT_EQ(cls::journal::ObjectPosition(),
                                      *master_position);
                            *master_position = *p;
                        }
            ...
            int get_last_mirror_snapshot(librados::IoCtx & io_ctx,
                ...
                ::SnapContext snapc;
                int r =
                    librbd::cls_client::get_snapcontext(&io_ctx, header_oid,
                                                        &snapc);
                if (r < 0) {
                    return r;
                }
              ...
              for (auto snap_id:snapc.snaps) {
                    ...
                    r = librbd::cls_client::snapshot_get(&io_ctx, header_oid,
                                                         snap_id, &snap_info);
                    if (r < 0) {
                        return r;
                    }
            ...
            void wait_for_snapshot_synced() {
                ...
                ASSERT_EQ(0,
                          get_last_mirror_snapshot(m_remote_ioctx,
                                                   m_remote_image_id,
                                                   &remote_snap_id,
                                                   &remote_mirror_ns));
                ...
                ASSERT_EQ(0,
                          librbd::cls_client::
                          mirror_image_get_image_id(&m_local_ioctx,
                                                    m_global_image_id,
                                                    &local_image_id));
                ...
                for (int i = 0; i < 100; i++) {
                    int r =
                        get_last_mirror_snapshot(m_local_ioctx, local_image_id,
                                                 &local_snap_id,
                                                 &local_mirror_ns);
                    if (r == 0
                        &&
                        ...
                        && local_mirror_ns.primary_mirror_uuid ==
                        m_remote_mirror_uuid
            ...
            void write_test_data(librbd::ImageCtx * ictx, const char *test_data,
                                 off_t off, size_t len) {
                size_t written;
                bufferlist bl;
                bl.append(std::string(test_data, len));
                written =
                    librbd::api::Io <>::write(*ictx, off, len, std::move(bl),
                                              0);
                printf("wrote: %d\n", (int)written);
                ASSERT_EQ(len, written);
            }

            void read_test_data(librbd::ImageCtx * ictx, const char *expected,
                                off_t off, size_t len) {
                ssize_t read;
                ...
                read =
                    librbd::api::Io <>::read(*ictx, off, len,
            ...
            void flush(librbd::ImageCtx * ictx) {
                ...
                if (MIRROR_IMAGE_MODE == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
                    ...
                    ASSERT_EQ(0,
                              librbd::api::Mirror <>::
                              image_snapshot_create(ictx, 0, &snap_id));
                }
        ...
        template < cls::rbd::MirrorImageMode _mirror_image_mode,
            uint64_t _features > class TestImageReplayerParams {
          public:
            static const cls::rbd::MirrorImageMode MIRROR_IMAGE_MODE =
                _mirror_image_mode;
        ...
        typedef::testing::Types < TestImageReplayerParams <
            ...
            TestImageReplayerParams <
            cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, 125 >> TestImageReplayerTypes;
        ...
        TYPED_TEST(TestImageReplayer, BootstrapErrorLocalImageExists) {
            int order = 0;
            EXPECT_EQ(0,
                      librbd::create(this->m_local_ioctx,
                                     this->m_image_name.c_str(), 1 << 22, false,
                                     0, &order, 0, 0));
        ...
        TYPED_TEST(TestImageReplayer, BootstrapMirrorDisabling) {
            ...
            if (gtest_TypeParam_::MIRROR_IMAGE_MODE ==
                cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
                ASSERT_EQ(0,
                          librbd::api::Mirror <>::mode_set(this->m_remote_ioctx,
                                                           RBD_MIRROR_MODE_IMAGE));
                librbd::ImageCtx * ictx;
                this->open_remote_image(&ictx);
                ASSERT_EQ(0,
                          librbd::api::Mirror <>::image_enable(ictx,
            ...
            cls::rbd::MirrorImage mirror_image;
            ASSERT_EQ(0,
                      librbd::cls_client::mirror_image_get(&this->
                                                           m_remote_ioctx,
                                                           this->
                                                           m_remote_image_id,
                                                           &mirror_image));
            ...
            ASSERT_EQ(0,
                      librbd::cls_client::mirror_image_set(&this->
                                                           m_remote_ioctx,
                                                           this->
                                                           m_remote_image_id,
                                                           mirror_image));
        ...
        TEST_F(TestImageReplayerJournal, NextTag) {
            ...
            for (int j = 0; j < N; j++) {
                this->open_remote_image(&ictx);
                for (int i = j * TEST_IO_COUNT; i < (j + 1) * TEST_IO_COUNT;
                     ++i) {
                    this->write_test_data(ictx, this->m_test_data,
        ...
        TEST_F(TestImageReplayerJournal, MultipleReplayFailures_SingleEpoch) {
            ...
            librbd::ImageCtx * ictx;
            this->open_image(this->m_local_ioctx, this->m_image_name, false,
                             &ictx);
            ictx->features &= ~RBD_FEATURE_JOURNALING;
            librbd::NoOpProgressContext prog_ctx;
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_create(cls::rbd::UserSnapshotNamespace(), "foo", 0,
                                  prog_ctx));
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_protect(cls::rbd::UserSnapshotNamespace(), "foo"));
            ASSERT_EQ(0,
                      librbd::cls_client::add_child(&ictx->md_ctx, RBD_CHILDREN,
                                                    {
                                                    ictx->md_ctx.get_id(), "",
                                                    ictx->id, ictx->snap_ids[ {
                                                                             cls::
                                                                             rbd::
            ...
            for (uint64_t i = 0; i < 10; ++i) {
                ...
                ictx->journal->append_op_event(i, librbd::journal::EventEntry {
                                               librbd::journal::
                                               SnapUnprotectEvent {
                                               i,
                                               cls::rbd::
                                               UserSnapshotNamespace(), "foo"}
                                               }
                                               , &append_ctx);
        ...
        TEST_F(TestImageReplayerJournal, MultipleReplayFailures_MultiEpoch) {
            ...
            this->open_remote_image(&ictx);
            {
                ...
                C_SaferCond append_ctx;
                ictx->journal->append_op_event(1U, librbd::journal::EventEntry {
                                               librbd::journal::
                                               SnapUnprotectEvent {
                                               1U,
                                               cls::rbd::
        ...
        TEST_F(TestImageReplayerJournal, Disconnect) {
            ...
            EXPECT_EQ(0,
                      this->m_local_cluster->
                      conf_set("rbd_mirroring_resync_after_disconnect",
                               "false"));
            ...
            std::string oid =::journal::Journaler::header_oid(this->
                                                              m_remote_image_id);
            ASSERT_EQ(0,
                      cls::journal::client::client_update_state(this->
                                                                m_remote_ioctx,
                                                                oid,
                                                                this->
                                                                m_local_mirror_uuid,
        ...
        TEST_F(TestImageReplayerJournal, UpdateFeatures) {
            ...
            if (features) {
                ASSERT_EQ(0,
                          ictx->operations->update_features(FEATURES_TO_UPDATE,
                                                            false));
            }
            ASSERT_EQ(0, librbd::get_features(ictx, &features));
            ...
            ASSERT_EQ(-EINVAL,
                      ictx->operations->
                      update_features(RBD_FEATURE_EXCLUSIVE_LOCK, false));
            this->generate_test_data();
            for (int i = 0; i < TEST_IO_COUNT; ++i) {
                this->write_test_data(ictx, this->m_test_data, TEST_IO_SIZE * i,
                                      TEST_IO_SIZE);
        ...
        TYPED_TEST(TestImageReplayer, ImageRename) {
            ...
            ASSERT_EQ(0,
                      remote_image_ctx->operations->rename(image_name.c_str()));
            this->flush(remote_image_ctx);

            this->wait_for_replay_complete();

            librbd::ImageCtx * local_image_ctx = nullptr;
            this->open_image(this->m_local_ioctx, image_name, true,
                             &local_image_ctx);
        ...
        TYPED_TEST(TestImageReplayer, UpdateFeatures) {
            ...
            REQUIRE((this->FEATURES & FEATURES_TO_UPDATE) ==
                    FEATURES_TO_UPDATE);

            librbd::ImageCtx * remote_image_ctx = nullptr;
            this->open_remote_image(&remote_image_ctx);

            ASSERT_EQ(0, remote_image_ctx->operations->update_features((RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF), false));
            this->flush(remote_image_ctx);
            ...
            ASSERT_EQ(0,
                      remote_image_ctx->operations->
                      update_features(RBD_FEATURE_DEEP_FLATTEN, false));
            this->flush(remote_image_ctx);
            this->wait_for_replay_complete();
        ...
        TYPED_TEST(TestImageReplayer, SnapshotUnprotect) {
            ...
            auto local_snap_info_it =
                local_image_ctx->snap_info.find(local_snap_id);
        ...
        TYPED_TEST(TestImageReplayer, SnapshotRename) {
            ...
            // rename the snapshot
            ASSERT_EQ(0,
                      remote_image_ctx->operations->snap_rename("snap1",
                                                                "snap1-renamed"));
            this->flush(remote_image_ctx);
            this->wait_for_replay_complete();
        ...
        TYPED_TEST(TestImageReplayer, SnapshotLimit) {
            ...
            ASSERT_EQ(0,
                      librbd::api::Snapshot <>::set_limit(remote_image_ctx,
                                                          123U));
            this->flush(remote_image_ctx);
            this->wait_for_replay_complete();
