...
template < typename I >
DeepCopyRequest < I >::DeepCopyRequest(I *src_image_ctx,
                                       I *dst_image_ctx,
                                       librados::
    ...
    : RefCountedObject(dst_image_ctx->cct), m_src_image_ctx(src_image_ctx),
      ...
      m_snap_seqs(snap_seqs), m_handler(handler), m_on_finish(on_finish),
      m_cct(dst_image_ctx->cct),
      m_lock(ceph::
             make_mutex(unique_lock_name("DeepCopyRequest::m_lock", this)))
{
...
template < typename I > void DeepCopyRequest < I >::send_copy_snapshots()
{
    ...
    Context *ctx = create_context_callback <
                   DeepCopyRequest < I >,
                   &DeepCopyRequest < I >::handle_copy_snapshots > (this);
    m_snapshot_copy_request =
        SnapshotCopyRequest < I >::create(m_src_image_ctx, m_dst_image_ctx,
                                          m_src_snap_id_start,
                                          m_src_snap_id_end,
                                          m_dst_snap_id_start, m_flatten,
...
void DeepCopyRequest < I >::handle_copy_snapshots(int r)
{
    ...
    if (r == -ECANCELED) {
        ...
        lderr(m_cct) << "failed to copy snapshot metadata: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
template < typename I > void DeepCopyRequest < I >::send_copy_image()
{
    ...
    Context *ctx = create_context_callback <
                   DeepCopyRequest < I >,
                   &DeepCopyRequest < I >::handle_copy_image > (this);
    m_image_copy_request =
        ImageCopyRequest < I >::create(m_src_image_ctx, m_dst_image_ctx,
                                       m_src_snap_id_start,
                                       m_src_snap_id_end,
                                       m_dst_snap_id_start, m_flatten,
                                       m_object_number, *m_snap_seqs,
                                       m_handler, ctx);
...
template < typename I > void DeepCopyRequest < I >::handle_copy_image(int r)
{
    ...
    if (r == -ECANCELED) {
        ...
        lderr(m_cct) << "failed to copy image: " << cpp_strerror(r) <<
                     dendl;
        finish(r);
        return;
    }
...
void DeepCopyRequest < I >::handle_copy_object_map(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to roll back object map: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
...
template < typename I > void DeepCopyRequest < I >::send_copy_metadata()
{
    ...
    Context *ctx = create_context_callback <
                   DeepCopyRequest < I >,
                   &DeepCopyRequest < I >::handle_copy_metadata > (this);
...
template < typename I > int DeepCopyRequest < I >::validate_copy_points()
{
    ...
    if (m_src_snap_id_start != 0 &&
        ...
        lderr(m_cct) << "invalid start snap_id " << m_src_snap_id_start <<
                     dendl;
        return -EINVAL;
    }
    ...
    if (m_src_snap_id_end != CEPH_NOSNAP &&
        ...
        m_src_image_ctx->snap_info.end()) {
        lderr(m_cct) << "invalid end snap_id " << m_src_snap_id_end <<
                     dendl;
        return -EINVAL;
    }
