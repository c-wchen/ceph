...
namespace crimson::os::seastore {
    ...
    template < typename bound_t > struct fixed_kv_node_meta_t {
        ...
        } std::pair < fixed_kv_node_meta_t,
            fixed_kv_node_meta_t > split_into(bound_t pivot) const {
            return std::make_pair(fixed_kv_node_meta_t {
                                  begin, pivot, depth}
                                  , fixed_kv_node_meta_t {
                                  pivot, end, depth}
            );
        } static fixed_kv_node_meta_t merge_from(const fixed_kv_node_meta_t &
                                                 lhs,
                                                 const fixed_kv_node_meta_t &
                                                 rhs) {
            ceph_assert(lhs.depth == rhs.depth);
            return fixed_kv_node_meta_t {
            lhs.begin, rhs.end, lhs.depth};
        }

        static std::pair < fixed_kv_node_meta_t, fixed_kv_node_meta_t >
            rebalance(const fixed_kv_node_meta_t & lhs,
                      const fixed_kv_node_meta_t & rhs, bound_t pivot) {
            ceph_assert(lhs.depth == rhs.depth);
            return std::make_pair(fixed_kv_node_meta_t {
                                  lhs.begin, pivot, lhs.depth}
                                  , fixed_kv_node_meta_t {
                                  pivot, rhs.end, lhs.depth}
            );
        }

        bool is_root() const {
            return begin == min_max_t < bound_t >::min
                && end == min_max_t < bound_t >::max;
    }};

    template < typename bound_t >
        inline std::ostream & operator<<(std::ostream & lhs,
                                         const fixed_kv_node_meta_t < bound_t >
                                         &rhs) {
        return lhs << "btree_node_meta_t(" << "begin=" << rhs.
            begin << ", end=" << rhs.end << ", depth=" << rhs.depth << ")";
    }
 ...
 */
    template < typename bound_le_t > struct fixed_kv_node_meta_le_t {
        ...
        fixed_kv_node_meta_le_t(const fixed_kv_node_meta_le_t < bound_le_t >
                                &) = default;
        explicit fixed_kv_node_meta_le_t(const fixed_kv_node_meta_t <
                                         typename bound_le_t::orig_type > &val)
  ...
  template < typename key_t, typename val_t > class BtreeNodeMapping:public PhysicalNodeMapping < key_t,
        val_t >
    {
   ...
   */
      ...
      :    
        ctx(ctx),
        parent(parent),
        value(value), len(len), range(std::move(meta)), pos(pos) {
        ...
        uint16_t get_pos() const final {
            ...
            auto ret =
                std::unique_ptr < BtreeNodeMapping < key_t,
                val_t >> (new BtreeNodeMapping < key_t, val_t > (ctx));
            ret->range = range;
            ret->value = value;
            ret->parent = parent;
            ret->len = len;
            ...
            get_logical_extent(Transaction &) final;
    };
