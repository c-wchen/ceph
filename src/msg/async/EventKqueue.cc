...
int KqueueDriver::add_event(int fd, int cur_mask, int add_mask)
{
    ...
    ldout(cct, 30) << __func__ << " add event kqfd = " << kqfd << " fd = " << fd
        << " cur_mask = " << cur_mask << " add_mask = " << add_mask << dendl;
...
int KqueueDriver::del_event(int fd, int cur_mask, int del_mask)
{
    ...
    if (num) {
        ...
        if ((r = kevent(kqfd, ke, num, NULL, 0, KEVENT_NOWAIT)) < 0) {
            lderr(cct) << __func__ << " kevent: delete fd=" << fd << " mask=" <<
...
int KqueueDriver::resize_events(int newsize)
{
    ...
    if (newsize > sav_max) {
        ...
        memset(&sav_events[size], 0,
               sizeof(struct SaveEvent) * (newsize - sav_max));
        sav_max = newsize;
    }
    return 0;
}

int KqueueDriver::event_wait(vector < FiredFileEvent > &fired_events,
                             struct timeval *tvp)
{
    ...
    if (tvp != NULL) {
        ...
        ldout(cct, 20) << __func__ << " "
            << timeout.tv_sec << " sec " << timeout.tv_nsec << " nsec" << dendl;
