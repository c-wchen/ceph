...
client(client_), finisher(f), cmd_finisher(g_ceph_context, "cmd_finisher",
                                           "cmdfin"), server(server),
...
PyObject *ActivePyModules::list_servers_python()
{
    ...
    return daemon_state.with_daemons_by_server([this, &no_gil]
      ...
      no_gil.acquire_gil(); PyFormatter f(false, true); for (const auto &[hostname, daemon_state]:all)
                                               {
                                               f.open_object_section("server");
                                               dump_server(hostname,
                                                           daemon_state, &f);
                                               f.close_section();}
                                               return f.get();}
    ) ;
...
PyObject *ActivePyModules::get_metadata_python(const std::string & svc_type,
                                               const std::string & svc_id)
{
    auto metadata = daemon_state.get(DaemonKey { svc_type, svc_id });
    if (metadata == nullptr) {
        derr << "Requested missing service " << svc_type << "." << svc_id <<
            dendl;
        Py_RETURN_NONE;
    }
    auto l = without_gil([&]{
...
PyObject *ActivePyModules::get_daemon_status_python(const std::
                                                    string & svc_type,
                                                    const std::string & svc_id)
{
    auto metadata = daemon_state.get(DaemonKey { svc_type, svc_id });
...
PyObject *ActivePyModules::get_python(const std::string & what)
{
    ...
    if (what == "fs_map") {
        ...
        cluster_state.with_fsmap([&](const FSMap & fsmap) {
                                 no_gil.acquire_gil(); fsmap.dump(&f);});
    }
      ...
      for (auto & name:names) {
        ...
        cluster_state.with_monmap([&](const MonMap & monmap) {
                                  no_gil.acquire_gil(); monmap.dump(&f);});
    }
      ...
      for (const auto &[key, state]:dmc) {
            ...
            with_gil(no_gil,[&f, &name = key.name, state = state] {
              f.open_object_section(name.c_str()); f.dump_string("hostname", state->hostname); for (const auto &[name, val]:state->metadata)
                     {
      ...
      for (const auto &[key, state]:dmc) {
        ...
        cluster_state.with_pgmap([&f, &no_gil] (const PGMap & pg_map) {
          std::map < std::string, std::map < std::string, uint32_t > >osds; std::map < std::string, std::map < std::string, uint32_t > >pools; std::map < std::string, uint32_t > all; for (const auto & i:pg_map.pg_stat)
                                 {
                                 const auto pool = i.first.m_pool;
                                 const std::string state =
                                 pg_state_string(i.second.state);
                                 // Insert to per-pool map
              pools[stringify(pool)][state]++; for (const auto & osd_id:i.second.acting)
                                 {
                                 osds[stringify(osd_id)][state]++;}
                                 all[state]++;}
          no_gil.acquire_gil(); f.open_object_section("by_osd"); for (const auto & i:osds)
                                 {
              f.open_object_section(i.first.c_str()); for (const auto & j:i.second)
                                 {
                                 f.dump_int(j.first.c_str(), j.second);}
                                 f.close_section();}
          f.close_section(); f.open_object_section("by_pool"); for (const auto & i:pools)
                                 {
              f.open_object_section(i.first.c_str()); for (const auto & j:i.second)
                                 {
                                 f.dump_int(j.first.c_str(), j.second);}
                                 f.close_section();}
          f.close_section(); f.open_object_section("all"); for (const auto & i:all)
                                 {
                                 f.dump_int(i.first.c_str(), i.second);}
                                 f.close_section();
                                 f.open_object_section("pg_stats_sum");
                                 pg_map.pg_sum.dump(&f); f.close_section();}
        ) ;
    }
        ...
        without_gil_t no_gil;
        cluster_state.with_pgmap([&](const PGMap & pg_map) {
                                 no_gil.acquire_gil(); pg_map.dump(&f, false);}
        );
    }
        ...
        if (!daemon_state.with_device(devid,[&](const DeviceState & dev) {
                                      with_gil_t with_gil {
                                      no_gil}; f.dump_object("device", dev);})) {
            // device not found
        }
    }
        ...
        without_gil_t no_gil;
        cluster_state.with_pgmap([&](const PGMap & pg_map) {
                                 no_gil.acquire_gil(); pg_map.dump_delta(&f);}
        );
    }
    else if (what == "df") {
        without_gil_t no_gil;
        cluster_state.with_osdmap_and_pgmap([&](const OSDMap & osd_map,
                                                const PGMap & pg_map) {
                                            no_gil.acquire_gil();
                                            pg_map.dump_cluster_stats(nullptr,
                                                                      &f, true);
                                            pg_map.dump_pool_stats_full(osd_map,
                                                                        nullptr,
                                                                        &f,
                                                                        true);});
    }
        ...
        cluster_state.with_osdmap_and_pgmap([&](const OSDMap & osdmap,
                                                const PGMap & pg_map) {
          no_gil.acquire_gil(); f.open_array_section("pool_stats"); for (auto & p:osdmap.get_pools())
                                            {
                                            poolid = p.first;
                                            pg_map.
        ...
        cluster_state.with_mgrmap([&](const MgrMap & mgr_map) {
                                  no_gil.acquire_gil(); mgr_map.dump(&f);});
    }
      ...
      for (auto & i:myaddrs.v) {
        ...
        without_gil_t no_gil;
        cluster_state.with_pgmap([&](const PGMap & pg_map) {
          no_gil.acquire_gil(); f.open_array_section("pg_stats"); for (auto & i:pg_map.pg_stat)
                                 {
                                 const auto state = i.second.state;
                                 const auto pgid_raw = i.first;
                                 const auto pgid =
                                 stringify(pgid_raw.m_pool) + "." +
                                 stringify(pgid_raw.m_seed);
                                 const auto reported_epoch =
                                 i.second.reported_epoch;
                                 if (state & PG_STATE_ACTIVE
                                     && state & PG_STATE_CLEAN) {
                                 f.open_object_section("pg_stat");
                                 f.dump_string("pgid", pgid);
                                 f.dump_string("state", pg_state_string(state));
                                 f.dump_unsigned("reported_epoch",
                                                 reported_epoch);
                                 f.close_section();}
                                 }
                                 f.close_section();
...
void ActivePyModules::start_one(PyModuleRef py_module)
{
    ...
    finisher.queue(new LambdaContext([this, active_module, name] (int) {
                                     ...
                                     std::lock_guard l(lock);
                                     pending_modules.erase(name); if (r != 0) {
                                     derr <<
                                     "Failed to run module in active mode ('" <<
                                     name << "')" << dendl;}
                                     else {
                                     auto em = modules.emplace(name, active_module); ceph_assert(em.second);    // actually inserted
                                     dout(4) << "Starting thread for " << name
                                     << dendl;
                                     active_module->thread.
                                     create(active_module->get_thread_name());
                                     dout(4) << "Starting active module " <<
                                     name << " finisher thread " <<
                                     active_module->
                                     get_fin_thread_name() << dendl;
...
void ActivePyModules::shutdown()
{
  ...
  for (auto &[name, module]:modules) {
        dout(4) << "Stopping active module " << name << " finisher thread" <<
            dendl;
                                 ...
                                 const std::string & notify_id)
{
  ...
  for (auto &[name, module]:modules) {
        ...
        // Send all python calls down a Finisher to avoid blocking
        // C++ code, and avoid any potential lock cycles.
        dout(15) << "queuing notify (" << notify_type << ") to " << name <<
            dendl;
        Finisher & mod_finisher =
            py_module_registry.get_active_module_finisher(name);
        // workaround for https://bugs.llvm.org/show_bug.cgi?id=35984
        mod_finisher.
            queue(new LambdaContext([module = module, notify_type, notify_id]
                                    (int r) {
...
void ActivePyModules::notify_all(const LogEntry & log_entry)
{
  ...
  for (auto &[name, module]:modules) {
        ...
        // workaround for https://bugs.llvm.org/show_bug.cgi?id=35984
        mod_finisher.
            queue(new LambdaContext([module = module, log_entry] (int r) {
...
PyObject *ActivePyModules::dispatch_remote(const std::string & other_module,
...
PyObject *ActivePyModules::get_typed_config(const std::string & module_name,
                                            const std::string & key,
                                            const std::
                                            string & prefix) constconst
{
    without_gil_t no_gil;
    std::string value;
                                ...
                                const std::optional < std::string > &val)
{
    ...
    if (set_cmd.r != 0) {
        ...
        dout(0) << "mon returned " << set_cmd.r << ": " << set_cmd.
            outs << dendl;
    }
}

std::pair < int,
    std::string > ActivePyModules::set_config(const std::string & module_name,
                                              const std::string & key,
                                              const std::optional <
                                              std::string > &val)
{
...
std::map < std::string, std::string > ActivePyModules::get_services() constconst
{
...
void ActivePyModules::update_kv_data(const std::string prefix,
                                     bool incremental,
...
void ActivePyModules::_refresh_config_map()
{
    ...
    for (auto p = store_cache.lower_bound("config/");
        ...
        if (!opt) {
            config_map.stray_options.push_back(std::unique_ptr < Option >
                                               (new
                                                Option(name, Option::TYPE_STR,
...
PyObject *ActivePyModules::with_perf_counters(std::function <
        ...
        if (metadata) {
            ...
            if (metadata->perf_counters.instances.count(path)) {
                auto counter_instance =
                    metadata->perf_counters.instances.at(path);
              ...
              for (const auto & i:metadata->perf_counters.
                     instances) {
...
PyObject *ActivePyModules::get_counter_python(const std::string & svc_name,
                                              const std::string & svc_id,
                                              const std::string & path)
{
    auto extract_counters =[](PerfCounterInstance & counter_instance,
        ...
        if (counter_type.type & PERFCOUNTER_LONGRUNAVG) {
          const auto & avg_data = counter_instance.get_data_avg(); for (const auto & datapoint:avg_data)
                                                         {
                                                         f.
                                                         open_array_section
                                                         ("datapoint");
          ...
          const auto & data = counter_instance.get_data(); for (const auto & datapoint:data)
                                                         {
                                                         f.
                                                         open_array_section
                                                         ("datapoint");
  ...
  for (auto &[key, state]:                              daemons)
                                                         ...
                                                         with_gil(no_gil,
                                                                  [&, key =
                                                                   ...
                                                                   state =
                                                                   state] {
          f.open_object_section(key.c_str()); for (auto ctr_inst_iter:state->perf_counters.instances) {
                                                                  const auto &
                                                                  counter_name =
                                                                  ctr_inst_iter.
                                                                  first;
 ...
 */
                                                         ...
                                                         PyTuple_Pack(1,
                                                                      wrapped_capsule);
                                                         auto wrapper_instance =
                                                         PyObject_CallObject
                                                         (wrapper_type, pArgs);
                                                         if (wrapper_instance ==
                                                             nullptr) {
                                                         derr <<
                                                         "Failed to construct python OSDMap:"
                                                         << dendl;
                                                         derr <<
                                                         handle_pyerror(true,
                                                                        module_name,
                                                         ...
                                                         {
                                                         dout(20) <<
                                                         "mon cluster wasn't pacific when we started: falling back to 'config get'"
                                                         << dendl;
                                                         without_gil_t no_gil;
                                                         Command cmd; {
                                                         std::
                                                         ...
                                                         cmd.wait();
                                                         dout(10) <<
                                                         "ceph_foreign_option_get (mon command) "
                                                         << who << " " << name
                                                         << " = " << cmd.outbl.
                                                         to_str() << dendl;
                                                         no_gil.acquire_gil();
                                                         return
                                                         get_python_typed_option_value
                                                         (opt->type,
                                                          cmd.outbl.to_str());}
                                                         ...
                                                         dout(10) <<
                                                         "ceph_foreign_option_get (configmap) "
                                                         << who << " " << name
                                                         << " = " << value <<
                                                         dendl; lock.unlock();
                                                         no_gil.acquire_gil();
                                                         ...
                                                         void ActivePyModules::
                                                         set_health_checks(const
                                                                           std::
                                                                           string
                                                                           &
                                                                           module_name,
                                                         ...
                                                         *ss << "Module '" <<
                                                         ...
                                                         (health_check_map_t *
                                                          checks) {
  std::lock_guard l(lock); for (auto &[name, module]:   modules)
                                                         {
                                                         dout(15) <<
                                                         "getting health checks for "
                                                         << name << dendl;
                                                         module->
                                                         get_health_checks
                                                         (checks);}
  ...
  std::lock_guard l(lock); for (auto &[name, module]:   modules)
                                                         ...
                                                         mod_finisher.
                                                         queue(new
                                                               LambdaContext([module = module] (int r) {
                                                                             module->
                                                                             config_notify
                                                                             ();}));}
                                                         }
                                                         ...
                                                         with_device(devid,
                                                                     ...
                                                                     metadata;});
                                                         // tell mon
  json_spirit::Object json_object; for (auto & i:       meta)
                                                         {
                                                         json_spirit::Config::
                                                         add(json_object,
                                                             i.first,
                                                         ...
                                                         if (r < 0) {
                                                         dout(0) <<
                                                         "remove_osd_perf_query for query_id="
                                                         << query_id <<
                                                         " failed: " <<
                                                         cpp_strerror(r) <<
                                                         dendl;}
                                                         ...
                                                         if (r < 0) {
                                                         dout(0) <<
                                                         "get_osd_perf_counters for query_id="
                                                         << query_id <<
                                                         " failed: " <<
                                                         cpp_strerror(r) <<
                                                         dendl; Py_RETURN_NONE;}

  PyFormatter f; const std::map < OSDPerfMetricKey, PerformanceCounters > &counters = collector.counters; f.open_array_section("counters"); for (auto &[key, instance_counters]:counters)
                                                         {
      f.open_object_section("i"); f.open_array_section("k"); for (auto & sub_key:key)
                                                         {
                                                         f.
                                                         open_array_section
                                                         ("s");
                                                         ...
                                                         f.close_section(); // k
      f.open_array_section("c"); for (auto & c:         instance_counters)
                                                         {
                                                         f.open_array_section("p"); f.dump_unsigned("0", c.first); f.dump_unsigned("1", c.second); f.close_section();   // p
                                                         }
                                                         f.close_section(); // c
                                                         f.close_section(); // i
                                                         }
                                                         ...
                                                         if (r < 0) {
                                                         dout(0) <<
                                                         "remove_mds_perf_query for query_id="
                                                         << query_id <<
                                                         " failed: " <<
                                                         cpp_strerror(r) <<
                                                         dendl;}
                                                         ...
                                                         if (r < 0) {
                                                         dout(0) <<
                                                         "get_mds_perf_counters for query_id="
                                                         << query_id <<
                                                         " failed: " <<
                                                         cpp_strerror(r) <<
                                                         dendl; Py_RETURN_NONE;}

                                                         PyFormatter f; const std::map < MDSPerfMetricKey, PerformanceCounters > &counters = collector.counters; f.open_array_section("metrics"); f.open_array_section("delayed_ranks"); f.dump_string("ranks", stringify(collector.delayed_ranks).c_str()); f.close_section(); // delayed_ranks
  f.open_array_section("counters"); for (auto &[key, instance_counters]:counters)
                                                         {
      f.open_object_section("i"); f.open_array_section("k"); for (auto & sub_key:key)
                                                         {
                                                         f.
      ...
      f.open_array_section("c"); for (auto & c:         instance_counters)
                                                         ...
                                                         f.close_section(); // counters
                                                         f.open_array_section("last_updated"); f.dump_float("last_updated_mono", collector.last_updated_mono); f.close_section();   // last_updated
                                                         f.close_section(); // metrics
                                                         return f.get();}

                                                         void ActivePyModules::
                                                         cluster_log(const std::
                                                                     string &
                                                                     channel,
                                                                     clog_type
                                                                     prio,
                                                         ...
                                                         addrv.parse(addrs.
                                                                     data());
                                                         dout(7) <<
                                                         "registering msgr client handle "
                                                         << addrv << dendl;
                                                         py_module_registry.
                                                         ...
                                                         without_gil_t no_gil;
                                                         return daemon_state.
                                                         with_daemons_by_server([&no_gil]
                                                                                (const std::map < std::string, DaemonStateCollection > &all) {
      no_gil.acquire_gil(); PyFormatter f; for (const auto &[hostname, daemon_state]:all) {
          for (const auto &[key, state]:                                       daemon_state) {
              f.open_array_section(ceph::to_string(key)); for (const auto & metric:state->daemon_health_metrics) {
                                                                                f.
                                                                                open_object_section
                                                                                (metric.
                                                                                 get_type_name
