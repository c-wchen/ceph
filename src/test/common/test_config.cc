...
public:
    ...
    void test_expand_meta()
    {
        ...
        }
        {
            ...
            early_expand_meta(val, &oss);
            EXPECT_EQ(before + "$$1/var/run/ceph$2/var/run/ceph$3$" + after,
                      val);
            EXPECT_EQ("", oss.str());
        }
        {
        ...
        // recursive variable expansion
        {
            ...
            EXPECT_EQ(public_network + " " +
                      public_network + " " + lockdep + " " + "localhost", val);
            EXPECT_EQ("", oss.str());
        }
...
TEST(md_config_t, set_val)
{
    ...
    // set_val should support 1 days 2 hours 4 minutes
    {
        ...
        EXPECT_EQ(expected.count(),
                  conf.get_val < seconds > ("mgr_tick_period").count());
        EXPECT_EQ(-EINVAL,
                  conf.set_val("mgr_tick_period", "21 centuries", nullptr));
        EXPECT_EQ(expected.count(),
                  conf.get_val < seconds > ("mgr_tick_period").count());
    }
...
TEST(Option, validation)
{
    ...
    EXPECT_EQ(-EINVAL,
              opt_enum.validate(Option::value_t(std::string("green")), &msg));
