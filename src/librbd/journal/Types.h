...
struct AioWriteEvent {
    ...
    {
    } AioWriteEvent(uint64_t _offset, uint64_t _length,
...
struct AioCompareAndWriteEvent {
    ...
    AioCompareAndWriteEvent(): offset(0), length(0)
    {
        ...
        : offset(_offset), length(_length), cmp_data(_cmp_data),
...
protected:
    SnapEventBase()
    {
    } SnapEventBase(uint64_t op_tid,
                    const cls::rbd::SnapshotNamespace &_snap_namespace,
                    const std::string &_snap_name)
        : OpEventBase(op_tid), snap_namespace(_snap_namespace),
...
struct SnapCreateEvent: public SnapEventBase {
    ...
    SnapCreateEvent()
    {
    } SnapCreateEvent(uint64_t op_tid,
                      const cls::rbd::
...
struct SnapRemoveEvent: public SnapEventBase {
    ...
    SnapRemoveEvent()
    {
    } SnapRemoveEvent(uint64_t op_tid,
                      const cls::rbd::
                      SnapshotNamespace &snap_namespace,
                      const std::string &snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
...
struct SnapRenameEvent: public OpEventBase {
    ...
    SnapRenameEvent(): snap_id(CEPH_NOSNAP)
    {
        ...
        : OpEventBase(op_tid),
          snap_id(src_snap_id),
          src_snap_name(src_snap_name), dst_snap_name(dest_snap_name)
    {
    }
...
struct SnapProtectEvent: public SnapEventBase {
    ...
    SnapProtectEvent()
    {
    } SnapProtectEvent(uint64_t op_tid,
                       const cls::rbd::
                       SnapshotNamespace &snap_namespace,
                       const std::string &snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
...
struct SnapUnprotectEvent: public SnapEventBase {
    ...
    SnapUnprotectEvent()
    {
    } SnapUnprotectEvent(uint64_t op_tid,
                         const cls::rbd::
                         SnapshotNamespace &snap_namespace,
                         const std::string &snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
...
struct SnapRollbackEvent: public SnapEventBase {
    ...
    SnapRollbackEvent()
    {
    } SnapRollbackEvent(uint64_t op_tid,
                        const cls::rbd::
                        SnapshotNamespace &snap_namespace,
                        const std::string &snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
...
struct UpdateFeaturesEvent: public OpEventBase {
    ...
    {
    } UpdateFeaturesEvent(uint64_t op_tid, uint64_t _features,
                          bool _enabled)
...
struct MetadataSetEvent: public OpEventBase {
    ...
    MetadataSetEvent()
    {
    } MetadataSetEvent(uint64_t op_tid, const std::string &_key,
...
typedef boost::mpl::vector < AioDiscardEvent,
        ...
        MetadataRemoveEvent,
        AioWriteSameEvent,
        AioCompareAndWriteEvent, UnknownEvent > EventVector;
...
struct EventEntry {
    ...
    EventEntry(const Event &_event, const utime_t &_timestamp =
                   utime_t())
...
struct MirrorPeerSyncPoint {
    ...
    MirrorPeerSyncPoint(const cls::rbd::
                        SnapshotNamespace &snap_namespace,
                        const std::string &snap_name,
...
struct MirrorPeerClientMeta {
    ...
    MirrorPeerClientMeta()
    {
                           ...
                           const SyncPoints &sync_points =
                               SyncPoints(), const SnapSeqs &snap_seqs =
                               SnapSeqs())
...
struct UnknownClientMeta {
    static const ClientMetaType TYPE =
        static_cast < ClientMetaType >(-1);
...
typedef boost::variant < ImageClientMeta,
        MirrorPeerClientMeta, CliClientMeta, UnknownClientMeta > ClientMeta;

struct ClientData {
    ClientData()
    {
    } ClientData(const ClientMeta &
                 client_meta): client_meta(client_meta)
    {
    }
...
struct TagPredecessor {
    ...
    TagPredecessor()
    {
        ...
        : mirror_uuid(mirror_uuid), commit_valid(commit_valid),
...
struct TagData {
    ...
    TagData(const std::string &mirror_uuid,
        ...
        : mirror_uuid(mirror_uuid), predecessor(predecessor_mirror_uuid,
