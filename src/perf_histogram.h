    ...
    struct axis_config_d {
        ...
        int32_t m_buckets = 0;
         ...
         axis_config_d(const char *name,
                       ...
                       int64_t min, int64_t quant_size, int32_t buckets)
        :m_name(name),
            m_scale_type(scale_type),
            m_min(min), m_quant_size(quant_size), m_buckets(buckets) {
    }};

  protected:
    /// Dump configuration of one axis to a formatter
    static void dump_formatted_axis(ceph::Formatter * f,
    ...
    static std::vector < std::pair < int64_t,
        int64_t >> get_axis_bucket_ranges(const axis_config_d & ac);
};
...
template < int DIM = 2 > class PerfHistogram:public PerfHistogramCommon {
  public:
    ...
    PerfHistogram(std::initializer_list < axis_config_d > axes_config) {
      ceph_assert(axes_config.size() == DIM && "Invalid number of axis configuration objects"); int i = 0; for (const auto & ac:axes_config) {
                                                                        ceph_assertf
                                                                        (ac.
                                                                         m_buckets
                                                                         > 0,
      ...
      f->open_array_section("axes"); for (auto & ac:                   m_axes_config) {
                                                                        dump_formatted_axis
                                                                        (f,
                                                                         ac);}
                                                                        f->
  ...
  protected:
                                                                        ...
                                                                        get_raw_size
                                                                        () {
      int64_t ret = 1; for (const auto & ac:                           m_axes_config) {
                                                                        ret *=
                                                                        ac.
                                                                        m_buckets;}
                                                                        return
                                                                        ...
                                                                        ([]
                                                                         ...
                                                                         (bucket
                                                                          >= 0,
                                                                          "Bucket index can not be negative");
                                                                         ceph_assertf
                                                                         (bucket
                                                                          <
                                                                          ac.
