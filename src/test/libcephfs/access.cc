...
TEST(AccessTest, Foo)
{
    ...
    ASSERT_EQ(0,
              do_mon_command
              ("{\"prefix\": \"auth get-or-create\", \"entity\": \"client." +
               user +
               "\", " "\"caps\": [\"mon\", \"allow *\", \"osd\", \"allow rw\", "
               "\"mds\", \"allow rw\"" "], \"format\": \"json\"}", &key));
...
TEST(AccessTest, Path)
{
    ...
    fd = ceph_open(cmount, string(good + "/y").c_str(), O_CREAT | O_WRONLY,
                   0755);
    ASSERT_GE(fd, 0);
    ...
    fd = ceph_open(cmount, string(bad + "/y").c_str(), O_CREAT | O_WRONLY,
                   0755);
    ASSERT_LT(fd, 0);

    // unlink open file
    fd = ceph_open(cmount, string(good + "/unlinkme").c_str(),
    ...
    // rename open file
    fd = ceph_open(cmount, string(good + "/renameme").c_str(),
                   O_CREAT | O_WRONLY, 0755);
    ...
    ASSERT_GE(ceph_fsetxattr(cmount, fd, "user.any", "bar", 3, 0),
              -CEPHFS_EACCES);
    ceph_close(cmount, fd);
...
TEST(AccessTest, ReadOnly)
{
    ...
    ASSERT_EQ(0, ceph_mkdir(admin, dir.c_str(), 0755));
    int fd =
        ceph_open(admin, string(dir + "/out").c_str(), O_CREAT | O_WRONLY,
                  0755);
    ...
    // not allowed
    fd = ceph_open(cmount, string(dir + "/bar").c_str(), O_CREAT | O_WRONLY,
                   0755);
    ASSERT_LT(fd, 0);
...
TEST(AccessTest, User)
{
    ...
    ASSERT_EQ(-CEPHFS_EACCES,
              ceph_mkdir(cmount, string(dir + "/no").c_str(), 0755));
