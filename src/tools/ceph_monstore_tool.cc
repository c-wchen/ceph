                  ...
                  vector < string > &subcmds, MonitorDBStore &store)
{
    ...
    version_t pending_pn =
        (store.get(prefix, "accepted_pn") / 100 + 4) * 100 + 1;
...
static int update_mgrmap(MonitorDBStore &st)
{
...

    {
        ...
        auto initial_modules =
            get_str_vec(g_ceph_context->_conf.get_val < string >
                        ("mgr_initial_modules"));
        copy(begin(initial_modules), end(initial_modules),
...
int main(int argc, char **argv)
{
    ...
    try {
        po::parsed_options parsed =
            po::command_line_parser(argc,
                                    argv).options(all_desc).positional
            (positional).allow_unregistered().run();
    ...
    if (vm.empty() || vm.count("help") ||
        store_path.empty() || cmd.empty() || *cmd.begin() == '-') {
        usage(argv[0], desc);
        return 1;
    }
    ...
    auto cct = global_init(NULL, ceph_options, CEPH_ENTITY_TYPE_MON,
    ...
    if (cmd == "dump-keys") {
        ...
        if (!outpath.empty()) {
            fd =::open(outpath.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_BINARY,
                       0666);
            if (fd < 0) {
        ...
        try {
            po::parsed_options op_parsed =
                po::command_line_parser(subcmds).options(op_desc_all).
