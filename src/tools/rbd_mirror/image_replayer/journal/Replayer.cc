            ...
            namespace journal {
                ...
                namespace {
                                                    ...
                                                    int mirroring_replay_delay)
                    {
                        ...
                        // ensure it is rounded up when converting to integer
                            return (event_time + mirroring_replay_delay - now) +
                            1;
                }} // anonymous namespace using librbd::util::
                ...
                template < typename I >
                    ...
                    utime_t replay_start_time;
                     ...
                     C_ReplayCommitted(Replayer * replayer, ReplayEntry
                                       && replay_entry, uint64_t replay_bytes,
                    ...
                    :replayer(replayer), replay_entry(std::move(replay_entry)),
                        ...
                        replayer->handle_process_entry_safe(replay_entry,
                                                            replay_bytes,
                                                            replay_start_time,
                                                            r);
                    }
                ...
                template < typename I >
                    struct Replayer <I >::RemoteJournalerListener:public::
                    journal::JournalMetadataListener {
                    Replayer *replayer;

                     RemoteJournalerListener(Replayer *
                ...
                template < typename I >
                    struct Replayer <I >::RemoteReplayHandler:public::journal::
                    ReplayHandler {
                    Replayer *replayer;
                ...
                template < typename I >
                    struct Replayer <I >::LocalJournalListener:public librbd::
                    journal::Listener {
                    Replayer *replayer;

                     LocalJournalListener(Replayer *
                                          replayer):replayer(replayer) {
                ...
                template < typename I >
                    Replayer < I >::Replayer(Threads < I > *threads,
                                             ...
                                             ReplayerListener *
                                             replayer_listener)
                :m_threads(threads), m_local_mirror_uuid(local_mirror_uuid),
                ...
                template < typename I >
...

                    {
                        ...
                        m_image_spec =
                            util::compute_image_spec(local_image_ctx->md_ctx,
                ...
                template < typename I >
                    ...
                    flush_local_replay(new
                                       C_TrackedOp(m_in_flight_op_tracker,
                                                   on_finish));
                }
                ...
                template < typename I >
                    ...
                    on_finish =
                        new C_TrackedOp(m_in_flight_op_tracker, on_finish);
                    return m_replay_status_formatter->
                        get_or_send_update(description, on_finish);
                }
                ...
                template < typename I >
                    ...
                    Context *ctx = create_context_callback <
                        Replayer,
                        &Replayer < I >::handle_init_remote_journaler > (this);
                ...
                template < typename I >
                    ...
                    r = m_state_builder->remote_journaler->
                        get_cached_client(m_local_mirror_uuid, &remote_client);
                    if (r < 0) {
                ...
                template < typename I >
                    void Replayer <
                    I >::start_external_replay(std::unique_lock < ceph::mutex >
                                               &locker) {
                    dout(10) << dendl;
                    ...
                    Context *start_ctx = create_context_callback <
                        Replayer,
                        &Replayer < I >::handle_start_external_replay > (this);
                    m_local_journal->
                ...
                template < typename I >
                    ...
                    m_replay_status_formatter =
                        ...
                        I >::create(m_state_builder->remote_journaler,
                                    m_local_mirror_uuid);
                ...
                template < typename I >
                    bool Replayer <
                    I >::add_local_journal_listener(std::unique_lock <
                                                    ceph::mutex > &locker) {
                    dout(10) << dendl;
                    ...
                    int r =
                        m_local_journal->is_resync_requested(&resync_requested);
                    if (r < 0) {
                        dout(10) << "failed to determine resync state: " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_complete(locker, r,
                ...
                template < typename I >
                    bool Replayer < I >::notify_init_complete(std::unique_lock <
                                                              ceph::mutex >
                                                              &locker) {
                    dout(10) << dendl;
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_shut_down_local_journal_replay >
                        (this);
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_shut_down_local_journal_replay(int r) {
                    dout(10) << "r=" << r << dendl;

                    std::unique_lock locker {
                    m_lock};
                    if (r < 0) {
                        derr << "error shutting down journal replay: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_error(r,
                ...
                template < typename I > void Replayer < I >::close_local_image() {
                    ...
                    if (m_event_preprocessor != nullptr) {
                        image_replayer::journal::EventPreprocessor <
                            I >::destroy(m_event_preprocessor);
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_close_local_image > (this);
                    auto request =
                        image_replayer::CloseImageRequest <
                        I >::create(&m_state_builder->local_image_ctx, ctx);
                    request->send();
                }
                ...
                template < typename I >
                    ...
                    m_lock};
                    if (r < 0) {
                        derr << "error closing local iamge: " << cpp_strerror(r)
                            << dendl;
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_stop_remote_journaler_replay(int r) {
                    dout(10) << "r=" << r << dendl;

                    std::unique_lock locker {
                    m_lock};
                    if (r < 0) {
                        derr << "failed to stop remote journaler replay : " <<
                            cpp_strerror(r)
                            << dendl;
                        handle_replay_error(r,
                ...
                template < typename I >
                    ...
                    ReplayStatusFormatter <
                        I >::destroy(m_replay_status_formatter);
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_remote_journal_metadata_updated() {
                    dout(20) << dendl;
                    ...
                    int r =
                        ...
                        get_cached_client(m_local_mirror_uuid, &remote_client);
                    if (r < 0) {
                        derr << "failed to retrieve client: " << cpp_strerror(r)
                            << dendl;
                        return;
                    }
                ...
                template < typename I >
                    ...
                    m_flush_local_replay_task =
                        create_async_context_callback(m_threads->work_queue,
                                                      ...
                                                      > (this));
                    m_threads->timer->add_event_after(30,
                                                      m_flush_local_replay_task);
                }
                ...
                template < typename I >
                    void Replayer <
                    I >::flush_local_replay(Context * on_flush) {
                    std::unique_lock locker {
                    m_lock};
                    if (m_state != STATE_REPLAYING) {
                        locker.unlock();
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_flush_local_replay(Context * on_flush, int r) {
                    dout(15) << "r=" << r << dendl;
                    if (r < 0) {
                        derr << "error flushing local replay: " <<
                            cpp_strerror(r) << dendl;
                        on_flush->complete(r);
                        return;
                    }
                ...
                template < typename I >
                    void Replayer <
                    I >::flush_commit_position(Context * on_flush) {
                    std::unique_lock locker {
                    m_lock};
                    if (m_state != STATE_REPLAYING) {
                        locker.unlock();
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_flush_commit_position(Context * on_flush,
                                                      int r) {
                    dout(15) << "r=" << r << dendl;
                    if (r < 0) {
                        derr <<
                ...
                template < typename I >
                    void Replayer < I >::handle_replay_error(int r,
                                                             const std::
                                                             string & error) {
                    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
                ...
                template < typename I >
                    bool Replayer <
                    I >::is_replay_complete(const std::unique_lock <
                                            ceph::mutex > &)const {
                    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
                ...
                template < typename I >
                    void Replayer <
                    I >::handle_replay_complete(const std::unique_lock <
                                                ceph::mutex > &, int r,
                    ...
                    if (r < 0) {
                        derr << "replay encountered an error: " <<
                            cpp_strerror(r) << dendl;
                ...
                template < typename I >
                    void Replayer < I >::handle_replay_ready(std::unique_lock <
                                                             ceph::mutex >
                                                             &locker) {
                    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
                    ...
                    if (!m_state_builder->remote_journaler->
                        try_pop_front(&m_replay_entry, &m_replay_tag_tid)) {
                    ...
                    if (!m_replay_tag_valid
                        || m_replay_tag.tid != m_replay_tag_tid) {
                ...
                template < typename I > void Replayer < I >::replay_flush() {
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_replay_flush_shut_down > (this);
                    ceph_assert(m_local_journal_replay != nullptr);
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_replay_flush > (this);
                    m_local_journal->
                        start_external_replay(&m_local_journal_replay, ctx);
                }
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        derr << "replay flush encountered an error: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_complete(locker, r,
                ...
                template < typename I > void Replayer < I >::get_remote_tag() {
                    ...
                    Context *ctx = create_context_callback <
                        Replayer,
                        &Replayer < I >::handle_get_remote_tag > (this);
                    m_state_builder->remote_journaler->get_tag(m_replay_tag_tid,
                                                               &m_replay_tag,
                                                               ctx);
                }
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        derr << "failed to retrieve remote tag " <<
                            m_replay_tag_tid << ": " << cpp_strerror(r) <<
                            dendl;
                ...
                template < typename I >
                    ...
                    if (mirror_uuid == librbd::Journal <>::LOCAL_MIRROR_UUID) {
                        ...
                        if (local_tag_data.mirror_uuid ==
                            ...
                            dout(15) << "skipping stale demotion event" <<
                                dendl;
                            handle_process_entry_safe(m_replay_entry,
                                                      m_replay_bytes,
                            ...
                            dout(5) << "encountered image demotion: stopping" <<
                                dendl;
                    ...
                    librbd::journal::
                        TagPredecessor predecessor(m_replay_tag_data.
                                                   predecessor);
                    ...
                    Context *ctx = create_context_callback <
                        Replayer,
                        &Replayer < I >::handle_allocate_local_tag > (this);
                    m_local_journal->allocate_tag(mirror_uuid, predecessor,
                                                  ctx);
                }
                ...
                template < typename I >
                    ...
                    dout(15) << "r=" << r << ", "
                        ...
                        get_tag_tid() << dendl;
                    if (r < 0) {
                        derr << "failed to allocate journal tag: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_complete(r,
                ...
                template < typename I > void Replayer < I >::preprocess_entry() {
                    dout(20) << "preprocessing entry tid=" << m_replay_entry.
                        get_commit_tid()
                        << dendl;
                    ...
                    uint32_t delay =
                        calculate_replay_delay(m_event_entry.timestamp,
                                               m_state_builder->
                                               local_image_ctx->
                    ...
                    dout(20) << "delaying replay by " << delay << " sec" <<
                        dendl;
                    std::unique_lock timer_locker {
                    m_threads->timer_lock};
                    ceph_assert(m_delayed_preprocess_task == nullptr);
                    m_delayed_preprocess_task = create_context_callback <
                        Replayer < I >,
                        &Replayer < I >::handle_delayed_preprocess_task >
                        (this);
                    m_threads->timer->add_event_after(delay,
                ...
                template < typename I >
                    ...
                    Context *ctx = create_context_callback <
                        Replayer,
                        &Replayer < I >::handle_preprocess_entry_safe > (this);
                ...
                template < typename I >
                    ...
                    if (r < 0) {
                        if (r == -ECANCELED) {
                            ...
                            derr << "failed to preprocess journal event" <<
                                dendl;
                            handle_replay_complete(r,
                ...
                template < typename I > void Replayer < I >::process_entry() {
                    dout(20) << "processing entry tid=" << m_replay_entry.
                        get_commit_tid()
                        << dendl;

                    Context *on_ready = create_context_callback <
                        Replayer,
                        &Replayer < I >::handle_process_entry_ready > (this);
                    Context *on_commit =
                        new C_ReplayCommitted(this, std::move(m_replay_entry),
                                              m_replay_bytes,
                                              m_replay_start_time);

                    m_local_journal_replay->process(m_event_entry, on_ready,
                                                    on_commit);
                }
                ...
                template < typename I >
                    void Replayer <
                    I >::
                    handle_process_entry_safe(const ReplayEntry & replay_entry,
                                              uint64_t replay_bytes,
                                              const utime_t & replay_start_time,
                                              int r) {
                    dout(20) << "commit_tid=" << replay_entry.
                        get_commit_tid() << ", r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to commit journal event: " <<
                            cpp_strerror(r) << dendl;
                        handle_replay_complete(r,
                                               "failed to commit journal event");
                    }
                    else {
                        ceph_assert(m_state_builder->remote_journaler !=
                                    nullptr);
                        m_state_builder->remote_journaler->
                    ...
                    if (g_journal_perf_counters) {
                        g_journal_perf_counters->
                            inc(l_rbd_mirror_journal_entries);
                        g_journal_perf_counters->
                            inc(l_rbd_mirror_journal_replay_bytes,
                                replay_bytes);
                        g_journal_perf_counters->
                            tinc(l_rbd_mirror_journal_replay_latency, latency);
                    }

                    auto ctx = new LambdaContext([this, replay_bytes,
                                                  latency] (int r){
                                                 std::
                    ...
                    bool canceled_delayed_preprocess_task = false;
                    {
                        ...
                        if (m_delayed_preprocess_task != nullptr) {
                            ...
                            canceled_delayed_preprocess_task =
                                m_threads->timer->
                                cancel_event(m_delayed_preprocess_task);
                ...
                template < typename I >
                    int Replayer <
                    I >::validate_remote_client_state(const cls::journal::
                    ...
                    if (!util::
                        decode_client_meta(remote_client, remote_client_meta)) {
                    ...
                    dout(5) << "image_id=" << local_image_ctx->id << ", "
                        ...
                        << remote_client_meta->image_id << ", "
                        << "remote_client.state=" << remote_client.
                        state << dendl;
                    if (remote_client_meta->image_id == local_image_ctx->id
                        ...
                        if (local_image_ctx->config.template get_val < bool >
                            ...
                            dout(10) << "disconnected: automatic resync" <<
                                dendl;
                ...
                template < typename I >
                    ...
                    std::string labels =
                        ceph::perf_counters::
                        key_create("rbd_mirror_journal_image", { {
                                   "pool",
                    ...
                    plb.add_u64_counter(l_rbd_mirror_journal_entries, "entries",
                                        "Number of entries replayed", nullptr,
                                        prio);
                    plb.add_u64_counter(l_rbd_mirror_journal_replay_bytes,
                                        "replay_bytes", "Total bytes replayed",
                                        nullptr, prio, unit_t(UNIT_BYTES));
                    plb.add_time_avg(l_rbd_mirror_journal_replay_latency,
                                     "replay_latency", "Replay latency",
                                     nullptr, prio);
...
template class rbd::mirror::image_replayer::journal::Replayer <
    librbd::ImageCtx >;
