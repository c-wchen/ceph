...
const Option *md_config_t::find_option(const std::string_view name) const const
{
                              ...
                              config_callback config_cb)
{
    ...
    for (auto &i : kv) {
        if (config_cb) {
            ...
            ldout(cct,
                  4) << __func__ << " callback ignored " << i.first << dendl;
        }
    ...
    for (const auto &opt_iter : schema) {
        ...
        if (ceph_argparse_witharg(args, i, &val, err,
            ...
            if (ceph_argparse_binary_flag(args, i, &res, oss, as_option.c_str(),
                ...
                if (res == 0)
                    ret =
                        _set_val(values, tracker, "false", opt, level,
                                 &error_message);
                else if (res == 1)
                    ret =
                                        ...
                                        bool emeta) const const
{
    ...
    if (emeta) {
        expand_stack_t stack;
        auto v =
            _expand_meta(values, Option::value_t(out), nullptr, &stack,
                         nullptr);
                          ...
                          int level, std::string *error_message)
{
    ...
    if (!opt.can_update_at_runtime() &&
        safe_to_start_threads && !observers.is_tracking(opt.name)) {
                         ...
                         const std::string_view key, int level)
{
    ...
    auto ret = values.rm_val(std::string { key }
                             , level);
    if (ret < 0) {
        return ret;
    }
