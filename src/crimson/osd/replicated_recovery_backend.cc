...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::recover_object(
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::maybe_push_shards(
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::maybe_pull_missing_obj(
    const hobject_t &soid,
    eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::push_delete(
    const hobject_t &soid,
    eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::handle_recovery_delete(
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::local_recover_delete(
    const hobject_t &soid,
    eversion_t need,
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::recover_delete(
...
RecoveryBackend::interruptible_future < PushOp > ReplicatedRecoveryBackend::prep_push_to_replica(
    const hobject_t &soid,
    eversion_t need,
    pg_shard_t pg_shard)
{
...
RecoveryBackend::interruptible_future < PushOp > ReplicatedRecoveryBackend::prep_push(
    const hobject_t &soid,
    eversion_t need,
    pg_shard_t pg_shard,
...
RecoveryBackend::interruptible_future < bool > ReplicatedRecoveryBackend::_handle_pull_response(
    pg_shard_t from,
    PushOp& push_op,
    PullOp* response,
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::handle_pull_response(
    Ref < MOSDPGPush > m)
{
    if (pg.can_discard_replica_op(*m)) {
        logger().debug("{}: discarding {}", __func__, *m);
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::_handle_push(
    pg_shard_t from,
    PushOp &push_op,
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::handle_push(
    Ref < MOSDPGPush > m)
{
    if (pg.can_discard_replica_op(*m)) {
        logger().debug("{}: discarding {}", __func__, *m);
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::handle_push_reply(
...
std::pair < interval_set < uint64_t >,
    bufferlist > ReplicatedRecoveryBackend::trim_pushed_data(
        const interval_set < uint64_t > &copy_subset,
...
RecoveryBackend::interruptible_future < > ReplicatedRecoveryBackend::handle_recovery_op(
