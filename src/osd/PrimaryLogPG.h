...
class PrimaryLogPG:public PG, public PGBackend::Listener {
   ...
   */
    ...
    struct ProxyReadOp {
        ...
        :op(_op), soid(oid),
            ...
            user_version(0), data_offset(0), canceled(false) {
    }};
    ...
    } std::string gen_dbg_prefix() const override {
        return gen_prefix();
    ...
    void log_operation(const vector < pg_log_entry_t > &logv,
    ...
    void schedule_recovery_work(GenContext <
                                ThreadPool::TPHandle & >*c) override;
    ...
    void send_message_osd_cluster(Message * m, Connection * con) override;
    ...
    LogClientTemp clog_error() override {
        return osd->clog->error();
    }
    LogClientTemp clog_warn() override {
    ...
    struct OpContext {
      ...
      OpContext(OpRequestRef _op, osd_reqid_t _reqid, vector < OSDOp > *_ops, ObjectContextRef & obc, PrimaryLogPG * _pg):
        op(_op), reqid(_reqid), ops(_ops),
            ...
            cache_evict(false), ignore_cache(false), ignore_log_op_stats(false),
            update_log_only(false), bytes_written(0), bytes_read(0),
            user_at_version(0), current_osd_subop_num(0), obc(obc), reply(NULL),
            pg(_pg), num_read(0), num_write(0), sent_reply(false),
            inflightreads(0), lock_type(ObjectContext::RWState::RWNONE) {
            if (obc->ssc) {
      ...
      OpContext(OpRequestRef _op, osd_reqid_t _reqid, vector < OSDOp > *_ops, PrimaryLogPG * _pg):
        op(_op), reqid(_reqid), ops(_ops), obs(NULL), snapset(0),
            ...
            cache_evict(false), ignore_cache(false), ignore_log_op_stats(false),
            update_log_only(false), bytes_written(0), bytes_read(0),
            user_at_version(0), current_osd_subop_num(0), reply(NULL), pg(_pg),
            num_read(0), num_write(0), inflightreads(0),
      ...
      public:
        ...
        RepGather(OpContext * c, ceph_tid_t rt,
            ...
            on_finish(std::move(c->on_finish)) {
                    ...
                    >&&on_complete, ceph_tid_t rt, eversion_t lc,
                    bool applies_with_commit, int r):op(o), queue_item(this),
            nref(1), r(r), rep_tid(rt), rep_aborted(false), rep_done(false),
            all_applied(false), all_committed(false),
            applies_with_commit(applies_with_commit),
            pg_local_last_complete(lc), lock_manager(std::move(manager)) {
            if (on_complete) {
   ...
   */
    ...
    RepGather *new_repop(OpContext * ctx,
  ...
  protected:
    ...
    SnapSetContext *get_snapset_context(const hobject_t & oid, bool can_create, const map < string, bufferlist > *attrs = 0, bool oid_existed = true    //indicate this oid whether exsited in backend
        );
    ...
    void dump_recovery_info(Formatter * f) const override {
        ...
        }
        {
            ...
            for (set < hobject_t >::const_iterator i =
        ...
        }
        {
            ...
            for (map < hobject_t, ObjectContextRef >::const_iterator i =
   ...
   */
    ...
    void scrub_snapshot_metadata(ScrubMap & map, const std::map < hobject_t,
                                 pair < boost::optional < uint32_t >,
                                 boost::optional <
                                 uint32_t >>> &missing_digest) override;
  ...
  public:
    ...
    int do_command(cmdmap_t cmdmap,
                   ostream & ss,
                   bufferlist & idata,
                   bufferlist & odata,
                   ConnectionRef conn, ceph_tid_t tid) override;
    ...
    void do_scan(OpRequestRef op, ThreadPool::TPHandle & handle) override;
  ...
  private:
    ...
    struct SnapTrimmer:public boost::statechart::state_machine < SnapTrimmer,
        ...
        bool can_trim() {
            return pg->is_clean() && !pg->scrubber.active
                && !pg->snap_trimq.empty();
        }
    }
    ...
    struct Trimming:boost::statechart::state < Trimming, SnapTrimmer,
        ...
        :my_base(ctx), NamedState(context < SnapTrimmer > ().pg, "Trimming") {
    ...
    struct WaitTrimTimer:boost::statechart::state < WaitTrimTimer, Trimming >,
        ...
        :my_base(ctx),
            NamedState(context < SnapTrimmer > ().pg,
            ...
            if (pg->cct->_conf->osd_snap_trim_sleep > 0) {
                ...
                wakeup =
                    pg->osd->snap_sleep_timer.add_event_after(pg->cct->_conf->
                                                              osd_snap_trim_sleep,
                                                              new OnTimer {
                                                              pg,
                                                              pg->get_osdmap()->
  ...
  public:
    ...
    void mark_all_unfound_lost(int what, ConnectionRef con, ceph_tid_t tid);
    ...
    void setattrs_maybe_cache(ObjectContextRef obc,
                              ...
                              PGTransaction * t,
                              map < string, bufferlist > &attrs);
    void rmattr_maybe_cache(ObjectContextRef obc,
                            OpContext * op,
                            ...
                            const PrimaryLogPG::RepGather & repop)
{
    out << "repgather(" << &repop
        ...
        << " applied?=" << repop.all_applied << " r=" << repop.r << ")";
    return out;
}
