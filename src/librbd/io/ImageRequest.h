...
namespace librbd {
    ...
    namespace io {
         ...
         template < typename ImageCtxT = ImageCtx > class ImageRequest {
          public:
            ...
            static ImageRequest *create_writesame_request(ImageCtxT & image_ctx,
                                                          ...
                                                          const ZTracer::
                                                          Trace & parent_trace);
            static ImageRequest *create_compare_and_write_request(ImageCtxT &
                                                                  image_ctx,
                                                                  AioCompletion
                                                                  * c, Extents
                                                                  &&
            ...
            static void aio_write(ImageCtxT * ictx, AioCompletion * c, Extents
            ...
            static void aio_compare_and_write(ImageCtxT * ictx,
            ...
            :m_image_ctx(image_ctx), m_aio_comp(aio_comp),
                ...
                m_trace(util::
                        create_trace(image_ctx, trace_name, parent_trace)) {
                m_trace.event("start");
            }
      ...
      template < typename ImageCtxT = ImageCtx > class AbstractImageWriteRequest:public ImageRequest < ImageCtxT >
        {
          ...
          protected:
            ...
            AbstractImageWriteRequest(ImageCtxT & image_ctx,
                                      AioCompletion * aio_comp, Extents
                                      && image_extents, const char *trace_name,
                                      const ZTracer::Trace & parent_trace)
            :ImageRequest < ImageCtxT > (image_ctx, aio_comp,
                                         std::move(image_extents), trace_name,
                                         parent_trace), m_synchronous(false) {
            }
            ...
            virtual uint32_t get_object_cache_request_count() const {
                return 0;
            } virtual void send_object_cache_requests(const ObjectExtents &
                                                      object_extents,
                                                      uint64_t journal_tid) = 0;

            virtual void send_object_requests(const ObjectExtents &
                                              object_extents,
                                              const::SnapContext & snapc,
                                              ObjectRequests * object_requests);
            virtual ObjectRequestHandle *create_object_request(const
                                                               ObjectExtent &
                                                               object_extent,
                                                               const::
                                                               SnapContext &
            ...
            virtual uint64_t append_journal_event(const ObjectRequests &
                                                  requests, bool synchronous) =
                0;
      ...
      template < typename ImageCtxT = ImageCtx > class ImageWriteRequest:public AbstractImageWriteRequest < ImageCtxT >
        {
          ...
          protected:
            using typename ImageRequest < ImageCtxT >::ObjectRequests;
            using typename AbstractImageWriteRequest <
                ImageCtxT >::ObjectExtents;

            aio_type_t get_aio_type() const override {
                return AIO_TYPE_WRITE;
            } const char *get_request_type() const override {
                return "aio_write";
            } void assemble_extent(const ObjectExtent & object_extent,
                                   bufferlist * bl);

            void send_image_cache_request() override;

            void send_object_cache_requests(const ObjectExtents &
                                            object_extents,
            ...
            void send_object_requests(const ObjectExtents & object_extents,
                                      const::SnapContext & snapc,
                                      ObjectRequests *
                                      aio_object_requests) override;

            ObjectRequestHandle *create_object_request(const ObjectExtent &
                                                       object_extent,
                                                       const::
                                                       SnapContext & snapc,
                                                       Context *
      ...
      template < typename ImageCtxT = ImageCtx > class ImageWriteSameRequest:public AbstractImageWriteRequest < ImageCtxT >
        {
          public:
            ImageWriteSameRequest(ImageCtxT & image_ctx,
      ...
      template < typename ImageCtxT = ImageCtx > class ImageCompareAndWriteRequest:public AbstractImageWriteRequest < ImageCtxT >
        {
          public:
            ...
            ImageCompareAndWriteRequest(ImageCtxT & image_ctx,
                                        AioCompletion * aio_comp, Extents
                                        && image_extents, bufferlist
                                        && cmp_bl, bufferlist
                                        && bl, uint64_t * mismatch_offset,
...
extern template class librbd::io::AbstractImageWriteRequest <
    librbd::ImageCtx >;
...
extern template class librbd::io::ImageCompareAndWriteRequest <
    librbd::ImageCtx >;
