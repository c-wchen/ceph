...
aio_callback(cb),
aio_callback_priv(cbpriv), aio_stop(false), aio_thread(this), injecting_crash(0)
{
}
...
int KernelDevice::collect_metadata(string prefix, map < string, string > *pm) const const
{
    ...
    if (S_ISBLK(st.st_mode)) {
        ...
        int rc =
            get_device_by_fd(fd_buffered, partition_path, dev_node, PATH_MAX);
        switch (rc) {
        case -EOPNOTSUPP:
        case -EINVAL:
        ...
        default:
            {
                ...
                (*pm)[prefix + "model"] =
                    get_dev_property(dev_node, "device/model");
                ...
                string nvme_vendor =
                    get_dev_property(dev_node, "device/device/vendor");
            ...
            for (int i = 0; i < r; ++i) {
                ...
                if (r < 0) {
                    ...
                    if (ioc->allow_eio && is_expected_ioerr(r)) {
                        ...
                        assert(0 ==
                               "got unexpected error from aio_t::get_return_value. "
