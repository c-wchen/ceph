...
struct Option {
   ...
   */ enum level_t {
    ...
    const char *level_to_str(level_t l) const {
        ...
        boost::blank,
        std::string, uint64_t, int64_t, double, bool, entity_addr_t, uuid_d >;
   ...
   */
    ...
    template < typename T > using is_integer =
        ...
        && !std::is_same < T, bool >::value, int >;
    template < typename T, typename is_not_integer < T >::type =
        0 > Option & set_value(value_t & v, const T & new_value) {
        v = new_value;
        return *this;
    }
