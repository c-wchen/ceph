...
// reuse ImageCtx's thread pool
        ...
        template < typename I > struct C_IsTagOwner:public Context {
            ...
            void finish(int r) override {
                ...
                FunctionContext *ctx = new FunctionContext([journaler,
                                                            on_finish] (int r){
                                                           on_finish->
                                                           complete(r);
                                                           delete journaler;});
...
// mirror uuid to use for orphaned (demoted) images
  ...
  :    
    ...
    m_metadata_listener(this) {
        ...
        m_work_queue =
            new ContextWQ("librbd::journal::work_queue",
                          ...
                          ("rbd_op_thread_timeout"), thread_pool_singleton);
    ...
    } template < typename I > bool Journal < I >::is_tag_owner(const Mutex &)const {
        assert(m_lock.is_locked());
    ...
    template < typename I >
        uint64_t Journal < I >::append_write_event(uint64_t offset,
                                                   size_t length,
                                                   const bufferlist & bl,
                                                   const IOObjectRequests &
                                                   requests, bool flush_entry) {
        ...
        return append_io_events(journal::EVENT_TYPE_AIO_WRITE, bufferlists,
                                requests, offset, length, flush_entry, 0);
    }
    ...
    template < typename I >
      ...
      for (auto & bl:bufferlists) {
...

        {
            ...
            m_events[tid] =
                Event(futures, requests, offset, length, filter_ret_val);
        }
    ...
    template < typename I >
        typename Journal < I >::Future Journal < I >::wait_event(Mutex & lock,
                                                                 uint64_t tid,
                                                                 Context *
                                                                 on_safe) {
    ...
    template < typename I >
        ...
        on_start = new FunctionContext([this, journal_replay, on_start] (int r) {
    ...
    template < typename I > void Journal < I >::create_journaler() {
        ...
        // disconnected.
        settings.whitelisted_laggy_clients = {
        IMAGE_CLIENT_ID};
    ...
    template < typename I > void Journal < I >::handle_replay_complete(int r) {
        ...
        ctx = new FunctionContext([this, cct, cancel_ops, ctx] (int r) {
                                  ...
                                  m_journal_replay->shut_down(cancel_ops,
                                                              ctx);});
    ...
    template < typename I >
        ...
        if (is_steady_state()) {
            Contexts wait_for_state_contexts(std::
                                             move(m_wait_for_state_contexts));
    ...
    template < typename I > void Journal < I >::handle_metadata_updated() {
        ...
        refresh_ctx->on_finish =
            new FunctionContext([this, refresh_sequence, refresh_ctx] (int r) {
