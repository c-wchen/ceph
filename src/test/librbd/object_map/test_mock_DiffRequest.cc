    ...
    namespace object_map {
        ...
        class TestMockObjectMapDiffRequest:public TestMockFixture {
          public:
            ...
            void SetUp() override {
            ...
            } void expect_get_flags(MockTestImageCtx & mock_image_ctx,
                                    uint64_t snap_id, int32_t flags, int r) {
                EXPECT_CALL(mock_image_ctx, get_flags(snap_id, _))
                    .WillOnce(WithArg < 1 >
                              (Invoke([flags, r] (uint64_t * out_flags) {
                                      *out_flags = flags; return r;}
                               )));
            } template < typename Lambda >
                void expect_load_map(MockTestImageCtx & mock_image_ctx,
                                     uint64_t snap_id,
                ...
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    ...
                    .WillOnce(WithArg < 5 >
                              (Invoke
                               ([object_map, r, lambda = std::move(lambda)]
                                (bufferlist * out_bl) {
                                lambda(); auto out_object_map {
                                object_map};
                                out_object_map.set_crc_enabled(false);
                                encode(out_object_map, *out_bl); return r;}
                               )));
            }
        ...
        TEST_F(TestMockObjectMapDiffRequest, LoadObjectMapDNE) {
            ...
            BitVector < 2 > object_map_head;
            expect_load_map(mock_image_ctx, CEPH_NOSNAP, object_map_head,
                            -ENOENT);
