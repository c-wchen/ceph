...
template < template < typename > class Allocator > using xattr_map = std::map < alloc_string < Allocator >, ceph::bufferptr, std::less < alloc_string < Allocator >>, Allocator < std::pair < const alloc_string < Allocator >, ceph::bufferptr >>>;    // FIXME bufferptr not in mempool

template < template < typename > class Allocator >
    inline void decode_noshare(xattr_map < Allocator > &xattrs,
...
template < template < typename > class Allocator >
    void old_inode_t < Allocator >::encode(ceph::buffer::list & bl,
...
template < template < typename > class Allocator >
    void old_inode_t <
    Allocator >::decode(ceph::buffer::list::const_iterator & bl)
{
...
template < template < typename > class Allocator >
    void old_inode_t < Allocator >::dump(ceph::Formatter * f) const const
{
...
template < template < typename > class Allocator >
    inline void encode(const old_inode_t < Allocator >
                       &c,::ceph::buffer::list & bl, uint64_t features)
{
    ENCODE_DUMP_PRE();
...
template < template < typename > class Allocator >
    inline void decode(old_inode_t < Allocator >
                       &c,::ceph::buffer::list::const_iterator & p)
{
    c.decode(p);
}
  ...
  public:
    ...
    bool test(size_t bit) const {
        if (bit >= bits_per_block * _vec.size())
            return false;
        return _vec[bit /
                    bits_per_block] & ((block_type) 1 << (bit %
                                                          bits_per_block));
...
struct metric_spec_t {
    metric_spec_t() {
    } metric_spec_t(const metric_spec_t & other):metric_flags(other.
                                                              metric_flags) {
    }
  metric_spec_t(metric_spec_t && other):
    metric_flags(std::move(other.metric_flags)) {
    }
    metric_spec_t(const feature_bitset_t & mf):metric_flags(mf) {
    }
...
struct client_metadata_t {
    ...
    client_metadata_t() {
                        ...
                        const metric_spec_t & mst):kv_map(kv), features(f),
        metric_spec(mst) {
    }
...
struct session_info_t {
    ...
    } bool has_feature(size_t bit) const {
...
static void decode_helper(ceph::buffer::list::const_iterator & bl,
                          std::string & nm, snapid_t & sn) {
    std::string key;
    using ceph::decode;
    decode(key, bl);
...
// cap info for client reconnect struct cap_reconnect_t {
    cap_reconnect_t() {
    } cap_reconnect_t(uint64_t cap_id, inodeno_t pino, std::string_view p,
                      int w, int i, inodeno_t sr, snapid_t sf,
                      ceph::buffer::list & lb):path(p) {
...
class inode_load_vec_t {
  public:
    ...
    inode_load_vec_t():vec {
    DecayCounter(DecayRate()), DecayCounter(DecayRate())} {
    }
    inode_load_vec_t(const DecayRate & rate):vec {
...
class dirfrag_load_vec_t {
  public:
    ...
    dirfrag_load_vec_t(const DecayRate & rate):vec {
    DecayCounter(rate), DecayCounter(rate), DecayCounter(rate),
            DecayCounter(rate), DecayCounter(rate)}
    {
    }
...
inline void decode(dirfrag_load_vec_t & c,
                   ceph::buffer::list::const_iterator & p)
{
    c.decode(p);
}
...
{
    CachedStackStringStream css;
    *css << std::setprecision(1) << std::fixed << "[pop" " IRD:" << dl.vec[0]
        << " IWR:" << dl.vec[1]
        << " RDR:" << dl.vec[2]
        << " FET:" << dl.vec[3]
        << " STR:" << dl.vec[4]
