...
class MMgrBeacon final:public PaxosServiceMessage {
  ...
  public:
     MMgrBeacon()
    :PaxosServiceMessage {
    MSG_MGR_BEACON, 0, HEAD_VERSION, COMPAT_VERSION}, gid(0), available(false) {
    }
    ...
    MMgrBeacon(const uuid_d & fsid_, uint64_t gid_, const std::string & name_,
               ...
               uint64_t feat)
    :PaxosServiceMessage {
    MSG_MGR_BEACON, 0, HEAD_VERSION, COMPAT_VERSION},
        gid(gid_), server_addrs(server_addrs_), available(available_),
        name(name_), fsid(fsid_), modules(std::move(modules_)),
        metadata(std::move(metadata_)), clients(std::move(clients_)),
        mgr_features(feat) {
    }
    ...
    uint64_t get_gid() const {
        ...
        return services;
    } uint64_t get_mgr_features() const {
        return mgr_features;
