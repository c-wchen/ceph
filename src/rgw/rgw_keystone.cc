...
#define END_CMS "-----END CMS-----"
    ...
    if (start < 0) {
        ldout(cct,
              0) << "failed to find " << BEGIN_CMS << " in response" << dendl;
        return -EINVAL;
    }
    ...
    if (end < 0) {
        ldout(cct,
              0) << "failed to find " << END_CMS << " in response" << dendl;
        return -EINVAL;
    }
...
int rgw_decode_b64_cms(CephContext * const cct,
                       const string & signed_b64, bufferlist & bl)
{
...
bool rgw_decode_pki_token(CephContext * const cct,
                          const string & token, bufferlist & bl)
{
    if (!rgw_is_pki_token(token)) {
        return false;
    }
...
namespace rgw {
    namespace keystone {
        ...
        void TokenCache::invalidate(const std::string & token_id) {
            ...
            ldout(cct,
                  20) << "invalidating revoked token id=" << token_id << dendl;
            token_entry & e = iter->second;
            tokens_lru.erase(e.lru_iter);
            tokens.erase(iter);
        }
        ...
        int TokenCache::RevokeThread::check_revoked() {
            ...
            if (rgw::keystone::Service::
                get_admin_token(cct, *cache, config, token) < 0) {
                return -EINVAL;
            }
            ...
            if (iter.end()) {
                ldout(cct,
                      0) << "revoked tokens response is missing signed section"
                    << dendl;
                return -EINVAL;
            }
            ...
            ldout(cct,
                  10) << "ceph_decode_cms: decoded: " << json.c_str() << dendl;
        ...
        bool TokenCache::going_down() const {
            ...
            do {
                ...
                if (r < 0) {
                    ldout(cct,
                          0) <<
                        "ERROR: keystone revocation processing returned error r="
                        << r << dendl;
                }
