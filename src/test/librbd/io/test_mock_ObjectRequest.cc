    ...
    namespace util {
        ...
        template <> struct CopyupRequest <librbd::MockImageCtx > {
            MOCK_METHOD0(send, void ());
             MOCK_METHOD2(append_request,
                          void (AbstractObjectWriteRequest <
                                librbd::MockTestImageCtx > *, const Extents &));
        };

         template <>
            struct CopyupRequest <librbd::
            MockTestImageCtx >:public CopyupRequest < librbd::MockImageCtx > {
        ...
        CopyupRequest < librbd::MockTestImageCtx > *CopyupRequest <
        ...
        template <> struct ImageListSnapsRequest <librbd::MockTestImageCtx > {
            ...
            SnapshotDelta *snapshot_delta;
             ...
             ImageListSnapsRequest() {
                s_instance = this;
            } ImageListSnapsRequest(librbd::MockImageCtx & image_ctx,
                                    AioCompletion * aio_comp, Extents
                                    && image_extents, ImageArea area, SnapIds
        ...
        ImageListSnapsRequest < librbd::MockTestImageCtx >
            *ImageListSnapsRequest < librbd::MockTestImageCtx >::s_instance =
            nullptr;

        namespace util {

            template <>
                void area_to_object_extents(MockTestImageCtx * image_ctx,
                                            uint64_t offset, uint64_t length,
                                            ImageArea area,
                                            uint64_t buffer_offset,
                                            striper::LightweightObjectExtents *
                                            object_extents) {
                Striper::file_to_extents(image_ctx->cct, &image_ctx->layout,
                                         offset, length, 0, buffer_offset,
                                         object_extents);
            } template <> std::pair < Extents,
                ImageArea > object_to_area_extents(MockTestImageCtx * image_ctx,
                                                   uint64_t object_no,
                                                   const Extents &
                                                   object_extents) {
                Extents extents;
              for (auto[off, len]:object_extents) {
                    Striper::extent_to_file(image_ctx->cct, &image_ctx->layout,
                                            object_no, off, len, extents);
                }
                return {
                std::move(extents), ImageArea::DATA};
            }
                    ...
                    static Mock *s_instance;
                     ...
                     Mock() {
                        s_instance = this;
                                   ...
                                   void (librbd::MockTestImageCtx *, uint64_t,
                                         ReadExtents *, librados::snap_t,
            ...
            template <> void read_parent(librbd::MockTestImageCtx * image_ctx,
                                         uint64_t object_no,
                                         ReadExtents * extents,
                                         librados::snap_t snap_id,
                                         const ZTracer::Trace & trace,
                                         Context * on_finish) {
                Mock::s_instance->read_parent(image_ctx, object_no, extents,
                                              snap_id, trace, on_finish);
            }
...
namespace librbd {
    namespace io {
        ...
        struct TestMockIoObjectRequest:public TestMockFixture {
            typedef ObjectRequest < librbd::MockTestImageCtx >
                MockObjectRequest;
            typedef ObjectReadRequest < librbd::MockTestImageCtx >
                MockObjectReadRequest;
            typedef ObjectWriteRequest < librbd::MockTestImageCtx >
                MockObjectWriteRequest;
            typedef ObjectDiscardRequest < librbd::MockTestImageCtx >
            ...
            typedef CopyupRequest < librbd::MockTestImageCtx >
                MockCopyupRequest;
            ...
            void expect_prune_parent_extents(MockTestImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(mock_image_ctx,
                            prune_parent_extents(_, _, overlap, _))
                    .WillOnce(WithArg < 0 >
                              (Invoke([extents, object_overlap] (Extents & e) {
                                      e = extents; return object_overlap;}
                               )));
            }
            ...
            void expect_is_lock_owner(MockExclusiveLock & mock_exclusive_lock) {
                EXPECT_CALL(mock_exclusive_lock,
            ...
            void expect_read(MockTestImageCtx & mock_image_ctx,
                             ...
                             uint64_t len, const std::string & data, int r) {
                bufferlist bl;
                bl.append(data);

                auto & mock_io_ctx =
                    librados::get_mock_io_ctx(mock_image_ctx.rados_api,
                                              *mock_image_ctx.
                                              get_data_io_context());
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, read(oid, len, off, _, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
                    expect.WillOnce(WithArg < 3 >
                                    (Invoke([bl] (bufferlist * out_bl) {
                                            out_bl->append(bl);
                                            return bl.length();}
                                     )));
                }
            ...
            void expect_read_parent(MockUtils & mock_utils, uint64_t object_no,
                ...
                EXPECT_CALL(mock_utils,
                    ...
                    .WillOnce(WithArg < 5 >
                              (CompleteContext
                               (r,
                                static_cast < asio::ContextWQ * >(nullptr))));
            }
            ...
            void expect_object_map_update(MockTestImageCtx & mock_image_ctx,
                                          ...
                                          uint64_t end_object, uint8_t state,
                                          const boost::optional < uint8_t >
                                          &current_state, bool updated,
                                          int ret_val) {
                if (mock_image_ctx.object_map != nullptr) {
                    EXPECT_CALL(*mock_image_ctx.object_map,
                                aio_update(CEPH_NOSNAP, start_object,
                                           end_object, state, current_state, _,
                                           false, _))
                        .WillOnce(WithArg < 7 >
                                  (Invoke
                                   ([&mock_image_ctx, updated,
                                     ret_val] (Context * ctx) {
                                    if (updated) {
                                    mock_image_ctx.op_work_queue->queue(ctx,
                                                                        ret_val);}
                                    return updated;}
                                   ))) ;
                }
                ...
                auto & mock_io_ctx =
                    librados::get_mock_io_ctx(mock_image_ctx.rados_api,
                                              *mock_image_ctx.
                                              get_data_io_context());
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, write(_, _, length, offset, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_create(MockTestImageCtx & mock_image_ctx,
                               bool exclusive) {
                auto & mock_io_ctx =
                    librados::get_mock_io_ctx(mock_image_ctx.rados_api,
                                              *mock_image_ctx.
            ...
            void expect_truncate(MockTestImageCtx & mock_image_ctx, int offset,
                                 int r) {
                auto & mock_io_ctx =
                    librados::get_mock_io_ctx(mock_image_ctx.rados_api,
                                              *mock_image_ctx.
                                              get_data_io_context());
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, truncate(_, offset, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_zero(MockTestImageCtx & mock_image_ctx, int offset,
                             int length, int r) {
                auto & mock_io_ctx =
                    librados::get_mock_io_ctx(mock_image_ctx.rados_api,
                                              *mock_image_ctx.
                                              get_data_io_context());
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, zero(_, offset, length, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_cmpext(MockTestImageCtx & mock_image_ctx, int offset,
                               int r) {
                auto & mock_io_ctx =
                    librados::get_mock_io_ctx(mock_image_ctx.rados_api,
                                              *mock_image_ctx.
                                              get_data_io_context());
                auto & expect =
                    EXPECT_CALL(mock_io_ctx, cmpext(_, offset, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_image_list_snaps(MockImageListSnapsRequest & req,
                                         ...
                                         const SnapshotDelta &
                                         image_snapshot_delta, int r) {
                EXPECT_CALL(req, execute_send())
                    .WillOnce(Invoke([&req, image_extents, image_snapshot_delta,
                                      r] () {
                                     ASSERT_EQ(image_extents,
                                               req.image_extents);
        ...
        TEST_F(TestMockIoObjectRequest, Read) {
            ...
            auto req =
                MockObjectReadRequest::create(&mock_image_ctx, 0, &extents,
                                              mock_image_ctx.
        ...
        TEST_F(TestMockIoObjectRequest, SparseReadThreshold) {
            ...
            expect_sparse_read(mock_image_ctx, ictx->get_object_name(0), 0,
                               ...
                               std::string(ictx->sparse_read_threshold_bytes,
                                           '1'), 0);
        ...
        TEST_F(TestMockIoObjectRequest, ParentRead) {
            ...
            int order = 0;
            ASSERT_EQ(0,
                      rbd.clone(m_ioctx, m_image_name.c_str(), "one", m_ioctx,
        ...
        TEST_F(TestMockIoObjectRequest, CopyOnRead) {
            ...
            expect_prune_parent_extents(mock_image_ctx, { {
                                        0, 4096}
                                        }
                                        , 4096, 4096);
        ...
        TEST_F(TestMockIoObjectRequest, Write) {
            ...
            expect_object_may_exist(mock_image_ctx, 0, true);
            expect_object_map_update(mock_image_ctx, 0, 1, OBJECT_EXISTS, {
                                     }
                                     , false, 0);
            ...
            C_SaferCond ctx;
            auto req =
                MockObjectWriteRequest::create_write(&mock_image_ctx, 0, 0,
                                                     std::move(bl),
                                                     mock_image_ctx.
                                                     get_data_io_context(),
                                                     0, 0, std::nullopt, { }
        ...
        TEST_F(TestMockIoObjectRequest, DiscardRemove) {
            ...
            expect_get_parent_overlap(mock_image_ctx, CEPH_NOSNAP, 0, 0);
            expect_object_may_exist(mock_image_ctx, 0, true);
            expect_object_map_update(mock_image_ctx, 0, 1, OBJECT_PENDING, {
                                     }
                                     , false, 0);
            ...
            C_SaferCond ctx;
            auto req =
                MockObjectDiscardRequest::create_discard(&mock_image_ctx, 0, 0,
                                                         mock_image_ctx.
                                                         get_object_size(),
                                                         mock_image_ctx.
                                                         get_data_io_context(),
        ...
        TEST_F(TestMockIoObjectRequest, DiscardTruncate) {
            ...
            C_SaferCond ctx;
            auto req =
                MockObjectDiscardRequest::create_discard(&mock_image_ctx, 0, 1,
                                                         mock_image_ctx.
                                                         get_object_size() - 1,
                                                         mock_image_ctx.
                                                         get_data_io_context(),
        ...
        TEST_F(TestMockIoObjectRequest, WriteSame) {
            ...
            C_SaferCond ctx;
            auto req =
                MockObjectWriteSameRequest::create_write_same(&mock_image_ctx,
                                                              0, 0, 4096,
                                                              std::move(bl),
                                                              mock_image_ctx.
                                                              get_data_io_context
        ...
        TEST_F(TestMockIoObjectRequest, CompareAndWrite) {
            ...
            auto req =
                MockObjectWriteSameRequest::
                create_compare_and_write(&mock_image_ctx, 0, 0,
        ...
        TEST_F(TestMockIoObjectRequest, ListSnaps) {
            ...
            SnapshotDelta snapshot_delta;
            C_SaferCond ctx;
            auto req = MockObjectListSnapsRequest::create(&mock_image_ctx, 0,
                                                          { {440320, 1024}
                                                          , {2122728, 1024}
                                                          , {2220032, 2048}
                                                          , {3072000, 4096}
            ...
            expected_snapshot_delta[ {
                     ...
                     SPARSE_EXTENT_STATE_DATA, 4096}
            );
            expected_snapshot_delta[ {
                                    5, 5}
            ].insert(3072000, 4096, {
                     SPARSE_EXTENT_STATE_ZEROED, 4096}
            );
        ...
        TEST_F(TestMockIoObjectRequest, ListSnapsENOENT) {
            ...
            expect_list_snaps(mock_image_ctx, {
                              }
                              , -ENOENT);
        ...
        TEST_F(TestMockIoObjectRequest, ListSnapsEmpty) {
            ...
            expected_snapshot_delta[ {
                     ...
                     SPARSE_EXTENT_STATE_ZEROED, 1024}
            );
            ASSERT_EQ(expected_snapshot_delta, snapshot_delta);
        }
        ...
        TEST_F(TestMockIoObjectRequest, ListSnapsWholeObject) {
            ...
            C_SaferCond ctx;
            auto req =
                MockObjectListSnapsRequest::create(&mock_image_ctx, 0,
                                                   { {0,
                                                      mock_image_ctx.layout.
                                                      object_size - 1}
                                                   }
                                                   ,
                                                   { 0, CEPH_NOSNAP }
            ...
            ].insert(0, mock_image_ctx.layout.object_size - 1, {
