...
namespace crimson::osd {
    ...
    void SnapTrimEvent::print(std::ostream & lhs) const {
        lhs << "SnapTrimEvent(" << "pgid=" << pg->get_pgid()
        << " snapid=" << snapid << " needs_pause=" << needs_pause << ")";
        ...
        return with_pg(pg->get_shard_services(), pg).finally([ref = IRef {
                                                              this}
                                                              , this] {
                                                             logger().
                                                             debug
    ...
    SnapTrimEvent::snap_trim_ertr::future < seastar::stop_iteration >
        SnapTrimEvent::with_pg(ShardServices & shard_services, Ref < PG > _pg) {
        return interruptor::with_interruption([&shard_services, this] {
                                              ...
                                              then_interruptible([&shard_services, this] {
                                                                 ...
                                                                 then_interruptible
                                                                 ([&shard_services, this] (const auto & to_trim) {
                                                                  if (to_trim.
                                                                      empty()) {
                                                                  // the legit ENOENT -> done
                                                                  logger().
                                                                  debug
                                                                  ("{}: to_trim is empty! Stopping iteration",
                                                                   *this);
                                                                  return
                                                                  snap_trim_iertr::
                                                                  make_ready_future
                  ...
                  for (const auto & object:                      to_trim)
                                              ...
                                              logger().
                                              debug("{}: interrupted {}", *this,
                                                    eptr);
                                              return crimson::ct_error::eagain::
                                              make();}
                                              , pg) ;
    }
    ...
    SnapTrimObjSubEvent::remove_or_update_iertr::future <>
        SnapTrimObjSubEvent::remove_clone(ObjectContextRef obc,
                                          ObjectContextRef head_obc,
                                          ceph::os::Transaction & txn,
                                          std::vector < pg_log_entry_t >
                                          &log_entries) {
        const auto p = std::find(head_obc->ssc->snapset.clones.begin(),
        ...
        if (p != head_obc->ssc->snapset.clones.begin()) {
            ...
            head_obc->ssc->snapset.clone_overlap[*n].intersection_of(head_obc->
                                                                     ssc->
                                                                     snapset.
                                                                     clone_overlap
                                                                     [*p]);
        ...
        log_entries.emplace_back(pg_log_entry_t {
                                 pg_log_entry_t::DELETE, coid, osd_op_p.at_version, obc->obs.oi.version, 0, osd_reqid_t(), obc->obs.oi.mtime,   // will be replaced in `apply_to()`
                                 0}
        );
        txn.remove(pg->get_collection_ref()->get_cid(), ghobject_t {
                   coid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD}
        );
        obc->obs.oi = object_info_t(coid);
        return OpsExecuter::snap_map_remove(coid, pg->snap_mapper, pg->osdriver,
                                            txn);
    }

    void SnapTrimObjSubEvent::remove_head_whiteout(ObjectContextRef obc,
                                                   ObjectContextRef head_obc,
                                                   ceph::os::Transaction & txn,
                                                   std::vector <
                                                   pg_log_entry_t >
        ...
        log_entries.emplace_back(pg_log_entry_t {
                                 pg_log_entry_t::DELETE, head_oid, osd_op_p.at_version, head_obc->obs.oi.version, 0, osd_reqid_t(), obc->obs.oi.mtime,  // will be replaced in `apply_to()`
                                 0}
        );
        ...
        txn.remove(pg->get_collection_ref()->get_cid(), ghobject_t {
                   head_oid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD}
        );
    }

    SnapTrimObjSubEvent::interruptible_future <>
        SnapTrimObjSubEvent::adjust_snaps(ObjectContextRef obc,
                                          ObjectContextRef head_obc,
        ...
        txn.setattr(pg->get_collection_ref()->get_cid(), ghobject_t {
                    coid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD},
                    OI_ATTR, bl);
        log_entries.emplace_back(pg_log_entry_t {
                                 ...
                                 0, osd_reqid_t(), obc->obs.oi.mtime, 0}
        );
        return OpsExecuter::snap_map_modify(coid, new_snaps, pg->snap_mapper,
                                            pg->osdriver, txn);
    }

    void SnapTrimObjSubEvent::update_head(ObjectContextRef obc,
                                          ObjectContextRef head_obc,
                                          ceph::os::Transaction & txn,
                                          std::vector < pg_log_entry_t >
                                          &log_entries) {
        ...
        txn.setattrs(pg->get_collection_ref()->get_cid(), ghobject_t {
                     head_oid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD},
                     attrs);
    }
    ...
    SnapTrimObjSubEvent::remove_or_update_iertr::future <
        ...
        SnapTrimObjSubEvent::remove_or_update(ObjectContextRef obc,
                                              ObjectContextRef head_obc) {
      ...
      for (const auto & old_snap:old_snaps) {
        ...
        return seastar::do_with(ceph::os::Transaction {
                                ...
                                int64_t num_objects_before_trim =
                                delta_stats.num_objects;
                                ...
                                ret =
                                remove_clone(obc, head_obc, txn, log_entries);}
                                else {
                                ...
                                logger().info("{}: {} snaps {} -> {}",
                                              *this, coid, old_snaps,
                                              new_snaps);
                                ret =
                                adjust_snaps(obc, head_obc, new_snaps, txn,
                                             log_entries);}
                                return std::move(ret).
                                safe_then_interruptible([&txn, obc,
                                ...
                                ).safe_then_interruptible([&txn, log_entries =
                                                           std::
                                                           move(log_entries)]
                                                          ()mutable {
                                                          return
    ...
    SnapTrimObjSubEvent::remove_or_update_iertr::future <>
        SnapTrimObjSubEvent::with_pg(ShardServices & shard_services,
                                     Ref < PG > _pg) {
        return enter_stage < interruptor >
            (pp().wait_for_active).then_interruptible([this] {
                             ...
                             return enter_stage < interruptor > (pp().get_obc);}
        ).then_interruptible([this] {
                             logger().debug("{}: getting obc for {}", *this,
                                            coid);
                             // end of commonality
                             // with_head_and_clone_obc lock both clone's and head's obcs
                             return pg->obc_loader.with_head_and_clone_obc <
                             RWState::RWWRITE > (coid,
                                                 [this] (auto head_obc,
                                                         auto clone_obc) {
                                                 logger().
                                                 debug("{}: got clone_obc={}",
                             ...
                             handle_error_interruptible(remove_or_update_iertr::
                                                        pass_further {
                                                        }
                                                        ,
                                                        crimson::ct_error::
