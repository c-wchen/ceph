...
TEST(CrushWrapper, get_immediate_parent)
{
...

    {
        ...
        EXPECT_EQ(0, c->insert_item(g_ceph_context, item, 1.0, "osd.0", loc));
    }
...
TEST(CrushWrapper, rename_bucket_or_item)
{
    ...
    item++;
    {
        ...
        EXPECT_EQ(0, c->insert_item(g_ceph_context, item, 1.0, "osd.1", loc));
    }
    ...
    EXPECT_EQ(-ENOENT,
              c->can_rename_item("doesnotexist", "somethingelse", &ss));
    EXPECT_EQ(-ENOENT, c->rename_item("doesnotexist", "somethingelse", &ss));
    EXPECT_EQ(-ENOENT,
              c->can_rename_bucket("doesnotexist", "somethingelse", &ss));
...
TEST(CrushWrapper, adjust_item_weight)
{
    ...
    item = 1;
    EXPECT_EQ(1,
              c->adjust_item_weightf_in_loc(g_ceph_context, item,
                                            modified_weight, loc_two));
...
TEST(CrushWrapper, remove_item)
{
    ...
    float weight;
    EXPECT_FALSE(c->
                 check_item_loc(g_ceph_context, item_to_remove, loc, &weight));
}
...
TEST(CrushWrapper, dump_rules)
{
...

    {
        ...
        f->flush(ss);
        delete f;
        EXPECT_EQ((unsigned)0,
                  ss.str().
                  find
                  ("<rule><rule_id>0</rule_id><rule_name>NAME</rule_name>"));
    }
...
TEST(CrushWrapper, choose_args_compat)
{
    ...
    uint64_t features =
        CEPH_FEATURE_CRUSH_TUNABLES5 | CEPH_FEATURE_INCARNATION_2;
    ...
    // if the client is capable, encode choose_args
    {
        ...
        ASSERT_EQ(1u,
                  c_new.choose_args[caid].args[-1 - id].weight_set_positions);
        ASSERT_EQ(weights,
                  c_new.choose_args[caid].args[-1 -
                                               id].weight_set[0].weights[0]);
...
TEST(CrushWrapper, device_class_clone)
{
    ...
    ASSERT_EQ(c.device_class_clone(12345, cl, old_class_bucket, used_ids,
                                   &other_clone_id, &cmap_item_weight),
              -ECHILD);
    ASSERT_EQ(c.
              device_class_clone(root_id, 12345, old_class_bucket, used_ids,
...
TEST(CrushWrapper, split_id_class)
{
    ...
    int clone_id;
    ASSERT_EQ(c.
              device_class_clone(item_id, class_id, old_class_bucket, used_ids,
                                 &clone_id, &cmap_item_weight), 0);
    int retrieved_item_id;
    int retrieved_class_id;
    ASSERT_EQ(c.
              split_id_class(clone_id, &retrieved_item_id, &retrieved_class_id),
              0);
    ASSERT_EQ(item_id, retrieved_item_id);
    ASSERT_EQ(class_id, retrieved_class_id);

    ASSERT_EQ(c.
              split_id_class(item_id, &retrieved_item_id, &retrieved_class_id),
              0);
    ASSERT_EQ(item_id, retrieved_item_id);
