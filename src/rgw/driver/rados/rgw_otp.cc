...
class RGWOTPMetadataObject:public RGWMetadataObject {
  ...
  public:
    ...
    } RGWOTPMetadataObject(otp_devices_list_t
                           && _devices, const obj_version & v,
                           const real_time m) {
...
class RGWOTPMetadataHandler:public RGWOTPMetadataHandlerBase {
    ...
    int init(RGWSI_Zone * zone, RGWSI_MetaBackend * _meta_be, RGWSI_OTP * _otp) {
        ...
        return be_handler->call([&](RGWSI_MetaBackend_Handler::Op * op) {
                                RGWSI_OTP_BE_Ctx ctx(op->ctx()); return f(ctx);}
        );
    }
    ...
    int do_put(RGWSI_MetaBackend_Handler::Op * op, string & entry,
        ...
        RGWOTPMetadataObject *obj =
            static_cast < RGWOTPMetadataObject * >(_obj);
                        ...
                        const GetParams & params)
{
    ...
    return meta_handler->call([&](RGWSI_OTP_BE_Ctx & ctx) {
                              return svc.otp->read_all(ctx, uid, &info->devices,
                                                       params.mtime,
...
int RGWOTPCtl::store_all(const DoutPrefixProvider * dpp,
                         ...
                         optional_yield y, const PutParams & params)
{
    return meta_handler->call([&](RGWSI_OTP_BE_Ctx & ctx) {
                              return svc.otp->store_all(dpp, ctx, info.uid,
                                                        info.devices,
                                                        params.mtime,
                                                        params.objv_tracker,
                                                        y);}
...
int RGWOTPCtl::remove_all(const DoutPrefixProvider * dpp,
                          const rgw_user & uid,
                          optional_yield y, const RemoveParams & params)
{
    return meta_handler->call([&](RGWSI_OTP_BE_Ctx & ctx) {
                              return svc.otp->remove_all(dpp, ctx, uid,
                                                         params.objv_tracker,
                                                         y);}
    );
}
