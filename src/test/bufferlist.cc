...
TEST(BufferPtr, accessors)
{
    ...
    EXPECT_NE((void *)nullptr,
              (void *)static_cast < instrumented_bptr & >(ptr).get_raw());
...
TEST(BufferListIterator, copy)
{
    ...
    {
        bufferlist bl;
        EXPECT_THROW(bl.begin((unsigned)100).copy((unsigned)100, (char *)0),
                     buffer::end_of_buffer);
        const char *expected = "ABC";
        bl.append(expected);
    ...
    //
    {
        bufferlist bl;
        bufferlist dest;
        EXPECT_THROW(bl.begin((unsigned)100).copy((unsigned)100, dest),
                     buffer::end_of_buffer);
        const char *expected = "ABC";
        bl.append(expected);
        bl.begin(1).copy(2, dest);
...
TEST(BufferListIterator, copy_in)
{
    ...
    //
    {
        bufferlist bl;
        bl.append("XXX");
        EXPECT_THROW(bl.begin((unsigned)100).copy_in((unsigned)100, (char *)0),
                     buffer::end_of_buffer);
    ...
    //
    {
        ...
        bufferlist src;
        src.append("ABC");
        EXPECT_THROW(bl.begin((unsigned)100).copy_in((unsigned)100, src),
                     buffer::end_of_buffer);
    ...
    for (size_t step = 4; step <= 16384; step *= 4) {
        ...
        for (size_t r = 0; r < rounds; ++r) {
            ...
            for (auto iter = std::begin(src);
                 iter != std::end(src); iter = std::next(iter, step)) {
                bl.append(&*iter, step);
            }
        }
...
TEST(BufferList, crc32c_append_perf)
{
    ...
    [[maybe_unused]] int base_cached_adjusted =
...
TEST(BufferList, TestCopyAll)
{
    ...
    std::shared_ptr < unsigned char >big2((unsigned char *)malloc(BIG_SZ),
                                          free);
...
TEST(BufferList, InvalidateCrc)
{
    ...
    std::shared_ptr < unsigned char >big((unsigned char *)malloc(buffer_size),
                                         free);
    unsigned char c = 0;
