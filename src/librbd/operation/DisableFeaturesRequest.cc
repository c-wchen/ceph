...
namespace librbd {
    namespace operation {
        ...
        :Request < I > (image_ctx, on_finish, journal_op_tid),
            m_features(features), m_force(force) {
            ...
            CephContext *cct = image_ctx.cct;
             ...
             ldout(cct,
                   20) << this << " " << __func__ << ": features=" << m_features
                << dendl;
        ...
        template < typename I >
            ...
            image_ctx.state->
                prepare_lock(create_async_context_callback
                             (image_ctx,
                              create_context_callback < DisableFeaturesRequest <
                              I >,
                              &DisableFeaturesRequest <
                              I >::handle_prepare_lock > (this)));
        }

        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_prepare_lock(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;

            if (*result < 0) {
                lderr(cct) << "failed to lock image: " << cpp_strerror(*result)
                    << dendl;
        ...
        template < typename I >
            ...
            image_ctx.io_image_dispatcher->
                block_writes(create_context_callback < DisableFeaturesRequest <
                             I >,
                             &DisableFeaturesRequest <
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_block_writes(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::send_acquire_exclusive_lock(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct, 20) << this << " " << __func__ << dendl;

            {
                ...
                if (image_ctx.exclusive_lock != nullptr &&
                    ...
                    Context *ctx = create_context_callback <
                        ...
                        &DisableFeaturesRequest <
                        I >::handle_acquire_exclusive_lock > (this,
                                                              image_ctx.
                                                              exclusive_lock);
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_acquire_exclusive_lock(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            ...
            m_out_bl.clear();
            int r =
                image_ctx.md_ctx.aio_operate(RBD_MIRRORING, comp, &op,
                                             &m_out_bl);
            ceph_assert(r == 0);
            comp->release();
        }
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_get_mirror_mode(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result == 0) {
                ...
                *result =
                    cls_client::mirror_mode_get_finish(&it, &m_mirror_mode);
            }
        ...
        template < typename I >
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_get_mirror_image > (this);
            m_out_bl.clear();
            int r =
                image_ctx.md_ctx.aio_operate(RBD_MIRRORING, comp, &op,
                                             &m_out_bl);
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_get_mirror_image(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct, 20) << this << " " << __func__ << dendl;
            ...
            if (mirror_image.state == cls::rbd::MIRROR_IMAGE_STATE_ENABLED &&
                mirror_image.mode == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL
                && !m_force) {
                lderr(cct) <<
        ...
        template < typename I >
            ...
            Context *ctx = create_context_callback <
                DisableFeaturesRequest < I >,
                &DisableFeaturesRequest < I >::handle_disable_mirror_image >
                (this);

            mirror::DisableRequest < I > *req =
                mirror::DisableRequest < I >::create(&image_ctx, m_force, true,
                                                     ctx);
            req->send();
        }
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_disable_mirror_image(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result < 0) {
                lderr(cct) << "failed to disable image mirroring: " <<
                    cpp_strerror(*result)
                    << dendl;
                // not fatal
            }
        ...
        template < typename I >
...

            {
                ...
                if (image_ctx.journal != nullptr) {
                    ...
                    Context *ctx = create_context_callback <
                        DisableFeaturesRequest < I >,
                        &DisableFeaturesRequest < I >::handle_close_journal >
                        (this);
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_close_journal(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result < 0) {
                lderr(cct) << "failed to close image journal: " <<
                    cpp_strerror(*result)
                    << dendl;
            }
        ...
        template < typename I >
            ...
            journal::RemoveRequest < I > *req =
                journal::RemoveRequest < I >::create(image_ctx.md_ctx,
                                                     image_ctx.id,
                                                     librbd::Journal <>::
                                                     IMAGE_CLIENT_ID,
                                                     context_wq, ctx);
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_remove_journal(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result < 0) {
                lderr(cct) << "failed to remove image journal: " <<
                    cpp_strerror(*result)
                    << dendl;
                return handle_finish(*result);
            }
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_append_op_event(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result < 0) {
                lderr(cct) << "failed to commit journal entry: " <<
                    cpp_strerror(*result)
                    << dendl;
                return handle_finish(*result);
            }
        ...
        template < typename I >
            ...
            Context *ctx = create_context_callback <
                DisableFeaturesRequest < I >,
                &DisableFeaturesRequest < I >::handle_remove_object_map >
                (this);
        ...
        template < typename I >
            ...
            librbd::cls_client::set_features(&op, m_new_features,
                                             m_features_mask);

            using klass = DisableFeaturesRequest < I >;
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_set_features > (this);
            int r =
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_set_features(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result == -EINVAL
                && (m_features_mask & RBD_FEATURE_JOURNALING) != 0) {
            ...
            if (*result < 0) {
                lderr(cct) << "failed to update features: " <<
                    cpp_strerror(*result)
                    << dendl;
                return handle_finish(*result);
            }
        ...
        template < typename I >
            ...
            image::SetFlagsRequest < I > *req =
                image::SetFlagsRequest < I >::create(&image_ctx, 0,
                                                     m_disable_flags, ctx);
            req->send();
        }
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_update_flags(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result < 0) {
                lderr(cct) << "failed to update image flags: " <<
                    cpp_strerror(*result)
                    << dendl;
                return handle_finish(*result);
            }
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_notify_update(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            Context * DisableFeaturesRequest <
            I >::handle_release_exclusive_lock(int *result) {
            I & image_ctx = this->m_image_ctx;
            CephContext *cct = image_ctx.cct;
            ldout(cct,
                  20) << this << " " << __func__ << ": r=" << *result << dendl;
