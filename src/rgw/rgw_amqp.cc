...
// key class for the connection list
    struct connection_id_t {
        ...
        // equality operator and hasher functor are needed // so that connection_id_t could be used as key in unordered_map
...
// since references to deleted objects may still exist in the calling code
    struct connection_t {
        ...
        connection_t():
            ...
            mandatory(false),
            use_ssl(false), verify_ssl(false), ca_location(boost::none) {
        }
        // cleanup of all internal connection resource// the object can still remain, and internal connection// resources created again on successful reconnection
            void destroy(int s) {
            status = s;
            ...
            std::for_each(callbacks.begin(), callbacks.end(),
                          ...
                          ldout(cct,
                                20) <<
                          "AMQP destroy: invoking callback with tag=" << cb_tag.
                          tag << dendl;});
            callbacks.clear();
            delivery_tag = 1;
        }
        ...
        // dtor also destroys the internals ~connection_t() {
        ...
        case AMQP_RESPONSE_SERVER_EXCEPTION:
            {
                ...
                if (reply.reply.decoded) {
                    amqp_connection_close_t *m =
                        (amqp_connection_close_t *) reply.reply.decoded;
                    ss << m->reply_code << " text: " << std::string((char *)m->
                                                                    reply_text.
                                                                    bytes,
                                                                    m->
                                                                    reply_text.
...
// utility function to create a connection, when the connection object already exists
    connection_ptr_t & create_connection(connection_ptr_t & conn,
        ...
        if (info.ssl) {
            ...
            SSL_CTX *ssl_ctx =
                reinterpret_cast <
                SSL_CTX * >(amqp_ssl_socket_get_context(socket));
#else
        ...
        // open channels
        {
            ...
            RETURN_ON_REPLY_ERROR(conn, state,
        ...
        }
        {
            ...
            RETURN_ON_ERROR(conn, RGW_AMQP_STATUS_CONSUME_DECLARE_FAILED,
                            consume_ok);
            RETURN_ON_REPLY_ERROR(conn, state,
...
// utility function to create a new connection
    connection_ptr_t create_new_connection(const amqp_connection_info & info,
                                           ...
                                           CephContext * cct, bool verify_ssl,
                                           boost::optional <
...
/// struct used for holding messages in the message queue
    struct message_wrapper_t {
        ...
        message_wrapper_t(connection_ptr_t & _conn,
                          ...
                          reply_callback_t _cb):conn(_conn), topic(_topic),
            message(_message), cb(_cb) {
    }};

    typedef std::unordered_map < connection_id_t, connection_ptr_t,
        connection_id_t::hasher > ConnectionList;
    ...
    class Manager {
      ...
      private:
        ...
        void publish_internal(message_wrapper_t * message) {
            ...
            if (!conn->is_ok()) {
                ...
                ldout(conn->cct,
                      1) <<
                    "AMQP publish: connection had an issue while message was in the queue"
                    << dendl;
                if (message->cb) {
            ...
            if (message->cb == nullptr) {
                ...
                ldout(conn->cct,
                      1) << "AMQP publish (no callback): failed with error " <<
            ...
            if (rc == AMQP_STATUS_OK) {
                ...
                if (q_len < max_inflight) {
                    ldout(conn->cct,
                        ...
                        delivery_tag << "): OK. Queue has: " << q_len <<
                        " callbacks" << dendl;
                    conn->callbacks.emplace_back(conn->delivery_tag++,
                                                 message->cb);
                }
                else {
                    ...
                    ldout(conn->cct,
                          1) <<
                        "AMQP publish (with callback): failed with error: callback queue full"
                        << dendl;
                ...
                // it will be retied by the main loop
                ldout(conn->cct,
                      1) << "AMQP publish (with callback): failed with error: "
                    << status_to_string(rc) << dendl;
                conn->destroy(rc);
                // immediately invoke callback with error
                message->cb(rc);
                ...
                for (; conn_it != end_it;) {
                    ...
                    if (!conn->is_ok()) {
                        ...
                        if (now >= conn->next_reconnect) {
                            ...
                            info.host =
                                const_cast < char *>(conn_key.host.c_str());
                            info.port = conn_key.port;
                            info.vhost =
                                const_cast < char *>(conn_key.vhost.c_str());
                            info.user =
                                const_cast < char *>(conn->user.c_str());
                            info.password =
                                const_cast < char *>(conn->password.c_str());
                            info.ssl = conn->use_ssl;
                            ldout(conn->cct,
                                  20) << "AMQP run: retry connection" << dendl;
                            if (create_connection(conn, info)->is_ok() == false) {
                                ldout(conn->cct,
                                      10) << "AMQP run: connection (" <<
                                    to_string(conn_key) <<
                                    ") retry failed. error: " <<
                                    status_to_string(conn->
                                                     status) << " (" << conn->
                                    reply_code << ")" << dendl;
                                ...
                                ldout(conn->cct,
                                      10) << "AMQP run: connection (" <<
                                    to_string(conn_key) << ") retry successfull"
                                    << dendl;
                            }
                        }
                    ...
                    const auto rc =
                        amqp_simple_wait_frame_noblock(conn->state, &frame,
                                                       &read_timeout);
                    ...
                    if (rc != AMQP_STATUS_OK) {
                        ...
                        // it will be retied by the main loop
                        ldout(conn->cct,
                              1) << "AMQP run: connection read error: " <<
                            status_to_string(rc) << dendl;
                        conn->destroy(rc);
                        INCREMENT_AND_CONTINUE(conn_it);
                    }
                    ...
                    case AMQP_BASIC_ACK_METHOD:
                        {
                            ...
                            const auto ack =
                                (amqp_basic_ack_t *) frame.payload.method.
                                decoded;
                            ceph_assert(ack);
                    ...
                    case AMQP_BASIC_NACK_METHOD:
                        {
                            ...
                            const auto nack =
                                (amqp_basic_nack_t *) frame.payload.method.
                                decoded;
                            ceph_assert(nack);
                    ...
                    case AMQP_BASIC_REJECT_METHOD:
                        {
                            result = RGW_AMQP_STATUS_BROKER_NACK;
                            const auto reject =
                                (amqp_basic_reject_t *) frame.payload.method.
                                decoded;
                    ...
                    case AMQP_CHANNEL_CLOSE_METHOD:
                        {
                            ...
                            ldout(conn->cct,
                                  10) <<
                                "AMQP run: connection was closed by broker" <<
                                dendl;
                            conn->destroy(rc);
                            INCREMENT_AND_CONTINUE(conn_it);
                        }
                    case AMQP_BASIC_RETURN_METHOD:
                        ...
                        ldout(conn->cct,
                              10) << "AMQP run: message was not routable" <<
                            dendl;
                        INCREMENT_AND_CONTINUE(conn_it);
                        break;
                    default:
                    ...
                    const auto tag_it =
                        std::find(callbacks_begin, callbacks_end, tag);
                    if (tag_it != callbacks_end) {
                        if (multiple) {
                            // n/ack all up to (and including) the tag
                            ldout(conn->cct,
                                  20) <<
                                "AMQP run: multiple n/acks received with tag="
                                << tag << " and result=" << result << dendl;
                            auto it = callbacks_begin;
                            while (it->tag <= tag
                                   && it != conn->callbacks.end()) {
                                ldout(conn->cct,
                                      20) <<
                                    "AMQP run: invoking callback with tag=" <<
                                    it->tag << dendl;
                                it->cb(result);
                        ...
                        ldout(conn->cct,
                              10) <<
                            "AMQP run: unsolicited n/ack received with tag=" <<
                            tag << dendl;
                    }
      ...
      public:
        Manager(size_t _max_connections,
            ...
            max_idle_time(30),
            connection_count(0), stopped(false), read_timeout {
        0, _usec_timeout},
            ...
            const auto rc =
                ceph_pthread_setname(runner.native_handle(), "amqp_manager");
            ceph_assert(rc == 0);
        }
        ...
        connection_ptr_t connect(const std::string & url,
                                 const std::string & exchange,
                                 bool mandatory_delivery, bool verify_ssl,
                                 boost::optional <
                                 const std::string & >ca_location) {
            if (stopped) {
            ...
            std::vector < char >url_cache(url.c_str(),
                                          url.c_str() + url.size() + 1);
            const auto retcode = amqp_parse_url(url_cache.data(), &info);
            if (AMQP_STATUS_OK != retcode) {
                ldout(cct,
                      1) << "AMQP connect: URL parsing failed. error: " <<
                    retcode << dendl;
                return nullptr;
            }
            ...
            const auto conn =
                create_new_connection(info, exchange, mandatory_delivery, cct,
                                      verify_ssl, ca_location);
            if (!conn->is_ok()) {
                ldout(cct,
                      10) << "AMQP connect: connection (" << to_string(id) <<
                    ") creation failed. error:" << status_to_string(conn->
                                                                    status) <<
            ...
            ldout(cct,
                  10) << "AMQP connect: new connection status is: " <<
        ...
        int publish(connection_ptr_t & conn,
            ...
            if (messages.
                push(new message_wrapper_t(conn, topic, message, nullptr))) {
                ++queued;
                return AMQP_STATUS_OK;
            }
        ...
        int publish_with_confirm(connection_ptr_t & conn,
            ...
            if (stopped) {
                ldout(cct,
                      1) << "AMQP publish_with_confirm: manager is not running"
                    << dendl;
                return RGW_AMQP_STATUS_MANAGER_STOPPED;
            }
            if (!conn || !conn->is_ok()) {
...
// TODO make the pointer atomic in allocation and deallocation to avoid race conditions
    ...
    bool init(CephContext * cct) {
        ...
        s_manager =
            new Manager(MAX_CONNECTIONS_DEFAULT, MAX_INFLIGHT_DEFAULT,
                        MAX_QUEUE_DEFAULT, READ_TIMEOUT_USEC, IDLE_TIME_MS,
                        RECONNECT_TIME_MS, cct);
        return true;
    }
    ...
    connection_ptr_t connect(const std::string & url,
        ...
        if (!s_manager)
            return nullptr;
        return s_manager->connect(url, exchange, mandatory_delivery, verify_ssl,
                                  ca_location);
    }
