...
Mutex::Mutex(const std::string & n, bool r, bool ld,
             bool bt,
             CephContext * cct):name(n), id(-1), recursive(r), lockdep(ld),
backtrace(bt), nlock(0), locked_by(0), cct(cct), logger(0)
{
    ...
    ANNOTATE_BENIGN_RACE_SIZED(&locked_by, sizeof(locked_by),
                               "Mutex locked_by");
    if (cct) {
...
void Mutex::Lock(bool no_lockdep)
{
    ...
    if (lockdep && g_lockdep && !no_lockdep && !recursive)
        _will_lock();
    ...
    if (logger && cct && cct->_conf->mutex_perf_counter) {
        ...
        logger->tinc(l_mutex_wait, ceph_clock_now() - start);
    }
    else {
        r = pthread_mutex_lock(&_m);
    }
