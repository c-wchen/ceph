    ...
    do {
        ...
        if (should_work(start)) {
            ...
            if (r < 0) {
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: do life cycle process() returned error r=" << r <<
                    dendl;
            }
...
static bool obj_has_expired(const DoutPrefixProvider * dpp, CephContext * cct,
                            ceph::real_time mtime, int days,
...
static bool pass_object_lock_check(rgw::sal::Driver * driver,
                                   rgw::sal::Object * obj,
...
class LCObjsLister {
  ...
  public:
    LCObjsLister(rgw::sal::Driver * _driver,
                 rgw::sal::Bucket * _bucket):driver(_driver), bucket(_bucket) {
        list_params.list_versions = bucket->versioned();
        list_params.allow_unordered = true;
        delay_ms =
            driver->ctx()->_conf.get_val < int64_t > ("rgw_lc_thread_delay");
    ...
    int fetch(const DoutPrefixProvider * dpp) {
        int ret =
            bucket->list(dpp, list_params, 1000, list_results, null_yield);
        if (ret < 0) {
            return ret;
        }
    ...
    bool get_obj(const DoutPrefixProvider * dpp, rgw_bucket_dir_entry ** obj, std::function < void (void) > fetch_barrier =[]() {   /* nada */
                 }) {
...
struct lc_op_ctx {
    ...
    :cct(env.driver->ctx()), env(env), o(o), next_key_name(next_key_name),
        ...
        return (next_key_name
                && key_name.compare(boost::get < std::string >
                                    (next_key_name)) == 0);
    }
...
static int remove_expired_obj(const DoutPrefixProvider * dpp, lc_op_ctx & oc,
                              bool remove_indeed,
                              rgw::notify::EventType event_type)
{
    ...
    del_op->params.obj_owner.set_id(rgw_user {
                                    meta.owner}
    );
    ...
    notify = driver->get_notification(dpp, obj.get(), nullptr, event_type,
                                      bucket.get(), lc_id,
                                      const_cast <
                                      std::string & >(oc.bucket->get_tenant()),
...
class WorkQ:public Thread {
  public:
    ...
    using work_f =
        std::function < void (RGWLC::LCWorker *, WorkQ *, WorkItem &) >;
...
class RGWLC::WorkPool {
  ...
  public:
    ...
    :wqs(TVector {
         n_threads,[&](const size_t ix, auto emplacer) {
         emplacer.emplace(wk, ix, qmax);}}), ix(0) {
    }
...
int RGWLC::handle_multipart_expiration(rgw::sal::Bucket * target,
                                       ...
                                       lc_op > &prefix_map, LCWorker * worker,
                                       time_t stop_at, bool once)
{
    ...
    auto pf =[&](RGWLC::LCWorker * wk, WorkQ * wq, WorkItem & wi) {
        ...
        if (obj_has_expired(this, cct, obj.meta.mtime, rule.mp_expiration)) {
            ...
            std::unique_ptr < rgw::sal::MultipartUpload > mpu =
                target->get_multipart_upload(key.name);
    ...
    for (auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end();
        ...
        if (!prefix_iter->second.status
            || prefix_iter->second.mp_expiration <= 0) {
            continue;
        }
        ...
        do {
            ...
            for (auto obj_iter = results.objs.begin();
                 obj_iter != results.objs.end(); ++obj_iter, ++offset) {
                std::tuple < lc_op, rgw_bucket_dir_entry > t1 = {
                prefix_iter->second, *obj_iter};
                worker->workpool->enqueue(WorkItem {
                                          t1}
                );
                if (going_down()) {
                    return 0;
            ...
            if ((offset % 100) == 0) {
                if (worker_should_stop(stop_at, once)) {
                    ldpp_dout(this,
                              5) << __func__ <<
                        " interval budget EXPIRED worker " << worker->
                        ix << dendl;
                    return 0;
                }
            }
...
static int read_obj_tags(const DoutPrefixProvider * dpp, rgw::sal::Object * obj,
                         bufferlist & tags_bl)
{
...
static bool is_valid_op(const lc_op & op)
{
    return (op.status &&
            (op.expiration > 0
             ...
             || op.dm_expiration || !op.transitions.empty()
...
static int check_tags(const DoutPrefixProvider * dpp, lc_op_ctx & oc,
                      bool * skip)
{
    auto & op = oc.op;
    ...
    if (op.obj_tags != boost::none) {
        *skip = true;
        ...
        try {
            ...
            ldpp_dout(oc.dpp,
                      0) <<
                "ERROR: caught buffer::error, couldn't decode TagSet " << oc.
                wq->thr_name() << dendl;
            return -EIO;
        }
    ...
    bool check(const DoutPrefixProvider * dpp, lc_op_ctx & oc)override {
        ...
        if (ret < 0) {
            ...
            ldpp_dout(oc.dpp, 0) << "ERROR: check_tags on obj=" << oc.obj
                << " returned ret=" << ret << " " << oc.wq->thr_name() << dendl;
            return false;
        }
    ...
    LCOpAction_CurrentExpiration(op_env & env) {
        ...
        if (!o.is_current()) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key
                << ": not current, skipping " << oc.wq->thr_name() << dendl;
            return false;
        }
        if (o.is_delete_marker()) {
            if (oc.next_key_name) {
                ...
                if (oc.next_has_same_name(o.key.name)) {
                    ldpp_dout(dpp,
                              7) << __func__ << "(): dm-check SAME: key=" << o.
                        key << " next_key_name: %%" << nkn << "%% " << oc.wq->
                        thr_name() << dendl;
                    return false;
                }
                else {
                    ldpp_dout(dpp,
                              7) << __func__ << "(): dm-check DELE: key=" << o.
                        key << " next_key_name: %%" << nkn << "%% " << oc.wq->
                        thr_name() << dendl;
        ...
        if (op.expiration <= 0) {
            ...
            *exp_time = *op.expiration_date;
            ...
            is_expired =
                obj_has_expired(dpp, oc.cct, mtime, op.expiration, exp_time);
        }

        ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": is_expired="
            << (int)is_expired << " " << oc.wq->thr_name() << dendl;
        return is_expired;
    }
...
class LCOpAction_NonCurrentExpiration:public LCOpAction {
  ...
  public:
    LCOpAction_NonCurrentExpiration(op_env & env) {
        ...
        if (o.is_current()) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key
                << ": current version, skipping " << oc.wq->thr_name() << dendl;
            return false;
        }

        int expiration = oc.op.noncur_expiration;
        bool is_expired =
            obj_has_expired(dpp, oc.cct, oc.effective_mtime, expiration,
                            exp_time);

        ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": is_expired="
            << is_expired << " " << oc.wq->thr_name() << dendl;
    ...
    int process(lc_op_ctx & oc) {
        ...
        if (r < 0) {
            ldpp_dout(oc.dpp,
                      0) <<
                "ERROR: remove_expired_obj (non-current expiration) " << oc.
                bucket << ":" << o.key << " " << cpp_strerror(r)
                << " " << oc.wq->thr_name() << dendl;
            return r;
        }
        if (perfcounter) {
        ...
        ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key
            << " (non-current expiration) " << oc.wq->thr_name() << dendl;
        return 0;
    }
};
  ...
  public:
    ...
    int process(lc_op_ctx & oc) {
        ...
        if (r < 0) {
            ldpp_dout(oc.dpp,
                      0) <<
                "ERROR: remove_expired_obj (delete marker expiration) " << oc.
                bucket << ":" << o.key << " " << cpp_strerror(r)
                << " " << oc.wq->thr_name()
                << dendl;
            return r;
        ...
        ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key
            << " (delete marker expiration) " << oc.wq->thr_name() << dendl;
        return 0;
    }
};
...
class LCOpAction_Transition:public LCOpAction {
  ...
  public:
    ...
    :transition(_transition) {
        ...
        ldpp_dout(oc.dpp,
                  20) << __func__ << "(): key=" << o.
            key << ": is_expired=" << is_expired << " " << oc.wq->
            thr_name() << dendl;

        need_to_process =
            (rgw_placement_rule::
             get_canonical_storage_class(o.meta.storage_class) !=
    ...
    int delete_tier_obj(lc_op_ctx & oc) {
         ...
         */
        if (oc.bucket->versioned() && oc.o.is_current()
            && !oc.o.is_delete_marker()) {
            ret =
                remove_expired_obj(oc.dpp, oc, false,
                                   rgw::notify::ObjectExpiration);
            ldpp_dout(oc.dpp,
                      20) << "delete_tier_obj Object(key:" << oc.o.
                key << ") current & not delete_marker" << " versioned_epoch:  "
                << oc.o.versioned_epoch << "flags: " << oc.o.flags << dendl;
        }
        else {
            ret =
                remove_expired_obj(oc.dpp, oc, true,
                                   rgw::notify::ObjectExpiration);
            ldpp_dout(oc.dpp,
                      20) << "delete_tier_obj Object(key:" << oc.o.
                key << ") not current " << "versioned_epoch:  " << oc.o.
                versioned_epoch << "flags: " << oc.o.flags << dendl;
        }
        return ret;
    }
    ...
    int transition_obj_to_cloud(lc_op_ctx & oc) {
        ...
        int ret = oc.obj->transition_to_cloud(oc.bucket, oc.tier.get(), oc.o,
                                              oc.env.worker->
                                              get_cloud_targets(), oc.cct,
                                              !delete_object, oc.dpp,
                                              null_yield);
        if (ret < 0) {
            return ret;
        }
        ...
        if (delete_object) {
            ...
            if (ret < 0) {
                ldpp_dout(oc.dpp,
                          0) << "ERROR: Deleting tier object(" << oc.o.
    ...
    int process(lc_op_ctx & oc) {
        ...
        if (oc.o.meta.category == RGWObjCategory::CloudTiered) {
            ...
            ldpp_dout(oc.dpp,
                      30) << "Object(key:" << oc.o.
                key << ") is already cloud tiered to cloud-s3 tier: " << oc.o.
                meta.storage_class << dendl;
            return 0;
        }
        ...
        if (!r && oc.tier->get_tier_type() == "cloud-s3") {
            ...
            if (!oc.o.is_current()
                ...
                ldpp_dout(oc.dpp,
                          10) << "Object(key:" << oc.o.
                    key << ") is locked. Skipping transition to cloud-s3 tier: "
            ...
            if (r < 0) {
                ldpp_dout(oc.dpp,
                          0) << "ERROR: failed to transition obj(key:" << oc.o.
                    key << ") to cloud (r=" << r << ")" << dendl;
                return r;
            }
        }
        else {
            ...
            int r =
                oc.obj->transition(oc.bucket, target_placement, o.meta.mtime,
        ...
        ldpp_dout(oc.dpp, 2) << "TRANSITIONED:" << oc.bucket
            << ":" << o.key << " -> "
            << transition.storage_class << " " << oc.wq->thr_name() << dendl;
        return 0;
    }
};
...
void LCOpRule::build()
{
  ...
  for (auto & iter:op.noncur_transitions) {
        actions.
            emplace_back(new LCOpAction_NonCurrentTransition(env, iter.second));
    }
}
  ...
  for (auto & a:actions) {
    ...
    if (selected && (*selected)->should_process()) {
      ...
      for (auto & f:filters) {
        ...
        if (!cont) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key
                << ": no rule match, skipping " << wq->thr_name() << dendl;
            return 0;
        }
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: remove_expired_obj "
                << env.bucket << ":" << o.key << " " << cpp_strerror(r)
                << " " << wq->thr_name() << dendl;
            return r;
        }
                             ...
                             time_t stop_at, bool once)
{
    ...
    ldpp_dout(this,
              5) << "RGWLC::bucket_lc_process ENTER " << bucket_name << dendl;
    if (unlikely(cct->_conf->rgwlc_skip_bucket_step)) {
        return 0;
    }

    int ret =
        driver->get_bucket(this, nullptr, bucket_tenant, bucket_name, &bucket,
                           null_yield);
    if (ret < 0) {
    ...
    auto stack_guard = make_scope_guard([&worker] {
                                        worker->workpool->drain();}
    );
    ...
    if (aiter == bucket->get_attrs().end()) {
        ldpp_dout(this,
                  0) << "WARNING: bucket_attrs.find(RGW_ATTR_LC) failed for " <<
            bucket_name << " (terminates bucket_lc_process(...))" << dendl;
        return 0;
    }
    ...
    for (auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end();
        ...
        if (!zone_check(op, zone)) {
            ldpp_dout(this,
                      7) << "LC rule not executable in " << zone->
                get_tier_type()
                << " zone, skipping" << dendl;
            continue;
        }
        ...
        rgw_bucket_dir_entry *o {
        nullptr};
        for (auto offset = 0; ol.get_obj(this, &o /* , fetch_barrier */ );
             ++offset, ol.next()) {
            orule.update();
            std::tuple < LCOpRule, rgw_bucket_dir_entry > t1 = {
            orule, *o};
            worker->workpool->enqueue(WorkItem {
                                      t1}
            );
    ...
    ret =
        handle_multipart_expiration(bucket.get(), prefix_map, worker, stop_at,
                                    once);
    return ret;
}
                          ...
                          LCWorker * worker)
{
    ...
    do {
        ...
        if (ret == -EBUSY || ret == -EEXIST) {
            ...
            ldpp_dout(this,
                      0) << "RGWLC::bucket_lc_post() failed to acquire lock on "
        ...
        if (ret < 0)
            return 0;
        ldpp_dout(this,
                  20) << "RGWLC::bucket_lc_post() lock " << obj_names[index]
            << dendl;
        ...
        if (result == -ENOENT) {
            ...
            if (ret < 0) {
                ldpp_dout(this,
                          0) <<
                    "RGWLC::bucket_lc_post() failed to remove entry " <<
                            ...
                            int &index)
{
    ...
    for (; index < max_objs; index++, marker = "") {
        ...
        int ret =
            sal_lc->list_entries(obj_names[index], marker, max_entries,
                                 entries);
        if (ret < 0) {
            if (ret == -ENOENT) {
                ldpp_dout(this,
                          10) << __func__ << "() ignoring unfound lc object=" <<
                    obj_names[index] << dendl;
                continue;
            }
            else {
        ...
        std::move(begin(entries), end(entries),
...
{
    vector < int >v(n, 0);
    std::generate(v.begin(), v.end(),[ix = 0] ()mutable {
                  return ix++;}
    );
...
bool RGWLC::expired_session(time_t started)
{
    ...
    time_t interval = (cct->_conf->rgw_lc_debug_interval > 0)
        ? cct->_conf->rgw_lc_debug_interval : 24 * 60 * 60;
                          ...
                          bool once = false)
{
    ...
    std::unique_lock < rgw::sal::LCSerializer > lock(*(serializer.get()),
                                                     std::adopt_lock);
    ...
    if (ret >= 0) {
        if (entry->get_status() == lc_processing) {
            if (expired_session(entry->get_start_time())) {
                ldpp_dout(this,
                          5) <<
                    "RGWLC::process_bucket(): STALE lc session found for: " <<
                    entry << " index: " << index << " worker ix: " << worker->
                    ix << " (clearing)" << dendl;
            }
            else {
    ...
    lock.unlock();
    ret =
        bucket_lc_process(entry->get_bucket(), worker, thread_stop_at(), once);
    bucket_lc_post(index, max_lock_secs, *entry, ret, worker);

    return ret;
}                               /* RGWLC::process_bucket */

static inline bool allow_shard_rollover(CephContext * cct, time_t now,
                   ...
                   bool once = false)
{
    ...
    std::unique_ptr < rgw::sal::LCSerializer > lock =
        sal_lc->get_serializer(lc_index_lock_name, lc_shard,
                               worker->thr_name());
    ...
    SimpleBackoff shard_lock(5 /* max retries */ , 50 ms);
    ...
    do {
        ...
        if (head->get_marker().empty() && allow_shard_rollover(cct, now, head->get_shard_rollover_date())   /* prevent multiple passes by diff.
                                                                                                             * rgws,in same cycle */ ) {

            ldpp_dout(this,
                      5) << "RGWLC::process() process shard rollover lc_shard="
                << lc_shard << " head.marker=" << head->get_marker()
                << " head.shard_rollover_date=" << head->
                get_shard_rollover_date()
                << dendl;

            vector < std::unique_ptr < rgw::sal::Lifecycle::LCEntry >> entries;
            int ret =
                sal_lc->list_entries(lc_shard, head->get_marker(), 1, entries);
            if (ret < 0) {
                ldpp_dout(this,
                          0) <<
                    "RGWLC::process() sal_lc->list_entries(lc_shard, head.marker, 1, "
            ...
            if (ret < 0) {
                ldpp_dout(this,
                          0) <<
                    "RGWLC::process() sal_lc->get_entry(lc_shard, head.marker, entry) "
        ...
        if (entry && !entry->get_bucket().empty()) {
            if (entry->get_status() == lc_processing) {
                if (expired_session(entry->get_start_time())) {
                    ldpp_dout(this, 5)
                        << "RGWLC::process(): STALE lc session found for: " <<
                        entry << " index: " << index << " worker ix: " <<
                        worker->ix << " (clearing)" << dendl;
                }
                else {
                    ldpp_dout(this, 5)
                        ...
                        ix << dendl;
                    /* skip to next entry */
                    if (advance_head(lc_shard, *head.get(), *entry.get(), now) <
                        0) {
                        goto exit;
                    }
                    ...
                    if (head->get_marker().empty()) {
                        ...
                        if (ret < 0) {
                            ldpp_dout(this,
                                      0) <<
                                "RGWLC::process() failed to put head " <<
                                lc_shard << dendl;
                        }
                        goto exit;
                    }
                ...
                if ((entry->get_status() == lc_complete) &&
                    ...
                    ldpp_dout(this,
                              5) << "RGWLC::process() worker ix; " << worker->
                        ix << " SKIP processing for already-processed bucket "
                        << entry->get_bucket()
                        << dendl;
                    /* done with this shard */
                    if (head->get_marker().empty()) {
        ...
        if (ret < 0) {
            ldpp_dout(this, 0) << "RGWLC::process() failed to set obj entry "
                << lc_shard << entry->get_bucket() << entry->
                get_status() << dendl;
            goto exit;
        }
        ...
        lock->unlock();
        ret =
            bucket_lc_process(entry->get_bucket(), worker, thread_stop_at(),
                              once);
    ...
    for (int ix = 0; ix < maxw; ++ix) {
        auto worker =
            std::make_unique < RGWLC::LCWorker > (this /* dpp */ , cct, this,
                                                  ix);
        worker->create((string {
                        "lifecycle_thr_"} +to_string(ix)).c_str());
...
unsigned RGWLC::get_subsys() const const
{
    return dout_subsys;
}

std::ostream & RGWLC::gen_prefix(std::ostream & out) const const
{
...
void RGWLifecycleConfiguration::generate_test_instances(list <
                                ...
                                bool merge_attrs)
{
    ...
    if (merge_attrs) {
        ...
        if (ret < 0) {
            ldpp_dout(this,
                      0) <<
                "RGWLC::RGWDeleteLC() failed to set attrs on bucket=" << b.
                name << " returned err=" << ret << dendl;
            return ret;
        }
    }
...
namespace rgw::lc {
    ...
    int fix_lc_shard_entry(const DoutPrefixProvider * dpp,
        ...
        if (ret == -ENOENT) {
            ...
            char cookie_buf[COOKIE_LEN + 1];
            gen_rand_alphanumeric(driver->ctx(), cookie_buf,
                                  sizeof(cookie_buf) - 1);
    ...
    std::string s3_expiration_header(DoutPrefixProvider * dpp,
                                     const rgw_obj_key & obj_key,
                                     const RGWObjTags & obj_tagset,
                                     const ceph::real_time & mtime,
                                     const std::map < std::string,
                                     buffer::list > &bucket_attrs) {
        ...
        if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 16)) {
          for (const auto & elt:obj_tag_map) {
                ldpp_dout(dpp, 16) << __func__
                    << "() key=" << elt.first << " val=" << elt.second << dendl;
            }
        }
      ...
      for (const auto & ri:rule_map) {
            ...
            auto & prefix =
                filter.has_prefix()? filter.get_prefix() : rule.get_prefix();
            ...
            if (filter.has_tags()) {
              ...
              for (auto & tag:rule_tagset.get_tags()) {
                    ...
                    if (obj_tag == obj_tag_map.end()
                        || obj_tag->second != tag.second) {
                        ldpp_dout(dpp,
                                  10) << "tag does not match obj_key=" <<
                            obj_key << " rule_id=" << id << " tag=" << tag <<
                            dendl;
                        tag_match = false;
                        break;
                    }
            ...
            if (rule_expiration.has_date()) {
                ...
                if (rule_expiration.has_days()) {
                    rule_expiration_date =
                        boost::optional < ceph::real_time >
                        (mtime +
                         make_timespan(double (rule_expiration.get_days()) *
                                       24 * 60 * 60 -
    ...
    bool s3_multipart_abort_header(DoutPrefixProvider * dpp,
                                   const rgw_obj_key & obj_key,
                                   const ceph::real_time & mtime,
                                   const std::map < std::string,
                                   buffer::list > &bucket_attrs,
      ...
      for (const auto & ri:rule_map) {
            ...
            if (mp_expiration.has_days()) {
                rule_abort_date =
                    ...
                    (mtime +
                     make_timespan(mp_expiration.get_days() * 24 * 60 * 60 -
                                   ceph::real_clock::to_time_t(mtime) % (24 *
                                                                         60 *
                                                                         60) +
                                   24 * 60 * 60));
            }
...
void lc_op::dump(Formatter * f) const const
{
...
void LCFilter::dump(Formatter * f) const const
{
...
void LCExpiration::dump(Formatter * f) const const
{
    f->dump_string("days", days);
    f->dump_string("date", date);
}

void LCRule::dump(Formatter * f) const const
{
    f->dump_string("id", id);
    f->dump_string("prefix", prefix);
...
void RGWLifecycleConfiguration::dump(Formatter * f) const const
{
