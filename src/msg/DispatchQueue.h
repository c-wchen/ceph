...
class DispatchQueue
{
    ...
    public:
        explicit QueueItem(const ceph::ref_t < Message > &m): type(-1), con(0),
            m(m)
        {
    ...
    std::map < ceph::ref_t < Message >,
        decltype(marrival)::iterator > marrival_map;
    void add_arrival(const ceph::ref_t < Message > &m)
    {
        marrival_map.insert(make_pair(m,
                                      marrival.
                                      insert(std::
                                             make_pair(m->get_recv_stamp(),
                                                     m)).first)
                           );
    }
    ...
    enum { D_CONNECT =
               1, D_ACCEPT, D_BAD_REMOTE_RESET, D_BAD_RESET, D_CONN_REFUSED,
           D_NUM_CODES
         };
...
public:
    ...
    void queue_connect(Connection *con)
    {
        ...
        mqueue.enqueue_strict(0,
                              CEPH_MSG_PRIO_HIGHEST, QueueItem(D_CONNECT, con));
        cond.notify_all();
    }
    void queue_accept(Connection *con)
    {
        ...
        mqueue.enqueue_strict(0,
                              CEPH_MSG_PRIO_HIGHEST, QueueItem(D_ACCEPT, con));
        cond.notify_all();
    }
    ...
    bool is_started() const
    {
        ...
        : cct(cct), msgr(msgr),
          ...
          local_delivery_lock(ceph::
                              make_mutex
                              ("Messenger::DispatchQueue::local_delivery_lock" +
                               name)), stop_local_delivery(false),
          local_delivery_thread(this), dispatch_throttler(cct,
                  std::
                  string
                  ("msgr_dispatch_throttler-")
                  + name,
                  cct->_conf->
