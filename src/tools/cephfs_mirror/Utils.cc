            ...
            std::vector < const char *>args)
{
    dout(20) << ": connecting to cluster=" << cluster_name <<
             ", client=" << client_name << ", mon_host=" << mon_host <<
             dendl;

    CephInitParameters iparams(CEPH_ENTITY_TYPE_CLIENT);
    if (client_name.empty() || !iparams.name.from_str(client_name)) {
        derr << ": error initializing cluster handle for " <<
             cluster_name << dendl;
        return -EINVAL;
    }
    CephContext *cct =
        common_preinit(iparams, CODE_ENVIRONMENT_LIBRARY,
    ...
    if (!args.empty()) {
        ...
        if (r < 0) {
            derr << ": could not parse command line args: " <<
                 cpp_strerror(r) << dendl;
            cct->put();
            return r;
        }
    ...
    if (!mon_host.empty()) {
        ...
        if (r < 0) {
            derr << "failed to set mon_host config: " << cpp_strerror(r)
                 << dendl;
            cct->put();
            return r;
        }
    ...
    if (!cephx_key.empty()) {
        ...
        if (r < 0) {
            derr << "failed to set key config: " << cpp_strerror(r) <<
                 dendl;
            cct->put();
            return r;
        }
    ...
    dout(10) << ": using mon addr=" << cct->_conf.get_val <
             std::string > ("mon_host") << dendl;
    ...
    r = (*cluster)->connect();
    if (r < 0) {
        derr << ": error connecting to " << cluster_name << ": " <<
             cpp_strerror(r)
             << dendl;
        return r;
    }
...
int mount(RadosRef cluster, const Filesystem &filesystem,
          bool cross_check_fscid, MountRef *mount)
{
    dout(20) << ": filesystem=" << filesystem << dendl;

    ceph_mount_info *cmi;
    int r =
        ceph_create_with_context(&cmi,
                                 reinterpret_cast <
                                 CephContext * >(cluster->cct()));
    if (r < 0) {
    ...
    if (cross_check_fscid && fs_id != filesystem.fscid) {
        ...
        derr << ": filesystem-id mismatch " << fs_id << " vs " <<
             filesystem.fscid << dendl;
