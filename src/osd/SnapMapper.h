...
class OSDriver:public MapCacher::StoreDriver < std::string, bufferlist > {
  ...
  public:
    class OSTransaction:public MapCacher::Transaction < std::string,
        ...
        OSTransaction(coll_t cid,
                      const ghobject_t & hoid, ObjectStore::Transaction * t)
        :cid(cid), hoid(hoid), t(t) {
      } public:
        void set_keys(const std::map < std::string,
                      bufferlist > &to_set) override {
    ...
    OSTransaction get_transaction(ObjectStore::Transaction * t) {
...
class SnapMapper {
  ...
  private:
    ...
    std::pair < std::string, bufferlist > to_raw(const std::pair < snapid_t,
                                                 hobject_t > &to_map);

    static bool is_mapping(const std::string & to_test);

    std::pair < snapid_t, hobject_t > from_raw(const std::pair < std::string,
                                               bufferlist > &image);
  ...
  public:
    ...
    SnapMapper(CephContext * cct, MapCacher::StoreDriver < std::string, bufferlist > *driver, uint32_t match,   ///< [in] pgid
  ...
  :    
    ...
    void update_bits(uint32_t new_bits  ///< [in] new split bits
        ...
        mask_bits = new_bits;
        set < string > _prefixes = hobject_t::get_prefixes(mask_bits,
                                                           match, pool);
        prefixes.clear();
