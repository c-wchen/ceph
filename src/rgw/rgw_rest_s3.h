...
public:
    ...
    void set_custom_http_response(int http_ret)
    {
...
class RGWPutBucketReplication_ObjStore_S3: public
    RGWPutBucketReplication_ObjStore
{
public:
    int get_params(optional_yield y) override;
    void send_response() override;
...
class RGWDeleteBucketReplication_ObjStore_S3: public
    RGWDeleteBucketReplication_ObjStore
{
protected:
...
public:
    ...
    int get_policy_from_state(rgw::sal::Driver *driver, req_state *s,
...
class RGWDeleteBucketEncryption_ObjStore_S3: public
...
public:
    ...
    int get_params(optional_yield y) override;
    void send_response() override;
    int prepare_encryption(std::map < std::string,
...
class RGWListBucketMultiparts_ObjStore_S3: public
...
class RGWPutBucketPublicAccessBlock_ObjStore_S3: public
    RGWPutBucketPublicAccessBlock
{
public:
    void send_response() override;
};

class RGWGetBucketPublicAccessBlock_ObjStore_S3: public
...
public:
    explicit RGWHandler_Auth_S3(const rgw::auth::
    ...
    int init(rgw::sal::Driver *driver,
             req_state *s, rgw::io::BasicClient *cio) override;
...
public:
    ...
    explicit RGWHandler_REST_S3(const rgw::auth::
                                StrategyRegistry &auth_registry)
        : RGWHandler_REST(), auth_registry(auth_registry)
    {
...
public:
    RGWHandler_REST_Service_S3(const rgw::auth::
                               StrategyRegistry &
                               auth_registry): RGWHandler_REST_S3(auth_registry)
    {
...
public:
    RGWHandler_REST_Bucket_S3(const rgw::auth::StrategyRegistry &auth_registry,
                              bool
                              _enable_pubsub): RGWHandler_REST_S3(auth_registry),
...
private:
    ...
    const bool enable_pubsub;
public:
    explicit RGWRESTMgr_S3(bool _enable_s3website = false, bool _enable_sts =
                               false, bool _enable_iam =
                               false, bool _enable_pubsub = false)
...
public:
    ...
    {
        static constexpr size_t DIGEST_SIZE_V2 =
            CEPH_CRYPTO_HMACSHA1_DIGESTSIZE;
        static constexpr size_t DIGEST_SIZE_V4 =
    ...
    public:
        ...
        using server_signature_t =
            basic_sstring < char, uint16_t, SIGNATURE_MAX_SIZE >;
        ...
        using completer_factory_t =
            std::function <
            rgw::auth::Completer::cmplptr_t(const boost::optional <
...
protected:
    ...
    AWSEngine(CephContext *const cct,
              const VersionAbstractor &ver_abstractor)
    ...
    virtual result_t authenticate(const DoutPrefixProvider *dpp,
                                  ...
                                  completer_factory, const req_state *s,
...
class AWSGeneralAbstractor: public AWSEngine::VersionAbstractor
{
    ...
    auth_data_t get_auth_data_v4(const req_state *s,
                                 const bool using_qs) const;
...
public:
    LDAPEngine(CephContext *const cct,
        ...
        : AWSEngine(cct, ver_abstractor),
          driver(driver), apl_factory(apl_factory)
    {
        init(cct);
...
class STSEngine: public AWSEngine
{
    ...
    auth_info_t get_creds_info(const STS::
                               SessionToken &token)const noexcept;

    int get_session_token(const DoutPrefixProvider *dpp,
                          const std::string_view &session_token,
...
public:
    STSEngine(CephContext *const cct,
              rgw::sal::Driver *driver,
              const VersionAbstractor &ver_abstractor,
              const rgw::auth::LocalApplier::Factory *
              const local_apl_factory,
              const rgw::auth::RemoteApplier::Factory *
              const remote_apl_factory,
              const rgw::auth::RoleApplier::Factory *
              const role_apl_factory)
        : AWSEngine(cct, ver_abstractor), driver(driver),
