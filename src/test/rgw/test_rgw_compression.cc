...
TEST(Compress, LimitedChunkSize)
{
    ...
    for (size_t s = 100; s < 10000000; s = s * 5 / 4) {
        ...
        RGWGetObj_Decompress decompress(g_ceph_context, &cs_info, false,
                                        &d_sink);
        ...
        decompress.handle_data(c_sink.get_sink(), 0,
                               c_sink.get_sink().length());
        decompress.handle_data(empty, 0, 0);

        ASSERT_LE(d_sink.get_size(),
                  (size_t) g_ceph_context->_conf->rgw_max_chunk_size);
    }
}
