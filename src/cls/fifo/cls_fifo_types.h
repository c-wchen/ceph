...
namespace rados::cls::fifo {
    ...
    struct data_params {
        ...
        0};
        std::uint64_t full_size_threshold {
        0};
...
// four uint64_ts in a row and only care about a subset at a time.
    class update {
      ...
      public:
        ...
        update
            && journal_entries_add(std::optional < std::vector <
                                   fifo::journal_entry >> &&entries) {
            if (entries) {
        ...
        update
            && journal_entries_rm(std::optional < std::vector <
                                  fifo::journal_entry >> &&entries) {
            if (entries) {
    ...
    struct info {
        ...
        std::int64_t tail_part_num {
        0};
        std::int64_t head_part_num {
        -1};
        std::int64_t min_push_part_num {
        0};
        std::int64_t max_push_part_num {
        -1};
    ...
    struct part_list_entry {
        ...
        part_list_entry() {
                          ...
                          uint64_t ofs, ceph::real_time mtime)
    ...
    struct part_header {
        ...
        std::uint64_t min_ofs {
        0};
        std::uint64_t last_ofs {
        0};
...
template <>
    ...
    rados::cls::fifo::part_header >:fmt::ostream_formatter {
};
#endif
