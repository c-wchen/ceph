 ...
 * any operation that comes from an authenticated user. */
        class Identity {
          public:
            ...
            virtual uint32_t get_perms_from_aclspec(const DoutPrefixProvider *
                                                    dpp,
            ...
            virtual std::string get_role_tenant() const {
                return "";
            }
          ...
          public:
            ...
            class AuthResult {
                ...
                int reason = 0;
                 ...
                 std::pair < IdentityApplier::aplptr_t,
                    Completer::cmplptr_t > result_pair;
 ...
 * the dynamic reconfiguration. */
        class Strategy:public Engine {
          public:
            ...
            } static int apply(const DoutPrefixProvider * dpp,
                               const Strategy & auth_strategy, req_state * s,
          ...
          private:
             ...
             * interested in storing nulls while preserving the dynamic polymorphism. */
             using stack_item_t =
                std::pair < std::reference_wrapper < const Engine >, Control >;
 ...
 * Each new Strategy should be exposed to it. */
        ...
        class WebIdentityApplier:public IdentityApplier {
          ...
          protected:
            CephContext * const cct;
             ...
             boost::optional < std::multimap < std::string,
                std::string >> role_tags;
             boost::optional < std::set < std::pair < std::string,
                std::string >>> principal_tags;

             std::string get_idp_url() const;

            void create_account(const DoutPrefixProvider * dpp, const rgw_user & acct_user, const std::string & display_name, RGWUserInfo & user_info) const;   /* out */
          public:
             WebIdentityApplier(CephContext * const cct,
                                ...
                                boost::optional < std::multimap < std::string,
                                std::string >> role_tags,
                                boost::optional < std::set < std::pair <
                                std::string, std::string >>> principal_tags)
            :cct(cct), driver(driver), role_session(role_session),
                ...
                if (user_name != token_claims.end()) {
                    ...
                    const auto & given_username =
                        token_claims.find("given_username");
            ...
            uint32_t get_perms_from_aclspec(const DoutPrefixProvider * dpp,
                ...
                if (uid.id == this->sub && uid.tenant == role_tenant
                    && uid.ns == "oidc") {
                    return true;
                } return false;
            }
        ...
        class ImplicitTenants:public md_config_obs_t {
            ...
            class ImplicitTenantValue {
              ...
              public:
                 bool inline is_split_mode() {
                    ...
                    return v == IMPLICIT_TENANTS_SWIFT
                        || v == IMPLICIT_TENANTS_S3;
                } bool inline implicit_tenants_for_(const
                                                    implicit_tenant_flag_bits
                                                    bit) {
                    assert(v != IMPLICIT_TENANTS_BAD);
          ...
          public:
            ImplicitTenants(const ConfigProxy & c) {
                recompute_value(c);
            }
        ...
        std::tuple < bool,
            bool > implicit_tenants_enabled_for_swift(CephContext * const cct);
        std::tuple < bool,
            bool > implicit_tenants_enabled_for_s3(CephContext * const cct);
 ...
 * data will be used to fill RGWUserInfo structure. */
        class RemoteApplier:public IdentityApplier {
          public:
            class AuthInfo {
              ...
              :    
                ...
                acct_type(acct_type),
                access_key_id(access_key_id), subuser(subuser) {
            }};

            using aclspec_t = rgw::auth::Identity::aclspec_t;
            typedef std::function < uint32_t(const aclspec_t &) >
                acl_strategy_t;
          ...
          protected:
            ...
            const rgw::auth::ImplicitTenants::
                implicit_tenant_flag_bits implicit_tenant_bit;

            virtual void create_account(const DoutPrefixProvider * dpp, const rgw_user & acct_user, bool implicit_tenant, RGWUserInfo & user_info) const;   /* out */
 ...
 * a way to avoid unnecessary calls to underlying RADOS store. */
        ...
        class RoleApplier:public IdentityApplier {
          ...
          public:
            ...
            :role(role), token_attrs(token_attrs) {
            } uint32_t get_perms_from_aclspec(const DoutPrefixProvider * dpp,
                                              const aclspec_t & aclspec) const
                override {
                return 0;
                ...
                return (this->token_attrs.user_id.id == uid.id
                        && this->token_attrs.user_id.tenant == uid.tenant
