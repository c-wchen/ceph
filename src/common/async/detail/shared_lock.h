...
// SharedMutexImpl instead, because the locks may outlive the SharedMutex itself
    ...
    template < typename Executor >
      ...
      public:
        using mutex_type =
            boost::intrusive_ptr < ceph::async::detail::SharedMutexImpl >;

        unique_lock() = default;
        explicit unique_lock(ceph::async::SharedMutex < Executor > &m)
        :impl(m.impl), locked(true) {
            impl->lock();
        } unique_lock(ceph::async::SharedMutex < Executor > &m,
                      defer_lock_t t) noexcept:impl(m.impl) {
        }
        unique_lock(ceph::async::SharedMutex < Executor > &m, try_to_lock_t t)
      :    impl(m.impl), locked(impl->try_lock()) {
        }
      unique_lock(ceph::async::SharedMutex < Executor > &m, adopt_lock_t t) noexcept:impl(m.impl), locked(true)
        {
        }
        ~unique_lock() {
            if (impl && locked)
      ...
      unique_lock(unique_lock && other) noexcept:impl(std::move(other.impl)),
            locked(other.
                   locked) {
            other.locked = false;
        }
    ...
    template < typename Executor >
      ...
      shared_lock(ceph::async::SharedMutex < Executor > &m, defer_lock_t t) noexcept:impl(m.impl)
        {
        }
        shared_lock(ceph::async::SharedMutex < Executor > &m, try_to_lock_t t)
      :    impl(m.impl), locked(impl->try_lock_shared()) {
        }
      shared_lock(ceph::async::SharedMutex < Executor > &m, adopt_lock_t t) noexcept:impl(m.impl), locked(true)
        {
        }
      ...
      shared_lock(shared_lock && other) noexcept:impl(std::move(other.impl)),
            locked(other.
                   locked) {
            other.locked = false;
        }
