...
namespace crimson::crush {
    ...
    seastar::future <> CrushLocation::update_from_conf() {
        auto crush_location =
            local_conf().get_val < std::string > ("crush_location");
    ...
    seastar::future <> CrushLocation::update_from_hook() {
        ...
        return seastar::file_exists(crush_location_hook).
            then([this] (bool result) {
                 ...
                 return seastar::file_accessible(local_conf().get_val <
                                                 std::string >
                                                 ("crush_location_hook"),
                                                 seastar::access_flags::
                                                 execute).
                 then([this] (bool result) {
                      if (!result) {
                      std::stringstream errmsg;
                      ...
                      .argv = {
                      local_conf().get_val < std::string >
                      ("crush_location_hook"), "--cluster",
                      local_conf()->cluster, "--id",
                      local_conf()->name.get_id(), "--type",
                      local_conf()->name.get_type_str()
                      }
                      };
                      return seastar::experimental::spawn_process(local_conf().
                                                                  get_val <
                                                                  std::string >
                                                                  ("crush_location_hook"),
                                                                  std::
                             ...
                             return do_with(std::move(process),
                                            std::move(stdout),
                                            [this] (auto & process,
                                                    auto & stdout) {
                                            return stdout.read().
    ...
    seastar::future <> CrushLocation::init_on_startup() {
        ...
        if (local_conf().get_val < std::string >
