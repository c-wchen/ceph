...
namespace dpdk {
    ...
    int eal::init(CephContext * c) {
        ...
        t = std::thread([&]() {
                        ...
                        string2vector("-c"),
                        string2vector(c->_conf->get_val < std::string >
                                      ("ms_dpdk_coremask")),
                        string2vector("-n"),
                        ...
                        if (!c->_conf->ms_dpdk_pmd.empty()) {
                        args.push_back(string2vector("--no-huge"));}

      std::string rte_file_prefix; rte_file_prefix = "rte_"; rte_file_prefix += c->_conf->name.to_str(); args.push_back(string2vector("--file-prefix")); args.push_back(string2vector(rte_file_prefix)); std::vector < char *>cargs; for (auto && a:args)
                        {
                        ...
                        initialized = true;
                        done = true; cond.notify_all(); while (true) {
                        if (!funcs.empty()) {
