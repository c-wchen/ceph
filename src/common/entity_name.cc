...
const char *EntityName:: to_cstr() const const
{
    return type_id.c_str();
}

bool EntityName::from_str(const string & s)
{
    size_t pos = s.find('.');
...
void EntityName:: set(uint32_t type_, const std::string & id_)
{
    type = type_;
    id = id_;
...
int EntityName:: set(const std::string & type_, const std::string & id_)
{
...
int EntityName:: set_type(const char *type_)
{
    return set(type_, id);
}

void EntityName:: set_id(const std::string & id_)
{
    set(type, id_);
}
...
const char *EntityName:: get_type_str() const const
{
    return ceph_entity_type_name(type);
}

const char *EntityName:: get_type_name() const const
{
    return ceph_entity_type_name(type);
}
...
bool EntityName::has_default_id() const const
{
    return (id == "admin");
}
...
std::string EntityName::get_valid_types_as_str()
{
    ...
    size_t i;
    std::string sep("");
    for (i = 0; i < sizeof(STR_TO_ENTITY_TYPE) / sizeof(STR_TO_ENTITY_TYPE[0]);
         ++i) {
        out += sep;
