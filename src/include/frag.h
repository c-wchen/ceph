...
class frag_t {
  ...
  frag_t(_frag_t e):_enc(e) {
    ...
    // tests bool contains(unsigned v)const {
    ...
    // splitting frag_t make_child(int i, int nb)const {
        assert(i < (1 << nb));
    ...
    // binary splitting frag_t left_child() const {
    ...
    // sequencing bool is_leftmost() const {
...
inline void encode(const frag_t & f, bufferlist & bl)
{
...
class fragtree_t {
  ...
  public:
    ...
    bool empty() const {
        ...
        if (!ls.empty() && ls.front() == x && ls.size() == 1)
            return true;
        return false;
    }
  /**
   ...
   */
    ...
    bool force_to_leaf(CephContext * cct, frag_t x) {
        ...
        if (parent.bits() < x.bits()) {
            ...
            for (std::list < frag_t >::iterator p = subs.begin();
                 p != subs.end(); ++p) {
                lgeneric_dout(cct,
                              10) << "splitting intermediate " << *p << " by "
                    << (nb - spread) << dendl;
    ...
    void encode_nohead(bufferlist & bl) const {
        for (compact_map < frag_t, int32_t >::const_iterator p =
             _splits.begin(); p != _splits.end(); ++p) {
            ::encode(p->first, bl);
            ::encode(p->second, bl);
  ...
  public:
    ...
    void simplify() {
        while (1) {
            ...
            while (p != _set.end()) {
                if (!p->is_root() && _set.count(p->get_sibling())) {
