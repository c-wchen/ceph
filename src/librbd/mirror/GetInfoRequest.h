...
public:
    static GetInfoRequest *create(librados::IoCtx &io_ctx,
                                  ...
                                  Context *on_finish)
    {
        return new GetInfoRequest(io_ctx, op_work_queue, image_id,
                                    ...
                                    Context *on_finish)
    {
        return new GetInfoRequest(image_ctx, mirror_image,
                                  promotion_state, primary_mirror_uuid,
                                  on_finish);
    } GetInfoRequest(librados::IoCtx &io_ctx,
                     asio::ContextWQ *op_work_queue,
                     const std::string &image_id,
                     cls::rbd::MirrorImage *mirror_image,
                     PromotionState *promotion_state,
...
private:
    ...
    void calc_promotion_state(const std::map < librados::snap_t,
                              SnapInfo > &snap_info);
};
