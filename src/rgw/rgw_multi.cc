...
int list_multipart_parts(RGWRados * store, RGWBucketInfo & bucket_info,
                         CephContext * cct, const string & upload_id,
                         string & meta_oid, int num_parts, int marker,
                         ...
                         bool assume_unsorted)
{
    ...
    if (sorted_omap) {
        ...
        ret =
            store->omap_get_vals(raw_obj, header, p, num_parts + 1, parts_map);
    }
    else {
                         ...
                         bool assume_unsorted)
{
    return list_multipart_parts(store, s->bucket_info, s->cct, upload_id,
                                meta_oid, num_parts, marker, parts, next_marker,
...
{
    rgw_obj meta_obj;
    meta_obj.init_ns(bucket_info.bucket, mp_obj.get_meta(),
    ...
    do {
        ret =
            list_multipart_parts(store, bucket_info, cct,
                                 mp_obj.get_upload_id(), mp_obj.get_meta(),
                                 1000, marker, obj_parts, &marker, &truncated);
        if (ret < 0)
            return ret;
        for (auto obj_iter = obj_parts.begin(); obj_iter != obj_parts.end();
             ++obj_iter) {
            ...
            if (obj_part.manifest.empty()) {
                ...
                RGWObjManifest::obj_iterator oiter =
                    obj_part.manifest.obj_begin();
                if (oiter != obj_part.manifest.obj_end()) {
                    rgw_obj head;
                    rgw_raw_obj raw_head =
                        oiter.get_location().get_raw_obj(store);
...
int list_bucket_multiparts(RGWRados * store, RGWBucketInfo & bucket_info,
                           ...
                           int &max_uploads,
                           vector < rgw_bucket_dir_entry > *objs, map < string,
                           bool > *common_prefixes, bool * is_truncated)
{
    ...
    return (list_op.
            list_objects(max_uploads, objs, common_prefixes, is_truncated));
}

int abort_bucket_multiparts(RGWRados * store, CephContext * cct,
                            RGWBucketInfo & bucket_info, string & prefix,
                            string & delim)
{
    ...
    do {
        ...
        if (!objs.empty()) {
          ...
          for (const auto & obj:objs) {
                ...
                entry.obj = obj;
                ret =
                    abort_multipart_upload(store, cct, &obj_ctx, bucket_info,
                                           entry.mp);
