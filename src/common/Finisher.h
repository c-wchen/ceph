...
class Finisher {
   ...
   * finishes, but this class should never be used in this way. */
    ...
    explicit Finisher(CephContext * cct_):
        ...
        finisher_empty_wait(false), thread_name("fn_anonymous"), logger(0),
  ...
  Finisher(CephContext * cct_, string name, string tn):
    cct(cct_), finisher_lock("Finisher::" + name),
        finisher_stop(false), finisher_running(false),
        finisher_empty_wait(false), thread_name(tn), logger(0),
        finisher_thread(this) {
        PerfCountersBuilder b(cct, string("finisher-") + name, l_finisher_first,
                              l_finisher_last);
