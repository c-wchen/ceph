...
static void forward_req_info(CephContext * cct, req_info & info,
                                       ...
                                       RGWAccessControlPolicy * policy)
{
    ...
    return get_bucket_instance_policy_from_attr(cct, store, bucket_info,
                                                bucket_attrs, policy,
                                                instance_obj);
}
...
static int get_obj_attrs(RGWRados * store, struct req_state *s, rgw_obj & obj,
                         map < string, bufferlist > &attrs)
{
...
static int modify_obj_attr(RGWRados * store, struct req_state *s, rgw_obj & obj,
                           const char *attr_name, bufferlist & attr_val)
{
    map < string, bufferlist > attrs;
    RGWRados::Object op_target(store, s->bucket_info,
                               *static_cast < RGWObjectCtx * >(s->obj_ctx),
    ...
    return store->set_attrs(s->obj_ctx, s->bucket_info, read_op.state.obj,
                            attrs, NULL);
}

static int get_system_obj_attrs(RGWRados * store, struct req_state *s,
                                rgw_raw_obj & obj, map < string,
                                bufferlist > &attrs, uint64_t * obj_size,
                                RGWObjVersionTracker * objv_tracker)
{
    RGWRados::SystemObject src(store,
                               *static_cast < RGWObjectCtx * >(s->obj_ctx),
                               obj);
                              ...
                              rgw_bucket & bucket)
{
    if (!s->system_request && bucket_info.flags & BUCKET_SUSPENDED) {
        ldout(s->cct,
              0) << "NOTICE: bucket " << bucket_info.bucket.
            name << " is suspended" << dendl;
    ...
    int ret =
        get_bucket_policy_from_attr(s->cct, store, bucket_info, bucket_attrs,
                                    policy);
    if (ret == -ENOENT) {
...
static int read_obj_policy(RGWRados * store,
                           ...
                           rgw_bucket & bucket, rgw_obj_key & object)
{
    string upload_id;
    ...
    policy =
        get_iam_policy_from_attr(s->cct, store, bucket_attrs, bucket.tenant);
    ...
    if (ret == -ENOENT) {
        ...
        ret =
            get_bucket_policy_from_attr(s->cct, store, bucket_info,
                                        bucket_attrs, &bucket_policy);
        if (ret < 0) {
            return ret;
        }
...
int rgw_build_bucket_policies(RGWRados * store, struct req_state *s)
{
    ...
    if (!bi.empty()) {
        ret =
            rgw_bucket_parse_bucket_instance(bi, &s->bucket_instance_id,
    ...
    if (s->dialect.compare("s3") == 0) {
        s->bucket_acl =
            ceph::make_unique < RGWAccessControlPolicy_S3 > (s->cct);
    }
        ...
        if (!s->user->user_id.empty()) {
            s->user_acl =
                ceph::make_unique < RGWAccessControlPolicy_SWIFTAcct > (s->cct);
        }
        s->bucket_acl =
            ceph::make_unique < RGWAccessControlPolicy_SWIFT > (s->cct);
    }
    else {
    ...
    if (!s->src_bucket_name.empty()) {
        ...
        if (s->bucket_instance_id.empty()) {
            ret =
                store->get_bucket_info(obj_ctx, s->src_tenant_name,
                                       s->src_bucket_name, source_info, NULL);
        }
        else {
            ret =
                store->get_bucket_instance_info(obj_ctx, s->bucket_instance_id,
                                                source_info, NULL, NULL);
        }
        if (ret == 0) {
    ...
    s->user->user_id, s->user->display_name,};

    if (!s->bucket_name.empty()) {
        s->bucket_exists = true;
        if (s->bucket_instance_id.empty()) {
            ret =
                store->get_bucket_info(obj_ctx, s->bucket_tenant,
        ...
        if (s->bucket_exists) {
            ...
            s->bucket_acl->create_default(s->user->user_id,
                                          s->user->display_name);
            ret = -ERR_NO_SUCH_BUCKET;
        }
        ...
        if (s->bucket_exists
            && !store->get_zonegroup().equals(s->bucket_info.zonegroup)) {
            ldout(s->cct,
                  0) << "NOTICE: request for data in a different zonegroup (" <<
                s->bucket_info.zonegroup << " != " << store->get_zonegroup().
                get_id() << ")" << dendl;
    ...
    if (s->user_acl) {
        ...
        if (!ret) {
            ret =
                get_user_policy_from_attr(s->cct, store, uattrs, *s->user_acl);
        }
        if (-ENOENT == ret) {
            ...
            ldout(s->cct,
                  0) <<
                "NOTICE: couldn't get user attrs for handling ACL (user_id=" <<
                s->user->user_id << ", ret=" << ret << ")" << dendl;
            return ret;
        }
    }
...
int RGWOp::verify_op_mask()
{
    ...
    if (!s->system_request && (required_mask & RGW_OP_TYPE_MODIFY)
        && !store->zone_is_writeable()) {
        ldout(s->cct,
              5) <<
            "NOTICE: modify request to a read-only zone by a non-system user, permission denied"
            << dendl;
        return -EPERM;
    }
...
int RGWOp::init_quota()
{
    ...
    if (s->user->user_id == s->bucket_owner.get_id()) {
        ...
        int r =
            rgw_get_user_info_by_uid(store, s->bucket_info.owner, owner_info);
        if (r < 0)
            return r;
        uinfo = &owner_info;
    }
 ...
 * */
static void get_cors_response_headers(RGWCORSRule * rule, const char *req_hdrs,
...
{
    ldout(s->cct,
          20) << "user manifest obj=" << ent.key.name << "[" << ent.key.
        instance << "]" << dendl;
    ...
    ldout(s->cct,
          20) << "reading obj=" << part << " ofs=" << cur_ofs << " end=" <<
        cur_end << dendl;
...
int RGWGetObj::handle_user_manifest(const char *prefix)
{
    ...
    int r = iterate_user_manifest_parts(s->cct, store, ofs, end,
                                        pbucket_info, obj_prefix, bucket_acl,
                                        *bucket_policy,
  ...
  for (const auto & entry:slo_info.entries) {
        ...
        if (bucket_name.compare(s->bucket.name) != 0) {
            ...
            if (piter != policies.end()) {
                ...
                bucket = bucket_info.bucket;
                bucket_acl = &_bucket_acl;
                r = read_bucket_policy(store, s, bucket_info, bucket_attrs,
                                       bucket_acl, bucket);
                if (r < 0) {
                ...
                auto _bucket_policy =
                    get_iam_policy_from_attr(s->cct, store, bucket_attrs,
                                             bucket_info.bucket.tenant);
        ...
        etag_sum.Update((const byte *)entry.etag.c_str(), entry.etag.length());
...
static bool object_is_expired(map < string, bufferlist > &attrs)
{
    ...
    if (iter != attrs.end()) {
        ...
        try {
            ...
            dout(0) << "ERROR: " << __func__ << ": failed to decode "
...
void RGWListBuckets::execute()
{
    ...
    do {
      ...
      for (const auto & policy:store->get_zonegroup().
             placement_targets) {
...
void RGWGetUsage::execute()
{
    ...
    if (!start_date.empty()) {
        ...
        if (op_ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: failed to parse start date" << dendl;
            return;
        }
    }
    ...
    if (!end_date.empty()) {
        ...
        if (op_ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: failed to parse end date" << dendl;
            return;
        }
    }
    ...
    while (is_truncated) {
        op_ret =
            store->read_usage(s->user->user_id, start_epoch, end_epoch,
    ...
    op_ret =
        rgw_user_get_all_buckets_stats(store, s->user->user_id, buckets_usage);
    if (op_ret < 0) {
...
void RGWSetBucketVersioning::execute()
{
    ...
    op_ret = retry_raced_bucket_write(store, s,[this] {
                                      ...
                                      return store->put_bucket_instance_info(s->
                                                                             bucket_info,
                                                                             false,
                                                                             real_time
                                                                             (),
    ...
    if (op_ret < 0) {
        ldout(s->cct,
            ...
            name << " returned err=" << op_ret << dendl;
        return;
    }
}
...
void RGWSetBucketWebsite::execute()
{
    ...
    op_ret = retry_raced_bucket_write(store, s,[this] {
                                      ...
                                      website_conf;
                                      op_ret =
                                      store->put_bucket_instance_info(s->
                                                                      bucket_info,
                                                                      false,
                                                                      real_time
                                                                      (),
...
#define MAX_REST_RESPONSE (128 * 1024)  // we expect a very small response
    int ret =
        store->rest_master_conn->forward(uid_str,
                                         (forward_info ? *forward_info : s->
                                          info),
                                         objv, MAX_REST_RESPONSE, &in_data,
                                         &response);
    if (ret < 0)
        return ret;
...
void RGWCreateBucket::execute()
{
    ...
    if (!store->get_zonegroup().is_master_zonegroup()
        ...
        ldout(s->cct,
              0) << "location constraint (" << location_constraint << ")" <<
            " doesn't match zonegroup" << " (" << store->get_zonegroup().
            api_name << ")" << dendl;
        op_ret = -ERR_INVALID_LOCATION_CONSTRAINT;
        s->err.message = "The specified location-constraint is not valid";
        return;
    ...
    if (!store->is_meta_master()) {
        ...
        ldout(s->cct,
              20) << "parsed: objv.tag=" << objv.tag << " objv.ver=" << objv.
            ver << dendl;
        ldout(s->cct,
    ...
    op_ret = store->create_bucket(*(s->user), s->bucket, zonegroup_id,
                                  placement_rule,
                                  s->bucket_info.swift_ver_location,
                                  pquota_info, attrs, info, pobjv, &ep_objv,
                                  creation_time, pmaster_bucket,
                                  pmaster_num_shards, true);
    ...
    if (need_metadata_upload() && existed) {
        ...
        do {
            ...
            op_ret =
                store->get_bucket_info(obj_ctx, s->bucket_tenant,
                                       s->bucket_name, binfo, nullptr, &battrs);
            if (op_ret < 0) {
                return;
            }
            ...
            if (swift_ver_location) {
                s->bucket_info.swift_ver_location = *swift_ver_location;
                s->bucket_info.swift_versioning =
            ...
            filter_out_website(attrs, rmattr_names,
                               s->bucket_info.website_conf);
            s->bucket_info.has_website =
                !s->bucket_info.website_conf.is_empty();
...
void RGWDeleteBucket::execute()
{
    ...
    if (!s->bucket_exists) {
        ldout(s->cct,
              0) << "ERROR: bucket " << s->bucket_name << " not found" << dendl;
        op_ret = -ERR_NO_SUCH_BUCKET;
        return;
    }
    ...
    op_ret =
        rgw_bucket_sync_user_stats(store, s->user->user_id, s->bucket_info);
    if (op_ret < 0) {
        ldout(s->cct,
              1) <<
    ...
    op_ret =
        abort_bucket_multiparts(store, s->cct, s->bucket_info, prefix,
                                delimiter);
    ...
    if (op_ret == 0) {
        op_ret =
            rgw_unlink_bucket(store, s->bucket_info.owner, s->bucket.tenant,
                              s->bucket.name, false);
        if (op_ret < 0) {
            ldout(s->cct,
                  0) << "WARNING: failed to unlink bucket: ret=" << op_ret <<
...
int RGWPutObj::verify_permission()
{
    if (!copy_source.empty()) {
        ...
        if (read_obj_policy
            (store, s, copy_source_bucket_info, cs_attrs, &cs_acl, policy,
            ...
            if (policy) {
                ...
                if (e == Effect::Deny) {
                    return -EACCES;
                         ...
                         !cs_acl.verify_permission(*s->auth.identity,
                                                   s->perm_mask,
                                                   RGW_PERM_READ)) {
                    return -EACCES;
                }
...
int RGWPutObjProcessor_Multipart::prepare(RGWRados * store, string * oid_rand)
{
    ...
    if (!err.empty()) {
        ldout(s->cct,
              10) << "bad part number: " << part_num << ": " << err << dendl;
        return -EINVAL;
    }
    ...
    manifest.set_multipart_part_rule(store->ctx()->_conf->rgw_obj_stripe_size,
                                     num);

    int r =
        manifest_gen.create_begin(store->ctx(), &manifest,
                                  s->bucket_info.placement_rule, bucket,
                                  target_obj);
    if (r < 0) {
        return r;
    }
...
RGWPutObjProcessor *RGWPutObj::select_processor(RGWObjectCtx & obj_ctx,
                                                bool * is_multipart)
{
    ...
    if (!multipart) {
        ...
        processor =
            new RGWPutObjProcessor_Multipart(obj_ctx, s->bucket_info, part_size,
                                             s);
    }
...
int RGWPutObj::get_data(const off_t fst, const off_t lst, bufferlist & bl)
{
    ...
    RGWRados::Object op_target(store, copy_source_bucket_info,
                               *static_cast < RGWObjectCtx * >(s->obj_ctx),
                               obj);
    RGWRados::Object::Read read_op(&op_target);
    read_op.params.obj_size = &obj_size;
...
void RGWPutObj::execute()
{
    ...
    const auto & compression_type =
        store->get_zone_params().get_compression_type(s->bucket_info.
                                                      placement_rule);
    ...
    if (!chunked_upload) {      /* with chunked upload we don't know how big is the upload.
        ...
        op_ret =
            store->check_bucket_shards(s->bucket_info, s->bucket, bucket_quota);
        if (op_ret < 0) {
            ldout(s->cct,
    ...
    processor =
        select_processor(*static_cast < RGWObjectCtx * >(s->obj_ctx),
                         &multipart);
    ...
    if (!multipart) {
        rgw_obj obj(s->bucket, s->object);
        op_ret =
            store->swift_versioning_copy(*static_cast <
                                         RGWObjectCtx * >(s->obj_ctx),
    ...
    if ((!copy_source.empty()) && !copy_source_range) {
        ...
        RGWObjState *astate;
        op_ret =
            store->get_obj_state(static_cast < RGWObjectCtx * >(s->obj_ctx),
                                 copy_source_bucket_info, obj, &astate, true,
                                 false);
        if (op_ret < 0) {
            ldout(s->cct,
                  0) << "ERROR: get copy source obj state returned with error"
                << op_ret << dendl;
            goto done;
        }
        if (!astate->exists) {
    ...
    do {
        ...
        if (copy_source.empty()) {
            ...
            uint64_t cur_lst =
                min(fst + s->cct->_conf->rgw_max_chunk_size - 1, lst);
    ...
    if (op_ret < 0) {
        ldout(s->cct,
              20) << "second check_quota() returned op_ret=" << op_ret << dendl;
        goto done;
    }
    ...
    if (op_ret < 0) {
        ldout(s->cct,
              20) << "check_bucket_shards() returned ret=" << op_ret << dendl;
        goto done;
    }
    ...
    if (dlo_manifest) {
        ...
        ldout(s->cct,
              10) << __func__ << ": calculated md5 for user manifest: " << etag
            << dendl;
    }
    ...
    dispose_processor(processor);
    perfcounter->tinc(l_rgw_put_lat, (ceph_clock_now() - s->time));
}
...
void RGWPostObj::execute()
{
    ...
    if (s->iam_policy) {
        ...
        if (e == Effect::Deny) {
            ...
            return;
                 ...
                 && !verify_bucket_permission_no_policy(s, RGW_PERM_WRITE)) {
            op_ret = -EACCES;
            return;
        }
    }
    ...
    do {
        ...
        RGWPutObjProcessor_Atomic processor(*static_cast <
                                            RGWObjectCtx * >(s->obj_ctx),
                                            s->bucket_info, s->bucket,
...
void RGWDeleteObj::execute()
{
    ...
    if (!s->object.empty()) {
        ...
        op_ret =
            store->swift_versioning_restore(*obj_ctx, s->bucket_owner.get_id(),
                                     ...
                                     string & bucket_name, rgw_obj_key & key)
{
    ...
    boost::string_view bn_view {
    dec_src.substr(0, pos)};
    bucket_name = std::string {
    bn_view.data(), bn_view.size()};

    boost::string_view kn_view {
    dec_src.substr(pos + 1)};
    key.name = std::string {
    kn_view.data(), kn_view.size()};
...
int RGWCopyObj::verify_permission()
{
    ...
    if (s->bucket_instance_id.empty()) {
        op_ret =
            store->get_bucket_info(obj_ctx, src_tenant_name, src_bucket_name,
    ...
    if (src_bucket_name.compare(dest_bucket_name) == 0) {   /* will only happen if s->local_source
        ...
        op_ret =
            store->get_bucket_info(obj_ctx, dest_tenant_name, dest_bucket_name,
...
void RGWPutACLs::execute()
{
    ...
    ldout(s->cct,
          15) << "read len=" << len << " data=" << (data ? data : "") << dendl;
...
#define ACL_GRANTS_MAX_NUM      100
    ...
    if (!s->object.empty()) {
        ...
        op_ret =
            rgw_bucket_set_attrs(store, s->bucket_info, attrs,
                                 &s->bucket_info.objv_tracker);
    }
    if (op_ret == -ECANCELED) {
...
static void get_lc_oid(struct req_state *s, string & oid)
{
    ...
    int max_objs =
        (s->cct->_conf->rgw_lc_max_objs >
         HASH_PRIME) ? HASH_PRIME : s->cct->_conf->rgw_lc_max_objs;
    int index =
        ceph_str_hash_linux(shard_id.c_str(),
...
void RGWPutLC::execute()
{
    ...
    config =
        ...
        >(parser.find_first("LifecycleConfiguration"));
    if (!config) {
        op_ret = -ERR_MALFORMED_XML;
        return;
    }
    ...
    if (op_ret < 0)
        return;
    string shard_id =
        s->bucket.tenant + ':' + s->bucket.name + ':' + s->bucket.bucket_id;
    string oid;
    get_lc_oid(s, oid);
    ...
    do {
        ...
        if (op_ret == -EBUSY) {
            dout(0) <<
                "RGWLC::RGWPutLC() failed to acquire lock on, sleep 5, try again"
                << oid << dendl;
            sleep(5);
            continue;
        }
        if (op_ret < 0) {
            dout(0) << "RGWLC::RGWPutLC() failed to acquire lock " << oid <<
                op_ret << dendl;
            break;
        }
        op_ret = cls_rgw_lc_set_entry(*ctx, oid, entry);
        if (op_ret < 0) {
            dout(0) << "RGWLC::RGWPutLC() failed to set entry " << oid << op_ret
                << dendl;
        }
        break;
    } while (1);
...
void RGWDeleteLC::execute()
{
    ...
    store->set_prefetch_data(s->obj_ctx, obj);
    op_ret =
        get_system_obj_attrs(store, s, obj, orig_attrs, NULL,
                             &s->bucket_info.objv_tracker);
    if (op_ret < 0)
        return;
    ...
    do {
        ...
        if (op_ret == -EBUSY) {
            dout(0) <<
                "RGWLC::RGWDeleteLC() failed to acquire lock on, sleep 5, try again"
                << oid << dendl;
            sleep(5);
            continue;
        }
        if (op_ret < 0) {
            dout(0) << "RGWLC::RGWDeleteLC() failed to acquire lock " << oid <<
                op_ret << dendl;
            break;
        }
        op_ret = cls_rgw_lc_rm_entry(*ctx, oid, entry);
        if (op_ret < 0) {
            dout(0) << "RGWLC::RGWDeleteLC() failed to set entry " << oid <<
                op_ret << dendl;
        }
        break;
    } while (1);
...
void RGWPutCORS::execute()
{
    ...
    op_ret = retry_raced_bucket_write(store, s,[this] {
                                      ...
                                      return rgw_bucket_set_attrs(store,
                                                                  s->
                                                                  bucket_info,
                                                                  attrs,
                                                                  &s->
...
void RGWDeleteCORS::execute()
{
    ...
    op_ret = retry_raced_bucket_write(store, s,[this] {
                                      rgw_raw_obj obj;
                                      store->get_bucket_instance_obj(s->bucket,
                                                                     obj);
                                      store->set_prefetch_data(s->obj_ctx, obj);
                                      ...
                                      map < string, bufferlist >::iterator iter;
                                      op_ret =
                                      get_system_obj_attrs(store, s, obj,
                                                           orig_attrs, NULL,
                                                           &s->bucket_info.
                                                           objv_tracker);
                                      if (op_ret < 0)
                                      ...
                                      for (iter = orig_attrs.begin();
                                           iter != orig_attrs.end(); ++iter) {
                                      const string & name = iter->first;
                                      dout(10) << "DeleteCORS : attr: " << name
                                      << dendl;
                                      if (name.
                                          compare(0,
                                                  (sizeof(RGW_ATTR_CORS) - 1),
                                                  RGW_ATTR_CORS) == 0) {
...
void RGWOptionsCORS::execute()
{
    ...
    if (!origin) {
        dout(0) << "Preflight request without mandatory Origin header" << dendl;
        op_ret = -EINVAL;
        return;
    }
        ...
        for (obj_iter = obj_parts.begin();
            ...
            if (iter->first != (int)obj_iter->first) {
                ldout(s->cct,
                    ...
                    iter->first << " next uploaded: " << obj_iter->
                    first << dendl;
                op_ret = -ERR_INVALID_PART;
                return;
            }
...
int RGWCompleteMultipart::MPSerializer::try_lock(const std::string & _oid,
                                                 utime_t dur)
{
    oid = _oid;
    op.assert_exists();
...
void RGWListBucketMultiparts::execute()
{
    ...
    op_ret =
        list_bucket_multiparts(store, s->bucket_info, prefix, marker_meta,
                               delimiter, max_uploads, &objs, &common_prefixes,
                               &is_truncated);
    if (op_ret < 0) {
        return;
    }
        ...
        if (s->iam_policy) {
            auto e = s->iam_policy->eval(s->env,
                                         *s->auth.identity,
                                         ...
                                         rgw::IAM::s3DeleteObjectVersion,
                                         obj);
            if ((e == Effect::Deny) || (e == Effect::Pass && !acl_allowed)) {
                                               ...
                                               )
{
    ...
    auto policy =
        get_iam_policy_from_attr(s->cct, store, battrs, binfo.bucket.tenant);
...
int RGWBulkUploadOp::handle_dir(const boost::string_ref path)
{
    ...
    if (!store->is_meta_master()) {
        ...
        op_ret =
            forward_request_to_master(s, nullptr, store, in_data, &jp, &info);
        if (op_ret < 0) {
            return op_ret;
        }
    ...
    if (op_ret && !existed && op_ret != -EEXIST) {
        ...
        ldout(s->cct, 20) << "bulk upload: containers already exists" << dendl;
        op_ret = -ERR_BUCKET_EXISTS;
    }
...
int RGWBulkUploadOp::handle_file(const boost::string_ref path,
                                 const size_t size, AlignedStreamGetter & body)
{
    ...
    RGWPutObjProcessor_Atomic processor(obj_ctx,
                                        binfo, binfo.bucket, object.name,
                                        /* part size */
                                        s->cct->_conf->rgw_obj_stripe_size,
                                        s->req_id, binfo.versioning_enabled());
    ...
    const auto & compression_type =
        store->get_zone_params().get_compression_type(binfo.placement_rule);
    CompressorRef plugin;
    if (compression_type != "none") {
        plugin = Compressor::create(s->cct, compression_type);
        if (!plugin) {
        ...
        if (!status.empty() && header) {
            ...
            case rgw::tar::FileType::NORMAL_FILE:{
                    ...
                    boost::string_ref filename =
                        ...
                        get_filename() : file_prefix +
                        header->get_filename().to_string();
                    auto body = AlignedStreamGetter(0, header->get_filesize(),
                                                    rgw::tar::BLOCK_SIZE,
                                                    *stream);
                    op_ret =
                        handle_file(filename, header->get_filesize(), body);
                    if (!op_ret) {
            ...
            case rgw::tar::FileType::DIRECTORY:{
                    ldout(s->cct,
                          2) << "bulk upload: handling regular directory" <<
                        dendl;

                    boost::string_ref dirname =
                        bucket_path.empty()? header->
...
void RGWSetAttrs::execute()
{
    ...
    if (!s->object.empty()) {
        store->set_atomic(s->obj_ctx, obj);
        op_ret =
            store->set_attrs(s->obj_ctx, s->bucket_info, obj, attrs, nullptr);
    }
    else {
      for (auto & iter:attrs) {
...
void RGWConfigBucketMetaSearch::execute()
{
    ...
    if (op_ret < 0) {
        ldout(s->cct,
              20) << "NOTICE: get_params() returned ret=" << op_ret << dendl;
        return;
    }
    ...
    op_ret =
        store->put_bucket_instance_info(s->bucket_info, false, real_time(),
                                        &s->bucket_attrs);
    if (op_ret < 0) {
        ldout(s->cct,
              0) << "NOTICE: put_bucket_info on bucket=" << s->bucket.
...
int RGWHandler::init(RGWRados * _store,
                     struct req_state *_s, rgw::io::BasicClient * cio)
{
    store = _store;
    s = _s;
...
void RGWPutBucketPolicy::execute()
{
    ...
    try {
        ...
        op_ret = retry_raced_bucket_write(store, s,[&p, this] {
                                          ...
                                          attrs[RGW_ATTR_IAM_POLICY].append(p.
                                                                            text);
                                          op_ret =
                                          rgw_bucket_set_attrs(store,
                                                               s->bucket_info,
...
void RGWGetBucketPolicy::execute()
{
    auto attrs = s->bucket_attrs;
    map < string, bufferlist >::iterator aiter =
        attrs.find(RGW_ATTR_IAM_POLICY);
    if (aiter == attrs.end()) {
