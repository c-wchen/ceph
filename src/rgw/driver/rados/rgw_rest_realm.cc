...
void RGWOp_Period_Post::execute(optional_yield y)
{
    ...
    period.init(this, cct,
                static_cast < rgw::sal::RadosStore * >(driver)->svc()->sysobj,
                y, false);
    ...
    if (period.get_id().empty()) {
        op_ret =
            period.commit(this, driver, realm, current_period, error_stream, y);
        if (op_ret < 0) {
            ldpp_dout(this,
                      -1) << "master zone failed to commit period" << dendl;
        }
        return;
    }

    // if it's not period commit, nobody is allowed to push to the master zone
    if (period.get_master_zone() == static_cast <
        rgw::sal::RadosStore *
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  -1) << "failed to store period " << period.get_id() << dendl;
        return;
    }
    // set as latest epoch
    ...
    auto period_history =
        static_cast <
        rgw::sal::RadosStore * >(driver)->svc()->mdlog->get_period_history();
    ...
    if (period.get_id() != current_period.get_id()) {
        ...
        auto cursor = period_history->attach(this, RGWPeriod { period }
                                             , y);
        if (!cursor) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      -1) << "failed to update realm's current period" << dendl;
            return;
        }
...
void RGWOp_Realm_Get::execute(optional_yield y)
{
    ...
    op_ret =
        realm->init(this, g_ceph_context,
                    static_cast <
                    rgw::sal::RadosStore * >(driver)->svc()->sysobj, y);
    if (op_ret < 0)
