...
struct ElasticConfig {
    ...
    string get_obj_path(const RGWBucketInfo &bucket_info, const rgw_obj_key &key)
    {
        if (es_info.version >= ES_V7) {
            return index_path + "/_doc/" + url_encode(bucket_info.bucket.bucket_id + ":" + key.name + ":" +
                    (key.instance.empty() ? "null" : key.instance));
            ;
        } else {
            return index_path +  "/object/" + url_encode(bucket_info.bucket.bucket_id + ":" + key.name + ":" +
                    (key.instance.empty() ? "null" : key.instance));
        }
    }
...
struct es_obj_metadata {
    ...
    es_obj_metadata(CephContext *_cct, ElasticConfigRef _es_conf, const RGWBucketInfo &_bucket_info,
                    ...
                    map<string, bufferlist> &_attrs, uint64_t _versioned_epoch) : cct(_cct), es_conf(_es_conf), bucket_info(_bucket_info),
        key(_key),
        ...
        for (auto i : custom_meta) {
            ...
            if (config == bucket_info.mdsearch_config.end()) {
                if (!es_conf->explicit_custom_meta) {
                    ...
                    ldpp_dout(dpp, 20) << "custom meta entry key=" << i.first << " not found in bucket mdsearch config: " <<
        ...
        reenter(this) {
            ...
            if (retcode < 0) {
                ...
                if (err_response.error.type != "index_already_exists_exception" &&
                    ...
                    ldpp_dout(dpp, 0) << "elasticsearch: failed to initialize index: response.type=" << err_response.error.type <<
...
public:
    RGWElasticHandleRemoteObjCR(RGWDataSyncCtx *_sc,
                                rgw_bucket_sync_pipe &_sync_pipe, rgw_obj_key &_key,
                                ElasticConfigRef _conf, uint64_t _versioned_epoch) : RGWCallStatRemoteObjCR(_sc, _sync_pipe.info.source_bs.bucket,
                                            _key),
...
public:
    RGWElasticDataSyncModule(const DoutPrefixProvider *dpp, CephContext *cct,
                             const JSONFormattable &config) : conf(std::make_shared<ElasticConfig>())
    {
    ...
    RGWCoroutine *sync_object(const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc, rgw_bucket_sync_pipe &sync_pipe,
                              rgw_obj_key &key, std::optional<uint64_t> versioned_epoch, const rgw_zone_set_entry &source_trace_entry,
                                ...
                                rgw_obj_key &key, real_time &mtime, bool versioned, uint64_t versioned_epoch, rgw_zone_set *zones_trace) override
    {
        ...
        ldpp_dout(dpp, 10) << conf->id << ": rm_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key << " mtime=" <<
                           mtime << " versioned=" << versioned << " versioned_epoch=" << versioned_epoch << dendl;
        if (!conf->should_handle_operation(sync_pipe.dest_bucket_info)) {
            ldpp_dout(dpp, 10) << conf->id << ": skipping operation (bucket not approved)" << dendl;
            return nullptr;
        }
    ...
    {
        ldpp_dout(dpp, 10) << conf->id << ": create_delete_marker: b=" << sync_pipe.info.source_bs.bucket << " k=" << key <<
                           " mtime=" << mtime
