 ...
 */
static std::string get_canon_resource(const DoutPrefixProvider *dpp,
                                      const char *const request_uri, const std::map < std::string,
 ...
 */
void rgw_create_s3_canonical_header(const DoutPrefixProvider *dpp,
                                    ...
                                    std::string &dest, const bool qsr)
{
    ...
    if (qsr) {
        ...
        if (header_time) {
            ...
            if (t.tm_year < 70) {
                ldpp_dout(dpp,
                          0) << "NOTICE: bad date (predates epoch): " <<
                             req_date << dendl;
                return false;
            }
                                        ...
                                        std::string_view &sessiontoken)     /* out */
{
    ...
    if (now >= req_sec + exp) {
        dout(10) << "NOTICE: now = " << now << ", req_sec = " << req_sec <<
                 ", exp = " << exp << dendl;
        return -EPERM;
    }
...
boost::container::small_vector < std::string_view, ExpectedStrNum > get_str_vec(const std::string_view &str,
        const char *const delims)
{
    boost::container::small_vector < std::string_view,
...
bool is_non_s3_op(RGWOpType op_type)
{
    if (op_type == RGW_STS_GET_SESSION_TOKEN ||
        ...
        op_type == RGW_OP_UNTAG_ROLE || op_type == RGW_OP_UPDATE_ROLE) {
        return true;
    }
    return false;
}
                         ...
                         const DoutPrefixProvider *dpp)
{
    ...
    ldpp_dout(dpp,
              10) << "credential scope = " << credential_scope << dendl;
                    ...
                    const string &region, const string &service)
{
    ...
    return fmt::
           format(FMT_STRING("{:d}{:02d}{:02d}/{:s}/{:s}/aws4_request"), year,
                  mon, day, region, service);
}
...
std::string get_v4_canonical_qs(const req_info &info, const bool using_qs)
{
    ...
    for (const auto &s : get_str_vec < 5 > (*params, "&")) {
        ...
        canonical_qs_map[aws4_uri_recode(key, true)] =
            aws4_uri_recode(val, true);
    }
...
static void add_v4_canonical_params_from_map(const map < string,
        string > &m, std::map < string,
        string > *result,
        bool is_non_s3_op)
{
    for (auto &entry : m) {
        ...
        (*result)[aws4_uri_recode(key, true)] =
            aws4_uri_recode(entry.second, true);
    }
}
...
std::string gen_v4_canonical_qs(const req_info &info, bool is_non_s3_op)
{
    ...
    add_v4_canonical_params_from_map(info.args.get_params(),
                                     &canonical_qs_map, is_non_s3_op);
        ...
        const bool force_boto2_compat)
{
    ...
    for (const auto &token : get_str_vec < 5 > (signedheaders, ";")) {
        ...
        if (!t) {
            dout(10) << "warning env var not available " << token_env.
                     c_str() << dendl;
            continue;
        }
 ...
 */
sha256_digest_t get_v4_canon_req_hash(CephContext *cct,
                                      ...
                                      const DoutPrefixProvider *dpp)
{
    ...
    ldpp_dout(dpp, 10) << "canonical request = " << sanitize {
        canonical_req} << dendl;
 ...
 */
AWSEngine::VersionAbstractor::string_to_sign_t get_v4_string_to_sign(CephContext *const cct,
...
static inline std::tuple < std::string_view,    /* date */
       ...
       std::string_view >          /* service */ parse_cred_scope(std::string_view credential_scope)
{
    /* date cred */
 ...
 */
static sha256_digest_t get_v4_signing_key(CephContext *const cct,
        const std::string_view &credential_scope,
 ...
 * dynamic allocations.
 */
AWSEngine::VersionAbstractor::server_signature_t get_v4_signature(const std::string_view &credential_scope,
...
{
    auto signing_key =
        get_v4_signing_key(cct, credential_scope, secret_key, dpp);
...
AWSEngine::VersionAbstractor::server_signature_t get_v2_signature(CephContext *const cct,
        ...
        string_to_sign_t &string_to_sign)
{
    ...
    const int ret = ceph_armor(std::begin(buf),
                               ...
                               reinterpret_cast <
                               const char * >(digest.v + digest.SIZE));
    if (ret < 0) {
        ...
        req_state *const s_rw)
{
    ...
    AWS_AUTHv4_IO(s_rw)->add_filter(std::static_pointer_cast < io_base_t >
                                    (shared_from_this()));
}
...
rgw::auth::Completer::cmplptr_t AWSv4ComplMulti::create(const req_state *const s,
...
rgw::auth::Completer::cmplptr_t AWSv4ComplSingle::create(const req_state *const s,
        const boost::optional < std::string > &)
{
