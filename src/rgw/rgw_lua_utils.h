...
// dump the lua stack to stdout
    ...
    class lua_state_guard {
      ...
      public:
        ...
        void reset(lua_State * _l = nullptr) {
            l = _l;
        }
    };
...
//
    ...
    template < typename MetaTable, typename ... Upvalues >
        void create_metatable(lua_State * L, bool toplevel,
                              Upvalues ... upvalues) {
        ...
        [[maybe_unused]] const auto rc =
            luaL_newmetatable(L, MetaTable::Name().c_str());
    ...
    template < typename MetaTable >
        void create_metatable(lua_State * L, bool toplevel,
                              std::unique_ptr < typename MetaTable::Type >
                              &ptr) {
        if (ptr) {
            create_metatable < MetaTable > (L, toplevel,
                                            reinterpret_cast <
                                            void *>(ptr.get()));
        }
        else {
...
// note, however, this is not mandatory to use it as a base
    struct EmptyMetaTable {
        ...
        // by default nothing is iterable// to change, overload this function in the derived
        ...
        static int error_unknown_field(lua_State * L, const std::string & index,
...
// and the "debug" library
    ...
    template < typename MapType = std::map < std::string, std::string >>
        ...
        const auto map =
            reinterpret_cast <
            MapType * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));

        const char *index = luaL_checkstring(L, 2);

        if (lua_isnil(L, 3) == 0) {
            const char *value = luaL_checkstring(L, 3);
            if (strnlen(value, MAX_LUA_VALUE_SIZE) +
                strnlen(index, MAX_LUA_VALUE_SIZE)
                > MAX_LUA_VALUE_SIZE) {
                return luaL_error(L,
