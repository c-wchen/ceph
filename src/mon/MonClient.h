...
class MonConnection {
  public:
    MonConnection(CephContext * cct,
                  ConnectionRef conn,
                  uint64_t global_id, AuthRegistry * auth_registry);
    ~MonConnection();
    ...
    int get_auth_request(uint32_t * method,
                         std::vector < uint32_t > *preferred_modes,
                         ceph::buffer::list * out,
                         const EntityName & entity_name,
                         uint32_t want_keys, RotatingKeyRing * keyring);
    int handle_auth_reply_more(AuthConnectionMeta * auth_meta,
                               const ceph::buffer::list & bl,
                               ceph::buffer::list * reply);
    int handle_auth_done(AuthConnectionMeta * auth_meta,
                         uint64_t global_id,
                         const ceph::buffer::list & bl,
  ...
  private:
    ...
    int _init_auth(uint32_t method,
                   const EntityName & entity_name,
                   uint32_t want_keys, RotatingKeyRing * keyring, bool msgr2);
...
struct MonClientPinger:public Dispatcher, public AuthClient {
    ...
    RotatingKeyRing *keyring;
     ...
     MonClientPinger(CephContext * cct_,
                     RotatingKeyRing * keyring,
                     std::string * res_):Dispatcher(cct_),
        result(res_), done(false), keyring(keyring) {
    } int wait_for_reply(double timeout = 0.0) {
        std::unique_lock locker {
        lock};
        if (timeout <= 0) {
            timeout =
                std::chrono::duration < double >(cct->_conf.get_val <
                                                 std::chrono::seconds >
...
class MonClient:public Dispatcher, public AuthClient, public AuthServer,    /* for mgr, osd, mds */
...
std::map < entity_addrvec_t,
    MonConnection >::iterator _find_pending_con(const ConnectionRef & con) {
...
struct MonCommand {
    ...
    :tid(t), onfinish(std::move(onfinish)) {
        auto timeout =
            monc.cct->_conf.get_val < std::chrono::seconds >
            ("rados_mon_op_timeout");
        if (timeout.count() > 0) {
            cancel_timer.emplace(monc.service, timeout);
            cancel_timer->async_wait([this,
                                      &monc] (boost::system::error_code ec) {
                                     if (ec)
                                     return;
...
void _finish_command(MonCommand * r, boost::system::error_code ret,
                     std::string_view rs, bufferlist && bl);
void _finish_auth();
...
template < typename CompletionToken >
    ...
    boost::asio::async_completion < CompletionToken, CommandSig > init(token);
    {
        ...
        if (!initialized || stopping) {
            ceph::async::post(std::move(h), monc_errc::shutting_down,
                              std::string {
                              }
                              , bufferlist {
                              }
            ...
            auto r =
                new MonCommand(*this, ++last_mon_command_tid, std::move(h));
            r->cmd = cmd;
            r->inbl = inbl;
...
template < typename CompletionToken >
    auto start_mon_command(int mon_rank, const std::vector < std::string > &cmd,
                           const ceph::buffer::list & inbl, CompletionToken
                           && token) {
    ldout(cct, 10) << __func__ << " cmd=" << cmd << dendl;
    boost::asio::async_completion < CompletionToken, CommandSig > init(token);
    {
...
class ContextVerter {
  ...
  public:
    ContextVerter(std::string * outs, ceph::bufferlist * outbl,
                  Context * onfinish)
    ...
    void operator() (boost::system::error_code e,
                     std::string s, ceph::bufferlist bl) {
        if (outs)
            *outs = std::move(s);
        if (outbl)
            *outbl = std::move(bl);
...
void start_mon_command(const std::vector < std::string > &cmd,
...
void start_mon_command(int mon_rank,
                       ...
                       const bufferlist & inbl, bufferlist * outbl,
                       std::string * outs, Context * onfinish) {
    start_mon_command(mon_rank, cmd, inbl,
...
void start_mon_command(const std::string & mon_name,    ///< mon name, with mon. prefix
                       ...
                       const bufferlist & inbl, bufferlist * outbl,
                       std::string * outs, Context * onfinish) {
    start_mon_command(mon_name, cmd, inbl,
                      ContextVerter(outs, outbl, onfinish));
}
...
template < typename CompletionToken >
    ...
    boost::asio::async_completion < CompletionToken, VersionSig > init(token);
    {
        ...
        version_requests.emplace(m->handle,
                                 VersionCompletion::create(service.
                                                           get_executor(),
                                                           std::move(init.
                                                                     completion_handler)));
        _send_mon_message(m);
...
template < typename Callback, typename ... Args >
    ...
    std::lock_guard l(monc_lock);
    return std::forward < Callback > (cb) (monmap,
