...
namespace librados {
    ...
    :m_client(rhs.m_client),
        ...
        m_namespace_name(rhs.m_namespace_name), m_snap_seq(rhs.m_snap_seq) {
        m_client->get();
        get();
    }
    ...
    void TestIoCtxImpl::aio_notify(const std::string & oid,
                                   AioCompletionImpl * c, bufferlist & bl,
                                   uint64_t timeout_ms, bufferlist * pbl) {
        m_pending_ops++;
        c->get();
        C_AioNotify *ctx = new C_AioNotify(this, c);
        m_client->get_watch_notify()->aio_notify(m_client, m_pool_id,
                                                 get_namespace(), oid, bl,
                                                 timeout_ms, pbl, ctx);
    }

    int TestIoCtxImpl::aio_operate(const std::string & oid,
        ...
        m_client->add_aio_operation(oid, true,
                                    std::bind(&TestIoCtxImpl::
                                              execute_aio_operations, this, oid,
                                              &ops,
                                              reinterpret_cast <
                                              bufferlist * >(0), m_snap_seq,
                                              snap_context !=
                                              NULL ? *snap_context : m_snapc,
                                              nullptr), c);
        return 0;
    }
    ...
    int TestIoCtxImpl::exec(const std::string & oid, TestClassHandler * handler,
        ...
        return (*call) (reinterpret_cast < cls_method_context_t >
                        (handler->get_method_context(this, oid, snap_id, snapc).
                         get()), &inbl, outbl);
    }
    ...
    int TestIoCtxImpl::list_watchers(const std::string & o,
        ...
        return m_client->get_watch_notify()->list_watchers(m_pool_id,
                                                           get_namespace(), o,
                                                           out_watchers);
    }
    ...
    void TestIoCtxImpl::notify_ack(const std::string & o, uint64_t notify_id,
        ...
        m_client->get_watch_notify()->notify_ack(m_client, m_pool_id,
                                                 get_namespace(), o, notify_id,
                                                 handle,
                                                 m_client->get_instance_id(),
                                                 bl);
    }
    ...
    int TestIoCtxImpl::operate(const std::string & oid,
        ...
        ops.get();
        m_pending_ops++;
        m_client->add_aio_operation(oid, false,
                                    std::bind(&TestIoCtxImpl::
                                              execute_aio_operations, this, oid,
                                              &ops,
                                              reinterpret_cast <
                                              bufferlist * >(0), m_snap_seq,
                                              m_snapc, nullptr), comp);
    ...
    void TestIoCtxImpl::aio_selfmanaged_snap_create(uint64_t * snapid,
                                                    AioCompletionImpl * c) {
        m_client->add_aio_operation("", true,
                                    std::bind(&TestIoCtxImpl::
                                              selfmanaged_snap_create, this,
                                              snapid), c);
    }
    ...
    int TestIoCtxImpl::execute_operation(const std::string & oid,
        ...
        TestRadosClient::Transaction transaction(m_client, get_namespace(),
                                                 oid);
