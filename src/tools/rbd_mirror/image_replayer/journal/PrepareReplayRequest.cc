            ...
            namespace journal {
                 ...
                 template < typename I > void PrepareReplayRequest < I >::send() {
                    ...
                    *m_syncing = false;
                    ...
                    if (m_state_builder->local_image_id !=
                        ...
                        finish(-EEXIST);
                        return;
                    } std::shared_lock image_locker(m_state_builder->
                                                    local_image_ctx->
                                                    image_lock);
                    if (m_state_builder->local_image_ctx->journal == nullptr) {
                        ...
                        m_state_builder->local_image_ctx->journal->
                        is_resync_requested(m_resync_requested);
                    if (r < 0) {
                        image_locker.unlock();
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        ...
                        I >::handle_update_client_state > (this);
                    m_state_builder->remote_journaler->update_client(data_bl,
                                                                     ctx);
                }

                template < typename I >
                    void PrepareReplayRequest <
                    I >::handle_update_client_state(int r) {
                    dout(15) << "r=" << r << dendl;
                    if (r < 0) {
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        PrepareReplayRequest < I >,
                        &PrepareReplayRequest <
                        I >::handle_get_remote_tag_class > (this);
                    m_state_builder->remote_journaler->
                        get_client(librbd::Journal <>::IMAGE_CLIENT_ID,
                                   &m_client, ctx);
                }
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        PrepareReplayRequest < I >,
                        &PrepareReplayRequest < I >::handle_get_remote_tags >
                        (this);
                    m_state_builder->remote_journaler->
                        get_tags(m_remote_tag_class, &m_remote_tags, ctx);
                }

                template < typename I >
                    void PrepareReplayRequest <
                    I >::handle_get_remote_tags(int r) {
                    dout(10) << "r=" << r << dendl;
                  ...
                  for (auto & remote_tag:m_remote_tags) {
                        ...
                        try {
                            ...
                            derr << "failed to decode remote tag " <<
                                remote_tag.tid << ": " << err.what() << dendl;
                            finish(-EBADMSG);
                            return;
                        }
                        ...
                        if (!m_local_tag_data.predecessor.commit_valid) {
                            ...
                            if (remote_tag_data.mirror_uuid !=
                                ...
                                dout(10) << "skipping non-primary remote tag" <<
                                    dendl;
                                continue;
                            }

                            dout(10) << "using initial primary remote tag" <<
                                dendl;
                            break;
                        }
                        ...
                        if (m_local_tag_data.mirror_uuid ==
                            ...
                            if (remote_tag_data.mirror_uuid ==
                                ...
                                *remote_orphan_tag_tid) {
                                ...
                                dout(10) << "found chained remote promotion tag"
                                    << dendl;
                    ...
                    if (remote_tag_data_valid &&
                        ...
                        dout(10) << "remote image was demoted/promoted" <<
                            dendl;
                    }
                    else {
