...
namespace rgw {
    ...
    namespace IAM {
        ...
        static constexpr std::uint64_t stsAssumeRoleWithWebIdentity =
            iamAll + 2;
        ...
        using NotAction_t = Action_t;
         ...
         template < size_t N >
            ...
            return s < 64 ? std::bitset < N > ((1ULL << s) - 1) :
                std::bitset < N > ((1ULL << 63) - 1) | make_bitmask < N >
                (s - 63) << 63;
        } template < size_t N > constexpr std::bitset < N >
            set_cont_bits(size_t start, size_t end) {
            return (make_bitmask < N > (end - start)) << start;
        } static const Action_t None(0);
        static const Action_t s3AllValue =
            set_cont_bits < allCount > (0, s3All);
        static const Action_t iamAllValue =
            set_cont_bits < allCount > (s3All + 1, iamAll);
        static const Action_t stsAllValue =
            set_cont_bits < allCount > (iamAll + 1, stsAll);
        static const Action_t allValue =
            set_cont_bits < allCount > (0, allCount);
...
// modify this function.
        ...
        struct Condition {
            ...
            using unordered_multimap_it_pair =
                std::pair < std::unordered_multimap < std::string,
                std::string >::const_iterator,
                std::unordered_multimap < std::string,
                std::string >::const_iterator >;

            template < typename F >
                static bool andible(F
                                    && f, const unordered_multimap_it_pair & it,
        ...
        struct Statement {
            ...
            Effect eval(const Environment & e,
                        ...
                        boost::optional < PolicyPrincipal & >princ_type =
                        boost::none) const;
        ...
        struct PolicyParseException:public std::exception {
            ...
            :pr(pr),
                msg(fmt::format("At character offset {}, {}",
                                ...
                                (pr.Code() ==
                                 rapidjson::
                                 kParseErrorTermination ? annotation :
        ...
        struct Policy {
            ...
            // backwards configuration.
             Policy(CephContext * cct, const std::string & tenant,
                    const bufferlist & text, bool reject_invalid_principals);
            ...
            Effect eval_conditions(const Environment & e) const;
             ...
             template < typename F >
                bool has_conditional(const std::string & conditional,
                                     F p) const {
            ...
            template < typename F >
                bool has_conditional_value(const std::string & conditional,
                                           F p)const {
                for (const auto & s:statements) {
                    if (std::any_of(s.conditions.begin(), s.conditions.end(),
                                    [&](const Condition & c) {
