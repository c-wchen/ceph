...
bool PgScrubber::should_abort() const const
{
...
void PgScrubber::initiate_scrub_after_repair(epoch_t epoch_queued)
{
    ...
    // queue
    if (check_interval(epoch_queued)) {
        dout(10) << "scrubber event -->> AfterRepairScrub epoch: " <<
            epoch_queued << dendl;
        reset_epoch(epoch_queued);
                                    ...
                                    Scrub::act_token_t token)
{
    dout(10) << "scrubber event -->> " << __func__ << " epoch: " << epoch_queued
        << " token: " << token << dendl;
    if (check_interval(epoch_queued) && is_token_current(token)) {
        m_fsm->process_event(SchedReplica {
                             }
...
bool PgScrubber::is_reserving() const const
{
    ...
    const const
{
    ...
    if (with_priority == Scrub::scrub_prio_t::high_priority) {
        qu_priority =
            std::max(qu_priority,
                     (unsigned int)m_pg->get_cct()->_conf->
...
{
    if (m_scrub_job) {
        dout(15) << __func__ << " prev. state: " << registration_state() <<
            dendl;
        m_osds->get_scrub_services().remove_from_osd_queue(m_scrub_job);
    }
}

bool PgScrubber::is_scrub_registered() const const
{
    return m_scrub_job && m_scrub_job->in_queues;
}

std::string_view PgScrubber::registration_state() constconst
{
    if (m_scrub_job) {
                                   ...
                                   const requested_scrub_t & request_flags)
{
    if (!m_scrub_job) {
        ...
        dout(10) << fmt::
            format("{}: (from {}& w/{}) {}.Reg-state:{:.7}. No scrub-job",
    ...
    if (is_primary()) {
        auto suggested =
            m_osds->get_scrub_services().determine_scrub_time(request_flags,
                                                              m_pg->info,
                                                              m_pg->
                                                              get_pgpool().info.
                                                              opts);
...
bool PgScrubber::reserve_local()
{
    ...
    dout(10) << __func__ << ": failed to reserve local scrub resources" <<
        dendl;
...
bool PgScrubber::has_pg_marked_new_updates() const const
{
...
bool PgScrubber::select_range()
{
    ...
    dout(20) << fmt::format("{} {} mins: {}d {}s, max: {}d {}s", __func__,
                            (m_is_deep ? "D" : "S"),
                            conf.get_val < int64_t > ("osd_scrub_chunk_min"),
                            conf.get_val < int64_t >
    ...
    const int min_from_conf =
        size_from_conf(m_is_deep, conf, "osd_scrub_chunk_min",
                       "osd_shallow_scrub_chunk_min");
    const int max_from_conf =
        size_from_conf(m_is_deep, conf, "osd_scrub_chunk_max",
                       "osd_shallow_scrub_chunk_max");
    ...
    dout(10) << fmt::format("{}: Min: {} Max: {} Div: {}", __func__,
                            min_chunk_sz, max_chunk_sz, divisor)
        << dendl;
    ...
    int ret =
        m_pg->get_pgbackend()->objects_list_partial(start, min_chunk_sz,
                                                    max_chunk_sz, &objects,
                                                    &candidate_end);
    ceph_assert(ret >= 0);
...
Scrub::BlockedRangeWarning PgScrubber::acquire_blocked_alarm()
{
    ...
    ceph::timespan grace_period {
        m_debug_blockrange ? 4 s : seconds {
        grace}
    };
...
void PgScrubber::add_delayed_scheduling()
{
    ...
    if (m_needs_sleep) {
        double scrub_sleep =
            1000.0 *
            m_osds->get_scrub_services().scrub_sleep_time(m_flags.required);
    ...
    if (sleep_time.count()) {
        ...
        dout(20) << __func__ << " scrub state is PendingTimer, sleeping" <<
            dendl;
        ...
        auto callbk = new LambdaContext([osds = m_osds, pgid, scrbr = this] ([[maybe_unused]] int r)mutable {
                                        PGRef pg =
                                        osds->osd->lookup_lock_pg(pgid);
                                        if (!pg) {
                                        lgeneric_subdout(g_ceph_context, osd,
                                                         10)
                                        <<
                                        ...
                                        osds->queue_for_scrub_resched(&(*pg),
                                                                      Scrub::
                                                                      scrub_prio_t::
                                                                      low_priority);
                                        pg->unlock();}
        ...
        // just a requeue
        m_osds->queue_for_scrub_resched(m_pg,
                                        Scrub::scrub_prio_t::high_priority);
    }
}

eversion_t PgScrubber::search_log_for_updates() const const
{
...
void PgScrubber::get_replicas_maps(bool replica_can_preempt)
{
    dout(10) << __func__ << " started in epoch/interval: " << m_epoch_start <<
        "/" << m_interval_start << " pg same_interval_since: " << m_pg->info.
        history.same_interval_since << dendl;
  ...
  for (const auto & i:m_pg->get_actingset()) {
        ...
        _request_scrub_map(i,
                           m_subset_last_update,
                           m_start, m_end, m_is_deep, replica_can_preempt);
    }

    dout(10) << __func__ << " awaiting" << m_maps_status << dendl;
}

bool PgScrubber::was_epoch_changed() const const
{
...
bool PgScrubber::are_all_maps_available() const const
{
    return m_maps_status.are_all_maps_available();
}

std::string PgScrubber::dump_awaited_maps() constconst
{
                                    ...
                                    bool deep, bool allow_preemption)
{
    ...
    auto repscrubop =
        new MOSDRepScrub(spg_t(m_pg->info.pgid.pgid, replica.shard),
                         version,
                         get_osdmap_epoch(),
    ...
    m_osds->send_message_osd_cluster(replica.osd, repscrubop,
                                     get_osdmap_epoch());
}
...
void PgScrubber::on_init()
{
    ...
    dout(10) << __func__ << " start same_interval:" << m_interval_start <<
        dendl;
    ...
    m_be = std::make_unique < ScrubBackend > (*this,
                                              *m_pg,
                                              m_pg_whoami,
                                              m_is_repair,
                                              m_is_deep ? scrub_level_t::
                                              deep : scrub_level_t::shallow,
                                              m_pg->get_actingset());
    ...
    //  create a new store
    {
        ...
        m_store.
            reset(Scrub::Store::
                  create(m_pg->osd->store, &t, m_pg->info.pgid, m_pg->coll));
...
void PgScrubber::on_replica_init()
{
    ...
    if (!m_active) {
        m_be =
            std::make_unique < ScrubBackend > (*this, *m_pg, m_pg_whoami,
                                               m_is_repair,
                                               m_is_deep ? scrub_level_t::
                                               deep : scrub_level_t::shallow);
        m_active = true;
                                      ...
                                      hobject_t start, hobject_t end, bool deep)
{
    ...
    while (pos.empty()) {
        ...
        dout(10) << __func__ << " pos.ls.empty()? " << (pos.ls.
                                                        empty()? "+" : "-")
            << dendl;
        if (pos.ls.empty()) {
            break;
...
void PgScrubber::repair_oinfo_oid(ScrubMap & smap)
{
    for (auto i = smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
        ...
        if (oi.soid != hoid) {
            ...
            m_osds->clog->error()
                << "osd." << m_pg_whoami << " found object info error on pg " <<
                m_pg_id << " oid " << hoid << " oid in object info: " << oi.
                soid << "...repaired";
            // Fix object info
            oi.soid = hoid;
            bl.clear();
            encode(oi,
                   bl,
                   m_pg->get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD,
                                                    nullptr));
            ...
            if (r != 0) {
                derr << __func__ << ": queue_transaction got " <<
                    cpp_strerror(r)
                    << dendl;
            }
        }
...
void PgScrubber::persist_scrub_results(inconsistent_objs_t && all_errors)
{
  ...
  for (auto & e:all_errors) {
        std::visit([this] (auto & e) {
                   m_store->add_error(m_pg->pool.id, e);
                   }
                   , e);
    }
...
void PgScrubber::apply_snap_mapper_fixes(const std::vector < snap_mapper_fix_t >
                                         &fix_list)
{
        ...
        if (fix_op != snap_mapper_op_t::add) {
            ...
            m_osds->clog->
                ...
                ("osd.{} found snap mapper error on pg {} oid {} snaps in mapper: {}, "
                 "oi: " "{} ...repaired", m_pg_whoami, m_pg_id, hoid,
                 bogus_snaps, snaps);
            ...
            m_osds->clog->
                ...
                ("osd.{} found snap mapper error on pg {} oid {} snaps missing in "
                 "mapper, should be: {} ...repaired", m_pg_whoami, m_pg_id,
                 hoid, snaps);
        }
...
{
    m_pg->add_objects_scrubbed_count(m_be->get_primary_scrubmap().objects.
                                     size());
    ...
    if (!required_fixes.inconsistent_objs.empty()) {
        if (state_test(PG_STATE_REPAIR)) {
            dout(10) << __func__ << ": discarding scrub results (repairing)" <<
                dendl;
        }
        else {
...
void PgScrubber::replica_scrub_op(OpRequestRef op)
{
    ...
    if (is_queued_or_active()) {
        ...
        scrub_clear_state();
        m_osds->clog->
            warn() << fmt::format("{}: after a reset. Now handling the new OP",
                                  __func__);
    }
    ...
    m_replica_request_priority = msg->high_priority
        ...
        : Scrub::scrub_prio_t::low_priority;
    m_flags.priority =
        msg->priority ? msg->priority : m_pg->get_scrub_priority();
...
ScrubMachineListener::MsgAndEpoch PgScrubber::
prep_replica_map_msg(PreemptionNoted was_preempted)
{
    dout(10) << __func__ << " min epoch:" << m_replica_min_epoch << dendl;

    auto reply =
        make_message < MOSDRepScrubMap >
...
void PgScrubber::send_preempted_replica()
{
    ...
    m_pg->send_cluster_message(m_pg->get_primary().osd,
                               reply, m_replica_min_epoch, false);
}
...
void PgScrubber::handle_scrub_reserve_request(OpRequestRef op)
{
    ...
    dout(20) << fmt::format("{}: request_ep:{} recovery:{}",
                            __func__, request_ep, m_osds->is_recovery_active())
        << dendl;
    ...
    if (request_ep < m_pg->get_same_interval_since()) {
        ...
        dout(10) << fmt::
            format("{}: stale reservation (request ep{} < {}) denied", __func__,
    ...
    Message *reply =
        new
        MOSDScrubReserve(spg_t(m_pg->info.pgid.pgid, m_pg->get_primary().shard),
                         request_ep,
                         granted ? MOSDScrubReserve::GRANT : MOSDScrubReserve::
                         REJECT,
                         m_pg_whoami);
...
bool PgScrubber::is_queued_or_active() const const
{
...
void PgScrubber::scrub_finish()
{
    ...
    if (m_is_repair && m_flags.auto_repair &&
        ...
        static_cast <
        int >(m_pg->cct->_conf->osd_scrub_auto_repair_num_errors)) {
    ...
    if (m_flags.deep_scrub_on_error && m_be->authoritative_peers_count() &&
        ...
        do_auto_scrub = true;
        dout(15) << __func__ << " Try to auto repair after scrub errors" <<
            dendl;
    }
    ...
    if (m_is_repair) {
        ...
        if (m_fixed_count == m_shallow_errors + m_deep_errors) {
            ...
            m_planned_scrub.req_scrub = m_planned_scrub.req_scrub
                || m_flags.required;
...

    {
        ...
        m_pg->recovery_state.update_stats([this] (auto & history, auto & stats) {
                                          dout(10) <<
                                          "m_pg->recovery_state.update_stats() errors:"
                                          << m_shallow_errors << "/" <<
                                          m_deep_errors << " deep? " <<
                                          m_is_deep << dendl;
                                          utime_t now = ceph_clock_now();
                                          history.last_scrub =
                                          m_pg->recovery_state.get_info().
                                          last_update;
                                          ...
                                          m_shallow_errors;
                                          stats.stats.sum.
                                          num_deep_scrub_errors = m_deep_errors;
                                          auto omap_stats =
                                          m_be->this_scrub_omapstats();
                                          stats.stats.sum.
                                          num_large_omap_objects =
                                          omap_stats.large_omap_objects;
                                          stats.stats.sum.num_omap_bytes =
                                          omap_stats.omap_bytes;
                                          stats.stats.sum.num_omap_keys =
                                          omap_stats.omap_keys;
                                          dout(19) << "scrub_finish shard " <<
                                          m_pg_whoami << " num_omap_bytes = " <<
                                          stats.stats.sum.
                                          num_omap_bytes << " num_omap_keys = "
                                          << stats.stats.sum.
                                          num_omap_keys << dendl;}
                                          else {
                                          stats.stats.sum.
                                          ...
                                          if (m_pg->info.stats.stats.sum.
                                              num_scrub_errors) {
                                          state_set(PG_STATE_FAILED_REPAIR);
                                          dout(10) << "scrub_finish " << m_pg->
                                          info.stats.stats.sum.
                                          num_scrub_errors <<
                                          " error(s) still present after re-scrub"
                                          << dendl;}
                                          }
                                          return true;}
                                          , &t) ;
...
void PgScrubber::dump_active_scrubber(ceph::Formatter * f, bool is_deep) const const
{
...
pg_scrubbing_status_t PgScrubber::get_schedule() const const
{
    if (!m_scrub_job) {
    ...
    dout(25) << fmt::format("{}: active:{} blocked:{}",
                            __func__, m_active, m_scrub_job->blocked)
        << dendl;
    ...
    if (m_active) {
        ...
        if (m_scrub_job->blocked) {
            ...
            int32_t blocked_for = (utime_t { now_is }
                                   -m_scrub_job->blocked_since).sec();
            return pg_scrubbing_status_t {
                utime_t {
                }
                , blocked_for, pg_scrub_sched_status_t::blocked, true,  // active
            (m_is_deep ? scrub_level_t::deep : scrub_level_t::shallow),
                    false};

        }
        else {
            int32_t duration = (utime_t { now_is }
                                -scrub_begin_stamp).sec();
            return pg_scrubbing_status_t {
                utime_t {
                }
                , duration, pg_scrub_sched_status_t::active, true,  // active
            (m_is_deep ? scrub_level_t::deep : scrub_level_t::shallow),
    ...
    if (m_scrub_job->state != ScrubQueue::qu_state_t::registered) {
        ...
        , 0, pg_scrub_sched_status_t::not_queued,
                false, scrub_level_t::shallow, false};
    }
    ...
    bool deep_expected = (now_is >= m_pg->next_deepscrub_interval()) ||
        m_planned_scrub.must_deep_scrub || m_planned_scrub.need_auto;
    ...
    if (now_is > m_scrub_job->schedule.scheduled_at) {
        ...
        return pg_scrubbing_status_t {
        m_scrub_job->schedule.scheduled_at,
                0,
                pg_scrub_sched_status_t::queued,
                false, expected_level, periodic};
    }
...
, m_planned_scrub {
    pg->get_planned_scrub(ScrubberPasskey {
                          }
)}
...
void PgScrubber::set_scrub_begin_time()
{
    ...
    m_osds->clog->debug() << fmt::format("{} {} starts",
                                         m_pg->info.pgid.pgid, m_mode_desc);
}
...
const OSDMapRef & PgScrubber::get_osdmap() const const
{
    return m_pg->get_osdmap();
}

LoggerSinkSet & PgScrubber::get_logger() constconst
{
...
std::ostream & PgScrubber::gen_prefix(std::ostream & out) const const
{
    if (m_pg) {
        return m_pg->
            gen_prefix(out) << "scrubber<" << m_fsm_state_name << ">: ";
    }
    else {
...
void PgScrubber::log_cluster_warning(const std::string & warning) const const
{
    m_osds->clog->do_log(CLOG_WARN, warning);
}

ostream & PgScrubber::show(ostream & out) const const
{
    return out << " [ " << m_pg_id << ": " << m_flags << " ] ";
}

int PgScrubber::asok_debug(std::string_view cmd,
                           std::string param, Formatter * f, stringstream & ss)
{
...
void PgScrubber::update_scrub_stats(ceph::coarse_real_clock::time_point now_is)
{
    ...
    period_inactive};
    if (m_active) {
        period = m_debug_blockrange ? 2 s : period_active;
    }
    ...
    if (g_conf()->subsys.should_gather < ceph_subsys_osd, 20 > ()) {
        ...
        dout(20) << fmt::format("{}: period: {}/{}-> {} last:{}",
                                __func__,
                                period_active, period_inactive, period, buf)
            << dendl;
    }
...
PgScrubber::preemption_data_t::preemption_data_t(PG * pg):m_pg
{
pg}

{
    m_left =
        static_cast < int >(m_pg->get_cct()->_conf.get_val < uint64_t >
                            ("osd_scrub_max_preemptions"));
}

void PgScrubber::preemption_data_t::reset()
{
    std::lock_guard < ceph::mutex > lk {
    m_preemption_lock};

    m_preemptable = false;
    m_preempted = false;
    m_left =
        static_cast < int >(m_pg->cct->_conf.get_val < uint64_t >
                            ("osd_scrub_max_preemptions"));
    m_size_divisor = 1;
}
...
namespace Scrub {
    ...
    pg->get_actingset()}
    , m_osds {
    m_pg->get_pg_osd(ScrubberPasskey())}
    , m_pending {
    static_cast < int >(m_acting_set.size()) - 1}, m_pg_info {
    m_pg->get_pg_info(ScrubberPasskey())}
    , m_scrub_job {
    scrubjob}
    , m_conf {
    conf}
    {
        ...
        m_timeout =
            conf.get_val < std::chrono::milliseconds >
            ("osd_scrub_slow_reservation_response");
        m_log_msg_prefix =
            fmt::format("osd.{} ep: {} scrubber::ReplicaReservations pg[{}]: ",
                        m_osds->whoami, epoch, pg->pg_id);
        ...
        if (m_pending <= 0) {
            ...
            m_no_reply =
                make_unique < ReplicaReservations::no_reply_t > (m_osds, m_conf,
                                                                 *this,
                                                                 m_log_msg_prefix);
          ...
          for (auto p:m_acting_set) {
                if (p == whoami)
                    continue;
                auto m =
                    new MOSDScrubReserve(spg_t(m_pg_info.pgid.pgid, p.shard),
                                         epoch, MOSDScrubReserve::REQUEST,
                                         m_pg->pg_whoami);
 ...
 */
    void ReplicaReservations::handle_reserve_grant(OpRequestRef op,
                                                   pg_shard_t from) {
...

        {
            ...
            // cleaner
            auto w =
                find(m_waited_for_peers.begin(), m_waited_for_peers.end(),
                     from);
    ...
    void ReplicaReservations::handle_no_reply_timeout() {
        dout(1) << fmt::format("{}: timeout! no reply from {}", __func__,
                               m_waited_for_peers)
            << dendl;
    ...
    } ReplicaReservations::no_reply_t::no_reply_t(OSDService * osds,
                                                  const ConfigProxy & conf,
    ...
    log_prfx}
    {
        ...
        m_abort_callback =
            new LambdaContext([this, now_is] ([[maybe_unused]] int r) {
                              // behave as if a REJECT was received
                              m_osds->clog->
                              warn() << fmt::
                              format
                              ("{} timeout on replica reservations (since {})",
                               m_log_prfx, now_is);
...
// note: no dout()s in LocalReservation functions. Client logs interactions.
  LocalReservation::LocalReservation(OSDService * osds):m_osds {
    osds}
    {
...
// ///////////////////// ReservedByRemotePrimary ///////////////////////////////
    ...
    ReservedByRemotePrimary::ReservedByRemotePrimary(const PgScrubber *
                                                     scrubber, PG * pg,
                                                     OSDService * osds,
                                                     epoch_t epoch)
    :m_scrubber {
    ...
    epoch}
    {
        if (!m_osds->get_scrub_services().inc_scrubs_remote()) {
            dout(10) << __func__ << ": failed to reserve at Primary request" <<
                dendl;
...
// ///////////////////// MapsCollectionStatus ////////////////////////////////
    ...
    ->std::tuple < bool, std::string_view > {
        auto fe =
            std::find(m_maps_awaited_for.begin(), m_maps_awaited_for.end(),
                      from);
  ...
  :    m_osds {
    ...
    pg_id}
    {
        ...
        m_callbk = new LambdaContext([this, now_is] ([[maybe_unused]] int r) {
                                     ...
                                     << buf << ")" << dendl;
                                     m_osds->clog->warn() << "osd." << m_osds->
                                     whoami << " PgScrubber: " << m_pgid <<
                                     " blocked on an object for too long (since "
                                     << buf << ")"; m_warning_issued = true;
                                     m_scrubber.set_scrub_blocked(utime_t {
                                                                  now_c, 0}
                                     ); return;}
        );
