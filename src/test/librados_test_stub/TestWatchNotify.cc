...
namespace librados {
    ...
    std::ostream & operator<<(std::ostream & out,
        ...
        :test_watch_notify(test_watch_notify), pool_id(pool_id),
            ...
            auto ctx =
                new
                LambdaContext([_test_watch_notify, _pool_id, _nspace,
                               _oid] (int r){
                              _test_watch_notify->
                              handle_object_removed(_pool_id, _nspace, _oid);});
  ...
  :    m_test_cluster(test_cluster) {
    ...
    int TestWatchNotify::list_watchers(int64_t pool_id,
    ...
    int TestWatchNotify::watch(TestRadosClient * rados_client, int64_t pool_id,
        ...
        C_SaferCond cond;
        aio_watch(rados_client, pool_id, nspace, o, gid, handle, ctx, ctx2,
                  &cond);
        return cond.wait();
    }
    ...
    void TestWatchNotify::aio_watch(TestRadosClient * rados_client,
        ...
        auto ctx = new LambdaContext([ =, this] (int){
                                     execute_watch(rados_client, pool_id,
                                                   nspace, o, gid, handle,
                                                   watch_ctx,
    ...
    void TestWatchNotify::aio_unwatch(TestRadosClient * rados_client,
        ...
        auto ctx =
            new LambdaContext([this, rados_client, handle, on_finish] (int){
                              execute_unwatch(rados_client, handle,
                                              on_finish);});
        rados_client->get_aio_finisher()->queue(ctx);
    }
    ...
    void TestWatchNotify::aio_notify(TestRadosClient * rados_client,
                                     ...
                                     const bufferlist & bl, uint64_t timeout_ms,
                                     bufferlist * pbl, Context * on_notify) {
        auto ctx = new LambdaContext([ =, this] (int){
                                     execute_notify(rados_client, pool_id,
                                                    nspace, oid, bl, pbl,
                                                    on_notify);});
        rados_client->get_aio_finisher()->queue(ctx);
    }
    ...
    int TestWatchNotify::notify(TestRadosClient * rados_client, int64_t pool_id,
        ...
        C_SaferCond cond;
        aio_notify(rados_client, pool_id, nspace, oid, bl, timeout_ms, pbl,
                   &cond);
        return cond.wait();
    }
    ...
    void TestWatchNotify::execute_watch(TestRadosClient * rados_client,
        ...
        watch_handle.addr =
            "127.0.0.1:0/" + stringify(rados_client->get_nonce());
        ...
        *handle = watch_handle.handle;
        ...
        ldout(cct,
              20) << "oid=" << o << ", gid=" << gid << ": handle=" << *handle <<
            dendl;
        m_lock.unlock();
    ...
    void TestWatchNotify::execute_unwatch(TestRadosClient * rados_client,
        ...
        ldout(cct, 20) << "handle=" << handle << dendl;
        {
            ...
            for (FileWatchers::iterator it = m_file_watchers.begin();
                ...
                WatchHandles::iterator w_it =
                    watcher->watch_handles.find(handle);
    ...
    TestWatchNotify::SharedWatcher TestWatchNotify::get_watcher(int64_t pool_id,
                                                                const std::
                                                                string & nspace,
                                                                const std::
                                                                string & oid) {
        ...
        if (it == m_file_watchers.end()) {
            ...
            watcher->object_handler.
                reset(new ObjectHandler(this, pool_id, nspace, oid));
            int r =
                m_test_cluster->register_object_handler(pool_id, { nspace,
                                                        oid },
                                                        watcher->object_handler.
                                                        get());
            if (r < 0) {
    ...
    void TestWatchNotify::maybe_remove_watcher(SharedWatcher watcher) {
        ...
        if (watcher->watch_handles.empty() && watcher->notify_handles.empty()) {
            ...
            if (watcher->object_handler) {
                m_test_cluster->unregister_object_handler(pool_id, {
                                                          nspace, oid}
                                                          ,
                                                          watcher->
                                                          object_handler.get());
    ...
    void TestWatchNotify::execute_notify(TestRadosClient * rados_client,
      ...
      for (auto & watch_handle_pair:watch_handles) {
            ...
            notify_handle->pending_watcher_ids.
                insert(std::make_pair(watch_handle.gid, watch_handle.handle));

            m_async_op_tracker.start_op();
            uint64_t notifier_id = rados_client->get_instance_id();
            watch_handle.rados_client->get_aio_finisher()->
                queue(new
                      LambdaContext([this, pool_id, nspace, oid, bl, notify_id,
                                     watch_handle, notifier_id] (int r) {
                                    bufferlist notify_bl; notify_bl.append(bl);
                                    if (watch_handle.watch_ctx2 != NULL) {
                                    watch_handle.watch_ctx2->
                                    handle_notify(notify_id,
                                                  watch_handle.handle,
                                                  notifier_id, notify_bl);}
                                    else
                                    if (watch_handle.watch_ctx != NULL) {
                                    watch_handle.watch_ctx->notify(0, 0,
                                                                   notify_bl);
                                    // auto ack old-style watch/notify clients
                                    ack_notify(watch_handle.rados_client,
                                               pool_id, nspace, oid, notify_id,
                                               {
                                               watch_handle.gid,
                                               watch_handle.handle}
                                               , bufferlist());}
    ...
    void TestWatchNotify::finish_notify(TestRadosClient * rados_client,
        ...
        notify_handle->rados_client->get_aio_finisher()->queue(notify_handle->
                                                               on_notify, 0);
    ...
    void TestWatchNotify::handle_object_removed(int64_t pool_id,
      ...
      for (auto & notify_handle_pair:watcher->notify_handles) {
            ...
            notify_handle->rados_client->get_aio_finisher()->
                queue(notify_handle->on_notify, -ENOENT);
        }
      ...
      for (auto & watch_handle_pair:watcher->watch_handles) {
            ...
            if (watch_ctx2 != nullptr) {
                auto ctx = new LambdaContext([handle, watch_ctx2] (int){
                                             watch_ctx2->handle_error(handle,
                                                                      -ENOTCONN);});
