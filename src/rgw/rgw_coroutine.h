...
struct rgw_spawned_stacks {
    ...
    void inherit(rgw_spawned_stacks * source) {
        for (vector < RGWCoroutinesStack * >::iterator iter =
             source->entries.begin(); iter != source->entries.end(); ++iter) {
            add_pending(*iter);
        }
...
#define MAX_COROUTINE_HISTORY 10
    ...
    struct Status {
        ...
        Status(CephContext * _cct):cct(_cct),
            lock("RGWCoroutine::Status::lock"),
...
class RGWCoroutinesManagerRegistry:public RefCountedObject,
  ...
  public:
    RGWCoroutinesManagerRegistry(CephContext * _cct):cct(_cct),
...
class RGWCoroutinesManager {
    ...
    void handle_unblocked_stack(set < RGWCoroutinesStack * >&context_stacks,
                                list < RGWCoroutinesStack * >&scheduled_stacks,
