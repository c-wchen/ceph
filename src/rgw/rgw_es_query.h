...
class ESInfixQueryParser {
  ...
  public:
     explicit ESInfixQueryParser(const std::string & _query):query(_query),
        size(query.size()), str(query.c_str()) {
...
class ESQueryCompiler {
    ...
    ESEntityTypeMap *generic_type_map {
    nullptr};
    ESEntityTypeMap *custom_type_map {
    nullptr};

     std::map < std::string, std::string, ltstr_nocase > *field_aliases =
        nullptr;
  ...
  public:
    ...
    const std::string & get_custom_prefix() {
    ...
    void set_field_aliases(std::map < std::string, std::string,
                           ltstr_nocase > *fa) {
        field_aliases = fa;
    }
