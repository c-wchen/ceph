    ...
    namespace mirror {
        ...
        namespace {
            ...
            int flush(librbd::ImageCtx * image_ctx) {
                C_SaferCond ctx;
                auto aio_comp =
                    librbd::io::AioCompletion::create_and_start(&ctx, image_ctx,
                                                                librbd::io::
                                                                AIO_TYPE_FLUSH);
                auto req =
                    librbd::io::ImageDispatchSpec::create_flush(*image_ctx,
                                                                librbd::io::
                ...
                for (int i = 0; i < num_ops; i++) {
                    ...
                    if (rand() % 4 == 0) {
                        ASSERT_EQ((int)len,
                                  librbd::api::Io <>::discard(*image_ctx, off,
                                                              len,
                                                              image_ctx->
                        ...
                        ASSERT_EQ((int)len,
                                  librbd::api::Io <>::write(*image_ctx, off,
                                                            len, std::move(bl),
                                                            0));
                    }
                }
        ...
        class TestImageSync:public TestFixture {
          public:
            ...
            void SetUp() override {
                ...
                auto cct =
                 ...
                 m_image_sync_throttler =
                    rbd::mirror::Throttler <>::create(cct,
                                                      "rbd_mirror_concurrent_image_syncs");

                 m_instance_watcher =
                    rbd::mirror::InstanceWatcher <>::create(m_local_io_ctx,
                                                            *m_threads->
                                                            asio_engine,
                                                            nullptr,
                                                            m_image_sync_throttler);
                ...
                ContextWQ *context_wq;
                 ...
                 m_remote_journaler =
                    new::journal::Journaler(context_wq, m_threads->timer,
                ...
                ASSERT_EQ(0,
                          m_remote_journaler->register_client(client_data_bl));

                m_state_builder =
                    rbd::mirror::image_replayer::journal::StateBuilder <
                ...
                m_sync_point_handler =
                    m_state_builder->create_sync_point_handler();
            }
            ...
            void create_and_open(librados::IoCtx & io_ctx,
                                 librbd::ImageCtx ** image_ctx) {
                librbd::RBD rbd;
                ASSERT_EQ(0,
                          create_image(rbd, io_ctx, m_image_name,
                                       m_image_size));
            ...
            ImageSync <> *create_request(Context * ctx) {
                return new ImageSync <> (m_threads, m_local_image_ctx,
            ...
            rbd::mirror::image_replayer::journal::StateBuilder <
                librbd::ImageCtx > *m_state_builder = nullptr;
            rbd::mirror::image_sync::SyncPointHandler * m_sync_point_handler =
                nullptr;
        };
        ...
        TEST_F(TestImageSync, Simple) {
            ...
            int64_t object_size =
                std::min < int64_t > (m_remote_image_ctx->size,
                                      1 << m_remote_image_ctx->order);
            ...
            for (uint64_t offset = 0; offset < m_remote_image_ctx->size;
                 offset += object_size) {
                ASSERT_LE(0,
                          librbd::api::Io <>::read(*m_remote_image_ctx, offset,
                                                   object_size,
                                                   librbd::io::ReadResult {
                                                   &read_remote_bl}
                                                   , 0));
                ASSERT_LE(0,
                          librbd::api::Io <>::read(*m_local_image_ctx, offset,
                                                   object_size,
                                                   librbd::io::ReadResult {
                                                   &read_local_bl}
                                                   , 0));
        ...
        TEST_F(TestImageSync, Resize) {
            ...
            bl.append(std::string(len, '1'));
            ASSERT_EQ((int)len,
                      librbd::api::Io <>::write(*m_remote_image_ctx, off, len,
                                                std::move(bl), 0));
            {
                std::shared_lock owner_locker {
            ...
            ASSERT_LE(0, librbd::api::Io <>::read(*m_remote_image_ctx, off, len,
                                                  librbd::io::ReadResult {
                                                  &read_remote_bl}, 0));
            ASSERT_LE(0, librbd::api::Io <>::read(*m_local_image_ctx, off, len,
                                                  librbd::io::ReadResult {
                                                  &read_local_bl}, 0));
        ...
        TEST_F(TestImageSync, Discard) {
            ...
            ASSERT_EQ((int)len - 2,
                      librbd::api::Io <>::discard(*m_remote_image_ctx, off + 1,
                                                  len - 2,
                                                  m_remote_image_ctx->
                                                  discard_granularity_bytes));
            {
        ...
        TEST_F(TestImageSync, SnapshotStress) {
          ...
          for (auto & snap_name:snap_names) {
                ...
                uint64_t remote_size;
                {
                    ...
                    m_remote_image_ctx->image_lock};
                    remote_size =
                        m_remote_image_ctx->get_image_size(m_remote_image_ctx->
                                                           snap_id);
                }
                ...
                uint64_t local_size;
                {
                    ...
                    m_local_image_ctx->image_lock};
                    local_size =
                        m_local_image_ctx->get_image_size(m_local_image_ctx->
                                                          snap_id);
                    bool flags_set;
                    ASSERT_EQ(0,
                              m_local_image_ctx->test_flags(m_local_image_ctx->
                                                            snap_id,
                ...
                for (uint64_t offset = 0; offset < remote_size;
                     offset += object_size) {
                    ASSERT_LE(0,
                              librbd::api::Io <>::read(*m_remote_image_ctx,
                                                       offset, object_size,
                                                       librbd::io::ReadResult {
                                                       &read_remote_bl}
                                                       , 0));
                    ASSERT_LE(0,
                              librbd::api::Io <>::read(*m_local_image_ctx,
                                                       offset, object_size,
                                                       librbd::io::ReadResult {
                                                       &read_local_bl}
                                                       , 0));
