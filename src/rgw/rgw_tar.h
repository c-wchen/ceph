...
static inline std::pair < class StatusIndicator,
           boost::optional < class HeaderView >> interpret_block(const StatusIndicator &status,
                       ceph::bufferlist &bl);
...
protected:
    ...
    } size_t get_filesize() const
    {
         ...
         * metadata in ASCII. */
        const std::string_view raw(header->filesize,
        ...
        const auto trimmed = raw.substr(0,
                                        pad_ends_at ==
                                        std::string_view::npos ? std::
                                        string_view::
                                        npos : pos2len(pad_ends_at));
...
static inline std::pair < StatusIndicator,
       boost::optional < HeaderView >> interpret_block(const StatusIndicator &status,
               ceph::bufferlist &bl)
{
    ...
    if (std::memcmp(zero_block.data(), block, BLOCK_SIZE) == 0) {
        return std::make_pair(StatusIndicator(status, true),
                              boost::none);
    } else {
        return std::make_pair(StatusIndicator(status, false),
                              HeaderView(block));
    }
}
