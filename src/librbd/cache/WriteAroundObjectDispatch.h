...
namespace librbd {
    ...
    namespace cache {
      ...
      template < typename ImageCtxT = ImageCtx > class WriteAroundObjectDispatch:public io::
            ObjectDispatchInterface
        {
          public:
            ...
            bool read(uint64_t object_no, io::ReadExtents * extents,
            ...
            bool discard(uint64_t object_no, uint64_t object_off,
            ...
            bool write(uint64_t object_no, uint64_t object_off, ceph::bufferlist
            ...
            bool write_same(uint64_t object_no, uint64_t object_off,
            ...
            bool compare_and_write(uint64_t object_no, uint64_t object_off,
                                   ceph::bufferlist
            ...
            bool flush(io::FlushSource flush_source,
                       const ZTracer::Trace & parent_trace,
                       uint64_t * journal_tid,
                       io::DispatchResult * dispatch_result,
                       Context ** on_finish, Context * on_dispatched) override;

            bool list_snaps(uint64_t object_no, io::Extents
                            && extents, io::SnapIds
                            && snap_ids, int list_snap_flags,
                            const ZTracer::Trace & parent_trace,
            ...
            void extent_overwritten(uint64_t object_no, uint64_t object_off,
                                    uint64_t object_len, uint64_t journal_tid,
                                    uint64_t new_journal_tid) override {
            }

            int prepare_copyup(uint64_t object_no,
                               io::SnapshotSparseBufferlist *
                               snapshot_sparse_bufferlist) override {
                return 0;
            }

          private:
            struct QueuedIO {
                QueuedIO(uint64_t length, Context * on_finish,
                         Context * on_dispatched)
                :length(length), on_finish(on_finish),
            ...
            typedef std::map < uint64_t,
                InFlightObjectExtents > InFlightExtents;
            ...
            bool dispatch_unoptimized_io(uint64_t object_no,
                                         uint64_t object_off,
                                         uint64_t object_len,
                                         io::DispatchResult * dispatch_result,
                                         Context * on_dispatched);
            ...
            bool block_overlapping_io(InFlightObjectExtents *
                                      in_flight_object_extents,
                                      uint64_t object_off, uint64_t object_len);
            void unblock_overlapping_ios(uint64_t object_no,
                                         uint64_t object_off,
                                         uint64_t object_len,
            ...
            void handle_in_flight_io_complete(int r, uint64_t tid,
                                              uint64_t object_no,
                                              uint64_t object_off,
                                              uint64_t object_len);
...
extern template class librbd::cache::WriteAroundObjectDispatch <
    librbd::ImageCtx >;
