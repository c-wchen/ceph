...
static void get_entries_by_object(librados::IoCtx & ioctx, string & oid,
                                  ...
                                  string & object, string & op_id, int expected)
{
    /* search everything */
    ...
    bufferlist obl;
    bool truncated;
    cls_statelog_list(*rop, empty_str, op_id, object, marker, 0, entries,
                      &marker, &truncated);
                                     ...
                                     int expected)
{
    ...
    bufferlist obl;
    bool truncated;
    cls_statelog_list(*rop, client_id, op_id, empty_str, marker, 0, entries,
                      &marker, &truncated);
    ASSERT_EQ(0, ioctx.operate(oid, rop, &obl));
    ASSERT_EQ(expected, (int)entries.size());
    delete rop;
...
static void get_all_entries(librados::IoCtx & ioctx, string & oid,
                            list < cls_statelog_entry > &entries, int expected)
{
    /* search everything */
    string object, op_id;
...
TEST(cls_rgw, test_statelog_basic)
{
    ...
    for (int j = 0; j < 2; j++) {
        ...
        cls_statelog_list(*rop, cid, op_id, obj, marker, 1, entries, &marker,
                          &truncated);
        ASSERT_EQ(0, ioctx.operate(oid, rop, &obl));
        ...
        reset_rop(&rop);
        marker.clear();
        cls_statelog_list(*rop, cid, op_id, obj, marker, 0, entries, &marker,
                          &truncated);
        obl.clear();
        ASSERT_EQ(0, ioctx.operate(oid, rop, &obl));
