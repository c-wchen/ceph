...
struct segment_info_t {
    ...
    } bool is_open() const
    {
        return state == Segment::segment_state_t::OPEN;
                       ...
                       data_category_t, rewrite_gen_t, segment_off_t);
    ...
    void update_modify_time(sea_time_point _modify_time,
                            std::size_t _num_extents)
    {
        ...
        if (modify_time == NULL_TIME) {
            ...
            modify_time =
                get_average_time(modify_time, num_extents, _modify_time,
                                 _num_extents);
            num_extents += _num_extents;
        }
    }
...
public:
    ...
    } journal_seq_t get_submitted_journal_head() const
    {
        ...
        return journal_seq_t {
            segment_info.seq,
            paddr_t::make_seg_paddr(journal_segment_id,
    ...
    void update_modify_time(segment_id_t id, sea_time_point tp,
                            std::size_t num)
    {
        if (num == 0) {
            return;
...
public:
    ...
    virtual TransactionRef create_transaction(Transaction::src_t,
            const char *name,
            bool is_weak = false) = 0;

    /// Creates empty transaction with interruptible context
    template < typename Func >
    auto with_transaction_intr(Transaction::src_t src,
                               const char *name, Func && f)
    {
        return do_with_transaction_intr < Func, false > (src, name,
                std::forward <
                Func > (f));
    } template < typename Func >
    auto with_transaction_weak(const char *name, Func && f)
    {
        return do_with_transaction_intr < Func,
               true > (Transaction::src_t::READ, name,
                       std::forward < Func > (f)
    ...
    using get_next_dirty_extents_ret =
        ...
        CachedExtentRef >>;
    virtual get_next_dirty_extents_ret get_next_dirty_extents(Transaction &t,   ///< [in] current transaction
    ...
    virtual rewrite_extent_ret rewrite_extent(Transaction &t,
    ...
    virtual get_extents_if_live_ret get_extents_if_live(Transaction &t,
            extent_types_t type,
            paddr_t addr,
            laddr_t laddr,
            extent_len_t len) =
    ...
    virtual submit_transaction_direct_ret submit_transaction_direct(Transaction &t,
            std::optional < journal_seq_t >
            seq_to_trim = std::nullopt) = 0;

private:
    template < typename Func, bool IsWeak >
    auto do_with_transaction_intr(Transaction::src_t src,
                                  const char *name, Func && f)
    {
        return seastar::do_with(create_transaction(src, name, IsWeak),
                                [f =
                                     std::forward < Func >
        (f)](auto & ref_t) mutable {
            return with_trans_intr(*ref_t,
                                   [f =
                                    std::forward <
                                    Func >
                                    (f)](auto & t)
...
public:
    ...
    virtual void update_journal_tails(journal_seq_t dirty_tail,
...
public:
    struct config_t {
        ...
        static config_t get_default(std::size_t roll_size,
                                    journal_type_t type);

        static config_t get_test(std::size_t roll_size,
                                 journal_type_t type);
    };
    ...
    } void update_journal_tails(journal_seq_t dirty_tail,
    ...
    } bool should_block_io_on_trim() const
    {
        return get_tail_limit() >
               get_journal_tail().add_offset(journal_type, reserved_usage,
    ...
    static JournalTrimmerImplRef create(BackrefManager &backref_manager,
                                        config_t config,
                                        journal_type_t type,
                                        device_off_t roll_start,
                                        device_off_t roll_size)
    {
        return std::make_unique < JournalTrimmerImpl > (backref_manager,
                config, type,
                roll_start,
                roll_size);
    }
...
public:
    ...
    virtual segment_id_t allocate_segment(segment_seq_t, segment_type_t,
                                          data_category_t, rewrite_gen_t) =
                                                  0;
    ...
    virtual void update_modify_time(segment_id_t, sea_time_point,
                                    std::size_t) = 0;
...
{
public:
    virtual int64_t allocate(segment_id_t segment,
                             segment_off_t offset, extent_len_t len) = 0;

    virtual int64_t release(segment_id_t segment,
                            segment_off_t offset, extent_len_t len) = 0;
...
class SpaceTrackerSimple: public SpaceTrackerI
{
    ...
    {
        for (auto sm : sms) {
            live_bytes_by_segment.add_device(sm->get_device_id(),
            sm->get_num_segments(), {
    ...
    int64_t allocate(segment_id_t segment,
                     segment_off_t offset, extent_len_t len) final
    {
        return update_usage(segment, len);
    }

    int64_t release(segment_id_t segment,
                    segment_off_t offset, extent_len_t len) final
    {
...
class SpaceTrackerDetailed: public SpaceTrackerI
{
    ...
    public:
        ...
        } int64_t update_usage(int64_t delta)
        {
            used += delta;
            return used;
        } int64_t allocate(device_segment_id_t segment,
                           segment_off_t offset,
                           extent_len_t len, const extent_len_t block_size);

        int64_t release(device_segment_id_t segment,
                        segment_off_t offset,
                        extent_len_t len, const extent_len_t block_size);
...
public:
    ...
    SpaceTrackerDetailed(const std::vector < SegmentManager * > &sms)
    {
        ...
        for (auto sm : sms) {
            segment_usage.add_device(sm->get_device_id(),
                                     sm->get_num_segments(),
                                     SegmentMap(sm->get_segment_size() /
                                                sm->get_block_size(),
                                                sm->get_segment_size()));
            block_size_by_segment_manager[sm->get_device_id()] =
                sm->get_block_size();
        }
    }
    ...
    int64_t allocate(segment_id_t segment,
                     segment_off_t offset, extent_len_t len) final
    {
        return segment_usage[segment].allocate(segment.device_segment_id(),
                                               offset,
                                               len,
                                               block_size_by_segment_manager
                                               [segment.device_id()]);
    }
    ...
    int64_t release(segment_id_t segment,
                    segment_off_t offset, extent_len_t len) final
    {
        return segment_usage[segment].release(segment.device_segment_id(),
                                              offset,
                                              len,
                                              block_size_by_segment_manager
                                              [segment.device_id()]);
...
public:
    ...
    T &operator[](paddr_t block)
    {
        ...
        auto block_id =
            get_block_id(block.get_device_id(), blk.get_device_off());
    ...
    } template < bool is_const = false > class iterator
    {
        ...
        std::conditional_t <
        is_const, const block_map_t &, block_map_t & > parent;
        ...
        iterator(decltype(parent) & parent,
                 device_id_t device_id, device_off_t device_block_off)
            :
...
public:
    ...
    RBMSpaceTracker(const std::vector < RandomBlockManager * > &rbms)
    {
        for (auto rbm : rbms) {
            block_usage.add_device(rbm->get_device_id(),
                                   rbm->get_device()->get_available_size() /
            rbm->get_block_size(), {
                false
            }
            , rbm->get_block_size());
    ...
    void allocate(paddr_t addr, extent_len_t len)
    {
        ...
        do {
            ...
            cursor =
                cursor.add_offset(block_usage.
                                  get_block_size(addr.get_device_id()));
        } while (cursor < end);
    }
...
public:
    ...
    static SegmentCleanerRef create(config_t config,
                                    SegmentManagerGroupRef && sm_group,
                                    BackrefManager &backref_manager,
                                    SegmentSeqAllocator &ool_seq_allocator,
                                    bool detailed, bool is_cold = false)
    {
        return std::make_unique < SegmentCleaner > (config,
               std::move(sm_group),
               backref_manager,
               ool_seq_allocator,
               detailed, is_cold);
    ...
    {
        return segments[id];
    } segment_id_t allocate_segment(segment_seq_t, segment_type_t,
                                    data_category_t, rewrite_gen_t) final;

    void close_segment(segment_id_t segment) final;

    void update_segment_avail_bytes(segment_type_t type,
    ...
    // Testing interfaces bool check_usage() final;
...
private:
    ...
    double calc_gc_benefit_cost(segment_id_t id,
    ...
    struct reclaim_state_t {
        ...
        static reclaim_state_t create(segment_id_t segment_id,
                                      rewrite_gen_t generation,
                                      segment_off_t segment_size)
        {
            ...
            return {
                ...
                target_gen,
                segment_size,
                P_ADDR_NULL, paddr_t::make_seg_paddr(segment_id, 0)};
        }
    ...
    do_reclaim_space_ret do_reclaim_space(const std::vector <
                                          CachedExtentRef >
                                          &backref_extents,
                                          const backref_pin_list_t &
                                          pin_list, std::size_t &reclaimed,
                                          std::size_t &runs);
    ...
    {
        assert(segments.get_num_closed() >=
               get_segments_in_journal_closed());
     ...
     */ std::size_t get_projected_available_bytes() const
    {
        return (segments.get_available_bytes() >
                stats.projected_used_bytes) ? segments.
               get_available_bytes() - stats.projected_used_bytes : 0;
    ...
    scan_extents_ret scan_no_tail_segment(const segment_header_t &header,
    ...
    void init_mark_segment_closed(segment_id_t segment,
                                  segment_seq_t seq,
...
{
public:
    RBMCleaner(RBMDeviceGroupRef && rb_group,
               BackrefManager &backref_manager, bool detailed);

    static RBMCleanerRef create(RBMDeviceGroupRef && rb_group,
                                BackrefManager &backref_manager,
                                bool detailed)
    {
        return std::make_unique < RBMCleaner > (std::move(rb_group),
    ...
    store_statfs_t get_stat() const final
    {
        ...
        st.available =
            get_total_bytes() - get_journal_bytes() - stats.used_bytes;
...
template <> struct fmt::formatter <
    crimson::os::seastore::segment_info_t > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::segments_info_t > : fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::AsyncCleaner::stat_printer_t > : fmt::
    ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::JournalTrimmerImpl::stat_printer_t > : fmt::
    ostream_formatter {
};
#endif
