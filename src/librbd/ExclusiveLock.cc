...
namespace librbd {
    ...
    :ML < I > (image_ctx.md_ctx, image_ctx.op_work_queue, image_ctx.header_oid,
               ...
               image_ctx.blacklist_expire_seconds), m_image_ctx(image_ctx) {
    ...
    template < typename I >
        ...
        m_image_ctx.io_work_queue->
            block_writes(new C_InitComplete(this, features, on_init));
    }
    ...
    template < typename I >
...

        {
            ...
            if (m_image_ctx.clone_copy_on_read ||
                ...
                m_image_ctx.io_work_queue->set_require_lock(io::DIRECTION_BOTH,
                                                            true);
            }
            else {
                m_image_ctx.io_work_queue->set_require_lock(io::DIRECTION_WRITE,
                                                            true);
            }
        }
    ...
    template < typename I >
        ...
        PreReleaseRequest < I > *req =
            PreReleaseRequest < I >::create(m_image_ctx, shutting_down,
                                            m_async_op_tracker, on_finish);
        m_image_ctx.op_work_queue->queue(new FunctionContext([req] (int r) {
                                                             req->send();}));
    }
    ...
    template < typename I >
        ...
        if (!shutting_down) {
            {
                Mutex::Locker locker(ML < I >::m_lock);
                assert(ML < I >::is_state_pre_releasing()
