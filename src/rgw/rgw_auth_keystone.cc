        ...
        namespace keystone {
            ...
            bool TokenEngine::is_applicable(const std::
                ...
                if (ret < 0) {
                 ...
                 ret =
                    token_body.parse(cct, token, token_body_bl,
                ...
                if (rgw::keystone::Service::
                    get_admin_token(cct, token_cache, config,
                                    admin_token) < 0) {
                    throw - EINVAL;
                }
                ...
                if (validate.get_http_status() ==
                    ...
                    ldout(cct,
                          5) << "Failed keystone auth from " << url << " with "
            ...
            TokenEngine::auth_info_t
                ...
                RGW_PERM_FULL_CONTROL, level, TYPE_KEYSTONE,};
            }
            ...
            TokenEngine::result_t
                ...
                if (t->expired()) {
                    ldout(cct,
                          0) << "got expired token: " << t->get_project_name()
                        << ":" << t->get_user_name()
              ...
              for (const auto & role:roles.plain) {
                    if (t->has_role(role) == true) {
                        ldout(cct,
                              0) << "validated token: " << t->get_project_name()
                            << ":" << t->get_user_name()
 ...
 */
            std::pair < boost::optional < rgw::keystone::TokenEnvelope >, int >
                EC2Engine::get_from_keystone(const boost::
                ...
                std::string admin_token;
                int ret =
                    rgw::keystone::Service::get_admin_token(cct, token_cache,
                                                            config,
                                                            admin_token);
                if (ret < 0) {
                    ldout(cct,
                ...
                ret =
                    token_envelope.parse(cct, std::string(), token_body_bl,
                                         api_version);
                if (ret < 0) {
                    ldout(cct,
            ...
            rgw::auth::Engine::result_t EC2Engine::authenticate(const boost::
                                                                string_view &
                                                                access_key_id,
                                                                const boost::
                                                                string_view &
