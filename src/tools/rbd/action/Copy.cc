    ...
    namespace action {
        namespace copy {
            ...
            static int do_copy(librbd::Image & src, librados::IoCtx & dest_pp,
                ...
                int r =
                    src.copy_with_progress4(dest_pp, destname, opts, pc,
                                            sparse_size);
                if (r < 0) {
                    pc.fail();
                    return r;
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: copy failed: " << cpp_strerror(r) << std::
                        endl;
                    return r;
                }
                return 0;
            ...
            static int do_deep_copy(librbd::Image & src,
                ...
                int r =
                    src.deep_copy_with_progress(dest_pp, destname, opts, pc);
                if (r < 0) {
                    pc.fail();
                    return r;
                }
            ...
            int execute_deep(const po::variables_map & vm,
                ...
                r = do_deep_copy(image, dst_io_ctx, dst_image_name.c_str(),
                                 opts, vm[at::NO_PROGRESS].as < bool > ());
                if (r < 0) {
                    std::
                        cerr << "rbd: deep copy failed: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
