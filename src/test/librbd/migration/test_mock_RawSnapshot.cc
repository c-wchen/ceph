    ...
    namespace migration {
        ...
        class TestMockMigrationRawSnapshot:public TestMockFixture {
          public:
            ...
            void expect_stream_get_size(MockStreamInterface &
                                        mock_stream_interface, uint64_t size,
                                        int r) {
                EXPECT_CALL(mock_stream_interface, get_size(_, _))
                    .
                    WillOnce(Invoke
                             ([size, r] (uint64_t * out_size, Context * ctx) {
                              *out_size = size; ctx->complete(r);}));
            }
        ...
        TEST_F(TestMockMigrationRawSnapshot, Read) {
            ...
            &bl};
            mock_raw_snapshot.read(aio_comp, { {
                                   123, 123}}, std::move(read_result), 0, 0, {
                                   });
        ...
        TEST_F(TestMockMigrationRawSnapshot, ListSnap) {
            ...
            mock_raw_snapshot.list_snap( { {
                                        0, 123}}, 0, &sparse_extents, {
                                        }, &ctx2);
            ASSERT_EQ(0, ctx2.wait());
