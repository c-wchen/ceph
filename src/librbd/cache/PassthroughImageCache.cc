...
namespace librbd {
    namespace cache {
        ...
        template < typename I >
            PassthroughImageCache <
            I >::PassthroughImageCache(ImageCtx & image_ctx)
        :m_image_ctx(image_ctx), m_image_writeback(image_ctx) {
        } template < typename I >
            void PassthroughImageCache < I >::aio_read(Extents
                                                       && image_extents,
                                                       bufferlist * bl,
                                                       int fadvise_flags,
                                                       Context * on_finish) {
        ...
        } template < typename I > void PassthroughImageCache <
            I >::aio_write(Extents && image_extents, bufferlist
        ...
        template < typename I >
            void PassthroughImageCache < I >::aio_discard(uint64_t offset,
                                                          uint64_t length,
                                                          bool
            ...
            m_image_writeback.aio_discard(offset, length, skip_partial_discard,
                                          on_finish);
        }
        ...
        template < typename I >
            void PassthroughImageCache < I >::aio_writesame(uint64_t offset,
                                                            uint64_t length,
                                                            bufferlist
                                                            && bl,
                                                            int fadvise_flags,
            ...
            m_image_writeback.aio_writesame(offset, length, std::move(bl),
                                            fadvise_flags, on_finish);
        }

        template < typename I >
            void PassthroughImageCache < I >::aio_compare_and_write(Extents
                                                                    &&
                                                                    image_extents,
                                                                    bufferlist
                                                                    && cmp_bl,
            ...
            m_image_writeback.aio_compare_and_write(std::move(image_extents),
                                                    std::move(cmp_bl),
                                                    std::move(bl),
                                                    mismatch_offset,
                                                    fadvise_flags, on_finish);
