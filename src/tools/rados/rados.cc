...
int write_full([[maybe_unused]] IoCtx &io_ctx, const std::string &oid,
               bufferlist &indata, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int append([[maybe_unused]] IoCtx &io_ctx, const std::string &oid, buffer::list &indata,
           const uint64_t count, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int setxattr([[maybe_unused]] IoCtx &io_ctx, const std::string &oid, const std::string &attr_name,
             buffer::list &bl, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int getxattr([[maybe_unused]] IoCtx &io_ctx, const std::string &oid, const std::string &attr_name,
             buffer::list &bl, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int rmxattr([[maybe_unused]] IoCtx &io_ctx, const std::string &oid,
            const std::string &attr_name, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int getxattrs([[maybe_unused]] IoCtx &io_ctx, const std::string &oid,
              std::map<std::string, buffer::list> &attrset, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int stat([[maybe_unused]] IoCtx &io_ctx, const std::string &oid, uint64_t &size,
         time_t &mtime, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int stat2([[maybe_unused]] IoCtx &io_ctx, const std::string &oid, uint64_t &size,
          timespec &mtime, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) {
...
int LoadGen::run()
{
    ...
    while (1) {
        ...
        if (now - stamp_time >= utime_t(1, 0)) {
            ...
            std::streamsize original_precision = cout.precision();
            cout.precision(3);
            cout << std::setw(5) << total_sec << ": throughput=" << rate  << "MB/sec" << " pending data=" << sent - completed <<
                 std::endl;
            cout.precision(original_precision);
            stamp_time = now;
        }
                       ...
                       Formatter &f)
{
    ...
    if ((inc.union_shards.has_hinfo_missing()
        ...
        map<std::string, ceph::bufferlist>::iterator k = (const_cast<shard_info_t &>(shard)).attrs.find(
                             ...
                             std::vector<const char *> &nargs)
{
    ...
    if (strcmp(nargs[0], "lspools") == 0) {
        ...
        if (ret < 0) {
            cerr << "error setting xattr " << pool_name << "/" << prettify(*obj_name) << "/" << attr_name << ": " << cpp_strerror(
                     ret) << std::endl;
            return 1;
        } else {
            ret = 0;
        ...
        if (ret < 0) {
            cerr << "error getting xattr " << pool_name << "/" << prettify(*obj_name) << "/" << attr_name << ": " << cpp_strerror(
                     ret) << std::endl;
            return 1;
        } else {
            ret = 0;
        ...
        if (ret < 0) {
            cerr << "error removing xattr " << pool_name << "/" << prettify(*obj_name) << "/" << attr_name << ": " << cpp_strerror(
                     ret) << std::endl;
            return 1;
        }
        ...
        if (ret < 0) {
            cerr << "error listing snap shots " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(
                     ret) << std::endl;
            return 1;
        } else {
            ret = 0;
