...
SocketMessenger::SocketMessenger(const entity_name_t &myname,
                                 ...
                                 uint32_t nonce)
    : master_sid {
    seastar::this_shard_id()}, logic_name {
    logic_name}, nonce {
    nonce}, my_name {
    myname}
{
        ...
        if (a.is_blank_ip()) {
            ...
            for (auto &b : addrs.v) {
                if (a.get_family() == b.get_family()) {
                    logger().
                    debug
                    (" assuming my addr {} matches provided addr {}", a,
                     b);
                    a = b;
                    a.set_nonce(nonce);
                    a.set_type(type);
...
crimson::net::listen_ertr::future <> SocketMessenger::do_listen(const entity_addrvec_t &addrs)
{
    ...
    .then([this]()->listen_ertr::future <> {
        ...
        logger().debug("{} do_listen: try listen {}...", *this,
                       listen_addr);
        if (!listener) {
...
SocketMessenger::bind_ertr::future <> SocketMessenger::try_bind(const entity_addrvec_t &addrs,
        uint32_t min_port, uint32_t max_port)
{
    ...
    [this, max_port, addr](auto & port) {
        return seastar::
               repeat_until_value([this, max_port, addr,
        &port] {
            auto to_bind = addr;
            to_bind.set_port(port);
            return
            do_listen(entity_addrvec_t {
                to_bind}
                     ).safe_then([this]()->
                                 seastar::future <
                                 std::optional <
                                 std::
            error_code >> {
                logger().info("{} try_bind: done", *this); return seastar::make_ready_future < std::optional < std::error_code >> (std::make_optional < std::error_code > (std::error_code { /* success! */
                }
                                                                                                                                                                          ));
            }
            ,
            listen_ertr::
            all_same_way([this, max_port, &port]
            (const std::error_code & e)mutable->seastar::future < std::optional < std::error_code >> {
                logger
                ().
                trace
                ("{} try_bind: {} got error {}",
...
SocketMessenger::bind_ertr::future <> SocketMessenger::bind(const entity_addrvec_t &addrs)
{
    ...
    return seastar::do_with(int64_t {
        local_conf()->ms_bind_retry_count}
    , [this, addrs](auto & count) {
        return seastar::
        repeat_until_value([this, addrs, &count] {
            assert(count >= 0);
            ...
            ,
            bind_ertr::
            all_same_way([this, &count](const std::error_code error)
            {
                if
                (count--
                 >
                 0) {
                    ...
                    ().
                    info
                    ("{} was unable to bind. Trying again in {} seconds",
                     *this,
                     local_conf
                     ()->
                     ms_bind_retry_delay);
                    ...
                    return
                        ...
                        (std::
                         ...
                         (error));
                                 ...
                                 ).then([](const std::error_code error)->
        bind_ertr::future <> {
            if (!error) {
                return bind_ertr::now(); // success!
...
seastar::future <> SocketMessenger::
start(const dispatchers_t &_dispatchers)
{
    assert(seastar::this_shard_id() == master_sid);
    ...
    if (listener) {
        ...
        return listener->
        accept([this](SocketRef socket, entity_addr_t peer_addr) {
            assert(seastar::this_shard_id() == master_sid);
            assert(get_myaddr().is_msgr2());
            SocketConnectionRef conn =
                seastar::make_shared < SocketConnection > (*this,
                    dispatchers);
...
crimson::net::ConnectionRef SocketMessenger::connect(const entity_addr_t &peer_addr,
...
seastar::future <> SocketMessenger::shutdown()
{
    ...
    .then([this] {
        return seastar::parallel_for_each(accepting_conns,
        [](auto conn) {
            return conn->
            close_clean_yielded();
        }
        ...
        ceph_assert(connections.empty()); shutdown_promise.set_value();
    }
                           );
}

static entity_addr_t choose_addr(const entity_addr_t &peer_addr_for_me,
                                   ...
                                   const SocketConnection &conn)
{
    ...
    if (!need_addr) {
        if ((!get_myaddr().is_any() &&
            ...
            logger().
            warn
            ("{} peer_addr_for_me {} type/family/IP doesn't match myaddr {}",
    ...
    if (get_myaddr().get_type() == entity_addr_t::TYPE_NONE) {
        ...
        if (!get_myaddr().is_any() &&
            get_myaddr().get_type() != peer_addr_for_me.get_type()) {
            logger().
            warn("{} peer_addr_for_me {} type doesn't match myaddr {}",
                 conn, peer_addr_for_me, get_myaddr());
            throw std::
            system_error(make_error_code
                         (crimson::net::error::bad_peer_address));
        ...
        if (get_myaddr().get_family() != peer_addr_for_me.get_family()) {
            logger().
            warn
            ("{} peer_addr_for_me {} family doesn't match myaddr {}",
             conn, peer_addr_for_me, get_myaddr());
            throw std::
            system_error(make_error_code
                         (crimson::net::error::bad_peer_address));
        ...
        if (get_myaddr().is_blank_ip()) {
            ...
            logger().
            warn("{} peer_addr_for_me {} IP doesn't match myaddr {}",
                 conn, peer_addr_for_me, get_myaddr());
            throw std::
            system_error(make_error_code
                         (crimson::net::error::bad_peer_address));
...
crimson::net::SocketConnectionRef SocketMessenger::
lookup_conn(const entity_addr_t &addr)
{
