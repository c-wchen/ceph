...
#include "rgw_iam_policy_keywords.frag.cc"
        ...
        "<Condition Key>", TokenKind::cond_key,
        ...
        struct ParseState {
            ...
            boost::optional < Principal > parse_principal(string
                                                          && s,
                                                          string * errmsg);
        ...
        struct PolicyParser:public BaseReaderHandler < UTF8 <>, PolicyParser > {
            ...
            void set(TokenID in) {
                ...
                if (dex(in) & (dex(TokenID::Sid) | dex(TokenID::Effect) |
                               dex(TokenID::Principal) | dex(TokenID::
                                                             NotPrincipal) |
                               dex(TokenID::Action) | dex(TokenID::
                                                          NotAction) |
                               dex(TokenID::Resource) | dex(TokenID::
                                                            NotResource) |
                               dex(TokenID::Condition) | dex(TokenID::
                                                             AWS) |
                               dex(TokenID::Federated) | dex(TokenID::
                                                             Service) |
            ...
            PolicyParser(CephContext * cct, const string & tenant,
                         Policy & policy, bool reject_invalid_principals)
...
//
        ...
        bool ParseState::obj_end() {
            ...
            annotate(fmt::
                     format("Attempt to end unopened object on keyword `{}`.",
                            w->name));
            return false;
        }
        ...
        bool ParseState::key(const char *s, size_t l) {
            ...
            if (w->id == TokenID::Condition && w->kind == TokenKind::statement) {
                ...
                if (boost::algorithm::ends_with(std::string_view {
                                                s, l}
                                                , IfExists)) {
                    ifexists = true;
            ...
            if (!k) {
                if (w->kind == TokenKind::cond_op) {
                    ...
                    annotate(fmt::format("Unknown key `{}`.", std::string_view {
                                         s, token_len}
                             ));
                    return false;
                }
            ...
            if ((((w->id == TokenID::Top) && (k->kind == TokenKind::top)) ||
                 ...
                 /// Principal
                 ((w->id == TokenID::Principal
                   || w->id == TokenID::NotPrincipal)
            ...
            annotate(fmt::
                     format("Token `{}` is not allowed in the context of `{}`.",
                            k->name, w->name));
            return false;
        }
...
// which will make all of this ever so much nicer.
                ...
                if (auto a = ARN::parse(s)) {
                    ...
                    if (regex_match(a->resource, match, rx)
                        ...
                        if (match[1] == "oidc-provider") {
                            return Principal::oidc_provider(std::
                                                            move(match[2]));
                        }
                        if (match[1] == "assumed-role") {
                            return Principal::assumed_role(std::
                                                           move(a->account),
                                                           match[2]);
                        }
                    }
        ...
        bool ParseState::do_string(CephContext * cct, const char *s, size_t l) {
            ...
            Statement *t =
                p.statements.empty()? nullptr : &(p.statements.back());
            ...
            if (w->id == TokenID::Version) {
                ...
                if (*s == '*') {
                  ...
                  for (auto & p:actpairs) {
                        if (match_policy( {
                                         s, l}
                                         , p.name, MATCH_POLICY_ACTION)) {
                            is_validaction = true;
                            (w->id == TokenID::Action ? t->action[p.bit] =
                             1 : t->notaction[p.bit] = 1);
                        }
            ...
            else if (w->id == TokenID::Resource
                     || w->id == TokenID::NotResource) {
                auto a = ARN::parse({ s, l }
                                    , true);
                if (!a) {
                    annotate(fmt::
                             format
                             ("`{}` is not a valid ARN. Resource ARNs should have a "
                              ...
                              "`arn:aws:s3:::resource`.", std::string_view {
                              s, l}
                             ));
                    return false;
                }
                ...
                if (a->account.empty() || a->account == pp->tenant ||
                    ...
                    annotate(fmt::
                             ...
                             ("Policy owned by tenant `{}` cannot grant access to "
                              "resource owned by tenant `{}`.", pp->tenant,
                              a->account));
                    return false;
                }
            }
                ...
                if (pp->s.size() <= 1) {
                    annotate(fmt::
                             format("Principle isn't allowed at top level."));
                    return false;
                }
                auto & pri =
                    pp->s[pp->s.size() - 2].w->id ==
                    TokenID::Principal ? t->princ : t->noprinc;

                string errmsg;
                if (auto o = parse_principal( {
                                             s, l}
                                             , &errmsg)) {
                    ...
                    ldout(cct, 0) << "Ignored principle `" << std::string_view {
                    s, l}
                    <<"`: " << errmsg << dendl;
                }
            }
                ...
                // Failure
                annotate(fmt::
                         format("`{}` is not valid in the context of `{}`.",
                                std::string_view {
                                s, l}
                                , w->name));
                return false;
        ...
        bool Condition::eval(const Environment & env)const {
            ...
            case TokenID::StringEqualsIgnoreCase:
                return orrible(ci_equal_to(), itr,
                               isruntime ? runtime_vals : vals);

            case TokenID::StringNotEqualsIgnoreCase:
                return orrible(std::not_fn(ci_equal_to()), itr,
                               isruntime ? runtime_vals : vals);

            case TokenID::ForAnyValueStringLike:
            case TokenID::StringLike:
                return orrible(string_like(), itr,
                               isruntime ? runtime_vals : vals);

            case TokenID::StringNotLike:
                return orrible(std::not_fn(string_like()), itr,
                               isruntime ? runtime_vals : vals);

            case TokenID::ForAllValuesStringEquals:
                return andible(std::equal_to < std::string > (), itr,
                               isruntime ? runtime_vals : vals);

            case TokenID::ForAllValuesStringLike:
                return andible(string_like(), itr,
                               isruntime ? runtime_vals : vals);

            case TokenID::ForAllValuesStringEqualsIgnoreCase:
                return andible(ci_equal_to(), itr,
                               isruntime ? runtime_vals : vals);

                // Numeric
            case TokenID::NumericEquals:
                return shortible(std::equal_to < double >(), as_number, s,
                                 vals);
            ...
            case TokenID::NumericLessThanEquals:
                return shortible(std::less_equal < double >(), as_number, s,
                                 vals);
            ...
            case TokenID::NumericGreaterThanEquals:
                return shortible(std::greater_equal < double >(), as_number, s,
                                 vals);

                // Date!
            case TokenID::DateEquals:
                return shortible(std::equal_to < ceph::real_time > (), as_date,
                                 s, vals);

            case TokenID::DateNotEquals:
                return shortible(std::
                                 not_fn(std::equal_to < ceph::real_time > ()),
                                 as_date, s, vals);

            case TokenID::DateLessThan:
                return shortible(std::less < ceph::real_time > (), as_date, s,
                                 vals);

            case TokenID::DateLessThanEquals:
                return shortible(std::less_equal < ceph::real_time > (),
                                 as_date, s, vals);

            case TokenID::DateGreaterThan:
                return shortible(std::greater < ceph::real_time > (), as_date,
                                 s, vals);

            case TokenID::DateGreaterThanEquals:
                return shortible(std::greater_equal < ceph::real_time > (),
                                 as_date, s, vals);
            ...
            case TokenID::BinaryEquals:
                return shortible(std::equal_to < ceph::bufferlist > (),
                                 as_binary, s, vals);

                // IP Address!
            case TokenID::IpAddress:
                return shortible(std::equal_to < MaskedIP > (), as_network, s,
                                 vals);
        ...
        boost::optional < MaskedIP > Condition::as_network(const string & s) {
            ...
            if (m.v6) {
                struct in6_addr a;
                if (inet_pton(AF_INET6, p->c_str(), static_cast < void *>(&a))
                    != 1) {
                    return boost::none;
                }
                ...
                struct in_addr a;
                if (inet_pton(AF_INET, p->c_str(), static_cast < void *>(&a)) !=
                    1) {
                    return boost::none;
                }
        ...
        namespace {
            ...
            template < typename Iterator >
                ostream & print_array(ostream & m, Iterator begin,
                                      Iterator end) {
                if (begin == end) {
                    m << "[]";
                }
            ...
            template < typename Iterator >
                ostream & print_dict(ostream & m, Iterator begin,
                                     Iterator end) {
                m << "{ ";
                std::copy(begin, end,
                          std::experimental::make_ostream_joiner(m, ", "));
        ...
        Effect Statement::eval(const Environment & e,
                               boost::optional <
                               const rgw::auth::Identity & >ida, uint64_t act,
                               boost::optional < const ARN & >res,
                               boost::optional <
                               PolicyPrincipal & >princ_type)const {
            ...
            if (std::all_of(conditions.begin(),
                            conditions.end(),[&e] (const Condition & c) {
                            return c.eval(e);}
                )) {
                return effect;
            }
        ...
        Policy::Policy(CephContext * cct, const string & tenant,
                       const bufferlist & _text, bool reject_invalid_principals)
      :    text(_text.to_str()) {
        ...
        ostream & operator <<(ostream & m, const Policy & p) {
            m << "{ Version: "
                << (p.version ==
                    Version::v2008_10_17 ? "2008-10-17" : "2012-10-17");
            ...
            bool operator() (const Statement & s)const {
                if (s.effect == Effect::Allow) {
                  for (const auto & p:s.princ) {
                        if (p.is_wildcard()) {
                            return s.eval_conditions(iam_all_env) ==
                                Effect::Allow;
                    }}
                    // no princ should not contain fixed values
                        return std::none_of(s.noprinc.begin(), s.noprinc.end(),
                                            [](const rgw::auth::Principal & p) {
