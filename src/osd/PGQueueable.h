...
class PGQueueable {
    typedef boost::variant <
        OpRequestRef, PGSnapTrim, PGScrub, PGRecovery > QVariant;
    QVariant qvariant;
    int cost;
    unsigned priority;
    utime_t start_time;
  ...
  :    
    ...
    owner(op->get_req()->get_source_inst()), map_epoch(e) {
    }
    PGQueueable(const PGSnapTrim & op, int cost, unsigned priority,
                utime_t start_time, const entity_inst_t & owner, epoch_t e)
  :    qvariant(op), cost(cost), priority(priority), start_time(start_time),
    ...
    PGQueueable(const PGRecovery & op, int cost, unsigned priority,
                utime_t start_time, const entity_inst_t & owner, epoch_t e)
    :qvariant(op), cost(cost), priority(priority), start_time(start_time),
        owner(owner), map_epoch(e) {
    }
    ...
    unsigned get_priority() const {
        ...
        return map_epoch;
    } const QVariant & get_variant() const {
        return qvariant;
}};                             // struct PGQueueable
