...
int ErasureCode::init(ErasureCodeProfile & profile, std::ostream * ss)
{
    ...
    err |= to_string("crush-failure-domain", profile,
                     &rule_failure_domain, DEFAULT_RULE_FAILURE_DOMAIN, ss);
...
int ErasureCode::create_rule(const std::string & name,
                             CrushWrapper & crush,
                             std::ostream * ss) const const
{
    int ruleid = crush.add_simple_rule(name,
                                       rule_root,
                                       rule_failure_domain,
                                       rule_device_class,
                                       "indep",
...
int ErasureCode::chunk_index(unsigned int i) const const
{
...
int ErasureCode::encode_prepare(const bufferlist & raw,
                                map < int, bufferlist > &encoded) const const
{
    unsigned int k = get_data_chunk_count();
...
int ErasureCode::encode(const set < int >&want_to_encode,
                        const bufferlist & in, map < int, bufferlist > *encoded)
{
    unsigned int k = get_data_chunk_count();
    unsigned int m = get_chunk_count() - k;
    bufferlist out;
...
const vector < int >&ErasureCode::get_chunk_mapping() const const
{
...
int ErasureCode::to_mapping(const ErasureCodeProfile & profile, ostream * ss)
{
    if (profile.find("mapping") != profile.end()) {
        ...
        for (std::string::iterator it = mapping.begin(); it != mapping.end();
             ++it) {
            if (*it == 'D')
