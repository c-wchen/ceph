...
class Journal {
  ...
  public:
   Journal(CephContext * cct, uuid_d f, Finisher * fin, Cond * c = 0):
    fsid(f), finisher(fin), cct(cct), logger(NULL),
        do_sync_cond(c), wait_on_full(false) {
    } virtual ~ Journal() {
    }
   ...
   */
    ...
    virtual bool read_entry(bufferlist & bl,    ///< [out] payload on successful read
    ...
    virtual int prepare_entry(vector < ObjectStore::Transaction > &tls,
                              bufferlist * tbl) = 0;
