...
class RGWBucketReshard {
  ...
  private:
    ...
    int do_reshard(int num_shards,
                   RGWBucketInfo & new_bucket_info,
                   int max_entries,
                   bool verbose, ostream * os, Formatter * formatter);
  public:
  ...
  protected:
     class ReshardWorker:public Thread {
      ...
      public:
        ...
        :cct(_cct), reshard(_reshard), lock("ReshardWorker") {
        } void *entry() override;
        void stop();
    };
    ...
    void get_bucket_logshard_oid(const string & tenant,
                                 const string & bucket_name, string * oid);

  public:
    RGWReshard(RGWRados * _store, bool _verbose = false, ostream * _out =
               nullptr, Formatter * _formatter = nullptr);
    ...
    int list(int logshard_num, string & marker, uint32_t max,
             std::list < cls_rgw_reshard_entry > &entries, bool * is_truncated);
    int clear_bucket_resharding(const string & bucket_instance_oid,
