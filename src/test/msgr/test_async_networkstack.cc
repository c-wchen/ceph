  ...
  public:
    ...
    void SetUp() override {
        ...
        if (strncmp(GetParam(), "dpdk", 4)) {
            ...
            g_ceph_context->_conf.set_val_or_die("ms_dpdk_debug_allow_loopback",
                                                 "true");
            g_ceph_context->_conf.set_val_or_die("ms_async_op_threads", "2");
            string ipv4_addr =
                g_ceph_context->_conf.get_val < std::string >
                ("ms_dpdk_host_ipv4_addr");
                ...
                ASSERT_TRUE(r == -ECONNREFUSED || r == -ECONNRESET);}
                    ...
                    TEST_P(NetworkWorkerTest, ComplexTest) {
                        ...
                        exec_events([this, bind_addr, listen_p, accepted_p,
                                    ...
                                    if (stack->support_local_listen_table()
                                        || worker->id == 0) {
                                    r =
                                    worker->listen(bind_addr, 0, options,
                                                   &bind_socket);
                    ...
                    class StressFactory {
                      ...
                      public:
                        ...
                        explicit StressFactory(const std::shared_ptr <
                                               NetworkStack > &s,
                                               const string & addr, size_t cli,
                                               size_t qd, size_t mc, size_t l)
                        ...
                        void add_client(ThreadData * t_data) {
                            static ceph::mutex lock =
                                ceph::make_mutex("add_client_lock");
                            std::lock_guard l {
                            lock};
                        ...
                        void start(Worker * worker) {
                            ...
                            if (stack->support_local_listen_table()
                                || worker->id == 0) {
                                r = worker->listen(bind_addr, 0, options,
                                                   &bind_socket);
                                ASSERT_EQ(0, r);
                                already_bind = true;
                            }
                    ...
                    INSTANTIATE_TEST_SUITE_P(NetworkStack,
                                             NetworkWorkerTest,::testing::
                                             Values(
#ifdef HAVE_DPDK
                                                             "dpdk",
#endif
