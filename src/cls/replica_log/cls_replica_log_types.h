...
struct cls_replica_log_item_marker {
    ...
    utime_t item_timestamp;     // the time stamp at which the item was outdated
     ...
     cls_replica_log_item_marker() {
                                  ...
                                  const utime_t & time):item_name(name),
        item_timestamp(time) {
    }
    ...
    static void generate_test_instances(std::list <
                                        cls_replica_log_item_marker * >&ls);
};
     ...
     cls_replica_log_progress_marker()
{
                                  ...
                                  const utime_t & time):entity_id(entity),
    position_marker(marker), position_time(time) {
}
cls_replica_log_progress_marker(const string & entity, const string & marker,
                                const utime_t & time,
                                const std::list < cls_replica_log_item_marker >
                                &b):entity_id(entity), position_marker(marker),
...
void decode_json(JSONObj * obj);
static void generate_test_instances(std::list <
                                    cls_replica_log_progress_marker * >&ls);
};
