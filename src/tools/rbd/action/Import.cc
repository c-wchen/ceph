        ...
        namespace import {
            ...
            struct ImportDiffContext {
                ...
                uint64_t last_offset;
                 ...
                 ImportDiffContext(librbd::Image * image, int fd, size_t size,
                                   bool no_progress)
                :image(image), fd(fd), size(size), pc("Importing image diff",
                                                      no_progress),
                    throttle((fd == STDIN_FILENO) ? 1 : g_conf->get_val <
            ...
            class C_ImportDiff:public Context {
              public:
                C_ImportDiff(ImportDiffContext * idiffctx, bufferlist data,
                             uint64_t offset, uint64_t length, bool discard)
                :m_idiffctx(idiffctx), m_data(data), m_offset(offset),
                    m_length(length), m_discard(discard) {
                ...
                int send() {
                    ...
                    C_OrderedThrottle *ctx =
                        m_idiffctx->throttle.start_op(this);
                    librbd::RBD::AioCompletion * aio_completion =
                        new librbd::RBD::AioCompletion(ctx,
                                                       &utils::
                                                       aio_context_callback);

                    int r;
                    if (m_discard) {
                        r = m_idiffctx->image->aio_discard(m_offset, m_length,
                                                           aio_completion);
                    }
                    else {
                        r = m_idiffctx->image->aio_write2(m_offset, m_length,
                                                          m_data,
                                                          aio_completion,
            ...
            static int do_image_snap_from(ImportDiffContext * idiffctx) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to decode start snap name" << std::
                        endl;
                    return r;
                }
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to query start snap state" << std::
                        endl;
                    return r;
                }
                ...
                if (!exists) {
                    std::cerr << "start snapshot '" << from
                        << "' does not exist in the image, aborting" << std::
                        endl;
                    return -EINVAL;
                }
            ...
            static int do_image_snap_to(ImportDiffContext * idiffctx,
                                        std::string * tosnap) {
                int r;
                string to;
                r = utils::read_string(idiffctx->fd, 4096, &to);    // 4k limit to make sure we don't get a garbage string
                if (r < 0) {
                    std::cerr << "rbd: failed to decode end snap name" << std::
                        endl;
                    return r;
                }
                ...
                if (r < 0) {
                    std::cerr << "rbd: failed to query end snap state" << std::
                        endl;
                    return r;
                }
            ...
            static int get_snap_protection_status(ImportDiffContext * idiffctx,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to decode snap protection status"
                        << std::endl;
                    return r;
                }
            ...
            static int do_image_resize(ImportDiffContext * idiffctx) {
                ...
                r = safe_read_exact(idiffctx->fd, buf, sizeof(buf));
                if (r < 0) {
                    std::cerr << "rbd: failed to decode image size" << std::
                        endl;
                    return r;
                }
            ...
            static int do_image_io(ImportDiffContext * idiffctx, bool discard,
                ...
                if (!discard) {
                    ...
                    r = safe_read_exact(idiffctx->fd, bp.c_str(),
                                        buffer_length);
                    if (r < 0) {
                        std::cerr << "rbd: failed to decode write data" << std::
                            endl;
                        return r;
                    }
                    ...
                    while (buffer_offset < buffer_length) {
                        ...
                        bool zeroed = false;
                        utils::calc_sparse_extent(bp, sparse_size,
                        ...
                        bufferlist write_bl;
                        if (!zeroed) {
                            bufferptr write_ptr(bp, buffer_offset,
                                                write_length);
                    ...
                    bufferlist data;
                    C_ImportDiff *ctx =
                        new C_ImportDiff(idiffctx, data, image_offset,
                                         buffer_length, true);
                    return ctx->send();
                }
                return r;
            ...
            static int validate_banner(int fd, std::string banner) {
                ...
                if (r < 0) {
                    std::cerr << "rbd: failed to decode diff banner" << std::
                        endl;
                    return r;
                }
                ...
                if (strcmp(buf, banner.c_str())) {
                    std::
                        cerr << "rbd: invalid or unexpected diff banner" <<
                        std::endl;
                    return -EINVAL;
                }
            ...
            static int skip_tag(int fd, uint64_t length) {
                ...
                if (fd == STDIN_FILENO) {
                    ...
                    while (len > 0) {
                        ...
                        if (r < 0) {
                            std::
                                cerr << "rbd: failed to decode skipped tag data"
                                << std::endl;
                            return r;
                        }
                        length -= len;
            ...
            static int read_tag(int fd, __u8 end_tag, int format, __u8 * tag,
                                uint64_t * readlen) {
                int r;
                __u8 read_tag;
                ...
                *tag = read_tag;
                if (read_tag != end_tag && format == 2) {
                    ...
                    if (r < 0) {
                        std::cerr << "rbd: failed to decode tag length" << std::
                            endl;
                        return r;
                    }
            ...
            int do_import_diff_fd(librados::Rados & rados,
                                  librbd::Image & image, int fd,
                ...
                if (!from_stdin) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to stat specified diff file" <<
                            std::endl;
                        return r;
                    }
                ...
                r = rados.conf_get("rbd_skip_partial_discard",
                ...
                while (r == 0) {
                    ...
                    if (tag == RBD_DIFF_FROM_SNAP) {
                        ...
                        r = get_snap_protection_status(&idiffctx,
                                                       &is_protected);
                    }
                        ...
                        std::
                            cerr << "unrecognized tag byte " << (int)tag <<
            ...
            int do_import_diff(librados::Rados & rados, librbd::Image & image,
                ...
                r = do_import_diff_fd(rados, image, fd, no_progress, 1,
                                      sparse_size);
            ...
            int execute_diff(const po::variables_map & vm) {
                ...
                librbd::Image image;
                r = utils::init_and_open_image(pool_name, image_name, "", "",
                ...
                if (r < 0) {
                    cerr << "rbd: import-diff failed: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            class C_Import:public Context {
              public:
                C_Import(SimpleThrottle & simple_throttle,
                         librbd::Image & image, bufferlist & bl,
                         uint64_t offset)
                :m_throttle(simple_throttle), m_image(image),
                    m_aio_completion(new librbd::RBD::
                                     AioCompletion(this,
                                                   &utils::
                                                   aio_context_callback)),
                    ...
                    int op_flags = LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL |
                        LIBRADOS_OP_FLAG_FADVISE_NOCACHE;
                    int r =
                        m_image.aio_write2(m_offset, m_bufferlist.length(),
                                           m_bufferlist,
            ...
            static int decode_and_set_image_option(int fd, uint64_t imageopt,
                ...
                r = safe_read_exact(fd, buf, sizeof(buf));
                if (r < 0) {
                    std::cerr << "rbd: failed to decode image option" << std::
                        endl;
                    return r;
                }
            ...
            static int do_import_metadata(int import_format,
                                          librbd::Image & image,
                                          const std::map < std::string,
                                          std::string > &imagemetas) {
                int r = 0;
                ...
                for (std::map < std::string, std::string >::const_iterator it =
                     imagemetas.begin(); it != imagemetas.end(); ++it) {
            ...
            static int decode_imagemeta(int fd, uint64_t length,
                ...
                if (r < 0) {
                    std::cerr << "rbd: failed to decode metadata key" << std::
                        endl;
                    return r;
                }
                ...
                if (r < 0) {
                    std::cerr << "rbd: failed to decode metadata value" << std::
                        endl;
                    return r;
                }
            ...
            static int do_import_header(int fd, int import_format,
                ...
                while (r == 0) {
                    __u8 tag;
                    uint64_t length;
                    r = read_tag(fd, RBD_EXPORT_IMAGE_END, image_format, &tag,
                                 &length);
                    ...
                    if (tag == RBD_EXPORT_IMAGE_ORDER) {
                        r = decode_and_set_image_option(fd,
            ...
            static int do_import_v2(librados::Rados & rados, int fd,
                ...
                r = safe_read_exact(fd, buf, sizeof(buf));
                if (r < 0) {
                    std::cerr << "rbd: failed to decode diff count" << std::
                        endl;
                    return r;
                }
                bufferlist bl;
                ...
                for (size_t i = 0; i < diff_num; i++) {
                    r = do_import_diff_fd(rados, image, fd, true, 2,
                                          sparse_size);
                    if (r < 0) {
                        pc.fail();
                        std::
                            cerr << "rbd: import-diff failed: " <<
            ...
            static int do_import_v1(int fd, librbd::Image & image,
                ...
                if (from_stdin) {
                    ...
                    throttle.
                        reset(new
                              SimpleThrottle(g_conf->get_val < int64_t >
                ...
                while ((readlen =::read(fd, p + blklen, reqlen)) >= 0) {
                    ...
                    if (from_stdin && (image_pos + (size_t) blklen) > size) {
                        ...
                        if (r < 0) {
                            std::
                                cerr << "rbd: can't resize image during import"
                                << std::endl;
                            goto out;
                        }
                    }
                    ...
                    while (buffer_offset < blklen) {
                        size_t write_length = 0;
                        bool zeroed = false;
                        utils::calc_sparse_extent(blkptr, sparse_size,
                                                  buffer_offset, blklen,
                                                  &write_length, &zeroed);

                        if (!zeroed) {
                            bufferlist write_bl;
                            bufferptr write_ptr(blkptr, buffer_offset,
                                                write_length);
                            write_bl.push_back(write_ptr);
                            assert(write_bl.length() == write_length);

                            C_Import *ctx =
                                new C_Import(*throttle, image, write_bl,
                ...
                if (fd == STDIN_FILENO) {
                    ...
                    if (r < 0) {
                        std::cerr << "rbd: final image resize failed" << std::
                            endl;
                        goto out;
                    }
                }
            ...
            static int do_import(librados::Rados & rados, librbd::RBD & rbd,
                                 ...
                                 bool no_progress, int import_format,
                                 size_t sparse_size) {
                int fd, r;
                struct stat stat_buf;
                ...
                if (r < 0) {
                    std::cerr << "rbd: failed to import image-meta" << std::
                        endl;
                    goto err;
                }
                ...
                if (import_format == 1) {
                    ...
                    r = do_import_v2(rados, fd, image, size, imgblklen, pc,
                                     sparse_size);
                }
                if (r < 0) {
            ...
            int execute(const po::variables_map & vm) {
                ...
                if (vm.count(at::IMAGE_NAME)) {
                    deprecated_image_name =
                        vm[at::IMAGE_NAME].as < std::string > ();
                    std::
                        cerr <<
                ...
                librbd::RBD rbd;
                r = do_import(rados, rbd, io_ctx, image_name.c_str(),
                              path.c_str(), opts,
                              vm[at::NO_PROGRESS].as < bool > (), format,
                              sparse_size);
                if (r < 0) {
                    std::
                        cerr << "rbd: import failed: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
            ...
            Shell::Action action( {
                                 ...
                                 at::get_long_features_help(), &get_arguments,
                                 &execute);
