...
template < class T > class RGWQuotaCache {
  protected:
    ...
    class StatsAsyncTestSet:public lru_map < T,
        RGWQuotaCacheStats >::UpdateContext {
        int objs_delta;
    ...
    virtual int fetch_stats_from_storage(const rgw_user & user,
                                         const rgw_bucket & bucket,
                                         RGWStorageStats & stats,
                                         optional_yield y,
                                         const DoutPrefixProvider * dpp) = 0;

    virtual bool map_find(const rgw_user & user, const rgw_bucket & bucket,
                          RGWQuotaCacheStats & qs) = 0;

    virtual bool map_find_and_update(const rgw_user & user,
                                     const rgw_bucket & bucket,
                                     typename lru_map < T,
                                     RGWQuotaCacheStats >::UpdateContext *
                                     ctx) = 0;
    virtual void map_add(const rgw_user & user, const rgw_bucket & bucket,
                         RGWQuotaCacheStats & qs) = 0;

    virtual void data_modified(const rgw_user & user, rgw_bucket & bucket) {
    }
  public:
    RGWQuotaCache(rgw::sal::Driver * _driver, int size):driver(_driver),
        stats_map(size) {
    ...
    int get_stats(const rgw_user & user, const rgw_bucket & bucket,
                  RGWStorageStats & stats, optional_yield y,
                  const DoutPrefixProvider * dpp);
    void adjust_stats(const rgw_user & user, rgw_bucket & bucket,
                      int objs_delta, uint64_t added_bytes,
                      uint64_t removed_bytes);

    void set_stats(const rgw_user & user, const rgw_bucket & bucket,
                   RGWQuotaCacheStats & qs, RGWStorageStats & stats);
    int async_refresh(const rgw_user & user, const rgw_bucket & bucket,
                      RGWQuotaCacheStats & qs);
    void async_refresh_response(const rgw_user & user, rgw_bucket & bucket,
                                RGWStorageStats & stats);
    ...
    class AsyncRefreshHandler {
      ...
      public:
        AsyncRefreshHandler(rgw::sal::Driver * _driver,
                            RGWQuotaCache < T > *_cache):driver(_driver),
            cache(_cache) {
    ...
    virtual AsyncRefreshHandler *allocate_refresh_handler(const rgw_user & user,
                                                          const rgw_bucket &
                                                          bucket) = 0;
};

template < class T >
    int RGWQuotaCache < T >::async_refresh(const rgw_user & user,
                                           const rgw_bucket & bucket,
                                           RGWQuotaCacheStats & qs)
{
...
template < class T >
    void RGWQuotaCache < T >::async_refresh_fail(const rgw_user & user,
                                                 rgw_bucket & bucket)
{
    ldout(driver->ctx(),
          20) << "async stats refresh response for bucket=" << bucket << dendl;

    async_refcount->put();
}

template < class T >
    void RGWQuotaCache < T >::async_refresh_response(const rgw_user & user,
                                                     rgw_bucket & bucket,
                                                     RGWStorageStats & stats)
{
    ldout(driver->ctx(),
...
template < class T >
    void RGWQuotaCache < T >::set_stats(const rgw_user & user,
                                        const rgw_bucket & bucket,
                                        RGWQuotaCacheStats & qs,
                                        RGWStorageStats & stats)
{
...
template < class T >
    int RGWQuotaCache < T >::get_stats(const rgw_user & user,
                                       const rgw_bucket & bucket,
                                       RGWStorageStats & stats,
                                       optional_yield y,
                                       const DoutPrefixProvider * dpp)
...
template < class T > class RGWQuotaStatsUpdate:public lru_map < T,
    RGWQuotaCacheStats >::
    UpdateContext {
  ...
  public:
    ...
    :objs_delta(objs_delta),
        added_bytes(added_bytes), removed_bytes(removed_bytes) {
    }
...
template < class T >
    void RGWQuotaCache < T >::adjust_stats(const rgw_user & user,
                                           rgw_bucket & bucket, int objs_delta,
                                           uint64_t added_bytes,
                                           uint64_t removed_bytes)
{
...
class BucketAsyncRefreshHandler:public RGWQuotaCache <
    rgw_bucket >::AsyncRefreshHandler, public RGWGetBucketStats_CB {
    rgw_user user;
  public:
     BucketAsyncRefreshHandler(rgw::sal::Driver * _driver,
                               RGWQuotaCache < rgw_bucket > *_cache,
                               const rgw_user & _user,
                               const rgw_bucket & _bucket):RGWQuotaCache <
        rgw_bucket >::AsyncRefreshHandler(_driver, _cache),
...
void BucketAsyncRefreshHandler::handle_response(const int r)
{
    if (r < 0) {
        ldout(driver->ctx(),
              20) << "AsyncRefreshHandler::handle_response() r=" << r << dendl;
...
class RGWBucketStatsCache:public RGWQuotaCache < rgw_bucket > {
  protected:
    bool map_find(const rgw_user & user, const rgw_bucket & bucket,
    ...
    void map_add(const rgw_user & user, const rgw_bucket & bucket,
                 RGWQuotaCacheStats & qs) override {
        stats_map.add(bucket, qs);
    }

    int fetch_stats_from_storage(const rgw_user & user,
                                 const rgw_bucket & bucket,
                                 RGWStorageStats & stats, optional_yield y,
                                 const DoutPrefixProvider * dpp) override;

  public:
  explicit RGWBucketStatsCache(rgw::sal::Driver * _driver):RGWQuotaCache < rgw_bucket > (_driver,
                                  _driver->ctx()->_conf->
                                  rgw_bucket_quota_cache_size) {
    }

    AsyncRefreshHandler *allocate_refresh_handler(const rgw_user & user,
                                                  const rgw_bucket & bucket)
        override {
...
int RGWBucketStatsCache::fetch_stats_from_storage(const rgw_user & _u,
                                                  const rgw_bucket & _b,
                                                  RGWStorageStats & stats,
                                                  optional_yield y,
                                                  ...
                                                  dpp)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "could not get bucket info for bucket=" << _b << " r="
            << r << dendl;
        return r;
    }
...
class UserAsyncRefreshHandler:public RGWQuotaCache <
    rgw_user >::AsyncRefreshHandler, public RGWGetUserStats_CB {
    const DoutPrefixProvider *dpp;
    rgw_bucket bucket;
  public:
    UserAsyncRefreshHandler(const DoutPrefixProvider * _dpp,
                            rgw::sal::Driver * _driver,
                            RGWQuotaCache < rgw_user > *_cache,
                            const rgw_user & _user,
                            const rgw_bucket & _bucket):RGWQuotaCache <
        rgw_user >::AsyncRefreshHandler(_driver, _cache),
        RGWGetUserStats_CB(_user), dpp(_dpp), bucket(_bucket) {
    } void drop_reference() override {
        put();
    }
    int init_fetch() override;
...
int UserAsyncRefreshHandler::init_fetch()
{
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "could not get bucket info for user=" << user << dendl;
...
class RGWUserStatsCache:public RGWQuotaCache < rgw_user > {
    ...
    class BucketsSyncThread:public Thread {
      ...
      public:
        ...
        BucketsSyncThread(CephContext * _cct, RGWUserStatsCache * _s):cct(_cct),
            stats(_s) {
        } void *entry() override {
            ...
            do {
                ...
                for (map < rgw_bucket, rgw_user >::iterator iter =
                     buckets.begin(); iter != buckets.end(); ++iter) {
                    rgw_bucket bucket = iter->first;
                    rgw_user & user = iter->second;
                    ldout(cct,
                          20) << "BucketsSyncThread: sync user=" << user <<
                        " bucket=" << bucket << dendl;
                    const DoutPrefix dp(cct, dout_subsys,
    ...
    class UserSyncThread:public Thread {
      ...
      public:
        ...
        UserSyncThread(CephContext * _cct, RGWUserStatsCache * _s):cct(_cct),
            stats(_s) {
        } void *entry() override {
  ...
  protected:
    ...
    int sync_user(const DoutPrefixProvider * dpp, const rgw_user & user,
                  optional_yield y);
  ...
  public:
    RGWUserStatsCache(const DoutPrefixProvider * dpp,
                      rgw::sal::Driver * _driver, bool quota_threads)
  :    RGWQuotaCache < rgw_user > (_driver,
                                    _driver->ctx()->_conf->
                                    rgw_bucket_quota_cache_size), dpp(dpp) {
        if (quota_threads) {
                                                ...
                                                const DoutPrefixProvider * dpp)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "could not get user stats for user=" << user << dendl;
        return r;
    }
...
int RGWUserStatsCache::sync_bucket(const rgw_user & _u, rgw_bucket & _b,
                                   optional_yield y,
                                   const DoutPrefixProvider * dpp)
{
    std::unique_ptr < rgw::sal::User > user = driver->get_user(_u);
    ...
    if (r < 0) {
        ldpp_dout(dpp,
            ...
            ", bucket=" << bucket << " returned " << r << dendl;
        return r;
    }
...
int RGWUserStatsCache::sync_user(const DoutPrefixProvider * dpp,
                                 const rgw_user & _u, optional_yield y)
{
    RGWStorageStats stats;
    ceph::real_time last_stats_sync;
    ceph::real_time last_stats_update;
    std::unique_ptr < rgw::sal::User > user =
        driver->get_user(rgw_user(_u.to_str()));

    int ret =
        user->read_stats(dpp, y, &stats, &last_stats_sync, &last_stats_update);
    if (ret < 0) {
        ldpp_dout(dpp,
                  5) << "ERROR: can't read user header: ret=" << ret << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  0) << "ERROR: failed user stats sync, ret=" << ret << dendl;
        return ret;
    }
...
int RGWUserStatsCache::sync_all_users(const DoutPrefixProvider * dpp,
                                      optional_yield y)
{
    string key = "user";
    void *handle;
    ...
    do {
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: lists_keys_next(): ret=" << ret << dendl;
            goto done;
        }
        for (list < string >::iterator iter = keys.begin();
            ...
            rgw_user user(*iter);
            ldpp_dout(dpp,
                      20) << "RGWUserStatsCache: sync user=" << user << dendl;
...
void RGWUserStatsCache::data_modified(const rgw_user & user,
                                      rgw_bucket & bucket)
{
    /* racy, but it's ok */
    mutex.lock_shared();
...
class RGWQuotaInfoDefApplier:public RGWQuotaInfoApplier {
  public:
    bool is_size_exceeded(const DoutPrefixProvider * dpp,
                          const char *const entity, const RGWQuotaInfo & qinfo,
                          const RGWStorageStats & stats,
                          const uint64_t size) const override;

    bool is_num_objs_exceeded(const DoutPrefixProvider * dpp,
                              const char *const entity,
                              const RGWQuotaInfo & qinfo,
                              const RGWStorageStats & stats,
...
bool RGWQuotaInfoDefApplier::is_size_exceeded(const DoutPrefixProvider * dpp,
                                              ...
                                              const RGWQuotaInfo & qinfo,
                                              const RGWStorageStats & stats,
                                              const uint64_t size) const const
{
    if (qinfo.max_size < 0) {
    ...
    if (std::cmp_greater(cur_size + new_size, qinfo.max_size)) {
        ldpp_dout(dpp,
            ...
            size_rounded << " size=" << new_size << " " << entity <<
...
bool RGWQuotaInfoDefApplier::is_num_objs_exceeded(const DoutPrefixProvider *
                                                  dpp, const char *const entity,
                                                  const RGWQuotaInfo & qinfo,
                                                  const RGWStorageStats & stats,
    ...
    const
{
    ...
    if (std::cmp_greater(stats.num_objects + num_objs, qinfo.max_objects)) {
        ldpp_dout(dpp,
            ...
            num_objects << " " << entity << "_quota.max_objects=" << qinfo.
            max_objects << dendl;
        return true;
    }
...
bool RGWQuotaInfoRawApplier::is_size_exceeded(const DoutPrefixProvider * dpp,
                                              ...
                                              const RGWQuotaInfo & qinfo,
                                              const RGWStorageStats & stats,
                                              const uint64_t size) constconst
{
    if (qinfo.max_size < 0) {
        /* The limit is not enabled. */
        return false;
...
bool RGWQuotaInfoRawApplier::is_num_objs_exceeded(const DoutPrefixProvider *
                                                  dpp, const char *const entity,
                                                  const RGWQuotaInfo & qinfo,
                                                  const RGWStorageStats & stats,
                                                  const uint64_t num_objs) const
...
const RGWQuotaInfoApplier & RGWQuotaInfoApplier::
get_instance(const RGWQuotaInfo & qinfo)
{
...
class RGWQuotaHandlerImpl:public RGWQuotaHandler {
    ...
    int check_quota(const DoutPrefixProvider * dpp,
        ...
        if (quota_applier.
            is_num_objs_exceeded(dpp, entity, quota, stats, num_objs)) {
  ...
  RGWQuotaHandlerImpl(const DoutPrefixProvider * dpp, rgw::sal::Driver * _driver, bool quota_threads):driver(_driver),
    ...
    int check_quota(const DoutPrefixProvider * dpp,
        ...
        if (quota.bucket_quota.enabled) {
            ...
            int ret =
                bucket_stats_cache.get_stats(user, bucket, bucket_stats, y,
                                             &dp);
            if (ret < 0) {
                return ret;
            }
            ret =
                check_quota(dpp, "bucket", quota.bucket_quota, bucket_stats,
                            num_objs, size);
            if (ret < 0) {
                return ret;
            }
        ...
        if (quota.user_quota.enabled) {
            ...
            int ret =
                user_stats_cache.get_stats(user, bucket, user_stats, y, &dp);
            if (ret < 0) {
                return ret;
            }
            ret =
                check_quota(dpp, "user", quota.user_quota, user_stats, num_objs,
                            size);
            if (ret < 0) {
                return ret;
            }
    ...
    void update_stats(const rgw_user & user, rgw_bucket & bucket, int obj_delta,
                      uint64_t added_bytes, uint64_t removed_bytes) override {
        bucket_stats_cache.adjust_stats(user, bucket, obj_delta, added_bytes,
                                        removed_bytes);
        user_stats_cache.adjust_stats(user, bucket, obj_delta, added_bytes,
                                      removed_bytes);
    }

    void check_bucket_shards(const DoutPrefixProvider * dpp,
                             uint64_t max_objs_per_shard, uint64_t num_shards,
                             uint64_t num_objs, bool is_multisite,
        ...
        if (num_objs > num_shards * max_objs_per_shard) {
            ldpp_dout(dpp,
                ...
                " shard max_objects=" << max_objs_per_shard *
                num_shards << dendl;
            ...
            if (suggested_num_shards) {
                ...
                *suggested_num_shards =
                    num_objs * obj_multiplier / max_objs_per_shard;
            }
        }
        else {
...
RGWQuotaHandler *RGWQuotaHandler::generate_handler(const DoutPrefixProvider *
                                                   dpp,
                                                   rgw::sal::Driver * driver,
                                                   bool quota_threads)
{
...
void rgw_apply_default_bucket_quota(RGWQuotaInfo & quota,
...
void rgw_apply_default_user_quota(RGWQuotaInfo & quota,
                                  const ConfigProxy & conf)
{
...
void RGWQuotaInfo::dump(Formatter * f) const const
{
