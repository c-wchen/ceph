...
class TestObjectRecorder:public RadosTestFixture {
  public:
    ...
    typedef std::map < std::string,
        shared_ptr < Mutex >> ObjectRecorderLocksMap;
    ...
    journal::AppendBuffer create_append_buffer(uint64_t tag_tid,
        ...
        journal::FutureImplPtr future(new journal::
                                      FutureImpl(tag_tid, entry_tid, 456));
    ...
    journal::ObjectRecorderPtr create_object(const std::string & oid,
        ...
        journal::ObjectRecorderPtr object(new journal::
                                          ObjectRecorder(m_ioctx, oid, 0, lock,
                                                         m_work_queue, *m_timer,
                                                         m_timer_lock,
                                                         &m_handler, order,
                                                         m_flush_interval,
                                                         m_flush_bytes,
...
TEST_F(TestObjectRecorder, Close)
{
...

    {
        ...
        while (!m_handler.is_closed) {
            if (m_handler.cond.WaitInterval(m_handler.lock, utime_t(10, 0)) !=
                0) {
                break;
            }
        }
