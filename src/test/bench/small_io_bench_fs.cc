...
int main(int argc, char **argv)
{
    ...
    desc.add_options()
        ...
        ("journal-path", po::value < string > (), "path to journal, mandatory")
    ...
    ceph::shared_ptr < StatCollector >
        col(new
            DetailedStatCollector(1, new JSONFormatter, detailed_ops, &cout,
    ...
    vector < ceph::shared_ptr < Bencher > >benchers(vm["num-writers"].as <
                                                    unsigned >());
    for (vector < ceph::shared_ptr < Bencher > >::iterator i = benchers.begin();
        ...
        Distribution <
            boost::tuple < string, uint64_t, uint64_t, Bencher::OpType > >*gen =
            0;
        if (vm["sequential"].as < bool > ()) {
            std::cout << "Using Sequential generator" << std::endl;
            gen = new SequentialLoad(objects,
                                     vm["object-size"].as < unsigned >(),
                                     vm["io-size"].as < unsigned >(),
                                     new WeightedDist < Bencher::OpType > (rng,
                                                                           ops)
                );
        }
        else {
            ...
            gen =
                new FourTupleDist < string, uint64_t, uint64_t,
                Bencher::OpType > (new RandomDist < string > (rng, objects),
                                   new Align(new
                                             UniformRandom(rng, 0,
                                                           vm["object-size"].
                                                           as <
        ...
        Bencher *bencher = new Bencher(gen,
                                       col,
                                       new TestFileStoreBackend(&fs,
                                                                vm
                                                                ["write-infos"].
                                                                as < bool > ()),
                                       vm["num-concurrent-ops"].as <
                                       unsigned >(),
