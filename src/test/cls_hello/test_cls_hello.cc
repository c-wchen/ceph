...
TEST(ClsHello, SayHello)
{
    ...
    in.append("Tester");
    ASSERT_EQ(0, ioctx.exec("myobject", "hello", "say_hello", in, out));
    ASSERT_EQ(std::string("Hello, Tester!"),
              std::string(out.c_str(), out.length()));
...
TEST(ClsHello, RecordHello)
{
    ...
    ASSERT_EQ(-EEXIST,
              ioctx.exec("myobject", "hello", "record_hello", in, out));

    in.append("Tester");
    ASSERT_EQ(0, ioctx.exec("myobject2", "hello", "record_hello", in, out));
    ASSERT_EQ(-EEXIST,
              ioctx.exec("myobject2", "hello", "record_hello", in, out));
...
TEST(ClsHello, WriteReturnData)
{
    ...
    bufferlist in, out;
    ASSERT_EQ(0,
              ioctx.exec("myobject", "hello", "writes_dont_return_data", in,
                         out));
    ...
    in.append(buf, sizeof(buf));
    ASSERT_EQ(-EINVAL,
              ioctx.exec("myobject2", "hello", "writes_dont_return_data", in,
                         out));
    ASSERT_EQ(std::string("too much input data!"),
              std::string(out.c_str(), out.length()));
