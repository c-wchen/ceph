...
RGWSyncErrorLogger::RGWSyncErrorLogger(RGWRados * _store,
...
RGWCoroutine *RGWSyncErrorLogger::log_error_cr(const string & source_zone,
...
#define READ_MDLOG_MAX_CONCURRENT 10
  ...
  public:
    RGWReadRemoteMDLogInfoCR(RGWMetaSyncEnv * _sync_env,
                             ...
                             map < int,
                             RGWMetadataLogInfo >
                             *_mdlog_info):RGWShardCollectCR(_sync_env->cct,
                                                             READ_MDLOG_MAX_CONCURRENT),
        sync_env(_sync_env), period(period), num_shards(_num_shards),
...
#define READ_MDLOG_MAX_CONCURRENT 10
  ...
  public:
    RGWListRemoteMDLogCR(RGWMetaSyncEnv * _sync_env,
                         const std::string & period, map < int,
                         string > &_shards, int _max_entries_per_shard,
                         map < int,
...
int RGWRemoteMetaLog::read_log_info(rgw_mdlog_info * log_info)
{
    ...
    ldout(store->ctx(),
          20) << "remote mdlog, num_shards=" << log_info->num_shards << dendl;

    return 0;
}

int RGWRemoteMetaLog::read_master_log_shards_info(const string & master_period,
                                                  map < int,
                                                  RGWMetadataLogInfo >
                                                  *shards_info)
{
    ...
    return run(new
               RGWReadRemoteMDLogInfoCR(&sync_env, master_period,
                                        log_info.num_shards, shards_info));
}

int RGWRemoteMetaLog::read_master_log_shards_next(const string & period,
                                                  map < int,
                                                  ...
                                                  *result)
{
    ...
    return run(new
               RGWListRemoteMDLogCR(&sync_env, period, shard_markers, 1,
                                    result));
}
...
int RGWMetaSyncStatusManager::init()
{
    ...
    int r =
        rgw_init_ioctx(store->get_rados_handle(),
                       store->get_zone_params().log_pool, ioctx, true);
    if (r < 0) {
        lderr(store->ctx()) << "ERROR: failed to open log pool (" << store->
            get_zone_params().log_pool << " ret=" << r << dendl;
        return r;
    }
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(store->
              ctx()) << "ERROR: failed to read sync status, r=" << r << dendl;
        return r;
    }
...
void RGWMetaSyncEnv::init(CephContext * _cct, RGWRados * _store,
                          RGWRESTConn * _conn,
...
class RGWAsyncReadMDLogEntries:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override {
        ...
        int ret =
            mdlog->list_entries(handle, max_entries, *entries, marker,
                                truncated);
    ...
    RGWAsyncReadMDLogEntries(RGWCoroutine * caller,
                             RGWAioCompletionNotifier * cn, RGWRados * _store,
                             RGWMetadataLog * mdlog, int _shard_id,
                             string * _marker, int _max_entries,
                             list < cls_log_entry > *_entries,
                             bool * _truncated)
...
class RGWReadMDLogEntriesCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    int send_request() override {
        marker = *pmarker;
        req =
            new RGWAsyncReadMDLogEntries(this,
            ...
            yield {
                ...
                for (int i = 0; i < (int)status.num_shards; i++) {
                    ...
                    spawn(new RGWSimpleRadosWriteCR < rgw_meta_sync_marker >
                          (sync_env->async_rados, store,
                           rgw_raw_obj(store->get_zone_params().log_pool,
                                       sync_env->shard_obj_name(i)), marker),
                          true);
                }
            }
            yield {
...
bool RGWReadSyncStatusMarkersCR::spawn_next()
{
    ...
    rgw_raw_obj obj {
    env->store->get_zone_params().log_pool, env->shard_obj_name(shard_id)};
...
class RGWFetchAllMetaCR:public RGWCoroutine {
  ...
  public:
    RGWFetchAllMetaCR(RGWMetaSyncEnv * _sync_env, int _num_shards,
                      ...
                      &_markers):RGWCoroutine(_sync_env->cct),
...
#define META_FULL_SYNC_CHUNK_SIZE "1000"
                    ...
                    if (get_ret_status() < 0) {
                        ldout(cct,
                              0) << "ERROR: failed to fetch metadata section: "
                            << *sections_iter << dendl;
                        yield entries_index->finish();
                        yield lease_cr->go_down();
                        drain_all();
                    ...
                    for (; iter != result.keys.end(); ++iter) {
                        ...
                        RGWRados *store = sync_env->store;
                        int ret =
                            store->meta_mgr->get_log_shard_id(*sections_iter,
                                                              *iter, &shard_id);
                        if (ret < 0) {
                            ldout(cct,
                                  0) <<
                                "ERROR: could not determine shard id for " <<
                                *sections_iter << ":" << *iter << dendl;
                            ret_status = ret;
                            break;
                        }
...
class RGWReadRemoteMetadataCR:public RGWCoroutine {
  ...
  public:
    RGWReadRemoteMetadataCR(RGWMetaSyncEnv * _sync_env,
                            ...
                            bufferlist * _pbl):RGWCoroutine(_sync_env->cct),
...
class RGWAsyncMetaStoreEntry:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override {
        int ret =
            store->meta_mgr->put(raw_key, bl, RGWMetadataHandler::APPLY_ALWAYS);
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: can't store key: " << raw_key << " ret=" << ret
                << dendl;
            return ret;
        } return 0;
    }
  public:
  RGWAsyncMetaStoreEntry(RGWCoroutine * caller, RGWAioCompletionNotifier * cn, RGWRados * _store, const string & _raw_key, bufferlist & _bl):RGWAsyncRadosRequest(caller, cn), store(_store),
        raw_key(_raw_key),
        bl(_bl) {
    }
};
...
class RGWAsyncMetaRemoveEntry:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override {
        ...
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: can't remove key: " << raw_key << " ret=" << ret
                << dendl;
            return ret;
        } return 0;
    }
  public:
  RGWAsyncMetaRemoveEntry(RGWCoroutine * caller, RGWAioCompletionNotifier * cn, RGWRados * _store, const string & _raw_key):RGWAsyncRadosRequest(caller, cn), store(_store),
        raw_key(_raw_key)
    {
    }
};
  ...
  public:
    RGWMetaSyncShardMarkerTrack(RGWMetaSyncEnv * _sync_env,
        ...
        ldout(sync_env->cct,
            ...
            << " marker=" << new_marker << " realm_epoch=" << sync_marker.
            realm_epoch << dendl;
        RGWRados *store = sync_env->store;
...
#define NUM_TRANSIENT_ERROR_RETRIES 10
        ...
        if (error_injection &&
            ...
            ldout(sync_env->cct,
                  0) << __FILE__ << ":" << __LINE__ <<
                ": injecting meta sync error on key=" << raw_key << dendl;
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if ((sync_status == -EAGAIN || sync_status == -ECANCELED)
                && (tries < NUM_TRANSIENT_ERROR_RETRIES - 1)) {
                ldout(sync_env->cct,
                      20) << *this << ": failed to fetch remote metadata: " <<
                    section << ":" << key << ", will retry" << dendl;
                continue;
            }
            ...
            if (sync_status < 0) {
                ldout(sync_env->cct,
                      10) << *this <<
                    ": failed to send read remote metadata entry: section=" <<
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if ((retcode == -EAGAIN || retcode == -ECANCELED)
                && (tries < NUM_TRANSIENT_ERROR_RETRIES - 1)) {
                ldout(sync_env->cct,
                      20) << *this << ": failed to store metadata: " << section
                    << ":" << key << ", got retcode=" << retcode << dendl;
                continue;
            }
            break;
        }
            ...
            *reset_backoff = true;
            ...
            do {
                ...
                for (; iter != entries.end(); ++iter) {
                    marker = *iter;
                    ldout(sync_env->cct,
                          20) << __func__ << ": full sync: " << marker << dendl;
                    total_entries++;
                    if (!marker_tracker->
            ...
            if (!lost_lock) {
                ...
                if (can_adjust_marker) {
                    ...
                    yield call(new
                               WriteMarkerCR(sync_env->async_rados,
                                             sync_env->store, rgw_raw_obj(pool,
                                                                          sync_env->
                                                                          shard_obj_name
                                                                          (shard_id)),
                                             *temp_marker));
...
#define INCREMENTAL_MAX_ENTRIES 100
                ...
                if (!period_marker.empty() && period_marker <= mdlog_marker) {
                    ldout(cct,
                          10) << "mdlog_marker past period_marker=" <<
                        period_marker << dendl;
                ...
                if (mdlog_marker <= max_marker) {
                    ...
                    ldout(sync_env->cct,
                          20) << __func__ << ":" << __LINE__ << ": shard_id=" <<
                        shard_id << " syncing mdlog for shard_id=" << shard_id
                        << dendl;
                    yield call(new
                ...
                *reset_backoff = true;  /* if we got to this point, all systems function */
                ldout(sync_env->cct,
                      20) << __func__ << ":" << __LINE__ << ": shard_id=" <<
                    shard_id << " mdlog_marker=" << mdlog_marker <<
                    " sync_marker.marker=" << sync_marker.marker << dendl;
                if (mdlog_marker > max_marker) {
                    marker = max_marker;
                    yield call(new
                    ...
                    if (retcode < 0) {
                        ldout(sync_env->cct,
                              10) << *this <<
                            ": failed to list mdlog entries, retcode=" <<
                            retcode << dendl;
                        yield lease_cr->go_down();
                        drain_all();
                        *reset_backoff = false; // back off and try again later
                    ...
                    for (log_iter = log_entries.begin();
                        ...
                        if (!mdlog_entry.convert_from(*log_iter)) {
                            ldout(sync_env->cct,
                                ...
                                section << ":" << log_iter->
                                name << ":" << log_iter->
                                timestamp << " ... skipping entry" << dendl;
                            continue;
                        }
                        ldout(sync_env->cct,
                              20) << __func__ << ":" << __LINE__ <<
                            ": shard_id=" << shard_id << " log_entry: " <<
                            log_iter->id << ":" << log_iter->
                            section << ":" << log_iter->
                            name << ":" << log_iter->timestamp << dendl;
                        if (!marker_tracker->
                            start(log_iter->id, 0,
                  ...
                  for (const auto & m:sync_status.
                        ...
                        if (next) {
                            ...
                            if (period_marker.empty()) {
                                ...
                                ldout(cct,
                                      10) << "RGWMetaSyncCR: skipping shard " <<
                                    shard_id << " with empty period marker" <<
                                    dendl;
                                continue;
                            }
                        }
...
{
    return run(new RGWSimpleRadosWriteCR < rgw_meta_sync_info >
               (async_rados, store,
                rgw_raw_obj(store->get_zone_params().log_pool,
...
int RGWRemoteMetaLog::run_sync()
{
    ...
    do {
        ...
        if (r < 0 && r != -ENOENT) {
            ldout(store->ctx(),
                  0) << "ERROR: failed to fetch sync status r=" << r << dendl;
            return r;
        }
        ...
        if (sync_status.sync_info.state == rgw_meta_sync_info::StateInit) {
            ...
            r = run(new
                    RGWInitSyncStatusCoroutine(&sync_env,
                                               sync_status.sync_info));
            if (r == -EBUSY) {
                backoff.backoff_sleep();
                continue;
    ...
    do {
        ...
        case rgw_meta_sync_info::StateBuildingFullSyncMaps:
            ...
            r = run(new
                    RGWFetchAllMetaCR(&sync_env, num_shards,
                                      sync_status.sync_markers));
            if (r == -EBUSY || r == -EAGAIN) {
                backoff.backoff_sleep();
                continue;
            ...
            if (r < 0) {
                ldout(store->ctx(),
                      0) << "ERROR: failed to fetch all metadata keys" << dendl;
                return r;
            }
            ...
            if (r < 0) {
                ldout(store->ctx(),
                      0) << "ERROR: failed to update sync status" << dendl;
                return r;
            }
            /* fall through */
...
int RGWCloneMetaLogCoroutine::state_store_mdlog_entries()
{
    ...
    int ret =
        mdlog->store_entries_in_shard(dest_entries, shard_id, cn->completion());
    if (ret < 0) {
        cn->put();
        ldout(cct,
...
class PurgePeriodLogsCR:public RGWCoroutine {
  ...
  public:
    PurgePeriodLogsCR(RGWRados * store, epoch_t realm_epoch,
                      epoch_t * last_trim)
...
int PurgePeriodLogsCR::operate()
{
    reenter(this) {
        ...
        while (cursor.get_epoch() < realm_epoch) {
            ...
            yield {
                const auto mdlog =
                    metadata->get_log(cursor.get_period().get_id());
            ...
            if (retcode == -ENOENT) {
                ...
                ldout(cct,
                      10) << "already removed log shards for realm_epoch=" <<
                    cursor.get_epoch()
                    << " period=" << cursor.get_period().get_id() << dendl;
                return set_cr_done();
            }
                ...
                ldout(cct, 1) << "failed to remove log shards for realm_epoch="
                    << cursor.get_epoch() << " period=" << cursor.get_period().
                    get_id()
...
namespace {
    ...
    using connection_map =
        std::map < std::string, std::unique_ptr < RGWRESTConn >>;
    ...
    template < typename Zonegroups >
      ...
      for (auto & g:zonegroups) {
          for (auto & z:g.second.zones) {
                ...
                new RGWRESTConn(store->ctx(), store, z.first,
                                    z.second.endpoints)};
...
/// comparison operator for take_min_status()
    bool operator<(const rgw_meta_sync_marker & lhs,
                   const rgw_meta_sync_marker & rhs) {
...
/// peer whose realm_epoch matches the minimum realm_epoch in the input
    ...
    struct MasterTrimEnv:public TrimEnv {
        ...
        :TrimEnv(store, http, num_shards), last_trim_markers(num_shards) {
            auto & period = current.get_period();
            connections =
                make_peer_connections(store, period.get_map().zonegroups);
    ...
    struct PeerTrimEnv:public TrimEnv {
        ...
        :TrimEnv(store, http, num_shards), last_trim_timestamps(num_shards) {
    ...
    while (shard_id < env.num_shards) {
        ...
        if (stable <= last_trim) {
            ...
            ldout(cct, 20) << "skipping log shard " << shard_id
                << " at marker=" << stable
                << " last_trim=" << last_trim
                << " realm_epoch=" << sync_status.sync_info.
                realm_epoch << dendl;
            shard_id++;
            continue;
        }
...
int MetaMasterTrimCR::operate()
{
    reenter(this) {
        ...
        if (ret < 0) {
            ldout(cct,
                  4) << "failed to calculate min sync status from peers" <<
                dendl;
            return set_cr_error(ret);
        }
        yield {
            ...
            if (epoch > env.last_trim_epoch + 1) {
                ...
                spawn(new PurgePeriodLogsCR(store, epoch, &env.last_trim_epoch),
                      true);
            }
            else {
            ...
            if (epoch == env.current.get_epoch()) {
                auto mdlog =
                    store->meta_mgr->get_log(env.current.get_period().get_id());
                spawn(new MetaMasterTrimShardCollectCR(env, mdlog, min_status),
                      true);
            }
        }
...
int MetaPeerTrimShardCR::operate()
{
    reenter(this) {
        ...
        yield {
            std::string oid;
            mdlog->get_shard_oid(shard_id, oid);
            call(new RGWRadosTimelogTrimCR(env.store, oid, real_time {
                                           }
                                           , stable, "", ""));
        }
...
bool MetaPeerTrimShardCollectCR::spawn_next()
{
    ...
    spawn(new
          MetaPeerTrimShardCR(meta_env, mdlog, period_id, shard_id, &last_trim),
          false);
    shard_id++;
    return true;
}
    ...
    reenter(this) {
        ...
        if (mdlog_info.realm_epoch == env.current.get_epoch()) {
            yield {
                ...
                auto mdlog =
                    meta_mgr->get_log(env.current.get_period().get_id());
    ...
    reenter(this) {
        for (;;) {
            ...
            if (retcode < 0) {
                ...
                yield call(new
                           RGWSimpleRadosUnlockCR(store->get_async_rados(),
                                                  store, obj, name, cookie));
            }
        }
    }
    return 0;
...
class MetaMasterTrimPollCR:public MetaTrimPollCR {
    ...
    :MetaTrimPollCR(store, interval), env(store, http, num_shards) {
    }
};
...
struct MetaMasterAdminTrimCR:private MasterTrimEnv, public MetaMasterTrimCR {
    MetaMasterAdminTrimCR(RGWRados * store, RGWHTTPManager * http,
                          int num_shards)
