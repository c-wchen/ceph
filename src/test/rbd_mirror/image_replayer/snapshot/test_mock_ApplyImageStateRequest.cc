...
GetMetadataRequest < MockTestImageCtx > *GetMetadataRequest <
...
class TestMockImageReplayerSnapshotApplyImageStateRequest: public
    TestMockFixture
{
public:
    typedef ApplyImageStateRequest < librbd::MockTestImageCtx >
    MockApplyImageStateRequest;
    typedef librbd::image::GetMetadataRequest <
    librbd::MockTestImageCtx > MockGetMetadataRequest;
    ...
    } void expect_rename_image(const std::string &name, int r)
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
                    execute_rename(name, _))
        .WillOnce(WithArg < 1 >
        (Invoke([this, r](Context * ctx) {
            m_threads->work_queue->queue(ctx,
                                         r);
        })));
    } void expect_update_features(uint64_t features,
                                  bool enable, int r)
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
                    execute_update_features(features, enable, _,
                                            0U))
        .WillOnce(WithArg < 2 >
        (Invoke([this, r](Context * ctx) {
            m_threads->work_queue->queue(ctx,
    ...
    void expect_get_metadata(MockGetMetadataRequest &
                             mock_get_metadata_request,
                             const std::map < std::string,
                             bufferlist > &pairs, int r)
    {
        EXPECT_CALL(mock_get_metadata_request, send())
        .
        WillOnce(Invoke
                 ([this, &mock_get_metadata_request, pairs,
        r]() {
            *mock_get_metadata_request.pairs = pairs;
            m_threads->work_queue->
            queue(mock_get_metadata_request.on_finish,
                  r);
        }));
    }

    void expect_update_metadata(const std::vector <
                                std::string > &remove,
                                const std::map < std::string,
                                bufferlist > &pairs, int r)
    {
        for (auto &key : remove) {
            bufferlist bl;
            ceph::encode(key, bl);
            EXPECT_CALL(get_mock_io_ctx
                        (m_mock_local_image_ctx->md_ctx),
    ...
    void expect_unprotect_snapshot(const std::string &name,
                                   int r)
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
    ...
    void expect_protect_snapshot(const std::string &name,
                                 int r)
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
    ...
    void expect_rename_snapshot(uint64_t snap_id,
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest,
       NoChanges)
{
    InSequence seq;
    ...
    C_SaferCond ctx;
    auto req =
        MockApplyImageStateRequest::create("local mirror uuid",
                                           "remote mirror uuid",
                                           m_mock_local_image_ctx,
                                           m_mock_remote_image_ctx,
                                           image_state, &ctx);
       ...
       UpdateFeatures)
{
    ...
    m_mock_local_image_ctx->features =
        RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_DEEP_FLATTEN;
       ...
       UpdateFeaturesError)
{
    ...
    expect_update_features(RBD_FEATURE_DEEP_FLATTEN, false,
                           -EINVAL);
    ...
    image_state.snapshots = {
        {
            1U, {
                ...
                }, "snap1", RBD_PROTECTION_STATUS_UNPROTECTED
            }
        }
    };
    ...
    image_state.snapshots = {
        {
            1U, {
                cls::rbd::UserSnapshotNamespace {
                }, "snap1", RBD_PROTECTION_STATUS_PROTECTED
            }
        }
    };
    m_mock_local_image_ctx->snap_info = {
