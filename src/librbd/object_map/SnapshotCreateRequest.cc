...
namespace librbd {
    namespace object_map {
        ...
        namespace {
            ...
            std::ostream & operator<<(std::ostream & os,
                ...
                case SnapshotCreateRequest::STATE_READ_MAP:
                    os << "READ_MAP";
                    break;
                    case SnapshotCreateRequest::
                        STATE_WRITE_MAP:os << "WRITE_MAP";
                    break;
                    case SnapshotCreateRequest::
                        STATE_ADD_SNAPSHOT:os << "ADD_SNAPSHOT";
                    break;
                    default:os << "UNKNOWN (" << static_cast < uint32_t >
                        (state) << ")";
                    break;
                } return os;
        }} // anonymous namespace void SnapshotCreateRequest::send() {
            send_read_map();
        }
        ...
        void SnapshotCreateRequest::send_write_map() {
            ...
            ldout(cct,
                  5) << this << " " << __func__ << ": snap_oid=" << snap_oid <<
                dendl;
            ...
            librados::AioCompletion * rados_completion =
                create_callback_completion();
            int r =
                m_image_ctx.md_ctx.aio_operate(snap_oid, rados_completion, &op);
            ceph_assert(r == 0);
            rados_completion->release();
        }
