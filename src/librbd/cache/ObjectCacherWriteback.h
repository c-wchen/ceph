...
public:
    ...
    ceph_tid_t write(const object_t &oid,
                     const object_locator_t &oloc, uint64_t off,
    ...
    struct write_result_d {
        ...
        std::string oid;
        Context *oncommit;
        write_result_d(const std::string &oid,
                       Context *oncommit): done(false), ret(0),
            oid(oid), oncommit(oncommit)
        {
        } private:
...
private:
    ...
    ceph::unordered_map < std::string,
         std::queue < write_result_d * >>m_writes;
