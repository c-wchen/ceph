...
int rgw_perf_start(CephContext * cct)
{
    ...
    plb.add_u64_counter(l_rgw_keystone_token_cache_hit,
                        "keystone_token_cache_hit",
                        "Keystone token cache hits");
    plb.add_u64_counter(l_rgw_keystone_token_cache_miss,
...
bool search_err(rgw_http_errors & errs, int err_no, bool is_website_redirect,
                int &http_ret, string & code)
{
...
{                               /* in  */
    ...
    if (prot_flags & RGW_REST_SWIFT) {
        if (search_err
            (rgw_http_swift_errors, err_no, is_website_redirect, err.http_ret,
             err.err_code))
            return;
    }
    ...
    if (search_err
        (rgw_http_s3_errors, err_no, is_website_redirect, err.http_ret,
         err.err_code))
        return;
...
void req_info::init_meta_info(bool * found_bad_meta)
{
  ...
  for (const auto & kv:x_meta_map) {
        dout(10) << "x>> " << kv.first << ":" << rgw::crypt_sanitize::x_meta_map {
...
{                               /* size should be the required string size + 1 */
    ...
    if (ret < 0) {
        lderr(cct) << "cannot get random bytes: " << cpp_strerror(-ret) <<
            dendl;
        return ret;
    }
...
static const char alphanum_upper_table[] =
    "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";

int gen_rand_alphanumeric_upper(CephContext * cct, char *dest, int size)
{                               /* size should be the required string size + 1 */
    ...
    for (i = 0; i < size - 1; i++) {
        ...
        dest[i] =
            alphanum_upper_table[pos % (sizeof(alphanum_upper_table) - 1)];
    }
    dest[i] = '\0';
...
static const char alphanum_lower_table[] =
    "0123456789abcdefghijklmnopqrstuvwxyz";

int gen_rand_alphanumeric_lower(CephContext * cct, char *dest, int size)
{                               /* size should be the required string size + 1 */
    int ret = get_random_bytes(dest, size);
    if (ret < 0) {
        lderr(cct) << "cannot get random bytes: " << cpp_strerror(-ret) <<
    ...
    for (i = 0; i < size - 1; i++) {
        int pos = (unsigned)dest[i];
        dest[i] =
            alphanum_lower_table[pos % (sizeof(alphanum_lower_table) - 1)];
    }
    dest[i] = '\0';
...
static const char alphanum_no_underscore_table[] =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-.";

int gen_rand_alphanumeric_no_underscore(CephContext * cct, char *dest, int size)
{                               /* size should be the required string size + 1 */
    int ret = get_random_bytes(dest, size);
    if (ret < 0) {
        lderr(cct) << "cannot get random bytes: " << cpp_strerror(-ret) <<
...
static const char alphanum_plain_table[] =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
...
{                               /* size should be the required string size + 1 */
    ...
    for (i = 0; i < size - 1; i++) {
        int pos = (unsigned)dest[i];
        dest[i] =
            alphanum_plain_table[pos % (sizeof(alphanum_plain_table) - 1)];
    }
    dest[i] = '\0';
    ...
    if ((name.compare("acl") == 0) ||
        ...
        if ((name.compare("response-content-type") == 0) ||
            ...
            has_resp_modifier = true;
             ...
             (name.compare("quota") == 0) || (name.compare("object") == 0)) {
...
string RGWHTTPArgs::sys_get(const string & name, bool * const exists) const const
{
...
bool verify_user_permission(struct req_state * const s, const int perm)
{
...
bool verify_bucket_permission(struct req_state * const s, const uint64_t op)
{
    return verify_bucket_permission(s,
                                    s->bucket,
                                    s->user_acl.get(),
                                    s->bucket_acl.get(), s->iam_policy, op);
}
                                               ...
                                               const uint64_t op)
{
    return (s->defer_to_bucket_acls == deferred_check
            && verify_bucket_permission(s, bucket, user_acl, bucket_acl,
                                        bucket_policy, op));
}
...
{
    return (s->defer_to_bucket_acls == deferred_check
            && verify_bucket_permission_no_policy(s, user_acl, bucket_acl,
                                                  perm));
}
                              ...
                              const uint64_t op)
{
    ...
    auto r =
        eval_or_pass(bucket_policy, s->env, *s->auth.identity, op, ARN(obj));
    if (r == Effect::Allow)
        // It looks like S3 ACLs only GRANT permissions rather than
        // denying them, so this should be safe.
        return true;
    ...
    if (check_deferred_bucket_perms
        (s, obj.bucket, user_acl, bucket_acl, bucket_policy,
         RGW_DEFER_TO_BUCKET_ACLS_RECURSE, op)
        || check_deferred_bucket_perms(s, obj.bucket, user_acl, bucket_acl,
                                       bucket_policy,
    ...
    bool ret =
        object_acl->verify_permission(*s->auth.identity, s->perm_mask, perm);
    if (ret) {
        return true;
    }
    ...
    return user_acl->verify_permission(*s->auth.identity, swift_perm,
                                       swift_perm);
}
                                        ...
                                        const object_acl, const int perm)
{
    if (check_deferred_bucket_only_acl
        (s, user_acl, bucket_acl, RGW_DEFER_TO_BUCKET_ACLS_RECURSE, perm)
        || check_deferred_bucket_only_acl(s, user_acl, bucket_acl,
                                          RGW_DEFER_TO_BUCKET_ACLS_FULL_CONTROL,
...
bool verify_object_permission(struct req_state * s, uint64_t op)
{
    return verify_object_permission(s,
                                    ...
                                    s->user_acl.get(),
                                    s->bucket_acl.get(),
                                    s->object_acl.get(), s->iam_policy, op);
}
...
std::string rgw_bucket::get_key(char tenant_delim, char id_delim) const const
{
    static constexpr size_t shard_len {
    12};                        // ":4294967295\0"
