...
struct taggedrep {
    ...
    {
    } taggedrep(uint64_t count, bool coarse): count(count),
        coarse(coarse)
    {
...
inline taggedrep operator +(const taggedrep &l,
                            const taggedrep &r)
{
    return {
        l.count + r.count, l.coarse || r.coarse};
}
inline taggedrep operator -(const taggedrep &l,
                            const taggedrep &r)
{
...
#ifndef _WIN32
        return {
            static_cast < time_t >(std::chrono::duration_cast <
                                   std::chrono::seconds >
                                   (ts).count()),
...
private:
    static time_point coarse_now()
    {
        return
            time_point(duration
                       (_logclock::taggedrep(coarse_real_clock::now()
                                             .time_since_epoch().
                                             count(), true)));
    }
    ...
    {
        return
            time_point(duration(_logclock::taggedrep(real_clock::now()
                                .
                                time_since_epoch().
                                count(), false)));
    }
...
inline int append_time(const log_time &t, char *out, int outlen)
{
    ...
    if (coarse) {
        ...
        r = std::snprintf(out, outlen,
                          ...
                          bdt.tm_year + 1900, bdt.tm_mon + 1,
                          bdt.tm_mday, bdt.tm_hour, bdt.tm_min,
                          bdt.tm_sec, static_cast < long >(tv.tv_usec),
                          tz);
    }
