                        ...
                        &ceph_global_init_args)
{
    ...
    SubProcess process(exe_path, SubProcess::KEEP,
                       SubProcess::KEEP, SubProcess::KEEP);
    ...
    if (process.spawn()) {
        std::cerr << "rbd: failed to run rbd-nbd: " << process.
                  err() << std::endl;
        return -EINVAL;
    } else if (process.join()) {
        std::cerr << "rbd: rbd-nbd failed with error: " << process.
                  err() << std::endl;
        return -EINVAL;
    }
                   ...
                   &ceph_global_init_args)
{
    ...
    if (vm.count(at::SNAPSHOT_ID)) {
        ...
        args.
        push_back(std::
                  to_string(vm[at::SNAPSHOT_ID].as < uint64_t >
                            ()));
    }
    ...
    if (vm.count("options")) {
        utils::append_options_as_args(vm["options"].as <
                                      std::vector <
                                      std::string >> (), &args);
    }
                   ...
                   &ceph_global_init_args)
{
    ...
    if (device_name.empty()) {
        ...
        if (vm.count(at::SNAPSHOT_ID)) {
            ...
            args.
            push_back(std::
                      to_string(vm[at::SNAPSHOT_ID].as <
                                uint64_t > ()));
        }
    }
...
void get_list_arguments_deprecated(po::options_description *
                                   positional,
...
{
    std::
    cerr << "rbd: 'nbd list' command is deprecated, " <<
         "use 'device list -t nbd' instead" << std::endl;
    return execute_list(vm, ceph_global_args);
}

void get_map_arguments_deprecated(po::options_description *
                                  positional,
                                  po::options_description *
                                  options)
{
...
{
    std::
    cerr << "rbd: 'nbd map' command is deprecated, " <<
    ...
    if (vm_deprecated.count("device")) {
        opts.push_back("device=" + vm_deprecated["device"].as <
                       std::string > ());
    }
    if (vm_deprecated.count("nbds_max")) {
        opts.push_back("nbds_max=" + vm_deprecated["nbds_max"].as <
                       std::string > ());
    }
    if (vm_deprecated.count("max_part")) {
        opts.push_back("max_part=" + vm_deprecated["max_part"].as <
                       std::string > ());
    }
    if (vm_deprecated.count("timeout")) {
        opts.push_back("timeout=" + vm_deprecated["timeout"].as <
                       std::string > ());
    }
...
void get_unmap_arguments_deprecated(po::options_description *
                                    positional,
                                    po::options_description *
                                    options)
{
...
{
    std::
    cerr << "rbd: 'nbd unmap' command is deprecated, " <<
