...
public:
    ...
    void discover_base_ino(inodeno_t want_ino, MDSContext *onfinish,
                           mds_rank_t from = MDS_RANK_NONE);
    void discover_dir_frag(CInode *base, frag_t approx_fg,
                           MDSContext *onfinish, mds_rank_t from =
                               MDS_RANK_NONE);
    void discover_path(CInode *base, snapid_t snap, filepath want_path,
                       MDSContext *onfinish, bool path_locked =
                           false, mds_rank_t from = MDS_RANK_NONE);
    void discover_path(CDir *base, snapid_t snap, filepath want_path,
                       MDSContext *onfinish, bool path_locked = false);
    ...
    {
        if constexpr
        (std::is_same_v < T, std::vector < CDir * >>) {
    ...
    void adjust_subtree_auth(CDir *root, mds_rank_t a, mds_rank_t b =
                                 CDIR_AUTH_UNKNOWN)
    {
        adjust_subtree_auth(root, mds_authority_t(a, b));
    }
    void adjust_bounded_subtree_auth(CDir *dir,
    ...
    void adjust_bounded_subtree_auth(CDir *dir,
                                     ...
                                     mds_rank_t a)
    {
        adjust_bounded_subtree_auth(dir, bounds,
                                    mds_authority_t(a, CDIR_AUTH_UNKNOWN));
    }
    void adjust_bounded_subtree_auth(CDir *dir,
    ...
    void map_dirfrag_set(const std::list < dirfrag_t > &dfs,
                         std::set < CDir * > &result);
    void try_subtree_merge(CDir *root);
    void try_subtree_merge_at(CDir *root, std::set < CInode * > *to_eval,
                              bool adjust_pop = true);
    ...
    void verify_subtree_bounds(CDir *root,
                               const std::list < dirfrag_t > &bounds);
    ...
    MDRequestRef request_start_peer(metareqid_t rid, __u32 attempt,
    ...
    void request_pin_ref(MDRequestRef &r, CInode *ref,
    ...
    void journal_cow_dentry(MutationImpl *mut, EMetaBlob *metablob,
                            CDentry *dn, snapid_t follows =
                                CEPH_NOSNAP, CInode **pcow_inode =
                                0, CDentry::linkage_t *dnl = 0);
    void journal_dirty_inode(MutationImpl *mut, EMetaBlob *metablob,
                             CInode *in, snapid_t follows = CEPH_NOSNAP);

    void project_rstat_inode_to_frag(const MutationRef &mut,
                                     CInode *cur, CDir *parent,
                                     snapid_t first, int linkunlink,
                                     SnapRealm *prealm);
    void _project_rstat_inode_to_frag(const CInode::mempool_inode *inode,
    ...
    void broadcast_quota_to_client(CInode *in, client_t exclude_ct =
                                       -1, bool quota_change = false);
    void predirty_journal_parents(MutationRef mut, EMetaBlob *blob,
                                  CInode *in, CDir *parent, int flags,
                                  int linkunlink = 0, snapid_t follows =
                                      CEPH_NOSNAP);

    // peers
    void add_uncommitted_leader(metareqid_t reqid, LogSegment *ls,
                                std::set < mds_rank_t > &peers, bool safe =
                                    false)
    {
    ...
    bool have_uncommitted_leader(metareqid_t reqid, mds_rank_t from)
    {
        ...
        return p != uncommitted_leaders.end()
               && p->second.peers.count(from) > 0;
    }
    ...
    void add_uncommitted_peer(metareqid_t reqid, LogSegment *, mds_rank_t,
    ...
    // ambiguous imports
    void add_ambiguous_import(dirfrag_t base,
                              const std::vector < dirfrag_t > &bounds);
    ...
    void get_ambiguous_import_bounds(dirfrag_t base,
    ...
    void _move_subtree_map_bound(dirfrag_t df, dirfrag_t oldparent,
                                 dirfrag_t newparent, std::map < dirfrag_t,
    ...
    void rejoin_export_caps(inodeno_t ino, client_t client,
                            const cap_reconnect_t &icr, int target =
                                -1, bool drop_path = false)
    {
    ...
    void rejoin_recovered_caps(inodeno_t ino, client_t client,
                               const cap_reconnect_t &icr, mds_rank_t frommds =
                                   MDS_RANK_NONE, bool drop_path = false)
    {
    ...
    const cap_reconnect_t *get_replay_cap_reconnect(inodeno_t ino,
            client_t client)
    {
    ...
    void add_reconnected_cap(client_t client, inodeno_t ino,
    ...
    void rejoin_open_sessions_finish(std::map < client_t, std::pair < Session *,
                                     uint64_t > > &session_map);
    bool process_imported_caps();
    void choose_lock_states_and_reconnect_caps();
    void prepare_realm_split(SnapRealm *realm, client_t client, inodeno_t ino,
                             std::map < client_t,
                             ref_t < MClientSnap >> &splits);
    void prepare_realm_merge(SnapRealm *realm, SnapRealm *parent_realm,
                             std::map < client_t,
                             ref_t < MClientSnap >> &splits);
    void send_snaps(std::map < client_t, ref_t < MClientSnap >> &splits);
    Capability *rejoin_import_cap(CInode *in, client_t client,
                                  const cap_reconnect_t &icr,
                                  mds_rank_t frommds);
    void finish_snaprealm_reconnect(client_t client, SnapRealm *realm,
                                    snapid_t seq, std::map < client_t,
                                    ref_t < MClientSnap >> &updates);
    ...
    void rebuild_need_snapflush(CInode *head_in, SnapRealm *realm,
    ...
    void find_nested_exports_under(CDir *import, CDir *dir,
                                   std::set < CDir * > &s);
    ...
    void _create_system_file(CDir *dir, std::string_view name, CInode *in,
                             MDSContext *fin);
    ...
    int path_traverse(MDRequestRef &mdr, MDSContextFactory &cf,
                      ...
                      std::vector < CDentry * > *pdnvec, CInode **pin =
                          nullptr);

    int maybe_request_forward_to_auth(MDRequestRef &mdr,
    ...
    CInode *get_dentry_inode(CDentry *dn, MDRequestRef &mdr, bool projected =
                                 false);

    bool parallel_fetch(std::map < inodeno_t, filepath > &pathmap,
    ...
    void _open_remote_dentry_finish(CDentry *dn, inodeno_t ino,
                                    MDSContext *fin, bool want_xlocked, int r);
    ...
    void find_ino_peers(inodeno_t ino, MDSContext *c,
                        mds_rank_t hint = MDS_RANK_NONE, bool path_locked =
                            false);
    ...
    SnapRealm *get_global_snaprealm() const
    {
    ...
    // == messages == void dispatch(const cref_t < Message > &m);
    ...
    void decode_replica_dir(CDir*&dir, bufferlist::const_iterator &p,
                            CInode *diri, mds_rank_t from,
                            MDSContext::vec &finished);
    void decode_replica_dentry(CDentry*&dn, bufferlist::const_iterator &p,
                               CDir *dir, MDSContext::vec &finished);
    void decode_replica_inode(CInode*&in, bufferlist::const_iterator &p,
                              CDentry *dn, MDSContext::vec &finished);

    void encode_replica_stray(CDentry *straydn, mds_rank_t who,
                              bufferlist &bl);
    void decode_replica_stray(CDentry*&straydn, CInode **in,
                              const bufferlist &bl, mds_rank_t from);

    // -- namespace --
    void encode_remote_dentry_link(CDentry::linkage_t *dnl, bufferlist &bl);
    void decode_remote_dentry_link(CDir *dir, CDentry *dn,
    ...
    void dump_tree(CInode *in, const int cur_depth, const int max_depth,
                   Formatter *f);
    ...
    std::array < float, client_lease_pools > client_lease_durations {
        5.0, 30.0, 300.0};
    ...
    // waiters
    std::map < int, std::map < inodeno_t,
        MDSContext::vec > >waiting_for_base_ino;
...
protected:
    ...
    std::map < CDir *, std::pair < std::vector < std::string >,
        MDSContext::vec > >open_ino_batched_fetch;
    ...
    void rejoin_scour_survivor_replicas(mds_rank_t from,
    ...
    void inode_remove_replica(CInode *in, mds_rank_t rep, bool rejoin,
                              std::set < SimpleLock * > &gather_locks);
    void dentry_remove_replica(CDentry *dn, mds_rank_t rep,
                               std::set < SimpleLock * > &gather_locks);
    ...
    void _open_ino_fetch_dir(inodeno_t ino, const cref_t < MMDSOpenIno > &m,
                             bool parent, CDir *dir, std::string_view dname);
    int open_ino_traverse_dir(inodeno_t ino, const cref_t < MMDSOpenIno > &m,
                              const std::vector < inode_backpointer_t >
                              &ancestors, bool discover, bool want_xlocked,
                              mds_rank_t *hint);
    ...
    // from MMDSResolves
    std::map < mds_rank_t, std::map < dirfrag_t,
        std::vector < dirfrag_t > > >other_ambiguous_imports;
    ...
    std::map < mds_rank_t, std::map < inodeno_t, std::map < client_t,
        Capability::Import > > >rejoin_imported_caps;
    std::map < inodeno_t, std::pair < mds_rank_t, std::map < client_t,
        Capability::Export > > >rejoin_peer_exports;
...
private:
    ...
    typedef std::map < dirfrag_t,
            fragment_info_t >::iterator fragment_info_iterator;
    ...
    std::pair < bool, uint64_t > trim_lru(uint64_t count,
                                          expiremap &expiremap);
    ...
    void get_force_dirfrag_bound_set(const std::vector < dirfrag_t > &dfs,
                                     std::set < CDir * > &bounds);
    ...
    void add_uncommitted_fragment(dirfrag_t basedirfrag, int bits,
                                  const frag_vec_t &old_frag, LogSegment *ls,
                                  bufferlist *rollback = NULL);
    void finish_uncommitted_fragment(dirfrag_t basedirfrag, int op);
    void rollback_uncommitted_fragment(dirfrag_t basedirfrag, frag_vec_t
                                       && old_frags);
    ...
    std::array < CInode *, NUM_STRAY > strays {
    };                          // my stray dir
...
{
public:
    CF_MDS_RetryRequestFactory(MDCache *cache, MDRequestRef &mdr,
                               bool dl): mdcache(cache), mdr(mdr),
        drop_locks(dl)
    {
