        ...
        namespace image_sync {
            ...
            using librbd::util::create_rados_callback;
             ...
             template < typename I >
                ...
                I >::SnapshotCreateRequest(I * local_image_ctx,
            ...
            :m_local_image_ctx(local_image_ctx), m_snap_name(snap_name),
                ...
                r = m_local_image_ctx->md_ctx.aio_operate(m_local_image_ctx->
                                                          header_oid, comp,
                                                          &op);
                assert(r == 0);
                comp->release();
            ...
            template < typename I >
...

                {
                    ...
                    RWLock::WLocker parent_locker(m_local_image_ctx->
                                                  parent_lock);
            ...
            template < typename I >
                ...
                librados::ObjectWriteOperation op;
                librbd::cls_client::set_parent(&op, m_parent_spec,
                                               m_parent_overlap);
            ...
            template < typename I >
                ...
                m_local_image_ctx->operations->
                    execute_snap_create(m_snap_namespace, m_snap_name.c_str(),
                                        ctx, 0U, true);
            }
            ...
            template < typename I >
                ...
                m_local_image_ctx->snap_lock.get_read();
                auto snap_it = m_local_image_ctx->snap_ids.find({ cls::rbd::
                ...
                std::string object_map_oid(librbd::ObjectMap <>::
                                           object_map_name(m_local_image_ctx->
                                                           id, local_snap_id));
                uint64_t object_count =
                    Striper::get_num_objects(m_local_image_ctx->layout,
                                             m_size);
                dout(20) << ": "
                ...
                librados::ObjectWriteOperation op;
                librbd::cls_client::object_map_resize(&op, object_count,
                                                      OBJECT_NONEXISTENT);
                ...
                auto ctx = new FunctionContext([this, finish_op_ctx] (int r){
                                               ...
                                               finish_op_ctx->complete(0);});
                librados::AioCompletion * comp = create_rados_callback(ctx);
                r = m_local_image_ctx->md_ctx.aio_operate(object_map_oid, comp,
                                                          &op);
                assert(r == 0);
                comp->release();
            }
            ...
            template < typename I >
                ...
                I >::handle_create_object_map(int r) {
                dout(20) << ": r=" << r << dendl;
