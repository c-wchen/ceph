    ...
    class Transaction {
      public:
        ...
        void add_to_read_set(CachedExtentRef ref) {
            ...
            auto it =
                ref->transactions.lower_bound(this,
                                              read_set_item_t <
                                              Transaction >::trans_cmp_t());
            if (it != ref->transactions.end() && it->t == this)
                return;

            auto[iter, inserted] = read_set.emplace(this, ref);
            ceph_assert(inserted);
            ref->transactions.insert_before(it,
                                            const_cast < read_set_item_t <
                                            Transaction > &>(*iter));
        }
        ...
        void add_fresh_extent(CachedExtentRef ref) {
            ...
            if (ref->is_exist_clean()) {
                ...
                delayed_alloc_list.emplace_back(ref->cast <
                                                LogicalCachedExtent > ());
        ...
        void replace_placeholder(CachedExtent & placeholder,
                                 CachedExtent & extent) {
            ceph_assert(!is_weak());

            assert(placeholder.get_type() ==
                   extent_types_t::RETIRED_PLACEHOLDER);
            ...
            assert(extent.get_paddr() == placeholder.get_paddr());
            {
                ...
                extent.transactions.insert(const_cast < read_set_item_t <
                                           Transaction > &>(*it));
            }
            {
        ...
        bool is_retired(paddr_t paddr, extent_len_t len) {
            ...
            return retired_paddr <= paddr &&
                retired_paddr.add_offset(retired_length) >=
                paddr.add_offset(len);
        }

        template < typename F > auto for_each_fresh_block(F && f) const {
            std::for_each(written_ool_block_list.begin(),
                          written_ool_block_list.end(), f);
            std::for_each(inline_block_list.begin(), inline_block_list.end(),
                          f);
      ...
      Transaction(OrderingHandle && handle, bool weak, src_t src, journal_seq_t initiated_after, on_destruct_func_t && f, transaction_id_t trans_id):weak(weak),
            handle(std::move(handle)),
            on_destruct(std::move(f)), src(src), trans_id(trans_id) {
        }
...
/// Should only be used with dummy staged-fltree node extent manager
    inline TransactionRef make_test_transaction() {
        ...
        return std::make_unique < Transaction > (get_dummy_ordering_handle(),
                                                 false,
        ...
        TransactionConflictCondition(Transaction & t):t(t) {
            ...
            if (t.conflicted) {
                return seastar::futurize <
                    Fut >::make_exception_future(transaction_conflict());
            }
            else {
    ...
    template < typename E >
        ...
        TransactionConflictCondition, E >;

    template < typename F, typename ... Args >
        auto with_trans_intr(Transaction & t, F && f, Args && ... args) {
        return trans_intr::with_interruption_to_error <
            crimson::ct_error::eagain > (std::move(f),
                                         TransactionConflictCondition(t), t,
...
template <> struct fmt::formatter <
    crimson::os::seastore::io_stat_t >:fmt::ostream_formatter {
};
#endif
