...
class Session:public RefCountedObject {
  ...
  public:
    ...
    bool have_completed_request(ceph_tid_t tid, inodeno_t * pcreated) const {
        map < ceph_tid_t, inodeno_t >::const_iterator p =
            info.completed_requests.find(tid);
...
class SessionMap:public SessionMapStore {
  ...
  public:
    ...
    // sessions void decode_legacy(bufferlist::iterator & blp) override;
    bool empty() const {
        ...
        map < int, xlist < Session * >*>::const_iterator p =
            by_state.find(state);
