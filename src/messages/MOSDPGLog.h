...
class MOSDPGLog final:public MOSDPeeringOp {
  ...
  public:
    ...
    epoch_t get_epoch() const {
        ...
        return new PGPeeringEvent(epoch, query_epoch,
                                  MLogRec(pg_shard_t(get_source().num(), from),
                                          this),
                                  true,
                                  new PGCreateInfo(get_spg(),
                                                   query_epoch,
                                                   info.history,
                                                   past_intervals, false));
    } MOSDPGLog():MOSDPeeringOp {
    MSG_OSD_PG_LOG, HEAD_VERSION, COMPAT_VERSION}
    {
