...
bool is_undefined() const {
    return osd == -1;
} string get_osd() const {
    return (osd == NO_OSD ? "NONE" : to_string(osd));
...
struct request_redirect_t {
  ...
  public:
    ...
    explicit request_redirect_t(const object_locator_t & orig,
                                const string & robj):redirect_locator(orig),
    ...
    template <> struct hash <pg_t > {
        size_t operator() (const pg_t & x)const {
            static hash < uint32_t > H;
             return H((x.pool() & 0xffffffff) ^ (x.pool() >> 32) ^ x.ps() ^ x.
                      preferred());
    }};
...
} // namespace std ostream & operator<<(ostream & out, const spg_t & pg);
...
struct objectstore_perf_stat_t {
    ...
    uint32_t os_apply_latency;
     ...
     objectstore_perf_stat_t():os_commit_latency(0), os_apply_latency(0) {
    ...
    static const char *get_type_name(int t)
{
    ...
    void convert_to_pg_shards(const vector < int >&from,
  ...
  :    
    ...
    hit_set_grade_decay_rate(0),
    hit_set_search_last_n(0),
    stripe_width(0), expected_num_objects(0), fast_read(false), opts() {
    }
    ...
    uint32_t get_stripe_width() const {
        ...
        case TYPE_REPLICATED:
            ...
            default:assert(0 == "unhandled pool type");
    ...
    bool maybe_updated_removed_snaps(const interval_set < snapid_t >
                                     &cached) const;
...
struct object_stat_sum_t {
   ...
   **************************************************************************/
    ...
    :num_bytes(0),
        ...
        num_objects_pinned(0), num_objects_missing(0), num_legacy_snapsets(0)
{
...
struct pg_stat_t {
   ...
   **************************************************************************/
    ...
    :reported_seq(0),
        ...
        hitset_bytes_stats_invalid(false), pin_stats_invalid(false) {
    ...
    void add(const pg_stat_t & o) {
        ...
        if (((uint64_t) snaptrimq_len + (uint64_t) o.snaptrimq_len) >
            (uint64_t) (1 << 31)) {
            snaptrimq_len = 1 << 31;
        }
        else {
...
bool is_zero() const {
    return (stats.is_zero() &&
            log_size == 0 && ondisk_log_size == 0 && up == 0 && acting == 0);
} void dump(Formatter * f) const;
void encode(bufferlist & bl, uint64_t features) const;
void decode(bufferlist::iterator & bl);
...
inline ostream & operator<<(ostream & out, const pg_history_t & h)
{
    return out << "ec=" << h.epoch_created << "/" << h.epoch_pool_created
        ...
        << " " << h.same_up_since
        << "/" << h.same_interval_since << "/" << h.same_primary_since;
}
...
struct pg_info_t {
    ...
    :last_epoch_started(0),
        last_interval_started(0),
        last_user_version(0),
        last_backfill(hobject_t::get_max()), last_backfill_bitwise(false) {
    }
    // cppcheck-suppress noExplicitConstructor
    pg_info_t(spg_t p)
  :    
    ...
    bool overlaps_with(const pg_info_t & oinfo) const {
        return last_update > oinfo.log_tail ?
            oinfo.last_update >= log_tail : last_update >= oinfo.log_tail;
        ...
        from(shard_id_t::NO_SHARD)
{
              ...
              shard_id_t from,
              epoch_t query_epoch, epoch_t epoch_sent, const pg_info_t & info)
:query_epoch(query_epoch),
    epoch_sent(epoch_sent), info(info), to(to), from(from) {
...
{
} pg_interval_t(vector < int32_t > &&up,
...
static void generate_test_instances(list < pg_interval_t * >&o);
  ...
  PastIntervals(bool ec_pool, bool compact):PastIntervals() {
    ...
    class interval_rep {
      public:
        ...
        virtual set < pg_shard_t > get_all_participants(bool ec_pool) const = 0;
        ...
        virtual void iterate_mayberw_back_to(bool ec_pool,
                                             epoch_t les,
   ...
   */
    ...
    template < typename F >
        void iterate_mayberw_back_to(bool ec_pool, epoch_t les, F && f) const {
        assert(past_intervals);
        past_intervals->iterate_mayberw_back_to(ec_pool, les,
                                                std::forward < F > (f));
    } void clear() {
        assert(past_intervals);
   ...
   */
    set < pg_shard_t > get_might_have_unfound(pg_shard_t pg_whoami,
                                              bool ec_pool) const {
        assert(past_intervals);
    ...
    struct PriorSet {
        ...
        // For verifying tests
         PriorSet(bool ec_pool,
                  ...
                  bool pg_down, IsPGRecoverablePredicate * pcontdec)
...
:ec_pool(ec_pool), pg_down(false), pcontdec(c)
{
  ...
  for (auto && i:all_probe) {
    ...
    past_intervals.iterate_mayberw_back_to(ec_pool,
                                           last_epoch_started,
                                           [&](epoch_t start,
...
class ObjectModDesc {
  ...
  public:
    class Visitor {
      public:
        ...
        } virtual void setattrs(map < string,
                                boost::optional < bufferlist > >&attrs) {
        }
  ...
  ObjectModDesc():can_local_rollback(true), rollback_info_completed(false) {
    ...
    void rollback_extents(version_t gen, const vector < pair < uint64_t,
                          uint64_t > >&extents) {
...
struct pg_missing_item {
  ...
  pg_missing_item(eversion_t n, eversion_t h, bool is_delete = false):need(n),
        have(h)
    {
...
class pg_missing_const_i {
  public:
    virtual const map < hobject_t, pg_missing_item > & get_items() const = 0;
...
template < bool TrackChanges > class pg_missing_set:public pg_missing_const_i {
  ...
  public:
    ...
    void encode(bufferlist & bl) const {
        ENCODE_START(4, 2, bl);
        ::encode(missing, bl,
                 may_include_deletes ? CEPH_FEATURE_OSD_RECOVERY_DELETES : 0);
...
template < bool TrackChanges >
    void encode(const pg_missing_set < TrackChanges > &c, bufferlist & bl,
                uint64_t features = 0)
{
    ENCODE_DUMP_PRE();
    c.encode(bl);
...
struct pg_nls_response_t {
    ...
    void encode(bufferlist & bl) const {
        ...
        ::encode(n, bl);
        for (list < librados::ListObjectImpl >::const_iterator i =
    ...
    void dump(Formatter * f) const {
        ...
        for (list < librados::ListObjectImpl >::const_iterator p =
    ...
    } void decode(bufferlist::iterator & bl)
{
    ...
    f->dump_stream("handle") << handle;
    f->open_array_section("entries");
    for (list < pair < object_t, string > >::const_iterator p = entries.begin();
         p != entries.end(); ++p) {
        f->open_object_section("object");
        f->dump_stream("object") << p->first;
...
{
} ObjectExtent(object_t o, uint64_t ono, uint64_t off, uint64_t l,
               uint64_t ts):oid(o), objectno(ono), offset(off), length(l),
    truncate_size(ts) {
}
};
    ...
    } void decode(bufferlist::iterator & bl)
{
    ...
    f->open_array_section("entries");
    for (list < watch_item_t >::const_iterator p = entries.begin();
         p != entries.end(); ++p) {
...
static void generate_test_instances(list < obj_list_watch_response_t * >&o) {
    ...
    ea.set_port(1024);
    o.back()->entries.
        push_back(watch_item_t
                  (entity_name_t(entity_name_t::TYPE_CLIENT, 1), 10, 30, ea));
    ea.set_nonce(1001);
    ea.set_in4_quad(3, 2);
    ea.set_port(1025);
    o.back()->entries.
        push_back(watch_item_t
                  (entity_name_t(entity_name_t::TYPE_CLIENT, 2), 20, 60, ea));
}
};
    ...
    } void decode(bufferlist::iterator & bl)
{
    ...
    for (vector < clone_info >::const_iterator p = clones.begin();
         p != clones.end(); ++p) {
...
struct PromoteCounter {
    std::atomic_ullong attempts
{
0};
std::atomic_ullong objects {
0};
