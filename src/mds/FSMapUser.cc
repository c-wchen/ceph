...
void FSMapUser::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(epoch, bl);
...
void FSMapUser::fs_info_t::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(cid, bl);
    ::encode(name, bl);
...
void FSMapUser::print(ostream & out) const const
{
