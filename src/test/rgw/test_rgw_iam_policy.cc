...
TEST_F(PolicyTest, Eval2)
{
    ...
    auto trueacct =
        FakeIdentity(Principal::tenant("ACCOUNT-ID-WITHOUT-HYPHENS"));
    ...
    for (auto i = 0ULL; i < s3Count; ++i) {
        EXPECT_EQ(p.eval(e, trueacct, 1ULL << i,
                         ARN(Partition::aws, Service::s3,
                             "", arbitrary_tenant, "mybucket")), Effect::Allow);
        EXPECT_EQ(p.eval(e, trueacct, 1ULL << i,
                         ARN(Partition::aws, Service::s3,
        ...
        EXPECT_EQ(p.eval(e, notacct, 1ULL << i,
                         ARN(Partition::aws, Service::s3,
                             "", arbitrary_tenant, "mybucket")), Effect::Pass);
        EXPECT_EQ(p.eval(e, notacct, 1ULL << i,
                         ARN(Partition::aws, Service::s3,
                             "", arbitrary_tenant, "mybucket/myobject")),
                  Effect::Pass);
        ...
        EXPECT_EQ(p.eval(e, trueacct, 1ULL << i,
                         ARN(Partition::aws, Service::s3,
                             "", arbitrary_tenant,
                             "notyourbucket/notyourobject")), Effect::Pass);
...
TEST_F(PolicyTest, Parse3)
{
    ...
    ASSERT_FALSE(p->statements[0].resource.empty());
    ASSERT_EQ(p->statements[0].resource.size(), 1U);
    EXPECT_EQ(p->statements[0].resource.begin()->partition,
              Partition::wildcard);
    ...
    EXPECT_EQ(p->statements[1].resource.begin()->partition,
              Partition::wildcard);
...
TEST_F(PolicyTest, Eval3)
{
    ...
    auto s3allow = (s3ListMultipartUploadParts | s3ListBucket |
                    ...
                    s3GetBucketLogging | s3GetBucketTagging | s3GetBucketWebsite
                    | s3GetLifecycleConfiguration |
                    s3GetReplicationConfiguration | s3GetObjectTagging |
                    s3GetObjectVersionTagging);
