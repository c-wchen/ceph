...
TEST_F(TestMockObjectMapResizeRequest, UpdateInMemory)
{
    ...
    AsyncRequest <> *req =
        new ResizeRequest(*ictx, &object_map_lock, &object_map,
                          CEPH_NOSNAP, object_map.size(), OBJECT_EXISTS,
                          &cond_ctx);
    req->send();
...
TEST_F(TestMockObjectMapResizeRequest, UpdateSnapOnDisk)
{
    ...
    C_SaferCond cond_ctx;
    AsyncRequest <> *req =
        new ResizeRequest(*ictx, &object_map_lock, &object_map, snap_id,
                          object_map.size(), OBJECT_EXISTS, &cond_ctx);
    req->send();
    ASSERT_EQ(0, cond_ctx.wait());
