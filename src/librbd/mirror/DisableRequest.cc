    ...
    namespace mirror {
        ...
        template < typename I >
            ...
            if (*result == 0) {
                ...
                *result =
                    cls_client::mirror_image_get_finish(&iter, &m_mirror_image);
            }
            ...
            if (*result < 0) {
                if (*result == -ENOENT) {
                    ...
                    *result = 0;
                    ...
                    lderr(cct) << "failed to retreive mirror image: " <<
                        cpp_strerror(*result)
                        << dendl;
                }
                return m_on_finish;
        ...
        template < typename I > void DisableRequest < I >::send_get_tag_owner() {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << this << " " << __func__ << dendl;
        ...
        template < typename I >
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_set_mirror_image > (this);
            m_out_bl.clear();
        ...
        template < typename I >
            Context * DisableRequest <
            I >::handle_set_mirror_image(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": r=" << *result << dendl;

            if (*result < 0) {
                lderr(cct) << "failed to disable mirroring: " <<
                    cpp_strerror(*result)
                    << dendl;
                return m_on_finish;
            }
        ...
        template < typename I >
            ...
            MirroringWatcher < I >::notify_image_updated(m_image_ctx->md_ctx,
                                                         cls::rbd::
        ...
        template < typename I >
            Context * DisableRequest <
            I >::handle_notify_mirroring_watcher(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I > void DisableRequest < I >::send_promote_image() {
            ...
            auto req =
                journal::PromoteRequest < I >::create(m_image_ctx, true, ctx);
            req->send();
        }
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to promote image: " <<
                    cpp_strerror(*result) << dendl;
                return m_on_finish;
            }
        ...
        template < typename I > void DisableRequest < I >::send_get_clients() {
            ...
            std::string header_oid =::journal::Journaler::
                header_oid(m_image_ctx->id);
            m_clients.clear();
            cls::journal::client::client_list(m_image_ctx->md_ctx, header_oid,
                                              &m_clients, ctx);
        }
        ...
        template < typename I >
            ...
            if (*result < 0) {
                lderr(cct) << "failed to get registered clients: " <<
                    cpp_strerror(*result)
                    << dendl;
                return m_on_finish;
            }
          ...
          for (auto client:m_clients) {
                ...
                journal::ClientMetaType type =
                    client_data.get_client_meta_type();
                if (type !=
                ...
                if (m_current_ops.find(client.id) != m_current_ops.end()) {
                    ...
                    lderr(cct) << this << " " << __func__ <<
                        ": clients with the same id " << client.id << dendl;
                    continue;
                }
                ...
                journal::MirrorPeerClientMeta client_meta =
                    boost::get < journal::MirrorPeerClientMeta >
                    (client_data.client_meta);

              for (const auto & sync:client_meta.sync_points) {
                    send_remove_snap(client.id, sync.snap_namespace,
                                     sync.snap_name);
                }
        ...
        template < typename I >
            void DisableRequest <
            I >::send_remove_snap(const std::string & client_id,
                                  const cls::rbd::
                                  SnapshotNamespace & snap_namespace,
                                  const std::string & snap_name) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": client_id=" << client_id
            ...
            Context *ctx =
                create_context_callback(&DisableRequest <
                                        I >::handle_remove_snap, client_id);

            ctx =
                new
                FunctionContext([this, snap_namespace, snap_name, ctx] (int r) {
                                m_image_ctx->operations->
                                snap_remove(snap_namespace, snap_name.c_str(),
                                            ctx);});
        ...
        template < typename I >
            void DisableRequest <
            I >::send_unregister_client(const std::string & client_id) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << this << " " << __func__ << dendl;
        ...
        template < typename I >
            Context * DisableRequest <
            I >::handle_unregister_client(int *result,
        ...
        template < typename I >
            ...
            librados::AioCompletion * comp =
                create_rados_callback < klass,
                &klass::handle_remove_mirror_image > (this);
            m_out_bl.clear();
            int r = m_image_ctx->md_ctx.aio_operate(RBD_MIRRORING, comp, &op);
            assert(r == 0);
            comp->release();
        ...
        template < typename I >
            Context * DisableRequest <
            I >::handle_remove_mirror_image(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": r=" << *result << dendl;
            ...
            if (*result < 0) {
                lderr(cct) << "failed to remove mirror image: " <<
                    cpp_strerror(*result)
                    << dendl;
                return m_on_finish;
            }
        ...
        template < typename I >
            Context * DisableRequest <
            I >::handle_notify_mirroring_watcher_removed(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct,
                  10) << this << " " << __func__ << ": r=" << *result << dendl;
        ...
        template < typename I >
            Context * DisableRequest <
            I >::create_context_callback(Context *
                                         (DisableRequest < I >::*handle) (int *,
                                                                          const
                                                                          std::
                                                                          string
                                                                          &
            ...
            return new FunctionContext([this, handle, client_id] (int r) {
                                       Context * on_finish =
                                       (this->*handle) (&r, client_id);
                                       if (on_finish != nullptr) {
                                       on_finish->complete(r); delete this;}
                                       }
            ) ;
        }
