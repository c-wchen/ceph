...
typedef interval_set <
    snapid_t, mempool::osdmap::flat_map > snap_interval_set_t;
...
class IsPGRecoverablePredicate {
   ...
   */
    virtual bool operator() (const std::set < pg_shard_t > &have) constconst =
        0;
...
// -----// a locator constrains the placement of an object.  mainly, which pool// does it go in. struct object_locator_t {
...
struct request_redirect_t {
  ...
  public:
    ...
    request_redirect_t() {
                                  ...
                                  int64_t rpool):redirect_locator(orig) {
        redirect_locator.pool = rpool;
    }
    explicit request_redirect_t(const object_locator_t &
                                rloc):redirect_locator(rloc) {
    }
    explicit request_redirect_t(const object_locator_t & orig,
                                const std::
                                string & robj):redirect_locator(orig),
...
inline std::ostream & operator<<(std::ostream & out,
                                 const request_redirect_t & redir)
{
    out << "object " << redir.redirect_object << ", locator{" << redir.
        redirect_locator << "}";
    return out;
}
...
{
} pg_t(ps_t seed, uint64_t pool):m_pool(pool), m_seed(seed) {
}
  // cppcheck-suppress noExplicitConstructor
pg_t(const ceph_pg & cpg):m_pool(cpg.pool), m_seed(cpg.ps) {
}
    ...
    template <> struct hash <pg_t > {
        size_t operator() (const pg_t & x)const {
            ...
            // xor (s32)-1 in there to preserve original m_preferred result (paranoia!)
             return H((x.pool() & 0xffffffff) ^ (x.pool() >> 32) ^ x.
                      ps() ^ (int32_t) (-1));
    }};
} // namespace std struct spg_t {
    pg_t pgid;
    shard_id_t shard;
...
ghobject_t make_temp_ghobject(const std::string & name) const {
    return ghobject_t(hobject_t(object_t(name), "", CEPH_NOSNAP,
                                pgid.ps(),
...
inline bool operator==(const coll_t & rhs) const {
  ...
  // get a TEMP collection that corresponds to the current collection,// which we presume is a pg collection.
...
explicit eversion_t(ceph::buffer::list & bl):__pad(0) {
    decode(bl);
}
...
inline bool operator<=(const eversion_t & l, const eversion_t & r)
{
    return (l.epoch == r.epoch) ? (l.version <= r.version) : (l.epoch <=
                                                              r.epoch);
}
...
struct objectstore_perf_stat_t {
    ...
    objectstore_perf_stat_t():os_commit_latency_ns(0), os_apply_latency_ns(0) {
...
struct pool_snap_info_t {
    ...
    void dump(ceph::Formatter * f) const const;
    void encode(ceph::buffer::list & bl, uint64_t features) const const;
    void decode(ceph::buffer::list::const_iterator & bl);
    ...
    static std::string_view get_type_name(int t)
{
    ...
    void convert_to_pg_shards(const std::vector < int >&from,
    ...
    const char *get_cache_mode_name() const {
        ...
        case CACHEMODE_PROXY:
            return false;
            case CACHEMODE_WRITEBACK:case CACHEMODE_READFORWARD:case
                CACHEMODE_READPROXY:return true;
    ...
    static pg_autoscale_mode_t get_pg_autoscale_mode_by_name(const std::
                                                             string & m) {
        if (m == "off") {
  ...
  public:
    ...
    } bool stretch_set_can_peer(const std::set < int >&want,
                                const OSDMap & osdmap, std::ostream * out)const;
    bool stretch_set_can_peer(const std::vector < int >&want,
                              const OSDMap & osdmap, std::ostream * out) const {
        if (!is_stretch_pool())
            return true;
        std::set < int >swant;
    ...
    static dedup_chunk_algo_t get_dedup_chunk_algorithm_from_str(const std::
                                                                 string & s) {
        if (s == "none")
  ...
  public:
    ...
    const utime_t & get_create_time() const {
        return create_time;
    ...
    /// true if incomplete clones may be present bool allow_incomplete_clones() const {
    ...
    uint32_t get_stripe_width() const {
        ...
        return pgp_num;
    } unsigned get_pg_num_target() const {
        return pg_num_target;
    } unsigned get_pgp_num_target() const {
    ...
    // if pg_num is not a multiple of two, pgs are not equally sized.// return, for a given pg, the fraction (denominator) of the total// pool size that it represents.
...
struct object_stat_sum_t {
   ...
   **************************************************************************/
    ...
    int64_t num_promote {
    0};
    int32_t num_flush_mode_high {
    0};                         // 1 when in high flush mode, otherwise 0
    int32_t num_flush_mode_low {
    0};                         // 1 when in low flush mode, otherwise 0
    int32_t num_evict_mode_some {
    0};                         // 1 when in evict some mode, otherwise 0
    int32_t num_evict_mode_full {
    0};                         // 1 when in evict full mode, otherwise 0
    int64_t num_objects_pinned {
    0};
    int64_t num_objects_missing {
    0};
    int64_t num_legacy_snapsets {
    0};                         ///< upper bound on pre-luminous-style SnapSets
    int64_t num_large_omap_objects {
    0};
    int64_t num_objects_manifest {
    0};
    int64_t num_omap_bytes {
    0};
    int64_t num_omap_keys {
    0};
    int64_t num_objects_repaired {
    0};
...
void dump(ceph::Formatter * f) const;
void padding_check() {
    static_assert(sizeof(object_stat_sum_t) ==
                  sizeof(num_bytes) +
                  sizeof(num_objects) +
...
struct pg_scrubbing_status_t {
    ...
    pg_scrub_sched_status_t m_sched_status {
    pg_scrub_sched_status_t::unknown};
    bool m_is_active {
    false};
    scrub_level_t m_is_deep {
...
struct pg_stat_t {
   ...
   **************************************************************************/
    ...
    :reported_seq(0),
        ...
        pin_stats_invalid(false), manifest_stats_invalid(false) {
...
struct pool_stat_t {
    ...
    uint64_t get_user_omap_bytes(float raw_used_rate,   ///< space amp factor
        ...
        if (per_pool_omap) {
            return raw_used_rate ? store_stats.omap_allocated /
                raw_used_rate : 0;
        }
        else {
                           ...
                           const pg_hit_set_info_t & r)
{
    return
        l.begin == r.begin &&
        l.end == r.end && l.version == r.version && l.using_gmt == r.using_gmt;
...
last_deep_scrub_stamp(stamp), last_clean_scrub_stamp(stamp) {
}
...
struct pg_info_t {
    ...
    :last_epoch_started(0),
        ...
        last_user_version(0), last_backfill(hobject_t::get_max()) {
    }
    // cppcheck-suppress noExplicitConstructor
    pg_info_t(spg_t p)
  :    
...
struct pg_fast_info_t {
    ...
    void populate_from(const pg_info_t & info) {
        ...
        stats.stats.sum.num_object_copies =
            info.stats.stats.sum.num_object_copies;
        ...
        stats.stats.sum.num_objects_dirty =
...
{
} pg_interval_t(std::vector < int32_t > &&up,
...
static void generate_test_instances(std::list < pg_interval_t * >&o);
    ...
    class interval_rep {
      public:
        ...
        virtual std::set < pg_shard_t >
            get_all_participants(bool ec_pool) const = 0;
        virtual void add_interval(bool ec_pool,
                                  const pg_interval_t & interval) = 0;
        ...
        virtual bool has_full_intervals() const {
            return false;
        } virtual void iterate_all_intervals(std::function <
   ...
   */
    static bool is_new_interval(int old_acting_primary,
   ...
   */
    static bool check_new_interval(int old_acting_primary,  ///< [in] primary as of lastmap
    ...
    static bool check_new_interval(int old_acting_primary,  ///< [in] primary as of lastmap
        ...
        return check_new_interval(old_acting_primary, new_acting_primary,
   ...
   */
    std::set < pg_shard_t > get_might_have_unfound(pg_shard_t pg_whoami,
                                                   bool ec_pool) const {
        ceph_assert(past_intervals);
   ...
   */ std::set < pg_shard_t > get_all_probe(
    ...
    struct PriorSet {
        ...
        } bool affected_by_map(const OSDMap & osdmap,
        ...
        template < typename F >
            PriorSet(const PastIntervals & past_intervals,
                     bool ec_pool,
...
std::ostream & operator<<(std::ostream & out,
                          const PastIntervals::PriorSet & i);

template < typename F >
    PastIntervals::PriorSet::PriorSet(const PastIntervals & past_intervals,
                                      bool ec_pool,
                                      epoch_t last_epoch_started,
                                      const IsPGRecoverablePredicate * c,
                                      F f,
...
:ec_pool(ec_pool), pg_down(false), pcontdec(c)
{
    ...
    for (unsigned i = 0; i < up.size(); i++) {
        if (up[i] != pg_pool_t::pg_CRUSH_ITEM_NONE)
            probe.
                insert(pg_shard_t
                       (up[i], ec_pool ? shard_id_t(i) : shard_id_t::NO_SHARD));
    }
  ...
  for (auto && i:all_probe) {
    ...
    past_intervals.iterate_mayberw_back_to(last_epoch_started,
                                           [&](epoch_t start,
                                               const std::set < pg_shard_t >
                                               &acting) {
                                           ldpp_dout(dpp,
                                                     10) <<
                                           "build_prior maybe_rw interval:" <<
                                           start << ", acting: " << acting <<
                                           dendl;
...
case UP:                                  {
                                           ...
                                           up_now.insert(so); break;}
case DNE:                                 {
                                           ldpp_dout(dpp,
                                                     10) <<
                                           "build_prior  prior osd." << so.
                                           osd << " no longer exists" << dendl;
                                           break;}
case LOST:                                {
                                           ldpp_dout(dpp,
                                                     10) <<
                                           "build_prior  prior osd." << so.
                                           osd << " is down, but lost_at " <<
                                           lost_at << dendl; up_now.insert(so);
                                           break;}
case DOWN:                                {
                                           ldpp_dout(dpp,
                                                     10) <<
                                           "build_prior  prior osd." << so.
                                           osd << " is down" << dendl;
                                           candidate_blocked_by[so.osd] =
                                           lost_at; any_down_now = true; break;}
                                           }
                                           }
                                           ...
                                           <<
                                           " insufficient up; including down osds"
                                           << dendl;
                                           ceph_assert(!candidate_blocked_by.
                                                       empty()); pg_down = true;
                                           blocked_by.
                                           insert(candidate_blocked_by.begin(),
    ...
    ldpp_dout(dpp, 10) << "build_prior final: probe " << probe
        << " down " << down
        << " blocked_by " << blocked_by << (pg_down ? " pg_down" : "")
        << dendl;
}
...
struct pg_notify_t {
    ...
    pg_notify_t():query_epoch(0), epoch_sent(0), to(shard_id_t::NO_SHARD),
        from(shard_id_t::NO_SHARD) {
    } pg_notify_t(shard_id_t to,
                  shard_id_t from,
                  epoch_t query_epoch,
                  epoch_t epoch_sent,
                  const pg_info_t & info, const PastIntervals & pi)
    :query_epoch(query_epoch),
        epoch_sent(epoch_sent),
        info(info), to(to), from(from), past_intervals(pi) {
...
struct pg_query_t {
    enum {
     ...
     std::string_view get_type_name() const const
{
    switch (type) {
    case INFO:
        return "info";
...
pg_query_t(int t,
           shard_id_t to,
           shard_id_t from, const pg_history_t & h, epoch_t epoch_sent)
...
pg_query_t(int t,
           shard_id_t to,
           shard_id_t from,
           eversion_t s, const pg_history_t & h, epoch_t epoch_sent)
...
struct pg_lease_t {
    ...
    friend std::ostream & operator<<(std::ostream & out, const pg_lease_t & l) {
        return out << "pg_lease(ru " << l.readable_until
            << " ub " << l.readable_until_ub << " int " << l.interval << ")";
    }
};
    ...
    class Visitor
{
  public:
    ...
    } virtual void setattrs(std::map < std::string,
                            std::optional < ceph::buffer::list >> &attrs) {
    }
    ...
    virtual void rollback_extents(version_t gen,
...
void setattrs(std::map < std::string,
              std::optional < ceph::buffer::list >> &old_attrs) {
    if (!can_local_rollback || rollback_info_completed)
        return;
    ENCODE_START(1, 1, bl);
    append_id(SETATTRS);
...
void rollback_extents(version_t gen, const std::vector < std::pair < uint64_t,
                      uint64_t > >&extents) {
     ...
     OSDOp()
{
   ...
   */
template < typename V >
    static void split_osd_op_vector_in_data(V & ops, ceph::buffer::list & in) {
...
template < typename V >
   ...
   * @param in  [in] combined data buffer
   */
static void split_osd_op_vector_out_data(std::vector < OSDOp > &ops,
                                         ceph::buffer::list & in);
   ...
   * @param out [out] combined data buffer
   */
static void merge_osd_op_vector_out_data(std::vector < OSDOp > &ops,
...
struct pg_log_op_return_item_t {
    ...
    friend std::ostream & operator<<(std::ostream & out,
                                     const pg_log_op_return_item_t & i) {
    ...
    static const char *get_op_name(int op)
{
  ...
  :    
    ...
    // Errors are only used for dup detection, whereas// the index by objects is used by recovery, copy_get,// and other facilities that don't expect or need to// be aware of error entries.
...
:reqid(entry.reqid), version(entry.version),
    ...
    return_code(entry.return_code), op_returns(entry.op_returns) {
}
        ...
        log(std::move(entries)), dups(std::move(dup_entries))
{
    ...
    void merge_from(const std::vector < pg_log_t * >&slogs,
                    eversion_t last_update) {
        log.clear();
    ...
    bool empty() const {
        ...
        return head.version - tail.version;
                             ...
                             const pg_log_t & in, pg_log_t & out,
                             pg_log_t & reject);
...
struct pg_missing_item {
    ...
    } explicit pg_missing_item(eversion_t n):need(n), flags(FLAG_NONE) {
    }                           // have no old version
...
class pg_missing_const_i {
  public:
    virtual const std::map < hobject_t, pg_missing_item > &
        get_items() const const = 0;
    virtual const std::map < version_t,
        hobject_t > &get_rmissing() const const = 0;
    virtual bool get_may_include_deletes() const = 0;
    virtual unsigned int num_missing() const = 0;
    virtual bool have_missing() const = 0;
    virtual bool is_missing(const hobject_t & oid, pg_missing_item * out =
                            nullptr) const = 0;
...
template < bool TrackChanges > class pg_missing_set:public pg_missing_const_i {
  ...
  public:
    ...
    bool is_missing(const hobject_t & oid, pg_missing_item * out =
    ...
    void add_next_event(const pg_log_entry_t & e) {
        ...
        if (e.prior_version == eversion_t() || e.is_clone()) {
            ...
            if (is_missing_divergent_item) {    // use iterator
                ...
                // .have = nil
                missing_it->second =
                    item(e.version, eversion_t(), e.is_delete());
    ...
    void split_into(pg_t child_pgid,
                    unsigned split_bits, pg_missing_set * omissing) {
    ...
    void encode(ceph::buffer::list & bl, uint64_t features) const {
      ...
      for (auto const &i:missing)
        ...
        for (std::map < hobject_t, item >::iterator it =
             missing.begin(); it != missing.end(); ++it)
    ...
    static void generate_test_instances(std::list < pg_missing_set * >&o) {
        ...
        o.push_back(new pg_missing_set);
        o.back()->add(hobject_t(object_t("foo"), "foo", 123, 456, 0, ""),
    ...
    bool is_clean() const {
        ...
        bool debug_verify_from_init(const missing_t & init_missing,
                                    std::ostream * oss)const {
        if (!TrackChanges)
            return true;
        auto check_missing(init_missing.get_items());
        tracker.get_changed([&](const hobject_t & hoid) {
                            check_missing.erase(hoid); if (missing.count(hoid)) {
...
template < bool TrackChanges >
    void encode(const pg_missing_set < TrackChanges > &c,
                ceph::buffer::list & bl, uint64_t features = 0)
{
    ENCODE_DUMP_PRE();
...
template < bool TrackChanges >
    void decode(pg_missing_set < TrackChanges > &c,
...
template < bool TrackChanges >
    std::ostream & operator<<(std::ostream & out,
                              const pg_missing_set < TrackChanges > &missing)
{
...
template < typename T > struct pg_nls_response_template {
    ...
    static void generate_test_instances(std::list < pg_nls_response_template <
                                        T > *>&o) {
...
struct object_copy_cursor_t {
    ...
    object_copy_cursor_t()
    :data_offset(0),
        attr_complete(false), data_complete(false), omap_complete(false)
{
...
struct object_copy_data_t {
  ...
  public:
    object_copy_data_t():
        ...
        omap_digest(-1), flags(0), truncate_seq(0), truncate_size(0)
{
...
{
} watch_info_t(uint64_t c, uint32_t t, const entity_addr_t & a):cookie(c),
    timeout_seconds(t), addr(a) {
}
                                        ...
                                        const notify_info_t & n)
{
    return out << "notify(cookie " << n.cookie
        << " notify" << n.notify_id << " " << n.timeout << "s)";
}
...
struct chunk_info_t {
    ...
    } chunk_info_t(uint32_t offset, uint32_t length,
                   hobject_t oid):offset(offset), length(length), oid(oid),
        flags((cflag_t) 0) {
    }
...
struct ObjectRecoveryProgress {
    ...
    :data_recovered_to(0),
        first(true), data_complete(false), omap_complete(false)
{
} bool is_complete(const ObjectRecoveryInfo & info) const {
    return (data_recovered_to >= (info.copy_subset.empty()?
...
struct PushReplyOp {
    ...
    void encode(ceph::buffer::list & bl) const const;
    void decode(ceph::buffer::list::const_iterator & bl);
     std::ostream & print(std::ostream & out) const const;
    void dump(ceph::Formatter * f) const const;
...
struct watch_item_t {
    ...
    void dump(ceph::Formatter * f) const {
        ...
        ea.set_port(1024);
        o.push_back(new
                    watch_item_t(entity_name_t(entity_name_t::TYPE_CLIENT, 1),
                                 10, 30, ea));
        ea.set_nonce(1001);
        ea.set_in4_quad(3, 2);
        ea.set_port(1025);
        o.push_back(new
                    watch_item_t(entity_name_t(entity_name_t::TYPE_CLIENT, 2),
                                 20, 60, ea));
    }
};
...
struct PromoteCounter {
    ...
    0};
     std::atomic < unsigned long long >objects {
    0};
...
struct pool_pg_num_history_t {
    ...
    static void generate_test_instances(std::list <
                                        pool_pg_num_history_t * >&ls) {
        ls.push_back(new pool_pg_num_history_t);
    }
    friend std::ostream & operator<<(std::ostream & out,
                                     const pool_pg_num_history_t & h) {
        return out << "pg_num_history(e" << h.epoch << " pg_nums " << h.
            pg_nums << " deleted_pools " << h.deleted_pools << ")";
    }
};
...
class PGLSFilter {
  ...
  public:
    ...
    virtual bool filter(const hobject_t & obj,
                        const ceph::buffer::list & xattr_data) const const = 0;
   ...
   */
    virtual const std::string & get_xattr() const const
{
    return xattr;
}
  /**
   ...
   */ virtual bool reject_empty_xattr() const {
    return true;
}};
