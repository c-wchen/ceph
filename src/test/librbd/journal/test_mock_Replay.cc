...
namespace librbd {
    ...
    namespace io {
        ...
        template <> struct ImageRequest <MockReplayImageCtx > {
            ...
            } MOCK_METHOD4(aio_discard,
                           void (AioCompletion * c, uint64_t off, uint64_t len,
                                 bool skip_partial_discard));
            static void aio_discard(MockReplayImageCtx * ictx,
                                    AioCompletion * c, uint64_t off,
            ...
            MOCK_METHOD5(aio_writesame,
                         void (AioCompletion * c, uint64_t off, uint64_t len,
                               const bufferlist & bl, int op_flags));
            static void aio_writesame(MockReplayImageCtx * ictx,
                                      AioCompletion * c, uint64_t off,
                                      uint64_t len, bufferlist
                                      && bl, int op_flags,
                                      const ZTracer::Trace & parent_trace) {
                assert(s_instance != nullptr);
    ...
    namespace journal {
          ...
          public:
            ...
            TestMockJournalReplay():m_invoke_lock("m_invoke_lock") {
                ...
                EXPECT_CALL(mock_io_image_request,
                            aio_discard(_, off, len, skip_partial_discard))
            ...
            void expect_aio_writesame(MockIoImageRequest &
                                      ...
                                      const char *data) {
                EXPECT_CALL(mock_io_image_request,
                            aio_writesame(_, off, len, BufferlistEqual(data),
                                          _))
                    .WillOnce(SaveArg < 0 > (aio_comp));
            }
            ...
            void expect_writeback_cache_enabled(MockReplayImageCtx &
                                                mock_image_ctx, bool enabled) {
        ...
        TEST_F(TestMockJournalReplay, AioDiscard) {
            ...
            when_process(mock_journal_replay, EventEntry {
                         AioDiscardEvent(123, 456, ictx->skip_partial_discard)}
                         , &on_ready, &on_safe);
