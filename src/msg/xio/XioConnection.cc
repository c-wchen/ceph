                       ...
                       const XioMsgHdr & hdr, const struct xio_msg *msg)
{
    if (msg) {
        ldout(cct, 4) << tag <<
            " xio msg:" <<
            " sn: " << msg->sn << " timestamp: " << msg->timestamp << dendl;
    }
    ...
    ldout(cct, 4) << tag <<
        ...
        " xio header: " << " msg_cnt: " << hdr.msg_cnt << dendl;
    ...
    ldout(cct, 4) << tag <<
        ...
        " sig: " << hdr.ftr->sig <<
        " flags: " << (uint32_t) hdr.ftr->flags << dendl;
}
...
void print_ceph_msg(CephContext * cct, const char *tag, Message * m)
{
    if (m->get_magic() & (MSG_MAGIC_XIO & MSG_MAGIC_TRACE_DTOR)) {
        ...
        ldout(cct, 4) << tag << " header version " << header.version <<
            " compat version " << header.compat_version << dendl;
    }
}
...
{
    return *_dout << "-- " << get_messenger()->get_myinst().
        addr << " >> " << peer_addr << " peer=" << peer.name.type_str()
        << " conn=" << conn << " sess=" << session << " ";
}

XioConnection::XioConnection(XioMessenger * m, XioConnection::type _type, const entity_inst_t & _peer):
Connection(m->cct, m),
xio_conn_type(_type),
portal(m->get_portal()),
connected(false),
...
session(NULL),
conn(NULL),
magic(m->get_magic()), scount(0), send_ctr(0), in_seq(), cstate(this)
{
    ...
    if (policy.throttler_bytes) {
        ...
        ldout(m->cct,
              4) << "XioMessenger throttle_bytes: " << max_bytes << dendl;
    }
    ...
    ldout(m->cct,
          4) << "throttle_msgs: " << xopt << " throttle_bytes: " << bytes_opt <<
        dendl;
...
void XioConnection::send_keepalive_or_ack_internal(bool ack, const utime_t * tp)
{
    ...
    ldout(msgr->cct,
          8) << __func__ << " sending command with tag " << (int)(*(char *)msg->
                                                                  out.header.
                                                                  iov_base)
...
int XioConnection::passive_setup()
{
    ...
    /* XXX fake authorizer! */
    msgr->ms_deliver_verify_authorizer(this, peer_type, CEPH_AUTH_NONE,
                                       auth.bl,
                                       authorizer_reply,
...
static inline XioDispatchHook *pool_alloc_xio_dispatch_hook(XioConnection *
                                                            xcon, Message * m,
                                                            XioInSeq & msg_seq)
{
                                   ...
                                   int more_in_batch, void *cb_user_context)
{
    ...
    if (!in_seq.p()) {
        if (!tmsg->in.header.iov_len) {
            ldout(msgr->cct,
                  0) << __func__ << " empty header: packet out of sequence?" <<
                dendl;
            xio_release_msg(msg);
            return 0;
        }
        const size_t sizeof_tag = 1;
        XioMsgCnt msg_cnt(buffer::
                          create_static(tmsg->in.header.iov_len - sizeof_tag,
                                        ((char *)tmsg->in.header.iov_base) +
                                        sizeof_tag));
        ldout(msgr->cct,
              10) << __func__ << " receive msg " << "tmsg " << tmsg <<
            " msg_cnt " << msg_cnt.msg_cnt << " iov_base " << tmsg->in.header.
            iov_base << " iov_len " << (int)tmsg->in.header.
            iov_len << " nents " << tmsg->in.pdata_iov.nents << " sn " << tmsg->
            sn << dendl;
    ...
    ldout(msgr->cct,
          4) << __func__ << " " << "msg_seq.size()=" << msg_seq.size() << dendl;
    ...
    while (blen && (msg_iter != msg_seq.end())) {
        ...
        for (; blen && (ix < iov_len); ++ix) {
            ...
            payload.
                append(buffer::
                       create_msg(take_len, (char *)msg_iov->iov_base, m_hook));
            blen -= take_len;
            if (!blen) {
    ...
    if (blen && left_len) {
        middle.append(buffer::create_msg(left_len, left_base, m_hook));
        left_len = 0;
    }
    ...
    if (blen && left_len) {
        data.append(buffer::create_msg(left_len, left_base, m_hook));
        left_len = 0;
    }
    ...
    while (blen && (msg_iter != msg_seq.end())) {
        ...
        for (; blen && (ix < iov_len); ++ix) {
            ...
            data.
                append(buffer::
                       create_msg(msg_iov->iov_len, (char *)msg_iov->iov_base,
                                  m_hook));
                          ...
                          int more_in_batch, void *cb_user_context)
{
    ...
    case CEPH_MSGR_TAG_KEEPALIVE2:
        if (header_len_without_tag < sizeof(ceph_timespec)) {
            lderr(msgr->
                  cct) << __func__ << " too few data for KEEPALIVE2: got " <<
                header_len_without_tag << " bytes instead of " <<
                sizeof(ceph_timespec) << " bytes" << dendl;
        }
        else {
            ceph_timespec *t =
                (ceph_timespec *) ((char *)msg->in.header.iov_base +
                                   sizeof(tag));
            utime_t kp_t = utime_t(*t);
            ldout(msgr->cct,
                  20) << __func__ << " got KEEPALIVE2 with timestamp" << kp_t <<
                dendl;
    ...
    case CEPH_MSGR_TAG_KEEPALIVE2_ACK:
        if (header_len_without_tag < sizeof(ceph_timespec)) {
            lderr(msgr->
                  cct) << __func__ << " too few data for KEEPALIVE2_ACK: got "
                << header_len_without_tag << " bytes instead of " <<
                sizeof(ceph_timespec) << " bytes" << dendl;
        }
        else {
            ...
            utime_t kp_t(*t);
            ldout(msgr->cct,
                  20) << __func__ << " got KEEPALIVE2_ACK with timestamp" <<
                kp_t << dendl;
    ...
    default:
        lderr(msgr->
              cct) << __func__ << " unsupported message tag " << (int)tag <<
            dendl;
                                           ...
                                           void *conn_user_context)
{
    ...
    if (xmsg) {
        ldout(msgr->cct, 11) << "on_msg_delivered xcon: " <<
            " type: " << xmsg->m->get_type() << " tid: " << xmsg->m->
            get_tid() << " seq: " << xmsg->m->get_seq() << dendl;
    }
...
int XioConnection::discard_out_queues(uint32_t flags)
{
    ...
    while (!deferred_q.empty()) {
        ...
        default:
            ldout(msgr->cct,
                  0) << __func__ << ": Unknown Msg type " << xs->type << dendl;
            break;
        }
    }
