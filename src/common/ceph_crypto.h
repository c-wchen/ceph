...
public:
        ...
        : mpType(type)
    {
        ...
        // just in the case.
        ::TOPNSPC::crypto::zeroize_for_security(&mContext,
                                                sizeof(mContext));
        const auto r =
            HMAC_Init_ex(&mContext, key, length, mpType, nullptr);
        if (r != 1) {
    ...
    {
        const auto r =
            HMAC_Init_ex(&mContext, nullptr, 0, mpType, nullptr);
        if (r != 1) {
            throw DigestException("HMAC_Init_ex() failed");
        }
    }
...
public:
    ...
    {
        const auto r =
            HMAC_Init_ex(mpContext, key, length, type, nullptr);
        if (r != 1) {
            throw DigestException("HMAC_Init_ex() failed");
        }
    } ~HMAC()
    ...
    void Restart()
    {
        ...
        const auto r =
            HMAC_Init_ex(mpContext, nullptr, 0, type, nullptr);
        if (r != 1) {
            throw DigestException("HMAC_Init_ex() failed");
        }
    }
...
template < class Digest > auto digest(const ceph::buffer::list &bl)
{
    ...
    return sha_digest_t < Digest::digest_size > {
        fingerprint
    };
}
}
