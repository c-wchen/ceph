...
int ObjectCache::get(const string & name, ObjectCacheInfo & info, uint32_t mask,
                     rgw_cache_entry_info * cache_info)
{
    ...
    if (expiry.count() &&
        ...
        expiry) {
        ldout(cct,
              10) << "cache get: name=" << name << " : expiry miss" << dendl;
        lock.unlock();
        lock.get_write();
    ...
    if (lru_counter - entry->lru_promotion_ts > lru_window) {
        ...
        if (iter == cache_map.end()) {
            ldout(cct,
                  10) << "lost race! cache get: name=" << name << " : miss" <<
                dendl;
            if (perfcounter)
                perfcounter->inc(l_rgw_cache_miss);
            return -ENOENT;
    ...
    if ((src.flags & mask) != mask) {
        ldout(cct,
              10) << "cache get: name=" << name << " : type miss (requested=0x"
            << std::hex << mask << ", cached=0x" << src.
            flags << std::dec << ")" << dendl;
        if (perfcounter)
            perfcounter->inc(l_rgw_cache_miss);
        return -ENOENT;
...
bool ObjectCache::chain_cache_entry(list <
                                    ...
                                    RGWChainedCache::Entry * chained_entry)
{
    ...
    for (citer = cache_info_entries.begin(); citer != cache_info_entries.end();
         ++citer) {
        ...
        ldout(cct,
            ...
            cache_locator << dendl;
        map < string, ObjectCacheEntry >::iterator iter =
            cache_map.find(cache_info->cache_locator);
        if (iter == cache_map.end()) {
            ldout(cct,
    ...
    for (liter = cache_entry_list.begin(); liter != cache_entry_list.end();
         ++liter) {
        ObjectCacheEntry *entry = *liter;

        entry->chained_entries.
            push_back(make_pair(chained_entry->cache, chained_entry->key));
    }

    return true;
}

void ObjectCache::put(const string & name, ObjectCacheInfo & info,
                      rgw_cache_entry_info * cache_info)
{
    RWLock::WLocker l(lock);
    ...
    if (info.flags & CACHE_FLAG_XATTRS) {
        ...
        for (iter = target.xattrs.begin(); iter != target.xattrs.end(); ++iter) {
            ldout(cct,
                ...
                first << " bl.length()=" << iter->second.length() << dendl;
        }
    }
    else if (info.flags & CACHE_FLAG_MODIFY_XATTRS) {
        map < string, bufferlist >::iterator iter;
        for (iter = info.rm_xattrs.begin(); iter != info.rm_xattrs.end();
             ++iter) {
...
bool ObjectCache::remove(const string & name)
{
    ...
    for (list < pair < RGWChainedCache *, string > >::iterator iiter =
         entry.chained_entries.begin(); iiter != entry.chained_entries.end();
         ++iiter) {
                            ...
                            std::list < string >::iterator & lru_iter)
{
    while (lru_size > (size_t) cct->_conf->rgw_cache_lru_size) {
        ...
        map < string, ObjectCacheEntry >::iterator map_iter =
            cache_map.find(*iter);
        ldout(cct,
              10) << "removing entry: name=" << *iter << " from cache LRU" <<
            dendl;
...
{
    for (list < pair < RGWChainedCache *, string > >::iterator iter =
         entry.chained_entries.begin(); iter != entry.chained_entries.end();
         ++iter) {
...
void ObjectCache::do_invalidate_all()
{
    ...
    for (list < RGWChainedCache * >::iterator iter = chained_cache.begin();
         iter != chained_cache.end(); ++iter) {
        (*iter)->invalidate_all();
    }
}
