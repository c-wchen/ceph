...
struct nest_info_t:public scatter_info_t {
    ...
    bool same_sums(const nest_info_t & o) const {
        return rctime <= o.rctime &&
            rbytes == o.rbytes &&
            rfiles == o.rfiles && rsubdirs == o.rsubdirs && rsnaps == o.rsnaps;
    } void encode(ceph::buffer::list & bl) const;
    void decode(ceph::buffer::list::const_iterator & bl);
    void dump(ceph::Formatter * f) const;
    void decode_json(JSONObj * obj);
...
struct quota_info_t {
    ...
    bool is_valid() const {
        ...
        case QUOTA_MAX_FILES:
            ...
            case QUOTA_ANY:default:return !!max_bytes || !!max_files;
    }} void decode_json(JSONObj * obj);
  ...
  public:
    ...
    size_t length() const {
        ...
        return length() == o.length() &&
            (length() == 0 ||
             (*const_cast < ceph::buffer::list * >(blp.get()) == *const_cast <
              ceph::buffer::list * >(o.blp.get())));
    } bool operator!=(const inline_data_t & o)const {
        return !(*this == o);
    } void encode(ceph::buffer::list & bl) const;
    void decode(ceph::buffer::list::const_iterator & bl);
...
static void old_pools_cb(compact_set < int64_t, std::less < int64_t >,
                         Allocator < int64_t > >&c, JSONObj * obj);
void decode_json(JSONObj * obj);
...
template < template < typename > class Allocator >
    void inode_t < Allocator >::encode(ceph::buffer::list & bl,
...
template < template < typename > class Allocator >
    void inode_t < Allocator >::dump(ceph::Formatter * f) const const
{
  ...
  for (const auto & p:old_pools) {
    ...
    f->dump_bool("export_ephemeral_distributed_pin",
...
template < template < typename > class Allocator >
    void inode_t < Allocator >::client_ranges_cb(typename inode_t <
                                                 Allocator >::
                                                 client_range_map & c,
                                                 JSONObj * obj)
{
    ...
    JSONDecoder::decode_json("follows", client_range_tmp.follows.val, obj,
                             true);
    c[client] = client_range_tmp;
}

template < template < typename > class Allocator >
    void inode_t < Allocator >::old_pools_cb(compact_set < int64_t,
                                             std::less < int64_t >,
                                             Allocator < int64_t > >&c,
                                             JSONObj * obj)
{
    ...
    void inode_t < Allocator >::decode_json(JSONObj * obj)
{
    ...
    JSONDecoder::decode_json("old_pools", old_pools,
                             inode_t < Allocator >::old_pools_cb, obj, true);
    ...
    JSONDecoder::decode_json("client_ranges", client_ranges,
                             inode_t < Allocator >::client_ranges_cb, obj,
                             true);
    ...
    JSONDecoder::decode_json("last_scrub_version", last_scrub_version, obj,
                             true);
}

template < template < typename > class Allocator >
    void inode_t < Allocator >::generate_test_instances(std::list <
                                                        inode_t * >&ls)
{
...
template < template < typename > class Allocator >
    inline void encode(const inode_t < Allocator > &c,::ceph::buffer::list & bl,
                       uint64_t features)
{
    ENCODE_DUMP_PRE();
