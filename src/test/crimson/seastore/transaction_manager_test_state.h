  ...
  protected:
    ...
    seastar::future <> restart_fut() {
        ...
        return teardown().then([this] {
                               devices->remount(); return _init().then([this] {
                                                                       return
                                                                       _mount().
                                                                       handle_error
                                                                       (crimson::
    ...
    seastar::future <> tm_setup(journal_type_t type = journal_type_t::SEGMENTED) {
        LOG_PREFIX(EphemeralTestState::tm_setup);
        journal_type = type;
        if (journal_type == journal_type_t::SEGMENTED) {
            devices.reset(new
                          EphemeralSegmentedDevices(num_main_device_managers,
                                                    num_cold_device_managers));
        }
        else {
        ...
        return devices->setup().then([this] {
                                     return _init();}
        ).then([this, FNAME] {
               return _mkfs().safe_then([this] {
                                        return restart_fut();}
               ).handle_error(crimson::ct_error::assert_all {
                              }
               ).then([FNAME] {
    ...
    seastar::future <> tm_teardown() {
        ...
        return teardown().then([this, FNAME] {
                               devices->reset(); SUBINFO(test, "finish");}
        );
    }
};
...
class TMTestState:public EphemeralTestState {
  protected:
    ...
    virtual FuturizedStore::mount_ertr::future <> _mount() {
        return tm->mount().handle_error(crimson::ct_error::assert_all {
                                        "Error in mount"}
        ).then([this] {
...
{
    segment_manager =
        segment_manager::EphemeralSegmentManagerRef(static_cast <
                                                    segment_manager::
...
{
    rb_device =
        random_block_device::RBMDeviceRef(static_cast <
...
class SeaStoreTestState:public EphemeralTestState {
          ...
          public:
            Store(TestMDStoreState & parent):parent(parent) {
                ...
                if (iter != parent.md.end()) {
                    return read_meta_ret(read_meta_ertr::ready_future_marker {
                                         }
                                         , iter->second);
                }
                else {
  ...
  SeaStoreTestState():EphemeralTestState(1, 0) {
    ...
    virtual seastar::future <> _init() final {
        seastore =
            make_test_seastore(std::make_unique < TestMDStoreState::Store >
                               (mdstore_state.get_mdstore()));
