    ...
    namespace {
        ...
        struct C_notify_Finish:public Context {
            ...
            size_t *preply_buf_len;
             ...
             C_notify_Finish(CephContext * _cct, Context * _ctx,
                             Objecter * _objecter,
        ...
        struct C_aio_linger_cancel:public Context {
            Objecter *objecter;
             ...
             C_aio_linger_cancel(Objecter * _objecter,
                                 Objecter::LingerOp * _linger_op)
        ...
        struct C_aio_linger_Complete:public Context {
            ...
            :c(_c), linger_op(_linger_op), cancel(_cancel) {
                ...
                if (cancel || r < 0)
                    c->io->client->finisher.
                        queue(new
                              C_aio_linger_cancel(c->io->objecter, linger_op));
                                 ...
                                 uint64_t snapid)
{
    ...
    objecter->mutate(oid, oloc,
                     op, snapc, ceph::real_clock::now(), 0, onack, NULL);
...
{
    objecter->change_pool_auid(poolid, new C_PoolAsync_Safe(c), auid);
    return 0;
}
                                      ...
                                      o, bufferlist * pbl, int flags)
{
    ...
    Objecter::Op * objecter_op =
        objecter->prepare_read_op(oid, oloc, *o, snap_seq, pbl, flags, onack,
                                  &ver);
    objecter->op_submit(objecter_op);
                                     ...
                                     const blkin_trace_info * trace_info)
{
    ...
    trace.event("init root span");
    Objecter::Op * op =
        objecter->prepare_mutate_op(oid, oloc, *o, snap_context, ut, flags,
                                    ...
                                    uint64_t off, bufferlist & cmp_bl)
{
    ...
    Objecter::Op * o =
        objecter->prepare_cmpext_op(oid, oloc, off, cmp_bl, snap_seq, 0, onack,
                                    &c->objver);
    objecter->op_submit(o, &c->tid);
                                    ...
                                    size_t cmp_len, uint64_t off)
{
    ...
    Objecter::Op * o =
        objecter->prepare_read_op(oid, oloc, onack->m_ops, snap_seq, NULL, 0,
                                  onack, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                      ...
                                      time_t > >*pls)
{
    ...
    Objecter::Op * o =
        objecter->prepare_pg_read_op(hash, oloc, rd, NULL, 0, oncomplete, NULL,
                                     NULL);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                                  ...
                                                  uint32_t * interval)
{
    ...
    Objecter::Op * o =
        objecter->prepare_pg_read_op(oloc.hash, oloc, op, nullptr,
                                     CEPH_OSD_FLAG_PGOP, oncomplete, nullptr,
                                     nullptr);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                  ...
                                  bufferlist & inbl, bufferlist * outbl)
{
    ...
    rd.call(cls, method, inbl);
    Objecter::Op * o =
        objecter->prepare_read_op(oid, oloc, rd, snap_seq, outbl, 0, oncomplete,
                                  &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                  ...
                                  bufferlist & inbl, char *buf, size_t out_len)
{
    ...
    rd.call(cls, method, inbl);
    Objecter::Op * o =
        objecter->prepare_read_op(oid, oloc, rd, snap_seq, &c->bl, 0,
                                  oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                ...
                                std::map < uint64_t, uint64_t > &m)
{
    ...
    objecter->mapext(oid, oloc, off, len, snap_seq, &bl, 0, onack);
...
struct WatchInfo:public Objecter::WatchContext {
    ...
    WatchInfo(librados::IoCtxImpl * io, object_t o,
              librados::WatchCtx * c, librados::WatchCtx2 * c2, bool inter)
...
int librados::IoCtxImpl::unwatch(uint64_t cookie)
{
    ...
    objecter->mutate(linger_op->target.base_oid, oloc, wr,
                     snapc, ceph::real_clock::now(), 0, &onfinish, &ver);
    objecter->linger_cancel(linger_op);
...
int librados::IoCtxImpl::aio_unwatch(uint64_t cookie, AioCompletionImpl * c)
{
    ...
    wr.watch(cookie, CEPH_OSD_WATCH_OP_UNWATCH);
    objecter->mutate(linger_op->target.base_oid, oloc, wr,
                     snapc, ceph::real_clock::now(), 0, oncomplete, &c->objver);
    return 0;
}
                                ...
                                char **preply_buf, size_t * preply_buf_len)
{
    ...
    Context *notify_finish =
        new C_notify_Finish(client->cct, &notify_finish_cond,
