...
public:
    ...
    virtual key_t get_intermediate_key() const
    {
        return min_max_t<key_t>::null;
    }
    virtual key_t get_intermediate_base() const
    {
        return min_max_t<key_t>::null;
    }
    virtual extent_len_t get_intermediate_length() const
    {
        return 0;
    }
...
{
public:
    template <typename... T> ChildableCachedExtent(T&&... t) : CachedExtent(std::forward<T>(t)...) {}
...
template <typename T> read_set_item_t<T>::read_set_item_t(T *t, CachedExtentRef ref)
    : t(t), ref(ref)
{}
