...
static inline void add_name_val_to_obj(std::string &n, std::string &v, rapidjson::GenericValue < E, A > &d,
                                       A &allocator)
{
...
template < typename E, typename A = ZeroPoolAllocator >
static inline void add_name_val_to_obj(std::string &n, bool v, rapidjson::GenericValue < E, A > &d,
                                       A &allocator)
{
    rapidjson::GenericValue < E, A > name, val;
    name.SetString(n.c_str(), n.length(), allocator);
...
template < typename E, typename A = ZeroPoolAllocator >
static inline void add_name_val_to_obj(const char *n, std::string &v, rapidjson::GenericValue < E, A > &d,
                                       A &allocator)
{
...
KmipGetTheKey&KmipGetTheKey::get_uniqueid_for_keyname(optional_yield y)
{
...
int KmipGetTheKey::get_key_for_uniqueid(optional_yield y, std::string& actual_key)
{
    if (failed) {
        return ret;
    }
