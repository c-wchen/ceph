...
std::unique_ptr<rgw::sal::RGWOIDCProvider> WebTokenEngine::get_provider(const DoutPrefixProvider *dpp,
        const string &role_arn, const string &iss, optional_yield y) const
{
...
template <typename T>
void WebTokenEngine::recurse_and_insert(const string &key, const jwt::claim &c, T &t) const
{
    string s_val;
...
std::tuple<boost::optional<WebTokenEngine::token_t>, boost::optional<WebTokenEngine::principal_tags_t>>
        WebTokenEngine::get_from_jwt(
            const DoutPrefixProvider *dpp, const std::string &token, const req_state *const s,
...
void WebTokenEngine::validate_signature(const DoutPrefixProvider *dpp, const jwt::decoded_jwt &decoded,
                                        const string &algorithm, const string &iss, const vector<string> &thumbprints, optional_yield y) const
{
            ...
            if (parser.get_data("keys", &val)) {
                ...
                if (parser.parse(val.str.c_str(), val.str.size())) {
                    ...
                    if (JSONDecoder::decode_json("x5c", x5c, &parser)) {
                        ...
                        if (! found_valid_cert) {
                            ldpp_dout(dpp, 0) << "Cert doesn't match that with the thumbprints registered with oidc provider: " << cert.c_str() <<
                                              dendl;
                            throw - EINVAL;
                        }
                        try {
...
RGWHandler_REST *RGWRESTMgr_STS::get_handler(rgw::sal::Driver *driver,
        req_state *const s,
        const rgw::auth::StrategyRegistry &auth_registry,
