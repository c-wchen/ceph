...
namespace {
    ...
    class DatalogTrimImplCR:public RGWSimpleCoroutine {
      ...
      public:
         DatalogTrimImplCR(const DoutPrefixProvider * dpp,
                           rgw::sal::RadosStore * store, int shard,
                           const std::string & marker,
                           std::string * last_trim_marker)
        :RGWSimpleCoroutine(store->ctx()), dpp(dpp), store(store), shard(shard),
            marker(marker), last_trim_marker(last_trim_marker) {
            set_description() << "Datalog trim shard=" << shard << " marker=" <<
                marker;
        ...
        int request_complete() override {
            ...
            ldpp_dout(dpp,
                      20) << __PRETTY_FUNCTION__ << "(): trim of shard=" <<
                shard << " marker=" << marker << " returned r=" << r << dendl;
...
int DataLogTrimCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ldpp_dout(dpp,
                  10) << "fetching sync status for zone " << zone_id << dendl;
        ...
        yield {
          ...
          for (auto & c:store->svc()->zone->
                 get_zone_data_notify_to_map()) {
                ldpp_dout(dpp,
                          20) << "query sync status from " << c.first << dendl;
...
int DataLogTrimPollCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        for (;;) {
            ...
            set_status("trimming");
            yield call(new
                       DataLogTrimCR(dpp, store, http, num_shards, last_trim));
