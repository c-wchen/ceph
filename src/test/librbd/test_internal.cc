...
public:
    ...
    {
        unlock_image();
        for (Snaps::iterator iter = m_snaps.begin(); iter != m_snaps.end();
             ++iter) {
...
void generate_random_iomap(librbd::Image &image, int num_objects,
                           int object_size, int max_count, map < uint64_t,
                           uint64_t > &iomap)
{
    ...
    while (max_count-- > 0) {
        ...
        Striper::file_to_extents(g_ceph_context, 1, &layout, imageoff,
                                 TEST_IO_SIZE, 0, ex);
...
TEST_F(TestInternal, OpenSnapDNE)
{
    ...
    ictx =
        new librbd::ImageCtx(m_image_name, "", "unknown_snap", m_ioctx,
                             true);
    ASSERT_EQ(-ENOENT,
              ictx->state->open(librbd::OPEN_FLAG_SKIP_OPEN_PARENT));
}
...
TEST_F(TestInternal, ResizeFailsToLockImage)
{
    ...
    ASSERT_EQ(0, open_image(m_image_name, &ictx));
    ASSERT_EQ(0,
              lock_image(*ictx, ClsLockType::EXCLUSIVE, "manually locked"));

    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(-EROFS,
              ictx->operations->resize(m_image_size >> 1, true, no_op));
}
...
TEST_F(TestInternal, SnapCreateLocksImage)
{
    ...
    BOOST_SCOPE_EXIT((ictx)) {
        ASSERT_EQ(0,
                  ictx->operations->
                  snap_remove(cls::rbd::UserSnapshotNamespace(), "snap1"));
    }
...
TEST_F(TestInternal, SnapRollbackLocksImage)
{
    ...
    ASSERT_EQ(0,
              ictx->operations->
              snap_rollback(cls::rbd::UserSnapshotNamespace(), "snap1",
                            no_op));
...
TEST_F(TestInternal, FlattenLocksImage)
{
    ...
    ASSERT_EQ(0,
              librbd::clone(m_ioctx, m_image_name.c_str(), "snap1", m_ioctx,
...
TEST_F(TestInternal, FlattenFailsToLockImage)
{
    ...
    BOOST_SCOPE_EXIT((&m_ioctx)(clone_name)(parent)(&ictx2)) {
        ...
        librbd::NoOpProgressContext no_op;
        ASSERT_EQ(0,
                  librbd::api::Image <>::remove(m_ioctx, clone_name,
                                                no_op));
    }
    BOOST_SCOPE_EXIT_END;
    ...
    ASSERT_EQ(0, open_image(clone_name, &ictx2));
    ASSERT_EQ(0,
              lock_image(*ictx2, ClsLockType::EXCLUSIVE,
                         "manually locked"));
...
TEST_F(TestInternal, WriteFailsToLockImageBlocklisted)
{
    ...
    auto ictx =
        new librbd::ImageCtx(m_image_name, "", nullptr, blocklist_ioctx,
                             false);
    ASSERT_EQ(0, ictx->state->open(0));
    ...
    ASSERT_EQ(-EBLOCKLISTED, api::Io <>::write(*ictx, 0, bl.length(),
              std::move(bl), 0));
    ASSERT_EQ(-EBLOCKLISTED,
              librbd::is_exclusive_lock_owner(ictx, &lock_owner));
...
TEST_F(TestInternal, AioWriteRequestsLock)
{
    ...
    bufferlist bl;
    bl.append(buffer);
    api::Io <>::aio_write(*ictx, c, 0, buffer.size(), std::move(bl), 0,
                          true);
...
TEST_F(TestInternal, MultipleResize)
{
    ...
    while (size > 0) {
        ...
        ictx->operations->execute_resize(new_size, true, prog_ctx,
                                         contexts.back(), 0);
    }
...
TEST_F(TestInternal, Metadata)
{
    ...
    map < string, bool > test_confs =
        boost::assign::map_list_of("aaaaaaa",
                                   false)
        ("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", false)("cccccccccccccc",
            false);
...
TEST_F(TestInternal, SnapshotCopyup)
{
    ...
    bufferlist bl;
    bl.append(std::string(256, '1'));
    ASSERT_EQ(256, api::Io <>::write(*ictx, 0, bl.length(), bufferlist {
        bl}
    , 0));
    ASSERT_EQ(256, api::Io <>::write(*ictx, 1024, bl.length(), bufferlist {
        bl}
    , 0));
    ...
    ASSERT_EQ(256, api::Io <>::write(*ictx2, 256, bl.length(), bufferlist {
        bl}, 0));
    ...
    uint64_t copyup_end =
        ictx2->enable_sparse_copyup ? 1024 + 256 : 1 << order;
    std::vector < std::pair < uint64_t, uint64_t > >expected_overlap =
        boost::assign::list_of(std::make_pair(0, 256))(std::
            make_pair(512,
                      copyup_end
                      - 512));
    ...
    for (std::list < std::string >::iterator it = snaps.begin();
        ...
        ASSERT_EQ(0,
                  librbd::api::Image <>::snap_set(ictx2,
                          cls::rbd::
                          UserSnapshotNamespace(),
                          snap_name));

        ASSERT_EQ(256,
        api::Io <>::read(*ictx2, 0, 256, librbd::io::ReadResult {
            read_result}, 0));
        ...
        // verify sparseness was preserved
        {
            ...
            EXPECT_EQ(0,
                      rados.conf_set("rbd_sparse_read_threshold_bytes",
                                     "256"));
            auto ictx3 =
                new librbd::ImageCtx(clone_name, "", snap_name, io_ctx,
                                     true);
...
TEST_F(TestInternal, ResizeCopyup)
{
    ...
    for (size_t i = 0; i < m_image_size; i += bl.length()) {
        ...
        api::Io <>::write(*ictx, i, bl.length(), bufferlist {
            bl}
        , 0));
    }
...
TEST_F(TestInternal, ImageOptions)
{
    ...
    ASSERT_EQ(-EINVAL,
              librbd::image_options_get(opts1, RBD_IMAGE_OPTION_FEATURES,
                                        &string_val1));
    ASSERT_EQ(-ENOENT,
              librbd::image_options_get(opts1, RBD_IMAGE_OPTION_FEATURES,
                                        &uint64_val1));
    ...
    ASSERT_EQ(-EINVAL,
              librbd::image_options_set(opts1, RBD_IMAGE_OPTION_FEATURES,
                                        string_val1));
    ...
    ASSERT_NE(uint64_val1, uint64_val2);
    ASSERT_EQ(-ENOENT,
              librbd::image_options_get(opts1, RBD_IMAGE_OPTION_ORDER,
                                        &uint64_val1));
    ASSERT_EQ(-ENOENT,
              librbd::image_options_get(opts2, RBD_IMAGE_OPTION_ORDER,
                                        &uint64_val2));
    ASSERT_EQ(0,
              librbd::image_options_set(opts2, RBD_IMAGE_OPTION_ORDER,
                                        uint64_val2));
    ASSERT_EQ(0,
              librbd::image_options_get(opts1, RBD_IMAGE_OPTION_ORDER,
                                        &uint64_val1));
    ...
    ASSERT_EQ(0,
              librbd::image_options_unset(opts2, RBD_IMAGE_OPTION_ORDER));
    ASSERT_EQ(-ENOENT,
              librbd::image_options_unset(opts2, RBD_IMAGE_OPTION_ORDER));

    librbd::image_options_clear(opts2);
    ASSERT_EQ(-ENOENT,
              librbd::image_options_get(opts2, RBD_IMAGE_OPTION_FEATURES,
                                        &uint64_val2));
...
TEST_F(TestInternal, WriteFullCopyup)
{
    ...
    ASSERT_EQ((ssize_t) bl.length(),
    api::Io <>::write(*ictx, 0, bl.length(), bufferlist {
        bl}
    , 0));
    ...
    ASSERT_EQ(0,
              librbd::clone(m_ioctx, m_image_name.c_str(), "snap1", m_ioctx,
                            clone_name.c_str(), ictx->features, &order, 0,
                            0));
    ...
    ASSERT_EQ(0,
              ictx2->operations->
              snap_create(cls::rbd::UserSnapshotNamespace(), "snap1", 0,
                          no_op));
...
TEST_F(TestInternal, DiffIterateCloneOverwrite)
{
    ...
    ASSERT_EQ(0,
              image.diff_iterate2(NULL, 0, size, false, false, iterate_cb,
                                  (void *)&one));
    ...
    ASSERT_EQ(0, rbd.clone(m_ioctx, m_image_name.c_str(), "one", m_ioctx,
                           clone_name.c_str(), RBD_FEATURE_LAYERING,
                           &order));
    ...
    ASSERT_EQ(0,
              ictx->operations->
              snap_protect(cls::rbd::UserSnapshotNamespace(), "one"));
    ...
    ASSERT_EQ(0, librbd::api::Image <>::snap_set(ictx,
              ...
              "one"));
    ASSERT_EQ(0,
              librbd::api::DiffIterate <>::diff_iterate(ictx,
                      cls::rbd::
                      UserSnapshotNamespace
                      (), nullptr, 0,
                      size, true, false,
...
TEST_F(TestInternal, TestCoR)
{
    ...
    ASSERT_EQ(0,
              create_image_full_pp(m_rbd, m_ioctx, m_image_name,
                                   m_image_size, features, false, &order));
    ...
    generate_random_iomap(image, object_num, info.obj_size, 100,
                          write_tracker);
    ...
    printf("verify written data by reading\n");
    {
        ...
        ASSERT_EQ(TEST_IO_SIZE,
                  image.read(itr->second, TEST_IO_SIZE, readbl));
    ...
    while (rados_nobjects_list_next(list_ctx, &entry, NULL, NULL) !=
           -ENOENT) {
        if (boost::starts_with(entry, block_name_prefix)) {
            const char *block_name_suffix =
                entry + block_name_prefix.length();
    ...
    ASSERT_EQ(0,
              m_rbd.open(m_ioctx, image, m_image_name.c_str(),
                         snapname.c_str()));
    ...
    ASSERT_EQ(0,
              clone_image_pp(m_rbd, image, m_ioctx, m_image_name.c_str(),
                             snapname.c_str(), m_ioctx, clonename.c_str(),
                             features));
    ASSERT_EQ(0, image.close());
    ...
    printf
    ("check whether child image has the same set of objects as parent\n");
    ASSERT_EQ(0, m_rbd.open(m_ioctx, image, clonename.c_str(), NULL));
    ...
    while (rados_nobjects_list_next(list_ctx, &entry, NULL, NULL) !=
        ...
        if (boost::starts_with(entry, block_name_prefix)) {
            ...
            set < string >::iterator it =
                obj_checker.find(block_name_suffix);
...
TEST_F(TestInternal, FlattenNoEmptyObjects)
{
    ...
    printf("made parent image \"%s\": %" PRIu64 "K (%d * %" PRIu64 "K)\n",
           m_image_name.c_str(), m_image_size, object_num,
           info.obj_size / 1024);
...
TEST_F(TestInternal, PoolMetadataConfApply)
{
    ...
    ASSERT_EQ(0,
              librbd::api::PoolMetadata <>::set(m_ioctx, "conf_rbd_cache",
                      new_rbd_conf_cache));
    ...
    ASSERT_EQ(0,
              librbd::api::PoolMetadata <>::remove(m_ioctx,
                      "conf_rbd_cache"));
    ASSERT_EQ(0, ictx->state->refresh());
    ASSERT_EQ(cache, ictx->cache);
    close_image(ictx);
    ...
    ASSERT_TRUE(::get_features(&features));
    ASSERT_EQ(0,
              create_image_full_pp(m_rbd, m_ioctx, image_name, m_image_size,
    ...
    if (is_feature_enabled(RBD_FEATURE_JOURNALING)) {
        ...
        int64_t pool_id;
        C_SaferCond cond;
        cls::journal::client::get_immutable_metadata(m_ioctx,
                "journal." + ictx->id,
                &order, &splay_width,
                &pool_id, &cond);
        ...
        ASSERT_EQ(order, 13);
        ASSERT_EQ(0,
                  ictx->operations->update_features(RBD_FEATURE_JOURNALING,
                          false));
        ASSERT_EQ(0,
                  librbd::api::PoolMetadata <>::set(m_ioctx,
                          "conf_rbd_journal_order",
                          "14"));
        ASSERT_EQ(0,
                  ictx->operations->update_features(RBD_FEATURE_JOURNALING,
                          true));
        ASSERT_EQ(ictx->config.get_val < uint64_t > ("rbd_journal_order"),
                  14U);
...
TEST_F(TestInternal, Sparsify)
{
    ...
    std::cout << "sparse_read_supported=" << sparse_read_supported << std::
              endl;

    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(0,
              ictx->operations->resize((1 << ictx->order) * 20, true,
                                       no_op));
    ...
    librbd::io::ReadResult read_result {
        &read_bl};
    ASSERT_EQ((ssize_t) read_bl.length(),
              api::Io <>::read(*ictx, (1 << ictx->order) * 10,
    read_bl.length(), librbd::io::ReadResult {
        read_result}
    , 0));
    ASSERT_TRUE(bl.contents_equal(read_bl));
...
TEST_F(TestInternal, SparsifyClone)
{
    ...
    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(0,
              ictx->operations->resize((1 << ictx->order) * 10, true,
                                       no_op));
    ...
    int order = ictx->order;
    ASSERT_EQ(0,
              librbd::clone(m_ioctx, m_image_name.c_str(), "snap", m_ioctx,
                            clone_name.c_str(), ictx->features, &order, 0,
                            0));
    close_image(ictx);
...
TEST_F(TestInternal, MissingDataPool)
{
    ...
    using ceph::encode;
    encode(pool_id, bl);
    ASSERT_EQ(0, m_ioctx.omap_set(header_oid, { {
            "data_pool_id", bl
        }
    }
                                 ));
    ...
    librbd::io::ReadResult read_result {
        &read_bl};
    ASSERT_EQ(-ENODEV,
    api::Io <>::read(*ictx, 0, 256, librbd::io::ReadResult {
        read_result}
    , 0));
    ASSERT_EQ(-ENODEV, api::Io <>::write(*ictx, 0, bl.length(), bufferlist {
        bl}
    , 0));
    ASSERT_EQ(-ENODEV, api::Io <>::discard(*ictx, 0, 1, 256));
    ASSERT_EQ(-ENODEV,
    api::Io <>::write_same(*ictx, 0, bl.length(), bufferlist {
        bl}
    , 0));
    ...
    ASSERT_EQ(0,
              librbd::api::Image <>::remove(m_ioctx, m_image_name, no_op));

    ASSERT_EQ(0,
              create_image_pp(m_rbd, m_ioctx, m_image_name, m_image_size));
}
