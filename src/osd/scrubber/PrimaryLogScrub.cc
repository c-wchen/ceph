...
bool PrimaryLogScrub::get_store_errors(const scrub_ls_arg_t & arg,
                                       scrub_ls_result_t & res_inout) constconst
{
    if (!m_store) {
        return false;
    }
...
void PrimaryLogScrub::submit_digest_fixes(const digests_fixes_t & fixes)
{
  ...
  for (auto &[obj, dgs]:fixes) {
        ...
        dout(15) << fmt::format("{}: {}, pg[{}] {}/{}", __func__,
...
{
    auto & info = m_pg->get_pg_info(ScrubberPasskey {
                                    }); ///< a temporary alias
    ...
    if (info.stats.stats_invalid) {
        m_pl_pg->recovery_state.
            update_stats([ =, this] (auto & history, auto & stats) {
                         stats.stats = m_scrub_cstat;
                         stats.stats_invalid = false; return false;}
        );
...
PrimaryLogScrub::PrimaryLogScrub(PrimaryLogPG * pg):PgScrubber
{
pg}
...
void PrimaryLogScrub::
stats_of_handled_objects(const object_stat_sum_t & delta_stats,
                         const hobject_t & soid)
{
    ...
    if (is_primary() && is_scrub_active()) {
        if (soid < m_start) {
            ...
            dout(20) << fmt::format("{} {} < [{},{})", __func__, soid, m_start,
                                    m_end)
                << dendl;
            ...
            dout(25)
                << fmt::format("{} {} >= [{},{})", __func__, soid, m_start,
                               m_end)
                << dendl;
        }
    }
