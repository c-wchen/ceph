...
// It should not be used generally.
        template < typename T > class rvalue_reference_wrapper {
          public:
            ...
            rvalue_reference_wrapper(T & r) noexcept:p(std::addressof(r)) {
            }
            // We write our semantics to match those of reference collapsing. If// we're treated as an lvalue, collapse to one.
                rvalue_reference_wrapper(const rvalue_reference_wrapper &)
                noexcept = default;
            rvalue_reference_wrapper(rvalue_reference_wrapper
                                     &&) noexcept = default;

            // assignment
            rvalue_reference_wrapper & operator=(const rvalue_reference_wrapper
                                                 & x) noexcept = default;
            rvalue_reference_wrapper & operator=(rvalue_reference_wrapper
                                                 && x) noexcept = default;
            ...
            template < typename ... Args >
                std::result_of_t < T & (Args && ...) > operator ()(Args
                                                                   && ... args)
                const {
        ...
        class base {
          protected:
            ceph::mutex lock =
                ceph::make_mutex("ceph::async::detail::base::lock");
  ...
  template < typename ... Ret > class waiter final:public detail::base {
      ...
      public:
        ...
        ~waiter() {
            using std::tuple;
            if (has_value)
                reinterpret_cast < tuple < Ret ... >*>(&ret)->~tuple <
                    Ret ... >();
        }
    };
}
