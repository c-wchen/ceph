...
struct DiffContext {
    ...
    template < typename I > DiffContext(I &image_ctx,
                                        DiffIterate <>::Callback callback,
                                        void *callback_arg, bool _whole_object,
                                        bool _include_parent, uint64_t _from_snap_id,
        ...
        : callback(callback), callback_arg(callback_arg),
          ...
          throttle(image_ctx.config.template get_val < uint64_t >
...
{
public:
    C_DiffObject(I &image_ctx, DiffContext &diff_context,
                 uint64_t image_offset, uint64_t image_length)
    ...
    } void send()
    {
        ...
        if (!m_diff_context.include_parent
            || m_diff_context.from_snap_id != 0) {
            list_snaps_flags |=
        ...
        auto req =
            io::ImageDispatchSpec::create_list_snaps(m_image_ctx,
                io::
    ...
    void compute_diffs(Diffs *diffs)
    {
        ...
        for (auto &[key, snapshot_extents] : m_snapshot_delta) {
            for (auto &snapshot_extent : snapshot_extents) {
                ...
                aggregate_snapshot_extents.insert(snapshot_extent.
                                                  get_off(),
                                                  snapshot_extent.
                get_len(), {
                    state,
        ...
        for (auto &snapshot_extent : aggregate_snapshot_extents) {
            ldout(cct,
                  20) << "off=" << snapshot_extent.
                      get_off() << ", " << "len=" << snapshot_extent.
                      get_len() << ", " << "state=" << snapshot_extent.
                      get_val().state << dendl;
            diffs->emplace_back(snapshot_extent.get_off(),
                                snapshot_extent.get_len(),
                                snapshot_extent.get_val().state ==
                                    ...
                                    void *arg)
{
    ...
    C_SaferCond flush_ctx;
    {
        std::shared_lock owner_locker {
            ictx->owner_lock};
        auto aio_comp =
            io::AioCompletion::create_and_start(&flush_ctx, ictx,
                                                io::AIO_TYPE_FLUSH);
        auto req =
            io::ImageDispatchSpec::create_flush(*ictx,
                                                io::
                                                IMAGE_DISPATCH_LAYER_INTERNAL_START,
                                                aio_comp,
                                                io::
...
template < typename I > int DiffIterate < I >::execute()
{
    ...
    if (m_whole_object) {
        C_SaferCond ctx;
        auto req =
            object_map::DiffRequest < I >::create(&m_image_ctx,
                from_snap_id,
                end_snap_id,
                &object_diff_state,
                &ctx);
        ...
        if (r < 0) {
            ...
            if (m_include_parent && from_snap_id == 0) {
                ...
                m_image_ctx.get_parent_overlap(m_image_ctx.snap_id,
                                               &raw_overlap);
                auto overlap =
                    m_image_ctx.reduce_parent_overlap(raw_overlap,
                                                      false);
                if (overlap.first > 0
    ...
    DiffContext diff_context(m_image_ctx, m_callback, m_callback_arg,
                             m_whole_object, m_include_parent,
    ...
    while (left > 0) {
        ...
        if (fast_diff_enabled) {
            ...
            Striper::file_to_extents(cct, m_image_ctx.format_string,
                                     &m_image_ctx.layout, off, read_len,
                                     0, object_extents, 0);
                ...
                if (diff_state == object_map::DIFF_STATE_HOLE &&
                    ...
                    for (auto &oe : extents) {
                        for (auto &be : oe.buffer_extents) {
                            ...
                            ldout(cct,
                                  20) << " reporting parent overlap " <<
                                      o << dendl;
                            for (auto e = o.begin(); e != o.end(); ++e) {
                                aggregate_sparse_extents.insert(e.
                                                                get_start
                                                                (),
                                                                e.
                                                                get_len
                    ...
                    auto state =
                        (diff_state ==
                         object_map::DIFF_STATE_HOLE_UPDATED ? io::
                         SPARSE_EXTENT_STATE_ZEROED : io::
                         SPARSE_EXTENT_STATE_DATA);
                    for (auto &oe : extents) {
                        for (auto &be : oe.buffer_extents) {
                            aggregate_sparse_extents.insert(off +
                                                            be.first,
                            be.second, {
                                state,
                                be.second
            ...
            auto diff_object =
                new C_DiffObject < I > (m_image_ctx, diff_context, off,
                                        read_len);
            diff_object->send();
