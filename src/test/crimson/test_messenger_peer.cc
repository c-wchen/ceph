...
class FailoverSuitePeer: public Dispatcher
{
    ...
    void ms_handle_fast_accept(Connection *conn) override
    {
        if (tracked_conn == nullptr) {
            ...
            ldout(cct,
                  0) << "[!TestPeer] accepted(stale event): " << conn <<
                     dendl;
        }
        flush_pending_send();
    }
    bool ms_handle_reset(Connection *conn) override
    {
        if (tracked_conn == conn) {
            ...
            ldout(cct,
                  0) << "[!TestPeer] reset(invalid event): conn(" << conn <<
                     ") != tracked_conn(" << tracked_conn << ")" << dendl;
        }
        return true;
    }
...
private:
    ...
    {
        peer_msgr.reset(Messenger::create(cct, "async",
                                          entity_name_t::OSD(TEST_PEER_OSD),
                                          "TestPeer", TEST_PEER_NONCE));
    ...
    void send_op()
    {
        ...
        spg_t spgid(pgid);
        tracked_conn->send_message2(make_message < MOSDOp >
...
public:
    ...
    {
        ldout(cct,
              0) << "[TestPeer] connect_peer(" << test_addr << ")" << dendl;
        auto conn =
            peer_msgr->connect_to_osd(entity_addrvec_t { test_addr });
        if (tracked_conn) {
            if (tracked_conn == conn.get()) {
                ldout(cct,
                      0) << "[TestPeer] this is not a new session " << conn.
                         get() << dendl;
            } else {
                ldout(cct,
                      0) << "[TestPeer] this is a new session " << conn.
                         get()
    ...
    void send_peer()
    {
        if (tracked_conn) {
            ...
            ++pending_send;
            ldout(cct,
                  0) << "[TestPeer] send_peer() (pending " << pending_send
                     << ")" << dendl;
        }
    }
    ...
    {
        auto suite =
            std::make_unique < FailoverSuitePeer > (cct, op_callback);
...
class FailoverTestPeer: public Dispatcher
{
    ...
    } bool ms_dispatch(Message *m) override
    {
        ...
        switch (m->get_type()) {
            case CEPH_MSG_PING: {
                ldout(cct,
                      0) << "[CmdSrv] got PING, sending PONG ..." << dendl;
    ...
    void ms_handle_fast_connect(Connection *) override
    {
        ceph_abort();
    }
    void ms_handle_fast_accept(Connection *conn) override
    {
        if (cmd_conn == nullptr) {
            ...
            ldout(cct,
                  0) << "[!CmdSrv] accepted(stale event): " << conn <<
                     dendl;
        }
    }
    bool ms_handle_reset(Connection *conn) override
...
private:
    ...
    void handle_cmd(cmd_t cmd, MRef < MCommand > m_cmd)
    {
        switch (cmd) {
            case cmd_t::suite_start: {
                ...
                auto suite =
                    FailoverSuitePeer::create(cct, test_peer_addr, policy,
                                              [this] { notify_recv_op();
                                                     }
                                             );
    ...
    {
        cmd_msgr.reset(Messenger::create(cct, "async",
...
public:
    FailoverTestPeer(CephContext *cct,
        ...
        :
        Dispatcher(cct),
        dummy_auth(cct), test_peer_addr(test_peer_addr), nonstop(nonstop)
    {
    }
    ...
    static std::unique_ptr < FailoverTestPeer > create(CephContext *cct,
            entity_addr_t cmd_peer_addr,
            entity_addr_t test_peer_addr, bool nonstop)
    {
        auto test_peer =
            std::make_unique < FailoverTestPeer > (cct, test_peer_addr,
                nonstop);
...
int main(int argc, char **argv)
{
    ...
    auto test_peer = FailoverTestPeer::create(cct.get(),
                     cmd_peer_addr,
                     test_peer_addr,
                     nonstop);
    test_peer->wait();
