...
class C_ReopenComplete:public MDSInternalContext {
  ...
  public:
    C_ReopenComplete(MDLog * mdlog_,
                     ...
                     on_complete_):MDSInternalContext(mdlog_->mds),
...
void MDLog::_expired(LogSegment * ls)
{
    ...
    if (!capped && ls == peek_current_segment()) {
        ...
        // Trigger all waiters
        for (std::list < MDSInternalContextBase * >::iterator i =
             ls->expiry_waiters.begin(); i != ls->expiry_waiters.end(); ++i) {
            (*i)->complete(0);
        }
...
void MDLog::_recovery_thread(MDSInternalContextBase * completion)
{
    ...
    if (read_result == -ENOENT) {
        ...
        derr << "Blacklisted during JournalPointer read!  Respawning..." <<
            dendl;
        mds->respawn();
    ...
    if (jp.back) {
        ...
        if (recovery_result == -EBLACKLISTED) {
            derr << "Blacklisted during journal recovery!  Respawning..." <<
                dendl;
            mds->respawn();
            ceph_abort();       // Should be unreachable because respawn calls execv
        }
            ...
            if (r == -ENOENT) {
                ...
                if (journaler->get_read_pos() < journaler->get_expire_pos()) {
                    ...
                    if (journaler->is_readonly()) {
                        dout(0) <<
                            "expire_pos is higher than read_pos, returning EAGAIN"
                            << dendl;
                        r = -EAGAIN;
                    }
                    else {
