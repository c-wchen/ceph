        ...
        namespace lock {
            ...
            void lock(ObjectWriteOperation * rados_op,
                ...
                ObjectWriteOperation op;
                 lock(&op, name, type, cookie, tag, description, duration,
                      flags);
            ...
            int list_locks(IoCtx * ioctx, const string & oid,
                           list < string > *locks) {
                bufferlist in, out;
            ...
            int get_lock_info(IoCtx * ioctx, const string & oid,
                              const string & name, map < locker_id_t,
                              locker_info_t > *lockers, ClsLockType * type,
                              string * tag) {
            ...
            int Lock::lock_exclusive_ephemeral(IoCtx * ioctx,
                                               const string & oid) {
                return lock(ioctx, oid, name, LOCK_EXCLUSIVE_EPHEMERAL, cookie,
                            tag, description, duration, flags);
            }
            ...
            void Lock::break_lock(ObjectWriteOperation * op,
                                  const entity_name_t & locker) {
                rados::cls::lock::break_lock(op, name, cookie, locker);
            }

            int Lock::break_lock(IoCtx * ioctx, const string & oid,
                                 const entity_name_t & locker) {
                return rados::cls::lock::break_lock(ioctx, oid, name, cookie,
                                                    locker);
            }
        }                       // namespace lock
    }                           // namespace cls
