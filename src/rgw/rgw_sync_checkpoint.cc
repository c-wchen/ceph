...
{
    return BucketIndexShardsManager::get_shard_marker(info.inc_marker.
            position);
}
...
std::ostream &operator<<(std::ostream &out,
                         const std::vector < rgw_bucket_shard_sync_info >
                         &rhs)
{
                                  ...
                                  ceph::coarse_mono_time timeout_at)
{
    ...
    int r =
        rgw_read_bucket_full_sync_status(dpp, store, pipe, &full_status,
                                         null_yield);
    ...
    while (full_status.state != BucketSyncState::Incremental) {
        ...
        ldout(store->ctx(),
              1) << "waiting to reach incremental sync.." << dendl;
        std::this_thread::sleep_until(delay_until);

        r = rgw_read_bucket_full_sync_status(dpp, store, pipe, &full_status,
                                             null_yield);
        if (r < 0 && r != -ENOENT) {    // retry on ENOENT
            return r;
        }
    ...
    while (full_status.incremental_gen < latest_gen) {
        ...
        ldout(store->ctx(),
              1) << "waiting to reach latest gen " << latest_gen << ", on "
                 << full_status.incremental_gen << ".." << dendl;
        std::this_thread::sleep_until(delay_until);
    ...
    while (status < remote_markers) {
        ...
        if (delay_until > timeout_at) {
            ldpp_dout(dpp,
                      0) <<
                         "bucket checkpoint timed out waiting for incremental sync to catch up"
                         << dendl;
            return -ETIMEDOUT;
        }
                      ...
                      optional_yield y)
{
    ...
    return rgw_read_remote_bilog_info(dpp, conn->second,
                                      *pipe.source.bucket, info, markers,
                                      y);
}
                               ...
                               ceph::coarse_mono_time timeout_at)
{
    ...
    for (const auto &[source_zone_id, pipe] : policy.get_all_sources()) {
        ...
        spawn::spawn(ioctx, [&](yield_context yield) {
            auto y = optional_yield {
                ioctx, yield};
            rgw_bucket_index_marker_info info;
            int r =
                source_bilog_info(dpp, store->svc()->zone, entry.pipe,
        ...
        spawn::spawn(ioctx, [&](yield_context yield) {
            auto y = optional_yield {
                ioctx, yield};
            int r =
                store->getRados()->get_bucket_instance_info(*entry.pipe.
                    source.bucket,
                    entry.
                    source_bucket_info,
                    nullptr,
    ...
    for (const auto &e : sources) {
        int r =
            bucket_source_sync_checkpoint(dpp, store, info,
                                          e.source_bucket_info,
                                          e.pipe, e.latest_gen,
                                          e.remote_markers,
