...
namespace {
    ...
    class Pauser {
        ...
        0};
      public:
         template < typename Func > void pause(int thread_count, Func && func);
        void unpause();
        void wait();
    };
    ...
    template < typename Func >
        ...
        lock.lock();
        cond_paused.wait(lock,[ =] {
                         return waiters == thread_count;
                         }
        );
    }
  ...
  template < typename Stream > class StreamIO:public rgw::asio::ClientIO {
      ...
      :    
        ...
        size_t write_data(const char *buf, size_t len) override {
            ...
            auto bytes =
                boost::asio::write(stream, boost::asio::buffer(buf, len), ec);
            if (ec) {
    ...
    template < typename Stream >
        ...
        for (;;) {
            ...
            // process the request
            RGWRequest req {
            env.store->get_new_req_id()};

            auto & socket = stream.lowest_layer();
            StreamIO < Stream > real_client {
            stream, parser, buffer, is_ssl,
    ...
    int AsioFrontend::init_ssl() {
        ...
        if (have_private_key) {
            if (!have_cert) {
                lderr(ctx()) <<
                    "no ssl_certificate configured for ssl_private_key" <<
                    dendl;
                return -EINVAL;
            }
            ssl_context->use_private_key_file(key->second, ssl::context::pem,
                                              ec);
            if (ec) {
        ...
        if (have_cert) {
            ...
            if (!have_private_key) {
                ...
                ssl_context->use_private_key_file(cert->second,
                                                  ssl::context::pem, ec);
                if (ec) {
                    lderr(ctx()) << "failed to use ssl_certificate=" << cert->
                        second << " as a private key: " << ec.
                        message() << dendl;
                    return -ec.value();
                }
            }
...
#ifdef WITH_RADOSGW_BEAST_OPENSSL
        if (l.use_ssl) {
            boost::asio::spawn(service, std::bind([this]
                                                  (boost::asio::
                                                   yield_context yield,
                                                   tcp::socket & s) mutable {
...
class RGWAsioFrontend::Impl:public AsioFrontend {
  public:
    Impl(const RGWProcessEnv & env, RGWFrontendConfig * conf):AsioFrontend(env,
                                                                           conf)
    {
}};
