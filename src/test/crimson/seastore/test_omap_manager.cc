...
struct omap_manager_test_t:public seastar_test_suite_t, TMTestState {
    ...
    seastar::future <> tear_down_fut()final {
        return tm_teardown().then([this] {
                                  omap_manager.reset(); return seastar::now();}
        );
    }
    ...
    void set_key(omap_root_t & omap_root,
                 Transaction & t, const string & key, const bufferlist & val) {
        with_trans_intr(t,[&, this] (auto & t) {
                        return omap_manager->omap_set_key(omap_root, t, key,
                                                          val);}
        ).unsafe_get0();
    ...
    void set_key(omap_root_t & omap_root,
                 Transaction & t, const string & key, const string & val) {
        bufferlist bl;
        bl.append(val);
    ...
    void get_value(omap_root_t & omap_root, Transaction & t, const string & key) {
        auto ret = with_trans_intr(t,
                                   [&, this] (auto & t) {
                                   return omap_manager->
                                   omap_get_value(omap_root, t, key);}
        ).unsafe_get0();
    ...
    std::vector < std::string > rm_key_range(omap_root_t & omap_root,
                                             Transaction & t,
        ...
        with_trans_intr(t,[&, this] (auto & t) {
                        return omap_manager->omap_rm_key_range(omap_root, t,
                                                               first, last,
                                                               config);}
        ).unsafe_get0();
    ...
    void list(const omap_root_t & omap_root,
              Transaction & t,
              const std::optional < std::string > &first,
              const std::optional < std::string > &last,
              size_t max = 128, bool inclusive = false) {
    ...
    auto initialize() {
        ...
        omap_root_t omap_root = with_trans_intr(*t,
                                                [this] (auto & t) {
                                                return omap_manager->
                                                initialize_omap(t, L_ADDR_MIN);}
        ).unsafe_get0();
...
TEST_F(omap_manager_test_t, basic)
{
    run_async([this] {
              ...
              string key = "owner"; string val = "test"; {
              auto t = create_mutate_transaction();
              logger().debug("first transaction");
              set_key(omap_root, *t, key, val);
              get_value(omap_root, *t, key); submit_transaction(std::move(t));}
              {
              auto t = create_mutate_transaction();
...
TEST_F(omap_manager_test_t, force_leafnode_split)
{
    run_async([this] {
              ...
              set_random_key(omap_root, *t); if ((i % 20 == 0) && (j == 5)) {
              check_mappings(omap_root, *t);}
              }
              logger().debug("force split submit transaction i = {}", i);
              submit_transaction(std::move(t)); check_mappings(omap_root);}
              }
    ) ;
}
...
TEST_F(omap_manager_test_t, force_leafnode_split_merge)
{
    run_async([this] {
              ...
              set_random_key(omap_root, *t); if ((i % 10 == 0) && (j == 3)) {
              check_mappings(omap_root, *t);}
              }
              logger().debug("submitting transaction");
              submit_transaction(std::move(t)); if (i % 50 == 0) {
              ...
              check_mappings(omap_root, *t); check_mappings(omap_root);}
              }
...
TEST_F(omap_manager_test_t, force_split_listkeys_list_rmkey_range_clear)
{
    run_async([this] {
              omap_root_t omap_root = initialize();
              string first, last; for (unsigned i = 0; i < 40; i++) {
              auto t = create_mutate_transaction();
              logger().debug("opened transaction");
              for (unsigned j = 0; j < 10; ++j) {
              auto key = set_random_key(omap_root, *t); if (i == 10) {
              first = key;}
              if (i == 30) {
              last = key; if (first > last) {
              std::swap(first, last);}
              }
              if ((i % 20 == 0) && (j == 5)) {
              check_mappings(omap_root, *t);}
              ...
              {
          auto t = create_mutate_transaction(); auto keys = rm_key_range(omap_root, *t, first, last); for (const auto & key:keys)
              {
              get_value(omap_root, *t, key);}
              submit_transaction(std::move(t));}
              ...
              {
              auto t = create_mutate_transaction();
              clear(omap_root, *t); submit_transaction(std::move(t));}
              }) ;
}
...
TEST_F(omap_manager_test_t, force_inner_node_split_list_rmkey_range)
{
    run_async([this] {
              omap_root_t omap_root = initialize();
              string first = "";
              string last; while (cache->get_omap_tree_depth() < 3) {
              for (unsigned i = 0; i < 40; i++) {
              auto t = create_mutate_transaction();
              logger().debug("opened transaction");
              for (unsigned j = 0; j < 10; ++j) {
...
TEST_F(omap_manager_test_t, internal_force_split)
{
    run_async([this] {
              ...
              set_random_key(omap_root, *t); if ((i % 2 == 0) && (j % 50 == 0)) {
              check_mappings(omap_root, *t);}
              }
              logger().debug("submitting transaction i = {}", i);
              submit_transaction(std::move(t));}
...
TEST_F(omap_manager_test_t, internal_force_merge_fullandbalanced)
{
    run_async([this] {
              ...
              auto t = create_mutate_transaction();
              for (unsigned i = 0; i < mkeys.size(); i++) {
              rm_key(omap_root, *t, mkeys[i]); if (i % 10 == 0) {
              logger().debug("submitting transaction i= {}", i);
              submit_transaction(std::move(t));
              t = create_mutate_transaction();}
              if (i % 50 == 0) {
...
TEST_F(omap_manager_test_t, replay)
{
    run_async([this] {
              ...
              logger().debug("submitting transaction i= {}", i);
              submit_transaction(std::move(t));
              replay(); t = create_mutate_transaction();}
              if (i % 50 == 0) {
              logger().debug("check_mappings  i= {}", i);
              check_mappings(omap_root, *t); check_mappings(omap_root);}
              }
              logger().debug("finally submitting transaction ");
              submit_transaction(std::move(t));
              replay(); check_mappings(omap_root);}
    ) ;
}
