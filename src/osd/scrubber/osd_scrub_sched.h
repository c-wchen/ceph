...
public:
     ...
     * reason.
     */
    virtual schedule_result_t initiate_a_scrub(spg_t pgid,
            bool
...
public:
    ...
    struct sched_params_t {
        ...
        double max_interval {
            0.0};
        must_scrub_t is_must {
            ScrubQueue::must_scrub_t::not_mandatory};
    };
    ...
    struct ScrubJob final: public RefCountedObject {
        ...
        ceph::atomic < qu_state_t > state {
            qu_state_t::not_registered};
        ...
        {
            return ScrubQueue::qu_state_text(state.
         ...
         */
        std::string scheduling_state(utime_t now_is,
     ...
     */
    Scrub::schedule_result_t select_pg_and_scrub(Scrub::
...
private:
     ...
     * scrub jobs.
     */
    ScrubQContainer collect_ripe_jobs(ScrubQContainer &group,
                                      utime_t time_now);
     ...
     */
    std::atomic_int_fast16_t blocked_scrubs_cnt {
        0};

    std::atomic_bool a_pg_is_reserving {
        false};
     ...
     */
    scrub_schedule_t adjust_target_time(const sched_params_t &
                                        recomputed_params) const;
...
template <>
struct fmt::formatter < ScrubQueue::qu_state_t >: fmt::formatter <
    std::string_view > {
    template < typename FormatContext >
    auto format(const ScrubQueue::qu_state_t &s, FormatContext &ctx)
    {
        auto out = ctx.out();
        out = fmt::formatter < string_view >::format(std::string {
            ScrubQueue::
            qu_state_text(s)}, ctx);
        return out;
    }
