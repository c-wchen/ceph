...
namespace librbd {
    ...
    template < typename I >
        int ObjectMap < I >::aio_remove(librados::IoCtx & io_ctx,
    ...
    template < typename I >
        ...
        I >::object_map_name(const std::string & image_id, uint64_t snap_id) {
    ...
    template < typename I >
        bool ObjectMap < I >::is_compatible(const file_layout_t & layout,
                                            uint64_t size) {
    ...
    template < typename I >
        ceph::BitVector < 2u >::Reference ObjectMap <
        I >::operator[](uint64_t object_no) {
    ...
    template < typename I >
        ...
        ldout(m_image_ctx.cct,
              20) << "object_no=" << object_no << " r=" << exists << dendl;
        return exists;
    }

    template < typename I >
        bool ObjectMap < I >::update_required(const ceph::BitVector <
                                              2 >::Iterator & it,
                                              uint8_t new_state) {
        assert(m_image_ctx.object_map_lock.is_wlocked());
        uint8_t state = *it;
    ...
    template < typename I > void ObjectMap < I >::open(Context * on_finish) {
        auto req =
            object_map::RefreshRequest < I >::create(m_image_ctx, &m_object_map,
                                                     m_snap_id, on_finish);
        req->send();
    }
    ...
    template < typename I > void ObjectMap < I >::close(Context * on_finish) {
        ...
        auto req =
            object_map::UnlockRequest < I >::create(m_image_ctx, on_finish);
        req->send();
    }

    template < typename I >
        bool ObjectMap < I >::set_object_map(ceph::BitVector < 2 >
                                             &target_object_map) {
    ...
    template < typename I >
        ...
        object_map::SnapshotRollbackRequest * req =
            new object_map::SnapshotRollbackRequest(m_image_ctx, snap_id,
                                                    on_finish);
        req->send();
    }

    template < typename I >
        void ObjectMap < I >::snapshot_add(uint64_t snap_id,
                                           Context * on_finish) {
        assert(m_image_ctx.snap_lock.is_locked());
        ...
        object_map::SnapshotCreateRequest * req =
            new object_map::SnapshotCreateRequest(m_image_ctx, &m_object_map,
                                                  snap_id, on_finish);
        req->send();
    }

    template < typename I >
        void ObjectMap < I >::snapshot_remove(uint64_t snap_id,
                                              Context * on_finish) {
        ...
        object_map::SnapshotRemoveRequest * req =
            new object_map::SnapshotRemoveRequest(m_image_ctx, &m_object_map,
                                                  snap_id, on_finish);
        req->send();
    }
    ...
    template < typename I >
        void ObjectMap < I >::aio_resize(uint64_t new_size,
        ...
        object_map::ResizeRequest * req =
            new object_map::ResizeRequest(m_image_ctx, &m_object_map, m_snap_id,
                                          new_size, default_object_state,
                                          on_finish);
        req->send();
    }
    ...
    template < typename I >
        ...
        int r = m_update_guard->detain({ op.start_object_no, op.end_object_no },
                                       &op, &cell);
        if (r < 0) {
            lderr(cct) << "failed to detain object map update: " <<
                cpp_strerror(r)
                << dendl;
            ...
            ldout(cct,
                ...
                << "start=" << op.start_object_no << ", " << "end=" << op.
                end_object_no << ", " << (op.
                                          current_state ? stringify(static_cast
                                                                    < uint32_t >
                                                                    (*op.
                                                                     current_state))
                                          : "")
        ...
        Context *ctx = new FunctionContext([this, cell, on_finish] (int r){
                                           handle_detained_aio_update(cell, r,
                                                                      on_finish);});
        aio_update(CEPH_NOSNAP, op.start_object_no, op.end_object_no,
                   op.new_state, op.current_state, op.parent_trace,
                   op.ignore_enoent, ctx);
    }

    template < typename I >
        void ObjectMap < I >::handle_detained_aio_update(BlockGuardCell * cell,
                                                         int r,
                                                         Context * on_finish) {
        CephContext *cct = m_image_ctx.cct;
    ...
    template < typename I >
        void ObjectMap < I >::aio_update(uint64_t snap_id,
        ...
        auto req =
            object_map::UpdateRequest < I >::create(m_image_ctx, &m_object_map,
                                                    snap_id, start_object_no,
                                                    end_object_no,
