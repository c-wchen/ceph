...
//  static void generate_test_instances(list<ACLOwner*>& o);
    void set_days(const string & _days) {
        days = _days;
    }
...
class LCFilter {
  ...
  public:
    const std::string & get_prefix() const const
{
    return prefix;
...
struct lc_op {
    ...
    int mp_expiration;
     ...
     lc_op():status(false), dm_expiration(false), expiration(0),
        noncur_expiration(0), mp_expiration(0)
{
}};
...
//  static void generate_test_instances(list<RGWAccessControlList*>& o);
    ...
    multimap < string, LCRule > &get_rule_map() {
        return rule_map;
    }
    map < string, lc_op > &get_prefix_map() {
        return prefix_map;
    }
/*
...
class LCWorker:public Thread {
  ...
  public:
     LCWorker(CephContext * _cct, RGWLC * _lc):cct(_cct), lc(_lc),
        lock("LCWorker") {
    } void *entry() override;
    void stop();
...
int list_lc_progress(const string & marker, uint32_t max_entries, map < string,
                     int >*progress_map);
int bucket_lc_prepare(int index);
int bucket_lc_process(string & shard_id);
int bucket_lc_post(int index, int max_lock_sec, pair < string, int >&entry,
                   int &result);
bool going_down();
...
int handle_multipart_expiration(RGWRados::Bucket * target, const map < string,
                                lc_op > &prefix_map);
};
