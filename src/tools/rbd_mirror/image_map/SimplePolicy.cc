...
calc_images_per_instance(const InstanceToImageMap &map,
                         size_t image_count)
{
...
void SimplePolicy::
do_shuffle_add_instances(const InstanceToImageMap &map,
                         size_t image_count,
                         std::set < std::string >
                         *remap_global_image_ids)
{
    uint64_t images_per_instance =
        calc_images_per_instance(map, image_count);
    ...
    for (auto const &instance : map) {
        ...
        uint64_t cut_off =
            instance.second.size() - images_per_instance;
                                 ...
                                 string &global_image_id)
{
    ...
    for (auto it = map.begin(); it != map.end(); ++it) {
        ceph_assert(it->second.find(global_image_id) ==
                    it->second.end());
        if (Policy::is_dead_instance(it->first)) {
            continue;
