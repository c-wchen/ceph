...
struct rgw_sync_group_pipe_map {
    ...
    std::vector < rgw_sync_bucket_pipe >
        find_source_pipes(const rgw_zone_id & source_zone,
     ...
     */
    std::vector < rgw_sync_bucket_pipe >
        find_pipes(const rgw_zone_id & source_zone,
                   std::optional < rgw_bucket > source_bucket,
                   const rgw_zone_id & dest_zone,
                   std::optional < rgw_bucket > dest_bucket) const;
};
    ...
    struct pipe_handler:public endpoints_pair {
        pipe_rules_ref rules;
         ...
         pipe_handler() {
                       ...
                       _pipe):endpoints_pair(_pipe), rules(_rules) {
        }
        bool specific() const {
...
class RGWBucketSyncPolicyHandler {
  ...
  public:
    ...
    std::multimap < rgw_zone_id,
        ...
        get_all_dests_in_zone(const rgw_zone_id & zone_id) const;
