...
public:
    ...
    PGMap::PGCreationBlockingEvent, CompletionEvent > tracking_events;
    ...
    public:
        ...
        PGPipeline::AwaitMap::BlockingEvent,
                   ...
                   CompletionEvent > pg_tracking_events;

        template < typename BlockingEventT, typename InterruptorT =
                   void, typename F > auto with_blocking_event(F
                           && f,
                           ClientRequest &op)
        {
            auto ret =
                std::forward < F >
                (f)(typename BlockingEventT::template Trigger <
            ClientRequest > {
            ...
            if constexpr(std::is_same_v < InterruptorT, void >) {
                ...
                return typename InterruptorT::template futurize_t < ret_t > {
                    std::move(ret)
                };
            }
        }
        ...
        auto enter_stage(StageT &stage, ClientRequest &op)
        {
            ...
            InterruptorT > ([&stage, this](auto && trigger) {
                return handle.template enter <
                       ClientRequest > (stage,
                                        std::move(trigger));
            }
            , op);
        ...
        template <
            ...
            void, typename BlockingObj, typename Method,
            typename ... Args > auto enter_blocker(ClientRequest &op,
                    BlockingObj &obj,
                    Method method, Args
                    && ... args)
        {
            return this->template with_blocking_event <
                       ...
                       InterruptorT >
            ([&obj, method, args =
                  std::forward_as_tuple(std::move(args) ...)](auto
                      && trigger)
            mutable {
                return apply_method_to_tuple(obj, method,
                                             std::tuple_cat(std::
                                                 forward_as_tuple
                                                 (std::
                                                  move
    ...
    ClientRequest(ShardServices &shard_services,
    ...
    seastar::future <> with_pg_int(ShardServices &shard_services,
                                   Ref < PG > pg);
...
private:
    ...
    interruptible_future <> do_process(instance_handle_t &ihref,
                                       Ref < PG > &pg,
                                       crimson::osd::ObjectContextRef obc);
    ::crimson::interruptible::interruptible_future <::crimson::osd::
    IOInterruptCondition > process_pg_op(Ref < PG > &pg);
    ::crimson::interruptible::interruptible_future <::crimson::osd::
    IOInterruptCondition > process_op(instance_handle_t &ihref,
                                      Ref < PG > &pg);
    bool is_pg_op() const;
    ...
    const SnapContext get_snapc(Ref < PG > &pg,
...
template <> struct fmt::formatter <
    crimson::osd::ClientRequest > : fmt::ostream_formatter {
};
#endif
