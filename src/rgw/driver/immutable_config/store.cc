...
// Realm
        int ImmutableConfigStore::
        write_default_realm_id(const DoutPrefixProvider * dpp,
                               optional_yield y, bool exclusive,
                               std::string_view realm_id) {
        return -EROFS;
    }

    int ImmutableConfigStore::read_default_realm_id(const DoutPrefixProvider *
                                                    dpp, optional_yield y,
                                                    std::string & realm_id) {
        return -ENOENT;
    }

    int ImmutableConfigStore::delete_default_realm_id(const DoutPrefixProvider *
                                                      dpp, optional_yield y) {
        return -EROFS;
    }
    ...
    int ImmutableConfigStore::realm_notify_new_period(const DoutPrefixProvider *
                                                      dpp, optional_yield y,
...
// ZoneGroup
    ...
    int ImmutableConfigStore::
        write_default_zonegroup_id(const DoutPrefixProvider * dpp,
                                   optional_yield y, bool exclusive,
                                   std::string_view realm_id,
    ...
    int ImmutableConfigStore::
        read_default_zonegroup_id(const DoutPrefixProvider * dpp,
                                  optional_yield y, std::string_view realm_id,
    ...
    int ImmutableConfigStore::create_zonegroup(const DoutPrefixProvider * dpp,
                                               ...
                                               std::unique_ptr <
                                               ZoneGroupWriter > *writer) {
        return -EROFS;
    }

    int ImmutableConfigStore::read_zonegroup_by_id(const DoutPrefixProvider *
                                                   dpp, optional_yield y,
                                                   std::
    ...
    int ImmutableConfigStore::read_zonegroup_by_name(const DoutPrefixProvider *
                                                     dpp, optional_yield y,
                                                     std::
    ...
    int ImmutableConfigStore::read_default_zonegroup(const DoutPrefixProvider *
                                                     dpp, optional_yield y,
                                                     std::string_view realm_id,
                                                     RGWZoneGroup & info,
                                                     std::unique_ptr <
    ...
    int ImmutableConfigStore::list_zonegroup_names(const DoutPrefixProvider *
                                                   dpp, optional_yield y,
                                                   const std::string & marker,
                                                   std::span < std::string >
                                                   entries,
...
// Zone
    ...
    int ImmutableConfigStore::write_default_zone_id(const DoutPrefixProvider *
                                                    dpp, optional_yield y,
                                                    bool exclusive,
                                                    std::string_view realm_id,
    ...
    int ImmutableConfigStore::read_default_zone_id(const DoutPrefixProvider *
                                                   dpp, optional_yield y,
                                                   std::string_view realm_id,
                                                   std::string & zone_id) {
        if (realm_id.empty()) {
    ...
    int ImmutableConfigStore::delete_default_zone_id(const DoutPrefixProvider *
                                                     dpp, optional_yield y,
                                                     std::
                                                     string_view realm_id) {
        return -EROFS;
    }
...
// PeriodConfig
    ...
    int ImmutableConfigStore::write_period_config(const DoutPrefixProvider *
                                                  dpp, optional_yield y,
                                                  bool exclusive,
                                                  std::string_view realm_id,
...
/// ImmutableConfigStore factory function
    ...
    ->std::unique_ptr < ConfigStore > {
        return std::make_unique < ImmutableConfigStore > (zonegroup, zone,
                                                          period_config);
    }
