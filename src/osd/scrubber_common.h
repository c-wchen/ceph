...
struct PgScrubBeListener {
    ...
    virtual uint64_t logical_to_ondisk_size(uint64_t logical_size) const =
        0;
...
template <> struct fmt::formatter < requested_scrub_t > {
    ...
    auto format(const requested_scrub_t &rs, FormatContext &ctx)
    {
        return fmt::format_to(ctx.out(),
                              ...
                              rs.
                              deep_scrub_on_error ? " deep_scrub_on_error" : "",
...
struct ScrubPgIF {
    ...
    virtual void on_applied_when_primary(const eversion_t &applied_version) =
        0;
    ...
    virtual unsigned int scrub_requeue_priority(Scrub::
     ...
     */
    virtual void update_scrub_stats(ceph::coarse_real_clock::
                                    time_point now_is) = 0;
    ...
    // and on the primary:
    virtual void handle_scrub_reserve_grant(OpRequestRef op, pg_shard_t from) =
        0;
    virtual void handle_scrub_reserve_reject(OpRequestRef op, pg_shard_t from) =
        0;
    ...
    virtual int asok_debug(std::string_view cmd,
                           std::string param,
                           Formatter *f, std::stringstream &ss) = 0;
};
