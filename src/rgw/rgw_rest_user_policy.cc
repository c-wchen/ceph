...
void RGWPutUserPolicy::execute(optional_yield y)
{
    ...
    std::unique_ptr < rgw::sal::User > user =
        driver->get_user(rgw_user(user_name));
    ...
    try {
        ...
        const unsigned int max_num =
            ...
            0 ? USER_POLICIES_MAX_NUM : s->cct->_conf->
        ...
        if (policies.size() > max_num) {
            ...
            s->err.message =
                ...
                "of " + std::to_string(max_num) + " policies.";
            return;
        }
...
void RGWGetUserPolicy::execute(optional_yield y)
{
    ...
    if (op_ret == -ENOENT) {
        ldpp_dout(this,
                  0) << "ERROR: attrs not found for user" << user_name << dendl;
        op_ret = -ERR_NO_SUCH_ENTITY;
        return;
    }
