...
int RGWOrphanStore::init(const DoutPrefixProvider *dpp)
{
    ...
    int r = rgw_init_ioctx(dpp, static_cast < rgw::sal::RadosStore* > (store)->getRados()->get_rados_handle(), log_pool,
                           ioctx);
    if (r < 0) {
...
int RGWOrphanStore::store_entries(const DoutPrefixProvider *dpp, const string& oid, const map < string,
                                  bufferlist > & entries)
{
...
int RGWOrphanSearch::build_all_oids_index(const DoutPrefixProvider *dpp)
{
    ...
    int ret = rgw_init_ioctx(dpp, static_cast < rgw::sal::RadosStore* > (store)->getRados()->get_rados_handle(),
        ...
        map < int, list < string> > & oids)
{
    ...
    do {
        ...
        for (vector < rgw_bucket_dir_entry >::iterator iter = result.begin(); iter != result.end(); ++iter) {
            ...
            ldpp_dout(dpp, 20) << __func__ << ": entry.key.name=" << entry.key.name << " entry.key.instance=" << entry.key.instance
                               << dendl;
