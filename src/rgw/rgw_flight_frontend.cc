...
FlightFrontend::FlightFrontend(RGWProcessEnv &_env,
                               ...
                               int _port): env(_env),
    config(_config),
    port(_port), dp(env.driver->ctx(), dout_subsys, dout_prefix_str)
{
...
FlightGetObj_Filter::FlightGetObj_Filter(const req_state *request,
        RGWGetObj_Filter *
        next): RGWGetObj_Filter(next),
    penv(request->penv), dp(request->cct->get(), dout_subsys,
                            dout_prefix_str), current_offset(0),
    expected_size(request->obj_size), uri(request->decoded_uri),
    ...
    if (temp_file.is_open()) {
        ...
        if (current_offset >= expected_size) {
            INFO << "data read is completed, current_offset=" <<
                 current_offset << ", expected_size=" << expected_size <<
                 dendl;
            temp_file.close();
            ...
            auto process_metadata =
                [&aw_schema, &num_rows, &kv_metadata,
            this]()->arrow::Status {
                ARROW_ASSIGN_OR_RAISE(std::shared_ptr <
                                      arrow::io::ReadableFile > file,
                ...
                metadata->schema();
                ARROW_RETURN_NOT_OK(parquet::arrow::
                                    FromParquetSchema(pq_schema,
                                                      &aw_schema));
            ...
            if (!schema_status.ok()) {
                ERROR << "reading metadata to access schema, error=" <<
                      schema_status << dendl;
            } else {
                ...
                auto key =
                    store->
                    add_flight(FlightData(uri, tenant_name, bucket_name,
                                          object_key, num_rows,
...
void code_snippets()
{
    ...
    INFO << "file schema: name=" << schema1->
         name() << ", ToString:" << schema1->
         ToString() << ", num_columns=" << schema1->num_columns() << dendl;
    for (int c = 0; c < schema1->num_columns(); ++c) {
        ...
        const std::shared_ptr < const parquet::LogicalType > lt =
            cd->logical_type();
        INFO << "column " << c << ": name=" << cd->
             name() << ", ToString=" << cd->
             ToString() << ", logical_type=" << lt->ToString() << dendl;
    }

    INFO << "There are " << md->num_rows() << " rows and " << md->
