                     ...
                     const string & name, lock_info_t * lock)
{
    ...
    if (lock->lockers.empty() && cls_lock_is_ephemeral(lock->lock_type)) {
        r = clean_lock(hctx);
        if (r < 0) {
            CLS_ERR("error, on read, cleaning lock object %s",
                    ...
                    uint8_t flags, const string & cookie, const string & tag)
{
    ...
    CLS_LOG(20,
            ...
            cls_lock_type_str(lock_type), fail_if_exists,
                       ...
                       entity_name_t & locker, const string & cookie)
{
    ...
    int r = read_lock(hctx, name, &linfo);
    if (r < 0) {
        CLS_ERR("Could not read list of current lockers off disk: %s",
                cpp_strerror(r).c_str());
        return r;
    }
    ...
    if (iter == lockers.end()) {    // no such key
        CLS_LOG(10, "locker %s [name: %s.%ld, cookie: %s] does not exist",
                name.c_str(), locker.type_str(), locker.num(), cookie.c_str());
        return -ENOENT;
    }
    lockers.erase(iter);
...
int assert_locked(cls_method_context_t hctx, bufferlist * in, bufferlist * out)
{
    ...
    if (linfo.tag != op.tag) {
        CLS_LOG(20, "lock tag mismatch: current=%s, assert=%s",
                linfo.tag.c_str(), op.tag.c_str());
        return -EBUSY;
    }
...
CLS_INIT(lock)
{
    ...
    cls_register_cxx_method(h_class, "get_info",
                            CLS_METHOD_RD, get_info, &h_get_info);
    cls_register_cxx_method(h_class, "list_locks",
                            CLS_METHOD_RD, list_locks, &h_list_locks);
