        ...
        struct MockTestJournal:public MockJournal {
            MOCK_METHOD4(append_write_event, uint64_t(uint64_t, size_t,
             ...
             MOCK_METHOD5(append_compare_and_write_event,
                          uint64_t(uint64_t, size_t, const bufferlist &,
                                   const bufferlist &, bool));
             MOCK_METHOD5(append_io_event_mock,
                          uint64_t(const journal::EventEntry &, uint64_t,
                                   size_t, bool, int));
            uint64_t append_io_event(journal::EventEntry
                                     && event_entry, uint64_t offset,
                                     size_t length, bool flush_entry,
                                     int filter_ret_val) {
                // googlemock doesn't support move semantics
                return append_io_event_mock(event_entry, offset, length,
...
namespace librbd {
    namespace io {
        ...
        struct TestMockIoImageRequest:public TestMockFixture {
            ...
            typedef ImageReadRequest < librbd::MockTestImageCtx >
                MockImageReadRequest;
            typedef ImageWriteRequest < librbd::MockTestImageCtx >
                MockImageWriteRequest;
            typedef ImageDiscardRequest < librbd::MockTestImageCtx >
                MockImageDiscardRequest;
            typedef ImageFlushRequest < librbd::MockTestImageCtx >
            ...
            void expect_is_journal_appending(MockTestJournal & mock_journal,
                                             bool appending) {
                EXPECT_CALL(mock_journal, is_journal_appending())
                    .WillOnce(Return(appending));
            } void expect_get_modify_timestamp(MockTestImageCtx &
                                               mock_image_ctx,
                                               bool needs_update) {
                if (needs_update) {
            ...
            void expect_journal_append_io_event(MockTestJournal & mock_journal,
                                                uint64_t journal_tid,
                                                uint64_t offset,
                                                size_t length) {
                EXPECT_CALL(mock_journal,
                            append_io_event_mock(_, offset, length, _, _))
                    .WillOnce(Return(journal_tid));
            }

            void expect_object_discard_request(MockTestImageCtx &
                                               mock_image_ctx,
                                               uint64_t object_no,
                ...
                EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
                    .
                    WillOnce(Invoke
                             ([&mock_image_ctx, object_no, offset, length, r]
                              (ObjectDispatchSpec * spec) {
                              auto * discard_spec =
                              boost::get < ObjectDispatchSpec::DiscardRequest >
                              (&spec->request);
            ...
            void expect_object_request_send(MockTestImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
                    .
                    WillOnce(Invoke
                             ([&mock_image_ctx, r] (ObjectDispatchSpec * spec) {
                              spec->dispatch_result =
                              io::DISPATCH_RESULT_COMPLETE;
                              mock_image_ctx.image_ctx->op_work_queue->
                              queue(&spec->dispatcher_ctx, r);}));
            ...
            void expect_object_list_snaps_request(MockTestImageCtx &
                                                  mock_image_ctx,
                                                  uint64_t object_no,
                                                  const SnapshotDelta &
                                                  snap_delta, int r) {
                EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
                    .WillOnce(Invoke([&mock_image_ctx, object_no, snap_delta, r]
                                     (ObjectDispatchSpec * spec) {
                                     auto request = boost::get <
                                     librbd::io::ObjectDispatchSpec::
                                     ListSnapsRequest > (&spec->request);
        ...
        TEST_F(TestMockIoImageRequest, AioWriteModifyTimestamp) {
            ...
            AioCompletion *aio_comp_1 =
                AioCompletion::create_and_start(&aio_comp_ctx_1, ictx,
                                                AIO_TYPE_WRITE);

            AioCompletion *aio_comp_2 =
                AioCompletion::create_and_start(&aio_comp_ctx_2, ictx,
                                                AIO_TYPE_WRITE);

            bufferlist bl;
            bl.append("1");
            MockImageWriteRequest mock_aio_image_write_1(mock_image_ctx,
                                                         aio_comp_1, { {
                                                         0, 1}},
                                                         ImageArea::DATA,
                                                         std::move(bl), 0, {
            ...
            bl.append("1");
            MockImageWriteRequest mock_aio_image_write_2(mock_image_ctx,
                                                         aio_comp_2, { {
                                                         0, 1}},
                                                         ImageArea::DATA,
                                                         std::move(bl), 0, {
        ...
        TEST_F(TestMockIoImageRequest, PartialDiscard) {
            ...
            AioCompletion *aio_comp =
                AioCompletion::create_and_start(&aio_comp_ctx, ictx,
                                                AIO_TYPE_DISCARD);
        ...
        TEST_F(TestMockIoImageRequest, TailDiscard) {
            ...
            InSequence seq;
            expect_get_modify_timestamp(mock_image_ctx, false);
            expect_object_discard_request(mock_image_ctx, 0,
        ...
        TEST_F(TestMockIoImageRequest, DiscardGranularity) {
            ...
            expect_object_discard_request(mock_image_ctx, 0,
                                          ictx->layout.object_size - 32, 32, 0);
        ...
        TEST_F(TestMockIoImageRequest, TailDiscardJournalAppendEnabled) {
            ...
            expect_get_modify_timestamp(mock_image_ctx, false);
            expect_is_journal_appending(mock_journal, true);
            expect_journal_append_io_event(mock_journal, 0,
                                           ictx->layout.object_size - 1024,
                                           1024);
            expect_object_discard_request(mock_image_ctx, 0,
                                          ictx->layout.object_size - 1024, 1024,
                                          0);
        ...
        TEST_F(TestMockIoImageRequest, PruneRequiredDiscardJournalAppendEnabled) {
            ...
            expect_is_journal_appending(mock_journal, true);
            EXPECT_CALL(mock_journal,
                        append_io_event_mock(_, _, _, _, _)).Times(0);
        ...
        TEST_F(TestMockIoImageRequest, DiscardGranularityJournalAppendEnabled) {
            ...
            expect_journal_append_io_event(mock_journal, 2,
                                           ictx->layout.object_size - 32, 32);
            expect_object_discard_request(mock_image_ctx, 0, 32, 32, 0);
            expect_object_discard_request(mock_image_ctx, 0, 96, 64, 0);
            expect_object_discard_request(mock_image_ctx, 0,
                                          ictx->layout.object_size - 32, 32, 0);
        ...
        TEST_F(TestMockIoImageRequest, AioWriteSameJournalAppendDisabled) {
            ...
            bufferlist bl;
            bl.append("1");
            MockImageWriteSameRequest mock_aio_image_writesame(mock_image_ctx,
                                                               aio_comp, { {
                                                               0, 1}},
                                                               ImageArea::DATA,
                                                               std::move(bl), 0,
        ...
        TEST_F(TestMockIoImageRequest, ListSnaps) {
            ...
            object_snapshot_delta[ {
                                  5, 6}].insert(0, 1024, {
                                                ...
                                                1024});
            object_snapshot_delta[ {
                                  5, 5}].insert(4096, 4096, {
                                                SPARSE_EXTENT_STATE_ZEROED,
                                                4096});
            expect_object_list_snaps_request(mock_image_ctx, 0,
                                             object_snapshot_delta, 0);
            object_snapshot_delta = {
            };
            object_snapshot_delta[ {
                                  5, 6}].insert(1024, 3072, {
                                                ...
                                                3072});
            object_snapshot_delta[ {
                                  5, 5}].insert(2048, 2048, {
                                                SPARSE_EXTENT_STATE_ZEROED,
                                                2048});
            expect_object_list_snaps_request(mock_image_ctx, 1,
                                             object_snapshot_delta, 0);
            ...
            AioCompletion *aio_comp =
                AioCompletion::create_and_start(&aio_comp_ctx, ictx,
                                                AIO_TYPE_GENERIC);
            MockImageListSnapsRequest
                mock_image_list_snaps_request(mock_image_ctx, aio_comp, { {
                                              0, 16384}, {
                                              16384, 16384}}, ImageArea::DATA, {
                                              0, CEPH_NOSNAP}, 0,
                                              &snapshot_delta, {
                                              });
            {
            ...
            expected_snapshot_delta[ {
                                    5, 6}].insert(0, 1024, {
                                                  ...
                                                  1024});
            expected_snapshot_delta[ {
                                    5, 6}].insert(5120, 3072, {
                                                  SPARSE_EXTENT_STATE_DATA,
                                                  3072});
            expected_snapshot_delta[ {
                                    5, 5}].insert(6144, 6144, {
                                                  SPARSE_EXTENT_STATE_ZEROED,
                                                  6144});
