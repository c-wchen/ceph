...
void PG::put_with_id(uint64_t id)
{
    ...
    lgeneric_subdout(cct, refs, 5) << "PG::put " << this << " " << info.pgid
        << " put id " << id << " " << (newref + 1) << " -> " << newref << dendl;
    {
        std::lock_guard l(_ref_id_lock);
        ceph_assert(_live_ids.count(id));
        _live_ids.erase(id);
...
PG::PG(OSDService * o, OSDMapRef curmap,
       const PGPool & _pool, spg_t p):pg_whoami(o->whoami, p.shard),
pg_id(p),
coll(p),
osd(o),
cct(o->cct),
...
recovery_state(o->cct,
               ...
               this,
               this),
pool(recovery_state.get_pgpool()), info(recovery_state.get_info())
{
#ifdef PG_DEBUG_REFS
    osd->add_pgid(p, this);
#endif
...
bool PG::is_locked() const const
{
    return ceph_mutex_is_locked(_lock);
}

void PG::unlock() const const
{
...
std::ostream & PG::gen_prefix(std::ostream & out) const const
{
...
void PG::queue_recovery()
{
    if (!is_primary() || !is_peered()) {
        ...
        uint64_t cost_per_object =
            std::max < uint64_t > (num_bytes / num_objects, 1);
                     ...
                     const hobject_t & end)
{
    ...
    b = ceph::make_ref < Backoff > (info.pgid, this, s, ++s->backoff_seq, begin,
                                    end);
...
void PG::send_cluster_message(int target, MessageRef m,
...
bool PG::check_in_progress_op(const osd_reqid_t & r,
                              ...
                              int *return_code,
                              vector < pg_log_op_return_item_t >
                              *op_returns) const const
{
    return (projected_log.get_request(r, version, user_version, return_code,
                                      op_returns) ||
            recovery_state.get_pg_log().get_log().get_request(r, version,
                                                              user_version,
                                                              return_code,
                                                              op_returns));
}
...
void PG::publish_stats_to_osd()
{
    ...
    recovery_state.update_stats_wo_resched([scrubber =
                                            m_scrubber.
                                            get()](pg_history_t & hist,
                                                   pg_stat_t & info) mutable->
                                           void {
                                           info.scrub_sched_status =
                                           scrubber->get_schedule();});

    std::lock_guard l {
    pg_stats_publish_lock};
    auto stats =
        recovery_state.prepare_stats_for_publish(pg_stats_publish,
                                                 unstable_stats);
    if (stats) {
...
unsigned PG::get_target_pg_log_entries() const const
{
...
void PG::read_state(ObjectStore * store)
{
    ...
    // initialize current mapping
    {
        ...
        get_osdmap()->pg_to_up_acting_osds(pg_id.pgid, &up, &up_primary,
                                           &acting, &primary);
        ...
        if (i->soid.snap < CEPH_MAXSNAP) {
            if (i->is_delete()) {
                int r = snap_mapper.remove_oid(i->soid,
                                               &_t);
                if (r)
                    derr << __func__ << " remove_oid " << i->
                        soid << " failed with " << r << dendl;
...
bool PG::get_must_scrub() const const
{
    dout(20) << __func__ << " must_scrub? " << (m_planned_scrub.
                                                must_scrub ? "true" : "false")
        << dendl;
    return m_planned_scrub.must_scrub;
}

unsigned int PG::scrub_requeue_priority(Scrub::scrub_prio_t with_priority) const const
{
    return m_scrubber->scrub_requeue_priority(with_priority);
}

unsigned int PG::scrub_requeue_priority(Scrub::scrub_prio_t with_priority,
...
double PG::next_deepscrub_interval() const const
{
...
bool PG::is_time_for_deep(bool allow_deep_scrub,
                          ...
                          const requested_scrub_t & planned) constconst
{
    dout(10) << fmt::format("{}: need-auto? {} allowed? {}/{} deep-errors? {} "
                            ...
                            allow_shallow_scrub,
                            allow_deep_scrub,
                            has_deep_errors, info.history.last_deep_scrub_stamp)
        << dendl;
...
/*
 ...
 clang-format on
*/
std::optional < requested_scrub_t >
    PG::validate_initiated_scrub(bool allow_deep_scrub, bool try_to_auto_repair,
                                 bool time_for_deep, bool has_deep_errors,
                                 const requested_scrub_t & planned) constconst
{
    requested_scrub_t upd_flags {
    planned};
...
*/
std::optional < requested_scrub_t >
    PG::validate_periodic_mode(bool allow_deep_scrub, bool try_to_auto_repair,
                               ...
                               const requested_scrub_t & planned) constconst
{
    ...
    dout(20) << fmt::format("{}: allowed:{}/{} t.f.d:{} req:{}",
                            __func__,
                            allow_shallow_scrub,
                            allow_deep_scrub, upd_flags.time_for_deep, planned)
        << dendl;
    ...
    if (upd_flags.time_for_deep) {
        if (allow_deep_scrub) {
            ...
            upd_flags.calculated_to_deep = true;
            dout(20) << fmt::format("{}: final: {}", __func__,
                                    upd_flags) << dendl;
            return upd_flags;
        }
        if (allow_shallow_scrub) {
 ...
 */
std::optional < requested_scrub_t > PG::validate_scrub_mode() constconst
{
    ...
    if (prevented_by_recovery) {
        dout(20) << __func__ << ": scrubbing prevented during recovery" <<
            dendl;
        return std::nullopt;
    }
...
{
    dout(20) << __func__ << " for a " << (is_primary()? "Primary" :
                                          "non-primary") << dendl;
                                                 ...
                                                 PGPeeringEventURef on_preempt)
{
    osd->local_reserver.request_reservation(pg_id,
                                            on_grant ? new QueuePeeringEvt(this,
                                                                           std::
                                                                           move
                                                                           (on_grant))
                                            : nullptr, priority,
                                            on_preempt ? new
                                            QueuePeeringEvt(this,
                                                            std::
                                                            move(on_preempt)) :
                                            nullptr);
}
...
void PG::on_active_advmap(const OSDMapRef & osdmap)
{
    ...
    if (j != new_purged_snaps.end()) {
        bool bad = false;
      for (auto k:j->second) {
            if (!recovery_state.get_info().purged_snaps.
                contains(k.first, k.second)) {
                interval_set < snapid_t > rm, overlap;
                rm.insert(k.first, k.second);
                overlap.intersection_of(recovery_state.get_info().purged_snaps,
                                        rm);
                derr << __func__ << " purged_snaps does not contain " << rm <<
                    ", only " << overlap << dendl;
                recovery_state.
                    adjust_purged_snaps([&overlap] (auto & purged_snaps) {
...
ceph::signedspan PG::get_mnow() constconst
{
    return osd->get_mnow();
}
...
{
    pglog.rebuild_missing_set_with_deletes(osd->store,
                                           ch, recovery_state.get_info());
}
                                ...
                                int64_t local_bytes)
{
    lgeneric_dout(cct, 20) << __func__ << " Adjust local usage "
        << (local_bytes >> 10) << "KiB" << " primary usage " << (bf_bytes >> 10)
        << "KiB" << dendl;
                             ...
                             std::string_view desc)
{
    ...
    if (is_active()) {
        ...
        dout(5) << __func__ << " refusing to forward. " << (is_clean()?
                                                            "(clean) " :
                                                            "(not clean) ") <<
...
Scrub::scrub_prio_t PG::is_scrub_blocking_ops() constconst
{
...
void PG::start_flush_on_transaction(ObjectStore::Transaction & t)
{
    ...
    t.register_on_applied(new ContainerContext < FlushStateRef >
                          (flush_trigger));
    t.register_on_commit(new ContainerContext < FlushStateRef >
                         (flush_trigger));
}
...
{
    Context *c =
        new QueuePeeringEvt(this, get_osdmap_epoch(),
    ...
    if (m->get_connection()->has_feature(CEPH_FEATURE_RESEND_ON_SPLIT)) {
        ...
        if (m->get_connection()->has_feature(CEPH_FEATURE_SERVER_NAUTILUS)) {
            ...
            if (m->get_map_epoch() <
                ...
                dout(7) << __func__ <<
                    " sent before last_force_op_resend_prenautilus " << pool.
                    info.
                    last_force_op_resend_prenautilus << ", dropping" << *m <<
                    dendl;
                return true;
            }
        }
    ...
    bool PG::can_discard_replica_op(OpRequestRef & op)
{
    ...
    if (next_map->get_down_at(from) >= m->map_epoch) {
        dout(20) << " " << __func__ << " dead for 'get_down_at' " << from <<
            dendl;
        return true;
    }
...
void PG::find_unfound(epoch_t queued, PeeringCtx & rctx)
{
    ...
    if (!recovery_state.discover_all_missing(rctx)) {
        ...
        if (state_test(PG_STATE_BACKFILLING)) {
            auto evt = PGPeeringEventRef(new PGPeeringEvent(queued,
                                                            queued,
                                                            PeeringState::
                                                            UnfoundBackfill()));
                            ...
                            PeeringCtx & rctx)
{
    ...
    recovery_state.advance_map(osdmap,
                               lastmap,
                               newup,
                               up_primary, newacting, acting_primary, rctx);
}
...
std::pair < ghobject_t, bool > PG::do_delete_work(ObjectStore::Transaction & t,
                                                  ghobject_t _next)
{
    dout(10) << __func__ << dendl;

    {
        ...
        if (osd_delete_sleep > 0 && delete_needs_sleep) {
            ...
            PGRef pgref(this);
            auto delete_requeue_callback =
                new LambdaContext([this, pgref, e] (int r){
                                  dout(20) <<
                                  "do_delete_work() [cb] wake up at " <<
                                  ceph_clock_now()
                                  << ", re-queuing delete" << dendl;
                                  std::scoped_lock locker {
                                  *this}; delete_needs_sleep = false;
            ...
            dout(20) << __func__ << " Delete scheduled at " <<
                delete_schedule_time << dendl;
    ...
    osd->store->collection_list(ch,
                                _next,
                                ghobject_t::get_max(), max, &olist, &next);
    ...
    if (_next != ghobject_t() && olist.empty()) {
      ...
      for (auto & oid:olist) {
            if (oid == pgmeta_oid) {
                dout(20) << __func__ << " removing pgmeta object " << oid <<
                    dendl;
            }
            else {
  ...
  for (auto & oid:olist) {
    ...
    if (num) {
        ...
        if (!osd->try_finish_pg_delete(this, pool.info.get_pg_num())) {
            dout(1) << __func__ << " raced with merge, reinstantiating" <<
                dendl;
            ch = osd->store->create_new_collection(coll);
            create_pg_collection(t,
                                 info.pgid,
                                 info.pgid.get_split_bits(pool.info.
                                                          get_pg_num()));
...
int PG::pg_stat_adjust(osd_stat_t * ns)
{
    ...
    if (reserved_num_bytes > 0) {
        ...
        dout(20) << __func__ << " reserved_num_bytes " << (reserved_num_bytes >>
                                                           10) << "KiB" <<
...
void PG::dump_missing(Formatter * f)
{
  for (auto & i:recovery_state.get_pg_log().get_missing().get_items()) {
        ...
        if (recovery_state.get_missing_loc().needs_recovery(i.first)) {
          ...
          for (auto l:recovery_state.get_missing_loc().get_locations(i.
                                                           first))
            {
...
void PG::with_pg_stats(ceph::coarse_real_clock::time_point now_is,
                       std::function < void (const pg_stat_t &,
                                             epoch_t lec) > &&f)
{
...
uint64_t PG::get_min_alloc_size() const const
{
