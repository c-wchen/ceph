...
int ErasureCodeClay::init(ErasureCodeProfile & profile, ostream * ss)
{
    ...
    r = registry.factory(mds.profile["plugin"], directory, mds.profile,
                         &mds.erasure_code, ss);
    if (r)
        return r;
    r = registry.factory(pft.profile["plugin"],
                         directory, pft.profile, &pft.erasure_code, ss);
    return r;

}

unsigned int ErasureCodeClay::get_chunk_size(unsigned int object_size) const const
{
        ...
        for (int j = 0; j < q; j++) {
            if (j != lost_node_index % q) {
                ...
                if (rep_node_index < k) {
                    ...
                    minimum->
                        insert(make_pair(rep_node_index - nu, sub_chunk_ind));
                }
            }
        }
  ...
  for (auto to_read:want_to_read) {
    ...
    for (int y = 0; y < t; y++) {
        repair_subchunks_count =
            repair_subchunks_count * (q - weight_vector[y]);
    }
                            ...
                            map < int, bufferlist > *repaired, int chunk_size)
{
    ...
    ceph_assert(helper_data.size() + aloof_nodes.size() +
                recovered_data.size() == (unsigned)q * t);
...
int ErasureCodeClay::repair_one_lost_chunk(map < int,
                                           bufferlist > &recovered_data,
                                           set < int >&aloof_nodes, map < int,
                                           bufferlist > &helper_data,
                                           ...
                                           int >>&repair_sub_chunks_ind)
{
  ...
  for (auto[index, count]:repair_sub_chunks_ind) {
    ...
    for (int i = 0; i < q * t; i++) {
        if (U_buf[i].length() == 0) {
            bufferptr buf(buffer::
                          create_aligned(sub_chunk_no * sub_chunksize,
                                         SIMD_ALIGN));
            buf.zero();
                    ...
                    if (erasures.count(node_xy) == 0) {
                        ...
                        if (aloof_nodes.count(node_sw) > 0) {
                            ...
                            known_subchunks[i0].substr_of(helper_data[node_xy],
                                                          ...
                                                          * sub_chunksize,
                                                          sub_chunksize);
                            known_subchunks[i3].substr_of(U_buf[node_sw],
                                                          z_sw * sub_chunksize,
                                                          sub_chunksize);
                            pftsubchunks[i0] = known_subchunks[i0];
                            pftsubchunks[i1] = temp_buf;
                            pftsubchunks[i2].substr_of(U_buf[node_xy],
                                                       z * sub_chunksize,
                                                       sub_chunksize);
                            ...
                            pft.erasure_code->decode_chunks(pft_erasures,
                                                            known_subchunks,
                                                            &pftsubchunks);
                        }
                        else {
                            ...
                            if (z_vec[y] != x) {
                                pft_erasures.insert(i2);
                                ceph_assert(repair_plane_to_ind.count(z_sw) >
                                            0);
                                known_subchunks[i0].
                                    substr_of(helper_data[node_xy],
                                              repair_plane_to_ind[z] *
                                              sub_chunksize, sub_chunksize);
                                known_subchunks[i1].
                                    substr_of(helper_data[node_sw],
                                ...
                                pftsubchunks[i2].substr_of(U_buf[node_xy],
                                                           z * sub_chunksize,
                                                           sub_chunksize);
                                pftsubchunks[i3].substr_of(temp_buf, 0,
                                                           sub_chunksize);
                                for (int i = 0; i < 3; i++) {
                                    pftsubchunks[i].rebuild_aligned(SIMD_ALIGN);
                                }
                                ...
                                char *coupled_chunk =
                                    helper_data[node_xy].c_str();
                                memcpy(&uncoupled_chunk[z * sub_chunksize],
                                       &coupled_chunk[repair_plane_to_ind[z] *
                                                      sub_chunksize],
                                       sub_chunksize);
                            }
                        }
                ...
                if (aloof_nodes.count(i) == 0) {
                    if (x == z_vec[y]) {    // hole-dot pair (type 0)
                        ...
                        known_subchunks[i0].substr_of(helper_data[i],
                                                      ...
                                                      sub_chunksize,
                                                      sub_chunksize);
                        known_subchunks[i2].substr_of(U_buf[i],
                                                      z * sub_chunksize,
                                                      sub_chunksize);

                        pftsubchunks[i0] = known_subchunks[i0];
                        pftsubchunks[i1].substr_of(recovered_data[node_sw],
                                                   z_sw * sub_chunksize,
                                                   sub_chunksize);
              ...
              for (auto node_xy:erased_chunks) {
                    ...
                    if (z_vec[y] != x) {
                        if (erased_chunks.count(node_sw) == 0) {
                            recover_type1_erasure(chunks, x, y, z, z_vec,
                                                  sc_size);
                        }
                        else if (z_vec[y] < x) {
                            ceph_assert(erased_chunks.count(node_sw) > 0);
                            ceph_assert(z_vec[y] != x);
                            get_coupled_from_uncoupled(chunks, x, y, z, z_vec,
                                                       sc_size);
                        }
                    }
                    else {
            ...
            if (erased_chunks.count(node_xy) == 0) {
                if (z_vec[y] < x) {
                    ...
                    memcpy(&uncoupled_chunk[z * sc_size],
                           &coupled_chunk[z * sc_size], sc_size);
                }
                else {
                    if (erased_chunks.count(node_sw) > 0) {
                        get_uncoupled_from_coupled(chunks, x, y, z, z_vec,
                                                   sc_size);
                    }
                }
            }
...
int ErasureCodeClay::decode_uncoupled(const set < int >&erased_chunks, int z,
                                      int sc_size)
{
    map < int, bufferlist > known_subchunks;
    ...
    mds.erasure_code->decode_chunks(erased_chunks, known_subchunks,
                                    &all_subchunks);
    return 0;
}
                                            ...
                                            int *z_vec, int sc_size)
{
    ...
    pft.erasure_code->decode_chunks(erased_chunks, known_subchunks,
                                    &pftsubchunks);
}

void ErasureCodeClay::get_coupled_from_uncoupled(map < int,
                                                 bufferlist > *chunks, int x,
                                                 int y, int z, int *z_vec,
                                                 int sc_size)
{
    ...
    pft.erasure_code->decode_chunks(erased_chunks, uncoupled_subchunks,
                                    &pftsubchunks);
}

void ErasureCodeClay::get_uncoupled_from_coupled(map < int,
                                                 bufferlist > *chunks, int x,
                                                 int y, int z, int *z_vec,
                                                 int sc_size)
{
    ...
    coupled_subchunks[i1].substr_of((*chunks)[node_sw], z_sw * sc_size,
                                    sc_size);
    ...
    pft.erasure_code->decode_chunks(erased_chunks, coupled_subchunks,
                                    &pftsubchunks);
}
