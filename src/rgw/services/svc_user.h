...
class RGWSI_User:public RGWServiceInstance {
  public:
    ...
    virtual int read_user_info(RGWSI_MetaBackend::Context * ctx,
                               ...
                               std::map < std::string,
                               bufferlist > *const pattrs, optional_yield y,
    ...
    virtual int get_user_info_by_email(RGWSI_MetaBackend::Context * ctx,
                                       ...
                                       const DoutPrefixProvider * dpp) = 0;
    virtual int get_user_info_by_swift(RGWSI_MetaBackend::Context * ctx, const std::string & swift_name, RGWUserInfo * info,    /* out */
    ...
    virtual int read_stats(const DoutPrefixProvider * dpp, RGWSI_MetaBackend::Context * ctx, const rgw_user & user, RGWStorageStats * stats, ceph::real_time * last_stats_sync, /* last time a full stats sync completed */
                           ceph::real_time * last_stats_update, optional_yield y) = 0;  /* last time a stats update was done */
