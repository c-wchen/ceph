...
void MemStore::dump(Formatter * f)
{
    ...
    for (ceph::unordered_map < coll_t, CollectionRef >::iterator p =
        ...
        for (map < ghobject_t, ObjectRef >::iterator q =
             p->second->object_map.begin(); q != p->second->object_map.end();
             ++q) {
...
int MemStore::stat(const coll_t & cid,
...
int MemStore::read(const coll_t & cid,
                              ...
                              vector < ghobject_t > *ls, ghobject_t * next)
{
    ...
    dout(10) << __func__ << " cid " << cid << " start " << start
        << " end " << end << dendl;
    map < ghobject_t, ObjectRef >::iterator p =
        c->object_map.lower_bound(start);
...
int MemStore::omap_get(const coll_t & cid,  ///< [in] Collection containing oid
...
int MemStore::omap_get_header(const coll_t & cid,   ///< [in] Collection containing oid
                              const ghobject_t & oid,   ///< [in] Object containing omap
                              bufferlist * header,  ///< [out] omap header
...
int MemStore::omap_get_keys(const coll_t & cid, ///< [in] Collection containing oid
                            const ghobject_t & oid, ///< [in] Object containing omap
...
int MemStore::omap_get_values(const coll_t & cid,   ///< [in] Collection containing oid
                              const ghobject_t & oid,   ///< [in] Object containing omap
...
int MemStore::omap_check_keys(const coll_t & cid,   ///< [in] Collection containing oid
                              const ghobject_t & oid,   ///< [in] Object containing omap
                                 ...
                                 TrackedOpRef op, ThreadPool::TPHandle * handle)
{
    ...
    struct OpSequencer:public Sequencer_impl {
        OpSequencer(CephContext * cct):Sequencer_impl(cct) {
        } std::mutex mutex;
        void flush() override {
        }
