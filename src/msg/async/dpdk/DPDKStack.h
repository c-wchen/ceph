...
template < typename Protocol > class DPDKServerSocketImpl:public ServerSocketImpl {
  ...
  public:
    DPDKServerSocketImpl(Protocol & proto, uint16_t port,
...
template < typename Protocol > class NativeConnectedSocketImpl:public ConnectedSocketImpl
{
  ...
  :    _conn(std::move(rhs._conn)), _buf(std::move(rhs.buf)) {
    ...
    virtual ssize_t zero_copy_read(bufferptr & data) override {
        ...
        auto del = std::bind([](Packet & p) { }
                             , std::move(p));
        data =
            buffer::claim_buffer(f.size, f.base, make_deleter(std::move(del)));
    ...
    virtual ssize_t send(bufferlist & bl, bool more) override {
        ...
        if (len != bl.length()) {
            ...
            auto del = std::bind([](bufferlist & bl) { }
                                 , std::move(swapped));
            return _conn.
                send(Packet(std::move(frags), make_deleter(std::move(del))));
        }
        else {
            auto del = std::bind([](bufferlist & bl) { }
                                 , std::move(bl));
...
template < typename Protocol >
    DPDKServerSocketImpl < Protocol >::DPDKServerSocketImpl(Protocol & proto,
                                                            uint16_t port,
                                                            const SocketOptions
                                                            & opt)
                                                  ...
                                                  Worker * w)
{
    ...
    if (out)
        *out = c->remote_addr();
    std::unique_ptr < NativeConnectedSocketImpl <
        Protocol >> csi(new NativeConnectedSocketImpl < Protocol >
                        (std::move(*c)));
...
class DPDKWorker:public Worker {
    struct Impl {
        ...
        ipv4 _inet;
         Impl(CephContext * cct, unsigned i, EventCenter * c,
...
class DPDKStack:public NetworkStack {
  ...
  public:
     explicit DPDKStack(CephContext * cct, const string & t):NetworkStack(cct,
                                                                          t) {
        funcs.resize(cct->_conf->ms_async_max_op_threads);
    } virtual bool support_zero_copy_read() const override {
        return true;
    } virtual bool support_local_listen_table() const {
        return true;
    } virtual void spawn_worker(unsigned i,
