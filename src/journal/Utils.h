...
namespace journal {
    namespace utils {
        ...
        } // namespace detail template < typename T, void (T::*MF) (int)>
        ...
        std::string get_object_name(const std::string & prefix,
                                    uint64_t number);
        ...
        template < typename M >
            Context * create_async_context_callback(M journal_metadata,
                                                    Context * on_finish) {
            // use async callback to acquire a clean lock context
            return new detail::C_AsyncCallback < M > (journal_metadata,
                                                      on_finish);
        }
