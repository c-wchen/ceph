                           ...
                           const bufferlist *opt_content)
{
    ...
    if (cct->_conf->subsys.should_gather < ceph_subsys_rgw, 20 > ()) {
        for (const auto& i : env.get_map()) {
            ldpp_dout(dpp, 20) << __func__ << "():> " << i.first << " -> " << rgw::crypt_sanitize::x_meta_map{i.first, i.second} <<
                               dendl;
        }
    }
...
int RGWRESTSimpleRequest::forward_request(const DoutPrefixProvider *dpp, const RGWAccessKey& key, const req_info& info,
        size_t max_response, bufferlist *inbl, bufferlist *outbl, optional_yield y, std::string service)
{
...
{
    dout(20) << "RGWRESTStreamOutCB::handle_data bl.length()=" << bl.length() << " bl_ofs=" << bl_ofs << " bl_len=" <<
             bl_len << dendl;
...
static bool grants_by_type_check_perm(map < int, string > & grants_by_type, int perm, const ACLGrant& grant,
                                      int check_perm)
{
...
void RGWRESTStreamS3PutObj::send_ready(const DoutPrefixProvider *dpp, RGWAccessKey& key, map < string,
...
void RGWRESTStreamS3PutObj::send_ready(const DoutPrefixProvider *dpp, RGWAccessKey& key, const map < string,
                                       string > & http_attrs,
                                       RGWAccessControlPolicy& policy)
{
...
int RGWRESTStreamRWRequest::send_request(const DoutPrefixProvider *dpp, RGWAccessKey& key, map < string,
        string > & extra_headers, const rgw_obj& obj, RGWHTTPManager *mgr)
{
    string resource;
...
int RGWRESTStreamRWRequest::send_prepare(const DoutPrefixProvider *dpp, RGWAccessKey& key, map < string,
        string > & extra_headers, const rgw_obj& obj)
{
    string resource;
    send_prepare_convert(obj, &resource);
...
int RGWRESTStreamRWRequest::send_prepare(const DoutPrefixProvider *dpp, RGWAccessKey *key, map < string,
        string > & extra_headers, const string& resource,
...
int RGWRESTStreamRWRequest::do_send_prepare(const DoutPrefixProvider *dpp, RGWAccessKey *key, map < string,
        string > & extra_headers, const string& resource,
        bufferlist *send_data)
{
    string new_url = url;
