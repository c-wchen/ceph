...
class KernelDevice:public BlockDevice {
  ...
  private:
    ...
    virtual int _post_open() {
        return 0;
    }                           // hook for child implementations
    virtual void _pre_close() {
    }                           // hook for child implementations

    void _aio_thread();
    void _discard_thread();
    int _queue_discard(interval_set < uint64_t > &to_release);
    bool try_discard(interval_set < uint64_t > &to_release, bool async =
                     true) override;
    ...
    int _sync_write(uint64_t off, ceph::buffer::list & bl, bool buffered,
                    int write_hint);
    ...
    ceph::unique_leakable_ptr < buffer::raw > create_custom_aligned(size_t len,
                                                                    IOContext *
                                                                    ioc) const;
  ...
  public:
    ...
    int write(uint64_t off, ceph::buffer::list & bl, bool buffered,
              int write_hint = WRITE_LIFE_NOT_SET) override;
    int aio_write(uint64_t off, ceph::buffer::list & bl, IOContext * ioc,
                  bool buffered, int write_hint = WRITE_LIFE_NOT_SET) override;
    int flush() override;
