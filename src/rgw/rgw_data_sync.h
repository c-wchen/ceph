...
{
} rgw_bucket_entry_owner(const string & _id,
                         const string & _display_name):id(_id),
...
struct RGWDataSyncEnv {
    ...
    RGWSyncModuleInstanceRef sync_module;
     ...
     RGWDataSyncEnv():cct(NULL), store(NULL), conn(NULL), async_rados(NULL),
        http_manager(NULL), error_logger(NULL), sync_module(NULL) {
...
class RGWRemoteDataLog:public RGWCoroutinesManager {
  ...
  public:
    ...
    :RGWCoroutinesManager(_store->ctx(), _store->get_cr_registry()),
        ...
        lock("RGWRemoteDataLog::lock"), data_sync_cr(NULL), initialized(false) {
    ...
    int read_source_log_shards_info(map < int,
                                    RGWDataChangesLogInfo > *shards_info);
    int read_source_log_shards_next(map < int, string > shard_markers,
                                    map < int,
                                    rgw_datalog_shard_data > *result);
    int read_sync_status(rgw_data_sync_status * sync_status);
    int read_recovering_shards(const int num_shards,
                               set < int >&recovering_shards);
    int read_shard_status(int shard_id, set < string > &lagging_buckets,
...
class RGWDataSyncStatusManager {
  ...
  public:
     RGWDataSyncStatusManager(RGWRados * _store,
                              RGWAsyncRadosProcessor * async_rados,
                              const string & _source_zone)
    :store(_store), source_zone(_source_zone), conn(NULL), error_logger(NULL),
        sync_module(nullptr), source_log(store, async_rados), num_shards(0) {
    } RGWDataSyncStatusManager(RGWRados * _store,
                               RGWAsyncRadosProcessor * async_rados,
                               const string & _source_zone,
                               const RGWSyncModuleInstanceRef & _sync_module)
    :store(_store), source_zone(_source_zone), conn(NULL), error_logger(NULL),
        sync_module(_sync_module), source_log(store, async_rados),
        num_shards(0) {
    }
    ...
    int read_shard_status(int shard_id, set < string > &lagging_buckets,
                          ...
                          rgw_data_sync_marker * sync_marker,
                          const int max_entries) {
        return source_log.read_shard_status(shard_id, lagging_buckets,
    ...
    int run() {
        return source_log.run_sync(num_shards);
    }
...
struct rgw_bucket_shard_sync_info {
    ...
    void decode_from_attrs(CephContext * cct, map < string,
                           bufferlist > &attrs);
...
class RGWRemoteBucketLog:public RGWCoroutinesManager {
  ...
  public:
     RGWRemoteBucketLog(RGWRados * _store, RGWBucketSyncStatusManager * _sm,
                        RGWAsyncRadosProcessor * _async_rados,
                        RGWHTTPManager *
                        _http_manager):RGWCoroutinesManager(_store->ctx(),
                                                            _store->
                                                            get_cr_registry()),
        store(_store), status_manager(_sm), async_rados(_async_rados),
        http_manager(_http_manager) {
    } int init(const string & _source_zone, RGWRESTConn * _conn,
...
class RGWBucketSyncStatusManager {
  ...
  public:
     RGWBucketSyncStatusManager(RGWRados * _store, const string & _source_zone,
        ...
        source_zone(_source_zone),
        conn(NULL), error_logger(NULL), bucket(bucket), num_shards(0) {
    ...
    static string status_oid(const string & source_zone,
...
class RGWDefaultSyncModule:public RGWSyncModule {
  public:
    ...
    int create_instance(CephContext * cct, map < string, string,
                        ltstr_nocase > &config,
