...
class RadosTestContext {
  ...
  RadosTestContext(const std::string & pool_name, int max_in_flight, uint64_t max_size, uint64_t min_stride_size, uint64_t max_stride_size, bool no_omap, bool no_sparse, bool pool_snaps, bool write_fadvise_dontneed, const std::string & low_tier_pool_name, bool enable_dedup, std::string chunk_algo, std::string chunk_size, const char *id = 0):
    pool_obj_cont(),
        ...
        chunk_algo(chunk_algo), chunk_size(chunk_size) {
    } int init() {
        ...
        bufferlist inbl;
        r = rados.mon_command("{\"prefix\": \"osd pool set\", \"pool\": \"" +
                              pool_name +
        ...
        if (enable_dedup) {
            r = rados.
                mon_command("{\"prefix\": \"osd pool set\", \"pool\": \"" +
                            pool_name +
                            "\", \"var\": \"fingerprint_algorithm\", \"val\": \""
                            + "sha256" + "\"}", inbl, NULL, NULL);
            if (r < 0) {
                rados.shutdown();
                return r;
            }
            r = rados.
                mon_command("{\"prefix\": \"osd pool set\", \"pool\": \"" +
                            pool_name +
                            "\", \"var\": \"dedup_tier\", \"val\": \"" +
                            low_tier_pool_name + "\"}", inbl, NULL, NULL);
            if (r < 0) {
                rados.shutdown();
                return r;
            }
    ...
    ObjectDesc get_most_recent(const std::string & oid) {
        ...
        for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
            std::map < std::string, ObjectDesc >::iterator j =
                i->second.find(oid);
            if (j != i->second.end()) {
                new_obj = j->second;
                break;
    ...
    void rm_object_attrs(const std::string & oid,
    ...
    void update_object_attrs(const std::string & oid,
    ...
    void update_object_full(const std::string & oid,
                            const ObjectDesc & contents) {
    ...
    bool find_object(const std::string & oid, ObjectDesc * contents, int snap =
                     -1) const {
    ...
    void update_object_redirect_target(const std::string & oid,
                                       const std::string & target) {
        redirect_objs[oid] = target;
    }

    void update_object_chunk_target(const std::string & oid, uint64_t offset,
    ...
    bool object_existed_at(const std::string & oid, int snap = -1)const {
        ...
        std::map < int, std::map < std::string,
            ObjectDesc > >::iterator next_iter = pool_obj_cont.find(snap);
        ceph_assert(next_iter != pool_obj_cont.end());
        std::map < int, std::map < std::string,
            ObjectDesc > >::iterator current_iter = next_iter++;
        ...
        for (auto i = current.begin(); i != current.end(); ++i) {
            if (next.count(i->first) == 0) {
                next.insert(std::pair < std::string,
                            ObjectDesc > (i->first, i->second));
            }
        }
    ...
    bool check_chunks_refcount(librados::IoCtx & chunk_pool_ctx,
        ...
        chunk_pool_ctx.object_list_slice(begin,
                                         end, 1, 1, &shard_start, &shard_end);

        librados::ObjectCursor c(shard_start);
        while (c < shard_end) {
            std::vector < librados::ObjectItem > result;
            int r =
                chunk_pool_ctx.object_list(c, shard_end, 12, { }, &result, &c);
            if (r < 0) {
                std::cerr << "error object_list : " << cpp_strerror(r) << std::
                    endl;
                return false;
            }
          ...
          for (const auto & i:result) {
              ...
              for (auto & pp:byo->by_object) {
                    ...
                    for (int tries = 0; tries < 10; tries++) {
                        r = cls_cas_references_chunk(manifest_pool_ctx,
                                                     pp.oid.name, oid);
...
class RemoveAttrsOp:public TestOp {
  public:
    ...
    RemoveAttrsOp(int n, RadosTestContext * context,
                  const std::string & oid, TestOpStat * stat)
        ...
        std::set < std::string > to_remove;
        {
            ...
            if (rand() % 30) {
                ContentsGenerator::iterator iter =
                    context->attr_gen.get_iterator(cont);
                for (auto i = obj.attrs.begin(); i != obj.attrs.end();
                     ++i, ++iter) {
                    if (!(*iter % 3)) {
...
class SetAttrsOp:public TestOp {
  public:
    ...
    :TestOp(n, context, stat), oid(oid), comp(NULL) {
        ...
        bufferlist header;
        ContentsGenerator::iterator keygen =
            context->attr_gen.get_iterator(cont);
        op.create(false);
        while (!*keygen)
            ++keygen;
...
class WriteOp:public TestOp {
  public:
    ...
    WriteOp(int n,
            RadosTestContext * context,
            const std::string & oid,
            bool do_append, bool do_excl, TestOpStat * stat = 0)
  :    
    TestOp(n, context, stat),
    oid(oid), do_append(do_append), do_excl(do_excl) {
    } void _begin() override {
        assert(!done);
        std::stringstream acc;
        std::lock_guard state_locker {
        context->state_lock};
        acc << context->prefix << "OID: " << oid << " snap " << context->
            current_snap << std::endl;
        std::string prefix = acc.str();

        cont =
            ContDesc(context->seq_num, context->current_snap, context->seq_num,
                     prefix);
        ...
        if (do_append) {
            ...
            uint64_t prev_length = found && old_value.has_contents()?
                old_value.most_recent_gen()->get_length(old_value.
                                                        most_recent()) : 0;
            bool requires_alignment;
            int r =
                context->io_ctx.pool_requires_alignment2(&requires_alignment);
            ceph_assert(r == 0);
            ...
            cont_gen = new AppendGenerator(prev_length,
                                           alignment,
            ...
            cont_gen =
                new VarLenGenerator(context->max_size, context->min_stride_size,
        ...
        std::cout << num << ":  seq_num " << context->
            seq_num << " ranges " << ranges << std::endl;
        context->seq_num++;
      ...
      for (auto[offset, len]:ranges) {
        ...
        context->io_ctx.aio_operate(context->prefix + oid, completion,
                                    &write_op);
        ...
        context->io_ctx.aio_operate(context->prefix + oid, rcompletion, &read_op, librados::OPERATION_ORDER_READS_WRITES,   // order wrt previous write/update
                                    0);
    }
    ...
    void _finish(CallbackInfo * info) override {
        ...
        std::
            cout << num << ":  finishing write tid " << tid << " to " <<
        ...
        if (waiting_on == 0) {
            ...
            for (auto i = waiting.begin(); i != waiting.end();) {
                ...
                if (int err = (*i)->get_return_value()) {
                    std::
                        cerr << "Error: oid " << oid <<
                        " write returned error code " << err << std::endl;
                    ceph_abort();
                }
                if ((*i)->get_version64() > version) {
                    std::
                        cout << num << ":  oid " << oid << " updating version "
                        << version << " to " << (*i)->
                        get_version64() << std::endl;
                    version = (*i)->get_version64();
                }
                else {
                    std::
                        cout << num << ":  oid " << oid << " version " <<
                        version << " is already newer than " << (*i)->
                        get_version64() << std::endl;
                }
                (*i)->release();
                waiting.erase(i++);
            ...
            if (rcompletion->get_version64() != version) {
                std::
                    cerr << "Error: racing read on " << oid <<
                    " returned version " << rcompletion->
                    get_version64() << " rather than version " << version <<
                    std::endl;
...
class WriteSameOp:public TestOp {
  public:
    ...
    WriteSameOp(int n,
                RadosTestContext * context,
                const std::string & oid, TestOpStat * stat = 0)
  :    
    TestOp(n, context, stat),
    oid(oid), rcompletion(NULL), waiting_on(0), last_acked_tid(0) {
    } void _begin() override {
        std::lock_guard state_locker {
        context->state_lock};
        done = 0;
    ...
    void _finish(CallbackInfo * info) override {
        ...
        std::
            cout << num << ":  finishing writesame tid " << tid << " to " <<
            context->prefix + oid << std::endl;
        ...
        if (waiting_on == 0) {
            ...
            for (auto i = waiting.begin(); i != waiting.end();) {
                ...
                if (int err = (*i)->get_return_value()) {
                    std::
                        cerr << "Error: oid " << oid <<
                        " writesame returned error code " << err << std::endl;
                    ceph_abort();
                }
                if ((*i)->get_version64() > version) {
...
class DeleteOp:public TestOp {
  ...
  :    TestOp(n, context, stat), oid(oid) {
        ...
        if (r && !(r == -ENOENT && !present)) {
            std::
                cerr << "r is " << r << " while deleting " << oid <<
...
class ReadOp:public TestOp {
  ...
  :    
    ...
    waiting_on(0), checksums(3), checksum_retvals(3), attrretval(0) {
        ...
        if (context->no_sparse || rand() % 2) {
            ...
            read_op.checksum(LIBRADOS_CHECKSUM_TYPE_CRC32C, init_value_bl, 0,
                             len, 0, &checksums[index],
    ...
    void _begin() override {
        ...
        std::cout << num << ": read oid " << oid << " snap " << snap << std::
            endl;
        done = 0;
        for (uint32_t i = 0; i < 3; i++) {
            completions[i] =
                context->rados.aio_create_completion((void *)this,
                                                     &read_callback);
        }
        ...
        if (!context->no_omap) {
            op.omap_get_vals_by_keys(omap_requested_keys, &omap_returned_values,
                                     0);
        ...
        ceph_assert(!context->io_ctx.
                    aio_operate(context->prefix + oid, completions[0], &op,
                                flags, NULL));
        waiting_on++;
        ...
        for (uint32_t i = 1; i < 3; ++i) {
            ...
            ceph_assert(!context->io_ctx.
                        aio_operate(context->prefix + oid, completions[i],
                                    &pipeline_op, 0));
            waiting_on++;
        }
        ...
        if (!retval) {
            ...
            if (old_value.has_contents()) {
                ...
                for (unsigned i = 0; i < results.size(); i++) {
                    if (is_sparse_read[i]) {
                        if (!old_value.
                            check_sparse(extent_results[i], results[i])) {
                            std::
                                cerr << num << ": oid " << oid << " contents "
                                << to_check << " corrupt" << std::endl;
                            context->errors++;
                        }
                    }
                    else {
                        ...
                        if (checksum_retvals[i] != 0
                            || checksum != results[i].crc32c(-1)) {
                            std::
                                cerr << num << ": oid " << oid << " checksum "
                                << checksums[i]
                                << " incorrect, expecting " << results[i].
                                crc32c(-1)
                                << std::endl;
                            context->errors++;
                        }
            ...
            if (!context->no_omap) {
                if (!(old_value.header == header)) {
                    std::
                        cerr << num << ": oid " << oid <<
                        " header does not match, old size: " << old_value.
                        header.length() << " new size " << header.length()
                        << std::endl;
                    ceph_assert(old_value.header == header);
                }
                if (omap.size() != old_value.attrs.size()) {
                    std::
                        cerr << num << ": oid " << oid << " omap.size() is " <<
                        omap.size()
                        << " and old is " << old_value.attrs.
                        size() << std::endl;
...
class SnapCreateOp:public TestOp {
  ...
  :    TestOp(n, context, stat) {
        ...
        if (!context->pool_snaps) {
            ...
            int r =
                context->io_ctx.selfmanaged_snap_set_write_ctx(context->seq,
                                                               snapset);
            if (r) {
                std::
                    cerr << "r is " << r << " snapset is " << snapset <<
                    " seq is " << context->seq << std::endl;
                ceph_abort();
            }
        }
    }
    ...
    bool must_quiesce_other_ops() override {
...
class WatchOp:public TestOp {
  ...
  public:
    WatchOp(int n,
            RadosTestContext * context,
            const std::string & _oid, TestOpStat * stat = 0)
...
class RollbackOp:public TestOp {
  ...
  :    
    ...
    comps(3, NULL), last_finished(-1), outstanding(3) {
    } void _begin() override {
        ...
        in_use = context->snaps_in_use.lookup_or_create(roll_back_to,
                                                        roll_back_to);

        std::cout << "rollback oid " << oid << " to " << roll_back_to << std::
            endl;
        ...
        if (existed_before) {
            ...
            comps[0] =
                context->rados.aio_create_completion((void *)cb_arg,
                                                     &write_callback);
            context->io_ctx.aio_operate(context->prefix + oid, comps[0],
                                        &zero_write_op1);
        }
        {
            std::pair < TestOp *, TestOp::CallbackInfo * >*cb_arg =
        ...
        if (existed_after) {
            ...
            comps[2] =
                context->rados.aio_create_completion((void *)cb_arg,
                                                     &write_callback);
            context->io_ctx.aio_operate(context->prefix + oid, comps[2],
                                        &zero_write_op2);
        }
    }
...
class CopyFromOp:public TestOp {
  public:
    ...
    CopyFromOp(int n,
               RadosTestContext * context,
               const std::string & oid,
               const std::string & oid_src, TestOpStat * stat)
    :TestOp(n, context, stat),
        oid(oid), oid_src(oid_src),
        comp(NULL), snap(-1), done(0), version(0), r(0) {
    } void _begin() override {
        ContDesc cont;
        {
            std::lock_guard l {
        ...
        std::pair < TestOp *, TestOp::CallbackInfo * >*read_cb_arg =
            new std::pair < TestOp *, TestOp::CallbackInfo * >(this,
                                                               ...
                                                               CallbackInfo(1));
        comp_racing_read =
            context->rados.aio_create_completion((void *)read_cb_arg,
                                                 &write_callback);
        rd_op.stat(NULL, NULL, NULL);
        context->io_ctx.aio_operate(context->prefix + oid, comp_racing_read, &rd_op, librados::OPERATION_ORDER_READS_WRITES,    // order wrt previous write/update
                                    NULL);
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (info->id == 0) {
            ...
            std::cout << num << ":  finishing copy_from to " << context->
                prefix + oid << std::endl;
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    std::
                        cout << num << ":  got expected ENOENT (src dne)" <<
                        std::endl;
                }
                else {
                    std::
                        cerr << "Error: oid " << oid << " copy_from " << oid_src
            ...
            std::
                cout << num << ":  finishing copy_from racing read to " <<
                context->prefix + oid << std::endl;
            if ((r = comp_racing_read->get_return_value())) {
                ...
                ceph_assert(!version
                            || comp_racing_read->get_version64() == version);
...
class ChunkReadOp:public TestOp {
  ...
  :    
    ...
    retvals(2),
    waiting_on(0),
    checksums(2), checksum_retvals(2), tgt_pool_name(tgt_pool_name) {
    } void _do_read(librados::ObjectReadOperation & read_op, uint32_t offset,
                    uint32_t length, int index) {
        read_op.read(offset, length, &results[index], &retvals[index]);
        if (index != 0) {
            bufferlist init_value_bl;
            encode(static_cast < uint32_t > (-1), init_value_bl);
            read_op.checksum(LIBRADOS_CHECKSUM_TYPE_CRC32C, init_value_bl,
                             offset, length, 0, &checksums[index],
                             &checksum_retvals[index]);
        }
    ...
    void _begin() override {
        context->state_lock.lock();
        std::
            cout << num << ": chunk read oid " << oid << " snap " << snap <<
            std::endl;
        done = 0;
        ...
        std::cout << num << ": ori offset " << iter->
            first << " req offset " << offset << " ori length " << iter->second.
            length << " req length " << length << " ori tgt_offset " << iter->
            second.
            offset << " req tgt_offset " << tgt_offset << " tgt_oid " << tgt_oid
            << std::endl;
        ...
        ceph_assert(!context->io_ctx.
                    aio_operate(context->prefix + tgt_oid, completions[1], &op,
                                flags, NULL));
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (!retval) {
            ...
            if (tgt_value.has_contents()) {
                ...
                if (checksum_retvals[1] != 0) {
                    std::
                        cerr << num << ": oid " << oid << " checksum retvals "
                        << checksums[0]
                        << " error " << std::endl;
                    context->errors++;
                }
...
class CopyOp:public TestOp {
  ...
  :    
    ...
    comp(NULL), done(0), r(0) {
        ...
        std::
            cout << "copy op oid " << oid_src << " to " << oid <<
        ...
        if (tgt_pool_name == context->low_tier_pool_name) {
            context->low_tier_io_ctx.aio_operate(context->prefix + oid, comp,
                                                 &op);
        }
        else {
            context->io_ctx.aio_operate(context->prefix + oid, comp, &op);
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (info->id == 0) {
            ceph_assert(comp->is_complete());
            std::cout << num << ":  finishing copy op to oid " << oid << std::
                endl;
            if ((r = comp->get_return_value())) {
                std::
                    cerr << "Error: oid " << oid <<
                    " write returned error code " << r << std::endl;
                ceph_abort();
            }
        }
...
class SetChunkOp:public TestOp {
  public:
    ...
    SetChunkOp(int n,
               RadosTestContext * context,
               const std::string & oid,
               const std::string & oid_tgt, TestOpStat * stat = 0)
  :    
    TestOp(n, context, stat),
    oid(oid), oid_tgt(oid_tgt),
    comp(NULL), done(0), r(0), offset(0), length(0), tgt_offset(0), snap(0) {
    ...
    void _begin() override {
        ...
        if (src_value.deleted()) {
            ...
            max_len =
                src_value.most_recent_gen()->get_length(src_value.
                                                        most_recent());
        }
        ...
        if (!src_value.deleted() && oid_tgt.empty()) {
            bufferlist bl;
            int r =
                context->io_ctx.read(context->prefix + oid, bl, length, offset);
            ceph_assert(r > 0);
            std::string fp_oid =
        ...
        std::
            cout << num << ": " << "set_chunk oid " << oid << " offset: " <<
            offset << " length: " << length << " target oid " << target_oid <<
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (info->id == 0) {
            ceph_assert(comp->is_complete());
            std::cout << num << ":  finishing set_chunk to oid " << oid << std::
                endl;
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    std::
                        ...
                        std::endl;
                         ...
                         && context->oid_set_chunk_tgt_pool.find(oid_tgt) !=
                         context->oid_set_chunk_tgt_pool.end()) {
                    std::
                        cout << num << ": get expected ENOENT tgt oid " <<
                        oid_tgt << std::endl;
                }
                else if (r == -ERANGE && src_value.deleted()) {
                    std::
                        cout << num << ":  got expected ERANGE (src dne)" <<
                        std::endl;
                }
                else if (r == -EOPNOTSUPP) {
                    std::
                        cout << "Range is overlapped: oid " << oid <<
                        " set_chunk " << oid_tgt << " returned error code " << r
                    ...
                    std::
                        cerr << "Error: oid " << oid << " set_chunk " << oid_tgt
                        << " returned error code " << r << std::endl;
                    ceph_abort();
                }
            }
  ...
  :    
    ...
    comp(NULL), done(0), r(0) {
        ...
        if (!context->redirect_objs[oid].empty()) {
            ...
            op.copy_from(src.c_str(), context->io_ctx, src_value.version, 0);
            context->low_tier_io_ctx.aio_operate(context->prefix + oid_tgt,
                                                 comp, &op,
                                                 librados::
            ...
            if ((r = comp->get_return_value())) {
                std::
                    cerr << "Error: oid " << oid << " copy_from " << oid_tgt <<
                    " returned error code " << r << std::endl;
                ceph_abort();
            }
            comp->release();
            ...
            context->oid_redirect_not_in_use.insert(context->
                                                    redirect_objs[oid]);
        ...
        context->io_ctx.aio_operate(context->prefix + oid, comp, &rd_op,
                                    librados::OPERATION_ORDER_READS_WRITES |
                                    librados::OPERATION_IGNORE_REDIRECT, NULL);
        comp->wait_for_complete();
        ...
        comp = context->rados.aio_create_completion();
        rd_op.stat(NULL, NULL, NULL);
        context->low_tier_io_ctx.aio_operate(context->prefix + oid_tgt, comp,
                                             &rd_op,
                                             librados::
                                             OPERATION_ORDER_READS_WRITES |
                                             librados::
                                             OPERATION_IGNORE_REDIRECT, NULL);
        comp->wait_for_complete();
        if ((r = comp->get_return_value())) {
            std::
        ...
        if (src_value.version != 0 && !src_value.deleted())
            op.assert_version(src_value.version);
        op.set_redirect(context->prefix + oid_tgt, context->low_tier_io_ctx,
                        tgt_version);
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (info->id == 0) {
            ceph_assert(comp->is_complete());
            std::
                cout << num << ":  finishing set_redirect to oid " << oid <<
                std::endl;
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    std::
                    ...
                    std::
                        cerr << "Error: oid " << oid << " set_redirect " <<
                        oid_tgt << " returned error code " << r << std::endl;
                    ceph_abort();
                }
            }
            else {
...
class TierFlushOp:public TestOp {
  public:
    ...
    :TestOp(n, context, stat), completion(NULL), oid(oid), snap(-1) {
        ...
        std::
            cout << num << ": tier_flush oid " << oid << " snap " << snap <<
            std::endl;
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (r == 0) {
            ...
            if (src_value.deleted()) {
                std::cout << num << ":  got expected ENOENT (src dne)" << std::
                    endl;
            }
            else {
            ...
            if (r != -ENOENT && src_value.deleted()) {
                std::cerr << num << ": src dne, but r is not ENOENT" << std::
                    endl;
            }
            ceph_abort_msg("shouldn't happen");
        }
...
class TierEvictOp:public TestOp {
  public:
    ...
    :TestOp(n, context, stat), completion(NULL), oid(oid), snap(-1) {
        ...
        std::
            cout << num << ": tier_evict oid " << oid << " snap " << snap <<
            std::endl;
...
class HitSetListOp:public TestOp {
  ...
  public:
    HitSetListOp(int n,
                 RadosTestContext * context,
                 uint32_t hash, TestOpStat * stat = 0)
    ...
    void _finish(CallbackInfo * info) override {
        ...
        if (!comp2) {
            ...
            if (r == 0) {
                ...
                auto p = bl.cbegin();
                decode(hitset, p);
                std::cout << num << ": got hitset of type " << hitset.
                    get_type_name()
                    << " size " << bl.length()
                    << std::endl;
            }
...
class IsDirtyOp:public TestOp {
  ...
  :    TestOp(n, context, stat), completion(NULL), oid(oid), dirty(false) {
    ...
    void _finish(CallbackInfo * info) override {
        ...
        int r = completion->get_return_value();
        if (r == 0) {
            std::cout << num << ":  " << (dirty ? "dirty" : "clean") << std::
                endl;
...
class CacheFlushOp:public TestOp {
  public:
    ...
    CacheFlushOp(int n,
                 RadosTestContext * context,
                 const std::string & oid, TestOpStat * stat, bool b)
    :TestOp(n, context, stat),
        completion(NULL), oid(oid), blocking(b), snap(0), can_fail(false) {
    } void _begin() override {
        context->state_lock.lock();
...
class CacheEvictOp:public TestOp {
  public:
    ...
    :TestOp(n, context, stat), completion(NULL), oid(oid) {
        ...
        std::
            cout << num << ": cache_evict oid " << oid << " snap " << snap <<
            std::endl;
