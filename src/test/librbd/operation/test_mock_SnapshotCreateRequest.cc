...
public:
    ...
    static SetImageStateRequest *create(MockImageCtx *image_ctx,
                                        uint64_t snap_id,
                                        Context *on_finish)
    {
...
SetImageStateRequest < MockImageCtx > *SetImageStateRequest <
...
{
public:
    typedef SnapshotCreateRequest < MockImageCtx >
    ...
    {
        EXPECT_CALL(*mock_image_ctx.image_watcher,
                    notify_quiesce(_, _, _))
        .WillOnce(WithArg < 2 >
                  (CompleteContext
    ...
    } void expect_allocate_snap_id(MockImageCtx &mock_image_ctx,
                                   int r)
    {
        auto &expect =
    ...
    void expect_set_image_state(MockImageCtx &mock_image_ctx,
                                MockSetImageStateRequest &
                                mock_set_image_state_request, int r)
    {
    ...
    void expect_update_snap_context(MockImageCtx &mock_image_ctx)
    {
        ...
        WillOnce(Return
                 (static_cast <
                  const librbd::SnapInfo * >(NULL)));
        EXPECT_CALL(mock_image_ctx,
                    add_snap(_, "snap1", _, _, _, _, _, _));
    }
...
TEST_F(TestMockOperationSnapshotCreateRequest, Success)
{
    ...
    MockSnapshotCreateRequest *req =
        new MockSnapshotCreateRequest(mock_image_ctx, &cond_ctx,
...
TEST_F(TestMockOperationSnapshotCreateRequest, SetImageState)
{
    ...
    expect_set_image_state(mock_image_ctx, mock_set_image_state_request,
                           0);
    expect_update_snap_context(mock_image_ctx);
    EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
    expect_unblock_writes(mock_image_ctx);
