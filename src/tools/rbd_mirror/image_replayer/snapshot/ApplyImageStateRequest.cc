            ...
            namespace snapshot {
                ...
                using librbd::util::create_rados_callback;
                 ...
                 template < typename I >
                    ...
                    I >::ApplyImageStateRequest(const std::
                                                string & local_mirror_uuid,
                                                const std::
                                                string & remote_mirror_uuid,
                                                I * local_image_ctx,
                                                I * remote_image_ctx,
                                                librbd::mirror::snapshot::
                ...
                :m_local_mirror_uuid(local_mirror_uuid),
                    ...
                    dout(15) << "local_image_name=" << m_local_image_ctx->
                        name << ", " << "remote_image_name=" << m_image_state.
                        name << dendl;

                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest < I >::handle_rename_image >
                        (this);
                    m_local_image_ctx->operations->execute_rename(m_image_state.
                                                                  name, ctx);
                }

                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_rename_image(int r) {
                    dout(15) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to rename image to '" << m_image_state.
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest < I >::handle_update_features >
                        (this);
                    m_local_image_ctx->operations->
                        execute_update_features(feature_updates, enabled, ctx,
                                                0U);
                }

                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_update_features(int r) {
                    dout(15) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to update image features: " <<
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest < I >::handle_get_image_meta >
                        (this);
                    auto req =
                        librbd::image::GetMetadataRequest <
                        I >::create(m_local_image_ctx->md_ctx,
                                    m_local_image_ctx->header_oid, true, "", "",
                                    0U,
                                    &m_metadata, ctx);
                    req->send();
                }
                ...
                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_get_image_meta(int r) {
                    dout(15) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to fetch local image metadata: " <<
                            cpp_strerror(r)
                            << dendl;
                        finish(r);
                        return;
                ...
                template < typename I >
                  ...
                  for (const auto &[key, value]:m_metadata) {
                        if (m_image_state.metadata.count(key) == 0) {
                            dout(15) << "removing image-meta key '" << key <<
                                "'" << dendl;
                  ...
                  for (const auto &[key, value]:m_image_state.
                         metadata)
                    {
                        auto it = m_metadata.find(key);
                        if (it == m_metadata.end()
                            || !it->second.contents_equal(value)) {
                            dout(15) << "updating image-meta key '" << key <<
                                "'" << dendl;
                  ...
                  for (const auto & key:keys_to_remove) {
                    ...
                    if (!metadata_to_update.empty()) {
                        librbd::cls_client::metadata_set(&op,
                                                         metadata_to_update);
                    }
                    ...
                    auto aio_comp = create_rados_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest <
                        I >::handle_update_image_meta > (this);
                    int r =
                        m_local_image_ctx->md_ctx.
                        aio_operate(m_local_image_ctx->header_oid, aio_comp,
                                    &op);
                    ceph_assert(r == 0);
                    aio_comp->release();
                }
                ...
                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_update_image_meta(int r) {
                    dout(15) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to update image metadata: " <<
                            cpp_strerror(r) << dendl;
                        finish(r);
                        return;
                    }
                ...
                template < typename I >
                    ...
                    if (m_prev_snap_id != CEPH_NOSNAP) {
                        snap_it =
                            m_local_image_ctx->snap_info.
                            upper_bound(m_prev_snap_id);
                    }

                    for (; snap_it != m_local_image_ctx->snap_info.end();
                         ++snap_it) {
                        auto snap_id = snap_it->first;
                        const auto & snap_info = snap_it->second;

                        auto user_ns =
                            std::get_if < cls::rbd::UserSnapshotNamespace >
                            (&snap_info.snap_namespace);
                        if (user_ns == nullptr) {
                            dout(20) << "snapshot " << snap_id <<
                                " is not a user snapshot" << dendl;
                            continue;
                        }

                        if (snap_info.protection_status ==
                            RBD_PROTECTION_STATUS_UNPROTECTED) {
                            dout(20) << "snapshot " << snap_id <<
                                " is already unprotected" << dendl;
                            continue;
                        }

                        auto snap_id_map_it =
                            m_local_to_remote_snap_ids.find(snap_id);
                        if (snap_id_map_it == m_local_to_remote_snap_ids.end()) {
                            dout(15) << "snapshot " << snap_id <<
                                " does not exist in remote image" << dendl;
                            break;
                        }

                        auto remote_snap_id = snap_id_map_it->second;
                        auto snap_state_it =
                            m_image_state.snapshots.find(remote_snap_id);
                        if (snap_state_it == m_image_state.snapshots.end()) {
                            dout(15) << "snapshot " << snap_id <<
                                " does not exist in remote image " << "state" <<
                                dendl;
                            break;
                        }

                        const auto & snap_state = snap_state_it->second;
                        if (snap_state.protection_status ==
                            RBD_PROTECTION_STATUS_UNPROTECTED) {
                            dout(15) << "snapshot " << snap_id <<
                                " is unprotected in remote image" << dendl;
                            break;
                        }
                    }
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest <
                        I >::handle_unprotect_snapshot > (this);
                    m_local_image_ctx->operations->
                        execute_snap_unprotect(cls::rbd::UserSnapshotNamespace {
                                               }
                                               , m_snap_name.c_str(), ctx);
                }

                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_unprotect_snapshot(int r) {
                    dout(15) << "r=" << r << dendl;
                ...
                template < typename I >
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest < I >::handle_remove_snapshot >
                        (this);
                    m_local_image_ctx->operations->
                ...
                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_remove_snapshot(int r) {
                    dout(15) << "r=" << r << dendl;
                ...
                template < typename I >
                    ...
                    for (; snap_it != m_local_image_ctx->snap_info.end();
                        ...
                        if (snap_state.protection_status ==
                            RBD_PROTECTION_STATUS_PROTECTED) {
                            dout(15) << "snapshot " << snap_id <<
                                " is protected in remote image" << dendl;
                            break;
                        }
                    }
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest < I >::handle_protect_snapshot >
                        (this);
                    m_local_image_ctx->operations->
                ...
                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_protect_snapshot(int r) {
                    dout(15) << "r=" << r << dendl;
                ...
                template < typename I >
                    ...
                    for (; snap_it != m_local_image_ctx->snap_info.end();
                        ...
                        if (snap_info.name != snap_state.name) {
                            dout(15) << "snapshot " << snap_id <<
                                " has been renamed from '" << snap_info.
                                name << "' to '" << snap_state.
                                name << "'" << dendl;
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest < I >::handle_rename_snapshot >
                        (this);
                    m_local_image_ctx->operations->
                        execute_snap_rename(m_prev_snap_id, m_snap_name.c_str(),
                                            ctx);
                }

                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_rename_snapshot(int r) {
                    dout(15) << "r=" << r << dendl;
                ...
                template < typename I >
                    ...
                    dout(15) << "snap_limit=" << m_image_state.
                        snap_limit << dendl;
                    ...
                    auto ctx = create_context_callback <
                        ApplyImageStateRequest < I >,
                        &ApplyImageStateRequest <
                        I >::handle_set_snapshot_limit > (this);
                    m_local_image_ctx->operations->
                        execute_snap_set_limit(m_image_state.snap_limit, ctx);
                }

                template < typename I >
                    void ApplyImageStateRequest <
                    I >::handle_set_snapshot_limit(int r) {
                    dout(15) << "r=" << r << dendl;

                    if (r < 0) {
                        derr << "failed to update snapshot limit: " <<
                            cpp_strerror(r)
                            << dendl;
                    }
                ...
                template < typename I >
                    uint64_t ApplyImageStateRequest <
                    I >::compute_remote_snap_id(uint64_t local_snap_id) {
                    ceph_assert(ceph_mutex_is_locked
                                (m_local_image_ctx->image_lock));
                    ceph_assert(ceph_mutex_is_locked
                                (m_remote_image_ctx->image_lock));
                    ...
                    auto remote_snap_id =
                        util::compute_remote_snap_id(m_local_image_ctx->
                                                     image_lock,
                                                     m_local_image_ctx->
                                                     snap_info,
                                                     local_snap_id,
                    ...
                    for (auto snap_it = m_remote_image_ctx->snap_info.begin();
                         snap_it != m_remote_image_ctx->snap_info.end();
                         ++snap_it) {
                        auto snap_id = snap_it->first;
                        auto mirror_ns =
                        ...
                        if (mirror_ns->primary_mirror_uuid !=
                            m_local_mirror_uuid) {
                            dout(20) << "remote snapshot " << snap_id <<
                                " not tied to local" << dendl;
                            continue;
                        }
                        else if (mirror_ns->primary_snap_id == local_snap_id) {
                            dout(15) << "local snapshot " << local_snap_id <<
                                " maps to " << "remote snapshot " << snap_id <<
                                dendl;
                            return snap_id;
                        }
                      ...
                      for (auto[local_snap_id_seq, remote_snap_id_seq]:snap_seqs)
                        {
                            if (local_snap_id_seq == local_snap_id) {
                                dout(15) << "local snapshot " << local_snap_id
                                    << " maps to " << "remote snapshot " <<
                                    remote_snap_id_seq << dendl;
                ...
                template < typename I >
                    void ApplyImageStateRequest <
                    I >::compute_local_to_remote_snap_ids() {
                    ceph_assert(ceph_mutex_is_locked
                                (m_local_image_ctx->image_lock));
                    std::shared_lock remote_image_locker {
                    m_remote_image_ctx->image_lock};

                  for (const auto &[snap_id, snap_info]:m_local_image_ctx->
                         snap_info)
                    {
                        m_local_to_remote_snap_ids[snap_id] =
                            compute_remote_snap_id(snap_id);
                    }

                    dout(15) << "local_to_remote_snap_ids=" <<
                        m_local_to_remote_snap_ids << dendl;
                }
