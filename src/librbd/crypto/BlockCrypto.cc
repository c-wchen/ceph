...
namespace librbd {
    namespace crypto {
        ...
        template < typename T >
            BlockCrypto < T >::BlockCrypto(CephContext * cct,
        ...
        :m_cct(cct), m_data_cryptor(data_cryptor), m_block_size(block_size),
            ...
            int BlockCrypto < T >::crypt(ceph::bufferlist * data,
            ...
            auto sg = make_scope_guard([&]{
                                       m_data_cryptor->return_context(ctx,
                                                                      mode);
                                       }
            );
            ...
            unsigned char *leftover_block =
                (unsigned char *)alloca(m_block_size);
            uint32_t leftover_size = 0;
            for (auto buf = src.buffers().begin(); buf != src.buffers().end();
                 ++buf) {
                auto in_buf_ptr =
                ...
                while (remaining_buf_bytes > 0) {
                    if (leftover_size == 0) {
                        ...
                        auto r =
                            m_data_cryptor->init_context(ctx, iv, m_iv_size);
                        if (r != 0) {
                            lderr(m_cct) << "unable to init cipher's IV" <<
                                dendl;
                            return r;
                        }
                    ...
                    if (leftover_size > 0 || remaining_buf_bytes < m_block_size) {
                        auto copy_size = std::min((uint32_t) m_block_size -
                                                  leftover_size,
                                                  remaining_buf_bytes);
                        memcpy(leftover_block + leftover_size, in_buf_ptr,
                               copy_size);
                    ...
                    if (leftover_size == 0) {
                        ...
                        crypto_output_length =
                            m_data_cryptor->update_context(ctx, leftover_block,
                                                           out_buf_ptr,
                                                           m_block_size);
                        leftover_size = 0;
                    }
        ...
        template < typename T >
            int BlockCrypto < T >::encrypt(ceph::bufferlist * data,
                                           uint64_t image_offset) {
            return crypt(data, image_offset, CipherMode::CIPHER_MODE_ENC);
        }

        template < typename T >
            int BlockCrypto < T >::decrypt(ceph::bufferlist * data,
                                           uint64_t image_offset) {
