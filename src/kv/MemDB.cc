...
void MemDB::_save()
{
    ...
    dout(10) << __func__ << " Saving MemDB to file: " << _get_data_fn().
        c_str() << dendl;
    int mode = 0644;
    ...
    if (fd < 0) {
        int err = errno;
        cerr << "write_file(" << _get_data_fn().
            c_str() << "): failed to open file: " << cpp_strerror(err) << std::
            endl;
        return;
    }
    bufferlist bl;
...
{
    std::lock_guard < std::mutex > l(m_lock);
    dout(10) << __func__ << " Reading MemDB from file: " << _get_data_fn().
        c_str() << dendl;
    /*
    ...
    int fd =
        TEMP_FAILURE_RETRY(::
                           open(_get_data_fn().c_str(), O_RDONLY | O_CLOEXEC));
    if (fd < 0) {
        int err = errno;
...
int MemDB::_init(bool create)
{
    ...
    if (create) {
        ...
        if (r < 0) {
            r = -errno;
            if (r != -EEXIST) {
                derr << __func__ << " mkdir failed: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
...
int MemDB::set_merge_operator(const string & prefix,
...
void MemDB::MDBTransactionImpl::set(const string & prefix, const string & k,
...
void MemDB::MDBTransactionImpl::rm_range_keys(const string & prefix,
                                              const string & start,
                                              const string & end)
{
    KeyValueDB::Iterator it = m_db->get_iterator(prefix);
...
void MemDB::MDBTransactionImpl::merge(const std::string & prefix,
                                      ...
                                      const bufferlist & value)
{
    ...
    ops.push_back(make_pair
                  (MERGE, make_pair(std::make_pair(prefix, key), value)));
    return;
}
...
int MemDB::_merge(ms_op_t & op)
{
    ...
    if (_get(op.first.first, op.first.second, &bl_old) == false) {
         ...
         */
        std::string new_val;
        mop->merge(bl_old.c_str(), bl_old.length(), bl.c_str(), bl.length(),
                   &new_val);
        m_map[key] = bufferptr(new_val.c_str(), new_val.length());
...
bool MemDB::_get_locked(const string & prefix, const string & k,
                        bufferlist * out)
{
    std::lock_guard < std::mutex > l(m_lock);
...
bool MemDB::MDBWholeSpaceIteratorImpl::
