...
int validate_features(CephContext *cct, uint64_t features)
{
    ...
    if ((features & RBD_FEATURES_INTERNAL) != 0) {
        lderr(cct) << "cannot use internally controlled features" <<
                   dendl;
        return -EINVAL;
    }
    ...
    if ((features & RBD_FEATURE_JOURNALING) != 0 &&
        (features & RBD_FEATURE_EXCLUSIVE_LOCK) == 0) {
        lderr(cct) << "cannot use journaling without exclusive lock"
                   << dendl;
        return -EINVAL;
    }
                      ...
                      uint64_t stripe_unit, uint64_t stripe_count)
{
    if ((stripe_unit && !stripe_count)
        ...
        lderr(cct) << "stripe unit must be at least 512 bytes" <<
                   dendl;
        return -EINVAL;
    }
    return 0;
...
template < typename I >
CreateRequest < I >::CreateRequest(const ConfigProxy &config,
                                   IoCtx &ioctx,
         ...
         m_op_work_queue(op_work_queue), m_on_finish(on_finish)
{
    ...
    if (image_options.get(RBD_IMAGE_OPTION_FEATURES, &m_features) != 0) {
        m_features =
            librbd::rbd_features_from_string(m_config.get_val <
                                             std::string >
    ...
    if (image_options.
        ...
        || m_stripe_unit == 0) {
        m_stripe_unit =
            m_config.get_val < Option::size_t >
    ...
    if (image_options.
        ...
        || m_stripe_count == 0) {
        m_stripe_count =
            m_config.get_val < uint64_t > ("rbd_default_stripe_count");
    }
    if (get_image_option
    ...
    if (get_image_option(image_options, RBD_IMAGE_OPTION_JOURNAL_ORDER,
                         &m_journal_order) != 0) {
        m_journal_order =
            m_config.get_val < uint64_t > ("rbd_journal_order");
    }
    if (get_image_option
    ...
    if (image_options.
        ...
        m_journal_pool =
            m_config.get_val < std::string > ("rbd_journal_pool");
    }
    if (image_options.get(RBD_IMAGE_OPTION_DATA_POOL, &m_data_pool) !=
        0) {
        m_data_pool =
            m_config.get_val < std::string > ("rbd_default_data_pool");
    }
...
template < typename I > void CreateRequest < I >::validate_data_pool()
{
    ...
    auto ctx = create_context_callback <
               CreateRequest < I >,
               &CreateRequest < I >::handle_validate_data_pool > (this);
...
void CreateRequest < I >::handle_validate_data_pool(int r)
{
    ...
    if (r == -EINVAL) {
        ...
        lderr(m_cct) << "failed to validate pool: " << cpp_strerror(r)
                     << dendl;
        complete(r);
        return;
    }
...
void CreateRequest < I >::add_image_to_directory()
{
    ...
    if (!m_io_ctx.get_namespace().empty()) {
        cls_client::dir_state_assert(&op,
...
template < typename I > void CreateRequest < I >::journal_create()
{
    ...
    tag_data.mirror_uuid =
        (use_primary_mirror_uuid ? m_primary_mirror_uuid : librbd::
         Journal < I >::LOCAL_MIRROR_UUID);

    typename journal::TypeTraits < I >::ContextWQ *context_wq;
    Journal <>::get_work_queue(m_cct, &context_wq);

    auto req =
        librbd::journal::CreateRequest < I >::create(m_io_ctx,
            m_image_id,
            m_journal_order,
            m_journal_splay_width,
            m_journal_pool,
...
template < typename I > void CreateRequest < I >::mirror_image_enable()
{
    ...
    auto req =
        mirror::EnableRequest < I >::create(m_io_ctx, m_image_id,
                                            m_mirror_image_mode,
...
template < typename I > void CreateRequest < I >::journal_remove()
{
    ...
    librbd::journal::RemoveRequest < I > *req =
        librbd::journal::RemoveRequest < I >::create(m_io_ctx,
            m_image_id,
            librbd::Journal <
            I >::
            IMAGE_CLIENT_ID,
