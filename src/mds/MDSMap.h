...
public:
    ...
    uint64_t get_max_xattr_size() const
    {
        return max_xattr_size;
    }
    void set_max_xattr_size(uint64_t m)
    {
        max_xattr_size = m;
    }
    ...
    void clear_balance_automate()
    {
        clear_flag(CEPH_MDSMAP_BALANCE_AUTOMATE);
    }
    bool allows_balance_automate() const
    {
        return test_flag(CEPH_MDSMAP_BALANCE_AUTOMATE);
    }
    bool was_balance_automate_ever_allowed() const
    {
    ...
    bool update_quiesce_db_cluster(mds_gid_t const& leader,
