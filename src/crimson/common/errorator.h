...
struct unthrowable_wrapper : error_t < unthrowable_wrapper < ErrorT, ErrorV>>
{
    ...
    public:
        template < std::size_t N > assert_failure(const char (&msg)[N])
            : msg(msg)
        {
        }
        assert_failure() = default;
        template < typename Func > assert_failure(Func&& f)
...
static inline typename errorator < AllowedErrors... >::template future < > parallel_for_each(Iterator first,
        Iterator last, Func&& func) noexcept;
