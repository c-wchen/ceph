...
class CollectionIndex {
   ...
   */
    class Path {
      public:
        ...
        /// Normal Constructor
         Path(string path,      ///< [in] Path to return.
              CollectionIndex * ref)
        :full_path(path), parent_ref(ref), parent_coll(parent_ref->coll()) {
        }
        /// Debugging Constructor Path(
        ...
        /// Getter for collection const coll_t & coll() const {
            return parent_coll;
        }
        /// Getter for parent CollectionIndex *get_index() const {
            return parent_ref;
    }};
  public:
   ...
   * @return Error Code, 0 for success
   */
    virtual int created(const ghobject_t & oid, ///< [in] Created object.
   ...
   * @return Error Code, 0 for success
   */
    virtual int unlink(const ghobject_t & oid   ///< [in] Object to remove
        ) = 0;
   ...
   * @return Error Code, 0 for success
   */
    virtual int lookup(const ghobject_t & oid,  ///< [in] Object to lookup
   ...
   */
    ...
    virtual int collection_list_partial(const ghobject_t & start,   ///< [in] object at which to start
  ...
  :    cct(cct), access_lock("CollectionIndex::access_lock", true, false) {
     ...
     */
    virtual int pre_hash_collection(uint32_t pg_num,    ///< [in] pg number of the pool this collection belongs to
