...
void LogSegment::try_to_expire(MDSRank * mds, MDSGatherBuilder & gather_bld,
                               int op_prio)
{
    set < CDir * >commit;
    ...
    for (elist < CDentry * >::iterator p = dirty_dentries.begin(); !p.end();
         ++p) {
    ...
    if (!commit.empty()) {
        for (set < CDir * >::iterator p = commit.begin();
            ...
            if (dir->can_auth_pin()) {
                ...
                dout(15) << "try_to_expire waiting for unfreeze on " << *dir <<
                    dendl;
    ...
    for (set < metareqid_t >::iterator p = uncommitted_masters.begin();
         p != uncommitted_masters.end(); ++p) {
        dout(10) << "try_to_expire waiting for slaves to ack commit on " << *p
            << dendl;
    ...
    for (elist < CInode * >::iterator p = dirty_dirfrag_dir.begin(); !p.end();
         ++p) {
        CInode *in = *p;
        dout(10) << "try_to_expire waiting for dirlock flush on " << *in <<
            dendl;
        mds->locker->scatter_nudge(&in->filelock, gather_bld.new_sub());
    }
    for (elist < CInode * >::iterator p = dirty_dirfrag_dirfragtree.begin();
         !p.end(); ++p) {
        CInode *in = *p;
    ...
    if (!open_files.empty()) {
        ...
        while (!p.end()) {
            ...
            if (in->last == CEPH_NOSNAP && in->is_auth() &&
                ...
                if (in->is_any_caps_wanted()) {
                    dout(20) << "try_to_expire requeueing open file " << *in <<
                        dendl;
                    if (!le) {
                    ...
                    dout(20) <<
                        "try_to_expire not requeueing and delisting unwanted file "
                        << *in << dendl;
                ...
                dout(20) << "try_to_expire requeueing snap needflush inode " <<
                    *in << dendl;
                if (!le) {
                    le = new EOpen(mds->mdlog);
                    mds->mdlog->start_entry(le);
                 ...
                 */
                dout(20) <<
                    "try_to_expire not requeueing and delisting capless file "
                    << *in << dendl;
                in->item_open_file.remove_myself();
            }
        }
        if (le) {
            ...
            dout(10) << "try_to_expire waiting for open files to rejournal" <<
                dendl;
        }
    }
    ...
    for (elist < CInode * >::iterator p = dirty_parent_inodes.begin(); !p.end();
        ...
        if (in->can_auth_pin()) {
            ...
            dout(15) << "try_to_expire waiting for unfreeze on " << *in <<
                dendl;
    ...
    // slave updates
    for (elist < MDSlaveUpdate * >::iterator p =
         slave_updates.begin(member_offset(MDSlaveUpdate, item)); !p.end();
         ++p) {
        MDSlaveUpdate *su = *p;
    ...
    for (map < int, ceph::unordered_set < version_t > >::iterator p =
         pending_commit_tids.begin(); p != pending_commit_tids.end(); ++p) {
        MDSTableClient *client = mds->get_table_client(p->first);
        assert(client);
        for (ceph::unordered_set < version_t >::iterator q = p->second.begin();
             q != p->second.end(); ++q) {
            dout(10) << "try_to_expire " << get_mdstable_name(p->
                                                              first) <<
                " transaction " << *q <<
    ...
    for (map < int, version_t >::iterator p = tablev.begin();
        ...
        if (p->second > server->get_committed_version()) {
            dout(10) << "try_to_expire waiting for " << get_mdstable_name(p->
                                                                          first)
...
void EMetaBlob::add_dir_context(CDir * dir, int mode)
{
    ...
    while (true) {
        ...
        if (mode == TO_AUTH_SUBTREE_ROOT) {
            ...
            if (dir->is_subtree_root()) {
                ...
                if (dir->get_dir_auth().first == mds->get_nodeid()) {
                    mds_authority_t parent_auth =
                        parent ? parent->authority() : CDIR_AUTH_UNDEF;
                    if (parent_auth.first == dir->get_dir_auth().first) {
                        if (parent_auth.second == CDIR_AUTH_UNKNOWN &&
                            ...
                            dout(0) <<
                                "EMetaBlob::add_dir_context unexpected subtree "
                                << *dir << dendl;
                            assert(0);
                        }
            ...
            if (!maybenot && last_subtree_map
                && diri->last_journaled >= last_subtree_map) {
                dout(20) << "EMetaBlob::add_dir_context(" << dir <<
                    ") already have diri in this segment (" << diri->
                    last_journaled << " >= " << last_subtree_map <<
    ...
    for (list < CDentry * >::iterator p = parents.begin(); p != parents.end();
         ++p) {
...
void EMetaBlob::fullbit::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(8, 5, bl);
    ::encode(dn, bl);
    ::encode(dnfirst, bl);
...
void EMetaBlob::fullbit::dump(Formatter * f) const const
{
...
void EMetaBlob::fullbit::generate_test_instances(list <
                                                 EMetaBlob::fullbit * >&ls)
{
...
void EMetaBlob::fullbit::update_inode(MDSRank * mds, CInode * in)
{
    ...
    if (in->inode.is_dir()) {
        if (!(in->dirfragtree == dirfragtree)) {
            dout(10) << "EMetaBlob::fullbit::update_inode dft " << in->
                dirfragtree << " -> " << dirfragtree << " on " << *in << dendl;
            ...
            if (in->has_dirfrags() && in->authority() == CDIR_AUTH_UNDEF) {
                ...
                for (list < CDir * >::iterator p = ls.begin(); p != ls.end();
                    ...
                    if (dir->get_num_any() == 0 &&
                        ...
                        dout(10) << " closing empty non-auth dirfrag " << *dir
                            << dendl;
...
void EMetaBlob::remotebit::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(dn, bl);
    ::encode(dnfirst, bl);
...
void EMetaBlob::remotebit::dump(Formatter * f) const const
{
    f->dump_string("dentry", dn);
...
{
    remotebit *remote =
        new remotebit("/test/dn", 0, 10, 15, 1, IFTODT(S_IFREG), false);
    ls.push_back(remote);
}

// EMetaBlob::nullbit

void EMetaBlob::nullbit::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(dn, bl);
    ::encode(dnfirst, bl);
...
void EMetaBlob::nullbit::dump(Formatter * f) const const
{
    f->dump_string("dentry", dn);
    f->dump_int("snapid.first", dnfirst);
    f->dump_int("snapid.last", dnlast);
...
void EMetaBlob::dirlump::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(fnode, bl);
    ::encode(state, bl);
...
void EMetaBlob::dirlump::dump(Formatter * f) const const
{
    if (!dn_decoded) {
 ...
 * EMetaBlob proper
 */
void EMetaBlob::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(8, 5, bl);
...
void EMetaBlob::decode(bufferlist::iterator & bl)
{
    ...
    if (struct_v >= 2) {
        ...
        ::decode(r, bl);
        while (!r.empty()) {
            client_reqs.push_back(pair < metareqid_t,
                                  uint64_t > (r.front(), 0));
            r.pop_front();
        }
    }
 ...
 */
void EMetaBlob::get_inodes(std::set < inodeno_t > &inodes) const const
{
 ...
 */
void EMetaBlob::get_dentries(std::map < dirfrag_t,
                             std::set < std::string > >&dentries) const const
{
    for (std::map < dirfrag_t, dirlump >::const_iterator i = lump_map.begin();
         i != lump_map.end(); ++i) {
        dirlump const &dl = i->second;
 ...
 * subtree.
 */
void EMetaBlob::get_paths(std::vector < std::string > &paths) const const
{
    ...
    for (std::map < dirfrag_t, dirlump >::const_iterator i = lump_map.begin();
        ...
        for (list < ceph::shared_ptr < fullbit > >::const_iterator
            ...
            ino_locations[(*iter)->inode.ino] =
                Location(dir_ino, std::string(dentry));
        }
    ...
    for (std::map < dirfrag_t, dirlump >::const_iterator i = lump_map.begin();
        ...
        for (list < ceph::shared_ptr < fullbit > >::const_iterator
            ...
            if (children.find((*iter)->inode.ino) == children.end()) {
                leaf_locations.
                    push_back(Location(dir_ino, std::string(dentry)));
...
void EMetaBlob::dump(Formatter * f) const const
{
    ...
    for (list < ceph::shared_ptr < fullbit > >::const_iterator i =
         roots.begin(); i != roots.end(); ++i) {
    ...
    for (list < pair < __u8, version_t > >::const_iterator i =
         table_tids.begin(); i != table_tids.end(); ++i) {
    ...
    for (map < inodeno_t, uint64_t >::const_iterator i =
         truncate_finish.begin(); i != truncate_finish.end(); ++i) {
    ...
    for (list < pair < metareqid_t, uint64_t > >::const_iterator i =
         client_reqs.begin(); i != client_reqs.end(); ++i) {
...
void EMetaBlob::replay(MDSRank * mds, LogSegment * logseg,
                       MDSlaveUpdate * slaveup)
{
    dout(10) << "EMetaBlob.replay " << lump_map.
        size() << " dirlumps by " << client_name << dendl;

    assert(logseg);

    assert(g_conf->mds_kill_journal_replay_at != 1);

    for (list < ceph::shared_ptr < fullbit > >::iterator p = roots.begin();
         p != roots.end(); ++p) {
        ...
        dout(10) << "EMetaBlob.replay " << (isnew ? " added root " :
                                            " updated root ") << *in << dendl;
    }
    ...
    if (renamed_dirino) {
        ...
        if (renamed_diri)
            ...
            dout(10) << "EMetaBlob.replay don't have renamed ino " <<
                renamed_dirino << dendl;

        int nnull = 0;
        for (list < dirfrag_t >::iterator lp = lump_order.begin();
             lp != lump_order.end(); ++lp) {
            dirlump & lump = lump_map[*lp];
            if (lump.nnull) {
                dout(10) << "EMetaBlob.replay found null dentry in dir " << *lp
                    << dendl;
                nnull += lump.nnull;
            }
        }
    ...
    for (list < dirfrag_t >::iterator lp = lump_order.begin();
        ...
        if (!dir) {
            ...
            if (!diri) {
                if (MDS_INO_IS_MDSDIR(lp->ino)) {
                    ...
                    diri =
                        mds->mdcache->create_system_inode(lp->ino,
                                                          S_IFDIR | 0755);
                    diri->state_clear(CInode::STATE_AUTH);
                    dout(10) << "EMetaBlob.replay created base " << *diri <<
                        dendl;
                }
                else {
                    dout(0) << "EMetaBlob.replay missing dir ino  " << (*lp).
                        ino << dendl;
                    mds->clog->
        ...
        if (lump.is_dirty()) {
            ...
            if (!(dir->fnode.rstat == dir->fnode.accounted_rstat)) {
                dout(10) << "EMetaBlob.replay      dirty nestinfo on " << *dir
                    << dendl;
                mds->locker->mark_updated_scatterlock(&dir->inode->nestlock);
                logseg->dirty_dirfrag_nest.push_back(&dir->inode->
                                                     item_dirty_dirfrag_nest);
            }
            else {
                dout(10) << "EMetaBlob.replay      clean nestinfo on " << *dir
                    << dendl;
            }
            if (!(dir->fnode.fragstat == dir->fnode.accounted_fragstat)) {
                dout(10) << "EMetaBlob.replay      dirty fragstat on " << *dir
                    << dendl;
                mds->locker->mark_updated_scatterlock(&dir->inode->filelock);
                logseg->dirty_dirfrag_dir.push_back(&dir->inode->
                                                    item_dirty_dirfrag_dir);
            }
            else {
                dout(10) << "EMetaBlob.replay      clean fragstat on " << *dir
                    << dendl;
            }
        }
        if (lump.is_dirty_dft()) {
            ...
            logseg->dirty_dirfrag_dirfragtree.push_back(&dir->inode->
        ...
        for (list < ceph::shared_ptr < fullbit > >::const_iterator pp =
             lump.get_dfull().begin(); pp != lump.get_dfull().end(); ++pp) {
            ...
            if (!dn) {
                ...
                if (p->is_dirty())
                    dn->_mark_dirty(logseg);
                dout(10) << "EMetaBlob.replay for [" << p->dnfirst << "," << p->
                    dnlast << "] had " << *dn << dendl;
                dn->first = p->dnfirst;
            ...
            if (!in) {
                in = new CInode(mds->mdcache, dn->is_auth(), p->dnfirst,
                                p->dnlast);
                p->update_inode(mds, in);
                mds->mdcache->add_inode(in);
                if (!dn->get_linkage()->is_null()) {
                    if (dn->get_linkage()->is_primary()) {
                        ...
                        stringstream ss;
                        ss <<
                            "EMetaBlob.replay FIXME had dentry linked to wrong inode "
                            << *dn << " " << *dn->get_linkage()->
                            get_inode() << " should be " << p->inode.ino;
                        dout(0) << ss.str() << dendl;
                        mds->clog->warn(ss);
                    }
                ...
                if (dn->get_linkage()->get_inode() != in) {
                    ...
                    dout(10) << "EMetaBlob.replay for [" << p->
                        dnfirst << "," << p->dnlast << "] had " << *in << dendl;
                }
                ...
                if (!dn->get_linkage()->is_null()) {
                    ...
                    if (dn->get_linkage()->is_primary()) {
                        ...
                        ss <<
                            "EMetaBlob.replay FIXME had dentry linked to wrong inode "
                            << *dn << " " << *dn->get_linkage()->
                            get_inode() << " should be remote " << p->ino;
                        dout(0) << ss.str() << dendl;
                    }
                    dir->unlink_inode(dn, false);
                }
        ...
        for (list < nullbit >::const_iterator p = lump.get_dnull().begin();
            ...
            dout(10) << "EMetaBlob.replay pushing to bottom of lru " << *dn <<
                dendl;
        }
    }
    ...
    if (renamed_dirino) {
        ...
        if (olddir) {
            if (olddir->authority() != CDIR_AUTH_UNDEF &&
                ...
                for (list < frag_t >::iterator p = leaves.begin();
                     p != leaves.end(); ++p) {
        ...
        if (renamed_diri->authority() != CDIR_AUTH_UNDEF) {
            for (list < frag_t >::iterator p = renamed_dir_frags.begin();
                 p != renamed_dir_frags.end(); ++p) {
                CDir *dir = renamed_diri->get_dirfrag(*p);
                if (dir) {
                    // we already had the inode before, and we already adjusted this subtree accordingly.
                    dout(10) << " already had+adjusted rename import bound " <<
                        *dir << dendl;
                    assert(olddir);
                    continue;
                }
                dir = renamed_diri->get_or_open_dirfrag(mds->mdcache, *p);
                dout(10) << " creating new rename import bound " << *dir <<
                    dendl;
    ...
    if (!unlinked.empty()) {
        for (set < CInode * >::iterator p = linked.begin(); p != linked.end();
             ++p)
            unlinked.erase(*p);
    ...
    if (inotablev) {
        if (mds->inotable->get_version() >= inotablev) {
            ...
            dout(10) << "EMetaBlob.replay inotable v " << inotablev
                ...
                << " prealloc " << preallocated_inos << dendl;
            if (allocated_ino)
    ...
    if (sessionmapv) {
        if (mds->sessionmap.get_version() >= sessionmapv) {
            ...
            if (session) {
                dout(20) << " (session prealloc " << session->info.
                    prealloc_inos << ")" << dendl;
                    ...
                    if (!session->info.prealloc_inos.empty()) {
                        ...
                        if (next != i)
                            mds->clog->
                                ...
                                " used ino " << i << " but session next is " <<
                                next;
                ...
                dout(10) << "EMetaBlob.replay no session for " << client_name <<
                    dendl;
                if (used_preallocated_ino) {
    ...
    for (vector < inodeno_t >::iterator p = destroyed_inodes.begin();
        ...
        CInode *in = mds->mdcache->get_inode(*p);
        if (in) {
            dout(10) << "EMetaBlob.replay destroyed " << *p << ", dropping " <<
                *in << dendl;
    ...
    // client requests
    for (list < pair < metareqid_t, uint64_t > >::iterator p =
         client_reqs.begin(); p != client_reqs.end(); ++p) {
        if (p->first.name.is_client()) {
            dout(10) << "EMetaBlob.replay request " << p->
                first << " trim_to " << p->second << dendl;
            inodeno_t created =
                allocated_ino ? allocated_ino : used_preallocated_ino;
    ...
    // client flushes
    for (list < pair < metareqid_t, uint64_t > >::iterator p =
         client_flushes.begin(); p != client_flushes.end(); ++p) {
        if (p->first.name.is_client()) {
            dout(10) << "EMetaBlob.replay flush " << p->
                first << " trim_to " << p->second << dendl;
            Session *session = mds->sessionmap.get_session(p->first.name);
            if (session) {
...
void ESession::replay(MDSRank * mds)
{
    if (mds->sessionmap.get_version() >= cmapv) {
        dout(10) << "ESession.replay sessionmap " << mds->sessionmap.
            get_version()
            << " >= " << cmapv << ", noop" << dendl;
    }
    else {
        ...
        if (open) {
            ...
            if (session) {      // there always should be a session, but there's a bug
                if (session->connection == NULL) {
                    dout(10) << " removed session " << session->info.
                        inst << dendl;
                    ...
                    dout(10) << " reset session " << session->info.
                        inst << " (they reconnected)" << dendl;
                }
            }
            else {
                mds->clog->
                    error() << "replayed stray Session close event for " <<
                    client_inst << " from time " << stamp << ", ignoring";
            }
        }
        if (session) {
            mds->sessionmap.replay_dirty_session(session);
    ...
    if (inos.size() && inotablev) {
        if (mds->inotable->get_version() >= inotablev) {
            dout(10) << "ESession.replay inotable " << mds->inotable->
                get_version()
...
void ESession::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(4, 3, bl);
    ::encode(stamp, bl);
...
void ESession::dump(Formatter * f) const const
{
...
void ESessions::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(1, 1, bl);
...
void ESessions::dump(Formatter * f) const const
{
    f->dump_int("client map version", cmapv);
...
{
    if (mds->sessionmap.get_version() >= cmapv) {
        dout(10) << "ESessions.replay sessionmap " << mds->sessionmap.
            get_version()
            << " >= " << cmapv << ", noop" << dendl;
    }
    else {
...
void ETableServer::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    ::encode(stamp, bl);
    ::encode(table, bl);
...
void ETableServer::dump(Formatter * f) const const
{
...
void ETableClient::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    ::encode(stamp, bl);
    ::encode(table, bl);
...
void ETableClient::dump(Formatter * f) const const
{
...
void EUpdate::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(4, 4, bl);
    ::encode(stamp, bl);
    ::encode(type, bl);
...
void EUpdate::dump(Formatter * f) const const
{
...
void EUpdate::replay(MDSRank * mds)
{
    ...
    if (client_map.length()) {
        if (mds->sessionmap.get_version() >= cmapv) {
            ...
            dout(10) << "EUpdate.replay sessionmap " << mds->sessionmap.
                get_version()
                << " < " << cmapv << dendl;
...
void EOpen::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(4, 3, bl);
    ::encode(stamp, bl);
    ::encode(metablob, bl, features);
...
void EOpen::dump(Formatter * f) const const
{
    f->open_object_section("metablob");
    metablob.dump(f);
    f->close_section();         // metablob
...
void EOpen::replay(MDSRank * mds)
{
  ...
  for (const auto & ino:inos) {
        ...
        if (!in) {
            dout(0) << "EOpen.replay ino " << ino << " not in metablob" <<
                dendl;
            assert(in);
        }
  ...
  for (const auto & vino:snap_inos) {
        ...
        if (!in) {
            dout(0) << "EOpen.replay ino " << vino << " not in metablob" <<
                dendl;
            assert(in);
        }
        _segment->open_files.push_back(&in->item_open_file);
...
void ECommitted::replay(MDSRank * mds)
{
    if (mds->mdcache->uncommitted_masters.count(reqid)) {
        ...
        mds->mdcache->uncommitted_masters[reqid].ls->uncommitted_masters.
            erase(reqid);
        mds->mdcache->uncommitted_masters.erase(reqid);
    }
    else {
        dout(10) << "ECommitted.replay " << reqid <<
            " -- didn't see original op" << dendl;
    }
}

void ECommitted::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    ::encode(stamp, bl);
    ::encode(reqid, bl);
...
void ECommitted::dump(Formatter * f) const const
{
...
void link_rollback::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(reqid, bl);
    ::encode(ino, bl);
...
void link_rollback::dump(Formatter * f) const const
{
...
void rmdir_rollback::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(reqid, bl);
    ::encode(src_dir, bl);
...
void rmdir_rollback::dump(Formatter * f) const const
{
    f->dump_stream("metareqid") << reqid;
...
void rename_rollback::drec::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(dirfrag, bl);
...
void rename_rollback::drec::dump(Formatter * f) const const
{
...
void rename_rollback::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(reqid, bl);
    encode(orig_src, bl);
...
void rename_rollback::dump(Formatter * f) const const
{
...
void ESlaveUpdate::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    ::encode(stamp, bl);
    ::encode(type, bl);
...
void ESlaveUpdate::dump(Formatter * f) const const
{
    f->open_object_section("metablob");
    commit.dump(f);
    f->close_section();         // metablob
...
void ESlaveUpdate::replay(MDSRank * mds)
{
    ...
    case ESlaveUpdate::OP_PREPARE:
        dout(10) << "ESlaveUpdate.replay prepare " << reqid << " for mds." <<
            master << ": applying commit, saving rollback info" << dendl;
    ...
    case ESlaveUpdate::OP_COMMIT:
        ...
        if (su) {
            dout(10) << "ESlaveUpdate.replay commit " << reqid << " for mds." <<
                master << dendl;
            mds->mdcache->finish_uncommitted_slave_update(reqid, master);
        }
        else {
            dout(10) << "ESlaveUpdate.replay commit " << reqid << " for mds." <<
                master << ": ignoring, no previously saved prepare" << dendl;
        }
        break;

    case ESlaveUpdate::OP_ROLLBACK:
        dout(10) << "ESlaveUpdate.replay abort " << reqid << " for mds." <<
            master << ": applying rollback commit blob" << dendl;
...
void ESubtreeMap::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(6, 5, bl);
    ::encode(stamp, bl);
    ::encode(metablob, bl, features);
...
void ESubtreeMap::dump(Formatter * f) const const
{
    f->open_object_section("metablob");
    metablob.dump(f);
    f->close_section();         // metablob
    ...
    for (map < dirfrag_t, vector < dirfrag_t > >::const_iterator i =
         subtrees.begin(); i != subtrees.end(); ++i) {
...
void ESubtreeMap::replay(MDSRank * mds)
{
    ...
    if (mds->mdcache->is_subtrees()) {
        ...
        for (map < dirfrag_t, vector < dirfrag_t > >::iterator p =
             subtrees.begin(); p != subtrees.end(); ++p) {
            ...
            if (dir->get_dir_auth().first != mds->get_nodeid()) {
                mds->clog->
                    error() << " replayed ESubtreeMap at " << get_start_off()
                    << " subtree root " << p->
                    first << " is not mine in cache (it's " << dir->
                    get_dir_auth() << ")";
                ++errors;
                continue;
            }
            ...
            for (vector < dirfrag_t >::iterator q = p->second.begin();
                ...
                if (!b) {
                    mds->clog->
                        error() << " replayed ESubtreeMap at " <<
                        get_start_off()
                        << " subtree " << p->
                        first << " bound " << *q << " not in cache";
                    ++errors;
                    continue;
                }
                if (bounds.count(b) == 0) {
                    mds->clog->
                        ...
                        get_start_off()
                        << " subtree " << p->
                        first << " bound " << *q << " not a bound in cache";
                    ++errors;
                    continue;
                }
                bounds.erase(b);
            ...
            for (set < CDir * >::iterator q = bounds.begin(); q != bounds.end();
                ...
                mds->clog->
                    error() << " replayed ESubtreeMap at " << get_start_off()
                    << " subtree " << p->
                    first << " has extra bound in cache " << (*q)->dirfrag();
                ++errors;
            }
            ...
            if (ambiguous_subtrees.count(p->first)) {
                if (!mds->mdcache->have_ambiguous_import(p->first)) {
                    mds->clog->
                        ...
                        get_start_off()
                        << " subtree " << p->
                        first << " is ambiguous but is not in our cache";
                    ++errors;
                }
            }
            else {
                if (mds->mdcache->have_ambiguous_import(p->first)) {
                    mds->clog->
                        ...
                        get_start_off()
                        << " subtree " << p->
                        first << " is not ambiguous but is in our cache";
                    ++errors;
                }
            }
        }
        ...
        if (errors) {
            ...
            dout(0) << "journal ambig_subtrees: " << ambiguous_subtrees <<
                dendl;
...
void EFragment::replay(MDSRank * mds)
{
    ...
    switch (op) {
    case OP_PREPARE:
        mds->mdcache->add_uncommitted_fragment(dirfrag_t(ino, basefrag), bits,
                                               orig_frags, _segment, &rollback);

        if (in)
            mds->mdcache->adjust_dir_fragments(in, basefrag, bits, resultfrags,
                                               waiters, true);
        break;
    ...
    case OP_ROLLBACK:
        ...
        mds->mdcache->rollback_uncommitted_fragment(dirfrag_t(ino, basefrag),
                                                    old_frags);
        break;
...
void EFragment::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(5, 4, bl);
    ::encode(stamp, bl);
    ::encode(op, bl);
...
void EFragment::dump(Formatter * f) const const
{
    /*f->open_object_section("Metablob");
...
void dirfrag_rollback::encode(bufferlist & bl) const const
{
    ENCODE_START(1, 1, bl);
    ::encode(fnode, bl);
    ENCODE_FINISH(bl);
...
void EExport::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(4, 3, bl);
    ::encode(stamp, bl);
    ::encode(metablob, bl, features);
...
void EExport::dump(Formatter * f) const const
{
...
void EImportStart::replay(MDSRank * mds)
{
    ...
    mds->mdcache->adjust_bounded_subtree_auth(dir, realbounds,
                                              mds_authority_t(mds->get_nodeid(),
                                                              mds->
                                                              get_nodeid()));

    // open client sessions?
    if (mds->sessionmap.get_version() >= cmapv) {
        dout(10) << "EImportStart.replay sessionmap " << mds->sessionmap.
            get_version()
            << " >= " << cmapv << ", noop" << dendl;
    }
    else {
...
void EImportStart::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(4, 3, bl);
    ::encode(stamp, bl);
    ::encode(base, bl);
...
void EImportStart::dump(Formatter * f) const const
{
    f->dump_stream("base dirfrag") << base;
...
void EImportFinish::replay(MDSRank * mds)
{
    if (mds->mdcache->have_ambiguous_import(base)) {
        ...
        if (success) {
            ...
            mds->mdcache->adjust_bounded_subtree_auth(dir, bounds,
                                                      CDIR_AUTH_UNDEF);
...
void EImportFinish::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    ::encode(stamp, bl);
    ::encode(base, bl);
...
void EImportFinish::dump(Formatter * f) const const
{
    f->dump_stream("base dirfrag") << base;
...
void EResetJournal::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(stamp, bl);
    ENCODE_FINISH(bl);
...
void EResetJournal::dump(Formatter * f) const const
{
...
void EResetJournal::replay(MDSRank * mds)
{
    ...
    if (mds->mdsmap->get_root() == mds->get_nodeid()) {
        CDir *rootdir =
            mds->mdcache->get_root()->get_or_open_dirfrag(mds->mdcache,
                                                          frag_t());
...
void ENoOp::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(pad_size, bl);
...
bool EMetaBlob::rewrite_truncate_finish(MDSRank const *mds,
                                        std::map < log_segment_seq_t,
                                        log_segment_seq_t > const &old_to_new)
{
    bool modified = false;
    map < inodeno_t, log_segment_seq_t > new_trunc_finish;
    for (std::map < inodeno_t, log_segment_seq_t >::iterator i =
         truncate_finish.begin(); i != truncate_finish.end(); ++i) {
        if (old_to_new.count(i->second)) {
            dout(20) << __func__ << " applying segment seq mapping "
                << i->second << " -> " << old_to_new.find(i->second)->
                second << dendl;
