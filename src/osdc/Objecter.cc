...
void Objecter::handle_osd_map(MOSDMap * m)
{
        ...
        if (osdmap->get_epoch()) {
            ...
            for (epoch_t e = osdmap->get_epoch() + 1; e <= m->get_last(); e++) {
                ...
                for (map < int, OSDSession * >::iterator p =
                     osd_sessions.begin(); p != osd_sessions.end();) {
            ...
            if (m->maps.count(m->get_last())) {
                for (map < int, OSDSession * >::iterator p =
                     osd_sessions.begin(); p != osd_sessions.end(); ++p) {
                    OSDSession *s = p->second;
    ...
    for (map < ceph_tid_t, CommandOp * >::iterator p =
         need_resend_command.begin(); p != need_resend_command.end(); ++p) {
...
int Objecter::pool_snap_list(int64_t poolid, vector < uint64_t > *snaps)
{
    ...
    for (map < snapid_t, pool_snap_info_t >::const_iterator p =
         pi->snaps.begin(); p != pi->snaps.end(); ++p) {
                              ...
                              map < uint64_t, LingerOp * >&lresend)
{
    ...
    for (map < ceph_tid_t, LingerOp * >::iterator j =
         session->linger_ops.begin(); j != session->linger_ops.end(); ++j) {
    ...
    for (map < ceph_tid_t, CommandOp * >::iterator k =
         session->command_ops.begin(); k != session->command_ops.end(); ++k) {
...
void Objecter::resend_mon_ops()
{
    ...
    for (map < uint64_t, LingerOp * >::iterator p =
         check_latest_map_lingers.begin(); p != check_latest_map_lingers.end();
         ++p) {
...
int Objecter::_calc_target(op_target_t * t, Connection * con, bool any_change)
{
    ...
    if (legacy_change || split || force_resend) {
        ...
        osdmap->
            get_primary_shard(pg_t
                              (ceph_stable_mod
                               (pgid.ps(), t->pg_num, t->pg_num_mask),
                               pgid.pool()), &t->actual_pgid);
        ...
        if (acting_primary == -1) {
            ...
            if (read && (t->flags & CEPH_OSD_FLAG_BALANCE_READS)) {
                ...
                osd = acting[p];
                ldout(cct,
                      10) << " chose random osd." << osd << " of " << acting <<
                    dendl;
            }
...
void Objecter::_dump_command_ops(const OSDSession * s, Formatter * fmt)
{
    for (map < uint64_t, CommandOp * >::const_iterator p =
         s->command_ops.begin(); p != s->command_ops.end(); ++p) {
        CommandOp *op = p->second;
...
void Objecter::dump_pool_stat_ops(Formatter * fmt) const const
{
    ...
    for (map < ceph_tid_t, PoolStatOp * >::const_iterator p =
         poolstat_ops.begin(); p != poolstat_ops.end(); ++p) {
