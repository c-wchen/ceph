...
namespace crimson {
    ...
    static constexpr core_id_t NULL_CORE =
    ...
    auto submit_to(core_id_t core, auto && f) {
        ...
        if constexpr(is_errorated_future_v < ret_type >) {
            auto ret = seastar::smp::submit_to(core,
    ...
    template < typename Obj, typename Method, typename ... Args >
        auto proxy_method_on_core(core_id_t core, Obj & obj, Method method, Args
                                  && ... args) {
        return crimson::submit_to(core,
                                  [&obj, method, arg_tuple =
 ...
 */
    template < typename F > auto reactor_map_seq(F && f) {
        using ret_type = decltype(f());
        if constexpr
            (is_errorated_future_v < ret_type >) {
            auto ret = crimson::do_for_each(seastar::smp::all_cpus().begin(),
                                            seastar::smp::all_cpus().end(),
                                            [f =
                                             std::move(f)] (auto core) mutable {
                                            return seastar::smp::submit_to(core,
                                                                           [&f] {
                                                                                 return
                                                                                 std::
                                                                                 invoke
