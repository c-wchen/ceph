    ...
    namespace api {
        ...
        namespace {
            ...
            struct DiffContext {
                ...
                OrderedThrottle throttle;
                 ...
                 template < typename I >
                    DiffContext(I & image_ctx,
                                DiffIterate <>::Callback callback,
                                void *callback_arg, bool _whole_object,
            ...
            class C_DiffObject:public Context {
              public:
                ...
                :m_cct(image_ctx.cct), m_head_ctx(head_ctx),
                    ...
                    C_OrderedThrottle *ctx =
                        m_diff_context.throttle.start_op(this);
                    ...
                    int r =
                        m_head_ctx.aio_operate(m_oid, rados_completion, &op,
                                               NULL);
                     assert(r == 0);
                ...
                void finish(int r) override {
                    ...
                    if (r == 0) {
                        ...
                        ldout(cct,
                              20) << "object " << m_oid <<
                            ": list_snaps failed: " << cpp_strerror(r) << dendl;
                    }

                    if (r == 0) {
                        for (Diffs::const_iterator d = diffs.begin();
                             d != diffs.end(); ++d) {
                            r = m_diff_context.callback(d->get < 0 > (),
                                                        d->get < 1 > (),
                                                        d->get < 2 > (),
                                                        m_diff_context.
                                                        callback_arg);
              ...
              private:
                ...
                void compute_diffs(Diffs * diffs) {
                    ...
                    bool whole_object;
                    calc_snap_set_diff(cct, m_snap_set,
                                       m_diff_context.from_snap_id,
                                       m_diff_context.end_snap_id, &diff,
                                       &end_size, &end_exists,
                                       &clone_end_snap_id, &whole_object);
                    if (whole_object) {
                        ldout(cct,
                              1) << "object " << m_oid <<
                            ": need to provide full object" << dendl;
                    }
                    ldout(cct,
                          20) << "  diff " << diff << " end_exists=" <<
                        end_exists << dendl;
                    if (diff.empty() && !whole_object) {
                        ...
                        for (vector < ObjectExtent >::iterator q =
                             m_object_extents.begin();
                             q != m_object_extents.end(); ++q) {
                            diffs->
                                push_back(boost::
                                          make_tuple(m_offset + q->offset,
                                                     q->length, end_exists));
                        }
                        return;
                    }
                    ...
                    for (vector < ObjectExtent >::iterator q =
                         m_object_extents.begin(); q != m_object_extents.end();
                         ++q) {
                        ldout(cct,
                              20) << "diff_iterate object " << m_oid <<
                            " extent " << q->offset << "~" << q->
                            length << " from " << q->buffer_extents << dendl;
                        uint64_t opos = q->offset;
                        for (vector < pair < uint64_t,
                             uint64_t > >::iterator r =
                            ...
                            for (interval_set < uint64_t >::iterator s =
                                 overlap.begin(); s != overlap.end(); ++s) {
                                uint64_t su_off = s.get_start() - opos;
                                uint64_t logical_off =
                                    m_offset + r->first + su_off;
                                ldout(cct,
                                      20) << "   overlap extent " << s.
                                    get_start() << "~" << s.
        ...
        template < typename I >
            int DiffIterate < I >::diff_iterate(I * ictx,
                                                ...
                                                bool include_parent,
                                                bool whole_object,
                                                int (*cb) (uint64_t, size_t,
                                                           int, void *),
                                                void *arg) {
            ldout(ictx->cct,
                  20) << "diff_iterate " << ictx << " off = " << off <<
                " len = " << len << dendl;
            ...
            DiffIterate command(*ictx, from_snap_namespace, fromsnapname, off,
                                len, include_parent, whole_object, cb, arg);
            r = command.execute();
            return r;
        }
        ...
        template < typename I > int DiffIterate < I >::execute() {
            ...
            uint64_t end_size;
            {
                ...
                if (m_from_snap_name) {
                    from_snap_id =
                        m_image_ctx.get_snap_id(m_from_snap_namespace,
                                                m_from_snap_name);
            ...
            BitVector < 2 > object_diff_state;
            {
                ...
                if (m_whole_object
                    ...
                    r = diff_object_map(from_snap_id, end_snap_id,
                                        &object_diff_state);
                    if (r < 0) {
            ...
            while (left > 0) {
                ...
                for (map < object_t, vector < ObjectExtent > >::iterator p =
                    ...
                    if (fast_diff_enabled) {
                        ...
                        if (object_diff_state[object_no] !=
                            OBJECT_DIFF_STATE_NONE) {
                            bool updated =
                                (object_diff_state[object_no] ==
                                 OBJECT_DIFF_STATE_UPDATED);
                            for (std::vector < ObjectExtent >::iterator q =
                                 p->second.begin(); q != p->second.end(); ++q) {
                                r = m_callback(off + q->offset, q->length,
                                               updated, m_callback_arg);
                                if (r < 0) {
                                    return r;
                                }
                            }
                        ...
                        C_DiffObject *diff_object =
                            new C_DiffObject(m_image_ctx, head_ctx,
                                             diff_context,
                                             p->first.name, off,
                                             p->second);
                        diff_object->send();
        ...
        template < typename I >
            int DiffIterate < I >::diff_object_map(uint64_t from_snap_id,
                                                   uint64_t to_snap_id,
                                                   BitVector < 2 >
                                                   *object_diff_state) {
            ...
            while (true) {
                ...
                if (current_snap_id != CEPH_NOSNAP) {
                    std::map < librados::snap_t,
                        SnapInfo >::const_iterator snap_it =
                ...
                if (r < 0) {
                    lderr(cct) <<
                        "diff_object_map: failed to retrieve image flags" <<
                        dendl;
                    return r;
                }
                if ((flags & RBD_FLAG_FAST_DIFF_INVALID) != 0) {
                    ldout(cct,
                          1) <<
                        "diff_object_map: cannot perform fast diff on invalid "
                        << "object map" << dendl;
                    return -EINVAL;
                }
                ...
                std::string oid(ObjectMap <>::object_map_name(m_image_ctx.id,
                                                              current_snap_id));
                r = cls_client::object_map_load(&m_image_ctx.md_ctx, oid,
                                                &object_map);
                if (r < 0) {
                    lderr(cct) << "diff_object_map: failed to load object map "
                        << oid << dendl;
                    return r;
                }
                ldout(cct,
                ...
                uint64_t overlap =
                    MIN(object_map.size(), prev_object_map.size());
                ...
                if (object_map.size() > prev_object_map.size() &&
                    ...
                    for (uint64_t i = overlap; i < object_diff_state->size();
                         ++i) {
                        ldout(cct,
                              20) << __func__ << ": object state: " << i << " "
                            << "->" << static_cast < uint32_t >
                            (object_map[i]) << dendl;
                        if (object_map[i] == OBJECT_NONEXISTENT) {
                ...
                if (current_snap_id == next_snap_id
                    || next_snap_id > to_snap_id) {
                    break;
                }
