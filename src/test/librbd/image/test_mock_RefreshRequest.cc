...
namespace librbd {
    ...
    namespace image {
        ...
        RefreshParentRequest < MockRefreshImageCtx > *RefreshParentRequest <
...
namespace librbd {
    namespace image {
          ...
          public:
            ...
            void expect_set_require_lock(MockRefreshImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(*mock_image_ctx.io_work_queue,
                            set_require_lock(direction, enabled));
            ...
            }} void expect_v1_get_snapshots(MockRefreshImageCtx &
                                            mock_image_ctx, int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
                                     StrEq("snap_list"), _, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_snap_timestamp_list(MockRefreshImageCtx &
                                            mock_image_ctx, int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
                                     StrEq("get_snapshot_timestamp"), _, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
            ...
            void expect_snap_namespace_list(MockRefreshImageCtx &
                                            mock_image_ctx, int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
                                     StrEq("get_snapshot_namespace"), _, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
        ...
        TEST_F(TestMockImageRefreshRequest, DisableExclusiveLock) {
            ...
            expect_refresh_parent_is_required(mock_refresh_parent_request,
                                              false);
            expect_shut_down_exclusive_lock(mock_image_ctx,
                                            *mock_exclusive_lock, 0);
        ...
        TEST_F(TestMockImageRefreshRequest, EnableJournalWithoutExclusiveLock) {
            ...
            expect_refresh_parent_is_required(mock_refresh_parent_request,
                                              false);
            expect_set_require_lock(mock_image_ctx, librbd::io::DIRECTION_BOTH,
                                    true);
        ...
        TEST_F(TestMockImageRefreshRequest, DisableJournal) {
            ...
            if (!mock_image_ctx.clone_copy_on_read) {
                expect_set_require_lock(mock_image_ctx,
                                        librbd::io::DIRECTION_READ, false);
            }
