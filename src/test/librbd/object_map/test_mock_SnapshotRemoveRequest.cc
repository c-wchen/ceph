...
namespace librbd {
    namespace object_map {
        ...
        class TestMockObjectMapSnapshotRemoveRequest:public TestMockFixture {
          public:
            void expect_load_map(librbd::ImageCtx * ictx, uint64_t snap_id,
                                 int r) {
                std::string snap_oid(ObjectMap <>::
                                     object_map_name(ictx->id, snap_id));
                if (r < 0) {
                    EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                                exec(snap_oid, _, StrEq("rbd"),
                ...
                if (r < 0) {
                    ...
                    EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                                exec(oid, _, StrEq("rbd"),
                                     StrEq("object_map_snap_remove"), _, _, _))
                        .WillOnce(DoDefault());
                }
            }

            void expect_remove_map(librbd::ImageCtx * ictx, uint64_t snap_id,
                                   int r) {
                std::string snap_oid(ObjectMap <>::
                                     object_map_name(ictx->id, snap_id));
                if (r < 0) {
        ...
        TEST_F(TestMockObjectMapSnapshotRemoveRequest, Success) {
            ...
            AsyncRequest <> *request =
                new SnapshotRemoveRequest(*ictx, &object_map, snap_id,
                                          &cond_ctx);
            {
        ...
        TEST_F(TestMockObjectMapSnapshotRemoveRequest, ScrubCleanObjects) {
            ...
            for (uint64_t i = 512; i < object_map.size(); ++i) {
                object_map[i] =
                    i % 2 == 0 ? OBJECT_EXISTS_CLEAN : OBJECT_NONEXISTENT;
            }
