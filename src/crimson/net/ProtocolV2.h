...
private:
    ...
    void trigger_state(state_t state, IOHandler::io_state_t io_state,
                       bool reentrant);
        ...
        gate.dispatch_in_background(what, who, [this, &who, &func] {
            if (!execution_done.available()) {
                ...
                ("gated_execute_abandon", who,
                 [fut =
                std::move(execution_done)]()mutable {
                    return std::move(fut);}
                );
            }
            seastar::promise <> pr;
    ...
    seastar::future <> client_auth(std::vector < uint32_t >
                                   &allowed_methods);
    ...
    seastar::future <> _handle_auth_request(bufferlist &auth_payload,
                                            bool more);
    ...
    seastar::future < next_step_t > reuse_connection(ProtocolV2 *existing_proto, bool do_reset =
                false, bool reconnect = false, uint64_t conn_seq =
                0, uint64_t msg_seq = 0);
...
inline create_handlers_ret create_handlers(ChainedDispatchers &dispatchers,
        SocketConnection &conn)
{
    std::unique_ptr < ConnectionHandler > io_handler =
        std::make_unique < IOHandler > (dispatchers, conn);
    IOHandler &io_handler_concrete =
        static_cast < IOHandler & >(*io_handler);
    auto protocol =
        std::make_unique < ProtocolV2 > (conn, io_handler_concrete);
    io_handler_concrete.set_handshake_listener(*protocol);
    return {
        std::move(io_handler), std::move(protocol)};
}
...
template <> struct fmt::formatter <
    crimson::net::ProtocolV2 > : fmt::ostream_formatter {
};
#endif
