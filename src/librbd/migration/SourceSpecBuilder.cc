...
namespace librbd {
    namespace migration {
        ...
        int SourceSpecBuilder <
            I >::parse_source_spec(const std::string & source_spec,
                                   json_spirit::mObject *
                                   source_spec_object) const {
        ...
        template < typename I >
            int SourceSpecBuilder <
            I >::build_format(const json_spirit::mObject & source_spec_object,
                              bool import_only,
            ...
            if (type == "native") {
                format->reset(NativeFormat <
                              I >::create(m_image_ctx, source_spec_object,
                                          import_only));
            }
            else if (type == "qcow") {
                format->reset(QCOWFormat <
                              I >::create(m_image_ctx, source_spec_object,
                                          this));
            }
            else if (type == "raw") {
        ...
        template < typename I >
            int SourceSpecBuilder <
            I >::build_snapshot(const json_spirit::mObject & source_spec_object,
                                uint64_t index,
        ...
        template < typename I >
            int SourceSpecBuilder <
            I >::build_stream(const json_spirit::mObject & source_spec_object,
            ...
            auto & type = type_value_it->second.get_str();
            if (type == "file") {
                stream->reset(FileStream <
                              I >::create(m_image_ctx, stream_obj));
            }
            else if (type == "http") {
                stream->reset(HttpStream <
                              I >::create(m_image_ctx, stream_obj));
            }
            else if (type == "s3") {
