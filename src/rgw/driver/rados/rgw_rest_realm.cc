...
void RGWOp_Period_Post::execute(optional_yield y)
{
    ...
    if (period.get_realm() != static_cast < rgw::sal::RadosStore * > (driver)->svc()->zone->get_realm().get_id()) {
        error_stream << "period with realm id " << period.get_realm()
                     << " doesn't match current realm " << static_cast<rgw::sal::RadosStore * > (driver)->svc()->zone->get_realm().get_id()
                     << std::endl;
        op_ret = -EINVAL;
        return;
    }
    ...
    op_ret = current_period.init(this, cct, static_cast < rgw::sal::RadosStore* > (driver)->svc()->sysobj, realm.get_id(),
                                 y);
    if (op_ret < 0) {
    ...
    if (period.get_master_zone() == static_cast < rgw::sal::RadosStore * >
