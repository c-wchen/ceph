...
typedef boost::tuple < Context *, Context *,
...
void RadosBackend::write(const string & oid,
                         uint64_t offset,
                         const bufferlist & bl,
                         Context * on_write_applied, Context * on_commit)
{
    ...
    completion->set_safe_callback(arg, on_complete);

    completion->set_complete_callback(arg, on_applied);

    ioctx->aio_write(oid, completion, bl, bl.length(), offset);
}

void RadosBackend::read(const string & oid,
                        uint64_t offset,
                        uint64_t length,
                        bufferlist * bl, Context * on_read_complete)
{
    librados::AioCompletion * completion =
        librados::Rados::aio_create_completion();
    ...
    completion->set_complete_callback(arg, on_complete);
