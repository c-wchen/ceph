...
public:
    ...
    boost::asio::io_context &context,
          GetClientCounters
          && counters, md_config_obs_t *observer, Args
          && ... args);
    ~AsyncScheduler();
    ...
    /// submit an async request for dmclock scheduling. the given completion/// handler will be invoked with (error_code, PhaseType) when the request/// is ready or canceled. on success, this grants a throttle unit that must/// be returned with a call to request_complete()
    template < typename CompletionToken >
    auto async_request(const client_id &client, const ReqParams &params,
                       const Time &time, Cost cost, CompletionToken
                       && token);
    ...
    private:
        int schedule_request_impl(const client_id &client,
                                  const ReqParams &params, const Time &time,
                                  const Cost &cost,
                                  optional_yield yield_ctx) override;

        static constexpr bool IsDelayed = false;
        using Queue =
            crimson::dmclock::PullPriorityQueue < client_id, Request,
            IsDelayed >;
        ...
        using Completion =
            async::Completion < Signature, async::AsBase < Request >>;
...
template < typename ... Args > AsyncScheduler::AsyncScheduler(CephContext *cct,
        boost::asio::io_context &context,
        GetClientCounters
        && counters, md_config_obs_t *observer,
        Args && ... args)
    :    queue(std::forward < Args > (args) ...), timer(context), cct(cct),
         observer(observer), counters(std::move(counters)),
         max_requests(cct->_conf.get_val < int64_t >
                                   ...
                                   CompletionToken && token)
{
    ...
    if (r == 0) {
        ...
        async::post(std::unique_ptr < Completion > {
            completion
        }
...
{
public:
    SimpleThrottler(CephContext *cct): max_requests(cct->_conf.get_val <
                int64_t >
                ("rgw_max_concurrent_requests")),
                            ...
                            &changed) override
    {
        if (changed.count("rgw_max_concurrent_requests")) {
            auto new_max =
                conf.get_val < int64_t > ("rgw_max_concurrent_requests");
            max_requests =
                new_max > 0 ? new_max : std::numeric_limits <
                int64_t >::max();
        }
    }
