...
int CrushTester::get_maximum_affected_by_rule(int ruleno)
{
     ...
     */
    int max_affected =
        std::max(crush.get_max_buckets(), crush.get_max_devices());
                                  ...
                                  vector < __u32 > &weight)
{
    ...
    int devices_requested =
        std::min(maxout, get_maximum_affected_by_rule(ruleno));
    ...
    for (int r = min_rule; r < crush.get_max_rules() && r <= max_rule; r++) {
        ...
        for (int nr = min_rep; nr <= max_rep; nr++) {
            ...
            float expected_objects =
                std::min(nr, get_maximum_affected_by_rule(r)) * num_objects;
