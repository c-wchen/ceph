...
namespace ceph::compression::onwire {
    ...
    std::optional < ceph::bufferlist >
        ...
        if (m_compressor->compress(input, out, compressor_message)) {
            ...
            ldout(m_cct,
                  20) << __func__ << " uncompressed.length()=" << input.length()
    ...
    std::optional < ceph::bufferlist >
        ...
        if (m_compressor->decompress(input, out, compressor_message)) {
            ...
            ldout(m_cct,
                  20) << __func__ << " compressed.length()=" << input.length()
    ...
    void TxHandler::done() {
        ldout(m_cct,
              25) << __func__ << " compression ratio=" << get_ratio() << dendl;
    }
