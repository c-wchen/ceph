 ...
 */
    class ExtentOolWriter {
      ...
      public:
        ...
        virtual alloc_write_iertr::
            future <> alloc_write_ool_extents(Transaction & t,
                                              std::list <
 ...
 */
    class SegmentedOolWriter:public ExtentOolWriter {
      ...
      private:
        alloc_write_iertr::future <> do_write(Transaction & t,
                                              std::list <
                                              LogicalCachedExtentRef > &extent);

        alloc_write_ertr::future <> write_record(Transaction & t,
                                                 record_t && record,
                                                 std::list <
                                                 LogicalCachedExtentRef >
                                                 &&extents,
    ...
    class RandomBlockOolWriter:public ExtentOolWriter {
      public:
        RandomBlockOolWriter(RBMCleaner * rb_cleaner):rb_cleaner(rb_cleaner) {
 ...
 */
    struct io_usage_t {
        ...
        friend std::ostream & operator<<(std::ostream & out,
            ...
            return out << "io_usage_t(" << "inline_usage=" << usage.
                inline_usage << ", main_cleaner_usage=" << usage.cleaner_usage.
                main_usage << ", cold_cleaner_usage=" << usage.cleaner_usage.
                cold_ool_usage << ")";
    }};
    ...
    class ExtentPlacementManager {
      public:
        ...
        :ool_segment_seq_allocator(std::make_unique < SegmentSeqAllocator >
                                   (segment_type_t::OOL)) {
        ...
        alloc_result_t alloc_new_extent(Transaction & t,
                                        extent_types_t type,
                                        extent_len_t length,
                                        placement_hint_t hint,
#ifdef UNIT_TESTS_BUILT
                                        rewrite_gen_t gen,
                                        std::optional < paddr_t >
                                        external_paddr = std::nullopt
#else
                                        rewrite_gen_t gen
#endif
            ) {
   ...
   */
        ...
        alloc_paddr_iertr::future <> write_delayed_ool_extents(Transaction & t,
                                                               extents_by_writer_t
                                                               & alloc_map);
   ...
   */
        alloc_paddr_iertr::
            future <> write_preallocated_ool_extents(Transaction & t,
                                                     std::list <
                                                     LogicalCachedExtentRef >
                                                     extents);
        ...
        read_ertr::future <> read(paddr_t addr,
                                  size_t len, ceph::bufferptr & out) {
        ...
        backend_type_t get_main_backend_type() const {
            ...
            // for test assert(primary_device);
        ...
        void test_init_no_background(Device * test_device) {
            assert(test_device->get_backend_type() ==
      ...
      private:
        rewrite_gen_t adjust_generation(data_category_t category,
                                        extent_types_t type,
                                        placement_hint_t hint,
                                        rewrite_gen_t gen) {
   ...
   */
        class BackgroundProcess:public BackgroundListener {
          public:
            ...
            void init(JournalTrimmerImplRef && _trimmer,
                ...
                if (_cold_cleaner) {
                    ...
                    eviction_state.init(crimson::common::get_conf <
                                        double
                                        >
            ...
            mount_ret mount() {
                ...
                register_metrics();
                return main_cleaner->mount().safe_then([this] {
            ...
            void release_projected_usage(const io_usage_t & usage) {
                if (is_ready()) {
                    ...
                    main_cleaner->release_projected_usage(usage.cleaner_usage.
                                                          main_usage);
                    if (has_cold_tier()) {
            ...
            // Testing interfaces bool check_usage() {
          ...
          private:
            ...
            reserve_cleaner_result_t try_reserve_cleaner(const cleaner_usage_t &
                                                         usage);
            ...
            bool main_cleaner_should_run() const {
                ...
                if (has_cold_tier()) {
                    auto main_alive_ratio =
                        main_cleaner->get_stat().get_used_raw_ratio();
            ...
            struct eviction_state_t {
                ...
                void init(double stop_ratio,
                          double default_ratio, double fast_ratio) {
                ...
                } rewrite_gen_t adjust_generation_with_eviction(rewrite_gen_t
                                                                gen) {
...
template <> struct fmt::formatter <
    crimson::os::seastore::io_usage_t >:fmt::ostream_formatter {
};
#endif
