...
struct AttributeDumpVisitor: public boost::static_visitor < void > {
    ...
    AttributeDumpVisitor(ceph::Formatter *f,
                         const std::string &name)
        : f(f), name(name)
    {
...
} // anonymous namespace using namespace service_daemon;

template < typename I >
ServiceDaemon < I >::ServiceDaemon(CephContext *cct,
                                   RadosRef rados,
                                   Threads < I > *threads)
...
template < typename I >
void ServiceDaemon < I >::add_pool(int64_t pool_id,
                                   const std::string &pool_name)
{
    dout(20) << "pool_id=" << pool_id << ", pool_name=" << pool_name <<
             dendl;
...
{
    dout(20) << "pool_id=" << pool_id << ", namespace=" <<
             namespace_name << dendl;
...
uint64_t ServiceDaemon < I >::add_or_update_callout(int64_t pool_id,
        ...
        string &text)
{
    dout(20) << "pool_id=" << pool_id << ", " << "callout_id=" <<
             callout_id << ", " << "callout_level=" << callout_level << ", "
             << "text=" << text << dendl;

    {
        ...
        pool_it->second.callouts[callout_id] = {
            callout_level, text
        };
    }
...
template < typename I >
void ServiceDaemon < I >::remove_callout(int64_t pool_id,
        uint64_t callout_id)
{
    dout(20) << "pool_id=" << pool_id << ", " << "callout_id=" <<
             callout_id << dendl;
...
template < typename I >
void ServiceDaemon <
I >::add_or_update_namespace_attribute(int64_t pool_id,
                                       const std::
                                       string &namespace_name,
...
template < typename I > void ServiceDaemon < I >::update_status()
{
    ...
    ceph::JSONFormatter f;
    {
        ...
        for (auto &pool_pair : m_pools) {
            ...
            for (auto &callout : pool_pair.second.
                 callouts) {
                f.open_object_section(stringify(callout.first).c_str());
                f.dump_string("level",
                              stringify(callout.second.level).c_str());
                f.dump_string("text", callout.second.text.c_str());
                f.close_section();
            }
            f.close_section();  // callouts

            for (auto &attribute : pool_pair.second.
                 attributes) {
                AttributeDumpVisitor attribute_dump_visitor(&f,
                        attribute.
                        first);
                boost::apply_visitor(attribute_dump_visitor,
                                     attribute.second);
            }

            if (!pool_pair.second.ns_attributes.empty()) {
                f.open_object_section("namespaces");
                for (auto &[ns, attributes] : pool_pair.second.
                     ns_attributes) {
    ...
    int r =
    m_rados->service_daemon_update_status({ {"json", ss.str()} });
    if (r < 0) {
        derr << "failed to update service daemon status: " <<
             cpp_strerror(r)
             << dendl;
    }
}
