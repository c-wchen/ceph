...
public:
    MigrationProgressContext(librados::IoCtx &io_ctx,
        ...
        : m_io_ctx(io_ctx), m_header_oid(header_oid), m_state(state),
          ...
          m_lock(ceph::
                 make_mutex(util::
                            unique_lock_name
                            ("librbd::api::MigrationProgressContext",
                             this)))
    {
    ...
    {
        ldout(m_cct,
              20) << "offset=" << offset << ", total=" << total <<
                  dendl;

        m_prog_ctx->update_progress(offset, total);

        std::string description =
            stringify(offset * 100 / total) + "% complete";
...
private:
    ...
    void send_state_description_update(const std::
                                       string &description)
    {
    ...
    void set_state_description()
    {
        ...
        cls_client::migration_set_state(&op, m_state,
                                        m_state_description);

        using klass = MigrationProgressContext;
        librados::AioCompletion *comp =
            create_rados_callback < klass,
            &klass::handle_set_state_description > (this);
    ...
    void handle_set_state_description(int r)
    {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to update migration state: " <<
                         cpp_strerror(r)
                         << dendl;
    ...
    void wait_for_in_flight_updates()
    {
        ...
        m_cond.wait(locker, [this] {
            return m_in_flight_state_updates <= 0;
        }
                   );
    }
};

int trash_search(librados::IoCtx &io_ctx,
...
template < typename I >
int open_images(librados::IoCtx &io_ctx,
                const std::string &image_name,
...
{
    CephContext *cct =
        reinterpret_cast < CephContext * >(io_ctx.cct());
    ...
    *dst_image_ctx = nullptr;
    ...
    ldout(cct,
          10) << "trying to open image by name " << io_ctx.
              ...
              << "/" << image_name << dendl;
    auto image_ctx =
        I::create(image_name, "", nullptr, io_ctx, false);
    int r = image_ctx->state->open(OPEN_FLAG_IGNORE_MIGRATING);
    if (r == -ENOENT) {
        // presume user passed the source image so we need to search the trash
        ldout(cct,
              10) << "Source image is not found. Trying trash" <<
                  dendl;

        std::string src_image_id;
        r = trash_search(io_ctx, RBD_TRASH_IMAGE_SOURCE_MIGRATION,
                         image_name, &src_image_id);
        if (r < 0) {
            lderr(cct) << "failed to determine image id: " <<
                       cpp_strerror(r)
                       << dendl;
            return r;
        }
        ...
        ldout(cct,
              10) << "source image id from trash: " << src_image_id
                  << dendl;
        image_ctx =
            I::create(image_name, src_image_id, nullptr, io_ctx,
                      false);
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            lderr(cct) << "failed to open image: " <<
    ...
    BOOST_SCOPE_EXIT_TPL(&r, &image_ctx, src_image_ctx,
                         dst_image_ctx) {
        if (r != 0) {
    ...
    r = cls_client::migration_get(&image_ctx->md_ctx,
                                  image_ctx->header_oid,
                                  &migration_spec);
    if (r < 0) {
        lderr(cct) << "failed retrieving migration header: " <<
                   cpp_strerror(r)
                   << dendl;
        return r;
    }
    ...
    if (migration_spec.header_type ==
        ...
        *src_migration_spec = migration_spec;
        ...
        image_ctx->state->close();
        image_ctx =
            I::create(image_name, image_id, nullptr, io_ctx, false);

        if (!skip_open_dst_image) {
            ldout(cct,
                  10) << "re-opening the destination image" <<
                      dendl;
            r = image_ctx->state->open(0);
            if (r < 0) {
                image_ctx = nullptr;
                lderr(cct) <<
                           "failed to re-open destination image: " <<
                           cpp_strerror(r)
                           << dendl;
                return r;
            }
    ...
    if (*src_image_ctx == nullptr &&
        ...
        r = util::create_ioctx(io_ctx, "source image",
        ...
        *src_image_ctx =
            I::create(other_image_name, other_image_id, nullptr,
                      other_io_ctx, false);
    } else if (*dst_image_ctx == nullptr) {
        r = util::create_ioctx(io_ctx, "destination image",
                               migration_spec.pool_id,
                               migration_spec.pool_namespace,
                               &other_io_ctx);
        if (r < 0) {
    ...
    if (other_image_ctx != nullptr) {
        ...
        if (r < 0) {
            lderr(cct) << "failed to open " << other_image_type <<
                       " image " << other_io_ctx.get_pool_name()
                       << "/" << (other_image_id.
                                  empty() ? other_image_name :
                                  other_image_id)
                       << ": " << cpp_strerror(r) << dendl;
...
{
public:
    SteppedProgressContext(ProgressContext *progress_ctx,
                           size_t total_steps)
    ...
    {
        return m_progress_ctx->update_progress(object_number +
                                               (object_count *
                                                (m_current_step -
                                                        1)),
                                               object_count *
                             ...
                             ImageOptions &opts)
{
    ...
    ldout(cct,
          10) << io_ctx.
              get_pool_name() << "/" << image_name << " -> " << dest_io_ctx.
              get_pool_name() << "/" << dest_image_name << ", opts=" << opts
              << dendl;
    ...
    if (r < 0) {
        lderr(cct) << "failed to open image: " << cpp_strerror(r) <<
                   dendl;
        return r;
    }
    ...
    auto list_watchers_request =
        librbd::image::ListWatchersRequest < I >::create(*src_image_ctx,
            flags,
            &watchers,
            &on_list_watchers);
    ...
    if (r < 0) {
        lderr(cct) << "failed listing watchers:" << cpp_strerror(r) <<
                   dendl;
        return r;
    }
    if (!watchers.empty()) {
    ...
    if (format != 2) {
        lderr(cct) << "unsupported destination image format: " << format
                   << dendl;
        return -EINVAL;
    }
    ...
    auto dst_image_ctx =
        I::create(dest_image_name, util::generate_image_id(dest_io_ctx),
                  nullptr,
                  dest_io_ctx, false);
    ...
    cls::rbd::MigrationSpec dst_migration_spec {
        ...
        src_image_ctx->md_ctx.get_id(),
                             src_image_ctx->md_ctx.get_namespace(), src_image_ctx->name,
        src_image_ctx->id, "", {
        }
        , 0, false,
    ...
    Migration migration(src_image_ctx, dst_image_ctx,
                        dst_migration_spec, opts, nullptr);
    r = migration.prepare();
...
template < typename I >
int Migration < I >::prepare_import(const std::string &source_spec,
                                    librados::IoCtx &dest_io_ctx,
                                    const std::
                                    ...
                                    ImageOptions &opts)
{
    ...
    auto req =
        migration::OpenSourceImageRequest < I >::create(dest_io_ctx,
            nullptr,
    CEPH_NOSNAP, {
        -1, "", "",
        "", source_spec,
    ...
    int r = open_ctx.wait();
    if (r < 0) {
        lderr(cct) << "failed to open source image: " << cpp_strerror(r)
                   << dendl;
        return r;
    }
    ...
    if (image_format != 2) {
        lderr(cct) << "unsupported destination image format: " <<
                   image_format << dendl;
        return -EINVAL;
    }
                             ...
                             ProgressContext &prog_ctx)
{
    ...
    ldout(cct,
          10) << io_ctx.get_pool_name() << "/" << image_name << dendl;
    ...
    cls::rbd::MigrationSpec dst_migration_spec;
    int r =
        open_images(io_ctx, image_name, &src_image_ctx, &dst_image_ctx,
    ...
    if (r < 0) {
        lderr(cct) << "failed to refresh destination image: " <<
                   cpp_strerror(r)
                   << dendl;
        return r;
    }
    ...
    if (dst_migration_spec.state != cls::rbd::MIGRATION_STATE_PREPARED
        ...
        lderr(cct) << "current migration state is '" <<
                   dst_migration_spec.
                   state << "' (should be 'prepared')" << dendl;
        return -EINVAL;
    }
    ...
    *_dout << " -> " << dst_image_ctx->md_ctx.get_pool_name() << "/"
    ...
    ImageOptions opts;
    Migration migration(src_image_ctx, dst_image_ctx,
                        dst_migration_spec, opts, &prog_ctx);
    r = migration.execute();
    if (r < 0) {
        return r;
    }
...
template < typename I >
int Migration < I >::abort(librados::IoCtx &io_ctx,
                           const std::string &image_name,
                           ProgressContext &prog_ctx)
{
    CephContext *cct = reinterpret_cast < CephContext * >(io_ctx.cct());
                            ...
                            ProgressContext &prog_ctx)
{
    ...
    if (dst_migration_spec.state != cls::rbd::MIGRATION_STATE_EXECUTED) {
        lderr(cct) << "current migration state is '" <<
                   dst_migration_spec.
                   state << "' (should be 'executed')" << dendl;
        dst_image_ctx->state->close();
        if (src_image_ctx != nullptr) {
            src_image_ctx->state->close();
        }
...
template < typename I >
int Migration < I >::get_source_spec(I *image_ctx,
                                     std::string *source_spec)
{
    ...
    if (migration_info.empty()) {
        ...
        cls::rbd::MigrationSpec migration_spec;
        int r =
            cls_client::migration_get(&image_ctx->md_ctx,
                                      image_ctx->header_oid,
                                      &migration_spec);
        if (r == -ENOENT) {
            return r;
    ...
    if (!migration_info.source_spec.empty()) {
        ...
        *source_spec =
            migration::NativeFormat <
            I >::build_source_spec(migration_info.pool_id,
...
Migration < I >::Migration(ImageCtx *src_image_ctx,
                           ...
                           const cls::rbd::
                           MigrationSpec &dst_migration_spec,
                           ImageOptions &opts,
                           ProgressContext *prog_ctx)
    :    m_cct(dst_image_ctx->cct), m_src_image_ctx(src_image_ctx),
         m_dst_image_ctx(dst_image_ctx), m_dst_io_ctx(dst_image_ctx->md_ctx),
         ...
         m_src_migration_spec(cls::rbd::MIGRATION_HEADER_TYPE_SRC,
                              m_dst_io_ctx.get_id(),
                              m_dst_io_ctx.get_namespace(), m_dst_image_name,
                              m_dst_image_id, "",
{
}, 0, m_mirroring, m_mirror_image_mode,
m_flatten, dst_migration_spec.state,
dst_migration_spec.state_description), m_dst_migration_spec(dst_migration_spec)
{
...
template < typename I > int Migration < I >::execute()
{
    ...
    {
        MigrationProgressContext dst_prog_ctx(m_dst_image_ctx->md_ctx,
                                              m_dst_image_ctx->
                                              header_oid,
                                              cls::rbd::
                                              MIGRATION_STATE_EXECUTING,
                                              m_prog_ctx);
        ...
        while (true) {
            r = m_dst_image_ctx->operations->
                migrate(*
                        (src_prog_ctx ? &src_prog_ctx.
                         value() : &dst_prog_ctx));
            if (r == -EROFS) {
                std::shared_lock owner_locker {
                    m_dst_image_ctx->owner_lock};
                if (m_dst_image_ctx->exclusive_lock != nullptr &&
                    !m_dst_image_ctx->exclusive_lock->accept_ops()) {
                    ldout(m_cct,
                          5) << "lost exclusive lock, retrying remote"
                             << dendl;
                    continue;
                }
            }
    ...
    if (r < 0) {
        lderr(m_cct) << "migration failed: " << cpp_strerror(r) <<
                     dendl;
        return r;
    }
...
template < typename I > int Migration < I >::abort()
{
    ...
    if (m_src_image_ctx != nullptr) {
        ...
        if (m_src_image_ctx->exclusive_lock != nullptr &&
            ...
            r = ctx.wait();
            if (r < 0) {
                lderr(m_cct) << "error acquiring exclusive lock: " <<
                             cpp_strerror(r)
                             << dendl;
                return r;
            }
    ...
    if (r < 0) {
        ldout(m_cct,
              1) << "failed to open destination image: " <<
                 cpp_strerror(r)
                 << dendl;
        ...
        int flags =
            librbd::image::
            LIST_WATCHERS_FILTER_OUT_MY_INSTANCE | librbd::image::
            LIST_WATCHERS_FILTER_OUT_MIRROR_INSTANCES;
        C_SaferCond on_list_watchers;
        auto list_watchers_request =
            librbd::image::ListWatchersRequest <
            I >::create(*m_dst_image_ctx, flags, &watchers,
                        &on_list_watchers);
        list_watchers_request->send();
        r = on_list_watchers.wait();
        if (r < 0) {
            lderr(m_cct) << "failed listing watchers:" <<
                         cpp_strerror(r) << dendl;
            return r;
        }
        if (!watchers.empty()) {
            lderr(m_cct) <<
                         "image has watchers - cannot abort migration" << dendl;
            return -EBUSY;
        }
        ...
        SteppedProgressContext progress_ctx(m_prog_ctx,
                                            (m_src_image_ctx !=
                                             nullptr ? 2 : 1));
        if (m_src_image_ctx != nullptr) {
            // copy dst HEAD -> src HEAD
            revert_data(m_dst_image_ctx, m_src_image_ctx,
                        &progress_ctx);
        ...
        if (r < 0) {
            lderr(m_cct) << "failed listing snapshots: " <<
                         cpp_strerror(r)
                         << dendl;
            return r;
        }
        ...
        for (auto &snap : snaps) {
            ...
            int r =
                Snapshot < I >::remove(m_dst_image_ctx,
                                       snap.name.c_str(),
                                       RBD_SNAP_REMOVE_UNPROTECT,
                                       prog_ctx);
            if (r < 0) {
                lderr(m_cct) << "failed removing snapshot: " <<
                             cpp_strerror(r)
                             << dendl;
                return r;
            }
        ...
        auto req =
            librbd::image::RemoveRequest <>::create(dst_io_ctx,
                m_dst_image_ctx,
                false, false,
                progress_ctx,
                asio_engine->
...
template < typename I > int Migration < I >::commit()
{
    ...
    r = enable_mirroring(m_dst_image_ctx, m_mirroring,
                         m_mirror_image_mode);
    if (r < 0) {
        return r;
    }
...
template < typename I >
int Migration < I >::set_state(I *image_ctx,
                               const std::
                                    ...
                                    librbd::snap_info_t > *snaps)
{
    ...
    for (auto &snap : *snaps) {
        ...
        r = Snapshot < I >::get_namespace_type(image_ctx, snap.id,
                                               &namespace_type);
        if (r < 0) {
            lderr(m_cct) << "error getting snap namespace type: " <<
                         cpp_strerror(r)
                         << dendl;
            return r;
        }
...
int Migration < I >::validate_src_snaps(I *image_ctx)
{
    ...
    for (auto &snap : snaps) {
        ...
        r = api::Image < I >::list_children(image_ctx, parent_spec,
                                            &child_images);
        if (r < 0) {
            lderr(m_cct) << "failed listing children: " <<
                         cpp_strerror(r)
                         << dendl;
            return r;
        }
...
int Migration < I >::set_src_migration(I *image_ctx)
{
    ...
    int r =
        cls_client::migration_set(&image_ctx->md_ctx,
                                  image_ctx->header_oid,
...
int Migration < I >::remove_migration(I *image_ctx)
{
    ...
    r = cls_client::migration_remove(&image_ctx->md_ctx,
                                     image_ctx->header_oid);
    if (r == -ENOENT) {
        r = 0;
    }
    if (r < 0) {
        lderr(m_cct) << "failed removing migration header: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
int Migration < I >::v1_unlink_src_image(I *image_ctx)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed removing " << image_ctx->
                     name << " from tmap: " << cpp_strerror(r) << dendl;
        return r;
    }
...
int Migration < I >::v2_unlink_src_image(I *image_ctx)
{
    ...
    if (image_ctx->exclusive_lock != nullptr &&
        ...
        int r = ctx.wait();
        if (r < 0) {
            lderr(m_cct) << "error releasing exclusive lock: " <<
                         cpp_strerror(r)
                         << dendl;
            return r;
        }
    ...
    int r =
        Trash < I >::move(image_ctx->md_ctx,
                          RBD_TRASH_IMAGE_SOURCE_MIGRATION,
                          image_ctx->name, 0);
    if (r < 0) {
        lderr(m_cct) << "failed moving image to trash: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
int Migration < I >::v1_relink_src_image(I *image_ctx)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed adding " << image_ctx->
                     name << " to tmap: " << cpp_strerror(r) << dendl;
        return r;
    }
...
int Migration < I >::v2_relink_src_image(I *image_ctx)
{
    ...
    int r = Trash < I >::restore(image_ctx->md_ctx, {
        cls::rbd::
        TRASH_IMAGE_SOURCE_MIGRATION
    },
    image_ctx->id, image_ctx->name);
    if (r < 0) {
        lderr(m_cct) << "failed restoring image from trash: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
int Migration < I >::create_dst_image(I **image_ctx)
{
    ...
    cls::rbd::ParentImageSpec parent_spec;
    {
        ...
        if (!m_src_image_ctx->snap_info.empty()) {
            parent_spec =
                m_src_image_ctx->snap_info.begin()->second.parent.spec;
        } else {
    ...
    if (parent_spec.pool_id == -1) {
        auto *req =
            image::CreateRequest < I >::create(config, m_dst_io_ctx,
                                               m_dst_image_name,
                                               m_dst_image_id, size,
                                               m_image_options,
                                               image::
        ...
        auto *req =
            image::CloneRequest < I >::create(config, parent_io_ctx,
    ...
    r = on_create.wait();
    if (r < 0) {
        lderr(m_cct) << "header creation failed: " << cpp_strerror(r) <<
                     dendl;
        return r;
    }
    ...
    *image_ctx = nullptr;   // prevent prepare from cleaning up the ImageCtx
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to open newly created header: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    {
        std::shared_lock owner_locker {
            dst_image_ctx->owner_lock};
        r = dst_image_ctx->operations->
            prepare_image_update(exclusive_lock::
                                 OPERATION_REQUEST_TYPE_GENERAL, true);
        if (r < 0) {
    ...
    auto snapshot_copy_req =
        librbd::deep_copy::SnapshotCopyRequest <
        I >::create(m_src_image_ctx, dst_image_ctx, 0, CEPH_NOSNAP, 0,
                    m_flatten,
                    m_src_image_ctx->op_work_queue, &snap_seqs,
                    &on_snapshot_copy);
    snapshot_copy_req->send();
    r = on_snapshot_copy.wait();
    if (r < 0) {
        lderr(m_cct) << "failed to copy snapshots: " << cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    if (!m_src_image_ctx->header_oid.empty()) {
        ...
        auto metadata_copy_req =
            ...
            I >::create(m_src_image_ctx, dst_image_ctx,
                        &on_metadata_copy);
        metadata_copy_req->send();
        r = on_metadata_copy.wait();
        if (r < 0) {
            lderr(m_cct) << "failed to copy metadata: " <<
                         cpp_strerror(r) << dendl;
            return r;
        }
    }
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to set migration header: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    if (m_dst_migration_spec.source_spec.empty()) {
        r = dst_image_ctx->state->refresh();
        if (r < 0) {
            lderr(m_cct) << "failed to refresh destination image: " <<
                         cpp_strerror(r)
                         << dendl;
            return r;
        }
...
template < typename I >
int Migration < I >::remove_group(I *image_ctx,
                                  group_info_t *group_info)
{
    int r =
        librbd::api::Group < I >::image_get_group(image_ctx,
            group_info);
    if (r < 0) {
        lderr(m_cct) << "failed to get image group: " << cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    r = librbd::api::Group < I >::image_remove_by_id(group_ioctx,
        ...
        c_str());
    if (r < 0) {
        lderr(m_cct) << "failed to remove image from group: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
template < typename I >
int Migration < I >::add_group(I *image_ctx,
                               group_info_t &group_info)
{
    if (group_info.pool == -1) {
        return 0;
    ...
    IoCtx group_ioctx;
    int r =
        util::create_ioctx(image_ctx->md_ctx, "group", group_info.pool,
                           { },
                           &group_ioctx);
    if (r < 0) {
        return r;
    ...
    r = librbd::api::Group < I >::image_add(group_ioctx,
                                            group_info.name.c_str(),
                                            image_ctx->md_ctx,
                                            image_ctx->name.c_str());
    if (r < 0) {
        lderr(m_cct) << "failed to add image to group: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
template < typename I >
int Migration < I >::update_group(I *from_image_ctx,
                                  I *to_image_ctx)
{
    ldout(m_cct, 10) << dendl;
...
template < typename I >
int Migration < I >::disable_mirroring(I *image_ctx,
                                       bool *was_enabled,
                                       ...
                                       mirror_image_mode)
{
    *was_enabled = false;
    ...
    int r =
        cls_client::mirror_image_get(&image_ctx->md_ctx, image_ctx->id,
                                     &mirror_image);
    if (r == -ENOENT) {
        ldout(m_cct,
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to retrieve mirror image: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    C_SaferCond ctx;
    auto req =
        mirror::DisableRequest < I >::create(image_ctx, false, true,
            &ctx);
    req->send();
    r = ctx.wait();
    if (r < 0) {
        lderr(m_cct) << "failed to disable mirroring: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
template < typename I >
int Migration < I >::enable_mirroring(I *image_ctx,
                                      bool was_enabled,
                                      cls::rbd::
                                      MirrorImageMode
                                      mirror_image_mode)
{
    ...
    int r =
        cls_client::mirror_mode_get(&image_ctx->md_ctx, &mirror_mode);
    if (r < 0 && r != -ENOENT) {
        lderr(m_cct) << "failed to retrieve mirror mode: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    C_SaferCond ctx;
    auto req =
        mirror::EnableRequest < I >::create(image_ctx,
                                            mirror_image_mode, "",
                                            false, &ctx);
    req->send();
    r = ctx.wait();
    if (r < 0) {
        lderr(m_cct) << "failed to enable mirroring: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
template < typename I >
int Migration < I >::relink_children(I *from_image_ctx,
                                     I *to_image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    std::vector < librbd::snap_info_t > snaps;
    int r =
        list_src_snaps(migration_abort ? to_image_ctx : from_image_ctx,
                       &snaps);
    if (r < 0) {
        return r;
    }
    ...
    for (auto it = snaps.begin(); it != snaps.end(); it++) {
        ...
        if (from_image_ctx != m_src_image_ctx) {
            ...
            std::shared_lock src_image_locker {
                to_image_ctx->image_lock};
            cls::rbd::ParentImageSpec src_parent_spec {
                to_image_ctx->md_ctx.get_id(),
                                    to_image_ctx->md_ctx.get_namespace(),
                                    to_image_ctx->id, snap.id};
            r = api::Image < I >::list_children(to_image_ctx,
                                                src_parent_spec,
                                                &src_child_images);
            if (r < 0) {
                lderr(m_cct) << "failed listing children: " <<
            ...
            if (snap.id == CEPH_NOSNAP) {
                ldout(m_cct,
                      5) << "skipping snapshot " << snap.name << dendl;
                continue;
            }
        }
        ...
        std::vector < librbd::linked_image_spec_t > child_images;
        {
            ...
            r = api::Image < I >::list_children(from_image_ctx,
                                                parent_spec,
                                                &child_images);
            if (r < 0) {
                lderr(m_cct) << "failed listing children: " <<
        ...
        for (auto &child_image : child_images) {
            r = relink_child(from_image_ctx, to_image_ctx, snap,
                             child_image, migration_abort, true);
            if (r < 0) {
                return r;
            }
        ...
        for (auto &child_image : src_child_images) {
            r = relink_child(from_image_ctx, to_image_ctx, snap,
                             child_image, migration_abort, false);
            if (r < 0) {
                return r;
            }
        }
...
template < typename I >
int Migration < I >::relink_child(I *from_image_ctx,
                                  I *to_image_ctx,
                                  const librbd::
                                  snap_info_t &from_snap,
                                  const librbd::
                                  linked_image_spec_t &child_image,
                                  bool migration_abort,
                                  bool reattach_child)
{
    ldout(m_cct,
          10) << from_snap.name << " " << child_image.
              pool_name << "/" << child_image.
              pool_namespace << "/" << child_image.
              image_name << " (migration_abort=" << migration_abort <<
    ...
    librados::snap_t to_snap_id;
    {
        std::shared_lock image_locker {
            to_image_ctx->image_lock};
        to_snap_id =
            to_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace(),
                                      from_snap.name);
        if (to_snap_id == CEPH_NOSNAP) {
            lderr(m_cct) << "no snapshot " << from_snap.
                         name << " on destination image" << dendl;
            return -ENOENT;
        }
    }
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to open child image: " <<
                     cpp_strerror(r) << dendl;
        return r;
    }
    ...
    {
        std::shared_lock image_locker {
            child_image_ctx->image_lock};

        // use oldest snapshot or HEAD for parent spec
        if (!child_image_ctx->snap_info.empty()) {
            parent_spec =
                child_image_ctx->snap_info.begin()->second.parent.spec;
            parent_overlap =
                child_image_ctx->snap_info.begin()->second.parent.
                overlap;
        } else {
    ...
    if (migration_abort &&
        ...
        if (parent_spec.pool_id != from_image_ctx->md_ctx.get_id() ||
            ...
            lderr(m_cct) << "parent is not source image: " <<
                         parent_spec.pool_id << "/" << parent_spec.
                         pool_namespace << "/" << parent_spec.
                         image_id << "@" << parent_spec.snap_id << dendl;
            return -ESTALE;
        }
        ...
        auto reattach_parent_req =
            image::AttachParentRequest < I >::create(*child_image_ctx,
                parent_spec,
                parent_overlap,
                true,
                &on_reattach_parent);
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to re-attach parent: " <<
                         cpp_strerror(r) << dendl;
            return r;
        }
    }
    ...
    if (reattach_child) {
        ...
        auto reattach_child_req =
            image::AttachChildRequest < I >::create(child_image_ctx,
                to_image_ctx,
                to_snap_id,
                from_image_ctx,
                from_snap.id,
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to re-attach child: " <<
                         cpp_strerror(r) << dendl;
            return r;
        }
    }
...
int Migration < I >::remove_src_image(I **image_ctx)
{
    ...
    for (auto it = snaps.rbegin(); it != snaps.rend(); it++) {
        ...
        int r = Snapshot < I >::remove(src_image_ctx, snap.name.c_str(),
                                       ...
                                       prog_ctx);
        if (r < 0) {
            lderr(m_cct) << "failed removing source image snapshot '" <<
    ...
    C_SaferCond on_remove;
    auto req =
        librbd::image::RemoveRequest < I >::create(src_io_ctx,
            src_image_ctx, false,
            true, *m_prog_ctx,
            asio_engine->
            get_work_queue(),
    ...
    *image_ctx = nullptr;
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(m_cct) << "failed removing source image: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
...
template < typename I >
int Migration < I >::revert_data(I *src_image_ctx,
                                 I *dst_image_ctx,
                                 ProgressContext *prog_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    if (r < 0) {
        lderr(m_cct) << "failed retrieving migration header: " <<
                     cpp_strerror(r)
                     << dendl;
        return r;
    }
    ...
    ldout(m_cct,
          20) << "src_snap_id_start=" << src_snap_id_start << ", " <<
    ...
    auto request =
        deep_copy::ImageCopyRequest < I >::create(src_image_ctx,
            dst_image_ctx,
            src_snap_id_start,
            src_snap_id_end,
            dst_snap_id_start,
    ...
    if (r < 0) {
        lderr(m_cct) <<
                     "error reverting destination image data blocks back to " <<
