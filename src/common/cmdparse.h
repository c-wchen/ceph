...
typedef boost::variant < std::string,
        ...
        std::vector < int64_t >, std::vector < double >>cmd_vartype;
...
void dump_cmddesc_to_json(ceph::Formatter *jf,
                          ...
                          const std::string &perm, uint64_t flags);
bool cmdmap_from_json(const std::vector < std::string > &cmd,
                      cmdmap_t *mapp, std::ostream &ss);
void cmdmap_dump(const cmdmap_t &cmdmap, ceph::Formatter *f);
void handle_bad_get(CephContext *cct, const std::string &k,
                    const char *name);
...
bool cmd_getval_compat_cephbool(const cmdmap_t &cmdmap,
                                const std::string &k, bool &val);

template < typename T >
bool cmd_getval(const cmdmap_t &cmdmap, std::string_view k, T &val)
{
...
template < typename T > void cmd_putval(CephContext *cct, cmdmap_t &cmdmap, std::string_view k,
                                        const T &val)
{
    cmdmap.insert_or_assign(std::string {
        k}, val);
}

bool validate_cmd(const std::string &desc,
                  const cmdmap_t &cmdmap, std::ostream &os);
