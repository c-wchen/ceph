...
namespace crimson::os::seastore::onode {
    ...
    template < IsFullKey Key >
        const laddr_packed_t *internal_sub_items_t::
        insert_at(NodeExtentMutable & mut,
                  const internal_sub_items_t & sub_items, const Key & key,
                  const laddr_t & value, index_t index, node_offset_t size,
        ...
        const char *p_shift_end =
            reinterpret_cast <
            const char *>(sub_items.p_first_item + 1 - index);
...
#define IA_TEMPLATE(Key)                                                    \
  template const laddr_packed_t* internal_sub_items_t::insert_at<Key>(      \
    ...
    node_offset_t internal_sub_items_t::trim_until(NodeExtentMutable & mut,
    ...
    node_offset_t internal_sub_items_t::erase_at(NodeExtentMutable & mut,
                                                 const internal_sub_items_t &
                                                 sub_items, index_t index,
                                                 const char *p_left_bound) {
        assert(index < sub_items.keys());
    ...
    template < KeyT KT >
        void internal_sub_items_t::Appender <
        KT >::append(const internal_sub_items_t & src, index_t from,
                     index_t items) {
        ...
        p_append -= size;
        p_mut->copy_in_absolute(p_append, src.p_first_item + 1 - from - items,
                                size);
    }

    template < KeyT KT >
        void internal_sub_items_t::Appender < KT >::append(const full_key_t <
                                                           KT > &key,
                                                           const laddr_t &
                                                           value,
    ...
    template < IsFullKey Key >
        const value_header_t *leaf_sub_items_t::
        insert_at(NodeExtentMutable & mut, const leaf_sub_items_t & sub_items,
        ...
        assert(p_insert + sizeof(snap_gen_t) + sizeof(node_offset_t) ==
               p_shift_end);
        ...
        for (auto i = index; i < sub_items.keys(); ++i) {
            ...
            mut.copy_in_absolute((void *)&offset_i,
                                 node_offset_t(offset_i.value + item_size));
        }

        // d. [item(index-1) ... item(0) ... offset(index)] <<< sizeof(node_offset_t)
        const char *p_offset = (index == 0 ?
                                (const char *)&sub_items.get_offset(0) +
                                sizeof(node_offset_t) : (const char *)
                                &sub_items.get_offset(index - 1));
        ...
        // e. insert offset
        node_offset_t offset_to_item_start =
            item_size + sub_items.get_offset_to_end(index);
        mut.copy_in_absolute(const_cast <
                             char *>(p_shift_end) - sizeof(node_offset_t),
                             offset_to_item_start);

        // f. update num_sub_keys
        mut.copy_in_absolute((void *)sub_items.p_num_keys,
                             num_keys_t(sub_items.keys() + 1));
    ...
    node_offset_t leaf_sub_items_t::trim_until(NodeExtentMutable & mut,
    ...
    node_offset_t leaf_sub_items_t::erase_at(NodeExtentMutable & mut,
        ...
        for (auto i = index + 1; i < sub_items.keys(); ++i) {
            const node_offset_packed_t & offset_i = sub_items.get_offset(i);
            mut.copy_in_absolute((void *)&offset_i,
                                 node_offset_t(offset_i.value -
                                               item_erase_size));
        }

        // b. kv[index-1] ... kv[0] ... offset[index+1] >> sizeof(node_offset_t)
        mut.shift_absolute(p_item_end, p_offset_end - p_item_end,
                           sizeof(node_offset_t));

        // c. ... kv[n] ... kv[index+1] >> item_erase_size
        mut.shift_absolute(p_left_bound, p_item_start - p_left_bound,
                           erase_size);
...
// explicit deduction guide
    ...
    template < KeyT KT >
        void leaf_sub_items_t::Appender <
        KT >::append(const leaf_sub_items_t & src, index_t from,
                     index_t items) {
        if (p_append) {
            // append from empty
            ...
            std::size_t src_items_size =
                src.get_item_end(from) - p_src_items_start;
            p_appended =
                const_cast <
                char *>(p_items_start) - src_offsets_size - src_items_size;
            p_mut->copy_in_absolute(p_appended, p_src_items_start,
                                    src_items_size);
        }
    }
    ...
    template < KeyT KT > char *leaf_sub_items_t::Appender < KT >::wrap() {
        ...
        for (auto i = 0u; i < cnt; ++i) {
            ...
            std::visit(overloaded {
                       ...
                       node_offset_t offset; for (auto i = arg.from;
                                                  i < arg.from + arg.items;
                                                  ++i) {
                       ...
                       ,[&](const kv_item_t & arg) {
                       last_offset +=
                       sizeof(snap_gen_t) + arg.value_config.allocation_size();
                       p_cur -= sizeof(node_offset_t);
        ...
        for (auto i = 0u; i < cnt; ++i) {
            ...
            std::visit(overloaded {
                       [&](const range_items_t & arg) {
                       auto _p_start =
                       op_src->get_item_end(arg.from + arg.items);
                       size_t _len = op_src->get_item_end(arg.from) - _p_start;
                       p_cur -= _len;
                       p_mut->copy_in_absolute(p_cur, _p_start, _len);},
                       [&](const kv_item_t & arg) {
                       assert(pp_value); p_cur -= sizeof(snap_gen_t);
                       p_mut->copy_in_absolute(p_cur,
                                               snap_gen_t::from_key(*arg.
                                                                    p_key));
                       p_cur -= arg.value_config.allocation_size();
                       auto p_value =
                       reinterpret_cast < value_header_t * >(p_cur);
