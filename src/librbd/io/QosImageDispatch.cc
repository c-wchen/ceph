...
namespace librbd {
    namespace io {
        ...
        namespace {
            ...
            } uint64_t calculate_tokens(bool read_op, uint64_t extent_length,
                                        uint64_t flag) {
                if (read_op
                    && ((flag & IMAGE_DISPATCH_FLAG_QOS_WRITE_MASK) != 0)) {
                    return 0;
                }
                else if (!read_op
                         && ((flag & IMAGE_DISPATCH_FLAG_QOS_READ_MASK) != 0)) {
                    return 0;
                }
      ...
      :    
        ...
        m_flush_tracker(new FlushTracker < I > (image_ctx)) {
          ...
          for (auto[flag, name]:throttle_flags) {
                m_throttles.emplace_back(flag,
                                         new TokenBucketThrottle(cct, name, 0,
                                                                 0, timer,
                                                                 timer_lock));
            }
        }
        ...
        template < typename I >
            void QosImageDispatch < I >::apply_qos_limit(uint64_t flag,
                                                         uint64_t limit,
                                                         uint64_t burst,
                                                         uint64_t burst_seconds)
        {
          ...
          for (auto pair:m_throttles) {
            ...
            if (r < 0) {
                lderr(cct) << throttle->
                    get_name() << ": invalid qos parameter: " << "burst(" <<
                    burst << ") is less than " << "limit(" << limit << ")" <<
                    dendl;
        ...
        template < typename I >
            bool QosImageDispatch < I >::read(AioCompletion * aio_comp, Extents
                                              && image_extents, ReadResult
                                              && read_result,
            ...
            auto cct = m_image_ctx->cct;
            ldout(cct,
                  20) << "tid=" << tid << ", image_extents=" << image_extents <<
                dendl;
        ...
        template < typename I >
            bool QosImageDispatch < I >::write(AioCompletion * aio_comp, Extents
        ...
        template < typename I >
            bool QosImageDispatch < I >::write_same(AioCompletion * aio_comp,
                                                    Extents
                                                    && image_extents, bufferlist
                                                    && bl, int op_flags,
                                                    const ZTracer::
        ...
        template < typename I >
            bool QosImageDispatch <
            I >::compare_and_write(AioCompletion * aio_comp, Extents
                                   && image_extents, bufferlist
                                   && cmp_bl, bufferlist
        ...
        template < typename I >
            bool QosImageDispatch < I >::set_throttle_flag(std::atomic <
                                                           uint32_t >
                                                           *image_dispatch_flags,
                                                           uint32_t flag) {
        ...
        template < typename I >
            bool QosImageDispatch < I >::needs_throttle(bool read_op,
                                                        const Extents &
                                                        image_extents,
                                                        uint64_t tid,
                                                        std::atomic < uint32_t >
            ...
            if (!read_op) {
                ...
                *on_finish =
                    new
                    LambdaContext([this, tid, on_finish = *on_finish] (int r) {
          ...
          for (auto[flag, throttle]:m_throttles) {
                if ((qos_enabled_flag & flag) == 0) {
                    all_qos_flags_set =
                        set_throttle_flag(image_dispatch_flags, flag);
                    continue;
                }

                auto tokens = calculate_tokens(read_op, extent_length, flag);
                if (tokens > 0 &&
                    throttle->get(tokens, this,
                                  &QosImageDispatch <
                                  I >::handle_throttle_ready, Tag {
                                  image_dispatch_flags, on_dispatched}
                                  , flag)) {
        ...
        template < typename I >
            void QosImageDispatch < I >::handle_throttle_ready(Tag
                                                               && tag,
                                                               uint64_t flag) {
            auto cct = m_image_ctx->cct;
