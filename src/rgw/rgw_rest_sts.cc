...
{
    STS::STSService _sts(s->cct, driver, s->user->get_id(),
                         s->auth.identity.get());
    sts = std::move(_sts);
    ...
    try {
        ...
        if (!s->principal_tags.empty()) {
            auto res =
                p.eval(s->env, *s->auth.identity, rgw::IAM::stsTagSession,
                       boost::none);
...
void RGWSTSGetSessionToken::execute(optional_yield y)
{
    ...
    STS::STSService sts(s->cct, driver, s->user->get_id(),
                        s->auth.identity.get());
...
int RGWSTSAssumeRoleWithWebIdentity::get_params()
{
    ...
    if (roleArn.empty() || roleSessionName.empty() || sub.empty()
        || aud.empty()) {
        ldpp_dout(this,
                  0) <<
...
void RGWSTSAssumeRoleWithWebIdentity::execute(optional_yield y)
{
    ...
    if (op_ret == 0) {
        ...
        encode_json("PackedPolicySize",
                    response.assumeRoleResp.packedPolicySize, s->formatter);
        s->formatter->close_section();
        s->formatter->close_section();
    }
}
...
void RGWSTSAssumeRole::execute(optional_yield y)
{
    ...
    if (op_ret == 0) {
        ...
        s->formatter->close_section();
        encode_json("PackedPolicySize", response.packedPolicySize,
                    s->formatter);
        s->formatter->close_section();
        s->formatter->close_section();
    }
