...
template < typename T > class TestObjectPlayer:public RadosTestFixture {
  public:
    static const uint32_t max_fetch_bytes = T::max_fetch_bytes;

    journal::ObjectPlayerPtr create_object(const std::string & oid,
                                           uint8_t order) {
        journal::ObjectPlayerPtr object(new journal::
                                        ObjectPlayer(m_ioctx, oid + ".", 0,
                                                     *m_timer, m_timer_lock,
                                                     order, max_fetch_bytes));
        return object;
    }
