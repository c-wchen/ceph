                                        ...
                                        bufferlist > &encoded)
{
    ...
    for (unsigned i = 0; i < encoded.size(); i++) {
        ...
        EXPECT_EQ(0,
                  memcmp(encoded[i].c_str(), in.c_str() + i * chunk_size,
                         chunk_length));
    }
}
...
void IsaErasureCodeTest::encode_decode(unsigned object_size)
{
    ...
    // all chunks are available
    {
        ...
        EXPECT_EQ(0,
                  Isa._decode(set < int >(want_to_decode, want_to_decode + 2),
                              encoded, &decoded));
    ...
    // one data chunk is missing
    {
        ...
        map < int, bufferlist > decoded;
        EXPECT_EQ(0,
                  Isa._decode(set < int >(want_to_decode, want_to_decode + 1),
                              degraded, &decoded));
...
{
    {
        ...
        ASSERT_EQ(EC_ISA_ADDRESS_ALIGNMENT,
                  Isa.get_chunk_size(EC_ISA_ADDRESS_ALIGNMENT * k - 1));
        ASSERT_EQ(EC_ISA_ADDRESS_ALIGNMENT * 2,
                  Isa.get_chunk_size(EC_ISA_ADDRESS_ALIGNMENT * k + 1));
    }
    {
        ErasureCodeIsaDefault Isa(tcache);
...
#define LARGE_ENOUGH 2048
    ...
    for (int i = 0; i < k; i++) {
        EXPECT_EQ(0,
                  memcmp(encoded[i].c_str(), in.c_str() + (i * length),
                         length));
    }
    ...
    for (int l1 = 0; l1 < (k + m); l1++) {
        ...
        for (int l2 = l1 + 1; l2 < (k + m); l2++) {
            ...
            for (int l3 = l2 + 1; l3 < (k + m); l3++) {
                ...
                err =
                    DecodeAndVerify(Isa, degraded, want_to_decode, enc, length);
                EXPECT_EQ(0, err);
                cnt_cf++;
                for (int l4 = l3 + 1; l4 < (k + m); l4++) {
                    ...
                    err =
                        DecodeAndVerify(Isa, degraded, want_to_decode, enc,
                                        length);
                    EXPECT_EQ(0, err);
...
#define LARGE_ENOUGH 2048
    ...
    EXPECT_EQ(2516, cnt_cf);
    EXPECT_EQ(2516,
              tcache.getDecodingTableCacheSize(ErasureCodeIsaDefault::kCauchy));
}
