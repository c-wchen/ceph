...
namespace crimson::admin {
    ...
    tell_result_t::tell_result_t(int ret, std::string && err, ceph::bufferlist
                                 && out)
    :ret {
    ret}, err(std::move(err)), out(std::move(out)) {
    }
    ...
    void AdminSocket::register_command(std::unique_ptr < AdminSocketHook >
                                       &&hook) {
    ...
    ->std::variant < parsed_command_t, tell_result_t > {
        ...
        try {
            ...
            if (!cmdmap_from_json(cmd, &cmdmap, errss)) {
                logger().error("{}: incoming command error: {}", __func__,
                               errss.str());
                out.append("error:" s);
                out.append(errss.str());
                return tell_result_t {
                -EINVAL, "invalid json", std::move(out)};
            }
        }
    ...
    seastar::future <> AdminSocket::finalize_response(seastar::output_stream <
                                                      char >&out,
                                                      ceph::bufferlist
                                                      && msgs) {
        ...
        return out.write(reinterpret_cast < char *>(&response_length),
            ...
            .then([&out, outbuf_cont] {
                  return out.write(outbuf_cont.c_str());
                  }
        );
    }

    seastar::future <> AdminSocket::handle_command(crimson::net::
                                                   ConnectionRef conn,
                                                   boost::intrusive_ptr <
                                                   MCommand > m) {
        return execute_command(m->cmd,
                               std::move(m->get_data())).then([conn, tid =
                                                               m->
                                                               get_tid()](auto
                                                                          result)
                                                              {
    ...
    seastar::future <> AdminSocket::execute_line(std::string cmdline,
        ...
        ).then([&out, this] (auto result) {
               auto[ret, stderr, stdout] = std::move(result); if (ret < 0) {
    ...
    ->seastar::future < tell_result_t > {
        ...
        if (auto * parsed = std::get_if < parsed_command_t > (&maybe_parsed);
            parsed) {
            stringstream os;
            string desc {
            parsed->hook.desc};
            if (!validate_cmd(desc, parsed->params, os)) {
                ...
                return seastar::make_ready_future < tell_result_t >
                    (tell_result_t {
                     -EINVAL, "invalid command json", std::move(out)}
                );
            }
            return parsed->hook.call(parsed->params, parsed->format,
                                     std::move(buf));
        }
        else {
...
// '\0' which indicates the end of command
            ...
            for (auto c:buf) {
                ...
                if (c == '\0') {
                    ...
                    return seastar::make_ready_future <
                        ...
                        (consumption_result_type::
                         stop_consuming_type(std::move(buf)));
                }
                else {
                    line.push_back(c);
                }
    ...
    seastar::future <> AdminSocket::handle_client(seastar::input_stream <
                                                  char >&in,
                                                  seastar::output_stream <
                                                  char >&out) {
        ...
        return in.consume(*consumer).then([consumer, &out, this] {
                                          logger().
                                          debug
                                          ("AdminSocket::handle_client: incoming asok string: {}",
                                           consumer->line);
                                          return execute_line(consumer->line,
                                                              out);}
        ).then([&out] {
               return out.flush();}
        ).finally([&out] {
    ...
    seastar::future <> AdminSocket::start(const std::string & path) {
        if (path.empty()) {
            logger().
                error
                ("{}: Admin Socket socket path missing from the configuration",
                 __func__);
            return seastar::now();
        }
        ...
        auto sock_path =
            seastar::socket_address { seastar::unix_domain_addr {path}
        };
        try {
            server_sock = seastar::engine().listen(sock_path);
        }
        catch(const std::system_error & e) {
            logger().error("{}: unable to listen({}): {}", __func__, path,
                           e.what());
            server_sock.reset();
            return seastar::make_ready_future <> ();
        }
...
*/
    ...
    class HelpHook final:public AdminSocketHook {
      ...
      public:
        explicit HelpHook(const AdminSocket & as):AdminSocketHook {
        "help", "", "list available commands"}, m_as {
        as}
        {
        }
    ...
    class GetdescsHook final:public AdminSocketHook {
      ...
      public:
        explicit GetdescsHook(const AdminSocket & as):AdminSocketHook {
        "get_command_descriptions", "", "list available commands"}, m_as {
        as} {
        }
 ...
 */
    class ConfigShowHook:public AdminSocketHook {
      public:
        ConfigShowHook():AdminSocketHook {
 ...
 */
    class ConfigGetHook:public AdminSocketHook {
      public:
        ConfigGetHook():AdminSocketHook("config get",
                                        ...
                                        "config get <field>: get the config value")
        {
            ...
            if (int r = local_conf().get_val(var, &conf_val); r < 0) {
                return seastar::make_ready_future < tell_result_t >
                    (tell_result_t {
                     r, fmt::format("error getting {}: {}", var,
                                    cpp_strerror(r))}
                );
            }
            unique_ptr < Formatter > f {
    ...
    class ConfigSetHook:public AdminSocketHook {
      public:
                                                 ...
                                                 ceph::bufferlist &&)const final
        {
            ...
            const std::string joined_values =
 ...
 */
    class ConfigHelpHook:public AdminSocketHook {
      public:
        ConfigHelpHook():AdminSocketHook {
