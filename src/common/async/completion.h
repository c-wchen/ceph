...
template < typename T > struct UserData {
    T user_data;
    template < typename ... Args > UserData(Args && ... args)
...
protected:
    ...
    template < typename ... TArgs > Completion(TArgs && ... args)
...
public:
    ...
    static std::unique_ptr < Completion > create(const Executor1 &ex1, Handler && handler, TArgs
            && ... args);
    ...
    static void defer(std::unique_ptr < Completion > &&c, Args2
                      && ... args);

    /// take ownership of the completion, bind any arguments to the completion
    /// handler, then dispatch() it on its associated executor
    template < typename ... Args2 >
    static void dispatch(std::unique_ptr < Completion > &&c, Args2
                         && ... args);

    /// take ownership of the completion, bind any arguments to the completion
    /// handler, then post() it to its associated executor
    template < typename ... Args2 >
    static void post(std::unique_ptr < Completion > &&c, Args2
                     && ... args);
};
...
// the C++ Networking TS
template < typename Executor1, typename Handler, typename T,
           typename ... Args > class CompletionImpl final: public Completion <
    void (Args ...), T >
{
    // use Handler's associated executor (or Executor1 by default) for callbacks
    using Executor2 =
        boost::asio::associated_executor_t < Handler, Executor1 >;
    ...
    using RebindAlloc2 =
        typename Traits2::template rebind_alloc < CompletionImpl >;
    ...
    {
        RebindTraits2::deallocate(alloc2,
                                  static_cast < CompletionImpl * >(p),
                                  1);
    }

    static auto bind_and_forward(Handler
                                 && h, std::tuple < Args ... >&&args)
    {
    ...
    // forwarded to UserData
    template < typename ... TArgs > CompletionImpl(const Executor1 &ex1, Handler && handler, TArgs
            && ... args)
        : Completion < void (Args ...),
          T > (std::forward < TArgs > (args) ...), work(ex1,
                  boost::asio::
                  make_work_guard
                  (handler, ex1)),
...
void Completion < void (Args ...),
     T >::defer(std::unique_ptr < Completion > &&ptr, Args2 && ... args)
{
    auto c = ptr.release();
...
template < typename ... Args2 >
void Completion < void (Args ...),
     T >::dispatch(std::unique_ptr < Completion > &&ptr, Args2 && ... args)
{
    auto c = ptr.release();
...
template < typename ... Args2 >
void Completion < void (Args ...),
     T >::post(std::unique_ptr < Completion > &&ptr, Args2 && ... args)
{
    auto c = ptr.release();
...
std::unique_ptr < Completion < Signature, T >> create_completion(const Executor1 &ex, Handler && handler, TArgs
        && ... args)
{
    return Completion < Signature, T >::create(ex,
            std::forward < Handler >
            (handler),
            std::forward < TArgs >
            (args) ...);
...
void defer(std::unique_ptr < Completion < Signature, T >> &&ptr, Args
           && ... args)
{
    Completion < Signature, T >::defer(std::move(ptr),
...
/// handler, then dispatch() it on its associated executor
template < typename Signature, typename T, typename ... Args >
void dispatch(std::unique_ptr < Completion < Signature, T >> &&ptr, Args
              && ... args)
{
    Completion < Signature, T >::dispatch(std::move(ptr),
                                          std::forward < Args > (args) ...);
}
...
/// handler, then post() it to its associated executor
template < typename Signature, typename T, typename ... Args >
void post(std::unique_ptr < Completion < Signature, T >> &&ptr, Args
          && ... args)
{
    Completion < Signature, T >::post(std::move(ptr),
                                      std::forward < Args > (args) ...);
}
