 ...
 */
    ...
    COMMAND("auth clear-pending "
        "name=entity,type=CephString", "clear pending key", "auth", "rwx")
 ...
 */
    COMMAND_WITH_FLAG("compact",
                  "cause compaction of monitor's leveldb/rocksdb storage", "mon",
                  "rw", FLAG(TELL))
    ...
    COMMAND("health mute "
        ...
        "name=sticky,type=CephBool,req=false", "mute health alert", "mon", "w")
    ...
    COMMAND("report name=tags,type=CephString,n=N,req=false",
        "report full status of cluster, optional title tag strings", "mon", "r")
 ...
 */
    COMMAND_WITH_FLAG("mon scrub",
                  "scrub the monitor stores", "mon", "rw", FLAG(NONE))
    COMMAND("mon metadata name=id,type=CephString,req=false",
        "fetch metadata for mon <id>", "mon", "r")
    COMMAND("mon count-metadata name=property,type=CephString",
        "count mons by metadata field property", "mon", "r")
...
#define FS_NAME_GOODCHARS "[A-Za-z0-9-_.]"
    ...
    COMMAND("mds metadata name=who,type=CephString,req=false",
        "fetch metadata for mds <role>", "mds", "r")
    COMMAND("mds count-metadata name=property,type=CephString",
        "count MDSs by metadata field property", "mds", "r")
...
// #include "include/ceph_fs.h"
    ...
    COMMAND("mds fail name=role_or_gid,type=CephString",
        "Mark MDS failed: trigger a failover if a standby is available", "mds",
        "rw")
    COMMAND("mds repaired name=role,type=CephString",
        "mark a damaged MDS rank as no longer damaged", "mds", "rw")
    COMMAND("mds rm " "name=gid,type=CephInt,range=0", "remove nonactive mds",
        "mds", "rw")
    COMMAND_WITH_FLAG("mds rmfailed name=role,type=CephString "
                  "name=yes_i_really_mean_it,type=CephBool,req=false",
                  "remove failed rank", "mds", "rw", FLAG(HIDDEN))
    COMMAND_WITH_FLAG("mds compat show", "show mds compatibility settings", "mds",
                  "r", FLAG(DEPRECATED))
    COMMAND("fs compat show " "name=fs_name,type=CephString ",
        "show fs compatibility settings", "mds", "r")
    COMMAND_WITH_FLAG("mds compat rm_compat " "name=feature,type=CephInt,range=0",
                  "remove compatible feature", "mds", "rw", FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("mds compat rm_incompat " "name=feature,type=CephInt,range=0",
                  "remove incompatible feature", "mds", "rw", FLAG(DEPRECATED))
    COMMAND("fs new " "name=fs_name,type=CephString,goodchars=" FS_NAME_GOODCHARS
        ...
        "make new filesystem using named pools <metadata> and <data>", "fs",
        "rw")
    COMMAND("fs fail " "name=fs_name,type=CephString ",
    ...
    COMMAND("fs get name=fs_name,type=CephString",
        "get info about one filesystem", "fs", "r")
    COMMAND("fs set "
    ...
    COMMAND("fs feature ls",
        "list available cephfs features to be set/unset", "mds", "r")

    COMMAND("fs lsflags name=fs_name,type=CephString",
        "list the flags set on a ceph filesystem", "fs", "r")
 ...
 */
    ...
    COMMAND("mon getmap "
        "name=epoch,type=CephInt,range=0,req=false", "get monmap", "mon", "r")
    COMMAND("mon add "
        ...
        "mon", "rw")
    COMMAND("mon rm " "name=name,type=CephString", "remove monitor named <name>",
        "mon", "rw")
    COMMAND_WITH_FLAG("mon remove " "name=name,type=CephString",
                  "remove monitor named <name>", "mon", "rw", FLAG(DEPRECATED))
    COMMAND("mon feature ls " "name=with_value,type=CephBool,req=false",
        "list available mon map features to be set/unset", "mon", "r")
    COMMAND("mon feature set " "name=feature_name,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "set provided feature on mon map", "mon", "rw")
    COMMAND("mon set-rank " "name=name,type=CephString " "name=rank,type=CephInt",
        "set the rank for the specified mon", "mon", "rw")
    COMMAND("mon set-addrs " "name=name,type=CephString "
        "name=addrs,type=CephString",
        "set the addrs (IPs and ports) a specific monitor binds to", "mon",
        "rw")
    COMMAND("mon set-weight " "name=name,type=CephString "
        "name=weight,type=CephInt,range=0|65535",
        "set the weight for the specified mon", "mon", "rw")
    COMMAND("mon enable-msgr2", "enable the msgr2 protocol on port 3300", "mon",
        "rw")
    COMMAND("mon set election_strategy " "name=strategy,type=CephString",
            "set the election strategy to use; choices classic, disallow, connectivity",
            "mon", "rw")
    COMMAND("mon add disallowed_leader " "name=name,type=CephString",
        "prevent the named mon from being a leader", "mon", "rw")
    COMMAND("mon rm disallowed_leader " "name=name,type=CephString",
        "allow the named mon to be a leader again", "mon", "rw")
    COMMAND("mon set_location " "name=name,type=CephString "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "specify location <args> for the monitor <name>, using CRUSH bucket names",
        "mon", "rw")
    COMMAND("mon enable_stretch_mode " "name=tiebreaker_mon,type=CephString, "
        ...
        "as the units for stretching across", "mon", "rw")
    COMMAND("mon set_new_tiebreaker " "name=name,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
 ...
 */
    ...
    COMMAND("osd getmap "
        "name=epoch,type=CephInt,range=0,req=false", "get OSD map", "osd", "r")
    ...
    COMMAND("osd count-metadata name=property,type=CephString",
        "count OSDs by metadata field property", "osd", "r")
    ...
    COMMAND("osd setcrushmap name=prior_version,type=CephInt,req=false",
        "set crush map from input file", "osd", "rw")
    ...
    COMMAND("osd crush set-device-class "
        ...
        "or use <all|any> to set all.", "osd", "rw")
    COMMAND("osd crush rm-device-class "
        "name=ids,type=CephString,n=N",
        "remove class of the osd(s) <id> [<id>...],"
        "or use <all|any> to remove all.", "osd", "rw")
    ...
    COMMAND("osd crush weight-set rm name=pool,type=CephPoolname",
        "remove the weight-set for a given pool", "osd", "rw")
    COMMAND("osd crush weight-set rm-compat",
        "remove the backward-compatible weight-set", "osd", "rw")
    ...
    COMMAND("osd erasure-code-profile ls",
        "list all erasure code profiles", "osd", "r")
    COMMAND("osd set "
    ...
    COMMAND("osd unset "
        "name=key,type=CephChoices,strings=full|pause|noup|nodown|"
        "noout|noin|nobackfill|norebalance|norecover|noscrub|nodeep-scrub|"
        "notieragent|nosnaptrim", "unset <key>", "osd", "rw")
    ...
    COMMAND("osd down "
        ...
        "or use <any|all> to set all osds down", "osd", "rw")
    COMMAND("osd stop "
    ...
    COMMAND("osd out "
        "name=ids,type=CephString,n=N",
        "set osd(s) <id> [<id>...] out, "
        "or use <any|all> to set all osds out", "osd", "rw")
    COMMAND("osd in "
        "name=ids,type=CephString,n=N",
        "set osd(s) <id> [<id>...] in, "
    ...
    COMMAND_WITH_FLAG("osd purge-actual "
                  ...
                  "`osd rm`, and `osd crush rm`.", "osd", "rw", FLAG(HIDDEN))
    COMMAND("osd lost "
        "name=id,type=CephOsdName "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
    ...
    COMMAND_WITH_FLAG("osd blacklist ls", "show blacklisted clients", "osd", "r",
                  FLAG(DEPRECATED))
    COMMAND_WITH_FLAG("osd blacklist clear", "clear all blacklisted clients", "osd",
                  "rw", FLAG(DEPRECATED))
    ...
    COMMAND("osd pool create "
        ...
        "name=yes_i_really_mean_it,type=CephBool,req=false"
        "name=crimson,type=CephBool,req=false", "create pool", "osd", "rw")
...
// with units.
    ...
    COMMAND("osd force_recovery_stretch_mode "
        ...
        "all monitor buckets are up", "osd", "rw")
...
// tiering
    ...
    COMMAND_WITH_FLAG("osd tier remove-overlay " "name=pool,type=CephPoolname ",
                  "remove the overlay pool for base pool <pool>", "osd", "rw",
                  FLAG(DEPRECATED))
 ...
 */
    COMMAND("config-key get "
        "name=key,type=CephString", "get <key>", "config-key", "r")
    ...
    COMMAND("config-key rm "
        "name=key,type=CephString", "rm <key>", "config-key", "rw")
 ...
 */
    ...
    COMMAND("mgr module enable "
        ...
        "name=force,type=CephBool,req=false", "enable mgr module", "mgr", "rw")
    COMMAND("mgr module disable "
        "name=module,type=CephString", "disable mgr module", "mgr", "rw")
    COMMAND("mgr metadata name=who,type=CephString,req=false",
        "dump metadata for all daemons or a specific daemon", "mgr", "r")
    COMMAND("mgr count-metadata name=property,type=CephString",
        "count ceph-mgr daemons by metadata field property", "mgr", "r")
    COMMAND("mgr versions",
        "check running versions of ceph-mgr daemons", "mgr", "r")
// ConfigMonitor
    COMMAND("config set"
    ...
    COMMAND("config log name=num,type=CephInt,req=false",
        "Show recent history of config changes", "config", "r")
    ...
    COMMAND("config generate-minimal-conf",
        "Generate a minimal ceph.conf file", "config", "r")
...
// that shunts these off to the asok machinery.
    ...
    COMMAND_WITH_FLAG("mon_status",
                  "report status of monitors", "mon", "r", FLAG(TELL))
    ...
    COMMAND_WITH_FLAG("add_bootstrap_peer_hint "
                  ...
                  "peer for cluster bringup", "mon", "rw", FLAG(TELL))
    ...
    COMMAND_WITH_FLAG("quorum enter ",
                  "force monitor back into quorum", "mon", "rw", FLAG(TELL))
    COMMAND_WITH_FLAG("quorum exit",
                  "force monitor out of the quorum", "mon", "rw", FLAG(TELL))
    COMMAND_WITH_FLAG("ops",
                  "show the ops currently in flight", "mon", "r", FLAG(TELL))
    COMMAND_WITH_FLAG("sessions", "list existing sessions", "mon", "r", FLAG(TELL))
    COMMAND_WITH_FLAG("dump_historic_ops",
                  "show recent ops", "mon", "r", FLAG(TELL))
    COMMAND_WITH_FLAG("dump_historic_slow_ops",
                  "show recent slow ops", "mon", "r", FLAG(TELL))
