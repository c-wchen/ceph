...
extern "C" {
                           ...
                           const std::string &name)
    {
        ...
        return plugin_registry->add(type, name,
                                    new librbd::plugin::WriteLogImageCache <
                                    librbd::ImageCtx > (cct));
    }
                                    ...
                                    Context *on_finish)
{
    ...
    auto hook_points =
        std::make_unique < WriteLogImageCache::HookPoints > (image_ctx,
            image_writeback,
            api);
...
template < typename I >
WriteLogImageCache < I >::HookPoints::HookPoints(I *image_ctx,
        cache::
        ImageWritebackInterface
        & image_writeback,
        plugin::Api < I >
...
template < typename I >
void WriteLogImageCache <
I >::HookPoints::acquired_exclusive_lock(Context *on_finish)
{
    cache::pwl::InitRequest < I > *req =
        cache::pwl::InitRequest < I >::create(*m_image_ctx,
            m_image_writeback,
            m_plugin_api, on_finish);
    req->send();
}

template < typename I >
void WriteLogImageCache <
I >::HookPoints::prerelease_exclusive_lock(Context *on_finish)
{
    m_image_ctx->io_image_dispatcher->
    shut_down_dispatch(io::IMAGE_DISPATCH_LAYER_WRITEBACK_CACHE,
                       on_finish);
}

template < typename I >
void WriteLogImageCache <
I >::HookPoints::discard(Context *on_finish)
{
    cache::pwl::DiscardRequest < I > *req =
        cache::pwl::DiscardRequest < I >::create(*m_image_ctx,
            m_plugin_api,
            on_finish);
    req->send();
}
