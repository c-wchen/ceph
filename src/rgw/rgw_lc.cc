...
void *RGWLC::LCWorker::entry()
{
    do {
        ...
        if (should_work(start)) {
            ...
            int r = lc->process();
            if (r < 0) {
                dout(0) << "ERROR: do life cycle process() returned error r=" <<
                    r << dendl;
            }
        ...
        dout(5) << "schedule life cycle next start time: " <<
...
#define MAX_LC_LIST_ENTRIES 100
    do {
        ...
        for (iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            ret =
                cls_rgw_lc_set_entry(store->lc_pool_ctx, obj_names[index],
                                     entry);
            if (ret < 0) {
...
{
    if (remove_indeed) {
        return rgw_remove_object(store, bucket_info, bucket_info.bucket,
                                 obj_key);
    }
    else {
...
int RGWLC::handle_multipart_expiration(RGWRados::Bucket * target,
                                       const map < string, lc_op > &prefix_map)
{
    ...
    for (auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end();
        ...
        do {
            ...
            for (auto obj_iter = objs.begin(); obj_iter != objs.end();
                 ++obj_iter) {
                if (obj_has_expired
                    (now - ceph::real_clock::to_time_t(obj_iter->meta.mtime),
                    ...
                    RGWObjectCtx rctx(store);
                    ret =
                        abort_multipart_upload(store, cct, &rctx, bucket_info,
                                               mp_obj);
                    if (ret < 0 && ret != -ERR_NO_SUCH_UPLOAD) {
                        ldout(cct,
                              0) << "ERROR: abort_multipart_upload failed, ret="
                            << ret << dendl;
                        return ret;
                    }
                    if (going_down())
...
int RGWLC::bucket_lc_process(string & shard_id)
{
    ...
    int ret =
        store->get_bucket_info(obj_ctx, bucket_tenant, bucket_name, bucket_info,
                               NULL, &bucket_attrs);
    if (ret < 0) {
    ...
    if (!bucket_info.versioned()) {
            ...
            do {
                ...
                for (auto obj_iter = objs.begin(); obj_iter != objs.end();
                    ...
                    if (is_expired) {
                        ...
                        RGWObjState *state;
                        int ret =
                            store->get_obj_state(&rctx, bucket_info, obj,
                                                 &state, false);
                        if (ret < 0) {
                            return ret;
                        }
                        if (state->mtime != obj_iter->meta.mtime)   //Check mtime again to avoid delete a recently update object as much as possible
                            continue;
                        ret =
                            remove_expired_obj(bucket_info, obj_iter->key,
                                               obj_iter->meta.owner,
                                               obj_iter->meta.
        ...
        for (auto prefix_iter = prefix_map.begin();
            ...
            if (!prefix_iter->second.status
                || (prefix_iter->second.expiration <= 0
                    && prefix_iter->second.expiration_date == boost::none
                    && prefix_iter->second.noncur_expiration <= 0
                    && !prefix_iter->second.dm_expiration)) {
                continue;
            }
            if (prefix_iter != prefix_map.begin() &&
                (prefix_iter->first.
                 compare(0, prev(prefix_iter)->first.length(),
                ...
                for (auto obj_iter = objs.begin(); obj_iter != objs.end();
                    ...
                    if (obj_iter->is_current()) {
                        if (prefix_iter->second.expiration <= 0
                            && prefix_iter->second.expiration_date ==
                            boost::none && !prefix_iter->second.dm_expiration) {
                            continue;
                        }
                        ...
                        mtime =
                            (obj_iter ==
                             objs.begin())? pre_obj.meta.mtime : (obj_iter -
                                                                  1)->meta.
                            mtime;
...
int RGWLC::bucket_lc_post(int index, int max_lock_sec, pair < string,
                          int >&entry, int &result)
{
    ...
    do {
        ...
        if (ret == -EBUSY) {    /* already locked by another lc processor */
            dout(0) <<
                "RGWLC::bucket_lc_post() failed to acquire lock on, sleep 5, try again"
                << obj_names[index] << dendl;
            sleep(5);
            continue;
        }
        if (ret < 0)
            return 0;
        dout(20) << "RGWLC::bucket_lc_post()  get lock" << obj_names[index] <<
            dendl;
        if (result == -ENOENT) {
            ret =
                cls_rgw_lc_rm_entry(store->lc_pool_ctx, obj_names[index],
                                    entry);
            if (ret < 0) {
                dout(0) << "RGWLC::bucket_lc_post() failed to remove entry " <<
                    obj_names[index] << dendl;
            }
            goto clean;
        }
        ...
        if (ret < 0) {
            dout(0) << "RGWLC::process() failed to set entry " <<
                obj_names[index] << dendl;
        }
      clean:
        l.unlock(&store->lc_pool_ctx, obj_names[index]);
        dout(20) << "RGWLC::bucket_lc_post()  unlock" << obj_names[index] <<
            dendl;
        return 0;
    } while (true);
}
...
int RGWLC::process(int index, int max_lock_secs)
{
    ...
    do {
        ...
        if (ret == -EBUSY) {    /* already locked by another lc processor */
            dout(0) <<
                "RGWLC::process() failed to acquire lock on, sleep 5, try again"
                << obj_names[index] << dendl;
            sleep(5);
            continue;
        }
        ...
        if (!if_already_run_today(head.start_date)) {
            ...
            if (ret < 0) {
                dout(0) << "RGWLC::process() failed to update lc object " <<
                    obj_names[index] << ret << dendl;
                goto exit;
            }
        }
        ...
        ret =
            cls_rgw_lc_get_next_entry(store->lc_pool_ctx, obj_names[index],
                                      head.marker, entry);
        if (ret < 0) {
            dout(0) << "RGWLC::process() failed to get obj entry " <<
                obj_names[index] << dendl;
            goto exit;
        }
