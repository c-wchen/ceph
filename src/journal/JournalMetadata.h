...
public:
    ...
    bool get_last_allocated_entry_tid(uint64_t tag_tid,
...
private:
    ...
    JournalMetadata(ContextWQ *work_queue, SafeTimer *timer,
    ...
    struct CommitEntry {
        ...
        CommitEntry(): object_num(0), tag_tid(0), entry_tid(0),
            committed(false)
        {
        } CommitEntry(uint64_t _object_num, uint64_t _tag_tid,
                      uint64_t _entry_tid)
    ...
    struct C_ImmutableMetadata: public Context {
        ...
        C_ImmutableMetadata(JournalMetadata *_journal_metadata,
                            Context *_on_finish)
    ...
    mutable ceph::mutex m_lock =
        ceph::make_mutex("JournalMetadata::m_lock");
