                                 ...
                                 std::map < std::string, std::string > &headers)
{
    ...
    /* fetch headers */
    ret =
        tier_ctx.conn.complete_request(in_req, nullptr, nullptr, nullptr,
                                       nullptr, &headers, null_yield);
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(tier_ctx.dpp,
                  20) << "ERROR: " << __func__ <<
                              ...
                              ceph::real_time & mtime)
{
  ...
  for (const auto & a:attrs) {
    ...
    snprintf(buf, sizeof(buf), "%lld.%09lld",
             (long long)ut.sec(), (long long)ut.nsec());
...
void RGWLCStreamRead::set_multipart(uint64_t part_size, off_t part_off,
                                    off_t part_end)
{
    multipart = true;
                                          ...
                                          string > &attrs)
{
  ...
  for (const auto & aiter:access_map) {
    ...
    snprintf(buf, sizeof(buf), "%llu",
             (long long)obj_properties.versioned_epoch);
...
{
    int ret =
        conn.complete_request(out_req, etag, &obj_properties.mtime, null_yield);
    return ret;
}
                                             ...
                                             const std::string & upload_id)
{
    ...
    ret =
        cloud_tier_abort_multipart(tier_ctx.dpp, tier_ctx.conn, dest_obj,
                                   upload_id);
...
static int cloud_tier_multipart_transfer(RGWLCCloudTierCtx & tier_ctx)
{
    ...
    rgw_pool pool =
        static_cast <
        rgw::sal::RadosStore *
        >(tier_ctx.driver)->svc()->zone->get_zone_params().log_pool;
    status_obj = rgw_raw_obj(pool, "lc_multipart_" + tier_ctx.obj->get_oid());

    ret =
        read_upload_status(tier_ctx.dpp, tier_ctx.driver, &status_obj, &status);
    ...
    if (ret == -ENOENT) {
        RGWLCStreamRead readf(tier_ctx.cct, tier_ctx.dpp, tier_ctx.obj,
                              tier_ctx.o.meta.mtime);

        readf.init();

        rest_obj = readf.get_rest_obj();

        RGWLCCloudStreamPut::init_send_attrs(tier_ctx.dpp, rest_obj,
                                             obj_properties, new_attrs);

        ret =
            cloud_tier_init_multipart(tier_ctx.dpp, tier_ctx.conn, dest_obj,
                                      obj_size, new_attrs, status.upload_id);
        if (ret < 0) {
            return ret;
        }
        ...
        ret =
            put_upload_status(tier_ctx.dpp, tier_ctx.driver, &status_obj,
                              &status);
...
#define MULTIPART_MAX_PARTS 10000
    ...
    ret =
        cloud_tier_complete_multipart(tier_ctx.dpp, tier_ctx.conn, dest_obj,
                                      status.upload_id, parts);
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp,
...
static int cloud_tier_create_bucket(RGWLCCloudTierCtx & tier_ctx)
{
    bufferlist out_bl;
    int ret = 0;
    pair < string, string > key(tier_ctx.storage_class,
    ...
    ret =
        tier_ctx.conn.send_resource(tier_ctx.dpp, "PUT", resource, nullptr,
                                    nullptr, out_bl, &bl, nullptr, null_yield);
