...
RGWKmipHandle *RGWKmipHandleBuilder::build() const const
{
    int failed = 1;
    ...
    r->encoding =
        ...
        uint8 *
        >(r->kmip_ctx->
          calloc_func(r->kmip_ctx->state, r->buffer_blocks,
                      r->buffer_block_size));
    if (!r->encoding) {
...
void RGWKmipHandles::flush_kmip_handles()
{
    ...
    if (!saved_kmip.empty()) {
        ldout(cct,
              0) << "ERROR: " << __func__ << " failed final cleanup" << dendl;
    }
...
int RGWKmipHandles::do_one_entry(RGWKMIPTransceiver & element)
{
    ...
    for (;;) {
        ...
        h->encoding =
            ...
            uint8 *
            >(h->kmip_ctx->
              calloc_func(h->kmip_ctx->state, h->buffer_blocks,
                          h->buffer_block_size));
        if (!h->encoding) {
            lderr(cct) << "kmip buffer alloc failed: "
    ...
    case KMIP_OP_CREATE:{
            ...
            element.out =
                static_cast < char *>(malloc(pld->unique_identifier->size + 1));
    ...
    case KMIP_OP_GET:{
            ...
            KeyBlock *kp =
                static_cast < SymmetricKey * >(pld->object)->key_block;
            ByteString *bp;
