...
class KeyValueDB:public PriorityCache::PriCache {
  public:
    class TransactionImpl {
      public:
        /// Set Keys
        void set(const std::string & prefix,    ///< [in] Prefix for keys
        ...
        /// Set Keys (via encoded bufferlist) void set(
        ...
        virtual void rm_single_key(const std::string & prefix,  ///< [in] Prefix to search for
                                   const std::string & k    ///< [in] Key to remove
            ) {
        ...
        virtual void rmkeys_by_prefix(const std::string & prefix    ///< [in] Prefix by which to remove keys
            ) = 0;

        virtual void rm_range_keys(const string & prefix,   ///< [in] Prefix by which to remove keys
        ...
        virtual void merge(const std::string & prefix,  ///< [in] Prefix ==> MUST match some established merge operator
    ...
    static KeyValueDB *create(CephContext * cct, const std::string & type,
                              const std::string & dir, void *p = NULL);
    ...
    /// Retrieve Keys
    virtual int get(const std::string & prefix, ///< [in] Prefix for key
    ...
    virtual int get(const string & prefix,
                    const char *key, size_t keylen, bufferlist * value) {
    ...
    class WholeSpaceIteratorImpl {
      public:
        ...
        virtual int upper_bound(const std::string & prefix,
                                const std::string & after) = 0;
        virtual int lower_bound(const std::string & prefix,
    ...
    class IteratorImpl:public GenericIteratorImpl {
      ...
      public:
        IteratorImpl(const std::string & prefix,
                     WholeSpaceIterator iter):prefix(prefix),
            generic_iter(iter) {
    ...
    virtual uint64_t get_estimated_size(std::map < std::string,
                                        uint64_t > &extra) = 0;
  ...
  public:
    ...
    // End PriCache virtual int set_cache_high_pri_pool_ratio(double ratio) {
        return -EOPNOTSUPP;
    }
