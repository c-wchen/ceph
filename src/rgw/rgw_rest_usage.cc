...
void RGWOp_Usage_Get::execute(optional_yield y)
{
    ...
    std::unique_ptr < rgw::sal::User > user =
        driver->get_user(rgw_user(uid_str));
    std::unique_ptr < rgw::sal::Bucket > bucket;

    if (!bucket_name.empty()) {
        driver->get_bucket(nullptr, user.get(), std::string(), bucket_name,
                           &bucket, null_yield);
    }
    ...
    op_ret =
        RGWUsage::show(this, driver, user.get(), bucket.get(), start, end,
...
void RGWOp_Usage_Delete::execute(optional_yield y)
{
    ...
    if (rgw::sal::User::empty(user.get()) &&
        bucket_name.empty() && !start && end == (uint64_t) - 1) {
        bool remove_all;
