...
int Filer::probe(inodeno_t ino, file_layout_t *layout, snapid_t snapid, uint64_t start_from, uint64_t *end, // LB, when !fwd
...
int Filer::probe_impl(Probe *probe, file_layout_t *layout, uint64_t start_from, uint64_t *end) // LB, when !fwd
{
...
void Filer::_probe(Probe *probe, Probe::unique_lock &pl)
{
    ...
    Striper::file_to_extents(cct, probe->ino, &probe->layout,
                             probe->probing_off, probe->probing_len, 0,
                             probe->probing);
        ...
        if (!probe->found_size)
        {
                 ...
                 i != p->buffer_extents.end(); ++i)
            {
                if (oleft <= (uint64_t)i->second)
                {
                    ...
                    ldout(cct,
                          10)
                        << "_probed  end is in buffer_extent " << i->first << "~" << i->second << " off " << oleft << ", from was " << probe->probing_off << ", end is " << end << dendl;
...
void Filer::_do_truncate_range(TruncRange *tr, int fin)
{
    ...
    ldout(cct,
          10)
        << "_do_truncate_range " << tr->ino << " objects " << tr->offset << "~" << tr->length << " uncommitted " << tr->uncommitted << dendl;
    ...
    for (const auto &p : extents)
    {
        ...
        objecter->_modify(p.oid, p.oloc, ops, tr->mtime, tr->snapc, tr->flags,
                          new C_OnFinisher(new C_TruncRange(this, tr),
                                           finisher));
    }
}
