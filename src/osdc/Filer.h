...
class Filer
{
    ...
    struct Probe
    {
        ...
        Probe(inodeno_t i, file_layout_t &l, snapid_t sn,
              ...
              Context *c) : ino(i), layout(l), snapid(sn),
                            psize(e), pmtime(m), pumtime(nullptr), flags(fl), fwd(fw),
                            onfinish(c), probing_off(f), probing_len(0), err(0),
                            found_size(false)
        {
        }
        Probe(inodeno_t i, file_layout_t &l, snapid_t sn, uint64_t f,
              uint64_t *e, utime_t *m, int fl, bool fw, Context *c) : ino(i),
                                                                      layout(l), snapid(sn), psize(e), pmtime(nullptr), pumtime(m),
...
public:
    ...
    void read(inodeno_t ino, file_layout_t *layout, snapid_t snap, uint64_t offset, uint64_t len, bufferlist *bl, // ptr to data
    ...
    void read_trunc(inodeno_t ino, file_layout_t *layout, snapid_t snap, uint64_t offset, uint64_t len, bufferlist *bl, // ptr to data
                    int flags,
                    uint64_t truncate_size,
                    __u32 truncate_seq, Context *onfinish, int op_flags = 0)
    {
        assert(snap); // (until there is a non-NOSNAP write)
        vector<ObjectExtent> extents;
    ...
    void write(inodeno_t ino,
               ...
               int flags, Context *oncommit, int op_flags = 0)
    {
        vector<ObjectExtent> extents;
        Striper::file_to_extents(cct, ino, layout, offset, len, 0, extents);
        objecter->sg_write(extents, snapc, bl, mtime, flags, oncommit,
                           op_flags);
    }
                     ...
                     __u32 truncate_seq, Context *oncommit, int op_flags = 0)
    {
        ...
        objecter->sg_write_trunc(extents, snapc, bl, mtime, flags,
                                 truncate_size, truncate_seq, oncommit,
                                 op_flags);
    }
    ...
    void zero(inodeno_t ino,
              ...
              uint64_t len,
              ceph::real_time mtime,
              int flags, bool keep_first, Context *oncommit)
    {
        vector<ObjectExtent> extents;
        Striper::file_to_extents(cct, ino, layout, offset, len, 0, extents);
        if (extents.size() == 1)
        {
            if (extents[0].offset == 0 && extents[0].length == layout->object_size && (!keep_first || extents[0].objectno != 0))
                objecter->remove(extents[0].oid, extents[0].oloc, snapc, mtime,
                                 flags, oncommit);
            else
                objecter->zero(extents[0].oid, extents[0].oloc,
                               extents[0].offset, extents[0].length, snapc,
    ...
    int probe(inodeno_t ino,
              ...
              snapid_t snapid,
              uint64_t start_from,
              uint64_t *end, bool fwd, int flags, Context *onfinish)
    {
