 ...
 */
    class NVMeBlockDevice:public RBMDevice {
      public:
        ...
        open_ertr::future <> open(const std::string & in_path,
                                  seastar::open_flags mode) override;

        write_ertr::future <> write(uint64_t offset,
                                    bufferptr && bptr,
        ...
        std::string get_device_path()const final {
            return device_path;
        } uint64_t get_preffered_write_granularity() const {
            return write_granularity;
        } uint64_t get_preffered_write_alignment() const {
            return write_alignment;
        } uint64_t get_atomic_write_unit() const {
         ...
         */ bool is_data_protection_enabled() const {
            return data_protection_enabled;
        }
        /*
         * Data Health
         *
         ...
         * protect data
         */ virtual nvme_command_ertr::future < std::list <
            uint64_t >> get_data_health() {
            std::list < uint64_t > fragile_lbas;
            return nvme_command_ertr::future < std::list <
                uint64_t >> (nvme_command_ertr::ready_future_marker {
                             }
                             , fragile_lbas);
        }
         ...
         */
        virtual nvme_command_ertr::
            future <> set_data_recovery_level(uint32_t level) {
        ...
        nvme_command_ertr::future <
            int >pass_admin(nvme_admin_command_t & admin_cmd, seastar::file f);
        nvme_command_ertr::future <
            int >pass_through_io(nvme_io_command_t & io_cmd);
      ...
      private:
        ...
        open_ertr::future <> open_for_io(const std::string & in_path,
