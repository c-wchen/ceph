...
namespace crimson::os::seastore::journal {
    ...
    } CircularJournalSpace::CircularJournalSpace(RBMDevice *
    ...
    extent_len_t CircularJournalSpace::get_block_size() const {
        ...
        paddr_t paddr = convert_abs_addr_to_paddr(get_records_start(),
                                                  get_device_id());
        auto seq = get_written_to();
        set_written_to(journal_seq_t {
                       ++seq.segment_seq, paddr}
        );
    ...
    CircularJournalSpace::write_ret
        ...
        paddr_t paddr = convert_abs_addr_to_paddr(new_written_to,
                                                  get_device_id());
        set_written_to(journal_seq_t {
                       get_written_to().segment_seq, paddr}
        );
        ...
        return device_write_bl(target, to_write).safe_then([this, target,
                                                            length =
                                                            encoded_size,
                                                            write_result,
                                                            FNAME] {
                                                           DEBUG
                                                           ("commit target {} used_size {} written length {}",
                                                            target,
                                                            get_records_used_size
                                                            (), length);
                                                           return write_result;}
        ).handle_error(base_ertr::pass_further {
                       }
    ...
    CircularJournalSpace::open_ret CircularJournalSpace::open(bool is_mkfs) {
        ...
        if (is_mkfs) {
            ...
            0, convert_abs_addr_to_paddr(get_records_start(),
            ...
            initialized = true;
            DEBUG("initialize header block in CircularJournalSpace length {}",
                  bl.length());
            return write_header().safe_then([this] () {
                                            return open_ret(open_ertr::
                                                            ready_future_marker
                                                            {
                                                            }
                                                            , get_written_to());}
        ...
        return open_ret(open_ertr::ready_future_marker {
                        }
                        , get_written_to());
    }
    ...
    ceph::bufferlist CircularJournalSpace::encode_header() {
        ...
        auto header_crc =
            bliter.crc32c(ceph::encoded_sizeof_bounded < cbj_header_t > (),
                          -1);
        ceph_le32 header_crc_le;
        header_crc_le = header_crc;
        header_crc_filler.copy_in(sizeof(checksum_t),
                                  reinterpret_cast <
                                  const char *>(&header_crc_le));
        return bl;
    }

    CircularJournalSpace::write_ertr::future <> CircularJournalSpace::
        device_write_bl(rbm_abs_addr offset, bufferlist & bl) {
        LOG_PREFIX(CircularJournalSpace::device_write_bl);
        auto length = bl.length();
        if (offset + length > get_journal_end()) {
            return crimson::ct_error::erange::make();
        }
        DEBUG("overwrite in CircularJournalSpace, offset {}, length {}",
              offset, length);
        return device->writev(offset, bl).handle_error(write_ertr::pass_further {
                                                       }
                                                       ,
                                                       crimson::ct_error::
                                                       assert_all {
    ...
    CircularJournalSpace::read_header_ret CircularJournalSpace::read_header() {
        ...
        auto bptr =
            bufferptr(ceph::buffer::
                      create_page_aligned(device->get_block_size()));
        DEBUG("reading {}", device->get_journal_start());
        return device->read(device->get_journal_start(),
                            bptr).safe_then([bptr,
                                             FNAME] ()mutable->read_header_ret {
                                            bufferlist bl; bl.append(bptr);
                                            auto bp = bl.cbegin();
                                            cbj_header_t cbj_header; try {
                                            decode(cbj_header, bp);}
                                            catch(ceph::buffer::error & e) {
                                            ERROR
                                            ("unable to read header block");
                                            return crimson::ct_error::enoent::
                                            make();}
                                            auto bliter = bl.cbegin();
                                            auto test_crc =
                                            bliter.
                                            ...
                                            return
                                            read_header_ret(read_header_ertr::
                                                            ready_future_marker
                                                            {
                                                            }
                                                            , std::nullopt);}
