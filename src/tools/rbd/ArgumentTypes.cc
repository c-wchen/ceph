...
namespace rbd {
    namespace argument_types {
        ...
        const std::map < uint64_t,
            std::string > ImageFeatures::FEATURE_MAPPING = {
        ...
        void add_pool_options(boost::program_options::options_description * pos,
            ...
            opt->add_options()
                ((POOL_NAME + ",p").c_str(), po::value < std::string > (),
                 "pool name");
        ...
        void add_create_image_options(po::options_description * opt,
            ...
            opt->add_options()
                ...
                (IMAGE_STRIPE_COUNT.c_str(), po::value < uint64_t > (),
                 "stripe count")
                (IMAGE_DATA_POOL.c_str(), po::value < std::string > (),
                 "data pool")
                (IMAGE_MIRROR_IMAGE_MODE.c_str(),
                 po::value < MirrorImageMode > (),
        ...
        void add_create_journal_options(po::options_description * opt) {
            opt->add_options()
                ...
                (JOURNAL_OBJECT_SIZE.c_str(),
                 po::value < JournalObjectSize > (),
        ...
        void add_path_options(boost::program_options::options_description * pos,
            ...
            pos->add_options()
                (PATH_NAME.c_str(), po::value < std::string > (),
                 description.c_str());
            opt->add_options()
                (PATH.c_str(), po::value < std::string > (),
                 description.c_str());
        }
        ...
        void add_no_progress_option(boost::program_options::
                                    options_description * opt) {
            opt->add_options()
                (NO_PROGRESS.c_str(), po::bool_switch(),
        ...
        void add_export_format_option(boost::program_options::
                                      options_description * opt) {
            opt->add_options()
                ("export-format", po::value < ExportFormat > (),
        ...
        void add_snap_create_options(po::options_description * opt) {
            opt->add_options()
                (SKIP_QUIESCE.c_str(), po::bool_switch(),
                 "do not run quiesce hooks")
          ...
          for (auto & pair:ImageFeatures::FEATURE_MAPPING) {
                if ((pair.first & RBD_FEATURES_IMPLICIT_ENABLE) != 0ULL) {
                    ...
                    continue;
                         ...
                         && (pair.first & RBD_FEATURES_MUTABLE) == 0ULL) {
                ...
                if (append_suffix) {
                    if ((pair.
                         first & rbd::utils::
                         get_rbd_default_features(g_ceph_context)) != 0) {
                        suffix += "+";
                    }
        ...
        std::string get_long_features_help() {
            ...
            oss << "Image Features:" << std::endl
                ...
                std::
                endl << "  (-) supports disabling-only on existing images" <<
                std::
                endl <<
