...
inline bool ceph_filelock_owner_equal(const ceph_filelock & l,
                                      const ceph_filelock & r)
{
...
inline int ceph_filelock_owner_compare(const ceph_filelock & l,
                                       const ceph_filelock & r)
{
...
inline int ceph_filelock_compare(const ceph_filelock & l,
                                 const ceph_filelock & r)
{
...
class ceph_lock_state_t {
    ...
    int type;
  public:
     explicit ceph_lock_state_t(CephContext * cct_, int type_):cct(cct_),
        type(type_) {
    } ~ceph_lock_state_t();
   ...
   */
    bool is_deadlock(const ceph_filelock & fl,
                     ...
                     ceph_filelock >::iterator > &overlapping_locks,
                     const ceph_filelock * first_fl = NULL, unsigned depth =
                     0) const;
   ...
   *    left or right side.
   */
    void adjust_locks(list < multimap < uint64_t,
                      ceph_filelock >::iterator > old_locks,
                      ceph_filelock & new_lock, list < multimap < uint64_t,
