...
namespace crimson::common {
    ...
    :_conf {
    crimson::common::local_conf()}, _perf_counters_collection {
    crimson::common::local_perf_coll()}, _crypto_random {
    std::make_unique < CryptoRandom > ()}
    {
    }
    ...
    class MempoolObs:public md_config_obs_t, public AdminSocketHook {
      ...
      public:
        ...
        :cct(cct), lock(ceph::make_mutex("mem_pool_obs")) {
            ...
            int r = cct->get_admin_socket()->register_command("dump_mempools",
                                                              this,
                                                              "get mempool stats");
            ceph_assert(r == 0);
        } ~MempoolObs() override {
        ...
        :_reopen_logs(false), _exit_thread(false), _cct(cct) {
            ...
            while (1) {
                ...
                if (_cct->_conf->heartbeat_interval) {
                    auto interval =
                        ceph::make_timespan(_cct->_conf->heartbeat_interval);
  ...
  public:
    ...
    void handle_conf_change(const ConfigProxy & conf,
        ...
        if (changed.count("log_stderr_prefix")) {
            log->set_log_stderr_prefix(conf.get_val < std::string >
                                       ("log_stderr_prefix"));
        }
        ...
        if (changed.count("log_to_graylog") || changed.count("err_to_graylog")) {
            int l =
                conf->log_to_graylog ? 99 : (conf->err_to_graylog ? -1 : -2);
            log->set_graylog_level(l, l);

            if (conf->log_to_graylog || conf->err_to_graylog) {
                log->start_graylog(conf->host,
                                   conf.get_val < uuid_d > ("fsid"));
            }
        ...
        if (log->graylog()
            && (changed.count("log_graylog_host")
                || changed.count("log_graylog_port"))) {
            log->graylog()->set_destination(conf->log_graylog_host,
                                            conf->log_graylog_port);
        }

        // journald
        if (changed.count("log_to_journald")
            || changed.count("err_to_journald")) {
            int l =
                conf.get_val < bool > ("log_to_journald") ? 99 : (conf.get_val <
                                                                  bool >
                                                                  ("err_to_journald")
                                                                  ? -1 : -2);
        ...
        if (changed.find("log_coarse_timestamps") != changed.end()) {
            log->set_coarse_timestamps(conf.get_val < bool >
                                       ("log_coarse_timestamps"));
        }
                ...
                if (getenv("CEPH_DEV") == NULL) {
                    if (!cct->_experimental_features.empty()) {
                        if (cct->_experimental_features.count("*")) {
                            lderr(cct) <<
                                "WARNING: all dangerous and experimental features are enabled."
                                << dendl;
                        }
                        else {
                            lderr(cct) <<
...
// perfcounter hooks
    ...
    bool CephContext::check_experimental_feature_enabled(const std::
                                                         string & feat) {
    ...
    bool CephContext::check_experimental_feature_enabled(const std::
        ...
        if (enabled) {
            (*message) << "WARNING: experimental feature '" << feat <<
                "' is enabled\n";
            (*message) <<
    ...
    int CephContext::do_command(std::string_view command,
...
#pragma GCC pop_options
    ...
    int CephContext::_do_command(std::string_view command,
                                 const cmdmap_t & cmdmap, Formatter * f,
                                 std::ostream & ss, bufferlist * out) {
        int r = 0;
        lgeneric_dout(this,
                      1) << "do_command '" << command << "' '" << cmdmap << "'"
            << dendl;
        ceph_assert_always(!
                           (command == "assert"
                            && _conf->debug_asok_assert_abort));
        if (command == "abort") {
        ...
        if (command == "leak_some_memory") {
            ...
            _perf_counters_collection->dump_formatted(f, false, false, logger,
                                                      counter);
        }
            ...
            cmd_getval(cmdmap, "logger", logger);
            cmd_getval(cmdmap, "counter", counter);
            _perf_counters_collection->dump_formatted_histograms(f, false,
                                                                 logger,
                                                                 counter);
        }
            ...
            if (command == "config show") {
                ...
                if (!argsvec.empty()) {
                    auto args =
                        joinify < std::string > (argsvec.begin(), argsvec.end(),
                                                 " ");
    ...
    CephContext::CephContext(uint32_t module_type_,
                             enum code_environment_t code_env, int init_flags_)
  :    CephContext(module_type_, create_options {
                    code_env, init_flags_, nullptr}) {
    }

    CephContext::CephContext(uint32_t module_type_,
                             const create_options & options)
    :nref(1), _conf {
    options.code_env == CODE_ENVIRONMENT_DAEMON},
        _log(NULL),
        ...
        _heartbeat_map(NULL),
        _crypto_none(NULL), _crypto_aes(NULL), _plugin_registry(NULL),
#ifdef CEPH_DEBUG_MUTEX
        _lockdep_obs(NULL),
#endif
        crush_location(this) {
        ...
        _admin_socket->register_command("perf schema", _admin_hook,
                                        "dump non-labeled counters schemas");
        _admin_socket->register_command("counter dump", _admin_hook,
                                        "dump all labeled and non-labeled counters and their values");
        _admin_socket->register_command("counter schema", _admin_hook,
                                        "dump all labeled and non-labeled counters schemas");
        _admin_socket->register_command("perf histogram schema", _admin_hook,
        ...
        _admin_socket->register_command("config show", _admin_hook,
                                        "dump current config settings");
        _admin_socket->
            register_command("config help name=var,type=CephString,req=false",
                             _admin_hook,
        ...
        _admin_socket->register_command("config unset name=var,type=CephString",
                                        _admin_hook,
                                        "config unset <field>: unset a config variable");
        _admin_socket->register_command("config get name=var,type=CephString",
                                        _admin_hook,
                                        "config get <field>: get the config value");
        _admin_socket->register_command("config diff", _admin_hook,
                                        "dump diff of current config and default config");
        _admin_socket->
            register_command("config diff get name=var,type=CephString",
                             _admin_hook,
        ...
        _admin_socket->
            ...
            _admin_socket->register_command("log flush", _admin_hook,
                                            "flush log entries to log file");
        _admin_socket->register_command("log dump", _admin_hook,
                                        "dump recent log entries to log file");
        _admin_socket->register_command("log reopen", _admin_hook,
                                        "reopen log file");
        ...
        lookup_or_create_singleton_object < MempoolObs > ("mempool_obs", false,
                                                          this);
    }
    ...
    void CephContext::_enable_perf_counter() {
        ...
        plb.add_u64(l_cct_unhealthy_workers, "unhealthy_workers",
                    "Unhealthy workers");
        ...
        for (unsigned i = 0; i < mempool::num_pools; ++i) {
            ...
            _mempool_perf_descriptions.push_back("mempool " s + n +
                                                 " total bytes");
    ...
    void CephContext::_refresh_perf_values() {
        if (_cct_perf) {
            _cct_perf->set(l_cct_total_workers,
                           _heartbeat_map->get_total_workers());
            _cct_perf->set(l_cct_unhealthy_workers,
                           _heartbeat_map->get_unhealthy_workers());
        }
        unsigned l = l_mempool_first + 1;
        for (unsigned i = 0; i < mempool::num_pools; ++i) {
