...
{
    size_t max_len = ENCODED_KEY_PREFIX_LEN + (oid.hobj.nspace.length() * 3 + 1) + (oid.hobj.get_key().length() * 3 + 1) + 1
                     +  // for '<', '=', or '>'
...
BlueStore::Extent *BlueStore::ExtentMap::set_lextent(CollectionRef &c,
...
_resize_shards(bool interval_stats)
{
    ...
    uint64_t max_shard_onodes = static_cast < uint64_t >((meta_alloc / (double)onode_shards) /
...
const char **BlueStore::get_tracked_conf_keys()const
{
    static const char *KEYS[] = {
        ...
        "bluestore_max_defer_interval", NULL
    };
    return KEYS;
}
...
int BlueStore::_set_cache_sizes()
{
    ...
    if (cache_meta_ratio + cache_kv_ratio > 1.0) {
        derr << __func__ << " bluestore_cache_meta_ratio (" <<
             ...
             cache_kv_ratio << "; must be <= 1.0" << dendl;
        return -EINVAL;
    }
    ...
    if (freelist_type == "zoned") {
        ...
        if (!a) {
            lderr(cct) << __func__ << " failed to create " << cct->_conf->
                       bluestore_allocator << " allocator" << dendl;
            delete alloc;
            return -EINVAL;
        }
                           ...
                           uint64_t > *zone_adjustments)
{
    ...
    if (!fm->is_null_manager()) {
        ...
        if (!cct->_conf->bluestore_allocation_from_file) {
            ...
            derr << __func__ << "::NCB::Please change the value of bluestore_allocation_from_file to TRUE in your ceph.conf file" <<
                 dendl;
...
int BlueStore::_open_bluefs(bool create, bool read_only)
{
    ...
    if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW ||
        ...
        if (cct->_conf->bluestore_volume_selection_policy ==
            ...
            vselector = new RocksDBBlueFSVolumeSelector(bluefs->get_block_device_size(BlueFS::BDEV_WAL) * 95 / 100,
                bluefs->get_block_device_size(BlueFS::BDEV_DB) * 95 / 100, bluefs->get_block_device_size(BlueFS::BDEV_SLOW) * 95 / 100,
                        ...
                        bool read_only)
{
    ...
    dout(1) << __func__ << " opened " << kv_backend << " path " <<
            kv_dir_fn << " options " << options << dendl;
    return 0;
}
...
void BlueStore::_close_db()
{
    ...
    if (do_destage && is_statfs_recoverable()) {
        ...
        if (per_pool_stat_collection) {
            ...
            uint64_t pool_id;
            for (it->upper_bound(string()); it->valid();
                 it->next()) {
                           ...
                           FSCK_ObjectCtx &ctx)
{
    ...
    store_statfs_t *res_statfs = (per_pool_stat_collection ||
                                  repairer) ? &ctx.expected_pool_statfs[pool_id] : &ctx.expected_store_statfs;
...
public:
    ...
    template < size_t BatchLen > struct FSCKWorkQueue: public ThreadPool::WorkQueue_ {
        ...
        FSCKWorkQueue(std::string n, size_t _batchCount, BlueStore *_store, ceph::mutex *_sb_info_lock,
                      sb_info_space_efficient_map_t &_sb_info, shared_blob_2hash_tracker_t &_sb_ref_counts, BlueStoreRepairer *_repairer):
        ...
        void _void_process(void *item, TPHandle &handle) override
        {
            ...
            BlueStore::FSCK_ObjectCtx ctx(batch->errors, batch->warnings, batch->num_objects, batch->num_extents, batch->num_blobs,
                                          batch->num_sharded_objects, batch->num_spanning_blobs, nullptr, // used_blocks
                             ...
                             bool repair)
{
    ...
    if (bdev->is_smr() && depth != FSCK_SHALLOW) {
        dout(1) << __func__ << " checking for leaked zone refs" <<
                dendl;
        for (uint32_t zone = 0; zone < zone_refs.size();
             ++zone) {
    ...
    if (repair && repairer.preprocess_misreference(db)) {
        ...
        if (it) {
            ...
            KeyValueDB::Transaction txn =
                repairer.get_fix_misreferences_txn();
...
void BlueStore::_clean_some(ghobject_t oid, uint32_t zone)
{
    ...
    if (to_move.empty()) {
        dout(10) << __func__ << " no references to zone 0x" << std::
                 hex << zone << std::dec << " from " << oid << dendl;
        return;
    }
        ...
        if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
            ...
            if (null_extent.offset || null_extent.length) {
                derr << "illegal trailer - null_extent = [" << null_extent.
                     offset << "," << null_extent.length << "]" << dendl;
                return -1;
            }
...
//---------------------------------------------------------
Allocator *BlueStore::clone_allocator_without_bluefs(Allocator *
        src_allocator)
{
...
//---------------------------------------------------------
Allocator *BlueStore::initialize_allocator_from_freelist(FreelistManager *
        real_fm)
{
