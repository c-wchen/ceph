...
struct test_meta_t {
    ...
    bool operator==(const test_meta_t & rhs) const {
        ...
        return std::make_pair(test_meta_t {
                              t1, pivot}
                              , test_meta_t {
                              pivot, t2}
        );
    ...
    static test_meta_t merge_from(const test_meta_t & lhs,
                                  const test_meta_t & rhs) {
        return test_meta_t {
        lhs.t1, rhs.t2};
    }

    static std::pair < test_meta_t, test_meta_t >
        rebalance(const test_meta_t & lhs, const test_meta_t & rhs,
                  uint32_t pivot) {
        return std::make_pair(test_meta_t {
                              lhs.t1, pivot}
                              , test_meta_t {
...
TEST(FixedKVNodeTest, at_capacity)
{
    ...
    auto iter = node.begin();
    while (num < CAPACITY) {
        node.journal_insert(iter, num, test_val_t {
                            num, num}
                            , nullptr);
        ++num;
        ++iter;
  ...
  for (auto & i:node) {
        ...
        ASSERT_EQ(i.get_val(), (test_val_t {
                                num, num}
                  ));
        if (num < (CAPACITY - 1)) {
...
TEST(FixedKVNodeTest, split)
{
    ...
    ASSERT_EQ(split_right.get_meta().t2,
...
TEST(FixedKVNodeTest, merge)
{
    ...
    while (num < (2 * (CAPACITY / 2))) {
        node2.journal_insert(iter, num, test_val_t {
                             num, num}
                             , nullptr);
        ++num;
        ++iter;
    ...
    ASSERT_EQ(node_merged.get_meta(), (test_meta_t {
                                       0,
                                       std::numeric_limits < uint32_t >::max()}
              ));
...
void run_balance_test(unsigned left, unsigned right, bool prefer_left)
{
    ...
    auto pivot = TestNode::balance_into_new_nodes(node,
                                                  node2,
                                                  prefer_left,
                                                  node_balanced,
                                                  node_balanced2);
    ...
    ASSERT_EQ(node_balanced.get_meta(), (test_meta_t {
                                         0, left_size}
              ));
    ASSERT_EQ(node_balanced2.get_meta(), (test_meta_t {
                                          left_size,
                                          std::numeric_limits <
                                          uint32_t >::max()}
              ));
...
void run_replay_test(std::vector < std::function <
                     void (TestNode &, TestNode::delta_buffer_t &) >> &&f)
{
    TestNode node;
...
TEST(FixedKVNodeTest, replay)
{
    run_replay_test( {
                    [](auto & n, auto & b) {
                    n.journal_insert(n.lower_bound(1), 1, test_val_t {
                                     1, 1}, &b);
                    ASSERT_EQ(1, n.get_size());},[](auto & n, auto & b) {
                    n.journal_insert(n.lower_bound(3), 3, test_val_t {
                                     1, 2}, &b);
                    ASSERT_EQ(2, n.get_size());},[](auto & n, auto & b) {
                    n.journal_remove(n.find(3), &b);
                    ASSERT_EQ(1, n.get_size());},[](auto & n, auto & b) {
                    n.journal_insert(n.lower_bound(2), 2, test_val_t {
                                     5, 1}, &b); ASSERT_EQ(2, n.get_size());}
                    });
