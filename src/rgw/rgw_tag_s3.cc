...
{
    RGWObjTagKey_S3 *key_obj =
        static_cast < RGWObjTagKey_S3 * >(find_first("Key"));
    RGWObjTagValue_S3 *val_obj =
        static_cast < RGWObjTagValue_S3 * >(find_first("Value"));
...
bool RGWObjTagSet_S3::xml_end(const char *)
{
    ...
    RGWObjTagEntry_S3 *tagentry =
        static_cast < RGWObjTagEntry_S3 * >(iter.get_next());
    while (tagentry) {
...
bool RGWObjTagging_S3::xml_end(const char *)
{
    RGWObjTagSet_S3 *tagset =
        static_cast < RGWObjTagSet_S3 * >(find_first("TagSet"));
