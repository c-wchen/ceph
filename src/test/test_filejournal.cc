...
int main(int argc, char **argv)
{
    ...
    if (path[0] == '\0') {
        srand(getpid() + time(0));
        snprintf(path, sizeof(path), "/var/tmp/ceph_test_filejournal.tmp.%d",
                 rand());
    }
...
TEST(TestFileJournal, WriteSmall)
{
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        fj.submit_entry(1, bl, orig_len,
                        new C_SafeCond(&wait_lock, &cond, &done));
        wait();
...
TEST(TestFileJournal, WriteMany)
{
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        C_GatherBuilder gb(g_ceph_context,
                           new C_SafeCond(&wait_lock, &cond, &done));
