...
class TestJournalRecorder:public RadosTestFixture {
  public:
    ...
    void TearDown() override {
        for (std::list < journal::JournalRecorder * >::iterator it =
             m_recorders.begin(); it != m_recorders.end(); ++it) {
            delete *it;
    ...
    journal::JournalRecorder * create_recorder(const std::string & oid,
        ...
        journal::JournalRecorder *
            recorder(new journal::
                     JournalRecorder(m_ioctx, oid + ".", metadata, 0,
...
TEST_F(TestJournalRecorder, AppendKnownOverflow)
{
    ...
    recorder->append(123,
                     create_payload(std::
                                    string(metadata->get_object_size() -
                                           journal::Entry::get_fixed_size(),
                                           '1')));
