...
public:
    ReplicatedBackend(PGBackend::Listener *pg,
                      const coll_t &coll,
    ...
    {
        {
            ...
            for (std::map < pg_shard_t,
                 std::set < hobject_t > >::const_iterator i =
                     pull_from_peer.begin(); i != pull_from_peer.end(); ++i) {
                f->open_object_section("pulling_from");
                f->dump_stream("pull_from") << i->first;
                {
                    f->open_array_section("pulls");
                    for (std::set < hobject_t >::const_iterator j =
        ...
        }
        {
            ...
            for (std::map < hobject_t, std::map < pg_shard_t,
                 PushInfo >>::const_iterator i = pushing.begin();
                 i != pushing.end(); ++i) {
                f->open_object_section("object");
                f->dump_stream("pushing") << i->first;
                {
                    f->open_array_section("pushing_to");
                    for (std::map < pg_shard_t, PushInfo >::const_iterator j =
                             i->second.begin(); j != i->second.end(); ++j) {
...
private:
    ...
    void clear_pull(std::map < hobject_t, PullInfo >::iterator piter,
    ...
    bool handle_push_reply(pg_shard_t peer, const PushReplyOp &op,
                           PushOp *reply);
    ...
    bool handle_pull_response(pg_shard_t from, const PushOp &op,
                              PullOp *response,
    ...
    void submit_push_data(const ObjectRecoveryInfo &recovery_info,
                          ...
                          std::less <>> &attrs, const std::map < std::string,
                          ceph::buffer::list > &omap_entries,
                          ObjectStore::Transaction *t);
    void submit_push_complete(const ObjectRecoveryInfo &recovery_info,
                              ObjectStore::Transaction *t);

    void calc_clone_subsets(SnapSet &snapset, const hobject_t &poid,
                            ...
                            ObcLockManager &lock_manager);
    void prepare_pull(eversion_t v, const hobject_t &soid,
                      ObjectContextRef headctx, RPGHandle *h);
    int start_pushes(const hobject_t &soid, ObjectContextRef obj,
                     RPGHandle *h);
    int prep_push_to_replica(ObjectContextRef obc, const hobject_t &soid,
                             pg_shard_t peer, PushOp *pop,
                             bool cache_dont_need = true);
    int prep_push(ObjectContextRef obc, const hobject_t &oid, pg_shard_t dest,
                  PushOp *op, bool cache_dont_need);
    int prep_push(ObjectContextRef obc, const hobject_t &soid, pg_shard_t peer,
                  eversion_t version, interval_set < uint64_t > &data_subset,
                  std::map < hobject_t,
                  interval_set < uint64_t >> &clone_subsets, PushOp *op,
                  bool cache, ObcLockManager && lock_manager);
    void calc_head_subsets(ObjectContextRef obc, SnapSet &snapset,
                           const hobject_t &head, const pg_missing_t &missing,
                           const hobject_t &last_backfill,
                           interval_set < uint64_t > &data_subset,
                           std::map < hobject_t,
                           interval_set < uint64_t >> &clone_subsets,
    ...
    struct InProgressOp: public RefCountedObject {
        ...
        InProgressOp(ceph_tid_t tid, Context *on_commit, OpRequestRef op,
                     eversion_t v)
...
private:
    Message *generate_subop(const hobject_t &soid,
                            const eversion_t &at_version,
                            ceph_tid_t tid,
                            osd_reqid_t reqid,
    ...
    struct RepModify {
        ...
        RepModify(): committed(false), ackerosd(-1), epoch_started(0)
        {
        }
    };
