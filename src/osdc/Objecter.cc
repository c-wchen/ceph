...
public:
    C_TwoContexts(Context *first, Context *second) : first(first), second(second)
    {
    }
...
Objecter::OSDSession::unique_completion_lock Objecter::OSDSession::
    get_lock(object_t &oid)
{
    if (oid.name.empty())
...
void Objecter::shutdown()
{
    ...
    {
        map<uint64_t, LingerOp *>::iterator i =
            check_latest_map_lingers.begin();
        i->second->put();
    ...
    {
        std::map<ceph_tid_t, Op *>::iterator i =
            homeless_session->ops.begin();
...
void Objecter::handle_osd_map(MOSDMap *m)
{
    ...
    else
    {
        ...
        if (osdmap->get_epoch())
        {
            ...
            for (epoch_t e = osdmap->get_epoch() + 1; e <= m->get_last(); e++)
            {
                    ...
                    m->incremental_maps.count(e))
                {
                    ldout(cct,
                          3)
                        << "handle_osd_map decoding incremental epoch " << e << dendl;
                ...
                else if (m->maps.count(e))
                {
                    ldout(cct,
                          3)
                        << "handle_osd_map decoding full epoch " << e << dendl;
                ...
                else
                {
                    if (e >= m->get_oldest())
                    {
                        ldout(cct,
                              3)
                            << "handle_osd_map requesting missing epoch "
                ...
                for (map<int, OSDSession *>::iterator p =
                         osd_sessions.begin();
                     p != osd_sessions.end();)
                {
        ...
        else
        {
            ...
            else
            {
                ldout(cct,
                      3)
                    << "handle_osd_map hmm, i want a full map, requesting"
                    << dendl;
    ...
    for (map<ceph_tid_t, CommandOp *>::iterator p =
             need_resend_command.begin();
                                     ...
                                     const OSDMap &new_osd_map)
{
    ...
    std::set_difference(new_set.begin(), new_set.end(), old_set.begin(),
                        old_set.end(), std::inserter(delta_set, delta_set.begin()));
...
int Objecter::pool_snap_list(int64_t poolid, vector<uint64_t> *snaps)
{
    ...
    if (!pi)
        return -ENOENT;
    for (map<snapid_t, pool_snap_info_t>::const_iterator p =
             pi->snaps.begin();
         p != pi->snaps.end(); ++p)
    {
                                   ...
                                   Context *fin)
{
    ...
    if (osdmap->get_epoch() >= newest)
    {
        ldout(cct,
              10)
            << __func__ << " latest " << newest << ", have it" << dendl;
        if (fin)
            fin->complete(0);
        return;
    }
                              ...
                              map<uint64_t, LingerOp *> &lresend)
{
    ...
    // resend lingers
    for (map<ceph_tid_t, LingerOp *>::iterator j =
             session->linger_ops.begin();
    ...
    for (map<ceph_tid_t, CommandOp *>::iterator k =
             session->command_ops.begin();
         ...
         siter != osd_sessions.end(); ++siter)
    {
             ...
             p != s->ops.end(); ++p)
        {
            ...
            if (op->stamp < cutoff)
            {
                ldout(cct,
                      2)
                    << " tid " << p->first << " on osd." << op->session->osd << " is laggy" << dendl;
                found = true;
                ++laggy_ops;
            }
        }
             ...
             p != s->linger_ops.end(); ++p)
        {
            ...
            ldout(cct,
                  10)
                << " pinging osd that serves lingering tid " << p->first << " (osd." << op->session->osd << ")" << dendl;
            found = true;
             ...
             p != s->command_ops.end(); ++p)
        {
            ...
            ldout(cct,
                  10)
                << " pinging osd that serves command tid " << p->first << " (osd." << op->session->osd << ")" << dendl;
            found = true;
        }
        if (found)
            toping.insert(s);
...
void Objecter::resend_mon_ops()
{
    ...
    for (map<uint64_t, LingerOp *>::iterator p =
             check_latest_map_lingers.begin();
    ...
    {
        C_Command_Map_Latest *c =
            new C_Command_Map_Latest(this, p->second->tid);
        monc->get_version("osdmap", &c->latest, NULL, c);
    }
}
...
void Objecter::_send_op_account(Op *op)
{
    ...
    for (vector<OSDOp>::iterator p = op->ops.begin(); p != op->ops.end();
         ++p)
    {
...
void Objecter::_op_submit(Op *op, shunique_lock &sul, ceph_tid_t *ptid)
{
    ...
    if (r == -EAGAIN || (check_for_latest_map && sul.owns_lock_shared()))
    {
        ...
        if (orig_epoch != osdmap->get_epoch())
        {
            ...
            ldout(cct,
                  10)
                << __func__ << " relock raced with osdmap, recalc target"
                << dendl;
...
epoch_t Objecter::op_cancel_writes(int r, int64_t pool)
{
         ...
         siter != osd_sessions.end(); ++siter)
    {
        ...
        for (map<ceph_tid_t, Op *>::iterator op_i = s->ops.begin();
             op_i != s->ops.end(); ++op_i)
        {
            if (op_i->second->target.flags & CEPH_OSD_FLAG_WRITE && (pool == -1 || op_i->second->target.target_oloc.pool == pool))
            {
...
bool Objecter::_osdmap_pool_full(const pg_pool_t &p) constconst
{
...
int Objecter::_calc_target(op_target_t *t, Connection *con, bool any_change)
{
    ...
    if (legacy_change || split || force_resend)
    {
        ...
        osdmap->get_primary_shard(pg_t(ceph_stable_mod(pgid.ps(), t->pg_num, t->pg_num_mask),
                                       pgid.pool()),
                                  &t->actual_pgid);
        ...
        ldout(cct, 10) << __func__ << " "
                       ...
                       << " acting " << acting << " primary " << acting_primary << dendl;
        ...
        else
        {
            ...
            if (read && (t->flags & CEPH_OSD_FLAG_BALANCE_READS))
            {
                ...
                ldout(cct,
                      10)
                    << " chose random osd." << osd << " of " << acting << dendl;
            }
...
void Objecter::_send_op(Op *op, MOSDOp *m)
{
    ...
    if (p != op->session->backoffs.end())
    {
        ...
        if (q != p->second.end())
        {
            ldout(cct,
                  20)
                << __func__ << " ? " << q->first << " [" << q->second.begin << "," << q->second.end << ")" << dendl;
            int r = cmp(hoid, q->second.begin);
            if (r == 0 || (r > 0 && hoid < q->second.end))
            {
                ldout(cct,
                      10)
                    << __func__ << " backoff " << op->target.actual_pgid << " id " << q->second.id << " on " << hoid << ", queuing " << op << " tid " << op->tid << dendl;
                return;
            }
        }
    }
...
void Objecter::handle_osd_backoff(MOSDBackoff *m)
{
    ...
    case CEPH_OSD_BACKOFF_OP_UNBLOCK:
    {
        ...
        if (p != s->backoffs_by_id.end())
        {
            ...
            {
                lderr(cct) << __func__ << " got " << m->pgid << " id " << m->id << " unblock on [" << m->begin << "," << m->end << ") but backoff is [" << b->begin << "," << b->end << ")" << dendl;
    ...
    default:
        ldout(cct,
              10)
            << __func__ << " unrecognized op " << (int)m->op << dendl;
    }
...
void Objecter::list_nobjects(NListContext *list_context, Context *onfinish)
{
    ...
    if (list_context->starting_pg_num != pg_num)
    {
        if (!sort_bitwise)
        {
            ...
            ldout(cct,
                  10)
                << " pg_num changed; restarting with " << pg_num << dendl;
    ...
    if (iter != pool_ops.end())
    {
        ...
        if (osdmap->get_epoch() < m->epoch)
        {
            ...
            if (osdmap->get_epoch() < m->epoch)
            {
                ldout(cct,
                      20)
                    << "waiting for client to reach epoch " << m->epoch << " before calling back" << dendl;
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_OSD)
    {
        ...
        if (session)
        {
            ...
            if (!(initialized && osdmap->is_up(session->osd)))
            {
                ldout(cct,
                      1)
                    << "ms_handle_reset aborted,initialized=" << initialized << dendl;
                wl.unlock();
                return false;
            }
            map<uint64_t, LingerOp *> lresend;
...
{
    for (map<uint64_t, CommandOp *>::const_iterator p =
             s->command_ops.begin();
         p != s->command_ops.end(); ++p)
    {
        CommandOp *op = p->second;
...
void Objecter::dump_pool_stat_ops(Formatter *fmt) const const
{
    ...
    for (map<ceph_tid_t, PoolStatOp *>::const_iterator p =
             poolstat_ops.begin();
         p != poolstat_ops.end(); ++p)
    {
...
Objecter::RequestStateHook::RequestStateHook(Objecter *objecter) : m_objecter(objecter)
{
}
...
void Objecter::handle_command_reply(MCommandReply *m)
{
    ...
    OSDSession::shared_lock sl(s->lock);
    map<ceph_tid_t, CommandOp *>::iterator p =
        s->command_ops.find(m->get_tid());
...
struct C_EnumerateReply : public Context
{
    ...
    C_EnumerateReply(Objecter *objecter_, hobject_t *next_,
                     ...
                     Context *on_finish_) : objecter(objecter_), next(next_),
                                            result(result_), end(end_), pool_id(pool_id_), on_finish(on_finish_),
                                            epoch(0), budget(0)
    {
    }
    void finish(int r) override
    {
        objecter->_enumerate_reply(bl, r, end, pool_id, budget, epoch, result,
                                   next, on_finish);
    }
};
                                 ...
                                 hobject_t *next, Context *on_finish)
{
    ...
    ldout(cct,
          20)
        << __func__ << ": start=" << start << " end=" << end << dendl;
...
void Objecter::_enumerate_reply(bufferlist &bl,
                                int r,
                                const hobject_t &end,
                                const int64_t pool_id,
                                int budget,
                                ...
                                hobject_t *next, Context *on_finish)
{
    ...
    else
    {
        ...
        *next = end;
        ...
        {
            uint32_t hash = response.entries.back().locator.empty() ? pool->hash_key(response.entries.back().oid,
            ...
            hobject_t last(response.entries.back().oid,
                           ...
                           CEPH_NOSNAP,
                           hash, pool_id, response.entries.back().nspace);
            if (last < end)
                break;
...
namespace
{
    ...
    template <typename T>
    void do_decode(std::vector<T> &items,
