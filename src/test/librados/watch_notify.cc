...
{
    ASSERT_NE(SEM_FAILED,
              (sem = sem_open("/test_watch_notify_sem", O_CREAT, 0644, 0)));
    char buf[128];
...
TEST_F(LibRadosWatchNotify, Watch3Timeout)
{
    ...
    const uint32_t timeout = 4;
    {
        ...
        ASSERT_EQ(0, rados_conf_get(cluster,
                                    "ms_tcp_read_timeout", conf, sizeof(conf)));
...
INSTANTIATE_TEST_CASE_P(LibRadosWatchNotifyPPTests,
