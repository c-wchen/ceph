...
void RGWOp_Ratelimit_Info::execute(optional_yield y)
{
    ...
    RESTArgs::get_string(s, "ratelimit-scope", ratelimit_scope,
                         &ratelimit_scope);
    ...
    if (exists) {
        if (!boost::iequals(sval, "true") && !boost::iequals(sval, "false")) {
            op_ret = -EINVAL;
            ldpp_dout(this,
                      20) << "global is not equal to true or false" << dendl;
            return;
        }
    }
    ...
    if (ratelimit_scope == "bucket" && !bucket_name.empty() && !global) {
        ...
        int r =
            driver->get_bucket(s, nullptr, tenant_name, bucket_name, &bucket,
                               y);
        if (r != 0) {
            op_ret = r;
    ...
    if (global) {
        ...
        op_ret =
            period_config.read(this,
                               static_cast <
                               rgw::sal::RadosStore * >(driver)->svc()->sysobj,
                               realm_id, y);
        ...
        encode_json("bucket_ratelimit", period_config.bucket_ratelimit,
                    s->formatter);
        encode_json("user_ratelimit", period_config.user_ratelimit,
                    s->formatter);
        encode_json("anonymous_ratelimit", period_config.anon_ratelimit,
                    s->formatter);
        s->formatter->close_section();
        flusher.flush();
        return;
...
void RGWOp_Ratelimit_Set::execute(optional_yield y)
{
    ...
    op_ret =
        RESTArgs::get_int64(s, "max-read-ops", 0, &max_read_ops,
                            &have_max_read_ops);
    op_ret |=
        RESTArgs::get_int64(s, "max-write-ops", 0, &max_write_ops,
                            &have_max_write_ops);
    op_ret |=
        RESTArgs::get_int64(s, "max-read-bytes", 0, &max_read_bytes,
                            &have_max_read_bytes);
    op_ret |=
        RESTArgs::get_int64(s, "max-write-bytes", 0, &max_write_bytes,
    ...
    if (have_enabled) {
        if (!boost::iequals(sval, "true") && !boost::iequals(sval, "false")) {
            ldpp_dout(this,
                      20) << "enabled is not equal to true or false" << dendl;
            op_ret = -EINVAL;
            return;
        }
    }
    ...
    if (exists) {
        if (!boost::iequals(sval, "true") && !boost::iequals(sval, "false")) {
            ldpp_dout(this,
                      20) << "global is not equal to true or faslse" << dendl;
            op_ret = -EINVAL;
            return;
        }
    }
    RESTArgs::get_bool(s, "global", false, &global, nullptr);
    set_ratelimit_info(have_max_read_ops, max_read_ops, have_max_write_ops,
                       max_write_ops, have_max_read_bytes, max_read_bytes,
                       have_max_write_bytes, max_write_bytes, have_enabled,
                       enabled, ratelimit_configured, ratelimit_info);
    if (op_ret) {
        return;
    }
    if (ratelimit_scope == "user" && !uid_str.empty() && !global) {
    ...
    if (ratelimit_scope == "bucket" && !bucket_name.empty() && !global) {
        ...
        std::unique_ptr < rgw::sal::Bucket > bucket;
        op_ret =
            driver->get_bucket(this, nullptr, tenant_name, bucket_name, &bucket,
                               y);
        if (op_ret) {
            ldpp_dout(this, 0) << "Error on getting bucket info" << dendl;
            return;
    ...
    if (global) {
        ...
        if (ratelimit_scope == "bucket") {
            ...
            set_ratelimit_info(have_max_read_ops, max_read_ops,
                               have_max_write_ops, max_write_ops,
                               have_max_read_bytes, max_read_bytes,
                               have_max_write_bytes, max_write_bytes,
                               have_enabled, enabled, ratelimit_configured,
                               ratelimit_info);
            period_config.bucket_ratelimit = ratelimit_info;
            op_ret =
                period_config.write(s,
                                    static_cast <
                                    rgw::sal::RadosStore *
                                    >(driver)->svc()->sysobj, realm_id, y);
            return;
        }
