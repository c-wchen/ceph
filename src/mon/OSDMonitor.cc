...
mapper(mn->cct, &mn->cpu_tp), op_tracker(cct, true, 1)
{
}
...
void OSDMonitor::create_initial()
{
    ...
    if (g_conf->mon_debug_no_require_luminous) {
        ...
        newmap.flags |=
            CEPH_OSDMAP_RECOVERY_DELETES | CEPH_OSDMAP_PURGED_SNAPDIRS;
...
void OSDMonitor::update_msgr_features()
{
    ...
    for (set < int >::iterator q = types.begin(); q != types.end(); ++q) {
        ...
        if ((mon->messenger->get_policy(*q).features_required & mask) !=
            features) {
...
void OSDMonitor::create_pending()
{
    ...
    if (osdmap.crush->has_legacy_rule_ids()) {
      ...
      for (const auto & i:osdmap.get_pools()) {
        ...
        dout(1) << __func__ << " Rewrote " << old_to_new << " crush IDs:" <<
            dendl;
      for (const auto & i:old_to_new) {
            dout(1) << __func__ << " " << i.first << " -> " << i.
                second << dendl;
        }
                                   ...
                                   const OSDMap & nextmap)
{
    ...
    if (pending_creatings.last_scan_epoch < inc.epoch) {
        if (osdmap.get_epoch() &&
            osdmap.require_osd_release < CEPH_RELEASE_LUMINOUS) {
            auto added =
                mon->pgservice->maybe_add_creating_pgs(creating_pgs.
                                                       last_scan_epoch,
                                                       osdmap.get_pools(),
                                                       &pending_creatings);
            dout(7) << __func__ << " " << added << " pgs added from pgmap" <<
                dendl;
        }
        unsigned queued = 0;
...
void OSDMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
        ...
        if (tmp.require_osd_release >= CEPH_RELEASE_LUMINOUS) {
            // remove any legacy osdmap nearfull/full flags
            {
                if (tmp.test_flag(CEPH_OSDMAP_FULL | CEPH_OSDMAP_NEARFULL)) {
                    dout(10) << __func__ <<
                        " clearing legacy osdmap nearfull/full flag" << dendl;
            ...
            if (!backfillfull_pool_ids.empty()) {
              for (auto & p:backfillfull_pool_ids) {
                    ...
                    dout(10) << __func__ << " marking pool '" << tmp.
                        pool_name[p]
            ...
            if (osdmap.require_osd_release < CEPH_RELEASE_LUMINOUS) {
                  ...
                  for (auto & q:p.second) {
                    ...
                    if (changed) {
                        dout(10) << " updated ec profile " << p.first << ": "
                            << newprofile << dendl;
                        pending_inc.new_erasure_code_profiles[p.first] =
                            newprofile;
                    }
                }
              ...
              for (auto & pool_pair:tmp.pools) {
                    ...
                    if (pending_fsmap.pool_in_use(pool_id)) {
                        dout(10) << __func__ <<
                            " auto-enabling CephFS on pool '" << pool_name <<
                            "'" << dendl;
                    ...
                    if (match_count == 1) {
                        ...
                        if (pstat != nullptr
                            && pstat->stats.sum.num_objects > 0) {
                            mon->clog->
                                info() <<
                                "unable to auto-enable application for pool " <<
                                "'" << pool_name << "'";
                        }
                    }
                }
    ...
    for (map < int32_t, entity_addr_t >::iterator i =
...
void OSDMonitor::share_map_with_random_osd()
{
    ...
    dout(10) << "committed, telling random " << s->
        inst << " all about it" << dendl;
...
bool OSDMonitor::preprocess_failure(MonOpRequestRef op)
{
    ...
    if (osdmap.is_down(badboy)) {
        dout(5) << "preprocess_failure dne(/dup?): " << m->
            get_target() << ", from " << m->get_orig_source_inst() << dendl;
        if (m->get_epoch() < osdmap.get_epoch())
            send_incremental(op, m->get_epoch() + 1);
        goto didit;
    }
    if (osdmap.get_inst(badboy) != m->get_target()) {
        dout(5) << "preprocess_failure wrong osd: report " << m->
            get_target() << " != map's " << osdmap.get_inst(badboy)
    ...
    if (osdmap.is_down(badboy) || osdmap.get_up_from(badboy) > m->get_epoch()) {
        dout(5) << "preprocess_failure dup/old: " << m->
            get_target() << ", from " << m->get_orig_source_inst() << dendl;
        if (m->get_epoch() < osdmap.get_epoch())
            send_incremental(op, m->get_epoch() + 1);
        goto didit;
    ...
    if (!can_mark_down(badboy)) {
        dout(5) << "preprocess_failure ignoring report of " << m->
            get_target() << " from " << m->get_orig_source_inst() << dendl;
        goto didit;
    }

    dout(10) << "preprocess_failure new: " << m->
        get_target() << ", from " << m->get_orig_source_inst() << dendl;
    return false;
...
class C_AckMarkedDown:public C_MonOp {
  ...
  public:
    ...
    :C_MonOp(op), osdmon(osdmon) {
        ...
        osdmon->mon->send_reply(op, new MOSDMarkMeDown(m->fsid, m->get_target(), m->get_epoch(), false));   // ACK itself does not request an ack
    }
...
bool OSDMonitor::preprocess_mark_me_down(MonOpRequestRef op)
{
    ...
    if (!osdmap.exists(from) ||
        osdmap.is_down(from) || osdmap.get_addr(from) != m->get_target().addr) {
...
bool OSDMonitor::can_mark_out(int i)
{
    if (osdmap.test_flag(CEPH_OSDMAP_NOOUT)) {
        dout(5) << __func__ << " NOOUT flag set, will not mark osds out" <<
            dendl;
        return false;
    }
...
bool OSDMonitor::check_failure(utime_t now, int target_osd, failure_info_t & fi)
{
    ...
    for (map < int, failure_reporter_t >::iterator p = fi.reporters.begin();
        ...
        map < string, string > reporter_loc =
            osdmap.crush->get_full_location(p->first);
        map < string, string >::iterator iter =
            reporter_loc.find(reporter_subtree_level);
    ...
    if (failed_for >= grace &&
        ...
        g_conf->mon_osd_min_down_reporters) {
...
bool OSDMonitor::prepare_failure(MonOpRequestRef op)
{
    ...
    if (m->if_osd_failed()) {
        ...
        if (m->is_immediate()) {
            mon->clog->debug() << m->
                get_target() << " reported immediately failed by " << m->
...
bool OSDMonitor::preprocess_boot(MonOpRequestRef op)
{
    ...
    if ((osdmap.get_features(CEPH_ENTITY_TYPE_OSD, NULL) &
        ...
        !(m->get_connection()->
          get_features() & CEPH_FEATURE_OSD_ERASURE_CODES)) {
    ...
    if ((osdmap.get_features(CEPH_ENTITY_TYPE_OSD, NULL) &
        ...
        !(m->get_connection()->
          get_features() & CEPH_FEATURE_ERASURE_CODE_PLUGINS_V2)) {
        dout(0) << __func__ <<
            " osdmap requires erasure code plugins v2 but osd at " << m->
            get_orig_source_inst()
            << " doesn't announce support -- ignore" << dendl;
        goto ignore;
    }
    ...
    if ((osdmap.get_features(CEPH_ENTITY_TYPE_OSD, NULL) &
        ...
        !(m->get_connection()->
          get_features() & CEPH_FEATURE_ERASURE_CODE_PLUGINS_V3)) {
        dout(0) << __func__ <<
            " osdmap requires erasure code plugins v3 but osd at " << m->
            get_orig_source_inst()
            << " doesn't announce support -- ignore" << dendl;
        goto ignore;
    }
    ...
    if (any_of(osdmap.get_pools().begin(),
        ...
        if (!(m->osd_features & CEPH_FEATURE_OSD_HITSET_GMT)) {
            dout(0) << __func__ <<
                " one or more pools uses GMT hitsets but osd at " << m->
                get_orig_source_inst()
                << " doesn't announce support -- ignore" << dendl;
            goto ignore;
        }
...
bool OSDMonitor::prepare_boot(MonOpRequestRef op)
{
    op->mark_osdmon_event(__func__);
    MOSDBoot *m = static_cast < MOSDBoot * >(op->get_req());
    dout(7) << __func__ << " from " << m->
        get_orig_source_inst() << " sb " << m->sb << " cluster_addr " << m->
        cluster_addr << " hb_back_addr " << m->
        hb_back_addr << " hb_front_addr " << m->hb_front_addr << dendl;
        ...
        if ((g_conf->mon_osd_auto_mark_auto_out_in &&
            ...
            if (can_mark_in(from)) {
                if (osdmap.osd_xinfo[from].old_weight > 0) {
                    pending_inc.new_weight[from] =
                        osdmap.osd_xinfo[from].old_weight;
                    xi.old_weight = 0;
                }
                else {
...
bool OSDMonitor::preprocess_alive(MonOpRequestRef op)
{
    ...
    if (!osdmap.is_up(from) ||
        osdmap.get_inst(from) != m->get_orig_source_inst()) {
        dout(7) << "preprocess_alive ignoring alive message from down " << m->
            get_orig_source_inst() << dendl;
        goto ignore;
    }
...
bool OSDMonitor::preprocess_pgtemp(MonOpRequestRef op)
{
    ...
    if (!osdmap.is_up(from) ||
        osdmap.get_inst(from) != m->get_orig_source_inst()) {
        dout(7) << "ignoring pgtemp message from down " << m->
            get_orig_source_inst() << dendl;
        goto ignore;
    }
...
bool OSDMonitor::preprocess_remove_snaps(MonOpRequestRef op)
{
    ...
    for (map < int, vector < snapid_t > >::iterator q = m->snaps.begin();
        ...
        if (!osdmap.have_pg_pool(q->first)) {
            dout(10) << " ignoring removed_snaps " << q->
                second << " on non-existent pool " << q->first << dendl;
            continue;
        }
        const pg_pool_t *pi = osdmap.get_pg_pool(q->first);
        for (vector < snapid_t >::iterator p = q->second.begin();
             p != q->second.end(); ++p) {
            if (*p > pi->get_snap_seq() || !pi->removed_snaps.contains(*p))
                return false;
        }
    }
...
bool OSDMonitor::prepare_remove_snaps(MonOpRequestRef op)
{
    ...
    for (map < int, vector < snapid_t > >::iterator p = m->snaps.begin();
        ...
        if (!osdmap.have_pg_pool(p->first)) {
            dout(10) << " ignoring removed_snaps " << p->
                second << " on non-existent pool " << p->first << dendl;
            continue;
        }
        ...
        for (vector < snapid_t >::iterator q = p->second.begin();
            ...
            if (!pi.removed_snaps.contains(*q) &&
                ...
                dout(10) << " pool " << p->
                    first << " removed_snaps added " << *q << " (now " <<
                    newpi->removed_snaps << ")" << dendl;
                if (*q > newpi->get_snap_seq()) {
                    dout(10) << " pool " << p->first << " snap_seq " << newpi->
                        get_snap_seq() << " -> " << *q << dendl;
                                       ...
                                       uint64_t features)
{
    dout(10) << "build_incremental [" << from << ".." << to <<
        "] with features " << std::hex << features << dendl;
                                  ...
                                  bool onetime, MonOpRequestRef req)
{
    dout(5) << "send_incremental [" << first << ".." << osdmap.
        get_epoch() << "]" << " to " << session->inst << dendl;
    ...
    if (first <= session->osd_epoch) {
        dout(10) << __func__ << " " << session->
            inst << " should already have epoch " << session->
            osd_epoch << dendl;
...
epoch_t OSDMonitor::send_pg_creates(int osd, Connection * con, epoch_t next) const const
{
    ...
    for (auto epoch_pgs = creating_pgs_by_epoch->second.lower_bound(next);
      ...
      for (auto & pg:pgs) {
            ...
            m->mkpg.emplace(pg, pg_create_t {
                            create->second.first, pg, 0}
            );
...
void OSDMonitor::tick()
{
    ...
    if (osdmap.require_osd_release >= CEPH_RELEASE_LUMINOUS &&
        mon->monmap->get_required_features().
        contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
    ...
    if (!osdmap.test_flag(CEPH_OSDMAP_PURGED_SNAPDIRS) &&
        ...
        dout(1) << __func__ <<
            " all snapsets converted, setting purged_snapdirs" << dendl;
        ...
        while (i != down_pending_out.end()) {
            ...
            if (osdmap.is_down(o) && osdmap.is_in(o) && can_mark_out(o)) {
                ...
                if (g_conf->mon_osd_down_out_subtree_limit.length()) {
                    int type =
                        osdmap.crush->get_type_id(g_conf->
                                                  mon_osd_down_out_subtree_limit);
                    if (type > 0) {
                        if (osdmap.
                            containing_subtree_is_down(g_ceph_context, o, type,
                                                       &down_cache)) {
                            dout(10) << "tick entire containing " << g_conf->
                                mon_osd_down_out_subtree_limit <<
                                " subtree for osd." << o <<
    ...
    for (ceph::unordered_map < entity_addr_t, utime_t >::iterator p =
         osdmap.blacklist.begin(); p != osdmap.blacklist.end(); ++p) {
        if (p->second < now) {
            dout(10) << "expiring blacklist item " << p->
    ...
    if (osdmap.require_osd_release < CEPH_RELEASE_LUMINOUS &&
        ...
        if (mon->pgservice->have_nearfull_osds()) {
            dout(5) << "There are near full osds, setting nearfull flag" <<
                dendl;
        ...
        if (pending_inc.new_flags != -1 &&
            (pending_inc.new_flags ^ osdmap.
             flags) & (CEPH_OSDMAP_FULL | CEPH_OSDMAP_NEARFULL)) {
            dout(1) << "New setting for" << (pending_inc.
                                             new_flags & CEPH_OSDMAP_FULL ?
                                             " CEPH_OSDMAP_FULL" : "") <<
                (pending_inc.
                 new_flags & CEPH_OSDMAP_NEARFULL ? " CEPH_OSDMAP_NEARFULL" :
                 "")
                << " -- doing propose" << dendl;
            do_propose = true;
        }
                                     ...
                                     std::map < int, utime_t > &last_osd_report)
{
    ...
    for (int i = 0; i < max_osd; ++i) {
        ...
        if (!osdmap.is_up(i))
            continue;
        const std::map < int, utime_t >::const_iterator t =
        ...
        if (t == last_osd_report.end()) {
            ...
            if (diff > timeo) {
                mon->clog->
                    ...
                    << diff << " seconds";
                derr << "no beacon from osd." << i << " since " << t->
                    second << ", " << diff << " seconds ago.  marking down" <<
                    dendl;
    ...
    if (num_osds == 0) {
            ...
            if (!osdmap.is_up(i)) {
                ...
                for (int type = 0; type <= max_type; type++) {
                    if (!osdmap.crush->get_type_name(type))
                        continue;
                    int r =
                        osdmap.crush->get_immediate_parent_id(current,
                                                              &parent_id);
                    if (r == -ENOENT)
                        break;
                    ...
                    if (!osdmap.
                        subtree_type_is_down(g_ceph_context, parent_id, type,
                                             &down_in_osds, &up_in_osds,
        ...
        for (int type = 1; type <= max_type; type++) {
            ...
            for (auto j = subtree_type_down[type].begin();
                ...
                if (type == 1) {
                    ...
                    int num = 0;
                    int num_children =
                        osdmap.crush->get_children(*j, &children);
                    if (num_children == 0)
                        continue;
        ...
        if (num_down_in_osds > 0) {
            ...
            if (detail) {
                ...
                for (auto it = down_in_osds.begin(); it != down_in_osds.end();
                     ++it) {
                    ostringstream ss;
                    ss << "osd." << *it << " (";
        ...
        uint64_t warn_flags =
            ...
            CEPH_OSDMAP_NOTIERAGENT | CEPH_OSDMAP_NOREBALANCE;
        ...
        if (g_conf->mon_warn_on_cache_pools_without_hit_sets) {
            ...
            for (map < int64_t, pg_pool_t >::const_iterator p =
                 osdmap.pools.begin(); p != osdmap.pools.end(); ++p) {
        ...
        if (g_conf->mon_warn_on_osd_down_out_interval_zero &&
            ...
            ostringstream ss;
            ss << "mon." << mon->
                name << " has mon_osd_down_out_interval set to 0";
            summary.push_back(make_pair(HEALTH_WARN, ss.str()));
            if (detail) {
...
bool OSDMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "osd stat") {
    ...
    else if (prefix == "osd perf" || prefix == "osd blocked-by") {
        r = mon->pgservice->process_pg_command(prefix, cmdmap,
             ...
             prefix == "osd ls" ||
             prefix == "osd getmap" ||
             prefix == "osd getcrushmap" || prefix == "osd ls-tree") {
        string val;

        epoch_t epoch = 0;
        int64_t epochnum;
        cmd_getval_throws(g_ceph_context, cmdmap, "epoch", epochnum,
                          (int64_t) osdmap.get_epoch());
        epoch = epochnum;
        ...
        auto sg = make_scope_guard([&]{
                                   ...
                                   if (prefix == "osd tree") {
  vector < string > states; cmd_getval_throws(g_ceph_context, cmdmap, "states", states); unsigned filter = 0; for (auto & s:states)
                                   {
                                   if (s == "up") {
                                   ...
                                   string bucket_name;
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     "name", bucket_name);
                                   set < int >osds;
                                   r =
...
bool first = true; for (auto & i: osds) {
                                   ...
                                   print_osd_utilization(osdmap, mon->pgservice,
                                                         ds, f.get(),
                                                         method == "tree");
                                   rdata.append(ds);}
                                   else
                                   if (prefix == "osd getmaxosd") {
                                   if (f) {
                                   ...
                                   if (prefix == "osd find") {
                                   int64_t osd;
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "id", osd)) {
...
f->close_section(); f->open_array_section("acting"); for (auto osd:acting)
                                   ...
                                   cmd_getval(g_ceph_context, cmdmap, "who",
                                              whostr); vector < string > pvec;
                                   get_str_vec(prefix, pvec);
                                   if (whostr == "*" || whostr == "all"
                                       || whostr == "any") {
                                   ss << "osds "; int c = 0; for (int i = 0;
                                                                  i <
                                                                  osdmap.
                                                                  get_max_osd();
                                                                  i++)
                                   ...
                                   ss << (c++ ? "," : "") << i;
                                   mon->
                                   try_send_message(new
                                                    MOSDScrub(osdmap.get_fsid(),
                                                              pvec.back() ==
                                                              "repair",
                                                              pvec.back() ==
                                                              "deep-scrub"),
                                   ...
                                   for (ceph::unordered_map < entity_addr_t,
                                        utime_t >::iterator p =
                                        osdmap.blacklist.begin();
                                        p != osdmap.blacklist.end(); ++p) {
                                   if (f) {
                                   ...
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     "detail", detail);
                                   if (!f && detail == "detail") {
                                   ostringstream ss; osdmap.print_pools(ss);
                                   rdata.append(ss.str());}
                                   else {
                                   if (f)
                                   f->open_array_section("pools");
                                   for (map < int64_t,
                                        pg_pool_t >::const_iterator it =
                                   ...
                                   assert(i != ALL_CHOICES.end()); switch (*it) {
case PG_NUM:
f->dump_int("pg_num", p->get_pg_num()); break; case PGP_NUM:
f->dump_int("pgp_num", p->get_pgp_num()); break; case AUID:
f->dump_int("auid", p->get_auid()); break; case SIZE:
f->dump_int("size", p->get_size()); break; case MIN_SIZE:
f->dump_int("min_size", p->get_min_size()); break; case CRASH_REPLAY_INTERVAL:
f->dump_int("crash_replay_interval", p->get_crash_replay_interval()); break; case CRUSH_RULE:
                                   if (osdmap.crush->
...
f->dump_bool("allow_ec_overwrites", p->has_flag(pg_pool_t::FLAG_EC_OVERWRITES)); break; case HASHPSPOOL:
case NODELETE:
case NOPGCHANGE:
case NOSIZECHANGE:
...
ss << "auid: " << p->get_auid() << "\n"; break; case SIZE:
ss << "size: " << p->get_size() << "\n"; break; case MIN_SIZE:
ss << "min_size: " << p->get_min_size() << "\n"; break; case CRASH_REPLAY_INTERVAL:
...
case NODEEP_SCRUB:
                                   ...
                                   if (i->second == *it)
                                   break;}
assert(i != ALL_CHOICES.end()); ss << i->first << ": " << (p->has_flag(pg_pool_t::get_flag_by_name(i->first)) ? "true" : "false") << "\n"; break; case MIN_WRITE_RECENCY_FOR_PROMOTE:
...
case CSUM_MIN_BLOCK:
                                   ...
                                   r =
                                   mon->pgservice->process_pg_command(prefix,
                                                                      cmdmap,
                                                                      osdmap,
                                                                      f.get(),
                                                                      &ss,
                                   ...
                                   "'"; r = -ENOENT; goto reply;}
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(poolid); if (f) {
                                   ...
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     "class", class_name);
                                   if (class_name.empty()) {
                                   ss << "no class specified"; r = -EINVAL;
                                   goto reply;}
                                   set < int >rules;
                                   r =
...
ostringstream rs; for (auto & rule:rules) {
                                   ...
                                   string name;
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "node", name)) {
                                   ss << "no node specified"; r = -EINVAL;
                                   goto reply;}
...
reply:
                                   ...
                                   pending_inc.get_new_pool(pool_id,
                                                            ...
                                                            get_pg_pool
                                                            (pool_id));
                                   assert(pool); pool->set_flag(flags);}
                                   ...
                                   pending_inc.get_new_pool(pool_id,
                                                            ...
                                                            get_pg_pool
                                                            (pool_id));
                                   assert(pool); pool->unset_flag(flags);}
                                   ...
                                   mon->pgservice->get_pool_stat(it->first);
                                   if (!pstat)
                                   continue;
                                   ...
                                   op->mark_osdmon_event(__func__);
                                   MPoolOp * m =
                                   static_cast < MPoolOp * >(op->get_req());
                                   ...
                                   prepare_new_pool(m->name, m->auid,
                                                    m->crush_rule, rule_name, 0,
                                                    0, erasure_code_profile,
                                                    pg_pool_t::TYPE_REPLICATED,
                                                    0, FAST_READ_OFF, &ss);
                                   else
                                   ret =
                                   prepare_new_pool(m->name, session->auid,
                                                    m->crush_rule, rule_name, 0,
                                                    0, erasure_code_profile,
                                                    pg_pool_t::TYPE_REPLICATED,
                                                    0, FAST_READ_OFF, &ss);
                                   ...
                                   *ss << "renamed bucket " << srcname <<
                                   ...
                                   strict_iecstrtoll(it->second.c_str(),
                                                     &err_str);
                                   if (!err_str.empty()) {
                                   ...
                                   *ss <<
                                   ...
                                   if (ruleid != -ENOENT) {
                                   *rule =
                                   osdmap.crush->get_rule_mask_ruleset(ruleid);
                                   return -EEXIST;}
                                   ...
                                   if (ruleid != -ENOENT) {
                                   *rule =
                                   newcrush.get_rule_mask_ruleset(ruleid);
                                   return -EALREADY;}
                                   else {
                                   ErasureCodeInterfaceRef erasure_code;
                                   int err =
                                   ...
                                   *ss <<
                                   ...
                                   ceph_release_name(mv)
                                   << " but require_min_compat_client is " <<
                                   ceph_release_name(newmap.
                                   ...
                                   if (p->second.erasure_code_profile ==
                                       profile) {
                                   ...
                                   *ss <<
                                   ...
                                   if (r)
                                   return r;
                                   assert((*erasure_code_profile_map).
                                          count("plugin"));
                                   ...
                                   {
                                   mon->clog->
                                   warn() << "erasure code profile property '"
                                   << key << "' is no longer supported; try " <<
...
*size = g_conf->osd_pool_default_size; *min_size = g_conf->get_osd_pool_default_min_size(); break; case pg_pool_t::TYPE_ERASURE:
                                   {
                                   ErasureCodeInterfaceRef erasure_code;
                                   err =
                                   ...
                                   *min_size =
                                   MIN(erasure_code->get_data_chunk_count() + 1,
                                       *size);}
                                   }
break; default:
...
break; case pg_pool_t::TYPE_ERASURE:
                                   ...
                                   erasure_code->get_data_chunk_count();
                                   uint32_t stripe_unit =
                                   g_conf->osd_pool_erasure_code_stripe_unit;
                                   auto it = profile.find("stripe_unit");
                                   if (it != profile.end()) {
                                   string err_str;
                                   stripe_unit =
...
case pg_pool_t::TYPE_REPLICATED:
                                   ...
                                   *crush_rule =
                                   osdmap.crush->
                                   get_osd_pool_default_crush_replicated_ruleset
                                   (g_ceph_context); if (*crush_rule < 0) {
...
break; default:
                                   ...
                                   *ss << "specified rule " << rule_name <<
                                   ...
                                   int OSDMonitor::check_pg_num(int64_t pool,
                                                                ...
                                                                int size,
                                                                ostream * ss) {
                                   auto max_pgs_per_osd = g_conf->get_val < uint64_t > ("mon_max_pg_per_osd"); auto num_osds = std::max(osdmap.get_num_in_osds(), 3u);  // assume min cluster size 3
                                   auto max_pgs = max_pgs_per_osd * num_osds;
                                   uint64_t projected = 0; if (pool < 0) {
  ...
  for (const auto & i:            osdmap.get_pools()) {
                                   ...
                                   projected +=
                                   i.second.get_pg_num() * i.second.get_size();}
                                   }
                                   if (projected > max_pgs) {
                                   if (pool >= 0) {
                                   *ss << "pool id " << pool;}
                                   *ss << " pg_num " << pg_num << " size " <<
                                   size << " would mean " << projected <<
 ...
 */
                                   ...
                                   if (pg_num >
                                       (unsigned)g_conf->mon_max_pool_pg_num) {
                                   *ss <<
                                   ...
                                   *ss << "crush test failed with " << r << ": "
                                   ...
                                   if (!osdmap.crush->
                                       check_crush_rule(crush_rule, pool_type,
                                                        size, *ss)) {
                                   return -EINVAL;}
...
fread = g_conf->mon_osd_pool_ec_fast_read; break; default:
                                   ...
                                   * 1000000;
                                   ...
                                   int OSDMonitor::
                                   prepare_command_pool_set(map < string,
                                                            cmd_vartype >
                                                            &cmdmap,
                                                            stringstream & ss) {
                                   string poolstr;
                                   ...
                                   cmd_getval(g_ceph_context, cmdmap, "val",
                                              val);
                                   ...
                                   return -EINVAL;}
                                   int r =
                                   check_pg_num(pool, p.get_pg_num(), n, &ss);
                                   if (r < 0) {
                                   return r;}
                                   p.size = n; if (n < p.min_size)
                                   p.min_size = n;}
                                   else
                                   if (var == "min_size") {
                                   if (p.has_flag(pg_pool_t::FLAG_NOSIZECHANGE)) {
                                   ...
                                   if (p.type != pg_pool_t::TYPE_ERASURE) {
                                   if (n < 1 || n > p.size) {
                                   ss << "pool min_size must be between 1 and "
                                   ...
                                   return -ERANGE;}
                                   int r =
                                   check_pg_num(pool, n, p.get_size(), &ss);
                                   if (r) {
                                   return r;}
                                   string force;
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                   ...
                                   return -EPERM;}
                                   int expected_osds =
                                   MIN(p.get_pg_num(), osdmap.get_num_osds());
                                   int64_t new_pgs = n - p.get_pg_num();
                                   if (new_pgs >
                                       g_conf->mon_osd_max_split_count *
                                       expected_osds) {
                                   ss << "specified pg_num " << n <<
                                   " is too large (creating " << new_pgs <<
                                   " new PGs on ~" << expected_osds <<
                                   " OSDs exceeds per-OSD max of " << g_conf->
                                   ...
                                   if (!osdmap.crush->
                                       check_crush_rule(id, p.get_type(),
                                                        p.get_size(), ss)) {
                                   return -EINVAL;}
                                   p.crush_rule = id;}
                                   else
    ...
    // make sure we only compare against 'n' if we didn't receive a string
                                   ...
                                   if (val == "true"
                                       || (interr.empty() && n == 1)) {
                                   string force;
                                   cmd_getval(g_ceph_context, cmdmap, "force",
                                              force);
                                   if (!osdmap.get_num_up_osds()
                                       && force != "--yes-i-really-mean-it") {
                                   ss <<
...
break; default:
                                   ...
                                   err =
                                   newcrush.remove_item_under(g_ceph_context,
                                                              id, ancestor,
                                                              unlink_only);}
                                   else {
                                   err =
                                   newcrush.remove_item(g_ceph_context, id,
                                                        unlink_only);}
                                   return err;}
                                   ...
                                   assert(existing_id); *existing_id = -1;
                                   ...
                                   *new_id = id; goto out;}
                                   ...
                                   pending_inc.new_weight[existing_id] =
                                   CEPH_OSD_OUT; *new_id = existing_id;}
                                   else
                                   if (allocated_id >= 0) {
...
out:
  ...
  // a previously destroyed osd?
                                   ...
                                   assert(existing_id >= 0); id = existing_id;}
                                   else
                                   if (err < 0) {
                                   return err;}
    ...
    // creation, so that we are compatible with `create`.
      ...
      // find an `id`
                                   ...
                                   assert(id >= 0); assert(osdmap.exists(id));}
  ...
  // osd that has been previously destroyed.
    ...
    // nothing to do, really.
      ...
      // to use validate_id.
                                   ...
                                   if (has_lockbox) {
                                   svc =
                                   (ConfigKeyService *) mon->config_key_service;
                                   err =
    ...
    // just return right away.
                                   ...
                                   pending_inc.new_state[id] |=
                                   CEPH_OSD_DESTROYED | CEPH_OSD_NEW;
                                   ...
                                   op->mark_osdmon_event(__func__);
                                   MMonCommand * m =
                                   static_cast < MMonCommand * >(op->get_req());
                                   stringstream ss;
                                   map < string, cmd_vartype > cmdmap;
                                   if (!cmdmap_from_json(m->cmd, &cmdmap, ss)) {
                                   string rs = ss.str();
      ...
      for (auto & osd_weight:     obj) {
                                   ...
                                   int err =
                                   mon->authmon()->validate_osd_destroy(id,
                                                                        uuid,
                                                                        cephx_entity,
                                                                        lockbox_entity,
                                                                        ss);
                                   ...
                                   ConfigKeyService * svc =
                                   (ConfigKeyService *) mon->config_key_service;
                                   err = svc->validate_osd_destroy(id, uuid);
                                   if (err < 0) {
                                   assert(err == -ENOENT); err = 0;
                                   idempotent_cks = true;}
  ...
  // no point destroying the osd again if it has already been marked destroyed
                                   ...
                                   prepare_command_impl(MonOpRequestRef op,
                                                        map < string,
                                                        cmd_vartype > &cmdmap) {
                                   op->mark_osdmon_event(__func__); MMonCommand * m = static_cast < MMonCommand * >(op->get_req()); bool ret = false; stringstream ss; string rs; bufferlist rdata; int err = 0; string format; cmd_getval_throws(g_ceph_context, cmdmap, "format", format, string("plain")); boost::scoped_ptr < Formatter > f(Formatter::create(format)); string prefix; cmd_getval_throws(g_ceph_context, cmdmap, "prefix", prefix); int64_t osdid; string name; bool osdid_present = false; if (prefix != "osd pg-temp" && prefix != "osd pg-upmap" && prefix != "osd pg-upmap-items") {    // avoid commands with non-int id arg
                                   osdid_present =
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     "id", osdid);}
                                   if (osdid_present) {
                                   ostringstream oss;
                                   oss << "osd." << osdid; name = oss.str();}
                                   ...
                                   if (cmd_getval_throws
                                       (g_ceph_context, cmdmap, "prior_version",
                                        prior_version)) {
                                   if (prior_version ==
                                   ...
                                   auto start = ceph::coarse_mono_clock::now();
                                   int r =
                                   tester.test_with_fork(g_conf->mon_lease);
                                   auto duration =
                                   ceph::coarse_mono_clock::now() - start;
                                   if (r < 0) {
                                   dout(10) << " tester.test_with_fork returns "
                                   ...
                                   {
                                   CrushWrapper newcrush;
                                   _get_pending_crush(newcrush); for (int b = 0;
                                                                      b <
                                                                      newcrush.
                                                                      get_max_buckets
                                                                      (); ++b) {
                                   ...
                                   CrushWrapper newcrush;
                                   _get_pending_crush(newcrush);
                                   set < int >updated; for (unsigned j = 0;
                                                            j < idvec.size()
                                                            && !stop; j++) {
                                   set < int >osds;
                                   // wildcard?
...
for (auto & osd:                  osds) {
                                   ...
                                   err =
                                   newcrush.remove_device_class(g_ceph_context,
                                                                osd, &ss);
                                   if (err < 0) {
                                   // ss has reason for failure
                                   goto reply;}
                                   ...
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "srcname",
                                        srcname)) {
                                   err = -EINVAL; goto reply;}
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "dstname",
                                        dstname)) {
                                   err = -EINVAL; goto reply;}
    ...
    // os crush add-bucket <name> <type>
                                   ...
                                   typestr << " to crush map"; goto update;}
                                   else
                                   if (prefix == "osd crush rename-bucket") {
                                   string srcname, dstname; cmd_getval_throws(g_ceph_context, cmdmap, "srcname", srcname); cmd_getval_throws(g_ceph_context, cmdmap, "dstname", dstname); err = crush_rename_bucket(srcname, dstname, &ss); if (err == -EALREADY)   // equivalent to success for idempotency
                                   err = 0; if (err)
                                   goto reply;
                                   else
                                   goto update;}
                                   ...
                                   CrushWrapper newcrush;
                                   _get_pending_crush(newcrush); int64_t pool;
                                   int positions;
                                   ...
                                   (g_ceph_context,
                                    newcrush.choose_args_get(pool),
                                    newcrush.get_item_id(item), weight, &ss);
                                   if (err < 0) {
                                   goto reply;}
                                   err = 0;
                                   pending_inc.crush.clear();
    ...
    // osd crush add <OsdName> <weight> <loc1> [<loc2> ...]
                                   ...
                                   " at location " << loc << ": does not exist";
                                   goto reply;}

                                   dout(5) << "adding/updating crush item id "
                                   << osdid << " name '" << name << "' weight "
                                   << weight << " at location " << loc << dendl;
                                   CrushWrapper newcrush;
                                   _get_pending_crush(newcrush); string action;
                                   if (prefix == "osd crush set"
                                       || newcrush.
                                       check_item_loc(g_ceph_context, osdid,
                                                      loc, (int *)NULL)) {
                                   action = "set";
                                   err =
                                   newcrush.update_item(g_ceph_context, osdid,
                                                        weight, name, loc);}
                                   else {
                                   action = "add";
                                   err =
                                   newcrush.insert_item(g_ceph_context, osdid,
                                                        weight, name, loc);
                                   if (err == 0)
                                   err = 1;}
      ...
      // osd crush create-or-move <OsdName> <initial_weight> <loc1> [<loc2> ...]
                                   ...
                                   _get_pending_crush(newcrush);
                                   err =
                                   newcrush.create_or_move_item(g_ceph_context,
                                                                osdid, weight,
                                                                name, loc);
                                   if (err == 0) {
      ...
      // osd crush move <name> <loc1> [<loc2> ...]
                                   ...
                                   if (!newcrush.
                                       check_item_loc(g_ceph_context, id, loc,
                                                      (int *)NULL)) {
                                   if (id >= 0) {
                                   err =
                                   newcrush.create_or_move_item(g_ceph_context,
                                   ...
                                   err =
                                   newcrush.move_bucket(g_ceph_context, id,
                                                        loc);}
                                   if (err >= 0) {
                                   ...
                                   err = -EPERM; goto reply;}
                                   int r =
                                   newcrush.swap_bucket(g_ceph_context, sid,
                                                        did); if (r < 0) {
    ...
    // osd crush link <name> <loc1> [<loc2> ...]
                                   ...
                                   if (!newcrush.
                                       check_item_loc(g_ceph_context, id, loc,
                                                      (int *)NULL)) {
                                   err =
                                   newcrush.link_bucket(g_ceph_context, id,
                                                        loc); if (err >= 0) {
      ...
      // osd crush rm <id> [ancestor]
                                   ...
                                   string ancestor_str;
                                   if (cmd_getval_throws
                                       (g_ceph_context, cmdmap, "ancestor",
                                        ancestor_str)) {
    ...
    // osd crush reweight <name> <weight>
                                   ...
                                   double w;
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "weight", w)) {
                                   ss << "unable to parse weight value '" <<
                                   cmd_vartype_stringify(cmdmap["weight"]) <<
                                   "'"; err = -EINVAL; goto reply;}

                                   err =
                                   newcrush.adjust_item_weightf(g_ceph_context,
                                                                id, w);
                                   if (err < 0)
    ...
    // osd crush reweight <name> <weight>
                                   ...
                                   err =
                                   newcrush.
                                   adjust_subtree_weightf(g_ceph_context, id,
                                                          w); if (err < 0)
                                   goto reply; pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon->
      ...
      // From the user point of view, the rule is more meaningfull.
                                   ...
                                   err =
                                   check_cluster_features
                                   (CEPH_FEATURE_ERASURE_CODE_PLUGINS_V2, ss);
                                   if (err == -EAGAIN)
                                   goto wait; if (err)
                                   goto reply;}
                                   else
                                   ...
                                   err =
                                   check_cluster_features
                                   (CEPH_FEATURE_ERASURE_CODE_PLUGINS_V3, ss);
                                   if (err == -EAGAIN)
                                   goto wait; if (err)
                                   goto reply;}
                                   err =
                                   ...
                                   err =
                                   osdmap.
                                   get_erasure_code_profile_default
                                   (g_ceph_context, profile_map, &ss); if (err)
                                   goto reply;
                                   err =
...
ss << "rule " << name << " already exists"; err = 0; break; default:   // non recoverable error
                                   ...
                                   // complexity now.
                                   int ruleset =
                                   newcrush.get_rule_mask_ruleset(ruleno);
                                   if (osdmap.crush_rule_in_use(ruleset)) {
                                   ss << "crush ruleset " << name << " " <<
                                   ruleset << " is in use"; err = -EBUSY;
                                   goto reply;}
      ...
      // ss has reason for failure
                                   ...
                                   " which is > conf.mon_max_osd (" << g_conf->
                                   mon_max_osd << ")"; goto reply;}
    ...
    // Note: setmaxosd sets the maximum OSD number and not the number of OSDs
      ...
      // of OSDs.
                                   ...
                                   << ceph_release_name(vno); err = -EPERM;
                                   goto reply;}
                                   string sure;
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     "sure", sure);
                                   if (sure != "--yes-i-really-mean-it") {
FeatureMap m; mon->get_combined_feature_map(&m); uint64_t features = ceph_release_features(vno); bool first = true; bool ok = true; for (int type:{
...
for (auto & q:                    p->second) {
      ...
      // we are reusing a jewel feature bit that was retired in luminous.
                                   ...
                                   pending_inc.new_flags |=
                                   CEPH_OSDMAP_REQUIRE_JEWEL; return r;}
                                   else {
                                   ss <<
...
for (auto & osd:                  osds) {
                                   ...
                                   OP_NOIN,
                                   OP_NOOUT,
                                   } option; if (prefix == "osd add-noup") {
                                   option = OP_NOUP;}
                                   else
                                   if (prefix == "osd add-nodown") {
                                   option = OP_NODOWN;}
                                   else
                                   if (prefix == "osd add-noin") {
                                   option = OP_NOIN;}
                                   else {
                                   option = OP_NOOUT;}
...
break; default:
                                   ...
                                   OP_NOIN,
                                   OP_NOOUT,
                                   } option; if (prefix == "osd rm-noup") {
                                   option = OP_NOUP;}
                                   else
                                   if (prefix == "osd rm-nodown") {
                                   option = OP_NODOWN;}
                                   else
                                   if (prefix == "osd rm-noin") {
                                   option = OP_NOIN;}
                                   else {
                                   option = OP_NOOUT;}
        ...
        // touch previous noup/nodown/noin/noout osds only
                                   switch (option) {
case OP_NOUP:
osdmap.get_noup_osds(&osds); break; case OP_NODOWN:
osdmap.get_nodown_osds(&osds); break; case OP_NOIN:
osdmap.get_noin_osds(&osds); break; case OP_NOOUT:
osdmap.get_noout_osds(&osds); break; default:
                                   assert(0 == "invalid option");}

        // cancel any pending noup/nodown/noin/noout requests too
vector < int >pending_state_osds; (void)pending_inc.get_pending_state_osds(&pending_state_osds); for (auto & p:pending_state_osds) {

                                   switch (option) {
case OP_NOUP:
                                   if (!osdmap.is_noup(p) &&
                                       pending_inc.pending_osd_state_clear(p,
                                                                           CEPH_OSD_NOUP))
                                   {
                                   any = true;}
break; case OP_NODOWN:
...
break; default:
                                   ...
                                   string pgidstr;
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "pgid",
                                        pgidstr)) {
                                   ss << "unable to parse 'pgid' value '" <<
                                   cmd_vartype_stringify(cmdmap["pgid"]) << "'";
                                   err = -EINVAL; goto reply;}
                                   pg_t pgid; if (!pgid.parse(pgidstr.c_str())) {
                                   ss << "invalid pgid '" << pgidstr << "'";
                                   err = -EINVAL; goto reply;}
                                   if (!osdmap.pg_exists(pgid)) {
                                   ...
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "id", id_vec)) {
...
for (auto osd:                    id_vec) {
                                   ...
                                   err =
                                   check_cluster_features
                                   (CEPH_FEATUREMASK_OSDMAP_PG_UPMAP, ss);
                                   if (err == -EAGAIN)
                                   goto wait; if (err < 0)
                                   goto reply;
                                   string pgidstr;
...
case OP_RM_PG_UPMAP:
                                   ...
                                   wait_for_finished_proposal(op,
                                                              ...
                                                              (this, op));
                                   return true;}
break; case OP_PG_UPMAP_ITEMS: // fall through
...
break; default:
                                   ...
                                   err =
                                   check_cluster_features
                                   (CEPH_FEATURE_OSD_PRIMARY_AFFINITY, ss);
                                   if (err == -EAGAIN)
                                   goto wait; if (err < 0)
                                   goto reply; if (osdmap.exists(id)) {
                                   pending_inc.new_primary_affinity[id] = ww;
                                   ss << "set osd." << id <<
                                   " primary-affinity to " << w << " (" << ios::
                                   hex << ww << ios::dec << ")";
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                   ...
                                   err =
                                   parse_reweights(g_ceph_context, cmdmap,
                                                   osdmap, &weights); if (err) {
    ...
    // make sure authmon is writeable.
                                   ...
                                   << "really do."; err = -EPERM; goto reply;}
                                   else
                                   if (!osdmap.exists(id)) {
    ...
    // optional id provided?
                                   ...
                                   if (cmd_getval_throws
                                       (g_ceph_context, cmdmap, "id", cmd_id)) {
                                   if (cmd_id < 0) {
      ...
      // this is an idempotent operation; we can go ahead and reply.
                                   ...
                                   if (prefix == "osd blacklist clear") {
pending_inc.new_blacklist.clear(); std::list < std::pair < entity_addr_t, utime_t > >blacklist; osdmap.get_blacklist(&blacklist); for (const auto & entry:blacklist) {
                                   pending_inc.old_blacklist.push_back(entry.
                                                                       first);}
                                   ...
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     ...
                                                     blacklistop);
                                   if (blacklistop == "add") {
                                   utime_t expires = ceph_clock_now(); double d;
                                   // default one hour
                                   cmd_getval_throws(g_ceph_context, cmdmap,
                                                     "expire", d,
                                                     g_conf->
          ...
          // cancel any pending un-blacklisting request too
                                   auto it =
                                   std::find(pending_inc.old_blacklist.begin(),
                                             pending_inc.old_blacklist.end(),
                                             addr);
                                   if (it != pending_inc.old_blacklist.end()) {
                                   pending_inc.old_blacklist.erase(it);}
                                   }

                                   ss << "blacklisting " << addr << " until " <<
                                   expires << " (" << d << " sec)";
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                   ...
                                   wait_for_finished_proposal(op,
                                                              ...
                                                              C_Command(mon, op,
                                                                        ...
                                                                        1));
                                   return true;}
                                   ss << addr << " isn't blacklisted"; err = 0;
                                   goto reply;}
                                   }
                                   }
                                   else
      ...
      //     and put expected_num_objects to rule field
                                   ...
                                   if (fast_read_param > 0)
                                   fast_read = FAST_READ_ON; err = prepare_new_pool(poolstr, 0, // auid=0 for admin created pool
...
goto reply; default:
    ...
    // osd pool delete/rm <poolname> <poolname again> --yes-i-really-really-mean-it
      ...
      // source pool exists and so does the destination pool
                                   ...
                                   "'"; err = -ENOENT; goto reply;}
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(pool_id); assert(p);
                                   const pg_pool_t * tp =
                                   osdmap.get_pg_pool(tierpool_id); assert(tp);
    ...
    // go
                                   ...
                                   int64_t size = 0;
                                   if (!cmd_getval_throws
                                       (g_ceph_context, cmdmap, "size", size)) {
    ...
    // make sure new tier is empty
                                   ...
                                   goto reply;}
                                   string modestr =
                                   g_conf->osd_tier_default_cache_mode;
                                   pg_pool_t::cache_mode_t mode =
                                   pg_pool_t::get_cache_mode_from_str(modestr);
                                   if (mode < 0) {
    ...
    // go
                                   ...
                                   ntp->cache_mode = mode;
                                   ntp->hit_set_count =
                                   g_conf->osd_tier_default_cache_hit_set_count;
                                   ntp->hit_set_period =
                                   g_conf->
    ...
    // val could contain unit designations, so we treat as a string
                                   ...
                                   "osd test-reweight-by-utilization";
                                   int64_t oload;
                                   cmd_getval(g_ceph_context, cmdmap, "oload",
                                              oload, int64_t(120));
                                   set < int64_t > pools;
                                   vector < string > poolnamevec;
                                   cmd_getval(g_ceph_context, cmdmap, "pools",
                                              poolnamevec); for (unsigned j = 0;
                                                                 j <
                                                                 poolnamevec.
                                                                 size(); j++) {
                                   int64_t pool =
                                   osdmap.lookup_pg_pool_name(poolnamevec[j]);
                                   if (pool < 0) {
                                   ss << "pool '" << poolnamevec[j] <<
                                   "' does not exist"; err = -ENOENT;
                                   goto reply;}
                                   pools.insert(pool);}
                                   double max_change =
                                   g_conf->mon_reweight_max_change;
                                   cmd_getval(g_ceph_context, cmdmap,
                                              "max_change", max_change);
                                   if (max_change <= 0.0) {
                                   ss << "max_change " << max_change <<
                                   " must be positive"; err = -EINVAL;
                                   goto reply;}
                                   int64_t max_osds =
                                   g_conf->mon_reweight_max_osds;
                                   cmd_getval(g_ceph_context, cmdmap,
                                              "max_osds", max_osds);
                                   if (max_osds <= 0) {
                                   ss << "max_osds " << max_osds <<
                                   " must be positive"; err = -EINVAL;
                                   goto reply;}
                                   ...
                                   string out_str;
                                   mempool::osdmap::map < int32_t,
                                   uint32_t > new_weights;
                                   err =
                                   mon->pgservice->
                                   reweight_by_utilization(osdmap, oload,
                                                           max_change, max_osds,
                                                           by_pg,
                                                           pools.
                                                           empty()? NULL :
                                   ...
                                   if (err >= 0) {
                                   dout(10) <<
                                   "reweight::by_utilization: finished with " <<
                                   out_str << dendl;}
                                   if (f)
                                   f->flush(rdata);
                                   else
                                   rdata.append(out_str); if (err < 0) {
                                   ...
                                   if (creating_now) {
                                   ss << "pg " << pgidstr <<
                                   " now creating, ok"; err = 0; goto update;}
                                   else {
...
return false; default:
                                   ...
                                   const pg_pool_t * pool =
                                   osdmap.get_pg_pool(m->pool); switch (m->op) {
case POOL_OP_CREATE_SNAP:
                                   if (pool->is_tier()) {
...
break; case POOL_OP_DELETE_SNAP:
                                   ...
                                   if (s) {
                                   pp.remove_snap(s); changed = true;}
                                   }
break; case POOL_OP_CREATE_UNMANAGED_SNAP:
                                   {
                                   uint64_t snapid;
                                   pp.
                                   add_unmanaged_snap(snapid);::encode(snapid,
                                                                       reply_data);
                                   changed = true;}
