...
struct rgw_obj_index_key {      // cls_rgw_obj_key now aliases this type
    ...
    rgw_obj_index_key(const std::string & n, const std::string & i):name(n),
        instance(i) {
    }
    ...
    size_t estimate_encoded_size() const {
        ...
        constexpr size_t string_overhead = sizeof(__u32);   // strings are encoded with 32-bit length prefix
         return start_overhead +
            string_overhead + name.size() + string_overhead + instance.size();
}};
...
rgw_obj_key(const std::string & n, const std::string & i,
            const std::string & _ns):name(n), instance(i), ns(_ns) {
}
...
static void parse_index_key(const std::string & key, std::string * name,
                            std::string * ns) {
    if (key[0] != '_') {
        *name = key;
        ns->clear();
...
static bool parse_raw_oid(const std::string & oid, rgw_obj_key * key) {
   ...
   */
static bool oid_to_key_in_ns(const std::string & oid, rgw_obj_key * key,
                             const std::string & ns) {
   ...
   * false if it fails.
   */
static bool strip_namespace_from_name(std::string & name, std::string & ns,
                                      std::string & instance) {
    ns.clear();
    instance.clear();
    if (name[0] != '_') {
...
template <> struct fmt::formatter < rgw_obj_key >:fmt::formatter <
    std::string_view > {
    template < typename FormatContext > auto format(const rgw_obj_key & key,
...
struct rgw_raw_obj {
    ...
    rgw_raw_obj(const rgw_pool & _pool, const std::string & _oid,
                const std::string & _loc):loc(_loc) {
        init(_pool, _oid);
    }
...
struct rgw_obj {
    ...
    rgw_obj() {
    } rgw_obj(const rgw_bucket & b, const std::string & name):bucket(b),
        key(name) {
    }
    ...
    void init(const rgw_bucket & b, const std::string & name,
              const std::string & i, const std::string & n) {
        bucket = b;
        key.set(name, i, n);
    }

    void init_ns(const rgw_bucket & b, const std::string & name,
                 const std::string & n) {
        bucket = b;
        key.name = name;
        key.instance.clear();
