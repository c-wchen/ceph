...
public:
    ...
    int err_count {
        0};
    int expected_err_count {
        0};
    void set_expected_err_count(int c)
    {
...
using CorruptFunc =
    std::function < RealObj(const RealObj &s,
                            [[maybe_unused]] int osd_num) >;
...
struct SnapsetMockData {
    ...
    SnapsetMockData(snapid_t seq,
                    ...
                    std::map < snapid_t,
                    interval_set < uint64_t >> clone_overlap,
                    std::map < snapid_t, uint64_t > clone_size,
                    std::map < snapid_t,
                    std::vector < snapid_t >> clone_snaps)
        : seq(seq)
        , snaps(snaps)
        , clones(clones)
...
template <> struct fmt::formatter < ScrubGenerator::RealObj > {
    ...
    auto format(const ScrubGenerator::RealObj &rlo, FormatContext &ctx)
    {
        ...
        return fmt::format_to(ctx.out(),
                              ...
                              rlo.ghobj,
                              rlo.data.size,
                              (rlo.snapset_mock_data ? rlo.snapset_mock_data->
        snaps : std::vector < snapid_t > {
        }
                              ));
    }
};
