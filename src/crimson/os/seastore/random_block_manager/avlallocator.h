...
namespace crimson::os::seastore {
    ...
    struct extent_range_t {
        ...
        extent_range_t(rbm_abs_addr start, rbm_abs_addr end):start(start),
            end(end) {
        } struct before_t {
            template < typename KeyLeft, typename KeyRight >
                bool operator() (const KeyLeft & lhs,
                                 const KeyRight & rhs)const {
        ...
        struct shorter_t {
            template < typename KeyType >
                bool operator() (const extent_range_t & lhs,
 ...
 */
    class AvlAllocator:public ExtentAllocator {
      public:
        AvlAllocator(bool detailed):detailed(detailed) {
        } std::optional < interval_set <
            rbm_abs_addr >> alloc_extent(size_t size) final;
        ...
        rbm_extent_state_t get_extent_state(rbm_abs_addr addr,
                                            size_t size) final {
