...
{
private:
    const static opentelemetry::nostd::shared_ptr <
    opentelemetry::trace::Tracer > noop_tracer;
...
public:
    ...
    jspan add_span(opentelemetry::nostd::string_view span_name,
...
void encode(const jspan_context &span, ceph::buffer::list &bl,
            uint64_t f = 0);
void decode(jspan_context &span_ctx,
...
struct span_stub {
    ...
    } void AddEvent(std::string_view)
    {
                    ...
                    jspan_attribute >> fields)
    {
    }
    template < typename T > void AddEvent(std::string_view name,
    const T &fields = {
    })
    {
    }
    const jspan_context &GetContext()
    {
        return _ctx;
    }
...
public:
    ...
    {
        return span;
    } const span_stub &operator*() const
    {
        return span;
    } span_stub *operator->()
    {
...
struct Tracer {
    ...
    {
        return false;
    } jspan start_trace(std::string_view, bool enabled = true)
    {
        return {
        };
    } jspan add_span(std::string_view, const jspan &)
    {
        return {
        };
    }
    jspan add_span(std::string_view span_name,
                   const jspan_context &parent_ctx)
    {
        return {
        };
...
inline void encode(const jspan_context &span, bufferlist &bl, uint64_t f =
                       0)
{
}
inline void decode(jspan_context &span_ctx,
                   ceph::buffer::list::const_iterator &bl)
{
}
}
