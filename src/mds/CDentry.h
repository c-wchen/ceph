...
public:
    ...
    CDentry(std::string_view n, __u32 h, mempool::mds_co::string alternate_name, snapid_t f, snapid_t l):
        ...
        name(n), alternate_name(std::move(alternate_name))
    {
    }
    CDentry(std::string_view n, __u32 h,
            mempool::mds_co::string alternate_name,
            inodeno_t ino, unsigned char dt,
            snapid_t f, snapid_t l): hash(h),
        first(f), last(l),
        item_dirty(this),
        lock(this, &lock_type),
        versionlock(this, &versionlock_type),
    ...
    std::string_view pin_name(int p) const override
    {
        switch (p) {
            ...
            case PIN_SCRUBPARENT:
                return "scrubparent";
            case PIN_WAITUNLINKSTATE:
                return "waitunlinkstate";
            default:
                return generic_pin_name(p);
        }
    }
    // -- wait --//static const int WAIT_LOCK_OFFSET = 8;
    ...
    // linkage const linkage_t *get_linkage() const {
    return &linkage;
...
{
    return use_projected(client,
                         mut) ? get_projected_linkage() : get_linkage();
}
...
static void decode_remote(char icode, inodeno_t &ino,
