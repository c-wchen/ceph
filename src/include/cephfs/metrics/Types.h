                                ...
                                const ClientMetricType &type)
{
    switch (type) {
        ...
        case ClientMetricType::CLIENT_METRIC_TYPE_STDEV_METADATA_LATENCY:
            ...
            break;
        default:
            os << "(UNKNOWN:" << static_cast < std::underlying_type <
               ClientMetricType >::type >(type) << ")";
            break;
    }
...
struct CapInfoPayload: public ClientMetricPayloadBase {
    ...
    } void print(std::ostream *out) const
    {
        *out << "cap_hits: " << cap_hits << " "
             << "cap_misses: " << cap_misses << " " << "num_caps: " << nr_caps;
    }
};
...
struct ReadLatencyPayload: public ClientMetricPayloadBase {
    ...
    {
    } ReadLatencyPayload(utime_t lat, utime_t mean, uint64_t sq_sum,
                         uint64_t count)
        : ClientMetricPayloadBase(ClientMetricType::CLIENT_METRIC_TYPE_READ_LATENCY),
          lat(lat), mean(mean), sq_sum(sq_sum), count(count)
    {
    }
...
struct WriteLatencyPayload: public ClientMetricPayloadBase {
    ...
    {
    } WriteLatencyPayload(utime_t lat, utime_t mean, uint64_t sq_sum,
                          uint64_t count)
        : ClientMetricPayloadBase(ClientMetricType::
                                  CLIENT_METRIC_TYPE_WRITE_LATENCY), lat(lat),
          mean(mean), sq_sum(sq_sum), count(count)
    {
    }
...
struct MetadataLatencyPayload: public ClientMetricPayloadBase {
    ...
    MetadataLatencyPayload()
        : ClientMetricPayloadBase(ClientMetricType::
                                  CLIENT_METRIC_TYPE_METADATA_LATENCY)
    {
    } MetadataLatencyPayload(utime_t lat, utime_t mean, uint64_t sq_sum,
                             uint64_t count)
        : ClientMetricPayloadBase(ClientMetricType::
                                  CLIENT_METRIC_TYPE_METADATA_LATENCY), lat(lat),
          mean(mean), sq_sum(sq_sum), count(count)
    {
    }
...
struct DentryLeasePayload: public ClientMetricPayloadBase {
    ...
    {
    } DentryLeasePayload(uint64_t dlease_hits, uint64_t dlease_misses,
                         uint64_t nr_dentries)
        : ClientMetricPayloadBase(ClientMetricType::CLIENT_METRIC_TYPE_DENTRY_LEASE),
          dlease_hits(dlease_hits), dlease_misses(dlease_misses),
...
struct OpenedInodesPayload: public ClientMetricPayloadBase {
        ...
        : ClientMetricPayloadBase(ClientMetricType::CLIENT_METRIC_TYPE_OPENED_INODES)
    {
        ...
        : ClientMetricPayloadBase(ClientMetricType::
                                  CLIENT_METRIC_TYPE_OPENED_INODES), opened_inodes(opened_inodes), total_inodes(total_inodes)
    {
    }
...
struct ReadIoSizesPayload: public ClientMetricPayloadBase {
        ...
        : ClientMetricPayloadBase(ClientMetricType::CLIENT_METRIC_TYPE_READ_IO_SIZES)
    {
        ...
        : ClientMetricPayloadBase(ClientMetricType::
                                  CLIENT_METRIC_TYPE_READ_IO_SIZES), total_ops(total_ops), total_size(total_size)
    {
    }
...
struct WriteIoSizesPayload: public ClientMetricPayloadBase {
                                  ...
                                  CLIENT_METRIC_TYPE_WRITE_IO_SIZES)
    {
        ...
        : ClientMetricPayloadBase(ClientMetricType::
                                  CLIENT_METRIC_TYPE_WRITE_IO_SIZES), total_ops(total_ops), total_size(total_size)
    {
    }
...
typedef boost::variant < CapInfoPayload,
        ...
        OpenedInodesPayload,
        ReadIoSizesPayload,
        WriteIoSizesPayload, UnknownPayload > ClientMetricPayload;
...
public:
    ...
    public:
        ...
        inline void operator()(const ClientMetricPayload &payload)const
        {
            m_formatter->dump_string("client_metric_type",
                                     stringify(payload.get_type()));
    ...
    } void decode(bufferlist::const_iterator &iter)
    {
        ...
        switch (metric_type) {
            ...
            default:
                payload =
                    UnknownPayload(static_cast < ClientMetricType >(metric_type));
                break;
        }
