...
void inode_backpointer_t::encode(bufferlist & bl) const const
{
    ENCODE_START(2, 2, bl);
    ::encode(dirino, bl);
    ::encode(dname, bl);
...
void inode_backpointer_t::dump(Formatter * f) const const
{
...
void inode_backpointer_t::generate_test_instances(list <
...
void inode_backtrace_t::encode(bufferlist & bl) const const
{
    ENCODE_START(5, 4, bl);
    ::encode(ino, bl);
    ::encode(ancestors, bl);
...
void inode_backtrace_t::dump(Formatter * f) const const
{
    f->dump_unsigned("ino", ino);
    f->open_array_section("ancestors");
    for (vector < inode_backpointer_t >::const_iterator p = ancestors.begin();
         p != ancestors.end(); ++p) {
