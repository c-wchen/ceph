    ...
    namespace logging {
        ...
        void Log::_configure_stderr() {
            ...
            if (int rc = fstat(m_fd_stderr, &info); rc == -1) {
                std::
                    cerr << "failed to stat stderr: " << cpp_strerror(errno) <<
                    std::endl;
                return;
            }
            ...
            if (S_ISFIFO(info.st_mode)) {
                ...
                if (flags == -1) {
                    std::
                        cerr << "failed to get fcntl flags for stderr: " <<
                ...
                if (!(flags & O_NONBLOCK)) {
                    ...
                    if (flags == -1) {
                        std::
                            cerr << "failed to set fcntl flags for stderr: " <<
                            cpp_strerror(errno) << std::endl;
                        return;
                    }
                }
...
///
        ...
        void Log::_flush(EntryVector & t, bool crash) {
          ...
          for (auto & e:t) {
                ...
                if (do_fd || do_syslog || do_stderr) {
                    ...
                    if (crash) {
                        used +=
                            (std::size_t) snprintf(pos + used, allocated - used,
                                                   "%6ld> ", -(--len));
                    }
                    used +=
                        (std::size_t) append_time(stamp, pos + used,
                                                  allocated - used);
                    used +=
                        (std::size_t) snprintf(pos + used, allocated - used,
                                               " %lx %2d ",
                                               (unsigned long)thread, prio);
        ...
        void Log::dump_recent() {
            ...
            {
                EntryVector t;
                t.insert(t.end(), std::make_move_iterator(m_recent.begin()),
          ...
          for (const auto & p:m_subs->m_subsys) {
                _log_message(fmt::format("  {:2d}/{:2d} {}",
                                         p.log_level, p.gather_level, p.name),
                             true);
            }
            ...
            _log_message("--- pthread ID / name mapping for recent threads ---",
                         true);
          for (const auto pthread_id:recent_pthread_ids) {
                char pthread_name[16] = { 0 };  //limited by 16B include terminating null byte.
                ceph_pthread_getname(pthread_id, pthread_name,
                                     sizeof(pthread_name));
            ...
            _log_message(fmt::format("  max_recent {:9}", m_recent.capacity()),
                         true);
