...
#define RBD_MIRROR_IMAGE_STATUS_LOCAL_MIRROR_UUID ""
    ...
    typedef int (*librbd_progress_fn_t) (uint64_t offset, uint64_t total,
                                         void *ptr);
...
/* rbd_write_zeroes / rbd_aio_write_zeroes flags */
    ...
    CEPH_RBD_API int rbd_image_options_unset(rbd_image_options_t opts,
                                             int optname);
...
/* helpers */
    ...
    CEPH_RBD_API void rbd_image_spec_list_cleanup(rbd_image_spec_t * images,
                                                  size_t num_images);
    CEPH_RBD_API void rbd_linked_image_spec_cleanup(rbd_linked_image_spec_t *
                                                    image);
    CEPH_RBD_API void rbd_linked_image_spec_list_cleanup(rbd_linked_image_spec_t
                                                         * images,
                                                         size_t num_images);
...
/* images */
    ...
    CEPH_RBD_API int rbd_create(rados_ioctx_t io, const char *name,
                                uint64_t size, int *order);
    CEPH_RBD_API int rbd_create2(rados_ioctx_t io, const char *name,
 ...
 */
    CEPH_RBD_API int rbd_create3(rados_ioctx_t io, const char *name,
                                 uint64_t size, uint64_t features, int *order,
                                 uint64_t stripe_unit, uint64_t stripe_count);
    CEPH_RBD_API int rbd_create4(rados_ioctx_t io, const char *name,
    ...
    CEPH_RBD_API int rbd_clone2(rados_ioctx_t p_ioctx, const char *p_name,
                                const char *p_snapname, rados_ioctx_t c_ioctx,
                                const char *c_name, uint64_t features,
                                int *c_order, uint64_t stripe_unit,
                                int stripe_count);
    ...
    CEPH_RBD_API int rbd_remove_with_progress(rados_ioctx_t io,
                                              const char *name,
    ...
    CEPH_RBD_API int rbd_trash_list(rados_ioctx_t io,
                                    ...
                                    size_t * num_entries);
    CEPH_RBD_API void rbd_trash_list_cleanup(rbd_trash_image_info_t *
                                             trash_entries, size_t num_entries);
    CEPH_RBD_API int rbd_trash_purge(rados_ioctx_t io, time_t expire_ts,
                                     float threshold);
    CEPH_RBD_API int rbd_trash_purge_with_progress(rados_ioctx_t io,
                                                   time_t expire_ts,
                                                   float threshold,
                                                   librbd_progress_fn_t cb,
                                                   void *cbdata);
    CEPH_RBD_API int rbd_trash_remove(rados_ioctx_t io, const char *id,
                                      bool force);
...
/* migration */
    ...
    CEPH_RBD_API int rbd_migration_prepare_import(const char *source_spec,
                                                  rados_ioctx_t dest_ioctx,
                                                  const char *dest_image_name,
                                                  rbd_image_options_t opts);
    ...
    CEPH_RBD_API int rbd_migration_status(rados_ioctx_t ioctx,
                                          ...
                                          size_t status_size);
    CEPH_RBD_API void rbd_migration_status_cleanup(rbd_image_migration_status_t
                                                   * status);
...
/* pool mirroring */
    ...
    CEPH_RBD_API int rbd_mirror_peer_bootstrap_create(rados_ioctx_t io_ctx,
                                                      char *token,
                                                      size_t * max_len);
    CEPH_RBD_API int rbd_mirror_peer_bootstrap_import(rados_ioctx_t io_ctx,
                                                      rbd_mirror_peer_direction_t
                                                      direction,
                                                      const char *token);

    CEPH_RBD_API int rbd_mirror_peer_site_add(rados_ioctx_t io_ctx, char *uuid,
                                              size_t uuid_max_length,
                                              rbd_mirror_peer_direction_t
                                              direction, const char *site_name,
                                              const char *client_name);
    CEPH_RBD_API int rbd_mirror_peer_site_set_name(rados_ioctx_t io_ctx,
                                                   const char *uuid,
                                                   const char *site_name);
    CEPH_RBD_API int rbd_mirror_peer_site_set_client_name(rados_ioctx_t io_ctx,
                                                          const char *uuid,
                                                          const char
                                                          *client_name);
    CEPH_RBD_API int rbd_mirror_peer_site_set_direction(rados_ioctx_t io_ctx,
                                                        const char *uuid,
                                                        rbd_mirror_peer_direction_t
                                                        direction);
    CEPH_RBD_API int rbd_mirror_peer_site_remove(rados_ioctx_t io_ctx,
                                                 const char *uuid);
    CEPH_RBD_API int rbd_mirror_peer_site_list(rados_ioctx_t io_ctx,
                                               rbd_mirror_peer_site_t * peers,
                                               int *max_peers);
    CEPH_RBD_API void rbd_mirror_peer_site_list_cleanup(rbd_mirror_peer_site_t *
                                                        peers, int max_peers);
    CEPH_RBD_API int rbd_mirror_peer_site_get_attributes(rados_ioctx_t p,
                                                         const char *uuid,
                                                         char *keys,
                                                         size_t * max_key_len,
                                                         char *values,
                                                         ...
                                                         size_t *
                                                         key_value_count);
    CEPH_RBD_API int rbd_mirror_peer_site_set_attributes(rados_ioctx_t p,
                                                         const char *uuid,
                                                         const char *keys,
                                                         const char *values,
                                                         size_t
    ...
    CEPH_RBD_API int rbd_mirror_image_global_status_list(rados_ioctx_t io_ctx,
                                                         const char *start_id,
                                                         size_t max,
                                                         char **image_ids,
                                                         rbd_mirror_image_global_status_t
                                                         * images,
                                                         size_t * len);
    CEPH_RBD_API void rbd_mirror_image_global_status_list_cleanup(char
                                                                  **image_ids,
                                                                  rbd_mirror_image_global_status_t
                                                                  * images,
                                                                  size_t len);
 ...
 * equivalents */
    CEPH_RBD_API int rbd_mirror_peer_add(rados_ioctx_t io_ctx, char *uuid,
                                         size_t uuid_max_length,
    ...
    CEPH_RBD_API int rbd_mirror_peer_set_cluster(rados_ioctx_t io_ctx,
                                                 ...
                                                 const char *cluster_name)
     CEPH_RBD_DEPRECATED;
    CEPH_RBD_API int rbd_mirror_peer_get_attributes(rados_ioctx_t p,
                                                    const char *uuid,
                                                    char *keys,
                                                    size_t * max_key_len,
                                                    char *values,
     ...
     CEPH_RBD_DEPRECATED;
    CEPH_RBD_API int rbd_mirror_peer_set_attributes(rados_ioctx_t p,
                                                    const char *uuid,
                                                    const char *keys,
                                                    const char *values,
                                                    size_t key_value_count)
 ...
 * rbd_mirror_image_global_status_list_ commands */
    ...
    CEPH_RBD_API int rbd_mirror_image_instance_id_list(rados_ioctx_t io_ctx,
                                                       ...
                                                       char **instance_ids,
                                                       size_t * len);
    CEPH_RBD_API void rbd_mirror_image_instance_id_list_cleanup(char
                                                                **image_ids,
                                                                char
                                                                **instance_ids,
                                                                size_t len);
    CEPH_RBD_API int rbd_mirror_image_info_list(rados_ioctx_t io_ctx,
                                                rbd_mirror_image_mode_t *
                                                mode_filter,
                                                const char *start_id,
                                                size_t max, char **image_ids,
                                                ...
                                                info_entries,
                                                size_t * num_entries);
    CEPH_RBD_API void rbd_mirror_image_info_list_cleanup(char **image_ids,
                                                         rbd_mirror_image_info_t
                                                         * info_entries,
                                                         size_t num_entries);

/* pool metadata */
    CEPH_RBD_API int rbd_pool_metadata_get(rados_ioctx_t io_ctx,
                                           const char *key, char *value,
                                           size_t * val_len);
    CEPH_RBD_API int rbd_pool_metadata_set(rados_ioctx_t io_ctx,
                                           const char *key, const char *value);
    CEPH_RBD_API int rbd_pool_metadata_remove(rados_ioctx_t io_ctx,
                                              const char *key);
    CEPH_RBD_API int rbd_pool_metadata_list(rados_ioctx_t io_ctx,
                                            const char *start, uint64_t max,
    ...
    CEPH_RBD_API int rbd_config_pool_list(rados_ioctx_t io_ctx,
                                          ...
                                          int *max_options);
    CEPH_RBD_API void rbd_config_pool_list_cleanup(rbd_config_option_t *
 ...
 */
    ...
    CEPH_RBD_API int rbd_aio_open_read_only(rados_ioctx_t io, const char *name,
                                            ...
                                            const char *snap_name,
                                            rbd_completion_t c);
    CEPH_RBD_API int rbd_aio_open_by_id_read_only(rados_ioctx_t io,
                                                  const char *id,
                                                  rbd_image_t * image,
                                                  const char *snap_name,
                                                  rbd_completion_t c);
    CEPH_RBD_API int rbd_features_to_string(uint64_t features,
                                            char *str_features, size_t * size);
    CEPH_RBD_API int rbd_features_from_string(const char *str_features,
                                              uint64_t * features);
    ...
    CEPH_RBD_API int rbd_resize2(rbd_image_t image, uint64_t size,
                                 bool allow_shrink, librbd_progress_fn_t cb,
                                 void *cbdata);
    ...
    CEPH_RBD_API int rbd_update_features(rbd_image_t image, uint64_t features,
                                         uint8_t enabled);
    CEPH_RBD_API int rbd_get_op_features(rbd_image_t image,
                                         uint64_t * op_features);
    CEPH_RBD_API int rbd_get_stripe_unit(rbd_image_t image,
    ...
    CEPH_RBD_API int rbd_get_name(rbd_image_t image, char *name,
                                  size_t * name_len);
    ...
    CEPH_RBD_API int rbd_get_parent_info(rbd_image_t image,
                                         ...
                                         size_t ppoolnamelen, char *parent_name,
                                         size_t pnamelen, char *parent_snapname,
                                         size_t psnapnamelen)
     CEPH_RBD_DEPRECATED;
    ...
    CEPH_RBD_API int rbd_get_group(rbd_image_t image,
                                   rbd_group_info_t * group_info,
                                   size_t group_info_size);
    CEPH_RBD_API int rbd_set_image_notification(rbd_image_t image, int fd,
                                                int type);

/* exclusive lock feature */
    CEPH_RBD_API int rbd_is_exclusive_lock_owner(rbd_image_t image,
                                                 int *is_owner);
    CEPH_RBD_API int rbd_lock_acquire(rbd_image_t image,
    ...
    CEPH_RBD_API int rbd_lock_break(rbd_image_t image,
                                    rbd_lock_mode_t lock_mode,
...
/* object map feature */
    ...
    CEPH_RBD_API int rbd_copy4(rbd_image_t src, rados_ioctx_t dest_io_ctx,
                               ...
                               rbd_image_options_t dest_opts,
                               size_t sparse_size);
    CEPH_RBD_API int rbd_copy_with_progress(rbd_image_t image,
    ...
    CEPH_RBD_API int rbd_copy_with_progress4(rbd_image_t image,
                                             ...
                                             rbd_image_options_t dest_opts,
                                             librbd_progress_fn_t cb,
                                             void *cbdata, size_t sparse_size);
...
/* snapshots */
    ...
    CEPH_RBD_API int rbd_snap_exists(rbd_image_t image, const char *snapname,
                                     bool * exists);
 ...
 */
    CEPH_RBD_API int rbd_snap_unprotect(rbd_image_t image,
                                        const char *snap_name);
/**
 ...
 * @returns 0 on success, negative error code on failure
 */
    CEPH_RBD_API int rbd_snap_is_protected(rbd_image_t image,
                                           const char *snap_name,
                                           int *is_protected);
/**
 ...
 * @returns 0 on success, negative error code on failure
 */
    CEPH_RBD_API int rbd_snap_get_timestamp(rbd_image_t image, uint64_t snap_id,
                                            struct timespec *timestamp);
    ...
    CEPH_RBD_API int rbd_snap_get_name(rbd_image_t image, uint64_t snap_id,
                                       char *snapname, size_t * name_len);
    CEPH_RBD_API int rbd_snap_get_id(rbd_image_t image, const char *snapname,
                                     uint64_t * snap_id);
    ...
    CEPH_RBD_API int rbd_snap_group_namespace_cleanup(rbd_snap_group_namespace_t
                                                      * group_snap,
                                                      size_t group_snap_size);
    CEPH_RBD_API int rbd_snap_get_trash_namespace(rbd_image_t image,
                                                  ...
                                                  char *original_name,
                                                  size_t max_length);
    CEPH_RBD_API int rbd_snap_get_mirror_namespace(rbd_image_t image,
                                                   uint64_t snap_id,
                                                   rbd_snap_mirror_namespace_t *
                                                   mirror_snap,
                                                   size_t mirror_snap_size);
    CEPH_RBD_API int
        rbd_snap_mirror_namespace_cleanup(rbd_snap_mirror_namespace_t *
                                          mirror_snap, size_t mirror_snap_size);
...
/* DEPRECATED; use rbd_read_iterate2 */
    CEPH_RBD_API int64_t rbd_read_iterate(rbd_image_t image, uint64_t ofs,
                                          size_t len, int (*cb) (uint64_t,
                                                                 size_t,
                                                                 const char *,
                                                                 void *),
                                          void *arg);
 ...
 */
    CEPH_RBD_API int rbd_read_iterate2(rbd_image_t image, uint64_t ofs,
                                       uint64_t len, int (*cb) (uint64_t,
                                                                size_t,
                                                                const char *,
                                                                void *),
                                       void *arg);
 ...
 */
    ...
    CEPH_RBD_API ssize_t rbd_writesame(rbd_image_t image, uint64_t ofs,
 ...
 */
    ...
    CEPH_RBD_API int rbd_aio_readv(rbd_image_t image, const struct iovec *iov,
                                   int iovcnt, uint64_t off,
                                   rbd_completion_t c);
    CEPH_RBD_API int rbd_aio_discard(rbd_image_t image, uint64_t off,
                                     uint64_t len, rbd_completion_t c);
    CEPH_RBD_API int rbd_aio_writesame(rbd_image_t image, uint64_t off,
                                       size_t len, const char *buf,
                                       size_t data_len, rbd_completion_t c,
                                       int op_flags);
 ...
 */
    ...
    CEPH_RBD_API int rbd_poll_io_events(rbd_image_t image,
                                        rbd_completion_t * comps, int numcomp);

    CEPH_RBD_API int rbd_metadata_get(rbd_image_t image, const char *key,
                                      char *value, size_t * val_len);
    CEPH_RBD_API int rbd_metadata_set(rbd_image_t image, const char *key,
                                      const char *value);
 ...
 * @returns -ERANGE if either buffer is too short
 */
    CEPH_RBD_API int rbd_metadata_list(rbd_image_t image, const char *start,
...
// RBD image mirroring support functions
    ...
    CEPH_RBD_API int rbd_mirror_image_get_info(rbd_image_t image,
                                               ...
                                               mirror_image_info,
                                               size_t info_size);
    CEPH_RBD_API void rbd_mirror_image_get_info_cleanup(rbd_mirror_image_info_t
                                                        * mirror_image_info);
...
// RBD groups support functions
    ...
    CEPH_RBD_API int rbd_group_list(rados_ioctx_t p, char *names,
                                    size_t * size);
 ...
 */
    ...
    CEPH_RBD_API int rbd_config_image_list(rbd_image_t image,
                                           rbd_config_option_t * options,
                                           int *max_options);
    CEPH_RBD_API void rbd_config_image_list_cleanup(rbd_config_option_t *
                                                    options, int max_options);
    ...
    CEPH_RBD_API int rbd_group_image_list(rados_ioctx_t group_p,
                                          ...
                                          size_t * num_entries);
    CEPH_RBD_API int rbd_group_image_list_cleanup(rbd_group_image_info_t *
                                                  images,
                                                  size_t group_image_info_size,
                                                  size_t num_entries);
    ...
    CEPH_RBD_API int rbd_group_snap_rollback(rados_ioctx_t group_p,
                                             const char *group_name,
                                             const char *snap_name);
    CEPH_RBD_API int rbd_group_snap_rollback_with_progress(rados_ioctx_t
                                                           group_p,
                                                           const char
                                                           *group_name,
                                                           const char
    ...
    CEPH_RBD_API int rbd_pool_stats_option_add_uint64(rbd_pool_stats_t stats,
                                                      int stat_option,
                                                      uint64_t * stat_val);
    CEPH_RBD_API int rbd_pool_stats_get(rados_ioctx_t io,
