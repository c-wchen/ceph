    ...
    map < uint64_t, float > & desired_primary_distribution) const
{
    ...
    if (!pool->is_replicated()) {   // read balancing works only for replicated pools
        ...
        ldout(cct, 30) << __func__ << " Pool: " << pid << " read ratio: " << read_ratio << " write ratio: " << write_ratio <<
                       dendl;
    ...
    float max_osd_score) const
{
    ...
    if (rbi.pa_avg != 0.) {
        ...
        if ((zpg = has_zero_pa_pgs(cct, pool_id)) >= 0) {
            ...
            std::stringstream ss;
            ss << pg;
            ldout(cct, 10) << __func__ << " pool " << pool_id << " has some PGs where all OSDs are with primary_affinity 0 (" << pg
                           << ",...)" << dendl;
            rbi.err_msg = fmt::format(
                                  ...
                                  const pg_pool_t *pgpool, read_balance_info_t &rbi) const
{
    ...
    if (num_pg_osd_legs != num_pgs * pgpool->get_size()) {
        ...
        rbi.err_msg =
            fmt::format("ERROR: pool {} has {} PG OSD legs, expected {} - Can't calculate size-optimal read balancer score",
