...
protected:
    ...
    virtual int fetch_stats_from_storage(const rgw_owner& owner, const rgw_bucket& bucket, RGWStorageStats& stats,
    ...
    virtual bool map_find_and_update(const rgw_owner& owner, const rgw_bucket& bucket, typename lru_map < T,
                                     RGWQuotaCacheStats >::UpdateContext *ctx) = 0;
...
public:
    ...
    void adjust_stats(const rgw_owner& owner, rgw_bucket& bucket, int objs_delta, uint64_t added_bytes,
                      uint64_t removed_bytes);
...
template < class T >
void RGWQuotaCache < T >::async_refresh_response(const rgw_owner& owner, rgw_bucket& bucket,
...
template < class T >
void RGWQuotaCache < T >::set_stats(const rgw_owner& owner, const rgw_bucket& bucket, RGWQuotaCacheStats& qs,
                                    const RGWStorageStats& stats)
{
    qs.stats = stats;
...
template < class T >
int RGWQuotaCache < T >::get_stats(const rgw_owner& owner, const rgw_bucket& bucket, RGWStorageStats& stats,
...
protected:
    ...
    bool map_find_and_update(const rgw_owner& owner, const rgw_bucket& bucket, lru_map < rgw_bucket,
                             RGWQuotaCacheStats >::UpdateContext *ctx) override
    {
    ...
    int fetch_stats_from_storage(const rgw_owner& owner, const rgw_bucket& bucket, RGWStorageStats& stats, optional_yield y,
...
int RGWBucketStatsCache::fetch_stats_from_storage(const rgw_owner& owner, const rgw_bucket& _b, RGWStorageStats& stats,
        optional_yield y, const DoutPrefixProvider *dpp)
{
...
protected:
    ...
    bool map_find_and_update(const rgw_owner& owner, const rgw_bucket& bucket, lru_map < rgw_owner,
                             RGWQuotaCacheStats >::UpdateContext *ctx) override
    {
...
public:
    ...
    void update_stats(const rgw_owner& owner, rgw_bucket& bucket, int obj_delta, uint64_t added_bytes,
