   ...
   template < typename ImageCtxT = ImageCtx > class ImageWatcher:public Watcher
    {
      public:
        ...
        void notify_rebuild_object_map(uint64_t request_id,
                                       ProgressContext & prog_ctx,
                                       Context * on_finish);
        void notify_rename(uint64_t request_id, const std::string & image_name,
                           Context * on_finish);
      ...
      private:
        ...
        class RemoteProgressContext:public ProgressContext {
          public:
            ...
            :m_image_watcher(image_watcher), m_async_request_id(id) {
                ...
                m_image_watcher.schedule_async_progress(m_async_request_id,
                                                        offset, total);
                return 0;
            }
        ...
        std::map < watch_notify::AsyncRequestId,
            AsyncRequest > m_async_requests;
        ...
        void notify_lock_owner(watch_notify::Payload * payload,
                               Context * on_finish);

        bool is_new_request(const watch_notify::AsyncRequestId & id) const;
        bool mark_async_request_complete(const watch_notify::
                                         AsyncRequestId & id, int r);
        Context *remove_async_request(const watch_notify::AsyncRequestId & id);
        Context *remove_async_request(const watch_notify::AsyncRequestId & id,
                                      ceph::shared_mutex & lock);
        void schedule_async_request_timed_out(const watch_notify::
                                              AsyncRequestId & id);
        ...
        int notify_async_progress(const watch_notify::AsyncRequestId & id,
                                  uint64_t offset, uint64_t total);
        void schedule_async_complete(const watch_notify::AsyncRequestId & id,
                                     int r);
        void notify_async_complete(const watch_notify::AsyncRequestId & id,
                                   int r);
        void handle_async_complete(const watch_notify::AsyncRequestId & request,
                                   int r, int ret_val);
        ...
        Context *prepare_quiesce_request(const watch_notify::
                                         AsyncRequestId & request,
                                         C_NotifyAck * ack_ctx);
        void prepare_unquiesce_request(const watch_notify::
                                       AsyncRequestId & request);
        void cancel_quiesce_requests();

        void notify_quiesce(const watch_notify::
                            AsyncRequestId & async_request_id, size_t attempts,
                            ProgressContext & prog_ctx, Context * on_finish);

        bool handle_operation_request(const watch_notify::
                                      AsyncRequestId & async_request_id,
                                      exclusive_lock::
                                      OperationRequestType request_type,
                                      Operation operation,
                                      std::function <
                                      void (ProgressContext & prog_ctx,
                                            Context *) > execute,
                                      C_NotifyAck * ack_ctx);
        ...
        bool handle_payload(const watch_notify::SnapUnprotectPayload & payload,
                            C_NotifyAck * ctx);
        bool handle_payload(const watch_notify::
        ...
        void send_notify(watch_notify::Payload * payload, Context * ctx =
                         nullptr);
