...
BlueStore::ExtentMap::debug_au_vector_t BlueStore::ExtentMap::debug_list_disk_layout()
{
...
int BlueStore::_init_alloc(std::map < uint64_t, uint64_t > *zone_adjustments)
{
    ...
    if (!fm->is_null_manager()) {
        ...
        if (restore_allocator(alloc, &num, &bytes) == 0) {
            ...
            dout(0) << __func__ << "::NCB::restore_allocator() failed! Run Full Recovery from ONodes (might take a while) ..." <<
                    dendl;
...
int BlueStore::expand_devices(ostream& out)
{
    ...
    for (auto devid : {
             BlueFS::BDEV_WAL, BlueFS::BDEV_DB
         }) {
...
struct allocator_image_trailer {
    ...
    int verify(CephContext* cct, const std::string &path, const allocator_image_header *p_header, uint64_t entries_count,
               uint64_t allocation_size)
    {
        if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
...
int BlueStore::store_allocator(Allocator* src_allocator)
{
    ...
    utime_t duration = ceph_clock_now() - start_time;
    dout(5) << "WRITE-extent_count=" << extent_count << ", allocation_size=" << allocation_size << ", serial=" << s_serial
            << dendl;
    ...
    while (extents_bytes_left) {
        ...
        if (read_bytes == sizeof(crc)) {
            ...
            if (crc != calc_crc) {
                ...
                derr << "extents_bytes_left=" << extents_bytes_left << ", offset=" << offset << ", extent_count=" << extent_count <<
                     dendl;
                return -1;
            }
