    ...
    case FN_ALL:
        {
            ...
            while (true) {
                ...
                int r =
                    fs->store->collection_list(cid, next, last, 1000, &ls,
                                               &next);
                if (r < 0)
                    return r;
              for (auto p:ls) {
    ...
    case FN_OBJECT_OMAP_HEADER:
        {
            int r = fs->open_file(path, fi,
                                  [&](bufferlist * pbl) {
                                  return fs->store->omap_get_header(cid, oid,
                                                                    pbl);}
            );
            if (r < 0)
                return r;
...
static int os_mkdir(const char *path, mode_t mode)
{
    ...
    if (!t.empty()) {
        ceph::shared_ptr < ObjectStore::Sequencer >
            osr(new ObjectStore::Sequencer("fuse"));
...
static int os_release(const char *path, struct fuse_file_info *fi)
{
    ...
    if (--o->ref == 0) {
        ldout(fs->store->cct,
              10) << __func__ << " closing last " << o->path << dendl;
...
static int os_truncate(const char *path, off_t size)
{
    ...
    FuseStore *fs = static_cast < FuseStore * >(fc->private_data);
    ldout(fs->store->cct,
          10) << __func__ << " " << path << " size " << size << dendl;
    coll_t cid;
    ghobject_t oid;
    string key;
    uint32_t hash_value, hash_bits;
