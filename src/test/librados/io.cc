...
TEST_F(LibRadosIo, TooBig)
{
    ...
    ASSERT_EQ(-E2BIG,
              rados_writesame(ioctx, "A", buf, sizeof(buf), UINT_MAX, 0));
    IoCtx ioctx;
    bufferlist bl;
...
TEST_F(LibRadosIo, RoundTrip)
{
    ...
    ASSERT_EQ((int)sizeof(buf2),
              rados_read(ioctx, "foo", buf2, sizeof(buf2), 0));
    ...
    memset(buf2, 0, sizeof(buf2));
    ASSERT_EQ((int)sizeof(buf2),
              rados_read(ioctx, "bar", buf2, sizeof(buf2), off));
    ASSERT_EQ(0, memcmp(buf, buf2, sizeof(buf)));
}
...
TEST_F(LibRadosIo, OverlappingWriteRoundTrip)
{
    ...
    ASSERT_EQ((int)sizeof(buf3),
              rados_read(ioctx, "foo", buf3, sizeof(buf3), 0));
...
TEST_F(LibRadosIo, XattrsRoundTrip)
{
    ...
    ASSERT_EQ(0,
              rados_setxattr(ioctx, "foo", attr1, attr1_buf,
                             sizeof(attr1_buf)));
...
TEST_F(LibRadosIo, RmXattr)
{
    ...
    ASSERT_EQ(0,
              rados_setxattr(ioctx, "foo_rmxattr", attr2, attr2_buf,
                             sizeof(attr2_buf)));
...
TEST_F(LibRadosIo, XattrIter)
{
    ...
    ASSERT_EQ(0,
              rados_setxattr(ioctx, "foo", attr1, attr1_buf,
                             sizeof(attr1_buf)));
    ASSERT_EQ(0,
              rados_setxattr(ioctx, "foo", attr2, attr2_buf,
                             sizeof(attr2_buf)));
...
TEST_F(LibRadosIoEC, AppendRoundTrip)
{
    ...
    ASSERT_EQ((int)alignment * 2,
              rados_read(ioctx, "foo", buf3, alignment * 2, 0));
