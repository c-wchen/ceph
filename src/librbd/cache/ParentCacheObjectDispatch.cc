...
namespace librbd {
    namespace cache {
        ...
        template < typename I >
            ...
            I >::ParentCacheObjectDispatch(I * image_ctx,
                                           plugin::Api < I > &plugin_api)
        :m_image_ctx(image_ctx), m_plugin_api(plugin_api),
            m_lock(ceph::
                   make_mutex("librbd::cache::ParentCacheObjectDispatch::lock",
                              true, false)) {
            ceph_assert(m_image_ctx->data_ctx.is_valid());
            auto controller_path =
                image_ctx->cct->_conf.template get_val < std::string >
                ("immutable_object_cache_sock");
            m_cache_client =
                new CacheClient(controller_path.c_str(), m_image_ctx->cct);
        } template < typename I > ParentCacheObjectDispatch <
            I >::~ParentCacheObjectDispatch() {
        ...
        template < typename I >
            bool ParentCacheObjectDispatch < I >::read(uint64_t object_no,
                                                       io::ReadExtents *
                                                       extents,
            ...
            if (!m_cache_client->is_session_work()) {
                ...
                ldout(cct, 5) << "Parent cache try to re-connect to RO daemon. "
                    << "dispatch current request to lower object layer" <<
                    dendl;
                return false;
            }
            ...
            CacheGenContextURef ctx =
                ...
                ([this, extents, dispatch_result, on_dispatched, object_no,
                  io_context, read_flags, &parent_trace]
                  (ObjectCacheRequest * ack) {
                 handle_read_cache(ack, object_no, extents, io_context,
                                   read_flags, parent_trace, dispatch_result,
                                   on_dispatched);});
            ...
            m_cache_client->lookup_object(m_image_ctx->data_ctx.get_namespace(),
                                          ...
                                          m_image_ctx->layout.object_size, oid,
                                          std::move(ctx));
            return true;
        }

        template < typename I >
            void ParentCacheObjectDispatch <
            I >::handle_read_cache(ObjectCacheRequest * ack, uint64_t object_no,
            ...
            std::string file_path =
                ((ObjectCacheReadReplyData *) ack)->cache_path;
            if (file_path.empty()) {
                ...
                auto ctx = new LambdaContext([this, dispatch_result,
                                              on_dispatched] (int r){
                                             if (r < 0 && r != -ENOENT) {
                                                                         lderr
                                                                         (m_image_ctx->
                                             ...
                                             m_plugin_api.
                                             read_parent(m_image_ctx, object_no,
                                                         extents,
                                                         io_context->
                                                         read_snap().
                                                         value_or(CEPH_NOSNAP),
          ...
          int read_len = 0; for (auto & extent:*extents)
                                             {
                                             // try to read from parent image cache
                                             int r =
                                             read_object(file_path, &extent.bl,
                                                         extent.offset,
                                                         extent.length,
                                                         on_dispatched);
                                             if (r < 0) {
                  ...
                  for (auto & read_extent:  *extents)
                                             ...
                                             *dispatch_result =
                                             ...
                                             if (!reg) {
                                             lderr(cct) <<
                                             "Parent cache register fails." <<
                                             dendl;}
                                             return 0;}

                                             template < typename I >
                                             void ParentCacheObjectDispatch <
                                             I >::create_cache_session(Context *
                                                                       on_finish,
                                                                       bool
                                                                       is_reconnect)
                                             {
                                             ...
                                             (ceph_mutex_is_locked_by_me
                                              (m_lock)); if (m_connecting) {
                                             return;}
                                             m_connecting = true;
                                             auto cct = m_image_ctx->cct;
                                             ldout(cct, 20) << dendl;
                                             Context * register_ctx =
                                             new
                                             LambdaContext([this, cct,
                                                            on_finish] (int ret)
                                                           {
                                                           if (ret < 0) {
                                                           lderr(cct) <<
                                                           "Parent cache fail to register client."
                                                           << dendl;}
                                             ...
                                             LambdaContext([this, cct,
                                                           ...
                                                           if (ret < 0) {
                                                           lderr(cct) <<
                                                           "Parent cache fail to connect RO daemon."
                                                           << dendl;
                                                           register_ctx->
                                                           complete(ret);
                                                           return;}
                                                           ...
                                                           ldout(cct,
                                                                 20) <<
                                                           "Parent cache connected to RO daemon."
                                                           << dendl;
                                                           m_cache_client->
                                                           register_client
                                                           (register_ctx);}
                                             ...
                                             std::string error;
                                             int ret =
                                             read_data->pread_file(file_path.
                                                                   c_str(),
                                                                   offset,
                                                                   length,
                                                                   &error);
