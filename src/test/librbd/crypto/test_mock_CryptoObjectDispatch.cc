...
template bool librbd::io::util::trigger_copyup(MockImageCtx *image_ctx,
        uint64_t object_no,
...
template <> struct CopyupRequest <librbd::MockImageCtx > {
    ...
    MOCK_METHOD2(append_request,
                 void (AbstractObjectWriteRequest <
                       librbd::MockImageCtx > *, const Extents &));
...
struct Mock {
    ...
    Mock()
    {
        s_instance = this;
                   ...
                   void (MockImageCtx *, uint64_t,
                         io::ReadExtents *, librados::snap_t,
...
template <> void read_parent(MockImageCtx *image_ctx,
                             uint64_t object_no,
...
template <>
uint64_t get_file_offset(MockImageCtx *image_ctx,
...
struct TestMockCryptoCryptoObjectDispatch: public TestMockFixture {
    ...
    } void expect_object_read(io::ReadExtents *extents,
                              uint64_t version = 0)
    {
        ...
        version](io::ObjectDispatchSpec * spec) {
            ...
            ASSERT_EQ(extents->size(),
                      read->extents->size());
            for (uint64_t i = 0; i < extents->size();
                 ++i) {
                ASSERT_EQ((*extents)[i].offset,
                          (*read->extents)[i].offset);
                ASSERT_EQ((*extents)[i].length,
                          (*read->extents)[i].length);
                (*read->extents)[i].bl = (*extents)[i].bl;
                (*read->extents)[i].extent_map =
                    (*extents)[i].extent_map;
            }
    ...
    void expect_object_write(uint64_t object_off,
                             const std::string &data, int write_flags,
                             std::optional < uint64_t >
                             assert_version)
    {
        EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
        .WillOnce(Invoke([this, object_off, data, write_flags,
                          assert_version](io::ObjectDispatchSpec *
        spec) {
            auto *write =
                boost::get <
    ...
    void expect_get_object_size()
    {
        EXPECT_CALL(*mock_image_ctx,
                    get_object_size()).WillOnce(Return(mock_image_ctx->
                                                layout.
                                                object_size));
    }
    ...
    void expect_copyup(MockAbstractObjectWriteRequest **write_request,
                       int r)
    {
...
{
    ASSERT_FALSE(mock_crypto_object_dispatch->
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedRead)
{
    ...
    extents[0].bl.append(std::string(1024, '1') +
                         std::string(1024, '2') + std::string(1024,
                                 '3') +
                         std::string(1024, '4'));
    ...
    auto expected_bl_data =
        (std::string(1024, '\0') + std::string(1024, '1') +
         std::string(1024, '\0') + std::string(1024,
             '2') + std::string(1024,
                                '3') +
         std::string(3072, '\0') + std::string(3072,
             '\0') + std::string(1024,
                                 '4'));
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedRead)
{
    ...
    aligned_extents[0].bl.append(std::string("1") +
                                 std::string(4096, '0'));
    aligned_extents[1].bl.append(std::string(4095, '0') +
                                 std::string("2"));
    aligned_extents[2].bl.append(std::string("03") +
                                 std::string(4094, '0'));
    aligned_extents[3].bl.append(std::string("0") +
                                 std::string(4095,
                                         '4') + std::string(4096,
                                                 '5') +
                                 std::string(4095,
    ...
    ASSERT_THAT(extents[3].extent_map,
                ElementsAre(Pair(16384 + 1, 4095),
                            Pair(16384 + 2 * 4096, 4096),
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedWrite)
{
    expect_encrypt();
    ASSERT_TRUE(mock_crypto_object_dispatch->
                write(11, 0, std::move(data),
                      mock_image_ctx->get_data_io_context(), 0, 0,
    std::nullopt, {
    }, nullptr, nullptr, &dispatch_result, &on_finish,
    on_dispatched));
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWrite)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->
                write(11, 1, std::move(write_data),
                      mock_image_ctx->get_data_io_context(), 0, 0,
    std::nullopt, {
    }, nullptr, nullptr, &dispatch_result, &on_finish,
    on_dispatched));
    ...
    auto expected_data =
        std::string("2") + std::string(8192, '1') + std::string(4095,
            '3');
    expect_object_write(0, expected_data, 0,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteWithNoObject)
{
    ...
    auto expected_data =
        (std::string(1, '\0') + std::string(8192, '1') +
         std::string(4095, '\0'));
    expect_object_write(0, expected_data,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteFailCreate)
{
    ...
    auto expected_data2 =
        std::string("2") + std::string(8192, '1') + std::string(4095,
            '3');
    expect_object_write(0, expected_data2, 0,
                        std::make_optional(version));
    dispatcher_ctx->complete(8192); // complete read
    ASSERT_EQ(ETIMEDOUT, dispatched_cond.wait_for(0));
    dispatcher_ctx->complete(0);    // complete write
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteCopyup)
{
    ...
    EXPECT_CALL(*mock_image_ctx->object_map,
                object_may_exist(11)).WillOnce(Return(false));
...
TEST_F(TestMockCryptoCryptoObjectDispatch,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, CompareAndWrite)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->
                compare_and_write(11, 1, std::move(cmp_data),
                                  std::move(write_data),
                                  mock_image_ctx->get_data_io_context(),
    0, {
    }, nullptr, nullptr, nullptr,
    &dispatch_result, &on_finish,
    on_dispatched));
    ASSERT_EQ(dispatch_result, io::DISPATCH_RESULT_COMPLETE);
    ASSERT_EQ(on_finish, &finished_cond);
...
TEST_F(TestMockCryptoCryptoObjectDispatch, CompareAndWriteFail)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->
                compare_and_write(11, 1, std::move(cmp_data),
                                  ...
                                  mock_image_ctx->get_data_io_context(),
    0, {
    }, &mismatch_offset, nullptr, nullptr,
    &dispatch_result, &on_finish,
    on_dispatched));
    ASSERT_EQ(dispatch_result, io::DISPATCH_RESULT_COMPLETE);
    ASSERT_EQ(on_finish, &finished_cond);
...
TEST_F(TestMockCryptoCryptoObjectDispatch, WriteSame)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->write_same(11, 0, 5, { {
            0, 5
        }
    },
    std::
...
TEST_F(TestMockCryptoCryptoObjectDispatch, PrepareCopyup)
{
    ...
    snap2.insert(0, 2, {
        io::SPARSE_EXTENT_STATE_ZEROED, 2
    });
    snap2.insert(8191, 3, {
        io::SPARSE_EXTENT_STATE_DATA, 3,
        ceph::bufferlist::static_from_mem(data + 6, 3)
    });
    snap2.insert(16384, 1, {
        io::SPARSE_EXTENT_STATE_DATA, 1,
    ...
    ASSERT_EQ(0,
              mock_crypto_object_dispatch->prepare_copyup(11,
