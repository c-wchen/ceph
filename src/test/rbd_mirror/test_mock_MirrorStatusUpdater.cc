...
namespace rbd {
    namespace mirror {
        ...
        MirrorStatusWatcher < librbd::MockTestImageCtx > *MirrorStatusWatcher <
    ...
    namespace mirror {
        ...
        class TestMockMirrorStatusUpdater:public TestMockFixture {
          public:
            ...
            typedef MirrorStatusWatcher < librbd::MockTestImageCtx >
            ...
            void SetUp() override {
                ...
                EXPECT_CALL(*m_mock_threads->timer, add_event_after(_, _))
                    .WillOnce(WithArg < 1 >
                              (Invoke([timer_event] (Context * ctx) {
                                      *timer_event = ctx; return ctx;}
                               )));
            }
            ...
            void expect_mirror_status_watcher_init(MockMirrorStatusWatcher &
            ...
            void expect_mirror_status_watcher_shut_down(MockMirrorStatusWatcher
                                                        &
                                                        mock_mirror_status_watcher,
                                                        int r) {
                EXPECT_CALL(*mock_mirror_status_watcher.s_instance,
                            shut_down(_))
                    .WillOnce(Invoke([this, r] (Context * ctx) {
                                     m_threads->work_queue->queue(ctx, r);}));
            }

            void expect_mirror_status_update(const std::
                                             string & global_image_id,
                                             const cls::rbd::
                                             MirrorImageSiteStatus &
                                             mirror_image_status, int r) {
                    ...
                    .WillOnce(WithArg < 4 >
                              (Invoke
                               ([r, global_image_id,
                                ...
                                EXPECT_EQ(global_image_id,
                                          decode_global_image_id);
                                cls::rbd::
                                MirrorImageSiteStatus
                                decode_mirror_image_status;
                                decode(decode_mirror_image_status, bl_it);
                                EXPECT_EQ(mirror_image_status,
                                          decode_mirror_image_status);
                                return r;}
                               )));
            }

            void expect_mirror_status_update(const MirrorImageSiteStatuses &
                ...
                EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
                    .WillOnce(Invoke([this] (auto && ... args) {
                                     int r =
                                     m_mock_local_io_ctx->
                                     do_aio_operate(decltype(args) (args) ...);
...
mirror_image_site_statuses) {
                    ...
                    expect_mirror_status_update(global_image_id,
                                                mirror_image_status, r);
                    if (r < 0) {
                        break;
                    }
                }
            ...
            void expect_mirror_status_remove(const std::
                                             string & global_image_id, int r) {
                EXPECT_CALL(*m_mock_local_io_ctx,
                            exec(RBD_MIRRORING, _, StrEq("rbd"),
                    ...
                    .WillOnce(WithArg < 4 >
                              (Invoke
                               ([r, global_image_id] (bufferlist & in_bl) {
                                ...
                                decode(decode_global_image_id, bl_it);
                                EXPECT_EQ(global_image_id,
                                          decode_global_image_id); return r;}
                               )));
            }

            void expect_mirror_status_removes(const std::set < std::string >
                                              &mirror_images, int r) {
                EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
                    .WillOnce(Invoke([this] (auto && ... args) {
                                     int r =
            ...
            void fire_timer_event(Context ** timer_event,
                ...
                EXPECT_CALL(*m_mock_threads->work_queue, queue(_, _))
                    .WillOnce(WithArg < 0 >
                              (Invoke([update_task] (Context * ctx) mutable {
                                      *update_task = ctx;})));

                // fire the timer task
                {
                    std::lock_guard timer_locker {
                    m_mock_threads->timer_lock};
                    ceph_assert(*timer_event != nullptr);
                    (*timer_event)->complete(0);
                }
            }

            void init_mirror_status_updater(MockMirrorStatusUpdater &
                                            mock_mirror_status_updater,
                                            MockMirrorStatusWatcher &
                                            mock_mirror_status_watcher,
                                            Context ** timer_event) {
                expect_timer_add_event(timer_event);
                expect_mirror_status_watcher_init(mock_mirror_status_watcher,
                                                  0);
            ...
            void shut_down_mirror_status_updater(MockMirrorStatusUpdater &
                                                 mock_mirror_status_updater,
                                                 MockMirrorStatusWatcher &
                                                 mock_mirror_status_watcher) {
                expect_timer_cancel_event();
                expect_mirror_status_watcher_shut_down
                    (mock_mirror_status_watcher, 0);
                expect_work_queue(true);
        ...
        TEST_F(TestMockMirrorStatusUpdater, SmallBatch) {
            ...
            for (auto i = 0; i < 100; ++i) {
                auto pair =
                    mirror_image_site_statuses.emplace(stringify(i),
                                                       cls::rbd::
                                                       MirrorImageSiteStatus { }
                );
                mock_mirror_status_updater.set_mirror_image_status(pair.first->
                                                                   first,
                                                                   pair.first->
                                                                   second,
                                                                   false);
        ...
        TEST_F(TestMockMirrorStatusUpdater, LargeBatch) {
            ...
            MirrorImageSiteStatuses mirror_image_site_statuses_1 {
            it_1, it_2};

            it_1 = it_2;
            std::advance(it_2, 100);
            MirrorImageSiteStatuses mirror_image_site_statuses_2 {
            it_1, it_2};
        ...
        TEST_F(TestMockMirrorStatusUpdater, OverwriteStatus) {
            ...
            mock_mirror_status_updater.set_mirror_image_status("1", {
                                                               }
                                                               , false);
            mock_mirror_status_updater.set_mirror_image_status("1", {
                                                               "",
        ...
        TEST_F(TestMockMirrorStatusUpdater, RemoveStatus) {
            ...
            mock_mirror_status_updater.remove_mirror_image_status("1", false,
                                                                  &ctx);
            ASSERT_EQ(0, ctx.wait());
            ...
            EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
                .WillOnce(Invoke([this, &remove_flush_ctx] (auto && ... args) {
                                 ...
                                 do_aio_operate(decltype(args) (args) ...);
                                 m_mock_local_io_ctx->aio_flush();
                                 remove_flush_ctx.complete(r); return r;}
                          ));
        ...
        TEST_F(TestMockMirrorStatusUpdater, OverwriteStatusInFlight) {
            ...
            EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
                .
                WillOnce(Invoke
                         ([this, &mock_mirror_status_updater] (auto
                                                               && ... args) {
                          mock_mirror_status_updater.
                          set_mirror_image_status("1", {
                                                  "",
                                                  cls::rbd::
                                                  MIRROR_IMAGE_STATUS_STATE_REPLAYING,
                                                  "description"}
                          ...
                          m_mock_local_io_ctx->
                          do_aio_operate(decltype(args) (args) ...);
                          m_mock_local_io_ctx->aio_flush(); return r;}
                         ));
        ...
        TEST_F(TestMockMirrorStatusUpdater, RemoveImmediateUpdate) {
            ...
            C_SaferCond ctx;
            expect_work_queue(false);
            expect_mirror_status_removes( {
                                         "1"}
                                         , 0);
            expect_work_queue(false);
            mock_mirror_status_updater.remove_mirror_image_status("1", true,
                                                                  &ctx);
            ASSERT_EQ(0, ctx.wait());
        ...
        TEST_F(TestMockMirrorStatusUpdater, RemoveRefreshIdleStatus) {
            ...
            C_SaferCond ctx;
            expect_work_queue(true);
            mock_mirror_status_updater.remove_refresh_mirror_image_status("1",
                                                                          &ctx);
            ASSERT_EQ(0, ctx.wait());
        ...
        TEST_F(TestMockMirrorStatusUpdater, RemoveRefreshInFlightStatus) {
            ...
            EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
                .WillOnce(Invoke([this, &mock_mirror_status_updater,
                                 ...
                                 mock_mirror_status_updater.
                                 remove_refresh_mirror_image_status("1",
                                                                    &on_removed);
                                 int r =
                                 m_mock_local_io_ctx->
                                 do_aio_operate(decltype(args) (args) ...);
        ...
        TEST_F(TestMockMirrorStatusUpdater, MirrorPeerSitePing) {
            ...
            expect_mirror_status_update(mirror_image_site_statuses,
                                        "mirror uuid", 0);
            update_task->complete(0);
