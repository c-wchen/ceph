...
protected:
    uint64_t get_actual_size() const
    {
        return data_offset;
    }
...
public:
    ...
    {
    } void set_head_chunk_size(uint64_t size)
    {
        head_chunk_size = size;
    }
    // cache first chunk for process_first_chunk(), then forward everything else// to the returned processor
    int process(bufferlist
                && data, uint64_t logical_offset) final override;
};
...
public:
    ManifestObjectProcessor(Aio *aio, RGWRados *store,
                            ...
                            const DoutPrefixProvider *dpp,
                            optional_yield y)
        : HeadObjectProcessor(0), store(store), bucket_info(bucket_info),
          owner(owner), obj_ctx(_obj_ctx), head_obj(_head_obj),
...
class AtomicObjectProcessor: public ManifestObjectProcessor
{
    ...
    int process_first_chunk(bufferlist
                            && data,
                            rgw::sal::DataProcessor **
                            processor) override;
public:
    AtomicObjectProcessor(Aio *aio, RGWRados *store,
                          RGWBucketInfo &bucket_info,
                          const rgw_placement_rule *
                          ptail_placement_rule, const rgw_user &owner,
                          RGWObjectCtx &obj_ctx,
                          const rgw_obj &_head_obj,
                          ...
                          const DoutPrefixProvider *dpp,
                          optional_yield y)
        : ManifestObjectProcessor(aio, store, bucket_info,
                                  ptail_placement_rule, owner, obj_ctx,
                                  _head_obj, dpp, y), olh_epoch(olh_epoch),
...
public:
    MultipartObjectProcessor(Aio *aio, RGWRados *store,
        ...
        : ManifestObjectProcessor(aio, store, bucket_info,
                                  ptail_placement_rule, owner, obj_ctx,
                                  _head_obj, dpp, y), target_obj(head_obj),
          upload_id(upload_id), part_num(part_num),
          part_num_str(part_num_str), mp(head_obj.key.name, upload_id)
    {
    }
...
public:
    AppendObjectProcessor(Aio *aio, RGWRados *store,
        ...
        : ManifestObjectProcessor(aio, store, bucket_info,
                                  ptail_placement_rule, owner, obj_ctx,
                                  _head_obj, dpp, y), position(position),
