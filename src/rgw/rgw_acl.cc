...
uint32_t RGWAccessControlList::get_perm(const rgw::auth::
...
uint32_t RGWAccessControlPolicy::get_perm(const rgw::auth::
                                          Identity & auth_identity,
                                          const uint32_t perm_mask,
                                          const char *const http_referer)
{
    ...
    if ((perm & perm_mask) != perm_mask) {
        ...
        if (false == auth_identity.is_owner_of(rgw_user(RGW_USER_ANON_ID))) {
            ...
            perm |=
                acl.get_group_perm(ACL_GROUP_AUTHENTICATED_USERS, perm_mask);
        }
    }
...
bool RGWAccessControlPolicy::verify_permission(const rgw::auth::
                                               Identity & auth_identity,
