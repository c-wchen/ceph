...
bool DataScan::parse_kwarg(const std::vector < const char *>&args,
                           ...
                           int *r)
{
    ...
    if (arg == std::string("--output-dir")) {
        if (driver != NULL) {
            derr << "Unexpected --output-dir: output already selected!" <<
                dendl;
            *r = -EINVAL;
            return false;
        }
...
bool DataScan::parse_arg(const std::vector < const char *>&args,
                         std::vector < const char *>::const_iterator & i)
{
    const std::string arg(*i);
    if (arg == "--force-pool") {
...
int DataScan::main(const std::vector < const char *>&args)
{
    ...
    if (command == "scan_inodes" ||
        command == "scan_extents" || command == "cleanup") {
        ...
        dout(4) << "data pool '" << data_pool_name << "' has ID " <<
            data_pool_id << dendl;
    ...
    if (command == "scan_extents") {
        if (autodetect_data_pools) {
          ...
          for (auto & pool_id:fs->mds_map.get_data_pools()) {
                ...
                if (r < 0) {
                    std::
                        cerr << "Failed to resolve data pool: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
      ...
      for (auto & data_pool_name:extra_data_pool_names) {
            ...
            if (data_pool_id < 0) {
                std::
                    cerr << "Data pool '" << data_pool_name << "' not found!" <<
                    std::endl;
                return -ENOENT;
            }
            else {
            ...
            if (!fs->mds_map.is_data_pool(pool_id)) {
                std::
                    cerr << "Warning: pool '" << data_pool_name << "' is not a "
            ...
            r = rados.ioctx_create(data_pool_name.c_str(),
                                   extra_data_ios.back());
            if (r != 0) {
                return r;
            }
    ...
    if (command == "scan_frags" || command == "scan_links") {
        ...
        if (fs == nullptr) {
            std::cerr << "Filesystem id " << fscid << " does not exist" << std::
                endl;
            return -ENOENT;
        }
        ...
        int r =
            rados.pool_reverse_lookup(metadata_pool_id, &metadata_pool_name);
        if (r < 0) {
...
int MetadataDriver::inject_unlinked_inode(inodeno_t inono, int mode,
...
int DataScan::scan_extents()
{
  ...
  for (auto ioctx:data_ios) {
        int r =
            forall_objects(*ioctx, false,[this, ioctx] (std::string const &oid,
                           ...
                           << "." << obj_name_offset << dendl; if (r != 0) {
                           dout(4) << "Cannot stat '" << oid << "': skipping" <<
                           dendl; return r;}
                           int64_t obj_pool_id =
                           data_io.get_id() !=
                           ioctx->get_id()? ioctx->get_id() : -1;
                           ...
                           r =
                           ClsCephFSClient::accumulate_inode_metadata(data_io,
                                                                      obj_name_ino,
                                                                      obj_name_offset,
                                                                      size,
                                                                      obj_pool_id,
...
int DataScan::probe_filter(librados::IoCtx & ioctx)
{
    ...
    int r =
        ioctx.object_list(ioctx.object_list_begin(), ioctx.object_list_end(),
                          1, filter_bl, &tmp_result, &tmp_next);

    return r >= 0;
}

int DataScan::forall_objects(librados::IoCtx & ioctx,
                             bool untagged_only,
                                                  ...
                                                  uint64_t) > handler)
{
    librados::ObjectCursor range_i;
    librados::ObjectCursor range_end;
    ioctx.object_list_slice(ioctx.object_list_begin(),
                            ioctx.object_list_end(),
                            n, m, &range_i, &range_end);
                ...
                if (r >= 0) {
                    ...
                    try {
                        ...
                        if (read_tag == filter_tag) {
                            dout(20) << "skipping " << oid <<
                                " because it has the filter_tag" << dendl;
                            continue;
                        }
                    }
                    catch(const buffer::error & err) {
                    }
                    dout(20) << "read non-matching tag '" << read_tag << "'" <<
                        dendl;
                }
                else {
...
int DataScan::scan_inodes()
{
    ...
    return forall_objects(data_io, true,[this] (std::string const &oid,
                                                uint64_t obj_name_ino,
                                                uint64_t obj_name_offset)->int {
                          int r = 0;
                          dout(10) << "handling object "
                          << std::
                          hex << obj_name_ino << "." << obj_name_offset << std::
                          dec << dendl; AccumulateResult accum_res;
                          inode_backtrace_t backtrace;
                          file_layout_t loaded_layout =
                          file_layout_t::get_default(); std::string symlink;
                          r =
                          ClsCephFSClient::
                          fetch_inode_accumulate_result(data_io, oid,
                                                        &backtrace,
                                                        &loaded_layout,
                                                        &symlink, &accum_res);
                          if (r == -EINVAL) {
                          ...
                          if (r < 0) {
                          dout(4) <<
                          "Unexpected error loading accumulated metadata from '"
                          << oid << "': " << cpp_strerror(r) << dendl;
                          ...
                          r =
                          librados::Rados(data_io).ioctx_create2(guessed_layout.
                                                                 pool_id,
                                                                 ioctx);
                          if (r != 0) {
                          derr << "Unexpected error opening file data pool id="
                          << guessed_layout.
                          pool_id << ": " << cpp_strerror(r) << dendl;
                          return r;}
                          ...
                          try {
                          auto q = bl.cbegin(); decode(loaded_layout, q);}
                          catch(ceph::buffer::error & e) {
                          derr << "Unexpected error decoding layout for " << oid
                          << dendl; return -EINVAL;}
                          }
                          }
                          ...
                          if ((accum_res.
                               max_obj_size & (accum_res.max_obj_size - 1)) ==
                              0) {
                          ...
                          if (!loaded_layout.is_valid() ||
                              loaded_layout.object_size <
                          ...
                          dout(4) << "bogus xattr layout on 0x" << std::
                          hex << obj_name_ino << std::
                          dec << ", ignoring in favour of best guess" << dendl;
                          guessed_layout.object_size = chunk_size;
                          guessed_layout.stripe_unit = chunk_size;
                          guessed_layout.stripe_count = 1;}
                          else {
                          ...
                          // pool we read from.
                          if (!fsmap->get_filesystem(fscid)->mds_map.
                              is_data_pool(guessed_layout.pool_id)) {
                          dout(20) << "overwriting layout pool_id " <<
                          data_pool_id << dendl;
                          guessed_layout.pool_id = data_pool_id;}
                          }
                          ...
                          if (accum_res.ceiling_obj_index >
                              guessed_layout.stripe_count - 1) {
                          complete_objs =
                          (accum_res.ceiling_obj_index /
                          ...
                          uint64_t partial_objs =
                          accum_res.ceiling_obj_index + 1 - complete_objs;
                          dout(10) <<
                          "calculating striped size from complete objs: " <<
                          ...
                          for (uint64_t i = complete_objs;
                               i < complete_objs + partial_objs; ++i) {
                          char buf[60];
                          snprintf(buf, sizeof(buf), "%llx.%08llx",
                                   (long long unsigned)obj_name_ino,
                                   (long long unsigned)i); uint64_t osize(0);
                          time_t omtime(0);
                          ...
                          * guessed_layout.stripe_unit + (osize - 1)
                          ...
                          incomplete_size =
                          std::max(incomplete_size, upper_size);}
                          }
                          else
                          if (r == -ENOENT) {
                          ...
                          if (r != 0 && r != -ENOENT) {
                          derr << "Unexpected error checking size of ino 0x" <<
                          std::hex << obj_name_ino << std::
                          dec << ": " << cpp_strerror(r) << dendl; return r;}
                          file_size =
                          complete_objs * guessed_layout.object_size +
                          incomplete_size;}
                          }
                          else {
                          ...
                          dout(4) << "Backtrace ino 0x" << std::
                          hex << backtrace.
                          ino << " doesn't match object name ino 0x" <<
                          obj_name_ino << std::dec << dendl;
                          have_backtrace = false;}

                          InodeStore dentry;
                          build_file_dentry(obj_name_ino, file_size, file_mtime,
                                            guessed_layout, &dentry, symlink);
                          // Inject inode to the metadata pool
                          if (have_backtrace) {
                          inode_backpointer_t root_bp =
                          *(backtrace.ancestors.rbegin());
                          ...
                          r =
                          driver->inject_lost_and_found(obj_name_ino, dentry);
                          if (r < 0) {
                          dout(4) << "Error injecting 0x" << std::
                          hex << backtrace.
                          ino << std::
                          dec << " into lost+found: " << cpp_strerror(r) <<
                          dendl; if (r == -EINVAL) {
                          dout(4) <<
                          "Use --force-corrupt to overwrite structures that "
                          ...
                          hex << backtrace.
                          ino << std::
                          dec << " with backtrace: " << cpp_strerror(r) <<
                          dendl; if (r == -EINVAL) {
                          dout(4) <<
                          "Use --force-corrupt to overwrite structures that "
                          "appear to be corrupt" << dendl;}
...
int DataScan::cleanup()
{
    ...
    return forall_objects(data_io, true,[this] (std::string const &oid,
                          ...
                          r =
                          ClsCephFSClient::
                          delete_inode_accumulate_result(data_io, oid);
                          if (r < 0) {
                          dout(4) <<
                          "Error deleting accumulated metadata from '" << oid <<
                          "': " << cpp_strerror(r) << dendl;}
                          return r;}
    ) ;
}

bool DataScan::valid_ino(inodeno_t ino) const const
{
...
int DataScan::scan_links()
{
    ...
    struct link_info_t {
        ...
        } link_info_t(inodeno_t di, frag_t df, const string & n,
                      const CInode::inode_const_ptr & i):dirino(di), frag(df),
            name(n), version(i->version), nlink(i->nlink),
    ...
    for (int step = SCAN_INOS; step <= CHECK_LINK; step++) {
        ...
        for (auto it = metadata_io.nobjects_begin(); it != it_end; ++it) {
            ...
            if (!valid_ino(dir_ino)) {
                dout(10) << "Not a dirfrag (invalid ino): '" << oid << "'" <<
                    dendl;
                continue;
            }
            ...
            if (r < 0) {
                derr << "Error getting omap from '" << oid << "': " <<
                    cpp_strerror(r) << dendl;
                return r;
            }
          ...
          for (auto & p:items) {
                ...
                try {
                    ...
                    if (dentry_type == 'I' || dentry_type == 'i') {
                        ...
                        if (step == SCAN_INOS) {
                            ...
                            if (inode.snap_blob.length()) {
                                ...
                                if (!srnode.past_parents.empty()) {
                                    snapid_t last =
                                        srnode.past_parents.rbegin()->first;
                            ...
                            if (q != dup_primaries.end()) {
                                q->second.
                                    push_back(link_info_t
                                              (dir_ino, frag_id, dname,
                                               inode.inode));
                                ...
                                if (inode.inode->nlink != nlink) {
                                    derr << "Bad nlink on " << ino <<
                                        " expected " << nlink << " has " <<
                                        inode.inode->nlink << dendl;
                                    bad_nlink_inos[ino] =
                                        link_info_t(dir_ino, frag_id, dname,
                                                    inode.inode);
                            ...
                            if (dnfirst == CEPH_NOSNAP) {
                                injected_inos[ino] =
                                    link_info_t(dir_ino, frag_id, dname,
                                                inode.inode);
                                dout(20) << "adding " << ino <<
                                    " for future processing to fix dnfirst" <<
                                    dendl;
                            }
                        }
                    }
                        ...
                        CDentry::decode_remote(dentry_type, ino, d_type,
                                               alternate_name, q);
                        ...
                        if (step == SCAN_INOS) {
                            ...
                            if (!used_inos.contains(ino, 1)) {
                                derr << "Bad remote link dentry 0x" << std::
                                    hex << dir_ino << std::
                                    dec << "/" << dname << ", ino " << ino <<
                                    " not found" << dendl;
                                std::string key;
                                ...
                                dn_key.encode(key);
                                to_remove[dirfrag_t(dir_ino, frag_id)].
                                    insert(key);
                            }
                        }
                    }
                    else {
                        derr << "Invalid tag char '" << dentry_type <<
                            "' dentry 0x" << dir_ino << std::
                            dec << "/" << dname << dendl;
                        return -EINVAL;
                    }
                }
...

    {
        objecter->with_osdmap([&](const OSDMap & o) {
          for (auto p:       data_pools) {
                              const pg_pool_t * pi = o.get_pg_pool(p); if (!pi)
                              continue; if (pi->snap_seq > last_snap)
  ...
  for (auto & p:to_remove) {
        ...
        if (r != 0) {
            derr << "Error removing duplicated dentries from " << p.
                first << dendl;
            return r;
        }
    }
  ...
  for (auto & p:bad_nlink_inos) {
        ...
        snapid_t first;
        int r =
            read_dentry(p.second.dirino, p.second.frag, p.second.name, &inode,
                        &first);
        if (r < 0) {
        ...
        r = metadata_driver->inject_linkage(p.second.dirino, p.second.name,
                                            p.second.frag, inode, first);
        if (r < 0)
            return r;
    }
  ...
  for (auto & p:injected_inos) {
        ...
        InodeStore inode;
        snapid_t first;
        dout(20) << " fixing linkage (dnfirst) of " << p.second.
            dirino << ":" << p.second.name << dendl;
        int r =
            read_dentry(p.second.dirino, p.second.frag, p.second.name, &inode,
                        &first);
        if (r < 0) {
...
int DataScan::scan_frags()
{
    ...
    return forall_objects(metadata_io, true,[this] (std::string const &oid,
                                                    uint64_t obj_name_ino,
                          ...
                          r = parse_oid(oid, &obj_name_ino, &obj_name_offset);
                          if (r != 0) {
                          dout(4) << "Bad object name '" << oid << "', skipping"
                          << dendl; return r;}
                          ...
                          // way
                          dout(10) << "Skipping system ino " << obj_name_ino <<
                          dendl; return 0;}
                          ...
                          cpp_strerror(parent_r) << dendl; return r;}

                          if (parent_r != -ENODATA) {
                          try {
                          auto q = parent_bl.cbegin(); backtrace.decode(q);}
                          catch(buffer::error & e) {
                          dout(4) << "Corrupt backtrace on '" << oid << "': " <<
                          e.what() << dendl; if (!force_corrupt) {
                          return -EINVAL;}
                          else {
                          ...
                          if (layout_r != -ENODATA) {
                          try {
                          auto q = layout_bl.cbegin(); decode(loaded_layout, q);}
                          catch(buffer::error & e) {
                          dout(4) << "Corrupt layout on '" << oid << "': " << e.
                          what() << dendl; if (!force_corrupt) {
                          return -EINVAL;}
                          }
                          }
                          ...
                          fnode_t fnode;
                          r =
                          read_fnode(obj_name_ino, frag_t(), &fnode,
                                     &fnode_version); if (r == -EINVAL) {
                          derr << "Corrupt fnode on " << oid << dendl;
                          if (force_corrupt) {
                          fnode.fragstat.mtime = 0; fnode.fragstat.nfiles = 1;
...
int MetadataTool::read_fnode(inodeno_t ino, frag_t frag, fnode_t * fnode,
                              ...
                              snapid_t * dnfirst)
{
    ...
    dout(20) << "oid=" << frag_oid.name
        << " dname=" << dname << " frag=" << frag << ", r=" << r << dendl;
    if (r < 0) {
        return r;
    }
...
int MetadataDriver::inject_lost_and_found(inodeno_t ino,
                                          const InodeStore & dentry)
{
    ...
    if (r == -ENOENT || r == -EINVAL) {
        ...
        fragstat.nfiles = 1,
            build_dir_dentry(CEPH_INO_LOST_AND_FOUND, fragstat, inherit_layout,
                             &lf_ino);
...
int MetadataDriver::get_frag_of(inodeno_t dirino,
                                const std::string & target_dname,
                                frag_t * result_ft)
{
    object_t root_frag_oid = InodeStore::get_object_name(dirino, frag_t(), "");

    dout(20) << "dirino=" << dirino << " target_dname=" << target_dname <<
        dendl;
    ...
    if (r >= 0) {
        ...
        *result_ft = existing_dentry.pick_dirfrag(target_dname);
        ...
        if (r == 0) {
            ...
            r = read_dentry(parent_ino, parent_frag, parent_dname,
                            &existing_dentry);
            if (r >= 0) {
                // Got it!
                *result_ft = existing_dentry.pick_dirfrag(target_dname);
                dout(20) << "resolved via parent, frag is " << *result_ft <<
                    dendl;
                return 0;
            }
            else {
...
int MetadataDriver::inject_with_backtrace(const inode_backtrace_t & backtrace,
                                          const InodeStore & dentry)
{
    ...
    for (std::vector < inode_backpointer_t >::const_iterator i =
         backtrace.ancestors.begin(); i != backtrace.ancestors.end(); ++i) {
        ...
        if (r == -ENOENT) {
            ...
            dout(20) << "don't know fragment for 0x" << std::hex <<
                parent_ino << std::
                dec << "/" << dname << ", will insert to root" << dendl;
        }
        ...
        if (write_dentry) {
            if (i == backtrace.ancestors.begin()) {
                ...
                dout(10) << "Linking ancestor directory of inode 0x" << std::
                    hex << ino << " at 0x" << std::
                    hex << parent_ino << ":" << dname << dendl;
                ...
                /* NOTE: dnfirst fixed in scan_links */
                r = inject_linkage(parent_ino, dname, fragment,
                                   ancestor_dentry);
            }
...
int MetadataDriver::find_or_create_dirfrag(inodeno_t ino,
                                           frag_t fragment, bool * created)
{
...
int MetadataDriver::inject_linkage(inodeno_t dir_ino, const std::string & dname,
...
int MetadataDriver::init(librados::Rados & rados,
                         std::string & metadata_pool_name, const FSMap * fsmap,
                         fs_cluster_id_t fscid)
{
    if (metadata_pool_name.empty()) {
        ...
        dout(4) << "found metadata pool '" << metadata_pool_name << "'" <<
            dendl;
    }
    else {
        dout(4) << "forcing metadata pool '" << metadata_pool_name << "'" <<
            dendl;
    }
    return rados.ioctx_create(metadata_pool_name.c_str(), metadata_io);
}

int LocalFileDriver::init(librados::Rados & rados,
                          std::string & metadata_pool_name, const FSMap * fsmap,
                          fs_cluster_id_t fscid)
{
    return 0;
...
int LocalFileDriver::inject_data(const std::string & file_path,
                                 uint64_t size,
                                 uint32_t chunk_size, inodeno_t ino)
{
...
int LocalFileDriver::inject_with_backtrace(const inode_backtrace_t & bt,
                                           const InodeStore & dentry)
{
...
int LocalFileDriver::inject_lost_and_found(inodeno_t ino,
                                           const InodeStore & dentry)
{
...
void MetadataTool::build_file_dentry(inodeno_t ino, uint64_t file_size,
                                     time_t file_mtime,
...
void MetadataTool::build_dir_dentry(inodeno_t ino, const frag_info_t & fragstat,
                                    const file_layout_t & layout,
                                    InodeStore * out)
{
    ceph_assert(out != NULL);
