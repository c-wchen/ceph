...
int create_etag_verifier(const DoutPrefixProvider *dpp,
                         ...
                         const std::optional < RGWCompressionInfo >
                         &compression, etag_verifier_ptr &verifier)
{
    ...
    if (!found) {
        ldpp_dout(dpp,
                  -1) << "ERROR: manifest->get_rule() could not find rule"
                      << dendl;
        return -EIO;
    }
     ...
     * Etag.
     */
    for (auto mi = manifest.obj_begin(dpp); mi != manifest.obj_end(dpp);
         ++mi) {
    ...
    if (compression) {
        ...
        for (auto &ofs : part_ofs) {
            ...
            constexpr auto less =
            [](const compression_block & block, uint64_t ofs) {
    ...
    verifier.emplace < ETagVerifier_MPU > (cct, std::move(part_ofs),
                                           filter);
    return 0;
}
