        ...
        pthread_t thread_id)
{
    ...
    ANNOTATE_BENIGN_RACE_SIZED(&h->suicide_timeout,
                               sizeof(h->suicide_timeout),
                                 ...
                                 ceph::timespan suicide_grace)
{
    ...
    if (suicide_grace > ceph::timespan::zero()) {
        h->suicide_timeout.store(now + suicide_grace,
...
bool HeartbeatMap::is_healthy()
{
    ...
    if (m_cct->_conf->heartbeat_inject_failure) {
        ldout(m_cct,
              0) << "is_healthy injecting failure for next " << m_cct->
                 _conf->heartbeat_inject_failure << " seconds" << dendl;
        m_inject_unhealthy_until =
            now +
            std::chrono::seconds(m_cct->_conf->heartbeat_inject_failure);
    ...
    if (now < m_inject_unhealthy_until) {
        auto sec =
            ...
            (m_inject_unhealthy_until - now).count();
        ldout(m_cct,
              0) << "is_healthy = false, injected failure for next " << sec
                 << " seconds" << dendl;
        healthy = false;
    }
    ...
    ldout(m_cct,
          20) << "is_healthy = " << (healthy ? "healthy" : "NOT HEALTHY")
              << ", total workers: " << total << ", number of unhealthy: " <<
              unhealthy << dendl;
    return healthy;
}
