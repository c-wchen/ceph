    ...
    public rgw::auth::RoleApplier::Factory
{
    ...
    aplptr_t create_apl_remote(CephContext *const cct,
                               ...
                               acl_strategy_t
                               && acl_alg,
                               const rgw::auth::RemoteApplier::
                               AuthInfo &info) const override
    {
        auto apl = rgw::auth::add_sysreq(cct, driver, s,
                                         rgw::auth::
                                         RemoteApplier(cct, driver,
                                             std::
                                             move
                                             (acl_alg),
                                             info,
    ...
    override
    {
        ...
        return aplptr_t(new decltype(apl)(std::move(apl)));
                               ...
                               const req_state *const s,
                               const rgw::auth::RoleApplier::
                               Role &role,
                               const rgw::auth::RoleApplier::
                               TokenAttrs &token_attrs)const
    override
    {
        auto apl = rgw::auth::add_sysreq(cct, driver, s,
                                         rgw::auth::RoleApplier(cct,
                                             role,
                                             token_attrs));
        return aplptr_t(new decltype(apl)(std::move(apl)));
    } public:
    STSAuthStrategy(CephContext *const cct,
        ...
        : driver(driver),
          ...
          sts_engine(cct, driver, *ver_abstractor,
                     static_cast <
                     rgw::auth::LocalApplier::Factory * >(this),
                     static_cast <
                     rgw::auth::RemoteApplier::Factory * >(this),
                     static_cast <
                     rgw::auth::RoleApplier::Factory * >(this))
    {
...
template < class AbstractorT, bool AllowAnonAccessT = false > class AWSAuthStrategy: public rgw::auth::Strategy,
    ...
    Factory
{
    ...
    aplptr_t create_apl_local(CephContext *const cct,
                              ...
                              &perm_mask,
                              const std::
                              string &access_key_id) const override
    {
        auto apl = rgw::auth::add_sysreq(cct, driver, s,
                                         rgw::auth::
                                         LocalApplier(cct,
    ...
    using engine_map_t =
        std::map < std::string,
        std::reference_wrapper < const Engine >>;
    ...
    auto parse_auth_order(CephContext *const cct)
    {
        ...
        const std::set < std::string_view > allowed_auth =
        { "sts", "external", "local" };
    ...
    AWSAuthStrategy(CephContext *const cct,
                    ...
                    ImplicitTenants &implicit_tenant_context,
                    rgw::sal::Driver *driver)
        :    driver(driver), ver_abstractor(cct), anonymous_engine(cct,
                static_cast
                <
                rgw::
                auth::
                ...
                *
                >
                (this)),
             external_engines(cct, driver, implicit_tenant_context,
                              &ver_abstractor), sts_engine(cct, driver,
                                      implicit_tenant_context,
                                      &ver_abstractor),
                          ...
                          rgw::auth::LocalApplier::Factory * >(this))
    {
        ...
        if (!sts_engine.is_empty()) {
            engine_map.
            insert(std::
                   make_pair("sts", std::cref(sts_engine)));
        }
        ...
        if (!external_engines.is_empty()) {
            ...
            insert(std::
                   make_pair("external",
                             std::cref(external_engines)));
        }
        /* The local auth. */
        if (cct->_conf->rgw_s3_auth_use_rados) {
            engine_map.
            insert(std::
                   make_pair("local", std::cref(local_engine)));
        }
...
class AWSv4ComplMulti: public rgw::auth::Completer,
    public rgw::io::DecoratedRestfulClient <
    ...
    public std::enable_shared_from_this < AWSv4ComplMulti >
{
    ...
    class ChunkMeta
    {
        ...
        static constexpr size_t META_MIN_SIZE =
            sarrlen("0;chunk-signature=") + SIG_SIZE +
            sarrlen("\r\n");
         ...
         * in a stream. */
        static ChunkMeta create_first(const std::
                                      string_view &
                                      seed_signature)
        {
        ...
        static std::pair < ChunkMeta,
               size_t > create_next(CephContext *cct, ChunkMeta
                                    && prev, const char *metabuf,
                                    size_t metabuf_len);
    } chunk_meta;

    size_t stream_pos;
    boost::container::static_vector < char,
          ChunkMeta::META_MAX_SIZE > parsing_buf;
    ceph::crypto::SHA256 *sha256_hash;
    std::string prev_chunk_signature;

    bool is_signature_mismatched();
    std::string calc_chunk_signature(const std::
...
void rgw_create_s3_canonical_header(const DoutPrefixProvider *dpp,
                                    ...
                                    std::string &dest_str);
bool rgw_create_s3_canonical_header(const DoutPrefixProvider *dpp, const req_info &info,
...
static inline std::tuple < bool, std::string, utime_t > rgw_create_s3_canonical_header(const DoutPrefixProvider *dpp,
        const req_info &info, const bool qsr)
{
    std::string dest;
    utime_t header_time;

    const bool ok =
        rgw_create_s3_canonical_header(dpp, info, &header_time, dest, qsr);
...
static constexpr char AWS4_UNSIGNED_PAYLOAD_HASH[] =
    "UNSIGNED-PAYLOAD";
...
static inline std::string aws4_uri_recode(const std::
        string_view &src,
        bool encode_slash)
{
...
static inline std::
string get_v4_canonical_uri(const req_info &info)
{
...
static inline std::
string gen_v4_canonical_uri(const req_info &info)
{
    /* The code should normalize according to RFC 3986 but S3 does NOT do path
     * normalization that SigV4 typically does. This code follows the same
     * approach that boto library. See auth.py:canonical_uri(...). */
...
static inline const string calc_v4_payload_hash(const string &
        payload)
{
    ceph::crypto::SHA256 *sha256_hash =
        calc_hash_sha256_open_stream();
    calc_hash_sha256_update_stream(sha256_hash, payload.c_str(),
                                   payload.length());
...
static inline const char *get_v4_exp_payload_hash(const req_info &
        info)
{
...
static inline bool is_v4_payload_unsigned(const char *const
        exp_payload_hash)
{
    return boost::equals(exp_payload_hash,
...
static inline bool is_v4_payload_streamed(const char *const
        exp_payload_hash)
{
    return boost::equals(exp_payload_hash,
...
extern sha256_digest_t get_v4_canon_req_hash(CephContext *cct,
...
AWSEngine::VersionAbstractor::string_to_sign_t get_v4_string_to_sign(CephContext *cct,
...
extern AWSEngine::VersionAbstractor::server_signature_t get_v4_signature(const std::string_view &credential_scope,
        CephContext *const cct,
...
extern AWSEngine::VersionAbstractor::server_signature_t get_v2_signature(CephContext *,
