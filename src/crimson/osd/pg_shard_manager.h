...
#define FORWARD_TO_OSD_SINGLETON(METHOD) \
  FORWARD(METHOD, METHOD, get_osd_singleton_state())
      ...
      public:
        ...
        seastar::future <> start(const int whoami,
        ...
        auto & get_local_state()const {
             ...
             * up front. */
            return seastar::do_with(std::vector < seastar::foreign_ptr <
                                    local_cached_map_t >> (),[this,
                                                              map] (auto &
                                                                    fmaps) {
              fmaps.resize(seastar::smp::count); for (auto & i:fmaps)
                                    {
                                    i = seastar::foreign_ptr(map);}
                                    return shard_services.invoke_on_all([&fmaps]
                                                                        (auto &
                                                                         local)
                                                                        mutable
                                                                        {
        ...
        FORWARD_TO_OSD_SINGLETON(send_pg_created)
            ...
            FORWARD(is_stopping, is_stopping,
                    get_osd_singleton_state().osd_state)
            FORWARD(is_prestop, is_prestop, get_osd_singleton_state().osd_state)
            FORWARD(is_initializing, is_initializing,
                    get_osd_singleton_state().osd_state)
            FORWARD(set_prestop, set_prestop,
                    get_osd_singleton_state().osd_state)
            FORWARD(set_preboot, set_preboot,
                    get_osd_singleton_state().osd_state)
            FORWARD(set_booting, set_booting,
                    get_osd_singleton_state().osd_state)
            FORWARD(set_stopping, set_stopping,
                    get_osd_singleton_state().osd_state)
            FORWARD(set_active, set_active, get_osd_singleton_state().osd_state)
            FORWARD(when_active, when_active,
                    get_osd_singleton_state().osd_state)
            FORWARD_CONST(get_osd_state_string, to_string,
                          get_osd_singleton_state().osd_state)

            FORWARD(got_map, got_map, get_osd_singleton_state().osdmap_gate)
            FORWARD(wait_for_map, wait_for_map,
                    get_osd_singleton_state().osdmap_gate)
            // Metacoll
        ...
        template < typename F >
            ...
            ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
            return shard_services.invoke_on(core,
                                            [f =
                                             std::
                                             move(f)] (auto &
        ...
        template < typename T, typename F >
            auto with_remote_shard_state_and_op(core_id_t core,
                                                typename T::IRef && op,
                                                F && f) {
            ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
            if (seastar::this_shard_id() == core) {
                ...
                return std::invoke(std::move(f),
                                   ...
                                   target_shard_services, std::move(op));
            }
            return op->prepare_remote_submission().
                then([op = std::move(op), f =
                      std::move(f), this, core] (auto f_conn) mutable {
                     return shard_services.invoke_on(core,
                                                     [f = std::move(f), op =
                                                      std::move(op), f_conn =
                                                      std::move(f_conn)
        ...
        template < typename T >
            seastar::future <> run_with_pg_maybe_create(typename T::IRef op) {
            ...
            auto core =
                get_osd_singleton_state().pg_to_shard_mapping.
                maybe_create_pg(op->get_pgid());

            get_local_state().registry.remove_from_registry(*op);
            return with_remote_shard_state_and_op < T > (core, std::move(op),
                                                         [](PerShardState &
                                                            per_shard_state,
                                                            ShardServices &
                                                            shard_services,
                                                         ...
                                                         handle_error(crimson::
                                                                      ct_error::
                                                                      ecanceled::
                                                                      handle([&logger, &opref] (auto) {
                                                                             logger.
                                                                             debug
                                                                             ("{}: pg creation canceled, dropping",
                                                                              opref);
                                                                             return
                                                                             seastar::
                                                                             now
        ...
        template < typename T >
            seastar::future <> run_with_pg_maybe_wait(typename T::IRef op) {
            ceph_assert(op->use_count() == 1);
            ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
            auto & logger = crimson::get_logger(ceph_subsys_osd);
            static_assert(!T::can_create());
   ...
   */
        template < typename F, typename ... Args >
            seastar::future <> invoke_on_each_shard_seq(F && f) const {
            return sharded_map_seq(shard_services,
                                   [f =
                                    std::forward < F >
                                    (f)] (const ShardServices & shard_services)
                                   mutable {
   ...
   */
        template < typename F > seastar::future <> for_each_pg(F && f) const {
            return invoke_on_each_shard_seq([f =
                                             std::
                                             move(f)] (const auto &
                                                       local_service)mutable {
              for (auto & pg:              local_service.local_state.pg_map.
                                                 get_pgs())
                                            {
                                            std::apply(f,
                                                       pg);} return seastar::
   ...
   */
        template < typename F > void for_each_pgid(F && f) const {
            return get_osd_singleton_state().pg_to_shard_mapping.
                for_each_pgid(std::forward < F > (f));
        ...
        template < typename F > auto with_pg(spg_t pgid, F && f) {
            core_id_t core =
                get_osd_singleton_state().pg_to_shard_mapping.
                get_pg_mapping(pgid);
            return with_remote_shard_state(core,
                                           [pgid, f =
                                            std::move(f)] (auto & local_state,
                                                           auto & local_service)
                                           mutable {
                                           return std::invoke(std::move(f),
                                                              local_state.
        ...
        template < typename T, typename ... Args >
            ...
            ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
            auto op =
                get_local_state().registry.create_operation < T >
            ...
            auto fut =
                opref.template enter_stage <> (opref.get_connection_pipeline().
                                               await_active).then([this, &opref,
                                                                   &logger] {
                                                                  ceph_assert
                                                                  (seastar::
                                                                   this_shard_id
                   ...
                   ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
                   logger.
                   debug("{}: start_pg_operation active, entering await_map",
                         opref);
                   return opref.template enter_stage <> (opref.
                                                         get_connection_pipeline
                                                         ().await_map);}
            ).then([this, &logger, &opref] {
                   ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
                   logger.debug("{}: start_pg_operation await_map stage",
                                opref);
                   using OSDMapBlockingEvent =
                   OSD_OSDMapGate::OSDMapBlocker::BlockingEvent;
                   return opref.template with_blocking_event <
                   OSDMapBlockingEvent > ([this, &opref] (auto && trigger) {
                                          std::ignore = this;
                                          return get_osd_singleton_state().
                                          osdmap_gate.
                   ...
                   ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
                   logger.debug("{}: got map {}, entering get_pg", opref,
                                epoch);
                   return opref.template enter_stage <> (opref.
                                                         get_connection_pipeline
                                                         ().get_pg);}
            ).then([this, &logger, &opref, op = std::move(op)] ()mutable {
                   logger.debug("{}: in get_pg core {}", opref,
