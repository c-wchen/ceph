...
{
public:
    DummySuper(Transaction &t, RootNodeTracker &tracker,
               laddr_t *p_root_laddr)
        : Super(t, tracker), p_root_laddr {
        p_root_laddr}
    {
...
public:
    ...
    bool is_read_isolated() const override
    {
        return false;
    } read_iertr::future < NodeExtentRef > read_extent(Transaction &t,
            laddr_t addr)
    override
    {
        ...
        (SYNC) {
            ...
            return seastar::sleep(1u s).then([this, &t, addr] {
                return read_extent_sync(t,
                                        addr);
            }
                                            );
        }
    ...
    alloc_iertr::future < NodeExtentRef > alloc_extent(Transaction &t,
            laddr_t hint,
            extent_len_t len)
    override
    {
        SUBTRACET(seastore_onode, "allocating {}B with hint {:#x} ...", t,
                  len, hint);
        if constexpr
        (SYNC) {
            ...
            using namespace std::chrono_literals;
            return seastar::sleep(1u s).then([this, &t, len] {
                return alloc_extent_sync(t,
                                         len);
            }
                                            );
        }
    ...
    retire_iertr::future <> retire_extent(Transaction &t,
                                          NodeExtentRef extent) override
    {
        SUBTRACET(seastore_onode, "retiring {}B at {:#x} -- {} ...", t,
        ...
        (SYNC) {
            ...
            using namespace std::chrono_literals;
            return seastar::sleep(1u s).then([this, &t, extent] {
                return retire_extent_sync(t,
                                          extent);
            }
                                            );
        }
    ...
    getsuper_iertr::future < Super::URef > get_super(Transaction &t,
            RootNodeTracker &
            tracker) override
    {
        ...
        (SYNC) {
            ...
            using namespace std::chrono_literals;
            return seastar::sleep(1u s).then([this, &t, &tracker] {
                return get_super_sync(t,
                                      tracker);
            }
                                            );
        }
    ...
    read_iertr::future < NodeExtentRef > read_extent_sync(Transaction &t,
            laddr_t addr)
    {
    ...
    alloc_iertr::future < NodeExtentRef > alloc_extent_sync(Transaction &t,
            extent_len_t
            len)
    {
        ...
        auto extent =
            Ref < DummyNodeExtent > (new DummyNodeExtent(std::move(bp)));
        extent->set_laddr(addr);
        assert(allocate_map.find(extent->get_laddr()) ==
               allocate_map.end());
        allocate_map.insert({
    ...
    retire_iertr::future <> retire_extent_sync(Transaction &t,
            NodeExtentRef _extent)
    {
    ...
    getsuper_iertr::future < Super::URef > get_super_sync(Transaction &t,
            RootNodeTracker &
            tracker)
    {
...
template <> struct fmt::formatter <
    crimson::os::seastore::onode::DummyNodeExtent > : fmt::ostream_formatter {
};
#endif
