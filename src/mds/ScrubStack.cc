...
int ScrubStack::_enqueue(MDSCacheObject * obj, ScrubHeaderRef & header,
                         bool top)
{
    ...
    if (CInode * in = dynamic_cast < CInode * >(obj)) {
        ...
        dout(10) << __func__ << " with {" << *in << "}" << ", top=" << top <<
            dendl;
...
class C_RetryScrub:public MDSInternalContext {
  public:
    C_RetryScrub(ScrubStack * s,
                 MDSCacheObject * o):MDSInternalContext(s->mdcache->mds),
        stack(s), obj(o) {
...
bool ScrubStack::validate_inode_auth(CInode * in)
{
    if (in->is_auth()) {
        ...
        if (in->is_ambiguous_auth()) {
            ...
            auto r =
                make_message < MMDSScrub > (MMDSScrub::OP_QUEUEINO, in->ino(),
...
void ScrubStack::scrub_dir_inode(CInode * in, bool * added_children,
                                 bool * done)
{
  ...
  for (auto & fg:frags) {
        ...
        if (!dir->is_auth()) {
            if (dir->is_ambiguous_auth()) {
                ...
                dir->add_waiter(MDSCacheObject::WAIT_SINGLEAUTH,
                                gather.new_sub());
            }
            else if (mds->is_cluster_degraded()) {
                dout(20) << __func__ << " cluster degraded" << dendl;
    ...
    if (!scrub_remote.empty()) {
      ...
      for (auto & p:scrub_remote) {
            p.second.simplify();
            dout(20) << __func__ << " forward " << p.second << " to mds." << p.
                first << dendl;
            auto r =
                make_message < MMDSScrub > (MMDSScrub::OP_QUEUEDIR, in->ino(),
                                            std::move(p.second),
                                            header->get_tag(),
                                            header->get_origin(),
                                            header->is_internal_tag(),
                                      ...
                                      const CInode::validated_data & result)
{
    ...
    if (result.backtrace.checked && !result.backtrace.passed &&
        ...
        if (parent) {
            ...
            mdcache->mds->damage_table.notify_dentry(dir->inode->ino(),
                                                     dir->frag, parent->last,
                                                     parent->get_name(), path);
        }
    }
    ...
    if (!result.passed_validation) {
        ...
        f.flush(*css);
        derr << __func__ << " scrub error on inode " << *in << ": " << css->
            strv()
            << dendl;
    }
    else {
    ...
    auto abort_one =[this] (MDSCacheObject * obj) {
        if (CInode * in = dynamic_cast < CInode * >(obj)) {
                                                           ...
                                                           for (auto it =
                                                                ...
                                                                begin();
                                                                !it.end(); ++it)
  abort_one(*it); stack_size = 0; scrub_stack.clear(); scrub_waiting.clear(); for (auto & p:remote_scrubs)
                                                           remove_from_waiting
                                                           (p.first, false);
                                                           remote_scrubs.
                                                           clear();
                                                           ...
                                                           send_state_message
                                                           (int op) {
  MDSRank * mds = mdcache->mds; set < mds_rank_t > up_mds; mds->get_mds_map()->get_up_mds_set(up_mds); for (auto & r:up_mds)
                                                           {
                                                           if (r == 0)
                                                           continue;
                                                           auto m =
...
// send current scrub summary to cluster log
                                                           ...
                                                           void ScrubStack::
                                                           dispatch(const cref_t
                                                                    < Message >
                                                                    &m) {
                                                           switch (m->
                                                                   get_type()) {
case MSG_MDS_SCRUB:
handle_scrub(ref_cast < MMDSScrub > (m)); break; case MSG_MDS_SCRUB_STATS:
handle_scrub_stats(ref_cast < MMDSScrubStats > (m)); break; default:
                                                           derr <<
                                                           " scrub stack unknown message "
                                                           << m->
                                                           get_type() <<
                                                           dendl_impl;
                                                           ceph_abort_msg
...
case MMDSScrub::OP_QUEUEDIR:
                                                           {
          CInode * diri = mdcache->get_inode(m->get_ino()); ceph_assert(diri); std::vector < CDir * >dfs; MDSGatherBuilder gather(g_ceph_context); for (const auto & fg:m->get_frags())
                                                           {
                                                           CDir * dir =
                                                           diri->
                                                           get_dirfrag(fg);
              ...
              for (auto dir:                              dfs)
                                                           ...
                                                           send_message_mds(r,
                                                                            from);}
break; case MMDSScrub::OP_QUEUEDIR_ACK:
                                                           {
                                                           CInode * diri =
                                                           mdcache->
                                                           get_inode(m->
                                                           ...
                                                           if (it->second.
                                                               gather_set.
                                                               erase(from)) {
                  auto & queued = diri->scrub_queued_frags(); for (auto & fg:m->get_frags())
                                                           queued.
                                                           insert_raw(fg);
                                                           queued.simplify();
                                                           if (it->second.
...
break; case MMDSScrub::OP_QUEUEINO:
                                                           {
                                                           CInode * in =
                                                           mdcache->
                                                           get_inode(m->
                                                           ...
                                                           send_message_mds(r,
                                                                            from);}
break; case MMDSScrub::OP_QUEUEINO_ACK:
                                                           {
                                                           CInode * in =
                                                           mdcache->
                                                           get_inode(m->
...
break; case MMDSScrub::OP_ABORT:
scrub_abort(nullptr); break; case MMDSScrub::OP_PAUSE:
scrub_pause(nullptr); break; case MMDSScrub::OP_RESUME:
scrub_resume(); break; default:
                                                           derr <<
                                                           " scrub stack unknown scrub operation "
                                                           << m->
                                                           get_op() <<
                                                           dendl_impl;
                                                           ceph_abort_msg
                                                           ...
                                                           if (scrub_epoch !=
                                                               m->get_epoch() -
                                                               1) {
          scrub_epoch = m->get_epoch() - 1; for (auto & p:scrubbing_map)
                                                           {
                                                           if (p.second->
                                                           ...
                                                           *up_mds.rbegin();
                                                           ...
                                                           if (mds_scrub_stats.
                                                               ...
                                                               (size_t)
                                                               (up_max)) {
  bool any_aborting = false; bool fully_acked = true; for (const auto & stat:mds_scrub_stats)
                                                           {
                                                           if (stat.aborting
                                                               || stat.
                                                               epoch_acked <=
