...
namespace crimson {
    namespace qos_simulation {
        ...
        struct cli_group_t {
           ...
           cli_group_t(uint _client_count = 100, uint _client_wait = 0, uint _client_total_ops = 1000, uint _client_server_select_range = 10, uint _client_iops_goal = 50, uint _client_outstanding_ops = 100, double _client_reservation = 20.0, double _client_limit = 60.0, double _client_weight = 1.0):
            client_count(_client_count),
                ...
                client_limit(_client_limit), client_weight(_client_weight) {
                // empty
                ...
                out <<
                    ...
                    cli_group.
                    client_outstanding_ops << "\n" << std::fixed << std::
                    setprecision(1) << "client_reservation = " << cli_group.
                    client_reservation << "\n" << "client_limit = " <<
                    cli_group.
                    client_limit << "\n" << "client_weight = " << cli_group.
                    client_weight;
                return out;
        }};                     // class cli_group_t
        ...
        struct srv_group_t {
           ...
           srv_group_t(uint _server_count = 100, uint _server_iops = 40, uint _server_threads = 1):
            server_count(_server_count),
                server_iops(_server_iops), server_threads(_server_threads) {
                // empty
            } friend std::ostream & operator<<(std::ostream & out,
        ...
        struct sim_config_t {
           ...
           sim_config_t(uint _server_groups = 1, uint _client_groups = 1, bool _server_random_selection = false, bool _server_soft_limit = true):
            ...
            {
                out << "server_groups = " << sim_config.
                    server_groups << "\n" << "client_groups = " << sim_config.
                    client_groups << "\n" << "server_random_selection = " <<
                    sim_config.
                    server_random_selection << "\n" << "server_soft_limit = " <<
                    sim_config.server_soft_limit;
                return out;
            }
        };                      // class sim_config_t
        ...
        bool ceph_argparse_witharg(std::vector < const char *>&args,
                                   ...
                                   std::string * ret, ...);
        void ceph_argparse_early_args(std::vector < const char *>&args,
