...
class RGWMetadataObject {
  ...
  public:
    ...
    RGWMetadataObject(const obj_version & v, real_time m):objv(v), mtime(m) {
    }
...
class RGWMetadataHandler {
  ...
  public:
    ...
    virtual RGWMetadataObject *get_meta_obj(JSONObj * jo,
                                            const obj_version & objv,
                                            const ceph::real_time & mtime) = 0;

    virtual int get(std::string & entry, RGWMetadataObject ** obj,
                    optional_yield, const DoutPrefixProvider * dpp) = 0;
    virtual int put(std::string & entry, RGWMetadataObject * obj,
                    RGWObjVersionTracker & objv_tracker, optional_yield,
                    const DoutPrefixProvider * dpp, RGWMDLogSyncType type,
                    bool from_remote_zone) = 0;
    virtual int remove(std::string & entry, RGWObjVersionTracker & objv_tracker,
                       optional_yield, const DoutPrefixProvider * dpp) = 0;
    ...
    virtual int mutate(const std::string & entry,
                       ...
                       RGWMDLogStatus op_type, std::function < int () > f) = 0;

    virtual int list_keys_init(const DoutPrefixProvider * dpp,
                               const std::string & marker, void **phandle) = 0;
    virtual int list_keys_next(const DoutPrefixProvider * dpp, void *handle,
...
class RGWMetadataHandler_GenericMetaBE:public RGWMetadataHandler {
  ...
  protected:
    ...
    virtual int do_get(RGWSI_MetaBackend_Handler::Op * op, std::string & entry,
                       RGWMetadataObject ** obj, optional_yield y,
                       const DoutPrefixProvider * dpp) = 0;
    virtual int do_put(RGWSI_MetaBackend_Handler::Op * op, std::string & entry,
                       RGWMetadataObject * obj,
                       RGWObjVersionTracker & objv_tracker, optional_yield y,
                       const DoutPrefixProvider * dpp, RGWMDLogSyncType type,
                       bool from_remote_zone) = 0;
    virtual int do_put_operate(Put * put_op, const DoutPrefixProvider * dpp);
    virtual int do_remove(RGWSI_MetaBackend_Handler::Op * op,
                          std::string & entry,
                          RGWObjVersionTracker & objv_tracker, optional_yield y,
                          const DoutPrefixProvider * dpp) = 0;
  ...
  public:
    ...
    class Put {
      protected:
        ...
        int get(RGWMetadataObject ** obj, const DoutPrefixProvider * dpp) {
         ...
         Put(RGWMetadataHandler_GenericMetaBE * _handler,
             RGWSI_MetaBackend_Handler::Op * _op, std::string & _entry,
             RGWMetadataObject * _obj, RGWObjVersionTracker & _objv_tracker,
             optional_yield _y, RGWMDLogSyncType _type, bool from_remote_zone);
    ...
    int get(std::string & entry, RGWMetadataObject ** obj, optional_yield,
    ...
    int remove(std::string & entry, RGWObjVersionTracker & objv_tracker,
               optional_yield, const DoutPrefixProvider * dpp) override;
    ...
    int mutate(const std::string & entry,
               ...
               optional_yield y,
               const DoutPrefixProvider * dpp,
               RGWMDLogStatus op_type, std::function < int () > f) override;
...
class RGWMetadataManager {
    ...
    int find_handler(const std::string & metadata_key,
                     RGWMetadataHandler ** handler, std::string & entry);
  ...
  public:
    ...
    int get(std::string & metadata_key, Formatter * f, optional_yield y,
            const DoutPrefixProvider * dpp);
    int put(std::string & metadata_key, bufferlist & bl, optional_yield y,
            const DoutPrefixProvider * dpp, RGWMDLogSyncType sync_mode,
            bool from_remote_zone, obj_version * existing_version = NULL);
    int remove(std::string & metadata_key, optional_yield y,
               const DoutPrefixProvider * dpp);
    ...
    int list_keys_init(const DoutPrefixProvider * dpp,
                       const std::string & section, void **phandle);
    int list_keys_init(const DoutPrefixProvider * dpp,
                       const std::string & section, const std::string & marker,
                       void **phandle);
    int list_keys_next(const DoutPrefixProvider * dpp, void *handle, int max,
    ...
    void parse_metadata_key(const std::string & metadata_key,
                            std::string & type, std::string & entry);

    int get_shard_id(const std::string & section, const std::string & key,
                     int *shard_id);
};
...
class RGWMetadataHandlerPut_SObj:public RGWMetadataHandler_GenericMetaBE::Put {
  protected:
    ...
    RGWMetadataObject *old_obj {
    nullptr};
    bool exists {
    false};

  public:
    RGWMetadataHandlerPut_SObj(RGWMetadataHandler_GenericMetaBE * handler,
                               RGWSI_MetaBackend_Handler::Op * op,
...
void rgw_shard_name(const std::string & prefix, unsigned max_shards,
                    const std::string & key, std::string & name, int *shard_id);
void rgw_shard_name(const std::string & prefix, unsigned max_shards,
                    const std::string & section, const std::string & key,
                    std::string & name);
void rgw_shard_name(const std::string & prefix, unsigned shard_id,
                    std::string & name);
