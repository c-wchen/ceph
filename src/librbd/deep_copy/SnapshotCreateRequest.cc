...
I >::SnapshotCreateRequest(I *dst_image_ctx,
    ...
    : m_dst_image_ctx(dst_image_ctx), m_snap_name(snap_name),
      m_snap_namespace(snap_namespace), m_size(size), m_parent_spec(spec),
...
void SnapshotCreateRequest < I >::send_set_head()
{
    ...
    auto ctx = create_context_callback <
               ...
               &SnapshotCreateRequest < I >::handle_set_head > (this);
    auto req =
        SetHeadRequest < I >::create(m_dst_image_ctx, m_size,
                                     m_parent_spec,
...
void SnapshotCreateRequest < I >::send_create_object_map()
{
    ...
    if (snap_it == m_dst_image_ctx->snap_ids.end()) {
        lderr(m_cct) << "failed to locate snap: " << m_snap_name <<
                     dendl;
    ...
    std::string object_map_oid(librbd::ObjectMap <>::
                               object_map_name(m_dst_image_ctx->id,
                                       local_snap_id));
...
void SnapshotCreateRequest < I >::handle_create_object_map(int r)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to create object map: " <<
                     cpp_strerror(r)
                     << dendl;
        finish(r);
        return;
