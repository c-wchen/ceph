...
template <> struct CreateRequest <librbd::MockTestImageCtx > {
    ...
    static CreateRequest *create(const ConfigProxy &config,
                                 IoCtx &ioctx,
...
template <> struct CloneRequest <librbd::MockTestImageCtx > {
    ...
    static CloneRequest *create(ConfigProxy &config, IoCtx &p_ioctx,
                                ...
                                const cls::rbd::
                                SnapshotNamespace &snap_ns,
                                uint64_t p_snap_id, IoCtx &c_ioctx,
...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(0,
                  open_image(m_remote_io_ctx, m_image_name,
                             &m_remote_image_ctx));
    } void snap_create(librbd::ImageCtx *image_ctx,
    ...
    void expect_test_op_features(librbd::
                                 MockTestImageCtx &mock_image_ctx,
                                 bool enabled)
    {
    ...
    MockCreateImageRequest *create_request(MockThreads *
                                           mock_threads,
                                           const std::
...
TEST_F(TestMockImageReplayerCreateImageRequest, Create)
{
    ...
    MockCreateImageRequest *request =
        create_request(&mock_threads, "global uuid",
       ...
       CloneParentImageSyncing)
{
    ...
    snap_create(m_remote_image_ctx,
                ".rbd-mirror.local parent uuid.1234");
