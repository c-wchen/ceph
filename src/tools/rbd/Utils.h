...
template < typename T,
           void (T::*MF)(int)>void aio_completion_callback(librbd::
                   completion_t
                   completion,
                   void *arg)
{
...
static const std::
string RBD_IMAGE_DIFFS_BANNER_V2("rbd image diffs v2\n");
        ...
        for (auto &spec : specs) {
            switch (spec.format) {
                case RBD_ENCRYPTION_FORMAT_LUKS: {
                    auto opts =
                        static_cast <
                        librbd::encryption_luks_format_options_t *
                        >(spec.opts);
                    ceph_memzero_s(opts->passphrase.data(),
                ...
                case RBD_ENCRYPTION_FORMAT_LUKS2: {
                    auto opts =
                        static_cast <
                        librbd::encryption_luks2_format_options_t *
                        >(spec.opts);
                    ceph_memzero_s(opts->passphrase.data(),
                                   opts->passphrase.size(),
                                   opts->passphrase.size());
...
template < typename T,
           void (T::*MF)(int)>librbd::RBD::AioCompletion *create_aio_completion(T *t)
{
    return new librbd::RBD::AioCompletion(t,
                                          &detail::
                                          aio_completion_callback < T,
                                          MF >);
}
...
std::string get_positional_argument(const boost::program_options::
                                    variables_map &vm, size_t index);

void normalize_pool_name(std::string *pool_name);
std::string get_default_pool_name();

int get_image_or_snap_spec(const boost::program_options::
                           variables_map &vm, std::string *spec);
...
int get_pool_image_snapshot_names(const boost::program_options::
                                  variables_map &vm,
...
int get_pool_generic_snapshot_names(const boost::program_options::
                                    variables_map &vm,
                                    argument_types::
                                    ArgumentModifier mod,
                                    size_t *spec_arg_index,
...
int get_snap_create_flags(const boost::program_options::
                          variables_map &vm, uint32_t *flags);
...
std::string mirror_image_mode(librbd::
                              mirror_image_mode_t mirror_image_mode);
std::string mirror_image_state(librbd::
                               mirror_image_state_t mirror_image_state);
