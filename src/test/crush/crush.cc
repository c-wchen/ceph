                                                 ...
                                                 int num_osd)
{
    ...
    for (int r = 0; r < num_rack; ++r) {
        ...
        for (int h = 0; h < num_host; ++h) {
            loc["host"] =
                string("host-") + stringify(r) + string("-") + stringify(h);
            for (int o = 0; o < num_osd; ++o, ++osd) {
                c->insert_item(cct, osd, 1.0, string("osd.") + stringify(osd),
                               loc);
            }
        }
    }
    ...
    assert(ret == 0);
    ret =
        c->set_rule_step(ruleno, 2, CRUSH_RULE_CHOOSELEAF_INDEP, CRUSH_CHOOSE_N,
                         1);
    assert(ret == 0);
...
TEST(CRUSH, indep_toosmall)
{
    std::unique_ptr < CrushWrapper >
        c(build_indep_map(g_ceph_context, 1, 3, 1));
...
{
    std::unique_ptr < CrushWrapper >
        c(build_indep_map(g_ceph_context, 3, 3, 3));
    vector < __u32 > weight(c->get_max_devices(), 0x10000);
    c->dump_tree(&cout, NULL);
...
TEST(CRUSH, straw_same)
{
    ...
    if (0) {
        crush_bucket_straw *sb0 =
            reinterpret_cast <
            crush_bucket_straw * >(c->get_crush_map()->buckets[-1 - root0]);
        crush_bucket_straw *sb1 =
            reinterpret_cast <
            crush_bucket_straw * >(c->get_crush_map()->buckets[-1 - root1]);
