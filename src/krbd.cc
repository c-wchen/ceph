...
static std::optional < krbd_spec > spec_from_dev(udev_device * dev)
{
    ...
    return std::make_optional < krbd_spec > (pool_name, nspace_name ? : "",
...
{
    return
        udev_device_uptr(udev_device_new_from_syspath
                         (udev, udev_list_entry_get_name(l)));
}
                         ...
                         const string & options, string * pbuf)
{
  ...
  for (const auto & p:monmap.mon_info) {
    ...
    if (oss.tellp() == 0) {
        std::
            cerr << "rbd: failed to get mon address (possible ms_mode mismatch)"
            << std::endl;
        return -ENOENT;
    }
    ...
    if (keyring.get_secret(cct->_conf->name, secret)) {
        ...
        if (r >= 0) {
            ...
            std::
                cerr << "rbd: failed to add secret '" << key_name <<
                "' to kernel" << std::endl;
            return r;
        }
    }
 ...
 */
template < typename F >
    static std::pair < int, bool > wait_for_mapping(int sysfs_r_fd,
                                                    udev_monitor * mon,
...
class UdevMapHandler {
  public:
    UdevMapHandler(const krbd_spec * spec, std::string * pdevnode,
                   std::string * majnum, std::string * minnum):m_spec(spec),
        m_pdevnode(pdevnode), m_majnum(majnum), m_minnum(minnum) {
    }
    /*
          ...
          for (const auto & p:m_block_devs) {
                if (udev_device_get_devnode(p.get()) == m_devnode) {
                    ...
                    *m_minnum =
                    ...
                    ceph_assert(*m_majnum ==
                                udev_device_get_sysattr_value(m_bus_dev.get(),
                                                              "major"));
                  ...
                  string * pname)
{
    ...
    auto mapper = make_named_thread("mapper",[&buf, sysfs_r_fd = fds[1]] (){
                                    ...
                                    int r =
                                    safe_write(sysfs_r_fd, &sysfs_r,
                                               sizeof(sysfs_r)); if (r < 0) {
                                     ...
                                     dev_t * pdevno, string * pid)
{
    ...
    if (have_minor_attr()) {
        min =
            strict_strtoll(udev_device_get_sysattr_value(dev.get(), "minor"),
                           10, &err);
        if (!err.empty()) {
...
static int do_unmap(krbd_ctx * ctx, dev_t devno, const string & buf)
{
    ...
    auto unmapper =
        make_named_thread("unmapper",
                          [&buf, sysfs_r_fd = fds[1], flags = ctx->flags] (){
                          /*
                          ...
                          run_cmd("udevadm", "settle", "--timeout", "10",
                                  (char *)NULL); if (!err.empty())
                          std::cerr << "rbd: " << err << std::endl;}
                          }
                          else {
                          int r =
                          safe_write(sysfs_r_fd, &sysfs_r, sizeof(sysfs_r));
                          if (r < 0) {
                          ceph_abort_msgf("safe_write failed: %d", r);}
                          break;}
                          }
    ...
    if (r < 0) {
        if (!unmapped) {
            ...
            std::cerr << "rbd: udev wait failed: " << cpp_strerror(r) << std::
                endl;
            r = 0;
        }
    }
                       ...
                       const char *options)
{
    ...
    if (stat(devnode, &sb) < 0 || !S_ISBLK(sb.st_mode)) {
        std::cerr << "rbd: '" << devnode << "' is not a block device" << std::
            endl;
        return -EINVAL;
    }
    ...
    for (int tries = 0;; tries++) {
        ...
        if (r == -ENOENT && tries < 2) {
            ...
            if (tries) {
                std::
                    cerr << "rbd: udev enumerate missed a device, tries = " <<
                    tries << std::endl;
            }
            break;
        }
