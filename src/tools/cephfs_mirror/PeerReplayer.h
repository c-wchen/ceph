...
public:
    PeerReplayer(CephContext *cct, FSMirror *fs_mirror,
                 ...
                 const Peer &peer, const std::set < std::string,
...
private:
    inline static const std::string PRIMARY_SNAP_ID_KEY =
        "primary_snap_id";

    inline static const std::
    string SERVICE_DAEMON_FAILED_DIR_COUNT_KEY = "failure_count";
    inline static const std::
    string SERVICE_DAEMON_RECOVERED_DIR_COUNT_KEY =
        "recovery_count";
    ...
    struct SnapSyncStat {
        ...
        boost::optional < std::pair < uint64_t,
              std::string >> last_synced_snap;
        boost::optional < std::pair < uint64_t,
              std::string >> current_syncing_snap;
    ...
    void _inc_failed_count(const std::string &dir_root)
    {
        ...
        if (++sync_stat.nr_failures >= max_failures
            ...
            m_service_daemon->add_or_update_peer_attribute(m_filesystem.
                    fscid,
                    m_peer,
    ...
    void _set_last_synced_snap(const std::string &dir_root,
                               uint64_t snap_id,
    ...
    void set_last_synced_snap(const std::string &dir_root,
                              uint64_t snap_id,
                              const std::string &snap_name)
    {
    ...
    void set_current_syncing_snap(const std::string &dir_root,
                                  uint64_t snap_id,
                                  const std::string &snap_name)
    {
        std::scoped_lock locker(m_lock);
        auto &sync_stat = m_snap_sync_stats.at(dir_root);
        sync_stat.current_syncing_snap =
            std::make_pair(snap_id, snap_name);
    }
    ...
    void set_last_synced_stat(const std::string &dir_root,
                              uint64_t snap_id,
                              const std::string &snap_name,
                              double duration)
    {
    ...
    SnapshotReplayerThread >> SnapshotReplayers;
    ...
    void unlock_directory(const std::string &dir_root,
                          const DirRegistry &registry);
    void sync_snaps(const std::string &dir_root,
                    std::unique_lock < ceph::mutex > &locker);

    int build_snap_map(const std::string &dir_root,
    ...
    int propagate_deleted_entries(const std::string &dir_root,
                                  ...
                                  const FHandles &fh);
    int cleanup_remote_dir(const std::string &dir_root,
                           const std::string &epath,
                           const FHandles &fh);

    int should_sync_entry(const std::string &epath,
                          const struct ceph_statx &cstx,
                          const FHandles &fh, bool *need_data_sync,
                          bool *need_attr_sync);

    int open_dir(MountRef mnt, const std::string &dir_path,
    ...
    int do_synchronize(const std::string &dir_root,
                       const Snapshot &current,
                       boost::optional < Snapshot > prev);

    int synchronize(const std::string &dir_root,
                    const Snapshot &current,
                    boost::optional < Snapshot > prev);
    int do_sync_snaps(const std::string &dir_root);

    int remote_mkdir(const std::string &epath,
                     const struct ceph_statx &stx, const FHandles &fh);
    int remote_file_op(const std::string &dir_root,
                       const std::string &epath,
                       const struct ceph_statx &stx,
                       const FHandles &fh, bool need_data_sync,
                       bool need_attr_sync);
    int copy_to_remote(const std::string &dir_root,
                       const std::string &epath,
                       const struct ceph_statx &stx,
                       const FHandles &fh);
};
