...
class SharedDriverQueueData {
  ...
  public:
    ...
    PerfCounters *logger = nullptr;
     ...
     SharedDriverQueueData(SharedDriverData * driver, spdk_nvme_ctrlr * c,
                           spdk_nvme_ns * ns, uint64_t block_size,
                           const std::string & sn_tag, uint32_t sector_size,
                           uint32_t core, uint32_t queue_id)
    :driver(driver), ctrlr(c), ns(ns), sn(sn_tag), block_size(block_size),
        sector_size(sector_size), core_id(core), queueid(queue_id),
        run_func([this] () {
                 _aio_thread();
                 }),
        queue_empty(false),
        ...
        flush_waiters(0), completed_op_seq(0), queue_op_seq(0) {

        qpair = spdk_nvme_ctrlr_alloc_io_qpair(ctrlr, SPDK_NVME_QPRIO_URGENT);
        PerfCountersBuilder b(g_ceph_context,
                              string("NVMEDevice-AIOThread-" + stringify(this)),
                              l_bluestore_nvmedevice_first,
                              l_bluestore_nvmedevice_last);
        b.add_time_avg(l_bluestore_nvmedevice_aio_write_lat, "aio_write_lat",
        ...
        b.add_time_avg(l_bluestore_nvmedevice_aio_write_queue_lat,
                       "aio_write_queue_lat",
...
class SharedDriverData {
  ...
  public:
    ...
    :id(_id), sn(sn_tag), ctrlr(c), ns(ns) {
        int i;
        ...
        RTE_LCORE_FOREACH_SLAVE(i) {
            queues.
                push_back(new
                          SharedDriverQueueData(this, ctrlr, ns, block_size, sn,
                                                sector_size, i,
                                                queue_number++));
        }
...
struct Task {
    ...
    SharedDriverQueueData *queue;
     Task(NVMEDevice * dev, IOCommand c, uint64_t off, uint64_t l, int64_t rc =
          0)
  :    device(dev), command(c), offset(off), len(l), return_code(rc),
    ...
    void release_segs(SharedDriverQueueData * queue_data) {
        if (io_request.extra_segs) {
            for (uint16_t i = 0; i < io_request.nseg; i++)
                queue_data->data_buf_mempool.push_back(io_request.
                                                       extra_segs[i]);
    ...
    void copy_to_buf(char *buf, uint64_t off, uint64_t len) {
        ...
        uint64_t left = len;
        void **segs =
            io_request.extra_segs ? io_request.extra_segs : io_request.
            inline_segs;
        uint16_t i = 0;
        while (left > 0) {
...
static int data_buf_next_sge(void *cb_arg, void **address, uint32_t * length)
{
    ...
    addr =
        t->io_request.extra_segs ? t->io_request.extra_segs[t->io_request.
                                                            cur_seg_idx] : t->
        io_request.inline_segs[t->io_request.cur_seg_idx];
    ...
    if (t->io_request.cur_seg_left) {
        *address =
            (void *)((uint64_t) addr + size - t->io_request.cur_seg_left);
...
void SharedDriverQueueData::_aio_thread()
{
    ...
    if (data_buf_mempool.empty()) {
        for (uint16_t i = 0; i < data_buffer_default_num; i++) {
            ...
            if (!b) {
                derr << __func__ <<
                    " failed to create memory pool for nvme data buffer" <<
                    dendl;
                assert(b);
            }
    ...
    while (true) {
      ...
      again:
        ...
        if (inflight) {
            if (!spdk_nvme_qpair_process_completions
                (qpair, g_conf->bluestore_spdk_max_io_completion)) {
            ...
            case IOCommand::WRITE_COMMAND:
                {
                    ...
                    r = spdk_nvme_ns_cmd_writev(ns, qpair, lba_off, lba_count,
                                                io_complete, t, 0,
                                                data_buf_reset_sgl,
                                                data_buf_next_sge);
                    if (r < 0) {
                        derr << __func__ << " failed to do write command" <<
                            dendl;
                        t->ctx->nvme_task_first = t->ctx->nvme_task_last =
                            nullptr;
                        t->release_segs(this);
                        delete t;
                        ceph_abort();
                    ...
                    auto dur =
                        ...
                        (cur - start);
                    logger->tinc(l_bluestore_nvmedevice_aio_write_queue_lat,
                                 dur);
                    break;
                }
            case IOCommand::READ_COMMAND:
                {
                    ...
                    r = spdk_nvme_ns_cmd_readv(ns, qpair, lba_off, lba_count,
                                               io_complete, t, 0,
                                               data_buf_reset_sgl,
                                               data_buf_next_sge);
                    if (r < 0) {
                        ...
                        auto dur =
                            ...
                            std::chrono::nanoseconds > (cur - start);
                        logger->tinc(l_bluestore_nvmedevice_read_queue_lat,
                                     dur);
                    }
                    break;
                }
            case IOCommand::FLUSH_COMMAND:
                {
                    ...
                    if (r < 0) {
                        ...
                        auto dur =
                            std::chrono::duration_cast <
                            std::chrono::nanoseconds > (cur - start);
                        logger->tinc(l_bluestore_nvmedevice_flush_queue_lat,
                                     dur);
                    }
                    break;
                }
        ...
        if (!queue_empty.load()) {
            ...
            if (!task_queue.empty()) {
                t = task_queue.front();
                task_queue.pop();
                logger->set(l_bluestore_nvmedevice_queue_ops,
                            task_queue.size());
            }
            if (!t)
                queue_empty = true;
...
class NVMEManager {
  ...
  public:
    ...
    void register_ctrlr(const string & sn_tag, spdk_nvme_ctrlr * c,
        ...
        if (num_ns > 1) {
            dout(0) << __func__ <<
                " namespace count larger than 1, currently only use the first namespace"
                << dendl;
        }
        ...
        dout(1) << __func__ << " successfully attach nvme device at" <<
            spdk_pci_device_get_bus(pci_dev)
            << ":" << spdk_pci_device_get_dev(pci_dev) << ":" <<
            spdk_pci_device_get_func(pci_dev) << dendl;
        ...
        shared_driver_datas.
            push_back(new
                      SharedDriverData(shared_driver_datas.size() + 1, sn_tag,
                                       c, ns));
                     ...
                     struct spdk_nvme_ctrlr_opts *opts)
{
    ...
    if (result) {
        dout(0) << __func__ << " failed to get pci address from %s, " << trid->
            traddr << " return value is: %d" << result << dendl;
        return false;
    }
    ...
    dout(0) << __func__ << " found device at bus: " <<
        spdk_pci_device_get_bus(pci_dev)
        << ":" << spdk_pci_device_get_dev(pci_dev) << ":" <<
        spdk_pci_device_get_func(pci_dev) << " vendor:0x" <<
        spdk_pci_device_get_vendor_id(pci_dev) << " device:0x" <<
        spdk_pci_device_get_device_id(pci_dev)
        << dendl;
    result = spdk_pci_device_get_serial_number(pci_dev, serial_number, 128);
    if (result < 0) {
        dout(10) << __func__ << " failed to get serial number from %p" <<
            pci_dev << dendl;
        return false;
    }

    if (ctx->sn_tag.compare(string(serial_number, 16))) {
        dout(0) << __func__ << " device serial number (" << ctx->
            sn_tag << ") not match " << serial_number << dendl;
        return false;
    }
...
int NVMEManager::try_get(const string & sn_tag, SharedDriverData ** driver)
{
    ...
    if (sn_tag.empty()) {
        r = -ENOENT;
        derr << __func__ << " empty serial number: " << cpp_strerror(r) <<
            dendl;
        return r;
    }
  ...
  for (auto && it:shared_driver_datas) {
    ...
    if (!init) {
        init = true;
        dpdk_thread = std::thread([this, coremask_arg, m_core_arg,
                                   mem_size_arg] () {
                                  static struct spdk_env_opts opts; int r;
                                  ...
                                  g_ceph_context->_conf->bdev_nvme_retry_count;
                                  if (spdk_nvme_retry_count < 0)
                                  spdk_nvme_retry_count =
                                  SPDK_NVME_DEFAULT_RETRY_COUNT;
                                  std::unique_lock < std::mutex >
                                  l(probe_queue_lock); while (true) {
                                  if (!probe_queue.empty()) {
                                  ProbeContext * ctxt = probe_queue.front();
                                  probe_queue.pop_front();
                                  r =
                                  spdk_nvme_probe(NULL, ctxt, probe_cb,
                                                  attach_cb, NULL); if (r < 0) {
                                  assert(!ctxt->driver);
                                  derr << __func__ <<
...
int NVMEDevice::open(const string & p)
{
    ...
    if (r < 0) {
        derr << __func__ << " failed to get nvme device with sn " <<
            serial_number << dendl;
        return r;
    }
...
int NVMEDevice::collect_metadata(string prefix, map < string, string > *pm) const const
{
...
int NVMEDevice::aio_write(uint64_t off,
                          bufferlist & bl, IOContext * ioc, bool buffered)
{
...
int NVMEDevice::aio_read(uint64_t off,
                         uint64_t len, bufferlist * pbl, IOContext * ioc)
{
