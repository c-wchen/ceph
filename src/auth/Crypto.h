...
public:
    ...
    template < class BlockSizeT > CryptoKeyHandler(BlockSizeT)
    {
    ...
    virtual int encrypt(const ceph::buffer::list &in,
                        ceph::buffer::list &out, std::string *error) const =
                            0;
    virtual int decrypt(const ceph::buffer::list &in, ceph::buffer::list &out,
...
public:
    ...
    const ceph::buffer::ptr &get_secret()
    {
        return secret;
    }
    const ceph::buffer::ptr &get_secret() const
    {
        return secret;
    } bool empty() const
    {
