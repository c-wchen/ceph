    ...
    namespace cache {
        namespace pwl {
            ...
            struct TestMockCacheSSDWriteLog:public TestMockFixture {
                typedef librbd::cache::pwl::ssd::WriteLog <
                    librbd::MockImageCtx > MockSSDWriteLog;
                typedef librbd::cache::pwl::ImageCacheState <
                    librbd::MockImageCtx > MockImageCacheStateSSD;
                typedef librbd::cache::ImageWriteback < librbd::MockImageCtx >
                    MockImageWriteback;
                typedef librbd::plugin::Api < librbd::MockImageCtx > MockApi;

                MockImageCacheStateSSD *get_cache_state(MockImageCtx &
                                                        mock_image_ctx,
                                                        MockApi & mock_api) {
                    ...
                    return ssd_state;
                                            ...
                                            bool present, bool empty,
                                            bool clean, string host,
                                            string path, uint64_t size) {
                ...
                } void expect_context_complete(MockContextSSD & mock_context,
                                               int r) {
                ...
                void expect_metadata_remove(MockImageCtx & mock_image_ctx) {
                    EXPECT_CALL(*mock_image_ctx.operations,
                                execute_metadata_remove(_, _))
                        .
                        WillRepeatedly(Invoke
                                       ([](std::string key, Context * ctx) {
                                        ctx->complete(0);}));
                }
            };
            ...
            TEST_F(TestMockCacheSSDWriteLog, init_state_write) {
                ...
                MockApi mock_api;
                MockImageCacheStateSSD image_cache_state(&mock_image_ctx,
                                                         mock_api);

                validate_cache_state(ictx, image_cache_state, false, true, true,
                                     "", "", 0);

                image_cache_state.empty = false;
                image_cache_state.clean = false;
                ceph::mutex lock =
                    ceph::make_mutex("MockImageCacheStateSSD lock");
            ...
            TEST_F(TestMockCacheSSDWriteLog, init_state_json_write) {
                ...
                validate_cache_state(ictx, image_cache_state, true, false,
                                     false, "testhost", "/tmp", 1024);
            ...
            TEST_F(TestMockCacheSSDWriteLog, init_shutdown) {
                ...
                MockApi mock_api;
                MockSSDWriteLog ssd(mock_image_ctx,
                                    get_cache_state(mock_image_ctx, mock_api),
            ...
            TEST_F(TestMockCacheSSDWriteLog, compare_and_write_compare_matched) {
                ...
                int fadvise_flags = 0;
                ssd.write(std::move(image_extents), std::move(bl1),
                          fadvise_flags, &finish_ctx2);
                ASSERT_EQ(0, finish_ctx2.wait());
                ...
                ssd.compare_and_write( { {
                                      0, 4096}}, std::move(com_bl),
                                      std::move(bl2), &mismatch_offset,
                ...
                expect_context_complete(finish_ctx_read, 0);
                ssd.read( { {
                         0, 4096}}, &read_bl, fadvise_flags, &finish_ctx_read);
                ASSERT_EQ(0, finish_ctx_read.wait());
                ASSERT_EQ(4096, read_bl.length());
            ...
            TEST_F(TestMockCacheSSDWriteLog, writesame) {
                ...
                int fadvise_flags = 0;
                ssd.writesame(0, 4096, std::move(bl), fadvise_flags,
                              &finish_ctx2);
                ASSERT_EQ(0, finish_ctx2.wait());
            ...
            TEST_F(TestMockCacheSSDWriteLog, flush_source_user) {
                ...
                MockContextSSD finish_ctx3;
                expect_context_complete(finish_ctx3, 0);
                Extents image_extents2 { {
                0, 4096}};
                bufferlist bl2;
                bl2.append(std::string(4096, '1'));
                int fadvise_flags2 = 0;
                ssd.write(std::move(image_extents2), std::move(bl2),
                          fadvise_flags2, &finish_ctx3);
                ASSERT_EQ(0, finish_ctx3.wait());
