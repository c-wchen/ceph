...
class LFNIndex:public CollectionIndex {
  ...
  public:
    /// Constructor
    LFNIndex(CephContext * cct, coll_t collection, const char *base_path,   ///< [in] path to Index root
  ...
  :    
    ...
    last_failure(0), current_failure(0), collection(collection) {
    ...
    coll_t coll() const override {
        return collection;
    }
    /// Virtual destructor ~LFNIndex() override {
    }
    ...
    /// @see CollectionIndex
    int created(const ghobject_t & oid, const char *path) override;

    /// @see CollectionIndex
    int unlink(const ghobject_t & oid) override;

    /// @see CollectionIndex
    int lookup(const ghobject_t & oid,
               IndexedPath * path, int *hardlink) override;

    /// @see CollectionIndex;
    int pre_hash_collection(uint32_t pg_num,
                            uint64_t expected_num_objs) override;

    /// @see CollectionIndex
    int collection_list_partial(const ghobject_t & start,
  ...
  protected:
    ...
    virtual int _pre_hash_collection(uint32_t pg_num,
                                     uint64_t expected_num_objs) = 0;

    /// @see CollectionIndex
    virtual int _collection_list_partial(const ghobject_t & start,
                                         const ghobject_t & end,
                                         int max_count,
                                         vector < ghobject_t > *ls,
                                         ghobject_t * next) = 0;
  ...
  protected:
    ...
    /// Sync a subdirectory
    int fsync_dir(const vector < string > &path ///< [in] Path to sync
        );                      ///< @return Error Code, 0 on success

    /// Link an object from from into to
    int link_object(const vector < string > &from,  ///< [in] Source subdirectory.
   ...
   * @return Error Code, 0 on success.
   */
    int remove_objects(const vector < string > &dir,
   ...
   * @return Error Code, 0 on success
   */
    int move_objects(const vector < string > &from, ///< [in] Source subdirectory.
                     const vector < string > &to    ///< [in] Dest subdirectory.
        );
   ...
   * @return Error Code, 0 on success
   */
    int remove_object(const vector < string > &from,    ///< [in] Directory from which to remove.
   ...
   */
    int get_mangled_name(const vector < string > &from, ///< [in] Subdirectory
    ...
    static int move_subdir(LFNIndex & from, ///< [in] from index
    ...
    static int move_object(LFNIndex & from, ///< [in] from index
                           LFNIndex & dest, ///< [in] to index
   ...
   * @return Error code on failure, 0 on success
   */
    int list_objects(const vector < string > &to_list,
                     int max_objects,
                     long *handle, map < string, ghobject_t > *out);

    /// Lists subdirectories.
    int list_subdirs(const vector < string > &to_list,  ///< [in] Directory to list.
                     vector < string > *out ///< [out] Subdirectories listed.
        );

    /// Create subdirectory.
    int create_path(const vector < string > &to_create  ///< [in] Subdirectory to create.
        );

    /// Remove subdirectory.
    int remove_path(const vector < string > &to_remove  ///< [in] Subdirectory to remove.
        );

    /// Check whether to_check exists.
    int path_exists(const vector < string > &to_check,  ///< [in] Subdirectory to check.
                    int *exists ///< [out] 1 if it exists, 0 else
        );

    /// Save attr_value to attr_name attribute on path.
    int add_attr_path(const vector < string > &path,    ///< [in] Path to modify.
    ...
    int get_attr_path(const vector < string > &path,    ///< [in] Path to read.
                      const string & attr_name, ///< [in] Attribute to read.
                      bufferlist & attr_value   ///< [out] Attribute value read.
        );

    /// Remove attr from path
    int remove_attr_path(const vector < string > &path, ///< [in] path from which to remove attr
   ...
   */ int lfn_get_name(
    ...
    bool lfn_is_object(const string & short_name    ///< [in] Filename to check
        );                      ///< True if short_name is an object, false otherwise

    /// Filters subdir filenames
    bool lfn_is_subdir(const string & short_name,   ///< [in] Filename to check.
                       string * demangled_name  ///< [out] Demangled subdir name.
        );                      ///< @return True if short_name is a subdir, false otherwise

    /// Generate object name
    string lfn_generate_object_name_keyless(const ghobject_t & oid  ///< [in] Object for which to generate.
        );                      ///< @return Generated object name.

    /// Generate object name
    string lfn_generate_object_name_poolless(const ghobject_t & oid ///< [in] Object for which to generate.
        );                      ///< @return Generated object name.

    /// Generate object name
    static string lfn_generate_object_name_current(const ghobject_t & oid   ///< [in] Object for which to generate.
        );                      ///< @return Generated object name.

    /// Generate object name
    string lfn_generate_object_name(const ghobject_t & oid  ///< [in] Object for which to generate.
        ) {
        if (index_version == HASH_INDEX_TAG)
    ...
    /// Parse object name
    int lfn_parse_object_name_keyless(const string & long_name, ///< [in] Name to parse
                                      ghobject_t * out  ///< [out] Resulting Object
        );                      ///< @return True if successfull, False otherwise.

    /// Parse object name
    int lfn_parse_object_name_poolless(const string & long_name,    ///< [in] Name to parse
                                       ghobject_t * out ///< [out] Resulting Object
        );                      ///< @return True if successfull, False otherwise.

    /// Parse object name
    int lfn_parse_object_name(const string & long_name, ///< [in] Name to parse
                              ghobject_t * out  ///< [out] Resulting Object
        );                      ///< @return True if successfull, False otherwise.

    /// Checks whether short_name is a hashed filename.
    bool lfn_is_hashed_filename(const string & short_name   ///< [in] Name to check.
        );                      ///< @return True if short_name is hashed, False otherwise.

    /// Checks whether long_name must be hashed.
    bool lfn_must_hash(const string & long_name ///< [in] Name to check.
        );                      ///< @return True if long_name must be hashed, False otherwise.

    /// Generate hashed name.
    string lfn_get_short_name(const ghobject_t & oid,   ///< [in] Object for which to generate.
    ...
    string get_full_path_subdir(const vector < string > &rel    ///< [in] The subdir.
        );                      ///< @return Full path to rel.

    /// Get full path to object
    string get_full_path(const vector < string > &rel,  ///< [in] Path to object.
                         const string & name    ///< [in] Filename of object.
        );                      ///< @return Fullpath to object at name in rel.

    /// Get mangled path component
    string mangle_path_component(const string & component   ///< [in] Component to mangle
        );                      /// @return Mangled component

    /// Demangle component
    string demangle_path_component(const string & component ///< [in] Subdir name to demangle
        );                      ///< @return Demangled path component.

    /// Decompose full path into object name and filename.
    int decompose_full_path(const char *in, ///< [in] Full path to object.
    ...
    string mangle_attr_name(const string & attr ///< [in] Attribute to mangle.
        );                      ///< @return Mangled attribute name.

    /// checks whether long_name could hash to short_name
    bool short_name_matches(const char *short_name, ///< [in] name to check against
