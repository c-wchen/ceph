...
TEST(AsyncCompletion, MoveOnly)
{
    ...
    std::optional < move_only > arg3;
    {
        ...
        auto c = Completion::create(ex1, [&ec1](error_code ec) {
            ec1 = ec;
        });
    ...
    }
    {
        ...
        Completion::post(std::move(c),
        boost::asio::error::operation_aborted, move_only {
        });
        EXPECT_FALSE(ec3);
    }
...
TEST(AsyncCompletion, CompletionList)
{
    ...
    completions.clear_and_dispose([](Completion * c) {
        Completion::post(std::unique_ptr <
        Completion > {
            c
        });
    });
...
TEST(AsyncCompletion, CompletionReference)
{
    ...
    auto c = Completion::create(ex1, [](int &i) {
        ++i;
    });
...
TEST(AsyncCompletion, ThrowOnCtor)
{
    ...
    auto ex1 = context.get_executor();
    {
        ...
        EXPECT_THROW(Completion::create(ex1, [t = throws_on_move {
        }](int &i) {
            static_cast < void >(t);
            ++i;
        }), std::runtime_error);
    }
    {
        ...
        EXPECT_THROW(Completion::create(ex1, [](int &i) {
            ++i;
        }, throws_on_move {
        }), std::runtime_error);
    }
...
TEST(AsyncCompletion, FreeFunctions)
{
    ...
    auto c2 = create_completion < void (int), int >(ex1, [](int) {
    }, 5);
