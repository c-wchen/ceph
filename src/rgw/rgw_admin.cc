...
void usage()
{
    ...
    cout << "\nScript options:\n";
    cout <<
        "   --context                 context in which the script runs. one of: "
        + LUA_CONTEXT_LIST + "\n";
    cout <<
  ...
  public:
    ...
    template < class Container >
        bool find_command(Container & args,
                          ...
                          string * error, vector < string > *expected) {
        auto node = &cmd_root;
...
void dump_bi_entry(bufferlist & bl, BIIndexType index_type,
                   Formatter * formatter)
{
    auto iter = bl.cbegin();
    switch (index_type) {
...
static void show_policy_names(std::vector < string > policy_names,
                              Formatter * formatter)
{
...
static void show_roles_info(vector < std::unique_ptr <
                            rgw::sal::RGWRole >> &roles, Formatter * formatter)
{
...
static void show_reshard_status(const list < cls_rgw_bucket_instance_entry >
                                &status, Formatter * formatter)
{
    formatter->open_array_section("status");
  for (const auto & entry:status) {
        formatter->open_object_section("entry");
        formatter->dump_string("reshard_status",
                               to_string(entry.reshard_status));
        formatter->close_section();
    }
    formatter->close_section();
    formatter->flush(cout);
...
{
    rgw_bucket b {
    tenant_name, bucket_name, bucket_id};
...
template < class T > static int read_decode_json(const string & infile, T & t)
{
    ...
    if (ret < 0) {
        cerr << "ERROR: failed to read input: " << cpp_strerror(-ret) << std::
            endl;
        return ret;
    }
    JSONParser p;
...
template < class T >
    static bool decode_dump(const char *field_name, bufferlist & bl,
                            Formatter * f)
{
    T t;
...
static bool dump_string(const char *field_name, bufferlist & bl, Formatter * f)
{
    ...
    f->dump_string(field_name,
                   val.c_str() /* hide encoded null termination chars */ );
...
void set_quota_info(RGWQuotaInfo & quota, OPT opt_cmd, int64_t max_size,
                    int64_t max_objects, bool have_max_size,
                     ...
                     bool have_max_size, bool have_max_objects)
{
    std::unique_ptr < rgw::sal::Bucket > bucket;
    int r =
        driver->get_bucket(dpp(), nullptr, tenant_name, bucket_name, &bucket,
                           null_yield);
    if (r < 0) {
                         ...
                         bool have_max_read_bytes, bool have_max_write_bytes)
{
    ...
    bool ratelimit_configured =
        set_ratelimit_info(ratelimit_info, opt_cmd, max_read_ops, max_write_ops,
    ...
    if (!ratelimit_configured) {
        ldpp_dout(dpp(),
                  0) << "ERROR: no rate limit values have been specified" <<
            dendl;
        return -EINVAL;
    }
    bufferlist bl;
...
int show_user_ratelimit(std::unique_ptr < rgw::sal::User > &user,
                        Formatter * formatter)
{
    RGWRateLimitInfo ratelimit_info;
    user->load_user(dpp(), null_yield);
...
int set_user_bucket_quota(OPT opt_cmd, RGWUser & user,
                          RGWUserAdminOpState & op_state, int64_t max_size,
                          int64_t max_objects, bool have_max_size,
                          bool have_max_objects)
{
...
int set_user_quota(OPT opt_cmd, RGWUser & user, RGWUserAdminOpState & op_state,
                   int64_t max_size, int64_t max_objects, bool have_max_size,
                   bool have_max_objects)
{
    RGWUserInfo & user_info = op_state.get_user_info();
...
int check_min_obj_stripe_size(rgw::sal::Driver * driver, rgw::sal::Object * obj,
                              uint64_t min_stripe_size, bool * need_rewrite)
{
    int ret = obj->get_obj_attrs(null_yield, dpp());
    if (ret < 0) {
        ldpp_dout(dpp(),
                  -1) << "ERROR: failed to stat object, returned error: " <<
...
int check_obj_locator_underscore(rgw::sal::Object * obj, bool fix,
                                 bool remove_bad, Formatter * f)
{
    ...
    if ((needs_fixing || remove_bad) && fix) {
        ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->getRados()->fix_head_obj_locator(dpp(),
                                                        obj->get_bucket()->
                                                        get_info(),
                                                        needs_fixing,
                                                        remove_bad,
...
int check_obj_tail_locator_underscore(RGWBucketInfo & bucket_info,
                                      ...
                                      Formatter * f)
{
    ...
    int ret =
        static_cast <
        rgw::sal::RadosStore *
        >(driver)->getRados()->fix_tail_obj_locator(dpp(), bucket_info, key,
                                                    fix, &needs_fixing,
                                                    null_yield);
    if (ret < 0) {
        cerr << "ERROR: fix_tail_object_locator_underscore() returned ret=" <<
            ret << std::endl;
        status = "failed";
    }
    else {
...
int do_check_object_locator(const string & tenant_name,
                            const string & bucket_name, bool fix,
                            bool remove_bad, Formatter * f)
{
    if (remove_bad && !fix) {
        cerr << "ERROR: can't have remove_bad specified without fix" << std::
            endl;
        return -EINVAL;
    }
    ...
    int ret =
        init_bucket(nullptr, tenant_name, bucket_name, bucket_id, &bucket);
    if (ret < 0) {
        cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret) << std::
            endl;
        return ret;
    }
    int count = 0;
    ...
    do {
        ret =
            bucket->list(dpp(), params, max_entries - count, results,
                         null_yield);
        if (ret < 0) {
        ...
        for (vector < rgw_bucket_dir_entry >::iterator iter =
             results.objs.begin(); iter != results.objs.end(); ++iter) {
            std::unique_ptr < rgw::sal::Object > obj =
                bucket->get_object(iter->key);

            if (obj->get_name()[0] == '_') {
                ret =
                    check_obj_locator_underscore(obj.get(), fix, remove_bad, f);

                if (ret >= 0) {
                    ret =
                        check_obj_tail_locator_underscore(bucket->get_info(),
                                                          obj->get_key(), fix,
                                                          f);
                    if (ret < 0) {
...
get_remote_conn(rgw::sal::RadosStore * driver, const RGWZoneGroup & zonegroup,
                const std::string & remote)
{
    boost::optional < RGWRESTConn > conn;
    if (remote == zonegroup.get_id()) {
        conn.emplace(driver->ctx(), driver, remote, zonegroup.endpoints,
                     zonegroup.api_name);
    }
    else {
      for (const auto & z:zonegroup.zones) {
            ...
            if (remote == zone.id) {
                conn.emplace(driver->ctx(), driver, remote, zone.endpoints,
                             zonegroup.api_name);
                break;
            }
        }
...
static boost::optional < RGWRESTConn >
get_remote_conn(rgw::sal::RadosStore * driver, const RGWPeriodMap & period_map,
                const std::string & remote)
{
    boost::optional < RGWRESTConn > conn;
                                  ...
                                  bufferlist & in_data, JSONParser & parser)
{
    ...
    rgw_user user;
    int ret =
        conn->forward(dpp(), user, info, nullptr, MAX_REST_RESPONSE, &in_data,
                      &response, null_yield);
                       ...
                       bufferlist & in_data, JSONParser & parser)
{
    ...
    bufferlist response;
    int ret =
        req.forward_request(dpp(), key, info, MAX_REST_RESPONSE, &in_data,
                            &response, null_yield);
                         ...
                         bool force)
{
    ...
    if (driver->get_zone()->get_id() == master_zone) {
        ...
        ret = rgw::commit_period(dpp(), null_yield, cfgstore, driver,
                                 ...
                                 period, cerr, force);
        if (ret < 0) {
            cerr << "failed to commit period: " << cpp_strerror(-ret) << std::
                endl;
        }
        return ret;
    }
    ...
    if (!remote.empty()) {
        conn =
            get_remote_conn(static_cast < rgw::sal::RadosStore * >(driver),
                            period.get_map(), remote);
        if (!conn) {
    ...
    JSONParser p;
    int ret =
        send_to_remote_or_url(remote_conn, url, opt_region, access, secret,
                              info, bl, p);
    if (ret < 0) {
    ...
    if (ret < 0) {
        cerr << "Error updating local objects: " << cpp_strerror(ret) << std::
            endl;
        return ret;
    }
                         ...
                         Formatter * formatter, bool force)
{
    ...
    ret = cfgstore->read_period(dpp(), null_yield, realm.current_period,
                                epoch, period);
    if (ret < 0) {
        cerr << "failed to load current period: " << cpp_strerror(-ret) << std::
            endl;
        return ret;
    }
                          ...
                          RGWPeriod * period)
{
    ...
    JSONParser p;
    int ret =
        send_to_remote_or_url(remote_conn, url, opt_region, access_key,
                              secret_key,
                              info, bl, p);
    if (ret < 0) {
        cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
    ...
    ret = cfgstore->create_period(dpp(), null_yield, exclusive, *period);
    if (ret < 0) {
        cerr << "Error storing period " << period->
            get_id() << ": " << cpp_strerror(ret) << std::endl;
    }
    return 0;
}
...
{
    RGWMetaSyncStatusManager sync(static_cast <
    ...
    if (ret < 0) {
        status.
            push_back(string
                      ("failed to retrieve sync info: sync.init() failed: ") +
                      cpp_strerror(-ret));
        return;
    }
    ...
    if (ret < 0) {
        status.push_back(string("failed to read sync status: ") +
                         cpp_strerror(-ret));
        return;
    }
  ...
  for (auto marker_iter:sync_status.sync_markers) {
    ...
    stringstream ss;
    push_ss(ss,
            status) << "full sync: " << num_full << "/" << total_shards <<
        " shards";

    if (num_full > 0) {
        push_ss(ss,
                status) << "full sync: " << full_total -
            full_complete << " entries to sync";
    }

    push_ss(ss,
            status) << "incremental sync: " << num_inc << "/" << total_shards <<
        " shards";

    map < int, RGWMetadataLogInfo > master_shards_info;
    string master_period =
        static_cast <
        rgw::sal::RadosStore * >(driver)->svc()->zone->get_current_period_id();

    ret =
        sync.read_master_log_shards_info(dpp(), master_period,
                                         &master_shards_info);
    if (ret < 0) {
        status.push_back(string("failed to fetch master sync status: ") +
                         cpp_strerror(-ret));
        return;
    }
    ...
    if (sync_status.sync_info.period != master_period) {
        status.
            push_back(string
                      ("master is on a different period: master_period=" +
                       master_period + " local_period=" +
      ...
      for (auto local_iter:sync_status.sync_markers) {
            ...
            if (iter == master_shards_info.end()) {
                /* huh? */
                derr <<
                    "ERROR: could not find remote sync shard status for shard_id="
                    << shard_id << dendl;
                continue;
            }
    ...
    if (!shards_behind.empty()) {
        ...
        ret =
            sync.read_master_log_shards_next(dpp(),
                                             sync_status.sync_info.period,
    ...
    int total_behind =
        shards_behind.size() + (sync_status.sync_info.num_shards - num_inc);
    if (total_behind == 0) {
        push_ss(ss, status) << "metadata is caught up with master";
    }
    else {
        push_ss(ss,
                status) << "metadata is behind on " << total_behind <<
            " shards";
        push_ss(ss,
                status) << "behind shards: " << "[" << shards_behind_set << "]";
        if (oldest) {
...
static void get_data_sync_status(const rgw_zone_id & source_zone,
                                 list < string > &status, int tab)
{
    stringstream ss;
    ...
    if (!
        (sz =
         static_cast <
         rgw::sal::RadosStore *
         >(driver)->svc()->zone->find_zone(source_zone))) {
    ...
    if (!static_cast <
        rgw::sal::RadosStore *
        >(driver)->svc()->zone->zone_syncs_from(static_cast <
                                                rgw::sal::RadosStore *
                                                >(driver)->svc()->zone->
                                                get_zone(), *sz)) {
    ...
    RGWDataSyncStatusManager sync(static_cast <
                                  rgw::sal::RadosStore * >(driver),
                                  static_cast <
                                  rgw::sal::RadosStore *
                                  >(driver)->svc()->rados->
                                  get_async_processor(), source_zone, nullptr);

    int ret = sync.init(dpp());
    if (ret < 0) {
        push_ss(ss, status,
                tab) << string("failed to retrieve sync info: ") +
            cpp_strerror(-ret);
        flush_ss(ss, status);
        return;
    }
    ...
    if (ret < 0 && ret != -ENOENT) {
        push_ss(ss, status,
                tab) << string("failed read sync status: ") +
            cpp_strerror(-ret);
        return;
    }
    ...
    ret =
        sync.read_recovering_shards(dpp(), sync_status.sync_info.num_shards,
                                    recovering_shards);
    if (ret < 0 && ret != ENOENT) {
        push_ss(ss, status,
                tab) << string("failed read recovering shards: ") +
            cpp_strerror(-ret);
        return;
    }
  ...
  for (auto marker_iter:sync_status.sync_markers) {
    ...
    push_ss(ss, status,
            tab) << "full sync: " << num_full << "/" << total_shards <<
        " shards";

    if (num_full > 0) {
        push_ss(ss, status,
                tab) << "full sync: " << full_total -
            full_complete << " buckets to sync";
    }

    push_ss(ss, status,
            tab) << "incremental sync: " << num_inc << "/" << total_shards <<
        " shards";
    ...
    if (ret < 0) {
        push_ss(ss, status,
                tab) << string("failed to fetch source sync status: ") +
            cpp_strerror(-ret);
        return;
    }
  ...
  for (auto local_iter:sync_status.sync_markers) {
    ...
    if (!shards_behind.empty()) {
        ...
        ret =
            sync.read_source_log_shards_next(dpp(), shards_behind, &master_pos);

        if (ret < 0) {
            derr << "ERROR: failed to fetch next positions (" <<
                cpp_strerror(-ret) << ")" << dendl;
        }
        else {
          for (auto iter:master_pos) {
    ...
    if (total_behind == 0 && total_recovering == 0) {
        ...
        push_ss(ss, status,
                tab) << "data is behind on " << total_behind << " shards";
        push_ss(ss, status,
                tab) << "behind shards: " << "[" << shards_behind_set << "]";
        if (oldest) {
            push_ss(ss, status,
                    tab) << "oldest incremental change not applied: " <<
                oldest->second << " [" << oldest->first << ']';
        }
    }
    ...
    if (total_recovering > 0) {
        push_ss(ss, status,
                tab) << total_recovering << " shards are recovering";
        push_ss(ss, status,
                tab) << "recovering shards: " << "[" << recovering_shards <<
            "]";
    }
...
static void tab_dump(const string & header, int width,
...
{
    auto features =
        rgw::zone_features::set { rgw::zone_features::supported.begin(),
...
static void sync_status(Formatter * formatter)
{
    ...
    cout << std::setw(width) << "realm" << std::setw(1) << " " << zone->
        get_realm_id() << " (" << zone->get_realm_name() << ")" << std::endl;
    cout << std::setw(width) << "zonegroup" << std::setw(1) << " " << zonegroup.
        get_id() << " (" << zonegroup.get_name() << ")" << std::endl;
    cout << std::setw(width) << "zone" << std::setw(1) << " " << zone->
        get_id() << " (" << zone->get_name() << ")" << std::endl;
    cout << std::setw(width) << "current time" << std::
        setw(1) << " " << to_iso_8601(ceph::real_clock::now(),
                                      iso_8601_format::YMDhms) << std::endl;

    const auto & rzg =
        static_cast < const rgw::sal::RadosZoneGroup & >(zonegroup).get_group();

    cout << std::setw(width) << "zonegroup features enabled: " << rzg.
        enabled_features << std::endl;
    if (auto d = get_disabled_features(rzg.enabled_features); !d.empty()) {
        cout << std::
            setw(width) << "                   disabled: " << d << std::endl;
    }
    ...
    auto & zone_conn_map =
        static_cast <
        rgw::sal::RadosStore * >(driver)->svc()->zone->get_zone_conn_map();
  ...
  for (auto iter:zone_conn_map) {
        ...
        if (driver->get_zone()->get_zonegroup().
            get_zone_by_id(source_id.id, &sz) == 0) {
...
static int bucket_source_sync_status(const DoutPrefixProvider * dpp,
                                     rgw::sal::RadosStore * driver,
                                     const RGWZone & zone,
                                     const RGWZone & source, RGWRESTConn * conn,
                                     const RGWBucketInfo & bucket_info,
                                     rgw_sync_bucket_pipe pipe, int width,
                                     std::ostream & out)
{
    out << indented {
    width,
            "source zone"} <<source.id << " (" << source.
        name << ")" << std::endl;
    ...
    if (r < 0) {
        ldpp_dout(dpp,
                  -1) << "failed to read source bucket info: " <<
    ...
    out << indented {
    width, "source bucket"}
    <<source_bucket->get_key() << std::endl;
    ...
    r = rgw_read_bucket_full_sync_status(dpp, driver, pipe, &full_status,
                                         null_yield);
    if (r >= 0) {
        ...
        if (full_status.state == BucketSyncState::Full) {
            out << indented {
            width}
            <<"full sync: " << full_status.full.count << " objects completed\n";
            return 0;
        }
        ...
        if (log.layout.type != rgw::BucketLogType::InIndex) {
            ldpp_dout(dpp,
                      -1) << "unrecognized log layout type " << log.layout.
                type << dendl;
            return -EINVAL;
        }
        ...
        lderr(driver->
              ctx()) << "failed to read bucket full sync status: " <<
            cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (r < 0) {
        lderr(driver->
              ctx()) << "failed to read bucket incremental sync status: " <<
            cpp_strerror(r) << dendl;
        return r;
    }
    ...
    out << indented {
    width} <<"incremental sync on " << total_shards << " shards\n";
  ...
  for (const auto & r:remote_markers.get()) {
    ...
    if (!shards_behind.empty()) {
        out << indented {
        width}
        <<"bucket is behind on " << shards_behind.size() << " shards\n";
        out << indented {
        width}
...
void encode_json(const char *name,
                 const RGWBucketSyncFlowManager::pipe_set & pset, Formatter * f)
{
...
static std::vector < string > convert_bucket_set_to_str_vec(const std::set <
                                                            rgw_bucket > &bs)
{
...
static void get_hint_entities(const std::set < rgw_zone_id > &zones,
                              const std::set < rgw_bucket > &buckets,
                              std::set < rgw_sync_bucket_entity >
                              *hint_entities)
{
  for (auto & zone_id:zones) {
      for (auto & b:buckets) {
            ...
            hint_entities->
                insert(rgw_sync_bucket_entity(zone_id, hint_bucket->get_key()));
        }
    }
}
...
{
    rgw_zone_id zone_id =
        opt_target_zone.value_or(driver->get_zone()->get_id());
    ...
    if (eff_bucket) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            cerr << "ERROR: init_bucket failed: " << cpp_strerror(-ret) << std::
                endl;
            return ret;
        }
        ...
        if (ret >= 0) {
            ...
            bucket_handler.reset(handler->
                                 alloc_child(bucket->get_info(),
                                             std::move(attrs)));
        }
        else {
    ...
    zone_policy_handler->reflect(dpp(), nullptr, nullptr, nullptr, nullptr, &source_zones, &target_zones, false);   /* relaxed: also get all zones that we allow to sync to/from */

    std::set < rgw_sync_bucket_entity > hint_entities;

    get_hint_entities(source_zones, handler->get_source_hints(),
                      &hint_entities);
    get_hint_entities(target_zones, handler->get_target_hints(),
                      &hint_entities);
  ...
  for (auto & hint_entity:hint_entities) {
        ...
        int r =
            driver->get_sync_policy_handler(dpp(), zid, hint_bucket,
                                            &hint_bucket_handler, null_yield);
        if (r < 0) {
            ldpp_dout(dpp(),
                      20) <<
        ...
        hint_bucket_handler->get_pipes(&resolved_dests, &resolved_sources, self_entity);    /* flipping resolved dests and sources as these are
...

    {
        ...
        {
            Formatter::ObjectSection resolved_hints_section(*formatter,
                                                            "resolved-hints-1");
        ...
        }
        {
            Formatter::ObjectSection resolved_hints_section(*formatter,
                                                            "resolved-hints");
            encode_json("sources", handler->get_resolved_source_hints(),
                        formatter);
...
static int bucket_sync_info(rgw::sal::Driver * driver,
                            const RGWBucketInfo & info, std::ostream & out)
{
    ...
    out << indented {
    width,
            "realm"} <<zone->get_realm_id() << " (" << zone->
        get_realm_name() << ")\n";
    out << indented {
    width,
            "zonegroup"} <<zonegroup.get_id() << " (" << zonegroup.
        get_name() << ")\n";
    out << indented {
    width, "zone"} <<zone->get_id() << " (" << zone->get_name() << ")\n";
    out << indented {
    width, "bucket"} <<info.bucket << "\n\n";

    if (!static_cast <
        rgw::sal::RadosStore *
        >(driver)->ctl()->bucket->bucket_imports_data(info.bucket, null_yield,
                                                      dpp())) {
    ...
    int r =
        driver->get_sync_policy_handler(dpp(), std::nullopt, info.bucket,
                                        &handler, null_yield);
    if (r < 0) {
        ldpp_dout(dpp(),
                  -1) << "ERROR: failed to get policy handler for bucket (" <<
            info.bucket << "): r=" << r << ": " << cpp_strerror(-r) << dendl;
        return r;
    }
  ...
  for (auto & m:sources) {
      ...
      for (auto & pipe_handler:m.second) {
            out << indented {
            width, "bucket"}
            <<*pipe_handler.source.bucket << std::endl;
        }
    }
...
static int bucket_sync_status(rgw::sal::Driver * driver,
                              ...
                              std::ostream & out)
{
    ...
    if (!static_cast <
        ...
        >(driver)->ctl()->bucket->bucket_imports_data(info.bucket, null_yield,
                                                      dpp())) {
        out << "Sync is disabled for bucket " << info.bucket.
            name << " or bucket has no sync sources" << std::endl;
        return 0;
    }
    ...
    if (!source_zone_id.empty()) {
        std::unique_ptr < rgw::sal::Zone > zone;
        int ret =
            driver->get_zone()->get_zonegroup().get_zone_by_id(source_zone_id.
                                                               id, &zone);
        if (ret < 0) {
  ...
  for (auto & zone_id:zone_ids) {
        auto z =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->zone->get_zonegroup().zones.find(zone_id.id);
...
static void parse_tier_config_param(const string & s, map < string, string,
                                    ltstr_nocase > &out)
{
    int level = 0;
    string cur_conf;
...
static int check_pool_support_omap(const rgw_pool & pool)
{
    ...
    int ret =
        static_cast <
        rgw::sal::RadosStore *
        >(driver)->getRados()->get_rados_handle()->ioctx_create(pool.to_str().
                                                                c_str(),
                                                                io_ctx);
    if (ret < 0) {
                                ...
                                std::unique_ptr < rgw::sal::Bucket > *bucket)
{
    ...
    if (num_shards > (int)static_cast <
        rgw::sal::RadosStore * >(driver)->getRados()->get_max_bucket_shards()) {
        cerr << "ERROR: num_shards too high, max value: " << static_cast <
            rgw::sal::RadosStore *
            >(driver)->getRados()->get_max_bucket_shards() << std::endl;
        return -EINVAL;
    }
    ...
    int num_source_shards =
        rgw::current_num_shards((*bucket)->get_info().layout);

    if (num_shards <= num_source_shards && !yes_i_really_mean_it) {
        cerr << "num shards is less or equal to current shards count" << std::
            endl << "do you really mean it? (requires --yes-i-really-mean-it)"
            << std::endl;
        return -EINVAL;
    }
    return 0;
...
static int scan_totp(CephContext * cct, ceph::real_time & now,
                     rados::cls::otp::otp_info_t & totp,
                     vector < string > &pins, time_t * pofs)
{
#define MAX_TOTP_SKEW_HOURS (24 * 7)
    ...
    while (time_ofs_abs < max_skew) {
        int rc =
            oath_totp_validate2(totp.seed_bin.c_str(), totp.seed_bin.length(),
                                start_time,
                                step_size,
                                time_ofs,
                                1,
        ...
        if (rc != OATH_INVALID_OTP) {
            rc = oath_totp_validate2(totp.seed_bin.c_str(), totp.seed_bin.length(), start_time, step_size, time_ofs - step_size,    /* smaller time_ofs moves time forward */
                                     1, nullptr, pins[1].c_str());
            if (rc != OATH_INVALID_OTP) {
                *pofs = time_ofs - step_size + step_size * totp.window / 2;
                ldpp_dout(dpp(),
                          20) << "found at time=" << start_time -
                    time_ofs << " time_ofs=" << time_ofs << dendl;
                return 0;
            }
        }
        sign = -sign;
...
static int trim_sync_error_log(int shard_id, const string & marker,
                               int delay_ms)
{
    auto oid =
    ...
    for (;;) {
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->cls->timelog.trim(dpp(),
                                                                       oid, { },
                                                                       { }, { },
                                                                       marker,
                                                                       nullptr,
...
template < class T >
    static bool require_non_empty_opt(std::optional < T > opt,
...
template < class T >
    static void show_result(T & obj, Formatter * formatter, ostream & os)
{
...
class SyncPolicyContext {
  ...
  public:
    ...
    :cfgstore(cfgstore), b(std::move(_bucket)) {
        ...
        if (ret < 0) {
            cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::
                endl;
            return ret;
        }
...
void resolve_zone_id_opt(std::optional < string > &zone_name,
...
void resolve_zone_ids_opt(std::optional < vector < string > >&names,
...
class JSONFormatter_PrettyZone:public JSONFormatter {
    class Handler:public JSONEncodeFilter::Handler < rgw_zone_id > {
        void encode_json(const char *name, const void *pval,
                         ceph::Formatter * f) const override {
            auto zone_id = *(static_cast < const rgw_zone_id * >(pval));
            string zone_name;
            std::unique_ptr < rgw::sal::Zone > zone;
            if (driver->get_zone()->get_zonegroup().
                get_zone_by_id(zone_id.id, &zone) == 0) {
...
void init_realm_param(CephContext * cct, string & var,
...
int main(int argc, const char **argv)
{
    ...
    init_realm_param(cct.get(), zonegroup_id, opt_zonegroup_id,
                     "rgw_zonegroup_id");
    ...
    for (std::vector < const char *>::iterator i = args.begin();
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            opt_tenant = val;
                 ...
                 (args, i, &val, "--user_ns", (char *)NULL)) {
            user_ns = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--access-key", (char *)NULL)) {
            access_key = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--subuser", (char *)NULL)) {
            subuser = val;
        }
        else if (ceph_argparse_witharg
            ...
            pool = rgw_pool(pool_name);
                 ...
                 (args, i, &val, "-o", "--object", (char *)NULL)) {
            object = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--objects-file", (char *)NULL)) {
            objects_file = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--object-version", (char *)NULL)) {
            object_version = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--client-id", (char *)NULL)) {
            client_id = val;
        }
            ...
            op_id = val;
                 ...
                 (args, i, &val, "--op-mask", (char *)NULL)) {
            op_mask_str = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--key-type", (char *)NULL)) {
            key_type_str = val;
            ...
            min_rewrite_stripe_size = (uint64_t) atoll(val.c_str());
                 ...
                 (args, i, &val, "--max-buckets", (char *)NULL)) {
            max_buckets = (int)strict_strtol(val.c_str(), 10, &err);
            if (!err.empty()) {
                cerr << "ERROR: failed to parse max buckets: " << err << std::
                    endl;
                return EINVAL;
            }
            max_buckets_specified = true;
                 ...
                 (args, i, &val, "--max-entries", (char *)NULL)) {
            max_entries = (int)strict_strtol(val.c_str(), 10, &err);
            max_entries_specified = true;
            if (!err.empty()) {
                cerr << "ERROR: failed to parse max entries: " << err << std::
                    endl;
                return EINVAL;
            }
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--max-size", (char *)NULL)) {
            ...
            have_max_size = true;
                 ...
                 (args, i, &val, "--max-objects", (char *)NULL)) {
            max_objects = (int64_t) strict_strtoll(val.c_str(), 10, &err);
            if (!err.empty()) {
                cerr << "ERROR: failed to parse max objects: " << err << std::
                    endl;
                return EINVAL;
            }
            have_max_objects = true;
                 ...
                 (args, i, &val, "--max-read-ops", (char *)NULL)) {
            ...
            have_max_read_ops = true;
                 ...
                 (args, i, &val, "--max-write-ops", (char *)NULL)) {
            ...
            have_max_write_ops = true;
                 ...
                 (args, i, &val, "--max-read-bytes", (char *)NULL)) {
            max_read_bytes = (int64_t) strict_strtoll(val.c_str(), 10, &err);
            if (!err.empty()) {
                cerr << "ERROR: failed to parse max read bytes: " << err <<
                    std::endl;
                return EINVAL;
            }
            ...
            if (!err.empty()) {
                cerr << "ERROR: failed to parse max write bytes: " << err <<
                    std::endl;
                return EINVAL;
            }
            have_max_write_bytes = true;
                 ...
                 (args, i, &val, "--date", "--time", (char *)NULL)) {
            date = val;
            if (end_date.empty())
                end_date = date;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--start-date", "--start-time",
                  (char *)NULL)) {
            start_date = val;
        }
        else if (ceph_argparse_witharg
            ...
            end_date = val;
                 ...
                 (args, i, &val, "--num-shards", (char *)NULL)) {
            num_shards = (int)strict_strtol(val.c_str(), 10, &err);
            if (!err.empty()) {
                cerr << "ERROR: failed to parse num shards: " << err << std::
                    endl;
                return EINVAL;
            }
            ...
            if (!err.empty()) {
                cerr << "ERROR: failed to parse bucket-index-max-shards: " <<
                    err << std::endl;
                return EINVAL;
            }
        }
            ...
            orphan_stale_secs =
                (uint64_t) strict_strtoll(val.c_str(), 10, &err);
            if (!err.empty()) {
            ...
            set_perm = true;
                 ...
                 (args, i, &val, "--temp-url-key", (char *)NULL)) {
            temp_url_keys[0] = val;
            ...
            set_temp_url_key = true;
                 ...
                 (args, i, &val, "--bucket-id", (char *)NULL)) {
            bucket_id = val;
            opt_bucket_id = val;
            if (bucket_id.empty()) {
            ...
            format = val;
                 ...
                 (args, i, &val, "--categories", (char *)NULL)) {
            string cat_str = val;
            ...
            infile = val;
                 ...
                 (args, i, &val, "--metadata-key", (char *)NULL)) {
            metadata_key = val;
        }
            ...
            marker = val;
                 ...
                 (args, i, &val, "--start-marker", (char *)NULL)) {
            start_marker = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--end-marker", (char *)NULL)) {
            end_marker = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--quota-scope", (char *)NULL)) {
            quota_scope = val;
        }
        else if (ceph_argparse_witharg
            ...
            ratelimit_scope = val;
                 ...
                 (args, i, &val, "--index-type", (char *)NULL)) {
            ...
            /* do nothing */
                 ...
                 (args, i, &val, "--redirect-zone", (char *)NULL)) {
            redirect_zone = val;
            ...
            is_read_only_set = true;
                 ...
                 (args, i, &val, "--master-zone", (char *)NULL)) {
            master_zone = val;
        }
            ...
            opt_region = val;
                 ...
                 (args, i, &val, "--realm-id", (char *)NULL)) {
            realm_id = val;
            opt_realm_id = val;
            g_conf().set_val("rgw_realm_id", val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--realm-new-name", (char *)NULL)) {
            realm_new_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--zonegroup-id", (char *)NULL)) {
            zonegroup_id = val;
            ...
            zonegroup_new_name = val;
                 ...
                 (args, i, &val, "--placement-id", (char *)NULL)) {
            placement_id = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--storage-class", (char *)NULL)) {
            ...
            get_str_list(val, ",", tags);
                 ...
                 (args, i, &val, "--tags-add", (char *)NULL)) {
            get_str_list(val, ",", tags_add);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--tags-rm", (char *)NULL)) {
            get_str_list(val, ",", tags_rm);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--api-name", (char *)NULL)) {
            api_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--zone-id", (char *)NULL)) {
            zone_id = val;
            opt_zone_id = val;
            g_conf().set_val("rgw_zone_id", val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--zone-new-name", (char *)NULL)) {
            zone_new_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--endpoints", (char *)NULL)) {
            get_str_list(val, endpoints);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--sync-from", (char *)NULL)) {
            get_str_list(val, sync_from);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--sync-from-rm", (char *)NULL)) {
            ...
            sync_from_all_specified = true;
                 ...
                 (args, i, &val, "--source-zone", (char *)NULL)) {
            source_zone_name = val;
            opt_source_zone_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--source-zone-id", (char *)NULL)) {
            opt_source_zone_id = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--dest-zone", (char *)NULL)) {
            opt_dest_zone_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--dest-zone-id", (char *)NULL)) {
            opt_dest_zone_id = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--tier-type", (char *)NULL)) {
            tier_type = val;
            tier_type_specified = true;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--tier-config", (char *)NULL)) {
            parse_tier_config_param(val, tier_config_add);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--tier-config-rm", (char *)NULL)) {
            parse_tier_config_param(val, tier_config_rm);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--index-pool", (char *)NULL)) {
            index_pool = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--data-pool", (char *)NULL)) {
            data_pool = val;
        }
        else if (ceph_argparse_witharg
            ...
            index_type_specified = true;
                 ...
                 (args, i, &val, "--compression", (char *)NULL)) {
            compression_type = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--role-name", (char *)NULL)) {
            role_name = val;
        }
            ...
            assume_role_doc = val;
                 ...
                 (args, i, &val, "--policy-name", (char *)NULL)) {
            policy_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--policy-doc", (char *)NULL)) {
            perm_policy_doc = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--path-prefix", (char *)NULL)) {
            path_prefix = val;
        }
        else if (ceph_argparse_witharg
            ...
            max_session_duration = val;
                 ...
                 (args, i, &val, "--totp-serial", (char *)NULL)) {
            totp_serial = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--totp-pin", (char *)NULL)) {
            totp_pin.push_back(val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--totp-seed", (char *)NULL)) {
            totp_seed = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--totp-seed-type", (char *)NULL)) {
            totp_seed_type = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--totp-seconds", (char *)NULL)) {
            totp_seconds = atoi(val.c_str());
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--totp-window", (char *)NULL)) {
            totp_window = atoi(val.c_str());
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--trim-delay-ms", (char *)NULL)) {
            ...
            topic_name = val;
                 ...
                 (args, i, &val, "--subscription", (char *)NULL)) {
            sub_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--event-id", (char *)NULL)) {
            event_id = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--group-id", (char *)NULL)) {
            opt_group_id = val;
        }
            ...
            opt_status = val;
                 ...
                 (args, i, &val, "--flow-type", (char *)NULL)) {
            opt_flow_type = val;
        }
        else if (ceph_argparse_witharg
            ...
            opt_dest_zone_names = std::move(v);
                 ...
                 (args, i, &val, "--dest-zone-ids", (char *)NULL)) {
            opt_dest_zone_ids = zone_ids_from_str(val);
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--flow-id", (char *)NULL)) {
            opt_flow_id = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--pipe-id", (char *)NULL)) {
            opt_pipe_id = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--source-tenant", (char *)NULL)) {
            opt_source_tenant = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--source-bucket", (char *)NULL)) {
            ...
            opt_source_bucket_id = val;
                 ...
                 (args, i, &val, "--dest-tenant", (char *)NULL)) {
            opt_dest_tenant = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--dest-bucket", (char *)NULL)) {
            opt_dest_bucket_name = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--dest-bucket-id", (char *)NULL)) {
            ...
            opt_prefix = val;
                 ...
                 (args, i, &val, "--prefix-rm", (char *)NULL)) {
            opt_prefix_rm = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--priority", (char *)NULL)) {
            ...
            opt_mode = val;
                 ...
                 (args, i, &val, "--dest-owner", (char *)NULL)) {
            opt_dest_owner.emplace(val);
            opt_dest_owner = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--retry-delay-ms", (char *)NULL)) {
            opt_retry_delay_ms = std::chrono::milliseconds(atoi(val.c_str()));
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--timeout-sec", (char *)NULL)) {
            ...
            // do nothing
                 ...
                 (args, i, &val, "--context", (char *)NULL)) {
            str_script_ctx = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--package", (char *)NULL)) {
            script_package = val;
        }
        else if (ceph_argparse_binary_flag
            ...
            // do nothing
                 ...
                 (args, i, &val, "--rgw-obj-fs", (char *)NULL)) {
            rgw_obj_fs = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--enable-feature", (char *)NULL)) {
            if (!rgw::zone_features::supports(val)) {
                std::
                    cerr << "ERROR: Cannot enable unrecognized zone feature \""
                    << val << "\"" << std::endl;
                return EINVAL;
            }
    ...
    if (args.empty()) {
        ...
        OPT::ZONEGROUP_ADD, OPT::ZONEGROUP_CREATE,
                ...
                OPT::ZONEGROUP_PLACEMENT_LIST, OPT::ZONEGROUP_PLACEMENT_GET,
                OPT::ZONEGROUP_PLACEMENT_DEFAULT, OPT::ZONE_CREATE,
                OPT::ZONE_DELETE, OPT::ZONE_GET, OPT::ZONE_SET,
                OPT::ZONE_RENAME, OPT::ZONE_LIST, OPT::ZONE_MODIFY,
                OPT::ZONE_DEFAULT, OPT::ZONE_PLACEMENT_ADD,
                OPT::ZONE_PLACEMENT_RM, OPT::ZONE_PLACEMENT_MODIFY,
                OPT::ZONE_PLACEMENT_LIST, OPT::ZONE_PLACEMENT_GET,
                OPT::REALM_CREATE, OPT::PERIOD_DELETE, OPT::PERIOD_GET,
                ...
                OPT::REALM_LIST_PERIODS, OPT::REALM_GET_DEFAULT,
                OPT::REALM_RENAME, OPT::REALM_SET, OPT::REALM_DEFAULT,
                OPT::REALM_PULL};
        ...
        OPT::USER_INFO,
                ...
                OPT::PUBSUB_TOPIC_GET, OPT::SCRIPT_GET,};

        std::set < OPT > gc_ops_list = {
            OPT::GC_LIST, OPT::GC_PROCESS, OPT::OBJECT_RM, OPT::BUCKET_RM,  // --purge-objects
                OPT::USER_RM,   // --purge-data
        OPT::OBJECTS_EXPIRE,
                OPT::OBJECTS_EXPIRE_STALE_RM,
                OPT::LC_PROCESS,
                OPT::BUCKET_SYNC_RUN,
                OPT::DATA_SYNC_RUN, OPT::BUCKET_REWRITE, OPT::OBJECT_REWRITE};

        raw_storage_op =
            (raw_storage_ops_list.find(opt_cmd) != raw_storage_ops_list.end()
             || raw_period_update || raw_period_pull);
        bool need_cache =
            readonly_ops_list.find(opt_cmd) == readonly_ops_list.end();
        ...
        auto config_store_type =
            g_conf().get_val < std::string > ("rgw_config_store");
        ...
        if (tenant.empty()) {
            ...
            if (rgw::sal::User::empty(user) && opt_cmd != OPT::ROLE_CREATE
                ...
                cerr << "ERROR: --tenant is set, but there's no user ID" <<
                    std::endl;
                return EINVAL;
            }
        ...
        if ((!access_key.empty()) && gen_access_key) {
            cerr <<
                "ERROR: key parameter conflict, --access-key & --gen-access-key"
                << std::endl;
            return EINVAL;
        }
        if ((!secret_key.empty()) && gen_secret_key) {
            cerr << "ERROR: key parameter conflict, --secret & --gen-secret" <<
                std::endl;
            return EINVAL;
        }
    }
    ...
    if (format == "xml")
        formatter =
            make_unique < XMLFormatter > (new XMLFormatter(pretty_format));
    else if (format == "json")
        formatter =
            make_unique < JSONFormatter > (new JSONFormatter(pretty_format));
    else {
    ...
    zone_formatter =
        std::make_unique < JSONFormatter_PrettyZone > (pretty_format);
    ...
    if (!source_zone_name.empty()) {
        std::unique_ptr < rgw::sal::Zone > zone;
        if (driver->get_zone()->get_zonegroup().
            get_zone_by_name(source_zone_name, &zone) < 0) {
    ...
    if (raw_storage_op) {
        ...
        case OPT::PERIOD_DELETE:
            {
                ...
                if (ret < 0) {
                    cerr << "ERROR: couldn't delete period: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
        ...
        case OPT::PERIOD_GET:
            {
                ...
                if (staging) {
                    ...
                    if (ret < 0) {
                        cerr << "failed to load realm: " << cpp_strerror(-ret)
                            << std::endl;
                        return -ret;
                    }
                ...
                int ret = cfgstore->read_period(dpp(), null_yield, period_id,
                                                epoch, period);
                if (ret < 0) {
                    cerr << "failed to load period: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT::PERIOD_GET_CURRENT:
            {
                ...
                encode_json("current_period", realm.current_period,
                            formatter.get());
                formatter->close_section();
                formatter->flush(cout);
            }
        ...
        case OPT::PERIOD_LIST:
            {
                Formatter::ObjectSection periods_list {
                *formatter, "periods_list"};
                Formatter::ArraySection periods {
                *formatter, "periods"};
                rgw::sal::ListResult < std::string > listing;
                std::array < std::string, 1000 > period_ids;    // list in pages of 1000
                do {
                    int ret =
                        cfgstore->list_period_ids(dpp(), null_yield,
                                                  listing.next,
                                                  period_ids, listing);
                    if (ret < 0) {
                        std::
                            cerr << "failed to list periods: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
        ...
        case OPT::PERIOD_PULL:
            {
                ...
                if (url.empty()) {
                    ...
                    ret = cfgstore->read_period(dpp(), null_yield, period_id,
                                                std::nullopt, current_period);
                    if (ret < 0) {
                        cerr << "failed to load current period: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                    if (remote.empty()) {
                    ...
                    conn =
                        get_remote_conn(static_cast <
                                        rgw::sal::RadosStore * >(driver),
                                        current_period.get_map(), remote);
                    if (!conn) {
                        cerr << "failed to find a zone or zonegroup for remote "
                            << remote << std::endl;
                        return -ENOENT;
                ...
                int ret = do_period_pull(cfgstore.get(), remote_conn, url,
                                         opt_region, access_key, secret_key,
                                         realm_id, realm_name, period_id,
                                         period_epoch,
                                         &period);
                if (ret < 0) {
                    cerr << "period pull failed: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT::GLOBAL_RATELIMIT_DISABLE:
            {
                if (realm_id.empty()) {
                    if (!realm_name.empty()) {
                        ...
                        int ret = cfgstore->read_realm_id(dpp(), null_yield,
                                                          realm_name, realm_id);
                        if (ret < 0) {
                            cerr << "ERROR: failed to read realm for " <<
                                realm_name << ": " << cpp_strerror(-ret) <<
                                std::endl;
                            return -ret;
                        }
                    }
                        ...
                        int ret =
                            cfgstore->read_default_realm_id(dpp(), null_yield,
                                                            realm_id);
                ...
                int ret =
                    cfgstore->read_period_config(dpp(), null_yield, realm_id,
                                                 period_config);
                if (ret < 0 && ret != -ENOENT) {
                ...
                if (ratelimit_scope == "bucket") {
                    ratelimit_configured =
                        set_ratelimit_info(period_config.bucket_ratelimit,
                                           opt_cmd, max_read_ops, max_write_ops,
                                           max_read_bytes, max_write_bytes,
                                           have_max_read_ops,
                                           have_max_write_ops,
                                           have_max_read_bytes,
                                           have_max_write_bytes);
                    encode_json("bucket_ratelimit",
                                period_config.bucket_ratelimit,
                                formatter.get());
                }
                else if (ratelimit_scope == "user") {
                    ratelimit_configured =
                        set_ratelimit_info(period_config.user_ratelimit,
                                           opt_cmd, max_read_ops, max_write_ops,
                                           max_read_bytes, max_write_bytes,
                                           have_max_read_ops,
                                           have_max_write_ops,
                                           have_max_read_bytes,
                                           have_max_write_bytes);
                    encode_json("user_ratelimit", period_config.user_ratelimit,
                                formatter.get());
                }
                else if (ratelimit_scope == "anonymous") {
                    ratelimit_configured =
                        set_ratelimit_info(period_config.anon_ratelimit,
                                           opt_cmd, max_read_ops, max_write_ops,
                                           max_read_bytes, max_write_bytes,
                                           have_max_read_ops,
                                           have_max_write_ops,
                                           have_max_read_bytes,
                                           have_max_write_bytes);
                    encode_json("anonymous_ratelimit",
                                period_config.anon_ratelimit, formatter.get());
                }
                else if (ratelimit_scope.empty()
                         && opt_cmd == OPT::GLOBAL_RATELIMIT_GET) {
                ...
                if (!ratelimit_configured) {
                    cerr << "ERROR: no rate limit values have been specified" <<
                        std::endl;
                    return EINVAL;
                }
                ...
                if (opt_cmd != OPT::GLOBAL_RATELIMIT_GET) {
                    ...
                    constexpr bool exclusive = false;
                    ret =
                        cfgstore->write_period_config(dpp(), null_yield,
                                                      exclusive, realm_id,
                                                      period_config);
                    if (ret < 0) {
        ...
        case OPT::GLOBAL_QUOTA_DISABLE:
            {
                ...
                if (quota_scope == "bucket") {
                    set_quota_info(period_config.quota.bucket_quota, opt_cmd,
                                   ...
                                   have_max_size, have_max_objects);
                    encode_json("bucket quota",
                                period_config.quota.bucket_quota,
                                formatter.get());
                }
                    ...
                    set_quota_info(period_config.quota.user_quota, opt_cmd,
                                   max_size, max_objects,
                                   have_max_size, have_max_objects);
                    encode_json("user quota", period_config.quota.user_quota,
                                formatter.get());
                }
                else if (quota_scope.empty()
                         && opt_cmd == OPT::GLOBAL_QUOTA_GET) {
                    // if no scope is given for GET, print both
                    encode_json("bucket quota",
                                period_config.quota.bucket_quota,
                                formatter.get());
        ...
        case OPT::REALM_DELETE:
            {
                ...
                if (ret < 0) {
                    cerr << "failed to remove realm: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT::REALM_GET:
            {
                ...
                int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(),
                                          realm_id, realm_name, realm);
                if (ret < 0) {
                    if (ret == -ENOENT && realm_name.empty()
                        && realm_id.empty()) {
                        cerr <<
        ...
        case OPT::REALM_GET_DEFAULT:
            {
                ...
                if (ret == -ENOENT) {
                    ...
                    cerr << "Error reading default realm: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
        ...
        case OPT::REALM_LIST:
            {
                ...
                int ret = cfgstore->read_default_realm_id(dpp(), null_yield,
                                                          default_id);
                if (ret < 0 && ret != -ENOENT) {
                    cerr << "could not determine default realm: " <<
                        cpp_strerror(-ret) << std::endl;
                }

                Formatter::ObjectSection realms_list {
                *formatter, "realms_list"};
                encode_json("default_info", default_id, formatter.get());

                Formatter::ArraySection realms {
                *formatter, "realms"};
                rgw::sal::ListResult < std::string > listing;
                std::array < std::string, 1000 > names; // list in pages of 1000
                do {
                    ret =
                        cfgstore->list_realm_names(dpp(), null_yield,
                                                   listing.next, names,
                                                   listing);
                    if (ret < 0) {
                        std::
                            cerr << "failed to list realms: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
        ...
        case OPT::REALM_RENAME:
            {
                ...
                if (ret < 0) {
                    cerr << "rename failed: " << cpp_strerror(-ret) << std::
                        endl;
                    return -ret;
                }
                cout <<
        ...
        case OPT::REALM_SET:
            {
                ...
                int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(),
                                          realm_id, realm_name, realm, &writer);
                if (ret < 0 && ret != -ENOENT) {
                    cerr << "failed to init realm: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
                else if (ret == -ENOENT) {
                ...
                if (!realm_name.empty() && realm.get_name() != realm_name) {
                    cerr << "mismatch between --rgw-realm " << realm_name <<
                        " and json input file name " << realm.
                        get_name() << std::endl;
                    return EINVAL;
                }
                /* new realm */
                if (new_realm) {
                    cout << "clearing period and epoch for new realm" << std::
                        endl;
                    ...
                    ret = rgw::create_realm(dpp(), null_yield, cfgstore.get(),
                                            exclusive, realm);
                    if (ret < 0) {
                        cerr << "ERROR: couldn't create new realm: " <<
                            cpp_strerror(-ret) << std::endl;
                        return 1;
                    }
                }
                    ...
                    if (ret < 0) {
                        cerr << "ERROR: couldn't driver realm info: " <<
                            cpp_strerror(-ret) << std::endl;
                        return 1;
                    }
                }
        ...
        case OPT::REALM_DEFAULT:
            {
                ...
                ret =
                    rgw::set_default_realm(dpp(), null_yield, cfgstore.get(),
                                           realm);
                if (ret < 0) {
                    cerr << "failed to set realm as default: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
        ...
        case OPT::REALM_PULL:
            {
                ...
                JSONParser p;
                int ret =
                    send_to_url(url, opt_region, access_key, secret_key, info,
                                bl, p);
                if (ret < 0) {
                    cerr << "request failed: " << cpp_strerror(-ret) << std::
                        endl;
                    if (ret == -EACCES) {
                        cerr <<
                ...
                try {
                    ...
                    cerr << "failed to decode JSON response: " << e.
                        what() << std::endl;
                    return EINVAL;
                }
                RGWPeriod period;
                ...
                if (!current_period.empty()) {
                    ...
                    ret =
                        do_period_pull(cfgstore.get(), nullptr, url, opt_region,
                                       access_key, secret_key, realm_id,
                                       realm_name, current_period, "", &period);
                    if (ret < 0) {
                        cerr << "could not fetch period " << current_period <<
                            std::endl;
                        return -ret;
                    }
                }
        ...
        case OPT::ZONEGROUP_ADD:
            {
                ...
                if (ret < 0) {
                    cerr << "failed to load zonegroup " << zonegroup_name <<
                        " id " << zonegroup_id << ": " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
                ...
                ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(),
                                     ...
                                     &zone_writer);
                if (ret < 0) {
                    cerr << "unable to load zone: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
                ...
                if (zone_params.realm_id != zonegroup.realm_id) {
                    if (!zone_params.realm_id.empty()) {
                        cerr << "WARNING: overwriting zone realm_id=" <<
                            zone_params.
                            realm_id << " to match zonegroup realm_id=" <<
                            zonegroup.realm_id << std::endl;
                    }
              ...
              for (auto a:tier_config_add) {
                    ...
                    if (ret < 0) {
                        cerr << "ERROR: failed to set configurable: " << a <<
                            std::endl;
                        return EINVAL;
                    }
                    need_zone_update = true;
                ...
                const string *ptier_type =
                    (tier_type_specified ? &tier_type : nullptr);
                if (ptier_type) {
                    auto sync_mgr =
                        static_cast <
                        rgw::sal::RadosStore *
                        >(driver)->svc()->sync_modules->get_manager();
                    if (!sync_mgr->get_module(*ptier_type, nullptr)) {
                        ldpp_dout(dpp(),
                                  -1) << "ERROR: could not find sync module: "
                ...
                if (enable_features.empty()) {  // enable all features by default
                    enable_features.insert(rgw::zone_features::supported.
                                           begin(),
                                           rgw::zone_features::supported.end());
                }
                ...
                if (ret < 0) {
                    cerr << "failed to write updated zonegroup " << zonegroup.
                        get_name()
                        << ": " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
        ...
        case OPT::ZONEGROUP_DEFAULT:
            {
                ...
                ret =
                    rgw::set_default_zonegroup(dpp(), null_yield,
                                               cfgstore.get(), zonegroup);
                if (ret < 0) {
                    cerr << "failed to set zonegroup as default: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
        ...
        case OPT::ZONEGROUP_DELETE:
            {
                ...
                int ret = rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(),
                                              ...
                                              zonegroup, &writer);
                if (ret < 0) {
                    cerr << "failed to load zonegroup: " << cpp_strerror(-ret)
                        << std::endl;
                    return -ret;
                }
                ret = writer->remove(dpp(), null_yield);
                if (ret < 0) {
                    cerr << "ERROR: couldn't delete zonegroup: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
        ...
        case OPT::ZONEGROUP_LIST:
            {
                ...
                int ret = rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(),
                                              { }, { }, default_zonegroup);
                if (ret < 0 && ret != -ENOENT) {
                    cerr << "could not determine default zonegroup: " <<
                        cpp_strerror(-ret) << std::endl;
                }

                Formatter::ObjectSection zonegroups_list {
                *formatter, "zonegroups_list"};
                encode_json("default_info", default_zonegroup.id,
                            formatter.get());

                Formatter::ArraySection zonegroups {
                *formatter, "zonegroups"};
                rgw::sal::ListResult < std::string > listing;
                std::array < std::string, 1000 > names; // list in pages of 1000
                do {
                    ret =
                        cfgstore->list_zonegroup_names(dpp(), null_yield,
                                                       listing.next, names,
                                                       listing);
                    if (ret < 0) {
                        std::
                            cerr << "failed to list zonegroups: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                  for (const auto & name:listing.entries) {
        ...
        case OPT::ZONEGROUP_MODIFY:
            {
                ...
                if (!realm_id.empty()) {
                    ...
                    ret = cfgstore->read_realm_id(dpp(), null_yield, realm_name,
                                                  zonegroup.realm_id);
                    if (ret < 0) {
                        cerr << "failed to find realm by name " << realm_name <<
                            std::endl;
                        return -ret;
                    }
                    need_update = true;
              ...
              for (const auto & feature:disable_features) {
                ...
                if (need_update) {
                    ...
                    if (ret < 0) {
                        cerr << "failed to update zonegroup: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                }
        ...
        case OPT::ZONEGROUP_SET:
            {
                ...
                int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(),
                                          realm_id, realm_name, realm);
                bool default_realm_not_exist = (ret == -ENOENT
                                                && realm_id.empty()
                                                && realm_name.empty());
              ...
              for (const auto & feature:zonegroup.
                     enabled_features)
                {
              ...
              for (const auto &[name, zone]:zonegroup.zones) {
                ...
                ret = rgw::create_zonegroup(dpp(), null_yield, cfgstore.get(),
                                            exclusive, zonegroup);
                if (ret < 0) {
                    cerr << "ERROR: couldn't create zonegroup info: " <<
                        cpp_strerror(-ret) << std::endl;
                    return 1;
                }
        ...
        case OPT::ZONEGROUP_REMOVE:
            {
                ...
                if (zone_id.empty()) {
                    if (zone_name.empty()) {
                        cerr << "no --zone-id or --rgw-zone name provided" <<
                            std::endl;
                        return EINVAL;
                    }
                  ...
                  for (auto & z:zonegroup.zones) {
                    ...
                    if (zone_id.empty()) {
                        cerr << "zone name " << zone_name <<
                            " not found in zonegroup " << zonegroup.
                            get_name() << std::endl;
                        return ENOENT;
                    }
                }
                ...
                if (ret < 0) {
                    cerr << "failed to remove zone: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT::ZONEGROUP_RENAME:
            {
                ...
                ret =
                    writer->rename(dpp(), null_yield, zonegroup,
                                   zonegroup_new_name);
                if (ret < 0) {
        ...
        case OPT::ZONEGROUP_PLACEMENT_LIST:
            {
                ...
                encode_json("placement_targets", zonegroup.placement_targets,
                            formatter.get());
                formatter->flush(cout);
            }
            break;
        ...
        case OPT::ZONEGROUP_PLACEMENT_DEFAULT:
            {
                ...
                if (!rule.storage_class.empty() && opt_storage_class &&
                    ...
                    cerr <<
                        "ERROR: provided contradicting storage class configuration"
                        << std::endl;
                    return EINVAL;
                }
                  ...
                  for (auto & t:tags_add) {
                    ...
                    if (ptiter != target.tier_targets.end()) {
                        ...
                        if (tier_type == "cloud-s3") {
                            ...
                            cerr << "ERROR: Invalid tier-type specified" <<
                                std::endl;
                            return EINVAL;
                        }
                    }
                    ...
                    if (tier_class) {
                          ...
                          for (auto add:tier_config_add)
                            {
                                ...
                                if (r < 0) {
                                    cerr <<
                                        "ERROR: failed to set configurable: " <<
                                        add << std::endl;
                                    return EINVAL;
                                }
                            }
                            ...
                            if (r < 0) {
                                cerr <<
                                    "ERROR: failed to update tier_config options"
                                    << std::endl;
                            }
                        }
                        ...
                        target.tier_targets.
                            emplace(std::make_pair(storage_class, *pt));
                    }

                    if (zonegroup.default_placement.empty()) {
                        zonegroup.default_placement.init(rule.name,
                    ...
                    if (!opt_storage_class || opt_storage_class->empty()) {
                        ...
                        auto iter =
                            zonegroup.placement_targets.find(placement_id);
                        if (iter != zonegroup.placement_targets.end()) {
                            ...
                            if (zonegroup.default_placement == rule) {
                                ...
                                zonegroup.default_placement.storage_class.
                                    clear();
                            }

                            auto ptiter =
                                info.tier_targets.find(*opt_storage_class);
        ...
        case OPT::ZONE_CREATE:
            {
                ...
                if (!zonegroup_id.empty() || !zonegroup_name.empty()) {
                    int ret =
                        rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(),
                                            zonegroup_id, zonegroup_name,
                                            zonegroup, &zonegroup_writer);
                    if (ret < 0) {
                        cerr << "failed to load zonegroup " << zonegroup_name <<
              ...
              for (const auto & a:tier_config_add) {
                ...
                if (zone_params.realm_id.empty()) {
                    ...
                    cerr << "NOTICE: set zone's realm_id=" << realm.
                        id << std::endl;
                }
                ...
                if (zonegroup_writer) {
                    ...
                    if (ptier_type) {
                        ...
                        if (!sync_mgr->get_module(*ptier_type, nullptr)) {
                            ldpp_dout(dpp(),
                                      -1) <<
                                "ERROR: could not find sync module: " <<
                                *ptier_type << ",  valid sync modules: " <<
                                sync_mgr->
                    ...
                    // add/update the public zone information stored in the zonegroup
                    ret = rgw::add_zone_to_group(dpp(), zonegroup, zone_params,
                                                 pis_master, pread_only,
                                                 endpoints, ptier_type,
                                                 psync_from_all, sync_from,
                                                 sync_from_rm, predirect_zone,
                                                 bucket_index_max_shards,
                                                 enable_features,
                                                 disable_features);
                    if (ret < 0) {
                    ...
                    ret = zonegroup_writer->write(dpp(), null_yield, zonegroup);
                    if (ret < 0) {
                        cerr << "failed to add zone " << zone_name <<
                            " to zonegroup " << zonegroup.get_name()
                            << ": " << cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                }
                ...
                if (set_default) {
                    ret =
                        rgw::set_default_zone(dpp(), null_yield, cfgstore.get(),
                                              zone_params);
                    if (ret < 0) {
        ...
        case OPT::ZONE_DEFAULT:
            {
                ...
                ret = rgw::set_default_zone(dpp(), null_yield, cfgstore.get(),
                                            zone_params);
                if (ret < 0) {
                    cerr << "failed to set zone as default: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
        ...
        case OPT::ZONE_DELETE:
            {
                ...
                int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(),
                                         ...
                                         &writer);
                if (ret < 0) {
                    cerr << "failed to load zone: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT::ZONE_SET:
            {
                ...
                if (zone.realm_id.empty()) {
                    ...
                    cerr << "NOTICE: set zone's realm_id=" << zone.
                        realm_id << std::endl;
                }
                ...
                ret = rgw::create_zone(dpp(), null_yield, cfgstore.get(),
                                       exclusive, zone);
                if (ret < 0) {
                    cerr << "ERROR: couldn't create zone: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
        ...
        case OPT::ZONE_LIST:
            {
                ...
                int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(),
                                         { }, { }, default_zone_params);
                if (ret < 0 && ret != -ENOENT) {
                    cerr << "could not determine default zone: " <<
                        cpp_strerror(-ret) << std::endl;
                }

                Formatter::ObjectSection zones_list {
                *formatter, "zones_list"};
                encode_json("default_info", default_zone_params.id,
                            formatter.get());

                Formatter::ArraySection zones {
                *formatter, "zones"};
                rgw::sal::ListResult < std::string > listing;
                std::array < std::string, 1000 > names; // list in pages of 1000
                do {
                    ret =
                        cfgstore->list_zone_names(dpp(), null_yield,
                                                  listing.next, names, listing);
                    if (ret < 0) {
                        std::
                            cerr << "failed to list zones: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                  for (const auto & name:listing.entries) {
        ...
        case OPT::ZONE_RENAME:
            {
                ...
                ret =
                    zone_writer->rename(dpp(), null_yield, zone_params,
                                        zone_new_name);
                if (ret < 0) {
                    cerr << "failed to rename zone " << zone_name << " to " <<
                        zone_new_name << ": " << cpp_strerror(-ret)
                        << std::endl;
                    return -ret;
                }
                ...
                ret = rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(),
                                          ...
                                          zonegroup, &zonegroup_writer);
                if (ret < 0) {
                    cerr << "WARNING: failed to load zonegroup " <<
                        zonegroup_name << std::endl;
                    return EXIT_SUCCESS;
                }
        ...
        case OPT::ZONE_PLACEMENT_RM:
            {
                ...
                int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(),
                                         zone_id, zone_name, zone, &writer);
                if (ret < 0) {
                    cerr << "failed to init zone: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
                ...
                if (opt_cmd == OPT::ZONE_PLACEMENT_ADD ||
                    ...
                    string storage_class =
                        rgw_placement_rule::
                        get_canonical_storage_class(opt_storage_class.
                                                    value_or(string()));
                    ...
                    RGWZonePlacementInfo & info =
                        zone.placement_pools[placement_id];
                    ...
                    if (opt_data_pool.empty()) {
                        const RGWZoneStorageClass *porig_sc {
                        nullptr};
                        ...
                        if (opt_data_pool.empty()) {
                            cerr <<
                                "ERROR: data pool not configured, need to specify --data-pool"
                                << std::endl;
                            return EINVAL;
                        }
                    }
                    ...
                    if (ret < 0) {
                        cerr << "ERROR: the data extra (non-ec) pool '" << info.
                            get_data_extra_pool()
        ...
        case OPT::ZONE_PLACEMENT_LIST:
            {
                ...
                encode_json("placement_pools", zone.placement_pools,
                            formatter.get());
                formatter->flush(cout);
            }
            break;
    ...
    if (!op_mask_str.empty()) {
        ...
        if (ret < 0) {
            cerr << "failed to parse op_mask: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    ...
    if (!placement_id.empty()) {
        ...
        if (!driver->valid_placement(target_rule)) {
            cerr << "NOTICE: invalid dest placement: " << target_rule.
                to_str() << std::endl;
            return EINVAL;
        }
    ...
    case OPT::PERIOD_PUSH:
        {
            ...
            int ret = cfgstore->read_period(dpp(), null_yield, period_id,
                                            std::nullopt, period);
            if (ret < 0) {
                cerr << "failed to load period: " << cpp_strerror(-ret) << std::
                    endl;
                return -ret;
            }
            // json format into a bufferlist
            ...
            ret = send_to_remote_or_url(nullptr, url, opt_region,
                                        access_key, secret_key, info, bl, p);
            if (ret < 0) {
                cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
    ...
    case OPT::PERIOD_COMMIT:
        {
            ...
            int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(),
                                      ...
                                      realm, &realm_writer);
            if (ret < 0) {
                cerr << "Error initializing realm: " << cpp_strerror(-ret) <<
                    std::endl;
                return -ret;
            }
            ...
            RGWPeriod period;
            ret =
                cfgstore->read_period(dpp(), null_yield, period_id, epoch,
                                      period);
            if (ret < 0) {
                cerr << "failed to load period: " << cpp_strerror(-ret) << std::
                    endl;
            ...
            if (ret < 0) {
                cerr << "failed to commit period: " << cpp_strerror(-ret) <<
                    std::endl;
                return -ret;
            }
    ...
    case OPT::ROLE_CREATE:
        {
            ...
            if (assume_role_doc.empty()) {
                cerr << "ERROR: assume role policy document is empty" << std::
                    endl;
                return -EINVAL;
            }
            ...
            std::unique_ptr < rgw::sal::RGWRole > role =
                driver->get_role(role_name, tenant, path, assume_role_doc);
    ...
    case OPT::ROLE_DELETE:
        {
            ...
            std::unique_ptr < rgw::sal::RGWRole > role =
                driver->get_role(role_name, tenant);
            ret = role->delete_obj(dpp(), null_yield);
            if (ret < 0) {
                return -ret;
            }
            cout << "role: " << role_name << " successfully deleted" << std::
                endl;
            return 0;
        }
    case OPT::ROLE_GET:
    ...
    case OPT::ROLE_TRUST_POLICY_MODIFY:
        {
            ...
            cout <<
                "Assume role policy document updated successfully for role: " <<
                role_name << std::endl;
            return 0;
        }
    case OPT::ROLE_LIST:
        {
            ...
            ret =
                driver->get_roles(dpp(), null_yield, path_prefix, tenant,
                                  result);
            if (ret < 0) {
                return -ret;
            }
    ...
    case OPT::ROLE_POLICY_PUT:
        {
            ...
            try {
                ...
                cerr << "failed to parse perm policy: " << e.
                    what() << std::endl;
                return -EINVAL;
            }
    ...
    case OPT::ROLE_UPDATE:
        {
            ...
            cout << "Max session duration updated successfully for role: " <<
                role_name << std::endl;
            return 0;
        }
    default:
    ...
    if (opt_cmd == OPT::POLICY) {
        if (format == "xml") {
            int ret =
                RGWBucketAdminOp::dump_s3_policy(driver, bucket_op, cout,
                                                 dpp());
            if (ret < 0) {
            ...
            int ret =
                RGWBucketAdminOp::get_policy(driver, bucket_op, stream_flusher,
                                             dpp());
            if (ret < 0) {
                cerr << "ERROR: failed to get policy: " << cpp_strerror(-ret) <<
                    std::endl;
    ...
    if (opt_cmd == OPT::BUCKET_LIMIT_CHECK) {
        ...
        if (!rgw::sal::User::empty(user)) {
            ...
            ret =
                RGWBucketAdminOp::limit_check(driver, bucket_op, user_ids,
                                              stream_flusher, null_yield, dpp(),
                                              warnings_only);
        }
        else {
            ...
            ret =
                driver->meta_list_keys_init(dpp(), metadata_key, string(),
                                            &handle);
            if (ret < 0) {
                cerr <<
            ...
            do {
                ...
                if (ret < 0 && ret != -ENOENT) {
                    ...
                    ret =
                        RGWBucketAdminOp::limit_check(driver, bucket_op,
                                                      user_ids, stream_flusher,
                                                      null_yield, dpp(),
                                                      warnings_only);
                    if (ret < 0)
                        break;
    ...
    if (opt_cmd == OPT::BUCKETS_LIST) {
        if (bucket_name.empty()) {
            ...
            RGWBucketAdminOp::info(driver, bucket_op, stream_flusher,
                                   null_yield, dpp());
        }
        else {
            int ret =
                init_bucket(user.get(), tenant, bucket_name, bucket_id,
                            &bucket);
            if (ret < 0) {
                cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret)
                    << std::endl;
            ...
            do {
                ...
                ret =
                    bucket->list(dpp(), params,
                                 std::min(remaining, paginate_size), results,
                                 null_yield);
                if (ret < 0) {
                    cerr << "ERROR: driver->list_objects(): " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                ldpp_dout(dpp(), 20) << "INFO: " << __func__ <<
                    ": list() returned without error; results.objs.sizie()=" <<
                    results.objs.size() << "results.is_truncated=" << results.
                    is_truncated << ", marker=" << params.marker << dendl;
    ...
    if (opt_cmd == OPT::BUCKET_LAYOUT) {
        ...
        int ret =
            init_bucket(user.get(), tenant, bucket_name, bucket_id, &bucket);
        if (ret < 0) {
            return -ret;
        }
    ...
    if (opt_cmd == OPT::BUCKET_SHARD_OBJECTS) {
        ...
        if (specified_shard_id) {
            ...
            for (uint64_t ctr = 0, shardsleft = num_shards; shardsleft > 0;
                 ++ctr) {
    ...
    if (opt_cmd == OPT::LOG_LIST) {
        ...
        int r =
            static_cast <
            rgw::sal::RadosStore * >(driver)->getRados()->log_list_init(dpp(),
                                                                        date,
                                                                        &h);
        if (r == -ENOENT) {
            // no logs.
            ...
            while (true) {
                ...
                int r =
                    static_cast <
                    rgw::sal::RadosStore *
                    >(driver)->getRados()->log_list_next(h, &name);
                if (r == -ENOENT)
                    break;
                if (r < 0) {
                    cerr << "log list: error " << r << std::endl;
    ...
    if (opt_cmd == OPT::LOG_SHOW || opt_cmd == OPT::LOG_RM) {
        if (object.empty()
            ...
            cerr << "specify an object or a date, bucket and bucket-id" << std::
                endl;
            exit(1);
        }
        ...
        if (opt_cmd == OPT::LOG_SHOW) {
            ...
            int r =
                static_cast <
                rgw::sal::RadosStore *
                >(driver)->getRados()->log_show_init(dpp(), oid, &h);
            if (r < 0) {
            ...
            r = static_cast <
                rgw::sal::RadosStore *
                >(driver)->getRados()->log_show_next(dpp(), h, &entry);
            if (r < 0) {
        ...
        if (opt_cmd == OPT::LOG_RM) {
            int r =
                static_cast <
                rgw::sal::RadosStore * >(driver)->getRados()->log_remove(dpp(),
                                                                         oid);
            if (r < 0) {
    ...
    if (opt_cmd == OPT::POOL_ADD) {
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->zone->add_bucket_placement(dpp(), pool,
                                                         null_yield);
        if (ret < 0)
            cerr << "failed to add bucket placement: " << cpp_strerror(-ret) <<
                std::endl;
    }
    ...
    if (opt_cmd == OPT::POOL_RM) {
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->zone->remove_bucket_placement(dpp(), pool,
                                                            null_yield);
        if (ret < 0)
            cerr << "failed to remove bucket placement: " << cpp_strerror(-ret)
                << std::endl;
    }
    ...
    if (opt_cmd == OPT::POOLS_LIST) {
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->zone->list_placement_set(dpp(), pools,
                                                       null_yield);
        if (ret < 0) {
            cerr << "could not list placement set: " << cpp_strerror(-ret) <<
                std::endl;
            return -ret;
        }
        formatter->reset();
    ...
    if (opt_cmd == OPT::USAGE_SHOW) {
        ...
        ret =
            RGWUsage::show(dpp(), driver, user.get(), bucket.get(), start_epoch,
                           end_epoch, show_log_entries, show_log_sum,
                           &categories, stream_flusher);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::USAGE_TRIM) {
        if (rgw::sal::User::empty(user) && bucket_name.empty() &&
            ...
            cerr <<
                "usage trim without user/date/bucket specified will remove *all* users data"
                << std::endl;
        ...
        ret =
            RGWUsage::trim(dpp(), driver, user.get(), bucket.get(), start_epoch,
                           end_epoch);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::USAGE_CLEAR) {
        if (!yes_i_really_mean_it) {
            cerr <<
                "usage clear would remove *all* users usage data for all time"
                << std::endl;
            cerr << "do you really mean it? (requires --yes-i-really-mean-it)"
                << std::endl;
            return 1;
    ...
    if (opt_cmd == OPT::OLH_GET) {
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->getRados()->get_olh(dpp(),
                                                                  bucket->
                                                                  get_info(),
                                                                  obj, &olh);
        if (ret < 0) {
            cerr << "ERROR: failed reading olh: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::OLH_READLOG) {
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->getRados()->bucket_index_read_olh_log(dpp(),
                                                             bucket->get_info(),
                                                             *state,
                                                             obj->get_obj(), 0,
                                                             &log,
    ...
    if (opt_cmd == OPT::BI_GET) {
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->getRados()->bi_get(dpp(),
                                                                 bucket->
                                                                 get_info(),
                                                                 obj,
                                                                 bi_index_type,
    ...
    if (opt_cmd == OPT::BI_PUT) {
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->getRados()->bi_put(dpp(),
                                                                 bucket->
                                                                 get_key(), obj,
                                                                 entry);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::BI_LIST) {
        ...
        ldpp_dout(dpp(),
                  20) << "INFO: " << __func__ << ": max_entries=" << max_entries
        ...
        for (; i < max_shards; i++) {
            ldpp_dout(dpp(),
                      20) << "INFO: " << __func__ << ": starting shard=" << i <<
                dendl;

            RGWRados::BucketShard bs(static_cast <
                                     rgw::sal::RadosStore *
                                     >(driver)->getRados());
            ...
            do {
                ...
                ret =
                    static_cast <
                    rgw::sal::RadosStore * >(driver)->getRados()->bi_list(bs,
                                                                          object,
                                                                          marker,
                                                                          max_entries,
                                                                          &entries,
                                                                          &is_truncated);
                if (ret < 0) {
                    cerr << "ERROR: bi_list(): " << cpp_strerror(-ret) << std::
                        endl;
                    return -ret;
                }
                ldpp_dout(dpp(), 20) << "INFO: " << __func__ <<
    ...
    if (opt_cmd == OPT::BI_PURGE) {
        ...
        ret =
            init_bucket(user.get(), tenant, bucket_name, string(), &cur_bucket);
        if (ret == -ENOENT) {
            ...
            return -ret;
                 ...
                 !yes_i_really_mean_it) {
            cerr <<
                "specified bucket instance points to a current bucket instance"
                << std::endl;
            cerr << "do you really mean it? (requires --yes-i-really-mean-it)"
                << std::endl;
            return EINVAL;
        ...
        if (index.layout.type == rgw::BucketIndexType::Indexless) {
            cerr << "ERROR: indexless bucket has no index to purge" << std::
                endl;
            return EINVAL;
        }
        ...
        for (int i = 0; i < max_shards; i++) {
            ...
            ret =
                static_cast <
                rgw::sal::RadosStore * >(driver)->getRados()->bi_remove(dpp(),
                                                                        bs);
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::OBJECT_PUT) {
        ...
        int ret =
            data_access.get_bucket(dpp(), tenant, bucket_name, bucket_id, &b,
                                   null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::OBJECT_REWRITE) {
        ...
        if (min_rewrite_stripe_size > 0) {
            ret =
                check_min_obj_stripe_size(driver, obj.get(),
                                          min_rewrite_stripe_size,
                                          &need_rewrite);
            if (ret < 0) {
                ldpp_dout(dpp(),
                          0) << "WARNING: check_min_obj_stripe_size failed, r="
                    << ret << dendl;
            }
        }
        if (need_rewrite) {
            ...
            ret =
                store->rewrite_obj(bucket->get_info(), obj->get_obj(), dpp(),
                                   null_yield);
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::OBJECT_REINDEX) {
        ...
        if (object.empty() && objects_file.empty()) {
            ...
            cerr <<
                "ERROR: both --object and --objects-file specified and only one is allowed."
                << std::endl;
            return EINVAL;
        }
        else if (!objects_file.empty() && !object_version.empty()) {
            cerr <<
                "ERROR: cannot specify --object_version when --objects-file specified."
                << std::endl;
            return EINVAL;
        }
        ...
        auto process =
            ...
            std::unique_ptr < rgw::sal::Object > obj =
                bucket->get_object(p_object);
            obj->set_instance(p_object_version);
            ret =
                store->reindex_obj(bucket->get_info(), obj->get_obj(), dpp(),
                                   null_yield);
            if (ret < 0) {
                return ret;
            } return 0;
    ...
    if (opt_cmd == OPT::OBJECTS_EXPIRE) {
        if (!static_cast <
            rgw::sal::RadosStore *
            >(driver)->getRados()->process_expire_objects(dpp())) {
            cerr << "ERROR: process_expire_objects() processing returned error."
                << std::endl;
            return 1;
        }
    }
    ...
    if (opt_cmd == OPT::OBJECTS_EXPIRE_STALE_LIST) {
        ret =
            RGWBucketAdminOp::fix_obj_expiry(driver, bucket_op, stream_flusher,
                                             dpp(), true);
        if (ret < 0) {
            cerr << "ERROR: listing returned " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::OBJECTS_EXPIRE_STALE_RM) {
        ret =
            RGWBucketAdminOp::fix_obj_expiry(driver, bucket_op, stream_flusher,
                                             dpp(), false);
        if (ret < 0) {
            cerr << "ERROR: removing returned " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::BUCKET_REWRITE) {
        ...
        while (is_truncated) {
            ...
            const auto & current_index =
                bucket->get_info().layout.current_index;
            int r =
                static_cast <
                rgw::sal::RadosStore *
                >(driver)->getRados()->cls_bucket_list_ordered(dpp(),
                                                               bucket->
                                                               get_info(),
                                                               current_index,
                                                               RGW_NO_SHARD,
            ...
            for (auto iter = result.begin(); iter != result.end(); ++iter) {
                ...
                if ((entry.meta.size < min_rewrite_size) ||
                    ...
                    bool need_rewrite = true;
                    if (min_rewrite_stripe_size > 0) {
                        r = check_min_obj_stripe_size(driver, obj.get(),
                                                      min_rewrite_stripe_size,
                                                      &need_rewrite);
                        if (r < 0) {
                            ldpp_dout(dpp(),
                                      0) <<
                                "WARNING: check_min_obj_stripe_size failed, r="
                                << r << dendl;
                        }
                    }
                    if (!need_rewrite) {
                        ...
                        RGWRados *store =
                            static_cast <
                            rgw::sal::RadosStore * >(driver)->getRados();
                        r = store->rewrite_obj(bucket->get_info(),
                                               obj->get_obj(), dpp(),
                                               null_yield);
                        if (r == 0) {
    ...
    if (opt_cmd == OPT::BUCKET_RESHARD) {
        ...
        if (!zone_svc->can_reshard()) {
            ...
            std::cerr << "The zonegroup '" << zonegroup.
                get_name() << "' does not "
        ...
        if (!RGWBucketReshard::can_reshard(bucket->get_info(), zone_svc) &&
            ...
            std::cerr << "Bucket '" << bucket->
                get_name() << "' already has too many " "log generations (" <<
                bucket->get_info().layout.logs.
                size() << ") "
                ...
                "you can force a reshard with --yes-i-really-mean-it." << std::
                endl;
            return EINVAL;
        }
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
        ...
        if (inject_error_at) {
            ...
            fault.inject(*inject_error_at, InjectError {
                         code, dpp()});
        }
            ...
            fault.inject(*inject_delay_at, InjectDelay {
                         inject_delay, dpp()}
            );
        }
    ...
    if (opt_cmd == OPT::RESHARD_ADD) {
        ...
        RGWReshard reshard(static_cast < rgw::sal::RadosStore * >(driver),
                           dpp());
    ...
    if (opt_cmd == OPT::RESHARD_LIST) {
        ...
        for (int i = 0; i < num_logshards; i++) {
            ...
            do {
                ...
                ret =
                    reshard.list(dpp(), i, marker, max_entries - count, entries,
                                 &is_truncated);
                if (ret < 0) {
                    cerr << "Error listing resharding buckets: " <<
                        cpp_strerror(-ret) << std::endl;
                    return ret;
                }
              for (const auto & entry:entries) {
    ...
    if (opt_cmd == OPT::RESHARD_PROCESS) {
        RGWReshard reshard(static_cast < rgw::sal::RadosStore * >(driver), true,
                           &cout);
            ...
            if (ret < 0) {
                if (ret == -EBUSY) {
                    cerr << "There is ongoing resharding, please retry after "
                        ...
                        ("rgw_reshard_bucket_lock_duration") << " seconds." <<
                        std::endl;
                    return -ret;
                }
                else if (ret == -EINVAL) {
    ...
    if (opt_cmd == OPT::OBJECT_STAT) {
        ...
        for (iter = obj->get_attrs().begin(); iter != obj->get_attrs().end();
            ...
            if (iter->first == RGW_ATTR_MANIFEST) {
                handled =
                    decode_dump < RGWObjManifest > ("manifest", bl,
                                                    formatter.get());
            }
            else if (iter->first == RGW_ATTR_ACL) {
                handled =
                    decode_dump < RGWAccessControlPolicy > ("policy", bl,
                                                            formatter.get());
            }
                ...
                handled =
                    decode_dump < RGWCompressionInfo > ("compression", bl,
                                                        formatter.get());
            }
            else if (iter->first == RGW_ATTR_DELETE_AT) {
                handled =
                    decode_dump < utime_t > ("delete_at", bl, formatter.get());
            }
    ...
    if (opt_cmd == OPT::BUCKET_CHECK) {
        if (check_head_obj_locator) {
            ...
            do_check_object_locator(tenant, bucket_name, fix, remove_bad,
                                    formatter.get());
        }
        else {
            RGWBucketAdminOp::check_index(driver, bucket_op, stream_flusher,
                                          null_yield, dpp());
        }
    }
    ...
    if (opt_cmd == OPT::BUCKET_RM) {
        if (!inconsistent_index) {
            RGWBucketAdminOp::remove_bucket(driver, bucket_op, null_yield,
                                            dpp(), bypass_gc, true);
        }
        else {
            if (!yes_i_really_mean_it) {
    ...
    if (opt_cmd == OPT::GC_LIST) {
        ...
        do {
            ...
            int ret =
                static_cast <
                rgw::sal::RadosStore *
                >(driver)->getRados()->list_gc_objs(&index, marker, 1000,
                                                    !include_all, result,
                                                    &truncated,
                                                    processing_queue);
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::GC_PROCESS) {
        int ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->getRados()->process_gc(!include_all);
        if (ret < 0) {
            cerr << "ERROR: gc processing returned error: " <<
                cpp_strerror(-ret) << std::endl;
            return 1;
        }
    }
    ...
    if (opt_cmd == OPT::LC_LIST) {
        ...
        vector < std::unique_ptr <
            rgw::sal::Lifecycle::LCEntry >> bucket_lc_map;
        string marker;
        int index {
        0};
#define MAX_LC_LIST_ENTRIES 100
        ...
        do {
            int ret =
                static_cast <
                rgw::sal::RadosStore *
                >(driver)->getRados()->list_lc_progress(marker, max_entries,
                                                        bucket_lc_map, index);
            if (ret < 0) {
                cerr << "ERROR: failed to list objs: " << cpp_strerror(-ret)
                    << std::endl;
    ...
    if (opt_cmd == OPT::LC_PROCESS) {
        if ((!bucket_name.empty()) || (!bucket_id.empty())) {
            int ret =
                init_bucket(nullptr, tenant, bucket_name, bucket_id, &bucket);
            if (ret < 0) {
                cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret)
                    << std::endl;
                return ret;
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->getRados()->process_lc(bucket);
        if (ret < 0) {
            cerr << "ERROR: lc processing returned error: " <<
                cpp_strerror(-ret) << std::endl;
            return 1;
        }
    }
    ...
    if (opt_cmd == OPT::LC_RESHARD_FIX) {
        ret =
            RGWBucketAdminOp::fix_lc_shards(driver, bucket_op, stream_flusher,
                                            dpp());
        if (ret < 0) {
            cerr << "ERROR: fixing lc shards: " << cpp_strerror(-ret) << std::
                endl;
        }
    ...
    if (opt_cmd == OPT::ORPHANS_FIND) {
        ...
        RGWOrphanSearch search(static_cast < rgw::sal::RadosStore * >(driver),
                               max_concurrent_ios, orphan_stale_secs);
    ...
    if (opt_cmd == OPT::ORPHANS_LIST_JOBS) {
        ...
        RGWOrphanStore orphan_store(static_cast <
                                    rgw::sal::RadosStore * >(driver));
    ...
    if (opt_cmd == OPT::USER_CHECK) {
        check_bad_user_bucket_mapping(driver, *user.get(), fix, null_yield,
                                      dpp());
    }
    ...
    if (opt_cmd == OPT::USER_STATS) {
        ...
        if (reset_stats) {
            ...
            if (sync_stats) {
                cerr <<
                    ...
                    "so at most one of the two should be specified" << std::
                    endl;
                return EINVAL;
            }
            ret =
                static_cast <
                rgw::sal::RadosStore *
                >(driver)->svc()->user->reset_bucket_stats(dpp(),
                                                           user->get_id(),
                                                           null_yield);
            if (ret < 0) {
                cerr << "ERROR: could not reset user stats: " <<
                    cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        }
            ...
            if (!bucket_name.empty()) {
                int ret =
                    init_bucket(user.get(), tenant, bucket_name, bucket_id,
                                &bucket);
                if (ret < 0) {
                    cerr << "ERROR: could not init bucket: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                ...
                int ret =
                    rgw_user_sync_all_stats(dpp(), driver, user.get(),
                                            null_yield);
                if (ret < 0) {
        ...
        if (ret < 0) {
            if (ret == -ENOENT) {   /* in case of ENOENT */
                ...
                cerr << "ERROR: can't read user: " << cpp_strerror(ret) << std::
                    endl;
            }
            return -ret;
        }
    ...
    if (opt_cmd == OPT::METADATA_GET) {
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->ctl()->meta.mgr->get(metadata_key,
                                                                   formatter.
                                                                   get(),
                                                                   null_yield,
                                                                   dpp());
    ...
    if (opt_cmd == OPT::METADATA_PUT) {
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->ctl()->meta.mgr->put(metadata_key,
                                                                   bl,
                                                                   null_yield,
                                                                   dpp(),
                                                                   RGWMDLogSyncType::
    ...
    if (opt_cmd == OPT::METADATA_RM) {
        int ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->ctl()->meta.mgr->remove(metadata_key, null_yield, dpp());
        if (ret < 0) {
            cerr << "ERROR: can't remove key: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::METADATA_LIST || opt_cmd == OPT::USER_LIST) {
        ...
        int max = 1000;
        int ret =
            driver->meta_list_keys_init(dpp(), metadata_key, marker, &handle);
        if (ret < 0) {
            cerr << "ERROR: can't get key: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
        ...
        do {
            ...
            ret =
                driver->meta_list_keys_next(dpp(), handle, left, keys,
                                            &truncated);
            if (ret < 0 && ret != -ENOENT) {
                cerr << "ERROR: lists_keys_next(): " << cpp_strerror(-ret) <<
                    std::endl;
                return -ret;
            }
            if (ret != -ENOENT) {
        ...
        if (max_entries_specified) {
            ...
            if (truncated) {
                encode_json("marker", driver->meta_get_marker(handle),
                            formatter.get());
            }
            formatter->close_section();
        }
    ...
    if (opt_cmd == OPT::MDLOG_LIST) {
        ...
        if (!start_marker.empty()) {
            if (marker.empty()) {
                ...
                std::
                    cerr << "start-marker and marker not both allowed." << std::
                    endl;
                return -EINVAL;
            }
        }
        ...
        if (period_id.empty()) {
            ...
            int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(),
                                      realm_id, realm_name, realm);
            if (ret < 0) {
                cerr << "failed to load realm: " << cpp_strerror(-ret) << std::
                    endl;
                return -ret;
            }
            period_id = realm.current_period;
        ...
        RGWMetadataLog *meta_log =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->mdlog->get_log(period_id);
            ...
            do {
                int ret =
                    meta_log->list_entries(dpp(), handle, 1000, entries, NULL,
                                           &truncated);
                if (ret < 0) {
                    cerr << "ERROR: meta_log->list_entries(): " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                ...
                for (list < cls_log_entry >::iterator iter = entries.begin();
                    ...
                    static_cast <
                        rgw::sal::RadosStore *
                        >(driver)->ctl()->meta.mgr->dump_log_entry(entry,
                                                                   formatter.
                                                                   get());
                }
                formatter->flush(cout);
    ...
    if (opt_cmd == OPT::MDLOG_AUTOTRIM) {
        ...
        auto mltcr =
            create_admin_meta_log_trim_cr(dpp(),
                                          static_cast <
                                          rgw::sal::RadosStore * >(driver),
                                          &http, num_shards);
        if (!mltcr) {
    ...
    if (opt_cmd == OPT::MDLOG_TRIM) {
        ...
        if (!end_marker.empty()) {
            if (marker.empty()) {
                ...
                std::cerr << "end-marker and marker not both allowed." << std::
                    endl;
                return -EINVAL;
            }
        }
        ...
        if (marker.empty()) {
            cerr << "ERROR: marker must be specified for trim operation" <<
                std::endl;
            return EINVAL;
        }
        ...
        // trim until -ENODATA
        do {
            ret = meta_log->trim(dpp(), shard_id, {
                                 }
                                 , {
                                 }
                                 , {
        ...
        if (ret < 0 && ret != -ENODATA) {
            cerr << "ERROR: meta_log->trim(): " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::DATA_SYNC_STATUS) {
        ...
        if (specified_shard_id) {
            ...
            ret =
                sync.read_shard_status(dpp(), shard_id, pending_buckets,
                                       recovering_buckets, &sync_marker,
                                       max_entries_specified ? max_entries :
                                       20);
            if (ret < 0 && ret != -ENOENT) {
            ...
            encode_json("recovering_buckets", recovering_buckets,
                        formatter.get());
            formatter->dump_string("current_time",
                                   to_iso_8601(ceph::real_clock::now(),
                                               iso_8601_format::YMDhms));
    ...
    if (opt_cmd == OPT::DATA_SYNC_RUN) {
        ...
        int ret =
            ...
            rgw::sal::RadosStore *
            >(driver)->svc()->sync_modules->get_manager()->
            create_instance(dpp(), g_ceph_context,
                            static_cast <
                            rgw::sal::RadosStore *
                            >(driver)->svc()->zone->get_zone().tier_type,
                            static_cast <
                            rgw::sal::RadosStore *
                            >(driver)->svc()->zone->get_zone_params().
                            tier_config, &sync_module);
        if (ret < 0) {
            ldpp_dout(dpp(),
                      -1) << "ERROR: failed to init sync module instance, ret="
                << ret << dendl;
            return ret;
        }
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_INIT) {
        ...
        int ret =
            init_bucket_for_sync(user.get(), tenant, bucket_name, bucket_id,
                                 &bucket);
        if (ret < 0) {
            return -ret;
        }
        ...
        if (opt_sb && opt_sb->bucket_id.empty()) {
            ...
            int ret =
                init_bucket_for_sync(user.get(), opt_sb->tenant, opt_sb->name,
                                     sbid, &sbuck);
            if (ret < 0) {
                return -ret;
            }
        ...
        auto sync =
            RGWBucketPipeSyncStatusManager::construct(dpp(),
                                                      static_cast <
                                                      rgw::sal::RadosStore *
                                                      >(driver), source_zone,
                                                      opt_sb,
        ...
        if (!sync) {
            cerr << "ERROR: sync.init() returned error=" << sync.
                error() << std::endl;
            return -sync.error();
        }
        ret = (*sync)->init_sync_status(dpp());
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_CHECKPOINT) {
        ...
        if (!static_cast <
            rgw::sal::RadosStore *
            >(driver)->ctl()->bucket->bucket_imports_data(bucket->get_key(),
                                                          null_yield, dpp())) {
            std::cout << "Sync is disabled for bucket " << bucket_name << std::
                endl;
            return 0;
        }
        ...
        RGWBucketSyncPolicyHandlerRef handler;
        ret =
            driver->get_sync_policy_handler(dpp(), std::nullopt,
                                            bucket->get_key(), &handler,
                                            null_yield);
        if (ret < 0) {
            std::cerr << "ERROR: failed to get policy handler for bucket ("
                << bucket << "): r=" << ret << ": " << cpp_strerror(-ret) <<
                std::endl;
            return -ret;
        }
        ...
        ret =
            rgw_bucket_sync_checkpoint(dpp(),
                                       static_cast <
                                       rgw::sal::RadosStore * >(driver),
                                       *handler, bucket->get_info(),
                                       opt_source_zone, opt_source_bucket,
                                       opt_retry_delay_ms, timeout_at);
        if (ret < 0) {
            ldpp_dout(dpp(),
                      -1) << "bucket sync checkpoint failed: " <<
                cpp_strerror(ret) << dendl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_STATUS) {
        ...
        bucket_sync_status(driver, bucket->get_info(), source_zone,
                           opt_source_bucket, std::cout);
    }
    ...
    if (opt_cmd == OPT::BILOG_LIST) {
        ...
        if (gen) {
            auto i =
                std::find_if(logs.begin(), logs.end(), rgw::matches_gen(*gen));
            if (i == logs.end()) {
        ...
        do {
            ...
            ret =
                static_cast <
                rgw::sal::RadosStore *
                >(driver)->svc()->bilog_rados->log_list(dpp(),
                                                        bucket->get_info(),
                                                        log_layout, shard_id,
                                                        marker,
                                                        max_entries - count,
    ...
    if (opt_cmd == OPT::SYNC_ERROR_LIST) {
        ...
        for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
            ...
            string oid =
                RGWSyncErrorLogger::
                get_shard_oid(RGW_SYNC_ERROR_LOG_SHARD_PREFIX, shard_id);
            ...
            do {
                ...
                ret =
                    static_cast <
                    rgw::sal::RadosStore *
                    >(driver)->svc()->cls->timelog.list(dpp(), oid, {
                                                        }
                                                        , {
                                                        }
                                                        , max_entries - count,
                ...
                if (ret < 0) {
                    cerr << "ERROR: svc.cls->timelog.list(): " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
              ...
              for (auto & cls_entry:entries) {
                    ...
                    try {
                        ...
                        cerr << "ERROR: failed to decode log entry" << std::
                            endl;
                        continue;
                    }
                    formatter->open_object_section("entry");
                    ...
                    encode_json("timestamp", cls_entry.timestamp,
                                formatter.get());
    ...
    if (opt_cmd == OPT::SYNC_ERROR_TRIM) {
        ...
        for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
            ...
            if (ret < 0) {
                cerr << "ERROR: sync error trim: " << cpp_strerror(-ret) <<
                    std::endl;
                return -ret;
            }
            if (specified_shard_id) {
    ...
    if (opt_cmd == OPT::SYNC_GROUP_CREATE || opt_cmd == OPT::SYNC_GROUP_MODIFY) {
        CHECK_TRUE(require_opt(opt_group_id), "ERROR: --group-id not specified",
                   EINVAL);
        CHECK_TRUE(require_opt(opt_status),
                   "ERROR: --status is not specified (options: forbidden, allowed, enabled)",
                   EINVAL);
        ...
        if (opt_status) {
            if (!group.set_status(*opt_status)) {
                cerr <<
                    "ERROR: unrecognized status (options: forbidden, allowed, enabled)"
                    << std::endl;
                return EINVAL;
            }
        }
    ...
    if (opt_cmd == OPT::SYNC_GROUP_FLOW_CREATE) {
        CHECK_TRUE(require_opt(opt_group_id), "ERROR: --group-id not specified",
                   EINVAL);
        CHECK_TRUE(require_opt(opt_flow_id), "ERROR: --flow-id not specified",
                   EINVAL);
        ...
        if (symmetrical_flow_opt(*opt_flow_type)) {
            ...
            group.data_flow.find_or_create_symmetrical(*opt_flow_id,
                                                       &flow_group);
          ...
          for (auto & z:*opt_zone_ids) {
            ...
            group.data_flow.find_or_create_directional(*opt_source_zone_id,
                                                       *opt_dest_zone_id,
                                                       &flow_rule);
        }
    ...
    if (opt_cmd == OPT::SYNC_GROUP_FLOW_REMOVE) {
        ...
        if (symmetrical_flow_opt(*opt_flow_type)) {
            ...
            group.data_flow.remove_directional(*opt_source_zone_id,
                                               *opt_dest_zone_id);
        }
    ...
    if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE ||
        ...
        CHECK_TRUE(require_opt(opt_group_id), "ERROR: --group-id not specified",
                   EINVAL);
        CHECK_TRUE(require_opt(opt_pipe_id), "ERROR: --pipe-id not specified",
                   EINVAL);
        if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE) {
        ...
        if (opt_mode) {
            if (*opt_mode == "system") {
                ...
                cerr <<
                    "ERROR: bad mode value: should be one of the following: system, user"
                    << std::endl;
                return EINVAL;
            }
        }
    ...
    if (opt_cmd == OPT::SYNC_GROUP_PIPE_REMOVE) {
        ...
        if (!(opt_source_zone_ids ||
              ...
              opt_source_bucket_id ||
              opt_dest_zone_ids ||
              opt_dest_tenant || opt_dest_bucket || opt_dest_bucket_id)) {
    ...
    if (opt_cmd == OPT::BILOG_STATUS) {
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->bilog_rados->get_log_status(dpp(),
                                                          bucket->get_info(),
                                                          log_layout, shard_id,
                                                          &markers, null_yield);
        if (ret < 0) {
            cerr << "ERROR: get_bi_log_status(): " << cpp_strerror(-ret) <<
                std::endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::BILOG_AUTOTRIM) {
        ...
        rgw::BucketTrimManager trim(static_cast <
                                    rgw::sal::RadosStore * >(driver), config);
        ret = trim.init();
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATALOG_LIST) {
        ...
        auto datalog_svc =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->datalog_rados;
        ...
        do {
            ...
            if (specified_shard_id) {
                ret =
                    datalog_svc->list_entries(dpp(), shard_id,
                ...
                ret =
                    datalog_svc->list_entries(dpp(), max_entries - count,
                                              entries, log_marker, &truncated,
                                              null_yield);
            }
            if (ret < 0) {
                cerr << "ERROR: datalog_svc->list_entries(): " <<
                    cpp_strerror(-ret) << std::endl;
                return -ret;
            }
    ...
    if (opt_cmd == OPT::DATALOG_AUTOTRIM) {
        ...
        ret =
            crs.run(dpp(),
                    create_admin_data_log_trim_cr(dpp(),
                                                  static_cast <
                                                  rgw::sal::RadosStore *
                                                  >(driver), &http, num_shards,
                                                  markers));
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATALOG_TRIM) {
        ...
        if (ret < 0 && ret != -ENODATA) {
            cerr << "ERROR: trim_entries(): " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::DATALOG_TYPE) {
        ...
        if (ret < 0) {
            cerr << "ERROR: change_format(): " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::DATALOG_PRUNE) {
        ...
        if (ret < 0) {
            cerr << "ERROR: trim_generations(): " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }

        if (through) {
            std::cout << "Pruned " << *through << " empty generations." << std::
                endl;
        }
        else {
    ...
    bool quota_op = (opt_cmd == OPT::QUOTA_SET || opt_cmd == OPT::QUOTA_ENABLE
                     || opt_cmd == OPT::QUOTA_DISABLE);

    if (quota_op) {
        if (bucket_name.empty() && rgw::sal::User::empty(user)) {
            cerr << "ERROR: bucket name or uid is required for quota operation"
                << std::endl;
            return EINVAL;
        }
        ...
        if (!bucket_name.empty()) {
            if (!quota_scope.empty() && quota_scope != "bucket") {
                cerr << "ERROR: invalid quota scope specification." << std::
                    endl;
                return EINVAL;
            }
            set_bucket_quota(driver, opt_cmd, tenant, bucket_name,
                             max_size, max_objects, have_max_size,
                             have_max_objects);
        }
        else if (!rgw::sal::User::empty(user)) {
            if (quota_scope == "bucket") {
                return set_user_bucket_quota(opt_cmd, ruser, user_op, max_size,
                                             max_objects, have_max_size,
                                             have_max_objects);
            }
            else if (quota_scope == "user") {
                return set_user_quota(opt_cmd, ruser, user_op, max_size,
                                      max_objects, have_max_size,
                                      have_max_objects);
            }
            else {
                cerr <<
                    "ERROR: invalid quota scope specification. Please specify either --quota-scope=bucket, or --quota-scope=user"
                    << std::endl;
                return EINVAL;
            }
        }
    ...
    bool ratelimit_op_set = (opt_cmd == OPT::RATELIMIT_SET
                             || opt_cmd == OPT::RATELIMIT_ENABLE
                             || opt_cmd == OPT::RATELIMIT_DISABLE);
    bool ratelimit_op_get = opt_cmd == OPT::RATELIMIT_GET;
    if (ratelimit_op_set) {
        if (bucket_name.empty() && rgw::sal::User::empty(user)) {
            cerr <<
                "ERROR: bucket name or uid is required for ratelimit operation"
                << std::endl;
            return EINVAL;
        }
        ...
        if (!bucket_name.empty()) {
            if (!ratelimit_scope.empty() && ratelimit_scope != "bucket") {
                cerr <<
                    "ERROR: invalid ratelimit scope specification. (bucket scope is not bucket but bucket has been specified)"
                    << std::endl;
                return EINVAL;
            }
        ...
        if (ratelimit_scope == "user") {
            return set_user_ratelimit(opt_cmd, user, max_read_ops,
                                      max_write_ops, max_read_bytes,
                                      max_write_bytes, have_max_read_ops,
                                      have_max_write_ops, have_max_read_bytes,
                                      have_max_write_bytes);
        }
        else {
            cerr <<
                "ERROR: invalid ratelimit scope specification. Please specify either --ratelimit-scope=bucket, or --ratelimit-scope=user"
                << std::endl;
            return EINVAL;
        }
    }
    ...
    if (ratelimit_op_get) {
        ...
        if (!bucket_name.empty()) {
            ...
            return show_bucket_ratelimit(driver, tenant, bucket_name,
                                         formatter.get());
        }
        else if (!rgw::sal::User::empty(user)) {
        }
    ...
    if (opt_cmd == OPT::MFA_CREATE) {
        ...
        if (totp_serial.empty()) {
            cerr <<
                "ERROR: TOTP device serial number was not provided (via --totp-serial)"
                << std::endl;
            return EINVAL;
        }
        ...
        string oid =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->cls->mfa.get_mfa_oid(user->get_id());

        int ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->ctl()->meta.mgr->
            mutate(RGWSI_MetaBackend_OTP::get_meta_key(user->get_id()),
                   mtime, &objv_tracker,
                   null_yield, dpp(),
                   MDLOG_STATUS_WRITE,
                   [&]{
                   return static_cast <
                   rgw::sal::RadosStore *
                   >(driver)->svc()->cls->mfa.create_mfa(dpp(), user->get_id(),
                                                         config, &objv_tracker,
                                                         mtime, null_yield);}
        );
        if (ret < 0) {
            cerr << "MFA creation failed, error: " << cpp_strerror(-ret) <<
                std::endl;
            return -ret;
        }
        ...
        if (ret < 0) {
            cerr << "ERROR: failed storing user info, error: " << err << std::
                endl;
            return -ret;
        }
    }
    ...
    if (opt_cmd == OPT::MFA_REMOVE) {
        ...
        int ret =
            ...
            mutate(RGWSI_MetaBackend_OTP::get_meta_key(user->get_id()),
                   ...
                   return static_cast <
                   rgw::sal::RadosStore *
                   >(driver)->svc()->cls->mfa.remove_mfa(dpp(), user->get_id(),
                                                         totp_serial,
                                                         &objv_tracker, mtime,
                                                         null_yield);}
        );
        if (ret < 0) {
            cerr << "MFA removal failed, error: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::MFA_GET) {
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->cls->mfa.get_mfa(dpp(),
                                                                      user->
                                                                      get_id(),
                                                                      totp_serial,
                                                                      &result,
    ...
    if (opt_cmd == OPT::MFA_LIST) {
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->cls->mfa.list_mfa(dpp(),
                                                                       user->
                                                                       get_id(),
                                                                       &result,
                                                                       null_yield);
        if (ret < 0 && ret != -ENOENT) {
            cerr << "MFA listing failed, error: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
        formatter->open_object_section("result");
    ...
    if (opt_cmd == OPT::MFA_CHECK) {
        ...
        if (totp_pin.empty()) {
            cerr <<
                "ERROR: TOTP device serial number was not provided (via --totp-pin)"
                << std::endl;
            return EINVAL;
        }
        ...
        int ret =
            static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->cls->mfa.check_mfa(dpp(),
                                                                        user->
                                                                        get_id
                                                                        (),
                                                                        totp_serial,
                                                                        ...
                                                                        null_yield);
        if (ret < 0) {
            cerr << "MFA check failed, error: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::MFA_RESYNC) {
        ...
        if (totp_pin.size() != 2) {
            cerr <<
                "ERROR: missing two --totp-pin params (--totp-pin=<first> --totp-pin=<second>)"
                << std::endl;
            return EINVAL;
        }
        ...
        ret =
            static_cast <
            rgw::sal::RadosStore *
            >(driver)->svc()->cls->mfa.otp_get_current_time(dpp(),
                                                            user->get_id(),
                                                            &now, null_yield);
        if (ret < 0) {
        ...
        if (ret < 0) {
            if (ret == -ENOENT) {
                cerr << "failed to resync, TOTP values not found in range" <<
                    std::endl;
            }
            else {
                cerr << "ERROR: failed to scan for TOTP values: " <<
                    cpp_strerror(-ret) << std::endl;
            }
            return -ret;
        }
        ...
        ret =
            ...
            rgw::sal::RadosStore *
            >(driver)->ctl()->meta.mgr->
            mutate(RGWSI_MetaBackend_OTP::get_meta_key(user->get_id()), mtime,
                   &objv_tracker, null_yield, dpp(), MDLOG_STATUS_WRITE,[&] {
                   return static_cast <
                   rgw::sal::RadosStore *
                   >(driver)->svc()->cls->mfa.create_mfa(dpp(), user->get_id(),
                                                         config, &objv_tracker,
        ...
        if (ret < 0) {
            cerr << "MFA update failed, error: " << cpp_strerror(-ret) << std::
                endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_LIST) {
        if (!static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->zone->can_reshard()
            && !yes_i_really_mean_it) {
            cerr <<
                "Resharding disabled in a multisite env, stale instances unlikely from resharding"
                << std::endl;
        ...
        ret =
            RGWBucketAdminOp::list_stale_instances(driver, bucket_op,
                                                   stream_flusher, dpp());
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_DELETE) {
        if (!static_cast <
            rgw::sal::RadosStore * >(driver)->svc()->zone->can_reshard()) {
            cerr <<
                "Resharding disabled in a multisite env. Stale instances are not safe to be deleted."
                << std::endl;
            return EINVAL;
        }

        ret =
            RGWBucketAdminOp::clear_stale_instances(driver, bucket_op,
                                                    stream_flusher, dpp());
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::PUBSUB_TOPIC_GET) {
        if (topic_name.empty()) {
            cerr << "ERROR: topic name was not provided (via --topic)" << std::
                endl;
            return EINVAL;
        }
        ...
        if (ret < 0) {
            cerr << "ERROR: could not get topic: " << cpp_strerror(-ret) <<
                std::endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::SCRIPT_PUT) {
        if (!str_script_ctx) {
            cerr << "ERROR: context was not provided (via --context)" << std::
                endl;
            return EINVAL;
        }
        if (infile.empty()) {
            cerr << "ERROR: infile was not provided (via --infile)" << std::
                endl;
            return EINVAL;
        }
        bufferlist bl;
        ...
        if (rc < 0) {
            cerr << "ERROR: failed to read script: '" << infile << "'. error: "
                << rc << std::endl;
            return -rc;
        }
        ...
        if (!rgw::lua::verify(script, err_msg)) {
            cerr << "ERROR: script: '" << infile << "' has error: " << std::
                endl << err_msg << std::endl;
            return EINVAL;
        }
        ...
        if (script_ctx == rgw::lua::context::background && !tenant.empty()) {
            cerr << "ERROR: cannot specify tenant in background context" <<
                std::endl;
            return EINVAL;
        }
        auto lua_manager = driver->get_lua_manager();
        rc = rgw::lua::write_script(dpp(), lua_manager.get(), tenant,
    ...
    if (opt_cmd == OPT::SCRIPT_GET) {
        ...
        std::string script;
        const auto rc =
            rgw::lua::read_script(dpp(), lua_manager.get(), tenant, null_yield,
                                  script_ctx, script);
        if (rc == -ENOENT) {
    ...
    if (opt_cmd == OPT::SCRIPT_RM) {
        ...
        auto lua_manager = driver->get_lua_manager();
        const auto rc =
            rgw::lua::delete_script(dpp(), lua_manager.get(), tenant,
                                    null_yield, script_ctx);
        if (rc < 0) {
            cerr << "ERROR: failed to remove script. error: " << rc << std::
                endl;
            return -rc;
        }
    }
    ...
    if (opt_cmd == OPT::SCRIPT_PACKAGE_ADD) {
        ...
        const auto rc =
            rgw::lua::add_package(dpp(), driver, null_yield, *script_package,
    ...
    if (opt_cmd == OPT::SCRIPT_PACKAGE_RM) {
        ...
        const auto rc =
            rgw::lua::remove_package(dpp(), driver, null_yield,
                                     *script_package);
        if (rc == -ENOENT) {
    ...
    if (opt_cmd == OPT::SCRIPT_PACKAGE_LIST) {
        ...
        const auto rc =
            rgw::lua::list_packages(dpp(), driver, null_yield, packages);
        if (rc == -ENOENT) {
            std::cout << "no lua packages in allowlist" << std::endl;
        }
        else if (rc < 0) {
            cerr << "ERROR: failed to read lua packages allowlist. error: " <<
                rc << std::endl;
            return rc;
        }
        else {
