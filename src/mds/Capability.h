...
namespace ceph {
  ...
  public:
    ...
    struct Export {
        ...
        } Export(int64_t id, int w, int i, int p, snapid_t cf, ceph_seq_t s,
                 ceph_seq_t m, utime_t lis):cap_id(id), wanted(w), issued(i),
            pending(p), client_follows(cf), seq(s), mseq(m),
    ...
    Export make_export() const {
        return Export(cap_id, wanted(), issued(), pending(), client_follows,
                      get_last_seq(), mseq + 1, last_issue_stamp);
