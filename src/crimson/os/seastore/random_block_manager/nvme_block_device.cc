...
namespace crimson::os::seastore::random_block_device::nvme {
    ...
    open_ertr::future <> NVMeBlockDevice::open(const std::string & in_path,
                                               seastar::open_flags mode) {
        return seastar::do_with(in_path,[this, mode] (auto & in_path) {
                                return seastar::file_stat(in_path).
                                then([this, mode, in_path] (auto stat) {
                                     return seastar::open_file_dma(in_path,
                                                                   mode).
                                     then([ =, this] (auto file) {
                                          device = file; logger().debug("open");
                                          ...
                                          safe_then([this, in_path,
                                                    ...
                                                    {
                                                    support_multistream =
                                                    id_controller_data.oacs.
                                                    support_directives;
                                                    if (support_multistream) {
                                                    stream_id_count =
                                                    WRITE_LIFE_MAX;} awupf =
                                                    id_controller_data.awupf +
                                                    1;
                                                    return
                                                    identify_namespace(device).
                                                    safe_then([this, in_path,
                                                               mode] (auto
                                                                      id_namespace_data)
                                                              {
                                          ...
                                          handle_error(crimson::ct_error::
                                                       ...
                                                       handle([this, in_path,
                                                              ...
                                                              logger().
                                                              error
                                                              ("open: id ctrlr failed. open without ioctl");
                                                              return
                                                              open_for_io
                                                              (in_path, mode);}
                                                       ),
    ...
    open_ertr::future <> NVMeBlockDevice::open_for_io(const std::
                                                      string & in_path,
                                                      seastar::
                                                      open_flags mode) {
    ...
    write_ertr::future <> NVMeBlockDevice::write(uint64_t offset,
                                                 bufferptr && bptr,
                                                 uint16_t stream) {
        ...
        return seastar::do_with(std::move(bptr),
                                [this, offset, length,
                                 supported_stream] (auto & bptr) {
                                return io_device[supported_stream].
                                dma_write(offset, bptr.c_str(),
                                          length).handle_exception([](auto e)->
                                                                   write_ertr::
                                                                   future <
                                                                   size_t > {
                                                                   logger().
                                                                   error
                                                                   ("write: dma_write got error{}",
                                                                    e);
                                                                   return
                                                                   crimson::
                                                                   ct_error::
                                                                   input_output_error::
                                                                   make();}
                                ).then([length] (auto result)->
                                       write_ertr::future <> {
                                       if (result != length) {
                                       logger().
                                       error
                                       ("write: dma_write got error with not proper length");
    ...
    read_ertr::future <> NVMeBlockDevice::read(uint64_t offset,
                                               bufferptr & bptr) {
    ...
    write_ertr::future <> NVMeBlockDevice::writev(uint64_t offset,
                                                  ceph::bufferlist bl,
                                                  uint16_t stream) {
        ...
        return seastar::do_with(bl.prepare_iovs(),
                                std::move(bl),
                                [this, supported_stream, offset] (auto & iovs,
                                                                  auto & bl) {
    ...
    Device::close_ertr::future <> NVMeBlockDevice::close() {
        ...
        return device.close().then([this] () {
                                   return seastar::do_for_each(io_device,
                                                               [](auto
                                                                  target_device)
                                                               {
                                                               return
    ...
    discard_ertr::future <> NVMeBlockDevice::discard(uint64_t offset,
                                                     uint64_t len) {
    ...
    nvme_command_ertr::future < nvme_identify_namespace_data_t >
        ...
        return get_nsid(f).safe_then([this, f] (auto nsid) {
                                     return seastar::
                                     do_with(nvme_admin_command_t(),
                                             nvme_identify_namespace_data_t(),
                                             [this, nsid,
                                              f] (auto & admin_command,
                                                  auto & data) {
                                             admin_command.common.opcode =
                                             nvme_admin_command_t::
                                             OPCODE_IDENTIFY;
                                             admin_command.common.addr =
                                             (uint64_t) & data;
                                             admin_command.common.data_len =
                                             sizeof(data);
                                             admin_command.common.nsid = nsid;
                                             admin_command.identify.cns =
                                             nvme_identify_command_t::
                                             CNS_NAMESPACE;
                                             return pass_admin(admin_command,
                                                               f).
                                             safe_then([&data] (auto status) {
    ...
    nvme_command_ertr::future < int >NVMeBlockDevice::get_nsid(seastar::file f) {
        return f.ioctl(NVME_IOCTL_ID, nullptr).handle_exception([](auto e)->
                                                                nvme_command_ertr::
                                                                future < int > {
                                                                logger().
                                                                error
                                                                ("pass_admin: ioctl failed");
                                                                return crimson::
                                                                ct_error::
                                                                input_output_error::
                                                                make();}
    ...
    nvme_command_ertr::future <
        int >NVMeBlockDevice::pass_admin(nvme_admin_command_t & admin_cmd,
                                         seastar::file f) {
        return f.ioctl(NVME_IOCTL_ADMIN_CMD,
                       &admin_cmd).handle_exception([](auto e)->
                                                    nvme_command_ertr::future <
                                                    int > {
                                                    logger().
                                                    error
    ...
    nvme_command_ertr::future <
        int >NVMeBlockDevice::pass_through_io(nvme_io_command_t & io_cmd) {
