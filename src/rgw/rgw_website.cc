...
void RGWBWRoutingRule::apply_rule(const string & default_protocol,
...
bool RGWBWRoutingRules::check_key_and_error_code_condition(const string & key,
                                                           int error_code,
                                                           RGWBWRoutingRule **
                                                           rule)
{
...
bool RGWBWRoutingRules::check_key_condition(const string & key,
...
bool RGWBWRoutingRules::check_error_code_condition(const int http_error_code,
                                                   RGWBWRoutingRule ** rule)
{
    for (list < RGWBWRoutingRule >::iterator iter = rules.begin();
         iter != rules.end(); ++iter) {
...
bool RGWBucketWebsiteConf::should_redirect(const string & key,
                                           ...
                                           RGWBWRoutingRule * redirect)
{
    ...
    else if (!routing_rules.
             check_key_and_error_code_condition(key, http_error_code, &rule)) {
        return false;
    }
...
bool RGWBucketWebsiteConf::get_effective_key(const string & key,
                                             string * effective_key,
                                             bool is_file) constconst
{
...
void RGWBWRedirectInfo::dump_xml(Formatter * f) const const
{
...
void RGWBWRedirectInfo::decode_xml(XMLObj * obj)
{
    ...
    if (has_http_redirect_code
        ...
        throw RGWXMLDecoder::
            err
            ("The provided HTTP redirect code is not valid. Valid codes are 3XX except 300.");
    }
    redirect.http_redirect_code = code;
    bool has_replace_key_prefix_with =
        RGWXMLDecoder::decode_xml("ReplaceKeyPrefixWith",
                                  replace_key_prefix_with, obj);
...
void RGWBWRoutingRuleCondition::decode_xml(XMLObj * obj)
{
    ...
    if (has_http_error_code_returned_equals
        ...
        throw RGWXMLDecoder::
            err
            ("The provided HTTP redirect code is not valid. Valid codes are 4XX or 5XX.");
    }
    http_error_code_returned_equals = code;
}

void RGWBWRoutingRule::dump_xml(Formatter * f) const const
{
...
static void encode_xml(const char *name,
...
void RGWBucketWebsiteConf::dump_xml(Formatter * f) const const
{
    if (!redirect_all.hostname.empty()) {
