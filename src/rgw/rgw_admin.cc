...
static int get_cmd(const char *cmd, const char *prev_cmd,
                   const char *prev_prev_cmd, bool * need_more)
{
    *need_more = false;
    ...
    if (strcmp(cmd, "bi") == 0 ||
        ...
        strcmp(cmd, "user") == 0 ||
        strcmp(cmd, "zone") == 0 ||
        strcmp(cmd, "zonegroup") == 0 || strcmp(cmd, "zonegroups") == 0) {
        *need_more = true;
        return 0;
    }
...
static int init_bucket(const string & tenant_name, const string & bucket_name,
                       const string & bucket_id, RGWBucketInfo & bucket_info,
                       rgw_bucket & bucket, map < string, bufferlist > *pattrs =
                       nullptr)
{
    if (!bucket_name.empty()) {
        ...
        int r;
        if (bucket_id.empty()) {
            r = store->get_bucket_info(obj_ctx, tenant_name, bucket_name,
                                       bucket_info, nullptr, pattrs);
        }
        else {
            string bucket_instance_id = bucket_name + ":" + bucket_id;
            r = store->get_bucket_instance_info(obj_ctx, bucket_instance_id,
                                                bucket_info, NULL, pattrs);
        }
        if (r < 0) {
...
void set_quota_info(RGWQuotaInfo & quota, int opt_cmd, int64_t max_size,
                     ...
                     bool have_max_size, bool have_max_objects)
{
    ...
    RGWObjectCtx obj_ctx(store);
    int r =
        store->get_bucket_info(obj_ctx, tenant_name, bucket_name, bucket_info,
                               NULL, &attrs);
    if (r < 0) {
        cerr << "could not get bucket info for bucket=" << bucket_name << ": "
    ...
    r = store->put_bucket_instance_info(bucket_info, false, real_time(),
                                        &attrs);
    if (r < 0) {
...
int set_user_bucket_quota(int opt_cmd, RGWUser & user,
...
int set_user_quota(int opt_cmd, RGWUser & user, RGWUserAdminOpState & op_state,
                   int64_t max_size, int64_t max_objects, bool have_max_size,
                   bool have_max_objects)
{
    RGWUserInfo & user_info = op_state.get_user_info();

    set_quota_info(user_info.user_quota, opt_cmd, max_size, max_objects,
                   have_max_size, have_max_objects);
...
int check_min_obj_stripe_size(RGWRados * store, RGWBucketInfo & bucket_info,
                              rgw_obj & obj, uint64_t min_stripe_size,
                              bool * need_rewrite)
{
    map < string, bufferlist > attrs;
    uint64_t obj_size;
    ...
    if (ret < 0) {
        lderr(store->
              ctx()) << "ERROR: failed to stat object, returned error: " <<
...
int check_obj_locator_underscore(RGWBucketInfo & bucket_info, rgw_obj & obj,
                                 rgw_obj_key & key, bool fix, bool remove_bad,
                                 Formatter * f)
{
    ...
    if ((needs_fixing || remove_bad) && fix) {
        ret =
            store->fix_head_obj_locator(bucket_info, needs_fixing, remove_bad,
                                        key);
        if (ret < 0) {
...
int check_obj_tail_locator_underscore(RGWBucketInfo & bucket_info,
                                      rgw_obj & obj, rgw_obj_key & key,
                                      bool fix, Formatter * f)
{
    f->open_object_section("object");
    f->open_object_section("key");
                            ...
                            bool remove_bad, Formatter * f)
{
    ...
    int ret =
        init_bucket(tenant_name, bucket_name, bucket_id, bucket_info, bucket);
    if (ret < 0) {
    ...
    do {
        ret =
            list_op.list_objects(max_entries - count, &result, &common_prefixes,
                                 &truncated);
        if (ret < 0) {
        ...
        for (vector < rgw_bucket_dir_entry >::iterator iter = result.begin();
            ...
            if (key.name[0] == '_') {
                ret =
                    check_obj_locator_underscore(bucket_info, obj, key, fix,
                                                 remove_bad, f);

                if (ret >= 0) {
                    ret =
                        check_obj_tail_locator_underscore(bucket_info, obj, key,
                                                          fix, f);
                }
            }
        }
...
int set_bucket_sync_enabled(RGWRados * store, int opt_cmd,
                                  ...
                                  bufferlist & in_data, JSONParser & parser)
{
    ...
    rgw_user user;
    int ret =
        conn->forward(user, info, nullptr, MAX_REST_RESPONSE, &in_data,
                      &response);
                       ...
                       bufferlist & in_data, JSONParser & parser)
{
    ...
    bufferlist response;
    int ret =
        req.forward_request(key, info, MAX_REST_RESPONSE, &in_data, &response);
                         ...
                         bool force)
{
    ...
    JSONParser p;
    int ret =
        send_to_remote_or_url(remote_conn, url, access, secret, info, bl, p);
    if (ret < 0) {
    ...
    if (ret < 0) {
        cerr << "Error updating period epoch: " << cpp_strerror(ret) << std::
            endl;
        return ret;
    }
    ret = period.reflect();
...
static int init_bucket_for_sync(const string & tenant,
                                const string & bucket_name,
...
static int read_current_period_id(RGWRados * store,
                                  ...
                                  std::string * period_id)
{
    ...
    int ret = realm.init(g_ceph_context, store);
    if (ret < 0) {
        std::cerr << "failed to read realm: " << cpp_strerror(-ret) << std::
            endl;
        return ret;
    }
...
static void get_md_sync_status(list < string > &status)
{
  ...
  for (auto marker_iter:sync_status.sync_markers) {
    ...
    if (ret < 0) {
        status.push_back(string("failed to fetch local sync status: ") +
                         cpp_strerror(-ret));
        return;
    }
    ...
    if (total_behind == 0) {
        ...
        ret =
            sync.read_master_log_shards_next(sync_status.sync_info.period,
        ...
        if (ret < 0) {
          ...
          for (auto iter:master_pos) {
            ...
            if (!ceph::real_clock::is_zero(oldest)) {
                push_ss(ss,
                        status) << "oldest incremental change not applied: " <<
                    oldest;
            }
        }
    }
...
static void get_data_sync_status(const string & source_zone,
                                 list < string > &status, int tab)
{
    ...
    ret =
        sync.read_recovering_shards(sync_status.sync_info.num_shards,
                                    recovering_shards);
    if (ret < 0 && ret != ENOENT) {
        push_ss(ss, status,
  ...
  for (auto marker_iter:sync_status.sync_markers) {
    ...
    if (ret < 0) {
        push_ss(ss, status,
                tab) << string("failed to fetch local sync status: ") +
            cpp_strerror(-ret);
        return;
    }
...
static void sync_status(Formatter * formatter)
{
    ...
    cout << std::setw(width) << "realm" << std::setw(1) << " " << realm.
        get_id() << " (" << realm.get_name() << ")" << std::endl;
                                ...
                                BucketIndexShardsManager * markers)
{
    ...
    if (r < 0) {
        lderr(store->
              ctx()) << "failed to fetch remote log markers: " <<
...
{
    out << indented {
    width, "source zone"} <<source.id << " (" << source.name << ")\n";
    ...
    out << indented {
    width}
    <<"full sync: " << num_full << "/" << total_shards << " shards\n";
    if (num_full > 0) {
        out << indented {
        width}
        <<"full sync: " << full_complete << " objects completed\n";
    }
    out << indented {
    width}
    <<"incremental sync: " << num_inc << "/" << total_shards << " shards\n";
                              ...
                              std::ostream & out)
{
    ...
    out << indented {
    width, "realm"} <<realm.get_id() << " (" << realm.get_name() << ")\n";
    out << indented {
    width,
            "zonegroup"} <<zonegroup.get_id() << " (" << zonegroup.
        get_name() << ")\n";
    out << indented {
    width, "zone"} <<zone.id << " (" << zone.name << ")\n";
    out << indented {
    ...
    if (!source_zone_id.empty()) {
        ...
        if (c == store->zone_conn_map.end()) {
            lderr(store->ctx()) << "No connection to zone " << z->second.
                name << dendl;
            return -EINVAL;
        }
                                ...
                                map < string, bufferlist > &attrs)
{
    ...
    if (num_shards > (int)store->get_max_bucket_shards()) {
        cerr << "ERROR: num_shards too high, max value: " << store->
                               ...
                               RGWBucketInfo & new_bucket_info)
{
    ...
    int ret =
        store->init_bucket_index(new_bucket_info, new_bucket_info.num_shards);
    if (ret < 0) {
        cerr << "ERROR: failed to init new bucket indexes: " <<
            cpp_strerror(-ret) << std::endl;
        return -ret;
    }

    ret =
        store->put_bucket_instance_info(new_bucket_info, true, real_time(),
                                        &attrs);
    if (ret < 0) {
        cerr << "ERROR: failed to store new bucket instance info: " <<
            cpp_strerror(-ret) << std::endl;
        return -ret;
    }
...
static int trim_sync_error_log(int shard_id, const ceph::real_time & start_time,
                               ...
                               const string & end_marker, int delay_ms)
{
    auto oid =
...
int main(int argc, const char **argv)
{
    ...
    for (std::vector < const char *>::iterator i = args.begin();
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            // do nothing
                 ...
                 (args, i, &show_log_entries, NULL, "--show_log_entries",
                  (char *)NULL)) {
            // do nothing
        }
        else if (ceph_argparse_binary_flag
                 (args, i, &show_log_sum, NULL, "--show_log_sum",
                  (char *)NULL)) {
            // do nothing
        }
        else if (ceph_argparse_binary_flag
                 (args, i, &skip_zero_entries, NULL, "--skip_zero_entries",
                  (char *)NULL)) {
            // do nothing
        }
        else if (ceph_argparse_binary_flag
            ...
            // do nothing
                 ...
                 (args, i, &tmp, errs, "-a", "--auth-uid", (char *)NULL)) {
            if (!errs.str().empty()) {
            ...
            specified_shard_id = true;
                 ...
                 (args, i, &val, "--daemon-id", (char *)NULL)) {
            daemon_id = val;
            ...
            quota_scope = val;
                 ...
                 (args, i, &val, "--replica-log-type", (char *)NULL)) {
            ...
            is_read_only_set = true;
                 ...
                 (args, i, &val, "--master-url", (char *)NULL)) {
            master_url = val;
        }
        else if (ceph_argparse_witharg
                 (args, i, &val, "--master-zonegroup", (char *)NULL)) {
            ...
            if (val == "normal") {
                ...
                placement_index_type =
                    (RGWBucketIndexType) strict_strtol(val.c_str(), 10, &err);
                if (!err.empty()) {
    ...
    std::set < int >raw_storage_ops_list =
    ...
    if (raw_storage_op) {
        ...
        store =
            RGWStoreManager::get_storage(g_ceph_context, false, false, false,
                                         false, false, need_cache
    ...
    if (raw_storage_op) {
        ...
        case OPT_PERIOD_DELETE:
            {
                ...
                if (ret < 0) {
                    cerr << "period.init failed: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT_PERIOD_LIST:
            {
                ...
                if (ret < 0) {
                    cerr << "failed to list periods: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT_PERIOD_UPDATE:
            {
                int ret =
                    update_period(realm_id, realm_name, period_id, period_epoch,
        ...
        case OPT_PERIOD_PULL:
            {
                ...
                if (url.empty()) {
                    ...
                    if (ret < 0) {
                        cerr << "failed to init current period: " <<
                            cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                    if (remote.empty()) {
                    ...
                    conn =
                        get_remote_conn(store, current_period.get_map(),
                                        remote);
                    if (!conn) {
                        cerr << "failed to find a zone or zonegroup for remote "
                            << remote << std::endl;
                ...
                RGWPeriod period;
                int ret =
                    do_period_pull(remote_conn, url, access_key, secret_key,
        ...
        case OPT_GLOBAL_QUOTA_DISABLE:
            {
                ...
                if (quota_scope == "bucket") {
                    set_quota_info(period_config.bucket_quota, opt_cmd,
                                   ...
                                   have_max_size, have_max_objects);
                    encode_json("bucket quota", period_config.bucket_quota,
                                formatter);
                }
                    ...
                    set_quota_info(period_config.user_quota, opt_cmd,
                                   max_size, max_objects,
                                   have_max_size, have_max_objects);
                    encode_json("user quota", period_config.user_quota,
                                formatter);
                }
        ...
        case OPT_REALM_DELETE:
            {
                ...
                int ret = realm.init(g_ceph_context, store);
                if (ret < 0) {
                    cerr << "realm.init failed: " << cpp_strerror(-ret) << std::
                        endl;
                    return -ret;
                }
                ret = realm.delete_obj();
                if (ret < 0) {
                    cerr << "ERROR: couldn't : " << cpp_strerror(-ret) << std::
                        endl;
                    return -ret;
                }
        ...
        case OPT_REALM_GET:
            {
                ...
                if (ret < 0) {
                    if (ret == -ENOENT && realm_name.empty()
                        ...
                        cerr << "realm.init failed: " << cpp_strerror(-ret) <<
                            std::endl;
                    }
                    return -ret;
                }
        ...
        case OPT_REALM_LIST:
            {
                ...
                if (ret < 0) {
                    cerr << "failed to list realms: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT_REALM_LIST_PERIODS:
            {
                ...
                if (ret < 0) {
                    cerr << "list periods failed: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT_REALM_RENAME:
            {
                ...
                if (ret < 0) {
                    cerr << "realm.rename failed: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
                cout <<
        ...
        case OPT_REALM_SET:
            {
                ...
                if (new_realm) {
                    ...
                    ret = realm.update();
                    if (ret < 0) {
                        cerr << "ERROR: couldn't store realm info: " <<
                            cpp_strerror(-ret) << std::endl;
                        return 1;
                    }
                }
        ...
        case OPT_ZONEGROUP_ADD:
            {
                ...
                if (ret < 0) {
                    cerr << "failed to initialize zonegroup " << zonegroup_name
                ...
                if (ret < 0) {
                    cerr << "failed to add zone " << zone_name <<
                        ...
                        get_name() << ": " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
        ...
        case OPT_ZONEGROUP_CREATE:
            {
                ...
                RGWZoneGroup zonegroup(zonegroup_name, is_master,
                                       g_ceph_context, store, realm.get_id(),
                                       endpoints);
                zonegroup.api_name =
        ...
        case OPT_ZONEGROUP_SET:
            {
                ...
                if (ret < 0 && ret != -EEXIST) {
                    ...
                    ret = zonegroup.update();
                    if (ret < 0) {
                        cerr << "ERROR: couldn't store zonegroup info: " <<
                            cpp_strerror(-ret) << std::endl;
                        return 1;
                    }
                }
        ...
        case OPT_ZONE_CREATE:
            {
                ...
                if (!zonegroup_id.empty() || !zonegroup_name.empty()) {
                    ret = zonegroup.init(g_ceph_context, store);
                    if (ret < 0) {
                        cerr << "unable to initialize zonegroup " <<
                            zonegroup_name << ": " << cpp_strerror(-ret) <<
                            std::endl;
                        return -ret;
                    }
                    if (realm_id.empty() && realm_name.empty()) {
                ...
                if (!zonegroup_id.empty() || !zonegroup_name.empty()) {
                    ...
                    bool *psync_from_all =
                        (sync_from_all_specified ? &sync_from_all : nullptr);
                    ret =
                        zonegroup.add_zone(zone,
                                           (is_master_set ? &is_master : NULL),
                                           (is_read_only_set ? &read_only :
                                            NULL), endpoints, ptier_type,
                                           psync_from_all, sync_from,
                                           sync_from_rm);
                    if (ret < 0) {
                        cerr << "failed to add zone " << zone_name <<
        ...
        case OPT_ZONE_DEFAULT:
            {
                ...
                int ret = zonegroup.init(g_ceph_context, store);
                if (ret < 0) {
                    cerr << "WARNING: failed to initialize zonegroup " <<
        ...
        case OPT_ZONE_DELETE:
            {
                ...
                for (list < string >::iterator iter = zonegroups.begin();
                    ...
                    if (ret < 0 && ret != -ENOENT) {
                        cerr << "failed to remove zone " << zone_name <<
                            " from zonegroup " << zonegroup.
                            get_name() << ": " << cpp_strerror(-ret) << std::
                            endl;
                    }
                }
                ...
                ret = zone.delete_obj();
                if (ret < 0) {
                    cerr << "failed to delete zone " << zone_name << ": " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
        ...
        case OPT_ZONE_SET:
            {
                ...
                if (zone.get_id().empty()) {
                    cerr <<
                        "no zone name id the json provided, assuming old format"
                        << std::endl;
                    if (zone_name.empty()) {
                ...
                if (ret < 0) {
                    cerr << "ERROR: couldn't fix zone: " << cpp_strerror(-ret)
                        << std::endl;
                    return -ret;
                }
        ...
        case OPT_ZONE_LIST:
            {
                ...
                if (ret < 0) {
                    cerr << "failed to list zones: " << cpp_strerror(-ret) <<
                        std::endl;
                    return -ret;
                }
        ...
        case OPT_ZONE_PLACEMENT_RM:
            {
                ...
                if (opt_cmd == OPT_ZONE_PLACEMENT_ADD) {
                    ...
                    if (!index_pool || index_pool->empty() ||
                        ...
                        cerr <<
                            "ERROR: need to specify both --index-pool and --data-pool"
                            << std::endl;
                        return EINVAL;
                    }
    ...
    case OPT_PERIOD_PUSH:
        {
            ...
            int ret = period.init(g_ceph_context, store);
            if (ret < 0) {
                cerr << "period init failed: " << cpp_strerror(-ret) << std::
                    endl;
                return -ret;
            }
            // json format into a bufferlist
    ...
    case OPT_PERIOD_COMMIT:
        {
            ...
            ret =
                commit_period(realm, period, remote, url, access_key,
                              secret_key, yes_i_really_mean_it);
            if (ret < 0) {
    ...
    case OPT_ROLE_CREATE:
        {
            ...
            JSONParser p;
            if (!p.parse(bl.c_str(), bl.length())) {
                cout << "ERROR: failed to parse JSON: " << assume_role_doc <<
                    std::endl;
                return -EINVAL;
            }
    ...
    case OPT_ROLE_LIST:
        {
            ...
            ret =
                RGWRole::get_roles_by_path_prefix(store, g_ceph_context,
                                                  path_prefix, tenant, result);
            if (ret < 0) {
                return -ret;
            }
        ...
        if (!user_id.empty()) {
            ...
            do {
                ...
                if (ret < 0 && ret != -ENOENT) {
                    ...
                    ret =
                        RGWBucketAdminOp::limit_check(store, bucket_op,
                                                      user_ids, f,
                                                      warnings_only);
                    if (ret < 0)
                        break;
    ...
    if (opt_cmd == OPT_BUCKETS_LIST) {
        if (bucket_name.empty()) {
            ...
            RGWBucketInfo bucket_info;
            int ret =
                init_bucket(tenant, bucket_name, bucket_id, bucket_info,
                            bucket);
            if (ret < 0) {
            ...
            do {
                ret =
                    list_op.list_objects(max_entries - count, &result,
                                         &common_prefixes, &truncated);
                if (ret < 0) {
                    cerr << "ERROR: store->list_objects(): " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
    ...
    if (opt_cmd == OPT_LOG_SHOW || opt_cmd == OPT_LOG_RM) {
        ...
        if (opt_cmd == OPT_LOG_SHOW) {
            ...
            do {
                uint64_t total_time =
                    entry.total_time.sec() * 1000000LL +
    ...
    if (opt_cmd == OPT_USAGE_SHOW) {
        ...
        ret = RGWUsage::show(store, user_id, start_epoch, end_epoch,
                             show_log_entries, show_log_sum, &categories, f);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT_USAGE_TRIM) {
        if (user_id.empty() && !yes_i_really_mean_it) {
            cerr <<
                "usage trim without user specified will remove *all* users data"
                << std::endl;
    ...
    if (opt_cmd == OPT_OLH_READLOG) {
        ...
        ret =
            store->bucket_index_read_olh_log(bucket_info, *state, obj, 0, &log,
                                             &is_truncated);
        if (ret < 0) {
            cerr << "ERROR: failed reading olh: " << cpp_strerror(-ret) << std::
                endl;
    ...
    if (opt_cmd == OPT_BI_LIST) {
        ...
        for (int i = 0; i < max_shards; i++) {
            ...
            int ret =
                bs.init(bucket, shard_id, nullptr /* no RGWBucketInfo */ );
            marker.clear();

            if (ret < 0) {
                cerr << "ERROR: bs.init(bucket=" << bucket << ", shard=" <<
                    shard_id << "): " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
            ...
            do {
                entries.clear();
                ret =
                    store->bi_list(bs, object, marker, max_entries, &entries,
                                   &is_truncated);
                if (ret < 0) {
    ...
    if (opt_cmd == OPT_BI_PURGE) {
        ...
        ret =
            init_bucket(tenant, bucket_name, string(), cur_bucket_info,
                        cur_bucket);
        if (ret < 0) {
        ...
        if (cur_bucket_info.bucket.bucket_id == bucket_info.bucket.bucket_id
    ...
    if (opt_cmd == OPT_OBJECT_REWRITE) {
        ...
        if (min_rewrite_stripe_size > 0) {
            ret =
                check_min_obj_stripe_size(store, bucket_info, obj,
    ...
    if (opt_cmd == OPT_BUCKET_REWRITE) {
        ...
        while (is_truncated) {
            ...
            int r =
                store->cls_bucket_list_ordered(bucket_info, RGW_NO_SHARD,
                                               marker,
                                               prefix, 1000, true,
            ...
            for (iter = result.begin(); iter != result.end(); ++iter) {
                ...
                if ((entry.meta.size < min_rewrite_size) ||
                    ...
                    bool need_rewrite = true;
                    if (min_rewrite_stripe_size > 0) {
                        r = check_min_obj_stripe_size(store, bucket_info, obj,
                                                      min_rewrite_stripe_size,
                                                      &need_rewrite);
                        if (r < 0) {
                            ldout(store->ctx(),
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
        ...
        return br.execute(num_shards, max_entries, verbose, &cout, formatter);
    }
    ...
    if (opt_cmd == OPT_RESHARD_LIST) {
        ...
        for (int i = 0; i < num_logshards; i++) {
            ...
            do {
                entries.clear();
                ret =
                    reshard.list(i, marker, max_entries, entries,
                                 &is_truncated);
                if (ret < 0) {
    ...
    if (opt_cmd == OPT_RESHARD_CANCEL) {
        ...
        if (ret < 0) {
            if (ret == -EBUSY) {
                cerr << "There is ongoing resharding, please retry after " <<
                    g_conf->
                    rgw_reshard_bucket_lock_duration << " seconds " << std::
                    endl;
            }
            else {
        ...
        if (ret < 0 && ret != -ENOENT) {
            cerr << "Error in getting bucket " << bucket_name << ": " <<
                cpp_strerror(-ret) << std::endl;
            return ret;
        }
    }
    ...
    if (opt_cmd == OPT_OBJECT_STAT) {
        ...
        for (iter = attrs.begin(); iter != attrs.end(); ++iter) {
            ...
            if (iter->first == RGW_ATTR_MANIFEST) {
                handled =
                    decode_dump < RGWObjManifest > ("manifest", bl, formatter);
            }
    ...
    if (opt_cmd == OPT_GC_LIST) {
        ...
        do {
            ...
            int ret =
                store->list_gc_objs(&index, marker, 1000, !include_all, result,
                                    &truncated);
            if (ret < 0) {
...
#define MAX_LC_LIST_ENTRIES 100
        ...
        do {
            int ret =
                store->list_lc_progress(marker, max_entries, &bucket_lc_map);
            if (ret < 0) {
                cerr << "ERROR: failed to list objs: " << cpp_strerror(-ret) <<
                    std::endl;
                return 1;
            ...
            for (iter = bucket_lc_map.begin(); iter != bucket_lc_map.end();
                 ++iter) {
    ...
    if (opt_cmd == OPT_USER_STATS) {
        ...
        if (reset_stats) {
            if (!bucket_name.empty()) {
                cerr << "ERROR: recalculate doesn't work on buckets" << std::
                    endl;
                return EINVAL;
            }
            ret = store->cls_user_reset_stats(user_str);
            if (ret < 0) {
                cerr << "ERROR: could not clear user stats: " <<
                    cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        }
        ...
        if (sync_stats) {
            if (!bucket_name.empty()) {
                int ret =
                    rgw_bucket_sync_user_stats(store, tenant, bucket_name);
                if (ret < 0) {
                ...
                if (ret < 0) {
                    cerr << "ERROR: failed to sync user stats: " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
    ...
    if (opt_cmd == OPT_METADATA_PUT) {
        ...
        ret =
            store->meta_mgr->put(metadata_key, bl,
    ...
    if (opt_cmd == OPT_METADATA_LIST || opt_cmd == OPT_USER_LIST) {
        ...
        int max = 1000;
        int ret =
            store->meta_mgr->list_keys_init(metadata_key, marker, &handle);
        if (ret < 0) {
            cerr << "ERROR: can't get key: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
        ...
        if (max_entries_specified) {
            ...
            if (truncated) {
                encode_json("marker", store->meta_mgr->get_marker(handle),
                            formatter);
            }
            formatter->close_section();
        }
    ...
    if (opt_cmd == OPT_MDLOG_LIST) {
        ...
        if (period_id.empty()) {
            int ret =
                read_current_period_id(store, realm_id, realm_name, &period_id);
            if (ret < 0) {
                return -ret;
            }
        ...
        for (; i < g_ceph_context->_conf->rgw_md_log_max_shards; i++) {
            ...
            meta_log->init_list_entries(i, start_time.to_real_time(),
                                        end_time.to_real_time(), marker,
                                        &handle);
            bool truncated;
            do {
                int ret =
    ...
    if (opt_cmd == OPT_MDLOG_TRIM) {
        ...
        ret =
            meta_log->trim(shard_id, start_time.to_real_time(),
    ...
    if (opt_cmd == OPT_DATA_SYNC_STATUS) {
        ...
        RGWDataSyncStatusManager sync(store, store->get_async_rados(),
                                      source_zone);
        ...
        if (specified_shard_id) {
            ...
            ret =
                sync.read_shard_status(shard_id, pending_buckets,
    ...
    if (opt_cmd == OPT_DATA_SYNC_RUN) {
        ...
        int ret =
            store->get_sync_modules_manager()->create_instance(g_ceph_context,
                                                               store->
                                                               get_zone().
                                                               tier_type,
                                                               store->
    ...
    if ((opt_cmd == OPT_BUCKET_SYNC_DISABLE)
        || (opt_cmd == OPT_BUCKET_SYNC_ENABLE)) {
        if (bucket_name.empty()) {
            cerr << "ERROR: bucket not specified" << std::endl;
            return EINVAL;
        }
        ...
        if (ret < 0) {
            cerr << "could not init realm " << ": " << cpp_strerror(-ret) <<
                std::endl;
            return ret;
        }
        RGWPeriod period;
        ...
        if (ret < 0) {
            cerr << "failed to init period " << ": " << cpp_strerror(-ret) <<
                std::endl;
            return ret;
        }
        ...
        if (!store->is_meta_master()) {
            cerr <<
                "failed to update bucket sync: only allowed on meta master zone "
                << std::endl;
            cerr << period.get_master_zone() << " | " << period.
                get_realm() << std::endl;
            return EINVAL;
        }
    ...
    if (opt_cmd == OPT_BILOG_LIST) {
        ...
        do {
            ...
            ret =
                store->list_bi_log_entries(bucket_info, shard_id, marker,
    ...
    if (opt_cmd == OPT_SYNC_ERROR_LIST) {
        ...
        for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
            ...
            do {
                list < cls_log_entry > entries;
                ret =
                    store->time_log_list(oid, start_time.to_real_time(),
                                         end_time.to_real_time(),
                                         max_entries - count, entries, marker,
                                         &marker, &truncated);
                if (ret == -ENOENT) {
                    break;
                }
                if (ret < 0) {
                    cerr << "ERROR: store->time_log_list(): " <<
                        cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
    ...
    if (opt_cmd == OPT_BILOG_TRIM) {
        ...
        ret =
            store->trim_bi_log_entries(bucket_info, shard_id, start_marker,
                                       end_marker);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT_DATALOG_LIST) {
        ...
        do {
            ...
            ret =
                log->list_entries(start_time.to_real_time(),
                                  end_time.to_real_time(), max_entries - count,
            ...
            for (list < rgw_data_change_log_entry >::iterator iter =
    ...
    if (opt_cmd == OPT_DATALOG_AUTOTRIM) {
        ...
        ret =
            crs.
            run(create_admin_data_log_trim_cr
                (store, &http, num_shards, markers));
        if (ret < 0) {
    ...
    if (opt_cmd == OPT_DATALOG_TRIM) {
        ...
        RGWDataChangesLog *log = store->data_log;
        ret =
            log->trim_entries(start_time.to_real_time(),
                              end_time.to_real_time(), start_marker,
                              end_marker);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT_OPSTATE_LIST) {
        ...
        do {
            ...
            if (ret < 0) {
                cerr << "oc.list_entries returned " << cpp_strerror(-ret) <<
                    std::endl;
                oc.finish_list_entries(handle);
                return -ret;
            }

            for (list < cls_statelog_entry >::iterator iter = entries.begin();
                 iter != entries.end(); ++iter) {
    ...
    if (opt_cmd == OPT_OPSTATE_SET || opt_cmd == OPT_OPSTATE_RENEW) {
        ...
        if (object.empty() || client_id.empty() || op_id.empty()) {
            cerr << "ERROR: need to specify client_id, op_id, and object" <<
                std::endl;
            return EINVAL;
        }
        if (state_str.empty()) {
        ...
        if (opt_cmd == OPT_OPSTATE_SET) {
            ...
            if (ret < 0) {
                cerr << "ERROR: failed to set state: " << cpp_strerror(-ret) <<
                    std::endl;
                return -ret;
            }
        }
            ...
            if (ret < 0) {
                cerr << "ERROR: failed to renew state: " << cpp_strerror(-ret)
                    << std::endl;
                return -ret;
            }
        }
    ...
    if (opt_cmd == OPT_REPLICALOG_GET || opt_cmd == OPT_REPLICALOG_UPDATE ||
        ...
        if (replica_log_type_str.empty()) {
            cerr <<
                "ERROR: need to specify --replica-log-type=<metadata | data | bucket>"
                << std::endl;
            return EINVAL;
        }
    }
    ...
    if (opt_cmd == OPT_REPLICALOG_GET) {
        ...
        if (replica_log_type == ReplicaLog_Metadata) {
            if (!specified_shard_id) {
                cerr << "ERROR: shard-id must be specified for get operation" <<
                    std::endl;
                return EINVAL;
            }

            RGWReplicaObjectLogger logger(store, pool,
    ...
    if (opt_cmd == OPT_REPLICALOG_DELETE) {
        if (replica_log_type == ReplicaLog_Metadata) {
            if (!specified_shard_id) {
                cerr << "ERROR: shard-id must be specified for delete operation"
                    << std::endl;
                return EINVAL;
            }
            if (!specified_daemon_id) {
                cerr <<
                    "ERROR: daemon-id must be specified for delete operation" <<
                    std::endl;
                return EINVAL;
            }
            RGWReplicaObjectLogger logger(store, pool,
    ...
    if (opt_cmd == OPT_REPLICALOG_UPDATE) {
        ...
        if (replica_log_type == ReplicaLog_Metadata) {
            ...
            RGWReplicaObjectLogger logger(store, pool,
                                          META_REPLICA_LOG_OBJ_PREFIX);
            int ret =
                logger.update_bound(shard_id, daemon_id, marker, time,
                                    &entries);
            if (ret < 0) {
                cerr << "ERROR: failed to update bounds: " << cpp_strerror(-ret)
                    << std::endl;
                return -ret;
            }
        }
            ...
            RGWReplicaBucketLogger logger(store);
            ret =
                logger.update_bound(bucket, shard_id, daemon_id, marker, time,
                                    &entries);
            if (ret < 0) {
                cerr << "ERROR: failed to update bounds: " << cpp_strerror(-ret)
                    << std::endl;
    ...
    bool quota_op = (opt_cmd == OPT_QUOTA_SET || opt_cmd == OPT_QUOTA_ENABLE
                     || opt_cmd == OPT_QUOTA_DISABLE);
    ...
    if (quota_op) {
        ...
        if (!bucket_name.empty()) {
            ...
            if (quota_scope == "bucket") {
                set_user_bucket_quota(opt_cmd, user, user_op, max_size,
                                      max_objects, have_max_size,
                                      have_max_objects);
            }
            else if (quota_scope == "user") {
                set_user_quota(opt_cmd, user, user_op, max_size, max_objects,
                               have_max_size, have_max_objects);
            }
            else {
                cerr <<
