    ...
    namespace qos_simulation {
        ...
        struct CliInst {
            ...
            // D is a duration type
             template < typename D >
                CliInst(wait_op_t, D duration):op(CliOp::wait) {
                args.wait_time =
                    std::chrono::duration_cast < std::chrono::milliseconds >
                    (duration);
            } CliInst(req_op_t, uint32_t count, double ops_per_sec,
                      uint16_t max_outstanding):op(CliOp::req) {
        ...
        using ServerSelectFunc =
            std::function < const ServerId & (uint64_t seed) >;

        template < typename SvcTrk, typename ReqPm, typename RespPm,
            typename Accum > class SimulatedClient {
          public:
            ...
            struct InternalStats {
                ...
                uint32_t get_req_params_count;
                 ...
                 InternalStats():track_resp_time(0),
                    get_req_params_time(0),
                    track_resp_count(0), get_req_params_count(0) {
                    // empty
            }};

            using SubmitFunc = std::function < void (const ServerId &,
                                                     TestRequest &&,
                                                     const ClientId &,
                                                     const ReqPm &) >;

            using ClientAccumFunc =
                std::function < void (Accum &, const RespPm &) >;
          ...
          public:
            ...
            SimulatedClient(ClientId _id,
                            ...
                            const std::vector < CliInst > &_instrs):id(_id),
                submit_f(_submit_f),
            ...
            SimulatedClient(ClientId _id,
                            ...
                            double _iops_goal,
                            uint16_t
                            _outstanding_ops_allowed):SimulatedClient(_id,
                                                                      _submit_f,
                                                                      _server_select_f,
                                                                      _accum_f,
                                                                      { {
            ...
            void receive_response(const TestResponse & resp,
                ...
                resp_queue.push_back(RespQueueItem {
                                     resp, server_id, resp_params});
                cv_resp.notify_one();
            }

            const std::vector < TimePoint > &get_op_times() const {
                return op_times;
            ...
            const Accum & get_accumulator() const {
                return accumulator;
            } const InternalStats & get_internal_stats() const {
              ...
              for (auto i:instructions) {
                    if (CliOp::wait == i.op) {
                        ...
                        for (uint64_t o = 0; o < i.args.req_params.count; ++o) {
                            while (outstanding_ops >=
                                   i.args.req_params.max_outstanding) {
                                cv_req.wait(l);
                            }
                            ...
                            ReqPm rp =
                                time_stats_w_return <
                                decltype(internal_stats.get_req_params_time),
                            ...
                            submit_f(server, TestRequest {
                                     server, static_cast < uint32_t > (o), 12}
                                     , id, rp);
                            ++outstanding_ops;
                            l.lock();   // lock for return to top of loop

                            auto delay_time =
                                now + i.args.req_params.time_bw_reqs;
