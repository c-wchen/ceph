...
// Call deleter and free
    ...
    void BinnedLRUCacheShard::
        ApplyToAllCacheEntries(void (*callback) (void *, size_t),
                               bool thread_safe) {
        if (thread_safe) {
            mutex_.lock();
        }
  ...
  :    ShardedCache(capacity, num_shard_bits, strict_capacity_limit) {
        ...
        for (int i = 0; i < num_shards_; i++) {
            new(&shards_[i])
                BinnedLRUCacheShard(per_shard, strict_capacity_limit,
                                    high_pri_pool_ratio);
        }
    }
...
// Do not drop data if compile with ASAN to suppress leak warning.
    ...
    std::shared_ptr < rocksdb::Cache > NewBinnedLRUCache(size_t capacity,
                                                         int num_shard_bits,
                                                         bool
