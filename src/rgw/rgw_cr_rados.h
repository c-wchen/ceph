...
class RGWAsyncRadosRequest:public RefCountedObject {
  ...
  public:
     RGWAsyncRadosRequest(RGWCoroutine * _caller,
                          RGWAioCompletionNotifier * _cn):caller(_caller),
        notifier(_cn), retcode(0), lock("RGWAsyncRadosRequest::lock") {
  ...
  protected:
    ...
    struct RGWWQ:public ThreadPool::WorkQueue < RGWAsyncRadosRequest > {
        RGWAsyncRadosProcessor *processor;
         RGWWQ(RGWAsyncRadosProcessor * p, time_t timeout,
...
class RGWAsyncGetSystemObj:public RGWAsyncRadosRequest {
  ...
  public:
     RGWAsyncGetSystemObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                          ...
                          const rgw_raw_obj & _obj, off_t _ofs, off_t _end,
                          bool want_attrs);
...
class RGWAsyncPutSystemObjAttrs:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override;
  public:
     RGWAsyncPutSystemObjAttrs(RGWCoroutine * caller,
                               RGWAioCompletionNotifier * cn, RGWRados * _store,
                               RGWObjVersionTracker * _objv_tracker,
                               const rgw_raw_obj & _obj, map < string,
                               bufferlist > _attrs);
...
template < class T > class RGWSimpleRadosReadCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWSimpleRadosReadCR(RGWAsyncRadosProcessor * _async_rados,
                         RGWRados * _store, const rgw_raw_obj & _obj,
                         T * _result, bool empty_on_enoent =
                         true, RGWObjVersionTracker * objv_tracker = nullptr)
  :    RGWSimpleCoroutine(_store->ctx()), async_rados(_async_rados),
        store(_store), obj(_obj), result(_result),
...
template < class T > int RGWSimpleRadosReadCR < T >::send_request()
{
...
template < class T > int RGWSimpleRadosReadCR < T >::request_complete()
{
...
class RGWSimpleRadosReadAttrsCR:public RGWSimpleCoroutine {
  ...
  public:
     RGWSimpleRadosReadAttrsCR(RGWAsyncRadosProcessor * _async_rados,
                               RGWRados * _store, const rgw_raw_obj & _obj,
                               map < string,
                               bufferlist >
                               *_pattrs):RGWSimpleCoroutine(_store->ctx()),
        async_rados(_async_rados), store(_store), obj(_obj), pattrs(_pattrs),
        req(NULL) {
...
template < class T > class RGWSimpleRadosWriteCR:public RGWSimpleCoroutine {
  ...
  public:
    RGWSimpleRadosWriteCR(RGWAsyncRadosProcessor * _async_rados,
                          RGWRados * _store, const rgw_raw_obj & _obj,
                          const T & _data, RGWObjVersionTracker * objv_tracker =
                          nullptr)
  :    RGWSimpleCoroutine(_store->ctx()), async_rados(_async_rados),
    ...
    int send_request() override {
        req =
            new RGWAsyncPutSystemObj(this, stack->create_completion_notifier(),
                                     store, objv_tracker, obj, false,
                                     std::move(bl));
        async_rados->queue(req);
        return 0;
    }
...
class RGWSimpleRadosWriteAttrsCR:public RGWSimpleCoroutine {
  ...
  :    
    ...
    int send_request() override {
        req =
            new RGWAsyncPutSystemObjAttrs(this,
...
class RGWSimpleRadosLockCR:public RGWSimpleCoroutine {
  ...
  public:
     RGWSimpleRadosLockCR(RGWAsyncRadosProcessor * _async_rados,
                          RGWRados * _store, const rgw_raw_obj & _obj,
                          const string & _lock_name, const string & _cookie,
                          uint32_t _duration);
...
class RGWAsyncWait:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override {
     ...
     RGWAsyncWait(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                  CephContext * _cct, Mutex * _lock, Cond * _cond,
                  int _secs):RGWAsyncRadosRequest(caller, cn), cct(_cct),
...
class RGWWaitCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    int send_request() override {
        req =
            new RGWAsyncWait(this, stack->create_completion_notifier(), cct,
                             lock, cond, secs);
        async_rados->queue(req);
        return 0;
    }
...
class RGWShardedOmapCRManager {
  ...
  public:
     RGWShardedOmapCRManager(RGWAsyncRadosProcessor * _async_rados,
                             RGWRados * _store, RGWCoroutine * _op,
    ...
    bool finish() {
        bool success = true;
        for (vector < RGWOmapAppend * >::iterator iter = shards.begin();
  ...
  public:
    // metadata key constructor
     RGWGetBucketInstanceInfoCR(RGWAsyncRadosProcessor * _async_rados,
                                RGWRados * _store, const std::string & meta_key,
                                RGWBucketInfo * _bucket_info)
    :RGWSimpleCoroutine(_store->ctx()), async_rados(_async_rados),
        store(_store), oid(RGW_BUCKET_INSTANCE_MD_PREFIX + meta_key),
    ...
    // rgw_bucket constructor
        RGWGetBucketInstanceInfoCR(RGWAsyncRadosProcessor * _async_rados,
                                   ...
                                   RGWBucketInfo * _bucket_info)
    :RGWSimpleCoroutine(_store->ctx()), async_rados(_async_rados),
        store(_store), oid(RGW_BUCKET_INSTANCE_MD_PREFIX + bucket.get_key(':')),
        bucket_info(_bucket_info) {
    }
...
class RGWAsyncFetchRemoteObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override;
  public:
     RGWAsyncFetchRemoteObj(RGWCoroutine * caller,
                            RGWAioCompletionNotifier * cn, RGWRados * _store,
                            ...
                            bool _if_newer,
                            rgw_zone_set *
                            _zones_trace):RGWAsyncRadosRequest(caller, cn),
        store(_store), source_zone(_source_zone), bucket_info(_bucket_info),
        key(_key), versioned_epoch(_versioned_epoch), copy_if_newer(_if_newer) {
        if (_zones_trace) {
    ...
    int send_request() override {
        req =
            new RGWAsyncFetchRemoteObj(this,
                                       stack->create_completion_notifier(),
                                       store, source_zone, bucket_info, key,
                                       versioned_epoch, copy_if_newer,
                                       zones_trace);
        async_rados->queue(req);
        return 0;
    }
...
class RGWAsyncStatRemoteObj:public RGWAsyncRadosRequest {
  ...
  public:
     RGWAsyncStatRemoteObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                           RGWRados * _store, const string & _source_zone,
                           RGWBucketInfo & _bucket_info,
                           const rgw_obj_key & _key, ceph::real_time * _pmtime,
                           uint64_t * _psize, map < string,
                           bufferlist > *_pattrs):RGWAsyncRadosRequest(caller,
                                                                       cn),
        store(_store), source_zone(_source_zone), bucket_info(_bucket_info),
        key(_key), pmtime(_pmtime), psize(_psize), pattrs(_pattrs) {
}};
...
class RGWStatRemoteObjCR:public RGWSimpleCoroutine {
  ...
  public:
     RGWStatRemoteObjCR(RGWAsyncRadosProcessor * _async_rados,
        ...
        cct(_store->ctx()), async_rados(_async_rados), store(_store),
        source_zone(_source_zone), bucket_info(_bucket_info), key(_key),
        pmtime(_pmtime), psize(_psize), pattrs(_pattrs), req(NULL) {
...
class RGWAsyncRemoveObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override;
  public:
     RGWAsyncRemoveObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                       RGWRados * _store, const string & _source_zone,
                       RGWBucketInfo & _bucket_info, const rgw_obj_key & _key,
                       const string & _owner,
                       const string & _owner_display_name, bool _versioned,
                       ...
                       rgw_zone_set * _zones_trace):RGWAsyncRadosRequest(caller,
                                                                         cn),
        store(_store), source_zone(_source_zone), bucket_info(_bucket_info),
        key(_key), owner(_owner), owner_display_name(_owner_display_name),
        versioned(_versioned), versioned_epoch(_versioned_epoch),
        del_if_older(_if_older), timestamp(_timestamp) {
        if (_delete_marker) {
...
class RGWRemoveObjCR:public RGWSimpleCoroutine {
  ...
  public:
    ...
    int send_request() override {
        req =
            new RGWAsyncRemoveObj(this, stack->create_completion_notifier(),
                                  store, source_zone, bucket_info, key, owner,
...
class RGWContinuousLeaseCR:public RGWCoroutine {
  ...
  public:
    RGWContinuousLeaseCR(RGWAsyncRadosProcessor * _async_rados,
                         RGWRados * _store, const rgw_raw_obj & _obj,
...
class RGWAsyncStatObj:public RGWAsyncRadosRequest {
  ...
  protected:
    int _send_request() override;
  public:
     RGWAsyncStatObj(RGWCoroutine * caller, RGWAioCompletionNotifier * cn,
                     RGWRados * store, const RGWBucketInfo & _bucket_info,
                     const rgw_obj & obj, uint64_t * psize =
                     nullptr, real_time * pmtime = nullptr, uint64_t * pepoch =
