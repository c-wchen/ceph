            ...
            int execute(const po::variables_map & vm, bool enabled) {
                ...
                if (vm.count(at::POSITIONAL_ARGUMENTS)) {
                    const std::vector < std::string > &args =
                        vm[at::POSITIONAL_ARGUMENTS].as < std::vector <
                        std::string > >();
                    feature_names.insert(feature_names.end(),
                                         args.begin() + arg_index, args.end());
                }
                ...
                r = image.update_features(boost::any_cast < uint64_t >
                                          (features_any), enabled);
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to update image features: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
