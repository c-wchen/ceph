      ...
      :    m_cct(cct), m_rados(new librados::Rados()) {
        ...
        int ObjectCacheStore::init(bool reset) {
            ...
            if (reset) {
                try {
                    ...
                    lderr(m_cct) <<
                        "failed to initialize cache store directory: " << e.
                        what() << dendl;
                    return -e.code().value();
                }
            }
        ...
        int ObjectCacheStore::do_promote(std::string pool_nspace,
            ...
            ldout(m_cct,
                ...
                " from pool id: " << pool_id << " namespace: " << pool_nspace <<
            ...
            auto ctx =
                new LambdaContext([this, read_buf, cache_file_name] (int ret){
                                  handle_promote_callback(ret, read_buf,
                                                          cache_file_name);});

            return promote_object(&ioctx, object_name, read_buf, ctx);
        }

        int ObjectCacheStore::handle_promote_callback(int ret,
                                                      bufferlist * read_buf,
                                                      std::
                                                      string cache_file_name) {
            ldout(m_cct,
                  20) << " cache_file_name: " << cache_file_name << dendl;
            ...
            // update metadata
            ceph_assert(OBJ_CACHE_SKIP ==
                        m_policy->get_status(cache_file_name));
        ...
        int ObjectCacheStore::lookup_object(std::string pool_nspace,
                                            ...
                                            std::
                                            string & target_cache_file_path) {
            ldout(m_cct,
                  20) << "object name = " << object_name << " in pool ID : " <<
                pool_id << dendl;
            ...
            case OBJ_CACHE_NONE:{
                    if (take_token_from_throttle(object_size, 1)) {
                        pret =
                            do_promote(pool_nspace, pool_id, snap_id,
                                       object_name);
                        if (pret < 0) {
                        ...
                        m_policy->update_status(cache_file_name,
                                                OBJ_CACHE_NONE);
                    }
                    return ret;
                }
        ...
        int ObjectCacheStore::promote_object(librados::IoCtx * ioctx,
            ...
            int ret =
                ioctx->aio_read(object_name, read_completion, read_buf, 0, 0);
            if (ret < 0) {
        ...
        std::string ObjectCacheStore::get_cache_file_name(std::
                                                          string pool_nspace,
                                                          uint64_t pool_id,
                                                          uint64_t snap_id,
                                                          std::string oid) {
        ...
        std::string ObjectCacheStore::get_cache_file_path(std::
                                                          string
                                                          cache_file_name,
                                                          bool mkdir) {
            ...
            if (mkdir) {
                ldout(m_cct,
                      20) << "creating cache dir: " << cache_file_dir << dendl;
                std::error_code ec;
        ...
        void ObjectCacheStore::handle_throttle_ready(uint64_t tokens,
                                                     uint64_t type) {
        ...
        bool ObjectCacheStore::take_token_from_throttle(uint64_t object_size,
            ...
            if (!wait && (m_qos_enabled_flag & ROC_QOS_IOPS_THROTTLE)) {
                ...
                if (object_num > m_iops_tokens) {
                    wait =
                        m_throttles[ROC_QOS_IOPS_THROTTLE]->get(object_num,
                                                                this,
                                                                &ObjectCacheStore::
            ...
            if (!wait && (m_qos_enabled_flag & ROC_QOS_BPS_THROTTLE)) {
                ...
                if (object_size > m_bps_tokens) {
                    wait =
                        m_throttles[ROC_QOS_BPS_THROTTLE]->get(object_size,
                                                               this,
                                                               &ObjectCacheStore::
                                                               handle_throttle_ready,
                                                               object_size,
