...
struct PGLog:DoutPrefixProvider {
   ...
   */
     ...
     */
      ...
      template < typename ... Args > IndexedLog(Args && ... args):
    ...
    /****/ void claim_log_and_clear_rollback_info(const pg_log_t & o) {
        ...
        bool get_request(const osd_reqid_t & r,
            ...
            assert(user_version);
            assert(return_code);
            ceph::unordered_map < osd_reqid_t,
                pg_log_entry_t * >::const_iterator p;
            if (!(indexed_data & PGLOG_INDEXED_CALLER_OPS)) {
                index_caller_ops();
            } p = caller_ops.find(r);
            if (p != caller_ops.end()) {
        ...
        void unindex(const pg_log_entry_t & e) {
            ...
            if (indexed_data & PGLOG_INDEXED_EXTRA_CALLER_OPS) {
                for (auto j = e.extra_reqids.begin();
                     j != e.extra_reqids.end(); ++j) {
                    for (ceph::unordered_multimap < osd_reqid_t,
                         pg_log_entry_t * >::iterator k =
        ...
        void trim(CephContext * cct,
                  ...
                  set < string > *trimmed_dups, eversion_t * write_from_dups);
  ...
  PGLog(CephContext * cct, DoutPrefixProvider * dpp = nullptr):
    prefix_provider(dpp),
        ...
        touched_log(false), clear_divergent_priors(false) {
    }
    ...
    void trim(eversion_t trim_to,
              pg_info_t & info, bool transaction_applied = true);

    void roll_forward_to(eversion_t roll_forward_to, LogEntryHandler * h) {
        log.roll_forward_to(roll_forward_to, h);
    }
    ...
    void recover_got(hobject_t oid, eversion_t v, pg_info_t & info) {
        if (missing.is_missing(oid, v)) {
            ...
            while (log.complete_to != log.log.end()) {
                if (missing.get_items().
                    at(missing.get_rmissing().begin()->second).need <=
    ...
    void reset_complete_to(pg_info_t * info) {
        ...
        while (!missing.get_items().empty() && log.complete_to->version <
               missing.get_items().at(missing.get_rmissing().begin()->second).
               need) {
   ...
   */
    template < typename missing_type > static void _merge_object_divergent_entries(const IndexedLog & log,  ///< [in] log to merge against
        ...
        ceph::unordered_map < hobject_t,
            pg_log_entry_t * >::const_iterator objiter = log.objects.find(hoid);
        ...
        for (list < pg_log_entry_t >::const_reverse_iterator i =
             entries.rbegin(); i != entries.rend(); ++i) {
    ...
    template < typename missing_type > static void _merge_divergent_entries(const IndexedLog & log, ///< [in] log to merge against
        ...
        for (map < hobject_t,
             mempool::osd_pglog::list < pg_log_entry_t >>::iterator i =
             split.begin(); i != split.end(); ++i) {
            _merge_object_divergent_entries(log, i->first, i->second, oinfo,
                                            olog_can_rollback_to, omissing,
                                            rollbacker, dpp);
        }
    }
  ...
  public:
    ...
    void read_log_and_missing(ObjectStore * store,
        ...
        return read_log_and_missing(store, pg_coll, log_coll, log_oid, info,
    ...
    template < typename missing_type >
        static void read_log_and_missing(ObjectStore * store,
                                         ...
                                         false) {
        ldpp_dout(dpp,
                  20) << "read_log_and_missing coll " << pg_coll << " log_oid "
            << log_oid << dendl;
        ...
        ObjectMap::ObjectMapIterator p =
            store->get_omap_iterator(log_coll, log_oid);
        ...
        if (must_rebuild || debug_verify_stored_missing) {
            ...
            if (debug_verify_stored_missing
                ...
                for (list < pg_log_entry_t >::reverse_iterator i =
                     log.log.rbegin(); i != log.log.rend(); ++i) {
                    if (!debug_verify_stored_missing
                        && i->version <= info.last_complete)
                        break;
