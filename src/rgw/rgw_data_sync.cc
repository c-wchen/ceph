...
void RGWSyncDebugLogger::init(CephContext * _cct, const string & source_zone,
                              const string & sync_type,
                              const string & sync_section,
                              const string & resource, bool log_start)
{
    ...
    prefix =
        "Sync:" + zone_str + ":" + sync_type + ":" + sync_section + ":" +
        resource;
    if (log_start) {
        log("start");
    }
...
class RGWDataSyncDebugLogger:public RGWSyncDebugLogger {
  public:
    ...
    } RGWDataSyncDebugLogger(RGWDataSyncEnv * sync_env,
                             const string & sync_section,
                             const string & resource, bool log_start = true) {
    ...
    void init(RGWDataSyncEnv * sync_env, const string & sync_section,
              const string & resource, bool log_start = true) {
        RGWSyncDebugLogger::init(sync_env->cct, sync_env->source_zone, "data",
...
class RGWReadDataSyncStatusMarkersCR:public RGWShardCollectCR {
  ...
  public:
    ...
    :RGWShardCollectCR(env->cct, MAX_CONCURRENT_SHARDS), env(env),
        num_shards(num_shards), markers(markers) {
...
class RGWReadDataSyncRecoveringShardsCR:public RGWShardCollectCR {
  ...
  public:
    RGWReadDataSyncRecoveringShardsCR(RGWDataSyncEnv * env,
                                      uint64_t _max_entries, int _num_shards,
                                      map < int,
                                      std::set < std::string >> &_entries_map)
    :RGWShardCollectCR(env->cct, MAX_CONCURRENT_SHARDS), env(env),
...
bool RGWReadDataSyncRecoveringShardsCR::spawn_next()
{
    ...
    string error_oid =
        RGWDataSyncStatusManager::shard_obj_name(env->source_zone,
                                                 shard_id) + ".retry";
    spawn(new
          RGWRadosGetOmapKeysCR(env->store,
                                rgw_raw_obj(env->store->get_zone_params().
                                            log_pool, error_oid), marker,
                                &entries_map[shard_id], max_entries), false);
    ...
    reenter(this) {
        ...
        yield {
            ...
            call(new ReadInfoCR(sync_env->async_rados, sync_env->store,
                                rgw_raw_obj(sync_env->store->get_zone_params().
                                            log_pool,
                                            RGWDataSyncStatusManager::
                                            sync_status_oid(sync_env->
                                                            source_zone)),
        ...
        yield call(new
                   ReadMarkersCR(sync_env, sync_status->sync_info.num_shards,
...
class RGWReadRemoteDataLogShardInfoCR:public RGWCoroutine {
  ...
  public:
    RGWReadRemoteDataLogShardInfoCR(RGWDataSyncEnv * _sync_env,
                                    ...
                                    _shard_info):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env), http_op(NULL), shard_id(_shard_id),
    ...
    int operate() override {
        reenter(this) {
            yield {
                ...
                http_op =
                    new RGWRESTReadResource(sync_env->conn, p, pairs, NULL,
...
class RGWReadRemoteDataLogShardCR:public RGWCoroutine {
  ...
  public:
    RGWReadRemoteDataLogShardCR(RGWDataSyncEnv * _sync_env, int _shard_id,
                                ...
                                bool * _truncated)
    :RGWCoroutine(_sync_env->cct), sync_env(_sync_env), shard_id(_shard_id),
...
#define READ_DATALOG_MAX_CONCURRENT 10
  ...
  public:
    RGWReadRemoteDataLogInfoCR(RGWDataSyncEnv * _sync_env,
                               ...
                               map < int,
                               RGWDataChangesLogInfo >
                               *_datalog_info):RGWShardCollectCR(_sync_env->cct,
...
bool RGWReadRemoteDataLogInfoCR::spawn_next()
{
    ...
    spawn(new
          RGWReadRemoteDataLogShardInfoCR(sync_env, shard_id,
...
#define READ_DATALOG_MAX_CONCURRENT 10
  ...
  public:
    RGWListRemoteDataLogCR(RGWDataSyncEnv * _sync_env,
                           ...
                           *_result):RGWShardCollectCR(_sync_env->cct,
                                                       READ_DATALOG_MAX_CONCURRENT),
...
bool RGWListRemoteDataLogCR::spawn_next()
{
    ...
    spawn(new
          RGWListRemoteDataLogShardCR(sync_env, iter->first, iter->second,
...
class RGWInitDataSyncStatusCoroutine:public RGWCoroutine {
  ...
  public:
    RGWInitDataSyncStatusCoroutine(RGWDataSyncEnv * _sync_env,
                                   uint32_t num_shards, uint64_t instance_id,
                                   rgw_data_sync_status * status)
    :RGWCoroutine(_sync_env->cct), sync_env(_sync_env), store(sync_env->store),
        pool(store->get_zone_params().log_pool), num_shards(num_shards),
        status(status) {
        lock_name = "sync_lock";
...
#define COOKIE_LEN 16
        ...
        reenter(this) {
            ...
            yield call(new LockCR(sync_env->async_rados, store, rgw_raw_obj {
                                  pool, sync_status_oid},
                                  lock_name, cookie, lock_duration));
            if (retcode < 0) {
                ldout(cct,
                      0) << "ERROR: failed to take a lock on " <<
                    sync_status_oid << dendl;
                return set_cr_error(retcode);
            }
            using WriteInfoCR = RGWSimpleRadosWriteCR < rgw_data_sync_info >;
            yield call(new WriteInfoCR(sync_env->async_rados, store, rgw_raw_obj {
                                       pool, sync_status_oid}
                                       , status->sync_info));
            if (retcode < 0) {
                ldout(cct,
                      0) << "ERROR: failed to write sync status info with " <<
                    retcode << dendl;
                return set_cr_error(retcode);
            }
            ...
            yield {
                RGWRESTConn *conn =
                    store->get_zone_conn_by_id(sync_env->source_zone);
                if (!conn) {
                    ldout(cct,
                          0) << "ERROR: connection to zone " << sync_env->
                        source_zone << " does not exist!" << dendl;
                    return set_cr_error(-EIO);
                }
                for (uint32_t i = 0; i < num_shards; i++) {
                    spawn(new
                          RGWReadRemoteDataLogShardInfoCR(sync_env, i,
                                                          &shards_info[i]),
                          true);
                }
            }
            while (collect(&ret, NULL)) {
                if (ret < 0) {
                    ldout(cct,
                          0) << "ERROR: failed to read remote data log shards"
                        << dendl;
            ...
            yield {
                for (uint32_t i = 0; i < num_shards; i++) {
                    ...
                    spawn(new
                          WriteMarkerCR(sync_env->async_rados, store,
                                        rgw_raw_obj {
...
int RGWRemoteDataLog::read_log_info(rgw_datalog_info * log_info)
{
    ...
    ldout(store->ctx(),
          20) << "remote datalog, num_shards=" << log_info->num_shards << dendl;

    return 0;
}

int RGWRemoteDataLog::read_source_log_shards_info(map < int,
                                                  RGWDataChangesLogInfo >
                                                  *shards_info)
{
    ...
    return run(new
               RGWReadRemoteDataLogInfoCR(&sync_env, log_info.num_shards,
                                          shards_info));
}

int RGWRemoteDataLog::read_source_log_shards_next(map < int,
...
{
    sync_env.init(store->ctx(), store, _conn, async_rados, &http_manager,
                  _error_logger, _source_zone, _sync_module);
    ...
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "failed in http_manager.set_threaded() ret=" << ret <<
            dendl;
        return ret;
    }
...
int RGWRemoteDataLog::read_sync_status(rgw_data_sync_status * sync_status)
{
    ...
    ret =
        crs.
        run(new RGWReadDataSyncStatusCoroutine(&sync_env_local, sync_status));
    http_manager.stop();
    return ret;
}

int RGWRemoteDataLog::read_recovering_shards(const int num_shards,
                                             set < int >&recovering_shards)
{
    ...
    ret =
        crs.
        run(new
            RGWReadDataSyncRecoveringShardsCR(&sync_env_local, max_entries,
                                              num_shards, entries_map));
    http_manager.stop();
...
int RGWRemoteDataLog::init_sync_status(int num_shards)
{
    ...
    ret =
        crs.
        run(new
            RGWInitDataSyncStatusCoroutine(&sync_env_local, num_shards,
                                           instance_id, &sync_status));
    http_manager.stop();
    return ret;
}

static string full_data_sync_index_shard_oid(const string & source_zone,
                                             int shard_id)
{
    char buf[datalog_sync_full_sync_index_prefix.size() + 1 +
             source_zone.size() + 1 + 16];
    snprintf(buf, sizeof(buf), "%s.%s.%d",
             datalog_sync_full_sync_index_prefix.c_str(), source_zone.c_str(),
             shard_id);
    return string(buf);
}
...
class RGWListBucketIndexesCR:public RGWCoroutine {
  ...
  public:
    RGWListBucketIndexesCR(RGWDataSyncEnv * _sync_env,
                           ...
                           _sync_status):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env), store(sync_env->store), sync_status(_sync_status),
        req_ret(0), ret(0), entries_index(NULL), i(0), failed(false) {
        oid_prefix =
            datalog_sync_full_sync_index_prefix + "." + sync_env->source_zone;
        ...
        reenter(this) {
            yield {
                ...
                call(new RGWReadRESTResourceCR < list < string >
                     >(store->ctx(), sync_env->conn, sync_env->http_manager,
                       entrypoint, NULL, &result));
            }
            if (retcode < 0) {
                ldout(sync_env->cct,
                      0) <<
                    "ERROR: failed to fetch metadata for section bucket.index"
                    << dendl;
                return set_cr_error(retcode);
            }
            entries_index =
                new RGWShardedOmapCRManager(sync_env->async_rados, store, this,
                                            num_shards,
            ...
            if (!failed) {
                ...
                yield call(sync_env->error_logger->
                           log_error_cr(sync_env->conn->get_remote_id(),
                                        "data.init", "", EIO,
                                        string
...
class RGWDataSyncShardMarkerTrack:public RGWSyncShardMarkerTrack < string,
  ...
  public:
  RGWDataSyncShardMarkerTrack(RGWDataSyncEnv * _sync_env, const string & _marker_oid, const rgw_data_sync_marker & _marker):RGWSyncShardMarkerTrack(DATA_SYNC_UPDATE_MARKER_WINDOW),
        sync_env(_sync_env),
        marker_oid(_marker_oid), sync_marker(_marker) {
    }
    ...
    RGWCoroutine *store_marker(const string & new_marker, uint64_t index_pos,
        ...
        ldout(sync_env->cct,
              20) << __func__ << "(): updating marker marker_oid=" << marker_oid
            << " marker=" << new_marker << dendl;
        RGWRados *store = sync_env->store;
...
class RGWRunBucketSyncCoroutine:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
        status_oid(RGWBucketSyncStatusManager::
                   status_oid(sync_env->source_zone, bs)) {
...
class RGWDataSyncSingleEntryCR:public RGWCoroutine {
  ...
  public:
     RGWDataSyncSingleEntryCR(RGWDataSyncEnv * _sync_env,
                              ...
                              bool _remove_from_repo):RGWCoroutine(_sync_env->
                                                                   cct),
        ...
        set_description() << "data sync single entry (source_zone=" <<
            sync_env->
            source_zone << ") key=" << _raw_key << " entry=" << entry_marker;
    } int operate() override {
        reenter(this) {
            do {
                yield {
                    int ret =
                        rgw_bucket_parse_bucket_key(sync_env->cct, raw_key,
            ...
            if (sync_status == -ENOENT) {
                ...
                // error_repo forever
                ldout(sync_env->store->ctx(),
                      0) << "WARNING: skipping data log entry "
            ...
            if (sync_status < 0) {
                ...
                raw_key};
                yield call(new
                           RGWRadosRemoveOmapKeysCR(sync_env->store,
                                                    error_repo->get_obj(),
                                                    keys));
                if (retcode < 0) {
                    ldout(sync_env->store->ctx(),
...
#define RETRY_BACKOFF_SECS_MAX 600
  ...
  public:
     RGWDataSyncShardCR(RGWDataSyncEnv * _sync_env,
        ...
        sync_env(_sync_env), pool(_pool), shard_id(_shard_id),
        sync_marker(_marker), marker_tracker(NULL), truncated(false),
        inc_lock("RGWDataSyncShardCR::inc_lock"), total_entries(0),
        spawn_window(BUCKET_SHARD_SYNC_SPAWN_WINDOW), reset_backoff(NULL),
        lease_cr(nullptr), lease_stack(nullptr), error_repo(nullptr),
        max_error_entries(DATA_SYNC_MAX_ERR_ENTRIES),
        retry_backoff_secs(RETRY_BACKOFF_SECS_DEFAULT) {
        set_description() << "data sync shard source_zone=" << sync_env->
...
#define OMAP_GET_MAX_ENTRIES 100
        ...
        reenter(&full_cr) {
            ...
            oid =
                full_data_sync_index_shard_oid(sync_env->source_zone, shard_id);
            set_marker_tracker(new
                               RGWDataSyncShardMarkerTrack(sync_env, status_oid,
                                                           sync_marker));
            ...
            do {
                ...
                yield call(new
                           RGWRadosGetOmapKeysCR(sync_env->store,
                ...
                for (; iter != entries.end(); ++iter) {
                    ldout(sync_env->cct,
                          20) << __func__ << ": full sync: " << *iter << dendl;
                    total_entries++;
                    if (!marker_tracker->
                        start(*iter, total_entries, real_time())) {
                        ldout(sync_env->cct,
                        ...
                        yield spawn(new
                                    RGWDataSyncSingleEntryCR(sync_env, *iter,
                                                             *iter,
                                                             marker_tracker,
                                                             error_repo, false),
                                    false);
                    ...
                    while ((int)num_spawned() > spawn_window) {
                        ...
                        while (collect(&ret, lease_stack.get())) {
                            if (ret < 0) {
                                ldout(cct,
                                      10) << "a sync operation returned error"
                                    << dendl;
                            }
                        }
                    }
            ...
            if (retcode < 0) {
                ldout(sync_env->cct,
                      0) << "ERROR: failed to set sync marker: retcode=" <<
                    retcode << dendl;
                lease_cr->go_down();
                drain_all();
                return set_cr_error(retcode);
    ...
    int incremental_sync() {
        reenter(&incremental_cr) {
            ...
            error_repo =
                new RGWOmapAppend(sync_env->async_rados, sync_env->store,
                ...
                for (modified_iter = current_modified.begin();
                    ...
                    yield {
                        ldout(sync_env->cct,
                            ...
                            << dendl;
                        spawn(new
                              RGWDataSyncSingleEntryCR(sync_env, *modified_iter,
                ...
                if (error_retry_time <= ceph::coarse_real_clock::now()) {
                    ...
                    yield call(new
                               RGWRadosGetOmapKeysCR(sync_env->store,
                                                     ...
                                                     max_error_entries));
                    ldout(sync_env->cct,
                          20) << __func__ << "(): read error repo, got " <<
                        error_entries.size() << " entries" << dendl;
                    iter = error_entries.begin();
                    for (; iter != error_entries.end(); ++iter) {
                        error_marker = *iter;
                        ldout(sync_env->cct,
                              20) << __func__ << "(): handle error entry: " <<
                            error_marker << dendl;
                        spawn(new
                              RGWDataSyncSingleEntryCR(sync_env, error_marker,
                                                       error_marker,
                                                       nullptr
                                                       /* no marker tracker */ ,
                                                       error_repo, true),
...
#define INCREMENTAL_MAX_ENTRIES 100
                ldout(sync_env->cct,
                      20) << __func__ << ":" << __LINE__ << ": shard_id=" <<
                    shard_id << " sync_marker=" << sync_marker.marker << dendl;
                spawned_keys.clear();
                yield call(new
                           RGWReadRemoteDataLogShardCR(sync_env, shard_id,
                                                       sync_marker.marker,
                                                       &next_marker,
                                                       &log_entries,
                                                       &truncated));
                ...
                for (log_iter = log_entries.begin();
                    ...
                    ldout(sync_env->cct,
                          20) << __func__ << ":" << __LINE__ << ": shard_id=" <<
                        shard_id << " log_entry: " << log_iter->
                        log_id << ":" << log_iter->
                        log_timestamp << ":" << log_iter->entry.key << dendl;
                    if (!marker_tracker->
                        index_key_to_marker(log_iter->entry.key,
                                            log_iter->log_id)) {
                        ldout(sync_env->cct,
                              20) << __func__ << ": skipping sync of entry: " <<
                            log_iter->log_id << ":" << log_iter->entry.
                            key << " sync already in progress for bucket shard"
                            << dendl;
                ...
                ldout(sync_env->cct,
                      20) << __func__ << ":" << __LINE__ << ": shard_id=" <<
                    shard_id << " sync_marker=" << sync_marker.
                    marker << " next_marker=" << next_marker << " truncated=" <<
                    truncated << dendl;
                if (!truncated) {
...
class RGWDataSyncShardControlCR:public RGWBackoffControlCR {
  ...
  public:
     RGWDataSyncShardControlCR(RGWDataSyncEnv * _sync_env, rgw_pool & _pool,
                               ...
                               _marker):RGWBackoffControlCR(_sync_env->cct,
                                                            false),
        sync_env(_sync_env), pool(_pool), shard_id(_shard_id),
        sync_marker(_marker) {
    } RGWCoroutine *alloc_cr() override {
        return new RGWDataSyncShardCR(sync_env, pool, shard_id, sync_marker,
                                      backoff_ptr());
    }
...
class RGWDataSyncCR:public RGWCoroutine {
    ...
    nullptr};
  public:
     RGWDataSyncCR(RGWDataSyncEnv * _sync_env, uint32_t _num_shards,
                   bool * _reset_backoff):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env), num_shards(_num_shards), marker_tracker(NULL),
    ...
    int operate() override {
        reenter(this) {
            ...
            /* read sync status */
            yield call(new
                       RGWReadDataSyncStatusCoroutine(sync_env, &sync_status));

            data_sync_module = sync_env->sync_module->get_data_handler();

            if (retcode < 0 && retcode != -ENOENT) {
                ldout(sync_env->cct,
                      0) << "ERROR: failed to fetch sync status, retcode=" <<
                    retcode << dendl;
                return set_cr_error(retcode);
            }
            ...
            if ((rgw_data_sync_info::SyncState) sync_status.sync_info.state ==
                ...
                get_random_bytes((char *)&instance_id, sizeof(instance_id));
                yield call(new
                           RGWInitDataSyncStatusCoroutine(sync_env, num_shards,
                                                          instance_id,
                                                          &sync_status));
                if (retcode < 0) {
                    ldout(sync_env->cct,
                          0) << "ERROR: failed to init sync, retcode=" <<
                        retcode << dendl;
                    return set_cr_error(retcode);
                }
            ...
            if ((rgw_data_sync_info::SyncState) sync_status.sync_info.state ==
                ...
                ldout(sync_env->cct,
                      20) << __func__ << "(): building full sync maps" << dendl;
                ...
                if (retcode < 0) {
                    ldout(sync_env->cct,
                          0) << "ERROR: failed to write sync status, retcode="
                        << retcode << dendl;
                    return set_cr_error(retcode);
                }
            ...
            yield {
                if ((rgw_data_sync_info::SyncState) sync_status.sync_info.
                    state == rgw_data_sync_info::StateSync) {
                    for (map < uint32_t, rgw_data_sync_marker >::iterator iter =
                         sync_status.sync_markers.begin();
                         iter != sync_status.sync_markers.end(); ++iter) {
...
class RGWDefaultDataSyncModule:public RGWDataSyncModule {
  public:
    ...
    } RGWCoroutine *sync_object(RGWDataSyncEnv * sync_env,
                                RGWBucketInfo & bucket_info, rgw_obj_key & key,
                                boost::optional < uint64_t > versioned_epoch,
                                rgw_zone_set * zones_trace) override;
    RGWCoroutine *remove_object(RGWDataSyncEnv * sync_env,
                                RGWBucketInfo & bucket_info, rgw_obj_key & key,
                                real_time & mtime, bool versioned,
                                uint64_t versioned_epoch,
                                rgw_zone_set * zones_trace) override;
    RGWCoroutine *create_delete_marker(RGWDataSyncEnv * sync_env,
                                       RGWBucketInfo & bucket_info,
...
int RGWDefaultSyncModule::create_instance(CephContext * cct, map < string,
                                          string, ltstr_nocase > &config,
...
RGWCoroutine *RGWDefaultDataSyncModule::sync_object(RGWDataSyncEnv * sync_env,
                                                    RGWBucketInfo & bucket_info,
                                                    rgw_obj_key & key,
...
{
    return new RGWFetchRemoteObjCR(sync_env->async_rados, sync_env->store,
                                   sync_env->source_zone, bucket_info, key,
                                   versioned_epoch, true, zones_trace);
}

RGWCoroutine *RGWDefaultDataSyncModule::remove_object(RGWDataSyncEnv * sync_env,
                                                      RGWBucketInfo &
                                                      bucket_info,
                                                      rgw_obj_key & key,
                                                      real_time & mtime,
                                                      ...
                                                      zones_trace)
{
    return new RGWRemoveObjCR(sync_env->async_rados, sync_env->store,
                              sync_env->source_zone, bucket_info, key,
                              versioned, versioned_epoch, NULL, NULL, false,
                              &mtime, zones_trace);
}

RGWCoroutine *RGWDefaultDataSyncModule::create_delete_marker(RGWDataSyncEnv *
                                                             sync_env,
                                                             RGWBucketInfo &
                                                             bucket_info,
                                                             rgw_obj_key & key,
                                                             ...
                                                             zones_trace)
{
    return new RGWRemoveObjCR(sync_env->async_rados, sync_env->store,
                              sync_env->source_zone, bucket_info, key,
                              versioned, versioned_epoch, &owner.id,
...
class RGWDataSyncControlCR:public RGWBackoffControlCR {
  ...
  public:
    RGWDataSyncControlCR(RGWDataSyncEnv * _sync_env,
                         uint32_t _num_shards):RGWBackoffControlCR(_sync_env->
                                                                   cct,
                                                                   exit_on_error),
        sync_env(_sync_env), num_shards(_num_shards) {
    } RGWCoroutine *alloc_cr() override {
...
int RGWDataSyncStatusManager::init()
{
    ...
    if (!store->get_sync_modules_manager()->
    ...
    error_logger =
        new RGWSyncErrorLogger(store, RGW_SYNC_ERROR_LOG_SHARD_PREFIX,
                               ERROR_LOGGER_SHARDS);

    int r = source_log.init(source_zone, conn, error_logger, sync_module);
    if (r < 0) {
        lderr(store->
              ctx()) << "ERROR: failed to init remote log, r=" << r << dendl;
        finalize();
        return r;
    }
    ...
    if (r < 0) {
        ldout(store->ctx(),
              5) << "ERROR: master.read_log_info() returned r=" << r << dendl;
        finalize();
        return r;
    }
...
string RGWDataSyncStatusManager::shard_obj_name(const string & source_zone,
                                                int shard_id)
{
    char buf[datalog_sync_status_shard_prefix.size() + source_zone.size() + 16];
    snprintf(buf, sizeof(buf), "%s.%s.%d",
             datalog_sync_status_shard_prefix.c_str(), source_zone.c_str(),
             shard_id);
        ...
        reenter(this) {
            yield {
                ...
                call(new RGWReadRESTResourceCR < rgw_bucket_index_marker_info >
                     (sync_env->cct, sync_env->conn, sync_env->http_manager, p,
                      pairs, info));
            }
            if (retcode < 0) {
                return set_cr_error(retcode);
...
class RGWInitBucketShardSyncStatusCoroutine:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
        sync_status_oid(RGWBucketSyncStatusManager::
                        status_oid(sync_env->source_zone, bs)),
        status(_status) {
    } int operate() override {
        reenter(this) {
            /* fetch current position in logs */
            yield call(new
                       RGWReadRemoteBucketIndexLogInfoCR(sync_env, bs, &info));
            if (retcode < 0 && retcode != -ENOENT) {
                ldout(cct,
                      0) << "ERROR: failed to fetch bucket index status" <<
                    dendl;
                return set_cr_error(retcode);
            }
            yield {
                ...
                if (info.syncstopped) {
                    ...
                    call(new
                         RGWSimpleRadosWriteAttrsCR(sync_env->async_rados,
                                                    store, obj, attrs));
                }
            }
            if (info.syncstopped) {
...
template < class T >
    static void decode_attr(CephContext * cct, map < string,
...
int RGWReadBucketSyncStatusCoroutine::operate()
{
    reenter(this) {
        yield call(new
                   RGWSimpleRadosReadAttrsCR(sync_env->async_rados,
                                             sync_env->store,
                                             rgw_raw_obj(sync_env->store->
                                                         get_zone_params().
                                                         log_pool, oid),
                                             &attrs));
        if (retcode == -ENOENT) {
...
class RGWReadRecoveringBucketShardsCoroutine:public RGWCoroutine {
  ...
  public:
    RGWReadRecoveringBucketShardsCoroutine(RGWDataSyncEnv * _sync_env,
                                           const int _shard_id,
                                           set < string > &_recovering_buckets,
                                           const int _max_entries)
    :RGWCoroutine(_sync_env->cct), sync_env(_sync_env), store(sync_env->store),
        shard_id(_shard_id), max_entries(_max_entries),
...
int RGWReadRecoveringBucketShardsCoroutine::operate()
{
    reenter(this) {
        ...
        do {
            yield call(new
                       RGWRadosGetOmapKeysCR(store,
                                             rgw_raw_obj(store->
                                                         get_zone_params().
                                                         log_pool, error_oid),
                                             marker, &error_entries,
                                             max_omap_entries));
            ...
            recovering_buckets.insert(error_entries.begin(),
                                      error_entries.end());
        } while ((int)error_entries.size() == max_omap_entries
                 && count < max_entries);
...
class RGWReadPendingBucketShardsCoroutine:public RGWCoroutine {
  ...
  public:
    RGWReadPendingBucketShardsCoroutine(RGWDataSyncEnv * _sync_env,
                                        const int _shard_id,
...
int RGWReadPendingBucketShardsCoroutine::operate()
{
    reenter(this) {
        ...
        do {
            yield call(new
                       RGWReadRemoteDataLogShardCR(sync_env, shard_id, marker,
...
int RGWRemoteDataLog::read_shard_status(int shard_id,
                                        set < string > &pending_buckets,
                                        set < string > &recovering_buckets,
                                        rgw_data_sync_marker * sync_marker,
                                        const int max_entries)
{
    ...
    RGWCoroutinesStack *recovering_stack =
        new RGWCoroutinesStack(store->ctx(), &crs);
    recovering_stack->
        call(new
             RGWReadRecoveringBucketShardsCoroutine(&sync_env_local, shard_id,
                                                    recovering_buckets,
                                                    max_entries));
    ...
    pending_stack->
        call(new
             RGWReadPendingBucketShardsCoroutine(&sync_env_local, shard_id,
...
RGWCoroutine *RGWRemoteBucketLog::
read_sync_status_cr(rgw_bucket_shard_sync_info * sync_status)
{
...
{
    for (map < int, RGWRemoteBucketLog * >::iterator iter = source_logs.begin();
         iter != source_logs.end(); ++iter) {
        delete iter->second;
    }
    delete error_logger;
}
...
class RGWListBucketIndexLogCR:public RGWCoroutine {
  ...
  public:
    RGWListBucketIndexLogCR(RGWDataSyncEnv * _sync_env,
                            const rgw_bucket_shard & bs, string & _marker,
        ...
        reenter(this) {
            yield {
                ...
                call(new RGWReadRESTResourceCR < list < rgw_bi_log_entry >
                     >(sync_env->cct, sync_env->conn, sync_env->http_manager,
...
class RGWBucketFullSyncShardMarkerTrack:public RGWSyncShardMarkerTrack <
    rgw_obj_key, rgw_obj_key > {
    RGWDataSyncEnv *sync_env;
  ...
  public:
    RGWBucketFullSyncShardMarkerTrack(RGWDataSyncEnv * _sync_env,
                                      ...
                                      const rgw_bucket_shard_full_sync_marker &
                                      _marker):RGWSyncShardMarkerTrack
        (BUCKET_SYNC_UPDATE_MARKER_WINDOW), sync_env(_sync_env),
        marker_oid(_marker_oid), sync_marker(_marker) {
...
template < class T, class K > class RGWBucketSyncSingleEntryCR:public RGWCoroutine
{
  ...
  public:
    RGWBucketSyncSingleEntryCR(RGWDataSyncEnv * _sync_env,
                               ...
                               K > *_marker_tracker,
                               rgw_zone_set &
                               _zones_trace):RGWCoroutine(_sync_env->cct),
        sync_env(_sync_env), bucket_info(_bucket_info), bs(bs), key(_key),
        versioned(_versioned), versioned_epoch(_versioned_epoch), owner(_owner),
        timestamp(_timestamp), op(_op), op_state(_op_state),
    ...
    int operate() override {
        reenter(this) {
                ...
                yield {
                    ...
                    if (error_injection &&
                        ...
                        ldout(sync_env->cct,
                            ...
                            source_zone << "/" << bucket_info->
                            bucket << "/" << key << "[" << versioned_epoch.
                            value_or(0) << "]" << dendl;
                        logger.log("fetch");
                        call(data_sync_module->
                             sync_object(sync_env, *bucket_info, key,
                                         versioned_epoch, &zones_trace));
                    }
                        ...
                        logger.log("remove");
                        call(data_sync_module->
                             remove_object(sync_env, *bucket_info, key,
                                           timestamp, versioned,
                        ...
                        ldout(sync_env->cct,
                              10) << "creating delete marker: obj: " <<
                            sync_env->source_zone << "/" << bucket_info->
                            bucket << "/" << key << "[" << versioned_epoch.
                            value_or(0) << "]" << dendl;
                        call(data_sync_module->
                             create_delete_marker(sync_env, *bucket_info, key,
            ...
            if (!error_ss.str().empty()) {
                yield call(sync_env->error_logger->
                           log_error_cr(sync_env->conn->get_remote_id(), "data",
...
int RGWBucketShardFullSyncCR::operate()
{
    ...
    reenter(this) {
        ...
        do {
            ...
            ldout(sync_env->cct,
                  20) << __func__ << "(): listing bucket for full sync" <<
                dendl;
            yield call(new
                       RGWListBucketShardCR(sync_env, bs, list_marker,
                                            &list_result));
            if (retcode < 0 && retcode != -ENOENT) {
            ...
            for (; entries_iter != list_result.entries.end(); ++entries_iter) {
                ...
                if (!marker_tracker.
                    ...
                    ldout(sync_env->cct,
                        ...
                        key << ". Duplicate entry?" << dendl;
                }
                else {
                    using SyncCR =
                        RGWBucketSyncSingleEntryCR < rgw_obj_key, rgw_obj_key >;
                    yield spawn(new SyncCR(sync_env, bucket_info, bs, entry->key, false,    /* versioned, only matters for object removal */
                                           entry->versioned_epoch, entry->mtime,
                                           entry->owner, entry->get_modify_op(),
                                           CLS_RGW_STATE_COMPLETE, entry->key,
                                           &marker_tracker, zones_trace),
                                false);
                }
        ...
        if (sync_status == 0) {
            yield {
                ...
                RGWRados *store = sync_env->store;
                call(new
                     RGWSimpleRadosWriteAttrsCR(sync_env->async_rados, store,
                                                rgw_raw_obj(store->
                                                            get_zone_params().
                                                            log_pool,
                                                            status_oid),
            ...
            ldout(sync_env->cct,
                  10) << "failure in sync, backing out (sync_status=" <<
                sync_status << ")" << dendl;
        }
...
int RGWBucketShardIncrementalSyncCR::operate()
{
    ...
    reenter(this) {
        do {
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
                    ldout(sync_env->cct,
                          20) << "syncstop on " << e.timestamp << dendl;
                    syncstopped = true;
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                if (!key.set(rgw_obj_index_key {
                    ...
                    ldout(sync_env->cct,
                        ...
                        object << " returned false, skipping entry" << dendl;
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                }
                ...
                ldout(sync_env->cct,
                      20) << "parsed entry: id=" << cur_id << " iter->object="
                    << entry->object << " iter->instance=" << entry->
                    instance << " name=" << key.name << " instance=" << key.
                    instance << " ns=" << key.ns << dendl;
                ...
                if (entry->state != CLS_RGW_STATE_COMPLETE) {
                    ...
                    ldout(sync_env->cct, 20) << "[inc sync] skipping object: "
                        << bucket_shard_str {
                    bs}
                    <<"/" << key << ": non-complete operation" << dendl;
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                }
                if (entry->zones_trace.find(zone_id) !=
                    entry->zones_trace.end()) {
                    set_status() << "redundant operation, skipping";
                    ldout(sync_env->cct, 20) << "[inc sync] skipping object: "
                        << bucket_shard_str {
                    bs}
                    <<"/" << key << ": redundant operation" << dendl;
                    marker_tracker.try_update_high_marker(cur_id, 0,
                                                          entry->timestamp);
                    continue;
                }
                ...
                while (!marker_tracker.can_do_op(key, has_olh_epoch(entry->op))) {
                    ...
                    ldout(sync_env->cct,
                          5) << *this << ": [inc sync] can't do op on key=" <<
                        key <<
                ...
                if (!marker_tracker.start(cur_id, 0, entry->timestamp)) {
                    ...
                    ldout(sync_env->cct,
                          20) << __func__ << "(): entry->timestamp=" << entry->
                        timestamp << dendl;
                    using SyncCR =
                        RGWBucketSyncSingleEntryCR < string, rgw_obj_key >;
                    spawn(new
                          SyncCR(sync_env, bucket_info, bs, key,
                                 entry->is_versioned(), versioned_epoch,
                                 entry->timestamp, owner, entry->op,
                                 entry->state, cur_id, &marker_tracker,
                                 entry->zones_trace), false);
                }
                // }
                while (num_spawned() > BUCKET_SYNC_SPAWN_WINDOW) {
                    set_status() << "num_spawned() > spawn_window";
        ...
        if (retcode < 0) {
            ldout(sync_env->cct,
                  0) << "ERROR: marker_tracker.flush() returned retcode=" <<
                retcode << dendl;
            return set_cr_error(retcode);
        }
        if (sync_status < 0) {
...
int RGWRunBucketSyncCoroutine::operate()
{
    reenter(this) {
        yield {
            ...
            lease_cr.
                reset(new
                      RGWContinuousLeaseCR(sync_env->async_rados, store,
                                           rgw_raw_obj(store->get_zone_params().
                                                       log_pool, status_oid),
                                           "sync_lock",
        ...
        yield call(new
                   RGWReadBucketSyncStatusCoroutine(sync_env, bs,
                                                    &sync_status));
        if (retcode < 0 && retcode != -ENOENT) {
            ldout(sync_env->cct,
        ...
        ldout(sync_env->cct, 20) << __func__ << "(): sync status for bucket "
            << bucket_shard_str {
        bs}
        <<": " << sync_status.state << dendl;

        yield call(new
                   RGWGetBucketInstanceInfoCR(sync_env->async_rados,
                                              sync_env->store, bs.bucket,
                                              &bucket_info));
        if (retcode == -ENOENT) {
            ...
            yield {
                ...
                string raw_key =
                    string("bucket.instance:") + bs.bucket.get_key();

                meta_sync_env.init(cct, sync_env->store,
            ...
            if (retcode < 0) {
                ldout(sync_env->cct,
                      0) << "ERROR: failed to fetch bucket instance info for "
                    << bucket_str {
                bs.bucket}
                <<dendl;
                lease_cr->go_down();
        ...
        if (retcode < 0) {
            ldout(sync_env->cct,
                  0) << "ERROR: failed to retrieve bucket info for bucket=" <<
                bucket_str {
            bs.bucket}
            <<dendl;
            lease_cr->go_down();
        ...
        do {
            if (sync_status.state == rgw_bucket_shard_sync_info::StateInit) {
                yield call(new
                           RGWInitBucketShardSyncStatusCoroutine(sync_env, bs,
                                                                 sync_status));
                if (retcode == -ENOENT) {
            ...
            if (sync_status.state == rgw_bucket_shard_sync_info::StateFullSync) {
                yield call(new
                           RGWBucketShardFullSyncCR(sync_env, bs, &bucket_info,
            ...
            if (sync_status.state ==
                rgw_bucket_shard_sync_info::StateIncrementalSync) {
                yield call(new
                           RGWBucketShardIncrementalSyncCR(sync_env, bs,
                                                           &bucket_info,
                                                           status_oid,
                                                           lease_cr.get(),
                                                           sync_status));
...
int RGWBucketSyncStatusManager::init()
{
    ...
    ret =
        cr_mgr.run(new RGWReadRESTResourceCR < bucket_instance_meta_info >
                   (store->ctx(), conn, &http_manager, path, pairs, &result));
    if (ret < 0) {
        ldout(store->ctx(),
              0) << "ERROR: failed to fetch bucket metadata info from zone=" <<
            source_zone << " path=" << path << " key=" << key << " ret=" << ret
            << dendl;
        return ret;
    }
    ...
    for (int i = 0; i < effective_num_shards; i++) {
        RGWRemoteBucketLog *l =
            new RGWRemoteBucketLog(store, this, async_rados, &http_manager);
        ret =
            l->init(source_zone, conn, bucket, (num_shards ? i : -1),
                    error_logger, sync_module);
        if (ret < 0) {
            ldout(store->ctx(),
                  0) << "ERROR: failed to initialize RGWRemoteBucketLog object"
                << dendl;
            return ret;
        }
        source_logs[i] = l;
...
int RGWBucketSyncStatusManager::init_sync_status()
{
    ...
    for (map < int, RGWRemoteBucketLog * >::iterator iter = source_logs.begin();
        ...
        RGWCoroutinesStack *stack =
            new RGWCoroutinesStack(store->ctx(), &cr_mgr);
...
class RGWCollectBucketSyncStatusCR:public RGWShardCollectCR {
  ...
  public:
    RGWCollectBucketSyncStatusCR(RGWRados * store, RGWDataSyncEnv * env,
                                 ...
                                 Vector * status)
    :RGWShardCollectCR(store->ctx(), max_concurrent_shards), store(store), env(env), num_shards(num_shards), bs(bucket, num_shards > 0 ? 0 : -1),   // start at shard 0 or -1
...
class DataLogTrimCR:public RGWCoroutine {
  ...
  public:
    ...
    :RGWCoroutine(store->ctx()), store(store), http(http),
        ...
        min_shard_markers(num_shards), last_trim(last_trim) {
    } int operate() override;
};
    ...
    reenter(this) {
        ...
        yield {
          ...
          for (auto & c:store->zone_conn_map) {
                ...
                spawn(new
                      StatusCR(cct, c.second, http, "/admin/log/", params, &*p),
                      false);
                ++p;
            }
        }
        ...
        for (;;) {
            ...
            if (retcode < 0) {
                ...
                ldout(cct,
                    ...
                    ", trying again in " << interval.sec() << "s" << dendl;
                continue;
            }
