...
boost::asio::awaitable < void > expect_error_code(auto&& coro, auto ...ecs)
{
    bool failed = false;
    try {
...
protected:
    ...
    /// \warning This function should only be called from test bodies
    /// (i.e. after `CoSetUp()`)
    const neorados::IOContext &pool() const noexcept
    {
        return pool_;
    }
    ...
    /// \warning This function should only be called from test bodies
    /// (i.e. after `CoSetUp()`)
    const DoutPrefixProvider *dpp() const noexcept
    {
        return dpp_.get();
    }
    ...
    boost::asio::awaitable < ceph::buffer::list > read(std::string_view oid, std::uint64_t off = 0, std::uint64_t len = 0)
    {
        ceph::buffer::list bl;
        neorados::ReadOp op;
        op.read(off, len, &bl);
    ...
    boost::asio::awaitable < ceph::buffer::list > read(std::string_view oid, neorados::IOContext ioc, std::uint64_t off = 0,
            std::uint64_t len = 0)
    {
        ceph::buffer::list bl;
        neorados::ReadOp op;
    ...
    boost::asio::awaitable < void > create_obj(std::string_view oid)
    {
        neorados::WriteOp op;
        op.create(true);
