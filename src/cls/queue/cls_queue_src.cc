    ...
    for (auto &bl_data : op.bl_data_vec) {
        ...
        if (head.tail.offset >= head.front.offset) {
            ...
            if ((head.tail.offset + bl.length()) <= head.queue_size) {
                CLS_LOG(5, "INFO: queue_enqueue: Writing data size and data: offset: %s, size: %u", head.tail.to_str().c_str(),
                        bl.length());
                ...
                if (bl.length() <= free_space_available) {
                    ...
                    CLS_LOG(5, "INFO: queue_enqueue: Writing spliced data at offset: %s and data size: %u", head.tail.to_str().c_str(),
                            bl_data_before_wrap.length());
                    auto ret = cls_cxx_write2(hctx, head.tail.offset, bl_data_before_wrap.length(), &bl_data_before_wrap,
                    ...
                    CLS_LOG(5, "INFO: queue_enqueue: Writing remaining data at offset: %s and data size: %u", head.tail.to_str().c_str(),
                            bl.length());
                       ...
                       cls_queue_head &head)
{
    ...
    CLS_LOG(10, "INFO: queue_list_entries(): front is: %s, tail is %s", head.front.to_str().c_str(),
...
int queue_remove_entries(cls_method_context_t hctx, const cls_queue_remove_op &op, cls_queue_head &head)
{
    ...
    if (end_marker.offset > head.front.offset && end_marker.gen == head.front.gen) {
        ...
        CLS_LOG(0, "INFO: queue_remove_entries: Invalid end marker: offset = %s, gen = %lu", end_marker.to_str().c_str(),
                end_marker.gen);
        return -EINVAL;
    }
    ...
    CLS_LOG(20, "INFO: queue_remove_entries: front offset is: %s and tail offset is %s", head.front.to_str().c_str(),
            head.tail.to_str().c_str());
