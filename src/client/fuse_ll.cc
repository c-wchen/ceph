...
public:
    ...
    ceph::unordered_map < uint64_t,
         struct ceph_fuse_fake_inode_stag >g_fino_maps;
...
static int already_fuse_mounted(const char *path, bool &already_mounted)
{
    ...
    if (-1 ==
        statx(AT_FDCWD, parent_path, AT_STATX_DONT_SYNC, STATX_INO,
              &parent_statx)) {
        err = errno;
    ...
    if (path_statx.stx_dev_major != parent_statx.stx_dev_major ||
        ...
        if (FUSE_SUPER_MAGIC == path_statfs.f_type) {
            ...
            if (getxattr(path, _CEPH_CLIENT_ID, &client_id, sizeof(client_id)) >
                0) {
                          ...
                          mode_t mode)
{
    ...
    if (cfuse->fino_snap(parent) == CEPH_SNAPDIR && fuse_multithreaded
        ...
        int err = 0;
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
        int fd =::open(cfuse->opts.mountpoint,
                         ...
                         struct fuse_file_info *fi)
{
    ...
    if (r >= 0) {
        ...
        bl.prepare_iov(&iov);
        len =
            sizeof(struct fuse_bufvec) + sizeof(struct fuse_buf) * (iov.size() -
                1);
        ...
        iov.insert(iov.begin(), {
            0
        }
                  );                      // the first one is reserved for fuse_out_header
...
CephFuse::Handle::Handle(Client *c, int fd): fd_on_success(fd), client(c)
{
...
int CephFuse::Handle::init(int argc, const char *argv[])
{
    ...
    auto fuse_allow_other =
        client->cct->_conf.get_val < bool > ("fuse_allow_other");
    auto fuse_default_permissions =
        client->cct->_conf.get_val < bool > ("fuse_default_permissions");
#if FUSE_VERSION < FUSE_MAKE_VERSION(3, 0)
    auto fuse_big_writes =
        client->cct->_conf.get_val < bool > ("fuse_big_writes");
#endif
    auto fuse_max_write =
    ...
    auto fuse_splice_read =
        client->cct->_conf.get_val < bool > ("fuse_splice_read");
    auto fuse_splice_write =
        client->cct->_conf.get_val < bool > ("fuse_splice_write");
    auto fuse_splice_move =
        client->cct->_conf.get_val < bool > ("fuse_splice_move");
...
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
    derr << "init, args.argv = " << args.argv << " args.argc=" << args.
         argc << dendl;
    ...
    struct ceph_client_callback_args cb_args = {
        ...
        this,
        ino_cb:
        client->cct->_conf.get_val < bool > ("fuse_use_invalidate_cb") ?
    ...
    if (client->use_faked_inos()) {
        ...
        if (snapid == CEPH_NOSNAP) {
            ...
            if (fino_maps.snap_stag_map.count(snapid)) {
                inodeno_t fino =
                    MAKE_FINO(ino, fino_maps.snap_stag_map[snapid]);
                return fino;
            }
...
std::string CephFuse::get_mount_point() constconst {
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
