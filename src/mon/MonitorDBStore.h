...
public:
    ...
    std::shared_ptr < PriorityCache::PriCache > get_priority_cache()const
    {
        ...
        Op(int t, const std::string &p, const std::string &start,
        ...
        } int approx_size() const
        {
            return 6 + 1 +
                   4 + prefix.size() +
                   4 + key.size() + 4 + endkey.size() + 4 + bl.length();
    ...
    struct Transaction {
        ...
        void put(const std::string &prefix, const std::string &key,
        ...
        void put(const std::string &prefix, version_t ver,
                 const ceph::buffer::list &bl)
        {
    ...
    int apply_transaction(MonitorDBStore::TransactionRef t)
    {
        ...
        std::list < std::pair < std::string, std::pair < std::string,
            std::string >>> compact;
        for (auto it = t->ops.begin(); it != t->ops.end(); ++it) {
            ...
            switch (op.type) {
                ...
                case Transaction::OP_COMPACT:
                    compact.
                    push_back(make_pair
                              (op.prefix, make_pair(op.key, op.endkey)));
                    break;
                default:
        ...
        if (r >= 0) {
            while (!compact.empty()) {
                if (compact.front().second.first == std::string() &&
                    ...
                    db->compact_range_async(compact.front().first,
                                            compact.front().second.first,
    ...
    struct C_DoTransaction: public Context {
        ...
        } void finish(int r) override
        {
            ...
            if (delay_prob && (rand() % 10000 < delay_prob * 10000.0)) {
                ...
                delay.set_from_double(delay_max * (double)(rand() % 10000) /
                                      10000.0);
    ...
    int get(const std::string &prefix, const version_t ver,
            ceph::buffer::list &bl)
    {
        std::ostringstream os;
        os << ver;
    ...
    std::string combine_strings(const std::string &prefix,
    ...
    void _open(const std::string &kv_type)
    {
        ...
        if (!db_ptr) {
            derr << __func__ << " error initializing "
                 << kv_type << " db back storage in " << full_path << dendl;
            ceph_abort_msg
        ...
        if (g_conf()->mon_debug_dump_transactions) {
            if (!g_conf()->mon_debug_dump_json) {
                dump_fd_binary =::open(g_conf()->mon_debug_dump_location.
                                       c_str(),
    ...
    int open(std::ostream &out)
    {
        ...
        if (db->get_perf_counters()) {
            db->get_perf_counters()->
            set_prio_adjust(PerfCountersBuilder::PRIO_USEFUL -
    ...
    explicit MonitorDBStore(const std::string &path)
        : path(path),
          ...
          dump_fd_binary(-1),
          dump_fmt(true),
          io_work(g_ceph_context, "monstore", "fn_monstore"), is_open(false)
    {
    }
    ~MonitorDBStore()
    {
