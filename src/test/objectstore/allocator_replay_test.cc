...
{
    cerr << "Usage: " << name <<
        " <log_to_replay> <raw_duplicates|duplicates|free_dump|try_alloc count want alloc_unit|replay_alloc alloc_list_file|export_binary out_file>"
        << std::endl;
}
...
void usage_replay_alloc(const string & name)
{
    cerr << "Detailed replay_alloc usage: " << name <<
        " <allocator_dump_JSON> replay_alloc <alloc_list_file> [number of replays]"
        << std::endl;
    cerr << "The number of replays defaults to 1." << std::endl;
    cerr <<
        "The \"alloc_list_file\" parameter should be a file with allocation requests, one per line."
        << std::endl;
    cerr <<
...
int replay_and_check_for_duplicate(char *fname)
{
    ...
    while (true) {
        ...
        if (sp) {
            ...
            std::cout << s << std::endl;
            if (!init_done) {
                std::cerr << "error: no allocator init before: " << s << std::
                    endl;
                return -1;
            }
            uint64_t offs, len;
        ...
        if (sp) {
          ...
          for (auto & ee:tmp) {
                std::cerr << "dump extent: " << std::hex
                    << ee.offset << "~" << ee.length << std::dec << std::endl;
            }
            std::cerr << "dump completed." << std::endl;
          for (auto & e:tmp) {
                if (owned_by_app.intersects(e.offset, e.length)) {
                    std::cerr << "error: unexpected allocated extent: " << std::
                        hex << e.offset << "~" << e.
                        length << " dumping all allocations:" << std::
                        dec << std::endl;
                  for (auto & ee:tmp) {
                        std::cerr << "dump extent: " << std::hex
                            << ee.offset << "~" << ee.length
...
int replay_free_dump_and_apply_raw(char *fname,
                                   std::function < void (std::string_view,
                                                         ...
                                                         int64_t,
                                                         std::string_view) >
                                   create, std::function < void (uint64_t,
                                                                 uint64_t) >
                                   add_ext)
{
    string alloc_type;
...
int replay_free_dump_and_apply(char *fname,
                               std::function < int (Allocator *,
                                                    const string & aname) > fn)
{
    unique_ptr < Allocator > alloc;
    auto create_fn =[&](std::string_view alloc_type,
        ...
        alloc.
            reset(Allocator::
                  create(g_ceph_context, alloc_type, capacity, alloc_unit, 0, 0,
                         alloc_name));
    };
    ...
    int r = replay_free_dump_and_apply_raw(fname,
                                           create_fn,
                                           add_fn);
    if (r == 0) {
...
void dump_alloc(Allocator * alloc, const string & aname)
{
    ...
    auto r = admin_socket->execute_command({ cmd },
                                           in, err, &out);
    if (r != 0) {
...
int export_as_binary(char *fname, char *target_fname)
{
    ...
    binary_alloc_map_t amap;
    auto dummy_create_fn =[&](std::string_view alloc_type,
                              int64_t capacity,
                              int64_t alloc_unit,
                              std::string_view alloc_name) {
    };
...
int check_duplicates(char *fname)
{
    ...
    while (itA != invalid_extentsA.end()) {
        ...
        while (itB != invalid_extentsB.end() &&
            ...
            std::cerr << " " << itB.get_start() << "~" << itB.get_end() -
                itB.get_start();
            ++itB;
        }
    ...
    if (strcmp(argv[2], "raw_duplicates") == 0) {
        ...
        return replay_free_dump_and_apply(argv[1],
                                          [&](Allocator * a,
                                              const string & aname) {
                                          ceph_assert(a);
                                          std::cout << "Fragmentation:" << a->
                                          get_fragmentation()
                                          << std::endl;
                                          std::
                                          cout << "Fragmentation score:" << a->
                                          get_fragmentation_score()
                                          << std::endl;
                                          std::cout << "Free:" << std::
                                          hex << a->
                                          get_free() << std::dec << std::endl; {
        ...
        if (argc < 6) {
            std::
                cerr <<
                "Error: insufficient arguments for \"try_alloc\" operation." <<
                std::endl;
            usage(argv[0]);
            return 1;
        }
        ...
        return replay_free_dump_and_apply(argv[1],
                                          ...
                                          extents.clear();
                                          auto r =
                                          a->allocate(want, alloc_unit, 0,
                                                      &extents); if (r < 0) {
                                          std::
                                          cerr <<
                                          "Error: allocation failure at step:"
                                          << i +
                                          1 << ", ret = " << r << std::endl;
                                          return -1;}
                                          }
        ...
        if (argc < 4) {
            std::
                cerr <<
                "Error: insufficient arguments for \"replay_alloc\" option." <<
                std::endl;
        ...
        return replay_free_dump_and_apply(argv[1],
                                          ...
                                          argv[3] << std::endl; return -1;}

                                          /* Replay user specified number of times to simulate extended activity
                                           * Defaults to 1 replay.
                                           */
                                          auto replay_count = 1; if (argc == 5) {
                                          replay_count = atoi(argv[4]);}

                                          for (auto i = 0; i < replay_count;
                                               ++i) {
                                          while (fgets
                                                 (s, sizeof(s),
                                                  f_alloc_list) != nullptr) {
                                          /* parse allocation request */
                                          uint64_t want = 0, unit = 0, max =
                                          0, hint = 0;
                                          if (std::
                                              sscanf(s, "%ji %ji %ji %ji",
                                                     &want, &unit, &max,
                                                     &hint) < 2) {
                                          cerr <<
                                          "Error: malformed allocation request:"
                                          << std::endl; cerr << s << std::endl;
                                          ...
                                          PExtentVector extents;
                                          auto r =
                                          a->allocate(want, unit, max, hint,
                                                      &extents); if (r < 0) {
                                          /* blind replays of allocations may run out of space, provide info for easy confirmation */
                                          std::
                                          cerr <<
                                          "Error: allocation failure code: " <<
                                          r <<
                                          " requested want/unit/max/hint (hex): "
                                          << std::
                                          hex << want << "/" << unit << "/" <<
                                          max << "/" << hint << std::
                                          dec << std::endl;
                                          std::cerr << "Fragmentation:" << a->
                                          get_fragmentation()
                                          << std::endl;
                                          std::
                                          cerr << "Fragmentation score:" << a->
                                          get_fragmentation_score()
                                          << std::endl;
                                          std::cerr << "Free:" << std::
                                          hex << a->
                                          ...
                                          std::
                                          cout << "Duration (ns): " << (ceph::
                                                                        mono_clock::
                                                                        now() -
                                                                        t0).
                                          count()
                                          << " want/unit/max/hint (hex): " <<
                                          std::
                                          hex << want << "/" << unit << "/" <<
                                          max << "/" << hint << std::
                                          dec << std::endl;
