...
static inline int stringcasecmp(const std::string & s1, int ofs, int size,
                                const std::string & s2)
{
...
// variadic sum() to add up string lengths for reserve()
    static inline constexpr size_t sum() {
        ...
        constexpr size_t sum(size_t v, Args ... args) {
        return v + sum(args ...);
    }

// traits for string_size()
    template < typename T > struct string_traits {
        static constexpr size_t size(const T & s) {
            return s.size();
    }};
...
// constexpr specializations for char[]/const char[]
    template < std::size_t N > struct string_traits <const char[N] > {
        ...
        } static constexpr size_t size(const char (&s)[N]) {
            return size_(s, 0);
        }
    };
...
// helpers for string_cat_reserve()
    ...
    template < typename ... Args >
        void append_to(std::string & s, const boost::string_view & v,
                       const Args & ... args) {
...
{
    size_t delim_size =
        delim.size() * std::max < ssize_t > (0, sizeof ... (args) - 1);
...
{
    return string_join_reserve(boost::string_view {
                               &delim, 1}
                               , args ...);
}
...
extern bool match_wildcards(boost::string_view pattern,
                            boost::string_view input, uint32_t flags = 0);
