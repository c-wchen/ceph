    ...
    namespace action {
        namespace snap {
            ...
            int do_list_snaps(librbd::Image & image, Formatter * f,
                              bool all_snaps, librados::Rados & rados) {
                ...
                if (!all_snaps) {
                    ...
                    if (r < 0) {
                        std::cerr << "rbd: unable to get image info" << std::
                            endl;
                        return r;
                }} if (f) {
                    ...
                    t.define_column("SNAPID", TextTable::LEFT,
                                    TextTable::RIGHT);
                    t.define_column("NAME", TextTable::LEFT, TextTable::LEFT);
                    t.define_column("SIZE", TextTable::LEFT, TextTable::RIGHT);
                    t.define_column("PROTECTED", TextTable::LEFT,
                                    TextTable::LEFT);
                    t.define_column("TIMESTAMP", TextTable::LEFT,
                                    TextTable::RIGHT);
                    if (all_snaps) {
                        t.define_column("NAMESPACE", TextTable::LEFT,
                                        TextTable::LEFT);
                    }
                }
                ...
                std::map < int64_t, std::string > pool_map(pool_list.begin(),
                                                           pool_list.end());

                for (std::vector < librbd::snap_info_t >::iterator s =
                     snaps.begin(); s != snaps.end(); ++s) {
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: unable to retrieve snap namespace" <<
                            std::endl;
                        return r;
                    }
                    ...
                    if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_GROUP) {
                        get_group_res =
                            image.snap_get_group_namespace(s->id, &group_snap,
                                                           sizeof(group_snap));
                    }
                    else if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_TRASH) {
                        get_trash_res =
                            image.snap_get_trash_namespace(s->id,
                                                           &trash_original_name);
                    }
                    else if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_MIRROR) {
                        get_mirror_res =
                            image.snap_get_mirror_namespace(s->id, &mirror_snap,
                                                            sizeof
                                                            (mirror_snap));
                    ...
                    if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_USER) {
                        r = image.snap_is_protected(s->name.c_str(),
                                                    &snap_protected);
                        if (r < 0) {
                            std::
                                cerr <<
                                "rbd: unable to retrieve snap protection" <<
                                std::endl;
                            return r;
                        }
                    }
                    ...
                    if (f) {
                        ...
                        t << s->id << s->
                            name << stringify(byte_u_t(s->size)) <<
                            protected_str << tt_str;
                        ...
                        if (all_snaps) {
                            ...
                            if (get_group_res == 0) {
                                std::string pool_name =
                                    pool_map[group_snap.group_pool];
                                oss << " (" << pool_name << "/" << group_snap.
                                    group_name << "@" << group_snap.
                                    group_snap_name << ")";
                            }
                            else if (get_trash_res == 0) {
            ...
            int do_rollback_snap(librbd::Image & image, const char *snapname,
                                 bool no_progress) {
                utils::ProgressContext pc("Rolling back to snapshot",
                                          no_progress);
            ...
            int do_purge_snaps(librbd::Image & image, bool no_progress) {
                utils::ProgressContext pc("Removing all snapshots",
                                          no_progress);
                std::vector < librbd::snap_info_t > snaps;
                ...
                if (r < 0) {
                    ...
                    for (auto it = snaps.begin(); it != snaps.end();) {
                        r = image.snap_is_protected(it->name.c_str(),
                                                    &is_protected);
                        if (r < 0) {
                            pc.fail();
                            return r;
                    ...
                    if (!protect.empty()) {
                        std::
                            cout << "rbd: error removing snapshot(s) '" <<
                            protect << "', which " << (1 ==
                                                       protect.
                                                       size()? "is" : "are")
                    ...
                    for (size_t i = 0; i < snaps.size(); ++i) {
                        ...
                        pc.update_progress(i + 1,
                                           snaps.size() + protect.size());
                    }
            ...
            void get_list_arguments(po::options_description * positional,
                ...
                options->add_options()
                    (name.c_str(), po::bool_switch(),
                     "list snapshots from all namespaces");
            }
            ...
            int execute_list(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to list snapshots: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            void get_create_arguments(po::options_description * positional,
                                      po::options_description * options) {
                at::add_snap_spec_options(positional, options,
                                          at::ARGUMENT_MODIFIER_NONE);
            ...
            int execute_create(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to create snapshot: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_remove(const po::variables_map & vm,
                ...
                if (image_id.empty()) {
                    ...
                    r = utils::open_image_by_id(io_ctx, image_id, false,
                                                &image);
                }
                if (r < 0) {
                    return r;
                ...
                if (r < 0) {
                    if (r == -EBUSY) {
                        std::cerr << "rbd: snapshot "
                            << (snap_name.empty()? std::string("id ") +
                                stringify(snap_id) : std::string("'") +
                                snap_name + "'")
                            << " is protected from removal." << std::endl;
                    }
                    else {
                        std::
                            cerr << "rbd: failed to remove snapshot: " <<
                            cpp_strerror(r)
                            << std::endl;
                    }
                    return r;
            ...
            int execute_purge(const po::variables_map & vm,
                ...
                if (r < 0) {
                    if (r != -EBUSY) {
                        std::
                            cerr << "rbd: removing snaps failed: " <<
                            cpp_strerror(r)
                            << std::endl;
                    }
                    return r;
            ...
            int execute_rollback(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: rollback failed: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_protect(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: protecting snap failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_unprotect(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: unprotecting snap failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                else if (!is_protected) {
                    std::cerr << "rbd: snap is already unprotected" << std::
                        endl;
                    return -EINVAL;
                }
            ...
            void get_set_limit_arguments(po::options_description * pos,
                ...
                at::add_image_spec_options(pos, opt,
                                           at::ARGUMENT_MODIFIER_NONE);
            ...
            int execute_set_limit(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: setting snapshot limit failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_clear_limit(const po::variables_map & vm,
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: clearing snapshot limit failed: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            int execute_rename(const po::variables_map & vm,
                ...
                r = image.snap_rename(src_snap_name.c_str(),
                                      dest_snap_name.c_str());
                if (r < 0) {
                    std::
                        cerr << "rbd: renaming snap failed: " << cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                return 0;
