                   ...
                   RGWZoneGroup & zone_group)
{
  ...
  for (const auto & http2rgw:generic_attrs) {
    ...
    hostnames_set.insert(zone_group.hostnames.begin(),
                         zone_group.hostnames.end());
    ...
    hostnames_s3website_set.insert(zone_group.hostnames_s3website.begin(),
                                   zone_group.hostnames_s3website.end());
...
static bool rgw_find_host_in_domains(const string & host, string * domain,
                                     string * subdomain,
                                     set < string > valid_hostnames_set)
{
...
void dump_header(struct req_state *const s,
                 const boost::string_ref & name, const boost::string_ref & val)
{
    try {
...
void dump_header(struct req_state *const s,
                 const boost::string_ref & name, ceph::buffer::list & bl)
{
    return dump_header(s, name, get_sanitized_hdrval(bl));
}

void dump_header(struct req_state *const s,
                 const boost::string_ref & name, const long long val)
{
    char buf[32];
...
void dump_header(struct req_state *const s,
                 const boost::string_ref & name, const utime_t & ut)
{
    char buf[32];
...
void dump_etag(struct req_state *const s,
               const boost::string_ref & etag, const bool quoted)
{
    if (etag.empty()) {
        return;
    }
...
void dump_etag(struct req_state *const s,
               ceph::buffer::list & bl_etag, const bool quoted)
{
...
int RESTArgs::get_epoch(struct req_state *s, const string & name,
                        uint64_t def_val, uint64_t * epoch, bool * existed)
{
    bool exists;
...
#define COMPLETE_MULTIPART_MAX_LEN (1024 * 1024)    /* api defines max 10,000 parts, this should be enough */
    op_ret =
        rgw_rest_read_all_input(s, &data, &len, COMPLETE_MULTIPART_MAX_LEN);
    if (op_ret < 0)
        return op_ret;
...
int RGWHandler_REST::allocate_formatter(struct req_state *s,
                                        int default_type, bool configurable)
{
    ...
    if (configurable) {
        ...
        if (format_str.compare("xml") == 0) {
            ...
            if (accept) {
                ...
                unsigned int i = 0;
                for (;
                     i < sizeof(format_buf) - 1 && accept[i]
                     && accept[i] != ';'; ++i) {
                    format_buf[i] = accept[i];
                }
                format_buf[i] = 0;
                if ((strcmp(format_buf, "text/xml") == 0)
                    || (strcmp(format_buf, "application/xml") == 0)) {
                    s->format = RGW_FORMAT_XML;
                }
                                      ...
                                      int *const init_error)
{
    *init_error = preprocess(s, rio);
    ...
    RGWHandler_REST *handler =
        m->get_handler(s, auth_registry, frontend_prefix);
    if (!handler) {
        *init_error = -ERR_METHOD_NOT_ALLOWED;
        return NULL;
    }
