...
:  
store(ns),
cid(c), lock("KStore::Collection::lock", true, false), onode_map(store->cct)
{
}
        ...
        dout(10) << __func__ << " " << c->cid << dendl;
        {
            ...
            while (c->onode_map.get_next(next.first, &next)) {
                ...
                if (!next.second->flush_txns.empty()) {
                    dout(10) << __func__ << " " << c->cid << " " << next.
                        second->oid << " flush_txns " << next.second->
                        flush_txns << dendl;
                    return;
                }
            }
...
int KStore::stat(const coll_t & cid,
...
int KStore::set_collection_opts(const coll_t & cid, const pool_opts_t & opts)
{
    return -EOPNOTSUPP;
}

int KStore::read(const coll_t & cid,
                 ...
                 size_t length, bufferlist & bl, uint32_t op_flags)
{
  ...
  out:
    dout(10) << __func__ << " " << cid << " " << oid
        << " " << offset << "~" << length << " = " << r << dendl;
    return r;
}

int KStore::_do_read(OnodeRef o,
                     uint64_t offset,
                     size_t length, bufferlist & bl, uint32_t op_flags)
{
    int r = 0;
        ...
        if (stripe.length()) {
            if (swant == stripe.length()) {
                ...
                if (stripe_off < stripe.length()) {
                    ...
                    bl.claim_append(t);
                    dout(30) << __func__ << " taking " << stripe_off << "~" << l
                        << dendl;
                }
                if (l < swant) {
                    bl.append_zero(swant - l);
                    dout(30) << __func__ << " adding " << swant -
                        l << " zeros" << dendl;
                }
            }
        }
        else {
            dout(30) << __func__ << " generating " << swant << " zeros" <<
                dendl;
...
int KStore::fiemap(const coll_t & cid,
                   const ghobject_t & oid,
...
int KStore::getattr(const coll_t & cid,
                    const ghobject_t & oid, const char *name, bufferptr & value)
{
...
int KStore::getattrs(const coll_t & cid,
...
int KStore::collection_list(const coll_t & cid, const ghobject_t & start,
...
int KStore::omap_get(const coll_t & cid,    ///< [in] Collection containing oid
...
int KStore::omap_get_header(const coll_t & cid, ///< [in] Collection containing oid
                            const ghobject_t & oid, ///< [in] Object containing omap
                            bufferlist * header,    ///< [out] omap header
...
int KStore::omap_get_keys(const coll_t & cid,   ///< [in] Collection containing oid
                          const ghobject_t & oid,   ///< [in] Object containing omap
...
int KStore::omap_get_values(const coll_t & cid, ///< [in] Collection containing oid
                            const ghobject_t & oid, ///< [in] Object containing omap
...
int KStore::omap_check_keys(const coll_t & cid, ///< [in] Collection containing oid
                            const ghobject_t & oid, ///< [in] Object containing omap
...
ObjectMap::ObjectMapIterator KStore::get_omap_iterator(const coll_t & cid,  ///< [in] collection
...
void KStore::_osr_reap_done(OpSequencer * osr)
{
    ...
    while (!osr->q.empty()) {
        ...
        if (txc->first_collection) {
            txc->first_collection->onode_map.trim(cct->_conf->
...
void KStore::_kv_sync_thread()
{
    ...
    while (true) {
        ...
        if (kv_queue.empty()) {
            ...
            dout(30) << __func__ << " committing txc " << kv_committing <<
                dendl;

            // one transaction to force a sync
            KeyValueDB::Transaction t = db->get_transaction();
            if (!cct->_conf->kstore_sync_submit_transaction) {
                for (std::deque < TransContext * >::iterator it =
                     kv_committing.begin(); it != kv_committing.end(); ++it) {
...
int KStore::queue_transactions(Sequencer * posr,
    ...
    *_dout << dendl;
    ...
    for (int pos = 0; i.have_op(); ++pos) {
        ...
        if (!o) {
            ...
            if (!create) {
                if (!o || !o->exists) {
                    dout(10) << __func__ << " op " << op->
                        op << " got ENOENT on " << oid << dendl;
                    r = -ENOENT;
                    goto endop;
                }
            }
...
void KStore::_dump_onode(OnodeRef o)
{
    dout(30) << __func__ << " " << o
        ...
        << " expected_write_size " << o->onode.expected_write_size << dendl;
...
int KStore::_do_write(TransContext * txc,
                      ...
                      bufferlist & orig_bl, uint32_t fadvise_flags)
{
    int r = 0;
    ...
    while (length > 0) {
        ...
        if (offset_rem) {
            ...
            if (p) {
                dout(20) << __func__ << " reuse leading " << p << " bytes" <<
                    dendl;
                bl.substr_of(prev, 0, p);
            }
            if (p < offset_rem) {
                dout(20) << __func__ << " add leading " << offset_rem -
                    p << " zeros" << dendl;
        ...
        if (end_rem) {
            if (end_rem < prev.length()) {
                ...
                dout(20) << __func__ << " reuse trailing " << l << " bytes" <<
                    dendl;
                bufferlist t;
                          ...
                          uint64_t expected_write_size, uint32_t flags)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid
        ...
        << " write_size " << expected_write_size << " flags " << flags << dendl;
    int r = 0;
    ...
    dout(10) << __func__ << " " << c->cid << " " << o->oid
        << " object_size " << expected_object_size
        << " write_size " << expected_write_size << " = " << r << dendl;
    return r;
}
...
int KStore::_create_collection(TransContext * txc,
                               coll_t cid, unsigned bits, CollectionRef * c)
{
