...
Throttle::Throttle(CephContext * cct, const std::string & n, int64_t m,
                   bool _use_perf)
:cct(cct), name(n), logger(NULL), max(m), lock("Throttle::lock"),
use_perf(_use_perf)
{
    assert(m >= 0);
    ...
    if (cct->_conf->throttler_perf_counter) {
        ...
        b.add_u64_counter(l_throttle_get_started, "get_started",
...
bool Throttle::get_or_fail(int64_t c)
{
    ...
    if (_should_wait(c) || !cond.empty()) {
        ...
        ldout(cct,
              10) << "get_or_fail " << c << " success (" << count.
            load() << " -> " << (count.load() + c) << ")" << dendl;
        count += c;
        if (logger) {
...
:cct(cct), name(n), logger(NULL), conds(expected_concurrency),  ///< [in] determines size of conds
use_perf(_use_perf)
{
    if (!use_perf)
        return;
    ...
    if (cct->_conf->throttler_perf_counter) {
        PerfCountersBuilder b(cct, string("throttle-") + name,
                              ...
                              l_backoff_throttle_last);
        b.add_u64(l_backoff_throttle_val, "val",
        ...
        b.add_u64_counter(l_backoff_throttle_take_sum, "take_sum",
                          "Taken data");
...
bool BackoffThrottle::set_params(double _low_threshhold,
...
std::chrono::duration < double >BackoffThrottle::_get_delay(uint64_t c) const const
{
    if (max == 0)
    ...
    if (r < low_threshhold) {
        ...
        return c * std::chrono::duration < double >((r - low_threshhold) * s0);
    }
    else {
        return c * std::chrono::duration <
            double >(high_delay_per_count + ((r - high_threshhold) * s1));
    }
}
...
m_max(max), m_current(0), m_ret(0), m_ignore_enoent(ignore_enoent)
{
}
...
bool SimpleThrottle::pending_error() const const
{
    Mutex::Locker l(m_lock);
    return (m_ret < 0);
}
...
bool OrderedThrottle::pending_error() const const
{
    Mutex::Locker locker(m_lock);
    return (m_ret_val < 0);
}
