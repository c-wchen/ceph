...
ops_in_flight(0),
max_purge_ops(0),
drain_initial(0), draining(false), delayed_flush(nullptr), recovered(false)
{
    assert(cct != nullptr);
...
void PurgeQueue::_execute_item(const PurgeItem & item, uint64_t expire_to)
{
    ...
    if (item.action == PurgeItem::PURGE_FILE) {
      ...
      for (const auto & frag:frags) {
        ...
        filer.zero(item.ino, &item.layout, item.snapc,
                   ...
                   ceph::real_clock::now(), 0, true, gather.new_sub());
    }
    else {
    ...
    gather.
        set_finisher(new
                     C_OnFinisher(new
                                  FunctionContext([this, expire_to] (int r) {
                                                  Mutex::Locker l(lock);
...
void PurgeQueue::_execute_item_complete(uint64_t expire_to)
{
    ...
    if (iter == in_flight.begin()) {
        ...
        dout(10) << "expiring to 0x" << std::hex << expire_to << std::
            dec << dendl;
