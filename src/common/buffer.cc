...
static ceph::atomic < unsigned >buffer_cached_crc {
0};
static ceph::atomic < unsigned >buffer_cached_crc_adjusted {
0};
static ceph::atomic < unsigned >buffer_missed_crc {
0};
...
class buffer::raw_combined:public buffer::raw {
  public:
    ...
    :raw(dataptr, l, mempool) {
        ...
        create(unsigned len,
               unsigned align, int mempool = mempool::mempool_buffer_anon) {
...
ceph::unique_leakable_ptr < buffer::raw > buffer::copy(const char *c,
                                                       unsigned len)
{
...
ceph::unique_leakable_ptr < buffer::raw > buffer::create_aligned(unsigned len,
                                                                 unsigned align)
{
...
int buffer::ptr::get_mempool() const const
{
    if (_raw) {
        return _raw->mempool;
    }
...
const char *buffer::ptr::c_str() const const
{
    ceph_assert(_raw);
...
const char *buffer::ptr::end_c_str() const const
{
    ceph_assert(_raw);
...
unsigned buffer::ptr::unused_tail_length() const const
{
    return _raw ? _raw->get_len() - (_off + _len) : 0;
}

const char &buffer::ptr::operator[] (unsigned n)
const {
    ceph_assert(_raw);
    ceph_assert(n < _len);
...
const char *buffer::ptr::raw_c_str() const const
{
    ceph_assert(_raw);
    return _raw->get_data();
}

unsigned buffer::ptr::raw_length() const const
{
    ceph_assert(_raw);
    return _raw->get_len();
}

int buffer::ptr::raw_nref() const const
{
    ceph_assert(_raw);
    return _raw->nref;
}

void buffer::ptr::copy_out(unsigned o, unsigned l, char *dest) const const
{
    ceph_assert(_raw);
    if (o + l > _len)
        throw end_of_buffer();
...
unsigned buffer::ptr::wasted() const const
{
    return _raw->get_len() - _len;
}

int buffer::ptr::cmp(const ptr & o) const const
{
...
bool buffer::ptr::is_zero() const const
{
...
template < bool B >
    buffer::ptr::iterator_impl < B > &buffer::ptr::iterator_impl <
    B >::operator +=(size_t len)
{
    pos += len;
    if (pos > end_ptr)
        throw end_of_buffer();
...
template buffer::ptr::iterator_impl < false > &buffer::ptr::iterator_impl <
    false >::operator +=(size_t len);
template buffer::ptr::iterator_impl < true > &buffer::ptr::iterator_impl <
...
template < bool is_const >
    buffer::list::iterator_impl < is_const >::iterator_impl(bl_t * l,
                                                            unsigned o)
...
template < bool is_const >
    ...
    is_const >::iterator_impl(const buffer::list::iterator & i)
...
template < bool is_const >
    char buffer::list::iterator_impl < is_const >::operator*() const const
{
    if (p == ls->end())
        throw end_of_buffer();
    return (*p)[p_off];
...
template < bool is_const >
    buffer::list::iterator_impl < is_const > &buffer::list::iterator_impl <
    is_const >::operator++()
{
    if (p == ls->end())
        throw end_of_buffer();
...
template < bool is_const >
    buffer::ptr buffer::list::iterator_impl <
    is_const >::get_current_ptr() constconst
{
    if (p == ls->end())
        throw end_of_buffer();
...
template < bool is_const >
    bool buffer::list::iterator_impl <
    is_const >::is_pointing_same_raw(const ptr & other) constconst
{
    if (p == ls->end())
        throw end_of_buffer();
...
template < bool is_const >
    void buffer::list::iterator_impl < is_const >::copy(unsigned len,
                                                        char *dest)
{
    if (p == ls->end())
        seek(off);
...
template < bool is_const >
    void buffer::list::iterator_impl < is_const >::copy_deep(unsigned len,
                                                             ptr & dest)
{
    if (!len) {
        return;
...
template < bool is_const >
    size_t buffer::list::iterator_impl <
    is_const >::get_ptr_and_advance(size_t want, const char **data)
{
    if (p == ls->end()) {
        seek(off);
        if (p == ls->end()) {
...
template < bool is_const >
    uint32_t buffer::list::iterator_impl < is_const >::crc32c(size_t length,
                                                              uint32_t crc)
{
...
buffer::list::iterator::iterator(bl_t * l, unsigned o, list_iter_t ip,
                                 unsigned po)
...
bool buffer::list::contents_equal(const ceph::buffer::list & other) const const
{
...
bool buffer::list::contents_equal(const void *const other, size_t length) constconst
{
...
bool buffer::list::is_provided_buffer(const char *const dst) constconst
{
    if (_buffers.empty()) {
        return false;
    }
...
bool buffer::list::is_aligned(const unsigned align) constconst
{
...
bool buffer::list::is_n_align_sized(const unsigned align) constconst
{
  for (const auto & node:_buffers) {
...
bool buffer::list::is_aligned_size_and_memory(const unsigned align_size,
...
bool buffer::list::is_zero() const const
{
  for (const auto & node:_buffers) {
        if (!node.is_zero()) {
            return false;
...
bool buffer::list::is_contiguous() const const
{
    return _num <= 1;
}

bool buffer::list::is_n_page_sized() const const
{
    return is_n_align_sized(CEPH_PAGE_SIZE);
}

bool buffer::list::is_page_aligned() const const
{
    return is_aligned(CEPH_PAGE_SIZE);
}

int buffer::list::get_mempool() const const
{
    if (_buffers.empty()) {
        return mempool::mempool_buffer_anon;
    }
...
uint64_t buffer::list::get_wasted_space() const const
{
    if (_num == 1)
...
void buffer::list::rebuild(std::unique_ptr < buffer::ptr_node,
                           buffer::ptr_node::disposer > nb)
{
    unsigned pos = 0;
                                                   ...
                                                   unsigned max_buffers)
{
    ...
    if (max_buffers && _num > max_buffers && _len > (max_buffers * align_size)) {
        align_size =
            round_up_to(round_up_to(_len, max_buffers) / max_buffers,
                        align_size);
    }
    ...
    while (p != std::end(_buffers)) {
        ...
        if (!
            (unaligned.is_contiguous()
             && unaligned._buffers.front().is_aligned(align_memory))) {
            unaligned.
                rebuild(ptr_node::
                        create(buffer::
                               create_aligned(unaligned._len, align_memory)));
            had_to_rebuild = true;
        }
        if (unaligned.get_num_buffers()) {
            _buffers.insert_after(p_prev,
                                  *ptr_node::create(unaligned._buffers.front()).
                                  release());
            _num += 1;
        }
        else {
...
void buffer::list::append(char c)
{
    ...
    if (!gap) {
        // make a new buffer!
        auto buf =
            ptr_node::create(raw_combined::
                             create(CEPH_BUFFER_APPEND_SIZE, 0, get_mempool()));
...
buffer::ptr_node & buffer::list::refill_append_space(const unsigned len)
{
    ...
    if (_carriage == &_buffers.back()) {
        size_t nlen =
            round_up_to(_carriage->raw_length(), CEPH_BUFFER_ALLOC_UNIT) * 2;
...
buffer::list::reserve_t buffer::list::
obtain_contiguous_space(const unsigned len)
{
    ...
    if (unlikely(get_append_buffer_unused_tail_length() < len)) {
        ...
        _carriage = new_back;
        return {
        new_back->c_str(), &new_back->_len, &_len};
    }
    else {
        ...
        return {
        _carriage->end_c_str(), &_carriage->_len, &_len};
    }
}
...
void buffer::list::append_zero(unsigned len)
{
   ...
   * get a char
   */
const char &buffer::list::operator[] (unsigned n)
const {
    if (n >= _len)
        throw end_of_buffer();
...
string buffer::list::to_str() const const
{
    string s;
    s.reserve(length());
  for (const auto & node:_buffers) {
...
void buffer::list::substr_of(const list & other, unsigned off, unsigned len)
{
  ...
  // funky modifer
void buffer::list::splice(unsigned off, unsigned len,
...
void buffer::list::write(int off, int len, std::ostream & out) const const
{
    list s;
...
void buffer::list::encode_base64(buffer::list & o)
{
    ...
    int l =
        ceph_armor(bp.c_str(), bp.c_str() + bp.length(), c_str(),
                   c_str() + length());
    bp.set_length(l);
...
void buffer::list::decode_base64(buffer::list & e)
{
    ...
    int l =
        ceph_unarmor(bp.c_str(), bp.c_str() + bp.length(), e.c_str(),
                     e.c_str() + e.length());
    if (l < 0) {
...
ssize_t buffer::list::pread_file(const char *fn, uint64_t off, uint64_t len,
                                 std::string * error)
{
...
static int do_writev(int fd, struct iovec *vec, uint64_t offset,
                     unsigned veclen, unsigned bytes)
{
    while (bytes > 0) {
        ssize_t r = 0;
#ifdef HAVE_PWRITEV
...
#ifndef _WIN32
int buffer::list::write_fd(int fd) const const
{
    // use writev!
    iovec iov[IOV_MAX];
    int iovlen = 0;
...
int buffer::list::send_fd(int fd) const const
{
    return buffer::list::write_fd(fd);
}

int buffer::list::write_fd(int fd, uint64_t offset) const const
{
    iovec iov[IOV_MAX];
...
buffer::list::iov_vec_t buffer::list::prepare_iovs() constconst
{
    size_t index = 0;
    uint64_t off = 0;
    iov_vec_t iovs {
  ...
  for (auto & bp:_buffers) {
        if (index == 0) {
            ...
            it->iov.
                resize(std::
                       min(_num - IOV_MAX * nr_iov_created, (size_t) IOV_MAX));
        }
...
__u32 buffer::list::crc32c(__u32 crc) const const
{
    int cache_misses = 0;
    int cache_hits = 0;
    int cache_adjusts = 0;
  ...
  for (const auto & node:_buffers) {
        if (node.length()) {
            ...
            pair < size_t, size_t > ofs(node.offset(),
            ...
            if (r->get_crc(ofs, &ccrc)) {
                if (ccrc.first == crc) {
                     ...
                     */
                    crc =
                        ccrc.second ^ ceph_crc32c(ccrc.first ^ crc, NULL,
                                                  node.length());
                    cache_adjusts++;
                }
            }
                ...
                uint32_t base = crc;
                crc =
                    ceph_crc32c(crc, (unsigned char *)node.c_str(),
                                node.length());
 ...
 */
void buffer::list::write_stream(std::ostream & out) const const
{
  for (const auto & node:_buffers) {
        if (node.length() > 0) {
            out.write(node.c_str(), node.length());
...
void buffer::list::hexdump(std::ostream & out, bool trailing_newline) const const
{
    if (!length())
        return;
...
bool buffer::ptr_node::dispose_if_hypercombined(buffer::ptr_node *
                                                const delete_this)
{
...
std::unique_ptr < buffer::ptr_node, buffer::ptr_node::disposer >
    buffer::ptr_node::create_hypercombined(ceph::unique_leakable_ptr <
                                           buffer::raw > r)
{
    ...
    return std::unique_ptr < buffer::ptr_node,
        buffer::ptr_node::disposer > (new ptr_node(std::move(r)));
}
...
{
    const unsigned alloc = std::max(min_alloc,
                                    shift_round_up(static_cast <
                                                   unsigned >(len),
                                                   static_cast <
                                                   unsigned
                                                   >(CEPH_PAGE_SHIFT)));
...
#pragma clang diagnostic pop
        ...
        const char *buffer_error_category::name() const noexcept {
            return "buffer";
        } const char *buffer_error_category::message(int ev, char *,
                                                     std::
            ...
            case errc::bad_alloc:
                ...
                case errc::end_of_buffer:case errc::
                    malformed_input:return boost::system::errc::io_error;
            } return {
            ev, *this};
        }

        bool buffer_error_category::equivalent(int ev,
                                               const boost::system::
                                               error_condition & c)const
            noexcept {
