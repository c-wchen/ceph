...
const unsigned long int OPS_APPENDED_TOGETHER =
    MAX_ALLOC_PER_TRANSACTION;

template < typename I >
Builder < AbstractWriteLog < I >> *WriteLog <
                               I >::create_builder()
{
                          ...
                          pwl::C_ReadRequest *read_ctx)
{
    ...
    entry_bl_copy.begin(read_buffer_offset).
                 copy(entry_hit_length, hit_bl);
    ceph_assert(hit_bl.length() == entry_hit_length);

    /* Add hit extent to read extents */
    auto hit_extent_buf =
        std::make_shared < ImageExtentBuf > (hit_extent,
            hit_bl);
...
I >::append_op_log_entries(GenericLogOperations &ops)
{
    ...
    for (auto &operation : ops) {
        if (!entries_to_flush.empty()) {
             ...
             * tail of the ring */
            if (entries_to_flush.back()->get_log_entry()->
                log_entry_index >
        ...
        ldout(m_image_ctx.cct,
              20) << "Copying entry for operation at index=" <<
                  ...
                  log_entry_index << " from " << &operation->
                  get_log_entry()->ram_entry << " to " << operation->
                  get_log_entry()->
                  cache_entry << " operation=[" << *operation << "]"
                  << dendl;
        operation->log_append_start_time = now;
        *operation->get_log_entry()->cache_entry =
            operation->get_log_entry()->ram_entry;
        ldout(m_image_ctx.cct,
              20) << "APPENDING: index=" << operation->
                  get_log_entry()->
                  log_entry_index << " pmem_entry=[" << *operation->
                  get_log_entry()->cache_entry << "]" << dendl;
            ...
            if (operation->reserved_allocated()) {
                auto write_op =
                    (std::shared_ptr < WriteLogOperation >
                     &)operation;
                pmemobj_tx_publish(&write_op->buffer_alloc->
                                   buffer_alloc_action, 1);
            } else {
                ldout(m_image_ctx.cct,
    ...
    m_perfcounter->tinc(l_librbd_pwl_append_tx_t,
                        tx_end - tx_start);
    m_perfcounter->hinc(l_librbd_pwl_append_tx_t_hist,
                        utime_t(tx_end - tx_start).to_nsec(),
                        ops.size());
    for (auto &operation : ops) {
...
void WriteLog <
I >::
flush_op_log_entries(GenericLogOperationsVector &ops)
{
    if (ops.empty()) {
        return;
    }
    ...
    if (ops.size() > 1) {
        ceph_assert(ops.front()->get_log_entry()->cache_entry <
    ...
    ldout(m_image_ctx.cct, 20) << "entry count=" << ops.size()
                               ...
                               << " bytes="
                               << ops.size() *
                               sizeof(*(ops.front()->get_log_entry()->cache_entry))
                               << dendl;
    pmemobj_flush(m_log_pool,
                  ops.front()->get_log_entry()->cache_entry,
                  ops.size() *
                  sizeof(*
                         (ops.front()->get_log_entry()->
                          cache_entry)));
}
        ...
        if (remove(this->m_log_pool_name.c_str()) != 0) {
            lderr(m_image_ctx.
                  cct) << "failed to remove empty pool \"" <<
                       this->
                       m_log_pool_name << "\": " << pmemobj_errormsg()
                       << dendl;
        } else {
                                     ...
                                     later)
{
    ...
    if (access(this->m_log_pool_name.c_str(), F_OK) != 0) {
        if ((m_log_pool =
            ...
            lderr(cct) << "failed to create pool: " << this->
                       m_log_pool_name << ". error: " <<
        ...
        m_cache_state->empty = true;
        pool_root =
            POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);

        /* new pool, calculate and store metadata */
        size_t effective_pool_size =
            (size_t)(this->m_log_pool_size * USABLE_SIZE);
        size_t small_write_size =
            MIN_WRITE_ALLOC_SIZE + BLOCK_ALLOC_OVERHEAD_BYTES +
            sizeof(struct WriteLogCacheEntry);
        uint64_t num_small_writes =
            (uint64_t)(effective_pool_size / small_write_size);
        if (num_small_writes > MAX_LOG_ENTRIES) {
            num_small_writes = MAX_LOG_ENTRIES;
        }
        if (num_small_writes <= 2) {
            lderr(cct) << "num_small_writes needs to > 2" <<
                       dendl;
            goto err_close_pool;
        }
        ...
        TX_BEGIN(m_log_pool) {
            TX_ADD(pool_root);
            D_RW(pool_root)->header.layout_version =
                RWL_LAYOUT_VERSION;
        ...
        TX_ONCOMMIT {
            ...
            lderr(cct) << "failed to initialize pool: " <<
                       this->
                       m_log_pool_name << ". pmemobj TX errno: " <<
                       pmemobj_tx_errno() << dendl;
            r = -pmemobj_tx_errno();
            goto err_close_pool;
        }
        TX_FINALLY {
        ...
        if ((m_log_pool =
                 pmemobj_open(this->m_log_pool_name.c_str(),
                              this->m_pwl_pool_layout_name)) ==
            NULL) {
            lderr(cct) << "failed to open pool (" << this->
                       m_log_pool_name << "): " << pmemobj_errormsg()
                       << dendl;
            on_finish->complete(-errno);
            return false;
        ...
        pool_root =
            POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
        if (D_RO(pool_root)->header.layout_version !=
            RWL_LAYOUT_VERSION) {
        ...
        if (D_RO(pool_root)->block_size != MIN_WRITE_ALLOC_SIZE) {
            lderr(cct) << "pool block size is " <<
                       D_RO(pool_root)->
                       block_size << " expected " <<
                       MIN_WRITE_ALLOC_SIZE << dendl;
            goto err_close_pool;
        }
        ...
        m_first_valid_entry =
            D_RO(pool_root)->first_valid_entry;
...
template < typename I >
void WriteLog <
I >::load_existing_entries(DeferredContexts &later)
{
    TOID(struct WriteLogPoolRoot) pool_root;
    pool_root = POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
    struct WriteLogCacheEntry *pmem_log_entries =
    ...
    while (entry_index != m_first_free_entry) {
        ...
        entry_index =
            (entry_index + 1) % this->m_total_log_entries;
    }
...
template < typename I >
void WriteLog < I >::write_data_to_buffer(std::shared_ptr <
        pwl::
        WriteLogEntry >
        ws_entry,
        WriteLogCacheEntry
...
I >::retire_entries(const unsigned long int frees_per_tx)
{
    ...
    ldout(cct, 20) << "Look for entries to retire" << dendl;
    {
        ...
        while (!m_log_entries.empty()
               ...
               && this->can_retire_entry(m_log_entries.
                                         front())) {
            auto entry = m_log_entries.front();
            if (entry->log_entry_index != first_valid_entry) {
                lderr(cct) << "retiring entry index (" <<
                           entry->
                           log_entry_index <<
                           ") and first valid log entry index (" <<
                           first_valid_entry << ") must be ==." <<
                           dendl;
            }
            ceph_assert(entry->log_entry_index ==
                        first_valid_entry);
            first_valid_entry =
                (first_valid_entry +
                 1) % this->m_total_log_entries;
    ...
    if (retiring_entries.size()) {
        ...
        /* Advance first valid entry and release buffers */
        {
            ...
            std::lock_guard append_locker(this->
                                          m_log_append_lock);
            {
                std::lock_guard locker(m_lock);
            ...
            TX_BEGIN(m_log_pool) {
                if (D_RO(pool_root)->flushed_sync_gen <
                    flushed_sync_gen) {
                    ldout(m_image_ctx.cct,
                          20) <<
                              "flushed_sync_gen in log updated from "
                              << D_RO(pool_root)->
                              flushed_sync_gen << " to " <<
            ...
            TX_ONABORT {
                lderr(cct) << "failed to commit free of" <<
        ...
        m_perfcounter->tinc(l_librbd_pwl_retire_tx_t,
                            tx_end - tx_start);
        ...
        /* Update runtime copy of first_valid, and free entries counts */
        {
            ...
            ceph_assert(this->m_first_valid_entry ==
            ...
            for (auto &entry : retiring_entries) {
                if (entry->write_bytes()) {
                    ceph_assert(this->m_bytes_cached >=
                                entry->write_bytes());
                    this->m_bytes_cached -=
                        entry->write_bytes();
                    uint64_t entry_allocation_size =
                        entry->write_bytes();
                    if (entry_allocation_size <
                        MIN_WRITE_ALLOC_SIZE) {
                        entry_allocation_size =
                            MIN_WRITE_ALLOC_SIZE;
                    }
                    ceph_assert(this->m_bytes_allocated >=
                                entry_allocation_size);
                    this->m_bytes_allocated -=
                        entry_allocation_size;
                }
    ...
    do {
        {
            ...
            if (m_ops_to_flush.size()) {
                ...
                ops.splice(ops.end(), m_ops_to_flush,
                           m_ops_to_flush.begin(),
                           last_in_batch);
                ops_remain = !m_ops_to_flush.empty();
                ldout(m_image_ctx.cct,
                      20) << "flushing " << ops.
                          size() << ", remain " << m_ops_to_flush.
                          size() << dendl;
            } else {
                ops_remain = false;
            }
...
void WriteLog < I >::append_scheduled_ops(void)
{
    ...
    do {
        ...
        if (num_ops) {
            ...
            this->complete_op_log_entries(std::move(ops),
                                          append_result);
        }
    } while (ops_remain);
}
                         ...
                         C_BlockIORequestT *req)
{
              ...
              std::back_inserter(appending));
    {
        ...
        need_finisher = this->m_ops_to_append.empty()
                        && !this->m_appending;
...
void WriteLog <
I >::
schedule_flush_and_append(GenericLogOperationsVector &ops)
{
...
template < typename I > void WriteLog < I >::process_work()
{
    ...
    uint64_t low_water_bytes =
        this->m_bytes_allocated_cap * RETIRE_LOW_WATER;
    uint64_t aggressive_high_water_entries =
        this->m_total_log_entries *
        AGGRESSIVE_RETIRE_HIGH_WATER;
    uint64_t high_water_entries =
        this->m_total_log_entries * RETIRE_HIGH_WATER;
    uint64_t low_water_entries =
        this->m_total_log_entries * RETIRE_LOW_WATER;
        ...
        if (this->m_alloc_failed_since_retire
            ...
            ldout(m_image_ctx.cct,
                  10) << "alloc_fail=" << this->
                      ...
                      (m_log_entries.size() > high_water_entries)
                      << dendl;
            while (this->m_alloc_failed_since_retire
                   || this->m_invalidating
                   || (this->m_bytes_allocated >
                       high_water_bytes)
                   || (m_log_entries.size() >
                   ...
                   (((this->m_bytes_allocated > low_water_bytes)
                     || (m_log_entries.size() >
                         low_water_entries))
                    && (utime_t(ceph_clock_now() - started).
                        to_msec() <
    ...
    if (D_RO(pool_root)->flushed_sync_gen < flushed_sync_gen) {
        ldout(m_image_ctx.cct,
              15) << "flushed_sync_gen in log updated from " <<
                  D_RO(pool_root)->
                  flushed_sync_gen << " to " << flushed_sync_gen <<
                  dendl;
        TX_BEGIN(m_log_pool) {
            D_RW(pool_root)->flushed_sync_gen =
                flushed_sync_gen;
        ...
        TX_ONABORT {
            lderr(m_image_ctx.
                  cct) <<
                       "failed to commit update of flushed sync point"
                       << dendl;
            ceph_assert(false);
        }
        TX_FINALLY {
                                   ...
                                   bool &no_space)
{
    ...
    for (auto &buffer : buffers) {
        ...
        ldout(m_image_ctx.cct,
              20) << "Allocated " << buffer.buffer_oid.oid.
                  pool_uuid_lo << "." << buffer.buffer_oid.oid.
                  off << ", size=" << buffer.allocation_size << dendl;
    }
}

template < typename I >
void WriteLog <
I >::copy_bl_to_buffer(WriteRequestResources *resources,
                       std::unique_ptr <
                       WriteLogOperationSet > &op_set)
{
...
I >::alloc_resources(C_BlockIORequestT *req)
{
    ...
    if (!alloc_succeeds) {
        ...
        for (auto &buffer : buffers) {
            if (buffer.allocated) {
                pmemobj_cancel(m_log_pool,
                               &buffer.buffer_alloc_action, 1);
            }
        }
    }
