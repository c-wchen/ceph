...
unsigned int ErasureCodeShec::get_chunk_size(unsigned int object_size) const const
{
                                        ...
                                        set < int > *minimum_chunks)
{
    ...
    for (set < int >::iterator it = want_to_read.begin();
...

    {
        ...
        if (shec_make_decoding_matrix
            (true, want, avails, decoding_matrix, dm_row, dm_column,
             minimum) < 0) {
            return -EIO;
        }
    }
...
unsigned ErasureCodeShecReedSolomonVandermonde::get_alignment() const const
{
    return k * w * sizeof(int);
}

int ErasureCodeShecReedSolomonVandermonde::
parse(const ErasureCodeProfile &profile)
{
    int err = 0;
    // k, m, c
...
double ErasureCodeShec::shec_calc_recovery_efficiency1(int k, int m1, int m2,
        int c1, int c2)
{
    int r_eff_k[k];
    double r_e1;
    ...
    for (rr = 0; rr < m1; rr++) {
        ...
        for (cc = start, first_flag = 1; first_flag || cc != end;
            ...
            first_flag = 0;
            r_eff_k[cc] =
                std::min(r_eff_k[cc], ((rr + c1) * k) / m1 - (rr * k) / m1);
        }
    ...
    for (rr = 0; rr < m2; rr++) {
        ...
        for (cc = start, first_flag = 1; first_flag || cc != end;
            ...
            first_flag = 0;
            r_eff_k[cc] =
                std::min(r_eff_k[cc], ((rr + c2) * k) / m2 - (rr * k) / m2);
        }
        ...
        for (c1 = 0; c1 <= c / 2; c1++) {
            for (m1 = 0; m1 <= m; m1++) {
                ...
                if (true) {
                    ...
                    if (min_r_e1 - r_e1 > std::numeric_limits <
                        double >::epsilon() && r_e1 < min_r_e1) {
                        min_r_e1 = r_e1;
                        c1_best = c1;
                        m1_best = m1;
                    }
...
int ErasureCodeShec::shec_make_decoding_matrix(bool prepare, int *want_,
        int *avails,
        int *decoding_matrix,
        ...
        int *minimum)
{
    ...
    for (int i = 0; i < mindup; i++) {
        for (int j = 0; j < mindup; j++) {
            if (dm_row[i] < k) {
                ...
                tmpmat[i * mindup + j] =
                    matrix[(dm_row[i] - k) * k + dm_column[j]];
            }
        }
        if (dm_row[i] < k) {
            for (int j = 0; j < mindup; j++) {
    ...
    tcache.putDecodingTableToCache(decoding_matrix, dm_row, dm_column, minimum,
                                   technique, k, m, c, w, want, avails);

    return ret;
}

int ErasureCodeShec::shec_matrix_decode(int *want, int *avails,
