                                     ...
                                     bufferlist * out)
{
    ...
    for (list < cls_user_bucket_entry >::iterator iter = op.entries.begin();
        ...
        CLS_LOG(20, "storing entry for key=%s size=%lld count=%lld",
                key.c_str(), (long long)update_entry.size,
    ...
    CLS_LOG(20, "header: total bytes=%lld entries=%lld",
                                  ...
                                  bufferlist * out)
{
    ...
    if (ret < 0) {
        CLS_LOG(0, "ERROR: get existing bucket entry, key=%s ret=%d",
                key.c_str(), ret);
        return ret;
    }
...
/// by RGW
static int cls_user_reset_stats(cls_method_context_t hctx, bufferlist * in,
                                bufferlist * out /*ignore */ )
{
    ...
    do {
        map < string, bufferlist > keys;
        int rc =
            cls_cxx_map_get_vals(hctx, from_index, prefix, MAX_ENTRIES, &keys,
                                 &truncated);
      ...
      for (const auto & kv:keys) {
            ...
            try {
                ...
                CLS_LOG(0, "ERROR: failed to decode bucket entry for %s",
                        kv.first.c_str());
                return -EIO;
            }
...
CLS_INIT(user)
{
    ...
    /* log */
    cls_register_cxx_method(h_class, "set_buckets_info",
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_user_set_buckets_info,
                            &h_user_set_buckets_info);
    cls_register_cxx_method(h_class, "complete_stats_sync",
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_user_complete_stats_sync,
                            &h_user_complete_stats_sync);
    cls_register_cxx_method(h_class, "remove_bucket",
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            cls_user_remove_bucket, &h_user_remove_bucket);
    cls_register_cxx_method(h_class, "list_buckets", CLS_METHOD_RD,
                            cls_user_list_buckets, &h_user_list_buckets);
    cls_register_cxx_method(h_class, "get_header", CLS_METHOD_RD,
                            cls_user_get_header, &h_user_get_header);
    cls_register_cxx_method(h_class, "reset_user_stats",
                            CLS_METHOD_RD | CLS_METHOD_WR, cls_user_reset_stats,
                            &h_user_reset_stats);
    return;
}
