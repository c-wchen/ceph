...
struct old_rgw_bucket {
    ...
    old_rgw_bucket(const char *t, const char *n, const char *dp, const char *ip,
                   const char *m, const char *id, const char *h):tenant(t),
...
class old_rgw_obj {
    ...
    std::string instance;
  public:
    const std::string & get_object() const const
{
    return object;
...
std::string & get_hash_object() {
   ...
   */
static bool translate_raw_obj_to_obj_in_ns(string & obj, std::string & instance,
                                           std::string & ns) {
    if (obj[0] != '_') {
        if (ns.empty()) {
            return true;
...
static bool parse_raw_oid(const std::string & oid, std::string * obj_name,
                          std::string * obj_instance, std::string * obj_ns) {
    obj_instance->clear();
    obj_ns->clear();
    if (oid[0] != '_') {
        *obj_name = oid;
   ...
   * false if it fails.
   */
static bool strip_namespace_from_object(string & obj, std::string & ns,
                                        std::string & instance) {
    ns.clear();
    instance.clear();
    if (obj[0] != '_') {
...
static inline void prepend_old_bucket_marker(const old_rgw_bucket & bucket,
                                             const string & orig_oid,
                                             string & oid)
{
    ...
    rgw_data_placement_target default_placement;
     ...
     test_rgw_env() {
        ...
        default_placement.data_pool =
            rgw_pool(zone_params.placement_pools[zonegroup.default_placement].
                     data_pool);
        default_placement.data_extra_pool =
            rgw_pool(zone_params.placement_pools[zonegroup.default_placement].
                     data_extra_pool);
    } rgw_data_placement_target get_placement(const std::string & placement_id) {
        const RGWZonePlacementInfo & pi =
            zone_params.placement_pools[placement_id];
