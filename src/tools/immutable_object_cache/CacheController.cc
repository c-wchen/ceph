...
int CacheController::run()
{
    try {
        ...
        if (controller_path.empty()) {
            lderr(m_cct) << "'immutable_object_cache_sock' path not set"
                         << dendl;
            return -EINVAL;
        }
                                     ...
                                     ObjectCacheRequest *req)
{
    ...
    switch (req->get_request_type()) {
        case RBDSC_REGISTER: {
            ...
            auto req_reg_data =
                reinterpret_cast < ObjectCacheRegData * >(req);
        ...
        case RBDSC_READ: {
            ...
            int ret =
                m_object_cache_store->lookup_object(req_read_data->
                                                    pool_namespace,
                                                    req_read_data->
                                                    pool_id,
                                                    req_read_data->
