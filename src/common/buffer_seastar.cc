...
namespace ceph::buffer {
    ...
    inline namespace v15_2_0 {
        ...
        ceph::unique_leakable_ptr < buffer::raw > create(temporary_buffer
                                                         && buf) {
...
// buffer::ptr conversions
    ...
    ptr::operator  seastar::temporary_buffer < char >() & {
        return {
        c_str(), _len, seastar::make_object_deleter(*this)};
    }
    ...
    ptr::operator  seastar::temporary_buffer < char >() && {
        ...
        auto length = _len;
        return {
        data, length, seastar::make_object_deleter(std::move(*this))};
    }
...
{
    return buffer::ptr {
    buffer::copy(get_pos_add(len), len)};
}
