...
INSTANTIATE_TEST_CASE_P(Compressor, CompressorTest,::testing::Values(
#ifdef HAVE_LZ4
                                                                        "lz4",
#endif
#ifdef __x86_64__
...
TEST(CompressionPlugin, all)
{
    ...
    EXPECT_TRUE(reg);
    CompressionPlugin *factory =
        dynamic_cast <
        CompressionPlugin * >(reg->get_with_load("compressor", "invalid"));
    EXPECT_FALSE(factory);
    factory =
        dynamic_cast <
        CompressionPlugin * >(reg->get_with_load("compressor", "example"));
    EXPECT_TRUE(factory);
    stringstream ss;
