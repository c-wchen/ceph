...
std::optional < record_t > decode_record(bufferlist & bl)
{
  ...
  for (auto & iter:*ex_infos) {
      for (auto e:iter.extent_infos) {
            ...
            logger().debug(" exten len {} remaining {} ", e.len,
                           bliter_ex.get_remaining());
...
struct entry_validator_t {
    ...
    void validate(CircularBoundedJournal & cbj) {
        ...
        for (int i = 0; i < entries; i++) {
            paddr_t paddr = convert_abs_addr_to_paddr(addr + offset,
                                                      cbj.get_device_id());
            auto[header, buf] =
                *(cbj.read_record(paddr, NULL_SEG_SEQ).unsafe_get0());
...
struct cbjournal_test_t:public seastar_test_suite_t, JournalTrimmer {
    ...
    std::size_t get_trim_size_per_cycle()const final {
        ...
        auto[addr, w_result] = cbj->submit_record(std::move(record),
                                                  handle).unsafe_get0();
        entries.back().addr =
    ...
    extent_t generate_extent(size_t blocks) {
        std::uniform_int_distribution < char >distribution(std::numeric_limits <
                                                           char >::min(),
                                                           std::numeric_limits <
                                                           char >::max()
            );
        ...
        return extent_t {
        extent_types_t::TEST_BLOCK, L_ADDR_NULL, bl};
    }
    ...
    delta_info_t generate_delta(size_t bytes) {
        ...
        return delta_info_t {
            extent_types_t::TEST_BLOCK, paddr_t {
            }
        , L_ADDR_NULL,
                0, 0,
                device->get_block_size(), 1, 0, segment_type_t::JOURNAL, bl};
    }
    ...
    auto replay() {
        return cbj->replay([this] (const auto & offsets,
                                   const auto & e,
                                   auto & dirty_seq,
                                   auto & alloc_seq, auto last_modified) {
          bool found = false; for (auto & i:entries) {
                           paddr_t base = offsets.write_result.start_seq.offset;
                           rbm_abs_addr addr = convert_paddr_to_abs_addr(base);
                           if (addr == i.addr) {
                           logger().debug(" compare addr: {} and i.addr {} ",
                                          base, i.addr);
                           found = i.validate_delta(e.bl); break;}
                           }
                           assert(found == true);
    ...
    auto mkfs() {
        ...
        return device->mkfs(config).safe_then([this] () {
                                              return device->mount().
                                              safe_then([this] () {
                                                        return cbj->
                                                        open_for_mkfs().
    ...
    void update_journal_tail(rbm_abs_addr addr, uint32_t len) {
        paddr_t paddr = convert_abs_addr_to_paddr(addr + len,
                                                  cbj->get_device_id());
    ...
    seastar::future <> set_up_fut()final {
        device =
            random_block_device::create_test_ephemeral(random_block_device::
                                                       DEFAULT_TEST_CBJOURNAL_SIZE,
                                                       0);
        cbj.reset(new
                  CircularBoundedJournal(*this, device.get(), std::string()));
        block_size = device->get_block_size();
        cbj->set_write_pipeline(&pipeline);
        return mkfs().safe_then([this] {
...
TEST_F(cbjournal_test_t, boudary_check_verify)
{
    run_async([this] {
              ...
              entries.erase(entries.begin());
              ASSERT_EQ(avail + (record_total_size * 2),
              ...
              ASSERT_TRUE(avail - record_total_size >=
                          get_records_available_size()); replay_and_check();}
    ) ;
}
...
TEST_F(cbjournal_test_t, update_header)
{
    run_async([this] {
              auto[header, _buf] =
              *(cbj->get_cjs().read_header().unsafe_get0()); record_t rec {
              {
              generate_extent(1), generate_extent(2)}
              , {
              generate_delta(20), generate_delta(21)}
              ...
              *(cbj->get_cjs().read_header().unsafe_get0());
              cbj->close().unsafe_get0(); replay().unsafe_get0();
              ASSERT_EQ(update_header.dirty_tail.offset,
                        update_header.dirty_tail.offset);}
    );
}
...
TEST_F(cbjournal_test_t, replay)
{
    run_async([this] {
              ...
              ASSERT_EQ(avail + (record_total_size * 2),
                        get_records_available_size());
              avail = get_records_available_size(); submit_record(record_t {
                                                                  {
                                                                  generate_extent
                                                                  (1),
                                                                  generate_extent
...
TEST_F(cbjournal_test_t, replay_after_reset)
{
    run_async([this] {
              ...
              set_written_to(journal_seq_t {
                             0,
                             convert_abs_addr_to_paddr(cbj->get_records_start(),
                                                       cbj->get_device_id())}
              );
