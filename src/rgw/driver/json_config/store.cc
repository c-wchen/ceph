...
static void parse_config(const DoutPrefixProvider *dpp,
                         const char *filename)
{
    bufferlist bl;
    std::string errmsg;
    ...
    if (!p.parse(bl.c_str(), bl.length())) {
        ...
        throw std::
        system_error(make_error_code(std::errc::invalid_argument));
    }
...
void sanity_check_config(const DoutPrefixProvider *dpp,
                         DecodedConfig &config)
{
    ...
    int r =
        rgw::init_zone_pool_names(dpp, null_yield, pools, config.zone);
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "failed to set default zone pool names" <<
                     dendl;
        throw std::system_error(-r, std::system_category());
    }
    ...
    if (config.zonegroup.zones.size() > 1) {
        ldpp_dout(dpp,
                  0) << "zonegroup cannot contain multiple zones" <<
                     dendl;
        throw std::
        system_error(make_error_code(std::errc::invalid_argument));
    }
    ...
    if (config.zonegroup.zones.size() == 1) {
        ...
        int r =
            rgw::add_zone_to_group(dpp, config.zonegroup, config.zone,
                                   &is_master, &read_only, endpoints,
                                   nullptr, nullptr, sync_from,
                                   sync_from_rm,
                                   nullptr, std::nullopt,
        ...
        config.zonegroup.enabled_features.
    ...
    auto target = RGWZoneGroupPlacementTarget {.name =
            "default-placement" };
