...
namespace librbd {
            ...
            template < typename ImageCtxT = librbd::ImageCtx >
              ...
              public:
                ...
                static UpdateRequest *create(librados::IoCtx & ioctx,
                                             ...
                                             Context * on_finish) {
                    return new UpdateRequest(ioctx, std::move(update_mapping),
   ...
   */
                 UpdateRequest(librados::IoCtx & ioctx,
                               std::map < std::string,
                               cls::rbd::MirrorImageMap > &&update_mapping,
                               std::set < std::string >
                               &&remove_global_image_ids, Context * on_finish);

                 librados::IoCtx & m_ioctx;
                 std::map < std::string,
                    cls::rbd::MirrorImageMap > m_update_mapping;
