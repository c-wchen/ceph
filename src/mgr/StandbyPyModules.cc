...
StandbyPyModules::StandbyPyModules(const MgrMap & mgr_map_,
...
void StandbyPyModules::start_one(PyModuleRef py_module)
{
    ...
    finisher.queue(new LambdaContext([this, standby_module, name] (int) {
                                     int r = standby_module->load(); if (r != 0) {
                                     derr <<
                                     "Failed to run module in standby mode ('"
                                     << name << "')" << dendl;
                                     delete standby_module;}
                                     else {
                                     std::lock_guard l(lock); auto em = modules.emplace(name, standby_module); ceph_assert(em.second);  // actually inserted
                                     dout(4) << "Starting thread for " << name
                                     << dendl;
                                     standby_module->thread.
                                     create(standby_module->get_thread_name());}
                                     }
                   )) ;
}
...
bool StandbyPyModule::get_config(const std::string & key, std::string * value) constconst
{
    ...
    return state.with_config([global_key, value] (const PyModuleConfig & config) {
                             ...
                             *value = config.config.at(global_key); return true;}
                             else {
                             return false;}
                             }
    ) ;
...
bool StandbyPyModule::get_store(const std::string & key, std::string * value) constconst
{
...
std::string StandbyPyModule::get_active_uri() constconst
{
    std::string result;
