...
int ErasureCode::init(ErasureCodeProfile &profile, std::ostream *ss)
{
    ...
    err |= to_string("crush-root", profile,
                     &rule_root, DEFAULT_RULE_ROOT, ss);
                                   ...
                                   int >>> *minimum)
{
    ...
    int r =
        _minimum_to_decode(want_to_read, available_chunks,
                           &minimum_shard_ids);
    if (r != 0) {
        return r;
    }
                                ...
                                map < int, bufferlist > &encoded) const
{
    ...
    if (padded_chunks) {
        ...
        raw.begin((k - padded_chunks) * blocksize).copy(remainder,
                buf.c_str());
                         ...
                         map < int, bufferlist > *decoded)
{
    ...
    if (includes
        (have.begin(), have.end(), want_to_read.begin(),
         want_to_read.end())) {
                              ...
                              ostream *ss)
{
    if (profile.find("mapping") != profile.end()) {
        ...
        for (std::string::iterator it = mapping.begin();
             it != mapping.end(); ++it) {
            if (*it == 'D') {
