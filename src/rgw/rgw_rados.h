...
static inline void get_obj_bucket_and_oid_loc(const rgw_obj & obj, string & oid,
                                              string & locator)
{
...
{
} RGWCompressionInfo(const RGWCompressionInfo &
                     cs_info):compression_type(cs_info.compression_type),
    orig_size(cs_info.orig_size), blocks(cs_info.blocks) {
}
        ...
        stripe_max_size(0)
{
                     ...
                     uint64_t _part_size,
                     uint64_t _stripe_max_size):start_part_num(_start_part_num),
    start_ofs(_start_ofs), part_size(_part_size),
...
class RGWObjManifest {
  protected:
    ...
    int append_explicit(RGWObjManifest & m, const RGWZoneGroup & zonegroup,
                        const RGWZoneParams & zone_params);
    void append_rules(RGWObjManifest & m, map < uint64_t,
                      RGWObjManifestRule >::iterator & iter,
                      string * override_prefix);
...
class obj_iterator {
  ...
  obj_iterator(RGWObjManifest * _m, uint64_t _ofs):manifest(_m) {
    ...
    /* stripe number */ int get_cur_stripe() const {
        return cur_stripe;
    }
    /* current stripe size */ uint64_t get_stripe_size() {
        if (manifest->explicit_objs) {
...
RGWSystemMetaObj(const string & _name, CephContext * _cct,
                 RGWRados * _store):name(_name), cct(_cct), store(_store) {
}
...
int init(CephContext * _cct, RGWRados * _store, bool setup_obj =
         true, bool old_format = false);
...
{
} RGWZoneGroup(const std::string & id,
               const std::string & name):RGWSystemMetaObj(id, name) {
}
...
class RGWRealm:public RGWSystemMetaObj {
  ...
  public:
     RGWRealm()
{
} RGWRealm(const string & _id, const string & _name = ""):RGWSystemMetaObj(_id,
                 _name)
{
}
RGWRealm(CephContext * _cct, RGWRados * _store):RGWSystemMetaObj(_cct, _store) {
}
RGWRealm(const string & _name, CephContext * _cct,
         RGWRados * _store):RGWSystemMetaObj(_name, _cct, _store) {
}
...
:  id(period_id), epoch(_epoch), cct(NULL), store(NULL) {
}
...
int get_zonegroup(RGWZoneGroup & zonegroup, const string & zonegroup_id);
...
class RGWOpStateSingleOp {
  ...
  public:
     RGWOpStateSingleOp(RGWRados * store, const string & cid,
...
class RGWGetBucketStats_CB:public RefCountedObject {
  ...
  public:
    explicit RGWGetBucketStats_CB(const rgw_bucket & _bucket):bucket(_bucket),
        stats(NULL) {
  ...
  public:
    ...
    struct Entry {
        ...
        void *data;
         ...
         Entry(RGWChainedCache * _c, const string & _k, void *_d):cache(_c),
            key(_k), data(_d) {
    }};
};

template < class T, class S > class RGWObjectCtxImpl {
    RGWRados *store;
    std::map < T, S > objs_state;
    RWLock lock;

  public:
  RGWObjectCtxImpl(RGWRados * _store):store(_store),
...
struct RGWObjectCtx {
    ...
    explicit RGWObjectCtx(RGWRados * _store):store(_store), user_ctx(NULL),
        obj(store), raw(store) {
    } RGWObjectCtx(RGWRados * _store, void *_user_ctx):store(_store),
        user_ctx(_user_ctx), obj(store), raw(store) {
    }
};
...
class RGWRados:public AdminSocketHook {
  ...
  /** Open the pool used as root for this gateway */
    ...
    int open_bucket_index(const RGWBucketInfo & bucket_info,
                          ...
                          string > &bucket_objs, int shard_id =
                          -1, map < int, string > *bucket_instance_ids = NULL);
     template < typename T >
        int open_bucket_index(const RGWBucketInfo & bucket_info,
                              librados::IoCtx & index_ctx, map < int,
                              string > &oids, map < int, T > &bucket_objs,
                              int shard_id =
                              -1, map < int, string > *bucket_instance_ids =
                              NULL);
    void build_bucket_index_marker(const string & shard_id_str,
                                   const string & shard_marker,
                                   string * marker);

    void get_bucket_instance_ids(const RGWBucketInfo & bucket_info,
                                 int shard_id, map < int, string > *result);
    ...
    int get_obj_head_ioctx(const RGWBucketInfo & bucket_info,
    ...
    int get_olh_target_state(RGWObjectCtx & rctx,
    ...
    int get_system_obj_state_impl(RGWObjectCtx * rctx, rgw_raw_obj & obj,
                                  ...
                                  RGWObjVersionTracker * objv_tracker);
    int get_obj_state_impl(RGWObjectCtx * rctx,
                           const RGWBucketInfo & bucket_info,
                           const rgw_obj & obj, RGWObjState ** state,
                           bool follow_olh, bool assume_noent = false);
    int append_atomic_test(RGWObjectCtx * rctx,
                           const RGWBucketInfo & bucket_info,
                           const rgw_obj & obj, librados::ObjectOperation & op,
                           RGWObjState ** state);

    int update_placement_map();
    int store_bucket_info(RGWBucketInfo & info, map < string,
                          bufferlist > *pattrs,
  ...
  RGWRados():lock("rados_timer_lock"), watchers_lock("watchers_lock"), timer(NULL),
        ...
        run_reshard_thread(false), async_rados(nullptr), meta_notifier(NULL),
        data_notifier(NULL), meta_sync_processor_thread(NULL),
        meta_sync_thread_lock("meta_sync_thread_lock"),
        data_sync_thread_lock("data_sync_thread_lock"), num_watchers(0),
        watchers(NULL), watch_initialized(false),
        bucket_id_lock("rados_bucket_id"), bucket_index_max_shards(0),
        max_bucket_id(0), cct(NULL), next_rados_handle(0),
        handle_lock("rados_handle_lock"), binfo_cache(NULL),
        obj_tombstone_cache(nullptr), pools_initialized(false),
        quota_handler(NULL), finisher(NULL), cr_registry(NULL),
        zone_short_id(0), rest_master_conn(NULL), meta_mgr(NULL),
   ...
   */
    ...
    bool has_zonegroup_api(const std::string & api)const {
        if (!current_period.get_id().empty()) {
            const auto & zonegroups_by_api =
                current_period.get_map().zonegroups_by_api;
    ...
    int list_raw_objects_init(const rgw_pool & pool, const string & marker,
  ...
  /** Initialize the RADOS instance and prepare to do other ops */
    ...
    int register_to_service_map(const string & daemon_type, const map < string,
                                string > &meta);
   ...
   */
    ...
    int read_usage(const rgw_user & user, uint64_t start_epoch,
    ...
    int create_bucket(RGWUserInfo & owner, rgw_bucket & bucket,
                      ...
                      uint32_t * master_num_shards, bool exclusive = true);
    ...
    class SystemObject {
      ...
      protected:
        int get_state(RGWRawObjState ** pstate,
                      RGWObjVersionTracker * objv_tracker);

      public:
         SystemObject(RGWRados * _store, RGWObjectCtx & _ctx,
                      rgw_raw_obj & _obj):store(_store), ctx(_ctx), obj(_obj),
            state(NULL) {
        ...
        struct Read {
            ...
            struct GetObjState {
                ...
                0};
                 ...
                 GetObjState() {
                } int get_ref(RGWRados * store, rgw_raw_obj & obj,
                              rgw_rados_ref ** pref);
            } state;
    ...
    class Object {
      ...
      protected:
        ...
        int prepare_atomic_modification(librados::ObjectWriteOperation & op,
                                        ...
                                        const char *ifmatch,
                                        const char *ifnomatch, bool removal_op,
                                        bool modify_tail);
      ...
      public:
         Object(RGWRados * _store, const RGWBucketInfo & _bucket_info,
            ...
            bucket_info(_bucket_info), ctx(_ctx), obj(_obj), bs(store),
            state(NULL), versioning_disabled(false), bs_initialized(false) {
        } RGWRados *get_store() {
            return store;
        }
        rgw_obj & get_obj() {
            ...
            struct MetaParams {
                ...
                bool completeMultipart;
                 ...
                 MetaParams():mtime(NULL), rmattrs(NULL), data(NULL),
                    ...
                    category(RGW_OBJ_CATEGORY_MAIN), flags(0), if_match(NULL),
            ...
            struct DeleteParams {
                ...
                rgw_zone_set *zones_trace;
                 ...
                 DeleteParams():versioning_status(0), olh_epoch(0),
                    bilog_flags(0), remove_objs(NULL),
    ...
    class Bucket {
      ...
      public:
        ...
        class UpdateIndex {
            ...
            int guard_reshard(BucketShard ** pbs,
          ...
          public:
            ...
            UpdateIndex(RGWRados::Bucket * _target,
                ...
                bs(target->get_store()) {
                blind =
                    (target->get_bucket_info().index_type ==
                     RGWBIType_Indexless);
            }
            ...
            int complete_del(int64_t poolid, uint64_t epoch, ceph::real_time & removed_mtime,   /* mtime of removed object */
                             list < rgw_obj_index_key > *remove_objs);
            int cancel();

            const string *get_optag() {
                return &optag;
            }
            ...
            struct Params {
                ...
                bool allow_unordered;
                 ...
                 Params():enforce_ns(true),
                    filter(NULL), list_versions(false), allow_unordered(false) {
            }} params;
  ...
  /** Write/overwrite an object to the bucket storage. */
    virtual int put_system_obj_impl(rgw_raw_obj & obj, uint64_t size,
                                    ceph::real_time * mtime, map < std::string,
    ...
    virtual int put_system_obj_data(void *ctx, rgw_raw_obj & obj,
                                    bufferlist & bl, off_t ofs, bool exclusive,
                                    RGWObjVersionTracker * objv_tracker =
                                    nullptr);
    int aio_put_obj_data(void *ctx, rgw_raw_obj & obj, bufferlist & bl,
                         off_t ofs, bool exclusive, void **handle);

    int put_system_obj(void *ctx, rgw_raw_obj & obj, const char *data,
                       size_t len, bool exclusive, ceph::real_time * mtime,
                       map < std::string, bufferlist > &attrs,
                       RGWObjVersionTracker * objv_tracker,
                       ceph::real_time set_mtime) {
        bufferlist bl;
        ...
        return put_system_obj_impl(obj, len, mtime, attrs, flags, bl,
    ...
    int copy_obj_to_remote_dest(RGWObjState * astate,
                                ...
                                rgw_obj & dest_obj, ceph::real_time * mtime);
   ...
   */
    int delete_bucket(RGWBucketInfo & bucket_info,
                      RGWObjVersionTracker & objv_tracker, bool check_empty =
                      true);
   ...
   */
    ...
    void wakeup_data_sync_shards(const string & source_zone, map < int,
                                 set < string > >&shard_ids);

    RGWMetaSyncStatusManager *get_meta_sync_manager();
    RGWDataSyncStatusManager *get_data_sync_manager(const std::
                                                    string & source_zone);
   ...
   */
    ...
    int system_obj_set_attr(void *ctx, rgw_raw_obj & obj, const char *name,
                            bufferlist & bl,
                            RGWObjVersionTracker * objv_tracker);
    virtual int system_obj_set_attrs(void *ctx, rgw_raw_obj & obj, map < string,
                                     bufferlist > &attrs, map < string,
                                     bufferlist > *rmattrs,
                                     RGWObjVersionTracker * objv_tracker);
   ...
   */
    ...
    virtual int stat_system_obj(RGWObjectCtx & obj_ctx,
                                ...
                                GetObjState & state, rgw_raw_obj & obj,
                                map < string, bufferlist > *attrs,
                                ceph::real_time * lastmod, uint64_t * obj_size,
                                RGWObjVersionTracker * objv_tracker);
    ...
    virtual bool chain_cache_entry(list <
                                   rgw_cache_entry_info * >&cache_info_entries,
    ...
    int get_obj_iterate_cb(RGWObjectCtx * ctx, RGWObjState * astate,
                           const RGWBucketInfo & bucket_info,
                           const rgw_obj & obj, const rgw_raw_obj & read_obj,
   ...
   */
    ...
    virtual int raw_obj_stat(rgw_raw_obj & obj, uint64_t * psize,
                             ceph::real_time * pmtime, uint64_t * epoch,
                             map < string, bufferlist > *attrs,
    ...
    int bucket_index_unlink_instance(const RGWBucketInfo & bucket_info,
                                     const rgw_obj & obj_instance,
                                     const string & op_tag,
                                     const string & olh_tag, uint64_t olh_epoch,
                                     rgw_zone_set * zones_trace = nullptr);
    ...
    int apply_olh_log(RGWObjectCtx & ctx, RGWObjState & obj_state,
                      const RGWBucketInfo & bucket_info, const rgw_obj & obj,
                      bufferlist & obj_tag, map < uint64_t,
                      vector < rgw_bucket_olh_log_entry > >&log,
    ...
    int set_olh(RGWObjectCtx & obj_ctx, RGWBucketInfo & bucket_info,
                ...
                rgw_zone_set * zones_trace = nullptr, bool log_data_change =
                false);
    ...
    int get_bucket_stats_async(RGWBucketInfo & bucket_info, int shard_id,
    ...
    int put_bucket_entrypoint_info(const string & tenant_name,
                                   ...
                                   bool exclusive,
                                   RGWObjVersionTracker & objv_tracker,
                                   ceph::real_time mtime, map < string,
                                   bufferlist > *pattrs);
    ...
    int get_bucket_entrypoint_info(RGWObjectCtx & obj_ctx,
                                   ...
                                   RGWBucketEntryPoint & entry_point,
                                   RGWObjVersionTracker * objv_tracker,
                                   ceph::real_time * pmtime, map < string,
                                   bufferlist > *pattrs,
                                   rgw_cache_entry_info * cache_info =
                                   NULL,
                                   boost::optional < obj_version >
                                   refresh_version = boost::none);
    int get_bucket_instance_info(RGWObjectCtx & obj_ctx,
                                 const string & meta_key, RGWBucketInfo & info,
                                 ceph::real_time * pmtime, map < string,
                                 bufferlist > *pattrs);
    int get_bucket_instance_info(RGWObjectCtx & obj_ctx,
    ...
    int convert_old_bucket_info(RGWObjectCtx & obj_ctx,
                                const string & tenant_name,
                                const string & bucket_name);
  ...
  public:
    ...
    int cls_obj_prepare_op(BucketShard & bs, RGWModifyOp op, string & tag,
                           ...
                           rgw_zone_set * zones_trace = nullptr);
    int cls_obj_complete_op(BucketShard & bs, const rgw_obj & obj,
    ...
    int cls_bucket_list_ordered(RGWBucketInfo & bucket_info, int shard_id,
                                ...
                                const string & prefix, uint32_t num_entries,
                                bool list_versions, map < string,
                                rgw_bucket_dir_entry > &m, bool * is_truncated,
    ...
    int cls_bucket_head(const RGWBucketInfo & bucket_info, int shard_id,
                        vector < rgw_bucket_dir_header > &headers, map < int,
                        string > *bucket_instance_ids = NULL);
    ...
    int get_bi_log_status(RGWBucketInfo & bucket_info, int shard_id, map < int,
                          string > &max_marker);

    int bi_get_instance(const RGWBucketInfo & bucket_info, const rgw_obj & obj,
                        rgw_bucket_dir_entry * dirent);
    int bi_get_olh(const RGWBucketInfo & bucket_info, const rgw_obj & obj,
                   rgw_bucket_olh_entry * olh);
    int bi_get(const RGWBucketInfo & bucket_info, const rgw_obj & obj,
    ...
    int bi_list(BucketShard & bs, const string & filter_obj,
                const string & marker, uint32_t max,
                list < rgw_cls_bi_entry > *entries, bool * is_truncated);
    int bi_list(rgw_bucket & bucket, const string & obj_name,
                const string & marker, uint32_t max,
                list < rgw_cls_bi_entry > *entries, bool * is_truncated);
    int bi_remove(BucketShard & bs);

    int cls_obj_usage_log_add(const string & oid, rgw_usage_log_info & info);
    int cls_obj_usage_log_read(string & oid, string & user,
    ...
    void time_log_prepare_entry(cls_log_entry & entry,
    ...
    int time_log_list(const string & oid, const ceph::real_time & start_time,
                      const ceph::real_time & end_time, int max_entries,
                      list < cls_log_entry > &entries, const string & marker,
    ...
    void objexp_get_shard(int shard_num, string & shard);   /* out */
    int objexp_hint_add(const ceph::real_time & delete_at,
                        const string & tenant_name,
                        const string & bucket_name,
                        const string & bucket_id,
                        const rgw_obj_index_key & obj_key);
    int objexp_hint_list(const string & oid, const ceph::real_time & start_time, const ceph::real_time & end_time, const int max_entries, const string & marker, list < cls_timeindex_entry > &entries, /* out */
                         string * out_marker,   /* out */
                         bool * truncated); /* out */
    int objexp_hint_parse(cls_timeindex_entry & ti_entry, objexp_hint_entry & hint_entry);  /* out */
    ...
    int list_gc_objs(int *index, string & marker, uint32_t max,
                     bool expired_only,
                     std::list < cls_rgw_gc_obj_info > &result,
                     bool * truncated);
    int process_gc();
    bool process_expire_objects();
    int defer_gc(void *ctx, const RGWBucketInfo & bucket_info,
                 const rgw_obj & obj);
    ...
    int move_rados_obj(librados::IoCtx & src_ioctx, const string & src_oid,
                       const string & src_locator, librados::IoCtx & dst_ioctx,
    ...
    int fix_tail_obj_locator(const RGWBucketInfo & bucket_info,
    ...
    int cls_user_sync_bucket_stats(rgw_raw_obj & user_obj,
                                   const RGWBucketInfo & bucket_info);
    int cls_user_list_buckets(rgw_raw_obj & obj, const string & in_marker,
    ...
    int cls_user_add_bucket(rgw_raw_obj & obj,
                            list < cls_user_bucket_entry > &entries);
    int cls_user_remove_bucket(rgw_raw_obj & obj,
                               const cls_user_bucket & bucket);
    int cls_user_get_bucket_stats(const rgw_bucket & bucket,
    ...
    string unique_trans_id(const uint64_t unique_num) {
        ...
        snprintf(buf, sizeof(buf), "tx%021llx-%010llx",
                 (unsigned long long)unique_num, (unsigned long long)timestamp);
    ...
    bool can_reshard() const {
        return current_period.get_id().empty() ||
            (zonegroup.zones.size() == 1
             && current_period.is_single_zonegroup());
   ...
   */
    void get_bucket_index_objects(const string & bucket_oid_base,
                                  uint32_t num_shards, map < int,
                                  string > &bucket_objs, int shard_id = -1);
   ...
   * Returns: 0 on success, -ERR# otherwise.
   */
    int pool_iterate_begin(const rgw_pool & pool, const string & cursor,
                           RGWPoolIterCtx & ctx);
...
class RGWStoreManager {
  public:
    RGWStoreManager() {
                                   ...
                                   bool run_sync_thread,
                                   bool run_reshard_thread, bool use_cache =
                                   true) {
        RGWRados *store =
            init_storage_provider(cct, use_gc_thread, use_lc_thread,
...
template < class T > class RGWChainedCacheImpl:public RGWChainedCache {
  ...
  RGWChainedCacheImpl():lock("RGWChainedCacheImpl::lock") {
    ...
    void init(RGWRados * store) {
        ...
        expiry =
            std::chrono::seconds(store->ctx()->_conf->get_val < uint64_t >
...
class RGWPutObjProcessor:public RGWPutObjDataProcessor {
  ...
  public:
     RGWPutObjProcessor(RGWObjectCtx & _obj_ctx,
                        RGWBucketInfo & _bi):store(NULL), obj_ctx(_obj_ctx),
        is_complete(false), bucket_info(_bi), canceled(false) {
    ...
    int complete(size_t accounted_size, const string & etag,
                 ...
                 const char *if_match = NULL, const char *if_nomatch =
                 NULL, const string * user_data =
                 nullptr, rgw_zone_set * zones_trace = nullptr);
...
class RGWPutObjProcessor_Aio:public RGWPutObjProcessor {
  ...
  RGWPutObjProcessor_Aio(RGWObjectCtx & obj_ctx, RGWBucketInfo & bucket_info):RGWPutObjProcessor(obj_ctx,
                       bucket_info)
    {
    }
...
class RGWPutObjProcessor_Atomic:public RGWPutObjProcessor_Aio {
  ...
  protected:
    ...
    int write_data(bufferlist & bl, off_t ofs, void **phandle,
                   rgw_raw_obj * pobj, bool exclusive);
  ...
  public:
    ...
    } RGWPutObjProcessor_Atomic(RGWObjectCtx & obj_ctx,
                                RGWBucketInfo & bucket_info, rgw_bucket & _b,
                                const string & _o, uint64_t _p,
                                const string & _t,
                                bool versioned):RGWPutObjProcessor_Aio(obj_ctx,
                                                                       bucket_info),
        part_size(_p), cur_part_ofs(0), next_part_ofs(_p), cur_part_id(0),
        data_ofs(0), max_chunk_size(0), versioned_object(versioned), bucket(_b),
        obj_str(_o), unique_tag(_t) {
    }
    int prepare(RGWRados * store, string * oid_rand) override;
  ...
  public:
     bool immutable_head() {
        return true;
                                   ...
                                   req_state *
                                   _s):RGWPutObjProcessor_Atomic(obj_ctx,
                                                                 bucket_info,
                                                                 _s->bucket,
                                                                 _s->object.
                                                                 name, _p,
