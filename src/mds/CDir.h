...
class CDir:public MDSCacheObject, public Counter < CDir > {
  ...
  public:
    ...
    const char *pin_name(int p) const override {
        ...
        case PIN_DNWAITER:
            ...
            default:return generic_pin_name(p);
                                                                    ...
                                                                    // -- state -- static const unsigned STATE_COMPLETE = (1 << 1);
                                                                    // the complete contents are in cache
    ...
    static const unsigned MASK_STATE_IMPORT_KEPT =
        (STATE_IMPORTING
         ...
         | STATE_FROZENTREE | STATE_STICKY);
    static const unsigned MASK_STATE_EXPORT_KEPT =
        (STATE_EXPORTING
         | STATE_IMPORTBOUND | STATE_EXPORTBOUND
         | STATE_FROZENTREE | STATE_FROZENDIR | STATE_STICKY);
  ...
  public:
    ...
    void assimilate_dirty_rstat_inodes_finish(MutationRef & mut,
                                              EMetaBlob * blob);
    ...
    class scrub_info_t {
      ...
      scrub_info_t():
        directory_scrubbing(false),
            ...
            last_scrub_dirty(false), pending_scrub_error(false) {
        }
    };
  /**
  ...
  // -- dentries and inodes -- public:
    ...
    CDentry *add_primary_dentry(boost::string_view dname, CInode * in,
                                ...
                                CEPH_NOSNAP);
    CDentry *add_remote_dentry(boost::string_view dname, inodeno_t ino,
                               unsigned char d_type, snapid_t first =
                               2, snapid_t last = CEPH_NOSNAP);
  ...
  public:
    void split(int bits, std::list < CDir * >&subs,
               list < MDSInternalContextBase * >&waiters, bool replay);
    void merge(std::list < CDir * >&subs,
               std::list < MDSInternalContextBase * >&waiters, bool replay);
    ...
    void prepare_old_fragment(map < string_snap_t,
                              std::list <
                              MDSInternalContextBase * > >&dentry_waiters,
                              bool replay);
    void steal_dentry(CDentry * dn);    // from another dir.  used by merge/split.
    void finish_old_fragment(std::list < MDSInternalContextBase * >&waiters,
                             bool replay);
  ...
  public:
    ...
    // -- fetch -- object_t get_ondisk_object() {
        return file_object_t(ino(), frag);
    }
    void fetch(MDSInternalContextBase * c, bool ignore_authpinnability = false);
    void fetch(MDSInternalContextBase * c, boost::string_view want_dn,
               bool ignore_authpinnability = false);
    void fetch(MDSInternalContextBase * c,
               const std::set < dentry_key_t > &keys);
  protected:
    mempool::mds_co::compact_set < mempool::mds_co::string > wanted_items;

    void _omap_fetch(MDSInternalContextBase * fin,
                     const std::set < dentry_key_t > &keys);
    void _omap_fetch_more(bufferlist & hdrbl, std::map < std::string,
                          bufferlist > &omap, MDSInternalContextBase * fin);
    CDentry *_load_dentry(boost::string_view key, boost::string_view dname,
                          snapid_t last, bufferlist & bl, int pos,
   ...
   */
    ...
    void _omap_fetched(bufferlist & hdrbl, std::map < std::string,
                       bufferlist > &omap, bool complete, int r);

    // -- commit --
    mempool::mds_co::compact_map < version_t,
        mempool::mds_co::list < MDSInternalContextBase * > >waiting_for_commit;
  ...
  public:
    ...
    void add_dentry_waiter(boost::string_view dentry, snapid_t snap,
                           MDSInternalContextBase * c);
    void take_dentry_waiting(boost::string_view dentry, snapid_t first,
                             snapid_t last,
                             std::list < MDSInternalContextBase * >&ls);
