...
struct LogSummary {
    ...
    // ---- pre-quincy ----
    // channel -> [(seq#, entry), ...]
     std::map < std::string, std::list < std::pair < uint64_t,
        LogEntry >>> tail_by_channel;
    uint64_t seq = 0;
...
inline std::ostream & operator<<(std::ostream & out, const LogEntry & e)
{
    return out << e.stamp << " " << e.name << " (" << e.rank << ") "
        << e.seq << " : " << e.channel << " " << e.prio << " " << e.msg;
}

template <> struct fmt::formatter < EntityName >:fmt::formatter <
    std::string_view > {
    template < typename FormatContext > auto format(const EntityName & e,
                                                    FormatContext & ctx) {
        return formatter < std::string_view >::format(e.to_str(), ctx);
}};

template <> struct fmt::formatter < LogEntry >:fmt::formatter <
    std::string_view > {
    template < typename FormatContext > auto format(const LogEntry & e,
                                                    FormatContext & ctx) {
        return fmt::format_to(ctx.out(), "{} {} ({}) {} : {} {} {}", e.stamp,
