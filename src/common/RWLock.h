...
class RWLock final {
    mutable pthread_rwlock_t L;
     std::string name;
    mutable int id;
    mutable std::atomic < unsigned >nrlock = { 0 }, nwlock = {
    0};
    bool track, lockdep;
  ...
  public:
    ...
    RWLock(const std::string & n, bool track_lock = true, bool ld =
           true, bool prioritize_write = false)
  :    name(n), id(-1), track(track_lock), lockdep(ld) {
    ...
    // write void get_write(bool lockdep = true) {
        if (lockdep && this->lockdep && g_lockdep)
            id = lockdep_will_lock(name.c_str(), id);
