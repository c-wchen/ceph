    ...
    namespace mirror {
        ...
        namespace {
            ...
            void scribble(librbd::ImageCtx * image_ctx, int num_ops,
                ...
                for (int i = 0; i < num_ops; i++) {
                    ...
                    if (rand() % 4 == 0) {
                        ASSERT_EQ((int)len,
                                  image_ctx->io_work_queue->discard(off, len,
                                                                    image_ctx->
        ...
        class TestImageSync:public TestFixture {
          public:
            ...
            void SetUp() override {
                ...
                m_remote_journaler =
                    new::journal::Journaler(m_threads->work_queue,
                                            m_threads->timer,
            ...
            ImageSync <> *create_request(Context * ctx) {
                return new ImageSync <> (m_local_image_ctx, m_remote_image_ctx,
                                         ...
                                         m_remote_journaler, &m_client_meta,
        ...
        TEST_F(TestImageSync, Simple) {
            ...
            for (uint64_t offset = 0; offset < m_remote_image_ctx->size;
                 offset += object_size) {
                ASSERT_LE(0,
                          m_remote_image_ctx->io_work_queue->read(offset,
                                                                  object_size,
                                                                  librbd::io::
                                                                  ReadResult {
                                                                  &read_remote_bl}
                                                                  , 0));
                ASSERT_LE(0,
                          m_local_image_ctx->io_work_queue->read(offset,
                                                                 object_size,
                                                                 librbd::io::
                                                                 ReadResult {
                                                                 &read_local_bl}
        ...
        TEST_F(TestImageSync, Resize) {
            ...
            bl.append(std::string(len, '1'));
            ASSERT_EQ((int)len,
                      m_remote_image_ctx->io_work_queue->write(off, len,
                                                               std::move(bl),
                                                               0));
            {
            ...
            ASSERT_LE(0,
                      m_remote_image_ctx->io_work_queue->read(off, len,
                                                              librbd::io::
                                                              ReadResult {
                                                              &read_remote_bl},
                                                              0));
            ASSERT_LE(0,
                      m_local_image_ctx->io_work_queue->read(off, len,
                                                             librbd::io::
                                                             ReadResult {
                                                             &read_local_bl},
                                                             0));
        ...
        TEST_F(TestImageSync, Discard) {
            ...
            ASSERT_EQ((int)len - 2,
                      m_remote_image_ctx->io_work_queue->discard(off + 1,
                                                                 len - 2,
                                                                 m_remote_image_ctx->
                                                                 skip_partial_discard));
            {
        ...
        TEST_F(TestImageSync, SnapshotStress) {
          ...
          for (auto & snap_name:snap_names) {
                ...
                {
                    C_SaferCond ctx;
                    m_remote_image_ctx->state->
                        snap_set(cls::rbd::UserSnapshotNamespace(), snap_name,
                                 &ctx);
                    ASSERT_EQ(0, ctx.wait());

                    RWLock::RLocker remote_snap_locker(m_remote_image_ctx->
                                                       snap_lock);
                    remote_size =
