...
namespace librbd {
    ...
    namespace {
        ...
        struct MockExclusiveLockImageCtx:public MockImageCtx {
            ...
            MockExclusiveLockImageCtx(ImageCtx &
    ...
    namespace exclusive_lock {
        ...
        template <>
            struct ImageDispatch <MockExclusiveLockImageCtx >:public librbd::
            io::MockImageDispatch {
            ...
            } MOCK_METHOD3(set_require_lock,
                           void (bool, io::Direction, Context *));
            MOCK_METHOD1(unset_require_lock, void (io::Direction));

        };

        ImageDispatch < MockExclusiveLockImageCtx > *ImageDispatch <
            MockExclusiveLockImageCtx >::s_instance = nullptr;

        template <>
            struct PreAcquireRequest <MockExclusiveLockImageCtx >:public
            BaseRequest < PreAcquireRequest < MockExclusiveLockImageCtx > > {
            static PreAcquireRequest < MockExclusiveLockImageCtx >
                *create(MockExclusiveLockImageCtx & image_ctx,
                        Context * on_finish) {
        ...
        template <>
            struct PostAcquireRequest <MockExclusiveLockImageCtx >:public
            BaseRequest < PostAcquireRequest < MockExclusiveLockImageCtx > > {
            MOCK_METHOD0(send, void ());
        };

        template <>
            struct PreReleaseRequest <MockExclusiveLockImageCtx >:public
            BaseRequest < PreReleaseRequest < MockExclusiveLockImageCtx > > {
            static PreReleaseRequest < MockExclusiveLockImageCtx >
                *create(MockExclusiveLockImageCtx & image_ctx,
                        ImageDispatch < MockExclusiveLockImageCtx >
                        *ImageDispatch, bool shutting_down,
...
namespace librbd {
    ...
    class TestMockExclusiveLock:public TestMockFixture {
      public:
        ...
        void expect_is_state_acquiring(MockManagedLock * managed_lock,
                                       bool ret_val) {
        ...
        void expect_is_state_releasing(MockManagedLock * managed_lock,
                                       bool ret_val) {
            EXPECT_CALL(*managed_lock, is_state_releasing())
                .WillOnce(Return(ret_val));
        }

        void expect_is_state_locked(MockManagedLock * managed_lock,
                                    bool ret_val) {
            EXPECT_CALL(*managed_lock, is_state_locked())
                .WillOnce(Return(ret_val));
        }

        void expect_is_state_shutdown(MockManagedLock * managed_lock,
                                      bool ret_val) {
        ...
        void expect_set_require_lock(MockImageDispatch & mock_image_dispatch,
            ...
            EXPECT_CALL(mock_image_dispatch,
                        set_require_lock(init_shutdown, direction, _))
        ...
        void expect_register_dispatch(MockExclusiveLockImageCtx &
                                      mock_image_ctx) {
            EXPECT_CALL(*mock_image_ctx.io_image_dispatcher,
                        register_dispatch(_));
        }

        void expect_shut_down_dispatch(MockExclusiveLockImageCtx &
                                       mock_image_ctx) {
            EXPECT_CALL(*mock_image_ctx.io_image_dispatcher,
                        shut_down_dispatch(_, _))
                .WillOnce(WithArg < 1 > (Invoke([](Context * ctx) {
                                                ctx->complete(0);})));
        }

        void expect_prepare_lock_complete(MockExclusiveLockImageCtx &
                                          mock_image_ctx) {
            EXPECT_CALL(*mock_image_ctx.state, handle_prepare_lock_complete());
        }

        void expect_pre_acquire_request(MockPreAcquireRequest &
                                        pre_acquire_request, int r) {
            EXPECT_CALL(pre_acquire_request, send())
                .WillOnce(CompleteRequest(&pre_acquire_request, r));
        }

        void expect_post_acquire_request(MockExclusiveLock *
                                         mock_exclusive_lock,
                                         MockPostAcquireRequest &
                                         post_acquire_request, int r) {
        ...
        void expect_pre_release_request(MockPreReleaseRequest &
                                        pre_release_request, int r) {
        ...
        void expect_notify_acquired_lock(MockExclusiveLockImageCtx &
                                         mock_image_ctx) {
            EXPECT_CALL(*mock_image_ctx.image_watcher, notify_acquired_lock())
                .Times(1);
        }

        void expect_notify_released_lock(MockExclusiveLockImageCtx &
                                         mock_image_ctx) {
        ...
        void expect_shut_down(MockManagedLock * managed_lock) {
            EXPECT_CALL(*managed_lock, shut_down(_))
                .
                WillOnce(CompleteContext
                         (0, static_cast < asio::ContextWQ * >(nullptr)));
        }

        void expect_accept_blocked_request(MockExclusiveLockImageCtx &
                                           mock_image_ctx,
        ...
        int when_post_acquire_lock_handler(MockManagedLock * managed_lock,
                                           int r) {
            C_SaferCond ctx;
        ...
        int when_post_reacquire_lock_handler(MockManagedLock * managed_lock,
                                             int r) {
            C_SaferCond ctx;
    ...
    TEST_F(TestMockExclusiveLock, TryLockAlreadyLocked) {
        ...
        BOOST_SCOPE_EXIT_END expect_set_state_initializing(exclusive_lock);
        MockImageDispatch mock_image_dispatch;
        expect_register_dispatch(mock_image_ctx);
        expect_set_require_lock(mock_image_ctx, mock_image_dispatch, true);
        expect_set_state_unlocked(exclusive_lock);
    ...
    TEST_F(TestMockExclusiveLock, TryLockError) {
        ...
        expect_is_action_acquire_lock(exclusive_lock, false);
        ASSERT_EQ(-EBUSY,
                  when_post_acquire_lock_handler(exclusive_lock, -EBUSY));
    }
    ...
    TEST_F(TestMockExclusiveLock, PreReleaseLockError) {
        ...
        ASSERT_EQ(-EINVAL,
                  when_pre_release_lock_handler(exclusive_lock, false));
    ...
    TEST_F(TestMockExclusiveLock, BlockRequests) {
        ...
        ASSERT_TRUE(exclusive_lock->
                    accept_request(exclusive_lock::
                                   OPERATION_REQUEST_TYPE_GENERAL, &ret_val));
        ASSERT_EQ(0, ret_val);

        exclusive_lock->block_requests(-EROFS);
        expect_is_state_shutdown(exclusive_lock, false);
        expect_is_state_locked(exclusive_lock, true);
        expect_accept_blocked_request(mock_image_ctx,
                                      mock_exclusive_lock_policy,
                                      exclusive_lock::
                                      OPERATION_REQUEST_TYPE_GENERAL, false);
        ASSERT_FALSE(exclusive_lock->
                     accept_request(exclusive_lock::
                                    OPERATION_REQUEST_TYPE_GENERAL, &ret_val));
