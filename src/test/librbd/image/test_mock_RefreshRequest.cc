...
namespace librbd {
    ...
    namespace image {
        ...
        template <> struct RefreshParentRequest <MockRefreshImageCtx > {
            ...
            static RefreshParentRequest *create(MockRefreshImageCtx &
                                                mock_image_ctx,
                ...
                auto instance = s_instances.front();
                 ...
                 return instance;
            } static bool is_refresh_required(MockRefreshImageCtx &
                                              mock_image_ctx,
        ...
        GetMetadataRequest < MockRefreshImageCtx > *GetMetadataRequest <
            MockRefreshImageCtx >::s_instance = nullptr;
...
namespace librbd {
    namespace image {
        ...
        class TestMockImageRefreshRequest:public TestMockFixture {
          public:
            typedef GetMetadataRequest < MockRefreshImageCtx >
                MockGetMetadataRequest;
            typedef RefreshRequest < MockRefreshImageCtx > MockRefreshRequest;
            typedef RefreshParentRequest < MockRefreshImageCtx >
            ...
            void set_v1_migration_header(ImageCtx * ictx) {
                bufferlist hdr;
                 ASSERT_EQ(0,
                           read_header_bl(ictx->md_ctx, ictx->header_oid, hdr,
                                          nullptr));
                 ASSERT_TRUE(hdr.length() >= sizeof(rbd_obj_header_ondisk));
                 ASSERT_EQ(0,
                           memcmp(RBD_HEADER_TEXT, hdr.c_str(),
                                  sizeof(RBD_HEADER_TEXT)));

                 bufferlist::iterator it = hdr.begin();
                 it.copy_in(sizeof(RBD_MIGRATE_HEADER_TEXT),
                            RBD_MIGRATE_HEADER_TEXT);
                 ASSERT_EQ(0,
                           ictx->md_ctx.write(ictx->header_oid, hdr,
                                              hdr.length(), 0));
            } void expect_set_require_lock(MockExclusiveLock &
                                           mock_exclusive_lock,
                                           librbd::io::Direction direction) {
                EXPECT_CALL(mock_exclusive_lock,
                            set_require_lock(true, direction, _))
                    .WillOnce(WithArg < 2 > (Invoke([](Context * ctx) {
                                                    ctx->complete(0);
                                                    })));
            } void expect_unset_require_lock(MockExclusiveLock &
                                             mock_exclusive_lock,
                                             librbd::io::Direction direction) {
                EXPECT_CALL(mock_exclusive_lock, unset_require_lock(direction));
            }

            void expect_v1_read_header(MockRefreshImageCtx & mock_image_ctx,
                                       int r) {
                auto & expect =
            ...
            void expect_v1_get_snapshots(MockRefreshImageCtx & mock_image_ctx,
                                         int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
            ...
            void expect_v1_get_locks(MockRefreshImageCtx & mock_image_ctx,
                                     int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _,
                                     StrEq("lock"), StrEq("get_info"), _, _, _,
                                     _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
            ...
            void expect_get_mutable_metadata(MockRefreshImageCtx &
                                             mock_image_ctx, uint64_t features,
                                             int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
                ...
                if (r < 0) {
                    ...
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                        ...
                        .WillOnce(WithArg < 5 >
                                  (Invoke
                                   ([features,
                                     incompatible] (bufferlist * out_bl) {
                                    encode(features, *out_bl);
                                    encode(incompatible, *out_bl); return 0;}
                                   )));
            ...
            void expect_parent_overlap_get(MockRefreshImageCtx & mock_image_ctx,
                                           int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
            ...
            void expect_get_parent_legacy(MockRefreshImageCtx & mock_image_ctx,
                                          int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
            ...
            void expect_get_migration_header(MockRefreshImageCtx &
                                             mock_image_ctx, int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
            ...
            void expect_get_op_features(MockRefreshImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    ...
                    .WillOnce(WithArg < 5 >
                              (Invoke([op_features, r] (bufferlist * out_bl) {
                                      encode(op_features, *out_bl); return r;}
                               )));
            }
            ...
            void expect_get_snapshots_legacy(MockRefreshImageCtx &
                                             mock_image_ctx,
                ...
                if (r < 0) {
                    ...
                    if (include_timestamp) {
                        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                    exec(mock_image_ctx.header_oid, _,
                                         StrEq("rbd"),
                                         StrEq("get_snapshot_timestamp"), _, _,
                                         _, _))
                            .WillOnce(DoDefault());
                    }
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
            ...
            void expect_add_snap(MockRefreshImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(mock_image_ctx,
                            add_snap(_, snap_name, snap_id, _, _, _, _, _));
            }

            void expect_init_exclusive_lock(MockRefreshImageCtx &
                                            mock_image_ctx,
                                            MockExclusiveLock &
                                            mock_exclusive_lock, int r) {
                EXPECT_CALL(mock_image_ctx, create_exclusive_lock())
                    .WillOnce(Return(&mock_exclusive_lock));
                EXPECT_CALL(mock_exclusive_lock,
                            init(mock_image_ctx.features, _))
                    .WillOnce(WithArg < 1 >
                              (CompleteContext
                               (r, mock_image_ctx.image_ctx->op_work_queue)));
            }

            void expect_shut_down_exclusive_lock(MockRefreshImageCtx &
                                                 mock_image_ctx,
                                                 MockExclusiveLock &
                                                 mock_exclusive_lock, int r) {
            ...
            void expect_refresh_parent_is_required(MockRefreshParentRequest &
            ...
            void expect_refresh_parent_send(MockRefreshImageCtx &
                                            mock_image_ctx,
                                            MockRefreshParentRequest &
                                            mock_refresh_parent_request,
                                            int r) {
            ...
            void expect_refresh_parent_apply(MockRefreshParentRequest &
                                             mock_refresh_parent_request) {
                EXPECT_CALL(mock_refresh_parent_request, apply());
            }

            void expect_refresh_parent_finalize(MockRefreshImageCtx &
                                                mock_image_ctx,
                                                MockRefreshParentRequest &
                                                mock_refresh_parent_request,
                                                int r) {
            ...
            void expect_is_exclusive_lock_owner(MockExclusiveLock &
                                                mock_exclusive_lock,
                                                bool is_owner) {
                EXPECT_CALL(mock_exclusive_lock,
            ...
            void expect_journal_disabled(MockJournalPolicy &
                                         mock_journal_policy, bool disabled) {
            ...
            void expect_get_snap_id(MockRefreshImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(mock_image_ctx,
                            get_snap_id(_,
                                        snap_name)).WillOnce(Return(snap_id));
            }
        ...
        TEST_F(TestMockImageRefreshRequest, SuccessSnapshotV1) {
            ...
            expect_v1_get_locks(mock_image_ctx, 0);
            expect_init_layout(mock_image_ctx);
            expect_add_snap(mock_image_ctx, "snap",
                            ictx->snap_ids.begin()->second);
            EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
        ...
        TEST_F(TestMockImageRefreshRequest, SuccessV2) {
            ...
            expect_refresh_parent_is_required(mock_refresh_parent_request,
                                              false);
            if (ictx->test_features(RBD_FEATURE_EXCLUSIVE_LOCK)) {
                expect_init_exclusive_lock(mock_image_ctx, mock_exclusive_lock,
                                           0);
            }
            EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
        ...
        TEST_F(TestMockImageRefreshRequest, SuccessSetSnapshotV2) {
            ...
            expect_add_snap(mock_image_ctx, "snap",
                            ictx->snap_ids.begin()->second);
            expect_get_snap_id(mock_image_ctx, "snap",
                               ictx->snap_ids.begin()->second);
            EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
        ...
        TEST_F(TestMockImageRefreshRequest, SuccessChild) {
            ...
            ASSERT_EQ(0,
                      librbd::clone(m_ioctx, m_image_name.c_str(), "snap",
                                    m_ioctx, clone_name.c_str(), ictx->features,
                                    &order, 0, 0));
            ...
            InSequence seq;
            expect_get_mutable_metadata(mock_image_ctx, mock_image_ctx.features,
                                        0);
            expect_get_parent(mock_image_ctx, 0);
            MockGetMetadataRequest mock_get_metadata_request;
            expect_get_metadata(mock_image_ctx, mock_get_metadata_request,
            ...
            expect_refresh_parent_is_required(*mock_refresh_parent_request,
                                              true);
            expect_refresh_parent_send(mock_image_ctx,
                                       *mock_refresh_parent_request, 0);
            if (ictx->test_features(RBD_FEATURE_EXCLUSIVE_LOCK)) {
                expect_init_exclusive_lock(mock_image_ctx, mock_exclusive_lock,
                                           0);
            }
            ...
            EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
            expect_refresh_parent_finalize(mock_image_ctx,
                                           *mock_refresh_parent_request, 0);
        ...
        TEST_F(TestMockImageRefreshRequest, SuccessChildBeingFlattened) {
            ...
            expect_refresh_parent_is_required(*mock_refresh_parent_request,
                                              true);
            expect_refresh_parent_send(mock_image_ctx,
                                       *mock_refresh_parent_request, -ENOENT);
            expect_get_mutable_metadata(mock_image_ctx, mock_image_ctx.features,
                                        0);
            expect_get_parent(mock_image_ctx, 0);
            expect_get_metadata(mock_image_ctx, mock_get_metadata_request,
            ...
            expect_apply_metadata(mock_image_ctx, 0);
            expect_get_group(mock_image_ctx, 0);
            expect_refresh_parent_is_required(mock_refresh_parent_request_ext,
                                              false);
            if (ictx->test_features(RBD_FEATURE_EXCLUSIVE_LOCK)) {
                expect_init_exclusive_lock(mock_image_ctx, mock_exclusive_lock,
                                           0);
        ...
        TEST_F(TestMockImageRefreshRequest, ChildEnoentRetriesLimit) {
          ...
          for (auto mock_refresh_parent_request:mock_refresh_parent_requests)
            ...
            expect_refresh_parent_apply(*mock_refresh_parent_requests
                                        [num_tries - 1]);
            EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
            expect_refresh_parent_finalize(mock_image_ctx,
                                           *mock_refresh_parent_requests
                                           [num_tries - 1], 0);
        ...
        TEST_F(TestMockImageRefreshRequest, DisableExclusiveLock) {
            ...
            if (ictx->test_features(RBD_FEATURE_JOURNALING)) {
                ASSERT_EQ(0,
                          ictx->operations->
                          update_features(RBD_FEATURE_JOURNALING, false));
            }

            if (ictx->test_features(RBD_FEATURE_OBJECT_MAP)) {
                ASSERT_EQ(0,
                          ictx->operations->
                          update_features(RBD_FEATURE_OBJECT_MAP, false));
            }
            ...
            expect_refresh_parent_is_required(mock_refresh_parent_request,
                                              false);
            EXPECT_CALL(mock_image_ctx, rebuild_data_io_context());
            expect_shut_down_exclusive_lock(mock_image_ctx, mock_exclusive_lock,
                                            0);
        ...
        TEST_F(TestMockImageRefreshRequest,
        ...
        TEST_F(TestMockImageRefreshRequest, JournalDisabledByPolicy) {
            ...
            if (ictx->test_features(RBD_FEATURE_FAST_DIFF)) {
                ASSERT_EQ(0,
                          ictx->operations->
                          update_features(RBD_FEATURE_FAST_DIFF, false));
            }
        ...
        TEST_F(TestMockImageRefreshRequest, EnableJournalWithoutExclusiveLock) {
            ...
            expect_refresh_parent_is_required(mock_refresh_parent_request,
                                              false);
            expect_set_require_lock(mock_exclusive_lock,
        ...
        TEST_F(TestMockImageRefreshRequest, DisableJournal) {
            ...
            if (!mock_image_ctx.clone_copy_on_read) {
                expect_unset_require_lock(mock_exclusive_lock,
            ...
            C_SaferCond ctx;
            MockRefreshRequest *req =
                new MockRefreshRequest(mock_image_ctx, false, false,
                                       &ctx);
            req->send();
        ...
        TEST_F(TestMockImageRefreshRequest, NonPrimaryFeature) {
            ...
            expect_get_mutable_metadata(mock_image_ctx,
                                        ictx->
                                        features | RBD_FEATURE_NON_PRIMARY, 0);
            expect_get_parent(mock_image_ctx, 0);
            MockGetMetadataRequest mock_get_metadata_request;
            expect_get_metadata(mock_image_ctx, mock_get_metadata_request,
                                mock_image_ctx.header_oid, {
            ...
            C_SaferCond ctx1;
            auto req =
                new MockRefreshRequest(mock_image_ctx, false, false, &ctx1);
            req->send();

            ASSERT_EQ(0, ctx1.wait());
            ASSERT_TRUE(mock_image_ctx.read_only);
            ASSERT_EQ(IMAGE_READ_ONLY_FLAG_NON_PRIMARY,
                      mock_image_ctx.read_only_flags);
