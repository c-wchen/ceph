    ...
    namespace mirror {
        namespace pool_watcher {
            ...
            class TestMockPoolWatcherRefreshImagesRequest:public TestMockFixture {
              public:
                typedef RefreshImagesRequest < librbd::MockTestImageCtx >
                    MockRefreshImagesRequest;

                void expect_mirror_image_list(librados::IoCtx & io_ctx,
                                              const std::map < std::string,
                                              std::string > &ids, int r) {
                    bufferlist bl;
                    ::encode(ids, bl);

                    EXPECT_CALL(get_mock_io_ctx(io_ctx),
                                exec(RBD_MIRRORING, _, StrEq("rbd"),
                                     StrEq("mirror_image_list"), _, _, _))
                    .
                        WillOnce(DoAll
                                 (WithArg < 5 >
            ...
            TEST_F(TestMockPoolWatcherRefreshImagesRequest, Success) {
                InSequence seq;
                expect_mirror_image_list(m_remote_io_ctx, { {
                                         "local id", "global id"}}, 0);

                C_SaferCond ctx;
                ImageIds image_ids;
                MockRefreshImagesRequest *req =
                    new MockRefreshImagesRequest(m_remote_io_ctx, &image_ids,
                                                 &ctx);
            ...
            TEST_F(TestMockPoolWatcherRefreshImagesRequest, LargeDirectory) {
                ...
                for (uint32_t idx = 1; idx <= 1024; ++idx) {
                    mirror_list.
                        insert(std::
                               make_pair("local id " + stringify(idx),
            ...
            TEST_F(TestMockPoolWatcherRefreshImagesRequest,
