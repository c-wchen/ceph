...
struct BatchStoredBacktrace:public MDSIOContext {
    ...
    BatchStoredBacktrace(MDSRank * m, MDSContext * f,
                         std::vector < CInodeCommitOperations >
                         &&ops):MDSIOContext(m), fin(f),
...
struct BatchCommitBacktrace:public Context {
    ...
    BatchCommitBacktrace(MDSRank * m, MDSContext * f,
                         std::vector < CInodeCommitOperations > &&ops):mds(m),
        fin(f), ops_vec(std::move(ops)) {
    } void finish(int r) override {
                               ...
                               int op_prio)
{
    ...
    for (set < metareqid_t >::iterator p = uncommitted_peers.begin();
        ...
        dout(10) << "try_to_expire waiting for leader to ack OP_FINISH on " <<
            *p << dendl;
    ...
    size_t count = 0;
    for (elist < CInode * >::iterator it = dirty_parent_inodes.begin();
         !it.end(); ++it)
        count++;
    ...
    if (!ops_vec.empty())
        mds->finisher->
            queue(new
                  BatchCommitBacktrace(mds, gather_bld.new_sub(),
                                       std::move(ops_vec)));
...
void EMetaBlob::fullbit::generate_test_instances(std::list <
...
generate_test_instances(std::list < EMetaBlob::remotebit * >&ls)
{
    ...
    ls.push_back(remote);
    remote =
        new remotebit("/test/dn2", "foo", 0, 10, 15, 1, IFTODT(S_IFREG), false);
    ls.push_back(remote);
}
...
void EMetaBlob::get_paths(std::vector < std::string > &paths) const const
{
    ...
    for (std::vector < Location >::iterator i = leaf_locations.begin();
        ...
        std::map < inodeno_t, Location >::iterator iter =
            ino_locations.find(ino);
...
void EMetaBlob::replay(MDSRank * mds, LogSegment * logseg, int type,
                       MDPeerUpdate * peerup)
{
  ...
  for (const auto & lp:lump_order) {
      ...
      for (auto & fb:lump._get_dfull()) {
            ...
            if (!dn) {
                ...
                if (fb.is_dirty())
                    dn->_mark_dirty(logseg);
                dout(10) << "EMetaBlob.replay for [" << fb.dnfirst << "," << fb.
                    dnlast << "] had " << *dn << dendl;
                dn->first = fb.dnfirst;
            ...
            if (!in) {
                in = new CInode(mds->mdcache, dn->is_auth(), fb.dnfirst,
                                fb.dnlast);
                ...
                if (dn->get_linkage()->get_inode() != in) {
                    ...
                    dout(10) << "EMetaBlob.replay for [" << fb.
                        dnfirst << "," << fb.dnlast << "] had " << *in << dendl;
                }
            ...
            {
                auto do_corruption =
                    mds->get_inject_journal_corrupt_dentry_first();
      ...
      for (const auto & rb:lump.get_dremote()) {
            ...
            if (!dn) {
                dn = dir->add_remote_dentry(rb.dn, rb.ino, rb.d_type,
                                            mempool::mds_co::string(rb.
                                                                    alternate_name),
                                            rb.dnfirst, rb.dnlast);
                ...
                if (!dn->get_linkage()->is_null()) {
                    ...
                    if (dn->get_linkage()->is_primary()) {
                        ...
                        *css <<
                            "EMetaBlob.replay FIXME had dentry linked to wrong inode "
                            << *dn << " " << *dn->get_linkage()->
                            get_inode() << " should be remote " << rb.ino;
                        dout(0) << css->strv() << dendl;
                    }
                    dir->unlink_inode(dn, false);
                }
                dn->set_alternate_name(mempool::mds_co::
                                       string(rb.alternate_name));
                dir->link_remote_inode(dn, rb.ino, rb.d_type);
                dn->set_version(rb.dnv);
                if (rb.dirty)
                    dn->_mark_dirty(logseg);
                dout(10) << "EMetaBlob.replay for [" << rb.dnfirst << "," << rb.
                    dnlast << "] had " << *dn << dendl;
                dn->first = rb.dnfirst;
            ...
            if (session) {
                ...
                if (used_preallocated_ino) {
                    if (!session->info.prealloc_inos.empty()) {
                        inodeno_t ino =
                            session->take_ino(used_preallocated_ino);
    ...
    if (!destroyed_inodes.empty()) {
        for (vector < inodeno_t >::iterator p = destroyed_inodes.begin();
            ...
            if (in) {
                ...
                if (parent) {
                    dout(10) << "EMetaBlob.replay unlinked from dentry " <<
                        *parent << dendl;
        ...
        mds->mdcache->open_file_table.note_destroyed_inos(logseg->seq,
                                                          destroyed_inodes);
    }
...
void EPurged::replay(MDSRank * mds)
{
    if (inos.size()) {
        ...
        if (mds->inotable->get_version() >= inotablev) {
            dout(10) << "EPurged.replay inotable " << mds->inotable->
                get_version()
...
void EPurged::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(1, 1, bl);
    encode(inos, bl);
    encode(inotablev, bl);
...
void EPurged::dump(Formatter * f) const const
{
...
void ECommitted::replay(MDSRank * mds)
{
    if (mds->mdcache->uncommitted_leaders.count(reqid)) {
        ...
        mds->mdcache->uncommitted_leaders[reqid].ls->uncommitted_leaders.
            erase(reqid);
...
void rename_rollback::generate_test_instances(std::list <
                                              rename_rollback * >&ls)
{
...
void EPeerUpdate::encode(bufferlist & bl, uint64_t features) const const
{
    ENCODE_START(3, 3, bl);
    encode(stamp, bl);
    encode(type, bl);
...
void EPeerUpdate::dump(Formatter * f) const const
{
    f->open_object_section("metablob");
    commit.dump(f);
    f->close_section();         // metablob
...
void EPeerUpdate::replay(MDSRank * mds)
{
    ...
    case EPeerUpdate::OP_PREPARE:
        dout(10) << "EPeerUpdate.replay prepare " << reqid << " for mds." <<
            leader << ": applying commit, saving rollback info" << dendl;
    ...
    case EPeerUpdate::OP_COMMIT:
        dout(10) << "EPeerUpdate.replay commit " << reqid << " for mds." <<
            leader << dendl;
        mds->mdcache->finish_uncommitted_peer(reqid, false);
        break;

    case EPeerUpdate::OP_ROLLBACK:
        dout(10) << "EPeerUpdate.replay abort " << reqid << " for mds." <<
            leader << ": applying rollback commit blob" << dendl;
...
void EFragment::replay(MDSRank * mds)
{
    ...
    case OP_ROLLBACK:{
            ...
            mds->mdcache->
                rollback_uncommitted_fragment(dirfrag_t(ino, basefrag),
                                              std::move(old_frags));
            break;
        }
...
bool EMetaBlob::rewrite_truncate_finish(MDSRank const *mds,
                                        std::map < LogSegment::seq_t,
                                        LogSegment::seq_t > const &old_to_new)
{
    bool modified = false;
