...
namespace librbd {
    ...
    namespace watcher {
        ...
        :ManagedLock < MockManagedLockImageCtx > (ioctx, work_queue, oid,
                                                  watcher,
                                                  librbd::managed_lock::
                                                  EXCLUSIVE, true, 0) {
        };
    ...
    namespace managed_lock {
        ...
        template < typename T > struct BaseRequest {
            ...
            static T *create(librados::IoCtx & ioctx,
                             MockImageWatcher * watcher, ContextWQ * work_queue,
                             const std::string & oid,
        ...
        template <>
            struct AcquireRequest <MockManagedLockImageCtx >:public BaseRequest
            < AcquireRequest < MockManagedLockImageCtx > > {
            static AcquireRequest *create(librados::IoCtx & ioctx,
                                          ...
                                          uint32_t blacklist_expire_seconds,
                                          Context * on_finish) {
                return BaseRequest::create(ioctx, watcher, work_queue, oid,
                                           cookie, on_finish);
            } MOCK_METHOD0(send, void ());
        };

        template <>
            struct ReacquireRequest <MockManagedLockImageCtx >:public
            BaseRequest < ReacquireRequest < MockManagedLockImageCtx > > {
            static ReacquireRequest *create(librados::IoCtx & ioctx,
                                            ...
                                            bool exclusive,
                                            Context * on_finish) {
                return BaseRequest::create(ioctx, nullptr, nullptr, oid,
                                           new_cookie, on_finish);
            } MOCK_METHOD0(send, void ());
        };

        template <>
            struct ReleaseRequest <MockManagedLockImageCtx >:public BaseRequest
            < ReleaseRequest < MockManagedLockImageCtx > > {
        ...
        template <> struct BreakRequest <MockManagedLockImageCtx > {
            static BreakRequest *create(librados::IoCtx & ioctx,
                                        ContextWQ * work_queue,
                                        const std::string & oid,
                                        const Locker & locker, bool exclusive,
...
namespace librbd {
      ...
      public:
        ...
        void expect_post_reacquired_lock_handler(MockImageWatcher & watcher,
            ...
            EXPECT_CALL(managed_lock, post_reacquire_lock_handler(_, _))
                .
                WillOnce(Invoke
                         ([&watcher, &client_id] (int r, Context * on_finish) {
                          if (r >= 0) {
                          client_id = 98765;}
        ...
        void expect_pre_release_lock_handler(MockMockManagedLock & managed_lock,
            ...
            EXPECT_CALL(managed_lock,
                        pre_release_lock_handler(shutting_down, _))
                .WillOnce(WithArg < 1 > (Invoke([r] (Context * on_finish) {
                                                on_finish->complete(r);})));
        }

        void expect_post_release_lock_handler(MockMockManagedLock &
                                              managed_lock, bool shutting_down,
                                              int expect_r, int r) {
            EXPECT_CALL(managed_lock,
                        post_release_lock_handler(shutting_down, expect_r, _))
    ...
    TEST_F(TestMockManagedLock, ReleaseLockBlacklist) {
        ...
        expect_post_release_lock_handler(managed_lock, false, -EBLACKLISTED,
                                         -EBLACKLISTED);
    ...
    TEST_F(TestMockManagedLock, ReacquireLock) {
        ...
        expect_reacquire_lock(*mock_image_ctx.image_watcher,
                              ictx->op_work_queue, mock_reacquire_request, 0);
    ...
    TEST_F(TestMockManagedLock, ReacquireLockError) {
        ...
        expect_get_watch_handle(*mock_image_ctx.image_watcher, 98765);
        expect_reacquire_lock(*mock_image_ctx.image_watcher,
                              ictx->op_work_queue, mock_reacquire_request,
                              -EOPNOTSUPP);
