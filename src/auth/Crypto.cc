...
class CryptoAESKeyHandler:public CryptoKeyHandler {
  public:
    ...
    int init(const bufferptr & s, ostringstream & err) {
        ...
        enc_key = new CryptoPP::AES::Encryption((byte *) secret.c_str(),
                                                CryptoPP::AES::
                                                DEFAULT_KEYLENGTH);
        dec_key =
            new CryptoPP::AES::Decryption((byte *) secret.c_str(),
    ...
    int encrypt(const bufferlist & in, bufferlist & out, std::string * error) const {
        string ciphertext;
         ...
         CryptoPP::CBC_Mode_ExternalCipher::Encryption cbc(*enc_key,
                                                           (const byte *)
                                                           CEPH_AES_IV);
    ...
    int decrypt(const bufferlist & in, bufferlist & out, std::string * error) const {
        string decryptedtext;
         ...
         CryptoPP::CBC_Mode_ExternalCipher::Decryption cbc(*dec_key,
                                                           (const byte *)
                                                           CEPH_AES_IV);
                             ...
                             std::string * error)
{
    ...
    ret = PK11_CipherOp(ectx,
                        (unsigned char *)out_tmp.c_str(), &written,
                        out_tmp.length(), in_buf, in.length());
    if (ret != SECSuccess) {
    ...
    ret = PK11_DigestFinal(ectx,
                           (unsigned char *)out_tmp.c_str() + written,
                           &written2, out_tmp.length() - written);
    PK11_DestroyContext(ectx, PR_TRUE);
    if (ret != SECSuccess) {
        if (error) {
            ostringstream oss;
...
class CryptoAESKeyHandler:public CryptoKeyHandler {
  ...
  public:
     CryptoAESKeyHandler()
    :mechanism(CKM_AES_CBC_PAD), slot(NULL), key(NULL), param(NULL) {
    ...
    int init(const bufferptr & s, ostringstream & err) {
        ...
        key =
            PK11_ImportSymKey_FIPS(slot, mechanism, PK11_OriginUnwrap,
    ...
    int encrypt(const bufferlist & in,
                bufferlist & out, std::string * error) const override {
        return nss_aes_operation(CKA_ENCRYPT, mechanism, key, param, in, out,
                                 error);
    } int decrypt(const bufferlist & in, bufferlist & out,
                  std::string * error) const override {
        return nss_aes_operation(CKA_DECRYPT, mechanism, key, param, in, out,
                                 error);
}};
