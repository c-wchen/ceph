...
string LogMonitor::log_channel_info::get_log_file(const string & channel)
{
    ...
    if (expanded_log_file.count(channel) == 0) {
        string fname =
            expand_channel_meta(get_str_map_key
                                (log_file, channel, &CLOG_CONFIG_DEFAULT_KEY),
                                channel);
        expanded_log_file[channel] = fname;

        dout(20) << __func__ << " for channel '"
            << channel << "' expanded to '" << fname << "'" << dendl;
    }
    return expanded_log_file[channel];
}

void LogMonitor::log_channel_info::expand_channel_meta(map < string,
                                                       string > &m)
{
...
string LogMonitor::log_channel_info::expand_channel_meta(const string & input,
                                                         const string &
                                                         change_to)
{
...
ceph::logging::Graylog::Ref LogMonitor::log_channel_info::
get_graylog(const string & channel)
{
    dout(25) << __func__ << " for channel '" << channel << "'" << dendl;
...
ceph::logging::JournaldClusterLogger & LogMonitor::log_channel_info::
get_journald()
{
...
void LogMonitor::log_external(const LogEntry & le)
{
    ...
    if (g_conf()->mon_cluster_log_to_file) {
        ...
        if (p == channel_fds.end()) {
            ...
            if (!log_file.empty()) {
                fd =::open(log_file.c_str(),
                           O_WRONLY | O_APPEND | O_CREAT | O_CLOEXEC, 0600);
                if (fd < 0) {
                    int err = -errno;
                    dout(1) << "unable to write to '" << log_file <<
                        "' for channel '" << channel << "': " <<
    ...
    if (do_stderr || fd >= 0) {
        ...
        if (fd >= 0) {
            ...
            if (err < 0) {
                dout(1) << "error writing to '" << channels.
...
void LogMonitor::log_external_backlog()
{
    if (!external_log_to) {
        ...
        if (r == 0) {
            ...
            dout(10) << __func__ << " initialized external_log_to = " <<
                external_log_to << " (recorded log_to position)" << dendl;
        }
        else {
...
void LogMonitor::generate_logentry_key(const std::string & channel,
                                       version_t v, std::string * out)
{
    out->append(channel);
    out->append("/");
    char vs[10];
...
bool LogMonitor::should_stash_full()
{
    ...
    auto period = std::min < uint64_t > (g_conf()->mon_log_full_interval,
...
version_t LogMonitor::get_trim_to() const const
{
    if (!mon.is_leader())
        return 0;
...
bool LogMonitor::preprocess_log(MonOpRequestRef op)
{
    ...
    dout(10) << "preprocess_log " << *m << " from " << m->
...
struct LogMonitor::C_Log:public C_MonOp {
    LogMonitor *logmon;
    C_Log(LogMonitor * p, MonOpRequestRef o):C_MonOp(o), logmon(p) {
...
bool LogMonitor::prepare_log(MonOpRequestRef op)
{
    ...
    auto m = op->get_req < MLog > ();
    dout(10) << "prepare_log " << *m << " from " << m->
        get_orig_source() << dendl;

    if (m->fsid != mon.monmap->fsid) {
        dout(0) << "handle_log on fsid " << m->fsid << " != " << mon.monmap->
            fsid << dendl;
        return false;
    }
        ...
        if (!summary.tail_by_channel.empty()) {
            ...
            if (channel == "*") {
              ...
              for (auto & p:                     summary.
                                                  ...
                                                  first << " from " << from <<
                                                  " to " << to << dendl;
                                                  for (version_t v = start;
                                                       v < to; ++v) {
                                                  bufferlist ebl; string key;
                                                  generate_logentry_key(p.first,
                                                                        v,
                                                                        &key);
                                                  int r =
                                                  mon.store->
              ...
              for (auto & p:                     entries)
                                                  ...
                                                  ("prepare_command");
                                                  auto m =
                                                  op->get_req < MMonCommand >
                                                  (); stringstream ss;
                                                  string rs; int err = -EINVAL;
                                                  cmdmap_t cmdmap;
                                                  if (!cmdmap_from_json
                                                      (m->cmd, &cmdmap, ss)) {
                                                  // ss has reason for failure
                                                  string rs = ss.str();
                                                  mon.reply_command(op, -EINVAL,
                                                                    rs,
                                                                    get_last_committed
                                                                    ());
                                                  return true;}
                                                  ...
                                                  cmd_getval(cmdmap, "logtext",
                                                             logtext);
                                                  LogEntry le;
                                                  le.rank =
                                                  m->get_orig_source();
                                                  le.addrs.v.push_back(m->
                                                  ...
                                                  getline(ss, rs);
                                                  mon.reply_command(op, err, rs,
                                                                    get_last_committed
                                                                    ());
                                                  return false;}

                                                  void LogMonitor::
                                                  dump_info(Formatter * f) {
                                                  f->
                                                  dump_unsigned
                                                  ("logm_first_committed",
                                                   get_first_committed());
                                                  f->
                                                  dump_unsigned
                                                  ("logm_last_committed",
                                                  ...
                                                  int LogMonitor::
                                                  sub_name_to_id(const string &
                                                                 n) {
                                                  if (n.substr(0, 4) == "log-"
                                                      && n.size() > 4) {
                                                  return LogEntry::
                                                  str_to_level(n.substr(4));}
                                                  else {
                                                  return CLOG_UNKNOWN;}
                                                  }
                                                  ...
                                                  void LogMonitor::
                                                  check_sub(Subscription * s) {
                                                  dout(10) << __func__ <<
                                                  " client wants " << s->
                                                  type << " ver " << s->
                                                  next << dendl;
                                                  int sub_level =
                                                  sub_name_to_id(s->type);
                                                  ceph_assert(sub_level >= 0);
                                                  version_t summary_version =
                                                  summary.version;
                                                  if (s->next >
                                                      summary_version) {
                                                  dout(10) << __func__ <<
                                                  ...
                                                  MLog * mlog =
                                                  new MLog(mon.monmap->fsid);
                                                  if (s->next == 0) {
                                                  /* First timer, heh? */
                                                  _create_sub_incremental(mlog,
                                                                          sub_level,
                                                                          get_last_committed
                                                                          ());}
                                                  else {
 ...
 */
                                                  ...
                                                  if (r < 0) {
                                                  derr << __func__ <<
                                                  " error parsing 'mon_cluster_log_to_syslog'"
                                                  << dendl; return;}
                                                  ...
                                                  if (r < 0) {
                                                  derr << __func__ <<
                                                  " error parsing 'mon_cluster_log_file'"
                                                  << dendl; return;}
