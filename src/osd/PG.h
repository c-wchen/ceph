...
class PGStateHistory {
  ...
  public:
    ...
    void set_pg_in_destructor() {
        pg_in_destructor = true;
    }
...
struct PGPool {
    ...
    :cct(cct),
        ...
        id(i), name(map->get_pool_name(id)), auid(map->get_pg_pool(id)->auid) {
...
class PG:public DoutPrefixProvider {
  ...
  /*** PG ****/ void update_snap_mapper_bits(uint32_t bits) {
  ...
  public:
    ...
    class MissingLoc {
      ...
      private:
        ...
        void pgs_by_shard_id(const set < pg_shard_t > &s, map < shard_id_t,
                             set < pg_shard_t > >&pgsbs) {
        ...
        void _dec_count(const set < pg_shard_t > &s) {
          ...
          for (auto shard:pgsbs) {
                auto p =
                    missing_by_count[shard.first].
                    find(_get_count(shard.second));
      ...
      public:
        ...
        void set_backend_predicates(IsPGReadablePredicate * _is_readable,
        ...
        } bool readable_with_acting(const hobject_t & hoid,
        ...
        void add_active_missing(const pg_missing_t & missing) {
            for (map < hobject_t, pg_missing_item >::const_iterator i =
                ...
                if (j == needs_recovery_map.end()) {
                    ...
                    lgeneric_dout(pg->cct,
                                  0) << this << " " << pg->info.
                        pgid << " unexpected need for " << i->
                        first << " have " << j->
                        second << " tried to add " << i->second << dendl;
                    assert(i->second.need == j->second.need);
                }
            }
        }

        void add_missing(const hobject_t & hoid, eversion_t need,
                         eversion_t have) {
        ...
        void add_batch_sources_info(const set < pg_shard_t > &sources,  ///< [in] a set of resources which can be used for all objects
                                    ThreadPool::TPHandle * handle   ///< [in] ThreadPool handle
            );
        ...
        void rebuild(const hobject_t & hoid,
                     pg_shard_t self,
            ...
            if (item->is_delete())
                return;
            auto mliter =
                missing_loc.insert(make_pair(hoid, set < pg_shard_t > ())).
                first;
        ...
        } const map < shard_id_t, map < loc_count_t,
  ...
  public:
    ...
    struct C_UpdateLastRollbackInfoTrimmedToApplied:Context {
        ...
        epoch_t e;
        eversion_t v;
        C_UpdateLastRollbackInfoTrimmedToApplied(PG * pg, epoch_t e,
                                                 eversion_t v)
        :pg(pg), e(e), v(v) {
  ...
  public:
    ...
    struct RecoveryCtx {
        ...
        RecoveryCtx(map < int, map < spg_t, pg_query_t > >*query_map,
                    ...
                    map < int,
                    vector < pair < pg_notify_t,
                    PastIntervals > > >*notify_list, C_Contexts * on_applied,
                    C_Contexts * on_safe,
                    ObjectStore::Transaction * transaction)
        :query_map(query_map), info_map(info_map), notify_list(notify_list),
            on_applied(on_applied), on_safe(on_safe), transaction(transaction),
            handle(NULL) {
        ...
        :query_map(&(buf.query_map)),
            ...
            transaction(rctx.transaction), handle(rctx.handle) {
        }
        ...
        void accept_buffered_messages(BufferedRecoveryMessages & m) {
            ...
            assert(info_map);
            assert(notify_list);
            for (map < int, map < spg_t, pg_query_t > >::iterator i =
                 m.query_map.begin(); i != m.query_map.end(); ++i) {
            ...
            for (map < int, vector < pair < pg_notify_t,
                 PastIntervals > > >::iterator i = m.info_map.begin();
                 i != m.info_map.end(); ++i) {
            ...
            for (map < int, vector < pair < pg_notify_t,
                 PastIntervals > > >::iterator i = m.notify_list.begin();
                 i != m.notify_list.end(); ++i) {
                vector < pair < pg_notify_t, PastIntervals > >&ovec =
                    (*notify_list)[i->first];
                ovec.reserve(ovec.size() + i->second.size());
                ovec.insert(ovec.end(), i->second.begin(), i->second.end());
   ...
   */
    struct BackfillInterval {
        ...
        /// clear objects list only void clear_objects() {
            objects.clear();
        }
  ...
  public:
    ...
    static pair < epoch_t,
        epoch_t > get_required_past_interval_bounds(const pg_info_t & info,
                                                    epoch_t oldest_map) {
        epoch_t start =
            MAX(info.history.last_epoch_clean ? info.history.
                last_epoch_clean : info.history.epoch_pool_created,
                oldest_map);
        epoch_t end = MAX(info.history.same_interval_since,
    ...
    struct PGLogEntryHandler:public PGLog::LogEntryHandler {
        ...
        // LogEntryHandler void remove(const hobject_t & hoid) override {
    ...
    void update_object_snap_mapping(ObjectStore::Transaction * t,
                                    const hobject_t & soid,
                                    const set < snapid_t > &snaps);
    void clear_object_snap_mapping(ObjectStore::Transaction * t,
                                   const hobject_t & soid);
    ...
    void discover_all_missing(std::map < int, map < spg_t,
                              pg_query_t > >&query_map);

    void trim_write_ahead();

    map < pg_shard_t,
        pg_info_t >::const_iterator find_best_info(const map < pg_shard_t,
                                                   pg_info_t > &infos,
                                                   bool restrict_to_up_acting,
                                                   bool *
                                                   history_les_bound) const;
    static void calc_ec_acting(map < pg_shard_t,
    ...
    static void calc_replicated_acting(map < pg_shard_t,
                                       pg_info_t >::
                                       ...
                                       set < pg_shard_t > *acting_backfill,
                                       pg_shard_t * want_primary, ostream & ss);
    bool choose_acting(pg_shard_t & auth_log_shard, bool restrict_to_up_acting,
   ...
   */
    virtual bool start_recovery_ops(uint64_t max,
                                    ThreadPool::TPHandle & handle,
    ...
    void add_backoff(SessionRef s, const hobject_t & begin,
                     const hobject_t & end);
    ...
    struct Scrubber {
        ...
        map < hobject_t, list < pair < ScrubMap::object,
            pg_shard_t > >>authoritative;

        // Cleaned map pending snap metadata scrub
        ScrubMap cleaned_meta_map;

        void clean_meta_map(ScrubMap & for_meta_scrub) {
            if (end.is_max() || cleaned_meta_map.objects.empty()) {
        ...
        static const char *state_string(const PG::Scrubber::State & state) {
            ...
            case FINISH:
                ret = "FINISH";
                break;
            case BUILD_MAP_REPLICA:
                ret = "BUILD_MAP_REPLICA";
                break;
            }
            return ret;
        }
    ...
    void repair_object(const hobject_t & soid, list < pair < ScrubMap::object,
   ...
   */
    virtual bool _range_available_for_scrub(const hobject_t & begin,
                                            const hobject_t & end) = 0;
    virtual void scrub_snapshot_metadata(ScrubMap & map,
    ...
    virtual void split_colls(spg_t child,
                             int split_bits,
                             int seed,
    ...
    template < class EVT > struct QueuePeeringEvt:Context {
        PGRef pg;
        epoch_t epoch;
        EVT evt;
        QueuePeeringEvt(PG * pg, epoch_t epoch, EVT evt):pg(pg), epoch(epoch),
            evt(evt) {
        } void finish(int r) override {
            pg->lock();
            pg->queue_peering_event(PG::
                                    CephPeeringEvtRef(new PG::
                                                      CephPeeringEvt(epoch,
                                                                     epoch,
                                                                     evt)));
            pg->unlock();
        }
      ...
      public:
        ...
        template < class T >
            CephPeeringEvt(epoch_t epoch_sent,
                           epoch_t epoch_requested,
                           const T & evt_):epoch_sent(epoch_sent),
            epoch_requested(epoch_requested), evt(evt_.intrusive_from_this()) {
            stringstream out;
    ...
    struct MInfoRec:boost::statechart::event < MInfoRec > {
        ...
        pg_info_t info;
        epoch_t msg_epoch;
        MInfoRec(pg_shard_t from, const pg_info_t & info,
                 epoch_t msg_epoch):from(from), info(info),
            msg_epoch(msg_epoch) {
        } void print(std::ostream * out) const {
    ...
    struct MLogRec:boost::statechart::event < MLogRec > {
        ...
        MLogRec(pg_shard_t from, MOSDPGLog * msg):from(from), msg(msg) {
        } void print(std::ostream * out) const {
    ...
    struct MNotifyRec:boost::statechart::event < MNotifyRec > {
        ...
        pg_notify_t notify;
        uint64_t features;
        MNotifyRec(pg_shard_t from, const pg_notify_t & notify,
                   uint64_t f):from(from), notify(notify), features(f) {
        } void print(std::ostream * out) const {
    ...
    struct MQuery:boost::statechart::event < MQuery > {
        ...
        pg_query_t query;
        epoch_t query_epoch;
        MQuery(pg_shard_t from, const pg_query_t & query,
               epoch_t query_epoch):from(from), query(query),
            query_epoch(query_epoch) {
        } void print(std::ostream * out) const {
            *out << "MQuery from " << from
                << " query_epoch " << query_epoch << " query: " << query;
    }};
    ...
    struct AdvMap:boost::statechart::event < AdvMap > {
        ...
        AdvMap(OSDMapRef osdmap, OSDMapRef lastmap,
               ...
               vector < int >&newacting, int acting_primary):osdmap(osdmap),
            lastmap(lastmap), newup(newup), newacting(newacting),
            up_primary(up_primary), acting_primary(acting_primary) {
        } void print(std::ostream * out) const {
            *out << "AdvMap";
    }};
    ...
    struct RequestBackfillPrio:boost::statechart::event < RequestBackfillPrio > {
        unsigned priority;
        explicit RequestBackfillPrio(unsigned prio):boost::statechart::event <
            RequestBackfillPrio > (), priority(prio) {
        } void print(std::ostream * out) const {
...
#define TrivialEvent(T) struct T : boost::statechart::event< T > { \
  ...
  protected:
    ...
    class RecoveryState {
      ...
      private:
        ...
        /* States */
        struct Initial;
        class RecoveryMachine:public boost::statechart::state_machine <
            RecoveryMachine, Initial > {
            RecoveryState *state;
          public:
            PG * pg;
            ...
            RecoveryMachine(RecoveryState * state, PG * pg):state(state),
                pg(pg), event_count(0) {
            }
            ...
            void send_query(pg_shard_t to, const pg_query_t & query) {
                ...
                (*state->rctx->query_map)[to.
                                          osd][spg_t(pg->info.pgid.pgid,
                                                     to.shard)] = query;
            }
            ...
            map < int, vector < pair < pg_notify_t,
                PastIntervals > >>*get_info_map() {
                assert(state->rctx);
        ...
        struct Initial:boost::statechart::state < Initial, RecoveryMachine >,
            ...
            boost::statechart::result react(const boost::statechart::
                                            event_base &) {
        ...
        struct Started:boost::statechart::state < Started, RecoveryMachine,
            Start >, NamedState {
        ...
        struct WaitActingChange:boost::statechart::state < WaitActingChange,
            ...
            typedef boost::mpl::list < boost::statechart::custom_reaction <
                QueryState >, boost::statechart::custom_reaction < AdvMap >,
        ...
        struct Active:boost::statechart::state < Active, Primary, Activating >,
            ...
            typedef boost::mpl::list <
                ...
                boost::statechart::custom_reaction < DoRecovery > >reactions;
            boost::statechart::result react(const QueryState & q);
            boost::statechart::result react(const ActMap &);
            boost::statechart::result react(const AdvMap &);
        ...
        struct Clean:boost::statechart::state < Clean, Active >, NamedState {
            typedef boost::mpl::list <
                ...
                WaitLocalRecoveryReserved > >reactions;
        ...
        struct Recovered:boost::statechart::state < Recovered, Active >,
            NamedState {
            typedef boost::mpl::list < boost::statechart::transition < GoClean,
                Clean >, boost::statechart::transition < DoRecovery,
                WaitLocalRecoveryReserved >,
                boost::statechart::custom_reaction < AllReplicasActivated >
                >reactions;
        ...
        struct Backfilling:boost::statechart::state < Backfilling, Active >,
            ...
            boost::statechart::
                result react(const RemoteReservationRejected & evt);
            boost::statechart::result react(const DeferBackfill & evt);
            boost::statechart::result react(const UnfoundBackfill & evt);
            void exit();
        };
        ...
        struct WaitLocalBackfillReserved:boost::statechart::state <
            ...
            typedef boost::mpl::list < boost::statechart::transition <
                LocalBackfillReserved, WaitRemoteBackfillReserved > >reactions;
            explicit WaitLocalBackfillReserved(my_context ctx);
            void exit();
        };

        struct NotBackfilling:boost::statechart::state < NotBackfilling,
            Active >, NamedState {
            typedef boost::mpl::list < boost::statechart::transition <
        ...
        struct RepRecovering:boost::statechart::state < RepRecovering,
            ...
            boost::statechart::transition < RemoteReservationRejected,
                RepNotRecovering >,
        ...
        struct RepWaitBackfillReserved:boost::statechart::state <
            ...
            boost::statechart::result react(const RemoteBackfillReserved & evt);
            boost::statechart::
                result react(const RejectRemoteReservation & evt);
            boost::statechart::
                result react(const RemoteReservationRejected & evt);
            boost::statechart::
        ...
        struct Stray:boost::statechart::state < Stray, Started >, NamedState {
            ...
            typedef boost::mpl::list <
                ...
                boost::statechart::custom_reaction < MInfoRec >,
                boost::statechart::custom_reaction < ActMap >,
                boost::statechart::custom_reaction < RecoveryDone > >reactions;
     ...
     */
      ...
      :    machine(this, pg), pg(pg), orig_ctx(0) {
        ...
        void handle_event(CephPeeringEvtRef evt, RecoveryCtx * rctx) {
            start_handle(rctx);
  ...
  public:
    ...
    } void init_primary_up_acting(const vector < int >&newup,
    ...
    //int  get_state() const { return state; } bool state_test(int m)const {
        return (state & m) != 0;
  ...
  public:
    static int _prepare_write_info(CephContext * cct,
    ...
    bool check_in_progress_op(const osd_reqid_t & r,
                              eversion_t * version,
                              version_t * user_version, int *return_code) const;
    ...
    void append_log(const vector < pg_log_entry_t > &logv,
                    eversion_t trim_to,
    ...
    static int read_info(ObjectStore * store, spg_t pgid, const coll_t & coll,
    ...
    void update_snap_map(const vector < pg_log_entry_t > &log_entries,
                         ObjectStore::Transaction & t);
   ...
   */
    ...
    bool old_peering_evt(CephPeeringEvtRef evt) {
        return old_peering_msg(evt->get_epoch_sent(),
    ...
    // abstract bits
    virtual void do_request(OpRequestRef & op,
    ...
    virtual void do_scan(OpRequestRef op, ThreadPool::TPHandle & handle) = 0;
    virtual void do_backfill(OpRequestRef op) = 0;
    virtual void snap_trimmer(epoch_t epoch_queued) = 0;

    virtual int do_command(cmdmap_t cmdmap,
                           ostream & ss,
                           bufferlist & idata,
                           bufferlist & odata,
                           ConnectionRef conn, ceph_tid_t tid) = 0;
