...
class PurgeLogShardsCR:public RGWShardCollectCR {
    ...
    int handle_result(int r) override {
        if (r == -ENOENT) {     // ENOENT is not a fatal error
            ...
            ldout(cct,
                  4) << "failed to remove mdlog shard: " << cpp_strerror(r) <<
                dendl;
        }
        return r;
    }
...
class PurgePeriodLogsCR:public RGWCoroutine {
  ...
  public:
    PurgePeriodLogsCR(const DoutPrefixProvider * dpp,
                      rgw::sal::RadosStore * store, epoch_t realm_epoch,
                      epoch_t * last_trim)
    :RGWCoroutine(store->ctx()), dpp(dpp), store(store),
        metadata(store->ctl()->meta.mgr), realm_epoch(realm_epoch),
...
int PurgePeriodLogsCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        while (cursor.get_epoch() < realm_epoch) {
            ...
            yield {
                const auto mdlog =
                    svc.mdlog->get_log(cursor.get_period().get_id());
    ...
    template < typename Zonegroups >
      ...
      for (auto & g:zonegroups) {
          for (auto & z:g.second.zones) {
                ...
                new RGWRESTConn(store->ctx(), store, z.first.id,
                                    z.second.endpoints, g.second.api_name)};
...
/// peer whose realm_epoch matches the minimum realm_epoch in the input
    ...
    struct TrimEnv {
        ...
        TrimEnv(const DoutPrefixProvider * dpp, rgw::sal::RadosStore * store,
    ...
    struct MasterTrimEnv:public TrimEnv {
        ...
        MasterTrimEnv(const DoutPrefixProvider * dpp,
                      rgw::sal::RadosStore * store, RGWHTTPManager * http,
                      int num_shards)
    ...
    struct PeerTrimEnv:public TrimEnv {
        ...
        PeerTrimEnv(const DoutPrefixProvider * dpp,
                    rgw::sal::RadosStore * store, RGWHTTPManager * http,
                    int num_shards)
  ...
  private:
    ...
    int handle_result(int r) override {
        if (r == -ENOENT) {     // ENOENT is not a fatal error
            ...
            ldout(cct,
                  4) << "failed to trim mdlog shard: " << cpp_strerror(r) <<
                dendl;
        }
        return r;
    }
...
bool MetaMasterTrimShardCollectCR::spawn_next()
{
    while (shard_id < env.num_shards) {
        ...
        spawn(new RGWSyncLogTrimCR(env.dpp, env.store, oid, stable, &last_trim),
              false);
        shard_id++;
        return true;
    }
...
int MetaMasterTrimCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        ldpp_dout(dpp,
                  10) << "fetching sync status for zone " << env.zone << dendl;
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      4) << "failed to fetch sync status from all peers" <<
                dendl;
        ...
        yield {
            ...
            if (epoch > env.last_trim_epoch + 1) {
                ...
                spawn(new
                      PurgePeriodLogsCR(dpp, store, epoch,
            ...
            if (epoch == env.current.get_epoch()) {
                auto mdlog =
                    store->svc()->mdlog->get_log(env.current.get_period().
                                                 get_id());
...
int MetaPeerTrimShardCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        if (result.entries.empty()) {
            ...
            yield
                call(create_read_remote_mdlog_shard_info_cr
            ...
            yield
                call(create_list_remote_mdlog_shard_cr
        ...
        yield {
            std::string oid;
            mdlog->get_shard_oid(shard_id, oid);
            call(new RGWRadosTimelogTrimCR(dpp, env.store, oid, real_time {
                                           }
                                           , stable, "", ""));
        }
...
class MetaPeerTrimShardCollectCR:public RGWShardCollectCR {
  ...
  :    
    ...
    env(env), mdlog(mdlog), period_id(env.current.get_period().get_id()) {
        meta_env.init(env.dpp, cct, env.store,
                      env.store->svc()->zone->get_master_conn(),
                      env.store->svc()->rados->get_async_processor(), env.http,
                      nullptr, env.store->getRados()->get_sync_tracer());
    }
...
class MetaPeerTrimCR:public RGWCoroutine {
  ...
  public:
    explicit MetaPeerTrimCR(PeerTrimEnv & env):RGWCoroutine(env.store->ctx()),
        env(env) {
    } int operate(const DoutPrefixProvider * dpp) override;
};
...
int MetaPeerTrimCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        ...
        yield {
            ...
            call(new
                 LogInfoCR(cct, env.store->svc()->zone->get_master_conn(),
                           env.http, "/admin/log/", params, &mdlog_info));
        }
        if (retcode < 0) {
            ldpp_dout(dpp,
        ...
        if (mdlog_info.realm_epoch > env.last_trim_epoch + 1) {
            // delete any prior mdlog periods
            yield call(new
                       PurgePeriodLogsCR(dpp, env.store, mdlog_info.realm_epoch,
                                         &env.last_trim_epoch));
        }
        else {
        ...
        if (mdlog_info.realm_epoch == env.current.get_epoch()) {
            yield {
                auto mdlog =
                    env.store->svc()->mdlog->get_log(env.current.get_period().
                                                     get_id());
...
int MetaTrimPollCR::operate(const DoutPrefixProvider * dpp)
{
    reenter(this) {
        for (;;) {
            ...
            yield call(new
                       RGWSimpleRadosLockCR(store->svc()->rados->
                                            get_async_processor(), store, obj,
                                            name, cookie, interval.sec()));
            if (retcode < 0) {
                ldout(cct,
            ...
            if (retcode < 0) {
                ...
                yield call(new
                           RGWSimpleRadosUnlockCR(store->svc()->rados->
...
class MetaMasterTrimPollCR:public MetaTrimPollCR {
    ...
    MetaMasterTrimPollCR(const DoutPrefixProvider * dpp,
                         rgw::sal::RadosStore * store, RGWHTTPManager * http,
...
class MetaPeerTrimPollCR:public MetaTrimPollCR {
    ...
    MetaPeerTrimPollCR(const DoutPrefixProvider * dpp,
                       rgw::sal::RadosStore * store, RGWHTTPManager * http,
                       int num_shards, utime_t interval)
    :MetaTrimPollCR(store, interval), env(dpp, store, http, num_shards) {
    }
      ...
      for (const auto &[_, zonegroup]:period.get_map().zonegroups) {
            if (zonegroup.endpoints.empty()) {
                ldpp_dout(dpp, -1)
                    ...
                    << period.get_realm_name() << " ( " << period.
                    get_realm() << ") " << " has no endpoints!" << dendl;
            }
          for (const auto &[_, zone]:zonegroup.zones) {
                if (zone.endpoints.empty()) {
                    ldpp_dout(dpp, -1)
                        ...
                        << " Zone " << zone.name << " (" << zone.
                        id << ") in Zonegroup " << zonegroup.
                        get_name() << " ( " << zonegroup.get_id()
                                      ...
                                      utime_t interval)
{
    if (!sanity_check_endpoints(dpp, store)) {
        ldpp_dout(dpp, -1)
            << __PRETTY_FUNCTION__ << ":" << __LINE__
            << " ERROR: Cluster is is misconfigured! Refusing to trim." <<
            dendl;
        return nullptr;
    }
...
struct MetaMasterAdminTrimCR:private MasterTrimEnv, public MetaMasterTrimCR {
    MetaMasterAdminTrimCR(const DoutPrefixProvider * dpp,
                          rgw::sal::RadosStore * store, RGWHTTPManager * http,
                          int num_shards)
...
struct MetaPeerAdminTrimCR:private PeerTrimEnv, public MetaPeerTrimCR {
    MetaPeerAdminTrimCR(const DoutPrefixProvider * dpp,
                        rgw::sal::RadosStore * store, RGWHTTPManager * http,
                        int num_shards)
