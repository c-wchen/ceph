                    ...
                    list < rgw_obj > *test_objs)
{
    ...
    *head = rgw_obj(*bucket, "oid");
    gen->create_begin(g_ceph_context, manifest, placement_rule, nullptr,
                      *bucket, *head);
...
TEST(TestRGWManifest, multipart)
{
    ...
    for (int i = 0; i < num_parts; ++i) {
        ...
        for (ofs = 0; ofs < part_size; ofs += stripe_size) {
            if (ofs == 0) {
                rgw_placement_rule rule(env.zonegroup.default_placement.name,
                                        RGW_STORAGE_CLASS_STANDARD);
                int r =
                    gen.create_begin(g_ceph_context, &manifest, rule, nullptr,
                                     bucket, head);
                ASSERT_EQ(r, 0);
                continue;
            }
