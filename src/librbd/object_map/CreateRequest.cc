...
namespace librbd {
    namespace object_map {
        ...
        using util::create_rados_callback;
         ...
         template < typename I >
            CreateRequest < I >::CreateRequest(I * image_ctx,
                                               Context * on_finish)
...

            {
                ...
                for (auto it:m_image_ctx->snap_info) {
                    ...
                    m_snap_ids.push_back(it.first);
                      ...
                      is_compatible(m_image_ctx->layout, max_size)) {
        ...
        template < typename I >
            ...
            Context *ctx = create_context_callback <
                CreateRequest < I >,
                &CreateRequest < I >::handle_object_map_resize > (this);
        ...
        template < typename I >
            ...
            I >::handle_object_map_resize(int *result) {
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 20) << __func__ << ": r=" << *result << dendl;

            if (*result < 0) {
                lderr(cct) << "object map resize failed: " <<
                    cpp_strerror(*result)
                    << dendl;
            }
            return m_on_finish;
