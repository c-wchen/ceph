...
static int create_image_pp(librbd::RBD & rbd,
                           ...
                           const char *name, uint64_t size, int *order)
{
    bool old_format;
    uint64_t features;
    int r = get_features(&old_format, &features);
...
void aio_write_test_data_and_poll(rbd_image_t image, int fd,
                                  const char *test_data, uint64_t off,
                                  size_t len, uint32_t iohint, bool * passed)
{
    rbd_completion_t comp;
    uint64_t data = 0x123;
    rbd_aio_create_completion((void *)&data, (rbd_callback_t) simple_write_cb,
                              &comp);
...
void aio_write_test_data(rbd_image_t image, const char *test_data, uint64_t off,
                         size_t len, uint32_t iohint, bool * passed)
{
    rbd_completion_t comp;
...
void write_test_data(rbd_image_t image, const char *test_data, uint64_t off,
                     size_t len, uint32_t iohint, bool * passed)
{
    ssize_t written;
    if (iohint)
...
void aio_read_test_data_and_poll(rbd_image_t image, int fd,
                                 const char *expected, uint64_t off, size_t len,
...
void aio_read_test_data(rbd_image_t image, const char *expected, uint64_t off,
                        size_t len, uint32_t iohint, bool * passed)
{
    rbd_completion_t comp;
    char *result = (char *)malloc(len + 1);
...
void read_test_data(rbd_image_t image, const char *expected, uint64_t off,
                    size_t len, uint32_t iohint, bool * passed)
{
    ssize_t read;
    char *result = (char *)malloc(len + 1);
...
void aio_writesame_test_data(rbd_image_t image, const char *test_data,
                             uint64_t off, uint64_t len, uint64_t data_len,
                             uint32_t iohint, bool * passed)
{
    rbd_completion_t comp;
    rbd_aio_create_completion(NULL, (rbd_callback_t) simple_write_cb, &comp);
...
void writesame_test_data(rbd_image_t image, const char *test_data, uint64_t off,
                         uint64_t len, uint64_t data_len, uint32_t iohint,
                         bool * passed)
{
    ssize_t written;
                                     ...
                                     size_t len, uint32_t iohint, bool * passed)
{
    ...
    rbd_aio_compare_and_write(image, off, len, cmp_data, test_data, comp,
                              &mismatch_offset, iohint);
...
void compare_and_write_test_data(rbd_image_t image, const char *cmp_data,
                                 const char *test_data, uint64_t off,
                                 size_t len, uint64_t * mismatch_off,
                                 uint32_t iohint, bool * passed)
{
    printf("start compare and write\n");
    ssize_t written;
    written =
        rbd_compare_and_write(image, off, len, cmp_data, test_data,
                              mismatch_off, iohint);
...
class TestLibRBD:public::testing::Test {
  public:
    ...
    void test_io(rbd_image_t image) {
        ...
        for (i = 5; i < 10; ++i)
            ASSERT_PASSED(aio_write_test_data, image, test_data,
                          TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

        for (i = 0; i < 5; ++i)
            ASSERT_PASSED(compare_and_write_test_data, image, test_data,
                          test_data, TEST_IO_SIZE * i, TEST_IO_SIZE,
                          &mismatch_offset, 0);

        for (i = 5; i < 10; ++i)
            ASSERT_PASSED(aio_compare_and_write_test_data, image, test_data,
                          test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);
        ...
        for (i = 5; i < 10; ++i)
            ASSERT_PASSED(aio_read_test_data, image, test_data,
                          TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

        // discard 2nd, 4th sections.
        ASSERT_PASSED(discard_test_data, image, TEST_IO_SIZE, TEST_IO_SIZE);
        ASSERT_PASSED(aio_discard_test_data, image, TEST_IO_SIZE * 3,
                      TEST_IO_SIZE);

        ASSERT_PASSED(read_test_data, image, test_data, 0, TEST_IO_SIZE, 0);
        ASSERT_PASSED(read_test_data, image,
                      skip_discard ? test_data : zero_data, TEST_IO_SIZE,
                      TEST_IO_SIZE, 0);
        ASSERT_PASSED(read_test_data, image, test_data, TEST_IO_SIZE * 2,
                      TEST_IO_SIZE, 0);
        ASSERT_PASSED(read_test_data, image,
                      skip_discard ? test_data : zero_data, TEST_IO_SIZE * 3,
                      TEST_IO_SIZE, 0);
        ...
        for (i = 0; i < 15; ++i) {
            if (i % 3 == 2) {
                ASSERT_PASSED(writesame_test_data, image, test_data,
                              TEST_IO_SIZE * i, TEST_IO_SIZE * i * 32 + i,
                              TEST_IO_SIZE, 0);
                ASSERT_PASSED(writesame_test_data, image, zero_data,
                              TEST_IO_SIZE * i, TEST_IO_SIZE * i * 32 + i,
                              TEST_IO_SIZE, 0);
            }
            else if (i % 3 == 1) {
                ASSERT_PASSED(writesame_test_data, image, test_data,
                              TEST_IO_SIZE + i, TEST_IO_SIZE * i * 32,
                              TEST_IO_SIZE, 0);
                ASSERT_PASSED(writesame_test_data, image, zero_data,
                              TEST_IO_SIZE + i, TEST_IO_SIZE * i * 32,
                              TEST_IO_SIZE, 0);
        ...
        mismatch_offset = 123;
        ASSERT_EQ(0,
                  rbd_aio_compare_and_write(image, 0, TEST_IO_SIZE,
                                            mismatch_data, mismatch_data, comp,
                                            &mismatch_offset, 0));
        ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
...
TEST_F(TestLibRBD, TestDeepCopyPP)
{
...

    {
        ...
        ASSERT_EQ(0,
                  image.deep_copy_with_progress(ioctx, name3.c_str(), opts,
                                                pp));
        ASSERT_EQ(3,
                  test_ls_pp(rbd, ioctx, 3, name.c_str(), name2.c_str(),
                             name3.c_str()));
        ASSERT_EQ(0, rbd.open(ioctx, image3, name3.c_str(), NULL));
    ...
    for (i = num_expected; i > 0; i--) {
        ...
        for (j = 0; j < snaps.size(); j++) {
            ...
            if (strcmp(snaps[j].name.c_str(), expected) == 0) {
                cout << "found " << snaps[j].name << " with size " << snaps[j].
                    size << std::endl;
                EXPECT_EQ(expected_size, snaps[j].size);
                snaps[j].name = "";
                found = 1;
...
TEST_F(TestLibRBD, TestCreateLsRenameSnapPP)
{
...

    {
        ...
        ASSERT_EQ(2,
                  test_ls_snaps(image, 2, "snap1-rename", size, "snap2",
                                size2));
        ASSERT_EQ(0, image.snap_exists2("snap1", &exists));
        ASSERT_FALSE(exists);
...
TEST_F(TestLibRBD, ConcurrentCreatesUnvalidatedPool)
{
  ...
  for (const auto & name:names) {
        threads.emplace_back([ioctx, &name, size] () {
                             int order = 0;
                             ASSERT_EQ(0,
                                       create_image(ioctx, name.c_str(), size,
                                                    &order));});
    }
...
TEST_F(TestLibRBD, TestIO)
{
    ...
    ASSERT_EQ(0, create_image(ioctx, name.c_str(), size, &order));
    ASSERT_EQ(0,
              rados_conf_set(_cluster, "rbd_read_from_replica_policy",
                             "balance"));
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
...
TEST_F(TestLibRBD, TestEncryptionLUKS1)
{
    ...
    ASSERT_EQ(-ENOTSUP,
              rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS1,
                                    &luks1_opts, sizeof(luks1_opts)));
    ASSERT_EQ(-ENOTSUP,
              rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1,
                                  &luks1_opts, sizeof(luks1_opts)));
    ASSERT_EQ(-ENOTSUP,
              rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS2,
                                    &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(-ENOTSUP,
              rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2,
                                  &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(-ENOTSUP,
              rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS,
                                    &luks_opts, sizeof(luks_opts)));
    ASSERT_EQ(-ENOTSUP,
              rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts,
                                  sizeof(luks_opts)));
#else
    ASSERT_EQ(-EINVAL,
              rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS,
...
TEST_F(TestLibRBD, TestCloneEncryption)
{
    ...
    ASSERT_EQ(-EINVAL,
              rbd_encryption_load(child1, RBD_ENCRYPTION_FORMAT_LUKS1,
                                  &child1_opts, sizeof(child1_opts)));
    ASSERT_EQ(0,
              rbd_encryption_format(child1, RBD_ENCRYPTION_FORMAT_LUKS1,
                                    &child1_opts, sizeof(child1_opts)));
    ASSERT_EQ(0,
              rbd_encryption_load(child1, RBD_ENCRYPTION_FORMAT_LUKS1,
                                  &child1_opts, sizeof(child1_opts)));
    ...
    ASSERT_EQ(0,
              rbd_encryption_format(child2, RBD_ENCRYPTION_FORMAT_LUKS2,
                                    &child2_opts, sizeof(child2_opts)));
    rbd_encryption_luks_format_options_t child2_lopts = {
        .passphrase = "password",
        .passphrase_size = 8,
    };
    ASSERT_EQ(0,
              rbd_encryption_load(child2, RBD_ENCRYPTION_FORMAT_LUKS,
                                  &child2_lopts, sizeof(child2_lopts)));
    ...
    ASSERT_EQ(0,
              rbd_encryption_format(child3, RBD_ENCRYPTION_FORMAT_LUKS2,
                                    &child3_opts, sizeof(child3_opts)));
    ASSERT_EQ(-EPERM,
              rbd_encryption_load(child3, RBD_ENCRYPTION_FORMAT_LUKS2,
                                  &child3_opts, sizeof(child3_opts)));
    ...
    ASSERT_EQ(0, rbd_open(ioctx, child4_name.c_str(), &child4, NULL));
    ASSERT_EQ(0,
              rbd_encryption_load(child4, RBD_ENCRYPTION_FORMAT_LUKS2,
                                  &child3_opts, sizeof(child3_opts)));
...
TEST_F(TestLibRBD, LUKS1UnderLUKS2WithoutResize)
{
...

    {
        ...
        ASSERT_EQ(0,
                  parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts,
                                           sizeof(fopts)));
...

    {
        ...
        ASSERT_EQ(0,
                  clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &fopts,
                                          sizeof(fopts)));

        librbd::encryption_luks_format_options_t opts1 = {
        parent_passphrase};
        librbd::encryption_luks_format_options_t opts2 = {
        clone_passphrase};
        ...
        expected_bl.
            append(std::
                   string(data_size + luks1_meta_size - luks2_meta_size, 'a'));
...
TEST_F(TestLibRBD, LUKS2UnderLUKS1WithoutResize)
{
...

    {
        ...
        RBD_ENCRYPTION_ALGORITHM_AES256, parent_passphrase};
        ASSERT_EQ(0,
                  parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &fopts,
                                           sizeof(fopts)));
...

    {
        ...
        RBD_ENCRYPTION_ALGORITHM_AES256, clone_passphrase};
        ASSERT_EQ(0,
                  clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts,
                                          sizeof(fopts)));
...
TEST_F(TestLibRBD, EncryptionFormatNoData)
{
...

    {
        int order = 0;
        ASSERT_EQ(0,
                  create_image_pp(rbd, ioctx, name.c_str(), luks1_meta_size - 1,
                                  &order));
        librbd::Image image;
...
TEST_F(TestLibRBD, EncryptionLoadBadSize)
{
...

    {
        ...
        passphrase};
        ASSERT_EQ(-EINVAL,
                  image.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS, &opts,
                                        sizeof(opts)));
        uint64_t size;
        ASSERT_EQ(0, image.size(&size));
        ASSERT_EQ(luks1_meta_size - 1, size);
...
TEST_F(TestLibRBD, EncryptionLoadBadStripePattern)
{
...

    {
        int order = 22;
        ASSERT_EQ(0,
                  rbd.create3(ioctx, name1.c_str(), 20 << 20, features, &order,
                              2 << 20, 2));
        librbd::Image image;
...
TEST_F(TestLibRBD, EncryptionLoadFormatMismatch)
{
    ...
    RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
    librbd::encryption_luks_format_options_t luks_opts = {
    passphrase};
...
#define LUKS_ANY {RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts)}
    ...
    static_assert(std::size(bad_specs) + std::size(good_specs) ==
                  1 + 3 + 9 + 27 + 1);
...
#undef LUKS_ANY

    {
        int order = 0;
        ASSERT_EQ(0,
                  create_image_pp(rbd, ioctx, name1.c_str(), 20 << 20, &order));
        librbd::Image image1;
        ...
        ASSERT_EQ(0,
                  rbd.clone(ioctx, name1.c_str(), "snap", ioctx, name2.c_str(),
                            features, &order));
        ...
        ASSERT_EQ(0,
                  rbd.clone(ioctx, name2.c_str(), "snap", ioctx, name3.c_str(),
                            features, &order));
...

    {
      ...
      for (auto & specs:bad_specs) {
            ASSERT_EQ(-EINVAL,
                      image3.encryption_load2(specs.data(), specs.size()));
        }
    }
...
TEST_F(TestLibRBD, EncryptedFlattenSmallData)
{
...

    {
        ...
        RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
        ASSERT_EQ(0,
                  parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &opts,
                                           sizeof(opts)));
...
struct LUKS1UnderLUKS2:LUKSTwoPassphrases {
    void setup_parent(librbd::Image & parent, uint64_t data_size, bool * passed) {
        ...
        uint64_t luks2_meta_size = 16 << 20;
        ASSERT_EQ(0,
                  parent.resize(data_size + luks2_meta_size - luks1_meta_size));
        *passed = true;
    }
...
template < typename FormatPolicy > class EncryptedFlattenTest:public TestLibRBD,
    FormatPolicy {
  protected:
    void create_and_setup(bool * passed) {
        ...
        librbd::Image parent;
        ASSERT_EQ(0,
                  m_rbd.open(m_ioctx, parent, m_parent_name.c_str(), nullptr));
        ...
        ASSERT_EQ(0, parent.features(&features));
        ASSERT_EQ(0,
                  m_rbd.clone(m_ioctx, m_parent_name.c_str(), "snap", m_ioctx,
    ...
    void verify_data(librbd::Image & image,
...
using EncryptedFlattenTestTypes =::testing::Types < PlaintextUnderLUKS1,
    PlaintextUnderLUKS2, UnformattedLUKS1, LUKS1UnderLUKS1, LUKS1UnderLUKS2,
...
TYPED_TEST(EncryptedFlattenTest, Simple)
{
...

    {
        ...
        this->verify_size_and_overlap(clone, this->m_data_size,
                                      this->m_data_size);
...
TEST_F(TestLibRBD, TestIOWithIOHint)
{
    ...
    for (i = 0; i < 15; ++i) {
        if (i % 3 == 2) {
            ...
            ASSERT_PASSED(writesame_test_data, image, test_data,
                          TEST_IO_SIZE + i, TEST_IO_SIZE * i * 32, TEST_IO_SIZE,
                          LIBRADOS_OP_FLAG_FADVISE_NOCACHE);
            ASSERT_PASSED(writesame_test_data, image, zero_data,
                          TEST_IO_SIZE + i, TEST_IO_SIZE * i * 32, TEST_IO_SIZE,
                          LIBRADOS_OP_FLAG_FADVISE_NOCACHE);
    ...
    mismatch_offset = 123;
    ASSERT_EQ(-EILSEQ,
              rbd_compare_and_write(image, 0, TEST_IO_SIZE, mismatch_data,
                                    mismatch_data, &mismatch_offset,
                                    LIBRADOS_OP_FLAG_FADVISE_DONTNEED));
    ASSERT_EQ(0U, mismatch_offset);
    rbd_aio_create_completion(NULL, (rbd_callback_t) simple_read_cb, &comp);
    mismatch_offset = 123;
    ASSERT_EQ(0,
              rbd_aio_compare_and_write(image, 0, TEST_IO_SIZE, mismatch_data,
                                        mismatch_data, comp, &mismatch_offset,
                                        LIBRADOS_OP_FLAG_FADVISE_DONTNEED));
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ASSERT_EQ(-EILSEQ, rbd_aio_get_return_value(comp));
    ASSERT_EQ(0U, mismatch_offset);
...
TEST_F(TestLibRBD, TestCompareAndWriteMismatch)
{
    ...
    ssize_t read =
        rbd_read(image, off, read_buffer.length(), read_buffer.data());
...
TEST_F(TestLibRBD, TestEmptyDiscard)
{
    ...
    ASSERT_PASSED(aio_discard_test_data, image, 3 * 1024 * 1024,
                  1 * 1024 * 1024);
                         ...
                         off_t off, uint32_t iohint, bool * passed)
{
    ...
    librbd::RBD::AioCompletion * comp =
        new librbd::RBD::AioCompletion(NULL,
                                       (librbd::callback_t) simple_write_cb_pp);
    printf("created completion\n");
    if (iohint)
...
void aio_read_test_data(librbd::Image & image, const char *expected, off_t off,
                        size_t expected_len, uint32_t iohint, bool * passed)
{
    librbd::RBD::AioCompletion * comp =
        new librbd::RBD::AioCompletion(NULL,
                                       (librbd::callback_t) simple_read_cb_pp);
    ceph::bufferlist bl;
    printf("created completion\n");
    if (iohint)
...
void aio_writesame_test_data(librbd::Image & image, const char *test_data,
                             off_t off, size_t len, size_t data_len,
                             uint32_t iohint, bool * passed)
{
    ceph::bufferlist bl;
...
void writesame_test_data(librbd::Image & image, const char *test_data,
                         off_t off, ssize_t len, size_t data_len,
                         uint32_t iohint, bool * passed)
{
    ssize_t written;
    ceph::bufferlist bl;
...
void aio_compare_and_write_test_data(librbd::Image & image,
                                     const char *cmp_data,
                                     const char *test_data, off_t off,
                                     ssize_t len, uint32_t iohint,
                                     bool * passed)
{
    ...
    uint64_t mismatch_offset;
    image.aio_compare_and_write(off, len, cmp_bl, test_bl, comp,
                                &mismatch_offset, iohint);
    printf("started aio compare and write\n");
    comp->wait_for_complete();
    int r = comp->get_return_value();
...
void compare_and_write_test_data(librbd::Image & image, const char *cmp_data,
                                 const char *test_data, off_t off, ssize_t len,
                                 uint64_t * mismatch_off, uint32_t iohint,
                                 bool * passed)
{
    size_t written;
    ceph::bufferlist cmp_bl;
    ...
    printf("start compare and write\n");
    written =
        image.compare_and_write(off, len, cmp_bl, test_bl, mismatch_off,
                                iohint);
    printf("compare and  wrote: %d\n", (int)written);
...
TEST_F(TestLibRBD, TestIOPP)
{
...

    {
        ...
        for (i = 0; i < 5; ++i)
            ASSERT_PASSED(write_test_data, image, test_data,
                          strlen(test_data) * i, 0);
        ...
        for (i = 0; i < 5; ++i)
            ASSERT_PASSED(read_test_data, image, test_data,
                          strlen(test_data) * i, TEST_IO_SIZE, 0);
...
TEST_F(TestLibRBD, TestCompareAndWriteCompareTooSmallPP)
{
...

    {
        ...
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off, cmp_bl.length(), small_bl,   /* cmp_bl */
...
TEST_F(TestLibRBD, TestCompareAndWriteWriteTooSmallPP)
{
...

    {
        ...
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off, cmp_bl.length(), cmp_bl, small_bl,   /* write_bl */
                                          &mismatch_off, 0);
        ASSERT_EQ(-EINVAL, written);
        ASSERT_EQ(0U, mismatch_off);
...
TEST_F(TestLibRBD, TestCompareAndWriteMismatchPP)
{
...

    {
        ...
        // this should execute the compare but fail because of mismatch
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off, write_bl.length(), mismatch_bl,  /* cmp_bl */
                                          write_bl, &mismatch_off, 0);
        ASSERT_EQ(-EILSEQ, written);
        ASSERT_EQ(5U, mismatch_off);
...
TEST_F(TestLibRBD, TestCompareAndWriteMismatchBufferlistGreaterLenPP)
{
...

    {
         ...
         */
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off, cmp_bl.length() - 1, mismatch_bl,    /* cmp_bl */
                                          write_bl, &mismatch_off, 0);
        ASSERT_EQ(-EILSEQ, written);
        ASSERT_EQ(5U, mismatch_off);
...
TEST_F(TestLibRBD, TestCompareAndWriteSuccessPP)
{
...

    {
        ...
        written = image.compare_and_write(off, cmp_bl.length(),
                                          cmp_bl, write_bl, &mismatch_off, 0);
        ASSERT_EQ(write_bl.length(), written);
        ASSERT_EQ(0U, mismatch_off);

        // check write_bl was written
        ASSERT_PASSED(compare_written, image, off, write_bl.length(),
                      write_buffer);
...
TEST_F(TestLibRBD, TestCompareAndWriteSuccessBufferlistGreaterLenPP)
{
...

    {
        ...
        ASSERT_PASSED(compare_written, image, off + len_m1, 1,
                      std::string("l"));
...
TEST_F(TestLibRBD, TestCompareAndWriteStripeUnitUnalignedPP)
{
...

    {
        ...
        written = image.compare_and_write(stripe_unit + 1, stripe_unit,
                                          large_cmp_bl,
                                          large_write_bl, &mismatch_off, 0);
        ASSERT_EQ(-EINVAL, written);
        ASSERT_EQ(0U, mismatch_off);

        // check nothing has been written
        ASSERT_PASSED(compare_written, image, stripe_unit,
                      large_cmp_bl.length(), large_cmp_buffer);
...
TEST_F(TestLibRBD, TestIOToSnapshot)
{
    ...
    ASSERT_PASSED(read_test_data, image_at_snap, orig_data, 0,
                  TEST_IO_TO_SNAP_SIZE, 0);
...
TEST_F(TestLibRBD, TestClone)
{
    ...
    BOOST_SCOPE_EXIT_ALL(&) {
        ASSERT_EQ(0,
                  rados_conf_set(_cluster, "rbd_default_clone_format", "auto"));
    };
    ...
    ASSERT_EQ(-EINVAL,
              clone_image(ioctx, parent, parent_name.c_str(), "parent_snap",
...
TEST_F(TestLibRBD, TestClone2)
{
    ...
    ASSERT_EQ((ssize_t) strlen(data), rbd_write(parent, 0, strlen(data), data));
    ASSERT_EQ((ssize_t) strlen(data),
              rbd_write(parent, 12, strlen(data), data));
    ...
    // write something in
    ASSERT_EQ((ssize_t) strlen(childata),
    ...
    for (i = num_expected; i > 0; i--) {
        ...
        for (j = 0; j < num_children; j++) {
            if (children[j].pool_name == NULL ||
                children[j].image_name == NULL || children[j].image_id == NULL)
                continue;
...
TEST_F(TestLibRBD, ListChildren)
{
    ...
    test_list_children2(parent, 1,
                        child_id1, pool_name2.c_str(), child_name1.c_str(),
                        false);
    ...
    test_list_children2(parent, 2,
                        child_id1, pool_name2.c_str(), child_name1.c_str(),
                        false, child_id2, pool_name1.c_str(),
                        child_name2.c_str(), false);
    ...
    test_list_children2(parent, 3,
                        child_id1, pool_name2.c_str(), child_name1.c_str(),
                        false, child_id2, pool_name1.c_str(),
                        child_name2.c_str(), false, child_id3,
    ...
    test_list_children2(parent, 4,
                        ...
                        child_name2.c_str(), false, child_id3,
                        pool_name2.c_str(), child_name3.c_str(), true,
                        child_id4, pool_name2.c_str(), child_name4.c_str(),
                        false);
    ...
    test_list_children(parent, 3,
                       ...
                       pool_name2.c_str(), child_name4.c_str());
    test_list_children2(parent, 3,
                        child_id2, pool_name1.c_str(), child_name2.c_str(),
                        false, child_id3, pool_name2.c_str(),
                        child_name3.c_str(), false, child_id4,
    ...
    test_list_children2(parent, 2,
                        child_id2, pool_name1.c_str(), child_name2.c_str(),
                        false, child_id4, pool_name2.c_str(),
                        child_name4.c_str(), false);
...
TEST_F(TestLibRBD, ListChildrenTiered)
{
    ...
    cmd[0] = (char *)cmdstr.c_str();
    ASSERT_EQ(0,
              rados_mon_command(_cluster, (const char **)cmd, 1, "", 0, NULL, 0,
                                NULL, 0));
    ...
    ASSERT_EQ(0,
              rados_mon_command(_cluster, (const char **)cmd, 1, "", 0, NULL, 0,
                                NULL, 0));
    cmdstr =
        "{\"prefix\": \"osd tier remove\", \"pool\": \"" + pool_name1 +
        "\", \"tierpool\":\"" + pool_name3 + "\"}";
    cmd[0] = (char *)cmdstr.c_str();
    ASSERT_EQ(0,
              rados_mon_command(_cluster, (const char **)cmd, 1, "", 0, NULL, 0,
...
int iterate_cb(uint64_t off, size_t len, int exists, void *arg)
{
    ...
    interval_set < uint64_t > *diff =
        static_cast < interval_set < uint64_t > *>(arg);
    diff->insert(off, len);
    return 0;
}
...
interval_set < uint64_t > round_diff_interval(const interval_set < uint64_t >
                                              &diff, uint64_t object_size)
{
    if (object_size == 0) {
        return diff;
    }
...
TYPED_TEST(DiffIterateTest, DiffIterate)
{
...

    {
        ...
        interval_set < uint64_t > diff;
        ASSERT_EQ(0,
                  image.diff_iterate2("one", 0, size, true, this->whole_object,
...
struct diff_extent {
    diff_extent(uint64_t _offset, uint64_t _length, bool _exists,
                uint64_t object_size):offset(_offset), length(_length),
        exists(_exists) {
        if (object_size != 0) {
...
TYPED_TEST(DiffIterateTest, DiffIterateStress)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        cout << " i=" << i << " exists " << curexists << " wrote " << w << std::
            endl;
...
TYPED_TEST(DiffIterateTest, DiffIterateParent)
{
...

    {
        ...
        ASSERT_EQ(0,
                  clone.diff_iterate2(NULL, 0, size, true, this->whole_object,
        ...
        ASSERT_EQ(diff_extent(12582912, 4194304, true, object_size),
                  extents[3]);
        ASSERT_EQ(diff_extent(16777216, 4194304, true, object_size),
                  extents[4]);
        extents.clear();
...
TYPED_TEST(DiffIterateTest, DiffIterateIgnoreParent)
{
    ...
    std::string clone_name = this->get_temp_image_name();
    ASSERT_EQ(0,
              rbd.clone(ioctx, name.c_str(), "one", ioctx, clone_name.c_str(),
                        features, &order));
...
TYPED_TEST(DiffIterateTest, DiffIterateStriping)
{
...

    {
        ...
        std::vector < diff_extent > extents;
        ASSERT_EQ(0,
                  image.diff_iterate2(NULL, 0, size, true, this->whole_object,
                                      vector_iterate_cb, &extents));
...
TEST_F(TestLibRBD, LargeCacheRead)
{
    ...
    BOOST_SCOPE_EXIT((orig_cache_size)) {
        ASSERT_EQ(0,
                  _rados.conf_set("rbd_cache_size", orig_cache_size.c_str()));
    }
    BOOST_SCOPE_EXIT_END;
                              ...
                              bool * passed)
{
    ...
    int order = 0;
    ASSERT_EQ(0,
              create_image_pp(rbd, ioctx, parent_name.c_str(), size, &order));
...
TEST_F(TestLibRBD, Metadata)
{
    ...
    ASSERT_EQ(-ERANGE,
              rbd_metadata_list(image1, "key", 0, keys, &keys_len, vals,
                                &vals_len));
    keys_len = sizeof(keys);
    vals_len = sizeof(vals);
    memset_rand(keys, keys_len);
    ...
    ASSERT_EQ(-EINVAL,
              rbd_metadata_set(image1, "conf_rbd_cache", "INVALID_VAL"));
    ...
    ASSERT_EQ(vals_len,
              strlen("value1") + 1 + strlen("value2") + 1 + strlen("value3") +
              1);
    ASSERT_STREQ(keys, "key1");
    ...
    ASSERT_EQ(vals_len,
              strlen("value1") + 1 + strlen("value2") + 1 + strlen("value3") +
              1 + strlen("value4") + 1);
    ASSERT_STREQ(keys + strlen("key1") + 1 + strlen("key2") + 1 +
                 strlen("key3") + 1, "key4");
    ...
    ASSERT_EQ(-ERANGE,
              rbd_metadata_list(image2, "key", 2, keys, &keys_len, vals,
                                &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1);

    ASSERT_EQ(-ERANGE,
              rbd_metadata_list(image2, "key", 0, keys, &keys_len, vals,
                                &vals_len));
    ASSERT_EQ(keys_len,
              strlen("key1") + 1 + strlen("key2") + 1 + strlen("key3") + 1 +
              strlen("key4") + 1);
...
TEST_F(TestLibRBD, UpdateFeatures)
{
    ...
    if (old_format) {
        ASSERT_EQ(-EINVAL,
                  image.update_features(RBD_FEATURE_EXCLUSIVE_LOCK, true));
        return;
    }
    ...
    ASSERT_EQ(0, image.update_features(RBD_FEATURE_OBJECT_MAP, true));
    ASSERT_EQ(-EINVAL,
              image.update_features(RBD_FEATURE_EXCLUSIVE_LOCK, false));
    ASSERT_EQ(0, image.update_features(RBD_FEATURE_OBJECT_MAP, false));
...
TEST_F(TestLibRBD, RebuildObjectMap)
{
    ...
    bl.append("foo");
    {
        ...
        ASSERT_EQ((ssize_t) bl.length(),
                  image.write(1 << order, bl.length(), bl));
    ...
    read_bl.clear();
    ASSERT_EQ((ssize_t) bl.length(),
              image2.read(1 << order, bl.length(), read_bl));
    ASSERT_TRUE(bl.contents_equal(read_bl));
...
TEST_F(TestLibRBD, CheckObjectMap)
{
    ...
    bl1.append("foo");
    {
        ...
        ASSERT_EQ(0, image.snap_create("snap1"));
        ASSERT_EQ((ssize_t) bl1.length(),
                  image.write(1 << order, bl1.length(), bl1));
    }
    ...
    bool lock_owner;
    ASSERT_EQ((ssize_t) bl1.length(),
              image1.write(3 * (1 << 18), bl1.length(), bl1));
    ASSERT_EQ(0, image1.is_exclusive_lock_owner(&lock_owner));
    ASSERT_TRUE(lock_owner);
...
TEST_F(TestLibRBD, ExclusiveLockTransition)
{
    ...
    for (size_t object_no = 0; object_no < (size >> 12); ++object_no) {
        ...
        if (object_no % 2 == 0) {
            ASSERT_EQ(0,
                      image1.aio_write(object_no << order, bl.length(), bl,
                                       comp));
        }
        else {
            ASSERT_EQ(0,
                      image2.aio_write(object_no << order, bl.length(), bl,
                                       comp));
        }
    }
...
TEST_F(TestLibRBD, ExclusiveLockReadTransition)
{
    ...
    for (size_t object_no = 0; object_no < (size >> 12); ++object_no) {
        ...
        if (object_no % 2 == 0) {
            ASSERT_EQ(0,
                      image1.aio_read(object_no << order, 1 << order,
                                      read_bls.back(), comp));
        }
        else {
            ASSERT_EQ(0,
                      image2.aio_read(object_no << order, 1 << order,
                                      read_bls.back(), comp));
        }
    }
...
TEST_F(TestLibRBD, TestImageOptions)
{
    ...
    ASSERT_EQ(0,
              rbd_copy_with_progress3(parent, ioctx, copy2_name.c_str(), opts,
                                      print_progress_percent, NULL));
...
TEST_F(TestLibRBD, TestImageOptionsPP)
{
    ...
    PrintProgress pp;
    ASSERT_EQ(0,
              parent.copy_with_progress3(ioctx, copy2_name.c_str(), opts, pp));
...
TEST_F(TestLibRBD, EventSocketEventfd)
{
    ...
    ASSERT_EQ(static_cast < ssize_t > (sizeof(uint64_t)),
              read(event_fd, buf, 32));
...
TEST_F(TestLibRBD, ImagePollIO)
{
    ...
    ASSERT_EQ(0,
              rbd_set_image_notification(image, fd, EVENT_SOCKET_TYPE_EVENTFD));
    ...
    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(aio_write_test_data_and_poll, image, fd, test_data,
                      TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(aio_read_test_data_and_poll, image, fd, test_data,
                      TEST_IO_SIZE * i, TEST_IO_SIZE, 0);
...
namespace librbd {
    ...
    static bool operator==(const image_spec_t & lhs, const image_spec_t & rhs) {
        ...
        return (lhs.pool_id == rhs.pool_id &&
                ...
                lhs.image_name == rhs.image_name && lhs.trash == rhs.trash);
    }
    ...
    static std::ostream & operator<<(std::ostream & os,
                                     const mirror_peer_t & peer) {
        os << "uuid=" << peer.uuid << ", " << "cluster=" << peer.
            cluster_name << ", " << "client=" << peer.client_name;
        return os;
    }
...
TEST_F(TestLibRBD, Mirror)
{
    ...
    std::string uuid1;
    ASSERT_EQ(-EINVAL,
              rbd.mirror_peer_add(ioctx, &uuid1, "cluster1", "client"));
    ...
    if ((features & RBD_FEATURE_LAYERING) != 0) {
        ...
        ASSERT_EQ(0, parent.close());
        ASSERT_EQ(0,
                  rbd.open(ioctx, parent, parent_name.c_str(), "parent_snap"));
        ASSERT_EQ(0, parent.snap_protect("parent_snap"));
        ASSERT_EQ(0, parent.close());
    ...
    ASSERT_EQ(-ENOENT,
              rbd.mirror_peer_set_client(ioctx, "uuid4", "new client"));
...
TEST_F(TestLibRBD, MirrorPeerAttributes)
{
    ...
    ASSERT_EQ(-ENOENT,
              rbd.mirror_peer_get_attributes(ioctx, uuid, &attributes));
    ASSERT_EQ(-ENOENT,
              rbd.mirror_peer_set_attributes(ioctx, "missing uuid",
                                             attributes));
...
TEST_F(TestLibRBD, CreateWithMirrorEnabled)
{
    ...
    ASSERT_EQ(0, image_options.set(RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE,
                                   static_cast < uint64_t >
                                   (RBD_MIRROR_IMAGE_MODE_SNAPSHOT)));

    std::string parent_name = get_temp_image_name();
    ASSERT_EQ(0,
              rbd.create4(ioctx, parent_name.c_str(), 2 << 20, image_options));
        ...
        for (int i = 0; i < 10; i++) {
            {
                ...
                if (owner_id == m_id) {
                    std::cout << m_id << ": releasing exclusive lock" << std::
                        endl;
                    EXPECT_EQ(0, rbd_lock_release(m_image));
                    int lock_owner;
                    EXPECT_EQ(0,
                              rbd_is_exclusive_lock_owner(m_image,
                                                          &lock_owner));
                    EXPECT_FALSE(lock_owner);
                    owner_id = -1;
                    std::cout << m_id << ": exclusive lock released" << std::
                        endl;
                    continue;
                }
            }
...
TEST_F(TestLibRBD, BreakLock)
{
    ...
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ASSERT_EQ(0,
              rbd_open(blocklist_ioctx, name.c_str(), &blocklist_image, NULL));

    ASSERT_EQ(0,
              rbd_metadata_set(image, "conf_rbd_blocklist_on_break_lock",
                               "true"));
    ...
    ASSERT_EQ(0,
              rbd_lock_break(image, RBD_LOCK_MODE_EXCLUSIVE, lock_owners[0]));
...
TEST_F(TestLibRBD, DefaultFeatures)
{
    ...
    ASSERT_EQ(0,
              _rados.conf_get("rbd_default_features", orig_default_features));
    ...
    std::list < std::pair < std::string,
        std::string > >feature_names_to_bitmask = {
        {
  ...
  for (auto & pair:feature_names_to_bitmask) {
        ASSERT_EQ(0,
                  _rados.conf_set("rbd_default_features", pair.first.c_str()));
        std::string features;
...
TEST_F(TestLibRBD, NamespacesPP)
{
    ...
    ASSERT_EQ(-ENOENT,
              create_image_pp(rbd, ns_io_ctx, name.c_str(), 0, &order));
...
TEST_F(TestLibRBD, Migration)
{
    ...
    if (old_format) {
        ...
        ASSERT_EQ(-EROFS,
                  rbd_trash_remove(ioctx, status.source_image_id, false));
        ASSERT_EQ(-EINVAL,
...
TEST_F(TestLibRBD, MigrationPP)
{
    ...
    if (old_format) {
        ...
        ASSERT_EQ(-EROFS,
                  rbd.trash_remove(ioctx, status.source_image_id.c_str(),
                                   false));
        ASSERT_EQ(-EINVAL,
                  rbd.trash_restore(ioctx, status.source_image_id.c_str(),
                                    name.c_str()));
    }
...
TEST_F(TestLibRBD, ZeroOverlapFlatten)
{
    ...
    std::string clone_name = this->get_temp_image_name();
    ASSERT_EQ(0,
              rbd.clone(ioctx, name.c_str(), "snap", ioctx, clone_name.c_str(),
                        features, &order));
...
TEST_F(TestLibRBD, PoolMetadata)
{
    ...
    ASSERT_EQ(-ERANGE,
              rbd_pool_metadata_list(ioctx, "", 0, keys, &keys_len, vals,
                                     &vals_len));
    keys_len = sizeof(keys);
    vals_len = sizeof(vals);
    memset_rand(keys, keys_len);
    ...
    ASSERT_EQ(-EINVAL,
              rbd_pool_metadata_set(ioctx, "conf_rbd_cache", "INVALID"));
    ...
    ASSERT_EQ(-ERANGE,
              rbd_pool_metadata_list(ioctx, "", 2, keys, &keys_len, vals,
                                     &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1);
...
TEST_F(TestLibRBD, PoolMetadataPP)
{
    ...
    ASSERT_EQ(-EINVAL,
              rbd.pool_metadata_set(ioctx, "conf_rbd_cache", "INVALID"));
...
TEST_F(TestLibRBD, PoolStatsPP)
{
    ...
    for (size_t idx = 0; idx < 4; ++idx) {
        ...
        int order = 0;
        ASSERT_EQ(0,
                  create_image_pp(rbd, ioctx, image_name.c_str(), size,
                                  &order));
        expected_size += size;
    }
...
TEST_F(TestLibRBD, ImageSpec)
{
    ...
    std::sort(expected_images.begin(), expected_images.end(),
              [](const librbd::image_spec_t & lhs,
                 const librbd::image_spec_t & rhs) {
    ...
    .pool_id = ioctx.get_id(),.pool_name =
            ioctx.get_pool_name(),.pool_namespace =
            ioctx.get_namespace(),.image_id = parent_id,.image_name =
            name,.trash = false};
    ...
    std::vector < librbd::linked_image_spec_t > expected_children {
        ...
        .pool_id = ioctx.get_id(),.pool_name =
                ioctx.get_pool_name(),.pool_namespace =
                ioctx.get_namespace(),.image_id = clone_id,.image_name =
                clone_name,.trash = false}
    };
    ...
    expected_children.push_back( {
                                .pool_id = ioctx.get_id(),.pool_name =
                                ioctx.get_pool_name(),.pool_namespace =
                                ioctx.get_namespace(),.image_id =
                                grand_clone_id,.image_name =
                                grand_clone_name,.trash = false}
    );
    ASSERT_EQ(expected_children, children);
}
...
TEST_F(TestLibRBD, DISABLED_TestSeqWriteAIOPP)
{
...

    {
      ...
      for (auto comp:comps) {
        ...
        for (uint64_t i = 0; i < size / TEST_IO_SIZE; ++i) {
            char *p = test_data + (TEST_IO_SIZE + 1) * (i % 10);
            ASSERT_PASSED(read_test_data, image, p, strlen(p) * i, TEST_IO_SIZE,
                          0);
        }
...
TEST_F(TestLibRBD, SnapRemoveWithChildMissing)
{
    ...
    test_list_children2(parent, 3,
                        child_id1, m_pool_name.c_str(), child_name1.c_str(),
                        false, child_id2, pool_name1.c_str(),
                        child_name2.c_str(), false, child_id3,
...
TEST_F(TestLibRBD, QuiesceWatchTimeout)
{
...

    {
        ...
        thread quiesce1([&image, &watcher, handle] () {
                        ...
                        sleep(8); image.quiesce_complete(handle, 0);}
        );
        ...
        thread quiesce2([&image, &watcher, handle, &timed_out] () {
                        ...
                        std::cerr << "waiting for timed out ... " << i << std::
                        endl; sleep(1);}
                        image.quiesce_complete(handle, 0);}) ;
...
TEST_F(TestLibRBD, WriteZeroes)
{
    ...
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false,
                                     iterate_cb, (void *)&diff));
    expected_diff = interval_set < uint64_t > { { {
    0, 192}}};
    ASSERT_EQ(expected_diff, diff);
...
TEST_F(TestLibRBD, WriteZeroesThickProvision)
{
    ...
    ASSERT_EQ(128,
              image.write_zeroes(0, 128, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION,
                                 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false,
                                     iterate_cb, (void *)&diff));
