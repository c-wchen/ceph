...
void chunk_refs_t::encode(ceph::buffer::list & bl) const const
{
    bufferlist t;
    _encode_r(t);
    _encode_final(bl, t);
...
void chunk_refs_t::_encode_r(ceph::bufferlist & bl) const const
{
    using ceph::encode;
    switch (r->get_type()) {
    case TYPE_BY_OBJECT:
...
void chunk_refs_t::dynamic_encode(ceph::buffer::list & bl, size_t max)
{
    ...
    while (true) {
        ...
        case TYPE_BY_OBJECT:
            r.reset(new
                    chunk_refs_by_hash_t(static_cast <
                                         chunk_refs_by_object_t * >(r.get())));
            break;
        case TYPE_BY_HASH:
            if (!static_cast < chunk_refs_by_hash_t * >(r.get())->shrink()) {
                r.reset(new
                        chunk_refs_by_pool_t(static_cast <
...
void chunk_refs_t::_encode_final(bufferlist & bl, bufferlist & t) const const
{
    ENCODE_START(1, 1, bl);
...
void chunk_refs_t::decode(ceph::buffer::list::const_iterator & p)
{
    ...
    case TYPE_COUNT:
        ...
        break;
    default:
        throw ceph::buffer::malformed_input(std::
                                            string
                                            ("unrecognized chunk ref encoding type ")
                                            + stringify((int)t));
    }
    DECODE_FINISH(p);
}
