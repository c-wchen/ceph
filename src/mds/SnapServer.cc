    ...
    case TABLE_OP_DESTROY:
        {
            ...
            pending_destroy[version] =
                pair < snapid_t, snapid_t > (snapid, last_snap);
...
bool SnapServer::_is_prepared(version_t tid) const const
{
    return pending_update.count(tid) || pending_destroy.count(tid);
}
...
void SnapServer::generate_test_instances(list < SnapServer * >&ls)
{
    ...
    for (list < SnapInfo * >::iterator i = snapinfo_instances.begin();
         i != snapinfo_instances.end(); ++i) {
        delete *i;
    }
