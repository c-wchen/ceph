  ...
  public:
    ...
    struct BufferSpace {
        ...
        typedef boost::intrusive::list <
         ...
         mempool::bluestore_cache_other::map < uint32_t,
        ...
        void write(Cache * cache, uint64_t seq, uint32_t offset,
    ...
    struct SharedBlobSet {
        ...
        // count
        mempool::bluestore_cache_other::unordered_map < uint64_t,
            SharedBlob * >sb_map;
...
//#define CACHE_BLOB_BL  // not sure if this is a win yet or not... :/
    ...
    struct Blob {
      ...
      public:
        ...
        }} void decode(Collection * /*coll */ ,
                       bufferptr::iterator & p, bool include_ref_map) {
    ...
    struct Onode {
        ...
        :nref(0), c(c), oid(o), key(k), exists(false), extent_map(this) {
        }
    ...
    struct Cache {
        ...
        static Cache *create(CephContext * cct, string type,
                             PerfCounters * logger);
    ...
    struct OnodeSpace {
      private:
        ...
        /// forward lookups
        mempool::bluestore_cache_other::unordered_map < ghobject_t,
            OnodeRef > onode_map;
    ...
    struct TransContext:public AioContext {
...
#if defined(WITH_LTTNG) && defined(WITH_EVENTTRACE)
            if (state >= l_bluestore_state_prepare_lat
        ...
        :osr(o), ioc(cct, this), start(ceph_clock_now()) {
            last_stamp = start;
        }
        ~TransContext() {
            delete deferred_txn;
    ...
    struct DBHistogram {
        ...
        void update_hist_entry(map < string, map < int,
  ...
  private:
    ...
    struct MempoolThread:public Thread {
                ...
                case PriorityCache::Priority::LAST:
                    {
                        ...
                        int64_t request =
                            PriorityCache::get_chunk(usage, chunk_bytes);
      ...
      :    
        ...
        meta_cache(MetaCache(s)), data_cache(DataCache(s)) {
        }
      ...
      private:
        ...
        void _balance_cache(const std::list <
                            PriorityCache::PriCache * >&caches);
  ...
  private:
    int _fsck_check_extents(const ghobject_t & oid,
    ...
    void _buffer_cache_write(TransContext * txc,
                             BlobRef b,
                             uint64_t offset, bufferlist & bl, unsigned flags) {
  ...
  public:
    ...
    int set_collection_opts(const coll_t & cid,
                            const pool_opts_t & opts) override;
    int stat(const coll_t & cid,
             const ghobject_t & oid,
             struct stat *st, bool allow_eio = false) override;
    int stat(CollectionHandle & c,
             const ghobject_t & oid,
             struct stat *st, bool allow_eio = false) override;
    int read(const coll_t & cid,
             const ghobject_t & oid,
             uint64_t offset,
  ...
  public:
    ...
    int fiemap(CollectionHandle & c, const ghobject_t & oid,
               uint64_t offset, size_t len, bufferlist & bl) override;
    int fiemap(const coll_t & cid, const ghobject_t & oid,
               uint64_t offset, size_t len, map < uint64_t,
    ...
    int omap_get(const coll_t & cid,    ///< [in] Collection containing oid
    ...
    /// Get omap header
    int omap_get_header(const coll_t & cid, ///< [in] Collection containing oid
                        const ghobject_t & oid, ///< [in] Object containing omap
                        bufferlist * header,    ///< [out] omap header
    ...
    int omap_get_keys(const coll_t & cid,   ///< [in] Collection containing oid
                      const ghobject_t & oid,   ///< [in] Object containing omap
    ...
    /// Get key values
    int omap_get_values(const coll_t & cid, ///< [in] Collection containing oid
                        const ghobject_t & oid, ///< [in] Object containing omap
    ...
    int omap_check_keys(const coll_t & cid, ///< [in] Collection containing oid
                        const ghobject_t & oid, ///< [in] Object containing omap
    ...
    ObjectMap::ObjectMapIterator get_omap_iterator(const coll_t & cid,  ///< [in] collection
    ...
    const PerfCounters *get_perf_counters() const override {
        return logger;
    } int queue_transactions(Sequencer * osr,
  ...
  private:
    ...
    int _verify_csum(OnodeRef & o,
                     ...
                     const bufferlist & bl, uint64_t logical_offset) const;
    ...
    int _do_transaction(Transaction * t,
                        TransContext * txc, ThreadPool::TPHandle * handle);
    ...
    void _do_write_data(TransContext * txc,
                        ...
                        OnodeRef o,
                        uint64_t offset,
                        uint64_t length, bufferlist & bl, WriteContext * wctx);
    ...
    int _omap_setheader(TransContext * txc,
                        CollectionRef & c, OnodeRef & o, bufferlist & header);
