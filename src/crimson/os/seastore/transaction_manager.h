...
public:
    ...
    template < typename T >
    std::variant < LBAMappingRef, base_iertr::future < TCachedExtentRef<T>>> get_extent_if_linked(
        Transaction &t,
        LBAMappingRef pin)
    {
     ...
     */
    template < class T >
    alloc_extents_iertr::future < std::vector < TCachedExtentRef<T>>> alloc_extents(
        Transaction &t,
        laddr_t hint,
        extent_len_t len,
        int num)
