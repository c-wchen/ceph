...
void DPDKWorker::initialize()
{
    ...
    if (i == 0) {
        ...
        std::unique_ptr < DPDKDevice > dev =
            create_dpdk_net_device(cct, cores, cct->_conf->ms_dpdk_port_id,
    ...
    if (i < sdev->hw_queues_count()) {
        auto qp =
            sdev->init_local_queue(cct, &center, cct->_conf->ms_dpdk_hugepages,
                                   i);
    ...
    sdev->workers[i] = this;
    _impl =
        std::unique_ptr < DPDKWorker::Impl >
...
static bool parse_available_address(const string & ips, const string & gates,
                                    const string & masks,
...
:  id(i), _netif(cct, dev, c), _dev(dev), _inet(cct, c, &_netif)
{
    ...
    bool parsed =
        parse_available_address(cct->_conf->get_val < std::string >
...
void DPDKStack::spawn_worker(unsigned i, std::function < void () > &&func)
{
    ...
    dpdk::eal::execute_on_master([&]() {
                                 ...
                                 rte_eal_remote_launch(dpdk_thread_adaptor,
                                                       ...
                                                       i + 1); if (r < 0) {
                                 lderr(cct) << __func__ <<
                                 " remote launch failed, r=" << r << dendl;
                                 ceph_abort();}
                                 }
    ) ;
}
