            ...
            namespace ssd {
                ...
                static bool is_valid_pool_root(const WriteLogPoolRoot & root) {
                    return root.pool_size % MIN_WRITE_ALLOC_SSD_SIZE == 0 &&
                        ...
                        && root.first_free_entry % MIN_WRITE_ALLOC_SSD_SIZE ==
                        0;
                } template < typename I > Builder < AbstractWriteLog <
                    I >> *WriteLog < I >::create_builder() {
                    m_builderobj = new Builder < This > ();
                    return m_builderobj;
                } template < typename I >
                    WriteLog < I >::WriteLog(I & image_ctx,
                                             librbd::cache::pwl::
                                             ImageCacheState < I > *cache_state,
                                             cache::
                ...
                :AbstractWriteLog < I > (image_ctx, cache_state,
                    ...
                    void WriteLog <
                    I >::collect_read_extents(uint64_t read_buffer_offset,
                                              LogMapEntry <
                                              GenericWriteLogEntry > map_entry,
                    ...
                    auto write_entry =
                        std::static_pointer_cast < WriteLogEntry >
                        (map_entry.log_entry);
                    buffer::list hit_bl;
                    write_entry->copy_cache_bl(&hit_bl);
                    bool writesame = write_entry->is_writesame_entry();
                    auto hit_extent_buf =
                        std::make_shared < ImageExtentBuf > (hit_extent, hit_bl,
                                                             true,
                                                             read_buffer_offset,
                                                             writesame);
                ...
                template < typename I >
                    void WriteLog < I >::complete_read(std::vector <
                                                       std::shared_ptr <
                                                       GenericWriteLogEntry >>
                                                       &log_entries_to_read,
                                                       std::vector <
                    ...
                    if (!log_entries_to_read.empty()) {
                        aio_read_data_blocks(log_entries_to_read, bls_to_read,
                                             ctx);
                    }
                    else {
                        ctx->complete(0);
                ...
                template < typename I >
                    ...
                    bdev =
                        BlockDevice::create(cct, this->m_log_pool_name,
                                            aio_cache_cb, nullptr, nullptr,
                                            nullptr);
                    ...
                    ceph_assert(this->m_log_pool_size %
                                MIN_WRITE_ALLOC_SSD_SIZE == 0);
                    if (bdev->get_size() != this->m_log_pool_size) {
                        lderr(cct) << "size mismatch: bdev size " << bdev->
                            get_size()
                            << " (block size " << bdev->get_block_size()
                            << ") != pool size " << this->
                            m_log_pool_size << dendl;
                        bdev->close();
                        delete bdev;
                        return -EINVAL;
                    }
                ...
                template < typename I >
                    ...
                    if (access(this->m_log_pool_name.c_str(), F_OK) != 0) {
                        int fd =::open(this->m_log_pool_name.c_str(),
                                       O_RDWR | O_CREAT, 0644);
                        bool succeed = true;
                        if (fd >= 0) {
                        ...
                        auto new_root =
                            std::make_shared < WriteLogPoolRoot > (pool_root);
                        ...
                        r = bdev->read(0, MIN_WRITE_ALLOC_SSD_SIZE, &bl, &ioctx,
                                       false);
                        if (r < 0) {
                            lderr(cct) << "read ssd cache superblock failed " <<
                                dendl;
                            goto err_close_bdev;
                        }
                        auto p = bl.cbegin();
                        ...
                        ldout(cct,
                            ...
                            pool_size << " first_valid_entry=" << pool_root.
                            first_valid_entry << " first_free_entry=" <<
                            pool_root.
                            first_free_entry << " flushed_sync_gen=" <<
                            pool_root.flushed_sync_gen << dendl;
                        ...
                        if (pool_root.block_size != MIN_WRITE_ALLOC_SSD_SIZE) {
                            lderr(cct) << "pool block size is " << pool_root.
                                block_size << " expected " <<
                                MIN_WRITE_ALLOC_SSD_SIZE << dendl;
                            goto err_close_bdev;
                        }
                        ...
                        m_cache_state->clean =
                            this->m_dirty_log_entries.empty();
                ...
                template < typename I > void WriteLog < I >::remove_pool_file() {
                    ...
                    if (m_cache_state->clean) {
                        ...
                        if (remove(this->m_log_pool_name.c_str()) != 0) {
                            lderr(m_image_ctx.
                                  cct) << "failed to remove empty pool \"" <<
                                this->m_log_pool_name << "\": " << dendl;
                        }
                        else {
                            m_cache_state->present = false;
                ...
                template < typename I >
                    void WriteLog <
                    I >::load_existing_entries(pwl::DeferredContexts & later) {
                    CephContext *cct = m_image_ctx.cct;
                    std::map < uint64_t,
                        std::shared_ptr <
                        SyncPointLogEntry >> sync_point_entries;
                    ...
                    for (uint64_t next_log_pos = this->m_first_valid_entry;
                        ...
                        bdev->read(next_log_pos, MIN_WRITE_ALLOC_SSD_SIZE,
                                   &bl_entries, &ioctx_entry, false);
                        ...
                        for (auto it = ssd_log_entries.begin();
                             it != ssd_log_entries.end(); ++it) {
                            this->update_entries(&log_entry, &*it,
                                                 missing_sync_points,
                                                 sync_point_entries,
                                                 curr_log_pos);
                        ...
                        if (next_log_pos >= this->m_log_pool_size) {
                            next_log_pos =
                                next_log_pos % this->m_log_pool_size +
                                DATA_RING_BUFFER_OFFSET;
                        }
                    }
                    this->update_sync_points(missing_sync_points,
                                             sync_point_entries, later);
                    if (m_first_valid_entry > m_first_free_entry) {
                        m_bytes_allocated =
                            this->m_log_pool_size - m_first_valid_entry +
                            m_first_free_entry - DATA_RING_BUFFER_OFFSET;
                    }
                    else {
                        m_bytes_allocated =
                            m_first_free_entry - m_first_valid_entry;
                    }
                }
...
// For SSD we don't calc m_bytes_allocated in this
                template < typename I >
                    void WriteLog <
                    I >::inc_allocated_cached_bytes(std::shared_ptr <
                                                    pwl::GenericLogEntry >
                                                    log_entry) {
                ...
                template < typename I >
                    bool WriteLog <
                    I >::alloc_resources(C_BlockIORequestT * req) {
                    ...
                    if (num_log_entries) {
                        bytes_allocated +=
                            num_log_entries * MIN_WRITE_ALLOC_SSD_SIZE;
                        num_log_entries = 0;
                    }
                ...
                template < typename I >
                    bool WriteLog <
                    I >::has_sync_point_logs(GenericLogOperations & ops) {
                  for (auto & op:ops) {
 ...
 */
                template < typename I >
                    void WriteLog <
                    I >::schedule_append_ops(GenericLogOperations & ops,
                              ...
                              std::back_inserter(appending));
                    {
                        ...
                        this->m_ops_to_append.splice(this->m_ops_to_append.
                                                     end(), ops);
 ...
 */
                template < typename I >
                    void WriteLog <
                    I >::append_op_log_entries(GenericLogOperations & ops) {
                    ...
                    Context *ctx = new LambdaContext([this, ops] (int r){
                                                     ...
                                                     ldout(m_image_ctx.cct,
                                                           20) <<
                                                     "Finished root update " <<
                                                     dendl;
                                                     auto captured_ops =
                                                     std::move(ops);
                                                     this->
                                                     complete_op_log_entries
                                                     (std::move(captured_ops),
                                                      r);
                                                     bool need_finisher = false;
                                                     {
                                                     std::
                                                     lock_guard locker1(m_lock);
                                                     ...
                                                     this->m_async_op_tracker.
                                                     finish_op();}
                    ) ;
                    uint64_t *new_first_free_entry = new(uint64_t);
                    Context *append_ctx = new LambdaContext([this,
                                                             new_first_free_entry,
                                                             ops, ctx] (int r){
                                                            std::shared_ptr <
                                                            WriteLogPoolRoot >
                                                            new_root; {
                          ldout(m_image_ctx.cct, 20) << "Finished appending at " << *new_first_free_entry << dendl; utime_t now = ceph_clock_now(); for (auto & operation:ops)
                                                            {
                                                            operation->
                ...
                template < typename I >
                    void WriteLog < I >::release_ram(std::shared_ptr <
                                                     GenericLogEntry >
                                                     log_entry) {
                ...
                template < typename I >
                    void WriteLog <
                    I >::alloc_op_log_entries(GenericLogOperations & ops) {
                      ...
                      for (auto & log_entry:entries_to_flush)
                        {
                            GuardedRequestFunctionContext *guarded_ctx =
                                new
                                GuardedRequestFunctionContext([this, log_entry,
                                                               invalidating]
                                                              ...
                                                              ctx =
                                                              new
                                                              LambdaContext([this, log_entry, ctx] (int r) {
                                                                            m_image_ctx.
                                                                            op_work_queue->
                                                                            queue
                                                                            (new
                                                              ...
                                                              ctx->
                                                              complete(0);}) ;
                            this->detain_flush_guard_request(log_entry,
                                                             guarded_ctx);
                        }
                    }
                    else {
                        ...
                        std::vector < std::shared_ptr <
                            GenericWriteLogEntry >> write_entries;
                      ...
                      for (auto & log_entry:entries_to_flush)
                        {
                            if (log_entry->is_write_entry()) {
                                ...
                                auto write_entry =
                                    static_pointer_cast < WriteLogEntry >
                                    (log_entry);
                                write_entry->inc_bl_refs();
                        ...
                        Context *ctx = new LambdaContext([this,
                                                          entries_to_flush,
                                                          read_bls] (int r){
                          int i = 0; GuardedRequestFunctionContext * guarded_ctx = nullptr; for (auto & log_entry:entries_to_flush)
                                                         {
                                                         if (log_entry->
                                                             is_write_entry()) {
                                                         bufferlist
                                                         ...
                                                         ([this, log_entry,
                                                          ...
                                                          op_work_queue->
                                                          queue(new
                                                                LambdaContext([this, log_entry, entry_bl = std::move(captured_entry_bl), ctx] (int r) {
                                                                              auto
                                                                              captured_entry_bl
                                                                              =
                                                                              std::
                    ...
                    do {
                        ...
                        if (this->m_alloc_failed_since_retire
                            ...
                            ldout(m_image_ctx.cct,
                                ...
                                (m_bytes_allocated > high_water_bytes)
                                << dendl;
                            retire_entries((this->m_shutting_down
                                            || this->m_invalidating
                                            || m_bytes_allocated >
...
*/
                template < typename I >
                    bool WriteLog <
                    I >::retire_entries(const unsigned long int frees_per_tx) {
                    CephContext *cct = m_image_ctx.cct;
                    ...
                    ldout(cct, 20) << "Look for entries to retire" << dendl;
                    {
                        ...
                        RWLock::WLocker entry_reader_locker(this->
                                                            m_entry_reader_lock);
                        std::lock_guard locker(m_lock);
                        initial_first_valid_entry = m_first_valid_entry;
                        first_valid_entry = m_first_valid_entry;
                        while (retiring_entries.size() < frees_per_tx
                               && !m_log_entries.empty()) {
                            GenericLogEntriesVector retiring_subentries;
                            uint64_t control_block_pos =
                                m_log_entries.front()->log_entry_index;
                            uint64_t data_length = 0;
                            for (auto it = m_log_entries.begin();
                                 it != m_log_entries.end(); ++it) {
                                if (this->can_retire_entry(*it)) {
                                    // log_entry_index is valid after appending to SSD
                                    if ((*it)->log_entry_index !=
                                        control_block_pos) {
                                        ldout(cct,
                                              20) << "Old log_entry_index is "
                                            << control_block_pos <<
                            ...
                            if (!retiring_subentries.empty()) {
                                for (auto it = retiring_subentries.begin();
                                    ...
                                    if ((*it)->write_bytes() > 0
                                        ...
                                        auto gen_write_entry =
                                            static_pointer_cast <
                                            GenericWriteLogEntry > (*it);
                                        if (gen_write_entry) {
                                            this->m_blocks_to_log_entries.
                                                remove_log_entry
                                                (gen_write_entry);
                                        }
                                    }
                                ...
                                ldout(cct,
                                      20) << "span with " <<
                                    retiring_subentries.size()
                                    << " entries: control_block_pos=" <<
                                    control_block_pos << " data_length=" <<
                                    data_length << dendl;
                    ...
                    if (retiring_entries.size()) {
                        ...
                        Context *ctx = new LambdaContext([this,
                                                          first_valid_entry,
                                                          initial_first_valid_entry,
                                                          retiring_entries] (int
                                                                             r){
                          uint64_t allocated_bytes = 0; uint64_t cached_bytes = 0; uint64_t former_log_pos = 0; for (auto & entry:retiring_entries)
                                                         {
                                                         ceph_assert(entry->
                                                                     log_entry_index
                                                                     != 0);
                ...
                template < typename I >
                    void WriteLog < I >::append_ops(GenericLogOperations & ops,
                                                    Context * ctx,
                                                    uint64_t *
                                                    new_first_free_entry) {
                ...
                template < typename I >
                    void WriteLog <
                    I >::write_log_entries(GenericLogEntriesVector log_entries,
                                           AioTransContext * aio,
                                           uint64_t * pos) {
                    CephContext *cct = m_image_ctx.cct;
                  ...
                  for (auto & log_entry:log_entries) {
                        ...
                        if (log_entry->is_write_entry()) {
                            ...
                            auto align_size =
                                write_entry->get_aligned_data_size();
                    ...
                    if (control_block_pos + bl.length() > this->m_log_pool_size) {
                        ...
                        bufferlist bl1;
                        bl.splice(0, this->m_log_pool_size - control_block_pos,
                                  &bl1);
                        ...
                        ldout(cct, 20) << "write " << control_block_pos << "~"
                            ...
                            << bl.length() << dendl;
                        bdev->aio_write(control_block_pos, bl1, &aio->ioc,
                                        false, WRITE_LIFE_NOT_SET);
                        bdev->aio_write(DATA_RING_BUFFER_OFFSET, bl, &aio->ioc,
                                        false, WRITE_LIFE_NOT_SET);
                    }
                    else {
                        ldout(cct, 20) << "write " << control_block_pos << "~"
                ...
                template < typename I >
                    void WriteLog < I >::schedule_update_root(std::shared_ptr <
                                                              WriteLogPoolRoot >
                                                              root,
                                                              Context * ctx) {
                    CephContext *cct = m_image_ctx.cct;
                    ...
                    bool need_finisher;
                    {
                        ...
                        need_finisher = m_poolroot_to_update.empty()
                            && !m_updating_pool_root;
                        std::shared_ptr < WriteLogPoolRootUpdate > entry =
                            std::make_shared < WriteLogPoolRootUpdate > (root,
                                                                         ctx);
                ...
                template < typename I >
                    ...
                    ldout(m_image_ctx.cct,
                          15) << "Update root number: " << root_updates.size()
                        << dendl;
                    ...
                    ctx =
                        new
                        LambdaContext([this, updates =
                                       std::move(root_updates)] (int r) {
                                      ldout(m_image_ctx.cct,
                                            15) << "Start to callback." <<
                                      dendl; for (auto it = updates.begin();
                                                  it != updates.end(); it++) {
                    ...
                    Context *append_ctx = new LambdaContext([this, ctx] (int r){
                                                            ldout(m_image_ctx.
                                                                  cct,
                                                                  15) <<
                                                            "Finish the update of pool root."
                                                            << dendl;
                                                            bool need_finisher =
                                                            false;
                                                            assert(r == 0); {
                    ...
                    AioTransContext *aio =
                        new AioTransContext(m_image_ctx.cct, append_ctx);
                    update_pool_root(root, aio);
                }

                template < typename I >
                    void WriteLog < I >::update_pool_root(std::shared_ptr <
                                                          WriteLogPoolRoot >
                                                          root,
                                                          AioTransContext *
                                                          aio) {
                    ...
                    bl.append_zero(MIN_WRITE_ALLOC_SSD_SIZE - bl.length());
                    ceph_assert(bl.length() % MIN_WRITE_ALLOC_SSD_SIZE == 0);
                    bdev->aio_write(0, bl, &aio->ioc, false,
                                    WRITE_LIFE_NOT_SET);
                    bdev->aio_submit(&aio->ioc);
                }

                template < typename I >
                    int WriteLog < I >::update_pool_root_sync(std::shared_ptr <
                                                              WriteLogPoolRoot >
                                                              root) {
                    bufferlist bl;
                    SuperBlock superblock;
                ...
                template < typename I >
                    void WriteLog < I >::aio_read_data_block(std::shared_ptr <
                                                             GenericWriteLogEntry
                                                             > log_entry,
                                                             bufferlist * bl,
                                                             Context * ctx) {
                    std::vector < std::shared_ptr <
                        GenericWriteLogEntry >> log_entries = {
                    std::move(log_entry)};
                ...
                template < typename I >
                    void WriteLog < I >::aio_read_data_blocks(std::vector <
                                                              std::shared_ptr <
                                                              GenericWriteLogEntry
                                                              >> &log_entries,
                                                              std::vector <
                    ...
                    //get the valid part
                    Context *read_ctx = new LambdaContext([log_entries, bls,
                                                           ctx] (int r){
                                                          for (unsigned int i =
                                                               0;
                                                               i <
                    ...
                    for (unsigned int i = 0; i < log_entries.size(); i++) {
                        WriteLogCacheEntry *log_entry =
                            &log_entries[i]->ram_entry;

                        ceph_assert(log_entry->is_write()
                                    || log_entry->is_writesame());
                        uint64_t len =
                            log_entry->is_write()? log_entry->
                            write_bytes : log_entry->ws_datalen;
                        uint64_t align_len =
                            round_up_to(len, MIN_WRITE_ALLOC_SSD_SIZE);

                        ldout(cct,
                              20) << "entry i=" << i << " " << log_entry->
                            write_data_pos << "~" << len << dendl;
                        ceph_assert(log_entry->write_data_pos >=
                                    DATA_RING_BUFFER_OFFSET
                                    && log_entry->write_data_pos <
                                    pool_root.pool_size);
                        ceph_assert(align_len);
                        if (log_entry->write_data_pos + align_len >
                            pool_root.pool_size) {
                            // spans boundary, need to split
                            uint64_t len1 =
                                pool_root.pool_size - log_entry->write_data_pos;
                            uint64_t len2 = align_len - len1;

                            ldout(cct,
                                  20) << "read " << log_entry->
                                write_data_pos << "~" << align_len <<
                                " spans boundary, split into " << log_entry->
                                write_data_pos << "~" << len1 << " and " <<
                                DATA_RING_BUFFER_OFFSET << "~" << len2 << dendl;
                            bdev->aio_read(log_entry->write_data_pos, len1,
                                           bls[i], &aio->ioc);
                            bdev->aio_read(DATA_RING_BUFFER_OFFSET, len2,
                                           bls[i], &aio->ioc);
                        }
                        else {
                            ldout(cct,
                                  20) << "read " << log_entry->
                                write_data_pos << "~" << align_len << dendl;
                            bdev->aio_read(log_entry->write_data_pos, align_len,
                                           bls[i], &aio->ioc);
                        }
                    }
                    bdev->aio_submit(&aio->ioc);
                ...
                template < typename I >
                    void WriteLog <
                    I >::complete_user_request(Context * &user_req, int r) {
