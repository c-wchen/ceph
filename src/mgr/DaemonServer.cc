...
void DaemonServer::adjust_pgs()
{
    ...
    with_osdmap_and_pgmap([&](const OSDMap & osdmap, const PGMap & pg_map) {
        ...
        dout(20) << "misplaced_ratio " << misplaced_ratio << " degraded_ratio " << degraded_ratio << " inactive_pgs_ratio " <<
                 inactive_pgs_ratio << " unknown_pgs_ratio " << unknown_pgs_ratio << "; target_max_misplaced_ratio " << max_misplaced <<
                 dendl;
        for (auto &i : osdmap.get_pools()) {
            ...
            if (p.get_pg_num_target() != p.get_pg_num()) {
                ...
                if (p.has_flag(pg_pool_t::FLAG_CREATING)) {
                    ...
                    unsigned max_jump =
                        cct->_conf->mgr_max_pg_num_change;
                    if (!active) {
                        dout(10) << "pool " << i.
                                 first << " pg_num_target " << p.
                                 get_pg_num_target()
            ...
            if (target != p.get_pgp_num()) {
                ...
                if (target > p.get_pgp_num() &&
                    ...
                    pool_stat_t s =
                        pg_map.get_pg_pool_sum_stat(i.first);
                    if (aggro ||
...
void DaemonServer::got_service_map()
{
    ...
    cluster_state.with_servicemap([&](const ServiceMap & service_map) {
        if (pending_service_map.epoch == 0) {
            ...
            pending_service_map.epoch =
                service_map.epoch + 1;
        } else if (pending_service_map.epoch <=
                   service_map.epoch) {
