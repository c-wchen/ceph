...
QuiesceTimeInterval QuiesceDbManager::replica_upkeep(decltype(pending_db_updates)&& db_updates)
{
    ...
    while (db_updates.size() > 1) {
        dout(10) << "skipping an older update from " << db_updates.front().origin << " version " <<
                 db_updates.front().db.db_version << dendl;
        db_updates.pop();
    }
    ...
    if (db.set_version > update.db_version.set_version) {
        dout(3) << "got an older version of DB from the leader: " << update.db_version.set_version << " < " << db.set_version <<
                dendl;
...
QuiesceTimeInterval QuiesceDbManager::leader_upkeep(decltype(pending_acks)&& acks,
...
void QuiesceDbManager::leader_record_ack(QuiesceInterface::PeerId from, QuiesceMap&& diff_map)
{
    ...
    if (diff_map.db_version > db.version()) {
        dout(15) << "future version ack by peer " << from << " (" << diff_map.db_version << " > " << db.version() << ")" <<
                 dendl;
    ...
    if (request.is_release()) {
        ...
        if (included_count == 0) {
            ...
            if (did_update |= set.rstate.update(next_state, db_age)) {
                dout(15) << dset("updated to match the min state of the remaining (") << included_count << ") members: " <<
...
size_t QuiesceDbManager::check_peer_reports(const QuiesceSetId& set_id, const QuiesceSet& set, const QuiesceRoot& root,
        const QuiesceSet::MemberInfo& member, QuiesceState& min_reported_state, QuiesceState& max_reported_state)
{
...
QuiesceTimeInterval QuiesceDbManager::leader_upkeep_set(Db::Sets::value_type& set_it)
{
    ...
    for (auto& [root, member] : set.members) {
        ...
        if (max_reported_state >= QS__FAILURE) {
            ...
            dout(15) << dsetroot("reported by at least one peer as ") << min_reported_state << " vs. the expected " <<
...
static QuiesceTimeInterval get_root_ttl(const QuiesceSet & set, const QuiesceSet::MemberInfo &member,
