...
static ostream & _prefix(std::ostream * _dout, epoch_t epoch,
...
void ElectionLogic::defer(int who)
{
    if (strategy == CLASSIC) {
        ...
        ldout(cct,
              5) << "defer to " << who << ", disallowed_leaders=" << elector->
                                    ...
                                    const ConnectionTracker * ct)
{
    ...
    if (from == elector->get_my_rank()) {
        lderr(cct) <<
            "I got a propose from my own rank, hopefully this is startup weirdness,dropping"
            << dendl;
        return;
    }
    switch (strategy) {
...
double ElectionLogic::connectivity_election_score(int rank)
{
    ...
    if (stable_peer_tracker) {
        ldout(cct,
              30) << "stable_peer_tracker exists so using that ..." << dendl;
        stable_peer_tracker->get_total_connection_score(rank, &score,
                                                        &liveness);
    }
    else {
        ldout(cct,
                                                 ...
                                                 const ConnectionTracker * ct)
{
    ...
    if ((epoch % 2 == 0) &&
        ...
        ldout(cct,
              30) << "Lets see if this out-of-quorum peer is worth it " <<
            dendl;
        int best_scorer = 0;
        double best_score = 0;
        ...
        for (unsigned i = 0; i < elector->paxos_size(); ++i) {
            ...
            if (last_voted_for >= 0
                && i == static_cast < unsigned >(last_voted_for)) {
                last_voted_for_score = score;
            }
        }
        ldout(cct,
              30) << "best_scorer: " << best_scorer << " best_score: " <<
            best_score << " last_voted_for: " << last_voted_for <<
            " last_voted_for_score: " << last_voted_for_score << dendl;
    ...
    ldout(cct,
          10) << "propose from rank=" << from << ",from_score=" << from_score <<
        "; my score=" << my_score << "; currently acked " << leader_acked <<
    ...
    if (my_win) {
        ...
        if (leader_acked >= 0) {    // we already acked someone
            ...
            if (!electing_me) {
                ldout(cct,
                      10) << " wait, i should win! triggering new election ..."
                    << dendl;
                elector->trigger_new_election();
            }
        }
        ...
        if (their_win || from == leader_acked) {
            if (leader_acked >= 0 && from != leader_acked) {
                ...
                ldout(cct,
                      10) << " make sure acked leader defer to: " << from <<
                    dendl;
                double leader_from_score;
                int leader_from_liveness;
                leader_peer_tracker->get_total_connection_score(from,
                                                                &leader_from_score,
                                                                &leader_from_liveness);
                double leader_leader_score;
                int leader_leader_liveness;
                leader_peer_tracker->get_total_connection_score(leader_acked,
                                                                &leader_leader_score,
                                                                &leader_leader_liveness);
                if ((from < leader_acked
                    ...
                    if ((from < leader_acked
                        ...
                        ldout(cct,
                              5) <<
                            "Bumping epoch and starting new election; acked " <<
...
void ElectionLogic::receive_ack(int from, epoch_t from_epoch)
{
    ...
    if (from_epoch > epoch) {
        ldout(cct,
              5) <<
            "woah, that's a newer epoch, i must have rebooted.  bumping and re-starting!"
            << dendl;
...
bool ElectionLogic::victory_makes_sense(int from)
{
    ...
    case CONNECTIVITY:
        ...
        ldout(cct, 5) << "victory from " << from << " makes sense? lscore:"
            << leader_score << "; my score:" << my_score << dendl;

        makes_sense = (leader_score >= my_score);
        break;
    default:
        ceph_assert(0 ==
                    "how did you get a nonsense election strategy assigned?");
    }
    return makes_sense;
}
...
bool ElectionLogic::receive_victory_claim(int from, epoch_t from_epoch)
{
    ...
    if (!election_okay) {
        ...
        ldout(cct,
              1) <<
            "I should have been elected over this leader; bumping and restarting!"
            << dendl;
        bump_epoch(from_epoch);
        start();
        return false;
    ...
    if (from_epoch != epoch + 1) {
        ldout(cct,
              5) <<
            "woah, that's a funny epoch, i must have rebooted.  bumping and re-starting!"
            << dendl;
        bump_epoch(from_epoch);
        start();
        return false;
