...
namespace cls {
    namespace rbd {
        ...
        using ceph::Formatter;
         ...
         std::ostream & operator<<(std::ostream & os,
            ...
            case MIRROR_PEER_DIRECTION_RX:
                os << "RX";
                break;
                case MIRROR_PEER_DIRECTION_TX:os << "TX";
                break;
                case MIRROR_PEER_DIRECTION_RX_TX:os << "RX/TX";
                break;
                default:os << "unknown";
                break;
            } return os;
            ...
            if (struct_v >= 2) {
                ...
                mirror_peer_direction =
                    static_cast < MirrorPeerDirection > (mpd);
        ...
        void MirrorPeer::dump(Formatter * f) const {
            ...
            o.push_back(new
                        MirrorPeer("uuid-123", MIRROR_PEER_DIRECTION_RX,
                                   "site A", "client name", ""));
            o.push_back(new
                        MirrorPeer("uuid-234", MIRROR_PEER_DIRECTION_TX,
                                   "site B", "", "mirror_uuid"));
            o.push_back(new
                        MirrorPeer("uuid-345", MIRROR_PEER_DIRECTION_RX_TX,
                                   "site C", "client name", "mirror_uuid"));
        }
        ...
        bool MirrorPeer::operator==(const MirrorPeer & rhs) const {
            ...
            case MIRROR_MODE_POOL:
                ...
                break;
            default:
                os << "unknown (" << static_cast < uint32_t >
                    (mirror_mode) << ")";
                break;
            }
            return os;
        ...
        } void MirrorImage::generate_test_instances(std::list <
                                                    MirrorImage * >&o) {
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const MirrorImageMode & mirror_mode) {
            switch (mirror_mode) {
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const MirrorImageState & mirror_state) {
            switch (mirror_state) {
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const MirrorImage & mirror_image) {
            os << "[" << "mode=" << mirror_image.
                mode << ", " << "global_image_id=" << mirror_image.
                global_image_id << ", " << "state=" << mirror_image.
                state << "]";
            return os;
        }
        ...
        void MirrorImageSiteStatus::encode_meta(uint8_t version,
                                                bufferlist & bl) const {
            if (version >= 2) {
        ...
        void MirrorImageSiteStatus::dump(Formatter * f) const {
            ...
            ss << (up ? "up+" : "down+") << state;
            return ss.str();
        } void MirrorImageSiteStatus::generate_test_instances(std::list <
            ...
            o.push_back(new
                        MirrorImageSiteStatus
                        ("2fb68ca9-1ba0-43b3-8cdf-8c5a9db71e65",
                         MIRROR_IMAGE_STATUS_STATE_STOPPED, ""));
        }

        bool MirrorImageSiteStatus::
            operator==(const MirrorImageSiteStatus & rhs) const {
            return state == rhs.state && description == rhs.description
                && up == rhs.up;
        } std::ostream & operator<<(std::ostream & os,
                                    const MirrorImageSiteStatus & status) {
            os << "{" << "state=" << status.
                state_to_string() << ", " << "description=" << status.
                description << ", " << "last_update=" << status.
                last_update << "]}";
            return os;
        }
        ...
        } void MirrorImageSiteStatusOnDisk::decode_meta(bufferlist::
                                                        const_iterator & it) {
            DECODE_START(1, it);
            decode(origin, it);
        ...
        void MirrorImageSiteStatusOnDisk::decode(bufferlist::
                                                 const_iterator & it) {
            decode_meta(it);
            cls::rbd::MirrorImageSiteStatus::decode(it);
        }

        void MirrorImageSiteStatusOnDisk::generate_test_instances(std::list <
        ...
        int MirrorImageStatus::
            get_local_mirror_image_site_status(MirrorImageSiteStatus *
                                               status) const {
            auto it = std::find_if(mirror_image_site_statuses.begin(),
        ...
        void MirrorImageStatus::decode(bufferlist::const_iterator & it) {
            ...
            if (struct_v < 2) {
                ...
                for (auto status_it = mirror_image_site_statuses.begin();
                     status_it != mirror_image_site_statuses.end();
                     ++status_it) {
                    if (local_status_valid
        ...
        bool MirrorImageStatus::operator==(const MirrorImageStatus & rhs) const {
            return (mirror_image_site_statuses ==
                    rhs.mirror_image_site_statuses);
        } void MirrorImageStatus::generate_test_instances(std::list <
                                                          MirrorImageStatus *
                                                          >&o) {
        ...
        } void ParentImageSpec::generate_test_instances(std::list <
                                                        ParentImageSpec * >&o) {
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const ParentImageSpec & rhs) {
            os << "[" << "pool_id=" << rhs.
                pool_id << ", " << "pool_namespace=" << rhs.
                pool_namespace << ", " << "image_id=" << rhs.
                image_id << ", " << "snap_id=" << rhs.snap_id << "]";
            return os;
        }
        ...
        void ChildImageSpec::dump(Formatter * f) const {
            ...
            f->dump_string("pool_namespace", pool_namespace);
            f->dump_string("image_id", image_id);
        } void ChildImageSpec::generate_test_instances(std::list <
                                                       ChildImageSpec * >&o) {
        ...
        std::string GroupImageSpec::image_key() {
            if (-1 == pool_id)
                ...
                oss << RBD_GROUP_IMAGE_KEY_PREFIX << std::setw(16)
                    << std::setfill('0') << std::
                    hex << pool_id << "_" << image_id;
                return oss.str();
            }
        }

        void GroupImageSpec::generate_test_instances(std::list <
                                                     GroupImageSpec * >&o) {
        ...
        void GroupImageStatus::dump(Formatter * f) const {
            spec.dump(f);
            f->dump_string("state", state_to_string());
        } void GroupImageStatus::generate_test_instances(std::list <
                                                         GroupImageStatus *
                                                         >&o) {
            o.push_back(new
                        GroupImageStatus(GroupImageSpec("10152ae8944a", 0),
                                         GROUP_IMAGE_LINK_STATE_ATTACHED));
            o.push_back(new
                        GroupImageStatus(GroupImageSpec("1018643c9869", 3),
                                         GROUP_IMAGE_LINK_STATE_ATTACHED));
            o.push_back(new
                        GroupImageStatus(GroupImageSpec("10152ae8944a", 0),
        ...
        void GroupSnapshotNamespace::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(static_cast < uint32_t > (original_snapshot_namespace_type),
                    bl);
        ...
        class EncodeSnapshotNamespaceVisitor {
          public:
            explicit EncodeSnapshotNamespaceVisitor(bufferlist & bl):m_bl(bl) {
                ...
                using ceph::encode;
                 encode(static_cast < uint32_t > (T::SNAPSHOT_NAMESPACE_TYPE),
                        m_bl);
                 t.encode(m_bl);
          } private:
             bufferlist & m_bl;
        ...
        class DumpSnapshotNamespaceVisitor {
          public:
            explicit DumpSnapshotNamespaceVisitor(Formatter * formatter,
        ...
        SnapshotNamespaceType get_snap_namespace_type(const SnapshotNamespace &
                                                      snapshot_namespace) {
            return static_cast < SnapshotNamespaceType >
        ...
        } void SnapshotInfo::generate_test_instances(std::list <
                                                     SnapshotInfo * >&o) {
            o.push_back(new SnapshotInfo(1ULL, UserSnapshotNamespace {
                                         }, "snap1", 123, {
                                         123456, 0}, 12));
            o.push_back(new SnapshotInfo(2ULL, GroupSnapshotNamespace {
                                         567, "group1", "snap1"}, "snap1", 123, {
                                         123456, 0}, 987));
            o.push_back(new SnapshotInfo(3ULL, TrashSnapshotNamespace {
                                         SNAPSHOT_NAMESPACE_TYPE_USER, "snap1"},
                                         "12345", 123, {
                                         123456, 0}, 429));
            o.push_back(new SnapshotInfo(1ULL, MirrorSnapshotNamespace {
        ...
        } void SnapshotNamespace::generate_test_instances(std::list <
                                                          SnapshotNamespace *
                                                          >&o) {
            o.push_back(new SnapshotNamespace(UserSnapshotNamespace()));
            o.push_back(new
                        SnapshotNamespace(GroupSnapshotNamespace
                                          (0, "10152ae8944a", "2118643c9732")));
            o.push_back(new
                        SnapshotNamespace(GroupSnapshotNamespace
                                          (5, "1018643c9869", "33352be8933c")));
            o.push_back(new SnapshotNamespace(TrashSnapshotNamespace()));
            o.push_back(new
                        SnapshotNamespace(MirrorSnapshotNamespace
                                          (MIRROR_SNAPSHOT_STATE_PRIMARY, {
                                           "peer uuid"}, "", CEPH_NOSNAP)));
            o.push_back(new
                        SnapshotNamespace(MirrorSnapshotNamespace
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const SnapshotNamespaceType & type) {
            switch (type) {
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const UserSnapshotNamespace & ns) {
            os << "[" << SNAPSHOT_NAMESPACE_TYPE_USER << "]";
            return os;
        }

        std::ostream & operator<<(std::ostream & os,
                                  const GroupSnapshotNamespace & ns) {
            os << "[" << SNAPSHOT_NAMESPACE_TYPE_GROUP << " " << "group_pool="
                << ns.group_pool << ", " << "group_id=" << ns.
                group_id << ", " << "group_snapshot_id=" << ns.
                group_snapshot_id << "]";
            return os;
        }

        std::ostream & operator<<(std::ostream & os,
                                  const TrashSnapshotNamespace & ns) {
            os << "[" << SNAPSHOT_NAMESPACE_TYPE_TRASH << " " <<
                "original_name=" << ns.
                original_name << ", " << "original_snapshot_namespace=" << ns.
                original_snapshot_namespace_type << "]";
            return os;
        }

        std::ostream & operator<<(std::ostream & os,
                                  const MirrorSnapshotNamespace & ns) {
            os << "[" << SNAPSHOT_NAMESPACE_TYPE_MIRROR << " " << "state=" <<
                ns.state << ", " << "complete=" << ns.
                complete << ", " << "mirror_peer_uuids=" << ns.
                mirror_peer_uuids << ", ";
            if (ns.is_primary()) {
                ...
                os << "primary_mirror_uuid=" << ns.primary_mirror_uuid << ", "
                    ...
                    << "last_copied_object_number=" << ns.
                    last_copied_object_number << ", " << "snap_seqs=" << ns.
                    snap_seqs;
            }
            os << "]";
            return os;
        ...
        std::ostream & operator<<(std::ostream & os,
                                  const UnknownSnapshotNamespace & ns) {
            os << "[unknown]";
            return os;
        }
        ...
        } void ImageSnapshotSpec::generate_test_instances(std::list <
                                                          ImageSnapshotSpec *
                                                          >&o) {
        ...
        } void GroupSnapshot::generate_test_instances(std::list <
                                                      GroupSnapshot * >&o) {
            o.push_back(new
                        GroupSnapshot("10152ae8944a", "groupsnapshot1",
                                      GROUP_SNAPSHOT_STATE_INCOMPLETE));
            o.push_back(new
                        GroupSnapshot("1018643c9869", "groupsnapshot2",
        ...
        } void MirrorImageMap::generate_test_instances(std::list <
                                                       MirrorImageMap * >&o) {
            bufferlist data;
        ...
        bool MirrorImageMap::operator==(const MirrorImageMap & rhs) const {
            ...
            return os << "[" << "instance_id=" << image_map.
                instance_id << ", mapped_time=" << image_map.mapped_time << "]";
        }
        ...
        void MigrationSpec::generate_test_instances(std::list <
                                                    MigrationSpec * >&o) {
            o.push_back(new MigrationSpec());
            o.push_back(new MigrationSpec(MIGRATION_HEADER_TYPE_SRC, 1, "ns",
                                          "image_name", "image_id", "", { {
                                          1, 2}}, 123,
                                          true, MIRROR_IMAGE_MODE_SNAPSHOT,
                                          true, MIGRATION_STATE_PREPARED,
                                          "description"));
            o.push_back(new
                        MigrationSpec(MIGRATION_HEADER_TYPE_DST, -1, "", "", "",
                                      "{\"format\": \"raw\"}", { {
                                      1, 2}}, 123, true,
        ...
        std::ostream & operator<<(std::ostream & os,
            ...
            if (migration_spec.header_type == MIGRATION_HEADER_TYPE_SRC ||
                ...
                os << "pool_id=" << migration_spec.pool_id << ", "
                    << "pool_namespace=" << migration_spec.
                    pool_namespace << ", " << "image_name=" << migration_spec.
                    image_name << ", " << "image_id=" << migration_spec.
                    image_id << ", ";
            }
            else {
            ...
            os << "snap_seqs=" << migration_spec.snap_seqs << ", "
                ...
                << "mirror_image_mode=" << migration_spec.
                mirror_image_mode << ", " << "state=" << migration_spec.
                state << ", " << "state_description=" << migration_spec.
                state_description << "]";
            return os;
        }

        std::ostream & operator<<(std::ostream & os,
                                  const AssertSnapcSeqState & state) {
            switch (state) {
