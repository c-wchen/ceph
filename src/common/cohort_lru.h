...
private:
    ...
    typedef bi::list < Object,
            ...
            &Object::lru_hook >, bi::constant_time_size < true >> Queue;
    ...
    typedef bi::slist < Object,
            ...
            &Object::q2_hook >, bi::constant_time_size < true >> Queue2;
...
public:
    ...
    template < typename T, typename TTree, typename CLT, typename CEQ,
               typename K, typename LK > friend class TreeX;
};
...
template < typename T, typename TTree, typename CLT, typename CEQ,
           typename K, typename LK > class TreeX
{
public:
    ...
    void lock()
    {
        std::for_each(locks.begin(), locks.end(), [](LK * lk) {
            lk->lock();
        });
    }
    ...
    TreeX(int n_part = 1, int csz = 127): n_part(n_part),
        csz(csz)
    {
    ...
    T *find_latch(uint64_t hk, const K &k, Latch &lat, uint32_t flags)
    {
        ...
        check_result r =
            lat.p->tr.insert_unique_check(k, CLT(), lat.commit_data);
