...
uint32_t qp_mempool_obj_size()
{
    ...
    mp_size +=
        align_up(rte_mempool_calc_obj_size(mbuf_overhead, 0, &mp_obj_sz) +
                 sizeof(struct rte_pktmbuf_pool_private), huge_page_size);
...
int DPDKDevice::init_port_start()
{
    ...
    if ((_dev_info.tx_offload_capa & DEV_TX_OFFLOAD_IPV4_CKSUM)) {
        ldout(cct,
              1) << __func__ << " TX ip checksum offload supported" << dendl;
    ...
    if (_dev_info.tx_offload_capa & DEV_TX_OFFLOAD_TCP_CKSUM) {
        ldout(cct,
              1) << __func__ << " TX TCP checksum offload supported" << dendl;
    ...
    if (_num_queues > 1) {
        if (!rte_eth_dev_filter_supported(_port_idx, RTE_ETH_FILTER_HASH)) {
            ...
            if (rte_eth_dev_filter_ctrl(_port_idx, RTE_ETH_FILTER_HASH,
                ...
                lderr(cct) << __func__ << " cannot set hash function on a port "
                    << _port_idx << dendl;
                return -1;
            }
        }
                                      ...
                                      float >&cpu_weights)
{
    ...
    register_packet_provider([this] {
                             Tub < Packet > p; if (!_proxy_packetq.empty()) {
...
{
    std::string name =
        std::string(pktmbuf_pool_name) + std::to_string(_qid) + "_rx";
    ...
    ldout(cct,
          20) << __func__ << " count " << rte_mempool_count(_pktmbuf_pool_rx) <<
        " free count " << rte_mempool_free_count(_pktmbuf_pool_rx) << dendl;
...
_tx_gc_poller(this)
{
    ...
    plb.add_u64_counter(l_dpdk_qp_rx_packets, "dpdk_receive_packets",
                        "DPDK received packets");
    plb.add_u64_counter(l_dpdk_qp_tx_packets, "dpdk_send_packets",
                        "DPDK sendd packets");
    plb.add_u64_counter(l_dpdk_qp_rx_bad_checksum_errors,
                        "dpdk_receive_bad_checksum_errors",
                        "DPDK received bad checksum packets");
    plb.add_u64_counter(l_dpdk_qp_rx_no_memory_errors,
                        "dpdk_receive_no_memory_errors",
                        "DPDK received no memory packets");
    plb.add_u64_counter(l_dpdk_qp_rx_bytes, "dpdk_receive_bytes",
                        "DPDK received bytes", NULL, 0, unit_t(BYTES));
    plb.add_u64_counter(l_dpdk_qp_tx_bytes, "dpdk_send_bytes",
                        "DPDK sendd bytes", NULL, 0, unit_t(BYTES));
    plb.add_u64_counter(l_dpdk_qp_rx_last_bunch, "dpdk_receive_last_bunch",
                        "DPDK last received bunch");
    plb.add_u64_counter(l_dpdk_qp_tx_last_bunch, "dpdk_send_last_bunch",
                        "DPDK last send bunch");
    plb.add_u64_counter(l_dpdk_qp_rx_fragments, "dpdk_receive_fragments",
                        "DPDK received total fragments");
    plb.add_u64_counter(l_dpdk_qp_tx_fragments, "dpdk_send_fragments",
                        "DPDK sendd total fragments");
    plb.add_u64_counter(l_dpdk_qp_rx_copy_ops, "dpdk_receive_copy_ops",
                        "DPDK received copy operations");
    plb.add_u64_counter(l_dpdk_qp_tx_copy_ops, "dpdk_send_copy_ops",
                        "DPDK sendd copy operations");
    plb.add_u64_counter(l_dpdk_qp_rx_copy_bytes, "dpdk_receive_copy_bytes",
                        "DPDK received copy bytes", NULL, 0, unit_t(BYTES));
    plb.add_u64_counter(l_dpdk_qp_tx_copy_bytes, "dpdk_send_copy_bytes",
                        "DPDK send copy bytes", NULL, 0, unit_t(BYTES));
    plb.add_u64_counter(l_dpdk_qp_rx_linearize_ops,
                        "dpdk_receive_linearize_ops",
                        "DPDK received linearize operations");
    plb.add_u64_counter(l_dpdk_qp_tx_linearize_ops, "dpdk_send_linearize_ops",
                        "DPDK send linearize operations");
    plb.add_u64_counter(l_dpdk_qp_tx_queue_length, "dpdk_send_queue_length",
    ...
    if (!_qid)
        device_stat_time_fd =
            center->create_time_event(1000 * 1000,
...
inline Tub < Packet > DPDKQueuePair::from_mbuf_lro(rte_mbuf * m)
{
    ...
    auto del = std::bind([this] (std::vector < char *>&bufs){
      for (auto && b:   bufs) {_alloc_bufs.push_back(b);}
                         ...
                         << " pool count " <<
                         rte_mempool_count(_pktmbuf_pool_rx)
                         << " free pool count " <<
                         rte_mempool_free_count(_pktmbuf_pool_rx)
...
DPDKQueuePair::tx_buf_factory::tx_buf_factory(CephContext * c, DPDKDevice * dev, uint8_t qid):cct(c)
  ...
  unsigned i = 0; for (auto & r:_redir_table) {
                         r = i++ % _num_queues;}
  ...
  i = 0; for (auto & x: reta_conf) {
      x.mask = ~0ULL; for (auto & r:x.reta) {
                         r = i++ % _num_queues;}
                         }

                         if (rte_eth_dev_rss_reta_update
                             (_port_idx, reta_conf, _dev_info.reta_size)) {
                         rte_exit(EXIT_FAILURE,
                                  "Port %d: Failed to update an RSS indirection table",
                                  _port_idx);}
                         }
