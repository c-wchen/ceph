...
int get_block_device_base(const char *dev, char *out, size_t out_len)
{
    ...
    int r = 0;
    DIR *dir;
    char devname[PATH_MAX] = { 0 }, fn[PATH_MAX] = {
    0};
    char *p;
    char realname[PATH_MAX] = { 0 };

    if (strncmp(dev, "/dev/", 5) != 0) {
        if (realpath(dev, realname) == NULL
            || (strncmp(realname, "/dev/", 5) != 0)) {
            return -EINVAL;
        }
    }
...
int64_t get_block_device_int_property(const char *devname, const char *property)
{
    char buff[256] = { 0 };
    int r =
        get_block_device_string_property(devname, property, buff, sizeof(buff));
    if (r < 0)
        return r;
    // take only digits
    for (char *p = buff; *p; ++p) {
...
{
    return get_block_device_int_property(devname,
...
{
    return get_block_device_string_property(devname, "device/model", model,
                                            max);
}
                       ...
                       char *device)
{
    ...
    if (dev) {
        ...
        rc = get_block_device_base(partition, basename, sizeof(basename));
        if (rc >= 0) {
