...
namespace {
    template < typename T >
        std::optional < T >
        find_value(const FuturizedStore::Shard::omap_values_t & values,
                   string_view key) {
...
{
    return store.open_collection(coll_t {
                                 pgid}
    ).then([this] (auto ch) {
           return store.omap_get_values(ch, pgid.make_pgmeta_oid(), {
                                        string {
                                        infover_key}
                                        , string {
                                        epoch_key}
...
seastar::future < std::tuple < pg_info_t, PastIntervals >> PGMeta::load()
{
    return store.open_collection(coll_t {
           ...
           return store.omap_get_values(ch, pgid.make_pgmeta_oid(), {
                ...
                assert(found); info = *std::move(found);}
                PastIntervals past_intervals; {
                using biginfo_t =
                std::pair < PastIntervals, decltype(info.purged_snaps) >;
                auto big_info = find_value < biginfo_t > (values, biginfo_key);
                assert(big_info); past_intervals = std::move(big_info->first);
                info.purged_snaps = std::move(big_info->second);}
                {
                auto fast_info =
                find_value < pg_fast_info_t > (values, fastinfo_key);
                if (fast_info) {
                fast_info->try_apply_to(&info);}
                }
                return seastar::make_ready_future < std::tuple < pg_info_t,
                PastIntervals >> (std::
                                  make_tuple(std::move(info),
