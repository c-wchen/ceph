...
{
    WRAP_RETRY(vector < string > path_comp;
               string short_name;
               r = decompose_full_path(path, &path_comp, 0, &short_name);
               if (r < 0)
               goto out; r = lfn_created(path_comp, oid, short_name); if (r < 0) {
               if (failed) {
               ...
               r = 0;}
               goto out;}
               r = _created(path_comp, oid, short_name); if (r < 0)
               goto out;) ;
}

int LFNIndex::unlink(const ghobject_t & oid)
{
    WRAP_RETRY(vector < string > path;
               string short_name;
               r = _lookup(oid, &path, &short_name, NULL); if (r < 0) {
               goto out;}
               r = _remove(path, oid, short_name); if (r < 0) {
               goto out;}
    ) ;
}

int LFNIndex::lookup(const ghobject_t & oid,
                     IndexedPath * out_path, int *hardlink)
{
    WRAP_RETRY(vector < string > path;
               string short_name;
               r = _lookup(oid, &path, &short_name, hardlink); if (r < 0)
               goto out;
                             ...
                             map < string, ghobject_t > *remaining)
{
    ...
    for (map < string, ghobject_t >::const_iterator to_clean =
         to_remove.begin(); to_clean != to_remove.end(); ++to_clean) {
        ...
        for (set < int >::iterator i = holes.begin(); i != holes.end(); ++i) {
            if (candidate == chain.rend() || *i > candidate->first) {
                string remove_path_name =
                    get_full_path(dir,
                                  lfn_get_short_name(to_clean->second, *i));
                maybe_inject_failure();
...
int LFNIndex::move_subdir(LFNIndex & from,
                          LFNIndex & dest,
...
int LFNIndex::move_object(LFNIndex & from,
                          LFNIndex & dest,
        ...
        if (lfn_is_object(short_name)) {
            ...
            if (r == -EINVAL) {
                ...
                if (index_version == HASH_INDEX_TAG)
                    get_hobject_from_oinfo(to_list_path.c_str(),
                                           short_name.c_str(), &obj);
...
int LFNIndex::add_attr_path(const vector < string > &path,
                            const string & attr_name, bufferlist & attr_value)
{
                           ...
                           int *hardlink)
{
    ...
    for (;; ++i) {
        ...
        bufferptr bp;
        r = chain_getxattr_buf(candidate_path.c_str(),
                               get_lfn_attr().c_str(), &bp);
        if (r < 0) {
        ...
        if (r > 0) {
            ...
            if (st.st_nlink <= 1) {
                ...
                maybe_inject_failure();
                dout(20) << __func__ << " found extra alt attr for " <<
                    candidate_path << ", long name " << string(bp.c_str(),
                                                               bp.
                                                               length()) <<
                    dendl;
            ...
            string lfn(bp.c_str(), bp.length());
            if (lfn == full_name) {
                dout(20) << __func__ << " used alt attr for " << full_name <<
                    dendl;
                if (mangled_name)
                    *mangled_name = candidate;
                if (out_path)
...
int LFNIndex::lfn_created(const vector < string > &path,
                          const ghobject_t & oid, const string & mangled_name)
{
    ...
    if (r > 0) {
        ...
        if (lfn != full_name) {
            dout(20) << __func__ << " " << mangled_name
                ...
                << lfn << ", new name is " << full_name << dendl;
                         ...
                         const ghobject_t & oid, const string & mangled_name)
{
    ...
    if (i == removed_index + 1) {
        ...
        string rename_from =
            get_full_path(path, lfn_get_short_name(oid, i - 1));
        maybe_inject_failure();
    ...
    if (r == 0 && st.st_nlink > 0) {
        // remove alt attr
        dout(20) << __func__ << " removing alt attr from " << full_path <<
            dendl;
        fsync_dir(path);
...
int LFNIndex::lfn_parse_object_name(const string & long_name, ghobject_t * out)
{
    ...
    (*out) =
        ghobject_t(hobject_t(name, key, snap, hash, (int64_t) pool, ns),
                   generation, shard_id);
    return 0;
}
...
bool LFNIndex::lfn_is_hashed_filename(const string & name)
{
    ...
    if (name.
        substr(name.size() - FILENAME_COOKIE.size(), FILENAME_COOKIE.size())
        == FILENAME_COOKIE) {
        return 1;
    }
    else {
                              ...
                              int len)
{
    ...
    while (1) {
        int suffix_len =
            sprintf(filename + ofs, "_%s_%d_%s", hash, i,
                    FILENAME_COOKIE.c_str());
...
string LFNIndex::get_full_path(const vector < string > &rel,
                               const string & name)
{
                                  ...
                                  ghobject_t * oid, string * shortname)
{
    ...
    while (1) {
        ...
        if (*end != '\0') {
            out->
                push_back(demangle_path_component
                          (string(beginning, end - beginning)));
            continue;
        }
        else {
            break;
