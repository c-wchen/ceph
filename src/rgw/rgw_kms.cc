...
typedef rapidjson::GenericDocument < rapidjson::UTF8 <>,
        ZeroPoolAllocator, rapidjson::CrtAllocator > ZeroPoolDocument;
typedef rapidjson::GenericValue < rapidjson::UTF8 <>,
        ZeroPoolAllocator > ZeroPoolValue;
                                    ...
                                    std::string_view needle)
{
    ...
    auto hay_suffix_start {
        hay.data() + (hay_len - needle_len)};
    while (hay_len > needle_len && hay[hay_len - 1] == '/') {
        --hay_len;
        --hay_suffix_start;
    }
    std::string_view hay_suffix {
        hay_suffix_start, needle_len};
    return hay_suffix == needle;
}

template < typename E, typename A = ZeroPoolAllocator >
static inline void add_name_val_to_obj(std::string &n, std::string &v,
                                       rapidjson::GenericValue < E, A > &d, A &allocator)
{
...
template < typename E, typename A = ZeroPoolAllocator >
static inline void add_name_val_to_obj(std::string &n, bool v, rapidjson::GenericValue < E,
                                       A > &d, A &allocator)
{
    rapidjson::GenericValue < E, A > name, val;
    name.SetString(n.c_str(), n.length(), allocator);
...
template < typename E, typename A = ZeroPoolAllocator >
static inline void add_name_val_to_obj(const char *n, std::string &v, rapidjson::GenericValue < E,
                                       A > &d, A &allocator)
{
    std::string ns {
        n, strlen(n)};
...
template < typename E, typename A = ZeroPoolAllocator >
static inline void add_name_val_to_obj(const char *n, bool v, rapidjson::GenericValue < E, A > &d,
                                       A &allocator)
{
    std::string ns {
        n, strlen(n)};
...
protected:
    ...
    int load_token_from_file(const DoutPrefixProvider *dpp,
                             std::string *vault_token)
    {
        ...
        if (res < 0) {
            if (-EACCES == res) {
                ldpp_dout(dpp,
                          0) <<
                             "ERROR: Permission denied reading Vault token file" <<
                             dendl;
            } else {
                ldpp_dout(dpp,
                          0) <<
    ...
    FORTEST_VIRTUAL
    int send_request(const DoutPrefixProvider *dpp, const char *method,
                     ...
                     const std::string &postdata, bufferlist &secret_bl)
    {
        ...
        if (!vault_token.empty()) {
            secret_req.append_header("X-Vault-Token", vault_token);
            vault_token.replace(0, vault_token.length(), vault_token.length(),
                                '\000');
        }
        ...
        if (!vault_namespace.empty()) {
            ldpp_dout(dpp,
                      20) << "Vault Namespace: " << vault_namespace << dendl;
        ...
        if (secret_req.get_http_status() ==
            ...
            ldpp_dout(dpp,
                      0) << "ERROR: Vault request failed authorization" <<
                         dendl;
            return -EACCES;
        }
    ...
    {
        return send_request(dpp, "GET", "", key_id, string {
        }
        , secret_bl);
    }

    int decode_secret(const DoutPrefixProvider *dpp, std::string encoded,
                      std::string &actual_key)
    {
        try {
            ...
            ldpp_dout(dpp,
                      0) <<
                         "ERROR: Failed to base64 decode key retrieved from Vault" <<
                         dendl;
            return -EINVAL;
        }
    ...
    int get_key_version(std::string_view key_id, string &version)
    {
        ...
        if (pos != std::string_view::npos) {
            std::string_view token =
                key_id.substr(pos + 1, key_id.length() - pos);
            if (!token.empty()
...
public:
    TransitSecretEngine(CephContext *cct, SSEContext &kctx, EngineParmMap parms): VaultSecretEngine(cct, kctx),
        parms(parms)
    {
        compat = COMPAT_UNSET;
        for (auto &e : parms) {
                ...
                std::string &actual_key)
    {
        ...
        int res =
            send_request(dpp, "GET",
                         compat ==
                         COMPAT_ONLY_OLD ? "" : "/export/encryption-key",
        ...
        if (!v || !v->IsString()) {
            ldpp_dout(dpp,
                      0) <<
                         "ERROR: Key not found in JSON response from Vault using Transit Engine"
                         << dendl;
            return -EINVAL;
        }
    ...
    int make_actual_key(const DoutPrefixProvider *dpp, map < string,
                        bufferlist > &attrs, std::string &actual_key)
    {
        ...
        */
        ...
        if (!d.IsObject()) {
            ldpp_dout(dpp,
                      0) << "ERROR: response from Vault is not an object" <<
                         dendl;
            return -EINVAL;
        }
        {
            ...
            if (data_itr == d.MemberEnd()) {
                ldpp_dout(dpp,
                          0) << "ERROR: no .data in response from Vault" <<
                             dendl;
                return -EINVAL;
            }
            auto ciphertext_itr {
                data_itr->value.FindMember("ciphertext")};
            auto plaintext_itr {
                data_itr->value.FindMember("plaintext")};
            ...
            if (plaintext_itr == data_itr->value.MemberEnd()) {
                ldpp_dout(dpp,
                          0) <<
                             "ERROR: no .data.plaintext in response from Vault" << dendl;
                return -EINVAL;
            }
            auto &ciphertext_v {
                ciphertext_itr->value};
            auto &plaintext_v {
                plaintext_itr->value};
    ...
    int reconstitute_actual_key(const DoutPrefixProvider *dpp, map < string,
                                bufferlist > &attrs, std::string &actual_key)
    {
        std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
                          ...
                          const std::string &key_name)
    {
        ...
        */
        ...
        if (dummy_bl.length() != 0) {
            ldpp_dout(dpp,
                      0) <<
                         "ERROR: unexpected response from Vault making a key: " <<
                         dummy_bl << dendl;
        }
        return 0;
    }
                          ...
                          const std::string &key_name)
    {
        ...
        */
        ...
        if (dummy_bl.length() != 0) {
            ldpp_dout(dpp,
                      0) <<
                         "ERROR: unexpected response from Vault marking key to delete: "
                         << dummy_bl << dendl;
            return -EINVAL;
        }

        res = send_request(dpp, "DELETE", "", delete_path, string {
        }
        , dummy_bl);
        if (res < 0) {
            return res;
        ...
        if (dummy_bl.length() != 0) {
            ldpp_dout(dpp,
                      0) <<
                         "ERROR: unexpected response from Vault deleting key: " <<
                         dummy_bl << dendl;
            return -EINVAL;
        }
        return 0;
...
public:
    ...
    KvSecretEngine(CephContext *cct, SSEContext &kctx,
                   EngineParmMap parms): VaultSecretEngine(cct, kctx)
    {
        if (!parms.empty()) {
            lderr(cct) <<
                       "ERROR: vault kv secrets engine takes no parameters (ignoring them)"
                       << dendl;
        }
                                        ...
                                        std::string &actual_key)
{
    ...
    if (rgw::keystone::Service::get_keystone_barbican_token(dpp, cct, token) <
        0) {
                                     ...
                                     std::string &actual_key, bool make_it)
{
    ...
    EngineParmMap secret_engine_parms;
    auto secret_engine {
        config_to_engine_and_parms(cct, "rgw_crypt_vault_secret_engine",
                                   secret_engine_str, secret_engine_parms)};
    ldpp_dout(dpp,
              20) << "Vault authentication method: " << kctx.auth() << dendl;
...
class KMSContext: public SSEContext
{
    CephContext *cct;
public:
    KMSContext(CephContext *_cct): cct {
        _cct}
    {
    };
    ~KMSContext()override
...
public:
    ...
    SseS3Context(CephContext *_cct): cct {
        _cct}
    {
    };
    ~SseS3Context()
                                     ...
                                     std::string &actual_key)
{
    ...
    KMSContext kctx {
        cct};
    const std::string &kms_backend {
        kctx.backend()};

    ldpp_dout(dpp,
              20) << "Getting KMS encryption key for key " << key_id << dendl;
                                ...
                                std::string &actual_key)
{
    ...
    if (RGW_SSE_KMS_BACKEND_VAULT != kms_backend) {
        ldpp_dout(dpp,
                  0) << "ERROR: Unsupported rgw_crypt_sse_s3_backend: " <<
                     kms_backend << dendl;
        return -EINVAL;
    }
    return make_actual_key_from_vault(dpp, cct, kctx, attrs, actual_key);
...
int create_sse_s3_bucket_key(const DoutPrefixProvider *dpp,
                             CephContext *cct, const std::string &bucket_key)
{
    SseS3Context kctx {
        cct};
    ...
    EngineParmMap secret_engine_parms;
    auto secret_engine {
        config_to_engine_and_parms(cct, "rgw_crypt_sse_s3_vault_secret_engine",
                                   secret_engine_str, secret_engine_parms)};
