...
namespace CrushTreeDumper {
    ...
    struct Item {
        ...
        } Item(int i, int p, int d, float w):id(i), parent(p), depth(d),
            weight(w) {
        } bool is_bucket() const {
            return id < 0;
    }};

  template < typename F > class Dumper:public list < Item > {
      public:
      ...
      :    crush(crush_), weight_set_names(weight_set_names_) {
        ...
        explicit Dumper(const CrushWrapper * crush_,
                        const name_map_t & weight_set_names_, bool show_shadow)
        :crush(crush_), weight_set_names(weight_set_names_) {
            if (show_shadow) {
        ...
        if (qi.parent < 0) {
          ...
          for (auto & p:crush->choose_args) {
                ...
                if (b &&
                    ...
                    for (unsigned opos = 0;
                         opos < cmap.args[bidx].weight_set_positions; ++opos) {
                        float w =
                            (float)cmap.args[bidx].weight_set[opos].
                            weights[bpos] / (float)0x10000;
