    ...
    metrics_handler(metrics_handler)
{
    ...
    caps_throttle_retry_request_timeout = g_conf().get_val <
    ...
    switch (m->get_type()) {
        ...
        default:
            derr << "Server unknown message " << m->get_type() << " from peer type " << m->get_connection()->get_peer_type() <<
                 dendl;
            ceph_abort_msg("server unknown message  " + to_string(m->get_type()) + " from peer type " + to_string(
...
std::pair < bool, uint64_t > Server::recall_client_state(MDSGatherBuilder* gather, RecallFlags flags)
{
    ...
    const auto recall_global_max_decay_threshold = g_conf().get_val <
    ...
    auto f = [&caps_session, enforce_max, enforce_liveness, trim, max_caps_per_client, cache_liveness_magnitude](auto & s) {
        ...
        if (trim || (enforce_max && num_caps > max_caps_per_client) || (enforce_liveness &&
    ...
    for (const auto& [num_caps, session] : boost::adaptors::reverse(caps_session)) {
        ...
        if (num_caps > newlim) {
            ...
            if (session_recall_throttle + recall > recall_max_decay_threshold) {
                dout(15) << "  session recall threshold (" << recall_max_decay_threshold << ") hit at " << session_recall_throttle <<
                            "; skipping!" << dendl;
                throttled = true;
                continue;
            } else if (session_recall_throttle2o + recall > recall_max_caps * 2) {
                dout(15) << "  session recall 2nd-order threshold (" << 2 * recall_max_caps << ") hit at " << session_recall_throttle2o
                         << "; skipping!" << dendl;
                throttled = true;
                continue;
            } else if (global_recall_throttle + recall > recall_global_max_decay_threshold) {
                dout(15) << "  global recall threshold (" << recall_global_max_decay_threshold << ") hit at " << global_recall_throttle
                         << "; skipping!" << dendl;
                throttled = true;
                break;
            }
 ...
 *    order.
 */
std::pair < CDentry *, CDentry * > Server::rdlock_two_paths_xlock_destdn(const MDRequestRef& mdr, bool xlock_srcdn)
{
...
void Server::handle_client_readdir(const MDRequestRef& mdr)
{
    ...
    if (num_caps > static_cast < uint64_t > (max_caps_per_client * max_caps_throttle_ratio) &&
        ...
        dout(20) << "readdir throttled. max_caps_per_client: " << max_caps_per_client << " num_caps: " << num_caps
                 << " session_cap_acquistion: " << session_cap_acquisition << " cap_acquisition_throttle: " << cap_acquisition_throttle
                 << dendl;
        if (logger) {
...
bool Server::_rmdir_prepare_witness(const MDRequestRef& mdr, mds_rank_t who, vector < CDentry* > & trace,
                                    CDentry *straydn)
{
    if (mds->is_cluster_degraded() &&
...
void Server::handle_peer_rename_prep(const MDRequestRef& mdr)
{
    ...
    if (srcdn->is_auth()) {
        ...
        for (set < mds_rank_t >::iterator p = srcdnrep.begin(); p != srcdnrep.end(); ++p) {
            if (*p == mdr->peer_to_mds ||
                mdr->peer_request->witnesses.count(*p)) {
                continue;
            }
...
void Server::handle_client_mksnap(const MDRequestRef& mdr)
{
    ...
    if (mdr->client_request->get_caller_uid() < g_conf()->mds_snap_min_uid ||
        ...
        dout(20) << "mksnap " << snapname << " on " << *diri << " denied to uid " << mdr->client_request->get_caller_uid() <<
                 dendl;
        respond_to_request(mdr, -CEPHFS_EPERM);
        return;
    }
...
void Server::handle_client_rmsnap(const MDRequestRef& mdr)
{
    ...
    if (mdr->client_request->get_caller_uid() < g_conf()->mds_snap_min_uid ||
        mdr->client_request->get_caller_uid() > g_conf()->mds_snap_max_uid) {
        dout(20) << "rmsnap " << snapname << " on " << *diri << " denied to uid " << mdr->client_request->get_caller_uid() <<
                 dendl;
        respond_to_request(mdr, -CEPHFS_EPERM);
        return;
    }
