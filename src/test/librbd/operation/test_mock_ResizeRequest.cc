...
} // namespace util namespace operation {
...
TrimRequest < MockImageCtx > *TrimRequest < MockImageCtx >::s_instance =
    nullptr;
...
public:
                    ...
                    bool disable_journal)
    {
        ...
        MockResizeRequest *req =
            new MockResizeRequest(mock_image_ctx, &cond_ctx, new_size,
...
TEST_F(TestMockOperationResizeRequest, NoOpSuccess)
{
    ...
    ASSERT_EQ(0,
              when_resize(mock_image_ctx, ictx->size, true, 0, false));
}
...
TEST_F(TestMockOperationResizeRequest, GrowSuccess)
{
    ...
    expect_commit_op_event(mock_image_ctx, 0);
    ASSERT_EQ(0,
              when_resize(mock_image_ctx, ictx->size * 2, true, 0,
                          false));
}
...
TEST_F(TestMockOperationResizeRequest, ShrinkError)
{
    ...
    expect_unblock_writes(mock_image_ctx);
    ASSERT_EQ(-EINVAL,
              when_resize(mock_image_ctx, ictx->size / 2, false, 0,
                          false));
}
...
TEST_F(TestMockOperationResizeRequest, JournalDisabled)
{
    ...
    expect_unblock_writes(mock_image_ctx);
    ASSERT_EQ(0,
              when_resize(mock_image_ctx, ictx->size, true, 0, true));
}
