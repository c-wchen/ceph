...
namespace rgw::flight {
    ...
    struct FlightData {
        ...
        FlightData(const std::string & _uri,
                   ...
                   std::shared_ptr < const arw::KeyValueMetadata >
                   &_kv_metadata, rgw_user _user_id);
    };
...
// stores flights that have been created and helps expire them
    class FlightStore {
      ...
      public:
        ...
        virtual arw::Result < FlightData >
            get_flight(const FlightKey & key) const = 0;
        virtual std::optional < FlightData >
            after_key(const FlightKey & key) const = 0;
    ...
    class MemoryFlightStore:public FlightStore {
      ...
      public:
        ...
        arw::Result < FlightData >
            get_flight(const FlightKey & key) const override;
        std::optional < FlightData >
            after_key(const FlightKey & key) const override;
    ...
    class FlightServer:public flt::FlightServerBase {
      ...
      public:
        ...
        FlightServer(RGWProcessEnv & env,
                     FlightStore * flight_store, const DoutPrefix & dp);
    ...
    class OwningStringView:public arw::util::string_view {
        ...
        OwningStringView(uint8_t * _buffer,
                         int64_t _size):arw::util::
            string_view((const char *)_buffer, _size), buffer(_buffer),
            capacity(_size), consumed(_size) {
        } OwningStringView(OwningStringView
                           && from, int64_t new_size):buffer(nullptr),
            capacity(from.capacity), consumed(new_size) {
            // should be impossible due to static function check
            ceph_assertf(consumed <= capacity,
                         "new size cannot exceed capacity");
