...
class Monitor:public Dispatcher,
    public AuthClient, public AuthServer, public md_config_obs_t {
  public:
    int orig_argc = 0;
   ...
   */
    ...
    ceph_release_t quorum_min_mon_release {
    ceph_release_t::unknown};

    std::set < std::string > outside_quorum;

    bool stretch_mode_engaged {
    false};
    bool degraded_stretch_mode {
    false};
    bool recovering_stretch_mode {
    false};
    ...
    bool need_set_crush_loc {
    false};
  public:
    bool is_stretch_mode() {
        return stretch_mode_engaged;
    }
    bool is_degraded_stretch_mode() {
        return degraded_stretch_mode;
    }
    bool is_recovering_stretch_mode() {
   ...
   */
    ...
    void trigger_degraded_stretch_mode(const std::set < std::string >
                                       &dead_mons,
   ...
   */
    ...
    bool _scrub(ScrubResult * r,
                std::pair < std::string, std::string > *start, int *num_keys);
   ...
   */
    ...
    static void _generate_command_map(cmdmap_t & cmdmap,
                                      std::map < std::string,
                                      std::string > &param_str_map);
    static const MonCommand *_get_moncommand(const std::string & cmd_prefix,
   ...
   */
    ...
    void log_health(const health_check_map_t & updated,
    ...
    class HealthCheckLogStatus {
      public:
        ...
        HealthCheckLogStatus(health_status_t severity_,
                             ...
                             utime_t updated_at_)
        :severity(severity_),
            last_message(last_message_), updated_at(updated_at_) {
        }
    };
  ...
  public:
    ...
    void reply_command(MonOpRequestRef op, int rc, const std::string & rs,
                       version_t version);
    void reply_command(MonOpRequestRef op, int rc, const std::string & rs,
                       ceph::buffer::list & rdata, version_t version);
   ...
   */
    void notify_new_monmap(bool can_change_external_state =
                           false, bool remove_rank_elector = true);
  ...
  public:
    struct C_Command:public C_MonOp {
        ...
        version_t version;
        C_Command(Monitor & _mm, MonOpRequestRef _op, int r, std::string s,
                  version_t v):C_MonOp(_op), mon(_mm), rc(r), rs(s),
            version(v) {
        } C_Command(Monitor & _mm, MonOpRequestRef _op, int r, std::string s,
                    ceph::buffer::list rd, version_t v):C_MonOp(_op), mon(_mm),
            rc(r), rs(s), rdata(rd), version(v) {
        }
  ...
  private:
    ...
    // AuthClient
    int get_auth_request(Connection * con,
                         AuthConnectionMeta * auth_meta,
                         uint32_t * method,
                         std::vector < uint32_t > *preferred_modes,
                         ceph::buffer::list * out) override;
    int handle_auth_reply_more(Connection * con,
                               AuthConnectionMeta * auth_meta,
                               const ceph::buffer::list & bl,
                               ceph::buffer::list * reply) override;
    int handle_auth_done(Connection * con,
                         AuthConnectionMeta * auth_meta,
                         uint64_t global_id,
                         uint32_t con_mode,
                         const ceph::buffer::list & bl,
    ...
    int handle_auth_bad_method(Connection * con,
                               AuthConnectionMeta * auth_meta,
                               uint32_t old_auth_method,
                               int result,
                               const std::vector < uint32_t > &allowed_methods,
                               const std::vector < uint32_t >
                               &allowed_modes) override;
    // /AuthClient
    // AuthServer
    int handle_auth_request(Connection * con,
                            AuthConnectionMeta * auth_meta,
                            bool more,
    ...
    void get_all_versions(std::map < std::string,
    ...
    static void read_features_off_disk(MonitorDBStore * store,
  ...
  public:
    static void format_command_descriptions(const std::vector < MonCommand >
                                            &commands, ceph::Formatter * f,
                                            uint64_t features,
                                            ceph::buffer::list * rdata);
...
template < typename T > class C_MonContext:public LambdaContext < T > {
  public:
    C_MonContext(const Monitor * m, T
                 && f):LambdaContext < T > (std::forward < T > (f)), mon(m) {
    }
