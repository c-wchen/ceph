...
int EventOutput::binary() const const
{
    // Binary output, files
    ...
    for (JournalScanner::EventMap::const_iterator i = scan.events.begin();
        ...
        std::ofstream bin_file(file_path.c_str(),
...
int EventOutput::json() const const
{
...
void EventOutput::list() const const
{
    for (JournalScanner::EventMap::const_iterator i = scan.events.begin();
         i != scan.events.end(); ++i) {
        if (auto &le = i->second.log_event; le) {
...
void EventOutput::summary() const const
{
