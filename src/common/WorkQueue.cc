...
ThreadPool::ThreadPool(CephContext * cct_, string nm, string tn, int n,
                       const char *option)
:cct(cct_), name(std::move(nm)), thread_name(std::move(tn)), lockname(name + "::lock"), _lock(lockname.c_str()),    // this should be safe due to declaration order
_stop(false),
_pause(0),
_draining(0),
ioprio_class(-1), ioprio_priority(-1), _num_threads(n), processing(0)
{
    if (option) {
...
void ThreadPool::worker(WorkThread * wt)
{
    ...
    while (!_stop) {
        ...
        cct->get_heartbeat_map()->reset_timeout(hb,
                                                cct->_conf->
                                                threadpool_default_timeout, 0);
        _cond.WaitInterval(_lock,
                           utime_t(cct->_conf->threadpool_empty_queue_max_wait,
                                   0));
    }
...
ShardedThreadPool::ShardedThreadPool(CephContext * pcct_, string nm, string tn, uint32_t pnum_threads):
cct(pcct_),
name(std::move(nm)),
...
void ShardedThreadPool::stop()
{
    ...
    for (vector < WorkThreadSharded * >::iterator p =
         threads_shardedpool.begin(); p != threads_shardedpool.end(); ++p) {
        (*p)->join();
        delete *p;
    }
