...
template < typename I > ExclusiveLock < I >::ExclusiveLock(I &image_ctx)
    : RefCountedObject(image_ctx.cct),
      ML < I > (image_ctx.md_ctx, *image_ctx.asio_engine,
                image_ctx.header_oid, image_ctx.image_watcher,
                ...
                ("rbd_blocklist_expire_seconds")), m_image_ctx(image_ctx)
{
...
template < typename I >
bool ExclusiveLock <
I >::accept_request(OperationRequestType request_type,
                    int *ret_val) const
{
    std::lock_guard locker {
        ML < I >::m_lock};
    ...
    bool accept_request =
        (!ML < I >::is_state_shutdown() && ML < I >::is_state_locked() &&
         (m_request_blocked_count == 0 ||
          ...
          accept_blocked_request(request_type)));
    if (ret_val != nullptr) {
...
} template < typename I > void ExclusiveLock <
I >::set_require_lock(bool init_shutdown, io::Direction direction,
                      Context *on_finish)
{
...
void ExclusiveLock < I >::init(uint64_t features, Context *on_init)
{
    ...
    m_image_dispatch =
        exclusive_lock::ImageDispatch < I >::create(&m_image_ctx);
    ...
    if (m_image_ctx.clone_copy_on_read ||
        ...
        m_image_dispatch->set_require_lock(true, io::DIRECTION_BOTH,
                                           on_init);
    } else {
...
void ExclusiveLock < I >::shutdown_handler(int r, Context *on_finish)
{
    ...
    m_image_ctx.io_image_dispatcher->shut_down_dispatch(m_image_dispatch->
               get_dispatch_layer
               (), on_finish);
}
...
template < typename I >
void ExclusiveLock <
I >::pre_acquire_lock_handler(Context *on_finish)
{
    ldout(m_image_ctx.cct, 10) << dendl;
...
template < typename I >
void ExclusiveLock < I >::post_acquire_lock_handler(int r,
        Context *
        on_finish)
{
    ...
    if (r == -EROFS) {
        ...
        if (ML < I >::is_action_acquire_lock()
            && (r == -EBUSY || r == -EAGAIN)) {
    ...
    PostAcquireRequest < I > *req =
        PostAcquireRequest < I >::create(m_image_ctx,
                                         util::create_context_callback < EL,
        ...
        on_finish)
{
    ...
    auto req =
        PreReleaseRequest < I >::create(m_image_ctx, m_image_dispatch,
                                        shutting_down, m_async_op_tracker,
                                        on_finish);
    m_image_ctx.op_work_queue->queue(new LambdaContext([req](int r) {
        req->send();
    }));
...
template < typename I >
void ExclusiveLock < I >::post_release_lock_handler(bool shutting_down,
        int r,
        Context *
        on_finish)
{
    ldout(m_image_ctx.cct,
          10) << ": r=" << r << " shutting_down=" << shutting_down << dendl;
    if (!shutting_down) {
        {
            std::lock_guard locker {
                ML < I >::m_lock};
        ...
        m_image_ctx.io_image_dispatcher->
        shut_down_dispatch(m_image_dispatch->get_dispatch_layer(),
                           on_finish);
    }
}

template < typename I >
void ExclusiveLock < I >::post_reacquire_lock_handler(int r,
        Context *
        on_finish)
{
    ldout(m_image_ctx.cct, 10) << dendl;
