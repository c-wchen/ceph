...
int CrushCompiler::decompile(ostream & out)
{
    ...
    for (int i = 0; i < crush.get_max_rules(); i++) {
        ...
        if (i != crush.get_rule_mask_ruleset(i)) {
            out << "\t# WARNING: ruleset " << crush.
                get_rule_mask_ruleset(i) << " != id " << i <<
...
int CrushCompiler::parse_bucket(iter_t const &i)
{
    ...
    string name = string_node(i->children[1]);
    if (item_id.count(name)) {
        err << "bucket or device '" << name << "' is already defined" << std::
            endl;
        return -1;
    }
        ...
        if (tag == "id") {
            ...
            if (sub->children.size() > 2) {
                ...
                if (class_id.count(cid) != 0) {
                    err << "duplicate device class " << class_name <<
                        " for bucket " << name << std::endl;
                    return -ERANGE;
                }
            ...
            for (unsigned q = 2; q < sub->children.size(); q++) {
                ...
                if (tag == "pos") {
                    ...
                    if (used_items.count(pos)) {
                        err << "item '" << string_node(sub->
                                                       children[1]) <<
                            "' in bucket '" << name << "' has explicit pos " <<
                            pos << ", which is occupied" << std::endl;
                        return -1;
                    }
    ...
    for (unsigned p = 3; p < i->children.size() - 1; p++) {
        ...
        if (tag == "item") {
                ...
                if (!have_uniform_weight) {
                    ...
                    if (uniform_weight != weight) {
                        err << "item '" << iname << "' in uniform bucket '" <<
                            ...
                            " but previous item(s) have weight " << (float)
                            uniform_weight /
                            (float)0x10000 <<
            ...
            if (pos >= size) {
                err << "item '" << iname << "' in bucket '" << name <<
                    "' has pos " << pos << " >= size " << size << std::endl;
                return -1;
            }
            if (pos < 0) {
            ...
            if (crush_addition_is_unsafe(bucketweight, weight)) {
                err <<
                    "oh no! our bucket weights are overflowing all over the place, better lower the item weights"
                    << std::endl;
                return -ERANGE;
            }
  ...
  for (auto & i:class_id)
    ...
    if (verbose)
        err << "bucket " << name << " (" << id << ") " << size <<
            " items and weight " << (float)bucketweight /
        ...
        case crush_grammar::_step_take:
            {
                ...
                if (s->children.size() > 2) {
                    ...
                    if (crush.class_bucket[id].count(c) == 0) {
                        err << "in rule '" << rname << "' step take " << item
                            << " no matching bucket for class " << class_name <<
                            std::endl;
                        return -EINVAL;
                    }
                                    ...
                                    crush_choose_arg * arg)
{
    ...
    arg->weight_set =
        (crush_weight_set *) calloc(arg->weight_set_positions,
...
int CrushCompiler::compile(istream & in, const char *infn)
{
    ...
    if (!info.full) {
        ...
        err << infn << ":" << line  //<< ":" << (pos+1)
            << " error: parse error at '" << line_val[line].
            substr(pos) << "'" << std::endl;
        return -1;
    }
