...
:  cct(cct), parent(lc), log_channel(channel), log_to_syslog(false),
...
{
    ldout(cct,
          20) << __func__ << " log_to_monitors " << conf_strings.
        log_to_monitors << " log_to_syslog " << conf_strings.
        log_to_syslog << " log_channels " << conf_strings.
        log_channels << " log_prios " << conf_strings.log_prios << dendl;
    ...
    if (graylog && !conf_strings.log_to_graylog_host.empty()
        && (graylog_port != 0)) {
        graylog->set_destination(conf_strings.log_to_graylog_host,
                                 graylog_port);
    }
...
clog_targets_conf_t LogChannel::parse_log_client_options(CephContext * cct)
{
    ...
    targets.log_channels =
        get_value_via_strmap(cct->_conf.get_val < string >
                             ("clog_to_syslog_facility"), log_channel,
                             CLOG_CONFIG_DEFAULT_KEY);
    targets.log_prios =
        get_value_via_strmap(cct->_conf.get_val < string >
                             ("clog_to_syslog_level"), log_channel,
                             CLOG_CONFIG_DEFAULT_KEY);
    ...
    targets.log_to_graylog_host =
        get_value_via_strmap(cct->_conf.get_val < string >
                             ("clog_to_graylog_host"), log_channel,
                             CLOG_CONFIG_DEFAULT_KEY);
    targets.log_to_graylog_port =
        get_value_via_strmap(cct->_conf.get_val < string >
                             ("clog_to_graylog_port"), log_channel,
                             CLOG_CONFIG_DEFAULT_KEY);
...
ceph::ref_t < Message > LogClient::_get_mon_log_message()
{
    ...
    ldout(cct,
        ...
        size() << " last_log " << last_log << " sent " << last_log_sent <<
        " num " << log_queue.size()
        << " unsent " << num_unsent << " sending " << num_send << dendl;
