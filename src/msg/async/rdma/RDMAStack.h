...
class RDMADispatcher {
    ...
    bool done = false;
     std::atomic < uint64_t > num_qp_conn = {
    0};
   ...
   */
    ceph::unordered_map < uint32_t, std::pair < QueuePair *,
        RDMAConnectedSocketImpl * > >qp_conns;
    ...
    std::atomic < uint64_t > num_pending_workers = {
    0};
  ...
  public:
    ...
    Infiniband::CompletionQueue * get_tx_cq()const {
        return tx_cq;
    } Infiniband::CompletionQueue * get_rx_cq() const {
        return rx_cq;
...
class RDMAWorker:public Worker {
  ...
  public:
    ...
    int get_reged_mem(RDMAConnectedSocketImpl * o, std::vector < Chunk * >&c,
                      size_t bytes);
    ...
    void set_dispatcher(std::shared_ptr < RDMADispatcher > &dispatcher) {
        this->dispatcher = dispatcher;
    }
    void set_ib(std::shared_ptr < Infiniband > &ib) {
        this->ib = ib;
    }
    void notify_worker() {
...
struct RDMACMInfo {
    RDMACMInfo(rdma_cm_id * cid, rdma_event_channel * cm_channel_,
               uint32_t qp_num_)
...
class RDMAConnectedSocketImpl:public ConnectedSocketImpl {
  ...
  protected:
    ...
    size_t tx_copy_chunk(std::vector < Chunk * >&tx_buffers,
                         size_t req_copy_len,
                         decltype(std::cbegin(pending_bl.buffers())) & start,
                         const decltype(std::
                                        cbegin(pending_bl.buffers())) & end);

  public:
     RDMAConnectedSocketImpl(CephContext * cct,
                             std::shared_ptr < Infiniband > &ib,
    ...
    uint32_t get_peer_qpn() const {
        return peer_qpn;
    } uint32_t get_local_qpn() const {
        return local_qpn;
    } Infiniband::QueuePair * get_qp() const {
        return qp;
...
class RDMAIWARPConnectedSocketImpl:public RDMAConnectedSocketImpl {
  public:
    RDMAIWARPConnectedSocketImpl(CephContext * cct,
                                 std::shared_ptr < Infiniband > &ib,
                                 std::shared_ptr < RDMADispatcher >
                                 &rdma_dispatcher, RDMAWorker * w,
                                 RDMACMInfo * info = nullptr);
    ~RDMAIWARPConnectedSocketImpl();
    virtual int try_connect(const entity_addr_t &,
...
class RDMAServerSocketImpl:public ServerSocketImpl {
  ...
  public:
    ...
    virtual int accept(ConnectedSocket * s, const SocketOptions & opts,
...
class RDMAIWARPServerSocketImpl:public RDMAServerSocketImpl {
  public:
    RDMAIWARPServerSocketImpl(CephContext * cct,
                              std::shared_ptr < Infiniband > &ib,
                              std::shared_ptr < RDMADispatcher >
                              &rdma_dispatcher, RDMAWorker * w,
...
class RDMAStack:public NetworkStack {
  ...
  public:
    ...
    virtual ~ RDMAStack();
    virtual bool nonblock_connect_need_writable_event() const override {
        return false;
