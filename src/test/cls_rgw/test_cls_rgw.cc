...
void test_stats(librados::IoCtx & ioctx, string & oid, RGWObjCategory category,
                    ...
                    bool log_op = true)
{
    ...
    cls_rgw_bucket_complete_op(op, index_op, tag, ver, key, meta, nullptr,
                               log_op, bi_flags, nullptr);
    ASSERT_EQ(0, ioctx.operate(oid, &op));
...
#define NUM_OBJS 10
    for (int i = 0; i < NUM_OBJS; i++) {
        ...
        meta.size = obj_size;
        index_complete(ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, epoch, obj,
                       meta);
    }
...
TEST_F(cls_rgw, index_remove_object)
{
    ...
    index_complete(ioctx, bucket_oid, CLS_RGW_OP_DEL, tag_remove, ++epoch, obj,
                   meta);
    ...
    index_complete(ioctx, bucket_oid, CLS_RGW_OP_ADD, tag_modify, ++epoch, obj,
                   meta);
...
TEST_F(cls_rgw, index_suggest)
{
    ...
    for (int i = num_objs / 2; i < num_objs; i++) {
        ...
        test_stats(ioctx, bucket_oid, RGWObjCategory::None, actual_num_objs,
                   total_size);

        rgw_bucket_dir_entry_meta meta;
        index_complete(ioctx, bucket_oid, CLS_RGW_OP_DEL, tag, ++epoch, obj,
                       meta);
    ...
    for (int i = 0; i < num_objs; i += 2) {
        ...
        char suggest_op =
            (i < num_objs / 2 ? CEPH_RGW_UPDATE : CEPH_RGW_REMOVE);
    ...
    int r =
        CLSRGWIssueSetTagTimeout(ioctx, bucket_objs, 8 /* max aio */ , 1) ();
    ASSERT_EQ(0, r);
    ...
    test_stats(ioctx, bucket_oid, RGWObjCategory::None, num_objs / 2,
               total_size);
}
...
TEST_F(cls_rgw, bi_list)
{
    ...
    ret =
        cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker,
                        num_objs + 10, &entries, &is_truncated);
    ASSERT_EQ(ret, 0);
    if (is_truncated) {
    ...
    const std::list < std::pair < const std::string,
        unsigned >>filters_results = { {str_int("obj", 9), 1},
    {str_int("об'єкт", 8), 1},
    {str_int("obj", 8), 0}
    };
  for (const auto & filter_result:filters_results) {
        ...
        ret =
            cls_rgw_bi_list(ioctx, bucket_oid, filter_result.first, marker, max,
                            &entries, &is_truncated);
    ...
    ret =
        cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker,
                        num_objs - 1, &entries, &is_truncated);
...
static bool cmp_objs(cls_rgw_obj & obj1, cls_rgw_obj & obj2)
{
    return (obj1.pool == obj2.pool) &&
        (obj1.key == obj2.key) && (obj1.loc == obj2.loc);
}
...
TEST_F(cls_rgw, gc_set)
{
    ...
    ASSERT_EQ(0,
              cls_rgw_gc_list(ioctx, oid, marker, 8, true, entries, &truncated,
                              next_marker));
    ...
    ASSERT_EQ(0,
              cls_rgw_gc_list(ioctx, oid, marker, 10, true, entries, &truncated,
                              next_marker));
...
TEST_F(cls_rgw, gc_list)
{
    ...
    ASSERT_EQ(0,
              cls_rgw_gc_list(ioctx, oid, marker, 8, true, entries2, &truncated,
                              next_marker));
...
TEST_F(cls_rgw, gc_defer)
{
    ...
    ASSERT_EQ(0,
              cls_rgw_gc_list(ioctx, oid, marker, 1, true, entries, &truncated,
                              next_marker));
...
auto populate_usage_log_info(std::string user, std::string payer,
...
auto gen_usage_log_info(std::string payer, std::string bucket,
                        int total_usage_entries)
{
    rgw_usage_log_info info;
    for (int i = 0; i < total_usage_entries; i++) {
...
TEST_F(cls_rgw, usage_basic)
{
    ...
    int ret =
        cls_rgw_usage_log_read(ioctx, oid, user, "", start_epoch, end_epoch,
    ...
    ASSERT_EQ(0,
              cls_rgw_usage_log_trim(ioctx, oid, user, "", start_epoch,
                                     end_epoch));
    ...
    ASSERT_EQ(0, ioctx.operate(oid, &op));
    ret =
        cls_rgw_usage_log_read(ioctx, oid, "", bucket1, start_epoch, end_epoch,
                               max_entries, read_iter, usage2, &truncated);
    ASSERT_EQ(0, ret);
    ...
    ASSERT_EQ(0,
              cls_rgw_usage_log_trim(ioctx, oid, "", bucket1, start_epoch,
                                     end_epoch));
    ...
    ASSERT_EQ(0u, usage2.size());
    ASSERT_EQ(0,
              cls_rgw_usage_log_trim(ioctx, oid, "", bucket2, start_epoch,
                                     end_epoch));
}
...
TEST_F(cls_rgw, index_racing_removes)
{
    ...
    // cancel on tag1
    index_complete(ioctx, bucket_oid, CLS_RGW_OP_CANCEL, tag1, ++epoch, obj,
                   meta);
    {
        std::map < int, rgw_cls_list_ret > results;
        list_entries(ioctx, bucket_oid, 1, results);
    ...
    index_complete(ioctx, bucket_oid, CLS_RGW_OP_CANCEL, tag3, ++epoch, obj,
                   meta);
