 ...
 */
        ...
        class C_OrderedWrite:public Context {
          public:
            ...
            void finish(int r) override {
                ldout(m_cct,
                      20) << "C_OrderedWrite completing " << m_result << dendl;
                {
                    std::lock_guard l {
                    m_wb_handler->m_lock};
                ...
                ldout(m_cct,
                      20) << "C_OrderedWrite finished " << m_result << dendl;
      ...
      :    m_tid(0), m_lock(lock), m_ictx(ictx) {
        ...
        void ObjectCacherWriteback::read(const object_t & oid,
                                         uint64_t object_no,
                                         const object_locator_t & oloc,
                                         uint64_t off, uint64_t len,
                                         snapid_t snapid, bufferlist * pbl,
            ...
            auto aio_comp =
                io::AioCompletion::create_and_start(onfinish, m_ictx,
                                                    io::AIO_TYPE_READ);
            aio_comp->read_result = io::ReadResult {
            pbl};
            ...
            auto req =
                io::ObjectDispatchSpec::create_read(m_ictx,
                                                    io::
                                                    OBJECT_DISPATCH_LAYER_CACHE,
                                                    object_no,
                                                    &req_comp->extents,
                                                    io_context, op_flags,
                                                    read_flags, trace, nullptr,
                                                    req_comp);
            req->send();
        }
        ...
        bool ObjectCacherWriteback::may_copy_on_write(const object_t & oid,
            ...
            if (raw_overlap > 0) {
                uint64_t object_no =
                    oid_to_object_no(oid.name, m_ictx->object_prefix);
                auto[parent_extents, area] =
                    io::util::object_to_area_extents(m_ictx, object_no, { {
                                                     0,
            ...
            ldout(m_ictx->cct,
                  10) << "may_copy_on_write " << oid << " " << read_off << "~"
                << read_len << " = " << may << dendl;
            return may;
        }
        ...
        ceph_tid_t ObjectCacherWriteback::write(const object_t & oid,
            ...
            ldout(m_ictx->cct,
                  20) << "write will wait for result " << result << dendl;
            ...
            auto req =
                io::ObjectDispatchSpec::create_write(m_ictx,
                                                     io::
                                                     OBJECT_DISPATCH_LAYER_CACHE,
                                                     object_no, off,
                                                     std::move(bl_copy),
            ...
            req->object_dispatch_flags =
                (io::OBJECT_DISPATCH_FLAG_FLUSH | io::
                 OBJECT_DISPATCH_FLAG_WILL_RETRY_ON_ERROR);
            req->send();

            return ++m_tid;
        }

        void ObjectCacherWriteback::overwrite_extent(const object_t & oid,
                                                     uint64_t off, uint64_t len,
                                                     ceph_tid_t
                                                     ...
                                                     ceph_tid_t new_journal_tid)
        {
            ...
            for (auto it = image_extents.begin(); it != image_extents.end();
                ...
                if (new_journal_tid != 0) {
                    ...
                    m_ictx->journal->flush_event(new_journal_tid,
                                                 new
                                                 C_CommitIOEventExtent(m_ictx,
                                                                       original_journal_tid,
                                                                       it->
                                                                       first,
                                                                       it->
                    ...
                    m_ictx->journal->
                        commit_io_event_extent(original_journal_tid, it->first,
                                               it->second, 0);
                }
            }
        }
        ...
        void ObjectCacherWriteback::complete_writes(const std::string & oid) {
            ...
            for (std::list < write_result_d * >::iterator it = finished.begin();
                ...
                ldout(m_ictx->cct,
                      20) << "complete_writes() completing " << result << dendl;
