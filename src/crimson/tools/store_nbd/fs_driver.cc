...
{
    return ghobject_t(shard_id_t::NO_SHARD,
                      0, (coll << 16), "", "", 0, ghobject_t::NO_GEN);
}
                ...
                unsigned entry_size, unsigned entries)
{
    ...
    while ((pg.log_head - pg.log_tail) > entries) {
        t.omap_rmkey(pg.collection->get_cid(),
                     pg.log_object, make_log_key(pg.log_tail));
        ++pg.log_tail;
    }
}
...
seastar::future <> FSDriver::write(off_t offset, bufferptr ptr)
{
    ...
    bufferlist bl;
    bl.append(ptr);
    t.write(mapping.pg.collection->get_cid(),
            mapping.object, mapping.offset, ptr.length(), bl, 0);
...
seastar::future < bufferlist > FSDriver::read(off_t offset, size_t size)
{
    ...
    return sharded_fs->read(mapping.pg.collection,
                            mapping.object,
                            mapping.offset,
                            size,
                            0).
...
{
    fs.reset();
    fs = FuturizedStore::create(config.get_fs_type(),
                                *config.path,
                                crimson::common::local_conf().
                                get_config_values()
        );
