 ...
 */
class tree_cursor_t final: public boost::intrusive_ref_counter <
...
public:
    ...
    eagain_ifuture < Ref < tree_cursor_t >> erase(context_t,
                                         bool get_next);

    std::strong_ordering compare_to(const tree_cursor_t &,
                                    value_magic_t) const;
...
private:
    ...
    const search_position_t &get_position() const
    {
        return position;
    } Ref < LeafNode > get_leaf_node() const
    {
        return ref_leaf_node;
    ...
    static Ref < tree_cursor_t > create_inserted(Ref < LeafNode > node,
    ...
    static Ref < tree_cursor_t > create_tracked(Ref < LeafNode > node,
            const search_position_t &
            pos, const key_view_t &key,
    ...
    public:
        ...
        } void update_all(const node_version_t &, const key_view_t &,
                          const value_header_t *);
        const key_view_t &get_key_view(value_magic_t magic,
                                       const search_position_t &pos)
        {
            make_latest(magic, pos);
            return *key_view;
        }
        const value_header_t *get_p_value_header(value_magic_t magic,
                const search_position_t &
                pos)
        {
            make_latest(magic, pos);
        ...
        std::pair < NodeExtentMutable &,
            ValueDeltaRecorder * >prepare_mutate_value_payload(context_t,
                    const
                    search_position_t
                    &);
 ...
 */
class Node: public boost::intrusive_ref_counter <
...
public:
    ...
    struct search_result_t {
        ...
        } void validate_input_key(const key_hobj_t &key,
     ...
     */
    virtual eagain_ifuture < Ref <
    tree_cursor_t >> lookup_smallest(context_t) = 0;
     ...
     */
    virtual eagain_ifuture < Ref <
    tree_cursor_t >> lookup_largest(context_t) = 0;
     ...
     */
    eagain_ifuture < search_result_t > lower_bound(context_t c,
    ...
    eagain_ifuture < std::pair < Ref < tree_cursor_t >,
                   bool >> insert(context_t, const key_hobj_t &, value_config_t,
                                  Ref < Node > &&);
     ...
     */
    eagain_ifuture < std::size_t > erase(context_t, const key_hobj_t &,
                                         Ref < Node > &&);
    ...
    static eagain_ifuture < Ref < Node >> load_root(context_t,
                                       RootNodeTracker &);

    // Only for unit test purposes.
    void test_make_destructable(context_t, NodeExtentMutable &, Super::URef
                                &&);
    virtual eagain_ifuture <> test_clone_root(context_t,
            RootNodeTracker &) const = 0;

protected:
    virtual eagain_ifuture <> test_clone_non_root(context_t,
            Ref <
            InternalNode >) const
    {
        ceph_abort("impossible path");
    } virtual eagain_ifuture < search_result_t >
    lower_bound_tracked(context_t, const key_hobj_t &, MatchHistory &) =
        0;
    virtual eagain_ifuture <> do_get_tree_stats(context_t, tree_stats_t &) =
        0;

    virtual bool is_tracking() const = 0;

    virtual void track_merge(Ref < Node >, match_stage_t,
                             search_position_t &) = 0;
...
protected:
    ...
    // as root void make_root(context_t c, Super::URef && _super);
    void make_root_new(context_t c, Super::URef && _super)
    {
        assert(_super->get_root_laddr() == L_ADDR_NULL);
        make_root(c, std::move(_super));
    }
    void make_root_from(context_t c, Super::URef
                        && _super, laddr_t from_addr)
    {
    ...
    const parent_info_t &parent_info() const
    {
        return *_parent_info;
    } Ref < InternalNode > deref_parent();

    eagain_ifuture <> apply_split_to_parent(context_t, Ref < Node > &&,
                                            Ref < Node > &&, bool);
    eagain_ifuture < Ref <
    tree_cursor_t >> get_next_cursor_from_parent(context_t);
    template < bool FORCE_MERGE =
               false > eagain_ifuture <> try_merge_adjacent(context_t, bool,
                       Ref < Node > &&);
    eagain_ifuture <> erase_node(context_t, Ref < Node > &&);
    template < bool FORCE_MERGE = false >
    eagain_ifuture <> fix_parent_index(context_t, Ref < Node > &&,
                                       bool);
...
private:
    static eagain_ifuture < Ref < Node >> load(context_t, laddr_t,
...
public:
    ...
    eagain_ifuture < Ref < tree_cursor_t >> get_next_cursor(context_t,
                                         const
                                         search_position_t
                                         &);

    eagain_ifuture <> apply_child_split(context_t, Ref < Node > &&left,
                                        Ref < Node > &&right, bool);
    ...
    template < bool VALIDATE > void do_track_child(Node &child)
    {
        ...
        assert(tracked_child_nodes.find(child_pos) ==
    ...
    void do_untrack_child(const Node &child)
    {
        ...
        [[maybe_unused]] auto removed =
            tracked_child_nodes.erase(child_pos);
        assert(removed);
    }
    ...
    eagain_ifuture < std::pair < Ref < Node >,
                   Ref < Node >>> get_child_peers(context_t,
                           const search_position_t &);
    ...
    template < bool FORCE_MERGE = false >
    eagain_ifuture <> apply_children_merge(context_t,
                                           Ref < Node > &&left, laddr_t,
                                           Ref < Node > &&right,
                                           bool update_index);
    ...
    static eagain_ifuture < Ref < InternalNode >> allocate_root(context_t,
            laddr_t,
            level_t,
            laddr_t,
            Super::URef
...
private:
    ...
    eagain_ifuture < Ref < InternalNode >> insert_or_split(context_t,
                                        const
                                        search_position_t
                                        &,
                                        const key_view_t
    ...
    eagain_ifuture < Ref < Node >> get_or_track_child(context_t,
                                const
                                search_position_t &,
                                laddr_t);
    template < bool VALIDATE =
               true > void track_insert(const search_position_t &, match_stage_t,
                                        Ref < Node >, Ref < Node > nxt_child =
                                            nullptr);
    ...
    static eagain_ifuture < fresh_node_t > allocate(context_t, laddr_t,
            field_type_t, bool,
            level_t);
...
public:
    ...
    std::tuple < key_view_t,
        const value_header_t *>get_kv(const search_position_t &) const;
    eagain_ifuture < Ref < tree_cursor_t >> get_next_cursor(context_t,
                                         const
                                         search_position_t
                                         &);
    ...
    eagain_ifuture <> extend_value(context_t, const search_position_t &,
                                   value_size_t);
    eagain_ifuture <> trim_value(context_t, const search_position_t &,
                                 value_size_t);
...
private:
    ...
    eagain_ifuture < Ref < tree_cursor_t >> insert_value(context_t,
                                         const key_hobj_t &,
                                         value_config_t,
                                         const
                                         search_position_t
                                         ...
                                         match_stat_t
                                         mstat);
    static eagain_ifuture < Ref < LeafNode >> allocate_root(context_t,
                                           RootNodeTracker
                                           &);
    friend class Node;
...
private:
    // XXX: extract a common tracker for InternalNode to track Node,
    // and LeafNode to track tree_cursor_t.
    Ref < tree_cursor_t > get_or_track_cursor(const search_position_t &,
            const key_view_t &,
            const value_header_t *);
    Ref < tree_cursor_t > track_insert(const search_position_t &,
                                       match_stage_t,
                                       const value_header_t *);
