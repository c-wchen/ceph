...
FatalSignal::FatalSignal()
{
    install_oneshot_signals_handler < SIGSEGV,
                                    SIGABRT, SIGBUS, SIGILL, SIGFPE, SIGXCPU, SIGXFSZ, SIGSYS > ();
}
...
[[gnu::noinline]]
void FatalSignal::signal_entry(const int signum, siginfo_t *const info, void *)
{
    if (static std::atomic_bool handled {
    false}; handled.exchange(true)) {
        return;
    }
    assert(info);
...
static void print_backtrace(std::string_view cause)
{
    ...
    std::cerr << boost::stacktrace::stacktrace(FRAMES_TO_SKIP,
              ...
              (-1)
              /* max depth same as the default one */
                                              );
    std::cerr << std::flush;
...
static void print_proc_maps()
{
    ...
    if (fd < 0) {
        std::cerr << "can't open /proc/self/maps. procfs not mounted?" << std::
                  endl;
        return;
    }
    ...
    while (true) {
        ...
        if (r < 0) {
            std::cerr << "error while reading /proc/self/maps: " << r << std::
                      endl;
            return;
        } else {
