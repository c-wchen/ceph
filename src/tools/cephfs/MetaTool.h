...
class MetaTool:public MDSUtility {
  ...
  private:
    class meta_op {
      ...
      meta_op(bool debug = false, std::string out = "", std::string in = "", bool confirm = false):
        _debug(debug),
            _out(out), _in(in), _confirm(confirm) {
        }
        void release();
        typedef enum {
            OP_LIST = 0,
        ...
        class sub_op {
          public:
            sub_op(meta_op * mop):trace_level(0), _proc(false), _mop(mop) {
            } void print() {
    ...
    std::string obj_name(inodeno_t ino, frag_t fg =
                         frag_t(), const char *suffix = NULL)const;
    std::string obj_name(inodeno_t ino, uint64_t offset, const char *suffix =
                         NULL) const;
    std::string obj_name(const char *ino, uint64_t offset, const char *suffix =
                         NULL) const;
    ...
    int show_child(std::string_view key,
                   ...
                   bool * force_dirty,
                   inodeno_t sp_ino = 0, meta_op * op = NULL);

    int process(std::string & mode, std::string & ino, std::string out,
                std::string in, bool confirm);
  ...
  public:
    ...
    int _amend_meta(std::string & k, inode_meta_t & i, const std::string & fn,
                    meta_op & op);
  ...
  MetaTool(bool debug = false):
    ...
    int main(std::string & mode,
             ...
             std::string & minfo,
             std::string & ino,
             std::string & out, std::string & in, bool confirm = false);
};
#endif // METATOOL_H__
