...
namespace rbd {
    namespace mirror {
        ...
        template < typename I >
            class PoolWatcher <
            I >::MirroringWatcher:public librbd::MirroringWatcher < I > {
          public:
            ...
            } void handle_mode_updated(cls::rbd::
                                       MirrorMode mirror_mode) override {
            ...
            void handle_image_updated(cls::rbd::MirrorImageState state,
                                      ...
                                      const std::
                                      string & global_image_id) override {
                bool enabled = (state == cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
                m_pool_watcher->handle_image_updated(remote_image_id,
                                                     global_image_id, enabled);
            }
      ...
      :    m_threads(threads), m_remote_io_ctx(remote_io_ctx),
            m_listener(listener),
            m_lock(librbd::util::
                   unique_lock_name("rbd::mirror::PoolWatcher", this)) {
            m_mirroring_watcher =
                new MirroringWatcher(m_remote_io_ctx, m_threads->work_queue,
                                     this);
        }
        ...
        template < typename I > bool PoolWatcher < I >::is_blacklisted()const {
        ...
        template < typename I > void PoolWatcher < I >::register_watcher() {
            ...
            Context *ctx = create_context_callback <
                PoolWatcher,
                &PoolWatcher < I >::handle_register_watcher > (this);
        ...
        template < typename I >
            ...
            Context *on_init_finish = nullptr;
            {
                ...
                if (r >= 0) {
                    ...
                    dout(0) <<
                        "detected client is blacklisted during image refresh" <<
                        dendl;
            ...
            if (retry_refresh) {
                derr << "failed to retrieve mirroring directory: " <<
                    cpp_strerror(r)
                    << dendl;
                schedule_refresh_images(10);
            }
        ...
        template < typename I > void PoolWatcher < I >::get_mirror_uuid() {
            dout(5) << dendl;
            ...
            librados::AioCompletion * aio_comp = create_rados_callback <
                PoolWatcher,
                &PoolWatcher < I >::handle_get_mirror_uuid > (this);
            int r =
        ...
        template < typename I >
            ...
            Context *on_init_finish = nullptr;
            {
                ...
                if (r >= 0) {
                    ...
                    r = librbd::cls_client::mirror_uuid_get_finish(&it,
                                                                   &m_pending_mirror_uuid);
                }
                ...
                if (m_deferred_refresh) {
                    ...
                    dout(10) << "mirror_uuid=" << m_pending_mirror_uuid <<
                        dendl;
        ...
        template < typename I >
            ...
            Mutex::Locker timer_locker(m_threads->timer_lock);
            Mutex::Locker locker(m_lock);
            if (m_shutting_down || m_refresh_in_progress
                || m_timer_ctx != nullptr) {
                if (m_refresh_in_progress && !m_deferred_refresh) {
                    dout(5) <<
                        "deferring refresh until in-flight refresh completes" <<
                        dendl;
            ...
            m_timer_ctx = m_threads->timer->add_event_after(interval,
                                                            new
        ...
        template < typename I >
            void PoolWatcher <
            I >::handle_image_updated(const std::string & remote_image_id,
                                      const std::string & global_image_id,
                                      bool enabled) {
            dout(10) << "remote_image_id=" << remote_image_id << ", " <<
                "global_image_id=" << global_image_id << ", " << "enabled=" <<
                enabled << dendl;
        ...
        template < typename I > void PoolWatcher < I >::schedule_listener() {
            ...
            if (m_shutting_down || m_image_ids_invalid
        ...
        template < typename I > void PoolWatcher < I >::notify_listener() {
            ...
            if (!removed_image_ids.empty()) {
                m_listener.handle_update(mirror_uuid, {
                                         }
