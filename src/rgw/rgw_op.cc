...
static void forward_req_info(const DoutPrefixProvider * dpp, CephContext * cct,
                             req_info & info, const std::string & bucket_name);
                                       ...
                                       optional_yield y)
{
    map < string, bufferlist >::iterator aiter =
        bucket_attrs.find(RGW_ATTR_ACL);
    ...
    if (aiter != bucket_attrs.end()) {
        ...
        std::unique_ptr < rgw::sal::User > user =
            driver->get_user(bucket_info.owner);
...
static int read_bucket_policy(const DoutPrefixProvider * dpp,
                              ...
                              map < string, bufferlist > &bucket_attrs,
                              RGWAccessControlPolicy * policy,
                              rgw_bucket & bucket, optional_yield y)
{
    ...
    int ret =
        rgw_op_get_bucket_policy_from_attr(dpp, s->cct, driver, bucket_info,
...
static int read_obj_policy(const DoutPrefixProvider * dpp,
                           ...
                           optional_yield y, bool copy_src = false)
{
    string upload_id;
    ...
    policy =
        get_iam_policy_from_attr(s->cct, bucket_attrs, bucket->get_tenant());
    ...
    if (ret == -ENOENT) {
        ...
        if (bucket_owner.compare(s->user->get_id()) != 0 &&
            ...
            auto r =
                eval_identity_or_session_policies(dpp, s->iam_user_policies,
                                                  s->env,
            ...
            if (policy) {
                ...
                r = policy->eval(s->env, *s->auth.identity,
                                 rgw::IAM::s3ListBucket, b_arn);
                if (r == Effect::Allow)
                    return -ENOENT;
                if (r == Effect::Deny)
                    return -EACCES;
            ...
            if (!s->session_policies.empty()) {
                r = eval_identity_or_session_policies(dpp, s->session_policies,
                                                      s->env,
                                                      rgw::IAM::s3ListBucket,
                                                      ARN(bucket->get_key()));
                if (r == Effect::Allow)
            ...
            if (!bucket_policy.
                verify_permission(s, *s->auth.identity, s->perm_mask,
                                  RGW_PERM_READ))
                ret = -EACCES;
            else
                ret = -ENOENT;
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int rgw_build_bucket_policies(const DoutPrefixProvider * dpp,
                              ...
                              optional_yield y)
{
    ...
    if (!bi.empty()) {
        ...
        ret =
            rgw_bucket_parse_bucket_instance(bi, &s->bucket_name,
    ...
    if (s->dialect.compare("s3") == 0) {
        ...
        if (!s->user->get_id().empty()) {
            s->user_acl =
                std::make_unique < RGWAccessControlPolicy_SWIFTAcct > (s->cct);
        }
        s->bucket_acl =
            std::make_unique < RGWAccessControlPolicy_SWIFT > (s->cct);
    }
    else {
    ...
    if (!s->src_bucket_name.empty()) {
        ...
        if (ret == 0) {
            ...
            s->local_source =
                driver->get_zone()->get_zonegroup().equals(zonegroup);
        }
    }
    ...
    s->user->get_id(), s->user->get_display_name(),};
    ...
    if (!s->bucket_name.empty()) {
         ...
         * overwritten. */
        ret =
            driver->get_bucket(dpp, s->user.get(),
                               rgw_bucket(s->bucket_tenant, s->bucket_name,
                                          s->bucket_instance_id), &s->bucket,
                               y);
        if (ret < 0) {
            if (ret != -ENOENT) {
                string bucket_log;
        ...
        if (!driver->get_zone()->get_zonegroup().
            equals(s->bucket->get_info().zonegroup)) {
            ldpp_dout(dpp,
                      0) <<
                "NOTICE: request for data in a different zonegroup (" << s->
                bucket->get_info().zonegroup << " != " << driver->get_zone()->
        ...
        s->bucket_access_conf =
            get_public_access_conf_from_attr(s->bucket->get_attrs());
    }

    /* handle user ACL only for those APIs which support it */
    if (s->user_acl) {
        std::unique_ptr < rgw::sal::User > acl_user =
            driver->get_user(acct_acl_user.uid);

        ret = acl_user->read_attrs(dpp, y);
        if (!ret) {
            ret =
                get_user_policy_from_attr(dpp, s->cct, acl_user->get_attrs(),
                                          *s->user_acl);
        }
        if (-ENOENT == ret) {
    ...
    if (!s->user->get_id().empty()
        && s->auth.identity->get_identity_type() != TYPE_ROLE) {
        try {
            ...
            if (ret == 0) {
                ...
                s->iam_user_policies.insert(s->iam_user_policies.end(),
                                            std::
                                            make_move_iterator(user_policies.
                                                               begin()),
                                            std::
                                            make_move_iterator(user_policies.
                                                               end()));
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int rgw_build_object_policies(const DoutPrefixProvider * dpp,
                              rgw::sal::Driver * driver, req_state * s,
                              bool prefetch_data, optional_yield y)
{
    ...
    if (!rgw::sal::Object::empty(s->object.get())) {
        ...
        ret =
            read_obj_policy(dpp, driver, s, s->bucket->get_info(),
                            s->bucket_attrs, s->object_acl.get(), nullptr,
                            s->iam_policy, s->bucket.get(), s->object.get(), y);
    }

    return ret;
}

static int rgw_iam_remove_objtags(const DoutPrefixProvider * dpp, req_state * s,
                                  rgw::sal::Object * object,
    ...
    if (tags != attrs.end()) {
      ...
      for (auto & tag:tagset.get_tags()) {
            if (has_existing_obj_tag) {
                vector < std::unordered_multimap < string,
...
void rgw_add_to_iam_environment(rgw::IAM::Environment & e, std::string_view key,
                                std::string_view val)
{
...
static int rgw_iam_add_tags_from_bl(req_state * s, bufferlist & bl,
                                    bool has_existing_obj_tag =
                                    false, bool has_resource_tag = false)
{
  ...
  for (const auto & tag:tagset.get_tags()) {
        if (has_existing_obj_tag)
            rgw_add_to_iam_environment(s->env,
                                       "s3:ExistingObjectTag/" + tag.first,
                                       tag.second);
        if (has_resource_tag)
            rgw_add_to_iam_environment(s->env, "s3:ResourceTag/" + tag.first,
                                       tag.second);
    }
    return 0;
}

static int rgw_iam_add_objtags(const DoutPrefixProvider * dpp, req_state * s,
                               rgw::sal::Object * object,
                               bool has_existing_obj_tag, bool has_resource_tag)
{
    ...
    auto tags = attrs.find(RGW_ATTR_TAGS);
    if (tags != attrs.end()) {
        return rgw_iam_add_tags_from_bl(s, tags->second, has_existing_obj_tag,
                                        has_resource_tag);
    }
    return 0;
}
...
{
    if (!rgw::sal::Object::empty(s->object.get())) {
        return rgw_iam_add_objtags(dpp, s, s->object.get(),
                                   has_existing_obj_tag, has_resource_tag);
    }
    return 0;
}
...
static std::tuple < bool,
    bool > rgw_check_policy_condition(const DoutPrefixProvider * dpp,
                                      boost::optional < rgw::IAM::Policy >
                                      iam_policy,
                                      boost::optional < vector <
                                      rgw::IAM::Policy >> identity_policies,
                                      boost::optional < vector <
                                      rgw::IAM::Policy >> session_policies,
                                      bool check_obj_exist_tag = true)
{
    ...
    bool identity_policy_s3_exist_tag = false, identity_policy_s3_resource_tag =
        false;
    ...
    bool session_policy_s3_exist_tag = false, session_policy_s3_resource_flag =
        false;
    if (session_policies) {
    ...
    has_existing_obj_tag = iam_policy_s3_exist_tag
        || identity_policy_s3_exist_tag || session_policy_s3_exist_tag;
    has_resource_tag = iam_policy_s3_resource_tag
        || identity_policy_s3_resource_tag || session_policy_s3_resource_flag;
    return make_tuple(has_existing_obj_tag, has_resource_tag);
}

static std::tuple < bool,
    bool > rgw_check_policy_condition(const DoutPrefixProvider * dpp,
                                      req_state * s, bool check_obj_exist_tag =
                                      true)
{
    return rgw_check_policy_condition(dpp, s->iam_policy, s->iam_user_policies,
                                      s->session_policies, check_obj_exist_tag);
}

static void rgw_add_grant_to_iam_environment(rgw::IAM::Environment & e,
                                             req_state * s)
{
...
void rgw_build_iam_environment(rgw::sal::Driver * driver, req_state * s)
{
    ...
    s->env.emplace("aws:CurrentTime",
                   std::to_string(ceph::real_clock::to_time_t(t)));
...
int RGWOp::verify_op_mask()
{
    ...
    if (!s->system_request && (required_mask & RGW_OP_TYPE_MODIFY)
        ...
        ldpp_dout(this,
                  5) << "NOTICE: modify request to a read-only zone by a "
...
int RGWGetObjTags::verify_permission(optional_yield y)
{
    auto iam_action = s->object->get_instance().empty()?
        rgw::IAM::s3GetObjectTagging : rgw::IAM::s3GetObjectVersionTagging;
...
int RGWDeleteObjTags::verify_permission(optional_yield y)
{
    if (!rgw::sal::Object::empty(s->object.get())) {
        ...
        auto[has_s3_existing_tag, has_s3_resource_tag] =
            rgw_check_policy_condition(this, s);
        if (has_s3_existing_tag || has_s3_resource_tag)
            rgw_iam_add_objtags(this, s, has_s3_existing_tag,
                                has_s3_resource_tag);
        if (!verify_object_permission(this, s, iam_action))
            return -EACCES;
    }
...
void RGWPutBucketTags::execute(optional_yield y)
{
    ...
    op_ret =
        driver->forward_request_to_master(this, s->user.get(), nullptr, in_data,
                                          nullptr, s->info, y);
    if (op_ret < 0) {
        ldpp_dout(this,
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[this, y] {
                                      ...
                                      return s->bucket->
                                      merge_and_store_attrs(this, attrs, y);}
    );
...
void RGWDeleteBucketTags::execute(optional_yield y)
{
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[this, y] {
                                      ...
                                      op_ret =
                                      s->bucket->merge_and_store_attrs(this,
                                                                       attrs,
                                                                       y);
                                      if (op_ret < 0) {
                                      ldpp_dout(this,
                                                0) <<
                                      "RGWDeleteBucketTags() failed to remove RGW_ATTR_TAGS on bucket="
                                      << s->bucket->get_name()
...
int RGWPutBucketReplication::verify_permission(optional_yield y)
{
    ...
    if (has_s3_resource_tag)
        rgw_iam_add_buckettags(this, s);
    return verify_bucket_owner_or_policy(s,
                                         rgw::IAM::
...
void RGWPutBucketReplication::execute(optional_yield y)
{
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[this] {
      auto sync_policy = (s->bucket->get_info().sync_policy ? *s->bucket->get_info().sync_policy : rgw_sync_policy_info()); for (auto & group:sync_policy_groups)
                                      {
                                      ...
                                      int ret =
                                      s->bucket->put_info(this, false,
                                                          real_time());
                                      if (ret < 0) {
                                      ldpp_dout(this,
                                                0) <<
                                      "ERROR: put_bucket_instance_info (bucket="
                                      << s->
                                      bucket << ") returned ret=" << ret <<
                                      dendl; return ret;}
...
int RGWOp::do_aws4_auth_completion()
{
    ...
    if (s->auth.completer) {
        if (!s->auth.completer->complete()) {
            ...
            ldpp_dout(this,
                      10) << "v4 auth ok -- do_aws4_auth_completion" << dendl;
        }
...
static bool validate_cors_rule_method(const DoutPrefixProvider * dpp,
                                      RGWCORSRule * rule, const char *req_meth)
{
    ...
    if (rule->get_allowed_methods() & flags) {
        ...
        ldpp_dout(dpp,
                  5) << "Method " << req_meth << " is not supported" << dendl;
        return false;
    }
...
int RGWOp::read_bucket_cors()
{
    ...
    map < string, bufferlist >::iterator aiter =
        s->bucket_attrs.find(RGW_ATTR_CORS);
    ...
    try {
        ...
        ldpp_dout(this,
                  0) << "ERROR: could not decode CORS, caught buffer::error" <<
            dendl;
        return -EIO;
    }
 ...
 * */
static void get_cors_response_headers(const DoutPrefixProvider * dpp,
                                      RGWCORSRule * rule, const char *req_hdrs,
 ...
 */
bool RGWOp::generate_cors_headers(string & origin, string & method,
                                  string & headers, string & exp_headers,
                                  unsigned *max_age)
{
    /* CORS 6.2.1. */
    ...
    if (!cors_exist) {
        ldpp_dout(this,
                  2) << "No CORS configuration set yet for this bucket" <<
            dendl;
        return false;
    }
    ...
    /* CORS 6.2.6. */
    get_cors_response_headers(this, rule, req_hdrs, headers, exp_headers,
                              max_age);

    return true;
}

int rgw_policy_from_attrset(const DoutPrefixProvider * dpp, CephContext * cct,
...
int RGWGetObj::read_user_manifest_part(rgw::sal::Bucket * bucket,
                                       ...
                                       const bucket_acl,
                                       const boost::optional < Policy >
                                       &bucket_policy, const off_t start_ofs,
                                       const off_t end_ofs, bool swift_slo)
{
    ...
    op_ret =
        rgw_compression_info_from_attrset(part->get_attrs(), need_decompress,
                                          cs_info);
    if (op_ret < 0) {
        ldpp_dout(this,
    ...
    if (need_decompress) {
        if (cs_info.orig_size != ent.meta.accounted_size) {
            ...
            ldpp_dout(this,
                ...
                orig_size << ", actual read size=" << ent.meta.size << dendl;
            return -EIO;
        }
        ...
        if (part->get_obj_size() != ent.meta.size) {
            // hmm.. something wrong, object not as expected, abort!
            ldpp_dout(this,
                      0) << "ERROR: expected obj_size=" << part->get_obj_size()
    ...
    if (s->system_request) {
        ldpp_dout(this,
                  2) << "overriding permissions due to system operation" <<
            dendl;
    }
    else if (s->auth.identity->is_admin_of(s->user->get_id())) {
        ldpp_dout(this,
                  2) << "overriding permissions due to admin operation" <<
            dendl;
    }
    else if (!verify_object_permission
             (this, s, part->get_obj(), s->user_acl.get(), bucket_acl,
              &obj_policy, bucket_policy, s->iam_user_policies,
    ...
    do {
      ...
      for (rgw_bucket_dir_entry & ent:results.objs) {
            ...
            if (found_start && !handled_end) {
                ...
                if (cb) {
                    r = cb(bucket, ent, bucket_acl, bucket_policy, start_ofs,
                           end_ofs, cb_param, false /* swift_slo */ );
                    if (r < 0) {
                        return r;
                    }
                }
...
static int iterate_slo_parts(const DoutPrefixProvider * dpp,
                             ...
                             int (*cb) (rgw::sal::Bucket * bucket,
                                        ...
                                        const boost::optional < Policy >
                                        &bucket_policy, off_t start_ofs,
                                        off_t end_ofs, void *param,
                                        bool swift_slo), void *cb_param)
{
        ...
        if (found_start) {
            if (cb) {
                ...
                int r = cb(part.bucket, ent, part.bucket_acl,
                           (part.bucket_policy ?
                            boost::optional < Policy >
                            (*part.bucket_policy) : none),
...
static int get_obj_user_manifest_iterate_cb(rgw::sal::Bucket * bucket,
                                            ...
                                            const off_t end_ofs,
                                            void *const param, bool swift_slo =
                                            false)
{
    RGWGetObj *op = static_cast < RGWGetObj * >(param);
    return op->read_user_manifest_part(bucket, ent, bucket_acl, bucket_policy,
                                       start_ofs, end_ofs, swift_slo);
}
...
int RGWGetObj::handle_user_manifest(const char *prefix, optional_yield y)
{
    ...
    if (bucket_name.compare(s->bucket->get_name()) != 0) {
        map < string, bufferlist > bucket_attrs;
        r = driver->get_bucket(this, s->user.get(), s->user->get_tenant(),
        ...
        r = read_bucket_policy(this, driver, s, ubucket->get_info(),
                               bucket_attrs, bucket_acl, ubucket->get_key(), y);
        if (r < 0) {
            ldpp_dout(this, 0) << "failed to read bucket policy" << dendl;
            return r;
        }
        _bucket_policy =
            get_iam_policy_from_attr(s->cct, bucket_attrs,
                                     s->user->get_tenant());
    ...
    r = iterate_user_manifest_parts(this, s->cct, driver, ofs, end,
                                    pbucket, obj_prefix, bucket_acl,
                                    *bucket_policy, nullptr, &s->obj_size,
                                    &lo_etag, nullptr /* cb */ ,
                                    nullptr /* cb arg */ , y);
    if (r < 0) {
        return r;
    ...
    r = iterate_user_manifest_parts(this, s->cct, driver, ofs, end,
                                    pbucket, obj_prefix, bucket_acl,
                                    *bucket_policy, &total_len, nullptr,
...
int RGWGetObj::handle_slo_manifest(bufferlist & bl, optional_yield y)
{
    ...
    map < string, pair < RGWAccessControlPolicy *,
        boost::optional < Policy >>> policies;
  ...
  for (const auto & entry:slo_info.entries) {
        ...
        if (bucket_name.compare(s->bucket->get_name()) != 0) {
            ...
            if (piter != policies.end()) {
                ...
                int r =
                    driver->get_bucket(this, s->user.get(),
                                       s->user->get_tenant(), bucket_name,
                                       &tmp_bucket, y);
                if (r < 0) {
                    ldpp_dout(this,
                ...
                bucket_acl = &_bucket_acl;
                r = read_bucket_policy(this, driver, s, tmp_bucket->get_info(),
                                       tmp_bucket->get_attrs(), bucket_acl,
                ...
                auto _bucket_policy =
                    get_iam_policy_from_attr(s->cct, tmp_bucket->get_attrs(),
                                             tmp_bucket->get_tenant());
        ...
        ldpp_dout(this, 20) << "slo_part: bucket=" << part.bucket
            << " obj=" << part.obj_name
            << " size=" << part.size << " etag=" << part.etag << dendl;
...
int RGWGetObj::get_lua_filter(std::unique_ptr < RGWGetObj_Filter > *filter,
                              RGWGetObj_Filter * cb)
{
    std::string script;
    const auto rc =
        rgw::lua::read_script(s, s->penv.lua.manager.get(), s->bucket_tenant,
                              s->yield, rgw::lua::context::getData, script);
    if (rc == -ENOENT) {
        ...
        ldpp_dout(this,
                  5) << "WARNING: failed to read data script. error: " << rc <<
            dendl;
        return rc;
    }
...
static inline void rgw_cond_decode_objtags(req_state * s,
                                           ...
                                           buffer::list > &attrs)
{
    ...
    if (tags != attrs.end()) {
        try {
            ...
            ldpp_dout(s, 0)
                << "ERROR: caught buffer::error, couldn't decode TagSet" <<
                dendl;
        }
    }
}
...
void RGWGetObj::execute(optional_yield y)
{
    ...
    std::unique_ptr < rgw::sal::Object::ReadOp >
        read_op(s->object->get_read_op());
    ...
    if (torrent.get_flag()) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      0) << "ERROR: failed to send_response_data ret= " <<
                op_ret << dendl;
            goto done_err;
        }
        return;
    ...
    if (s->penv.flight_store) {
        ...
        ldpp_dout(this,
                  0) << "ERROR: flight_store not created in " << __func__ <<
            dendl;
    }
#endif
        ...
        try {
          ...
          for (const auto & zone:zones) {
                if (zone == dst_zone_trace) {
                    ...
                    ldpp_dout(this,
                              4) <<
                        "Object already has been copied to this destination. Returning "
                        << op_ret << dendl;
                    goto done_err;
                }
            }
    ...
    if (get_type() == RGW_OP_GET_OBJ && get_data) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      4) << "Cannot get cloud tiered object: " << *s->
                object << ". Failing with " << op_ret << dendl;
    ...
    if (attr_iter != attrs.end() && !skip_manifest) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      0) << "ERROR: failed to handle slo manifest ret=" <<
                op_ret << dendl;
            goto done_err;
        }
        return;
...
void RGWListBuckets::execute(optional_yield y)
{
    ...
    do {
        ...
        op_ret =
            s->user->list_buckets(this, marker, end_marker, read_count,
                                  should_get_stats(), buckets, y);
        ...
        if (driver->get_zone()->get_zonegroup().
            get_placement_target_names(targets)) {
          for (const auto & policy:targets) {
                policies_stats.emplace(policy,
                                       decltype(policies_stats)::mapped_type());
            }
        }

        std::map < std::string, std::unique_ptr < rgw::sal::Bucket >> &m =
            buckets.get_buckets();
      for (const auto & kv:m) {
             ...
             * for first time. */
            auto & policy_stats =
                policies_stats[bucket->get_placement_rule().to_str()];
        ...
        total_count += m.size();
                ...
                || (limit >= 0 && total_count >= (uint64_t) limit));
...
void RGWGetUsage::execute(optional_yield y)
{
    ...
    while (s->bucket && is_truncated) {
        op_ret =
            s->bucket->read_usage(this, start_epoch, end_epoch, max_entries,
    ...
    op_ret =
        rgw_user_get_all_buckets_stats(this, driver, s->user.get(),
                                       buckets_usage, y);
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "ERROR: failed to get user's buckets stats" << dendl;
        return;
    }
...
void RGWStatAccount::execute(optional_yield y)
{
    ...
    do {
        ...
        lastmarker = nullptr;
        op_ret =
            s->user->list_buckets(this, marker, string(), max_buckets, true,
                                  buckets, y);
        if (op_ret < 0) {
            /* hmm.. something wrong here.. the user was authenticated, so it
               should exist */
            ...
            driver->get_zone()->get_zonegroup().
                get_placement_target_names(names);
        ...
        if (!lastmarker) {
            ldpp_dout(this,
                      -1) << "ERROR: rgw_read_user_buckets, stasis at marker="
...
void RGWSetBucketVersioning::execute(optional_yield y)
{
    ...
    if (cur_mfa_status) {
        ...
        if (versioning_status == VersioningEnabled) {
            req_versioning_status =
                (s->bucket->get_info().flags & BUCKET_VERSIONS_SUSPENDED) != 0;
        }
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[&] {
                                      ...
                                      return s->bucket->put_info(this, false,
                                                                 real_time());}
    ) ;
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "NOTICE: put_bucket_info on bucket=" << s->bucket->
            get_name()
            << " returned err=" << op_ret << dendl;
        return;
    }
...
void RGWDeleteBucketWebsite::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "NOTICE: forward_to_master failed on bucket=" << s->
            bucket->get_name()
            << "returned err=" << op_ret << dendl;
        return;
    }
...
void RGWStatBucket::execute(optional_yield y)
{
    ...
    op_ret =
        driver->get_bucket(this, s->user.get(), s->bucket->get_key(), &bucket,
                           y);
    if (op_ret) {
        return;
    }
...
int RGWListBucket::parse_max_keys()
{
    ...
    return parse_value_and_bound(max_keys, max, 0,
                                 ...
                                 ("rgw_max_listing_results"), default_max);
}
...
int RGWCreateBucket::verify_permission(optional_yield y)
{
    ...
    if (s->user->get_max_buckets()) {
        ...
        string marker;
        op_ret =
            s->user->list_buckets(this, marker, string(),
                                  s->user->get_max_buckets(), false, buckets,
                                  y);
        if (op_ret < 0) {
            return op_ret;
        }
...
static int filter_out_quota_info(std::map < std::string,
                                 bufferlist > &add_attrs,
                                 const std::set < std::string > &rmattr_names,
                                 RGWQuotaInfo & quota, bool * quota_extracted =
                                 nullptr)
{
    ...
    if (std::end(add_attrs) != iter) {
        quota.max_objects =
            static_cast < int64_t >
            (strict_strtoll(iter->second.c_str(), 10, &err));
        if (!err.empty()) {
            return -EINVAL;
        }
...
static void filter_out_website(std::map < std::string,
                               ceph::bufferlist > &add_attrs,
                               const std::set < std::string > &rmattr_names,
                               RGWBucketWebsiteConf & ws_conf)
{
    ...
    const auto mapping = {
        ...
        std::make_pair(RGW_ATTR_WEB_LIST_CSS,
                       std::ref(ws_conf.listing_css_doc)),
...
void RGWCreateBucket::execute(optional_yield y)
{
    ...
    if (!relaxed_region_enforcement
        && !driver->get_zone()->get_zonegroup().is_master_zonegroup()
        && !location_constraint.empty()
        && driver->get_zone()->get_zonegroup().get_api_name() !=
        location_constraint) {
        ldpp_dout(this,
                  0) << "location constraint (" << location_constraint << ")" <<
            " doesn't match zonegroup" << " (" << driver->get_zone()->
            get_zonegroup().get_api_name() << ")" << dendl;
        op_ret = -ERR_INVALID_LOCATION_CONSTRAINT;
        s->err.message = "The specified location-constraint is not valid";
        return;
    }
    ...
    ldpp_dout(this,
              10) << "user=" << s->user << " bucket=" << tmp_bucket << dendl;
    op_ret =
        s->user->create_bucket(this, tmp_bucket, zonegroup_id, placement_rule,
                               info.swift_ver_location, pquota_info, policy,
                               attrs, info, ep_objv, true, obj_lock_enabled,
                               &s->bucket_exists, s->info, &s->bucket, y);

    /* continue if EEXIST and create_bucket will fail below.  this way we can
     * recover from a partial create by retrying it. */
    ldpp_dout(this,
              20) << "rgw_create_bucket returned ret=" << op_ret << " bucket="
        << s->bucket.get() << dendl;
    ...
    if (need_metadata_upload() && existed) {
        ...
        do {
            ...
            op_ret =
                rgw_get_request_metadata(this, s->cct, s->info, attrs, false);
            if (op_ret < 0) {
                return;
            }
            prepare_add_del_attrs(s->bucket_attrs, rmattr_names, attrs);
            populate_with_generic_attrs(s, attrs);
            op_ret =
                filter_out_quota_info(attrs, rmattr_names,
...
void RGWDeleteBucket::execute(optional_yield y)
{
    ...
    bufferlist in_data;
    op_ret =
        driver->forward_request_to_master(this, s->user.get(), &ot.read_version,
...
int RGWPutObj::init_processing(optional_yield y)
{
    ...
    if (!cs_view.empty()) {
        ...
        copy_source_object_name =
            copy_source_bucket_name.substr(pos + 1,
                                           copy_source_bucket_name.size());
...
#define VERSION_ID_STR "?versionId="
        ...
        if (pos == std::string::npos) {
            ...
            copy_source_version_id =
                copy_source_object_name.substr(pos + sizeof(VERSION_ID_STR) -
                                               1);
            copy_source_object_name =
        ...
        ret =
            driver->get_bucket(this, s->user.get(), copy_source_tenant_name,
                               copy_source_bucket_name, &bucket, y);
        if (ret < 0) {
            ldpp_dout(this,
                      5) << __func__ << "(): get_bucket() returned ret=" << ret
                << dendl;
            if (ret == -ENOENT) {
                ret = -ERR_NO_SUCH_BUCKET;
            }
        ...
        if (ret < 0) {
            ldpp_dout(this,
                      5) << __func__ << "(): load_bucket() returned ret=" << ret
                << dendl;
            return ret;
        }
        ...
        if (copy_source_range) {
            ...
            if (first.find_first_not_of("0123456789") != std::string::npos ||
                ...
                ldpp_dout(this,
                          5) <<
                    "x-amz-copy-source-range bad format not an integer" <<
                    dendl;
                ret = -EINVAL;
                return ret;
            }
            ...
            if (copy_source_range_fst > copy_source_range_lst) {
                ...
                ldpp_dout(this,
                          5) <<
                    "x-amz-copy-source-range bad format first number bigger than second"
                    << dendl;
                return ret;
            }
        }
...
int RGWPutObj::verify_permission(optional_yield y)
{
    if (!copy_source.empty()) {
        ...
        std::unique_ptr < rgw::sal::Object > cs_object =
            ...
            get_object(rgw_obj_key
                       (copy_source_object_name, copy_source_version_id));
        ...
        if (ret =
            read_obj_policy(this, driver, s, copy_source_bucket_info, cs_attrs,
                            &cs_acl, nullptr, policy, cs_bucket.get(),
                            cs_object.get(), y, true); ret < 0) {
            return ret;
        }
            ...
            if (policy || !s->iam_user_policies.empty()
                ...
                auto[has_s3_existing_tag, has_s3_resource_tag] =
                    rgw_check_policy_condition(this, policy,
                                               ...
                                               s->session_policies);
                if (has_s3_existing_tag || has_s3_resource_tag)
                    rgw_iam_add_objtags(this, s, cs_object.get(),
                                        has_s3_existing_tag,
                                        has_s3_resource_tag);
              ...
              for (auto & user_policy:s->iam_user_policies) {
                    if (usr_policy_res =
                        user_policy.eval(s->env, *s->auth.identity,
                                         cs_object->get_instance().
                                         empty()? rgw::IAM::s3GetObject : rgw::
                                         IAM::s3GetObjectVersion, obj_arn);
                ...
                if (policy) {
                    ...
                    e = policy->eval(s->env, *s->auth.identity,
                                     ...
                                     rgw::IAM::s3GetObject :
                                     rgw::IAM::s3GetObjectVersion, obj_arn);
                }
                if (e == Effect::Deny) {
                    return -EACCES;
                }
                else if (usr_policy_res == Effect::Pass && e == Effect::Pass &&
                         !cs_acl.verify_permission(this, *s->auth.identity,
                                                   s->perm_mask,
                                                   RGW_PERM_READ)) {
                    return -EACCES;
                }
                rgw_iam_remove_objtags(this, s, cs_object.get(),
                     ...
                     verify_permission(this, *s->auth.identity, s->perm_mask,
                                       RGW_PERM_READ)) {
                return -EACCES;
            }
        }
    ...
    if (s->iam_policy || !s->iam_user_policies.empty()
        ...
        auto identity_policy_res =
            eval_identity_or_session_policies(this, s->iam_user_policies,
                                              s->env,
                                              rgw::IAM::s3PutObject,
                                              s->object->get_obj());
        ...
        if (!s->session_policies.empty()) {
            auto session_policy_res =
                eval_identity_or_session_policies(this, s->session_policies,
                                                  s->env,
                                                  rgw::IAM::s3PutObject,
                                                  s->object->get_obj());
            ...
            if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
                ...
                if ((session_policy_res == Effect::Allow
                    ...
                    || (session_policy_res == Effect::Allow
                        && e == Effect::Allow))
                    return 0;
            }
...
int RGWPutObj::get_data(const off_t fst, const off_t lst, bufferlist & bl)
{
    ...
    bool need_decompress;
    op_ret =
        rgw_compression_info_from_attrset(obj->get_attrs(), need_decompress,
                                          cs_info);
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "ERROR: failed to decode compression info" << dendl;
                                           ...
                                           const std::string & compression_type)
{
    ...
    const auto alg =
        std::hash < std::string > { } (upload_id) % Compressor::COMP_ALG_LAST;
...
int RGWPutObj::get_lua_filter(std::unique_ptr < rgw::sal::DataProcessor >
                              *filter, rgw::sal::DataProcessor * cb)
{
    std::string script;
    const auto rc =
        rgw::lua::read_script(s, s->penv.lua.manager.get(), s->bucket_tenant,
                              s->yield, rgw::lua::context::putData, script);
    if (rc == -ENOENT) {
        // no script, nothing to do
        return 0;
    }
...
void RGWPutObj::execute(optional_yield y)
{
    ...
    if (supplied_md5_b64) {
        ...
        buf_to_hex((const unsigned char *)supplied_md5_bin,
                   CEPH_CRYPTO_MD5_DIGESTSIZE, supplied_md5);
    ...
    if (!chunked_upload) {      /* with chunked upload we don't know how big is the upload.
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      20) << "check_quota() returned ret=" << op_ret << dendl;
            return;
        }
    }
    ...
    if (multipart) {
        ...
        multipart_trace =
            tracing::rgw::tracer.add_span(name(), upload->get_trace());

        if (op_ret < 0) {
            if (op_ret != -ENOENT) {
                ldpp_dout(this,
                          0) << "ERROR: get_multipart_info returned " << op_ret
                    << ": " << cpp_strerror(-op_ret) << dendl;
            }
            else {              // -ENOENT: raced with upload complete/cancel, no need to spam log
                ldpp_dout(this,
                          20) << "failed to get multipart info (returned " <<
        ...
        ldpp_dout(this,
                  20) << "dest_placement for part=" << *pdest_placement <<
            dendl;
        processor =
            upload->get_writer(this, s->yield, s->object.get(),
    ...
    if ((!copy_source.empty()) && !copy_source_range) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      0) << "ERROR: failed to get bucket with error" << op_ret
                << dendl;
            return;
        }
        std::unique_ptr < rgw::sal::Object > obj =
        ...
        if (astate->get_attr(RGW_ATTR_MANIFEST, bl)) {
            ...
            try {
                ...
                if (m.get_tier_type() == "cloud-s3") {
                    ...
                    ldpp_dout(this,
                              4) <<
                        "Cannot copy cloud tiered object. Failing with " <<
                        op_ret << dendl;
                    return;
                }
            }
                ...
                ldpp_dout(this,
                          1) << "WARNING: failed to decode object manifest for "
    ...
    do {
        ...
        if (copy_source.empty()) {
            ...
            off_t cur_lst =
                min < off_t > (fst + s->cct->_conf->rgw_max_chunk_size - 1,
                               lst);
        ...
        if (len < 0) {
            op_ret = len;
            ldpp_dout(this,
                      20) << "get_data() returned ret=" << op_ret << dendl;
            return;
        }
        else if (len == 0) {
            break;
    ...
    if (dlo_manifest) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this,
                      0) << "bad user manifest: " << dlo_manifest << dendl;
            return;
        }
    }
    ...
    int ret =
        res->publish_commit(this, s->obj_size, mtime, etag,
                            s->object->get_instance());
    if (ret < 0) {
        ldpp_dout(this,
                  1) << "ERROR: publishing notification failed, with error: " <<
            ret << dendl;
...
void RGWPostObj::execute(optional_yield y)
{
    ...
    do {
        ...
        if (supplied_md5_b64) {
            char supplied_md5_bin[CEPH_CRYPTO_MD5_DIGESTSIZE + 1];
            ldpp_dout(this,
                      15) << "supplied_md5_b64=" << supplied_md5_b64 << dendl;
            op_ret =
                ceph_unarmor(supplied_md5_bin,
                             &supplied_md5_bin[CEPH_CRYPTO_MD5_DIGESTSIZE + 1],
                             supplied_md5_b64,
        ...
        op_ret =
            processor->complete(s->obj_size, etag, nullptr, real_time(), attrs,
                                (delete_at ? *delete_at : real_time()), nullptr,
                                nullptr, nullptr, nullptr, nullptr, s->yield);
        if (op_ret < 0) {
            return;
        }
    ...
    // send request to notification manager
    int ret =
        res->publish_commit(this, ofs, s->object->get_mtime(), etag,
                            s->object->get_instance());
    if (ret < 0) {
        ldpp_dout(this,
                  1) << "ERROR: publishing notification failed, with error: " <<
...
void RGWPutMetadataAccount::filter_out_temp_url(map < string,
                                                bufferlist > &add_attrs,
                                                const set < string >
                                                &rmattr_names, map < int,
                                                string > &temp_url_keys)
{
...
void RGWPutMetadataAccount::execute(optional_yield y)
{
    ...
    if (!temp_url_keys.empty()) {
      for (auto & pair:temp_url_keys) {
            s->user->get_info().temp_url_keys[pair.first] =
...
void RGWPutMetadataBucket::execute(optional_yield y)
{
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[this] {
                                      ...
                                      auto new_policy =
                                      static_cast <
                                      RGWAccessControlPolicy_SWIFT * >(&policy);
                                      new_policy->filter_merge(policy_rw_mask,
                                                               old_policy);
                                      policy = *new_policy;}
                                      buffer::list bl; policy.encode(bl);
                                      ...
                                      if (swift_ver_location) {
                                      s->bucket->get_info().swift_ver_location =
                                      *swift_ver_location;
                                      s->bucket->get_info().swift_versioning =
                                      (!swift_ver_location->empty());}
...
int RGWDeleteObj::handle_slo_manifest(bufferlist & bl, optional_yield y)
{
    ...
    try {
        deleter =
            std::unique_ptr < RGWBulkDelete::Deleter >
  ...
  for (const auto & iter:slo_info.entries) {
        ...
        path.bucket_name =
            url_decode(path_str.substr(pos_init, sep_pos - pos_init));
...
int RGWDeleteObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty()
        ...
        if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
            ...
            if (r == Effect::Deny) {
                ...
                r = eval_identity_or_session_policies(this, s->session_policies,
                                                      s->env,
                                                      rgw::IAM::
                                                      s3BypassGovernanceRetention,
                                                      ARN(s->bucket->get_key(),
    ...
    if (s->bucket->get_info().mfa_enabled() &&
        ...
        ldpp_dout(this,
                  5) <<
            "NOTICE: object delete request with a versioned object, mfa auth not provided"
            << dendl;
...
void RGWDeleteObj::execute(optional_yield y)
{
    ...
    if (!rgw::sal::Object::empty(s->object.get())) {
        ...
        std::string etag;
        {
            ...
            if (check_obj_lock) {
                ...
                int object_lock_response =
                    verify_object_lock(this, astate->attrset, bypass_perm,
            ...
            if (multipart_delete) {
                ...
                const auto slo_attr =
                    astate->attrset.find(RGW_ATTR_SLO_MANIFEST);
                ...
                if (slo_attr != astate->attrset.end()) {
                    ...
                    if (op_ret < 0) {
                        ldpp_dout(this,
                                  0) <<
                            "ERROR: failed to handle slo manifest ret=" <<
                            op_ret << dendl;
                    }
                }
                else {
        ...
        // send request to notification manager
        int ret =
            res->publish_commit(this, obj_size, ceph::real_clock::now(), etag,
                                version_id);
        if (ret < 0) {
            ldpp_dout(this,
                      1) <<
...
int RGWCopyObj::verify_permission(optional_yield y)
{
    ...
    if (s->local_source && source_zone.empty()) {
        ...
        /* check source object permissions */
        op_ret =
            read_obj_policy(this, driver, s, src_bucket->get_info(),
                            src_bucket->get_attrs(), &src_acl,
                            &src_placement.storage_class, src_policy,
                            src_bucket.get(), s->src_object.get(), y);
        if (op_ret < 0) {
            return op_ret;
        }
            ...
            if (src_policy || !s->iam_user_policies.empty()
                ...
                auto[has_s3_existing_tag, has_s3_resource_tag] =
                    rgw_check_policy_condition(this, src_policy,
                                               ...
                                               s->session_policies);
                if (has_s3_existing_tag || has_s3_resource_tag)
                    rgw_iam_add_objtags(this, s, s->src_object.get(),
                                        has_s3_existing_tag,
                                        has_s3_resource_tag);

                ARN obj_arn(s->src_object->get_obj());
                auto identity_policy_res =
                    eval_identity_or_session_policies(this,
                                                      s->iam_user_policies,
                                                      s->env,
                                                      s->src_object->
                                                      get_instance().
                ...
                auto e = Effect::Pass;
                rgw::IAM::PolicyPrincipal princ_type =
                ...
                if (identity_policy_res == Effect::Pass && e == Effect::Pass &&
                    !src_acl.verify_permission(this, *s->auth.identity,
    ...
    auto dest_iam_policy =
        get_iam_policy_from_attr(s->cct, s->bucket->get_attrs(),
                                 s->bucket->get_tenant());
    /* admin request overrides permission checks */
    if (!s->auth.identity->is_admin_of(dest_policy.get_owner().get_id())) {
        if (dest_iam_policy != boost::none || !s->iam_user_policies.empty()
            ...
            rgw_add_to_iam_environment(s->env, "s3:x-amz-copy-source",
                                       copy_source);
            if (md_directive)
                rgw_add_to_iam_environment(s->env,
...
void RGWCopyObj::execute(optional_yield y)
{
    ...
    // make reservation for notification if needed
    std::unique_ptr < rgw::sal::Notification > res
        = driver->get_notification(s->object.get(), s->src_object.get(),
    ...
    uint64_t obj_size = 0;
    {
        ...
        if (!s->system_request) {   // no quota enforcement for system requests
            if (astate->accounted_size > static_cast < size_t >
                (s->cct->_conf->rgw_max_put_size)) {
                op_ret = -ERR_TOO_LARGE;
                return;
            }
            // enforce quota against the destination bucket owner
            op_ret =
                s->bucket->check_quota(this, quota, astate->accounted_size, y);
            if (op_ret < 0) {
                return;
            }
        }
    ...
    op_ret = s->src_object->copy_object(s->user.get(), &s->info, source_zone, s->object.get(), s->bucket.get(), src_bucket.get(), s->dest_placement, &src_mtime, &mtime, mod_ptr, unmod_ptr, high_precision_time, if_match, if_nomatch, attrs_mod, copy_if_newer, attrs, RGWObjCategory::Main, olh_epoch, delete_at, (version_id.empty()? NULL : &version_id), &s->req_id,  /* use req_id as tag */
                                        &etag,
    ...
    // send request to notification manager
    int ret =
        res->publish_commit(this, obj_size, mtime, etag,
                            s->object->get_instance());
    if (ret < 0) {
        ldpp_dout(this,
                  1) << "ERROR: publishing notification failed, with error: " <<
...
int RGWGetACLs::verify_permission(optional_yield y)
{
    ...
    if (!rgw::sal::Object::empty(s->object.get())) {
        auto iam_action = s->object->get_instance().empty()?
            rgw::IAM::s3GetObjectAcl : rgw::IAM::s3GetObjectVersionAcl;
        if (has_s3_existing_tag || has_s3_resource_tag)
            rgw_iam_add_objtags(this, s, has_s3_existing_tag,
                                has_s3_resource_tag);
...
void RGWGetACLs::execute(optional_yield y)
{
    ...
    RGWAccessControlPolicy *const acl =
        (!rgw::sal::Object::empty(s->object.get())? s->object_acl.get() : s->
         bucket_acl.get());
...
void RGWPutACLs::execute(optional_yield y)
{
    ...
    RGWAccessControlPolicy *const existing_policy =
        (rgw::sal::Object::empty(s->object.get())? s->bucket_acl.get() : s->
         object_acl.get());
    ...
    const multimap < string, ACLGrant > &req_grant_map =
        req_acl.get_grant_map();
#define ACL_GRANTS_MAX_NUM      100
    ...
    if ((rgw::sal::Object::empty(s->object.get()))) {
        ...
        op_ret =
            driver->forward_request_to_master(this, s->user.get(), nullptr,
                                              in_data, nullptr, s->info, y);
        if (op_ret < 0) {
            ldpp_dout(this,
                      0) << "forward_request_to_master returned ret=" << op_ret
...
void RGWPutLC::execute(optional_yield y)
{
    ...
    if (content_md5_bin) {
        ...
        data_hash.Update(reinterpret_cast < const unsigned char *>(buf),
                         data.length());
        ...
        if (memcmp
            ...
            ldpp_dout(this,
                      5) << s->err.
                message << " Specified content md5: " << content_md5 <<
    ...
    if (s->cct->_conf->subsys.should_gather < ceph_subsys_rgw, 15 > ()) {
        ...
        xf.flush(ss);
        ldpp_dout(this,
                  15) << "New LifecycleConfiguration:" << ss.str() << dendl;
    }

    op_ret =
        driver->forward_request_to_master(this, s->user.get(), nullptr, data,
                                          nullptr, s->info, y);
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "forward_request_to_master returned ret=" << op_ret <<
    ...
    op_ret =
        driver->get_rgwlc()->set_bucket_config(s->bucket.get(), s->bucket_attrs,
                                               &new_config);
    if (op_ret < 0) {
        return;
    }
...
void RGWDeleteLC::execute(optional_yield y)
{
    ...
    op_ret =
        driver->get_rgwlc()->remove_bucket_config(s->bucket.get(),
                                                  s->bucket_attrs);
    if (op_ret < 0) {
        return;
    }
...
void RGWDeleteCORS::execute(optional_yield y)
{
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[this] {
                                      op_ret = read_bucket_cors();
                                      if (op_ret < 0)
                                      return op_ret; if (!cors_exist) {
                                      ldpp_dout(this,
                                                2) <<
                                      "No CORS configuration set yet for this bucket"
                                      << dendl; op_ret = -ENOENT;
                                      return op_ret;}
...
int RGWOptionsCORS::validate_cors_request(RGWCORSConfiguration * cc)
{
    ...
    if (!rule) {
        ldpp_dout(this,
                  10) << "There is no cors rule present for " << origin <<
            dendl;
        return -ENOENT;
    }
...
void RGWOptionsCORS::execute(optional_yield y)
{
    ...
    if (!req_meth) {
        ldpp_dout(this,
                  0) << "Missing mandatory Access-control-request-method header"
            << dendl;
        op_ret = -EINVAL;
        return;
    }
...
{
    multipart_trace =
        tracing::rgw::tracer.start_trace(tracing::rgw::MULTIPART,
                                         s->trace_enabled);
...
void RGWCompleteMultipart::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ...
        if (op_ret == -ENOENT && check_previously_completed(parts)) {
            ldpp_dout(this,
                      1) <<
                "NOTICE: This multipart completion is already completed" <<
                dendl;
            op_ret = 0;
            return;
        }
    ...
    std::unique_ptr < rgw::sal::Notification > res
        =
        driver->get_notification(meta_obj.get(), nullptr, s,
                                 rgw::notify::
                                 ObjectCreatedCompleteMultipartUpload, y,
                                 &s->object->get_name());
    op_ret = res->publish_reserve(this);
    if (op_ret < 0) {
        return;
    ...
    op_ret =
        upload->complete(this, y, s->cct, parts->parts, remove_objs,
                         accounted_size, compressed, cs_info, ofs, s->req_id,
                         s->owner, olh_epoch, target_obj.get());
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "ERROR: upload complete failed ret=" << op_ret << dendl;
        return;
    }
    ...
    if (r >= 0) {
        ...
        ldpp_dout(this,
                  0) << "WARNING: failed to remove object " << meta_obj <<
            dendl;
    }

    // send request to notification manager
    int ret =
        res->publish_commit(this, ofs, upload->get_mtime(), etag,
                            target_obj->get_instance());
    if (ret < 0) {
        ldpp_dout(this,
                  1) << "ERROR: publishing notification failed, with error: " <<
            ret << dendl;
...
bool RGWCompleteMultipart::
check_previously_completed(const RGWMultiCompleteUpload * parts)
{
    // re-calculate the etag from the parts and compare to the existing object
    int ret = s->object->get_obj_attrs(s->yield, this);
    if (ret < 0) {
        ldpp_dout(this,
                  0) << __func__ << "() ERROR: get_obj_attrs() returned ret=" <<
            ret << dendl;
        return false;
    }
  ...
  for (const auto &[index, part]:parts->parts) {
        ...
        ldpp_dout(this,
                  20) << __func__ << "() re-calculating multipart etag: part: "
    ...
    if (oetag.compare(final_etag_str) != 0) {
        ldpp_dout(this,
                  1) << __func__ << "() NOTICE: etag mismatch: object etag:" <<
            oetag << ", re-calculated etag:" << final_etag_str << dendl;
        return false;
    }
    ldpp_dout(this,
              5) << __func__ <<
        "() object etag and re-calculated etag match, etag: " << oetag << dendl;
    return true;
}
...
void RGWCompleteMultipart::complete()
{
    ...
    if (unlikely(serializer.get() && serializer->is_locked())) {
        ...
        if (r < 0) {
            ldpp_dout(this,
                      0) << "WARNING: failed to unlock " << *serializer.
                get() << dendl;
        }
    }
    send_response();
...
int RGWListMultipart::verify_permission(optional_yield y)
{
    ...
    if (!verify_object_permission
        (this, s, rgw::IAM::s3ListMultipartUploadParts))
        return -EACCES;
...
void RGWListMultipart::execute(optional_yield y)
{
    ...
    op_ret =
        upload->list_parts(this, s->cct, max_parts, marker, NULL, &truncated);
}
...
int RGWListBucketMultiparts::verify_permission(optional_yield y)
{
    ...
    if (!verify_bucket_permission(this,
                                  s, rgw::IAM::s3ListBucketMultipartUploads))
        return -EACCES;
...
int RGWDeleteMultiObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty()
        ...
        bool not_versioned = rgw::sal::Object::empty(s->object.get())
            || s->object->get_instance().empty();

        auto identity_policy_res =
            eval_identity_or_session_policies(this, s->iam_user_policies,
                                              s->env,
                                              not_versioned ? rgw::IAM::
                                              s3DeleteObject : rgw::IAM::
                                              s3DeleteObjectVersion,
                                              ARN(s->bucket->get_key()));
        if (identity_policy_res == Effect::Deny) {
...
void RGWDeleteMultiObj::write_ops_log_entry(rgw_log_entry & entry) const const
{
    int num_err = 0;
    int num_ok = 0;
    for (auto iter = ops_log_entries.begin();
         iter != ops_log_entries.end(); ++iter) {
        if (iter->error) {
            num_err++;
        }
        else {
...
void RGWDeleteMultiObj::handle_individual_object(const rgw_obj_key & o,
                                                 optional_yield y,
                                                 boost::asio::deadline_timer *
                                                 formatter_flush_cond)
{
    ...
    send_partial_response(o, del_op->result.delete_marker,
...
void RGWDeleteMultiObj::execute(optional_yield y)
{
    ...
    if (y) {
        formatter_flush_cond =
            std::make_optional < boost::asio::deadline_timer >
            (y.get_io_context());
    }
...
#define DELETE_MULTI_OBJ_MAX_NUM      1000
    ...
    if (s->bucket->get_info().mfa_enabled()) {
      ...
      for (auto i:multi_delete->objects) {
        ...
        if (has_versioned && !s->mfa_verified) {
            ldpp_dout(this,
                      5) <<
                "NOTICE: multi-object delete request with a versioned object, mfa auth not provided"
                << dendl;
            op_ret = -ERR_MFA_REQUIRED;
            goto error;
        }
    ...
    for (iter = multi_delete->objects.begin();
         iter != multi_delete->objects.end(); ++iter) {
        ...
        if (y && max_aio > 1) {
            ...
            spawn::spawn(y.get_yield_context(),
                         [this, &y, &aio_count, obj_key,
                          &formatter_flush_cond] (yield_context yield) {
                         handle_individual_object(obj_key, optional_yield {
                                                  y.get_io_context(), yield}
                                                  , &*formatter_flush_cond);
                         aio_count--;}
            );
        }
    ...
    if (formatter_flush_cond) {
        wait_flush(y, &*formatter_flush_cond,
                   [this, n = multi_delete->objects.size()] {
                                               ...
                                               optional_yield y)
{
    ...
    int ret =
        read_bucket_policy(dpp, driver, s, binfo, battrs, &bacl, binfo.bucket,
                           y);
    if (ret < 0) {
        return false;
    }
    ...
    return verify_bucket_permission(dpp, s, binfo.bucket, s->user_acl.get(),
                                    &bacl, policy, s->iam_user_policies,
                                    s->session_policies,
                                           ...
                                           optional_yield y)
{
    ...
    int ret =
        driver->get_bucket(dpp, s->user.get(), s->user->get_tenant(),
    ...
    if (!path.obj_key.empty()) {
        ...
        std::unique_ptr < rgw::sal::Object > obj =
            bucket->get_object(path.obj_key);
        obj->set_atomic();
  ...
  binfo_fail:
    if (-ENOENT == ret) {
        ldpp_dout(dpp,
                  20) << "cannot find bucket = " << path.bucket_name << dendl;
        num_unfound++;
    }
    else {
...
bool RGWBulkDelete::Deleter::delete_chunk(const std::list < acct_path_t >
                                          &paths, optional_yield y)
{
    ...
    if (std::string_view::npos != start_pos) {
        ...
        if (std::string_view::npos != sep_pos) {
            const auto bucket_name =
                path.substr(start_pos, sep_pos - start_pos);
...
int RGWBulkUploadOp::handle_dir_verify_permission(optional_yield y)
{
    if (s->user->get_max_buckets() > 0) {
        ...
        std::string marker;
        op_ret =
            s->user->list_buckets(this, marker, std::string(),
                                  s->user->get_max_buckets(), false, buckets,
                                  y);
        if (op_ret < 0) {
            return op_ret;
        ...
        if (buckets.count() >= static_cast < size_t >
...
void RGWBulkUploadOp::init(rgw::sal::Driver * const driver,
                           req_state * const s, RGWHandler * const h)
{
...
int RGWBulkUploadOp::handle_dir(const std::string_view path, optional_yield y)
{
    ...
    op_ret = s->user->create_bucket(this, new_bucket,
                                    driver->get_zone()->get_zonegroup().
                                    get_id(), placement_rule,
                                    swift_ver_location, pquota_info, policy,
                                    attrs, out_info, ep_objv, true, false,
                                    &bucket_exists, info, &bucket, y);
    /* continue if EEXIST and create_bucket will fail below.  this way we can
     * recover from a partial create by retrying it. */
                                                    ...
                                                    optional_yield y)
{
    RGWAccessControlPolicy bacl(driver->ctx());
    op_ret =
        read_bucket_policy(this, driver, s, binfo, battrs, &bacl, binfo.bucket,
                           y);
    if (op_ret < 0) {
                                 ...
                                 AlignedStreamGetter & body, optional_yield y)
{
    ...
    op_ret =
        driver->get_bucket(this, s->user.get(),
                           rgw_bucket(rgw_bucket_key
                                      (s->user->get_tenant(), bucket_name)),
                           &bucket, y);
    if (op_ret < 0) {
        if (op_ret == -ENOENT) {
            ldpp_dout(this,
                      20) << "non existent directory=" << bucket_name << dendl;
        }
        return op_ret;
    }
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  20) << "cannot prepare processor due to ret=" << op_ret <<
            dendl;
        return op_ret;
    }
    ...
    const auto & compression_type =
        driver->get_compression_type(dest_placement);
    CompressorRef plugin;
    boost::optional < RGWPutObj_Compress > compressor;
    if (compression_type != "none") {
        plugin = Compressor::create(s->cct, compression_type);
    ...
    do {
        ...
        if (len < 0) {
            ...
            if (op_ret < 0) {
                ldpp_dout(this,
                          20) << "filter->process() returned ret=" << op_ret <<
                    dendl;
                return op_ret;
            }
    ...
    if (ofs != size) {
        ldpp_dout(this,
                  10) << "real file size different from declared" << dendl;
        op_ret = -EINVAL;
        return op_ret;
    }
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  20) << "processor::complete returned op_ret=" << op_ret <<
            dendl;
    }
        ...
        if (!status.empty() && header) {
            ...
            case rgw::tar::FileType::NORMAL_FILE:{
                    ...
                    auto body = AlignedStreamGetter(0, header->get_filesize(),
                                                    ...
                                                    *stream);
                    op_ret =
                        handle_file(filename, header->get_filesize(), body, y);
                    if (!op_ret) {
            ...
            if (boost::algorithm::contains(std::initializer_list < int > {
                                           op_ret}, terminal_errors)) {
                ldpp_dout(this,
                          2) << "terminating due to ret=" << op_ret << dendl;
                break;
            }
        }
        else {
...
int RGWGetAttrs::verify_permission(optional_yield y)
{
    ...
    auto iam_action = s->object->get_instance().empty()?
        rgw::IAM::s3GetObject : rgw::IAM::s3GetObjectVersion;
...
void RGWGetAttrs::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "ERROR: failed to get obj attrs, obj=" << s->
            object << " ret=" << op_ret << dendl;
        return;
    }
...
void RGWRMAttrs::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  0) << "ERROR: failed to delete obj attrs, obj=" << s->
            object << " ret=" << op_ret << dendl;
    }
    return;
}
...
int RGWHandler::init(rgw::sal::Driver * _driver,
                     req_state * _s, rgw::io::BasicClient * cio)
{
    driver = _driver;
    s = _s;
...
int RGWHandler::do_init_permissions(const DoutPrefixProvider * dpp,
                                    optional_yield y)
{
...
std::ostream & RGWOp::gen_prefix(std::ostream & out) const const
{
...
void RGWPutBucketPolicy::execute(optional_yield y)
{
    ...
    try {
        ...
        op_ret =
            retry_raced_bucket_write(this, s->bucket.get(),[&p, this, &attrs] {
...
void RGWPutBucketObjectLock::execute(optional_yield y)
{
    ...
    try {
        RGWXMLDecoder::decode_xml("ObjectLockConfiguration", obj_lock, &parser,
                                  true);
    }
    catch(RGWXMLDecoder::err & err) {
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  20) << __func__ << "forward_request_to_master returned ret="
            << op_ret << dendl;
        return;
    }
...
void RGWPutObjRetention::execute(optional_yield y)
{
    ...
    if (aiter != attrs.end()) {
        ...
        if (ceph::real_clock::to_time_t(obj_retention.get_retain_until_date()) <
            ceph::real_clock::to_time_t(old_obj_retention.
    ...
    op_ret =
        s->object->modify_obj_attrs(RGW_ATTR_OBJECT_RETENTION, bl, s->yield,
                                    this);
...
void RGWGetObjRetention::execute(optional_yield y)
{
    ...
    try {
        ...
        ldpp_dout(this,
                  0) << __func__ << "decode object retention config failed" <<
            dendl;
        op_ret = -EIO;
        return;
    }
...
void RGWPutObjLegalHold::execute(optional_yield y)
{
    ...
    //if instance is empty, we should modify the latest object
    op_ret =
        s->object->modify_obj_attrs(RGW_ATTR_OBJECT_LEGAL_HOLD, bl, s->yield,
                                    this);
    return;
}
...
void RGWGetObjLegalHold::execute(optional_yield y)
{
    ...
    try {
        ...
        ldpp_dout(this,
                  0) << __func__ << "decode object legal hold config failed" <<
            dendl;
        op_ret = -EIO;
        return;
    }
...
void RGWPutBucketPublicAccessBlock::execute(optional_yield y)
{
    ...
    try {
        RGWXMLDecoder::decode_xml("PublicAccessBlockConfiguration", access_conf,
                                  &parser, true);
    }
    catch(RGWXMLDecoder::err & err) {
        ldpp_dout(this, 5) << "unexpected xml:" << err << dendl;
...
void RGWGetBucketPublicAccessBlock::execute(optional_yield y)
{
    ...
    if (auto aiter = attrs.find(RGW_ATTR_PUBLIC_ACCESS); aiter == attrs.end()) {
        ...
        try {
            ...
            ldpp_dout(this,
                      0) << __func__ << "decode access_conf failed" << dendl;
            op_ret = -EIO;
            return;
        }
    }
...
void RGWPutBucketEncryption::execute(optional_yield y)
{
    ...
    try {
        RGWXMLDecoder::decode_xml("ServerSideEncryptionConfiguration",
                                  bucket_encryption_conf, &parser, true);
    }
    catch(RGWXMLDecoder::err & err) {
        ldpp_dout(this, 5) << "unexpected xml:" << err << dendl;
        op_ret = -ERR_MALFORMED_XML;
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(),[this, y, &conf_bl] {
                                      rgw::sal::Attrs attrs =
                                      s->bucket->get_attrs();
                                      attrs[RGW_ATTR_BUCKET_ENCRYPTION_POLICY] =
                                      conf_bl;
                                      return s->bucket->
                                      merge_and_store_attrs(this, attrs, y);}
    );
...
void RGWGetBucketEncryption::execute(optional_yield y)
{
    ...
    if (auto aiter = attrs.find(RGW_ATTR_BUCKET_ENCRYPTION_POLICY);
        aiter == attrs.end()) {
        ldpp_dout(this,
                  0) << "can't find BUCKET ENCRYPTION attr for bucket_name = "
            << s->bucket_name << dendl;
        op_ret = -ENOENT;
        s->err.message =
        ...
        try {
            ...
            ldpp_dout(this,
                      0) << __func__ << "decode bucket_encryption_conf failed"
                << dendl;
            op_ret = -EIO;
            return;
        }
