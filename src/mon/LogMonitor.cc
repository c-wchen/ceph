...
void LogMonitor::update_from_paxos(bool * need_bootstrap)
{
    ...
    while (version > summary.version) {
        ...
        while (!p.end()) {
            LogEntry le;
            le.decode(p);
            dout(7) << "update_from_paxos applying incremental log " << summary.
                version + 1 << " " << le << dendl;
            ...
            if (channels.do_log_to_graylog(channel)) {
                ceph::logging::Graylog::Ref graylog =
                    channels.get_graylog(channel);
                if (graylog) {
...
bool LogMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "log last") {
        ...
        auto match =[level, channel] (const LogEntry & entry){
            return entry.prio >= level && (entry.channel == channel
                                           || channel == "*");
        };
...
bool LogMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "log") {
        ...
        wait_for_finished_proposal(op,
                                   new Monitor::C_Command(mon, op, 0, string(),
                                                          get_last_committed() +
                                                          1));
        return true;
    }
...
void LogMonitor::check_subs()
{
    ...
    for (map < string, xlist < Subscription * >*>::iterator i =
         mon->session_map.subs.begin(); i != mon->session_map.subs.end(); ++i) {
        for (xlist < Subscription * >::iterator j = i->second->begin();
             !j.end(); ++j) {
...
{
    dout(10) << __func__ << " client wants " << s->type << " ver " << s->
        next << dendl;
...
void LogMonitor::_create_sub_incremental(MLog * mlog, int level, version_t sv)
{
    ...
    if (sv < get_first_committed()) {
        ...
        le.prio = CLOG_WARN;
        ostringstream ss;
        ss << "skipped log messages from " << sv << " to " <<
            get_first_committed();
        le.msg = ss.str();
...
void LogMonitor::update_log_channels()
{
    ...
    if (r < 0) {
        derr << __func__ << " error parsing 'mon_cluster_log_to_syslog'" <<
            dendl;
        return;
    }
    ...
    if (r < 0) {
        derr << __func__ <<
            " error parsing 'mon_cluster_log_to_syslog_facility'" << dendl;
        return;
    }

    r = get_conf_str_map_helper(g_conf->mon_cluster_log_file, oss,
                                &channels.log_file, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
...
{
    generic_dout(25) << __func__ << " for channel '" << channel << "'" << dendl;
