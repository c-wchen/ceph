    ...
    namespace mirror {
        ...
        template < typename I >
            ...
            if (r == 0
                ...
                ldout(m_cct,
                      10) <<
                    "enabling mirroring on in-progress image replication" <<
                    dendl;
            }
            else if (r == 0) {
                if (m_mirror_image.mode != m_mode) {
                    lderr(m_cct) << "invalid current image mirror mode" <<
                        dendl;
                    r = -EINVAL;
                }
        ...
        template < typename I > void EnableRequest < I >::get_tag_owner() {
            ...
            librbd::Journal <>::is_tag_owner(m_io_ctx, m_image_id,
                                             &m_is_primary, m_op_work_queue,
                                             ctx);
        }
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "failed to check tag ownership: " <<
                    cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
            ...
            if (!m_is_primary) {
                lderr(m_cct) << "last journal tag not owned by local cluster" <<
                    dendl;
                finish(-EINVAL);
                return;
            }
        ...
        template < typename I > void EnableRequest < I >::open_image() {
            ...
            m_close_image = true;
            m_image_ctx =
                I::create("", m_image_id, CEPH_NOSNAP, m_io_ctx, false);

            auto ctx = create_context_callback <
                EnableRequest < I >,
                &EnableRequest < I >::handle_open_image > (this);
            m_image_ctx->state->
                open(OPEN_FLAG_SKIP_OPEN_PARENT | OPEN_FLAG_IGNORE_MIGRATING,
                     ctx);
        }
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "failed to open image: " << cpp_strerror(r) <<
                    dendl;
                m_image_ctx = nullptr;
                finish(r);
                return;
        ...
        template < typename I >
            ...
            int r =
                util::snap_create_flags_api_to_internal(m_cct,
                                                        util::
            ...
            auto req =
                snapshot::CreatePrimaryRequest < I >::create(m_image_ctx,
                                                             m_mirror_image.
                                                             global_image_id,
                                                             (m_image_clean ? 0
                                                              : CEPH_NOSNAP),
        ...
        template < typename I > void EnableRequest < I >::close_image() {
            ...
            auto ctx = create_context_callback <
                EnableRequest < I >,
                &EnableRequest < I >::handle_close_image > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "failed to close image: " << cpp_strerror(r) <<
                    dendl;
                if (m_ret_val == 0) {
                    m_ret_val = r;
                }
        ...
        template < typename I >
            ...
            auto aio_comp = create_rados_callback <
                EnableRequest < I >,
                &EnableRequest < I >::handle_enable_non_primary_feature >
                (this);
            int r =
                m_io_ctx.aio_operate(util::header_name(m_image_id), aio_comp,
                                     &op);
            ceph_assert(r == 0);
            aio_comp->release();
        }
        ...
        template < typename I > void EnableRequest < I >::image_state_update() {
            ...
            auto ctx = create_context_callback <
                EnableRequest < I >,
                &EnableRequest < I >::handle_image_state_update > (this);
            auto req =
                ImageStateUpdateRequest < I >::create(m_io_ctx, m_image_id,
                                                      cls::rbd::
