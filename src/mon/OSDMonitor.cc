...
namespace {
        ...
        virtual int64_t get_cache_bytes(PriorityCache::Priority pri) const {
            ...
            for (int i = 0; i < PriorityCache::Priority::LAST + 1; i++) {
                PriorityCache::Priority pri =
                    static_cast < PriorityCache::Priority > (i);
    ...
    bool is_osd_writable(const OSDCapGrant & grant,
    ...
    bool is_unmanaged_snap_op_permitted(CephContext * cct,
        ...
        if (caps_info.caps.length() > 0) {
            ...
            try {
                ...
                derr << "corrupt OSD cap data for " << entity_name <<
                    " in auth db" << dendl;
                return false;
            }
        }
...
epoch_t LastEpochClean::get_lower_bound(const OSDMap & latest) constconst
{
...
void LastEpochClean::dump(Formatter * f) const const
{
...
class C_UpdateCreatingPGs:public Context {
  public:
    ...
    utime_t start;
    epoch_t epoch;
    C_UpdateCreatingPGs(OSDMonitor * osdmon, epoch_t e):osdmon(osdmon),
...
OSDMonitor::OSDMonitor(CephContext * cct,
                       Monitor & mn, Paxos & p, const string & service_name)
:  
PaxosService(mn, p, service_name),
cct(cct),
inc_osd_cache(g_conf()->mon_osd_cache_size),
full_osd_cache(g_conf()->mon_osd_cache_size),
has_osdmap_manifest(false), mapper(mn.cct, &mn.cpu_tp)
{
    ...
    if (r < 0) {
        derr << __func__ <<
            " using default osd cache size - mon_osd_cache_size (" << g_conf()->
            mon_osd_cache_size << ") without priority cache management" <<
            dendl;
    }
}

const char **OSDMonitor::get_tracked_conf_keys() const const
{
...
void OSDMonitor::create_initial()
{
    ...
    newmap.flags |=
        ...
        CEPH_OSDMAP_PURGED_SNAPDIRS | CEPH_OSDMAP_PGLOG_HARDLIMIT;
    ...
    if (g_conf().get_val < bool > ("mon_debug_no_require_reef")) {
        if (g_conf().get_val < bool > ("mon_debug_no_require_quincy")) {
            derr << __func__ << " mon_debug_no_require_reef and quincy=true" <<
                dendl;
    ...
    ceph_release_t r =
        ceph_release_from_name(g_conf()->
                               mon_osd_initial_require_min_compat_client);
    if (!r) {
        ceph_abort_msg
            ("mon_osd_initial_require_min_compat_client is not valid");
    }
...
void OSDMonitor::get_store_prefixes(std::set < string > &s) const const
{
...
void OSDMonitor::update_from_paxos(bool * need_bootstrap)
{
    ...
    if (get_first_committed() > 1 && latest_full < get_first_committed()) {
        ...
        for (version_t v = lc; v >= fc; v--) {
            ...
            if (mon.store->exists(get_service_name(), full_key)) {
                dout(10) << __func__ << " found latest full map v " << v <<
                    dendl;
                latest_full = v;
                break;
            }
    ...
    if (!mon.store->get(OSD_PG_CREATING_PREFIX, "creating", bl)) {
        ...
        dout(1) << __func__ <<
            " missing creating pgs; upgrade from post-kraken?" << dendl;
    }
    ...
    while (version > osdmap.epoch) {
        ...
        dout(7) << "update_from_paxos  applying incremental " << osdmap.epoch +
            1 << dendl;
        ...
        if (orig_full_bl.length()) {
            ...
            if (inc.full_crc != osdmap.crc) {
                ...
                derr << __func__ <<
                    " full map CRC mismatch, resetting to canonical" << dendl;
    ...
    for (int o = 0; o < osdmap.get_max_osd(); o++) {
        ...
        if (osdmap.is_down(o)) {
            ...
            if (found == down_pending_out.end()) {
                dout(10) << " adding osd." << o << " to down_pending_out map" <<
                    dendl;
    ...
    if (osdmap.stretch_mode_enabled) {
        ...
        if (marked_osd_down &&
            ...
            dout(20) << "Checking degraded stretch mode due to osd changes" <<
                dendl;
...
int OSDMonitor::register_cache_with_pcm()
{
    ...
    pcm =
        std::make_shared < PriorityCache::Manager > (cct, min, max, target,
                                                     true);
...
int OSDMonitor::_set_cache_ratios()
{
    ...
    dout(1) << __func__ << " kv ratio " << cache_kv_ratio
        ...
        << " full ratio " << cache_full_ratio << dendl;
    return 0;
}
...
void OSDMonitor::update_msgr_features()
{
  ...
  for (int type:types) {
        ...
        if ((mon.messenger->get_policy(type).features_required & mask) !=
            features) {
...
void OSDMonitor::create_pending()
{
    ...
    // safety checks (this shouldn't really happen)
    {
        if (osdmap.backfillfull_ratio <= 0) {
            pending_inc.new_backfillfull_ratio =
                                   ...
                                   const OSDMap & nextmap)
{
    ...
    if (pending_creatings.last_scan_epoch < inc.epoch) {
        ...
        queued += scan_for_creating_pgs(osdmap.get_pools(),
                                        inc.old_pools,
                                        inc.modified, &pending_creatings);
    ...
    dout(10) << __func__ << " queue remaining: " << pending_creatings.queue.
        size()
        << " pools" << dendl;
    ...
    if (mon.monmap->min_mon_release >= ceph_release_t::octopus) {
      ...
      for (auto & i:pending_creatings.pgs) {
            ...
            nextmap.pg_to_up_acting_osds(pgid, &up, &up_primary, &acting,
                                         &acting_primary);
            if (i.second.history.epoch_created == 0) {
                ...
                if (PastIntervals::
                    check_new_interval(i.second.acting_primary, acting_primary,
                                       i.second.acting, acting,
                                       i.second.up_primary, up_primary,
                                       i.second.up, up,
                    ...
                    if (pgid.is_split(osdmap.get_pg_num(pgid.pool()),
...
void OSDMonitor::maybe_prime_pg_temp()
{
    ...
    for (auto p = pending_inc.new_weight.begin();
        ...
        if (osdmap.exists(p->first) && p->second < osdmap.get_weight(p->first)) {
            ...
            dout(10) << __func__ << " osd." << p->
                first << " weight increase, all" << dendl;
            all = true;
        }
    }
...
void OSDMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    if (mapping_job) {
        ...
        dout(1) << __func__ <<
            " skipping prime_pg_temp; mapping job did not start" << dendl;
    }
    mapping_job.reset();
    ...
    while (p != pending_inc.new_state.end()) {
        if (p->second == 0) {
            dout(10) << "new_state for osd." << p->
                first << " is 0, removing" << dendl;
  ...
  for (auto & i:pending_inc.new_weight) {
...

    {
        ...
        if (mon.monmap->min_mon_release < ceph_release_t::octopus) {
            dout(20) << __func__ <<
                " encoding pending pgs without octopus features" << dendl;
      ...
      for (auto & i:tmp.get_pools()) {
            if (tmp.require_osd_release < ceph_release_t::nautilus) {
                ...
                if (pending_inc.new_pools.count(i.first)) {
                    pending_inc.new_pools[i.first].flags &=
        ...
        if (full_pool_ids.empty() ||
            backfillfull_pool_ids.empty() || nearfull_pool_ids.empty()) {
          ...
          for (auto & pool:tmp.get_pools()) {
                ...
                if (tmp.get_pg_pool(p)->has_flag(pg_pool_t::FLAG_NEARFULL) &&
                    ...
                    dout(10) << __func__ << " clearing pool '" << tmp.
                        pool_name[p]
        ...
        if (osdmap.require_osd_release < ceph_release_t::nautilus &&
          ...
          for (auto & i:tmp.get_pools()) {
                if (pending_creatings.still_creating_pool(i.first)) {
                    dout(10) << __func__ << " adding CREATING flag to pool " <<
                        i.first << dendl;
                    if (pending_inc.new_pools.count(i.first) == 0) {
                        pending_inc.new_pools[i.first] = i.second;
                    }
        ...
        if (osdmap.require_osd_release < ceph_release_t::octopus &&
          ...
          for (auto &[poolid, pi]:tmp.pools) {
                ...
                dout(10) << __func__ << " clearing pool " << poolid <<
                    " removed_snaps" << dendl;
            ...
            if (!combined.empty()) {
                ...
                dout(10) << __func__ <<
                    ...
                    (pending_inc.epoch -
                     1) << ", " << v.length() << " bytes" << dendl;
            }
            else {
                dout(10) << __func__ <<
                    " there were no pre-octopus purged snaps" << dendl;
            }
    ...
    for (auto i = pending_inc.new_state.begin();
        ...
        if (s & CEPH_OSD_UP) {
            ...
            if ((xi.laggy_probability || xi.laggy_interval)
                && xi.down_stamp.sec()) {
                int last_failure_interval =
                    pending_inc.modified.sec() - xi.down_stamp.sec();
    ...
    for (map < int32_t, uint32_t >::iterator i = pending_inc.new_weight.begin();
        ...
        if (i->second == CEPH_OSD_OUT) {
            ...
            dout(2) << " osd." << i->first << " WEIGHT " << hex << i->
                second << dec << dendl;
        }
    }
...
void OSDMonitor::check_for_filestore_osds(health_check_map_t * checks)
{
    if (g_conf()->mon_warn_on_filestore_osds && filestore_osds.size() > 0) {
        ...
        ss << filestore_osds.size()
            << " osd(s) " << (filestore_osds.size() == 1 ? "is" : "are")
            << " running Filestore";
...
version_t OSDMonitor::get_trim_to() const const
{
...
epoch_t OSDMonitor::get_min_last_epoch_clean() const const
{
...
bool OSDMonitor::should_prune() const const
{
    ...
    if ((last - first) <= min_osdmap_epochs) {
        ...
        dout(10) << __func__ << " could only prune " << (last_to_pin - first)
            ...
            " is less than the required minimum (" << prune_min << ")" << dendl;
        return false;

    }
    else if (has_osdmap_manifest && last_pinned >= last_to_pin) {
        dout(10) << __func__
            << " we have pruned as far as we can; do not prune." << dendl;
        return false;
        ...
        dout(10) << __func__
            ...
            << last_to_pin << ", interval: " << prune_interval << ")" << dendl;
        return false;
    }
...
void OSDMonitor::_prune_update_trimmed(MonitorDBStore::TransactionRef tx,
                                       version_t first)
{
    dout(10) << __func__
        << " first " << first
...
void OSDMonitor::prune_init(osdmap_manifest_t & manifest)
{
    ...
    if (!has_osdmap_manifest) {
        ...
        ceph_assert(!osdmap_manifest.pinned.empty());
        ceph_assert(osdmap_manifest.get_first_pinned() ==
                    get_first_committed());
...
bool OSDMonitor::_prune_sanitize_options() const const
{
    ...
    if (prune_interval == 0) {
        derr << __func__
            << " prune is enabled BUT prune interval is zero; abort." << dendl;
        r = false;
    }
    else if (prune_interval == 1) {
        derr << __func__
            << " prune interval is equal to one, which essentially means"
            " no pruning; abort." << dendl;
        r = false;
    }
    if (prune_min == 0) {
        derr << __func__
            << " prune is enabled BUT prune min is zero; abort." << dendl;
        r = false;
    }
...
bool OSDMonitor::is_prune_enabled() const const
{
    return g_conf().get_val < bool > ("mon_osdmap_full_prune_enabled");
}

bool OSDMonitor::is_prune_supported() const const
{
    return mon.get_required_mon_features().
        contains_any(ceph::features::mon::FEATURE_OSDMAP_PRUNE);
}
...
bool OSDMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    dout(10) << "preprocess_query " << *m << " from " << m->
...
bool OSDMonitor::prepare_update(MonOpRequestRef op)
{
    op->mark_osdmon_event(__func__);
    Message *m = op->get_req();
    dout(7) << "prepare_update " << *m << " from " << m->
        get_orig_source_inst() << dendl;
...
bool OSDMonitor::preprocess_failure(MonOpRequestRef op)
{
    ...
    if (osdmap.get_addrs(badboy) != m->get_target_addrs()) {
        dout(5) << "preprocess_failure wrong osd: report osd." << m->
            get_target_osd()
            << " " << m->get_target_addrs()
  ...
  public:
    ...
    :C_MonOp(op), osdmon(osdmon) {
        ...
        if (r == 0) {
            ...
            osdmon->mon.send_reply(op, new MOSDMarkMeDown(m->fsid, m->target_osd, m->target_addrs, m->get_epoch(), false)); // ACK itself does not request an ack
        }
        else if (r == -EAGAIN) {
            osdmon->dispatch(op);
        }
...
bool OSDMonitor::preprocess_mark_me_down(MonOpRequestRef op)
{
    ...
    if (!osdmap.exists(from) ||
        osdmap.is_down(from) || osdmap.get_addrs(from) != m->target_addrs) {
...
bool OSDMonitor::prepare_mark_me_down(MonOpRequestRef op)
{
    ...
    mon.clog->
        info() << "osd." << target_osd << " marked itself " <<
                                   ...
                                   failure_info_t & fi) constconst
{
  ...
  for (auto &[reporter, report]:fi.reporters) {
        if (osdmap.exists(reporter)) {
            ...
            peer_grace +=
                decay * (double)xi.laggy_interval * xi.laggy_probability;
        }
    }
    ...
    dout(10) << " osd." << target_osd << " has "
        ...
        << " + " << peer_grace << "), max_failed_since " << fi.
        get_failed_since()
        << dendl;
...
bool OSDMonitor::check_failure(utime_t now, int target_osd, failure_info_t & fi)
{
    ...
    if (reporters_by_subtree.size() < g_conf().get_val < uint64_t >
...
bool OSDMonitor::is_failure_stale(utime_t now, failure_info_t & fi) const const
{
    ...
    const utime_t failed_for = now - fi.get_failed_since();
    auto heartbeat_grace =
        cct->_conf.get_val < int64_t > ("osd_heartbeat_grace");
    auto heartbeat_stale =
        cct->_conf.get_val < int64_t > ("osd_heartbeat_stale");
    ...
    if (m->if_osd_failed()) {
        ...
        if (m->is_immediate()) {
            mon.clog->debug() << "osd." << m->get_target_osd()
                << " reported immediately failed by " << m->get_orig_source();
            force_failure(target_osd, reporter);
            return true;
        }
        mon.clog->debug() << "osd." << m->
            get_target_osd() << " reported failed by " << m->get_orig_source();
...
bool OSDMonitor::preprocess_boot(MonOpRequestRef op)
{
    ...
    if (!session->is_capable("osd", MON_CAP_X)) {
        dout(0) <<
            "got preprocess_boot message from entity with insufficient caps" <<
            session->caps << dendl;
        goto ignore;
    }
    ...
    if (m->get_orig_source_inst().addr.is_blank_ip()) {
        dout(0) << "preprocess_boot got blank addr for " << m->
            get_orig_source_inst() << dendl;
        goto ignore;
    }
    ...
    if (osdmap.exists(from) &&
        ...
        osdmap.get_most_recent_addrs(from).legacy_equals(m->
                                                         get_orig_source_addrs
                                                         ())) {
        dout(7) << "prepare_boot msg from before last up_from, ignoring" <<
            dendl;
    ...
    // noup?
    if (!can_mark_up(from)) {
        dout(7) << "preprocess_boot ignoring boot from " << m->
            get_orig_source_inst() << dendl;
        send_latest(op, m->sb.current_epoch + 1);
        return true;
    }
...
bool OSDMonitor::prepare_boot(MonOpRequestRef op)
{
    ...
    dout(7) << __func__ << " from " << m->get_source()
        ...
        << " hb_front_addrs " << m->hb_front_addrs << dendl;
    ...
    if (osdmap.is_up(from)) {
        ...
        ceph_assert(!osdmap.get_addrs(from).
                    legacy_equals(m->get_orig_source_addrs())
                    || !osdmap.get_cluster_addrs(from).legacy_equals(m->
                                                                     cluster_addrs));
        ...
        if (!osdmap.exists(from) || osdmap.get_uuid(from) != m->sb.osd_fsid) {
            ...
            ceph_assert(!osdmap.exists(from)
                        || osdmap.get_uuid(from).is_zero());
...
bool OSDMonitor::preprocess_full(MonOpRequestRef op)
{
    ...
    if ((!osdmap.is_up(from) &&
         osdmap.get_most_recent_addrs(from).legacy_equals(m->
                                                          ...
                                                          ()))
        || (osdmap.is_up(from)
            && !osdmap.get_addrs(from).legacy_equals(m->
                                                     get_orig_source_addrs())))
    {
        dout(7) << __func__ << " ignoring full message from down " << m->
            get_orig_source_inst() << dendl;
        goto ignore;
    }
...
bool OSDMonitor::prepare_full(MonOpRequestRef op)
{
    ...
    if (cur_state != want_state) {
        ...
        pending_inc.new_state[from] |=
            (osdmap.get_state(from) & mask) ^ want_state;
...
bool OSDMonitor::preprocess_alive(MonOpRequestRef op)
{
    ...
    if (osdmap.get_up_thru(from) >= m->want) {
        // yup.
        dout(7) << "preprocess_alive want up_thru " << m->
            want << " dup from " << m->get_orig_source_inst() << dendl;
        _reply_map(op, m->version);
        return true;
    }
...
bool OSDMonitor::prepare_alive(MonOpRequestRef op)
{
    ...
    dout(7) << "prepare_alive want up_thru " << m->want << " have " << m->
        version << " from " << m->get_orig_source_inst() << dendl;
...
bool OSDMonitor::prepare_pg_created(MonOpRequestRef op)
{
    ...
    if (!src.is_osd() ||
        ...
        !mon.osdmon()->osdmap.get_addrs(from).legacy_equals(m->
                                                            get_orig_source_addrs
                                                            ())) {
...
bool OSDMonitor::preprocess_pg_ready_to_merge(MonOpRequestRef op)
{
    ...
    if (pi->get_pg_num() <= m->pgid.ps()) {
        dout(20) << " pg_num " << pi->get_pg_num() << " already < " << m->
            pgid << dendl;
        goto ignore;
    }
    ...
    if (pi->get_pg_num_pending() > m->pgid.ps()) {
        dout(20) << " pg_num_pending " << pi->
            get_pg_num_pending() << " > " << m->pgid << dendl;
        goto ignore;
    }
    return false;
...
bool OSDMonitor::prepare_pg_ready_to_merge(MonOpRequestRef op)
{
    ...
    if (m->ready && prob > 0 && prob > (double)(rand() % 1000) / 1000.0) {
        ...
        MonOpRequestRef nop =
            mon.op_tracker.create_request < MonOpRequest > (n);
...
bool OSDMonitor::preprocess_pgtemp(MonOpRequestRef op)
{
    ...
    if (!session->is_capable("osd", MON_CAP_X)) {
        dout(0) <<
            "attempt to send MOSDPGTemp from entity with insufficient caps " <<
            session->caps << dendl;
        goto ignore;
    }
    ...
    for (auto p = m->pg_temp.begin(); p != m->pg_temp.end(); ++p) {
        dout(20) << " " << p->first
            << (osdmap.pg_temp->count(p->first) ? osdmap.pg_temp->
                get(p->first) : empty)
            << " -> " << p->second << dendl;
        ...
        if (!osdmap.have_pg_pool(p->first.pool())) {
             ...
             */
            dout(10) << __func__ << " ignore " << p->first << " -> " << p->
                second << ": pool has been removed" << dendl;
            ignore_cnt++;
            continue;
        }

        int acting_primary = -1;
        osdmap.pg_to_up_acting_osds(p->first, nullptr, nullptr, nullptr,
                                    &acting_primary);
        if (acting_primary != from) {
             ...
             */
            dout(10) << __func__ << " ignore " << p->first << " -> " << p->
                second << ": primary has changed" << dendl;
            ignore_cnt++;
            continue;
        }
    ...
    dout(7) << "preprocess_pgtemp e" << m->
        map_epoch << " no changes from " << m->get_orig_source_inst() << dendl;
...
bool OSDMonitor::prepare_pgtemp(MonOpRequestRef op)
{
    ...
    auto m = op->get_req < MOSDPGTemp > ();
    int from = m->get_orig_source().num();
    dout(7) << "prepare_pgtemp e" << m->map_epoch << " from " << m->
        get_orig_source_inst() << dendl;
    for (map < pg_t, vector < int32_t > >::iterator p = m->pg_temp.begin();
        ...
        if (pending_inc.old_pools.count(pool)) {
            dout(10) << __func__ << " ignore " << p->first << " -> " << p->
                second << ": pool pending removal" << dendl;
            continue;
        }
...
bool OSDMonitor::preprocess_remove_snaps(MonOpRequestRef op)
{
    ...
    if (!session->caps.is_capable(cct,
        ...
        dout(0) <<
            "got preprocess_remove_snaps from entity with insufficient caps " <<
            session->caps << dendl;
        goto ignore;
    }
...
bool OSDMonitor::preprocess_get_purged_snaps(MonOpRequestRef op)
{
    ...
    while (it->valid()) {
        ...
        if (n != 1) {
            derr << __func__ << " unable to parse key '" << it->
                key() << "'" << dendl;
        }
            ...
            try {
                ...
                derr << __func__ << " unable to parse value for key '" << it->
                    key()
                    << "': \n";
                bl.hexdump(*_dout);
                *_dout << dendl;
...
bool OSDMonitor::prepare_beacon(MonOpRequestRef op)
{
    ...
    if (!src.is_osd() ||
        !osdmap.is_up(from) ||
        !osdmap.get_addrs(from).legacy_equals(beacon->
                                              get_orig_source_addrs())) {
...
MOSDMap *OSDMonitor::build_incremental(epoch_t from, epoch_t to,
                                       uint64_t features)
{
    dout(10) << "build_incremental [" << from << ".." << to <<
        "] with features " << std::hex << features << std::dec << dendl;
...
{
    op->mark_osdmon_event(__func__);
    dout(5) << "send_full to " << op->get_req()->
        get_orig_source_inst() << dendl;
...
void OSDMonitor::send_incremental(epoch_t first,
                                  MonSession * session,
                                  bool onetime, MonOpRequestRef req)
{
    dout(5) << "send_incremental [" << first << ".." << osdmap.
        get_epoch() << "]" << " to " << session->name << dendl;
    ...
    if (first <= session->osd_epoch) {
        dout(10) << __func__ << " " << session->
            name << " should already have epoch " << session->
            osd_epoch << dendl;
    ...
    while (first <= osdmap.get_epoch()) {
        epoch_t last =
            std::min < epoch_t > (first + g_conf()->osd_map_message_max - 1,
                                  osdmap.get_epoch());
...
int OSDMonitor::get_version(version_t ver, uint64_t features, bufferlist & bl)
{
    ...
    if (inc_osd_cache.lookup( {
                             ver, significant_features}
                             , &bl)) {
        return 0;
    }
...
int OSDMonitor::get_full_from_pinned_map(version_t ver, bufferlist & bl)
{
    ...
    for (version_t v = ver - 1; v >= closest_pinned; --v) {
        if (full_osd_cache.lookup( {
                                  v, mon.get_quorum_con_features()}
                                  , &osdm_bl)) {
    ...
    for (version_t v = closest_pinned + 1; v <= ver; ++v) {
        ...
        if (g_conf().get_val < bool > ("mon_debug_extra_checks") &&
            ...
            if (tosdm.get_crc() != inc.full_crc) {
                derr << __func__
                    << "    osdmap crc mismatch! (osdmap crc " << tosdm.
                    get_crc()
...
void OSDMonitor::check_osdmap_sub(Subscription * sub)
{
    ...
    if (sub->next <= osdmap.get_epoch()) {
        if (sub->next >= 1)
            ...
            sub->session->con->
                send_message(build_latest_full(sub->session->con_features));
        if (sub->onetime)
...
void OSDMonitor::check_pg_creates_subs()
{
    ...
    mon.with_session_map([this] (const MonSessionMap & session_map) {
                         auto pg_creates_subs =
                         session_map.subs.find("osd_pg_creates");
                                 ...
                                 pool_opts_t::value_t val)
{
    dout(10) << __func__ << " pool: " << pool_id << " option: " << opt
        << " val: " << val << dendl;
    auto p =
        pending_inc.new_pools.try_emplace(pool_id,
                                          *osdmap.get_pg_pool(pool_id));
    p.first->second.opts.set(opt, val);
}

unsigned OSDMonitor::scan_for_creating_pgs(const mempool::osdmap::map < int64_t,
                                           pg_pool_t > &pools,
                                           const mempool::osdmap::set <
                                           int64_t > &removed_pools,
                                           utime_t modified,
                                           creating_pgs_t *
                                           creating_pgs) const const
{
    unsigned queued = 0;
  for (auto & p:pools) {
        int64_t poolid = p.first;
...
void OSDMonitor::update_creating_pgs()
{
  ...
  for (const auto & pg:creating_pgs.pgs) {
        ...
        auto pgid = pg.first;
        if (!osdmap.pg_exists(pgid)) {
            dout(20) << __func__ << " ignoring " << pgid <<
                " which should not exist" << dendl;
            continue;
        }
        auto mapped = pg.second.create_epoch;
        dout(20) << __func__ << " looking up " << pgid << "@" << mapped <<
            dendl;
        spg_t spgid(pgid);
...
epoch_t OSDMonitor::send_pg_creates(int osd, Connection * con, epoch_t next) const const
{
    ...
    for (auto epoch_pgs = creating_pgs_by_epoch->second.lower_bound(next);
      ...
      for (auto & pg:pgs) {
            ...
            if (create->second.history.epoch_created) {
                dout(20) << __func__ << "   " << pg << " " << create->second.
                    history << " " << create->second.past_intervals << dendl;
                m->pg_extra.emplace(pg,
                                    make_pair(create->second.history,
...
void OSDMonitor::tick()
{
            ...
            if (osdmap.is_down(o) && osdmap.is_in(o) && can_mark_out(o)) {
                ...
                if (g_conf()->mon_osd_adjust_down_out_interval) {
                    ...
                    dout(20) << "osd." << o << " laggy halflife " << halflife <<
                        ...
                        " decay " << decay << dendl;
                    my_grace =
                        decay * (double)xi.laggy_interval *
                        xi.laggy_probability;
                    grace += my_grace;
                }
                ...
                if (down_out_subtree_limit.length()) {
                    int type =
                        osdmap.crush->get_type_id(down_out_subtree_limit);
                    if (type > 0) {
                        if (osdmap.
                            containing_subtree_is_down(cct, o, type,
                                                       &down_cache)) {
                            dout(10) << "tick entire containing " <<
                                down_out_subtree_limit << " subtree for osd." <<
                                o << " is down; resetting timer" << dendl;
                            // reset timer, too.
                ...
                if (down_out || destroyed_out) {
                    dout(10) << "tick marking osd." << o << " OUT after " <<
                        down << " sec (target " << grace << " = " << orig_grace
                        << " + " << my_grace << ")" << dendl;
                    ...
                    mon.clog->
                        info() << "Marking osd." << o <<
                        " out (has been down for " << int (down.
                                                           sec()) <<
                        " seconds)";
                }
                else
    ...
    for (ceph::unordered_map < entity_addr_t, utime_t >::iterator p =
         osdmap.blocklist.begin(); p != osdmap.blocklist.end(); ++p) {
        if (p->second < now) {
            dout(10) << "expiring blocklist item " << p->
                first << " expired " << p->second << " < now " << now << dendl;
...
void OSDMonitor::_set_new_cache_sizes()
{
    ...
    dout(1) << __func__ << " cache_size:" << cache_size
        << " inc_alloc: " << inc_alloc
        << " full_alloc: " << full_alloc << " kv_alloc: " << kv_alloc << dendl;
}

bool OSDMonitor::handle_osd_timeouts(const utime_t & now,
                                     std::map < int, std::pair < utime_t,
                                     int >>&last_osd_report)
{
    ...
    for (int i = 0; i < max_osd; ++i) {
        ...
        if (t == last_osd_report.end()) {
            ...
            utime_t max_timeout(std::
                                max(mon_osd_report_timeout,
                                    2 * t->second.second), 0);
            if (diff > max_timeout) {
                mon.clog->
                    info() << "osd." << i << " marked down after no beacon for "
                    << diff << " seconds";
                derr << "no beacon from osd." << i << " since " << t->second.
                    first << ", " << diff << " seconds ago.  marking down" <<
                    dendl;
...
namespace {
    ...
    std::set < osd_pool_get_choices >
        subtract_second_from_first(const std::set < osd_pool_get_choices >
                                   &first,
                                   const std::set < osd_pool_get_choices >
                                   &second) {
...
bool OSDMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "osd stat") {
        ...
        epoch_t epoch =
            cmd_getval_or < int64_t > (cmdmap, "epoch", osdmap.get_epoch());
        ...
        auto sg = make_scope_guard([&]{
                                   ...
                                   stringstream ds; if (f) {
                                   f->open_object_section("osdmap");
                                   p->dump(f.get(), cct);
                                   f->close_section(); f->flush(ds);}
                                   else {
                                   p->print(cct, ds);}
                                   rdata.append(ds); if (!f)
                                   ds << " ";}
                                   else
                                   if (prefix == "osd ls") {
                                   if (f) {
                                   f->open_array_section("osds");
                                   for (int i = 0; i < osdmap.get_max_osd();
                                        i++) {
                                   if (osdmap.exists(i)) {
                                   f->dump_int("osd", i);}
                                   }
                                   ...
                                   if (prefix == "osd tree"
                                       || prefix == "osd tree-from") {
                                   string bucket; if (prefix == "osd tree-from") {
                                   cmd_getval(cmdmap, "bucket", bucket);
                                   if (!osdmap.crush->name_exists(bucket)) {
                                   ss << "bucket '" << bucket <<
                                   "' does not exist"; r = -ENOENT; goto reply;}
...
vector < string > states; cmd_getval(cmdmap, "states", states); unsigned filter = 0; for (auto & s:states)
                                   {
                                   if (s == "up") {
                                   ...
                                   if (prefix == "osd ls-tree") {
                                   string bucket_name;
                                   cmd_getval(cmdmap, "name", bucket_name);
                                   set < int >osds;
                                   r =
                                   p->get_osds_by_bucket_name(bucket_name,
                                                              &osds);
                                   if (r == -ENOENT) {
                                   ss << "\"" << bucket_name <<
                                   "\" does not exist"; goto reply;}
                                   else
                                   if (r < 0) {
                                   ss << "can not parse bucket name:\"" <<
                                   bucket_name << "\""; goto reply;}

                                   if (f) {
f->open_array_section("osds"); for (auto & i:osds) {
                                   if (osdmap.exists(i)) {
                                   f->dump_int("osd", i);}
                                   }
                                   f->close_section(); f->flush(ds);}
                                   else {
bool first = true; for (auto & i: osds) {
                                   if (osdmap.exists(i)) {
                                   if (!first)
                                   ds << "\n"; first = false; ds << i;}
                                   }
                                   ...
                                   f->dump_unsigned("epoch",
                                                    osdmap.get_epoch());
                                   f->dump_int("max_osd", osdmap.get_max_osd());
                                   f->close_section(); f->flush(rdata);}
                                   else {
                                   ds << "max_osd = " << osdmap.
                                   get_max_osd() << " in epoch " << osdmap.
                                   get_epoch(); rdata.append(ds);}
                                   }
                                   else
                                   if (prefix == "osd utilization") {
                                   string out;
                                   osdmap.summarize_mapping_stats(NULL, NULL,
                                                                  &out,
                                                                  f.get());
                                   if (f)
                                   f->flush(rdata);
                                   else
                                   rdata.append(out); r = 0; goto reply;}
                                   else
                                   if (prefix == "osd find") {
                                   int64_t osd;
...
for (auto & k:                    {
    ...
    // crush is helpful too
                                   ...
                                   osdmap.crush->get_full_location(osd);
                                   for (map < string, string >::iterator p =
                                        loc.begin(); p != loc.end(); ++p)
                                   f->dump_string(p->first.c_str(), p->second);
                                   ...
                                   int64_t osd = -1;
                                   if (cmd_vartype_stringify(cmdmap["id"]).
                                       size()
                                   ...
                                   if (prefix == "osd versions") {
                                   if (!f)
                                   f.reset(Formatter::create("json-pretty"));
                                   count_metadata("ceph_version", f.get());
                                   f->flush(rdata); r = 0;}
                                   else
                                   if (prefix == "osd count-metadata") {
                                   if (!f)
                                   f.reset(Formatter::create("json-pretty"));
                                   string field;
                                   ...
                                   if (prefix == "osd numa-status") {
                                   TextTable tbl; if (f) {
                                   f->open_array_section("osds");}
                                   else {
                                   tbl.define_column("OSD", TextTable::LEFT,
                                                     TextTable::RIGHT);
                                   tbl.define_column("HOST", TextTable::LEFT,
                                                     TextTable::LEFT);
                                   tbl.define_column("NETWORK",
                                                     TextTable::RIGHT,
                                                     TextTable::RIGHT);
                                   tbl.define_column("STORAGE",
                                                     TextTable::RIGHT,
                                                     TextTable::RIGHT);
                                   tbl.define_column("AFFINITY",
                                                     TextTable::RIGHT,
                                                     TextTable::RIGHT);
                                   tbl.define_column("CPUS", TextTable::LEFT,
                                                     TextTable::LEFT);}
                                   for (int i = 0; i < osdmap.get_max_osd();
                                        ++i) {
                                   if (osdmap.exists(i)) {
                                   ...
                                   host = p->second;}
                                   if (f) {
f->open_object_section("osd"); f->dump_int("osd", i); f->dump_string("host", host); for (auto n:{
                                                                                                                            "network_numa_node",
                                                                                                                            "objectstore_numa_node",
                                                                                                                            "numa_node"}
                                   ) {
                                   p = m.find(n); if (p != m.end()) {
...
for (auto n:                      {
                                   ...
                                   p = m.find(n); if (p != m.end()) {
  list < string > ls = get_str_list(p->second, ","); f->open_array_section(n); for (auto node:ls)
                                   {
...
for (auto n:                      {
                                   ...
                                   int64_t pool =
                                   osdmap.lookup_pg_pool_name(poolstr.c_str());
                                   if (pool < 0) {
                                   ss << "pool " << poolstr <<
                                   " does not exist"; r = -ENOENT; goto reply;}
                                   ...
                                   object_t oid(objstr);
                                   pg_t pgid =
                                   osdmap.object_locator_to_pg(oid, oloc);
                                   pg_t mpgid = osdmap.raw_pg_to_pg(pgid);
                                   vector < int >up, acting; int up_p, acting_p;
                                   osdmap.pg_to_up_acting_osds(mpgid, &up,
                                                               &up_p, &acting,
                                                               &acting_p);
                                   string fullobjname;
                                   if (!namespacestr.empty())
                                   fullobjname =
                                   namespacestr + string("/") + oid.name;
                                   else
                                   fullobjname = oid.name; if (f) {
                                   ...
                                   for (vector < int >::iterator p = up.begin();
                                        p != up.end(); ++p)
                                   f->dump_int("osd", *p); f->close_section();
                                   f->dump_int("up_primary", up_p);
                                   f->open_array_section("acting");
                                   for (vector < int >::iterator p =
                                        acting.begin(); p != acting.end(); ++p)
                                   f->dump_int("osd", *p); f->close_section(); f->dump_int("acting_primary", acting_p); f->close_section(); // osd_map
                                   f->flush(rdata);}
                                   else {
                                   ...
                                   pg_t pgid;
                                   vector < int >up, acting;
                                   r = parse_pgid(cmdmap, ss, pgid); if (r < 0)
                                   goto reply;
                                   pg_t mpgid = osdmap.raw_pg_to_pg(pgid);
                                   osdmap.pg_to_up_acting_osds(pgid, up,
                                                               acting); if (f) {
f->open_object_section("pg_map"); f->dump_unsigned("epoch", osdmap.get_epoch()); f->dump_stream("raw_pgid") << pgid; f->dump_stream("pgid") << mpgid; f->open_array_section("up"); for (auto osd:up)
                                   {
                                   f->dump_int("up_osd", osd);}
f->close_section(); f->open_array_section("acting"); for (auto osd:acting)
                                   {
                                   ...
                                   if (prefix == "osd lspools") {
                                   if (f)
                                   f->open_array_section("pools");
                                   for (map < int64_t, pg_pool_t >::iterator p =
                                        osdmap.pools.begin();
                                        p != osdmap.pools.end(); ++p) {
                                   if (f) {
                                   f->open_object_section("pool");
                                   f->dump_int("poolnum", p->first);
                                   f->dump_string("poolname",
                                                  osdmap.pool_name[p->first]);
                                   f->close_section();}
                                   else {
                                   ds << p->first << ' ' << osdmap.pool_name[p->
                                                                             first];
                                   ...
                                   for (ceph::unordered_map < entity_addr_t,
                                        utime_t >::iterator p =
                                        osdmap.blocklist.begin();
                                        p != osdmap.blocklist.end(); ++p) {
                                   if (f) {
                                   f->open_object_section("entry");
                                   f->dump_string("addr",
                                                  p->first.get_legacy_str());
                                   ...
                                   if (f) {
                                   f->close_section(); f->flush(rdata);}
                                   ss << "listed " << osdmap.blocklist.size() +
                                   osdmap.range_blocklist.size() << " entries";}
                                   else
                                   ...
                                   if (f)
f->open_array_section("pools"); for (auto &[pid, pdata]:osdmap.get_pools()) {
                                   if (f) {
                                   if (detail == "detail") {
                                   f->open_object_section("pool");
                                   f->dump_int("pool_id", pid);
                                   f->dump_string("pool_name",
                                                  osdmap.get_pool_name(pid));
                                   pdata.dump(f.get());
                                   osdmap.dump_read_balance_score(cct, pid,
                                                                  pdata,
                                                                  f.get());
                                   f->close_section();}
                                   else {
                                   ...
                                   ostringstream rss; if (f)
                                   f->open_object_section("tunable");
                                   if (tunable == "straw_calc_version") {
                                   if (f)
                                   f->dump_int(tunable.c_str(),
                                               osdmap.crush->
                                   ...
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(pool); string var;
                                   cmd_getval(cmdmap, "var", var);
                                   typedef std::map < std::string,
                                   osd_pool_get_choices > choices_map_t;
                                   ...
                                   typedef std::set < osd_pool_get_choices >
                                   choices_set_t;
                                   const choices_set_t ONLY_TIER_CHOICES = {
                                   HIT_SET_TYPE, HIT_SET_PERIOD, HIT_SET_COUNT,
                                   HIT_SET_FPP, TARGET_MAX_OBJECTS,
                                   TARGET_MAX_BYTES, CACHE_TARGET_FULL_RATIO,
                                   CACHE_TARGET_DIRTY_RATIO,
                                   CACHE_TARGET_DIRTY_HIGH_RATIO,
                                   CACHE_MIN_FLUSH_AGE, CACHE_MIN_EVICT_AGE,
                                   MIN_READ_RECENCY_FOR_PROMOTE,
                                   ...
                                   if (var == "all") {
                                   for (choices_map_t::const_iterator it =
                                        ALL_CHOICES.begin();
                                   ...
                                   choices_map_t::const_iterator found =
                                   ALL_CHOICES.find(var);
                                   if (found == ALL_CHOICES.end()) {
                                   ss << "pool '" << poolstr <<
                                   "': invalid variable: '" << var << "'";
                                   r = -EINVAL; goto reply;}

                                   osd_pool_get_choices selected =
                                   found->second;
                                   if (!p->is_tier()
                                       && ONLY_TIER_CHOICES.find(selected) !=
                                       ONLY_TIER_CHOICES.end()) {
                                   ss << "pool '" << poolstr <<
                                   ...
                                   switch (*it) {
case PG_NUM:
f->dump_int("pg_num", p->get_pg_num()); break; case PGP_NUM:
f->dump_int("pgp_num", p->get_pgp_num()); break; case SIZE:
f->dump_int("size", p->get_size()); break; case MIN_SIZE:
f->dump_int("min_size", p->get_min_size()); break; case CRUSH_RULE:
                                   if (osdmap.crush->
                                       rule_exists(p->get_crush_rule())) {
                                   ...
                                   f->dump_string("crush_rule",
                                                  stringify(p->
                                                            get_crush_rule()));}
break; case EC_OVERWRITES:
f->dump_bool("allow_ec_overwrites", p->has_flag(pg_pool_t::FLAG_EC_OVERWRITES)); break; case PG_AUTOSCALE_MODE:
f->dump_string("pg_autoscale_mode", pg_pool_t::get_pg_autoscale_mode_name(p->pg_autoscale_mode)); break; case HASHPSPOOL:
case POOL_EIO:
case NODELETE:
case BULK:
case NOPGCHANGE:
...
case NOSCRUB:
case NODEEP_SCRUB:
f->dump_bool(i->first.c_str(), p->has_flag(pg_pool_t::get_flag_by_name(i->first))); break; case HIT_SET_PERIOD:
f->dump_int("hit_set_period", p->hit_set_period); break; case HIT_SET_COUNT:
f->dump_int("hit_set_count", p->hit_set_count); break; case HIT_SET_TYPE:
f->dump_string("hit_set_type", HitSet::get_type_name(p->hit_set_params.get_type())); break; case HIT_SET_FPP:
                                   {
                                   if (p->hit_set_params.get_type() ==
                                       HitSet::TYPE_BLOOM) {
                                   BloomHitSet::Params * bloomp =
                                   static_cast <
                                   BloomHitSet::Params *
                                   >(p->hit_set_params.impl.get());
                                   f->dump_float("hit_set_fpp",
                                                 bloomp->get_fpp());}
                                   else
                                   if (var != "all") {
                                   f->close_section();
...
break; case USE_GMT_HITSET:
f->dump_bool("use_gmt_hitset", p->use_gmt_hitset); break; case TARGET_MAX_OBJECTS:
f->dump_unsigned("target_max_objects", p->target_max_objects); break; case TARGET_MAX_BYTES:
f->dump_unsigned("target_max_bytes", p->target_max_bytes); break; case CACHE_TARGET_DIRTY_RATIO:
f->dump_unsigned("cache_target_dirty_ratio_micro", p->cache_target_dirty_ratio_micro); f->dump_float("cache_target_dirty_ratio", ((float)p->cache_target_dirty_ratio_micro / 1000000)); break; case CACHE_TARGET_DIRTY_HIGH_RATIO:
f->dump_unsigned("cache_target_dirty_high_ratio_micro", p->cache_target_dirty_high_ratio_micro); f->dump_float("cache_target_dirty_high_ratio", ((float)p->cache_target_dirty_high_ratio_micro / 1000000)); break; case CACHE_TARGET_FULL_RATIO:
f->dump_unsigned("cache_target_full_ratio_micro", p->cache_target_full_ratio_micro); f->dump_float("cache_target_full_ratio", ((float)p->cache_target_full_ratio_micro / 1000000)); break; case CACHE_MIN_FLUSH_AGE:
f->dump_unsigned("cache_min_flush_age", p->cache_min_flush_age); break; case CACHE_MIN_EVICT_AGE:
f->dump_unsigned("cache_min_evict_age", p->cache_min_evict_age); break; case ERASURE_CODE_PROFILE:
f->dump_string("erasure_code_profile", p->erasure_code_profile); break; case MIN_READ_RECENCY_FOR_PROMOTE:
f->dump_int("min_read_recency_for_promote", p->min_read_recency_for_promote); break; case MIN_WRITE_RECENCY_FOR_PROMOTE:
f->dump_int("min_write_recency_for_promote", p->min_write_recency_for_promote); break; case FAST_READ:
f->dump_int("fast_read", p->fast_read); break; case HIT_SET_GRADE_DECAY_RATE:
f->dump_int("hit_set_grade_decay_rate", p->hit_set_grade_decay_rate); break; case HIT_SET_SEARCH_LAST_N:
f->dump_int("hit_set_search_last_n", p->hit_set_search_last_n); break; case SCRUB_MIN_INTERVAL:
...
case DEDUP_CDC_CHUNK_SIZE:
                                   ...
                                   pool_opts_t::get_opt_desc(i->first).key;
...
case NODEEP_SCRUB:
                                   ...
                                   if (i->second == *it)
                                   break;}
ceph_assert(i != ALL_CHOICES.end()); ss << i->first << ": " << (p->has_flag(pg_pool_t::get_flag_by_name(i->first)) ? "true" : "false") << "\n"; break; case MIN_WRITE_RECENCY_FOR_PROMOTE:
ss << "min_write_recency_for_promote: " << p->min_write_recency_for_promote << "\n"; break; case FAST_READ:
ss << "fast_read: " << p->fast_read << "\n"; break; case SCRUB_MIN_INTERVAL:
case SCRUB_MAX_INTERVAL:
case DEEP_SCRUB_INTERVAL:
case RECOVERY_PRIORITY:
case RECOVERY_OP_PRIORITY:
...
case DEDUP_CDC_CHUNK_SIZE:
                                   ...
                                   int64_t poolid =
                                   osdmap.lookup_pg_pool_name(pool_name);
                                   if (poolid < 0) {
                                   ceph_assert(poolid == -ENOENT);
                                   ss << "unrecognized pool '" << pool_name <<
                                   "'"; r = -ENOENT; goto reply;}
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(poolid);
                                   const pool_stat_t * pstat =
                                   mon.mgrstatmon()->get_pool_stat(poolid);
                                   if (!pstat) {
                                   ss << "no stats for pool '" << pool_name <<
                                   "'"; r = -ENOENT; goto reply;}
                                   const object_stat_sum_t & sum =
                                   pstat->stats.sum; if (f) {
                                   f->open_object_section("pool_quotas");
                                   f->dump_string("pool_name", pool_name);
                                   f->dump_unsigned("pool_id", poolid);
                                   f->dump_unsigned("quota_max_objects",
                                                    p->quota_max_objects);
                                   f->dump_int("current_num_objects",
                                               sum.num_objects);
                                   f->dump_unsigned("quota_max_bytes",
                                                    p->quota_max_bytes);
                                   f->dump_int("current_num_bytes",
                                               sum.num_bytes);
                                   f->close_section(); f->flush(rdata);}
                                   else {
                                   stringstream rs;
                                   ...
                                   rs << "  (current num bytes: " << sum.
                                   num_bytes << " bytes)";}
                                   rdata.append(rs.str());}
                                   rdata.append("\n"); r = 0;}
                                   else
                                   if (prefix == "osd crush rule list" ||
                                   ...
                                   if (prefix == "osd crush rule ls-by-class") {
                                   string class_name;
                                   ...
                                   set < int >rules;
                                   r =
                                   osdmap.crush->get_rules_by_class(class_name,
                                                                    &rules);
                                   if (r < 0) {
                                   ss << "failed to get rules by class '" <<
                                   class_name << "'"; goto reply;}
                                   if (f) {
f->open_array_section("rules"); for (auto & rule:rules) {
                                   f->dump_string("name",
                                                  osdmap.crush->
                                                  get_rule_name(rule));}
                                   f->close_section(); f->flush(rdata);}
                                   else {
ostringstream rs; for (auto & rule:rules) {
                                   rs << osdmap.crush->
                                   get_rule_name(rule) << "\n";}
                                   rdata.append(rs.str());}
                                   }
                                   ...
                                   f->close_section(); ostringstream rs;
                                   f->flush(rs); rs << "\n";
                                   rdata.append(rs.str());}
                                   else
                                   if (prefix == "osd crush show-tunables") {
                                   string format;
                                   cmd_getval(cmdmap, "format", format);
                                   boost::scoped_ptr < Formatter >
                                   f(Formatter::
                                   ...
                                   if (!cmd_getval_compat_cephbool
                                       (cmdmap, "show_shadow", show_shadow)) {
                                   std::string shadow;
                                   ...
                                   boost::scoped_ptr < Formatter >
                                   f(Formatter::create(format)); if (f) {
                                   f->open_object_section("crush_tree");
                                   osdmap.crush->dump_tree(nullptr, f.get(),
                                                           osdmap.
                                                           get_pool_names(),
                                                           show_shadow);
                                   f->close_section(); f->flush(rdata);}
                                   ...
                                   list < int >result; if (id >= 0) {
                                   result.push_back(id);}
                                   else {
                                   int num = osdmap.crush->get_bucket_size(id);
                                   for (int i = 0; i < num; ++i) {
                                   result.push_back(osdmap.crush->
                                                    get_bucket_item(id, i));}
                                   }
                                   if (f) {
f->open_array_section("items"); for (auto i:result) {
                                   f->dump_string("item",
                                                  osdmap.crush->
                                                  get_item_name(i));}
                                   f->close_section(); f->flush(rdata);}
                                   else {
ostringstream ss; for (auto i:    result) {
                                   ss << osdmap.crush->get_item_name(i) << "\n";}
                                   rdata.append(ss.str());}
                                   r = 0;}
                                   else
                                   if (prefix == "osd crush class ls") {
boost::scoped_ptr < Formatter > f(Formatter::create(format, "json-pretty", "json-pretty")); f->open_array_section("crush_classes"); for (auto i:osdmap.crush->class_name)
                                   f->dump_string("class", i.second);
                                   f->close_section(); f->flush(rdata);}
                                   else
                                   if (prefix == "osd crush class ls-osd") {
                                   string name;
                                   cmd_getval(cmdmap, "class", name);
                                   set < int >osds;
                                   osdmap.crush->get_devices_by_class(name,
                                                                      &osds);
                                   if (f) {
f->open_array_section("osds"); for (auto & osd:osds)
                                   f->dump_int("osd", osd);
                                   f->close_section(); f->flush(rdata);}
                                   else {
bool first = true; for (auto & osd:osds) {
                                   if (!first)
                                   ds << "\n"; first = false; ds << osd;}
                                   rdata.append(ds);}
                                   }
                                   else
                                   if (prefix == "osd crush get-device-class") {
vector < string > idvec; cmd_getval(cmdmap, "ids", idvec); map < int, string > class_by_osd; for (auto & id:idvec) {
                                   ostringstream ts;
                                   ...
                                   "'"; r = -EINVAL; goto reply;}
                                   auto device_class =
                                   osdmap.crush->get_item_class(osd);
                                   if (device_class)
                                   ...
                                   if (f) {
f->open_array_section("osd_device_classes"); for (auto & i:class_by_osd) {
        ...
        // note that we do not group osds by class here
                                   for (auto it = class_by_osd.begin();
                                        it != class_by_osd.end(); it++) {
                                   ds << "osd." << it->first << ' ' << it->
                                   second; if (next(it) != class_by_osd.end())
                                   ds << '\n';}
                                   }
                                   rdata.append(ds);}
                                   }
                                   ...
                                   const auto & profiles =
                                   osdmap.get_erasure_code_profiles(); if (f)
                                   f->
                                   open_array_section("erasure-code-profiles");
                                   for (auto i = profiles.begin();
                                        i != profiles.end(); ++i) {
                                   if (f)
...
for (auto & i:                    osdmap.crush->choose_args) {
                                   ...
                                   if (prefix == "osd crush weight-set dump") {
                                   boost::scoped_ptr < Formatter >
                                   f(Formatter::
                                     create(format, "json-pretty",
                                            "json-pretty"));
                                   ...
                                   ss << "unknown erasure code profile '" <<
                                   name << "'"; r = -ENOENT; goto reply;}
                                   const map < string, string > &profile =
                                   osdmap.get_erasure_code_profile(name); if (f)
                                   f->open_object_section("profile");
                                   for (map < string,
                                        string >::const_iterator i =
                                        profile.begin(); i != profile.end();
                                        ++i) {
                                   if (f)
                                   f->dump_string(i->first.c_str(),
                                                  i->second.c_str());
                                   ...
                                   f(Formatter::
                                     create(format, "json-pretty",
                                            "json-pretty")); string pool_name;
                                   cmd_getval(cmdmap, "pool", pool_name);
                                   string app; cmd_getval(cmdmap, "app", app);
                                   string key; cmd_getval(cmdmap, "key", key);
                                   if (pool_name.empty()) {
      // all
f->open_object_section("pools"); for (const auto & pool:osdmap.pools) {
                                   std::string name("<unknown>");
                                   const auto & pni =
                                   osdmap.pool_name.find(pool.first);
                                   if (pni != osdmap.pool_name.end())
name = pni->second; f->open_object_section(name.c_str()); for (auto & app_pair:pool.second.application_metadata) {
f->open_object_section(app_pair.first.c_str()); for (auto & kv_pair:app_pair.second) {
                                   f->dump_string(kv_pair.first.c_str(),
                                                  kv_pair.second);}
                                   f->close_section();}
                                   ...
                                   // filter by pool
                                   if (app.empty()) {
f->open_object_section(pool_name.c_str()); for (auto & app_pair:p->application_metadata) {
f->open_object_section(app_pair.first.c_str()); for (auto & kv_pair:app_pair.second) {
                                   f->dump_string(kv_pair.first.c_str(),
                                                  kv_pair.second);}
                                   ...
                                   f->flush(rdata); goto reply;}

                                   auto app_it =
                                   p->application_metadata.find(app);
                                   if (app_it ==
                                       p->application_metadata.end()) {
                                   ss << "pool '" << pool_name <<
                                   "' has no application '" << app << "'";
                                   r = -ENOENT; goto reply;}
      // filter by pool + app
                                   if (key.empty()) {
f->open_object_section(app_it->first.c_str()); for (auto & kv_pair:app_it->second) {
                                   f->dump_string(kv_pair.first.c_str(),
                                                  kv_pair.second);}
                                   f->close_section();  // application
                                   f->flush(rdata); goto reply;}
      // filter by pool + app + key
                                   ...
                                   ss << "application '" << app << "' on pool '"
                                   << pool_name << "' does not have key '" <<
                                   key << "'"; r = -ENOENT; goto reply;}
                                   ss << key_it->second << "\n";
                                   rdata.append(ss.str()); ss.str("");}
                                   }
                                   else
                                   ...
                                   ss << osdmap.
                                   require_min_compat_client << std::endl;
                                   rdata.append(ss.str()); ss.str("");
                                   goto reply;}
                                   else
...
reply:
                                   string rs;
                                   getline(ss, rs);
                                   mon.reply_command(op, r, rs, rdata,
                                                     get_last_committed());
                                   return true;}

                                   void OSDMonitor::
                                   set_pool_flags(int64_t pool_id,
                                                  uint64_t flags) {
                                   pg_pool_t * pool =
                                   pending_inc.get_new_pool(pool_id,
                                                            osdmap.
                                                            get_pg_pool
                                                            (pool_id));
                                   ceph_assert(pool); pool->set_flag(flags);}

                                   void OSDMonitor::
                                   clear_pool_flags(int64_t pool_id,
                                                    uint64_t flags) {
                                   pg_pool_t * pool =
                                   pending_inc.get_new_pool(pool_id,
                                                            osdmap.
                                                            get_pg_pool
                                                            (pool_id));
                                   ceph_assert(pool); pool->unset_flag(flags);}

                                   string OSDMonitor::
                                   make_purged_snap_epoch_key(epoch_t epoch) {
                                   char k[80];
                                   snprintf(k, sizeof(k), "purged_epoch_%08lx",
                                            (unsigned long)epoch); return k;}
                                   ...
                                   encode(snap + num, *v);
                                   encode(epoch, *v);
                                   return make_purged_snap_key(pool,
                                                               snap + num - 1);}

                                   int OSDMonitor::
                                   lookup_purged_snap(int64_t pool,
                                                      snapid_t snap,
                                                      snapid_t * begin,
                                                      snapid_t * end) {
                                   string k = make_purged_snap_key(pool, snap);
                                   auto it =
                                   mon.store->get_iterator(OSD_SNAP_PREFIX);
                                   it->lower_bound(k); if (!it->valid()) {
                                   dout(20) << __func__ << " pool " << pool <<
                                   " snap " << snap << " - key '" << k <<
                                   "' not found" << dendl; return -ENOENT;}
                                   if (it->key().find("purged_snap_") != 0) {
                                   dout(20) << __func__
                                   << " pool " << pool << " snap " << snap
                                   << " - key '" << k << "' got '" << it->key()
                                   ...
                                   int n =
                                   sscanf(gotk.c_str(), format, &keypool);
                                   if (n != 1) {
                                   derr << __func__ << " invalid k '" << gotk <<
                                   "'" << dendl; return -ENOENT;}
                                   if (pool != keypool) {
                                   dout(20) << __func__
                                   << " pool " << pool << " snap " << snap
                                   ...
                                   << " pool " << pool << " snap " << snap
                                   << " - found [" << *begin << "," << *end <<
                                   "), no overlap" << dendl; return -ENOENT;}
                                   return 0;}

                                   void OSDMonitor::
                                   insert_purged_snap_update(int64_t pool,
                                                             snapid_t start,
                                                             snapid_t end,
                                                             epoch_t epoch,
                                                             MonitorDBStore::
                                   ...
                                   int b =
                                   lookup_purged_snap(pool, start - 1,
                                                      &before_begin,
                                                      &before_end);
                                   int a =
                                   lookup_purged_snap(pool, end, &after_begin,
                                                      &after_end);
                                   if (!b && !a) {
                                   dout(10) << __func__ << " [" << start << ","
                                   << end << ") - joins [" << before_begin <<
                                   "," << before_end << ") and [" << after_begin
                                   << "," << after_end << ")" << dendl;
                                   // erase only the begin record; we'll overwrite the end one.
                                   t->erase(OSD_SNAP_PREFIX,
                                   ...
                                   if (!max_prune) {
                                   max_prune = 100000;}
  dout(10) << __func__ << " max_prune " << max_prune << dendl; unsigned actually_pruned = 0; auto & purged_snaps = mon.mgrstatmon()->get_digest().purged_snaps; for (auto & p:osdmap.get_pools())
                                   {
                                   ...
                                   dout(20) << __func__ << "  we've already purged " << pbegin << "~" << (pend - pbegin) << dendl; break;   // next pool
                                   }
                                   ...
                                   auto r =
                                   osdmap.removed_snaps_queue.find(p.first);
                                   if (r != osdmap.removed_snaps_queue.end()) {
                                   actual.intersection_of(to_prune, r->second);}
                                   actually_pruned += actual.size();
                                   dout(10) << __func__ << " pool " << p.
                                   first << " reports pruned " << to_prune <<
                                   ", actual pruned " << actual << dendl;
                                   if (!actual.empty()) {
                                   pending_inc.new_purged_snaps[p.first].
                                   swap(actual);}
                                   }
                                   if (actually_pruned >= max_prune) {
                                   break;}
                                   }
                                   dout(10) << __func__ << " actually pruned "
                                   << actually_pruned << dendl;
                                   ...
                                   for (auto it = pools.begin();
                                        it != pools.end(); ++it) {
                                   const pool_stat_t * pstat =
                                   mon.mgrstatmon()->get_pool_stat(it->first);
                                   if (!pstat)
                                   continue;
                                   const object_stat_sum_t & sum =
                                   pstat->stats.sum;
                                   ...
                                   warn() << "pool '" << pool_name <<
                                   "' is full" <<
                                   " (reached quota's max_objects: " << pool.
                                   quota_max_objects << ")";}
                                   ...
                                   set_pool_flags(it->first,
                                                  pg_pool_t::
                                                  FLAG_FULL_QUOTA | pg_pool_t::
                                                  FLAG_FULL);
                                   clear_pool_flags(it->first,
                                                    pg_pool_t::
                                                    FLAG_NEARFULL | pg_pool_t::
                                                    FLAG_BACKFILLFULL);
                                   ret = true;}
                                   }
                                   return ret;}
                                   ...
                                   int OSDMonitor::
                                   prepare_new_pool(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   dout(10) << "prepare_new_pool from " << m->
                                   get_connection() << dendl;
                                   MonSession * session = op->get_session();
                                   if (!session)
                                   return -EPERM; string erasure_code_profile;
                                   stringstream ss; string rule_name;
                                   bool bulk = false; int ret = 0;
                                   ret =
                                   prepare_new_pool(m->name, m->crush_rule,
                                                    rule_name, 0, 0, 0, 0, 0, 0,
                                                    0.0, erasure_code_profile,
                                                    ...
                                                    &ss); if (ret < 0) {
                                   dout(10) << __func__ << " got " << ret << " "
                                   << ss.str() << dendl;}
                                   return ret;}

                                   int OSDMonitor::
                                   crush_rename_bucket(const string & srcname,
                                   ...
                                   ret =
                                   _get_stable_crush().
                                   can_rename_bucket(srcname, dstname, ss);
                                   if (ret)
                                   return ret;}
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   *ss << "renamed bucket " << srcname <<
                                   " into " << dstname; return 0;}

                                   void OSDMonitor::
                                   check_legacy_ec_plugin(const string & plugin,
                                                          const string &
                                                          profile)
                                   const
                                   {
                                   ...
                                   if (replacement != "") {
                                   dout(0) << "WARNING: erasure coding profile "
                                   << profile << " uses plugin " << plugin <<
                                   " that has been deprecated. Please use " <<
                                   replacement << " instead." << dendl;}
                                   }

                                   int OSDMonitor::
                                   normalize_profile(const string & profilename,
                                                     ErasureCodeProfile &
                                                     profile, bool force,
                                                     ostream * ss) {
                                   ...
                                   profile.find("plugin");
                                   check_legacy_ec_plugin(plugin->second,
                                                          profilename);
                                   int err =
                                   instance.factory(plugin->second,
                                                    g_conf().get_val <
                                                    std::string >
                                                    ("erasure_code_dir"),
                                   ...
                                   strict_iecstrtoll(it->second, &err_str);
                                   if (!err_str.empty()) {
                                   *ss << "could not parse stripe_unit '" <<
                                   it->second << "': " << err_str << std::endl;
                                   return -EINVAL;}
                                   uint32_t data_chunks =
                                   erasure_code->get_data_chunk_count();
                                   uint32_t chunk_size =
                                   erasure_code->get_chunk_size(stripe_unit *
                                                                data_chunks);
                                   if (chunk_size != stripe_unit) {
                                   *ss << "stripe_unit " << stripe_unit <<
                                   " does not match ec profile " <<
                                   "alignment. Would be padded to " <<
                                   chunk_size << std::endl; return -EINVAL;}
                                   if ((stripe_unit % 4096) != 0 && !force) {
                                   *ss <<
                                   "stripe_unit should be a multiple of 4096 bytes for best performance."
                                   << "use --force to override this check" <<
                                   std::endl; return -EINVAL;}
                                   }
                                   return 0;}

                                   int OSDMonitor::
                                   crush_rule_create_erasure(const string &
                                                             name,
                                                             const string &
                                                             profile, int *rule,
                                                             ostream * ss) {
                                   ...
                                   if (ruleid != -ENOENT) {
                                   *rule = ruleid; return -EEXIST;}

                                   CrushWrapper newcrush = _get_pending_crush();
                                   ruleid = newcrush.get_rule_id(name);
                                   if (ruleid != -ENOENT) {
                                   *rule = ruleid; return -EALREADY;}
                                   else {
                                   ErasureCodeInterfaceRef erasure_code;
                                   int err =
                                   get_erasure_code(profile, &erasure_code, ss);
                                   if (err) {
                                   *ss << "failed to load plugin using profile "
                                   << profile << std::endl; return err;}

                                   err =
                                   erasure_code->create_rule(name, newcrush,
                                                             ss);
                                   erasure_code.reset(); if (err < 0)
                                   return err; *rule = err;
                                   pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                   ...
                                   int OSDMonitor::
                                   get_erasure_code(const string &
                                                    erasure_code_profile,
                                                    ErasureCodeInterfaceRef *
                                                    erasure_code, ostream * ss)
                                   const
                                   {
                                   if (pending_inc.
                                       has_erasure_code_profile
                                       (erasure_code_profile))
                                   return -EAGAIN;
                                   ...
                                   *ss <<
                                   ...
                                   << profile << std::endl; return -EINVAL;}
                                   check_legacy_ec_plugin(plugin->second,
                                                          erasure_code_profile);
                                   auto & instance =
                                   ErasureCodePluginRegistry::instance();
                                   ...
                                   int OSDMonitor::
                                   check_cluster_features(uint64_t features,
                                                          stringstream & ss) {
                                   ...
                                   for (set < int32_t >::iterator it =
                                        up_osds.begin(); it != up_osds.end();
                                        ++it) {
                                   const osd_xinfo_t & xi =
                                   osdmap.get_xinfo(*it);
                                   ...
                                   for (map < int32_t,
                                        osd_xinfo_t >::const_iterator p =
                                        pending_inc.new_xinfo.begin();
                                        p != pending_inc.new_xinfo.end(); ++p) {
                                   const osd_xinfo_t & xi = p->second;
                                   if ((xi.features & features) != features) {
                                   dout(10) << __func__ << " pending osd." <<
                                   p->
                                   first << " features are insufficient; retry"
                                   << dendl; return -EAGAIN;}
                                   }
                                   ...
                                   bool OSDMonitor::
                                   validate_crush_against_features(const
                                                                   CrushWrapper
                                                                   * newcrush,
                                                                   stringstream
                                                                   & ss) {
                                   OSDMap::Incremental new_pending =
                                   pending_inc;
                                   ...
                                   // client compat
                                   if (newmap.require_min_compat_client !=
                                       ceph_release_t::unknown) {
                                   auto mv = newmap.get_min_compat_client();
                                   if (mv > newmap.require_min_compat_client) {
                                   ss <<
                                   "new crush map requires client version " <<
                                   mv << " but require_min_compat_client is " <<
                                   ...
                                   // osd compat
                                   uint64_t features =
                                   newmap.get_features(CEPH_ENTITY_TYPE_MON,
                                                       NULL) | newmap.
                                   get_features(CEPH_ENTITY_TYPE_OSD, NULL);
                                   stringstream features_ss;
                                   int r =
                                   check_cluster_features(features,
                                                          features_ss); if (r) {
                                   ss << "Could not change CRUSH: " <<
                                   features_ss.str(); return false;}

                                   return true;}

                                   bool OSDMonitor::
                                   erasure_code_profile_in_use(const mempool::
                                                               osdmap::map <
                                                               int64_t,
                                                               pg_pool_t >
                                                               &pools,
                                   ...
                                   bool found = false;
                                   for (map < int64_t,
                                        pg_pool_t >::const_iterator p =
                                        pools.begin(); p != pools.end(); ++p) {
                                   ...
                                   *ss << osdmap.pool_name[p->first] << " ";
                                   ...
                                   if (found) {
                                   *ss <<
                                   "pool(s) are using the erasure code profile '"
                                   << profile << "'";}
                                   return found;}

                                   int OSDMonitor::
                                   parse_erasure_code_profile(const vector <
                                                              string >
                                                              &erasure_code_profile,
                                                              map < string,
                                                              string >
                                                              *erasure_code_profile_map,
                                                              ostream * ss) {
                                   int r =
                                   g_conf().with_val < string >
                                   ("osd_pool_default_erasure_code_profile",
                                    get_json_str_map, *ss,
                                    erasure_code_profile_map, true); if (r)
                                   return r;
                                   ceph_assert((*erasure_code_profile_map).
                                               count("plugin"));
                                   string default_plugin =
                                   (*erasure_code_profile_map)["plugin"];
                                   map < string, string > user_map;
                                   for (vector < string >::const_iterator i =
                                        erasure_code_profile.begin();
                                   ...
                                   *ss << "property '" << key <<
                                   "' is no longer supported; try " << "'crush-"
                                   << key.substr(8) << "' instead";
                                   return -EINVAL;}
                                   user_map[key] = value;
                                   (*erasure_code_profile_map)[key] = value;}
                                   }

                                   if (user_map.count("plugin")
                                       && user_map["plugin"] != default_plugin)
                                   (*erasure_code_profile_map) = user_map;
                                   return 0;}

                                   int OSDMonitor::
                                   prepare_pool_size(const unsigned pool_type,
                                                     const string &
                                                     erasure_code_profile,
                                                     uint8_t repl_size,
                                                     unsigned *size,
                                                     unsigned *min_size,
                                                     ostream * ss) {
                                   int err = 0; bool set_min_size = false;
                                   switch (pool_type) {
case pg_pool_t::TYPE_REPLICATED:
                                   ...
                                   *size = repl_size; if (!set_min_size)
* min_size = g_conf().get_osd_pool_default_min_size(repl_size); break; case pg_pool_t::TYPE_ERASURE:
                                   {
                                   if (osdmap.stretch_mode_enabled) {
                                   *ss <<
...
break; default:
                                   *ss << "prepare_pool_size: " << pool_type <<
                                   " is not a known pool type"; err = -EINVAL;
                                   break;}
                                   return err;}

                                   int OSDMonitor::
                                   prepare_pool_stripe_width(const unsigned
                                                             pool_type,
                                                             const string &
                                                             erasure_code_profile,
                                                             uint32_t *
                                                             stripe_width,
                                                             ostream * ss) {
                                   int err = 0; switch (pool_type) {
case pg_pool_t::TYPE_REPLICATED:
                                   // ignored
break; case pg_pool_t::TYPE_ERASURE:
                                   {
                                   ErasureCodeProfile profile =
                                   osdmap.
                                   get_erasure_code_profile
                                   ...
                                   err =
                                   get_erasure_code(erasure_code_profile,
                                                    &erasure_code, ss); if (err)
                                   break;
                                   uint32_t data_chunks =
                                   erasure_code->get_data_chunk_count();
                                   uint32_t stripe_unit =
                                   g_conf().get_val < Option::size_t >
...
break; default:
                                   *ss << "prepare_pool_stripe_width: "
                                    ...
                                    * the most users!
                                    */
  map < int, int >rule_counts; for (const auto & pooli:osdmap.pools)
                                   {
  ...
  int most_used_count = 0; int most_used_rule = -1; for (auto i:rule_counts)
                                   {
                                   ...
                                   int OSDMonitor::
                                   prepare_pool_crush_rule(const unsigned
                                                           pool_type,
                                                           const string &
                                                           erasure_code_profile,
                                                           const string &
...
case pg_pool_t::TYPE_REPLICATED:
                                   ...
                                   *crush_rule =
                                   osdmap.crush->
                                   get_osd_pool_default_crush_replicated_rule
                                   (cct);}
                                   if (*crush_rule < 0) {
                                   ...
                                   *ss <<
                                   ...
                                   return get_crush_rule(rule_name, crush_rule,
                                                         ss);}
                                   }
break; case pg_pool_t::TYPE_ERASURE:
                                   {
...
err = -EAGAIN; break; case -EEXIST:
                                   err = 0; break;}
                                   return err;}
break; default:
                                   *ss << "prepare_pool_crush_rule: " <<
                                   pool_type << " is not a known pool type";
                                   return -EINVAL;}
                                   }
                                   else {
                                   if (!osdmap.crush->rule_exists(*crush_rule)) {
                                   *ss << "CRUSH rule " << *crush_rule <<
                                   ...
                                   int OSDMonitor::
                                   get_crush_rule(const string & rule_name,
                                                  int *crush_rule,
                                                  ostream * ss) {
                                   int ret;
                                   ...
                                   *crush_rule = ret;}
                                   ...
                                   dout(20) << __func__ << ": rule " <<
                                   rule_name << " try again" << dendl;
                                   return -EAGAIN;}
                                   else {
                                   // Cannot find it , return error
                                   *ss << "specified rule " << rule_name <<
                                   " doesn't exist"; return ret;}
                                   }
                                   return 0;}
...
*/
                                   uint32_t OSDMonitor::
                                   get_osd_num_by_crush(int crush_rule) {
  set < int >out_osds; set < int >crush_in_osds; set < int >roots; CrushWrapper newcrush = _get_pending_crush(); newcrush.find_takes_by_rule(crush_rule, &roots); for (auto root:roots)
                                   {
                                   const char *rootname =
                                   newcrush.get_item_name(root);
                                   set < int >crush_all_osds;
                                   newcrush.get_leaves(rootname,
                                                       &crush_all_osds);
  ...
  osd_num_by_crush = get_osd_num_by_crush(crush_rule); osdmap.get_pool_ids_by_rule(crush_rule, &crush_pool_ids); for (const auto &[pool_id, pool_info]:osdmap.get_pools())
                                   {
                                   ...
                                   projected +=
                                   pool_info.get_pg_num_target() *
                                   pool_info.get_size();}
                                   }
                                   }
                                   // assume min cluster size 3
                                   osd_num_by_crush =
                                   std::max(osd_num_by_crush, 3u);
                                   auto projected_pgs_per_osd =
                                   projected / osd_num_by_crush;
                                   if (projected_pgs_per_osd >
                                       max_pgs_per_osd) {
                                   if (pool >= 0) {
                                   *ss << "pool id " << pool;}
                                   *ss << " pg_num " << pg_num
 ...
 */
                                   ...
                                   pg_num =
                                   pg_num_from_mode(pg_autoscale_mode.
                                   ...
                                   pgp_num =
                                   g_conf().get_val < uint64_t >
                                   ("osd_pool_default_pgp_num"); if (!pgp_num)
                                   pgp_num = pg_num;
                                   if (pg_num > g_conf().get_val < uint64_t >
                                       ("mon_max_pool_pg_num")) {
                                   *ss <<
                                   ...
                                   *ss <<
                                   ...
                                   if (r) {
                                   dout(10) <<
                                   "prepare_pool_crush_rule returns " << r <<
                                   dendl; return r;}
                                   ...
                                   prepare_pool_size(pool_type,
                                                     ...
                                                     repl_size, &size,
                                                     &min_size, ss); if (r) {
                                   dout(10) << "prepare_pool_size returns " << r
                                   << dendl; return r;}
                                   ...
                                   r =
                                   tester.test_with_fork(cct,
                                                         g_conf()->mon_lease);
                                   dout(10) << __func__ <<
                                   " crush test_with_fork tester created " <<
                                   dendl;
                                   auto duration =
                                   ceph::coarse_mono_clock::now() - start;
                                   if (r < 0) {
                                   dout(10) << "tester.test_with_fork returns "
                                   << r << ": " << err.str() << dendl;
                                   *ss << "crush test failed with " << r << ": "
                                   << err.str(); return r;}
                                   dout(10) << __func__ <<
                                   " crush smoke test duration: " << duration <<
                                   dendl;}
                                   r =
                                   check_pg_num(-1, pg_num, size, crush_rule,
                                                ss); if (r) {
                                   dout(10) << "check_pg_num returns " << r <<
                                   dendl; return r;}
                                   ...
                                   if (osdmap.crush->
                                       ...
                                       (int)pool_type) {
                                   *ss << "crush rule " << crush_rule <<
                                   " type does not match pool"; return -EINVAL;}
                                   ...
                                   if (r) {
                                   dout(10) <<
                                   "prepare_pool_stripe_width returns " << r <<
                                   dendl; return r;}
                                   ...
                                   switch (fast_read) {
case FAST_READ_OFF:
fread = false; break; case FAST_READ_ON:
fread = true; break; case FAST_READ_DEFAULT:
fread = g_conf()->osd_pool_default_ec_fast_read; break; default:
                                   *ss << "invalid fast_read setting: " <<
                                   fast_read; return -EINVAL;}
                                   }

                                   for (map < int64_t, string >::iterator p =
                                        pending_inc.new_pool_names.begin();
                                   ...
                                   pg_pool_t empty;
                                   pg_pool_t * pi =
                                   pending_inc.get_new_pool(pool, &empty);
                                   pi->create_time = ceph_clock_now();
                                   pi->type = pool_type; pi->fast_read = fread;
                                   pi->flags = g_conf()->osd_pool_default_flags;
                                   if (bulk) {
                                   pi->set_flag(pg_pool_t::FLAG_BULK);}
                                   else
                                   if (g_conf()->osd_pool_default_flag_bulk) {
                                   pi->set_flag(pg_pool_t::FLAG_BULK);}
                                   if (g_conf()->
                                   ...
                                   pi->use_gmt_hitset = false; if (crimson) {
                                   ...
                                   osdmap.stretch_mode_bucket;
                                   pi->peering_crush_mandatory_member =
                                   CRUSH_ITEM_NONE;
                                   if (osdmap.degraded_stretch_mode) {
                                   pi->peering_crush_bucket_count =
                                   osdmap.degraded_stretch_mode;
                                   pi->peering_crush_bucket_target =
                                   osdmap.degraded_stretch_mode;
                                   ...
                                   pi->min_size = pi->min_size / 2; pi->size = pi->size / 2;    // only support 2 zones now
                                   }
                                   }

                                   if (auto m =
                                       pg_pool_t::
                                       get_pg_autoscale_mode_by_name(g_conf().
                                                                     get_val <
                                                                     string >
                                                                     ("osd_pool_default_pg_autoscale_mode"));
                                       m !=
                                       pg_pool_t::pg_autoscale_mode_t::
                                       UNKNOWN) {
                                   pi->pg_autoscale_mode = m;}
                                   else {
                                   pi->pg_autoscale_mode =
                                   pg_pool_t::pg_autoscale_mode_t::OFF;}
                                   auto max =
                                   g_conf().get_val < int64_t >
                                   ("mon_osd_max_initial_pgs");
                                   pi->set_pg_num(max > 0 ? std::min <
                                                  uint64_t > (pg_num,
                                                              std::max <
                                                              int64_t > (1,
                                                                         max))
                                   ...
                                   if (osdmap.require_osd_release >=
                                       ceph_release_t::nautilus && pg_num_min) {
                                   pi->opts.set(pool_opts_t::PG_NUM_MIN,
                                                static_cast < int64_t >
                                                (pg_num_min));}
                                   if (osdmap.require_osd_release >=
                                       ceph_release_t::quincy && pg_num_max) {
                                   pi->opts.set(pool_opts_t::PG_NUM_MAX,
                                                static_cast < int64_t >
                                                (pg_num_max));}
                                   if (auto m =
                                       pg_pool_t::
                                       get_pg_autoscale_mode_by_name
                                       (pg_autoscale_mode);
                                       m !=
                                       pg_pool_t::pg_autoscale_mode_t::
                                       UNKNOWN) {
                                   ...
                                   pi->opts.set(pool_opts_t::TARGET_SIZE_RATIO,
                                                target_size_ratio);}
                                   ...
                                   * 1000000;
                                   ...
                                   1000000;
                                   pi->cache_min_flush_age =
                                   g_conf()->
                                   osd_pool_default_cache_min_flush_age;
                                   pi->cache_min_evict_age =
                                   g_conf()->
                                   ...
                                   bool OSDMonitor::
                                   prepare_set_flag(MonOpRequestRef op,
                                                    int flag) {
                                   op->mark_osdmon_event(__func__);
                                   ostringstream ss;
                                   ...
                                   bool OSDMonitor::
                                   prepare_unset_flag(MonOpRequestRef op,
                                                      int flag) {
                                   op->mark_osdmon_event(__func__);
                                   ostringstream ss;
                                   if (pending_inc.new_flags < 0)
                                   ...
                                   int OSDMonitor::
                                   prepare_command_pool_set(const cmdmap_t &
                                                            cmdmap,
                                                            stringstream & ss) {
                                   string poolstr;
                                   cmd_getval(cmdmap, "pool", poolstr);
                                   ...
                                   ss << "unrecognized pool '" << poolstr <<
                                   "'"; return -ENOENT;}
                                   string var; cmd_getval(cmdmap, "var", var);
                                   ...
                                   string val; string interr, floaterr; int64_t n = 0; double f = 0; int64_t uf = 0;    // micro-f
                                   cmd_getval(cmdmap, "val", val);
                                   auto si_options = {
                                   "target_max_objects"}; auto iec_options = {
                                   "target_max_bytes",
                                   "target_size_bytes",
                                   "compression_max_blob_size",
                                   "compression_min_blob_size",
                                   ...
                                   "csum_min_block",};
                                   if (count
                                       (begin(si_options), end(si_options),
                                        var)) {
                                   n =
                                   strict_si_cast < int64_t > (val, &interr);}
                                   else
                                   if (count
                                       (begin(iec_options), end(iec_options),
                                        var)) {
                                   n =
                                   strict_iec_cast < int64_t > (val, &interr);}
                                   else {
                                   ...
                                   if (!p.is_tier() &&
                                       (var == "hit_set_type"
                                        ...
                                        "cache_target_dirty_high_ratio"
                                        || var == "use_gmt_hitset"
                                        || var == "cache_min_flush_age"
                                        || var == "cache_min_evict_age"
                                        || var == "hit_set_grade_decay_rate"
                                        || var == "hit_set_search_last_n"
                                   ...
                                   return -ENOTSUP;}
                                   if (interr.length()) {
                                   ss << "error parsing integer value '" << val
                                   << "': " << interr; return -EINVAL;}
                                   if (n <= 0 || n > 10) {
                                   ...
                                   return -EPERM;}
                                   bool sure = false;
                                   cmd_getval(cmdmap, "yes_i_really_mean_it",
                                              sure); if (!sure) {
                                   ss <<
                                   ...
                                   if (osdmap.crush->
                                       get_rule_type(p.get_crush_rule()) !=
                                       (int)p.type) {
                                   ss << "crush rule " << p.
                                   get_crush_rule() <<
                                   " type does not match pool"; return -EINVAL;}
                                   ...
                                   int r =
                                   check_pg_num(pool, p.get_pg_num(), n,
                                                p.get_crush_rule(), &ss);
                                   if (r < 0) {
                                   return r;}
                                   }
                                   ...
                                   if (n < 1 || n > p.size) {
                                   ss <<
                                   "pool min_size must be between 1 and size, which is set to "
                                   << (int)p.size; return -EINVAL;}
                                   }
                                   else {
                                   ErasureCodeInterfaceRef erasure_code;
                                   int k;
                                   stringstream tmp;
                                   int err =
                                   get_erasure_code(p.erasure_code_profile,
                                                    &erasure_code, &tmp);
                                   if (err == 0) {
                                   k = erasure_code->get_data_chunk_count();}
                                   else {
                                   ss << __func__ <<
                                   " get_erasure_code failed: " << tmp.str();
                                   return err;}
                                   ...
                                   // force pre-nautilus clients to resend their ops, since they
                                   // don't understand pg_num_pending changes form a new interval
                                   p.last_force_op_resend_prenautilus =
                                   pending_inc.epoch;}
                                   p.set_pg_num(n);}
                                   else {
                                   if (osdmap.require_osd_release <
    ...
    // don't understand that split PGs now form a new interval.
                                   p.last_force_op_resend_preluminous =
                                   pending_inc.epoch;}
                                   else
                                   if (var == "pg_num") {
                                   if (p.has_flag(pg_pool_t::FLAG_NOPGCHANGE)) {
                                   ...
                                   if (n > (int)p.get_pg_num_target()) {
                                   int r =
                                   check_pg_num(pool, n, p.get_size(),
                                                p.get_crush_rule(), &ss);
                                   if (r) {
                                   return r;}
                                   bool force = false;
      ...
      // pre-nautilus osdmap format; increase pg_num directly
                                   ...
                                   // don't understand that split PGs now form a new interval.
                                   p.last_force_op_resend_preluminous =
                                   pending_inc.epoch; p.set_pg_num(n);}
                                   else {
      ...
      // manually.
                                   ...
                                   if (n <= 0) {
                                   ss <<
                                   "specified pgp_num must > 0, but you set to "
                                   << n; return -EINVAL;}
                                   if (n > (int)p.get_pg_num()) {
                                   ...
                                   ss << "specified pgp_num " << n
                                   << " > pg_num_pending " << p.
                                   get_pg_num_pending(); return -EINVAL;}
                                   p.set_pgp_num(n);}
                                   else
                                   if (var == "pgp_num") {
                                   if (p.has_flag(pg_pool_t::FLAG_NOPGCHANGE)) {
      ...
      // pre-nautilus osdmap format; increase pgp_num directly
                                   ...
                                   auto m =
                                   pg_pool_t::
                                   get_pg_autoscale_mode_by_name(val);
                                   if (m ==
                                       pg_pool_t::pg_autoscale_mode_t::
                                       UNKNOWN) {
                                   ...
                                   if (id == -ENOENT) {
                                   ss << "crush rule " << val <<
                                   " does not exist"; return -ENOENT;}
                                   if (id < 0) {
                                   ss << cpp_strerror(id); return -ENOENT;}
                                   if (osdmap.crush->get_rule_type(id) !=
                                       (int)p.get_type()) {
                                   ss << "crush rule " << id <<
                                   " type does not match pool"; return -EINVAL;}
                                   p.crush_rule = id;}
                                   else
                                   if (var == "nodelete" || var == "nopgchange"
                                       || var == "nosizechange"
                                       || var == "write_fadvise_dontneed"
                                       || var == "noscrub"
                                       || var == "nodeep-scrub"
                                       || var == "bulk") {
                                   uint64_t flag =
                                   ...
                                   if (val == "false"
                                       || (interr.empty() && n == 0)) {
                                   if (flag == pg_pool_t::FLAG_NOPGCHANGE
                                       && p.is_crimson()) {
                                   ss <<
    ...
    // make sure we only compare against 'n' if we didn't receive a string
                                   ...
                                   (CEPH_FEATURE_OSD_CACHEPOOL, ss); if (err)
                                   return err; if (val == "bloom") {
                                   BloomHitSet::Params * bsp =
                                   new BloomHitSet::Params;
                                   bsp->set_fpp(g_conf().get_val <
                                                double
                                                >
                                                ("osd_pool_default_hit_set_bloom_fpp"));
                                   p.hit_set_params = HitSet::Params(bsp);}
                                   else
                                   if (val == "explicit_hash")
                                   p.hit_set_params =
                                   HitSet::Params(new ExplicitHashHitSet::
                                                  Params);
                                   else
                                   if (val == "explicit_object")
                                   p.hit_set_params =
                                   HitSet::Params(new ExplicitObjectHitSet::
                                                  Params);
                                   else {
                                   ...
                                   if (var == "hit_set_fpp") {
                                   if (floaterr.length()) {
                                   ss << "error parsing floating point value '"
                                   << val << "': " << floaterr; return -EINVAL;}
                                   else
                                   if (f < 0 || f > 1.0) {
                                   ss <<
                                   ...
                                   if (interr.length()) {
                                   ss << "error parsing int '" << val << "': "
                                   << interr; return -EINVAL;}
                                   p.target_max_objects = n;}
                                   else
                                   if (var == "target_max_bytes") {
                                   if (interr.length()) {
                                   ss << "error parsing int '" << val << "': "
                                   << interr; return -EINVAL;}
                                   ...
                                   if (floaterr.length()) {
                                   ss << "error parsing float '" << val << "': "
                                   << floaterr; return -EINVAL;}
                                   if (f < 0 || f > 1.0) {
                                   ...
                                   if (var == "cache_target_full_ratio") {
                                   if (floaterr.length()) {
                                   ss << "error parsing float '" << val << "': "
                                   << floaterr; return -EINVAL;}
                                   if (f < 0 || f > 1.0) {
                                   ...
                                   if (var == "cache_min_flush_age") {
                                   if (interr.length()) {
                                   ss << "error parsing int '" << val << "': "
                                   << interr; return -EINVAL;}
                                   p.cache_min_flush_age = n;}
                                   else
                                   if (var == "cache_min_evict_age") {
                                   if (interr.length()) {
                                   ss << "error parsing int '" << val << "': "
                                   << interr; return -EINVAL;}
                                   p.cache_min_evict_age = n;}
                                   else
                                   if (var == "min_read_recency_for_promote") {
                                   if (interr.length()) {
                                   ss << "error parsing integer value '" << val
                                   << "': " << interr; return -EINVAL;}
                                   p.min_read_recency_for_promote = n;}
                                   else
                                   if (var == "hit_set_grade_decay_rate") {
                                   if (interr.length()) {
                                   ss << "error parsing integer value '" << val
                                   << "': " << interr; return -EINVAL;}
                                   if (n > 100 || n < 0) {
                                   ...
                                   if (var == "min_write_recency_for_promote") {
                                   if (interr.length()) {
                                   ss << "error parsing integer value '" << val
                                   << "': " << interr; return -EINVAL;}
                                   ...
                                   if (!cmode) {
                                   ss << "unrecognized compression mode '" <<
                                   val << "'"; return -EINVAL;}
                                   }
                                   }
                                   else
                                   ...
                                   if (!alg) {
                                   ss << "unrecognized compression_algorithm '"
                                   << val << "'"; return -EINVAL;}
                                   }
                                   }
                                   else
                                   if (var == "compression_required_ratio") {
                                   if (floaterr.length()) {
                                   ss << "error parsing float value '" << val <<
                                   "': " << floaterr; return -EINVAL;}
                                   if (f < 0 || f > 1) {
                                   ss <<
                                   "compression_required_ratio is out of range (0-1): '"
                                   << val << "'"; return -EINVAL;}
                                   }
                                   else
                                   if (var == "csum_type") {
                                   auto t =
                                   unset ? 0 : Checksummer::
                                   get_csum_string_type(val); if (t < 0) {
                                   ss << "unrecognized csum_type '" << val <<
                                   "'"; return -EINVAL;}
      //preserve csum_type numeric value
                                   n = t; interr.clear();}
                                   else
                                   if (var == "compression_max_blob_size" ||
                                       var == "compression_min_blob_size" ||
                                       var == "csum_max_block" ||
                                       var == "csum_min_block") {
                                   if (interr.length()) {
                                   ss << "error parsing int value '" << val <<
                                   "': " << interr; return -EINVAL;}
                                   }
                                   else
                                   if (var == "fingerprint_algorithm") {
                                   if (!unset) {
                                   auto alg =
                                   pg_pool_t::get_fingerprint_from_str(val);
                                   if (!alg) {
                                   ss << "unrecognized fingerprint_algorithm '"
                                   << val << "'"; return -EINVAL;}
                                   }
                                   }
                                   else
                                   if (var == "target_size_bytes") {
                                   if (interr.length()) {
                                   ss << "error parsing unit value '" << val <<
                                   "': " << interr; return -EINVAL;}
                                   if (osdmap.require_osd_release <
                                       ceph_release_t::nautilus) {
                                   ss <<
                                   ...
                                   if (var == "recovery_priority") {
                                   if (interr.length()) {
                                   ss << "error parsing int value '" << val <<
                                   "': " << interr; return -EINVAL;}
                                   if (!g_conf()->debug_allow_any_pool_priority) {
                                   if (n > OSD_POOL_PRIORITY_MAX
                                       || n < OSD_POOL_PRIORITY_MIN) {
                                   ss <<
                                   "pool recovery_priority must be between " <<
                                   OSD_POOL_PRIORITY_MIN << " and " <<
                                   OSD_POOL_PRIORITY_MAX; return -EINVAL;}
                                   }
                                   }
                                   else
                                   if (var == "pg_autoscale_bias") {
                                   if (f < 0.0 || f > 1000.0) {
                                   ss <<
      ...
      // Current base tier in dedup does not support ec pool 
                                   ...
                                   return -ENOTSUP;}
                                   int64_t lowtierpool_id =
                                   osdmap.lookup_pg_pool_name(val);
                                   if (lowtierpool_id < 0) {
                                   ss << "unrecognized pool '" << val << "'";
                                   return -ENOENT;}
                                   const pg_pool_t * tp =
                                   osdmap.get_pg_pool(lowtierpool_id);
                                   ceph_assert(tp); n = lowtierpool_id;
                                   // The original input is string (pool name), but we convert it to int64_t.
                                   // So, clear interr
                                   interr.clear();}
                                   else
                                   if (var == "dedup_chunk_algorithm") {
                                   if (!unset) {
                                   auto alg =
                                   pg_pool_t::
                                   get_dedup_chunk_algorithm_from_str(val);
                                   if (!alg) {
                                   ss << "unrecognized fingerprint_algorithm '"
                                   << val << "'"; return -EINVAL;}
                                   }
                                   ...
                                   if (var == "dedup_cdc_chunk_size") {
                                   if (interr.length()) {
                                   ss << "error parsing int value '" << val <<
                                   "': " << interr; return -EINVAL;}
                                   }

                                   pool_opts_t::opt_desc_t desc =
...
break; default:
                                   ...
                                   "'"; return -EINVAL;}
                                   if (val != "unset") {
                                   ss << "set pool " << pool << " " << var <<
                                   " to " << val;}
                                   else {
                                   ss << "unset pool " << pool << " " << var;}
                                   p.last_change = pending_inc.epoch;
                                   pending_inc.new_pools[pool] = p; return 0;}

                                   int OSDMonitor::
                                   prepare_command_pool_application(const string
                                                                    & prefix,
                                                                    const
                                                                    cmdmap_t &
                                                                    cmdmap,
                                                                    stringstream
                                                                    & ss) {
                                   return _command_pool_application(prefix,
                                                                    cmdmap, ss,
                                                                    nullptr,
                                                                    true);}

                                   int OSDMonitor::
                                   preprocess_command_pool_application(const
                                                                       string &
                                                                       prefix,
                                                                       const
                                                                       cmdmap_t
 ...
 */
                                   int OSDMonitor::
                                   _command_pool_application(const string &
                                                             prefix,
                                                             const cmdmap_t &
                                                             cmdmap,
                                                             stringstream & ss,
                                   ...
                                   bool app_exists =
                                   (p.application_metadata.count(app) > 0);
                                   string key; cmd_getval(cmdmap, "key", key);
                                   if (key == "all") {
                                   ss << "key cannot be 'all'"; return -EINVAL;}
                                   ...
                                   cmd_getval(cmdmap, "yes_i_really_mean_it",
                                              force);
                                   if (!app_exists
                                       && !p.application_metadata.empty()
                                       && !force) {
                                   ss << "Are you SURE? Pool '" << pool_name <<
                                   "' already has an enabled " <<
                                   "application; pass --yes-i-really-mean-it to proceed anyway";
                                   return -EPERM;}

                                   if (!app_exists
                                       && p.application_metadata.size() >=
                                       MAX_POOL_APPLICATIONS) {
                                   ss <<
                                   "too many enabled applications on pool '" <<
                                   pool_name << "'; " << "max " <<
                                   MAX_POOL_APPLICATIONS; return -EINVAL;}

                                   if (app.length() >
                                       MAX_POOL_APPLICATION_LENGTH) {
                                   ss << "application name '" << app <<
                                   "' too long; max length " <<
                                   MAX_POOL_APPLICATION_LENGTH; return -EINVAL;}

                                   if (!app_exists) {
                                   p.application_metadata[app] = {
                                   };}
                                   ss << "enabled application '" << app <<
                                   "' on pool '" << pool_name << "'";}
                                   else
                                   if (boost::algorithm::
                                       ends_with(prefix, "disable")) {
                                   bool force = false;
                                   cmd_getval(cmdmap, "yes_i_really_mean_it",
                                              force); if (!force) {
                                   ss <<
                                   ...
                                   if (!app_exists) {
                                   ss << "application '" << app << "' is not enabled on pool '" << pool_name << "'"; return 0;  // idempotent
                                   }

                                   p.application_metadata.erase(app);
                                   ss << "disable application '" << app <<
                                   "' on pool '" << pool_name << "'";}
                                   else
                                   if (boost::algorithm::
                                       ends_with(prefix, "set")) {
                                   if (p.is_tier()) {
                                   ss <<
                                   ...
                                   if (key.empty()) {
                                   ss << "key must be provided"; return -EINVAL;}
                                   ...
                                   ss << "too many keys set for application '"
                                   << app << "' on pool '" << pool_name <<
                                   ...
                                   ss << "set application '" << app << "' key '"
                                   << key << "' to '" << value << "' on pool '"
                                   << pool_name << "'";}
                                   else
                                   if (boost::algorithm::
                                       ends_with(prefix, "rm")) {
                                   ...
                                   cmd_getval(cmdmap, "key", key);
                                   auto it =
                                   p.application_metadata[app].find(key);
                                   if (it ==
                                       p.application_metadata[app].end()) {
                                   ss << "application '" << app << "' on pool '" << pool_name << "' does not have key '" << key << "'"; return 0;   // idempotent
                                   }

                                   p.application_metadata[app].erase(it);
                                   ss << "removed application '" << app <<
                                   "' key '" << key << "' on pool '" <<
                                   pool_name << "'";}
                                   else {
                                   ceph_abort();}
  ...
  // so pool was definitely modified
                                   ...
                                   *modified = true;}
                                   ...
                                   (CrushWrapper & newcrush, int32_t id,
                                    int32_t ancestor, bool has_ancestor,
                                    bool unlink_only) {
                                   int err = 0; if (has_ancestor) {
                                   err =
                                   newcrush.remove_item_under(cct, id, ancestor,
                                                              unlink_only);}
                                   else {
                                   err =
                                   newcrush.remove_item(cct, id, unlink_only);}
                                   return err;}

                                   void OSDMonitor::
                                   do_osd_crush_remove(CrushWrapper & newcrush)
                                   {
                                   pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                   ...
                                   int OSDMonitor::
                                   prepare_command_osd_crush_remove(CrushWrapper
                                                                    & newcrush,
                                                                    int32_t id,
                                                                    int32_t
                                                                    ancestor,
                                   ...
                                   _prepare_command_osd_crush_remove(newcrush,
                                                                     ...
                                                                     unlink_only);
                                   if (err < 0)
                                   return err; ceph_assert(err == 0);
                                   do_osd_crush_remove(newcrush); return 0;}

                                   int OSDMonitor::
                                   prepare_command_osd_remove(int32_t id) {
                                   if (osdmap.is_up(id)) {
                                   return -EBUSY;}

                                   pending_inc.new_state[id] =
                                   osdmap.get_state(id);
                                   pending_inc.new_uuid[id] = uuid_d();
                                   pending_metadata_rm.insert(id);
                                   pending_metadata.erase(id); return 0;}

                                   int32_t OSDMonitor::
                                   _allocate_osd_id(int32_t * existing_id) {
                                   ceph_assert(existing_id); *existing_id = -1;
                                   for (int32_t i = 0; i < osdmap.get_max_osd();
                                        ++i) {
                                   if (!osdmap.exists(i)
                                       && pending_inc.new_up_client.count(i) ==
                                       0 && (pending_inc.new_state.count(i) == 0
                                             || (pending_inc.
                                                 new_state[i] & CEPH_OSD_EXISTS)
                                             == 0)) {
                                   *existing_id = i; return -1;}
                                   }
                                   ...
                                   void OSDMonitor::
                                   do_osd_create(const int32_t id,
                                                 const uuid_d & uuid,
                                                 const string & device_class,
                                                 int32_t * new_id) {
                                   dout(10) << __func__ << " uuid " << uuid <<
                                   dendl; ceph_assert(new_id);
                                   ...
                                   *new_id = id; goto out;}
                                   ...
                                   dout(10) << __func__ << " allocated id " <<
                                   allocated_id << " existing id " <<
                                   existing_id << dendl; if (existing_id >= 0) {
...
out:
                                   ...
                                   ostringstream ss;
                                   int r =
                                   newcrush.update_device_class(*new_id,
                                                                device_class,
                                                                name, &ss);
                                   if (r < 0) {
                                   derr << __func__ << " failed to set " << name
                                   << " device_class " << device_class << ": "
                                   << cpp_strerror(r) << " - " << ss.str()
                                   << dendl;
                                   // non-fatal... this might be a replay and we want to be idempotent.
                                   }
                                   else {
                                   dout(20) << __func__ << " set " << name <<
                                   ...
                                   dout(20) << __func__ << " no device_class" <<
                                   dendl;}

                                   dout(10) << __func__ << " using id " <<
                                   *new_id << dendl;
                                   if (osdmap.get_max_osd() <= *new_id
                                       && pending_inc.new_max_osd <= *new_id) {
                                   pending_inc.new_max_osd = *new_id + 1;}

                                   pending_inc.new_weight[*new_id] =
                                   CEPH_OSD_IN;
                                   // do not set EXISTS; OSDMap::set_weight, called by apply_incremental, will
                                   // set it for us.  (ugh.)
                                   pending_inc.new_state[*new_id] |=
                                   CEPH_OSD_NEW; if (!uuid.is_zero())
                                   pending_inc.new_uuid[*new_id] = uuid;}

                                   int OSDMonitor::
                                   validate_osd_create(const int32_t id,
                                                       const uuid_d & uuid,
                                                       const bool
                                                       check_osd_exists,
                                                       int32_t * existing_id,
                                   ...
                                   dout(10) << __func__ << " id " << id <<
                                   " uuid " << uuid << " check_osd_exists " <<
                                   check_osd_exists << dendl;
                                   ceph_assert(existing_id);
  ...
  // i < 0
                                   ...
                                   int OSDMonitor::
                                   prepare_command_osd_create(const int32_t id,
                                                              const uuid_d &
                                                              uuid,
                                                              int32_t *
                                                              existing_id,
                                   ...
                                   "instead."; return -EINVAL;}

                                   if (uuid.is_zero()) {
                                   dout(10) << __func__ <<
                                   " no uuid; assuming legacy `osd create`" <<
                                   dendl;}

                                   return validate_osd_create(id, uuid, true,
                                                              existing_id, ss);}

                                   int OSDMonitor::
                                   prepare_command_osd_new(MonOpRequestRef op,
                                                           const cmdmap_t &
                                                           cmdmap,
                                                           const map < string,
                                                           string > &params,
                                                           stringstream & ss,
                                                           Formatter * f) {
                                   uuid_d uuid; string uuidstr; int64_t id = -1;
  ...
  // a previously destroyed osd?
                                   ...
                                   bool is_recreate_destroyed = (id >= 0
                                                                 && osdmap.
                                                                 is_destroyed
                                                                 (id));
    ...
    // creation, so that we are compatible with `create`.
                                   if (id >= 0 && osdmap.exists(id)
                                       && !osdmap.is_destroyed(id)) {
                                   dout(10) << __func__ << " osd." << id <<
      ...
      // find an `id`
                                   ...
                                   ceph_assert(existing_id >= 0);
                                   id = existing_id;}
                                   dout(10) << __func__ << " found id " << id <<
                                   " to use" << dendl;}
                                   else
                                   if (id >= 0 && osdmap.is_destroyed(id)) {
                                   dout(10) << __func__ << " recreating osd." <<
                                   id << dendl;}
                                   else {
                                   dout(10) << __func__ << " creating new osd."
                                   << id << dendl;}
                                   }
                                   else {
                                   ceph_assert(id >= 0);
  ...
  // osd that has been previously destroyed.
    ...
    // nothing to do, really.
                                   dout(10) << __func__ <<
                                   " idempotent and no params -- no op." <<
                                   dendl; ceph_assert(id >= 0); if (f) {
                                   ...
                                   dendl;}
                                   string cephx_secret, lockbox_secret,
                                   dmcrypt_key; bool has_lockbox = false;
                                   bool has_secrets =
                                   params.count("cephx_secret")
                                   || params.count("cephx_lockbox_secret")
                                   || params.count("dmcrypt_key");
                                   KVMonitor * svc = nullptr;
                                   AuthMonitor::auth_entity_t cephx_entity,
                                   lockbox_entity; if (has_secrets) {
                                   if (params.count("cephx_secret") == 0) {
                                   ss << "requires a cephx secret.";
                                   return -EINVAL;}
                                   cephx_secret = params.at("cephx_secret");
                                   bool has_lockbox_secret =
                                   (params.count("cephx_lockbox_secret") > 0);
                                   bool has_dmcrypt_key =
                                   (params.count("dmcrypt_key") > 0);
                                   dout(10) << __func__ << " has lockbox " <<
                                   has_lockbox_secret << " dmcrypt " <<
                                   has_dmcrypt_key << dendl;
                                   if (has_lockbox_secret && has_dmcrypt_key) {
                                   has_lockbox = true;
                                   lockbox_secret =
                                   params.at("cephx_lockbox_secret");
                                   ...
                                   dendl;
                                   err =
                                   mon.authmon()->validate_osd_new(id, uuid,
                                                                   cephx_secret,
                                                                   lockbox_secret,
                                                                   cephx_entity,
                                                                   lockbox_entity,
                                   ...
                                   if (may_be_idempotent && err != EEXIST) {
      ...
      // to use validate_id.
                                   ...
                                   svc = mon.kvmon();
                                   err =
                                   svc->validate_osd_new(uuid, dmcrypt_key, ss);
                                   if (err < 0) {
                                   return err;}
                                   else
                                   if (may_be_idempotent && err != EEXIST) {
                                   ...
                                   ceph_assert(!has_secrets
                                               || !cephx_secret.empty());
                                   ceph_assert(!has_lockbox
                                               || !lockbox_secret.empty());
                                   if (may_be_idempotent) {
    ...
    // just return right away.
                                   ...
                                   ceph_assert((lockbox_secret.empty()
                                                && dmcrypt_key.empty())
                                               || (!lockbox_secret.empty()
                                                   && !dmcrypt_key.empty()));
                                   err =
                                   mon.authmon()->do_osd_new(cephx_entity,
                                                             lockbox_entity,
                                                             has_lockbox);
                                   ceph_assert(0 == err); if (has_lockbox) {
                                   ...
                                   pending_inc.new_state[id] |=
                                   CEPH_OSD_DESTROYED;
                                   if ((osdmap.get_state(id) & CEPH_OSD_NEW) ==
                                       0) {
                                   ...
                                   ceph_assert(id >= 0);
                                   int32_t new_id = -1;
                                   do_osd_create(id, uuid, device_class,
                                                 &new_id);
                                   ...
                                   bool OSDMonitor::
                                   prepare_command(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MMonCommand > ();
                                   stringstream ss; cmdmap_t cmdmap;
                                   if (!cmdmap_from_json(m->cmd, &cmdmap, ss)) {
                                   string rs = ss.str();
                                   mon.reply_command(op, -EINVAL, rs,
                                                     get_last_committed());
                                   return true;}
                                   ...
                                   string weights_str;
                                   if (!cmd_getval
                                       (cmdmap, "weights", weights_str)) {
                                   return -EINVAL;}
                                   std::replace(begin(weights_str),
                                                end(weights_str), '\'', '"');
                                   json_spirit::mValue json_value;
                                   if (!json_spirit::
                                       read(weights_str, json_value)) {
                                   return -EINVAL;}
                                   if (json_value.type() !=
                                       json_spirit::obj_type) {
                                   return -EINVAL;}
                                   const auto obj = json_value.get_obj(); try {
      for (auto & osd_weight:     obj) {
                                   auto osd_id = std::stoi(osd_weight.first);
                                   if (!osdmap.exists(osd_id)) {
                                   return -ENOENT;}
                                   if (osd_weight.second.type() !=
                                       json_spirit::str_type) {
                                   return -EINVAL;}
                                   auto weight =
                                   std::stoul(osd_weight.second.get_str());
                                   weights->insert( {
                                                   osd_id, weight}
                                   );}
                                   }
                                   ...
                                   int OSDMonitor::
                                   prepare_command_osd_destroy(int32_t id,
                                                               stringstream &
                                                               ss) {
                                   ceph_assert(paxos.is_plugged());
                                   ...
                                   if (!osdmap.exists(id)) {
                                   dout(10) << __func__ << " osd." << id <<
                                   " does not exist." << dendl; return -ENOENT;}

                                   uuid_d uuid = osdmap.get_uuid(id);
                                   dout(10) << __func__ << " destroying osd." <<
                                   id << " uuid " << uuid << dendl;
                                   // if it has been destroyed, we assume our work here is done.
                                   if (osdmap.is_destroyed(id)) {
                                   ss << "destroyed osd." << id; return 0;}

                                   EntityName cephx_entity, lockbox_entity;
                                   bool idempotent_auth =
                                   false, idempotent_cks = false;
                                   int err =
                                   mon.authmon()->validate_osd_destroy(id, uuid,
                                                                       cephx_entity,
                                                                       lockbox_entity,
                                                                       ss);
                                   if (err < 0) {
                                   ...
                                   if (err < 0) {
                                   ceph_assert(err == -ENOENT);
                                   err = 0; idempotent_cks = true;}

                                   if (!idempotent_auth) {
                                   err =
                                   mon.authmon()->do_osd_destroy(cephx_entity,
                                                                 lockbox_entity);
                                   ceph_assert(0 == err);}
                                   ...
                                   // proposing.
                                   ceph_assert(err == 0); return 0;}

                                   int OSDMonitor::
                                   prepare_command_osd_purge(int32_t id,
                                                             stringstream & ss)
                                   {
                                   ceph_assert(paxos.is_plugged());
                                   dout(10) << __func__ << " purging osd." << id
                                   << dendl; ceph_assert(!osdmap.is_up(id));
                                   /*
                                   ...
                                   _prepare_command_osd_crush_remove(newcrush,
                                                                     ...
                                                                     false);
                                   if (err == -ENOENT) {
                                   err = 0; may_be_idempotent = true;}
                                   else
                                   if (err < 0) {
                                   ss << "error removing osd." << id <<
                                   " from crush"; return err;}
  ...
  // no point destroying the osd again if it has already been marked destroyed
                                   ...
                                   int OSDMonitor::
                                   parse_pgid(const cmdmap_t & cmdmap,
                                              stringstream & ss,
                                              /* out */ pg_t & pgid,
                                   ...
                                   if (pending_inc.crush.length()) {
                                   dout(10) << __func__ <<
                                   " waiting for pending crush update " <<
                                   dendl;
                                   wait_for_finished_proposal(op,
                                                              new
                                                              C_RetryMessage
                                   ...
                                   return true;}
                                   dout(10) <<
                                   "prepare_command setting new crush map" <<
                                   dendl; bufferlist data(m->get_data());
                                   CrushWrapper crush; try {
                                   auto bl = data.cbegin(); crush.decode(bl);}
                                   catch(const std::exception & e) {
                                   err = -EINVAL;
                                   ss << "Failed to parse crushmap: " << e.
                                   what(); goto reply;}

                                   int64_t prior_version = 0;
                                   if (cmd_getval
                                       (cmdmap, "prior_version",
                                        prior_version)) {
                                   if (prior_version ==
                                       osdmap.get_crush_version() - 1) {
                                   ...
                                   osdmap.crush->encode(current,
                                                        mon.
                                                        get_quorum_con_features
                                                        ());
                                   crush.encode(proposed,
                                                mon.get_quorum_con_features());
                                   ...
                                   err = -EPERM;
                                   ss << "prior_version " << prior_version <<
                                   " != crush version " << osdmap.
                                   get_crush_version(); goto reply;}
                                   }

                                   if (!validate_crush_against_features
                                       (&crush, ss)) {
                                   err = -EINVAL; goto reply;}

                                   err =
                                   osdmap.validate_crush_rules(&crush, &ss);
                                   if (err < 0) {
                                   goto reply;}
                                   ...
                                   // totally broken
                                   dout(10) << " testing map" << dendl; stringstream ess; CrushTester tester(crush, ess); tester.set_min_x(0); tester.set_max_x(50); tester.set_num_rep(3); // arbitrary
                                   auto start = ceph::coarse_mono_clock::now();
                                   int r =
                                   tester.test_with_fork(cct,
                                                         g_conf()->mon_lease);
                                   ...
                                   dout(10) << " tester.test_with_fork returns "
                                   << r << ": " << ess.str() << dendl;
                                   ss << "crush smoke test failed with " << r <<
                                   ": " << ess.str(); err = r; goto reply;}
                                   dout(10) << __func__ <<
                                   " crush somke test duration: " << duration <<
                                   ...
                                   if (prefix == "osd crush set-device-class") {
                                   string device_class;
                                   if (!cmd_getval
                                       (cmdmap, "class", device_class)) {
                                   err = -EINVAL;   // no value!
                                   goto reply;}
                                   ...
                                   // wildcard?
                                   if (j == 0 &&
                                       (idvec[0] == "any" || idvec[0] == "all"
                                        || idvec[0] == "*")) {
                                   osdmap.get_all_osds(osds); stop = true;}
                                   else {
                                   // try traditional single osd way
                                   long osd =
                                   parse_osd_id(idvec[j].c_str(), &ss);
                                   if (osd < 0) {
                                   // ss has reason for failure
                                   ss << ", unable to parse osd id:\"" <<
                                   idvec[j] << "\". "; err = -EINVAL; continue;}
                                   osds.insert(osd);}
  ...
  for (auto & osd:                osds) {
                                   ...
                                   string action; if (newcrush.item_exists(osd)) {
                                   action = "updating";}
                                   else {
                                   action = "creating";
                                   newcrush.set_item_name(osd, name);}

                                   dout(5) << action << " crush item id " << osd
                                   << " name '" << name << "' device_class '" <<
                                   device_class << "'" << dendl;
                                   err =
                                   newcrush.update_device_class(osd,
                                                                device_class,
                                                                name, &ss);
                                   if (err < 0) {
                                   goto reply;}
                                   ...
                                   ss << "set-device-class item id " << osd <<
                                   " name '" << name << "' device_class '" <<
                                   ...
                                   if (prefix == "osd crush rm-device-class") {
                                   bool stop = false;
                                   vector < string > idvec;
                                   cmd_getval(cmdmap, "ids", idvec);
                                   CrushWrapper newcrush = _get_pending_crush();
...
for (auto & osd:                  osds) {
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "done removing class of osd(s): " <<
                                   updated; getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                                                        0, rs,
                                   ...
                                   if (prefix == "osd crush class create") {
                                   string device_class;
                                   if (!cmd_getval
                                       (cmdmap, "class", device_class)) {
                                   err = -EINVAL;   // no value!
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "created class " << device_class <<
                                   " with id " << class_id << " to crush map";
                                   goto update;}
                                   else
                                   if (prefix == "osd crush class rm") {
                                   string device_class;
                                   if (!cmd_getval
                                       (cmdmap, "class", device_class)) {
                                   err = -EINVAL;   // no value!
                                   ...
                                   goto wait;}
                                   int class_id =
                                   newcrush.get_class_id(device_class);
                                   stringstream ts;
                                   if (newcrush.
                                       class_is_in_use(class_id, &ts)) {
                                   err = -EBUSY;
                                   ss << "class '" << device_class << "' " <<
                                   ts.str(); goto reply;}

     // check if class is used by any erasure-code-profiles
                                   mempool::osdmap::map < string, map < string,
                                   string >> old_ec_profiles =
                                   osdmap.get_erasure_code_profiles();
                                   auto ec_profiles =
                                   pending_inc.get_erasure_code_profiles();
#ifdef HAVE_STDLIB_MAP_SPLICING
                                   ec_profiles.merge(old_ec_profiles);
#else
                                   ec_profiles.
                                   insert(make_move_iterator
                                          (begin(old_ec_profiles)),
                                          make_move_iterator(end
                                                             (old_ec_profiles)));
#endif
list < string > referenced_by; for (auto & i:ec_profiles) {
for (auto & j:                    i.second) {
                                   if ("crush-device-class" == j.first
                                       && device_class == j.second) {
                                   ...
                                   << referenced_by; goto reply;}

set < int >osds; newcrush.get_devices_by_class(device_class, &osds); for (auto & p:osds)
                                   {
                                   err =
                                   newcrush.remove_device_class(cct, p, &ss);
                                   if (err < 0) {
         // ss has reason for failure
                                   goto reply;}
                                   }
       ...
       // empty class, remove directly
                                   err =
                                   newcrush.remove_class_name(device_class);
                                   if (err < 0) {
                                   ss << "class '" << device_class <<
                                   "' cannot be removed '" << cpp_strerror(err)
                                   << "'"; goto reply;}
                                   }
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "removed class " << device_class <<
                                   " with id " << class_id << " from crush map";
                                   goto update;}
                                   else
                                   if (prefix == "osd crush class rename") {
      ...
      // so command is idempotent
                                   ...
                                   err =
                                   newcrush.rename_class(srcname, dstname);
                                   if (err < 0) {
                                   ss << "fail to rename '" << srcname <<
                                   "' to '" << dstname << "' : " <<
                                   cpp_strerror(err); goto reply;}

                                   pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "rename class '" << srcname << "' to '"
                                   << dstname << "'"; goto update;}
                                   else
    ...
    // os crush add-bucket <name> <type>
                                   ...
                                   dout(0) << "will create and move bucket '" <<
                                   name << "' to location " << loc << dendl;}
                                   ...
                                   int bucketno;
                                   err =
                                   newcrush.add_bucket(0, 0, CRUSH_HASH_DEFAULT,
                                                       type, 0, NULL, NULL,
                                                       &bucketno);
                                   if (err < 0) {
                                   ss << "add_bucket error: '" <<
                                   ...
                                   if (err < 0) {
                                   ss << "error setting bucket name to '" <<
                                   name << "'"; goto reply;}

                                   if (!loc.empty()) {
                                   if (!newcrush.
                                       check_item_loc(cct, bucketno, loc,
                                                      (int *)NULL)) {
                                   err =
                                   newcrush.move_bucket(cct, bucketno, loc);
                                   if (err < 0) {
                                   ss << "error moving bucket '" << name <<
                                   "' to location " << loc; goto reply;}
                                   }
                                   else {
                                   ss << "no need to move item id " << bucketno
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   ...
                                                   get_quorum_con_features());
                                   if (loc.empty()) {
                                   ss << "added bucket " << name << " type " <<
                                   typestr << " to crush map";}
                                   else {
                                   ss << "added bucket " << name << " type " <<
                                   typestr << " to location " << loc;}
                                   goto update;}
                                   else
                                   if (prefix == "osd crush rename-bucket") {
                                   string srcname, dstname; cmd_getval(cmdmap, "srcname", srcname); cmd_getval(cmdmap, "dstname", dstname); err = crush_rename_bucket(srcname, dstname, &ss); if (err == -EALREADY) // equivalent to success for idempotency
                                   err = 0; if (err)
                                   goto reply;
                                   else
                                   goto update;}
                                   ...
                                   if (_have_pending_crush()) {
                                   dout(10) <<
                                   " first waiting for pending crush changes to commit"
                                   << dendl; goto wait;}
                                   CrushWrapper newcrush = _get_pending_crush();
                                   ...
                                   if (osdmap.require_min_compat_client !=
                                       ...
                                       ceph_release_t::luminous) {
                                   ss << "require_min_compat_client " << osdmap.
                                   ...
                                   pool =
                                   osdmap.lookup_pg_pool_name(poolname.c_str());
                                   if (pool < 0) {
                                   ss << "pool '" << poolname << "' not found";
                                   err = -ENOENT; goto reply;}
                                   cmd_getval(cmdmap, "mode", mode);
                                   if (mode != "flat" && mode != "positional") {
                                   ss << "unrecognized weight-set mode '" <<
                                   mode << "'"; err = -EINVAL; goto reply;}
                                   positions =
                                   mode ==
                                   "flat" ? 1 : osdmap.get_pg_pool(pool)->
                                   get_size();}
                                   else {
                                   pool = CrushWrapper::DEFAULT_CHOOSE_ARGS;
                                   positions = 1;}
                                   if (!newcrush.
                                       create_choose_args(pool, positions)) {
                                   if (pool ==
                                       CrushWrapper::DEFAULT_CHOOSE_ARGS) {
                                   ss << "compat weight-set already created";}
                                   else {
                                   ss << "weight-set for pool '" << osdmap.
                                   get_pool_name(pool)
                                   << "' already created";}
                                   goto reply;}
                                   pending_inc.crush.clear();
                                   ...
                                   int positions =
                                   newcrush.get_choose_args_positions(arg_map);
                                   if (weight.size() != (size_t) positions) {
                                   ss << "must specify exact " << positions <<
                                   " weight values"; err = -EINVAL; goto reply;}
                                   }
                                   else {
                                   pool = CrushWrapper::DEFAULT_CHOOSE_ARGS;
                                   if (!newcrush.have_choose_args(pool)) {
                                   ...
                                   err = -ENOENT; goto reply;}
                                   err =
                                   newcrush.choose_args_adjust_item_weightf(cct,
                                                                            newcrush.
                                                                            choose_args_get
                                                                            (pool),
                                                                            newcrush.
    ...
    // osd crush add <OsdName> <weight> <loc1> [<loc2> ...]
                                   ...
                                   if (prefix == "osd crush set"
                                       ...
                                       item_exists(osdid)) {
                                   err = -ENOENT;
                                   ss << "unable to set item id " << osdid <<
                                   " name '" << osd_name << "' weight " <<
                                   weight << " at location " << loc <<
                                   ": does not exist"; goto reply;}

                                   dout(5) << "adding/updating crush item id "
                                   << osdid << " name '" << osd_name <<
                                   "' weight " << weight << " at location " <<
                                   loc << dendl;
                                   CrushWrapper newcrush = _get_pending_crush();
                                   string action;
                                   if (prefix == "osd crush set"
                                       || newcrush.check_item_loc(cct, osdid,
                                                                  loc,
                                                                  (int *)NULL))
                                   {
                                   action = "set";
                                   err =
                                   newcrush.update_item(cct, osdid, weight,
                                                        osd_name, loc);}
                                   else {
                                   action = "add";
                                   err =
                                   newcrush.insert_item(cct, osdid, weight,
                                                        osd_name, loc);
                                   if (err == 0)
                                   err = 1;}
                                   ...
                                   goto reply;
                                   if (err == 0 && !_have_pending_crush()) {
                                   ss << action << " item id " << osdid <<
                                   " name '" << osd_name << "' weight " <<
                                   weight << " at location " << loc <<
                                   ": no change"; goto reply;}
                                   ...
                                   if (prefix == "osd crush create-or-move") {
                                   do {
      // osd crush create-or-move <OsdName> <initial_weight> <loc1> [<loc2> ...]
                                   ...
                                   map < string, string > loc;
                                   CrushWrapper::parse_loc_map(argvec, &loc);
                                   dout(0) << "create-or-move crush item name '"
                                   << osd_name << "' initial_weight " << weight
                                   << " at location " << loc << dendl;
                                   CrushWrapper newcrush = _get_pending_crush();
                                   err =
                                   newcrush.create_or_move_item(cct, osdid,
                                                                weight,
                                                                osd_name, loc,
                                                                g_conf()->
                                                                osd_crush_update_weight_set);
                                   if (err == 0) {
                                   ss << "create-or-move updated item name '" <<
                                   osd_name << "' weight " << weight <<
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "create-or-move updating item name '"
                                   << osd_name << "' weight " << weight <<
                                   " at location " << loc << " to crush map";
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
      ...
      // osd crush move <name> <loc1> [<loc2> ...]
                                   ...
                                   map < string, string > loc;
                                   CrushWrapper::parse_loc_map(argvec, &loc);
                                   dout(0) << "moving crush item name '" << name
                                   ...
                                   if (!newcrush.
                                       check_item_loc(cct, id, loc,
                                                      (int *)NULL)) {
                                   if (id >= 0) {
                                   err =
                                   newcrush.create_or_move_item(cct, id, 0,
                                                                name, loc,
                                                                g_conf()->
                                                                osd_crush_update_weight_set);}
                                   else {
                                   ...
                                   if (err >= 0) {
                                   ss << "moved item id " << id << " name '" <<
                                   name << "' to location " << loc <<
                                   " in crush map"; pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   getline(ss, rs);
                                   ...
                                   ss << "no need to move item id " << id <<
                                   " name '" << name << "' to location " << loc
                                   << " in crush map"; err = 0;}
                                   } while (false);}
                                   else
                                   if (prefix == "osd crush swap-bucket") {
                                   string source, dest;
                                   ...
                                   err = -EPERM; goto reply;}
                                   if (newcrush.get_bucket_alg(sid) !=
                                       newcrush.get_bucket_alg(did) && !force) {
                                   ss << "source bucket alg " <<
                                   crush_alg_name(newcrush.
                                                  get_bucket_alg(sid)) << " != "
                                   << "dest bucket alg " <<
                                   ...
                                   if (r < 0) {
                                   ss << "failed to swap bucket contents: " <<
                                   cpp_strerror(r); err = r; goto reply;}
                                   ss << "swapped bucket of " << source <<
                                   " to " << dest; pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   wait_for_finished_proposal(op,
    ...
    // osd crush link <name> <loc1> [<loc2> ...]
                                   ...
                                   dout(5) << "resolved crush name '" << name <<
                                   "' to id " << id << dendl;}
                                   if (osdmap.crush->
                                       check_item_loc(cct, id, loc,
                                                      (int *)NULL)) {
                                   ss << "no need to move item id " << id <<
                                   " name '" << name << "' to location " << loc
                                   << " in crush map"; err = 0; goto reply;}

                                   dout(5) << "linking crush item name '" <<
                                   name << "' at location " << loc << dendl;
                                   CrushWrapper newcrush = _get_pending_crush();
                                   if (!newcrush.name_exists(name)) {
                                   err = -ENOENT;
                                   ss << "item " << name << " does not exist";
      ...
      // osd crush rm <id> [ancestor]
                                   ...
                                   err = 0;
                                   ss << "device '" << name <<
                                   "' does not appear in the crush map"; break;}
                                   if (!newcrush.name_exists(name)) {
                                   err = 0;
                                   ss << "device '" << name <<
                                   ...
                                   break;}
                                   ancestor =
                                   newcrush.get_item_id(ancestor_str);}

                                   err =
                                   prepare_command_osd_crush_remove(newcrush,
                                                                    id,
                                                                    ancestor,
                                                                    (ancestor <
                                                                     0),
                                   ...
                                   if (err == -ENOENT) {
                                   ss << "item " << id <<
                                   " does not appear in that position"; err = 0;
                                   break;}
                                   if (err == 0) {
                                   if (!unlink_only)
                                   pending_inc.new_crush_node_flags[id] = 0;
                                   ss << "removed item id " << id << " name '"
                                   << name << "' from crush map";
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                   ...
                                   if (prefix == "osd crush reweight-all") {
                                   CrushWrapper newcrush = _get_pending_crush();
    ...
    // osd crush reweight <name> <weight>
                                   ...
                                   err =
                                   newcrush.adjust_item_weightf(cct, id, w,
                                                                g_conf()->
                                                                osd_crush_update_weight_set);
                                   if (err < 0)
                                   goto reply; pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "reweighted item id " << id <<
                                   " name '" << name << "' to " << w <<
                                   " in crush map"; getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                                                        0, rs,
                                   ...
                                   if (prefix == "osd crush reweight-subtree") {
    // osd crush reweight <name> <weight>
                                   CrushWrapper newcrush = _get_pending_crush();
                                   string name;
                                   cmd_getval(cmdmap, "name", name);
                                   ...
                                   err =
                                   newcrush.adjust_subtree_weightf(cct, id, w,
                                                                   g_conf()->
                                                                   osd_crush_update_weight_set);
                                   if (err < 0)
                                   goto reply; pending_inc.crush.clear();
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "adjusted tunables profile to " <<
                                   profile; getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                                                        0, rs,
                                   ...
                                   if (prefix == "osd crush set-tunable") {
                                   CrushWrapper newcrush = _get_pending_crush();
                                   err = 0;
                                   string tunable;
                                   cmd_getval(cmdmap, "tunable", tunable);
                                   ...
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());
                                   ss << "adjusted tunable " << tunable <<
                                   " to " << value; getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                                                        0, rs,
                                   ...
                                   if (prefix == "osd crush rule create-simple") {
      ...
      // From the user point of view, the rule is more meaningfull.
                                   ...
                                   int ruleno =
                                   newcrush.add_simple_rule(name, root, type,
                                                            "", mode,
                                                            pg_pool_t::
                                                            TYPE_REPLICATED,
                                                            &ss);
                                   if (ruleno < 0) {
                                   err = ruleno; goto reply;}
      ...
      // From the user point of view, the rule is more meaningfull.
                                   ...
                                   string name;
                                   cmd_getval(cmdmap, "name", name);
                                   if (erasure_code_profile_in_use
                                       (pending_inc.new_pools, name, &ss))
                                   goto wait;
                                   if (erasure_code_profile_in_use
                                       (osdmap.pools, name, &ss)) {
                                   err = -EBUSY; goto reply;}

                                   if (osdmap.has_erasure_code_profile(name) ||
                                       pending_inc.new_erasure_code_profiles.
                                       count(name)) {
                                   if (osdmap.has_erasure_code_profile(name)) {
                                   pending_inc.old_erasure_code_profiles.
                                   push_back(name);}
                                   else {
                                   dout(20) << "erasure code profile rm " <<
                                   name << ": creation canceled" << dendl;
                                   pending_inc.new_erasure_code_profiles.
                                   erase(name);}
                                   ...
                                   ss << "erasure-code-profile " << name <<
                                   " does not exist"; err = 0; goto reply;}
                                   ...
                                   goto reply;
                                   if (auto found =
                                       profile_map.find("crush-failure-domain");
                                       found != profile_map.end()) {
                                   const auto & failure_domain = found->second;
                                   int failure_domain_type =
                                   osdmap.crush->get_type_id(failure_domain);
                                   if (failure_domain_type < 0) {
                                   ss << "erasure-code-profile " << profile_map
                                   << " contains an invalid failure-domain " <<
                                   std::quoted(failure_domain); err = -EINVAL;
                                   goto reply;}
                                   }

                                   if (profile_map.find("plugin") ==
                                       profile_map.end()) {
                                   ss << "erasure-code-profile " << profile_map
                                   << " must contain a plugin entry" << std::
                                   endl; err = -EINVAL; goto reply;}
                                   string plugin = profile_map["plugin"];
                                   if (pending_inc.
                                       has_erasure_code_profile(name)) {
                                   dout(20) << "erasure code profile " << name
                                   << " try again" << dendl; goto wait;}
                                   else {
                                   err =
                                   normalize_profile(name, profile_map, force,
                                                     &ss); if (err)
                                   goto reply;
                                   if (osdmap.has_erasure_code_profile(name)) {
                                   ...
                                   err =
                                   normalize_profile(name, existing_profile_map,
                                                     force, &ss); if (err)
                                   goto reply;
                                   ...
                                   err = -EPERM;
                                   ss <<
                                   "will not override erasure code profile " <<
                                   name << " because the existing profile " <<
                                   existing_profile_map <<
                                   " is different from the proposed profile " <<
                                   profile_map; goto reply;}
                                   }

                                   dout(20) << "erasure code profile set " <<
                                   name << "=" << profile_map << dendl;
                                   pending_inc.set_erasure_code_profile(name,
                                                                        profile_map);}
                                   ...
                                   if (pending_inc.
                                       has_erasure_code_profile(profile)) {
                                   dout(20) << "erasure code profile " <<
                                   profile << " already pending" << dendl;
                                   goto wait;}

                                   map < string, string > profile_map;
                                   err =
                                   osdmap.get_erasure_code_profile_default(cct,
                                                                           profile_map,
                                                                           &ss);
                                   if (err)
                                   goto reply;
                                   err =
                                   normalize_profile(name, profile_map, true,
                                                     &ss); if (err)
                                   goto reply;
                                   dout(20) << "erasure code profile set " <<
                                   profile << "=" << profile_map << dendl;
                                   pending_inc.set_erasure_code_profile(profile,
                                                                        profile_map);
                                   goto wait;}
                                   }
                                   ...
                                   int rule;
                                   err =
                                   crush_rule_create_erasure(name, profile,
                                                             &rule, &ss);
                                   if (err < 0) {
                                   switch (err) {
case -EEXIST:                  // return immediately
ss << "rule " << name << " already exists"; err = 0; goto reply; break; case -EALREADY:    // wait for pending to be proposed
ss << "rule " << name << " already exists"; err = 0; break; default:   // non recoverable error
                                   goto reply; break;}
                                   }
                                   else {
                                   ss << "created rule " << name << " at " <<
                                   rule;}
                                   ...
                                   ss << "crush rule " << name << " (" << ruleno
                                   << ") is in use"; err = -EBUSY; goto reply;}
                                   ...
                                   if (prefix == "osd crush rule rename") {
                                   string srcname;
                                   string dstname;
                                   ...
                                   CrushWrapper newcrush = _get_pending_crush();
                                   if (!newcrush.rule_exists(srcname)
                                       && newcrush.rule_exists(dstname)) {
      ...
      // (so this command is idempotent)
                                   ...
                                   err =
                                   newcrush.rename_rule(srcname, dstname, &ss);
                                   if (err < 0) {
      // ss has reason for failure
                                   goto reply;}
                                   pending_inc.crush.clear();
    ...
    // Note: setmaxosd sets the maximum OSD number and not the number of OSDs
      ...
      // of OSDs.
                                   ...
                                   ss << "set new max_osd = " << pending_inc.
                                   new_max_osd; getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                                                        0, rs,
                                   ...
                                   if (prefix == "osd set-backfillfull-ratio")
                                   pending_inc.new_backfillfull_ratio = n;
                                   else
                                   if (prefix == "osd set-nearfull-ratio")
                                   ...
                                   string v; cmd_getval(cmdmap, "version", v);
                                   ceph_release_t vno =
                                   ceph_release_from_name(v); if (!vno) {
                                   ss << "version " << v <<
                                   " is not recognized"; err = -EINVAL;
                                   goto reply;}
                                   OSDMap newmap;
                                   newmap.deepish_copy_from(osdmap);
                                   ...
                                   << vno; err = -EPERM; goto reply;}
                                   bool sure = false;
                                   cmd_getval(cmdmap, "yes_i_really_mean_it",
                                              sure); if (!sure) {
FeatureMap m; mon.get_combined_feature_map(&m); uint64_t features = ceph_release_features(to_integer < int >(vno)); bool first = true; bool ok = true; for (int type:{
                                                                                                                                                                                               CEPH_ENTITY_TYPE_CLIENT,
                                                                                                                                                                                               CEPH_ENTITY_TYPE_MDS,
                                                                                                                                                                                               CEPH_ENTITY_TYPE_MGR})
                                   {
                                   auto p = m.m.find(type); if (p == m.m.end()) {
                                   continue;}
for (auto & q:                    p->second) {
                                   ...
                                   if (first) {
                                   ss <<
                                   "cannot set require_min_compat_client to " <<
                                   v << ": ";}
                                   else {
                                   ss << "; ";}
                                   first = false;
                                   ...
                                   << "(s) look like " <<
                                   ceph_release_name(ceph_release_from_features
                                                     (q.first))
                                   << " (missing 0x" << std::
                                   hex << missing << std::dec << ")";
                                   ok = false;}
                                   ...
                                   ss << "set require_min_compat_client to " <<
                                   vno;
                                   pending_inc.new_require_min_compat_client =
                                   vno; getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                   ...
                                   if (prefix == "osd pause") {
                                   return prepare_set_flag(op,
                                                           CEPH_OSDMAP_PAUSERD |
                                                           CEPH_OSDMAP_PAUSEWR);}
                                   else
                                   if (prefix == "osd unpause") {
                                   return prepare_unset_flag(op,
                                                             CEPH_OSDMAP_PAUSERD
                                                             |
                                                             CEPH_OSDMAP_PAUSEWR);}
                                   else
      ...
      // we are reusing a jewel feature bit that was retired in luminous.
                                   ...
                                   cmd_getval(cmdmap, "yes_i_really_mean_it",
                                              sure);
                                   ceph_release_t rel =
                                   ceph_release_from_name(release.c_str());
                                   if (!rel) {
      ...
      // idempotent
                                   ...
                                   if (!mon.monmap->get_required_features().
                                       contains_all(ceph::features::mon::
                                                    FEATURE_PACIFIC)) {
                                   ...
                                   if (!mon.monmap->get_required_features().
                                       contains_all(ceph::features::mon::
                                                    FEATURE_REEF)) {
                                   ss << "not all mons are reef"; err = -EPERM;
                                   goto reply;}
                                   if ((!HAVE_FEATURE
                                        (osdmap.get_up_osd_features(),
                                         SERVER_REEF))
                                   ...
                                   vector < string > idvec;
                                   cmd_getval(cmdmap, "ids", idvec);
                                   cmd_getval(cmdmap, "definitely_dead",
                                              definitely_dead);
                                   derr << "definitely_dead " << (int)
                                   definitely_dead << dendl;
                                   for (unsigned j = 0;
                                        j < idvec.size() && !stop; j++) {
                                   set < int >osds;
                                   // wildcard?
          ...
          // touch out osds only
                                   ...
                                   stop = true; verbose = false;    // so the output is less noisy.
                                   }
                                   else {
                                   long osd =
                                   parse_osd_id(idvec[j].c_str(), &ss);
                                   if (osd < 0) {
                                   ss << "invalid osd id" << osd; err = -EINVAL;
                                   continue;}
                                   else
                                   if (!osdmap.exists(osd)) {
                                   ss << "osd." << osd << " does not exist. ";
...
for (auto & osd:                  osds) {
                                   ...
                                   pending_inc.pending_osd_state_set(osd,
                                                                     CEPH_OSD_UP);
                                   ss << "marked down osd." << osd << ". ";
                                   any = true;}
                                   if (definitely_dead) {
                                   if (!pending_inc.new_xinfo.count(osd)) {
                                   pending_inc.new_xinfo[osd] =
                                   osdmap.osd_xinfo[osd];}
                                   if (pending_inc.new_xinfo[osd].dead_epoch <
                                       pending_inc.epoch) {
                                   any = true;}
                                   pending_inc.new_xinfo[osd].dead_epoch =
                                   pending_inc.epoch;}
                                   }
                                   else
                                   if (prefix == "osd out") {
                                   ...
                                   pending_inc.new_xinfo[osd] =
                                   osdmap.osd_xinfo[osd];}
                                   pending_inc.new_xinfo[osd].old_weight =
                                   osdmap.osd_weight[osd];}
                                   ss << "marked out osd." << osd << ". ";
                                   std::ostringstream msg;
                                   msg << "Client " << op->get_session()->
                                   entity_name << " marked osd." << osd <<
                                   " out"; if (osdmap.is_up(osd)) {
                                   msg << ", while it was still marked up";}
                                   else {
                                   auto period =
                                   ceph_clock_now() - down_pending_out[osd];
                                   msg << ", after it was down for " <<
                                   int (period.sec())
                                   << " seconds";}

                                   mon.clog->info() << msg.str(); any = true;}
                                   }
                                   else
                                   if (prefix == "osd in") {
                                   if (osdmap.is_in(osd)) {
                                   ...
                                   pending_inc.new_weight[osd] =
                                   osdmap.osd_xinfo[osd].old_weight;
                                   if (pending_inc.new_xinfo.count(osd) == 0) {
                                   pending_inc.new_xinfo[osd] =
                                   osdmap.osd_xinfo[osd];}
                                   ...
                                   if (prefix == "osd set-group"
                                       || prefix == "osd unset-group") {
cmd_getval(cmdmap, "flags", flag_str); cmd_getval(cmdmap, "who", who); vector < string > raw_flags; boost::split(raw_flags, flag_str, boost::is_any_of(",")); for (auto & f:raw_flags) {
                                   if (f == "noup")
                                   flags |= CEPH_OSD_NOUP;
                                   else
                                   if (f == "nodown")
                                   ...
                                   ss << "unrecognized flag '" << f <<
                                   "', must be one of " <<
                                   "{noup,nodown,noin,noout}"; err = -EINVAL;
                                   goto reply;}
                                   }
                                   }
                                   else {
                                   ...
                                   if (prefix.find("nodown") != string::npos)
                                   flags = CEPH_OSD_NODOWN;
                                   else
                                   if (prefix.find("noin") != string::npos)
                                   flags = CEPH_OSD_NOIN;
                                   else
                                   if (prefix.find("noout") != string::npos)
                                   flags = CEPH_OSD_NOOUT;
                                   else
                                   ...
                                   err = -EINVAL; goto reply;}
set < int >osds; set < int >crush_nodes; set < int >device_classes; for (auto & w:who) {
                                   if (w == "any" || w == "all" || w == "*") {
                                   osdmap.get_all_osds(osds); break;}
                                   std::stringstream ts;
                                   if (auto osd = parse_osd_id(w.c_str(), &ts);
                                       osd >= 0) {
                                   osds.insert(osd);}
                                   else
                                   if (osdmap.crush->name_exists(w)) {
                                   crush_nodes.insert(osdmap.crush->
                                                      get_item_id(w));}
                                   else
                                   if (osdmap.crush->class_exists(w)) {
                                   device_classes.insert(osdmap.crush->
                                                         get_class_id(w));}
                                   else {
                                   ss <<
                                   ...
                                   if (osds.empty() && crush_nodes.empty()
                                       && device_classes.empty()) {
      // ss has reason for failure
                                   err = -EINVAL; goto reply;}
bool any = false; for (auto osd:  osds) {
                                   if (!osdmap.exists(osd)) {
                                   ss << "osd." << osd << " does not exist. ";
                                   continue;}
                                   if (do_set) {
                                   ...
                                   pending_inc.pending_osd_state_clear(osd,
                                                                       CEPH_OSD_NOUP)
                                   : pending_inc.pending_osd_state_set(osd,
                                                                       CEPH_OSD_NOUP);}
...
for (auto & id:                   crush_nodes) {
                                   auto old_flags = osdmap.get_crush_node_flags(id); auto & pending_flags = pending_inc.new_crush_node_flags[id]; pending_flags |= old_flags;   // adopt existing flags first!
                                   if (do_set) {
                                   pending_flags |= flags;}
                                   else {
                                   pending_flags &= ~flags;}
...
for (auto & id:                   device_classes) {
                                   auto old_flags =
                                   osdmap.get_device_class_flags(id);
                                   auto & pending_flags =
                                   pending_inc.new_device_class_flags[id];
                                   pending_flags |= old_flags; if (do_set) {
                                   pending_flags |= flags;}
                                   else {
                                   pending_flags &= ~flags;}
                                   any = true;}
...
for (auto osd:                    id_vec) {
                                   ...
                                   int pool_min_size =
                                   osdmap.get_pg_pool_min_size(pgid);
                                   if ((int)new_pg_temp.size() <
                                       pool_min_size) {
                                   ss << "num of osds (" << new_pg_temp.
                                   size() << ") < pool min size (" <<
                                   pool_min_size << ")"; err = -EINVAL;
                                   goto reply;}

                                   int pool_size =
                                   osdmap.get_pg_pool_size(pgid);
                                   if ((int)new_pg_temp.size() > pool_size) {
                                   ss << "num of osds (" << new_pg_temp.
                                   size() << ") > pool size (" << pool_size <<
                                   ")"; err = -EINVAL; goto reply;}

                                   pending_inc.new_pg_temp[pgid] =
                                   mempool::osdmap::vector <
                                   int >(new_pg_temp.begin(),
                                         new_pg_temp.end());
                                   ss << "set " << pgid <<
      ...
      // that this won't work if there is only one suitable OSD in the cluster.
                                   ...
                                   if (i == primary || !osdmap.is_up(i)
                                       || !osdmap.exists(i)) {
                                   continue;}
                                   pending_inc.new_pg_temp[pgid] = {
                                   primary, i}; done = true; break;}
                                   if (!done) {
                                   err = -EAGAIN;
                                   ss <<
                                   ...
                                   } upmap_option; if (prefix == "osd pg-upmap") {
...
min_release = ceph_release_t::luminous; feature_name = "pg-upmap"; break; case OP_PG_UPMAP_PRIMARY:    // fall through
case OP_RM_PG_UPMAP_PRIMARY:
min_release = ceph_release_t::reef; feature_name = "pg-upmap-primary"; break; default:
                                   ceph_abort_msg
                                   ("invalid upmap option");}
                                   uint64_t min_feature =
                                   CEPH_FEATUREMASK_OSDMAP_PG_UPMAP;
                                   string min_release_name =
                                   ceph_release_name(static_cast <
                                                     int >(min_release));
                                   if (osdmap.require_min_compat_client <
                                       min_release) {
                                   ss << "min_compat_client " << osdmap.
                                   require_min_compat_client << " < " <<
                                   min_release_name <<
    ...
    //TODO: Should I add feature and test for upmap-primary?
                                   err =
                                   check_cluster_features(min_feature, ss);
                                   if (err == -EAGAIN)
                                   goto wait; if (err < 0)
                                   goto reply;
                                   pg_t pgid;
                                   ...
                                   if (err < 0)
                                   goto reply;
                                   if (pending_inc.old_pools.
                                       count(pgid.pool())) {
                                   ss << "pool of " << pgid <<
                                   " is pending removal"; err = -ENOENT;
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
...
case OP_RM_PG_UPMAP:
                                   ...
                                   wait_for_finished_proposal(op,
                                                              ...
                                                              (this, op));
                                   return true;}
break; case OP_PG_UPMAP_PRIMARY:   // fall through
case OP_RM_PG_UPMAP_PRIMARY:
                                   {
                                   const pg_pool_t * pt =
                                   osdmap.get_pg_pool(pgid.pool());
                                   if (!pt->is_replicated()) {
                                   ss <<
...
case OP_RM_PG_UPMAP_ITEMS:     // fall through
                                   if (pending_inc.new_pg_upmap_items.
                                       count(pgid)
...
vector < int32_t > new_pg_upmap; for (auto osd:id_vec) {
                                   if (osd != CRUSH_ITEM_NONE
                                       && !osdmap.exists(osd)) {
                                   ss << "osd." << osd << " does not exist";
                                   err = -ENOENT; goto reply;}
                                   auto it =
                                   std::find(new_pg_upmap.begin(),
                                             new_pg_upmap.end(), osd);
...
break; case OP_RM_PG_UPMAP:
                                   {
                                   pending_inc.old_pg_upmap.insert(pgid);
                                   ss << "clear " << pgid <<
                                   " pg_upmap mapping";}
break; case OP_PG_UPMAP_ITEMS:
                                   {
                                   vector < int64_t > id_vec;
                                   if (!cmd_getval(cmdmap, "id", id_vec)) {
                                   ss << "unable to parse 'id' value(s) '"
                                   ...
                                   ss << "num of osd pairs (" << id_vec.size() /
                                   2 << ") > pool size (" << pool_size << ")";
                                   err = -EINVAL; goto reply;}

                                   vector < pair < int32_t,
                                   int32_t >> new_pg_upmap_items;
                                   ostringstream items; items << "[";
                                   for (auto p = id_vec.begin();
                                        p != id_vec.end(); ++p) {
                                   int from = *p++; int to = *p; if (from == to) {
                                   ss << "from osd." << from << " == to osd." <<
                                   to << ", "; continue;}
                                   if (!osdmap.exists(from)) {
                                   ...
                                   make_pair(from, to);
                                   auto it =
                                   std::find(new_pg_upmap_items.begin(),
                                   ...
                                   items << from << "->" << to << ",";}
                                   string out(items.str()); out.resize(out.size() - 1); // drop last ','
                                   out += "]"; if (new_pg_upmap_items.empty()) {
                                   ss <<
                                   "no valid upmap items(pairs) is specified";
                                   err = -EINVAL; goto reply;}

                                   pending_inc.new_pg_upmap_items[pgid] =
                                   mempool::osdmap::vector < pair < int32_t,
                                   int32_t >> (new_pg_upmap_items.begin(),
                                               new_pg_upmap_items.end());
                                   ss << "set " << pgid <<
                                   " pg_upmap_items mapping to " << out;}
break; case OP_RM_PG_UPMAP_ITEMS:
                                   {
                                   pending_inc.old_pg_upmap_items.insert(pgid);
                                   ss << "clear " << pgid <<
                                   " pg_upmap_items mapping";}
break; case OP_PG_UPMAP_PRIMARY:
                                   {
                                   int64_t id;
                                   ...
                                   << cmd_vartype_stringify(cmdmap.
                                                            at("id")) << "'";
                                   err = -EINVAL; goto reply;}
                                   if (id != CRUSH_ITEM_NONE
                                       && !osdmap.exists(id)) {
                                   ss << "osd." << id << " does not exist";
                                   err = -ENOENT; goto reply;}
                                   vector < int >acting; int primary;
                                   osdmap.pg_to_acting_osds(pgid, &acting,
                                                            &primary);
                                   if (id == primary) {
                                   ss << "osd." << id <<
                                   ...
                                   err = -EINVAL; goto reply;}
                                   int found_idx = 0; for (int i = 1; i < (int)acting.size(); i++) {    // skip 0 on purpose
                                   if (acting[i] == id) {
                                   found_idx = i; break;}
                                   }
                                   if (found_idx == 0) {
                                   ...
                                   if (osdmap.crush->
                                       verify_upmap(cct,
                                                    osdmap.
                                                    get_pg_pool_crush_rule
                                                    (pgid), pool_size,
                                                    new_acting) >= 0) {
                                   ss << "change primary for pg " << pgid <<
                                   " to osd." << id;}
                                   else {
                                   ss << "can't change primary for pg " << pgid
                                   << " to osd." << id <<
                                   ...
                                   ldout(cct,
                                         20) << "pg " << pgid <<
                                   ": set pg_upmap_primary to " << id << dendl;}
break; case OP_RM_PG_UPMAP_PRIMARY:
                                   {
                                   pending_inc.old_pg_upmap_primary.
                                   insert(pgid);
                                   ss << "clear " << pgid <<
...
break; default:
                                   ...
                                   (int)((double)CEPH_OSD_MAX_PRIMARY_AFFINITY *
                                         w); if (ww < 0L) {
                                   ss << "weight must be >= 0"; err = -EINVAL;
                                   goto reply;}
                                   if (osdmap.require_min_compat_client !=
                                       ceph_release_t::unknown
                                       && osdmap.require_min_compat_client <
                                   ...
                                   ss << "set osd." << id <<
                                   " primary-affinity to " << w << " (" << std::
                                   ios::hex << ww << std::ios::dec << ")";
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                   ...
                                   if (prefix == "osd reweightn") {
                                   map < int32_t, uint32_t > weights;
                                   err =
                                   parse_reweights(cct, cmdmap, osdmap,
                                                   &weights); if (err) {
                                   ss << "unable to parse 'weights' value '" <<
                                   cmd_vartype_stringify(cmdmap.
                                                         at("weights")) << "'";
                                   goto reply;}
                                   pending_inc.new_weight.insert(weights.
                                                                 begin(),
                                                                 weights.end());
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
    ...
    // make sure authmon is writeable.
                                   ...
                                   << "osd destroy" << dendl;
                                   mon.authmon()->wait_for_writeable(op,
                                                                     new
                                                                     C_RetryMessage
                                                                     (this,
                                                                      op));
                                   ...
                                   if (!osdmap.exists(id)) {
                                   ss << "osd." << id << " does not exist"; err = 0;    // idempotent
                                   goto reply;}
                                   else
                                   if (osdmap.is_up(id)) {
                                   ss << "osd." << id << " is not `down`.";
                                   ...
                                   if (is_destroy && osdmap.is_destroyed(id)) {
                                   ss << "destroyed osd." << id;
                                   err = 0; goto reply;}

                                   if (prefix == "osd purge-new" &&
                                       (osdmap.get_state(id) & CEPH_OSD_NEW) ==
                                       0) {
                                   ss << "osd." << id << " is not new";
                                   err = -EPERM; goto reply;}

                                   bool goto_reply = false;
                                   paxos.plug(); if (is_destroy) {
                                   err = prepare_command_osd_destroy(id, ss);
                                   ...
                                   paxos.unplug(); if (err < 0 || goto_reply) {
                                   goto reply;}
                                   ...
                                   wait_for_finished_proposal(op,
                                                              ...
                                                              C_Command(mon, op,
                                                                        ...
                                                                        () +
                                                                        1));
                                   force_immediate_propose(); return true;}
                                   else
                                   if (prefix == "osd new") {
    ...
    // make sure kvmon is writeable.
                                   ...
                                   "osd new" << dendl;
                                   mon.kvmon()->wait_for_writeable(op,
                                                                   new
                                                                   C_RetryMessage
                                                                   (this, op));
                                   return false;}
                                   ...
                                   dout(20) << __func__ << " osd new json = " <<
                                   param_json << dendl;
                                   err =
                                   get_json_str_map(param_json, ss, &param_map);
                                   if (err < 0)
                                   goto reply;
                                   dout(20) << __func__ << " osd new params " <<
                                   param_map << dendl; paxos.plug();
                                   err =
                                   prepare_command_osd_new(op, cmdmap,
                                                           param_map, ss,
                                                           f.get());
                                   paxos.unplug(); if (err < 0) {
                                   goto reply;}
    ...
    // optional id provided?
                                   ...
                                   "'"; err = -EINVAL; goto reply;}
                                   dout(10) << " osd create got id " << cmd_id
                                   << dendl;}
      ...
      // ensure the operation's idempotency.
                                   ...
                                   int32_t new_id = -1;
                                   err =
                                   prepare_command_osd_create(id, uuid, &new_id,
                                                              ss);
                                   if (err < 0) {
                                   if (err == -EAGAIN) {
                                   wait_for_finished_proposal(op,
      ...
      // this is an idempotent operation; we can go ahead and reply.
                                   ...
                                   ss << new_id; rdata.append(ss);}
                                   err = 0; goto reply;}

                                   string empty_device_class;
                                   do_osd_create(id, uuid, empty_device_class,
                                                 &new_id); if (f) {
                                   f->open_object_section("created_osd");
                                   f->dump_int("osdid", new_id);
                                   f->close_section(); f->flush(rdata);}
                                   ...
                                   if (prefix == "osd blocklist clear" ||
                                       prefix == "osd blacklist clear") {
pending_inc.new_blocklist.clear(); std::list < std::pair < entity_addr_t, utime_t > >blocklist; std::list < std::pair < entity_addr_t, utime_t > >range_b; osdmap.get_blocklist(&blocklist, &range_b); for (const auto & entry:blocklist) {
                                   pending_inc.old_blocklist.push_back(entry.
                                                                       first);}
for (const auto & entry:          range_b) {
                                   ...
                                   entity_addr_t addr; if (!addr.parse(addrstr)) {
                                   ss << "unable to parse address " << addrstr;
                                   err = -EINVAL; goto reply;}
                                   else {
                                   if (range) {
                                   if (!addr.maybe_cidr()) {
                                   ss << "You specified a range command, but "
                                   << addr << " does not parse as a CIDR range";
                                   err = -EINVAL; goto reply;}
                                   addr.type = entity_addr_t::TYPE_CIDR;
                                   err =
                                   check_cluster_features
                                   (CEPH_FEATUREMASK_RANGE_BLOCKLIST, ss);
                                   if (err) {
                                   goto reply;}
      ...
      // always blocklist type ANY
                                   ...
                                   string blocklistop;
                                   if (!cmd_getval
                                       (cmdmap, "blocklistop", blocklistop)) {
                                   cmd_getval(cmdmap, "blacklistop",
                                              blocklistop);}
                                   if (blocklistop == "add") {
                                   utime_t expires = ceph_clock_now();
                                   // default one hour
                                   double d =
                                   cmd_getval_or < double >(cmdmap, "expire",
                                                            g_conf()->
                                                            mon_osd_blocklist_default_expire);
                                   expires += d;
                                   auto add_to_pending_blocklists =
                                   [](auto & nb, auto & ob, const auto & addr,
                                      const auto & expires) {
                                   nb[addr] = expires;
                                   ...
                                   add_to_pending_blocklists(pending_inc.
                                                             new_range_blocklist,
                                                             pending_inc.
                                                             old_range_blocklist,
                                                             addr, expires);}
                                   ...
                                   if (blocklistop == "rm") {
                                   auto rm_from_pending_blocklists =
                                   [](const auto & addr, auto & blocklist,
                                      auto & ob, auto & pb) {
                                   if (blocklist.count(addr)) {
                                   ob.push_back(addr); return true;}
                                   else
                                   if (pb.count(addr)) {
                                   pb.erase(addr); return true;}
                                   return false;};
                                   if ((!range
                                   ...
                                   wait_for_finished_proposal(op,
                                                              ...
                                                              C_Command(mon, op,
                                                                        ...
                                                                        1));
                                   return true;}
                                   ss << addr << " isn't blocklisted"; err = 0;
                                   goto reply;}
                                   }
                                   }
                                   else
                                   ...
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(pool);
                                   if (p->is_unmanaged_snaps_mode()) {
                                   ss << "pool " << poolstr <<
                                   " is in unmanaged snaps mode"; err = -EINVAL;
                                   goto reply;}
                                   else
                                   if (p->snap_exists(snapname.c_str())) {
                                   ss << "pool " << poolstr << " snap " <<
                                   snapname << " already exists"; err = 0;
                                   goto reply;}
                                   else
                                   if (p->is_tier()) {
                                   ss << "pool " << poolstr <<
                                   " is a cache tier"; err = -EINVAL;
                                   goto reply;}
                                   pg_pool_t * pp = 0;
                                   if (pending_inc.new_pools.count(pool))
                                   pp = &pending_inc.new_pools[pool]; if (!pp) {
                                   pp = &pending_inc.new_pools[pool]; *pp = *p;}
                                   if (pp->snap_exists(snapname.c_str())) {
                                   ss << "pool " << poolstr << " snap " <<
                                   snapname << " already exists";}
                                   else {
                                   if (const auto & fsmap =
                                       mon.mdsmon()->get_fsmap();
                                       fsmap.pool_in_use(pool)) {
                                   dout(20) <<
                                   "pool-level snapshots have been disabled for pools "
                                   "attached to an fs - poolid:" << pool <<
                                   dendl; err = -EOPNOTSUPP; goto reply;}
                                   pp->add_snap(snapname.c_str(),
                                                ceph_clock_now());
                                   pp->set_snap_epoch(pending_inc.epoch);
                                   ss << "created pool " << poolstr << " snap "
                                   << snapname;}
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                   ...
                                   pp = &pending_inc.new_pools[pool]; *pp = *p;}
                                   snapid_t sn =
                                   pp->snap_exists(snapname.c_str()); if (sn) {
                                   pp->remove_snap(sn);
                                   pp->set_snap_epoch(pending_inc.epoch);
                                   ss << "removed pool " << poolstr << " snap "
                                   << snapname;}
                                   else {
                                   ss << "already removed pool " << poolstr <<
                                   " snap " << snapname;}
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                   ...
                                   int64_t pg_num =
                                   cmd_getval_or < int64_t > (cmdmap, "pg_num",
                                                              0);
                                   int64_t pg_num_min =
                                   cmd_getval_or < int64_t > (cmdmap,
                                                              "pg_num_min", 0);
                                   int64_t pg_num_max =
                                   cmd_getval_or < int64_t > (cmdmap,
                                                              "pg_num_max", 0);
                                   int64_t pgp_num =
                                   cmd_getval_or < int64_t > (cmdmap, "pgp_num",
                                                              pg_num);
                                   string pool_type_str;
                                   cmd_getval(cmdmap, "pool_type",
                                              pool_type_str);
                                   if (pool_type_str.empty())
                                   pool_type_str =
                                   g_conf().get_val < string >
                                   ("osd_pool_default_type"); string poolstr;
                                   cmd_getval(cmdmap, "pool", poolstr);
                                   bool confirm = false;
                                   ...
                                   err = 0; goto reply;}
                                   int64_t pool_id =
                                   osdmap.lookup_pg_pool_name(poolstr);
                                   if (pool_id >= 0) {
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(pool_id);
                                   ...
                                   cmd_getval(cmdmap, "erasure_code_profile",
                                              erasure_code_profile);
                                   if (pool_type == pg_pool_t::TYPE_ERASURE) {
                                   ...
                                   if (pending_inc.
                                       ...
                                       (erasure_code_profile)) {
                                   dout(20) << "erasure code profile " <<
                                   erasure_code_profile << " already pending" <<
                                   dendl; goto wait;}
                                   ...
                                   goto reply;
                                   dout(20) << "erasure code profile " <<
                                   erasure_code_profile << " set" << dendl;
                                   pending_inc.
                                   ...
                                   dout(1) <<
                                   "implicitly use rule named after the pool: "
                                   << poolstr << dendl; rule_name = poolstr;}
                                   }
                                   expected_num_objects =
                                   cmd_getval_or < int64_t > (cmdmap,
                                                              "expected_num_objects",
      ...
      //     and put expected_num_objects to rule field
                                   ...
                                   string interr;
                                   expected_num_objects =
                                   strict_strtoll(rule_name.c_str(), 10,
                                                  &interr);
                                   if (interr.length()) {
                                   ss << "error parsing integer value '" <<
                                   rule_name << "': " << interr; err = -EINVAL;
                                   goto reply;}
                                   }
                                   ...
                                   set < int32_t > osds;
                                   osdmap.get_all_osds(osds);
                                   bool has_filestore_osd =
                                   std::any_of(osds.begin(), osds.end(),
                                               [this] (int osd) {
                                               string type;
                                               if (!get_osd_objectstore_type
                                                   (osd, &type)) {
                                   ...
                                   if (fast_read_param > 0)
                                   fast_read = FAST_READ_ON; int64_t repl_size = 0; cmd_getval(cmdmap, "size", repl_size); int64_t target_size_bytes = 0; double target_size_ratio = 0.0; cmd_getval(cmdmap, "target_size_bytes", target_size_bytes); cmd_getval(cmdmap, "target_size_ratio", target_size_ratio); string pg_autoscale_mode; cmd_getval(cmdmap, "autoscale_mode", pg_autoscale_mode); bool bulk = cmd_getval_or < bool > (cmdmap, "bulk", 0); bool crimson = cmd_getval_or < bool > (cmdmap, "crimson", false) || cct->_conf.get_val < bool > ("osd_pool_default_crimson"); err = prepare_new_pool(poolstr, -1,  // default crush rule
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  rule_name,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  pg_num,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  pgp_num,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  pg_num_min,
                                   ...
                                   switch (err) {
case -EEXIST:
ss << "pool '" << poolstr << "' already exists"; break; case -EAGAIN:
wait_for_finished_proposal(op, new C_RetryMessage(this, op)); return true; case -ERANGE:
goto reply; default:
                                   goto reply; break;}
                                   }
                                   else {
    ...
    // osd pool delete/rm <poolname> <poolname again> --yes-i-really-really-mean-it
                                   ...
                                   if (poolstr2 != poolstr
                                       || (!force && !force_no_fake)) {
                                   ss <<
                                   "WARNING: this will *PERMANENTLY DESTROY* all data stored in pool "
                                   << poolstr <<
                                   ...
                                   int64_t pool_src =
                                   osdmap.lookup_pg_pool_name(srcpoolstr.
                                                              c_str());
                                   int64_t pool_dst =
                                   osdmap.lookup_pg_pool_name(destpoolstr.
                                                              c_str());
                                   bool confirm = false;
                                   //confirmation may be set to true only by internal operations.
                                   cmd_getval(cmdmap, "yes_i_really_mean_it",
        ...
        // report in case it was intentional, while expecting a failure.
                                   ss << "pool '" << srcpoolstr <<
                                   "' does not exist; pool '" << destpoolstr <<
      ...
      // source pool exists and so does the destination pool
                                   ss << "pool '" << destpoolstr <<
                                   "' already exists"; err = -EEXIST;
                                   goto reply;}

                                   int ret =
                                   _prepare_rename_pool(pool_src, destpoolstr);
                                   if (ret == 0) {
                                   ss << "pool '" << srcpoolstr <<
                                   "' renamed to '" << destpoolstr << "'";}
                                   else {
                                   ss << "failed to rename pool '" << srcpoolstr
                                   << "' to '" << destpoolstr << "': " <<
                                   cpp_strerror(ret);}
                                   getline(ss, rs);
                                   wait_for_finished_proposal(op,
                                   ...
                                   int64_t tierpool_id =
                                   osdmap.lookup_pg_pool_name(tierpoolstr);
                                   if (tierpool_id < 0) {
                                   ss << "unrecognized pool '" << tierpoolstr <<
                                   "'"; err = -ENOENT; goto reply;}
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(pool_id); ceph_assert(p);
                                   const pg_pool_t * tp =
                                   osdmap.get_pg_pool(tierpool_id);
                                   ceph_assert(tp);
                                   if (!_check_become_tier
                                       (tierpool_id, tp, pool_id, p, &err,
                                        &ss)) {
                                   goto reply;}

    // make sure new tier is empty
                                   bool force_nonempty = false;
                                   cmd_getval_compat_cephbool(cmdmap,
                                                              "force_nonempty",
                                                              force_nonempty);
                                   const pool_stat_t * pstats =
                                   mon.mgrstatmon()->get_pool_stat(tierpool_id);
                                   if (pstats
                                       && pstats->stats.sum.num_objects != 0
                                       && !force_nonempty) {
                                   ss << "tier pool '" << tierpoolstr <<
                                   "' is not empty; --force-nonempty to force";
                                   err = -ENOTEMPTY; goto reply;}
                                   if (tp->is_erasure()) {
                                   ss << "tier pool '" << tierpoolstr
                                   << "' is an ec pool, which cannot be a tier";
                                   err = -ENOTSUP; goto reply;}
                                   if ((!tp->removed_snaps.empty()
                                        || !tp->snaps.empty())
                                       && (!force_nonempty
                                           || !g_conf()->
    ...
    // go
                                   pg_pool_t * np =
                                   pending_inc.get_new_pool(pool_id, p);
                                   pg_pool_t * ntp =
                                   pending_inc.get_new_pool(tierpool_id, tp);
                                   if (np->tiers.count(tierpool_id)
                                       || ntp->is_tier()) {
                                   wait_for_finished_proposal(op,
                                                              new
                                                              C_RetryMessage
                                                              (this, op));
                                   return true;}
                                   np->tiers.insert(tierpool_id); np->set_snap_epoch(pending_inc.epoch);    // tier will update to our snap info
                                   ntp->tier_of = pool_id;
                                   ss << "pool '" << tierpoolstr <<
                                   ...
                                   osdmap.get_pg_pool(tierpool_id);
                                   ceph_assert(tp);
                                   if (!_check_remove_tier
                                       (pool_id, p, tp, &err, &ss)) {
                                   goto reply;}

                                   if (p->tiers.count(tierpool_id) == 0) {
                                   ss << "pool '" << tierpoolstr <<
                                   "' is now (or already was) not a tier of '"
                                   << poolstr << "'"; err = 0; goto reply;}
                                   if (tp->tier_of != pool_id) {
                                   ss << "tier pool '" << tierpoolstr <<
                                   "' is a tier of '" << osdmap.
                                   get_pool_name(tp->tier_of) << "': "
    ...
    // go
                                   ...
                                   cmd_getval(cmdmap, "overlaypool",
                                              overlaypoolstr);
                                   int64_t overlaypool_id =
                                   osdmap.lookup_pg_pool_name(overlaypoolstr);
                                   if (overlaypool_id < 0) {
                                   ss << "unrecognized pool '" << overlaypoolstr
                                   << "'"; err = -ENOENT; goto reply;}
                                   const pg_pool_t * p =
                                   osdmap.get_pg_pool(pool_id); ceph_assert(p);
                                   const pg_pool_t * overlay_p =
                                   osdmap.get_pg_pool(overlaypool_id);
                                   ceph_assert(overlay_p);
                                   if (p->tiers.count(overlaypool_id) == 0) {
                                   ss << "tier pool '" << overlaypoolstr <<
                                   "' is not a tier of '" << poolstr << "'";
                                   err = -EINVAL; goto reply;}
                                   if (p->read_tier == overlaypool_id) {
                                   err = 0;
                                   ss << "overlay for '" << poolstr <<
                                   "' is now (or already was) '" <<
                                   overlaypoolstr << "'"; goto reply;}
                                   if (p->has_read_tier()) {
                                   ss << "pool '" << poolstr <<
                                   "' has overlay '" << osdmap.get_pool_name(p->
                                                                             read_tier)
                                   << "'; please remove-overlay first";
                                   err = -EINVAL; goto reply;}
    ...
    // go
                                   ...
                                   np->set_last_force_op_resend(pending_inc.
                                                                epoch);
                                   pg_pool_t * noverlay_p =
                                   pending_inc.get_new_pool(overlaypool_id,
                                                            overlay_p);
                                   noverlay_p->
                                   set_last_force_op_resend(pending_inc.epoch);
                                   ss << "overlay for '" << poolstr <<
                                   "' is now (or already was) '" <<
                                   overlaypoolstr << "'";
                                   if (overlay_p->cache_mode ==
                                   ...
                                   err = 0;
                                   ss <<
                                   "there is now (or already was) no overlay for '"
                                   << poolstr << "'"; goto reply;}

                                   if (!_check_remove_tier
                                       (pool_id, p, NULL, &err, &ss)) {
                                   goto reply;}
    ...
    // go
                                   ...
                                   if (np->has_read_tier()) {
                                   const pg_pool_t * op =
                                   osdmap.get_pg_pool(np->read_tier);
                                   pg_pool_t * nop =
                                   pending_inc.get_new_pool(np->read_tier, op);
                                   nop->set_last_force_op_resend(pending_inc.
                                                                 epoch);}
                                   if (np->has_write_tier()) {
                                   const pg_pool_t * op =
                                   osdmap.get_pg_pool(np->write_tier);
                                   pg_pool_t * nop =
                                   pending_inc.get_new_pool(np->write_tier, op);
                                   nop->set_last_force_op_resend(pending_inc.
                                                                 epoch);}
                                   np->clear_read_tier();
                                   np->clear_write_tier();
                                   ...
                                   if (!p->is_tier()) {
                                   ss << "pool '" << poolstr <<
                                   "' is not a tier"; err = -EINVAL;
                                   goto reply;}
                                   string modestr;
                                   cmd_getval(cmdmap, "mode", modestr);
                                   pg_pool_t::cache_mode_t mode =
                                   pg_pool_t::get_cache_mode_from_str(modestr);
                                   if (int (mode) < 0) {
                                   ss << "'" << modestr <<
                                   "' is not a valid cache mode"; err = -EINVAL;
                                   goto reply;}
    ...
    // whatever mode is on the pending state.
                                   ...
                                   << "' on a '" << pg_pool_t::
                                   get_cache_mode_name(p->cache_mode)
                                   << "' pool; only '" << pg_pool_t::
                                   get_cache_mode_name(pg_pool_t::
                                                       CACHEMODE_PROXY)
                                   << "','" << pg_pool_t::
                                   get_cache_mode_name(pg_pool_t::
                                                       CACHEMODE_READPROXY)
                                   << "' allowed."; err = -EINVAL; goto reply;}
                                   if ((p->cache_mode ==
                                        pg_pool_t::CACHEMODE_READFORWARD
                                        && (mode !=
                                            pg_pool_t::CACHEMODE_WRITEBACK
                                   ...
                                   if (pstats
                                       && pstats->stats.sum.num_objects_dirty >
                                       0) {
                                   ss << "unable to set cache-mode '" <<
                                   pg_pool_t::
                                   get_cache_mode_name(mode) << "' on pool '" <<
                                   poolstr << "': dirty objects found";
                                   err = -EBUSY; goto reply;}
                                   }
    // go
                                   pg_pool_t * np =
    ...
    // make sure new tier is empty
                                   ...
                                   pg_pool_t::get_cache_mode_from_str(modestr);
                                   if (int (mode) < 0) {
                                   ss << "osd tier cache default mode '" <<
                                   modestr << "' is not a valid cache mode";
                                   err = -EINVAL; goto reply;}
                                   HitSet::Params hsp;
                                   ...
                                   if (cache_hit_set_type == "explicit_hash") {
                                   hsp =
                                   HitSet::Params(new ExplicitHashHitSet::
                                                  Params);}
                                   else
                                   if (cache_hit_set_type == "explicit_object") {
                                   hsp =
                                   HitSet::Params(new ExplicitObjectHitSet::
                                                  Params);}
                                   else {
                                   ...
                                   << cache_hit_set_type <<
                                   "' is not a known type"; err = -EINVAL;
                                   goto reply;}
    // go
                                   pg_pool_t * np =
                                   pending_inc.get_new_pool(pool_id, p);
                                   ...
                                   wait_for_finished_proposal(op,
                                                              ...
                                                              (this, op));
                                   return true;}
                                   np->tiers.insert(tierpool_id); np->read_tier = np->write_tier = tierpool_id; np->set_snap_epoch(pending_inc.epoch);  // tier will update to our snap info
                                   np->set_last_force_op_resend(pending_inc.
                                                                epoch);
                                   ntp->set_last_force_op_resend(pending_inc.
                                                                 epoch);
                                   ntp->tier_of = pool_id;
                                   ntp->cache_mode = mode;
                                   ntp->hit_set_count =
    ...
    // val could contain unit designations, so we treat as a string
                                   ...
                                   cmd_getval(cmdmap, "val", val);
                                   string tss;
                                   int64_t value; if (field == "max_objects") {
                                   value =
                                   strict_si_cast < uint64_t > (val, &tss);}
                                   else
                                   if (field == "max_bytes") {
                                   ...
                                   if (!tss.empty()) {
                                   ss << "error parsing value '" << val << "': "
                                   << tss; err = -EINVAL; goto reply;}
                                   ...
                                   ceph_abort_msg("unrecognized option");}
                                   ss << "set-quota " << field << " = " << value
                                   << " for pool " << poolstr; rs = ss.str();
                                   wait_for_finished_proposal(op,
                                                              new Monitor::
                                                              C_Command(mon, op,
                                                                        0, rs,
                                   ...
                                   if (pending_inc.new_pools.
                                       count(pgid.pool()) == 0) {
                                   pending_inc.new_pools[pgid.pool()] =
                                   *osdmap.get_pg_pool(pgid.pool());}
                                   pending_inc.new_pools[pgid.pool()].flags |=
                                   pg_pool_t::FLAG_CREATING; err = 0;
                                   goto update;}
                                   else {
                                   ...
                                   if (prefix == "osd set-allow-crimson") {
                                   ...
                                   err = 0; if (osdmap.get_allow_crimson()) {
                                   goto reply;}
                                   else {
                                   pending_inc.set_allow_crimson(); goto update;}
                                   }
                                   else {
                                   err = -EINVAL;}
...
rs = cpp_strerror(err); mon.reply_command(op, err, rs, rdata, get_last_committed()); return ret; update:
getline(ss, rs); wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1)); return true; wait:
                                   wait_for_finished_proposal(op,
                                                              new
                                                              C_RetryMessage
                                                              (this, op));
                                   ...
                                   bool OSDMonitor::
                                   enforce_pool_op_caps(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   MonSession * session = op->get_session();
                                   if (!session) {
                                   _pool_op_reply(op, -EPERM,
                                                  osdmap.get_epoch());
                                   return true;}
...
case POOL_OP_DELETE_UNMANAGED_SNAP:
                                   ...
                                   if (!is_unmanaged_snap_op_permitted
                                       (cct, mon.key_server,
                                        ...
                                        pool_name)) {
                                   dout(0) <<
                                   "got unmanaged-snap pool op from entity with insufficient "
                                   << "privileges. message: " << *m << std::
                                   endl << "caps: " << session->caps << dendl;
                                   _pool_op_reply(op, -EPERM,
                                                  osdmap.get_epoch());
                                   return true;}
                                   }
break; default:
                                   ...
                                   dout(0) <<
                                   "got pool op from entity with insufficient privileges. "
                                   << "message: " << *m << std::
                                   endl << "caps: " << session->caps << dendl;
                                   _pool_op_reply(op, -EPERM,
                                                  osdmap.get_epoch());
                                   ...
                                   bool OSDMonitor::
                                   preprocess_pool_op(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   ...
                                   if (p == nullptr) {
                                   dout(10) <<
                                   "attempt to operate on non-existent pool id "
                                   << m->pool << dendl;
                                   if (m->op == POOL_OP_DELETE) {
                                   _pool_op_reply(op, 0, osdmap.get_epoch());}
                                   else {
                                   _pool_op_reply(op, -ENOENT,
                                                  osdmap.get_epoch());}
                                   return true;}
...
case POOL_OP_CREATE_SNAP:
                                   ...
                                   _pool_op_reply(op, 0, osdmap.get_epoch());
                                   return true;}
return false; case POOL_OP_CREATE_UNMANAGED_SNAP:
                                   if (p->is_pool_snaps_mode()) {
                                   _pool_op_reply(op, -EINVAL,
                                                  osdmap.get_epoch());
                                   return true;}
return false; case POOL_OP_DELETE_SNAP:
                                   if (p->is_unmanaged_snaps_mode()) {
                                   _pool_op_reply(op, -EINVAL,
                                                  osdmap.get_epoch());
                                   return true;}
                                   ...
                                   _pool_op_reply(op, 0, osdmap.get_epoch());
                                   return true;}
return false; case POOL_OP_DELETE_UNMANAGED_SNAP:
                                   if (p->is_pool_snaps_mode()) {
                                   _pool_op_reply(op, -EINVAL,
                                                  osdmap.get_epoch());
                                   return true;}
                                   ...
                                   _pool_op_reply(op, 0, osdmap.get_epoch());
                                   return true;}
return false; case POOL_OP_DELETE:
                                   if (osdmap.
                                       lookup_pg_pool_name(m->name.c_str()) >=
                                       0) {
                                   _pool_op_reply(op, 0, osdmap.get_epoch());
                                   return true;}
return false; case POOL_OP_AUID_CHANGE:
return false; default:
                                   ...
                                   bool OSDMonitor::
                                   _is_removed_snap(int64_t pool,
                                                    snapid_t snap) {
                                   if (!osdmap.have_pg_pool(pool)) {
                                   dout(10) << __func__ << " pool " << pool <<
                                   " snap " << snap << " - pool dne" << dendl;
                                   return true;}
                                   if (osdmap.
                                       in_removed_snaps_queue(pool, snap)) {
                                   dout(10) << __func__ << " pool " << pool <<
                                   " snap " << snap <<
                                   ...
                                   snapid_t begin, end;
                                   int r =
                                   lookup_purged_snap(pool, snap, &begin, &end);
                                   if (r == 0) {
                                   dout(10) << __func__ << " pool " << pool <<
                                   " snap " << snap << " - purged, [" << begin
                                   << "," << end << ")" << dendl; return true;}
                                   return false;}

                                   bool OSDMonitor::
                                   _is_pending_removed_snap(int64_t pool,
                                                            snapid_t snap) {
                                   if (pending_inc.old_pools.count(pool)) {
                                   dout(10) << __func__ << " pool " << pool <<
                                   " snap " << snap <<
                                   ...
                                   return true;}
                                   if (pending_inc.
                                       in_new_removed_snaps(pool, snap)) {
                                   dout(10) << __func__ << " pool " << pool <<
                                   " snap " << snap <<
                                   ...
                                   bool OSDMonitor::
                                   preprocess_pool_op_create(MonOpRequestRef op)
                                   {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   int64_t pool =
                                   osdmap.lookup_pg_pool_name(m->name.c_str());
                                   if (pool >= 0) {
                                   _pool_op_reply(op, 0, osdmap.get_epoch());
                                   return true;}
                                   ...
                                   bool OSDMonitor::
                                   prepare_pool_op(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   dout(10) << "prepare_pool_op " << *m <<
                                   dendl; if (m->op == POOL_OP_CREATE) {
                                   ...
                                   dout(20) <<
                                   "monitor-managed snapshots have been disabled for pools "
                                   " attached to an fs - pool:" << m->
                                   pool << dendl;
                                   _pool_op_reply(op, -EOPNOTSUPP,
                                                  osdmap.get_epoch());
                                   return false;}
                                   }
...
case POOL_OP_CREATE_SNAP:
                                   if (pool->is_tier()) {
                                   ret = -EINVAL; _pool_op_reply(op, ret, osdmap.get_epoch()); return false;}   // else, fall through
case POOL_OP_DELETE_SNAP:
                                   if (!pool->is_unmanaged_snaps_mode()) {
                                   bool snap_exists =
                                   pool->snap_exists(m->name.c_str());
                                   if ((m->op == POOL_OP_CREATE_SNAP
                                        && snap_exists)
                                   ...
                                   ret = -EINVAL;}
_pool_op_reply(op, ret, osdmap.get_epoch()); return false; case POOL_OP_DELETE_UNMANAGED_SNAP:
      // we won't allow removal of an unmanaged snapshot from a pool
      // not in unmanaged snaps mode.
                                   if (!pool->
                                       is_unmanaged_snaps_mode()) {
                                   _pool_op_reply(op, -ENOTSUP,
                                                  osdmap.get_epoch());
                                   return false;}
      /* fall-thru */
case POOL_OP_CREATE_UNMANAGED_SNAP:
...
case POOL_OP_DELETE_SNAP:
                                   ...
                                   ret = -EINVAL; goto out;}
break; case POOL_OP_CREATE_UNMANAGED_SNAP:
case POOL_OP_DELETE_UNMANAGED_SNAP:
...
case POOL_OP_CREATE_SNAP:
                                   ...
                                   pp.add_snap(m->name.c_str(),
                                               ceph_clock_now());
                                   dout(10) << "create snap in pool " << m->
                                   pool << " " << m->name << " seq " << pp.
                                   get_snap_epoch() << dendl; changed = true;}
break; case POOL_OP_DELETE_SNAP:
                                   {
                                   snapid_t s = pp.snap_exists(m->name.c_str());
                                   if (s) {
                                   pp.remove_snap(s);
                                   pending_inc.new_removed_snaps[m->pool].
                                   insert(s); changed = true;}
                                   }
break; case POOL_OP_CREATE_UNMANAGED_SNAP:
                                   {
                                   uint64_t snapid =
                                   pp.add_unmanaged_snap(osdmap.
                                                         require_osd_release <
                                                         ceph_release_t::
                                                         octopus);
                                   encode(snapid, reply_data); changed = true;}
break; case POOL_OP_DELETE_UNMANAGED_SNAP:
                                   if (!_is_removed_snap(m->pool, m->snapid) &&
                                       !_is_pending_removed_snap(m->pool,
                                                                 m->snapid)) {
                                   ...
                                   pending_pseudo_purged_snaps[m->pool].
                                   insert(pp.get_snap_seq()); changed = true;}
break; case POOL_OP_AUID_CHANGE:
_pool_op_reply(op, -EOPNOTSUPP, osdmap.get_epoch()); return false; default:
                                   ceph_abort(); break;}
...
out:
                                   ...
                                   bool OSDMonitor::
                                   prepare_pool_op_create(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   ...
                                   int OSDMonitor::
                                   _check_remove_pool(int64_t pool_id,
                                                      const pg_pool_t & pool,
                                                      ostream * ss) {
                                   const string & poolstr =
                                   osdmap.get_pool_name(pool_id);
                                   // If the Pool is in use by CephFS, refuse to delete it
                                   FSMap const &pending_fsmap =
                                   mon.mdsmon()->get_pending_fsmap();
                                   if (pending_fsmap.pool_in_use(pool_id)) {
                                   *ss << "pool '" << poolstr <<
                                   "' is in use by CephFS"; return -EBUSY;}

                                   if (pool.tier_of >= 0) {
                                   *ss << "pool '" << poolstr <<
                                   "' is a tier of '" << osdmap.
                                   get_pool_name(pool.tier_of) << "'";
                                   return -EBUSY;}
                                   if (!pool.tiers.empty()) {
*ss << "pool '" << poolstr << "' has tiers"; for (auto tier:pool.tiers) {
 ...
 */
                                   bool OSDMonitor::
                                   _check_become_tier(const int64_t
                                                      tier_pool_id,
                                                      const pg_pool_t *
                                                      tier_pool,
                                                      const int64_t
                                                      ...
                                                      base_pool, int *err,
                                                      ostream * ss)const {
                                   const std::string & tier_pool_name =
                                   osdmap.get_pool_name(tier_pool_id);
                                   const std::string & base_pool_name =
                                   osdmap.get_pool_name(base_pool_id);
                                   if (tier_pool->is_crimson()) {
                                   *ss << "pool '" << tier_pool_name <<
                                   "' is a crimson pool, tiering " <<
                                   "features are not supported"; *err = -EINVAL;
                                   return false;}
                                   ...
                                   *ss << "pool '" << base_pool_name <<
                                   ...
                                   const FSMap & pending_fsmap =
                                   mon.mdsmon()->get_pending_fsmap();
                                   if (pending_fsmap.
                                       pool_in_use(tier_pool_id)) {
                                   *ss << "pool '" << tier_pool_name <<
                                   "' is in use by CephFS"; *err = -EBUSY;
                                   return false;}

                                   if (base_pool->tiers.count(tier_pool_id)) {
                                   ceph_assert(tier_pool->tier_of ==
                                               base_pool_id); *err = 0;
                                   *ss << "pool '" << tier_pool_name <<
                                   "' is now (or already was) a tier of '" <<
                                   base_pool_name << "'"; return false;}

                                   if (base_pool->is_tier()) {
                                   *ss << "pool '" << base_pool_name <<
                                   "' is already a tier of '" << osdmap.
                                   get_pool_name(base_pool->
                                                 tier_of) << "', " <<
                                   "multiple tiers are not yet supported.";
                                   *err = -EINVAL; return false;}

                                   if (tier_pool->has_tiers()) {
                                   *ss << "pool '" << tier_pool_name <<
                                   "' has following tier(s) already:";
                                   for (set < uint64_t >::iterator it =
                                        tier_pool->tiers.begin();
                                   ...
                                   *ss << "tier pool '" << tier_pool_name <<
                                   "' is already a tier of '" << osdmap.
                                   get_pool_name(tier_pool->tier_of) << "'";
                                   *err = -EINVAL; return false;}
 ...
 */
                                   bool OSDMonitor::
                                   _check_remove_tier(const int64_t
                                                      base_pool_id,
                                                      const pg_pool_t *
                                                      base_pool,
                                                      const pg_pool_t *
      ...
      // CephFS relies on to access it
                                   *ss << "pool '" << base_pool_name <<
                                   ...
                                   " via its tier";
                                   *err = -EBUSY; return false;}

                                   if (tier_pool
                                       && tier_pool->cache_mode ==
                                       pg_pool_t::CACHEMODE_WRITEBACK) {
                                   *ss << "pool '" << base_pool_name <<
                                   ...
                                   *err = 0; return true;}
                                   ...
                                   int OSDMonitor::
                                   _prepare_remove_pool(int64_t pool,
                                                        ostream * ss,
                                                        bool no_fake) {
                                   ...
                                   return r;
                                   auto new_pool =
                                   pending_inc.new_pools.find(pool);
                                   if (new_pool !=
                                       pending_inc.new_pools.end()) {
    ...
    // this op.
                                   ...
                                   " already pending removal" << dendl;
                                   return 0;}

                                   if (g_conf()->mon_fake_pool_delete
                                       && !no_fake) {
                                   string old_name = osdmap.get_pool_name(pool);
                                   string new_name =
                                   old_name + "." + stringify(pool) +
                                   ".DELETED";
                                   dout(1) << __func__ <<
  ...
  // remove any primary_temp mappings for this pool
                                   ...
                                   if (p->first.pool() == pool) {
                                   dout(10) << __func__ << " " << pool
                                   << " removing obsolete primary_temp" << p->
                                   first << dendl;
                                   pending_inc.new_primary_temp[p->first] = -1;}
                                   }
...
for (auto & p:                    osdmap.pg_upmap_items) {
                                   if (p.first.pool() == pool) {
                                   dout(10) << __func__ << " " << pool
                                   << " removing obsolete pg_upmap_items " << p.
                                   first << dendl;
                                   pending_inc.old_pg_upmap_items.insert(p.
                                                                         first);}
                                   }
  // remove any pending pg_upmap mappings for this pool
                                   {
                                   auto it =
                                   pending_inc.new_pg_upmap_items.begin();
                                   while (it !=
                                          pending_inc.new_pg_upmap_items.
                                          end()) {
                                   if (it->first.pool() == pool) {
                                   dout(10) << __func__ << " " << pool
  ...
  // remove any choose_args for this pool
                                   ...
                                   dendl; newcrush.rm_choose_args(pool);
                                   pending_inc.crush.clear();
                                   newcrush.encode(pending_inc.crush,
                                                   mon.
                                                   get_quorum_con_features());}
                                   ...
                                   int OSDMonitor::
                                   _prepare_rename_pool(int64_t pool,
                                                        string newname) {
                                   dout(10) << "_prepare_rename_pool " << pool
                                   << dendl;
                                   if (pending_inc.old_pools.count(pool)) {
                                   dout(10) << "_prepare_rename_pool " << pool
                                   ...
                                   bool OSDMonitor::
                                   prepare_pool_op_delete(MonOpRequestRef op) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   ostringstream ss;
                                   int ret =
                                   ...
                                   void OSDMonitor::
                                   _pool_op_reply(MonOpRequestRef op, int ret,
                                                  epoch_t epoch,
                                                  bufferlist * blp) {
                                   op->mark_osdmon_event(__func__);
                                   auto m = op->get_req < MPoolOp > ();
                                   dout(20) << "_pool_op_reply " << ret <<
                                   dendl;
                                   MPoolOpReply * reply =
                                   new MPoolOpReply(m->fsid, m->get_tid(), ret,
                                                    epoch, get_last_committed(),
                                                    blp);
                                   mon.send_reply(op, reply);}

                                   void OSDMonitor::
                                   convert_pool_priorities(void) {
pool_opts_t::key_t key = pool_opts_t::get_opt_desc("recovery_priority").key; int64_t max_prio = 0; int64_t min_prio = 0; for (const auto & i:osdmap.get_pools()) {
                                   const auto & pool = i.second;
                                   if (pool.opts.is_set(key)) {
                                   int64_t prio = 0;
                                   pool.opts.get(key, &prio);
                                   if (prio > max_prio)
                                   max_prio = prio; if (prio < min_prio)
                                   min_prio = prio;}
                                   }
                                   if (max_prio <= OSD_POOL_PRIORITY_MAX
                                       && min_prio >= OSD_POOL_PRIORITY_MIN) {
                                   dout(20) << __func__ << " nothing to fix" <<
                                   dendl; return;}
  // Current pool priorities exceeds new maximum
for (const auto & i:              osdmap.get_pools()) {
                                   const auto pool_id = i.first; pg_pool_t pool = i.second; int64_t prio = 0; pool.opts.get(key, &prio); int64_t n; if (prio > 0 && max_prio > OSD_POOL_PRIORITY_MAX) { // Likely scenario
      ...
      // Scaled  priority range OSD_POOL_PRIORITY_MIN to 0
                                   ...
                                   << " recovery_priority adjusted " << prio <<
                                   " to " << n << dendl;
                                   ...
                                   void OSDMonitor::
                                   try_enable_stretch_mode_pools(stringstream &
                                                                 ss,
                                                                 bool * okay,
                                                                 int *errcode,
                                                                 set <
                                                                 ...
                                                                 new_crush_rule)
                                   {
                                   dout(20) << __func__ << dendl; *okay = false;
                                   int new_crush_rule_result =
                                   osdmap.crush->get_rule_id(new_crush_rule);
                                   if (new_crush_rule_result < 0) {
                                   ss << "unrecognized crush rule " <<
                                   new_crush_rule_result;
                                   *errcode = new_crush_rule_result; return;}
__u8 new_rule = static_cast < __u8 > (new_crush_rule_result); for (const auto & pooli:osdmap.pools) {
                                   int64_t poolid = pooli.first;
                                   const pg_pool_t * p = &pooli.second;
                                   if (!p->is_replicated()) {
                                   ss << "stretched pools must be replicated; '"
                                   << osdmap.
                                   pool_name[poolid] << "' is erasure-coded";
                                   *errcode = -EINVAL; return;}
                                   ...
                                   " default size/min_size, which '" << osdmap.
                                   pool_name[poolid] << "' does not";
                                   *errcode = -EINVAL; return;}
                                   pg_pool_t * pp =
                                   pending_inc.get_new_pool(poolid, p);
                                   ...
                                   *okay = true; return;}
                                   ...
                                   void OSDMonitor::
                                   try_enable_stretch_mode(stringstream & ss,
                                                           bool * okay,
                                                           int *errcode,
                                                           bool commit,
                                                           const string &
                                   ...
                                   *errcode = -ENOENT; ceph_assert(!commit);
                                   return;}
                                   else {
                                   dividing_id = *type_id;}
                                   vector < int >subtrees;
                                   crush.get_subtree_of_type(dividing_id,
                                                             &subtrees);
                                   if (subtrees.size() != 2) {
                                   ss << "there are " << subtrees.
                                   ...
                                   *errcode = -EINVAL;
                                   ...
                                   int new_crush_rule_result =
                                   crush.get_rule_id(new_crush_rule);
                                   if (new_crush_rule_result < 0) {
                                   ss << "unrecognized crush rule " <<
                                   new_crush_rule;
                                   *errcode = new_crush_rule_result;
                                   ceph_assert(!commit
                                               || (new_crush_rule_result > 0));
                                   return;}
                                   __u8 new_rule =
                                   static_cast < __u8 > (new_crush_rule_result);
                                   int weight1 =
                                   crush.get_item_weight(subtrees[0]);
                                   int weight2 =
                                   crush.get_item_weight(subtrees[1]);
                                   if (weight1 != weight2) {
...
for (auto pool:                   pools) {
                                   ...
                                   pending_inc.new_stretch_bucket_count =
                                   bucket_count;
                                   pending_inc.new_degraded_stretch_mode = 0;
                                   pending_inc.new_stretch_mode_bucket =
                                   dividing_id;}
                                   *okay = true; return;}

                                   bool OSDMonitor::
                                   check_for_dead_crush_zones(const map <
                                                              string,
                                                              set <
                                                              string >>
                                                              &dead_buckets,
                                                              ...
                                                              *really_down_mons)
                                   {
dout(20) << __func__ << " with dead mon zones " << dead_buckets << dendl; ceph_assert(is_readable()); if (dead_buckets.empty())return false; set < int >down_cache; bool really_down = false; for (auto dbi:dead_buckets) {
                                   const string & bucket_name = dbi.first;
                                   ceph_assert(osdmap.crush->
                                               name_exists(bucket_name));
                                   int bucket_id =
                                   osdmap.crush->get_item_id(bucket_name);
                                   dout(20) << "Checking " << bucket_name <<
                                   " id " << bucket_id <<
                                   " to see if OSDs are also down" << dendl;
                                   bool subtree_down =
                                   osdmap.subtree_is_down(bucket_id,
                                                          &down_cache);
                                   if (subtree_down) {
                                   dout(20) << "subtree is down!" << dendl;
                                   really_down = true;
                                   really_down_buckets->insert(bucket_id);
                                   really_down_mons->insert(dbi.second.begin(),
                                                            dbi.second.end());}
                                   }
                                   dout(10) << "We determined CRUSH buckets " <<
                                   *really_down_buckets << " and mons " <<
                                   *really_down_mons << " are really down" <<
                                   dendl; return really_down;}

                                   void OSDMonitor::
                                   trigger_degraded_stretch_mode(const set <
                                                                 int
                                                                 >&dead_buckets,
                                                                 const set <
                                                                 string >
                                                                 &live_zones) {
                                   dout(20) << __func__ << dendl; stretch_recovery_triggered.set_from_double(0);    // reset this; we can't go clean now!
                                   // update the general OSDMap changes
                                   pending_inc.change_stretch_mode = true; pending_inc.stretch_mode_enabled = osdmap.stretch_mode_enabled; pending_inc.new_stretch_bucket_count = osdmap.stretch_bucket_count; int new_site_count = osdmap.stretch_bucket_count - dead_buckets.size(); ceph_assert(new_site_count == 1);    // stretch count 2!
                                   pending_inc.new_degraded_stretch_mode =
                                   new_site_count;
...
const string & remaining_site_name = *(live_zones.begin()); ceph_assert(osdmap.crush->name_exists(remaining_site_name)); int remaining_site = osdmap.crush->get_item_id(remaining_site_name); for (auto pgi:osdmap.pools) {
                                   if (pgi.second.peering_crush_bucket_count) {
                                   pg_pool_t & newp = *pending_inc.get_new_pool(pgi.first, &pgi.second); newp.peering_crush_bucket_count = new_site_count; newp.peering_crush_mandatory_member = remaining_site; newp.min_size = pgi.second.min_size / 2;   // only support 2 zones now
                                   newp.set_last_force_op_resend(pending_inc.
                                                                 epoch);}
                                   }
                                   propose_pending();}

                                   void OSDMonitor::
                                   trigger_recovery_stretch_mode() {
                                   dout(20) << __func__ << dendl; stretch_recovery_triggered.set_from_double(0);    // reset this so we don't go full-active prematurely
pending_inc.change_stretch_mode = true; pending_inc.stretch_mode_enabled = osdmap.stretch_mode_enabled; pending_inc.new_stretch_bucket_count = osdmap.stretch_bucket_count; pending_inc.new_degraded_stretch_mode = osdmap.degraded_stretch_mode; pending_inc.new_recovering_stretch_mode = 1; pending_inc.new_stretch_mode_bucket = osdmap.stretch_mode_bucket; for (auto pgi:osdmap.pools)
                                   {
                                   if (pgi.second.peering_crush_bucket_count) {
                                   pg_pool_t & newp =
                                   *pending_inc.get_new_pool(pgi.first,
                                                             &pgi.second);
                                   newp.set_last_force_op_resend(pending_inc.
                                                                 epoch);}
                                   }
...
OSDMonitor * m; bool force; CMonExitRecovery(OSDMonitor * mon, bool f):m(mon), force(f) {
                                   }
                                   void finish(int r) {
                                   m->try_end_recovery_stretch_mode(force);}
                                   };
                                   void OSDMonitor::
                                   try_end_recovery_stretch_mode(bool force) {
                                   dout(20) << __func__ << dendl;
                                   if (!mon.is_leader())return;
                                   if (!mon.is_degraded_stretch_mode())return;
                                   if (!mon.is_recovering_stretch_mode())return;
                                   if (!is_readable()) {
                                   wait_for_readable_ctx(new
                                                         CMonExitRecovery(this,
                                                                          force));
                                   return;}
                                   ...
                                   if (osdmap.recovering_stretch_mode &&
                                       ((!stretch_recovery_triggered.is_zero()
                                         ...
                                         stretch_recovery_triggered)||force)) {
                                   if (!mon.mgrstatmon()->is_readable()) {
                                   mon.mgrstatmon()->
                                   wait_for_readable_ctx(new
                                                         CMonExitRecovery(this,
                                                                          force));
                                   return;}
                                   const PGMapDigest & pgd =
                                   mon.mgrstatmon()->get_digest();
                                   double misplaced, degraded, inactive,
                                   unknown;
                                   pgd.get_recovery_stats(&misplaced, &degraded,
                                                          &inactive, &unknown);
                                   if (force
                                       || (degraded == 0.0 && inactive == 0.0
                                           && unknown == 0.0)) {
      // we can exit degraded stretch mode!
                                   ...
                                   ceph_assert(is_writeable()); stretch_recovery_triggered.set_from_double(0); pending_inc.change_stretch_mode = true; pending_inc.stretch_mode_enabled = osdmap.stretch_mode_enabled; pending_inc.new_stretch_bucket_count = osdmap.stretch_bucket_count; pending_inc.new_degraded_stretch_mode = 0;   // turn off degraded mode...
                                   pending_inc.new_recovering_stretch_mode = 0; //...and recovering mode!
pending_inc.new_stretch_mode_bucket = osdmap.stretch_mode_bucket; for (auto pgi:osdmap.pools) {
                                   if (pgi.second.peering_crush_bucket_count) {
                                   pg_pool_t & newp =
                                   *pending_inc.get_new_pool(pgi.first,
                                                             &pgi.second);
                                   ...
                                   osdmap.stretch_bucket_count;
                                   newp.peering_crush_mandatory_member =
                                   CRUSH_ITEM_NONE;
                                   newp.min_size =
                                   g_conf().get_val < uint64_t >
                                   ("mon_stretch_pool_min_size");
