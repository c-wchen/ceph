...
int DB::Object::get_object_impl(const DoutPrefixProvider *dpp, DBOpParams &params)
{
    ...
    if (!ret && !params.op.obj.state.exists) {
        ldpp_dout(dpp, 0) << "Object(bucket:" << bucket_info.bucket.name << ", Object:" << obj.key.name << ") doesn't exist" <<
                          dendl;
        ret = -ENOENT;
    }
                           ...
                           RGWObjState *astate, void *arg)
{
    ...
    while (read_ofs < bl.length()) {
        ...
        read_ofs += chunk_len;
        read_len += chunk_len;
        ldpp_dout(dpp, 20) << "dbstore->get_obj_iterate_cb  obj-ofs=" << obj_ofs << " len=" << len <<  " chunk_len = " <<
                           chunk_len << " read_len = " << read_len << dendl;
    }
