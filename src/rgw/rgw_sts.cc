...
int AssumeRoleWithWebIdentityRequest::validate_input(const DoutPrefixProvider *dpp) const
{
    if (! providerId.empty()) {
        if (providerId.length() < MIN_PROVIDER_ID_LEN ||
            ...
            ldpp_dout(dpp, 0) << "ERROR: Either provider id is empty or provider id length is incorrect: " << providerId.length() <<
                              dendl;
            return -EINVAL;
        }
    }
...
int AssumeRoleRequest::validate_input(const DoutPrefixProvider *dpp) const
{
    if (! externalId.empty()) {
        if (externalId.length() < MIN_EXTERNAL_ID_LEN ||
            ...
            ldpp_dout(dpp, 0) << "ERROR: Either external id is empty or external id length is incorrect: " << externalId.length() <<
                              dendl;
            return -EINVAL;
        }
    ...
    if (! serialNumber.empty()) {
        if (serialNumber.size() < MIN_SERIAL_NUMBER_SIZE || serialNumber.size() > MAX_SERIAL_NUMBER_SIZE) {
            ldpp_dout(dpp, 0) << "Either serial number is empty or serial number length is incorrect: " << serialNumber.size() <<
                              dendl;
            return -EINVAL;
        }
        ...
        optional_yield y)
{
    if (auto r_arn = rgw::ARN::parse(arn); r_arn) {
        ...
        if (int ret = role->get(dpp, y); ret < 0) {
            ...
            if (path != r_path) {
                ldpp_dout(dpp, 0) << "Invalid Role ARN: Path in ARN does not match with the role path: " << path << " " << r_path <<
                                  dendl;
