    ...
    class SnapTrimEvent final:public PhasedOperationT < SnapTrimEvent > {
      public:
        ...
        SnapTrimEvent(Ref < PG > pg,
                      ...
                      const snapid_t snapid, const bool needs_pause)
        :pg(std::move(pg)),
            snap_mapper(snap_mapper), snapid(snapid), needs_pause(needs_pause) {
        } void print(std::ostream &) const final;
        void dump_detail(ceph::Formatter * f) const final;
        snap_trim_ertr::future < seastar::stop_iteration > start();
        snap_trim_ertr::future < seastar::stop_iteration >
            with_pg(ShardServices & shard_services, Ref < PG > pg);
      ...
      public:
        PipelineHandle & get_handle() {
            ...
            WaitTrimTimer::BlockingEvent, CompletionEvent > tracking_events;
    };
...
// cannot revisite a pipeline's stage it already saw.
    class SnapTrimObjSubEvent:public PhasedOperationT < SnapTrimObjSubEvent > {
      public:
        ...
        SnapTrimObjSubEvent(Ref < PG > pg,
                            const hobject_t & coid, snapid_t snap_to_trim)
        :pg(std::move(pg)), coid(coid), snap_to_trim(snap_to_trim) {
        } void print(std::ostream &) const final;
        void dump_detail(ceph::Formatter * f) const final;
        remove_or_update_iertr::future <> start();
        remove_or_update_iertr::
            future <> with_pg(ShardServices & shard_services, Ref < PG > pg);
      ...
      private:
        ...
        remove_or_update_iertr::future <> remove_clone(ObjectContextRef obc,
                                                       ObjectContextRef
                                                       head_obc,
                                                       ceph::os::
                                                       Transaction & txn,
                                                       ...
                                                       pg_log_entry_t >
                                                       &log_entries);
        void remove_head_whiteout(ObjectContextRef obc,
                                  ObjectContextRef head_obc,
                                  ceph::os::Transaction & txn,
                                  std::vector < pg_log_entry_t > &log_entries);
        interruptible_future <> adjust_snaps(ObjectContextRef obc,
                                             ObjectContextRef head_obc,
                                             ...
                                             &log_entries);
        void update_head(ObjectContextRef obc, ObjectContextRef head_obc,
                         ceph::os::Transaction & txn,
                         std::vector < pg_log_entry_t > &log_entries);
...
template <> struct fmt::formatter <
    crimson::osd::SnapTrimEvent >:fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::osd::SnapTrimObjSubEvent >:fmt::ostream_formatter {
};
#endif
