...
template class librbd::deep_copy::ObjectCopyRequest <
...
namespace librbd {
    namespace deep_copy {
        ...
        namespace {
            ...
            void scribble(librbd::ImageCtx * image_ctx, int num_ops,
                ...
                for (int i = 0; i < num_ops; i++) {
                    ...
                    int r =
                        api::Io <>::write(*image_ctx, off, len, std::move(bl),
                                          0);
        ...
        }} // anonymous namespace MATCHER(IsListSnaps, "") {
            auto req =
                boost::get < io::ImageDispatchSpec::ListSnaps > (&arg->request);
            return (req != nullptr);
        }

        MATCHER_P2(IsRead, snap_id, image_interval, "") {
            auto req =
                boost::get < io::ImageDispatchSpec::Read > (&arg->request);
            if (req == nullptr
                || arg->io_context->read_snap().value_or(CEPH_NOSNAP) !=
                snap_id) {
                return false;
            }
        ...
        class TestMockDeepCopyObjectCopyRequest:public TestMockFixture {
          public:
            typedef ObjectCopyRequest < librbd::MockTestImageCtx >
            ...
            void SetUp() override {
                ...
                ASSERT_EQ(0,
                          m_src_image_ctx->operations->resize(m_image_size,
                                                              true, no_op));

                librbd::RBD rbd;
                std::string dst_image_name = get_temp_image_name();
                ASSERT_EQ(0,
                          create_image_pp(rbd, m_ioctx, dst_image_name,
                                          m_image_size));
                ASSERT_EQ(0, open_image(dst_image_name, &m_dst_image_ctx));

                m_asio_engine =
                    std::make_shared < librbd::AsioEngine >
            ...
            void expect_get_object_count(librbd::MockImageCtx & mock_image_ctx) {
                EXPECT_CALL(mock_image_ctx, get_object_count(_))
                    ...
                    WillRepeatedly(Invoke
                                   ([&mock_image_ctx] (librados::snap_t snap_id)
                                    {
                                    return mock_image_ctx.image_ctx->
            ...
            void expect_start_op(librbd::
                                 MockExclusiveLock & mock_exclusive_lock) {
                if ((m_src_image_ctx->features & RBD_FEATURE_EXCLUSIVE_LOCK) ==
                    0) {
                    return;
                }
            ...
            void expect_get_object_name(librbd::
                                        MockTestImageCtx & mock_image_ctx) {
                EXPECT_CALL(mock_image_ctx, get_object_name(0))
                    .
                    WillOnce(Return
                             (mock_image_ctx.image_ctx->get_object_name(0)));
            }

            MockObjectCopyRequest *create_request(librbd::
                                                  MockTestImageCtx &
                                                  mock_src_image_ctx,
                                                  librbd::
                                                  MockTestImageCtx &
                                                  ...
                                                  librados::
                                                  snap_t src_snap_id_end,
                                                  librados::
                                                  snap_t dst_snap_id_start,
                                                  uint32_t flags,
                                                  Context * on_finish) {
                SnapMap snap_map;
                util::compute_snap_map(mock_dst_image_ctx.cct,
                ...
                return new MockObjectCopyRequest(&mock_src_image_ctx,
                                                 &mock_dst_image_ctx,
                                                 src_snap_id_start,
                                                 dst_snap_id_start, snap_map, 0,
                                                 flags, nullptr, on_finish);
            }
            ...
            void expect_write(librados::MockTestMemIoCtxImpl & mock_io_ctx,
              ...
              for (auto extent:extents) {
                    expect_write(mock_io_ctx, extent.first, extent.second,
                                 snapc, r);
                    if (r < 0) {
                        break;
                    }
            ...
            void expect_remove(librados::MockTestMemIoCtxImpl & mock_io_ctx,
                               int r) {
            ...
            void expect_update_object_map(librbd::
                                          MockTestImageCtx & mock_image_ctx,
                                          librbd::
                ...
                if (mock_image_ctx.image_ctx->object_map != nullptr) {
                    auto & expect =
                        EXPECT_CALL(mock_object_map,
                                    aio_update(snap_id, 0, 1, state, _, _,
                                               false, _));
                    if (r < 0) {
                        expect.
                            WillOnce(DoAll
                        ...
                        expect.
                            WillOnce(DoAll
                                     (WithArg < 7 >
                                      (Invoke
                                       ([&mock_image_ctx, snap_id,
                                         state] (Context * ctx) {
                                        ceph_assert(ceph_mutex_is_locked
                                                    (mock_image_ctx.image_ctx->
                                                     image_lock));
                                        mock_image_ctx.image_ctx->object_map->
                                        aio_update < Context > (snap_id, 0, 1,
                                                                state,
                                                                boost::none, {
                                                                }
                                                                , false, ctx);}
            ...
            int create_snap(librbd::ImageCtx * image_ctx, const char *snap_name,
                ...
                int r =
                    ...
                    snap_create(cls::rbd::UserSnapshotNamespace(), snap_name, 0,
                                prog_ctx);
                if (r < 0) {
                    return r;
                }
            ...
            std::string get_snap_name(librbd::ImageCtx * image_ctx,
                ...
                auto it = std::find_if(image_ctx->snap_ids.begin(),
                                       ...
                                       [snap_id] (const std::pair < std::pair <
            ...
            int copy_objects() {
                ...
                r = api::Io <>::write(*m_dst_image_ctx, 0, object_size,
                                      std::move(bl), 0);
                if (r < 0) {
                    return r;
                }
            ...
            int compare_objects() {
                ...
                while (!snap_map.empty()) {
                    ...
                    librados::snap_t dst_snap_id =
                        *snap_map.begin()->second.begin();
                    snap_map.erase(snap_map.begin());

                    std::string snap_name =
                        get_snap_name(m_src_image_ctx, src_snap_id);
                    if (snap_name.empty()) {
                        return -ENOENT;
                    }

                    std::
                        cout << "comparing '" << snap_name << " (" <<
                        src_snap_id << " to " << dst_snap_id << ")" << std::
                        endl;
                    ...
                    r = api::Io <>::read(*m_dst_image_ctx, 0, object_size,
                                         librbd::io::ReadResult {
                                         &dst_bl}
                                         , 0);
                    if (r < 0) {
        ...
        TEST_F(TestMockDeepCopyObjectCopyRequest, Write) {
            ...
            librados::
                ...
                mock_dst_io_ctx(get_mock_io_ctx(request->get_dst_io_ctx()));

            InSequence seq;
            expect_list_snaps(mock_src_image_ctx, 0);
            expect_read(mock_src_image_ctx, m_src_snap_ids[0], 0,
                        one.range_end(), 0);
        ...
        TEST_F(TestMockDeepCopyObjectCopyRequest, WriteSnaps) {
            ...
            if (one.range_end() < two.range_end()) {
                ...
                resize_diff.insert(one.range_end(),
                                   two.range_end() - one.range_end());
        ...
        TEST_F(TestMockDeepCopyObjectCopyRequest, Trim) {
            ASSERT_EQ(0,
                      m_src_image_ctx->operations->
                      metadata_set("conf_rbd_skip_partial_discard", "false"));
            ...
            ASSERT_LE(0,
                      api::Io <>::discard(*m_src_image_ctx, trim_offset,
                                          one.range_end() - trim_offset,
                                          m_src_image_ctx->
        ...
        TEST_F(TestMockDeepCopyObjectCopyRequest, Remove) {
            ...
            InSequence seq;
            expect_list_snaps(mock_src_image_ctx, 0);
            expect_read(mock_src_image_ctx, m_src_snap_ids[1], 0,
                        one.range_end(), 0);
        ...
        TEST_F(TestMockDeepCopyObjectCopyRequest, ObjectMapUpdateError) {
            ...
            expect_start_op(mock_exclusive_lock);
            expect_update_object_map(mock_dst_image_ctx, mock_object_map,
                                     m_dst_snap_ids[0], OBJECT_EXISTS,
                                     -EBLOCKLISTED);
        ...
        TEST_F(TestMockDeepCopyObjectCopyRequest, Incremental) {
            ...
            C_SaferCond ctx1;
            auto request1 =
                create_request(mock_src_image_ctx, mock_dst_image_ctx,
            ...
            librados::
                MockTestMemIoCtxImpl &
                mock_dst_io_ctx(get_mock_io_ctx(request1->get_dst_io_ctx()));
            expect_prepare_copyup(mock_dst_image_ctx);
            expect_start_op(mock_exclusive_lock);
            expect_write(mock_dst_io_ctx, 0, one.range_end(), {
                         0, {
