...
class RGWProcess {
  ...
  public:
    RGWProcess(CephContext * const cct,
               ...
               const int num_threads, RGWFrontendConfig * const conf)
    :cct(cct),
        store(pe->store),
...
class RGWLoadGenProcess:public RGWProcess {
  ...
  public:
     RGWLoadGenProcess(CephContext * cct, RGWProcessEnv * pe, int num_threads,
                       RGWFrontendConfig * _conf):RGWProcess(cct, pe,
                                                             num_threads,
                                                             _conf) {
    } void run() override;
    void checkpoint();
...
extern int process_request(RGWRados * store,
                           ...
                           OpsLogSocket * olog, int *http_ret = nullptr);

extern int rgw_process_authenticated(RGWHandler_REST * handler,
                                     RGWOp * &op,
                                     RGWRequest * req,
                                     req_state * s, bool skip_retarget = false);
