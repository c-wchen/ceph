...
} // namespace journal } // namespace librbd namespace rbd {
    namespace mirror {
        ...
        namespace image_replayer {
            ...
            template <>
                ...
                static PrepareRemoteImageRequest *create(Threads <
                                                         librbd::
                                                         MockTestImageCtx >
                                                         *threads,
                                                         librados::IoCtx &,
            ...
            template <> struct EventPreprocessor <librbd::MockTestImageCtx > {
                ...
                MOCK_METHOD2(preprocess,
                             void (librbd::journal::EventEntry *, Context *));
            };
            ...
            BootstrapRequest < librbd::MockTestImageCtx > *BootstrapRequest <
                librbd::MockTestImageCtx >::s_instance = nullptr;
            CloseImageRequest < librbd::MockTestImageCtx > *CloseImageRequest <
                librbd::MockTestImageCtx >::s_instance = nullptr;
            EventPreprocessor < librbd::MockTestImageCtx > *EventPreprocessor <
                librbd::MockTestImageCtx >::s_instance = nullptr;
...
namespace rbd {
    namespace mirror {
        ...
        class TestMockImageReplayer:public TestMockFixture {
          public:
            ...
            typedef librbd::journal::Replay < librbd::MockTestImageCtx >
                MockReplay;
            typedef ImageReplayer < librbd::MockTestImageCtx >
                MockImageReplayer;
            ...
            void expect_add_event_after_repeatedly(MockThreads & mock_threads) {
                EXPECT_CALL(*mock_threads.timer, add_event_after(_, _))
                    ...
                    WillRepeatedly(DoAll
                                   (Invoke
                                    ([this] (double seconds, Context * ctx) {
                                     m_threads->timer->add_event_after(seconds,
                                                                       ctx);}),
                                    ReturnArg < 1 > ()));
            ...
            void expect_wait_for_scheduled_deletion(MockImageDeleter &
                                                    mock_image_deleter,
                                                    const std::
                                                    string & global_image_id,
                                                    int r) {
                EXPECT_CALL(mock_image_deleter,
                            wait_for_scheduled_deletion(_, global_image_id, _,
                                                        false))
            ...
            void expect_cancel_waiter(MockImageDeleter & mock_image_deleter) {
                EXPECT_CALL(mock_image_deleter,
                            cancel_waiter(m_local_io_ctx.get_id(),
                                          "global image id"));
            }

            void expect_schedule_image_delete(MockImageDeleter &
                                              mock_image_deleter,
                                              const std::
                                              string & global_image_id,
                                              bool ignore_orphan) {
                EXPECT_CALL(mock_image_deleter,
                            schedule_image_delete(_, _, global_image_id,
                                                  ignore_orphan));
            }
            ...
            void expect_get_or_send_update(MockReplayStatusFormatter &
                                           mock_replay_status_formatter) {
                EXPECT_CALL(mock_replay_status_formatter,
                            get_or_send_update(_, _))
                    .
                    WillRepeatedly(DoAll
                                   (WithArg < 1 > (CompleteContext(-EEXIST)),
                                    Return(true)));
            }
            ...
            void expect_send(MockPrepareLocalImageRequest & mock_request,
                ...
                EXPECT_CALL(mock_request, send())
                    .
                    WillOnce(Invoke
                             ([&mock_request, local_image_id, local_image_name,
                               tag_owner, r] () {
                              if (r == 0) {
            ...
            void expect_send(MockPrepareRemoteImageRequest & mock_request,
                ...
                EXPECT_CALL(mock_request, send())
                    .
                    WillOnce(Invoke
                             ([&mock_request, image_id, mirror_uuid, r] () {
                              if (r >= 0) {
                              *mock_request.remote_journaler =
            ...
            void expect_send(MockBootstrapRequest & mock_bootstrap_request,
                ...
                EXPECT_CALL(mock_bootstrap_request, send())
                    .
                    WillOnce(Invoke
                             ([&mock_bootstrap_request, &mock_local_image_ctx,
                               do_resync, r] () {
                              if (r == 0) {
                              *mock_bootstrap_request.image_ctx =
            ...
            void expect_shut_down(MockReplay & mock_replay, bool cancel_ops,
                                  int r) {
                EXPECT_CALL(mock_replay, shut_down(cancel_ops, _))
                    .WillOnce(WithArg < 1 > (CompleteContext(r)));
            }

            void expect_shut_down(journal::MockJournaler & mock_journaler,
                                  int r) {
                EXPECT_CALL(mock_journaler, shut_down(_))
                    .WillOnce(CompleteContext(r));
            }

            void expect_send(MockCloseImageRequest & mock_close_image_request,
                             int r) {
                EXPECT_CALL(mock_close_image_request, send())
                    .WillOnce(Invoke([&mock_close_image_request, r] () {
                                     *mock_close_image_request.image_ctx =
                                     nullptr;
                                     mock_close_image_request.on_finish->
                                     complete(r);}));
            }
                             ...
                             (Invoke
                              ([on_finish]
                               (::journal::MockReplayEntryProxy * e,
                                uint64_t * t) {
                               on_finish->complete(0);}), Return(false)));
            }
            ...
            void expect_allocate_tag(librbd::MockTestJournal & mock_journal,
                                     int r) {
            ...
            void expect_preprocess(MockEventPreprocessor &
                                   mock_event_preprocessor, bool required,
                                   int r) {
            ...
            void create_image_replayer(MockThreads & mock_threads,
                ...
                m_image_replayer =
                    new MockImageReplayer(&mock_threads, &mock_image_deleter,
                                          &m_instance_watcher,
                                          rbd::mirror::RadosRef(new librados::
                                                                Rados
                                                                (m_local_io_ctx)),
                                          "local_mirror_uuid",
                                          m_local_io_ctx.get_id(),
                                          "global image id");
        ...
        TEST_F(TestMockImageReplayer, StartStop) {
            ...
            InSequence seq;
            expect_wait_for_scheduled_deletion(mock_image_deleter,
                                               "global image id", 0);
            expect_send(mock_prepare_local_image_request,
                        mock_local_image_ctx.id, mock_local_image_ctx.name,
            ...
            expect_start_external_replay(mock_local_journal, &mock_local_replay,
                                         0);
        ...
        TEST_F(TestMockImageReplayer, LocalImageDNE) {
            ...
            expect_send(mock_prepare_remote_image_request, "remote mirror uuid",
                        m_remote_image_ctx->id, 0);
            EXPECT_CALL(mock_remote_journaler, construct());
            expect_send(mock_bootstrap_request, mock_local_image_ctx, false,
                        -EREMOTEIO);
        ...
        TEST_F(TestMockImageReplayer, GetRemoteImageIdDNE) {
            ...
            expect_send(mock_prepare_remote_image_request, "remote mirror uuid",
                        "", -ENOENT);
            expect_schedule_image_delete(mock_image_deleter, "global image id",
                                         false);
        ...
        TEST_F(TestMockImageReplayer, Replay) {
            ...
            expect_try_pop_front_return_no_entries(mock_remote_journaler,
                                                   &replay_ctx);
        ...
        TEST_F(TestMockImageReplayer, DecodeError) {
            ...
            EXPECT_CALL(mock_close_local_image_request, send())
                .
                WillOnce(Invoke
                         ([&mock_close_local_image_request, &close_ctx] () {
