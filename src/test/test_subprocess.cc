...
{
    SubProcess p("NOTEXISTENTBINARY", SubProcess::CLOSE, SubProcess::CLOSE,
                 SubProcess::PIPE);
    ASSERT_EQ(p.spawn(), 0);
    std::string buf;
...
{
    SubProcess sh("/bin/sh", SubProcess::PIPE, SubProcess::PIPE,
                  SubProcess::PIPE);
    sh.add_cmd_args("-c",
                    "sleep 0; " "cat; " "echo 'error from subshell' >&2; "
...
{
    SubProcessTimed p("true", SubProcess::CLOSE, SubProcess::CLOSE,
                      SubProcess::CLOSE, 10);
    ASSERT_EQ(p.spawn(), 0);
    ASSERT_EQ(p.join(), 0);
    ASSERT_TRUE(p.err().c_str()[0] == '\0');
...
{
    SubProcessTimed sleep("sleep", SubProcess::CLOSE, SubProcess::CLOSE,
                          SubProcess::CLOSE, 0);
    sleep.add_cmd_arg("1");
...
{
    SubProcessTimed sh("/bin/sh", SubProcess::PIPE, SubProcess::PIPE,
                       SubProcess::PIPE, 0);
