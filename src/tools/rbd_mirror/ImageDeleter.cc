...
ImageDeleter < I >::ImageDeleter(librados::IoCtx &local_io_ctx,
                                 Threads < librbd::ImageCtx >
                                 *threads,
                                 Throttler < librbd::ImageCtx >
    ...
    :    m_local_io_ctx(local_io_ctx), m_threads(threads),
         ...
         m_lock(ceph::
                make_mutex(librbd::util::
                           unique_lock_name
                           ("rbd::mirror::ImageDeleter::m_lock", this)))
{
}
                                    ...
                                    Context *on_finish)
{
    ...
    auto req =
        ...
        create(local_io_ctx, global_image_id, resync, work_queue,
               on_finish);
    req->send();
}
...
void ImageDeleter < I >::init(Context *on_finish)
{
    ...
    m_asok_hook =
        new ImageDeleterAdminSocketHook < I > (g_ceph_context,
            m_local_io_ctx.
            get_pool_name(), this);

    m_trash_watcher =
        image_deleter::TrashWatcher < I >::create(m_local_io_ctx,
            m_threads,
            m_trash_listener);
...
template < typename I >
void ImageDeleter <
I >::shut_down_trash_watcher(Context *on_finish)
{
    dout(10) << dendl;
...
void ImageDeleter < I >::cancel_all_deletions(Context *on_finish)
{
                                      ...
                                      -ECANCELED);
    {
        ...
        for (auto &queue : {
                 &m_delete_queue, &m_retry_delete_queue
             }
            ) {
            for (auto &info : *queue) {
...
template < typename I >
void ImageDeleter <
I >::wait_for_deletion(const std::string &image_id,
...
template < typename I >
void ImageDeleter <
I >::complete_active_delete(DeleteInfoRef *delete_info, int r)
{
...
template < typename I >
void ImageDeleter <
I >::enqueue_failed_delete(DeleteInfoRef *delete_info,
                           int error_code, double retry_delay)
{
    dout(20) << "info=" << *delete_info << ", r=" << error_code <<
             dendl;
...
ImageDeleter < I >::find_delete_info(const std::string &image_id)
{
    ...
    for (auto &queue : delete_queues) {
        auto it = std::find_if(queue.begin(), queue.end(),
                               [&delete_info](const DeleteInfoRef &
        ref) {
...
I >::handle_retry_timer()
{
    ...
    while (!m_retry_delete_queue.
           empty()) {
        auto &delete_info =
            m_retry_delete_queue.front();
        if (delete_info->retry_time >
            now) {
            break;
        }
             ...
             print_failure_info)
{
    ...
    open_object_section
    ("delete_info");
    f->dump_string("image_id",
                   image_id);
    if (print_failure_info) {
        f->dump_string("error_code",
                       cpp_strerror
                       (error_code));
        f->dump_int("retries",
                    retries);
