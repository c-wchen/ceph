...
void Server::create_logger()
{
    ...
    plb.add_u64_counter(l_mdss_dispatch_slave_request,
...
recall_throttle(ceph_clock_now(),
                g_conf->get_val < double >("mds_recall_max_decay_rate"))
{
}
...
class C_MDS_session_finish:public ServerLogContext {
  ...
  C_MDS_session_finish(Server * srv, Session * se, uint64_t sseq, bool s, version_t mv, Context * fin_ = NULL):
    ...
    ServerLogContext(srv), session(se), state_seq(sseq), open(s), cmapv(mv),
        inos(i), inotablev(iv), fin(fin_) {
    }
    void finish(int r) override {
        assert(r == 0);
        server->_session_logged(session, state_seq, open, cmapv, inos,
                                inotablev);
        if (fin) {
            fin->complete(r);
        }
...
void Server::handle_client_session(MClientSession * m)
{
    ...
    case CEPH_SESSION_REQUEST_OPEN:
        if (session->is_opening() ||
            ...
            dout(10) <<
                "currently open|opening|stale|killing, dropping this req" <<
                dendl;
        ...
        blacklisted = mds->objecter->with_osdmap([session]
                                                 (const OSDMap & osd_map)->
                                                 bool {
                                                 return osd_map.
                                                 is_blacklisted(session->info.
                                                                inst.addr);}
        );

        {
            ...
            auto log_session_status =[this, m =
                                      m->get(),
                                      session] (boost::string_view status,
            ...
            for (map < string, string >::iterator i =
                 session->info.client_metadata.begin();
            ...
            if (session->info.client_metadata.count("root")) {
                const auto claimed_root =
                    session->info.client_metadata.at("root");
                ...
                if (claimed_root.empty() || claimed_root[0] != '/' ||
                    ...
                    mds->
                        send_message_client(new
                                            MClientSession(CEPH_SESSION_REJECT),
                                            session);
                    mds->clog->
                        warn() << "client session with invalid root '" <<
                        claimed_root << "' denied (" << session->info.
                        inst << ")";
                    session->clear();
            ...
            auto fin =
                new
                FunctionContext([log_session_status =
    ...
    case CEPH_SESSION_REQUEST_RENEWCAPS:
        if (session->is_open() || session->is_stale()) {
            ...
            m->get_connection()->
                send_message(new
                             MClientSession(CEPH_SESSION_RENEWCAPS,
                                            m->get_seq()));
        }
        else {
...
void Server::flush_session(Session * session, MDSGatherBuilder * gather)
{
    ...
    mds->send_message_client(new MClientSession(CEPH_SESSION_FLUSHMSG, seq),
                             session);
}
...
{
    for (set < client_t >::iterator p = client_set.begin();
         p != client_set.end(); ++p) {
        Session *session =
            mds->sessionmap.get_session(entity_name_t::CLIENT(p->v));
        assert(session);
...
void Server::_session_logged(Session * session, uint64_t state_seq, bool open,
                             version_t pv, interval_set < inodeno_t > &inos,
                             version_t piv)
{
    dout(10) << "_session_logged " << session->info.
        inst << " state_seq " << state_seq << " " << (open ? "open" : "close")
        << " " << pv << dendl;

    if (piv) {
        assert(session->is_closing() || session->is_killing() || session->is_opening());    // re-open closing session
    ...
    if (session->get_state_seq() != state_seq) {
        ...
        if (session->is_closing()) {
            ...
            // reset session
            mds->send_message_client(new MClientSession(CEPH_SESSION_CLOSE),
                                     session);
                                              ...
                                              uint64_t > >&smap)
{
    ...
    mds->objecter->with_osdmap([this, &cm] (const OSDMap & osd_map) {
                               for (auto p = cm.begin(); p != cm.end();) {
                               if (osd_map.is_blacklisted(p->second.addr)) {
                               dout(10) << " ignoring blacklisted client." <<
                               p->first << " (" << p->second.
                               addr << ")" << dendl; cm.erase(p++);}
                               else {
                               ++p;}
                               }
                               }
  ...
  for (auto & it:smap) {
        ...
        if (sseq > 0) {
            if (session->get_state_seq() != sseq) {
                ...
                mds->sessionmap.set_state(session, Session::STATE_OPEN);
                mds->sessionmap.touch_session(session);
                mds->send_message_client(new MClientSession(CEPH_SESSION_OPEN),
                                         session);
                if (mdcache->is_readonly())
                    mds->
                        send_message_client(new
...
void Server::find_idle_sessions()
{
    ...
    if (mds->sessionmap.get_sessions().size() == 1
        ...
        dout(20) << "skipping client eviction because there is only one" <<
            dendl;
        return;
    }
  ...
  for (const auto & session:*stale_sessions) {
        auto last_cap_renew_span =
            ...
            double >(now - session->last_cap_renew).count();
        if (session->is_importing()) {
            dout(10) << "stopping at importing session " << session->info.
                inst << dendl;
            break;
        }
        assert(session->is_stale());
        if (last_cap_renew_span < cutoff) {
            dout(20) << "oldest stale session is " << session->info.
                inst << " and recently renewed caps " << last_cap_renew_span <<
                "s ago" << dendl;
            break;
        }
...
void Server::evict_cap_revoke_non_responders()
{
    ...
    mds->locker->get_late_revoking_clients(&to_evict,
                                ...
                                const std::set < std::string > &changed)
{
    if (changed.count("mds_cap_revoke_eviction_timeout")) {
        cap_revoke_eviction_timeout =
            conf->get_val < double >("mds_cap_revoke_eviction_timeout");
...
void Server::kill_session(Session * session, Context * on_safe)
{
    ...
    if ((session->is_opening() ||
        ...
        dout(10) << "kill_session importing or already closing/killing " <<
            session << dendl;
        assert(session->is_closing() || session->is_closed()
               || session->is_killing() || session->is_importing());
        if (on_safe) {
            on_safe->complete(0);
        }
    }
                                   ...
                                   Context * on_safe)
{
    ...
    mdlog->
        start_submit_entry(new
                           ESession(session->info.inst, false, pv, both, piv),
                           new C_MDS_session_finish(this, session, sseq, false,
                                                    pv, both, piv, on_safe));
    mdlog->flush();
...
void Server::handle_client_reconnect(MClientReconnect * m)
{
    ...
    if (!mds->is_reconnect()
        ...
        dout(1) << " session is closed, ignoring reconnect, sending close" <<
            dendl;
        mds->clog->
            info() << "denied reconnect attempt (mds is " <<
            ceph_mds_state_name(mds->get_state())
    ...
    if (deny) {
        m->get_connection()->
            send_message(new MClientSession(CEPH_SESSION_CLOSE));
        m->put();
        return;
    }
    ...
    // snaprealms
    for (vector < ceph_mds_snaprealm_reconnect >::iterator p =
         m->realms.begin(); p != m->realms.end(); ++p) {
        ...
        if (in) {
            ...
            if (in->snaprealm->have_past_parents_open()) {
                dout(15) << "open snaprealm (w/ past parents) on " << *in <<
                    dendl;
                mdcache->finish_snaprealm_reconnect(from, in->snaprealm,
                                                    snapid_t(p->seq));
            }
            else {
                dout(15) << "open snaprealm (w/o past parents) on " << *in <<
                    dendl;
                mdcache->add_reconnected_snaprealm(from, inodeno_t(p->ino),
                                                   snapid_t(p->seq));
            }
        }
        else {
    ...
    for (map < inodeno_t, cap_reconnect_t >::iterator p = m->caps.begin();
        ...
        if (in && in->is_auth()) {
            ...
            recover_filelocks(in, p->second.flockbl,
                              m->get_orig_source().num());
            continue;
        }
                                           ...
                                           RecallFlags flags)
{
    ...
    const auto recall_global_max_decay_threshold =
        g_conf->get_val < uint64_t > ("mds_recall_global_max_decay_threshold");
    const auto recall_max_caps =
        g_conf->get_val < uint64_t > ("mds_recall_max_caps");
    const auto recall_max_decay_threshold =
        g_conf->get_val < uint64_t > ("mds_recall_max_decay_threshold");

    dout(7) << __func__ << ":"
        << " min=" << min_caps_per_client
        << " max=" << max_caps_per_client << " total=" << Capability::count()
        << " flags=0x" << std::hex << flags << dendl;
  ...
  for (const auto p:boost::adaptors::reverse(caps_session)) {
        ...
        if (!session->is_open() ||
            !session->connection.get() || !session->info.inst.name.is_client())
            continue;
...
void Server::force_clients_readonly()
{
    ...
    for (set < Session * >::const_iterator p = sessions.begin();
        ...
        if (!session->info.inst.name.is_client() ||
            ...
            continue;
        mds->send_message_client(new MClientSession(CEPH_SESSION_FORCE_RO),
                                 session);
    }
}
...
void Server::submit_mdlog_entry(LogEvent * le, MDSLogContextBase * fin,
                                MDRequestRef & mdr, const char *event)
{
    if (mdr) {
...
void Server::early_reply(MDRequestRef & mdr, CInode * tracei, CDentry * tracedn)
{
    ...
    if (mdr->has_more() && mdr->more()->has_journaled_slaves) {
        dout(10) << "early_reply - there are journaled slaves, not allowed." <<
            dendl;
        return;
    }
...
void Server::set_trace_dist(Session * session, MClientReply * reply,
                            ...
                            snapid_t snapid,
                            int dentry_wanted, MDRequestRef & mdr)
{
    ...
    if (dn) {
        ...
        dout(20) << "set_trace_dist added dn   " << snapid << " " << *dn <<
            dendl;
    }
    else
...
void Server::handle_client_request(MClientRequest * req)
{
    ...
    if (!req->releases.empty() && req->get_source().is_client()
        ...
        for (vector < MClientRequest::Release >::iterator p =
             req->releases.begin(); p != req->releases.end(); ++p)
            mds->locker->process_request_cap_release(mdr, client, p->item,
                                                     p->dname);
        req->releases.clear();
    }
...
void Server::handle_osd_map()
{
    ...
    mds->objecter->with_osdmap([this] (const OSDMap & o) {
                               auto pi =
                               o.get_pg_pool(mds->mdsmap->get_metadata_pool());
                               is_full = pi
...
{
    dout(4) << "handle_slave_request " << m->get_reqid() << " from " << m->
        get_source() << dendl;
    mds_rank_t from = mds_rank_t(m->get_source().num());

    if (logger)
        logger->inc(l_mdss_handle_slave_request);
    ...
    if (m->get_op() == MMDSSlaveRequest::OP_RENAMENOTIFY) {
        MMDSSlaveRequest *reply =
            new MMDSSlaveRequest(m->get_reqid(), m->get_attempt(),
...
void Server::handle_slave_request_reply(MMDSSlaveRequest * m)
{
    ...
    if (!mds->is_clientreplay() && !mds->is_active() && !mds->is_stopping()) {
        ...
        if (!mdcache->have_uncommitted_master(r, from)) {
            dout(10) <<
                "handle_slave_request_reply ignoring slave reply from mds." <<
                from << " reqid " << r << dendl;
            m->put();
            return;
        }
    ...
    if (m->get_attempt() != mdr->attempt) {
        dout(10) << "handle_slave_request_reply " << *mdr <<
...
{
    dout(7) << "dispatch_slave_request " << *mdr << " " << *mdr->
        slave_request << dendl;
    ...
    if (logger)
        logger->inc(l_mdss_dispatch_slave_request);
    ...
    case MMDSSlaveRequest::OP_WRLOCK:
        {
            // identify object
            SimpleLock *lock =
                mds->locker->get_lock(mdr->slave_request->get_lock_type(),
            ...
            if (op == MMDSSlaveRequest::OP_XLOCK
                ...
                // ack
                MMDSSlaveRequest *r =
                    new MMDSSlaveRequest(mdr->reqid, mdr->attempt, replycode);
...
void Server::handle_slave_auth_pin(MDRequestRef & mdr)
{
    ...
    if (!fail) {
        for (vector < MDSCacheObjectInfo >::iterator p =
             mdr->slave_request->get_authpins().begin();
             p != mdr->slave_request->get_authpins().end(); ++p) {
        ...
        for (list < MDSCacheObject * >::iterator p = objects.begin();
            ...
            if (!mdr->can_auth_pin(*p)) {
                if (mdr->slave_request->is_nonblock()) {
                    dout(10) << " can't auth_pin (freezing?) " << **p <<
                        " nonblocking" << dendl;
                    fail = true;
                    wouldblock = true;
                    break;
                ...
                (*p)->add_waiter(CDir::WAIT_UNFREEZE,
...
void Server::handle_slave_auth_pin_ack(MDRequestRef & mdr,
                                       MMDSSlaveRequest * ack)
{
    ...
    for (vector < MDSCacheObjectInfo >::iterator p =
         ack->get_authpins().begin(); p != ack->get_authpins().end(); ++p) {
        MDSCacheObject *object = mdcache->get_object(*p);
    ...
    // removed auth pins?
    map < MDSCacheObject *, mds_rank_t >::iterator p =
        mdr->remote_auth_pins.begin();
    ...
    if (mdr->more()->waiting_on_slave.empty())
        ...
        dout(10) << "still waiting on slaves " << mdr->more()->
            waiting_on_slave << dendl;
}
...
bool Server::check_fragment_space(MDRequestRef & mdr, CDir * in)
{
    ...
    if (size >= g_conf->mds_bal_fragment_size_max) {
        dout(10) << "fragment " << *in << " size exceeds " << g_conf->
            mds_bal_fragment_size_max << " (ENOSPC)" << dendl;
        respond_to_request(mdr, -ENOSPC);
        return false;
    }
 ...
 */
CDir *Server::validate_dentry_dir(MDRequestRef & mdr, CInode * diri,
 ...
 */
CDentry *Server::prepare_null_dentry(MDRequestRef & mdr, CDir * dir,
                                     boost::string_view dname, bool okexist)
{
    ...
    if (!dir->is_complete() && (!dir->has_bloom() || dir->is_in_bloom(dname))) {
        dout(7) << " incomplete dir contents for " << *dir << ", fetching" <<
            dendl;
    ...
    dn = dir->add_null_dentry(dname,
                              dir->inode->find_snaprealm()->get_newest_seq() +
                              1);
    dn->mark_new();
                                  ...
                                  file_layout_t * layout)
{
    ...
    if (allow_prealloc_inos && mdr->session->info.prealloc_inos.size()) {
        mdr->used_prealloc_ino = in->inode.ino = mdr->session->take_ino(useino);    // prealloc -> used
        ...
        mdr->alloc_ino = in->inode.ino = mds->inotable->project_alloc_id();
    ...
    if (useino && useino != in->inode.ino) {
        ...
        mds->clog->error() << mdr->client_request->get_source()
            ...
            << " but mds." << mds->get_nodeid() << " allocated " << in->inode.
            ino;
    ...
    dout(10) << oct << " dir mode 0" << diri->inode.
    ...
    if (req->get_data().length()) {
      ...
      for (const auto & p:xattrs) {
            ...
            auto em =
                in->xattrs.emplace(std::piecewise_construct,
...
CDir *Server::traverse_to_auth_dir(MDRequestRef & mdr,
                                   vector < CDentry * >&trace, filepath refpath)
{
    ...
    dout(10) << "traverse_to_auth_dir dirpath " << refpath << " dname " << dname
        << dendl;

    // traverse to parent dir
    CInode *diri;
    int r =
        mdcache->path_traverse(mdr, NULL, NULL, refpath, &trace, &diri,
    ...
    if (r < 0) {
        if (r == -ESTALE) {
            ...
            mdcache->find_ino_peers(refpath.get_ino(),
...
CInode *Server::rdlock_path_pin_ref(MDRequestRef & mdr, int n, set < SimpleLock * >&rdlocks, bool want_auth, bool no_want_auth, /* for readdir, who doesn't want auth _even_if_ it's
                                                                                                                                   a snapped dir */
                                    file_layout_t ** layout, bool no_lookup)    // true if we cannot return a null dentry lease
{
    ...
    // traverse
    int r =
        mdcache->path_traverse(mdr, NULL, NULL, refpath, &mdr->dn[n],
                               &mdr->in[n], MDS_TRAVERSE_FORWARD);
    if (r > 0)
        return NULL;            // delayed
    if (r < 0) {                // error
    ...
    if (want_auth) {
        ...
        if (ref->is_frozen() || ref->is_frozen_auth_pin() ||
            ...
            ref->add_waiter(CInode::WAIT_UNFREEZE,
                            new C_MDS_RetryRequest(mdcache, mdr));
                                          ...
                                          file_layout_t ** layout)
{
    ...
    if (layout)
        mds->locker->include_snap_rdlocks_wlayout(rdlocks, dn->get_dir()->inode,
                                                  layout);
    else
...
void Server::handle_client_getattr(MDRequestRef & mdr, bool is_lookup)
{
    ...
    mds->balancer->hit_inode(now, ref, META_POP_IRD, req->get_source().num());
...
void Server::handle_client_open(MDRequestRef & mdr)
{
    ...
    if ((flags & CEPH_O_DIRECTORY) && !cur->inode.is_dir()
        && !cur->inode.is_symlink()) {
    ...
    if (cur->is_file() || cur->is_dir()) {
        if (mdr->snapid == CEPH_NOSNAP) {
            // register new cap
            Capability *cap =
                mds->locker->issue_new_caps(cur, cmode, mdr->session, 0,
                                            req->is_replay());
            if (cap)
...
class C_MDS_openc_finish:public ServerLogContext {
  ...
  public:
    C_MDS_openc_finish(Server * s, MDRequestRef & r, CDentry * d, CInode * ni,
                       snapid_t f):ServerLogContext(s, r), dn(d), newi(ni),
        follows(f) {
    } void finish(int r) override {
        assert(r == 0);
...
void Server::handle_client_openc(MDRequestRef & mdr)
{
    ...
    C_MDS_openc_finish *fin =
        new C_MDS_openc_finish(this, mdr, dn, in, follows);

    if (mdr->client_request->get_connection()->
...
class C_MDS_inode_update_finish:public ServerLogContext {
  ...
  public:
  C_MDS_inode_update_finish(Server * s, MDRequestRef & r, CInode * i, bool sm = false, bool cr = false):
    ServerLogContext(s, r), in(i), truncating_smaller(sm),
        changed_ranges(cr) {
    } void finish(int r) override {
        assert(r == 0);
...
void Server::handle_client_setattr(MDRequestRef & mdr)
{
    ...
    if ((mask & CEPH_SETATTR_UID)
        && (cur->inode.uid != req->head.args.setattr.uid))
        access_mask |= MAY_CHOWN;

    if ((mask & CEPH_SETATTR_GID)
        && (cur->inode.gid != req->head.args.setattr.gid))
    ...
    if (mask & CEPH_SETATTR_SIZE) {
        ...
        mds->locker->calc_new_client_ranges(cur, pi.inode.size, true,
                                            &new_ranges, &max_increased);
        if (pi.inode.client_ranges != new_ranges) {
            dout(10) << " client_ranges " << pi.inode.
                client_ranges << " -> " << new_ranges << dendl;
...
void Server::do_open_truncate(MDRequestRef & mdr, int cmode)
{
    ...
    if (cap && (cmode & CEPH_FILE_MODE_WR)) {
        ...
        pi.inode.client_ranges[client].follows =
            in->find_snaprealm()->get_newest_seq();
        changed_ranges = true;
        cap->mark_clientwriteable();
    }
...
void Server::handle_client_setdirlayout(MDRequestRef & mdr)
{
    ...
    file_layout_t *dir_layout = NULL;
    CInode *cur =
        rdlock_path_pin_ref(mdr, 0, rdlocks, true, false, &dir_layout);
    if (!cur)
        return;
...
int Server::parse_layout_vxattr(string name, string value,
                                const OSDMap & osdmap, file_layout_t * layout,
                                bool validate)
{
    dout(20) << "parse_layout_vxattr name " << name << " value '" << value <<
        "'" << dendl;
    try {
        if (name == "layout") {
        ...
        dout(10) << "bad vxattr value, unable to parse int for " << name <<
            dendl;
        return -EINVAL;
    }
    ...
    try {
        if (name == "quota") {
            ...
            for (map < string, string >::iterator q = m.begin(); q != m.end();
                 ++q) {
                int r =
                    parse_quota_vxattr(string("quota.") + q->first, q->second,
                                       quota);
                if (r < 0)
                    return r;
            }
...
int Server::check_layout_vxattr(MDRequestRef & mdr,
                                string name,
                                string value, file_layout_t * layout)
{
    MClientRequest *req = mdr->client_request;
    epoch_t epoch;
    int r;
    ...
    mds->objecter->with_osdmap([&](const OSDMap & osdmap) {
                               r =
                               parse_layout_vxattr(name, value, osdmap, layout);
    ...
    if (r == -ENOENT) {
        ...
        if (req_epoch > epoch) {
            ...
            Context *fin =
                new C_IO_Wrapper(mds, new C_MDS_RetryRequest(mdcache, mdr));
            ...
            mds->objecter->with_osdmap([&](const OSDMap & osdmap) {
                                       r =
                                       parse_layout_vxattr(name, value, osdmap,
                                                           layout);
                                       epoch = osdmap.get_epoch();});
                               ...
                               set < SimpleLock * >xlocks)
{
    ...
    dout(10) << "handle_set_vxattr " << name << " val " << value.length()
        << " bytes on " << *cur << dendl;
                                  ...
                                  set < SimpleLock * >xlocks)
{
    ...
    if (name == "ceph.dir.layout") {
        ...
        if (cur->is_root()) {
            dout(10) << "can't remove layout policy on the root directory" <<
                dendl;
            respond_to_request(mdr, -EINVAL);
            return;
        }
...
class C_MDS_inode_xattr_update_finish:public ServerLogContext {
  ...
  public:
    ...
    C_MDS_inode_xattr_update_finish(Server * s, MDRequestRef & r,
                                    CInode * i):ServerLogContext(s, r), in(i) {
    } void finish(int r) override {
        assert(r == 0);
...
void Server::handle_client_setxattr(MDRequestRef & mdr)
{
  ...
  for (const auto & p:*pxattrs) {
        if ((flags & CEPH_XATTR_REPLACE)
            && (name.compare(std::string(boost::string_view(p.first))) == 0)) {
            continue;
        }
    ...
    if ((flags & CEPH_XATTR_CREATE)
        && pxattrs->count(mempool::mds_co::string(boost::string_view(name)))) {
    ...
    if ((flags & CEPH_XATTR_REPLACE)
        && !pxattrs->count(mempool::mds_co::string(boost::string_view(name)))) {
        dout(10) << "setxattr '" << name << "' XATTR_REPLACE and ENODATA on " <<
            *cur << dendl;
        respond_to_request(mdr, -ENODATA);
        return;
    }
    ...
    if ((flags & CEPH_XATTR_REMOVE)) {
        ...
        auto em =
            px.emplace(std::piecewise_construct,
                       std::forward_as_tuple(mempool::mds_co::
                                             string(boost::string_view(name))),
...
void Server::handle_client_removexattr(MDRequestRef & mdr)
{
    ...
    if (pxattrs->count(mempool::mds_co::string(boost::string_view(name))) == 0) {
        dout(10) << "removexattr '" << name << "' and ENODATA on " << *cur <<
            dendl;
        respond_to_request(mdr, -ENODATA);
        return;
    }
...
class C_MDS_mknod_finish:public ServerLogContext {
    ...
    CInode *newi;
  public:
    C_MDS_mknod_finish(Server * s, MDRequestRef & r, CDentry * d,
...
void Server::handle_client_mknod(MDRequestRef & mdr)
{
    ...
    file_layout_t *dir_layout = NULL;
    CDentry *dn =
        rdlock_path_xlock_dentry(mdr, 0, rdlocks, wrlocks, xlocks, false, false,
                                 false,
                                 &dir_layout);
    if (!dn)
        return;
    ...
    if (S_ISREG(newi->inode.mode)) {
        ...
        Capability *cap =
            mds->locker->issue_new_caps(newi, cmode, mdr->session, realm,
                                        req->is_replay());
        if (cap) {
            cap->set_wanted(0);
            ...
            newi->inode.client_ranges[client].range.last =
                newi->inode.get_layout_size_increment();
    ...
    dout(10) << "mknod mode " << newi->inode.mode << " rdev " << newi->inode.
        rdev << dendl;
    ...
    journal_and_reply(mdr, newi, dn, le,
                      new C_MDS_mknod_finish(this, mdr, dn, newi));
}
...
void Server::handle_client_mkdir(MDRequestRef & mdr)
{
    ...
    mode |= S_IFDIR;
    CInode *newi =
        prepare_new_inode(mdr, dn->get_dir(), inodeno_t(req->head.ino), mode);
    assert(newi);
...
void Server::handle_client_symlink(MDRequestRef & mdr)
{
    ...
    newi->symlink =
        mempool::mds_co::string(boost::string_view(req->get_path2()));
...
void Server::handle_client_link(MDRequestRef & mdr)
{
    ...
    CDir *dir = dn->get_dir();
    dout(7) << "handle_client_link link " << dn->
...
class C_MDS_link_local_finish:public ServerLogContext {
    ...
    version_t tipv;
  public:
    C_MDS_link_local_finish(Server * s, MDRequestRef & r, CDentry * d,
                            CInode * ti, version_t dnpv_,
                            version_t tipv_):ServerLogContext(s, r), dn(d),
        targeti(ti), dnpv(dnpv_), tipv(tipv_) {
    } void finish(int r) override {
        assert(r == 0);
...
void Server::_link_local(MDRequestRef & mdr, CDentry * dn, CInode * targeti)
{
    ...
    journal_and_reply(mdr, targeti, dn, le,
                      new C_MDS_link_local_finish(this, mdr, dn, targeti, dnpv,
                                                  tipv));
}

void Server::_link_local_finish(MDRequestRef & mdr, CDentry * dn,
                                CInode * targeti, version_t dnpv,
                                version_t tipv)
{
...
class C_MDS_link_remote_finish:public ServerLogContext {
    ...
    version_t dpv;
  public:
    C_MDS_link_remote_finish(Server * s, MDRequestRef & r, bool i, CDentry * d,
                             CInode * ti):ServerLogContext(s, r), inc(i), dn(d),
...
void Server::_link_remote(MDRequestRef & mdr, bool inc, CDentry * dn,
                          CInode * targeti)
{
    dout(10) << "_link_remote " << (inc ? "link " : "unlink ")
        << *dn << " to " << *targeti << dendl;
    ...
    if (mdr->more()->witnessed.count(linkauth) == 0) {
        if (mds->is_cluster_degraded() &&
            ...
            dout(10) << " targeti auth mds." << linkauth << " is not active" <<
                dendl;
            if (mdr->more()->waiting_on_slave.empty())
        ...
        MMDSSlaveRequest *req =
            new MMDSSlaveRequest(mdr->reqid, mdr->attempt, op);
    ...
    if (!mdr->more()->witnessed.empty()) {
        dout(20) << " noting uncommitted_slaves " << mdr->more()->
            witnessed << dendl;
        le->reqid = mdr->reqid;
        le->had_slaves = true;
        mdcache->add_uncommitted_master(mdr->reqid, mdr->ls,
                                        mdr->more()->witnessed);
    }
    ...
    journal_and_reply(mdr, targeti, dn, le,
                      new C_MDS_link_remote_finish(this, mdr, inc, dn,
                                                   targeti));
}

void Server::_link_remote_finish(MDRequestRef & mdr, bool inc,
                                 CDentry * dn, CInode * targeti, version_t dpv)
{
    dout(10) << "_link_remote_finish " << (inc ? "link " : "unlink ")
        << *dn << " to " << *targeti << dendl;
...
class C_MDS_SlaveLinkPrep:public ServerLogContext {
    CInode *targeti;
  public:
    C_MDS_SlaveLinkPrep(Server * s, MDRequestRef & r,
                        CInode * t):ServerLogContext(s, r), targeti(t) {
    } void finish(int r) override {
        assert(r == 0);
...
class C_MDS_SlaveLinkCommit:public ServerContext {
    ...
    CInode *targeti;
  public:
    C_MDS_SlaveLinkCommit(Server * s, MDRequestRef & r,
                          CInode * t):ServerContext(s), mdr(r), targeti(t) {
    } void finish(int r) override {
...
void Server::handle_slave_link_prep(MDRequestRef & mdr)
{
    ...
    CInode *targeti =
        mdcache->get_inode(mdr->slave_request->get_object_info().ino);
    assert(targeti);
    ...
    mdr->ls = mdlog->get_current_segment();
    ESlaveUpdate *le =
        new ESlaveUpdate(mdlog, "slave_link_prep", mdr->reqid,
                         mdr->slave_to_mds,
    ...
    const fnode_t *pf =
        targeti->get_parent_dn()->get_dir()->get_projected_fnode();
    ...
    dout(10) << " projected inode " << pi.inode.ino << " v " << pi.inode.
        version << dendl;

    // commit case
    mdcache->predirty_journal_parents(mdr, &le->commit, dnl->get_inode(), 0,
...
void Server::_commit_slave_link(MDRequestRef & mdr, int r, CInode * targeti)
{
    ...
    if (r == 0) {
        ...
        ESlaveUpdate *le =
            new ESlaveUpdate(mdlog, "slave_link_commit", mdr->reqid,
                             mdr->slave_to_mds,
                             ESlaveUpdate::OP_COMMIT, ESlaveUpdate::LINK);
        mdlog->start_entry(le);
        submit_mdlog_entry(le, new C_MDS_CommittedSlave(this, mdr), mdr,
                           __func__);
        mdlog->flush();
    }
    else {
...
struct C_MDS_LoggedLinkRollback:public ServerLogContext {
    MutationRef mut;
    C_MDS_LoggedLinkRollback(Server * s, MutationRef & m,
                             MDRequestRef & r):ServerLogContext(s, r), mut(m) {
    } void finish(int r) override {
        server->_link_rollback_finish(mut, mdr);
    }
};

void Server::do_link_rollback(bufferlist & rbl, mds_rank_t master,
                              MDRequestRef & mdr)
{
    link_rollback rollback;
    ...
    // journal it
    ESlaveUpdate *le =
        new ESlaveUpdate(mdlog, "slave_link_rollback", rollback.reqid, master,
...
void Server::handle_slave_link_prep_ack(MDRequestRef & mdr,
                                        MMDSSlaveRequest * m)
{
    dout(10) << "handle_slave_link_prep_ack " << *mdr << " " << *m << dendl;
    mds_rank_t from = mds_rank_t(m->get_source().num());
...
void Server::handle_client_unlink(MDRequestRef & mdr)
{
    ...
    CInode *in;
    int r =
        mdcache->path_traverse(mdr, NULL, NULL, refpath, &trace, &in,
                               MDS_TRAVERSE_FORWARD);
    if (r > 0)
        return;
    if (r < 0) {
    ...
    if (in->is_dir()) {
        ...
        if (rmdir) {
            // unlink
            dout(7) << "handle_client_rmdir on non-dir " << *in <<
                ", returning error" << dendl;
            respond_to_request(mdr, -ENOTDIR);
            return;
        }
    ...
    if (in->is_dir() && in->has_subtree_root_dirfrag()) {
        ...
        dout(10) << " witnesses " << witnesses << ", have " << mdr->more()->
            witnessed << dendl;
...
class C_MDS_unlink_local_finish:public ServerLogContext {
  ...
  public:
    C_MDS_unlink_local_finish(Server * s, MDRequestRef & r, CDentry * d,
                              CDentry * sd):ServerLogContext(s, r), dn(d),
        straydn(sd), dnpv(d->get_projected_version()) {
    } void finish(int r) override {
        assert(r == 0);
...
void Server::_unlink_local(MDRequestRef & mdr, CDentry * dn, CDentry * straydn)
{
    ...
    if (dnl->is_primary()) {
        ...
        if (in->snaprealm || follows + 1 > in->get_oldest_snap())
            in->project_past_snaprealm_parent(straydn->get_dir()->inode->
                                              find_snaprealm());
    ...
    if (in->is_dir()) {
        dout(10) << " noting renamed (unlinked) dir ino " << in->
    ...
    journal_and_reply(mdr, 0, dn, le,
                      new C_MDS_unlink_local_finish(this, mdr, dn, straydn));
}
                                  ...
                                  version_t dnpv)
{
    ...
    if (snap_is_new)            //only new if strayin exists
        mdcache->do_realm_invalidate_and_update_notify(strayin,
                                                       CEPH_SNAP_OP_SPLIT,
                                                       true);
...
bool Server::_rmdir_prepare_witness(MDRequestRef & mdr, mds_rank_t who,
                                    vector < CDentry * >&trace,
                                    CDentry * straydn)
{
    if (mds->is_cluster_degraded() &&
...
struct C_MDS_SlaveRmdirPrep:public ServerLogContext {
    CDentry *dn, *straydn;
    C_MDS_SlaveRmdirPrep(Server * s, MDRequestRef & r, CDentry * d,
                         CDentry * st)
...
void Server::handle_slave_rmdir_prep(MDRequestRef & mdr)
{
    dout(10) << "handle_slave_rmdir_prep " << *mdr
        ...
        << " to " << mdr->slave_request->destdnpath << dendl;
    ...
    CInode *in;
    int r =
        mdcache->path_traverse(mdr, NULL, NULL, srcpath, &trace, &in,
    ...
    dout(20) << " rollback is " << mdr->more()->rollback_bl.
        length() << " bytes" << dendl;
    ...
    if (!in->has_subtree_root_dirfrag(mds->get_nodeid())) {
        dout(10) << " no auth subtree in " << *in << ", skipping journal" <<
            dendl;
        dn->get_dir()->unlink_inode(dn);
    ...
    ESlaveUpdate *le =
        new ESlaveUpdate(mdlog, "slave_rmdir", mdr->reqid, mdr->slave_to_mds,
...
void Server::_logged_slave_rmdir(MDRequestRef & mdr, CDentry * dn,
                                 CDentry * straydn)
{
...
void Server::handle_slave_rmdir_prep_ack(MDRequestRef & mdr,
                                         MMDSSlaveRequest * ack)
{
    dout(10) << "handle_slave_rmdir_prep_ack " << *mdr << " " << *ack << dendl;
...
void Server::_commit_slave_rmdir(MDRequestRef & mdr, int r, CDentry * straydn)
{
    ...
    if (r == 0) {
        ...
        if (mdr->more()->slave_update_journaled) {
            // write a commit to the journal
            ESlaveUpdate *le =
                new ESlaveUpdate(mdlog, "slave_rmdir_commit", mdr->reqid,
...
struct C_MDS_LoggedRmdirRollback:public ServerLogContext {
    ...
    CDentry *dn;
    CDentry *straydn;
    C_MDS_LoggedRmdirRollback(Server * s, MDRequestRef & m, metareqid_t mr,
...
void Server::do_rmdir_rollback(bufferlist & rbl, mds_rank_t master,
                               MDRequestRef & mdr)
{
    ...
    ESlaveUpdate *le =
        new ESlaveUpdate(mdlog, "slave_rmdir_rollback", rollback.reqid, master,
...
void Server::_rmdir_rollback_finish(MDRequestRef & mdr, metareqid_t reqid,
                                    CDentry * dn, CDentry * straydn)
{
        ...
        if (dir->is_auth()) {
            if (dir->get_projected_fnode()->fragstat.size()) {
                dout(10) << "dir_is_nonempty_unlocked dirstat has "
                    << dir->get_projected_fnode()->fragstat.
...
class C_MDS_rename_finish:public ServerLogContext {
  ...
  public:
    C_MDS_rename_finish(Server * s, MDRequestRef & r,
                        CDentry * sdn, CDentry * ddn,
                        CDentry * stdn):ServerLogContext(s, r), srcdn(sdn),
        destdn(ddn), straydn(stdn) {
    } void finish(int r) override {
        assert(r == 0);
...
void Server::handle_client_rename(MDRequestRef & mdr)
{
    ...
    CDentry *destdn =
        rdlock_path_xlock_dentry(mdr, 0, rdlocks, wrlocks, xlocks, true, false,
                                 true);
    if (!destdn)
        return;
    ...
    int r =
        mdcache->path_traverse(mdr, NULL, NULL, srcpath, &srctrace, NULL,
    ...
    if (destpath.get_ino() != srcpath.get_ino() && !(req->get_source().is_mds() && MDS_INO_IS_MDSDIR(srcpath.get_ino()))) { // <-- mds 'rename' out of stray dir is ok!
    ...
    if (srcdirauth != mds->get_nodeid()) {
        dout(10) << " will remote_wrlock srcdir scatterlocks on mds." <<
            srcdirauth << dendl;
    ...
    CInode *auth_pin_freeze = !srcdn->is_auth()
        && srcdnl->is_primary()? srci : NULL;
    if (!mds->locker->
        acquire_locks(mdr, rdlocks, wrlocks, xlocks, &remote_wrlocks,
                      auth_pin_freeze))
        return;
    ...
    if (srcdnl->is_primary() && !srcdn->is_auth() && srci->is_dir()) {
        dout(10) <<
            "srci is remote dir, setting stickydirs and opening all frags" <<
            dendl;
        ...
        for (list < frag_t >::iterator p = frags.begin(); p != frags.end(); ++p) {
            ...
            if (!dir) {
                ...
                mdcache->open_remote_dirfrag(srci, *p,
                                             new C_MDS_RetryRequest(mdcache,
                                                                    mdr));
                return;
            }
    ...
    if (!srcdn->is_auth()) {
        ...
        if (srcdnl->is_primary() && !mdr->more()->is_ambiguous_auth) {
            ...
            _rename_prepare_witness(mdr, last, witnesses, srctrace, desttrace,
                                    straydn);
            return;
        }
    }
    ...
    _rename_prepare(mdr, &le->metablob, &le->client_map, srcdn, destdn,
                    straydn);
...
void Server::_rename_finish(MDRequestRef & mdr, CDentry * srcdn,
...
bool Server::_rename_prepare_witness(MDRequestRef & mdr, mds_rank_t who,
...
version_t Server::_rename_prepare_import(MDRequestRef & mdr, CDentry * srcdn,
    ...
    if (empty) {
        for (list < CDir * >::iterator p = ls.begin(); p != ls.end(); ++p) {
            if ((*p)->is_subtree_root()
                && (*p)->get_dir_auth().first == mds->get_nodeid()) {
                dout(10) << " frag " << (*p)->
                    get_frag() << " is auth subtree dirfrag, will force journal"
                    << dendl;
                force_journal = true;
                break;
            }
        ...
        for (list < CDir * >::iterator p = ls.begin(); p != ls.end(); ++p) {
            CDir *dir = *p;
            for (list < CDir * >::iterator q = subtrees.begin();
                 q != subtrees.end(); ++q) {
                if (dir->contains(*q)) {
                    ...
                    dout(20) << " frag " << (*p)->
                        get_frag() << " does not contain " << **q << dendl;
            }
            if (force_journal)
                break;
        }
                             ...
                             CDentry * straydn)
{
    ...
    if (srci->is_dir() && !destdn->is_auth()) {
        if (srci->is_auth()) {
            ...
            dout(10) << " we are exporting srci, will force journal destdn" <<
                dendl;
    ...
    if (linkmerge)
        dout(10) << " merging remote and primary links to the same inode" <<
            dendl;
    if (silent)
        dout(10) <<
    ...
    if (srci->is_dir() && (destdn->is_auth() || force_journal_dest)) {
        ...
        dout(10) << " noting rename target dir " << oldin->
            ino() << " in metablob" << dendl;
    ...
    if (srcdnl->is_remote()) {
        if (!linkmerge) {
            ...
            destdn->push_projected_linkage(srcdnl->get_remote_ino(),
            ...
            if (destdn->is_auth()) {
                ...
                pi.inode.version = mdr->more()->pvmap[destdn] =
                    destdn->pre_dirty(oldin->inode.version);
                spi = &pi.inode;
            }
        }
    }
        ...
        if (destdn->is_auth()) {
            ...
            if (srcdn->is_auth())
                ...
                if (srci->is_dir()) {
                    ...
                    dout(10) << " noting renamed dir open frags " << metablob->
                        renamed_dir_frags << dendl;
                }
            }
    ...
    if (destdn->is_auth() && !destdnl->is_null()) {
        mdcache->predirty_journal_parents(mdr, metablob, oldin,
                                          destdn->get_dir(),
                                          (destdnl->
                                           is_primary()? PREDIRTY_PRIMARY : 0) |
                                          predirty_dir, -1);
        if (destdnl->is_primary()) {
            assert(straydn);
            mdcache->predirty_journal_parents(mdr, metablob, oldin,
                                              straydn->get_dir(),
                                              PREDIRTY_PRIMARY | PREDIRTY_DIR,
                                              1);
        }
    }
    ...
    if (!linkmerge) {
        if (destdnl->is_primary()) {
            ...
            if (destdn->is_auth()) {
                // project snaprealm, too
                if (oldin->snaprealm
                    || dest_realm->get_newest_seq() + 1 >
                    oldin->get_oldest_snap())
                    oldin->project_past_snaprealm_parent(straydn->get_dir()->
                                                         inode->
                                                         find_snaprealm());
            ...
            if (oldin->is_auth()) {
                ...
                metablob->add_primary_dentry(oldin->get_projected_parent_dn(),
                                             oldin, true);
            }
        }
    }
    ...
    if (srcdnl->is_remote()) {
        if (!linkmerge) {
            if (destdn->is_auth() && !destdnl->is_null())
                mdcache->journal_cow_dentry(mdr.get(), metablob, destdn,
                                            CEPH_NOSNAP, 0, destdnl);
            else
                destdn->first = MAX(destdn->first, next_dest_snap);

            if (destdn->is_auth())
                metablob->add_remote_dentry(destdn, true,
            ...
            if (srci->get_projected_parent_dn()->is_auth()) {   // it's remote
                ...
                mdcache->journal_cow_dentry(mdr.get(), metablob,
                                            ...
                                            CEPH_NOSNAP, 0, srcdnl);
                metablob->add_primary_dentry(srci->get_projected_parent_dn(),
                                             srci, true);
            }
        }
        else {
        ...
        if (destdn->is_auth() && src_realm != dest_realm &&
            (srci->snaprealm
             || src_realm->get_newest_seq() + 1 > srci->get_oldest_snap()))
        ...
        if (destdn->is_auth())
            ...
            if (srcdn->is_auth() && srci->is_dir()) {
                ...
                list < CDir * >ls;
                srci->get_dirfrags(ls);
                for (list < CDir * >::iterator p = ls.begin(); p != ls.end();
                     ++p) {
                    CDir *dir = *p;
                    if (dir->is_auth())
    ...
    if (srcdn->is_auth()) {
        ...
        mdcache->journal_cow_dentry(mdr.get(), metablob, srcdn, CEPH_NOSNAP, 0,
                                    srcdnl);
    ...
    if (oldin && oldin->is_dir()) {
        assert(straydn);
        mdcache->project_subtree_rename(oldin, destdn->get_dir(),
                                        straydn->get_dir());
    }
    if (srci->is_dir())
        mdcache->project_subtree_rename(srci, srcdn->get_dir(),
                                        destdn->get_dir());

}

void Server::_rename_apply(MDRequestRef & mdr, CDentry * srcdn,
                           CDentry * destdn, CDentry * straydn)
{
        ...
        if (destdnl->is_primary()) {
            ...
            if (destdn->is_auth()) {
                ...
                if (oldin->snaprealm && !hadrealm)
                    mdcache->do_realm_invalidate_and_update_notify(oldin,
                                                                   CEPH_SNAP_OP_SPLIT);
            }
            else {
    ...
    if (srcdn_was_remote) {
        ...
        if (!srcdn->is_auth() && destdn->is_auth()) {
            ...
            if (mdr->more()->cap_imports.count(destdnl->get_inode())) {
                mdcache->migrator->finish_import_inode_caps(destdnl->
                                                            get_inode(),
                                                            mdr->more()->
                                                            srcdn_auth_mds,
                                                            true,
...
class C_MDS_SlaveRenamePrep:public ServerLogContext {
  ...
  public:
    C_MDS_SlaveRenamePrep(Server * s, MDRequestRef & m, CDentry * sr,
                          CDentry * de, CDentry * st):ServerLogContext(s, m),
        srcdn(sr), destdn(de), straydn(st) {
    } void finish(int r) override {
...
class C_MDS_SlaveRenameCommit:public ServerContext {
    ...
    CDentry *srcdn, *destdn, *straydn;
  public:
    C_MDS_SlaveRenameCommit(Server * s, MDRequestRef & m, CDentry * sr,
                            CDentry * de, CDentry * st):ServerContext(s),
        mdr(m), srcdn(sr), destdn(de), straydn(st) {
    } void finish(int r) override {
...
class C_MDS_SlaveRenameSessionsFlushed:public ServerContext {
    MDRequestRef mdr;
  public:
    C_MDS_SlaveRenameSessionsFlushed(Server * s,
                                     MDRequestRef & r):ServerContext(s),
        mdr(r) {
    } void finish(int r) override {
...
void Server::handle_slave_rename_prep(MDRequestRef & mdr)
{
    ...
    vector < CDentry * >trace;
    int r =
        mdcache->path_traverse(mdr, NULL, NULL, destpath, &trace, NULL,
                               MDS_TRAVERSE_DISCOVERXLOCK);
    if (r > 0)
        return;
    if (r == -ESTALE) {
    ...
    dout(10) << " src " << srcpath << dendl;
    CInode *srci = nullptr;
    r = mdcache->path_traverse(mdr, NULL, NULL, srcpath, &trace, &srci,
                               MDS_TRAVERSE_DISCOVERXLOCK);
    if (r > 0)
        return;
    assert(r == 0);
    ...
    if (!mdr->more()->slave_commit)
        mdr->more()->slave_commit =
            new C_MDS_SlaveRenameCommit(this, mdr, srcdn, destdn, straydn);
    ...
    if (srcdn->is_auth()) {
        ...
        if (srcdnl->is_primary()
            ...
            dout(10) << " freezing srci " << *srcdnl->
                get_inode() << " with allowance " << allowance << dendl;
            ...
            if (!frozen_inode) {
                srcdnl->get_inode()->add_waiter(CInode::WAIT_FROZEN,
                                                new C_MDS_RetryRequest(mdcache,
                                                                       mdr));
                return;
            }
            ...
            for (set < mds_rank_t >::iterator p = srcdnrep.begin();
                 p != srcdnrep.end(); ++p) {
            ...
            mdcache->migrator->get_export_client_set(srcdnl->get_inode(),
                                                     export_client_set);
            ...
            if (gather.has_subs()) {
                ...
                gather.
                    set_finisher(new
                                 C_MDS_SlaveRenameSessionsFlushed(this, mdr));
                gather.activate();
            }
        }
        ...
        for (set < mds_rank_t >::iterator p = srcdnrep.begin();
             p != srcdnrep.end(); ++p) {
            if (*p == mdr->slave_to_mds
                || mdr->slave_request->witnesses.count(*p))
                continue;
            dout(10) <<
    ...
    mdr->ls = mdlog->get_current_segment();
    ESlaveUpdate *le =
        new ESlaveUpdate(mdlog, "slave_rename_prep", mdr->reqid,
                         mdr->slave_to_mds,
    ...
    if (le->commit.empty()) {
        ...
        mdr->more()->slave_update_journaled = true;
        submit_mdlog_entry(le,
                           new C_MDS_SlaveRenamePrep(this, mdr, srcdn, destdn,
                                                     straydn), mdr, __func__);
        mdlog->flush();
    }
}
                                  ...
                                  CDentry * straydn)
{
    ...
    if (srcdn->is_auth() && srcdnl->is_primary()) {
        ...
        if (srcdnl->get_inode()->is_dir()) {
            ...
            for (list < CDir * >::iterator p = bounds.begin();
                 p != bounds.end(); ++p)
        ...
        for (list < CDir * >::iterator p = bounds.begin(); p != bounds.end();
             ++p)
                                  ...
                                  CDentry * straydn)
{
    ...
    if (r == 0) {
        ...
        if (mdr->more()->slave_update_journaled) {
            // write a commit to the journal
            ESlaveUpdate *le =
                new ESlaveUpdate(mdlog, "slave_rename_commit", mdr->reqid,
                                 mdr->slave_to_mds, ESlaveUpdate::OP_COMMIT,
                                 ESlaveUpdate::RENAME);
            mdlog->start_entry(le);
            submit_mdlog_entry(le, new C_MDS_CommittedSlave(this, mdr), mdr,
        ...
        if (mdr->more()->rollback_bl.length()) {
            ...
            if (mdcache->
                is_ambiguous_slave_update(mdr->reqid, mdr->slave_to_mds)) {
                mdcache->remove_ambiguous_slave_update(mdr->reqid,
                                                       mdr->slave_to_mds);
                // rollback but preserve the slave request
                do_rename_rollback(mdr->more()->rollback_bl, mdr->slave_to_mds,
                                   mdr, false);
            ...
            dout(10) <<
                " rollback_bl empty, not rollback back rename (master failed after getting extra witnesses?)"
                << dendl;
            // singleauth
            if (mdr->more()->is_ambiguous_auth) {
                if (srcdn->is_auth())
...
void _rollback_repair_dir(MutationRef & mut, CDir * dir,
                          rename_rollback::drec & r, utime_t ctime, bool isdir,
                          int linkunlink, nest_info_t & rstat)
{
    fnode_t *pf;
...
struct C_MDS_LoggedRenameRollback:public ServerLogContext {
    ...
    C_MDS_LoggedRenameRollback(Server * s, MutationRef & m, MDRequestRef & r,
                               ...
                               CDentry * st, bool f):ServerLogContext(s, r),
        mut(m), srcdn(sd), srcdnpv(pv), destdn(dd), straydn(st), finish_mdr(f) {
    } void finish(int r) override {
...
void Server::do_rename_rollback(bufferlist & rbl, mds_rank_t master,
                                MDRequestRef & mdr, bool finish_mdr)
{
    rename_rollback rollback;
    bufferlist::iterator p = rbl.begin();
    ::decode(rollback, p);
    ...
    if (!srcdir)
        srcdir =
            mdcache->get_dirfrag(rollback.orig_src.dirfrag.ino,
    ...
    if (!destdir)
        destdir =
            mdcache->get_dirfrag(rollback.orig_dest.dirfrag.ino,
    ...
    // journal it
    ESlaveUpdate *le =
        new ESlaveUpdate(mdlog, "slave_rename_rollback", rollback.reqid, master,
    ...
    if (target && target != in && target->authority().first == whoami) {
        ...
        le->commit.add_primary_dentry(target->get_projected_parent_dn(), target,
                                      true);
    }
    ...
    if (in && in->is_dir()
        ...
        if (srcdn->authority().first == whoami) {
            ...
            dout(10) << " noting renamed dir open frags " << le->commit.
                renamed_dir_frags << dendl;
        }
    }
    else if (force_journal_dest) {
        dout(10) << " noting rename target ino " << target->
            ino() << " in metablob" << dendl;
    ...
    if (mdr && !mdr->more()->slave_update_journaled) {
        ...
        mdlog->cancel_entry(le);
        mut->ls = NULL;
        _rename_rollback_finish(mut, mdr, srcdn, srcdnpv, destdn, straydn,
                                finish_mdr);
    }
    else {
        assert(!le->commit.empty());
        ...
        MDSLogContextBase *fin =
            new C_MDS_LoggedRenameRollback(this, mut, mdr, srcdn, srcdnpv,
                                           destdn, straydn, finish_mdr);
        submit_mdlog_entry(le, fin, mdr, __func__);
        mdlog->flush();
    }
...
void Server::_rename_rollback_finish(MutationRef & mut, MDRequestRef & mdr,
...
/* This function DOES put the passed message before returning*/
void Server::handle_slave_rename_prep_ack(MDRequestRef & mdr,
                                          MMDSSlaveRequest * ack)
{
    ...
    if (ack->is_interrupted()) {
        ...
        dout(10) << " extra witnesses (srcdn replicas) are " << ack->
            witnesses << dendl;
...
void Server::handle_slave_rename_notify_ack(MDRequestRef & mdr,
                                            MMDSSlaveRequest * ack)
{
...
void Server::handle_client_lssnap(MDRequestRef & mdr)
{
    ...
    for (; p != infomap.end() && num < max_entries; ++p) {
        ...
        unsigned start_len = dnbl.length();
        if (int
            (start_len + snap_name.length() + sizeof(__u32) +
...
struct C_MDS_mksnap_finish:public ServerLogContext {
    CInode *diri;
    SnapInfo info;
    C_MDS_mksnap_finish(Server * s, MDRequestRef & r, CInode * di,
                        SnapInfo & i):ServerLogContext(s, r), diri(di),
        info(i) {
    } void finish(int r) override {
...
void Server::handle_client_mksnap(MDRequestRef & mdr)
{
    ...
    if (mdr->client_request->get_caller_uid() < g_conf->mds_snap_min_uid
        ...
        dout(20) << "mksnap " << snapname << " on " << *diri <<
            " denied to uid " << mdr->client_request->get_caller_uid() << dendl;
        respond_to_request(mdr, -EPERM);
        return;
    }
    ...
    mdcache->predirty_journal_parents(mdr, &le->metablob, diri, 0,
...
struct C_MDS_rmsnap_finish:public ServerLogContext {
    CInode *diri;
    snapid_t snapid;
    C_MDS_rmsnap_finish(Server * s, MDRequestRef & r, CInode * di,
                        snapid_t sn):ServerLogContext(s, r), diri(di),
        snapid(sn) {
    } void finish(int r) override {
...
void Server::handle_client_rmsnap(MDRequestRef & mdr)
{
    ...
    if (mdr->client_request->get_caller_uid() < g_conf->mds_snap_min_uid
        || mdr->client_request->get_caller_uid() > g_conf->mds_snap_max_uid) {
        dout(20) << "rmsnap " << snapname << " on " << *diri <<
            " denied to uid " << mdr->client_request->get_caller_uid() << dendl;
        respond_to_request(mdr, -EPERM);
        return;
    }
...
struct C_MDS_renamesnap_finish:public ServerLogContext {
    CInode *diri;
    snapid_t snapid;
    C_MDS_renamesnap_finish(Server * s, MDRequestRef & r, CInode * di,
                            snapid_t sn):ServerLogContext(s, r), diri(di),
        snapid(sn) {
    } void finish(int r) override {
...
void Server::_renamesnap_finish(MDRequestRef & mdr, CInode * diri,
                                snapid_t snapid)
{
 ...
 */
bool Server::waiting_for_reconnect(client_t c) const const
{
    return client_reconnect_gather.count(c) > 0;
}

void Server::dump_reconnect_status(Formatter * f) const const
{
