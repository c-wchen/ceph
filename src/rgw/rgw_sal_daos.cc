...
namespace rgw::sal {
    ...
    int DaosUser::list_buckets(const DoutPrefixProvider * dpp,
                               const string & marker, const string & end_marker,
                               uint64_t max, bool need_stats,
                               BucketList & buckets, optional_yield y) {
        ldpp_dout(dpp,
                  20) << "DEBUG: list_user_buckets: marker=" << marker <<
        ...
        vector < vector < uint8_t >> values(bcount,
                                            vector < uint8_t >
                                            (DS3_MAX_ENCODED_LEN));
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: ds3_bucket_list failed!" << ret << dendl;
            return ret;
        }
      ...
      for (const auto & bi:bucket_infos) {
            ...
            bufferlist bl;
            bl.append(reinterpret_cast < char *>(bi.encoded),
                      bi.encoded_length);
            auto iter = bl.cbegin();
            dbinfo.decode(iter);
            buckets.add(std::make_unique < DaosBucket >
                        (this->store, dbinfo.info, this));
        }
    ...
    int DaosUser::create_bucket(const DoutPrefixProvider * dpp,
        ...
        if (!*existed) {
            ...
            // Create a new bucket:
            DaosBucket *daos_bucket =
                static_cast < DaosBucket * >(bucket.get());
            bufferlist bl;
            std::unique_ptr < struct ds3_bucket_info >bucket_info =
                daos_bucket->get_encoded_info(bl, ceph::real_time());
            ret =
                ds3_bucket_create(bucket->get_name().c_str(), bucket_info.get(),
                                  nullptr, store->ds3, nullptr);
            if (ret != 0) {
                ldpp_dout(dpp,
                          0) << "ERROR: ds3_bucket_create failed! ret=" << ret
                    << dendl;
                return ret;
            }
        }
...
/* stats - Not for first pass */
    ...
    int DaosUser::read_usage(const DoutPrefixProvider * dpp,
    ...
    int DaosUser::trim_usage(const DoutPrefixProvider * dpp,
                             uint64_t start_epoch, uint64_t end_epoch) {
        return DAOS_NOT_IMPLEMENTED_LOG(dpp);
    }
    ...
    int DaosUser::load_user(const DoutPrefixProvider * dpp, optional_yield y) {
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: load_user failed, name=" << name << dendl;
            return ret;
        }
    ...
    int DaosUser::merge_and_store_attrs(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  20) << "DEBUG: merge_and_store_attrs, new_attrs=" << new_attrs
            << dendl;
    ...
    int DaosUser::store_user(const DoutPrefixProvider * dpp, optional_yield y,
        ...
        const string name = info.user_id.to_str();
        ldpp_dout(dpp,
                  10) << "DEBUG: Store_user(): User name=" << name << dendl;
        ...
        if (ret == 0 && obj_ver.ver) {
          ...
          for (auto const &[id, key]:duinfo.info.access_keys) {
            ...
            old_user_info.reset(new ds3_user_info {
                                .name =
                                duinfo.info.user_id.to_str().c_str(),.email =
                                duinfo.info.user_email.c_str(),.access_ids =
                                old_access_ids.data(),.access_ids_nr =
                                old_access_ids.size()});
        }
        else {
            obj_ver.ver = 1;
    ...
    int DaosUser::read_user(const DoutPrefixProvider * dpp, std::string name,
        ...
        struct ds3_user_info user_info = {.encoded =
                bl.append_hole(size).c_str(),
            .encoded_length = size
        };
    ...
    std::unique_ptr <
        struct ds3_user_info >DaosUser::get_encoded_info(bufferlist & bl,
                                                         obj_version & obj_ver)
    {
        // Encode user data
        struct DaosUserInfo duinfo;
    ...
    int DaosUser::remove_user(const DoutPrefixProvider * dpp, optional_yield y) {
        ...
        // Remove user
        int ret =
            ds3_user_remove(name.c_str(), user_info.get(), store->ds3, nullptr);
        if (ret != 0) {
            ldpp_dout(dpp, 0) << "Error: ds3_user_set failed, name=" << name
                << " ret=" << ret << dendl;
        }
    ...
    int DaosBucket::open(const DoutPrefixProvider * dpp) {
        ...
        int ret =
            ds3_bucket_open(get_name().c_str(), &ds3b, store->ds3, nullptr);
    ...
    std::unique_ptr <
        struct ds3_bucket_info >DaosBucket::get_encoded_info(bufferlist & bl,
                                                             ceph::
                                                             real_time _mtime) {
        DaosBucketInfo dbinfo;
        dbinfo.info = info;
        ...
        std::strncpy(bucket_info->name, get_name().c_str(),
                     sizeof(bucket_info->name));
        return bucket_info;
    }
    ...
    int DaosBucket::remove_bucket(const DoutPrefixProvider * dpp,
        ...
        return ds3_bucket_destroy(get_name().c_str(), delete_children,
                                  store->ds3, nullptr);
    }
    ...
    int DaosBucket::remove_bucket_bypass_gc(int concurrent_max,
        ...
        ldpp_dout(dpp, 20) << "DEBUG: remove_bucket_bypass_gc, concurrent_max="
            << concurrent_max
            << " keep_index_consistent=" << keep_index_consistent << dendl;
        return ds3_bucket_destroy(get_name().c_str(), true, store->ds3,
                                  nullptr);
    }
    ...
    int DaosBucket::put_info(const DoutPrefixProvider * dpp, bool exclusive,
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: ds3_bucket_set_info failed: " << ret <<
                dendl;
        }
        return ret;
    }

    int DaosBucket::load_bucket(const DoutPrefixProvider * dpp,
        ...
        DaosBucketInfo dbinfo;
        uint64_t size = DS3_MAX_ENCODED_LEN;
        struct ds3_bucket_info bucket_info = {.encoded =
                bl.append_hole(size).c_str(),
            .encoded_length = size
        };
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: ds3_bucket_get_info failed: " << ret <<
                dendl;
            return ret;
        }
...
/* stats - Not for first pass */
    ...
    int DaosBucket::read_stats_async(const DoutPrefixProvider * dpp,
                                     const bucket_index_layout_generation &
                                     idx_layout, int shard_id,
...
/* Make sure to call load_bucket() if you need it first */
    ...
    int DaosBucket::check_empty(const DoutPrefixProvider * dpp,
                                optional_yield y) {
...
/* XXX: usage and stats not supported in the first pass */
    int DaosBucket::read_usage(const DoutPrefixProvider * dpp,
                               uint64_t start_epoch, uint64_t end_epoch,
                               uint32_t max_entries, bool * is_truncated,
                               RGWUsageIter & usage_iter, map < rgw_user_bucket,
                               rgw_usage_log_entry > &usage) {
    ...
    int DaosBucket::trim_usage(const DoutPrefixProvider * dpp,
                               uint64_t start_epoch, uint64_t end_epoch) {
        return DAOS_NOT_IMPLEMENTED_LOG(dpp);
    }

    int DaosBucket::remove_objs_from_index(const DoutPrefixProvider * dpp,
    ...
    int DaosBucket::check_index(const DoutPrefixProvider * dpp,
                                std::map < RGWObjCategory,
    ...
    int DaosBucket::list(const DoutPrefixProvider * dpp, ListParams & params,
                         int max, ListResults & results, optional_yield y) {
        ldpp_dout(dpp,
                  20) << "DEBUG: list bucket=" << get_name() << " max=" << max
            << " params=" << params << dendl;
        // End
        if (max == 0) {
            return 0;
        ...
        vector < vector < uint8_t >> values(nobj,
                                            vector < uint8_t >
                                            (DS3_MAX_ENCODED_LEN));
      ...
      for (auto const &obj:object_infos) {
            ...
            bl.append(reinterpret_cast < char *>(obj.encoded),
                      obj.encoded_length);
            auto iter = bl.cbegin();
            ent.decode(iter);
    ...
    int DaosBucket::list_multiparts(const DoutPrefixProvider * dpp,
                                    const string & prefix, string & marker,
                                    const string & delim,
        ...
        vector <
            ...
            >multipart_upload_infos(max_uploads);
        uint32_t nmp = multipart_upload_infos.size();
        vector < vector < uint8_t >> values(nmp,
                                            vector < uint8_t >
                                            (DS3_MAX_ENCODED_LEN));
        ...
        int ret =
            ds3_bucket_list_multipart(get_name().c_str(), &nmp,
                                      multipart_upload_infos.data(), &ncp,
                                      cps.data(),
                                      prefix.c_str(), delim.c_str(),
                                      daos_marker, is_truncated, store->ds3);
      ...
      for (auto const &mp:multipart_upload_infos) {
            ...
            bufferlist bl;
            rgw_bucket_dir_entry ent;
            bl.append(reinterpret_cast < char *>(mp.encoded),
                      mp.encoded_length);
            auto iter = bl.cbegin();
            ent.decode(iter);
            ...
            uploads.push_back(this->
                              get_multipart_upload(name, mp.upload_id,
                                                   std::move(owner),
                                                   ent.meta.mtime));
        }
    ...
    void DaosStore::finalize(void) {
        ...
        if (ret != 0) {
            ldout(cctx,
                  0) << "ERROR: ds3_disconnect() failed: " << ret << dendl;
        }
        ds3 = nullptr;
    ...
    int DaosStore::initialize(CephContext * cct, const DoutPrefixProvider * dpp) {
        ...
        // cct somehow?
        const auto & daos_pool =
            cct->_conf.get_val < std::string > ("daos_pool");
    ...
    } int DaosZoneGroup::get_placement_target_names(std::set < std::string >
                                                    &names) const {
    ...
    int DaosObject::set_obj_attrs(const DoutPrefixProvider * dpp,
    ...
    int DaosObject::get_obj_attrs(optional_yield y,
                                  const DoutPrefixProvider * dpp,
                                  rgw_obj * target_obj) {
    ...
    int DaosObject::modify_obj_attrs(const char *attr_name,
...
// Taken from rgw_rados.cc
    ...
    int DaosObject::chown(User & new_user, const DoutPrefixProvider * dpp,
                          optional_yield y) {
        return 0;
    }

    std::unique_ptr < MPSerializer >
        DaosObject::get_serializer(const DoutPrefixProvider * dpp,
                                   const std::string & lock_name) {
        return std::make_unique < MPDaosSerializer > (dpp, store, this,
                                                      lock_name);
    }
    ...
    int DaosObject::transition_to_cloud(Bucket * bucket,
    ...
    int DaosObject::dump_obj_layout(const DoutPrefixProvider * dpp,
  ...
  DaosObject::DaosReadOp::DaosReadOp(DaosObject * _source):source(_source) {
    ...
    int DaosObject::DaosReadOp::read(int64_t off, int64_t end, bufferlist & bl,
                                     ...
                                     const DoutPrefixProvider * dpp) {
        ldpp_dout(dpp,
                  20) << __func__ << ": off=" << off << " end=" << end << dendl;
...
// a block of data each time and call 'cb' for post-processing.
    int DaosObject::DaosReadOp::iterate(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  20) << __func__ << ": call cb to process data, actual=" <<
            size << dendl;
...
// 4. Fail when file doesn't exist
    int DaosObject::DaosDeleteOp::delete_obj(const DoutPrefixProvider * dpp,
        ...
        ldpp_dout(dpp,
                  20) << "DEBUG: ds3_obj_destroy key=" << key << " ret=" << ret
            << dendl;
    ...
    int DaosObject::copy_object(User * user, req_info * info,
                                ...
                                std::string * version_id, std::string * tag,
                                std::string * etag, void (*progress_cb) (off_t,
                                                                         void
                                                                         *),
                                void *progress_data,
                                const DoutPrefixProvider * dpp,
    ...
    int DaosObject::lookup(const DoutPrefixProvider * dpp) {
        ...
        ret =
            ds3_obj_open(get_key().get_oid().c_str(), &ds3o, daos_bucket->ds3b);

        if (ret == -ENOENT) {
            ldpp_dout(dpp,
                      20) << "DEBUG: daos object (" << get_bucket()->get_name()
                << ", " << get_key().get_oid()
    ...
    int DaosObject::create(const DoutPrefixProvider * dpp) {
        ...
        ret =
            ds3_obj_create(get_key().get_oid().c_str(), &ds3o,
                           daos_bucket->ds3b);
    ...
    int DaosObject::write(const DoutPrefixProvider * dpp, bufferlist && data,
        ...
        int ret =
            ds3_obj_write(data.c_str(), offset, &size, get_daos_bucket()->ds3b,
                          ds3o, nullptr);
        if (ret != 0) {
...
// Get the object's dirent and attrs
    int DaosObject::get_dir_entry_attrs(const DoutPrefixProvider * dpp,
        ...
        if (get_key().ns == RGW_OBJ_NS_MULTIPART) {
            ...
            ret =
                ds3_obj_get_info(object_info.get(), get_daos_bucket()->ds3b,
                                 ds3o);
...
// Set the object's dirent and attrs
    int DaosObject::set_dir_entry_attrs(const DoutPrefixProvider * dpp,
        ...
        ret =
            ds3_obj_set_info(object_info.get(), get_daos_bucket()->ds3b, ds3o);
        if (ret != 0) {
    ...
    int DaosObject::mark_as_latest(const DoutPrefixProvider * dpp,
        ...
        std::unique_ptr < DaosObject > latest_object =
            std::make_unique < DaosObject > (store,
                                             rgw_obj_key(get_name(),
                                                         DS3_LATEST_INSTANCE),
                                             get_bucket());
        ...
        if (ret == 0) {
            ...
            Attrs latest_attrs;
            ret =
                latest_object->get_dir_entry_attrs(dpp, &latest_ent,
                                                   &latest_attrs);
            if (ret != 0) {
                return ret;
            }
            ...
            ret =
                latest_object->set_dir_entry_attrs(dpp, &latest_ent,
                                                   &latest_attrs);
            if (ret != 0) {
                return ret;
            }
        ...
        // version.
        ret =
            ds3_obj_mark_latest(get_key().get_oid().c_str(),
                                get_daos_bucket()->ds3b);
  ...
  :    StoreWriter(dpp, y), store(_store), owner(_owner),
        ptail_placement_rule(_ptail_placement_rule), olh_epoch(_olh_epoch),
        unique_tag(_unique_tag), obj(_store, obj->get_key(),
                                     obj->get_bucket()) {
    }
...
// XXX: Do we need to accumulate writes as motr does?
    ...
    int DaosAtomicWriter::complete(size_t accounted_size,
    ...
    int DaosMultipartUpload::abort(const DoutPrefixProvider * dpp,
        ...
        return ds3_upload_remove(bucket->get_name().c_str(),
    ...
    std::unique_ptr < rgw::sal::Object > DaosMultipartUpload::get_meta_obj() {
        return bucket->
            get_object(rgw_obj_key
                       (get_upload_id(), string(), RGW_OBJ_NS_MULTIPART));
    }

    int DaosMultipartUpload::init(const DoutPrefixProvider * dpp,
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to create multipart upload dir (" <<
    ...
    int DaosMultipartUpload::list_parts(const DoutPrefixProvider * dpp,
        ...
        // Init needed structures
        vector <
            struct ds3_multipart_part_info >multipart_part_infos(num_parts);
        uint32_t npart = multipart_part_infos.size();
        vector < vector < uint8_t >> values(npart,
                                            vector < uint8_t >
                                            (DS3_MAX_ENCODED_LEN));
        ...
        int ret =
            ds3_upload_list_parts(bucket->get_name().c_str(),
                                  get_upload_id().c_str(), &npart,
                                  multipart_part_infos.data(), &daos_marker,
                                  truncated, store->ds3);
      ...
      for (auto const &pi:multipart_part_infos) {
            bufferlist bl;
            bl.append(reinterpret_cast < char *>(pi.encoded),
                      pi.encoded_length);
...
// Heavily copied from rgw_sal_rados.cc
    int DaosMultipartUpload::complete(const DoutPrefixProvider * dpp,
        ...
        do {
            ldpp_dout(dpp,
                      20) << "DaosMultipartUpload::complete(): list_parts()" <<
                dendl;
            ...
            ldpp_dout(dpp,
                      20) << "DaosMultipartUpload::complete(): parts.size()=" <<
                parts.size() << dendl;
            ...
            for (auto obj_iter = parts.begin();
                ...
                DaosMultipartPart *part =
                    dynamic_cast <
                    rgw::sal::DaosMultipartPart * >(obj_iter->second.get());
                uint64_t part_size = part->get_size();
                ldpp_dout(dpp,
                          20) << "DaosMultipartUpload::complete(): part_size="
                    << part_size << dendl;
                ...
                ldpp_dout(dpp,
                          20) << "DaosMultipartUpload::complete(): calc etag "
                    << dendl;
                ...
                ldpp_dout(dpp,
                          20) <<
                    "DaosMultipartUpload::complete(): part compression" <<
                    dendl;
                if (part_compressed) {
                ...
                ldpp_dout(dpp,
                          20) << "DaosMultipartUpload::complete(): off=" << off
      ...
      for (auto const &[part_num, part]:get_parts()) {
            ds3_part_t *ds3p;
            ret =
                ds3_part_open(get_bucket_name().c_str(),
                              get_upload_id().c_str(), part_num, false, &ds3p,
                              store->ds3);
            if (ret != 0) {
                return ret;
            }
            ...
            ldpp_dout(dpp,
                      20) << "DaosMultipartUpload::complete(): part " <<
                part_num << " size is " << size << dendl;
        ...
        // Remove upload from bucket multipart index
        ret =
            ds3_upload_remove(get_bucket_name().c_str(),
                              get_upload_id().c_str(), store->ds3);
        return ret;
    }
    ...
    std::unique_ptr < Writer >
        DaosMultipartUpload::get_writer(const DoutPrefixProvider * dpp,
                                        optional_yield y,
                                        rgw::sal::Object * obj,
        ...
        ldpp_dout(dpp,
                  20) << "DaosMultipartUpload::get_writer(): enter part=" <<
            part_num << " head_obj=" << _head_obj << dendl;
        return std::make_unique < DaosMultipartWriter > (dpp, y, this, obj,
                                                         store, owner,
    ...
    int DaosMultipartWriter::process(bufferlist && data, uint64_t offset) {
        ...
        uint64_t size = data.length();
        int ret =
            ds3_part_write(data.c_str(), offset, &size, ds3p, store->ds3,
                           nullptr);
        if (ret == 0) {
            // XXX: Combine multiple streams into one as motr does
    ...
    int DaosMultipartWriter::complete(size_t accounted_size,
                                      const std::string & etag,
                                      ceph::real_time * mtime,
                                      ceph::real_time set_mtime,
                                      std::map < std::string,
        ...
        ldpp_dout(dpp,
                  20) << "DaosMultipartWriter::complete(): enter part=" <<
            part_num_str << dendl;
        ...
        ldpp_dout(dpp,
                  20) << "DaosMultipartWriter::complete(): compression ret=" <<
            ret << dendl;
        if (ret != 0) {
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "ERROR: failed to set part info (" <<
                get_bucket_name()
                << ", " << upload_id << ", " << part_num << "): ret=" << ret <<
                dendl;
            if (ret == ENOENT) {
                ret = -ERR_NO_SUCH_UPLOAD;
            }
    ...
    std::unique_ptr < MultipartUpload >
        DaosBucket::get_multipart_upload(const std::string & oid,
                                         std::optional < std::string >
                                         upload_id, ACLOwner owner,
                                         ceph::real_time mtime) {
        return std::make_unique < DaosMultipartUpload > (store, this, oid,
                                                         upload_id, owner,
                                                         mtime);
    }

    std::unique_ptr < Writer >
        DaosStore::get_append_writer(const DoutPrefixProvider * dpp,
                                     optional_yield y, rgw::sal::Object * obj,
                                     const rgw_user & owner,
                                     const rgw_placement_rule *
                                     ptail_placement_rule,
    ...
    std::unique_ptr < Writer >
        DaosStore::get_atomic_writer(const DoutPrefixProvider * dpp,
                                     optional_yield y, rgw::sal::Object * obj,
                                     const rgw_user & owner,
                                     const rgw_placement_rule *
    ...
    int DaosStore::get_user_by_access_key(const DoutPrefixProvider * dpp,
        ...
        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "Error: ds3_user_get_by_key failed, key=" << key <<
                " ret=" << ret << dendl;
            return ret;
        }
    ...
    int DaosStore::get_user_by_email(const DoutPrefixProvider * dpp,
        ...
        int ret =
            ds3_user_get_by_email(email.c_str(), &user_info, ds3, nullptr);

        if (ret != 0) {
            ldpp_dout(dpp,
                      0) << "Error: ds3_user_get_by_email failed, email=" <<
                email << " ret=" << ret << dendl;
            return ret;
        }
    ...
    std::unique_ptr < Notification >
        DaosStore::get_notification(rgw::sal::Object * obj,
    ...
    std::unique_ptr < Notification >
        DaosStore::get_notification(const DoutPrefixProvider * dpp,
    ...
    RGWDataSyncStatusManager *DaosStore::
        get_data_sync_manager(const rgw_zone_id & source_zone) {
        DAOS_NOT_IMPLEMENTED_LOG(nullptr);
        return 0;
    }

    int DaosStore::read_all_usage(const DoutPrefixProvider * dpp,
                                  uint64_t start_epoch, uint64_t end_epoch,
                                  uint32_t max_entries, bool * is_truncated,
    ...
    int DaosStore::meta_list_keys_next(const DoutPrefixProvider * dpp,
                                       void *handle, int max,
                                       list < string > &keys,
                                       bool * truncated) {
        return DAOS_NOT_IMPLEMENTED_LOG(dpp);
    ...
    int DaosStore::meta_remove(const DoutPrefixProvider * dpp,
