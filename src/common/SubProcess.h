...
inline SubProcess::SubProcess(const char *cmd_, std_fd_op stdin_op_,
                              std_fd_op stdout_op_,
                              std_fd_op stderr_op_):cmd(cmd_), cmd_args(),
stdin_op(stdin_op_), stdout_op(stdout_op_), stderr_op(stderr_op_),
...
inline int SubProcess::get_stdin() const const
{
    assert(is_spawned());
...
inline int SubProcess::get_stdout() const const
{
    assert(is_spawned());
...
inline int SubProcess::get_stderr() const const
{
    assert(is_spawned());
...
inline void SubProcess::kill(int signo) const const
{
    assert(is_spawned());
...
inline const std::string SubProcess::err() const const
{
    return errstr.str();
}
...
inline SubProcessTimed::SubProcessTimed(const char *cmd, std_fd_op stdin_op,
                                        std_fd_op stdout_op,
                                        std_fd_op stderr_op, int timeout_,
                                        int sigkill_):SubProcess(cmd, stdin_op,
                                                                 stdout_op,
                                                                 stderr_op),
