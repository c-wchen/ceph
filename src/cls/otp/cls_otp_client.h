...
{
public:
    static void create(librados::ObjectWriteOperation *op,
                       const otp_info_t &config);
    static void set(librados::ObjectWriteOperation *op,
                    const std::list < otp_info_t > &entries);
    static void remove(librados::ObjectWriteOperation *op,
                       const std::string &id);
...
// rgw_rados_operate() should be called after the overloads w/o calls to io_ctx.operate()/exec()
    ...
    static int get(librados::ObjectReadOperation *op,
                   ...
                   bool get_all, std::list < otp_info_t > *result);
    static int check(CephContext *cct, librados::IoCtx &ioctx,
                     const std::string &oid,
                     const std::string &id,
                     const std::string &val, otp_check_t *result);
    static int get_current_time(librados::IoCtx &ioctx,
                                const std::string &oid,
