...
void MDCache::adjust_bounded_subtree_auth(CDir *dir, const vector < dirfrag_t > & bound_dfs,
        ...
        if (in->get_projected_parent_dn() != dn) {
            ...
            if (dir_follows + 1 > dn->first) {
                ...
                if (realm->has_snaps_in_range(oldfirst, dir_follows)) {
                    CDir *dir = dn->dir;
                    CDentry *olddn = dir->add_remote_dentry(dn->get_name(), in->ino(), in->d_type(), dn->alternate_name, oldfirst,
                                                            dir_follows);
    ...
    while (parent) {
        ...
        if (do_parent_mtime || linkunlink) {
            ...
            if (do_parent_mtime) {
                ...
                dout(10) << "predirty_journal_parents bumping fragstat change_attr to " << pf->fragstat.change_attr << " on " << parent
                         << dendl;
    ...
    for (const auto &p : strong->strong_dirfrags) {
        ...
        if (it != strong->strong_dentries.end()) {
            ...
            for (const auto &q : dmap) {
                ...
                if (!dn) {
                    if (d.is_remote()) {
                        dn = dir->add_remote_dentry(ss.name, d.remote_ino, d.remote_d_type, mempool::mds_co::string(d.alternate_name), d.first,
                                                    ss.snapid);
                    } else if (d.is_null()) {
    ...
    for (const auto &p : m->realms) {
        ...
        if (p.first.ino > 0) {
            ...
            if (!parent_dir->is_auth() ||
                ...
                auto em = delayed_expire[parent_dir].emplace(std::piecewise_construct, std::forward_as_tuple(from),
                          std::forward_as_tuple());
                if (em.second) {
...
int MDCache::send_dir_updates(CDir *dir, bool bcast)
{
    ...
    for (set < mds_rank_t >::iterator it = who.begin();
        ...
        mds->send_message_mds(make_message < MDirUpdate > (mds->get_nodeid(), dir->dirfrag(), dir->dir_rep, dir_rep_set, path,
                              bcast), *it);
    }
    ...
    for (auto it = fragments.lower_bound({diri->ino(), {}});
        ...
        if (it->first.frag.contains(mydf.frag)) {
            ...
            mds->queue_waiter(new MDSInternalContextWrapper(mds, new LambdaContext([this, basefrag = it->first, mdr]() {
                if (!mdr->is_live()) {
                    dout(20) << "quiesce_overdrive_fragmenting_async: bailing out, mdr " << *mdr << "is dead: " << mdr->dead << "; killed: "
                             << mdr->killed << dendl;
                    return;
                }
