...
void MDSTableServer::handle_request(MMDSTableRequest * req)
{
    ...
    default:
        assert(0 == "unrecognized mds_table_server request op");
    }
}
...
class C_Prepare:public MDSLogContextBase {
    ...
    C_Prepare(MDSTableServer * s, MMDSTableRequest * r, version_t v):server(s),
        req(r), tid(v) {
    }
...
void MDSTableServer::_prepare_logged(MMDSTableRequest * req, version_t tid)
{
    ...
    MMDSTableRequest *reply =
        new MMDSTableRequest(table, TABLESERVER_OP_AGREE, req->reqid, tid);
    reply->bl = req->bl;
...
void MDSTableServer::_commit_logged(MMDSTableRequest * req)
{
    ...
    MMDSTableRequest *reply =
        new MMDSTableRequest(table, TABLESERVER_OP_ACK, req->reqid,
                             req->get_tid());
    mds->send_message_mds(reply, mds_rank_t(req->get_source().num()));
    req->put();
}
...
void MDSTableServer::finish_recovery(set < mds_rank_t > &active)
{
    ...
    for (set < mds_rank_t >::iterator p = active.begin(); p != active.end();
         ++p)
...
void MDSTableServer::handle_mds_recovery(mds_rank_t who)
{
    ...
    for (map < version_t, mds_table_pending_t >::iterator p =
         pending_for_mds.begin(); p != pending_for_mds.end(); ++p) {
        if (p->second.mds != who)
            continue;
        if (p->second.reqid >= next_reqid)
            next_reqid = p->second.reqid + 1;
        MMDSTableRequest *reply =
            new MMDSTableRequest(table, TABLESERVER_OP_AGREE, p->second.reqid,
                                 p->second.tid);
        mds->send_message_mds(reply, who);
    }

    MMDSTableRequest *reply =
        new MMDSTableRequest(table, TABLESERVER_OP_SERVER_READY, next_reqid);
    mds->send_message_mds(reply, who);
}
