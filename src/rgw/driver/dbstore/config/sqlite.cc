...
// parameter names for prepared statement bindings
        ...
        void read_period_row(const sqlite::stmt_execution & stmt,
                             RGWPeriod & row) {
        ...
        void read_zonegroup_row(const sqlite::stmt_execution & stmt,
                                ZoneGroupRow & row) {
            std::string data = sqlite::column_text(stmt, 3);
            row.ver = sqlite::column_int(stmt, 4);
            row.tag = sqlite::column_text(stmt, 5);
...
// Realm
    ...
    class SQLiteRealmWriter:public sal::RealmWriter {
      ...
      public:
        ...
        :impl(impl), ver(ver), tag(std::move(tag)), realm_id(realm_id),
            ...
            *dpp, "dbconfig:sqlite:realm_write "};
            ...
            try {
                ...
                ldpp_dout(dpp,
                          20) << "realm update failed: " << e.what() << dendl;
        ...
        int rename(const DoutPrefixProvider * dpp, optional_yield y,
            ...
            Prefix prefix {
            *dpp, "dbconfig:sqlite:realm_rename "};
            dpp = &prefix;
            ...
            try {
                ...
                ldpp_dout(dpp,
                          20) << "realm rename failed: " << e.what() << dendl;
        ...
        int remove(const DoutPrefixProvider * dpp, optional_yield y) override {
            Prefix prefix {
            *dpp, "dbconfig:sqlite:realm_remove "};
            dpp = &prefix;
            ...
            try {
                ...
                if (!stmt) {
                    const std::string sql =
                        fmt::format(schema::realm_delete3, P1, P2, P3);
                    stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
                auto binding = sqlite::stmt_binding { stmt.get() };
                sqlite::bind_text(dpp, binding, P1, realm_id);
                ...
                ldpp_dout(dpp,
                          20) << "realm delete failed: " << e.what() << dendl;
    ...
    int SQLiteConfigStore::write_default_realm_id(const DoutPrefixProvider *
                                                  dpp, optional_yield y,
                                                  bool exclusive,
                                                  std::string_view realm_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:write_default_realm_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (exclusive) {
                ...
                if (!*stmt) {
                    const std::string sql =
                        fmt::format(schema::default_realm_insert1, P1);
                    *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
            }
            else {
                ...
                if (!*stmt) {
                    const std::string sql =
                        fmt::format(schema::default_realm_upsert1, P1);
                    *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
            }
            auto binding = sqlite::stmt_binding { stmt->get() };
            ...
            ldpp_dout(dpp,
                      20) << "default realm insert failed: " << e.
                what() << dendl;
    ...
    int SQLiteConfigStore::read_default_realm_id(const DoutPrefixProvider * dpp,
                                                 ...
                                                 std::string & realm_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_default_realm_id "};
        dpp = &prefix;
        ...
        try {
            ...
            ldpp_dout(dpp,
                      20) << "default realm select failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::busy) {
                return -EBUSY;
            }
    ...
    int SQLiteConfigStore::delete_default_realm_id(const DoutPrefixProvider *
                                                   dpp, optional_yield y)
    {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:delete_default_realm_id "};
        dpp = &prefix;
        ...
        try {
            ...
            ldpp_dout(dpp,
                      20) << "default realm delete failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::busy) {
                return -EBUSY;
            }
    ...
    int SQLiteConfigStore::create_realm(const DoutPrefixProvider * dpp,
                                        ...
                                        const RGWRealm & info,
                                        std::unique_ptr < sal::RealmWriter >
                                        *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:create_realm "};
        dpp = &prefix;
        ...
        if (writer) {
            *writer =
                std::make_unique < SQLiteRealmWriter > (impl.get(), ver,
                                                        std::move(tag), info.id,
                                                        info.name);
        }
        return 0;
    }
    ...
    int SQLiteConfigStore::read_realm_by_id(const DoutPrefixProvider * dpp,
                                            ...
                                            *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_realm_by_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::realm_select_id1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, realm_id);
        ...
        if (writer) {
            *writer =
                std::make_unique < SQLiteRealmWriter > (impl.get(), row.ver,
                                                        std::move(row.tag),
                                                        info.id, info.name);
        }
        return 0;
    ...
    int SQLiteConfigStore::read_realm_by_name(const DoutPrefixProvider * dpp,
        ...
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_realm_by_name "};
        dpp = &prefix;
    ...
    int SQLiteConfigStore::read_default_realm(const DoutPrefixProvider * dpp,
                                              ...
                                              sal::RealmWriter > *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_default_realm "};
        dpp = &prefix;
    ...
    int SQLiteConfigStore::read_realm_id(const DoutPrefixProvider * dpp,
                                         ...
                                         std::string & realm_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_realm_id "};
        dpp = &prefix;
    ...
    int SQLiteConfigStore::realm_notify_new_period(const DoutPrefixProvider *
                                                   dpp, optional_yield y,
    ...
    int SQLiteConfigStore::list_realm_names(const DoutPrefixProvider * dpp,
                                            ...
                                            std::span < std::string > entries,
                                            sal::ListResult < std::string >
                                            &result) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:list_realm_names "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::realm_select_names2, P1, P2);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
...
// Period
    ...
    int SQLiteConfigStore::create_period(const DoutPrefixProvider * dpp,
        ...
        Prefix prefix {
        *dpp, "dbconfig:sqlite:create_period "};
        dpp = &prefix;
    ...
    static void period_select_epoch(const DoutPrefixProvider * dpp,
        ...
        if (!stmt) {
            const std::string sql =
                fmt::format(schema::period_select_epoch2, P1, P2);
            stmt = sqlite::prepare_statement(dpp, conn.db.get(), sql);
        }
        auto binding = sqlite::stmt_binding { stmt.get() };
    ...
    static void period_select_latest(const DoutPrefixProvider * dpp,
        ...
        if (!stmt) {
            const std::string sql =
                fmt::format(schema::period_select_latest1, P1);
            stmt = sqlite::prepare_statement(dpp, conn.db.get(), sql);
        }
        auto binding = sqlite::stmt_binding { stmt.get() };
        sqlite::bind_text(dpp, binding, P1, id);
    ...
    int SQLiteConfigStore::read_period(const DoutPrefixProvider * dpp,
                                       ...
                                       RGWPeriod & info) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_period "};
        dpp = &prefix;
    ...
    int SQLiteConfigStore::delete_period(const DoutPrefixProvider * dpp,
                                         ...
                                         std::string_view period_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:delete_period "};
        dpp = &prefix;
    ...
    int SQLiteConfigStore::list_period_ids(const DoutPrefixProvider * dpp,
                                           ...
                                           &result) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:list_period_ids "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::period_select_ids2, P1, P2);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, marker);
...
// ZoneGroup
    ...
    class SQLiteZoneGroupWriter:public sal::ZoneGroupWriter {
      ...
      public:
        ...
        :impl(impl), ver(ver), tag(std::move(tag)),
            ...
            Prefix prefix {
            *dpp, "dbconfig:sqlite:zonegroup_write "};
            dpp = &prefix;
            ...
            try {
                ...
                if (!stmt) {
                    const std::string sql =
                        fmt::format(schema::zonegroup_update5,
                                    P1, P2, P3, P4, P5);
                    stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
                auto binding = sqlite::stmt_binding { stmt.get() };
        ...
        int rename(const DoutPrefixProvider * dpp, optional_yield y,
            ...
            Prefix prefix {
            *dpp, "dbconfig:sqlite:zonegroup_rename "};
            dpp = &prefix;

            if (!impl) {
                return -EINVAL; // can't write after conflict or delete
            }
            if (zonegroup_id != info.get_id()
                || zonegroup_name != info.get_name()) {
                return -EINVAL; // can't modify zonegroup id or name directly
            }
            if (new_name.empty()) {
                ldpp_dout(dpp,
            ...
            try {
                ...
                if (!stmt) {
                    const std::string sql =
                        fmt::format(schema::zonegroup_rename4,
                                    P1, P2, P3, P4);
                    stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
                auto binding = sqlite::stmt_binding { stmt.get() };
        ...
        int remove(const DoutPrefixProvider * dpp, optional_yield y) override {
            Prefix prefix {
            *dpp, "dbconfig:sqlite:zonegroup_remove "};
            dpp = &prefix;
            ...
            try {
                ...
                if (!stmt) {
                    const std::string sql =
                        fmt::format(schema::zonegroup_delete3,
                                    P1, P2, P3);
                    stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
                auto binding = sqlite::stmt_binding { stmt.get() };
    ...
    int SQLiteConfigStore::write_default_zonegroup_id(const DoutPrefixProvider *
                                                      dpp, optional_yield y,
                                                      bool exclusive,
                                                      std::string_view realm_id,
                                                      std::
                                                      string_view zonegroup_id)
    {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:write_default_zonegroup_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (exclusive) {
                ...
                if (!*stmt) {
                    const std::string sql =
                        fmt::format(schema::default_zonegroup_insert2,
                                    P1, P2);
                    *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
            }
                ...
                if (!*stmt) {
                    const std::string sql =
                        fmt::format(schema::default_zonegroup_upsert2,
                                    P1, P2);
                    *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
            }
            ...
            ldpp_dout(dpp,
                      20) << "default zonegroup insert failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::busy) {
                return -EBUSY;
            }
    ...
    int SQLiteConfigStore::read_default_zonegroup_id(const DoutPrefixProvider *
                                                     dpp, optional_yield y,
                                                     std::string_view realm_id,
                                                     std::
                                                     string & zonegroup_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_default_zonegroup_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::default_zonegroup_select1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, realm_id);
            ...
            ldpp_dout(dpp,
                      20) << "default zonegroup select failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::done) {
                return -ENOENT;
            }
    ...
    int SQLiteConfigStore::
        delete_default_zonegroup_id(const DoutPrefixProvider * dpp,
                                    optional_yield y,
                                    std::string_view realm_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:delete_default_zonegroup_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::default_zonegroup_delete1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, realm_id);
            ...
            ldpp_dout(dpp,
                      20) << "default zonegroup delete failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::busy) {
                return -EBUSY;
            }
    ...
    int SQLiteConfigStore::create_zonegroup(const DoutPrefixProvider * dpp,
                                            ...
                                            std::unique_ptr <
                                            sal::ZoneGroupWriter > *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:create_zonegroup "};
        dpp = &prefix;
        ...
        try {
            ...
            if (exclusive) {
                ...
                if (!*stmt) {
                    const std::string sql =
                        fmt::format(schema::zonegroup_insert6,
                                    P1, P2, P3, P4, P5, P6);
                    *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
            }
                ...
                if (!*stmt) {
                    const std::string sql =
                        fmt::format(schema::zonegroup_upsert6,
                                    P1, P2, P3, P4, P5, P6);
                    *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
            }
            ...
            ldpp_dout(dpp,
                      20) << "zonegroup insert failed: " << e.what() << dendl;
            if (e.code() == sqlite::errc::foreign_key_constraint) {
                return -EINVAL; // refers to nonexistent RealmID
            }
        ...
        if (writer) {
            *writer =
                std::make_unique < SQLiteZoneGroupWriter > (impl.get(), ver,
                                                            std::move(tag),
                                                            info.id, info.name);
        }
        return 0;
    ...
    int SQLiteConfigStore::read_zonegroup_by_id(const DoutPrefixProvider * dpp,
                                                ...
                                                *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_zonegroup_by_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::zonegroup_select_id1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, zonegroup_id);
            ...
            ldpp_dout(dpp,
                      20) << "zonegroup decode failed: " << e.what() << dendl;
            return -EIO;
        }
        catch(const sqlite::error & e) {
            ldpp_dout(dpp,
                      20) << "zonegroup select failed: " << e.what() << dendl;
            if (e.code() == sqlite::errc::done) {
                return -ENOENT;
            }
            else if (e.code() == sqlite::errc::busy) {
        ...
        if (writer) {
            *writer =
                std::make_unique < SQLiteZoneGroupWriter > (impl.get(), row.ver,
                                                            std::move(row.tag),
                                                            info.id, info.name);
        }
        return 0;
    ...
    int SQLiteConfigStore::read_zonegroup_by_name(const DoutPrefixProvider *
                                                  dpp, optional_yield y,
                                                  std::
                                                  string_view zonegroup_name,
                                                  RGWZoneGroup & info,
                                                  std::unique_ptr <
                                                  sal::ZoneGroupWriter >
                                                  *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_zonegroup_by_name "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::zonegroup_select_name1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
    ...
    int SQLiteConfigStore::read_default_zonegroup(const DoutPrefixProvider *
                                                  dpp, optional_yield y,
                                                  std::string_view realm_id,
                                                  RGWZoneGroup & info,
                                                  std::unique_ptr <
                                                  ...
                                                  *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_default_zonegroup "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                static constexpr std::string_view sql =
                    schema::zonegroup_select_default0;
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto reset = sqlite::stmt_execution { stmt.get() };
            sqlite::eval1(dpp, reset);
    ...
    int SQLiteConfigStore::list_zonegroup_names(const DoutPrefixProvider * dpp,
                                                ...
                                                &result) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:list_zonegroup_names "};
        dpp = &prefix;
...
// Zone
    ...
    class SQLiteZoneWriter:public sal::ZoneWriter {
      ...
      public:
        ...
        :impl(impl), ver(ver), tag(std::move(tag)),
            ...
            Prefix prefix {
            *dpp, "dbconfig:sqlite:zone_write "};
            dpp = &prefix;
            ...
            try {
                ...
                ldpp_dout(dpp,
                          20) << "zone update failed: " << e.what() << dendl;
                if (e.code() == sqlite::errc::foreign_key_constraint) {
                    return -EINVAL; // refers to nonexistent RealmID
                }
                else if (e.code() == sqlite::errc::busy) {
        ...
        int rename(const DoutPrefixProvider * dpp, optional_yield y,
            ...
            Prefix prefix {
            *dpp, "dbconfig:sqlite:zone_rename "};
            dpp = &prefix;
            ...
            try {
                ...
                ldpp_dout(dpp,
                          20) << "zone rename failed: " << e.what() << dendl;
                if (e.code() == sqlite::errc::unique_constraint) {
                    return -EEXIST; // Name already taken
                }
                else if (e.code() == sqlite::errc::busy) {
        ...
        int remove(const DoutPrefixProvider * dpp, optional_yield y) override {
            Prefix prefix {
            *dpp, "dbconfig:sqlite:zone_remove "};
            dpp = &prefix;
            ...
            try {
                ...
                if (!stmt) {
                    const std::string sql =
                        fmt::format(schema::zone_delete3, P1, P2, P3);
                    stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
                }
                auto binding = sqlite::stmt_binding { stmt.get() };
                ...
                ldpp_dout(dpp,
                          20) << "zone delete failed: " << e.what() << dendl;
                if (e.code() == sqlite::errc::busy) {
                    return -EBUSY;
                }
                return -EIO;
    ...
    int SQLiteConfigStore::write_default_zone_id(const DoutPrefixProvider * dpp,
        ...
        Prefix prefix {
        *dpp, "dbconfig:sqlite:write_default_zone_id "};
        dpp = &prefix;
        ...
        try {
            ...
            ldpp_dout(dpp,
                      20) << "default zone insert failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::busy) {
                return -EBUSY;
            }
    ...
    int SQLiteConfigStore::read_default_zone_id(const DoutPrefixProvider * dpp,
                                                ...
                                                std::string & zone_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_default_zone_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::default_zone_select1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, realm_id);
            ...
            ldpp_dout(dpp,
                      20) << "default zone select failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::done) {
                return -ENOENT;
            }
    ...
    int SQLiteConfigStore::delete_default_zone_id(const DoutPrefixProvider *
                                                  dpp, optional_yield y,
                                                  std::string_view realm_id) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:delete_default_zone_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::default_zone_delete1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, realm_id);
            ...
            ldpp_dout(dpp,
                      20) << "default zone delete failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::busy) {
                return -EBUSY;
            }
    ...
    int SQLiteConfigStore::create_zone(const DoutPrefixProvider * dpp,
                                       ...
                                       std::unique_ptr < sal::ZoneWriter >
                                       *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:create_zone "};
        dpp = &prefix;
        ...
        if (writer) {
            *writer =
                std::make_unique < SQLiteZoneWriter > (impl.get(), ver,
                                                       std::move(tag), info.id,
                                                       info.name);
        }
        return 0;
    ...
    int SQLiteConfigStore::read_zone_by_id(const DoutPrefixProvider * dpp,
                                           ...
                                           *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_zone_by_id "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::zone_select_id1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, zone_id);
        ...
        if (writer) {
            *writer =
                std::make_unique < SQLiteZoneWriter > (impl.get(), row.ver,
                                                       std::move(row.tag),
                                                       info.id, info.name);
        }
        return 0;
    ...
    int SQLiteConfigStore::read_zone_by_name(const DoutPrefixProvider * dpp,
                                             ...
                                             *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_zone_by_name "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::zone_select_name1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
    ...
    int SQLiteConfigStore::read_default_zone(const DoutPrefixProvider * dpp,
                                             ...
                                             *writer) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_default_zone "};
        dpp = &prefix;
    ...
    int SQLiteConfigStore::list_zone_names(const DoutPrefixProvider * dpp,
                                           ...
                                           &result) {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:list_zone_names "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::zone_select_names2, P1, P2);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, marker);
...
// PeriodConfig
    ...
    int SQLiteConfigStore::read_period_config(const DoutPrefixProvider * dpp,
        ...
        Prefix prefix {
        *dpp, "dbconfig:sqlite:read_period_config "};
        dpp = &prefix;
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql =
                    fmt::format(schema::period_config_select1, P1);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding { stmt.get() };
            sqlite::bind_text(dpp, binding, P1, realm_id);
            ...
            ldpp_dout(dpp,
                      20) << "period config decode failed: " << e.
                what() << dendl;
            return -EIO;
        }
        catch(const sqlite::error & e) {
            ldpp_dout(dpp,
                      20) << "period config select failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::done) {
                return -ENOENT;
            }
    ...
    int SQLiteConfigStore::write_period_config(const DoutPrefixProvider * dpp,
        ...
        Prefix prefix {
        *dpp, "dbconfig:sqlite:write_period_config "};
        dpp = &prefix;
        ...
        try {
            ...
            ldpp_dout(dpp,
                      20) << "period config insert failed: " << e.
                what() << dendl;
            if (e.code() == sqlite::errc::primary_key_constraint) {
                return -EEXIST;
            }
    ...
    namespace {
        ...
        void apply_schema_migrations(const DoutPrefixProvider * dpp,
                                     sqlite3 * db) {
            sqlite::execute(dpp, db, "PRAGMA foreign_keys = ON", nullptr,
                            nullptr);

            // initiate a transaction and read the current schema version
            uint32_t version = 0;
            sqlite::execute(dpp, db, "BEGIN; PRAGMA user_version", version_cb,
                            &version);
            ...
            for (; m != schema::migrations.end(); ++m, ++version) {
                try {
                    ...
                    ldpp_dout(dpp,
                              -1) << "ERROR: schema migration failed on v" <<
                        version << ": " << m->description << dendl;
                    throw;
                }
            }
            ...
            if (version > initial_version) {
                ...
                const auto commit =
                    fmt::format("PRAGMA user_version = {}; COMMIT", version);
                sqlite::execute(dpp, db, commit.c_str(), nullptr, nullptr);

                ldpp_dout(dpp,
                          4) << "upgraded database schema to version " <<
                    version << dendl;
            }
            else {
                // nothing to commit
    ...
    auto create_sqlite_store(const DoutPrefixProvider * dpp,
                             const std::string & uri)
    ->std::unique_ptr < config::SQLiteConfigStore > {
        Prefix prefix {
        *dpp, "dbconfig:sqlite:create_sqlite_store "};
        dpp = &prefix;

        // build the connection pool
        int flags =
            SQLITE_OPEN_CREATE | SQLITE_OPEN_URI | SQLITE_OPEN_READWRITE |
            SQLITE_OPEN_NOMUTEX;
        ...
        auto impl =
            std::make_unique < SQLiteImpl > (std::move(factory),
                                             max_connections);
