...
template<class K, class V>
typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::operator[](const K &key)
{
...
template<class K, class V>
typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::find(const K &key)
{
    if (auto found = cache.find(key); found) {
        return *found;
    }
...
template<class K, class V>
typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::lower_bound(const K &key)
{
    if (weak_refs.empty()) {
        return {};
    }
