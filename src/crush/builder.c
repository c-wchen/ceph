...
int crush_add_rule(struct crush_map *map, struct crush_rule *rule, int ruleno)
{
    ...
    if (r >= map->max_rules) {
        ...
        if ((_realloc =
             realloc(map->rules,
                     map->max_rules * sizeof(map->rules[0]))) == NULL) {
            return -ENOMEM;
        }
        else {
            map->rules = _realloc;
        }
        memset(map->rules + oldsize, 0,
               (map->max_rules - oldsize) * sizeof(map->rules[0]));
    }
...
struct crush_rule *crush_make_rule(int len, int ruleset, int type, int minsize,
                                   int maxsize)
{
 ...
 */
void crush_rule_set_step(struct crush_rule *rule, int n, int op, int arg1,
                         int arg2)
{
...
int crush_add_bucket(struct crush_map *map,
                     int id, struct crush_bucket *bucket, int *idout)
{
    int pos;
    ...
    while (pos >= map->max_buckets) {
        ...
        if ((_realloc =
             realloc(map->buckets,
                     map->max_buckets * sizeof(map->buckets[0]))) == NULL) {
            return -ENOMEM;
        }
        else {
            map->buckets = _realloc;
        }
        memset(map->buckets + oldsize, 0,
               (map->max_buckets - oldsize) * sizeof(map->buckets[0]));
    }
...
struct crush_bucket_uniform *crush_make_uniform_bucket(int hash, int type,
                                                       int size, int *items,
                                                       int item_weight)
{
    int i;
...
struct crush_bucket_list *crush_make_list_bucket(int hash, int type, int size,
...
struct crush_bucket_straw *crush_make_straw_bucket(struct crush_map *map,
                                                   int hash,
                                                   int type,
                                                   int size,
                                                   int *items, int *weights)
...
struct crush_bucket_straw2 *crush_make_straw2_bucket(struct crush_map *map,
                                                     int hash,
                                                     int type,
                                                     int size,
                                                     int *items, int *weights)
                                       ...
                                       int *items, int *weights)
{
    ...
    case CRUSH_BUCKET_UNIFORM:
        if (size && weights)
            ...
            item_weight = 0;
        return (struct crush_bucket *)crush_make_uniform_bucket(hash, type,
                                                                size, items,
                                                                item_weight);

    case CRUSH_BUCKET_LIST:
        return (struct crush_bucket *)crush_make_list_bucket(hash, type, size,
                                                             items, weights);

    case CRUSH_BUCKET_TREE:
        return (struct crush_bucket *)crush_make_tree_bucket(hash, type, size,
                                                             items, weights);

    case CRUSH_BUCKET_STRAW:
        return (struct crush_bucket *)crush_make_straw_bucket(map, hash, type,
                                                              size, items,
                                                              weights);
    case CRUSH_BUCKET_STRAW2:
        return (struct crush_bucket *)crush_make_straw2_bucket(map, hash, type,
                                                               size, items,
                                                               weights);
    }
    return 0;
...
int crush_add_uniform_bucket_item(struct crush_bucket_uniform *bucket, int item,
                                  int weight)
{
...
int crush_add_list_bucket_item(struct crush_bucket_list *bucket, int item,
                               int weight)
{
    int newsize = bucket->h.size + 1;
    void *_realloc = NULL;
    ...
    if ((_realloc =
         realloc(bucket->item_weights, sizeof(__u32) * newsize)) == NULL) {
        return -ENOMEM;
    }
    else {
    ...
    if ((_realloc =
         realloc(bucket->sum_weights, sizeof(__u32) * newsize)) == NULL) {
        return -ENOMEM;
    }
    else {
    ...
    if (newsize > 1) {
        ...
        bucket->sum_weights[newsize - 1] =
            bucket->sum_weights[newsize - 2] + weight;
    }
...
int crush_add_tree_bucket_item(struct crush_bucket_tree *bucket, int item,
                               int weight)
{
    int newsize = bucket->h.size + 1;
                          ...
                          struct crush_bucket *b, int item, int weight)
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM:
        return crush_add_uniform_bucket_item((struct crush_bucket_uniform *)b,
                                             item, weight);
    case CRUSH_BUCKET_LIST:
        return crush_add_list_bucket_item((struct crush_bucket_list *)b, item,
                                          weight);
    case CRUSH_BUCKET_TREE:
        return crush_add_tree_bucket_item((struct crush_bucket_tree *)b, item,
                                          weight);
    case CRUSH_BUCKET_STRAW:
        return crush_add_straw_bucket_item(map, (struct crush_bucket_straw *)b,
                                           item, weight);
    case CRUSH_BUCKET_STRAW2:
        return crush_add_straw2_bucket_item(map,
...
int crush_remove_uniform_bucket_item(struct crush_bucket_uniform *bucket,
                                     int item)
{
    unsigned i, j;
    int newsize;
...
int crush_remove_tree_bucket_item(struct crush_bucket_tree *bucket, int item)
{
    ...
    if (newsize != bucket->h.size) {
        ...
        if ((_realloc =
             realloc(bucket->h.items, sizeof(__s32) * newsize)) == NULL) {
            return -ENOMEM;
        }
        else {
            bucket->h.items = _realloc;
...
int crush_bucket_remove_item(struct crush_map *map, struct crush_bucket *b,
                             int item)
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM:
        return crush_remove_uniform_bucket_item((struct crush_bucket_uniform *)
                                                b, item);
    case CRUSH_BUCKET_LIST:
        return crush_remove_list_bucket_item((struct crush_bucket_list *)b,
                                             item);
    case CRUSH_BUCKET_TREE:
        return crush_remove_tree_bucket_item((struct crush_bucket_tree *)b,
                                             item);
    case CRUSH_BUCKET_STRAW:
        return crush_remove_straw_bucket_item(map,
                                              (struct crush_bucket_straw *)b,
                                              item);
    case CRUSH_BUCKET_STRAW2:
        return crush_remove_straw2_bucket_item(map,
                                               (struct crush_bucket_straw2 *)b,
                                               item);
    default:
        return -1;
...
int crush_adjust_uniform_bucket_item_weight(struct crush_bucket_uniform *bucket,
                                            int item, int weight)
{
...
int crush_adjust_list_bucket_item_weight(struct crush_bucket_list *bucket,
                                         int item, int weight)
{
    int diff;
    unsigned i, j;
...
int crush_adjust_tree_bucket_item_weight(struct crush_bucket_tree *bucket,
                                         int item, int weight)
{
    int diff;
    int node;
                                    ...
                                    int item, int weight)
{
    ...
    case CRUSH_BUCKET_UNIFORM:
        return
            crush_adjust_uniform_bucket_item_weight((struct crush_bucket_uniform
                                                     *)b, item, weight);
    case CRUSH_BUCKET_LIST:
        return crush_adjust_list_bucket_item_weight((struct crush_bucket_list *)
                                                    b, item, weight);
    case CRUSH_BUCKET_TREE:
        return crush_adjust_tree_bucket_item_weight((struct crush_bucket_tree *)
                                                    b, item, weight);
    case CRUSH_BUCKET_STRAW:
...
static int crush_reweight_uniform_bucket(struct crush_map *map,
                                         struct crush_bucket_uniform *bucket)
{
    unsigned i;
...
static int crush_reweight_list_bucket(struct crush_map *map,
                                      struct crush_bucket_list *bucket)
{
    unsigned i;
...
static int crush_reweight_tree_bucket(struct crush_map *map,
                                      struct crush_bucket_tree *bucket)
{
    unsigned i;
...
static int crush_reweight_straw_bucket(struct crush_map *map,
                                       struct crush_bucket_straw *bucket)
{
    unsigned i;
...
static int crush_reweight_straw2_bucket(struct crush_map *map,
                                        struct crush_bucket_straw2 *bucket)
{
    unsigned i;
...
int crush_reweight_bucket(struct crush_map *map, struct crush_bucket *b)
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM:
        return crush_reweight_uniform_bucket(map,
    ...
    case CRUSH_BUCKET_STRAW2:
        return crush_reweight_straw2_bucket(map,
                                            (struct crush_bucket_straw2 *)b);
    default:
        return -1;
    }
...
struct crush_choose_arg *crush_make_choose_args(struct crush_map *map,
                                                int num_positions)
{
    int b;
    ...
    int size = (sizeof(struct crush_choose_arg) * map->max_buckets + sizeof(struct crush_weight_set) * bucket_count * num_positions + sizeof(__u32) * sum_bucket_size * num_positions + // weights
    ...
    struct crush_weight_set *weight_set =
        (struct crush_weight_set *)(arg + map->max_buckets);
    ...
    for (b = 0; b < map->max_buckets; b++) {
        ...
        struct crush_bucket_straw2 *bucket =
            (struct crush_bucket_straw2 *)map->buckets[b];

        int position;
        for (position = 0; position < num_positions; position++) {
            memcpy(weights, bucket->item_weights,
                   sizeof(__u32) * bucket->h.size);
