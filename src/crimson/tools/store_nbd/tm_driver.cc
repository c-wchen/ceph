...
TMDriver::read_extents_ret TMDriver::read_extents(Transaction & t,
                                                  laddr_t offset,
                                                  extent_len_t length)
{
    return seastar::do_with(lba_pin_list_t(),
                            lextent_list_t < TestBlock > (),
                            [this, &t, offset, length] (auto & pins,
                                                        auto & ret) {
                            return tm->get_pins(t, offset,
                                                length).si_then([this, &t,
                                                                 &pins,
                                                                 &ret] (auto
                                                                        _pins) {
                                                                _pins.
...
seastar::future < bufferlist > TMDriver::read(off_t offset, size_t size)
{
    ...
    return repeat_eagain([ =, &blret, this] {
                         return tm->
                         with_transaction_intr(Transaction::src_t::READ, "read",
                                               [ =, &blret, this] (auto & t) {
                                               return read_extents(t, offset,
                                                                   size).
                                               si_then([ =,
                                                        &blret] (auto ext_list)
                                                       {
              size_t cur = offset; for (auto & i:     ext_list)
                                                       {
                                                       if (cur != i.first) {
                                                       assert(cur < i.first);
                                                       blret.append_zero(i.
        ...
        .handle_error(crimson::ct_error::assert_all {
           ...
           logger().debug("read complete"); return std::move(*blptrret);}
    );
}
...
size_t TMDriver::get_size() const const
{
...
seastar::future <> TMDriver::mkfs()
{
    ...
    return Device::make_device(*config.path,
                               device_type_t::SSD).then([this] (DeviceRef dev) {
                                                        device = std::move(dev);
                ...
                logger().debug("device mkfs done"); return device->mount();}
    ).safe_then([this] {
                init(); logger().debug("tm mkfs"); return tm->mkfs();}
    ).safe_then([this] {
                logger().debug("tm close"); return tm->close();}
    ).safe_then([this] {
                logger().debug("sm close"); return device->close();}
    ).safe_then([this] {
                clear();
                device.reset();
...
seastar::future <> TMDriver::mount()
{
    return (config.mkfs ? mkfs() : seastar::now()
           ...
           device = std::move(dev); return device->mount();}
    ).safe_then([this] {
                init(); return tm->mount();}
    ).handle_error(crimson::ct_error::assert_all {
