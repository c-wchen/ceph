...
bool add_challenge(CephContext * cct,
...
struct CephXTicketManager {
    ...
    void set_have_need_key(uint32_t service_id, uint32_t & have,
                           uint32_t & need);
 ...
 */
extern bool cephx_verify_authorizer(CephContext * cct,
                                    const KeyStore & keys,
                               ...
                               std::string & error)
{
    ...
    if (magic != AUTH_ENC_MAGIC) {
        ...
        oss << "bad magic in decode_decrypt, " << magic << " != " <<
            AUTH_ENC_MAGIC;
        error = oss.str();
        return;
    }
...
template < typename T >
    void encode_encrypt_enc_bl(CephContext * cct, const T & t,
                               const CryptoKey & key, ceph::buffer::list & out,
                               std::string & error)
{
    ceph::buffer::list bl;
    __u8 struct_v = 1;
