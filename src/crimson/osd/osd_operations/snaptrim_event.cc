...
SnapTrimEvent::snap_trim_ertr::future<seastar::stop_iteration> SnapTrimEvent::with_pg(
...
SnapTrimObjSubEvent::remove_or_update_ret_t > SnapTrimObjSubEvent::remove_or_update(
    ObjectContextRef obc,
    ObjectContextRef head_obc)
{
    ...
    return seastar::do_with(ceph::os::Transaction{}, [ =, this](auto & txn) {
        ...
        return std::move(ret).safe_then_interruptible(
                   [&txn, obc, num_objects_before_trim, log_entries = std::move(log_entries), head_obc = std::move(head_obc),
        this]() mutable {
            osd_op_p.at_version = pg->next_version();
...
SnapTrimObjSubEvent::remove_or_update_iertr::future<> SnapTrimObjSubEvent::with_pg(
    ShardServices &shard_services, Ref<PG> _pg)
{
    return enter_stage<interruptor>(
               client_pp().wait_for_active
