...
struct bluefs_fnode_t {
    ...
    {
    } bluefs_fnode_t(uint64_t _ino, uint64_t _size, utime_t _mtime): ino(_ino),
        size(_size), mtime(_mtime), allocated(0), allocated_commited(0)
    {
    }
    bluefs_fnode_t(const bluefs_fnode_t &other): ino(other.ino), size(other.size),
        mtime(other.mtime), allocated(other.allocated),
    ...
    DENC_HELPERS void bound_encode(size_t &p) const
    {
        _denc_friend(*this, p);
    ...
    friend std::enable_if_t < std::is_same_v < bluefs_fnode_t,
           std::remove_const_t < T >>> _denc_friend(T &v, P &p)
    {
        DENC_START(1, 1, p);
        denc_varint(v.ino, p);
        denc_varint(v.size, p);
