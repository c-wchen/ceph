...
static bool is_not_trash_user(const librbd::
                   ...
                   &ceph_global_init_args)
{
    ...
    if (vm.find(EXPIRES_AT) != vm.end()) {
        ...
        if (r < 0) {
            std::
            cerr << "rbd: error calling /bin/date: " <<
                 cpp_strerror(r)
                 << std::endl;
            return r;
        }
    ...
    if (dt < 0) {
        std::
        cerr <<
             "rbd: cannot use a date in the past as an expiration date"
             << std::endl;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        std::
        cerr << "rbd: deferred delete error: " <<
             cpp_strerror(r)
             << std::endl;
    }
    ...
    if (expires_at != "now") {
        std::
        cout << "rbd: image " << image_name <<
             " will expire at " << exp_time << std::endl;
    }
    return r;
}
                          ...
                          po::options_description *options)
{
    ...
    ("force", po::bool_switch(),
     "force remove of non-expired delayed images");
}
                   ...
                   &ceph_global_init_args)
{
    ...
    std::string image_id;
    int r =
        utils::get_pool_image_id(vm, &arg_index, &pool_name,
                                 &namespace_name,
                                 &image_id);
    if (r < 0) {
        return r;
    ...
    utils::ProgressContext pc("Removing image",
    ...
    if (r < 0) {
        if (r == -ENOTEMPTY) {
            ...
            std::
            cerr <<
                 "rbd: error: image not fully moved to trash." <<
                 std::endl;
        } else if (r == -EBUSY) {
...
int do_list(librbd::RBD &rbd, librados::IoCtx &io_ctx,
            bool long_flag, bool all_flag, Formatter *f)
{
    ...
    if (f) {
        ...
        tbl.define_column("SOURCE", TextTable::LEFT,
                          TextTable::LEFT);
        tbl.define_column("DELETED_AT", TextTable::LEFT,
                          TextTable::LEFT);
        tbl.define_column("STATUS", TextTable::LEFT,
                          TextTable::LEFT);
        tbl.define_column("PARENT", TextTable::LEFT,
                          TextTable::LEFT);
    }
    ...
    for (const auto &entry : trash_entries) {
        ...
        r = rbd.open_by_id_read_only(io_ctx, im, entry.id.c_str(),
                                     NULL);
        ...
        if (r < 0) {
            if (r != -ENOENT) {
                std::cerr << "rbd: error opening " << entry.
                          id << ": " << cpp_strerror(r) << std::endl;
            }
        ...
        if (r == -ENOENT) {
            ...
            parent +=
                parent_image.image_name + "@" + parent_snap.name;
            has_parent = true;
        }
                 ...
                 &ceph_global_init_args)
{
    ...
    librbd::RBD rbd;
    r = do_list(rbd, io_ctx, vm["long"].as < bool > (),
                vm["all"].as < bool > (), formatter.get());
    if (r < 0) {
        std::cerr << "rbd: trash list: " << cpp_strerror(r) << std::
                  endl;
        return r;
    }
                  ...
                  &ceph_global_init_args)
{
    ...
    if (vm.find(THRESHOLD) != vm.end()) {
        ...
        if (vm.find(EXPIRED_BEFORE) != vm.end()) {
            utime_t new_time;
            r = utime_t::invoke_date(vm[EXPIRED_BEFORE].as <
                                     std::string > (), &new_time);
            if (r < 0) {
                std::
                cerr << "rbd: error calling /bin/date: " <<
    ...
    utils::ProgressContext pc("Removing images",
                              vm[at::NO_PROGRESS].as < bool > ());
    r = rbd.trash_purge_with_progress(io_ctx, expire_ts, threshold,
                                      pc);
    if (r < 0) {
        pc.fail();
                    ...
                    &ceph_global_init_args)
{
    ...
    if (r < 0) {
        if (r == -ENOENT) {
            ...
            std::
            cerr << "rbd: restore error: " << cpp_strerror(r) <<
                 std::endl;
        }
        return r;
    }
