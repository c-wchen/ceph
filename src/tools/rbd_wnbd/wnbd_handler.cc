...
int WnbdHandler::dump_stats(Formatter * f)
{
    ...
    if (err) {
        derr << "Failed to retrieve WNBD userspace stats. Error: " << err <<
            dendl;
        return -EINVAL;
    }
...
void WnbdHandler::aio_callback(librbd::completion_t cb, void *arg)
{
    ...
    if (ret == -EINVAL) {
        ...
        dout(0) << __func__ << ": masking IO out-of-bounds error" << *ctx <<
            dendl;
        ctx->data.clear();
        ret = 0;
    }
...
{
    std::unique_ptr < WnbdHandler::IOContext > pctx {
    ctx};
    ...
    err = WnbdSendResponseEx(pctx->handler->wnbd_disk,
                             &wnbd_rsp,
                             pctx->data.c_str(),
    ...
    if (err == ERROR_IO_PENDING) {
        ...
        if (!GetOverlappedResult(pctx->handler->wnbd_disk, overlapped,
                                 &returned_bytes, TRUE)) {
            err = GetLastError();
            derr << "Could not send response. Request id: " << wnbd_rsp.
                RequestHandle << ". Error: " << err << dendl;
        }
    }
}
...
void WnbdHandler::Read(PWNBD_DISK Disk,
                       UINT64 RequestHandle,
                       PVOID Buffer,
                       UINT64 BlockAddress,
                       UINT32 BlockCount, BOOLEAN ForceUnitAccess)
{
    ...
    handler->image.aio_read2(ctx->req_from, ctx->req_size, ctx->data, c,
                             op_flags);

    dout(20) << *ctx << ": submitted" << dendl;
}

void WnbdHandler::Write(PWNBD_DISK Disk,
                        UINT64 RequestHandle,
                        PVOID Buffer,
                        UINT64 BlockAddress,
                        UINT32 BlockCount, BOOLEAN ForceUnitAccess)
{
    ...
    librbd::RBD::AioCompletion * c =
        new librbd::RBD::AioCompletion(ctx, aio_callback);
    handler->image.aio_write2(ctx->req_from, ctx->req_size, ctx->data, c,
                              op_flags);

    dout(20) << *ctx << ": submitted" << dendl;
}

void WnbdHandler::Flush(PWNBD_DISK Disk,
                        UINT64 RequestHandle,
...
void WnbdHandler::Unmap(PWNBD_DISK Disk,
                        UINT64 RequestHandle,
                        PWNBD_UNMAP_DESCRIPTOR Descriptors, UINT32 Count)
{
    WnbdHandler *handler = nullptr;
    ceph_assert(!WnbdGetUserContext(Disk, (PVOID *) & handler));
    ceph_assert(1 == Count);
...
void WnbdHandler::LogMessage(WnbdLogLevel LogLevel,
                             const char *Message,
                             const char *FileName,
...
int WnbdHandler::start()
{
    ...
    instance_name.copy(wnbd_props.InstanceName,
                       sizeof(wnbd_props.InstanceName));
