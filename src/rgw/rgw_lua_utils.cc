...
// lua_push(lua_State* L, const ceph::real_time& tp)
    ...
    constexpr const char *RGWDebugLogAction {
    "RGWDebugLog"};

    int RGWDebugLog(lua_State * L) {
        auto cct =
            reinterpret_cast <
            CephContext * >(lua_touserdata(L, lua_upvalueindex(FIRST_UPVAL)));
    ...
    void stack_dump(lua_State * L) {
        ...
        for (int i = 1, j = -top; i <= top; i++, j++) {
            std::cout << "[" << i << "," << j << "]: " << luaL_tolstring(L, i,
                                                                         NULL)
                << std::endl;
            lua_pop(L, 1);
        }
    ...
    void set_package_path(lua_State * L, const std::string & install_dir) {
        ...
        const auto path =
            install_dir + "/share/lua/" + CEPH_LUA_VERSION + "/?.lua";
        pushstring(L, path);
        lua_setfield(L, -2, "path");

        const auto cpath =
            install_dir + "/lib/lua/" + CEPH_LUA_VERSION + "/?.so";
        pushstring(L, cpath);
