...
namespace jwt {
    ...
    namespace helper {
        inline
            std::string extract_pubkey_from_cert(const std::string & certstr,
                                                 const std::string & pw = "") {
            // TODO: Cannot find the exact version this change happended
#if OPENSSL_VERSION_NUMBER <= 0x1000114fL
            std::unique_ptr < BIO,
                decltype(&BIO_free_all) >
                certbio(BIO_new_mem_buf
                        (const_cast < char *>(certstr.data()), certstr.size()),
                        BIO_free_all);
#else
            std::unique_ptr < BIO,
                decltype(&BIO_free_all) >
                certbio(BIO_new_mem_buf(certstr.data(), certstr.size()),
                        BIO_free_all);
#endif
             std::unique_ptr < BIO,
            ...
            if (!cert)
             ...
             std::unique_ptr < EVP_PKEY,
                decltype(&EVP_PKEY_free) > key(X509_get_pubkey(cert.get()),
                                               EVP_PKEY_free);
            if (!key)
                throw
                    rsa_exception("Error getting public key from certificate");
            if (!PEM_write_bio_PUBKEY(keybio.get(), key.get()))
                throw
                    rsa_exception
                    ("Error writing public key data in PEM format");
            char *ptr = nullptr;
            ...
            std::unique_ptr < BIO,
                decltype(&BIO_free_all) > pubkey_bio(BIO_new(BIO_s_mem()),
                                                     BIO_free_all);
            if (key.substr(0, 27) == "-----BEGIN CERTIFICATE-----") {
                auto epkey = helper::extract_pubkey_from_cert(key, password);
                if ((size_t)
                    BIO_write(pubkey_bio.get(), epkey.data(),
                              epkey.size()) != epkey.size())
                    throw
                        rsa_exception
            ...
            std::shared_ptr < EVP_PKEY >
                pkey(PEM_read_bio_PUBKEY
                     (pubkey_bio.get(), nullptr, nullptr,
                      (void *)password.c_str()), EVP_PKEY_free);
            if (!pkey)
                throw
                    rsa_exception
        ...
        inline
            ...
            load_private_key_from_string(const std::string & key,
                                         const std::string & password = "") {
            std::unique_ptr < BIO,
                decltype(&BIO_free_all) > privkey_bio(BIO_new(BIO_s_mem()),
                                                      BIO_free_all);
    ...
    namespace algorithm {
        ...
        struct none {
            ...
            /// Get algorithm name std::string name() const {
                return "none";
        }};
        /**
        ...
        struct hmacsha {
			 ...
			 */
            hmacsha(std::string key, const EVP_MD * (*md) (),
        ...
        struct rsa {
            ...
            :md(md), alg_name(name) {
                if (!private_key.empty()) {
                    pkey =
                        helper::load_private_key_from_string(private_key,
                                                             private_key_password);
                }
                else if (!public_key.empty()) {
                    pkey =
                        helper::load_public_key_from_string(public_key,
                                                            public_key_password);
                }
                else
                    throw
        ...
        struct ecdsa {
            ...
            :md(md), alg_name(name), signature_length(siglen) {
                if (!public_key.empty()) {
                    std::unique_ptr < BIO,
                        decltype(&BIO_free_all) >
                        pubkey_bio(BIO_new(BIO_s_mem()), BIO_free_all);
                    if (public_key.substr(0, 27) ==
                        "-----BEGIN CERTIFICATE-----") {
                        auto epkey =
                            helper::extract_pubkey_from_cert(public_key,
                                                             public_key_password);
                        if ((size_t)
                            BIO_write(pubkey_bio.get(), epkey.data(),
                                      epkey.size()) != epkey.size())
                        ...
                        if ((size_t)
                            BIO_write(pubkey_bio.get(), public_key.data(),
                    ...
                    size_t keysize =
                        EC_GROUP_get_degree(EC_KEY_get0_group(pkey.get()));
                ...
                if (!private_key.empty()) {
                    std::unique_ptr < BIO,
                        decltype(&BIO_free_all) >
                        privkey_bio(BIO_new(BIO_s_mem()), BIO_free_all);
                    if ((size_t)
                        BIO_write(privkey_bio.get(), private_key.data(),
                                  private_key.size()) != private_key.size())
                        throw
                            rsa_exception
                            ("failed to load private key: bio_write failed");
                    pkey.
                        reset(PEM_read_bio_ECPrivateKey
                              (privkey_bio.get(), nullptr, nullptr,
                               const_cast <
                               char *>(private_key_password.c_str())),
                              EC_KEY_free);
                    if (!pkey)
                        throw
                            rsa_exception
            ...
            std::string sign(const std::string & data) const {
                ...
                auto rs = bn2raw(s);
#endif
                if (rr.size() > signature_length / 2
                    || rs.size() > signature_length / 2)
                    throw std::
                        logic_error("bignum size exceeded expected length");
			 ...
			 */
                ...
                if (ECDSA_do_verify
                    ((const unsigned char *)hash.data(), hash.size(), &sig,
                     pkey.get()) != 1)
                    throw signature_verification_exception("Invalid signature");
#else
                 std::unique_ptr < ECDSA_SIG,
                    decltype(&ECDSA_SIG_free) > sig(ECDSA_SIG_new(),
                                                    ECDSA_SIG_free);
			 ...
			 */
            static std::unique_ptr < BIGNUM,
                decltype(&BN_free) > raw2bn(const std::string & raw) {
                return std::unique_ptr < BIGNUM,
                    decltype(&BN_free) >
                    (BN_bin2bn
                     ((const unsigned char *)raw.data(), raw.size(), nullptr),
                     BN_free);
            }
            ...
            std::string generate_hash(const std::string & data) const {
                ...
                if (EVP_DigestInit(ctx.get(), md()) == 0)
                    throw
                        signature_generation_exception("EVP_DigestInit failed");
                ...
                if (EVP_DigestFinal
                    (ctx.get(), (unsigned char *)res.data(), &len) == 0)
                    throw
                        signature_generation_exception
            ...
            /// OpenSSL struct containing keys std::shared_ptr < EC_KEY > pkey;
        ...
        struct pss {
            ...
            std::string sign(const std::string & data) const {
                auto hash = this->generate_hash(data);
                 ...
                 std::unique_ptr < RSA,
                    decltype(&RSA_free) > key(EVP_PKEY_get1_RSA(pkey.get()),
                                              RSA_free);
        ...
        struct rs256:public rsa {
            ...
            explicit rs256(const std::string & public_key,
                           ...
                           "", const std::string & public_key_password =
                           "", const std::string & private_key_password = "")
          :    rsa(public_key, private_key, public_key_password,
                    private_key_password, EVP_sha256, "RS256") {
        }};
        /**
		 * RS384 algorithm
		 */
        struct rs384:public rsa {
            ...
            explicit rs384(const std::string & public_key,
                           ...
                           "", const std::string & private_key_password = "")
          :    rsa(public_key, private_key, public_key_password,
                    private_key_password, EVP_sha384, "RS384") {
        }};
        /**
		 * RS512 algorithm
		 */
        struct rs512:public rsa {
            ...
            explicit rs512(const std::string & public_key,
                           ...
                           "", const std::string & private_key_password = "")
          :    rsa(public_key, private_key, public_key_password,
                    private_key_password, EVP_sha512, "RS512") {
        }};
        /**
		 * ES256 algorithm
		 */
        ...
        struct ps256:public pss {
            ...
            explicit ps256(const std::string & public_key,
                           ...
                           "", const std::string & public_key_password =
                           "", const std::string & private_key_password = "")
          :    pss(public_key, private_key, public_key_password,
                    private_key_password, EVP_sha256, "PS256") {
        }};
        /**
		 * PS384 algorithm
		 */
        struct ps384:public pss {
            ...
            explicit ps384(const std::string & public_key,
                           ...
                           "", const std::string & private_key_password = "")
          :    pss(public_key, private_key, public_key_password,
                    private_key_password, EVP_sha384, "PS384") {
        }};
        /**
		 * PS512 algorithm
		 */
        struct ps512:public pss {
            ...
            explicit ps512(const std::string & public_key,
                           ...
                           "", const std::string & private_key_password = "")
          :    pss(public_key, private_key, public_key_password,
                    private_key_password, EVP_sha512, "PS512") {
        }};
    }
    ...
    class payload {
      ...
      public:
		 ...
		 */
            ...
            claim > get_payload_claims() const {
    ...
    class decoded_jwt:public header, public payload {
      ...
      public:
        ...
        :token(token) {
            ...
            auto fix_padding =[](std::string & str) {
                ...
                case 1:
                                         ...
                                         header =
                                         base::decode < alphabet::base64url >
                                         (header);
                                         payload =
                                         base::decode < alphabet::base64url >
                                         (payload);
                                         ...
                                         std::unordered_map < std::string,
                                         claim > res; picojson::value val;
                                         if (!picojson::parse(val, str).empty())
              throw std::runtime_error("Invalid json"); for (auto & e:val.get < picojson::object > ())
                                         {
                                         res.insert( {
                                                    e.first, claim(e.second)}
                                         );}
		 ...
		 */
                                         const std::
                                         string & get_token()const noexcept {
                                         return token;}
        /**
		 ...
		 */
                                         const std::
                                         string & get_header()const noexcept {
                                         return header;}
        /**
		 ...
		 */
                                         const std::
                                         string & get_signature()const noexcept
                                         {
                                         return signature;}
        /**
		 ...
		 */
                                         const std::
                                         string & get_header_base64()const
                                         noexcept {
		 ...
		 */
                                         const std::
                                         string & get_payload_base64()const
                                         noexcept {
		 ...
		 */
                                         const std::
                                         string & get_signature_base64()const
                                         noexcept {
	 ...
	 */
                                         ...
                                         claim > header_claims;
                                         std::unordered_map < std::string,
                                         claim > payload_claims; builder() {
                                         }
      friend builder create(); public:
        /**
		 * Set a header claim.
		 ...
		 */
                                         builder & set_header_claim(const std::
                                                                    string & id,
                                                                    claim c) {
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_payload_claim(const std::
                                                                     string &
                                                                     id,
                                                                     claim c) {
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_algorithm(const std::
                                                                 string & str) {
                                         return set_header_claim("alg",
                                                                 claim(str));}
        /**
		 * Set type claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_type(const std::
                                                            string & str) {
                                         return set_header_claim("typ",
                                                                 claim(str));}
        /**
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_content_type(const std::
                                                                    string &
                                                                    str) {
                                         return set_header_claim("cty",
                                                                 claim(str));}
        /**
		 * Set key id claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_key_id(const std::
                                                              string & str) {
                                         return set_header_claim("kid",
                                                                 claim(str));}
        /**
		 * Set issuer claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_issuer(const std::
                                                              string & str) {
                                         return set_payload_claim("iss",
                                                                  claim(str));}
        /**
		 * Set subject claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_subject(const std::
                                                               string & str) {
                                         return set_payload_claim("sub",
                                                                  claim(str));}
        /**
		 * Set audience claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_audience(const std::set <
                                                                std::string >
                                                                &l) {
                                         return set_payload_claim("aud",
                                                                  claim(l));}
        /**
		 * Set audience claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_audience(const std::
                                                                string & aud) {
                                         return set_payload_claim("aud",
                                                                  claim(aud));}
        /**
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_expires_at(const date &
                                                                  d) {
                                         return set_payload_claim("exp",
                                                                  claim(d));}
        /**
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_not_before(const date &
                                                                  d) {
                                         return set_payload_claim("nbf",
                                                                  claim(d));}
        /**
		 * Set issued at claim
		 ...
		 * \return *this to allow for method chaining
		 */
                                         builder & set_id(const std::
                                                          string & str) {
                                         return set_payload_claim("jti",
                                                                  claim(str));}
		 ...
		 */
          ...
          picojson::object obj_header; obj_header["alg"] = picojson::value(algo.name()); for (auto & e:header_claims)
                                         {
                                         obj_header[e.first] =
                                         e.second.to_json();}
          picojson::object obj_payload; for (auto & e:payload_claims)
                                         {
                                         obj_payload.insert( {
                                                            e.first,
                                                            e.second.to_json()}
                                         ...
                                         auto base =
                                         base::encode < alphabet::base64url >
                                         (data);
                                         auto pos =
                                         base.find(alphabet::base64url::fill());
                                         base = base.substr(0, pos);
                                         return base;};
                                         std::string header =
                                         encode(picojson::value(obj_header).
                                                serialize());
                                         std::string payload =
                                         encode(picojson::value(obj_payload).
                                                serialize());
                                         std::string token =
                                         header + "." + payload;
                                         return token + "." +
                                         encode(algo.sign(token));}
                                         };
    /**
	 ...
	 */
      ...
      virtual void verify(const std::string & data, const std::string & sig) = 0;}; template < typename T > struct algo:public algo_base
                                         {
          T alg; explicit algo(T a):    alg(a)
                                         {
                                         }
                                         virtual void verify(const std::
                                                             string & data,
                                                             const std::
                                                             string & sig)
                                         override {
      ...
      std::unordered_map < std::string, std::shared_ptr < algo_base >> algs; public:
        /**
      ...
      explicit verifier(Clock c):       clock(c)
		 ...
		 */
                                         ...
                                         default_leeway = leeway; return *this;}
        /**
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & expires_at_leeway(size_t
                                                                      leeway) {
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & not_before_leeway(size_t
                                                                      leeway) {
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & issued_at_leeway(size_t
                                                                     leeway) {
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & with_issuer(const std::
                                                                string & iss) {
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & with_subject(const std::
                                                                 string & sub) {
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & with_audience(const std::
                                                                  set <
                                                                  std::string >
                                                                  &aud) {
		 ...
		 * \return *this to allow chaining
		 */
                                         verifier & with_claim(const std::
                                                               string & name,
                                                               claim c) {
                                         claims[name] = c; return *this;}
		 ...
		 */
                                         ...
                                         verifier & allow_algorithm(Algorithm
                                                                    alg) {
                                         algs[alg.name()] =
                                         std::make_shared < algo <
                                         Algorithm >> (alg); return *this;}
		 ...
		 */
                                         ...
                                         if (c.get_type() == claim::type::array) {
                                         auto s1 = c.as_set();
                                         auto s2 = jc.as_set();
                                         if (s1.size() != s2.size())
                                         throw
                                         ...
                                         to_time_t(claims.at("exp").
                                                   as_date()) : default_leeway;
                                         auto exp = jwt.get_expires_at();
                                         if (time >
                                             exp + std::chrono::seconds(leeway))
                                         throw
                                         token_verification_exception
                                         ("token expired");}
                                         ...
                                         claims.count("iat") ==
                                         1 ? std::chrono::system_clock::
                                         to_time_t(claims.at("iat").
                                                   as_date()) : default_leeway;
                                         auto iat = jwt.get_issued_at();
                                         if (time <
                                             iat - std::chrono::seconds(leeway))
                                         throw
                                         token_verification_exception
                                         ("token expired");}
                                         ...
                                         claims.count("nbf") ==
                                         1 ? std::chrono::system_clock::
                                         to_time_t(claims.at("nbf").
                                                   as_date()) : default_leeway;
                                         auto nbf = jwt.get_not_before();
                                         if (time <
                                             nbf - std::chrono::seconds(leeway))
                                         throw
                                         token_verification_exception
                                         ("token expired");}
          for (auto & c:                claims) {
                                         ...
                                         if (c.first == "aud") {
                                         if (!jwt.has_audience())
              throw token_verification_exception("token doesn't contain the required audience"); auto aud = jwt.get_audience(); auto expected = c.second.as_set(); for (auto & e:expected)
                                         if (aud.count(e) == 0)
                                         throw
                                         token_verification_exception
                                         ("token doesn't contain the required audience");}
                                         else {
                                         assert_claim_eq(jwt, c.first,
                                                         c.second);}
                                         }
                                         }
                                         };
