...
seastar::future <> InternalClientRequest::start()
{
    ...
    return crimson::common::handle_system_shutdown([this] {
        ...
        repeat([this] {
            ...
            return
            interruptor::
            with_interruption([this]()mutable {
                return
                enter_stage
                <
                interruptor
                ...
                ([this]()->PG::load_obc_iertr::future <> {
                     ...
                     mutable
                    {
                        ...
                        ().
                        debug
                        ("InternalClientRequest: got {} OSDOps to execute",
                         std::
                         size
                         (osd_ops));
                        [[maybe_unused]] const int ret = op_info.set_from_op(std::as_const(osd_ops), pg->get_pgid().pgid, *pg->get_osdmap()); assert(ret == 0); return pg->with_locked_obc(get_target_oid(), op_info, [&osd_ops, this](auto obc) {
                            return
                            enter_stage
                            <
                            interruptor
