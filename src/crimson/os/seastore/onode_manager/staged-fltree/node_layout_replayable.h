 ...
 */
    template < typename FieldType, node_type_t NODE_TYPE > struct NodeLayoutReplayableT {
        ...
        template < KeyT KT >
            static const value_t *insert(NodeExtentMutable & mut,
            ...
            auto p_value =
                stage_t::template proceed_insert < KT, false > (mut, node_stage,
                                                                key, value,
                                                                insert_pos,
                                                                insert_stage,
                                                                insert_size);
            return p_value;
        } static void split(NodeExtentMutable & mut,
                            const node_stage_t & node_stage,
                            StagedIterator & split_at) {
            node_stage_t::update_is_level_tail(mut, node_stage, false);
            stage_t::trim(mut, split_at);
        } template < KeyT KT >
            static const value_t *split_insert(NodeExtentMutable & mut,
                                               const node_stage_t & node_stage,
                                               StagedIterator & split_at,
                                               const full_key_t < KT > &key,
                                               const value_input_t & value,
            ...
            stage_t::trim(mut, split_at);
            auto p_value =
                stage_t::template proceed_insert < KT, true > (mut, node_stage,
                                                               key, value,
                                                               insert_pos,
                                                               insert_stage,
                                                               insert_size);
        ...
        static void update_child_addr(NodeExtentMutable & mut,
        ...
        static std::tuple < match_stage_t,
            position_t > erase(NodeExtentMutable & mut,
                               const node_stage_t & node_stage,
            ...
            return {
            erase_stage, erase_pos};
        }

        static position_t make_tail(NodeExtentMutable & mut,
                                    const node_stage_t & node_stage) {
            assert(!node_stage.is_level_tail());
            if constexpr
                (NODE_TYPE == node_type_t::INTERNAL) {
                auto[r_stage, r_last_pos] =
                    update_last_to_tail(mut, node_stage);
                std::ignore = r_stage;
                return r_last_pos;
                }
            else {
      ...
      private:
        static std::tuple < match_stage_t,
            position_t > update_last_to_tail(NodeExtentMutable & mut,
                                             const node_stage_t & node_stage) {
            if constexpr
                (NODE_TYPE == node_type_t::INTERNAL) {
                assert(node_stage.keys() != 0);
                ...
                laddr_t last_value;
                {
                    ...
                    stage_t::template get_largest_slot < true, false,
                        true > (node_stage, &last_pos, nullptr, &p_last_value);
                ...
                auto p_last_value =
                    const_cast <
                    laddr_packed_t * >(node_stage.get_end_p_laddr());
                mut.copy_in_absolute(p_last_value, last_value);
                // return erase_stage, last_pos
                return {
                erase_stage, last_pos};
                }
            else {
