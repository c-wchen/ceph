...
namespace crimson::os::seastore::onode {
    ...
    template < typename ValueImpl > class Btree {
      ...
      :    nm {
        std::move(_nm)}
        , root_tracker {
        RootNodeTracker::create(nm->is_read_isolated())}
        {
        }
        ~Btree() {
        ...
        class Cursor {
          public:
            ...
            ghobject_t get_ghobj() const {
                assert(!is_end());
                auto view =
                    p_cursor->get_key_view(p_tree->value_builder.
                                           get_header_magic());
                ...
                assert(!is_end());
                return p_tree->value_builder.build_value(*p_tree->nm,
                                                         p_tree->value_builder,
                                                         p_cursor);
            }
            ...
            bool operator==(const Cursor & o)const {
                ...
                return p_cursor->get_next(p_tree->get_context(t)
                              ...
                              next_cursor->assert_next_to(*this_obj.p_cursor,
                                                          this_obj.p_tree->
                                                          value_builder.
                                                          get_header_magic());
                              auto ret = Cursor {
                              this_obj.p_tree, next_cursor};
                              assert(this_obj < ret); return ret;}
                );
            }
            ...
            template < bool FORCE_MERGE = false >
                ...
                assert(!is_end());
                auto this_obj = *this;
                return p_cursor->erase < FORCE_MERGE > (p_tree->get_context(t),
                                                        true).si_then([this_obj,
                                                                       this]
                                                                      (Ref <
                                                                       tree_cursor_t
                                                                       >
          ...
          Cursor(Btree * p_tree):p_tree {
            ...
            std::strong_ordering operator<=>(const Cursor & o) const {
                ...
                return p_cursor->compare_to(*o.p_cursor,
                                            p_tree->value_builder.
                                            get_header_magic());
        ...
        eagain_ifuture < bool > contains(Transaction & t,
                                         const ghobject_t & obj) {
            return seastar::do_with(key_hobj_t {
                                    obj}
                                    ,
                                    [this,
                                     &t] (auto & key)->eagain_ifuture < bool > {
                                    return get_root(t).
                                    si_then([this, &t, &key] (auto root) {
                                            // TODO: improve lower_bound()
                                            return root->
                                            lower_bound(get_context(t), key);}
                                    ).si_then([](auto result) {
        ...
        eagain_ifuture < Cursor > find(Transaction & t, const ghobject_t & obj) {
            return seastar::do_with(key_hobj_t {
                                    ...
                                    ,
                                    [this,
                                     &t] (auto & key)->eagain_ifuture < Cursor >
                                    {
                                    return get_root(t).
                                    si_then([this, &t, &key] (auto root) {
                                            // TODO: improve lower_bound()
   ...
   * doesn't exist.
   */
        eagain_ifuture < Cursor > lower_bound(Transaction & t,
                                              const ghobject_t & obj) {
            return seastar::do_with(key_hobj_t {
                                    obj}
                                    ,
        ...
        insert_iertr::future < std::pair < Cursor, bool >>
            insert(Transaction & t, const ghobject_t & obj,
                   tree_value_config_t _vconf) {
            LOG_PREFIX(OTree::insert);
            if (_vconf.payload_size >
                value_builder.get_max_value_payload_size()) {
                SUBERRORT(seastore_onode,
                          "value payload size {} too large to insert {}", t,
                          _vconf.payload_size, key_hobj_t {
                          obj}
                );
            ...
            if (obj.hobj.nspace.size() > value_builder.get_max_ns_size()) {
                SUBERRORT(seastore_onode,
                          "namespace size {} too large to insert {}", t,
                          obj.hobj.nspace.size(), key_hobj_t {
                          obj}
                );
                return crimson::ct_error::value_too_large::make();
            }
            if (obj.hobj.oid.name.size() > value_builder.get_max_oid_size()) {
                SUBERRORT(seastore_onode, "oid size {} too large to insert {}",
                          t, obj.hobj.oid.name.size(), key_hobj_t {
                          obj}
                );
                return crimson::ct_error::value_too_large::make();
            }
            value_config_t vconf {
            value_builder.get_header_magic(), _vconf.payload_size};
            return seastar::do_with(key_hobj_t {
                                    obj}
                                    ,
                                    [this, &t,
                                     vconf] (auto & key)->eagain_ifuture <
                                    std::pair < Cursor, bool >> {
                                    ceph_assert(key.is_valid());
                                    return get_root(t).
                                    si_then([this, &t, &key,
                                             vconf] (auto root) {
                                            return root->insert(get_context(t),
                                                                key, vconf,
                                                                std::
                                                                move(root));}
                                              ...
                                              return std::
                                              make_pair(Cursor(this, cursor),
                                                        success);}
                                    );}
            );
        }

        eagain_ifuture < std::size_t > erase(Transaction & t,
                                             const ghobject_t & obj) {
            return seastar::do_with(key_hobj_t {
                                    obj}
                                    ,
                                    [this,
                                     &t] (auto & key)->eagain_ifuture <
                                    std::size_t > {
                                    return get_root(t).
                                    si_then([this, &t, &key] (auto root) {
                                            return root->erase(get_context(t),
                                                               key,
                                                               std::
                                                               move(root));}
                                    );}
        ...
        eagain_ifuture <> erase(Transaction & t, Value & value) {
            ...
            return ref_cursor->erase(get_context(t),
                                     false).
                si_then([ref_cursor] (auto next_cursor) {
                        assert(ref_cursor->is_invalid()); assert(!next_cursor);}
            );
        }
        ...
        } eagain_ifuture <> test_clone_from(Transaction & t,
                                            Transaction & t_from,
                                            Btree & from) {
