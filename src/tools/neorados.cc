            ...
            s::yield_context y)
{
    ...
    if (ec)
        throw bs::system_error(ec,
                               fmt::
                               format("when creating object '{}' in pool '{}'",
                                      obj, pname));
}
    ...
    std::uint64_t len;
    {
        ...
        if (ec)
            throw bs::system_error(ec,
                                   fmt::
                                   format
                                   ("when getting length of object '{}' in pool '{}'",
                                    obj, pname));
    }
    ...
    while (auto toread = std::max(len - off, io_size)) {
        ...
        if (ec)
            throw bs::system_error(ec,
                                   fmt::
                                   format
                                   ("when reading from object '{}' in pool '{}'",
                                    obj, pool));
...
void rm(R::RADOS &r, const std::vector < std::string > &p, s::yield_context y)
{
    ...
    if (ec)
        throw bs::system_error(ec,
                               fmt::
                               format("when removing object '{}' in pool '{}'",
                                      obj, pname));
}
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        desc.add_options()
            ...
            ("parameters",
             po::value < std::vector < std::string >> (&parameters),
