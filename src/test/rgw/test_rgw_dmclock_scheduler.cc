...
TEST(Queue, SyncRequest)
{
    ...
    auto now = get_time();
    ready = true;
    queue.add_request(client_id::admin, {
    }
    , now, 1);
    queue.add_request(client_id::auth, {
    }
    , now, 1);
    ...
    EXPECT_EQ(0u,
              counters(client_id::admin)->get(queue_counters::l_cancel));
...
TEST(Queue, RateLimit)
{
    ...
    ClientCounters counters(g_ceph_context);
    AsyncScheduler queue(g_ceph_context, context, std::ref(counters),
    nullptr, [](client_id client)->ClientInfo * {
        static ClientInfo clients[] = {
            {
                1, 1, 1
            },  // admin
    ...
    auto now = get_time();
    queue.async_request(client_id::admin, {
    }
    , now, 1, capture(ec1, p1));
    queue.async_request(client_id::admin, {
    }
    , now, 1, capture(ec2, p2));
    queue.async_request(client_id::auth, {
    }
    , now, 1, capture(ec3, p3));
    queue.async_request(client_id::auth, {
    }
    , now, 1, capture(ec4, p4));
    EXPECT_FALSE(ec1);
    EXPECT_FALSE(ec2);
    EXPECT_FALSE(ec3);
    EXPECT_FALSE(ec4);
...
TEST(Queue, AsyncRequest)
{
    ...
    nullptr, [](client_id client)->ClientInfo * {
        static ClientInfo clients[] = {
            {
                1, 1, 1
            },  // admin: satisfy by reservation
            {
                0, 1, 1
            },  // auth: satisfy by priority
        };
        return &clients[static_cast < size_t > (client)];
    }
    , AtLimit::Reject);
...
TEST(Queue, CancelOnDestructor)
{
    ...
    ClientCounters counters(g_ceph_context);
    {
        ...
        EXPECT_EQ(1u,
                  counters(client_id::admin)->get(queue_counters::l_qlen));
        EXPECT_EQ(1u,
                  counters(client_id::auth)->get(queue_counters::l_qlen));
    }
...
TEST(Queue, CrossExecutorRequest)
{
    ...
    ClientCounters counters(g_ceph_context);
    AsyncScheduler queue(g_ceph_context, queue_context, std::ref(counters),
    nullptr, [](client_id client)->ClientInfo * {
        static ClientInfo info {
            0, 1, 1}; return &info;
    }
    ...
    , now, 1, capture(ex2, ec1, p1));
    queue.async_request(client_id::auth, {
    }
    , now, 1, capture(ex2, ec2, p2));
...
TEST(Queue, SpawnAsyncRequest)
{
    ...
    spawn::spawn(context, [&](yield_context yield) {
        ClientCounters counters(g_ceph_context);
        AsyncScheduler queue(g_ceph_context, context,
                             std::ref(counters), nullptr,
        [](client_id client)->ClientInfo * {
            ...
            return &clients[static_cast < size_t >
                            (client)];
        }
                            );
        error_code ec1, ec2;
        auto p1 = queue.async_request(client_id::admin, {
        }
        , get_time(), 1, yield[ec1]);
        EXPECT_EQ(boost::system::errc::success, ec1);
        EXPECT_EQ(PhaseType::reservation, p1);
        auto p2 = queue.async_request(client_id::auth, {
        }
        , get_time(), 1, yield[ec2]);
        EXPECT_EQ(boost::system::errc::success, ec2);
        EXPECT_EQ(PhaseType::priority, p2);
    }
                );
