...
void netmask_ipv4(const struct in_addr *addr,
                  unsigned int prefix_len, struct in_addr *out)
{
    uint32_t mask;
    ...
    case AF_INET:
        {
            ...
            netmask_ipv4(&((const sockaddr_in *)addr.get_sockaddr())->sin_addr,
                         prefix_len, &b);
    ...
    case AF_INET6:
        {
            ...
            netmask_ipv6(&((const sockaddr_in6 *)addr.get_sockaddr())->
                         sin6_addr, prefix_len, &b);
            if (memcmp(&a, &b, sizeof(a)) == 0) {
                return true;
            }
        }
