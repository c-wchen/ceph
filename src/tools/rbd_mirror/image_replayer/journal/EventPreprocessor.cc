...
template < typename I >
EventPreprocessor <
I >::EventPreprocessor(I &local_image_ctx,
...
{
} template < typename I > EventPreprocessor <
I >::~EventPreprocessor()
{
    ceph_assert(!m_in_progress);
} template < typename I >
bool EventPreprocessor <
I >::is_required(const EventEntry &event_entry)
{
...
void EventPreprocessor <
I >::preprocess(EventEntry *event_entry,
                Context *on_finish)
{
    ceph_assert(!m_in_progress);
    m_in_progress = true;
...
void EventPreprocessor < I >::refresh_image()
{
    ...
    Context *ctx = create_context_callback <
                   EventPreprocessor < I >,
                   &EventPreprocessor < I >::handle_refresh_image > (this);
...
void EventPreprocessor < I >::handle_refresh_image(int r)
{
    ...
    if (r < 0) {
        derr << "error encountered during image refresh: " <<
             cpp_strerror(r)
             << dendl;
        finish(r);
        return;
...
template < typename I >
int EventPreprocessor <
I >::preprocess_snap_rename(librbd::journal::
                            SnapRenameEvent &event)
{
    dout(20) << "remote_snap_id=" << event.
             snap_id << ", " << "src_snap_name=" << event.
             src_snap_name << ", " << "dest_snap_name=" << event.
             dst_snap_name << dendl;

    auto snap_seq_it = m_snap_seqs.find(event.snap_id);
    if (snap_seq_it != m_snap_seqs.end()) {
        dout(20) << "remapping remote snap id " << snap_seq_it->
                 first << " " << "to local snap id " << snap_seq_it->
                 second << dendl;
    ...
    if (snap_id_it == m_local_image_ctx.snap_ids.end()) {
        dout(20) << "cannot map remote snapshot '" << event.
                 src_snap_name << "' " << "to local snapshot" <<
                 dendl;
        event.snap_id = CEPH_NOSNAP;
        return -ENOENT;
    }

    dout(20) << "mapping remote snap id " << event.
             snap_id << " " << "to local snap id " << snap_id_it->
             second << dendl;
...
void EventPreprocessor < I >::update_client()
{
    ...
    Context *ctx = create_context_callback <
                   EventPreprocessor < I >,
                   &EventPreprocessor < I >::handle_update_client > (this);
...
template < typename I >
bool EventPreprocessor <
I >::prune_snap_map(SnapSeqs *snap_seqs)
{
    bool pruned = false;
    ...
    for (auto it = snap_seqs->begin(); it != snap_seqs->end();) {
        auto current_it(it++);
        if (m_local_image_ctx.snap_info.
            count(current_it->second) == 0) {
