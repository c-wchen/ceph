    ...
    namespace image {
        ...
        template < typename I >
            ...
            I >::is_open_required(I & child_image_ctx,
                                  const ParentImageInfo & parent_md,
                                  const MigrationInfo & migration_info) {
            return (does_parent_exist
                    (child_image_ctx, parent_md, migration_info)
        ...
        template < typename I >
            bool RefreshParentRequest <
            I >::does_parent_exist(I & child_image_ctx,
                                   const ParentImageInfo & parent_md,
                                   const MigrationInfo & migration_info) {
            if (child_image_ctx.child != nullptr
                && child_image_ctx.child->migration_info.empty()
                && parent_md.overlap == 0) {
        ...
        template < typename I > void RefreshParentRequest < I >::send() {
            if (is_open_required
                (m_child_image_ctx, m_parent_md, m_migration_info)) {
                send_open_parent();
            }
            else {
        ...
        template < typename I >
            ...
            if (!m_migration_info.empty()) {
                auto ctx =
                    create_async_context_callback(m_child_image_ctx,
                                                  ...
                                                  &RefreshParentRequest <
                                                  I >::handle_open_parent,
                                                  false > (this));
                auto req =
                    migration::OpenSourceImageRequest <
                    I >::create(m_child_image_ctx.md_ctx, &m_child_image_ctx,
            ...
            m_parent_image_ctx = new I("", m_parent_md.spec.image_id,
                                       m_parent_md.spec.snap_id, parent_io_ctx,
                                       true);
        ...
        template < typename I >
            ...
            Context *ctx =
                create_async_context_callback(m_child_image_ctx,
                                              ...
                                              RefreshParentRequest < I >,
                                              &RefreshParentRequest <
                                              I >::handle_reset_existence_cache,
                                              false > (this));
        ...
        template < typename I >
            Context * RefreshParentRequest <
            I >::handle_reset_existence_cache(int *result) {
            CephContext *cct = m_child_image_ctx.cct;
            ldout(cct,
                  10) << this << " " << __func__ << " r=" << *result << dendl;
