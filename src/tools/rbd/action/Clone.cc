    ...
    namespace action {
        namespace clone {
            ...
            int do_clone(librbd::RBD & rbd, librados::IoCtx & p_ioctx,
                ...
                return rbd.clone3(p_ioctx, p_name, p_snapname, c_ioctx, c_name,
                                  opts);
                ...
                librbd::RBD rbd;
                r = do_clone(rbd, io_ctx, image_name.c_str(), snap_name.c_str(),
                             dst_io_ctx, dst_image_name.c_str(), opts);
                if (r == -EXDEV) {
                    std::
                        cerr <<
                        "rbd: clone v2 required for cross-namespace clones." <<
                        std::endl;
                    return r;
                }
                else if (r < 0) {
                    std::
                        cerr << "rbd: clone error: " << cpp_strerror(r) << std::
                        endl;
                    return r;
                }
                return 0;
