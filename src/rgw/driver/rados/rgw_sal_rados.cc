                                       ...
                                       std::unique_ptr<User> *user)
{
    ...
    int r = ctl()->user->get_info_by_access_key(dpp, key, &uinfo, y,
...
std::unique_ptr<Notification> RadosStore::get_notification(const DoutPrefixProvider *dpp, rgw::sal::Object *obj,
        rgw::sal::Object *src_obj, rgw::notify::EventType event_type, rgw::sal::Bucket *_bucket, std::string &_user_id,
        std::string &_user_tenant, std::string &_req_id, optional_yield y)
{
    return std::make_unique<RadosNotification>(dpp, this, obj, src_obj, event_type, _bucket, _user_id, _user_tenant,
            _req_id, y);
}
...
int RadosObject::transition(Bucket *bucket,
                            ...
                            optional_yield y)
{
    return store->getRados()->transition_obj(*rados_ctx, bucket->get_info(), get_obj(), placement_rule, mtime, olh_epoch,
            dpp, y);
}
                                 ...
                                 optional_yield y)
{
    ...
    using ceph::encode;
    encode(*this, bl);
    return rgw_put_system_obj(dpp, sysobj, store->svc()->zone->get_zone_params().oidc_pool, oid, bl, exclusive, nullptr,
                              real_time(), y);
}
