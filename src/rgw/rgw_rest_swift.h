...
class RGWListBuckets_ObjStore_SWIFT: public RGWListBuckets_ObjStore
{
    ...
    RGWListBuckets_ObjStore_SWIFT()
        : need_stats(true), wants_reversed(false)
    {
    ...
    bool should_get_stats() override
    {
        return need_stats;
    }
    bool supports_account_metadata() override
    {
        return true;
    }
};
...
public:
    ...
    bool need_container_stats() override
    {
        return true;
    }
};
...
protected:
    struct info {
        bool is_admin_info;
        std::function < void (Formatter &, const ConfigProxy &,
                              rgw::sal::Driver *) > list_data;
    };

    static const std::vector < std::pair < std::string,
           struct info >>swift_info;
public:
    ...
    void execute(optional_yield y) override;
    void send_response() override;
    static void list_swift_data(Formatter &formatter,
                                const ConfigProxy &config,
                                rgw::sal::Driver *driver);
    static void list_tempauth_data(Formatter &formatter,
                                   const ConfigProxy &config,
                                   rgw::sal::Driver *driver);
    static void list_tempurl_data(Formatter &formatter,
                                  const ConfigProxy &config,
                                  rgw::sal::Driver *driver);
    static void list_slo_data(Formatter &formatter, const ConfigProxy &config,
                              rgw::sal::Driver *driver);
...
public:
    ...
    void init(rgw::sal::Driver *driver,
              req_state *s, RGWHandler *dialect_handler) override;
...
public:
    RGWSwiftWebsiteHandler(rgw::sal::Driver *const driver,
                           req_state *const s, RGWHandler_REST *const handler)
        : driver(driver), s(s), handler(handler)
    {
    } int error_handler(const int err_no,
                        std::string *const error_content, optional_yield y);
...
public:
    ...
    int init(rgw::sal::Driver *driver, req_state *s,
...
public:
    ...
    int init(rgw::sal::Driver *const driver,
             req_state *const s, rgw::io::BasicClient *const cio) override
    {
        website_handler =
            boost::in_place < RGWSwiftWebsiteHandler > (driver, s, this);
...
class RGWGetCrossDomainPolicy_ObjStore_SWIFT: public
    RGWGetCrossDomainPolicy_ObjStore
{
public:
