...
class MWatchNotify final:public Message {
  ...
  public:
    ...
    uint64_t notifier_gid;      ///< who sent the notify
     ...
     MWatchNotify()
    :Message {
    CEPH_MSG_WATCH_NOTIFY, HEAD_VERSION, COMPAT_VERSION} {
    }
    MWatchNotify(uint64_t c, uint64_t v, uint64_t i, uint8_t o,
                 ceph::buffer::list b, uint64_t n = 0)
  :    Message {
    CEPH_MSG_WATCH_NOTIFY, HEAD_VERSION, COMPAT_VERSION},
        cookie(c),
        ver(v),
        notify_id(i), opcode(o), bl(b), return_code(0), notifier_gid(n) {
    }
  private:
    ~MWatchNotify()final {
    }
