...
CyanStore::Shard::base_errorator::future < bool > CyanStore::Shard::exists(
    CollectionRef ch,
...
seastar::future < > CyanStore::Shard::set_collection_opts(CollectionRef ch,
...
CyanStore::Shard::read_errorator::future < ceph::bufferlist > CyanStore::Shard::read(
    CollectionRef ch,
    const ghobject_t &oid,
    uint64_t offset,
    size_t len,
...
CyanStore::Shard::read_errorator::future < ceph::bufferlist > CyanStore::Shard::readv(
    CollectionRef ch,
    const ghobject_t &oid,
...
CyanStore::Shard::get_attr_errorator::future < ceph::bufferlist > CyanStore::Shard::get_attr(
    CollectionRef ch,
    const ghobject_t &oid,
...
CyanStore::Shard::get_attrs_ertr::future < CyanStore::Shard::attrs_t > CyanStore::Shard::get_attrs(
    CollectionRef ch,
    const ghobject_t &oid)
{
    auto c = static_cast < Collection * > (ch.get());
...
CyanStore::Shard::read_errorator::future < std::map < uint64_t, uint64_t>> CyanStore::Shard::fiemap(
    CollectionRef ch,
    const ghobject_t &oid,
    uint64_t off,
    uint64_t len)
...
seastar::future < struct stat > CyanStore::Shard::stat(
    CollectionRef ch,
    const ghobject_t &oid)
{
    auto c = static_cast < Collection * > (ch.get());
