...
static int do_resize(librbd::Image &image, uint64_t size,
                     bool allow_shrink, bool no_progress)
{
              ...
              &ceph_global_init_args)
{
    ...
    if (!encryption_options.specs.empty()) {
        ...
        if (r < 0) {
            std::
            cerr << "rbd: encryption load failed: " <<
                 cpp_strerror(r)
                 << std::endl;
            return r;
        }
    ...
    if (r < 0) {
        std::
        cerr << "rbd: resize error: " << cpp_strerror(r) <<
             std::endl;
        return r;
    }
    ...
    if (info.size == size) {
        std::
        cerr << "rbd: new size is equal to original size " <<
             std::endl;
        return -EINVAL;
    }
