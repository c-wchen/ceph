...
namespace librbd {
    ...
    namespace deep_copy {
        ...
        template <> struct SnapshotCreateRequest <librbd::MockTestImageCtx > {
            ...
            static SnapshotCreateRequest *create(librbd::MockTestImageCtx *
                                                 image_ctx,
                                                 const std::string & snap_name,
                                                 const cls::rbd::
                                                 SnapshotNamespace &
                                                 snap_namespace, uint64_t size,
                                                 const cls::rbd::
                                                 ParentImageSpec & parent_spec,
                                                 uint64_t parent_overlap,
                                                 Context * on_finish) {
        ...
        SetHeadRequest < librbd::MockTestImageCtx > *SetHeadRequest <
            librbd::MockTestImageCtx >::s_instance = nullptr;
        SnapshotCreateRequest < librbd::MockTestImageCtx >
            *SnapshotCreateRequest < librbd::MockTestImageCtx >::s_instance =
            nullptr;
...
template class librbd::deep_copy::SnapshotCopyRequest <
...
namespace librbd {
    namespace deep_copy {
        ...
        class TestMockDeepCopySnapshotCopyRequest:public TestMockFixture {
          public:
            typedef SetHeadRequest < librbd::MockTestImageCtx >
                MockSetHeadRequest;
            typedef SnapshotCopyRequest < librbd::MockTestImageCtx >
            ...
            void expect_get_snap_namespace(librbd::
                ...
                EXPECT_CALL(mock_snapshot_create_request, send())
                    ...
                    WillOnce(DoAll
                             (Invoke([&mock_image_ctx, snap_id, snap_name] () {
                                     inject_snap(mock_image_ctx, snap_id,
                                                 snap_name);}), Invoke([this,
                                                                        &mock_snapshot_create_request,
                                                                        r] () {
                                                                       m_work_queue->
                                                                       queue
            ...
            void expect_snap_unprotect(librbd::
                                       MockTestImageCtx & mock_image_ctx,
                                       const std::string & snap_name, int r) {
                EXPECT_CALL(*mock_image_ctx.operations,
            ...
            void expect_snap_is_protected(librbd::
                                          MockTestImageCtx & mock_image_ctx,
                                          uint64_t snap_id, bool is_protected,
                                          int r) {
            ...
            void expect_snap_is_unprotected(librbd::
                                            MockTestImageCtx & mock_image_ctx,
                                            uint64_t snap_id,
            ...
            void expect_set_head(MockSetHeadRequest & mock_set_head_request,
                                 int r) {
                EXPECT_CALL(mock_set_head_request, send())
                    .WillOnce(Invoke([&mock_set_head_request, r] () {
                                     mock_set_head_request.on_finish->
                                     complete(r);}));
            }
            ...
            static void inject_snap(librbd::MockTestImageCtx & mock_image_ctx,
                                    uint64_t snap_id,
                                    const std::string & snap_name) {
                mock_image_ctx.snap_ids[ {
                                        cls::rbd::UserSnapshotNamespace(),
                                        snap_name}] = snap_id;
            }

            MockSnapshotCopyRequest *create_request(librbd::
                                                    MockTestImageCtx &
                                                    mock_src_image_ctx,
                                                    librbd::
                                                    MockTestImageCtx &
                ...
                return new MockSnapshotCopyRequest(&mock_src_image_ctx,
                                                   ...
                                                   src_snap_id_start,
                                                   src_snap_id_end,
                                                   dst_snap_id_start, false,
                                                   m_work_queue, &m_snap_seqs,
                                                   on_finish);
            }
            ...
            int create_snap(librbd::ImageCtx * image_ctx,
                ...
                int r =
                    image_ctx->operations->snap_create(snap_ns,
                                                       snap_name.c_str(), 0,
                                                       prog_ctx);
                if (r < 0) {
                    return r;
                ...
                if (protect) {
                    EXPECT_TRUE(std::holds_alternative <
                                cls::rbd::UserSnapshotNamespace > (snap_ns));
                    r = image_ctx->operations->snap_protect(snap_ns,
                                                            snap_name.c_str());
                    if (r < 0) {
                        return r;
                    }
            ...
            int create_snap(librbd::ImageCtx * image_ctx,
                            const std::string & snap_name, bool protect =
                            false) {
                return create_snap(image_ctx, cls::rbd::UserSnapshotNamespace {
                                   }
                                   , snap_name, protect);
            }
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapCreate) {
            ...
            uint64_t src_snap_id1 = m_src_image_ctx->snap_ids[{ cls::rbd::
                                                              UserSnapshotNamespace
                                                              (), "snap1" }];
            uint64_t src_snap_id2 =
                m_src_image_ctx->snap_ids[{ cls::rbd::UserSnapshotNamespace(),
                                          "snap2" }];
            ...
            expect_snap_create(mock_dst_image_ctx, mock_snapshot_create_request,
                               "snap1", 12, 0);
            expect_get_snap_namespace(mock_src_image_ctx, src_snap_id2);
            expect_start_op(mock_exclusive_lock);
            expect_snap_create(mock_dst_image_ctx, mock_snapshot_create_request,
                               "snap2", 14, 0);
            expect_snap_is_protected(mock_src_image_ctx, src_snap_id1, false,
                                     0);
            expect_snap_is_protected(mock_src_image_ctx, src_snap_id2, false,
                                     0);
            expect_set_head(mock_set_head_request, 0);
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapCreateError) {
            ...
            uint64_t src_snap_id1 = mock_src_image_ctx.snap_ids[{ cls::rbd::
                                                                UserSnapshotNamespace
                                                                (), "snap1" }];
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapRemoveAndCreate) {
            ...
            uint64_t dst_snap_id1 =
                m_dst_image_ctx->snap_ids[{ cls::rbd::UserSnapshotNamespace(),
                                          "snap1" }];
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapRemoveError) {
            ...
            uint64_t dst_snap_id1 = m_dst_image_ctx->snap_ids[{ cls::rbd::
                                                              UserSnapshotNamespace
                                                              (), "snap1" }];
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapUnprotect) {
            ...
            InSequence seq;
            expect_snap_is_unprotected(mock_dst_image_ctx, dst_snap_id1, false,
                                       0);
            expect_snap_is_unprotected(mock_src_image_ctx, src_snap_id1, true,
                                       0);
            expect_start_op(mock_exclusive_lock);
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapProtect) {
            ...
            InSequence seq;
            expect_snap_is_unprotected(mock_dst_image_ctx, dst_snap_id1, true,
                                       0);
            expect_get_snap_namespace(mock_dst_image_ctx, dst_snap_id1);
            expect_get_snap_namespace(mock_src_image_ctx, src_snap_id1);
            expect_snap_is_protected(mock_src_image_ctx, src_snap_id1, true, 0);
            expect_snap_is_protected(mock_dst_image_ctx, dst_snap_id1, false,
                                     0);
            expect_start_op(mock_exclusive_lock);
            expect_snap_protect(mock_dst_image_ctx, "snap1", 0);
            expect_set_head(mock_set_head_request, 0);
        ...
        TEST_F(TestMockDeepCopySnapshotCopyRequest, StartEndLimit) {
            ...
            expect_snap_is_protected(mock_src_image_ctx, src_snap_id2, false,
                                     0);
            expect_snap_is_protected(mock_src_image_ctx, src_snap_id3, false,
                                     0);
