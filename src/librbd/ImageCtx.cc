...
namespace librbd {
    ...
    :cct((CephContext *) p.cct()),
        ...
        owner_lock(ceph::
                   make_shared_mutex(util::
                                     unique_lock_name
                                     ("librbd::ImageCtx::owner_lock", this))),
        image_lock(ceph::
                   make_shared_mutex(util::
                                     unique_lock_name
                                     ("librbd::ImageCtx::image_lock", this))),
        timestamp_lock(ceph::
                       make_shared_mutex(util::
                                         unique_lock_name
                                         ("librbd::ImageCtx::timestamp_lock",
                                          this))),
        async_ops_lock(ceph::
                       make_mutex(util::
                                  unique_lock_name
                                  ("librbd::ImageCtx::async_ops_lock", this))),
        copyup_list_lock(ceph::
                         make_mutex(util::
                                    unique_lock_name
                                    ("librbd::ImageCtx::copyup_list_lock",
                                     this))), extra_read_flags(0),
        old_format(false), order(0), size(0), features(0), format_string(NULL),
        id(image_id), parent(NULL), stripe_unit(0), stripe_count(0), flags(0),
        readahead(), total_bytes_read(0), state(new ImageState <> (this)),
        operations(new Operations <> (*this)), exclusive_lock(nullptr),
        object_map(nullptr), op_work_queue(asio_engine->get_work_queue()),
        plugin_registry(new PluginRegistry < ImageCtx > (this)),
        event_socket_completions(32), asok_hook(nullptr),
        trace_endpoint("librbd") {
        ldout(cct,
              10) << this << " " << __func__ << ": " << "image_name=" <<
            image_name << ", " << "image_id=" << image_id << dendl;
    ...
    void ImageCtx::perf_start(string name) {
        ...
        plb.add_u64_counter(l_librbd_cmp_bytes, "cmp_bytes",
                            "Data size in cmps", NULL, 0, unit_t(UNIT_BYTES));
        ...
        plb.add_u64_counter(l_librbd_readahead_bytes, "readahead_bytes",
                            "Data size in read ahead", NULL, 0,
                            unit_t(UNIT_BYTES));
    ...
    snap_t ImageCtx::get_snap_id(const cls::rbd::
    ...
    uint64_t ImageCtx::get_area_size(io::ImageArea area) const {
        ...
        ceph_assert(size.first <= raw_size
                    && size.second == io::ImageArea::DATA);
    ...
    bool ImageCtx::test_op_features(uint64_t in_op_features,
                                    const ceph::
                                    shared_mutex & in_image_lock)const {
        ceph_assert(ceph_mutex_is_locked(image_lock));
    ...
    std::pair < uint64_t,
        io::ImageArea > ImageCtx::reduce_parent_overlap(uint64_t raw_overlap,
    ...
    void ImageCtx::cancel_async_requests(Context * on_finish) {
        {
            std::lock_guard async_ops_locker {
            async_ops_lock};
...
#undef ASSIGN_OPTION
        ...
        alloc_hint_flags = 0;
        auto compression_hint =
            config.get_val < std::string > ("rbd_compression_hint");
        ...
        io_image_dispatcher->apply_qos_schedule_tick_min(config.get_val <
                                                         uint64_t >
                                                         ("rbd_qos_schedule_tick_min"));

        io_image_dispatcher->
            apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_IOPS_THROTTLE,
        ...
        io_image_dispatcher->
            apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_BPS_THROTTLE,
                            config.get_val < uint64_t > ("rbd_qos_bps_limit"),
                            config.get_val < uint64_t > ("rbd_qos_bps_burst"),
                            config.get_val < uint64_t >
                            ("rbd_qos_bps_burst_seconds"));
        io_image_dispatcher->
            apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_READ_IOPS_THROTTLE,
                            config.get_val < uint64_t >
        ...
        io_image_dispatcher->
            apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_WRITE_IOPS_THROTTLE,
                            config.get_val < uint64_t >
        ...
        io_image_dispatcher->
            apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_READ_BPS_THROTTLE,
                            config.get_val < uint64_t >
        ...
        io_image_dispatcher->
            apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_WRITE_BPS_THROTTLE,
                            config.get_val < uint64_t >
        ...
        io_image_dispatcher->
            apply_qos_exclude_ops(librbd::io::
                                  rbd_io_operations_from_string(config.get_val <
                                                                std::string >
                                                                ("rbd_qos_exclude_ops"),
                                                                nullptr));
    ...
    void ImageCtx::rebuild_data_io_context() {
        auto ctx =
            std::make_shared < neorados::IOContext > (data_ctx.get_id(),
                                                      data_ctx.get_namespace());
