...
namespace journal {
    ...
    } // anonymous namespace using namespace cls::journal;
    ...
    Journaler::Threads::Threads(CephContext * cct) {
        thread_pool =
            new ThreadPool(cct, "Journaler::thread_pool", "tp_journal", 1);
        thread_pool->start();

        work_queue = new ContextWQ("Journaler::work_queue",
                                   ceph::make_timespan(60), thread_pool);
  ...
  :    m_threads(new
    ...
    void Journaler::set_up(ContextWQ * work_queue, SafeTimer * timer,
        ...
        m_object_oid_prefix =
            object_oid_prefix(m_header_ioctx.get_id(), journal_id);

        m_metadata =
            ceph::make_ref < JournalMetadata > (work_queue, timer, timer_lock,
                                                m_header_ioctx, m_header_oid,
                                                m_client_id, settings);
    }
    ...
    int Journaler::init_complete() {
        ...
        if (pool_id < 0 || pool_id == m_header_ioctx.get_id()) {
            ...
            ldout(m_cct,
                  20) << "using pool id=" << pool_id << " for journal data" <<
                dendl;
            ...
            if (r < 0) {
                if (r == -ENOENT) {
                    ldout(m_cct,
                          1) << "pool id=" << pool_id << " no longer exists" <<
                        dendl;
                }
                return r;
            }
    ...
    } void Journaler::get_immutable_metadata(uint8_t * order,
                                             uint8_t * splay_width,
                                             int64_t * pool_id,
                                             Context * on_finish) {
        m_metadata->get_immutable_metadata(order, splay_width, pool_id,
                                           on_finish);
    }
    ...
    void Journaler::get_mutable_metadata(uint64_t * minimum_set,
                                         ...
                                         Context * on_finish) {
        m_metadata->get_mutable_metadata(minimum_set, active_set, clients,
                                         on_finish);
    }
    ...
    void Journaler::register_client(const bufferlist & data,
                                    Context * on_finish) {
    ...
    void Journaler::allocate_tag(const bufferlist & data,
    ...
    void Journaler::get_tags(uint64_t tag_class, Tags * tags,
                             Context * on_finish) {
    ...
    void Journaler::start_append(uint64_t max_in_flight_appends) {
        ...
        m_recorder =
            std::make_unique < JournalRecorder > (m_data_ioctx,
                                                  m_object_oid_prefix,
                                                  m_metadata,
    ...
    void Journaler::set_append_batch_options(int flush_interval,
                                             ...
                                             double flush_age) {
        ceph_assert(m_recorder != nullptr);
        m_recorder->set_append_batch_options(flush_interval, flush_bytes,
                                             flush_age);
    }
    ...
    void Journaler::stop_append(Context * on_safe) {
        ...
        on_safe =
            new LambdaContext([recorder = std::move(recorder), on_safe] (int r)
                              {
                              on_safe->complete(r);});
    ...
    void Journaler::create_player(ReplayHandler * replay_handler) {
        ceph_assert(m_player == nullptr);
        m_player =
            std::make_unique < JournalPlayer > (m_data_ioctx,
                                                m_object_oid_prefix, m_metadata,
                                                replay_handler,
