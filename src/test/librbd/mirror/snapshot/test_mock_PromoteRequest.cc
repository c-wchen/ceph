...
struct MockTestImageCtx: public MockImageCtx {
    explicit MockTestImageCtx(librbd::
                              ImageCtx &
...
ListWatchersRequest < MockTestImageCtx > *ListWatchersRequest <
...
struct Mock {
    ...
    Mock()
    {
        s_instance = this;
                   ...
                   bool(librbd::MockTestImageCtx *, bool,
                        bool, bool *, uint64_t *));
};
...
template <> bool can_create_primary_snapshot(librbd::
        MockTestImageCtx
        * image_ctx,
        bool demoted,
        bool force,
        ...
        rollback_snap_id)
{
    return Mock::s_instance->
           can_create_primary_snapshot(image_ctx, demoted, force,
                                       requires_orphan,
                                       rollback_snap_id);
}
} // namespace util template <>
...
template <> struct CreatePrimaryRequest <MockTestImageCtx > {
    ...
    static CreatePrimaryRequest *create(MockTestImageCtx *
                                        image_ctx,
                                        const std::
                                        ...
                                        Context *on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->demoted =
            ((flags & CREATE_PRIMARY_FLAG_DEMOTED) != 0);
        s_instance->force =
            ((flags & CREATE_PRIMARY_FLAG_FORCE) != 0);
        s_instance->on_finish = on_finish;
        return s_instance;
    } MOCK_METHOD0(send, void ());
...
CreatePrimaryRequest < MockTestImageCtx > *CreatePrimaryRequest <
MockTestImageCtx >::s_instance = nullptr;
...
template class librbd::mirror::snapshot::PromoteRequest <
...
public:
    ...
    typedef CreateNonPrimaryRequest < MockTestImageCtx >
    MockCreateNonPrimaryRequest;
    typedef CreatePrimaryRequest < MockTestImageCtx >
    MockCreatePrimaryRequest;
    typedef util::Mock MockUtils;

    void expect_can_create_primary_snapshot(MockUtils &mock_utils,
                                            bool force,
                                            ...
                                            bool result)
    {
        ...
        WillOnce(DoAll
                 (WithArgs < 3,
                  4 >
                  (Invoke([requires_orphan, rollback_snap_id]
        (bool * orphan, uint64_t *snap_id) {
            *orphan = requires_orphan;
            *snap_id = rollback_snap_id;
        })),
        Return(result)));
    } void expect_create_orphan_snapshot(MockTestImageCtx &
                                         mock_image_ctx,
                                         MockCreateNonPrimaryRequest
                                         &
                                         ...
                                         int r)
    {
        ...
        WillOnce(Invoke
                 ([&mock_image_ctx,
        &mock_create_non_primary_request, r]() {
            mock_image_ctx.image_ctx->op_work_queue->
            queue(mock_create_non_primary_request.
                  on_finish, r);
        }));
    } void expect_list_watchers(MockTestImageCtx &mock_image_ctx,
                                MockListWatchersRequest &
                                mock_list_watchers_request,
                                const std::list < obj_watch_t >
                                &watchers, int r)
    {
        EXPECT_CALL(mock_list_watchers_request, send())
        .
        WillOnce(Invoke
                 ([&mock_image_ctx, &mock_list_watchers_request,
        watchers, r]() {
            *mock_list_watchers_request.watchers =
                watchers;
            mock_image_ctx.image_ctx->op_work_queue->
            queue(mock_list_watchers_request.on_finish,
                  r);
        }));
    }

    void expect_acquire_lock(MockTestImageCtx &mock_image_ctx,
                             int r)
    {
    ...
    void expect_rollback(MockTestImageCtx &mock_image_ctx,
                         uint64_t snap_id,
    ...
    void expect_create_promote_snapshot(MockTestImageCtx &
                                        mock_image_ctx,
                                        MockCreatePrimaryRequest &
                                        ...
                                        int r)
    {
        ...
        WillOnce(Invoke
                 ([&mock_image_ctx,
        &mock_create_primary_request, r]() {
            mock_image_ctx.image_ctx->op_work_queue->
            queue(mock_create_primary_request.on_finish,
                  r);
        }));
    }

    void expect_release_lock(MockTestImageCtx &mock_image_ctx,
                             int r)
    {
        if (mock_image_ctx.exclusive_lock == nullptr) {
            return;
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, Success)
{
    ...
    expect_can_create_primary_snapshot(mock_utils, true, false,
                                       CEPH_NOSNAP, true);
    MockCreatePrimaryRequest mock_create_primary_request;
    expect_create_promote_snapshot(mock_image_ctx,
                                   mock_create_primary_request, 0);
    C_SaferCond ctx;
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, SuccessForce)
{
    ...
    MockUtils mock_utils;
    expect_can_create_primary_snapshot(mock_utils, true, true,
                                       CEPH_NOSNAP, true);
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_orphan_snapshot(mock_image_ctx,
                                  mock_create_non_primary_request,
                                  0);
    MockListWatchersRequest mock_list_watchers_request;
    expect_list_watchers(mock_image_ctx, mock_list_watchers_request, {
    }
    , 0);
    expect_acquire_lock(mock_image_ctx, 0);

    SnapInfo snap_info = { "snap", cls::rbd::MirrorSnapshotNamespace {}
                           , 0,
                           {}
                           , 0, 0, {}
                         };
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, SuccessRollback)
{
    ...
    MockUtils mock_utils;
    expect_can_create_primary_snapshot(mock_utils, true, false, 123,
                                       true);
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_orphan_snapshot(mock_image_ctx,
                                  mock_create_non_primary_request,
