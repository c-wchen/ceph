...
void dump_cmddesc_to_json(ceph::Formatter * jf,
                          ...
                          const std::string & avail, uint64_t flags);
...
template < typename T >
    void cmd_getval(CephContext * cct, const cmdmap_t & cmdmap,
                    const std::string & k, T & val, const T & defval)
{
...
template < typename T >
    void
cmd_putval(CephContext * cct, cmdmap_t & cmdmap, const std::string & k,
           const T & val)
{
    cmdmap[k] = val;
}
