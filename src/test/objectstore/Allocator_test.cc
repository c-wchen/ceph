     ...
     */
    {
        ...
        EXPECT_EQ(4 * block_size,
                  alloc->allocate(4 * (uint64_t) block_size,
                                  (uint64_t) block_size, 0, (int64_t) 0,
                                  &extents));
...
TEST_P(AllocTest, test_alloc_min_max_alloc)
{
     ...
     */
    {
        ...
        EXPECT_EQ(16 * block_size,
                  alloc->allocate(16 * (uint64_t) block_size,
                                  (uint64_t) block_size, 2 * block_size,
                                  (int64_t) 0, &extents));
    ...
    for (size_t round = 0; round < rounds; round++) {
        for (size_t j = 0; j < actions_per_round; j++) {
            ...
            if (rng() % capacity >= allocated_cnt) {
                //allocate
                want_size =
                    (rng() % one_alloc_max) / alloc_unit * alloc_unit +
                    alloc_unit;
                tmp.clear();
                ...
                interval_set < uint64_t > release_set;
                release_set.insert(allocated[item].offset,
                                   allocated[item].length);
                alloc->release(release_set);
