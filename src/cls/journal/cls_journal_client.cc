...
struct C_AioExec: public Context {
    ...
    C_AioExec(librados::IoCtx &_ioctx,
              const std::string &_oid)
        : ioctx(_ioctx), oid(_oid)
    {
    } static void rados_callback(rados_completion_t c,
                                 void *arg)
    {
...
struct C_ClientList: public C_AioExec {
    ...
    C_ClientList(librados::IoCtx &_ioctx,
                 const std::string &_oid,
                 std::set < cls::journal::Client > *_clients,
                 Context *_on_finish)
        : C_AioExec(_ioctx, _oid), clients(_clients),
    ...
    } void send(const std::string &start_after)
    {
        ...
        librados::AioCompletion *rados_completion =
            librados::Rados::aio_create_completion(this,
                rados_callback);
        int r =
            ioctx.aio_operate(oid, rados_completion, &op,
                              &outbl);
        ceph_assert(r == 0);
    ...
    } void complete(int r) override
    {
        ...
        try {
            ...
            if (!partial_clients.empty()) {
                ...
                clients->insert(partial_clients.begin(),
                                partial_clients.end());
            }
...
struct C_ImmutableMetadata: public C_AioExec {
    ...
    C_ImmutableMetadata(librados::IoCtx &_ioctx,
                        const std::string &_oid,
                        uint8_t *_order,
                        uint8_t *_splay_width,
                        int64_t *_pool_id,
                        Context *_on_finish)
        : C_AioExec(_ioctx, _oid), order(_order),
          splay_width(_splay_width), pool_id(_pool_id),
          on_finish(_on_finish)
    {
    } void send()
    {
...
struct C_MutableMetadata: public C_AioExec {
    ...
    C_MutableMetadata(librados::IoCtx &_ioctx,
                      const std::string &_oid,
...
int create(librados::IoCtx &ioctx, const std::string &oid,
           uint8_t order, uint8_t splay, int64_t pool_id)
{
...
void get_immutable_metadata(librados::IoCtx &ioctx,
                            const std::string &oid,
                            uint8_t *order, uint8_t *splay_width,
                            int64_t *pool_id,
                            Context *on_finish)
{
    C_ImmutableMetadata *metadata =
        new C_ImmutableMetadata(ioctx, oid, order,
                                splay_width, pool_id,
                                on_finish);
    metadata->send();
}

void get_mutable_metadata(librados::IoCtx &ioctx,
                          const std::string &oid,
                          uint64_t *minimum_set,
                          uint64_t *active_set,
                          std::set < cls::journal::Client >
                          *clients, Context *on_finish)
{
    C_ClientList *client_list =
        new C_ClientList(ioctx, oid, clients, on_finish);
    C_MutableMetadata *metadata =
        new C_MutableMetadata(ioctx, oid, minimum_set, active_set,
                              client_list);
    metadata->send();
}

void set_minimum_set(librados::ObjectWriteOperation *op,
                     uint64_t object_set)
{
    bufferlist bl;
...
void set_active_set(librados::ObjectWriteOperation *op,
                    uint64_t object_set)
{
    bufferlist bl;
    encode(object_set, bl);
...
int client_register(librados::IoCtx &ioctx,
                    const std::string &oid, const std::string &id,
...
int client_update_data(librados::IoCtx &ioctx,
                       const std::string &oid,
                       const std::string &id,
                       const bufferlist &data)
{
...
int client_update_state(librados::IoCtx &ioctx,
                        const std::string &oid,
                        const std::string &id,
...
int client_unregister(librados::IoCtx &ioctx,
                      const std::string &oid,
                      const std::string &id)
{
    librados::ObjectWriteOperation op;
...
void client_commit(librados::ObjectWriteOperation *op,
                   const std::string &id,
                   const cls::journal::
...
int get_next_tag_tid(librados::IoCtx &ioctx,
                     const std::string &oid, uint64_t *tag_tid)
{
    librados::ObjectReadOperation op;
...
int get_tag_finish(bufferlist::const_iterator *iter,
                   cls::journal::Tag *tag)
{
    try {
        decode(*tag, *iter);
             ...
             std::set < cls::journal::Tag > *tags)
{
    ...
    while (true) {
        librados::ObjectReadOperation op;
        tag_list_start(&op, start_after_tag_tid, JOURNAL_MAX_RETURN,
                       client_id, tag_class);
...
void guard_append(librados::ObjectWriteOperation *op,
...
void append(librados::ObjectWriteOperation *op,
            uint64_t soft_max_size, bufferlist &data)
{
    bufferlist bl;
    encode(soft_max_size, bl);
    encode(data, bl);
