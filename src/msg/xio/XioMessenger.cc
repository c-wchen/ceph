...
namespace xio_log {
    ...
    void log_dout(const char *file, unsigned line,
                  const char *function, unsigned level, const char *fmt, ...) {
        char buffer[2048];
        va_list args;
        va_start(args, fmt);
                            ...
                            void *cb_user_context)
{
    ...
    ldout(cct,
          4) << "session event: " << xio_session_event_str(event_data->event)
...
static int on_msg(struct xio_session *session,
                  struct xio_msg *req, int more_in_batch, void *cb_user_context)
{
    ...
    return xcon->on_msg(session, req, more_in_batch, cb_user_context);
}
                             ...
                             struct xio_msg *msg, void *conn_user_context)
{
    ...
    ldout(cct,
          25) << "on cancel request: session: " << session << " msg: " << msg <<
        " conn_user_context " << conn_user_context << dendl;
        ...
        if (!initialized) {
            ...
            if (g_code_env == CODE_ENVIRONMENT_DAEMON) {
                ...
                xio_set_opt(NULL, XIO_OPTLEVEL_RDMA,
                            XIO_OPTNAME_ENABLE_FORK_INIT, &xopt, sizeof(xopt));
            }
            ...
            xio_set_opt(NULL, XIO_OPTLEVEL_ACCELIO,
                        XIO_OPTNAME_ENABLE_FLOW_CONTROL, &xopt, sizeof(xopt));

            /* and set threshold for buffer callouts */
            xopt = max(cct->_conf->xio_max_send_inline, 512);
            xio_set_opt(NULL, XIO_OPTLEVEL_ACCELIO,
                        XIO_OPTNAME_MAX_INLINE_XIO_DATA, &xopt, sizeof(xopt));

            xopt = XioMsgHdr::get_max_encoded_length();
            ldout(cct,
                  2) << "setting accelio max header size " << xopt << dendl;
            xio_set_opt(NULL, XIO_OPTLEVEL_ACCELIO,
                        XIO_OPTNAME_MAX_INLINE_XIO_HEADER, &xopt, sizeof(xopt));
...
#define XMSG_MEMPOOL_QUANTUM 4096
            ...
            xio_msgr_noreg_mpool = xio_mempool_create(-1 /* nodeid */ ,
...
sh_cond(), need_addr(true), did_bind(false), nonce(_nonce)
{
...
int XioMessenger::get_nconns_per_portal(uint64_t cflags)
{
    ...
    if (cflags & Messenger::HAS_MANY_CONNECTIONS)
        nconns =
            max(cct->_conf->xio_max_conns_per_portal,
                XIO_DEFAULT_NUM_CONNS_PER_PORTAL);
    else if (cflags & Messenger::HEARTBEAT)
        nconns =
            max(cct->_conf->osd_heartbeat_min_peers * 4,
                XIO_DEFAULT_NUM_CONNS_PER_PORTAL);
...
{
    if (shutdown_called) {
        return xio_reject(session, XIO_E_SESSION_REFUSED, NULL /* udata */ ,
                          0 /* udata len */ );
    }
                                ...
                                void *cb_user_context)
{
    ...
    case XIO_SESSION_CONNECTION_ESTABLISHED_EVENT:
        {
            ...
            xcon =
                static_cast < XioConnection * >(event_data->conn_user_context);
            ...
            ldout(cct,
                  2) << "client: connected from " << peer_addr_for_me << " to "
                << paddr << dendl;
    ...
    case XIO_SESSION_NEW_CONNECTION_EVENT:
        {
            ...
            (void)xio_query_context(xcona.ctx, &xctxa,
                                    XIO_CONTEXT_ATTR_USER_CTX);
            ...
            ldout(cct, 2) << "New connection session " << session
                << " xcon " << xcon << " on msgr: " << this << " portal: " <<
                xcon->portal << dendl;
            ldout(cct,
                  2) << "Server: connected from " << s_inst.
                addr << " to " << peer_addr_for_me << dendl;
        }
        break;
    ...
    case XIO_SESSION_TEARDOWN_EVENT:
        ...
        if (unlikely(XioPool::trace_mempool)) {
            xp_stats.dump("xio session dtor",
...
static inline int
xio_count_buffers(const buffer::list & bl, int &req_size, int &msg_off,
                  int &req_off)
{
    ...
    for (;;) {
        ...
        ++msg_off;
        if (unlikely
            (msg_off >= XIO_MSGR_IOVLEN || req_size >= MAX_XIO_BUF_SIZE)) {
            ++req_off;
            msg_off = 0;
            req_size = 0;
        }
...
int XioMessenger::bind(const entity_addr_t & addr)
{
    ...
    uint16_t port0;
    int r =
        portals.bind(&xio_msgr_ops, base_uri, shift_addr.get_port(), &port0);
    if (r == 0) {
...
int XioMessenger::_send_message_impl(Message * m, XioConnection * xcon)
{
    ...
    if (msg_off == 0 && ex_cnt > 0) {
        ...
        ldout(cct,
              10) << "msg_off 0, ex_cnt " << ex_cnt << " -> " << ex_cnt -
            1 << dendl;
        ex_cnt--;
    }
    ...
    if (xmsg->get_msg_count() > 1) {
        ...
        for (req_off = 0; ((unsigned)req_off) < xmsg->get_msg_count() - 1;
             ++req_off) {
            req = &xmsg->req_arr[req_off].msg;
...
ConnectionRef XioMessenger::get_connection(const entity_inst_t & dest)
{
    ...
    if ((&dest == &self_inst) || (dest == self_inst)) {
...
static inline XioMarkDownHook *pool_alloc_markdown_hook(XioConnection * xcon,
                                                        Message * m)
{
    struct xio_reg_mem mp_mem;
