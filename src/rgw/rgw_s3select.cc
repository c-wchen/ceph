...
int RGWSelectObj_ObjStore_S3::run_s3select_on_csv(const char* query, const char* input, size_t input_length)
{
    ...
    if (s3select_syntax.get_error_description().empty() == false) {
        ...
        m_aws_response_handler.send_error_response_rgw_formatter(s3select_syntax_error,
                s3select_syntax.get_error_description().c_str(), s3select_resource_id);
        ...
        status = m_s3_csv_object.run_s3select_on_stream(m_aws_response_handler.get_sql_result(), input, input_length,
        ...
        if (status < 0) {
            ...
            m_aws_response_handler.send_error_response_rgw_formatter(s3select_processTime_error,
                    m_s3_csv_object.get_error_description().c_str(), s3select_resource_id);

            ldpp_dout(this, 10) << "s3-select query: failed to process query; {" << m_s3_csv_object.get_error_description() << "}"
                                << dendl;
            return -1;
        }
...
int RGWSelectObj_ObjStore_S3::run_s3select_on_json(const char* query, const char* input, size_t input_length)
{
    ...
    //query is correct(syntax), processing is starting.
    try {
        status = m_s3_json_object.run_s3select_on_stream(m_aws_response_handler.get_sql_result(), input, input_length,
                 m_object_size_for_processing);
    } catch (base_s3select_exception& e) {
...
int RGWSelectObj_ObjStore_S3::range_request(int64_t ofs, int64_t len, void* buff, optional_yield y)
{
    ...
    m_request_range = len;
    ldout(s->cct, 10) << "S3select: calling execute(async):" << " request-offset :" << ofs << " request-length :" << len <<
...
void RGWSelectObj_ObjStore_S3::execute(optional_yield y)
{
    ...
    if (m_parquet_type) {
        ...
        if (status) {
            ldout(s->cct, 10) << "S3select: failed to process query <" << m_sql_query << "> on object " << s->object->get_name() <<
                              dendl;
            op_ret = -ERR_INVALID_REQUEST;
        } else {
...
int RGWSelectObj_ObjStore_S3::parquet_processing(bufferlist& bl, off_t ofs, off_t len)
{
    ...
    if (requested_buffer.size() < m_request_range) {
        ldout(s->cct, 10) << "S3select: need another round buffe-size: " << requested_buffer.size() << " request range length:"
                          << m_request_range << dendl;
        return 0;
    } else {//buffer is complete
        ldout(s->cct, 10) << "S3select: buffer is complete " << requested_buffer.size() << " request range length:" <<
                          m_request_range << dendl;
        m_request_range = 0;
    }
    return 0;
    ...
    if (m_scan_range_ind) { //only upon range-scan
        ...
        if ((m_aws_response_handler.get_processed_size() + len) >= m_requested_range) {
            ...
            for (sc = start; sc < len; sc++) { //assumption : row-delimiter must exist or its end ebject
                ...
                if (!strncmp(row_delimiter, p, 1)) {
                    ldout(s->cct, 10) << "S3select: found row-delimiter on " << sc << " get_processed_size = " <<
...
int RGWSelectObj_ObjStore_S3::csv_processing(bufferlist& bl, off_t ofs, off_t len)
{
    ...
    if (s->obj_size == 0 || m_object_size_for_processing == 0) {
        ...
        for (auto& it : bl.buffers()) {
            ...
            ldpp_dout(this, 10) << "s3select: chunk:  ofs = " << ofs << " len = " << len << " it.length() = " << it.length() <<
    ...
    if (m_aws_response_handler.get_processed_size() >= uint64_t(m_object_size_for_processing) ||
        ...
        if (status >= 0) {
            ...
            ldpp_dout(this, 10) << "s3select : reached the end of query request : aws_response_handler.get_processed_size() " <<
                                m_aws_response_handler.get_processed_size()
                                << "m_object_size_for_processing : " << m_object_size_for_processing << dendl;
        }
        if (m_s3_csv_object.is_sql_limit_reached()) {
    ...
    if (s->obj_size == 0 || m_object_size_for_processing == 0) {
        ...
        for (auto& it : bl.buffers()) {
            ...
            if ((ofs + len) > it.length()) {
                ldpp_dout(this, 10) << "s3select: offset and length may cause invalid read: ofs = " << ofs << " len = " << len <<
                                       " it.length() = " << it.length() << dendl;
                ofs = 0;
                len = it.length();
            }
