...
class MDiscover final:public MMDSOp {
  ...
  public:
     inodeno_t get_base_ino() const {
        ...
        return snapid;
    } const filepath & get_want() const {
        return want;
    } const std::string & get_dentry(int n) const {
        return want[n];
    } bool wants_base_dir() const {
  ...
  MDiscover(inodeno_t base_ino_, frag_t base_frag_, snapid_t s, filepath & want_path_, bool want_base_dir_ = true, bool path_locked_ = false):
    MMDSOp
    {
    MSG_MDS_DISCOVER}, base_ino(base_ino_), base_dir_frag(base_frag_),
        snapid(s), want(want_path_), want_base_dir(want_base_dir_),
  ...
  public:
    std::string_view get_type_name()const override {
        ...
        out << "discover(" << header.
            tid << " " << base_ino << "." << base_dir_frag << " " << want <<
            ")";
