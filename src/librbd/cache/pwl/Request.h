             ...
             template < typename T > class C_BlockIORequest:public Context {
              public:
                ...
                utime_t allocated_time; /* When allocation began */
                 ...
                 C_BlockIORequest(T & pwl, const utime_t arrived, io::Extents
                                  && extents, bufferlist
                                  && bl, const int fadvise_flags,
                                  Context * user_req);
                ~C_BlockIORequest() override;
                 C_BlockIORequest(const C_BlockIORequest &) = delete;
                 C_BlockIORequest & operator=(const C_BlockIORequest &) =
                    delete;
              ...
              private:
                ...
                false};
                std::atomic < bool > m_finish_called = {
                false};
                std::atomic < bool > m_cell_released = {
                false};
                BlockGuardCell *m_cell = nullptr;

                template < typename U >
                    friend std::ostream & operator<<(std::ostream & os,
                                                     const C_BlockIORequest <
                                                     U > &req);
            };
          ...
          template < typename T > class C_WriteRequest:public C_BlockIORequest < T >
            {
              public:
                ...
                C_WriteRequest(T & pwl, const utime_t arrived, io::Extents
                ...
                C_WriteRequest(T & pwl, const utime_t arrived, io::Extents
                               ...
                               int fadvise_flags, ceph::mutex & lock,
                               PerfCounters * perfcounter, Context * user_req);

                ~C_WriteRequest()override;

                void blockguard_acquired(GuardedRequestFunctionContext &
                                         guard_ctx);
                ...
                virtual std::shared_ptr < WriteLogOperation >
                    create_operation(uint64_t offset, uint64_t len);

                virtual void setup_log_operations(DeferredContexts & on_exit);

                bool append_write_request(std::shared_ptr < SyncPoint >
                                          sync_point);
          ...
          template < typename T > class C_FlushRequest:public C_BlockIORequest < T >
            {
              public:
                ...
                } void setup_buffer_resources(uint64_t * bytes_cached,
                                              uint64_t * bytes_dirtied,
                                              uint64_t * bytes_allocated,
                                              uint64_t * number_lanes,
          ...
          template < typename T > class C_DiscardRequest:public C_BlockIORequest < T >
            {
              public:
                ...
                C_DiscardRequest(T & pwl, const utime_t arrived, io::Extents
                                 && image_extents,
          ...
          template < typename T > class C_WriteSameRequest:public C_WriteRequest < T >
            {
              public:
                using C_BlockIORequest < T >::pwl;
                C_WriteSameRequest(T & pwl, const utime_t arrived, io::Extents
                                   && image_extents, bufferlist
                                   && bl, const int fadvise_flags,
                                   ceph::mutex & lock,
                ...
                std::shared_ptr < WriteLogOperation >
                    create_operation(uint64_t offset, uint64_t len) override;

                const char *get_name() const override {
                    return "C_WriteSameRequest";
                } template < typename U >
                    friend std::ostream & operator<<(std::ostream & os,
                                                     const C_WriteSameRequest <
                                                     U > &req);
            };
            ...
            class GuardedRequestFunctionContext:public Context {
              public:
                ...
                BlockGuardReqState state;
                 GuardedRequestFunctionContext(boost::function <
                                               void
                ...
                GuardedRequestFunctionContext(const
                                              GuardedRequestFunctionContext &) =
                    delete;
                GuardedRequestFunctionContext & operator=(const
                                                          GuardedRequestFunctionContext
                                                          &) = delete;

              private:
                boost::function < void (GuardedRequestFunctionContext &) >
                    m_callback;
            ...
            class GuardedRequest {
              public:
                ...
                GuardedRequestFunctionContext *guard_ctx;   /* Work to do when guard on range obtained */
                 ...
                 GuardedRequest(const BlockExtent block_extent,
                                GuardedRequestFunctionContext *
                                on_guard_acquire, bool barrier = false)
              :    block_extent(block_extent),
                    ...
                    os << "guard_ctx->state=[" << r.guard_ctx->state
                        << "], block_extent.block_start=" << r.block_extent.
                        block_start << ", block_extent.block_end=" << r.
                        block_extent.block_end;
                    return os;
                }
            };
