...
class PGRecoveryStats {
  ...
  public:
    ...
    void dump(ostream & out) {
        std::lock_guard l(lock);
        for (std::map < const char *, per_state_info >::iterator p =
             info.begin(); p != info.end(); ++p) {
...
class PG:public DoutPrefixProvider,
    public PeeringState::PeeringListener, public Scrub::PgScrubBeListener {
  ...
  public:
    ...
    /// scrubbing state for both Primary & replicas bool is_scrub_active() const {
        return m_scrubber->is_scrub_active();
    }
    /// set when the scrub request is queued, and reset after scrubbing fully/// cleaned up.
    ...
    void log_state_exit(const char *state_name, utime_t enter_time,
    ...
    } static void set_last_scrub_stamp(utime_t t, pg_history_t & history,
                                       pg_stat_t & stats) {
    ...
    static void set_last_deep_scrub_stamp(utime_t t, pg_history_t & history,
                                          pg_stat_t & stats) {
    ...
    void set_last_deep_scrub_stamp(utime_t t) {
        recovery_state.update_stats([t] (auto & history, auto & stats) {
                                    set_last_deep_scrub_stamp(t, history,
                                                              stats);
                                    return true;}
        );
    }

    static void add_objects_scrubbed_count(int64_t count, pg_stat_t & stats) {
    ...
    void reset_objects_scrubbed() {
        recovery_state.update_stats([](auto & history, auto & stats) {
                                    reset_objects_scrubbed(stats); return true;}
        );
    }
    ...
    void init(int role,
              ...
              const PastIntervals & pim, ObjectStore::Transaction & t);
    ...
    virtual void split_colls(spg_t child, int split_bits, int seed,
    ...
    void scrub(epoch_t queued, ThreadPool::TPHandle & handle) {
        // a new scrub
        forward_scrub_event(&ScrubPgIF::initiate_regular_scrub, queued,
                            "StartScrub");
    }
   ...
   */
    ...
    void scrub_send_resources_granted(epoch_t queued,
                                      ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_remotes_reserved, queued,
                            "RemotesReserved");
    }

    void scrub_send_resources_denied(epoch_t queued,
                                     ThreadPool::TPHandle & handle) {
    ...
    void scrub_send_scrub_resched(epoch_t queued, ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_scrub_resched, queued,
                            "InternalSchedScrub");
    }
    ...
    void scrub_send_applied_update(epoch_t queued,
                                   ThreadPool::TPHandle & handle) {
    ...
    void scrub_send_unblocking(epoch_t queued, ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_scrub_unblock, queued,
                            "Unblocked");
    }

    void scrub_send_digest_update(epoch_t queued, ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::digest_update_notification, queued,
                            "DigestUpdate");
    }

    void scrub_send_local_map_ready(epoch_t queued,
                                    ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_local_map_done, queued,
                            "IntLocalMapDone");
    }

    void scrub_send_replmaps_ready(epoch_t queued,
                                   ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_replica_maps_ready, queued,
                            "GotReplicas");
    }

    void scrub_send_replica_pushes(epoch_t queued,
                                   ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_replica_pushes_upd, queued,
                            "ReplicaPushesUpd");
    }

    void scrub_send_get_next_chunk(epoch_t queued,
                                   ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_get_next_chunk, queued,
                            "NextChunk");
    }

    void scrub_send_scrub_is_finished(epoch_t queued,
                                      ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_scrub_is_finished, queued,
                            "ScrubFinished");
    }

    void scrub_send_chunk_free(epoch_t queued, ThreadPool::TPHandle & handle) {
        forward_scrub_event(&ScrubPgIF::send_chunk_free, queued,
                            "SelectedChunkFree");
    }
    ...
    } static void add_objects_trimmed_count(int64_t count, pg_stat_t & stats) {
        stats.objects_trimmed += count;
    }

    void add_objects_trimmed_count(int64_t count) {
        recovery_state.update_stats_wo_resched([count]
                                               (auto & history, auto & stats) {
                                               add_objects_trimmed_count(count,
                                                                         stats);});
    }
    ...
    void set_snaptrim_duration() {
        ...
        recovery_state.update_stats_wo_resched([duration]
                                               (auto & history, auto & stats) {
                                               stats.snaptrim_duration =
                                               double (duration);});
    }
    ...
    void request_local_background_io_reservation(unsigned priority,
    ...
    void request_remote_recovery_reservation(unsigned priority,
                                             PGPeeringEventURef on_grant,
                                             PGPeeringEventURef on_preempt)
        override;
    void cancel_remote_recovery_reservation() override;

    void schedule_event_on_commit(ObjectStore::Transaction & t,
    ...
    } PGLog::LogEntryHandlerRef get_log_handler(ObjectStore::
    ...
    void set_ready_to_merge_target(eversion_t lu, epoch_t les,
                                   epoch_t lec) override;
   ...
   */
    ...
    unsigned int scrub_requeue_priority(Scrub::
                                        scrub_prio_t with_priority) const;
  private:
   ...
   */
    ...
    std::optional < requested_scrub_t >
        validate_periodic_mode(bool allow_deep_scrub, bool try_to_auto_repair,
                               bool allow_shallow_scrub, bool time_for_deep,
                               bool has_deep_errors,
                               const requested_scrub_t & planned) const;

    std::optional < requested_scrub_t >
        validate_initiated_scrub(bool allow_deep_scrub, bool try_to_auto_repair,
  ...
  public:
    ...
    virtual void do_command(const std::string_view & prefix,
    ...
    virtual void set_dynamic_perf_stats_queries(const std::list <
  ...
  protected:
    ...
    friend uint64_t get_with_id(PG * pg) {
        return pg->get_with_id();
    }
    friend void put_with_id(PG * pg, uint64_t id) {
  ...
  public:
    ...
    void send_cluster_message(int osd, MessageRef m, epoch_t epoch,
  ...
  protected:
    ...
    /* heartbeat peers */
        void set_probe_targets(const std::set < pg_shard_t >
                               &probe_set) override;
  ...
  public:
    ...
    void add_local_num_bytes(int64_t num_bytes) {
        if (num_bytes) {
            ...
            do {
                ...
                if (new_bytes < 0)
                    new_bytes = 0;
            } while (!local_num_bytes.
                     compare_exchange_weak(prev_bytes, new_bytes));
        }
    }
    ...
    void add_num_bytes(int64_t num_bytes) {
        ...
        if (num_bytes) {
            recovery_state.update_stats([num_bytes]
                                        (auto & history, auto & stats) {
    ...
    int64_t get_stats_num_bytes() {
        ...
        if (lnb && lnb != num_bytes) {
            lgeneric_dout(cct,
                          0) << this << " " << info.
                pgid << " num_bytes mismatch " << lnb << " vs stats " << info.
                stats.stats.sum.num_bytes << " / chunk " << get_pgbackend()->
  ...
  protected:
    ...
    // matter.
    std::unordered_map < entity_name_t,
        std::list < OpRequestRef >> waiting_for_map;
    ...
    std::map < hobject_t,
        std::list < OpRequestRef >> waiting_for_unreadable_object,
    ...
    void requeue_object_waiters(std::map < hobject_t,
                                std::list < OpRequestRef >> &m);
    ...
    void add_backoff(const ceph::ref_t < Session > &s, const hobject_t & begin,
                     const hobject_t & end);
   ...
   */
    ...
    struct QueuePeeringEvt:Context {
        ...
        template < class EVT >
            QueuePeeringEvt(PG * pg, epoch_t epoch, EVT evt):pg(pg),
            evt(std::make_shared < PGPeeringEvent > (epoch, epoch, evt)) {
        } QueuePeeringEvt(PG * pg, PGPeeringEventRef evt):pg(pg),
            evt(std::move(evt)) {
        }
  ...
  protected:
    ...
    } // Primary only bool is_remapped() const {
    ...
    // pg on-disk state void do_pending_flush();

  public:
    void prepare_write(pg_info_t & info,
  ...
  protected:
    ...
    eversion_t get_next_version() const {
        eversion_t at_version(get_osdmap_epoch(),
    ...
    void update_snap_map(const std::vector < pg_log_entry_t > &log_entries,
                         ObjectStore::Transaction & t);
...
// ScrubberPasskey getters/misc:
  public:
    ...
    void force_object_missing(ScrubberPasskey,
                              ...
                              const hobject_t & oid, eversion_t version) final {
