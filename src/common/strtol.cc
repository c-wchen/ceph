...
long long strict_strtoll(const boost::string_view str, int base,
                         std::string * err)
{
    char *endptr;
...
template int strict_iec_cast < int >(const boost::string_view str,
                                     std::string * err);
template long strict_iec_cast < long >(const boost::string_view str,
                                       std::string * err);
template long long strict_iec_cast < long long >(const boost::string_view str,
                                                 std::string * err);
template uint64_t strict_iec_cast < uint64_t > (const boost::string_view str,
                                                std::string * err);
template uint32_t strict_iec_cast < uint32_t > (const boost::string_view str,
                                                std::string * err);
...
template long long strict_iec_cast < long long >(const char *str,
                                                 std::string * err);
template uint64_t strict_iec_cast < uint64_t > (const char *str,
                                                std::string * err);
template uint32_t strict_iec_cast < uint32_t > (const char *str,
                                                std::string * err);
    ...
    T strict_si_cast(const boost::string_view str, std::string * err)
{
    ...
    using promoted_t = typename std::common_type < decltype(ll), T >::type;
    if (static_cast < promoted_t > (ll) <
        static_cast < promoted_t > (std::numeric_limits < T >::min()) / pow(10,
                                                                            m))
    {
    ...
    if (static_cast < promoted_t > (ll) >
        static_cast < promoted_t > (std::numeric_limits < T >::max()) / pow(10,
                                                                            m))
    {
...
template int strict_si_cast < int >(const boost::string_view str,
                                    std::string * err);
template long strict_si_cast < long >(const boost::string_view str,
                                      std::string * err);
template long long strict_si_cast < long long >(const boost::string_view str,
                                                std::string * err);
template uint64_t strict_si_cast < uint64_t > (const boost::string_view str,
                                               std::string * err);
template uint32_t strict_si_cast < uint32_t > (const boost::string_view str,
                                               std::string * err);
...
template long long strict_si_cast < long long >(const char *str,
                                                std::string * err);
template uint64_t strict_si_cast < uint64_t > (const char *str,
                                               std::string * err);
template uint32_t strict_si_cast < uint32_t > (const char *str,
                                               std::string * err);
