    ...
    void encode(ceph::buffer::list & bl) const const
{
  ...
  for (auto i:offsets) {
        map.insert(map.end(),
                   std::make_pair(i.first, (ceph_le32 *) (start + i.second)));
    }
}
void rebuild() {
    ceph::buffer::list bl;
...
class iterator {
    mempool::osdmap::map < pg_t,
        mempool::osdmap::vector < int32_t > >::const_iterator it;
  public:
    ...
    :it(p) {
        ...
        return *it;
    } const std::pair < const pg_t,
        mempool::osdmap::vector < int32_t >> *operator->() const {
        return &*it;
    } friend bool operator==(const iterator & l, const iterator & r) {
        return l.it == r.it;
    }
  ...
  public:
    ...
    class Incremental {
      public:
        ...
        int32_t new_flags;
        ceph_release_t new_require_osd_release {
        0xff};
        uint32_t new_stretch_bucket_count {
        0};
        uint32_t new_degraded_stretch_mode {
        0};
        uint32_t new_recovering_stretch_mode {
        0};
        int32_t new_stretch_mode_bucket {
        0};
        bool stretch_mode_enabled {
        false};
        bool change_stretch_mode {
        false};
        ...
        int32_t new_max_osd;
         ...
         mempool::osdmap::map < std::string, std::map < std::string,
            std::string > >new_erasure_code_profiles;
         ...
         mempool::osdmap::map < int32_t, std::pair < epoch_t,
            epoch_t > >new_last_clean_interval;
         ...
         mempool::osdmap::map < pg_t,
            mempool::osdmap::vector < int32_t >> new_pg_upmap;
         mempool::osdmap::map < pg_t,
            mempool::osdmap::vector < std::pair < int32_t,
        ...
        ceph_release_t new_require_min_compat_client {
        0xff};
        ...
        void encode_client_old(ceph::buffer::list & bl) const const;
        void encode_classic(ceph::buffer::list & bl,
                            uint64_t features) const const;
        void encode(ceph::buffer::list & bl, uint64_t features =
                    CEPH_FEATURES_ALL) const const;
...
void set_allow_crimson() {
  ...
  private:
    ...
    static constexpr uint64_t SIGNIFICANT_FEATURES =
        ...
        CEPH_FEATUREMASK_SERVER_NAUTILUS | CEPH_FEATUREMASK_SERVER_OCTOPUS;

    struct addrs_s {
        mempool::osdmap::vector < std::shared_ptr < entity_addrvec_t >
            >client_addrs;
        mempool::osdmap::vector < std::shared_ptr < entity_addrvec_t >
            >cluster_addrs;
        mempool::osdmap::vector < std::shared_ptr < entity_addrvec_t >
    ...
    mempool::osdmap::map < std::string, std::map < std::string,
        std::string >> erasure_code_profiles;
    ...
    ceph_release_t require_min_compat_client {
  ...
  OSDMap():epoch(0),
        ...
        primary_temp(std::make_shared < mempool::osdmap::map < pg_t,
                     int32_t >> ()),
        osd_uuid(std::make_shared < mempool::osdmap::vector < uuid_d >> ()),
        cluster_snapshot_epoch(0), new_blocklist_entries(false),
        cached_up_osd_features(0), crc_defined(false), crc(0),
  ...
  public:
    ...
    void deepish_copy_from(const OSDMap & o) {
        *this = o;
        primary_temp.reset(new mempool::osdmap::map < pg_t,
                           int32_t > (*o.primary_temp));
    ...
    /* stamps etc */ const utime_t & get_created() const {
        return created;
    } const utime_t & get_modified() const {
        return modified;
    } bool is_blocklisted(const entity_addr_t & a, CephContext * cct =
                          nullptr)const;
    bool is_blocklisted(const entity_addrvec_t & a, CephContext * cct =
                        nullptr) const;
    void get_blocklist(std::list < std::pair < entity_addr_t, utime_t > >*bl,
                       std::list < std::pair < entity_addr_t,
                       utime_t > >*rl) const;
  ...
  /***** cluster state *****/
    ...
    } void adjust_osd_weights(const std::map < int, double >&weights,
    ...
    const std::map < std::string,
        ...
        if (i == erasure_code_profiles.end())
            return empty;
             ...
             return i->second;
    } const mempool::osdmap::map < std::string, std::map < std::string,
        std::string >> &get_erasure_code_profiles() const {
    ...
    bool is_noup_by_osd(int osd) const {
        ...
        if (get_osd_crush_node_flags(osd) & CEPH_OSD_NOUP)  // by crush-node?
            return true;
        if (auto class_id = crush->get_item_class_id(osd); class_id >= 0 && get_device_class_flags(class_id) & CEPH_OSD_NOUP)   // by device-class?
            return true;
        return false;
   ...
   */ bool subtree_is_down(int id, std::set < int >*down_cache)const;
    bool containing_subtree_is_down(CephContext * cct, int osd,
                                    int subtree_type,
                                    std::set < int >*down_cache) const;

    bool subtree_type_is_down(CephContext * cct, int id, int subtree_type,
                              std::set < int >*down_in_osds,
                              std::set < int >*up_in_osds,
                              std::set < int >*subtree_up,
                              std::unordered_map < int,
                              std::set < int > >*subtree_type_down) const;
    ...
    void get_random_up_osds_by_subtree(int n,   // whoami
                                       std::string & subtree, int limit,    // how many
                                       std::set < int >skip,
   ...
   */
    ...
    bool check_pg_upmaps(CephContext * cct,
                         const std::vector < pg_t > &to_check,
                         std::vector < pg_t > *to_cancel,
                         std::map < pg_t,
                         mempool::osdmap::vector < std::pair < int,
                         int >>>*to_remap) const;
    void clean_pg_upmaps(CephContext * cct, Incremental * pending_inc,
  ...
  public:
    void encode(ceph::buffer::list & bl, uint64_t features =
                CEPH_FEATURES_ALL) const;
    void decode(ceph::buffer::list & bl);
    void decode(ceph::buffer::list::const_iterator & bl);

  /****   mapping facilities   ****/
    int map_to_pg(int64_t pool,
                  const std::string & name,
                  const std::string & key,
                  const std::string & nspace, pg_t * pg) const;
    ...
    pg_t object_locator_to_pg(const object_t & oid,
        ...
        int ret = object_locator_to_pg(oid, loc, pg);
         ceph_assert(ret == 0);
         return pg;
    } static object_locator_t file_to_object_locator(const file_layout_t &
                                                     layout) {
  ...
  private:
    /// pg -> (raw osd std::list)
    void _pg_to_raw_osds(const pg_pool_t & pool, pg_t pg,
                         std::vector < int >*osds, ps_t * ppps) const;
    int _pick_primary(const std::vector < int >&osds) const;
    void _remove_nonexistent_osds(const pg_pool_t & pool,
    ...
    void _apply_upmap(const pg_pool_t & pi, pg_t pg,
   ...
   */
    void _pg_to_up_acting_osds(const pg_t & pg, std::vector < int >*up,
                               int *up_primary, std::vector < int >*acting,
                               int *acting_primary, bool raw_pg_to_pg =
                               true) const;
   ...
   */
    ...
    } void pg_to_up_acting_osds(pg_t pg, std::vector < int >&up,
                                std::vector < int >&acting) const {
        ...
        for (auto & p:pools) {
            if (p.second.get_crush_rule() == rule_id) {
                pool_ids->insert(p.first);
                                 ...
                                 int osd, std::set < int64_t > *pool_ids) const;
    ...
    // pg -> acting primary osd int get_pg_acting_primary(pg_t pg) const {
        int primary = -1;
    ...
    static int calc_pg_role_broken(int osd, const std::vector < int >&acting,
                                   int nrep = 0);
    static int calc_pg_role(pg_shard_t who, const std::vector < int >&acting);
    static bool primary_changed_broken(int oldprimary,
    ...
    int get_pg_acting_role(spg_t pg, int osd) const {
        std::vector < int >group;
         ...
         return calc_pg_role(pg_shard_t(osd, pg.shard), group);
    } bool try_pg_upmap(CephContext * cct, pg_t pg, ///< pg to potentially remap
                        ...
                        std::vector < int >*orig, std::vector < int >*out); ///< resulting alternative mapping

    int balance_primaries(CephContext * cct,
                          int64_t pid,
                          Incremental * pending_inc,
                          OSDMap & tmp_osd_map) const;

    int calc_desired_primary_distribution(CephContext * cct, int64_t pid,   // pool id
                                          const std::vector < uint64_t > &osds, std::map < uint64_t, float >&desired_primary_distribution) const;   // vector of osd ids

    int calc_pg_upmaps(CephContext * cct, uint32_t max_deviation,   ///< max deviation from target (value >= 1)
                       int max_iterations,  ///< max iterations to run
                       const std::set < int64_t > &pools,   ///< [optional] restrict to pool
                       Incremental * pending_inc, std::random_device::result_type * p_seed = nullptr    ///< [optional] for regression tests
        );

    std::map < uint64_t, std::set < pg_t >> get_pgs_by_osd(CephContext * cct, int64_t pid, std::map < uint64_t, std::set < pg_t >> *p_primaries_by_osd = nullptr, std::map < uint64_t, std::set < pg_t >> *p_acting_primaries_by_osd = nullptr) const;  // used in calc_desired_primary_distribution()

  private:                     // Bunch of internal functions used only by calc_pg_upmaps (result of code refactoring)

    float get_osds_weight(CephContext * cct,
                          const OSDMap & tmp_osd_map,
                          int64_t pid,
                          std::map < int, float >&osds_weight) const;

    float build_pool_pgs_info(CephContext * cct, const std::set < int64_t > &pools, ///< [optional] restrict to pool
                              const OSDMap & tmp_osd_map, int &total_pgs, std::map < int, std::set < pg_t >> &pgs_by_osd, std::map < int, float >&osds_weight); // return total weight of all OSDs

    float calc_deviations(CephContext * cct, const std::map < int, std::set < pg_t >> &pgs_by_osd, const std::map < int, float >&osd_weight, float pgs_per_weight, std::map < int, float >&osd_deviation, std::multimap < float, int >&deviation_osd, float &stddev);   // return current max deviation

    void fill_overfull_underfull(CephContext * cct,
                                 const std::multimap < float,
                                 int >&deviation_osd, int max_deviation,
                                 std::set < int >&overfull,
                                 std::set < int >&more_overfull,
                                 std::vector < int >&underfull,
                                 std::vector < int >&more_underfull);

    int pack_upmap_results(CephContext * cct,
                           const std::set < pg_t > &to_unmap,
                           const std::map < pg_t,
                           mempool::osdmap::vector < std::pair < int,
                           int >>>&to_upmap, OSDMap & tmp_osd_map,
                           OSDMap::Incremental * pending_inc);

    std::default_random_engine get_random_engine(CephContext * cct,
                                                 std::random_device::
                                                 result_type * p_seed);

    bool try_drop_remap_overfull(CephContext * cct,
                                 const std::vector < pg_t > &pgs,
                                 const OSDMap & tmp_osd_map,
                                 int osd,
                                 std::map < int,
                                 std::set < pg_t >> &temp_pgs_by_osd,
                                 std::set < pg_t > &to_unmap, std::map < pg_t,
                                 mempool::osdmap::vector < std::pair < int32_t,
                                 int32_t >>> &to_upmap);

    typedef std::vector < std::pair < pg_t,
        mempool::osdmap::vector < std::pair < int, int >>>> candidates_t;

    bool try_drop_remap_underfull(CephContext * cct,
    ...
    void add_remap_pair(CephContext * cct,
                        int orig,
                        int out,
                        pg_t pg,
                        size_t pg_pool_size,
                        ...
                        mempool::osdmap::vector < std::pair < int32_t,
                        int32_t >> new_upmap_items, std::map < pg_t,
                        mempool::osdmap::vector < std::pair < int32_t,
                        int32_t >>> &to_upmap);

    int find_best_remap(CephContext * cct,
                        const std::vector < int >&orig,
                        const std::vector < int >&out,
                        const std::set < int >&existing,
                        const std::map < int, float >osd_deviation);

    candidates_t build_candidates(CephContext * cct,
                                  const OSDMap & tmp_osd_map,
  ...
  public:
    ...
    //
    int calc_read_balance_score(CephContext * cct,
                                int64_t pool_id,
  ...
  public:
    int get_osds_by_bucket_name(const std::string & name,
                                std::set < int >*osds) const;
  ...
  public:
    ...
    static int build_simple_crush_map_from_conf(CephContext * cct,
                                                CrushWrapper & crush,
                                                std::ostream * ss);
    static int build_simple_crush_rules(CephContext * cct, CrushWrapper & crush,
  ...
  public:
    ...
    int summarize_mapping_stats(OSDMap * newmap,
                                const std::set < int64_t > *pools,
                                std::string * out, ceph::Formatter * f) const;

    std::string get_flag_string()const;
    static std::string get_flag_string(unsigned flags);
    static void dump_erasure_code_profiles(const mempool::osdmap::map <
    ...
    void dump_pool(CephContext * cct, int64_t pid, const pg_pool_t & pdata,
                   ceph::Formatter * f) const;
    void dump_read_balance_score(CephContext * cct, int64_t pid,
                                 const pg_pool_t & pdata,
                                 ceph::Formatter * f) const;
    static void generate_test_instances(std::list < OSDMap * >&o);
    bool check_new_blocklist_entries() const {
    ...
    int parse_osd_id_list(const std::vector < std::string > &ls,
                          std::set < int >*out, std::ostream * ss) const;
...
void print_osd_utilization(const OSDMap & osdmap,
                           ...
                           std::ostream & out,
                           ceph::Formatter * f,
                           bool tree, const std::string & filter);
