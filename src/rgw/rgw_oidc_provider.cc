...
namespace rgw {
    namespace sal {
        ...
        int RGWOIDCProvider::get_tenant_url_from_arn(string & tenant,
                                                     string & url) {
        ...
        int RGWOIDCProvider::create(const DoutPrefixProvider * dpp,
        ...
        int RGWOIDCProvider::get(const DoutPrefixProvider * dpp) {
            ...
            if (this->tenant != tenant) {
                ldpp_dout(dpp,
                          0) <<
                    "ERROR: tenant in arn doesn't match that of user " << this->
                    tenant << ", " << tenant << ": " << dendl;
                return -EINVAL;
            }
        ...
        bool RGWOIDCProvider::validate_input(const DoutPrefixProvider * dpp) {
          ...
          for (auto & it:client_ids) {
            ...
            if (thumbprints.size() > MAX_OIDC_NUM_THUMBPRINTS) {
                ldpp_dout(dpp,
                          0) << "ERROR: Invalid number of thumbprints " <<
                    thumbprints.size() << dendl;
                return false;
            }
