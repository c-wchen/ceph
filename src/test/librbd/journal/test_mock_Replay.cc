...
struct MockReplayImageCtx: public MockImageCtx {
    explicit MockReplayImageCtx(ImageCtx &
                                image_ctx): MockImageCtx(image_ctx)
    {
    }
};
...
template <> struct ImageRequest <MockReplayImageCtx > {
    ...
    {
        ceph_assert(s_instance != nullptr);
        s_instance->aio_discard(c, image_extents,
    ...
    MOCK_METHOD4(aio_writesame, void (AioCompletion *c,
                                      ...
                                      const bufferlist &bl,
                                      int op_flags));
    static void aio_writesame(MockReplayImageCtx *ictx,
                              AioCompletion *c, Extents
                              && image_extents, ImageArea area,
                              bufferlist
                              && bl, int op_flags,
    ...
    MOCK_METHOD6(aio_compare_and_write,
    ...
    static void aio_compare_and_write(MockReplayImageCtx *ictx,
                                      AioCompletion *c, Extents
                                      && image_extents, ImageArea area,
                                      bufferlist && cmp_bl, bufferlist
...
ImageRequest < MockReplayImageCtx > *ImageRequest <
MockReplayImageCtx >::s_instance = nullptr;
...
public:
    ...
    void expect_accept_ops(MockExclusiveLock &mock_exclusive_lock,
                           bool accept)
    {
        EXPECT_CALL(mock_exclusive_lock,
                    accept_ops()).WillRepeatedly(Return(accept));
    } void expect_aio_discard(MockIoImageRequest &
                              mock_io_image_request,
                              io::AioCompletion **aio_comp,
                              uint64_t off, uint64_t len,
                              uint32_t discard_granularity_bytes)
    {
        EXPECT_CALL(mock_io_image_request, aio_discard(_, io::Extents { {
                off, len
            }},
        discard_granularity_bytes))
                          ...
                          int r)
    {
        EXPECT_CALL(mock_io_image_request, aio_flush(_))
        .
        WillOnce(CompleteAioCompletion
                 (r, mock_image_ctx.image_ctx));
    }
    ...
    {
        EXPECT_CALL(mock_io_image_request, aio_write(_, io::Extents { {
                off, len
            }},
        BufferlistEqual
        (data), _))
    ...
    void expect_aio_writesame(MockIoImageRequest &
                              mock_io_image_request,
                              io::AioCompletion **aio_comp,
                              uint64_t off, uint64_t len,
                              const char *data)
    {
        EXPECT_CALL(mock_io_image_request, aio_writesame(_, io::Extents { {
                off, len
            }},
        BufferlistEqual
        (data), _))
    ...
    void expect_aio_compare_and_write(MockIoImageRequest &
                                      mock_io_image_request,
                                      io::AioCompletion **aio_comp,
                                      uint64_t off, uint64_t len,
                                      const char *cmp_data,
                                      ...
                                      uint64_t *mismatch_offset)
    {
        ...
        aio_compare_and_write(_, io::Extents { {
                off, len
            }},
    ...
    void expect_flatten(MockReplayImageCtx &mock_image_ctx,
                        Context **on_finish)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_flatten(_, _))
        .WillOnce(DoAll(SaveArg < 1 > (on_finish),
                        NotifyInvoke(&m_invoke_lock,
                                     &m_invoke_cond)));
    }

    void expect_rename(MockReplayImageCtx &mock_image_ctx,
                       Context **on_finish, const char *image_name)
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_rename(StrEq(image_name), _))
        .
        WillOnce(DoAll
                 (SaveArg < 1 > (on_finish),
    ...
    void expect_resize(MockReplayImageCtx &mock_image_ctx,
                       Context **on_finish, uint64_t size,
                       uint64_t op_tid)
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_resize(size, _, _, _, op_tid))
        .
        WillOnce(DoAll
                 (SaveArg < 3 > (on_finish),
                  NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    ...
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_snap_remove(_, StrEq(snap_name), _))
        .
        WillOnce(DoAll
                 (SaveArg < 2 > (on_finish),
                  NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    ...
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_snap_protect(_, StrEq(snap_name), _))
        .
        WillOnce(DoAll
                 (SaveArg < 2 > (on_finish),
                  NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    ...
    void expect_update_features(MockReplayImageCtx &mock_image_ctx,
                                Context **on_finish, uint64_t features,
                                bool enabled, uint64_t op_tid)
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_update_features(features, enabled, _,
                                            op_tid))
        .
        WillOnce(DoAll
                 (SaveArg < 2 > (on_finish),
    ...
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_metadata_remove(StrEq(key), _))
        .
        WillOnce(DoAll
                 (SaveArg < 1 > (on_finish),
                  NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    ...
    void expect_refresh_image(MockReplayImageCtx &mock_image_ctx,
                              bool required, int r)
    {
    ...
    {
        aio_comp->get();
        aio_comp->init_time(mock_image_ctx.image_ctx,
    ...
    {
        {
            std::unique_lock locker {
                m_invoke_lock};
            m_invoke_cond.wait(locker, [on_finish] {
                return *on_finish != nullptr;
            }
                              );
        }
...
TEST_F(TestMockJournalReplay, AioDiscard)
{
    ...
    when_process(mock_journal_replay, EventEntry {
...
TEST_F(TestMockJournalReplay, AioWrite)
{
    ...
    C_SaferCond on_ready;
    C_SaferCond on_safe;
    expect_aio_write(mock_io_image_request, &aio_comp, 123, 456,
                     "test");
    when_process(mock_journal_replay, EventEntry {
...
TEST_F(TestMockJournalReplay, AioWriteSame)
{
    ...
    C_SaferCond on_ready;
    C_SaferCond on_safe;
    expect_aio_writesame(mock_io_image_request, &aio_comp, 123, 456,
                         "333");
    when_process(mock_journal_replay, EventEntry {
...
TEST_F(TestMockJournalReplay, AioCompareAndWrite)
{
    ...
    C_SaferCond on_ready;
    C_SaferCond on_safe;
    expect_aio_write(mock_io_image_request, &aio_comp, 512, 512,
                     "test");
    when_process(mock_write_journal_replay, EventEntry {
    ...
    when_process(mock_compare_and_write_journal_replay, EventEntry {
    ...
    expect_aio_flush(mock_image_ctx, mock_io_image_request, 0);
    ASSERT_EQ(0,
              when_shut_down(mock_compare_and_write_journal_replay,
                             false));
    ASSERT_EQ(0, on_safe.wait());

    expect_aio_compare_and_write(mock_io_image_request, &aio_comp,
                                 512, 512, "111", "test", nullptr);
    when_process(mock_mis_compare_and_write_journal_replay, EventEntry {
    ...
    expect_aio_flush(mock_image_ctx, mock_io_image_request, 0);
    ASSERT_EQ(0,
              when_shut_down(mock_mis_compare_and_write_journal_replay,
                             false));
    ASSERT_EQ(0, on_safe.wait());
...
TEST_F(TestMockJournalReplay, PauseIO)
{
    ...
    for (size_t i = 0; i < io_count; ++i) {
        ...
        if ((i + 1) % 32 == 0) {
            ...
            expect_aio_flush(mock_io_image_request,
                             &flush_comps.back());
        }
        when_process(mock_journal_replay, EventEntry {
            AioWriteEvent(123, 456, to_bl("test"))}
...
TEST_F(TestMockJournalReplay, MissingOpFinishEvent)
{
    ...
    expect_snap_create(mock_image_ctx, &on_snap_create_finish, "snap",
                       123);
    ...
    when_replay_op_ready(mock_journal_replay, 123,
                         &on_snap_create_resume);
...
TEST_F(TestMockJournalReplay, UpdateFeaturesEvent)
{
    ...
    Context *on_finish = nullptr;
    expect_refresh_image(mock_image_ctx, false, 0);
    expect_update_features(mock_image_ctx, &on_finish, features,
                           enabled, 123);

    C_SaferCond on_start_ready;
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay, EventEntry {
        UpdateFeaturesEvent(123, features, enabled)}
    , &on_start_ready, &on_start_safe);
