  ...
  public:
    ...
    bool get_id(rgw_user & _id) const {
        ...
        case ACL_TYPE_EMAIL_USER:
            ...
            return true;
            case ACL_TYPE_GROUP:case ACL_TYPE_REFERER:return false;
            default:_id = id;
            return true;
        ...
        case ACL_TYPE_EMAIL_USER:
            ...
            return &email_id;
            case ACL_TYPE_GROUP:case ACL_TYPE_REFERER:return nullptr;
            default:return &id;
    }} ACLGranteeType & get_type() {
        return type;
    }
    const ACLGranteeType & get_type() const {
        return type;
    } ACLPermission & get_permission() {
        return permission;
    }
    const ACLPermission & get_permission() const {
        return permission;
    } ACLGroupTypeEnum get_group() const {
        return group;
    ...
    void set_canon(const rgw_user & _id, const std::string & _name,
                   const uint32_t perm) {
...
uint32_t get_perm(const DoutPrefixProvider * dpp,
                  ...
                  uint32_t perm_mask);
uint32_t get_group_perm(const DoutPrefixProvider * dpp, ACLGroupTypeEnum group,
                        uint32_t perm_mask) const;
uint32_t get_referer_perm(const DoutPrefixProvider * dpp, uint32_t current_perm,
                          std::string http_referer, uint32_t perm_mask);
void encode(bufferlist & bl) const {
    ENCODE_START(4, 3, bl);
...
ACLGrantMap & get_grant_map() {
    return grant_map;
}
const ACLGrantMap & get_grant_map() const {
    return grant_map;
...
friend bool operator==(const RGWAccessControlList & lhs,
...
const RGWAccessControlList & get_acl() const {
    return acl;
} virtual bool compare_group_name(std::string & id, ACLGroupTypeEnum group) {
    return false;
}
bool is_public(const DoutPrefixProvider * dpp)const;

friend bool operator==(const RGWAccessControlPolicy & lhs,
