...
public:
    ...
    typename futurator_t::type get_future()
    {
        return on_done.wait().then([this](size_t) {
            if (state.failed()) {
                return futurator_t::
                       make_exception_future(state.
                                             get_exception());
            } else {
                return futurator_t::from_tuple(state.
                                               get_value
                                               ());
            }
        }
...
private:
    ...
    crimson::counting_semaphore < QUEUE_SIZE > sem {
        0};
...
public:
    ...
    template < typename Func > auto submit(Func && func)
    {
