...
template <
    ...
    typename V,
    typename VINT,
    bool VALIDATE_INVARIANTS = true > class FixedKVNodeLayout
{
    char *buf = nullptr;

    using L =
        absl::container_internal::Layout < ceph_le32, MetaInt, KINT, VINT >;
    static constexpr L layout {
        1, 1, CAPACITY, CAPACITY};

public:
    template < bool is_const > struct iter_t {
        friend class FixedKVNodeLayout;
        using parent_t =
            typename maybe_const_t < FixedKVNodeLayout, is_const >::type;

        parent_t node;
        uint16_t offset = 0;

        iter_t() = default;
        iter_t(parent_t parent,
               uint16_t offset): node(parent), offset(offset)
        {
        } iter_t(const iter_t &) noexcept = default;
        iter_t(iter_t &&) noexcept = default;
        template < bool is_const_ = is_const > iter_t(const iter_t < false > &it, std::enable_if_t < is_const_,
                int > = 0)
            :    iter_t {
            it.node, it.offset}
        {
        ...
        friend bool operator==(const iter_t < is_const > &lhs,
                               const iter_t < !is_const > &rhs)
        {
            assert(lhs.node == rhs.node);
            return lhs.offset == rhs.offset;
        }
        ...
        } typename maybe_const_t < char, is_const >::type get_key_ptr() const
        {
            return reinterpret_cast <
                   typename maybe_const_t < char,
                   is_const >::type >(node->get_key_ptr() + offset);
        } typename maybe_const_t < char, is_const >::type get_val_ptr() const
        {
            return reinterpret_cast <
                   typename maybe_const_t < char,
                   is_const >::type >(node->get_val_ptr() + offset);
        }
    };
...
public:
    ...
    public:
        ...
        } void insert(const K &key, const V &val)
        {
            KINT k;
            k = key;
            buffer.push_back(delta_t {
                delta_t::op_t::INSERT, k, VINT(val)
            });
        }
        void update(const K &key, const V &val)
        {
            ...
            k = key;
            buffer.push_back(delta_t {
                delta_t::op_t::UPDATE, k, VINT(val)
            });
        }
        ...
        } void copy_out(char *out, size_t len)
        {
            ...
            ::memcpy(out, reinterpret_cast < const void *>(buffer.data()),
                     get_bytes());
            buffer.clear();
        }
        void copy_in(const char *out, size_t len)
        {
            ...
            buffer = std::vector(reinterpret_cast < const delta_t * >(out),
                                 reinterpret_cast <
                                 const delta_t * >(out + len));
        }
    ...
    void journal_insert(const_iterator _iter,
                        const K &key,
                        const V &val, delta_buffer_t *recorder)
    {
    ...
    void journal_update(const_iterator _iter,
                        const V &val, delta_buffer_t *recorder)
    {
        auto iter = iterator(this, _iter.offset);
        if (recorder) {
    ...
    void journal_replace(const_iterator _iter,
                         const K &key,
                         const V &val, delta_buffer_t *recorder)
    {
        auto iter = iterator(this, _iter.offset);
    ...
    FixedKVNodeLayout(char *buf): buf(buf)
    {
    }
    ...
    {
        auto it =
            std::lower_bound(boost::make_counting_iterator < uint16_t > (0),
    ...
    {
        auto it =
            std::upper_bound(boost::make_counting_iterator < uint16_t > (0),
                             boost::make_counting_iterator < uint16_t >
                             (get_size()),
                             l,
     ...
     */
    static K balance_into_new_nodes(const FixedKVNodeLayout &left,
                                    const FixedKVNodeLayout &right,
                                    bool prefer_left,
                                    ...
                                    FixedKVNodeLayout &replacement_right)
    {
        ...
        if (pivot_idx < left.get_size()) {
            replacement_left.copy_from_foreign(replacement_left.end(),
                                               left.begin(),
                                               left.iter_idx(pivot_idx));
            replacement_left.set_size(pivot_idx);

            replacement_right.copy_from_foreign(replacement_right.end(),
                                                left.iter_idx(pivot_idx),
                                                left.end());
        ...
        auto[lmeta, rmeta] =
            Meta::rebalance(left.get_meta(), right.get_meta(),
                            replacement_pivot);
...
private:
        ...
        const_iterator to_src)
    {
        ...
        memcpy(tgt->get_val_ptr(), from_src->get_val_ptr(),
               to_src->get_val_ptr() - from_src->get_val_ptr());
        memcpy(tgt->get_key_ptr(), from_src->get_key_ptr(),
     ...
     */
    static void copy_from_local(iterator tgt,
                                iterator from_src, iterator to_src)
    {
        assert(tgt->node == from_src->node);
        assert(to_src->node == from_src->node);
        memmove(tgt->get_val_ptr(), from_src->get_val_ptr(),
                to_src->get_val_ptr() - from_src->get_val_ptr());
        memmove(tgt->get_key_ptr(), from_src->get_key_ptr(),
                to_src->get_key_ptr() - from_src->get_key_ptr());
    }
};
