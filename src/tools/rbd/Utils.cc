...
namespace rbd {
    namespace utils {
        ...
        namespace po = boost::program_options;
         ...
         namespace {
            ...
            static std::string mgr_command_args_to_str(const std::map <
                                                       std::string,
                                                       std::string > &args) {
                std::string out = "";
            ...
            int ProgressContext::update_progress(uint64_t offset,
                                                 uint64_t total) {
            if (progress) {
        ...
        void ProgressContext::finish() {
            if (progress) {
                std::
                    cerr << "\r" << operation << ": 100% complete...done." <<
                    std::endl;
            }
        }
        ...
        void ProgressContext::fail() {
            if (progress) {
                std::
                    cerr << "\r" << operation << ": " << last_pc <<
                    "% complete...failed." << std::endl;
            }
        }
        ...
        std::string get_positional_argument(const po::variables_map & vm,
                                            size_t index) {
        ...
        std::string get_default_pool_name() {
            return g_ceph_context->_conf.get_val < std::string >
                ("rbd_default_pool");
        }

        int get_pool_and_namespace_names(const boost::program_options::
                                         variables_map & vm,
            ...
            if (validate_pool_name &&
                ...
                std::cerr << "rbd: invalid pool '" << *pool_name << "'" << std::
                    endl;
                return -EINVAL;
            }
                ...
                std::
                    cerr << "rbd: invalid namespace '" << *namespace_name << "'"
                    << std::endl;
                return -EINVAL;
            }
        ...
        int get_pool_image_id(const po::variables_map & vm,
            ...
            int r;
            if (image_id != nullptr && spec_arg_index != nullptr
                && image_id->empty()) {
                std::string spec =
                    get_positional_argument(vm, (*spec_arg_index)++);
                if (!spec.empty()) {
                    r = extract_spec(spec, pool_name, namespace_name, image_id,
                                     nullptr, SPEC_VALIDATION_FULL);
                    if (r < 0) {
                        return r;
                    }
                }
        ...
        int get_image_or_snap_spec(const po::variables_map & vm,
                                   std::string * spec) {
            size_t arg_index = 0;
            std::string pool_name;
            ...
            std::string snap_name;
            int r =
                get_pool_image_snapshot_names(vm, at::ARGUMENT_MODIFIER_NONE,
                                              &arg_index, &pool_name,
                                              &nspace_name,
        ...
        int get_pool_image_snapshot_names(const po::variables_map & vm,
            ...
            return get_pool_generic_snapshot_names(vm, mod, spec_arg_index,
                                                   pool_key, pool_name,
                                                   namespace_name, image_key,
                                                   "image", image_name,
                                                   snap_name,
                                                   image_name_required,
                                                   snapshot_presence,
        ...
        int get_pool_generic_snapshot_names(const po::variables_map & vm,
            ...
            std::string namespace_key =
                (mod ==
                 at::ARGUMENT_MODIFIER_DEST ? at::DEST_NAMESPACE_NAME : at::
                 NAMESPACE_NAME);
            std::string snap_key =
                (mod ==
                 at::ARGUMENT_MODIFIER_DEST ? at::DEST_SNAPSHOT_NAME : at::
                 SNAPSHOT_NAME);
            ...
            int r;
            if ((generic_key == at::IMAGE_NAME
                 || generic_key == at::DEST_IMAGE_NAME)
                ...
                r = extract_spec(image_name_copy, pool_name, namespace_name,
                                 generic_name, snap_name, spec_validation);
                if (r < 0) {
                    return r;
                }
            }
            ...
            if (generic_name != nullptr && generic_name_required &&
                ...
                std::cerr << "rbd: "
                    ...
                    << generic_key_desc << " name was not specified" << std::
                    endl;
                return -EINVAL;
            }
            ...
            if (spec_validation == SPEC_VALIDATION_FULL) {
                ...
                if ((pool_name != nullptr)
                    && !std::regex_match(*pool_name, pattern)) {
                    std::
                        cerr << "rbd: invalid pool name '" << *pool_name << "'"
                        << std::endl;
                    return -EINVAL;
                }
            }
            ...
            if (namespace_name != nullptr && !namespace_name->empty() &&
                ...
                std::
                    cerr << "rbd: invalid namespace name '" << *namespace_name
                    << "'" << std::endl;
                return -EINVAL;
            }
        ...
        int validate_snapshot_name(at::ArgumentModifier mod,
            ...
            if (spec_validation == SPEC_VALIDATION_SNAP) {
                ...
                if (!std::regex_match(snap_name, pattern)) {
                    std::
                        cerr << "rbd: invalid snap name '" << snap_name << "'"
                        << std::endl;
                    return -EINVAL;
                }
            }
        ...
        int get_image_options(const boost::program_options::variables_map & vm,
            ...
            uint64_t order = 0, stripe_unit = 0, stripe_count = 0, object_size =
                0;
            ...
            if (vm.count(at::IMAGE_SHARED)
                && vm[at::IMAGE_SHARED].as < bool > ()) {
            ...
            if (get_format) {
                ...
                if (format == 1) {
                    std::cerr << "rbd: image format 1 is deprecated" << std::
                        endl;
                }
                ...
                if (!data_pool.empty()) {
                    if (format_specified && format == 1) {
                        std::
                            cerr << "rbd: data pool not allowed with format 1; "
                            << "use --image-format 2" << std::endl;
                        return -EINVAL;
                    }
                    else {
                ...
                if (format_specified) {
                    int r =
                        g_conf().set_val("rbd_default_format",
                                         stringify(format));
                    ceph_assert(r == 0);
            ...
            if (vm.count(at::IMAGE_MIRROR_IMAGE_MODE)) {
                opts->set(RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE,
                          vm[at::IMAGE_MIRROR_IMAGE_MODE].as <
        ...
        int get_journal_options(const boost::program_options::
                                variables_map & vm,
            ...
            if (vm.count(at::JOURNAL_SPLAY_WIDTH)) {
                ...
                int r = g_conf().set_val("rbd_journal_splay_width",
                                         stringify(vm[at::JOURNAL_SPLAY_WIDTH].
                                                   as < uint64_t > ()));
                ceph_assert(r == 0);
            }
        ...
        int get_flatten_option(const boost::program_options::variables_map & vm,
                               librbd::ImageOptions * opts) {
            if (vm.count(at::IMAGE_FLATTEN)
                && vm[at::IMAGE_FLATTEN].as < bool > ()) {
                uint64_t flatten = 1;
        ...
        int get_formatter(const po::variables_map & vm,
            ...
            if (vm.count(at::FORMAT)) {
                ...
                *formatter =
                    vm[at::FORMAT].as < at::Format >
                    ().create_formatter(pretty);
                if (*formatter == nullptr && pretty) {
                    std::
                        cerr << "rbd: --pretty-format only works when --format "
        ...
        int get_snap_create_flags(const po::variables_map & vm,
                                  uint32_t * flags) {
            if (vm[at::SKIP_QUIESCE].as < bool > ()
                && vm[at::IGNORE_QUIESCE_ERROR].as < bool > ()) {
                std::cerr << "rbd: " << at::
                    IGNORE_QUIESCE_ERROR << " cannot be used together with " <<
                    at::SKIP_QUIESCE << std::endl;
                return -EINVAL;
            }
        ...
        int get_encryption_options(const boost::program_options::
            ...
            if (vm.count(at::ENCRYPTION_FORMAT)) {
                ...
                formats.resize(passphrase_files.size(), at::EncryptionFormat {
            ...
            for (size_t i = 0; i < formats.size(); ++i) {
                std::ifstream file(passphrase_files[i],
                ...
                std::
                    string passphrase((std::istreambuf_iterator < char >(file)),
        ...
        void init_context() {
            g_conf().set_val_or_die("rbd_cache_writethrough_until_flush",
                                    "false");
        ...
        int init_rados(librados::Rados * rados) {
            ...
            r = rados->connect();
            if (r < 0) {
                std::cerr << "rbd: couldn't connect to the cluster!" << std::
                    endl;
                return r;
            }
        ...
        int init(const std::string & pool_name,
                 const std::string & namespace_name, librados::Rados * rados,
        ...
        int init_io_ctx(librados::Rados & rados, std::string pool_name,
            ...
            if (r < 0) {
                if (r == -ENOENT && pool_name == get_default_pool_name()) {
                    ...
                    std::
                        cerr << "rbd: error opening pool '" << pool_name <<
        ...
        int set_namespace(const std::string & namespace_name,
                          librados::IoCtx * io_ctx) {
            if (!namespace_name.empty()) {
                librbd::RBD rbd;
                bool exists = false;
                int r =
                    rbd.namespace_exists(*io_ctx, namespace_name.c_str(),
                                         &exists);
                if (r < 0) {
                ...
                if (!exists) {
                    std::
                        cerr << "rbd: namespace '" << namespace_name <<
        ...
        int open_image(librados::IoCtx & io_ctx, const std::string & image_name,
            ...
            librbd::RBD rbd;
            if (read_only) {
                r = rbd.open_read_only(io_ctx, *image, image_name.c_str(),
                                       NULL);
            }
            else {
        ...
        int open_image_by_id(librados::IoCtx & io_ctx,
                             const std::string & image_id, bool read_only,
                             librbd::Image * image) {
            int r;
            librbd::RBD rbd;
            if (read_only) {
                r = rbd.open_by_id_read_only(io_ctx, *image, image_id.c_str(),
                                             NULL);
            }
            else {
            ...
            if (r < 0) {
                std::
                    cerr << "rbd: error opening image with id " << image_id <<
                    ": " << cpp_strerror(r) << std::endl;
                return r;
            }
            return 0;
        ...
        int snap_set(librbd::Image & image, const std::string & snap_name) {
            ...
            if (r < 0) {
                std::
                    cerr << "error setting snapshot context: " <<
                    cpp_strerror(r)
                    << std::endl;
                return r;
            }
        ...
        std::string mirror_image_status_state(librbd::
                                              mirror_image_status_state_t state)
        {
            ...
            case MIRROR_IMAGE_STATUS_STATE_STOPPED:
                return "stopped";
            default:
                return "unknown (" + stringify(static_cast < uint32_t >
                                               (state)) + ")";
            }
        }

        std::string mirror_image_site_status_state(const librbd::
        ...
        std::string mirror_image_global_status_state(const librbd::
        ...
        int get_local_mirror_image_status(const librbd::
        ...
        bool is_not_user_snap_namespace(librbd::Image * image,
            ...
            int r =
                image->snap_get_namespace_type(snap_info.id, &namespace_type);
            if (r < 0) {
                return false;
            }
        ...
        void get_mirror_peer_sites(librados::IoCtx & io_ctx,
                                   std::vector < librbd::mirror_peer_site_t >
                                   *mirror_peers) {
        ...
        void get_mirror_peer_mirror_uuids_to_names(const std::vector <
        ...
        void populate_unknown_mirror_image_site_statuses(const std::vector <
                                                         librbd::
                                                         mirror_peer_site_t >
                                                         &mirror_peers,
                                                         librbd::
    ...
    })"; bufferlist in_bl; bufferlist out_bl; std::string outs; int r = rados.mgr_command(command, in_bl, &out_bl, &outs); if (r < 0) {
                                                                               (*err_os) << "rbd: " << cmd << " failed: " << cpp_strerror(r); if (!outs.empty()) {
                                                                               (*err_os) << ": " << outs;}
                                                                               (*err_os) << std::endl; return r;}
