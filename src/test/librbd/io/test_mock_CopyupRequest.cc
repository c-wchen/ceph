    ...
    namespace {
        ...
        struct MockTestImageCtx:public MockImageCtx {
          ...
          :    MockImageCtx(image_ctx) {
                ...
                librbd::io::CopyupRequest < librbd::MockTestImageCtx >
                *>copyup_list;
        };
    ...
    namespace util {
        ...
        template <> struct ObjectCopyRequest <librbd::MockTestImageCtx > {
            ...
            static ObjectCopyRequest *create(librbd::MockImageCtx *
                ...
                s_instance->flatten = ((flags & deep_copy::
                                        OBJECT_COPY_REQUEST_FLAG_FLATTEN) != 0);
        ...
        ObjectCopyRequest < librbd::MockTestImageCtx > *ObjectCopyRequest <
...
{
    auto req =
        boost::get < librbd::io::ImageDispatchSpec::Read > (&arg->request);
...
namespace librbd {
    namespace io {
        ...
        struct TestMockIoCopyupRequest:public TestMockFixture {
            ...
            void SetUp() override {
                ...
                if (!is_feature_enabled(RBD_FEATURE_LAYERING)) {
                 ...
                 librbd::RBD rbd;
                 ASSERT_EQ(0,
                           rbd.open(m_ioctx, image, m_parent_image_name.c_str(),
                                    nullptr));
                ...
                int order = 0;
                 ASSERT_EQ(0,
                           rbd.clone(m_ioctx, m_parent_image_name.c_str(),
                                     "one", m_ioctx, m_image_name.c_str(),
                                     features, &order));
            ...
            void expect_read_parent(librbd::MockTestImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(*mock_image_ctx.io_image_dispatcher,
                    ...
                    .WillOnce(Invoke([&mock_image_ctx, image_extents, data,
                                     ...
                                     (&spec->request);
                                     ASSERT_TRUE(req != nullptr); if (r < 0) {
                                     spec->fail(r); return;}
                                     ...
                                     std::move(req->read_result);
                                     aio_comp->read_result.
                                     set_image_extents(image_extents);
                                     aio_comp->set_request_count(1);
                                     auto ctx =
                                     new ReadResult::
                                     C_ImageReadRequest(aio_comp, 0,
                                                        image_extents);
                                     ctx->bl.append(data);
            ...
            void expect_copyup(MockTestImageCtx & mock_image_ctx,
                               uint64_t snap_id, const std::string & oid,
                               const std::string & data, int r) {
                bufferlist in_bl;
                in_bl.append(data);
            ...
            void expect_sparse_copyup(MockTestImageCtx & mock_image_ctx,
                                      uint64_t snap_id, const std::string & oid,
            ...
            void expect_is_empty_write_op(MockAbstractObjectWriteRequest &
                                          mock_write_request, bool is_empty) {
                EXPECT_CALL(mock_write_request, is_empty_write_op())
                    .WillOnce(Return(is_empty));
            }

            void expect_add_copyup_ops(MockAbstractObjectWriteRequest &
                                       mock_write_request) {
            ...
            void expect_get_pre_write_object_map_state(MockTestImageCtx &
                                                       mock_image_ctx,
                                                       MockAbstractObjectWriteRequest
                                                       & mock_write_request,
                                                       uint8_t state) {
            ...
            void expect_object_map_update(MockTestImageCtx & mock_image_ctx,
                                          ...
                                          uint8_t state, bool updated,
                                          int ret_val) {
                if (mock_image_ctx.object_map != nullptr) {
                    if (!mock_image_ctx.image_ctx->
                    ...
                    EXPECT_CALL(*mock_image_ctx.object_map,
                                aio_update(snap_id, object_no, object_no + 1,
                                           state,
            ...
            void expect_prepare_copyup(MockTestImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(*mock_image_ctx.io_object_dispatcher,
                            prepare_copyup(_, _))
                    .WillOnce(WithArg < 1 > (Invoke([in_sparse_bl,
                                                     out_sparse_bl]
                                                    (SnapshotSparseBufferlist *
                                                     snap_sparse_bl) {
                                                    auto & sparse_bl =
                                                    (*snap_sparse_bl)[0];
                                                    EXPECT_EQ(in_sparse_bl,
                                                              sparse_bl);
        ...
        TEST_F(TestMockIoCopyupRequest, Standard) {
            ...
            expect_read_parent(mock_parent_image_ctx, { {
                               0, 4096}}, data, 0);
            expect_prepare_copyup(mock_image_ctx);

            MockAbstractObjectWriteRequest mock_write_request;
            expect_get_pre_write_object_map_state(mock_image_ctx,
                                                  mock_write_request,
                                                  OBJECT_EXISTS);
            expect_object_map_at(mock_image_ctx, 0, OBJECT_NONEXISTENT);
            expect_object_map_update(mock_image_ctx, CEPH_NOSNAP, 0,
                                     OBJECT_EXISTS, true, 0);

            expect_add_copyup_ops(mock_write_request);
            expect_sparse_copyup(mock_image_ctx, CEPH_NOSNAP,
                                 ictx->get_object_name(0), { {
                                 0, 4096}}, data, 0);
            expect_write(mock_image_ctx, CEPH_NOSNAP, ictx->get_object_name(0),
                         0);
        ...
        TEST_F(TestMockIoCopyupRequest, StandardWithSnaps) {
            ...
            ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "2", 2,
                           ictx->size, ictx->parent_md,
                           RBD_PROTECTION_STATUS_UNPROTECTED, 0, {
                           });
            ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "1", 1,
                           ictx->size, ictx->parent_md,
                           RBD_PROTECTION_STATUS_UNPROTECTED, 0, {
                           });
            ictx->snapc = {
            ...
            expect_get_pre_write_object_map_state(mock_image_ctx,
                                                  ...
                                                  OBJECT_EXISTS);
            expect_object_map_at(mock_image_ctx, 0, OBJECT_NONEXISTENT);
            expect_object_map_update(mock_image_ctx, 1, 0, OBJECT_EXISTS, true,
                                     0);
            expect_object_map_update(mock_image_ctx, 2, 0, OBJECT_EXISTS_CLEAN,
                                     true, 0);
            expect_object_map_update(mock_image_ctx, CEPH_NOSNAP, 0,
                                     OBJECT_EXISTS, true, 0);
        ...
        TEST_F(TestMockIoCopyupRequest, DeepCopy) {
            ...
            expect_object_copy(mock_image_ctx, mock_object_copy_request, true,
                               0);
        ...
        TEST_F(TestMockIoCopyupRequest, DeepCopyWithPostSnaps) {
            ...
            ASSERT_EQ(0, open_image(m_image_name, &ictx));
            ictx->image_lock.lock();
            ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "3", 3,
                           ictx->size, ictx->parent_md,
                           RBD_PROTECTION_STATUS_UNPROTECTED, 0, {
                           });
            ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "2", 2,
            ...
            expect_get_pre_write_object_map_state(mock_image_ctx,
                                                  ...
                                                  OBJECT_EXISTS);
            expect_object_map_at(mock_image_ctx, 0, OBJECT_NONEXISTENT);
            expect_object_map_update(mock_image_ctx, 2, 0, OBJECT_EXISTS, true,
                                     0);
            expect_object_map_update(mock_image_ctx, 3, 0, OBJECT_EXISTS_CLEAN,
                                     true, 0);
            expect_object_map_update(mock_image_ctx, CEPH_NOSNAP, 0,
                                     OBJECT_EXISTS, true, 0);
        ...
        TEST_F(TestMockIoCopyupRequest, DeepCopyWithPreAndPostSnaps) {
            ...
            ASSERT_EQ(0, open_image(m_image_name, &ictx));
            ictx->image_lock.lock();
            ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "4", 4,
                           ictx->size, ictx->parent_md,
                           RBD_PROTECTION_STATUS_UNPROTECTED, 0, {
                           });
            ictx->add_snap(cls::rbd::UserSnapshotNamespace(), "3", 3,
            ...
            expect_object_map_update(mock_image_ctx, 3, 0, OBJECT_EXISTS_CLEAN,
                                     true, 0);
            expect_object_map_update(mock_image_ctx, 4, 0, OBJECT_EXISTS_CLEAN,
                                     true, 0);
            expect_object_map_update(mock_image_ctx, CEPH_NOSNAP, 0,
                                     OBJECT_EXISTS, true, 0);
        ...
        TEST_F(TestMockIoCopyupRequest, SparseCopyupNotSupported) {
            ...
            expect_add_copyup_ops(mock_write_request);
            expect_copyup(mock_image_ctx, CEPH_NOSNAP, ictx->get_object_name(0),
                          data, 0);
            expect_write(mock_image_ctx, CEPH_NOSNAP, ictx->get_object_name(0),
                         0);
        ...
        TEST_F(TestMockIoCopyupRequest, ProcessCopyupOverwrite) {
            ...
            expect_prepare_copyup(mock_image_ctx, { {
                                  ...
                                  0, {
                                  1024, {
                                  SPARSE_EXTENT_STATE_DATA, 1024, bufferlist {
                                  out_prepare_bl}
                                  }
                                  }
                                  }
