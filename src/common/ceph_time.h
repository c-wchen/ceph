    ...
    namespace time_detail {
        ...
        class real_clock {
          public:
            ...
            // Allow conversion to/from any clock with the same interface as// std::chrono::system_clock)
                template < typename Clock, typename Duration >
            static time_point to_system_time_point(const std::chrono::
                                                   time_point < Clock,
                                                   Duration > &t) {
                return time_point(seconds(Clock::to_time_t(t)) + duration_cast <
                                  duration >
                                  (t.time_since_epoch() % seconds(1)));
            } template < typename Clock,
                typename Duration > static std::chrono::time_point < Clock,
                Duration > to_system_time_point(const time_point & t) {
                return (Clock::from_time_t(to_time_t(t)) + duration_cast <
                        Duration > (t.time_since_epoch() % seconds(1)));
            }
            ...
            static time_point from_ceph_timespec(const struct ceph_timespec
                                                 &ts);
          ...
          public:
            ...
            static time_point now() noexcept {
                struct timespec ts;
                 ...
                 return time_point(seconds(ts.tv_sec) +
...
namespace std {
    namespace chrono {
        template < typename Clock, typename Duration >
            ...
            void decode(time_point < Clock, Duration > &t,
                        bufferlist::iterator & p) {
            ::decode(t, p);
    }}                          // namespace chrono
    // An overload of our own namespace {
