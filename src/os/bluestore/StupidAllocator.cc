...
:  cct(cct), num_free(0), free(10), last_alloc(0)
{
}
  ...
  found:
    ...
    *length =
    ...
    if (cct->_conf->bluestore_debug_small_allocations) {
        ...
        if (max && *length > max) {
            dout(10) << __func__ << " shortening allocation of 0x" << std::hex
                << *length << " -> 0x"
                << max << " due to debug_small_allocations" << std::
                dec << dendl;
            *length = max;
        }
    }
    ...
    if (*offset && free[bin].contains(*offset - skew - 1, &off, &len)) {
        ...
        if (newbin != bin) {
            dout(30) << __func__ << " demoting 0x" << std::
                hex << off << "~" << len << std::
...
void StupidAllocator::init_rm_free(uint64_t offset, uint64_t length)
{
    ...
    for (unsigned i = 0; i < free.size() && !rm.empty(); ++i) {
        ...
        if (!overlap.empty()) {
            dout(20) << __func__ << " bin " << i << " rm 0x" << std::
