...
static constexpr const std::size_t AESGCM_KEY_LEN {
    16};
static constexpr const std::size_t AESGCM_IV_LEN {
    12};
static constexpr const std::size_t AESGCM_TAG_LEN {
    16};
static constexpr const std::size_t AESGCM_BLOCK_LEN {
    16};
...
class AES128GCM_OnWireTxHandler: public ceph::crypto::onwire::TxHandler
{
    ...
    std::unique_ptr < EVP_CIPHER_CTX,
        decltype(&::EVP_CIPHER_CTX_free) > ectx;
...
public:
    AES128GCM_OnWireTxHandler(CephContext *const cct,
                              const key_t &key,
                              const nonce_t &nonce, bool new_nonce_format)
        : cct(cct),
    ...
    } ~AES128GCM_OnWireTxHandler() override
    {
        ...
        ::TOPNSPC::crypto::zeroize_for_security(&initial_nonce,
                                                sizeof(initial_nonce));
    }

    void reset_tx_handler(const uint32_t *first,
                          const uint32_t *last) override;

    void authenticated_encrypt_update(const ceph::
                                      bufferlist &plaintext) override;
...
void AES128GCM_OnWireTxHandler::authenticated_encrypt_update(const ceph::
        bufferlist &
        plaintext)
{
    ...
    for (const auto &plainbuf : plaintext.buffers()) {
        ...
        if (1 != EVP_EncryptUpdate(ectx.get(),
                                   reinterpret_cast <
                                   unsigned char * >(filler.c_str()),
                                   &update_len,
                                   reinterpret_cast <
                                   const unsigned char * >(plainbuf.c_str()),
                                   plainbuf.length())) {
            throw std::runtime_error("EVP_EncryptUpdate failed");
        }
        ceph_assert_always(update_len >= 0);
        ceph_assert(static_cast < unsigned >(update_len) ==
                    plainbuf.length());
...
void AES128GCM_OnWireRxHandler::authenticated_decrypt_update(ceph::
        bufferlist &
        bl)
{
    ...
    for (auto &buf : bl.buffers()) {
        ...
        if (1 !=
            EVP_DecryptUpdate(ectx.get(), p, &update_len, p,
                              buf.length())) {
...
void AES128GCM_OnWireRxHandler::authenticated_decrypt_update_final(ceph::
        bufferlist
        & bl)
{
...
ceph::crypto::onwire::rxtx_t ceph::crypto::onwire::rxtx_t::
create_handler_pair(CephContext *cct,
                    ...
                    bool new_nonce_format, bool crossed)
{
    if (auth_meta.is_mode_secure()) {
        ...
        return {
            std::make_unique < AES128GCM_OnWireRxHandler > (cct, key,
                    crossed ?
                    tx_nonce :
                    rx_nonce,
                    new_nonce_format),
            std::make_unique < AES128GCM_OnWireTxHandler > (cct, key,
                    crossed ?
                    rx_nonce :
                    tx_nonce,
                    new_nonce_format)
