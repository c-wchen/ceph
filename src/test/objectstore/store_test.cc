...
class StoreTestSpecificAUSize:public StoreTestDeferredSetup {
  ...
  public:
    typedef
        std::function < void (boost::scoped_ptr < ObjectStore > &store,
                              uint64_t num_ops,
                              uint64_t max_obj,
  ...
  protected:
        ...
        if (matrix[i][0]) {
            ...
            for (int j = 1; matrix[i][j]; ++j) {
                ...
                do_matrix_choose(matrix,
                                 i + 1,
                                 pos * count + j - 1, num * count, store, fn);
            }
        }
        else {
            cout << "---------------------- " << (pos + 1) << " / " << num
    ...
    void do_matrix(const char *matrix[][10],
                   boost::scoped_ptr < ObjectStore > &store, MatrixTest fn) {
...
TEST_P(StoreTest, FiemapHoles)
{
    ...
    }
    {
        ...
        m.clear();
        e.clear();
        store->fiemap(cid, oid, SKIP_STEP, SKIP_STEP * (MAX_EXTENTS - 2) + 3,
                      bl);
        p = bl.begin();
        ::decode(m, p);
        cout << " got " << m << std::endl;
...
TEST_P(StoreTest, SimpleColPreHashTest)
{
    ...
    if (merge_threshold > 0) {
        oss << "-" << merge_threshold;
        g_ceph_context->_conf->set_val("filestore_merge_threshold",
                                       oss.str().c_str());
    }
...
void doCompressionTest(boost::scoped_ptr < ObjectStore > &store)
{
    ...
    g_conf->set_val("bluestore_compression_min_blob_size",
                    stringify(orig_min_blob_size));
    g_ceph_context->_conf->apply_changes(NULL);
}
...
TEST_P(StoreTestSpecificAUSize, BluestoreFragmentedBlobTest)
{
    ...
    }
    {
        ...
        cerr <<
            "Rewrite an object and create two holes at the begining and the end"
            << std::endl;
        r = apply_transaction(store, &osr, std::move(t));
        ASSERT_EQ(r, 0);
...
TEST_P(StoreTest, MultipoolListTest)
{
    ...
    }
    {
        ...
        while (!next.is_max()) {
            int r =
                store->collection_list(cid, current, ghobject_t::get_max(), 50,
                                       &objects, &next);
            ASSERT_EQ(r, 0);
            cout << " got " << objects.size() << " next " << next << std::endl;
            for (vector < ghobject_t >::iterator p = objects.begin();
  ...
  public:
    ...
    class C_SyntheticOnReadable:public Context {
      public:
        ...
        :state(state), hoid(hoid) {
            ...
            bufferlist r2;
            r = state->store->read(state->cid, hoid, 0,
    ...
    class C_SyntheticOnStash:public Context {
      public:
        ...
        :state(state), oid(oid), noid(noid) {
            ...
            --(state->in_flight);
            bufferlist r2;
            r = state->store->read(state->cid, noid, 0,
    ...
    void shutdown() {
        while (1) {
            vector < ghobject_t > objects;
            int r =
                store->collection_list(cid, ghobject_t(), ghobject_t::get_max(),
                                       10, &objects, 0);
            assert(r >= 0);
            if (objects.empty())
                break;
    ...
    int stash() {
        ...
        int status = store->queue_transaction(osr, std::move(t),
                                              new C_SyntheticOnStash(this,
                                                                     old_obj,
                                                                     new_obj));
        return status;
    }
    ...
    void scan() {
        ...
        while (1) {
            ...
            int r =
                store->collection_list(cid, current, ghobject_t::get_max(), 100,
                                       &objects, &next);
            ASSERT_EQ(r, 0);
            ASSERT_TRUE(sorted(objects));
    ...
    int unlink() {
        ...
        int status =
            store->queue_transaction(osr, std::move(t),
                                     new C_SyntheticOnReadable(this,
                                                               to_remove));
        return status;
    }
...
INSTANTIATE_TEST_CASE_P(ObjectStore,
                        StoreTest,::testing::Values("memstore", "filestore",
#if defined(HAVE_LIBAIO)
                                                    "bluestore",
#endif
                                                    "kstore"));
...
void doMany4KWritesTest(boost::scoped_ptr < ObjectStore > &store,
                        ...
                        unsigned write_alignment, store_statfs_t * res_stat)
{
    ObjectStore::Sequencer osr("test");
    MixedGenerator gen(555);
    gen_type rng(time(NULL));
...
TEST_P(StoreTestSpecificAUSize, ExcessiveFragmentation)
{
    ...
    ASSERT_EQ(g_conf->get_val < uint64_t > ("bluefs_alloc_size"), 1024 * 1024);
...
TEST_P(StoreTest, SpuriousReadErrorTest)
{
    ...
    {
        bufferlist in;
        r = store->read(cid, hoid, 0, 0x2000, in,
...
int main(int argc, char **argv)
{
    ...
    g_ceph_context->_conf->set_val("filestore_op_thread_suicide_timeout",
                                   "10000");
    ...
    g_ceph_context->_conf->
        set_val("bluestore_debug_randomize_serial_transaction", "10");
