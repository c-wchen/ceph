...
// an input_stream consumer that reads buffer segments into a bufferlist up to// the given number of remaining bytes struct bufferlist_consumer {
bufferlist &bl;
size_t &remaining;
...
using consumption_result_type =
    typename seastar::input_stream <
    char >::consumption_result_type;

// consume some or all of a buffer segment
seastar::future < consumption_result_type > operator()(tmp_buf
        && data)
{
...
seastar::future <> Socket::close()
{
    ...
    closed = true;
#endif
    return seastar::when_all_succeed(inject_delay(),
                                     in.close(),
        ...
        logger().
        error("Socket::close(): unexpected exception {}",
              eptr);
        ceph_abort();
    }
                                                                   );
...
seastar::future <> Socket::inject_delay()
{
    if (float delay_period = local_conf()->ms_inject_internal_delays;
        ...
        return seastar::sleep(std::chrono::
                              milliseconds((int)(delay_period * 1000.0)));
    }
    return seastar::now();
}
    ...
    if (local_conf()->ms_inject_socket_failures) {
        ...
        if (rand % local_conf()->ms_inject_socket_failures == 0) {
            if (true) {
                logger().
                warn
                ("Socket::inject_failure: injecting socket failure");
                throw std::
...
void Socket::set_trap(bp_type_t type, bp_action_t action,
                      socket_blocker *blocker_)
{
    blocker = blocker_;
    if (type == bp_type_t::READ) {
        ...
        if (next_trap_write == bp_action_t::CONTINUE) {
            ...
            ceph_assert(action == bp_action_t::FAULT
                        || action == bp_action_t::CONTINUE);
        } else {
            ceph_abort();
        }
    }
...
crimson::net::listen_ertr::future <> FixedCPUServerSocket::listen(entity_addr_t addr)
{
    ...
    listen_ertr::future <> {
        if (e.code() == std::errc::address_in_use) {
            logger().
            trace
            ("FixedCPUServerSocket::listen({}): address in use",
             addr);
            return crimson::ct_error::address_in_use::
            make();
        } else if (e.code() ==
                   std::errc::address_not_available)
        {
            logger().
            trace
            ("FixedCPUServerSocket::listen({}): address not available",
             addr);
            return crimson::ct_error::
            address_not_available::make();
        }
        logger().
        error("FixedCPUServerSocket::listen({}): "
...
seastar::future < FixedCPUServerSocket * >FixedCPUServerSocket::create()
{
    ...
    return seastar::smp::submit_to(0, [cpu] {
        auto service =
        std::make_unique < sharded_service_t >
        ();
        return service->start(cpu,
        construct_tag {
        }
                             ).
        then([service =
        std::move(service)]()mutable {
            auto p_shard = service.get();
            p_shard->local().service =
            std::move(service); return p_shard;}
            );
    }
    ).then([](auto p_shard) {
