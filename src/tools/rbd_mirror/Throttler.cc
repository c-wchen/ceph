...
namespace rbd {
    namespace mirror {
        ...
        template < typename I >
            Throttler < I >::Throttler(CephContext * cct,
                                       const std::string & config_key)
        :m_cct(cct), m_config_key(config_key), m_config_keys {
        m_config_key.c_str(), nullptr},
            m_lock(ceph::
                   make_mutex(librbd::util::
                              unique_lock_name("rbd::mirror::Throttler",
                                               this))),
            m_max_concurrent_ops(cct->_conf.get_val < uint64_t >
                                 (m_config_key)) {
        ...
        template < typename I >
            ...
            int r = 0;
            {
                ...
                if (m_inflight_ops.count(id) > 0) {
                    ...
                    dout(20) << "duplicate for already queued op " << id <<
                        dendl;
                    ...
                    dout(20) << "ready to start op for " << id << " ["
                        << m_inflight_ops.
                        size() << "/" << m_max_concurrent_ops << "]" << dendl;
                }
                else {
        ...
        template < typename I >
            ...
            Context *on_start = nullptr;
            {
                ...
                if (m_inflight_ops.size() < m_max_concurrent_ops
                    && !m_queue.empty()) {
        ...
        template < typename I >
            ...
            std::map < Id, Context * >queued_ops;
            {
                ...
                for (auto it = m_inflight_ops.begin();
                     it != m_inflight_ops.end();) {
                    if (it->first == ns) {
        ...
        template < typename I >
            ...
            if (changed.count(m_config_key)) {
                set_max_concurrent_ops(conf.get_val < uint64_t >
                                       (m_config_key));
            }
        }
