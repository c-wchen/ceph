...
TEST_F(TestSSEKMS, test_transit_key_version_extraction)
{
    const NoDoutPrefix no_dpp(cct, 1);
    string json =
        R "({" data ": {" keys ": {" 6 ": " 8 qgPWvdtf6zrriS5 +
        nkOzDJ14IGVR6Bgkub5dJn6qeg = "}}})";
    EXPECT_CALL(*old_engine,
                send_request(&no_dpp, StrEq("GET"), StrEq(""),
                             StrEq("1/2/3/4/5/6"), StrEq(""),
  ...
  for (const auto & test:tests) {
    ...
    res =
        old_engine->get_key(&no_dpp, std::string_view("1/2/3/4/5/6"),
                            actual_key);
    ASSERT_EQ(res, 0);
...
TEST_F(TestSSEKMS, test_transit_backend)
{
    ...
    string json =
        R "({" data ": {" keys ": {" 1 ": " 8 qgPWvdtf6zrriS5 +
        nkOzDJ14IGVR6Bgkub5dJn6qeg = "}}})";
    const NoDoutPrefix no_dpp(cct, 1);
    EXPECT_CALL(*old_engine,
                send_request(&no_dpp, StrEq("GET"), StrEq(""),
                             StrEq("my_key/1"), StrEq(""),
                             _)).WillOnce(SetPointedValue(json));
...
TEST_F(TestSSEKMS, test_transit_reconstitutekey)
{
    ...
    set_attr(attrs, RGW_ATTR_CRYPT_DATAKEY,
             "vault:v2:HbdxLnUztGVo+RseCIaYVn/4wEUiJNT6GQfw57KXQmhXVe7i1/kgLWegEPg1I6lexhIuXAM6Q2YvY0aZ");
    string post_json =
        R "({" data ": {" key_version ": 1," plaintext ": " 3 xfTra /
        dsIf3TMa3mAT2IxPpM7YWm / NvUb4gDfSDX4g = "}})";
    EXPECT_CALL(*transit_engine,
  ...
  set_attr(attrs, RGW_ATTR_CRYPT_CONTEXT, R "({" aws: s3:arn ": " fred
    ...
    int res =
        transit_engine->reconstitute_actual_key(&no_dpp, attrs, actual_key);
...
TEST_F(TestSSEKMS, test_kv_backend)
{
    ...
    // Mocks the expected return value from Vault Server using custom Argument Action
    string json =
        R "({" data ": {" data ": {" key ": " 8 qgPWvdtf6zrriS5 +
        nkOzDJ14IGVR6Bgkub5dJn6qeg = "}}})";
...
TEST_F(TestSSEKMS, test_transit_backend_empty_response)
{
    ...
    // Mocks the expected return Value from Vault Server using custom Argument Action
    string json =
        R "({" errors ": [" version does not exist or cannot be found "]})";
    EXPECT_CALL(*old_engine,
                send_request(&no_dpp, StrEq("GET"), StrEq(""),
                             StrEq("/key/nonexistent/1"), StrEq(""),
                             _)).WillOnce(SetPointedValue(json));
