...
namespace librbd {
    namespace image {
        ...
        using util::create_rados_callback;
         ...
         template < typename I >
            AttachChildRequest < I >::AttachChildRequest(I * image_ctx,
                                                         I * parent_image_ctx,
                                                         const librados::
                                                         snap_t &
                                                         parent_snap_id,
        ...
        :m_image_ctx(image_ctx), m_parent_image_ctx(parent_image_ctx),
            ...
            m_clone_format(clone_format), m_on_finish(on_finish),
        ...
        template < typename I > void AttachChildRequest < I >::v1_add_child() {
            ...
            cls_client::add_child(&op, {
                                  m_parent_image_ctx->md_ctx.get_id(), "",
            ...
            librados::AioCompletion * comp =
                ...
                &klass::handle_v1_add_child > (this);
        ...
        template < typename I > void AttachChildRequest < I >::v1_refresh() {
            ...
            RefreshRequest < I > *req =
                RefreshRequest < I >::create(*m_parent_image_ctx, false, false,
                                             create_context_callback < klass,
                                             &klass::handle_v1_refresh >
                                             (this));
            req->send();
        }
        ...
        template < typename I >
            void AttachChildRequest <
            I >::handle_v1_remove_child_from_old_parent(int r) {
            ldout(m_cct, 15) << "r=" << r << dendl;

            if (r < 0 && r != -ENOENT) {
                lderr(m_cct) << "couldn't remove child: " << cpp_strerror(r) <<
                    dendl;
                finish(r);
                return;
            }
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(m_cct) << "failed to enable clone v2: " << cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
            }
        ...
        template < typename I > void AttachChildRequest < I >::v2_child_attach() {
            ...
            int r =
                m_parent_image_ctx->md_ctx.aio_operate(m_parent_image_ctx->
                                                       header_oid,
                                                       aio_comp, &op);
            ceph_assert(r == 0);
            aio_comp->release();
        ...
        template < typename I >
            ...
            if (r < 0) {
                if (r == -EEXIST && m_old_parent_image_ctx != nullptr) {
                    ...
                    lderr(m_cct) << "failed to attach child image: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
        ...
        template < typename I >
            ...
            int r =
                m_old_parent_image_ctx->md_ctx.
                aio_operate(m_old_parent_image_ctx->header_oid, aio_comp, &op);
            ceph_assert(r == 0);
            aio_comp->release();
        }
        ...
        template < typename I >
            void AttachChildRequest <
            I >::handle_v2_child_detach_from_old_parent(int r) {
            ldout(m_cct, 15) << "r=" << r << dendl;

            if (r < 0 && r != -ENOENT) {
                lderr(m_cct) << "failed to detach child image: " <<
                    cpp_strerror(r)
                    << dendl;
                finish(r);
                return;
