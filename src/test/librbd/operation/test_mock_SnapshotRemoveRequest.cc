...
namespace librbd {
    namespace image {
        ...
        DetachChildRequest < MockImageCtx > *DetachChildRequest <
    ...
    namespace mirror {
        namespace snapshot {
            ...
            RemoveImageStateRequest < MockImageCtx > *RemoveImageStateRequest <
                MockImageCtx >::s_instance;
...
namespace librbd {
    namespace operation {
        ...
        class TestMockOperationSnapshotRemoveRequest:public TestMockFixture {
          public:
            typedef SnapshotRemoveRequest < MockImageCtx >
                MockSnapshotRemoveRequest;
            typedef image::DetachChildRequest < MockImageCtx >
            ...
            void expect_snapshot_get(MockImageCtx & mock_image_ctx,
                ...
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    ...
                    .WillOnce(WithArg < 5 >
                              (Invoke([snap_info, r] (bufferlist * bl) {
                                      encode(snap_info, *bl); return r;}
                               )));
            }

            void expect_children_list(MockImageCtx & mock_image_ctx,
                                      const cls::rbd::
                                      ChildImageSpecs & child_images, int r) {
                if (mock_image_ctx.old_format) {
                    return;
                }
                ...
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    ...
                    .WillOnce(WithArg < 5 >
                              (Invoke([child_images, r] (bufferlist * bl) {
                                      encode(child_images, *bl); return r;}
                               )));
            }
            ...
            void expect_detach_stale_child(MockImageCtx & mock_image_ctx, int r) {
                ...
                encode(cls::rbd::ChildImageSpec {
                       mock_image_ctx.md_ctx.get_id(), "",
                       mock_image_ctx.id}, bl);
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                            exec(util::header_name(parent_spec.image_id),
                                 _, StrEq("rbd"), StrEq("child_detach"),
                                 ContentsEqual(bl), _, _, _))
                    .WillOnce(Return(r));
            }

            void expect_object_map_snap_remove(MockImageCtx & mock_image_ctx,
                                               int r) {
            ...
            void expect_get_parent_spec(MockImageCtx & mock_image_ctx, int r) {
                ...
                auto & expect =
                    EXPECT_CALL(mock_image_ctx, get_parent_spec(_, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
                    auto & parent_spec =
                        mock_image_ctx.snap_info.rbegin()->second.parent.spec;
                    expect.
                        WillOnce(DoAll
                                 (SetArgPointee < 1 > (parent_spec),
                                  Return(0)));
                }
            }
            ...
            void expect_snap_remove(MockImageCtx & mock_image_ctx, int r) {
                auto & expect =
                    EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                exec(mock_image_ctx.header_oid, _, StrEq("rbd"),
                                     StrEq(mock_image_ctx.
                                           old_format ? "snap_remove" :
                                           "snapshot_remove"), _, _, _, _));
                if (r < 0) {
                    expect.WillOnce(Return(r));
                }
                else {
        ...
        TEST_F(TestMockOperationSnapshotRemoveRequest, Success) {
            ...
            MockSnapshotRemoveRequest *req =
                new MockSnapshotRemoveRequest(mock_image_ctx, &cond_ctx,
                                              cls::rbd::UserSnapshotNamespace(),
                                              "snap1",
                                              snap_id);
            {
        ...
        TEST_F(TestMockOperationSnapshotRemoveRequest,
            ...
            expect_detach_child(mock_image_ctx, mock_detach_child_request,
                                -ENOENT);
        ...
        TEST_F(TestMockOperationSnapshotRemoveRequest, MirrorSnapshot) {
            ...
            expect_remove_image_state(mock_image_ctx,
                                      mock_remove_image_state_request, 0);
            expect_release_snap_id(mock_image_ctx);
            expect_snap_remove(mock_image_ctx, 0);
            expect_rm_snap(mock_image_ctx);
