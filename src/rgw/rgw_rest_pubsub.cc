...
bool verify_transport_security(CephContext * cct, const RGWEnv & env)
{
    ...
    if (!is_secure
        ...
        ldout(cct,
              0) <<
            "WARNING: bypassing endpoint validation, allows sending secrets over insecure transport"
            << dendl;
        return true;
    }
    return is_secure;
...
bool validate_and_update_endpoint_secret(rgw_pubsub_dest & dest,
                                         CephContext * cct, const RGWEnv & env)
{
    ...
    if (!rgw::parse_url_userinfo(dest.push_endpoint, user, password)) {
        ldout(cct,
              1) << "endpoint validation error: malformed endpoint URL:" <<
            dest.push_endpoint << dendl;
        return false;
    }
    ...
    if (!user.empty()) {
        ...
        if (!verify_transport_security(cct, env)) {
            ldout(cct,
                  1) <<
                "endpoint validation error: sending secrets over insecure transport"
                << dendl;
            return false;
        }
    }
...
class RGWPSCreateTopicOp:public RGWOp {
  private:
    ...
    int get_params() {
        ...
        if (topic_name.empty()) {
            ldpp_dout(this,
                      1) << "CreateTopic Action 'Name' argument is missing" <<
                dendl;
            return -EINVAL;
      ...
      for (const auto & param:s->info.args.get_params()) {
        ...
        if (!dest.push_endpoint.empty() && dest.persistent) {
            const auto ret =
                rgw::notify::add_persistent_topic(topic_name, s->yield);
            if (ret < 0) {
                ldpp_dout(this,
                          1) <<
                    "CreateTopic Action failed to create queue for persistent topics. error:"
                    << ret << dendl;
                return ret;
            }
        }
...
void RGWPSCreateTopicOp::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  1) << "failed to create topic '" << topic_name << "', ret=" <<
            op_ret << dendl;
        return;
    }
    ldpp_dout(this,
              20) << "successfully created topic '" << topic_name << "'" <<
        dendl;
}
...
void RGWPSListTopicsOp::execute(optional_yield y)
{
    ...
    if (topics_has_endpoint_secret(result)
        ...
        ldpp_dout(this,
                  1) <<
            "topics contain secrets and cannot be sent over insecure transport"
            << dendl;
        op_ret = -EPERM;
        return;
    }
  ...
  private:
    ...
    int get_params() {
        ...
        if (!topic_arn || topic_arn->resource.empty()) {
            ldpp_dout(this,
                      1) <<
                "GetTopic Action 'TopicArn' argument is missing or invalid" <<
                dendl;
            return -EINVAL;
...
void RGWPSGetTopicOp::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  1) << "failed to get topic '" << topic_name << "', ret=" <<
            op_ret << dendl;
        return;
    }
    if (topic_has_endpoint_secret(result)
        ...
        ldpp_dout(this,
                  1) << "topic '" << topic_name <<
            "' contain secret and cannot be sent over insecure transport" <<
            dendl;
        op_ret = -EPERM;
        return;
    }
    ldpp_dout(this,
              1) << "successfully got topic '" << topic_name << "'" << dendl;
}
  ...
  private:
    ...
    int get_params() {
        ...
        if (!topic_arn || topic_arn->resource.empty()) {
            ldpp_dout(this,
                      1) <<
                "GetTopicAttribute Action 'TopicArn' argument is missing or invalid"
                << dendl;
            return -EINVAL;
        } topic_name = topic_arn->resource;
        return 0;
  ...
  private:
    ...
    int get_params() {
        ...
        if (!topic_arn || topic_arn->resource.empty()) {
            ldpp_dout(this,
                      1) <<
                "DeleteTopic Action 'TopicArn' argument is missing or invalid"
                << dendl;
            return -EINVAL;
        } topic_name = topic_arn->resource;
        ...
        const auto ret =
            rgw::notify::remove_persistent_topic(topic_name, s->yield);
        if (ret == -ENOENT) {
        ...
        if (ret < 0) {
            ldpp_dout(this,
                      1) <<
                "DeleteTopic Action failed to remove queue for persistent topics. error:"
                << ret << dendl;
            return ret;
        }
...
void RGWPSDeleteTopicOp::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this,
                  1) << "failed to remove topic '" << topic_name << ", ret=" <<
            op_ret << dendl;
        return;
    }
    ldpp_dout(this,
              1) << "successfully removed topic '" << topic_name << "'" <<
        dendl;
}
...
static const std::unordered_map < std::string, op_generator > op_generators = {
    {"CreateTopic",[]()->RGWOp * {return new RGWPSCreateTopicOp;
                                  ...
                                  , {
                                  "GetTopicAttributes",[]()->RGWOp * {
                                  return new RGWPSGetTopicAttributesOp;
                                  }
                                  }
                                  };
                                  bool RGWHandler_REST_PSTopic_AWS::
                                  ...
                                  ldpp_dout(s,
                                            10) << "unknown action '" <<
                                  action_name << "' for Topic handler" <<
                                  dendl;}
                                  else {
                                  ldpp_dout(s,
                                            10) <<
                                  "missing action argument in Topic handler" <<
                                  dendl;}
                                  return nullptr;}

                                  int RGWHandler_REST_PSTopic_AWS::
                                  authorize(const DoutPrefixProvider * dpp,
                                            optional_yield y) {
                                  const auto rc =
                                  RGW_Auth_S3::authorize(dpp, driver,
                                                         auth_registry, s, y);
                                  if (rc < 0) {
                                  return rc;}
                                  ...
                                  ldpp_dout(dpp,
                                            1) <<
                                  "anonymous user not allowed in topic operations"
                                  << dendl; return -ERR_INVALID_REQUEST;}
                                  return 0;}

                                  namespace {
// return a unique topic by prefexing with the notification name: <notification>_<topic>
                                  std::string topic_to_unique(const std::
                                                              string & topic,
                                                              const std::
                                                              string &
                                                              notification) {
...
// extract the topic from a unique topic of the form: <notification>_<topic>
                                  [[maybe_unused]] std::
                                  string unique_to_topic(const std::
                                                         string & unique_topic,
                                                         const std::
                                                         string & notification)
                                  {
                                  if (unique_topic.find(notification + "_") ==
                                      std::string::npos) {
                                  return "";}
                                  return unique_topic.substr(notification.
                                                             length() + 1);}

// from list of bucket topics, find the one that was auto-generated by a notification
                                  auto find_unique_topic(const
                                                         rgw_pubsub_bucket_topics
                                                         & bucket_topics,
                                                         const std::
                                                         string & notif_name) {
                                  auto it =
                                  std::find_if(bucket_topics.topics.begin(),
                                               bucket_topics.topics.end(),
                                               [&](const auto & val) {
                                               return notif_name ==
                                               val.second.s3_id;}
                                  );
                                  return it !=
                                  bucket_topics.topics.end()? std::optional <
                                  std::reference_wrapper <
                                  const rgw_pubsub_topic_filter >> (it->
                                                                    second) :
                                  std::nullopt;}
                                  }

                                  int remove_notification_by_topic(const
                                                                   DoutPrefixProvider
                                                                   * dpp,
                                                                   const std::
                                                                   string &
                                                                   ...
                                                                   ps) {
                                  int op_ret =
                                  b.remove_notification(dpp, topic_name, y);
                                  if (op_ret < 0) {
                                  ldpp_dout(dpp,
                                            1) <<
                                  "failed to remove notification of topic '" <<
                                  topic_name << "', ret=" << op_ret << dendl;}
                                  op_ret = ps.remove_topic(dpp, topic_name, y);
                                  if (op_ret < 0) {
                                  ldpp_dout(dpp,
                                            1) <<
                                  "failed to remove auto-generated topic '" <<
                                  topic_name << "', ret=" << op_ret << dendl;}
                                  return op_ret;}

                                  int delete_all_notifications(const
                                                               DoutPrefixProvider
                                                               * dpp,
                                                               const
                                                               rgw_pubsub_bucket_topics
  ...
  for (const auto & topic:       bucket_topics.
                                       topics) {
                                  const auto op_ret =
                                  remove_notification_by_topic(dpp, topic.first,
                                                               b, y, ps);
                                  if (op_ret < 0) {
                                  return op_ret;}
                                  }
...
class RGWPSCreateNotifOp:        public RGWDefaultResponseOp
                                  ...
                                  bool exists;
                                  const auto no_value =
                                  s->info.args.get("notification", &exists);
                                  if (!exists) {
                                  ldpp_dout(this,
                                            1) <<
                                  "missing required param 'notification'" <<
                                  dendl; return -EINVAL;}
                                  ...
                                  {
                                  const auto max_size =
                                  s->cct->_conf->rgw_max_put_param_size; int r;
                                  bufferlist data;
                                  std::tie(r, data) =
                                  read_all_input(s, max_size, false);
                                  if (r < 0) {
                                  ldpp_dout(this,
                                            1) << "failed to read XML payload"
                                  << dendl; return r;}
                                  if (data.length() == 0) {
                                  ldpp_dout(this,
                                            1) << "XML payload missing" <<
                                  dendl; return -EINVAL;}
                                  ...
                                  ldpp_dout(this,
                                            1) <<
                                  "failed to initialize XML parser" << dendl;
                                  return -EINVAL;}
                                  if (!parser.
                                      parse(data.c_str(), data.length(), 1)) {
                                  ldpp_dout(this,
                                            1) << "failed to parse XML payload"
                                  << dendl; return -ERR_MALFORMED_XML;}
                                  try {
                                  // NotificationConfigurations is mandatory
                                  // It can be empty which means we delete all the notifications
                                  RGWXMLDecoder::
                                  decode_xml("NotificationConfiguration",
                                             configurations, &parser, true);}
                                  catch(RGWXMLDecoder::err & err) {
                                  ldpp_dout(this,
                                            1) <<
                                  "failed to parse XML payload. error: " << err
                                  << dendl; return -ERR_MALFORMED_XML;}
                                  return 0;}
  public:
                                  int verify_permission(optional_yield y)
                                  override; void pre_exec()override {
                                  rgw_bucket_object_pre_exec(s);}
                                  ...
                                  void execute(optional_yield) override;};
                                  void RGWPSCreateNotifOp::
                                  execute(optional_yield y) {
                                  op_ret = verify_params(); if (op_ret < 0) {
                                  return;}
                                  ...
                                  driver->get_user(s->owner.get_id());
                                  std::unique_ptr < rgw::sal::Bucket > bucket;
                                  op_ret =
                                  driver->get_bucket(this, user.get(),
                                                     s->owner.get_id().tenant,
                                                     s->bucket_name, &bucket,
                                                     y); if (op_ret < 0) {
                                  ldpp_dout(this,
                                            1) << "failed to get bucket '" <<
                                  s->
                                  bucket_name << "' info, ret = " << op_ret <<
                                  dendl; return;}

                                  const RGWPubSub ps(driver,
                                                     s->owner.get_id().tenant);
                                  ...
                                  ldpp_dout(this,
                                            1) <<
                                  "failed to get list of topics from bucket '"
                                  << s->
                                  bucket_name << "', ret=" << op_ret << dendl;
                                  return;}
  ...
  for (const auto & c:           configurations.list) {
                                  ...
                                  dendl; op_ret = -EINVAL; return;}
                                  if (c.topic_arn.empty()) {
                                  ldpp_dout(this,
                                            1) <<
                                  "missing topic ARN in notification: '" <<
                                  notif_name << "'" << dendl; op_ret = -EINVAL;
                                  return;}
                                  ...
                                  ldpp_dout(this,
                                            1) <<
                                  "topic ARN has invalid format: '" << c.
                                  topic_arn << "' in notification: '" <<
                                  notif_name << "'" << dendl; op_ret = -EINVAL;
                                  return;}

                                  if (std::
                                      find(c.events.begin(), c.events.end(),
                                           rgw::notify::UnknownEvent) !=
                                      c.events.end()) {
                                  ldpp_dout(this,
                                            1) <<
                                  ...
                                  op_ret =
                                  ps.get_topic(this, topic_name, topic_info, y);
                                  if (op_ret < 0) {
                                  ldpp_dout(this,
                                            1) << "failed to get topic '" <<
                                  topic_name << "', ret=" << op_ret << dendl;
                                  return;}
                                  ...
                                  const auto unique_topic_name =
                                  topic_to_unique(topic_name, notif_name);
                                  ...
                                  op_ret =
                                  ps.create_topic(this, unique_topic_name,
                                                  topic_info.dest,
                                                  topic_info.arn,
                                                  topic_info.opaque_data, y);
                                  if (op_ret < 0) {
                                  ldpp_dout(this,
                                            1) <<
                                  "failed to auto-generate unique topic '" <<
                                  unique_topic_name << "', ret=" << op_ret <<
                                  dendl; return;}
                                  ldpp_dout(this,
                                            20) <<
                                  ...
                                  op_ret =
                                  b.create_notification(this, unique_topic_name,
                                                        c.events,
                                                        std::make_optional(c.
                                                                           filter),
                                                        notif_name, y);
                                  ...
                                  int RGWPSCreateNotifOp::
                                  verify_permission(optional_yield y) {
...
class RGWPSDeleteNotifOp:        public RGWDefaultResponseOp
  ...
  public:
                                  ...
                                  const auto unique_topic =
                                  find_unique_topic(bucket_topics, notif_name);
                                  if (unique_topic) {
                                  const auto unique_topic_name =
                                  unique_topic->get().topic.name;
                                  op_ret =
                                  remove_notification_by_topic(this,
                                                               unique_topic_name,
                                                               b, y, ps);
                                  return;}
                                  ...
                                  ldpp_dout(this,
                                            20) << "notification '" <<
                                  notif_name << "' already removed" << dendl;
                                  return;}
...
class RGWPSListNotifsOp:         public RGWOp {
  ...
  public:
                                  ...
                                  rgw_flush_formatter_and_reset(s,
                                                                s->formatter);}
                                  };
                                  void RGWPSListNotifsOp::
                                  execute(optional_yield y) {
                                  std::string notif_name;
                                  op_ret = get_params(notif_name);
                                  if (op_ret < 0) {
                                  ...
                                  ldpp_dout(this,
                                            1) <<
                                  "failed to get notification info for '" <<
                                  notif_name << "', ret=" << op_ret << dendl;
                                  return;}
