...
int ZonedFreelistManager::create(uint64_t new_size,
...
void ZonedFreelistManager::allocate(uint64_t offset,
                                    uint64_t length,
                                    KeyValueDB::Transaction txn)
{
    while (length > 0) {
...
void ZonedFreelistManager::release(uint64_t offset,
                                   uint64_t length, KeyValueDB::Transaction txn)
{
    while (length > 0) {
        uint64_t zone_num = offset / zone_size;
        uint64_t this_len = std::min(length, zone_size - offset % zone_size);
...
void ZonedFreelistManager::get_meta(uint64_t target_size,
