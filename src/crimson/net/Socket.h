...
public:
    ...
    Socket(seastar::connected_socket
           && _socket, side_t _side, uint16_t e_port, construct_tag)
        : sid {
        seastar::this_shard_id()},
    socket(std::move(_socket)), in(socket.input()),
    ...
    socket_is_shutdown(false), side(_side), ephemeral_port(e_port)
    {
    } ~Socket()
    {
#ifndef NDEBUG
    ...
    {
        inject_failure();
        return inject_delay().then([peer_addr] {
            return seastar::connect(peer_addr.
                                    in4_addr());
        }
    ...
    seastar::future <> write_flush(packet && buf)
    {
        ...
        then([buf = std::move(buf), this]()mutable {
            ...
            return inject_delay().
            then([buf = std::move(buf), this]()mutable {
                return out.write(std::move(buf)).then([this] {
                    return out.
                    flush();
                }
                                                     );}
    ...
    // learn my ephemeral_port as connector.// unfortunately, there's no way to identify which port I'm using as// connector with current seastar interface.
...
#ifdef UNIT_TESTS_BUILT
public:
    void set_trap(bp_type_t type, bp_action_t action,
...
using listen_ertr = crimson::errorator < crimson::ct_error::address_in_use, // The address is already bound
...
class FixedCPUServerSocket: public seastar::peering_sharded_service <
...
public:
    FixedCPUServerSocket(seastar::shard_id cpu, construct_tag): cpu {
        cpu}
    {
    }
    ~FixedCPUServerSocket()
    ...
    template < typename Func > seastar::future <> accept(Func && fn_accept)
    {
        assert(seastar::this_shard_id() == cpu);
        logger().trace("FixedCPUServerSocket({})::accept()...", addr);
        return container().invoke_on_all([fn_accept =
                                              std::
                                              move(fn_accept)](auto & ss)
        mutable {
            assert(ss.listener);
            // gate accepting
            ...
            ()mutable {
                ...
                e) {
                    if (e.code() ==
                        ...
                        logger().
                        trace
                        ("FixedCPUServerSocket({})::accept(): stopped ({})",
                         ss.addr, e);
                    } else {
                        throw;
                    }
