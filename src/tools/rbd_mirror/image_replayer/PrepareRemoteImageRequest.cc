...
namespace rbd {
    namespace mirror {
        namespace image_replayer {
            ...
            using librbd::util::create_rados_callback;
             ...
             template < typename I >
                ...
                if (*m_state_builder != nullptr) {
                    ...
                    auto state_builder =
                        dynamic_cast < snapshot::StateBuilder < I >
                        *>(*m_state_builder);
                    if (state_builder) {
                ...
                Context *ctx = create_context_callback <
                    ...
                    I >::handle_get_remote_image_id > (this);
                auto req =
                    GetMirrorImageIdRequest < I >::create(m_remote_io_ctx,
                                                          m_global_image_id,
                                                          &m_remote_image_id,
                                                          ctx);
                req->send();
            ...
            template < typename I >
                void PrepareRemoteImageRequest <
                I >::handle_get_remote_image_id(int r) {
                dout(10) << "r=" << r << ", " << "remote_image_id=" <<
            ...
            template < typename I >
                ...
                auto ctx = create_context_callback <
                    PrepareRemoteImageRequest < I >,
                    &PrepareRemoteImageRequest < I >::handle_get_mirror_info >
                    (this);
                auto req =
                    librbd::mirror::GetInfoRequest <
                    I >::create(m_remote_io_ctx, m_threads->work_queue,
                                m_remote_image_id,
                                &m_mirror_image, &m_promotion_state,
            ...
            template < typename I >
                void PrepareRemoteImageRequest <
                I >::handle_get_mirror_info(int r) {
                dout(10) << "r=" << r << dendl;

                if (r == -ENOENT) {
                    dout(10) << "image " << m_global_image_id << " not mirrored"
                        << dendl;
                    finish(r);
                    return;
                }
                ...
                if (state_builder != nullptr &&
                    state_builder->get_mirror_image_mode() !=
                    m_mirror_image.mode) {
                    derr <<
                        "local and remote mirror image using different mirroring modes "
                        << "for image " << m_global_image_id << ": split-brain"
                        << dendl;
                    finish(-EEXIST);
                    return;
                }
                    ...
                    dout(5) << "remote image mirroring is being disabled" <<
                        dendl;
                    finish(-ENOENT);
                    return;
                }
                ...
                case cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT:
                    ...
                    break;
                default:
                    derr << "unsupported mirror image mode " << m_mirror_image.
                        mode << " " << "for image " << m_global_image_id <<
                        dendl;
                    finish(-EOPNOTSUPP);
                    break;
                }
            ...
            template < typename I >
                ...
                Context *ctx =
                    create_async_context_callback(m_threads->work_queue,
                                                  ...
                                                  I >::handle_get_client >
                                                  (this));
                m_remote_journaler->get_client(m_local_mirror_uuid, &m_client,
                                               ctx);
            }
            ...
            template < typename I >
                ...
                (state_builder ==
                     nullptr ? "" : state_builder->local_image_id)};
                client_meta.state =
            ...
            template < typename I >
                void PrepareRemoteImageRequest <
                I >::handle_register_client(int r) {
                dout(10) << "r=" << r << dendl;
                ...
                client_meta.state =
                    librbd::journal::MIRROR_PEER_STATE_REPLAYING;
                finalize_journal_state_builder(cls::journal::
            ...
            template < typename I >
                void PrepareRemoteImageRequest <
                I >::finalize_journal_state_builder(cls::journal::
                ...
                if (*m_state_builder != nullptr) {
                    ...
                    state_builder =
                        dynamic_cast < journal::StateBuilder < I >
                        *>(*m_state_builder);
                    ceph_assert(state_builder != nullptr);
                }
                else {
                    state_builder =
                        journal::StateBuilder < I >::create(m_global_image_id);
                    *m_state_builder = state_builder;
                }

                state_builder->remote_mirror_uuid =
                    m_remote_pool_meta.mirror_uuid;
            ...
            template < typename I >
                void PrepareRemoteImageRequest <
                I >::finalize_snapshot_state_builder() {
                ...
                if (*m_state_builder != nullptr) {
                    ...
                    state_builder =
                        snapshot::StateBuilder < I >::create(m_global_image_id);
                    *m_state_builder = state_builder;
                }

                dout(10) << "remote_mirror_uuid=" << m_remote_pool_meta.
                    mirror_uuid << ", " << "remote_mirror_peer_uuid=" <<
                    m_remote_pool_meta.
                    mirror_peer_uuid << ", " << "remote_image_id=" <<
                    m_remote_image_id << ", " << "remote_promotion_state=" <<
                    m_promotion_state << dendl;
                state_builder->remote_mirror_uuid =
                    m_remote_pool_meta.mirror_uuid;
                state_builder->remote_mirror_peer_uuid =
                    m_remote_pool_meta.mirror_peer_uuid;
