...
struct request_context_t {
    ...
    bool check_magic() const {
        ...
        if (!ret) {
            logger().error("Invalid magic {} should be {}",
    ...
    uint32_t get_command() const {
        ...
        return in.read_exactly(sizeof(struct nbd_request)
                   ...
                   return in.read_exactly(len).then([this] (auto buf) {
                                                    in_buffer =
                                                    ceph::buffer::
                                                    create_page_aligned(len);
                                                    in_buffer->copy_in(0, len,
                                                                       buf.
                                                                       get());
                                                    return seastar::now();}
                   );}
...
struct RequestWriter {
    ...
    RequestWriter(seastar::output_stream <
                  char >&&stream):stream(std::move(stream)) {
    } RequestWriter(RequestWriter &&) = default;

    seastar::future <> complete(request_context_t::ref && req) {
        auto & request = *req;
        return lock.write_lock().then([&request, this] {
                                      return request.write_reply(stream);}
        ).finally([&, this, req = std::move(req)] {
                  lock.write_unlock();
                  logger().debug("complete"); return seastar::now();}
        );
    }
  ...
  public:
    struct config_t {
        ...
        void populate_options(po::options_description & desc) {
            desc.add_options()
                ("uds-path", po::value < std::string > ()
                 ->default_value("/tmp/store_nbd_socket.sock")
                 ->notifier([this] (auto s) {
                            uds_path = s;}), "Path to domain socket for nbd");
    }};
...
int main(int argc, char **argv)
{
    ...
    std::transform(begin(unrecognized_options),
                   ...
                   std::back_inserter(av),[](auto & s) {
                   return const_cast < char *>(s.c_str());});
    return app.run(av.size(), av.data(),[&] {
                   if (debug) {
                   seastar::global_logger_registry().
                   set_all_loggers_level(seastar::log_level::debug);}
                   return seastar::async([&] {
                                         ...
                                         logger().
                                         debug("Running nbd server...");
                                         nbd.run();
                                         auto stop_nbd = seastar::defer([&] {
                                                                        nbd.
                                                                        stop().
                                                                        get();}
                                         ); should_stop.wait().get(); return 0;}
                   );}
    ) ;
}
...
seastar::future <> send_negotiation(size_t size,
...
seastar::future <> handle_command(BlockDriver & backend,
...
case NBD_CMD_WRITE:
return backend.write(request.from, *request.in_buffer); case NBD_CMD_READ:
            return backend.read(request.from, request.len).then([&](auto buffer) {
                                                                logger().
                                                                debug
                                                                ("read returned buffer len {}",
                                                                 buffer.
                                                                 length());
                                                                request.
                                                                out_buffer =
...
throw std::system_error(std::make_error_code(std::errc::bad_message)); case NBD_CMD_TRIM:
throw std::system_error(std::make_error_code(std::errc::bad_message)); default:
            throw std::system_error(std::
...
seastar::future <> handle_commands(BlockDriver & backend,
