...
static boost::optional < PublicAccessBlockConfiguration > get_public_access_conf_from_attr(const map < string,
        bufferlist > & attrs)
{
...
static std::tuple < bool, bool > rgw_check_policy_condition(const DoutPrefixProvider *dpp, req_state* s,
        bool check_obj_exist_tag = true)
{
    return rgw_check_policy_condition(dpp, s->iam_policy, s->iam_identity_policies, s->session_policies,
                                      check_obj_exist_tag);
}
...
int RGWPutObj::verify_permission(optional_yield y)
{
    if (! copy_source.empty()) {
        ...
        auto [has_s3_existing_tag, has_s3_resource_tag] = rgw_check_policy_condition(this, policy, s->iam_identity_policies,
            s->session_policies);
        if (has_s3_existing_tag || has_s3_resource_tag) {
...
int RGWCopyObj::verify_permission(optional_yield y)
{
    ...
    if (s->local_source &&  source_zone.empty()) {
        ...
        auto [has_s3_existing_tag, has_s3_resource_tag] = rgw_check_policy_condition(this, src_policy, s->iam_identity_policies,
            s->session_policies);
        if (has_s3_existing_tag || has_s3_resource_tag) {
...
#define DELETE_MULTI_OBJ_MAX_NUM      1000
    ...
    for (iter = multi_delete->objects.begin();
        ...
        if (y) {
            ...
            aio_count++;
            boost::asio::spawn(y.get_yield_context(), [this, &aio_count, obj_key,
            &formatter_flush_cond](boost::asio::yield_context yield) {
