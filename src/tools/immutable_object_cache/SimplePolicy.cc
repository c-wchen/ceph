...
namespace ceph {
    namespace immutable_obj_cache {
        ...
        void SimplePolicy::update_status(std::string file_name,
                                         ...
                                         uint64_t size) {
            ldout(cct,
                  20) << "update status for: " << file_name << " new status = "
                << new_status << dendl;
        ...
        void SimplePolicy::get_evict_list(std::list < std::string > *obj_list) {
            ...
            if ((double)m_cache_size > m_max_cache_size * m_watermark) {
                ...
                for (int i = 0; i < evict_num; i++) {
                    Entry *entry =
                        reinterpret_cast <
                        Entry * >(m_promoted_lru.lru_expire());
                    if (entry == nullptr) {
                        continue;
                    }
...
// for unit test
        ...
        std::string SimplePolicy::get_evict_entry() {
            Entry *entry =
                reinterpret_cast <
                Entry * >(m_promoted_lru.lru_get_next_expire());
            if (entry == nullptr) {
                return "";
            }
