    ...
    namespace migration {
        ...
        class TestMockMigrationQCOWFormat:public TestMockFixture {
          public:
            ...
            typedef SourceSpecBuilder < MockTestImageCtx >
                MockSourceSpecBuilder;
            ...
            } void expect_build_stream(MockSourceSpecBuilder &
                                       mock_source_spec_builder,
                                       MockStreamInterface *
                                       mock_stream_interface, int r) {
            ...
            } void expect_stream_open(MockStreamInterface &
                                      mock_stream_interface, int r) {
            ...
            void expect_stream_close(MockStreamInterface &
                                     mock_stream_interface, int r) {
            ...
            void expect_probe_header(MockStreamInterface &
                                     mock_stream_interface, uint32_t magic,
                                     uint32_t version, int r) {
                magic = htobe32(magic);
                ...
                probe_bl.
                    append(std::
                           string_view(reinterpret_cast < char *>(&magic), 4));
                probe_bl.
                    append(std::
                           string_view(reinterpret_cast < char *>(&version),
                                       4));
            ...
            void expect_read_header(MockStreamInterface & mock_stream_interface,
                ...
                header_bl.
                    append(std::
                           string_view(reinterpret_cast < char *>(&qcow_header),
                                       sizeof(qcow_header)));
                expect_stream_read(mock_stream_interface, { {
                                   0, sizeof(qcow_header)}
                                   }
                                   , header_bl, r);
            }

            void expect_read_l1_table(MockStreamInterface &
                                      mock_stream_interface,
                                      std::optional < std::vector <
                                      uint64_t >> &&l1_table, int r) {
                ...
                l1_table_bl.
                    append(std::
                           string_view(reinterpret_cast <
                                       char *>(l1_table->data()), 16));
                expect_stream_read(mock_stream_interface, { {
                                   1 << 20, 16}}, l1_table_bl, r);
            }

            void expect_read_l2_table(MockStreamInterface &
                                      mock_stream_interface,
                                      uint64_t l2_table_offset,
                                      std::optional < std::vector <
                                      uint64_t >> &&l2_table, int r) {
            ...
            void expect_read_cluster(MockStreamInterface &
                                     mock_stream_interface,
                ...
                expect_stream_read(mock_stream_interface, { {
                                   cluster_offset, cluster_bl.length()}
                                   }
                                   , cluster_bl, r);
            }
            ...
            void expect_open(MockStreamInterface & mock_stream_interface,
                ...
                expect_read_l1_table(mock_stream_interface, std::move(l1_table),
                                     0);
            }

            void expect_read_snapshot_header(MockStreamInterface &
                                             mock_stream_interface,
                                             const std::string & id,
            ...
            void expect_read_snapshot_header_extra(MockStreamInterface &
                                                   mock_stream_interface,
                                                   const std::string & id,
                                                   const std::string & name,
                                                   uint64_t size,
                ...
                memset(&snapshot_header_extra, 0,
                       sizeof(snapshot_header_extra));
                snapshot_header_extra.disk_size = htobe64(size);

                bufferlist snapshot_header_extra_bl;
                snapshot_header_extra_bl.
                    append(std::
                           string_view(reinterpret_cast <
                                       char *>(&snapshot_header_extra), 16));
                snapshot_header_extra_bl.append(id);
                snapshot_header_extra_bl.append(name);
                expect_stream_read(mock_stream_interface, { {
                                   *snapshot_offset,
                                   16 + id.size() + name.size()}},
                                   snapshot_header_extra_bl, r);

                *snapshot_offset += 16 + id.size() + name.size();
                *snapshot_offset =
                    p2roundup(*snapshot_offset, static_cast < uint64_t > (8));
            }

            void expect_read_snapshot_l1_table(MockStreamInterface &
                                               mock_stream_interface,
                                               uint64_t l1_table_offset,
                                               int r) {
                ...
                snapshot_l1_table_bl.
                    append(std::
                           string_view(reinterpret_cast <
                                       char *>(&l2_table_cluster), 8));
                expect_stream_read(mock_stream_interface, { {
                                   l1_table_offset, 8}},
                                   snapshot_l1_table_bl, r);
            }

            void expect_read_snapshot(MockStreamInterface &
                                      mock_stream_interface,
                                      const std::string & id,
                ...
                expect_read_snapshot_header(mock_stream_interface, id, name,
                                            ...
                                            0);
                expect_read_snapshot_header_extra(mock_stream_interface, id,
                                                  name, size, snapshot_offset,
                                                  0);
                expect_read_snapshot_l1_table(mock_stream_interface,
                                              l1_table_offset, 0);
            }
        ...
        TEST_F(TestMockMigrationQCOWFormat, OpenCloseV1) {
            ...
            expect_build_stream(mock_source_spec_builder, mock_stream_interface,
                                0);
        ...
        TEST_F(TestMockMigrationQCOWFormat, ReadL1TableError) {
            ...
            expect_read_l1_table(*mock_stream_interface, {
                                 }
                                 , -EPERM);
        ...
        TEST_F(TestMockMigrationQCOWFormat, Snapshots) {
            ...
            expect_read_snapshot(*mock_stream_interface, "1", "snap1", 1 << 29,
                                 1 << 22, &snapshot_offset);
            expect_read_snapshot(*mock_stream_interface, "2", "snap2", 1 << 29,
                                 1 << 22, &snapshot_offset);
        ...
        TEST_F(TestMockMigrationQCOWFormat, SnapshotExtraError) {
            ...
            expect_read_snapshot_header_extra(*mock_stream_interface, "1",
                                              "snap1", 1 << 29,
                                              &snapshot_offset, -EINVAL);
        ...
        TEST_F(TestMockMigrationQCOWFormat, SnapshotL1TableError) {
            ...
            expect_read_snapshot_header_extra(*mock_stream_interface, "1",
                                              "snap1", 1 << 29,
                                              &snapshot_offset, 0);
            expect_read_snapshot_l1_table(*mock_stream_interface, 1 << 22,
                                          -EINVAL);
        ...
        TEST_F(TestMockMigrationQCOWFormat, Read) {
            ...
            expect_read_cluster(*mock_stream_interface, 1ULL << 32, 123,
                                expect_bl, 0);
            ...
            C_SaferCond ctx2;
            auto aio_comp =
                io::AioCompletion::create_and_start(&ctx2, m_image_ctx,
                                                    io::AIO_TYPE_READ);
            bufferlist bl;
            io::ReadResult read_result {
            &bl};
            ASSERT_TRUE(mock_qcow_format.read(aio_comp, CEPH_NOSNAP, { {
                                              65659, 123}
                                              }
        ...
        TEST_F(TestMockMigrationQCOWFormat, ReadSnap) {
            ...
            io::ReadResult read_result {
            &bl};
            ASSERT_TRUE(mock_qcow_format.read(aio_comp, 1, { {
                                              65659, 123}
                                              }
                                              , std::move(read_result), 0, 0, {
                                              }
        ...
        TEST_F(TestMockMigrationQCOWFormat, ReadClusterCacheHit) {
            ...
            expect_read_cluster(*mock_stream_interface, 1ULL << 32, 0,
                                cluster_bl, 0);
            ...
            auto aio_comp1 =
                io::AioCompletion::create_and_start(&ctx2, m_image_ctx,
                                                    io::AIO_TYPE_READ);
            bufferlist bl1;
            io::ReadResult read_result1 {
            &bl1};
            ASSERT_TRUE(mock_qcow_format.read(aio_comp1, CEPH_NOSNAP, { {
                                              65659, 123}
                                              }
            ...
            C_SaferCond ctx3;
            auto aio_comp2 =
                io::AioCompletion::create_and_start(&ctx3, m_image_ctx,
                                                    io::AIO_TYPE_READ);
            bufferlist bl2;
            io::ReadResult read_result2 {
            &bl2};
            ASSERT_TRUE(mock_qcow_format.read(aio_comp2, CEPH_NOSNAP, { {
                                              66016, 234}
                                              }
        ...
        TEST_F(TestMockMigrationQCOWFormat, ListSnaps) {
            ...
            sparse_extents_1.insert(0, 196608, {
                                    io::SPARSE_EXTENT_STATE_DATA, 196608}
            );
            ...
            sparse_extents_2.insert(0, 65536, {
            ...
            expect_read_l2_table(*mock_stream_interface, 1ULL << 23, { {
                                 1ULL << 26, QCOW_OFLAG_ZERO, 1ULL << 25}
                                 }
                                 , 0);
            ...
            expect_read_l2_table(*mock_stream_interface, 1ULL << 28, { {
                                 1ULL << 26, QCOW_OFLAG_ZERO, 1ULL << 27}
                                 }
                                 , 0);
