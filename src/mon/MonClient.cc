...
MonClient::MonClient(CephContext *cct_, boost::asio::io_context &service):
    ...
    reopen_interval_multiplier(cct_->_conf.get_val <
                               double > ("mon_client_hunt_interval_min_multiple")),
    last_mon_command_tid(0), version_req_id(0)
{
}
...
int MonClient::get_monmap_and_config()
{
    ...
    if (r < 0) {
        lderr(cct) << __func__ << " cannot identify monitors to contact" <<
                   dendl;
        return r;
    }
    ...
    while (tries-- > 0) {
        ...
        r = authenticate(std::chrono::duration <
                         double > (cct->_conf.get_val < std::chrono::seconds >
        ...
        }
        {
            std::unique_lock l(monc_lock);
            if (monmap.get_epoch() &&
                !monmap.persistent_features.
                contains_all(ceph::features::mon::FEATURE_MIMIC)) {
                ldout(cct,
                      10) << __func__ <<
                          " pre-mimic monitor, no config to fetch" << dendl;
                r = 0;
                break;
            }
            while ((!bootstrap_config || monmap.get_epoch() == 0) && r == 0) {
                ldout(cct,
                      20) << __func__ << " waiting for monmap|config" << dendl;
                auto status =
                    map_cond.wait_for(l,
...
int MonClient::ping_monitor(const string &mon_id, string *result_reply)
{
    ...
    if (new_mon_id.empty()) {
        ...
        ldout(cct,
              10) << __func__ << " no such monitor 'mon." << new_mon_id << "'"
                  << dendl;
        return -ENOENT;
    }
    ...
    if (!m->get_connection()->is_anon() &&
        ...
        if (_hunting()) {
            ...
            if (p == pending_cons.end()) {
                ...
                ldout(cct,
                      10) << "discarding stray monitor message " << *m << dendl;
                m->put();
                return true;
            }
...
void MonClient::handle_monmap(MMonMap *m)
{
    ...
    ldout(cct, 10) << " got monmap " << monmap.epoch
                   << " from mon." << old_name
                   << " (according to old e" << monmap.get_epoch() << ")" << dendl;
    ldout(cct, 10) << "dump:\n";
    monmap.print(*_dout);
    *_dout << dendl;
...
int MonClient::init()
{
    ...
    rotating_secrets.
    reset(new RotatingKeyRing(cct, cct->get_module_type(), keyring.get()));
    ...
    cct->get_admin_socket()->register_command("rotate-key",
            this,
...
void MonClient::shutdown()
{
    ...
    while (!version_requests.empty()) {
        ...
        ldout(cct,
              20) << __func__ << " canceling and discarding version request " <<
    ...
    while (!active_con && authenticate_err >= 0) {
        if (timeout > 0.0) {
            ...
            if (r == std::cv_status::timeout && !active_con) {
                ldout(cct,
                      0) << "authenticate timed out after " << timeout << dendl;
...
int MonClient::call(std::string_view command,
                    const cmdmap_t &cmdmap,
                    const ceph::buffer::list &inbl,
                    ceph::Formatter *f,
                    std::ostream &errss, ceph::buffer::list &out)
{
...
void MonClient::handle_auth(MAuthReply *m)
{
    ...
    if (!_hunting()) {
        ...
        if (global_id != active_con->get_global_id()) {
            lderr(cct) << __func__ <<
                       " peer assigned me a different global_id: " << active_con->
                       get_global_id() << dendl;
        }
        if (ret != -EAGAIN) {
            _finish_auth(ret);
...
void MonClient::_add_conns()
{
    ...
    auto get_next_batch = [this]()->std::vector < unsigned > {
        ...
        boost::copy(monmap.mon_info | boost::adaptors::filtered([this]
        (auto & info) {
            auto rank =
            monmap.
            get_rank(info.
                                                               ...
                                                               ) | boost::adaptors::transformed([this](auto & info)
        {
            ...
            return std::make_pair(info.
                                  ...
                                  rank);
                                                                                               ...
                                                                                               ), std::inserter(ranks_by_priority,
        ...
        auto cands =
            boost::make_iterator_range(ranks_by_priority.
                                       equal_range(ranks_by_priority.begin()->
                                           first));
    ...
    if (ranks.size() > 1) {
        ...
        if (std::accumulate(begin(weights), end(weights), 0u) == 0) {
            std::shuffle(begin(ranks), end(ranks), std::mt19937 {
                rd()}
                        );
        } else {
            weighted_shuffle(begin(ranks), end(ranks), begin(weights),
            end(weights), std::mt19937 {
                rd()}
                            );
        }
    }
...
bool MonClient::ms_handle_reset(Connection *con)
{
    ...
    if (_hunting()) {
        if (pending_cons.count(con->get_peer_addrs())) {
            ldout(cct,
                  10) << __func__ << " hunted mon " << con->get_peer_addrs()
                      << dendl;
        } else {
        ...
        if (active_con && con == active_con->get_con()) {
            ldout(cct,
                  10) << __func__ << " current mon " << con->get_peer_addrs()
                      << dendl;
            _reopen_session();
            return false;
        } else {
            ldout(cct,
                  10) << "ms_handle_reset stray mon " << con->get_peer_addrs()
                      << dendl;
            return true;
        }
    }
...
bool MonClient::_opened() const const
{
    ceph_assert(ceph_mutex_is_locked(monc_lock));
    return active_con || _hunting();
}

bool MonClient::_hunting() const const
{
    ...
    if (!auth_err) {
        ...
        if (active_con) {
            ...
            if (global_id && global_id != active_con->get_global_id()) {
                lderr(cct) << __func__ << " global_id changed from " <<
                           global_id << " to " << active_con->get_global_id() << dendl;
            }
        ...
        if (now > last_keepalive + cct->_conf->mon_client_ping_interval) {
            ...
            if (cct->_conf->mon_client_ping_timeout > 0 &&
                ...
                if (interval > cct->_conf->mon_client_ping_timeout) {
                    ldout(cct,
                          1) << "no keepalive since " << lk << " (" << interval
...
void MonClient::_un_backoff()
{
    ...
    reopen_interval_multiplier =
        std::max(cct->_conf.get_val <
                 double > ("mon_client_hunt_interval_min_multiple"),
                 reopen_interval_multiplier / cct->_conf.get_val <
                 double > ("mon_client_hunt_interval_backoff"));
    ldout(cct,
          20) << __func__ << " reopen_interval_multipler now " <<
              reopen_interval_multiplier << dendl;
}

void MonClient::schedule_tick()
{
    auto do_tick = make_lambda_context([this](int) {
        tick();
    });
    if (!is_connected()) {
...
int MonClient::wait_auth_rotating(double timeout)
{
    ...
    return (!auth_principal_needs_rotating_keys
            (entity_name)
            || !rotating_secrets->
    ...
    if (r->is_tell()) {
        ...
        if (monmap.min_mon_release >= ceph_release_t::octopus) {
            ...
            if (r->target_rank >= 0) {
                ...
                r->target_con =
                    messenger->connect_to_mon(monmap.get_addrs(r->target_rank),
                                              true /* anon */);
            } else {
                ...
                r->target_con =
                    messenger->connect_to_mon(monmap.get_addrs(r->target_name),
                                              true /* anon */);
            }
...
void MonClient::_check_tell_commands()
{
    ...
    while (p != mon_commands.end()) {
        ...
        if (cmd->is_tell() &&
            ...
            now - cmd->last_send_attempt >
            cct->_conf->mon_client_hunt_interval) {
            ldout(cct,
                  5) << __func__ << " timeout tell command " << cmd->
                     tid << dendl;
...
void MonClient::handle_mon_command_ack(MMonCommandAck *ack)
{
    ...
    if (tid == 0 && !mon_commands.empty()) {
        ...
        ldout(cct,
              10) << __func__ << " has tid 0, assuming it is " << r->
                  tid << dendl;
    } else {
...
void MonClient::handle_get_version_reply(MMonGetVersionReply *m)
{
    ...
    if (iter == version_requests.end()) {
        ldout(cct,
              0) << __func__ << " version request with handle " << m->
                 handle << " not found" << dendl;
    } else {
        auto req = std::move(iter->second);
        ldout(cct,
              10) << __func__ << " finishing " << iter->
                  first << " version " << m->version << dendl;
...
int MonClient::get_auth_request(Connection *con,
                                ...
                                ceph::buffer::list *bl)
{
    std::lock_guard l(monc_lock);
    ldout(cct,
          10) << __func__ << " con " << con << " auth_method " << *auth_method
              << dendl;
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        ...
        if (con->is_anon()) {
            for (auto &i : mon_commands) {
                if (i.second->target_con == con) {
                    return i.second->target_session->
                           get_auth_request(auth_method, preferred_modes, bl,
                                            entity_name, want_keys,
                                            rotating_secrets.get());
                }
            }
        }
        for (auto &i : pending_cons) {
            if (i.second.is_con(con)) {
                return i.second.get_auth_request(auth_method, preferred_modes,
                                                 bl, entity_name, want_keys,
                                                 rotating_secrets.get());
            }
        }
        return -ENOENT;
    ...
    auth_registry.get_supported_modes(con->get_peer_type(),
                                      auth_meta->auth_method, preferred_modes);
    *bl = auth_meta->authorizer->bl;
    return 0;
}

int MonClient::handle_auth_reply_more(Connection *con,
                                      AuthConnectionMeta *auth_meta,
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        if (con->is_anon()) {
            for (auto &i : mon_commands) {
                if (i.second->target_con == con) {
                    return i.second->target_session->
                           handle_auth_reply_more(auth_meta, bl, reply);
                }
            }
        }
        for (auto &i : pending_cons) {
...
int MonClient::handle_auth_done(Connection *con,
                                AuthConnectionMeta *auth_meta,
                                uint64_t global_id,
                                uint32_t con_mode,
                                const ceph::buffer::list &bl,
                                ...
                                std::string *connection_secret)
{
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        ...
        if (con->is_anon()) {
            for (auto &i : mon_commands) {
                if (i.second->target_con == con) {
                    return i.second->target_session->handle_auth_done(auth_meta,
                            global_id,
                            bl,
                            session_key,
                            connection_secret);
        ...
        for (auto &i : pending_cons) {
            if (i.second.is_con(con)) {
                int r = i.second.handle_auth_done(auth_meta, global_id, bl,
                                                  session_key,
                                                  connection_secret);
                if (r) {
        ...
        auto p = bl.begin();
        if (!auth_meta->authorizer->
            verify_reply(p, &auth_meta->connection_secret)) {
            ldout(cct,
                  0) << __func__ << " failed verifying authorizer reply" <<
                     dendl;
            return -EACCES;
        }
...
int MonClient::handle_auth_bad_method(Connection *con,
                                      AuthConnectionMeta *auth_meta,
                                      ...
                                      &allowed_modes)
{
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        ...
        // huh...
        ldout(cct,
              10) << __func__ << " hmm, they didn't like " << old_auth_method <<
...
int MonClient::handle_auth_request(Connection *con,
                                   AuthConnectionMeta *auth_meta,
                                   bool more,
                                   uint32_t auth_method,
...
AuthAuthorizer *MonClient::build_authorizer(int service_id) constconst {
    std::lock_guard l(monc_lock);
    if (auth)
    {
    ...
    {
        ldout(cct,
              0) << __func__ << " for " << ceph_entity_type_name(service_id)
...
MonConnection::MonConnection(CephContext *cct, ConnectionRef con,
                             uint64_t global_id, AuthRegistry *ar)
...
bool MonConnection::have_session() const const
{
...
int MonConnection::get_auth_request(uint32_t *method,
                                    std::vector < uint32_t > *preferred_modes,
                                    ceph::buffer::list *bl,
...
int MonConnection::handle_auth_reply_more(AuthConnectionMeta *auth_meta,
        const ceph::buffer::list &bl,
        ceph::buffer::list *reply)
{
...
int MonConnection::handle_auth_done(AuthConnectionMeta *auth_meta,
...
int MonConnection::handle_auth_bad_method(uint32_t old_auth_method,
        int result,
        const std::vector < uint32_t >
        &allowed_methods,
        const std::vector < uint32_t >
        &allowed_modes)
{
    ...
    auth_method = *p;
    ldout(cct,
          10) << __func__ << " will try " << auth_method << " next" << dendl;
    return 0;
}

int MonConnection::handle_auth(MAuthReply *m,
                               const EntityName &entity_name,
                               uint32_t want_keys, RotatingKeyRing *keyring)
{
...
int MonConnection::_init_auth(uint32_t method,
                              const EntityName &entity_name,
                              uint32_t want_keys,
                              RotatingKeyRing *keyring, bool msgr2)
{
    ...
    if (!msgr2 &&
        ...
        ldout(cct, 1) << __func__
                      << " not requesting MGR keys from pre-kraken monitor" << dendl;
        want_keys &= ~CEPH_ENTITY_TYPE_MGR;
    }
...
const char *monc_error_category::name() const const noexcept
{
    return "monc";
}

const char *monc_error_category::message(int ev, char *,
...
std::string monc_error_category::message(int ev) constconst {
    return message(ev, nullptr, 0);
}

bs::error_condition monc_error_category::
...
bool monc_error_category::equivalent(int ev,
                                     const bs::
...
int monc_error_category::from_code(int ev) const const noexcept
{
    if (ev == 0) {
        return 0;
    }
