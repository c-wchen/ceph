...
namespace librbd {
    ...
    class C_OrderedWrite:public Context {
      public:
        C_OrderedWrite(CephContext * cct,
                       LibrbdWriteback::write_result_d * result,
    ...
    struct C_WriteJournalCommit:public Context {
        ...
        bool request_sent = false;
         ...
         C_WriteJournalCommit(ImageCtx * _image_ctx, const std::string & _oid,
                              ...
                              const bufferlist & _bl,
                              const SnapContext & _snapc, uint64_t _journal_tid,
                              const ZTracer::Trace & trace, Context * _req_comp)
        :image_ctx(_image_ctx), oid(_oid), object_no(_object_no), off(_off),
            bl(_bl), snapc(_snapc), journal_tid(_journal_tid), trace(trace),
            req_comp(_req_comp) {
        ...
        void commit_io_event_extent(int r) {
            CephContext *cct = image_ctx->cct;
            ldout(cct, 20) << this << " C_WriteJournalCommit: "
                << "write committed: updating journal commit position" << dendl;
            ...
            for (Extents::iterator it = file_extents.begin();
                ...
                image_ctx->journal->commit_io_event_extent(journal_tid,
                                                           it->first,
                                                           it->second, r);
            }
        }
        ...
        void send_request() {
            ...
            request_sent = true;
            auto req =
                new io::ObjectWriteRequest <> (image_ctx, oid, object_no, off,
  ...
  :    m_tid(0), m_lock(lock), m_ictx(ictx) {
    ...
    void LibrbdWriteback::read(const object_t & oid, uint64_t object_no,
        ...
        auto req_comp =
            new io::ReadResult::C_SparseReadRequest <> (aio_comp, { {0, len}
                                                        }
                                                        , false);
        auto req =
            io::ObjectReadRequest <>::create(m_ictx, oid.name, object_no, off,
    ...
    bool LibrbdWriteback::may_copy_on_write(const object_t & oid,
        ...
        uint64_t object_overlap =
            m_ictx->prune_parent_extents(objectx, overlap);
    ...
    ceph_tid_t LibrbdWriteback::write(const object_t & oid,
        ...
        C_OrderedWrite *req_comp =
            new C_OrderedWrite(m_ictx->cct, result, trace,
                               this);

        // all IO operations are flushed prior to closing the journal
        assert(journal_tid == 0 || m_ictx->journal != NULL);
        if (journal_tid != 0) {
            m_ictx->journal->flush_event(journal_tid,
            ...
            auto req =
                new io::ObjectWriteRequest <> (m_ictx, oid.name, object_no, off,
                                               bl, snapc, 0, trace, req_comp);
            req->send();
        }
        return ++m_tid;
    }
    ...
    void LibrbdWriteback::overwrite_extent(const object_t & oid, uint64_t off,
        ...
        for (Extents::iterator it = file_extents.begin();
            ...
            if (new_journal_tid != 0) {
                ...
                m_ictx->journal->commit_io_event_extent(original_journal_tid,
                                                        it->first, it->second,
                                                        0);
            }
        }
