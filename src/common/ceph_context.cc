  ...
  public:
    ...
    void handle_conf_change(const md_config_t * conf,
        ...
        if (changed.count("log_stderr_prefix")) {
            log->set_log_stderr_prefix(conf->get_val < string >
                                       ("log_stderr_prefix"));
        }
...
class CephContextObs:public md_config_obs_t {
  ...
  public:
    ...
    void handle_conf_change(const md_config_t * conf,
                            const std::set < std::string > &changed) override {
        if (changed.
            count("enable_experimental_unrecoverable_data_corrupting_features"))
        {
    ...
    if (command == "perfcounters_dump" || command == "1" ||
        ...
        if (command == "config show") {
            ...
            if (!(cmd_getval(this, cmdmap, "var", var)) ||
                ...
                f->dump_string("error",
                               "syntax error: 'config set <var> <value>'");
            }
            else {
                ...
                if (r < 0) {
                    f->dump_stream("error") << "error setting '" << var <<
                        "' to '" << valstr << "': " << cpp_strerror(r);
                }
                else {
                    ostringstream ss;
            ...
            if (!cmd_getval(this, cmdmap, "var", var)) {
                ...
                if (r < 0) {
                    f->dump_stream("error") << "error getting '" << var << "': "
                        << cpp_strerror(r);
                }
                else {
            ...
            for (map < string, pair < string, string > >::iterator p =
                 diff.begin(); p != diff.end(); ++p) {
            ...
            if (!cmd_getval(this, cmdmap, "var", setting)) {
                f->dump_string("error",
                               "syntax error: 'config diff get <var>'");
            }
            else {
                md_config_t def_conf;
                def_conf.set_val("cluster", _conf->cluster);
...
_lockdep_obs(NULL), crush_location(this), _cct_perf(NULL)
{
    ...
    _admin_socket->register_command("perfcounters_dump", "perfcounters_dump",
                                    _admin_hook, "");
    _admin_socket->register_command("1", "1", _admin_hook, "");
    _admin_socket->register_command("perf dump",
                                    "perf dump name=logger,type=CephString,req=false name=counter,type=CephString,req=false",
                                    _admin_hook, "dump perfcounters value");
    _admin_socket->register_command("perfcounters_schema",
                                    "perfcounters_schema", _admin_hook, "");
    _admin_socket->register_command("perf histogram dump",
    ...
    _admin_socket->register_command("perf schema", "perf schema", _admin_hook,
                                    "dump perfcounters schema");
    _admin_socket->register_command("perf histogram schema",
                                    "perf histogram schema", _admin_hook,
                                    "dump perf histogram schema");
    _admin_socket->register_command("perf reset",
                                    "perf reset name=var,type=CephString",
                                    _admin_hook,
                                    "perf reset <name>: perf reset all or one perfcounter name");
    _admin_socket->register_command("config show", "config show", _admin_hook,
                                    "dump current config settings");
    _admin_socket->register_command("config help",
                                    "config help name=var,type=CephString,req=false",
                                    _admin_hook,
                                    "get config setting schema and descriptions");
    _admin_socket->register_command("config set",
                                    "config set name=var,type=CephString name=val,type=CephString,n=N",
                                    _admin_hook,
                                    "config set <field> <val> [<val> ...]: set a config variable");
    _admin_socket->register_command("config get",
                                    "config get name=var,type=CephString",
                                    _admin_hook,
                                    "config get <field>: get the config value");
    _admin_socket->register_command("config diff", "config diff", _admin_hook,
                                    "dump diff of current config and default config");
    _admin_socket->register_command("config diff get",
                                    "config diff get name=var,type=CephString",
                                    _admin_hook,
                                    "dump diff get <field>: dump diff of current and default config setting <field>");
    _admin_socket->register_command("log flush", "log flush", _admin_hook,
                                    "flush log entries to log file");
    _admin_socket->register_command("log dump", "log dump", _admin_hook,
                                    "dump recent log entries to log file");
    _admin_socket->register_command("log reopen", "log reopen", _admin_hook,
                                    "reopen log file");
...
CephContext::~CephContext()
{
    ...
    for (map < string, SingletonWrapper * >::iterator it =
         _associated_objs.begin(); it != _associated_objs.end(); ++it)
        delete it->second;
...
uint32_t CephContext::get_module_type() const const
{
    return _module_type;
}
...
int CephContext::get_init_flags() const const
{
    return _init_flags;
}
