        ...
        fs_cluster_id_t fscid, bool recover)
{
    ...
    auto [it, inserted] = filesystems.emplace(std::piecewise_construct, std::forward_as_tuple(fs.fscid),
                       ...
                       list < pair < health_status_t, string> > *detail) const
{
    ...
    if (standby_count_wanted) {
        CachedStackStringStream css;
        *css << "insufficient standby daemons available: have " << standby_daemons.size() << "; want " << standby_count_wanted
             << " more";
...
void FSMap::decode(bufferlist::const_iterator& p)
{
    ...
    decode(enable_multiple, p);
    {
        ...
        for (__u32 i = 0; i < len; i++) {
            ...
            [[maybe_unused]] auto [it, inserted] = filesystems.emplace(std::piecewise_construct, std::forward_as_tuple(fs.fscid),
                                                   std::forward_as_tuple(std::move(fs)));
            ceph_assert(inserted);
        }
    }
...
void FSMap::sanity(bool pending) const
{
    ...
    for ([[maybe_unused]] const auto& [fscid, fs] : filesystems) {
        ...
        ceph_assert(std::ranges::all_of(members, [&infos = fs.mds_map.mds_info](auto m) {
