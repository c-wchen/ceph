...
namespace rbd {
    namespace mirror {
        ...
        class MirrorAdminSocketHook:public AdminSocketHook {
          ...
          private:
            typedef std::map < std::string,
                MirrorAdminSocketCommand * >Commands;
        ...
        Mirror::Mirror(CephContext * cct,
            ...
            m_args(args), m_lock("rbd::mirror::Mirror"),
            m_local(new librados::Rados()),
            m_asok_hook(new MirrorAdminSocketHook(cct, this)) {
            cct->lookup_or_create_singleton_object < Threads <
                librbd::ImageCtx > >(m_threads, "rbd_mirror::threads");
            m_service_daemon.
        ...
        void Mirror::run() {
            ...
            while (!m_stopping) {
                ...
                m_cond.WaitInterval(m_lock,
                                    utime_t(m_cct->_conf->get_val < int64_t >
              ...
              for (auto & peer:kv.second) {
                    ...
                    if (pool_replayers_it != m_pool_replayers.end()) {
                        ...
                        if (pool_replayer->is_blacklisted()) {
                            derr << "restarting blacklisted pool replayer for "
                                << peer << dendl;
                            // TODO: make async
                        ...
                        dout(20) << "starting pool replayer for " << peer <<
                            dendl;
                        unique_ptr < PoolReplayer >
                            pool_replayer(new
                                          PoolReplayer(m_threads,
                                                       m_service_daemon.get(),
                                                       m_image_deleter.get(),
