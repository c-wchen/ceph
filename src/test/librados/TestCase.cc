...
void RadosTestParamPPNS::TearDownTestCase()
{
    if (cache_pool_name.length()) {
        ...
        ASSERT_EQ(0,
                  s_cluster.
                  mon_command("{\"prefix\": \"osd pool delete\", \"pool\": \"" +
                              cache_pool_name + "\", \"pool2\": \"" +
                              cache_pool_name +
...
void RadosTestParamPPNS::SetUp()
{
    if (strcmp(GetParam(), "cache") == 0 && cache_pool_name.empty()) {
        ...
        ASSERT_EQ(0,
                  cluster.
                  mon_command("{\"prefix\": \"osd pool create\", \"pool\": \"" +
                              cache_pool_name + "\", \"pg_num\": 4}", inbl,
                              NULL, NULL));
        ASSERT_EQ(0,
                  cluster.
        ...
        ASSERT_EQ(0,
                  ...
                  ("{\"prefix\": \"osd tier set-overlay\", \"pool\": \"" +
                   pool_name + "\", \"overlaypool\": \"" + cache_pool_name +
                   "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  cluster.
                  mon_command
                  ("{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" +
                   cache_pool_name + "\", \"mode\": \"writeback\"}", inbl, NULL,
                   NULL));
    ...
    while (--tries) {
        ...
        for (NObjectIterator it = ioctx.nobjects_begin();
            ...
            if (completion->get_return_value() == -ENOENT) {
                ++got_enoent;
                std::cout << " got ENOENT removing " << it->
                    get_oid() << std::endl;
            }
            else {
