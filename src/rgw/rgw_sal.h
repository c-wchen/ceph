...
class RGWGetBucketStats_CB:public RefCountedObject {
  ...
  public:
    ...
    virtual void set_response(std::map < RGWObjCategory,
                              RGWStorageStats > *_stats) {
        stats = _stats;
    }
};
 ...
 */
     ...
     * there is a Bucket, otherwise use the get_object() in the Bucket class. */
            virtual std::unique_ptr < Object >
                get_object(const rgw_obj_key & k) = 0;
    ...
    /** Get a unique Swift transaction ID specific to this zone */
            virtual std::
                string zone_unique_trans_id(const uint64_t unique_num) = 0;
    ...
    /** Wake up sync threads for bucket metadata sync */
            virtual void wakeup_meta_sync_shards(std::set < int >&shard_ids) =
                0;
    ...
    /** Get a configuration value for the given name */
            virtual int get_config_key_val(std::string name, bufferlist * bl) =
                0;
    ...
    /** Get an IAM Role by ID */
            ...
            virtual std::unique_ptr < RGWRole >
                get_role(const RGWRoleInfo & info) = 0;
 ...
 */
    ...
    /** Check if a User pointer is empty */
            static bool empty(const User * u) {
                return (!u || u->empty());
            }
    /** Check if a User unique_pointer is empty */
            static bool empty(const std::unique_ptr < User > &u) {
                return (!u || u->empty());
            }
 ...
 */
     ...
     */
            struct ListParams {
                ...
                friend std::ostream & operator<<(std::ostream & out,
                                                 const ListParams & p) {
                    out << "rgw::sal::Bucket::ListParams{ prefix=\"" << p.
                        prefix << "\", delim=\"" << p.
                        delim << "\", marker=\"" << p.
                        marker << "\", end_marker=\"" << p.
                        end_marker << "\", ns=\"" << p.
                        ns << "\", enforce_ns=" << p.
                        enforce_ns << ", list_versions=" << p.
                        list_versions << ", allow_unordered=" << p.
                        allow_unordered << ", shard_id=" << p.shard_id << " }";
                    return out;
            }};
    /**
     ...
     * If @a get_stats is true, then statistics on the bucket are also looked up. */
            virtual int load_bucket(const DoutPrefixProvider * dpp,
                                    optional_yield y, bool get_stats = false) =
                0;
    /** Read the bucket stats from the backing Store, synchronous */
            virtual int read_stats(const DoutPrefixProvider * dpp,
                                   ...
                                   nullptr, bool * syncstopped = nullptr) = 0;
    /** Read the bucket stats from the backing Store, asynchronous */
            virtual int read_stats_async(const DoutPrefixProvider * dpp,
                                         const bucket_index_layout_generation &
                                         idx_layout, int shard_id,
                                         RGWGetBucketStats_CB * ctx) = 0;
    ...
    /** Refresh the metadata stats (size, count, and so on) from the backing store */
            virtual int update_container_stats(const DoutPrefixProvider * dpp) =
                0;
    ...
    /** Check the state of the bucket index, and get stats from it.  May be removed from API */
            virtual int check_index(const DoutPrefixProvider * dpp,
                                    ...
                                    std::map < RGWObjCategory,
                                    RGWStorageStats > &calculated_stats) = 0;
    ...
    /** Check if a Bucket pointer is empty */
            static bool empty(const Bucket * b) {
                return (!b || b->empty());
            }
    /** Check if a Bucket unique pointer is empty */
            static bool empty(const std::unique_ptr < Bucket > &b) {
                return (!b || b->empty());
            }
    ...
    /** Write @a notifications with (optionally) @a objv_tracker into the bucket notification config */
            virtual int write_topics(const rgw_pubsub_bucket_topics &
                                     notifications,
                                     RGWObjVersionTracker * objv_tracker,
                                     optional_yield y,
                                     const DoutPrefixProvider * dpp) = 0;
 ...
 */
        class BucketList {
          ...
          public:
            BucketList():buckets(), truncated(false) {
                ...
                buckets(std::move(_bl.buckets)), truncated(_bl.truncated) {
            }
  ...
  /** Get the list of buckets.  The list is a map of <bucket-name, Bucket> pairs. */
            std::map < std::string, std::unique_ptr < Bucket >> &get_buckets() {
                return buckets;
            }
  ...
  /** Clear the list */ void clear(void) {
                buckets.clear();
                truncated = false;
            }
        };
 ...
 */
    ...
    /** Dump driver-specific object layout info in JSON */
            virtual int dump_obj_layout(const DoutPrefixProvider * dpp,
                                        optional_yield y, Formatter * f) = 0;
    ...
    /** Check to see if the given object pointer is uninitialized */
            static bool empty(const Object * o) {
                return (!o || o->empty());
            }
    /** Check to see if the given object unique pointer is uninitialized */
            static bool empty(const std::unique_ptr < Object > &o) {
                return (!o || o->empty());
            }
 ...
 */
  ...
  /** Get all the cached parts that make up this upload */
            virtual std::map < uint32_t,
                std::unique_ptr < MultipartPart >> &get_parts() = 0;
 ...
 */
  ...
  /** Check if a placement target (by name) exists in this zonegroup */
            virtual bool placement_target_exists(std::string & target) const =
                0;
  ...
  /** Get the list of hostnames from this zone */
            virtual int get_hostnames(std::list < std::string > &names) const =
                0;
...
class DriverManager {
  ...
  /** Get a full driver by service name */
        ...
        rgw::sal::Driver * driver =
            init_storage_provider(dpp, cct, cfg, use_gc_thread, use_lc_thread,
  ...
  /** Initialize a new full Driver */
    static rgw::sal::Driver * init_storage_provider(const DoutPrefixProvider *
                                                    dpp, CephContext * cct,
                                                    const Config & cfg,
                                                    bool use_gc_thread,
                                                    bool use_lc_thread,
  ...
  /** Initialize a new raw Driver */
    static rgw::sal::Driver *
        init_raw_storage_provider(const DoutPrefixProvider * dpp,
