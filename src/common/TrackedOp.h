...
class OpHistory {
    ...
    std::atomic_size_t history_size {
    0};
    std::atomic_uint32_t history_duration {
    0};
    std::atomic_size_t history_slow_op_size {
    0};
    std::atomic_uint32_t history_slow_op_threshold {
    0};
  ...
  OpHistory(CephContext * c):cct(c), opsvc(this) {
    ...
    void dump_ops(utime_t now, ceph::Formatter * f,
                  std::set < std::string > filters = {
                  ""}, bool by_duration = false);
    void dump_slow_ops(utime_t now, ceph::Formatter * f,
                       std::set < std::string > filters = {
                       ""});
    void on_shutdown();
...
class OpTracker {
  ...
  public:
    ...
    void set_history_slow_op_size_and_threshold(uint32_t new_size,
    ...
    bool dump_ops_in_flight(ceph::Formatter * f, bool print_only_blocked =
                            false, std::set < std::string > filters = {
                            ""}, bool count_only = false);
    bool dump_historic_ops(ceph::Formatter * f, bool by_duration =
                           false, std::set < std::string > filters = {
                           ""});
    bool dump_historic_slow_ops(ceph::Formatter * f,
                                std::set < std::string > filters = {
                                ""});
...
class TrackedOp:public boost::intrusive::list_base_hook <> {
  ...
  protected:
    ...
    TrackedOp(OpTracker * _tracker,
              const utime_t & initiated):tracker(_tracker),
    ...
    virtual bool filter_out(const std::set < std::string > &filters) {
        return true;
    }
        ...
        if (nref_snap == 1) {
            ...
            case STATE_LIVE:
                ...
                if (!tracker->is_tracking()) {
                    ...
                    tracker->
                        record_history_op(TrackedOpRef
                                          (this, /* add_ref = */ false));
                }
                break;
  ...
  private:
    ...
    virtual std::string_view state_string() const {
        std::lock_guard l(lock);
        return events.empty() ? std::string_view() : std::string_view(events.
                                                                      rbegin()->
                                                                      str);
