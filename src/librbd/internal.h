...
namespace librbd {
    ...
    int clone(ImageCtx * p_imctx, IoCtx & c_ioctx, const std::string & c_name,
              ...
              const std::string & non_primary_global_image_id,
              const std::string & primary_mirror_uuid);
    int rename(librados::IoCtx & io_ctx, const char *srcname,
               const char *dstname);
    ...
    void image_info(const ImageCtx * ictx, image_info_t & info,
                    size_t info_size);
    ...
    int poll_io_events(ImageCtx * ictx, io::AioCompletion ** comps,
                       int numcomp);
    int metadata_list(ImageCtx * ictx, const string & last, uint64_t max,
                      map < string, bufferlist > *pairs);
    int metadata_get(ImageCtx * ictx, const std::string & key,
                     std::string * value);
