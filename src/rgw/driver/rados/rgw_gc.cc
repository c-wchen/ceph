                ...
                bool *truncated, bool &processing_queue)
{
    ...
    for (; *index < max_objs && result.size() < max; (*index)++, marker.clear(), check_queue = false) {
        ...
        if (! transitioned_objects_cache[*index] && ! check_queue && ! processing_queue) {
            ret = cls_rgw_gc_list(store->gc_pool_ctx, obj_names[*index], marker, max - result.size(), expired_only, entries,
            ...
            if (ret == -ENOENT || entries.size() == 0) {
                if (objv.ver == 0) {
                    ...
                    if (! expired_only) {
                        ...
                        ret = cls_rgw_gc_list(store->gc_pool_ctx, obj_names[*index], marker, 1, false, non_expired_entries, truncated,
                                              next_marker);
        ...
        if (transitioned_objects_cache[*index] || check_queue || processing_queue) {
            ...
            ret = cls_rgw_gc_queue_list_entries(store->gc_pool_ctx, obj_names[*index], marker,
                                                (max - result.size()) - entries.size(), expired_only, queue_entries, truncated, next_marker);
            if (ret < 0) {
                return ret;
            }
        }
                   ...
                   RGWGCIOManager &io_manager, optional_yield y)
{
    ...
    do {
        ...
        if (! transitioned_objects_cache[index]) {
            ret = cls_rgw_gc_list(store->gc_pool_ctx, obj_names[index], marker, max, expired_only, entries, &truncated,
                                  next_marker);
            ldpp_dout(this, 20) <<
            ...
            if ((objv.ver == 1) && entries.size() == 0) {
                ...
                if (non_expired_entries.size() == 0) {
                    transitioned_objects_cache[index] = true;
                    marker.clear();
                    ldpp_dout(this, 20) << "RGWGC::process cls_rgw_gc_list returned NO non expired entries, so setting cache entry to TRUE"
                                        << dendl;
                } else {
                    ret = 0;
                    goto done;
        ...
        if (transitioned_objects_cache[index]) {
            ret = cls_rgw_gc_queue_list_entries(store->gc_pool_ctx, obj_names[index], marker, max, expired_only, entries,
                                                &truncated, next_marker);
            ldpp_dout(this, 20) <<
