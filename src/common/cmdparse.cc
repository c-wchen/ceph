...
void dump_cmd_to_json(Formatter * f, const string & cmd)
{
...
dump_cmddesc_to_json(Formatter * jf,
                     ...
                     const string & module,
                     const string & perm, const string & avail, uint64_t flags)
{
    jf->open_object_section(secname.c_str());
    jf->open_array_section("sig");
    dump_cmd_to_json(jf, cmdsig);
...
bool
cmdmap_from_json(vector < string > cmd, map < string, cmd_vartype > *mapp,
                 stringstream & ss)
{
    ...
    try {
        ...
        for (map < string, json_spirit::mValue >::iterator it = o.begin();
            ...
            default:
                throw(runtime_error
                      ("JSON array/object not allowed " + fullcmd));
                break;
