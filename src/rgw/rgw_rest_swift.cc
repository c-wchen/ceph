...
static void dump_account_metadata(req_state * const s,
                                  ...
                                  RGWUsageStats > &policies_stats,
                                  /* const */ map < string, bufferlist > &attrs,
                                  const RGWQuotaInfo & quota,
                                  const RGWAccessControlPolicy_SWIFTAcct &
                                  policy)
{
    ...
    dump_header(s, "X-Account-Bytes-Used-Actual",
                global_stats.bytes_used_rounded);
  ...
  for (const auto & kv:policies_stats) {
    ...
    for (iter = attrs.lower_bound(RGW_ATTR_PREFIX); iter != attrs.end(); ++iter) {
        ...
        map < string, string >::const_iterator geniter =
...
void RGWListBuckets_ObjStore_SWIFT::send_response_begin(bool has_buckets)
{
    ...
    if (!s->cct->_conf->rgw_swift_enforce_content_length) {
        ...
        dump_account_metadata(s,
                              ...
                              static_cast <
                              RGWAccessControlPolicy_SWIFTAcct & >(*s->
                                                                   user_acl));
        dump_errno(s);
...
void RGWListBuckets_ObjStore_SWIFT::handle_listing_chunk(rgw::sal::BucketList
                                                         && buckets)
{
    if (wants_reversed) {
...
void RGWListBuckets_ObjStore_SWIFT::send_response_data(rgw::sal::
                                                       BucketList & buckets)
{
    if (!sent_data) {
        return;
...
void RGWListBuckets_ObjStore_SWIFT::dump_bucket_entry(const rgw::sal::
                                                      Bucket & bucket)
{
...
void RGWListBuckets_ObjStore_SWIFT::send_response_data_reversed(rgw::sal::
                                                                BucketList &
                                                                buckets)
{
    if (!sent_data) {
    ...
    for ( /* initialized above */ ;
         iter != m.rend()
         && !boost::algorithm::starts_with(iter->first, prefix); ++iter) {
        /* NOP */ ;
    }
    ...
    while (iter != objs.end() || pref_iter != common_prefixes.end()) {
        ...
        if (do_objs && (allow_unordered || marker.empty() || marker < key)) {
            ...
            if (!iter->meta.user_data.empty())
                s->formatter->dump_string("user_custom_data",
                                          iter->meta.user_data);
                                    ...
                                    const RGWBucketWebsiteConf & ws_conf)
{
    ...
    if (rgw::sal::Object::empty(s->object.get())) {
        ...
        if (!s->bucket->get_placement_rule().name.empty()) {
            dump_header(s, "X-Storage-Policy",
                        s->bucket->get_placement_rule().name);
        }
        dump_header(s, "X-Storage-Class",
                                        ...
                                        bool * const has_cors)
{
    ...
    *has_cors = false;
    ...
    if (allow_origins) {
        ...
        int r =
            swift_cors->create_update(allow_origins, allow_headers,
                                      expose_headers, max_age);
        if (r < 0) {
            ldpp_dout(s,
                      0) << "Error creating/updating the cors configuration" <<
                dendl;
            delete swift_cors;
            return r;
        }
...
static int get_swift_versioning_settings(req_state * const s,
...
int RGWCreateBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
    ...
    placement_rule.init(s->info.env->get("HTTP_X_STORAGE_POLICY", ""),
...
static inline int handle_metadata_errors(req_state * const s, const int op_ret)
{
    if (op_ret == -EFBIG) {
        ...
        const auto error_message =
            boost::str(boost::format("Metadata value longer than %lld")
                       % s->cct->_conf.get_val < Option::size_t >
                       ("rgw_max_attr_size"));
        ...
        const auto error_message =
            boost::str(boost::format("Too many metadata items; max %lld")
...
static int get_delete_at_param(req_state * s,
                               boost::optional < real_time > &delete_at)
{
...
int RGWPutObj_ObjStore_SWIFT::update_slo_segment_size(rgw_slo_entry & entry)
{
    ...
    if (bucket_name.compare(s->bucket->get_name()) != 0) {
        r = driver->get_bucket(s, s->user.get(), s->user->get_id().tenant,
                               bucket_name, &bucket, s->yield);
        if (r < 0) {
    ...
    r = rgw_compression_info_from_attrset(slo_seg->get_attrs(), compressed,
                                          cs_info);
    if (r < 0) {
        return -EIO;
    }
...
int RGWPutObj_ObjStore_SWIFT::get_params(optional_yield y)
{
    ...
    if (!s->length) {
        ...
        if (!encoding || strcmp(encoding, "chunked") != 0) {
            ldpp_dout(this,
                      20) << "neither length nor chunked encoding" << dendl;
    ...
    if (!s->generic_attrs.count(RGW_ATTR_CONTENT_TYPE)) {
        ldpp_dout(this,
                  5) << "content type wasn't provided, trying to guess" <<
            dendl;
    ...
    if (exists) {
        if (multipart_manifest != "put") {
            ldpp_dout(this,
                      5) << "invalid multipart-manifest http param: " <<
...
#define MAX_SLO_ENTRY_SIZE (1024 + 128) // 1024 - max obj name, 128 - enough extra for other info
        uint64_t max_len =
            s->cct->_conf->rgw_max_slo_entries * MAX_SLO_ENTRY_SIZE;

        slo_info = new RGWSLOInfo;

        int r = 0;
        std::tie(r, slo_info->raw_data) =
            rgw_rest_get_json_input_keep_data(s->cct, s, slo_info->entries,
                                              max_len);
        if (r < 0) {
            ldpp_dout(this,
                      5) << "failed to read input for slo r=" << r << dendl;
            return r;
        }

        if ((int64_t) slo_info->entries.size() >
            s->cct->_conf->rgw_max_slo_entries) {
            ldpp_dout(this,
                      5) << "too many entries in slo request: " << slo_info->
                entries.size() << dendl;
            return -EINVAL;
        }
...
static int get_swift_account_settings(req_state * const s,
                                      ...
                                      const policy, bool * const has_policy)
{
    *has_policy = false;
...
static void bulkdelete_respond(const unsigned num_deleted, const unsigned int num_unfound, const std::list < RGWBulkDelete::fail_desc_t > &failures, const int prot_flags,  /* in  */
...
{
    map < string, bufferlist >::iterator iter =
        attrs.find(RGW_ATTR_CONTENT_TYPE);
    if (iter != attrs.end()) {
        content_type = rgw_bl_str(iter->second);
    }
}

static void dump_object_metadata(const DoutPrefixProvider * dpp,
                                 req_state * const s, const map < string,
                                 bufferlist > &attrs)
{
  ...
  for (const auto & kv:response_attrs) {
    ...
    if (iter != std::end(attrs)) {
        ...
        try {
            ...
            ldpp_dout(dpp,
                      0) << "ERROR: cannot decode object's " RGW_ATTR_DELETE_AT
                " attr, ignoring" << dendl;
        }
    }
}
...
int RGWBulkDelete_ObjStore_SWIFT::get_data(list < RGWBulkDelete::acct_path_t >
                                           &items, bool * const is_truncated)
{
    ...
    while (cioin.getline(buf, sizeof(buf))) {
        ...
        ldpp_dout(this,
                  20) << "extracted Bulk Delete entry: " << path_str << dendl;
    ...
    RGWBulkUploadOp_ObjStore_SWIFT::create_stream()
{
    class SwiftStreamGetter:public StreamGetter {
      ...
      public:
         SwiftStreamGetter(const DoutPrefixProvider * dpp, req_state * const s,
                           const size_t conlen)
        :dpp(dpp), conlen(conlen), curpos(0), s(s) {
            ...
            ldpp_dout(dpp, 20) << "bulk_upload: get_at_most max_to_read="
                << max_to_read
                << ", dst.c_str()=" << reinterpret_cast < intptr_t >
                (dst.c_str()) << dendl;
        ...
        ssize_t get_exactly(size_t want, ceph::bufferlist & dst) override {
            ldpp_dout(dpp,
                      20) << "bulk_upload: get_exactly want=" << want << dendl;

            /* FIXME: do this in a loop. */
            const auto ret = get_at_most(want, dst);
            ldpp_dout(dpp,
                      20) << "bulk_upload: get_exactly ret=" << ret << dendl;
            if (ret < 0) {
                return ret;
            }
...
const vector < pair < string,
    RGWInfo_ObjStore_SWIFT::info >> RGWInfo_ObjStore_SWIFT::swift_info = {
...
void RGWInfo_ObjStore_SWIFT::execute(optional_yield y)
{
    ...
    if (!swiftinfo_sig.empty() &&
        !swiftinfo_expires.empty() && !is_expired(swiftinfo_expires, this)) {
                                             ...
                                             rgw::sal::Driver * driver)
{
    ...
    if (zonegroup.get_placement_target_names(targets)) {
      for (const auto & placement_targets:targets) {
            ...
            if (placement_targets.
                compare(zonegroup.get_default_placement_name()) == 0)
    ...
    formatter.dump_int("max_object_name_size",
                       RGWHandler_REST::MAX_OBJ_NAME_LEN);
    formatter.dump_bool("strict_cors_mode", true);
    formatter.dump_int("max_container_name_length",
                       RGWHandler_REST::MAX_BUCKET_NAME_LEN);
    formatter.close_section();
}
...
bool RGWInfo_ObjStore_SWIFT::is_expired(const std::string & expires,
                                        const DoutPrefixProvider * dpp)
{
    string err;
    ...
    if (!err.empty()) {
        ldpp_dout(dpp,
                  5) << "failed to parse siginfo_expires: " << err << dendl;
        return true;
    }

    if (expiration <= (uint64_t) now.sec()) {
        ldpp_dout(dpp,
                  5) << "siginfo expired: " << expiration << " <= " << now.
            sec() << dendl;
        return true;
    }
...
void RGWFormPost::init(rgw::sal::Driver * const driver,
                       req_state * const s, RGWHandler * const dialect_handler)
{
...
std::size_t RGWFormPost::get_max_file_size()
{                               /*const */
    ...
    if (!err.empty()) {
        ldpp_dout(this,
                  5) << "failed to parse FormPost's max_file_size: " << err <<
            dendl;
        return 0;
    }
...
bool RGWFormPost::is_non_expired()
{
    ...
    if (!err.empty()) {
        ldpp_dout(this,
                  5) << "failed to parse FormPost's expires: " << err << dendl;
        return false;
    }
...
void RGWFormPost::get_owner_info(const req_state * const s,
                                 RGWUserInfo & owner_info) const const
{
    ...
    std::unique_ptr < rgw::sal::Bucket > bucket;
    int ret =
        driver->get_bucket(s, user.get(), user->get_tenant(), bucket_name,
                           &bucket, s->yield);
    if (ret < 0) {
        throw ret;
    }
    ...
    ldpp_dout(this,
              20) << "temp url user (bucket owner): " << bucket->get_info().
        owner << dendl;
...
std::string RGWFormPost::get_current_filename() constconst
{
    try {
        const auto & field =
            current_data_part->fields.at("Content-Disposition");
...
std::string RGWFormPost::get_current_content_type() constconst
{
    try {
...
bool RGWFormPost::is_next_file_to_upload()
{
    if (!stream_done) {
        ...
        if (std::end(part.fields) != field_iter) {
            ...
            if (std::end(params) != filename_iter
                && !filename_iter->second.empty()) {
                current_data_part = std::move(part);
                return true;
            }
        }
                                           ...
                                           optional_yield y)
{
    ...
    class RGWGetErrorPage:public RGWGetObj_ObjStore_SWIFT {
      public:
        RGWGetErrorPage(rgw::sal::Driver * const driver,
                        ...
                        req_state * const s, const int http_ret) {
    ...
    if (!rgw::sal::Bucket::empty(s->bucket.get())) {
        s->object =
            s->bucket->
            get_object(rgw_obj_key(std::to_string(http_ret) + error_doc));
    }
    else {
        s->object =
            driver->
...
bool RGWSwiftWebsiteHandler::is_web_mode() const const
{
    const std::string_view webmode = s->info.env->get("HTTP_X_WEB_MODE", "");
    return boost::algorithm::iequals(webmode, "true");
}

bool RGWSwiftWebsiteHandler::can_be_website_req() const const
{
...
bool RGWSwiftWebsiteHandler::is_web_dir() const const
{
    ...
    const std::string subdir_marker = ws_conf.subdir_marker.empty()
        ? "application/directory" : ws_conf.subdir_marker;
    return subdir_marker == content_type && state->size <= 1;
}

bool RGWSwiftWebsiteHandler::is_index_present(const std::string & index) constconst
{
    std::unique_ptr < rgw::sal::Object > obj =
...
int RGWHandler_REST_SWIFT::validate_bucket_name(const string & bucket)
{
    ...
    if (len > MAX_BUCKET_NAME_LEN) {
        ...
        const auto msg =
            boost::str(boost::
                       format("Container name length of %lld longer than %lld")
                                            ...
                                            const std::string & frontend_prefix)
{
    ...
    ldpp_dout(s,
              10) << "ver=" << ver << " first=" << first << " req=" << req <<
        dendl;
    if (first.size() == 0)
        return 0;
    ...
    if (req.size()) {
        s->object = driver->get_object(rgw_obj_key(req, s->info.env->get("HTTP_X_OBJECT_VERSION_ID", ""))); /* rgw swift extension */
...
RGWHandler_REST *RGWRESTMgr_SWIFT::get_handler(rgw::sal::Driver * driver,
                                               req_state * const s,
                                               const rgw::auth::
                                               StrategyRegistry & auth_registry,
                                               const std::
                                               string & frontend_prefix)
{
    int ret =
        RGWHandler_REST_SWIFT::init_from_header(driver, s, frontend_prefix);
    if (ret < 0) {
...
RGWHandler_REST *RGWRESTMgr_SWIFT_Info::get_handler(rgw::sal::Driver * driver,
                                                    req_state * const s,
                                                    const rgw::auth::
                                                    StrategyRegistry &
                                                    auth_registry,
