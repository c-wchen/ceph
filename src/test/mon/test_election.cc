...
struct Election {
    ...
    int last_leader = -1;
     ...
     Election(int c, ElectionLogic::election_strategy es, int pingi =
              1, double tracker_halflife = 5);
    ~Election();
    ...
    void add_disallowed_leader(int disallowed) {
...
struct Owner:public ElectionOwner, RankProvider {
    ...
    string prefix_str;
     Owner(int r, ElectionLogic::election_strategy es, double tracker_halflife,
        ...
        victory_accepters(0), timer_steps(-1), timer_election(true) {
        std::stringstream str;
        str << "Owner" << rank << "        ";
        prefix_str = str.str();
    }
    // in-memory store: just save to variable void persist_epoch(epoch_t e) {
        persisted_epoch = e;
    }
    ...
    // in-memory store: don't need to validate void validate_store() {
        return;
    }
    ...
    bool is_current_member(int r) const {
    ...
    void send_pings() {
        ...
        for (int i = 0; i < parent->get_paxos_size(); ++i) {
            ...
            parent->queue_stable_or_timeout(rank, i,[o, r = rank, bl] {
                                            o->receive_ping(r, bl);
                                            }
                                            ,[o, r = rank] {
                                            o->receive_ping_timeout(r);
                                            }
            );
        }
    }
...
Election::Election(int c, ElectionLogic::election_strategy es, int pingi, double tracker_halflife):count(c), election_strategy(es), ping_interval(pingi),
pending_election_messages(0), timesteps_run(0), last_quorum_change(0),
...
void Election::claim_victory(int from, int to, epoch_t e,
                             const set < int >&members)
{
    Owner *o = electors[to];
...
bool Election::election_stable() const const
{
    // see if anybody has a timer running
  for (auto i:electors) {
        if (i.second->timer_steps != -1) {
            ldout(g_ceph_context,
                  30) << "rank " << i.first << " has timer value " << i.second->
                timer_steps << dendl;
            return false;
        }
    }
...
bool Election::quorum_stable(int timesteps_stable) constconst
{
    ldout(g_ceph_context,
          1) << "quorum_stable? last formed:" << last_quorum_formed <<
        ", last changed " << last_quorum_change << ", last reported members " <<
...
bool Election::all_agree_on_leader() const const
{
  ...
  for (auto & i:electors) {
        if (leader != i.second->logic.get_election_winner()) {
            ldout(g_ceph_context,
                  10) << "rank " << i.first << " has different leader " << i.
                second->logic.get_election_winner() << dendl;
            return false;
        }
    }
...
bool Election::check_epoch_agreement() const const
{
...
void single_startup_election_completes(ElectionLogic::
...
void blocked_connection_continues_election(ElectionLogic::
                                           election_strategy strategy)
{
    Election election(5, strategy);
...
void blocked_connection_converges_election(ElectionLogic::
                                           election_strategy strategy)
{
    Election election(5, strategy);
    election.block_bidirectional_messages(0, 1);
...
void netsplit_with_disallowed_tiebreaker_converges(ElectionLogic::
                                                   election_strategy strategy)
{
    Election election(5, strategy);
    ...
    auto call_elections =[&]{
      for (auto i:election.electors) {
                            ...
                            for (int i = 0; i < 5; ++i) {
                            netsplit(); call_elections(); election.run_timesteps(15);   // tests fail when I run 10 because 0 and 1 time out on same timestamp for some reason, why?
                            ...
                            int presplit_quorum_time =
                            election.last_quorum_formed; netsplit();
                            ...
                            call_elections(); election.run_timesteps(100);
                            ASSERT_EQ(election.last_quorum_formed,
  ...
  Election election(5, strategy); election.block_bidirectional_messages(0, 1); election.block_bidirectional_messages(0, 2); election.block_bidirectional_messages(0, 3); election.block_bidirectional_messages(0, 4); election.start_all(); election.run_timesteps(20); ASSERT_TRUE(election.quorum_stable(5)); ASSERT_FALSE(election.election_stable()); election.unblock_bidirectional_messages(0, 1); election.run_timesteps(100); ASSERT_TRUE(election.quorum_stable(50)); ASSERT_TRUE(election.election_stable()); ASSERT_TRUE(election.all_agree_on_leader()); ASSERT_TRUE(election.check_epoch_agreement()); election.block_bidirectional_messages(0, 1); election.unblock_bidirectional_messages(0, 4); for (auto i:election.electors)
                            ...
                            *get_peer_reports(ConnectionTracker & ct) {
                            ...
                            void handles_outdated_scoring(ElectionLogic::
                                                          election_strategy
                                                          strategy) {
                            ...
                            ldout(g_ceph_context,
                                  5) << "mangled the scores to be different" <<
                            dendl; election.start_all();
                            ...
                            void
                            handles_disagreeing_connectivity(ElectionLogic::
                                                             election_strategy
                                                             strategy) {
                            ...
                            election.start_all(); election.run_timesteps(50);   // let them all demote scores of their peers
                            ...
                            void handles_removing_ranks(ElectionLogic::
                                                        election_strategy
                                                        strategy) {
                            ceph_assert(strategy ==
                                        ElectionLogic::CONNECTIVITY);
                            for (int deletee = 0; deletee < 5; ++deletee) {
                            Election election(5, strategy); election.start_all(); int steps = election.run_timesteps(0); ldout(g_ceph_context, 10) << "ran in " << steps << " timesteps" << dendl; ASSERT_TRUE(election.election_stable()); ASSERT_TRUE(election.quorum_stable(6)); // double the timer_steps we use
                            ASSERT_TRUE(election.all_agree_on_leader()); ASSERT_TRUE(election.check_epoch_agreement()); election.remove_elector(deletee); ldout(g_ceph_context, 1) << "removed rank " << deletee << " from set" << dendl; election.start_all(); steps = election.run_timesteps(0); ASSERT_TRUE(election.election_stable()); ASSERT_TRUE(election.quorum_stable(6)); // double the timer_steps we use
                            ASSERT_TRUE(election.all_agree_on_leader());
                            ASSERT_TRUE(election.check_epoch_agreement());}
                            {
                            Election election(7, strategy);
                            for (int i = 0; i < (7 - 3); ++i) {
                            election.start_all(); election.remove_elector(0); int steps = election.run_timesteps(0); ldout(g_ceph_context, 1) << "ran in " << steps << " timesteps" << dendl; ASSERT_TRUE(election.election_stable()); ASSERT_TRUE(election.quorum_stable(6));  // double the timer_steps we use
                            ASSERT_TRUE(election.all_agree_on_leader());
                            ASSERT_TRUE(election.check_epoch_agreement());}
                            }
                            }
...
// TODO: test for expected failures; gtest probably supports that?
                            ...
                            test_disallowed
                            (blocked_connection_continues_election)
...
/* skip single_startup_election_completes because we crash
 ...
 quite following the rules anyway. */
                            ...
                            test_connectivity
                            (blocked_connection_converges_election)
