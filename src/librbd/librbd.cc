...
namespace {
    ...
    void mirror_image_status_cpp_to_c(const librbd::
                                      mirror_image_status_t & cpp_status,
    ...
    struct C_MirrorImageGetStatus:public Context {
        ...
        :mirror_image_status(mirror_image_status), on_finish(on_finish) {
            ...
            mirror_image_status_cpp_to_c(cpp_mirror_image_status,
                                         mirror_image_status);
            on_finish->complete(0);
        }
    };
...
namespace librbd {
    ...
    int RBD::aio_open(IoCtx & io_ctx, Image & image, const char *name,
        ...
        if (image.ctx != NULL) {
            ...
            ictx->state->open(false,
                              new C_OpenComplete(ictx, get_aio_completion(c),
                                                 &image.ctx));
        }
    ...
    int RBD::list(IoCtx & io_ctx, vector < string > &names) {
        ...
        if (r >= 0) {
            for (vector < string >::iterator itr = names.begin(), end =
                 names.end(); itr != end; ++itr) {
    ...
    int RBD::mirror_peer_set_client(IoCtx & io_ctx, const std::string & uuid,
                                    const std::string & client_name) {
        return librbd::api::Mirror <>::peer_set_client(io_ctx, uuid,
                                                       client_name);
    }

    int RBD::mirror_peer_set_cluster(IoCtx & io_ctx, const std::string & uuid,
                                     const std::string & cluster_name) {
        return librbd::api::Mirror <>::peer_set_cluster(io_ctx, uuid,
                                                        cluster_name);
    }
    ...
    RBD::AioCompletion::AioCompletion(void *cb_arg, callback_t complete_cb) {
        pc = reinterpret_cast <
            void *>(librbd::io::AioCompletion::
                    create(cb_arg, complete_cb, this));
    }
    ...
    int Image::list_children(set < pair < string, string > >*children) {
        ...
        if (r >= 0) {
            for (set < pair < string, string > >::const_iterator it =
                ...
                tracepoint(librbd, list_children_entry, it->first.c_str(),
                           it->second.c_str());
            }
        }
    ...
    bool Image::snap_exists(const char *snap_name) {
        ...
        bool exists;
        int r =
            librbd::snap_exists(ictx, cls::rbd::UserSnapshotNamespace(),
                                snap_name, &exists);
    ...
    int Image::snap_set(const char *snap_name) {
        ...
        tracepoint(librbd, snap_set_enter, ictx, ictx->name.c_str(),
                   ictx->snap_name.c_str(), ictx->read_only, snap_name);
        int r =
            librbd::snap_set(ictx, cls::rbd::UserSnapshotNamespace(),
                             snap_name);
    ...
    ssize_t Image::read2(uint64_t ofs, size_t len, bufferlist & bl,
        ...
        bl.push_back(std::move(ptr));
        int r =
            ictx->io_work_queue->read(ofs, len, io::ReadResult { &bl },
                                      op_flags);
        tracepoint(librbd, read_exit, r);
        return r;
    }
    ...
    ssize_t Image::write2(uint64_t ofs, size_t len, bufferlist & bl,
        ...
        int r =
            ictx->io_work_queue->write(ofs, len, bufferlist { bl }, op_flags);
        tracepoint(librbd, write_exit, r);
        return r;
    }
    ...
    int Image::discard(uint64_t ofs, uint64_t len) {
        ...
        int r =
            ictx->io_work_queue->discard(ofs, len, ictx->skip_partial_discard);
    ...
    ssize_t Image::writesame(uint64_t ofs, size_t len, bufferlist & bl,
        ...
        int r =
            ictx->io_work_queue->writesame(ofs, len, bufferlist { bl },
                                           op_flags);
        tracepoint(librbd, writesame_exit, r);
        return r;
    }
    ...
    ssize_t Image::compare_and_write(uint64_t ofs, size_t len,
        ...
        int r =
            ictx->io_work_queue->compare_and_write(ofs, len,
                                                   bufferlist { cmp_bl },
                                                   bufferlist { bl },
                                                   mismatch_off,
                                                   op_flags);
    ...
    int Image::aio_compare_and_write(uint64_t off, size_t len,
        ...
        ictx->io_work_queue->aio_compare_and_write(get_aio_completion(c), off,
                                                   len, bufferlist {
                                                   cmp_bl}
                                                   , bufferlist {
                                                   bl}
    ...
    int Image::mirror_image_get_status(mirror_image_status_t *
                                       ...
                                       size_t status_size) {
        ImageCtx *ictx = (ImageCtx *) ctx;
        return librbd::api::Mirror <>::image_get_status(ictx,
                                                        mirror_image_status,
                                                        status_size);
    }
    ...
    int Image::aio_mirror_image_get_status(mirror_image_status_t * status,
                                           ...
                                           RBD::AioCompletion * c) {
        ImageCtx *ictx = (ImageCtx *) ctx;
        librbd::api::Mirror <>::image_get_status(ictx, status, status_size,
                                                 new C_AioCompletion(ictx,
                                                                     librbd::
                                                                     io::
                                                                     AIO_TYPE_GENERIC,
...
extern "C" int rbd_trash_move(rados_ioctx_t p, const char *name, uint64_t delay)
{
    ...
    tracepoint(librbd, trash_move_enter, io_ctx.get_pool_name().c_str(),
               io_ctx.get_id(), name);
    int r =
        librbd::trash_move(io_ctx, RBD_TRASH_IMAGE_SOURCE_USER, name, delay);
    tracepoint(librbd, trash_move_exit, r);
    return r;
}
...
extern "C" int rbd_snap_set(rbd_image_t image, const char *snap_name)
{
    ...
    tracepoint(librbd, snap_set_enter, ictx, ictx->name.c_str(),
               ictx->snap_name.c_str(), ictx->read_only, snap_name);
    int r =
        librbd::snap_set(ictx, cls::rbd::UserSnapshotNamespace(), snap_name);
    tracepoint(librbd, snap_set_exit, r);
    return r;
}
                            ...
                            char *buf)
{
    ...
    tracepoint(librbd, read_enter, ictx, ictx->name.c_str(),
               ictx->snap_name.c_str(), ictx->read_only, ofs, len);
    int r =
        ictx->io_work_queue->read(ofs, len, librbd::io::ReadResult { buf, len },
                                  0);
    tracepoint(librbd, read_exit, r);
    return r;
}
                                             ...
                                             int op_flags)
{
    ...
    bufferlist bl;
    bl.push_back(create_write_raw(ictx, buf, len));
    ictx->io_work_queue->aio_compare_and_write(get_aio_completion(comp), off,
                                               len, std::move(cmp_bl),
                                               std::move(bl), mismatch_off,
                                               op_flags, false);
                                             ...
                                             rbd_completion_t c)
{
    ...
    librbd::api::Mirror <>::image_get_info(ictx, &ctx->cpp_mirror_image_info,
                                           sizeof(ctx->cpp_mirror_image_info),
                                           ctx);
    return 0;
}
    ...
    auto ctx =
        new C_MirrorImageGetStatus(status,
                                   new C_AioCompletion(ictx,
                                                       ...
                                                       get_aio_completion
                                                       (comp)));
    librbd::api::Mirror <>::image_get_status(ictx,
                                             &ctx->cpp_mirror_image_status,
                                             sizeof(ctx->
