    ...
    namespace mirror {
        namespace image_map {
                ...
                class DumpVisitor:public boost::static_visitor < void > {
                  public:
                    ...
                    :m_formatter(formatter), m_key(key) {
                        ...
                        inline void operator() (const T & t)const {
                        auto type = T::TYPE;
                         m_formatter->dump_string(m_key.c_str(),
                                                  stringify(type));
                         t.dump(m_formatter);
                  } private:
            ...
            PolicyMetaType PolicyData::get_policy_meta_type() const {
                return boost::apply_visitor(GetTypeVisitor < PolicyMetaType >
                                            (), policy_meta);
            ...
            void PolicyData::dump(Formatter * f) const {
                boost::apply_visitor(DumpVisitor(f, "policy_meta_type"),
                                     policy_meta);
            } void PolicyData::generate_test_instances(std::list <
                                                       PolicyData * >&o) {
