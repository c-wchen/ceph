    ...
    : m_threads(threads),
      ...
      m_lock(ceph::
             make_mutex(librbd::util::
                        unique_lock_name("rbd::mirror::PoolWatcher",
                                         this)))
{
    m_mirroring_watcher =
...
template < typename I > void PoolWatcher < I >::refresh_images()
{
    ...
    auto req =
        pool_watcher::RefreshImagesRequest < I >::create(m_io_ctx,
            &m_refresh_image_ids,
            ctx);
    req->send();
}
...
void PoolWatcher < I >::handle_refresh_images(int r)
{
    ...
    Context *on_init_finish = nullptr;
    {
        ...
        if (m_deferred_refresh) {
            ...
            dout(0) <<
                    "detected client is blocklisted during image refresh" <<
                    dendl;
...
void PoolWatcher < I >::handle_image_updated(const std::string &id,
        ...
        bool enabled)
{
    dout(10) << "image_id=" << id << ", " << "global_image_id=" <<
             global_image_id << ", " << "enabled=" << enabled << dendl;
