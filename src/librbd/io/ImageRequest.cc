         ...
         namespace {
            ...
            template < typename I >
                ...
                SnapshotDelta *snapshot_delta;
                 ...
                 ceph::mutex lock =
                    ceph::
                    make_mutex("librbd::io::C_AssembleSnapshotDeltas::lock",
                               false);
                 std::map < uint64_t, SnapshotDelta > object_snapshot_delta;

                 C_AssembleSnapshotDeltas(I * image_ctx,
                  ...
                  for (auto &[object_no, object_snapshot_delta]:object_snapshot_delta)
                    {
                        ...
                        object_to_image_intervals(object_no,
                    ...
                    ldout(cct,
                          20) << "snapshot_delta=" << *snapshot_delta << dendl;
                    C_AioRequest::finish(0);
                }

                void object_to_image_intervals(uint64_t object_no,
                                               const SnapshotDelta &
                                               object_snapshot_delta,
                                               SnapshotDelta *
                                               image_snapshot_delta,
                      ...
                      for (auto & object_extent:object_extents)
                        {
                            auto[image_extents, _] =
                                io::util::object_to_area_extents(image_ctx,
                                                                 object_no, { {
                                                                 object_extent.
                                                                 get_off(),
                                                                 object_extent.
                            ...
                            auto & assembled_intervals =
                                (*assembled_image_snapshot_delta)[key];
                          for (auto[image_offset, image_length]:image_extents)
                            {
                                SparseExtent sparse_extent {
                                object_extent.get_val().state, image_length};
                                intervals.insert(image_offset, image_length,
                                                 sparse_extent);
                                assembled_intervals.insert(image_offset,
                                                           image_length,
                                                           sparse_extent);
                            }
                        }
            ...
            template < typename I > struct C_RBD_Readahead:public Context {
                ...
                uint64_t object_no;
                 ...
                 C_RBD_Readahead(I * ictx, uint64_t object_no, uint64_t offset,
                                 uint64_t length)
                :ictx(ictx), object_no(object_no), extents( { {
                                                           offset, length}}) {
            ...
            template < typename I >
              ...
              for (auto & image_extent:image_extents) {
                ...
                auto total_bytes_read =
                    ictx->total_bytes_read.fetch_add(total_bytes);
                ...
                auto readahead_extent =
                    ictx->readahead.update(image_extents, data_size);
                ...
                if (readahead_length > 0) {
                    ...
                    io::util::area_to_object_extents(ictx, readahead_offset,
                                                     readahead_length,
                                                     ImageArea::DATA, 0,
                  ...
                  for (auto & object_extent:readahead_object_extents)
                    {
                        ldout(ictx->cct,
                              20) << "(readahead) " << data_object_name(ictx,
                                                                        object_extent.
                                                                        object_no)
                            << " " << object_extent.
                            offset << "~" << object_extent.length << dendl;

                        auto req_comp =
                            new C_RBD_Readahead < I > (ictx,
                                                       object_extent.object_no,
                                                       object_extent.offset,
                                                       object_extent.length);
                        auto req =
                            io::ObjectDispatchSpec::create_read(ictx,
                                                                io::
                    ...
                    ictx->perfcounter->inc(l_librbd_readahead_bytes,
                                           readahead_length);
                }
            }
            ...
            template < typename I > struct C_UpdateTimestamp:public Context {
              public:
                ...
                AsyncOperation m_async_op;
                 ...
                 C_UpdateTimestamp(I & ictx, bool m):m_image_ctx(ictx),
                    m_modify(m) {
            ...
            bool should_update_timestamp(const utime_t & now,
                                         const utime_t & timestamp,
                                         uint64_t interval) {
                return (interval
                        && (static_cast < uint64_t > (now.sec()) >=
                            interval + timestamp));
            }
...
#define dout_prefix *_dout << "librbd::io::ImageRequest: " << this \
        ...
        template < typename I >
            ...
            ImageReadRequest < I > req(*ictx, c, std::move(image_extents), area,
                                       std::move(read_result), io_context,
        ...
        template < typename I >
            ...
            ImageDiscardRequest < I > req(*ictx, c, std::move(image_extents),
                                          area, discard_granularity_bytes,
                                          parent_trace);
            req.send();
        }
        ...
        template < typename I >
            void ImageRequest < I >::aio_compare_and_write(I * ictx,
                                                           AioCompletion * c,
                                                           Extents
                                                           && image_extents,
                                                           ImageArea area,
                                                           ...
                                                           Trace & parent_trace)
        {
            ImageCompareAndWriteRequest < I > req(*ictx, c,
                                                  std::move(image_extents),
                                                  area, std::move(cmp_bl),
                                                  std::move(bl),
        ...
        template < typename I > void ImageRequest < I >::update_timestamp() {
            ...
            utime_t ts = ceph_clock_now();
            {
                ...
                m_image_ctx.timestamp_lock};
                if (!should_update_timestamp
                    (ts, std::invoke(get_timestamp_fn, m_image_ctx),
                     update_interval)) {
                    return;
                }
            }
            ...
            {
                std::unique_lock timestamp_locker {
                m_image_ctx.timestamp_lock};
                bool update =
                    should_update_timestamp(ts,
        ...
        template < typename I >
            ImageReadRequest < I >::ImageReadRequest(I & image_ctx,
                                                     AioCompletion * aio_comp,
                                                     Extents
                                                     && image_extents,
                                                     ImageArea area, ReadResult
                                                     && read_result,
                                                     IOContext io_context,
                                                     int op_flags,
                                                     int read_flags,
      ...
      :    ImageRequest < I > (image_ctx, aio_comp, std::move(image_extents),
                                area, "read", parent_trace),
            m_io_context(io_context), m_op_flags(op_flags),
        ...
        template < typename I > void ImageReadRequest < I >::send_request() {
          ...
          for (auto & extent:image_extents) {
                ...
                util::area_to_object_extents(&image_ctx, extent.first,
                                             extent.second, this->m_image_area,
                                             buffer_ofs, &object_extents);
          ...
          for (auto & oe:object_extents) {
                ldout(cct, 20) << data_object_name(&image_ctx,
                                                   oe.object_no) << " " << oe.
                    offset << "~" << oe.length << " from " << oe.
                    buffer_extents << dendl;

                auto req_comp =
                    new io::ReadResult::C_ObjectReadRequest(aio_comp,
                                                            { {oe.offset,
                                                               oe.length,
                                                               std::move(oe.
                                                                         buffer_extents)}
                ...
                auto req =
                    ObjectDispatchSpec::create_read(&image_ctx,
                                                    OBJECT_DISPATCH_LAYER_NONE,
                                                    oe.object_no,
                                                    &req_comp->extents,
                                                    m_io_context, m_op_flags,
                                                    m_read_flags,
                                                    this->m_trace, nullptr,
                                                    req_comp);
                req->send();
            }
        ...
        template < typename I >
          ...
          for (auto & extent:this->m_image_extents) {
                ...
                io::util::area_to_object_extents(&image_ctx, extent.first,
                                                 extent.second,
        ...
        template < typename I >
            void AbstractImageWriteRequest <
            I >::
            send_object_requests(const LightweightObjectExtents &
                                 object_extents, IOContext io_context,
          ...
          for (auto & oe:object_extents) {
                ...
                auto request =
                    create_object_request(oe, io_context, journal_tid,
        ...
        template < typename I >
            void ImageWriteRequest <
            I >::assemble_extent(const LightweightObjectExtent & object_extent,
                                 bufferlist * bl) {
        ...
        template < typename I >
            uint64_t ImageWriteRequest <
            I >::append_journal_event(bool synchronous) {
            I & image_ctx = this->m_image_ctx;
          ...
          for (auto & extent:this->m_image_extents) {
                ...
                tid =
                    image_ctx.journal->append_write_event(extent.first,
                                                          extent.second, sub_bl,
                                                          synchronous);
            }
        ...
        template < typename I >
            ObjectDispatchSpec * ImageWriteRequest <
            I >::
            create_object_request(const LightweightObjectExtent & object_extent,
            ...
            auto req =
                ObjectDispatchSpec::create_write(&image_ctx,
                                                 OBJECT_DISPATCH_LAYER_NONE,
                                                 object_extent.object_no,
                                                 object_extent.offset,
                                                 std::move(bl), io_context,
                                                 m_op_flags, 0,
                                                 std::nullopt, journal_tid,
        ...
        template < typename I >
            ...
            ceph_assert(!this->m_image_extents.empty());
          for (auto & extent:this->m_image_extents) {
                journal::EventEntry event_entry(journal::
                                                AioDiscardEvent(extent.first,
                                                                extent.second,
                                                                this->
        ...
        template < typename I >
            ObjectDispatchSpec * ImageDiscardRequest <
            I >::
            create_object_request(const LightweightObjectExtent & object_extent,
                                  IOContext io_context, uint64_t journal_tid,
                                  bool single_extent, Context * on_finish) {
            I & image_ctx = this->m_image_ctx;
            auto req =
                ObjectDispatchSpec::create_discard(&image_ctx,
                                                   OBJECT_DISPATCH_LAYER_NONE,
                                                   object_extent.object_no,
                                                   object_extent.offset,
                                                   object_extent.length,
        ...
        template < typename I >
            int ImageDiscardRequest <
            I >::prune_object_extents(LightweightObjectExtents *
                                      object_extents) const {
            if (m_discard_granularity_bytes == 0) {
                return 0;
            }
            // Align the range to discard_granularity_bytes boundary and skip// and discards that are too small to free up any space.//// discard_granularity_bytes >= object_size && tail truncation// is a special case for filestore
            ...
            auto xform_lambda =
                ...
                if ((discard_granularity_bytes < object_size) ||
                    ...
                    offset =
                        p2roundup < uint64_t > (offset,
                                                discard_granularity_bytes);
                    next_offset =
                        p2align < uint64_t > (next_offset,
                                              discard_granularity_bytes);
            ...
            if (prune_required) {
                ...
                object_extents->
                    erase(std::
                          remove_if(object_extents->begin(),
                                    object_extents->end(), remove_lambda),
                          object_extents->end());
            }
        ...
        template < typename I > void ImageFlushRequest < I >::send_request() {
            ...
            if (journaling) {
                ...
                journal_tid =
                    image_ctx.journal->
                    append_io_event(journal::
                                    EventEntry(journal::AioFlushEvent()), 0, 0,
                                    false, 0);
                image_ctx.journal->user_flushed();
            }

            auto object_dispatch_spec =
                ObjectDispatchSpec::create_flush(&image_ctx,
                                                 OBJECT_DISPATCH_LAYER_NONE,
                                                 m_flush_source, journal_tid,
                                                 this->m_trace, ctx);
        ...
        template < typename I >
            uint64_t ImageWriteSameRequest <
            I >::append_journal_event(bool synchronous) {
            I & image_ctx = this->m_image_ctx;
          ...
          for (auto & extent:this->m_image_extents) {
                journal::EventEntry event_entry(journal::
                                                AioWriteSameEvent(extent.first,
                                                                  extent.second,
                                                                  m_data_bl));
                tid =
                    image_ctx.journal->append_io_event(std::move(event_entry),
        ...
        template < typename I >
            ObjectDispatchSpec * ImageWriteSameRequest <
            I >::
            create_object_request(const LightweightObjectExtent & object_extent,
                                  IOContext io_context, uint64_t journal_tid,
                                  bool single_extent, Context * on_finish) {
            ...
            if (util::
                assemble_write_same_extent(object_extent, m_data_bl, &bl,
                                           false)) {
                auto buffer_extents {
                object_extent.buffer_extents};

                req =
                    ObjectDispatchSpec::create_write_same(&image_ctx,
                                                          OBJECT_DISPATCH_LAYER_NONE,
                                                          object_extent.
                                                          object_no,
                                                          object_extent.offset,
        ...
        template < typename I >
            uint64_t ImageCompareAndWriteRequest <
            I >::append_journal_event(bool synchronous) {
            I & image_ctx = this->m_image_ctx;
            ...
            tid =
                image_ctx.journal->append_compare_and_write_event(extent.first,
                                                                  extent.second,
                                                                  m_cmp_bl,
                                                                  m_bl,
                                                                  synchronous);
        ...
        template < typename I >
            void ImageCompareAndWriteRequest <
            I >::assemble_extent(const LightweightObjectExtent & object_extent,
        ...
        template < typename I >
            ObjectDispatchSpec * ImageCompareAndWriteRequest <
            I >::
            create_object_request(const LightweightObjectExtent & object_extent,
                                  IOContext io_context, uint64_t journal_tid,
                                  bool single_extent, Context * on_finish) {
            ...
            auto req =
                ObjectDispatchSpec::create_compare_and_write(&image_ctx,
                                                             OBJECT_DISPATCH_LAYER_NONE,
                                                             object_extent.
                                                             object_no,
                                                             object_extent.
        ...
        template < typename I >
            int ImageCompareAndWriteRequest <
            I >::prune_object_extents(LightweightObjectExtents *
                                      object_extents) const {
            ...
            if (su == 0
                || (object_extent.offset % su + object_extent.length > su))
                return -EINVAL;

             return 0;
        } template < typename I >
            ImageListSnapsRequest < I >::ImageListSnapsRequest(I & image_ctx,
                                                               AioCompletion *
                                                               aio_comp, Extents
                                                               && image_extents,
                                                               ImageArea area,
        ...
        template < typename I > void ImageListSnapsRequest < I >::send_request() {
          ...
          for (auto & image_extent:image_extents) {
                ...
                io::util::area_to_object_extents(&image_ctx, image_extent.first,
                                                 ...
                                                 &object_extents);
              for (auto & object_extent:object_extents) {
                    object_number_extents[object_extent.object_no].
                        emplace_back(object_extent.offset,
                                     object_extent.length);
                }
            }
            ...
            aio_comp->set_request_count(1);
            auto assemble_ctx =
                new C_AssembleSnapshotDeltas < I > (&image_ctx, aio_comp,
                                                    m_snapshot_delta);
            auto sub_aio_comp =
                AioCompletion::create_and_start < Context,
                &Context::complete > (assemble_ctx, get_image_ctx(&image_ctx),
                                      AIO_TYPE_GENERIC);
          ...
          for (auto & oe:object_number_extents) {
                ...
                auto req =
                    ObjectDispatchSpec::create_list_snaps(&image_ctx,
                                                          OBJECT_DISPATCH_LAYER_NONE,
                                                          oe.first,
                                                          std::move(oe.second),
                                                          SnapIds { m_snap_ids }
