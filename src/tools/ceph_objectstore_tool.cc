...
struct action_on_object_t {
    ...
    } virtual int call(ObjectStore * store, coll_t coll, ghobject_t & ghobj,
                       object_info_t & oi) = 0;
};
                                 ...
                                 action_on_object_t & action, bool debug)
{
    ...
    while (!next.is_max()) {
        ...
        int r = store->collection_list(coll,
                                       next,
                                       ghobject_t::get_max(),
                                       LIST_AT_A_TIME,
                                       &list,
...
struct lookup_ghobject:public action_on_object_t {
  ...
  lookup_ghobject(const string & name, const boost::optional < std::string > &nspace, bool need_snapset = false):_name(name),
    ...
    } int call(ObjectStore * store, coll_t coll, ghobject_t & ghobj,
            ...
            PGLog::IndexedLog & log, pg_missing_t & missing)
{
    try {
        ...
        PGLog::read_log_and_missing(fs, coll,
                                    ...
                                    struct_ver >=
                                    8 ? pgid.make_pgmeta_oid() : log_oid, info,
                                    log, missing, struct_ver < 9, oss,
               ...
               PastIntervals & past_intervals)
{
    ...
    int ret = PG::_prepare_write_info(g_ceph_context,
                                      &km, epoch,
                                      info,
                                      last_written_info,
                                      past_intervals,
             ...
             divergent_priors_t & divergent, pg_missing_t & missing)
{
    ...
    if (!divergent.empty()) {
        ...
        PGLog::write_log_and_missing(t, &km, log, coll,
                                     ...
                                     true, &rebuilt_missing_set_with_deletes);
    }
                   ...
                   PastIntervals & past_intervals)
{
    ...
    while (!done) {
        ...
        {
            ObjectMap::ObjectMapIterator p =
                store->get_omap_iterator(coll, oid);
            if (!p)
                break;
                   ...
                   ObjectStore::Sequencer & osr)
{
    ...
    if (ret) {
        cerr << "Failed to set inc-osdmap (" << inc_oid << "): " << ret << std::
            endl;
    }
    else {
               ...
               ObjectStore::Sequencer & osr)
{
    ...
    int ret = store->apply_transaction(&osr, std::move(t));
    if (ret) {
        cerr << "Failed to set osdmap (" << full_oid << "): " << ret << std::
            endl;
    }
    else {
...
{
    bool found =
        store->read(coll_t::meta(), OSD::get_osdmap_pobject_name(e), 0, 0,
                    bl) >= 0;
    if (!found) {
...
int ObjectStoreTool::do_export(ObjectStore * fs, coll_t coll, spg_t pgid,
                               pg_info_t & info, epoch_t map_epoch,
                               __u8 struct_ver,
    ...
    if (hoid.generation == ghobject_t::NO_GEN) {
        if (hoid.hobj.snap < CEPH_MAXSNAP) {
            ...
            if (mi != as.data.end()) {
                ...
                set < snapid_t > oi_snaps(oi.legacy_snaps.begin(),
                                          oi.legacy_snaps.end());
                if (!oi_snaps.empty()) {
                    if (debug)
                        cerr << "\tsetting legacy snaps " << oi_snaps << std::
                            endl;
                ...
                if (mi != as.data.end()) {
                    ...
                    if (!snapset.is_legacy()) {
                      for (auto & p:snapset.
                            ...
                            set < snapid_t > snaps(p.second.begin(),
                                                   p.second.end());
                            if (!store->exists(coll, clone)) {
                                ...
                                if (debug)
                                    cerr << "\tskipping missing " << clone <<
                                        " (snaps " << snaps << ")" << std::endl;
                                continue;
                            }
                            if (debug)
                                cerr << "\tsetting " << clone.
                                    hobj << " snaps " << snaps << std::endl;
                            OSDriver::OSTransaction _t(driver.
                                                       get_transaction(t));
                            assert(!snaps.empty());
                    ...
                    const OSDSuperblock & sb, OSDMap & curmap, spg_t pgid)
{
    ...
    if (pgid.pgid.ps() >= new_pg_num) {
        cerr << "Illegal pgid, the seed is larger than current pg_num" << std::
            endl;
        return -EINVAL;
    }
    ...
    if (old_pg_num) {
        ...
        if (pgid.pgid.ps() >= old_pg_num) {
            ...
            if (parent.is_split(old_pg_num, new_pg_num, NULL)) {
                cerr << "WARNING: Split occurred, some objects may be ignored"
                    << std::endl;
            }
        }
    }
    ...
    if (debug) {
        ...
        cerr << "history.same_interval_since " << ms.info.history.
            same_interval_since << std::endl;
    }
    ...
    if (debug)
        cerr << "Changing pg epoch " << ms.map_epoch << " to " << sb.
...
int ObjectStoreTool::dump_import(Formatter * formatter)
{
    ...
    while (!done) {
        ...
        if (debug) {
            cerr << "dump_import: Section type " << std::
                               ...
                               ObjectStore::Sequencer & osr)
{
    ...
    if (pgidstr.length()) {
        ...
        if (pgid != user_pgid) {
            if (pgid.pool() != user_pgid.pool()) {
                cerr << "Can't specify a different pgid pool, must be " << pgid.
                    pool() << std::endl;
                return -EINVAL;
            }
            if (pgid.is_no_shard() && !user_pgid.is_no_shard()) {
                cerr << "Can't specify a sharded pgid with a non-sharded export"
                    << std::endl;
                return -EINVAL;
            }
            ...
            if (pgid.shard != user_pgid.shard) {
                cerr << "Can't specify a different shard, must be " << pgid.
                    shard << std::endl;
                return -EINVAL;
            }
            pgid = user_pgid;
    ...
    if (!dry_run) {
        ObjectStore::Transaction t;
        PG::_create(t, pgid,
                    pgid.get_split_bits(curmap.get_pg_pool(pgid.pool())->
                                        get_pg_num()));
    ...
    if (!dry_run) {
        ...
        pg_log_t::filter_log(pgid, curmap,
        ...
        filter_divergent_priors(pgid, curmap,
                                g_ceph_context->_conf->osd_hit_set_namespace,
        ...
        ms.missing.filter_objects([&](const hobject_t & obj) {
                                  ...
                                  return false; assert(!obj.is_temp());
                                  object_t oid = obj.oid;
                                  object_locator_t loc(obj);
                                  pg_t raw_pgid =
                                  curmap.object_locator_to_pg(oid, loc);
...
int do_set_bytes(ObjectStore * store, coll_t coll,
                 ghobject_t & ghobj, int fd, ObjectStore::Sequencer & osr)
{
    ObjectStore::Transaction tran;
    ObjectStore::Transaction * t = &tran;
...
int do_rm_attr(ObjectStore * store, coll_t coll,
               ghobject_t & ghobj, string key, ObjectStore::Sequencer & osr)
{
    ObjectStore::Transaction tran;
    ObjectStore::Transaction * t = &tran;
...
struct do_fix_lost:public action_on_object_t {
    ...
    explicit do_fix_lost(ObjectStore::Sequencer * _osr):osr(_osr) {
        ...
        if (oi.is_lost()) {
            ...
            int r = store->apply_transaction(osr, std::move(t));
            if (r < 0) {
                cerr << "Error getting fixing attr on : " << make_pair(coll,
                                                                       ghobj)
                    << ", " << cpp_strerror(r) << std::endl;
                return r;
            }
...
int corrupt_info(ObjectStore * store, coll_t coll, ghobject_t & ghobj,
                 Formatter * formatter, ObjectStore::Sequencer & osr)
{
    bufferlist attr;
...
int set_size(ObjectStore * store, coll_t coll, ghobject_t & ghobj,
             uint64_t setsize, Formatter * formatter,
...
int remove_clone(ObjectStore * store, coll_t coll, ghobject_t & ghobj,
                 snapid_t cloneid, bool force, ObjectStore::Sequencer & osr)
{
...
int dup(string srcpath, ObjectStore * src, string dstpath, ObjectStore * dst)
{
  ...
  for (auto cid:collections) {
        ...
        while (true) {
            ...
            r = src->collection_list(cid, pos, ghobject_t::get_max(), 1000, &ls,
                                     &pos);
            if (r < 0) {
...
int apply_layout_settings(ObjectStore * os, const OSDSuperblock & superblock,
                          const string & pool_name, const spg_t & pgid,
                          bool dry_run, int target_level)
{
    ...
    if (pool_name.length()) {
        ...
        if (poolid < 0) {
            cerr << "Couldn't find pool " << pool_name << ": " <<
                cpp_strerror(poolid)
                << std::endl;
            return poolid;
        }
  ...
  for (auto const &coll:filtered_colls) {
        if (dry_run) {
            ...
            if (r < 0) {
                cerr << "Error applying layout settings to " << coll << std::
                    endl;
                return r;
            }
        }
    ...
    cerr << "Finished " << total << "/" << total << " collections" << "\r" <<
        std::endl;
    return r;
}
...
int main(int argc, char **argv)
{
    ...
    if (!vm.count("data-path") &&
        op != "dump-import" && !(op == "dump-journal" && type == "filestore")) {
    ...
    if (op != "list" && op != "apply-layout-settings" &&
        ...
        cerr << "Can't specify both --op and object command syntax" << std::
            endl;
        usage(desc);
        return 1;
    }
    ...
    if (op != "list" && op != "apply-layout-settings" && vm.count("object")
        && !vm.count("objcmd")) {
    ...
    else if (op == "import" || op == "dump-import" || op == "set-osdmap"
             || op == "set-inc-osdmap") {
        if (!vm.count("file") || file == "-") {
    ...
    auto cct = global_init(NULL, ceph_options, CEPH_ENTITY_TYPE_OSD,
    ...
    if (op == "dump-import") {
        ...
        if (ret < 0) {
            cerr << "dump-import: " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        return 0;
    }
    ...
    if (op == "dup") {
        ...
        ::close(fd);
        ObjectStore *targetfs = ObjectStore::create(g_ceph_context, target_type,
                                                    target_data_path, "", 0);
        if (targetfs == NULL) {
    ...
    if (ret < 0) {
        cerr << "Failure to read OSD superblock: " << cpp_strerror(ret) << std::
            endl;
        goto out;
    }
    ...
    if (supported.compare(superblock.compat_features) == -1) {
        ...
        cerr << "On-disk OSD incompatible features set " << unsupported << std::
            endl;
        ret = -EINVAL;
        goto out;
    }
    ...
    if (op == "apply-layout-settings") {
        ...
        ret =
            apply_layout_settings(fs, superblock, pool, pgid, dry_run,
                                  target_level);
        goto out;
    }
        ...
        if (object == "") {
            ...
            try {
                if (!json_spirit::read(object, v) ||
                    ...
                    if (pgidstr.length())
                        ret =
                            action_on_all_objects_in_exact_pg(fs, coll_t(pgid),
                                                              lookup, debug);
                    else
    ...
    if (op == "fix-lost") {
        ...
        if (pgidstr.length())
            ret =
                action_on_all_objects_in_exact_pg(fs, coll_t(pgid), *action,
                                                  debug);
        else
    ...
    if (!vm.count("objcmd") && op != "export" && op != "export-remove"
        && op != "info" && op != "log" && op != "rm-past-intervals"
        && op != "mark-complete" && op != "trim-pg-log") {
        cerr <<
...
// The following code for export, info, log require omap or !skip-mount-omap
    if (it != ls.end()) {
        ...
        if (op == "export" || op == "export-remove") {
            ret =
                tool.do_export(fs, coll, pgid, info, map_epoch, struct_ver,
            ...
            if (struct_ver < PG::compat_struct_v) {
                cerr << "Can't remove past-intervals, version mismatch " <<
                    (int)struct_ver << " (pg)  < compat " << (int)PG::
                    compat_struct_v << " (tool)" << std::endl;
                ret = -EFAULT;
                goto out;
            }
