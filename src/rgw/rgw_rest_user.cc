...
void RGWOp_Quota_Set::execute()
{
    ...
    if (http_ret < 0) {
        ldout(store->ctx(),
              20) << "failed initializing user info: " << http_ret << dendl;
        return;
    }
...
#define QUOTA_INPUT_MAX_LEN 1024
    if (set_all) {
        ...
        if ((http_ret =
             rgw_rest_get_json_input(store->ctx(), s, quotas,
                                     QUOTA_INPUT_MAX_LEN, NULL)) < 0) {
        ...
        if (!use_http_params) {
            bool empty;
            http_ret =
                rgw_rest_get_json_input(store->ctx(), s, quota,
                                        QUOTA_INPUT_MAX_LEN, &empty);
            if (http_ret < 0) {
                ldout(store->ctx(), 20) << "failed to retrieve input" << dendl;
                if (!empty)
                    return;
        ...
        if (use_http_params) {
            ...
            if (http_ret < 0) {
                ldout(store->ctx(),
                      20) << "failed to get user info: " << http_ret << dendl;
                return;
            }
    ...
    if (http_ret < 0) {
        ldout(store->ctx(),
              20) << "failed updating user info: " << http_ret << ": " << err <<
            dendl;
        return;
    }
}
