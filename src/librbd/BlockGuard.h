 ...
 */
   ...
   */
        int detain(const BlockExtent & block_extent,
                   BlockOperation * block_operation, BlockGuardCell ** cell) {
            Mutex::Locker locker(m_lock);
            ldout(m_cct,
                  20) << "block_start=" << block_extent.
                block_start << ", " << "block_end=" << block_extent.
                block_end << ", " << "free_slots=" <<
            ...
            if (it != m_detained_block_extents.end()) {
                ...
                if (block_operation != nullptr) {
                    detained_block_extent->block_operations.
                        emplace_back(std::move(*block_operation));
                }
                ...
                *cell = nullptr;
                ...
                if (!m_free_detained_block_extents.empty()) {
                    detained_block_extent =
                        &m_free_detained_block_extents.front();
                    ...
                    detained_block_extent =
                        &m_detained_block_extent_pool.back();
                }
                ...
                *cell =
                    reinterpret_cast <
                    BlockGuardCell * >(detained_block_extent);
                return 0;
            }
        }
   ...
   */
        void release(BlockGuardCell * cell, BlockOperations * block_operations) {
            ...
            assert(cell != nullptr);
            auto & detained_block_extent =
                reinterpret_cast < DetainedBlockExtent & >(*cell);
            ldout(m_cct,
                  20) << "block_start=" << detained_block_extent.block_extent.
                block_start << ", " << "block_end=" << detained_block_extent.
                block_extent.
                block_end << ", " << "pending_ops=" << (detained_block_extent.
                                                        block_operations.
                                                        empty()? 0 :
                                                        detained_block_extent.
                                                        block_operations.
            ...
            *block_operations =
                std::move(detained_block_extent.block_operations);
      ...
      private:
        ...
        typedef boost::intrusive::list < DetainedBlockExtent >
            DetainedBlockExtents;
        typedef boost::intrusive::set < DetainedBlockExtent,
