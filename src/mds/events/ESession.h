...
public:
    ESession(): LogEvent(EVENT_SESSION), open(false)
    {
               ...
               const client_metadata_t &cm): LogEvent(EVENT_SESSION),
    ...
    ESession(const entity_inst_t &inst, bool o, version_t v,
             ...
             const interval_set < inodeno_t >
             &to_purge): LogEvent(EVENT_SESSION), client_inst(inst), open(o),
        cmapv(v), inos_to_free(to_free), inotablev(iv),
