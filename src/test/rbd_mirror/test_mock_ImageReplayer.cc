...
template <> struct MirrorStatusUpdater <librbd::MockTestImageCtx > {
    ...
    MOCK_METHOD2(remove_refresh_mirror_image_status,
                 void (const std::string &, Context *));
    MOCK_METHOD3(remove_mirror_image_status,
                 void (const std::string &, bool, Context *));
};
...
template <> struct BootstrapRequest <librbd::MockTestImageCtx > {
    ...
    static BootstrapRequest *create(Threads <
...
struct MockReplayer: public Replayer {
    ...
    MOCK_METHOD2(get_replay_status,
                 bool(std::string *, Context *));
...
template <> struct StateBuilder <librbd::MockTestImageCtx > {
    ...
    MOCK_METHOD5(create_replayer,
                 Replayer * (Threads < librbd::MockTestImageCtx > *,
                             InstanceWatcher <
                             librbd::MockTestImageCtx > *,
...
public:
    ...
    typedef image_replayer::BootstrapRequest <
    librbd::MockTestImageCtx > MockBootstrapRequest;
    typedef image_replayer::StateBuilder < librbd::MockTestImageCtx >
    MockStateBuilder;
                     ...
                     bool do_resync, bool set_local_image, int r)
    {
        ...
        set_local_image, do_resync, r]() {
            if (r == 0 || r == -ENOLINK) {
                ...
                *mock_bootstrap_request.state_builder =
                    &mock_state_builder;
            }
            if (r == 0) {
                mock_state_builder.local_image_ctx =
                                ...
                                MockReplayer &mock_replayer)
    {
        ...
        .WillOnce(WithArg < 4 > (Invoke([&mock_replayer]
                                        (image_replayer::
                                         ReplayerListener *
        replayer_listener) {
            mock_replayer.
    ...
    {
        EXPECT_CALL(m_local_status_updater,
                    set_mirror_image_status(_, _, _))
        .WillRepeatedly(Invoke([](auto, auto, auto) {
        }));
        EXPECT_CALL(m_remote_status_updater,
                    set_mirror_image_status(_, _, _))
        .WillRepeatedly(Invoke([](auto, auto, auto) {
        }));
    }
    ...
    {
        m_image_replayer =
            new MockImageReplayer(m_local_io_ctx, "local_mirror_uuid",
                                  "global image id", &mock_threads,
                                  &m_instance_watcher,
                                  &m_local_status_updater, nullptr,
                                  nullptr);
        m_image_replayer->add_peer({
            "peer_uuid", m_remote_io_ctx, {
                "remote mirror uuid",
...
TEST_F(TestMockImageReplayer, StartStop)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder,
                mock_local_image_ctx, false, false, 0);
...
TEST_F(TestMockImageReplayer, LocalImagePrimary)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder,
                mock_local_image_ctx, false, false, -ENOMSG);
...
TEST_F(TestMockImageReplayer, MetadataCleanup)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder,
                mock_local_image_ctx, false, true, -ENOLINK);
...
TEST_F(TestMockImageReplayer, BootstrapRemoteDeleted)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder,
                mock_local_image_ctx, false, false, -ENOLINK);
...
TEST_F(TestMockImageReplayer, BootstrapResyncRequested)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder,
                mock_local_image_ctx, true, false, 0);
...
TEST_F(TestMockImageReplayer, BootstrapError)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder,
                mock_local_image_ctx, false, false, -EINVAL);
...
TEST_F(TestMockImageReplayer, BootstrapCancel)
{
    ...
    .WillOnce(Invoke([this, &mock_bootstrap_request]() {
        ...
        mock_bootstrap_request.on_finish->
        complete(-ECANCELED);
    }
                    ));
