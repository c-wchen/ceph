...
namespace librados {
    namespace {
        ...
        struct CB_notify_Finish {
            ...
            CB_notify_Finish(CephContext * _cct, Context * _ctx,
                             Objecter * _objecter,
                             Objecter::LingerOp * _linger_op,
                             bufferlist * _preply_bl, char **_preply_buf,
            ...
            // move-only CB_notify_Finish(const CB_notify_Finish &) = delete;
                ...
                if (preply_buf) {
                    if (reply_bl.length()) {
                        *preply_buf = (char *)malloc(reply_bl.length());
                        memcpy(*preply_buf, reply_bl.c_str(),
                               reply_bl.length());
                    }
                    else {
                        *preply_buf = NULL;
        ...
        struct C_aio_linger_Complete:public Context {
            ...
            C_aio_linger_Complete(AioCompletionImpl * _c,
                                  Objecter::LingerOp * _linger_op, bool _cancel)
        ...
        struct C_aio_notify_Ack:public Context {
            ...
            :cct(_cct), oncomplete(_oncomplete) {
                ...
                ldout(cct,
                      10) << __func__ << " linger op " << oncomplete->
                    linger_op << " " << "acked (" << r << ")" << dendl;
        ...
        struct C_aio_selfmanaged_snap_create_Complete:public
            C_aio_selfmanaged_snap_op_Complete {
            snapid_t snapid;
            uint64_t *dest_snapid;

            C_aio_selfmanaged_snap_create_Complete(librados::RadosClient *
                                                   client,
                                                   librados::AioCompletionImpl *
                                                   c, uint64_t * dest_snapid)
...
oloc(poolid), aio_write_seq(0), objecter(objecter)
{
}
...
void librados::IoCtxImpl::set_snap_read(snapid_t s)
{
    if (!s)
        s = CEPH_NOSNAP;
    ldout(client->cct,
          10) << "set snap read " << snap_seq << " -> " << s << dendl;
    snap_seq = s;
}

int librados::IoCtxImpl::set_snap_write_context(snapid_t seq,
    ...
    while (waiters != aio_write_waiters.end()) {
        if (!aio_write_list.empty() &&
            ...
            ldout(client->cct,
                ...
                front()->aio_write_seq << " <= waiter " << waiters->
                first << ", stopping" << dendl;
            break;
        }
        ldout(client->cct,
              20) << " waking waiters on seq " << waiters->first << dendl;
        for (std::list < AioCompletionImpl * >::iterator it =
             waiters->second.begin(); it != waiters->second.end(); ++it) {
            boost::asio::defer(client->finish_strand,
...
void librados::IoCtxImpl::flush_aio_writes_async(AioCompletionImpl * c)
{
    ...
    if (aio_write_list.empty()) {
        ...
        ldout(client->cct,
              20) << "flush_aio_writes_async " << aio_write_list.size()
...
selfmanaged_snap_rollback_object(const object_t & oid,::SnapContext & snapc,
                                 uint64_t snapid)
{
    int reply;
...
int librados::IoCtxImpl::nlist(Objecter::NListContext * context,
                               int max_entries)
{
    bool done;
    int r = 0;
...
rados_object_list_cursor librados::IoCtxImpl::nlist_get_cursor(Objecter::
                                                               NListContext *
                                                               context)
{
 ...
 * methods.
 */
::ObjectOperation * librados::IoCtxImpl::prepare_assert_ops(::ObjectOperation *
                                                            op)
{
...
int librados::IoCtxImpl::append(const object_t & oid, bufferlist & bl,
                                size_t len)
{
    if (len > UINT_MAX / 2)
        return -E2BIG;
...
int librados::IoCtxImpl::operate_read(const object_t & oid,::ObjectOperation *
                                      o, bufferlist * pbl, int flags)
{
    if (!o->size())
        return 0;
    ...
    int op = o->ops[0].op.op;
    ldout(client->cct,
          10) << ceph_osd_op_name(op) << " oid=" << oid << " nspace=" << oloc.
        nspace << dendl;
    Objecter::Op * objecter_op =
        objecter->prepare_read_op(oid, oloc, *o, snap_seq, pbl,
                                  flags | extra_op_flags, onack, &ver);
    objecter->op_submit(objecter_op);
                                          ...
                                          const blkin_trace_info * trace_info)
{
    ...
    if (trace_info) {
        ...
        trace.init("rados operate read", &objecter->trace_endpoint,
                   &parent_trace);
    }
...
int librados::IoCtxImpl::aio_operate(const object_t & oid,::ObjectOperation * o,
                                     AioCompletionImpl * c,
                                     ...
                                     const blkin_trace_info * trace_info)
{
    ...
    trace.event("init root span");
    Objecter::Op * op =
        objecter->prepare_mutate_op(oid, oloc, *o, snap_context, ut,
                                    flags | extra_op_flags, oncomplete,
                                  ...
                                  const blkin_trace_info * info)
{
    ...
    Objecter::Op * o = objecter->prepare_read_op(oid, oloc,
                                                 off, len, snapid, pbl,
                                                 extra_op_flags, oncomplete,
                                                 &c->objver, nullptr, 0,
                                                 &trace);
                                         ...
                                         uint64_t off, uint64_t snapid)
{
    ...
    Objecter::Op * o = objecter->prepare_read_op(oid, oloc,
                                                 onack->m_ops, snapid, NULL,
                                                 extra_op_flags, onack,
                                                 &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                    ...
                                    uint64_t off, bufferlist & cmp_bl)
{
    ...
    Objecter::Op * o =
        objecter->prepare_cmpext_op(oid, oloc, off, cmp_bl, snap_seq,
                                    extra_op_flags, onack, &c->objver);
    objecter->op_submit(o, &c->tid);
                                    ...
                                    size_t cmp_len, uint64_t off)
{
    ...
    Objecter::Op * o =
        objecter->prepare_read_op(oid, oloc, onack->m_ops, snap_seq, NULL,
                                  extra_op_flags, onack, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                   ...
                                   uint64_t off, const blkin_trace_info * info)
{
    ...
    ldout(client->cct,
          20) << "aio_write " << oid << " " << off << "~" << len << " snapc=" <<
        snapc << " snap_seq=" << snap_seq << dendl;
    OID_EVENT_TRACE(oid.name.c_str(), "RADOS_WRITE_OP_BEGIN");
    ...
    Objecter::Op * o = objecter->prepare_write_op(oid, oloc,
                                                  off, len, snapc, bl, ut,
                                                  extra_op_flags, oncomplete,
                                                  &c->objver, nullptr, 0,
                                                  &trace);
                                    ...
                                    const bufferlist & bl, size_t len)
{
    ...
    Objecter::Op * o = objecter->prepare_append_op(oid, oloc,
                                                   len, snapc, bl, ut,
                                                   extra_op_flags, oncomplete,
                                                   &c->objver);
    objecter->op_submit(o, &c->tid);
                                        ...
                                        const bufferlist & bl)
{
    ...
    Objecter::Op * o = objecter->prepare_write_full_op(oid, oloc,
                                                       snapc, bl, ut,
                                                       extra_op_flags,
                                                       oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
                                       ...
                                       size_t write_len, uint64_t off)
{
    ...
    Objecter::Op * o = objecter->prepare_writesame_op(oid, oloc,
                                                      write_len, off,
                                                      snapc, bl, ut,
                                                      extra_op_flags,
                                                      oncomplete, &c->objver);
                                    ...
                                    int flags)
{
    ...
    Objecter::Op * o = objecter->prepare_remove_op(oid, oloc,
                                                   snapc, ut,
                                  ...
                                  uint64_t * psize, time_t * pmtime)
{
    ...
    c->is_read = true;
    c->io = this;
    Objecter::Op * o = objecter->prepare_stat_op(oid, oloc,
                                                 snap_seq, psize, &onack->mtime,
                                                 extra_op_flags, onack,
                                                 &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
...
int librados::IoCtxImpl::aio_getxattr(const object_t & oid,
                                      AioCompletionImpl * c, const char *name,
                                      bufferlist & bl)
{
    ::ObjectOperation rd;
...
int librados::IoCtxImpl::aio_rmxattr(const object_t & oid,
                                     AioCompletionImpl * c, const char *name)
{
    ::ObjectOperation op;
    prepare_assert_ops(&op);
...
int librados::IoCtxImpl::aio_setxattr(const object_t & oid,
                                      AioCompletionImpl * c, const char *name,
                                      bufferlist & bl)
{
    ::ObjectOperation op;
...
namespace {
    struct AioGetxattrsData {
        AioGetxattrsData(librados::AioCompletionImpl * c, map < string,
                         bufferlist > *attrset,
                         librados::RadosClient * _client):user_completion(c),
            user_attrset(attrset), client(_client) {
...
static void aio_getxattrs_complete(rados_completion_t c, void *arg)
{
    ...
    if (rc >= 0) {
        for (map < string, bufferlist >::iterator p =
             cdata->result_attrset.begin(); p != cdata->result_attrset.end();
             ++p) {
            ldout(cdata->client->cct,
                  10) << "IoCtxImpl::getxattrs: xattr=" << p->first << dendl;
...
int librados::IoCtxImpl::aio_getxattrs(const object_t & oid,
                                       AioCompletionImpl * c, map < std::string,
                                       bufferlist > &attrset)
{
                                      ...
                                      time_t > >*pls)
{
    ...
    Objecter::Op * o =
        objecter->prepare_pg_read_op(hash, oloc, rd, NULL, extra_op_flags,
...
int librados::IoCtxImpl::get_inconsistent_objects(const pg_t & pg,
                                                  const librados::
                                                  object_id_t & start_after,
                                                  uint64_t max_to_get,
                                                  AioCompletionImpl * c,
                                                  std::vector <
                                                  inconsistent_obj_t > *objects,
                                                  uint32_t * interval)
{
    Context *oncomplete = new C_aio_Complete(c);
    c->is_read = true;
    ...
    poolid, pg.ps()};
    Objecter::Op * o =
        objecter->prepare_pg_read_op(oloc.hash, oloc, op, nullptr,
                                     CEPH_OSD_FLAG_PGOP | extra_op_flags,
                                  ...
                                  bufferlist & inbl, bufferlist * outbl)
{
    ...
    rd.call(cls, method, inbl);
    Objecter::Op * o =
        objecter->prepare_read_op(oid, oloc, rd, snap_seq, outbl,
                                  ...
                                  bufferlist & inbl, char *buf, size_t out_len)
{
    ...
    rd.call(cls, method, inbl);
    Objecter::Op * o =
        objecter->prepare_read_op(oid, oloc, rd, snap_seq, &c->bl,
                                  extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
...
int librados::IoCtxImpl::stat(const object_t & oid, uint64_t * psize,
                              time_t * pmtime)
{
    uint64_t size;
    real_time mtime;
...
int librados::IoCtxImpl::stat2(const object_t & oid, uint64_t * psize,
                               struct timespec *pts)
{
    uint64_t size;
...
struct WatchInfo {
    ...
    :ioctx(io), oid(o), ctx(c), ctx2(c2) {
        ...
        ldout(ioctx->client->cct, 10) << __func__ << " " << notify_id
            << " cookie " << cookie
            << " notifier_id " << notifier_id << " len " << bl.length()
            << dendl;
...
int librados::IoCtxImpl::watch(const object_t & oid, uint64_t * handle,
                               librados::WatchCtx * ctx,
                               librados::WatchCtx2 * ctx2,
                               uint32_t timeout, bool internal)
{
    ::ObjectOperation wr;
    version_t objver;
...
int librados::IoCtxImpl::notify_ack(const object_t & oid,
                                    uint64_t notify_id,
                                    uint64_t cookie, bufferlist & bl)
{
    ::ObjectOperation rd;
    prepare_assert_ops(&rd);
    rd.notify_ack(notify_id, cookie, bl);
    objecter->read(oid, oloc, rd, snap_seq, (bufferlist *) NULL, extra_op_flags,
                   0, 0);
    return 0;
}
                                ...
                                char **preply_buf, size_t * preply_buf_len)
{
    ...
    linger_op->on_notify_finish =
        Objecter::LingerOp::OpComp::create(objecter->service.get_executor(),
                                           CB_notify_Finish(client->cct,
                                                            &notify_finish_cond,
                                                            objecter, linger_op,
                                                            preply_bl,
                                                            preply_buf,
    ...
    objecter->linger_notify(linger_op,
                            rd, snap_seq, inbl, NULL, &onack, &objver);

    ldout(client->cct,
          10) << __func__ << " issued linger op " << linger_op << dendl;
    int r = onack.wait();
    ...
    if (r == 0) {
        ldout(client->cct,
              10) << __func__ << " waiting for watch_notify finish " <<
            linger_op << dendl;
                                    ...
                                    size_t * preply_buf_len)
{
    ...
    // Issue RADOS op
    objecter->linger_notify(linger_op,
                            rd, snap_seq, inbl, NULL, onack, &c->objver);
    return 0;
}
                                            ...
                                            hobject_t * split_finish)
{
    ...
    if (n == 0) {
        ...
        *split_start = hobject_t(object_t(), string(), CEPH_NOSNAP,
                                 hobject_t::_reverse_bits(rev_start), poolid,
                                 string());
    }
    ...
    if (n == m - 1)
        ...
        *split_finish = hobject_t(object_t(), string(), CEPH_NOSNAP,
                                  hobject_t::_reverse_bits(rev_finish), poolid,
                                  string());
}
                                                   ...
                                                   PoolAsyncCompletionImpl * c)
{
    ...
    if (!client->get_required_monitor_features().
                                                  ...
                                                  std::string * value)
{
    ...
    objecter->with_osdmap([&](const OSDMap & o) {
                          ...
                          auto app_it =
                          pg_pool->application_metadata.find(app_name);
...
int librados::IoCtxImpl::application_metadata_remove(const std::
                                                     string & app_name,
                                                     const std::string & key)
{
    std::stringstream cmd;
