...
void RGWRoleInfo::decode_json(JSONObj *obj)
{
    ...
    JSONDecoder::decode_json("MaxSessionDuration", max_session_duration,
                             obj);
    JSONDecoder::decode_json("AssumeRolePolicyDocument", trust_policy,
                             obj);
...
bool RGWRole::validate_max_session_duration(const DoutPrefixProvider *
        dpp)
{
    if (info.max_session_duration < SESSION_DURATION_MIN
        ...
        ldpp_dout(dpp,
                  0) <<
                     "ERROR: Invalid session duration, should be between 3600 and 43200 seconds "
                     << dendl;
        return false;
    }
    return true;
...
void RGWRole::set_perm_policy(const string &policy_name,
                              const string &perm_policy)
{
...
int RGWRole::get_role_policy(const DoutPrefixProvider *dpp,
...
int RGWRole::delete_policy(const DoutPrefixProvider *dpp,
                           const string &policy_name)
{
    const auto &it = info.perm_policy_map.find(policy_name);
    if (it == info.perm_policy_map.end()) {
...
int RGWRole::set_tags(const DoutPrefixProvider *dpp,
...
void RGWRole::update_max_session_duration(const std::
        string &
                                   ...
                                   const DoutPrefixProvider *dpp)
{
    ...
    RGWRoleMetadataObject *rdo =
        new RGWRoleMetadataObject(info, objv_tracker.read_version,
                                  mtime, driver);
    *obj = rdo;
...
int RGWRoleMetadataHandler::do_remove(RGWSI_MetaBackend_Handler::Op *
                                      op, std::string &entry,
...
public:
    RGWMetadataHandlerPut_Role(RGWRoleMetadataHandler *handler,
        ...
        (handler, op, entry, obj, objv_tracker, y, type,
         from_remote_zone), rhandler(handler)
    {
...
{
    RGWMetadataHandlerPut_Role put_op(this, op, entry, obj,
                                      objv_tracker, y, type,
                                      from_remote_zone);
