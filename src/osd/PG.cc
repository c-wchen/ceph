...
void PGStateHistory::exit(const char *state)
{
    ...
    if (::strstr(state, "Trimming") != NULL || pg_in_destructor) {
        ...
        if (pi == nullptr) {
            buffer.push_back(std::unique_ptr < PGStateInstance >
                             (tmppi.release()));
            pi = buffer.back().get();
...
uint64_t PG::get_with_id()
{
    ...
    stringstream ss;
    bt.print(ss);
    dout(20) << __func__ << ": " << info.
        pgid << " got id " << id << " (new) ref==" << ref << dendl;
...
{
    dout(20) << __func__ << ": " << info.
        pgid << " put id " << id << " (current) ref==" << ref << dendl;
    {
        Mutex::Locker l(_ref_id_lock);
        assert(_live_ids.count(id));
        _live_ids.erase(id);
...
void PGPool::update(OSDMapRef map)
{
        ...
        if (pi->maybe_updated_removed_snaps(cached_removed_snaps)) {
            ...
            if (cached_removed_snaps.subset_of(newly_removed_snaps)) {
                ...
                lgeneric_subdout(cct, osd, 0) << __func__
                    << " cached_removed_snaps shrank from " <<
                    cached_removed_snaps << " to " << newly_removed_snaps <<
                    dendl;
    ...
    lgeneric_subdout(cct, osd, 20)
        ...
        << " newly_removed_snaps "
        << newly_removed_snaps
        << " snapc " << snapc << (updated ? " (updated)" : " (no change)")
        << dendl;
}

PG::PG(OSDService * o, OSDMapRef curmap,
       const PGPool & _pool, spg_t p):osd(o),
cct(o->cct),
...
missing_loc(this),
past_intervals(curmap->get_pools().at(p.pgid.pool()).ec_pool(),
               *curmap),
...
upacting_features(CEPH_FEATURES_SUPPORTED_DEFAULT), last_epoch(0)
{
#ifdef PG_DEBUG_REFS
    osd->add_pgid(p, this);
#endif
...
void PG::lock(bool no_lockdep) const const
{
...
std::string PG::gen_prefix() constconst
{
    stringstream out;
...
void PG::proc_master_log(ObjectStore::Transaction & t, pg_info_t & oinfo,
                         ...
                         pg_shard_t from)
{
    ...
    dout(10) << " peer osd." << from << " now " << oinfo << " " << omissing <<
        dendl;
...
void PG::proc_replica_log(pg_info_t & oinfo,
                          const pg_log_t & olog,
                          pg_missing_t & omissing, pg_shard_t from)
{
    ...
    for (map < hobject_t, pg_missing_item >::const_iterator i =
         omissing.get_items().begin(); i != omissing.get_items().end(); ++i) {
                           ...
                           epoch_t send_epoch)
{
    ...
    if (p != peer_info.end() && p->second.last_update == oinfo.last_update) {
        dout(10) << " got dup osd." << from << " info " << oinfo <<
                                   ...
                                   const hobject_t & soid)
{
    ...
    if (soid.snap < CEPH_MAXSNAP) {
        int r = snap_mapper.remove_oid(soid,
                                       &_t);
        if (!(r == 0 || r == -ENOENT)) {
...
void PG::merge_log(ObjectStore::Transaction & t, pg_info_t & oinfo,
                   pg_log_t & olog, pg_shard_t from)
{
    PGLogEntryHandler rollbacker {
    this, &t};
    pg_log.merge_log(oinfo, olog, from, info, &rollbacker, dirty_info,
                     dirty_big_info);
}
...
void PG::rewind_divergent_log(ObjectStore::Transaction & t, eversion_t newhead)
{
    PGLogEntryHandler rollbacker {
    this, &t};
    pg_log.rewind_divergent_log(newhead, info, &rollbacker, dirty_info,
                                dirty_big_info);
}
 ...
 */
bool PG::search_for_missing(const pg_info_t & oinfo,
                            const pg_missing_t & omissing, pg_shard_t from,
                            RecoveryCtx * ctx)
{
    ...
    if (found_missing &&
        (get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, NULL) &
         CEPH_FEATURE_OSD_ERASURE_CODES) && oinfo.last_update != eversion_t()) {
        pg_info_t tinfo(oinfo);
        tinfo.pgid.shard = pg_whoami.shard;
        (*(ctx->info_map))[from.osd].
            push_back(make_pair
                      (pg_notify_t
                       (from.shard, pg_whoami.shard, get_osdmap()->get_epoch(),
                        get_osdmap()->get_epoch(), tinfo), past_intervals));
    }
...
bool PG::MissingLoc::readable_with_acting(const hobject_t & hoid,
...
void PG::MissingLoc::add_batch_sources_info(const set < pg_shard_t > &sources,
                                            ThreadPool::TPHandle * handle)
{
    ...
    unsigned loop = 0;
    for (map < hobject_t, pg_missing_item >::const_iterator i =
         needs_recovery_map.begin(); i != needs_recovery_map.end(); ++i) {
...
bool PG::MissingLoc::add_source_info(pg_shard_t fromosd,
                                     ...
                                     ThreadPool::TPHandle * handle)
{
    ...
    for (map < hobject_t, pg_missing_item >::const_iterator p =
         needs_recovery_map.begin(); p != needs_recovery_map.end(); ++p) {
        ...
        if (!oinfo.last_backfill.is_max() && !oinfo.last_backfill_bitwise) {
            ldout(pg->cct, 10) << "search_for_missing " << soid << " " << need
                ...
                << " but with wrong sort order)" << dendl;
            continue;
        }
...
void PG::MissingLoc::check_recovery_sources(const OSDMapRef & osdmap)
{
    ...
    for (set < pg_shard_t >::iterator p = missing_loc_sources.begin();
        ...
        ldout(pg->cct,
              10) << __func__ << " source osd." << *p << " now down" << dendl;
        now_down.insert(*p);
    ...
    if (now_down.empty()) {
        ldout(pg->cct,
              10) << __func__ << " no source osds (" << missing_loc_sources <<
            ") went down" << dendl;
    }
    else {
        ldout(pg->cct,
              10) << __func__ << " sources osds " << now_down <<
            " now down, remaining sources are " << missing_loc_sources << dendl;
...
void PG::discover_all_missing(map < int, map < spg_t, pg_query_t > >&query_map)
{
    ...
    for (; m != mend; ++m) {
        ...
        map < pg_shard_t, pg_info_t >::const_iterator iter =
            peer_info.find(peer);
...
/******* PG ***********/
bool PG::needs_recovery() const const
{
    assert(is_primary());
...
bool PG::needs_backfill() const const
{
    assert(is_primary());
    ...
    for (; a != end; ++a) {
        ...
        if (!pi->second.last_backfill.is_max()) {
            dout(10) << __func__ << " osd." << peer << " has last_backfill " <<
...
void PG::check_past_interval_bounds() const const
{
...
bool PG::adjust_need_up_thru(const OSDMapRef osdmap)
{
    ...
    if (need_up_thru && up_thru >= info.history.same_interval_since) {
        dout(10) << "adjust_need_up_thru now " << up_thru <<
...
void PG::remove_down_peer_info(const OSDMapRef osdmap)
{
    ...
    while (p != peer_info.end()) {
        if (!osdmap->is_up(p->first.osd)) {
            dout(10) << " dropping down osd." << p->first << " info " << p->
                second << dendl;
 ...
 */
bool PG::all_unfound_are_queried_or_lost(const OSDMapRef osdmap) const const
{
    assert(is_primary());
    ...
    dout(10) << "all_unfound_are_queried_or_lost all of might_have_unfound " <<
        might_have_unfound << " have been queried or are marked lost" << dendl;
    return true;
}
...
PastIntervals::PriorSet PG::build_prior()
{
    if (1) {
        ...
        for (map < pg_shard_t, pg_info_t >::iterator it = peer_info.begin();
             it != peer_info.end(); ++it) {
            assert(info.history.last_epoch_started >=
    ...
    PastIntervals::PriorSet prior =
        past_intervals.get_prior_set(pool.info.ec_pool(),
    ...
    if (get_osdmap()->get_up_thru(osd->whoami) <
...
auto_repair(false), num_digest_updates_pending(0), state(INACTIVE), deep(false)
{
}
 ...
 */
map < pg_shard_t,
    pg_info_t >::const_iterator PG::find_best_info(const map < pg_shard_t,
                                                   pg_info_t > &infos,
                                                   bool restrict_to_up_acting,
                                                   bool *
                                                   history_les_bound) const
    const
{
...
void PG::calc_ec_acting(map < pg_shard_t,
                        pg_info_t >::const_iterator auth_log_shard,
                        unsigned size, const vector < int >&acting,
                        pg_shard_t acting_primary, const vector < int >&up,
                        pg_shard_t up_primary, const map < pg_shard_t,
                        pg_info_t > &all_info, bool restrict_to_up_acting,
                        vector < int >*_want, set < pg_shard_t > *backfill,
                        set < pg_shard_t > *acting_backfill,
                        pg_shard_t * want_primary, ostream & ss)
{
 ...
 */
void PG::calc_replicated_acting(map < pg_shard_t,
                                pg_info_t >::const_iterator auth_log_shard,
                                unsigned size, const vector < int >&acting,
                                ...
                                pg_shard_t * want_primary, ostream & ss)
{
    ...
    if (up.size() &&
        ...
        auth_log_shard->second.log_tail) {
        ss << "up_primary: " << up_primary << ") selected as primary" << std::
            endl;
    ...
    *want_primary = primary->first;
    ...
    for (vector < int >::const_iterator i = up.begin(); i != up.end(); ++i) {
        ...
        if (cur_info.is_incomplete() ||
            cur_info.last_update < MIN(primary->second.log_tail,
                                       auth_log_shard->second.log_tail)) {
    ...
    for (vector < int >::const_iterator i = acting.begin();
        ...
        vector < int >::const_iterator up_it =
            find(up.begin(), up.end(), acting_cand.osd);
        if (up_it != up.end())
            continue;
    ...
    for (map < pg_shard_t, pg_info_t >::const_iterator i = all_info.begin();
        ...
        if (i->first == primary->first)
            continue;
        vector < int >::const_iterator up_it =
            find(up.begin(), up.end(), i->first.osd);
        if (up_it != up.end())
            continue;
...
bool PG::choose_acting(pg_shard_t & auth_log_shard_id,
                       bool restrict_to_up_acting, bool * history_les_bound)
{
    ...
    for (map < pg_shard_t, pg_info_t >::iterator p = all_info.begin();
         p != all_info.end(); ++p) {
        dout(10) << __func__ << " all_info osd." << p->first << " " << p->
            second << dendl;
    }
    ...
    if (auth_log_shard == all_info.end()) {
        if (up != acting) {
            dout(10) <<
                "choose_acting no suitable info found (incomplete backfills?),"
    ...
    if (!pool.info.ec_pool())
        calc_replicated_acting(auth_log_shard,
                               ...
                               &want,
                               &want_backfill,
                               &want_acting_backfill, &want_primary, ss);
    else
    ...
    for (int i = 0; i < (int)want.size(); ++i) {
        if (want[i] != CRUSH_ITEM_NONE) {
            ...
            have.insert(pg_shard_t(want[i],
                                   pool.info.
                                   ec_pool()? shard_id_t(i) : shard_id_t::
                                   NO_SHARD));
        }
    }
    ...
    boost::scoped_ptr < IsPGRecoverablePredicate >
        recoverable_predicate(get_pgbackend()->get_is_recoverable_predicate());
    ...
    dout(10) << "choose_acting want " << want <<
        " (== acting) backfill_targets " << want_backfill << dendl;
    return true;
}
...
void PG::build_might_have_unfound()
{
    ...
    might_have_unfound = past_intervals.get_might_have_unfound(pg_whoami,
                                                               pool.info.
                                                               ec_pool());
...
void PG::activate(ObjectStore::Transaction & t,
                  ...
                  vector <
                  pair < pg_notify_t,
                  PastIntervals > > >*activator_map, RecoveryCtx * ctx)
{
    assert(!is_peered());
    ...
    t.register_on_complete(new C_PG_ActivateCommitted(this,
    ...
    if (missing.num_missing() == 0) {
        dout(10) << "activate - no missing, moving last_complete " << info.
    ...
    if (is_primary()) {
        ...
        for (set < pg_shard_t >::iterator i = actingbackfill.begin();
            ...
            bool force_restart_backfill =
                !pi.last_backfill.is_max() && !pi.last_backfill_bitwise;
            ...
            if (pi.last_update == info.last_update && !force_restart_backfill) {
                ...
                if (!pi.is_empty() && activator_map) {
                    dout(10) << "activate peer osd." << peer <<
                        " is up to date, queueing in pending_activators" <<
                        dendl;
                    (*activator_map)[peer.osd].
                        push_back(make_pair
                                  (pg_notify_t
                                   (peer.shard, pg_whoami.shard,
                                    get_osdmap()->get_epoch(),
                    ...
                    m = new MOSDPGLog(i->shard, pg_whoami.shard,
                                      get_osdmap()->get_epoch(), info);
                }
            }
                ...
                // backfill
                osd->clog->debug() << info.
                    pgid << " starting backfill to osd." << peer << " from (" <<
                    pi.log_tail << "," << pi.last_update << "] " << pi.
                    last_backfill << " to " << info.last_update;
                ...
                m->log.copy_up_to(pg_log.get_log(),
            ...
            if (m) {
                dout(10) << "activate peer osd." << peer << " sending " << m->
                    log << dendl;
                //m->log.print(cout);
        ...
        for (set < pg_shard_t >::iterator i = actingbackfill.begin();
             i != actingbackfill.end(); ++i) {
            dout(20) << __func__ << " setting up missing_loc from shard " << *i
                << " " << dendl;
            if (*i == get_primary()) {
        ...
        if (needs_recovery()) {
            ...
            if (complete_shards.size() + 1 == actingbackfill.size()) {
                ...
                missing_loc.add_source_info(pg_whoami, info,
                                            pg_log.get_missing(), ctx->handle);
                for (set < pg_shard_t >::iterator i = actingbackfill.begin();
                     i != actingbackfill.end(); ++i) {
                    if (*i == pg_whoami)
                        continue;
                    dout(10) << __func__ << ": adding " << *i << " as a source"
                        << dendl;
                    assert(peer_missing.count(*i));
                    assert(peer_info.count(*i));
                    missing_loc.add_source_info(*i,
                                                peer_info[*i],
                                                peer_missing[*i], ctx->handle);
                }
            }
            for (map < pg_shard_t, pg_missing_t >::iterator i =
                 peer_missing.begin(); i != peer_missing.end(); ++i) {
...
bool PG::op_has_sufficient_caps(OpRequestRef & op)
{
    ...
    Session *session =
        static_cast < Session * >(req->get_connection()->get_priv());
    if (!session) {
        dout(0) << "op_has_sufficient_caps: no session for op " << *req <<
            dendl;
        return false;
    }
    ...
    const string & key = req->get_hobj().get_key().empty()?
        req->get_oid().name : req->get_hobj().get_key();
    ...
    dout(20) << "op_has_sufficient_caps "
        ...
        << " " << req->get_hobj().nspace
        << ") owner=" << pool.auid << " need_read_cap=" << op->need_read_cap()
...
void PG::_activate_committed(epoch_t epoch, epoch_t activation_epoch)
{
    ...
    if (pg_has_reset_since(epoch)) {
        ...
        dout(10) << "_activate_committed " << epoch
            ...
            << " same_interval_since " << info.history.
            same_interval_since << dendl;
        assert(!actingbackfill.empty());
        if (peer_activated.size() == actingbackfill.size())
            all_activated_and_committed();
    }
    else {
        dout(10) << "_activate_committed " << epoch << " telling primary" <<
            dendl;
        MOSDPGInfo *m = new MOSDPGInfo(epoch);
        pg_notify_t i = pg_notify_t(get_primary().shard, pg_whoami.shard,
                                    get_osdmap()->get_epoch(),
                                    get_osdmap()->get_epoch(),
                                    info);

        i.info.history.last_epoch_started = activation_epoch;
        i.info.history.last_interval_started =
            i.info.history.same_interval_since;
        if (acting.size() >= pool.info.min_size) {
        ...
        osd->send_message_osd_cluster(get_primary().osd, m,
                                      get_osdmap()->get_epoch());
...
void PG::all_activated_and_committed()
{
    ...
    queue_peering_event(CephPeeringEvtRef
                        (std::make_shared < CephPeeringEvt >
                         (get_osdmap()->get_epoch(), get_osdmap()->get_epoch(),
...
unsigned PG::get_scrub_priority()
{
    ...
    return pool_scrub_priority >
        0 ? pool_scrub_priority : cct->_conf->osd_scrub_priority;
}
...
bool PG::set_force_recovery(bool b)
{
    ...
    unlock();
    if (did) {
        dout(20) << __func__ << " state " << pgstate_history.
...
unsigned PG::get_recovery_priority()
{
    ...
    dout(20) << __func__ << " recovery priority for " << *this << " is " << ret
...
unsigned PG::get_backfill_priority()
{
    ...
    if (state & PG_STATE_FORCED_BACKFILL) {
        ...
        if (acting.size() < pool.info.min_size) {
            ...
            ret =
                OSD_BACKFILL_INACTIVE_PRIORITY_BASE + (pool.info.min_size -
                                                       acting.size());
            ...
            ret =
                OSD_BACKFILL_DEGRADED_PRIORITY_BASE + (pool.info.size -
                                                       actingset.size());
...
void PG::split_into(pg_t child_pgid, PG * child, unsigned split_bits)
{
    ...
    get_osdmap()->pg_to_up_acting_osds(child->info.pgid.pgid, &newup,
                                       &up_primary, &newacting, &primary);
...
void PG::add_backoff(SessionRef s, const hobject_t & begin,
                     const hobject_t & end)
{
    ...
    con->send_message(new MOSDBackoff(info.pgid,
                                      get_osdmap()->get_epoch(),
...
void PG::purge_strays()
{
    ...
    for (set < pg_shard_t >::iterator p = stray_set.begin();
        ...
        if (get_osdmap()->is_up(p->osd)) {
            ...
            MOSDPGRemove *m = new MOSDPGRemove(get_osdmap()->get_epoch(),
                                               to_remove);
...
bool PG::check_in_progress_op(const osd_reqid_t & r,
                              eversion_t * version,
                              version_t * user_version,
                              int *return_code) const const
{
    return (projected_log.get_request(r, version, user_version, return_code) ||
            pg_log.get_log().get_request(r, version, user_version,
                                         return_code));
}
...
static pg_shard_t get_another_shard(const set < pg_shard_t > &pgs,
                                    pg_shard_t skip, shard_id_t shard)
{
  for (auto & p:pgs) {
        if (p == skip)
            continue;
...
void PG::_update_calc_stats()
{
    ...
    if ((is_remapped() || is_undersized() || !is_clean())
        ...
        dout(20) << __func__ << " actingset " << actingset << " upset " << upset
            << " actingbackfill " << actingbackfill << dendl;
      ...
      for (auto & peer:peer_info) {
            ...
            if (is_backfill_targets(peer.first)) {
                ...
                if (peer_missing.count(peer.first)) {
                    ...
                    dout(20) << __func__ << " no peer_missing found for " <<
                        peer.first << dendl;
                    if (is_recovering()) {
                        estimate = true;
                    }
          ...
          for (auto & sml:missing_loc.get_missing_by_count()) {
              for (auto & ml:sml.second) {
                    ...
                    if (sml.first == shard_id_t::NO_SHARD) {
                        dout(20) << __func__ << " ml " << ml.
                            second << " upset size " << upset.
                            size() << " up " << ml.first.up << dendl;
                        ...
                        dout(20) << __func__ << " shard " << sml.
                            first << " ml " << ml.
                            second << " missing shards " << missing_shards <<
                            dendl;
                    }
            ...
            dout(20) << __func__ << " missing based degraded " << degraded <<
                dendl;
            dout(20) << __func__ << " missing based misplaced " << misplaced <<
                dendl;
      ...
      for (const auto & item:missing_target_objects)
            dout(20) << __func__ << " missing shard " << std::get < 1 >
                (item) << " missing= " << std::get < 0 > (item) << dendl;
      for (const auto & item:acting_source_objects)
            dout(20) << __func__ << " acting shard " << std::get < 1 >
                (item) << " missing= " << std::get < 0 > (item) << dendl;
      ...
      for (const auto & a:acting_source_objects) {
            ...
            dout(20) << __func__ << " extra acting misplaced " <<
                extra_misplaced << dendl;
            misplaced += extra_misplaced;
        }
      out:
        // NOTE: Tests use these messages to verify this code
        dout(20) << __func__ << " degraded " << degraded << (estimate ? " (est)"
                                                             : "") << dendl;
        dout(20) << __func__ << " misplaced " << misplaced << (estimate ?
                                                               " (est)" : "") <<
            dendl;
...
{
    dout(10) << "init role " << role << " up " << newup << " acting " <<
...
int PG::_prepare_write_info(CephContext * cct,
                            ...
                            bool dirty_big_info,
                            bool dirty_epoch,
                            bool try_fast_info, PerfCounters * logger)
{
    if (dirty_epoch) {
...
int PG::peek_map_epoch(ObjectStore * store,
                       spg_t pgid, epoch_t * pepoch, bufferlist * bl)
{
    coll_t coll(pgid);
...
void PG::write_if_dirty(ObjectStore::Transaction & t)
{
    ...
    pg_log.write_log_and_missing(t, &km, coll, pgmeta_oid,
...
void PG::append_log(const vector < pg_log_entry_t > &logv,
                    eversion_t trim_to,
                    eversion_t roll_forward_to,
                    ObjectStore::Transaction & t, bool transaction_applied)
{
    ...
    if (transaction_applied && roll_forward_to > pg_log.get_can_rollback_to()) {
        pg_log.roll_forward_to(roll_forward_to, &handler);
        t.register_on_applied(new C_UpdateLastRollbackInfoTrimmedToApplied(this,
                                                                           get_osdmap
                                                                           ()->
                                                                           get_epoch
                                                                           (),
...
std::string PG::get_corrupt_pg_log_name() constconst
{
...
int PG::read_info(ObjectStore * store, spg_t pgid, const coll_t & coll,
                  ...
                  PastIntervals & past_intervals, __u8 & struct_v)
{
    // try for v8 or later
    set < string > keys;
    keys.insert(infover_key);
    ...
    keys.insert(bk);
    values.clear();
    store->omap_get_values(coll_t::meta(), ghobject_t(infos_oid), keys,
                           &values);
    assert(values.size() == 2);
...
void PG::read_state(ObjectStore * store, bufferlist & bl)
{
    ...
    if (force_rebuild_missing) {
        dout(10) << __func__ <<
            " detected upgrade from jewel, force_rebuild_missing" << dendl;
    }
    ...
    pg_log.read_log_and_missing(store,
                                ...
                                ghobject_t(info_struct_v <
                                           8 ? OSD::
                                           make_pg_log_oid(pg_id) : pgmeta_oid),
                                info, force_rebuild_missing, oss,
                         ...
                         ObjectStore::Transaction & t)
{
    for (vector < pg_log_entry_t >::const_iterator i = log_entries.begin();
        ...
        if (i->soid.snap < CEPH_MAXSNAP) {
            if (i->is_delete()) {
                int r = snap_mapper.remove_oid(i->soid,
                                               &_t);
                assert(r == 0);
            }
            else if (i->is_update()) {
                ...
                try {
                    ...
                    derr << __func__ << " decode snaps failure on " << *i <<
                        dendl;
                    snaps.clear();
                }
                ...
                if (i->is_clone() || i->is_promote()) {
                    ...
                    assert(i->is_modify());
                    int r = snap_mapper.update_snaps(i->soid,
                                                     _snaps,
                                                     0,
                                                     &_t);
                    assert(r == 0);
...
void PG::filter_snapc(vector < snapid_t > &snaps)
{
    ...
    for (vector < snapid_t >::iterator p = snaps.begin(); p != snaps.end(); ++p) {
        if (snap_trimq.contains(*p) || info.purged_snaps.contains(*p)) {
            ...
            dout(20) << "filter_snapc  removing trimq|purged snap " << *p <<
                dendl;
        }
        else {
            if (filtering)
...
void PG::requeue_op(OpRequestRef op)
{
    ...
    if (p != waiting_for_map.end()) {
        dout(20) << __func__ << " " << op << " (waiting_for_map " << p->
            first << ")" << dendl;
        p->second.push_front(op);
    }
    else {
        dout(20) << __func__ << " " << op << dendl;
        osd->enqueue_front(info.pgid,
                           PGQueueable(op, get_osdmap()->get_epoch()));
    }
}
...
void PG::requeue_ops(list < OpRequestRef > &ls)
{
    for (list < OpRequestRef >::reverse_iterator i = ls.rbegin();
        ...
        if (p != waiting_for_map.end()) {
            dout(20) << __func__ << " " << *i << " (waiting_for_map " << p->
                first << ")" << dendl;
            ...
            osd->enqueue_front(info.pgid,
                               PGQueueable(*i, get_osdmap()->get_epoch()));
        }
    }
    ls.clear();
}
...
bool PG::sched_scrub()
{
    ...
    dout(20) << __func__ << ": time_for_deep=" << time_for_deep <<
    ...
    if (cct->_conf->osd_scrub_auto_repair
        ...
        && !scrubber.must_scrub && !scrubber.must_deep_scrub) {
    ...
    if (!scrubber.reserved) {
        ...
        if ((cct->_conf->osd_scrub_during_recovery
             || !osd->is_recovery_active()) && osd->inc_scrubs_pending()) {
            dout(20) << __func__ << ": reserved locally, reserving replicas" <<
                dendl;
            scrubber.reserved = true;
    ...
    if (scrubber.reserved) {
        if (scrubber.reserve_failed) {
            ...
            dout(20) << "sched_scrub: success, reserved self and replicas" <<
                dendl;
            if (time_for_deep) {
                dout(10) << "sched_scrub: scrub will be deep" << dendl;
                state_set(PG_STATE_DEEP_SCRUB);
            }
            else if (!scrubber.must_deep_scrub
            ...
            dout(20) << "sched_scrub: reserved " << scrubber.
                reserved_peers << ", waiting for replicas" << dendl;
        }
    }
...
{
    const MOSDRepScrubMap *m =
        static_cast < const MOSDRepScrubMap * >(op->get_req());
    ...
    dout(10) << "map version is "
        << scrubber.received_maps[m->from].valid_through << dendl;
                            ...
                            bool deep, bool allow_preemption)
{
    ...
    osd->send_message_osd_cluster(replica.osd, repscrubop,
                                  get_osdmap()->get_epoch());
}
...
void PG::handle_scrub_reserve_request(OpRequestRef op)
{
    ...
    if (op->get_req()->get_type() == MSG_OSD_SCRUB_RESERVE) {
        ...
        Message *reply =
            new MOSDScrubReserve(spg_t(info.pgid.pgid, primary.shard),
                                 m->map_epoch,
                                 scrubber.
                                 reserved ? MOSDScrubReserve::
                                 GRANT : MOSDScrubReserve::REJECT,
                                 pg_whoami);
        ...
        MOSDSubOpReply *reply =
            new MOSDSubOpReply(req, pg_whoami, 0, get_osdmap()->get_epoch(),
                               CEPH_OSD_FLAG_ACK);
...
{
    osd->send_message_osd_cluster(primary.osd,
                                  new MBackfillReserve(MBackfillReserve::REJECT,
...
void PG::schedule_backfill_retry(float delay)
{
    ...
    osd->recovery_request_timer.add_event_after(delay,
                                                new QueuePeeringEvt <
                                                RequestBackfill > (this,
                                                                   get_osdmap
                                                                   ()->
                                                                   get_epoch(),
                                                                   RequestBackfill
...
void PG::scrub_reserve_replicas()
{
    ...
    for (set < pg_shard_t >::iterator i = actingbackfill.begin();
        ...
        if (HAVE_FEATURE(get_min_acting_features(), SERVER_LUMINOUS)) {
            osd->send_message_osd_cluster(i->osd,
                                          new
                                          MOSDScrubReserve(spg_t
                                                           (info.pgid.pgid,
                                                            i->shard),
                                                           get_osdmap()->
                                                           get_epoch(),
            ...
            osd_reqid_t reqid;
            MOSDSubOp *subop =
                new MOSDSubOp(reqid, pg_whoami, spg_t(info.pgid.pgid, i->shard),
                              poid, 0,
                              get_osdmap()->get_epoch(), osd->get_tid(), v);
            subop->ops = scrub;
            osd->send_message_osd_cluster(i->osd, subop,
                                          get_osdmap()->get_epoch());
        }
    }
}
                            ...
                            ThreadPool::TPHandle & handle)
{
    ...
    if (!t.empty()) {
        derr << __func__ <<
            ": queueing trans to clean up obsolete rollback objs" << dendl;
...
void PG::_scan_snaps(ScrubMap & smap)
{
    ...
    for (map < hobject_t, ScrubMap::object >::reverse_iterator i =
         smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
        ...
        if (hoid.snap < CEPH_MAXSNAP) {
            ...
            if (!snapset.is_legacy()) {
                ...
                if (p == snapset.clone_snaps.end()) {
                    derr << __func__ << " no clone_snaps for " << hoid << " in "
                        << snapset << dendl;
                    continue;
                }
                ...
                obj_snaps.insert(oi.legacy_snaps.begin(),
                                 oi.legacy_snaps.end());
            }
            set < snapid_t > cur_snaps;
            int r = snap_mapper.get_snaps(hoid, &cur_snaps);
            if (r != 0 && r != -ENOENT) {
                derr << __func__ << ": get_snaps returned " << cpp_strerror(r)
                    << dendl;
                ceph_abort();
            }
            if (r == -ENOENT || cur_snaps != obj_snaps) {
                ...
                if (r == 0) {
                    ...
                    if (r != 0) {
                        derr << __func__ << ": remove_oid returned " <<
                            cpp_strerror(r)
                            << dendl;
                        ceph_abort();
                    }
                    osd->clog->error() << "osd." << osd->whoami
                        ...
                        << cur_snaps << ", oi: " << obj_snaps << "...repaired";
                }
                else {
                    osd->clog->error() << "osd." << osd->whoami
                        << " found snap mapper error on pg "
                ...
                // wait for repair to apply to avoid confusing other bits of the system.
                {
                    ...
                    int r = 0;
                    bool done;
                    t.register_on_applied_sync(new
                                               C_SafeCond(&my_lock, &my_cond,
                                                          &done, &r));
                    r = osd->store->queue_transaction(osr.get(), std::move(t),
                                                      nullptr);
                    if (r != 0) {
                        derr << __func__ << ": apply_transaction got " <<
                            cpp_strerror(r)
                            << dendl;
                    }
                    else {
...
int PG::build_scrub_map_chunk(ScrubMap & map,
                              ...
                              hobject_t start,
                              hobject_t end,
                              bool deep, ThreadPool::TPHandle & handle)
{
    ...
    while (pos.empty()) {
        ...
        pos.ret = get_pgbackend()->objects_list_range(start,
                                                      end,
                                                      0,
                                                      &pos.ls, &rollback_obs);
        if (pos.ret < 0) {
...
void PG::Scrubber::cleanup_store(ObjectStore::Transaction * t)
{
    ...
    struct OnComplete:Context {
        ...
        OnComplete(std::unique_ptr < Scrub::Store > &&store)
...
void PG::repair_object(const hobject_t & soid, list < pair < ScrubMap::object,
                       pg_shard_t > >*ok_peers, pg_shard_t bad_peer)
{
  ...
  for (auto i:*ok_peers) {
    ...
    if (is_ec_pg() || bad_peer == primary) {
        ...
        for (i = ok_peers->begin(); i != ok_peers->end(); ++i)
...
{
    const MOSDRepScrub *msg =
        static_cast < const MOSDRepScrub * >(op->get_req());
...
void PG::scrub(epoch_t queued, ThreadPool::TPHandle & handle)
{
    if (cct->_conf->osd_scrub_sleep > 0 &&
        ...
        dout(20) << __func__ << " state is INACTIVE|NEW_CHUNK, sleeping" <<
            dendl;
        ...
        auto scrub_requeue_callback =
            new FunctionContext([osds, pgid, state] (int r){
                                ...
                                ceph_clock_now() -
                                pg->scrubber.
                                sleep_start << ", re-queuing scrub with state "
                                << state << dendl; pg->scrub_queued = false;
                                pg->requeue_scrub();
    ...
    while (!done) {
        ...
        case PG::Scrubber::NEW_CHUNK:
...

            {
                ...
                osr->flush();
                ret = get_pgbackend()->objects_list_partial(start,
                                                            min,
                                                            max,
                                                            &objects,
                                                            &candidate_end);
                ...
                if (!_range_available_for_scrub(scrubber.start, candidate_end)) {
                    ...
                    dout(10) << __func__ <<
                        ...
                        scrubber.start << ", " << candidate_end << ")" << dendl;
                    done = true;
                    break;
                }
            ...
            dout(10) << __func__ << " waiting_on_whom " << scrubber.
                waiting_on_whom << dendl;
        ...
        case PG::Scrubber::BUILD_MAP_REPLICA:
            ...
            if (scrub_preempted) {
                ...
                ret = build_scrub_map_chunk(scrubber.replica_scrubmap,
            ...
            if (HAVE_FEATURE(acting_features, SERVER_LUMINOUS)) {
                MOSDRepScrubMap *reply =
                    new
                    MOSDRepScrubMap(spg_t(info.pgid.pgid, get_primary().shard),
                ...
                eversion_t v;
                osd_reqid_t reqid;
                MOSDSubOp *subop = new MOSDSubOp(reqid,
                                                 pg_whoami,
                                                 spg_t(info.pgid.pgid,
                                                       get_primary().shard),
                                                 poid,
                ...
                subop->ops = scrub;
                osd->send_message_osd_cluster(get_primary().osd, subop,
                                              scrubber.replica_scrub_start);
            }
            scrub_preempted = false;
            scrub_can_preempt = false;
...
bool PG::write_blocked_by_scrub(const hobject_t & soid)
{
    ...
    if (scrub_can_preempt) {
        if (!scrub_preempted) {
            ...
            dout(10) << __func__ << " " << soid << " already preempted" <<
                dendl;
        }
        return false;
    }
...
void PG::scrub_compare_maps()
{
  ...
  for (const auto map:maps) {
    ...
    if (acting.size() > 1) {
        ...
        get_pgbackend()->be_compare_scrubmaps(maps,
                                              master_set,
        ...
        for (map < hobject_t, list < pg_shard_t >>::iterator i =
             authoritative.begin(); i != authoritative.end(); ++i) {
            list < pair < ScrubMap::object, pg_shard_t > >good_peers;
            for (list < pg_shard_t >::const_iterator j = i->second.begin();
                 j != i->second.end(); ++j) {
                good_peers.
                    push_back(make_pair(maps[*j]->objects[i->first], *j));
            }
            scrubber.authoritative.insert(make_pair(i->first, good_peers));
        }

        for (map < hobject_t, list < pg_shard_t >>::iterator i =
             authoritative.begin(); i != authoritative.end(); ++i) {
            scrubber.cleaned_meta_map.objects.erase(i->first);
            scrubber.cleaned_meta_map.objects.
                insert(*(maps[i->second.back()]->objects.find(i->first))
                );
        }
    }
...
bool PG::scrub_process_inconsistent()
{
    ...
    if (!scrubber.authoritative.empty()) {
        ...
        if (repair) {
            ...
            for (map < hobject_t, list < pair < ScrubMap::object,
                 pg_shard_t > >>::iterator i = scrubber.authoritative.begin();
                ...
                if (missing_entry != scrubber.missing.end()) {
                    for (j = missing_entry->second.begin();
                         j != missing_entry->second.end(); ++j) {
                        repair_object(i->first, &(i->second), *j);
                        ++scrubber.fixed;
                    }
                }
                if (scrubber.inconsistent.count(i->first)) {
                    for (j = scrubber.inconsistent[i->first].begin();
                         j != scrubber.inconsistent[i->first].end(); ++j) {
                        repair_object(i->first, &(i->second), *j);
                        ++scrubber.fixed;
                    }
                }
...
bool PG::ops_blocked_by_scrub() const const
{
...
void PG::scrub_finish()
{
    ...
    if (deep_scrub) {
        ...
        info.stats.stats.sum.num_large_omap_objects =
...
void PG::share_pg_info()
{
    ...
    for (set < pg_shard_t >::iterator i = actingbackfill.begin();
        ...
        m->pg_list.push_back(make_pair(pg_notify_t(peer.shard, pg_whoami.shard,
                                                   get_osdmap()->get_epoch(),
                                                   get_osdmap()->get_epoch(),
                                                   info), PastIntervals()));
...
bool PG::append_log_entries_update_missing(const mempool::osd_pglog::list <
                                           pg_log_entry_t > &entries,
                                           ObjectStore::Transaction & t,
                                           boost::optional < eversion_t >
                                           trim_to,
                                           boost::optional < eversion_t >
                                           roll_forward_to)
{
    ...
    PGLogEntryHandler rollbacker {
    this, &t};
    bool invalidate_stats = pg_log.append_new_log_entries(info.last_backfill,
                                                          info.
    ...
    dout(20) << __func__ << "trim_to bool = " << bool(trim_to) << " trim_to = "
        << (trim_to ? *trim_to : eversion_t()) << dendl;
    if (trim_to)
...
void PG::merge_new_log_entries(const mempool::osd_pglog::list < pg_log_entry_t >
                               &entries, ObjectStore::Transaction & t,
                               ...
                               boost::optional < eversion_t > roll_forward_to)
{
    ...
    for (set < pg_shard_t >::const_iterator i = actingbackfill.begin();
        ...
        dout(20) << __func__ << " peer_missing for " << peer << " = " <<
            pmissing << dendl;
        pg_info_t & pinfo(peer_info[peer]);
        bool invalidate_stats =
            PGLog::append_log_entries_update_missing(pinfo.last_backfill,
...
void PG::update_history(const pg_history_t & new_history)
{
    ...
    if (info.history.merge(new_history)) {
        dout(20) << __func__ << " advanced history from " << new_history <<
            dendl;
        dirty_info = true;
        if (info.history.last_epoch_clean >= info.history.same_interval_since) {
            dout(20) << __func__ << " clearing past_intervals" << dendl;
...
void PG::fulfill_info(pg_shard_t from, const pg_query_t & query,
...
void PG::fulfill_log(pg_shard_t from, const pg_query_t & query,
                     epoch_t query_epoch)
{
    ...
    assert(query.type != pg_query_t::INFO);
    ConnectionRef con =
        osd->get_con_osd_cluster(from.osd, get_osdmap()->get_epoch());
    if (!con)
        return;
...
void PG::fulfill_query(const MQuery & query, RecoveryCtx * rctx)
{
    if (query.query.type == pg_query_t::INFO) {
        ...
        rctx->send_notify(notify_info.first,
...
void PG::check_full_transition(OSDMapRef lastmap, OSDMapRef osdmap)
{
    ...
    if (osdmap->test_flag(CEPH_OSDMAP_FULL) &&
        ...
        dout(10) << " cluster was marked full in " << osdmap->
            get_epoch() << dendl;
        changed = true;
    }
    ...
    if (pi->has_flag(pg_pool_t::FLAG_FULL)) {
        ...
        if (!opi || !opi->has_flag(pg_pool_t::FLAG_FULL)) {
            dout(10) << " pool was marked full in " << osdmap->
                get_epoch() << dendl;
            changed = true;
        }
    }
...
bool PG::should_restart_peering(int newupprimary,
...
{
    if (last_peering_reset > reply_epoch || last_peering_reset > query_epoch) {
        dout(10) << "old_peering_msg reply_epoch " << reply_epoch <<
            " query_epoch " << query_epoch << " last_peering_reset " <<
...
void PG::start_flush(ObjectStore::Transaction * t,
                     list < Context * >*on_applied, list < Context * >*on_safe)
{
    ...
    t->nop();
    flushes_in_progress++;
    on_applied->push_back(new ContainerContext < FlushStateRef >
                          (flush_trigger));
...
void PG::reset_interval_flush()
{
    ...
    Context *c =
        new QueuePeeringEvt < IntervalFlush > (this, get_osdmap()->get_epoch(),
                                               IntervalFlush());
                                ...
                                ObjectStore::Transaction * t)
{
    ...
    if (!lastmap) {
        ...
        boost::scoped_ptr < IsPGRecoverablePredicate >
            recoverable(get_is_recoverable_predicate());
        bool new_interval =
        ...
        if (new_interval) {
            if (osdmap->get_epoch() == osd->get_superblock().oldest_map &&
                ...
                dout(10) << " map gap, clearing past_intervals and faking" <<
                    dendl;
            ...
            info.history.same_interval_since = osdmap->get_epoch();
            if (info.pgid.pgid.
                is_split(lastmap->get_pg_num(info.pgid.pgid.pool()),
    ...
    dout(1) << __func__ << " up " << oldup << " -> " << up
        ...
        new_up_primary << ", role " << oldrole << " -> " << role <<
        ", features acting " << acting_features << " upacting " <<
...
void PG::proc_primary_info(ObjectStore::Transaction & t,
                           const pg_info_t & oinfo)
{
    assert(!is_primary());
    ...
    if (!(info.purged_snaps == oinfo.purged_snaps)) {
        dout(10) << __func__ << " updating purged_snaps to " << oinfo.
            purged_snaps << dendl;
    ...
    if (m->get_connection()->has_feature(CEPH_FEATURE_RESEND_ON_SPLIT)) {
        if (m->get_map_epoch() < pool.info.get_last_force_op_resend()) {
            dout(7) << __func__ << " sent before last_force_op_resend "
                << pool.info.
                last_force_op_resend << ", dropping" << *m << dendl;
            return true;
        }
        ...
        if (m->get_map_epoch() <
            ...
            dout(7) << __func__ <<
                " sent before last_force_op_resend_preluminous " << pool.info.
                last_force_op_resend_preluminous << ", dropping" << *m << dendl;
            return true;
        }
    }
...
bool PG::can_discard_request(OpRequestRef & op)
{
    ...
    case MSG_OSD_PG_PUSH_REPLY:
        return can_discard_replica_op < MOSDPGPushReply,
            MSG_OSD_PG_PUSH_REPLY > (op);
    case MSG_OSD_SUBOPREPLY:
        return can_discard_replica_op < MOSDSubOpReply,
            MSG_OSD_SUBOPREPLY > (op);
    case MSG_OSD_REPOPREPLY:
        return can_discard_replica_op < MOSDRepOpReply,
            MSG_OSD_REPOPREPLY > (op);
    ...
    case MSG_OSD_EC_WRITE:
        return can_discard_replica_op < MOSDECSubOpWrite,
            MSG_OSD_EC_WRITE > (op);
...
void PG::queue_query(epoch_t msg_epoch,
                     epoch_t query_epoch, pg_shard_t from, const pg_query_t & q)
{
...
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Crashed")
{
...
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Initial")
{
    context < RecoveryMachine > ().log_enter(state_name);
}
...
boost::statechart::result PG::RecoveryState::Initial::
...
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Started")
{
    context < RecoveryMachine > ().log_enter(state_name);
}
...
boost::statechart::result PG::RecoveryState::Started::react(const FlushedEvt &)
{
    PG *pg = context < RecoveryMachine > ().pg;
    pg->on_flushed();
    return discard_event();
...
boost::statechart::result PG::RecoveryState::Started::
...
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Reset")
{
    context < RecoveryMachine > ().log_enter(state_name);
    PG *pg = context < RecoveryMachine > ().pg;
...
boost::statechart::result PG::RecoveryState::Reset::react(const FlushedEvt &)
{
    PG *pg = context < RecoveryMachine > ().pg;
    pg->on_flushed();
    return discard_event();
...
boost::statechart::result PG::RecoveryState::Reset::react(const IntervalFlush &)
{
    PG *pg = context < RecoveryMachine > ().pg;
    ldout(pg->cct, 10) << "Ending blocked outgoing recovery messages" << dendl;
    context < RecoveryMachine > ().pg->recovery_state.end_block_outgoing();
...
boost::statechart::result PG::RecoveryState::Reset::react(const AdvMap & advmap)
{
    ...
    if (pg->should_restart_peering(advmap.up_primary,
        ...
        ldout(pg->cct,
            ...
            dendl;
        pg->start_peering_interval(advmap.lastmap, advmap.newup,
                                   advmap.up_primary, advmap.newacting,
                                   advmap.acting_primary,
                                   context < RecoveryMachine >
                                   ().get_cur_transaction());
    }
    pg->remove_down_peer_info(advmap.osdmap);
...
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Start")
{
    context < RecoveryMachine > ().log_enter(state_name);
...
:  
my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Started/Primary")
{
    context < RecoveryMachine > ().log_enter(state_name);
    PG *pg = context < RecoveryMachine > ().pg;
...
boost::statechart::result PG::RecoveryState::Primary::
...
boost::statechart::result PG::RecoveryState::Peering::
react(const AdvMap & advmap)
{
    PG *pg = context < RecoveryMachine > ().pg;
    ...
    PG::RecoveryState::Backfilling::react(const DeferBackfill & c)
{
    ...
    for (set < pg_shard_t >::iterator it = pg->backfill_targets.begin();
         it != pg->backfill_targets.end(); ++it) {
        assert(*it != pg->pg_whoami);
        ConnectionRef con =
            pg->osd->get_con_osd_cluster(it->osd,
                                         pg->get_osdmap()->get_epoch());
        if (con) {
            pg->osd->
                send_message_osd_cluster(new
                                         MBackfillReserve(MBackfillReserve::
                                                          REJECT,
                                                          spg_t(pg->info.pgid.
                                                                pgid,
                                                                it->shard),
...
PG::RecoveryState::WaitRemoteBackfillReserved::
WaitRemoteBackfillReserved(my_context ctx)
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg,
...
react(const RemoteBackfillReserved & evt)
{
    ...
    if (backfill_osd_it != context < Active >
        ...
        ConnectionRef con =
            pg->osd->get_con_osd_cluster(backfill_osd_it->osd,
                                         pg->get_osdmap()->get_epoch());
        if (con) {
            pg->osd->
                send_message_osd_cluster(new
...
void PG::RecoveryState::WaitRemoteBackfillReserved::exit()
{
    ...
    PG *pg = context < RecoveryMachine > ().pg;
    utime_t dur = ceph_clock_now() - enter_time;
    pg->osd->recoverystate_perf->tinc(rs_waitremotebackfillreserved_latency,
                                      dur);
}

boost::statechart::result
    PG::RecoveryState::WaitRemoteBackfillReserved::
react(const RemoteReservationRejected & evt)
{
    PG *pg = context < RecoveryMachine > ().pg;
    pg->osd->local_reserver.cancel_reservation(pg->info.pgid);
...
PG::RecoveryState::WaitLocalBackfillReserved::
                             ...
                             "Started/Primary/Active/WaitLocalBackfillReserved")
{
    ...
    PG *pg = context < RecoveryMachine > ().pg;
    pg->state_set(PG_STATE_BACKFILL_WAIT);
    pg->osd->local_reserver.request_reservation(pg->info.pgid,
                                                new QueuePeeringEvt <
                                                LocalBackfillReserved > (pg,
                                                                         pg->
                                                                         get_osdmap
                                                                         ()->
                                                                         get_epoch
...
void PG::RecoveryState::WaitLocalBackfillReserved::exit()
{
    ...
    PG *pg = context < RecoveryMachine > ().pg;
    utime_t dur = ceph_clock_now() - enter_time;
    pg->osd->recoverystate_perf->tinc(rs_waitlocalbackfillreserved_latency,
                                      dur);
}
...
boost::statechart::result
    PG::RecoveryState::NotBackfilling::
react(const RemoteReservationRejected & evt)
{
    return discard_event();
}
...
boost::statechart::result
    PG::RecoveryState::RepNotRecovering::
react(const RejectRemoteReservation & evt)
{
    PG *pg = context < RecoveryMachine > ().pg;
...
PG::RecoveryState::RepWaitRecoveryReserved::
                             ...
                             "Started/ReplicaActive/RepWaitRecoveryReserved")
{
    ...
    pg->osd->remote_reserver.request_reservation(pg->info.pgid,
                                                 new QueuePeeringEvt <
                                                 RemoteRecoveryReserved > (pg,
                                                                           pg->
                                                                           get_osdmap
                                                                           ()->
                                                                           get_epoch
...
{
    PG *pg = context < RecoveryMachine > ().pg;
    pg->osd->send_message_osd_cluster(pg->primary.osd,
                                      new MRecoveryReserve(MRecoveryReserve::
                                                           GRANT,
                                                           spg_t(pg->info.pgid.
                                                                 pgid,
                                                                 pg->primary.
...
PG::RecoveryState::RepWaitBackfillReserved::
    ...
    PG::RecoveryState::RepNotRecovering::react(const RequestBackfillPrio & evt)
{
    ...
    if (pg->cct->_conf->osd_debug_reject_backfill_probability > 0 &&
        (rand() % 1000 <
         (pg->cct->_conf->osd_debug_reject_backfill_probability * 1000.0))) {
        ldout(pg->cct,
        ...
        ldout(pg->cct,
              10) << "backfill reservation rejected: " << ss.str() << dendl;
        post_event(RejectRemoteReservation());
    }
    else {
        pg->osd->remote_reserver.request_reservation(pg->info.pgid,
...
react(const RemoteBackfillReserved & evt)
{
    ...
    if (pg->cct->_conf->osd_debug_reject_backfill_probability > 0 &&
        ...
        return discard_event();
             ...
             && pg->osd->check_backfill_full(ss)) {
        ldout(pg->cct,
              10) << "backfill reservation rejected after reservation: " << ss.
            str() << dendl;
        post_event(RejectRemoteReservation());
        return discard_event();
    }
...
PG::RecoveryState::WaitLocalRecoveryReserved::
...
void PG::RecoveryState::WaitLocalRecoveryReserved::exit()
{
    ...
    PG *pg = context < RecoveryMachine > ().pg;
    utime_t dur = ceph_clock_now() - enter_time;
    pg->osd->recoverystate_perf->tinc(rs_waitlocalrecoveryreserved_latency,
                                      dur);
}

PG::RecoveryState::WaitRemoteRecoveryReserved::
...
react(const RemoteRecoveryReserved & evt)
{
    ...
    if (remote_recovery_reservation_it != context < Active >
        ().remote_shards_to_reserve_recovery.end()) {
        assert(*remote_recovery_reservation_it != pg->pg_whoami);
        ConnectionRef con =
            pg->osd->get_con_osd_cluster(remote_recovery_reservation_it->osd,
                                         pg->get_osdmap()->get_epoch());
        if (con) {
            pg->osd->
                send_message_osd_cluster(new
                                         MRecoveryReserve(MRecoveryReserve::
                                                          REQUEST,
                                                          spg_t(pg->info.pgid.
                                                                pgid,
...
void PG::RecoveryState::WaitRemoteRecoveryReserved::exit()
{
    ...
    PG *pg = context < RecoveryMachine > ().pg;
    utime_t dur = ceph_clock_now() - enter_time;
    pg->osd->recoverystate_perf->tinc(rs_waitremoterecoveryreserved_latency,
                                      dur);
}
...
void PG::RecoveryState::Recovering::release_reservations(bool cancel)
{
    ...
    for (set < pg_shard_t >::const_iterator i =
        ...
        if (*i == pg->pg_whoami)    // skip myself
            continue;
        ConnectionRef con =
            pg->osd->get_con_osd_cluster(i->osd, pg->get_osdmap()->get_epoch());
        if (con) {
            pg->osd->
                send_message_osd_cluster(new
                                         MRecoveryReserve(MRecoveryReserve::
...
NamedState(context < RecoveryMachine > ().pg, "Started/Primary/Active/Clean")
{
    ...
    pg->finish_recovery(*context < RecoveryMachine >
...
boost::statechart::result PG::RecoveryState::Active::
react(const AdvMap & advmap)
{
    PG *pg = context < RecoveryMachine > ().pg;
    ...
    if (!pg->pool.newly_removed_snaps.empty()) {
        ...
        ldout(pg->cct,
              10) << *pg << " snap_trimq now " << pg->snap_trimq << dendl;
        pg->dirty_info = true;
    ...
    if (pg->info.stats.reported_epoch +
        pg->cct->_conf->osd_pg_stat_report_interval_max <
        advmap.osdmap->get_epoch()) {
        ldout(pg->cct,
              20) << "reporting stats to osd after " << (advmap.osdmap->
                                                         get_epoch() -
                                                         pg->info.stats.
                                                         reported_epoch)
            << " epochs" << dendl;
...
boost::statechart::result PG::RecoveryState::Active::react(const ActMap &)
{
    ...
    if (pg->have_unfound()) {
        ...
        pg->discover_all_missing(*context < RecoveryMachine >
                                 ().get_query_map());
    }
    ...
    if (unfound > 0 && pg->all_unfound_are_queried_or_lost(pg->get_osdmap())) {
    ...
    if (pg->is_peered() &&
        ...
        (!pg->get_osdmap()->test_flag(CEPH_OSDMAP_NOREBALANCE)
         || pg->is_degraded())) {
        pg->queue_recovery();
    }
    return forward_event();
...
react(const MLogRec & logevt)
{
    ...
    pg->proc_replica_log(logevt.msg->info, logevt.msg->log, logevt.msg->missing,
                         logevt.from);
    bool got_missing = pg->search_for_missing(pg->peer_info[logevt.from],
...
boost::statechart::result PG::RecoveryState::Active::react(const QueryState & q)
{
...

    {
        ...
        q.f->dump_string("scrubber.state",
                         Scrubber::state_string(pg->scrubber.state));
        ...
        q.f->dump_stream("scrubber.subset_last_update") << pg->scrubber.
            subset_last_update;
        q.f->dump_bool("scrubber.deep", pg->scrubber.deep);
        {
            q.f->open_array_section("scrubber.waiting_on_whom");
            for (set < pg_shard_t >::iterator p =
                 pg->scrubber.waiting_on_whom.begin();
                 p != pg->scrubber.waiting_on_whom.end(); ++p) {
...
boost::statechart::result PG::RecoveryState::ReplicaActive::
...
:  my_base(ctx), NamedState(context < RecoveryMachine > ().pg, "Started/Stray")
{
    context < RecoveryMachine > ().log_enter(state_name);
...
boost::statechart::result PG::RecoveryState::Stray::
react(const MLogRec & logevt)
{
    PG *pg = context < RecoveryMachine > ().pg;
    MOSDPGLog *msg = logevt.msg.get();
    ldout(pg->cct,
          10) << "got info+log from osd." << logevt.from << " " << msg->
        info << " " << msg->log << dendl;
...
react(const MInfoRec & infoevt)
{
    PG *pg = context < RecoveryMachine > ().pg;
    ldout(pg->cct,
          10) << "got info from osd." << infoevt.from << " " << infoevt.
        info << dendl;
...
void PG::RecoveryState::GetInfo::get_infos()
{
    ...
    for (set < pg_shard_t >::const_iterator it = prior_set.probe.begin();
        ...
        if (pg->peer_info.count(peer)) {
            ldout(pg->cct,
                  10) << " have osd." << peer << " info " << pg->
                peer_info[peer] << dendl;
            continue;
        }
        if (peer_info_requested.count(peer)) {
            ldout(pg->cct,
                  10) << " already requested info from osd." << peer << dendl;
            pg->blocked_by.insert(peer.osd);
        }
        else if (!pg->get_osdmap()->is_up(peer.osd)) {
            ldout(pg->cct,
                  10) << " not querying info from down osd." << peer << dendl;
        }
        else {
            ldout(pg->cct, 10) << " querying info from osd." << peer << dendl;
            context < RecoveryMachine > ().send_query(peer,
                                                      pg_query_t(pg_query_t::
                                                                 INFO,
                                                                 it->shard,
                                                                 pg->pg_whoami.
                                                                 shard,
...
boost::statechart::result PG::RecoveryState::GetInfo::
react(const MNotifyRec & infoevt)
{
    ...
    if (pg->
        proc_replica_info(infoevt.from, infoevt.notify.info,
                          infoevt.notify.epoch_sent)) {
        // we got something new ...
        PastIntervals::PriorSet & prior_set = context < Peering > ().prior_set;
        if (old_start < pg->info.history.last_epoch_started) {
            ldout(pg->cct,
                  10) << " last_epoch_started moved forward, rebuilding prior"
                << dendl;
            prior_set = pg->build_prior();
        ...
        if (peer_info_requested.empty() && !prior_set.pg_down) {
            ldout(pg->cct,
                  20) << "Common peer features: " << hex << pg->
                get_min_peer_features() << dec << dendl;
            ldout(pg->cct,
                  20) << "Common acting features: " << hex << pg->
                get_min_acting_features() << dec << dendl;
            ldout(pg->cct,
                  20) << "Common upacting features: " << hex << pg->
                get_min_upacting_features() << dec << dendl;
            post_event(GotInfo());
        }
    }
    return discard_event();
...
:  
my_base(ctx),
NamedState(context < RecoveryMachine > ().pg, "Started/Primary/Peering/GetLog"),
msg(0)
{
    context < RecoveryMachine > ().log_enter(state_name);
    ...
    if (pg->info.last_update < best.log_tail) {
        ldout(pg->cct,
              10) << " not contiguous with osd." << auth_log_shard << ", down"
            << dendl;
        post_event(IsIncomplete());
        return;
    }
    ...
    for (set < pg_shard_t >::iterator p = pg->actingbackfill.begin();
        ...
        if (ri.last_update < pg->info.log_tail
    ...
    // how much?
    ldout(pg->cct,
          10) << " requesting log from osd." << auth_log_shard << dendl;
    context < RecoveryMachine > ().send_query(auth_log_shard,
...
boost::statechart::result PG::RecoveryState::GetLog::
react(const AdvMap & advmap)
{
    PG *pg = context < RecoveryMachine > ().pg;
...
boost::statechart::result PG::RecoveryState::GetLog::react(const GotLog &)
{
    ...
    if (msg) {
        ...
        pg->proc_master_log(*context < RecoveryMachine >
                            ().get_cur_transaction(), msg->info, msg->log,
                            msg->missing, auth_log_shard);
    }
    pg->start_flush(context < RecoveryMachine > ().get_cur_transaction(),
                    context < RecoveryMachine >
                    ().get_on_applied_context_list(),
...
boost::statechart::result PG::RecoveryState::WaitActingChange::
react(const AdvMap & advmap)
{
    PG *pg = context < RecoveryMachine > ().pg;
...
boost::statechart::result PG::RecoveryState::Incomplete::
react(const AdvMap & advmap)
{
    PG *pg = context < RecoveryMachine > ().pg;
           ...
           "Started/Primary/Peering/GetMissing")
{
    ...
    for (set < pg_shard_t >::iterator i = pg->actingbackfill.begin();
        ...
        pg->peer_missing[*i].may_include_deletes =
            !pg->perform_deletes_during_peering();
        ...
        if (pi.last_update == pi.last_complete &&   // peer has no missing
            ...
            //        can infer the rest!
            ldout(pg->cct,
                  10) << " osd." << *i << " has no missing, identical log" <<
                dendl;
            pg->peer_missing[*i].clear();
            continue;
        }
...
boost::statechart::result PG::RecoveryState::GetMissing::
react(const MLogRec & logevt)
{
    PG *pg = context < RecoveryMachine > ().pg;
...
boost::statechart::result PG::RecoveryState::WaitUpThru::
                                                  ...
                                                  utime_t enter_time)
{
    ...
    PG *pg = context < RecoveryMachine > ().pg;
    ldout(pg->cct,
          5) << "exit " << state_name << " " << dur << " " << event_count << " "
        << event_time << dendl;
    pg->osd->pg_recovery_stats.log_exit(state_name,
