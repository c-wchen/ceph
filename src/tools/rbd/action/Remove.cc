        ...
        namespace remove {
            ...
            namespace {
                ...
                bool is_auto_delete_snapshot(librbd::Image * image,
                    ...
                    int r =
                        image->snap_get_namespace_type(snap_info.id,
                                                       &namespace_type);
                    if (r < 0) {
                        return false;
            ...
            }}} // anonymous namespace namespace at = argument_types;
            ...
            int execute(const po::variables_map & vm,
                ...
                if (r < 0) {
                    if (r == -ENOTEMPTY) {
                        ...
                        int image_r =
                            utils::open_image(io_ctx, image_name, true, &image);
                        if (image_r >= 0) {
                        ...
                        if (image_r >= 0) {
                            snaps.
                                erase(std::
                                      remove_if(snaps.begin(), snaps.end(),
                                                [&image] (const librbd::
                                                          snap_info_t & snap) {
                                                return
                                                is_auto_delete_snapshot(&image,
                                                                        snap);}
                                      ), snaps.end());
                        }
                        ...
                        if (image_r == 0) {
                            ...
                            image_r =
                                rados.ioctx_create2(group_info.pool,
                                                    pool_io_ctx);
                            if (image_r < 0) {
                                pool_name =
                                    "<missing group pool " +
                        ...
                        std::
                            cerr << "rbd: delete error: " << cpp_strerror(r) <<
                            std::endl;
                    }
                    return r;
                }
