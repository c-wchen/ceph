                     ...
                     bool connection_must_exist)
    {
        ...
        if (profile.conn_conf && default_profile.conn_conf) {
            if (!profile.conn_conf->has_endpoint) {
                profile.conn_conf->endpoint =
                    default_profile.conn_conf->endpoint;
            }
            if (!profile.conn_conf->has_host_style) {
                profile.conn_conf->host_style =
                    default_profile.conn_conf->host_style;
            }
...
public:
                                ...
                                map < string, string > *attrs)
    {
        ...
        snprintf(buf, sizeof(buf), "%llu",
                 (long long)src_properties.versioned_epoch);
        ...
        if (!rest_obj.key.instance.empty()) {
            new_attrs["x-amz-meta-rgwx-source-version-id"] =
                rest_obj.key.instance;
        }
    }
                    ...
                    const rgw_rest_obj &rest_obj) override
    {
        ...
        if (!multipart.is_multipart) {
            init_send_attrs(dpp, sc->cct, rest_obj, src_properties,
                            target.get(), &new_attrs);
        }
...
public:
    ...
    } int operate(const DoutPrefixProvider *dpp) override
    {
        reenter(this) {
            ...
            yield call(new
                       RGWStreamSpliceCR(cct, sc->env->http_manager, in_crf,
                                         out_crf));
            if (retcode < 0) {
...
public:
    ...
    } int operate(const DoutPrefixProvider *dpp) override
    {
        reenter(this) {
            ...
            out_crf->set_multipart(upload_id, part_info.part_num,
                                   part_info.size);
            ...
            if (!(static_cast < RGWAWSStreamPutCRF * >(out_crf.get()))->
                get_etag(petag)) {
                ldpp_dout(dpp,
...
public:
    ...
    } int operate(const DoutPrefixProvider *dpp) override
    {
        reenter(this) {
            ...
            if (retcode == -ENOENT) {
                RGWAWSStreamPutCRF::init_send_attrs(dpp, sc->cct, rest_obj,
                                                    src_properties,
                                                    target.get(), &new_attrs);

                yield call(new
                           RGWAWSInitMultipartCR(sc, target->conn.get(),
                                                 dest_obj, status.obj_size,
                                                 std::move(new_attrs),
                                                 &status.upload_id));
                if (retcode < 0) {
                    return set_cr_error(retcode);
...
#define MULTIPART_MAX_PARTS 10000
                ...
                status.num_parts =
                    (obj_size + status.part_size - 1) / status.part_size;
                status.cur_part = 1;
            }
            ...
            yield call(new
                       RGWAWSCompleteMultipartCR(sc, target->conn.get(),
                                                 dest_obj, status.upload_id,
                                                 status.parts));
            if (retcode < 0) {
                ldpp_dout(dpp,
                          0) <<
...
public:
    ...
    int operate(const DoutPrefixProvider *dpp) override
    {
        reenter(this) {
            ...
            instance.conf.get_target(target, sync_pipe.dest_bucket_info, key,
                                     &dest_obj.bucket.name, &dest_obj.key.name);

            if (bucket_created.find(dest_obj.bucket.name) ==
                bucket_created.end()) {
                yield {
                    ldpp_dout(dpp,
            ...
            yield {
                ...
                } else
                {
                    ...
                    call(new
                         RGWAWSStreamObjToCloudMultipartCR(sc, sync_pipe,
                                                           instance.conf,
                                                           source_conn, src_obj,
                                                           target, dest_obj,
                                                           size, src_properties,
