...
m_stats_show_secs(5), m_stats_total_written(0), m_stats_begin()
{
    int err = 0;
...
void WorkloadGenerator::do_pgmeta_omap_set(ObjectStore::Transaction * t,
                                           spg_t pgid, coll_t coll,
                                           C_StatState * stat)
{
    ...
    if (!size)
        size =
            get_random_byte_amount(min_xattr_coll_bytes, max_xattr_coll_bytes);
                                      ...
                                      coll_entry_t * entry, C_StatState * stat)
{
    ...
    size_t size =
        (m_write_pglog_bytes ? m_write_pglog_bytes : log_append_bytes);
                                              ...
                                              C_StatState * stat)
{
    ...
    dout(2) << __func__ << " id " << entry->m_id << " coll " << entry->
        m_coll << dendl;
    t->create_collection(entry->m_coll, 32);
    dout(2) << __func__ << " meta " << coll_t::meta() << "/" << entry->
        m_meta_obj << dendl;
