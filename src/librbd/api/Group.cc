...
snap_t get_group_snap_id(I *ictx,
                         const cls::rbd::
                         SnapshotNamespace &in_snap_namespace)
{
...
int group_snap_list(librados::IoCtx &group_ioctx,
                    const char *group_name,
                    std::vector < cls::rbd::GroupSnapshot >
                    *cls_snaps)
{
    ...
    int r =
        cls_client::dir_get_id(&group_ioctx, RBD_GROUP_DIRECTORY,
                               group_name, &group_id);
    if (r < 0) {
    ...
    for (;;) {
        ...
        r = cls_client::group_snap_list(&group_ioctx,
                                        group_header_oid, snap_last,
                                        max_read, &snaps_page);
        ...
        cls_snaps->insert(cls_snaps->end(), snaps_page.begin(),
                          snaps_page.end());
                     ...
                     *image_ids)
{
    ...
    do {
        ...
        r = cls_client::group_image_list(&group_ioctx,
                                         group_header_oid,
                                         start_last, max_read,
                                         &image_ids_page);
...
int group_image_remove(librados::IoCtx &group_ioctx,
                       string group_id,
                       ...
                       string image_id)
{
    ...
    int r =
        cls_client::group_image_set(&group_ioctx, group_header_oid,
                                    incomplete_st);
    ...
    r = cls_client::image_group_remove(&image_ioctx,
                                       image_header_oid,
                                       group_spec);
    if ((r < 0) && (r != -ENOENT)) {
        ...
        ImageWatcher <>::notify_header_update(image_ioctx,
                                              image_header_oid);
    }

    r = cls_client::group_image_remove(&group_ioctx,
                                ...
                                string &group_header_oid)
{
    ...
    for (int i = 0; i < snap_count; ++i) {
        ...
        r = util::create_ioctx(group_ioctx, "image",
        group_snap.snaps[i].pool, {
        }, &image_io_ctx);
        if (r < 0) {
            return r;
        }
        ...
        librbd::ImageCtx *image_ctx =
            new ImageCtx("", group_snap.snaps[i].image_id, nullptr,
                         image_io_ctx, false);
    ...
    for (int i = 0; i < snap_count; ++i) {
        ...
        if (r >= 0) {
            ldout(cct,
                  20) << "removing individual snapshot from image "
                      << ictx->name << dendl;
            ictx->operations->snap_remove(ne, snap_name,
                                          on_finishes[i]);
        } else {
    ...
    ldout(cct,
          20) <<
              "Removed images snapshots removing snapshot record." <<
              dendl;

    r = cls_client::group_snap_remove(&group_ioctx,
                                      group_header_oid,
                                      group_snap.id);
    if (r < 0) {
        ret_code = r;
                                  ...
                                  ProgressContext &pctx)
{
    ...
    for (int i = 0; i < snap_count; ++i) {
        ...
        if (r >= 0) {
            ldout(cct,
                  20) <<
                      "rolling back to individual snapshot for image " <<
                      ictx->name << dendl;
            ictx->operations->execute_snap_rollback(ne, snap_name,
                                                    pctx,
                                                    on_finishes[i]);
        } else {
    ...
    for (int i = 0; i < snap_count; ++i) {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(cct) <<
                       "Failed rolling back group to snapshot. Ret code: "
                       << r << dendl;
            ret_code = r;
        }
    }
                      ...
                      &requests)
{
    ...
    for (int i = 0; i < image_count; ++i) {
        ...
        ictx->image_watcher->notify_unquiesce(requests[i],
                                              &on_finishes[i]);
    }
...
template < typename I >
int notify_quiesce(std::vector < I * > &ictxs,
                   ...
                   std::vector < uint64_t > *requests)
{
    ...
    for (int i = 0; i < image_count; ++i) {
        ...
        ictx->image_watcher->notify_quiesce(&(*requests)[i],
                                            prog_ctx,
                                            &on_finishes[i]);
    }
                                    ...
                                    const char *image_id)
{
    ...
    return group_image_remove(group_ioctx, group_id, image_ioctx,
                              string(image_id));
}

template < typename I >
int Group < I >::create(librados::IoCtx &io_ctx,
                        const char *group_name)
{
    ...
    int r =
        cls_client::group_dir_add(&io_ctx, RBD_GROUP_DIRECTORY,
                                  group_name,
                                  id);
    if (r < 0) {
    ...
    if (r < 0) {
        lderr(cct) << "error creating group header: " << cpp_strerror(r)
                   << dendl;
...
err_remove_from_dir:
    int remove_r =
        cls_client::group_dir_remove(&io_ctx, RBD_GROUP_DIRECTORY,
                                     group_name, id);
    if (remove_r < 0) {
        lderr(cct) << "error cleaning up group from rbd_directory "
                   << "object after creation failed: " <<
                   cpp_strerror(remove_r)
                   << dendl;
    }
                        ...
                        const char *group_name)
{
    CephContext *cct((CephContext *) io_ctx.cct());
    ldout(cct,
          20) << "group_remove " << &io_ctx << " " << group_name <<
              dendl;
    ...
    for (auto &snap : snaps) {
        r = group_snap_remove_by_record(io_ctx, snap, group_id,
                                        group_header_oid);
        if (r < 0) {
            return r;
        }
    ...
    for (auto image : images) {
        ...
        r = group_image_remove(io_ctx, group_id, image_ioctx,
                               image.spec.image_id);
        if (r < 0 && r != -ENOENT) {
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "error removing header: " << cpp_strerror(-r) <<
                   dendl;
        return r;
    }
...
int Group < I >::list(IoCtx &io_ctx, vector < string > *names)
{
    ...
    do {
        ...
        r = cls_client::group_dir_list(&io_ctx, RBD_GROUP_DIRECTORY,
                                       last_read, max_read, &groups);
        if (r < 0) {
            if (r != -ENOENT) {
...
template < typename I >
int Group < I >::image_add(librados::IoCtx &group_ioctx,
                           const char *group_name,
                           librados::IoCtx &image_ioctx,
                           const char *image_name)
{
    ...
    if (group_ioctx.get_namespace() != image_ioctx.get_namespace()) {
        lderr(cct) <<
                   "group and image cannot be in different namespaces" <<
                   dendl;
        return -EINVAL;
    }
    ...
    cls::rbd::GroupImageStatus incomplete_st(image_id,
            ...
            cls::rbd::
            GROUP_IMAGE_LINK_STATE_INCOMPLETE);
    cls::rbd::GroupImageStatus attached_st(image_id,
                                           image_ioctx.get_id(),
                                           cls::rbd::
    ...
    r = cls_client::image_group_add(&image_ioctx, image_header_oid,
                                    group_spec);
    if (r < 0) {
        lderr(cct) << "error adding group reference to image: "
                   << cpp_strerror(-r) << dendl;
        cls::rbd::GroupImageSpec spec(image_id, image_ioctx.get_id());
        cls_client::group_image_remove(&group_ioctx, group_header_oid,
                                       spec);
                              ...
                              const char *image_name)
{
    ...
    r = group_image_remove(group_ioctx, group_id, image_ioctx,
                           image_id);
                            ...
                            *images)
{
    ...
    for (auto image_id : image_ids) {
        IoCtx ioctx;
        int r =
            util::create_ioctx(group_ioctx, "image",
                               image_id.spec.pool_id, { },
                               &ioctx);
        if (r < 0) {
            return r;
        }
        ...
        images->push_back(group_image_info_t {
            image_name,
            ioctx.get_id(),
                 static_cast < group_image_state_t >
                 (image_id.state)}
                         );
    }
...
template < typename I >
int Group < I >::rename(librados::IoCtx &io_ctx,
                        const char *src_name,
...
template < typename I >
int Group < I >::image_get_group(I *ictx,
                                 group_info_t *group_info)
{
    ...
    if (RBD_GROUP_INVALID_POOL != ictx->group_spec.pool_id) {
        IoCtx ioctx;
        r = util::create_ioctx(ictx->md_ctx, "group",
                             ...
                             uint32_t flags)
{
    ...
    int r =
        util::snap_create_flags_api_to_internal(cct, flags,
            &internal_flags);
    if (r < 0) {
        return r;
    }
    ...
    ldout(cct,
          20) << "Found " << image_count << " images in group" << dendl;
    ...
    r = cls_client::group_snap_set(&group_ioctx, group_header_oid,
                                   group_snap);
    if (r == -EEXIST) {
    ...
    for (auto image : images) {
        ...
        ldout(cct,
              20) << "Opening image with id " << image.spec.
                  image_id << dendl;

        librbd::ImageCtx *image_ctx =
            new ImageCtx("", image.spec.image_id.c_str(), nullptr,
                         image_io_ctx, false);
    ...
    ldout(cct,
          20) << "Issued open request waiting for the completion" <<
              dendl;
    ret_code = 0;
    for (int i = 0; i < image_count; ++i) {
    ...
    if ((flags & RBD_SNAP_CREATE_SKIP_QUIESCE) == 0) {
        ...
        if (ret_code != 0
            && (flags & RBD_SNAP_CREATE_IGNORE_QUIESCE_ERROR) == 0) {
            goto remove_record;
        }
    }
    ...
    ind_snap_name =
        calc_ind_image_snap_name(group_ioctx.get_id(), group_id,
                                 group_snap.id);
    ...
    for (int i = 0; i < image_count; ++i) {
        ...
        std::shared_lock owner_locker {
            ictx->owner_lock};
        ictx->operations->execute_snap_create(ne, ind_snap_name.c_str(),
                                              on_finish, 0,
    ...
    for (int i = 0; i < image_count; ++i) {
        ...
        if (r < 0) {
            ...
            if (snap_id == CEPH_NOSNAP) {
                ldout(cct,
                      20) <<
                          "Couldn't find created snapshot with namespace: " <<
                          ne << dendl;
                ret_code = -ENOENT;
            } else {
...
remove_image_snaps:
    ...
    for (int i = 0; i < image_count; ++i) {
        ...
        ictx->image_lock.unlock_shared();
        if (r >= 0) {
            ictx->operations->snap_remove(ne, snap_name.c_str(),
                                          on_finishes[i]);
        } else {
...
template < typename I >
int Group < I >::snap_remove(librados::IoCtx &group_ioctx,
                             const char *group_name,
                             const char *snap_name)
{
    CephContext *cct = (CephContext *) group_ioctx.cct();
...
template < typename I >
int Group < I >::snap_rename(librados::IoCtx &group_ioctx,
                             const char *group_name,
...
template < typename I >
int Group < I >::snap_list(librados::IoCtx &group_ioctx,
                           const char *group_name,
                           std::vector < group_snap_info_t >
                           *snaps)
{
    ...
    for (auto snap : cls_snaps) {
        snaps->push_back(group_snap_info_t {
            snap.name,
            static_cast < group_snap_state_t >
            (snap.state)}
                        );
                               ...
                               ProgressContext &pctx)
{
    ...
    string group_header_oid = util::group_header_name(group_id);
    r = group_snap_rollback_by_record(group_ioctx, *group_snap,
                                      group_id, group_header_oid, pctx);
    return r;
}
