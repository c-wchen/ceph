...
class MMonProbe final:public Message {
  public:
    ...
    MMonProbe()
  :    Message {
    MSG_MON_PROBE, HEAD_VERSION, COMPAT_VERSION} {
    }
    MMonProbe(const uuid_d & f, int o, const std::string & n, bool hej,
              ceph_release_t mr)
    :Message {
    MSG_MON_PROBE, HEAD_VERSION, COMPAT_VERSION},
        fsid(f),
        ...
        has_ever_joined(hej), required_features(0), mon_release {
    mr} {
    }
  private:
    ~MMonProbe()final {
