...
//   - etc.
    ...
    [[noreturn]] void abort_protocol() {
        throw std::
            system_error(make_error_code
                         (crimson::net::error::protocol_aborted));
    }
...
#define ABORT_IN_CLOSE(is_dispatch_reset) { \
  ...
  abort_protocol();                         \
    ...
    inline uint64_t generate_client_cookie() {
        return ceph::util::generate_random_number < uint64_t > (1,
                                                                std::
                                                                numeric_limits <
                                                                uint64_t >::
                                                                max());
...
// should be consistent to intercept_frame() in FrameAssemblerV2.cc
    void intercept(Breakpoint bp,
                   ...
                   Interceptor * interceptor, SocketRef & socket) {
        if (interceptor) {
...
#define INTERCEPT_CUSTOM(bp, type)
    ...
    seastar::future <> ProtocolV2::Timer::backoff(double seconds) {
        ...
        return seastar::sleep_abortable(dur,
                                        *as).
  ...
  :    conn {
    conn}, messenger {
    conn.messenger}, io_handler {
    io_handler}, frame_assembler {
    FrameAssemblerV2::create(conn)}, auth_meta {
    seastar::make_lw_shared < AuthConnectionMeta > ()}, protocol_timer {
    conn}
    {
    }
    ...
    void ProtocolV2::start_connect(const entity_addr_t & _peer_addr,
        ...
        logger().
            info
            ("{} ProtocolV2::start_connect(): peer_addr={}, peer_name={}, cc={}"
             " policy(lossy={}, server={}, standby={}, resetcheck={})", conn,
             _peer_addr, _peer_name, client_cookie, conn.policy.lossy,
             conn.policy.server, conn.policy.standby, conn.policy.resetcheck);
        messenger.register_conn(seastar::static_pointer_cast <
                                SocketConnection > (conn.shared_from_this()));
        execute_connecting();
    }
    ...
    void ProtocolV2::start_accept(SocketRef && new_socket,
        ...
        logger().info("{} ProtocolV2::start_accept(): target_addr={}", conn,
                      _peer_addr);
        messenger.accept_conn(seastar::static_pointer_cast < SocketConnection >
    ...
    void ProtocolV2::fault(state_t expected_state,
                           const char *where, std::exception_ptr eptr) {
        ...
        if (state != expected_state) {
            logger().
                info("{} protocol {} {} is aborted at inconsistent {} -- {}",
                     conn, get_state_name(expected_state), where,
        ...
        if (conn.policy.server ||
            ...
            if (conn.policy.server) {
                logger().
                    info
                    ("{} protocol {} {} fault as server, going to STANDBY {} -- {}",
                     conn, get_state_name(state), where, io_stat_printer {
                     io_handler}
                     , e_what);
            }
            else {
            ...
            logger().
                info("{} protocol {} {} fault, going to CONNECTING {} -- {}",
                     conn, get_state_name(state), where, io_stat_printer {
                     io_handler}
                     , e_what);
            execute_connecting();
    ...
    seastar::future < std::tuple < entity_type_t, entity_addr_t >>
        ...
        return frame_assembler->write_flush(std::move(bl)).then([this] {
                                                                // 2. read peer banner
                                                                unsigned banner_len = strlen(CEPH_BANNER_V2_PREFIX) + sizeof(ceph_le16); INTERCEPT_CUSTOM(custom_bp_t::BANNER_READ, bp_type_t::READ); return frame_assembler->read_exactly(banner_len); // or read exactly?
                                                                }
        ).then([this] (auto bl) {
               ...
               logger().debug("{} RECV({}) banner: \"{}\"",
                              conn, bl.size(),
                              std::string((const char *)bl.get(),
                                          banner_prefix_len));
               if (memcmp(bl.get(), CEPH_BANNER_V2_PREFIX, banner_prefix_len) !=
                   0) {
               ...
               auto ti = buf.cbegin(); try {
               decode(payload_len, ti);}
               catch(const buffer::error & e) {
               logger().warn("{} decode banner payload len failed", conn);
               abort_in_fault();}
               logger().debug("{} GOT banner: payload_len={}", conn,
                              payload_len);
               INTERCEPT_CUSTOM(custom_bp_t::BANNER_PAYLOAD_READ,
                                bp_type_t::READ);
            ...
            .then([this, is_connect] (bufferlist bl) {
                  ...
                  uint64_t _peer_required_features; try {
                  ...
                  abort_in_fault();}
                  logger().
                  debug("{} RECV({}) banner features: supported={} required={}",
                  ...
                  if ((required_features & _peer_supported_features) !=
                      required_features) {
                  logger().
                  error("{} peer does not support all required features"
                        " required={} peer_supported={}", conn,
                        required_features, _peer_supported_features);
                  ABORT_IN_CLOSE(is_connect);}
                  if ((supported_features & _peer_required_features) !=
                      _peer_required_features) {
                  logger().
                  error("{} we do not support all peer required features"
                        " peer_required={} supported={}", conn,
                        _peer_required_features, supported_features);
                  ABORT_IN_CLOSE(is_connect);}
                  ...
                  logger().
                  debug("{} WRITE HelloFrame: my_type={}, peer_addr={}", conn,
            ...
            .then([this] {
               ...
               logger().debug("{} GOT HelloFrame: my_type={} peer_addr={}",
                              ...
                              hello.peer_addr());
               return seastar::make_ready_future < std::tuple < entity_type_t,
               entity_addr_t >> (std::
                                 make_tuple(hello.entity_type(),
                                            hello.peer_addr()));}
        );
    }
...
case Tag::AUTH_BAD_METHOD:
                                                          ...
                                                          then([this]
                                                               ...
                                                               logger().
                                                               warn
                                                               ("{} auth_client handle_auth_bad_method returned {}",
                                                                conn, r);
                                                               abort_in_fault
                                                               ();}
                                                               return
                                                          ...
                                                          then([this]
                                                               ...
                                                               logger().
                                                               debug
                                                               ("{} GOT AuthReplyMoreFrame: payload_len={}",
                                                                conn,
                                                                auth_more.
                                                                auth_payload().
                                                                length());
                                                               ...
                                                               logger().
                                                               debug
                                                               ("{} WRITE AuthRequestMoreFrame: payload_len={}",
                                                                conn,
                                                                reply.length());
                                                               return
                                                               frame_assembler->
                                                          ...
                                                          then([this]
                                                               ...
                                                               logger().
                                                               debug
                                                               ("{} GOT AuthDoneFrame: gid={}, con_mode={}, payload_len={}",
                                                                conn,
                                                                auth_done.
                                                                global_id(),
                                                                ceph_con_mode_name
                                                               ...
                                                               logger().
                                                               warn
                                                               ("{} auth_client handle_auth_done returned {}",
                                                                conn, r);
                                                               abort_in_fault
                                                               ();}
                                                               auth_meta->
                                                               ...
                                                               con_mode();
                                                               frame_assembler->
                                                               create_session_stream_handlers
                                                               (*auth_meta,
                                                                false);
                                                               return
                                                               finish_auth();}
    ...
    seastar::future <> ProtocolV2::client_auth(std::vector < uint32_t >
                                               &allowed_methods) {
        ...
        try {
            ...
            auto frame =
                AuthRequestFrame::Encode(auth_method, preferred_modes, bl);
            logger().
                debug("{} WRITE AuthRequestFrame: method={},"
                      " preferred_modes={}, payload_len={}", conn, auth_method,
                      preferred_modes, bl.length());
            ...
            logger().error("{} get_initial_auth_request returned {}", conn,
                           e.what());
            ABORT_IN_CLOSE(true);
            return seastar::now();
        }
...
case Tag::IDENT_MISSING_FEATURES:
               ...
               then([this] (auto payload) {
                    ...
                    logger().
                    warn("{} GOT IdentMissingFeaturesFrame: features={}"
                         " (client does not support all server features)", conn,
                         ident_missing.features()); abort_in_fault();
                    return next_step_t::none;}
); case Tag::WAIT:
return process_wait(); case Tag::SERVER_IDENT:
               return frame_assembler->read_frame_payload().
               then([this] (auto payload) {
                    // handle_server_ident() logic
                    io_handler.requeue_out_sent();
                    auto server_ident =
                    ServerIdentFrame::Decode(payload->back());
                    logger().
                    debug("{} GOT ServerIdentFrame:" " addrs={}, gid={}, gs={},"
                          " features_supported={}, features_required={},"
                          " flags={}, cookie={}", conn, server_ident.addrs(),
                          server_ident.gid(), server_ident.global_seq(),
                    ...
                    logger().
                    warn("{} peer identifies as {}, does not include {}", conn,
                         server_ident.addrs(), conn.target_addr);
                    throw std::
                    system_error(make_error_code
                                 (crimson::net::error::bad_peer_address));}
                    ...
                    logger().
                    error("{} connection peer id ({}) does not match "
                          "what it should be ({}) during connecting, close",
                          conn, server_ident.gid(), conn.get_peer_id());
                    ABORT_IN_CLOSE(true);}
                    conn.set_peer_id(server_ident.gid());
                    conn.set_features(server_ident.supported_features() & conn.
                                      policy.features_supported);
                    logger().debug("{} UPDATE: features={}", conn,
                                   conn.get_features());
                    peer_global_seq = server_ident.global_seq();
                    bool lossy = server_ident.flags() & CEPH_MSG_CONNECT_LOSSY;
                    if (lossy != conn.policy.lossy) {
                    logger().
                    warn("{} UPDATE Policy(lossy={}) from server flags", conn,
                         lossy); conn.policy.lossy = lossy;}
                    if (lossy && (connect_seq != 0 || server_cookie != 0)) {
                    logger().
                    warn("{} UPDATE cs=0({}) sc=0({}) for lossy policy", conn,
                         connect_seq, server_cookie); connect_seq = 0;
                    server_cookie = 0;}

                    return seastar::make_ready_future < next_step_t >
                    (next_step_t::ready);}
); default:   {
...
case Tag::SESSION_RETRY_GLOBAL:
               ...
               then([this] (auto payload) {
                    ...
                    logger().warn("{} UPDATE: gs={} for retry global", conn,
                                  global_seq); return client_reconnect();}
               ...
               then([this] (auto payload) {
                    ...
                    logger().warn("{} GOT ResetFrame: full={}", conn,
                                  reset.full()); reset_session(reset.full());
                    return client_connect();}
); case Tag::WAIT:
return process_wait(); case Tag::SESSION_RECONNECT_OK:
               return frame_assembler->read_frame_payload().
               then([this] (auto payload) {
                    // handle_reconnect_ok() logic
                    auto reconnect_ok =
                    ReconnectOkFrame::Decode(payload->back());
                    logger().debug("{} GOT ReconnectOkFrame: msg_seq={}", conn,
    ...
    void ProtocolV2::execute_connecting() {
        ...
        gated_execute("execute_connecting", conn,[this] {
                      ...
                      logger().debug("{} UPDATE: gs={} for connect", conn,
                                     global_seq);}
...
return seastar::now(); case bp_action_t::FAULT:
logger().info("[Test] got FAULT"); abort_in_fault(); case bp_action_t::BLOCK:
logger().info("[Test] got BLOCK"); return conn.interceptor->blocker.block(); default:
                                                 ceph_abort
                                                 ("unexpected action from trap");}
                                                 }
                                                 else {
                                                 return seastar::now();}
                                                 }
                             ...
                             gate.
                             dispatch_in_background("replace_socket_connecting",
                                                    conn,[this, new_socket =
                                                          std::
                                                          move(new_socket)]
                                                    ()mutable {
                             ...
                             is_socket_valid = true; return seastar::now();}
                      ).then([this] {
                             auth_meta =
                             seastar::make_lw_shared < AuthConnectionMeta > ();
                             frame_assembler->reset_handlers();
                             frame_assembler->start_recording();
                             return banner_exchange(true);}
                      ).then([this] (auto && ret) {
                             auto[_peer_type, _my_addr_from_peer] =
                             std::move(ret);
                             ...
                             logger().
                             debug
                             ("{} triggered {} during banner_exchange(), abort",
                              conn, get_state_name(state)); abort_protocol();}
                             frame_assembler->
                             learn_socket_ephemeral_port_as_connector
                             (_my_addr_from_peer.get_port());
                             if (unlikely(_my_addr_from_peer.is_legacy())) {
                             logger().
                             warn("{} peer sent a legacy address for me: {}",
                             ...
                             logger().
                             debug
                             ("{} triggered {} at the end of execute_connecting()",
                              conn, get_state_name(state)); abort_protocol();}
                             switch (next) {
case next_step_t::ready:    {
                             logger().
                             info("{} connected: gs={}, pgs={}, cs={}, "
                                  "client_cookie={}, server_cookie={}, {}",
                                  conn, global_seq, peer_global_seq,
                                  connect_seq, client_cookie, server_cookie,
                                  io_stat_printer {
                                  io_handler}
                             );
                             ...
                             logger().
                             debug
                             ("{} triggered {} after ms_handle_connect(), abort",
                              conn, get_state_name(state)); abort_protocol();}
                             execute_ready(); break;}
case next_step_t::wait:     {
                             logger().
                             info
                             ("{} execute_connecting(): going to WAIT(max-backoff)",
                              conn); ceph_assert_always(is_socket_valid);
                             frame_assembler->shutdown_socket();
                             is_socket_valid = false; execute_wait(true);
                             break;}
default:                    {
...
// ACCEPTING state
    ...
    seastar::future <> ProtocolV2::_auth_bad_method(int r) {
        ...
        auto[allowed_methods, allowed_modes] =
            messenger.get_auth_server()->get_supported_auth_methods(conn.
                                                                    get_peer_type
                                                                    ());
        auto bad_method =
            AuthBadMethodFrame::Encode(auth_meta->auth_method, r,
                                       allowed_methods, allowed_modes);
        logger().
            warn("{} WRITE AuthBadMethodFrame: method={}, result={}, "
                 "allowed_methods={}, allowed_modes={})", conn,
                 auth_meta->auth_method, cpp_strerror(r), allowed_methods,
                 allowed_modes);
    ...
    seastar::future <> ProtocolV2::
        _handle_auth_request(bufferlist & auth_payload, bool more) {
        // _handle_auth_request() logic
        ceph_assert(messenger.get_auth_server());
        bufferlist reply;
        int r = messenger.get_auth_server()->handle_auth_request(conn,
                                                                 *auth_meta,
                                                                 more,
                                                                 auth_meta->
                                                                 auth_method,
        ...
        case 1:{
                auto auth_done =
                    AuthDoneFrame::Encode(conn.peer_global_id,
                                          auth_meta->con_mode, reply);
                logger().
                    debug
                    ("{} WRITE AuthDoneFrame: gid={}, con_mode={}, payload_len={}",
        ...
        case 0:{
                ...
                return frame_assembler->write_flush_frame(more).then([this] {
                       ...
                       expect_tag(Tag::AUTH_REQUEST_MORE, ret.tag, conn,
                                  "read_auth_request_more");
                       return frame_assembler->read_frame_payload();}
                ).then([this] (auto payload) {
                       auto auth_more =
                       AuthRequestMoreFrame::Decode(payload->back());
                       logger().
                       debug("{} GOT AuthRequestMoreFrame: payload_len={}",
                             conn, auth_more.auth_payload().length());
                       return _handle_auth_request(auth_more.auth_payload(),
                                                   true);}
                );
            }
        case -EBUSY:{
                logger().
                    warn("{} auth_server handle_auth_request returned -EBUSY",
                         conn);
                abort_in_fault();
                return seastar::now();
            }
        default:{
                logger().warn("{} auth_server handle_auth_request returned {}",
                              conn, r);
    ...
    seastar::future <> ProtocolV2::server_auth() {
        return frame_assembler->read_main_preamble().then([this] (auto ret) {
               ...
               logger().
               debug("{} GOT AuthRequestFrame: method={}, preferred_modes={},"
                     " payload_len={}", conn, request.method(),
               ...
               logger().
               warn
               ("{} auth_server pick_con_mode returned mode CEPH_CON_MODE_UNKNOWN",
                conn); return _auth_bad_method(-EOPNOTSUPP);}
    ...
    seastar::future < ProtocolV2::next_step_t >
        ProtocolV2::reuse_connection(ProtocolV2 * existing_proto, bool do_reset,
    ...
    seastar::future < ProtocolV2::next_step_t >
        ...
        if (!validate_peer_name(existing_conn->get_peer_name())) {
            logger().error("{} server_connect: my peer_name doesn't match"
                           " the existing connection {}, abort", conn,
                           fmt::ptr(existing_conn.get()));
            abort_in_fault();
        }
        ...
        if (existing_proto->server_cookie != 0) {
            if (existing_proto->client_cookie != client_cookie) {
                ...
                logger().
                    ...
                    ("{} server_connect: found client session with existing {} {}"
                     ...
                     conn, get_state_name(existing_proto->state),
                     *existing_conn, client_cookie,
                     existing_proto->server_cookie);
            ...
            if (existing_proto->client_cookie != client_cookie) {
                if (existing_conn->peer_wins()) {
                    ...
                    logger().
                        warn
                        ("{} server_connect: connection race detected (cs={}, e_cs={}, ss=0)"
                         " and win, reusing existing {} {}", conn,
                         client_cookie, existing_proto->client_cookie,
                    ...
                    logger().
                        warn
                        ("{} server_connect: connection race detected (cs={}, e_cs={}, ss=0)"
                         " and lose to existing {}, ask client to wait", conn,
                         client_cookie, existing_proto->client_cookie,
                         *existing_conn);
    ...
    seastar::future < ProtocolV2::next_step_t > ProtocolV2::server_connect() {
        return frame_assembler->read_frame_payload().then([this] (auto payload) {
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} oops, client_ident.addrs() is empty",
                                                           conn);
                                                          throw std::
                                                          system_error
                                                          (make_error_code
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} peer is trying to reach {} which is not us ({})",
                                                           conn,
                                                           client_ident.
                                                           target_addr(),
                                                           messenger.
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} we don't know how to reconnect to peer {}",
                                                           conn,
                                                           conn.target_addr);
                                                          throw std::
                                                          system_error
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} WRITE IdentMissingFeaturesFrame: features={} (peer missing)",
                                                           conn, feat_missing);
                                                          return
                                                          frame_assembler->
                                                          write_flush_frame
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} my lossy policy {} doesn't match client {}, ignore",
                                                           conn,
                                                           conn.policy.lossy,
                                                           lossy);}
                                                          ...
                                                          logger().
                                                          debug
                                                          ("{} triggered {} before execute_establishing()",
                                                           conn,
                                                           get_state_name
                                                           (state));
                                                          abort_protocol();}
    ...
    seastar::future < ProtocolV2::next_step_t > ProtocolV2::server_reconnect() {
        return frame_assembler->read_frame_payload().then([this] (auto payload) {
                                                          ...
                                                          logger().
                                                          debug
                                                          ("{} GOT ReconnectFrame: addrs={}, client_cookie={},"
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} server_reconnect: I was a client (cc={}) and didn't received the"
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} server_reconnect: stale peer connect_seq peer_cs({}) < exist_cs({}),"
                                                          ...
                                                          logger().
                                                          warn
                                                          ("{} server_reconnect: stale exsiting connect_seq exist_cs({}) < peer_cs({}),"
                                                           " reusing existing {} {}",
                                                           conn,
                                                           existing_proto->
                                                           connect_seq,
    ...
    void ProtocolV2::execute_accepting() {
        ...
        gate.dispatch_in_background("execute_accepting", conn,[this] {
                                    return seastar::futurize_invoke([this] {
                                                                    ...
                                                                    (action) {
case bp_action_t::CONTINUE:
break; case bp_action_t::FAULT:
logger().info("[Test] got FAULT"); abort_in_fault(); default:
                                                                    ceph_abort
                                                                    ("unexpected action from trap");}
                                                                    }
#endif
                                           ...
                                           conn.policy =
                                           messenger.get_policy(_peer_type);
                                           logger().
                                           info("{} UPDATE: peer_type={},"
                                                " policy(lossy={} server={} standby={} resetcheck={})",
                                                conn,
                                                ceph_entity_type_name
                                                (_peer_type), conn.policy.lossy,
                                                conn.policy.server,
                                                conn.policy.standby,
                                                conn.policy.resetcheck);
                                           if (!messenger.get_myaddr().
                                               is_blank_ip()
...
return server_connect(); case Tag::SESSION_RECONNECT:
return server_reconnect(); default:       {
                                           unexpected_tag(ret.tag, conn,
                                                          "post_server_auth");
                                           return seastar::make_ready_future <
                                           next_step_t > (next_step_t::none);}
                                           }
                                           }
                                    ).then([this] (next_step_t next) {
                                           switch (next) {
case next_step_t::ready:
assert(state != state_t::ACCEPTING); break; case next_step_t::wait:
                                           if (unlikely
                                               (state != state_t::ACCEPTING)) {
                                           logger().
                                           debug
                                           ("{} triggered {} at the end of execute_accepting()",
                                            conn, get_state_name(state));
                                           abort_protocol();}
logger().info("{} execute_accepting(): going to SERVER_WAIT", conn); execute_server_wait(); break; default:
                                           ceph_abort
                                           ("impossible next step");}
                                           }
                                    ).
                                    handle_exception([this]
                                                     (std::exception_ptr eptr) {
                                                     const char *e_what; try {
                                                     std::
...
// CONNECTING or ACCEPTING state
    ...
    seastar::future <> ProtocolV2::finish_auth() {
        ...
        return frame_assembler->write_flush_frame(sig_frame).then([this] {
               ...
               expect_tag(Tag::AUTH_SIGNATURE, ret.tag, conn,
                          "post_finish_auth");
               return frame_assembler->read_frame_payload();}
               ...
               logger().debug("{} GOT AuthSignatureFrame: signature={}", conn,
                              sig_frame.signature());
               const auto actual_tx_sig =
               auth_meta->session_key.empty()? sha256_digest_t() : auth_meta->
               session_key.hmac_sha256(nullptr, txbuf);
               if (sig_frame.signature() != actual_tx_sig) {
               logger().
               warn("{} pre-auth signature mismatch actual_tx_sig={}"
                    " sig_frame.signature()={}", conn, actual_tx_sig,
                    sig_frame.signature()); abort_in_fault();}
               }
        ) ;
    }
    ...
    void ProtocolV2::execute_establishing(SocketConnectionRef existing_conn) {
        auto accept_me =[this] {
            messenger.register_conn(seastar::static_pointer_cast <
                                    SocketConnection >
                                    (conn.shared_from_this()));
            messenger.unaccept_conn(seastar::static_pointer_cast <
                                    SocketConnection >
                                    (conn.shared_from_this()));
        };
        ...
        if (existing_conn) {
            static_cast <
                ...
                do_close(true /* is_dispatch_reset */ , std::move(accept_me));
        ...
        if (unlikely(state != state_t::ESTABLISHING)) {
            logger().
                debug
                ("{} triggered {} after ms_handle_accept() during execute_establishing()",
                 conn, get_state_name(state));
            abort_protocol();
        }
        ...
        gated_execute("execute_establishing", conn,[this] {
                      return seastar::futurize_invoke([this] {
                             ...
                             logger().
                             debug
                             ("{} triggered {} at the end of execute_establishing()",
                              conn, get_state_name(state)); abort_protocol();}
                             logger().
                             info("{} established: gs={}, pgs={}, cs={}, "
                                  "client_cookie={}, server_cookie={}, {}",
                                  conn, global_seq, peer_global_seq,
                                  connect_seq, client_cookie, server_cookie,
                                  io_stat_printer {
...
// ESTABLISHING or REPLACING state
    ...
    seastar::future <> ProtocolV2::send_server_ident() {
        ...
        if (!conn.policy.lossy) {
            server_cookie =
                ceph::util::generate_random_number < uint64_t > (1, -1ll);
        }
...
// REPLACING state
    ...
    void ProtocolV2::trigger_replacing(bool reconnect,
        ...
        gate.dispatch_in_background("trigger_replacing",
                                    conn,
                                    [this,
                                     reconnect,
                                     do_reset,
                                     ...
                                     new_peer_global_seq, new_connect_seq,
                                     new_msg_seq] ()mutable {
                                    ceph_assert_always(state ==
                                                       state_t::REPLACING);
                                    io_handler.dispatch_accept();
                                    ...
                                    then([this, reconnect, do_reset, mover =
                                          std::move(mover), new_auth_meta =
                                         ...
                                         ("replace_frame_assembler", conn,
                                          [this, mover =
                                           std::move(mover)] ()mutable {
                                          return frame_assembler->
                                          replace_by(std::move(mover));}
                                         ); is_socket_valid = true;
                                         has_socket = true; if (reconnect) {
                                         ...
                                         logger().
                                         debug
                                         ("{} WRITE ReconnectOkFrame: msg_seq={}",
                                          conn, io_handler.get_in_seq());
                                         return frame_assembler->
                                         write_flush_frame(reconnect_ok);}
                                         else {
                                         client_cookie = new_client_cookie;
                                         assert(conn.get_peer_type() ==
                                                new_peer_name.type());
                                         if (conn.get_peer_id() ==
                                             entity_name_t::NEW) {
                                         ...
                                         bool is_rev1 =
                                         HAVE_MSGR2_FEATURE
                                         (peer_supported_features, REVISION_1);
                                         frame_assembler->set_is_rev1(is_rev1);
                                         return send_server_ident();}
                                           ...
                                           logger().
                                           debug
                                           ("{} triggered {} at the end of trigger_replacing()",
                                            conn, get_state_name(state));
                                           abort_protocol();}
                                           logger().
                                           info
                                           ("{} replaced ({}): gs={}, pgs={}, cs={}, "
                                            ...
                                            "connected", global_seq,
                                            peer_global_seq, connect_seq,
                                            client_cookie, server_cookie,
                                            io_stat_printer {
                                            io_handler}
                                           ); execute_ready();}
...
// READY state
    ...
    void ProtocolV2::notify_out_fault(const char *where,
                                      std::exception_ptr eptr) {
...
// WAIT state
    ...
    void ProtocolV2::execute_wait(bool max_backoff) {
        ...
        gated_execute("execute_wait", conn,[this, max_backoff] {
                      ...
                      if (max_backoff) {
                      backoff =
                      local_conf().get_val < double >("ms_max_backoff");}
                      else
                      if (backoff > 0) {
                      backoff =
                      ...
                      backoff =
                      local_conf().get_val < double >("ms_initial_backoff");}
                      return protocol_timer.backoff(backoff).then([this] {
                                                                  ...
                                                                  logger().
                                                                  debug
                                                                  ("{} triggered {} at the end of execute_wait()",
                                                                   conn,
                                                                   get_state_name
                                                                   (state));
                                                                  abort_protocol
                                                                  ...
                                                                  logger().
                                                                  info
                                                                  ("{} execute_wait(): going to CONNECTING",
                                                                   conn);
                                                                  execute_connecting
                                                                  ();}
                      ).handle_exception([this] (std::exception_ptr eptr) {
                                         const char *e_what; try {
                                         std::
                                         rethrow_exception(eptr);} catch(std::
                                                                         exception
                                                                         & e) {
                                         e_what = e.what();}
                                         logger().
                                         info
                                         ("{} execute_wait(): protocol aborted at {} -- {}",
...
// SERVER_WAIT state
    ...
    void ProtocolV2::execute_server_wait() {
        ...
        gated_execute("execute_server_wait", conn,[this] {
                      ...
                      then([this] (auto bl) {
                           logger().warn("{} SERVER_WAIT got read, abort",
                                         conn); abort_in_fault();}
                      ).handle_exception([this] (std::exception_ptr eptr) {
                                         const char *e_what; try {
                                         std::
...
// CLOSING state
    ...
    seastar::future <> ProtocolV2::close_clean_yielded() {
        ...
        return seastar::yield().then([this, conn_ref = conn.shared_from_this()] {
                                     do_close(false);
    ...
    void ProtocolV2::do_close(bool is_dispatch_reset,
        ...
        // trigger close
        messenger.closing_conn(seastar::static_pointer_cast < SocketConnection >
                               (conn.shared_from_this()));
        if (state == state_t::ACCEPTING || state == state_t::SERVER_WAIT) {
            ...
            messenger.unregister_conn(seastar::static_pointer_cast <
                                      SocketConnection >
                                      (conn.shared_from_this()));
        }
        else {
        ...
        auto io_closed = io_handler.close_io(is_dispatch_reset, is_replace);

        // asynchronous operations
        assert(!closed_clean_fut.valid());
        closed_clean_fut =
            seastar::when_all(std::move(handshake_closed), std::move(io_closed)
            ...
            .then([this] {
                  ...
                  messenger.closed_conn(seastar::static_pointer_cast <
                                        SocketConnection >
                                        (conn.shared_from_this()));
#ifdef UNIT_TESTS_BUILT
                  closed_clean = true; if (conn.interceptor) {
                  conn.interceptor->register_conn_closed(conn);}
#endif
                  }
        )
            .handle_exception([conn_ref =
                               conn.shared_from_this(), this] (auto eptr) {
                              logger().
                              error
                              ("{} closing: closed_clean_fut got unexpected exception {}",
