...
namespace crimson::net {
    ...
    :dispatchers(dispatchers),
        conn(conn), conn_ref(conn.get_local_shared_foreign_from_this()) {
    } IOHandler::~IOHandler() {
        ceph_assert(gate.is_closed());
        assert(!out_exit_dispatching);
    }

    ceph::bufferlist IOHandler::
        sweep_out_pending_msgs_to_sent(bool require_keepalive,
                                       std::optional < utime_t >
                                       maybe_keepalive_ack, bool require_ack) {
    ...
    void IOHandler::mark_down() {
        ...
        logger().info("{} mark_down() with {}", conn, io_stat_printer {
                      *this}
        );
    ...
    void IOHandler::print_io_stat(std::ostream & out) const {
        out << "io_stat(" << "io_state=" << fmt::format("{}", io_state)
        << ", in_seq=" << in_seq
            << ", out_seq=" << out_seq
        ...
        ceph_assert_always(!((new_state == io_state_t::none
                              && io_state != io_state_t::none)
                             || (new_state == io_state_t::open
                                 && io_state == io_state_t::open)
                             || (new_state != io_state_t::drop
                                 && io_state == io_state_t::drop)
                           ));
    ...
    seastar::future < FrameAssemblerV2Ref >
    ...
    void IOHandler::requeue_out_sent_up_to(seq_num_t seq) {
        ...
        if (out_sent_msgs.empty() && out_pending_msgs.empty()) {
            logger().
                debug("{} nothing to requeue, reset out_seq from {} to seq {}",
                      conn, out_seq, seq);
            out_seq = seq;
            return;
        }
        logger().
            debug("{} discarding sent msgs by seq {} (sent_len={}, out_seq={})",
    ...
    seastar::future < stop_t > IOHandler::try_exit_out_dispatch() {
        ...
        return frame_assembler->flush().then([this] {
                                             ...
                                             logger().
                                             info
                                             ("{} do_out_dispatch: nothing queued at {},"
                                              " set out_exit_dispatching", conn,
                                              io_state);}
...
case io_state_t::open:        {
                               ...
                               write(sweep_out_pending_msgs_to_sent
                                     (need_keepalive, next_keepalive_ack,
                                      to_ack > 0)
                               ).
                               then([this, prv_keepalive_ack =
                                     next_keepalive_ack, to_ack] {
                                    need_keepalive = false;
                                    if (next_keepalive_ack ==
                                        prv_keepalive_ack) {
                                    next_keepalive_ack = std::nullopt;}
                                    assert(ack_left >= to_ack);
                                    ack_left -= to_ack; if (!is_out_queued()) {
                                    return try_exit_out_dispatch();}
                                    else {
...
case io_state_t::delay:
                               ...
                               logger().
                               info
                               ("{} do_out_dispatch: delay and set out_exit_dispatching ...",
                                conn);}
                               else {
                               logger().info("{} do_out_dispatch: delay ...",
                                             conn);}
                               return io_state_changed.get_future().then([] {
                                                                         return
                                                                         stop_t::
                                                                         no;}
                               ...
                               logger().
                               info
                               ("{} do_out_dispatch: dropped and set out_exit_dispatching",
                                conn);}
                               else {
                               logger().info("{} do_out_dispatch: dropped",
                                             conn);}
return seastar::make_ready_future < stop_t > (stop_t::yes); default:
                               ceph_assert(false);}
                               }
        )
            .handle_exception_type([this] (const std::system_error & e) {
                                   ...
                                   logger().
                                   error
                                   ("{} do_out_dispatch(): unexpected error at {} -- {}",
                                    conn, io_state, e.what()); ceph_abort();}
        ...
        return frame_assembler->read_frame_payload().
            then([this, throttle_stamp, msg_size] (auto payload) {
                 ...
                 logger().trace("{} got {} + {} + {} byte message,"
                                ...
                                current_header.seq); ceph_msg_header header {
                 current_header.seq,
                 current_header.tid,
                 current_header.type,
                 ...
                 in_seq = message->get_seq(); if (conn.policy.lossy) {
                 logger().debug("{} <== #{} === {} ({})",
                                conn,
                                message->get_seq(),
                 ...
                 // notify ack
                 if (!conn.policy.lossy) {
                 ++ack_left; notify_out_dispatch();}

                 ack_out_sent(current_header.ack_seq);
                 // TODO: change MessageRef with seastar::shared_ptr
                 auto msg_ref = MessageRef {
                 message, false}; assert(io_state == io_state_t::open);
...
case Tag::ACK:
                                                                    ...
                                                                    then([this]
                                                                         ...
                                                                         ().
                                                                         debug
                                                                         ("{} GOT KeepAliveFrameAck: timestamp={}",
                                                                          conn,
                                                                          _last_keepalive_ack);}
); default:                                                        {
                                                                    logger().
                                    ...
                                    handle_exception([this]
                                                     ...
                                                     logger().
                                                     info
                                                     ("{} do_in_dispatch(): fault at {} -- {}",
                                                      conn, io_state, e_what);}
                                                     }
                                    ).finally([this] {
                                              ceph_assert_always
                                              (in_exit_dispatching.has_value());
