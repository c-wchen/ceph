...
TEST_F(TestClsRbd, parents)
{
    ...
    ASSERT_EQ(-ENOENT,
              get_parent(&ioctx, "doesnotexist", CEPH_NOSNAP, &pspec, &size));
    ...
    ASSERT_EQ(-ENOEXEC,
              set_parent(&ioctx, "old", ParentSpec(-1, "parent", 3), 10 << 20));
    ...
    ASSERT_EQ(-EINVAL,
              set_parent(&ioctx, oid, ParentSpec(-1, "parent", 3), 10 << 20));
    ASSERT_EQ(-EINVAL, set_parent(&ioctx, oid, ParentSpec(1, "", 3), 10 << 20));
    ASSERT_EQ(-EINVAL,
              set_parent(&ioctx, oid, ParentSpec(1, "parent", CEPH_NOSNAP),
                         10 << 20));
    ...
    ASSERT_EQ(-EEXIST,
              set_parent(&ioctx, oid, ParentSpec(2, "parent", 34), 10 << 20));
    ...
    ASSERT_EQ(0,
              set_parent(&ioctx, oid, ParentSpec(1, "parent", 3), 100 << 20));
    ASSERT_EQ(0, get_parent(&ioctx, oid, CEPH_NOSNAP, &pspec, &size));
    ASSERT_EQ(pspec.pool_id, 1);
    ASSERT_EQ(pspec.image_id, "parent");
    ASSERT_EQ(pspec.snap_id, snapid_t(3));
...
TEST_F(TestClsRbd, snapshots)
{
    ...
    ASSERT_EQ(0,
              snapshot_timestamp_list(&ioctx, oid, snapc.snaps,
                                      &snap_timestamps));
...
TEST_F(TestClsRbd, object_map_snap_add)
{
    ...
    librados::ObjectWriteOperation op2;
    object_map_update(&op2, 0, 4, OBJECT_NONEXISTENT,
...
TEST_F(TestClsRbd, object_map_snap_remove)
{
    ...
    librados::ObjectWriteOperation op2;
    object_map_update(&op2, 0, 4, OBJECT_EXISTS_CLEAN,
                      boost::optional < uint8_t > ());
    ASSERT_EQ(0, ioctx.operate(oid, &op2));
...
TEST_F(TestClsRbd, mirror)
{
    ...
    ASSERT_EQ(-EINVAL,
              mirror_peer_add(&ioctx, "mirror-uuid", "cluster1", "client"));
    ...
    ASSERT_EQ(-EEXIST,
              mirror_peer_add(&ioctx, "uuid4", "cluster3", "admin", 123));
...
TEST_F(TestClsRbd, mirror_image)
{
    ...
    cls::rbd::MirrorImage image2("uuid2",
...
TEST_F(TestClsRbd, mirror_image_status)
{
    ...
    cls::rbd::MirrorImageStatus status1(cls::rbd::
                                        MIRROR_IMAGE_STATUS_STATE_UNKNOWN);
    cls::rbd::MirrorImageStatus status2(cls::rbd::
                                        MIRROR_IMAGE_STATUS_STATE_REPLAYING);
    cls::rbd::MirrorImageStatus status3(cls::rbd::
    ...
    statuses.clear();
    ASSERT_EQ(0,
              mirror_image_status_list(&ioctx, "", 1024, &images, &statuses));
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        cls::rbd::MirrorImageStatus status(cls::rbd::
                                           MIRROR_IMAGE_STATUS_STATE_UNKNOWN);
...
TEST_F(TestClsRbd, group_dir_list)
{
    ...
    ASSERT_EQ(0,
              group_dir_add(&ioctx, RBD_GROUP_DIRECTORY, group_name1,
                            group_id1));
    ASSERT_EQ(0,
              group_dir_add(&ioctx, RBD_GROUP_DIRECTORY, group_name2,
                            group_id2));
...
{
    ASSERT_EQ(0,
              group_dir_add(&ioctx, RBD_GROUP_DIRECTORY, group_name, group_id));
...
TEST_F(TestClsRbd, group_dir_remove)
{
    ...
    ASSERT_EQ(0,
              group_dir_remove(&ioctx, RBD_GROUP_DIRECTORY, group_name,
                               group_id));
...
TEST_F(TestClsRbd, trash_methods)
{
    ...
    now1_delay += 380;
    cls::rbd::TrashImageSpec trash_spec(cls::rbd::TRASH_IMAGE_SOURCE_USER,
                                        "name", now1, now1_delay);
