...
namespace {
    ...
    static int is_loopback_addr(sockaddr * addr) {
        if (addr->sa_family == AF_INET) {
            ...
            const in_addr_t net =
                ntohl(sin->sin_addr.s_addr) >> IN_CLASSA_NSHIFT;
    ...
    bool matches_with_net(const ifaddrs & ifa,
                          const sockaddr * net,
                          unsigned int prefix_len, unsigned ipv) {
        switch (net->sa_family) {
        case AF_INET:
            if (ipv & CEPH_PICK_ADDRESS_IPV4) {
                return matches_ipv4_in_subnet(ifa, (struct sockaddr_in *)net,
                                              prefix_len);
            }
            break;
        case AF_INET6:
            if (ipv & CEPH_PICK_ADDRESS_IPV6) {
                return matches_ipv6_in_subnet(ifa, (struct sockaddr_in6 *)net,
                                              prefix_len);
            }
            break;
        }
    ...
    bool matches_with_net(CephContext * cct,
                          const ifaddrs & ifa,
                          const std::string & s, unsigned ipv) {
                                              ...
                                              int numa_node)
{
    ...
    if (!ifs.empty() && nets.empty()) {
        lderr(cct) << "interface names specified but not network names" <<
            dendl;
        exit(1);
    }
...
static void fill_in_one_address(CephContext * cct,
                                ...
                                const char *conf_var, int numa_node = -1)
{
    const struct sockaddr *found = find_ip_in_subnet_list(cct,
                                                          ifa,
    ...
    err = getnameinfo(found, (found->sa_family == AF_INET)
...
void pick_addresses(CephContext * cct, int needs)
{
    ...
    if (r < 0) {
        ...
        lderr(cct) << "unable to fetch interfaces and addresses: " << err <<
            dendl;
        exit(1);
    }
    ...
    if ((needs & CEPH_PICK_ADDRESS_CLUSTER) && cluster_addr.is_blank_ip()) {
        if (!cluster_network.empty()) {
            fill_in_one_address(cct, ifa, cluster_network,
                                cluster_network_interface, "cluster_addr");
        }
        else {
            if (!public_network.empty()) {
                lderr(cct) <<
                    "Public network was set, but cluster network was not set "
                    << dendl;
                lderr(cct) <<
                    "    Using public network also for cluster network" <<
                    dendl;
                fill_in_one_address(cct, ifa, public_network,
                                    public_network_interface, "cluster_addr");
            }
        }
    }
}
...
static std::optional < entity_addr_t > get_one_address(CephContext * cct,
                                                       ...
                                                       int numa_node = -1)
{
    ...
    if (!found) {
        ...
        lderr(cct) << "unable to find any " << ip_type <<
            " address in networks '" << networks << "' interfaces '" <<
            interfaces << "'" << dendl;
        return {
        };
    }
...
int pick_addresses(CephContext * cct,
                   unsigned flags,
                   struct ifaddrs *ifa,
                   entity_addrvec_t * addrs, int preferred_numa_node)
{
    addrs->v.clear();
    ...
    if (addrs->v.empty()) {
      ...
      for (auto pick_mask:{
            ...
            if (ipv & pick_mask) {
                addr.set_family(pick_mask ==
                                CEPH_PICK_ADDRESS_IPV4 ? AF_INET : AF_INET6);
                     ...
                     entity_addr_t * match)
{
    ...
    for (struct ifaddrs * addrs = ifa; addrs != nullptr;
