...
promote_max_bytes(0),
objecter(new
         Objecter(osd->client_messenger->cct, osd->objecter_messenger,
                  osd->monc, NULL, 0, 0)),
objecter_finisher(osd->client_messenger->cct),
watch_lock("OSDService::watch_lock"), watch_timer(osd->client_messenger->cct,
                                                  watch_lock), next_notif_id(0),
...
recovery_sleep_timer(cct, recovery_sleep_lock, false), reserver_finisher(cct),
local_reserver(cct, &reserver_finisher, cct->_conf->osd_max_backfills,
               cct->_conf->osd_min_recovery_priority), remote_reserver(cct,
                                                                       &reserver_finisher,
                                                                       cct->
                                                                       _conf->
                                                                       osd_max_backfills,
...
snap_sleep_timer(osd->client_messenger->cct, snap_sleep_lock,
                 false /* relax locking */ ),
    scrub_sleep_lock("OSDService::scrub_sleep_lock"),
scrub_sleep_timer(osd->client_messenger->cct, scrub_sleep_lock,
                  false /* relax locking */ ),
    snap_reserver(cct, &reserver_finisher, cct->_conf->osd_max_trimming_pgs),
recovery_lock("OSDService::recovery_lock"), recovery_ops_active(0),
recovery_ops_reserved(0), recovery_paused(false),
map_cache_lock("OSDService::map_cache_lock"), map_cache(cct,
                                                        cct->_conf->
                                                        osd_map_cache_size),
...
full_status_lock("OSDService::full_status_lock"), cur_state(NONE), cur_ratio(0),
epoch_lock("OSDService::epoch_lock"), boot_epoch(0), up_epoch(0), bind_epoch(0),
                                        ...
                                        const set < spg_t > &children)
{
    ...
    map < spg_t, set < spg_t > >::iterator piter =
        rev_pending_splits.find(parent);
                                   ...
                                   OSDMapRef new_map, spg_t pgid)
{
    ...
    if (pgid.ps() < static_cast < unsigned >(old_pgnum)) {
        ...
        assert(pgid.ps() < static_cast <
               unsigned >(new_map->get_pg_num(pgid.pool())));
    }
}

void OSDService::init_splits_between(spg_t pgid,
                                     OSDMapRef frommap, OSDMapRef tomap)
{
    ...
    if (tomap->have_pg_pool(pgid.pool()) &&
        pgid.is_split(frommap->get_pg_num(pgid.pool()),
                      tomap->get_pg_num(pgid.pool()), NULL)) {
        ...
        for (epoch_t e = frommap->get_epoch() + 1; e <= tomap->get_epoch(); ++e) {
            OSDMapRef nextmap(try_get_map(e));
            if (!nextmap)
                continue;
            set < spg_t > even_newer_pgs;   // pgs added in this loop
            for (set < spg_t >::iterator i = new_pgs.begin();
                 i != new_pgs.end(); ++i) {
                set < spg_t > split_pgs;
...
void OSDService::agent_entry()
{
    ...
    while (!agent_stop_flag) {
        ...
        dout(10) << __func__ << " tiers " << agent_queue.size()
            << ", top is " << level << " with pgs " << top.size()
            << ", ops " << agent_ops << "/"
        ...
        if (!pg->agent_work(max, agent_flush_quota)) {
            dout(10) << __func__ << " " << pg->get_pgid()
                << " no agent_work, delay for " << cct->_conf->
                osd_agent_delay_time << " seconds" << dendl;
...
{
    {
        ...
        if (!agent_queue.empty()) {
            set < PGRef > &top = agent_queue.rbegin()->second;
            derr << "agent queue not empty, for example " << (*top.begin())->
                info.pgid << dendl;
...
void OSDService::promote_throttle_recalibrate()
{
    ...
    dout(10) << __func__ << " " << attempts << " attempts, promoted "
        ...
        << byte_u_t(target_bytes_sec) << "/sec" << dendl;
    ...
    dout(10) << __func__ << "  actual " << actual
        ...
        << ", prob " << promote_probability_millis << " -> " << prob << dendl;
...
void OSDService::check_full_status(float ratio)
{
    ...
    if (cur_state != new_state) {
        ...
        if (new_state == FAILSAFE) {
            clog->
                error() <<
                "full status failsafe engaged, dropping updates, now " << (int)
                roundf(ratio * 100) << "% full";
        }
        else if (cur_state == FAILSAFE) {
            clog->
                error() <<
                "full status failsafe disengaged, no longer dropping " <<
...
bool OSDService::_check_full(s_names type, ostream & ss) const const
{
    Mutex::Locker l(full_status_lock);
...
bool OSDService::check_failsafe_full(ostream & ss) const const
{
    return _check_full(FAILSAFE, ss);
}

bool OSDService::check_full(ostream & ss) const const
{
    return _check_full(FULL, ss);
}

bool OSDService::check_backfill_full(ostream & ss) const const
{
    return _check_full(BACKFILLFULL, ss);
}

bool OSDService::check_nearfull(ostream & ss) const const
{
    return _check_full(NEARFULL, ss);
}

bool OSDService::is_failsafe_full() const const
{
    Mutex::Locker l(full_status_lock);
    return cur_state == FAILSAFE;
}

bool OSDService::is_full() const const
{
    Mutex::Locker l(full_status_lock);
    return cur_state >= FULL;
}

bool OSDService::is_backfillfull() const const
{
    Mutex::Locker l(full_status_lock);
    return cur_state >= BACKFILLFULL;
}

bool OSDService::is_nearfull() const const
{
    Mutex::Locker l(full_status_lock);
...
osd_stat_t OSDService::set_osd_stat(const struct store_statfs_t &stbuf,
                                    vector < int >&hb_peers, int num_pgs)
{
...
ConnectionRef OSDService::get_con_osd_cluster(int peer, epoch_t from_epoch)
{
    ...
    ConnectionRef con =
        osd->cluster_messenger->get_connection(next_map->
                                                                 ...
                                                                 from_epoch)
{
    ...
    ret.first =
        osd->hb_back_client_messenger->get_connection(next_map->
                                                      get_hb_back_inst(peer));
    if (next_map->get_hb_front_addr(peer) != entity_addr_t())
        ret.second =
            osd->hb_front_client_messenger->get_connection(next_map->
                                                           get_hb_front_inst
                                                           (peer));
    release_map(next_map);
    return ret;
...
void OSDService::send_pg_temp()
{
  ...
  for (auto & pg_temp:pg_temp_wanted) {
        ...
        m->pg_temp.emplace(pg_temp.first, pg_temp.second.acting);
    }
  for (auto m:ms) {
        if (m) {
...
epoch_t OSDService::note_peer_epoch(int peer, epoch_t e)
{
    ...
    if (p != peer_map_epoch.end()) {
        if (p->second < e) {
            ...
            dout(30) << "note_peer_epoch osd." << peer << " has " << p->
                second << " >= " << e << dendl;
        }
        return p->second;
    }
...
{
    dout(20) << "should_share_map " << name << " " << con->get_peer_addr()
        << " " << epoch << dendl;
...
void OSDService::share_map(entity_name_t name,
                           Connection * con,
                           epoch_t epoch,
                           OSDMapRef & osdmap, epoch_t * sent_epoch_p)
{
    dout(20) << "share_map " << name << " " << con->get_peer_addr()
        << " " << epoch << dendl;
...
void OSDService::share_map_peer(int peer, Connection * con, OSDMapRef map)
{
    ...
    if (pe) {
        if (pe < map->get_epoch()) {
            ...
            dout(20) << "share_map_peer " << con << " already has epoch " << pe
                << dendl;
    }
    else {
        dout(20) << "share_map_peer " << con <<
...
bool OSDService::can_inc_scrubs_pending()
{
    ...
    if (scrubs_pending + scrubs_active < cct->_conf->osd_max_scrubs) {
        dout(20) << __func__ << " " << scrubs_pending << " -> " <<
            (scrubs_pending + 1)
            << " (max " << cct->_conf->
            osd_max_scrubs << ", active " << scrubs_active << ")" << dendl;
        can_inc = true;
    }
    else {
...
bool OSDService::inc_scrubs_pending()
{
    ...
    if (scrubs_pending + scrubs_active < cct->_conf->osd_max_scrubs) {
        dout(20) << "inc_scrubs_pending " << scrubs_pending << " -> " <<
            (scrubs_pending + 1)
            << " (max " << cct->_conf->
            osd_max_scrubs << ", active " << scrubs_active << ")" << dendl;
        result = true;
        ...
        dout(20) << "inc_scrubs_pending " << scrubs_pending << " + " <<
            scrubs_active << " active >= max " << cct->_conf->
            osd_max_scrubs << dendl;
    }
...
{
    sched_scrub_lock.Lock();
    dout(20) << "dec_scrubs_pending " << scrubs_pending << " -> " <<
        (scrubs_pending - 1)
        << " (max " << cct->_conf->
        osd_max_scrubs << ", active " << scrubs_active << ")" << dendl;
    --scrubs_pending;
...
void OSDService::inc_scrubs_active(bool reserved)
{
    ...
    if (reserved) {
        --(scrubs_pending);
        dout(20) << "inc_scrubs_active " << (scrubs_active -
                                             1) << " -> " << scrubs_active <<
            " (max " << cct->_conf->
            osd_max_scrubs << ", pending " << (scrubs_pending +
                                               1) << " -> " << scrubs_pending <<
            ")" << dendl;
        assert(scrubs_pending >= 0);
    }
        ...
        dout(20) << "inc_scrubs_active " << (scrubs_active -
                                             1) << " -> " << scrubs_active <<
            " (max " << cct->_conf->
            osd_max_scrubs << ", pending " << scrubs_pending << ")" << dendl;
    }
    sched_scrub_lock.Unlock();
}
...
{
    sched_scrub_lock.Lock();
    dout(20) << "dec_scrubs_active " << scrubs_active << " -> " <<
        (scrubs_active - 1)
        << " (max " << cct->_conf->
        osd_max_scrubs << ", pending " << scrubs_pending << ")" << dendl;
    --scrubs_active;
...
void OSDService::retrieve_epochs(epoch_t * _boot_epoch, epoch_t * _up_epoch,
                                 epoch_t * _bind_epoch) const const
{
...
bool OSDService::prepare_to_stop()
{
    ...
    if (osdmap && osdmap->is_up(whoami)) {
        ...
        monc->send_mon_message(new MOSDMarkMeDown(monc->get_fsid(), osdmap->get_inst(whoami), osdmap->get_epoch(), true // request ack
                               ));
        utime_t now = ceph_clock_now();
        utime_t timeout;
        timeout.set_from_double(now + cct->_conf->osd_mon_shutdown_timeout);
        while ((ceph_clock_now() < timeout) && (get_state() != STOPPING)) {
                                      ...
                                      OSDMapRef & osdmap)
{
    ...
    while (!m) {
        ...
        if (to > since
            ...
            dout(10) << "  " << (to -
                                 since) << " > max " << cct->_conf->
                osd_map_share_max_epochs << ", only sending most recent" <<
                dendl;
...
OSDMapRef OSDService::try_get_map(epoch_t epoch)
{
    ...
    if (logger) {
        ...
        if (epoch < lb) {
            dout(30) << "get_map " << epoch << " - miss, below lower bound" <<
                dendl;
...
void OSDService::queue_for_snap_trim(PG * pg)
{
    ...
    osd->op_shardedwq.queue(make_pair(pg->info.pgid,
                                      PGQueueable(PGSnapTrim
                                                  (pg->get_osdmap()->
                                                   get_epoch()),
                                                  cct->_conf->
                                                  osd_snap_trim_cost,
...
namespace ceph {
    namespace osd_cmds {
        ...
        int heap(CephContext & cct, cmdmap_t & cmdmap, Formatter & f,
                 std::ostream & os);
                  ...
                  uuid_d fsid, int whoami)
{
    ...
    ceph::shared_ptr < ObjectStore::Sequencer >
        osr(new ObjectStore::Sequencer("mkfs"));
    OSDSuperblock sb;
    bufferlist sbbl;
    C_SaferCond waiter;
    ...
    if (ret >= 0) {
        ...
        ::decode(sb, p);
        if (whoami != sb.whoami) {
            derr << "provided osd id " << whoami << " != superblock's " << sb.
                whoami << dendl;
            ret = -EINVAL;
            goto umount_store;
        }
...
int OSD::write_meta(CephContext * cct, ObjectStore * store,
...
OSD::OSD(CephContext * cct_, ObjectStore * store_, int id, Messenger * internal_messenger, Messenger * external_messenger, Messenger * hb_client_front, Messenger * hb_client_back, Messenger * hb_front_serverm, Messenger * hb_back_serverm, Messenger * osdc_messenger, MonClient * mc, const std::string & dev, const std::string & jdev):
Dispatcher(cct_),
...
authorize_handler_cluster_registry(new AuthAuthorizeHandlerRegistry(cct,
                                                                    ...
                                                                    _conf->
                                                                    auth_supported)),
authorize_handler_service_registry(new
                                   AuthAuthorizeHandlerRegistry(cct,
                                                                cct->_conf->
                                                                auth_supported.
                                                                empty()? cct->
                                                                _conf->
                                                                ...
                                                                : cct->_conf->
                                                                auth_supported)),
cluster_messenger(internal_messenger), client_messenger(external_messenger),
objecter_messenger(osdc_messenger), monc(mc), mgrc(cct_, client_messenger),
logger(NULL), recoverystate_perf(NULL), store(store_), log_client(cct,
                                                                  client_messenger,
                                                                  &mc->monmap,
                                                                  LogClient::
                                                                  NO_FLAGS),
...
osd_compat(get_osd_compat_set()), peering_tp(cct, "OSD::peering_tp",
                                             "tp_peering",
                                             cct->_conf->osd_peering_wq_threads,
                                             "osd_peering_tp_threads"),
osd_op_tp(cct, "OSD::osd_op_tp", "tp_osd_tp", get_num_op_threads()),
remove_tp(cct, "OSD::remove_tp", "tp_osd_remove",
          cct->_conf->osd_remove_threads, "osd_remove_threads"),
recovery_tp(cct, "OSD::recovery_tp", "tp_osd_recovery",
            cct->_conf->osd_recovery_threads, "osd_recovery_threads"),
...
heartbeat_lock("OSD::heartbeat_lock"), heartbeat_stop(false),
heartbeat_need_update(true), hb_front_client_messenger(hb_client_front),
...
heartbeat_thread(this), heartbeat_dispatcher(this), op_tracker(cct,
                                                               cct->_conf->
                                                               osd_enable_op_tracker,
                                                               cct->_conf->
                                                               osd_num_op_tracker_shard),
test_ops_hook(NULL), op_queue(get_io_queue()),
op_prio_cutoff(get_io_prio_cut()), op_shardedwq(get_num_op_shards(), this,
                                                cct->_conf->
                                                osd_op_thread_timeout,
                                                cct->_conf->
                                                osd_op_thread_suicide_timeout,
                                                &osd_op_tp), peering_wq(this,
                                                                        cct->
                                                                        _conf->
                                                                        osd_op_thread_timeout,
                                                                        cct->
                                                                        ...
                                                                        osd_op_thread_suicide_timeout,
                                                                        &peering_tp),
map_lock("OSD::map_lock"), pg_map_lock("OSD::pg_map_lock"),
last_pg_create_epoch(0), mon_report_lock("OSD::mon_report_lock"),
stats_ack_timeout(cct->_conf->osd_mon_ack_timeout), up_thru_wanted(0),
requested_full_first(0), requested_full_last(0),
pg_stat_queue_lock("OSD::pg_stat_queue_lock"), osd_stat_updated(false),
pg_stat_tid(0), pg_stat_tid_flushed(0), command_wq(this,
                                                   cct->_conf->
                                                   osd_command_thread_timeout,
                                                   cct->_conf->
                                                   osd_command_thread_suicide_timeout,
                                                   &command_tp), remove_wq(cct,
                                                                           store,
                                                                           cct->
                                                                           _conf->
...
service(this)
{
    ...
    op_tracker.set_history_slow_op_size_and_threshold(cct->_conf->
...
will start to track new ops received afterwards.";
        ...
        if (admin_command == "dump_ops_in_flight" || admin_command == "ops") {
        ...
        // scan pg's
        {
            ...
            for (ceph::unordered_map < spg_t, PG * >::iterator it =
                 pg_map.begin(); it != pg_map.end(); ++it) {
        ...
        dout(1) << "finished manual compaction in " << time_span.count()
            << " seconds" << dendl;
...
int OSD::enable_disable_fuse(bool stop)
{
    ...
    if (!fuse_store && cct->_conf->osd_objectstore_fuse) {
        ...
        r = fuse_store->start();
        if (r < 0) {
            derr << __func__ << " unable to start fuse: " << cpp_strerror(r) <<
                dendl;
            delete fuse_store;
            fuse_store = NULL;
            return r;
...
int OSD::get_num_op_threads()
{
    ...
    if (store_is_rotational)
        return get_num_op_shards() *
            cct->_conf->osd_op_num_threads_per_shard_hdd;
    else
        return get_num_op_shards() *
            cct->_conf->osd_op_num_threads_per_shard_ssd;
}
...
int OSD::init()
{
    ...
    // sanity check long object name handling
    {
        ...
        if (r < 0) {
            ...
            derr << "   osd max object name len = " << cct->_conf->
                osd_max_object_name_len << dendl;
            derr << "   osd max object namespace len = " << cct->_conf->
                osd_max_object_namespace_len << dendl;
            ...
            dout(20) << "configured osd_max_object_name[space]_len looks ok" <<
                dendl;
        }
    }
    ...
    if (osd_compat.compare(superblock.compat_features) < 0) {
        derr << "The disk uses features unsupported by the executable." <<
            dendl;
   ...
   */
    mgrc.set_pgstats_cb([this] () {
                        ...
                        MPGStats * m =
                        new MPGStats(monc->get_fsid(), osdmap->get_epoch(),
                                     had_for); m->osd_stat = cur_stat;
                        Mutex::Locker lec {
      min_last_epoch_clean_lock}; min_last_epoch_clean = osdmap->get_epoch(); min_last_epoch_clean_pgs.clear(); RWLock::RLocker lpg(pg_map_lock); for (const auto & i:pg_map)
                        {
                        PG * pg = i.second; if (!pg->is_primary()) {
                        continue;}
                        ...
                        const auto lec =
                        pg->pg_stats_publish.get_effective_last_epoch_clean();
...
void OSD::final_init()
{
    ...
    r = admin_socket->register_command("dump_scrubs",
                                       "dump_scrubs",
                                       asok_hook, "print scheduled scrubs");
    assert(r == 0);
    ...
    r = admin_socket->register_command("dump_pgstate_history",
                                       "dump_pgstate_history", asok_hook,
    ...
    assert(r == 0);
    r = admin_socket->register_command("trigger_deep_scrub",
                                       "trigger_deep_scrub "
                                       "name=pgid,type=CephString "
                                       "name=time,type=CephInt,req=false",
                                       test_ops_hook,
...
void OSD::create_logger()
{
    ...
    osd_plb.add_u64_counter(l_osd_sop_inb, "subop_in_bytes",
                            "Suboperations total size", NULL, 0, unit_t(BYTES));
    osd_plb.add_time_avg(l_osd_sop_lat, "subop_latency",
    ...
    osd_plb.add_u64(l_osd_buf, "buffer_bytes", "Total allocated buffer size",
                    NULL, 0, unit_t(BYTES));
    osd_plb.add_u64(l_osd_history_alloc_bytes, "history_alloc_Mbytes", NULL, 0,
                    unit_t(BYTES));
    ...
    osd_plb.add_u64(l_osd_stat_bytes_used, "stat_bytes_used", "Used space",
                    "used", PerfCountersBuilder::PRIO_USEFUL, unit_t(BYTES));
    osd_plb.add_u64(l_osd_stat_bytes_avail, "stat_bytes_avail",
                    "Available space", NULL, 0, unit_t(BYTES));
...
int OSD::shutdown()
{
    ...
    cct->get_admin_socket()->
        unregister_command("calc_objectstore_db_histogram");
    ...
    // note unmount epoch
    dout(10) << "noting clean unmount in epoch " << osdmap->
        get_epoch() << dendl;
...
int OSD::update_crush_location()
{
    ...
    if (cct->_conf->osd_crush_initial_weight >= 0) {
        ...
        snprintf(weight, sizeof(weight), "%.4lf",
                 MAX((double).00001,
                     (double)(st.total) / (double)(1ull << 40 /* TB */ )));
    }
    ...
    string cmd =
        ...
        string("\"weight\":") + weight + string(", ") + string("\"args\": [");
    for (multimap < string, string >::iterator p = loc.begin(); p != loc.end();
         ++p) {
        if (p != loc.begin())
            cmd += ", ";
...
void OSD::write_superblock(ObjectStore::Transaction & t)
{
    ...
    if (!superblock.compat_features.incompat.
        contains(CEPH_OSD_FEATURE_INCOMPAT_BASE))
        superblock.compat_features.incompat.
...
void OSD::clear_temp_objects()
{
    ...
    for (vector < coll_t >::iterator p = ls.begin(); p != ls.end(); ++p) {
        ...
        if (!temps.empty()) {
            ObjectStore::Transaction t;
            int removed = 0;
            for (vector < ghobject_t >::iterator q = temps.begin();
                 q != temps.end(); ++q) {
                dout(20) << "  removing " << *p << " object " << *q << dendl;
                t.remove(*p, *q);
                if (++removed > cct->_conf->osd_target_transaction_size) {
                    store->apply_transaction(service.meta_osr.get(),
                                             std::move(t));
...
PGPool OSD::_get_pool(int id, OSDMapRef createmap)
{
    if (!createmap->have_pg_pool(id)) {
        dout(5) << __func__ <<
            ": the OSDmap does not contain a PG pool with id = " << id << dendl;
        ceph_abort();
    }
...
void OSD::add_newly_split_pg(PG * pg, PG::RecoveryCtx * rctx)
{
    ...
    if (to_wake != peering_wait_for_split.end()) {
        for (list < PG::CephPeeringEvtRef >::iterator i =
             to_wake->second.begin(); i != to_wake->second.end(); ++i) {
...
PG *OSD::_create_lock_pg(OSDMapRef createmap,
                         spg_t pgid,
                         bool hold_map_lock,
                         bool backfill,
                         int role,
                         ...
                         const PastIntervals & pi, ObjectStore::Transaction & t)
{
    assert(osd_lock.is_locked());
    ...
    pg->init(role,
             up, up_primary, acting, acting_primary, history, pi, backfill, &t);
    ...
    for (vector < coll_t >::iterator it = ls.begin(); it != ls.end(); ++it) {
        ...
        if (map_epoch > 0) {
            ...
            if (!pgosdmap) {
                if (!osdmap->have_pg_pool(pgid.pool())) {
                    derr << __func__ << ": could not find map for epoch " <<
                        map_epoch << " on pg " << pgid <<
        ...
        if (pg->dne()) {
            ...
            pg->unlock();
            {
                ...
                dout(20) << __func__ << " removed pg " << pg << " from pg_map"
                    << dendl;
                pg_map.erase(p);
                pg->put("PGMap");
            }
        ...
        dout(10) << "load_pgs loaded " << *pg << " " << pg->pg_log.
            get_log() << dendl;
...
void OSD::build_past_intervals_parallel()
{
    ...
    epoch_t cur_epoch = superblock.newest_map;
    {
        ...
        for (ceph::unordered_map < spg_t, PG * >::iterator i = pg_map.begin();
            ...
            auto rpib = pg->get_required_past_interval_bounds(pg->info,
                                                              superblock.
                                                              oldest_map);
            if (rpib.first >= rpib.second && pg->past_intervals.empty()) {
                ...
                if (apib.second >= rpib.second && apib.first <= rpib.first) {
                    if (pg->info.history.same_interval_since == 0) {
                        pg->info.history.same_interval_since = rpib.second;
                    }
                    continue;
    ...
    for (; cur_epoch <= end_epoch; cur_epoch++) {
        ...
        for (map < PG *, pistate >::iterator i = pis.begin(); i != pis.end();
             ++i) {
            PG *pg = i->first;
            pistate & p = i->second;
            ...
            if (p.same_interval_since
                && last_map->get_pools().count(pgid.pool()))
                pgid = pgid.get_ancestor(last_map->get_pg_num(pgid.pool()));
            cur_map->pg_to_up_acting_osds(pgid, &up, &up_primary, &acting,
                                          &primary);

            if (p.same_interval_since == 0) {
                dout(10) << __func__ << " epoch " << cur_epoch << " pg " << pg->
                    info.
                    pgid << " first map, acting " << acting << " up " << up <<
                    ", same_interval_since = " << cur_epoch << dendl;
            ...
            boost::scoped_ptr < IsPGRecoverablePredicate >
                recoverable(pg->get_is_recoverable_predicate());
            std::stringstream debug;
            bool new_interval = PastIntervals::check_new_interval(p.primary,
                                                                  primary,
                                                                  p.old_acting,
                                                                  acting,
                                                                  p.up_primary,
    ...
    for (map < PG *, pistate >::iterator i = pis.begin(); i != pis.end(); ++i) {
        ...
        if (pg->info.history.same_interval_since == 0) {
            ...
            dout(10) << __func__ << " fix same_interval_since " << p.
                same_interval_since << " pg " << *pg << dendl;
            dout(10) << __func__ << " past_intervals " << pg->
 ...
 */
int OSD::handle_pg_peering_evt(spg_t pgid,
                               const pg_history_t & orig_history,
                               const PastIntervals & pi,
                               epoch_t epoch, PG::CephPeeringEvtRef evt)
{
    ...
    if (!pg) {
        ...
        bool valid_history =
            project_pg_history(pgid, history, epoch, up, up_primary, acting,
                               acting_primary);
        ...
        spg_t resurrected;
        PGRef old_pg_state;
        res_result result = _try_resurrect_pg(service.get_osdmap(),
                                              pgid,
                                              &resurrected,
                                              &old_pg_state);
        ...
        case RES_NONE:{
                ...
                pg = _create_lock_pg(get_map(epoch),
                                     pgid, false, false,
                                     role,
                                     up, up_primary,
                                     acting, acting_primary,
                                     history, pi, *rctx.transaction);
        ...
        case RES_SELF:{
                ...
                old_pg_state->unlock();
                pg = _create_lock_pg(old_osd_map,
                                     resurrected,
                                     false,
                                     true,
                                     old_role,
                                     ...
                                     old_primary,
                                     old_history,
                                     old_past_intervals, *rctx.transaction);
                pg->handle_create(&rctx);
                pg->write_if_dirty(*rctx.transaction);
                dispatch_context(rctx, pg, osdmap);
        ...
        case RES_PARENT:{
                ...
                PastIntervals old_past_intervals = old_pg_state->past_intervals;
                old_pg_state->unlock();
                PG *parent = _create_lock_pg(old_osd_map,
                                             resurrected,
                                             false,
                                             true,
                                             old_role,
...
bool OSD::maybe_wait_for_max_pg(spg_t pgid, bool is_mon_create)
{
    ...
    RWLock::RLocker pg_map_locker {
    pg_map_lock};
    if (pg_map.size() < max_pgs_per_osd) {
        return false;
    }
    lock_guard < mutex > pending_creates_locker {
...
void OSD::build_initial_pg_history(spg_t pgid,
                                   epoch_t created,
                                   utime_t created_stamp,
                                   pg_history_t * h, PastIntervals * pi)
{
                             ...
                             int currentactingprimary)
{
    ...
    for (e = osdmap->get_epoch(); e > from; e--) {
        ...
        vector < int >up, acting;
        oldmap->pg_to_up_acting_osds(pgid.pgid,
                                     &up, &upprimary, &acting, &actingprimary);
        ...
        if ((actingprimary != currentactingprimary ||
            ...
            dout(15) << "project_pg_history " << pgid <<
                " acting|up changed in " << e << " from " << acting << "/" << up
                << " " << actingprimary << "/" << upprimary << " -> " <<
                currentacting << "/" << currentup << " " << currentactingprimary
        ...
        // primary change?
        if (OSDMap::primary_changed(actingprimary,
                                    acting,
                                    currentactingprimary,
                                    currentacting) &&
            e > h.same_primary_since) {
            dout(15) << "project_pg_history " << pgid << " primary changed in "
                << e << dendl;
            h.same_primary_since = e;
        }
...
void OSD::_remove_heartbeat_peer(int n)
{
    ...
    dout(20) << " removing heartbeat peer osd." << n
        ...
        << " " << (q->second.con_front ? q->second.con_front->
...
void OSD::maybe_update_heartbeat_peers()
{
    ...
    if (is_active()) {
        ...
        for (ceph::unordered_map < spg_t, PG * >::iterator i = pg_map.begin();
            ...
            dout(20) << i->first << " heartbeat_peers " << pg->
                heartbeat_peers << dendl;
    ...
    dout(10) << "maybe_update_heartbeat_peers " << heartbeat_peers.
    ...
    case MOSDPing::PING:
        {
            if (cct->_conf->osd_debug_drop_ping_probability > 0) {
                auto heartbeat_drop =
                    debug_heartbeat_drops_remaining.find(from);
                if (heartbeat_drop != debug_heartbeat_drops_remaining.end()) {
                    ...
                    heartbeat_drop =
                        ...
                        insert(std::
                               make_pair(from,
                                         cct->_conf->
                                         osd_debug_drop_ping_duration)).first;
                    dout(5) << "Dropping heartbeat from " << from << ", " <<
            ...
            if (i != heartbeat_peers.end()) {
                if (m->get_connection() == i->second.con_back) {
                    dout(25) << "handle_osd_ping got reply from osd." << from
                        ...
                        << " last_rx_back " << i->second.
                        last_rx_back << " -> " << m->
                        stamp << " last_rx_front " << i->second.
                        last_rx_front << dendl;
                    ...
                    dout(25) << "handle_osd_ping got reply from osd." << from
                        ...
                        << " last_rx_front " << i->second.
                        last_rx_front << " -> " << m->stamp << dendl;
                    i->second.last_rx_front = m->stamp;
                }
...
void OSD::heartbeat_check()
{
    ...
    for (map < int, HeartbeatInfo >::iterator p = heartbeat_peers.begin();
        ...
        if (p->second.first_tx == utime_t()) {
            dout(25) << "heartbeat_check we haven't sent ping to osd." << p->
                first << "yet, skipping" << dendl;
            continue;
        }
        ...
        dout(25) << "heartbeat_check osd." << p->first
            ...
            << " last_rx_front " << p->second.last_rx_front << dendl;
        if (p->second.is_unhealthy(cutoff)) {
            if (p->second.last_rx_back == utime_t() ||
                p->second.last_rx_front == utime_t()) {
                derr << "heartbeat_check: no reply from " << p->second.
                    con_front->get_peer_addr().get_sockaddr()
                    << " osd." << p->
                    first << " ever on either front or back, first ping sent "
                    << p->second.
                    first_tx << " (cutoff " << cutoff << ")" << dendl;
                // fail
                ...
                derr << "heartbeat_check: no reply from " << p->second.
                    con_front->get_peer_addr().get_sockaddr()
                    << " osd." << p->first << " since back " << p->second.
                    last_rx_back << " front " << p->second.
                    last_rx_front << " (cutoff " << cutoff << ")" << dendl;
                // fail
...
void OSD::heartbeat()
{
    ...
    if (getloadavg(loadavgs, 1) == 1) {
        ...
        daily_loadavg =
            (daily_loadavg * (n_samples - 1) + loadavgs[0]) / n_samples;
    ...
    if (s) {
        ...
        if (p != heartbeat_peers.end() &&
            ...
            if (newcon.first) {
                ...
                dout(10) << "heartbeat_reset failed hb con " << con <<
                    " for osd." << p->second.
                    peer << ", raced with osdmap update, closing out peer" <<
                    dendl;
...
void OSD::tick()
{
    ...
    if (is_waiting_for_healthy()) {
        start_boot();
             ...
             monc->monmap.get_required_features().
...
void OSD::tick_without_osd_lock()
{
    ...
    logger->set(l_osd_history_alloc_bytes,
...
void TestOpsSocketHook::test_ops(OSDService * service, ObjectStore * store,
                                 const std::string & command, cmdmap_t & cmdmap,
                                 ostream & ss)
{
    //Test support
    ...
    if (command == "setomapval" || command == "rmomapkey" ||
        ...
        if (command == "setomapval") {
            ...
            r = store->omap_get(coll_t(pgid), ghobject_t(obj), &hdrbl,
                                &keyvals);
            if (r >= 0) {
                ...
                bool * finished, ThreadPool::TPHandle & handle)
{
    ...
    store->collection_list(coll,
                           next,
                           ghobject_t::get_max(),
                           store->get_ideal_list_max(), &olist, &next);
    ...
    for (vector < ghobject_t >::iterator i = olist.begin();
        ...
        if (++num >= cct->_conf->osd_target_transaction_size) {
            ...
            if (cct->_conf->osd_delete_sleep) {
                ...
                lgeneric_subdout(cct, osd,
                                 10) << __func__ << " inject delay of " << t <<
                    dendl;
                t.sleep();
            }
            handle.reset_tp_timeout();
                             ...
                             ThreadPool::TPHandle & handle)
{
    ...
    bool cont =
        remove_dir(pg->cct, store, &mapper, &driver, pg->osr.get(), coll,
                   item.second,
                   &finished, handle);
    if (!cont)
        return;
    ...
    store->queue_transaction(pg->osr.get(), std::move(t), 0,    // onapplied
                             0, // oncommit
                             0, // onreadable sync
...
bool OSD::ms_handle_refused(Connection * con)
{
    ...
    Session *session = static_cast < Session * >(con->get_priv());
    dout(2) << "ms_handle_refused con " << con << " session " << session <<
        dendl;
    if (!session)
        return false;
...
void OSD::_preboot(epoch_t oldest, epoch_t newest)
{
    ...
    else if (!monc->monmap.get_required_features().
             contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
        derr <<
    ...
    if (is_waiting_for_healthy()) {
        ...
        if ((float)up <
            ...
            dout(1) << "is_healthy false -- only " << up << "/" << num <<
                " up peers (less than " << int (cct->_conf->
...
void OSD::_send_boot()
{
    ...
    MOSDBoot *mboot =
        new MOSDBoot(superblock, get_osdmap_epoch(), service.get_boot_epoch(),
...
void OSD::_collect_metadata(map < string, string > *pm)
{
    ...
    (*pm)["back_iface"] =
        pick_iface(cct, cluster_messenger->get_myaddr().get_sockaddr_storage());
...
void OSD::queue_want_up_thru(epoch_t want)
{
    ...
    if (want > up_thru_wanted) {
        dout(10) << "queue_want_up_thru now " << want << " (was " <<
...
void OSD::send_alive()
{
    ...
    if (up_thru_wanted > up_thru) {
        ...
        monc->
            send_mon_message(new
                             MOSDAlive(osdmap->get_epoch(), up_thru_wanted));
    }
}
...
void OSD::got_full_map(epoch_t e)
{
    ...
    if (e < requested_full_first) {
        dout(10) << __func__ << " " << e << ", requested " <<
            requested_full_first << ".." << requested_full_last << ", ignoring"
            << dendl;
        return;
    }
    if (e >= requested_full_last) {
        dout(10) << __func__ << " " << e << ", requested " <<
            requested_full_first << ".." << requested_full_last << ", resetting"
            << dendl;
...
void OSD::requeue_failures()
{
    ...
    for (map < int, pair < utime_t, entity_inst_t > >::iterator p =
         failure_pending.begin(); p != failure_pending.end();) {
...
void OSD::send_failures()
{
    ...
    while (!failure_queue.empty()) {
        ...
        if (!failure_pending.count(osd)) {
            ...
            monc->
                send_mon_message(new
                                 MOSDFailure(monc->get_fsid(), i, failed_for,
                                             osdmap->get_epoch()));
...
{
    MOSDFailure *m =
        new MOSDFailure(monc->get_fsid(), i, 0, epoch, MOSDFailure::FLAG_ALIVE);
    monc->send_mon_message(m);
}
...
void OSD::send_pg_stats(const utime_t & now)
{
    ...
    if (osd_stat_updated || !pg_stat_queue.empty()) {
        ...
        dout(10) << "send_pg_stats - " << pg_stat_queue.
        ...
        MPGStats *m =
            new MPGStats(monc->get_fsid(), osdmap->get_epoch(), had_for);
        ...
        while (!p.end()) {
            ...
            if (pg->pg_stats_publish_valid) {
                m->pg_stat[pg->info.pgid.pgid] = pg->pg_stats_publish;
                dout(25) << " sending " << pg->info.pgid << " " << pg->
                    pg_stats_publish.reported_epoch << ":" << pg->
                    pg_stats_publish.reported_seq << dendl;
            }
            else {
                dout(25) << " NOT sending " << pg->info.pgid << " " << pg->
                    pg_stats_publish.reported_epoch << ":" << pg->
                    pg_stats_publish.reported_seq << ", not valid" << dendl;
            }
            pg->pg_stats_publish_lock.Unlock();
        }
        ...
        dout(20) << __func__ << "  updates pending: " << outstanding_pg_stats <<
            dendl;
    ...
    while (!p.end()) {
        ...
        if (acked != ack->pg_stat.end()) {
            ...
            if (acked->second.first == pg->pg_stats_publish.reported_seq &&
                ...
                dout(25) << " ack on " << pg->info.pgid << " " << pg->
                    pg_stats_publish.reported_epoch << ":" << pg->
                    pg_stats_publish.reported_seq << dendl;
                pg->stat_queue_item.remove_myself();
                pg->put("pg_stat_queue");
                ...
                dout(25) << " still pending " << pg->info.pgid << " " << pg->
                    pg_stats_publish.reported_epoch << ":" << pg->
                    pg_stats_publish.reported_seq << " > acked " << acked->
                    second << dendl;
            }
            pg->pg_stats_publish_lock.Unlock();
        }
        else {
            dout(30) << " still pending " << pg->info.pgid << " " << pg->
                pg_stats_publish.reported_epoch << ":" << pg->pg_stats_publish.
                reported_seq << dendl;
        }
    }

    outstanding_pg_stats.erase(ack->get_tid());
    dout(20) << __func__ << "  still pending: " << outstanding_pg_stats <<
        dendl;
...
void OSD::flush_pg_stats()
{
    ...
    pg_stat_queue_lock.Lock();
    uint64_t tid = pg_stat_tid;
    dout(10) << "flush_pg_stats waiting for stats tid " << tid << " to flush" <<
        dendl;
    while (tid > pg_stat_tid_flushed)
        pg_stat_queue_cond.Wait(pg_stat_queue_lock);
    dout(10) << "flush_pg_stats finished waiting for stats tid " << tid <<
        " to flush" << dendl;
    pg_stat_queue_lock.Unlock();
...
// new form: tell <pgid> <cmd> for both cli and rest
        ...
        COMMAND("perf histogram dump "
                ...
                "Get histogram data", "osd", "r", "cli,rest")
// tell <osd.n> commands.  Validation of osd.n must be special-cased in client
        ...
        COMMAND("get_command_descriptions", "list commands descriptions", "osd",
                "r", "cli,rest")
        COMMAND("injectargs " "name=injected_args,type=CephString,n=N",
                "inject configuration arguments into running OSD", "osd", "rw",
                "cli,rest")
        COMMAND("config set "
                "name=key,type=CephString name=value,type=CephString",
                "Set a configuration option at runtime (not persistent)", "osd",
                "rw", "cli,rest")
        COMMAND("cluster_log "
                "name=level,type=CephChoices,strings=error,warning,info,debug "
                "name=message,type=CephString,n=N",
                "log a message to the cluster log", "osd", "rw", "cli,rest")
        COMMAND("bench " "name=count,type=CephInt,req=false "
                ...
                "(default 1G size 4MB). Results in log.", "osd", "rw",
                "cli,rest")
        ...
        COMMAND("dump_pg_recovery_stats", "dump pg recovery statistics", "osd",
                "r", "cli,rest")
        COMMAND("reset_pg_recovery_stats", "reset pg recovery statistics",
                "osd", "rw", "cli,rest")
        COMMAND("compact",
                "compact object store's omap. "
                "WARNING: Compaction probably slows your requests", "osd", "rw",
                "cli,rest")
};
                     ...
                     bufferlist & data)
{
    ...
    if (prefix == "get_command_descriptions") {
        ...
        for (OSDCommand * cp = osd_commands;
            ...
            dump_cmddesc_to_json(f, secname.str(), cp->cmdstring,
                                 cp->helpstring, cp->module, cp->perm,
                                 cp->availability, 0);
            cmdnum++;
        }
    ...
    if (prefix == "version") {
        ...
        for (vector < string >::iterator a = ++argsvec.begin();
             a != argsvec.end(); ++a)
            args += " " + *a;
        unlock_guard unlock {
        osd_lock};
    ...
    // 'tell <pgid>' (which comes in without any of that prefix)?
             ...
             prefix == "query" ||
             prefix == "mark_unfound_lost" || prefix == "list_missing") {
        pg_t pgid;
        ...
        if (bsize > (int64_t) cct->_conf->osd_bench_max_block_size) {
            ...
            ss << "block 'size' values are capped at "
                << byte_u_t(cct->_conf->
                            osd_bench_max_block_size) << ". If you wish to use"
            ...
            if (count > max_count) {
                ss << "'count' values greater than " << max_count
                    << " for a block size of " << byte_u_t(bsize) <<
                    ", assuming " << cct->_conf->
                    osd_bench_small_size_max_iops << " IOPS," << " for " <<
        ...
        for (int64_t pos = 0; pos < count; pos += bsize) {
            ...
            unsigned offset = 0;
            if (onum && osize) {
                snprintf(nm, sizeof(nm), "disk_bw_test_%d",
                         (int)(rand() % onum));
        ...
        for (ceph::unordered_map < spg_t, PG * >::const_iterator pg_map_e =
            ...
            for (; mi != mend; ++mi) {
                ...
                const set < pg_shard_t >
                    &mls(pg->missing_loc.get_locations(mi->first));
                if (mls.empty())
                    continue;
        ...
        if (f) {
            cct->get_perfcounters_collection()->dump_formatted_histograms(f.
                                                                          get(),
                                                                          false,
                                                                          logger,
                                                                          counter);
...
bool OSD::heartbeat_dispatch(Message * m)
{
    ...
    default:
        dout(0) << "dropping unexpected message " << *m << " from " << m->
...
void OSD::maybe_share_map(Session * session, OpRequestRef op, OSDMapRef osdmap)
{
    ...
    service.share_map(m->get_source(),
                      m->get_connection().get(),
                      op->sent_epoch,
                      osdmap, session ? &last_sent_epoch : NULL);
...
void OSD::dispatch_session_waiting(Session * session, OSDMapRef osdmap)
{
    ...
    while (i != session->waiting_on_map.end()) {
        ...
        const MOSDFastDispatchOp *m =
            static_cast < const MOSDFastDispatchOp * >(op->get_req());
        ...
        if (m->get_type() == CEPH_MSG_OSD_OP) {
            pg_t actual_pgid =
                osdmap->raw_pg_to_pg(static_cast <
                                     const MOSDOp * >(m)->get_pg());
...
bool OSD::ms_verify_authorizer(Connection * con, int peer_type,
                               ...
                               bufferlist & authorizer_reply, bool & isvalid,
                               ...
                               *challenge)
{
    ...
    case CEPH_ENTITY_TYPE_MGR:
        authorize_handler =
            authorize_handler_cluster_registry->get_handler(protocol);
        break;
    default:
        authorize_handler =
            authorize_handler_service_registry->get_handler(protocol);
    }
    if (!authorize_handler) {
        dout(0) << "No AuthAuthorizeHandler found for protocol " << protocol <<
            dendl;
        isvalid = false;
        return true;
    }
    ...
    if (isvalid) {
        ...
        if (!s) {
            ...
            con->set_priv(s->get());
            s->con = con;
            dout(10) << " new session " << s << " con=" << s->
                con << " addr=" << s->con->get_peer_addr() << dendl;
        }
        ...
        if (caps_info.caps.length() > 0) {
            ...
            if (success)
                dout(10) << " session " << s << " " << s->
                    entity_name << " has caps " << s->
                    caps << " '" << str << "'" << dendl;
            else
                dout(10) << " session " << s << " " << s->
                    entity_name << " failed to parse caps '" << str << "'" <<
                    dendl;
        }
...
void OSD::_dispatch(Message * m)
{
    ...
    case MSG_OSD_RECOVERY_RESERVE:
        {
            OpRequestRef op =
                op_tracker.create_request < OpRequest, Message * >(m);
            if (m->trace)
                op->osd_trace.init("osd op", &trace_endpoint, &m->trace);
            // no map?  starting up?
            if (!osdmap) {
...
void OSD::handle_scrub(MOSDScrub * m)
{
    ...
    if (m->fsid != monc->get_fsid()) {
        dout(0) << "handle_scrub fsid " << m->fsid << " != " << monc->
            get_fsid() << dendl;
        m->put();
        return;
    }
...
bool OSDService::ScrubJob::ScrubJob::operator<(const OSDService::ScrubJob & rhs) const const
{
...
bool OSD::scrub_time_permit(utime_t now)
{
    ...
    if (!day_permit) {
        dout(20) << __func__ << " should run between week day " << cct->_conf->
            osd_scrub_begin_week_day << " - " << cct->_conf->
            osd_scrub_end_week_day << " now " << bdt.
            tm_wday << " = no" << dendl;
        return false;
    }
    ...
    if (!time_permit) {
        dout(20) << __func__ << " should run between " << cct->_conf->
            osd_scrub_begin_hour << " - " << cct->_conf->
            osd_scrub_end_hour << " now " << bdt.tm_hour << " = no" << dendl;
    }
    else {
        dout(20) << __func__ << " should run between " << cct->_conf->
            osd_scrub_begin_hour << " - " << cct->_conf->
            osd_scrub_end_hour << " now " << bdt.tm_hour << " = yes" << dendl;
    }
    return time_permit;
}
...
void OSD::sched_scrub()
{
    ...
    if (service.first_scrub_stamp(&scrub)) {
        do {
            dout(30) << "sched_scrub examine " << scrub.pgid << " at " << scrub.
                sched_time << dendl;

            if (scrub.sched_time > now) {
                // save ourselves some effort
                dout(10) << "sched_scrub " << scrub.
                    pgid << " scheduled at " << scrub.
                    sched_time << " > " << now << dendl;
                break;
            }

            if ((scrub.deadline >= now) && !(time_permit && load_is_low)) {
                dout(10) << __func__ << " not scheduling scrub for " << scrub.
                    pgid << " due to " << (!time_permit ? "time not permit" :
                                           "high load") << dendl;
                continue;
            }
            ...
            if (pg->get_pgbackend()->scrub_supported() && pg->is_active()) {
                dout(10) << "sched_scrub scrubbing " << scrub.
                    pgid << " at " << scrub.sched_time << (pg->scrubber.
                                                           must_scrub ?
...
void OSD::trim_maps(epoch_t oldest, int nreceived, bool skip_maps)
{
    ...
    for (epoch_t e = superblock.oldest_map; e < min; ++e) {
        ...
        if (num >= cct->_conf->osd_target_transaction_size && num >= nreceived) {
            ...
            write_superblock(t);
            int tr =
                store->queue_transaction(service.meta_osr.get(), std::move(t),
                                         nullptr);
            assert(tr == 0);
            num = 0;
            if (!skip_maps) {
...
void OSD::handle_osd_map(MOSDMap * m)
{
    ...
    for (epoch_t e = start; e <= last; e++) {
        ...
        if (p != m->incremental_maps.end()) {
            ...
            if (o->apply_incremental(inc) < 0) {
                derr << "ERROR: bad fsid?  i have " << osdmap->
                    get_fsid() << " and inc has " << inc.fsid << dendl;
                assert(0 == "bad fsid");
            }
            ...
            if ((inc.have_crc && o->get_crc() != inc.full_crc)
                || injected_failure) {
    ...
    if (!m->maps.empty() && requested_full_first) {
        dout(10) << __func__ << " still missing full maps " <<
            requested_full_first << ".." << requested_full_last << dendl;
        rerequest_full_maps();
    }
    ...
    write_superblock(t);
    store->queue_transaction(service.meta_osr.get(),
                             std::move(t),
...
void OSD::_committed_osd_maps(epoch_t first, epoch_t last, MOSDMap * m)
{
    ...
    for (epoch_t cur = first; cur <= last; cur++) {
        ...
        if ((osdmap->test_flag(CEPH_OSDMAP_NOUP) !=
            ...
            dout(10) << __func__ << " NOUP flag changed in " << newmap->
                get_epoch()
                << dendl;
            if (is_booting()) {
        ...
        if (!osdmap->exists(whoami)) {
            ...
            // everything paused
                 ...
                 || (osdmap->get_hb_front_addr(whoami) != entity_addr_t()
                     ...
                     probably_equals(hb_front_server_messenger->
                                     get_myaddr()))) {
            if (!osdmap->is_up(whoami)) {
            ...
            else if (!osdmap->get_addr(whoami).
                     probably_equals(client_messenger->get_myaddr())) {
            ...
            if (!service.is_stopping()) {
                ...
                if ((int)osd_markdown_log.size() >
                    cct->_conf->osd_max_markdown_count) {
                    dout(0) << __func__ << " marked down " << osd_markdown_log.
                        size()
                        << " > osd_max_markdown_count " << cct->_conf->
                        osd_max_markdown_count << " in last " << grace <<
                ...
                avoid_ports.insert(hb_back_server_messenger->get_myaddr().
                                   get_port());
                avoid_ports.insert(hb_front_server_messenger->get_myaddr().
                                   get_port());
...
void OSD::check_osdmap_features(ObjectStore * fs)
{
...

    {
        ...
        uint64_t mask;
        uint64_t features =
            osdmap->get_features(entity_name_t::TYPE_CLIENT, &mask);
    ...
    {
        Messenger::Policy p =
            client_messenger->get_policy(entity_name_t::TYPE_MON);
        uint64_t mask;
        uint64_t features =
            osdmap->get_features(entity_name_t::TYPE_MON, &mask);
        if ((p.features_required & mask) != features) {
            dout(0) << "crush map has features " << features
...
bool OSD::advance_pg(epoch_t osd_epoch, PG * pg,
                     ThreadPool::TPHandle & handle,
                     PG::RecoveryCtx * rctx, set < PGRef > *new_pgs)
{
    assert(pg->is_locked());
    ...
    for (; next_epoch <= osd_epoch && next_epoch <= max; ++next_epoch) {
        ...
        int up_primary, acting_primary;
        nextmap->pg_to_up_acting_osds(pg->info.pgid.pgid,
                                      &newup, &up_primary,
                                      &newacting, &acting_primary);
        pg->handle_advance_map(nextmap, lastmap, newup, up_primary,
...
void OSD::consume_map()
{
   ...
   */
    ...
    for (list < PGRef >::iterator i = to_remove.begin();
         i != to_remove.end(); to_remove.erase(i++)) {
...
void OSD::split_pgs(PG * parent,
                    const set < spg_t > &childpgids, set < PGRef > *out_pgs,
                    OSDMapRef curmap, OSDMapRef nextmap, PG::RecoveryCtx * rctx)
{
    ...
    for (set < spg_t >::const_iterator i = childpgids.begin();
        ...
        parent->split_colls(*i,
                            split_bits,
                            i->ps(), &child->pool.info, rctx->transaction);
...
void OSD::handle_pg_create(OpRequestRef op)
{
    ...
    for (map < pg_t, pg_create_t >::const_iterator p = m->mkpg.begin();
        ...
        if (acting_primary != whoami) {
            dout(10) << "mkpg " << on << "  not acting_primary (" <<
                acting_primary << "), my role=" << role << ", skipping" <<
                dendl;
            continue;
        }
        ...
        PastIntervals pi(osdmap->get_pools().at(pgid.pool()).ec_pool(),
                         *osdmap);
        pg_history_t history;
        ...
        if (handle_pg_peering_evt(pgid,
                                  history,
                                  pi,
                                  osdmap->get_epoch(),
                                  PG::CephPeeringEvtRef(new PG::
                                                        CephPeeringEvt(osdmap->
                                                                       get_epoch
                                                                       (),
                                                                       osdmap->
                                                                       get_epoch
                                       ...
                                       ThreadPool::TPHandle * handle)
{
    if (!ctx.transaction->empty()) {
        ...
        int tr = store->queue_transaction(pg->osr.get(),
                                          std::move(*ctx.transaction),
                                          ctx.on_applied, ctx.on_safe, NULL,
                                          TrackedOpRef(), handle);
...
void OSD::do_notifies(map < int, vector < pair < pg_notify_t,
                      PastIntervals > > >&notify_list, OSDMapRef curmap)
{
    for (map < int,
        ...
        ConnectionRef con =
            service.get_con_osd_cluster(it->first, curmap->get_epoch());
        if (!con) {
...
void OSD::do_queries(map < int, map < spg_t, pg_query_t > >&query_map,
                     OSDMapRef curmap)
{
    for (map < int, map < spg_t, pg_query_t > >::iterator pit =
         query_map.begin(); pit != query_map.end(); ++pit) {
        if (!curmap->is_up(pit->first)) {
            dout(20) << __func__ << " skipping down osd." << pit->
                first << dendl;
            continue;
        }
        int who = pit->first;
        ConnectionRef con =
            service.get_con_osd_cluster(who, curmap->get_epoch());
        if (!con) {
                   ...
                   OSDMapRef curmap)
{
    for (map < int,
        ...
        for (vector < pair < pg_notify_t, PastIntervals > >::iterator i =
             p->second.begin(); i != p->second.end(); ++i) {
            dout(20) << __func__ << " sending info " << i->first.
                info << " to shard " << p->first << dendl;
        }
        ConnectionRef con =
            service.get_con_osd_cluster(p->first, curmap->get_epoch());
        if (!con) {
...
void OSD::handle_pg_notify(OpRequestRef op)
{
    ...
    for (auto it = m->get_pg_list().begin(); it != m->get_pg_list().end(); ++it) {
        if (it->first.info.pgid.preferred() >= 0) {
            dout(20) << "ignoring localized pg " << it->first.info.
                pgid << dendl;
            continue;
        }
...
void OSD::handle_pg_log(OpRequestRef op)
{
    ...
    op->mark_started();
    handle_pg_peering_evt(spg_t(m->info.pgid.pgid, m->to),
                          m->info.history, m->past_intervals, m->get_epoch(),
                          PG::CephPeeringEvtRef(new PG::
                                                CephPeeringEvt(m->get_epoch(),
                                                               m->
                                                               get_query_epoch
                                                               (),
                                                               PG::
...
void OSD::handle_pg_info(OpRequestRef op)
{
    ...
    for (auto p = m->pg_list.begin(); p != m->pg_list.end(); ++p) {
        ...
        handle_pg_peering_evt(spg_t(p->first.info.pgid.pgid, p->first.to),
                              p->first.info.history, p->second,
                              p->first.epoch_sent,
                              PG::CephPeeringEvtRef(new PG::
                                                    CephPeeringEvt(p->first.
                                                                   epoch_sent,
                                                                   p->first.
                                                                   query_epoch,
                                                                   PG::
...
void OSD::handle_pg_trim(OpRequestRef op)
{
    ...
    if (m->epoch < pg->info.history.same_interval_since) {
        dout(10) << *pg << " got old trim to " << m->
            trim_to << ", ignoring" << dendl;
        pg->unlock();
        return;
    }
    ...
    if (pg->is_primary()) {
        ...
        dout(10) << *pg << " replica osd." << from << " lcod " << m->
            trim_to << dendl;
...
{
    const MBackfillReserve *m =
        static_cast < const MBackfillReserve * >(op->get_req());
    ...
    if (m->type == MBackfillReserve::REQUEST) {
        evt = PG::CephPeeringEvtRef(new PG::CephPeeringEvt(m->query_epoch,
                                                           m->query_epoch,
                                                           PG::
                                                           RequestBackfillPrio
                                                           (m->priority)));
...
{
    const MRecoveryReserve *m =
        static_cast < const MRecoveryReserve * >(op->get_req());
...
void OSD::handle_pg_query(OpRequestRef op)
{
    ...
    dout(7) << "handle_pg_query from " << m->get_source() << " epoch " << m->
        get_epoch() << dendl;
    int from = m->get_source().num();
    ...
    for (auto it = m->pg_list.begin(); it != m->pg_list.end(); ++it) {
        ...
        if (service.splitting(pgid)) {
            peering_wait_for_split[pgid].
                push_back(PG::
                          CephPeeringEvtRef(new PG::
                                            CephPeeringEvt(it->second.
                                                           epoch_sent,
                                                           it->second.
                                                           epoch_sent,
                                                           PG::
        ...
        bool valid_history =
            project_pg_history(pgid, history, it->second.epoch_sent,
        ...
        if (it->second.type == pg_query_t::LOG ||
            ...
            ConnectionRef con =
                service.get_con_osd_cluster(from, osdmap->get_epoch());
            if (con) {
                ...
                push_back(make_pair
                          (pg_notify_t
                           (it->second.from, it->second.to,
                            it->second.epoch_sent, osdmap->get_epoch(), empty),
                           PastIntervals(osdmap->get_pools().at(pgid.pool()).
                                         ec_pool(), *osdmap)));
        }
    }
...
void OSD::handle_pg_remove(OpRequestRef op)
{
    ...
    for (auto it = m->pg_list.begin(); it != m->pg_list.end(); ++it) {
        ...
        osdmap->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary, &acting,
                                     &acting_primary);
        bool valid_history =
            project_pg_history(pg->info.pgid, history,
...
void OSD::_remove_pg(PG * pg)
{
    ...
    int tr = store->queue_transaction(pg->osr.get(), std::move(rmt), NULL,
                                      new ContainerContext <
                                      SequencerRef > (pg->osr));
    assert(tr == 0);

    DeletingStateRef deleting =
        service.deleting_pgs.lookup_or_create(pg->info.pgid,
                                              make_pair(pg->info.pgid,
                                                        PGRef(pg))
        );
...
void OSDService::_maybe_queue_recovery()
{
    ...
    while (!awaiting_throttle.empty() && _recover_now(&available_pushes)) {
        uint64_t to_start = MIN(available_pushes,
...
bool OSDService::_recover_now(uint64_t * available_pushes)
{
    ...
    if (ceph_clock_now() < defer_recovery_until) {
        dout(15) << __func__ << " defer until " << defer_recovery_until <<
            dendl;
        return false;
    }
...
void OSDService::adjust_pg_priorities(const vector < PGRef > &pgs, int newflags)
{
    ...
    if (did.empty()) {
        dout(10) << __func__ << " " << ((newflags & OFR_CANCEL) ? "cleared" :
                                        "set")
            << " force_" << ((newflags & OFR_BACKFILL) ? "backfill" :
                             "recovery")
            << " on no pgs" << dendl;
    }
    else {
                      ...
                      ThreadPool::TPHandle & handle)
{
    ...
    float recovery_sleep = get_osd_recovery_sleep();
    {
        ...
        if (recovery_sleep > 0 && service.recovery_needs_sleep) {
            ...
            auto recovery_requeue_callback =
                new
                FunctionContext([this, pgref, queued, reserved_pushes] (int r){
                                dout(20) << "do_recovery wake up at " <<
                                ceph_clock_now()
                                << ", re-queuing recovery" << dendl;
                                Mutex::Locker l(service.recovery_sleep_lock);
                                service.recovery_needs_sleep = false;
                                service.queue_recovery_after_sleep(pgref.get(),
                                                                   queued,
                                                                   reserved_pushes);});
            ...
            dout(20) << "Recovery event scheduled at " << service.
                recovery_schedule_time << dendl;
            return;
        }
    }

    {
        ...
        dout(20) << "  active was " << service.recovery_oids[pg->info.
                                                             pgid] << dendl;
#endif
        ...
        if (!more && pg->have_unfound()) {
            ...
            if (rctx.query_map->empty()) {
                ...
                if (pg->state_test(PG_STATE_BACKFILLING)) {
                    auto evt =
                        PG::CephPeeringEvtRef(new PG::CephPeeringEvt(queued,
                                                                     queued,
                                                                     PG::
                                                                     DeferBackfill
                                                                     (cct->
...
void OSDService::start_recovery_op(PG * pg, const hobject_t & soid)
{
    ...
    dout(10) << "start_recovery_op " << *pg << " " << soid
        << " (" << recovery_ops_active << "/"
        << cct->_conf->osd_recovery_max_active << " rops)" << dendl;
    recovery_ops_active++;
                                    ...
                                    bool dequeue)
{
    ...
    dout(10) << "finish_recovery_op " << *pg << " " << soid
        << " dequeue=" << dequeue
        << " (" << recovery_ops_active << "/" << cct->_conf->
        osd_recovery_max_active << " rops)" << dendl;
...
{
    return local_reserver.has_reservation()
        || remote_reserver.has_reservation();
}
...
const char **OSD::get_tracked_conf_keys() const const
{
                             ...
                             const std::set < std::string > &changed)
{
    ...
    if (changed.count("osd_min_recovery_priority")) {
        service.local_reserver.set_min_priority(cct->_conf->
                                                osd_min_recovery_priority);
        service.remote_reserver.set_min_priority(cct->_conf->
                                                 osd_min_recovery_priority);
    }
    ...
    if (changed.count("osd_op_complaint_time") ||
        ...
        op_tracker.set_complaint_and_threshold(cct->_conf->
                                               osd_op_complaint_time,
                                               cct->_conf->
                                               osd_op_log_threshold);
    }
    if (changed.count("osd_op_history_size") ||
        ...
        op_tracker.set_history_size_and_duration(cct->_conf->
                                                 osd_op_history_size,
                                                 cct->_conf->
                                                 osd_op_history_duration);
    }
    ...
    if (changed.count("clog_to_monitors") ||
        ...
        changed.count("host") || changed.count("fsid")) {
        update_log_config();
    }
...
void OSD::check_config()
{
    ...
    if (cct->_conf->osd_map_cache_size <= cct->_conf->osd_map_max_advance + 2) {
        clog->warn() << "osd_map_cache_size (" << cct->_conf->
            osd_map_cache_size << ")" << " is not > osd_map_max_advance (" <<
    ...
    if (cct->_conf->osd_map_cache_size <=
        ...
        clog->warn() << "osd_map_cache_size (" << cct->_conf->
            osd_map_cache_size << ")" <<
            " is not > osd_pg_epoch_persisted_max_stale (" << cct->_conf->
            osd_pg_epoch_persisted_max_stale << ")";
    }
}
...
void OSD::set_disk_tp_priority()
{
    dout(10) << __func__
        ...
        << " priority " << cct->_conf->osd_disk_thread_ioprio_priority << dendl;
    ...
    if (cls < 0) {
        derr << __func__ << cpp_strerror(cls) << ": "
            << "osd_disk_thread_ioprio_class is " << cct->_conf->
            osd_disk_thread_ioprio_class <<
        ...
        case CEPH_OSD_OP_CALL:
            {
                bufferlist::iterator bp =
                    const_cast < bufferlist & >(iter->indata).begin();
                ...
                if (r) {
                    derr << "class " << cname << " open got " << cpp_strerror(r)
                        << dendl;
                    if (r == -ENOENT)
                        r = -EOPNOTSUPP;
        ...
        while (p != sdata->pg_slots.end()) {
            ...
            if (!slot.to_process.empty() && slot.num_running == 0) {
                if (osdmap->is_up_acting_osd_shard(p->first, whoami)) {
                    dout(20) << __func__ << "  " << p->
                        first << " maps to us, keeping" << dendl;
                    ++p;
                    continue;
                }
                while (!slot.to_process.empty() &&
                       slot.to_process.front().get_map_epoch() <=
                       osdmap->get_epoch()) {
                    auto & qi = slot.to_process.front();
                    dout(20) << __func__ << "  " << p->first
                        << " item " << qi << " epoch " << qi.get_map_epoch()
                        << " <= " << osdmap->get_epoch()
            ...
            if (slot.to_process.empty() && slot.num_running == 0 && !slot.pg) {
                dout(20) << __func__ << "  " << p->
                    first << " empty, pruning" << dendl;
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d * hb)
{
    ...
    if (requeue_seq != slot.requeue_seq) {
        dout(20) << __func__ << " " << item.first
            ...
            << requeue_seq << ", we raced with wake_pg_waiters" << dendl;
        if (pg) {
            pg->unlock();
        }
        sdata->sdata_op_ordering_lock.Unlock();
    ...
    if (pg && !slot.pg && !pg->deleting) {
        dout(20) << __func__ << " " << item.
            first << " set pg to " << pg << dendl;
        slot.pg = pg;
    }
    dout(30) << __func__ << " " << item.first << " to_process " << slot.
        to_process << " waiting_for_pg=" << (int)slot.waiting_for_pg << dendl;
    ...
    if (!pg) {
        ...
        if (osdmap->is_up_acting_osd_shard(item.first, osd->whoami)) {
            ...
            dout(20) << __func__ << " " << item.
                first << " no pg, item epoch is " << qi->
                get_map_epoch() << " > " << osdmap->get_epoch()
            ...
            dout(20) << __func__ << " " << item.
                first << " no pg, shouldn't exist," << " dropping " << *qi <<
                dendl;
            // share map with client?
            if (boost::optional < OpRequestRef > _op = qi->maybe_get_op()) {
                Session *session = static_cast < Session * >((*_op)->get_req()->
                                                             get_connection()->
                                                             get_priv());
                if (session) {
                    osd->maybe_share_map(session, *_op,
                                         sdata->waiting_for_pg_osdmap);
                    session->put();
                }
            }
...
void OSD::ShardedOpWQ::_enqueue(pair < spg_t, PGQueueable > item)
{
    ...
    if (priority >= osd->op_prio_cutoff)
        sdata->pqueue->enqueue_strict(item.second.get_owner(), priority, item);
    else
        sdata->pqueue->enqueue(item.second.get_owner(), priority, cost, item);
    sdata->sdata_op_ordering_lock.Unlock();
...
void OSD::ShardedOpWQ::_enqueue_front(pair < spg_t, PGQueueable > item)
{
    ...
    if (p != sdata->pg_slots.end() && !p->second.to_process.empty()) {
        ...
        dout(20) << __func__ << " " << item.first << " " << item.
            second << dendl;
    }
