...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::recover_object(const hobject_t & soid,
                                              eversion_t need)
{
    ...
    return maybe_pull_missing_obj(soid,
                                  need).then_interruptible([this, soid, need] {
                                                           logger().
                                                           debug
                                                           ("recover_object: loading obc: {}",
                                                            soid);
                                                           ...
                                                           (crimson::osd::PG::
                                                            ...
                                                            all_same_way([soid]
                                                                         ...
                                                                         ().
                                                                         error
                                                                         ("recover_object saw error code {}, ignoring object {}",
                                                                          code,
                                                                          soid);}
                                                            ));}
    );
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::maybe_push_shards(const hobject_t & soid,
                                                 eversion_t need)
{
    return seastar::do_with(get_shards_to_push(soid),
        ...
        .handle_exception_interruptible([this, soid] (auto e) {
                                        ...
                                        return seastar::
                                        make_exception_future <> (e);}
    ) ;
}

RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::maybe_pull_missing_obj(const hobject_t & soid,
                                                      eversion_t need)
{
    ...
    return interruptor::make_interruptible(shard_services.
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::push_delete(const hobject_t & soid,
                                           eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    ...
    return interruptor::parallel_for_each(pg.get_acting_recovery_backfill(),
                                          [this, soid, need,
                                           min_epoch] (pg_shard_t shard)
                                          ->interruptible_future <> {
                                          if (shard == pg.get_pg_whoami())
                                          return seastar::
                                          make_ready_future <> ();
                                          auto iter =
                                          pg.get_shard_missing().find(shard);
                                          if (iter ==
                                              pg.get_shard_missing().end())
                                          return seastar::
                                          make_ready_future <> ();
                                          if (iter->second.is_missing(soid)) {
                                          logger().
                                          debug
                                          ("push_delete: will remove {} from {}",
                                           soid, shard);
                                          pg.begin_peer_recover(shard, soid);
                                          spg_t target_pg(pg.get_info().pgid.
                                                          pgid, shard.shard);
                                          auto msg =
                                          crimson::make_message <
                                          MOSDPGRecoveryDelete >
                                          (pg.get_pg_whoami(), target_pg,
                                           pg.get_osdmap_epoch(), min_epoch);
                                          msg->set_priority(pg.
                                                            get_recovery_op_priority
                                                            ());
                                          msg->objects.
                                          push_back(std::make_pair(soid, need));
                                          return interruptor::
                                          make_interruptible(shard_services.
                                                             send_to_osd(shard.
                                                                         osd,
                                                                         std::
                                                                         move
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::handle_recovery_delete(Ref <
                                                      MOSDPGRecoveryDelete > m)
{
    ...
    return local_recover_delete(p.first, p.second, pg.get_osdmap_epoch())
        .then_interruptible([this, m] {
                            auto reply =
                            crimson::make_message < MOSDPGRecoveryDeleteReply >
                            (); reply->from = pg.get_pg_whoami();
                            reply->set_priority(m->get_priority());
                            reply->pgid =
                            spg_t(pg.get_info().pgid.pgid, m->from.shard);
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::on_local_recover_persist(const hobject_t & soid,
                                                        ...
                                                        epoch_t epoch_frozen)
{
    ...
    pg.get_recovery_handler()->on_local_recover(soid, _recovery_info, is_delete,
                                                t);
    logger().
        debug
        ("ReplicatedRecoveryBackend::on_local_recover_persist: do_transaction...");
    return interruptor::make_interruptible(shard_services.get_store().
                                           do_transaction(coll, std::move(t)))
        .
        then_interruptible([this, epoch_frozen, last_complete =
                            pg.get_info().last_complete] {
                           pg.get_recovery_handler()->
                           _committed_pushed_object(epoch_frozen,
                                                    last_complete);
                           return seastar::make_ready_future <> ();}
    );
}
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::local_recover_delete(const hobject_t & soid,
                                                    eversion_t need,
    ...
    return backend->load_metadata(soid).safe_then_interruptible([this]
                                                                ...
                                                                do_with(ceph::
                                                                        ...
                                                                        ([this,
                                                                         ...
                                                                         ().
                                                                         debug
                                                                         ("ReplicatedRecoveryBackend::local_recover_delete: do_transaction...");
                                                                         return
                                                                         shard_services.
                                                                         get_store
                                                                         ().
        ...
        .safe_then_interruptible([this, soid, epoch_to_freeze, need] {
                                 ...
                                 return on_local_recover_persist(soid,
                                                                 recovery_info,
                                                                 true,
                                                                 epoch_to_freeze);}
                                 ,
                                 PGBackend::load_metadata_ertr::
                                 all_same_way([this, soid, epoch_to_freeze,
                                               need] (auto e) {
                                              ObjectRecoveryInfo recovery_info;
                                              recovery_info.soid = soid;
                                              recovery_info.version = need;
                                              return
                                              on_local_recover_persist(soid,
                                                                       recovery_info,
                                                                       true,
                                                                       epoch_to_freeze);}
                                 )
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::recover_delete(const hobject_t & soid,
                                              eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    ...
    return seastar::do_with(object_stat_sum_t(),
                            ...
                            return local_recover_delete(soid, need,
                                                        cur_epoch).
                            then_interruptible([this, &stat_diff, cur_epoch,
                                                soid, need] ()
                                               ->interruptible_future <> {
                                               if (!pg.
                                                   has_reset_since(cur_epoch)) {
              bool object_missing = false; for (const auto & shard:pg.get_acting_recovery_backfill()) {
                                               if (shard == pg.get_pg_whoami())
                                               continue;
                                               if (pg.get_shard_missing(shard)->
                                                   is_missing(soid)) {
                                               logger().
                                               debug
                                               ("recover_delete: soid {} needs to deleted from replca {}",
                                                soid, shard);
                                               object_missing = true; break;}
                                               }

                                               if (!object_missing) {
                                               stat_diff.num_objects_recovered =
                                               1;
                                               return seastar::
                                               make_ready_future <> ();}
                                               else {
...
RecoveryBackend::interruptible_future < PushOp >
    ReplicatedRecoveryBackend::prep_push(const hobject_t & soid,
                                         eversion_t need, pg_shard_t pg_shard)
{
    ...
    return build_push_op(push_info.recovery_info,
                         ...
                         &push_info.stat).then_interruptible([this, soid,
                                                              pg_shard] (auto
                                                                         push_op)
                                                             {
                                                             auto &
                                             ...
                                             eversion_t need)
{
    ...
    pull_op.recovery_info.copy_subset.intersection_of(missing_iter->second.
                                                      clean_regions.
                                                      get_dirty_regions());
...
build_push_op(const ObjectRecoveryInfo & recovery_info,
              const ObjectRecoveryProgress & progress, object_stat_sum_t * stat)
{
    ...
    return seastar::do_with(ObjectRecoveryProgress(progress),
                            ...
                            PushOp(),[this, &recovery_info, &progress, stat]
                            (auto & new_progress, auto & available, auto & v,
                             auto & push_op) {
                            ...
                            then_interruptible([&](eversion_t local_ver) mutable
                                               ...
                                               logger().
                                               error
                                               ("build_push_op: {} push {} v{} failed because local copy is {}",
                                                pg.get_pgid(),
                                                recovery_info.soid,
                                                recovery_info.version,
                                                local_ver);
                                               ...
                                               (recovery_info.soid, progress,
                                                new_progress, available,
                                                &push_op);}
                            ).
                            then_interruptible([this, &recovery_info, &progress,
                                               ...
                                               logger().
                                               debug
                                               ("build_push_op: available: {}, copy_subset: {}",
                                                available,
                            ...
                            then_interruptible([&recovery_info, &v, &progress,
                                               ...
                                               new_progress.data_complete =
                                               true; if (stat)
                                               stat->num_objects_recovered++;}
                                               else
                                               if (progress.first
                                                   && progress.omap_complete) {
                                               // If omap is not changed, we need recovery omap
                                               // when recovery cannot be completed once
                                               new_progress.omap_complete =
                                               false;}
                                               if (stat) {
                                               stat->num_keys_recovered +=
                                               push_op.omap_entries.size();
                                               stat->num_bytes_recovered +=
                                               push_op.data.length();}
                                               push_op.version = v;
                                               push_op.soid =
                                               recovery_info.soid;
                                               ...
                                               new_progress;
                                               push_op.before_progress =
                                               progress;
                                               logger().
                                               debug
                                               ("build_push_op: push_op version:"
                                                " {}, push_op data length: {}",
                                                push_op.version,
                                                push_op.data.length());
                                               return seastar::
                                               make_ready_future < PushOp >
                                               (std::move(push_op));}
                            );}
    ) ;
}
...
RecoveryBackend::interruptible_future < eversion_t >
    ReplicatedRecoveryBackend::read_metadata_for_push_op(const hobject_t & oid,
                                                         const
                                                         ...
                                                         PushOp * push_op)
{
    ...
    return interruptor::make_interruptible(interruptor::
                                           when_all_succeed(backend->
                                                            omap_get_header
                                                            (coll,
                                                             ghobject_t(oid)).
                                                            handle_error_interruptible
                                                            ...
                                                            (crimson::os::
                                                             ...
                                                             all_same_way([oid]
                                                                          ...
                                                                          ().
                                                                          debug
                                                                          ("read_metadata_for_push_op, error {} when getting omap header: {}",
                                                                           e,
                                                                           oid);
                                                                          return
                                                                          seastar::
                                                            ...
                                                            (crimson::os::
                                                             ...
                                                             all_same_way([oid]
                                                                          ...
                                                                          ().
                                                                          debug
                                                                          ("read_metadata_for_push_op, error {} when getting attrs: {}",
                                                                           e,
                                                                           oid);
                                                                          return
                                                                          seastar::
        ...
        then_unpack_interruptible([&new_progress, push_op] (auto bl,
                                                            auto attrs) {
                                  if (bl.length() == 0) {
                                  logger().
                                  warn
                                  ("read_metadata_for_push_op: fail to read omap header");}
                                  else
                                  if (attrs.empty()) {
                                  logger().
                                  error
                                  ("read_metadata_for_push_op: fail to read attrs");
                                  return eversion_t {
                                  };}
  push_op->omap_header.claim_append(std::move(bl)); for (auto &&[key, val]:attrs)
                                  {
                                  push_op->attrset.emplace(std::move(key),
                                                           std::move(val));}
                                  logger().
                                  debug("read_metadata_for_push_op: {}",
                                        push_op->attrset[OI_ATTR]);
                                  object_info_t oi;
...
RecoveryBackend::interruptible_future < uint64_t >
    ReplicatedRecoveryBackend::read_object_for_push_op(const hobject_t & oid,
                                                       const interval_set <
                                                       ...
                                                       PushOp * push_op)
{
    ...
    return interruptor::make_interruptible(backend->fiemap(coll, ghobject_t {
                                                           oid}
                                                           ,
                                                           0,
                                                           copy_subset.
        ...
        safe_then_interruptible([ =, this] (auto && fiemap_included) mutable {
                                interval_set < uint64_t > extents; try {
                                extents.intersection_of(copy_subset,
                                                        std::
                                                        move(fiemap_included));}
                                catch(std::exception &) {
                                ...
                                push_op->data_included.span_of(extents, offset,
                                                               max_len);
                                ...
                                return interruptor::make_interruptible(store->
                                                                       ...
                                                                       (coll,
                                                                        ...
                                                                        data_included,
                                                                        0));}
    ).safe_then_interruptible([push_op, range_end =
                               copy_subset.range_end()](auto && bl) {
                              ...
                              all_same_way([](auto e) {
                                           logger().
                                           debug
                                           ("build_push_op: read exception");
                                           return seastar::
                                           make_exception_future < uint64_t >
                                           (e);}
                              )) ;
}
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::read_omap_for_push_op(const hobject_t & oid,
                                                     const
                                                     ObjectRecoveryProgress &
                                                     progress,
                                                     ObjectRecoveryProgress &
                                                     ...
                                                     PushOp * push_op)
{
    ...
    return seastar::repeat([&new_progress, &max_len, push_op, &oid, this] {
                           ...
                           omap_get_values(coll, ghobject_t {
                                           oid}
                                           ,
                                           nullopt_if_empty(new_progress.
                                                            omap_recovered_to)
                           ).
                           safe_then([&new_progress, &max_len,
                                      push_op] (const auto & ret) {
          ...
          for (const auto &[key, value]:kvs)
                                     ...
                                     std::rbegin(push_op->omap_entries)->first;}
                                     if (done) {
                                     new_progress.omap_complete = true;}
                                     return seastar::make_ready_future <
                                     seastar::stop_iteration >
                                     (stop ? seastar::stop_iteration::
                                      yes : seastar::stop_iteration::no);}
                                     ,
...
std::vector < pg_shard_t >
    ReplicatedRecoveryBackend::
get_shards_to_push(const hobject_t & soid) constconst
{
    ...
    ReplicatedRecoveryBackend::handle_pull(Ref < MOSDPGPull > m)
{
    ...
    return seastar::do_with(m->take_pulls(),
                            [this, from = m->from] (auto & pulls) {
...
RecoveryBackend::interruptible_future < bool >
    ReplicatedRecoveryBackend::_handle_pull_response(pg_shard_t from,
                                                     PushOp & push_op,
                                                     PullOp * response,
                                                     ceph::os::Transaction * t)
{
    logger().
        debug("handle_pull_response {} {} data.size() is {} data_included: {}",
    ...
    if (pull_info.recovery_info.size == (uint64_t(-1))) {
        ...
        pull_info.recovery_info.copy_subset.intersection_of(push_op.
                                                            recovery_info.
                                                            copy_subset);
    }
    ...
    auto prepare_waiter =
        interruptor::make_interruptible(seastar::make_ready_future <> ());
    if (pull_info.recovery_progress.first) {
        prepare_waiter =
            pg.obc_loader.with_obc < RWState::RWNONE >
            (pull_info.recovery_info.soid,
             [&pull_info, &recovery_waiter, &push_op] (auto obc) {
             pull_info.obc = obc; recovery_waiter.obc = obc;
    ...
    return prepare_waiter.then_interruptible([this, &pull_info, &push_op, t,
                                              response] ()mutable {
                                             const bool first =
                                             pull_info.recovery_progress.first;
                                             pull_info.recovery_progress =
                                             push_op.after_progress;
                                             logger().
                                             debug
                                             ("new recovery_info {}, new progress {}",
                                             ...
                                             trim_pushed_data(pull_info.
                                                              ...
                                                              push_op.data);
                                             bool complete =
                                             pull_info.is_complete();
                                             bool clear_omap =
                                             !push_op.before_progress.
                                             omap_complete;
                                             return submit_push_data(pull_info.
                                                                     recovery_info,
                                                                     first,
                                                                     complete,
                                                                     clear_omap,
                                                                     ...
                                                                     t)
                                             .
                                             then_interruptible([this, response,
                                                                 &pull_info,
                                                                 &push_op,
                                                                 complete, t,
                                                                 bytes_recovered
    ...
    ReplicatedRecoveryBackend::handle_pull_response(Ref < MOSDPGPush > m)
{
    ...
    if (push_op.version == eversion_t()) {
        ...
        pg.get_recovery_handler()->on_failed_recover( {
                                                     ...
                                                     pull_info->recovery_info.
                                                     version);
        return seastar::make_exception_future <> (std::
                                                  runtime_error(fmt::
                                                                format
                            ...
                            return seastar::do_with(ceph::os::Transaction(),
                                                    m.get(),[this,
                                                             &response] (auto &
                                                                         ...
                                                                         auto &
                                                                         m) {
                                                    pg_shard_t from = m->from; PushOp & push_op = m->pushes[0]; // only one push per message for now
                                                    return
                                                    _handle_pull_response(from,
                                                                          push_op,
                                                                          &response,
                                                                          &t).
                                                    then_interruptible([this,
                                                                       ...
                                                                       logger().
                                                                       debug
                                                                       ("ReplicatedRecoveryBackend::handle_pull_response: do_transaction...");
                                                                       return
                                                                       shard_services.
                                                                       get_store
                                                                       ().
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::_handle_push(pg_shard_t from,
                                            PushOp & push_op,
                                            ...
                                            ceph::os::Transaction * t)
{
    ...
    return submit_push_data(push_op.recovery_info, first, complete, clear_omap,
        ...
        .then_interruptible([this, complete, &push_op, t] {
                            if (complete) {
                            pg.get_recovery_handler()->on_local_recover(push_op.
                                                                        recovery_info.
                                                                        soid,
                                                                        push_op.
                                                                        recovery_info,
    ...
    return seastar::do_with(PushReplyOp(),[this, m] (auto & response) {
                            ...
                            return seastar::do_with(ceph::os::Transaction(),
                                                    ...
                                                    then_interruptible([this,
                                                                       ...
                                                                       logger().
                                                                       debug
                                                                       ("ReplicatedRecoveryBackend::handle_push: do_transaction...");
                                                                       return
                                                                       interruptor::
                                                                       make_interruptible
                                                                       (shard_services.
...
RecoveryBackend::interruptible_future < std::optional < PushOp >>
    ReplicatedRecoveryBackend::_handle_push_reply(pg_shard_t peer,
        ...
        if (!push_info.recovery_progress.data_complete && !error) {
            return build_push_op(push_info.recovery_info,
                ...
                then_interruptible([&push_info] (auto push_op) {
                                   ...
                                   return seastar::make_ready_future <
                                   std::optional <
                                   PushOp >> (std::move(push_op));}
            ).handle_exception_interruptible([recovering_iter, &push_info,
                                              peer] (auto e) {
                                             push_info.recovery_progress.error =
                                             true;
                                             recovering_iter->second->
                                             set_push_failed(peer, e);
                                             return seastar::make_ready_future <
                                             std::optional < PushOp >> ();}
            );
        }
    ...
    ReplicatedRecoveryBackend::handle_push_reply(Ref < MOSDPGPushReply > m)
{
    ...
    return _handle_push_reply(from, push_reply).then_interruptible([this,
                                                                    from] (std::
                                                                           optional
                                                                           <
                                                                           PushOp
...
std::pair < interval_set < uint64_t >,
    bufferlist >
    ReplicatedRecoveryBackend::trim_pushed_data(const interval_set < uint64_t >
                                                &copy_subset,
                                                ...
                                                ceph::bufferlist data_received)
{
    ...
    if (intervals_received.subset_of(copy_subset)) {
        return {
        intervals_received, data_received};
    }
  ...
  for (auto[have_start, have_len]:intervals_received) {
    ...
    return {
    intervals_usable, data_usable};
}

RecoveryBackend::interruptible_future < hobject_t >
    ReplicatedRecoveryBackend::
prep_push_target(const ObjectRecoveryInfo & recovery_info, bool first,
                 bool complete, bool clear_omap, ObjectStore::Transaction * t,
                 const map < string, bufferlist, less <>> &attrs, bufferlist
                 && omap_header)
{
    if (!first) {
    ...
    if (complete) {
        ...
        target_oid = ghobject_t(get_temp_recovery_object(recovery_info.soid,
                                                         recovery_info.
                                                         version));
        logger().debug("{}: Adding oid {} in the temp collection", __func__,
                       target_oid);
    ...
    return interruptor::make_interruptible(store->
        ...
        .then_interruptible([this, &recovery_info, t, target_oid] (auto st) {
                            ...
                            local_intervals_excluded; if (local_size) {
...
RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::
submit_push_data(const ObjectRecoveryInfo & recovery_info, bool first,
                 bool complete, bool clear_omap,
                 interval_set < uint64_t > &&data_zeros,
                 interval_set < uint64_t > &&intervals_included, bufferlist
                 && data_included, bufferlist
                 && omap_header, const map < string, bufferlist,
                 less <>> &attrs, map < string, bufferlist > &&omap_entries,
                 ObjectStore::Transaction * t)
{
    logger().debug("{}", __func__);
    return prep_push_target(recovery_info, first, complete,
                            clear_omap, t, attrs,
                            std::move(omap_header)).then_interruptible([this,
                                                                        &recovery_info,
                                                                        t,
                                                                        first,
                                                                        complete,
      ...
      uint64_t off = 0; for (auto[start, len]:                        intervals_included) {
                                                                       bufferlist
                                                                       bit;
                                                                       bit.
                                                                       substr_of
                                                                       ...
                                                                       logger().
                                                                       debug
                                                                       ("submit_push_data: Removing oid {} from the temp collection",
                                                                        target_oid);
                                                                       clear_temp_obj
                                                                       (target_oid);
                                                                       t->
...
void ReplicatedRecoveryBackend::
submit_push_complete(const ObjectRecoveryInfo & recovery_info,
                     ObjectStore::Transaction * t)
{
  for (const auto &[oid, extents]:recovery_info.clone_subset) {
      for (const auto &[off, len]:extents) {
            logger().debug(" clone_range {} {}~{}", oid, off, len);
            t->clone_range(coll->get_cid(), ghobject_t(oid),
                           ghobject_t(recovery_info.soid), off, len, off);
        }
    }
}

RecoveryBackend::interruptible_future <>
    ReplicatedRecoveryBackend::handle_recovery_delete_reply(Ref <
                                                  ...
                                                  ConnectionRef conn)
{
    ...
    case MSG_OSD_PG_PUSH_REPLY:
        return handle_push_reply(boost::static_pointer_cast < MOSDPGPushReply >
                                 (m));
    case MSG_OSD_PG_RECOVERY_DELETE:
        return handle_recovery_delete(boost::static_pointer_cast <
                                      MOSDPGRecoveryDelete > (m));
    case MSG_OSD_PG_RECOVERY_DELETE_REPLY:
        return handle_recovery_delete_reply(boost::static_pointer_cast <
                                            MOSDPGRecoveryDeleteReply > (m));
    default:
