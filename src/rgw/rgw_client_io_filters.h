...
public:
    template < typename U > AccountingFilter(CephContext *cct, U && decoratee)
        : DecoratedRestfulClient < T > (std::forward < U > (decoratee)),
          enabled(false), total_sent(0), total_received(0), cct(cct)
    {
    } size_t send_status(const int status,
                         const char *const status_name)override
    {
        const auto sent =
            DecoratedRestfulClient < T >::send_status(status,
                status_name);
    ...
    {
        const auto sent =
            DecoratedRestfulClient < T >::send_100_continue();
        lsubdout(cct, rgw,
    ...
    {
        const auto sent =
            DecoratedRestfulClient < T >::send_header(name, value);
        lsubdout(cct, rgw,
                 30) << "AccountingFilter::send_header: e=" << (enabled
                         ? "1" :
                         "0") <<
    ...
    {
        const auto sent =
            DecoratedRestfulClient < T >::send_content_length(len);
        lsubdout(cct, rgw,
    ...
    size_t send_chunked_transfer_encoding() override
    {
        const auto sent =
            ...
            T >::send_chunked_transfer_encoding();
        lsubdout(cct, rgw,
                 30) <<
    ...
    {
        const auto sent =
            DecoratedRestfulClient < T >::complete_header();
        lsubdout(cct, rgw,
    ...
    {
        const auto received =
            DecoratedRestfulClient < T >::recv_body(buf, max);
        lsubdout(cct, rgw,
                 30) << "AccountingFilter::recv_body: e=" << (enabled ?
                         "1" : "0")
    ...
    {
        const auto sent =
            DecoratedRestfulClient < T >::send_body(buf, len);
        lsubdout(cct, rgw,
                 30) << "AccountingFilter::send_body: e=" << (enabled ?
                         "1" : "0")
                     << ", sent=" << sent << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
    ...
    {
        const auto sent =
            DecoratedRestfulClient < T >::complete_request();
        lsubdout(cct, rgw,
...
public:
    template < typename U > BufferingFilter(CephContext *cct, U && decoratee)
        :
        DecoratedRestfulClient < T > (std::forward < U > (decoratee)),
        has_content_length(false), buffer_data(false), cct(cct)
    {
    }
...
template < typename T >
size_t BufferingFilter <
T >::send_content_length(const uint64_t len)
{
...
template < typename T > size_t BufferingFilter < T >::complete_request()
{
    ...
    if (!has_content_length) {
        ...
        sent +=
            DecoratedRestfulClient <
            T >::send_content_length(data.length());
    ...
    if (buffer_data) {
        ...
        lsubdout(cct, rgw,
                 30) <<
                     "BufferingFilter::complete_request: buffer_data: sent=" <<
                     sent << dendl;
    }
...
BufferingFilter < T > add_buffering(CephContext *cct, T && t)
{
...
public:
    template < typename U > explicit ChunkingFilter(U && decoratee)
        :
        DecoratedRestfulClient < T > (std::forward < U > (decoratee)),
    ...
    size_t send_chunked_transfer_encoding() override
    {
        ...
        return DecoratedRestfulClient <
               T >::send_header("Transfer-Encoding", "chunked");
    }
    ...
    size_t send_body(const char *buf, const size_t len)override
    {
        if (!chunking_enabled) {
            ...
            const auto chunk_size_len =
                snprintf(chunk_size, sizeof(chunk_size),
                         "%zx\r\n", len);
            size_t sent = 0;

            sent +=
                DecoratedRestfulClient < T >::send_body(chunk_size,
                    chunk_size_len);
...
public:
                       ...
                       const char *const status_name) override
    {
        ...
        return DecoratedRestfulClient < T >::send_status(status,
                status_name);
    }
                       ...
                       const std::string_view &value) override
    {
        switch (phase) {
            ...
            case ReorderState::RGW_DATA:
                return DecoratedRestfulClient < T >::send_header(name,
                        value);
        }
...
public:
    template < typename U > explicit ReorderingFilter(U && decoratee)
        :
        DecoratedRestfulClient < T > (std::forward < U > (decoratee)),
    ...
    size_t complete_header() override
    {
        ...
        if (content_length) {
            sent +=
                DecoratedRestfulClient <
                T >::send_content_length(*content_length);
        }

        /* Header data in buffers are already counted. */
        for (const auto &kv : headers) {
            sent +=
                DecoratedRestfulClient < T >::send_header(kv.first,
                    kv.second);
        }
        headers.clear();
