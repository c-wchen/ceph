...
class PGStatService {
   ...
   */
    ...
    virtual const osd_stat_t & get_osd_sum() const {
        ...
        ceph_abort();
    } virtual const mempool::pgmap::unordered_map < int32_t,
        osd_stat_t > &get_osd_stat() const {
        ceph_abort();
    ...
    virtual void dump_fs_stats(stringstream * ss, Formatter * f,
                               bool verbose) const = 0;
    virtual void dump_pool_stats(const OSDMap & osdm, stringstream * ss,
                                 Formatter * f, bool verbose) const = 0;
...
class MonPGStatService:virtual public PGStatService {
   ...
   */
    virtual unsigned maybe_add_creating_pgs(epoch_t scan_epoch,
                                            const mempool::osdmap::map <
                                            int64_t, pg_pool_t > &pools,
                                            creating_pgs_t *
                                            pending_creates) const {
        ceph_abort();
        return 0;
    }
  /**
