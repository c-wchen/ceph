...
class ContDesc {
  public:
    ...
    unsigned seqnum;
     ...
     ContDesc(): objnum(0), cursnap(0), seqnum(0), prefix("") {
    } ContDesc(int objnum,
               int cursnap,
               unsigned seqnum,
               const std::string & prefix):objnum(objnum), cursnap(cursnap),
...
virtual void get_ranges_map(const ContDesc & cont, std::map < uint64_t,
                            uint64_t > &out) = 0;
  ...
  public:
    ...
    class iterator_impl:public ContentsGenerator::iterator_impl {
      public:
        ...
        char current;
         iterator_impl(const ContDesc & cont, RandGenerator * cont_gen):pos(0),
            cont(cont), rand(cont.seqnum), cont_gen(cont_gen) {
            current = rand();
        } ContDesc get_cont() const override {
            return cont;
    ...
    ContentsGenerator::iterator_impl *
        dup_iterator_impl(const ContentsGenerator::iterator_impl *
                          in) override {
        ContentsGenerator::iterator_impl * retval =
            get_iterator_impl(in->get_cont());
...
class VarLenGenerator:public RandGenerator {
  ...
  public:
     VarLenGenerator(uint64_t length, uint64_t min_stride_size,
                     uint64_t max_stride_size):max_length(length),
        min_stride_size(min_stride_size), max_stride_size(max_stride_size) {
    } void get_ranges_map(const ContDesc & cont, std::map < uint64_t,
...
class AppendGenerator:public RandGenerator {
    ...
    uint64_t round_up(uint64_t in, uint64_t by) {
     ...
     AppendGenerator(uint64_t off,
                     uint64_t alignment,
                     uint64_t min_append_size,
                     uint64_t _max_append_size,
                     uint64_t max_append_multiple):off(off),
        alignment(alignment),
...
class ObjectDesc {
  public:
    ObjectDesc()
    :exists(false), dirty(false), version(0) {
    } ObjectDesc(const ContDesc & init, ContentsGenerator * cont_gen)
    :exists(false), dirty(false), version(0) {
        layers.push_front(std::pair < ceph::shared_ptr < ContentsGenerator >,
                          ContDesc > (ceph::shared_ptr < ContentsGenerator >
                                      (cont_gen), init));
    }
    ...
    class iterator {
      public:
        ...
        class ContState {
          ...
          public:
            ...
            bool covers(uint64_t pos) {
                return ranges.contains(pos) || (!ranges.starts_after(pos)
                                                && pos >= size);
            }

            uint64_t next(uint64_t pos) {
                assert(!covers(pos));
                return ranges.starts_after(pos) ? ranges.
                    start_after(pos) : size;
            }
        ...
        std::list < std::pair < std::list < ContState >::iterator,
            StackState > >stack;
        std::list < ContState >::iterator current;

        explicit iterator(ObjectDesc & obj):
            pos(0),
            size(obj.layers.begin()->first->
                 get_length(obj.layers.begin()->second)), cur_valid_till(0) {
          for (auto && i:obj.layers) {
                layers.push_back( {
                                 i.second, i.first,
                                 i.first->get_iterator(i.second)}
                );
            }
        ...
        bool check_bl_advance(bufferlist & bl, uint64_t * error_at = nullptr) {
            ...
            while (off < bl.length()) {
                assert(cur_valid_till >= pos);
                uint64_t next =
                    std::min(bl.length() - off, cur_valid_till - pos);
