...
public:
    ...
    static ceph::unique_leakable_ptr<buffer::raw> create(unsigned len,
            unsigned align,
...
template<bool is_const> buffer::list::iterator_impl<is_const>::iterator_impl(bl_t *l, unsigned o)
    : bl(l), ls(&bl->_buffers), p(ls->begin()), off(0), p_off(0)
{
    *this += o;
}

template<bool is_const> buffer::list::iterator_impl<is_const>::iterator_impl(const buffer::list::iterator &i)
...
template<bool is_const>
buffer::list::iterator_impl<is_const> &buffer::list::iterator_impl<is_const>::operator++()
{
    if (p == ls->end()) {
        throw end_of_buffer();
    }
