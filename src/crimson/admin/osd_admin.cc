 ...
 */ class OsdStatusHook:public AdminSocketHook {
      public:
        explicit OsdStatusHook(const crimson::osd::OSD & osd):AdminSocketHook {
        "status", "", "OSD status"}, osd(osd) {
        }
 ...
 */
    class SendBeaconHook:public AdminSocketHook {
      public:
        explicit SendBeaconHook(crimson::osd::OSD & osd):AdminSocketHook {
        "send_beacon", "", "send OSD beacon to mon immediately"}, osd(osd) {
        }
        seastar::future < tell_result_t > call(const cmdmap_t &,
                                               std::string_view format,
                                               ceph::bufferlist
 ...
 */
    class FlushPgStatsHook:public AdminSocketHook {
      public:
        explicit FlushPgStatsHook(crimson::osd::
                                  OSD & osd):AdminSocketHook("flush_pg_stats",
                                                             "",
                                                             "flush pg stats"),
            osd {
        osd} {
    ...
    template std::unique_ptr < AdminSocketHook > make_asok_hook <
        FlushPgStatsHook > (crimson::osd::OSD & osd);

/// dump the history of PGs' peering state
    class DumpPGStateHistory final:public AdminSocketHook {
      public:
        explicit DumpPGStateHistory(const crimson::osd::
                                    PGShardManager &
                                    pg_shard_manager):AdminSocketHook {
        ...
        seastar::future < tell_result_t > call(const cmdmap_t &,
            ...
            return pg_shard_manager.for_each_pg([f] (auto & pgid, auto & pg) {
                                                ...
                                                const auto & peering_state =
                                                pg->get_peering_state();
                                                f->dump_string("currently",
                                                               peering_state.
                                                               get_current_state
                                                               ());
                ...
                then([fref = std::move(fref)] ()mutable {
                     fref->close_section(); fref->close_section();
 ...
 */
    class AssertAlwaysHook:public AdminSocketHook {
      public:
        AssertAlwaysHook():AdminSocketHook {
        "assert", "", "asserts"} {
        }
        seastar::future < tell_result_t > call(const cmdmap_t &,
                                               std::string_view format,
 ...
 */
    class DumpMetricsHook:public AdminSocketHook {
      public:
        DumpMetricsHook():AdminSocketHook("dump_metrics",
                                          ...
                                          "dump current configured seastar metrics and their values")
        {
            ...
            return seastar::do_with(std::move(prefix),[f] (auto & prefix) {
                  ...
                  for (const auto &[full_name, metric_family]:     seastar::
                                                                         scollectd::
                                                                         get_value_map
                                                                         ()) {
                                                                    if (!prefix.
    ...
    static ghobject_t test_ops_get_object_name(const OSDMap & osdmap,
        ...
        auto[objname, nspace, raw_pg] =[&] {
            ...
            if (object_locator_t oloc(pool, nspace);
                osdmap.object_locator_to_pg(object_t(objname), oloc,
                                            raw_pg) < 0) {
                throw std::invalid_argument {
                "Invalid namespace/objname"};
            }
            return std::make_tuple(std::move(objname),
                                   std::move(nspace), std::move(raw_pg));
        }
        ();
        ...
        return ghobject_t {
            ...
            , CEPH_NOSNAP, raw_pg.ps(), pool, nspace}
            , ghobject_t::NO_GEN, shard_id_t {
...
//   injectdataerr <pool> [namespace/]<obj-name> [shardid]
    class InjectDataErrorHook:public AdminSocketHook {
      public:
        InjectDataErrorHook(crimson::osd::
                            ShardServices &
                            shard_services):AdminSocketHook("injectdataerr",
            ...
            try {
                obj =
                    test_ops_get_object_name(*shard_services.get_map(), cmdmap);
            } catch(const std::invalid_argument & e) {
                logger().info("error during data error injection: {}",
                              e.what());
            ...
            return shard_services.get_store().inject_data_error(obj).then([ =] {
                                                                          ...
                                                                          ().
                                                                          info
                                                                          ("successfully injected data error for obj={}",
                                                                           obj);
                                                                          ceph::
                                                                          bufferlist
                                                                          bl;
    ...
    template std::unique_ptr < AdminSocketHook > make_asok_hook <
        InjectDataErrorHook > (crimson::osd::ShardServices &);

// Usage:
//   injectmdataerr <pool> [namespace/]<obj-name> [shardid]
    class InjectMDataErrorHook:public AdminSocketHook {
      public:
        InjectMDataErrorHook(crimson::osd::
                             ShardServices &
                             shard_services):AdminSocketHook("injectmdataerr",
                                                             "name=pool,type=CephString "
                                                             "name=objname,type=CephObjectname "
                                                             "name=shardid,type=CephInt,req=false,range=0|255",
                                                             "inject data error to an object"),
            ...
            try {
                ...
                logger().info("error during metadata error injection: {}",
                              e.what());
                return seastar::make_ready_future < tell_result_t > (-EINVAL,
                                                                     e.what());
            }
            return shard_services.get_store().inject_mdata_error(obj).then([ =] {
                                                                           ...
                                                                           ().
                                                                           info
                                                                           ("successfully injected metadata error for obj={}",
                                                                            obj);
                                                                           ceph::
                                                                           bufferlist
                                                                           bl;
    ...
    template std::unique_ptr < AdminSocketHook > make_asok_hook <
        InjectMDataErrorHook > (crimson::osd::ShardServices &);
 ...
 */
    class DumpInFlightOpsHook:public AdminSocketHook {
      public:
        explicit DumpInFlightOpsHook(const crimson::osd::
                                     PGShardManager &
                                     pg_shard_manager):AdminSocketHook {
        ...
        seastar::future < tell_result_t > call(const cmdmap_t &,
            ...
            return pg_shard_manager.
                invoke_on_each_shard_seq([f] (const auto & shard_services) {
    ...
    class DumpHistoricOpsHook:public AdminSocketHook {
      public:
        explicit DumpHistoricOpsHook(const crimson::osd::
                                     OSDOperationRegistry &
                                     op_registry):AdminSocketHook {
        "dump_historic_ops", "", "show recent ops"}, op_registry(op_registry) {
        }
        seastar::future < tell_result_t > call(const cmdmap_t &,
                                               std::string_view format,
                                               ceph::bufferlist
    ...
    class DumpSlowestHistoricOpsHook:public AdminSocketHook {
      public:
        explicit DumpSlowestHistoricOpsHook(const crimson::osd::
                                            OSDOperationRegistry &
                                            op_registry):AdminSocketHook {
    ...
    class DumpRecoveryReservationsHook:public AdminSocketHook {
      public:
        explicit DumpRecoveryReservationsHook(crimson::osd::
                                              ShardServices &
                                              shard_services):AdminSocketHook {
        ...
        seastar::future < tell_result_t > call(const cmdmap_t &,
            ...
            return seastar::do_with(std::move(f),[this] (auto && f) {
                                    ...
                                    then([&f, this] {
                                         ...
                                         ("remote_reservations");
                                         return shard_services.
                                         remote_dump_reservations(f.get()).
                                         then([&f] {
                                              f->close_section();
                                              f->close_section();
                                              return seastar::
                                              make_ready_future <
                                              tell_result_t > (std::move(f));}
                                         );}
                                    );}
            );
        }
