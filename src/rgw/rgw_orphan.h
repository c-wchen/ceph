...
struct RGWOrphanSearchStage {
    ...
    RGWOrphanSearchStage(RGWOrphanSearchStageId _stage, int _shard, const std::string& _marker) : stage(_stage),
...
public:
    ...
    int store_entries(const DoutPrefixProvider *dpp, const std::string& oid, const std::map < std::string,
                      bufferlist > & entries);
...
public:
    RGWOrphanSearch(rgw::sal::RadosStore* _store, int _max_ios, uint64_t _stale_secs) : store(_store), orphan_store(store),
