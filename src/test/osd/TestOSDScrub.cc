...
class TestOSDScrub:public OSD {
  ...
  public:
    TestOSDScrub(CephContext * cct_,
                 ...
                 const std::string & jdev):OSD(cct_, store_, id, internal,
                                               external, hb_front_client,
                                               hb_back_client, hb_front_server,
                                               hb_back_server, osdc_messenger,
                                               mc, dev, jdev) {
...
TEST(TestOSDScrub, scrub_time_permit)
{
    ...
    TestOSDScrub *osd =
        new TestOSDScrub(g_ceph_context, store, 0, ms, ms, ms, ms, ms, ms, ms,
                         &mc, "", "");
