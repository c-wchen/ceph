...
class ClusterState {
  ...
  public:
    ...
    const bufferlist & get_health() const {
        return health_json;
    } const bufferlist & get_mon_status() const {
        return mon_status_json;
    ...
    bool have_fsmap() const {
        ...
        Mutex::Locker l(lock);
        std::forward < Callback > (cb) (servicemap,
                                        std::forward < Args > (args) ...);
    } template < typename Callback, typename ... Args > void with_fsmap(Callback
                                                                        && cb,
                                                                        Args
                                                                        && ...
                                                                        args)
        const {
        Mutex::Locker l(lock);
        std::forward < Callback > (cb) (fsmap,
                                        std::forward < Args > (args) ...);
    } template < typename Callback,
        typename ... Args > void with_mgrmap(Callback && cb, Args
                                             && ... args) const {
        Mutex::Locker l(lock);
        ...
        typename ... Args > auto with_pgservice(Callback && cb, Args
                                                && ... args) const->
    ...
    template < typename ... Args > void with_monmap(Args && ... args) const {
        Mutex::Locker l(lock);
        assert(monc != nullptr);
