        ...
        if (pending_prepare.count(reqid)) {
            ...
            *pending_prepare[reqid].ptid = tid;
            ...
            MMDSTableRequest *req =
                new MMDSTableRequest(table, TABLESERVER_OP_ROLLBACK, 0, tid);
        ...
        if (pending_commit.count(tid) &&
            ...
            mds->mdlog->
                start_submit_entry(new
                                   ETableClient(table, TABLESERVER_OP_ACK, tid),
...
void MDSTableClient::_prepare(bufferlist & mutation, version_t * ptid,
                              bufferlist * pbl,
                              MDSInternalContextBase * onfinish)
{
    if (last_reqid == ~0ULL) {
        dout(10) << "tableserver is not ready yet, waiting for request id" <<
            dendl;
        waiting_for_reqid.
            push_back(_pending_prepare(onfinish, ptid, pbl, mutation));
        return;
    }
    ...
    if (server_ready) {
        // send message
        MMDSTableRequest *req =
            new MMDSTableRequest(table, TABLESERVER_OP_PREPARE, reqid);
        req->bl = mutation;
        mds->send_message_mds(req, mds->get_mds_map()->get_tableserver());
    }
    else
...
void MDSTableClient::commit(version_t tid, LogSegment * ls)
{
    ...
    if (server_ready) {
        // send message
        MMDSTableRequest *req =
            new MMDSTableRequest(table, TABLESERVER_OP_COMMIT, 0, tid);
        mds->send_message_mds(req, mds->get_mds_map()->get_tableserver());
    }
    else
        dout(10) << "tableserver is not ready yet, deferring request" << dendl;
...
void MDSTableClient::resend_commits()
{
    for (map < version_t, LogSegment * >::iterator p = pending_commit.begin();
         p != pending_commit.end(); ++p) {
        dout(10) << "resending commit on " << p->first << dendl;
        MMDSTableRequest *req =
            new MMDSTableRequest(table, TABLESERVER_OP_COMMIT, 0, p->first);
        mds->send_message_mds(req, mds->get_mds_map()->get_tableserver());
    }
}
...
void MDSTableClient::resend_prepares()
{
    ...
    for (map < uint64_t, _pending_prepare >::iterator p =
         pending_prepare.begin(); p != pending_prepare.end(); ++p) {
        dout(10) << "resending prepare on " << p->first << dendl;
        MMDSTableRequest *req =
            new MMDSTableRequest(table, TABLESERVER_OP_PREPARE, p->first);
