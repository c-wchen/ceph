...
struct SyncRequest: public Request {
    ...
    explicit SyncRequest(client_id _id, Time started, Cost cost,
                         ...
                         ReqState &_state,
                         GetClientCounters &counters): Request {
        _id, started, cost}, req_mtx(mtx), req_cv(_cv), req_state(_state),
    counters(counters)
    {
    };
};
...
{
public:
    template < typename ... Args > SyncScheduler(CephContext *cct, GetClientCounters && counters,
            Args && ... args);
    ~SyncScheduler();
    ...
    static void handle_request_cb(const client_id &c,
...
private:
    ...
    using Queue =
        crimson::dmclock::PushPriorityQueue < client_id, SyncRequest,
        IsDelayed >;
...
template < typename ... Args > SyncScheduler::SyncScheduler(CephContext *cct, GetClientCounters && counters,
        Args && ... args):
    queue(std::forward < Args > (args) ...), cct(cct),
