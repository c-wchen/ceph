...
int DPDKDevice::init_port_start()
{
    ...
    if (_num_queues > 1) {
        ...
        port_conf.rx_adv_conf.rss_conf.rss_hf =
            _dev_info.flow_type_rss_offloads;
...
bool DPDKQueuePair::init_rx_mbuf_pool()
{
    ...
    if (!_pktmbuf_pool_rx) {
        ...
        int bufs_count =
            cct->_conf->ms_dpdk_rx_buffer_count_per_core - mbufs_per_queue_rx;
        int mz_flags = RTE_MEMZONE_1GB | RTE_MEMZONE_SIZE_HINT_ONLY;
        std::string mz_name = "rx_buffer_data" + std::to_string(_qid);
        const struct rte_memzone *mz =
            rte_memzone_reserve_aligned(mz_name.c_str(),
                                        mbuf_data_size * bufs_count,
                                        _pktmbuf_pool_rx->socket_id, mz_flags,
                                        mbuf_data_size);
        ceph_assert(mz);
        void *m = mz->addr;
       ...
       _tx_gc_poller(this)
{
    ...
    plb.add_u64_counter(l_dpdk_qp_rx_bytes, "dpdk_receive_bytes",
                        "DPDK received bytes", NULL, 0, unit_t(UNIT_BYTES));
    plb.add_u64_counter(l_dpdk_qp_tx_bytes, "dpdk_send_bytes",
                        "DPDK sendd bytes", NULL, 0, unit_t(UNIT_BYTES));
    ...
    plb.add_u64_counter(l_dpdk_qp_rx_copy_bytes, "dpdk_receive_copy_bytes",
                        "DPDK received copy bytes", NULL, 0,
                        unit_t(UNIT_BYTES));
    plb.add_u64_counter(l_dpdk_qp_tx_copy_bytes, "dpdk_send_copy_bytes",
                        "DPDK send copy bytes", NULL, 0, unit_t(UNIT_BYTES));
