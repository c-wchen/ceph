...
namespace librbd {
    namespace cache {
        ...
        template < typename I >
            ...
            Context *ctx = new LambdaContext([this, on_finish] (int r){
             ...
             cache::pwl::ShutdownRequest < I > *req =
                cache::pwl::ShutdownRequest < I >::create(*m_image_ctx,
                                                          m_image_cache,
                                                          m_plugin_api, ctx);
             req->send();
        } template < typename I >
            bool WriteLogImageDispatch < I >::read(io::AioCompletion * aio_comp,
                                                   io::Extents
                                                   && image_extents,
                                                   io::ReadResult
                                                   && read_result,
            ...
            *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
            ...
            m_plugin_api.update_aio_comp(aio_comp, 1, read_result,
                                         image_extents);

            auto *req_comp =
                m_plugin_api.create_image_read_request(aio_comp, 0,
                                                       image_extents);

            m_image_cache->read(std::move(image_extents),
                                &req_comp->bl, op_flags, req_comp);
            return true;
        }

        template < typename I >
            bool WriteLogImageDispatch <
            I >::write(io::AioCompletion * aio_comp, io::Extents
            ...
            *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
            ...
            io::C_AioRequest * req_comp =
                m_plugin_api.create_aio_request(aio_comp);
            m_image_cache->write(std::move(image_extents), std::move(bl),
                                 op_flags, req_comp);
            return true;
        }
        ...
        template < typename I >
            bool WriteLogImageDispatch <
            I >::discard(io::AioCompletion * aio_comp, io::Extents
          ...
          for (auto & extent:image_extents) {
                ...
                m_image_cache->discard(extent.first, extent.second,
                                       discard_granularity_bytes, req_comp);
            }
            return true;
        }
        ...
        template < typename I >
            bool WriteLogImageDispatch <
            I >::write_same(io::AioCompletion * aio_comp, io::Extents
                            && image_extents, bufferlist
                            && bl, int op_flags,
                            const ZTracer::Trace & parent_trace, uint64_t tid,
                            std::atomic < uint32_t > *image_dispatch_flags,
        ...
        template < typename I >
            ...
            *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
            ...
            io::C_AioRequest * req_comp =
                m_plugin_api.create_aio_request(aio_comp);
            m_image_cache->compare_and_write(std::move(image_extents),
        ...
        template < typename I >
            bool WriteLogImageDispatch <
            I >::flush(io::AioCompletion * aio_comp,
        ...
        template < typename I >
            bool WriteLogImageDispatch <
            I >::list_snaps(io::AioCompletion * aio_comp, io::Extents
        ...
        template < typename I >
            bool WriteLogImageDispatch <
            I >::preprocess_length(io::AioCompletion * aio_comp,
