...
SnapshotCreateRequest < I >::SnapshotCreateRequest(I &image_ctx,
    ...
    : Request < I > (image_ctx, on_finish, journal_op_tid),
      ...
      m_ignore_notify_quiesce_error(flags &
...
void SnapshotCreateRequest < I >::send_notify_quiesce()
{
    ...
    image_ctx.image_watcher->notify_quiesce(&m_request_id, m_prog_ctx,
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_notify_quiesce(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
    ...
    if (*result < 0 && !m_ignore_notify_quiesce_error) {
        lderr(cct) << "failed to notify quiesce: " <<
                   cpp_strerror(*result)
                   << dendl;
        save_result(result);
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_suspend_requests(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
void SnapshotCreateRequest < I >::send_suspend_aio()
{
    ...
    block_writes(create_context_callback < SnapshotCreateRequest <
                 I >,
                 &SnapshotCreateRequest < I >::handle_suspend_aio >
                 (this));
}

template < typename I >
Context *SnapshotCreateRequest <
I >::handle_suspend_aio(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
void SnapshotCreateRequest < I >::send_allocate_snap_id()
{
    ...
    image_ctx.data_ctx.aio_selfmanaged_snap_create(&m_snap_id,
             rados_completion);
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_allocate_snap_id(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
          5) << this << " " << __func__ << ": r=" << *result << ", " <<
             "snap_id=" << m_snap_id << dendl;

    if (*result < 0) {
        lderr(cct) << "failed to allocate snapshot id: " <<
                   cpp_strerror(*result)
                   << dendl;
        save_result(result);
        return send_notify_unquiesce();
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_create_snap(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
Context *SnapshotCreateRequest < I >::send_create_image_state()
{
    ...
    auto req =
        mirror::snapshot::SetImageStateRequest < I >::create(&image_ctx,
            m_snap_id,
            create_context_callback
            <
            SnapshotCreateRequest
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_create_image_state(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
void SnapshotCreateRequest < I >::send_release_snap_id()
{
    ...
    image_ctx.data_ctx.aio_selfmanaged_snap_remove(m_snap_id,
             rados_completion);
    rados_completion->release();
}
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_release_snap_id(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
...
Context *SnapshotCreateRequest < I >::send_notify_unquiesce()
{
    ...
    image_ctx.image_watcher->notify_unquiesce(m_request_id,
             create_context_callback <
             SnapshotCreateRequest <
             I >,
             &SnapshotCreateRequest <
...
template < typename I >
Context *SnapshotCreateRequest <
I >::handle_notify_unquiesce(int *result)
{
    I &image_ctx = this->m_image_ctx;
    CephContext *cct = image_ctx.cct;
    ldout(cct,
    ...
    if (*result < 0) {
        lderr(cct) << "failed to notify unquiesce: " <<
                   cpp_strerror(*result)
                   << dendl;
        // ignore error
    }
...
void SnapshotCreateRequest < I >::update_snap_context()
{
    ...
    image_ctx.data_ctx.selfmanaged_snap_set_write_ctx(image_ctx.snapc.
             seq,
             image_ctx.snaps);
