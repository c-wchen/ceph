...
private:
    ...
    int decode_layered(std::set < int > &erased_chunks, std::map < int,
                       ceph::bufferlist > *chunks);

    int repair_one_lost_chunk(std::map < int,
                              ceph::bufferlist > &recovered_data,
                              std::set < int > &aloof_nodes, std::map < int,
                              ceph::bufferlist > &helper_data,
    ...
    void recover_type1_erasure(std::map < int, ceph::bufferlist > *chunks,
                               int x, int y, int z, int *z_vec, int sc_size);

    void get_uncoupled_from_coupled(std::map < int, ceph::bufferlist > *chunks,
                                    int x, int y, int z, int *z_vec,
                                    int sc_size);
