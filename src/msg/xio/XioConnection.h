...
class XioConnection:public Connection {
  ...
  private:
    ...
    std::atomic < int64_t > send = {
    0};
    std::atomic < int64_t > recv = {
    0};
    ...
    class CState {
      public:
        ...
        std::atomic < session_states > session_state = {
        0};
        std::atomic < session_startup_state > startup_state = {
        0};
        ...
        :features(0),
            ...
            connect_seq(0),
            global_seq(0),
            peer_global_seq(0), in_seq(0), out_seq_acked(0), flags(FLAG_NONE) {
        }
    ...
    struct EntityComp {
        ...
        // for external search by entity_inst_t(peer)
            bool operator() (const entity_inst_t & peer,
                             const XioConnection & c)const {
...
#if 0
        cout << "XioConnection::get " << this << " " << nref.
            load() << std::endl;
#endif
...
#if 0
        cout << "XioConnection::put " << this << " " << nref.
            load() << std::endl;
#endif
    }
  ...
  private:
    void send_keepalive_or_ack_internal(bool ack = false, const utime_t * tp =
                                        nullptr);
