...
int RGWGetObj_ObjStore_S3::get_decrypt_filter(std::unique_ptr < RGWGetDataCB >
                                              *filter, RGWGetDataCB * cb,
                                              bufferlist * manifest_bl)
{
    ...
    if (res == 0) {
        if (block_crypt != nullptr) {
            auto f =
                ceph::make_unique < RGWGetObj_BlockDecrypt > (s->cct, cb,
                                                              std::
                                                              move
                                                              (block_crypt));
...
int RGWPutObjTags_ObjStore_S3::get_params()
{
    ...
    auto data_deleter = std::unique_ptr < char, decltype(free) * > {
    data, free};
    ...
    obj_tags_s3 =
        static_cast < RGWObjTagSet_S3 * >(tagging->find_first("TagSet"));
    if (!obj_tags_s3) {
        return -ERR_MALFORMED_XML;
    }
...
void RGWGetUsage_ObjStore_S3::send_response()
{
    ...
    if (show_log_sum) {
        ...
        formatter->dump_int("TotalBytesRounded",
                            header.stats.total_bytes_rounded);
...
int RGWListBucket_ObjStore_S3::get_params()
{
    ...
    if (s->system_request) {
        ...
        if (shard_id_str) {
            ...
            if (!err.empty()) {
                ldout(s->cct,
                      5) << "bad shard id specified: " << shard_id_str << dendl;
                return -EINVAL;
            }
        }
        else {
...
int RGWSetBucketVersioning_ObjStore_S3::get_params()
{
    ...
    int len = 0;
    int r =
        rgw_rest_read_all_input(s, &data, &len,
                                s->cct->_conf->rgw_max_put_param_size, false);
    if (r < 0) {
        return r;
    }
...
#define VERSION_ID_STR "?versionId="
        ...
        ret = store->get_bucket_info(obj_ctx,
                                     ...
                                     copy_source_bucket_name,
                                     copy_source_bucket_info, NULL, &src_attrs);
        if (ret < 0) {
            ldout(s->cct,
                  5) << __func__ << "(): get_bucket_info() returned ret=" << ret
                << dendl;
            return ret;
        }
...
static inline int get_obj_attrs(RGWRados * store, struct req_state *s,
                                rgw_obj & obj, map < string,
                                bufferlist > &attrs)
{
...
int RGWPutObj_ObjStore_S3::get_decrypt_filter(std::unique_ptr < RGWGetDataCB >
                                              *filter, RGWGetDataCB * cb,
    ...
    if (res == 0) {
        if (block_crypt != nullptr) {
            auto f =
                ...
                (new
                 RGWGetObj_BlockDecrypt(s->cct, cb, std::move(block_crypt)));
...
{
    int res = 0;
    RGWPutObjProcessor_Multipart *multi_processor =
        dynamic_cast < RGWPutObjProcessor_Multipart * >(cb);
    if (multi_processor != nullptr) {
        ...
        if (mp != nullptr) {
            ...
            if (res == 0) {
                ...
                res =
                    rgw_s3_prepare_decrypt(s, xattrs, &block_crypt,
                                           crypt_http_responses);
                if (res == 0 && block_crypt != nullptr)
                    *filter =
                        std::unique_ptr < RGWPutObj_BlockEncrypt >
                        (new
                         RGWPutObj_BlockEncrypt(s->cct, cb,
        ...
        std::unique_ptr < BlockCrypt > block_crypt;
        res =
            rgw_s3_prepare_encrypt(s, attrs, nullptr, &block_crypt,
                                   crypt_http_responses);
        if (res == 0 && block_crypt != nullptr) {
            *filter =
                std::unique_ptr < RGWPutObj_BlockEncrypt >
                (new
                 RGWPutObj_BlockEncrypt(s->cct, cb, std::move(block_crypt)));
        }
    }
    return res;
}
...
int RGWPostObj_ObjStore_S3::get_tags()
{
    ...
    if (part_str(parts, "tagging", &tags_str)) {
        ...
        tagging =
            static_cast < RGWObjTagging_S3 * >(parser.find_first("Tagging"));
        obj_tags_s3 =
            static_cast < RGWObjTagSet_S3 * >(tagging->find_first("TagSet"));
        if (!obj_tags_s3) {
            return -ERR_MALFORMED_XML;
...
int RGWPostObj_ObjStore_S3::get_policy()
{
    if (part_bl(parts, "policy", &s->auth.s3_postobj_creds.encoded_policy)) {
         ...
         * method. */
        const int ret =
            rgw::auth::Strategy::apply(auth_registry_ptr->get_s3_post(), s);
        if (ret != 0) {
            return -EACCES;
        }
        else {
                                               ...
                                               RGWPutObjDataProcessor * cb)
{
    ...
    std::unique_ptr < BlockCrypt > block_crypt;
    res =
        rgw_s3_prepare_encrypt(s, attrs, &parts, &block_crypt,
                               crypt_http_responses);
    if (res == 0 && block_crypt != nullptr) {
        *filter =
            std::unique_ptr < RGWPutObj_BlockEncrypt >
...
int RGWDeleteObj_ObjStore_S3::get_params()
{
    ...
    if (if_unmod) {
        ...
        if (utime_t::parse_date(if_unmod_decoded, &epoch, &nsec) < 0) {
            ldout(s->cct,
                  10) << "failed to parse time: " << if_unmod_decoded << dendl;
            return -EINVAL;
        }
...
int RGWCopyObj_ObjStore_S3::get_params()
{
        ...
        if (!source_zone.empty()) {
            ...
            if (client_id.empty() || op_id.empty()) {
                ldout(s->cct, 0) <<
                    ...
                    "required for intra-region copy" << dendl;
                return -EINVAL;
            }
        }
    }
    ...
    const char *md_directive =
        s->info.env->get("HTTP_X_AMZ_METADATA_DIRECTIVE");
    if (md_directive) {
                                                     ...
                                                     bufferlist > &attrs)
{
    int res = 0;
    res =
        rgw_s3_prepare_encrypt(s, attrs, nullptr, nullptr,
                               crypt_http_responses);
    return res;
}
...
void RGWGetObjLayout_ObjStore_S3::send_response()
{
    ...
    for (auto miter = manifest->obj_begin(); miter != manifest->obj_end();
         ++miter) {
...
int RGWHandler_REST_S3::postauth_init()
{
    ...
    dout(10) << "s->object=" << (!s->object.empty()? s->
                                 object : rgw_obj_key("<NULL>"))
...
int RGWHandler_REST_S3Website::serve_errordoc(int http_ret,
...
int RGWHandler_REST_S3Website::error_handler(int err_no, string * error_content)
{
    ...
    ldout(s->cct,
        ...
        " http_ret=" << http_error_code << dendl;
    ...
    namespace auth {
        namespace s3 {
            ...
            static rgw::auth::Completer::cmplptr_t
                ...
                AWSGeneralAbstractor::get_auth_data(const req_state *
                                                    const s)const {
                AwsVersion version;
                AwsRoute route;
                ...
                AWSGeneralAbstractor::
                get_v4_canonical_headers(const req_info & info,
                                         const boost::
                                         string_view & signedheaders,
                                         const bool using_qs) const {
                return rgw::auth::s3::get_v4_canonical_headers(info,
                                                               signedheaders,
                                                               using_qs, false);
            } AWSEngine::VersionAbstractor::auth_data_t AWSGeneralAbstractor::
                get_auth_data_v4(const req_state * const s,
                                 /* FIXME: const. */
                                 bool using_qs)const {
                ...
                if (is_v4_payload_unsigned(exp_payload_hash)
                    ...
                    if (!is_v4_payload_streamed(exp_payload_hash)) {
                        ...
                        default:
                            dout(10) <<
                                "ERROR: AWS4 completion for this operation NOT IMPLEMENTED"
                                << dendl;
                        ...
                        dout(10) << "body content detected in multiple chunks"
                            << dendl;
                        ...
                        dout(10) << "aws4 seed signature ok... delaying v4 auth"
                            << dendl;
            ...
            boost::optional < std::string >
                ...
                if (!http_auth || http_auth[0] == '\0') {
                    ...
                    const boost::string_view auth_str(http_auth +
                                                      strlen("AWS "));
                ...
                if (!rgw_create_s3_canonical_header
                    (s->info, &header_time, string_to_sign, qsr)) {
                    ldout(cct,
            ...
            AWSEngine::VersionAbstractor::auth_data_t
                AWSBrowserUploadAbstractor::get_auth_data(const req_state *
                                                          const s)const {
                if (s->auth.s3_postobj_creds.x_amz_algorithm ==
                    AWS4_HMAC_SHA256_STR) {
                    ldout(s->cct,
                    ...
                    ldout(s->cct,
                          0) << "Signature verification algorithm AWS v2" <<
                        dendl;
                    return get_auth_data_v2(s);
                }
            }
    ...
    noexcept
{
    ...
    rgw_user(token.id),
            token.id,
            RGW_PERM_FULL_CONTROL, acct_privilege_t::IS_PLAIN_ACCT, TYPE_LDAP};
}

rgw::auth::Engine::result_t
    rgw::auth::s3::LDAPEngine::authenticate(const boost::
...
/* LocalEndgine */
rgw::auth::Engine::result_t
    rgw::auth::s3::LocalEngine::authenticate(const boost::
                                             ...
                                             const s) constconst
{
    ...
    ldout(cct, 15) << "string_to_sign=" << rgw::crypt_sanitize::log_content {
    string_to_sign}
    <<dendl;
    ldout(cct, 15) << "server signature=" << server_signature << dendl;
    ldout(cct, 15) << "client signature=" << signature << dendl;
    ldout(cct,
          15) << "compare=" << signature.compare(server_signature) << dendl;
