...
authorize_handler_service_registry(new
                                   AuthAuthorizeHandlerRegistry(m->cct,
                                                                ...
                                                                _conf->
                                                                auth_supported)),
name(n), messenger(m), monc(mc), mgrc(m->cct, m), log_client(m->cct, messenger,
                                                             &mc->monmap,
                                                             LogClient::
                                                             NO_FLAGS),
mds_rank(NULL), asok_hook(NULL), starttime(mono_clock::now())
{
    orig_argc = 0;
    orig_argv = NULL;
...
void MDSDaemon::dump_status(Formatter * f)
{
    ...
    if (mds_rank) {
        ...
        f->dump_unsigned("osdmap_epoch_barrier",
                         mds_rank->get_osd_epoch_barrier());
    }
    else {
...
void MDSDaemon::set_up_admin_socket()
{
    ...
    r = admin_socket->register_command("session evict",
                                       ...
                                       asok_hook, "Evict a CephFS client");
    assert(r == 0);
    ...
    r = admin_socket->register_command("get subtrees",
                                       "get subtrees",
                                       asok_hook, "Return the subtree map");
    assert(r == 0);
...
const char **MDSDaemon::get_tracked_conf_keys() const const
{
                                   ...
                                   const std::set < std::string > &changed)
{
    ...
    if (changed.count("mds_op_complaint_time") ||
        ...
        if (mds_rank) {
            mds_rank->op_tracker.set_complaint_and_threshold(conf->
    ...
    if (changed.count("mds_op_history_size") ||
        ...
        if (mds_rank) {
            mds_rank->op_tracker.set_history_size_and_duration(conf->
                                                               mds_op_history_size,
                                                               conf->
...
int MDSDaemon::init()
{
    ...
    dout(10) << sizeof(elist <
                       void *>::item) << "\t elist<>::item   *7=" << 7 *
        sizeof(elist < void *>::item) << dendl;
    ...
    dout(10) << sizeof(SimpleLock) << "\t SimpleLock   *5=" << 5 *
        sizeof(SimpleLock) << dendl;
    dout(10) << sizeof(ScatterLock) << "\t ScatterLock  *3=" << 3 *
        sizeof(ScatterLock) << dendl;
    ...
    dout(10) << sizeof(xlist <
                       void *>::item) << "\t xlist<>::item   *2=" << 2 *
        sizeof(xlist < void *>::item) << dendl;
...
void MDSDaemon::reset_tick()
{
    ...
    // schedule
    tick_event = timer.add_event_after(g_conf->mds_tick_interval,
                                   ...
                                   boost::string_view outs)
{
    ...
    const bool live_session =
        ...
        mds_rank && mds_rank->sessionmap.get_session(session->info.inst.name);
  ...
  {parsesig, helptext, module, perm, availability},
    ...
    COMMAND("injectargs "
        ...
        COMMAND("config set "
                ...
                "mds", "*", "cli,rest")
        COMMAND("exit", "Terminate this MDS", "mds", "*", "cli,rest")
        COMMAND("respawn", "Restart this MDS", "mds", "*", "cli,rest")
        COMMAND("session kill "
                "name=session_id,type=CephInt",
                "End a client session", "mds", "*", "cli,rest")
        ...
        COMMAND("heap "
                ...
                "mds", "*", "cli,rest")
        COMMAND("cache drop name=timeout,type=CephInt,range=0,req=false",
                "trim cache and optionally "
                "request client to release all caps and flush the journal",
                "mds", "r", "cli,rest")
};

int MDSDaemon::_handle_command(const cmdmap_t & cmdmap,
                               MCommand * m,
                               bufferlist * outbl,
                               std::string * outs,
                               Context ** run_later, bool * need_reply)
{
    assert(outbl != NULL);
    assert(outs != NULL);
    ...
    if (prefix == "get_command_descriptions") {
        ...
        for (MDSCommand * cp = mds_commands;
            ...
            dump_cmddesc_to_json(f.get(), secname.str(), cp->cmdstring,
...
void MDSDaemon::handle_mds_map(MMDSMap * m)
{
    ...
    dout(1) << "Updating MDS map to version " << epoch << " from " << m->
        get_source() << dendl;
    ...
    const MDSMap::DaemonState new_state =
        mdsmap->get_state_gid(mds_gid_t(monc->get_global_id()));
    const int incarnation =
        mdsmap->get_inc_gid(mds_gid_t(monc->get_global_id()));
    ...
    for (map < mds_gid_t, MDSMap::mds_info_t >::const_iterator p =
         oldmap->get_mds_info().begin(); p != oldmap->get_mds_info().end();
         ++p) {
        if (mdsmap->get_mds_info().count(p->first) == 0) {
            dout(10) << " peer mds gid " << p->
                first << " removed from map" << dendl;
            messenger->mark_down(p->second.addr);
        }
    }

    if (whoami == MDS_RANK_NONE && new_state == MDSMap::STATE_STANDBY_REPLAY) {
        whoami =
            mdsmap->get_mds_info_gid(mds_gid_t(monc->get_global_id())).
            standby_for_rank;
    }
    ...
    dout(10) << "map says I am " << addr << " mds." << whoami << "." <<
        incarnation << " state " << ceph_mds_state_name(new_state) << dendl;
        ...
        if (mds_rank != NULL) {
            ...
            if (g_conf->mds_enforce_unique_name) {
                if (mds_gid_t existing = mdsmap->find_mds_gid_by_name(name)) {
                    const MDSMap::mds_info_t & i =
                        mdsmap->get_info_gid(existing);
                    if (i.global_id > myid) {
                        dout(1) << "Map replaced me with another mds." << whoami
                            << " with gid (" << i.
                            global_id << ") larger than myself (" << myid <<
                            "); quitting!" << dendl;
            ...
            dout(1) << "Map removed me (mds." << whoami << " gid:"
                << myid << ") from cluster due to lost contact; respawning" <<
                dendl;
            respawn();
        }
                                     ...
                                     *challenge)
{
    ...
    if (is_valid) {
        ...
        if (!s) {
            ...
            s->info.inst.name = n;
            dout(10) << " new session " << s << " for " << s->info.
                inst << " con " << con << dendl;
            con->set_priv(s);
            s->connection = con;
            if (mds_rank) {
            ...
            dout(10) << " existing session " << s << " for " << s->info.
                inst << " existing con " << s->
                connection << ", new/authorizing con " << con << dendl;
            con->set_priv(s->get());
        ...
        if (caps_info.allow_all) {
            ...
            try {
                ...
                dout(10) << __func__ << ": parsing auth_cap_str='" <<
                    auth_cap_str << "'" << dendl;
                std::ostringstream errstr;
                if (!s->auth_caps.parse(g_ceph_context, auth_cap_str, &errstr)) {
                    dout(1) << __func__ << ": auth cap parse error: " << errstr.
                        str()
...
void MDSDaemon::ms_handle_accept(Connection * con)
{
    ...
    dout(10) << "ms_handle_accept " << con->
        get_peer_addr() << " con " << con << " session " << s << dendl;
    if (s) {
        if (s->connection != con) {
            dout(10) << " session connection " << s->
                connection << " -> " << con << dendl;
            s->connection = con;
