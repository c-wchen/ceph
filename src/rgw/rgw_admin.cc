...
int check_obj_locator_underscore(rgw::sal::Object* obj, bool fix, bool remove_bad, Formatter *f)
{
    ...
    if ((needs_fixing || remove_bad) && fix) {
        ret = static_cast < rgw::sal::RadosStore * > (driver)->getRados()->fix_head_obj_locator(dpp(),
              obj->get_bucket()->get_info(), needs_fixing, remove_bad, obj->get_key(), null_yield);
        if (ret < 0) {
...
int check_obj_tail_locator_underscore(RGWBucketInfo& bucket_info, rgw_obj_key& key, bool fix, Formatter *f)
{
    ...
    int ret = static_cast < rgw::sal::RadosStore * > (driver)->getRados()->fix_tail_obj_locator(dpp(), bucket_info, key,
              fix, &needs_fixing, null_yield);
    if (ret < 0) {
                     ...
                     Formatter *formatter)
{
    ...
    for (auto& hint_entity : hint_entities) {
        ...
        if (r < 0) {
            ldpp_dout(dpp(), 20) << "could not get bucket sync policy handler for hint bucket=" << hint_bucket << " ... skipping" <<
                                 dendl;
            continue;
        }
                            ...
                            std::ostream& out)
{
    ...
    if (!static_cast < rgw::sal::RadosStore * > (driver)->ctl()->bucket->bucket_imports_data(info.bucket, null_yield,
            dpp())) {
                              ...
                              std::ostream& out)
{
    ...
    for (auto& zone_id : zone_ids) {
        ...
        if (z == static_cast < rgw::sal::RadosStore * >
            (driver)->svc()->zone->get_zonegroup().zones.end()) { /* shouldn't happen */
            continue;
        }
                                ...
                                std::unique_ptr < rgw::sal::Bucket > * bucket)
{
    ...
    if (num_shards > (int)static_cast < rgw::sal::RadosStore * > (driver)->getRados()->get_max_bucket_shards()) {
        cerr << "ERROR: num_shards too high, max value: " << static_cast < rgw::sal::RadosStore * >
...
static int scan_totp(CephContext *cct, ceph::real_time& now, rados::cls::otp::otp_info_t &totp,
                     vector < string > & pins,
                     time_t *pofs)
{
...
static int trim_sync_error_log(int shard_id, const string& marker, int delay_ms)
{
    ...
    for (;;) {
        int ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->cls->timelog.trim(dpp(), oid, {}, {}, {}, marker,
                  nullptr,
                  null_yield);
        if (ret == -ENODATA) {
            return 0;
...
int main(int argc, const char **argv)
{
    ...
    for (std::vector < const char * >::iterator i = args.begin(); i != args.end();) {
        if (ceph_argparse_double_dash(args, i)) {
            ...
            // do nothing
        } else if (ceph_argparse_binary_flag(args, i, &check_head_obj_locator, NULL, "--check-head-obj-locator",
                                             (char *)NULL)) {
            // do nothing
            ...
            case OPT::ZONE_PLACEMENT_RM: {
                ...
                if (opt_cmd == OPT::ZONE_PLACEMENT_ADD ||
                    ...
                    if (ptiter->second.storage_classes.find(storage_class) == ptiter->second.storage_classes.end()) {
                        cerr << "ERROR: storage class '" << storage_class << "' is not defined in zonegroup '" << placement_id <<
                                "' placement target" << std::endl;
                        return EINVAL;
                    }
                    if (ptiter->second.tier_targets.find(storage_class) != ptiter->second.tier_targets.end()) {
                        cerr << "ERROR: storage class '" << storage_class << "' is of tier type in zonegroup '" << placement_id <<
                                "' placement target" << std::endl;
                        return EINVAL;
                    }
    ...
    if (print_warning_message) {
        cerr << "Please run the command on master zone. Performing this operation on non-master zone leads to inconsistent metadata between zones"
             << std::endl;
...
next:
    ...
    if (opt_cmd == OPT::OLH_READLOG) {
        ...
        ret = static_cast < rgw::sal::RadosStore * > (driver)->getRados()->bucket_index_read_olh_log(dpp(), bucket->get_info(),
              *state, obj->get_obj(), 0, &log, &is_truncated, null_yield);
        if (ret < 0) {
            cerr << "ERROR: failed reading olh: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::BI_GET) {
        ...
        ret = static_cast < rgw::sal::RadosStore * > (driver)->getRados()->bi_get(dpp(), bucket->get_info(), obj, bi_index_type,
              &entry, null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::BI_LIST) {
        ...
        for (; i < max_shards; i++) {
            ...
            do {
                ...
                ret = static_cast < rgw::sal::RadosStore * > (driver)->getRados()->bi_list(bs, object, marker, max_entries, &entries,
    ...
    if (opt_cmd == OPT::BI_PURGE) {
        ...
        if (ret == -ENOENT) {
            ...
            cerr << "ERROR: could not init current bucket info for bucket_name=" << bucket_name << ": " << cpp_strerror(
                     -ret) << std::endl;
            return -ret;
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
    ...
    if (opt_cmd == OPT::GC_LIST) {
        ...
        do {
            ...
            int ret = static_cast < rgw::sal::RadosStore * > (driver)->getRados()->list_gc_objs(&index, marker, 1000, !include_all,
                      result, &truncated, processing_queue);
            if (ret < 0) {
...
#define MAX_LC_LIST_ENTRIES 100
    ...
    if (opt_cmd == OPT::METADATA_GET) {
        int ret = static_cast < rgw::sal::RadosStore * > (driver)->ctl()->meta.mgr->get(metadata_key, formatter.get(),
                  null_yield, dpp());
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::METADATA_PUT) {
        ...
        ret = static_cast < rgw::sal::RadosStore * > (driver)->ctl()->meta.mgr->put(metadata_key, bl, null_yield, dpp(),
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_CHECKPOINT) {
        ...
        if (!static_cast < rgw::sal::RadosStore * > (driver)->ctl()->bucket->bucket_imports_data(bucket->get_key(), null_yield,
                dpp())) {
    ...
    if (opt_cmd == OPT::BILOG_LIST) {
        ...
        do {
            ...
            ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->bilog_rados->log_list(dpp(), bucket->get_info(),
                  log_layout, shard_id, marker, max_entries - count, entries, &truncated);
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::SYNC_ERROR_LIST) {
        ...
        for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
            ...
            do {
                list < cls_log_entry > entries;
                ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->cls->timelog.list(dpp(), oid, {}, {}, max_entries - count,
    ...
    if (opt_cmd == OPT::SYNC_GROUP_FLOW_CREATE) {
        ...
        if (symmetrical_flow_opt(*opt_flow_type)) {
            CHECK_TRUE(require_non_empty_opt(opt_zone_ids), "ERROR: --zones not provided for symmetrical flow, or is empty",
                       EINVAL);
    ...
    if (opt_cmd == OPT::BILOG_STATUS) {
        ...
        ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->bilog_rados->get_log_status(dpp(), bucket->get_info(),
              log_layout, shard_id,
              &markers, null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATALOG_AUTOTRIM) {
        ...
        ret = crs.run(dpp(), create_admin_data_log_trim_cr(dpp(), static_cast < rgw::sal::RadosStore* > (driver), &http,
                      num_shards, markers));
        if (ret < 0) {
    ...
    if (quota_op) {
        if (!bucket_name.empty()) {
            ...
            if (quota_scope == "bucket") {
                ...
                cerr << "ERROR: invalid quota scope specification. Please specify either --quota-scope=bucket, or --quota-scope=user" <<
                     std::endl;
                return EINVAL;
            }
    ...
    bool ratelimit_op_set = (opt_cmd == OPT::RATELIMIT_SET || opt_cmd == OPT::RATELIMIT_ENABLE ||
    ...
    if (ratelimit_op_set) {
        ...
        if (!bucket_name.empty()) {
            if (!ratelimit_scope.empty() && ratelimit_scope != "bucket") {
                cerr << "ERROR: invalid ratelimit scope specification. (bucket scope is not bucket but bucket has been specified)" <<
                     std::endl;
                return EINVAL;
            }
            ...
            if (ratelimit_scope == "user") {
                ...
                cerr << "ERROR: invalid ratelimit scope specification. Please specify either --ratelimit-scope=bucket, or --ratelimit-scope=user"
                     << std::endl;
                return EINVAL;
            }
        }
    ...
    if (opt_cmd == OPT::MFA_CREATE) {
        ...
        int ret = static_cast < rgw::sal::RadosStore * > (driver)->ctl()->meta.mgr->mutate(RGWSI_MetaBackend_OTP::get_meta_key(
                      user->get_id()),
                  mtime, &objv_tracker,
                  null_yield, dpp(),
                  MDLOG_STATUS_WRITE,
    ...
    if (opt_cmd == OPT::MFA_GET) {
        ...
        int ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->cls->mfa.get_mfa(dpp(), user->get_id(), totp_serial,
                  &result, null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_LIST) {
        ...
        int ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->cls->mfa.list_mfa(dpp(), user->get_id(), &result,
                  null_yield);
        if (ret < 0 && ret != -ENOENT) {
    ...
    if (opt_cmd == OPT::MFA_CHECK) {
        ...
        list < rados::cls::otp::otp_info_t > result;
        int ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->cls->mfa.check_mfa(dpp(), user->get_id(), totp_serial,
                  totp_pin.front(), null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_RESYNC) {
        ...
        ret = static_cast < rgw::sal::RadosStore * > (driver)->svc()->cls->mfa.otp_get_current_time(dpp(), user->get_id(), &now,
              null_yield);
        if (ret < 0) {
        ...
        ret = static_cast < rgw::sal::RadosStore * > (driver)->ctl()->meta.mgr->mutate(RGWSI_MetaBackend_OTP::get_meta_key(
                  user->get_id()),
              mtime, &objv_tracker,
              null_yield, dpp(),
              MDLOG_STATUS_WRITE,
    ...
    if (opt_cmd == OPT::PUBSUB_NOTIFICATION_RM) {
        ...
        if (rgw::all_zonegroups_support(*site, rgw::zone_features::notification_v2)) {
            if (ret = driver->stat_topics_v1(tenant, null_yield, dpp()); ret != -ENOENT) {
                cerr << "WARNING: " << (ret == 0 ? "topic migration in process" : "cannot determine topic migration status. ret = " +
                                        std::to_string(ret))
