  ...
  public:
    ...
    struct Release {
        mutable ceph_mds_request_release item;
         ...
         Release():item(), dname()
{
} Release(const ceph_mds_request_release & rel, std::string name):item(rel),
    dname(name) {
} void encode(ceph::buffer::list & bl) const {
    using ceph::encode;
  ...
  public:
    ...
    // normal fields void set_stamp(utime_t t) {
        stamp = t;
    }
    ...
    } const std::vector < uint64_t > &get_caller_gid_list() const {
        return gid_list;
    } const std::string & get_path() const {
        return path.get_path();
    } const filepath & get_filepath() const {
        return path;
    } const std::string & get_path2() const {
    ...
    } int get_dentry_wanted() const {
        return get_flags() & CEPH_MDS_FLAG_WANT_DENTRY;
    } void mark_queued_for_replay() const {
    ...
    std::string_view get_type_name()const override {
        ...
        if (head.op == CEPH_MDS_OP_SETATTR) {
            if (head.args.setattr.mask & CEPH_SETATTR_MODE)
                out << " mode=0" << std::oct << head.args.setattr.
                    mode << std::dec;
