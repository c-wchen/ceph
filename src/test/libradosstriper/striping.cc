...
class StriperTestRT:public StriperTestParam {
  public:
    ...
    void checkObjectFromRados(const std::string & soid, bufferlist & bl,
        ...
        ASSERT_LT(0,
                  ioctx.getxattr(firstOid, "striper.layout.stripe_unit",
                                 xattrbl));
        ...
        xattrbl.clear();
        ASSERT_LT(0,
                  ioctx.getxattr(firstOid, "striper.layout.stripe_count",
                                 xattrbl));
        ...
        xattrbl.clear();
        ASSERT_LT(0,
                  ioctx.getxattr(firstOid, "striper.layout.object_size",
                                 xattrbl));
        s_xattr = std::string(xattrbl.c_str(), xattrbl.length());   // adds 0 byte at the end
        ...
        for (uint64_t stripe_nb = 0;
             stripe_nb < nb_stripes_in_object; stripe_nb++) {
            ...
            if (stripe_nb == nb_stripes_in_object - 1 and size % stripe_unit !=
                0) {
            ...
            if (actual_size_if_sparse < size and
                (actual_size_if_sparse + stripe_unit - 1) / stripe_unit <=
                stripe_nb) {
                ...
                ASSERT_EQ(0,
                          memcmp(original_data.c_str(), stripe_data.c_str(),
                                 len));
            }
            free(oid);
        }
        ...
        for (uint64_t object_nb = 0; object_nb < nb_objects; object_nb++) {
            ...
            uint64_t object_start_stripe =
                nb_full_object_set * stripe_per_objectset + object_index_in_set;
