...
{
    return out << ceph_clock_now() << " mds." << cache->mds->
...
num_dentries_auth_subtree_nested(0), dir_auth(CDIR_AUTH_DEFAULT)
{
...
bool CDir::check_rstats(bool scrub)
{
    ...
    if (!is_complete() || !is_auth() || is_frozen()) {
        assert(!scrub);
        dout(10) <<
            "check_rstats bailing out -- incomplete or non-auth or frozen dir!"
            << dendl;
        return true;
    }
...
CDentry *CDir::add_remote_dentry(boost::string_view dname, inodeno_t ino,
                                 unsigned char d_type, snapid_t first,
                                 snapid_t last)
{
    // foreign
    assert(lookup_exact_snap(dname, last) == 0);
    ...
    // create dentry
    CDentry *dn =
        new CDentry(dname, inode->hash_dentry_name(dname), ino, d_type, first,
                    last);
    if (is_auth())
        dn->state_set(CDentry::STATE_AUTH);
    cache->lru.lru_insert_mid(dn);
...
{
    if (dn->get_linkage()->is_primary()) {
        dout(12) << "unlink_inode " << *dn << " " << *dn->get_linkage()->
            get_inode() << dendl;
    }
    else {
...
void CDir::try_remove_unlinked_dn(CDentry * dn)
{
    ...
    // was the dn new?
    if (dn->is_new()) {
        dout(10) << "try_remove_unlinked_dn " << *dn << " in " << *this <<
            dendl;
        if (dn->is_dirty())
            dn->mark_clean();
        remove_dentry(dn);
...
void CDir::unlink_inode_work(CDentry * dn)
{
    ...
    if (dn->get_linkage()->is_remote()) {
        ...
        if (in->auth_pins + in->nested_auth_pins)
            dn->adjust_nested_auth_pins(0 -
                                        (in->auth_pins + in->nested_auth_pins),
                                        0 - in->auth_pins, NULL);
...
void CDir::prepare_old_fragment(map < string_snap_t,
                                std::list <
                                MDSInternalContextBase * > >&dentry_waiters,
                                bool replay)
{
...
void CDir::finish_old_fragment(list < MDSInternalContextBase * >&waiters,
                               bool replay)
{
...
void CDir::split(int bits, list < CDir * >&subs,
                 list < MDSInternalContextBase * >&waiters, bool replay)
{
  ...
  for (const auto & p:dentry_waiters) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        dout(10) << " rstat " << f->fnode.rstat << " fragstat " << f->fnode.
            fragstat << " on " << *f << dendl;
    }
...
void CDir::merge(list < CDir * >&subs,
                 list < MDSInternalContextBase * >&waiters, bool replay)
{
  ...
  for (auto dir:subs) {
        ...
        if (dir->fnode.accounted_fragstat.version == dirstat_version)
            fragstatdiff.add_delta(dir->fnode.accounted_fragstat,
                                   dir->fnode.fragstat, &touched_mtime,
                                   &touched_chattr);
...
void CDir::resync_accounted_fragstat()
{
    ...
    if (pf->accounted_fragstat.version != pi->dirstat.version) {
        ...
        dout(10) << "resync_accounted_fragstat " << pf->
            accounted_fragstat << " -> " << pf->fragstat << dendl;
...
void CDir::resync_accounted_rstat()
{
    ...
    if (pf->accounted_rstat.version != pi->rstat.version) {
        ...
        dout(10) << "resync_accounted_rstat " << pf->
            accounted_rstat << " -> " << pf->rstat << dendl;
...
void CDir::assimilate_dirty_rstat_inodes_finish(MutationRef & mut,
                                                EMetaBlob * blob)
{
...
void CDir::add_dentry_waiter(boost::string_view dname, snapid_t snapid,
...
void CDir::take_dentry_waiting(boost::string_view dname, snapid_t first,
                               snapid_t last,
    ...
    if (tag & WAIT_ATFREEZEROOT) {
        if (!(is_freezing_tree_root() || is_frozen_tree_root() ||
            ...
            dout(10) << "add_waiter " << std::hex << tag << std::
                dec << " " << c << " should be ATFREEZEROOT, " << *this <<
...
{
    dout(11) << "finish_waiting mask " << hex << mask << dec << " result " <<
...
{
    if (!state_test(STATE_DIRTY)) {
        dout(10) << "mark_dirty (was clean) " << *this << " version " <<
            get_version() << dendl;
        _set_dirty_flag();
        assert(ls);
    }
    else {
        dout(10) << "mark_dirty (already dirty) " << *this << " version " <<
            get_version() << dendl;
    }
    if (ls) {
...
void CDir::fetch(MDSInternalContextBase * c, boost::string_view want_dn,
                 bool ignore_authpinnability)
{
    ...
    if (!want_dn.empty())
        wanted_items.insert(mempool::mds_co::string(want_dn));
    ...
    if (cache->mds->logger)
        cache->mds->logger->inc(l_mds_dir_fetch);

    std::set < dentry_key_t > empty;
    _omap_fetch(NULL, empty);
}

void CDir::fetch(MDSInternalContextBase * c,
                 const std::set < dentry_key_t > &keys)
{
...
void CDir::_omap_fetch(MDSInternalContextBase * c,
                       const std::set < dentry_key_t > &keys)
{
...
void CDir::_omap_fetch_more(bufferlist & hdrbl,
                            map < string, bufferlist > &omap,
                            MDSInternalContextBase * c)
{
...
CDentry *CDir::_load_dentry(boost::string_view key,
    ...
    if (type == 'L') {
        ...
        if (!dn || undef_inode) {
            ...
            if (!in || undef_inode) {
                ...
                cache->mds->clog->error() << "loaded dup inode " << inode_data.
                    inode.
                    ino << " [" << first << "," << last << "] v" << inode_data.
                    inode.
                    version << " at " << dirpath << "/" << dname <<
                    ", but inode " << in->vino() << " v" << in->inode.
                    version << " already exists at " << inopath;
                return dn;
            }
        }
    }
                         ...
                         bool complete, int r)
{
    ...
    if (get_version() == 0) {
        ...
        fnode = got_fnode;
        projected_version = committing_version = committed_version =
            got_fnode.version;
    ...
    if (!realm->have_past_parents_open()) {
        dout(10) << " no snap purge, one or more past parents NOT open" <<
            dendl;
    }
    ...
    for (map < string, bufferlist >::reverse_iterator p = omap.rbegin();
        ...
        if (!complete
            || wanted_items.
            count(mempool::mds_co::string(boost::string_view(dname))) > 0) {
...
void CDir::go_bad_dentry(snapid_t last, boost::string_view dname)
{
    ...
    const bool fatal =
        cache->mds->damage_table.notify_dentry(inode->ino(), frag, last, dname,
                                               path);
    if (fatal) {
        cache->mds->damaged();
...
void CDir::go_bad(bool complete)
{
    ...
    const bool fatal =
        cache->mds->damage_table.notify_dirfrag(inode->ino(), frag, get_path());
    if (fatal) {
        cache->mds->damaged();
        ceph_abort();           // unreachable, damaged() respawns us
    }
 ...
 */
void CDir::commit(version_t want, MDSInternalContextBase * c,
                  bool ignore_authpinnability, int op_prio)
{
                          ...
                          const set < snapid_t > *snaps)
{
    ...
    if (dn->linkage.is_remote()) {
        ...
        dout(14) << " pos " << bl.length() << " dn '" << dn->
            get_name() << "' remote ino " << ino << dendl;
...
void CDir::_commit(version_t want, int op_prio)
{
    ...
    if (cache->mds->logger)
        cache->mds->logger->inc(l_mds_dir_commit);
                         ...
                         LogSegment * ls)
{
    ...
    if (is_dirty_dft()) {
        if (inode->dirfragtreelock.get_state() != LOCK_MIX &&
            ...
            cache->mds->locker->mark_updated_scatterlock(&inode->
                                                         dirfragtreelock);
...
void CDir::auth_pin(void *by)
{
    ...
    // nest pins?
    if (!is_subtree_root() && get_cum_auth_pins() == 1)
...
void CDir::adjust_nested_auth_pins(int inc, int dirinc, void *by)
{
    ...
    dout(15) << "adjust_nested_auth_pins " << inc << "/" << dirinc << " on " <<
        *this << " by " << by << " count now " << auth_pins << " + " <<
        nested_auth_pins << dendl;
    assert(nested_auth_pins >= 0);
    assert(dir_auth_pins >= 0);
...
bool CDir::can_auth_pin(int *err_ret) const const
{
    int err;
    if (!is_auth()) {
        err = ERR_NOT_AUTH;
...
pair < bool, bool > CDir::is_freezing_or_frozen_tree() constconst
{
...
void CDir::maybe_finish_freeze()
{
    ...
    if (state_test(STATE_FREEZINGTREE)) {
        if (!is_subtree_root() && inode->is_frozen()) {
            dout(10) <<
                "maybe_finish_freeze !subtree root and frozen inode, waiting for unfreeze on "
                << inode << dendl;
            // retake an auth_pin...
            auth_pin(inode);
 ...
 * debug output.
 */
void CDir::dump(Formatter * f) const const
{
    assert(f != NULL);
                              ...
                              MDSInternalContext * cb, CDentry ** dnout)
{
    ...
    while (!dns.empty()) {
        ...
        if (!dn) {
            if (!is_complete() && (!has_bloom() || is_in_bloom(dnkey.name))) {
...
int CDir::scrub_dentry_next(MDSInternalContext * cb, CDentry ** dnout)
{
    ...
    if (rval == 0) {
        ...
        rval =
            _next_dentry_on_set(scrub_infop->others_to_scrub, false, cb, dnout);
        if (rval == 0) {
...
void CDir::scrub_dentries_scrubbing(list < CDentry * >*out_dentries)
{
    ...
    for (set < dentry_key_t >::iterator i =
         ...
         i != scrub_infop->directories_scrubbing.end(); ++i) {
        CDentry *d = lookup(i->name, i->snapid);
        assert(d);
        out_dentries->push_back(d);
    }
    for (set < dentry_key_t >::iterator i =
         scrub_infop->others_scrubbing.begin();
         i != scrub_infop->others_scrubbing.end(); ++i) {
        CDentry *d = lookup(i->name, i->snapid);
        assert(d);
        out_dentries->push_back(d);
    }
