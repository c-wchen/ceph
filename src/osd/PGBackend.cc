                                      ...
                                      RecoveryHandle * h)
{
  ...
  for (const auto & shard:get_parent()->get_actingbackfill_shards()) {
        ...
        if (get_parent()->get_shard_missing(shard).is_missing(oid)) {
            dout(20) << __func__ << " will remove " << oid << " " << v <<
                " from " << shard << dendl;
                                      ...
                                      eversion_t > > >&deletes)
{
  ...
  for (const auto & p:deletes) {
        ...
        ConnectionRef con = get_parent()->get_con_osd_cluster(shard.osd,
                                                              get_osdmap()->
                                                              get_epoch());
        if (!con)
            continue;
        ...
        while (it != objects.end()) {
            ...
            uint64_t deletes = 0;
            spg_t target_pg =
                spg_t(get_parent()->get_info().pgid.pgid, shard.shard);
            ...
            while (it != objects.end() &&
                ...
                dout(20) << __func__ << ": sending recovery delete << " << it->
                    first << " " << it->second << " to osd." << shard << dendl;
...
void PGBackend::handle_recovery_delete(OpRequestRef op)
{
    const MOSDPGRecoveryDelete *m =
        static_cast < const MOSDPGRecoveryDelete * >(op->get_req());
  ...
  for (const auto & p:m->objects) {
        get_parent()->remove_missing_object(p.first, p.second,
                                            gather.new_sub());
    }
    ...
    gather.set_finisher(new FunctionContext([ =] (int r) {
                                            if (r != -EAGAIN) {
                                            get_parent()->
...
void PGBackend::handle_recovery_delete_reply(OpRequestRef op)
{
    const MOSDPGRecoveryDeleteReply *m =
        static_cast < const MOSDPGRecoveryDeleteReply * >(op->get_req());
      ...
      for (const auto & shard:get_parent()->
            ...
            if (get_parent()->get_shard_missing(shard).is_missing(oid)) {
                dout(20) << __func__ << " " << oid <<
                    " still missing on at least " << shard << dendl;
                         ...
                         ObjectStore::Transaction * t)
{
    ...
    struct RollbackVisitor:public ObjectModDesc::Visitor {
        ...
        PGBackend *pg;
        ObjectStore::Transaction t;
        RollbackVisitor(const hobject_t & hoid,
        ...
        void setattrs(map < string,
                      boost::optional < bufferlist > >&attrs) override {
            ObjectStore::Transaction temp;
...
struct Trimmer:public ObjectModDesc::Visitor {
    ...
    PGBackend *pg;
    ObjectStore::Transaction * t;
    Trimmer(const hobject_t & soid,
            PGBackend * pg, ObjectStore::Transaction * t)
    :soid(soid), pg(pg), t(t) {
    } void rmobject(version_t old_version) override {
...
void PGBackend::try_stash(const hobject_t & hoid,
                          version_t v, ObjectStore::Transaction * t)
{
    t->try_rename(coll,
...
int PGBackend::objects_list_partial(const hobject_t & begin,
                                    int min,
                                    int max,
                                    vector < hobject_t > *ls, hobject_t * next)
{
    ...
    while (!_next.is_max() && ls->size() < (unsigned)min) {
        ...
        if (r != 0) {
            derr << __func__ << " list collection " << ch << " got: " <<
                                  ...
                                  vector < ghobject_t > *gen_obs)
{
    ...
    int r = store->collection_list(ch,
                                   ghobject_t(start, ghobject_t::NO_GEN,
                                              get_parent()->whoami_shard().
                                              shard),
                                   ghobject_t(end, ghobject_t::NO_GEN,
                                              get_parent()->whoami_shard().
                                              shard),
                                   INT_MAX,
                                   &objects,
...
int PGBackend::objects_get_attr(const hobject_t & hoid,
                                const string & attr, bufferlist * out)
{
    bufferptr bp;
...
{
    return store->getattrs(ch,
                           ghobject_t(hoid, ghobject_t::NO_GEN,
                                      get_parent()->whoami_shard().shard),
                           *out);
}

void PGBackend::rollback_setattrs(const hobject_t & hoid,
                                  map < string,
                                  boost::optional < bufferlist > >&old_attrs,
                                  ObjectStore::Transaction * t)
{
    map < string, bufferlist > to_set;
    assert(!hoid.is_temp());
    for (map < string, boost::optional < bufferlist > >::iterator i =
         old_attrs.begin(); i != old_attrs.end(); ++i) {
        if (i->second) {
            ...
            t->rmattr(coll,
                      ghobject_t(hoid, ghobject_t::NO_GEN,
                                 get_parent()->whoami_shard().shard), i->first);
        }
    }
    t->setattrs(coll,
                ghobject_t(hoid, ghobject_t::NO_GEN,
                           get_parent()->whoami_shard().shard), to_set);
}

void PGBackend::rollback_append(const hobject_t & hoid,
                                uint64_t old_size, ObjectStore::Transaction * t)
{
    assert(!hoid.is_temp());
    t->truncate(coll,
                ghobject_t(hoid, ghobject_t::NO_GEN,
                           get_parent()->whoami_shard().shard), old_size);
}

void PGBackend::rollback_stash(const hobject_t & hoid,
                               ...
                               ObjectStore::Transaction * t)
{
    ...
    t->collection_move_rename(coll,
                              ghobject_t(hoid, old_version,
                                         get_parent()->whoami_shard().shard),
                              coll, ghobject_t(hoid, ghobject_t::NO_GEN,
                                               get_parent()->whoami_shard().
                                               shard));
}

void PGBackend::rollback_try_stash(const hobject_t & hoid,
                                   version_t old_version,
                                   ObjectStore::Transaction * t)
{
    assert(!hoid.is_temp());
...
void PGBackend::rollback_extents(version_t gen,
                                 const vector < pair < uint64_t,
                                 uint64_t > >&extents, const hobject_t & hoid,
                                 ObjectStore::Transaction * t)
{
  ...
  for (auto && extent:extents) {
        t->clone_range(coll,
                       ...
                       extent.first, extent.second, extent.first);
    }
                                       ...
                                       ObjectStore * store, CephContext * cct)
{
    ...
    case pg_pool_t::TYPE_ERASURE:{
            ...
            ErasureCodeProfile profile =
                curmap->get_erasure_code_profile(pool.erasure_code_profile);
            assert(profile.count("plugin"));
            stringstream ss;
            ceph::ErasureCodePluginRegistry::instance().factory(profile.
                                                                find("plugin")->
                                                                second,
                                                                cct->_conf->
                                                                get_val <
                                                                ...
                                                                &ec_impl, &ss);
            assert(ec_impl);
            return new ECBackend(l,
                                 coll,
                                 ch, store, cct, ec_impl, pool.stripe_width);
        }
    default:
        ceph_abort();
        return NULL;
                                         ...
                                         bool has_snapset)
{
        ...
        if (auth_oi.is_omap_digest() && candidate.omap_digest_present) {
            if (auth_oi.omap_digest != candidate.omap_digest) {
                if (error != CLEAN)
                    errorstream << ", ";
                error = FOUND_ERROR;
                errorstream << "omap_digest 0x" << std::hex << candidate.
                    omap_digest << " != omap_digest 0x" << auth_oi.
                    omap_digest << std::dec << " from auth oi " << auth_oi;
    ...
    if (oi_size != candidate.size) {
        ...
        errorstream << "size " << candidate.size
            << " != size " << oi_size << " from auth oi " << auth_oi;
    ...
    if (auth.size != candidate.size) {
        ...
        error = FOUND_ERROR;
        errorstream << "size " << candidate.size
            << " != size " << auth.size << " from shard " << auth_shard;
...
map < pg_shard_t, ScrubMap * >::const_iterator
    PGBackend::be_select_auth_object(const hobject_t & obj,
                                     const map < pg_shard_t, ScrubMap * >&maps,
                                     object_info_t * auth_oi,
                                     map < pg_shard_t,
                                     shard_info_wrapper > &shard_map,
                                     bool & digest_match, spg_t pgid,
                                     ostream & errorstream)
{
  ...
  for (auto & l:shards) {
        ...
        if (i->second.size != be_get_ondisk_size(oi.size)) {
            ...
            if (error)
                shard_errorstream << ", ";
            error = true;
            shard_errorstream << "candidate size " << i->second.
                size << " info size " << oi.size << " mismatch";
        }
        ...
        if (auth_version != eversion_t()
            ...
            dout(10) << __func__ << " digest_match = false, " << obj <<
                " data_digest 0x" << std::hex << i->second.
                digest << " != data_digest 0x" << auth->second->objects[obj].
        ...
        if (parent->get_pool().is_replicated()
            && cct->_conf->osd_distrust_data_digest) {
            ...
            if (j->second->has_builtin_csum
                && j->second->has_builtin_csum == true) {
                oi_prio = true;
            }
        }
        ...
        if (auth_version == eversion_t() || oi.version > auth_version ||
            (oi.version == auth_version
             && dcount(oi, oi_prio) > dcount(*auth_oi, auth_prio))) {
            auth = j;
            *auth_oi = oi;
    ...
    dout(10) << __func__ << ": selecting osd " << auth->first
        << " for obj " << obj << " with oi " << *auth_oi << dendl;
    return auth;
}

void PGBackend::be_compare_scrubmaps(const map < pg_shard_t, ScrubMap * >&maps,
                                     const set < hobject_t > &master_set,
                                     bool repair,
                                     map < hobject_t,
                                     set < pg_shard_t >> &missing,
                                     map < hobject_t,
                                     set < pg_shard_t >> &inconsistent,
                                     map < hobject_t,
                                     list < pg_shard_t >> &authoritative,
                                     map < hobject_t,
                                     pair < boost::optional < uint32_t >,
                                     boost::optional <
                                     uint32_t >>> &missing_digest,
                                     int &shallow_errors, int &deep_errors,
                                     Scrub::Store * store, const spg_t & pgid,
        ...
        *k};
        ...
        for (auto j = maps.cbegin(); j != maps.cend(); ++j) {
            ...
            if (j->second->objects.count(*k)) {
                ...
                dout(20) << __func__ << (repair ? " repair " : " ") << (parent->
                                                                        get_pool
                                                                        ().
                                                                        is_replicated
                                                                        ()?
                                                                        ...
                                                                        : "")
                    << (j ==
                        auth ? "auth " : "") << "shards " << shard_map.
                    size() << (digest_match ? " digest_match " : " ")
                    << (shard_map[j->first].
                ...
                else if (repair && parent->get_pool().is_replicated()
                         && j == auth && shard_map.size() > 1 && digest_match
        ...
        if (!cur_inconsistent.empty() || !cur_missing.empty()) {
            ...
            if (auth_object.digest_present && !auth_oi.is_data_digest()) {
                dout(20) << __func__ << " missing data digest on " << *k <<
                    dendl;
                update = MAYBE;
            }
            if (auth_object.omap_digest_present && !auth_oi.is_omap_digest()) {
                dout(20) << __func__ << " missing omap digest on " << *k <<
                    dendl;
                update = MAYBE;
            }
            ...
            if (auth_oi.is_data_digest() && auth_object.digest_present &&
                ...
                assert(cct->_conf->osd_distrust_data_digest
                       || shard_map[auth->first].
                       has_data_digest_mismatch_info());
                errorstream << pgid << " recorded data digest 0x" << std::
                    hex << auth_oi.
                    data_digest << " != on disk 0x" << auth_object.
                    digest << std::dec << " on " << auth_oi.soid << "\n";
                if (repair)
                    update = FORCE;
            }
            ...
            if (update != NO) {
                ...
                if (update == FORCE ||
                    ...
                    if (auth_object.digest_present) {
                        data_digest = auth_object.digest;
                        dout(20) << __func__ << " will update data digest on "
                            << *k << dendl;
                    }
                    if (auth_object.omap_digest_present) {
                        omap_digest = auth_object.omap_digest;
                        dout(20) << __func__ << " will update omap digest on "
                            << *k << dendl;
                    }
                    missing_digest[*k] = make_pair(data_digest, omap_digest);
                }
                    ...
                    dout(20) << __func__ << " missing digest but age " << age
                        << " < " << cct->_conf->
                        osd_deep_scrub_update_digest_min_age << " on " << *k <<
                        dendl;
                }
            }
        }
...
void PGBackend::be_large_omap_check(const map < pg_shard_t, ScrubMap * >&maps,
                                    ...
                                    ostream & warnstream) const const
{
      ...
      for (const auto & map:maps) {
            ...
            if (obj.large_omap_object_found) {
                ...
                warnstream << "Large omap object found. Object: " << k <<
                    " Key count: " << obj.
                    large_omap_object_key_count << " Size (bytes): " << obj.
