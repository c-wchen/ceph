 ...
 * over all time.
 */
template < typename T > class AsyncReserver {
    CephContext *cct;
    Finisher *f;
    ...
    map < T, pair < unsigned,
        typename list < Reservation >::iterator >> queue_pointers;
  ...
  public:
    AsyncReserver(CephContext * cct,
                  Finisher * f, unsigned max_allowed, unsigned min_priority = 0)
  :    
    cct(cct),
    f(f),
    max_allowed(max_allowed),
    min_priority(min_priority), lock("AsyncReserver::lock") {
    }
   ...
   */
    void update_priority(T item, unsigned newprio) {
        ...
        if (i != queue_pointers.end()) {
            ...
            rdout(10) << __func__ << " update " << r << " (was queued)" <<
                dendl;
            ...
            assert(!queue_pointers.count(item) && !in_progress.count(item));
            r.prio = newprio;
            ...
            if (p != in_progress.end()) {
                ...
                if (p->second.preempt) {
                    ...
                    preempt_by_prio.
                        erase(make_pair(p->second.prio, p->second.item));
                    p->second.prio = newprio;
                    preempt_by_prio.
                        insert(make_pair(p->second.prio, p->second.item));
                }
                else {
                    p->second.prio = newprio;
                }
                ...
                rdout(10) << __func__ << " update " << item << " (not found)" <<
                    dendl;
            }
        }
        do_queues();
   ...
   */
    void cancel_reservation(T item  ///< [in] key for reservation to cancel
        ...
        if (i != queue_pointers.end()) {
            ...
            rdout(10) << __func__ << " cancel " << r << " (was queued)" <<
                dendl;
            delete r.grant;
            delete r.preempt;
            queues[prio].erase(i->second.second);
            ...
            if (p != in_progress.end()) {
                ...
                rdout(10) << __func__ << " cancel " << item << " (not found)" <<
                    dendl;
            }
        }
        do_queues();
