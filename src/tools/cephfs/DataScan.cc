                                          ...
                                          int64_t data_pool_id)
{
    ...
    inode.inode.ctime = inode.inode.mtime = ceph_clock_now();
    inode.inode.nlink = 1;
...
{
    return forall_objects(data_io, false,[this] (std::string const &oid,
                          ...
                          oid << "': " << cpp_strerror(r) << dendl; return r;}
...
bool DataScan::valid_ino(inodeno_t ino) const const
{
    return (ino >= inodeno_t((1ull << 40)))
        ...
        || ino == MDS_INO_ROOT || ino == MDS_INO_CEPH;
}
...
int DataScan::scan_links()
{
    ...
    struct link_info_t {
        ...
        link_info_t():version(0), nlink(0), is_dir(false) {
                      ...
                      const CInode::mempool_inode & i):dirino(di), frag(df),
  ...
  for (auto & p:max_ino_map) {
        std::cout << "mds." << p.first << " max used ino " << p.
            second << std::endl;
    }
...
int DataScan::scan_frags()
{
    ...
    return forall_objects(metadata_io, true,[this] (std::string const &oid,
                          ...
                          if (layout_r != -ENODATA) {
                          try {
                          bufferlist::iterator q =
                          layout_bl.begin();::decode(loaded_layout, q);}
                          catch(buffer::error & e) {
                                ...
                                frag_t * result_ft)
{
    ...
    if (parent_bl.length()) {
        try {
            ...
            dout(4) << "Corrupt backtrace on '" << root_frag_oid << "': " << e
                << dendl;
        }
    }
