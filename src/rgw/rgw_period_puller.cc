...
#define MAX_REST_RESPONSE (128 * 1024)
        int r =
            conn->forward(user, info, nullptr, MAX_REST_RESPONSE, nullptr,
                          &data);
        if (r < 0) {
            return r;
        }
        ...
        if (r < 0) {
            lderr(conn->
                  get_ctx()) << "request failed: " << cpp_strerror(-r) << dendl;
            return r;
        }
...
int RGWPeriodPuller::pull(const std::string & period_id, RGWPeriod & period)
{
    ...
    if (r < 0) {
        ...
        if (r < 0) {
            lderr(store->
                  ctx()) << "failed to pull period " << period_id << dendl;
            return r;
        }
        ...
        if (r == -EEXIST) {
            ...
            lderr(store->
                  ctx()) << "failed to store period " << period_id << dendl;
            return r;
        }
        // update latest epoch
