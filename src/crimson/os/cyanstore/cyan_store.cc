...
namespace crimson::os {
    ...
    seastar::future < store_statfs_t > CyanStore::stat()const {
        ...
        return shard_stores.map_reduce0([](const CyanStore::Shard & local_store) {
               ...
               ("memstore_device_bytes"); st.available = st.total - used_bytes;
    ...
    CyanStore::mkfs_ertr::future <> CyanStore::mkfs(uuid_d new_osd_fsid) {
        ceph_assert(seastar::this_shard_id() == primary_core);
        static const char read_meta_errmsg[] {
        "read_meta"};
        static const char parse_fsid_errmsg[] {
        "failed to parse fsid"};
        static const char match_ofsid_errmsg[] {
        "unmatched osd_fsid"};
        return read_meta("fsid").then([ =, this] (auto && ret)->
                                      mkfs_ertr::future <> {
                                      auto &[r, fsid_str] = ret;
                                      if (r == -ENOENT) {
                                      ...
                                      singleton_ec < read_meta_errmsg > ()};}
                                      else {
                                      logger().info("mkfs already has fsid {}",
                                                    fsid_str);
                                      if (!osd_fsid.parse(fsid_str.c_str())) {
                                      return crimson::stateful_ec {
                                      singleton_ec < parse_fsid_errmsg > ()};}
                                      else
                                      if (osd_fsid != new_osd_fsid) {
                                      logger().
                                      error("on-disk fsid {} != provided {}",
                                            osd_fsid, new_osd_fsid);
                                      return crimson::stateful_ec {
                                      singleton_ec < match_ofsid_errmsg > ()};}
                                      else {
    ...
    CyanStore::mount_ertr::future <> CyanStore::Shard::mount() {
        static const char read_file_errmsg[] {
        "read_file"};
        ceph::bufferlist bl;
        std::string fn =
            path + "/collections" + std::to_string(seastar::this_shard_id());
      ...
      for (auto & coll:collections) {
            ...
            boost::intrusive_ptr < Collection > c {
                new Collection {
                coll}
            };
            auto p = cbl.cbegin();
    ...
    seastar::future <> CyanStore::Shard::umount() {
        return seastar::do_with(std::set < coll_t > {
                                }
                                ,[this] (auto & collections) {
                                return seastar::do_for_each(coll_map,
                                       ...
                                       fmt::format("{}/collections{}", path,
                                                   std::to_string(seastar::
                                                                  this_shard_id
                                                                  ()));
                                       return crimson::write_file(std::move(bl),
                                                                  fn);}
                                );}
        );
    }
    ...
    seastar::future < std::tuple < std::vector < ghobject_t >, ghobject_t >>
        CyanStore::Shard::list_objects(CollectionRef ch,
        ...
        for (const auto &[oid, obj]:boost::
             make_iterator_range(c->object_map.lower_bound(start),
                                 c->object_map.end())) {
            std::ignore = obj;
        ...
        return seastar::make_ready_future < std::tuple < std::vector <
            ghobject_t >,
            ghobject_t >> (std::make_tuple(std::move(objects), next));
    }
    ...
    seastar::future < std::vector < coll_core_t >>
      ...
      for (auto & coll:coll_map) {
            collections.
                push_back(std::make_pair(coll.first, seastar::this_shard_id()));
        }
    ...
    CyanStore::Shard::read_errorator::future < ceph::bufferlist >
        CyanStore::Shard::read(CollectionRef ch,
    ...
    CyanStore::Shard::read_errorator::future < ceph::bufferlist >
        CyanStore::Shard::readv(CollectionRef ch,
                                const ghobject_t & oid,
        ...
        return seastar::do_with(ceph::bufferlist {
                                ...
                                return crimson::do_for_each(m,
                                            ...
                                            make_ready_future <
                                            ceph::bufferlist > (std::move(bl));}
                                );}
        );
    }

    CyanStore::Shard::get_attr_errorator::future < ceph::bufferlist >
        CyanStore::Shard::get_attr(CollectionRef ch,
                                   const ghobject_t & oid,
    ...
    CyanStore::Shard::get_attrs_ertr::future < CyanStore::Shard::attrs_t >
        CyanStore::Shard::get_attrs(CollectionRef ch, const ghobject_t & oid) {
        auto c = static_cast < Collection * >(ch.get());
        logger().debug("{} {} {}", __func__, c->get_cid(), oid);
        auto o = c->get_object(oid);
        if (!o) {
    ...
    auto CyanStore::Shard::omap_get_values(CollectionRef ch,
                                           const ghobject_t & oid,
    ...
    auto CyanStore::Shard::omap_get_values(CollectionRef ch,
                                           ...
                                           &start)
    ->CyanStore::Shard::read_errorator::future < std::tuple < bool,
        omap_values_t >> {
        auto c = static_cast < Collection * >(ch.get());
        logger().debug("{} {} {}", __func__, c->get_cid(), oid);
        auto o = c->get_object(oid);
        ...
        return seastar::make_ready_future < std::tuple < bool,
            omap_values_t >> (std::make_tuple(true, std::move(values)));
    }

    auto CyanStore::Shard::omap_get_header(CollectionRef ch,
                                           const ghobject_t & oid)
    ->CyanStore::Shard::get_attr_errorator::future < ceph::bufferlist > {
        auto c = static_cast < Collection * >(ch.get());
        auto o = c->get_object(oid);
    ...
    seastar::future <> CyanStore::Shard::
        do_transaction_no_callbacks(CollectionRef ch, ceph::os::Transaction
                                    && t) {
    ...
    int CyanStore::Shard::_write(const coll_t & cid,
                                 const ghobject_t & oid,
                                 uint64_t offset,
                                 size_t len,
    ...
    int CyanStore::Shard::_zero(const coll_t & cid,
                                const ghobject_t & oid,
    ...
    int CyanStore::Shard::_omap_clear(const coll_t & cid,
                                      const ghobject_t & oid) {
    ...
    int CyanStore::Shard::_omap_set_values(const coll_t & cid,
                                           const ghobject_t & oid,
                                           std::map < std::string,
    ...
    int CyanStore::Shard::_omap_set_header(const coll_t & cid,
                                           const ghobject_t & oid,
    ...
    int CyanStore::Shard::_omap_rmkeys(const coll_t & cid,
                                       const ghobject_t & oid,
    ...
    int CyanStore::Shard::_omap_rmkeyrange(const coll_t & cid,
                                           const ghobject_t & oid,
    ...
    int CyanStore::Shard::_truncate(const coll_t & cid,
                                    const ghobject_t & oid, uint64_t size) {
    ...
    int CyanStore::Shard::_clone(const coll_t & cid,
                                 const ghobject_t & oid,
    ...
    int CyanStore::Shard::_setattrs(const coll_t & cid,
                                    const ghobject_t & oid,
                                    std::map < std::string, bufferlist > &&aset)
    {
        logger().debug("{} cid={} oid={}", __func__, cid, oid);
    ...
    int CyanStore::Shard::_rm_attr(const coll_t & cid,
                                   const ghobject_t & oid,
    ...
    seastar::future < std::tuple < int, std::string >>
        ...
        int r =
            safe_read_file(path.c_str(), key.c_str(), fsid.data(), fsid.size());
        if (r > 0) {
            fsid.resize(r);
        ...
        return seastar::make_ready_future < std::tuple < int,
            std::string >> (std::make_tuple(r, fsid));
    }
    ...
    uuid_d CyanStore::get_fsid() const {
        ...
        return 256;
    } CyanStore::Shard::read_errorator::future < std::map < uint64_t,
        uint64_t >> CyanStore::Shard::fiemap(CollectionRef ch,
                                             const ghobject_t & oid,
        ...
        return seastar::make_ready_future < std::map < uint64_t,
            uint64_t >> (std::move(m));
    }

    seastar::future < struct stat >CyanStore::Shard::stat(CollectionRef ch,
                                                          const ghobject_t &
                                                          oid) {
        auto c = static_cast < Collection * >(ch.get());
        auto o = c->get_object(oid);
