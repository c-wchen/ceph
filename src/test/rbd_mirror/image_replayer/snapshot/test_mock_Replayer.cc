...
template <> struct SnapshotCopyRequest <MockTestImageCtx > {
    ...
    static SnapshotCopyRequest *create(MockTestImageCtx *src_image_ctx,
                                       MockTestImageCtx *dst_image_ctx,
                                       librados::
                                       snap_t src_snap_id_start,
                                       librados::snap_t src_snap_id_end,
                                       librados::
                                       snap_t dst_snap_id_start,
                                       bool flatten, ::MockContextWQ *
                                       work_queue, SnapSeqs *snap_seqs,
                                       Context *on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->src_snap_id_start = src_snap_id_start;
...
ImageCopyRequest < MockTestImageCtx > *ImageCopyRequest <
MockTestImageCtx >::s_instance = nullptr;
SnapshotCopyRequest < MockTestImageCtx > *SnapshotCopyRequest <
MockTestImageCtx >::s_instance = nullptr;
...
ImageStateUpdateRequest < MockTestImageCtx > *ImageStateUpdateRequest <
MockTestImageCtx >::s_instance = nullptr;
...
template <> struct CreateNonPrimaryRequest <MockTestImageCtx > {
    ...
    static CreateNonPrimaryRequest *create(MockTestImageCtx *
                                           image_ctx, bool demoted,
                                           const std::
                                           string &
                                           primary_mirror_uuid,
                                           uint64_t primary_snap_id,
...
template <> struct GetImageStateRequest <MockTestImageCtx > {
    ...
    static GetImageStateRequest *create(MockTestImageCtx *
                                        image_ctx, uint64_t snap_id,
...
*CreateNonPrimaryRequest < MockTestImageCtx >::s_instance =
    nullptr;
GetImageStateRequest < MockTestImageCtx > *GetImageStateRequest <
MockTestImageCtx >::s_instance = nullptr;
UnlinkPeerRequest < MockTestImageCtx > *UnlinkPeerRequest <
MockTestImageCtx >::s_instance = nullptr;
...
template <> struct CloseImageRequest <librbd::MockTestImageCtx > {
    ...
    static CloseImageRequest *create(librbd::MockTestImageCtx **
                                     image_ctx,
                                     Context *on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
CloseImageRequest < librbd::MockTestImageCtx > *CloseImageRequest <
librbd::MockTestImageCtx >::s_instance = nullptr;
...
struct ApplyImageStateRequest <librbd::MockTestImageCtx > {
    ...
    static ApplyImageStateRequest *create(const std::
                                          string &
                                          local_mirror_uuid,
                                          const std::
                                          string &
...
template <> struct StateBuilder <librbd::MockTestImageCtx > {
    StateBuilder(librbd::MockTestImageCtx &local_image_ctx,
                 ...
                 librbd::mirror::snapshot::ImageMeta <
                 librbd::MockTestImageCtx > &local_image_meta)
    ...
    librbd::mirror::snapshot::ImageMeta <
    librbd::MockTestImageCtx > *local_image_meta = nullptr;
};
...
class TestMockImageReplayerSnapshotReplayer: public
    TestMockFixture
{
public:
    ...
    typedef StateBuilder < librbd::MockTestImageCtx >
    MockStateBuilder;
    typedef InstanceWatcher < librbd::MockTestImageCtx >
    MockInstanceWatcher;
    typedef Threads < librbd::MockTestImageCtx > MockThreads;
    typedef CloseImageRequest < librbd::MockTestImageCtx >
    MockCloseImageRequest;
    typedef librbd::deep_copy::ImageCopyRequest <
    librbd::MockTestImageCtx > MockImageCopyRequest;
    typedef librbd::deep_copy::SnapshotCopyRequest <
    librbd::MockTestImageCtx > MockSnapshotCopyRequest;
    typedef librbd::mirror::ImageStateUpdateRequest <
    librbd::MockTestImageCtx > MockImageStateUpdateRequest;
    typedef librbd::mirror::snapshot::CreateNonPrimaryRequest <
    librbd::MockTestImageCtx > MockCreateNonPrimaryRequest;
    typedef librbd::mirror::snapshot::GetImageStateRequest <
    librbd::MockTestImageCtx > MockGetImageStateRequest;
    typedef librbd::mirror::snapshot::ImageMeta <
    librbd::MockTestImageCtx > MockImageMeta;
    typedef librbd::mirror::snapshot::UnlinkPeerRequest <
    librbd::MockTestImageCtx > MockUnlinkPeerRequest;
    ...
    void SetUp() override
    {
        ...
        librbd::RBD rbd;
        ASSERT_EQ(0,
                  create_image(rbd, m_local_io_ctx,
                               m_image_name, m_image_size));
        ASSERT_EQ(0,
                  open_image(m_local_io_ctx, m_image_name,
                             &m_local_image_ctx));

        ASSERT_EQ(0,
                  create_image(rbd, m_remote_io_ctx,
                               m_image_name, m_image_size));
        ASSERT_EQ(0,
                  open_image(m_remote_io_ctx, m_image_name,
                             &m_remote_image_ctx));
    } void expect_work_queue_repeatedly(MockThreads &
                                        mock_threads)
    {
    ...
    } void expect_add_event_after_repeatedly(MockThreads &
            mock_threads)
    {
        ...
        WillRepeatedly(DoAll
                       (Invoke
                        ([this]
        (double seconds, Context * ctx) {
            m_threads->timer->
            add_event_after(seconds, ctx);
        }),
        ReturnArg < 1 > ()));
        EXPECT_CALL(*mock_threads.timer, cancel_event(_))
        .WillRepeatedly(Invoke([this](Context * ctx) {
            return m_threads->timer->
                   cancel_event(ctx);
        }
                              ));
    }

    void expect_register_update_watcher(librbd::
                                        MockTestImageCtx &
                                        mock_image_ctx,
                                        librbd::
                                        UpdateWatchCtx **
                                        ...
                                        int r)
    {
        EXPECT_CALL(*mock_image_ctx.state,
                    register_update_watcher(_, _))
        .WillOnce(Invoke([update_watch_ctx, watch_handle, r]
                         (librbd::UpdateWatchCtx * ctx,
        uint64_t *handle) {
            if (r >= 0) {
    ...
    void expect_unregister_update_watcher(librbd::
                                          MockTestImageCtx &
                                          mock_image_ctx,
                                          uint64_t watch_handle,
                                          int r)
    {
        EXPECT_CALL(*mock_image_ctx.state,
                    unregister_update_watcher(watch_handle, _))
        .WillOnce(WithArg < 1 >
        (Invoke([this, r](Context * ctx) {
                                ...
                                bool resync_requested, int r)
    {
        ...
        .
        WillOnce(Invoke
                 ([this, &mock_image_meta, resync_requested,
        r](Context * ctx) {
            mock_image_meta.resync_requested =
                resync_requested;
            m_threads->work_queue->queue(ctx, r);
        }));
    }

    void expect_is_refresh_required(librbd::
                                    MockTestImageCtx &
                                    mock_image_ctx,
                                    bool is_required)
    {
    ...
    void expect_refresh(librbd::
                        MockTestImageCtx &mock_image_ctx,
                        const std::map < uint64_t,
                        librbd::SnapInfo > &snaps, int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, refresh(_))
        .
        WillOnce(Invoke
                 ([this, &mock_image_ctx, snaps,
        r](Context * ctx) {
    ...
    void expect_notify_update(librbd::
                              MockTestImageCtx &mock_image_ctx)
    {
    ...
    void expect_prune_non_primary_snapshot(librbd::
                                           MockTestImageCtx &
                                           mock_image_ctx,
                                           uint64_t snap_id,
                                           int r)
    {
        ...
        .
        WillOnce(Invoke
                 ([&mock_image_ctx](uint64_t snap_id)->
        librbd::SnapInfo * {
            auto it =
            mock_image_ctx.snap_info.find(snap_id);
            if (it ==
                mock_image_ctx.snap_info.end()) {
                return nullptr;
            }
            return &it->second;
        }
    ...
    void expect_snapshot_copy(MockSnapshotCopyRequest &
                              mock_snapshot_copy_request,
                              uint64_t src_snap_id_start,
                              uint64_t src_snap_id_end,
                              uint64_t dst_snap_id_start,
                              const librbd::
                              SnapSeqs &snap_seqs, int r)
    {
        ...
        dst_snap_id_start, snap_seqs, r]() {
            ASSERT_EQ(src_snap_id_start,
                      req.src_snap_id_start);
            ASSERT_EQ(src_snap_id_end,
                      req.src_snap_id_end);
            ASSERT_EQ(dst_snap_id_start,
                      req.dst_snap_id_start);
            *req.snap_seqs = snap_seqs;
            m_threads->work_queue->queue(req.
                                         on_finish,
                                         r);
        }));
    }

    void expect_get_image_state(MockGetImageStateRequest &
                                ...
                                uint64_t snap_id, int r)
    {
        ...
        WillOnce(Invoke
                 ([this, &req =
                       mock_get_image_state_request, snap_id,
        r]() {
    ...
    void expect_create_non_primary_request
    (MockCreateNonPrimaryRequest &
     mock_create_non_primary_request, bool demoted,
     const std::string &primary_mirror_uuid,
     uint64_t primary_snap_id,
     const librbd::SnapSeqs &snap_seqs, uint64_t snap_id,
     int r)
    {
        ...
        WillOnce(Invoke
                 ([this, &req =
                       mock_create_non_primary_request, demoted,
                   primary_mirror_uuid, primary_snap_id,
        snap_seqs, snap_id, r]() {
            ASSERT_EQ(demoted, req.demoted);
            ASSERT_EQ(primary_mirror_uuid,
                      req.primary_mirror_uuid);
            ASSERT_EQ(primary_snap_id,
                      req.primary_snap_id);
    ...
    void expect_update_mirror_image_state
    (MockImageStateUpdateRequest &
     mock_image_state_update_request, int r)
    {
        EXPECT_CALL(mock_image_state_update_request, send())
        .
        WillOnce(Invoke
                 ([this, &req =
        mock_image_state_update_request, r]() {
            m_threads->work_queue->queue(req.
                                         on_finish,
                                         r);
        }));
    ...
    void expect_notify_sync_request(MockInstanceWatcher &
                                    mock_instance_watcher,
                                    const std::
                                    string &image_id, int r)
    {
        EXPECT_CALL(mock_instance_watcher,
                    notify_sync_request(image_id, _))
        .WillOnce(WithArg < 1 >
        (Invoke([this, r](Context * ctx) {
            m_threads->work_queue->queue(ctx,
                                         r);
    ...
    void expect_notify_sync_complete(MockInstanceWatcher &
                                     mock_instance_watcher,
                                     const std::
                                     string &image_id)
    {
    ...
    void expect_cancel_sync_request(MockInstanceWatcher &
                                    mock_instance_watcher,
                                    const std::
                                    string &image_id)
    {
    ...
    void expect_image_copy(MockImageCopyRequest &
                           mock_image_copy_request,
                           uint64_t src_snap_id_start,
                           uint64_t src_snap_id_end,
                           uint64_t dst_snap_id_start,
                           const librbd::deep_copy::
                           ...
                           int r)
    {
        ...
        WillOnce(Invoke
                 ([this, &req =
                       mock_image_copy_request,
                   src_snap_id_start, src_snap_id_end,
                   dst_snap_id_start, object_number,
        snap_seqs, r]() {
            ASSERT_EQ(src_snap_id_start,
                      req.src_snap_id_start);
            ASSERT_EQ(src_snap_id_end,
    ...
    void expect_unlink_peer(MockUnlinkPeerRequest &
                            ...
                            bool allow_remove, int r)
    {
        ...
        WillOnce(Invoke
                 ([this, &req =
                       mock_unlink_peer_request, snap_id,
        mirror_peer_uuid, allow_remove, r]() {
            ASSERT_EQ(snap_id, req.snap_id);
            ASSERT_EQ(mirror_peer_uuid,
                      req.mirror_peer_uuid);
    ...
    void expect_apply_image_state(MockApplyImageStateRequest &
                                  mock_request, int r)
    {
    ...
    void expect_mirror_image_snapshot_set_copy_progress(librbd::
            MockTestImageCtx
            &
            mock_test_image_ctx,
            uint64_t
            ...
            int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_test_image_ctx.md_ctx),
                    exec(mock_test_image_ctx.header_oid, _,
                         StrEq("rbd"),
                         StrEq
    ...
    void expect_send(MockCloseImageRequest &
                     mock_close_image_request, int r)
    {
        EXPECT_CALL(mock_close_image_request, send())
        .
        WillOnce(Invoke
        ([this, &mock_close_image_request, r]() {
            *mock_close_image_request.image_ctx =
                nullptr;
            m_threads->work_queue->
            queue(mock_close_image_request.on_finish,
                  r);
        }));
    }

    void expect_notification(MockThreads &mock_threads,
                             MockReplayerListener &
                             mock_replayer_listener)
    {
        EXPECT_CALL(mock_replayer_listener,
                    handle_notification())
    ...
    int wait_for_notification(uint32_t count)
    {
        ...
        for (uint32_t idx = 0; idx < count; ++idx) {
            while (m_notifications == 0) {
                if (m_cond.wait_for(locker, 10 s) ==
    ...
    int init_entry_replayer(MockReplayer &mock_replayer,
                            ...
                            librbd::UpdateWatchCtx **
                            update_watch_ctx)
    {
        expect_register_update_watcher(mock_local_image_ctx,
                                       update_watch_ctx, 123,
                                       0);
        expect_register_update_watcher(mock_remote_image_ctx,
                                       update_watch_ctx, 234,
                                       0);
        expect_load_image_meta(mock_image_meta, false, 0);
        expect_is_refresh_required(mock_local_image_ctx, false);
        expect_is_refresh_required(mock_remote_image_ctx,
                                   false);
    ...
    int shut_down_entry_replayer(MockReplayer &mock_replayer,
                                 ...
                                 mock_remote_image_ctx)
    {
        expect_unregister_update_watcher(mock_remote_image_ctx,
                                         234, 0);
        expect_unregister_update_watcher(mock_local_image_ctx,
                                         123, 0);
    ...
    ceph::mutex m_lock =
        ceph::
        make_mutex("TestMockImageReplayerSnapshotReplayer");
...
TEST_F(TestMockImageReplayerSnapshotReplayer, InitShutDown)
{
    ...
    MockReplayer mock_replayer {
        &mock_threads, &mock_instance_watcher,
        "local mirror uuid", &m_pool_meta_cache,
        &mock_state_builder, &mock_replayer_listener};
    m_pool_meta_cache.set_remote_pool_meta(m_remote_io_ctx.
    get_id(), {
        "remote mirror uuid",
    ...
    ASSERT_EQ(0,
              init_entry_replayer(mock_replayer, mock_threads,
                                  mock_local_image_ctx,
                                  mock_remote_image_ctx,
                                  mock_replayer_listener,
                                  mock_image_meta,
                                  &update_watch_ctx));
    ASSERT_EQ(0,
              shut_down_entry_replayer(mock_replayer,
                                       mock_threads,
                                       mock_local_image_ctx,
                                       mock_remote_image_ctx));
}
...
TEST_F(TestMockImageReplayerSnapshotReplayer, SyncSnapshot)
{
    ...
    (mock_create_non_primary_request, false,
    "remote mirror uuid", 1, { {
            1, CEPH_NOSNAP
        }
    }
    , 11, 0);
    ...
    (mock_image_state_update_request, 0);
    expect_notify_sync_request(mock_instance_watcher,
    ...
    expect_notify_sync_complete(mock_instance_watcher,
    ...
    expect_unlink_peer(mock_unlink_peer_request, 1,
                       "remote mirror peer uuid", false, 0);
    expect_notify_sync_complete(mock_instance_watcher,
                                mock_local_image_ctx.id);
    ...
    , 0);
    expect_prune_non_primary_snapshot(mock_local_image_ctx, 11,
                                      0);
...
TEST_F(TestMockImageReplayerSnapshotReplayer,
    ...
    mock_local_image_ctx.snap_info = {
        {
            11U, librbd::SnapInfo {
                "snap1", cls::rbd::MirrorSnapshotNamespace {
                    ...
                    , "remote mirror uuid", 1, false, 123, { {
                            1, CEPH_NOSNAP
                        }
                    }
                }
        ...
        , {
            12U, librbd::SnapInfo {
                "snap2", cls::rbd::MirrorSnapshotNamespace {
                    ...
                    , "remote mirror uuid", 2, false, 123, { {
                            2, CEPH_NOSNAP
                        }
                    }
                }
       ...
       InterruptedPendingSyncDelta)
{
    ...
    expect_is_refresh_required(mock_local_image_ctx, false);
    expect_is_refresh_required(mock_remote_image_ctx, false);
    expect_prune_non_primary_snapshot(mock_local_image_ctx, 12,
                                      0);
       ...
       RemoteImageDemoted)
{
    ...
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_non_primary_request
    (mock_create_non_primary_request, true,
    "remote mirror uuid", 1, { {
            1, CEPH_NOSNAP
        }
    }
       ...
       RegisterRemoteUpdateWatcherError)
{
    ...
    expect_unregister_update_watcher(mock_local_image_ctx, 123,
                                     0);
       ...
       UnregisterRemoteUpdateWatcherError)
{
    ...
    // shut down
    expect_unregister_update_watcher(mock_remote_image_ctx, 234,
                                     -EINVAL);
    expect_unregister_update_watcher(mock_local_image_ctx, 123,
                                     0);
       ...
       GetImageStateError)
{
    ...
    , 0);
    MockGetImageStateRequest mock_get_image_state_request;
    expect_get_image_state(mock_get_image_state_request, 1,
                           -EINVAL);
       ...
       UpdateMirrorImageStateError)
{
    ...
    MockImageStateUpdateRequest mock_image_state_update_request;
    expect_update_mirror_image_state
    (mock_image_state_update_request, -EIO);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, RemoteFailover)
{
    ...
    expect_notify_update(mock_local_image_ctx);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    expect_unlink_peer(mock_unlink_peer_request, 2,
                       "remote mirror peer uuid", false, 0);
    expect_notify_sync_complete(mock_instance_watcher,
                                mock_local_image_ctx.id);
       ...
       ApplyImageStatePendingShutdown)
{
    ...
    expect_cancel_sync_request(mock_instance_watcher,
                               mock_local_image_ctx.id);
    expect_mirror_image_snapshot_set_copy_progress
    (mock_local_image_ctx, 11, true, 0, 0);
    expect_notify_update(mock_local_image_ctx);
