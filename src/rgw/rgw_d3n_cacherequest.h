...
struct D3nL1CacheRequest {
            ...
            if (c->aio_fildes > 0) {
                if (::close(c->aio_fildes) != 0) {
                    lsubdout(g_ceph_context, rgw_datacache,
                             2) << "D3nDataCache: " << __func__ << "(): Error - can't close file, errno=" << -errno << dendl;
                }
            }
            delete c;
        }
    ...
    struct AsyncFileReadOp {
                            ...
                            void *arg)
        {
            ...
            if (aio_cb->aio_fildes < 0) {
                int err = errno;
                ldpp_dout(dpp, 1) << "ERROR: D3nDataCache: " << __func__ << "(): can't open " << location << " : " << cpp_strerror(
                                      err) << dendl;
                return -err;
            }
                                ...
                                rgw::Aio *aio, rgw::AioResult &r)
    {
        ...
        async_read(dpp, context, cache_location + "/" + url_encode(r.obj.oid, true), read_ofs, read_len, bind_executor(ex,
                   d3n_libaio_handler{aio, r}));
    }
