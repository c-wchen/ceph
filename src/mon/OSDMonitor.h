    ...
    utime_t max_failed_since;   ///< most recent failed_since
     ...
     failure_info_t() {
        ...
        if (max_failed_since == utime_t() && !reporters.empty()) {
            ...
            for (map < int, failure_reporter_t >::iterator p =
                 reporters.begin(); p != reporters.end(); ++p)
    ...
    MonOpRequestRef add_report(int who, utime_t failed_since,
        ...
        if (p == reporters.end()) {
            if (max_failed_since != utime_t()
                ...
                max_failed_since = failed_since;
            p = reporters.insert(map < int,
                                 failure_reporter_t >::value_type(who,
                                                                  failure_reporter_t
                                                                  (failed_since))).
                first;
        }
...
class OSDMonitor:public PaxosService {
  ...
  public:
    ...
    using osdmap_cache_t = SimpleLRU < osdmap_key_t,
        bufferlist, std::less < osdmap_key_t >, boost::hash < osdmap_key_t >>;
  ...
  private:
    ...
    int normalize_profile(const string & profilename,
                          ...
                          bool force, ostream * ss);
    int crush_rule_create_erasure(const string & name,
                                  const string & profile,
                                  int *rule, ostream * ss);
    int get_crush_rule(const string & rule_name, int *crush_rule, ostream * ss);
    ...
    int prepare_pool_crush_rule(const unsigned pool_type,
                                ...
                                int *crush_rule, ostream * ss);
    bool erasure_code_profile_in_use(const mempool::osdmap::map < int64_t,
                                     pg_pool_t > &pools, const string & profile,
                                     ostream * ss);
    int parse_erasure_code_profile(const vector < string >
                                   &erasure_code_profile, map < string,
                                   string > *erasure_code_profile_map,
                                   ostream * ss);
    int prepare_pool_size(const unsigned pool_type,
                          const string & erasure_code_profile, unsigned *size,
                          unsigned *min_size, ostream * ss);
    int prepare_pool_stripe_width(const unsigned pool_type,
                                  const string & erasure_code_profile,
                                  unsigned *stripe_width, ostream * ss);
    ...
    int prepare_new_pool(string & name, uint64_t auid,
                         ...
                         FastReadType fast_read, ostream * ss);
  ...
  protected:
    ...
    bool is_pool_currently_all_bluestore(int64_t pool_id,
                                         const pg_pool_t & pool, ostream * err);
    ...
    std::map < int, std::map < epoch_t,
        std::set < pg_t >>> creating_pgs_by_osd_epoch;
  ...
  public:
    ...
    bool prepare_command_impl(MonOpRequestRef op, map < string,
                              cmd_vartype > &cmdmap);
    ...
    void do_osd_create(const int32_t id, const uuid_d & uuid,
                       const string & device_class, int32_t * new_id);
