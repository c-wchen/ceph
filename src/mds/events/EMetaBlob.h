...
class EMetaBlob {
  ...
  public:
    ...
    struct fullbit {
       ...
       fullbit(boost::string_view d, snapid_t df, snapid_t dl, version_t v, const CInode::mempool_inode & i, const fragtree_t & dft, const CInode::mempool_xattr_map & xa, boost::string_view sym, snapid_t os, const bufferlist & sbl, __u8 st, const CInode::mempool_old_inode_map * oi = NULL):
    ...
    WRITE_CLASS_ENCODER_FEATURES(fullbit)
        ...
        bool dirty;
         ...
         remotebit(boost::string_view d, snapid_t df, snapid_t dl, version_t v,
                   inodeno_t i, unsigned char dt, bool dr):dn(d), dnfirst(df),
            dnlast(dl), dnv(v), ino(i), d_type(dt), dirty(dr) {
      ...
      remotebit():dnfirst(0), dnlast(0), dnv(0), ino(0),
        ...
        void print(ostream & out) const {
            out << " remotebit dn " << dn << " [" << dnfirst << "," << dnlast <<
                "] dnv " << dnv << " ino " << ino << " dirty=" << dirty << std::
                endl;
        } void dump(Formatter * f) const;
    ...
    WRITE_CLASS_ENCODER(remotebit)
        ...
        bool dirty;
         ...
         nullbit(boost::string_view d, snapid_t df, snapid_t dl, version_t v,
      ...
      public:
        ...
        void print(dirfrag_t dirfrag, ostream & out) {
            ...
            for (list < ceph::shared_ptr < fullbit > >::iterator p =
                 dfull.begin(); p != dfull.end(); ++p)
                (*p)->print(out);
            for (list < remotebit >::iterator p = dremote.begin();
                 p != dremote.end(); ++p)
                p->print(out);
            for (list < nullbit >::iterator p = dnull.begin(); p != dnull.end();
                 ++p)
                p->print(out);
        }
  ...
  public:
    ...
    void add_root(bool dirty, CInode * in, const CInode::mempool_inode * pi =
                  0, fragtree_t * pdft = 0, bufferlist * psnapbl =
                  0, CInode::mempool_xattr_map * px = 0) {
        in->last_journaled = event_seq;
        //cout << "journaling " << in->inode.ino << " at " << my_offset << std::endl;
        ...
        roots.push_back(ceph::shared_ptr < fullbit >
                        (new
                         fullbit(empty, in->first, in->last, 0, *pi, *pdft, *px,
    ...
    dirlump & add_dir(CDir * dir, bool dirty, bool complete = false) {
        return add_dir(dir->dirfrag(), dir->get_projected_fnode(),
                       dir->get_projected_version(), dirty, complete);
    }
    dirlump & add_new_dir(CDir * dir) {
        return add_dir(dir->dirfrag(), dir->get_projected_fnode(), dir->get_projected_version(), true, true, true); // dirty AND complete AND new
    }
    dirlump & add_import_dir(CDir * dir) {
        // dirty=false would be okay in some cases
        return add_dir(dir->dirfrag(), dir->get_projected_fnode(),
                       dir->get_projected_version(), dir->is_dirty(),
                       dir->is_complete(), false, true, dir->is_dirty_dft());
    }
    dirlump & add_fragmented_dir(CDir * dir, bool dirty, bool dirtydft) {
        return add_dir(dir->dirfrag(), dir->get_projected_fnode(),
                       dir->get_projected_version(), dirty, false, false, false,
                       dirtydft);
    }
    dirlump & add_dir(dirfrag_t df, const fnode_t * pf, version_t pv,
