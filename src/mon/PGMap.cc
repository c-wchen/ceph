...
void PGMapDigest::dump(ceph::Formatter * f) const const
{
...
void PGMapDigest::print_summary(ceph::Formatter * f, ostream * out) const const
{
    if (f)
    ...
    if (f) {
        ...
        *out << "    usage:   " << byte_u_t(osd_sum.statfs.
                                            get_used_raw()) << " used, " <<
            byte_u_t(osd_sum.statfs.available) << " / " << byte_u_t(osd_sum.
                                                                    statfs.
                                                                    total) <<
            " avail\n";
        *out << "    pgs:     ";
...
void PGMapDigest::print_oneline_summary(ceph::Formatter * f, ostream * out) const const
{
    std::stringstream ss;
...
void PGMapDigest::get_recovery_stats(double *misplaced_ratio,
                                     double *degraded_ratio,
                                     double *inactive_pgs_ratio,
                                     double *unknown_pgs_ratio) const const
{
...
void PGMapDigest::overall_recovery_rate_summary(ceph::Formatter * f,
                                                ostream * out) const const
{
    recovery_rate_summary(f, out, pg_sum_delta, stamp_delta);
}

void PGMapDigest::overall_recovery_summary(ceph::Formatter * f,
                                           list < string > *psl) const const
{
...
void PGMapDigest::overall_client_io_rate_summary(ceph::Formatter * f,
                                                 ostream * out) const const
{
    client_io_rate_summary(f, out, pg_sum_delta, stamp_delta);
}

void PGMapDigest::pool_client_io_rate_summary(ceph::Formatter * f,
                                              ostream * out,
                                              uint64_t poolid) const const
{
    auto p = per_pool_sum_delta.find(poolid);
    ...
    if (pos_delta.stats.sum.num_flush_mode_low) {
        if (f) {
            ...
            *out << si_u_t(pos_delta.stats.sum.
                           num_flush_mode_low) << " PGs flushing";
            have_output = true;
        }
    }
    if (pos_delta.stats.sum.num_flush_mode_high) {
        if (f) {
            ...
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_flush_mode_high) << " PGs flushing (high)";
            have_output = true;
        }
    }
    if (pos_delta.stats.sum.num_evict_mode_some) {
        if (f) {
            ...
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_evict_mode_some) << " PGs evicting";
            have_output = true;
        }
    }
    if (pos_delta.stats.sum.num_evict_mode_full) {
        if (f) {
            ...
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_evict_mode_full) << " PGs evicting (full)";
        }
    }
}

void PGMapDigest::overall_cache_io_rate_summary(ceph::Formatter * f,
                                                ostream * out) const const
{
                                       ...
                                       bool verbose) const const
{
    ...
    if (f) {
        ...
        if (verbose) {
            tbl.define_column("QUOTA OBJECTS", TextTable::RIGHT,
                              TextTable::RIGHT);
            tbl.define_column("QUOTA BYTES", TextTable::RIGHT,
                              TextTable::RIGHT);
            tbl.define_column("DIRTY", TextTable::RIGHT, TextTable::RIGHT);
            tbl.define_column("USED COMPR", TextTable::RIGHT, TextTable::RIGHT);
            tbl.define_column("UNDER COMPR", TextTable::RIGHT,
                              TextTable::RIGHT);
        }
    }
    ...
    for (auto p = osd_map.get_pools().begin();
        ...
        dump_object_stat_sum(tbl, f, stat, avail, raw_used_rate, verbose,
                             per_pool, per_pool_omap, pool);
        if (f) {
                                     ...
                                     bool verbose) const const
{
    if (f) {
        ...
        f->dump_float("total_used_raw_ratio",
                      osd_sum.statfs.get_used_raw_ratio());
        f->dump_unsigned("num_osds", osd_sum.num_osds);
        f->dump_unsigned("num_per_pool_osds", osd_sum.num_per_pool_osds);
        f->dump_unsigned("num_per_pool_omap_osds",
                         osd_sum.num_per_pool_omap_osds);
        f->close_section();
...
void PGMapDigest::dump_object_stat_sum(TextTable & tbl, ceph::Formatter * f,
                                       ...
                                       const pg_pool_t * pool)
{
    ...
    uint64_t used_omap_bytes =
        pool_stat.get_allocated_omap_bytes(per_pool_omap);
    ...
    if (f) {
        ...
        if (verbose) {
            ...
            f->dump_int("compress_bytes_used",
                        statfs.data_compressed_allocated);
            f->dump_int("compress_under_bytes",
...
int64_t PGMap::get_rule_avail(const OSDMap & osdmap, int ruleno) constconst
{
    ...
    for (auto p = wm.begin(); p != wm.end(); ++p) {
        ...
        if (osd_info != osd_stat.end()) {
            ...
            double avail =
                std::max(0.0,
                         (double)osd_info->second.statfs.kb_avail() - unusable);
            avail *= 1024.0;
...
void PGMap::Incremental::dump(ceph::Formatter * f) const const
{
    ...
    for (auto p = inc.pool_statfs_updates.begin();
        ...
        if (pg_pool_sum.count(update_pool)) {
            ...
            if (pool_statfs_iter == pool_statfs.end()) {
                pool_statfs.emplace(std::make_pair(update_pool, update_osd),
                                    statfs_inc);
            }
            else {
...
void PGMap::dump(ceph::Formatter * f, bool with_net) const const
{
    dump_basic(f);
...
void PGMap::dump_basic(ceph::Formatter * f) const const
{
    f->dump_unsigned("version", version);
    f->dump_stream("stamp") << stamp;
...
void PGMap::dump_delta(ceph::Formatter * f) const const
{
...
void PGMap::dump_pg_stats(ceph::Formatter * f, bool brief) const const
{
...
void PGMap::dump_pg_progress(ceph::Formatter * f) const const
{
  ...
  for (auto & i:pg_stat) {
        ...
        f->dump_int("num_bytes_recovered",
                    i.second.stats.sum.num_bytes_recovered);
...
void PGMap::dump_pool_stats(ceph::Formatter * f) const const
{
    f->open_array_section("pool_stats");
...
void PGMap::dump_osd_stats(ceph::Formatter * f, bool with_net) const const
{
    f->open_array_section("osd_stats");
...
void PGMap::dump_osd_ping_times(ceph::Formatter * f) const const
{
                                ...
                                pg_stat_t > &pg_stats, bool brief)
{
    ...
    if (brief) {
        ...
        tab.define_column("LAST_SCRUB_DURATION", TextTable::LEFT,
                          TextTable::RIGHT);
        tab.define_column("SCRUB_SCHEDULING", TextTable::LEFT, TextTable::LEFT);
        tab.define_column("OBJECTS_SCRUBBED", TextTable::LEFT,
                          TextTable::RIGHT);
  ...
  for (const auto &[pg, st]:pg_stats) {
        if (brief) {
            tab << pg << pg_state_string(st.state)
                << st.up
                << st.up_primary
...
void PGMap::dump_stuck(ceph::Formatter * f, int types, utime_t cutoff) const const
{
...
void PGMap::dump_osd_perf_stats(ceph::Formatter * f) const const
{
...
void PGMap::dump_osd_blocked_by_stats(ceph::Formatter * f) const const
{
...
void PGMap::update_pool_deltas(CephContext * cct, const utime_t ts,
                               const mempool::pgmap::unordered_map < int32_t,
...
void PGMap::dump_filtered_pg_stats(ceph::Formatter * f, set < pg_t > &pgs) const const
{
    f->open_array_section("pg_stats");
...
set < std::string > PGMap::osd_parentage(const OSDMap & osdmap, int id) const const
{
...
health_code = "PG_DEGRADED"; summary = "Degraded data redundancy: "; sev = HEALTH_WARN; break; case BACKFILL_FULL:
health_code = "PG_BACKFILL_FULL"; summary = "Low space hindering backfill (add storage if this doesn't resolve itself): "; sev = HEALTH_WARN; break; case DAMAGED:
health_code = "PG_DAMAGED"; summary = "Possible data damage: "; sev = HEALTH_ERR; break; case RECOVERY_FULL:
health_code = "PG_RECOVERY_FULL"; summary = "Full OSDs blocking recovery: "; sev = HEALTH_ERR; break; default:
                                              ceph_abort
                                              ();}
      ...
      std::vector < std::string > pg_msgs; int64_t count = 0; for (const auto & j:i.second.states)
                                              {
      ...
      list < string > detail; unsigned num_pools = 0; for (auto & p:pools)
                                              ...
                                              double ratio = cct->_conf.get_val < double >("mon_warn_on_slow_ping_ratio"); warn_slow_ping_time = grace; warn_slow_ping_time *= 1000000 * ratio; // Seconds of grace to microseconds at ratio
                                              }
                                              if (warn_slow_ping_time > 0) {
                                              ...
                                              return true;
                                              if (pingtime > rhs.pingtime)
                                              return false; if (from < rhs.from)
                                              return true; if (from > rhs.from)
                                              return false; return to < rhs.to;}
      }; list < string > detail_back; list < string > detail_front; list < string > detail; set < mon_ping_item_t > back_sorted, front_sorted; for (auto i:osd_stat)
                                              {
          for (auto j:                       i.second.
                                                   hb_pingtime)
                                              {
                                              ...
                                              if (now.sec() -
                                                  j.second.last_update >
                                                  grace * 60)
                                              continue; mon_ping_item_t back;
                                              back.pingtime =
                                              std::max(j.second.
                                                       back_pingtime[0],
                                                       j.second.
                                              ...
                                              if (back.pingtime >
                                                  warn_slow_ping_time) {
                                              back.improving =
                                              (j.second.back_pingtime[0] <
                                               j.second.back_pingtime[1]
                                               && j.second.back_pingtime[1] <
                                               j.second.back_pingtime[2]);
                                              ...
                                              ostringstream ss;
                                              ss <<
                                              "Too many repaired reads on " <<
                                              detail.size() << " OSDs";
                                              auto & d =
                                              checks->
                                              add("OSD_TOO_MANY_REPAIRS",
                                                  HEALTH_WARN, ss.str(),
                                                  detail.size());
                                              d.detail.swap(detail);}
      int max_detail = 10; for (auto & sback:boost::adaptors::reverse(back_sorted)) {
                                              ostringstream ss;
                                              if (max_detail == 0) {
                                              ss <<
                                              ...
                                              " msec" << (sback.
                                                          ...
                                                          : "");
                                              detail_back.push_back(ss.str());}
      max_detail = 10; for (auto & sfront:   boost::adaptors::reverse
                                                                    (front_sorted))
                                              {
                                              ostringstream ss;
                                              if (max_detail == 0) {
                                              ...
                                              (osdmap.
                                               is_down(sfront.
                                                       to) ? " (down)" : "")
                                              << " " <<
                                              fixed_u_to_string(sfront.pingtime,
                                                                3) << " msec" <<
                                              (sfront.
                                               improving ? " possibly improving"
                                               : "");
                                              detail_front.push_back(ss.str());}
                                              ...
                                              << fixed_u_to_string(back_sorted.
                                                                   rbegin()->
                                                                   pingtime,
                                                                   3) << "ms)";
                                              auto & d =
                                              checks->
                                              add("OSD_SLOW_PING_TIME_BACK",
                                                  HEALTH_WARN, ss.str(),
                                                  back_sorted.size());
                                              ...
                                              << fixed_u_to_string(front_sorted.
                                                                   rbegin()->
                                                                   pingtime,
                                                                   3) << "ms)";
                                              auto & d =
                                              checks->
                                              add("OSD_SLOW_PING_TIME_FRONT",
                                                  HEALTH_WARN, ss.str(),
                                                  front_sorted.size());
                                              ...
                                              if (mon_pg_warn_max_object_skew >
                                                  ...
                                                  mon_pg_warn_max_object_skew) {
                                              ostringstream ss;
                                              if (pi->pg_autoscale_mode !=
                                                  pg_pool_t::
                                                  pg_autoscale_mode_t::ON) {
                                              ss << "pool " << name <<
                                              " objects per pg (" <<
                                              objects_per_pg <<
                                              ") is more than " << ratio <<
      ...
      double pc = (double)pg_sum.stats.sum.num_objects_unfound / (double)pg_sum.stats.sum.num_objects * (double)100.0; char b[20]; snprintf(b, sizeof(b), "%.3lf", pc); ostringstream ss; ss << pg_sum.stats.sum.num_objects_unfound << "/" << pg_sum.stats.sum.num_objects << " objects unfound (" << b << "%)"; auto & d = checks->add("OBJECT_UNFOUND", HEALTH_WARN, ss.str(), pg_sum.stats.sum.num_objects_unfound); for (auto & p:pg_stat)
          ...
          for (auto & p:                     osd_stat) {
                                              ...
                                              if (!warn_detail.empty()) {
          ostringstream ss; ss << warn << " slow requests are blocked > " << cct->_conf->mon_osd_warn_op_age << " sec"; auto & d = checks->add("REQUEST_SLOW", HEALTH_WARN, ss.str(), warn); d.detail.swap(warn_detail); int left = max; for (auto & p:warn_osd_by_max)
                                              {
                                              ostringstream ss;
                                              if (p.second.size() > 1) {
                                              ss << "osds " << p.
          ...
          ostringstream ss; ss << error << " stuck requests are blocked > " << err_age << " sec"; auto & d = checks->add("REQUEST_STUCK", HEALTH_ERR, ss.str(), error); d.detail.swap(error_detail); int left = max; for (auto & p:error_osd_by_max)
                                              {
                                              ostringstream ss;
                                              if (p.second.size() > 1) {
                                              ss << "osds " << p.
      ...
      map < string, pair < size_t, list < string >>> os_alerts_sum; for (auto & a:osd_sum.os_alerts)
                                              {
          int left = max; string s0 = " osd."; s0 += stringify(a.first); for (auto & aa:a.second)
                                              {
                                              string s(s0); s += " ";
                                              s += aa.second;
                                              auto it =
                                              os_alerts_sum.find(aa.first);
                                              if (it == os_alerts_sum.end()) {
                                              list < string > d;
                                              d.emplace_back(s);
                                              os_alerts_sum.emplace(aa.first,
                                                                    std::
                                                                    make_pair(1,
                                                                              d));}
                                              else {
...
auto & d = checks->add(asum.first, HEALTH_WARN, summary, asum.second.first); for (auto & s:asum.second.second)
                                              {
      ...
      list < string > detail; for (auto & it:pools)
                                              ...
                                              ss << detail.
                                              size() <<
                                              " pool(s) do not have an application enabled";
                                              auto & d =
                                              checks->
      ...
      uint32_t snapthreshold = cct->_conf->mon_osd_snap_trim_queue_warn_on; uint64_t snaptrimq_exceeded = 0; uint32_t longest_queue = 0; const pg_t * longest_q_pg = nullptr; list < string > detail; for (auto & i:pg_stat)
                                              ...
                                              *longest_q_pg << " at " <<
                                              ...
                                              const {
                                              if (f) {
      f->open_array_section("pgs_by_pool_state"); for (auto & i:num_pg_by_pool_state)
                                              {
          f->open_object_section("per_pool_pgs_by_state"); f->dump_int("pool_id", i.first); f->open_array_section("pg_state_counts"); for (auto & j:i.second)
                                              {
                                              f->
                                              open_object_section
                                              ("pg_state_count");
                                              f->dump_string("state_name",
                                                             pg_state_string(j.
                                                                             first));
...
bool degraded_pgs_exist = false; for (const auto & p:pg_map.pg_stat)
  ...
  map < int64_t, unsigned >new_pg_num; for (auto & p:osdmap.get_pools())
                                              {
      ...
      for (auto osd:                         need_check_down_pg_osds)
              ...
              for (auto pgid:                p->second)
                                              ...
                                              ceph_assert(stat.acting_primary ==
                                                          osd);
          ...
          continue; for (const auto acting:  pg.second.acting)
                                              ...
                                              *ss <<
                                              ...
                                              std::max < size_t > (1,
                                                                   pgm.osd_stat.
                                                                   size());
                                              if ((uint64_t) pgm.osd_sum.statfs.
                                                  total / num_osd <
                                                  g_conf()->
