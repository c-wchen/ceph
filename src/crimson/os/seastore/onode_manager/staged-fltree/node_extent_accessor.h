 ...
 */
    template < typename FieldType, node_type_t NODE_TYPE >
      ...
      public:
        ...
        template < KeyT KT >
            void encode_insert(const full_key_t < KT > &key,
        ...
        } void encode_split(const StagedIterator & split_at,
        ...
        template < KeyT KT >
            void encode_split_insert(const StagedIterator & split_at,
        ...
        void encode_update_child_addr(const laddr_t new_addr,
            ...
            int node_offset =
                reinterpret_cast < const char *>(p_addr) - p_node_start;
        ...
        static DeltaRecorderURef create_for_encode(const ValueBuilder &
                                                   v_builder) {
            return std::unique_ptr < DeltaRecorder >
                (new DeltaRecorderT(v_builder));
        }
      ...
      DeltaRecorderT(const ValueBuilder & vb):DeltaRecorder(vb) {
        ...
        node_type_t node_type() const override {
            ...
            assert(is_empty());
            node_stage_t stage(reinterpret_cast <
                               const FieldType * >(mut.get_read()),
                               mut.get_length());
            node_delta_op_t op;
            try {
                ...
                case node_delta_op_t::INSERT:{
                        ...
                        SUBDEBUG(seastore_onode,
                                 ...
                                 "insert_size={}B ...",
                                 key, value, insert_pos, insert_stage,
                                 insert_size);
                        layout_t::template insert < KeyT::HOBJ > (mut, stage,
                                                                  key, value,
                                                                  insert_pos,
                                                                  insert_stage,
                                                                  insert_size);
                ...
                case node_delta_op_t::SPLIT:{
                        ...
                        auto split_at =
                            StagedIterator::decode(mut.get_read(),
                                                   mut.get_length(), delta);
                        SUBDEBUG(seastore_onode, "apply split_at={} ...",
                                 split_at);
                ...
                case node_delta_op_t::SPLIT_INSERT:{
                        ...
                        SUBDEBUG(seastore_onode,
                                 ...
                                 insert_size);
                        layout_t::template split_insert < KeyT::HOBJ > (mut,
                                                                        stage,
                                                                        split_at,
                                                                        key,
                                                                        value,
                ...
                case node_delta_op_t::UPDATE_CHILD_ADDR:{
                        ...
                        auto p_addr =
                            reinterpret_cast <
                            laddr_packed_t * >(mut.get_write() + update_offset);
                        SUBDEBUG(seastore_onode,
                                 "apply {:#x} to offset {:#x} ...", new_addr,
                                 update_offset);
                ...
                case node_delta_op_t::ERASE:{
                        ...
                        SUBDEBUG(seastore_onode, "apply erase_pos({}) ...",
                                 erase_pos);
                ...
                default:
                    SUBERROR(seastore_onode,
                             "got unknown op {} when replay {}", op, node);
                    ceph_abort("fatal error");
                }
            }
            catch(buffer::error & e) {
                SUBERROR(seastore_onode,
                         "got decode error {} when replay {}", e.what(), node);
                ceph_abort("fatal error");
            }
        }
      ...
      private:
        ValueDeltaRecorder * get_value_replayer(value_magic_t magic) {
            ...
            if (value_replayer) {
                ...
                if (!value_replayer) {
                    ceph_abort_msgf
                        ("OTree::Extent::Replay: got unexpected value magic = %x",
                         magic);
                }
            }
        ...
        void encode_value(const value_input_t & value,
                          ceph::bufferlist & encoded) const {
      ...
      :    extent {
        ...
        void prepare_mutate(context_t c) {
            ...
            if (state == nextent_state_t::READ_ONLY) {
                ...
                node_stage =
                    node_stage_t(reinterpret_cast <
                                 const FieldType * >(extent->get_read()),
                                 get_length());
                assert(recorder == static_cast <
                       recorder_t * >(extent->get_recorder()));
                mut.emplace(extent->get_mutable());
            }
            assert(extent->is_mutable());
        }
        ...
        template < KeyT KT >
            const value_t *insert_replayable(const full_key_t < KT > &key,
                                             const value_input_t & value,
            ...
            if (state == nextent_state_t::MUTATION_PENDING) {
                recorder->template encode_insert < KT > (key, value, insert_pos,
                                                         insert_stage,
                                                         insert_size);
            }
#ifndef NDEBUG
            test_extent->prepare_replay(extent);
            test_recorder->template encode_insert < KT > (key, value,
                                                          insert_pos,
                                                          insert_stage,
                                                          insert_size);
#endif
            auto ret =
                layout_t::template insert < KT > (*mut, read(), key, value,
        ...
        template < KeyT KT >
            const value_t *split_insert_replayable(StagedIterator & split_at,
                                                   const full_key_t < KT > &key,
                                                   const value_input_t & value,
                                                   position_t & insert_pos,
                                                   match_stage_t & insert_stage,
                                                   node_offset_t & insert_size)
        {
            ...
            assert(state != nextent_state_t::READ_ONLY);
            if (state == nextent_state_t::MUTATION_PENDING) {
                recorder->template encode_split_insert < KT > (split_at, key,
                                                               value,
                                                               insert_pos,
                                                               insert_stage,
                                                               insert_size,
...
#ifndef NDEBUG
            test_extent->prepare_replay(extent);
            test_recorder->template encode_split_insert < KT > (split_at, key,
                                                                value,
                                                                insert_pos,
                                                                insert_stage,
                                                                insert_size,
...
#endif
            auto ret =
                layout_t::template split_insert < KT > (*mut, read(), split_at,
                                                        key, value,
                                                        insert_pos,
                                                        insert_stage,
                                                        insert_size);
        ...
        void update_child_addr_replayable(const laddr_t new_addr,
                                          laddr_packed_t * p_addr) {
            assert(extent->is_mutable());
            assert(state != nextent_state_t::READ_ONLY);
            if (state == nextent_state_t::MUTATION_PENDING) {
                recorder->encode_update_child_addr(new_addr, p_addr,
                                                   read().p_start(),
                                                   get_length());
            }
#ifndef NDEBUG
            test_extent->prepare_replay(extent);
            test_recorder->encode_update_child_addr(new_addr, p_addr,
                                                    read().p_start(),
                                                    get_length());
#endif
            layout_t::update_child_addr(*mut, new_addr, p_addr);
        ...
        std::tuple < match_stage_t,
            position_t > erase_replayable(const position_t & pos) {
            assert(extent->is_mutable());
            assert(state != nextent_state_t::READ_ONLY);
            if (state == nextent_state_t::MUTATION_PENDING) {
        ...
        void test_copy_to(NodeExtentMutable & to) const {
            ...
            if (state == nextent_state_t::FRESH) {
                ...
                return eagain_iertr::make_ready_future < NodeExtentMutable >
                    (*mut);
            }
            assert(!extent->is_initial_pending());
            ...
            return c.nm.alloc_extent(c.t, hint,
                ...
                handle_error_interruptible(eagain_iertr::pass_further {
                          ...
                          std::memcpy(fresh_mut.get_write(), extent->get_read(),
                                      get_length());
                          ...
                          node_stage_t(reinterpret_cast <
                                       ...
                                       get_length());
                          state = nextent_state_t::FRESH;
                          mut.emplace(fresh_mut); recorder = nullptr;
                          return c.nm.retire_extent(c.t,
                                                    to_discard).
                          handle_error_interruptible(eagain_iertr::
                                                     pass_further {
                                                     }
                                                     ,
                                                     crimson::ct_error::
                      ...
                      assert(!c.t.is_conflicted()); return *mut;}
            );
        }
