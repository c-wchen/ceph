...
public:
    ...
    void add_namespace(int64_t pool_id,
                       const std::string &namespace_name);
    void remove_namespace(int64_t pool_id,
                          const std::string &namespace_name);
    ...
    void add_or_update_attribute(int64_t pool_id,
                                 const std::string &key,
                                 const service_daemon::
                                 AttributeValue &value);
    void add_or_update_namespace_attribute(int64_t pool_id,
                                           const std::
...
private:
    struct Callout {
        ...
        {
        } Callout(service_daemon::CalloutLevel level,
                  const std::string &text)
    ...
    typedef std::map < std::string,
            service_daemon::AttributeValue > Attributes;
