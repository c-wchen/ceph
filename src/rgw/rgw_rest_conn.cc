...
int RGWRESTConn::get_url(string &endpoint)
{
    if (endpoints.empty()) {
        ldout(cct,
              0) << "ERROR: endpoints not configured for upstream zone" <<
                 dendl;
        return -EIO;
    }
...
void RGWRESTConn::populate_params(param_vec_t &params, const rgw_user *uid,
...
int RGWRESTConn::forward(const DoutPrefixProvider *dpp, const rgw_user &uid,
                         req_info &info, obj_version *objv,
                                   ...
                                   RGWRESTStreamS3PutObj **req)
{
    ...
    RGWRESTStreamS3PutObj *wr =
        new RGWRESTStreamS3PutObj(cct, "PUT", url, NULL, &params, api_name,
                                  host_style);
    wr->send_init(obj);
    *req = wr;
    return 0;
...
int RGWRESTConn::put_obj_async_init(const DoutPrefixProvider *dpp,
                                    const rgw_user &uid, const rgw_obj &obj,
...
static void set_date_header(const real_time *t, map < string,
                            string > &headers, bool high_precision_time,
...
int RGWRESTConn::get_obj(const DoutPrefixProvider *dpp, const rgw_user &uid,
                         ...
                         RGWRESTStreamRWRequest **req)
{
    ...
    if (in_params.prepend_metadata) {
        ...
        push_back(param_pair_t
                  (RGW_SYS_PARAM_PREFIX "prepend-metadata", "true"));
    }
    ...
    if (in_params.sync_manifest) {
        params.
        push_back(param_pair_t(RGW_SYS_PARAM_PREFIX "sync-manifest", ""));
    }
    if (in_params.sync_cloudtiered) {
        params.
        push_back(param_pair_t
                  (RGW_SYS_PARAM_PREFIX "sync-cloudtiered", ""));
    }
    ...
    if (in_params.get_op) {
        *req =
            new RGWRESTStreamReadRequest(cct, url, in_params.cb, NULL, &params,
                                         api_name, host_style);
    } else {
        *req =
            new RGWRESTStreamHeadRequest(cct, url, in_params.cb, NULL, &params,
                                         api_name);
    }
    ...
    if (in_params.info) {
        ...
        for (auto iter = orig_map.lower_bound(SEARCH_AMZ_PREFIX);
            ...
            if (name.compare(0, strlen(SEARCH_AMZ_PREFIX), SEARCH_AMZ_PREFIX) !=
                0) {
                break;
            }
    ...
    if (in_params.mod_zone_id != 0) {
        set_header(in_params.mod_zone_id, extra_headers,
    ...
    if (in_params.range_is_set) {
        ...
        snprintf(buf, sizeof(buf), "bytes=%lld-%lld",
                 (long long)in_params.range_start,
                              ...
                              RGWHTTPManager *mgr, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  5) << __func__ << ": send_request() resource=" << resource <<
...
int RGWRESTConn::send_resource(const DoutPrefixProvider *dpp,
                               ...
                               std::map < std::string,
                               std::string > *extra_headers, bufferlist &bl,
                               ...
                               optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp,
                  5) << __func__ << ": complete_request() resource=" << resource
                     << " returned ret=" << ret << dendl;
    }
...
RGWRESTSendResource::RGWRESTSendResource(RGWRESTConn *_conn,
        ...
        RGWHTTPManager *_mgr)
    : cct(_conn->get_ctx()), conn(_conn), method(_method), resource(_resource),
      params(params), cb(bl), mgr(_mgr), req(cct, method.c_str(), conn->get_url(),
              &cb, NULL, NULL, _conn->get_api_name(),
...
int RGWRESTSendResource::send(const DoutPrefixProvider *dpp,
                              bufferlist &outbl, optional_yield y)
{
