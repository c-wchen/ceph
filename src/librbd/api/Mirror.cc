...
namespace librbd {
    namespace api {
        ...
        namespace {
            ...
            template < typename I > int validate_mirroring_enabled(I * ictx) {
            ...
            int list_mirror_images(librados::IoCtx & io_ctx,
                ...
                do {
                    ...
                    r = cls_client::mirror_image_list(&io_ctx, last_read,
                    ...
                    for (auto it = mirror_images.begin();
                         it != mirror_images.end(); ++it) {
            ...
            struct C_ImageGetStatus:public C_ImageGetInfo {
                ...
                :C_ImageGetInfo(&mirror_image_status->info, on_finish),
                    ...
                    mirror_image_status->state =
                        static_cast < mirror_image_status_state_t >
                        (mirror_image_status_internal.state);
                    mirror_image_status->description =
                        mirror_image_status_internal.description;
        ...
        template < typename I >
                ...
                if (parent) {
                    if (relax_same_pool_parent_check &&
                        ...
                        if (!parent->test_features(RBD_FEATURE_JOURNALING)) {
                            lderr(cct) <<
                                "journaling is not enabled for the parent" <<
                                dendl;
                            return -EINVAL;
                        }
                    }
                        ...
                        cls::rbd::MirrorImage mirror_image_internal;
                        r = cls_client::mirror_image_get(&(parent->md_ctx),
                                                         parent->id,
                                                         &mirror_image_internal);
                        if (r == -ENOENT) {
                            lderr(cct) <<
            ...
            if ((ictx->features & RBD_FEATURE_JOURNALING) == 0) {
                lderr(cct) <<
                    "cannot enable mirroring: journaling is not enabled" <<
                    dendl;
                return -EINVAL;
            }
            ...
            r = ctx.wait();
            if (r < 0) {
                lderr(cct) << "cannot enable mirroring: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
        ...
        template < typename I >
            ...
            r = cls_client::mirror_image_set(&ictx->md_ctx, ictx->id,
                                             mirror_image_internal);
            if (r < 0) {
                lderr(cct) << "cannot disable mirroring: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
            else {
                ...
                BOOST_SCOPE_EXIT_ALL(ictx, &mirror_image_internal, &rollback) {
                    if (rollback) {
                        ...
                        mirror_image_internal.state =
                            cls::rbd::MIRROR_IMAGE_STATE_ENABLED;
                        int r =
                            cls_client::mirror_image_set(&ictx->md_ctx,
                                                         ictx->id,
                                                         mirror_image_internal);
                        if (r < 0) {
                            lderr(cct) <<
...

                {
                    ...
                    map < librados::snap_t, SnapInfo > snap_info =
                        ictx->snap_info;
                  for (auto & info:snap_info) {
                        ParentSpec parent_spec(ictx->md_ctx.get_id(), ictx->id,
                                               info.first);
                        map < pair < int64_t, string >,
                            set < string > >image_info;

                        r = Image < I >::list_children(ictx, parent_spec,
                                                       &image_info);
                        if (r < 0) {
                            rollback = true;
                            return r;
                ...
                C_SaferCond ctx;
                auto req =
                    mirror::DisableRequest < ImageCtx >::create(ictx, force,
                                                                true,
                                                                &ctx);
                req->send();
                ...
                r = ctx.wait();
                if (r < 0) {
                    lderr(cct) << "cannot disable mirroring: " <<
                        cpp_strerror(r) << dendl;
                    rollback = true;
                    return r;
                }
        ...
        template < typename I >
            void Mirror < I >::image_promote(I * ictx, bool force,
                                             Context * on_finish) {
            CephContext *cct = ictx->cct;
            ...
            auto req =
                mirror::PromoteRequest <>::create(*ictx, force, on_finish);
            req->send();
        }
        ...
        template < typename I > int Mirror < I >::image_resync(I * ictx) {
            ...
            if (r < 0) {
                ...
                lderr(cct) << "image is primary, cannot resync to itself" <<
                    dendl;
                return -EINVAL;
            }
            ...
            if (r < 0) {
                lderr(cct) << "failed to request resync: " << cpp_strerror(r) <<
                    dendl;
                return r;
            }
        ...
        template < typename I >
            void Mirror < I >::image_get_info(I * ictx,
                                              mirror_image_info_t *
                                              mirror_image_info,
                                              size_t info_size,
                                              Context * on_finish) {
            ...
            auto req =
                mirror::GetInfoRequest < I >::create(*ictx, &ctx->mirror_image,
                                                     &ctx->promotion_state,
                                                     ctx);
            req->send();
        }
        ...
        template < typename I >
            int Mirror < I >::image_get_info(I * ictx,
                                             mirror_image_info_t *
                                             mirror_image_info,
                                             size_t info_size) {
            C_SaferCond ctx;
        ...
        template < typename I >
            void Mirror < I >::image_get_status(I * ictx,
            ...
            auto req =
                mirror::GetStatusRequest < I >::create(*ictx,
                                                       &ctx->
        ...
        template < typename I >
            int Mirror < I >::image_get_status(I * ictx,
                                               mirror_image_status_t * status,
                                               size_t status_size) {
            C_SaferCond ctx;
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(cct) << "failed to retrieve mirror mode: " <<
                    cpp_strerror(r)
                    << dendl;
                return r;
            }
            ...
            case cls::rbd::MIRROR_MODE_POOL:
                *mirror_mode =
                    static_cast < rbd_mirror_mode_t > (mirror_mode_internal);
                break;
            default:
        ...
        template < typename I >
            ...
            case RBD_MIRROR_MODE_POOL:
                next_mirror_mode =
                    static_cast < cls::rbd::MirrorMode > (mirror_mode);
                break;
            default:
                lderr(cct) << "unknown mirror mode ("
            ...
            if (next_mirror_mode == cls::rbd::MIRROR_MODE_DISABLED) {
                ...
                if (r < 0 && r != -ENOENT) {
                    lderr(cct) << "failed to list peers: " << cpp_strerror(r) <<
                        dendl;
                    return r;
                }
            ...
            if (current_mirror_mode == next_mirror_mode) {
                ...
                if (r < 0) {
                    lderr(cct) << "failed to allocate mirroring uuid: " <<
                        cpp_strerror(r)
                        << dendl;
                    return r;
                }
            ...
            if (current_mirror_mode != cls::rbd::MIRROR_MODE_IMAGE) {
                r = cls_client::mirror_mode_set(&io_ctx,
            ...
            if (next_mirror_mode == cls::rbd::MIRROR_MODE_POOL) {
                ...
                if (r < 0) {
                    lderr(cct) << "failed listing images: " << cpp_strerror(r)
                        << dendl;
                    return r;
                }
              ...
              for (const auto & img_pair:images) {
                    ...
                    if (r < 0) {
                        lderr(cct) << "error getting features for image " <<
                    ...
                    if ((features & RBD_FEATURE_JOURNALING) != 0) {
                        I *img_ctx =
                            I::create("", img_pair.second, nullptr, io_ctx,
                                      false);
                        r = img_ctx->state->open(false);
                        if (r < 0) {
                            lderr(cct) << "error opening image " << img_pair.
                        ...
                        if (r < 0) {
                            ...
                            lderr(cct) << "failed to close image " << img_pair.
                                first << ": " << cpp_strerror(close_r) << dendl;
                            return close_r;
                        }
                    }
                }
              ...
              for (const auto & img_id:image_ids) {
                    if (current_mirror_mode == cls::rbd::MIRROR_MODE_IMAGE) {
                        cls::rbd::MirrorImage mirror_image;
                        r = cls_client::mirror_image_get(&io_ctx, img_id,
                                                         &mirror_image);
                        if (r < 0 && r != -ENOENT) {
                            lderr(cct) <<
                        ...
                        I *img_ctx =
                            I::create("", img_id, nullptr, io_ctx, false);
                        r = img_ctx->state->open(false);
                        if (r < 0) {
            ...
            if (r < 0) {
                lderr(cct) << "failed to set mirror mode: " << cpp_strerror(r)
                    << dendl;
                return r;
            }
        ...
        template < typename I >
            int Mirror < I >::peer_add(librados::IoCtx & io_ctx,
                                       std::string * uuid,
        ...
        template < typename I >
            int Mirror < I >::peer_remove(librados::IoCtx & io_ctx,
        ...
        template < typename I >
            int Mirror < I >::peer_set_client(librados::IoCtx & io_ctx,
                                              const std::string & uuid,
                                              const std::string & client_name) {
            CephContext *cct = reinterpret_cast < CephContext * >(io_ctx.cct());
            ...
            int r =
                cls_client::mirror_peer_set_client(&io_ctx, uuid, client_name);
            if (r < 0) {
        ...
        template < typename I >
            ...
            int r =
                cls_client::mirror_peer_set_cluster(&io_ctx, uuid,
                                                    cluster_name);
            if (r < 0) {
        ...
        template < typename I >
            ...
            r = librbd::cls_client::mirror_image_status_list(&io_ctx, start_id,
                                                             max, &images_,
                                                             &statuses_);
            if (r < 0 && r != -ENOENT) {
            ...
            cls::rbd::MirrorImageStatus unknown_status(cls::rbd::
            ...
            for (auto it = images_.begin(); it != images_.end(); ++it) {
                ...
                if (image_name.empty()) {
                    lderr(cct) << "failed to find image name for image " <<
                        image_id << ", " << "using image id as name" << dendl;
                    image_name = image_id;
                }
                auto s_it = statuses_.find(image_id);
                auto & s =
                    s_it != statuses_.end()? s_it->second : unknown_status;
                (*images)[image_id] = mirror_image_status_t {
                    image_name, mirror_image_info_t {
                    info.global_image_id,
                            static_cast < mirror_image_state_t >
                            (info.state), false}
                    ,           // XXX: To set "primary" right would require an additional call.
                static_cast < mirror_image_status_state_t > (s.state),
                        s.description, s.last_update.sec(), s.up};
            }
        ...
        template < typename I >
            ...
            int r =
                cls_client::mirror_image_status_get_summary(&io_ctx, &states_);
            if (r < 0 && r != -ENOENT) {
