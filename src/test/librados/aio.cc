...
class AioTestData {
  public:
    ...
    std::string init() {
        ...
        if (!err.empty()) {
            ostringstream oss;
            oss << "create_one_pool(" << m_pool_name << ") failed: error " <<
                err;
            return oss.str();
        }
...
TEST(LibRadosAio, TooBigPP)
{
    ...
    AioCompletion *aio_completion =
        test_data.m_cluster.aio_create_completion(nullptr, NULL, NULL);
    ASSERT_EQ(-E2BIG,
              test_data.m_ioctx.aio_write("foo", aio_completion, bl, UINT_MAX,
                                          0));
    ASSERT_EQ(-E2BIG,
              test_data.m_ioctx.aio_append("foo", aio_completion, bl,
                                           UINT_MAX));
...
TEST(LibRadosAio, PoolQuotaPP)
{
    ...
    ASSERT_EQ(0,
              test_data.m_cluster.
              mon_command("{\"prefix\": \"osd pool set-quota\", \"pool\": \"" +
                          p +
    ...
    for (n = 0; n < 1024; ++n) {
        ...
        ASSERT_EQ(0, ioctx.aio_operate("foo" + stringify(n), completion, &op,
...
TEST(LibRadosAio, SimpleWrite)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_write(test_data.m_ioctx, "foo", my_completion2, buf,
                              sizeof(buf), 0));
    {
        TestAlarm alarm;
...

    {
        ...
        AioCompletion *my_completion =
            test_data.m_cluster.aio_create_completion(nullptr, nullptr,
                                                      nullptr);
        ASSERT_EQ(0,
                  test_data.m_ioctx.aio_write("foo", my_completion, bl1,
                                              sizeof(buf), 0));
        {
            TestAlarm alarm;
            ASSERT_EQ(0, my_completion->wait_for_complete());
...
TEST(LibRadosAio, WaitForSafePP)
{
    ...
    ASSERT_EQ("", test_data.init());
    AioCompletion *my_completion =
        test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr);
    AioCompletion *my_completion_null = NULL;
    ASSERT_NE(my_completion, my_completion_null);
    char buf[128];
    memset(buf, 0xcc, sizeof(buf));
...
TEST(LibRadosAio, RoundTrip)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_read(test_data.m_ioctx, "foo", my_completion2, buf2,
                             sizeof(buf2), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
...
TEST(LibRadosAio, RoundTrip3)
{
    ...
    rados_write_op_set_alloc_hint2(op1, 0, 0,
                                   LIBRADOS_OP_FLAG_FADVISE_DONTNEED);
    ASSERT_EQ(0,
              rados_aio_write_op_operate(op1, test_data.m_ioctx, my_completion,
                                         "foo", NULL, 0));
    ...
    ASSERT_EQ(0,
              rados_aio_read_op_operate(op2, test_data.m_ioctx, my_completion2,
                                        "foo", 0));
...
TEST(LibRadosAio, RoundTripPP3)
{
    ...
    boost::scoped_ptr < AioCompletion >
        my_completion1(cluster.aio_create_completion(0, 0, 0));
    ObjectWriteOperation op;
    char buf[128];
    memset(buf, 0xcc, sizeof(buf));
    bufferlist bl;
    ...
    boost::scoped_ptr < AioCompletion >
        my_completion2(cluster.aio_create_completion(0, 0, 0));
    bl.clear();
...
TEST(LibRadosAio, RoundTripAppend)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_append(test_data.m_ioctx, "foo", my_completion2, buf2,
                               sizeof(buf2)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion3));
    ASSERT_EQ(0,
              rados_aio_read(test_data.m_ioctx, "foo", my_completion3, buf3,
                             sizeof(buf3), 0));
    {
        TestAlarm alarm;
...
TEST(LibRadosAio, RemoveTest)
{
    ...
    memset(buf2, 0, sizeof(buf2));
    ASSERT_EQ(-ENOENT,
              rados_read(test_data.m_ioctx, "foo", buf2, sizeof(buf2), 0));
    rados_aio_release(my_completion);
}
...
TEST(LibRadosAio, XattrsRoundTrip)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             nullptr, nullptr, &my_completion));
    ASSERT_EQ(0,
              rados_aio_getxattr(test_data.m_ioctx, "foo", my_completion, attr1,
                                 buf, sizeof(buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion));
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_setxattr(test_data.m_ioctx, "foo", my_completion2,
                                 attr1, attr1_buf, sizeof(attr1_buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
    }
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion3));
    ASSERT_EQ(0,
              rados_aio_getxattr(test_data.m_ioctx, "foo", my_completion3,
                                 attr1, buf, sizeof(buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion3));
    }
...
TEST(LibRadosAioPP, XattrsRoundTripPP)
{
    ...
    boost::scoped_ptr < AioCompletion > my_completion
        (test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_getxattr("foo", my_completion.get(), attr1,
                                             bl2));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    ...
    boost::scoped_ptr < AioCompletion > my_completion2
        (test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_setxattr("foo", my_completion2.get(), attr1,
                                             bl3));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    ...
    boost::scoped_ptr < AioCompletion > my_completion3
        (test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_getxattr("foo", my_completion3.get(), attr1,
                                             bl4));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
...
TEST(LibRadosAio, RmXattr)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             nullptr, nullptr, &my_completion));
    ASSERT_EQ(0,
              rados_aio_setxattr(test_data.m_ioctx, "foo", my_completion, attr1,
                                 attr1_buf, sizeof(attr1_buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion));
    }
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_rmxattr(test_data.m_ioctx, "foo", my_completion2,
                                attr1));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
    ...
    ASSERT_EQ(0,
              rados_write(test_data.m_ioctx, "foo_rmxattr", buf2, sizeof(buf2),
                          0));
    // asynx setxattr
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion4));
    ASSERT_EQ(0,
              rados_aio_setxattr(test_data.m_ioctx, "foo_rmxattr",
                                 my_completion4, attr2, attr2_buf,
                                 sizeof(attr2_buf)));
    {
        TestAlarm alarm;
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion5));
    ASSERT_EQ(0,
              rados_aio_rmxattr(test_data.m_ioctx, "foo_rmxattr",
                                my_completion5, attr2));
    {
        TestAlarm alarm;
...
TEST(LibRadosAioPP, RmXattrPP)
{
    ...
    boost::scoped_ptr < AioCompletion > my_completion
        (test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_setxattr("foo", my_completion.get(), attr1,
                                             bl2));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    ...
    boost::scoped_ptr < AioCompletion > my_completion2
        (test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_rmxattr("foo", my_completion2.get(),
                                            attr1));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    ...
    boost::scoped_ptr < AioCompletion > my_completion5
        (test_data.m_cluster.aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_rmxattr("foo_rmxattr", my_completion5.get(),
                                            attr2));
    {
        TestAlarm alarm;
...
TEST(LibRadosAio, XattrIter)
{
    ...
    ASSERT_EQ(0, rados_append(test_data.m_ioctx, "foo", buf, sizeof(buf)));
    ASSERT_EQ(0,
              rados_setxattr(test_data.m_ioctx, "foo", attr1, attr1_buf,
                             sizeof(attr1_buf)));
    ASSERT_EQ(0,
              rados_setxattr(test_data.m_ioctx, "foo", attr2, attr2_buf,
                             sizeof(attr2_buf)));
    ...
    ASSERT_EQ(0,
              rados_aio_getxattrs(test_data.m_ioctx, "foo", my_completion,
                                  &iter));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion));
...
TEST(LibRadosIoPP, XattrListPP)
{
    ...
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_getxattrs("foo", my_completion.get(),
                                              attrset));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
...
TEST(LibRadosAio, FlushAsync)
{
    ...
    ASSERT_EQ(0,
              rados_aio_create_completion(NULL, NULL, NULL, &flush_completion));
    char buf[128];
    memset(buf, 0xee, sizeof(buf));
    ASSERT_EQ(0, rados_aio_write(test_data.m_ioctx, "foo",
                                 my_completion, buf, sizeof(buf), 0));
...
TEST(LibRadosAio, RoundTripWriteFull)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_write_full(test_data.m_ioctx, "foo", my_completion2,
                                   buf2, sizeof(buf2)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
...
TEST(LibRadosAio, RoundTripWriteSame)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_writesame(test_data.m_ioctx, "foo", my_completion2, buf,
                                  sizeof(buf), ws_write_len, 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
    }
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion3));
    ASSERT_EQ(0,
              rados_aio_read(test_data.m_ioctx, "foo", my_completion3, full,
                             sizeof(full), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion3));
...
TEST(LibRadosAio, SimpleStat)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_stat(test_data.m_ioctx, "foo", my_completion2, &psize,
                             &pmtime));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
...
TEST(LibRadosAio, SimpleStatNS)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion3));
    ASSERT_EQ(0,
              rados_aio_stat(test_data.m_ioctx, "foo", my_completion3, &psize,
                             &pmtime));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion3));
...
TEST(LibRadosAio, StatRemove)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion4));
    ASSERT_EQ(0,
              rados_aio_stat(test_data.m_ioctx, "foo", my_completion4, &psize2,
                             &pmtime2));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion4));
...
TEST(LibRadosAio, OmapPP)
{
    ...
    {
        boost::scoped_ptr < AioCompletion >
            my_completion(cluster.aio_create_completion(0, 0, 0));
        ObjectWriteOperation op;
...

    {
        ...
        assertions["foo"] =
            pair < bufferlist, int >(val, CEPH_OSD_CMPXATTR_OP_EQ);
...
TEST(LibRadosAio, MultiWrite)
{
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion2));
    ASSERT_EQ(0,
              rados_aio_write(test_data.m_ioctx, "foo", my_completion2, buf2,
                              sizeof(buf2), sizeof(buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion2));
    }
...
TEST(LibRadosAio, MultiWritePP)
{
    ...
    ASSERT_EQ((int)(sizeof(buf) + sizeof(buf2)),
              my_completion3->get_return_value());
...
TEST(LibRadosAio, AioUnlock)
{
    ...
    ASSERT_EQ("", test_data.init());
    ASSERT_EQ(0,
              rados_lock_exclusive(test_data.m_ioctx, "foo", "TestLock",
                                   "Cookie", "", NULL, 0));
    rados_completion_t my_completion;
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             nullptr, nullptr, &my_completion));
    ASSERT_EQ(0,
              rados_aio_unlock(test_data.m_ioctx, "foo", "TestLock", "Cookie",
                               my_completion));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion));
...
TEST(LibRadosAio, AioUnlockPP)
{
    ...
    ASSERT_EQ("", test_data.init());
    ASSERT_EQ(0,
              test_data.m_ioctx.lock_exclusive("foo", "TestLock", "Cookie", "",
                                               NULL, 0));
    boost::scoped_ptr < AioCompletion >
        my_completion(test_data.m_cluster.
                      aio_create_completion(nullptr, nullptr, nullptr));
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_unlock("foo", "TestLock", "Cookie",
                                           my_completion.get()));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
...
class AioTestDataEC {
  public:
    ...
    std::string init() {
        ...
        if (!err.empty()) {
            ostringstream oss;
            oss << "create_one_ec_pool(" << m_pool_name << ") failed: error " <<
                err;
            return oss.str();
        }
        ret = rados_ioctx_create(m_cluster, m_pool_name.c_str(), &m_ioctx);
...
TEST(LibRadosAioEC, RoundTripAppend)
{
    ...
    int requires;
    ASSERT_EQ(0,
              rados_ioctx_pool_requires_alignment2(test_data.m_ioctx,
                                                   &requires));
    ASSERT_NE(0, requires);
    uint64_t alignment;
    ASSERT_EQ(0,
              rados_ioctx_pool_required_alignment2(test_data.m_ioctx,
                                                   &alignment));
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion3));
    ASSERT_EQ(0,
              rados_aio_append(test_data.m_ioctx, "foo", my_completion3, buf2,
                               hbsize));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion3));
    ...
    ASSERT_EQ(0, rados_aio_create_completion(nullptr,
                                             ...
                                             &my_completion4));
    ASSERT_EQ(0,
              rados_aio_read(test_data.m_ioctx, "foo", my_completion4, buf3,
                             bsize * 3, 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, rados_aio_wait_for_complete(my_completion4));
...
TEST(LibRadosAioEC, RoundTripWriteFullPP2)
{
    ...
    ObjectReadOperation op1;
    op1.read(0, sizeof(buf), &bl, NULL);
    op1.set_op_flags2(LIBRADOS_OP_FLAG_FADVISE_NOCACHE |
                      LIBRADOS_OP_FLAG_FADVISE_RANDOM);
    ioctx.aio_operate("test_obj", my_completion2.get(), &op1, 0);
    {
        TestAlarm alarm;
