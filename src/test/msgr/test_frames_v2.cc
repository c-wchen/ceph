                       ...
                       Tag &tag, segment_bls_t &segment_bls)
{
    ...
    return frame_asm.disassemble_segments(preamble_bl, segment_bls.data(),
                                          epilogue_bl);
}
...
protected:
    RoundTripTestBase()
        : m_tx_frame_asm(&m_tx_crypto, std::get < 1 > (GetParam()).is_rev1, true,
          ...
          m_rx_frame_asm(&m_rx_crypto, std::get < 1 > (GetParam()).is_rev1,
                         true, &m_rx_comp),
          m_header(make_bufferlist
                   (std::get < 0 > (GetParam()).header_len, 'H')),
          m_front(make_bufferlist
                  (std::get < 0 > (GetParam()).front_len, 'F')),
          m_middle(make_bufferlist
                   (std::get < 0 > (GetParam()).middle_len, 'M')),
          m_data(make_bufferlist(std::get < 0 > (GetParam()).data_len, 'D'))
    {
        ...
        if (m.is_secure) {
            ...
            g_ceph_context->random()->get_bytes(auth_meta.connection_secret.
                                                data(),
                                                auth_meta.connection_secret.
                                                size());
            m_tx_crypto =
                ceph::crypto::onwire::rxtx_t::
                create_handler_pair(g_ceph_context, auth_meta,
        ...
        if (m.is_compress) {
            ...
            m_tx_comp =
                ...
                create_handler_pair(g_ceph_context, comp_meta,
                                    /*min_compress_size= */ COMP_THRESHOLD
                                   );
            m_rx_comp =
                ceph::compression::onwire::rxtx_t::
                create_handler_pair(g_ceph_context, comp_meta,
    ...
    void check_frame_assembler(const FrameAssembler &frame_asm)
    {
        ...
        const auto &onwire_lens =
            rti.onwire_lens[m.is_rev1 << 1 | m.is_secure];

        EXPECT_COMPRESSED(m.is_compress,
                          rti.header_len + rti.front_len + rti.middle_len +
                          rti.data_len, frame_asm.get_frame_logical_len());
        ASSERT_EQ(rti.num_segments, frame_asm.get_num_segments());
        EXPECT_COMPRESSED(m.is_compress, onwire_lens[0],
                          frame_asm.get_preamble_onwire_len());
        for (size_t i = 0; i < rti.num_segments; i++) {
            EXPECT_COMPRESSED(m.is_compress, onwire_lens[i + 1],
        ...
        EXPECT_COMPRESSED(m.is_compress,
                          std::accumulate(std::begin(onwire_lens),
                                          std::end(onwire_lens),
                                          uint64_t(0)),
    ...
    {
        auto tx_frame =
            TestFrame::Encode(m_header, m_front, m_middle, m_data);
        auto onwire_bl = tx_frame.get_buffer(m_tx_frame_asm);
        check_frame_assembler(m_tx_frame_asm);
        EXPECT_EQ(m_tx_frame_asm.get_frame_onwire_len(),
                  onwire_bl.length());
...
INSTANTIATE_TEST_SUITE_P(RoundTripTests,
                         RoundTripTest, ::testing::Combine(::testing::
                                 ValuesIn
...
INSTANTIATE_TEST_SUITE_P(RoundTripPerfTests,
                         RoundTripPerfTest, ::testing::Combine(::testing::
                                 ValuesIn
                                 (round_trip_perf_instances), ::
                                 testing::
                                 ValuesIn
                                 (modes)));
