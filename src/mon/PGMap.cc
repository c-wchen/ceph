...
void PGMapDigest::encode(bufferlist & bl, uint64_t features) const const
{
...
void PGMapDigest::dump(Formatter * f) const const
{
...
void PGMapDigest::print_summary(Formatter * f, ostream * out) const const
{
    if (f)
    ...
    if (f) {
        ...
        *out << "    pools:   " << pg_pool_sum.size() << " pools, "
            << num_pg << " pgs\n";
        *out << "    objects: " << si_u_t(pg_sum.stats.sum.
                                          num_objects) << " objects, " <<
            byte_u_t(pg_sum.stats.sum.num_bytes) << "\n";
        *out << "    usage:   " << byte_u_t(osd_sum.
                                            kb_used << 10) << " used, " <<
            byte_u_t(osd_sum.kb_avail << 10) << " / " << byte_u_t(osd_sum.
                                                                  kb << 10) <<
            " avail\n";
        *out << "    pgs:     ";
    }
    ...
    if (!f) {
        ...
        for (multimap < int, int >::reverse_iterator p =
             state_by_count.rbegin(); p != state_by_count.rend(); ++p) {
            std::stringstream ss;
            ss << p->first;
...
void PGMapDigest::print_oneline_summary(Formatter * f, ostream * out) const const
{
    std::stringstream ss;
    ...
    if (pos_delta.stats.sum.num_rd || pos_delta.stats.sum.num_wr) {
        ...
        if (pos_delta.stats.sum.num_rd) {
            int64_t rd =
                (pos_delta.stats.sum.num_rd_kb << 10) / (double)stamp_delta;
            if (out)
        ...
        if (pos_delta.stats.sum.num_wr) {
            int64_t wr =
                (pos_delta.stats.sum.num_wr_kb << 10) / (double)stamp_delta;
            if (out)
        ...
        int64_t iops =
            (pos_delta.stats.sum.num_rd +
             pos_delta.stats.sum.num_wr) / (double)stamp_delta;
        if (out)
...
void PGMapDigest::recovery_summary(Formatter * f, list < string > *psl,
                                   const pool_stat_t & pool_sum) const const
{
    if (pool_sum.stats.sum.num_objects_degraded
        && pool_sum.stats.sum.num_object_copies > 0) {
        double pc =
            (double)pool_sum.stats.sum.num_objects_degraded /
            (double)pool_sum.stats.sum.num_object_copies * (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        if (f) {
            f->dump_unsigned("degraded_objects",
                             pool_sum.stats.sum.num_objects_degraded);
            f->dump_unsigned("degraded_total",
                             pool_sum.stats.sum.num_object_copies);
            ...
            ss << pool_sum.stats.sum.num_objects_degraded
                << "/" << pool_sum.stats.sum.
                num_object_copies << " objects degraded (" << b << "%)";
    ...
    if (pool_sum.stats.sum.num_objects_misplaced
        && pool_sum.stats.sum.num_object_copies > 0) {
        double pc =
            (double)pool_sum.stats.sum.num_objects_misplaced /
            (double)pool_sum.stats.sum.num_object_copies * (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        if (f) {
            f->dump_unsigned("misplaced_objects",
                             pool_sum.stats.sum.num_objects_misplaced);
            f->dump_unsigned("misplaced_total",
                             pool_sum.stats.sum.num_object_copies);
            ...
            ss << pool_sum.stats.sum.num_objects_misplaced
                << "/" << pool_sum.stats.sum.
                num_object_copies << " objects misplaced (" << b << "%)";
            psl->push_back(ss.str());
        }
    }
    if (pool_sum.stats.sum.num_objects_unfound
        && pool_sum.stats.sum.num_objects) {
        double pc =
            (double)pool_sum.stats.sum.num_objects_unfound /
            (double)pool_sum.stats.sum.num_objects * (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        if (f) {
            f->dump_unsigned("unfound_objects",
...
void PGMapDigest::recovery_rate_summary(Formatter * f, ostream * out,
                                        ...
                                        utime_t delta_stamp) const const
{
    // make non-negative; we can get negative values if osds send
    // uncommitted stats and then "go backward" or if they are just
    // buggy/wrong.
    ...
    if (pos_delta.stats.sum.num_objects_recovered ||
        ...
        int64_t objps =
            pos_delta.stats.sum.num_objects_recovered / (double)delta_stamp;
        int64_t bps =
            pos_delta.stats.sum.num_bytes_recovered / (double)delta_stamp;
        int64_t kps =
            pos_delta.stats.sum.num_keys_recovered / (double)delta_stamp;
        if (f) {
            f->dump_int("recovering_objects_per_sec", objps);
            f->dump_int("recovering_bytes_per_sec", bps);
            f->dump_int("recovering_keys_per_sec", kps);
            f->dump_int("num_objects_recovered",
                        pos_delta.stats.sum.num_objects_recovered);
            f->dump_int("num_bytes_recovered",
                        pos_delta.stats.sum.num_bytes_recovered);
            f->dump_int("num_keys_recovered",
                        pos_delta.stats.sum.num_keys_recovered);
        }
        else {
            *out << byte_u_t(bps) << "/s";
...
void PGMapDigest::overall_recovery_rate_summary(Formatter * f, ostream * out) const const
{
    recovery_rate_summary(f, out, pg_sum_delta, stamp_delta);
}

void PGMapDigest::overall_recovery_summary(Formatter * f, list < string > *psl) const const
{
    recovery_summary(f, psl, pg_sum);
}

void PGMapDigest::pool_recovery_rate_summary(Formatter * f, ostream * out,
                                             uint64_t poolid) const const
{
                                         ...
                                         utime_t delta_stamp) const const
{
    ...
    if (pos_delta.stats.sum.num_rd || pos_delta.stats.sum.num_wr) {
        if (pos_delta.stats.sum.num_rd) {
            int64_t rd =
                (pos_delta.stats.sum.num_rd_kb << 10) / (double)delta_stamp;
            if (f) {
        ...
        if (pos_delta.stats.sum.num_wr) {
            int64_t wr =
                (pos_delta.stats.sum.num_wr_kb << 10) / (double)delta_stamp;
            if (f) {
        ...
        if (f) {
            ...
            *out << si_u_t(iops_rd) << "op/s rd, " << si_u_t(iops_wr) <<
                "op/s wr";
        }
    }
}

void PGMapDigest::overall_client_io_rate_summary(Formatter * f, ostream * out) const const
{
                                        ...
                                        utime_t delta_stamp) const const
{
    ...
    if (pos_delta.stats.sum.num_flush) {
        int64_t flush =
            (pos_delta.stats.sum.num_flush_kb << 10) / (double)delta_stamp;
        if (f) {
    ...
    if (pos_delta.stats.sum.num_evict) {
        int64_t evict =
            (pos_delta.stats.sum.num_evict_kb << 10) / (double)delta_stamp;
        if (f) {
    ...
    if (pos_delta.stats.sum.num_flush_mode_low) {
        if (f) {
            f->dump_int("num_flush_mode_low",
                        pos_delta.stats.sum.num_flush_mode_low);
        }
        else {
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_flush_mode_low) << "PG(s) flushing";
            have_output = true;
        }
    }
    if (pos_delta.stats.sum.num_flush_mode_high) {
        if (f) {
            f->dump_int("num_flush_mode_high",
                        pos_delta.stats.sum.num_flush_mode_high);
        }
        else {
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_flush_mode_high) << "PG(s) flushing (high)";
            have_output = true;
        }
    }
    if (pos_delta.stats.sum.num_evict_mode_some) {
        if (f) {
            f->dump_int("num_evict_mode_some",
                        pos_delta.stats.sum.num_evict_mode_some);
        }
        else {
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_evict_mode_some) << "PG(s) evicting";
            have_output = true;
        }
    }
    if (pos_delta.stats.sum.num_evict_mode_full) {
        if (f) {
            f->dump_int("num_evict_mode_full",
                        pos_delta.stats.sum.num_evict_mode_full);
        }
        else {
            if (have_output)
                *out << ", ";
            *out << si_u_t(pos_delta.stats.sum.
                           num_evict_mode_full) << "PG(s) evicting (full)";
        }
    }
}

void PGMapDigest::overall_cache_io_rate_summary(Formatter * f, ostream * out) const const
{
...
void PGMapDigest::dump_pool_stats_full(const OSDMap & osd_map,
                                       stringstream * ss,
                                       Formatter * f, bool verbose) const const
{
    TextTable tbl;
    ...
    if (f) {
        ...
        if (verbose) {
            tbl.define_column("QUOTA OBJECTS", TextTable::LEFT,
                              TextTable::LEFT);
    ...
    for (auto p = osd_map.get_pools().begin();
        ...
        dump_object_stat_sum(tbl, f, stat.stats.sum, avail, raw_used_rate,
                             verbose, pool);
        if (f)
...
void PGMapDigest::dump_fs_stats(stringstream * ss, Formatter * f, bool verbose) const const
{
    if (f) {
        f->open_object_section("stats");
...
void PGMapDigest::dump_object_stat_sum(TextTable & tbl, Formatter * f,
                                       const object_stat_sum_t & sum,
                                       uint64_t avail, float raw_used_rate,
                                       bool verbose, const pg_pool_t * pool)
{
    ...
    if (f) {
        ...
        if (verbose) {
            ...
            f->dump_int("raw_bytes_used",
                        sum.num_bytes * raw_used_rate *
                        curr_object_copies_rate);
        }
    }
    else {
...
int64_t PGMapDigest::get_pool_free_space(const OSDMap & osd_map, int64_t poolid) const const
{
    const pg_pool_t *pool = osd_map.get_pg_pool(poolid);
    int ruleno = osd_map.crush->find_rule(pool->get_crush_rule(),
                                          pool->get_type(),
...
int64_t PGMap::get_rule_avail(const OSDMap & osdmap, int ruleno) constconst
{
    map < int, float >wm;
    ...
    for (auto p = wm.begin(); p != wm.end(); ++p) {
        ...
        if (osd_info != osd_stat.end()) {
            ...
            double avail =
                MAX(0.0, (double)osd_info->second.kb_avail - unusable);
            avail *= 1024.0;
            ...
            if (osdmap.is_up(p->first)) {
                ...
                dout(4) << "OSD " << p->
                    first << " is up, but has no stats" << dendl;
            }
        }
    }
    return min;
...
void PGMap::get_rules_avail(const OSDMap & osdmap,
                            std::map < int, int64_t > *avail_map) const const
{
    avail_map->clear();
...
void PGMap::Incremental::encode(bufferlist & bl, uint64_t features) const const
{
...
void PGMap::Incremental::dump(Formatter * f) const const
{
...
void PGMap::Incremental::generate_test_instances(list <
                                                 PGMap::Incremental * >&o)
{
...
void PGMap::apply_incremental(CephContext * cct, const Incremental & inc)
{
    ...
    if (!stamp.is_zero() && !pg_sum_old.stats.sum.is_zero()) {
        ...
        auto smooth_intervals =
            cct ? cct->_conf->get_val < uint64_t >
            ("mon_stat_smooth_intervals") : 1;
...
void PGMap::update_pg(pg_t pgid, bufferlist & bl)
{
    ...
    if (min_last_epoch_clean && (lec < min_last_epoch_clean ||  // we did
...
void PGMap::update_osd(int osd, bufferlist & bl)
{
    ...
    if (!p.end()) {
        ...
        if (e < min_last_epoch_clean ||
            (e > min_last_epoch_clean && old_lec == min_last_epoch_clean))
            min_last_epoch_clean = 0;
    }
    else {
...
void PGMap::stat_pg_add(const pg_t & pgid, const pg_stat_t & s, bool sameosds)
{
    ...
    if ((s.state & PG_STATE_CREATING) && s.parent_split_bits == 0) {
        ...
        if (s.acting_primary >= 0) {
            creating_pgs_by_osd_epoch[s.acting_primary][s.mapping_epoch].
                insert(pgid);
        }
    }
...
void PGMap::stat_pg_sub(const pg_t & pgid, const pg_stat_t & s, bool sameosds)
{
    ...
    if ((s.state & PG_STATE_CREATING) && s.parent_split_bits == 0) {
        ...
        if (s.acting_primary >= 0) {
            map < epoch_t, set < pg_t > >&r =
                creating_pgs_by_osd_epoch[s.acting_primary];
                           ...
                           bufferlist::iterator & blp)
{
    ...
    bool sameosds =
        s.acting == n.acting && s.up == n.up && s.blocked_by == n.blocked_by;
...
epoch_t PGMap::calc_min_last_epoch_clean() const const
{
    if (pg_stat.empty())
        return 0;
    ...
    for (auto i = osd_epochs.begin(); i != osd_epochs.end(); ++i) {
        if (i->second < min)
            min = i->second;
    }
    return min;
...
void PGMap::encode_digest(const OSDMap & osdmap,
                          bufferlist & bl, uint64_t features) const const
{
    get_rules_avail(osdmap, &avail_space_by_rule);
    PGMapDigest::encode(bl, features);
}

void PGMap::encode(bufferlist & bl, uint64_t features) const const
{
    if ((features & CEPH_FEATURE_MONENC) == 0) {
        __u8 v = 3;
        ::encode(v, bl);
...
void PGMap::dirty_all(Incremental & inc)
{
    ...
    for (auto p = osd_stat.begin(); p != osd_stat.end(); ++p) {
        ...
        inc.update_stat(p->first,
                        inc.get_osd_epochs().find(p->first)->second, p->second);
    }
}

void PGMap::dump(Formatter * f) const const
{
    dump_basic(f);
...
void PGMap::dump_basic(Formatter * f) const const
{
    f->dump_unsigned("version", version);
    f->dump_stream("stamp") << stamp;
...
void PGMap::dump_delta(Formatter * f) const const
{
...
void PGMap::dump_pg_stats(Formatter * f, bool brief) const const
{
...
void PGMap::dump_pool_stats(Formatter * f) const const
{
    f->open_array_section("pool_stats");
...
void PGMap::dump_osd_stats(Formatter * f) const const
{
    f->open_array_section("osd_stats");
...
void PGMap::dump_pg_stats_plain(ostream & ss,
                                const mempool::pgmap::unordered_map < pg_t,
                                pg_stat_t > &pg_stats, bool brief) const const
{
    TextTable tab;
    ...
    if (brief) {
        ...
        tab.define_column("MISSING_ON_PRIMARY", TextTable::LEFT,
                          TextTable::RIGHT);
        ...
        tab.define_column("DEEP_SCRUB_STAMP", TextTable::LEFT,
                          TextTable::RIGHT);
    ...
    for (auto i = pg_stats.begin(); i != pg_stats.end(); ++i) {
        ...
        if (brief) {
            tab << i->first << pg_state_string(st.state)
                << st.up
                << st.up_primary
            ...
            tab << i->first
                ...
                << st.log_size
                << st.ondisk_log_size << pg_state_string(st.state)
                << st.last_change << st.version << reported.str()
                << pg_vector_string(st.up)
                << st.up_primary << pg_vector_string(st.acting)
                << st.acting_primary
                << st.last_scrub
                << st.last_scrub_stamp
                << st.last_deep_scrub
                << st.last_deep_scrub_stamp
                << st.snaptrimq_len << TextTable::endrow;
        }
    }
...
void PGMap::dump(ostream & ss) const const
{
    dump_basic(ss);
...
void PGMap::dump_basic(ostream & ss) const const
{
...
void PGMap::dump_pg_stats(ostream & ss, bool brief) const const
{
    dump_pg_stats_plain(ss, pg_stat, brief);
}

void PGMap::dump_pool_stats(ostream & ss, bool header) const const
{
    TextTable tab;
...
void PGMap::dump_pg_sum_stats(ostream & ss, bool header) const const
{
    TextTable tab;
...
void PGMap::dump_osd_stats(ostream & ss) const const
{
    TextTable tab;
    ...
    for (auto p = osd_stat.begin(); p != osd_stat.end(); ++p) {
        tab << p->first << byte_u_t(p->second.kb_used << 10)
            << byte_u_t(p->second.kb_avail << 10)
            << byte_u_t(p->second.kb << 10)
            << p->second.hb_peers << get_num_pg_by_osd(p->first)
            << get_num_primary_pg_by_osd(p->first)
            << TextTable::endrow;
    }

    tab << "sum" << byte_u_t(osd_sum.kb_used << 10)
        << byte_u_t(osd_sum.kb_avail << 10)
...
void PGMap::dump_osd_sum_stats(ostream & ss) const const
{
    TextTable tab;
...
void PGMap::get_stuck_stats(int types, const utime_t cutoff,
                            mempool::pgmap::unordered_map < pg_t,
                            pg_stat_t > &stuck_pgs) const const
{
    assert(types != 0);
    for (auto i = pg_stat.begin(); i != pg_stat.end(); ++i) {
        ...
        if ((types & STUCK_UNDERSIZED)
            && (i->second.state & PG_STATE_UNDERSIZED)) {
...
bool PGMap::get_stuck_counts(const utime_t cutoff, map < string, int >&note) const const
{
    int inactive = 0;
    int unclean = 0;
    int degraded = 0;
...
void PGMap::dump_stuck(Formatter * f, int types, utime_t cutoff) const const
{
...
void PGMap::dump_stuck_plain(ostream & ss, int types, utime_t cutoff) const const
{
    mempool::pgmap::unordered_map < pg_t, pg_stat_t > stuck_pg_stats;
    get_stuck_stats(types, cutoff, stuck_pg_stats);
...
int PGMap::dump_stuck_pg_stats(stringstream & ds,
                               Formatter * f,
                               int threshold,
                               vector < string > &args) const const
{
    int stuck_types = 0;
...
void PGMap::dump_osd_perf_stats(Formatter * f) const const
{
...
void PGMap::print_osd_perf_stats(std::ostream * ss) const const
{
    TextTable tab;
...
void PGMap::dump_osd_blocked_by_stats(Formatter * f) const const
{
...
void PGMap::print_osd_blocked_by_stats(std::ostream * ss) const const
{
    TextTable tab;
    tab.define_column("osd", TextTable::LEFT, TextTable::RIGHT);
...
void PGMap::update_delta(CephContext * cct,
                         ...
                         mempool::pgmap::list < pair < pool_stat_t,
                         utime_t > >*delta_avg_list)
{
...
void PGMap::update_pool_deltas(CephContext * cct, const utime_t ts,
                               const mempool::pgmap::unordered_map < uint64_t,
                               pool_stat_t > &pg_pool_sum_old)
{
...
void PGMap::get_filtered_pg_stats(uint32_t state, int64_t poolid, int64_t osdid,
                                  bool primary, set < pg_t > &pgs) const const
{
    for (auto i = pg_stat.begin(); i != pg_stat.end(); ++i) {
...
void PGMap::dump_filtered_pg_stats(Formatter * f, set < pg_t > &pgs) const const
{
    f->open_array_section("pg_stats");
...
void PGMap::dump_filtered_pg_stats(ostream & ss, set < pg_t > &pgs) const const
{
    TextTable tab;
    ...
    for (auto i = pgs.begin(); i != pgs.end(); ++i) {
        ...
        tab << *i
            ...
            << st.stats.sum.num_objects_unfound
            << st.stats.sum.num_bytes
            << st.log_size << st.ondisk_log_size << pg_state_string(st.state)
            << st.last_change << st.version << reported.str()
            << st.up
            << st.up_primary
            << st.acting
            ...
            << st.last_scrub_stamp
            << st.last_deep_scrub
            << st.last_deep_scrub_stamp << TextTable::endrow;
    }
...
static void note_stuck_detail(int what,
                              mempool::pgmap::unordered_map < pg_t,
                              pg_stat_t > &stuck_pgs, int max_detail,
                              list < pair < health_status_t, string > >*detail)
{
    int n = 0;
    for (auto p = stuck_pgs.begin(); p != stuck_pgs.end(); ++p) {
        ostringstream ss;
        utime_t since;
        ...
        if (--max_detail == 0) {
            ostringstream ss;
            ss << (stuck_pgs.size() -
                   n) << " more pgs are also stuck " << whatname;
...
static pair < int, int >_warn_slow_request_histogram(CephContext * cct,
                                                     const pow2_hist_t & h,
                                                     string suffix,
                                                     list < pair <
                                                     health_status_t,
...
namespace {
    ...
    void print_unscrubbed_detailed(const std::pair < const pg_t,
                                   pg_stat_t > &pg_entry,
                                   list < pair < health_status_t,
                                   string > >*detail,
        ...
        if (how_scrubbed == scrubbed_or_deepscrubbed_t::SCRUBBED) {
            ss << "scrubbed, last_scrub_stamp " << pg_stat.last_scrub_stamp;
        }
    ...
    using pg_stat_map_t =
        const mempool::pgmap::unordered_map < pg_t, pg_stat_t >;
    ...
    void print_unscrubbed_pgs(pg_stat_map_t & pg_stats,
      ...
      for (const auto & pg_entry:pg_stats) {
            ...
            const int mon_warn_not_scrubbed =
                cct->_conf->mon_warn_not_scrubbed +
            ...
            bool not_deep_scrubbed =
                (time_since_lds >= mon_warn_not_deep_scrubbed
                 && cct->_conf->mon_warn_not_deep_scrubbed != 0);
...
void PGMap::get_health_checks(CephContext * cct,
                              const OSDMap & osdmap,
                              health_check_map_t * checks) const const
{
    utime_t now = ceph_clock_now();
    ...
    std::map < unsigned, PgStateResponse > state_to_response = {
        ...
        {PG_STATE_PEERING,
         {UNAVAILABLE,[](const pg_stat_t & p){return p.last_peered;
                                              }
                                              }
                                              }
                                              , {
                                              ...
                                              std::map < pg_consequence_t,
                                              PgCauses > detected;
                                              // Optimisation: trim down the number of checks to apply based on
                                              // the summary counters
  std::map < unsigned, PgStateResponse > possible_responses; for (const auto & i:num_pg_by_state)
                                              {
  ...
  for (const auto & j:                       state_to_response)
                                              ...
                                              const auto & found =
                                              num_pg_by_state.find(j.first);
                                              if (found == num_pg_by_state.end()
                                                  || found->second != num_pg) {
                                              possible_responses.insert(j);}
                                              }
                                              }
      ...
      for (const auto & i:                   pg_stat)
                                              {
          const auto & pg_id = i.first; const auto & pg_info = i.second; for (const auto & j:state_to_response)
                                              {
                                              const auto & pg_response_state =
                                              j.first;
                                              const auto & pg_response =
                                              j.second;
                                              // Apply the state test
                                              if (!
                                                  (bool
                                                   (pg_info.
                                                    state & pg_response_state)
                                                   != pg_response.invert)) {
                                              continue;}
                                              ...
                                              utime_t last_whatever =
                                              pg_response.stuck_since(pg_info);
                                              ...
                                              ss << ", acting " << pg_info.
                                              acting;
                                              if (pg_info.stats.sum.
                                                  num_objects_unfound) {
                                              ss << ", " << pg_info.stats.sum.
                                              num_objects_unfound <<
                                              " unfound";}
                                              }

                                              if (pg_info.
                                                  state & PG_STATE_INCOMPLETE) {
                                              const pg_pool_t * pi =
                                              osdmap.get_pg_pool(pg_id.pool());
                                              if (pi && pi->min_size > 1) {
                                              ss << " (reducing pool " <<
                                              osdmap.get_pool_name(pg_id.pool())
                                              << " min_size from " << (int)pi->
                                              min_size <<
                                              " may help; search ceph.com/docs for 'incomplete')";}
                                              }

                                              causes.pg_messages[pg_id] =
                                              ss.str();}
                                              }
                                              }
                                              else {
                                              dout(10) << __func__ <<
                                              " skipping loop over PGs: counters look OK"
                                              << dendl;}
  ...
  for (const auto & i:                       detected) {
                                              ...
                                              switch (i.first) {
case UNAVAILABLE:
health_code = "PG_AVAILABILITY"; sev = HEALTH_WARN; summary = "Reduced data availability: "; break; case DEGRADED:
health_code = "PG_DEGRADED"; summary = "Degraded data redundancy: "; sev = HEALTH_WARN; break; case DEGRADED_FULL:
health_code = "PG_DEGRADED_FULL"; summary = "Degraded data redundancy (low space): "; sev = HEALTH_ERR; break; case DAMAGED:
health_code = "PG_DAMAGED"; summary = "Possible data damage: "; sev = HEALTH_ERR; break; default:
                                              assert
                                              (false);}
                                              ...
                                              if (pg_sum.stats.sum.
                                                  ...
                                                  num_object_copies > 0) {
                                              double pc =
                                              (double)pg_sum.stats.sum.
                                              num_objects_degraded /
                                              (double)pg_sum.stats.sum.
                                              num_object_copies * (double)100.0;
                                              char b[20];
                                              snprintf(b, sizeof(b), "%.3lf",
                                                       pc); ostringstream ss;
                                              ss << pg_sum.stats.sum.
                                              num_objects_degraded << "/" <<
                                              pg_sum.stats.sum.
      ...
      std::vector < std::string > pg_msgs; for (const auto & j:i.second.states)
                                              {
                                              std::ostringstream msg;
                                              msg << j.second << (j.second >
                                                                  1 ? " pgs " :
                                                                  " pg ") <<
                                              state_name(j.first);
                                              pg_msgs.push_back(msg.str());}
                                              summary +=
                                              joinify(pg_msgs.begin(),
                                                      pg_msgs.end(),
                                                      std::string(", "));
                                              health_check_t * check =
                                              &checks->add(health_code, sev,
                                                           summary);
      ...
      list < string > detail; for (auto & pool:pools)
                                              {
                                              const string & pool_name =
                                              osdmap.get_pool_name(pool.first);
                                              auto it2 =
                                              pg_pool_sum.find(pool.first);
                                              ...
                                              {
      list < string > detail; unsigned num_pools = 0; for (auto & p:pools)
                                              {
                                              ...
                                              bool nearfull = false;
                                              const string & name =
                                              osdmap.get_pool_name(p.first);
                                              const pool_stat_t & st =
                                              get_pg_pool_sum_stat(p.first);
                                              uint64_t ratio =
                                              p.second.
                                              ...
                                              if (p.second.target_max_objects
                                                  && (uint64_t) (st.stats.sum.
                                                                 num_objects -
                                                                 st.stats.sum.
                                                                 num_objects_hit_set_archive)
                                                  >
                                                  p.second.target_max_objects *
                                                  (ratio / 1000000.0)) {
                                              ostringstream ss;
                                              ss << "cache pool '" << name <<
                                              "' with " << si_u_t(st.stats.sum.
                                                                  num_objects)
                                              ...
                                              if (p.second.target_max_bytes
                                                  ...
                                                  >
                                                  p.second.target_max_bytes *
                                                  (ratio / 1000000.0)) {
                                              ostringstream ss;
                                              ss << "cache pool '" << name <<
                                              "' with " << byte_u_t(st.stats.
                                                                    sum.
                                                                    num_bytes)
                                              << " at/near target max " <<
                                              byte_u_t(p.second.
                                              ...
                                              if (num_in && min_pg_per_osd > 0
                                                  && osdmap.get_pools().size() >
                                                  0) {
                                              auto per = sum_pg_up / num_in;
                                              if (per < min_pg_per_osd && per) {
                                              ostringstream ss;
                                              ss << "too few PGs per OSD (" <<
                                              per << " < min " << min_pg_per_osd
                                              << ")";
                                              checks->add("TOO_FEW_PGS",
                                                          HEALTH_WARN,
                                                          ss.str());}
                                              }
                                              ...
                                              ostringstream ss;
                                              ss << "too many PGs per OSD (" <<
                                              per << " > max " << max_pg_per_osd
                                              << ")";
                                              checks->add("TOO_MANY_PGS",
                                                          HEALTH_WARN,
                                                          ss.str());}
                                              }
                                              ...
                                              ss << pgp_detail.
                                              size() <<
                                              " pools have pg_num > pgp_num";
                                              auto & d =
                                              checks->add("SMALLER_PGP_NUM",
                                                          HEALTH_WARN,
                                                          ss.str());
                                              ...
                                              ss << many_detail.
                                              size() <<
                                              " pools have many more objects per pg than"
                                              << " average";
                                              auto & d =
                                              checks->add("MANY_OBJECTS_PER_PG",
                                                          HEALTH_WARN,
                                                          ss.str());
                                              ...
                                              // POOL_NEAR_FULL
                                              {
      float warn_threshold = (float)g_conf->get_val < int64_t > ("mon_pool_quota_warn_threshold") / 100; float crit_threshold = (float)g_conf->get_val < int64_t > ("mon_pool_quota_crit_threshold") / 100; list < string > full_detail, nearfull_detail; unsigned full_pools = 0, nearfull_pools = 0; for (auto it:pools)
                                              {
                                              auto it2 =
                                              pg_pool_sum.find(it.first);
                                              if (it2 == pg_pool_sum.end()) {
                                              continue;}
                                              const pool_stat_t * pstat =
                                              &it2->second;
                                              ...
                                              it.second;
                                              bool full = false, nearfull =
                                              false;
                                              if (pool.quota_max_objects > 0) {
                                              stringstream ss;
                                              if ((uint64_t) sum.num_objects >=
                                              ...
                                              " (max " << pool.
                                              quota_max_objects << ")";
                                              nearfull_detail.push_back(ss.
                                                                        str());
                                              nearfull = true;}
                                              }
                                              if (pool.quota_max_bytes > 0) {
                                              stringstream ss;
                                              if ((uint64_t) sum.num_bytes >=
      ...
      double pc = (double)pg_sum.stats.sum.num_objects_unfound / (double)pg_sum.stats.sum.num_objects * (double)100.0; char b[20]; snprintf(b, sizeof(b), "%.3lf", pc); ostringstream ss; ss << pg_sum.stats.sum.num_objects_unfound << "/" << pg_sum.stats.sum.num_objects << " objects unfound (" << b << "%)"; auto & d = checks->add("OBJECT_UNFOUND", HEALTH_WARN, ss.str()); for (auto & p:pg_stat)
                                              ...
                                              // REQUEST_STUCK
                                              if (cct->_conf->
                                                  mon_osd_warn_op_age > 0
                                                  && !osd_sum.op_queue_age_hist.
                                                  h.empty()
                                                  && osd_sum.op_queue_age_hist.
                                                  upper_bound() / 1000.0 >
                                                  cct->_conf->
                                                  mon_osd_warn_op_age) {
                                              list < string > warn_detail,
                                              error_detail;
                                              unsigned warn = 0, error = 0;
                                              float err_age =
          ...
          for (auto & p:                     osd_stat) {
                                              ...
                                              break; if (h.h[i]) {
                                              if (ub > err_age) {
                                              error_osd_by_max[ub].insert(p.
                                                                          first);}
                                              else {
                                              warn_osd_by_max[ub].insert(p.
                                                                         first);}
                                              break;}
                                              }
                                              }
                                              ...
                                              if (!warn_detail.empty()) {
          int left = max; set < int >implicated_osds; for (auto & p:warn_osd_by_max)
                                              {
                                              ostringstream ss;
                                              implicated_osds.insert(p.second.
                                                                     begin(),
                                                                     p.second.
                                                                     end());
                                              if (p.second.size() > 1) {
                                              ...
                                              implicated_osds;
                                              auto & d =
                                              checks->add("REQUEST_SLOW",
                                                          HEALTH_WARN,
                                                          ss.str());
                                              d.detail.swap(warn_detail);}
                                              if (!error_detail.empty()) {
          int left = max; set < int >implicated_osds; for (auto & p:error_osd_by_max)
                                              {
                                              ostringstream ss;
                                              implicated_osds.insert(p.second.
                                                                     begin(),
      ...
      list < string > detail, deep_detail; int detail_max = max, deep_detail_max = max; int detail_more = 0, deep_detail_more = 0; int detail_total = 0, deep_detail_total = 0; for (auto & p:pg_stat)
                                              {
                                              int64_t pnum = p.first.pool();
                                              auto pool =
                                              osdmap.get_pg_pool(pnum);
                                              if (!pool)
                                              continue;
                                              if (cct->_conf->
                                                  mon_warn_not_scrubbed) {
                                              ...
                                              utime_t cutoff = now;
                                              cutoff -= age;
                                              if (p.second.last_scrub_stamp <
                                                  cutoff) {
                                              if (detail_max > 0) {
                                              ostringstream ss;
                                              ss << "pg " << p.
                                              first << " not scrubbed since " <<
                                              ...
                                              ss << "pg " << p.
                                              first <<
                                              " not deep-scrubbed since " << p.
                                              second.last_deep_scrub_stamp;
                                              ...
                                              ostringstream ss;
                                              ss << deep_detail_total <<
                                              " pgs not deep-scrubbed in time";
                                              auto & d =
                                              checks->
                                              add("PG_NOT_DEEP_SCRUBBED",
                                                  HEALTH_WARN, ss.str());
      ...
      list < string > detail; for (auto & it:pools)
                                              {
                                              const pg_pool_t & pool =
                                              it.second;
                                              const string & pool_name =
                                              ...
                                              if (!pg_stat.empty()
                                                  ...
                                                  > 0) {
      uint32_t snapthreshold = cct->_conf->mon_osd_snap_trim_queue_warn_on; uint64_t snaptrimq_exceeded = 0; uint32_t longest_queue = 0; const pg_t * longest_q_pg = nullptr; list < string > detail; for (auto & i:pg_stat)
                                              {
                                              uint32_t current_len =
                                              i.second.snaptrimq_len;
                                              if (current_len >=
                                                  snapthreshold) {
                                              snaptrimq_exceeded++;
                                              if (longest_queue <=
                                              ...
                                              stringstream ss;
                                              ss << "snap trim queue for pg " <<
                                              i.first << " at " << current_len;
                                              detail.push_back(ss.str());
                                              continue;}
                                              if (detail.size() < max) {
                                              detail.
                                              ...
                                              {
                                              ostringstream ss;
                                              ss << "longest queue on pg " <<
                                              *longest_q_pg << " at " <<
                                              longest_queue;
                                              detail.push_back(ss.str());}

                                              stringstream ss;
                                              ss << "snap trim queue for " <<
                                              snaptrimq_exceeded << " pg(s) >= "
                                              << snapthreshold <<
                                              " (mon_osd_snap_trim_queue_warn_on)";
                                              auto & d =
                                              checks->
                                              add("PG_SLOW_SNAP_TRIMMING",
                                                  HEALTH_WARN, ss.str());
                                              detail.
                                              push_back
                                              ...
                                              p->second;
                                              if (p->
                                                  first & PG_STATE_BACKFILLING)
                                              note["backfilling"] += p->second;
                                              if (p->
                                                  first &
                                                  PG_STATE_BACKFILL_TOOFULL)
                                              note["backfill_toofull"] +=
                                              p->second;
                                              if (p->
                                                  first &
                                                  PG_STATE_RECOVERY_TOOFULL)
                                              note["recovery_toofull"] +=
                                              p->second;
                                              if (p->
                                                  first &
                                              ...
                                              pg_t, pg_stat_t > stuck_pgs;
                                              utime_t now(ceph_clock_now());
                                              utime_t cutoff =
                                              now - utime_t(g_conf->get_val <
                                                            int64_t >
                                                            ("mon_pg_stuck_threshold"),
                                                            0);
                                              ...
                                              if (note.find("stuck inactive") !=
                                                  note.end()) {
                                              get_stuck_stats(PGMap::
                                                              STUCK_INACTIVE,
                                                              cutoff,
                                                              stuck_pgs);
                                              note["stuck inactive"] =
                                              ...
                                              if (note.
                                                  find("stuck undersized") !=
                                                  note.end()) {
                                              get_stuck_stats(PGMap::
                                                              STUCK_UNDERSIZED,
                                                              cutoff,
                                                              stuck_pgs);
                                              note["stuck undersized"] =
                                              stuck_pgs.size();
                                              note_stuck_detail(PGMap::
                                                                STUCK_UNDERSIZED,
                                                                stuck_pgs,
                                              ...
                                              if (cct->_conf->
                                                  ...
                                                  mon_osd_warn_op_age) {
                                              auto sum =
                                              _warn_slow_request_histogram(cct,
                                                                           osd_sum.
                                                                           op_queue_age_hist,
                                                                           "",
                                                                           summary,
  ...
  for (auto it:                              pools) {
                                              ...
                                              status = HEALTH_WARN;}
                                              if (status != HEALTH_OK) {
                                              pair < health_status_t,
                                              string > s(status, ss.str());
                                              summary.push_back(s); if (detail)
                                              detail->push_back(s);}
                                              }

                                              if (pool.quota_max_bytes > 0) {
                                              health_status_t status =
                                              HEALTH_OK; stringstream ss;
                                              if ((uint64_t) sum.num_bytes >=
                                                  pool.quota_max_bytes) {
                                              }
                                              else
                                              ...
                                              print_unscrubbed_pgs(pg_stat,
                                                                   summary,
                                                                   detail,
                                                                   cct);}

                                              int process_pg_map_command(const
                                                                         string
                                                                         &
                                                                         orig_prefix,
                                                                         const
                                              ...
                                              cmd_putval(g_ceph_context, cmdmap,
                                                         "format",
                                                         string("json"));
                                              cmd_putval(g_ceph_context, cmdmap,
                                                         "dumpcontents", v);
                                              ...
                                              if (prefix == "pg ls-by-primary") {
                                              primary = true; prefix = "pg ls";}
                                              else
                                              if (prefix == "pg ls-by-osd") {
                                              prefix = "pg ls";}
                                              else
                                              if (prefix == "pg ls-by-pool") {
                                              prefix = "pg ls";
                                              string poolstr;
                                              ...
                                              *ss << "pool " << poolstr <<
                                              ...
                                              pg_map.print_oneline_summary(f,
                                                                           NULL);
                                              f->close_section(); f->flush(ds);}
                                              else {
                                              ds << pg_map;}
                                              odata->append(ds); return 0;}

                                              if (prefix == "pg getmap") {
                                              pg_map.encode(*odata);
                                              *ss << "got pgmap version " <<
                                              pg_map.version; return 0;}
                                              ...
                                              set < string > what;
                                              if (cmd_getval
                                                  (g_ceph_context, cmdmap,
                                                   "dumpcontents",
                                                   dumpcontents)) {
                                              copy(dumpcontents.begin(),
                                                   dumpcontents.end(),
                                                   inserter(what, what.end()));}
                                              if (what.empty())
                                              what.insert("all"); if (f) {
                                              if (what.count("all")) {
                                              f->open_object_section("pg_map");
                                              pg_map.dump(f);
                                              f->close_section();}
                                              else
                                              if (what.count("summary")
                                                  || what.count("sum")) {
                                              f->open_object_section("pg_map");
                                              pg_map.dump_basic(f);
                                              f->close_section();}
                                              ...
                                              *ss << "'" << state_str <<
                                              ...
                                              pg_map.
                                              get_filtered_pg_stats(state, pool,
                                                                    osd,
                                                                    primary,
                                                                    pgs);
                                              if (f && !pgs.empty()) {
                                              pg_map.dump_filtered_pg_stats(f,
                                                                            pgs);
                                              f->flush(*odata);}
                                              else
                                              if (!pgs.empty()) {
                                              pg_map.dump_filtered_pg_stats(ds,
                                                                            pgs);
                                              odata->append(ds);}
                                              return 0;}
                                              ...
                                              cmd_getval(g_ceph_context, cmdmap,
                                                         ...
                                                         ("mon_pg_stuck_threshold"));
                                              r =
                                              pg_map.dump_stuck_pg_stats(ds, f,
                                                                         (int)
                                                                         threshold,
                                                                         stuckop_vec);
                                              odata->append(ds); if (r < 0)
                                              * ss << "failed";
                                              else
                                              *ss << "ok"; return 0;}
                                              ...
                                              if (debugop ==
                                                  "unfound_objects_exist") {
bool unfound_objects_exist = false; for (const auto & p:pg_map.pg_stat)
                                              {
                                              if (p.second.stats.sum.
                                              ...
                                              if (debugop ==
                                                  "degraded_pgs_exist") {
bool degraded_pgs_exist = false; for (const auto & p:pg_map.pg_stat)
                                              {
                                              if (p.second.stats.sum.
                                                  num_objects_degraded > 0) {
                                              degraded_pgs_exist = true; break;}
                                              }
                                              ...
                                              *ss << "unrecognized pool '" <<
                                              pool_name << "'"; return -ENOENT;}
                                              one_pool = true;}

                                              stringstream rs; if (f)
                                              f->
...
for (auto & p:                               osdmap.get_pools()) {
                                              ...
                                              list < string > sl;
                                              stringstream tss;
                                              pg_map.pool_recovery_summary(f,
                                                                           &sl,
                                                                           poolid);
                                              if (!f && !sl.empty()) {
for (auto & p:                               sl)
...
stats_out:
                                              ...
                                              check_osd_map(const OSDMap::
                                                            ...
                                                            *last_osd_report,
                                                            ...
                                                            PGMap::Incremental *
                                                            pending_inc) {
  for (const auto & p:                       osd_inc.
                                                   new_weight)
                                              {
                                              if (p.second == CEPH_OSD_OUT) {
                                              dout(10) << __func__ << "  osd."
                                              << p.
                                              first << " went OUT" << dendl;
                                              auto j =
                                              pg_map->osd_epochs.find(p.first);
  ...
  for (const auto & p:                       osd_inc.
                                              ...
                                              // which
                                              need_check_down_pg_osds->insert(p.
                                                                              first);
                                              // clear out the last_osd_report for this OSD
                                              auto report =
                                              last_osd_report->find(p.first);
                                              if (report !=
                                                  last_osd_report->end()) {
                                              ...
                                              dout(20) << __func__ <<
                                              " clearing osd." << p.
                                              first << " request histogram" <<
                                              dendl;
                                              pending_inc->stat_osd_down_up(p.
                                                                            first,
                                                                            osd_inc.
                                                                            epoch,
                                                                            *pg_map);}
  ...
  for (auto & p:                             pgmap.osd_stat) {
                                              ...
                                              if (osdmap.is_out(p.first)) {
                                              // zero osd_stat
                                              if (p.second.kb != 0) {
                                              auto j =
                                              pgmap.osd_epochs.find(p.first);
  ...
  for (auto & p:                             pgmap.
          ...
          ldout(cct, 10) << __func__ << " pool " << p.first << " gone, removing pgs" << dendl; for (auto & q:pgmap.pg_stat)
                                              {
                                              if (q.first.pool() ==
                                                  (uint64_t) p.first) {
                                              pending_inc->pg_remove.insert(q.
                                                                            first);}
                                              }
                                              auto q =
                                              pending_inc->pg_stat_updates.
                                              begin();
                                              while (q !=
                                                     pending_inc->
                                                     pg_stat_updates.end()) {
  ...
  for (auto & p:                             osdmap.
                                              ...
                                              auto q =
                                              pgmap.num_pg_by_pool.find(poolid);
                                              unsigned my_pg_num = 0;
                                              if (q !=
                                                  pgmap.num_pg_by_pool.end())
                                              my_pg_num = q->second;
                                              unsigned pg_num = pi.get_pg_num();
                                              if (my_pg_num != pg_num) {
                                              ldout(cct,
                                                    10) << __func__ << " pool "
                                              << poolid << " pg_num " << pg_num
                                              << " != my pg_num " << my_pg_num
                                              << dendl;
                                              for (unsigned ps = my_pg_num;
                                                   ps < pg_num; ++ps) {
                                              pg_t pgid(ps, poolid);
                                              if (pending_inc->pg_stat_updates.
                                                  count(pgid) == 0) {
                                              ldout(cct,
                                                    20) << __func__ <<
                                              " adding " << pgid << dendl;
                                              ...
                                              stats.last_unstale =
                                              osdmap.get_modified();
                                              stats.last_undegraded =
                                              osdmap.get_modified();
                                              stats.last_fullsized =
                                              osdmap.get_modified();
                                              stats.last_scrub_stamp =
                                              osdmap.get_modified();
                                              stats.last_deep_scrub_stamp =
                                              osdmap.get_modified();
                                              ...
                                              ps.last_scrub_stamp;
                                              stats.last_deep_scrub_stamp =
                                              ps.last_deep_scrub_stamp;
                                              stats.last_clean_scrub_stamp =
                                              ps.last_clean_scrub_stamp;}
                                              else {
                                              utime_t now =
                                              osd_map.get_modified();
                                              ...
                                              osd_map.pg_to_up_acting_osds(pgid,
                                                                           &stats.
                                                                           up,
                                                                           &stats.
                                                                           up_primary,
                                              ...
                                              acting_primary << " acting " <<
                                              stats.
                                              acting << " parent " << parent <<
                                              " by " << split_bits << " bits" <<
                                              dendl;}
                                              }

                                              void PGMapUpdater::
                                              register_new_pgs(const OSDMap &
                                                               osd_map,
                                                               const PGMap &
                                                               pg_map,
                                                               PGMap::
                                                               Incremental *
                                                               pending_inc) {
  epoch_t epoch = osd_map.get_epoch(); dout(10) << __func__ << " checking pg pools for osdmap epoch " << epoch << ", last_pg_scan " << pg_map.last_pg_scan << dendl; int created = 0; const auto & pools = osd_map.get_pools(); for (const auto & p:pools)
                                              {
                                              int64_t poolid = p.first;
                                              const pg_pool_t & pool = p.second;
                                              int ruleno =
                                              osd_map.crush->find_rule(pool.
                                                                       get_crush_rule
                                                                       (),
                                                                       pool.
                                                                       get_type
                                              ...
                                              if (ruleno < 0
                                                  ...
                                                  rule_exists(ruleno))
                                              continue;
                                              if (pool.get_last_change() <=
                                                  pg_map.last_pg_scan
                                                  || pool.get_last_change() <=
                                                  pending_inc->pg_scan) {
                                              dout(10) << " no change in pool "
                                              << poolid << " " << pool << dendl;
                                              continue;}
                                              ...
                                              dout(20) <<
                                              "register_new_pgs  have " << pgid
                                              << dendl; continue;}
                                              created++;
                                              register_pg(osd_map, pgid,
                                                          pool.
                                                          get_last_change(),
                                                          new_pool, pg_map,
                                                          pending_inc);}
  ...
  int removed = 0; for (const auto & p:      pg_map.creating_pgs)
                                              {
                                              if (p.preferred() >= 0) {
                                              dout(20) <<
                                              " removing creating_pg " << p <<
                                              " because it is localized and obsolete"
                                              << dendl;
                                              ...
                                              dout(20) <<
                                              " removing creating_pg " << p <<
                                              " because containing pool deleted"
                                              << dendl;
                                              pending_inc->pg_remove.insert(p);
                                              ++removed;}
                                              }
                                              ...
                                              // deleted pools?
  for (const auto & p:                       pg_map.
                                                   pg_stat)
                                              {
                                              if (!osd_map.
                                                  have_pg_pool(p.first.
                                                               pool())) {
                                              dout(20) << " removing pg_stat "
                                              << p.
                                              first << " because " <<
                                              "containing pool deleted" <<
                                              dendl;
                                              pending_inc->pg_remove.insert(p.
                                                                            first);
                                              ++removed;}
                                              ...
                                              dout(10) <<
                                              "register_new_pgs registered " <<
                                              created << " new pgs, removed " <<
                                              removed << " uncreated pgs" <<
                                              dendl;}

                                              void PGMapUpdater::
                                              update_creating_pgs(const OSDMap &
                                                                  osd_map,
                                                                  const PGMap &
                                                                  pg_map,
                                                                  PGMap::
                                                                  ...
                                                                  pending_inc) {
                                              dout(10) << __func__ << " to " <<
                                              pg_map.creating_pgs.size()
                                              << " pgs, osdmap epoch " <<
                                              osd_map.get_epoch()
                                              << dendl; unsigned changed = 0;
                                              for (auto p =
                                                   pg_map.creating_pgs.begin();
                                                   p !=
                                                   pg_map.creating_pgs.end();
                                                   ++p) {
                                              pg_t pgid = *p; pg_t on = pgid;
                                              auto q =
                                              ...
                                              osd_map.pg_to_up_acting_osds(on,
                                                                           &up,
                                                                           &up_primary,
                                                                           &acting,
                                                                           &acting_primary);
                                              ...
                                              * ns = *s;
                                              ...
                                              if (acting_primary !=
                                                  ns->acting_primary)
                                              ns->mapping_epoch =
                                              osd_map.get_epoch(); ns->up = up;
                                              ns->up_primary = up_primary;
                                              ns->acting = acting;
                                              ns->acting_primary =
                                              acting_primary; ++changed;}
                                              else {
                                              dout(20) << __func__ << "  " <<
                                              pgid <<
                                              " has pending update from newer"
                                              << " epoch " << ns->
                                              reported_epoch << dendl;}
                                              }
                                              }
                                              if (changed) {
                                              dout(10) << __func__ << " " <<
                                              changed << " pgs changed primary"
                                              << dendl;}
                                              }

                                              static void
                                              _try_mark_pg_stale(const OSDMap &
                                                                 osdmap,
                                                                 pg_t pgid,
                                                                 const pg_stat_t
                                                                 & cur,
                                              ...
                                              *newstat = cur;}
                                              dout(10) << __func__ <<
                                              " marking pg " << pgid <<
                                              " stale (acting_primary " <<
                                              newstat->
                                              acting_primary << ")" << dendl;
                                              newstat->state |= PG_STATE_STALE;
                                              newstat->last_unstale =
                                              ceph_clock_now();}
                                              }

                                              void PGMapUpdater::
                                              check_down_pgs(const OSDMap &
                                                             osdmap,
                                                             const PGMap &
                                                             pg_map,
                                                             bool check_all,
      ...
      for (auto osd:                         need_check_down_pg_osds)
                                              ...
                                              auto p =
                                              pg_map.pg_by_osd.find(osd);
                                              if (p == pg_map.pg_by_osd.end()) {
                                              continue;}
              for (auto pgid:                p->second)
                                              {
                                              const pg_stat_t & stat =
                                              pg_map.pg_stat.at(pgid);
                                              assert(stat.acting_primary ==
                                                     osd);
                                              _try_mark_pg_stale(osdmap, pgid,
                                                                 stat,
                                                                 pending_inc);}
                                              }
                                              }
                                              ...
                                              *ss <<
                                              ...
                                              // average_util
                                              double average_util; if (by_pg) {
                                              // by pg mapping
                                              double weight_sum = 0.0;  // sum up the crush weights
      unsigned num_pg_copies = 0; int num_osds = 0; for (const auto & pg:pgm.pg_stat)
                                              {
                                              if (pools
                                                  && pools->count(pg.first.
                                                                  pool()) == 0)
          continue; for (const auto acting:  pg.second.acting)
                                              {
                                              ...
                                              *ss <<
                                              ...
                                              // by osd utilization
                                              int num_osd =
                                              MAX(1, pgm.osd_stat.size());
                                              if ((uint64_t) pgm.osd_sum.kb *
                                                  1024 / num_osd <
                                                  g_conf->
                                              ...
                                              *ss <<
                                              "Refusing to reweight: we only have "
                                              << pgm.osd_sum.
                                              kb << " kb across all osds!\n";
                                              return -EDOM;}
                                              if ((uint64_t) pgm.osd_sum.
                                                  kb_used * 1024 / num_osd <
                                                  g_conf->
                                                  mon_reweight_min_bytes_per_osd)
                                              {
                                              *ss <<
                                              ...
                                              average_util =
                                              (double)pgm.osd_sum.kb_used /
                                              (double)pgm.osd_sum.kb;}

                                              // adjust down only if we are above the threshold
                                              const double overload_util =
                                              average_util * (double)oload /
                                              100.0;
                                              // but aggressively adjust weights up whenever possible.
                                              const double underload_util =
                                              average_util;
                                              const unsigned max_change =
                                              (unsigned)(max_changef *
                                                         (double)0x10000);
                                              ostringstream oss; if (f) {
                                              f->
                                              open_object_section
                                              ("reweight_by_utilization");
                                              f->dump_int("overload_min",
                                                          oload);
                                              f->dump_float("max_change",
                                                            max_changef);
                                              f->dump_int("max_change_osds",
                                                          max_osds);
                                              f->
                                              dump_float("average_utilization",
                                                         average_util);
                                              f->
                                              dump_float("overload_utilization",
                                                         overload_util);}
                                              else {
                                              oss << "oload " << oload << "\n";
                                              oss << "max_change " <<
                                              ...
                                              max_osds << "\n";
                                              oss.precision(4);
                                              oss << "average_utilization " <<
                                              std::
                                              fixed << average_util << "\n";
                                              oss << "overload_utilization " <<
                                              overload_util << "\n";}
                                              int num_changed = 0;
                                              // precompute util for each OSD
  std::vector < std::pair < int, float > >util_by_osd; for (const auto & p:pgm.osd_stat)
                                              {
  ...
  f->open_array_section("reweights"); for (const auto & p:util_by_osd)
                                              {
                                              unsigned weight =
                                              osdmap.get_weight(p.first);
                                              if (weight == 0) {
