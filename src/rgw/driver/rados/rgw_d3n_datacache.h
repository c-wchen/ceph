...
struct D3nCacheAioWriteRequest {
    ...
    {
    } int d3n_libaio_prepare_write_op(bufferlist &bl, unsigned int len,
                                      std::string oid,
...
public:
    ...
    int d3n_libaio_create_write_request(bufferlist &bl, unsigned int len,
                                        std::string oid);
...
template < typename T >
int D3nRGWDataCache <
T >::get_obj_iterate_cb(const DoutPrefixProvider *dpp,
                        const rgw_raw_obj &read_obj, off_t obj_ofs,
                        off_t read_ofs, off_t len, bool is_head_obj,
                        RGWObjState *astate, void *arg)
{
    lsubdout(g_ceph_context, rgw_datacache,
             30) << "D3nDataCache::" << __func__ << "(): is head object : " <<
                 is_head_obj << dendl;
    ...
    if (is_head_obj) {
        ...
        ldpp_dout(dpp,
                  20) << "D3nDataCache::" << __func__ << "(): oid=" << read_obj.
        ...
        auto completed =
            d->aio->get(obj, rgw::Aio::librados_op(std::move(op), d->yield),
                        cost, id);
        return d->flush(std::move(completed));
    } else {
        ldpp_dout(dpp,
                  20) << "D3nDataCache::" << __func__ << "(): oid=" << read_obj.
                      oid << ", is_head_obj=" << is_head_obj << ", obj-ofs=" << obj_ofs <<
        ...
        if (r < 0) {
            lsubdout(g_ceph_context, rgw,
                     0) <<
                        "D3nDataCache: Error: failed to open rados context for " <<
                        read_obj << ", r=" << r << dendl;
            return r;
        }

        const bool is_compressed =
            (astate->attrset.find(RGW_ATTR_COMPRESSION) !=
             astate->attrset.end());
        const bool is_encrypted =
            (astate->attrset.find(RGW_ATTR_CRYPT_MODE) !=
             astate->attrset.end());
        if (read_ofs != 0 || astate->size != astate->accounted_size
            ...
            lsubdout(g_ceph_context, rgw,
                     5) << "D3nDataCache: " << __func__ <<
                        ...
                        oid << ", read_ofs!=0 = " << read_ofs << ", size=" << astate->
                        size << " != accounted_size=" << astate->
                        accounted_size << ", is_compressed=" << is_compressed <<
        ...
        if (d->rgwrados->d3n_data_cache->get(oid, len)) {
            ...
            ldpp_dout(dpp,
                      20) << "D3nDataCache: " << __func__ <<
                          ...
                          oid << ", obj-ofs=" << obj_ofs << ", read_ofs=" << read_ofs <<
                          ", len=" << len << dendl;
            auto completed =
                d->aio->get(obj,
                            rgw::Aio::d3n_cache_op(dpp, d->yield, read_ofs, len,
                                                   d->rgwrados->d3n_data_cache->
                                                   cache_location), cost, id);
            r = d->flush(std::move(completed));
            if (r < 0) {
                lsubdout(g_ceph_context, rgw,
