...
class LCExpiration_S3:public LCExpiration, public XMLObj {
  ...
  public:
    ...
    void to_xml(ostream & out) {
        out << "<Expiration>";
        if (dm_expiration) {
            out << "<ExpiredObjectDeleteMarker>" << "true" <<
...
class LCMPExpiration_S3:public LCExpiration, public XMLObj {
  public:
    ...
    void to_xml(ostream & out) {
        out << "<AbortIncompleteMultipartUpload>" << "<DaysAfterInitiation>" <<
            days << "</DaysAfterInitiation>" <<
  ...
  public:
    ...
    void dump_xml(Formatter * f) const {
        ...
        if (!expiration.empty() || dm_expiration) {
            LCExpiration_S3 expir(expiration.get_days_str(),
                                  expiration.get_date(), dm_expiration);
            expir.dump_xml(f);
        }
        if (!noncur_expiration.empty()) {
            const LCNoncurExpiration_S3 & noncur_expir =
                static_cast <
                const LCNoncurExpiration_S3 & >(noncur_expiration);
...
class RGWLifecycleConfiguration_S3:public RGWLifecycleConfiguration,
    public XMLObj {
  public:
    RGWLifecycleConfiguration_S3(CephContext *
                                 _cct):RGWLifecycleConfiguration(_cct) {
