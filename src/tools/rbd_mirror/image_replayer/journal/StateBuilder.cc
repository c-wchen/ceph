...
{
    return (remote_client_state ==
            cls::journal::CLIENT_STATE_DISCONNECTED);
} template < typename I > bool StateBuilder <
I >::is_linked_impl() const
{
    ceph_assert(!this->remote_mirror_uuid.empty());
    return (local_primary_mirror_uuid ==
...
I >::create_sync_point_handler()
{
    ...
    this->m_sync_point_handler =
        SyncPointHandler < I >::create(this);
...
{
    return Replayer < I >::create(threads, local_mirror_uuid,
                                  this, replayer_listener);
}

template < typename I >
void StateBuilder <
I >::shut_down_remote_journaler(Context *on_finish)
{
    ...
    auto ctx = new LambdaContext([this, on_finish](int r) {
        handle_shut_down_remote_journaler
        (r, on_finish);
    });
...
template < typename I >
void StateBuilder <
I >::handle_shut_down_remote_journaler(int r,
                                       Context *
                                       on_finish)
{
    dout(10) << "r=" << r << dendl;
    ...
    if (r < 0) {
        derr << "failed to shut down remote journaler: " <<
             cpp_strerror(r)
             << dendl;
    }
