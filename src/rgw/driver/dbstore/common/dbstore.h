...
struct DBOpUserPrepareInfo {
    ...
    static constexpr const char *access_keys_secret =
        ":access_keys_secret";
...
struct DBOpBucketPrepareInfo {
    ...
    static constexpr const char *swift_ver_location =
        ":swift_ver_location";
...
struct DBOpObjectPrepareInfo {
    ...
    static constexpr const char *owner_display_name =
        ":owner_display_name";
    ...
    static constexpr const char *index_hash_source =
        ":index_hash_source";
    ...
    static constexpr const char *manifest_part_objs =
        ":manifest_part_objs";
    static constexpr const char *manifest_part_rules =
        ":manifest_part_rules";
...
struct DBOpObjectDataPrepareInfo {
    ...
    static constexpr const char *size = ":size";
    static constexpr const char *multipart_part_str =
        ":multipart_part_str";
};
...
public:
    ...
    std::shared_ptr < class ListVersionedObjectsOp >
    ...
    std::shared_ptr < class DeleteStaleObjectDataOp >
        DeleteStaleObjectData;

    virtual int InitializeObjectOps(std::string db_name,
...
private:
    ...
    static constexpr std::string_view DropQ =
...
public:
                                         ...
                                         const DBOpParams *params)
    {
        ...
        ceph_abort_msgf("incorrect table type %.*s", type.size(),
                        type.data());
    }
    ...
    virtual int Prepare(const DoutPrefixProvider *dpp,
                        DBOpParams *params)
    {
        return 0;
    }
    virtual int Bind(const DoutPrefixProvider *dpp,
                     DBOpParams *params)
    {
        return 0;
    }
    virtual int Execute(const DoutPrefixProvider *dpp,
                        DBOpParams *params)
    {
        return 0;
    }
...
public:
    ...
    } static std::string Schema(DBOpPrepareParams &params)
    {
        return fmt::format(Query, params.user_table,
                           ...
                           params.op.user.tenant, params.op.user.ns,
                           ...
                           params.op.user.admin, params.op.user.system,
...
public:
    ...
    } static std::string Schema(DBOpPrepareParams &params)
    {
        return fmt::format(Query,
                           ...
                           params.op.bucket.bucket_name,
                           params.op.obj.acls, params.op.obj.index_ver,
                           params.op.obj.tag, params.op.obj.flags,
                           ...
                           params.op.obj.mtime, params.op.obj.epoch,
    ...
    } static std::string Schema(DBOpPrepareParams &params)
    {
        ...
        if (params.op.query_str == "meta") {
            return fmt::format(MetaQuery,
                               params.object_table,
                               params.op.obj.obj_ns, params.op.obj.acls,
...
public:
    ...
    } static std::string Schema(DBOpPrepareParams &params)
    {
        return fmt::format(Query,
                           ...
                           params.op.obj_data.size, params.op.obj.mtime,
...
public:
    DB(std::string db_name, CephContext *_cct): db_name(db_name),
        ...
        cct(_cct), dp(_cct, ceph_subsys_rgw, "rgw DBStore backend: ")
    {
    }
    /*  DB() {} */ DB(CephContext *_cct): db_name("default_db"),
        user_table(db_name + "_user_table"),
        bucket_table(db_name + "_bucket_table"),
        quota_table(db_name + "_quota_table"),
        lc_head_table(db_name + "_lc_head_table"),
    ...
    const std::string getUserTable()
    {
        return user_table;
    }
    const std::string getBucketTable()
    {
        return bucket_table;
    }
    const std::string getQuotaTable()
    {
        return quota_table;
    }
    const std::string getLCHeadTable()
    {
        return lc_head_table;
    }
    const std::string getLCEntryTable()
    {
    ...
    const DoutPrefixProvider *get_def_dpp()
    {
        return &dp;
    }
    ...
    int InitializeParams(const DoutPrefixProvider *dpp,
                         DBOpParams *params);
    int ProcessOp(const DoutPrefixProvider *dpp, std::string_view Op,
                  DBOpParams *params);
    std::shared_ptr < class DBOp > getDBOp(const DoutPrefixProvider *dpp, std::string_view Op,
                                               const DBOpParams *params);
    int objectmapInsert(const DoutPrefixProvider *dpp,
                        std::string bucket, class ObjectOp *ptr);
    int objectmapDelete(const DoutPrefixProvider *dpp,
                        std::string bucket);

    virtual uint64_t get_blob_limit()
    {
        return 0;
    };
    ...
    virtual int closeDB(const DoutPrefixProvider *dpp)
    {
        return 0;
    }
    virtual int createTables(const DoutPrefixProvider *dpp)
    {
        return 0;
    }
    ...
    virtual int ListAllBuckets(const DoutPrefixProvider *dpp,
                               DBOpParams *params) = 0;
    virtual int ListAllUsers(const DoutPrefixProvider *dpp,
                             DBOpParams *params) = 0;
    virtual int ListAllObjects(const DoutPrefixProvider *dpp,
                               DBOpParams *params) = 0;
    ...
    int store_user(const DoutPrefixProvider *dpp, RGWUserInfo &uinfo,
                   bool exclusive, std::map < std::string,
                   bufferlist > *pattrs,
                   RGWObjVersionTracker *pobjv_tracker,
                   RGWUserInfo *pold_info);
    int remove_user(const DoutPrefixProvider *dpp, RGWUserInfo &uinfo,
                    RGWObjVersionTracker *pobjv_tracker);
    int get_bucket_info(const DoutPrefixProvider *dpp,
                        const std::string &query_str,
    ...
    int create_bucket(const DoutPrefixProvider *dpp,
                      ...
                      RGWBucketInfo &info, obj_version *pobjv,
                      obj_version *pep_objv, real_time creation_time,
                      rgw_bucket *pmaster_bucket,
                      uint32_t *pmaster_num_shards, optional_yield y,
                      bool exclusive);
    ...
    int list_buckets(const DoutPrefixProvider *dpp,
                     ...
                     bool need_stats, RGWUserBuckets *buckets,
                     bool *is_truncated);
    int update_bucket(const DoutPrefixProvider *dpp,
                      const std::string &query_str,
    ...
    static constexpr std::string_view raw_obj_oid =
        "{0}_{1}_{2}_{3}_{4}";

    std::string to_oid(std::string_view bucket,
                       std::string_view obj_name,
                       std::string_view obj_instance,
                       std::string_view obj_id, std::string_view mp_str,
                       uint64_t partnum)
    {
        return fmt::format(raw_obj_oid, bucket, obj_name, obj_instance,
                           obj_id, mp_str, partnum);
    }
    int from_oid(const std::string &oid, std::string &bucket,
                 std::string &obj_name, std::string &obj_id,
                 std::string &obj_instance, std::string &mp_str,
                 uint64_t &partnum)
    {
    ...
    struct raw_obj {
        ...
        {
            db = _db;
        } raw_obj(DB *_db, std::string &_bname,
                  std::string &_obj_name, std::string &_obj_instance,
                  std::string &_obj_ns, std::string &_obj_id,
                  std::string _mp_part_str, int _part_num)
        {
            db = _db;
            bucket_name = _bname;
            obj_name = _obj_name;
        ...
        raw_obj(DB *_db, std::string &oid)
        {
            ...
            db = _db;
            r = db->from_oid(oid, bucket_name, obj_name, obj_instance,
                             obj_id, multipart_part_str, part_num);
            if (r < 0) {
        ...
        int InitializeParamsfromRawObj(const DoutPrefixProvider *dpp,
                                       DBOpParams *params);

        int read(const DoutPrefixProvider *dpp, int64_t ofs,
                 uint64_t end, bufferlist &bl);
        int write(const DoutPrefixProvider *dpp, int64_t ofs,
                  int64_t write_ofs, uint64_t len, bufferlist &bl);
    };
    ...
    public:
        GC(const DoutPrefixProvider *_dpp, DB *_db): dpp(_dpp),
            db(_db)
        {
        } void *entry() override;
    ...
    public:
        Bucket(DB *_store,
               const RGWBucketInfo &_binfo): store(_store),
            bucket_info(_binfo)
        {
        } DB *get_store()
        {
        ...
        protected:
            ...
            static constexpr int64_t bucket_list_objects_absolute_max =
                25000;
    ...
    public:
        Object(DB *_store, const RGWBucketInfo &_bucket_info,
               const rgw_obj &_obj): store(_store),
            bucket_info(_bucket_info), obj(_obj),
            versioning_disabled(false), bs_initialized(false)
        {
        } Object(DB *_store, const RGWBucketInfo &_bucket_info,
                 const rgw_obj &_obj,
                 const std::string &_obj_id): store(_store),
            bucket_info(_bucket_info), obj(_obj), obj_id(_obj_id)
        {
        }
        ...
        struct Read {
            ...
            struct ConditionParams {
                ...
                ConditionParams():
                    mod_ptr(NULL), unmod_ptr(NULL),
                    high_precision_time(false), mod_zone_id(0),
                    mod_pg_ver(0), if_match(NULL), if_nomatch(NULL)
                {
                }
            } conds;
            ...
            struct Params {
                ...
                Params(): lastmod(nullptr), obj_size(nullptr),
                    attrs(nullptr), target_obj(nullptr)
                {
                }
            } params;
            ...
            int read(int64_t ofs, int64_t end, bufferlist &bl,
                     const DoutPrefixProvider *dpp);
            int iterate(const DoutPrefixProvider *dpp, int64_t ofs,
                        int64_t end, RGWGetDataCB *cb);
            int get_attr(const DoutPrefixProvider *dpp,
        ...
        struct Write {
            ...
            struct MetaParams {
                ...
                MetaParams(): mtime(NULL), rmattrs(NULL), data(NULL),
                    ...
                    category(RGWObjCategory::Main), flags(0),
                    if_match(NULL), if_nomatch(NULL), canceled(false),
            ...
            void set_mp_part_str(std::string _mp_part_str)
            {
            ...
            int _do_write_meta(const DoutPrefixProvider *dpp,
                               ...
                               std::map < std::string,
                               bufferlist > &attrs, bool assume_noent,
                               bool modify_tail);
            int write_meta(const DoutPrefixProvider *dpp,
                           uint64_t size, uint64_t accounted_size,
                           std::map < std::string, bufferlist > &attrs);
        };
        ...
        int InitializeParamsfromObject(const DoutPrefixProvider *dpp,
                                       DBOpParams *params);
        ...
        int obj_omap_set_val_by_key(const DoutPrefixProvider *dpp,
        ...
        int obj_omap_get_vals_by_keys(const DoutPrefixProvider *dpp,
                                      ...
                                      bufferlist > *vals);
        int obj_omap_get_all(const DoutPrefixProvider *dpp,
                             std::map < std::string, bufferlist > *m);
        int obj_omap_get_vals(const DoutPrefixProvider *dpp,
                              const std::string &marker,
                              uint64_t count, std::map < std::string,
                              bufferlist > *m, bool *pmore);
        using iterate_obj_cb =
            int (*)(const DoutPrefixProvider *, const raw_obj &, off_t,
                    off_t,
                    bool, RGWObjState *, void *);
        int add_mp_part(const DoutPrefixProvider *dpp,
                        RGWUploadPartInfo info);
        int get_mp_parts_list(const DoutPrefixProvider *dpp,
    ...
    int get_entry(const std::string &oid, const std::string &marker,
                  ...
                  *entry);
    int get_next_entry(const std::string &oid,
                       const std::string &marker,
                       std::unique_ptr < rgw::sal::Lifecycle::LCEntry >
                       *entry);
    int set_entry(const std::string &oid,
                  rgw::sal::Lifecycle::LCEntry &entry);
    int list_entries(const std::string &oid,
                     const std::string &marker, uint32_t max_entries,
                     std::vector < std::unique_ptr <
                     rgw::sal::Lifecycle::LCEntry >> &entries);
    int rm_entry(const std::string &oid,
                 rgw::sal::Lifecycle::LCEntry &entry);
    int get_head(const std::string &oid,
                 std::unique_ptr < rgw::sal::Lifecycle::LCHead > *head);
    int put_head(const std::string &oid,
                 rgw::sal::Lifecycle::LCHead &head);
    int delete_stale_objs(const DoutPrefixProvider *dpp,
...
struct db_get_obj_data {
    ...
    db_get_obj_data(DB *db, RGWGetDataCB *cb,
                    uint64_t offset): store(db), client_cb(cb),
        offset(offset)
    {
    } ~db_get_obj_data()
    {
