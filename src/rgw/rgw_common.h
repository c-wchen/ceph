...
extern int gen_rand_alphanumeric_no_underscore(CephContext * cct, char *dest,
                                               int size);
extern int gen_rand_alphanumeric_plain(CephContext * cct, char *dest, int size);

extern int gen_rand_alphanumeric_lower(CephContext * cct, string * str,
                                       int length);
...
class RGWHTTPArgs {
  ...
  /** Set the arguments; as received */ void set(string s) {
...
const char *rgw_conf_get(const map < string, string, ltstr_nocase > &conf_map,
                         const char *name, const char *def_val);
int rgw_conf_get_int(const map < string, string, ltstr_nocase > &conf_map,
                     const char *name, int def_val);
bool rgw_conf_get_bool(const map < string, string, ltstr_nocase > &conf_map,
...
class RGWEnv {
  ...
  public:
    ...
    const std::map < string, string, ltstr_nocase > &get_map() const {
        return env_map;
}};
...
struct rgw_data_placement_target {
    ...
    :data_pool(data_pool),
        data_extra_pool(data_extra_pool), index_pool(index_pool)
{
...
struct rgw_bucket {
    ...
    void convert(cls_user_bucket * b) const {
        ...
        b->explicit_placement.index_pool =
            explicit_placement.index_pool.to_str();
    } void encode(bufferlist & bl) const {
        ENCODE_START(10, 10, bl);
        ::encode(name, bl);
        ::encode(marker, bl);
...
struct rgw_bucket_shard {
    ...
    } rgw_bucket_shard(const rgw_bucket & _b, int _sid):bucket(_b),
        shard_id(_sid) {
    }
...
struct RGWBucketInfo {
    ...
    } RGWBucketInfo():flags(0), has_instance_obj(false), num_shards(0),
        bucket_index_shard_hash_type(MOD), requester_pays(false),
...
struct RGWStorageStats {
    ...
    RGWStorageStats()
    :category(RGW_OBJ_CATEGORY_NONE), size(0), size_rounded(0), num_objects(0)
{
} void dump(Formatter * f) const;
};
...
struct rgw_obj_key {
    ...
    rgw_obj_key(const string & n, const string & i, const string & _ns):name(n),
        instance(i), ns(_ns) {
    }
   ...
   */
    static bool oid_to_key_in_ns(const string & oid, rgw_obj_key * key,
                                 const string & ns) {
   ...
   * false if it fails.
   */
    static bool strip_namespace_from_name(string & name, string & ns,
                                          string & instance) {
        ns.clear();
        instance.clear();
        if (name[0] != '_') {
...
void init(const rgw_bucket & b, const std::string & name, const string & i,
          const string & n) {
    bucket = b;
    key.set(name, i, n);
}
...
static inline void append_rand_alpha(CephContext * cct, const string & src,
                                     string & dest, int len)
{
    dest = src;
    char buf[len + 1];
...
static inline void add_amz_meta_header(std::map < std::string,
                                       std::string > &x_meta_map,
                                       const std::string & k,
                                       const std::string & v)
{
...
extern int parse_key_value(string & in_str, const char *delim, string & key,
                           string & val);
...
bool verify_bucket_permission(struct req_state *const s,
                              ...
                              &bucket_policy, const uint64_t op);
...
extern bool verify_object_permission(struct req_state *const s,
                                     const rgw_obj & obj,
                                     RGWAccessControlPolicy * const user_acl,
                                     RGWAccessControlPolicy * const bucket_acl,
...
extern bool verify_object_permission_no_policy(struct req_state *const s,
                                               RGWAccessControlPolicy *
                                               const user_acl,
                                               RGWAccessControlPolicy *
                                               const bucket_acl,
...
extern bool verify_object_permission_no_policy(struct req_state *s, int perm);
...
using sha1_digest_t = std::array < char, CEPH_CRYPTO_HMACSHA1_DIGESTSIZE >;
