    ...
    struct MDSCapParser:qi::grammar < Iterator, MDSAuthCaps() >
{
  MDSCapParser():MDSCapParser::base_type(mdscaps) {
        ...
        // capspec = * | r[w]
        capspec = spaces >> (lit("*")[_val = MDSCapSpec(true, true, true, true)]
                             |
                             (lit("rwp"))[_val =
        ...
        grant = lit("allow") >> (capspec >> match)[_val =
                                                   phoenix::construct <
                                                   MDSCapGrant > (_1, _2)];
...
bool MDSCapMatch::match_path(boost::string_view target_path) const const
{
    if (path.length()) {
 ...
 */
bool MDSAuthCaps::path_capable(boost::string_view inode_path) const const
{
...
bool MDSAuthCaps::is_capable(boost::string_view inode_path,
                             ...
                             unsigned mask,
                             uid_t new_uid, gid_t new_gid) constconst
{
    if (cct)
...
// << "[" << caller_gid_list << "]";
    ...
    for (std::vector < MDSCapGrant >::const_iterator i = grants.begin();
        ...
        if (i->match.match(inode_path, caller_uid, caller_gid, caller_gid_list)
            ...
            if (std::
                find(i->match.gids.begin(), i->match.gids.end(),
                     caller_gid) != i->match.gids.end()) {
                gids.push_back(caller_gid);
            }
            if (caller_gid_list) {
                std::set_intersection(i->match.gids.begin(),
                                      i->match.gids.end(),
...
{
    grants.clear();
    grants.push_back(MDSCapGrant(MDSCapSpec(true, true, true, true),
                                 MDSCapMatch()));
}
...
bool MDSAuthCaps::parse(CephContext * c, boost::string_view str, ostream * err)
{
    ...
    if (str == "allow") {
        ...
        grants.
            push_back(MDSCapGrant
                      (MDSCapSpec(true, true, false, true), MDSCapMatch()));
        return true;
    }
    ...
    if (r && iter == end) {
      for (auto & grant:grants) {
        ...
        if (err)
            *err << "MDSAuthCaps parse failed, stopped at '" << std::
                string(iter, end)
                << "' of '" << str << "'\n";
        return false;
    }
