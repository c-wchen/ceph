                                      ...
                                      std::shared_ptr < PerfCounters > *l)
{
    ...
    plb.add_u64_counter(P_UPDATE_METADATA, "update_metadata",
                        "Number of metadata updates");
    plb.add_u64_counter(P_UPDATE_ALLOCATED, "update_allocated",
                        "Number of allocated updates");
    plb.add_u64_counter(P_UPDATE_SIZE, "update_size", "Number of size updates");
    plb.add_u64_counter(P_UPDATE_VERSION, "update_version",
...
int SimpleRADOSStriper::lock(uint64_t timeoutms)
{
    ...
    while (true) {
        ...
        rados::cls::lock::lock(&op, biglock, ClsLockType::EXCLUSIVE,
                               cookie.to_string(), "", lockdesc, duration, 0);
