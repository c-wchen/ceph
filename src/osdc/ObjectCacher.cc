...
void ObjectCacher::Object::merge_left(BufferHead * left, BufferHead * right)
{
    ...
    left->last_write_tid =
        std::max(left->last_write_tid, right->last_write_tid);
...
void ObjectCacher::perf_start()
{
    ...
    plb.add_u64_counter(l_objectcacher_cache_bytes_hit,
                        "cache_bytes_hit", "Hit data", NULL, 0,
                        unit_t(UNIT_BYTES));
        ...
        if (!missing.empty() || !rx.empty()) {
            ...
            if (!error) {
                ...
                while (1) {
                    ...
                    uint64_t len =
                        std::min(f_it->second - foff, bh->length() - bhoff);
                    ldout(cct,
                ...
                ceph_assert(opos ==
                            (loff_t) ex_it->offset + (loff_t) ex_it->length);
            }
...
void ObjectCacher::flusher_entry()
{
    ...
    read_cond.wait(l,[this] {
                   ...
                   reads_outstanding << dendl; return false;}
                   else {
                   return true;}
                   }
    ) ;
                                     ...
                                     Context * on_finish)
{
    ...
    if (gather.has_subs()) {
        ...
        gather.set_finisher(new LambdaContext([this, oset, flushed,
                                               on_finish] (int) {
