...
void MDCache::handle_conf_change(const std::set < std::string > &changed,
                                 const MDSMap &mdsmap)
{
    ...
    if (changed.count("mds_cache_reservation"))
        cache_reservation =
            g_conf().get_val < double >("mds_cache_reservation");

    bool ephemeral_pin_config_changed = false;
    if (changed.count("mds_export_ephemeral_distributed")) {
        export_ephemeral_distributed_config =
            g_conf().get_val < bool > ("mds_export_ephemeral_distributed");
        dout(10) << "Migrating any ephemeral distributed pinned inodes" <<
                 dendl;
    ...
    if (ephemeral_pin_config_changed) {
        ...
        migrate.assign(export_ephemeral_pins.begin(),
                       export_ephemeral_pins.end());
        for (auto &in : migrate) {
...
void MDCache::create_unlinked_system_inode(CInode *in, inodeno_t ino, int mode) const const
{
    ...
    if (in->is_base()) {
        if (in->is_root())
            ...
            in->inode_auth =
                mds_authority_t(mds_rank_t(in->ino() - MDS_INO_MDSDIR_OFFSET),
                                CDIR_AUTH_UNKNOWN);
...
void MDCache::create_empty_hierarchy(MDSGather *gather)
{
    ...
    ceph_assert(rootdir->get_fnode()->accounted_fragstat ==
                rootdir->get_fnode()->fragstat);
    ceph_assert(rootdir->get_fnode()->fragstat == root->get_inode()->dirstat);
    ceph_assert(rootdir->get_fnode()->accounted_rstat ==
                rootdir->get_fnode()->rstat);
...
struct C_MDC_CreateSystemFile: public MDCacheLogContext {
    ...
    version_t dpv;
    MDSContext *fin;
    C_MDC_CreateSystemFile(MDCache *c, MutationRef &mu, CDentry *d,
                           version_t v, MDSContext *f): MDCacheLogContext(c),
        mut(mu), dn(d), dpv(v), fin(f)
    {
...
void MDCache::_create_system_file(CDir *dir, std::string_view name,
                                  CInode *in, MDSContext *fin)
{
    ...
    if (!in->is_mdsdir()) {
        predirty_journal_parents(mut, &le->metablob, in, dir,
    ...
    mds->mdlog->submit_entry(le,
                             new C_MDC_CreateSystemFile(this, mut, dn, dpv,
                                     fin));
    mds->mdlog->flush();
}

void MDCache::_create_system_file_finish(MutationRef &mut, CDentry *dn,
...
struct C_MDS_RetryOpenRoot: public MDSInternalContext {
    MDCache *cache;
    explicit C_MDS_RetryOpenRoot(MDCache *c): MDSInternalContext(c->mds),
        cache(c)
    {
    } void finish(int r) override
    {
...
{
    open_mydir_inode(new MDSInternalContextWrapper(mds,
                     new
                     LambdaContext([this,
    c](int r) {
        if (r < 0) {
...
void MDCache::populate_mydir()
{
    ...
    if (mydir->get_version() == 0 && mydir->state_test(CDir::STATE_BADFRAG)) {
        ...
        mds->clog->warn() << "fragment " << mydir->
                          dirfrag() << " was unreadable, " "recreating it now";
        LogSegment *ls = mds->mdlog->get_current_segment();
    ...
    for (int i = 0; i < NUM_STRAY; ++i) {
        ...
        *css << "stray" << i;
        ...
        if (!straydn || !straydn->get_linkage()->get_inode()) {
            _create_system_file(mydir, css->strv(),
                                create_system_inode(MDS_INO_STRAY
                                                    (mds->get_nodeid(), i),
                                                    S_IFDIR),
...
void MDCache::adjust_bounded_subtree_auth(CDir *dir,
        const set < CDir * > &bounds,
        mds_authority_t auth)
{
    dout(7) << "adjust_bounded_subtree_auth " << dir->
            get_dir_auth() << " -> " << auth << " on " << *dir << " bounds " <<
            bounds << dendl;
    ...
    for (const auto &bound : bounds) {
        ...
        if (subtrees[dir].count(bound) == 0) {
            if (get_subtree_root(bound) == dir) {
                dout(10) << "  new bound " << *bound <<
                         ", adjusting auth back to old " << oldauth << dendl;
                ...
                while (1) {
                    ...
                    dout(10) << "  swallowing intervening subtree at " << *t <<
                             dendl;
        ...
        set < CDir * > &bounds)
{
    ...
    for (map < inodeno_t, fragset_t >::iterator p = byino.begin();
         p != byino.end(); ++p) {
        p->second.simplify();
        ...
        dout(10) << " checking fragset " << p->second.
                 get() << " on " << *diri << dendl;

        fragtree_t tmpdft;
        for (set < frag_t >::iterator q = p->second.begin();
             q != p->second.end(); ++q) {
...
{
    dout(7) << "adjust_bounded_subtree_auth " << dir->
            get_dir_auth() << " -> " << auth << " on " << *dir << " bound_dfs " <<
            bound_dfs << dendl;
...
void MDCache::map_dirfrag_set(const list < dirfrag_t > &dfs,
                              set < CDir * > &result)
{
...
void MDCache::verify_subtree_bounds(CDir *dir,
                                    const list < dirfrag_t > &bounds)
{
    // for debugging only.
    ceph_assert(subtrees.count(dir));
    ...
    for (const auto &fg : bounds) {
        ...
        if (subtrees[dir].count(bd) == 0) {
            dout(0) << "verify_subtree_bounds failed: extra bound " << *bd <<
                    dendl;
            failed++;
        }
    }
...
void MDCache::project_subtree_rename(CInode *diri, CDir *olddir,
                                     CDir *newdir)
{
    dout(10) << "project_subtree_rename " << *diri << " from " << *olddir
             << " to " << *newdir << dendl;
    projected_subtree_renames[diri].push_back(pair < CDir *,
            CDir * >(olddir, newdir));
}
        ...
        bool pop)
{
    ...
    if (pop) {
        map < CInode *, list < pair < CDir *, CDir * > > >::iterator p =
            projected_subtree_renames.find(diri);
        ...
        if (ret.first) {
            ...
            if (!oldin->client_snap_caps.empty()) {
                for (int i = 0; i < num_cinode_locks; i++) {
                    SimpleLock *lock =
                        oldin->get_lock(cinode_lock_info[i].lock);
                    ceph_assert(lock);
            ...
            if (!client_snap_caps.empty()) {
                ...
                for (int i = 0; i < num_cinode_locks; i++) {
                    ...
                    if (!lock->get_num_wrlocks()) {
                        ...
                        lock->
                        take_waiting(SimpleLock::WAIT_STABLE | SimpleLock::
                                     WAIT_RD, finished);
                        in->auth_unpin(lock);
                    }
                }
                                 ...
                                 CInode **pcow_inode, CDentry::linkage_t *dnl)
{
    ...
    if (in && (in->is_multiversion() || cow_head)) {
        ...
        if (in->get_projected_parent_dn() != dn) {
            ...
            if (dir_follows + 1 > dn->first) {
                ...
                if (realm->has_snaps_in_range(oldfirst, dir_follows)) {
                    CDir *dir = dn->dir;
                    CDentry *olddn =
                        dir->add_remote_dentry(dn->get_name(), in->ino(),
                                               in->d_type(), dn->alternate_name,
        ...
        // already cloned?
        if (follows < in->first) {
            dout(10) << "journal_cow_dentry follows " << follows <<
                     " < first on " << *in << dendl;
            return;
        }

        if (!realm->has_snaps_in_range(in->first, follows)) {
            dout(10) << "journal_cow_dentry no snapshot follows " << follows <<
                     " on " << *in << dendl;
            in->first = follows + 1;
            return;
        }
        ...
        if (in) {
            ...
            CDentry *olddn =
                dir->add_primary_dentry(dn->get_name(), oldin,
            ...
            olddn->set_projected_version(dir->get_projected_version());
            metablob->add_primary_dentry(olddn, 0, true, false, false,
                                         need_snapflush);
            mut->add_cow_dentry(olddn);
        } else {
            ceph_assert(dnl->is_remote());
            CDentry *olddn =
                dir->add_remote_dentry(dn->get_name(), dnl->get_remote_ino(),
                                       dnl->get_remote_d_type(),
                                       dn->alternate_name, oldfirst, follows);
            dout(10) << " olddn " << *olddn << dendl;
        ...
        SnapRealm *prealm)
{
    ...
    dout(10) << "projected_rstat_inode_to_frag first " << first <<
             " linkunlink " << linkunlink << " " << *cur << dendl;
    dout(20) << "    frag head is [" << parent->first << ",head] " << dendl;
    dout(20) << " inode update is [" << first << "," << cur->
             last << "]" << dendl;
    ...
    dout(20) << " floor of " << floor << " from parent dn " << *parentdn <<
             dendl;
    ...
    if (cur->last != CEPH_NOSNAP) {
        ...
        set < snapid_t >::const_iterator q =
            snaps.lower_bound(std::max(first, floor));
    ...
    if (cur->last >= floor) {
        ...
        _project_rstat_inode_to_frag(pi, std::max(first, floor), cur->last,
                                     parent, linkunlink, update);
    }
...
void MDCache::_project_rstat_inode_to_frag(const CInode::mempool_inode *inode,
    ...
    while (last >= ofirst) {
        ...
        if (last == CEPH_NOSNAP) {
            ...
            if (it == parent->dirty_old_rstat.end()) {
                dout(20) << "  no dirty_old_rstat with last >= last " << last <<
                         dendl;
                if (!parent->dirty_old_rstat.empty()
                    && parent->dirty_old_rstat.rbegin()->first >= first) {
                    dout(20) << "  last dirty_old_rstat ends at " << parent->
                             dirty_old_rstat.rbegin()->first << dendl;
                ...
                if (it->second.first <= last) {
                    ...
                    if (it->second.first < first) {
                        dout(10) << " splitting off left bit [" << it->second.
                                 first << "," << first - 1 << "]" << dendl;
                    ...
                    if (last < it->first) {
                        dout(10) << " splitting off right bit [" << last +
                                 1 << "," << it->first << "]" << dendl;
                    ...
                    if (it->second.first <= last && // new *it isn't also to the right, and
                        ...
                        dout(10) << " staying to the right of [" << it->second.
                                 first << "," << it->first << "]..." << dendl;
                        first = it->first + 1;
                    }
            ...
            dout(20) << " projecting to dirty_old_rstat [" << first << "," <<
                     last << "]" << dendl;
            parent->dirty_old_rstat[last].first = first;
            prstat = &parent->dirty_old_rstat[last].rstat;
        }
    ...
    while (last >= ofirst) {
        ...
        if (last == pin->last) {
            ...
            if (first > pin->first) {
                ...
                dout(20) << "   cloned old_inode rstat is " << old.inode.
                         rstat << dendl;
            }
        } else {
            if (!_old_inodes) {
            ...
            if (last >= pin->first) {
                ...
                if (it == _old_inodes->end()) {
                    dout(10) << " no old_inode <= " << last << ", done." <<
                             dendl;
                    break;
                }
                first = it->second.first;
                if (first > last) {
                    dout(10) << " oldest old_inode is [" << first << "," << it->
                             first << "], done." << dendl;
            ...
            if (first < ofirst) {
                dout(10) << " splitting left old_inode [" << first << "," <<
                         last << "] to [" << first << "," << ofirst -
                         1 << "]" << dendl;
        ...
        dout(20) << " projecting to [" << first << "," << last << "] " << pi->
                 rstat << dendl;
        pi->rstat.add(delta);
        dout(20) << "        result [" << first << "," << last << "] " << pi->
                 rstat << dendl;
                                        ...
                                        bool quota_change)
{
    ...
    for (auto &p : in->client_caps) {
        ...
        if (pi->quota.max_bytes > 0) {
            if (pi->rstat.rbytes >
                pi->quota.max_bytes - (pi->quota.max_bytes >> 3)) {
                goto update;
            }
    ...
    while (parent) {
        ...
        if (do_parent_mtime || linkunlink) {
            ...
            if (do_parent_mtime) {
                ...
                dout(10) <<
                         "predirty_journal_parents bumping fragstat change_attr to "
                         << pf->fragstat.change_attr << " on " << parent << dendl;
                if (pf->fragstat.mtime > pf->rstat.rctime) {
                    dout(10) << "predirty_journal_parents updating mtime on " <<
                             *parent << dendl;
                    pf->rstat.rctime = pf->fragstat.mtime;
                } else {
                    dout(10) <<
                             "predirty_journal_parents updating mtime UNDERWATER on "
                             << *parent << dendl;
                }
            }
            if (linkunlink) {
                dout(10) << "predirty_journal_parents updating size on " <<
                         *parent << dendl;
                if (in->is_dir()) {
        ...
        if (!primary_dn) {
            ...
            if (!mut->is_wrlocked(&pin->nestlock)) {
                dout(10) << " taking wrlock on " << pin->
                         nestlock << " on " << *pin << dendl;
            ...
            project_rstat_inode_to_frag(mut, cur, parent, first, linkunlink,
                                        prealm);
        ...
        if (!pin->is_auth()
            ...
            dout(10) <<
                     "predirty_journal_parents !auth or ambig or can't authpin on "
                     << *pin << dendl;
            stop = true;
        }
        ...
        if (!stop && !first && g_conf()->mds_dirstat_min_interval > 0) {
            double since_last_prop =
                mut->get_mds_stamp() - pin->last_dirstat_prop;
            if (since_last_prop < g_conf()->mds_dirstat_min_interval) {
                dout(10) << "predirty_journal_parents last prop " <<
                         since_last_prop << " < " << g_conf()->
                         mds_dirstat_min_interval << ", stopping" << dendl;
                stop = true;
            } else {
                dout(10) << "predirty_journal_parents last prop " <<
                         since_last_prop << " ago, continuing" << dendl;
            }
        }

        // can cast only because i'm passing nowait=true in the sole user
        if (!stop && !mut->is_wrlocked(&pin->nestlock) && (!pin->versionlock.can_wrlock() ||
                // make sure we can take versionlock, too
                !mds->locker->
                wrlock_try(&pin->
                           nestlock,
                           mut)
                                                          )) {                // ** do not initiate.. see above comment **
            dout(10) << "predirty_journal_parents can't wrlock one of " << pin->
                     versionlock << " or " << pin->
                     nestlock << " on " << *pin << dendl;
            stop = true;
        }
        ...
        // dirstat
        if (do_parent_mtime || linkunlink) {
            dout(20) << "predirty_journal_parents add_delta " << pf->
                     fragstat << dendl;
            dout(20) << "predirty_journal_parents         - " << pf->
                     accounted_fragstat << dendl;
            bool touched_mtime = false, touched_chattr = false;
            pi.inode->dirstat.add_delta(pf->fragstat, pf->accounted_fragstat,
            ...
            dout(20) << "predirty_journal_parents     gives " << pi.inode->
                     dirstat << " on " << *pin << dendl;

            if (parent->get_frag() == frag_t()) {   // i.e., we are the only frag
                if (pi.inode->dirstat.size() < 0)
                    ceph_assert(!"negative dirstat size" ==
                                g_conf()->mds_verify_scatter);
                if (pi.inode->dirstat.size() != pf->fragstat.size()) {
                    mds->clog->
                    error() << "unmatched fragstat size on single dirfrag "
                            << parent->dirfrag() << ", inode has " << pi.inode->
                            dirstat << ", dirfrag has " << pf->fragstat;

                    // trust the dirfrag for now
                    pi.inode->dirstat = pf->fragstat;

                    ceph_assert(!"unmatched fragstat size" ==
                                g_conf()->mds_verify_scatter);
                }
            }
        }
        ...
        // rstat
        dout(10) << "predirty_journal_parents frag->inode on " << *parent <<
                 dendl;
        ...
        if (g_conf()->mds_snap_rstat) {
            for (auto &p : parent->dirty_old_rstat) {
                project_rstat_frag_to_inode(p.second.rstat,
                                            p.second.accounted_rstat,
                                            p.second.first, p.first, pin, true);
            }
        }
        ...
        if (parent->get_frag() == frag_t()) {   // i.e., we are the only frag
            if (pi.inode->rstat.rbytes != pf->rstat.rbytes) {
                ...
                error() << "unmatched rstat rbytes on single dirfrag " <<
                        parent->dirfrag() << ", inode has " << pi.inode->
                        rstat << ", dirfrag has " << pf->rstat;

                // trust the dirfrag for now
                pi.inode->rstat = pf->rstat;

                ceph_assert(!"unmatched rstat rbytes" ==
                            g_conf()->mds_verify_scatter);
            }
        }
...
struct C_MDC_CommittedLeader: public MDCacheLogContext {
    metareqid_t reqid;
    C_MDC_CommittedLeader(MDCache *s, metareqid_t r): MDCacheLogContext(s),
        reqid(r)
    {
    } void finish(int r) override
    {
...
void MDCache::committed_leader_peer(metareqid_t r, mds_rank_t from)
{
    ...
    if (!uncommitted_leaders[r].recovering
        && uncommitted_leaders[r].peers.empty()) {
        log_leader_commit(r);
    }
}
...
struct C_MDC_PeerCommit: public MDCacheLogContext {
    mds_rank_t from;
    metareqid_t reqid;
    C_MDC_PeerCommit(MDCache *c, int f, metareqid_t r): MDCacheLogContext(c),
        from(f), reqid(r)
    {
    } void finish(int r) override
    {
...
{
    dout(10) << "_logged_peer_commit from mds." << from << " " << reqid <<
             dendl;

    // send a message
    auto req =
        make_message < MMDSPeerRequest > (reqid, 0,
...
void MDCache::_move_subtree_map_bound(dirfrag_t df, dirfrag_t oldparent,
                                      dirfrag_t newparent, map < dirfrag_t,
                                      vector < dirfrag_t > > &subtrees)
{
    if (subtrees.count(oldparent)) {
        vector < dirfrag_t > &v = subtrees[oldparent];
        dout(10) << " removing " << df << " from " << oldparent << " bounds " <<
                 v << dendl;
    ...
    for (const auto &[diri, renames] : projected_subtree_renames) {
        for (const auto &[olddir, newdir] : renames) {
            ...
            for (const auto &dir : dfls) {
                ...
                dout(15) << " old parent " << oldparent->
                         dirfrag() << " " << *oldparent << dendl;
                CDir *newparent = get_projected_subtree_root(newdir);
                dout(15) << " new parent " << newparent->
                         dirfrag() << " " << *newparent << dendl;

                if (oldparent == newparent) {
                    dout(15) << "parent unchanged for " << dir->
                             dirfrag() << " at " << oldparent->dirfrag() << dendl;
                    continue;
                }
                ...
                if (dir->is_subtree_root()) {
                    ...
                    // children are fine.  change parent.
                    _move_subtree_map_bound(dir->dirfrag(),
                                            oldparent->dirfrag(),
                                            newparent->dirfrag(), le->subtrees);
                } else {
                    // mid-subtree.

                    if (oldparent->get_dir_auth() != newparent->get_dir_auth()) {
                        dout(10) << " creating subtree for " << dir->
                                 dirfrag() << dendl;
                    ...
                    for (auto &bound : subtrees.at(oldparent)) {
                        if (dir->contains(bound->get_parent_dir()))
                            _move_subtree_map_bound(bound->dirfrag(),
                                                    oldparent->dirfrag(),
                                                    newparent->dirfrag(),
                                                    le->subtrees);
                    }
...
void MDCache::dump_resolve_status(Formatter *f) const const
{
...
void MDCache::send_resolves()
{
    ...
    if (!resolve_done) {
        ...
        mds->snapclient->sync(new MDSInternalContextWrapper(mds,
                              new
                              LambdaContext([this]
                                            (int
        r) {
        ...
        dout(10) << "send_resolves waiting for snapclient cache to sync" <<
                 dendl;
        return;
    }
...
void MDCache::send_peer_resolves()
{
    ...
    if (mds->is_resolve()) {
        ...
        for (ceph::unordered_map < metareqid_t, MDRequestRef >::iterator p =
                 active_requests.begin(); p != active_requests.end(); ++p) {
            ...
            if (resolve_set.count(leader)
                ...
                if (!mdr->committing &&
                    ...
                    resolves[leader]->add_peer_request(p->first,
                                                       mdr->committing);
                }
            }
        }
...
void MDCache::send_subtree_resolves()
{
    ...
    for (map < CDir *, set < CDir * > >::iterator p = subtrees.begin();
        ...
        if (migrator->is_ambiguous_import(dir->dirfrag())) {
            ...
            for (set < CDir * >::iterator q = bounds.begin(); q != bounds.end();
                 ++q) {
    ...
    // ambiguous
    for (map < dirfrag_t, vector < dirfrag_t > >::iterator p =
             my_ambiguous_imports.begin(); p != my_ambiguous_imports.end(); ++p) {
    ...
    for (auto p = my_subtrees.begin(); p != my_subtrees.end(); ++p) {
        ...
        while (i < p->second.size()) {
            ...
            if (my_subtrees.count(b)) {
                ...
                dout(10) << " simplify: " << p->
                         first << " swallowing " << b << " with bounds " << bb <<
                         dendl;
                for (vector < dirfrag_t >::iterator r = bb.begin();
                     r != bb.end(); ++r) {
    ...
    for (auto &p : resolves) {
        ...
        if (mds->is_resolve()) {
            ...
            m->add_table_commits(TABLE_SNAP,
                                 mds->snapclient->get_journaled_tids());
        }
...
void MDCache::handle_mds_failure(mds_rank_t who)
{
    ...
    for (ceph::unordered_map < metareqid_t, MDRequestRef >::iterator p =
        ...
        if (mdr->peer_to_mds == who) {
            if (mdr->peer_did_prepare()) {
                ...
                if (!mdr->more()->waiting_on_peer.empty()) {
                    ceph_assert(mdr->more()->srcdn_auth_mds ==
                                mds->get_nodeid());
        ...
        if (mdr->is_peer() && mdr->peer_did_prepare()) {
            ...
            if (mdr->more()->srcdn_auth_mds == who &&
                mds->mdsmap->is_clientreplay_or_active_or_stopping(mdr->
                        peer_to_mds)) {
        ...
        if (mdr->is_leader() && !mdr->committing) {
            ...
            if (mdr->more()->witnessed.count(who)) {
                ...
                if (srcdn_auth >= 0
                    && mdr->more()->waiting_on_peer.count(srcdn_auth)) {
                    dout(10) << " leader request " << *mdr <<
                             " waiting for rename srcdn's auth mds." << mdr->more()->
                             srcdn_auth_mds << " to reply" << dendl;
                    ...
                    dout(10) << " leader request " << *mdr <<
                             ...
                             " to recover" << dendl;
                    if (srcdn_auth >= 0)
                        ceph_assert(mdr->more()->witnessed.count(srcdn_auth) ==
                                    0);
            ...
            if (mdr->more()->waiting_on_peer.count(who)) {
                dout(10) << " leader request " << *mdr <<
                         " waiting for peer mds." << who << " to recover" << dendl;
                // retry request when peer recovers
                mdr->more()->waiting_on_peer.erase(who);
                if (mdr->more()->waiting_on_peer.empty())
                    mds->wait_for_active_peer(who,
                                              new C_MDS_RetryRequest(this,
                                                      mdr));
            }
    ...
    for (map < dirfrag_t, fragment_info_t >::iterator p = fragments.begin();
        ...
        ++p;
        dout(10) << "cancelling fragment " << df << " bit " << info.
                 bits << dendl;
...
void MDCache::handle_resolve(const cref_t < MMDSResolve > &m)
{
    ...
    if (!m->peer_requests.empty()) {
        if (mds->is_clientreplay() || mds->is_active() || mds->is_stopping()) {
            for (auto p = m->peer_requests.begin(); p != m->peer_requests.end();
                 ++p) {
            ...
            if (!pending_leaders.empty()) {
                dout(10) <<
                         " still have pending updates, delay processing peer resolve"
                         << dendl;
        ...
        for (const auto &p : m->peer_requests) {
            if (uncommitted_leaders.count(p.first)) {   //mds->sessionmap.have_completed_request(p.first)) {
                ...
                if (p.second.committing) {
                    ...
                    dout(10) << " already committing peer request " << p <<
                             " noop " << dendl;
                } else {
                    dout(10) << " ambiguous peer request " << p <<
                             " will COMMIT" << dendl;
                ...
                if (p.second.inode_caps.length() > 0) {
                    ...
                    map < client_t, Capability::Export > cap_exports =
                        inode_caps.cap_exports;
                    ceph_assert(get_inode(ino));

                    for (map < client_t, Capability::Export >::iterator q =
                             cap_exports.begin(); q != cap_exports.end(); ++q) {
                        Capability::Import &im =
                            rejoin_imported_caps[from][ino][q->first];
                        ...
                        Session *session =
                            mds->sessionmap.
                            get_session(entity_name_t::CLIENT(q->first.v));
                        if (session)
                            rejoin_client_map.emplace(q->first,
                                                      session->info.inst);
                    }
                    ...
                    encode(rejoin_imported_caps[from][ino],
                           ack->commit[p.first]);
                }
            } else {
                // ABORT
                dout(10) << " ambiguous peer request " << p << " will ABORT" <<
                         dendl;
                ceph_assert(!p.second.committing);
    ...
    if (mds->is_clientreplay() || mds->is_active() || mds->is_stopping()) {
        ...
        map < dirfrag_t, vector < dirfrag_t > >::iterator p =
            my_ambiguous_imports.begin();
        while (p != my_ambiguous_imports.end()) {
            ...
            if (migrator->is_importing(dir->dirfrag()) &&
                ...
                ceph_assert(migrator->get_import_state(dir->dirfrag()) ==
                ...
                if (claimed_by_sender) {
                    ...
                    dout(7) << "ambiguous import succeeded on " << *dir <<
                            dendl;
    ...
    for (const auto &p : m->subtrees) {
        dout(10) << "peer claims " << p.first << " bounds " << p.
                 second << dendl;
    ...
    for (const auto &p : m->ambiguous_imports) {
        dout(10) << "noting ambiguous import on " << p.first << " bounds " << p.
                 second << dendl;
...
{
    dout(10) << "handle_resolve_ack " << *ack << " from " << ack->
             get_source() << dendl;
    ...
    if (ambiguous_peer_updates.count(from)) {
        ...
        ceph_assert(mds->is_clientreplay() || mds->is_active()
                    || mds->is_stopping());
    }
    ...
    for (const auto &p : ack->commit) {
        ...
        if (mds->is_resolve()) {
            ...
            start_submit_entry(new
                               EPeerUpdate(mds->mdlog, "unknown", p.first,
                                           from, EPeerUpdate::OP_COMMIT,
                                           su->origop),
...
void MDCache::add_uncommitted_peer(metareqid_t reqid, LogSegment *ls,
                                   mds_rank_t leader, MDPeerUpdate *su)
{
    ...
    for (set < CInode * >::iterator p = su->olddirs.begin();
         p != su->olddirs.end(); ++p) {
        uncommitted_peer_rename_olddir[*p]++;
    }
    for (set < CInode * >::iterator p = su->unlinked.begin();
         p != su->unlinked.end(); ++p) {
...
void MDCache::finish_uncommitted_peer(metareqid_t reqid, bool assert_exist)
{
    ...
    for (set < CInode * >::iterator p = su->olddirs.begin();
         p != su->olddirs.end(); ++p) {
        CInode *diri = *p;
        map < CInode *, int >::iterator it =
            uncommitted_peer_rename_olddir.find(diri);
...
MDPeerUpdate *MDCache::get_uncommitted_peer(metareqid_t reqid,
        mds_rank_t leader)
{
...
void MDCache::disambiguate_other_imports()
{
    ...
    bool recovering = !(mds->is_clientreplay() || mds->is_active()
                        || mds->is_stopping());
    // other nodes' ambiguous imports
    for (map < mds_rank_t, map < dirfrag_t,
         vector < dirfrag_t > > >::iterator p = other_ambiguous_imports.begin();
         p != other_ambiguous_imports.end(); ++p) {
        mds_rank_t who = p->first;
        dout(10) << "ambiguous imports for mds." << who << dendl;

        for (map < dirfrag_t, vector < dirfrag_t > >::iterator q =
                 p->second.begin(); q != p->second.end(); ++q) {
            dout(10) << " ambiguous import " << q->first << " bounds " << q->
                     second << dendl;
            ...
            if (dir->is_ambiguous_auth() || // works for me_ambig or if i am a surviving bystander
                ...
                dout(10) << "  mds." << who << " did not import " << *dir <<
                         dendl;
            }
        }
    }
...
void MDCache::disambiguate_my_imports()
{
    ...
    while (!my_ambiguous_imports.empty()) {
        map < dirfrag_t, vector < dirfrag_t > >::iterator q =
            my_ambiguous_imports.begin();
    ...
    for (map < CDir *, set < CDir * > >::iterator p = subtrees.begin();
        ...
        if (dir->is_ambiguous_dir_auth()) {
            dout(0) <<
                    "disambiguate_imports uh oh, dir_auth is still ambiguous for "
                    << *dir << dendl;
        }
...
void MDCache::add_ambiguous_import(dirfrag_t base,
                                   const vector < dirfrag_t > &bounds)
{
...
bool MDCache::expire_recursive(CInode *in, expiremap &expiremap)
{
    ...
    for (const auto &subdir : ls) {
        if (!in->is_mdsdir() && subdir->is_subtree_root()) {
            dout(10) << __func__ << ": stray still has subtree " << *in <<
                     dendl;
            return true;
        }
        ...
        for (auto it = subdir->items.begin(); it != subdir->items.end();) {
            ...
            if (dnl->is_primary()) {
                ...
                if (!tin->is_stray() && tin->get_inode()->nlink) {
                    dout(10) << __func__ << ": stray still has linkage " << *tin
                             << dendl;
                    return true;
                }
            ...
            if (dn->lru_is_expireable()) {
                ...
                dout(10) << __func__ << ": stray dn is not expireable " << *dn
                         << dendl;
                return true;
            }
        }
...
void MDCache::dump_rejoin_status(Formatter *f) const const
{
...
void MDCache::rejoin_send_rejoins()
{
    ...
    if (rejoin_gather.count(mds->get_nodeid())) {
        dout(7) <<
                "rejoin_send_rejoins still processing imported caps, delaying" <<
                dendl;
    ...
    for (const auto &rank : recovery_set) {
        ...
        if (mds->is_rejoin())
            rejoins[rank] =
                make_message < MMDSCacheRejoin > (MMDSCacheRejoin::OP_WEAK);
        else if (mds->mdsmap->is_rejoin(rank))
            rejoins[rank] =
                make_message < MMDSCacheRejoin > (MMDSCacheRejoin::OP_STRONG);
    }
    ...
    if (mds->is_rejoin()) {
        ...
        for (auto &p : client_exports) {
            ...
            for (auto &q : p.second.second) {
                ...
                rejoin->client_metadata_map[p.first] =
        ...
        if (mds->is_rejoin()) {
            ...
            if (p.first == 0 && root) {
                ...
                if (root->is_dirty_scattered()) {
                    dout(10) << " sending scatterlock state on root " << *root
                             << dendl;
        ...
        for (ceph::unordered_map < metareqid_t, MDRequestRef >::iterator p =
            ...
            for (const auto &q : mdr->object_states) {
                ...
                if (!q.first->is_auth()) {
                    ...
                    dout(15) << " " << *mdr << " authpin on " << *q.
                             first << dendl;
                    MDSCacheObjectInfo i;
                    q.first->set_object_info(i);
                    if (i.ino)
                        rejoin->add_inode_authpin(vinodeno_t(i.ino, i.snapid),
                                                  mdr->reqid, mdr->attempt);
                    else
                        rejoin->add_dentry_authpin(i.dirfrag, i.dname, i.snapid,
                                                   mdr->reqid, mdr->attempt);

                    if (mdr->has_more() && mdr->more()->is_remote_frozen_authpin
                        && mdr->more()->rename_inode == q.first)
                        rejoin->
                        add_inode_frozen_authpin(vinodeno_t
                                                 (i.ino, i.snapid),
                                                 mdr->reqid, mdr->attempt);
                }
            }
            ...
            for (const auto &q : mdr->locks) {
                ...
                if (q.is_xlock() && !obj->is_auth()) {
                    ...
                    dout(15) << " " << *mdr << " xlock on " << *lock << " " <<
                             *obj << dendl;
                    MDSCacheObjectInfo i;
                    obj->set_object_info(i);
                    if (i.ino)
                        rejoin->add_inode_xlock(vinodeno_t(i.ino, i.snapid),
                                                lock->get_type(), mdr->reqid,
                                                mdr->attempt);
                    else
                    ...
                    dout(15) << " " << *mdr << " wrlock on " << *lock << " " <<
                             *obj << dendl;
                    MDSCacheObjectInfo i;
                    obj->set_object_info(i);
                    ceph_assert(i.ino);
                    rejoin->add_inode_wrlock(vinodeno_t(i.ino, i.snapid),
                                             lock->get_type(), mdr->reqid,
                                             mdr->attempt);
                }
            }
...
void MDCache::rejoin_walk(CDir *dir, const ref_t < MMDSCacheRejoin > &rejoin)
{
    ...
    if (mds->is_rejoin()) {
        ...
        for (auto &p : dir->items) {
            ...
            rejoin->add_weak_primary_dentry(dir->ino(), dn->get_name(),
                                            dn->first, dn->last, in->ino());
            {
                auto && dirs = in->get_nested_dirfrags();
                nested.insert(std::end(nested), std::begin(dirs),
                              std::end(dirs));
            }
        ...
        for (auto it = dir->items.begin(); it != dir->items.end();) {
            ...
            rejoin->add_strong_dentry(dir->dirfrag(), dn->get_name(),
                                      dn->get_alternate_name(), dn->first,
                                      dn->last,
                                      dnl->is_primary() ? dnl->get_inode()->
                                      ino() : inodeno_t(0),
                                      dnl->is_remote() ? dnl->
            ...
            if (dnl->is_primary()) {
                ...
                if (in->is_dirty_scattered()) {
                    dout(10) << " sending scatterlock state on " << *in <<
                             dendl;
                    rejoin->add_scatterlock_state(in);
                }
            }
...
void MDCache::handle_cache_rejoin_weak(const cref_t < MMDSCacheRejoin > &weak)
{
    ...
    if (mds->is_clientreplay() || mds->is_active() || mds->is_stopping()) {
        ...
        // check cap exports
        for (auto p = weak->cap_exports.begin(); p != weak->cap_exports.end();
             ++p) {
            CInode *in = get_inode(p->first);
            ceph_assert(!in || in->is_auth());
            for (auto q = p->second.begin(); q != p->second.end(); ++q) {
                dout(10) << " claiming cap import " << p->
                         first << " client." << q->first << " on " << *in << dendl;
                Capability *cap =
        ...
        // check cap exports.
        rejoin_client_map.insert(weak->client_map.begin(),
                                 weak->client_map.end());
    ...
    for (const auto &p : weak->weak_dirfrags) {
        ...
        for (const auto &leaf : leaves) {
            ...
            if (!dir) {
                dout(0) << " missing dir for " << p.
                        frag << " (which maps to " << leaf << ") on " << *diri <<
                        dendl;
                continue;
            }
            ceph_assert(dir);
            if (dirs_to_share.count(dir)) {
                dout(10) << " already have " << p.
                         frag << " -> " << leaf << " " << *dir << dendl;
            } else {
    ...
    for (const auto &p : weak->weak) {
        ...
        for (const auto &q : p.second) {
            ...
            if (!dir || dir->get_frag() != fg) {
                ...
                if (!dir)
                    dout(0) << " missing dir frag " << fg << " on " << *diri <<
                            dendl;
                ceph_assert(dir);
            ...
            if (ack)
                ack->add_strong_dentry(dir->dirfrag(), dn->get_name(),
                                       dn->get_alternate_name(), dn->first,
                                       dn->last, dnl->get_inode()->ino(),
                                       inodeno_t(0), 0, dnonce,
    ...
    if (survivor) {
        ...
        for (const auto &p : weak->inode_scatterlocks) {
            ...
            ceph_assert(in);
            dout(10) <<
                     " including base inode (due to potential scatterlock update) "
                     << *in << dendl;
            acked_inodes.insert(in->vino());
            ack->add_inode_base(in, mds->mdsmap->get_up_features());
        }
        ...
        for (set < SimpleLock * >::iterator p = gather_locks.begin();
             p != gather_locks.end(); ++p) {
            if (!(*p)->is_stable()) {
 ...
 */
void MDCache::rejoin_scour_survivor_replicas(mds_rank_t from,
        const cref_t < MMDSCacheRejoin >
        &ack,
        ...
        set < SimpleLock * > &gather_locks)
{
    ...
    auto scour_func =
    [this, from, ack, &acked_inodes, &gather_locks](CInode * in) {
        // inode?
        if (in->is_auth() &&
            in->is_replica(from) &&
            (ack == NULL || acked_inodes.count(in->vino()) == 0)) {
            inode_remove_replica
            (in, from,
             false,
             gather_locks);
            dout(10) <<
 ...
 > &strong)
{
    ...
    if (!mds->
        ...
        ceph_abort_msg
        ("got unexpected rejoin message during recovery");
    }
                    ...
                    if (d.
                        ...
                        (vinodeno_t
                         ...
                         vino()) {
                            ...
                            insert(in);
                            dout(7) <<
                                    " sender has primary dentry but wrong inode"
                                    << dendl;
                        }
                    } else {
                        ...
                        (strong->
                         ...
                         0);
                        dout(7) <<
                                " sender doesn't have primay dentry"
                                << dendl;
                    }
                } else {
                    if (d.
                        ...
                        insert(in);
                        dout(7) <<
                                " sender has primary dentry but we don't"
                                << dendl;
                    }
                }
            }
...
class C_MDC_RejoinOpenInoFinish:                                  public
    MDCacheContext
{
    inodeno_t ino;
public:
...
class C_MDC_RejoinSessionsOpened:                                 public
    MDCacheLogContext
{
public:
...
struct C_MDC_OpenSnapRealms:                                      public
    MDCacheContext {
...
()
{
    ...
    notify_global_snaprealm_update
    (CEPH_SNAP_OP_UPDATE);
    if
    (!reconnected_snaprealms.
     empty()) {
 ...
 >> &updates)
{
    if (seq <
        ...
        dout(10) <<
                 "finish_snaprealm_reconnect client."
                 << client <<
                 " up to date"
                 << " on " <<
                 *realm <<
        ...
        while (!dq.
            ...
            for (auto &p :                                          dir->items) {
                ...
                for (auto &r :                            in->get_replicas()) {
                    ...
                    bl;
                    in->
                    _encode_locks_state_for_rejoin
                    (bl,
                     r.first);
                    it->second->
                    add_inode_locks
...
class C_MDC_ReIssueCaps:                                          public
    MDCacheContext
{
    CInode *in;
public:
...
class C_MDC_RetryTruncate:                                        public
    MDCacheContext
{
    CInode *in;
    LogSegment *ls;
...
struct C_IO_MDC_TruncateWriteFinish:                              public
    MDCacheIOContext {
    CInode *in;
    LogSegment *ls;
    uint32_t block_size;
...
struct C_IO_MDC_TruncateFinish:                                public
    MDCacheIOContext {
    CInode *in;
    LogSegment *ls;
 ...
 ls)
{
    ...
    dout(10) <<
             "_truncate_inode  snapc "
             << snapc <<
             " on " << *in
             <<
             " fscrypt_last_block length is "
             << pi->
             fscrypt_last_block.
             length()
             << dendl;
    auto layout =
        pi->layout;
    struct
        ceph_fscrypt_last_block_header
        header;
    memset
    (&header, 0,
     sizeof
    ...
    if (data.
        length()) {
        dout(10) <<
                 "_truncate_inode write on inode "
                 << *in <<
                 " change_attr: "
                 << header.
                 change_attr
        ...
        if (pi->
            ...
            length()) {
            dout(10) <<
                     "_truncate_inode truncate on inode "
                     << *in <<
                     " hits a hole!"
                     << dendl;
            length +=
...
struct C_MDC_TruncateLogged:                                      public
    MDCacheLogContext {
    CInode *in;
    MutationRef mut;
...
class C_MDS_purge_completed_finish:                               public
    MDCacheLogContext
{
 ...
 ls)
{
    ...
    ls](int r) {
        ...
        ceph_assert(piv != 0);
        mds->mdlog->start_submit_entry(new EPurged(inos, ls->seq, piv), new C_MDS_purge_completed_finish(this, inos, ls, piv));
        mds->mdlog->flush();
    });
     ...
     count)
{
    ...
    for (auto rank : stopping) {
        ...
        if (!aborted) {
            ...
            dout(20) <<
                     __func__ <<
                     ": some unexpirable contents in mdsdir"
                     << dendl;
        }
    }
 ...
 expiremap)
{
    ...
    if (con)
        ...
        dout(12) <<
                 " no container; under a not-yet-linked dir"
                 << dendl;
        ceph_assert
        (dn->
         is_auth());
    ...
    for (set <
        ...
        if (bd->get_dir_auth().first != mds->get_nodeid() && // we are not auth
            ...
            CInode *bi =
                ...
                get_inode();
            dout(10) <<
                     " closing empty non-auth child subtree "
                     << *bd <<
                     dendl;
            remove_subtree
            (bd);
        ...
        while (true) {
            ...
            if (diri->
                ...
                if (!diri->
                    ...
                    ()) {
                    dout(10) <<
                             " closing empty non-auth subtree "
                             << *dir <<
                             dendl;
                    remove_subtree
                    (dir);
 ...
 > &m)
{
    ...
    for (const auto &p :                                            m->realms) {
        ...
        if (p.first.
             ...
             (export_state
              ...
              ((export_state == Migrator::EXPORT_WARNING && migrator->export_has_warned(parent_dir, from)) ||
               ...
               (export_state == Migrator::EXPORT_NOTIFYING && !migrator->export_has_notified(parent_dir, from))))) {
                ...
                // not auth.
                dout(7) <<
                        "delaying nonauth|warned expires for "
                        <<
                        *parent_dir
                        << dendl;
                ceph_assert
            ...
            dout(7) <<
                    "containerless expires (root, stray inodes)"
                    << dendl;
        }
            ...
            if (!dir) {
                ...
                diri =
                    get_inode(q.
                              ...
                              ino);
                if (diri) {
                    if (mds->is_rejoin() && rejoin_ack_gather.count(mds->get_nodeid()) &&    // haven't sent rejoin ack yet
                        !diri->
                        is_replica
                        (from)) {
                        auto && ls = diri->get_nested_dirfrags();
                        dout(7) << " dir expire on dirfrag " << q.first << " from mds." << from << " while rejoining, inode isn't replicated" <<
                                dendl;
                        for (const auto &d : ls) {
                            dir = d;
                            if (dir->
...
void
MDCache::
discard_delayed_expire
(CDir *
 dir)
{
    dout(7) <<
            "discard_delayed_expire on "
            << *dir <<
            dendl;
    delayed_expire.
    erase(dir);
...
class C_MDC_ShutdownCheck:                                        public
    MDCacheContext
{
public:
...
()
{
    ...
    if (mds->
        ...
        () > 1) {
        dout(7) <<
                "still >1 segments, waiting for log to trim"
                << dendl;
        return
            false;
    }
    ...
    if ((myin
        ...
        || (mydir
            ...
            (mydir->
             ...
             ()))) {
        dout(7) <<
                "still have auth pinned objects"
                << dendl;
        return
            false;
    }
    ...
    if
    (!did_shutdown_log_cap) {
        // flush journal header
        dout(7) <<
                "writing header for (now-empty) journal"
                << dendl;
        ceph_assert
        (mds->mdlog->
         empty());
    ...
    if (lru.
        ...
        () > 0) {
        dout(7) <<
                "there's still stuff in the cache: "
                << lru.
                lru_get_size
                () << "/" <<
                bottom_lru.
    ...
    if (mydir) {
        if (mydir->get_num_ref() > 1) {  // subtree pin
            dout(7) <<
                    "there's still reference to mydir "
                    << *mydir <<
                    dendl;
            show_cache();
            return
 ...
 pin)
{
    ...
    for (unsigned
        ...
        if (mds->
            ...
            (curdir,
             ...
             snapid)) {
            dout(4) <<
                    "traverse: stopped lookup at damaged dentry "
                    << *curdir <<
                    "/" <<
                    path[depth]
                    << " snap="
                ...
                if (in) {
                    ...
                    if (mds->
                        ...
                        (dnl->
                         ...
                         ())) {
                        dout(4) <<
                                "traverse: remote dentry points to damaged ino "
                                << *dn <<
                                dendl;
                        return
                            -CEPHFS_EIO;
        ...
        if (curdir->
            ...
            if (curdir->
                ...
                if (pdnvec) {
                    ...
                    if (depth <
                        ...
                        1) {
                        dout(20) <<
                                 " didn't traverse full path; not returning pdnvec"
                                 << dendl;
                    } else if (snapid <
                               CEPH_MAXSNAP) {
                        dout(20) <<
                                 " not adding null for snapid "
                                 << snapid <<
                                 dendl;
                    } else if (curdir->
                               is_frozen
                ...
                dout(7) <<
                        "traverse: incomplete dir contents for "
                        << *cur <<
                        ", fetching"
                        << dendl;
                touch_inode
            ...
            if (forward
                ...
                dout(7) <<
                        ...
                        get_num_fwd()
                        <<
                        ", discovering instead of forwarding"
                        << dendl;
                discover =
                    true;
            }
    ...
    if (mds->
        ...
        (l_mds_traverse_hit);
    dout(10) <<
             "path_traverse finish on snapid "
             << snapid <<
             dendl;
    if (mdr)
        ceph_assert
 ...
 projected)
{
    ...
    if (in) {
        dout(7) <<
                "get_dentry_inode linking in remote in "
                << *in <<
                dendl;
        dn->
        link_remote
        ...
        dout(10) <<
                 "get_dentry_inode on remote dn, opening inode for "
                 << *dn <<
                 dendl;
        open_remote_dentry
        (dn,
...
struct C_MDC_OpenRemoteDentry:                                    public
    MDCacheContext {
    CDentry *dn;
    inodeno_t ino;
    MDSContext *onfinish;
 ...
 want_xlocked)
{
    ...
    open_ino(ino, pool, new C_MDC_OpenRemoteDentry(this, dn, ino, fin, want_xlocked), true, want_xlocked);  // backtrace
}
 ...
 int r)
{
    if (r < 0) {
        ...
        if (dnl->
            ...
            ino) {
            dout(0) <<
                    "open_remote_dentry_finish bad remote dentry "
                    << *dn <<
                    dendl;
            dn->
            state_set
...
class C_IO_MDC_OpenInoBacktraceFetched:                           public
    MDCacheIOContext
{
    inodeno_t ino;
public:
...
struct C_MDC_OpenInoTraverseDir:                               public
    MDCacheContext {
    inodeno_t ino;
    cref_t < MMDSOpenIno > msg;
    bool parent;
public:
    C_MDC_OpenInoTraverseDir(MDCache *c, inodeno_t i, const cref_t < MMDSOpenIno > &m, bool p): MDCacheContext(c), ino(i),
        msg(m), parent(p)
    {
    }
    void finish(int r)
    override
    {
        ...
        auto &info =
            ...
            at(ino);
        mdcache->
        _open_ino_traverse_dir
        (ino, info,
         r);
    }
};
struct C_MDC_OpenInoParentOpened:                              public
    MDCacheContext {
    inodeno_t ino;
public:
 ...
 & info)
{
    ...
    if (peer < 0) {
        ...
        if (all !=
            ...
            checked) {
            dout(10) <<
                     " waiting for more peers to be active"
                     << dendl;
        } else {
            dout(10) <<
                     " all MDS peers have been checked "
                     << dendl;
            do_open_ino
            (ino, info,
             0);
 ...
 & fip)
{
    ...
    if (m ==
        ...
        if (all !=
            ...
            checked) {
            dout(10) <<
                     "_do_find_ino_peer waiting for more peers to be active"
                     << dendl;
        } else {
            dout(10) <<
                     "_do_find_ino_peer failed on "
                     << fip.
                     ino << dendl;
            fip.fin->
            complete
    ...
    if (p !=
        ...
        if (get_inode
            (fip.
             ino)) {
            dout(10) <<
                     "handle_find_ino_reply successfully found "
                     << fip.
                     ino << dendl;
            mds->
            queue_waiter
        ...
        if (!m->path.
            ...
            dout(0) <<
                    "handle_find_ino_reply failed with "
                    << r <<
                    " on " << m->
                    path <<
                    ", retrying"
    ...
    (active_requests.
     count(req->
        ...
        if (mdr->
            ...
            dout(10) <<
                     "request_start already processing "
                     << *mdr <<
                     ", dropping new msg"
                     << dendl;
        }
    ...
    if (mdr->
        ...
        if (mds->
            ...
            (l_mds_forward);
               ...
               >= 0) {
        dout(10) <<
                 "request_forward on internal op; cancelling"
                 << dendl;
        mdr->
        internal_op_finish->
        complete
 ...
 & mdr)
{
    ...
    for (auto it
        ...
        if (it->
            ...
            ()) {
            dout(10) <<
                     "request_drop_foreign_locks forgetting lock "
                     << *lock <<
                     " on " <<
                     lock->
                     get_parent()
            ...
            erase(it++);
                   ...
                   ()) {
            dout(10) <<
                     "request_drop_foreign_locks forgetting remote_wrlock "
                     << *lock <<
                     " on mds." <<
                     it->
                     wrlock_target
    ...
    if (mdr->
        ...
        if (!
            ...
            dout(10) <<
                     "request_kill "
                     << *mdr <<
                     " -- waiting for peer reply, delaying"
                     << dendl;
        } else {
            dout(10) <<
                     "request_kill "
                     << *mdr <<
                     " -- already started peer prep, no-op"
                     << dendl;
        }
...
struct C_MDC_RetryScanStray:                                      public
    MDCacheContext {
    dirfrag_t next;
...
struct C_MDC_RetryDiscoverPath:                                   public
    MDCacheContext {
    CInode *base;
    snapid_t snapid;
    filepath path;
...
struct C_MDC_RetryDiscoverPath2:                                  public
    MDCacheContext {
    CDir *base;
    snapid_t snapid;
    filepath path;
 ...
 path_locked)
{
    ...
    if ((path_locked && want_path.depth() == 1) || !base->is_waiting_for_dentry(want_path[0].c_str(), snap) || !onfinish) {
        discover_info_t
        & d =
            _create_discover
            (from);
 ...
 &dis)
{
    ...
    if (mds->
        ...
        if (!mds->
            ...
            (from)) {
            dout(0) <<
                    "discover_reply not yet active(|still rejoining), delaying"
                    << dendl;
            mds->
            wait_for_replay
            (new
    ...
    for (unsigned
        ...
        if ((!curdir
            ...
            if (reply->
                ...
                if (curdir) {
                    dout(7) <<
                            " not dirfrag auth, setting dir_auth_hint for "
                            << *curdir <<
                            dendl;
                    reply->
                    set_dir_auth_hint
        ...
        if (!curdir) {   // open dir?
            if (cur->
                ...
                if (!reply->
                    ...
                    dout(7) <<
                            *cur <<
                            " is frozen, non-empty reply, stopping"
                            << dendl;
                    break;
                }
                dout(7) <<
                        *cur <<
                        " is frozen, empty reply, waiting"
                        << dendl;
                cur->
                add_waiter
                (CInode::
            ...
            if (dis->
                ...
                dout(7) <<
                        *curdir <<
                        " is frozen, dirfrag mismatch, stopping"
                        << dendl;
                reply->
                set_flag_error_dir
                ();
        ...
        if (curdir->
            ...
            // fetch newly opened dir
                   ...
                   ()) {
            dout(7) <<
                    "handle_discover not adding unwanted base dir "
                    << *curdir <<
                    dendl;
        ...
        if (dn->lock.
            ...
            if (dis->
                ...
                ()) {
                dout(7) <<
                        "handle_discover allowing discovery of xlocked "
                        << *dn <<
                        dendl;
            } else if (reply->
                       is_empty
                       ()) {
                dout(7) <<
                        "handle_discover blocking on xlocked "
                        << *dn <<
                        dendl;
                dn->lock.
                add_waiter
                ...
                dout(7) <<
                        "handle_discover non-empty reply, xlocked tail "
                        << *dn <<
                        dendl;
                break;
            }
        ...
        if (dnl->
            ...
            if (tailitem
                ...
                ()) {
                dout(7) <<
                        "handle_discover allowing discovery of frozen tail "
                        << *dnl->
                        get_inode()
                        << dendl;
            } else if (reply->
    ...
    (!reply->
     is_empty());
    dout(7) <<
            "handle_discover sending result back to asker mds."
            << from <<
            dendl;
    mds->
    send_message
    ...
    if (m->
        ...
        if (m->
            ...
            if (cur->
                ...
                dout(7) <<
                        " doing nothing, nobody is waiting for dir"
                        << dendl;
        }
        ...
        if (m->
            ...
            if (dir
                ...
                dout(7) <<
                        " doing nothing, have dir but nobody is waiting on dentry "
                        << m->
                        get_error_dentry
                        () << dendl;
        }
 ...
 features)
{
    ...
    (bl,
     features);
    in->
    _encode_locks_state_for_replica
    (bl,
     mds->
     get_state()
     <
 ...
 finished)
{
    ...
    if (!in) {
        ...
        (p);
        in->
        _decode_locks_state_for_replica
        (p, true);
        add_inode
        (in);
        if (in->
    ...
    if (struct_v
        ...
        s &=
            CInode::
            MASK_STATE_REPLICATED;
        if (s &
            CInode::
            STATE_RANDEPHEMERALPIN) {
            dout(10) <<
                     "replica inode is random ephemeral pinned"
                     << dendl;
            in->
            set_ephemeral_pin
            (false,
        ...
        if (m->
            ...
            filepath path
                =
                    ...
                    get_path();
            dout(5) <<
                    "trying discover on dir_update for "
                    << path <<
                    dendl;
            logger->
            inc
 ...
 bool replay)
{
    ...
    if (!dir) {
        ...
        if (src.
            empty()) {
            dout(10) <<
                     "force_dir_fragment no frags under "
                     << fg <<
                     dendl;
        } else {
            dout(10) <<
        ...
        (any_subtree) {
            ...
            for (const auto &dir :                                  srcfrags) {
                ...
                (dir->
                 ...
                 ());
                dout(10) <<
                         " taking srcfrag subtree bounds from "
                         << *dir <<
                         dendl;
                map < CDir *,
                    set <
...
class C_MDC_FragmentFrozen:                                       public
    MDSInternalContext
{
    MDCache *mdcache;
    MDRequestRef mdr;
 ...
 > &dirs)
{
    ...
    (is_readonly
     ()) {
        dout(7) <<
                "can_fragment: read-only FS, no fragmenting for now"
                << dendl;
        return
            false;
    }
    if (mds->
        ...
        ()) {
        dout(7) <<
                "can_fragment: cluster degraded, no fragmenting for now"
                << dendl;
        return
            false;
    }
    if (diri->
        ...
        ()) {
        dout(7) <<
                "can_fragment: i won't merge|split anything in stray"
                << dendl;
        return
            false;
    }
    if (diri->
        ...
        CEPH_INO_CEPH) {
        dout(7) <<
                "can_fragment: i won't fragment mdsdir or .ceph"
                << dendl;
        return
            false;
    }
    ...
    for (const auto &dir :                                          dirs) {
        if (dir->
            ...
            ()) {
            dout(7) <<
                    "can_fragment: scrub in progress "
                    << *dir <<
                    dendl;
            return
                false;
        ...
        if (dir->
            ...
            (CDir::
             STATE_FRAGMENTING)) {
            dout(7) <<
                    "can_fragment: already fragmenting "
                    << *dir <<
                    dendl;
            return
                false;
        ...
        if (dir->
            ...
            ()) {
            dout(7) <<
                    "can_fragment: can't merge, freezing|frozen.  wait for other exports to finish first."
                    << dendl;
            return
                false;
        }
 ...
 int bits)
{
    ...
    (!can_fragment
     (diri,
      dirs)) {
        dout(7) <<
                __func__ <<
                " cannot fragment right now, dropping"
                << dendl;
        return;
    }
    ...
    last_cum_auth_pins_change
        =
            ceph_clock_now
            ();
    fragment_freeze_dirs
    (dirs);
    // initial mark+complete pass
    fragment_mark_and_complete
    (mdr);
}
...
class C_MDC_FragmentMarking:                                      public
    MDCacheContext
{
    MDRequestRef mdr;
public:
        ...
        if (!dir->
            ...
            if (dir->
                ...
                (CDir::
                 STATE_CREATING)) {
                dout(15) <<
                         " waiting until new dir gets journaled "
                         << *dir <<
                         dendl;
                dir->
                add_waiter
...
void
MDCache::
fragment_freeze_inc_num_waiters
(CDir *
 dir)
{
    map <
...
class C_MDC_FragmentPrep:                                         public
    MDCacheLogContext
{
    MDRequestRef mdr;
public:
...
class C_MDC_FragmentStore:                                     public
    MDCacheContext
{
    MDRequestRef mdr;
public:
...
class C_MDC_FragmentCommit:                                    public
    MDCacheLogContext
{
    dirfrag_t basedirfrag;
    MDRequestRef mdr;
public:
    C_MDC_FragmentCommit(MDCache *m, dirfrag_t df, const MDRequestRef &r): MDCacheLogContext(m), basedirfrag(df), mdr(r)
    {
    }
    void finish(int r)
    override
...
class C_IO_MDC_FragmentPurgeOld:                               public
    MDCacheIOContext
{
    dirfrag_t basedirfrag;
    int bits;
    MDRequestRef mdr;
public:
    C_IO_MDC_FragmentPurgeOld(MDCache *m, dirfrag_t f, int b, const MDRequestRef &r): MDCacheIOContext(m), basedirfrag(f),
        bits(b), mdr(r)
    {
    }
    void finish(int r)
 ...
 & mdr)
{
    ...
    uf =
        ...
        (basedirfrag);
    // remove old frags
    C_GatherBuilder gather(g_ceph_context, new C_OnFinisher(new C_IO_MDC_FragmentPurgeOld(this, basedirfrag, uf.bits, mdr),
                           mds->finisher));
    SnapContext nullsnapc;
 ...
 > &ack)
{
    ...
    if (it ==
        ...
        ()) {
        dout(10) <<
                 "handle_fragment_notify_ack obsolete message, dropping"
                 << dendl;
        return;
    }
...
{
    dout(10) <<
             "add_uncommitted_fragment: base dirfrag "
             <<
             basedirfrag
             << " bits "
             << bits <<
...
{
    dout(10) <<
             "finish_uncommitted_fragments: base dirfrag "
             <<
             basedirfrag
             << " op " <<
             EFragment::
...
{
    dout(10) <<
             "rollback_uncommitted_fragment: base dirfrag "
             <<
             basedirfrag
             <<
             " old_frags ("
...
void
MDCache::
wait_for_uncommitted_fragments
(MDSContext *
 finisher)
{
...
struct C_MDC_FragmentRollback:                                    public
    MDCacheLogContext {
    MutationRef mut;
...
class C_MDS_EnqueueScrub:                                         public
    Context
{
    std::string tag;
    Formatter *formatter;
...
struct C_MDC_RespondInternalRequest:                              public
    MDCacheLogContext {
    MDRequestRef mdr;
...
struct C_MDC_ScrubRepaired:                                    public
    MDCacheContext {
    ScrubHeaderRef header;
public:
...
public:
    C_FinishIOMDR(MDSRank *mds_, MDRequestRef &mdr_):             mds(mds_),
        mdr(mdr_)
    {
    }
    void finish(int r)
 ...
 & oldmap)
{
    ...
    if (mdsmap.
        ...
        ()) {
        dout(10) <<
                 "Checking ephemerally pinned directories for redistribute due to max_mds change."
                 << dendl;
        /* copy to vector to avoid removals during iteration */
        std::vector < CInode * >migrate;
    ...
    if (max_mds
        <= 1) {
        export_ephemeral_dist_frag_bits
            = 0;
    } else {
        double want =
            g_conf().
        ...
        if (since >=
            ...
            * .90) {
            ...
            if (mds->
                ...
                ()) {
                dout(20) <<
                         "upkeep thread trimming cache; last trim "
                         << since <<
                         " ago" <<
                         dendl;
                bool
        ...
        auto interval
            =
                ...
                (release_interval,
                 trim_interval);
        dout(20) <<
                 "upkeep thread waiting interval "
                 << interval
                 << dendl;
        upkeep_cvar.
        wait_for
