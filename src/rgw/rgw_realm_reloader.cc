...
RGWRealmReloader::RGWRealmReloader(RGWRados * &store, std::map < std::string,
                                   std::string > &service_map_meta,
                                   Pauser * frontends)
:  store(store), service_map_meta(service_map_meta), frontends(frontends),
timer(store->ctx(), mutex, USE_SAFE_TIMER_CALLBACKS), mutex("RGWRealmReloader"),
...
void RGWRealmReloader::reload()
{
    ...
    if (r < 0) {
        lderr(cct) << "ERROR: failed to register to service map: " <<
