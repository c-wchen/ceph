...
MetricAggregator::MetricAggregator(CephContext * cct, MDSRank * mds,
                                   MgrClient * mgrc)
:  Dispatcher(cct), mds(mds), mgrc(mgrc), mds_pinger(mds)
{
}

void MetricAggregator::ping_all_active_ranks()
{
    dout(10) << ": pinging " << active_rank_addrs.
...
int MetricAggregator::init()
{
    ...
    pinger = std::thread([this] () {
                         ...
                         ("mds_ping_interval").count(); sleep(timo);
                         locker.lock();}
                         }) ;

    mgrc->set_perf_metric_query_cb([this] (const ConfigPayload & config_payload) {
                                   set_perf_queries(config_payload);},[this] () {
...
bool MetricAggregator::ms_can_fast_dispatch2(const cref_t < Message > &m) const const
{
                                                ...
                                                const Metrics & metrics)
{
    ...
    auto update_counter_func =
        [&metrics] (const MDSPerformanceCounterDescriptor & d,
...
void MetricAggregator::handle_mds_metrics(const cref_t < MMDSMetrics > &m)
{
    ...
    dout(20) << ": applying " << client_metrics_map.
        size() << " updates for rank=" << rank << " with sequence number " <<
        seq << dendl;
...
void MetricAggregator::notify_mdsmap(const MDSMap & mdsmap)
{
  ...
  for (auto & rank:diff) {
        ...
        dout(10) << ": active rank=" << rank << " (mds." << mdsmap.
            get_mds_info(rank).name << ") has addr=" << rank_addr << dendl;
...
void MetricAggregator::set_perf_queries(const ConfigPayload & config_payload)
{
    const MDSConfigPayload & mds_config_payload =
        boost::get < MDSConfigPayload > (config_payload);
    ...
    std::scoped_lock locker(lock);
    std::map < MDSPerfMetricQuery, std::map < MDSPerfMetricKey,
...
MetricPayload MetricAggregator::get_perf_reports()
{
  ...
  for (auto &[query, counters]:query_metrics_map) {
      ...
      for (auto & p:counters) {
            dout(20) << ": packing perf_metric_key=" << p.
                first << ", perf_counter=" << p.second << dendl;
