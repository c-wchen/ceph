...
struct SnapSetContext {
    ...
    explicit SnapSetContext(const hobject_t & o):oid(o), ref(0),
        registered(false), exists(true) {
}};
  ...
  public:
    ...
    struct RWState {
      ...
      :    
        count(0),
        state(RWNONE),
        recovery_read_marker(false), snaptrimmer_write_marker(false) {
        }
    ...
    void put_lock_type(ObjectContext::RWState::State type,
                       list < OpRequestRef > *to_wake,
                       bool * requeue_recovery, bool * requeue_snaptrimmer) {
        switch (type) {
...
class ObcLockManager {
    struct ObjectLockState {
        ...
        ObjectLockState(ObjectContextRef obc,
                        ObjectContext::RWState::State type)
        :obc(obc), type(type) {
    }};
  ...
  public:
    ...
    bool empty() const {
        return locks.empty();
    } bool get_lock_type(ObjectContext::RWState::State type,
    ...
    bool take_write_lock(const hobject_t & hoid, ObjectContextRef obc) {
        ...
        if (obc->rwstate.take_write_lock()) {
            locks.
                insert(make_pair
                       (hoid,
                        ObjectLockState(obc, ObjectContext::RWState::RWWRITE)));
            return true;
        }
        else {
            return false;
    ...
    bool get_snaptrimmer_write(const hobject_t & hoid,
                               ObjectContextRef obc,
    ...
    bool get_write_greedy(const hobject_t & hoid,
                          ObjectContextRef obc, OpRequestRef op) {
        assert(locks.find(hoid) == locks.end());
    ...
    void put_locks(list < pair < hobject_t,
                   list < OpRequestRef > > >*to_requeue,
                   bool * requeue_recovery, bool * requeue_snaptrimmer) {
      for (auto p:locks) {
            list < OpRequestRef > _to_requeue;
            p.second.obc->put_lock_type(p.second.type,
                                        &_to_requeue,
                                        requeue_recovery, requeue_snaptrimmer);
            if (to_requeue) {
                to_requeue->push_back(make_pair(p.second.obc->obs.oi.soid,
