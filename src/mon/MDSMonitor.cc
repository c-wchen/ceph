...
void MDSMonitor::get_store_prefixes(std::set < string > &s) const const
{
...
void MDSMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  for (auto & p:pending.filesystems) {
    ...
    for (std::map < uint64_t, MDSHealth >::iterator i =
         pending_daemon_health.begin(); i != pending_daemon_health.end(); ++i) {
        bufferlist bl;
        i->second.encode(bl);
  ...
  for (const auto & i:info_map) {
      ...
      for (const auto & metric:health.metrics) {
            ...
            health_check_t *check =
                &new_checks.get_or_add(mds_metric_name(metric.type),
                                       metric.sev,
                                       mds_metric_summary(metric.type),
                                       1);
            ostringstream ss;
            ss << "mds." << info.name << "(mds." << rank << "): " << metric.
                message;
            bool first = true;
  ...
  for (auto & p:new_checks.checks) {
        p.second.summary = std::regex_replace(p.second.summary,
                                              std::regex("%num%"),
                                              stringify(p.second.detail.
                                                        size()));
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%plurals%"),
                               p.second.detail.size() > 1 ? "s" : "");
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%isorare%"),
                               p.second.detail.size() > 1 ? "are" : "is");
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%hasorhave%"),
                               p.second.detail.size() > 1 ? "have" : "has");
    }
    encode_health(new_checks, t);
}

version_t MDSMonitor::get_trim_to() const const
{
    version_t floor = 0;
...
bool MDSMonitor::preprocess_beacon(MonOpRequestRef op)
{
    ...
    if (!session->is_capable("mds", MON_CAP_X)) {
        dout(0) <<
            "preprocess_beacon got MMDSBeacon from entity with insufficient privileges "
            << session->caps << dendl;
        goto ignore;
    }

    if (m->get_fsid() != mon.monmap->fsid) {
        dout(0) << "preprocess_beacon on fsid " << m->
            get_fsid() << " != " << mon.monmap->fsid << dendl;
        goto ignore;
    }
    ...
    if (!fsmap.gid_exists(gid)) {
        if (state != MDSMap::STATE_BOOT) {
             ...
             */
            auto m =
                make_message < MMDSMap > (mon.monmap->fsid,
    ...
    if (!(pending_daemon_health[gid] == m->get_health())) {
        dout(10) << __func__ << " health metrics for gid " << gid <<
            " were updated" << dendl;
        _note_beacon(m);
        return false;
    }
...
bool MDSMonitor::preprocess_offload_targets(MonOpRequestRef op)
{
    ...
    dout(10) << "preprocess_offload_targets " << *m << " from " << m->
    ...
    if (!session->is_capable("mds", MON_CAP_X)) {
        dout(0) <<
            "preprocess_offload_targets got MMDSLoadTargets from entity with insufficient caps "
            << session->caps << dendl;
        goto ignore;
    }
...
bool MDSMonitor::prepare_beacon(MonOpRequestRef op)
{
    ...
    dout(15) << __func__ << " got health from gid " << gid << " with " << m->
        get_health().metrics.size() << " metrics." << dendl;
  ...
  for (const auto & old_metric:old_health) {
    ...
    if (state == MDSMap::STATE_BOOT) {
        ...
        if (g_conf()->mds_enforce_unique_name) {
            ...
            while (mds_gid_t existing =
                   pending.find_mds_gid_by_name(m->get_name())) {
                if (!mon.osdmon()->is_writeable()) {
                    mon.osdmon()->wait_for_writeable(op,
                                                     new C_RetryMessage(this,
                                                                        op));
                    return false;
                }
        ...
        if (info.laggy()) {
            dout(1) << "prepare_beacon clearing laggy flag on " << addrs <<
                dendl;
        ...
        if (state == MDSMap::STATE_STOPPED) {
          ...
          for (const auto & erased_gid:erased) {
            ...
            // won't try to run it.
            dout(0) << __func__ << ": marking rank " << rank << " damaged" <<
                dendl;
            ...
            if (info.state != MDSMap::STATE_ACTIVE
            ...
            pending.modify_daemon(gid,[state, seq] (auto & info) {
                                  info.state = state; info.state_seq = seq;});
        }
    }
  ...
  evict:
    if (!mon.osdmon()->is_writeable()) {
        dout(1) << __func__ << ": waiting for writeable OSDMap to evict" <<
            dendl;
...
bool MDSMonitor::prepare_offload_targets(MonOpRequestRef op)
{
    ...
    if (pending.gid_has_rank(gid)) {
        dout(10) << "prepare_offload_targets " << gid << " " << m->
            targets << dendl;
        ...
        if (all) {
          ...
          for (const auto & i:all_info) {
                ...
                if (r == -EINVAL || r == -ENOENT) {
                    ...
                    derr << "Unexpected error reading metadata: " <<
                        cpp_strerror(r)
                        << dendl;
                    ss << get_err.str();
                    f->close_section();
        ...
        if (f) {
          ...
          for (const auto & p:fsmap.filesystems) {
                ...
                f->open_object_section("filesystem");
                {
                    ...
                    f->dump_string("metadata_pool",
                                   mon.osdmon()->osdmap.get_pool_name(mds_map.
                                                                      metadata_pool));
                  ...
                  for (const auto & id:mds_map.data_pools) {
                        const auto & name =
                            mon.osdmon()->osdmap.get_pool_name(id);
...
bool MDSMonitor::fail_mds_gid(FSMap & fsmap, mds_gid_t gid)
{
    ...
    dout(1) << "fail_mds_gid " << gid << " mds." << info.
        name << " role " << info.rank << dendl;
...
mds_gid_t MDSMonitor::gid_from_arg(const FSMap & fsmap, const std::string & arg,
                                   std::ostream & ss)
{
...
bool MDSMonitor::prepare_command(MonOpRequestRef op)
{
  ...
  for (const auto & h:handlers) {
        ...
        if (r == -EAGAIN) {
            ...
            dout(4) << __func__ << " enqueue for retry by prepare_command" <<
                dendl;
            return false;
        }
        else {
    ...
    if (r >= 0) {
        ...
        dout(4) << __func__ << " enqueue for retry by filesystem_command" <<
            dendl;
        return false;
    }
    else if (r != -ENOSYS) {
...
int MDSMonitor::filesystem_command(FSMap & fsmap,
                                   MonOpRequestRef op,
                                   ...
                                   std::stringstream & ss)
{
    ...
    if (prefix == "mds set_state") {
        ...
        if (r < 0 && r == -EAGAIN) {
            ...
            if (failed_info.global_id != MDS_GID_NONE) {
                mon.clog->info() << failed_info.
                    human_name() << " marked failed by " << op->get_session()->
                    entity_name;
            }
        }
    }
        ...
        string_view fs_name =
            fsmap.get_filesystem(role.fscid)->mds_map.get_fs_name();
        if (!op->get_session()->fs_name_capable(fs_name, MON_CAP_W)) {
            ss << "Permission denied.";
            return -EPERM;
        }
        ...
        if (fsmap.default_compat.compat.contains(f)) {
            ...
            ss << "compat feature " << f << " not present in " << fsmap.
                default_compat;
        }
        r = 0;
    }
        ...
        if (fsmap.default_compat.incompat.contains(f)) {
            ...
            ss << "incompat feature " << f << " not present in " << fsmap.
                default_compat;
        }
        r = 0;
    }
        ...
        auto f =[freeze, gid, &ss] (auto & info) {
            if (freeze) {
                         ss << "freezing mds." << gid; info.freeze();}
                         else {
                         ss << "unfreezing mds." << gid; info.unfreeze();}
                         }; fsmap.modify_daemon(gid, f); r = 0;}
                         else {
                         return -ENOSYS;}
  ...
  "fsmap", "fsmap.user", "mdsmap",}; for (const auto & p:get_fsmap().filesystems)
                         {
  ...
  for (const auto & type:types) {
                         ...
                         while (!sub_it.end()) {
                         auto sub = *sub_it; ++sub_it;  // N.B. check_sub may remove sub!
                         check_sub(sub);}
                         }
                         }
                         ...
                         // to use const qualifier filter fsmap beforehand
                         FSMap _fsmap_copy = get_fsmap();
                         _fsmap_copy.filter(sub->session->
                         ...
                         if (sub->type == "fsmap") {
                         sub->session->con->
                         send_message(new MFSMap(mon.monmap->fsid, fsmap));
                         if (sub->onetime) {
                         ...
                         if (sub->type == "fsmap.user") {
  FSMapUser fsmap_u; fsmap_u.epoch = fsmap.get_epoch(); fsmap_u.legacy_client_fscid = fsmap.legacy_client_fscid; for (const auto & p:fsmap.filesystems)
                         {
                         FSMapUser::fs_info_t & fs_info =
                         fsmap_u.filesystems[p.second->fscid];
                         fs_info.cid = p.second->fscid;
                         fs_info.name = p.second->mds_map.fs_name;}
                         sub->session->con->
                         send_message(new
                                      MFSMapUser(mon.monmap->fsid, fsmap_u));
                         if (sub->onetime) {
                         mon.session_map.remove_sub(sub);}
                         else {
                         sub->next = fsmap.get_epoch() + 1;}
  ...
  auto mds_info = fsmap.get_mds_info(); for (const auto & p:mds_info)
                         ...
                         auto namespace_id_str =
                         sub->type.substr(std::string("mdsmap.").size());
                         dout(10) << __func__ << ": namespace_id " <<
                         namespace_id_str << dendl; std::string err;
                         fscid =
                         strict_strtoll(namespace_id_str.c_str(), 10, &err);
                         if (!err.empty()) {
                         ...
                         dout(1) <<
                         "Client subscribed for legacy filesystem but "
                         "none is configured" << dendl; return;}
                         }
                         ...
                         // flag to MMDSMap?
                         dout(1) <<
                         "Client subscribed to non-existent namespace '" <<
                         fscid << "'" << dendl; return;}
                         }
                         dout(10) << __func__ << ": is_mds=" << is_mds <<
                         ", fscid=" << fscid << dendl;
                         ...
                         mds_map->
                         epoch << " for namespace " << fscid <<
                         " for subscriber " << sub->session->
                         name << " who wants epoch " << sub->next << dendl;
                         if (sub->next > mds_map->epoch) {
                         return;}
                         auto msg =
                         make_message < MMDSMap > (mon.monmap->fsid, *mds_map);
                         ...
                         void MDSMonitor::update_metadata(mds_gid_t gid,
                                                          const map < string,
                                                          string > &metadata) {
                         dout(20) << __func__ << ": mds." << gid << ": " <<
                         metadata << dendl; if (metadata.empty()) {
                         dout(5) << __func__ << ": mds." << gid <<
                         ": no metadata!" << dendl; return;}
                         pending_metadata[gid] = metadata;
                         MonitorDBStore::TransactionRef t =
                         paxos.get_pending_transaction(); bufferlist bl;
                         encode(pending_metadata, bl);
                         t->put(MDS_METADATA_PREFIX, "last_metadata", bl);}

                         void MDSMonitor::
                         remove_from_metadata(const FSMap & fsmap,
                                              MonitorDBStore::
                                              TransactionRef t) {
                         bool update = false;
                         for (auto it = pending_metadata.begin();
                              it != pending_metadata.end();) {
                         if (!fsmap.gid_exists(it->first)) {
                         it = pending_metadata.erase(it); update = true;}
                         else {
                         ++it;}
                         }
                         if (!update)
                         ...
                         int MDSMonitor::load_metadata(map < mds_gid_t,
                                                       Metadata > &m) {
                         bufferlist bl;
                         int r =
                         mon.store->get(MDS_METADATA_PREFIX, "last_metadata",
                                        bl); if (r) {
                         dout(5) << "Unable to load 'last_metadata'" << dendl;
                         return r;}

                         auto it = bl.cbegin(); ceph::decode(m, it); return 0;}

                         void MDSMonitor::count_metadata(const std::
                                                         string & field,
                                                         map < string,
                                                         int >*out) {
  map < mds_gid_t, Metadata > meta; load_metadata(meta); for (auto & p:meta)
                         {
                         auto q = p.second.find(field); if (q == p.second.end()) {
                         (*out)["unknown"]++;}
                         else {
                         (*out)[q->second]++;}
                         }
                         ...
                         void MDSMonitor::count_metadata(const std::
                                                         string & field,
                                                         Formatter * f) {
  map < string, int >by_val; count_metadata(field, &by_val); f->open_object_section(field.c_str()); for (auto & p:by_val)
                         {
                         f->dump_int(p.first.c_str(), p.second);}
                         f->close_section();}

                         void MDSMonitor::get_versions(std::map < string,
                                                       list < string >
                                                       >&versions) {
  map < mds_gid_t, Metadata > meta; load_metadata(meta); const auto & fsmap = get_fsmap(); std::map < mds_gid_t, mds_info_t > map = fsmap.get_mds_info(); dout(10) << __func__ << " mds meta=" << meta << dendl; for (auto & p:meta)
                         {
                         auto q = p.second.find("ceph_version_short");
                         if (q == p.second.end())continue;
                         versions[q->second].push_back(string("mds.") +
                                                       map[p.first].name);}
                         }

                         int MDSMonitor::dump_metadata(const FSMap & fsmap,
                         ...
                         err << "Unable to load 'last_metadata'"; return r;}

                         if (!metadata.count(gid)) {
                         return -ENOENT;}
                         const Metadata & m = metadata[gid];
                         for (Metadata::const_iterator p = m.begin();
                              p != m.end(); ++p) {
  ...
  for (const auto & p:  metadata) {
                         ...
                         continue;}
                         if (!fsmap.gid_exists(gid)) {
                         dout(5) << __func__ << ": GID " << gid <<
                         " not existent" << dendl; continue;}
                         const MDSMap::mds_info_t & mds_info =
                         fsmap.get_info_gid(gid);
 ...
 */
                         bool MDSMonitor::maybe_resize_cluster(FSMap & fsmap,
                                                               fs_cluster_id_t
                                                               fscid) {
                         ...
                         dout(20) << __func__ << " in " << in << " max " << max
                         << dendl;
                         ...
                         if (!get_fsmap().filesystem_exists(fscid) ||
                             !get_fsmap().get_filesystem(fscid)->mds_map.
                             is_resizeable() || !mds_map.is_resizeable()) {
                         dout(5) << __func__ <<
                         ...
                         mds++;}
                         auto info = fsmap.find_replacement_for( {
                                                                fscid, mds}
                         ); if (!info) {
                         return false;}
                         ...
                         mds_map.
                         fs_name << " as rank " << mds << " (now has " <<
                         mds_map.get_num_in_mds() + 1 << " ranks)";
                         fsmap.promote(info->global_id, *fs, mds); return true;}
                         else
                         if (in > max) {
                         ...
                         fsmap.modify_daemon(info.global_id, f); return true;}
                         else {
 ...
 */
                         bool MDSMonitor::drop_mds(FSMap & fsmap, mds_gid_t gid,
                         ...
                         const auto & info = fsmap.get_info_gid(gid);
                         const auto rank = info.rank;
                         const auto state = info.state; if (info.is_frozen()) {
                         return false;}
                         else
                         if (state == MDSMap::STATE_STANDBY_REPLAY ||
                             state == MDSMap::STATE_STANDBY) {
                         dout(1) << " failing and removing standby " << gid <<
                         " " << info.addrs << " mds." << rank << "." << info.
                         inc << " " << ceph_mds_state_name(state)
                         << dendl; *osd_propose |= fail_mds_gid(fsmap, gid);
                         return true;}
                         else
                         if (rank >= 0 && rep_info) {
    ...
    // and is there a non-laggy standby that can take over for us?
                         ...
                         << " mds." << rank << "." << info.inc
                         << " " << ceph_mds_state_name(state)
                         << " with " << rep_info->global_id << "/" << rep_info->
                         name << " " << rep_info->addrs << dendl;
                         mon.clog->warn() << "Replacing " << info.human_name()
                         << " as rank " << rank << " with standby " <<
                         rep_info->human_name();
                         // Remove the old one
                         *osd_propose |= fail_mds_gid(fsmap, gid);
                         ...
                         bool MDSMonitor::check_health(FSMap & fsmap,
                                                       bool * propose_osdmap) {
                         bool do_propose = false;
                         const auto now = mono_clock::now();
                         const bool osdmap_writeable =
                         mon.osdmon()->is_writeable();
                         const auto mds_beacon_grace =
                         g_conf().get_val < double >("mds_beacon_grace");
                         ...
                         {
                         auto since_last =
                         std::chrono::duration < double >(now - last_tick);
                         if (since_last.count() >
                             (mds_beacon_grace - mds_beacon_interval)) {
                         // This case handles either local slowness (calls being delayed
                         // for whatever reason) or cluster election slowness (a long gap
                         // between calls while an election happened)
          dout(1) << __func__ << ": resetting beacon timeouts due to mon delay " "(slow election?) of " << since_last.count() << " seconds" << dendl; for (auto & p:last_beacon)
                         {
                         p.second.stamp = now;}
                         }
                         }
  ...
  mono_time latest_beacon = mono_clock::zero(); for (const auto & p:last_beacon)
                         {
                         latest_beacon =
                         std::max(p.second.stamp, latest_beacon);}
                         auto since =
                         std::chrono::duration < double >(now - latest_beacon);
                         const bool may_replace =
                         since.count() < std::max(g_conf()->mds_beacon_interval,
                         ...
                         const auto quorum_age =
                         std::chrono::seconds(mon.quorum_age());
                         const bool new_quorum =
                         quorum_age < mds_beacon_mon_down_grace;
                         for (auto it = last_beacon.begin();
                              it != last_beacon.end();) {
                         auto &[gid, beacon_info] = *it;
                         auto since_last =
                         std::chrono::duration <
                         double >(now - beacon_info.stamp);
                         if (!fsmap.gid_exists(gid)) {
                         // gid no longer exists, remove from tracked beacons
                         it = last_beacon.erase(it); continue;}

                         if (since_last.count() >= g_conf()->mds_beacon_grace) {
                         auto & info = fsmap.get_info_gid(gid);
                         dout(1) << "no beacon from mds." << info.
                         rank << "." << info.
                         inc << " (gid: " << gid << " addr: " << info.
                         addrs << " state: " << ceph_mds_state_name(info.
                                                                    state) <<
                         ")" << " since " << since_last.count() << dendl;
                         if ((mon_down || new_quorum)
                             && since_last < mds_beacon_mon_down_grace) {
  ...
  for (const auto & gid:to_remove) {
                         ...
                         rep_info = fsmap.find_replacement_for( {
                                                               fscid, info.rank}
                         );}
                         bool dropped =
                         drop_mds(fsmap, gid, rep_info, propose_osdmap);
                         if (dropped) {
      ...
      for (auto &[fscid, fs]:fsmap.filesystems) {
              ...
              for (const auto &[gid, info]:fs->mds_map.
                              get_mds_info())
                         {
                         const auto join_fscid = info.join_fscid;
                         if (join_fscid == fscid)
                         continue; const auto rank = info.rank;
                         ...
                         *propose_osdmap |= fail_mds_gid(fsmap, gid);
                         ...
                         bool MDSMonitor::maybe_promote_standby(FSMap & fsmap,
                                                                Filesystem & fs)
                         {
                         if (fs.mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE)) {
                         return false;}
  ...
  set < mds_rank_t > failed; fs.mds_map.get_failed_mds_set(failed); for (const auto & rank:failed)
                         {
                         auto info = fsmap.find_replacement_for( {
                                                                fs.fscid,
                                                                rank});
                         if (info) {
                         dout(1) << " taking over failed mds." << rank <<
                         " with " << info->global_id << "/" << info->
                         name << " " << info->addrs << dendl;
                         mon.clog->info() << "Standby " << info->human_name()
                         << " assigned to filesystem " << fs.mds_map.
                         fs_name << " as rank " << rank;
                         fsmap.promote(info->global_id, fs, rank);
                         do_propose = true;}
                         }

                         if (fs.mds_map.is_resizeable()
                             && fs.mds_map.allows_standby_replay()) {
                         ...
                         for (;;) {
          auto info = fsmap.get_available_standby(fs); if (!info) break; dout(20) << "standby available mds." << info->global_id << dendl; bool changed = false; for (const auto & rank:fs.mds_map.in)
                         {
                         dout(20) << "examining " << rank << dendl;
                         if (fs.mds_map.is_followable(rank)) {
                         dout(1) << "  setting mds." << info->
                         global_id << " to follow mds rank " << rank << dendl;
                         fsmap.assign_standby_replay(info->global_id, fs.fscid,
                                                     rank); do_propose = true;
                         changed = true; break;}
                         }
                         if (!changed) break;}
                         }
                         ...
                         int err = get_version(v, bl); if (err) {
                         ...
                         dout(5) <<
                         "flushing old fsmap struct because unable to decode FSMap: "
                         << e.what() << dendl;}
  ...
  for (auto & p:        pending.filesystems) {
                         do_propose |=
                         maybe_resize_cluster(pending, p.second->fscid);}
  ...
  for (auto & p:        pending.filesystems) {
                         ...
                         MDSMonitor::MDSMonitor(Monitor & mn, Paxos & p,
                                                string service_name)
:                       PaxosService(mn, p, service_name) {
                         ...
                         last_tick = mono_clock::now(); last_beacon.clear();}
