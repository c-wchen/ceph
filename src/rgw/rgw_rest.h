...
template < class T >
    int rgw_rest_get_json_input_keep_data(CephContext * cct, req_state * s,
                                          T & out, uint64_t max_len,
                                          char **pdata, int *len)
{
    int rv, data_len;
    char *data;
...
class RGWRESTFlusher:public RGWFormatterFlusher {
  ...
  public:
    RGWRESTFlusher(struct req_state *_s,
                   RGWOp * _op):RGWFormatterFlusher(_s->formatter), s(_s),
        op(_op) {
...
class RGWPostObj_ObjStore:public RGWPostObj {
  ...
  protected:
    ...
    int read_line(ceph::bufferlist & bl,
                  uint64_t max, bool & reached_boundary, bool & done);
    ...
    static bool part_bl(parts_collection_t & parts,
                        const std::string & name, ceph::bufferlist * pbl);
...
class RGWREST {
  ...
  public:
    ...
    RGWHandler *get_handler(RGWRados * store, struct req_state *s,
                            RGWLibIO * io, RGWRESTMgr ** pmgr, int *init_error);
#endif
...
extern void dump_header(struct req_state *s,
                        ...
                        ceph::buffer::list & bl);
extern void dump_header(struct req_state *s,
                        const boost::string_ref & name, long long val);
extern void dump_header(struct req_state *s,
                        const boost::string_ref & name, const utime_t & val);

template < class ... Args >
    static inline void dump_header_prefixed(struct req_state *s,
                                            const boost::
                                            string_ref & name_prefix,
                                            const boost::string_ref & name, Args
                                            && ... args)
{
    ...
    return dump_header(s, std::move(full_name),
...
extern void dump_etag(struct req_state *s,
                      const boost::string_ref & etag, bool quoted = false);
extern void dump_etag(struct req_state *s,
                      ceph::buffer::list & bl_etag, bool quoted = false);
extern void dump_epoch_header(struct req_state *s, const char *name,
                              real_time t);
extern void dump_time_header(struct req_state *s, const char *name,
                             real_time t);
