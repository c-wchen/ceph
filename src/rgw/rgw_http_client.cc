...
class RGWCurlHandles:public Thread {
  public:
    ...
    "RGWCurlHandles::cleaner_lock"}, cleaner_shutdown {
    0} {
    }
                                            ...
                                            void *const _info)
{
    ...
    if (ret < 0) {
        dout(0) << "WARNING: client->send_data() returned ret=" << ret << dendl;
    }
                                          ...
                                          const size_t nmemb, void *const _info)
{
    ...
    if (ret < 0) {
        dout(0) << "WARNING: client->receive_header() returned ret=" << ret <<
            dendl;
    }
                                        ...
                                        const size_t nmemb, void *const _info)
{
    ...
    if (ret < 0) {
        dout(0) << "WARNING: client->receive_data() returned ret=" << ret <<
            dendl;
    }
...
int RGWHTTPClient::process(const char *method, const char *url)
{
    ...
    if (status) {
        dout(0) << "curl_easy_perform returned status " << status << " error: "
            << error_buf << dendl;
        ret = -EINVAL;
    }
 ...
 */
int RGWHTTPClient::init_request(const char *method, const char *url,
                                rgw_http_req_data * _req_data,
                                bool send_data_hint)
{
    assert(!req_data);
    _req_data->get();
...
{
    const boost::string_ref header_line(static_cast < const char *const >(ptr),
                                        len);
                                      ...
                                      int write_fd, int read_fd)
{
    ...
    if (ret < 0) {
        ret = -errno;
        ldout(cct,
              0) << "ERROR: " << __func__ << "(): write() returned " << ret <<
            dendl;
        return ret;
    }
...
void RGWHTTPManager::register_request(rgw_http_req_data * req_data)
{
    ...
    ldout(cct,
        ...
        id << ", easy_handle=" << req_data->easy_handle << dendl;
}
...
int RGWHTTPManager::link_request(rgw_http_req_data * req_data)
{
    ldout(cct,
        ...
        req_data->id << ", easy_handle=" << req_data->easy_handle << dendl;
    CURLMcode mstatus =
        curl_multi_add_handle((CURLM *) multi_handle, req_data->easy_handle);
    if (mstatus) {
...
int RGWHTTPManager::add_request(RGWHTTPClient * client, const char *method,
                                const char *url, bool send_data_hint)
{
