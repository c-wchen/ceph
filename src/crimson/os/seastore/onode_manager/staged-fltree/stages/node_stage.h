...
public:
    ...
    std::enable_if_t < T == node_type_t::INTERNAL,
        const laddr_packed_t *> get_end_p_laddr() const
    {
        ...
        if constexpr(FIELD_TYPE == field_type_t::N3) {
            ...
            auto offset_start =
                p_fields->get_item_end_offset(keys(), node_size);
    ...
    {
        auto free_size =
            p_fields->template free_size_before < NODE_TYPE > (index,
                    node_size);
    ...
    std::enable_if_t < T::FIELD_TYPE == field_type_t::N3,
        const value_t *> get_p_value(index_t index) const
    {
        assert(index < keys());
        if constexpr(NODE_TYPE == node_type_t::INTERNAL) {
            ...
            auto ret =
                reinterpret_cast < const value_header_t * >(range.p_start);
    ...
    {
        // nothing to decode
        return node_extent_t(reinterpret_cast <
                             const FieldType * >(p_node_start), node_size);
    }
    ...
    template < IsFullKey Key >
    static const value_t *insert_at(NodeExtentMutable &mut,
                                    const node_extent_t &,
                                    const Key &key,
                                    const value_input_t &value,
                                    index_t index, node_offset_t size,
    ...
    template < IsFullKey Key >
    static memory_range_t insert_prefix_at(NodeExtentMutable &,
                                           const node_extent_t &,
                                           const Key &key,
                                           index_t index,
                                           node_offset_t size,
    ...
    static void update_size_at(NodeExtentMutable &, const node_extent_t &,
                               index_t index, int change);

    static node_offset_t trim_until(NodeExtentMutable &,
                                    const node_extent_t &, index_t index);
...
public:
    ...
    void append(const full_key_t < KT > &, const value_input_t &,
                const value_t *&);
    char *wrap();
    std::tuple < NodeExtentMutable *, char *>open_nxt(const key_get_type &);
    std::tuple < NodeExtentMutable *,
        char *>open_nxt(const full_key_t < KT > &);
