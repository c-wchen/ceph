...
template <>
struct InvalidateRequest <MockObjectMapImageCtx >: public
    MockInvalidateRequestBase < MockObjectMapImageCtx > {
};

LockRequest < MockObjectMapImageCtx > *LockRequest <
MockObjectMapImageCtx >::s_instance = nullptr;
...
public:
    ...
    typedef InvalidateRequest < MockObjectMapImageCtx >
                                ...
                                MockLockRequest &mock_lock_request)
    {
        ...
        .WillOnce(FinishRequest(&mock_lock_request, 0,
                                &mock_image_ctx));
    } void expect_object_map_load(MockObjectMapImageCtx &
                                  mock_image_ctx,
                                  ceph::BitVector < 2 > *object_map,
                                  uint64_t snap_id, int r)
    {
        std::string oid(ObjectMap <>::
                        object_map_name(mock_image_ctx.id, snap_id));
        auto &expect =
        ...
        if (r < 0) {
            ...
            expect.
            WillOnce(DoAll
                     (WithArg < 5 > (CopyInBufferlist(str)),
                      Return(0)));
        }
    } void expect_get_image_size(MockObjectMapImageCtx &
    ...
    void expect_invalidate_request(MockObjectMapImageCtx &
                                   mock_image_ctx,
                                   MockInvalidateRequest &
                                   invalidate_request, int r)
    {
    ...
    void expect_object_map_resize(MockObjectMapImageCtx &
                                  mock_image_ctx, uint64_t num_objects,
                                  int r)
    {
        std::string oid(ObjectMap <>::
                        object_map_name(mock_image_ctx.id,
                                        TEST_SNAP_ID));
        auto &expect =
            EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                        exec(oid, _, StrEq("rbd"),
    ...
    {
        uint64_t num_objs =
            Striper::get_num_objects(mock_image_ctx.layout,
...
TEST_F(TestMockObjectMapRefreshRequest, SuccessHead)
{
    ...
    MockRefreshRequest *req =
        new MockRefreshRequest(mock_image_ctx, &object_map_lock,
                               &object_map, CEPH_NOSNAP, &ctx);

    InSequence seq;
    expect_get_image_size(mock_image_ctx, CEPH_NOSNAP,
                          mock_image_ctx.image_ctx->size);
    expect_object_map_lock(mock_image_ctx, mock_lock_request);
    expect_object_map_load(mock_image_ctx, &on_disk_object_map,
                           CEPH_NOSNAP, 0);
    expect_get_image_size(mock_image_ctx, CEPH_NOSNAP,
                          mock_image_ctx.image_ctx->size);
    req->send();
...
TEST_F(TestMockObjectMapRefreshRequest, LoadError)
{
    ...
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID,
                          mock_image_ctx.image_ctx->size);
    expect_object_map_load(mock_image_ctx, nullptr, TEST_SNAP_ID,
                           -ENOENT);
...
TEST_F(TestMockObjectMapRefreshRequest, LoadCorrupt)
{
    ...
    expect_object_map_resize(mock_image_ctx, on_disk_object_map.size(),
                             0);
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID,
                          mock_image_ctx.image_ctx->size);
...
TEST_F(TestMockObjectMapRefreshRequest, TooSmall)
{
    ...
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID,
                          mock_image_ctx.image_ctx->size);
    expect_object_map_load(mock_image_ctx, &small_object_map,
                           TEST_SNAP_ID, 0);
...
TEST_F(TestMockObjectMapRefreshRequest, TooLarge)
{
    ...
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID,
                          mock_image_ctx.image_ctx->size);
    expect_object_map_load(mock_image_ctx, &large_object_map,
                           TEST_SNAP_ID, 0);
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID,
                          mock_image_ctx.image_ctx->size);
    req->send();
