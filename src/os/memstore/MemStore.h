...
class MemStore:public ObjectStore {
  public:
    ...
    struct Collection:public CollectionImpl {
        ...
        void encode(bufferlist & bl) const {
            ...
            ::encode(s, bl);
            for (map < ghobject_t, ObjectRef >::const_iterator p =
                 object_map.begin(); p != object_map.end(); ++p) {
                ::encode(p->first, bl);
                p->second->encode(bl);
            } ENCODE_FINISH(bl);
        }
        ...
        :cid(c),
            ...
            lock("MemStore::Collection::lock", true, false), exists(true) {
        }
    };
  ...
  private:
    ...
    int _clone_range(const coll_t & cid, const ghobject_t & oldoid,
                     const ghobject_t & newoid, uint64_t srcoff, uint64_t len,
                     uint64_t dstoff);
  ...
  public:
    ...
    :ObjectStore(cct, path),
        coll_lock("MemStore::coll_lock"), finisher(cct), used_bytes(0) {
    }
    ~MemStore()override {
    }
    ...
    int getattrs(CollectionHandle & c, const ghobject_t & oid, map < string,
                 bufferptr > &aset) override;
