...
class SimplePolicyMessenger:public Messenger {
  ...
  public:
    ...
    :Messenger(cct, name), policy_lock("SimplePolicyMessenger::policy_lock") {
    }
    /**
   ...
   */ Policy get_policy(int t) override {
        ...
        map < int, Policy >::iterator iter = policy_map.find(t);
   ...
   */
    void set_policy_throttlers(int type,
        ...
        Mutex::Locker l(policy_lock);
        map < int, Policy >::iterator iter = policy_map.find(type);
        if (iter != policy_map.end()) {
