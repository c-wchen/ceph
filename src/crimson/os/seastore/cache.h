...
template <
    ...
    typename pin_t,
    size_t node_size, bool leaf_has_children > class FixedKVBtree;
class BackrefManager;
class SegmentProvider;

struct backref_entry_t {
    backref_entry_t(const paddr_t paddr,
                    const laddr_t laddr,
                    const extent_len_t len,
                    const extent_types_t type, const journal_seq_t seq)
        : paddr(paddr), laddr(laddr), len(len), type(type), seq(seq)
    {
    } backref_entry_t(alloc_blk_t alloc_blk)
        : paddr(alloc_blk.paddr),
          laddr(alloc_blk.laddr), len(alloc_blk.len), type(alloc_blk.type)
    {
    ...
    journal_seq_t seq;
    friend bool operator<(const backref_entry_t &l,
    ...
    using backref_set_member_options = boost::intrusive::member_hook <
                                       backref_entry_t, set_hook_t, &backref_entry_t::backref_set_hook >;
    ...
    struct cmp_t {
        ...
        bool operator()(const backref_entry_t &l,
...
using backref_entryrefs_by_seq_t =
    std::map < journal_seq_t, backref_entry_refs_t >;
...
public:
    ...
    TransactionRef create_transaction(Transaction::src_t src,
                                      const char *name, bool is_weak)
    {
        ...
        auto ret =
            std::make_unique < Transaction > (get_dummy_ordering_handle(),
                                              is_weak,
                                              src,
                                              last_commit,
        [this](Transaction & t) {
            return
                on_transaction_destruct(t);
        }
        , ++next_id);
    ...
    retire_extent_ret retire_extent_addr(Transaction &t, paddr_t addr,
                                         extent_len_t length);
    ...
    template < typename T >
    using get_extent_ret =
        get_extent_ertr::future < TCachedExtentRef < T >>;
    template < typename T, typename Func, typename OnCache > get_extent_ret < T > get_extent(
        ...
        on_cache)
    {
        ...
        if (!cached) {
            auto ret =
                CachedExtent::make_cached_extent_ref < T >
                (alloc_cache_buf(length));
            ret->init(CachedExtent::extent_state_t::CLEAN_PENDING, offset,
        ...
        if (cached->get_type() == extent_types_t::RETIRED_PLACEHOLDER) {
            ...
            auto ret =
                TCachedExtentRef < T > (static_cast < T * >(cached.get()));
            on_cache(*ret);
    ...
    template < typename T > get_extent_ret < T > get_extent(paddr_t offset, ///< [in] starting addr
            extent_len_t length,    ///< [in] length
            const src_ext_t *p_metric_key   ///< [in] cache query metric key
                                                           )
    {
        return get_extent < T > (offset, length, p_metric_key, [](T &) {
        }
        , [](T &) {
        }
                                );
    ...
    get_extent_if_cached_ret get_extent_if_cached(Transaction &t,
            paddr_t offset,
            extent_types_t type)
    {
        CachedExtentRef ret;
        ...
        if (result == Transaction::get_extent_ret::RETIRED) {
            ...
            return ret->wait_io().then([ret] {
                return get_extent_if_cached_iertr::
                make_ready_future < CachedExtentRef >
                (ret);
            }
                                      );
        }
    ...
    get_extent_iertr::future < TCachedExtentRef <
    T >> get_extent(Transaction &t, paddr_t offset,
                    extent_len_t length, Func && extent_init_func)
    {
        CachedExtentRef ret;
        LOG_PREFIX(Cache::get_extent);
        auto result = t.get_extent(offset, &ret);
        if (result != Transaction::get_extent_ret::ABSENT) {
            SUBTRACET(seastore_cache, "{} {}~{} is {} on t -- {}",
                      ...
                      length,
                      result ==
                      Transaction::get_extent_ret::
                      PRESENT ? "present" : "retired", *ret);
        ...
        SUBTRACET(seastore_cache,
                  "{} {}~{} is absent on t, query cache ...", t, T::TYPE,
                  offset, length);
        ...
        return trans_intr::make_interruptible(get_extent < T >
    ...
    template < typename T, typename Func >
    get_extent_iertr::future < TCachedExtentRef <
    T >> get_absent_extent(Transaction &t, paddr_t offset,
                           extent_len_t length, Func
                           && extent_init_func)
    {
        CachedExtentRef ret;
        LOG_PREFIX(Cache::get_extent);
        ...
        if (r != Transaction::get_extent_ret::ABSENT) {
            SUBERRORT(seastore_cache, "unexpected non-absent extent {}", t,
                      *ret);
            ceph_abort();
        }
#endif
    ...
    seastar::future < CachedExtentRef > get_extent_viewable_by_trans(Transaction &t,
            CachedExtentRef extent)
    {
        ...
        return p_extent->wait_io().then([p_extent] {
    ...
    template < typename T >
    seastar::future < TCachedExtentRef <
    T >> get_extent_viewable_by_trans(Transaction &t,
    ...
    public:
        ...
        template < typename Func > extent_init_func_t(Func && func): wrapped(std::make_unique < callable_wrapper <
    ...
    get_extent_ertr::future < CachedExtentRef > _get_extent_by_type(extent_types_t type, paddr_t offset,
    ...
    using get_extent_by_type_ret = get_extent_by_type_iertr::future <
                                   CachedExtentRef >;
    get_extent_by_type_ret _get_extent_by_type(Transaction &t,
            extent_types_t type,
            paddr_t offset,
            laddr_t laddr,
            extent_len_t length,
    ...
    get_extent_by_type_ret _get_absent_extent_by_type(Transaction &t,
            extent_types_t type,
            paddr_t offset,
            laddr_t laddr,
            extent_len_t length,
            ...
            && extent_init_func)
    {
        ...
        SUBTRACET(seastore_cache,
                  "{} {}~{} {} is absent on t, query cache ...", t, type,
                  offset, length, laddr);
        auto f = [&t, this](CachedExtent & ext) {
            t.add_to_read_set(CachedExtentRef(&ext));
            touch_extent(ext);
    ...
    backref_entry_query_mset_t get_backref_entries_in_range(paddr_t start,
            paddr_t end)
    {
        auto start_iter = backref_entry_mset.lower_bound(start,
                          backref_entry_t::
                          cmp_t());
        auto end_iter = backref_entry_mset.lower_bound(end,
                        backref_entry_t::
                        cmp_t());
...
public:
     ...
     */
    template < typename Func > get_extent_by_type_ret get_extent_by_type(Transaction &t,    ///< [in] transaction
    ...
    {
        return _get_extent_by_type(t,
                                   type,
                                   offset,
                                   laddr,
                                   length,
                                   extent_init_func_t(std::forward < Func >
                                           (extent_init_func)));
    }
     ...
     * the specific CachedExtent
     */
    template < typename Func > get_extent_by_type_ret get_absent_extent_by_type(Transaction &t,     ///< [in] transaction
            extent_types_t type,    ///< [in] type tag
            paddr_t offset, ///< [in] starting addr
            laddr_t laddr,  ///< [in] logical address if logical
            extent_len_t length,    ///< [in] length
    ...
    {
        return _get_absent_extent_by_type(t,
                                          type,
                                          offset,
                                          laddr,
                                          length,
                                          extent_init_func_t(std::forward <
                                                  Func >
                                                  (extent_init_func)));
    }

    get_extent_by_type_ret get_extent_by_type(Transaction &t,
            extent_types_t type,
            paddr_t offset,
            laddr_t laddr,
            extent_len_t length)
    {
        return get_extent_by_type(t, type, offset, laddr, length,
        [](CachedExtent &) {
        }
                                 );
    }
     ...
     * the specific CachedExtent
     */
    get_extent_by_type_ret get_absent_extent_by_type(Transaction &t,
            extent_types_t type,
            paddr_t offset,
            laddr_t laddr,
            extent_len_t length)
    {
        return get_absent_extent_by_type(t, type, offset, laddr, length,
        [](CachedExtent &) {
        }
                                        );
    }
    ...
    void trim_backref_bufs(const journal_seq_t &trim_to)
    {
        ...
        if (!backref_entryrefs_by_seq.empty()) {
            ...
            backref_entryrefs_by_seq.erase(backref_entryrefs_by_seq.begin(),
                                           iter);
        }
        if (backref_entryrefs_by_seq.empty()) {
            SUBDEBUG(seastore_cache,
                     "backref_entryrefs_by_seq all trimmed");
        }
    }
     ...
     */
    template < typename T > TCachedExtentRef < T > alloc_new_extent(Transaction &t,     ///< [in, out] current transaction
            extent_len_t length,    ///< [in] length
                                                                   ...
                                                                   )
    {
        ...
        t, T::TYPE, length, hint, rewrite_gen_printer_t {
            gen}
                 );
#ifdef UNIT_TESTS_BUILT
        auto result =
            epm.alloc_new_extent(t, T::TYPE, length, hint, gen, epaddr);
#else
        ...
        ret->init(CachedExtent::extent_state_t::INITIAL_WRITE_PENDING,
                  result.paddr, hint, result.gen, t.get_trans_id());
        t.add_fresh_extent(ret);
        SUBDEBUGT(seastore_cache,
                  "allocated {} {}B extent at {}, hint={}, gen={} -- {}",
                  t, T::TYPE, length, result.paddr,
        hint, rewrite_gen_printer_t {
            result.gen}
        , *ret);
        return ret;
    }
    ...
    replay_delta_ret replay_delta(journal_seq_t seq,
    ...
    template < typename F >
    init_cached_extents_ret init_cached_extents(Transaction &t,
            F && f)
    {
        ...
        return seastar::do_with(std::forward < F > (f),
                                ...
                                [this, FNAME, &t](auto & f,
        auto & refs) mutable {
            return trans_intr::do_for_each(refs,
                                           [this, FNAME,
                                            &t,
                                            &f](auto &
            e) {
                ...
                si_then([this, FNAME, &t, e](bool is_alive) {
                    ...
                    (!is_alive) {
                        SUBDEBUGT
                        (seastore_cache,
                         "extent is not alive, remove extent -- {}",
                         t,
                         *e);
                        remove_extent
                        (e);
        ...
        .handle_error_interruptible(init_cached_extents_iertr::
        pass_further {
        }
     ...
     */
    CachedExtentRef update_extent_from_transaction(Transaction &t,
            CachedExtentRef extent)
    {
    ...
    using get_next_dirty_extents_ret =
        get_next_dirty_extents_iertr::future < std::vector <
        CachedExtentRef >>;
    get_next_dirty_extents_ret get_next_dirty_extents(Transaction &t,
            journal_seq_t seq,
            size_t max_bytes);
    ...
    struct backref_extent_entry_t {
        backref_extent_entry_t(paddr_t paddr,
                               paddr_t key, extent_types_t type)
        ...
        struct cmp_t {
            using is_transparent = paddr_t;
            bool operator()(const backref_extent_entry_t &l,
                            const backref_extent_entry_t &r)const
            {
                return l.paddr < r.paddr;
            } bool operator()(const paddr_t &l,
                              const backref_extent_entry_t &r)const
            {
                return l < r.paddr;
            } bool operator()(const backref_extent_entry_t &l,
...
private:
    ...
    backref_extent_entry_query_set_t get_backref_extents_in_range(paddr_t
            start,
            paddr_t
            end)
    {
    ...
    using counter_by_extent_t =
        std::array < CounterT, EXTENT_TYPES_MAX >;
    ...
    template < typename CounterT >
    using counter_by_src_t =
        std::array < CounterT, TRANSACTION_TYPE_MAX >;
    ...
    struct {
        ...
        counter_by_src_t < commit_trans_efforts_t >
        committed_efforts_by_src;
        counter_by_src_t < invalid_trans_efforts_t >
    ...
    template < typename CounterT >
    CounterT &get_by_src(counter_by_src_t < CounterT >
                         &counters_by_src, Transaction::src_t src)
    {
    ...
    template < typename CounterT >
    CounterT &get_by_ext(counter_by_extent_t < CounterT >
                         &counters_by_ext, extent_types_t ext)
    {
    ...
    void account_conflict(Transaction::src_t src1, Transaction::src_t src2)
    {
        ...
        auto conflict_index =
            num_srcs * src1_value + src2_value - src1_value * (src1_value +
                1) / 2;
    ...
    void backref_batch_update(std::vector < backref_entry_ref > &&,
                              const journal_seq_t &);
    ...
    void commit_replace_extent(Transaction &t, CachedExtentRef next,
    ...
    void mark_transaction_conflicted(Transaction &t,
    ...
    template < typename T >
    get_extent_ret < T > read_extent(TCachedExtentRef < T > &&extent)
    {
        assert(extent->state ==
               CachedExtent::extent_state_t::CLEAN_PENDING);
        extent->set_io_wait();
        return epm.read(extent->get_paddr(),
                        extent->get_length(), extent->get_bptr()
        ).safe_then([extent = std::move(extent)]()mutable {
            ...
            extent->complete_io();
            SUBDEBUG(seastore_cache, "read extent done -- {}",
                     *extent);
                                ...
                                const src_ext_t *p_metric_key)
    {
        ...
        if (p_metric_key) {
            p_counters =
                &get_by_src(stats.cache_query_by_src, p_metric_key->first);
            ++p_counters->access;
        }
    ...
    template <
        ...
        typename pin_t,
        size_t node_size,
        bool leaf_has_children > friend class FixedKVBtree;
};
