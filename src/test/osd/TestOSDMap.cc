...
class OSDMapTest:public testing::Test {
  ...
  public:
    ...
    OSDMapTest() {
        ...
        int r = osdmap.crush->add_simple_rule("erasure", "default", "osd", "",
...
TEST_F(OSDMapTest, CleanPGUpmaps)
{
...

    {
        ...
        }
        {
            ...
            nextmap.maybe_remove_pg_upmaps(g_ceph_context, nextmap,
                                           &pending_inc);
            tmpmap.apply_incremental(pending_inc);
            ASSERT_TRUE(!tmpmap.have_pg_upmaps(ec_pgid));
        }
...

    {
        ...
        int steps = 6;
        crush_rule *rule =
            crush_make_rule(steps, rno, rule_type, min_size, max_size);
        int step = 0;
...
TEST_F(OSDMapTest, BUG_38897)
{
    ...
    // ready to go
    {
        ...
        auto ret =
            g_ceph_context->_conf->set_val("osd_calc_pg_upmaps_max_stddev",
                                           "0");
        ASSERT_EQ(0, ret);
