...
namespace {
    ...
    obj_rec dirs1_b {
    dirs1_bucket_name, nullptr, nullptr, nullptr};
    ...
    if (do_hier1) {
        ...
        if (do_create) {
            ...
            "foo/bar/baz/quux",
                    ...
                    "foo/bar/baz/sasquatch/", "foo/bar/baz/frobozz"};
          ...
          for (const auto & obj_name:obj_names) {
                ...
                RGWPutObjRequest req(cct, fs_private->get_user(), bucket_name,
                                     obj_name, bl);
...
TEST(LibRGW, SETUP_DIRS1)
{
    if (do_dirs1) {
        ...
        for (d_ix = 0; d_ix < n_dirs1_dirs; ++d_ix) {
            ...
            obj_rec dir {
            dname, nullptr, dirs1_b.fh, nullptr};
            ovec.clear();
            ...
            if (!dir.fh) {
                if (do_create) {
                    rc = rgw_mkdir(fs, dir.parent_fh, dir.name.c_str(), &st,
                                   create_mask, &dir.fh, RGW_MKDIR_FLAG_NONE);
                    ASSERT_EQ(rc, 0);
                }
            }
            ...
            for (f_ix = 0; f_ix < n_dirs1_objs; ++f_ix) {
                ...
                obj_rec sdir {
                sdname, nullptr, dir.fh, nullptr};

                (void)rgw_lookup(fs, sdir.parent_fh, sdir.name.c_str(),
                                 &sdir.fh, RGW_LOOKUP_FLAG_NONE);

                if (!sdir.fh) {
                    if (do_create) {
                        rc = rgw_mkdir(fs, sdir.parent_fh, sdir.name.c_str(),
                                       &st, create_mask, &sdir.fh,
                                       RGW_MKDIR_FLAG_NONE);
                        ASSERT_EQ(rc, 0);
                    }
                }
                ...
                if (!sf.fh) {
                    if (do_create) {
                        ...
                        rc = rgw_lookup(fs, sf.parent_fh, sf.name.c_str(),
                                        &sf.fh, RGW_LOOKUP_FLAG_CREATE);
                        ASSERT_EQ(rc, 0);
                        sf.sync();
                        ...
                        /* open handle */
                        rc = rgw_open(fs, sf.fh, 0 /* posix flags */ ,
                                      0 /* flags */ );
                        ASSERT_EQ(rc, 0);
...
TEST(LibRGW, SETATTR)
{
    if (do_dirs1) {
        if (do_setattr) {
            ...
            rc = rgw_setattr(fs, sf.fh, &st, create_mask,
                             RGW_SETATTR_FLAG_NONE);
            ASSERT_EQ(rc, 0);

            /* force evict--subsequent lookups must reload */
            static_cast <
                RGWLibFS * >(fs->fs_private)->release_evict(sf.rgw_fh);
        ...
        if (do_create) {
          ...
          for (auto & dirs_rec:dirs_vec) {
                ...
                (void)rgw_lookup(fs, sf.parent_fh, sf.name.c_str(), &sf.fh,
                                 RGW_LOOKUP_FLAG_NONE);
                if (!sf.fh) {
                    rc = rgw_create(fs, sf.parent_fh, sf.name.c_str(), &st,
                                    create_mask, &sf.fh, 0 /* posix flags */ ,
    ...
    if (do_rename) {
      ...
      for (int b_ix:{
          ...
          for (int f_ix:{
                ...
                obj_rec rf {
                rfname, nullptr, brec.fh, nullptr};
                (void)rgw_lookup(fs, rf.parent_fh, rf.name.c_str(), &rf.fh,
                                 RGW_LOOKUP_FLAG_NONE);
                if (!rf.fh) {
                    rc = rgw_create(fs, rf.parent_fh, rf.name.c_str(), &st,
                                    create_mask, &rf.fh, 0 /* posix flags */ ,
                                    RGW_CREATE_FLAG_NONE);
                    ASSERT_EQ(rc, 0);
                }
                rf.sync();
            ...
            renames_vec.push_back(dirs1_rec {
                                  brec, ovec}
            );
            ovec.clear();
        }                       /* b_ix */
    ...
    if (do_rename) {
        ...
        if (verbose) {
            std::cout << "rename file " << src_obj.name << " to "
                << rfname << " (bucket " << bdir0.name << ")" << std::endl;
        }
    ...
    if (do_rename) {
        ...
        if (verbose) {
            std::cout << "rename file " << src_obj.name
                ...
                << rfname << " (bucket " << bdir1.name << ")" << std::endl;
        }
    ...
    if (do_dirs1) {
      ...
      for (auto & dirs_rec:dirs_vec) {
          ...
          for (auto & sobj:get < 1 > (dirs_rec)) {
                ...
                if (sobj.rgw_fh->object_name().find("sfile") !=
                    std::string::npos) {
                    ASSERT_TRUE(sobj.rgw_fh->is_file());
                    ASSERT_TRUE(S_ISREG(st.st_mode));
                }
                if (sobj.rgw_fh->object_name().find("sdir") !=
                    std::string::npos) {
      ...
      for (auto & dirs_rec:dirs_vec) {
          ...
          for (auto & sobj:get < 1 > (dirs_rec)) {
                ...
                if ((sobj.rgw_fh->object_name().find("sfile_0")
                    ...
                    memset(buf, 0, 256);
                    if (verbose) {
                        std::cout << "reading 0,256 " << sobj.rgw_fh->
                            relative_object_name()
                            << std::endl;
                    }
                    rc = rgw_read(fs, sobj.fh, 0, 256, &nread, buf,
                                  RGW_READ_FLAG_NONE);
                    ASSERT_EQ(rc, 0);
                    if (verbose) {
    ...
    if (do_dirs1) {
        if (do_create) {
            if ((!stat(readf_out_name.c_str(), &st)) &&
                (S_ISREG(st.st_mode)) && (st.st_size == 6291456))
                return;
            ofstream of;
...
TEST(LibRGW, READF_DIRS1)
{
    if (do_dirs1) {
        if (do_readf) {
            obj_rec fobj {
            readf_name, nullptr, dirs1_b.fh, nullptr};
...
TEST(LibRGW, WRITEF_DIRS1)
{
    if (do_dirs1) {
        if (do_writef) {
            ...
            obj_rec fobj {
            writef_name, nullptr, dirs1_b.fh, nullptr};
            ...
            if (!fobj.fh) {
                if (do_create) {
                    /* make a new file object (the hard way) */
                    rc = rgw_lookup(fs, fobj.parent_fh, fobj.name.c_str(),
                                    &fobj.fh, RGW_LOOKUP_FLAG_CREATE);
                    ASSERT_EQ(rc, 0);
                }
            }
            ASSERT_NE(fobj.fh, nullptr);
...
extern "C" {
    static bool r1_cb(const char *name, void *arg, uint64_t offset,
        ...
        if (!((name_str == ".") || (name_str == ".."))) {
            obj_stack.push(obj_rec {
                           std::move(name_str), nullptr, parent_fh, nullptr}
            );
        }
        return true;            /* XXX */
    }
...
TEST(LibRGW, HIER1)
{
    if (do_hier1) {
        int rc;
        obj_stack.push(obj_rec {
                       bucket_name, nullptr, nullptr, nullptr});
                ...
                case RGW_FS_TYPE_DIRECTORY:
                    if (!elt.state.readdir) {
                        ...
                        lsubdout(cct, rgw, 10)
                            ...
                            << dendl;
                        rc = rgw_readdir(fs, elt.fh, &offset, r1_cb, elt.fh,
                                         &eof, RGW_READDIR_FLAG_DOTDOT);
...
TEST(LibRGW, MARKER1_SETUP_OBJECTS)
{
    ...
    if (do_marker1 && do_create) {
        ...
        for (int ix = 0; ix < marker_nobjs; ++ix) {
            ...
            obj_rec obj {
            object_name, nullptr, marker_fh, nullptr};
            ...
            ret =
                rgw_open(fs, obj.fh, 0 /* posix flags */ , RGW_OPEN_FLAG_NONE);
            ASSERT_EQ(ret, 0);
...
extern "C" {
    static bool r2_cb(const char *name, void *arg, uint64_t offset,
        ...
        name};
        if (!((name_str == ".") || (name_str == ".."))) {
            dvec.obj_names.push_back(dirent_t {
                                     std::move(name_str), offset}
            );
        }
        return true;            /* XXX */
    }
...
TEST(LibRGW, CLEANUP)
{
    ...
    if (do_marker1) {
        cleanup_queue.push_back(obj_rec {
                                bucket_name, bucket_fh, fs->root_fh,
                                get_rgwfh(fs->root_fh)}
        );
    }
