...
public:
    ...
    } int parse(const std::string &status)
    {
        ...
        try {
            ...
            if (s["scheduled_images"].type() !=
                json_spirit::array_type) {
                std::
                cerr <<
                     "rbd: unexpected schedule JSON received: "
                     << "scheduled_images is not array" << std::
                     endl;
                return -EBADMSG;
            }
            for (auto &item_val : s["scheduled_images"].
                 get_array()) {
...
std::ostream &operator<<(std::ostream &os, ScheduleStatus &s)
{
    ...
    tbl.define_column("SCHEDULE TIME", TextTable::LEFT,
                      TextTable::LEFT);
    tbl.define_column("IMAGE", TextTable::LEFT,
                      TextTable::LEFT);
