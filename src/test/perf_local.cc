...
class CondPingPong {
  ...
  public:
  CondPingPong():mutex("CondPingPong::mutex"), prod(0), cons(0), count(10000),
        consumer(this) {
    }
...
double div32()
{
    ...
    uint32_t remainder;
    for (int i = 0; i < count; i++) {
        __asm__ __volatile__("div %4":"=a"(quotient),
                             "=d"(remainder):"a"(numeratorLo), "d"(numeratorHi),
                             "r"(divisor):"cc");
    }
    uint64_t stop = Cycles::rdtsc();
    return Cycles::to_seconds(stop - start) / count;
...
double div64()
{
    ...
    uint64_t remainder;
    for (int i = 0; i < count; i++) {
        __asm__ __volatile__("divq %4":"=a"(quotient),
                             "=d"(remainder):"a"(numeratorLo), "d"(numeratorHi),
                             "r"(divisor):"cc");
    }
    uint64_t stop = Cycles::rdtsc();
