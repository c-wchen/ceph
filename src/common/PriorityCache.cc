...
namespace PriorityCache {
    int64_t get_chunk(uint64_t usage, uint64_t total_bytes) {
        ...
        if (r > 0)
         ...
         return val;
                       ...
                       uint64_t target,
                       bool reserve_extra,
                       const std::string & name):cct(c), caches {
    }, min_mem(min),
        max_mem(max),
        target_mem(target),
        tuned_mem(min),
        ...
        PerfCountersBuilder b(cct, this->name, MallocStats::M_FIRST,
                              MallocStats::M_LAST);
    ...
    void Manager::tune_memory() {
        ...
        ldout(cct, 5) << __func__
            ...
            << " unmapped: " << unmapped
            << " heap: " << heap_size
            << " old mem: " << tuned_mem << " new mem: " << new_size << dendl;
    ...
    void Manager::insert(const std::string & name,
    ...
    void Manager::balance() {
      ...
      for (auto & l:loggers) {
            ...
            l.second->set(indexes[it->first][Extra::E_RESERVED],
                          committed - alloc);
    ...
    void Manager::balance_priority(int64_t * mem_avail, Priority pri) {
        std::unordered_map < std::string,
            std::shared_ptr < PriCache >> tmp_caches = caches;
        double cur_ratios = 0;
        double new_ratios = 0;
        uint64_t round = 0;
        ...
        while (!tmp_caches.empty()
            ...
            for (auto it = tmp_caches.begin(); it != tmp_caches.end();) {
                int64_t cache_wants =
                    it->second->request_cache_bytes(pri, tuned_mem);
                ...
                int64_t fair_share =
                    static_cast < int64_t > (*mem_avail * ratio);
