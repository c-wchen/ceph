...
static NTSTATUS WinCephCreateDirectory(LPCWSTR FileName,
...
static NTSTATUS WinCephCreateFile(LPCWSTR FileName,
                                  ...
                                  PDOKAN_FILE_INFO DokanFileInfo)
{
    ...
    DokanMapKernelToUserCreateFileFlags(DesiredAccess, FileAttributes,
                                        CreateOptions, CreateDisposition,
                                        &AccessMode, &FlagsAndAttributes,
                                        &CreationDisposition);
...
static NTSTATUS WinCephReadFile(LPCWSTR FileName,
                                LPVOID Buffer,
                                DWORD BufferLength,
                                LPDWORD ReadLength,
...
static NTSTATUS WinCephWriteFile(LPCWSTR FileName,
                                 LPCVOID Buffer,
...
static NTSTATUS WinCephFlushFileBuffers(LPCWSTR FileName,
                                        PDOKAN_FILE_INFO DokanFileInfo)
{
    pfd_context fdc = (pfd_context) & (DokanFileInfo->Context);
    if (!fdc->fd) {
                                          ...
                                          PDOKAN_FILE_INFO DokanFileInfo)
{
    ...
    to_filetime(stbuf.stx_atime.tv_sec,
                &HandleFileInformation->ftLastAccessTime);
    to_filetime(stbuf.stx_mtime.tv_sec,
...
static NTSTATUS WinCephFindFiles(LPCWSTR FileName, PFillFindData FillFindData,  // function pointer
                                 PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
    dout(20) << __func__ << " " << path << dendl;
 ...
 */
static NTSTATUS WinCephDeleteFile(LPCWSTR FileName,
                                  PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
    dout(20) << __func__ << " " << path << dendl;
...
static NTSTATUS WinCephDeleteDirectory(LPCWSTR FileName,
                                       PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
    dout(20) << __func__ << " " << path << dendl;
...
static NTSTATUS WinCephSetEndOfFile(LPCWSTR FileName,
                                    LONGLONG ByteOffset,
                                    PDOKAN_FILE_INFO DokanFileInfo)
{
    pfd_context fdc = (pfd_context) & (DokanFileInfo->Context);
...
static NTSTATUS WinCephSetFileSecurity(LPCWSTR FileName,
...
static NTSTATUS WinCephGetDiskFreeSpace(PULONGLONG FreeBytesAvailable,
