...
TEST(TestARNResource, Basic)
{
    for (auto i = 0; i < BASIC_ENTRIES; ++i) {
        ...
        boost::optional < ARNResource > resource =
            ARNResource::parse(arn->resource);
        ASSERT_TRUE(resource);
        EXPECT_STREQ(resource->resource.c_str(), "resource");
        EXPECT_STREQ(resource->resource_type.c_str(),
                     expected_basic_resource_type[i].c_str());
        EXPECT_STREQ(resource->qualifier.c_str(),
                     expected_basic_qualifier[i].c_str());
    }
}
