...
namespace crimson::os::seastore {
    ...
    TransactionManager::TransactionManager(JournalRef _journal,
                                           CacheRef _cache,
                                           LBAManagerRef _lba_manager,
                                           ExtentPlacementManagerRef && _epm,
                                           BackrefManagerRef
                                           && _backref_manager)
    :cache(std::move(_cache)), lba_manager(std::move(_lba_manager)),
        journal(std::move(_journal)), epm(std::move(_epm)),
        ...
        INFO("enter");
        return epm->mount().safe_then([this] {
                                      return journal->open_for_mkfs();}
        ).safe_then([this] (auto start_seq) {
                    journal->get_trimmer().update_journal_tails(start_seq,
                                                                start_seq);
    ...
    TransactionManager::mount_ertr::future <> TransactionManager::mount() {
        ...
        return epm->mount().safe_then([this] {
                                                 ...
                                                 init_cached_extents(t,
                                                           ...
                                                           scan_mapped_space(t,
                                                                             ...
                                                                             (backref_key
                                                                              ...
                                                                              P_ADDR_NULL);
                                                                             backref_manager->
                                                                             cache_new_backref_extent
                                                                             (paddr,
                                                                              backref_key,
                                                                              type);
                                                                             cache->
                                                                             update_tree_extents_num
                                                                             (type,
                                                                              1);
                                                                             epm->
                                                                             mark_space_used
            ...
            .safe_then([this] {
                    ...
                    epm->start_background(); INFO("completed");}
    ...
    TransactionManager::close_ertr::future <> TransactionManager::close() {
        ...
        INFO("enter");
        return epm->stop_background().then([this] {
                                           return cache->close();}
        ).safe_then([this] {
                    cache->dump_contents(); return journal->close();}
        ).safe_then([this] {
                    return epm->close();}
        ).safe_then([FNAME] {
                    INFO("completed"); return seastar::now();}
        );
    }

    TransactionManager::ref_ret TransactionManager::inc_ref(Transaction & t,
    ...
    TransactionManager::ref_ret TransactionManager::inc_ref(Transaction & t,
        ...
        LOG_PREFIX(TransactionManager::inc_ref);
        TRACET("{}", t, offset);
        return lba_manager->incref_extent(t,
                                          offset).si_then([FNAME, offset,
                                                           &t] (auto result) {
                                                          DEBUGT
                                                          ("extent refcount is incremented to {} -- {}~{}, {}",
                                                           t, result.refcount,
                                                           offset,
                                                           result.length,
                                                           result.addr);
    ...
    TransactionManager::ref_ret TransactionManager::dec_ref(Transaction & t,
                                                            LogicalCachedExtentRef
                                                            & ref) {
    ...
    TransactionManager::ref_ret TransactionManager::dec_ref(Transaction & t,
        ...
        LOG_PREFIX(TransactionManager::dec_ref);
        TRACET("{}", t, offset);
        return lba_manager->decref_extent(t,
                                          offset).si_then([this, FNAME, offset,
                                                           &t] (auto result)->
                                                          ref_ret {
                                                          DEBUGT
                                                          ("extent refcount is decremented to {} -- {}~{}, {}",
                                                           t, result.refcount,
                                                           offset,
                                                           result.length,
                                                           result.addr);
    ...
    TransactionManager::refs_ret TransactionManager::dec_ref(Transaction & t,
                                                             std::vector <
                                                             laddr_t >
                                                             offsets) {
        LOG_PREFIX(TransactionManager::dec_ref);
    ...
    TransactionManager::submit_transaction_iertr::future <>
        ...
        return trans_intr::make_interruptible(t.get_handle().
                                              enter(write_pipeline.
                                                    reserve_projected_usage)
            ).then_interruptible([this, FNAME, &t] {
                                 auto dispatch_result =
                                 epm->dispatch_delayed_extents(t);
                                 auto projected_usage = dispatch_result.usage;
                                 SUBTRACET(seastore_t,
                                           "waiting for projected_usage: {}", t,
                                           projected_usage);
                                 return trans_intr::make_interruptible(epm->
                                                                       reserve_projected_usage
                                                                       (projected_usage)
                                 ).
                                 then_interruptible([this, &t, dispatch_result =
                                                     std::
                                                     move(dispatch_result)] {
                                                    return
                                                    do_submit_transaction(t,
                                                                          std::
                                                                          move
                                                                          (dispatch_result));}
                                 ).finally([this, FNAME, projected_usage, &t] {
                                           SUBTRACET(seastore_t,
                                                     "releasing projected_usage: {}",
                                                     t, projected_usage);
                                           epm->
                                           release_projected_usage
                                           (projected_usage);}
                                 );}
        );
    }
    ...
    TransactionManager::submit_transaction_direct_ret
        TransactionManager::submit_transaction_direct(Transaction & tref,
                                                      std::optional <
                                                      journal_seq_t >
                                                      trim_alloc_to) {
        return do_submit_transaction(tref, epm->dispatch_delayed_extents(tref),
                                     trim_alloc_to);
    }

    TransactionManager::submit_transaction_direct_ret
        TransactionManager::do_submit_transaction(Transaction & tref,
        ...
        return trans_intr::make_interruptible(tref.get_handle().
                                              enter(write_pipeline.ool_writes)
            ).
            then_interruptible([this, FNAME, &tref, dispatch_result =
                               ...
                               return seastar::do_with(std::
                                                       ...
                                                       si_then([this, FNAME,
                                                               ...
                                                               SUBTRACET
                                                               (seastore_t,
                                                                "update delayed extent mappings",
                                                                tref);
                                                               return
                                                               lba_manager->
                                                               update_mappings
                  ...
                  SUBTRACET(seastore_t, "process {} allocated extents", tref,
                            num_extents);
                  return epm->write_preallocated_ool_extents(tref,
                                                             allocated_extents).
                  handle_error_interruptible(crimson::ct_error::
                                             input_output_error::pass_further(),
                                             crimson::ct_error::
                                             assert_all("invalid error")
                  );}
        ).si_then([this, FNAME, &tref] {
                  SUBTRACET(seastore_t, "about to prepare", tref);
                  return tref.get_handle().enter(write_pipeline.prepare);}
        ).si_then([this, FNAME, &tref, trim_alloc_to =
                   std::move(trim_alloc_to)] ()mutable->
              ...
              SUBDEBUGT(seastore_t, "committed with {}", tref, submit_result); auto start_seq = submit_result.write_result.start_seq; journal->get_trimmer().set_journal_head(start_seq); cache->complete_commit(tref, submit_result.record_block_base, start_seq); std::vector < CachedExtentRef > lba_to_clear; std::vector < CachedExtentRef > backref_to_clear; lba_to_clear.reserve(tref.get_retired_set().size()); backref_to_clear.reserve(tref.get_retired_set().size()); for (auto & e:tref.get_retired_set())
                              {
                  ...
                  ).handle_error(submit_transaction_iertr::pass_further {
                                 }
                                 , crimson::ct_error::all_same_way([](auto e) {
                                                                   ceph_assert(0
                                                                               ==
                                                                               "Hit error submitting to journal");}
                                 )
                  );}
        )
            .finally([&tref] () {
    ...
    TransactionManager::get_next_dirty_extents_ret
        TransactionManager::get_next_dirty_extents(Transaction & t,
                                                   journal_seq_t seq,
                                                   size_t max_bytes) {
    ...
    TransactionManager::rewrite_extent_ret
        TransactionManager::rewrite_logical_extent(Transaction & t,
                                                   LogicalCachedExtentRef
                                                   extent) {
        ...
        auto nlextent = cache->alloc_new_extent_by_type(t,
                                                        lextent->get_type(),
                                                        lextent->get_length(),
                                                        lextent->
                                                        get_user_hint(),
                                                        ...
                                                        lextent->
                                                        get_rewrite_generation
                                                        ())->cast <
            LogicalCachedExtent > ();
        lextent->get_bptr().copy_out(0, lextent->get_length(),
        ...
        return lba_manager->update_mapping(t,
                                           lextent->get_laddr(),
                                           lextent->get_paddr(),
                                           nlextent->get_paddr(),
                                           nlextent.get());
    ...
    TransactionManager::rewrite_extent_ret TransactionManager::
        rewrite_extent(Transaction & t, CachedExtentRef extent,
        ...
        if (extent->is_logical()) {
            return rewrite_logical_extent(t,
    ...
    TransactionManager::get_extents_if_live_ret
        TransactionManager::get_extents_if_live(Transaction & t,
                                                extent_types_t type,
                                                paddr_t paddr,
                                                laddr_t laddr,
                                                extent_len_t len) {
        ...
        return cache->get_extent_if_cached(t, paddr,
                                           type).si_then([ =, this,
                                                          &t] (auto extent)
                                                         ->
                                                         ...
                                                         (t, type, paddr, laddr,
                                                          len).si_then([ =,
                                                                       ...
                                                                       DEBUGT
                                                                       ("{} {}~{} {} is not live as physical extent",
                                                                        t, type,
                                                                        laddr,
                                                                        len,
                                                                        paddr);}
    ...
    TransactionManagerRef make_transaction_manager(Device * primary_device,
                                                   const std::vector <
                                                   Device * >&secondary_devices,
                                                   bool is_test) {
        ...
        std::vector < SegmentProvider * >segment_providers_by_id {
        DEVICE_ID_MAX, nullptr};
        ...
        if (p_backend_type == backend_type_t::SEGMENTED) {
            ...
            sms->add_segment_manager(static_cast <
                                     SegmentManager * >(primary_device));
        }
        else {
            auto rbm =
                std::make_unique < BlockRBManager > (static_cast <
                                                     RBMDevice *
                                                     >(primary_device), "",
                                                     is_test);
      ...
      for (auto & p_dev:secondary_devices) {
            if (p_dev->get_backend_type() == backend_type_t::SEGMENTED) {
                if (p_dev->get_device_type() ==
                    primary_device->get_device_type()) {
                    sms->add_segment_manager(static_cast <
                                             SegmentManager * >(p_dev));
                }
                else {
                    if (!cold_sms) {
                        cold_sms = std::make_unique < SegmentManagerGroup > ();
                    }
                    cold_sms->add_segment_manager(static_cast <
                                                  SegmentManager * >(p_dev));
                }
            }
            else {
        ...
        if (journal_type == journal_type_t::SEGMENTED) {
            roll_size =
                static_cast <
                SegmentManager * >(primary_device)->get_segment_size();
            roll_start = 0;
        }
        else {
            roll_size =
        ...
        if (is_test) {
            ...
            trimmer_config =
                JournalTrimmerImpl::config_t::get_test(roll_size, journal_type);
        }
        else {
            cleaner_is_detailed = false;
            cleaner_config = SegmentCleaner::config_t::get_default();
            trimmer_config =
                JournalTrimmerImpl::config_t::get_default(roll_size,
                                                          journal_type);
        }

        auto journal_trimmer =
            JournalTrimmerImpl::create(*backref_manager, trimmer_config,
        ...
        if (cold_sms) {
            cold_segment_cleaner = SegmentCleaner::create(cleaner_config,
                                                          ...
                                                          *backref_manager,
                                                          ...
                                                          /* is_cold = */ true);
            if (journal_type == journal_type_t::SEGMENTED) {
              for (auto id:cold_segment_cleaner->
                     get_device_ids()) {
                    segment_providers_by_id[id] =
                        static_cast <
                        SegmentProvider * >(cold_segment_cleaner.get());
                }
            }
        }
        ...
        if (journal_type == journal_type_t::SEGMENTED) {
            cleaner = SegmentCleaner::create(cleaner_config,
                                             ...
                                             *backref_manager,
                                             ...
                                             get_ool_segment_seq_allocator(),
                                             cleaner_is_detailed);
            auto segment_cleaner =
                static_cast < SegmentCleaner * >(cleaner.get());
          for (auto id:segment_cleaner->get_device_ids()) {
            ...
            cleaner = RBMCleaner::create(std::move(rbs),
                                         *backref_manager, cleaner_is_detailed);
            journal = journal::make_circularbounded(*journal_trimmer,
                                                    static_cast <
                                                    random_block_device::
                                                    RBMDevice *
                                                    >(primary_device), "");
        ...
        epm->init(std::move(journal_trimmer),
                  std::move(cleaner), std::move(cold_segment_cleaner));
        epm->set_primary_device(primary_device);

        return std::make_unique < TransactionManager > (std::move(journal),
                                                        std::move(cache),
                                                        std::move(lba_manager),
                                                        std::move(epm),
                                                        std::
                                                        move(backref_manager));
    }
