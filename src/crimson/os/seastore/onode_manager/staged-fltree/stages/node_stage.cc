...
container_range_t NODE_T::get_nxt_container(index_t index) const
{
    if constexpr(std::is_same_v < FieldType, internal_fields_3_t >) {
        ...
        auto item_start_offset =
            p_fields->get_item_start_offset(index, node_size);
        auto item_end_offset =
            p_fields->get_item_end_offset(index, node_size);
...
template < typename FieldType, node_type_t NODE_TYPE >
void NODE_T::bootstrap_extent(NodeExtentMutable &mut,
                              field_type_t field_type,
                              node_type_t node_type, bool is_level_tail,
                              level_t level)
{
    node_header_t::bootstrap_extent(mut, field_type, node_type,
                                    is_level_tail, level);
    mut.copy_in_relative(sizeof(node_header_t),
                         typename FieldType::num_keys_t(0u));
}

template < typename FieldType, node_type_t NODE_TYPE >
void NODE_T::update_is_level_tail(NodeExtentMutable &mut,
                                  const node_extent_t &extent,
                                  bool value)
{
    assert(mut.get_length() == extent.node_size);
    assert(mut.get_read() == extent.p_start());
    node_header_t::update_is_level_tail(mut, extent.p_fields->header,
                                        value);
}

template < typename FieldType, node_type_t NODE_TYPE >
template < IsFullKey Key >
memory_range_t NODE_T::insert_prefix_at(NodeExtentMutable &mut,
                                        const node_extent_t &node,
                                        const Key &key, index_t index,
                                        node_offset_t size,
                                        const char *p_left_bound)
{
    ...
    (FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
        ...
        mut.shift_absolute(p_left_bound,
                           p_insert - p_left_bound, -(int)size_right);
        return {
            p_insert_front, p_insert};
    } else if constexpr
    (FIELD_TYPE == field_type_t::N2) {
...
template < typename FieldType, node_type_t NODE_TYPE >
void NODE_T::update_size_at(NodeExtentMutable &mut,
                            const node_extent_t &node, index_t index,
                            int change)
{
    assert(mut.get_length() == node.node_size);
    assert(mut.get_read() == node.p_start());
...
template < typename FieldType, node_type_t NODE_TYPE >
node_offset_t NODE_T::trim_until(NodeExtentMutable &mut,
                                 const node_extent_t &node,
                                 index_t index)
{
    assert(mut.get_length() == node.node_size);
    ...
    if constexpr
    (std::is_same_v < FieldType, internal_fields_3_t >) {
        ceph_abort("not implemented");
    } else {
        mut.copy_in_absolute((void *)&node.p_fields->num_keys,
                             num_keys_t(index));
    }
...
template < typename FieldType, node_type_t NODE_TYPE >
node_offset_t NODE_T::trim_at(NodeExtentMutable &mut,
                              const node_extent_t &node, index_t index,
                              node_offset_t trimmed)
{
    ...
    (std::is_same_v < FieldType, internal_fields_3_t >) {
        ...
        assert(new_offset <
               node.p_fields->get_item_end_offset(index, node_size));
        mut.copy_in_absolute(const_cast <
                             void * >(node.p_fields->p_offset(index)),
...
template < typename FieldType, node_type_t NODE_TYPE >
node_offset_t NODE_T::erase_at(NodeExtentMutable &mut,
                               const node_extent_t &node,
                               index_t index,
                               const char *p_left_bound)
{
...
template < typename FieldType, node_type_t NODE_TYPE >
template < KeyT KT > APPEND_T::Appender(NodeExtentMutable *p_mut,
                                        const node_extent_t &node, bool open)
    : p_mut {
    p_mut}, p_start {
    p_mut->get_write()}
{
    ...
    if (open) {
        ...
        (FIELD_TYPE == field_type_t::N0 ||
         FIELD_TYPE == field_type_t::N1) {
            p_append_left =
            p_start + node.fields().get_key_start_offset(node.keys() -
                          1, node_size);
        ...
        (std::is_same_v < FieldType, internal_fields_3_t >) {
            ...
            p_append_left =
                p_start + node.fields().get_key_start_offset(node.keys(),
                    node_size);
            p_append_right =
                p_start + node.fields().get_item_end_offset(node.keys(),
                    node_size);
        }
        num_keys = node.keys();
    }
...
template < typename FieldType, node_type_t NODE_TYPE >
template < KeyT KT >
void APPEND_T::append(const node_extent_t &src, index_t from,
                      index_t items)
{
    ...
    (std::is_same_v < FieldType, internal_fields_3_t >) {
        ...
        if (num_keys == 0) {
            ...
            for (auto i = from; i < from + items; ++i) {
                int new_offset =
                    src.fields().get_item_start_offset(i,
                                                       node_size) +
                    offset_change;
                assert(new_offset > 0);
            ...
            assert(p_append_left + left_size +
                   sizeof(typename FieldType::key_t) == p_offset_dst);
        }
        ...
        p_mut->copy_in_absolute(p_append_right,
                                src.p_start() + offset_right_start,
...
template < typename FieldType, node_type_t NODE_TYPE >
template < KeyT KT >
void APPEND_T::append(const full_key_t < KT > &key,
                      const value_input_t &value,
                      const value_t*&p_value)
{
    if constexpr
    (FIELD_TYPE == field_type_t::N3) {
        ceph_abort("not implemented");
    } else {
...
template < typename FieldType, node_type_t NODE_TYPE >
template < KeyT KT > char *APPEND_T::wrap()
{
    ...
    p_mut->copy_in_absolute(p_start + offsetof(FieldType, num_keys),
                            num_keys);
