...
static int do_map(int argc, const char *argv[])
{
    ...
    if (poolname.empty()) {
        poolname =
            g_ceph_context->_conf.get_val < std::string > ("rbd_default_pool");
    }
    ...
    if (r < 0) {
        std::
        cerr << "rbd-ggate: failed to connect to cluster: " <<
             cpp_strerror(r)
             << std::endl;
        goto done;
    }
    ...
    if (!snapname.empty()) {
        ...
        if (r < 0) {
            std::
            cerr << "rbd-ggate: failed to set snapshot " << snapname << ": "
                 << cpp_strerror(r) << std::endl;
            goto done;
        }
        readonly = true;
    ...
    if (r < 0) {
        r = -errno;
        std::
        cerr << "rbd-ggate: failed to create ggate device: " <<
             cpp_strerror(r)
             << std::endl;
        goto done;
    }
    ...
    if (r < 0) {
        std::cerr << "rbd-ggate: failed to map: " << cpp_strerror(r) << std::
                  endl;
    }
...
static int do_list(const std::string &format, bool pretty_format)
{
    ...
    if (format == "json") {
        ...
        std::cerr << "rbd-ggate: invalid output format: " << format << std::
                  endl;
        return -EINVAL;
    }
    ...
    for (auto &it : devs) {
        ...
        if (f) {
            ...
            tbl << id << poolname << nsname << imgname << snapname << "/dev/" +
                name << TextTable::endrow;
        }
        count++;
    }
...
int main(int argc, const char *argv[])
{
    ...
    if (args.begin() != args.end()) {
        if (strcmp(*args.begin(), "map") == 0) {
            ...
            cerr << "rbd-ggate: unknown command: " << *args.
                 begin() << std::endl;
            return EXIT_FAILURE;
        }
    ...
    switch (cmd) {
        ...
        case Disconnect:
            if (args.begin() == args.end()) {
                std::
                cerr <<
                     "rbd-ggate: must specify ggate device or image-or-snap-spec" <<
                     std::endl;
                return EXIT_FAILURE;
            }
