...
class ObjectCacher::C_ReadFinish:public Context {
  ...
  public:
    ...
    C_ReadFinish(ObjectCacher * c, Object * ob, ceph_tid_t t, loff_t s,
                 uint64_t l, const ZTracer::Trace & trace):oc(c),
        poolid(ob->oloc.pool), oid(ob->get_soid()), start(s), length(l),
        set_item(this), trust_enoent(true), tid(t), trace(trace) {
        ob->reads.push_back(&set_item);
    } void finish(int r) override {
        oc->bh_read_finish(poolid, oid, tid, start, length, bl, r,
                           trust_enoent);
        trace.event("finish");
...
void ObjectCacher::Object::merge_left(BufferHead * left, BufferHead * right)
{
    ...
    for (map < loff_t, list < Context * > >::iterator p =
         right->waitfor_read.begin(); p != right->waitfor_read.end(); ++p)
        left->waitfor_read[p->first].splice(left->waitfor_read[p->first].
                                            begin(), p->second);
 ...
 */
bool ObjectCacher::Object::is_cached(loff_t cur, loff_t left) const const
{
    assert(oc->lock.is_locked());
                                   ...
                                   map < loff_t, BufferHead * >&errors)
{
    ...
    map < loff_t, BufferHead * >::const_iterator p =
        data_lower_bound(ex.offset);
    while (left > 0) {
        // at end?
        if (p == data.end()) {
            // rest is a miss.
            ...
            if (complete) {
                ...
                hits[cur] = n;
                ldout(oc->cct,
                      20) << "map_read miss+complete+zero " << left << " left, "
                    << *n << dendl;
            }
            else {
                missing[cur] = n;
                ldout(oc->cct,
                      20) << "map_read miss " << left << " left, " << *n <<
                    dendl;
            }
            cur += left;
        ...
        if (p->first <= cur) {
            ...
            if (complete) {
                ...
                hits[cur] = n;
                ldout(oc->cct,
                      20) << "map_read gap+complete+zero " << *n << dendl;
            }
            else {
                missing[cur] = n;
...
void ObjectCacher::Object::audit_buffers()
{
    ...
    for (map < loff_t, BufferHead * >::const_iterator it = data.begin();
        ...
        if (it->first < offset) {
            lderr(oc->cct) << "AUDIT FAILURE: " << it->first << " " << *it->
                second << " overlaps with previous bh " << *((--it)->second)
                << dendl;
    ...
    while (left > 0) {
        ...
        if (p == data.end()) {
            if (final == NULL) {
                ...
                ldout(oc->cct,
                      10) << "map_write adding trailing bh " << *final << dendl;
            }
            else {
        ...
        ldout(oc->cct,
              10) << "cur is " << cur << ", p is " << *p->second << dendl;
        //oc->verify_stats();

        if (p->first <= cur) {
            BufferHead *bh = p->second;
            ldout(oc->cct,
                  10) << "map_write bh " << *bh << " intersected" << dendl;
            ...
            ldout(oc->cct,
                  10) << "map_write gap " << cur << "~" << glen << dendl;
            if (final) {
                oc->bh_stat_sub(final);
                                   ...
                                   C_GatherBuilder * commit_gather)
{
    ...
    while (p != data.end()) {
        ...
        if (bh->is_tx() && commit_gather != nullptr) {
            ...
            waitfor_commit[bh->last_write_tid].emplace_back(commit_gather->
                                                            new_sub());
        }
        else if (bh->is_rx()) {
...
void ObjectCacher::perf_start()
{
    ...
    plb.add_u64_counter(l_objectcacher_cache_bytes_miss,
                        "cache_bytes_miss", "Miss data", NULL, 0,
                        unit_t(BYTES));
                                  ...
                                  bool trust_enoent)
{
    ...
    ldout(cct, 7) << "bh_read_finish "
        ...
        << " (bl is " << bl.length() << ")"
        << " returned " << r
        << " outstanding reads " << reads_outstanding << dendl;
    ...
    if (objects[poolid].count(oid) == 0) {
        ...
        while (true) {
            map < loff_t, BufferHead * >::const_iterator p =
                ob->data_lower_bound(opos);
            if (p == ob->data.end())
                break;
            if (opos >= start + (loff_t) length) {
                ldout(cct,
                      20) << "break due to opos " << opos << " >= start+length "
                    << start << "+" << length << "=" << start +
                    (loff_t) length << dendl;
                break;
            }
            ...
            for (map < loff_t, list < Context * > >::iterator it
                 = bh->waitfor_read.begin(); it != bh->waitfor_read.end(); ++it)
                ls.splice(ls.end(), it->second);
            bh->waitfor_read.clear();

            if (bh->start() > opos) {
                ldout(cct, 1) << "bh_read_finish skipping gap "
                    << opos << "~" << bh->start() - opos << dendl;
                opos = bh->start();
                continue;
            }
            ...
            if (!bh->is_rx()) {
                ldout(cct,
                      10) << "bh_read_finish skipping non-rx " << *bh << dendl;
                opos = bh->end();
                continue;
            }
            ...
            if (r == -ENOENT) {
                if (trust_enoent) {
                    ldout(cct,
                          10) << "bh_read_finish removing " << *bh << dendl;
                    bh_remove(ob, bh);
                    delete bh;
                }
                else {
                    ldout(cct,
                          10) <<
                        "skipping unstrusted -ENOENT and will retry for " << *bh
                        << dendl;
                }
                continue;
            }
                                        ...
                                        int64_t * max_amount, int *max_count)
{
    ...
    int count = 0;
    int64_t total_len = 0;
    set < BufferHead *, BufferHead::ptr_lt >::iterator it =
        dirty_or_tx_bh.find(bh);
...
class ObjectCacher::C_WriteCommit:public Context {
  ...
  public:
    ...
    C_WriteCommit(ObjectCacher * c, int64_t _poolid, sobject_t o, loff_t s,
                  uint64_t l, const ZTracer::Trace & trace):oc(c),
        poolid(_poolid), oid(o), trace(trace) {
        ranges.push_back(make_pair(s, l));
    } C_WriteCommit(ObjectCacher * c, int64_t _poolid, sobject_t o,
                    vector < pair < loff_t, uint64_t > >&_ranges):oc(c),
        poolid(_poolid), oid(o), tid(0) {
        ranges.swap(_ranges);
    }
    void finish(int r) override {
...
void ObjectCacher::bh_write_scattered(list < BufferHead * >&blist)
{
    ...
    for (list < BufferHead * >::iterator p = blist.begin(); p != blist.end();
         ++p) {
        BufferHead *bh = *p;
                                   ...
                                   ceph_tid_t tid, int r)
{
    ...
    for (vector < pair < loff_t, uint64_t > >::iterator p = ranges.begin();
        ...
        if (!ob->exists) {
            ldout(cct,
                  10) << "bh_write_commit marking exists on " << *ob << dendl;
            ob->exists = true;

            if (writeback_handler.
                may_copy_on_write(ob->get_oid(), start, length,
                                  ob->get_snap())) {
                ldout(cct,
        ...
        for (map < loff_t, BufferHead * >::const_iterator p =
             ob->data_lower_bound(start); p != ob->data.end(); ++p) {
            BufferHead *bh = p->second;
            ...
            if (!bh->is_tx()) {
                ldout(cct,
                      10) << "bh_write_commit skipping non-tx " << *bh << dendl;
                continue;
            }
            ...
            if (bh->last_write_tid != tid) {
                ...
                ldout(cct,
                      10) << "bh_write_commit newer tid on " << *bh << dendl;
                continue;
            }
                         ...
                         bool external_call, ZTracer::Trace * trace)
{
    ...
    for (vector < ObjectExtent >::iterator ex_it = rd->extents.begin();
        ...
        if (oset->return_enoent && !o->exists) {
            ...
            if (writeback_handler.may_copy_on_write(soid.oid, ex_it->offset,
                ...
                bool wait = false;
                list < BufferHead * >blist;
                for (map < loff_t, BufferHead * >::iterator bh_it =
                     o->data.begin(); bh_it != o->data.end(); ++bh_it) {
                ...
                if (wait) {
                    ldout(cct,
                        ...
                        last_write_tid << " on " << *o << dendl;
                    o->waitfor_commit[o->last_write_tid].
                        push_back(new
                                  C_RetryRead(this, rd, oset, onfinish,
                                              *trace));
                    // FIXME: perfcounter!
                    return 0;
                }
            ...
            for (map < loff_t, BufferHead * >::iterator bh_it = o->data.begin();
                ...
                ldout(cct,
                      20) << "readx  ob has bh " << *bh_it->second << dendl;
        ...
        if (!missing.empty() || !rx.empty()) {
            ...
            for (map < loff_t, BufferHead * >::iterator bh_it = missing.begin();
                ...
                if (!waitfor_read.empty()
                    || (stat_rx > 0 && rx_bytes > max_size)) {
            ...
            if (last != missing.end()) {
                ...
                last->second->waitfor_read[last->first].
                    push_back(new
                              C_RetryRead(this, rd, oset, onfinish, *trace));
            ...
            for (map < loff_t, BufferHead * >::iterator bh_it = rx.begin();
                ...
                if (success && onfinish) {
                    ldout(cct,
                        ...
                        second << " off " << bh_it->first << dendl;
                    bh_it->second->waitfor_read[bh_it->first].
                        push_back(new
                                  C_RetryRead(this, rd, oset, onfinish,
                                              *trace));
                }
            ...
            if (!error) {
                ...
                while (1) {
                    ...
                    uint64_t len =
                        MIN(f_it->second - foff, bh->length() - bhoff);
                    ldout(cct,
                          10) << "readx rmap opos " << opos << ": " << *bh <<
                        " +" << bhoff << " frag " << f_it->
                        first << "~" << f_it->
                        second << " +" << foff << "~" << len << dendl;
            ...
            if (dontneed
                && o->include_all_cached_data(ex_it->offset, ex_it->length))
                bottouch_ob(o);
        }
    }
    ...
    if (!success) {
        ...
        if (onfinish) {
            ...
            ldout(cct,
                  20) << "readx drop " << rd << " (no complete, but no waiter)"
                << dendl;
            delete rd;
        }
        return 0;               // wait!
    ...
    for (vector < ObjectExtent >::iterator ex_it = wr->extents.begin();
        ...
        for (vector < pair < uint64_t, uint64_t > >::iterator f_it
            ...
            ldout(cct, 10) << "writex writing " << f_it->first << "~"
                << f_it->second << " into " << *bh << " at " << opos << dendl;
...
class ObjectCacher::C_WaitForWrite:public Context {
  public:
    C_WaitForWrite(ObjectCacher * oc, uint64_t len,
                   const ZTracer::Trace & trace, Context * onfinish):m_oc(oc),
        m_len(len), m_trace(trace), m_onfinish(onfinish) {
    } void finish(int r) override;
  private:
    ObjectCacher * m_oc;
    uint64_t m_len;
...
void ObjectCacher::flusher_entry()
{
    ...
    while (!flusher_stop) {
        ...
        ldout(cct, 11) << "flusher "
            ...
            << target_dirty << " target, " << max_dirty << " max)" << dendl;
        ...
        if (actual > 0 && (uint64_t) actual > target_dirty) {
            ...
            while ((bh =
                    static_cast <
                    BufferHead * >(bh_lru_dirty.lru_get_next_expire())) != 0
                   && bh->last_write <= cutoff && max > 0) {
                ldout(cct,
                      10) << "flusher flushing aged dirty bh " << *bh << dendl;
                if (scattered_write) {
                         ...
                         ZTracer::Trace * trace)
{
    ...
    bool clean = true;
    ldout(cct,
          10) << "flush " << *ob << " " << offset << "~" << length << dendl;
    for (map < loff_t, BufferHead * >::const_iterator p =
         ob->data_lower_bound(offset); p != ob->data.end(); ++p) {
        BufferHead *bh = p->second;
    ...
    for (xlist < Object * >::iterator p = oset->objects.begin(); !p.end();) {
        ...
        if (o_unclean)
            ldout(cct, 10) << "release_set " << oset << " " << *ob
                << " has " << o_unclean << " bytes left" << dendl;
        p = q;
    }
...
void ObjectCacher::clear_nonexistence(ObjectSet * oset)
{
    ...
    for (xlist < Object * >::iterator p = oset->objects.begin(); !p.end(); ++p) {
        ...
        if (!ob->exists) {
            ldout(cct,
                  10) << " setting exists and complete on " << *ob << dendl;
            ob->exists = true;
            ob->complete = false;
        }
                                     ...
                                     Context * on_finish)
{
    ...
    if (gather.has_subs()) {
        ...
        gather.set_finisher(new FunctionContext([this, oset, flushed,
                                                 on_finish] (int) {
                                                assert(lock.is_locked());
                                                if (flushed
                                                    && flush_set_callback)
                                                flush_set_callback
                                                (flush_set_callback_arg, oset);
                                                if (on_finish)
...
void ObjectCacher::_discard(ObjectSet * oset,
                            const vector < ObjectExtent > &exls,
...
void ObjectCacher::verify_stats() const const
{
    assert(lock.is_locked());
    ...
    loff_t clean = 0, zero = 0, dirty = 0, rx = 0, tx = 0, missing = 0,
        error = 0;
    for (vector < ceph::unordered_map < sobject_t,
         Object * > >::const_iterator i = objects.begin(); i != objects.end();
         ++i) {
        for (ceph::unordered_map < sobject_t, Object * >::const_iterator p =
             i->begin(); p != i->end(); ++p) {
            Object *ob = p->second;
            for (map < loff_t, BufferHead * >::const_iterator q =
                 ob->data.begin(); q != ob->data.end(); ++q) {
                BufferHead *bh = q->second;
                switch (bh->get_state()) {
