...
namespace rbd {
    namespace action {
        namespace disk_usage {
            ...
            static int disk_usage_callback(uint64_t offset, size_t len,
                                           int exists, void *arg) {
                ...
                if (exists) {
                    (*used_size) += len;
                                              ...
                                              const std::
                                              string & from_snap_name,
                                              librbd::Image & image, bool exact,
                                              uint64_t size,
                                              uint64_t * used_size) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to retrieve image flags: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
                if ((flags & RBD_FLAG_FAST_DIFF_INVALID) != 0) {
                    std::
                        ...
                        << (snap_name.empty()? "" : "@" +
                            snap_name) << ". " << "operation may be slow." <<
                        std::endl;
                }
                ...
                *used_size = 0;
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: failed to iterate diffs: " <<
                        cpp_strerror(r)
                        << std::endl;
                    return r;
                }
            ...
            static int do_disk_usage(librbd::RBD & rbd,
                                     librados::IoCtx & io_ctx,
                                     const char *imgname, const char *snapname,
                                     const char *from_snapname, bool exact,
                                     Formatter * f, bool merge_snap) {
                ...
                if (f) {
                    ...
                    tbl.define_column("PROVISIONED", TextTable::LEFT,
                                      TextTable::RIGHT);
                    tbl.define_column("USED", TextTable::LEFT,
                                      TextTable::RIGHT);
                }
              ...
              for (auto & image_spec:images) {
                    ...
                    librbd::Image image;
                    r = rbd.open_read_only(io_ctx, image,
                                           image_spec.name.c_str(), NULL);
                    if (r < 0) {
                        if (r != -ENOENT) {
                            std::cerr << "rbd: error opening " << image_spec.
                                name << ": " << cpp_strerror(r) << std::endl;
                        }
                        continue;
                    }
                    ...
                    if (r < 0) {
                        std::
                            cerr << "rbd: failed to retrieve image features: "
                            << cpp_strerror(r)
                            << std::endl;
                        goto out;
                    }
                    if ((features & RBD_FEATURE_FAST_DIFF) == 0) {
                        std::
                            cerr << "warning: fast-diff map is not enabled for "
                            << image_spec.
                            name << ". " << "operation may be slow." << std::
                            endl;
                    }
                    ...
                    if (r < 0) {
                        std::cerr << "rbd: error opening " << image_spec.
                            name << " snapshots: " << cpp_strerror(r) << std::
                            endl;
                        continue;
                    }
                    ...
                    if ((snapname != nullptr && snap_id == CEPH_NOSNAP) ||
                        ...
                        std::cerr << "specified snapshot is not found." << std::
                            endl;
                        return -ENOENT;
                    }
                    ...
                    for (std::vector <
                         librbd::snap_info_t >::const_iterator snap =
                         snap_list.begin(); snap != snap_list.end(); ++snap) {
                        librbd::Image snap_image;
                        r = rbd.open_read_only(io_ctx, snap_image,
                                               image_spec.name.c_str(),
                                               snap->name.c_str());
                        if (r < 0) {
                            std::
                                cerr << "rbd: error opening snapshot " <<
                                image_spec.name << "@" << snap->
                                name << ": " << cpp_strerror(r) << std::endl;
                            goto out;
                        }

                        if (imgname == nullptr || found_from_snap ||
                            (found_from_snap && snapname != nullptr
                             && snap->name == snapname)) {

                            r = get_image_disk_usage(image_spec.name,
                                                     snap->name, last_snap_name,
                                                     snap_image, exact,
                                                     snap->size, &used_size);
                            if (r < 0) {
                                goto out;
                            ...
                            if (!merge_snap) {
                                format_image_disk_usage(image_spec.name,
                                                        image_spec.id,
                                                        snap->name, snap->id,
                                                        snap->size, used_size,
                                                        tbl, f);
            ...
            void get_arguments(po::options_description * positional,
                ...
                options->add_options()
                    ...
                    ("exact", po::bool_switch(),
                     "compute exact disk usage (slow)")
            ...
            int execute(const po::variables_map & vm,
                ...
                r = do_disk_usage(rbd, io_ctx,
                                  image_name.empty()? nullptr : image_name.
                                  c_str(),
                ...
                if (r < 0) {
                    std::cerr << "rbd: du failed: " << cpp_strerror(r) << std::
                        endl;
                    return r;
                }
                return 0;
