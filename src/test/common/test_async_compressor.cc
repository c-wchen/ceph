  ...
  public:
    ...
    void generate_random_data(bufferlist & bl, uint64_t len = 0) {
        static const char *base =
            "znvm,x12399zasdfjkl1209zxcvjlkasjdfljwqelrjzx,cvn,m123#*(@)";
        if (!len) {
...
TEST_F(AsyncCompressorTest, SimpleTest)
{
    ...
    ASSERT_EQ(0,
              async_compressor->get_compress_data(id, compress_data, true,
                                                  &finished));
    ASSERT_TRUE(finished == true);
    id = async_compressor->async_decompress(compress_data);
    do {
        ASSERT_EQ(0,
                  async_compressor->get_decompress_data(id, decompress_data,
                                                        false, &finished));
    } while (!finished);
    ASSERT_TRUE(finished == true);
    ASSERT_TRUE(rawdata.contents_equal(decompress_data));
    ASSERT_EQ(-ENOENT,
              async_compressor->get_decompress_data(id, decompress_data, true,
                                                    &finished));
}
...
class SyntheticWorkload {
  ...
  public:
    explicit SyntheticWorkload(AsyncCompressor * ac):async_compressor(ac),
        rng(time(NULL)) {
        for (int i = 0; i < 100; i++) {
            bufferlist bl;
            ...
            compress_jobs.
                insert(make_pair
                       (async_compressor->async_compress(rand_data[i]), i));
            if (!(i % 10))
                cerr << "seeding compress data " << i << std::endl;
        }
    ...
    void do_compress() {
        ...
        compress_jobs.
            insert(make_pair
                   (async_compressor->async_compress(rand_data[index]), index));
    }
    void do_decompress() {
        ...
        if (compress_data[index].length())
            decompress_jobs.
                insert(make_pair
                       (async_compressor->
                        async_decompress(compress_data[index]), index));
    }
    void reap(bool blocking) {
        ...
        do {
            for (set < pair < uint64_t, uint64_t > >::iterator it =
                 compress_jobs.begin(); it != compress_jobs.end();) {
                prev = it;
                ++it;
                ASSERT_EQ(0,
                          async_compressor->get_compress_data(prev->first, data,
                                                              blocking,
                                                              &finished));
                if (finished) {
                    c_reap++;
                    if (compress_data[prev->second].length())
                        ASSERT_TRUE(compress_data[prev->second].
                                    contents_equal(data));
                    else
            ...
            for (set < pair < uint64_t, uint64_t > >::iterator it =
                 decompress_jobs.begin(); it != decompress_jobs.end();) {
                prev = it;
                ++it;
                ASSERT_EQ(0,
                          async_compressor->get_decompress_data(prev->first,
                                                                data, blocking,
                                                                &finished));
                if (finished) {
                    d_reap++;
        ...
        cerr << " reap compress jobs " << c_reap << " decompress jobs " <<
            d_reap << std::endl;
    }
    void print_internal_state() {
        cerr << "inlfight compress jobs: " << compress_jobs.size()
            << " inflight decompress jobs: " << decompress_jobs.
            size() << std::endl;
    }
    bool empty() const {
