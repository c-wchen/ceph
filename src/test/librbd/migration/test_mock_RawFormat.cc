...
namespace librbd {
    ...
    namespace migration {
        ...
        template <> struct SourceSpecBuilder <librbd::MockTestImageCtx > {
            ...
            MOCK_CONST_METHOD3(build_snapshot,
                               int (const json_spirit::mObject &, uint64_t,
    ...
    namespace migration {
        ...
        class TestMockMigrationRawFormat:public TestMockFixture {
          public:
            ...
            } void expect_build_snapshot(MockSourceSpecBuilder &
                                         mock_source_spec_builder,
                                         uint64_t index,
                                         MockSnapshotInterface *
                                         mock_snapshot_interface, int r) {
                EXPECT_CALL(mock_source_spec_builder,
                            build_snapshot(_, index, _))
                    .WillOnce(WithArgs < 2 >
                              (Invoke([mock_snapshot_interface, r]
                                      (std::shared_ptr < SnapshotInterface >
                                       *ptr) {
                                      ptr->reset(mock_snapshot_interface);
                                      return r;}
                               )));
            } void expect_snapshot_open(MockSnapshotInterface &
                                        mock_snapshot_interface, int r) {
                EXPECT_CALL(mock_snapshot_interface, open(_, _))
                    .WillOnce(WithArg < 1 > (Invoke([r] (Context * ctx) {
                                                    ctx->complete(r);
                                                    })));
            }

            void expect_snapshot_close(MockSnapshotInterface &
                                       mock_snapshot_interface, int r) {
            ...
            void expect_snapshot_get_info(MockSnapshotInterface &
            ...
            void expect_snapshot_read(MockSnapshotInterface &
                                      mock_snapshot_interface,
            ...
            void expect_snapshot_list_snap(MockSnapshotInterface &
                                           mock_snapshot_interface,
                                           const io::Extents & image_extents,
                                           const io::
                                           SparseExtents & sparse_extents,
                                           int r) {
                EXPECT_CALL(mock_snapshot_interface,
                            list_snap(image_extents, _, _))
                    .WillOnce(WithArgs < 1,
                              2 >
                              (Invoke
                               ([sparse_extents,
                                 r] (io::SparseExtents * out_sparse_extents,
                                     Context * ctx) {
        ...
        TEST_F(TestMockMigrationRawFormat, Read) {
            ...
            expect_snapshot_read(*mock_snapshot_interface, { {
                                 123, 123}}, expect_bl, 0);
            ...
            &bl};
            ASSERT_TRUE(mock_raw_format.read(aio_comp, CEPH_NOSNAP, { {
                                             123, 123}},
        ...
        TEST_F(TestMockMigrationRawFormat, ListSnaps) {
            ...
            sparse_extents.insert(0, 123, {
                                  io::SPARSE_EXTENT_STATE_DATA, 123});
            expect_snapshot_list_snap(*mock_snapshot_interface, { {
                                      0, 123}}, sparse_extents, 0);
            ...
            mock_raw_format.list_snaps( { {
                                       0, 123}}, {
                                       CEPH_NOSNAP}, 0, &snapshot_delta, {
                                       }, &ctx2);
            ASSERT_EQ(0, ctx2.wait());

            io::SnapshotDelta expected_snapshot_delta;
            expected_snapshot_delta[ {
                                    CEPH_NOSNAP, CEPH_NOSNAP}] = sparse_extents;
        ...
        TEST_F(TestMockMigrationRawFormat, ListSnapsMerge) {
            ...
            SnapInfo snap_info_head { {
            }, {
            }, 256, {
            }, 0, 0, {
            }};
            SnapInfo snap_info_1 {
            snap_info_head};
            snap_info_1.size = 123;
            expect_snapshot_get_info(*mock_snapshot_interface_1, snap_info_1);
            io::SparseExtents sparse_extents_1;
            sparse_extents_1.insert(0, 123, {
                                    io::SPARSE_EXTENT_STATE_DATA, 123});
            expect_snapshot_list_snap(*mock_snapshot_interface_1, { {
                                      0, 123}}, sparse_extents_1, 0);

            SnapInfo snap_info_2 {
            snap_info_head};
            snap_info_2.size = 64;
            expect_snapshot_get_info(*mock_snapshot_interface_2, snap_info_2);
            io::SparseExtents sparse_extents_2;
            sparse_extents_2.insert(0, 32, {
                                    io::SPARSE_EXTENT_STATE_DATA, 32});
            expect_snapshot_list_snap(*mock_snapshot_interface_2, { {
                                      0, 123}}, sparse_extents_2, 0);

            expect_snapshot_get_info(*mock_snapshot_interface_head,
                                     snap_info_head);
            io::SparseExtents sparse_extents_head;
            sparse_extents_head.insert(0, 16, {
                                       io::SPARSE_EXTENT_STATE_DATA, 16});
            expect_snapshot_list_snap(*mock_snapshot_interface_head, { {
                                      0, 123}}, sparse_extents_head, 0);
            ...
            mock_raw_format.list_snaps( { {
                                       0, 123}}, {
                                       1, CEPH_NOSNAP}, 0, &snapshot_delta, {
                                       }, &ctx2);
            ASSERT_EQ(0, ctx2.wait());

            io::SnapshotDelta expected_snapshot_delta;
            expected_snapshot_delta[ {
                                    1, 1}] = sparse_extents_1;
            sparse_extents_2.erase(0, 16);
            sparse_extents_2.insert(64, 59, {
                                    io::SPARSE_EXTENT_STATE_ZEROED, 59});
            expected_snapshot_delta[ {
                                    CEPH_NOSNAP, 2}] = sparse_extents_2;
            expected_snapshot_delta[ {
