        ...
        :m_callback(callback), m_finisher(finisher), m_comp(c) {
            ...
            if (m_comp != NULL) {
                if (m_finisher != NULL) {
                    m_finisher->
                        queue(new
                              FunctionContext(boost::
                                              bind(&finish_aio_completion,
                                                   m_comp, ret)));
                }
                else {
  ...
  :    
    ...
    void TestRadosClient::add_aio_operation(const std::string & oid,
        ...
        AioFunctionContext *ctx =
            new AioFunctionContext(aio_function,
                                   queue_callback ? m_aio_finisher : NULL, c);
        ...
        int flushed() {
            if (--count == 0) {
                aio_finisher->
                    queue(new
                          FunctionContext(boost::
                                          bind(&finish_aio_completion, c, 0)));
                delete this;
            } return 0;
        }
    ...
    void TestRadosClient::flush_aio_operations(AioCompletionImpl * c) {
        ...
        for (size_t i = 0; i < m_finishers.size(); ++i) {
            AioFunctionContext *ctx =
                new AioFunctionContext(boost::
                                       bind(&WaitForFlush::flushed,
                                            wait_for_flush),
                                       nullptr, nullptr);
    ...
    int TestRadosClient::aio_watch_flush(AioCompletionImpl * c) {
        ...
        Context *ctx =
            new FunctionContext(boost::
                                bind(&TestRadosClient::finish_aio_completion,
                                     this, c, _1));
