...
template < class T > class RGWQuotaCache {
  ...
  public:
    ...
    virtual bool can_use_cached_stats(RGWQuotaInfo & quota,
                                      RGWStorageStats & stats);
    ...
    class AsyncRefreshHandler {
      ...
      public:
        AsyncRefreshHandler(RGWRados * _store,
                            RGWQuotaCache < T > *_cache):store(_store),
            cache(_cache) {
...
template < class T >
    bool RGWQuotaCache < T >::can_use_cached_stats(RGWQuotaInfo & quota,
                                                   RGWStorageStats &
                                                   cached_stats)
{
    if (quota.max_size >= 0) {
        if (quota.max_size_soft_threshold < 0) {
            quota.max_size_soft_threshold =
                quota.max_size *
                store->ctx()->_conf->rgw_bucket_quota_soft_threshold;
        }

        if (cached_stats.size_rounded >=
            (uint64_t) quota.max_size_soft_threshold) {
            ldout(store->ctx(),
                  20) <<
                "quota: can't use cached stats, exceeded soft threshold (size): "
                << cached_stats.size_rounded << " >= " << quota.
                max_size_soft_threshold << dendl;
            return false;
        }
    }
    ...
    if (quota.max_objects >= 0) {
        if (quota.max_objs_soft_threshold < 0) {
            quota.max_objs_soft_threshold =
                quota.max_objects *
                store->ctx()->_conf->rgw_bucket_quota_soft_threshold;
        }

        if (cached_stats.num_objects >=
            (uint64_t) quota.max_objs_soft_threshold) {
            ldout(store->ctx(),
                  20) <<
                "quota: can't use cached stats, exceeded soft threshold (num objs): "
                << cached_stats.num_objects << " >= " << quota.
                max_objs_soft_threshold << dendl;
            return false;
        }
    }
...
int BucketAsyncRefreshHandler::init_fetch()
{
    ...
    int r =
        store->get_bucket_instance_info(obj_ctx, bucket, bucket_info, NULL,
                                        NULL);
    if (r < 0) {
        ldout(store->ctx(),
              0) << "could not get bucket info for bucket=" << bucket << " r="
            << r << dendl;
        return r;
    }
...
class RGWBucketStatsCache:public RGWQuotaCache < rgw_bucket > {
  protected:
    ...
    int fetch_stats_from_storage(const rgw_user & user,
                                 const rgw_bucket & bucket,
                                 RGWStorageStats & stats) override;

  public:
  explicit RGWBucketStatsCache(RGWRados * _store):RGWQuotaCache < rgw_bucket > (_store,
                                  _store->ctx()->_conf->
...
int RGWBucketStatsCache::fetch_stats_from_storage(const rgw_user & user,
                                                  const rgw_bucket & bucket,
                                                  RGWStorageStats & stats)
{
    RGWBucketInfo bucket_info;
...
class UserAsyncRefreshHandler:public RGWQuotaCache <
    ...
    rgw_bucket bucket;
  public:
    UserAsyncRefreshHandler(RGWRados * _store,
                            RGWQuotaCache < rgw_user > *_cache,
                            const rgw_user & _user,
                            const rgw_bucket & _bucket):RGWQuotaCache <
        rgw_user >::AsyncRefreshHandler(_store, _cache),
        RGWGetUserStats_CB(_user), bucket(_bucket) {
    } void drop_reference() override {
        put();
    }
    int init_fetch() override;
...
{
    ldout(store->ctx(),
          20) << "initiating async quota refresh for user=" << user << dendl;
...
class RGWUserStatsCache:public RGWQuotaCache < rgw_user > {
    ...
    class BucketsSyncThread:public Thread {
      ...
      public:
        ...
        BucketsSyncThread(CephContext * _cct, RGWUserStatsCache * _s):cct(_cct),
            stats(_s), lock("RGWUserStatsCache::BucketsSyncThread") {
        } void *entry() override {
    ...
    class UserSyncThread:public Thread {
      ...
      public:
        ...
        UserSyncThread(CephContext * _cct, RGWUserStatsCache * _s):cct(_cct),
            stats(_s), lock("RGWUserStatsCache::UserSyncThread") {
        } void *entry() override {
  ...
  public:
  RGWUserStatsCache(RGWRados * _store, bool quota_threads):RGWQuotaCache < rgw_user > (_store,
                                _store->ctx()->_conf->
                                rgw_bucket_quota_cache_size),
    ...
    bool can_use_cached_stats(RGWQuotaInfo & quota,
                              RGWStorageStats & stats) override {
...
int RGWUserStatsCache::sync_bucket(const rgw_user & user, rgw_bucket & bucket)
{
    ...
    if (r < 0) {
        ldout(store->ctx(),
              0) << "ERROR: rgw_bucket_sync_user_stats() for user=" << user <<
...
int RGWUserStatsCache::sync_user(const rgw_user & user)
{
    ...
    when_need_full_sync +=
        make_timespan(store->ctx()->_conf->rgw_user_quota_sync_wait_time);
    ...
    const
{
    ...
    if (stats.num_objects + num_objs > static_cast < uint64_t >
        (qinfo.max_objects)) {
        dout(10) << "quota exceeded: stats.num_objects=" << stats.
...
class RGWQuotaHandlerImpl:public RGWQuotaHandler {
  ...
  RGWQuotaHandlerImpl(RGWRados * _store, bool quota_threads):store(_store),
    ...
    int check_quota(const rgw_user & user,
                    ...
                    uint64_t num_objs, uint64_t size) override {
        ...
        if (bucket_quota.enabled) {
            ...
            ret =
                check_quota("bucket", bucket_quota, bucket_stats, num_objs,
                            size);
            if (ret < 0) {
                return ret;
            }
        ...
        if (user_quota.enabled) {
            ...
            int ret =
                user_stats_cache.get_stats(user, bucket, user_stats,
                                           user_quota);
            if (ret < 0) {
                return ret;
            }
    ...
    int check_bucket_shards(uint64_t max_objs_per_shard, uint64_t num_shards,
                            ...
                            RGWQuotaInfo & bucket_quota, uint64_t num_objs,
        ...
        if (bucket_stats.num_objects + num_objs >
            num_shards * max_objs_per_shard) {
            ldout(store->ctx(),
                  0) << __func__ << ": resharding needed: stats.num_objects=" <<
                bucket_stats.
                num_objects << " shard max_objects=" << max_objs_per_shard *
                num_shards << dendl;
            need_resharding = true;
            if (suggested_num_shards) {
                *suggested_num_shards =
                    (bucket_stats.num_objects +
                     num_objs) * 2 / max_objs_per_shard;
            }
        }
        else {
