...
namespace rgw {
    namespace auth {
        namespace keystone {
            ...
            bool TokenEngine::is_applicable(const std::
                                            string & token)const noexcept {
                ...
                RGWValidateKeystoneToken validate(cct, "GET", "",
                                                  &token_body_bl);
                ...
                if (rgw::keystone::Service::
                    get_admin_token(dpp, cct, token_cache, config,
                                    admin_token) < 0) {
                    throw - EINVAL;
                }
                ...
                if (validate.get_http_status() ==
                    ...
                    ldpp_dout(dpp,
                        ...
                        " with " << validate.get_http_status() << dendl;
                    return boost::none;
                }
                ...
                ret =
                    token_body.parse(dpp, cct, token, token_body_bl,
            ...
            TokenEngine::auth_info_t
                TokenEngine::get_creds_info(const TokenEngine::
                                            token_envelope_t & token,
                                            const std::vector < std::string >
                                            &admin_roles)const noexcept {
            ...
            static inline const std::string
                make_spec_item(const std::string & tenant,
                               const std::string & id) {
                return tenant + ":" + id;
            }

            TokenEngine::acl_strategy_t
                TokenEngine::get_acl_strategy(const TokenEngine::
                                              token_envelope_t & token)const {
                ...
                return[allowed_items] (const rgw::auth::Identity::
                                       aclspec_t & aclspec) {
                    uint32_t perm = 0;
            ...
            TokenEngine::result_t
                ...
                static const struct RolesCacher {
                    explicit RolesCacher(CephContext * const cct) {
                        get_str_vec(cct->_conf->rgw_keystone_accepted_roles,
                                    plain);
                        get_str_vec(cct->_conf->
                                    rgw_keystone_accepted_admin_roles, admin);

                        /* Let's suppose that having an admin role implies also a regular one. */
                        plain.insert(std::end(plain), std::begin(admin),
                                     std::end(admin));
                ...
                if (t) {
                    ldpp_dout(dpp,
                        ...
                        << dendl;
                    auto apl =
                        apl_factory->create_apl_remote(cct, s,
                                                       get_acl_strategy(*t),
                                                       get_creds_info(*t,
                                                                      roles.
                                                                      admin));
                ...
                if (g_conf()->rgw_keystone_service_token_enabled
                    && !service_token.empty()) {
                    boost::optional < TokenEngine::token_envelope_t > st;

                    const auto & service_token_id =
                        rgw_get_token_id(service_token);
                    ldpp_dout(dpp,
                              20) << "service_token_id=" << service_token_id <<
                        dendl;
                    ...
                    if (st) {
                        ldpp_dout(dpp,
                                  20) << "cached service_token.project.id=" <<
                            st->get_project_id()
                            << dendl;
                        ...
                        ldpp_dout(dpp,
                            ...
                            << service_token_id << " was found in cache" <<
                            dendl;
                    }
                    else {
                        ...
                        st = get_from_keystone(dpp, service_token,
                                               allow_expired);
                      ...
                      for (const auto & role:service_token_roles.
                             plain)
                        {
                            if (st->has_role(role) == true) {
                                ...
                                ldpp_dout(dpp,
                                          20) <<
                                    "allowing expired tokens because service_token_id="
                                    << service_token_id << " is valid, role: "
                                    << role << dendl;
                                allow_expired = true;
                        ...
                        if (!allow_expired) {
                            ldpp_dout(dpp,
                                      0) <<
                                "service token user does not hold a matching role; required roles: "
                                << g_conf()->
                                rgw_keystone_service_token_accepted_roles <<
                                dendl;
                            return result_t::deny(-EPERM);
 ...
 */
            std::pair < boost::optional < rgw::keystone::TokenEnvelope >, int >
                EC2Engine::get_from_keystone(const DoutPrefixProvider * dpp,
                                             const std::
                                             string_view & access_key_id,
                                             const std::string & string_to_sign,
                                             const std::
                                             string_view & signature) const {
                ...
                std::string admin_token;
                int ret =
                    rgw::keystone::Service::get_admin_token(dpp, cct,
                                                            token_cache, config,
                                                            admin_token);
                if (ret < 0) {
                    ldpp_dout(dpp,
                              2) <<
                        "s3 keystone: cannot get token for keystone access" <<
                        dendl;
                    throw ret;
                }
                 ...
                 * parsed token_envelope_t::parse method. */
                ceph::bufferlist token_body_bl;
                RGWValidateKeystoneToken validate(cct, "POST", keystone_url,
                                                  &token_body_bl);
                ...
                credentials.dump_string("access",
                                        sview2cstr(access_key_id).data());
                credentials.dump_string("token",
                                        rgw::to_base64(string_to_sign));
                credentials.dump_string("signature",
                                        sview2cstr(signature).data());
                ...
                ret =
                    token_envelope.parse(dpp, cct, std::string(), token_body_bl,
                                         api_version);
                if (ret < 0) {
                    ldpp_dout(dpp,
                              2) << "s3 keystone: token parsing failed, ret=0"
                        << ret << dendl;
                    throw ret;
                }
            ...
            std::pair < boost::optional < std::string >,
                int >EC2Engine::
                get_secret_from_keystone(const DoutPrefixProvider * dpp,
                                         const std::string & user_id,
                                         const std::
                                         string_view & access_key_id) const {
                ...
                ceph::bufferlist token_body_bl;
                RGWGetAccessSecret secret(cct, "GET", keystone_url,
                                          &token_body_bl);
                ...
                JSONParser parser;
                if (!parser.
                    parse(token_body_bl.c_str(), token_body_bl.length())) {
                    ldpp_dout(dpp,
                              0) <<
                ...
                try {
                    if (!credential_iter.end()) {
                        JSONDecoder::decode_json("secret", secret_string,
                                                 *credential_iter, true);
                    }
                    else {
                        ldpp_dout(dpp,
                                  0) <<
 ...
 */
            auto EC2Engine::get_access_token(const DoutPrefixProvider * dpp,
                                             ...
                                             signature_factory)const
            ->access_token_result {
                using server_signature_t =
                ...
                /* Get a token from the cache if one has already been stored */
                 boost::optional < boost::tuple < rgw::keystone::TokenEnvelope,
                    std::string >> t =
                ...
                if (t) {
                    ...
                    ldpp_dout(dpp,
                              0) << "No stored secret string, cache miss" <<
                        dendl;
                }

                /* No cached token, token expired, or secret invalid: fall back to keystone */
                std::tie(token, failure_reason) =
                    get_from_keystone(dpp, access_key_id, string_to_sign,
                                      signature);

                if (token) {
                    /* Fetch secret from keystone for the access_key_id */
                    std::tie(secret, failure_reason) =
                        get_secret_from_keystone(dpp, token->get_user_id(),
                                                 access_key_id);

                    if (secret) {
                        /* Add token, secret pair to cache, and set timeout */
                        secret_cache.add(std::string(access_key_id), *token,
                                         *secret);
                    }
                }

                return {
                token, secret, failure_reason};
            }

            EC2Engine::acl_strategy_t
                EC2Engine::get_acl_strategy(const EC2Engine::
                                            token_envelope_t &)const {
                ...
                EC2Engine::get_creds_info(const EC2Engine::
                                          token_envelope_t & token,
                                          const std::vector < std::string >
                                          &admin_roles,
                                          const std::
                                          string & access_key_id)const noexcept
            {
                using acct_privilege_t =
                    rgw::auth::RemoteApplier::AuthInfo::acct_privilege_t;
            ...
            rgw::auth::Engine::result_t EC2Engine::
                authenticate(const DoutPrefixProvider * dpp,
              ...
              for (const auto & role:accepted_roles.plain) {
                ...
                if (!found) {
                    ldpp_dout(dpp,
                              5) <<
                        "s3 keystone: user does not hold a matching role;"
                        " required roles: " << cct->_conf->
                        rgw_keystone_accepted_roles << dendl;
                    return result_t::deny();
                }
                else {
            ...
            bool SecretCache::find(const std::string & token_id,
                ...
                map < std::string, secret_entry >::iterator iter =
                    secrets.find(token_id);
                if (iter == secrets.end()) {
                    return false;
                }
            ...
            void SecretCache::add(const std::string & token_id,
                ...
                map < string, secret_entry >::iterator iter =
                    secrets.find(token_id);
                if (iter != secrets.end()) {
