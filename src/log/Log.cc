...
namespace ceph {
    namespace logging {
        ...
        :m_indirect_this(NULL),
            ...
            m_max_recent(DEFAULT_MAX_RECENT), m_inject_segv(false) {
            int ret;
...
///
        ...
        Entry *Log::create_entry(int level, int subsys, size_t * expected_size) {
            if (true) {
                ANNOTATE_BENIGN_RACE_SIZED(expected_size,
                                           sizeof(*expected_size), "Log hint");
        ...
        void Log::_flush(EntryQueue * t, EntryQueue * requeue, bool crash) {
            ...
            while ((e = t->dequeue()) != NULL) {
                ...
                bool should_log = crash
                    || m_subs->get_log_level(sub) >= e->m_prio;
                ...
                if (do_fd || do_syslog || do_stderr) {
                    ...
                    buflen +=
                        e->m_stamp.sprintf(buf + buflen, buf_size - buflen);
                    buflen +=
                        snprintf(buf + buflen, buf_size - buflen, " %lx %2d ",
        ...
        void Log::dump_recent() {
            ...
            for (vector < Subsystem >::iterator p = m_subs->m_subsys.begin();
                 p != m_subs->m_subsys.end(); ++p) {
                snprintf(buf, sizeof(buf), "  %2d/%2d %s", p->log_level,
                         p->gather_level, p->name.c_str());
                _log_message(buf, true);
            }

            sprintf(buf, "  %2d/%2d (syslog threshold)", m_syslog_log,
                    m_syslog_crash);
            _log_message(buf, true);
            sprintf(buf, "  %2d/%2d (stderr threshold)", m_stderr_log,
                    m_stderr_crash);
            _log_message(buf, true);
