...
client_byte_throttler(new Throttle(g_ceph_context, "mgr_client_bytes",
                                   ...
                                   ("mgr_client_bytes"))),
client_msg_throttler(new
                     Throttle(g_ceph_context, "mgr_client_messages",
                              g_conf().get_val < uint64_t >
                              ("mgr_client_messages"))),
osd_byte_throttler(new
                   Throttle(g_ceph_context, "mgr_osd_bytes",
                            g_conf().get_val < Option::size_t >
                            ("mgr_osd_bytes"))),
osd_msg_throttler(new
                  Throttle(g_ceph_context, "mgr_osd_messsages",
                           g_conf().get_val < uint64_t > ("mgr_osd_messages"))),
mds_byte_throttler(new
                   Throttle(g_ceph_context, "mgr_mds_bytes",
                            g_conf().get_val < Option::size_t >
                            ("mgr_mds_bytes"))),
mds_msg_throttler(new
                  Throttle(g_ceph_context, "mgr_mds_messsages",
                           g_conf().get_val < uint64_t > ("mgr_mds_messages"))),
mon_byte_throttler(new
                   Throttle(g_ceph_context, "mgr_mon_bytes",
                            g_conf().get_val < Option::size_t >
                            ("mgr_mon_bytes"))),
mon_msg_throttler(new
                  Throttle(g_ceph_context, "mgr_mon_messsages",
                           g_conf().get_val < uint64_t > ("mgr_mon_messages"))),
msgr(nullptr), monc(monc_), finisher(finisher_), daemon_state(daemon_state_),
cluster_state(cluster_state_), py_modules(py_modules_), clog(clog_),
audit_clog(audit_clog_), pgmap_ready(false), timer(g_ceph_context, lock),
shutting_down(false), tick_event(nullptr),
...
int DaemonServer::init(uint64_t gid, entity_addrvec_t client_addrs)
{
    ...
    schedule_tick_locked(g_conf().get_val < std::chrono::seconds >
                         ("mgr_tick_period").count());

    return 0;
}

entity_addrvec_t DaemonServer::get_myaddrs() const const
{
...
int DaemonServer::ms_handle_authentication(Connection * con)
{
    ...
    dout(10) << __func__ << " new session " << s << " con " << con
        << " entity " << con->peer_name << " addr " << con->get_peer_addrs()
        << dendl;
    ...
    if (pgmap_ready.load()) {
        ...
        if (reported_osds.find(osd_id) == reported_osds.end()) {
            ...
            std::set_difference(up_osds.begin(), up_osds.end(),
                                ...
                                std::inserter(unreported_osds,
                                              unreported_osds.begin()));

            if (unreported_osds.size() == 0) {
                dout(4) << "all osds have reported, sending PG state to mon" <<
                    dendl;
                pgmap_ready = true;
                reported_osds.clear();
                ...
                dout(4) << "still waiting for " << unreported_osds.
                    size() << " osds" " to report in before PGMap is ready" <<
                    dendl;
            }
        }
    }
...
void DaemonServer::handle_osd_perf_metric_query_updated()
{
    ...
    finisher.queue(new LambdaContext([this] (int r) {
      std::lock_guard l(lock); for (auto & c:daemon_connections)
                                     {
                                     if (c->peer_is_osd()) {
                                     _send_configure(c);}
                                     }
                                  ...
                                  const std::string & daemon_name)
{
    if (!service_name.empty()) {
        ...
        return DaemonKey {
        ceph_entity_type_name(peer_type), daemon_name};
    }
}
...
bool DaemonServer::handle_open(const ref_t < MMgrOpen > &m)
{
    ...
    if (!daemon) {
        if (m->service_daemon) {
             ...
             */
            dout(2) << "ignoring open from " << key << " " << con->
                get_peer_addr()
    ...
    if (daemon) {
        ...
        if (m->service_daemon) {
            ...
            if (added || d->gid != (uint64_t) m->get_source().num()) {
                dout(10) << "registering " << key << " in pending_service_map"
                    << dendl;
        ...
        dout(20) << " got config_defaults_bl " << daemon->config_defaults_bl.
            length()
            << " bytes" << dendl;
    }
...
bool DaemonServer::handle_update(const ref_t < MMgrUpdate > &m)
{
    ...
    if (m->get_connection()->get_peer_type() == entity_name_t::TYPE_CLIENT &&
        ...
        dout(10) << "rejecting update request from non-daemon client " << m->
            daemon_name << dendl;
        clog->warn() << "rejecting report from non-daemon client " << m->
            daemon_name << " at " << m->get_connection()->get_peer_addrs();
...
void DaemonServer::update_task_status(DaemonKey key,
...

    {
        ...
        ceph_assert(daemon != nullptr);
        {
            ...
            if (m->get_connection()->peer_is_osd()
                ...
                daemon->daemon_health_metrics =
                    std::move(m->daemon_health_metrics);
                dout(10) << "daemon_health_metrics " << daemon->
                    daemon_health_metrics << dendl;
            }
        }
    }
...
void DaemonServer::_generate_command_map(cmdmap_t & cmdmap,
...
const MonCommand *DaemonServer::_get_mgrcommand(const string & cmd_prefix,
...
bool DaemonServer::_allowed_command(MgrSession * s,
...
class CommandContext {
  public:
    ...
    :m_tell {
    std::move(m)}, cmd(m_tell->cmd), data(m_tell->get_data()) {
    }
    explicit CommandContext(ceph::ref_t < MMgrCommand > m)
    :m_mgr {
    std::move(m)}, cmd(m_mgr->cmd), data(m_mgr->get_data()) {
    }
...
void DaemonServer::log_access_denied(std::shared_ptr < CommandContext > &cmdctx,
                                     MgrSession * session,
                                     std::stringstream & ss)
{
...
void DaemonServer::_check_offlines_pgs(const set < int >&osds,
                                       const OSDMap & osdmap,
                                       const PGMap & pgmap,
...
void DaemonServer::_maximize_ok_to_stop_set(const set < int >&orig_osds,
                                            unsigned max,
                                            const OSDMap & osdmap,
                                            const PGMap & pgmap,
...
bool DaemonServer::_handle_command(std::shared_ptr < CommandContext > &cmdctx)
{
    ...
    dout(10) << "decoded-size=" << cmdctx->cmdmap.
        size() << " prefix=" << prefix << dendl;
    ...
    boost::scoped_ptr < Formatter > f;
    {
        ...
        if (boost::algorithm::ends_with(prefix, "_json")) {
            ...
            format =
                cmd_getval_or < string > (cmdctx->cmdmap, "format", "plain");
        }
    ...
    if (prefix == "pg scrub" ||
        prefix == "pg repair" || prefix == "pg deep-scrub") {
      ...
      for (auto & con:p->second) {
        ...
        return true;
             ...
             prefix == "osd deep-scrub" || prefix == "osd repair") {
        string whostr;
      ...
      for (auto osd:osds) {
            ...
            cluster_state.
                with_osdmap_and_pgmap([&]
                  ...
                  for (auto pgid:    p->second)
                                      {
                                      int primary; spg_t spg;
                                      osdmap.get_primary_shard(pgid, &primary,
                                                               &spg);
                                      if (primary == osd) {
                                      spgs.push_back(spg);}
                                      }
        ...
        return true;
             ...
             prefix == "osd pool deep-scrub" || prefix == "osd pool repair") {
        ...
        cluster_state.with_osdmap([&](const OSDMap & osdmap) {
          epoch = osdmap.get_epoch(); for (auto & pool_name:pool_names)
                                  {
                                  auto pool_id =
                                  ...
                                  pg_t pg(i, pool_id); int primary; spg_t spg;
                                  auto got =
                                  osdmap.get_primary_shard(pg, &primary, &spg);
                                  if (!got)
                                  continue;
      ...
      for (auto & it:spgs_by_primary) {
        ...
        bool dry_run = prefix == "osd test-reweight-by-pg"
            || prefix == "osd test-reweight-by-utilization";
        int64_t oload =
            cmd_getval_or < int64_t > (cmdctx->cmdmap, "oload", 120);
        set < int64_t > pools;
        ...
        cluster_state.with_osdmap([&](const OSDMap & osdmap) {
          for (const auto & poolname:poolnames)
                                  {
                                  int64_t pool =
                                  osdmap.lookup_pg_pool_name(poolname);
                                  if (pool < 0) {
                                  ss << "pool '" << poolname <<
                                  "' does not exist"; r = -ENOENT;}
                                  pools.insert(pool);}
                                  }
        ) ;
        if (r) {
        ...
        double max_change =
            g_conf().get_val < double >("mon_reweight_max_change");
        ...
        int64_t max_osds =
            g_conf().get_val < int64_t > ("mon_reweight_max_osds");
        ...
        cmd_getval_compat_cephbool(cmdctx->cmdmap, "no_increasing",
                                   no_increasing);
        string out_str;
        ...
        if (r < 0) {
          ...
          for (const auto & osd_weight:new_weights) {
            ...
            monc->start_mon_command( {
                                    ...
                                    &on_finish->from_mon, &on_finish->outs,
                                    on_finish);
            return true;
        }
    }
        ...
        r = cluster_state.
            with_osdmap_and_pgmap([&]
                                  ...
                                  print_osd_utilization(osdmap, pgmap, ss,
                                                        ...
                                                        method == "tree",
                                                        filter);
                                  cmdctx->odata.append(ss); return 0;}
        ) ;
        cmdctx->reply(r, rs);
        return true;
    }
        ...
        r = cluster_state.
            with_osdmap_and_pgmap([&]
                                  ...
                                  ss << "there are no pools!"; goto stats_out;}
                                  }
          for (auto & p:         osdmap.get_pools()) {
                                  if (!one_pool) {
                                  poolid = p.first;}
                                  pg_map.dump_pool_stats_and_io_rate(poolid,
                                                                     osdmap,
                                                                     f.get(),
                                                                     &rs);
                                  if (one_pool) {
          ...
          stats_out:
                                  if (f) {
                                  f->close_section(); f->flush(cmdctx->odata);}
                                  else {
        ...
        if (r != -EOPNOTSUPP) {
            ...
            return true;
             ...
             prefix == "osd destroy" || prefix == "osd purge") {
        set < int >osds;
        int r = 0;
        if (prefix == "osd safe-to-destroy") {
            ...
            cluster_state.with_osdmap([&](const OSDMap & osdmap) {
                                      r =
                                      osdmap.parse_osd_id_list(ids, &osds,
                                                               &ss);});
            if (!r && osds.empty()) {
        ...
        cluster_state.
            with_osdmap_and_pgmap([&]
                                  ...
                                  " any conclusions"; r = -EAGAIN; return;}
          int num_active_clean = 0; for (auto & p:pg_map.num_pg_by_state)
                                  {
                                  unsigned want =
          ...
          for (auto osd:         osds) {
                                  ...
                                  safe_to_destroy.insert(osd); continue;    // clearly safe to destroy
                                  }
                                  auto q = pg_map.num_pg_by_osd.find(osd);
                                  if (q != pg_map.num_pg_by_osd.end()) {
                                  if (q->second.acting > 0
                                      || q->second.up_not_acting > 0) {
                                  active_osds.insert(osd);
                                  // XXX: For overlapping PGs, this counts them again
                                  affected_pgs +=
                                  q->second.acting + q->second.up_not_acting;
                                  continue;}
                                  }
                                  ...
                                  if (p == pg_map.osd_stat.end()
                                      || !osdmap.is_up(osd)) {
                                  missing_stats.insert(osd); continue;}
                                  else
                                  if (p->second.num_pgs > 0) {
                                  stored_pgs.insert(osd); continue;}
                                  }
        ...
        monc->start_mon_command( {
                                cmd}, {
                                }, nullptr, &on_finish->outs, on_finish);
        return true;
    }
        ...
        cluster_state.with_osdmap([&](const OSDMap & osdmap) {
                                  r =
                                  osdmap.parse_osd_id_list(ids, &osds, &ss);});
        if (!r && osds.empty()) {
            ss << "must specify one or more OSDs";
            r = -EINVAL;
        }
        ...
        cluster_state.
            with_osdmap_and_pgmap([&]
                                  (const OSDMap & osdmap,
                                   const PGMap & pg_map) {
                                  _maximize_ok_to_stop_set(osds, max, osdmap,
                                                           pg_map,
                                                           &out_report);});
        if (!f) {
        ...
        if (!out_report.ok_to_stop()) {
            ss << "unsafe to stop osd(s) at this time (" << out_report.not_ok.
...
case OFR_RECOVERY:
                                 ...
                                 // not fatal.
                                 ss << "pg " << i <<
                                 " doesn't require recovery; "; continue;}
                                 else
                                 ...
                                 r = -EINVAL; continue;}
break; case OFR_BACKFILL:
                                 if ((state & (PG_STATE_DEGRADED |
                                               PG_STATE_BACKFILL_WAIT |
                                               PG_STATE_BACKFILLING)) == 0) {
                                 ss << "pg " << i <<
                                 " doesn't require backfilling; "; continue;}
                                 else
                                 if (state & PG_STATE_FORCED_BACKFILL) {
                                 ss << "pg " << i <<
                                 " backfill already forced; "; r = -EINVAL;
                                 continue;}
break; case OFR_BACKFILL | OFR_CANCEL:
                                 if ((state & PG_STATE_FORCED_BACKFILL) == 0) {
                                 ss << "pg " << i << " backfill not forced; ";
                                 continue;}
break; case OFR_RECOVERY | OFR_CANCEL:
                                 if ((state & PG_STATE_FORCED_RECOVERY) == 0) {
                                 ss << "pg " << i << " recovery not forced; ";
                                 continue;}
break; default:
                                 ceph_abort_msg
                                 ("actual_op value is not supported");}
                                 pgs.push_back(i);} // for
                                 }
        ) ;
        ...
        cluster_state.with_osdmap([&](const OSDMap & osdmap) {
          ...
          map < int, vector < spg_t >> osdpgs; for (auto & pgid:pgs)
                                  {
                                  int primary; spg_t spg;
                                  if (osdmap.
                                      get_primary_shard(pgid, &primary, &spg)) {
          ...
          for (auto & i:         osdpgs) {
                                  ...
                                  if (p == osd_cons.end()) {
                                  ss << "osd." << i.
                                  first << " is not currently connected";
                                  r = -EAGAIN; continue;}
                  for (auto & con:p->second) {
                                  con->
                                  send_message(new
                                               MOSDForceRecovery(monc->
                                                                 get_fsid(),
                                                                 i.second,
                                                                 actual_op));}
                                  ss << "instructing pg(s) " << i.
                                  second << " on osd." << i.
                                  first << " to " << forceop << "; ";}
                                  }
                                  }
        ...
        if (cmd_getval(cmdctx->cmdmap, "key", name)) {
            ...
            if (f) {
                ...
                tbl.define_column("OVERRIDES", TextTable::LEFT,
                                  TextTable::LEFT);
            ...
            if (prefix == "config show") {
              ...
              for (auto & i:daemon->config) {
                    ...
                    if (f) {
                        ...
                        f->dump_string("source",
                                       ceph_conf_level_name(i.second.rbegin()->
                                                            first));
                        if (i.second.size() > 1) {
              ...
              for (auto & i:defaults) {
                    ...
                    if (j != daemon->config.end() && !j->second.empty()) {
                        ...
                        if (f) {
                            ...
                            tbl << ceph_conf_level_name(j->second.rbegin()->
                                                        first);
                            if (j->second.size() > 1) {
                                list < string > ov;
                                auto k = j->second.rend();
                                for (--k; k != j->second.rbegin(); --k) {
                                    if (k->second == j->second.rbegin()->second) {
                                        ...
                                        ov.push_front(ceph_conf_level_name
                                                      (k->first) + string("[") +
                                                      k->second + string("]"));
                                    }
                                }
        ...
        if (f) {
            ...
            tbl.define_column("LIFE EXPECTANCY", TextTable::LEFT,
                              TextTable::LEFT);
            auto now = ceph_clock_now();
            daemon_state.with_devices([&tbl, now] (const DeviceState & dev) {
              string h; for (auto & i:dev.attachments)
                                      {
                                      if (h.size()) {
                                      h += " ";}
                                      h +=
                                      std::get < 0 > (i) + ":" + std::get < 1 >
                                      (i);}
              string d; for (auto & i:dev.daemons)
                                      {
                                      if (d.size()) {
                                      d += " ";}
                                      d += to_string(i);}
                                      char wear_level_str[16] = {
                                      0}; if (dev.wear_level >= 0) {
                                      snprintf(wear_level_str,
                                               sizeof(wear_level_str) - 1,
                                               "%d%%",
                                               (int)(100.1 * dev.wear_level));}
                                      tbl << dev.
                                      devid << h << d << wear_level_str << dev.
            ...
            if (dm) {
                if (f) {
                  ...
                  for (auto & i:dm->devices) {
                    ...
                    TextTable tbl;
                    tbl.define_column("DEVICE", TextTable::LEFT,
                                      TextTable::LEFT);
                    tbl.define_column("HOST:DEV", TextTable::LEFT,
                                      TextTable::LEFT);
        ...
        if (f) {
          ...
          for (auto & devid:devids) {
            ...
            auto now = ceph_clock_now();
          for (auto & devid:devids) {
                daemon_state.with_device(devid,
                                         [&tbl, &host,
                                          now] (const DeviceState & dev) {
                  string n; for (auto & j:dev.attachments)
                                         {
                                         if (std::get < 0 > (j) == host) {
                                         if (n.size()) {
                                         n += " ";}
        ...
        if (!from.parse(from_str)) {
            ...
            daemon_state.with_device_create(devid,
                                            [from, to,
                                             &meta] (DeviceState & dev) {
                                            dev.set_life_expectancy(from, to,
                                                                    ceph_clock_now
                                                                    ());
                                            meta = dev.metadata;}
            );
          ...
          for (auto & i:meta) {
            ...
            auto on_finish = new ReplyOnFinish(cmdctx);
            monc->start_mon_command( {
                                    cmd}, json, nullptr, nullptr, on_finish);
        }
        return true;
    }
        ...
        if (daemon_state.with_device_write(devid,[&meta] (DeviceState & dev) {
            ...
            monc->start_mon_command( {
                                    cmd}
                                    , json, nullptr, nullptr, on_finish);
        }
        else {
            cmdctx->reply(0, ss);
        }
        ...
        r = cluster_state.
            with_osdmap_and_pgmap([&]
                                  (const OSDMap & osdmap,
                                   const PGMap & pg_map) {
                                  return process_pg_map_command(prefix,
                                                                cmdctx->cmdmap,
                                                                pg_map, osdmap,
                                                                f.get(), &ss,
                                                                &cmdctx->
    ...
    dout(10) << "passing through command '" << prefix << "' size " << cmdctx->
        cmdmap.size() << dendl;
    Finisher & mod_finisher = py_modules.get_active_module_finisher(mod_name);
    mod_finisher.
        queue(new LambdaContext([this, cmdctx, session, py_command, prefix]
                                (int r_)mutable {
                                std::stringstream ss;
                                dout(10) << "dispatching command '" << prefix <<
                                "' size " << cmdctx->cmdmap.size() << dendl;
                                // Validate that the module is enabled
                                auto & py_handler_name = py_command.module_name;
                                PyModuleRef module =
                                py_modules.get_module(py_handler_name);
                                ceph_assert(module);
                                if (!module->is_enabled()) {
                                ss << "Module '" << py_handler_name <<
                                "' is not enabled (required by " "command '" <<
                                prefix << "'): use `ceph mgr module enable " <<
                                py_handler_name << "` to enable it";
                                dout(4) << ss.str() << dendl;
                                cmdctx->reply(-EOPNOTSUPP, ss); return;}
                                ...
                                std::string self_test_prefix =
                                py_handler_name + " " + "self-test";
                                // Validate that the module is healthy
                                bool accept_command; if (module->is_loaded()) {
                                if (module->get_can_run()
                                    && !module->is_failed()) {
                                // Healthy module
                                accept_command = true;}
                                else
                                if (self_test_prefix == prefix) {
                                ...
                                if (!accept_command) {
                                dout(4) << ss.str() << dendl;
                                cmdctx->reply(-EIO, ss); return;}

                                std::stringstream ds;
                                bufferlist inbl = cmdctx->data;
                                int r =
                                py_modules.handle_command(py_command, *session,
                                                          cmdctx->cmdmap, inbl,
                                                          &ds, &ss);
                                if (r == -EACCES) {
                                log_access_denied(cmdctx, session, ss);}
...
{
    if (!pgmap_ready) {
        if (ceph_clock_now() - started_at > g_conf().get_val < int64_t >
            ("mgr_stats_period") * 4.0) {
            pgmap_ready = true;
            reported_osds.clear();
    ...
    cluster_state.with_mutable_pgmap([&](PGMap & pg_map) {
                                     ...
                                     if (pending_service_map_dirty >=
                                         pending_service_map.epoch) {
                                     pending_service_map.modified =
                                     ceph_clock_now();
                                     encode(pending_service_map,
                                            m->service_map_bl,
                                            CEPH_FEATURES_ALL);
                                     dout(10) << "sending service_map e" <<
                                     pending_service_map.epoch << dendl;
                                     pending_service_map.epoch++;}
                                     }
                                     ...
                                     with_osdmap([&](const OSDMap & osdmap) {
                                                 ...
                                                 pg_map.
                                                 get_health_checks
                                                 (g_ceph_context, osdmap,
                                                  &m->health_checks);
                                                 dout(10) << m->health_checks.
                                                 checks.
                                                 size() << " health checks" <<
                                                 dendl;
                                                 dout(20) << "health checks:\n";
                                                 JSONFormatter jf(true);
                                                 jf.dump_object("health_checks",
                                                                m->
                                                                health_checks);
                                                 jf.flush(*_dout);
                                                 *_dout << dendl;
    ...
    map < daemon_metric,
        unique_ptr < DaemonHealthMetricCollector >> accumulated;
  for (auto service:{
         "osd", "mon"}
    ) {
                ...
                if (acc == accumulated.end()) {
                    ...
                    if (!collector) {
                        derr << __func__ << " " << key
                            ...
                            << std::hex << static_cast < uint8_t >
                            (metric.get_type())
                            << std::dec << dendl;
                        continue;
                    }
                    tie(acc, std::ignore) =
                        accumulated.emplace(metric.get_type(),
                                            std::move(collector));
                }
...
void DaemonServer::adjust_pgs()
{
    ...
    double max_misplaced =
        g_conf().get_val < double >("target_max_misplaced_ratio");
    bool aggro =
        g_conf().get_val < bool > ("mgr_debug_aggressive_pg_num_changes");

    map < string, unsigned >pg_num_to_set;
    map < string, unsigned >pgp_num_to_set;
    set < pg_t > upmaps_to_clear;
    cluster_state.
        with_osdmap_and_pgmap([&](const OSDMap & osdmap, const PGMap & pg_map) {
      unsigned creating_or_unknown = 0; for (auto & i:pg_map.num_pg_by_state)
                              {
                              ...
                              dout(10) << "creating_or_unknown " <<
                              creating_or_unknown << " max_creating " << max <<
                              " left " << left << dendl;
      ...
      double misplaced_ratio, degraded_ratio; double inactive_pgs_ratio, unknown_pgs_ratio; pg_map.get_recovery_stats(&misplaced_ratio, &degraded_ratio, &inactive_pgs_ratio, &unknown_pgs_ratio); dout(20) << "misplaced_ratio " << misplaced_ratio << " degraded_ratio " << degraded_ratio << " inactive_pgs_ratio " << inactive_pgs_ratio << " unknown_pgs_ratio " << unknown_pgs_ratio << "; target_max_misplaced_ratio " << max_misplaced << dendl; for (auto & i:osdmap.get_pools())
                              {
                              ...
                              << p.get_pgp_num()
                              << dendl; ok = false;}
          vector < int32_t > source_acting; for (auto & merge_participant:{
                                                                     merge_source,
                                                                     merge_target}
                              ) {
                              bool is_merge_source =
                              merge_participant == merge_source;
                              if (osdmap.have_pg_upmaps(merge_participant)) {
                              dout(10) << "pool " << i.
                              first << " pg_num_target " << p.
                              get_pg_num_target()
                              << " pg_num " << p.get_pg_num()
                              ...
                              if ((q->second.
                                   state & (PG_STATE_ACTIVE | PG_STATE_CLEAN))
                                  != (PG_STATE_ACTIVE | PG_STATE_CLEAN)) {
                              dout(10) << "pool " << i.
                              first << " pg_num_target " << p.
                              get_pg_num_target()
                              ...
                              pg_state_string(q->second.state)
                              << ")" << dendl; ok = false;}
                              if (is_merge_source) {
                              source_acting = q->second.acting;}
                              else
                              if (ok && q->second.acting != source_acting) {
                              dout(10) << "pool " << i.first
                              << " pg_num_target " << p.get_pg_num_target()
                              << " pg_num " << p.get_pg_num()
                              << (is_merge_source ? " - merge source " :
                              ...
                              << " and " << merge_target
                              << ")" << dendl;
                              pg_num_to_set[osdmap.get_pool_name(i.first)] =
                              target; continue;}
                              }
                              else
                              if (p.get_pg_num_target() > p.get_pg_num()) {
                              ...
                              bool active = true;
                              auto q =
                              pg_map.num_pg_by_pool_state.find(i.first);
                              if (q != pg_map.num_pg_by_pool_state.end()) {
              for (auto & j: q->second) {
                              if ((j.
                                   first & (PG_STATE_ACTIVE | PG_STATE_PEERED))
                                  == 0) {
                              dout(20) << "pool " << i.first << " has " << j.
                              second << " pgs in " << pg_state_string(j.first)
                              << dendl; active = false; break;}
                              }
                              }
                              else {
                              active = false;}
                              unsigned pg_gap =
                              p.get_pg_num() - p.get_pgp_num();
                              unsigned max_jump =
                              cct->_conf->mgr_max_pg_num_change; if (!active) {
                              dout(10) << "pool " << i.
                              first << " pg_num_target " << p.
                              get_pg_num_target()
                              << " pg_num " << p.get_pg_num()
                              ...
                              unsigned add =
                              std::min(std::min(left, max_jump - pg_gap),
                              ...
                              << " pgp_num " << p.get_pgp_num()
                              << " - increase blocked by pg_num " << p.
                              get_pg_num()
                              << dendl;}
                              else
                              if (!aggro && (inactive_pgs_ratio > 0 ||
                                             degraded_ratio > 0 ||
                                             unknown_pgs_ratio > 0)) {
                              dout(10) << "pool " << i.first
                              << " pgp_num_target " << p.get_pgp_num_target()
                              ...
                              if (!aggro && (misplaced_ratio > max_misplaced)) {
                              dout(10) << "pool " << i.first
                              << " pgp_num_target " << p.get_pgp_num_target()
                              << " pgp_num " << p.get_pgp_num()
                              ...
                              MAX_NUM_OBJECTS_PER_PG_FOR_LEAP = 1;
                              pool_stat_t s =
                              pg_map.get_pg_pool_sum_stat(i.first); if (aggro ||
                              ...
                              double room =
                              std::min <
                              double >(max_misplaced - misplaced_ratio,
                                       max_misplaced / 2.0);
                              unsigned estmax =
                              std::max <
                              unsigned >((double)p.get_pg_num() * room, 1u);
                              unsigned next_min = 0;
                              ...
                              dout(20) << " room " << room << " estmax " <<
                              estmax << " delta " << (target - p.get_pgp_num())
                              << " next " << next << dendl;
                              if (p.get_pgp_num_target() ==
                                  p.get_pg_num_target()
                              ...
                              << " pgp_num " << p.get_pgp_num()
                              << " -> " << next << dendl;
                              pgp_num_to_set[osdmap.get_pool_name(i.first)] =
                              next;}
                              }
                              }
                              if (left == 0) {
...
void DaemonServer::got_service_map()
{
    ...
    cluster_state.with_servicemap([&](const ServiceMap & service_map) {
                                  ...
                                  dout(10) << "got newer initial map e" <<
                                  service_map.epoch << dendl;
                                  ceph_assert(pending_service_map_dirty == 0);
                                  pending_service_map = service_map;
                                  pending_service_map.epoch =
  ...
  for (auto &[type, service]:pending_service_map.services) {
      ...
      for (auto & q:service.daemons) {
            ...
            if (!daemon_state.exists(key)) {
                auto daemon =
                    std::make_shared < DaemonState > (daemon_state.types);
                daemon->key = key;
...
void DaemonServer::got_mgr_map()
{
    ...
    cluster_state.with_mgrmap([&](const MgrMap & mgrmap) {
                              ...
                              DaemonKey key {
                              "mgr", mgrmap.active_name};
                              have.insert(mgrmap.active_name);
                              if (!daemon_state.exists(key)
                                  && !daemon_state.is_updating(key)) {
                              md_update(key);
                              dout(10) << "triggered addition of " << key <<
...
const char **DaemonServer::get_tracked_conf_keys() const const
{
                                      ...
                                      const std::set < std::string > &changed)
{
    ...
    if (changed.count("mgr_stats_threshold")
        ...
        dout(4) << "Updating stats threshold/period on " << daemon_connections.
            size() << " clients" << dendl;
        // Send a fresh MMgrConfigure to all clients, so that they can follow
        // the new policy for transmitting stats
        finisher.queue(new LambdaContext([this] (int r) {
...
void DaemonServer::_send_configure(ConnectionRef c)
{
    ...
    configure->stats_threshold =
        g_conf().get_val < int64_t > ("mgr_stats_threshold");
    ...
    if (c->peer_is_osd()) {
        ...
        configure->metric_config_message =
            MetricConfigMessage(MDSConfigPayload
                                (mds_perf_metric_collector.get_queries()));
    }
