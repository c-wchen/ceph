...
class RGWCompletionManager:public RefCountedObject {
  ...
  protected:
    ...
    void _complete(RGWAioCompletionNotifier * cn, const rgw_io_id & io_id,
                   void *user_info);
  public:
    explicit RGWCompletionManager(CephContext * _cct);
    virtual ~ RGWCompletionManager()override;

    void complete(RGWAioCompletionNotifier * cn, const rgw_io_id & io_id,
                  void *user_info);
...
class RGWAioCompletionNotifier:public RefCountedObject {
  ...
  public:
     RGWAioCompletionNotifier(RGWCompletionManager * _mgr,
                              const rgw_io_id & _io_id, void *_user_data);
...
struct RGWCoroutinesEnv {
    ...
    RGWCoroutinesStack *stack;
     ...
     RGWCoroutinesEnv():run_context(0), manager(NULL), scheduled_stacks(NULL),
        stack(NULL) {
}};
...
#define MAX_COROUTINE_HISTORY 10
    ...
    struct Status {
        ...
        explicit Status(CephContext * _cct):cct(_cct),
  ...
  RGWCoroutine(CephContext * _cct):status(_cct), _yield_ret(false), cct(_cct), stack(NULL), retcode(0),
    ...
    bool drain_children(int num_cr_left, RGWCoroutinesStack * skip_stack = nullptr, std::optional < std::function < void (uint64_t stack_id, int ret) > >cb = std::nullopt);    /* returns true if needed to be called again,
                                                                                                                                                                                   cb will be called on completion of every
                                                                                                                                                                                   completion. */
    bool drain_children(int num_cr_left, std::optional < std::function < int (uint64_t stack_id, int ret) > >cb);   /* returns true if needed to be called again,
...
class RGWCoroutinesStack:public RefCountedObject {
  ...
  protected:
    ...
    RGWCoroutinesStack *spawn(RGWCoroutine * source_op, RGWCoroutine * next_op,
                              bool wait);
  ...
  public:
     RGWCoroutinesStack(CephContext * _cct, RGWCoroutinesManager * _ops_mgr,
...
class RGWCoroutinesManager {
    ...
    std::atomic < int64_t > max_io_id = {
    0};
    std::atomic < uint64_t > max_stack_id = {
    0};
    ...
    void handle_unblocked_stack(std::set <
                                RGWCoroutinesStack * >&context_stacks,
                                std::list <
                                RGWCoroutinesStack * >&scheduled_stacks,
                                RGWCompletionManager::io_completion & io,
                                int *waiting_count, int *interval_wait_count);
  protected:
    RGWCompletionManager * completion_mgr;
  ...
  RGWCoroutinesManager(CephContext * _cct, RGWCoroutinesManagerRegistry * _cr_registry):cct(_cct),
    ...
    int run(const DoutPrefixProvider * dpp,
            std::list < RGWCoroutinesStack * >&ops);
    ...
    RGWAioCompletionNotifier *create_completion_notifier(RGWCoroutinesStack *
                                                         stack);
    template < typename T >
        RGWAioCompletionNotifier *
        create_completion_notifier(RGWCoroutinesStack * stack, T value);
    RGWCompletionManager *get_completion_mgr() {
                                                     ...
                                                     T value)
{
    ...
    RGWAioCompletionNotifier *cn =
        new RGWAioCompletionNotifierWith < T > (completion_mgr, io_id,
                                                (void *)stack,
                                                std::move(value));
