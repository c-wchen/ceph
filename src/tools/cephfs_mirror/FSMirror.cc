...
namespace cephfs {
    namespace mirror {
        ...
        namespace {
            ...
            const std::
                string SERVICE_DAEMON_PEER_INIT_FAILED_KEY("peer_init_failed");
          ...
          public:
            ...
            :admin_socket(cct->get_admin_socket()) {
                ...
                // mirror status format is name@fscid
                 cmd =
                    "fs mirror status " + stringify(filesystem.fs_name) + "@" +
                    stringify(filesystem.fscid);
                 r = admin_socket->register_command(cmd, this,
                                                    "get filesystem mirror status");
                if (r == 0) {
        ...
        :m_cct(cct), m_filesystem(filesystem), m_pool_id(pool_id),
            m_service_daemon(service_daemon), m_args(args),
            m_work_queue(work_queue), m_snap_listener(this),
            m_asok_hook(new MirrorAdminSocketHook(cct, filesystem, this)) {
            m_service_daemon->add_or_update_fs_attribute(m_filesystem.fscid,
        ...
        void FSMirror::reopen_logs() {
            ...
            if (m_cluster) {
                reinterpret_cast <
                    CephContext * >(m_cluster->cct())->reopen_logs();
            }
        ...
        void FSMirror::shutdown(Context * on_finish) {
...

            {
                ...
                if (m_on_init_finish != nullptr) {
                    dout(10) << ": delaying shutdown -- init in progress" <<
                        dendl;
        ...
        void FSMirror::shutdown_peer_replayers() {
          ...
          for (auto &[peer, peer_replayer]:m_peer_replayers) {
                dout(5) << ": shutting down replayer for peer=" << peer <<
                    dendl;
        ...
        void FSMirror::init_instance_watcher(Context * on_finish) {
            ...
            m_instance_watcher =
                InstanceWatcher::create(m_ioctx, m_snap_listener, m_work_queue);
        ...
        void FSMirror::init_mirror_watcher() {
            ...
            m_mirror_watcher =
                MirrorWatcher::create(m_ioctx, this, m_work_queue);
        ...
        void FSMirror::shutdown_instance_watcher() {
            ...
            m_instance_watcher->shutdown(new C_AsyncCallback < ContextWQ >
                                         (m_work_queue, ctx));
        }
        ...
        void FSMirror::handle_release_directory(string_view dir_path) {
...

            {
                ...
                if (it != m_directories.end()) {
                    ...
                    m_service_daemon->add_or_update_fs_attribute(m_filesystem.
                                                                 fscid,
                                                                 SERVICE_DAEMON_DIR_COUNT_KEY,
                                                                 m_directories.
                                                                 size());
        ...
        void FSMirror::add_peer(const Peer & peer) {
            ...
            auto replayer =
                std::make_unique < PeerReplayer > (m_cct, this, m_cluster,
                                                   m_filesystem, peer,
                                                   m_directories, m_mount,
                                                   m_service_daemon);
        ...
        void FSMirror::remove_peer(const Peer & peer) {
            ...
            if (replayer) {
                dout(5) << ": shutting down replayers for peer=" << peer <<
                    dendl;
