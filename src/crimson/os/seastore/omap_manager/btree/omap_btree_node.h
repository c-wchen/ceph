...
namespace crimson::os::seastore::omap_manager {
    ...
    struct OMapNode:LogicalCachedExtent {
        ...
        struct mutation_result_t {
            ...
            std::optional < std::tuple < OMapNodeRef, OMapNodeRef,
                std::string >> split_tuple;
            /// only sopulated if need merged, indicate which entry need be doing merge in upper layer.
            std::optional < OMapNodeRef > need_merge;

            mutation_result_t(mutation_status_t s,
                              std::optional < std::tuple < OMapNodeRef,
                              OMapNodeRef, std::string >> tuple,
        ...
        virtual full_merge_ret make_full_merge(omap_context_t oc,
                                               OMapNodeRef right) = 0;
        ...
        virtual make_balanced_ret make_balanced(omap_context_t oc,
                                                OMapNodeRef _right) = 0;

        virtual omap_node_meta_t get_node_meta() const = 0;
        virtual bool extent_will_overflow(size_t ksize,
                                          std::optional < size_t >
                                          vsize) const = 0;
...
template <> struct fmt::formatter <
    crimson::os::seastore::omap_manager::OMapNode >:fmt::ostream_formatter {
};
#endif
