...
namespace librbd {
    namespace image {
        ...
        } // anonymous namespace using util::create_rados_callback;
        using util::create_context_callback;
        using util::create_async_context_callback;

         template < typename I >
            ValidatePoolRequest <
            I >::ValidatePoolRequest(librados::IoCtx & io_ctx,
                                     Context * on_finish)
        ...
        template < typename I >
            ...
            if (r == -EOPNOTSUPP) {
                ...
                lderr(m_cct) << "failed to write RBD info: " << cpp_strerror(r)
                    << dendl;
                m_ret_val = r;
            }
        ...
        template < typename I >
            ...
            if (r < 0) {
                // not a fatal error
                lderr(m_cct) << "failed to remove validation snapshot: " <<
                    cpp_strerror(r)
                    << dendl;
            }
