...
#define dout_prefix *_dout << "librbd::migration::util::" << __func__ << ": "
            ...
            int parse_url(CephContext * cct, const std::string & url,
                ...
                *url_spec = UrlSpec {
                ...
                if (scheme == "http" || scheme == "") {
                    ...
                    lderr(cct) << "invalid url scheme: '" << url << "'" <<
                        dendl;
                    return -EINVAL;
                }
                ...
                if (port.matched) {
                    try {
                        ...
                        lderr(cct) << "invalid url port: '" << url << "'" <<
                            dendl;
                        return -EINVAL;
                    }
                    url_spec->port = port;
            ...
            void zero_shrunk_snapshot(CephContext * cct,
                ...
                if (*previous_size && **previous_size > new_size) {
                    ldout(cct,
                          20) << "snapshot resize " << **previous_size << " -> "
                        << new_size << dendl;
                  ...
                  for (auto & image_extent:image_extents) {
                        ...
                        image_interval.insert(image_extent.first,
                                              image_extent.second);
            ...
            void merge_snapshot_delta(const io::SnapIds & snap_ids,
                ...
                io::SnapshotDelta orig_snapshot_delta =
                    std::move(*snapshot_delta);
              ...
              for (auto &[snap_key, sparse_extents]:orig_snapshot_delta)
                {
                    ...
                    while (snap_id_it != snap_ids.end()
                           && *snap_id_it < snap_key.first) {
                        ++snap_id_it;
                    }
                  ...
                  for (auto prev_sparse_extents:pending_sparse_extents)
                    ...
                    auto write_read_snap_ids =
                        std::make_pair(*snap_id_it, snap_key.second);
                    (*snapshot_delta)[write_read_snap_ids] =
                        std::move(sparse_extents);
                    ...
                    if (write_read_snap_ids.first > snap_key.first) {
                        ...
                        pending_sparse_extents.
                            push_back(&(*snapshot_delta)[write_read_snap_ids]);
                    }
                    else {
