    ...
    namespace mirror {
        namespace image_deleter {
            ...
            class TestMockImageDeleterTrashWatcher:public TestMockFixture {
              public:
                typedef TrashWatcher < librbd::MockTestImageCtx >
                    MockTrashWatcher;
                ...
                void expect_work_queue(MockThreads & mock_threads) {
                    ...
                    expect_trash_watcher_is_unregistered(MockLibrbdTrashWatcher
                                                         & mock_trash_watcher,
                                                         bool unregistered) {
                    EXPECT_CALL(mock_trash_watcher, is_unregistered())
                        .WillOnce(Return(unregistered));
                } void expect_trash_watcher_register(MockLibrbdTrashWatcher &
                                                     mock_trash_watcher,
                                                     int r) {
                    EXPECT_CALL(mock_trash_watcher, register_watch(_))
                        .WillOnce(CompleteContext(r));
                } void expect_trash_watcher_unregister(MockLibrbdTrashWatcher &
                                                       mock_trash_watcher,
                                                       int r) {
                ...
                void expect_trash_list(librados::IoCtx & io_ctx,
                    ...
                    EXPECT_CALL(get_mock_io_ctx(io_ctx),
                                exec(RBD_TRASH, _, StrEq("rbd"),
                                     StrEq("trash_list"), ContentsEqual(bl), _,
                                     _, _))
                        .
                        WillOnce(DoAll
                                 (WithArg < 5 >
                                  (Invoke([out_bl] (bufferlist * bl) {
                                          *bl = out_bl;})), Return(r)));
                }
                ...
                void expect_handle_trash_image(MockListener & mock_listener,
                    ...
                    EXPECT_CALL(mock_listener,
                                handle_trash_image(global_image_id, _));
                }
            ...
            TEST_F(TestMockImageDeleterTrashWatcher, EmptyPool) {
                ...
                expect_trash_watcher_is_unregistered(mock_librbd_trash_watcher,
                                                     true);
                expect_trash_watcher_register(mock_librbd_trash_watcher, 0);
                expect_trash_list(m_local_io_ctx, "", {
                                  }
                                  , 0);

                MockListener mock_listener;
                MockTrashWatcher mock_trash_watcher(m_local_io_ctx,
                                                    &mock_threads,
                                                    mock_listener);
                C_SaferCond ctx;
            ...
            TEST_F(TestMockImageDeleterTrashWatcher, NonEmptyPool) {
                ...
                expect_trash_list(m_local_io_ctx, "image999", std::move(images),
                                  0);
            ...
            TEST_F(TestMockImageDeleterTrashWatcher, RegisterWatcherBlocklist) {
                ...
                expect_trash_watcher_is_unregistered(mock_librbd_trash_watcher,
                                                     true);
                expect_trash_watcher_register(mock_librbd_trash_watcher,
                                              -EBLOCKLISTED);
            ...
            TEST_F(TestMockImageDeleterTrashWatcher, RewatchBlocklist) {
                ...
                LibrbdTrashWatcher::get_instance().
