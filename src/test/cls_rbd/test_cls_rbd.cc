...
TEST_F(TestClsRbd, sparse_copyup)
{
    ...
    ASSERT_EQ((int)expected_m.size(),
              ioctx.sparse_read(oid, m, outbl, 65536, 0));
...
TEST_F(TestClsRbd, directory_methods)
{
    ...
    ASSERT_EQ(0,
              dir_state_assert(&ioctx, oid, cls::rbd::DIRECTORY_STATE_READY));
...
TEST_F(TestClsRbd, parents_v2)
{
    ...
    ASSERT_EQ(-ENOEXEC,
              parent_attach(&ioctx, oid, parent_image_spec, 0ULL, false));
...
TEST_F(TestClsRbd, mirror_image_status)
{
    ...
    ASSERT_EQ(-ESTALE,
              mirror_image_instance_get(&ioctx, "uuid1", &read_instance));
    instances.clear();
...
TEST_F(TestClsRbd, group_dir_rename)
{
    ...
    ASSERT_EQ(0,
              group_dir_remove(&ioctx, RBD_GROUP_DIRECTORY, dest_name,
                               group_id));
...
TEST_F(TestClsRbd, group_snap_list)
{
    ...
    ASSERT_EQ(0,
              group_snap_list(&ioctx, group_id, cls::rbd::GroupSnapshot(), 10,
                              &snapshots));
...
TEST_F(TestClsRbd, group_snap_list_max_read)
{
    ...
    std::vector < cls::rbd::GroupSnapshot > snapshots;
    ASSERT_EQ(0,
              group_snap_list(&ioctx, group_id, cls::rbd::GroupSnapshot(), 500,
                              &snapshots));
...
TEST_F(TestClsRbd, group_snap_get_by_id)
{
    ...
    ASSERT_EQ(0,
              group_snap_get_by_id(&ioctx, group_id, snap_id, &received_snap));
