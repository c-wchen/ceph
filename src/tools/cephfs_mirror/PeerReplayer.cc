...
std::string snapshot_dir_path(CephContext *cct,
                              const std::string &path)
{
    return path + "/" + cct->_conf->client_snapdir;
} std::string snapshot_path(const std::string &snap_dir,
                            const std::string &snap_name)
{
    return snap_dir + "/" + snap_name;
} std::string snapshot_path(CephContext *cct,
                            const std::string &path,
                            const std::string &snap_name)
{
    return path + "/" + cct->_conf->client_snapdir + "/" +
...
std::string entry_path(const std::string &dir,
...
std::map < std::string,
    std::string > decode_snap_metadata(snap_metadata *snap_metadata,
                                       size_t nr_snap_metadata)
{
    std::map < std::string, std::string > metadata;
    for (size_t i = 0; i < nr_snap_metadata; ++i) {
        metadata.emplace(snap_metadata[i].key,
                         snap_metadata[i].value);
    }

    return metadata;
}

std::string peer_config_key(const std::string &fs_name,
...
int opendirat(MountRef mnt, int dirfd, const std::string &relpath,
              int flags, ceph_dir_result **dirp)
{
...
{
public:
    PeerReplayerAdminSocketHook(CephContext *cct,
...
private:
    typedef std::map < std::string, PeerAdminSocketCommand *,
    std::less <>> Commands;
...
PeerReplayer::PeerReplayer(CephContext *cct, FSMirror *fs_mirror,
                           ...
                           std::less <>> &directories, MountRef mount,
    ...
    : m_cct(cct), m_fs_mirror(fs_mirror), m_local_cluster(local_cluster),
      ...
      m_asok_hook(new
                  PeerReplayerAdminSocketHook(cct, filesystem, peer,
                          this)),
      m_lock(ceph::
             make_mutex("cephfs::mirror::PeerReplayer::" +
                        stringify(peer.uuid)))
{
...
int PeerReplayer::init()
{
    ...
    std::string key =
        peer_config_key(m_filesystem.fs_name, m_peer.uuid);
    std::string cmd =
    ...
    r = connect(remote_client, remote_cluster, &m_remote_cluster,
                mon_host, cephx_key);
    if (r < 0) {
        derr << ": error connecting to remote cluster: " <<
             cpp_strerror(r)
             << dendl;
        return r;
    }

    r = mount(m_remote_cluster, remote_filesystem, false,
              &m_remote_mount);
    if (r < 0) {
        m_remote_cluster.reset();
        derr << ": error mounting remote filesystem=" <<
             remote_filesystem << dendl;
        return r;
    }
    ...
    dout(20) << ": spawning " << nr_replayers << " snapshot replayer(s)"
             << dendl;

    while (nr_replayers-- > 0) {
        std::unique_ptr < SnapshotReplayerThread >
        replayer(new SnapshotReplayerThread(this));
...
void PeerReplayer::remove_directory(string_view dir_root)
{
    ...
    std::scoped_lock locker(m_lock);
    auto it =
        std::find(m_directories.begin(), m_directories.end(),
                  _dir_root);
...
boost::optional < std::string > PeerReplayer::pick_directory()
{
    ...
    std::rotate(m_directories.begin(), m_directories.begin() + 1,
                m_directories.end());
    return candidate;
}
...
void PeerReplayer::unregister_directory(const std::string &dir_root)
{
    ...
    if (std::
        find(m_directories.begin(), m_directories.end(),
             dir_root) == m_directories.end()) {
                                     ...
                                     DirRegistry *registry)
{
    ...
    int r =
        ceph_open(m_remote_mount, dir_root.c_str(),
    ...
    if (r == -ENOENT) {
        ...
        r = ceph_open(m_remote_mount, dir_root.c_str(),
                      O_RDONLY | O_DIRECTORY, 0);
        if (r < 0) {
            derr << ": failed to open remote dir_root=" << dir_root <<
                 ": " << cpp_strerror(r)
    ...
    int fd = r;
    r = ceph_flock(m_remote_mount, fd, LOCK_EX | LOCK_NB,
                   (uint64_t) replayer->get_thread_id());
    if (r != 0) {
        if (r == -EWOULDBLOCK) {
            dout(5) << ": dir_root=" << dir_root <<
                    " is locked by cephfs-mirror, " << "will retry again" <<
                    dendl;
        } else {
        ...
        if (ceph_close(m_remote_mount, fd) < 0) {
            derr << ": failed to close (cleanup) remote dir_root=" <<
...
void PeerReplayer::unlock_directory(const std::string &dir_root,
                                 ...
                                 bool is_remote)
{
    ...
    if (r < 0) {
        ...
        derr << ": failed to open " << lr_str << " snap directory=" <<
    ...
    for (auto &snap : snaps) {
        ...
        if (is_remote) {
            if (!info.nr_snap_metadata) {
                derr << ": snap_path=" << snap_path <<
                     " has invalid metadata in remote snapshot" << dendl;
                rv = -EINVAL;
            } else {
                auto metadata =
                    decode_snap_metadata(info.snap_metadata,
                                         info.nr_snap_metadata);
                dout(20) << ": snap_path=" << snap_path << ", metadata="
                         << metadata << dendl;
    ...
    if (r < 0) {
        derr << ": failed to close " << lr_str << " snap directory=" <<
             snap_dir << ": " << cpp_strerror(r) << dendl;
    }
...
int PeerReplayer::propagate_snap_deletes(const std::string &dir_root,
        ...
        &snaps)
{
    dout(5) << ": dir_root=" << dir_root << ", deleted snapshots=" <<
            snaps << dendl;

    for (auto &snap : snaps) {
        dout(20) << ": deleting dir_root=" << dir_root << ", snapshot="
                 << snap << dendl;
        int r =
            ceph_rmsnap(m_remote_mount, dir_root.c_str(), snap.c_str());
        if (r < 0) {
            derr << ": failed to delete remote snap dir_root=" <<
                 dir_root << ", snapshot=" << snaps << ": " <<
                 cpp_strerror(r) << dendl;
            return r;
        }
...
int PeerReplayer::propagate_snap_renames(const std::string &dir_root,
        ...
        std::string >> &snaps)
{
    dout(10) << ": dir_root=" << dir_root << ", renamed snapshots=" <<
             snaps << dendl;

    for (auto &snapp : snaps) {
        auto from = snapshot_path(m_cct, dir_root, snapp.first);
        auto to = snapshot_path(m_cct, dir_root, snapp.second);
        dout(20) << ": renaming dir_root=" << dir_root <<
                 ", snapshot from=" << from << ", to=" << to << dendl;
        int r = ceph_rename(m_remote_mount, from.c_str(), to.c_str());
        if (r < 0) {
            derr << ": failed to rename remote snap dir_root=" <<
                 dir_root << ", snapshot from =" << from << ", to=" << to
                 << ": " << cpp_strerror(r) << dendl;
            return r;
        }
                               ...
                               const FHandles &fh)
{
    ...
    int r =
        ceph_mkdirat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(),
    ...
    r = ceph_utimensat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(),
                                 ...
                                 const FHandles &fh)
{
    ...
    int r =
        ceph_openat(m_local_mount, fh.c_fd, epath.c_str(),
...
{
    dout(10) << ": dir_root=" << dir_root << ", epath=" << epath <<
             ", need_data_sync=" << need_data_sync << ", need_attr_sync=" <<
    ...
    if (need_data_sync) {
        if (S_ISREG(stx.stx_mode)) {
            ...
            if (r < 0) {
                derr << ": failed to copy path=" << epath << ": " <<
                     cpp_strerror(r) << dendl;
                return r;
            }
            ...
            r = ceph_unlinkat(m_remote_mount, fh.r_fd_dir_root,
                              epath.c_str(), 0);
            if (r < 0 && r != -ENOENT) {
                derr << ": failed to remove remote symlink=" << epath <<
                     dendl;
                return r;
            }
            char *target = (char *)alloca(stx.stx_size + 1);
            r = ceph_readlinkat(m_local_mount, fh.c_fd, epath.c_str(),
                                target, stx.stx_size);
            if (r < 0) {
            ...
            if (r < 0 && r != EEXIST) {
                derr << ": failed to symlink remote path=" << epath <<
                     " to target=" << target << ": " << cpp_strerror(r)
                     << dendl;
                return r;
            }
        } else {
    ...
    if (need_attr_sync) {
        r = ceph_chownat(m_remote_mount, fh.r_fd_dir_root,
                         epath.c_str(), stx.stx_uid, stx.stx_gid,
                         AT_SYMLINK_NOFOLLOW);
        if (r < 0) {
            derr << ": failed to chown remote directory=" << epath <<
                 ": " << cpp_strerror(r)
        ...
        r = ceph_chmodat(m_remote_mount, fh.r_fd_dir_root,
                         epath.c_str(), stx.stx_mode & ~S_IFMT,
                         AT_SYMLINK_NOFOLLOW);
        if (r < 0) {
            derr << ": failed to chmod remote directory=" << epath <<
                 ": " << cpp_strerror(r)
        ...
        r = ceph_utimensat(m_remote_mount, fh.r_fd_dir_root,
                           epath.c_str(), times, AT_SYMLINK_NOFOLLOW);
        if (r < 0) {
            derr << ": failed to change [am]time on remote directory="
                 << epath << ": " << cpp_strerror(r) << dendl;
            return r;
                                     ...
                                     const FHandles &fh)
{
    ...
    int r =
        ceph_statxat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(),
                     &tstx,
    ...
    while (!rm_stack.empty()) {
        ...
        if (entry.is_directory()) {
            ...
            struct dirent de;
            while (true) {
                r = ceph_readdirplus_r(m_remote_mount, entry.dirp, &de,
                                       &stx, CEPH_STATX_MODE,
                                       AT_STATX_DONT_SYNC |
                                       AT_SYMLINK_NOFOLLOW, NULL);
                if (r < 0) {
                    derr << ": failed to read remote directory=" <<
                         entry.epath << dendl;
                    break;
                }
                if (r == 0) {
            ...
            if (r == 0) {
                r = ceph_unlinkat(m_remote_mount, fh.r_fd_dir_root,
                                  entry.epath.c_str(), AT_REMOVEDIR);
                if (r < 0) {
                    derr << ": failed to remove remote directory=" <<
                         entry.epath << ": " << cpp_strerror(r) << dendl;
                    break;
                }

                dout(10) << ": done for remote directory=" << entry.
                         epath << dendl;
                if (ceph_closedir(m_remote_mount, entry.dirp) < 0) {
                    derr << ": failed to close remote directory=" <<
                         entry.epath << dendl;
                }
                rm_stack.pop();
                continue;
            ...
            if (S_ISDIR(stx.stx_mode)) {
                ...
                if (r < 0) {
                    derr << ": failed to open remote directory=" <<
                         epath << ": " << cpp_strerror(r) << dendl;
                    break;
                }
            ...
            r = ceph_unlinkat(m_remote_mount, fh.r_fd_dir_root,
                              entry.epath.c_str(), 0);
            if (r < 0) {
                derr << ": failed to remove remote directory=" << entry.
                     epath << ": " << cpp_strerror(r) << dendl;
                break;
            }
            dout(10) << ": done for remote file=" << entry.
                     epath << dendl;
            rm_stack.pop();
        }
    }
    ...
    while (!rm_stack.empty()) {
        auto &entry = rm_stack.top();
        if (entry.is_directory()) {
            dout(20) << ": closing remote directory=" << entry.
                     epath << dendl;
            if (ceph_closedir(m_remote_mount, entry.dirp) < 0) {
                derr << ": failed to close remote directory=" << entry.
                     epath << dendl;
            }
        }
                                    ...
                                    bool *need_attr_sync)
{
    ...
    *need_attr_sync = false;
    ...
    dout(10) << ": local cur statx: mode=" << cstx.
             stx_mode << ", uid=" << cstx.stx_uid << ", gid=" << cstx.
             stx_gid << ", size=" << cstx.stx_size << ", ctime=" << cstx.
             stx_ctime << ", mtime=" << cstx.stx_mtime << dendl;
    dout(10) << ": local prev statx: mode=" << pstx.
             stx_mode << ", uid=" << pstx.stx_uid << ", gid=" << pstx.
             stx_gid << ", size=" << pstx.stx_size << ", ctime=" << pstx.
             stx_ctime << ", mtime=" << pstx.stx_mtime << dendl;
...
int PeerReplayer::propagate_deleted_entries(const std::
        string &dir_root,
        const std::string &epath,
        const FHandles &fh)
{
    ...
    ceph_dir_result *dirp;
    int r =
        opendirat(fh.p_mnt, fh.p_fd, epath, AT_SYMLINK_NOFOLLOW, &dirp);
    if (r < 0) {
        if (r == -ELOOP) {
            dout(5) << ": epath=" << epath <<
    ...
    struct dirent *dire =
        (struct dirent *)alloca(512 * sizeof(struct dirent));
    while (true) {
        if (should_backoff(dir_root, &r)) {
            dout(0) << ": backing off r=" << r << dendl;
            break;
        ...
        if (len < 0) {
            derr << ": failed to read directory entries: " <<
        ...
        for (int i = 0; i < nr; ++i) {
            ...
            r = ceph_statxat(m_local_mount, fh.c_fd, dpath.c_str(),
                             &cstx, CEPH_STATX_MODE,
                             AT_STATX_DONT_SYNC | AT_SYMLINK_NOFOLLOW);
            if (r < 0 && r != -ENOENT) {
                derr << ": failed to stat local (cur) directory=" <<
                     dpath << ": " << cpp_strerror(r) << dendl;
                return r;
            }
            ...
            if (r == 0) {
                ...
                // type
                if ((pstx.stx_mode & S_IFMT) ==
                    (cstx.stx_mode & S_IFMT)) {
                    dout(5) << ": mode matches for entry=" << d_name <<
                            dendl;
                    purge_remote = false;
                } else {
                    dout(5) << ": mode mismatch for entry=" << d_name <<
                            dendl;
                }
            } else {
                dout(5) << ": entry=" << d_name <<
            ...
            if (purge_remote) {
                ...
                if (r < 0 && r != -ENOENT) {
                    derr << ": failed to cleanup remote entry=" <<
                         d_name << ": " << cpp_strerror(r) << dendl;
                    return r;
                }
            }
        }
                           ...
                           boost::optional < uint64_t > snap_id)
{
    ...
    int fd =
        ceph_open(mnt, dir_path.c_str(), O_DIRECTORY | O_RDONLY, 0);
    if (fd < 0) {
        derr << ": cannot open dir_path=" << dir_path << ": " <<
             cpp_strerror(fd)
             << dendl;
        return fd;
    }
    ...
    if (info.id != *snap_id) {
        dout(5) << ": got mismatching snapshot id for path=" << dir_path
                << " (" << info.
                id << " vs " << *snap_id << ") -- possible recreate" <<
                dendl;
        ceph_close(mnt, fd);
        return -EINVAL;
    }
...
int PeerReplayer::pre_sync_check_and_open_handles(const std::
        string &dir_root,
        const Snapshot &
        current,
        boost::optional <
        ...
        FHandles *fh)
{
    dout(20) << ": dir_root=" << dir_root << ", current=" << current <<
             dendl;
    if (prev) {
    ...
    if (prev) {
        mnt = m_local_mount;
        auto prev_snap_path =
            snapshot_path(m_cct, dir_root, (*prev).first);
    ...
    if (fd < 0) {
        ...
        dout(5) << ": previous snapshot=" << *prev << " missing" <<
                dendl;
        mnt = m_remote_mount;
        fd = open_dir(mnt, dir_root, boost::none);
        if (fd < 0) {
    ...
    dout(5) << ": using " << ((fh->p_mnt == m_local_mount) ?
                                 ...
                                 boost::optional < Snapshot > prev)
{
    dout(20) << ": dir_root=" << dir_root << ", current=" << current <<
             dendl;
    if (prev) {
        dout(20) << ": incremental sync check from prev=" << prev <<
                 dendl;
    }

    FHandles fh;
    int r =
        pre_sync_check_and_open_handles(dir_root, current, prev, &fh);
    if (r < 0) {
        dout(5) << ": cannot proceeed with sync: " << cpp_strerror(r) <<
                dendl;
        return r;
    }
    ...
    r = ceph_fstatx(m_local_mount, fh.c_fd, &tstx,
                    ...
                    AT_STATX_DONT_SYNC | AT_SYMLINK_NOFOLLOW);
    if (r < 0) {
        derr << ": failed to stat snap=" << current.
             first << ": " << cpp_strerror(r)
             << dendl;
        return r;
    }
    ...
    if (r < 0) {
        derr << ": failed to open local snap=" << current.
             first << ": " << cpp_strerror(r)
             << dendl;
        return r;
    }
    ...
    while (!sync_stack.empty()) {
        ...
        if (entry.is_directory()) {
            ...
            if (!entry.needs_remote_sync()) {
                r = propagate_deleted_entries(dir_root, entry.epath,
                                              fh);
                if (r < 0 && r != -ENOENT) {
                    derr << ": failed to propagate missing dirs: " <<
                         cpp_strerror(r) << dendl;
                    break;
                }
            ...
            struct dirent de;
            while (true) {
                r = ceph_readdirplus_r(m_local_mount, entry.dirp, &de,
                                       &stx,
                                       CEPH_STATX_MODE | CEPH_STATX_UID
                                       | CEPH_STATX_GID |
                                       CEPH_STATX_SIZE |
                                       ...
                                       AT_SYMLINK_NOFOLLOW, NULL);
                if (r < 0) {
                    derr << ": failed to local read directory=" <<
                         entry.epath << dendl;
                    break;
                }
                if (r == 0) {
            ...
            if (r == 0) {
                dout(10) << ": done for directory=" << entry.
                         epath << dendl;
                if (ceph_closedir(m_local_mount, entry.dirp) < 0) {
                    derr << ": failed to close local directory=" <<
                         entry.epath << dendl;
                }
                sync_stack.pop();
                continue;
            ...
            if (S_ISDIR(stx.stx_mode)) {
                ...
                ceph_dir_result *dirp;
                r = opendirat(m_local_mount, fh.c_fd, epath,
                              AT_SYMLINK_NOFOLLOW, &dirp);
                if (r < 0) {
            ...
            dout(5) << ": entry=" << entry.
                    epath << ", data_sync=" << need_data_sync <<
    ...
    while (!sync_stack.empty()) {
        auto &entry = sync_stack.top();
        if (entry.is_directory()) {
            dout(20) << ": closing local directory=" << entry.
                     epath << dendl;
            if (ceph_closedir(m_local_mount, entry.dirp) < 0) {
                derr << ": failed to close local directory=" << entry.
                     epath << dendl;
            }
        }
                              ...
                              boost::optional < Snapshot > prev)
{
    ...
    int r =
        ceph_getxattr(m_remote_mount, dir_root.c_str(),
                      "ceph.mirror.dirty_snap_id", nullptr, 0);
    if (r < 0 && r != -ENODATA) {
        derr <<
             ": failed to fetch primary_snap_id length from dir_root=" <<
             dir_root << ": " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (r < 0) {
        ...
        r = ceph_getxattr(m_remote_mount, dir_root.c_str(),
                          "ceph.mirror.dirty_snap_id", (void *)val,
                          xlen);
        if (r < 0) {
            derr << ": failed to fetch \"dirty_snap_id\" for dir_root: "
                 << dir_root << ": " << cpp_strerror(r) << dendl;
            return r;
        }
        ...
        dout(20) << ": dirty_snap_id: " << dirty_snap_id << " vs (" <<
                 current.
                 second << "," << (prev ? stringify((*prev).second) : "~") <<
                 ")" << dendl;
        if (prev
            && (dirty_snap_id == (*prev).second
                || dirty_snap_id == current.second)) {
            dout(5) <<
                    ": match -- using incremental sync with local scan" <<
                    dendl;
            r = do_synchronize(dir_root, current, prev);
        } else {
            dout(5) <<
                    ": mismatch -- using incremental sync with remote scan"
                    << dendl;
            r = do_synchronize(dir_root, current, boost::none);
        }
    }
    ...
    auto cur_snap_id_str {
        stringify(current.second)};
    snap_metadata snap_meta[] = {
        {PRIMARY_SNAP_ID_KEY.c_str(), cur_snap_id_str.c_str()}
    };
    r = ceph_mksnap(m_remote_mount, dir_root.c_str(),
                    current.first.c_str(), 0755, snap_meta,
                    sizeof(snap_meta) / sizeof(snap_metadata));
    if (r < 0) {
        derr << ": failed to snap remote directory dir_root=" <<
             dir_root << ": " << cpp_strerror(r) << dendl;
    }
...
int PeerReplayer::do_sync_snaps(const std::string &dir_root)
{
    ...
    for (auto &[primary_snap_id, snap_name] : remote_snap_map) {
        ...
        if (it == local_snap_map.end()) {
            ...
            snaps_renamed.
            emplace(std::make_pair(snap_name, it->second));
        }
    }
    ...
    for (; it != local_snap_map.end(); ++it) {
        ...
        r = synchronize(dir_root, std::make_pair(it->second, it->first),
                        prev);
        if (r < 0) {
        ...
        set_last_synced_stat(dir_root, it->first, it->second,
                             duration.count());
                              ...
                              &locker)
{
    ...
    if (r < 0) {
        derr << ": failed to sync snapshots for dir_root=" << dir_root
             << dendl;
    }
    locker.lock();
    if (r < 0) {
...
void PeerReplayer::run(SnapshotReplayerThread *replayer)
{
    ...
    while (true) {
        ...
        m_cond.wait_for(locker, 1 s, [this] {
            return is_stopping();
        }
                       );
        if (is_stopping()) {
        ...
        auto now = clock::now();
        std::chrono::duration < double >timo =
            now - last_directory_scan;
        if (timo.count() >= scan_interval && m_directories.size()) {
            dout(20) << ": trying to pick from " << m_directories.
...
void PeerReplayer::peer_status(Formatter *f)
{
    ...
    for (auto &[dir_root, sync_stat] : m_snap_sync_stats) {
        ...
        if (sync_stat.failed) {
            ...
            f->dump_string("name",
                           (*sync_stat.current_syncing_snap).second);
            f->close_section();
        }
        if (sync_stat.last_synced_snap) {
            ...
            f->dump_string("name",
                           (*sync_stat.last_synced_snap).second);
            if (sync_stat.last_sync_duration) {
                f->dump_float("sync_duration",
                              *sync_stat.last_sync_duration);
                f->dump_stream("sync_time_stamp") << sync_stat.
                                                  last_synced;
            }
            f->close_section();
        }
...
void PeerReplayer::reopen_logs()
{
    ...
    if (m_remote_cluster) {
        reinterpret_cast <
        CephContext * >(m_remote_cluster->cct())->reopen_logs();
    }
}
