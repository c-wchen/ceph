...
:  Dispatcher(cct_), cct(cct_), msgr(msgr_), timer(cct_, lock)
{
    assert(cct != nullptr);
}
...
void MgrClient::_send_report()
{
    ...
    pcc->
        with_counters([this,
                       report] (const PerfCountersCollection::
                                CounterMap & by_path) {
      ...
      for (const auto & i:by_path) {
                      ...
                      ::encode(static_cast < uint64_t > (data.avgcount),
                               report->packed);::encode(static_cast < uint64_t >
                                                        (data.avgcount2),
                                                        report->packed);}
                      }
...
bool MgrClient::handle_mgr_configure(MMgrConfigure * m)
{
    ...
    if (stats_threshold != m->stats_threshold) {
        ldout(cct,
              4) << "updated stats threshold: " << m->stats_threshold << dendl;
                                       ...
                                       std::string > &metadata)
{
    Mutex::Locker l(lock);
    if (name == "osd" ||
        name == "mds" || name == "client" || name == "mon" || name == "mgr") {
    ...
    ldout(cct,
          1) << service << "." << name << " metadata " << metadata << dendl;
    service_daemon = true;
...
int MgrClient::service_daemon_update_status(const std::map < std::string,
                                            std::string > &status)
{
    Mutex::Locker l(lock);
