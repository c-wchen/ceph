...
int tcpv4_listen(tcp < ipv4_traits > &tcpv4, uint16_t port,
                 const SocketOptions & opts, ServerSocket * sock)
{
    auto p =
        new DPDKServerSocketImpl < tcp < ipv4_traits >> (tcpv4, port, opts);
    int r = p->listen();
    if (r < 0) {
        delete p;
        return r;
                  ...
                  ConnectedSocket * sock)
{
    ...
    *sock =
        ConnectedSocket(std::unique_ptr < ConnectedSocketImpl >
                        (new NativeConnectedSocketImpl < tcp <
                         ipv4_traits >> (std::move(conn))));
    return 0;
}
                                                ...
                                                ipaddr foreign_ip)
{
    ...
    *th = th->hton();
    ...
    checksummer csum;
    offload_info oi;
    InetTraits::tcp_pseudo_header_checksum(csum, local_ip, foreign_ip,
                                           sizeof(*th));
                                                            ...
                                                            Packet p)
{
    ...
    auto opt_start =
        reinterpret_cast <
        uint8_t * >(p.get_header(0, th->data_offset * 4)) + sizeof(tcp_hdr);
    ...
    ldout(_tcp.cct,
          10) << __func__ << " listen: LISTEN -> SYN_RECEIVED" << dendl;
                                                              ...
                                                              Packet p)
{
    ...
    if (th->f_syn) {
        ...
        if (_snd.unacknowledged > _snd.initial) {
            ...
            ldout(_tcp.cct,
                  20) << __func__ << " syn: SYN_SENT -> ESTABLISHED" << dendl;
            init_from_options(th, opt_start, opt_end);
            do_established();
            output();
        }
            ...
            // <SEQ=ISS><ACK=RCV.NXT><CTL=SYN,ACK>
            ldout(_tcp.cct,
                  20) << __func__ << " syn: SYN_SENT -> SYN_RECEIVED" << dendl;
            do_syn_received();
        }
    }
    ...
    if (!th->f_ack) {
        ...
        if (in_state(ESTABLISHED | CLOSE_WAIT)) {
            ...
            if (_snd.unacknowledged < seg_ack && seg_ack <= _snd.next) {
                ...
                if (_snd.wl1 < seg_seq
                    || (_snd.wl1 == seg_seq && _snd.wl2 <= seg_ack)) {
                    update_window();
                }
