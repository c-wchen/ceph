...
template <> struct ImageReplayer <librbd::MockTestImageCtx > {
    ...
    static ImageReplayer *create(librados::IoCtx &local_io_ctx,
    ...
    MOCK_METHOD1(add_peer,
                 void (const Peer < librbd::MockTestImageCtx > &peer));
...
ImageReplayer < librbd::MockTestImageCtx > *ImageReplayer <
        ...
        timer_ctx](Context * ctx) {
            ...
            if (timer_ctx != nullptr) {
                *timer_ctx = ctx;
                ...
                queue(new
                LambdaContext([&mock_threads, ctx](int) {
...
TEST_F(TestMockInstanceReplayer, AcquireReleaseImage)
{
    ...
    instance_replayer.acquire_image(&mock_instance_watcher,
                                    global_image_id, &on_acquire);
...
TEST_F(TestMockInstanceReplayer, RemoveFinishedImage)
{
    ...
    .
    WillOnce(Invoke
    ([&start_image_replayers_ctx](Context * ctx, int r) {
