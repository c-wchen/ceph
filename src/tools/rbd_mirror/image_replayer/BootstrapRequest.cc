...
template < typename I >
BootstrapRequest < I >::BootstrapRequest(Threads < I > *threads,
        librados::
        IoCtx &local_io_ctx,
        librados::
        IoCtx &remote_io_ctx,
    ...
    : CancelableRequest("rbd::mirror::image_replayer::BootstrapRequest",
                        reinterpret_cast <
                        CephContext * >(local_io_ctx.cct()), on_finish),
      ...
      m_progress_ctx(progress_ctx), m_state_builder(state_builder),
...
template < typename I >
std::string BootstrapRequest <
I >::get_local_image_name()const
{
...
void BootstrapRequest < I >::prepare_local_image()
{
    ...
    auto ctx = create_context_callback <
               BootstrapRequest,
               &BootstrapRequest < I >::handle_prepare_local_image >
               (this);
    auto req =
        image_replayer::PrepareLocalImageRequest <
        I >::create(m_local_io_ctx, m_global_image_id,
...
void BootstrapRequest < I >::handle_prepare_local_image(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        derr << "error preparing local image for replay: " <<
             cpp_strerror(r)
             << dendl;
        finish(r);
        return;
...
void BootstrapRequest < I >::prepare_remote_image()
{
    ...
    Context *ctx = create_context_callback <
                   BootstrapRequest,
                   &BootstrapRequest < I >::handle_prepare_remote_image >
                   (this);
    auto req =
        image_replayer::PrepareRemoteImageRequest <
        I >::create(m_threads, m_local_io_ctx, m_remote_io_ctx,
                    m_global_image_id,
                    m_local_mirror_uuid, m_remote_pool_meta,
...
I >::handle_prepare_remote_image(int r)
{
    dout(10) << "r=" << r << dendl;
    ...
    if (state_builder != nullptr
        && state_builder->is_local_primary()) {
        ...
        if (state_builder != nullptr) {
            *_dout << ": "
                   << "local_image_id=" << state_builder->
                   local_image_id << ", " << "remote_image_id=" <<
                   state_builder->
                   remote_image_id << ", " << "is_linked=" <<
        ...
        *_dout << dendl;
        ...
        derr << "error preparing remote image for replay: " <<
             cpp_strerror(r)
             << dendl;
        finish(r);
        return;
    ...
    if (!state_builder->is_remote_primary()) {
        ...
        if (state_builder->local_image_id.empty()) {
            dout(10) <<
                     "local image does not exist and remote image is not primary"
                     << dendl;
            finish(-EREMOTEIO);
            return;
        } else if (!state_builder->is_linked()) {
            dout(10) <<
                     "local image is unlinked and remote image is not primary"
                     << dendl;
            finish(-EREMOTEIO);
            return;
        }
...
void BootstrapRequest < I >::handle_open_remote_image(int r)
{
    ...
    if (r < 0) {
        derr << "failed to open remote image: " << cpp_strerror(r)
             << dendl;
        ceph_assert((*m_state_builder)->remote_image_ctx ==
                    nullptr);
        finish(r);
        return;
    }
...
void BootstrapRequest < I >::prepare_replay()
{
    ...
    auto request =
        (*m_state_builder)->
        create_prepare_replay_request(m_local_mirror_uuid,
                                      m_progress_ctx, m_do_resync,
                                      &m_syncing, ctx);
    request->send();
}
...
void BootstrapRequest < I >::handle_prepare_replay(int r)
{
    ...
    if (r < 0) {
        derr << "failed to prepare local replay: " <<
        ...
        dout(10) << "local image still syncing to remote image" <<
                 dendl;
        image_sync();
        return;
    }
...
void BootstrapRequest < I >::create_local_image()
{
    ...
    auto request =
        (*m_state_builder)->create_local_image_request(m_threads,
            m_local_io_ctx,
            m_global_image_id,
            m_pool_meta_cache,
            m_progress_ctx,
...
template < typename I > void BootstrapRequest < I >::image_sync()
{
    ...
    auto state_builder = *m_state_builder;
    auto sync_point_handler =
        state_builder->create_sync_point_handler();

    Context *ctx = create_context_callback <
                   BootstrapRequest < I >,
                   &BootstrapRequest < I >::handle_image_sync > (this);
    m_image_sync =
        ImageSync < I >::create(m_threads,
                                state_builder->local_image_ctx,
                                state_builder->remote_image_ctx,
                                m_local_mirror_uuid,
                                sync_point_handler,
                                m_instance_watcher, m_progress_ctx,
                                ctx);
    m_image_sync->get();
    locker.unlock();
