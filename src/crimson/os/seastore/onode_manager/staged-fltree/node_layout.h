 ...
 */
    template < typename FieldType, node_type_t NODE_TYPE >
      ...
      public:
        ...
        static URef load(NodeExtentRef extent) {
            ...
            allocate(context_t c, laddr_t hint, bool is_level_tail,
                     level_t level) {
            ...
            return c.nm.alloc_extent(c.t, hint,
                                     extent_size).
                handle_error_interruptible(eagain_iertr::pass_further {
                          ...
                          node_stage_t::bootstrap_extent(mut, FIELD_TYPE,
                                                         NODE_TYPE,
                                                         is_level_tail, level);
                          return typename parent_t::fresh_impl_t {
                          std::unique_ptr < parent_t >
                          (new NodeLayoutT(extent)), mut};}
            );
        }
      ...
      protected:
        ...
        } void prepare_mutate(context_t c) override {
        ...
        bool is_keys_empty() const override {
            ...
            if constexpr(NODE_TYPE == node_type_t::INTERNAL) {
                return ((is_level_tail() && is_keys_empty()) ||
                        (!is_level_tail()
                         && stage_t::is_keys_one(extent.read())));
            }
            else {
        ...
        level_t level() const override {
            ...
            stage_t::template get_largest_slot < false, true,
                false > (extent.read(), nullptr, &pivot_index, nullptr);
            return {
            pivot_index};
        }
     ...
     */
            ...
            if (unlikely
                (LOGGER(seastore_onode).
                 is_enabled(seastar::log_level::trace))) {
            ...
            SUBDEBUG(seastore_onode, "done  at erase_stage={}, n/l_pos({})",
...
#endif
            return {
            stage, normalize(std::move(next_or_last_pos))};
        }

        std::tuple < match_stage_t,
            std::size_t > evaluate_merge(NodeImpl & _right_node) override {
            ...
            if (right_node.is_keys_empty()) {
                ...
                stage_t::template get_largest_slot < false, true,
                    false > (left_node_stage, nullptr, &left_pivot_index,
                             nullptr);
            ...
            return {
            merge_stage, merge_size};
        }

        search_position_t merge(NodeExtentMutable & mut,
            ...
            if (unlikely
                (LOGGER(seastore_onode).
                 is_enabled(seastar::log_level::debug))) {
                {
                    std::ostringstream sos;
                    dump(sos);
            ...
            stage_t::template get_largest_slot < true, false,
                false > (left_node_stage, &left_last_pos, nullptr, nullptr);
            ...
            if (right_node.is_keys_empty()) {
                if constexpr
                    ...
                    assert(right_node.is_level_tail());
                    laddr_t tail_value =
                        right_node_stage.get_end_p_laddr()->value;
                ...
                typename stage_t::template StagedAppender < KeyT::VIEW >
                    left_appender;
                ...
                stage_t::template append_until < KeyT::VIEW > (right_append_at,
                                                               left_appender,
                                                               pos_end, STAGE);
        ...
        std::ostream & dump(std::ostream & os) const override {
            ...
            if constexpr
                ...
                if (node_stage.is_level_tail()) {
                    ...
                    os << "\n  tail value: 0x" << std::hex << value_ptr->
                        value << std::
                        dec << " " << size << "B" << "  @" << offset << "B";
                }
                }
        ...
        void get_slot(const search_position_t & pos,
            ...
            if (p_index_key && pp_value) {
                stage_t::template get_slot < true, true > (extent.read(),
                                                           cast_down < STAGE >
                                                           (pos), p_index_key,
                                                           pp_value);
            }
            else if (!p_index_key && pp_value) {
                stage_t::template get_slot < false, true > (extent.read(),
                                                            cast_down < STAGE >
                                                            (pos), nullptr,
                                                            pp_value);
            }
...
#endif
            if (!p_index_key && pp_value) {
                stage_t::template get_prev_slot < false, true > (extent.read(),
                                                                 _pos, nullptr,
                                                                 pp_value);
            }
            else {
...
#ifndef NDEBUG
            auto _nxt_pos = _pos;
            stage_t::template get_next_slot < false, false > (extent.read(),
                                                              _nxt_pos, nullptr,
                                                              nullptr);
        ...
        void get_next_slot(search_position_t & pos,
            ...
            if (p_index_key && pp_value) {
                find_next =
                    stage_t::template get_next_slot < true,
                    true > (extent.read(), cast_down < STAGE > (pos),
                            p_index_key, pp_value);
            }
            else if (!p_index_key && pp_value) {
                find_next =
                    stage_t::template get_next_slot < false,
                    true > (extent.read(), cast_down < STAGE > (pos), nullptr,
                            pp_value);
            }
            else {
                ceph_abort("not implemented");
        ...
        void get_largest_slot(search_position_t * p_pos = nullptr,
            ...
            if (p_pos && p_index_key && pp_value) {
                stage_t::template get_largest_slot < true, true,
                    true > (extent.read(), &cast_down_fill_0 < STAGE > (*p_pos),
                            p_index_key, pp_value);
            }
            else if (!p_pos && p_index_key && !pp_value) {
                stage_t::template get_largest_slot < false, true,
                    false > (extent.read(), nullptr, p_index_key, nullptr);
            }
            else if (p_pos && !p_index_key && pp_value) {
                stage_t::template get_largest_slot < true, false,
                    true > (extent.read(), &cast_down_fill_0 < STAGE > (*p_pos),
                ...
                stage_t::template get_largest_slot < true, false,
                    false > (extent.read(),
                             &cast_down_fill_0 < STAGE > (*p_pos), nullptr,
                             nullptr);
            }
            else {
                ceph_abort("not implemented");
        ...
        lookup_result_t < NODE_TYPE > lower_bound(const key_hobj_t & key,
            ...
            if (index_key) {
                result_raw =
                    stage_t::template lower_bound < true > (node_stage, key,
                                                            history, index_key);
#ifndef NDEBUG
                if (!result_raw.is_end()) {
                    key_view_t index;
                    stage_t::template get_slot < true, false > (node_stage,
                                                                result_raw.
                                                                position,
                                                                &index,
                                                                nullptr);
        ...
        const value_t *insert(const full_key_t < KEY_TYPE > &key,
            ...
            auto ret =
                extent.template insert_replayable < KEY_TYPE > (key, value,
                                                                cast_down <
                                                                STAGE >
                                                                (insert_pos),
                                                                insert_stage,
        ...
        std::tuple < search_position_t, bool,
            const value_t *>split_insert(NodeExtentMutable & right_mut,
       ...
       */
                target_split_size =
                    empty_size + (filled_kv_size + insert_size) / 2;
                assert(insert_size <
                       (node_stage.total_size() - empty_size) / 2);

                std::optional < bool > _is_insert_left;
                split_at.set(node_stage);
                split_size = 0;
                bool locate_nxt =
                    stage_t::recursively_locate_split_inserted(split_size, 0,
                                                               target_split_size,
                                                               insert_pos,
                                                               insert_stage,
                                                               insert_size,
            ...
            typename stage_t::template StagedAppender < KEY_TYPE >
                right_appender;
            ...
            if (!is_insert_left) {
                ...
                stage_t::template append_until < KEY_TYPE > (append_at,
                                                             right_appender,
                                                             insert_pos,
                                                             insert_stage);
                SUBDEBUG(seastore_onode,
                ...
                [[maybe_unused]] bool is_end =
                    stage_t::template append_insert < KEY_TYPE > (key, value,
                                                                  append_at,
                                                                  right_appender,
                                                                  is_front_insert,
                                                                  insert_stage,
            ...
            if (is_insert_left) {
                SUBDEBUG(seastore_onode,
                         ...
                         insert_pos, insert_stage);
                p_value =
                    extent.template split_insert_replayable < KEY_TYPE >
                    (split_at, key, value, insert_pos, insert_stage,
                     insert_size);
#ifndef NDEBUG
                key_view_t index;
            ...
            if (is_insert_left) {
                stage_t::template get_largest_slot < true, false,
                    false > (extent.read(),
                             &cast_down_fill_0 < STAGE > (last_pos), nullptr,
                             nullptr);
            }
            else {
            ...
            last_split = {
            split_pos, insert_stage_pre, is_insert_left, insert_type};
#endif
            return {
            split_pos, is_insert_left, p_value};
        }
        ...
        void replace_child_addr(const search_position_t & pos, laddr_t dst,
                                laddr_t src) override {
            if constexpr
                (NODE_TYPE == node_type_t::INTERNAL) {
                LOG_PREFIX(OTree::Layout::replace_child_addr);
                SUBDEBUG(seastore_onode,
                         "update from {:#x} to {:#x} at pos({}) ...", src, dst,
                         pos);
                ...
                extent.update_child_addr_replayable(dst,
                                                    const_cast <
                                                    laddr_packed_t *
                                                    >(p_value));
                }
        ...
        std::tuple < match_stage_t,
            node_offset_t > evaluate_insert(const key_view_t & key,
                                            const laddr_t & value,
                                            search_position_t & insert_pos)
            const override {
            if constexpr(NODE_TYPE == node_type_t::INTERNAL) {
                ...
                if (unlikely(is_keys_empty())) {
                    ...
                    std::tie(insert_stage, insert_size) =
                        stage_t::evaluate_insert(node_stage, key, value,
                                                 cast_down < STAGE >
                                                 (insert_pos), false);
                }
                return {
         ...
         */
        std::tuple < match_stage_t,
            node_offset_t > evaluate_insert(const key_hobj_t & key,
            ...
            if constexpr(NODE_TYPE == node_type_t::LEAF) {
                if (unlikely(is_keys_empty())) {
                    ...
                    assert(is_level_tail());
                    return {
                    STAGE, stage_t::insert_size(key, value)};
                }
                else {
                    return stage_t::evaluate_insert(key, value, history, mstat,
                                                    cast_down < STAGE >
                                                    (insert_pos));
                }
            }
        ...
        std::pair < NodeExtentMutable &,
            ValueDeltaRecorder * >prepare_mutate_value_payload(context_t c) {
            return extent.prepare_mutate_value_payload(c);
        }

      private:
      NodeLayoutT(NodeExtentRef extent):extent {
        extent}
        {
            build_name();
        }
    ...
    using InternalNode0 =
        NodeLayoutT < node_fields_0_t, node_type_t::INTERNAL >;
    using InternalNode1 =
        NodeLayoutT < node_fields_1_t, node_type_t::INTERNAL >;
    using InternalNode2 =
        NodeLayoutT < node_fields_2_t, node_type_t::INTERNAL >;
    using InternalNode3 =
        NodeLayoutT < internal_fields_3_t, node_type_t::INTERNAL >;
