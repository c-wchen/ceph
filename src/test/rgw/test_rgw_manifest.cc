...
static void gen_obj(test_rgw_env & env, uint64_t obj_size,
                    ...
                    RGWObjManifest * manifest, const string & placement_id,
...
TEST(TestRGWManifest, head_only_obj)
{
    ...
    for (iter = manifest.obj_begin(), liter = objs.begin();
         iter != manifest.obj_end() && liter != objs.end(); ++iter, ++liter) {
...
TEST(TestRGWManifest, multipart)
{
    ...
    for (int i = 0; i < num_parts; ++i) {
        ...
        for (ofs = 0; ofs < part_size; ofs += stripe_size) {
            if (ofs == 0) {
                int r =
                    gen.create_begin(g_ceph_context, &manifest,
                                     env.zonegroup.default_placement, bucket,
                                     head);
                ASSERT_EQ(r, 0);
                continue;
            }
