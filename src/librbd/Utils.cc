...
const std::string CONFIG_KEY_URI_PREFIX {
    "config://"};
                 ...
                 librados::IoCtx *dst_io_ctx)
{
    ...
    if (r == -ENOENT) {
        ldout(cct,
              1) << pool_desc << " pool " << pool_id <<
                 " no longer exists" << dendl;
        return r;
    } else if (r < 0) {
        lderr(cct) << "error accessing " << pool_desc << " pool " <<
                   pool_id << dendl;
        return r;
    }

    dst_io_ctx->
    set_namespace(pool_namespace ? *pool_namespace : src_io_ctx.
                  get_namespace());
...
{
    auto mode =
        ictx->config.get_val < std::string >
