...
auto uses_allocator_construction_args(const Alloc &a, Args
                                      && ... args);
...
struct has_allocator <std::pair < T1, T2 >,
           A >: std::integral_constant < bool, has_allocator < T1, A >::value
           || has_allocator < T2, A >::value > {
};

template < bool V > using boolean_constant =
...
template < class T, class Unused1, class Unused2, class Alloc,
           class ... Args >
...
// Return a tuple of arguments appropriate for uses-allocator construction// with allocator `Alloc` and ctor arguments `Args`.// This overload handles non-pair `T` for which `has_allocator<T, Alloc>` is// true and constructor `T(allocator_arg_t, a, args...)` is valid.
template < class T, class Alloc, class ... Args >
...
auto uses_allocator_args_imp(std::true_type /* is_pair */,
                             ...
                             false_type /* prefix allocator arg */,
                             const Alloc &a,
                             std::piecewise_construct_t, Tuple1
                             && x, Tuple2 && y)
{
...
auto uses_allocator_args_imp(std::true_type /* is_pair */,
                             ...
                             std::
                             false_type /* prefix allocator arg */,
                             const Alloc &a, const std::pair < U1,
                             U2 > &arg)
{
    // using T1 = typename T::first_type;
    // using T2 = typename T::second_type;
        ...
        Args && ... args)
{
    ...
    uses_allocator_construction_args < T > (a,
                                            std::forward <
                                            Args >
                                            (args) ...));
}
