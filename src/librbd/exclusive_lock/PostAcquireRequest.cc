    ...
    namespace exclusive_lock {
        ...
        using util::create_rados_callback;
         ...
         template < typename I >
            ...
            I >::create(I & image_ctx, Context * on_acquire,
                        Context * on_finish) {
            return new PostAcquireRequest(image_ctx, on_acquire, on_finish);
        } template < typename I >
            PostAcquireRequest < I >::PostAcquireRequest(I & image_ctx,
                                                         Context * on_acquire,
                                                         Context * on_finish)
        :m_image_ctx(image_ctx), m_on_acquire(on_acquire),
            m_on_finish(create_async_context_callback(image_ctx, on_finish)),
            m_object_map(nullptr), m_journal(nullptr), m_error_result(0) {
        } template < typename I > PostAcquireRequest <
            I >::~PostAcquireRequest() {
        ...
        template < typename I > void PostAcquireRequest < I >::send_refresh() {
            ...
            image::RefreshRequest < I > *req =
                image::RefreshRequest < I >::create(m_image_ctx, true, false,
                                                    ctx);
            req->send();
        }
        ...
        template < typename I >
            ...
            Context *ctx =
                create_context_callback < klass,
                &klass::handle_open_journal > (this);
        ...
        template < typename I >
            ...
            save_result(r);
            if (r < 0) {
                lderr(cct) << "failed to allocate journal tag: " <<
                    cpp_strerror(r)
                    << dendl;
                send_close_journal();
                return;
        ...
        template < typename I >
            void PostAcquireRequest <
            I >::handle_process_plugin_acquire_lock(int r) {
            CephContext *cct = m_image_ctx.cct;
            ldout(cct, 10) << "r=" << r << dendl;
        ...
        template < typename I >
            ...
            Context *ctx =
                create_context_callback < klass,
                &klass::handle_open_object_map > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                lderr(cct) << "failed to open object map: " << cpp_strerror(r)
                    << dendl;
                m_object_map->put();
