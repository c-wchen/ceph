...
extern int rgw_user_sync_all_stats(const DoutPrefixProvider * dpp,
...
extern int rgw_user_get_all_buckets_stats(const DoutPrefixProvider * dpp,
                                          ...
                                          std::map < std::string,
                                          bucket_meta_entry >
                                          &buckets_usage_map, optional_yield y);
...
struct RGWUserAdminOpState {
    ...
    bool type_specified {
    false};
    bool key_type_setbycontext {
    false};                     // key type set by user or subuser context
    bool purge_data {
    false};
    bool purge_keys {
    false};
    ...
    false};
    bool bucket_ratelimit_specified {
    false};
    bool user_ratelimit_specified {
    false};
...
bool get_access_key_exist() {
    return access_key_exist;
}
uint32_t get_subuser_perm() {
    return perm_mask;
}
int32_t get_max_buckets() {
    return max_buckets;
}
...
std::set < std::string > &get_mfa_ids() {
    return mfa_ids;
}

rgw::sal::User * get_user() {
    return user.get();
}
...
rgw_user & get_new_uid() {
    return new_user_id;
}
bool get_overwrite_new_user() const {
    return overwrite_new_user;
} std::map < int, std::string > &get_temp_url_keys() {
...
class RGWAccessKeyPool {
  ...
  private:
    int create_key(RGWUserAdminOpState & op_state, std::string * err_msg =
                   NULL);
    int generate_key(const DoutPrefixProvider * dpp,
                     RGWUserAdminOpState & op_state, optional_yield y,
                     std::string * err_msg = NULL);
    int modify_key(RGWUserAdminOpState & op_state, std::string * err_msg =
                   NULL);
    ...
    int execute_remove(const DoutPrefixProvider * dpp,
                       RGWUserAdminOpState & op_state, std::string * err_msg,
                       bool defer_save, optional_yield y);
    int remove_subuser_keys(const DoutPrefixProvider * dpp,
                            RGWUserAdminOpState & op_state,
                            std::string * err_msg, bool defer_save,
                            optional_yield y);

    int add(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
            std::string * err_msg, bool defer_save, optional_yield y);
    int remove(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
               std::string * err_msg, bool defer_save, optional_yield y);
  public:
...
class RGWSubUserPool {
    ...
    std::map < std::string, RGWSubUser > *subuser_map {
    nullptr};
  ...
  private:
    ...
    int execute_remove(const DoutPrefixProvider * dpp,
                       RGWUserAdminOpState & op_state, std::string * err_msg,
                       bool defer_save, optional_yield y);
    int execute_modify(const DoutPrefixProvider * dpp,
                       RGWUserAdminOpState & op_state, std::string * err_msg,
                       bool defer_save, optional_yield y);
    ...
    int remove(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
               std::string * err_msg, bool defer_save, optional_yield y);
    int modify(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
               optional_yield y, std::string * err_msg, bool defer_save);
  public:
...
class RGWUser {
  ...
  private:
    ...
    int update(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
    ...
    int execute_modify(const DoutPrefixProvider * dpp,
                       RGWUserAdminOpState & op_state, std::string * err_msg,
                       optional_yield y);
    int execute_rename(const DoutPrefixProvider * dpp,
                       RGWUserAdminOpState & op_state, std::string * err_msg,
                       optional_yield y);
  ...
  public:
    ...
    int init(const DoutPrefixProvider * dpp, rgw::sal::Driver * storage,
             RGWUserAdminOpState & op_state, optional_yield y);

    int init_storage(rgw::sal::Driver * storage);
    int init(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
             optional_yield y);
    ...
    int rename(RGWUserAdminOpState & op_state, optional_yield y,
    ...
    int info(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
             RGWUserInfo & fetched_info, optional_yield y,
             std::string * err_msg = NULL);
    ...
    int list(const DoutPrefixProvider * dpp, RGWUserAdminOpState & op_state,
...
class RGWUserMetadataObject:public RGWMetadataObject {
  ...
  public:
    ...
    } RGWUserMetadataObject(const RGWUserCompleteInfo & _uci,
...
class RGWUserCtl {
  ...
  public:
     RGWUserCtl(RGWSI_Zone * zone_svc,
                RGWSI_User * user_svc, RGWUserMetadataHandler * _umhandler);
    ...
    int list_buckets(const DoutPrefixProvider * dpp,
                     ...
                     bool * is_truncated,
                     optional_yield y, uint64_t default_max = 1000);

    int read_stats(const DoutPrefixProvider * dpp, const rgw_user & user, RGWStorageStats * stats, optional_yield y, ceph::real_time * last_stats_sync = nullptr,   /* last time a full stats sync completed */
