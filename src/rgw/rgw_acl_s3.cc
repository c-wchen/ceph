...
int RGWAccessControlPolicy_S3::create_from_headers(RGWRados * store,
                                                   const RGWEnv * env,
                                                   ACLOwner & _owner)
{
 ...
 */
int RGWAccessControlPolicy_S3::rebuild(RGWRados * store, ACLOwner * owner,
        ...
        case ACL_TYPE_CANON_USER:
            {
                ...
                if (grant_user.user_id.empty()
                    && rgw_get_user_info_by_uid(store, uid, grant_user) < 0) {
                    ldout(cct,
                          10) << "grant user does not exist:" << uid << dendl;
                    return -EINVAL;
                }
                else {
