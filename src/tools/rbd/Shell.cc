...
boost::intrusive_ptr < CephContext > global_init(int argc,
        const char **argv,
        std::vector <
        std::string >
        *command_args,
                              ...
                              const Shell::CommandSpec &alias_spec)
{
    ...
    if (level < 2) {
        return joinify < std::string > (alias_spec.begin(),
                                        alias_spec.end(), " ");
    } else {
        return "... " + joinify < std::string > (alias_it,
                alias_spec.end(), " ");
    }
}
...
std::string format_option_suffix(const boost::shared_ptr <
                                 po::option_description > &option)
{
    std::string suffix;
    if (option->semantic()->max_tokens() != 0) {
        if (option->description().find("path") != std::string::npos ||
            ...
            suffix += " path";
        } else if (option->description().find("host") !=
                   std::string::npos) {
            suffix += " host";
        } else {
            suffix += " arg";
...
void print_deprecated_warning(po::option_description option,
                              std::string description)
{
    ...
    if (pos != std::string::npos) {
        ...
        std::cerr << "rbd: " << option.
                  format_name() << " is deprecated, use --" << param << std::endl;
    }
}
...
int Shell::execute(int argc, const char **argv)
{
    ...
    if (command_spec.empty() || command_spec == CommandSpec({
    "help"}
                                                       )) {
    ...
    try {
        ...
        if (!positional_opts.options().empty()) {
            ...
            if (positional_opts.options().back()->semantic()->max_tokens() >
                1) {
                max_count = -1;
            }
            positional_options.add(at::POSITIONAL_ARGUMENTS.c_str(),
                                   max_count);
        }
        ...
        if (vm.size() > 0) {
            for (auto opt : vm) {
                try {
                    ...
                    auto result =
                        boost::find_first(description, "deprecated");
                    if (!result.empty()) {
        ...
        for (; it != ceph_global_init_args.end(); ++it) {
            ...
            for (size_t i = 0; i < global_opts.options().size(); ++i) {
                std::string param_name =
                    global_opts.options()[i]->
                    canonical_display_name(prefix_style);
                ...
                if (!result.empty() && *it == param_name) {
                    print_deprecated_warning(*global_opts.options()[i],
                                             description);
                    break;
                }
            }
        ...
        if (arg == "-h" || arg == "--help") {
            *command_spec = {
            ...
            if (arg.size() >= 2 &&
                ...
                (arg[1] != '-' ||
                 get_switch_arguments().count(arg.
                                              substr(2,
                                                      std::string::npos)) ==
                 0)
                && at::SWITCH_ARGUMENTS.count(arg.
                                              substr(2,
                                                      std::string::npos)) ==
                0 && arg.find('=') == std::string::npos) {
                ++i;
            }
        } else {
...
void Shell::print_help()
{
    ...
    name_width =
        std::min(name_width, OptionPrinter::MAX_DESCRIPTION_OFFSET) + 1;
    ...
    for (size_t i = 0; i < actions.size(); ++i) {
        ...
        std::stringstream ss;
        ss << indent
           << format_command_name(action->command_spec,
                                  action->alias_command_spec);
    ...
    std::cout << std::endl << OptionPrinter::
...
void Shell::print_action_help(Action *action, bool is_alias)
{
    std::stringstream ss;
    ss << "usage: " << APP_NAME << " "
       << format_command_spec(is_alias ? action->
                              alias_command_spec : action->command_spec);
    std::cout << ss.str();
...
void Shell::print_unknown_action(const std::vector < std::string >
                                 &command_spec)
{
    std::cerr << "error: unknown option '" << joinify < std::string >
...
void Shell::print_bash_completion_options(const po::
        options_description &ops)
{
    for (size_t i = 0; i < ops.options().size(); ++i) {
        auto option = ops.options()[i];
        std::string long_name(option->canonical_display_name(0));
        std::string short_name(option->
                               canonical_display_name(po::
                                       command_line_style::
