...
I >::create(I &image_ctx, AbstractWriteLog < I > *image_cache,
            plugin::Api < I > &plugin_api,
            Context *on_finish)
{
    return new ShutdownRequest(image_ctx, image_cache, plugin_api,
                               on_finish);
} template < typename I > ShutdownRequest <
I >::ShutdownRequest(I &image_ctx,
                     AbstractWriteLog < I > *image_cache,
                     plugin::Api < I > &plugin_api,
                     Context *on_finish)
    : m_image_ctx(image_ctx), m_image_cache(image_cache),
      m_plugin_api(plugin_api),
      m_on_finish(create_async_context_callback
                  (image_ctx, on_finish)), m_error_result(0)
{
...
void ShutdownRequest < I >::send_shutdown_image_cache()
{
    ...
    Context *ctx =
        ...
        &klass::handle_shutdown_image_cache > (this);
...
void ShutdownRequest < I >::handle_shutdown_image_cache(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to shut down the image cache: " <<
                   cpp_strerror(r)
                   << dendl;
        save_result(r);
        finish();
...
void ShutdownRequest < I >::send_remove_feature_bit()
{
    ...
    int r =
        librbd::cls_client::set_features(&m_image_ctx.md_ctx,
    ...
    Context *ctx =
        create_context_callback < klass,
        &klass::handle_remove_feature_bit > (this);
    ctx->complete(r);
}
...
void ShutdownRequest < I >::handle_remove_feature_bit(int r)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to remove the feature bit: " <<
                   cpp_strerror(r)
                   << dendl;
        save_result(r);
        finish();
...
void ShutdownRequest < I >::send_remove_image_cache_state()
{
    ...
    Context *ctx =
        create_context_callback < klass,
        &klass::handle_remove_image_cache_state > (this);
...
I >::handle_remove_image_cache_state(int r)
{
    CephContext *cct = m_image_ctx.cct;
    ldout(cct, 10) << dendl;

    if (r < 0) {
        lderr(cct) << "failed to remove the image cache state: " <<
                   cpp_strerror(r)
                   << dendl;
        save_result(r);
    }
