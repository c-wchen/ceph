...
TEST(LibCephFS, Xattrs)
{
    ...
    int len =
        ceph_listxattr(cmount, test_xattr_file, xattrlist, sizeof(xattrlist));
    char *p = xattrlist;
    char *n;
    i = 'a';
    while (len > 0) {
...
TEST(LibCephFS, LazyStatx)
{
    ...
    ASSERT_EQ(ceph_ll_getattr
              (cmount2, file2, &stx, CEPH_STATX_CTIME, AT_NO_ATTR_SYNC, perms2),
              0);
...
TEST(LibCephFS, ClearSetuid)
{
    ...
    ASSERT_EQ(ceph_ll_getattr(cmount, in, &stx, CEPH_STATX_MODE, 0, usercred),
              0);
    ...
    // truncate
    stx.stx_size = 1;
    ASSERT_EQ(ceph_ll_setattr(cmount, in, &stx, CEPH_SETATTR_SIZE, usercred),
              0);
    ASSERT_EQ(ceph_ll_getattr(cmount, in, &stx, CEPH_STATX_MODE, 0, usercred),
              0);
    ASSERT_TRUE(stx.stx_mask & CEPH_STATX_MODE);
    ...
    ASSERT_EQ(ceph_ll_setattr
              (cmount, in, &stx,
               CEPH_STATX_MODE | CEPH_SETATTR_UID | CEPH_SETATTR_GID, rootcred),
              0);
    ASSERT_EQ(ceph_ll_getattr(cmount, in, &stx, CEPH_STATX_MODE, 0, altcred),
              0);
