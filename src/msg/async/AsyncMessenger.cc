...
int Processor::bind(const entity_addr_t & bind_addr,
                    const set < int >&avoid_ports, entity_addr_t * bound_addr)
{
    ...
    for (int i = 0; i < conf->ms_bind_retry_count; i++) {
        if (i > 0) {
            lderr(msgr->
                  cct) << __func__ << " was unable to bind. Trying again in " <<
        ...
        if (listen_addr.get_port()) {
            worker->center.submit_to(worker->center.get_id(),
                                     ...
                                     r =
                                     worker->listen(listen_addr, opts,
                                                    &listen_socket);}
                                     , false);
            if (r < 0) {
                lderr(msgr->
                      cct) << __func__ << " unable to bind to " << listen_addr
            ...
            if (r < 0) {
                lderr(msgr->
                      cct) << __func__ << " unable to bind to " << listen_addr
                    << " on any port in range " << msgr->cct->_conf->
                    ms_bind_port_min << "-" << msgr->cct->_conf->
                    ms_bind_port_max << ": " << cpp_strerror(r) << dendl;
                listen_addr.set_port(0);    // Clear port before retry, otherwise we shall fail again.
                continue;
            }
            ldout(msgr->cct,
                  10) << __func__ << " bound on random port " << listen_addr <<
                dendl;
        }
        if (r == 0)
            break;
    ...
    if (r < 0) {
        lderr(msgr->cct) << __func__ << " was unable to bind after " << conf->
            ms_bind_retry_count << " attempts: " << cpp_strerror(r) << dendl;
        return r;
    }
...
void Processor::start()
{
    ...
    if (listen_socket) {
        worker->center.submit_to(worker->center.get_id(),[this] () {
                                 worker->center.create_file_event(listen_socket.
                                                                  fd(),
                                                                  EVENT_READABLE,
                                                                  listen_handler);
                                 }
...
{
    ldout(msgr->cct,
          10) << __func__ << " listen_fd=" << listen_socket.fd() << dendl;
    SocketOptions opts;
    opts.nodelay = msgr->cct->_conf->ms_tcp_nodelay;
    opts.rcbuf_size = msgr->cct->_conf->ms_tcp_rcvbuf;
    ...
    while (true) {
        ...
        if (r == 0) {
            ldout(msgr->cct,
                  10) << __func__ << " accepted incoming on sd " << cli_socket.
                fd() << dendl;
            ...
            if (r == -EINTR) {
                ...
                lderr(msgr->
                      cct) << __func__ <<
                    " open file descriptions limit reached sd = " <<
                    listen_socket.fd()
                    << " errno " << r << " " << cpp_strerror(r) << dendl;
                if (++accept_error_num >
                    msgr->cct->_conf->ms_max_accept_failures) {
                    lderr(msgr->
                          cct) <<
                ...
                ldout(msgr->cct,
                      0) << __func__ <<
                    " it was closed because of rst arrived sd = " <<
                    listen_socket.fd()
                    << " errno " << r << " " << cpp_strerror(r) << dendl;
                continue;
            }
...
AsyncMessenger::AsyncMessenger(CephContext * cct, entity_name_t name,
                               const std::string & type, string mname,
                               uint64_t _nonce)
:SimplePolicyMessenger(cct, name, mname, _nonce), dispatch_queue(cct, this,
                                                                 mname),
lock("AsyncMessenger::lock"), nonce(_nonce), need_addr(true), did_bind(false),
...
cluster_protocol(0), stopped(true)
{
    ...
    cct->lookup_or_create_singleton_object < StackSingleton > (single,
                                                               "AsyncMessenger::NetworkStack::"
                                                               +
                                                               transport_type);
    ...
    local_connection =
        new AsyncConnection(cct, this, &dispatch_queue, local_worker);
                                  ...
                                  const entity_addr_t & listen_addr)
{
    ...
    ldout(cct,
          1) << __func__ << " bind my_inst.addr is " << get_myaddr() << dendl;
    did_bind = true;
}
                                ...
                                entity_addr_t & addr)
{
    ...
    AsyncConnectionRef conn =
        new AsyncConnection(cct, this, &dispatch_queue, w);
    conn->accept(std::move(cli_socket), addr);
    accepting_conns.insert(conn);
    lock.Unlock();
}

AsyncConnectionRef AsyncMessenger::create_connect(const entity_addr_t & addr,
                                                  int type)
{
...
ConnectionRef AsyncMessenger::get_connection(const entity_inst_t & dest)
{
    ...
    if (conn) {
        ldout(cct,
              10) << __func__ << " " << dest << " existing " << conn << dendl;
    }
    else {
...
int AsyncMessenger::_send_message(Message * m, const entity_inst_t & dest)
{
    ...
    if (m->get_type() == CEPH_MSG_OSD_OP)
        OID_EVENT_TRACE(((MOSDOp *) m)->get_oid().name.c_str(),
                        "SEND_MSG_OSD_OP");
    else if (m->get_type() == CEPH_MSG_OSD_OPREPLY)
        OID_EVENT_TRACE(((MOSDOpReply *) m)->get_oid().name.c_str(),
                        "SEND_MSG_OSD_OP_REPLY");
                                    ...
                                    int dest_type)
{
    ...
    if (policy.server) {
        ldout(cct, 20) << __func__ << " " << *m << " remote, " << dest_addr
            ...
            << ceph_entity_type_name(dest_type) << ", no session, dropping." <<
            dendl;
        m->put();
    }
    else {
        ldout(cct,
              20) << __func__ << " " << *m << " remote, " << dest_addr <<
...
void AsyncMessenger::shutdown_connections(bool queue_reset)
{
    ...
    while (!conns.empty()) {
        ceph::unordered_map < entity_addr_t, AsyncConnectionRef >::iterator it =
            conns.begin();
        AsyncConnectionRef p = it->second;
        ldout(cct,
              5) << __func__ << " mark down " << it->first << " " << p << dendl;
        conns.erase(it);
...
int AsyncMessenger::get_proto_version(int peer_type, bool connect) const const
{
...
void AsyncMessenger::learned_addr(const entity_addr_t & peer_addr_for_me)
{
    ...
    if (need_addr) {
        ...
        my_inst.addr = t;
        ldout(cct,
              1) << __func__ << " learned my addr " << my_inst.addr << dendl;
        _init_local_connection();
    }
    lock.Unlock();
}
