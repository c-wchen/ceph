    ...
    namespace journal {
        ...
        class TestJournalReplay:public TestFixture {
          public:
            ...
            void get_journal_commit_position(librbd::ImageCtx * ictx,
                                             int64_t * tag, int64_t * entry) {
                ...
                cls::journal::client::get_mutable_metadata(ictx->md_ctx, oid,
                                                           &minimum_set,
                                                           &active_set,
                                                           &registered_clients,
                                                           &cond);
        ...
        TEST_F(TestJournalReplay, AioWriteEvent) {
            ...
            payload_bl.append(payload);
            inject_into_journal(ictx,
                                librbd::journal::AioWriteEvent(0,
                                                               payload.size(),
                                                               payload_bl));
            close_image(ictx);
            ...
            // verify lock ordering constraints
            aio_comp = new librbd::io::AioCompletion();
            api::Io <>::aio_write(*ictx, aio_comp, 0, payload.size(), bufferlist {
                                  payload_bl}
                                  , 0, true);
            ASSERT_EQ(0, aio_comp->wait_for_complete());
            aio_comp->release();
        ...
        TEST_F(TestJournalReplay, SnapCreate) {
            ...
            inject_into_journal(ictx,
                                librbd::journal::SnapCreateEvent(1,
                                                                 cls::rbd::
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_create(cls::rbd::UserSnapshotNamespace(), "snap2", 0,
                                  no_op_progress));
        }
        ...
        TEST_F(TestJournalReplay, SnapProtect) {
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_create(cls::rbd::UserSnapshotNamespace(), "snap", 0,
                                  no_op_progress));
            ...
            bool is_protected;
            ASSERT_EQ(0,
                      librbd::api::Snapshot <>::is_protected(ictx, "snap",
                                                             &is_protected));
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_protect(cls::rbd::UserSnapshotNamespace(), "snap2"));
        }
        ...
        TEST_F(TestJournalReplay, SnapUnprotect) {
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_protect(cls::rbd::UserSnapshotNamespace(), "snap"));
        ...
        TEST_F(TestJournalReplay, SnapRollback) {
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_rollback(cls::rbd::UserSnapshotNamespace(), "snap",
                                    no_op_progress));
        }
        ...
        TEST_F(TestJournalReplay, SnapRemove) {
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      snap_remove(cls::rbd::UserSnapshotNamespace(), "snap"));
        }
        ...
        TEST_F(TestJournalReplay, Rename) {
            ...
            librbd::RBD rbd;
            ASSERT_EQ(0,
                      rbd.rename(m_ioctx, new_image_name.c_str(),
                                 m_image_name.c_str()));
        }
        ...
        TEST_F(TestJournalReplay, UpdateFeatures) {
            ...
            inject_into_journal(ictx,
                                librbd::journal::UpdateFeaturesEvent(1,
                                                                     features,
                                                                     enabled));
            close_image(ictx);
        ...
        TEST_F(TestJournalReplay, MetadataSet) {
            ...
            inject_into_journal(ictx,
                                librbd::journal::MetadataSetEvent(1,
        ...
        TEST_F(TestJournalReplay, MetadataRemove) {
            ...
            ASSERT_EQ(0,
                      ictx->operations->
                      metadata_set("conf_rbd_mirroring_replay_delay", "9876"));
            ...
            inject_into_journal(ictx,
                                librbd::journal::MetadataRemoveEvent(1,
                                                                     "conf_rbd_mirroring_replay_delay"));
            inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
            close_image(ictx);
        ...
        TEST_F(TestJournalReplay, ObjectPosition) {
            ...
            aio_comp =
                librbd::io::AioCompletion::create_and_start(&flush_ctx, ictx,
                                                            librbd::io::
                                                            AIO_TYPE_FLUSH);
            auto req =
                librbd::io::ImageDispatchSpec::create_flush(*ictx,
                                                            librbd::io::
