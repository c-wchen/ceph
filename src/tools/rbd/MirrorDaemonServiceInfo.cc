...
namespace rbd {
    ...
    std::ostream & operator<<(std::ostream & os, MirrorHealth mirror_health) {
        ...
        case MIRROR_HEALTH_OK:
            os << "OK";
            break;
            case MIRROR_HEALTH_UNKNOWN:os << "UNKNOWN";
            break;
            case MIRROR_HEALTH_WARNING:os << "WARNING";
            break;
            case MIRROR_HEALTH_ERROR:os << "ERROR";
            break;
        } return os;
    }
    ...
    const MirrorService *MirrorDaemonServiceInfo::get_by_service_id(const std::
                                                                    string &
                                                                    service_id)
        const {
    ...
    const MirrorService *MirrorDaemonServiceInfo::get_by_instance_id(const std::
                                                                     string &
                                                                     instance_id)
        const {
    ...
    int MirrorDaemonServiceInfo::get_mirror_service_dump() {
        ...
        std::string cmd =
            R "({" prefix ": " service dump ", " format ": " json "})";
        bufferlist in_bl;
        bufferlist out_bl;
        ...
        try {
            ...
            if (services.is_null()) {
                std::
                    cerr << "rbd: missing services in service dump JSON" <<
                    std::endl;
                return -EBADMSG;
            }
          ...
          for (auto & daemon_pair:daemons.get_obj()) {
            ...
            std::cerr << "rbd: unexpected service dump JSON received" << std::
                endl;
            return -EBADMSG;
        }
    ...
    int MirrorDaemonServiceInfo::get_mirror_service_status() {
        librados::Rados rados(m_io_ctx);
        std::string cmd =
            R "({" prefix ": " service status ", " format ": " json "})";
        bufferlist in_bl;
        bufferlist out_bl;
        ...
        if (r < 0) {
            std::
                cerr << "rbd: failed to query service status: " <<
                cpp_strerror(r)
                << std::endl;
            return r;
        }
        json_spirit::mValue json_root;
        if (!json_spirit::read(out_bl.to_str(), json_root)) {
            std::cerr << "rbd: invalid service status JSON received" << std::
                endl;
            return -EBADMSG;
        }
        ...
        try {
          ...
          for (auto & daemon_pair:service.get_obj()) {
                ...
                if (!json_spirit::read(json.get_str(), json_status)) {
                    std::
                        cerr <<
                        "rbd: invalid service status daemon status JSON received"
                        << std::endl;
                    return -EBADMSG;
                }

                auto & pool_val =
                    json_status.get_obj()[stringify(m_io_ctx.get_id())];
                if (pool_val.is_null()) {
                    mirror_service.callouts.
                        push_back("not reporting status for pool");
                    mirror_service.health = MIRROR_HEALTH_WARNING;
                    continue;
                }
                ...
                if (!instance_id.is_null()) {
                    ...
                    m_instance_to_service_ids[mirror_service.instance_id] =
                        service_id;
                }
                ...
                if (!callouts.is_null()) {
                  for (auto & callout_pair:callouts.
                         get_obj())
                    {
            ...
            std::cerr << "rbd: unexpected service status JSON received" << std::
                endl;
            return -EBADMSG;
        }
