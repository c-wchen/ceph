...
namespace crimson {
     ...
     */
     template < typename I,
        ...
        typename C, uint K = 2 > class IndIntruHeap {
        ...
        class Iterator {
            ...
            HeapIndex index;
             ...
             Iterator(IndIntruHeap < I, T, heap_info, C, K > &_heap,
                      HeapIndex _index):heap(_heap), index(_index) {
                // empty
          } public:

             Iterator(Iterator && other):heap(other.heap), index(other.index) {
                // empty
            } Iterator(const Iterator & other):heap(other.heap),
                index(other.index) {
                // empty
            }
        ...
        class ConstIterator {
            ...
            HeapIndex index;
             ...
             ConstIterator(ConstIterator && other):heap(other.heap),
                index(other.index) {
                // empty
            }

            ConstIterator(const ConstIterator & other):heap(other.heap),
                index(other.index) {
                // empty
            }
      ...
      protected:
        ...
        static bool all_filter(const T & data) {
            return true;
        }
        ...
        static inline HeapIndex lhs(HeapIndex i) {
            return K * i + 1;
        }

        // index of right child when K==2, index of right-most child when K>2
        static inline HeapIndex rhs(HeapIndex i) {
            return K * i + K;
        }
        ...
        template < bool EnableBool = true >
            typename std::enable_if < (K > 2)
            && EnableBool, void >::type sift_down(HeapIndex i) {
            if (i >= count)
                return;
            while (true) {
                HeapIndex li = lhs(i);
