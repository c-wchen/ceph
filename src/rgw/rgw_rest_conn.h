...
class RGWRESTConn {
  ...
  public:
    ...
    RGWRESTConn(CephContext * _cct, RGWRados * store, const string & _remote_id,
    ...
    /* sync request */
        int forward(const rgw_user & uid, req_info & info, obj_version * objv,
                    size_t max_response, bufferlist * inbl, bufferlist * outbl);

    /* async request */
    int put_obj_init(const rgw_user & uid, rgw_obj & obj, uint64_t obj_size,
                     map < string, bufferlist > &attrs,
                     RGWRESTStreamWriteRequest ** req);
    int complete_request(RGWRESTStreamWriteRequest * req, string & etag,
                         ceph::real_time * mtime);

    int get_obj(const rgw_user & uid, req_info * info /* optional */ ,
                rgw_obj & obj,
    ...
    int complete_request(RGWRESTStreamRWRequest * req, string & etag,
    ...
    template < class T >
        int get_json_resource(const string & resource, param_vec_t * params,
                              bufferlist * in_data, T & t);
    template < class T > int get_json_resource(const string & resource,
                                               param_vec_t * params, T & t);
    template < class T > int get_json_resource(const string & resource,
...
template < class T >
    int RGWRESTConn::get_json_resource(const string & resource,
                                       const rgw_http_param_pair * pp, T & t)
{
...
class RGWRESTReadResource:public RefCountedObject {
  ...
  public:
    ...
    template < class T > int wait(T * dest);

    template < class T > int fetch(T * dest);
};
...
template < class T > int RGWRESTReadResource::fetch(T * dest)
{
    int ret = read();
    if (ret < 0) {
        return ret;
...
template < class T > int RGWRESTReadResource::wait(T * dest)
{
    int ret = req.wait();
    if (ret < 0) {
        return ret;
...
class RGWRESTSendResource:public RefCountedObject {
  ...
  public:
    ...
    template < class T, class E > int decode_resource(T * dest, E * err_result);
    ...
    template < class T, class E > int wait(T * dest, E * err_result = nullptr);
};
  ...
  public:
    RGWRESTPostResource(RGWRESTConn * _conn,
                        ...
                        RGWHTTPManager * _mgr):RGWRESTSendResource(_conn,
                                                                   ...
                                                                   extra_headers,
                                                                   _mgr) {
    } RGWRESTPostResource(RGWRESTConn * _conn, const string & _resource,
  ...
  public:
    RGWRESTPutResource(RGWRESTConn * _conn,
                       ...
                       RGWHTTPManager * _mgr):RGWRESTSendResource(_conn, "PUT",
                                                                  ...
                                                                  extra_headers,
                                                                  _mgr) {
    } RGWRESTPutResource(RGWRESTConn * _conn, const string & _resource,
                         param_vec_t & params, param_vec_t * extra_headers,
                         RGWHTTPManager * _mgr):RGWRESTSendResource(_conn,
                                                                    "PUT",
                                                                    _resource,
  ...
  public:
    RGWRESTDeleteResource(RGWRESTConn * _conn,
                          ...
                          RGWHTTPManager * _mgr):RGWRESTSendResource(_conn,
                                                                     ...
                                                                     extra_headers,
                                                                     _mgr) {
    } RGWRESTDeleteResource(RGWRESTConn * _conn, const string & _resource,
                            param_vec_t & params, param_vec_t * extra_headers,
                            RGWHTTPManager * _mgr):RGWRESTSendResource(_conn,
                                                                       "DELETE",
                                                                       _resource,
