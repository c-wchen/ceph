...
LoadRequest < I >::LoadRequest(librados::IoCtx &ioctx,
                               ...
                               *image_mapping,
                               Context *on_finish)
    : m_ioctx(ioctx), m_image_mapping(image_mapping),
...
} template < typename I > void LoadRequest < I >::image_map_list()
{
    ...
    librbd::cls_client::mirror_image_map_list_start(&op,
            m_start_after,
            MAX_RETURN);
    ...
    m_out_bl.clear();
    int r =
        m_ioctx.aio_operate(RBD_MIRROR_LEADER, aio_comp, &op,
                            &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
} template < typename I >
void LoadRequest < I >::handle_image_map_list(int r)
{
    ...
    if (r == 0) {
        ...
        r = librbd::cls_client::mirror_image_map_list_finish(&it,
            &image_mapping);
    }
    ...
    m_image_mapping->insert(image_mapping.begin(),
                            image_mapping.end());
...
template < typename I > void LoadRequest < I >::mirror_image_list()
{
    ...
    int r =
        m_ioctx.aio_operate(RBD_MIRRORING, aio_comp, &op,
                            &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}
