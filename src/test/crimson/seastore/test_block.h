...
namespace crimson::os::seastore {
    ...
    inline std::ostream & operator<<(std::ostream & lhs,
                                     const test_extent_desc_t & rhs) {
        return lhs << "test_extent_desc_t(len=" << rhs.
            len << ", checksum=" << rhs.checksum << ")";
    }
    ...
    struct TestBlockPhysical:crimson::os::seastore::CachedExtent {
      ...
      :    CachedExtent(std::move(ptr)) {
        ...
        void apply_delta_and_adjust_crc(paddr_t,
    ...
    struct test_block_mutator_t {
        std::uniform_int_distribution < int8_t >
            ...
            std::uniform_int_distribution < int8_t > (std::numeric_limits <
                                                      int8_t >::min(),
                                                      std::numeric_limits <
                                                      int8_t >::max());
        ...
        std::uniform_int_distribution < uint16_t >
            length_distribution(uint16_t offset) {
            return std::uniform_int_distribution < uint16_t > (0,
                                                               TestBlock::SIZE -
                                                               offset - 1);
        } template < typename generator_t > void mutate(TestBlock & block,
                                                        generator_t & gen) {
            auto offset = offset_distribution(gen);
            block.set_contents(contents_distribution(gen),
                               offset, length_distribution(offset) (gen));
        }
    };
...
template <> struct fmt::formatter <
    crimson::os::seastore::test_extent_desc_t >:fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::TestBlock >:fmt::ostream_formatter {
};
template <> struct fmt::formatter <
    crimson::os::seastore::TestBlockPhysical >:fmt::ostream_formatter {
};
#endif
