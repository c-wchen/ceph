...
void SnapClient::handle_notify_prep(const cref_t < MMDSTableRequest > &m)
{
    ...
    auto ack =
        make_message < MMDSTableRequest > (table, TABLESERVER_OP_NOTIFY_ACK, 0,
                                           m->get_tid());
...
void SnapClient::refresh(version_t want, MDSContext * onfinish)
{
    ...
    auto req =
        make_message < MMDSTableRequest > (table, TABLESERVER_OP_QUERY,
                                           ++last_reqid, 0);
    using ceph::encode;
    char op = 'F';
    encode(op, req->bl);
...
const SnapInfo *SnapClient::get_snap_info(snapid_t snapid) const const
{
    ceph_assert(cached_version > 0);
