    ...
    if (mf == true || offset != 0) {
        ...
        if (frag.is_complete()) {
            ...
            if (l4) {
                ...
                cpu_id =
                    _netif->
                    hash2cpu(toeplitz_hash(_netif->rss_key(), hash_data));
            }
                ...
                Packet p, ethernet_address e_dst)
{
    ...
    if (needs_frag) {
        ...
        while (remaining) {
            auto can_send =
                std::min(uint16_t(mtu - ipv4_hdr_len_min), remaining);
            remaining -= can_send;
...
std::optional < l3_protocol::l3packet > ipv4::get_packet()
{
    ...
    if (_packetq.empty()) {
        for (size_t i = 0; i < _pkt_providers.size(); i++) {
            ...
            if (l4p) {
                ldout(cct,
                      20) << " ipv4::get_packet len " << l4p->p.len() << dendl;
