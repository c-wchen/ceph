...
struct TestMockWatcherRewatchRequest: public TestMockFixture {
    ...
    void expect_aio_watch(MockImageCtx &mock_image_ctx, int r)
    {
        ...
        mock_io_ctx(get_mock_io_ctx(mock_image_ctx.md_ctx));

        EXPECT_CALL(mock_io_ctx,
                    aio_watch(mock_image_ctx.header_oid, _, _, _))
        .
        WillOnce(DoAll
                 (WithArgs < 1,
                  2 >
                  (Invoke
                   ([&mock_image_ctx, &mock_io_ctx,
        ...
        uint64_t *cookie) {
            *cookie = 234;
            ...
            queue(new
            LambdaContext([&mock_io_ctx, c](int r) {
                mock_io_ctx.
    ...
    ceph::shared_mutex m_watch_lock =
        ceph::make_shared_mutex("watch_lock");
...
TEST_F(TestMockWatcherRewatchRequest, Success)
{
    ...
    C_SaferCond ctx;
    MockRewatchRequest *req =
        MockRewatchRequest::create(mock_image_ctx.md_ctx,
