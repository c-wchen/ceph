                     ...
                     Formatter *formatter)
{
    ...
    zone_policy_handler->reflect(dpp(), nullptr, nullptr, nullptr, nullptr, &source_zones, &target_zones,
    ...
    for (auto &hint_entity : hint_entities) {
        ...
        hint_bucket_handler->get_pipes(&resolved_dests, &resolved_sources,
                              ...
                              std::ostream &out)
{
    ...
    for (auto &zone_id : zone_ids) {
        ...
        if (z == static_cast < rgw::sal::RadosStore * >
            (driver)->svc()->zone->get_zonegroup().zones.end()) {    /* should't happen */
            continue;
        }
...
#define MAX_TOTP_SKEW_HOURS (24 * 7)
    ...
    while (time_ofs_abs < max_skew) {
        ...
        if (rc != OATH_INVALID_OTP) {
            rc = oath_totp_validate2(totp.seed_bin.c_str(), totp.seed_bin.length(), start_time, step_size,
