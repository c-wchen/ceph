...
public:
    struct StoreLCHead : LCHead {
        ...
        StoreLCHead() = default;
        StoreLCHead(time_t _start_date, time_t _rollover_date, std::string &_marker) : start_date(_start_date),
    ...
    struct StoreLCEntry : LCEntry {
        ...
        StoreLCEntry(std::string &_bucket, uint64_t _time, uint32_t _status) : bucket(_bucket), start_time(_time),
            status(_status) {}
        StoreLCEntry(std::string &_bucket, std::string _oid, uint64_t _time, uint32_t _status) : bucket(_bucket), oid(_oid),
