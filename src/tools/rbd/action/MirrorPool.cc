...
{
    options->add_options()
    (SITE_NAME.c_str(), po::value < std::string > (),
     "local site name");
} int set_site_name(librados::Rados &rados,
                    const std::string &site_name)
{
    ...
    if (r == -EOPNOTSUPP) {
        std::
        cerr << "rbd: cluster does not support site names"
             << std::endl;
        return r;
    } else if (r < 0) {
        std::
        cerr << "rbd: failed to set site name" <<
             cpp_strerror(r)
             << std::endl;
        return r;
    }
...
int validate_mirroring_enabled(librados::IoCtx &io_ctx)
{
    ...
    if (mirror_mode == RBD_MIRROR_MODE_DISABLED) {
        std::
        cerr << "rbd: mirroring not enabled on the pool" <<
             std::endl;
        return -EINVAL;
    }
    return 0;
...
int read_key_file(std::string path, std::string *key)
{
    ...
    key_file.open(path);
    if (key_file.fail()) {
        std::cerr << "rbd: failed to open " << path << std::
                  endl;
        return -EINVAL;
    }
    ...
    if (key_file.bad()) {
        std::
        cerr << "rbd: failed to read key from " << path <<
             std::endl;
        return -EINVAL;
    }
                            ...
                            std::string > *attributes)
{
    ...
    if (vm.count("remote-cluster")) {
        *remote_cluster =
            vm["remote-cluster"].as < std::string > ();
    }
    if (vm.count("remote-mon-host")) {
        (*attributes)["mon_host"] =
            vm["remote-mon-host"].as < std::string > ();
    }
    if (vm.count("remote-key-file")) {
        std::string key;
        int r =
            read_key_file(vm["remote-key-file"].as <
                          std::string > (), &key);
        if (r < 0) {
            return r;
        }
    ...
    if (remote_cluster->empty()) {
        std::
        cerr << "rbd: remote cluster was not specified" <<
             std::endl;
        return -EINVAL;
    }
    return 0;
...
int set_peer_config_key(librados::IoCtx &io_ctx,
                        ...
                        std::string > &&attributes)
{
    librbd::RBD rbd;
    int r =
        rbd.mirror_peer_site_set_attributes(io_ctx, peer_uuid,
                                            attributes);
    if (r == -EPERM) {
        std::
        cerr <<
...
int get_peer_config_key(librados::IoCtx &io_ctx,
                        const std::string &peer_uuid,
                        std::map < std::string,
                        std::string > *attributes)
{
    librbd::RBD rbd;
    int r =
        rbd.mirror_peer_site_get_attributes(io_ctx, peer_uuid,
                                            attributes);
    if (r == -ENOENT) {
        return r;
    } else if (r == -EPERM) {
        ...
        cerr <<
             ...
             << "config-key secrets from the monitor" << std::
             endl;
        return r;
    } else if (r == -EINVAL) {
        std::
        cerr << "rbd: corrupt mirroring peer config" <<
             std::endl;
        return r;
    } else if (r < 0) {
        std::
        cerr << "rbd: error reading mirroring peer config: "
             << cpp_strerror(r) << std::endl;
        return r;
    }
                           ...
                           const std::string &value)
{
    ...
    if (r == -ENOENT) {
        return set_peer_config_key(io_ctx, peer_uuid, { {
                key, value
            }
        }
                                  );
...
int format_mirror_peers(librados::IoCtx &io_ctx,
                        ...
                        const std::vector <
                        librbd::mirror_peer_site_t > &peers,
                        bool config_key)
{
    ...
    for (auto &peer : peers) {
        ...
        if (config_key) {
            int r =
                get_peer_config_key(io_ctx, peer.uuid,
                                    &attributes);
            if (r < 0 && r != -ENOENT) {
                return r;
            }
        ...
        if (formatter != nullptr) {
            ...
            formatter->dump_string("mirror_uuid",
                                   peer.mirror_uuid);
            formatter->dump_string("client_name",
                                   peer.client_name);
            for (auto &pair : attributes) {
                formatter->dump_string(pair.first.c_str(),
                                       pair.second);
            }
            ...
            if (peer.direction != RBD_MIRROR_PEER_DIRECTION_RX
                ...
                std::cout << "Mirror UUID: " << peer.
                          mirror_uuid << std::endl;
            }
            std::cout << "Direction: " << direction << std::
                      endl;
            if (peer.direction != RBD_MIRROR_PEER_DIRECTION_TX
                || !peer.client_name.empty()) {
                std::cout << "Client: " << peer.
                          client_name << std::endl;
            }
            if (config_key) {
                std::
                cout << "Mon Host: " <<
                     attributes["mon_host"] << std::
                     endl << "Key: " << attributes["key"] <<
                     std::endl;
            }
...
public:
    void send()
    {
        dout(20) << this << " " << __func__ << ": image_name="
                 << m_image_name << dendl;
...
protected:
    ImageRequestBase(librados::IoCtx &io_ctx,
                     OrderedThrottle &throttle,
                     const std::string &image_name)
        : m_io_ctx(io_ctx), m_throttle(throttle),
    ...
    {
        return false;
    } virtual void get_info(librbd::Image &image,
                            librbd::mirror_image_info_t *info,
                            librbd::RBD::AioCompletion *
                            aio_comp)
    {
        image.aio_mirror_image_get_info(info,
                                        sizeof(librbd::
                                               mirror_image_info_t),
                                        aio_comp);
    }

    virtual bool skip_action(const librbd::
                             mirror_image_info_t &info) const
    {
        return false;
    ...
    {
        dout(20) << this << " " << __func__ << ": r=" << r <<
                 dendl;

        if (r < 0 && r != -ENOENT) {
            std::
            cerr << "rbd: failed to " << get_action_type()
                 << " image " << m_image_name << ": " <<
                 cpp_strerror(r) << std::endl;
            m_ret_val = r;
        }
...
private:
    ...
    void open_image()
    {
        ...
        auto aio_completion = utils::create_aio_completion <
                              ImageRequestBase,
                              &ImageRequestBase::handle_open_image > (this);
        rbd.aio_open(m_io_ctx, m_image, m_image_name.c_str(),
                     nullptr, aio_completion);
    }
    ...
    void get_info()
    {
        ...
        auto aio_completion = utils::create_aio_completion <
                              ImageRequestBase,
                              &ImageRequestBase::handle_get_info > (this);
    ...
    void execute_action()
    {
        ...
        auto aio_completion = utils::create_aio_completion <
                              ImageRequestBase,
                              &ImageRequestBase::handle_execute_action > (this);
    ...
    void close_image()
    {
        ...
        auto aio_completion = utils::create_aio_completion <
                              ImageRequestBase,
                              &ImageRequestBase::handle_close_image > (this);
...
{
public:
    PromoteImageRequest(librados::IoCtx &io_ctx,
                        OrderedThrottle &throttle,
                        const std::string &image_name,
                        std::atomic < unsigned > *counter,
                        bool force)
        : ImageRequestBase(io_ctx, throttle, image_name),
          m_counter(counter), m_force(force)
    {
    } protected:
    bool skip_action(const librbd::
                     mirror_image_info_t &info) const override
    {
        return (info.state != RBD_MIRROR_IMAGE_ENABLED
                || info.primary);
...
{
public:
    DemoteImageRequest(librados::IoCtx &io_ctx,
                       OrderedThrottle &throttle,
                       const std::string &image_name,
                       std::atomic < unsigned > *counter)
        : ImageRequestBase(io_ctx, throttle, image_name),
...
{
public:
    StatusImageRequest(librados::IoCtx &io_ctx,
                       OrderedThrottle &throttle,
                       const std::string &image_name,
                       ...
                       &mirror_peers,
                       const std::map < std::string,
                       std::string > &peer_mirror_uuids_to_name,
                       const MirrorDaemonServiceInfo &
                       daemon_service_info,
                       at::Format::Formatter formatter)
        : ImageRequestBase(io_ctx, throttle, image_name),
                          ...
                          aio_comp) override
    {
        ...
        (&m_mirror_image_global_status,
         sizeof(m_mirror_image_global_status), aio_comp);
    }
    ...
    void finalize_action() override
    {
        ...
        m_mirror_image_global_status.site_statuses.
        erase(std::
              remove_if(m_mirror_image_global_status.
                        site_statuses.begin(),
                        m_mirror_image_global_status.
                        site_statuses.end(),
        [](auto & status) {
        ...
        std::string instance_id = (local_site_r >= 0
                                   && local_status.up
                                   && m_instance_ids.
                                   count(m_image_id)) ?
        ...
        if (m_formatter != nullptr) {
            ...
            if (local_site_r >= 0) {
                ...
                m_formatter->dump_string("last_update",
                                         utils::
                                         timestr(local_status.
                                                 last_update));
            }
            if (!m_mirror_image_global_status.site_statuses.
                empty()) {
                m_formatter->open_array_section("peer_sites");
                for (auto &status : m_mirror_image_global_status.
                     site_statuses) {
                    m_formatter->
                    open_object_section("peer_site");

                    auto name_it =
                        m_peer_mirror_uuids_to_name.find(status.
                                                         mirror_uuid);
                    m_formatter->dump_string("site_name",
                                             (name_it !=
                                              ...
                                              second : ""));
                    m_formatter->dump_string("mirror_uuids",
                                             status.
                                             mirror_uuid);
            ...
            std::cout << std::endl
                      ...
                      name << ":" << std::
                      endl << "  global_id:   " <<
                      m_mirror_image_global_status.info.
                      global_id << std::endl;
            if (local_site_r >= 0) {
                std::cout << "  state:       " << utils::
                          mirror_image_site_status_state(local_status)
                          << std::
                          endl << "  description: " << local_status.
            ...
            if (!m_mirror_image_global_status.site_statuses.
                ...
                for (auto &site : m_mirror_image_global_status.
                     site_statuses) {
                    if (!first_site) {
                        std::cout << std::endl;
                    }
                    ...
                    auto name_it =
                        m_peer_mirror_uuids_to_name.find(site.
                                                         mirror_uuid);
                    std::cout << "    name: " << (name_it !=
                                                  m_peer_mirror_uuids_to_name.
                                                  end() ?
                                                  name_it->
                                                  second : site.
                                                  mirror_uuid)
                              << std::endl << "    state: " << utils::
                              mirror_image_site_status_state(site) <<
                              std::
                              endl << "    description: " << site.
                              description << std::
                              endl << "    last_update: " << utils::
                              timestr(site.last_update) << std::endl;
                }
            }
        }
    }
    ...
    const std::vector < librbd::mirror_peer_site_t >
    &m_mirror_peers;
    const std::map < std::string,
          std::string > &m_peer_mirror_uuids_to_name;
...
public:
    template < class ... Args >
    RequestT *operator()(librados::IoCtx &io_ctx,
                         OrderedThrottle &throttle,
                         const std::string &image_name,
                         Args && ... args)
    {
...
{
public:
    template < class ... Args > ImageRequestGenerator(librados::IoCtx &io_ctx, Args
            && ... args)
        :    m_io_ctx(io_ctx),
             m_factory(std::
                       bind(ImageRequestAllocator < RequestT > (),
                            std::ref(m_io_ctx), std::ref(m_throttle),
                            std::placeholders::_1,
                            std::forward < Args > (args) ...)),
             m_throttle(g_conf().get_val < uint64_t >
                        ("rbd_concurrent_management_ops"), true)
    {
    }
    ...
    int execute()
    {
        ...
        if (r < 0 && r != -ENOENT) {
            std::
            cerr << "rbd: failed to list images within pool"
                 << std::endl;
            return r;
        }
...
private:
    ...
    RequestT *(const std::string &) > Factory;
...
int get_mirror_image_status(librados::IoCtx &io_ctx,
                            ...
                            mirror_image_health)
{
    librbd::RBD rbd;
    int r =
        rbd.mirror_image_status_summary(io_ctx,
                                        mirror_image_states);
    if (r < 0) {
        std::
        cerr <<
...
int execute_peer_bootstrap_create(const po::variables_map &vm,
                                  ...
                                  &ceph_global_init_args)
{
    ...
    size_t arg_index = 0;
    int r =
        utils::get_pool_and_namespace_names(vm, true, &pool_name,
                                            nullptr, &arg_index);
    if (r < 0) {
        return r;
    }
    ...
    if (vm.count(SITE_NAME)) {
        r = set_site_name(rados,
                          vm[SITE_NAME].as < std::string > ());
        if (r < 0) {
            return r;
        }
    }
...
void get_peer_bootstrap_import_arguments(po::options_description *
        positional,
        po::options_description *
        options)
{
...
int execute_peer_bootstrap_import(const po::variables_map &vm,
                                  const std::vector < std::string >
                                  &ceph_global_init_args)
{
    std::string pool_name;
    ...
    if (token_path.empty()) {
        std::cerr << "rbd: token path was not specified" << std::
                  endl;
        return -EINVAL;
    }
    ...
    if (vm.count("direction")) {
        mirror_peer_direction =
            vm["direction"].as < rbd_mirror_peer_direction_t > ();
    }
    ...
    if (token_path != "-") {
        ...
        if (fd < 0) {
            r = -errno;
            std::
            cerr << "rbd: error opening " << token_path << std::
                 endl;
            return r;
        }
    }
    ...
    if (r < 0) {
        std::
        cerr << "rbd: error reading token file: " <<
             cpp_strerror(r)
             << std::endl;
        return r;
    }
    ...
    librbd::RBD rbd;
    r = rbd.mirror_peer_bootstrap_import(io_ctx,
                                         mirror_peer_direction,
                                         token);
    if (r == -ENOSYS) {
        std::
        cerr << "rbd: mirroring is not enabled on remote peer"
             << std::endl;
        return r;
    } else if (r < 0) {
        std::
        cerr << "rbd: failed to import peer bootstrap token" <<
             std::endl;
        return r;
    }
                            ...
                            po::options_description *options)
{
    ...
    options->add_options()
    ("remote-client-name", po::value < std::string > (),
     "remote client name")
    ("remote-cluster", po::value < std::string > (),
     "remote cluster name")
    ("remote-mon-host", po::value < std::string > (),
     "remote mon host(s)")
                     ...
                     &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to list mirror peers" << std::
                  endl;
        return r;
    }
    ...
    mirror_peers.
    erase(std::
          remove_if(mirror_peers.begin(), mirror_peers.end(),
                    [](const librbd::
    ...
    std::string uuid;
    r = rbd.mirror_peer_site_add(io_ctx, &uuid,
                                 mirror_peer_direction,
                                 remote_cluster,
                                 remote_client_name);
    if (r == -EEXIST) {
    ...
    if (!attributes.empty()) {
        r = set_peer_config_key(io_ctx, uuid,
                                std::move(attributes));
        if (r < 0) {
            return r;
        }
                     ...
                     &ceph_global_init_args)
{
    ...
    if (value.empty() && (key == "client" || key == "cluster")) {
        std::
        cerr << "rbd: must specify new " << key << " value." <<
             std::endl;
    ...
    librbd::RBD rbd;
    if (key == "client") {
        r = rbd.mirror_peer_site_set_client_name(io_ctx,
            uuid.c_str(),
            value.c_str());
    } else if (key == "site-name" || key == "cluster") {
        r = rbd.mirror_peer_site_set_name(io_ctx, uuid.c_str(),
                                          value.c_str());
        ...
        auto peer_direction =
            boost::any_cast < rbd_mirror_peer_direction_t >
            (direction);
        if (peer_direction != RBD_MIRROR_PEER_DIRECTION_TX) {
            ...
            if (r < 0) {
                std::
                cerr << "rbd: failed to list mirror peers" <<
                     std::endl;
                return r;
            }
        ...
        r = rbd.mirror_peer_site_set_direction(io_ctx, uuid,
                                               peer_direction);
    } else {
    ...
    if (r == -ENOENT) {
        std::
        cerr << "rbd: mirror peer " << uuid << " does not exist"
             << std::endl;
    }
                           ...
                           bool ignore_no_update)
{
    ...
    if (current_mirror_mode == next_mirror_mode) {
        if (!ignore_no_update) {
            if (mode == "disabled") {
                std::
                cout << "rbd: mirroring is already " << mode <<
                     std::endl;
            } else {
                std::
                cout <<
                    ...
                    &ceph_global_init_args)
{
    ...
    return execute_enable_disable(io_ctx, RBD_MIRROR_MODE_DISABLED,
                                  "disabled", false);
}
                   ...
                   &ceph_global_init_args)
{
    ...
    if (mode == "image") {
        ...
        std::
        cerr << "rbd: must specify 'image' or 'pool' mode." <<
             std::endl;
        return -EINVAL;
    }
    ...
    return execute_enable_disable(io_ctx, mirror_mode, mode,
                                  updated);
}
                 ...
                 &ceph_global_init_args)
{
    ...
    size_t arg_index = 0;
    int r =
        utils::get_pool_and_namespace_names(vm, false, &pool_name,
                                            &namespace_name,
                                            &arg_index);
    if (r < 0) {
        return r;
                   ...
                   &ceph_global_init_args)
{
    ...
    std::map < librbd::mirror_image_status_state_t,
        int >mirror_image_states;
    MirrorHealth mirror_image_health = MIRROR_HEALTH_UNKNOWN;
    r = get_mirror_image_status(io_ctx, &total_images,
                                &mirror_image_states,
                                &mirror_image_health);
    if (r < 0) {
        return r;
    ...
    MirrorHealth mirror_daemon_health =
        daemon_service_info.get_daemon_health();
    auto mirror_services =
        daemon_service_info.get_mirror_services();

    auto mirror_health =
        std::max(mirror_image_health, mirror_daemon_health);
    ...
    if (formatter != nullptr) {
        ...
        for (auto &it : mirror_image_states) {
            std::string state_name =
                utils::mirror_image_status_state(it.first);
        ...
        std::
        cout << "daemon health: " << mirror_daemon_health <<
             std::endl;
        std::
        cout << "image health: " << mirror_image_health << std::
             endl;
        std::cout << "images: " << total_images << " total" << std::
                  endl;
        for (auto &it : mirror_image_states) {
            std::cout << "    " << it.second << " "
    ...
    if (verbose) {
        ...
        if (formatter != nullptr) {
            ...
            for (auto &mirror_service : mirror_services) {
                ...
                formatter->dump_string("service_id",
                                       mirror_service.service_id);
                formatter->dump_string("instance_id",
                                       mirror_service.instance_id);
                formatter->dump_string("client_id",
                                       mirror_service.client_id);
                formatter->dump_string("hostname",
                                       mirror_service.hostname);
                formatter->dump_string("ceph_version",
                                       mirror_service.ceph_version);
                formatter->dump_bool("leader",
                                     mirror_service.leader);
                formatter->dump_stream("health") << mirror_service.
                                                 health;
                if (!mirror_service.callouts.empty()) {
                    formatter->open_array_section("callouts");
                    for (auto &callout : mirror_service.
                         callouts) {
            ...
            for (auto &mirror_service : mirror_services) {
                std::cout << "service " << mirror_service.
                          service_id << ":" << std::
                          endl << "  instance_id: " << mirror_service.
                          instance_id << std::
                          endl << "  client_id: " << mirror_service.
                          client_id << std::
                          endl << "  hostname: " << mirror_service.
                          hostname << std::
                          endl << "  version: " << mirror_service.
                          ceph_version << std::
                          endl << "  leader: " << (mirror_service.
                                                   leader ? "true" :
                                                   "false")
                          << std::endl << "  health: " << mirror_service.
                          health << std::endl;
                if (!mirror_service.callouts.empty()) {
                    std::cout << "  callouts: " << mirror_service.
                              callouts << std::endl;
                }
                std::cout << std::endl;
            }
        ...
        std::vector < librbd::mirror_peer_site_t > mirror_peers;
        utils::get_mirror_peer_sites(default_ns_io_ctx,
                                     &mirror_peers);
        ...
        while (true) {
            ...
            r = rbd.mirror_image_instance_id_list(io_ctx,
                                                  start_image_id,
                                                  1024, &ids);
            if (r < 0) {
                if (r == -EOPNOTSUPP) {
                    ...
                    cerr <<
                         ...
                         << "to rbd-mirror daemon instance" << std::
                         endl;
                } else {
                    std::
                    cerr <<
        ...
        ImageRequestGenerator < StatusImageRequest >
        generator(io_ctx, instance_ids, mirror_peers,
                  peer_mirror_uuids_to_name,
                    ...
                    &ceph_global_init_args)
{
    ...
    ImageRequestGenerator < PromoteImageRequest > generator(io_ctx,
            &counter,
            vm
            ["force"].
            as <
                   ...
                   &ceph_global_init_args)
{
    ...
    std::atomic < unsigned >counter {
        0};
    ImageRequestGenerator < DemoteImageRequest > generator(io_ctx,
            &counter);
    r = generator.execute();

    std::cout << "Demoted " << counter.
              load() << " mirrored images" << std::endl;
    return r;
}
