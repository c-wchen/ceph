...
int CLSRGWConcurrentIO::operator()()
{
    ...
    while (manager.wait_for_completions(valid_ret_code(), &num_completions, &r,
                                        need_multiple_rounds() ? &completed_objs
                                        : nullptr,
                                        !need_multiple_rounds() ? &retry_objs :
                                        nullptr)) {
        if (r >= 0 && ret >= 0) {
            for (; num_completions && iter != objs_container.end();
                 --num_completions, ++iter) {
...
private:
    ...
    int *ret_code;
public:
    ClsBucketIndexOpCtx(T *_data, int *_ret_code): data(_data),
        ret_code(_ret_code)
    {
        ceph_assert(data);
    }
...
bool BucketIndexAioManager::wait_for_completions(int valid_ret_code,
        ...
        int *ret_code,
        std::map < int,
        std::string > *completed_objs,
        std::map < int,
        std::string > *retry_objs)
{
        ...
        if (completed_objs || retry_objs) {
            ...
            if (liter != completion_objs.end()) {
                if (completed_objs && r == 0) { /* update list of successfully completed objs */
                    (*completed_objs)[liter->second.shard_id] =
                        liter->second.oid;
                }
                ...
                if (r == RGWBIAdvanceAndRetryError) {
                    r = 0;
                    if (retry_objs) {
                        (*retry_objs)[liter->second.shard_id] =
                            liter->second.oid;
                    }
                }
            } else {
...
int CLSRGWIssueBucketIndexClean::issue_op(const int shard_id,
        const string &oid)
{
...
{
    return issue_bucket_set_tag_timeout_op(io_ctx, shard_id, oid, tag_timeout,
                                           &manager);
}

void cls_rgw_bucket_update_stats(librados::ObjectWriteOperation &o,
                                 bool absolute,
                                 const map < RGWObjCategory,
...
void cls_rgw_bucket_prepare_op(ObjectWriteOperation &o, RGWModifyOp op,
                               const string &tag, const cls_rgw_obj_key &key,
...
void cls_rgw_bi_put(ObjectWriteOperation &op, const string oid,
                    const rgw_cls_bi_entry &entry)
{
    bufferlist in, out;
    rgw_cls_bi_put_op call;
                         ...
                         rgw_cls_read_olh_log_ret &log_ret, int &op_ret)
{
    ...
    call.olh_tag = olh_tag;
    encode(call, in);
    op.exec(RGW_CLASS, RGW_BUCKET_READ_OLH_LOG, in,
            new ClsBucketIndexOpCtx < rgw_cls_read_olh_log_ret > (&log_ret,
                    &op_ret));
}
...
int cls_rgw_clear_olh(IoCtx &io_ctx, string &oid, const cls_rgw_obj_key &olh,
                      const string &olh_tag)
{
    librados::ObjectWriteOperation op;
                        ...
                        cls_rgw_bi_log_list_ret *pdata, int *ret)
{
    ...
    bufferlist in;
    encode(call, in);
    op.exec(RGW_CLASS, RGW_BI_LOG_LIST, in,
            new ClsBucketIndexOpCtx < cls_rgw_bi_log_list_ret > (pdata, ret));
}

static bool issue_bi_log_list_op(librados::IoCtx &io_ctx, const string &oid,
                                 const int shard_id,
                                 BucketIndexShardsManager &marker_mgr,
                                 uint32_t max, BucketIndexAioManager *manager,
...
{
    return issue_bi_log_list_op(io_ctx, oid, shard_id, marker_mgr, max,
...
static bool issue_bi_log_trim(librados::IoCtx &io_ctx, const string &oid,
                              const int shard_id,
...
{
    return issue_bi_log_trim(io_ctx, oid, shard_id, start_marker_mgr,
                             end_marker_mgr, &manager);
}

static bool issue_bucket_check_index_op(IoCtx &io_ctx, const int shard_id,
                                        const string &oid,
                                        BucketIndexAioManager *manager,
                                        rgw_cls_check_index_ret *pdata)
{
    bufferlist in;
    librados::ObjectReadOperation op;
    op.exec(RGW_CLASS, RGW_BUCKET_CHECK_INDEX, in,
            new ClsBucketIndexOpCtx < rgw_cls_check_index_ret > (pdata, NULL));
    return manager->aio_operate(io_ctx, shard_id, oid, &op);
}

int CLSRGWIssueBucketCheck::issue_op(int shard_id, const string &oid)
{
    return issue_bucket_check_index_op(io_ctx, shard_id, oid, &manager,
                                       &result[shard_id]);
}

static bool issue_bucket_rebuild_index_op(IoCtx &io_ctx, const int shard_id,
        const string &oid,
        BucketIndexAioManager *manager)
{
    bufferlist in;
...
static bool issue_resync_bi_log(librados::IoCtx &io_ctx, const int shard_id,
                                const string &oid,
                                BucketIndexAioManager *manager)
{
    bufferlist in;
...
int CLSRGWIssueResyncBucketBILog::issue_op(const int shard_id,
        const string &oid)
{
    return issue_resync_bi_log(io_ctx, shard_id, oid, &manager);
}

static bool issue_bi_log_stop(librados::IoCtx &io_ctx, const int shard_id,
                              const string &oid,
                              BucketIndexAioManager *manager)
{
    bufferlist in;
...
int cls_rgw_get_dir_header_async(IoCtx &io_ctx, string &oid,
...
int cls_rgw_usage_log_read(IoCtx &io_ctx, const string &oid,
                           const string &user, const string &bucket,
...
int cls_rgw_usage_log_trim(IoCtx &io_ctx, const string &oid,
                           const string &user, const string &bucket,
                           uint64_t start_epoch, uint64_t end_epoch)
{
    bufferlist in;
...
void cls_rgw_gc_set_entry(ObjectWriteOperation &op, uint32_t expiration_secs,
...
void cls_rgw_gc_defer_entry(ObjectWriteOperation &op, uint32_t expiration_secs,
                            const string &tag)
{
    bufferlist in;
...
int cls_rgw_gc_list(IoCtx &io_ctx, string &oid, string &marker, uint32_t max,
                    bool expired_only, list < cls_rgw_gc_obj_info > &entries,
...
int cls_rgw_lc_get_head(IoCtx &io_ctx, const string &oid,
...
int cls_rgw_lc_put_head(IoCtx &io_ctx, const string &oid,
                        cls_rgw_lc_obj_head &head)
{
    bufferlist in, out;
...
int cls_rgw_lc_get_next_entry(IoCtx &io_ctx, const string &oid,
                              const string &marker, cls_rgw_lc_entry &entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_list(IoCtx &io_ctx, const string &oid,
                    const string &marker,
                    uint32_t max_entries, vector < cls_rgw_lc_entry > &entries)
{
    bufferlist in, out;
...
static bool issue_set_bucket_resharding(librados::IoCtx &io_ctx,
                                        ...
                                        entry, BucketIndexAioManager *manager)
{
    bufferlist in;
    cls_rgw_set_bucket_resharding_op call;
    call.entry = entry;
...
int CLSRGWIssueSetBucketResharding::issue_op(const int shard_id,
        const string &oid)
{
