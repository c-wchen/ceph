...
monc(monc), name(name), compat(MDSMap::get_compat_set_all())
{
}
...
void Beacon::init(const MDSMap & mdsmap)
{
    ...
    sender = std::thread([this] () {
                         std::unique_lock < std::mutex > lock(mutex);
                         bool sent; while (!finished) {
                         auto now = clock::now();
                         auto since =
                         std::chrono::duration <
                         double >(now - last_send).count();
                         auto interval = beacon_interval; sent = false;
                         ...
                         interval -= since;}
                         dout(20) << "sender thread waiting interval " <<
                         interval << "s" << dendl;
                         if (cvar.wait_for(lock, interval * 1 s) ==
                             std::cv_status::timeout) {
                         if (sent) {
                         //missed beacon ack because we timedout after a beacon send
                         dout(0) << "missed beacon ack from the monitors" <<
                         dendl; missed_beacon_ack_dump = true;}
                         }
                         }
                         }
    ) ;
...
bool Beacon::ms_can_fast_dispatch2(const cref_t < Message > &m) const const
{
...
void Beacon::handle_mds_beacon(const cref_t < MMDSBeacon > &m)
{
    ...
    if (it != seq_stamp.end()) {
        ...
        auto rtt =
            std::chrono::duration < double >(now - last_acked_stamp).count();

        dout(5) << "received beacon reply " << ceph_mds_state_name(m->
                                                                   get_state())
...
void Beacon::send_and_wait(const double duration)
{
    ...
    while (!seq_stamp.empty() && seq_stamp.begin()->first <= awaiting_seq) {
        auto now = clock::now();
        auto s =
            duration * .95 - std::chrono::duration <
            double >(now - start).count();
        if (s < 0) {
            //missed beacon ACKs
            missed_beacon_ack_dump = true;
            break;
        }
        cvar.wait_for(lock, s * 1 s);
    }
}
...
bool Beacon::_send()
{
    ...
    auto beacon =
        make_message < MMDSBeacon > (monc->get_fsid(),
...
bool Beacon::is_laggy()
{
    ...
    if (since > g_conf()->mds_beacon_grace) {
        if (!laggy) {
            dout(1) << "MDS connection to Monitors appears to be laggy; " <<
                since << "s since last acked beacon" << dendl;
        }
        laggy = true;
        return true;
    }
...
void Beacon::notify_health(MDSRank const *mds)
{
    ...
    if (!mds->damage_table.empty()) {
        MDSHealthMetric m(MDS_HEALTH_DAMAGE, HEALTH_ERR,
                          std::string("Metadata damage detected"));
        health.metrics.push_back(m);
    }
    ...
    // Indicates MDS is not trimming promptly
    {
        if (mds->mdlog->get_num_segments() >
            (size_t) (g_conf()->mds_log_max_segments * g_conf().get_val <
                      double >("mds_log_warn_factor"))) {
            ...
            *css << "Behind on trimming (" << mds->mdlog->get_num_segments()
            ...
            m.metadata["num_segments"] =
                stringify(mds->mdlog->get_num_segments());
            m.metadata["max_segments"] =
                stringify(g_conf()->mds_log_max_segments);
            health.metrics.push_back(m);
        }
    }
    ...
    // CLIENT_CAPS messages.
    {
        auto &&
            late_clients =
            mds->locker->get_late_revoking_clients(mds->mdsmap->
                                                   get_session_timeout());
      ...
      for (const auto & client:late_clients) {
            ...
            *css << "Client " << s->
            ...
            MDSHealthMetric m(MDS_HEALTH_CLIENT_LATE_RELEASE, HEALTH_WARN,
                              css->strv());
        ...
        if (late_cap_metrics.size() <=
            ...
            *css << "Many clients (" << late_cap_metrics.size()
            ...
            MDSHealthMetric m(MDS_HEALTH_CLIENT_LATE_RELEASE_MANY, HEALTH_WARN,
                              css->strv());
    ...
    // Detect clients failing to advance their old_client_tid
    {
        ...
        const auto max_completed_requests =
      ...
      for (auto & session:sessions) {
            ...
            if (recall_caps > recall_warning_threshold
                ...
                *css << "Client " << session->
                ...
                MDSHealthMetric m(MDS_HEALTH_CLIENT_RECALL, HEALTH_WARN,
                                  css->strv());
            ...
            if ((session->get_num_trim_requests_warnings() > 0 &&
                ...
                || (session->get_num_trim_flushes_warnings() > 0
                    ...
                    max_completed_flushes)) {
                CachedStackStringStream css;
                *css << "Client " << session->
                    get_human_name() <<
                    " failing to advance its oldest client/flush tid. ";
                MDSHealthMetric m(MDS_HEALTH_CLIENT_OLDEST_TID, HEALTH_WARN,
                                  css->strv());
                m.metadata["client_id"] = stringify(session->get_client());
        ...
        if (late_recall_metrics.size() <=
            ...
            *css << "Many clients (" << late_recall_metrics.size()
            ...
            MDSHealthMetric m(MDS_HEALTH_CLIENT_RECALL_MANY, HEALTH_WARN,
                              css->strv());
        ...
        if (large_completed_requests_metrics.size() <=
            ...
            *css << "Many clients (" << large_completed_requests_metrics.size()
            ...
            MDSHealthMetric m(MDS_HEALTH_CLIENT_OLDEST_TID_MANY, HEALTH_WARN,
                              css->strv());
    ...
    // Detect MDS_HEALTH_SLOW_REQUEST condition
    {
        ...
        if (slow) {
            ...
            CachedStackStringStream css;
            *css << slow << " slow requests are blocked > " << g_conf()->
                mds_op_complaint_time << " secs";

            MDSHealthMetric m(MDS_HEALTH_SLOW_REQUEST, HEALTH_WARN,
                              css->strv());
            health.metrics.push_back(m);
        }
    }

    {
        ...
        if (MDSIOContextBase::check_ios_in_flight(cutoff, count, oldest)) {
            ...
            auto oldest_secs =
                std::chrono::duration < double >(now - oldest).count();
            CachedStackStringStream css;
            *css << count << " slow metadata IOs are blocked > " <<
                complaint_time << " secs, oldest blocked for " << (int64_t)
                oldest_secs << " secs";

            MDSHealthMetric m(MDS_HEALTH_SLOW_METADATA_IO, HEALTH_WARN,
                              css->strv());
            health.metrics.push_back(m);
        }
    }
    ...
    if (mds->mdcache->cache_overfull()) {
        CachedStackStringStream css;
        *css << "MDS cache is too large (" << bytes2str(mds->mdcache->
                                                        cache_size())
            << "/" << bytes2str(mds->mdcache->
                                cache_limit_memory()) << "); " << mds->mdcache->
            num_inodes_with_caps << " inodes in use by clients, " << mds->
            mdcache->get_num_strays() << " stray files";
...
MDSMap::DaemonState Beacon::get_want_state() constconst
{
    std::unique_lock lock(mutex);
    return want_state;
}
