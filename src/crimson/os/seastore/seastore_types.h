...
// Identifies segment location on disk, see SegmentManager,
    struct segment_id_t {
      ...
      :    segment(_segment) {
            assert(device_id_to_paddr_type(device_id()) ==
        ...
        constexpr device_id_t device_id() const {
            return static_cast < device_id_t >
                (segment >> DEVICE_SEGMENT_ID_BITS);
        }[[gnu::always_inline]]
        ...
        static constexpr segment_id_t create_const(device_id_t id,
      ...
      private:
        ...
        constexpr static inline internal_segment_id_t make_internal(device_id_t
                                                                    d_id,
                                                                    device_segment_id_t
                                                                    s_id) {
            return static_cast < internal_segment_id_t > (s_id) | (static_cast <
...
// ondisk type of segment_id_t
    struct __attribute ((packed)) segment_id_le_t {
        ...
        segment_id_le_t(const segment_id_t id):segment(ceph_le32(id.segment)) {
        }
...
// segment_id_t() == MAX_SEG_ID == NULL_SEG_ID
    constexpr segment_id_t MAX_SEG_ID =
        segment_id_t::create_const(DEVICE_ID_MAX_VALID_SEGMENT,
 ...
 */
    template < typename T > class segment_map_t {
      public:
        ...
        void add_device(device_id_t device, std::size_t segments,
                        const T & init) {
        ...
        T & operator[](segment_id_t id) {
            assert(id.device_segment_id() <
                   device_to_segments[id.device_id()].size());
        ...
        template < bool is_const = false > class iterator {
            ...
            std::conditional_t <
                is_const, const segment_map_t &, segment_map_t & >parent;
            ...
            std::optional <
                std::pair <
                const segment_id_t,
                std::conditional_t < is_const, const T &, T & > >>current;
            ...
            bool is_end() const {
                ...
                if (is_end()) {
                    ...
                    current.emplace(segment_id_t {
                                    device_id, device_segment_id}
                                    ,
                                    parent.
                                    device_to_segments[device_id]
                                    [device_segment_id]
                        );
                }
            }

            iterator(decltype(parent) & parent,
                     device_id_t device_id,
                     device_segment_id_t device_segment_id)
          :    
          ...
          public:
            static iterator lower_bound(decltype(parent) & parent,
                                        device_id_t device_id,
                                        device_segment_id_t device_segment_id) {
                if (device_id == DEVICE_ID_MAX_VALID) {
                    ...
                    auto ret =
                        iterator { parent, device_id, device_segment_id };
                    ret.find_valid();
                    return ret;
                }
            }

            static iterator end_iterator(decltype(parent) & parent) {
                return iterator {
                parent, DEVICE_ID_MAX_VALID, 0};
            }
 ...
 */
    ...
    constexpr auto PADDR_BITS =
        std::numeric_limits < internal_paddr_t >::digits;
 ...
 */
    ...
    constexpr auto SEGMENT_OFF_MAX =
        std::numeric_limits < segment_off_t >::max();
    constexpr auto SEGMENT_OFF_MIN =
        std::numeric_limits < segment_off_t >::min();
    constexpr auto SEGMENT_OFF_BITS =
        std::numeric_limits < u_segment_off_t >::digits;
    ...
    struct paddr_t {
      public:
        ...
        } static paddr_t make_seg_paddr(segment_id_t seg, segment_off_t offset) {
            return paddr_t(seg, offset);
        }

        static paddr_t make_seg_paddr(device_id_t device,
        ...
        static paddr_t make_blk_paddr(device_id_t device, device_off_t offset) {
            assert(device_id_to_paddr_type(device) ==
        ...
        device_id_t get_device_id() const {
            return static_cast < device_id_t >
                (internal_paddr >> DEVICE_OFF_BITS);
   ...
   */
        ...
        /// Denotes special null addr bool is_null() const {
            return get_device_id() == DEVICE_ID_NULL;
        }
        /// Denotes special zero addr bool is_zero() const {
            return get_device_id() == DEVICE_ID_ZERO;
        }
        /// Denotes the root addr bool is_root() const {
   ...
   */ bool is_real() const {
        ...
        constexpr static paddr_t create_const(device_id_t d_id,
                                              device_off_t offset) {
      ...
      :    
        paddr_t((static_cast < internal_paddr_t >
                 (seg.segment) << SEGMENT_OFF_BITS) | static_cast <
                u_segment_off_t > (offset)) {
        }
      ...
      :    
        ...
        constexpr paddr_t(device_id_t d_id, device_off_t offset,
                          const_construct_t)
        :internal_paddr((static_cast < internal_paddr_t >
                         (d_id) << DEVICE_OFF_BITS) | static_cast <
                        u_device_off_t > (offset)) {
        }
...
#define PADDR_OPERATION(a_type, base, func)        \
  if (get_addr_type() == a_type) {                 \
    ...
    inline paddr_t paddr_t::add_offset(device_off_t o) const {
        ...
        paddr_le_t() = default;
        paddr_le_t(const paddr_t &
                   addr):internal_paddr(ceph_le64(addr.internal_paddr)) {
        }
...
// JOURNAL_SEQ_NULL == JOURNAL_SEQ_MAX == journal_seq_t{}
    struct journal_seq_t {
        ...
        device_off_t relative_to(journal_type_t type,
        ...
        bool operator==(const journal_seq_t & o) const {
            return cmp(o) == 0;
        } bool operator!=(const journal_seq_t & o)const {
            return cmp(o) != 0;
        } bool operator<(const journal_seq_t & o)const {
...
// INIT_GENERATION requires EPM decision to INLINE/OOL_GENERATION
                                                                     constexpr rewrite_gen_t INIT_GENERATION = 0; constexpr rewrite_gen_t INLINE_GENERATION = 1;    // to the journal
                                                                     constexpr
                                                                     rewrite_gen_t
                                                                     OOL_GENERATION
                                                                     = 2;
...
/* description of a mutation to a physical extent */
                                                                     ...
                                                                     uint32_t prev_crc = 0; uint32_t final_crc = 0; extent_len_t length = 0;    ///< extent length
                                                                     extent_version_t pversion; ///< prior version
                                                                     segment_seq_t ext_seq; ///< seq of the extent's segment
                                                                     segment_type_t seg_type; ceph::bufferlist bl;  ///< payload
                                                                     DENC
                                                                     (delta_info_t,
                                                                      v, p) {
                                                                     DENC_START
...
/* contains the latest journal tail information */
                                                                     ...
                                                                     object_data_t
                                                                     {
      laddr_t reserved_data_base = L_ADDR_NULL; extent_len_t reserved_data_len = 0; bool dirty = false; public:
      ...
      :                                                             addr(addr),
                                                                     ...
                                                                     omap_root_le_t
                                                                     {
      laddr_le_t addr = laddr_le_t(L_ADDR_NULL); depth_le_t depth = init_depth_le(0); public:
                                                                     omap_root_le_t() = default; omap_root_le_t(laddr_t addr, depth_t depth)
      :                                                             addr(addr),
                                                                     depth
                                                                     (init_depth_le
                                                                      (depth)) {
 ...
 */
                                                                     ...
                                                                     phy_tree_root_t
                                                                     {
      paddr_le_t root_addr; depth_le_t depth = init_extent_len_le(0); public:
                                                                     phy_tree_root_t() = default; phy_tree_root_t(paddr_t addr, depth_t depth)
      :                                                             root_addr
                                                                     (addr),
                                                                     depth
                                                                     (init_depth_le
                                                                     ...
                                                                     coll_root_t
                                                                     {
      laddr_t addr = L_ADDR_NULL; extent_len_t size = 0; bool mutated = false; public:
      coll_root_t() = default; coll_root_t(laddr_t addr, extent_len_t size):addr(addr),
                                                                     size(size)
                                                                     {
                                                                     }
 ...
 */
                                                                     ...
                                                                     coll_root_le_t
                                                                     {
      laddr_le_t addr; extent_len_le_t size = init_extent_len_le(0); public:
                                                                     coll_root_le_t() = default; coll_root_le_t(laddr_t laddr, extent_len_t size)
      :                                                             addr
                                                                     (laddr),
                                                                     size
                                                                     (init_extent_len_le
 ...
 */
                                                                     ...
                                                                     segment_header_t
                                                                     {
                                                                     segment_seq_t segment_seq; segment_id_t physical_segment_id;   // debugging
                                                                     journal_seq_t
                                                                     dirty_tail;
                                                                     journal_seq_t
                                                                     alloc_tail;
                                                                     ...
                                                                     DENC_FINISH
                                                                     (p);}
  }; std::ostream & operator<<(std::ostream & out, const segment_tail_t & tail); enum class transaction_type_t:uint8_t
                                                                     {
                                                                     MUTATE = 0,
                                                                     ...
                                                                     struct
                                                                     record_t {
      transaction_type_t type = TRANSACTION_TYPE_NULL; std::vector < extent_t > extents; std::vector < delta_info_t > deltas; record_size_t size; sea_time_point modify_time = NULL_TIME; record_t(transaction_type_t type):type
                                                                     {
                                                                     type} {
                                                                     }
                                                                     ...
                                                                     (std::
                                                                      ...
                                                                      &&_deltas)
                                                                     {
          auto modify_time = seastar::lowres_system_clock::now(); for (auto & e:_extents)
                                                                     {
                                                                     push_back
                                                                     (std::
                                                                      move(e),
          ...
          for (auto & d:                                            _deltas)
                                                                     ...
                                                                     record_header_t
                                                                     {
                                                                     transaction_type_t type; uint32_t deltas;  // number of deltas
                                                                     ...
                                                                     {
                                                                     uint32_t records; extent_len_t mdlength;   // block aligned, length of metadata
...
template <> struct fmt::formatter < crimson::os::seastore::data_category_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::delta_info_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::device_id_printer_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::extent_types_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::journal_seq_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::journal_tail_delta_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::laddr_list_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::omap_root_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::paddr_list_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::paddr_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::placement_hint_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::device_type_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::record_group_header_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::record_group_size_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::record_header_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::record_locator_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::record_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::rewrite_gen_printer_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::scan_valid_records_cursor >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::sea_time_point_printer_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::segment_header_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::segment_id_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::segment_seq_printer_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::segment_tail_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::segment_type_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::transaction_type_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < crimson::os::seastore::write_result_t >:fmt::
                                                                     ostream_formatter
                                                                     {
}; template <> struct fmt::formatter < ceph::buffer::list >:        fmt::
                                                                     ostream_formatter
                                                                     {
                                                                     };
#endif
