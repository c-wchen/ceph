...
struct C_HandleComplete: public Context {
    ...
    explicit C_HandleComplete(ReplayHandler *
                              r): replay_handler(std::move(r))
    {
...
struct C_HandleEntriesAvailable: public Context {
    ...
    explicit C_HandleEntriesAvailable(ReplayHandler *
                                      r): replay_handler(std::move(r))
    {
         ...
         m_cache_rebalance_handler(this)
{
    ...
    if (m_cache_manager_handler != nullptr) {
        m_cache_name =
            "JournalPlayer/" + stringify(m_ioctx.get_id()) + "/" +
            m_object_oid_prefix;
        ...
        m_cache_manager_handler->register_cache(m_cache_name, min_size,
                                                max_size,
...
void JournalPlayer::prefetch()
{
    ...
    for (auto &position : m_commit_positions) {
        ...
        splay_offset_to_objects[position.first] =
            position.second.object_number;
    }
    ...
    ldout(m_cct, 10) << __func__ << ": prefetching "
                     << prefetch_object_numbers.size() << " " << "objects" << dendl;
...
bool JournalPlayer::try_pop_front(Entry *entry, uint64_t *commit_tid)
{
    ...
    if (m_journal_metadata->
        get_last_allocated_entry_tid(entry->get_tag_tid(), &last_entry_tid)
    ...
    m_journal_metadata->reserve_entry_tid(entry->get_tag_tid(),
                                          entry->get_entry_tid());
    *commit_tid =
        m_journal_metadata->allocate_commit_tid(object_player->
            get_object_number(),
            entry->get_tag_tid(),
            entry->get_entry_tid());
    return true;
...
{
    ldout(m_cct,
          10) << __func__ << ": object_num=" << object_number << dendl;
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    if (m_fetch_object_numbers.count(object_player->get_object_number()) ==
        0) {
        ...
        if (m_commit_positions.count(splay_offset) != 0 &&
            ...
            ldout(m_cct,
                  15) << "seeking known commit position " << position <<
                      " in " << object_player->get_oid() << dendl;
            ...
            while (!object_player->empty()) {
                ...
                ldout(m_cct,
                      20) << "skipping committed entry: " << entry << dendl;
                m_journal_metadata->reserve_entry_tid(entry.get_tag_tid(),
                                                      entry.
                                                      get_entry_tid());
                object_player->pop_front();
...
bool JournalPlayer::is_object_set_ready() const
{
    ...
    if (m_watch_scheduled || !m_fetch_object_numbers.empty()) {
        ldout(m_cct,
              20) << __func__ << ": waiting for in-flight fetch" << dendl;
        return false;
    }
    return true;
}
...
bool JournalPlayer::verify_playback_ready()
{
    ...
    while (true) {
        if (!is_object_set_ready()) {
            ldout(m_cct,
                  10) << __func__ << ": waiting for full object set" <<
                      dendl;
            return false;
        }
        ...
        if (!object_player->empty()) {
            ...
            if (!m_active_tag_tid) {
                ...
                return true;
                       ...
                       (m_prune_tag_tid
                        && entry.get_tag_tid() <= *m_prune_tag_tid)) {
            ...
            if (!m_active_tag_tid) {
                ...
                ldout(m_cct,
                      10) << __func__ << ": waiting for first entry: " <<
                          "object_num=" << object_num << dendl;
                return false;
            } else if (m_prune_tag_tid
                       && *m_prune_tag_tid == *m_active_tag_tid) {
                ldout(m_cct,
                      10) << __func__ << ": no more entries" << dendl;
                return false;
                ...
                ldout(m_cct,
                      10) << __func__ << ": no more in-sequence entries: "
                          << "object_num=" << object_num << ", " << "active_tag="
                          << *m_active_tag_tid << dendl;
                prune_active_tag(boost::none);
                continue;
                ...
                ldout(m_cct,
                      10) << __func__ <<
                          ": no more entries -- watch required" << dendl;
                return false;
            }
        }
    }
...
void JournalPlayer::prune_tag(uint64_t tag_tid)
{
    ...
    for (const auto &player_pair : m_object_players) {
        ...
        ldout(m_cct,
              15) << __func__ << ": checking " << object_player->get_oid()
                  << dendl;
        while (!object_player->empty()) {
            Entry entry;
            object_player->front(&entry);
            if (entry.get_tag_tid() == tag_tid) {
                ldout(m_cct,
                      20) << __func__ << ": pruned " << entry << dendl;
                object_player->pop_front();
                pruned = true;
            } else {
                break;
    ...
    if (pruned) {
        ldout(m_cct,
              15) << __func__ << ": resetting refetch state to immediate" <<
                  dendl;
        for (const auto &player_pair : m_object_players) {
            auto &object_player = player_pair.second;
            object_player->
...
void JournalPlayer::prune_active_tag(const boost::optional < uint64_t >
                                     &tag_tid)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
bool JournalPlayer::remove_empty_object_player(const ceph::ref_t <
        ObjectPlayer > &player)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ceph_assert(!m_watch_scheduled);
    ...
    if (!player->empty() || object_set == active_set) {
        ...
        ldout(m_cct,
              20) << __func__ << ": " << player->
                  get_oid() << " requires " << "a refetch" << dendl;
        return false;
    } else if (m_active_set != active_set) {
        ldout(m_cct,
              20) << __func__ << ": new active set detected, all players "
...
void JournalPlayer::schedule_watch(bool immediate)
{
    ...
    switch (m_watch_step) {
        case WATCH_STEP_FETCH_CURRENT: {
            ...
            if (immediate
                ...
                || (object_set < active_set
                    && object_player->refetch_required())) {
                ldout(m_cct,
                      20) << __func__ << ": immediately refetching " <<
    ...
    Context *ctx =
        utils::create_async_context_callback(m_journal_metadata,
            new C_Watch(this,
                        object_player->
                        get_object_number
                        ()));
...
void JournalPlayer::notify_entries_available()
{
    ...
    m_journal_metadata->
    queue(new C_HandleEntriesAvailable(m_replay_handler), 0);
}
...
void JournalPlayer::handle_cache_rebalanced(uint64_t new_cache_bytes)
{
    ...
    auto splay_width = m_journal_metadata->get_splay_width();
    m_max_fetch_bytes =
        p2align < uint64_t > (new_cache_bytes / splay_width, 4096);
    ...
    if (m_state == STATE_WAITCACHE) {
        ...
        min_bytes =
            p2align < uint64_t > (min_bytes - (rand() % min_bytes) / 2,
                                  4096);
    }

    if (m_max_fetch_bytes < min_bytes) {
        lderr(m_cct) << __func__ <<
                     ": can't allocate enough memory from cache" << dendl;
        m_state = STATE_ERROR;
