...
void PGLog::IndexedLog::split_out_child(pg_t child_pgid,
                                        unsigned split_bits,
...
void PGLog::IndexedLog::trim(CephContext *cct,
                             eversion_t s,
    ...
    while (!log.empty()) {
        ...
        if (e.version.version >= earliest_dup_version) {
            if (write_from_dups != nullptr && *write_from_dups > e.version) {
                lgeneric_subdout(cct, osd,
                                 20) << "updating write_from_dups from " <<
                                     *write_from_dups << " to " << e.version << dendl;
...
ostream &PGLog::IndexedLog::print(ostream &out) const const
{
    out << *this << std::endl;
    for (auto p = log.begin(); p != log.end(); ++p) {
        out << *p << " " <<
            (logged_object(p->soid) ? "indexed" : "NOT INDEXED") << std::endl;
...
void PGLog::trim(eversion_t trim_to,
                 pg_info_t &info, bool transaction_applied, bool async)
{
    ...
    if (trim_to > log.tail) {
        ...
        if (log.complete_to != log.log.end())
            dout(10) << " after trim complete_to " << log.complete_to->
                     version << dendl;
    }
}
                             ...
                             pg_shard_t from) const const
{
    ...
    if (olog.head == log.head) {
        dout(10) << __func__ << ": osd." << from <<
                 " same log head, not looking " << "for divergent objects" << dendl;
        return;
    }
    ...
    eversion_t lu =
        ...
        limit : first_non_divergent->version;
    ...
    _merge_divergent_entries(folog,
                             divergent,
                             oinfo,
                             olog.get_can_rollback_to(), omissing, 0, this);
    ...
    if (omissing.have_missing()) {
        eversion_t first_missing =
            omissing.get_items().at(omissing.get_rmissing().begin()->second).
            need;
                                 ...
                                 bool &dirty_info, bool &dirty_big_info)
{
    ...
    _merge_divergent_entries(log,
                             divergent,
                             info, original_crt, missing, rollbacker, this);
                      ...
                      bool &dirty_info, bool &dirty_big_info)
{
    ...
    if (olog.head > log.head) {
        ...
        append_log_entries_update_missing(info.last_backfill,
                                          new_entries,
                                          false,
                                  ...
                                  bool require_rollback)
{
    if (needs_write()) {
        ...
        _write_log_and_missing(t, km, log, coll, log_oid,
                               dirty_to,
                               dirty_from,
                               writeout_from,
                               std::move(trimmed),
...
// static
void PGLog::write_log_and_missing_wo_missing(ObjectStore::Transaction &t,
        map < string, bufferlist > *km,
        pg_log_t &log,
        const coll_t &coll,
        const ghobject_t &log_oid,
...
{
    _write_log_and_missing_wo_missing(t, km, log, coll, log_oid,
                                      divergent_priors, eversion_t::max(),
                                      eversion_t(), eversion_t(), true, true,
                                      require_rollback, eversion_t::max(),
                                      eversion_t(), eversion_t(), nullptr, dpp);
}
...
// static
void PGLog::_write_log_and_missing_wo_missing(ObjectStore::Transaction &t,
        map < string, bufferlist > *km,
        pg_log_t &log,
        const coll_t &coll,
        const ghobject_t &log_oid,
...
{
    ldpp_dout(dpp,
              10) << "_write_log_and_missing_wo_missing, clearing up to " <<
                  dirty_to << " dirty_to_dups=" << dirty_to_dups << " dirty_from_dups=" <<
                  dirty_from_dups << " write_from_dups=" << write_from_dups << dendl;
    if (touch_log) {
    ...
    if (dirty_to_dups != eversion_t()) {
        ...
        ldpp_dout(dpp,
                  10) << __func__ << " remove dups min=" << min.get_key_name()
...
void PGLog::_write_log_and_missing(ObjectStore::Transaction &t, map < string, bufferlist > *km, pg_log_t &log,
                                   const coll_t &coll, const ghobject_t &log_oid, eversion_t dirty_to, eversion_t dirty_from, eversion_t writeout_from,
                                   set < eversion_t > &&trimmed, set < string > &&trimmed_dups, const pg_missing_tracker_t &missing, bool touch_log,
                                   bool require_rollback, bool clear_divergent_priors, eversion_t dirty_to_dups, eversion_t dirty_from_dups,
                                   eversion_t write_from_dups, bool *may_include_deletes_in_missing_dirty,    // in/out param
                                   set < string > *log_keys_debug,
                                   const DoutPrefixProvider *dpp)
{
        ...
        const pg_info_t &info)
{
    ...
    for (auto i = log.log.rbegin(); i != log.log.rend(); ++i) {
        ...
        if (i->soid > info.last_backfill ||
            i->is_error() || did.find(i->soid) != did.end()) {
            continue;
        }
        did.insert(i->soid);
        ...
        int r = store->getattr(ch,
                               ghobject_t(i->soid, ghobject_t::NO_GEN,
                                          info.pgid.shard),
                               OI_ATTR,
                               bv);
...
struct FuturizedShardStoreLogReader {
                            ...
                            ghobject_t pgmeta_oid)
    {
        ...
        return seastar::do_with(std::move(ch),
                                std::move(pgmeta_oid),
        ...
        &start) {
            return seastar::
            repeat([this, &ch, &pgmeta_oid, &start]() {
                return store.omap_get_values(ch,
                                             pgmeta_oid,
                                             start).
                       safe_then([this,
                &start](const auto & ret) {
                ...
                log =
                PGLog::IndexedLog(info.last_update,
                                  info.log_tail,
...
seastar::future <> PGLog::read_log_and_missing_crimson(crimson::os::
        FuturizedStore::
        Shard &store,
        crimson::os::
        CollectionRef ch,
        ...
        * dpp)
{
    ...
    ,
    [ch,
     pgmeta_oid](FuturizedShardStoreLogReader & reader) {
        return reader.read(ch, pgmeta_oid);
    }
                           );
}

seastar::future <> PGLog::rebuild_missing_set_with_deletes_crimson(crimson::os::
        FuturizedStore::
        Shard &
        store,
        crimson::os::
        ...
        info)
{
    ...
    for (const auto &p : missing.get_items()) {
        if (!log.logged_object(p.first)) {
            ldpp_dout(this,
                      20) << __func__ << " extra missing entry: " << p.
    ...
    // versions on disk, just as if we were reading the log + metadata
    // off disk originally
    return seastar::do_with(set < hobject_t > (),
                            log.log.rbegin(),
    [this, &store, ch, &info](auto & did, auto & it) {
        return seastar::
        repeat([this, &store, ch, &info, &it, &did] {
            if (it == log.log.rend()) {
                return seastar::make_ready_future <
                seastar::stop_iteration >
                (seastar::stop_iteration::yes);
            }
            auto &log_entry = *it; it++;
            ...
            return store.get_attr(ch,
                                  ghobject_t(log_entry.
                                             soid,
                                             ghobject_t::
                                             NO_GEN,
                                             info.pgid.
                        ...
                        safe_then([this, &log_entry](auto bv)
            {
                ...
                if (oi.version <
                    ...
                    ldpp_dout(this, 20)
                            ...
                            << log_entry.
                            soid << " for version = " <<
                            log_entry.version << dendl;
                    missing.add(log_entry.soid,
                                log_entry.version,
                                oi.version,
                                log_entry.
                                is_delete());
