...
struct OSDMetricPayload {
    static const MetricReportType METRIC_REPORT_TYPE =
     ...
     OSDMetricPayload() {
    } OSDMetricPayload(const std::map < OSDPerfMetricQuery,
                       OSDPerfMetricReport > &report)
    :report(report) {
    }
...
typedef boost::variant < OSDMetricPayload,
    MDSMetricPayload, UnknownMetricPayload > MetricPayload;
...
class DecodeMetricPayloadVisitor:public boost::static_visitor < void > {
  public:
    DecodeMetricPayloadVisitor(ceph::buffer::list::
                               const_iterator & iter):m_iter(iter) {
    } template < typename MetricPayload >
...
struct OSDConfigPayload {
    ...
    OSDConfigPayload() {
    } OSDConfigPayload(const std::map < OSDPerfMetricQuery,
                       OSDPerfMetricLimits > &config)
    :config(config) {
    }
...
struct MDSConfigPayload {
    ...
    MDSConfigPayload() {
    } MDSConfigPayload(const std::map < MDSPerfMetricQuery,
                       MDSPerfMetricLimits > &config)
    :config(config) {
    }
...
typedef boost::variant < OSDConfigPayload,
    MDSConfigPayload, UnknownConfigPayload > ConfigPayload;
...
class DecodeConfigPayloadVisitor:public boost::static_visitor < void > {
  public:
    DecodeConfigPayloadVisitor(ceph::buffer::list::
                               const_iterator & iter):m_iter(iter) {
    } template < typename ConfigPayload >
