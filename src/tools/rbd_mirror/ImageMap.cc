...
struct ImageMap <I >::C_NotifyInstance: public Context {
    ...
    C_NotifyInstance(ImageMap *image_map,
...
template < typename I >
ImageMap < I >::ImageMap(librados::IoCtx &ioctx,
                         Threads < I > *threads,
    ...
    : m_ioctx(ioctx), m_threads(threads), m_instance_id(instance_id),
      ...
      m_lock(ceph::
             make_mutex(unique_lock_name
                        ("rbd::mirror::ImageMap::m_lock", this)))
{
}
...
template < typename I >
void ImageMap < I >::continue_action(const std::set < std::string >
                                     &global_image_ids, int r)
{
    dout(20) << dendl;
        ...
        &&map_removals)
{
    ...
    image_map::UpdateRequest < I > *req =
        image_map::UpdateRequest < I >::create(m_ioctx,
            std::
            move(update_mapping),
...
template < typename I > void ImageMap < I >::process_updates()
{
    ...
    for (auto const &global_image_id : m_global_image_ids) {
        ...
        switch (action_type) {
            ...
            case image_map::ACTION_TYPE_MAP_UPDATE:
                ceph_assert(info.instance_id !=
                            image_map::UNMAPPED_INSTANCE_ID);
            ...
            case image_map::ACTION_TYPE_ACQUIRE:
                ceph_assert(info.instance_id !=
                            image_map::UNMAPPED_INSTANCE_ID);
                acquire_updates.emplace_back(global_image_id,
                                             info.instance_id);
                break;
            case image_map::ACTION_TYPE_RELEASE:
                ceph_assert(info.instance_id !=
                            image_map::UNMAPPED_INSTANCE_ID);
                release_updates.emplace_back(global_image_id,
                                             info.instance_id);
                break;
        }
    }
    ...
    // timer lock.
    notify_listener_acquire_release_images(acquire_updates,
                                           release_updates);
    update_image_mapping(std::move(map_updates),
                         std::move(map_removals));
}

template < typename I > void ImageMap < I >::schedule_update_task()
{
    std::lock_guard timer_lock {
...
template < typename I >
void ImageMap <
I >::schedule_update_task(const ceph::mutex &timer_lock)
{
    ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
    ...
    double after =
        cct->_conf.get_val <
        double > ("rbd_mirror_image_policy_update_throttle_interval");
...
template < typename I > void ImageMap < I >::schedule_rebalance_task()
{
    ...
    double resched_after =
        cct->_conf.get_val <
        double > ("rbd_mirror_image_policy_rebalance_timeout");
    if (!resched_after) {
        return;
    }
...
template < typename I >
void ImageMap <
I >::schedule_action(const std::string &global_image_id)
{
...
template < typename I >
void ImageMap <
I >::notify_listener_acquire_release_images(const Updates &acquire,
...
template < typename I >
void ImageMap <
I >::notify_listener_remove_images(const std::string &peer_uuid,
                                   const Updates &remove)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", " << "remove=[" << remove
            << "]" << dendl;

    for (auto const &update : remove) {
        m_listener.remove_image(peer_uuid, update.global_image_id,
                                update.instance_id,
                                create_async_context_callback
                                (m_threads->work_queue,
                                 new C_NotifyInstance(this,
...
template < typename I >
void ImageMap <
I >::handle_peer_ack_remove(const std::string &global_image_id,
                            int r)
{
    std::lock_guard locker {
        m_lock};
    ...
    if (r < 0) {
        derr << "failed to remove global_image_id=" << global_image_id
             << dendl;
    }
...
template < typename I >
void ImageMap <
I >::update_images_added(const std::string &peer_uuid,
                         const std::set < std::string >
                         &global_image_ids)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", " << "global_image_ids=["
...
template < typename I >
void ImageMap <
I >::update_images_removed(const std::string &peer_uuid,
                           const std::set < std::string >
                           &global_image_ids)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", " << "global_image_ids=["
    ...
    for (auto const &global_image_id : global_image_ids) {
        image_map::LookupInfo info = m_policy->lookup(global_image_id);
        bool image_mapped =
            (info.instance_id != image_map::UNMAPPED_INSTANCE_ID);
...
template < typename I >
void ImageMap < I >::update_instances_added(const std::vector <
        std::string >
        &instance_ids)
{
    {
        ...
        filter_instance_ids(instance_ids, &filtered_instance_ids,
                            false);
        ...
        std::set < std::string > remap_global_image_ids;
        m_policy->add_instances(filtered_instance_ids,
                                &remap_global_image_ids);
...
template < typename I >
void ImageMap < I >::update_instances_removed(const std::vector <
        std::string >
        &instance_ids)
{
    {
        ...
        std::set < std::string > remap_global_image_ids;
        m_policy->remove_instances(filtered_instance_ids,
                                   &remap_global_image_ids);
                                   ...
                                   &&removed_global_image_ids)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", " << "added_count=" <<
            added_global_image_ids.
            size() << ", " << "removed_count=" << removed_global_image_ids.
            size() << dendl;
...
template < typename I >
void ImageMap <
I >::handle_peer_ack(const std::string &global_image_id, int r)
{
...
template < typename I > void ImageMap < I >::init(Context *on_finish)
{
    ...
    image_map::LoadRequest < I > *req =
        image_map::LoadRequest < I >::create(m_ioctx,
            &ctx->image_mapping, ctx);
    req->send();
}
...
template < typename I >
void ImageMap < I >::filter_instance_ids(const std::vector <
        std::string >
        &instance_ids,
