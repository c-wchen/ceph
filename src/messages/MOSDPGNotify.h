...
class MOSDPGNotify:public Message {
  ...
  public:
    ...
    :Message(MSG_OSD_PG_NOTIFY, HEAD_VERSION, COMPAT_VERSION), epoch(e) {
        pg_list.swap(l);
        set_priority(CEPH_MSG_PRIO_HIGH);
    }
  private:
  ...
  public:
    const char *get_type_name() const override {
        ...
        if (HAVE_FEATURE(features, SERVER_LUMINOUS)) {
            ...
            for (vector < pair < pg_notify_t, PastIntervals > >::iterator p =
                 pg_list.begin(); p != pg_list.end(); p++)
                ::encode(pair < epoch_t,
                         epoch_t > (p->first.epoch_sent, p->first.query_epoch),
                         payload);
