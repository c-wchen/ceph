...
public:
    ...
    hobject_t(const object_t &oid, const std::string &key, snapid_t snap,
        ...
        pool(pool), nspace(nspace), key(oid.name == key ? std::string() : key)
    {
        build_hash_cache();
    }
 ...
 */
static std::set < std::string > get_prefixes(uint32_t bits,
...
struct ghobject_t {
    ...
    ghobject_t(shard_id_t shard, int64_t pool, uint32_t reversed_hash,
        ...
        : shard_id(shard),
          hobj(oid, snap, reversed_hash, pool, nspace), generation(gen)
    {
    }

    static ghobject_t make_pgmeta(int64_t pool, uint32_t hash, shard_id_t shard)
    {
        hobject_t h(object_t(), std::string(), CEPH_NOSNAP, hash, pool,
                    std::string());
