...
namespace crimson {
    ...
    // refs and return true if the first precedes the second
     template < typename T, typename I, typename C > class IntruHeap {
        ...
        static_assert(std::is_same < IntruHeapData &,
                      typename std::result_of < I(T &) >::type >::value,
      ...
      public:
         ...
         IntruHeap():count(0) {
            // empty
        } IntruHeap(const IntruHeap < T, I, C > &other):count(other.count) {
      ...
      protected:
        ...
        static inline index_t lhs(index_t i) {
            return 2 * i + 1;
        }

        static inline index_t rhs(index_t i) {
            return 2 * i + 2;
        }
