...
namespace journal {
    ...
    namespace {
        ...
        bool advance_to_last_pad_byte(uint32_t off,
  ...
  :    
    ...
    m_lock(ceph::
           make_mutex(utils::unique_lock_name("ObjectPlayer::m_lock", this))) {
        m_ioctx.dup(ioctx);
    ...
    void ObjectPlayer::fetch(Context * on_finish) {
        ...
        auto rados_completion =
            librados::Rados::aio_create_completion(context,
        ...
        while (!iter.end()) {
            ...
            if (!Entry::is_readable(iter, &bytes_needed)) {
                if (bytes_needed != 0) {
                    ...
                    partial_entry = true;
                    if (full_fetch) {
                        lderr(m_cct) << ": partial record at offset " <<
                ...
                if (!advance_to_last_pad_byte
                    (m_read_bl_off + iter.get_off(), &iter, &pad_len,
                     &partial_entry)) {
                    invalid_start_off = m_read_bl_off + bl_off;
                    invalid = true;
                    if (partial_entry) {
                        if (full_fetch) {
                            lderr(m_cct) << ": partial pad at offset " <<
                                invalid_start_off << dendl;
                        }
                        else {
                            ldout(m_cct,
                                  20) << ": partial pad detected, will re-fetch"
                                << dendl;
                        }
                    }
                    else {
            ...
            if (m_entry_keys.find(entry_key) == m_entry_keys.end()) {
                m_entry_keys[entry_key] =
                    m_entries.insert(m_entries.end(), entry);
            }
            else {
                ldout(m_cct,
                      10) << ": " << entry << " is duplicate, replacing" <<
                    dendl;
    ...
    void ObjectPlayer::schedule_watch() {
        ...
        ldout(m_cct,
              20) << __func__ << ": " << m_oid << " scheduling watch" << dendl;
        ceph_assert(m_watch_task == nullptr);
        m_watch_task = m_timer.add_event_after(m_watch_interval,
    ...
    bool ObjectPlayer::cancel_watch() {
        ceph_assert(ceph_mutex_is_locked(m_timer_lock));
        ldout(m_cct,
              20) << __func__ << ": " << m_oid << " cancelling watch" << dendl;
    ...
    void ObjectPlayer::handle_watch_fetched(int r) {
        ldout(m_cct,
              10) << __func__ << ": " << m_oid << " poll complete, r=" << r <<
            dendl;
