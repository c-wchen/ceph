    ...
    class PerfCountersBuilder {
      ...
      private:
        ...
        void add_impl(int idx, const char *name,
                      ...
                      int ty, int unit =
                      UNIT_NONE, std::unique_ptr < PerfHistogram <>> histogram =
                      nullptr);
 ...
 */
  ...
  /** Represents a PerfCounters data element. */
        struct perf_counter_data_any_d {
            ...
            :name(NULL),
                description(NULL),
                nick(NULL), type(PERFCOUNTER_NONE), unit(UNIT_NONE) {
        ...
        void dump_formatted_generic(ceph::Formatter * f, bool schema,
                                    bool histograms, bool dump_labeled,
                                    const std::string & counter = "") const;
  ...
  /** Protects m_data */
    ...
    typedef std::set < PerfCounters *,
        SortPerfCountersByName > perf_counters_set_t;
    ...
    class PerfCountersCollectionImpl {
      public:
        ...
        void dump_formatted(ceph::Formatter * f, bool schema, bool dump_labeled,
                            ...
                            const std::string & counter = "") const {
            dump_formatted_generic(f, schema, false, dump_labeled, logger,
                                   counter);
        } void dump_formatted_histograms(ceph::Formatter * f, bool schema,
                                         const std::string & logger =
                                         "", const std::string & counter =
        ...
        // A reference to a perf_counter_data_any_d, with an accompanying// pointer to the enclosing PerfCounters, in order that the consumer// can see the prio_adjust class PerfCounterRef
        {
          public:
