...
template < class ThrottleType > struct Policy {
...
#else
    static constexpr uint64_t features_supported {
        CEPH_FEATURES_SUPPORTED_DEFAULT
    };
#endif
    ...
    Policy()
        : lossy(false), server(false), standby(false), resetcheck(true),
          ...
          throttler_messages(NULL), features_required(0)
    {
    } private:
    Policy(bool l, bool s, bool st, bool r, bool rlc, uint64_t req)
        : lossy(l), server(s), standby(st), resetcheck(r),
          ...
          throttler_bytes(NULL),
          throttler_messages(NULL), features_required(req)
    {
    } public:
...
public:
    ...
    {
        if (auto found = policy_map.find(peer_type);
            found != policy_map.end()) {
            return found->second;
        } else {
