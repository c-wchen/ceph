...
class NVMEDevice:public BlockDevice {
   ...
   */
    ...
    struct BufferedExtents {
            ...
            while (it != buffered_extents.end()) {
                ...
                if (it->first <= off) {
                    if (extent_it_end > end) {
                        ...
                        extent_it_end - end,
                                it->second.x_off + it->second.x_len + len,
                                it->second.data, it->second.data_len};
                    }
                    else {
                        //         <-     data    ->
                        // <-     it    ->
                    ...
                    if (extent_it_end > end) {
                        ...
                        buffered_extents[end] = Extent {
                        it->second.x_len - overlap,
                                it->second.x_off + overlap, it->second.data,
                                it->second.data_len};
                    }
                    else {
                        //  <-     data    ->
        ...
        void memcpy_check(char *dst, uint64_t dst_raw_len, uint64_t dst_off,
                          map < Offset, Extent >::iterator & it,
                          uint64_t src_off, uint64_t copylen) {
            if (0) {
                assert(dst_off + copylen <= dst_raw_len);
                assert(it->second.x_off + src_off + copylen <=
                       it->second.data_len);
            }
            memcpy(dst + dst_off, it->second.data + it->second.x_off + src_off,
                   copylen);
        }
            ...
            while (it != buffered_extents.end()) {
                ...
                if (it->first >= off) {
                    if (extent_it_end > end) {
                        ...
                        memcpy_check(buf, len, it->first - off, it, 0,
                                     copy_len);
                    }
                    else {
                        //  <-     data    ->
                    ...
                    if (extent_it_end > end) {
                        ...
                        // <-           it          ->
                        copy_len = len;
                        memcpy_check(buf, len, 0, it, off - it->first,
                                     copy_len);
                    }
                    else {
                        //         <-     data    ->
  ...
  public:
    ...
    int aio_read(uint64_t off,
                 uint64_t len, bufferlist * pbl, IOContext * ioc) override;
    ...
    void close() override;
    int collect_metadata(string prefix, map < string,
                         string > *pm) const override;
};
