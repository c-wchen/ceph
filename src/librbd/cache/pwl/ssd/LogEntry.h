...
public:
    WriteLogEntry(std::shared_ptr < SyncPointLogEntry >
        ...
        : pwl::WriteLogEntry(sync_point_entry, image_offset_bytes,
                             write_bytes)
    {
    } WriteLogEntry(uint64_t image_offset_bytes,
                    uint64_t write_bytes)
    ...
    void writeback_bl(librbd::cache::
                      ImageWritebackInterface &image_writeback,
                      Context *ctx, ceph::bufferlist
                      && bl) override;
    void init_cache_bl(bufferlist &src_bl, uint64_t off,
...
{
public:
    WriteSameLogEntry(std::shared_ptr < SyncPointLogEntry >
                      sync_point_entry,
                      uint64_t image_offset_bytes,
                      uint64_t write_bytes,
        ...
        : WriteLogEntry(sync_point_entry, image_offset_bytes,
                        write_bytes, data_length)
    {
    } WriteSameLogEntry(uint64_t image_offset_bytes,
                        uint64_t write_bytes,
                        uint32_t data_length)
    ...
    WriteSameLogEntry &operator=(const WriteSameLogEntry &) =
        delete;
    void writeback_bl(librbd::cache::
                      ImageWritebackInterface &image_writeback,
                      Context *ctx, ceph::bufferlist
