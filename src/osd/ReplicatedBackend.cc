...
ReplicatedBackend::ReplicatedBackend(PGBackend::Listener * pg,
                                     ...
                                     ObjectStore * store,
                                     CephContext * cct):PGBackend(cct, pg,
                                                                  store, coll,
                                                                  c)
{
}
                          ...
                          ceph_release_t::unknown)
{
  ...
  for (auto && le:log_entries) {
    ...
    pgt->safe_create_traverse([&]
          ...
          map < string, bufferlist, less <>> attrs; for (auto && p:op.attr_updates)
                              {
                              if (p.second)
...
t->omap_setkeys(coll, goid, up.second); break; case UpdateType::RemoveRange:
                              t->omap_rmkeyrange(coll, goid, up.second); break;}
                              }
                                           ...
                                           OpRequestRef orig_op)
{
    ...
    generate_transaction(t,
                         ...
                         log_entries,
                         &op_t,
                         &added, &removed, get_osdmap()->require_osd_release);
    ceph_assert(added.size() <= 1);
    ceph_assert(removed.size() <= 1);

    auto insert_res = in_progress_ops.insert(make_pair(tid,
                                                       ceph::make_ref <
                                                       InProgressOp > (tid,
                                                                       on_all_commit,
                                                                       orig_op,
    ...
    op.waiting_for_commit.insert(parent->get_acting_recovery_backfill_shards().
                                 begin(),
                                 parent->get_acting_recovery_backfill_shards().
                                 end());
    ...
    parent->log_operation(std::move(log_entries),
                          hset_history,
                          trim_to,
                          at_version, min_last_complete_ondisk, true, op_t);
                                     ...
                                     ScrubMap::object & o)
{
    ...
    if (pos.omap_pos.empty()) {
        ...
        if (r == 0 && hdrbl.length()) {
            bool encoded = false;
            dout(25) << "CRC header " << cleanbin(hdrbl, encoded,
                                                  true) << dendl;
            pos.omap_hash << hdrbl;
        }
    }
    ...
    // omap
    ObjectMap::ObjectMapIterator iter = store->get_omap_iterator(ch,
                                                                 ghobject_t
                                                                 (poid,
                                                                  ghobject_t::
                                                                  NO_GEN,
                                 ...
                                 ObjectStore::Transaction & op_t)
{
    if (parent->get_acting_recovery_backfill_shards().size() > 1) {
      ...
      for (const auto & shard:get_parent()->
             get_acting_recovery_backfill_shards())
        {
            if (shard == parent->whoami_shard())
                continue;
            const pg_info_t & pinfo =
                parent->get_shard_info().find(shard)->second;
            ...
            get_parent()->send_message_osd_cluster(shard.osd, wr,
                                                   get_osdmap_epoch());
        }
    }
}
...
void ReplicatedBackend::do_repop(OpRequestRef op)
{
    ...
    if (pmissing.is_missing(soid)) {
        async = true;
        dout(30) << __func__ << " is_missing " << pmissing.
            is_missing(soid) << dendl;
      for (auto && e:log) {
    ...
    parent->log_operation(std::move(log), m->updated_hit_set_history, m->pg_trim_to, m->version,    /* Replicated PGs don't have rollback info */
                          m->min_last_complete_ondisk,
                          update_snaps, rm->localt, async);
                                          ...
                                          ObcLockManager & manager)
{
    ...
    if (cloning.num_intervals() > g_conf().get_val < uint64_t >
                                     ...
                                     ObjectContextRef headctx, RPGHandle * h)
{
    ...
    if (soid.is_snap()) {
        ceph_assert(!get_parent()->get_local_missing().
                    is_missing(soid.get_head()));
        ceph_assert(headctx);
        // check snapset
        ...
        assert(HAVE_FEATURE(parent->min_peer_features(), SERVER_OCTOPUS));
        recovery_info.copy_subset.intersection_of(missing_iter->second.
                                                  clean_regions.
                                                  get_dirty_regions());
...
submit_push_data(const ObjectRecoveryInfo & recovery_info, bool first,
                 bool complete, bool clear_omap, bool cache_dont_need,
    ...
    if (first) {
        ...
        if (get_parent()->pg_is_remote_backfilling()) {
            ...
            if (size != recovery_info.oi.size) {
                get_parent()->pg_add_local_num_bytes((int64_t) recovery_info.oi.
                                                     size - (int64_t) size);
                get_parent()->pg_add_num_bytes((int64_t) recovery_info.oi.size -
                                               (int64_t) size);
                dout(10) << __func__ << " " << recovery_info.
                    soid << " backfill size " << recovery_info.oi.
                    size << " previous size " << size << " net size " <<
                    recovery_info.oi.size - size << dendl;
            }
        }
        if (!complete) {
            ...
            if (recovery_info.object_exist) {
                ...
                for (interval_set < uint64_t >::const_iterator q =
                    ...
                    dout(15) << " clone_range " << recovery_info.
                        soid << " " << q.get_start() << "~" << q.
                        get_len() << dendl;
                    t->clone_range(coll, ghobject_t(recovery_info.soid),
                                   ghobject_t(target_oid), q.get_start(),
                                             ...
                                             ObjectStore::Transaction * t)
{
    ...
    uint64_t z_length =
        pop.after_progress.data_recovered_to -
                                    ...
                                    bool is_repair)
{
    ...
    submit_push_data(pop.recovery_info, first, complete, clear_omap, true,  // must be replicate
                     data_zeros,
                     pop.data_included,
                     data, pop.omap_header, pop.attrset, pop.omap_entries, t);
                                     ...
                                     bool cache_dont_need)
{
    ...
    if (progress.first) {
        int r =
            store->omap_get_header(ch, ghobject_t(recovery_info.soid),
                                   &out_op->omap_header);
        if (r < 0) {
    ...
    if (cct->_conf->osd_debug_random_push_read_error
        && (rand() %
            (int)(cct->_conf->osd_debug_random_push_read_error * 100.0)) == 0) {
        ...
        if (!pi->recovery_progress.data_complete && !error) {
            ...
            if (pushing[soid].empty()) {
                if (!error)
                    ...
                    get_parent()->on_failed_pull(std::set < pg_shard_t > {
                                                 get_parent()->whoami_shard()}
                                                 , soid, v);
                pushing.erase(soid);
            }
...
void ReplicatedBackend::_failed_pull(pg_shard_t from, const hobject_t & soid)
{
    ...
    get_parent()->on_failed_pull( {
                                 from}, soid, it->second.recovery_info.version);
