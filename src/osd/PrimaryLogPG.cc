...
struct PrimaryLogPG::C_OSD_OnApplied:Context {
    ...
    epoch_t epoch;
    eversion_t v;
     C_OSD_OnApplied(PrimaryLogPGRef pg, epoch_t epoch, eversion_t v)
...
class PrimaryLogPG::C_OSD_OndiskWriteUnlock:public Context {
  ...
  public:
  C_OSD_OndiskWriteUnlock(ObjectContextRef o, ObjectContextRef o2 = ObjectContextRef(), ObjectContextRef o3 = ObjectContextRef()):obc(o), obc2(o2),
        obc3(o3)
    {
    } void finish(int r) override {
                                    ...
                                    ObjectStore::Transaction * t)
{
    ...
    if (!is_delete && recovery_info.soid.is_snap()) {
        ...
        if (recovery_info.ss.is_legacy() ||
            ...
            if (p != recovery_info.ss.clone_snaps.end()) {
                ...
                derr << __func__ << " " << hoid << " had no clone_snaps" <<
                    dendl;
                error = true;
            }
        }
    ...
    if (!is_delete && pg_log.get_missing().is_missing(recovery_info.soid) &&
        pg_log.get_missing().get_items().find(recovery_info.soid)->second.need >
        recovery_info.version) {
        assert(is_primary());
        const pg_log_entry_t *latest =
            pg_log.get_log().objects.find(recovery_info.soid)->second;
...
void PrimaryLogPG::on_peer_recover(pg_shard_t peer,
...
void PrimaryLogPG::begin_peer_recover(pg_shard_t peer, const hobject_t soid)
{
...
void PrimaryLogPG::send_message_osd_cluster(int peer, Message * m,
                                            epoch_t from_epoch)
{
    osd->send_message_osd_cluster(peer, m, from_epoch);
}

void PrimaryLogPG::send_message_osd_cluster(Message * m, Connection * con)
{
    osd->send_message_osd_cluster(m, con);
}

void PrimaryLogPG::send_message_osd_cluster(Message * m,
                                            const ConnectionRef & con)
{
    osd->send_message_osd_cluster(m, con);
}

void PrimaryLogPG::on_primary_error(const hobject_t & oid, eversion_t v)
{
...
void PrimaryLogPG::backfill_add_missing(const hobject_t & oid, eversion_t v)
{
    dout(0) << __func__ << ": oid " << oid << " version " << v << dendl;
...
bool PrimaryLogPG::maybe_await_blocked_snapset(const hobject_t & hoid,
                                               OpRequestRef op)
{
    ObjectContextRef obc;
                              ...
                              bufferlist & xattr_data, bufferlist & outdata)
{
    ...
    for (vi = bt.ancestors.begin(); vi != bt.ancestors.end(); ++vi) {
        generic_dout(0) << "vi->dirino=" << vi->
            dirino << " parent_ino=" << parent_ino << dendl;
...
int PrimaryLogPG::get_pgls_filter(bufferlist::iterator & iter,
                                  PGLSFilter ** pfilter)
{
    string type;
    PGLSFilter *filter;
...
int PrimaryLogPG::do_command(cmdmap_t cmdmap,
                             ostream & ss,
                             bufferlist & idata,
                             bufferlist & odata,
                             ConnectionRef con, ceph_tid_t tid)
{
    ...
    boost::scoped_ptr < Formatter >
        f(Formatter::create(format, "json-pretty", "json"));
    ...
    if (command == "query") {
        ...
        for (vector < int >::iterator p = acting.begin(); p != acting.end();
             ++p)
            f->dump_unsigned("osd", *p);
        f->close_section();
    ...
    for (vector < OSDOp >::iterator p = ops.begin(); p != ops.end(); ++p) {
        ...
        case CEPH_OSD_OP_PGNLS:
            ...
            if (get_osdmap()->raw_pg_to_pg(m->get_pg()) != info.pgid.pgid) {
                ...
                unsigned list_size =
                    MIN(cct->_conf->osd_max_pgls, p->op.pgls.count);
...
void PrimaryLogPG::calc_trim_to()
{
    ...
    if (limit != eversion_t() &&
        limit != pg_trim_to && pg_log.get_log().approx_size() > target) {
        ...
        list < pg_log_entry_t >::const_iterator it =
            pg_log.get_log().log.begin();
...
void PrimaryLogPG::calc_trim_to_aggressive()
{
    ...
    eversion_t limit = std::min(pg_log.get_head(),
                                pg_log.get_can_rollback_to());
...
PrimaryLogPG::PrimaryLogPG(OSDService * o, OSDMapRef curmap, const PGPool & _pool, spg_t p):
PG(o, curmap, _pool, p),
pgbackend(PGBackend::
          build_pg_backend(_pool.info, curmap, this, coll_t(p), ch, o->store,
                           cct)), object_contexts(o->cct,
                                                  o->cct->_conf->
...
{
    missing_loc.set_backend_predicates(pgbackend->get_is_readable_predicate(),
                                       pgbackend->
...
void PrimaryLogPG::do_request(OpRequestRef & op, ThreadPool::TPHandle & handle)
{
    ...
    case CEPH_MSG_OSD_BACKOFF:
        if (!is_active()) {
            dout(20) << " peered, not active, waiting for active on " << op <<
                dendl;
...
hobject_t PrimaryLogPG::earliest_backfill() const const
{
...
void PrimaryLogPG::do_op(OpRequestRef & op)
{
    ...
    if (op->may_write() || op->may_cache()) {
        ...
        if (got) {
            ...
            if (already_complete(version)) {
                ...
                waiting_for_ondisk[version].
                    push_back(make_pair(op, user_version));
    ...
    if (!obc->obs.exists)
        ctx->snapset_obc =
            get_object_context(obc->obs.oi.soid.get_snapdir(), false);
                                      ...
                                      MOSDOpReply * orig_reply, int r)
{
    ...
    const osd_reqid_t & reqid =
        static_cast < const MOSDOp * >(op->get_req())->get_reqid();
...
void PrimaryLogPG::do_proxy_write(OpRequestRef op,
                                  const hobject_t & missing_oid,
                                  ObjectContextRef obc)
{
...
void PrimaryLogPG::execute_ctx(OpContext * ctx)
{
    ...
    bool successful_write = !ctx->op_t->empty() && op->may_write()
        && result >= 0;
    // prepare the reply
    ...
    ctx->register_on_commit([m, ctx, this] () {
                            if (ctx->op)
                            log_op_stats(ctx); if (m && !ctx->sent_reply) {
                            MOSDOpReply * reply = ctx->reply; if (reply)
                            ctx->reply = nullptr;
                            else {
                            reply =
...
{
    const MOSDSubOpReply *r =
        static_cast < const MOSDSubOpReply * >(op->get_req());
        ...
        case CEPH_OSD_OP_SCRUB_RESERVE:
            {
                ...
                bufferlist::iterator p =
                    const_cast < bufferlist & >(r->get_data()).begin();
                bool reserved;
                ::decode(reserved, p);
                if (reserved) {
    ...
    case MOSDPGScan::OP_SCAN_GET_DIGEST:
        {
            ...
            if (osd->check_backfill_full(ss)) {
                dout(1) << __func__ << ": Canceling backfill, " << ss.
                    str() << dendl;
                queue_peering_event(CephPeeringEvtRef
    ...
    case MOSDPGScan::OP_SCAN_DIGEST:
        {
            ...
            bi.end = m->end;
            bufferlist::iterator p =
                const_cast < bufferlist & >(m->get_data()).begin();
            ...
            if (waiting_on_backfill.erase(from)) {
                if (waiting_on_backfill.empty()) {
                    assert(peer_backfill_info.size() ==
                           backfill_targets.size());
...
{
    const MOSDPGBackfill *m =
        static_cast < const MOSDPGBackfill * >(op->get_req());
    ...
    case MOSDPGBackfill::OP_BACKFILL_FINISH:
        {
            ...
            queue_peering_event(CephPeeringEvtRef
                                (std::make_shared < CephPeeringEvt >
                                 (get_osdmap()->get_epoch(),
                                  get_osdmap()->get_epoch(), RecoveryDone())));
        }
        // fall-thru
                              ...
                              PrimaryLogPG::OpContextUPtr * ctxp)
{
    *ctxp = NULL;
    ...
    hobject_t snapoid(coid.oid, coid.get_key(),
                      obc->ssc->snapset.
                      head_exists ? CEPH_NOSNAP : CEPH_SNAPDIR, coid.get_hash(),
    ...
    if (!ctx->lock_manager.get_snaptrimmer_write(snapoid, snapset_obc, first)) {
        close_op_ctx(ctx.release());
        dout(10) << __func__ << ": Unable to get a wlock on " << snapoid <<
            dendl;
        return -ENOLCK;
    }
...
int PrimaryLogPG::do_tmapup_slow(OpContext * ctx, bufferlist::iterator & bp,
...
int PrimaryLogPG::do_tmapup(OpContext * ctx, bufferlist::iterator & bp,
                            OSDOp & osd_op)
{
...
struct FillInVerifyExtent:public Context {
    ...
    FillInVerifyExtent(ceph_le64 * r, int32_t * rv, bufferlist * blp,
                       ...
                       OSDService * osd, hobject_t soid, __le32 flags):r(r),
    ...
    for (auto p = ops.begin(); p != ops.end();
                ...
                for (auto clone_iter = ssc->snapset.clones.begin();
                    ...
                    if (!ssc->snapset.is_legacy()) {
                        ...
                        if (is_missing_object(clone_oid)) {
                            dout(20) << "LIST_SNAPS " << clone_oid << " missing"
                                << dendl;
                        ...
                        ObjectContextRef clone_obc =
                            get_object_context(clone_oid, false);
                        if (!clone_obc) {
                            if (maybe_handle_cache
                                (ctx->op, true, clone_obc, -ENOENT, clone_oid,
                                 true)) {
                                // promoting the clone
                                result = -EAGAIN;
                            }
                        ...
                        for (vector < snapid_t >::reverse_iterator p =
                             ...
                             p != clone_obc->obs.oi.legacy_snaps.rend(); ++p) {
        ...
        case CEPH_OSD_OP_WRITEFULL:
            ...
            {                   // write full object
                ...
                result =
                    check_offset_and_length(0, op.extent.length,
                                            cct->_conf->osd_max_object_size);
                if (result < 0)
                    break;
        ...
        case CEPH_OSD_OP_SETXATTR:
            ++ctx->num_write;
            {
                ...
                unsigned max_name_len =
                    MIN(osd->store->get_max_attr_name_length(),
        ...
        case CEPH_OSD_OP_STARTSYNC:
            tracepoint(osd, do_osd_op_pre_startsync, soid.oid.name.c_str(),
                       soid.snap.val);
            t->nop(soid);
            break;
            ...
            ++ctx->num_read;
            {
                ...
                if (oi.is_omap()) {
                    ObjectMap::ObjectMapIterator iter =
                        osd->store->get_omap_iterator(coll, ghobject_t(soid)
                        );
                    assert(iter);
            ...
            ++ctx->num_read;
            {
                ...
                if (oi.is_omap()) {
                    ...
                    for (num = 0;
                         ...
                         iter->key().substr(0,
                                            filter_prefix.size()) ==
                         filter_prefix; ++num, iter->next(false)) {
                                     ...
                                     bool try_no_whiteout)  // try not to whiteout
{
    ...
    if (get_osdmap()->require_osd_release >= CEPH_RELEASE_LUMINOUS) {
        ...
        if (!snapset.clones.empty() ||
            ...
            if (no_whiteout) {
                dout(20) << __func__ <<
                    " has or will have clones but no_whiteout=1" << dendl;
            }
            else {
                dout(20) << __func__ <<
                    " has or will have clones; will whiteout" << dendl;
                whiteout = true;
            }
        }
    }
...
void PrimaryLogPG::make_writeable(OpContext * ctx)
{
    ...
    if ((ctx->obs->exists && !ctx->obs->oi.is_whiteout()) &&    // head exist(ed)
        ...
        !ctx->cache_evict && snapc.snaps[0] > ctx->new_snapset.seq) {   // existing object is old
        // clone
        hobject_t coid = soid;
        coid.snap = snapc.seq;
        ...
        if (is_primary()) {
            ctx->clone_obc =
                object_contexts.lookup_or_create(static_snap_oi.soid);
            ...
            bool got = ctx->lock_manager.get_write_greedy(coid,
                                                          ctx->clone_obc,
                                                          ctx->op);
            assert(got);
            dout(20) << " got greedy write on clone_obc " << *ctx->
                clone_obc << dendl;
        }
        else {
        ...
        if (ctx->obs->oi.size)
            ctx->new_snapset.clone_overlap[coid.snap].insert(0,
                                                             ctx->obs->oi.size);
        ...
        ctx->log.
            push_back(pg_log_entry_t
                      (pg_log_entry_t::CLONE, coid, ctx->at_version,
                       ctx->obs->oi.version, ctx->obs->oi.user_version,
...
{
    for (interval_set < uint64_t >::const_iterator p = s.begin(); p != s.end();
         ++p) {
...
void PrimaryLogPG::complete_disconnect_watches(ObjectContextRef obc,
                                               const list < watch_disconnect_t >
                                               &to_disconnect)
{
    for (list < watch_disconnect_t >::const_iterator i =
         to_disconnect.begin(); i != to_disconnect.end(); ++i) {
        ...
        if (watchers_entry != obc->watchers.end()) {
            ...
            dout(10) << "do_osd_op_effects disconnect watcher " << watcher <<
                dendl;
...
{
    entity_name_t entity = ctx->reqid.name;
    dout(15) << "do_osd_op_effects " << entity << " con " << conn.
        get() << dendl;
    ...
    for (list < pair < watch_info_t, bool > >::iterator i =
         ctx->watch_connects.begin(); i != ctx->watch_connects.end(); ++i) {
        ...
        if (ctx->obc->watchers.count(watcher)) {
            ...
            watch =
                Watch::makeWatchRef(this, osd, ctx->obc,
                                    i->first.timeout_seconds, i->first.cookie,
    ...
    for (list < notify_info_t >::iterator p = ctx->notifies.begin();
        ...
        for (map < pair < uint64_t, entity_name_t >, WatchRef >::iterator i =
             ctx->obc->watchers.begin(); i != ctx->obc->watchers.end(); ++i) {
    ...
    for (list < OpContext::NotifyAck >::iterator p = ctx->notify_acks.begin();
        ...
        if (p->watch_cookie)
            ...
            dout(10) << "notify_ack " << make_pair("NULL",
                                                   p->notify_id) << dendl;
        for (map < pair < uint64_t, entity_name_t >, WatchRef >::iterator i =
             ctx->obc->watchers.begin(); i != ctx->obc->watchers.end(); ++i) {
            if (i->first.second != entity)
                continue;
            if (p->watch_cookie && p->watch_cookie.get() != i->first.first)
                continue;
                                                 ...
                                                 eversion_t version)
{
    ...
    ss << "temp_recovering_" << info.pgid   // (note this includes the shardid)
        << "_" << version
        << "_" << info.history.same_interval_since << "_" << target.snap;
...
int PrimaryLogPG::prepare_transaction(OpContext * ctx)
{
    ...
    if ((ctx->delta_stats.num_bytes > 0 || ctx->delta_stats.num_objects > 0) && // FIXME: keys?
        ...
        if (ctx->reqid.name.is_mds() || // FIXME: ignore MDS for now
            m->has_flag(CEPH_OSD_FLAG_FULL_FORCE)) {
            dout(20) << __func__ <<
                " full, but proceeding due to FULL_FORCE or MDS" << dendl;
        }
            ...
            // drop request
            dout(20) << __func__ << " full, dropping request (bad client)" <<
                dendl;
            return -EAGAIN;
        }
    }
                              ...
                              bool maintain_ssc)
{
        ...
        if (ctx->new_obs.exists) {
            if (!ctx->obs->exists) {
                if (ctx->snapset_obc && ctx->snapset_obc->obs.exists) {
                    ...
                    dout(10) << " removing unneeded snapdir " << snapoid <<
                        dendl;
                    ctx->log.
                        push_back(pg_log_entry_t
                                  (pg_log_entry_t::DELETE, snapoid,
                                   ctx->at_version,
                                   ctx->snapset_obc->obs.oi.version, 0,
            ...
            hobject_t snapoid(soid.oid, soid.get_key(), CEPH_SNAPDIR,
                              soid.get_hash(), info.pgid.pool(),
                              soid.get_namespace());
            dout(10) << " final snapset " << ctx->
                new_snapset << " in " << snapoid << dendl;
            ...
            if (ctx->lock_type == ObjectContext::RWState::RWWRITE) {
                got = ctx->lock_manager.get_write_greedy(snapoid,
                                                         ctx->snapset_obc,
                                                         ctx->op);
            }
            else {
                ...
                got =
                    ctx->lock_manager.
                    get_lock_type(ObjectContext::RWState::RWEXCL, snapoid,
                                  ctx->snapset_obc, ctx->op);
            }
            assert(got);
            dout(20) << " got greedy write on snapset_obc " << *ctx->
                snapset_obc << dendl;
    ...
    if (ctx->new_obs.exists) {
        ...
        if (ctx->mtime != utime_t()) {
            ...
            dout(10) << " mtime unchanged at " << ctx->new_obs.oi.
                mtime << dendl;
        }
        ...
        case pg_log_entry_t::CLEAN:
            if (legacy_snapset) {
                dout(20) << __func__ << " encoding legacy_snaps "
                    << ctx->new_obs.oi.legacy_snaps << dendl;
                ::encode(ctx->new_obs.oi.legacy_snaps, ctx->log.back().snaps);
            }
            else {
                dout(20) << __func__ << " encoding snaps from " << ctx->
                    new_snapset << dendl;
                ::encode(ctx->new_snapset.clone_snaps[soid.snap],
                         ctx->log.back().snaps);
            }
            break;
        default:
...
struct C_CopyFrom_AsyncReadCb:public Context {
    ...
    uint64_t features;
    size_t len;
    C_CopyFrom_AsyncReadCb(OSDOp * osd_op, uint64_t features):osd_op(osd_op),
        features(features), len(0) {
    } void finish(int r) override {
        osd_op->rval = r;
        if (r < 0) {
            return;
                              ...
                              OSDOp & osd_op, ObjectContextRef & obc)
{
    ...
    if (left > 0 && !cursor.data_complete) {
        if (cursor.data_offset < oi.size) {
            uint64_t max_read =
                MIN(oi.size - cursor.data_offset, (uint64_t) left);
            if (cb) {
                async_read_started = true;
                ctx->pending_async_reads.
                    push_back(make_pair
                              (boost::
                               make_tuple(cursor.data_offset, max_read,
                                          osd_op.op.flags), make_pair(&bl,
                                                                      cb)));
                cb->len = max_read;
                ...
                dout(10) << __func__ << ": async_read noted for " << soid <<
                    dendl;
            }
            else {
                result =
                    pgbackend->objects_read_sync(oi.soid, cursor.data_offset,
                                                 max_read, osd_op.op.flags,
                                                 &bl);
                if (result < 0)
                    return result;
            }
                                          ...
                                          OSDOp & osd_op)
{
    ...
    osd_op.rval = -ENOENT;
    MOSDOpReply *reply =
        new MOSDOpReply(m, 0, get_osdmap()->get_epoch(), 0, false);
                              ...
                              unsigned dest_obj_fadvise_flags)
{
    ...
    dout(10) << __func__ << " " << dest
        ...
        << " flags " << flags << (mirror_snapset ? " mirror_snapset" : "")
        << dendl;
    ...
    CopyOpRef cop(std::make_shared < CopyOp >
                  (cb, obc, src, oloc, version, flags, mirror_snapset,
                   src_obj_fadvise_flags, dest_obj_fadvise_flags));
    copy_ops[dest] = cop;
    obc->start_block();
...
void PrimaryLogPG::_copy_some(ObjectContextRef obc, CopyOpRef cop)
{
    ...
    gather.set_finisher(new C_OnFinisher(fin, &osd->objecter_finisher));
...
void PrimaryLogPG::process_copy_chunk(hobject_t oid, ceph_tid_t tid, int r)
{
    ...
    if (!cop->cursor.is_complete()) {
        ...
        if (cop->temp_cursor.is_initial()) {
            ...
            dout(20) << __func__ << " using temp " << cop->results.
                temp_oid << dendl;
        }
        ObjectContextRef tempobc =
            get_object_context(cop->results.temp_oid, true);
    ...
    if (cop->results.is_data_digest() || cop->results.is_omap_digest()) {
        dout(20) << __func__ << std::hex
            ...
            << std::dec << " flags " << cop->results.flags << dendl;
    }
    if (cop->results.is_data_digest() &&
        cop->results.data_digest != cop->results.source_data_digest) {
        derr << __func__ << std::hex << " data digest 0x" << cop->results.
            data_digest << " != source 0x" << cop->results.
            source_data_digest << std::dec << dendl;
        osd->clog->error() << info.pgid << " copy from " << cop->
            src << " to " << cop->obc->obs.oi.
            soid << std::hex << " data digest 0x" << cop->results.
            data_digest << " != source 0x" << cop->results.
            source_data_digest << std::dec;
        r = -EIO;
        goto out;
    }
    if (cop->results.is_omap_digest() &&
        ...
        osd->clog->error() << info.pgid << " copy from " << cop->src
            ...
            << " omap digest 0x" << cop->results.omap_digest
            << " != source 0x" << cop->results.source_omap_digest << std::dec;
        r = -EIO;
        goto out;
    }
    ...
    cop->results.fill_in_final_tx = std::function < void (PGTransaction *) > ([this, &cop /* avoid ref cycle */ ](PGTransaction * t) {
                                                                              ObjectState
                                                                              &
                                                                              obs
                                                                              =
                                                                              ...
                                                                              (20)
                                                                              <<
                                                                              "fill_in_final_tx: writing "
                                                                              <<
                                                                              "directly to final object"
                                                                              <<
                                                                              dendl;
  ...
  out:
    ...
    if (!r) {
        for (map < ceph_tid_t, ProxyReadOpRef >::iterator it =
             proxyread_ops.begin(); it != proxyread_ops.end();) {
        ...
        for (map < ceph_tid_t, ProxyWriteOpRef >::iterator it =
             proxywrite_ops.begin(); it != proxywrite_ops.end();) {
            if (it->second->soid == cobc->obs.oi.soid) {
...
void PrimaryLogPG::_write_copy_chunk(CopyOpRef cop, PGTransaction * t)
{
    ...
    if (!cop->temp_cursor.data_complete) {
        ...
        if (pool.info.requires_aligned_append() && !cop->cursor.data_complete) {
      /**
       ...
       */
        ...
        if (cop->data.length()) {
            t->write(cop->results.temp_oid,
                     ...
                     cop->data.length(),
                     cop->data, cop->dest_obj_fadvise_flags);
        }
        cop->data.clear();
    }
    if (pool.info.supports_omap()) {
                              ...
                              void () >> &&on_flush)
{
    ...
    // verify there are no (older) check for dirty clones
    {
        ...
        if (p != snapset.clones.rend()) {
            ...
            if (older_obc) {
                dout(20) << __func__ << " next oldest clone is " << older_obc->
                    obs.oi << dendl;
    ...
    if (p != flush_ops.end()) {
        ...
        dout(20) << __func__ << " canceling previous flush; it will fail" <<
            dendl;
        if (fop->op)
   ...
   */
    ...
    if (snapset.seq != 0) {
        ...
        for (vector < snapid_t >::reverse_iterator citer =
             snapset.clones.rbegin(); citer != snapset.clones.rend(); ++citer) {
            if (*citer < soid.snap) {
                prev_snapc = *citer;
                break;
            }
    ...
    ceph_tid_t tid = osd->objecter->mutate(soid.oid, base_oloc, o, snapc,
                                           ceph::real_clock::
                                           from_ceph_timespec(oi.mtime),
                                           CEPH_OSD_FLAG_IGNORE_OVERLAY |
                                           CEPH_OSD_FLAG_ENFORCE_SNAPC,
...
int PrimaryLogPG::try_flush_mark_clean(FlushOpRef fop)
{
    ...
    // have a ctx yet.
    if (ctx->lock_manager.get_lock_type(ObjectContext::RWState::RWWRITE,
                                        oid, obc, fop->op)) {
      ...
      for (auto op:fop->dup_ops) {
            bool locked =
                ctx->lock_manager.get_lock_type(ObjectContext::RWState::RWWRITE,
                                                oid,
                                                obc,
                                                op);
            assert(!locked);
...
{
    dout(10) << __func__ << ": repop tid " << repop->
...
void PrimaryLogPG::op_applied(const eversion_t & applied_version)
{
    ...
    if (is_primary()) {
        ...
        if (scrubber.active_rep_scrub) {
            if (last_update_applied >= static_cast <
                const MOSDRepScrub *
                >(scrubber.active_rep_scrub->get_req())->scrub_to) {
                osd->enqueue_back(info.pgid,
...
void PrimaryLogPG::eval_repop(RepGather * repop)
{
    ...
    if (m)
        dout(10) << "eval_repop " << *repop << (repop->rep_done ? " DONE" : "")
            << dendl;
    else
    ...
    if (repop->all_committed) {
        ...
        for (auto p = repop->on_committed.begin();
             p != repop->on_committed.end(); repop->on_committed.erase(p++)) {
            (*p) ();
        }
        ...
        if (waiting_for_ondisk.count(repop->v)) {
            ...
            for (list < pair < OpRequestRef, version_t > >::iterator i =
                 ...
                 i != waiting_for_ondisk[repop->v].end(); ++i) {
    ...
    if (repop->all_applied) {
        ...
        for (auto p = repop->on_applied.begin();
             p != repop->on_applied.end(); repop->on_applied.erase(p++)) {
            (*p) ();
        }
    }
...
void PrimaryLogPG::issue_repop(RepGather * repop, OpContext * ctx)
{
    ...
    Context *onapplied_sync = new C_OSD_OndiskWriteUnlock(ctx->obc,
                                                          ctx->clone_obc,
                                                          unlock_snapset_obc ?
                                                          ctx->
                                                          snapset_obc :
...
{
    if (ctx->op)
        dout(10) << "new_repop rep_tid " << rep_tid << " on " << *ctx->op->
            get_req() << dendl;
    else
        dout(10) << "new_repop rep_tid " << rep_tid << " (no op)" << dendl;

    RepGather *repop = new RepGather(ctx, rep_tid, info.last_complete, false);
...
boost::intrusive_ptr < PrimaryLogPG::RepGather >
    PrimaryLogPG::new_repop(eversion_t version, int r, ObcLockManager
                            && manager, OpRequestRef
                            && op,
                            boost::optional < std::function < void (void) >
                            >&&on_complete)
{
...
void PrimaryLogPG::remove_repop(RepGather * repop)
{
    ...
    for (auto p = repop->on_finish.begin();
         p != repop->on_finish.end(); repop->on_finish.erase(p++)) {
        (*p) ();
    }
...
void PrimaryLogPG::submit_log_entries(const mempool::osd_pglog::list <
                                      pg_log_entry_t > &entries, ObcLockManager
                                      && manager,
                                      boost::optional < std::function <
                                      ...
                                      OpRequestRef op, int r)
{
    ...
    if (get_osdmap()->require_osd_release >= CEPH_RELEASE_JEWEL) {
        repop = new_repop(version,
                          r,
                          std::move(manager),
                          std::move(op), std::move(_on_complete));
    }
    else {
    ...
    pgbackend->call_write_ordered([this, entries, repop, on_complete] () {
                                  ...
                                  new MOSDPGUpdateLogMissing(entries,
                                                             spg_t(info.pgid.
                                                                   pgid,
                                                                   i->shard),
                                                             pg_whoami.shard,
                                  ...
                                  MOSDPGLog * m =
                                  new MOSDPGLog(peer.shard, pg_whoami.shard,
                                  ...
                                  insert(make_pair(rep_tid, LogUpdateCtx {
                                                   std::move(repop),
                                                   std::move(waiting_on)}
                                  ...
                                  OnComplete(PrimaryLogPGRef pg,
                                             ceph_tid_t rep_tid, epoch_t epoch)
              :                  pg(pg), rep_tid(rep_tid),
                                  epoch(epoch) {
                                  }
                                  void finish(int) override {
                                  pg->lock();
                                  if (!pg->pg_has_reset_since(epoch)) {
                                  auto it =
                                  pg->log_entry_update_waiting_on.find(rep_tid);
                                  assert(it !=
                                         pg->log_entry_update_waiting_on.end());
                                  auto it2 =
                                  it->second.waiting_on.find(pg->pg_whoami);
                                  assert(it2 != it->second.waiting_on.end());
                                  it->second.waiting_on.erase(it2);
                                  if (it->second.waiting_on.empty()) {
                                  pg->repop_all_committed(it->second.repop.
                                                          get());
                                  pg->log_entry_update_waiting_on.erase(it);}
                                  }
                                  pg->unlock();}
                                  }; t.register_on_commit(new OnComplete {
                                                          this, rep_tid,
                                                          get_osdmap()->
                                                          get_epoch()}
                                  );}
                                  ...
                                  OnComplete(PrimaryLogPGRef pg,
                                             ...
                                             &on_complete, epoch_t epoch)
                                  :pg(pg), on_complete(std::move(on_complete)),
                                  epoch(epoch) {
                                  }
                                  void finish(int) override {
                                  pg->lock();
                                  if (!pg->pg_has_reset_since(epoch))
                                  on_complete(); pg->unlock();}
                                  ...
                                  t.register_on_applied(new C_OSD_OnApplied {
                                                        this,
                                                        get_osdmap()->
                                                        get_epoch(),
                                                        info.last_update}
                                  ...
                                  int r =
                                  osd->store->queue_transaction(osr.get(),
                                                                std::move(t),
                                                                NULL);
                                  assert(r == 0);}) ;
                                    ...
                                    list < obj_watch_item_t > &pg_watchers)
{
    for (map < pair < uint64_t, entity_name_t >, WatchRef >::iterator j =
         obc->watchers.begin(); j != obc->watchers.end(); ++j) {
...
{
    dout(20) << "PrimaryLogPG::check_blacklisted_obc_watchers for obc " << obc->
        obs.oi.soid << dendl;
    for (map < pair < uint64_t, entity_name_t >, WatchRef >::iterator k =
         obc->watchers.begin(); k != obc->watchers.end();) {
        ...
        dout(30) << "watch: Found " << j->second->
            get_entity() << " cookie " << j->second->get_cookie() << dendl;
...
{
    assert(is_active());
    assert((recovering.count(obc->obs.oi.soid) || !is_missing_object(obc->obs.oi.soid)) || (pg_log.get_log().objects.count(obc->obs.oi.soid) && // or this is a revert... see recover_primary()
                                                                                            pg_log.
                                                                                            get_log
                                                                                            ().
                                                                                            objects.
    ...
    for (map < pair < uint64_t, entity_name_t >, watch_info_t >::iterator p =
         obc->obs.oi.watchers.begin(); p != obc->obs.oi.watchers.end(); ++p) {
        ...
        WatchRef watch(Watch::
                       makeWatchRef(this, osd, obc, p->second.timeout_seconds,
                                    p->first.first, p->first.second,
                                    p->second.addr));
        watch->disconnect();
        obc->watchers.
            insert(make_pair
                   (make_pair(p->first.first, p->first.second), watch));
    }
...
void PrimaryLogPG::handle_watch_timeout(WatchRef watch)
{
    ...
    if (!obc->obs.exists) {
        dout(10) << __func__ << " object " << obc->obs.oi.
            soid << " dne" << dendl;
        return;
    }
    if (is_degraded_or_backfilling_object(obc->obs.oi.soid)) {
        callbacks_for_degraded_object[obc->obs.oi.soid].push_back(watch->
                                                                  get_delayed_cb
                                                                  ()
            );
    ...
    ctx->register_on_success([this, obc, watch_disconnects] () {
                             complete_disconnect_watches(obc,
                                                         watch_disconnects);}
    );
                                                     ...
                                                     SnapSetContext * ssc)
{
    ...
    dout(10) << "create_object_context " << (void *)obc.get() << " " << oi.
        soid << " " << dendl;
    if (is_active())
                                                  ...
                                                  bufferlist > *attrs)
{
    ...
    if (obc) {
        ...
        obc->ssc = get_snapset_context(soid, true,
    ...
    while (object_contexts.get_next(i.first, &i)) {
        ...
        if (obc) {
            for (map < pair < uint64_t, entity_name_t >,
                 WatchRef >::iterator j = obc->watchers.begin();
                                      ...
                                      hobject_t * pmissing)
{
    ...
    if (map_snapid_to_clone) {
        ...
        if (oid.snap > ssc->snapset.seq) {
            ...
            ObjectContextRef obc = get_object_context(head, false);
            dout(10) << "find_object_context " << oid << " @" << oid.snap
                << " snapset " << ssc->snapset << " maps to head" << dendl;
            *pobc = obc;
            put_snapset_context(ssc);
            ...
            vector < snapid_t >::const_iterator citer =
                std::find(ssc->snapset.clones.begin(),
                          ssc->snapset.clones.end(), oid.snap);
            ...
            dout(10) << "find_object_context " << oid << " @" << oid.snap
                << " snapset " << ssc->snapset << " maps to " << oid << dendl;
            ...
            dout(10) << "find_object_context " << oid << " @" << oid.snap
                << " snapset " << ssc->snapset << " " << oid << " HIT" << dendl;
            *pobc = obc;
            put_snapset_context(ssc);
            return 0;
        }
    ...
    hobject_t soid(oid.oid, oid.get_key(), ssc->snapset.clones[k],
                   oid.get_hash(), info.pgid.pool(), oid.get_namespace());
    ...
    dout(20) << "find_object_context  " << soid
        ...
        << " legacy_snaps " << obc->obs.oi.legacy_snaps << dendl;
    snapid_t first, last;
    if (obc->ssc->snapset.is_legacy()) {
        ...
        if (p->second.empty()) {
            dout(1) << __func__ << " " << soid << " empty snapset -- DNE" <<
                dendl;
    ...
    if (first <= oid.snap) {
        dout(20) << "find_object_context  " << soid << " [" << first << "," <<
            last << "] contains " << oid.snap << " -- HIT " << obc->
            obs << dendl;
        *pobc = obc;
        return 0;
    }
        ...
        dout(20) << "find_object_context  " << soid << " [" << first << "," <<
            last << "] does not contain " << oid.snap << " -- DNE" << dendl;
        return -ENOENT;
    }
}
    ...
    if (oi.soid.snap && oi.soid.snap != CEPH_NOSNAP
        ...
        if (obc->ssc->snapset.clone_overlap.count(oi.soid.snap)) {
            interval_set < uint64_t > &o =
                obc->ssc->snapset.clone_overlap[oi.soid.snap];
...
SnapSetContext *PrimaryLogPG::get_snapset_context(const hobject_t & oid,
                                                  bool can_create,
                                                  const map < string,
                                                  bufferlist > *attrs,
                                                  bool oid_existed)
        ...
        if (!attrs) {
            ...
            if (r < 0) {
                ...
                if (!(oid.is_snapdir() && !oid_existed))
                    r = pgbackend->objects_get_attr(oid.get_snapdir(), SS_ATTR,
                                                    &bv);
                if (r < 0 && !can_create)
                    return NULL;
            }
                                  ...
                                  int priority, PGBackend::RecoveryHandle * h)
{
    ...
    if (missing_loc.is_deleted(soid)) {
        ...
        remove_missing_object(soid, v, new FunctionContext([ =] (int) {
                                                           lock();
                                                           if
                                                           (!pg_has_reset_since
                                                            (cur_epoch)) {
              bool object_missing = false; for (const auto & shard:actingbackfill)
                                                           {
                                                           if (shard ==
                                                               pg_whoami)
                                                           continue;
    ...
    if (soid.snap && soid.snap < CEPH_NOSNAP) {
        ...
        if (pg_log.get_missing().is_missing(head)) {
            if (recovering.count(head)) {
                dout(10) << " missing but already recovering head " << head <<
                    dendl;
                return PULL_NONE;
            }
            else {
                int r =
                    recover_missing(head,
                                    pg_log.get_missing().get_items().
                                    find(head)->second.need, priority,
                                    h);
                if (r != PULL_NONE)
                    return PULL_OTHER;
                return PULL_NONE;
        ...
        if (pg_log.get_missing().is_missing(head)) {
            if (recovering.count(head)) {
                dout(10) << " missing but already recovering snapdir " << head
                    << dendl;
                return PULL_NONE;
            }
            else {
...
void PrimaryLogPG::send_remove_op(const hobject_t & oid, eversion_t v,
                                  pg_shard_t peer)
{
    ...
    MOSDSubOp *subop =
        new MOSDSubOp(rid, pg_whoami, spg_t(info.pgid.pgid, peer.shard),
                                         ...
                                         eversion_t v, Context * on_complete)
{
    ...
    epoch_t cur_epoch = get_osdmap()->get_epoch();
    t.register_on_complete(new FunctionContext([ =] (int) {
                                               lock();
                                               if (!pg_has_reset_since
                                                   (cur_epoch)) {
                                               ObjectStore::Transaction t2;
                                               on_local_recover(soid,
                                                                recovery_info,
                                                                ObjectContextRef
                                                                (), true, &t2);
                                               t2.
                                            ...
                                            eversion_t last_complete)
{
    ...
    if (!pg_has_reset_since(epoch)) {
        dout(10) << "_committed_pushed_object last_complete " << last_complete
            << " now ondisk" << dendl;
        ...
        if (last_complete_ondisk == info.last_update) {
            if (!is_primary()) {
                ...
                osd->send_message_osd_cluster(get_primary().osd,
                                              new MOSDPGTrim(get_osdmap()->
                                                             get_epoch(),
                                                             spg_t(info.pgid.
                                                                   pgid,
                                                                   get_primary
        ...
        dout(10) <<
            "_committed_pushed_object pg has changed, not touching last_complete_ondisk"
            << dendl;
    }
...
void PrimaryLogPG::_applied_recovered_object(ObjectContextRef obc)
{
    ...
    if (!deleting && active_pushes == 0 && scrubber.is_chunky_scrub_active()) {
        if (ops_blocked_by_scrub()) {
            requeue_scrub(true);
        }
        else {
...
void PrimaryLogPG::_applied_recovered_object_replica()
{
    ...
    if (!deleting && active_pushes == 0 &&
        ...
        && static_cast <
        const MOSDRepScrub * >(scrubber.active_rep_scrub->get_req())->chunky) {
        osd->enqueue_back(info.pgid,
                          PGQueueable(scrubber.active_rep_scrub,
                                      get_osdmap()->get_epoch()));
        scrubber.active_rep_scrub = OpRequestRef();
...
void PrimaryLogPG::do_update_log_missing(OpRequestRef & op)
{
    ...
    Context *complete = new FunctionContext([ =] (int){
                                            ...
                                            reply =
                                            new
                                            MOSDPGUpdateLogMissingReply(spg_t
                                                                        (info.
                                                                         pgid.
                                                                         pgid,
                                                                         primary_shard
    ...
    t.register_on_applied(new C_OSD_OnApplied {
                          this, get_osdmap()->get_epoch(), info.last_update}
    );
    int tr = osd->store->queue_transaction(osr.get(),
                                           std::move(t),
                                           nullptr);
    assert(tr == 0);
}
...
void PrimaryLogPG::do_update_log_missing_reply(OpRequestRef & op)
{
    ...
    if (it != log_entry_update_waiting_on.end()) {
        if (it->second.waiting_on.count(m->get_from())) {
            ...
            osd->clog->error()
                << info.pgid << " got reply "
                << *m << " from shard we are not waiting for " << m->get_from();
        }
 ...
 */
void PrimaryLogPG::mark_all_unfound_lost(int what,
                                         ConnectionRef con, ceph_tid_t tid)
{
    ...
    *_dout << dendl;
    ...
    while (m != mend) {
        ...
        case pg_log_entry_t::LOST_REVERT:
            ...
            if (prev > eversion_t()) {
                // log it
                pg_log_entry_t e(pg_log_entry_t::LOST_REVERT, oid, v,
        ...
        case pg_log_entry_t::LOST_DELETE:
            {
                pg_log_entry_t e(pg_log_entry_t::LOST_DELETE, oid, v,
                                 m->second.need, 0, osd_reqid_t(), mtime, 0);
                if (get_osdmap()->require_osd_release >= CEPH_RELEASE_JEWEL) {
                    if (pool.info.require_rollback()) {
    ...
    submit_log_entries(log_entries,
                       std::move(manager),
                       boost::optional < std::function < void (void) > >([this,
                                                                          oids,
                                                                          con,
                                                                          num_unfound,
                                                                          tid]
...
void PrimaryLogPG::apply_and_flush_repops(bool requeue)
{
    ...
    while (!repop_queue.empty()) {
        ...
        if (requeue) {
            ...
            map < eversion_t, list < pair < OpRequestRef,
                version_t > > >::iterator p = waiting_for_ondisk.find(repop->v);
            if (p != waiting_for_ondisk.end()) {
                dout(10) << " also requeuing ondisk waiters " << p->
                    second << dendl;
                for (list < pair < OpRequestRef, version_t > >::iterator i =
    ...
    if (requeue) {
        ...
        if (!waiting_for_ondisk.empty()) {
            for (map < eversion_t, list < pair < OpRequestRef,
                 version_t > > >::iterator i = waiting_for_ondisk.begin();
                 i != waiting_for_ondisk.end(); ++i) {
                for (list < pair < OpRequestRef, version_t > >::iterator j =
                     i->second.begin(); j != i->second.end(); ++j) {
                    derr << __func__ << ": op " << *(j->first->
                                                     get_req()) <<
...
void PrimaryLogPG::on_activate()
{
    ...
    if (needs_recovery()) {
        ...
        queue_peering_event(CephPeeringEvtRef
                            (std::make_shared < CephPeeringEvt >
                             (get_osdmap()->get_epoch(),
                              get_osdmap()->get_epoch(), DoRecovery())));
    }
        ...
        queue_peering_event(CephPeeringEvtRef
                            (std::make_shared < CephPeeringEvt >
                             (get_osdmap()->get_epoch(),
                              get_osdmap()->get_epoch(), RequestBackfill())));
    }
    else {
    ...
    if (!backfill_targets.empty()) {
        ...
        for (set < pg_shard_t >::iterator i = backfill_targets.begin();
            ...
            dout(5) << "target shard " << *i
                << " from " << peer_info[*i].last_backfill << dendl;
        }
    }
...
{
    dout(20) << __func__ << " checking missing set deletes flag. missing = " <<
        pg_log.get_missing() << dendl;
    ...
    assert(pg_log.get_missing().may_include_deletes ==
           get_osdmap()->test_flag(CEPH_OSDMAP_RECOVERY_DELETES));
}
                                      ...
                                      uint64_t * ops_started)
{
    ...
    if (recovering.empty() &&
        ...
        if (get_osdmap()->test_flag(CEPH_OSDMAP_NOBACKFILL)) {
            ...
            started +=
                recover_backfill(max - started, handle, &work_in_progress);
        }
    }
    ...
    dout(10) << __func__ << " needs_recovery: "
        << missing_loc.get_needs_recovery()
        << dendl;
    dout(10) << __func__ << " missing_loc: " << missing_loc.get_missing_locs()
        << dendl;
    ...
    if (missing.num_missing() > 0) {
        ...
        osd->clog->error() << info.
            pgid << " Unexpected Error: recovery ending with " << missing.
            num_missing() << ": " << missing.get_items();
        return work_in_progress;
    }
    ...
    if (state_test(PG_STATE_RECOVERING)) {
        ...
        queue_peering_event(CephPeeringEvtRef
                            (std::make_shared < CephPeeringEvt >
                             (get_osdmap()->get_epoch(),
                              get_osdmap()->get_epoch(), Backfilled())));
    }
    ...
    while (p != missing.get_rmissing().end()) {
            ...
            case pg_log_entry_t::LOST_REVERT:
                {
                    if (item.have == latest->reverting_to) {
                        ...
                        dout(10) << " need to pull prior_version " <<
                            alternate_need << " for revert " << item << dendl;

                        for (map < pg_shard_t, pg_missing_t >::iterator p =
                             peer_missing.begin(); p != peer_missing.end(); ++p)
                            if (p->second.is_missing(soid, need)
                                && p->second.get_items().at(soid).have ==
                                alternate_need) {
                                missing_loc.add_location(soid, p->first);
                            }
                        dout(10) << " will pull " << alternate_need << " or " <<
                            need << " from one of " << missing_loc.
                            get_locations(soid)
                            << dendl;
                    }
                }
        ...
        if (!recovering.count(soid)) {
            if (recovering.count(head)) {
                ...
                int r =
                    recover_missing(soid, need, get_recovery_op_priority(), h);
                switch (r) {
                case PULL_YES:
                    ++started;
                    break;
...
bool PrimaryLogPG::primary_error(const hobject_t & soid, eversion_t v)
{
    ...
    for (set < pg_shard_t >::iterator i = actingbackfill.begin();
        ...
        if (!peer_missing[peer].is_missing(soid, v)) {
            missing_loc.add_location(soid, peer);
            dout(10) << info.
                pgid << " unexpectedly missing " << soid << " v" << v <<
    ...
    if (uhoh)
        osd->clog->error() << info.
            pgid << " missing primary copy of " << soid << ", unfound";
    else
                                        ...
                                        ThreadPool::TPHandle & handle)
{
    ...
    for (set < pg_shard_t >::iterator i = actingbackfill.begin();
        ...
        dout(10) << " peer osd." << peer << " missing " << m_sz << " objects."
            << dendl;
        dout(20) << " peer osd." << peer << " missing " << pm->second.
            get_items() << dendl;

        // oldest first!
        const pg_missing_t & m(pm->second);
        for (map < version_t, hobject_t >::const_iterator p =
            ...
            if (missing_loc.is_unfound(soid)) {
                dout(10) << __func__ << ": " << soid << " still unfound" <<
                    dendl;
                continue;
            }
            ...
            if (recovering.count(soid)) {
                dout(10) << __func__ << ": already recovering " << soid <<
                    dendl;
                continue;
            }
            ...
            if (soid.is_snap()
                && pg_log.get_missing().is_missing(soid.get_head())) {
            ...
            if (soid.is_snap()
                && pg_log.get_missing().is_missing(soid.get_snapdir())) {
...
hobject_t PrimaryLogPG::earliest_peer_backfill() const const
{
    hobject_t e = hobject_t::get_max();
...
bool PrimaryLogPG::all_peer_done() const const
{
                                        ...
                                        bool * work_started)
{
    ...
    for (set < pg_shard_t >::iterator i = backfill_targets.begin();
         i != backfill_targets.end(); ++i) {
        dout(10) << "peer osd." << *i << " info " << peer_info[*i]
    ...
    while (ops < max) {
        ...
        for (set < pg_shard_t >::iterator i = backfill_targets.begin();
            ...
            if (pbi.begin <= backfill_info.begin &&
                ...
                dout(10) << " scanning peer osd." << bt << " from " << pbi.
                    end << dendl;
                epoch_t e = get_osdmap()->get_epoch();
                MOSDPGScan *m =
                    new MOSDPGScan(MOSDPGScan::OP_SCAN_GET_DIGEST, pg_whoami, e,
                                   last_peering_reset,
                                   spg_t(info.pgid.pgid, bt.shard),
                                   pbi.end, hobject_t());
                osd->send_message_osd_cluster(bt.osd, m,
                                              get_osdmap()->get_epoch());
                assert(waiting_on_backfill.find(bt) ==
        ...
        if (check < backfill_info.begin) {
            ...
            for (set < pg_shard_t >::iterator i = check_targets.begin();
                ...
                to_remove.
                    push_back(boost::
                              make_tuple(check, pbi.objects.begin()->second,
                                         bt));
                pbi.pop_front();
            }
            ...
            if (!need_ver_targs.empty() || !missing_targs.empty()) {
                ObjectContextRef obc =
                    get_object_context(backfill_info.begin, false);
                assert(obc);
                if (obc->get_recovery_read()) {
                    ...
                    handle.reset_tp_timeout();
                    int r =
                        prep_backfill_object_push(backfill_info.begin, obj_v,
                                                  obc, all_push, h);
                    if (r < 0) {
                        *work_started = true;
    ...
    if (HAVE_FEATURE(get_min_upacting_features(), SERVER_LUMINOUS)) {
        ...
        for (unsigned i = 0; i < to_remove.size(); ++i) {
            ...
            if (it != reqs.end()) {
                ...
                m = reqs[peer] =
                    new MOSDPGBackfillRemove(spg_t(info.pgid.pgid, peer.shard),
                                             get_osdmap()->get_epoch());
            }
                                            ...
                                            PGBackend::RecoveryHandle * h)
{
    ...
    for (unsigned int i = 0; i < peers.size(); ++i) {
        map < pg_shard_t, pg_missing_t >::iterator bpm =
            peer_missing.find(peers[i]);
    ...
    obc->ondisk_read_lock();
    int r = pgbackend->recover_object(oid,
                                      v,
                                      ObjectContextRef(),
                                      obc,
                                      h);
    ...
    if (bi->version >= projected_last_update) {
        ...
        auto func =[&](const pg_log_entry_t & e){
            ...
            if (soid >= bi->begin && soid < bi->end) {
                if (e.is_update()) {
                    dout(10) << __func__ << ": " << e.
                        soid << " updated to version " << e.version << dendl;
...
void PrimaryLogPG::check_local()
{
    ...
    for (list < pg_log_entry_t >::const_reverse_iterator p =
         pg_log.get_log().log.rbegin(); p != pg_log.get_log().log.rend(); ++p) {
        if (did.count(p->soid))
            continue;
        did.insert(p->soid);
        ...
        if (p->is_delete() && !is_missing_object(p->soid)) {
            dout(10) << " checking " << p->soid
                << " at " << p->version << dendl;
            struct stat st;
            int r = osd->store->stat(ch,
                                     ghobject_t(p->soid, ghobject_t::NO_GEN,
                                                pg_whoami.shard),
                                     &st);
            if (r != -ENOENT) {
                derr << __func__ << " " << p->
                    soid << " exists, but should have been " << "deleted" <<
                    dendl;
...
void PrimaryLogPG::hit_set_remove_all()
{
    ...
    if (!info.hit_set.history.empty()) {
        list < pg_hit_set_info_t >::reverse_iterator p =
...
void PrimaryLogPG::hit_set_persist()
{
    ...
    new_hset.end = now;
    oid = get_hit_set_archive_object(new_hset.begin,
                                     new_hset.end, new_hset.using_gmt);
    ...
    if (agent_state) {
        ...
        if (size >= pool.info.hit_set_count) {
            size =
                pool.info.hit_set_count > 0 ? pool.info.hit_set_count - 1 : 0;
        }
...
void PrimaryLogPG::agent_setup()
{
    ...
    if (!agent_state) {
        ...
        agent_state->position.set_hash(pool.info.
                                       get_random_pg_position(info.pgid.pgid,
                                                              rand()));
...
bool PrimaryLogPG::agent_work(int start_max, int agent_flush_quota)
{
    ...
    hobject_t next;
    int r =
        pgbackend->objects_list_partial(agent_state->position, ls_min, ls_max,
                                        &ls, &next);
    assert(r >= 0);
    ...
    for (vector < hobject_t >::iterator p = ls.begin(); p != ls.end(); ++p) {
        ...
        if (range_intersects_scrub(obc->obs.oi.soid,
            ...
            dout(20) << __func__ << " skip (scrubbing) " << obc->obs.
                oi << dendl;
            osd->logger->inc(l_osd_agent_skip);
            continue;
        }
        ...
        if (obc->is_request_pending()) {
            dout(20) << __func__ << " skip (request pending) " << obc->obs.
                oi << dendl;
            osd->logger->inc(l_osd_agent_skip);
            continue;
        }
        ...
        if (!base_pool->supports_omap() && obc->obs.oi.is_omap()) {
            dout(20) << __func__ << " skip (omap to EC) " << obc->obs.
                oi << dendl;
            osd->logger->inc(l_osd_agent_skip);
            continue;
        }
    ...
    dout(20) << __func__ << " start pos " << agent_state->position
        << " next start pos " << next << " started " << total_started << dendl;
...
void PrimaryLogPG::agent_load_hit_sets()
{
    ...
    if (agent_state->hit_set_map.size() < info.hit_set.history.size()) {
        dout(10) << __func__ << dendl;
        for (list < pg_hit_set_info_t >::iterator p =
             info.hit_set.history.begin(); p != info.hit_set.history.end();
             ++p) {
            if (agent_state->hit_set_map.count(p->begin.sec()) == 0) {
                ...
                hobject_t oid =
                    get_hit_set_archive_object(p->begin, p->end, p->using_gmt);
                if (is_unreadable_object(oid)) {
                    dout(10) << __func__ << " unreadable " << oid << ", waiting"
                        << dendl;
                    break;
                }
                ...
                ObjectContextRef obc = get_object_context(oid, false);
                if (!obc) {
                    derr << __func__ << ": could not load hitset " << oid <<
                        dendl;
                    break;
                }
...
bool PrimaryLogPG::agent_maybe_flush(ObjectContextRef & obc)
{
    ...
    int result = start_flush(OpRequestRef(), obc, false, NULL,
                             on_flush);
    if (result != -EINPROGRESS) {
        on_flush();
...
bool PrimaryLogPG::agent_maybe_evict(ObjectContextRef & obc, bool after_flush)
{
    ...
    if (agent_state->evict_mode != TierAgentState::EVICT_MODE_FULL) {
        ...
        if (ob_local_mtime + utime_t(pool.info.cache_min_evict_age, 0) > now) {
            dout(20) << __func__ << " skip (too young) " << obc->obs.
                oi << dendl;
            osd->logger->inc(l_osd_agent_skip);
            return false;
        }
    ...
    if (!ctx->lock_manager.get_lock_type(ObjectContext::RWState::RWWRITE,
                                         ...
                                         obc, OpRequestRef())) {
        close_op_ctx(ctx.release());
        dout(20) << __func__ << " skip (cannot get lock) " << obc->obs.
            oi << dendl;
        return false;
    }
...
bool PrimaryLogPG::agent_choose_mode(bool restart, OpRequestRef op)
{
...

    {
        ...
        const pg_pool_t *base_pool =
            get_osdmap()->get_pg_pool(pool.info.tier_of);
        assert(base_pool);
        ...
        uint64_t flush_high_target =
            pool.info.cache_target_dirty_high_ratio_micro;
        ...
        if (full_micro > 1000000) {
            ...
            dout(30) << __func__ << " evict_effort " << was << " quantized by "
                << inc << " to " << evict_effort << dendl;
        }
    }
...
bool PrimaryLogPG::already_complete(eversion_t v)
{
    ...
    for (xlist < RepGather * >::iterator i = repop_queue.begin(); !i.end(); ++i) {
        ...
        if (!(*i)->all_committed) {
            dout(20) << __func__ << ": " << **i
                << " not committed, returning false" << dendl;
            return false;
        }
    }
...
bool PrimaryLogPG::already_ack(eversion_t v)
{
    ...
    for (xlist < RepGather * >::iterator i = repop_queue.begin(); !i.end(); ++i) {
        ...
        if (!(*i)->all_applied) {
            dout(20) << __func__ << ": " << **i
                << " not applied, returning false" << dendl;
            return false;
        }
    }
    dout(20) << __func__ << ": returning true" << dendl;
...
void PrimaryLogPG::log_missing(unsigned missing,
                               ...
                               const spg_t & pgid,
                               const char *func,
                               const char *mode, bool allow_incomplete_clones)
{
    assert(head);
...
unsigned PrimaryLogPG::process_clones_to(const boost::optional < hobject_t >
                                         &head,
                                         const boost::optional < SnapSet >
                                         &snapset, LogChannelRef clog,
                                         const spg_t & pgid, const char *mode,
 ...
 */
void PrimaryLogPG::scrub_snapshot_metadata(ScrubMap & scrubmap,
                                           const map < hobject_t,
                                           ...
                                           uint32_t >>> &missing_digest)
{
    ...
    for (map < hobject_t, ScrubMap::object >::reverse_iterator
        ...
        soid_error = inconsistent_snapset_wrapper {
        soid};
        object_stat_sum_t stat;
        ...
        if (doing_clones(snapset, curclone)) {
            ...
            if (soid.has_snapset() || soid.get_head() != head->get_head()) {
                ...
                dout(10) << __func__ << " " << mode << " " << info.
                    pgid << " new object " << soid << " while processing " <<
                    head.get() << dendl;
        ...
        if (soid.has_snapset()) {
            ...
            dout(20) << __func__ << " " << mode << " new head " << head <<
                dendl;
            ...
            if (p->second.attrs.count(SS_ATTR) == 0) {
                ...
                try {
                    ...
                    snapset = boost::none;
                    osd->clog->error() << mode << " " << info.
                        pgid << " " << soid << " : can't decode '" << SS_ATTR <<
                        "' attr " << e.what();
                    ++scrubber.shallow_errors;
            ...
            if (snapset) {
                ...
                if (get_osdmap()->require_osd_release >= CEPH_RELEASE_LUMINOUS) {
                    if (soid.is_snapdir()) {
                        dout(10) << " will move snapset to head from " << soid
                            << dendl;
                        snapset_to_repair[soid.get_head()] = *snapset;
                    }
                    else if (snapset->is_legacy()) {
                        dout(10) << " will convert legacy snapset on " << soid
                            << " " << *snapset << dendl;
                        snapset_to_repair[soid.get_head()] = *snapset;
                    }
                }
            ...
            dout(20) << __func__ << " " << mode << " matched clone " << soid <<
                dendl;
            ...
            if (snapset->clone_size.count(soid.snap) == 0) {
                ...
                if (snapset->clone_overlap.count(soid.snap) == 0) {
                    osd->clog->error() << mode << " " << info.
                        pgid << " " << soid << " : is missing in clone_overlap";
                    ++scrubber.shallow_errors;
                    soid_error.set_size_mismatch();
                }
                else {
                    ...
                    for (interval_set < uint64_t >::const_iterator i =
                         overlap.begin(); i != overlap.end(); ++i) {
                        if (size < i.get_len()) {
            ...
            if (p != snapset_to_repair.end()) {
                if (!oi || oi->legacy_snaps.empty()) {
                    ...
                    dout(20) << __func__ << "   copying legacy_snaps " << oi->
                        legacy_snaps << " to snapset " << p->second << dendl;
  ...
  for (auto & p:snapset_to_repair) {
        ...
        if (pool.info.allow_incomplete_clones()) {
          for (auto s:p.second.clones) {
                if (p.second.clone_snaps.count(s) == 0) {
                    dout(10) << __func__ << " " << p.
                        first << " faking clone_snaps for " << s << dendl;
        ...
        if (!ctx->new_obs.exists) {
            dout(20) << __func__ << "   making " << p.
                first << " a whiteout" << dendl;
            ...
            if (p.first < scrubber.start) {
                dout(20) << __func__ <<
                    " kludging around update outside of scrub range" << dendl;
            }
            else {
                scrub_cstat.add(ctx->delta_stats);
            }
        }
        dout(20) << __func__ << "   final snapset " << ctx->
            new_snapset << dendl;
...
void PrimaryLogPG::_scrub_finish()
{
    ...
    dout(10) << mode << " got "
        << scrub_cstat.sum.num_objects << "/" << info.stats.stats.sum.
        num_objects << " objects, " << scrub_cstat.sum.
        num_object_clones << "/" << info.stats.stats.sum.
        num_object_clones << " clones, " << scrub_cstat.sum.
        num_objects_dirty << "/" << info.stats.stats.sum.
        num_objects_dirty << " dirty, " << scrub_cstat.sum.
        num_objects_omap << "/" << info.stats.stats.sum.
        num_objects_omap << " omap, " << scrub_cstat.sum.
        num_objects_pinned << "/" << info.stats.stats.sum.
        num_objects_pinned << " pinned, " << scrub_cstat.sum.
        num_objects_hit_set_archive << "/" << info.stats.stats.sum.
        num_objects_hit_set_archive << " hit_set_archive, " << scrub_cstat.sum.
        num_bytes << "/" << info.stats.stats.sum.
        num_bytes << " bytes, " << scrub_cstat.sum.
        num_bytes_hit_set_archive << "/" << info.stats.stats.sum.
        num_bytes_hit_set_archive << " hit_set_archive bytes." << dendl;
    ...
    if (scrub_cstat.sum.num_objects != info.stats.stats.sum.num_objects ||
        ...
        osd->clog->error() << info.
            pgid << " " << mode << " : stat mismatch, got " << scrub_cstat.sum.
            ...
            num_objects_pinned << " pinned, " << scrub_cstat.sum.
            num_objects_hit_set_archive << "/" << info.stats.stats.sum.
            num_objects_hit_set_archive << " hit_set_archive, " << scrub_cstat.
            sum.num_whiteouts << "/" << info.stats.stats.sum.
            num_whiteouts << " whiteouts, " << scrub_cstat.sum.
            num_bytes << "/" << info.stats.stats.sum.
            num_bytes << " bytes, " << scrub_cstat.sum.
            num_bytes_hit_set_archive << "/" << info.stats.stats.sum.
            num_bytes_hit_set_archive << " hit_set_archive bytes.";
        ++scrubber.shallow_errors;
        ...
        osd->clog->info() << info.
            pgid << " " << mode << " updated num_legacy_snapsets" << " from " <<
            info.stats.stats.sum.num_legacy_snapsets << " -> " << scrub_cstat.
            sum.num_legacy_snapsets << "\n";
        info.stats.stats.sum.num_legacy_snapsets =
            scrub_cstat.sum.num_legacy_snapsets;
        publish_stats_to_osd();
        share_pg_info();
                                            ...
                                            OpRequestRef op)
{
    ...
    if (r < 0) {
        ...
        dout(0) << __func__ <<
            ": Need version of replica, objects_get_attr failed: " << soid <<
            " error=" << r << dendl;
    }
    else
        try {
    ...
    if (primary_error(soid, v)) {
        dout(0) << __func__ << " No other replicas available for " << soid <<
            dendl;
...
:  my_base(ctx), NamedState(context < SnapTrimmer > ().pg, "NotTrimming")
{
...
boost::statechart::result PrimaryLogPG::WaitReservation::
...
boost::statechart::result PrimaryLogPG::AwaitAsyncWork::
react(const DoSnapWork &)
{
    ...
    ldout(pg->cct,
          10) << "AwaitAsyncWork: trimming snap " << snap_to_trim << dendl;

    vector < hobject_t > to_trim;
    unsigned max = pg->cct->_conf->osd_pg_max_concurrent_snap_trims;
    to_trim.reserve(max);
    int r = pg->snap_mapper.get_next_objects_to_trim(snap_to_trim,
                                                     max,
                                                     &to_trim);
    if (r != 0 && r != -ENOENT) {
        ...
        pg->write_if_dirty(t);
        int tr =
            pg->osd->store->queue_transaction(pg->osr.get(), std::move(t),
                                              NULL);
        assert(tr == 0);
  ...
  for (auto && object:to_trim) {
        // Get next
        ldout(pg->cct,
              10) << "AwaitAsyncWork react trimming " << object << dendl;
        OpContextUPtr ctx;
        ...
        if (error) {
            ...
            if (!in_flight.empty()) {
                ldout(pg->cct,
                      10) << "letting the ones we already started finish" <<
                    dendl;
...
void PrimaryLogPG::setattr_maybe_cache(ObjectContextRef obc,
                                       OpContext * op,
                                       PGTransaction * t,
...
void PrimaryLogPG::setattrs_maybe_cache(ObjectContextRef obc,
                                        OpContext * op,
                                        PGTransaction * t,
                                        map < string, bufferlist > &attrs)
{
...
void PrimaryLogPG::rmattr_maybe_cache(ObjectContextRef obc,
                                      OpContext * op,
                                      PGTransaction * t, const string & key)
{
    t->rmattr(obc->obs.oi.soid, key);
}

int PrimaryLogPG::getattr_maybe_cache(ObjectContextRef obc,
                                      const string & key, bufferlist * val)
{
    if (pool.info.require_rollback()) {
