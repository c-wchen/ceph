...
class PyModuleRegistry {
  ...
  :    clog(clog_) {
    ...
    void active_start(PyModuleConfig & config_,
                      DaemonStateIndex & ds, ClusterState & cs, MonClient & mc,
                      LogChannelRef clog_, Objecter & objecter_,
                      Client & client_, Finisher & f);
    void standby_start(MonClient * monc);
    ...
    template < typename Callback, typename ... Args >
        ...
        std::forward < Callback > (cb) (*active_modules,
                                        std::forward < Args > (args) ...);
    }
    // FIXME: breaking interface so that I don't have to go rewrite all// the places that call into these (for now)// >>>
