...
class MClientRequest:public Message {
  ...
  public:
    ...
    struct Release {
        ...
        string dname;
         ...
         Release():item(), dname() {
        } Release(const ceph_mds_request_release & rel, string name):item(rel),
            dname(name) {
  ...
  public:
    ...
    void set_string2(const char *s) {
        path2.set_path(boost::string_view(s), 0);
    }
    ...
    } const vector < uint64_t > &get_caller_gid_list() const {
        return gid_list;
    ...
    void mark_queued_for_replay() {
        queued_for_replay = true;
    }
    bool is_queued_for_replay() {
