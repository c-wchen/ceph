...
void TrashMoveRequest < I >::get_mirror_image_id()
{
    ...
    auto aio_comp = create_rados_callback <
                    TrashMoveRequest < I >,
                    &TrashMoveRequest < I >::handle_get_mirror_image_id >
                    (this);
    m_out_bl.clear();
    int r =
        m_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op,
                             &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
} template < typename I >
void TrashMoveRequest < I >::handle_get_mirror_image_id(int r)
{
    ...
    if (r == 0) {
        ...
        r = librbd::cls_client::
            mirror_image_get_image_id_finish(&bl_it, &m_image_id);
    }
    if (r == -ENOENT) {
        dout(10) << "image " << m_global_image_id <<
...
void TrashMoveRequest < I >::get_mirror_info()
{
    ...
    auto ctx = create_context_callback <
               TrashMoveRequest < I >,
               &TrashMoveRequest < I >::handle_get_mirror_info > (this);
    auto req =
        librbd::mirror::GetInfoRequest < I >::create(m_io_ctx,
            m_op_work_queue,
            m_image_id,
            &m_mirror_image,
            &m_promotion_state,
...
void TrashMoveRequest < I >::handle_get_mirror_info(int r)
{
    ...
    if (r == -ENOENT) {
        dout(5) << "image " << m_global_image_id <<
                " is not mirrored" << dendl;
        finish(r);
        return;
    } else if (r < 0) {
        derr << "error retrieving image primary info for image "
             << m_global_image_id << ": " << cpp_strerror(r) <<
             dendl;
        finish(r);
        return;
    }

    if (m_promotion_state ==
        librbd::mirror::PROMOTION_STATE_PRIMARY) {
        dout(10) << "image " << m_global_image_id <<
                 " is local primary" << dendl;
        finish(-EPERM);
        return;
    } else if (m_promotion_state ==
               librbd::mirror::PROMOTION_STATE_ORPHAN && !m_resync) {
        dout(10) << "image " << m_global_image_id << " is orphaned"
                 << dendl;
        finish(-EPERM);
        return;
    }
...
void TrashMoveRequest < I >::disable_mirror_image()
{
    ...
    librbd::cls_client::mirror_image_set(&op, m_image_id,
                                         m_mirror_image);

    auto aio_comp = create_rados_callback <
                    TrashMoveRequest < I >,
                    &TrashMoveRequest < I >::handle_disable_mirror_image >
                    (this);
...
template < typename I >
void TrashMoveRequest <
I >::handle_disable_mirror_image(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r == -ENOENT) {
        dout(10) <<
                 "local image is not mirrored, aborting deletion." <<
                 dendl;
        finish(r);
        return;
    } else if (r == -EEXIST || r == -EINVAL) {
        derr << "cannot disable mirroring for image " <<
             m_global_image_id <<
...
template < typename I > void TrashMoveRequest < I >::open_image()
{
    ...
    m_image_ctx =
        I::create("", m_image_id, nullptr, m_io_ctx, false);
    ...
    Context *ctx = create_context_callback <
                   TrashMoveRequest < I >,
                   &TrashMoveRequest < I >::handle_open_image > (this);
...
void TrashMoveRequest < I >::handle_open_image(int r)
{
    ...
    if (r == -ENOENT) {
        dout(5) <<
                "mirror image does not exist, removing orphaned metadata"
                << dendl;
        m_image_ctx = nullptr;
        remove_mirror_image();
        return;
    ...
    if (r < 0) {
        derr << "failed to open image: " << cpp_strerror(r) <<
             dendl;
        m_image_ctx = nullptr;
        finish(r);
        return;
...
template < typename I > void TrashMoveRequest < I >::reset_journal()
{
    ...
    librbd::Journal <>::get_work_queue(reinterpret_cast <
                                       CephContext *
                                       >(m_io_ctx.cct()),
                                       &context_wq);
    ...
    auto ctx = create_context_callback <
               TrashMoveRequest < I >,
               &TrashMoveRequest < I >::handle_reset_journal > (this);
    auto req =
        librbd::journal::ResetRequest < I >::create(m_io_ctx,
            m_image_id,
            librbd::
            Journal <>::
            IMAGE_CLIENT_ID,
...
void TrashMoveRequest < I >::handle_reset_journal(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        derr << "failed to reset journal: " << cpp_strerror(r) <<
             dendl;
        m_ret_val = r;
        close_image();
        return;
...
template < typename I > void TrashMoveRequest < I >::acquire_lock()
{
    ...
    Context *ctx = create_context_callback <
                   TrashMoveRequest < I >,
                   &TrashMoveRequest < I >::handle_acquire_lock > (this);
...
void TrashMoveRequest < I >::handle_acquire_lock(int r)
{
    ...
    if (r < 0) {
        derr << "failed to acquire exclusive lock: " <<
             cpp_strerror(r) << dendl;
        m_ret_val = r;
        close_image();
        return;
...
template < typename I > void TrashMoveRequest < I >::trash_move()
{
    ...
    m_trash_image_spec = {
        cls::rbd::TRASH_IMAGE_SOURCE_MIRRORING, m_image_ctx->name,
        delete_time, deferment_end_time
    };

    Context *ctx = create_context_callback <
                   TrashMoveRequest < I >,
                   &TrashMoveRequest < I >::handle_trash_move > (this);
    auto req =
        librbd::trash::MoveRequest < I >::create(m_io_ctx,
            m_image_id,
            m_trash_image_spec,
            ctx);
    req->send();
...
void TrashMoveRequest < I >::handle_trash_move(int r)
{
    ...
    if (r < 0) {
        derr << "failed to move image to trash: " << cpp_strerror(r)
             << dendl;
        m_ret_val = r;
        close_image();
        return;
...
void TrashMoveRequest < I >::remove_mirror_image()
{
    ...
    auto ctx = create_context_callback <
               TrashMoveRequest < I >,
               &TrashMoveRequest < I >::handle_remove_mirror_image >
               (this);
    auto req =
        librbd::mirror::ImageRemoveRequest < I >::create(m_io_ctx,
            m_global_image_id,
            m_image_id,
            ctx);
    req->send();
...
void TrashMoveRequest < I >::handle_remove_mirror_image(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        derr << "failed to remove mirror image state for " <<
             m_global_image_id << ": " << cpp_strerror(r) << dendl;
        m_ret_val = r;
    }
...
template < typename I > void TrashMoveRequest < I >::close_image()
{
    ...
    Context *ctx = create_context_callback <
                   TrashMoveRequest < I >,
                   &TrashMoveRequest < I >::handle_close_image > (this);
...
void TrashMoveRequest < I >::handle_close_image(int r)
{
    ...
    if (r < 0) {
        derr << "failed to close image: " << cpp_strerror(r) <<
             dendl;
    }
...
void TrashMoveRequest < I >::notify_trash_add()
{
    ...
    Context *ctx = create_context_callback <
                   TrashMoveRequest < I >,
                   &TrashMoveRequest < I >::handle_notify_trash_add > (this);
    librbd::TrashWatcher < I >::notify_image_added(m_io_ctx,
            m_image_id,
            m_trash_image_spec,
            ctx);
}
...
void TrashMoveRequest < I >::handle_notify_trash_add(int r)
{
    ...
    if (r < 0) {
        derr << "failed to notify trash watchers: " <<
             cpp_strerror(r) << dendl;
    }
...
template class rbd::mirror::image_deleter::TrashMoveRequest <
    librbd::ImageCtx >;
