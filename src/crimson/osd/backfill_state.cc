...
namespace crimson::osd {
    ...
    BackfillState::BackfillState(BackfillState::
                                 BackfillListener & backfill_listener,
                                 std::unique_ptr <
                                 BackfillState::PeeringFacade > peering_state,
                                 std::unique_ptr < BackfillState::PGFacade > pg)
    :backfill_machine(*this, backfill_listener, std::move(peering_state),
                      std::move(pg)),
        progress_tracker(std::make_unique < BackfillState::ProgressTracker >
                         (backfill_machine)) {
    ...
    BackfillState::BackfillMachine::
        BackfillMachine(BackfillState & backfill_state,
                        ...
                        std::unique_ptr < BackfillState::PeeringFacade >
                        peering_state,
                        std::unique_ptr < BackfillState::PGFacade > pg)
  :    backfill_state(backfill_state), backfill_listener(backfill_listener),
        peering_state(std::move(peering_state)), pg(std::move(pg)) {
    }
  ...
  :    my_base(ctx) {
        ...
        logger().debug("{}: bft={} from {}", __func__,
    ...
    boost::statechart::result
      ...
      for (const auto & bt:peering_state().get_backfill_targets()) {
            backfill_state().peer_backfill_info[bt].reset(peering_state().
                                                          get_peer_last_backfill
                                                          (bt));
        }
        backfill_state().backfill_info.reset(backfill_state().
                                             last_backfill_started);
        if (Enqueuing::
            all_enqueued(peering_state(), backfill_state().backfill_info,
...
// -- Enqueuing
    void BackfillState::Enqueuing::maybe_update_range() {
        if (auto & primary_bi = backfill_state().backfill_info;
            ...
            peering_state().scan_log_after(primary_bi.version,
                                           ...
                                           logger().
                                           debug
                                           ("maybe_update_range(lambda): updating from version {}",
                                            e.version);
                                           if (e.soid >= primary_bi.begin
                                               && e.soid < primary_bi.end) {
                                           if (e.is_update()) {
                                           logger().
                                           debug
                                           ("maybe_update_range(lambda): {} updated to ver {}",
                                            e.soid, e.version);
                                           ...
                                           logger().
                                           debug
                                           ("maybe_update_range(lambda): {} removed",
                                            e.soid);
                                           primary_bi.objects.erase(e.soid);}
                                           }
                                           }
    ...
    void BackfillState::Enqueuing::trim_backfill_infos() {
      for (const auto & bt:peering_state().get_backfill_targets()) {
            backfill_state().peer_backfill_info[bt].
                trim_to(std::
                        max(peering_state().get_peer_last_backfill(bt),
                            backfill_state().last_backfill_started));
        }
        backfill_state().backfill_info.trim_to(backfill_state().
                                               last_backfill_started);
    }
...
/* static */ bool BackfillState::Enqueuing::all_enqueued(
                                                            ...
                                                            &peer_backfill_info)
    {
        ...
        const bool all_peer_enqueued =
            std::all_of(std::begin(peer_backfill_info),
                        std::end(peer_backfill_info),
                        [](const auto & kv){
                        [[maybe_unused]] const auto &[shard,
                                                      peer_backfill_info] = kv;
    ...
    hobject_t BackfillState::Enqueuing::earliest_peer_backfill(const std::map <
                                                               pg_shard_t,
                                                               BackfillInterval
                                                               >
                                                               &peer_backfill_info)
    ...
    bool BackfillState::Enqueuing::should_rescan_replicas(const std::map <
                                                          pg_shard_t,
                                                          BackfillInterval >
                                                          &peer_backfill_info,
                                                          const BackfillInterval
                                                          & backfill_info)const
    {
        const auto & targets = peering_state().get_backfill_targets();
        return std::any_of(std::begin(targets), std::end(targets),
                           [&](const auto & bt) {
                           return ReplicasScanning::
                           replica_needs_scan(peer_backfill_info.at(bt),
                                              backfill_info);}
        );
    }

    bool BackfillState::Enqueuing::should_rescan_primary(const std::map <
                                                         pg_shard_t,
                                                         BackfillInterval >
                                                         &peer_backfill_info,
                                                         const BackfillInterval
    ...
    } void BackfillState::Enqueuing::
        trim_backfilled_object_from_intervals(BackfillState::Enqueuing::result_t
                                              && result,
                                              hobject_t & last_backfill_started,
                                              std::map < pg_shard_t,
                                              BackfillInterval >
                                              &peer_backfill_info) {
        std::for_each(std::begin(result.pbi_targets),
                      std::end(result.pbi_targets),
      ...
      for (const auto & bt:peering_state().get_backfill_targets()) {
            ...
            if (const eversion_t & obj_v = primary_bi.objects.begin()->second;
                ...
                if (peer_bi.objects.begin()->second != obj_v &&
                    backfill_state().progress_tracker->enqueue_push(primary_bi.
                                                                    begin)) {
                ...
                if (primary_bi.begin >
                    peering_state().get_peer_last_backfill(bt)
                    && backfill_state().progress_tracker->
                    enqueue_push(primary_bi.begin)) {
                    backfill_listener().enqueue_push(primary_bi.begin, obj_v);
                }
            }
        }
    ...
    bool BackfillState::Enqueuing::Enqueuing::
        all_emptied(const BackfillInterval & local_backfill_info,
                    const std::map < pg_shard_t,
                    BackfillInterval > &peer_backfill_info)const {
        const auto & targets = peering_state().get_backfill_targets();
...
// -- PrimaryScanning
  ...
  :    my_base(ctx) {
        backfill_state().backfill_info.version =
            peering_state().get_last_update();
        backfill_listener().request_primary_scan(backfill_state().backfill_info.
                                                 begin);
    }
    ...
    boost::statechart::result
        ...
        logger().
            debug("PrimaryScanning::react() on ObjectPushed; evt.object={}",
                  evt.object);
...
// -- ReplicasScanning
    bool BackfillState::ReplicasScanning::
        replica_needs_scan(const BackfillInterval & replica_backfill_info,
  ...
  :    my_base(ctx) {
    ...
    boost::statechart::result
        ...
        if (waiting_on_backfill.erase(evt.from)) {
            backfill_state().peer_backfill_info[evt.from] =
                std::move(evt.result);
    ...
    boost::statechart::result
        ...
        logger().
            debug("ReplicasScanning::react() on ObjectPushed; evt.object={}",
                  evt.object);
        backfill_state().progress_tracker->complete_to(evt.object, evt.stat);
        return discard_event();
    }
...
// updates.
    bool BackfillState::ProgressTracker::tracked_objects_completed() const {
        ...
        [[maybe_unused]] const auto[it, first_seen] =
            registry.try_emplace(obj,
                                 registry_item_t { op_stage_t::enqueued_push,
                                 std::nullopt });
        return first_seen;
    }

    void BackfillState::ProgressTracker::enqueue_drop(const hobject_t & obj) {
        registry.try_emplace(obj, registry_item_t {
                             op_stage_t::enqueued_drop, pg_stat_t {
                             }});
    }

    void BackfillState::ProgressTracker::complete_to(const hobject_t & obj,
                                                     const pg_stat_t & stats) {
        logger().debug("{}: obj={}", __func__, obj);
        if (auto completion_iter = registry.find(obj);
            completion_iter != std::end(registry)) {
            completion_iter->second = registry_item_t {
            op_stage_t::completed_push, stats};
        }
        else {
        ...
        for (auto it = std::begin(registry);
            ...
            auto &[soid, item] = *it;
            assert(item.stats);
            peering_state().update_complete_backfill_object_stats(soid,
                                                                  *item.stats);
        }
        if (Enqueuing::all_enqueued(peering_state(),
                                    backfill_state().backfill_info,
