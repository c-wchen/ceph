...
std::ostream &operator<<(std::ostream &out, const block_shard_info_t &sf)
{
    ...
    return out;
                           ...
                           const block_sm_superblock_t &sb)
{
    out << "superblock(" << "shard_num=" << sb.
        shard_num << ", segment_size=" << sb.
        segment_size << ", block_size=" << sb.block_size << ", shard_info:";
...
seastar::future < crimson::os::seastore::SegmentManagerRef > SegmentManager::get_segment_manager(
    const std::string &device,
    device_type_t dtype)
{
#ifdef HAVE_ZNS
    LOG_PREFIX(SegmentManager::get_segment_manager);
    return seastar::do_with(static_cast < size_t >(0), [&](auto & nr_zones) {
        return seastar::open_file_dma(device + "/block",
                                      seastar::
                                      open_flags::rw).
        then([&](auto file) {
            return seastar::do_with(file,
                                    [ =,
                                      &nr_zones](auto &
                                                 f)->
                                    seastar::future <
    ...
    return seastar::make_ready_future <
           crimson::os::seastore::SegmentManagerRef > (std::make_unique <
                   segment_manager::block::
                   BlockSegmentManager >
                   (device + "/block",
                    dtype));
#endif
