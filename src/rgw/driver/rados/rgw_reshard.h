...
class RGWBucketReshardLock {
    ...
    RGWBucketReshardLock(rgw::sal::RadosStore * _store,
                         const std::string & reshard_lock_oid, bool _ephemeral);
    RGWBucketReshardLock(rgw::sal::RadosStore * _store,
                         const RGWBucketInfo & bucket_info,
                         bool _ephemeral):RGWBucketReshardLock(_store,
                                                               bucket_info.
                                                               bucket.
                                                               get_key(':'),
                                                               _ephemeral) {
...
class RGWBucketReshard {
  ...
  public:
    ...
    // returns the prime in our list less than or equal to the// parameter; the lowest value that can be returned is 1
        static uint32_t get_prime_shards_less_or_equal(uint32_t
                                                       requested_shards) {
    ...
    // for multisite, the RGWBucketInfo keeps a history of old log generations// until all peers are done with them. prevent this log history from growing// too large by refusing to reshard the bucket until the old logs get trimmed
...
class RGWReshard {
  ...
  protected:
    ...
    std::string get_logshard_key(const std::string & tenant,
                                 const std::string & bucket_name);
    void get_bucket_logshard_oid(const std::string & tenant,
                                 const std::string & bucket_name,
                                 std::string * oid);

  public:
    RGWReshard(rgw::sal::RadosStore * _store, bool _verbose =
               false, std::ostream * _out = nullptr, Formatter * _formatter =
               nullptr);
    ...
    int list(const DoutPrefixProvider * dpp, int logshard_num,
             std::string & marker, uint32_t max,
             std::list < cls_rgw_reshard_entry > &entries, bool * is_truncated);
    int clear_bucket_resharding(const DoutPrefixProvider * dpp,
                                const std::string & bucket_instance_oid,
