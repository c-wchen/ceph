...
namespace librbd {
    namespace image {
        ...
        class TestMockImageAttachChildRequest:public TestMockFixture {
          public:
            ...
            void expect_add_child(MockImageCtx & mock_image_ctx, int r) {
                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                            exec(RBD_CHILDREN, _, StrEq("rbd"),
                                 StrEq("add_child"), _, _, _, _))
                    .WillOnce(Return(r));
            }

            void expect_refresh(MockRefreshRequest & mock_refresh_request,
                                int r) {
                EXPECT_CALL(mock_refresh_request, send())
                    .WillOnce(Invoke([this, &mock_refresh_request, r] () {
                                     image_ctx->op_work_queue->
                                     queue(mock_refresh_request.on_finish,
                                           r);}));
            }
            ...
            void expect_op_features_set(MockImageCtx & mock_image_ctx, int r) {
                bufferlist bl;
                encode(static_cast < uint64_t >
                       (RBD_OPERATION_FEATURE_CLONE_CHILD), bl);
                encode(static_cast < uint64_t >
                       (RBD_OPERATION_FEATURE_CLONE_CHILD), bl);

                EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                            exec(util::header_name(mock_image_ctx.id), _,
                                 StrEq("rbd"), StrEq("op_features_set"),
            ...
            void expect_child_attach(MockImageCtx & mock_image_ctx, int r) {
                ...
                encode(cls::rbd::ChildImageSpec {
                       m_ioctx.get_id(), "", mock_image_ctx.id}, bl);
        ...
        TEST_F(TestMockImageAttachChildRequest, SuccessV1) {
            ...
            C_SaferCond ctx;
            auto req =
                MockAttachChildRequest::create(&mock_image_ctx, &mock_image_ctx,
                                               image_ctx->snap_id, nullptr, 0,
                                               1,
                                               &ctx);
            req->send();
            ASSERT_EQ(0, ctx.wait());
