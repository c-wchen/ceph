                                       ...
                                       const DoutPrefixProvider *dpp)
{
    ...
    for (const auto& [id, key] : info.access_keys) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: could not remove " << id << " (access key object), should be fixed (err=" << ret << ")" <<
                              dendl;
            return ret;
        }
    }
    ...
    for (const auto& [id, key] : info.swift_keys) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: could not remove " << id << " (swift name object), should be fixed (err=" << ret << ")" <<
                              dendl;
            return ret;
        }
    }
