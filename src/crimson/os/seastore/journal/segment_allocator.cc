...
namespace crimson::os::seastore::journal {
    ...
    SegmentAllocator::SegmentAllocator(JournalTrimmer * trimmer,
                                       data_category_t category,
                                       rewrite_gen_t gen,
                                       SegmentProvider & sp,
                                       SegmentSeqAllocator & ssa)
    :print_name {
    fmt::format("{}_G{}", category, gen)}, type {
    trimmer == nullptr ?
            segment_type_t::OOL : segment_type_t::JOURNAL}, category {
    category}, gen {
    gen}, segment_provider {
    sp}, sm_group {
    *sp.get_segment_manager_group()}, segment_seq_allocator(ssa), trimmer {
    trimmer}
    {
        reset();
    }

    segment_nonce_t calc_new_nonce(segment_type_t type,
                                   uint32_t crc,
                                   unsigned char const *data, unsigned length) {
    ...
    SegmentAllocator::open_ret SegmentAllocator::do_open(bool is_mkfs) {
        ...
        auto new_segment_id =
            segment_provider.allocate_segment(new_segment_seq, type, category,
                                              gen);
        ...
        return sm_group.open(new_segment_id).
            handle_error(open_ertr::pass_further {
                    ...
                    alloc_tail = trimmer->get_alloc_tail(); if (is_mkfs) {
                    ...
                    }; dirty_tail = mkfs_seq; alloc_tail = mkfs_seq;}
                    else {
                    ceph_assert(dirty_tail != JOURNAL_SEQ_NULL);
                    ceph_assert(alloc_tail != JOURNAL_SEQ_NULL);}
                    }
                    ...
                    bufferptr bp(ceph::buffer::
                                 create_page_aligned(header_length)); bp.zero();
                    auto iter = bl.cbegin(); iter.copy(bl.length(), bp.c_str());
                    bl.clear(); bl.append(bp);
                    ...
                    new_segment_seq, paddr_t::make_seg_paddr(segment_id,
                                                             written_to)};
    ...
    SegmentAllocator::write_ret
        ...
        auto write_start_seq = journal_seq_t {
            segment_provider.get_seg_info(current_segment->get_segment_id()).
                seq,
            paddr_t::make_seg_paddr(current_segment->get_segment_id(),
                                    write_start_offset)
        };
    ...
    SegmentAllocator::close_segment_ertr::future <>
        ...
        return p_seg_to_close->advance_wp(sm_group.get_segment_size() -
                                          sm_group.get_rounded_tail_length()
            ).
            safe_then([this, FNAME, bl =
                       std::move(bl), p_seg_to_close] ()mutable {
                      DEBUG("Writing tail info to segment {}",
                            p_seg_to_close->get_segment_id());
                      return p_seg_to_close->write(sm_group.get_segment_size() -
                                                   sm_group.
                    ...
                    segment_provider.close_segment(seg_to_close->
                                                   get_segment_id());}
