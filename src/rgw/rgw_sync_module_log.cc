...
class RGWLogStatRemoteObjCBCR:public RGWStatRemoteObjCBCR {
  public:
    RGWLogStatRemoteObjCBCR(RGWDataSyncEnv * _sync_env,
                            ...
                            rgw_obj_key & _key):RGWStatRemoteObjCBCR(_sync_env,
                                                                     _bucket_info,
                                                                     _key) {
...
class RGWLogStatRemoteObjCR:public RGWCallStatRemoteObjCR {
  public:
    RGWLogStatRemoteObjCR(RGWDataSyncEnv * _sync_env,
                          RGWBucketInfo & _bucket_info,
                          rgw_obj_key & _key):RGWCallStatRemoteObjCR(_sync_env,
                                                                     _bucket_info,
                                                                     _key) {
...
class RGWLogDataSyncModule:public RGWDataSyncModule {
  ...
  public:
     RGWLogDataSyncModule(const string & _prefix):prefix(_prefix) {
        ...
        ldout(sync_env->cct,
              0) << prefix << ": SYNC_LOG: sync_object: b=" << bucket_info.
    ...
    RGWCoroutine *remove_object(RGWDataSyncEnv * sync_env,
                                ...
                                rgw_zone_set * zones_trace) override {
        ldout(sync_env->cct,
              0) << prefix << ": SYNC_LOG: rm_object: b=" << bucket_info.
    ...
    RGWCoroutine *create_delete_marker(RGWDataSyncEnv * sync_env,
                                       ...
                                       rgw_zone_set * zones_trace) override {
        ldout(sync_env->cct,
              0) << prefix << ": SYNC_LOG: create_delete_marker: b=" <<
            bucket_info.
            bucket << " k=" << key << " mtime=" << mtime << " versioned=" <<
            versioned << " versioned_epoch=" << versioned_epoch << dendl;
        return NULL;
...
int RGWLogSyncModule::create_instance(CephContext * cct, map < string, string,
                                      ltstr_nocase > &config,
