...
class ClassHandler {
  public:
    ...
    struct ClassData {
        ...
        ClassMethod *register_method(const char *mname, int flags,
                                     cls_method_call_t func);
        ClassMethod *register_cxx_method(const char *mname, int flags,
        ...
        ClassFilter *get_filter(const std::string & filter_name) {
            Mutex::Locker l(handler->mutex);
            std::map < std::string, ClassFilter >::iterator i =
                filters_map.find(filter_name);
