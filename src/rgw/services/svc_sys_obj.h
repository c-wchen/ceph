     ...
     class Obj {
      ...
      public:
        ...
        struct ROp {
            ...
            ceph::static_ptr < RGWSI_SysObj_Obj_GetObjState,
                sizeof(RGWSI_SysObj_Core_GetObjState) > state;
            ...
            int read(const DoutPrefixProvider * dpp, int64_t ofs, int64_t end,
                     bufferlist * pbl, optional_yield y);
            int read(const DoutPrefixProvider * dpp, bufferlist * pbl,
                     optional_yield y) {
        ...
        struct WOp {
          ...
          WOp(Obj & _source):source(_source) {
            ...
            int write(const DoutPrefixProvider * dpp, bufferlist & bl,
                      optional_yield y);
            ...
            int write_attr(const DoutPrefixProvider * dpp, const char *name, bufferlist & bl, optional_yield y);    /* write attrs only */
        };
        ...
        struct OmapOp {
            ...
            int get_all(const DoutPrefixProvider * dpp, std::map < std::string,
                        bufferlist > *m, optional_yield y);
            int get_vals(const DoutPrefixProvider * dpp,
                         ...
                         optional_yield y);
            int set(const DoutPrefixProvider * dpp, const std::string & key,
                    bufferlist & bl, optional_yield y);
            int set(const DoutPrefixProvider * dpp,
                    const std::map < std::string, bufferlist > &m,
                    optional_yield y);
            int del(const DoutPrefixProvider * dpp, const std::string & key,
                    optional_yield y);
        };
        ...
        struct WNOp {
            ...
            } int notify(const DoutPrefixProvider * dpp, bufferlist & bl,
    ...
    class Pool {
      ...
      public:
         Pool(RGWSI_SysObj_Core * _core_svc,
              const rgw_pool & _pool):core_svc(_core_svc), pool(_pool) {
        } rgw_pool & get_pool() {
            return pool;
        }
        ...
        struct Op {
            ...
            ListCtx ctx;
             ...
             Op(Pool & _source):source(_source) {
                       ...
                       const std::string & marker, const std::string & prefix);
        ...
        int list_prefixed_objs(const DoutPrefixProvider * dpp,
