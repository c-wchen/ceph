...
bool rgw_cls_bi_entry::get_info(cls_rgw_obj_key * key, uint8_t * category,
...
void rgw_bucket_dir_header::dump(Formatter * f) const const
{
    ...
    map < uint8_t, struct rgw_bucket_category_stats >::const_iterator iter =
        stats.begin();
...
void rgw_bucket_dir::dump(Formatter * f) const const
{
    ...
    header.dump(f);
    f->close_section();
    map < string, struct rgw_bucket_dir_entry >::const_iterator iter =
        m.begin();
