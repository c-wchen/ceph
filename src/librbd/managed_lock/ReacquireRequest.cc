...
namespace librbd {
    namespace managed_lock {
        ...
        :m_ioctx(ioctx), m_oid(oid), m_old_cookie(old_cookie),
            m_new_cookie(new_cookie), m_exclusive(exclusive),
            ...
            rados::cls::lock::set_cookie(&op, RBD_LOCK_NAME,
                                         m_exclusive ? LOCK_EXCLUSIVE :
                                         LOCK_SHARED, m_old_cookie,
        ...
        template < typename I >
            ...
            if (r == -EOPNOTSUPP) {
                ldout(cct,
                      10) << ": OSD doesn't support updating lock" << dendl;
            }
            else if (r < 0) {
                lderr(cct) << ": failed to update lock: " << cpp_strerror(r) <<
                    dendl;
            }
