        ...
        namespace snapshot {
            ...
            using librbd::util::create_rados_callback;
             ...
             template < typename I > void SetImageStateRequest < I >::send() {
                ...
                if (r == 0) {
                    auto it = m_bl.cbegin();
                     r = cls_client::dir_get_name_finish(&it,
                                                         &m_image_state.name);
                } if (r < 0) {
                    lderr(cct) << "failed to retrieve image name: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
            ...
            template < typename I >
                ...
                if (r == 0) {
                    auto it = m_bl.cbegin();
                    r = cls_client::snapshot_get_limit_finish(&it,
                                                              &m_image_state.
                                                              snap_limit);
                }

                if (r < 0) {
                    lderr(cct) << "failed to retrieve snapshot limit: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
                ...
                ldout(cct,
                      15) << "snap_limit=" << m_image_state.snap_limit << dendl;
            ...
            template < typename I >
                ...
                auto req =
                    image::GetMetadataRequest < I >::create(m_image_ctx->md_ctx,
                                                            m_image_ctx->
                                                            header_oid, true,
                                                            "", "", 0,
                                                            &m_image_state.
            ...
            template < typename I >
...

                {
                  ...
                  for (auto &[snap_id, snap_info]:m_image_ctx->
                         snap_info)
                    {
                        auto type =
                        ...
                        snap_info.snap_namespace,
                                snap_info.name, snap_info.protection_status};
                    }
                }
            ...
            template < typename I >
                ...
                auto ctx = create_context_callback <
                    SetImageStateRequest < I >,
                    &SetImageStateRequest < I >::handle_write_image_state >
                    (this);

                auto req =
                    WriteImageStateRequest < I >::create(m_image_ctx, m_snap_id,
                                                         m_image_state, ctx);
                req->send();
            }

            template < typename I >
                void SetImageStateRequest <
            ...
            template < typename I >
                ...
                librbd::cls_client::mirror_image_snapshot_set_copy_progress(&op,
                                                                            m_snap_id,
                                                                            true,
                                                                            0);
                ...
                auto aio_comp = create_rados_callback <
                    ...
                    I >::handle_update_primary_snapshot > (this);
                int r =
                    m_image_ctx->md_ctx.aio_operate(m_image_ctx->header_oid,
                                                    aio_comp,
                                                    &op);
            ...
            template < typename I >
                void SetImageStateRequest <
                I >::handle_update_primary_snapshot(int r) {
                CephContext *cct = m_image_ctx->cct;
                ldout(cct, 15) << "r=" << r << dendl;

                if (r < 0) {
                    lderr(cct) << "failed to update primary snapshot: " <<
                        cpp_strerror(r)
                        << dendl;
                    finish(r);
                    return;
...
template class librbd::mirror::snapshot::SetImageStateRequest <
    librbd::ImageCtx >;
