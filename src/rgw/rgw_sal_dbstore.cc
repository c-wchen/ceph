...
namespace rgw::sal {
    ...
    int DBUser::list_buckets(const DoutPrefixProvider * dpp,
        ...
        buckets.clear();
        ret =
            store->getDB()->list_buckets(dpp, "", info.user_id, marker,
                                         end_marker, max, need_stats, &ulist,
                                         &is_truncated);
        if (ret < 0)
            return ret;
        ...
        for (const auto & ent:ulist.get_buckets()) {
            buckets.add(std::make_unique < DBBucket >
                        (this->store, ent.second, this));
        } return 0;
    }
    ...
    int DBUser::create_bucket(const DoutPrefixProvider * dpp,
        ...
        if (*existed) {
            ...
            ret =
                store->getDB()->create_bucket(dpp, this->get_info(),
                                              bucket->get_key(), zid,
                                              placement_rule,
                                              swift_ver_location, pquota_info,
                                              attrs, info, pobjv, &ep_objv,
                                              creation_time, pmaster_bucket,
                                              pmaster_num_shards, y, exclusive);
            if (ret == -EEXIST) {
                *existed = true;
                ret = 0;
            }
    ...
    int DBUser::read_attrs(const DoutPrefixProvider * dpp, optional_yield y) {
        int ret;
        ret =
            store->getDB()->get_user(dpp, string("user_id"), get_id().id, info,
                                     &attrs, &objv_tracker);
        return ret;
    }
    ...
    int DBUser::read_stats_async(const DoutPrefixProvider * dpp,
                                 RGWGetUserStats_CB * cb) {
        return 0;
    }

    int DBUser::complete_flush_stats(const DoutPrefixProvider * dpp,
                                     optional_yield y) {
        return 0;
    }

    int DBUser::read_usage(const DoutPrefixProvider * dpp, uint64_t start_epoch,
    ...
    int DBUser::trim_usage(const DoutPrefixProvider * dpp, uint64_t start_epoch,
                           uint64_t end_epoch) {
        return 0;
    }
    ...
    int DBUser::merge_and_store_attrs(const DoutPrefixProvider * dpp,
    ...
    int DBUser::store_user(const DoutPrefixProvider * dpp, optional_yield y,
                           bool exclusive, RGWUserInfo * old_info) {
        int ret = 0;

        ret =
            store->getDB()->store_user(dpp, info, exclusive, &attrs,
                                       &objv_tracker, old_info);
    ...
    int DBUser::verify_mfa(const std::string & mfa_str, bool * verified,
                           const DoutPrefixProvider * dpp, optional_yield y) {
        *verified = false;
        return 0;
    }

    int DBBucket::remove_bucket(const DoutPrefixProvider * dpp,
        ...
        if (!delete_children) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp,
                          20) << __func__ << ": Bucket list objects returned "
                    << ret << dendl;
                return ret;
            }
            ...
            if (!results.objs.empty()) {
                ret = -ENOTEMPTY;
                ldpp_dout(dpp,
                          -1) << __func__ << ": Bucket Not Empty.. returning "
                    << ret << dendl;
                return ret;
            }
        }
    ...
    int DBBucket::load_bucket(const DoutPrefixProvider * dpp, optional_yield y,
        ...
        ret =
            store->getDB()->get_bucket_info(dpp, string("name"), "", info,
                                            &attrs, &mtime, &bucket_version);
    ...
    int DBBucket::read_stats_async(const DoutPrefixProvider * dpp,
    ...
    int DBBucket::sync_user_stats(const DoutPrefixProvider * dpp,
                                  optional_yield y) {
        return 0;
    }
    ...
    int DBBucket::chown(const DoutPrefixProvider * dpp, User & new_user,
                        optional_yield y) {
        int ret;

        ret =
            store->getDB()->update_bucket(dpp, "owner", info, false,
                                          &(new_user.get_id()), nullptr,
                                          nullptr, nullptr);
        return ret;
    }

    int DBBucket::put_info(const DoutPrefixProvider * dpp, bool exclusive,
                           ceph::real_time _mtime) {
        int ret;

        ret =
            store->getDB()->update_bucket(dpp, "info", info, exclusive, nullptr,
                                          nullptr, &_mtime, &info.objv_tracker);
    ...
    int DBBucket::check_quota(const DoutPrefixProvider * dpp, RGWQuota & quota,
    ...
    int DBBucket::merge_and_store_attrs(const DoutPrefixProvider * dpp,
                                        Attrs & new_attrs, optional_yield y) {
        int ret = 0;
      ...
      for (auto & it:new_attrs) {
        ...
        ret =
            store->getDB()->update_bucket(dpp, "attrs", info, false, nullptr,
                                          &new_attrs, nullptr,
    ...
    int DBBucket::try_refresh_info(const DoutPrefixProvider * dpp,
                                   ceph::real_time * pmtime) {
        int ret = 0;

        ret =
            store->getDB()->get_bucket_info(dpp, string("name"), "", info,
                                            &attrs, pmtime, &bucket_version);

        return ret;
    }

    /* XXX: usage and stats not supported in the first pass */
    int DBBucket::read_usage(const DoutPrefixProvider * dpp,
    ...
    int DBBucket::trim_usage(const DoutPrefixProvider * dpp,
                             uint64_t start_epoch, uint64_t end_epoch) {
        return 0;
    }

    int DBBucket::remove_objs_from_index(const DoutPrefixProvider * dpp,
    ...
    int DBBucket::check_index(const DoutPrefixProvider * dpp,
                              std::map < RGWObjCategory,
    ...
    int DBBucket::set_acl(const DoutPrefixProvider * dpp,
        ...
        ret =
            store->getDB()->update_bucket(dpp, "attrs", info, false,
                                          &(acl.get_owner().get_id()), &attrs,
                                          nullptr, nullptr);
    ...
    int DBBucket::list(const DoutPrefixProvider * dpp, ListParams & params,
    ...
    std::unique_ptr < MultipartUpload >
        DBBucket::get_multipart_upload(const std::string & oid,
                                       std::optional < std::string > upload_id,
                                       ACLOwner owner, ceph::real_time mtime) {
        return std::make_unique < DBMultipartUpload > (this->store, this, oid,
                                                       upload_id,
    ...
    } int DBZoneGroup::get_placement_target_names(std::set < std::string >
                                                  &names) const {
    ...
    int DBObject::get_obj_state(const DoutPrefixProvider * dpp,
                                RGWObjState ** pstate, optional_yield y,
                                bool follow_olh) {
        RGWObjState *astate;
        DB::Object op_target(store->getDB(), get_bucket()->get_info(),
                             get_obj());
        int ret =
            op_target.get_obj_state(dpp, get_bucket()->get_info(), get_obj(),
                                    follow_olh, &astate);
        if (ret < 0) {
            return ret;
        }
    ...
    int DBObject::read_attrs(const DoutPrefixProvider * dpp,
                             DB::Object::Read & read_op, optional_yield y,
                             rgw_obj * target_obj) {
    ...
    int DBObject::set_obj_attrs(const DoutPrefixProvider * dpp,
    ...
    int DBObject::get_obj_attrs(optional_yield y,
    ...
    int DBObject::modify_obj_attrs(const char *attr_name, bufferlist & attr_val,
                                   optional_yield y,
                                   const DoutPrefixProvider * dpp) {
    ...
    int DBObject::delete_obj_attrs(const DoutPrefixProvider * dpp,
    ...
    int DBObject::omap_get_vals(const DoutPrefixProvider * dpp,
    ...
    int DBObject::omap_get_all(const DoutPrefixProvider * dpp,
                               std::map < std::string, bufferlist > *m,
                               optional_yield y) {
        DB::Object op_target(store->getDB(), get_bucket()->get_info(),
                             get_obj());
    ...
    int DBObject::omap_set_val_by_key(const DoutPrefixProvider * dpp,
    ...
    int DBObject::chown(User & new_user, const DoutPrefixProvider * dpp,
                        optional_yield y) {
        return 0;
    }

    std::unique_ptr < MPSerializer >
        DBObject::get_serializer(const DoutPrefixProvider * dpp,
                                 const std::string & lock_name) {
        return std::make_unique < MPDBSerializer > (dpp, store, this,
                                                    lock_name);
    }
    ...
    bool DBObject::placement_rules_match(rgw_placement_rule & r1,
                                         rgw_placement_rule & r2) {
        /* XXX: support single default zone and zonegroup for now */
        return true;
    }

    int DBObject::dump_obj_layout(const DoutPrefixProvider * dpp,
  ...
  DBObject::DBReadOp::DBReadOp(DBObject * _source, RGWObjectCtx * _rctx):
    ...
    int DBObject::DBReadOp::prepare(optional_yield y,
                                    const DoutPrefixProvider * dpp) {
        uint64_t obj_size;
    ...
    int DBObject::DBReadOp::read(int64_t ofs, int64_t end, bufferlist & bl,
                                 optional_yield y,
                                 const DoutPrefixProvider * dpp) {
        return parent_op.read(ofs, end, bl, dpp);
    }

    int DBObject::DBReadOp::get_attr(const DoutPrefixProvider * dpp,
  ...
  DBObject::DBDeleteOp::DBDeleteOp(DBObject * _source):
    ...
    int DBObject::DBDeleteOp::delete_obj(const DoutPrefixProvider * dpp,
                                         optional_yield y) {
    ...
    int DBObject::delete_obj_aio(const DoutPrefixProvider * dpp,
    ...
    int DBObject::DBReadOp::iterate(const DoutPrefixProvider * dpp, int64_t ofs,
    ...
    int DBMultipartUpload::abort(const DoutPrefixProvider * dpp,
                                 CephContext * cct) {
    ...
    int DBMultipartUpload::init(const DoutPrefixProvider * dpp,
    ...
    int DBMultipartUpload::list_parts(const DoutPrefixProvider * dpp,
                                      CephContext * cct, int num_parts,
                                      int marker, int *next_marker,
                                      bool * truncated, bool assume_unsorted) {
        ...
        while (!parts_map.empty()) {
            std::unique_ptr < DBMultipartPart > part =
                std::make_unique < DBMultipartPart > ();
        ...
        do {
            ...
            for (auto obj_iter = parts.begin();
                ...
                DBMultipartPart *part =
                    dynamic_cast <
                    rgw::sal::DBMultipartPart * >(obj_iter->second.get());
    ...
    int DBMultipartUpload::get_info(const DoutPrefixProvider * dpp,
                                    optional_yield y,
        ...
        ret =
            read_op->read(0, store->getDB()->get_max_head_size(), headbl, y,
                          dpp);
        if (ret < 0) {
            if (ret == -ENOENT) {
                return -ERR_NO_SUCH_UPLOAD;
    ...
    std::unique_ptr < Writer >
        DBMultipartUpload::get_writer(const DoutPrefixProvider * dpp,
                                      optional_yield y, rgw::sal::Object * obj,
                                      const rgw_user & owner,
                                      const rgw_placement_rule *
                                      ptail_placement_rule, uint64_t part_num,
                                      const std::string & part_num_str) {
        return std::make_unique < DBMultipartWriter > (dpp, y, this, obj, store,
                                                       owner,
                                                       ptail_placement_rule,
                                                       part_num, part_num_str);
    }

  DBMultipartWriter::DBMultipartWriter(const DoutPrefixProvider * dpp, optional_yield y, MultipartUpload * upload, rgw::sal::Object * obj, DBStore * _driver, const rgw_user & _owner, const rgw_placement_rule * _ptail_placement_rule, uint64_t _part_num, const std::string & _part_num_str):
    StoreWriter(dpp, y),
        store(_driver),
        owner(_owner),
        ...
        op_target(_driver->getDB(), head_obj->get_bucket()->get_info(),
                  head_obj->get_obj(), upload_id), parent_op(&op_target),
    ...
    int DBMultipartWriter::process(bufferlist && data, uint64_t offset) {
        ...
        if (data.length() != 0) {
            ...
            int ret =
                parent_op.write_data(dpp, tail_part_data, tail_part_offset);
            if (ret < 0) {
                return ret;
            }
            tail_part_size = 0;
    ...
    int DBMultipartWriter::complete(size_t accounted_size,
  ...
  DBAtomicWriter::DBAtomicWriter(const DoutPrefixProvider * dpp, optional_yield y, rgw::sal::Object * _obj, DBStore * _driver, const rgw_user & _owner, const rgw_placement_rule * _ptail_placement_rule, uint64_t _olh_epoch, const std::string & _unique_tag):
    StoreWriter(dpp, y),
        store(_driver),
        owner(_owner),
        ptail_placement_rule(_ptail_placement_rule),
        ...
        op_target(_driver->getDB(), obj.get_bucket()->get_info(),
                  obj.get_obj()), parent_op(&op_target) {
    }
    ...
    int DBAtomicWriter::complete(size_t accounted_size,
                                 const std::string & etag,
                                 ceph::real_time * mtime,
                                 ceph::real_time set_mtime,
                                 std::map < std::string, bufferlist > &attrs,
        ...
        accounted_size = total_data_size;
        int ret =
            parent_op.write_meta(dpp, total_data_size, accounted_size, attrs);
        if (canceled) {
    ...
    std::unique_ptr < Writer >
        DBStore::get_append_writer(const DoutPrefixProvider * dpp,
                                   optional_yield y, rgw::sal::Object * obj,
                                   const rgw_user & owner,
                                   const rgw_placement_rule *
                                   ptail_placement_rule,
    ...
    std::unique_ptr < Writer >
        DBStore::get_atomic_writer(const DoutPrefixProvider * dpp,
                                   optional_yield y, rgw::sal::Object * obj,
                                   const rgw_user & owner,
                                   const rgw_placement_rule *
    ...
    int DBStore::get_user_by_access_key(const DoutPrefixProvider * dpp,
    ...
    int DBStore::get_user_by_email(const DoutPrefixProvider * dpp,
    ...
    int DBStore::get_user_by_swift(const DoutPrefixProvider * dpp,
    ...
    std::string DBStore::get_cluster_id(const DoutPrefixProvider * dpp,
                                        optional_yield y) {
    ...
    int DBStore::get_bucket(User * u, const RGWBucketInfo & i,
                            std::unique_ptr < Bucket > *bucket) {
        Bucket *bp;
    ...
    int DBStore::forward_request_to_master(const DoutPrefixProvider * dpp,
    ...
    int DBStore::forward_iam_request_to_master(const DoutPrefixProvider * dpp,
    ...
    int DBStore::get_zonegroup(const std::string & id,
                               std::unique_ptr < ZoneGroup > *zg) {
        /* XXX: for now only one zonegroup supported */
        ZoneGroup *group =
            new DBZoneGroup(this, std::make_unique < RGWZoneGroup > ());
        if (!group)
            return -ENOMEM;
    ...
    int DBLifecycle::get_entry(const std::string & oid,
                               const std::string & marker,
                               std::unique_ptr < LCEntry > *entry) {
        return store->getDB()->get_entry(oid, marker, entry);
    }

    int DBLifecycle::get_next_entry(const std::string & oid,
                                    const std::string & marker,
                                    std::unique_ptr < LCEntry > *entry) {
    ...
    int DBLifecycle::list_entries(const std::string & oid,
                                  const std::string & marker,
    ...
    int DBLifecycle::get_head(const std::string & oid,
    ...
    std::unique_ptr < LCSerializer >
        DBLifecycle::get_serializer(const std::string & lock_name,
                                    const std::string & oid,
                                    const std::string & cookie) {
        return std::make_unique < LCDBSerializer > (store, oid, lock_name,
                                                    cookie);
    }

    std::unique_ptr < Notification >
        DBStore::get_notification(rgw::sal::Object * obj,
    ...
    std::unique_ptr < Notification >
        DBStore::get_notification(const DoutPrefixProvider * dpp,
                                  rgw::sal::Object * obj,
    ...
    int DBStore::log_usage(const DoutPrefixProvider * dpp,
                           map < rgw_user_bucket, RGWUsageBatch > &usage_info) {
        return 0;
    }

    int DBStore::log_op(const DoutPrefixProvider * dpp, string & oid,
                        bufferlist & bl) {
        return 0;
    }

    int DBStore::register_to_service_map(const DoutPrefixProvider * dpp,
                                         const string & daemon_type,
                                         const map < string, string > &meta) {
        return 0;
    }

    void DBStore::get_ratelimit(RGWRateLimitInfo & bucket_ratelimit,
    ...
    int DBStore::set_buckets_enabled(const DoutPrefixProvider * dpp,
        ...
        for (iter = buckets.begin(); iter != buckets.end(); ++iter) {
            ...
            int r =
                getDB()->get_bucket_info(dpp, string("name"), "", info, &attrs,
                                         nullptr, nullptr);
            if (r < 0) {
                ldpp_dout(dpp,
                          0) << "NOTICE: get_bucket_info on bucket=" << bucket.
                    name << " returned err=" << r << ", skipping bucket" <<
                    dendl;
                ret = r;
                continue;
            }
            ...
            r = getDB()->update_bucket(dpp, "info", info, false, nullptr,
                                       &attrs, nullptr, &info.objv_tracker);
            if (r < 0) {
                ldpp_dout(dpp,
    ...
    RGWDataSyncStatusManager *DBStore::
        get_data_sync_manager(const rgw_zone_id & source_zone) {
        return 0;
    }

    int DBStore::read_all_usage(const DoutPrefixProvider * dpp,
                                uint64_t start_epoch, uint64_t end_epoch,
                                uint32_t max_entries, bool * is_truncated,
    ...
    int DBStore::trim_all_usage(const DoutPrefixProvider * dpp,
                                uint64_t start_epoch, uint64_t end_epoch) {
        return 0;
    }
    ...
    int DBStore::meta_list_keys_init(const DoutPrefixProvider * dpp,
                                     const string & section,
                                     const string & marker, void **phandle) {
        return 0;
    }

    int DBStore::meta_list_keys_next(const DoutPrefixProvider * dpp,
                                     void *handle, int max,
    ...
    int DBStore::meta_remove(const DoutPrefixProvider * dpp,
    ...
    int DBStore::initialize(CephContext * _cct, const DoutPrefixProvider * _dpp) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp,
                      0) << "GC thread creation failed: ret = " << ret << dendl;
        }

        return ret;
    }

    int DBLuaManager::get_script(const DoutPrefixProvider * dpp,
                                 optional_yield y, const std::string & key,
                                 std::string & script) {
        return -ENOENT;
    }

    int DBLuaManager::put_script(const DoutPrefixProvider * dpp,
                                 optional_yield y, const std::string & key,
                                 const std::string & script) {
        return -ENOENT;
    }

    int DBLuaManager::del_script(const DoutPrefixProvider * dpp,
                                 optional_yield y, const std::string & key) {
        return -ENOENT;
    }

    int DBLuaManager::add_package(const DoutPrefixProvider * dpp,
                                  optional_yield y,
                                  const std::string & package_name) {
        return -ENOENT;
    }

    int DBLuaManager::remove_package(const DoutPrefixProvider * dpp,
                                     optional_yield y,
                                     const std::string & package_name) {
        return -ENOENT;
    }

    int DBLuaManager::list_packages(const DoutPrefixProvider * dpp,
                                    optional_yield y,
