...
void MDBalancer::handle_conf_change(const std::set < std::string > &changed,
...
void MDBalancer::handle_export_pins(void)
{
    ...
    while (it != q.end()) {
        ...
        if (export_pin >= max_mds) {
            dout(20) << " delay export_pin=" << export_pin << " on " << *in <<
                dendl;
      ...
      for (auto && dir:in->get_dirfrags()) {
            ...
            if (export_pin == MDS_RANK_EPHEMERAL_DIST) {
                ...
                target =
                    mdcache->hash_into_rank_bucket(in->ino(), dir->get_frag());
            }
  ...
  for (auto & cd:authsubs) {
        ...
        if (export_pin == MDS_RANK_EPHEMERAL_DIST) {
            export_pin =
                mdcache->hash_into_rank_bucket(cd->ino(), cd->get_frag());
        }
        else if (export_pin == MDS_RANK_EPHEMERAL_RAND) {
...
void MDBalancer::tick()
{
    ...
    // balance?
    if (mds->get_nodeid() == 0 && mds->is_active()
        && bal_interval > 0
        && chrono::duration_cast < chrono::seconds >
        (now - last_heartbeat).count() >= bal_interval && (num_bal_times
                                                           || (bal_max_until >=
                                                               0
                                                               && mds->
                                                               get_uptime().
...
double mds_load_t::mds_load() const const
{
...
mds_load_t MDBalancer::get_load()
{
    ...
    if (last_get_load != clock::zero() && now > last_get_load) {
        double el =
            std::chrono::duration < double >(now - last_get_load).count();
        if (el >= 1.0) {
            ...
            if (num_traverse > last_num_traverse
                ...
                load.cache_hit_rate =
                    (double)(num_traverse_hit -
                             last_num_traverse_hit) / (num_traverse -
                                                       last_num_traverse);
        }
        else {
...
void MDBalancer::send_heartbeat()
{
    ...
    auto em =
        mds_load.emplace(std::piecewise_construct,
                         std::forward_as_tuple(mds->get_nodeid()),
...
void MDBalancer::queue_split(const CDir * dir, bool fast)
{
    ...
    auto callback =[this, df] (int r){
        if (split_pending.erase(df) == 0) {
                                           ...
                                           auto mdcache = mds->mdcache;
                                           CDir * dir =
                                           mdcache->get_dirfrag(df); if (!dir) {
                                           dout(10) << "drop split on " << df <<
                                           ...
                                           dout(10) << __func__ << " splitting "
                                           << *dir << dendl;
                                           int bits =
                                           g_conf()->mds_bal_split_bits;
                                           if (dir->inode->
                                               is_ephemeral_dist()) {
                                           unsigned min_frag_bits =
                                           mdcache->
                                           get_ephemeral_dist_frag_bits();
                                           if (df.frag.bits() + bits <
                                               min_frag_bits)
                                           bits =
                                           min_frag_bits - df.frag.bits();}
                                           mdcache->split_dir(dir, bits);};
                                           auto ret = split_pending.insert(df);
                                           bool is_new = ret.second; if (fast) {
                                           ...
                                           auto mdcache = mds->mdcache;
                                           CDir * dir =
                                           mdcache->get_dirfrag(frag);
                                           if (!dir) {
                                           dout(10) << "drop merge on " << frag
                                           << " because not in cache" << dendl;
                                           return;}
                                           ceph_assert(dir->dirfrag() == frag);
                                           if (!dir->is_auth()) {
                                           dout(10) << "drop merge on " << *dir
                                           ...
                                           << sibfg << " in cache (have " <<
                                           sibs << ")" << dendl; break;}
          bool all = true; for (auto & sib:sibs)
                                           {
                                           if (!sib->is_auth()
                                               || !sib->should_merge()) {
                                           all = false; break;}
                                           ...
                                           dout(20) << " enqueued dir " << *dir
                                           << dendl; merge_pending.insert(frag);
                                           mds->timer.
                                           add_event_after
                                           (bal_fragment_interval,
                                            new LambdaContext(std::
      ...
      set < mds_rank_t > up_mds; mds->get_mds_map()->get_up_mds_set(up_mds); for (const auto & rank:up_mds)
                                           ...
                                           int cluster_size =
                                           mds->get_mds_map()->get_num_in_mds();
                                           mds_rank_t whoami =
                                           mds->get_nodeid();
                                           rebalance_time = clock::now();
                                           dout(7) << "cluster loads are" <<
                                           dendl;
                                           ...
                                           total_load /
                                           (double)mds->mdsmap->
                                           get_num_mdss_in_rank_mask_bitset();
      ...
      for (const auto &[load, rank]:      load_map)
                                           ...
                                           mds_last_epoch_under_map[it->second];
                                           if (!
  ...
  multimap < double, CDir * >import_pop_map; multimap < mds_rank_t, pair < CDir *, double > >import_from_map; for (auto & dir:mds->mdcache->get_fullauth_subtrees()) {
                                           CInode * diri = dir->get_inode();
                                           if (diri->is_mdsdir())
                                           continue;
  ...
  map < mds_rank_t, double >export_pop_map; for (auto & it:state.targets)
                                           ...
                                           dout(7) << "considering " << *dir <<
                                           " from " << (*p.first).
                                           first << dendl;
                                           auto plast = p.first++;
                                           if (dir->inode->is_base())
                                           continue; ceph_assert(dir->inode->authority().first == target);  // cuz that's how i put it in the map, dummy
                                           if (pop <= amount - have) {
                                           dout(7) << "reexporting " << *dir <<
                                           " pop " << pop << " back to mds." <<
                                           target << dendl;
                                           mds->mdcache->migrator->
                                           ...
                                           for (auto q =
                                                import_pop_map.equal_range(pop);
                                                q.first != q.second;) {
                                           if (q.first->second == dir) {
                                           import_pop_map.erase(q.first); break;}
                                           q.first++;}
                                           }
                                           else {
                                           dout(7) << "can't reexport " << *dir
                                           << ", too big " << pop << dendl;}
  ...
  set < CDir * >already_exporting; for (auto & it:state.targets)
      ...
      for (const auto & dir:              exports) {
                                           dout(5) << "   - exporting " << dir->
                                           pop_auth_subtree << " " << dir->
                                           pop_auth_subtree.meta_load()
                                           << " to mds." << target << " " <<
                                           *dir << dendl;
                                           mds->mdcache->migrator->
                                           export_dir_nicely(dir, target);}
                                           ...
                                           void MDBalancer::find_exports(CDir *
                                                                         dir,
                                                                         double
                                                                         amount,
                                                                         std::
                                           ...
                                           *dir << " need " << need << " (" <<
                                           needmin << " - " << needmax << ")" <<
                                           dendl; double subdir_sum = 0;
                                           for (elist <
                                                CInode * >::iterator it =
                                                dir->pop_lru_subdirs.
                                                begin_use_current();
                                                !it.end();) {
      CInode * in = *it; ++it; ceph_assert(in->is_dir()); ceph_assert(in->get_parent_dir() == dir); auto && dfls = in->get_nested_dirfrags(); size_t num_idle_frags = 0; for (const auto & subdir:dfls)
                                           {
                                           ...
                                           // how popular?
                                           double pop =
                                           subdir->pop_auth_subtree.meta_load();
                                           subdir_sum += pop;
  ...
  for (const auto & dir:                  bigger_rep) {
                                           ...
                                           *dir << dendl;
                                           ...
                                           void MDBalancer::hit_inode(CInode *
                                                                      in,
                                                                      int type)
                                           {
                                           // hit inode
                                           ...
                                           maybe_fragment(CDir * dir,
                                                          bool hot) {
                                           // split/merge
                                           if (bal_fragment_dirs && bal_fragment_interval > 0 && dir->is_auth() && !dir->inode->is_base() &&    // not root/mdsdir (for now at least)
                                           ...
                                           // hit me
                                           double v =
                                           dir->pop_me.get(type).hit(amount);
                                           const bool hot =
                                           (v > g_conf()->mds_bal_split_rd
                                            && type == META_POP_IRD)
                                           || (v > g_conf()->mds_bal_split_wr
                                               && type == META_POP_IWR);
                                           dout(20) << type << " pop is " << v
                                           << ", frag " << dir->get_frag()
                                           << " size " << dir->
                                           get_frag_size() << " " << dir->
                                           pop_me << dendl;
                                           ...
                                           // replicate
                                           double rdp = dir->pop_me.get(META_POP_IRD).get(); rd_adj = rdp / mds->get_mds_map()->get_num_in_mds() - rdp; rd_adj /= 2.0;  // temper somewhat
                                           dout(5) << "replicating dir " << *dir
                                           << " pop " << dir_pop << " .. rdp "
                                           << rdp << " adj " << rd_adj << dendl;
                                           ...
                                           *dir << " pop " << dir_pop << dendl;
                                           ...
                                           dir->pop_nested.get(type).
                                           hit(amount); if (rd_adj != 0.0)
                                           dir->pop_nested.get(META_POP_IRD).
                                           adjust(rd_adj); if (hit_subtree) {
 ...
 */
                                           ...
                                           dir->pop_auth_subtree; while (true) {
                                           ...
                                           if (inc) {
                                           pdir->pop_nested.add(dir->
                                                                pop_nested);
                                           if (adjust_subtree) {
                                           pdir->pop_auth_subtree.add(dir->
                                                                      pop_auth_subtree);
                                           pdir->pop_lru_subdirs.
                                           ...
                                           add(dir->pop_auth_subtree_nested);}
                                           else {
                                           pdir->pop_nested.sub(dir->
                                                                pop_nested);
                                           if (adjust_subtree)
                                           pdir->pop_auth_subtree.sub(dir->
                                                                      pop_auth_subtree);
                                           if (adjust_subtree_nest)
                                           pdir->pop_auth_subtree_nested.
                                           sub(dir->pop_auth_subtree_nested);}
                                           ...
                                           get_inode();
                                           if (!in || !in->is_dir())
          continue; auto && ls = in->get_dirfrags(); for (const auto & subdir:ls)
                                           {

                                           if (subdir->pop_nested.meta_load() <
                                               .001)
                                           continue;
                                           dfs.
                                           push_back(make_pair
  ...
  f->open_object_section("mds_meta_load"); for (auto &[rank, mload]:mds_meta_load)
                                           {
                                           CachedStackStringStream css;
                                           *css << "mds." << rank;
                                           f->dump_float(css->strv(), mload);}
                                           f->close_section();  // mds_meta_load
  f->open_object_section("mds_import_map"); for (auto &[rank, imports]:mds_import_map)
                                           {
                                           {
                                           CachedStackStringStream css;
                                           *css << "mds." << rank;
