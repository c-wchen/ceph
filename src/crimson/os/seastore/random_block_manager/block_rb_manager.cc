...
device_config_t get_rbm_ephemeral_device_config(std::size_t index,
        std::size_t num_devices)
{
    ...
    if (index == 0) {
        ...
        for (std::size_t secondary_index = index + 1;
             secondary_index < num_devices; ++secondary_index) {
            device_id_t secondary_id =
                static_cast < device_id_t >(secondary_index);
            secondary_devices.insert({
                secondary_index, device_spec_t {
                    magic, type, secondary_id}}
                                    );
        }
    } else {                // index > 0
    ...
    device_id_t id =
        static_cast < device_id_t >(DEVICE_ID_RANDOM_BLOCK_MIN + index);
...
paddr_t BlockRBManager::alloc_extent(size_t size)
{
    ...
    paddr_t paddr = convert_abs_addr_to_paddr(extent.get_start(),
...
BlockRBManager::open_ertr::future <> BlockRBManager::open()
{
    ...
    allocator->init(ool_start,
                    device->get_available_size() -
                    ool_start, device->get_block_size());
    return open_ertr::now();
}

BlockRBManager::write_ertr::future <> BlockRBManager::write(paddr_t paddr,
        bufferptr &
        bptr)
{
    ...
    bufferptr bp =
        bufferptr(ceph::buffer::create_page_aligned(bptr.length()));
    bp.copy_in(0, bptr.length(), bptr.c_str());
    return device->write(addr, std::move(bp));
}

BlockRBManager::read_ertr::future <> BlockRBManager::read(paddr_t paddr,
        bufferptr &bptr)
{
...
BlockRBManager::write_ertr::future <> BlockRBManager::
write(rbm_abs_addr addr, bufferlist &bl)
{
    LOG_PREFIX(BlockRBManager::write);
    ceph_assert(device);
    bufferptr bptr;
...
std::ostream &operator<<(std::ostream &out,
                         const rbm_metadata_header_t &header)
{
    out << " rbm_metadata_header_t(size=" << header.
        size << ", block_size=" << header.
        block_size << ", feature=" << header.
        feature << ", journal_size=" << header.
        journal_size << ", crc=" << header.crc << ", config=" << header.
        config;
    return out << ")";
}
