...
struct rgw_sync_symmetric_group {
    ...
    rgw_sync_symmetric_group()
    {
                               ...
                               const std::set < rgw_zone_id > _zones): id(_id),
        zones(_zones)
    {
    }
...
struct rgw_sync_bucket_entity {
    ...
    rgw_sync_bucket_entity(const rgw_zone_id &_zone, std::optional < rgw_bucket > _bucket): zone(_zone),
        bucket(_bucket.
               value_or(rgw_bucket()))
    {
    }
    ...
    static bool match_bucket_id(const std::string &bid1,
                                const std::string &bid2)
    {
...
struct rgw_sync_pipe_filter_tag {
    ...
    rgw_sync_pipe_filter_tag(const std::string &_key,
                             const std::string &_value): key(_key), value(_value)
    {
    }
...
struct rgw_sync_pipe_filter {
    ...
    bool has_tags() const;
    bool check_tag(const std::string &s) const const;
    bool check_tag(const std::string &k, const std::string &v) const const;
    bool check_tags(const std::vector < std::string > &tags) const const;
    bool check_tags(const RGWObjTags::tag_map_t &tags) const const;
};
...
struct rgw_sync_bucket_pipes {
    ...
    } bool contains_zone_bucket(const rgw_zone_id &zone,
...
std::ostream &operator<<(std::ostream &os,
                         const rgw_sync_bucket_entities &e);
std::ostream &operator<<(std::ostream &os,
...
struct rgw_sync_data_flow_group {
    ...
    } bool find_or_create_symmetrical(const std::string &flow_id,
                                      rgw_sync_symmetric_group **flow_group);
    void remove_symmetrical(const std::string &flow_id,
                            std::optional < std::vector < rgw_zone_id > >zones);
    bool find_or_create_directional(const rgw_zone_id &source_zone,
                                    const rgw_zone_id &dest_zone,
                                    rgw_sync_directional_rule **flow_group);
    void remove_directional(const rgw_zone_id &source_zone,
                            const rgw_zone_id &dest_zone);
...
struct rgw_sync_policy_group {
    ...
    bool find_pipe(const std::string &pipe_id, bool create,
