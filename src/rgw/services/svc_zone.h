...
class RGWSI_Zone: public RGWServiceInstance
{
    ...
    std::map < rgw_zone_id,
        std::shared_ptr < RGWBucketSyncPolicyHandler > >sync_policy_handlers;
...
public:
    ...
    std::shared_ptr < RGWBucketSyncPolicyHandler > get_sync_policy_handler(std::optional < rgw_zone_id > zone =
                std::nullopt) const;
    ...
    bool zone_syncs_from(const RGWZone &target_zone,
    ...
    } bool sync_module_exports_data() const
    {
        return exports_data;
    ...
    int select_bucket_placement(const DoutPrefixProvider *dpp,
                                ...
                                optional_yield y);
    int select_legacy_bucket_placement(const DoutPrefixProvider *dpp,
                                       RGWZonePlacementInfo *rule_info,
                                       optional_yield y);
    int select_new_bucket_location(const DoutPrefixProvider *dpp,
                                   const RGWUserInfo &user_info,
                                   const std::string &zonegroup_id,
                                   const rgw_placement_rule &rule,
                                   ...
                                   RGWZonePlacementInfo *rule_info,
                                   optional_yield y);
    int select_bucket_location_by_rule(const DoutPrefixProvider *dpp,
                                       const rgw_placement_rule &location_rule,
                                       RGWZonePlacementInfo *rule_info,
                                       optional_yield y);

    int add_bucket_placement(const DoutPrefixProvider *dpp,
                             const rgw_pool &new_pool, optional_yield y);
    int remove_bucket_placement(const DoutPrefixProvider *dpp,
                                const rgw_pool &old_pool, optional_yield y);
    int list_placement_set(const DoutPrefixProvider *dpp,
                           std::set < rgw_pool > &names, optional_yield y);
    ...
    int list_zonegroups(const DoutPrefixProvider *dpp,
                        std::list < std::string > &zonegroups);
    int list_regions(const DoutPrefixProvider *dpp,
                     std::list < std::string > &regions);
    int list_zones(const DoutPrefixProvider *dpp,
                   std::list < std::string > &zones);
    int list_realms(const DoutPrefixProvider *dpp,
                    std::list < std::string > &realms);
    int list_periods(const DoutPrefixProvider *dpp,
