 ...
 */
         template < typename ImageCtxT = ImageCtx > class ObjectRequest {
          public:
            static ObjectRequest *create_write(ImageCtxT * ictx,
                                               uint64_t object_no,
                                               uint64_t object_off,
                                               ceph::bufferlist
                                               && data, IOContext io_context,
                                               ...
                                               Trace & parent_trace,
                                               Context * completion);
            static ObjectRequest *create_discard(ImageCtxT * ictx,
                                                 uint64_t object_no,
                                                 uint64_t object_off,
                                                 uint64_t object_len,
                                                 ...
                                                 Trace & parent_trace,
                                                 Context * completion);
            static ObjectRequest *create_write_same(ImageCtxT * ictx,
                                                    uint64_t object_no,
                                                    uint64_t object_off,
                                                    uint64_t object_len,
                                                    ceph::bufferlist
            ...
            static ObjectRequest *create_compare_and_write(ImageCtxT * ictx,
             ...
             ObjectRequest(ImageCtxT * ictx, uint64_t objectno,
      ...
      template < typename ImageCtxT = ImageCtx > class ObjectReadRequest:public ObjectRequest < ImageCtxT >
        {
          public:
            static ObjectReadRequest *create(ImageCtxT * ictx,
                                             uint64_t objectno,
                                             ReadExtents * extents,
                                             ...
                                             uint64_t * version,
                                             Context * completion) {
                return new ObjectReadRequest(ictx, objectno, extents,
                                             io_context, op_flags, read_flags,
                                             parent_trace, version, completion);
            }

            ObjectReadRequest(ImageCtxT * ictx, uint64_t objectno,
                              ReadExtents * extents, IOContext io_context,
   ...
   */
      ...
      template < typename ImageCtxT = ImageCtx > class AbstractObjectWriteRequest:public ObjectRequest < ImageCtxT >
        {
          public:
            AbstractObjectWriteRequest(ImageCtxT * ictx, uint64_t object_no,
   ...
   */
      ...
      template < typename ImageCtxT = ImageCtx > class ObjectWriteRequest:public AbstractObjectWriteRequest < ImageCtxT >
        {
          public:
            ObjectWriteRequest(ImageCtxT * ictx, uint64_t object_no,
                               ...
                               const ZTracer::Trace & parent_trace,
                               Context * completion)
            :AbstractObjectWriteRequest < ImageCtxT > (ictx, object_no,
                                                       object_off,
                                                       data.length(),
                                                       io_context, "write",
                                                       parent_trace,
      ...
      template < typename ImageCtxT = ImageCtx > class ObjectDiscardRequest:public AbstractObjectWriteRequest < ImageCtxT >
        {
          public:
            ObjectDiscardRequest(ImageCtxT * ictx, uint64_t object_no,
            ...
            :AbstractObjectWriteRequest < ImageCtxT > (ictx, object_no,
                ...
                if (this->m_full_object) {
                    if ((m_discard_flags &
                         OBJECT_DISCARD_FLAG_DISABLE_CLONE_REMOVE) != 0
                        && this->has_parent()) {
            ...
            const char *get_op_type() const override {
                ...
                case DISCARD_ACTION_REMOVE:
                    return "remove";
                    case DISCARD_ACTION_REMOVE_TRUNCATE:return
          ...
          protected:
            bool is_no_op_for_nonexistent_object()const override {
                ...
                return ((m_discard_flags &
                         OBJECT_DISCARD_FLAG_DISABLE_OBJECT_MAP_UPDATE) == 0);
      ...
      template < typename ImageCtxT = ImageCtx > class ObjectWriteSameRequest:public AbstractObjectWriteRequest < ImageCtxT >
        {
          public:
            ObjectWriteSameRequest(ImageCtxT * ictx, uint64_t object_no,
                                   uint64_t object_off, uint64_t object_len,
                                   ceph::bufferlist
                                   && data, IOContext io_context, int op_flags,
                                   const ZTracer::Trace & parent_trace,
      ...
      template < typename ImageCtxT = ImageCtx > class ObjectCompareAndWriteRequest:public AbstractObjectWriteRequest < ImageCtxT >
        {
          public:
            ObjectCompareAndWriteRequest(ImageCtxT * ictx, uint64_t object_no,
                                         uint64_t object_off, ceph::bufferlist
                                         && cmp_bl, ceph::bufferlist
                                         && write_bl, IOContext io_context,
      ...
      template < typename ImageCtxT = ImageCtx > class ObjectListSnapsRequest:public ObjectRequest < ImageCtxT >
        {
          public:
            static ObjectListSnapsRequest *create(ImageCtxT * ictx,
                                                  uint64_t objectno, Extents
                                                  && object_extents, SnapIds
                                                  && snap_ids,
                ...
                return new ObjectListSnapsRequest(ictx, objectno,
                                                  ...
                                                  std::move(snap_ids),
                                                  list_snaps_flags,
                                                  parent_trace, snapshot_delta,
                                                  completion);
            }

            ObjectListSnapsRequest(ImageCtxT * ictx, uint64_t objectno, Extents
                                   && object_extents, SnapIds
...
extern template class librbd::io::AbstractObjectWriteRequest <
    librbd::ImageCtx >;
...
extern template class librbd::io::ObjectCompareAndWriteRequest <
    librbd::ImageCtx >;
