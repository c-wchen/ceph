...
public:
    ...
    // accessors
    inodeno_t get_base_ino() const
    {
        return base_ino;
    } frag_t get_base_dir_frag() const
    {
        return base_dir_frag;
    } bool get_wanted_base_dir() const
    {
        return wanted_base_dir;
    } bool is_path_locked() const
    {
        return path_locked;
    } snapid_t get_wanted_snapid() const
    {
        return wanted_snapid;
    } bool is_flag_error_dn() const
    {
        return flag_error_dn;
    } bool is_flag_error_dir() const
    {
        return flag_error_dir;
    } const std::string &get_error_dentry() const
    {
        return error_dentry;
    } int get_starts_with() const
    {
        return starts_with;
    } mds_rank_t get_dir_auth_hint() const
    {
        return dir_auth_hint;
    } bool is_unsolicited() const
    {
        return unsolicited;
...
protected:
    MDiscoverReply(): MMDSOp {
        MSG_MDS_DISCOVERREPLY, HEAD_VERSION, COMPAT_VERSION}
    {
    }
    MDiscoverReply(const MDiscover &dis): MMDSOp {
        MSG_MDS_DISCOVERREPLY, HEAD_VERSION, COMPAT_VERSION},
    base_ino(dis.get_base_ino()),
             ...
             unsolicited(false), dir_auth_hint(CDIR_AUTH_UNKNOWN), starts_with(DIR)
    {
...
public:
    ...
    // builders bool is_empty() const {
    return trace.length() == 0 &&
           !flag_error_dn &&
           !flag_error_dir && dir_auth_hint == CDIR_AUTH_UNKNOWN;
}
