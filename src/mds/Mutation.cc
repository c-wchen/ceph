...
bool MutationImpl::is_rdlocked(SimpleLock *lock) const const
{
    ...
    if (lock_cache)
        return static_cast <
               const MutationImpl * >(lock_cache)->is_rdlocked(lock);
    return false;
}

bool MutationImpl::is_wrlocked(SimpleLock *lock) const const
{
    auto it = locks.find(lock);
    ...
    if (lock_cache)
        return static_cast <
               const MutationImpl * >(lock_cache)->is_wrlocked(lock);
    return false;
}
...
void MutationImpl::set_remote_auth_pinned(MDSCacheObject *object,
        mds_rank_t from)
{
    auto &stat = object_states[object];
        ...
        if (_client_request) {
            ...
            if (_peer_request) {
                ...
                f->dump_string("op_type",
                               MMDSPeerRequest::get_opname(_peer_request->
                                       get_op()));
                f->dump_int("lock_type", _peer_request->get_lock_type());
                f->dump_stream("object_info") << _peer_request->
                                              get_object_info();
