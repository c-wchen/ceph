...
public:
    ...
    // write scatter/gather struct OSDWrite {
    vector<ObjectExtent> extents;
    SnapContext snapc;
    bufferlist bl;
    ...
    OSDWrite(const SnapContext &sc, const bufferlist &b,
             ceph::real_time mt, int f, ceph_tid_t _journal_tid)
...
OSDWrite *prepare_write(const SnapContext &sc,
                        const bufferlist &b,
                        ceph::real_time mt,
                        int f, ceph_tid_t journal_tid) const
{
    return new OSDWrite(sc, b, mt, f, journal_tid);
}
// ******* BufferHead ********* class BufferHead:public LRUObject {
public:
// states
...
// cons
explicit BufferHead(Object *o) : state(STATE_MISSING),
                                 ref(0),
                                 dontneed(false),
                                 nocache(false),
                                 ob(o),
                                 last_write_tid(0), last_read_tid(0), journal_tid(0), error(0)
{
    ex.start = ex.length = 0;
}
// extent loff_t start() const {
return ex.start;
}
...
public:
    ...
    Object(ObjectCacher *_oc, sobject_t o, uint64_t ono, ObjectSet *os,
           object_locator_t &l, uint64_t ts, uint64_t tq) : ref(0),
                                                            oc(_oc),
    ...
    ObjectSet *get_object_set() const
    {
        return oset;
    }
    ...
    const object_locator_t &get_oloc() const
    {
        return oloc;
    }
    ...
    // mid-level BufferHead *split(BufferHead * bh, loff_t off);
...
loff_t get_stat_tx() const
{
    return stat_tx;
}
loff_t get_stat_rx() const
{
    return stat_rx;
}
loff_t get_stat_dirty() const
{
    return stat_dirty;
}
loff_t get_stat_clean() const
{
    return stat_clean;
}
loff_t get_stat_zero() const
{
    return stat_zero;
}
loff_t get_stat_dirty_waiting() const
{
    return stat_dirty_waiting;
}
size_t get_stat_nr_dirty_waiters() const
{
...
ObjectCacher(CephContext *cct_, string name, WritebackHandler &wb,
             Mutex &l, flush_set_callback_t flush_callback,
             void *flush_callback_arg, uint64_t max_bytes,
             uint64_t max_objects, uint64_t max_dirty,
             uint64_t target_dirty, double max_age,
             bool block_writes_upfront);
~ObjectCacher();
...
inline ostream &operator<<(ostream &out, const ObjectCacher::BufferHead &bh)
{
    ...
    out << "]";
    out << " waiters = {";
    for (map<loff_t, list<Context *>>::const_iterator it = bh.waitfor_read.begin(); it != bh.waitfor_read.end(); ++it)
    {
        out << " " << it->first << "->[";
