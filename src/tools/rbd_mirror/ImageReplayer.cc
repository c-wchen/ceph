...
namespace rbd {
    namespace mirror {
        ...
        namespace {
            ...
            template < typename I >
                ...
                void handle_complete(int r) override {
                    std::stringstream ss;
                    if (r < 0) {
                        ss << "replay completed with error: " <<
                            cpp_strerror(r);
                    }
          ...
          template < typename I > class StatusCommand:public ImageReplayerAdminSocketCommand < I >
            {
              public:
                explicit StatusCommand(const std::string & desc,
                                       ImageReplayer < I > *replayer)
          ...
          template < typename I > class StartCommand:public ImageReplayerAdminSocketCommand < I >
            {
              public:
                explicit StartCommand(const std::string & desc,
                                      ImageReplayer < I > *replayer)
              :    ImageReplayerAdminSocketCommand < I > (desc,
                                                           replayer)
                {
                }
          ...
          template < typename I > class StopCommand:public ImageReplayerAdminSocketCommand < I >
            {
              public:
                explicit StopCommand(const std::string & desc,
                                     ImageReplayer < I > *replayer)
              :    ImageReplayerAdminSocketCommand < I > (desc,
                                                           replayer)
                {
          ...
          template < typename I > class RestartCommand:public ImageReplayerAdminSocketCommand < I >
            {
              public:
                explicit RestartCommand(const std::string & desc,
                                        ImageReplayer < I > *replayer)
              :    ImageReplayerAdminSocketCommand < I > (desc,
                                                           replayer)
                {
          ...
          template < typename I > class FlushCommand:public ImageReplayerAdminSocketCommand < I >
            {
              public:
                explicit FlushCommand(const std::string & desc,
                                      ImageReplayer < I > *replayer)
              :    ImageReplayerAdminSocketCommand < I > (desc,
                                                           replayer)
                {
          ...
          template < typename I > class ImageReplayerAdminSocketHook:public AdminSocketHook
            {
              public:
                ImageReplayerAdminSocketHook(CephContext * cct,
              ...
              :    admin_socket(cct->get_admin_socket()),
                ...
                "rbd mirror flush " + name,
                            new FlushCommand < I >
                            ("flush rbd mirror " + name, replayer)}, {
                "rbd mirror restart " + name,
                        new RestartCommand < I >
                        ("restart rbd mirror " + name, replayer)}, {
                "rbd mirror start " + name,
                        new StartCommand < I > ("start rbd mirror " + name,
                                                    replayer)}, {
                "rbd mirror status " + name,
                        new StatusCommand < I >
                        ("get status for rbd mirror " + name, replayer)}, {
                "rbd mirror stop " + name,
                        new StopCommand < I > ("stop rbd mirror " + name,
                                                   replayer)}} {
                }
                ...
                int register_commands() {
                  for (auto & it:commands) {
                        int r =
                            admin_socket->register_command(it.first, it.first,
                                                           this,
                                                           it.second->desc);
                        if (r < 0) {
                            return r;
              ...
              private:
                typedef std::map < std::string,
                    ImageReplayerAdminSocketCommand < I > *>Commands;
        ...
        template < typename I >
            void ImageReplayer <
            I >::BootstrapProgressContext::update_progress(const std::
                                                           string & description,
                                                           bool flush) {
        ...
        template < typename I >
            void ImageReplayer <
            I >::RemoteJournalerListener::handle_update(::journal::
                                                        JournalMetadata *) {
        ...
        template < typename I >
            ImageReplayer < I >::ImageReplayer(Threads < I > *threads,
                                               ...
                                               *instance_watcher,
                                               ...
                                               const std::
                                               string &
                                               global_image_id):m_threads
            (threads), m_image_deleter(image_deleter),
            m_instance_watcher(instance_watcher), m_local(local),
            m_local_mirror_uuid(local_mirror_uuid),
            m_local_pool_id(local_pool_id), m_global_image_id(global_image_id),
            m_local_image_name(global_image_id),
            m_lock("rbd::mirror::ImageReplayer " + stringify(local_pool_id) +
                   " " + global_image_id), m_progress_cxt(this),
        ...
        template < typename I >
            image_replayer::HealthState ImageReplayer <
            I >::get_health_state()const {
        ...
        template < typename I >
            void ImageReplayer < I >::set_state_description(int r,
                                                            const std::
                                                            string & desc) {
        ...
        template < typename I >
            ...
            int r = 0;
            {
                ...
                if (!is_stopped_()) {
                    ...
                    dout(5) <<
                        "stopped manually, ignoring start without manual flag"
                        << dendl;
                    r = -EPERM;
                }
                else {
        ...
        template < typename I > void ImageReplayer < I >::wait_for_deletion() {
            dout(20) << dendl;

            Context *ctx = create_context_callback <
                ImageReplayer,
                &ImageReplayer < I >::handle_wait_for_deletion > (this);
            m_image_deleter->wait_for_scheduled_deletion(m_local_pool_id,
        ...
        template < typename I > void ImageReplayer < I >::prepare_local_image() {
            dout(20) << dendl;

            m_local_image_id = "";
            Context *ctx = create_context_callback <
                ImageReplayer,
                &ImageReplayer < I >::handle_prepare_local_image > (this);
            auto req =
                PrepareLocalImageRequest < I >::create(m_local_ioctx,
                                                       m_global_image_id,
                                                       &m_local_image_id,
                                                       &m_local_image_name,
        ...
        template < typename I > void ImageReplayer < I >::prepare_remote_image() {
            ...
            *m_peers.begin()};
            ...
            Context *ctx = create_context_callback <
                ImageReplayer,
                &ImageReplayer < I >::handle_prepare_remote_image > (this);
            auto req =
                PrepareRemoteImageRequest < I >::create(m_threads,
        ...
        template < typename I >
            ...
            assert(r < 0 ? m_remote_journaler == nullptr : m_remote_journaler !=
                   nullptr);
        ...
        template < typename I > void ImageReplayer < I >::bootstrap() {
            ...
            BootstrapRequest < I > *request =
                BootstrapRequest < I >::create(m_local_ioctx,
                                               m_remote_image.io_ctx,
                                               m_instance_watcher,
                                               &m_local_image_ctx,
                                               m_local_image_id,
                                               ...
                                               m_local_mirror_uuid,
                                               m_remote_image.mirror_uuid,
                                               m_remote_journaler,
                                               &m_client_state, &m_client_meta,
                                               ctx, &m_resync_requested,
                                               &m_progress_cxt);
        ...
        template < typename I >
            ...
            Context *ctx = create_context_callback <
                ImageReplayer,
                &ImageReplayer < I >::handle_init_remote_journaler > (this);
        ...
        template < typename I >
            ...
            if (r < 0) {
                derr << "failed to initialize remote journal: " <<
                    cpp_strerror(r) << dendl;
            ...
            r = m_remote_journaler->get_cached_client(m_local_mirror_uuid,
                                                      &client);
            if (r < 0) {
                derr << "error retrieving remote journal client: " <<
                    cpp_strerror(r)
                    << dendl;
            ...
            if (m_client_meta.image_id == m_local_image_id &&
                ...
                dout(5) << "client flagged disconnected, stopping image replay"
                    << dendl;
        ...
        template < typename I > void ImageReplayer < I >::start_replay() {
            ...
            Context *start_ctx = create_context_callback <
                ImageReplayer,
                &ImageReplayer < I >::handle_start_replay > (this);
        ...
        template < typename I >
            ...
            m_replay_status_formatter =
                ReplayStatusFormatter < I >::create(m_remote_journaler,
                                                    m_local_mirror_uuid);
            ...
            m_event_preprocessor =
                EventPreprocessor < I >::create(*m_local_image_ctx,
                                                *m_remote_journaler,
                                                m_local_mirror_uuid,
                                                &m_client_meta,
                                                m_threads->work_queue);
            ...
            {
                CephContext *cct =
                    static_cast < CephContext * >(m_local->cct());
                double poll_seconds =
                    cct->_conf->get_val <
                ...
                dout(20) << "m_remote_journaler=" << *m_remote_journaler <<
                    dendl;
            }
        ...
        template < typename I >
            void ImageReplayer < I >::on_start_fail(int r,
            ...
            Context *ctx = new FunctionContext([this, r, desc] (int _r){
                                               ...
                                               *bootstrap_request = nullptr;
                                               ...
                                               dout(20) << "canceling start" <<
                                               dendl; if (m_bootstrap_request) {
                                               bootstrap_request =
                                               m_bootstrap_request;
                                               bootstrap_request->get();}
                                               }
                                               else {
                                               dout(20) << "interrupting replay"
                                               << dendl;
                                               shut_down_replay = true;}

                                               assert(m_on_stop_finish ==
                                                      nullptr);
                                               std::swap(m_on_stop_finish,
                                                         on_finish);
                                               ...
                                               m_event_replay_tracker.
                                               start_op(); m_lock.Lock();
                                               bool stopping =
                                               (m_state == STATE_STOPPING);
                                               m_lock.Unlock(); if (stopping) {
                                               dout(10) <<
                                               "stopping event replay" << dendl;
                                               m_event_replay_tracker.
                                               finish_op(); return;}

                                               if (m_replay_tag_valid
                                                   && m_replay_tag.tid ==
                                                   m_replay_tag_tid) {
                                               preprocess_entry(); return;}
                                               ...
                                               template < typename I >
                                               void ImageReplayer <
                                               I >::flush(Context * on_finish) {
                                               dout(20) << "enter" << dendl; {
                                               Mutex::Locker locker(m_lock);
                                               if (m_state == STATE_REPLAYING) {
                                               Context * ctx =
                                               new
                                               FunctionContext([on_finish]
                                                               (int r) {
                                                               if (on_finish !=
                                                                   nullptr) {
                                                               on_finish->
                                               ...
                                               FunctionContext([this,
                                                                on_flush] (int
                                                                           r) {
                                                               on_flush_local_replay_flush_finish
                                                               (on_flush, r);});
                                               ...
                                               I >::
                                               on_flush_local_replay_flush_finish
                                               (Context * on_flush, int r) {
                                               dout(20) << "r=" << r << dendl;
                                               if (r < 0) {
                                               derr <<
                                               "error flushing local replay: "
                                               << cpp_strerror(r) << dendl;
                                               on_flush->complete(r); return;}
                                               ...
                                               FunctionContext([this,
                                                                on_flush] (int
                                                                           r) {
                                                               on_flush_flush_commit_position_finish
                                                               (on_flush, r);});
                                               m_remote_journaler->
                                               ...
                                               update_mirror_image_status(false,
                                                                          boost::
                                                                          none);
                                               dout(20) << "flush complete, r="
                                               << r << dendl;
                                               on_flush->complete(r);}

                                               template < typename I >
                                               bool ImageReplayer <
                                               I >::on_replay_interrupted() {
                                               bool shut_down; {
                                               Mutex::Locker locker(m_lock);
                                               ...
                                               template < typename I >
                                               void ImageReplayer <
                                               I >::print_status(Formatter * f,
                                                                 stringstream *
                                                                 ss) {
                                               dout(20) << "enter" << dendl;
                                               Mutex::Locker l(m_lock); if (f) {
                                               f->
                                               open_object_section
                                               ("image_replayer");
                                               ...
                                               *ss << m_name << ": state: " <<
                                               ...
                                               if (r < 0) {
                                               derr <<
                                               "replay encountered an error: "
                                               << cpp_strerror(r) << dendl;
                                               set_state_description(r,
                                                                     error_desc);}
                                               ...
                                               Mutex::Locker locker(m_lock);
                                               if (m_state != STATE_REPLAYING) {
                                               dout(20) << "replay interrupted"
                                               << dendl; interrupted = true;}
                                               else {
                                               ...
                                               m_local_replay->shut_down(false,
                                                                         ctx);}

                                               template < typename I >
                                               void ImageReplayer <
                                               I >::handle_replay_flush(int r) {
                                               dout(20) << "r=" << r << dendl; {
                                               Mutex::Locker locker(m_lock);
                                               assert(m_state ==
                                               ...
                                               I >::get_remote_tag() {
                                               dout(20) << "tag_tid: " <<
                                               m_replay_tag_tid << dendl;
                                               Context * ctx =
                                               create_context_callback <
                                               ImageReplayer,
                                               &ImageReplayer <
                                               I >::handle_get_remote_tag >
                                               (this);
                                               m_remote_journaler->
                                               ...
                                               template < typename I >
                                               void ImageReplayer <
                                               I >::allocate_local_tag() {
                                               dout(15) << dendl;
                                               std::string mirror_uuid =
                                               m_replay_tag_data.mirror_uuid;
                                               if (mirror_uuid ==
                                                   librbd::Journal <>::
                                                   LOCAL_MIRROR_UUID) {
                                               mirror_uuid =
                                               ...
                                               auto local_tag_data =
                                               m_local_journal->get_tag_data();
                                               if (local_tag_data.mirror_uuid ==
                                                   librbd::Journal <>::
                                                   ORPHAN_MIRROR_UUID
                                                   && (local_tag_data.
                                                       predecessor.commit_valid
                                                       && local_tag_data.
                                                       predecessor.
                                                       mirror_uuid ==
                                                       librbd::Journal <>::
                                                       LOCAL_MIRROR_UUID)) {
                                               dout(15) <<
                                               "skipping stale demotion event"
                                               << dendl;
                                               handle_process_entry_safe
                                               (m_replay_entry, 0);
                                               handle_replay_ready(); return;}
                                               else {
                                               dout(5) <<
                                               "encountered image demotion: stopping"
                                               << dendl;
                                               Mutex::Locker locker(m_lock);
                                               m_stop_requested = true;}
                                               }
                                               ...
                                               predecessor(m_replay_tag_data.
                                                           predecessor);
                                               if (predecessor.mirror_uuid ==
                                                   librbd::Journal <>::
                                                   LOCAL_MIRROR_UUID) {
                                               predecessor.mirror_uuid =
                                               m_remote_image.mirror_uuid;}
                                               else
                                               if (predecessor.mirror_uuid ==
                                                   m_local_mirror_uuid) {
                                               ...
                                               m_replay_entry.get_commit_tid()
                                               << dendl;
                                               bufferlist data =
                                               m_replay_entry.get_data();
                                               bufferlist::iterator it =
                                               data.begin();
                                               int r =
                                               m_local_replay->decode(&it,
                                                                      &m_event_entry);
                                               if (r < 0) {
                                               derr <<
                                               ...
                                               dout(20) << "delaying replay by "
                                               << delay << " sec" << dendl;
                                               Mutex::
                                               Locker timer_locker(m_threads->
                                                                   timer_lock);
                                               assert(m_delayed_preprocess_task
                                                      == nullptr);
                                               m_delayed_preprocess_task =
                                               new
                                               FunctionContext([this] (int r) {
                                                               assert
                                                               (m_threads->
                                                                timer_lock.
                                                                is_locked());
                                               ...
                                               void ImageReplayer <
                                               I >::
                                               handle_preprocess_entry_ready(int
                                                                             r)
                                               {
                                               dout(20) << "r=" << r << dendl;
                                               assert(r == 0);
                                               if (!m_event_preprocessor->
                                                   is_required(m_event_entry)) {
                                               process_entry(); return;}
                                               ...
                                               (this);
                                               m_event_preprocessor->
                                               preprocess(&m_event_entry, ctx);}

                                               template < typename I >
                                               void ImageReplayer <
                                               I >::
                                               handle_preprocess_entry_safe(int
                                                                            r) {
                                               dout(20) << "r=" << r << dendl;
                                               if (r < 0) {
                                               m_event_replay_tracker.
                                               ...
                                               ImageReplayer,
                                               &ImageReplayer <
                                               I >::handle_process_entry_ready >
                                               (this);
                                               Context * on_commit =
                                               new C_ReplayCommitted(this,
                                                                     std::
                                                                     move
                                                                     (m_replay_entry));
                                               m_local_replay->
                                               process(m_event_entry, on_ready,
                                                       on_commit);}
                                               ...
                                               void ImageReplayer <
                                               I >::
                                               handle_process_entry_safe(const
                                                                         ReplayEntry
                                                                         &
                                                                         replay_entry,
                                                                         int r)
                                               ...
                                               bool ImageReplayer <
                                               I >::
                                               update_mirror_image_status(bool
                                                                          force,
                                                                          const
                                                                          OptionalState
                                                                          &
                                               ...
                                               Mutex::Locker locker(m_lock);
                                               if
                                               (!start_mirror_image_status_update
                                                (force, false)) {
                                               return false;}
                                               }

                                               queue_mirror_image_status_update
                                               (state); return true;}
                                               ...
                                               I >::
                                               start_mirror_image_status_update
                                               (bool force, bool restarting) {
                                               assert(m_lock.is_locked());
                                               if (!force && !is_stopped_()) {
                                               if (!is_running_()) {
                                               dout(20) <<
                                               "shut down in-progress: ignoring update"
                                               << dendl; return false;}
                                               else
                                               if (m_in_flight_status_updates >
                                                   (restarting ? 1 : 0)) {
                                               dout(20) <<
                                               "already sending update" <<
                                               dendl;
                                               ...
                                               Context * on_finish = nullptr; {
                                               Mutex::Locker locker(m_lock);
                                               assert(m_in_flight_status_updates
                                                      > 0);
                                               if (--m_in_flight_status_updates
                                                   > 0) {
                                               dout(20) << "waiting on " <<
                                               ...
                                               void ImageReplayer <
                                               I >::
                                               send_mirror_status_update(const
                                                                         OptionalState
                                                                         &
                                                                         opt_state)
                                               {
                                               ...
                                               int last_r; bool stopping_replay;
...
case STATE_STARTING:
                                               ...
                                               status.description =
                                               "starting replay";}
break; case STATE_REPLAYING:
case STATE_REPLAY_FLUSHING:
                                               status.state =
                                               cls::rbd::
                                               MIRROR_IMAGE_STATUS_STATE_REPLAYING;
                                               {
                                               Context * on_req_finish =
                                               new
                                               FunctionContext([this] (int r) {
                                                               dout(20) <<
                                                               "replay status ready: r="
                                                               << r << dendl;
                                                               if (r >= 0) {
                                                               send_mirror_status_update
                                                               (boost::none);}
                                               ...
                                               std::string desc;
                                               if (!m_replay_status_formatter->
                                                   get_or_send_update(&desc,
                                                                      on_req_finish))
                                               {
                                               dout(20) <<
                                               "waiting for replay status" <<
                                               dendl; return;}
                                               status.description =
                                               "replaying, " + desc;
                                               mirror_image_status_state =
                                               boost::none;}
break; case STATE_STOPPING:
                                               if (stopping_replay) {
                                               status.state =
                                               cls::rbd::
...
break; default:
                                               ...
                                               *mirror_image_status_state;}
                                               ...
                                               dout(20) << "status=" << status
                                               << dendl;
                                               librados::
                                               ObjectWriteOperation op;
                                               librbd::cls_client::
                                               mirror_image_status_set(&op,
                                                                       m_global_image_id,
                                                                       status);
                                               ...
                                               void ImageReplayer <
                                               I >::
                                               handle_mirror_status_update(int
                                                                           r) {
                                               dout(20) << "r=" << r << dendl;
                                               bool running = false;
                                               bool started = false; {
                                               Mutex::Locker locker(m_lock);
                                               bool update_status_requested =
                                               false;
                                               std::
                                               swap(update_status_requested,
                                               ...
                                               void ImageReplayer <
                                               I >::
                                               reschedule_update_status_task(int
                                                                             new_interval)
                                               {
                                               dout(20) << dendl;
                                               ...
                                               if (m_on_update_status_finish ==
                                                   nullptr) {
                                               dout(20) <<
                                               "waiting for in-flight status update"
                                               << dendl;
                                               m_on_update_status_finish =
                                               new
                                               FunctionContext([this,
                                               ...
                                               FunctionContext([this,
                                                               ...
                                                               if (r < 0) {
                                                               derr <<
                                                               "error shutting down journal replay: "
                                                               <<
                                                               cpp_strerror(r)
                                                               << dendl;}
                                               ...
                                               template < typename I >
                                               void ImageReplayer <
                                               I >::handle_shut_down(int r) {
                                               reschedule_update_status_task
                                               (-1);
                                               bool unregister_asok_hook =
                                               false; {
                                               Mutex::Locker locker(m_lock);
                                               // if status updates are in-flight, wait for them to complete
                                               // before proceeding
                                               ...
                                               if (m_delete_requested
                                                   && !m_local_image_id.
                                                   empty()) {
                                               assert(m_remote_image.image_id.
                                                      empty());
                                               dout(0) <<
                                               ...
                                               dout(20) << "stop complete" <<
                                               dendl; m_local_ioctx.close();
                                               ...
                                               if (on_start != nullptr) {
                                               dout(20) <<
                                               "on start finish complete, r=" <<
                                               r << dendl;
                                               on_start->complete(r); r = 0;}
                                               if (on_stop != nullptr) {
                                               dout(20) <<
                                               "on stop finish complete, r=" <<
                                               r << dendl;
                                               on_stop->complete(r);}
                                               }
                                               ...
                                               dout(0) <<
                                               "client flagged disconnected, stopping image replay"
                                               << dendl;
                                               stop(nullptr, false, -ENOTCONN,
                                                    "disconnected");}
                                               }

                                               template < typename I >
                                               std::string ImageReplayer <
                                               I >::
                                               to_string(const State state) {
                                               switch (state) {
case ImageReplayer < I >::STATE_STARTING:
return "Starting"; case ImageReplayer < I >::STATE_REPLAYING:
return "Replaying"; case ImageReplayer < I >::STATE_REPLAY_FLUSHING:
return "ReplayFlushing"; case ImageReplayer < I >::STATE_STOPPING:
return "Stopping"; case ImageReplayer < I >::STATE_STOPPED:
return "Stopped"; default:
                                               ...
                                               dout(15) <<
                                               "registered asok hook: " <<
                                               m_name << dendl;
                                               asok_hook =
                                               new ImageReplayerAdminSocketHook
                                               < I > (g_ceph_context, m_name,
                                                      this);
                                               int r =
                                               asok_hook->register_commands();
                                               if (r == 0) {
                                               m_asok_hook = asok_hook; return;}
                                               derr <<
                                               "error registering admin socket commands"
                                               << dendl;}
                                               delete asok_hook;}
