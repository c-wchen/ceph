...
public:
    ...
    struct PriorSet {
        ...
        template < typename F > PriorSet(const PastIntervals &past_intervals,
                                         bool ec_pool,
...
template < typename F > PastIntervals::PriorSet::PriorSet(const PastIntervals &past_intervals,
        bool ec_pool,
        epoch_t last_epoch_started,
        const IsPGRecoverablePredicate *c,
        F f,
...
{
public:
    virtual const std::map < hobject_t, pg_missing_item > &get_items() const const = 0;
...
public:
    ...
    std::vector
    < std::pair < uint64_t, uint64_t > >buffer_extents; // off -> len.  extents in buffer being mapped (may be fragmented bc of striping!)
