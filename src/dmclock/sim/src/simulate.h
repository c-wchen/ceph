...
namespace crimson {
    namespace qos_simulation {
        ...
        template < typename ServerId, typename ClientId, typename TS,
            typename TC > class Simulation {
          ...
          public:
            ...
            using ServerDataOutF = std::function < void (std::ostream & out,
                                                         Simulation * sim,
                                                         ServerFilter,
                                                         int header_w,
                                                         int data_w,
            ...
            using ClientDataOutF = std::function < void (std::ostream & out,
                                                         Simulation * sim,
                                                         ClientFilter,
                                                         int header_w,
                                                         int data_w,
          ...
          Simulation():
            early_time(now()),
                prng(std::chrono::system_clock::now().time_since_epoch().
                     count()) {
                // empty
            }
            ...
            uint get_client_count() const {
                return client_count;
            } uint get_server_count() const {
                return server_count;
            } TC & get_client(ClientId id) {
                return *clients[id];
            }
            TS & get_server(ServerId id) {
                return *servers[id];
            }
            const ServerId & get_server_id(uint index) const {
                return server_ids[index];
            } void add_servers(uint count,
                               std::function < TS * (ServerId) >
                               create_server_f) {
                uint i = server_count;
              ...
              for (auto const &i:clients) {
                ...
                std::cout << "simulation completed in " <<
                    ...
                    (clients_finished_time -
                     servers_created_time).count() << " millisecs" << std::endl;
            ...
            void display_stats(std::ostream & out,
                               ...
                               ,
                               int head_w = 12, int data_w = 7, int data_prec =
                               2) {
                assert(has_run);
              ...
              for (auto const &c:clients) {
                ...
                double ops_factor =
                    ...
                    double >>(measure_unit) / std::chrono::duration_cast <
                    std::chrono::duration < double >>(report_unit);
...

                {
                    ...
                    do {
                      ...
                      for (auto const &c:clients) {
                            ...
                            out << " " << std::setw(data_w) << std::
                                setprecision(data_prec) << std::fixed << data;
                        }
                        out << " " << std::setw(data_w) << std::
                            setprecision(data_prec) << std::
                            fixed << total << std::endl;
                        ++i;
                    } while (has_data);
                }

                client_out_f(out, this, client_filter, head_w, data_w,
                             data_prec);
              ...
              for (auto const &s:servers) {
                ...
                server_out_f(out, this, server_filter, head_w, data_w,
                             data_prec);
            ...
            template < typename T >
                ...
                out << "total time to add requests: " <<
                    std::fixed << add_request_time.
                    count() << " " << time_unit << ";" << std::
                    endl << "    count: " << add_request_count << ";" << std::
                    endl << "    average: " << add_request_time_per_unit << " "
                    << time_unit << " per request/response" << std::endl;

                double request_complete_time_unit =
                    double (request_complete_time.count()) /
                    request_complete_count;
                out << "total time to note requests complete: " << std::
                    fixed << request_complete_time.
                    count() << " " << time_unit << ";" << std::
                    endl << "    count: " << request_complete_count << ";" <<
                    std::
                    endl << "    average: " << request_complete_time_unit << " "
                    << time_unit << " per request/response" << std::endl;
            ...
            template < typename T >
                ...
                out << "total time to track responses: " <<
                    std::fixed << track_resp_time.
                    count() << " " << time_unit << ";" << std::
                    endl << "    count: " << track_resp_count << ";" << std::
                    endl << "    average: " << track_resp_time_unit << " " <<
                    time_unit << " per request/response" << std::endl;
                ...
                out << "total time to get request parameters: " <<
                    std::fixed << get_req_params_time.
                    count() << " " << time_unit << ";" << std::
                    endl << "    count: " << get_req_params_count << ";" <<
                    std::
                    endl << "    average: " << get_req_params_time_unit << " "
                    << time_unit << " per request/response" << std::endl;
            ...
            const ServerId & server_select_random(uint64_t seed,
                                                  uint16_t client_idx) {
