...
class C_Prepare:public MDSLogContextBase {
    ...
    C_Prepare(MDSTableServer * s, const cref_t < MMDSTableRequest > r,
              version_t v):server(s), req(r), tid(v) {
    }
...
void MDSTableServer::handle_prepare(const cref_t < MMDSTableRequest > &req)
{
    ...
    ETableServer *le =
        new ETableServer(table, TABLESERVER_OP_PREPARE, req->reqid, from,
...
void MDSTableServer::_prepare_logged(const cref_t < MMDSTableRequest > &req,
                                     version_t tid)
{
    ...
    auto reply =
        make_message < MMDSTableRequest > (table, TABLESERVER_OP_AGREE,
                                           req->reqid, tid);
...
void MDSTableServer::handle_notify_ack(const cref_t < MMDSTableRequest > &m)
{
    ...
    if (p != pending_notifies.end()) {
        if (p->second.notify_ack_gather.erase(from)) {
            ...
            dout(0) << "got unexpected notify ack for tid " << tid <<
                " from mds." << from << dendl;
        }
    }
    else {
...
class C_Commit:public MDSLogContextBase {
    ...
    C_Commit(MDSTableServer * s,
             const cref_t < MMDSTableRequest > &r):server(s), req(r) {
    }
    void finish(int r) override {
    ...
    if (pending_for_mds.count(tid)) {
        ...
        mds->mdlog->
            start_submit_entry(new
                               ETableServer(table, TABLESERVER_OP_COMMIT, 0,
                                            MDS_RANK_NONE, tid,
                                            projected_version),
        ...
        auto reply =
            make_message < MMDSTableRequest > (table, TABLESERVER_OP_ACK,
                                               req->reqid, tid);
...
void MDSTableServer::handle_rollback(const cref_t < MMDSTableRequest > &req)
{
    ...
    mds->mdlog->
        start_submit_entry(new
                           ETableServer(table, TABLESERVER_OP_ROLLBACK, 0,
                                        MDS_RANK_NONE, tid, projected_version),
...
void MDSTableServer::do_server_update(bufferlist & bl)
{
    ...
    ETableServer *le =
        new ETableServer(table, TABLESERVER_OP_SERVER_UPDATE, 0, MDS_RANK_NONE,
                         0, projected_version);
    mds->mdlog->start_entry(le);
    le->mutation = bl;
...
void MDSTableServer::_do_server_recovery()
{
  ...
  for (auto p:active_clients) {
        auto reply =
            make_message < MMDSTableRequest > (table,
...
void MDSTableServer::handle_mds_recovery(mds_rank_t who)
{
    ...
    auto reply =
        make_message < MMDSTableRequest > (table, TABLESERVER_OP_SERVER_READY,
                                           next_reqid);
    mds->send_message_mds(reply, who);
}
