...
extern void rgw_get_buckets_obj(const rgw_user & user_id,
...
class RGWBucketEntryMetadataObject:public RGWMetadataObject {
  ...
  public:
     RGWBucketEntryMetadataObject(RGWBucketEntryPoint & _ep,
                                  const obj_version & v, real_time m):ep(_ep) {
        objv = v;
        mtime = m;
        set_pattrs(&attrs);
    } RGWBucketEntryMetadataObject(RGWBucketEntryPoint & _ep,
...
class RGWBucketInstanceMetadataObject:public RGWMetadataObject {
  ...
  public:
    ...
    } RGWBucketInstanceMetadataObject(RGWBucketCompleteInfo & i,
                                      const obj_version & v,
                                      real_time m):info(i) {
        objv = v;
        mtime = m;
...
class RGWUserBuckets {
   ...
   */
    std::map < std::string, RGWBucketEnt > &get_buckets() {
        return buckets;
    }
...
class RGWBucketInstanceMetadataHandlerBase:public
...
class RGWBucketInstanceMetaHandlerAllocator {
  public:
    static RGWBucketInstanceMetadataHandlerBase *alloc(rgw::sal::Driver *
                                                       driver);
};
...
extern int rgw_object_get_attr(rgw::sal::Driver * driver,
                               rgw::sal::Object * obj, const char *attr_name,
                               bufferlist & out_bl, optional_yield y);

extern void check_bad_user_bucket_mapping(rgw::sal::Driver * driver,
...
struct RGWBucketAdminOpState {
    ...
    std::string & get_tenant() {
        return uid.tenant;
    }

    rgw::sal::Bucket * get_bucket() {
        return bucket.get();
    }
    ...
    const std::string & get_bucket_id() {
        return bucket_id;
    }
...
class RGWBucket {
  ...
  public:
    ...
    } int init(rgw::sal::Driver * storage, RGWBucketAdminOpState & op_state,
...
class RGWBucketAdminOp {
  public:
    static int get_policy(rgw::sal::Driver * driver,
                          RGWBucketAdminOpState & op_state,
                          RGWFormatterFlusher & flusher,
                          const DoutPrefixProvider * dpp);
    static int get_policy(rgw::sal::Driver * driver,
                          ...
                          const DoutPrefixProvider * dpp);
    static int dump_s3_policy(rgw::sal::Driver * driver,
                              RGWBucketAdminOpState & op_state,
                              std::ostream & os,
                              const DoutPrefixProvider * dpp);

    static int unlink(rgw::sal::Driver * driver,
                      RGWBucketAdminOpState & op_state,
                      const DoutPrefixProvider * dpp);
    static int link(rgw::sal::Driver * driver, RGWBucketAdminOpState & op_state,
                    const DoutPrefixProvider * dpp, std::string * err_msg =
                    NULL);
    static int chown(rgw::sal::Driver * driver,
                     RGWBucketAdminOpState & op_state,
                     const std::string & marker, const DoutPrefixProvider * dpp,
                     std::string * err_msg = NULL);

    static int check_index(rgw::sal::Driver * driver,
                           RGWBucketAdminOpState & op_state,
                           RGWFormatterFlusher & flusher, optional_yield y,
                           const DoutPrefixProvider * dpp);

    static int remove_bucket(rgw::sal::Driver * driver,
                             RGWBucketAdminOpState & op_state, optional_yield y,
                             const DoutPrefixProvider * dpp, bool bypass_gc =
                             false, bool keep_index_consistent = true);
    static int remove_object(rgw::sal::Driver * driver,
                             RGWBucketAdminOpState & op_state,
                             const DoutPrefixProvider * dpp);
    static int info(rgw::sal::Driver * driver, RGWBucketAdminOpState & op_state,
                    RGWFormatterFlusher & flusher, optional_yield y,
                    const DoutPrefixProvider * dpp);
    static int limit_check(rgw::sal::Driver * driver,
                           RGWBucketAdminOpState & op_state,
                           const std::list < std::string > &user_ids,
                           RGWFormatterFlusher & flusher, optional_yield y,
                           const DoutPrefixProvider * dpp, bool warnings_only =
                           false);
    static int set_quota(rgw::sal::Driver * driver,
                         RGWBucketAdminOpState & op_state,
                         const DoutPrefixProvider * dpp);

    static int list_stale_instances(rgw::sal::Driver * driver,
                                    RGWBucketAdminOpState & op_state,
                                    RGWFormatterFlusher & flusher,
                                    const DoutPrefixProvider * dpp);

    static int clear_stale_instances(rgw::sal::Driver * driver,
                                     RGWBucketAdminOpState & op_state,
                                     RGWFormatterFlusher & flusher,
                                     const DoutPrefixProvider * dpp);
    static int fix_lc_shards(rgw::sal::Driver * driver,
                             RGWBucketAdminOpState & op_state,
                             RGWFormatterFlusher & flusher,
                             const DoutPrefixProvider * dpp);
    static int fix_obj_expiry(rgw::sal::Driver * driver,
                              RGWBucketAdminOpState & op_state,
                              RGWFormatterFlusher & flusher,
                              const DoutPrefixProvider * dpp, bool dry_run =
                              false);

    static int sync_bucket(rgw::sal::Driver * driver,
                           RGWBucketAdminOpState & op_state,
                           const DoutPrefixProvider * dpp,
                           std::string * err_msg = NULL);
};
...
struct rgw_ep_info {
    ...
    RGWObjVersionTracker ep_objv;
     rgw_ep_info(RGWBucketEntryPoint & ep, std::map < std::string,
                 bufferlist > &attrs)
    :ep(ep), attrs(attrs) {
}};
...
class RGWBucketCtl {
  ...
  public:
    ...
    void init(RGWUserCtl * user_ctl,
              ...
              RGWDataChangesLog * datalog, const DoutPrefixProvider * dpp);
    ...
    struct Bucket {
        struct GetParams {
            ...
            GetParams & set_refresh_version(const obj_version &
                                            _refresh_version) {
                refresh_version = _refresh_version;
                return *this;
            }

            GetParams & set_bectx_params(std::optional <
        ...
        struct RemoveParams {
            ...
            nullptr};
             ...
             RemoveParams() {
            } RemoveParams & set_objv_tracker(RGWObjVersionTracker *
                                              _objv_tracker) {
                objv_tracker = _objv_tracker;
                return *this;
            }
    ...
    int sync_user_stats(const DoutPrefixProvider * dpp,
                        ...
                        const RGWBucketInfo & bucket_info, optional_yield y,
                        RGWBucketEnt * pent);
  ...
  private:
    ...
    int do_store_linked_bucket_info(RGWSI_Bucket_X_Ctx & ctx,
                                    ...
                                    std::map < std::string,
                                    bufferlist > *pattrs,
                                    bool create_entry_point, optional_yield,
                                    const DoutPrefixProvider * dpp);
