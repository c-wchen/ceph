...
search_result_bs_t binary_search(const key_hobj_t &key,
                                 index_t begin, index_t end, FGetKey
                                 && f_get_key)
{
    assert(begin <= end);
    while (begin < end) {
    ...
    return {
        begin, MatchKindBS::NE};
}

template < typename PivotType, typename FGet >
search_result_bs_t binary_search_r(index_t rend, index_t rbegin, FGet
                                   && f_get, const PivotType &key)
{
    assert(rend <= rbegin);
    while (rend < rbegin) {
...
inline void assert_mstat(const key_hobj_t &key,
                         const key_view_t &index, match_stat_t mstat)
{
...
template < typename Params > struct staged {
    ...
    static void _left_or_right(index_t &split_index,
    ...
    template < ContainerType CTYPE, typename Enable =
               void >class _iterator_t;
    template < ContainerType CTYPE > class _iterator_t < CTYPE,
                 std::enable_if_t < CTYPE == ContainerType::INDEXABLE >>
    {
        /*
    ...
    public:
        ...
        _iterator_t(const container_t &container): container {
            container}
        {
        ...
        } node_offset_t size() const
        {
            ...
            assert(container.size_before(_index + 1) >
        ...
        std::enable_if_t < IS_BOTTOM,
            const T *>insert(NodeExtentMutable &mut, const Key &key,
                             const value_input_t &value,
                             node_offset_t insert_size,
                             const char *p_left_bound)
        {
            return container_t::insert_at(mut, container, key, value,
                                          _index, insert_size,
                                          p_left_bound);
        }
        ...
        std::enable_if_t < !IS_BOTTOM,
            T > insert_prefix(NodeExtentMutable &mut, const Key &key,
                              node_offset_t size,
                              const char *p_left_bound)
        {
            return container_t::insert_prefix_at(mut, container, key,
                                                 _index, size,
                                                 p_left_bound);
        }

        template < typename T = void >
        std::enable_if_t < !IS_BOTTOM, T > update_size(NodeExtentMutable &mut, int insert_size)
        {
            assert(!is_end());
            container_t::update_size_at(mut, container, _index,
                                        insert_size);
        }
                                   ...
                                   &is_insert_left)
        {
            ...
            _index =
                binary_search_r(0, s_end, f_get_used_size,
                                target_size).index;
            size_t current_size = f_get_used_size(_index);
            assert(current_size <= target_size);

            _left_or_right < is_exclusive > (_index, insert_index,
                                             is_insert_left);
            return current_size;
        }

        size_t seek_split(size_t start_size, size_t extra_size,
                          size_t target_size)
        {
            assert(!is_end());
            assert(index() == 0);
            auto start_size_1 = start_size + extra_size;
            auto f_get_used_size =
            [this, start_size, start_size_1](index_t index) {
                size_t current_size;
                if (unlikely(index == 0)) {
                    current_size = start_size;
                } else {
                    // already includes header size
                    current_size =
                        start_size_1 + container.size_before(index);
                }
                return current_size;
            };
            _index =
                binary_search_r(0, container.keys() - 1, f_get_used_size,
                                target_size).index;
            size_t current_size = f_get_used_size(_index);
            assert(current_size <= target_size);
            return current_size;
        ...
        template < KeyT KT >
        void copy_out_until(typename container_t::template Appender <
                                KT > &appender, index_t &to_index)
        {
        ...
        node_offset_t erase(NodeExtentMutable &mut,
                            const char *p_left_bound)
        {
            assert(!is_end());
            return container_t::erase_at(mut, container, _index,
                                         p_left_bound);
        }

        template < KeyT KT >
        typename container_t::template Appender < KT > get_appender(NodeExtentMutable *p_mut)
        {
            assert(_index + 1 == container.keys());
            return typename container_t::template Appender < KT > (p_mut,
                    container);
        }

        template < KeyT KT >
        typename container_t::template Appender < KT > get_appender_opened(NodeExtentMutable *p_mut)
        {
            if constexpr
            (!IS_BOTTOM) {
                assert(_index + 1 == container.keys());
                return typename container_t::template Appender < KT >
                (p_mut, container, true);
            } else {
                ceph_abort("impossible path");
            }
        }
        ...
        {
            auto container =
                container_t::decode(p_node_start, node_size, delta);
    ...
    class _iterator_t < CTYPE,
              std::enable_if_t < CTYPE == ContainerType::ITERATIVE >>
    {
        /*
    ...
    public:
        ...
        template < IsFullKey Key >
        memory_range_t insert_prefix(NodeExtentMutable &mut,
                                     const Key &key,
                                     node_offset_t size,
                                     const char *p_left_bound)
        {
            return container_t::insert_prefix(mut, container, key, is_end(),
                                              size, p_left_bound);
        }
                                   ...
                                   &is_insert_left)
        {
            ...
            _left_or_right < is_exclusive > (split_index, insert_index,
                                             is_insert_left);
            assert(split_index == index());
            return current_size;
        }
        ...
        node_offset_t trim_at(NodeExtentMutable &mut,
    ...
    static bool is_keys_one(const container_t &container)       // IN
    {
                                    ...
                                    key_view_t *p_index_key)
    {
        ...
        NXT_STAGE_T::template get_slot < GET_KEY, true > (nxt_container,
                pos_smallest,
                p_index_key,
                &p_value);
        if constexpr
        ...
        return result_t { {
                iter.index(), pos_smallest
            }
            , p_value, STAGE};
    }

    template < bool GET_KEY >
    static result_t nxt_lower_bound(const key_hobj_t &key,
                                    iterator_t &iter,
                                    ...
                                    key_view_t *index_key)
    {
        ...
        auto nxt_container = iter.get_nxt_container();
        auto nxt_result =
            NXT_STAGE_T::template lower_bound < GET_KEY > (nxt_container,
                    key, history,
                    index_key);
    ...
    template < bool GET_POS, bool GET_KEY, bool GET_VAL > static void get_largest_slot(const container_t
            &container,    // IN
            ...
            const value_t **pp_value)    // OUT
    {
        ...
        (IS_BOTTOM) {
            ...
            if constexpr
            (GET_POS) {
                NXT_STAGE_T::template get_largest_slot < true, GET_KEY,
                        GET_VAL > (nxt_container, &p_position->nxt, p_index_key,
                                   pp_value);
            } else {
                NXT_STAGE_T::template get_largest_slot < false, GET_KEY,
                        GET_VAL > (nxt_container, nullptr, p_index_key,
                                   pp_value);
            }
        }
    }

    template < bool GET_KEY, bool GET_VAL > static void get_slot(const container_t &container,  // IN
            ...
            const value_t **pp_value)      // OUT
    {
        ...
        (!IS_BOTTOM) {
            auto nxt_container = iter.get_nxt_container();
            NXT_STAGE_T::template get_slot < GET_KEY,
                                             GET_VAL > (nxt_container, pos.nxt, p_index_key, pp_value);
        } else {
            if constexpr
            (GET_VAL) {
                assert(pp_value);
    ...
    static result_t lower_bound(const container_t &container,
                                ...
                                key_view_t *index_key = nullptr)
    {
            ...
            if (*history.get < STAGE > () == MatchKindCMP::EQ) {
                ...
                (!IS_BOTTOM) {
                    ...
                    if (history.is_GT < STAGE - 1 > ()) {
                        ...
                        if (test_key_equal) {
                            return nxt_lower_bound < GET_KEY > (key, iter,
                                                                history,
                                                                index_key);
                        } else {
                            // key[stage] < index[stage][left-most]
                            return smallest_result < GET_KEY > (iter,
                                                                index_key);
                        }
                    }
                }
                ...
                (IS_BOTTOM) {
                    ...
                    auto nxt_container = iter.get_nxt_container();
                    auto nxt_result =
                        NXT_STAGE_T::template lower_bound < GET_KEY >
        ...
        (IS_BOTTOM) {
            ...
            if (bs_match == MatchKindBS::EQ) {
                return nxt_lower_bound < GET_KEY > (key, iter, history,
                                                    index_key);
            } else {
                return smallest_result < GET_KEY > (iter, index_key);
            }
    ...
    static std::enable_if_t < NODE_TYPE == node_type_t::INTERNAL,
           T > evaluate_insert(const container_t &container,
                               const key_view_t &key,
                               const value_input_t &value,
                               position_t &position, bool evaluate_last)
    {
        auto iter = iterator_t(container);
        ...
        if (evaluate_last || index == INDEX_END) {
            ...
            if (match == 0) {
                ...
                (IS_BOTTOM) {
                    ...
                    auto nxt_container = iter.get_nxt_container();
                    return NXT_STAGE_T::evaluate_insert(nxt_container, key,
                                                        value, position.nxt,
                                                        false);
                }
            } else {
            ...
            position_t &position)
    {
        ...
        if (stage == STAGE) {
            ...
            (IS_BOTTOM) {
                ...
                assert(stage < STAGE);
                bool compensate =
                    NXT_STAGE_T::compensate_insert_position_at(stage,
                        position.
                        nxt);
                if (compensate) {
                    assert(is_valid_index(index));
    ...
    static void patch_insert_end(position_t &insert_pos,
    ...
    template < typename T = std::tuple < match_stage_t, node_offset_t >>
    static std::enable_if_t < NODE_TYPE == node_type_t::LEAF,
           T > evaluate_insert(const key_hobj_t &key,
                               ...
                               match_stat_t mstat, position_t &position)
    {
        ...
        node_offset_t insert_size =
            insert_size_at(insert_stage, key, value);
    ...
    template < KeyT KT >
    static const value_t *insert_new(NodeExtentMutable &mut,
    ...
    static const value_t *proceed_insert_recursively(NodeExtentMutable &
            mut,
            const container_t &
            container,
            const full_key_t <
            ...
            *p_left_bound)
    {
        ...
        if (do_insert) {
            ...
            if constexpr
            (IS_BOTTOM) {
                return iter.insert(mut, key, value, _insert_size,
                                   p_left_bound);
            } else {
                auto range =
                    iter.insert_prefix(mut, key, _insert_size,
                                       p_left_bound);
                return NXT_STAGE_T::template insert_new < KT > (mut, range,
                        key, value);
            }
        } else {
            if constexpr
            (!IS_BOTTOM) {
                auto nxt_container = iter.get_nxt_container();
                auto p_value =
                    NXT_STAGE_T::template proceed_insert_recursively < KT,
                        SPLIT > (mut, nxt_container, key, value, position.nxt,
                                 stage, _insert_size, p_left_bound);
    ...
    template < KeyT KT, bool SPLIT >
    static const value_t *proceed_insert(NodeExtentMutable &mut,
                                         ...
                                         node_offset_t &_insert_size)
    {
        ...
        if (unlikely(!container.keys())) {
            ...
            if constexpr
            (IS_BOTTOM) {
                return container_t::insert_at(mut, container, key, value, 0,
                                              _insert_size, p_left_bound);
            } else {
                auto range =
                    container_t::template insert_prefix_at(mut, container,
                        key, 0,
                        _insert_size,
                        p_left_bound);
                return NXT_STAGE_T::template insert_new < KT > (mut, range,
            ...
            return proceed_insert_recursively < KT, SPLIT > (mut, container,
                    key, value,
                    position,
                    stage,
                    _insert_size,
    ...
    static std::ostream &dump(const container_t &container,
                              ...
                              size_t &size, const char *p_start)
    {
        auto iter = iterator_t(container);
        assert(!iter.is_end());
        ...
        do {
            ...
            (!IS_BOTTOM) {
                ...
                auto value_ptr = iter.get_p_value();
                int offset =
                    reinterpret_cast < const char *>(value_ptr) - p_start;
                size += iter.size();
                os << "\n" << i_prefix;
                if constexpr
    ...
    static void get_stats(const container_t &container,
                          node_stats_t &stats, key_view_t &index_key)
    {
        auto iter = iterator_t(container);
        assert(!iter.is_end());
    ...
    template < bool GET_KEY, bool GET_VAL > static bool get_next_slot(const container_t &container,     // IN
            ...
            const value_t **pp_value)     // OUT
    {
        ...
        (!IS_BOTTOM) {
            auto nxt_container = iter.get_nxt_container();
            find_next =
                NXT_STAGE_T::template get_next_slot < GET_KEY,
                    GET_VAL > (nxt_container, pos.nxt, p_index_key, pp_value);
        } else {
            find_next = true;
        }
        ...
        if (find_next) {
            if (iter.is_last()) {
                ...
                get_slot < GET_KEY, GET_VAL > (container, pos, p_index_key,
                                               pp_value);
                return false;
            }
    ...
    template < bool GET_KEY, bool GET_VAL > static void get_prev_slot(const container_t &container,     // IN
            position_t &pos,  // IN&OUT
            key_view_t *p_index_key,  // OUT
            const value_t **pp_value)     // OUT
    {
        ...
        (!IS_BOTTOM) {
            ...
            if (nxt_pos == NXT_STAGE_T::position_t::begin()) {
                ...
                auto nxt_container = iter.get_nxt_container();
                NXT_STAGE_T::template get_largest_slot < true, GET_KEY,
                        GET_VAL > (nxt_container, &nxt_pos, p_index_key,
                                   pp_value);
            } else {
                iter.seek_at(index);
                auto nxt_container = iter.get_nxt_container();
                NXT_STAGE_T::template get_prev_slot < GET_KEY,
                                                      GET_VAL > (nxt_container, nxt_pos, p_index_key,
                                                              pp_value);
            }
        } else {
    ...
    class StagedIterator: std::conditional_t < IS_BOTTOM, _BaseEmpty,
    ...
    public:
                                       ...
                                       const_iterator &delta)
        {
            ...
            if (present) {
                ret.iter =
                    iterator_t::decode(p_node_start, node_size, delta);
                if constexpr
                (!IS_BOTTOM) {
                    ret._nxt =
                        NXT_STAGE_T::StagedIterator::decode(p_node_start,
                                                            node_size,
                                                            delta);
                }
            }
    ...
    static bool recursively_locate_split(size_t &current_size,
                                         size_t extra_size,
                                         size_t target_size,
                                         StagedIterator &split_at)
    {
        ...
        current_size =
            split_iter.seek_split(current_size, extra_size, target_size);
        assert(current_size <= target_size);
        ...
        (!IS_BOTTOM) {
            locate_nxt =
                NXT_STAGE_T::recursively_locate_split(current_size,
                    extra_size +
                    split_iter.
                    size_to_nxt(),
                    target_size,
    ...
    static bool recursively_locate_split_inserted(size_t &current_size,
            size_t extra_size,
            size_t target_size,
            ...
            StagedIterator &split_at)
    {
        ...
        if (insert_stage == STAGE) {
            current_size =
                split_iter.template seek_split_inserted < true >
                (current_size, extra_size, target_size, insert_index,
                 insert_size, is_insert_left);
            assert(is_insert_left.has_value());
            assert(current_size <= target_size);
            if (split_iter.index() == 0) {
                if (insert_index == 0) {
            ...
            (!IS_BOTTOM) {
                assert(insert_stage < STAGE);
                current_size =
                    split_iter.template seek_split_inserted < false >
                    (current_size, extra_size, target_size, insert_index,
                     insert_size, is_insert_left);
                assert(!split_iter.is_end());
                assert(current_size <= target_size);
                ...
                if (!is_insert_left.has_value()) {
                    ...
                    locate_nxt =
                        NXT_STAGE_T::
                        recursively_locate_split_inserted(current_size,
                                                          extra_size +
                                                          split_iter.
                                                          size_to_nxt(),
                                                          target_size,
    ...
    template < KeyT KT > class StagedAppender: std::conditional_t < IS_BOTTOM, _BaseEmpty,
    ...
    public:
        ...
        void init_empty(NodeExtentMutable *p_mut, char *p_start)
        {
            assert(!valid());
            appender =
                typename container_t::template Appender < KT > (p_mut,
                        p_start);
            _index = 0;
        }
        void init_tail(NodeExtentMutable *p_mut,
                       const container_t &container, match_stage_t stage)
        {
            assert(!valid());
            auto iter = iterator_t(container);
            iter.seek_last();
            if (stage == STAGE) {
                ...
                (!IS_BOTTOM) {
                    appender =
                        iter.template get_appender_opened < KT > (p_mut);
                    _index = iter.index();
        ...
        void append_until(StagedIterator &src_iter, index_t &to_index)
        {
            ...
            src_iter.get().template copy_out_until < KT > (*appender,
                    to_index);
    ...
    private:
        std::optional < typename container_t::template Appender <
            KT >> appender;
        index_t _index;
    ...
    template < KeyT KT >
    static void _append_range(StagedIterator &src_iter,
                              StagedAppender < KT > &appender,
                              index_t &to_index)
    {
        if (src_iter.is_end()) {
        ...
        (!IS_BOTTOM) {
            if (appender.in_progress()) {
                ...
                NXT_STAGE_T::template _append_range < KT > (src_iter.nxt(),
                        appender.
                        get_nxt(),
                        to_index_nxt);
                ++src_iter;
                ...
                (src_iter.get_nxt(),
                 appender.open_nxt(src_iter.get_key()), to_index_nxt);
                ++src_iter;
                appender.wrap_nxt();
            } else {
    ...
    template < KeyT KT >
    static void _append_into(StagedIterator &src_iter,
                             StagedAppender < KT > &appender,
                             position_t &position,
                             match_stage_t stage)
    {
        ...
        if (stage == STAGE) {
            ...
            NXT_STAGE_T::template append_until < KT > (src_iter.nxt(),
                    appender.
                    open_nxt(src_iter.
                             get_key()),
                    position.nxt, stage);
    ...
    template < KeyT KT >
    static void append_until(StagedIterator &src_iter,
                             StagedAppender < KT > &appender,
                             position_t &position,
                             match_stage_t stage)
    {
    ...
    template < KeyT KT >
    static bool append_insert(const full_key_t < KT > &key,
                              const value_input_t &value,
                              ...
                              const value_t*&p_value)
    {
        ...
        if (stage == STAGE) {
            ...
            (!IS_BOTTOM) {
                auto nxt_is_end =
                    NXT_STAGE_T::template append_insert < KT > (key, value,
                            src_iter.
                            get_nxt(),
                            appender.
                            get_nxt(),
     ...
     */
    static std::tuple < TrimType, node_offset_t > recursively_trim(NodeExtentMutable &mut,
            StagedIterator &trim_at)
    {
        if (!trim_at.valid()) {
            return {
                TrimType::BEFORE, 0u};
    ...
    static std::optional < std::tuple < match_stage_t, node_offset_t, bool >> proceed_erase_recursively(
                NodeExtentMutable &mut, const container_t &container,  // IN
                ...
                position_t &pos)    // IN&OUT
    {
        ...
        (!IS_BOTTOM) {
            auto nxt_container = iter.get_nxt_container();
            auto ret =
                NXT_STAGE_T::proceed_erase_recursively(mut, nxt_container,
                    p_left_bound,
                    pos.nxt);
            if (ret.has_value()) {
    ...
    static match_stage_t erase(NodeExtentMutable &mut, const container_t &node_stage,   // IN
                               position_t &erase_pos)       // IN&OUT
    {
        auto p_left_bound = node_stage.p_left_bound();
        auto ret =
            proceed_erase_recursively(mut, node_stage, p_left_bound,
                                      erase_pos);
        if (ret.has_value()) {
            auto[r_stage, r_erase_size, r_done] = *ret;
    ...
    static std::tuple < match_stage_t,
           node_offset_t > evaluate_merge(const key_view_t &left_pivot_index,
        ...
        if (cmp == std::strong_ordering::equal) {
            ...
            (!IS_BOTTOM) {
                ...
                auto[ret_stage, ret_compensate] =
                    NXT_STAGE_T::evaluate_merge(left_pivot_index,
                                                r_nxt_container);
                compensate += ret_compensate;
                return {
                    ret_stage, compensate};
            } else {
                ceph_abort
                ("impossible path: left_pivot_key == right_first_key");
            }
...
template < typename NodeType, typename Enable =
           void >struct _node_to_stage_t;
template < typename NodeType > struct _node_to_stage_t < NodeType,
...
template < HasDoFormatTo T > struct fmt::formatter < T >: fmt::formatter <
    std::string_view > {
    template < typename FormatContext > auto format(const T &staged_iterator,
            FormatContext &ctx)
    {
