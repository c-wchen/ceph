...
namespace {
    ...
    struct client_config {
        ...
        std::string str() const {
            ...
            entity_addr_t addr;
            ceph_assert(addr.
                        parse(options["addr"].as < std::string > ().c_str(),
                              nullptr));
    ...
    static seastar::future <> run(perf_mode_t mode,
        ...
        struct test_state {
            struct Server;
            using ServerFRef =
                seastar::foreign_ptr < std::unique_ptr < Server >>;
            ...
            struct Server final:public crimson::net::Dispatcher {
                ...
                std::optional <
                    ...
                    const static hobject_t hobj(object_t(), oloc.key,
                                                CEPH_NOSNAP, pgid.ps(),
                                                pgid.pool(), oloc.nspace);
                    static spg_t spgid(pgid);
                    auto rep =
                        crimson::make_message < MOSDOp > (0, 0, hobj, spgid, 0,
                                                          0, 0);
                ...
                seastar::future <> init(const entity_addr_t & addr) {
                    return seastar::smp::submit_to(msgr_sid,[addr, this] {
                                                   ...
                                                   create(entity_name_t::
                                                          OSD(msgr_sid), lname,
                                                          0);
                                                   msgr->
                                                   set_default_policy(crimson::
                                                                      net::
                                                                      SocketPolicy::
                                                                      stateless_server
                                                                      (0));
                ...
                static seastar::future < ServerFRef >
                    create(seastar::shard_id msgr_sid, unsigned msg_len) {
            ...
            struct Client final:public crimson::net::Dispatcher,
                ...
                struct PeriodStats {
                    ...
                    void reset(unsigned received_count, PeriodStats * snap =
                               nullptr) {
                        if (snap) {
                ...
                :sid {
                seastar::this_shard_id()}, jobs {
                jobs}, msg_len {
                msg_len}, nr_depth {
                depth / jobs}, depth {
                nr_depth}, time_msgs_sent {
                depth / jobs, mono_clock::zero()} {
                    lname = "client#";
                ...
                std::optional <
                    ...
                    if (msg_id % SAMPLE_RATE == 0) {
                        ...
                        ceph_assert(time_msgs_sent[index] !=
                                    mono_clock::zero());
                        std::chrono::duration < double >cur_latency =
                            mono_clock::now() - time_msgs_sent[index];
                ...
                seastar::
                    future <> connect_wait_verify(const entity_addr_t &
                                                  peer_addr) {
                    return container().invoke_on_all([peer_addr] (auto & client) {
                                                     ...
                                                     return seastar::sleep(1 s).
                                                     then([&client,
                                                           start_time] {
                                                          if (client.conn_stats.
                                                              connected_time ==
                                                              mono_clock::
                                                              zero()) {
                                                          logger().
              ...
              private:
                class TimerReport {
                  ...
                  public:
                    ...
                    :jobs {
                    jobs}, msgtime {
                    msgtime}, bytes_of_block {
                    bs}, start_times {
                    jobs, mono_clock::zero()}, snaps {
                    jobs}, summaries {
                    jobs} {
                    }
                    ...
                    bool should_stop() const {
                        ...
                        return seastar::sleep(1 s).then([this] {
                                                        ++elapsed;}
                        );
                    }
                    ...
                    void report_period() {
                        ...
                        std::chrono::duration < double >elapsed_d = 0 s;
                        unsigned depth = 0u;
                        unsigned ops = 0u;
                        unsigned sampled_count = 0u;
                        double total_lat_s = 0.0;
                      for (const auto & snap:snaps) {
                        ...
                        sout << setfill(' ')
                            ...
                            << std::setw(8) << (total_lat_s / sampled_count *
                                                1000);
                        std::cout << sout.str() << std::endl;
                    }
                    ...
                    void report_summary() const {
                        ...
                        for (const auto & summary:summaries) {
                            elapsed_d +=
                                (summary.finish_time - summary.start_time);
                            ops +=
                                (summary.received_count - summary.start_count);
              ...
              public:
                seastar::future <> dispatch_with_timer(unsigned ramptime,
                                                       unsigned msgtime) {
                    logger().
                        info
                        ("[all clients]: start sending MOSDOps from {} clients",
                         jobs);
                    return container().invoke_on_all([](auto & client) {
                                                     if (client.is_active()) {
                                                     client.
                        ...
                        .then([ramptime] {
                              logger().
                              info("[all clients]: ramping up {} seconds...",
                                   ramptime);
                              return seastar::sleep(std::chrono::
                                                    seconds(ramptime));}
                    ).then([this] {
                           return container().invoke_on_all([](auto & client) {
                                                            if (client.
                        ...
                        .then([this, msgtime] {
                              logger().
                              info("[all clients]: reporting {} seconds...\n",
                                   msgtime);
                              return seastar::
                              do_with(TimerReport(jobs, msgtime, msg_len),
                                      [this] (auto & report) {
                                      report.report_header();
                                      return seastar::do_until([&report] {
                                                               return report.
                                                               should_stop();
                                                               }
                                                               ,[&report, this] {
              ...
              private:
                seastar::future <> send_msg(crimson::net::Connection * conn) {
                    ...
                    return depth.wait(1).then([this, conn] {
                                              const static pg_t pgid;
                                              const static object_locator_t
                                              oloc;
                                              const static hobject_t
                                              hobj(object_t(), oloc.key,
                                                   CEPH_NOSNAP, pgid.ps(),
                                                   pgid.pool(), oloc.nspace);
                                              static spg_t spgid(pgid);
                                              auto m =
                                              crimson::make_message < MOSDOp >
                ...
                void do_dispatch_messages(crimson::net::Connection * conn) {
                    ...
                    (void)seastar::do_until([this] {
                                            ...
                                            // ok, stopped by stop_dispatch_messages()
                                                    ...
                                                    std::chrono::duration <
                                                    double >dur_conn =
                                                    conn_stats.connected_time -
                                                    conn_stats.connecting_time;
                                                    std::chrono::duration <
                                                    double >dur_msg =
                                                    mono_clock::now() -
                                                    conn_stats.start_time;
                                                    unsigned ops =
                                                    conn_stats.received_count -
                                                    conn_stats.start_count;
                                                    logger().
                                                    info
                                                    ("{}: stopped sending OSDOPs.\n"
                                                     "{}(depth={}):\n"
                                                     "  connect time: {}s\n"
                                                     "  messages received: {}\n"
                                                     "  messaging time: {}s\n"
        ...
        return seastar::when_all(test_state::Server::
                                 create(server_conf.core,
                                        server_conf.block_size),
                                 create_sharded < test_state::Client >
                                 (client_conf.jobs, client_conf.block_size,
                                  client_conf.depth),
                   ...
                   ceph_assert(server_conf.core == 0
                               || server_conf.core > client_conf.jobs);
                   return seastar::when_all_succeed(server->
                                                    init(server_conf.addr),
                                                    client->init()
                          ...
                          then([cleanup = std::move(fp_server)] {
                               }
                          );}
                   );}
                   else
