            ...
            static int parse_map_options(char *options) {
                for (char *this_char = strtok(options, ", ");
                    ...
                    if (!strcmp(this_char, "fsid")) {
                        ...
                        if (put_map_option_value
                            ("ip", value_char, map_option_ip_cb))
                            return -EINVAL;
                    }
 ...
 */
            ...
            static int do_kernel_map(const char *poolname, const char *imgname,
                ...
                for (std::map < std::string, std::string >::iterator it =
                     map_options.begin(); it != map_options.end();) {
                ...
                r = krbd_map(krbd, poolname, imgname, snapname,
            ...
            static int do_kernel_unmap(const char *dev, const char *poolname,
                ...
                for (auto it = map_options.cbegin(); it != map_options.cend();
                     ++it) {
            ...
            int execute_show(const po::variables_map & vm) {
                ...
                if (r < 0) {
                    std::
                        cerr << "rbd: showmapped failed: " << cpp_strerror(r) <<
                        std::endl;
                    return r;
                }
                return 0;
            ...
            int execute_map(const po::variables_map & vm) {
                ...
                char *default_map_options =
                    strdup(g_conf->get_val < std::string >
                ...
                if (parse_map_options(default_map_options)) {
                    std::
                        cerr << "rbd: couldn't parse default map options" <<
                        std::endl;
                    return -EINVAL;
                }

                if (vm.count("options")) {
                    char *cli_map_options =
                        strdup(vm["options"].as < std::string > ().c_str());
                    BOOST_SCOPE_EXIT((cli_map_options)) {
                        free(cli_map_options);
                    } BOOST_SCOPE_EXIT_END;

                    if (parse_map_options(cli_map_options)) {
                        std::cerr << "rbd: couldn't parse map options" << std::
                            endl;
                        return -EINVAL;
                    }
                }
                ...
                r = do_kernel_map(pool_name.c_str(), image_name.c_str(),
                                  snap_name.c_str());
                if (r < 0) {
            ...
            int execute_unmap(const po::variables_map & vm) {
                ...
                if (vm.count("options")) {
                    ...
                    if (parse_unmap_options(cli_unmap_options)) {
                        std::
                            cerr << "rbd: couldn't parse unmap options" << std::
                            endl;
                        return -EINVAL;
                    }
                }
