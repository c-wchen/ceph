        ...
        // you use up ten seconds of CPU time.
         ...
         template < class TC > class timer {
            ...
            struct event {
                ...
                sh event_link;
                 ...
                 event():t(TC::time_point::min()), id(0) {
                        ...
                        const std::function < void () > &_f):t(_t), id(_id),
                    f(_f) {
                }
            ...
            using schedule_type = set < event,
                ...
                constant_time_size < false >, compare < SchedCompare > >;

            schedule_type schedule;

            using event_set_type = set < event,
                member_hook < event, sh, &event::event_link >,
                constant_time_size < false >, compare < EventCompare > >;
          ...
          public:
            ...
            uint64_t reschedule_me(typename TC::time_point when) {
                if (std::this_thread::get_id() != thread.get_id())
                    throw std::make_error_condition(std::errc::
