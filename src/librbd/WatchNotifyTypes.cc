...
namespace librbd {
    namespace watch_notify {
        ...
        void AsyncRequestId::encode(bufferlist & bl) const {
            ...
            using ceph::encode;
             encode(client_id, bl);
        } void AcquiredLockPayload::decode(__u8 version,
            ...
            using ceph::encode;
             encode(client_id, bl);
        } void ReleasedLockPayload::decode(__u8 version,
                                           bufferlist::const_iterator & iter) {
            using ceph::decode;
            if (version >= 2) {
                decode(client_id, iter);
        ...
        void ReleasedLockPayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             encode(client_id, bl);
             encode(force, bl);
        } void RequestLockPayload::decode(__u8 version,
                                          bufferlist::const_iterator & iter) {
            using ceph::decode;
            if (version >= 2) {
                decode(client_id, iter);
        ...
        } void HeaderUpdatePayload::decode(__u8 version,
                                           bufferlist::const_iterator & iter) {
        }
        ...
        } void AsyncRequestPayloadBase::decode(__u8 version,
                                               bufferlist::
                                               const_iterator & iter) {
            using ceph::decode;
        ...
        void AsyncRequestPayloadBase::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(offset, bl);
             encode(total, bl);
        } void AsyncProgressPayload::decode(__u8 version,
                                            bufferlist::const_iterator & iter) {
            using ceph::decode;
        ...
        void AsyncProgressPayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             AsyncRequestPayloadBase::encode(bl);
             encode(result, bl);
        } void AsyncCompletePayload::decode(__u8 version,
                                            bufferlist::const_iterator & iter) {
            using ceph::decode;
            AsyncRequestPayloadBase::decode(version, iter);
            decode(result, iter);
        ...
        } void ResizePayload::decode(__u8 version,
                                     bufferlist::const_iterator & iter) {
            using ceph::decode;
            decode(size, iter);
            AsyncRequestPayloadBase::decode(version, iter);
        ...
        void ResizePayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(async_request_id, bl);
        } void SnapPayloadBase::decode(__u8 version,
                                       bufferlist::const_iterator & iter) {
            using ceph::decode;
        ...
        void SnapPayloadBase::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(flags, bl);
        } void SnapCreatePayload::decode(__u8 version,
                                         bufferlist::const_iterator & iter) {
            using ceph::decode;
        ...
        void SnapCreatePayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             encode(snap_id, bl);
             SnapPayloadBase::encode(bl);
        } void SnapRenamePayload::decode(__u8 version,
                                         bufferlist::const_iterator & iter) {
            using ceph::decode;
            decode(snap_id, iter);
            SnapPayloadBase::decode(version, iter);
        ...
        void SnapRenamePayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(async_request_id, bl);
        } void RenamePayload::decode(__u8 version,
                                     bufferlist::const_iterator & iter) {
            using ceph::decode;
        ...
        void RenamePayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(enabled, bl);
             encode(async_request_id, bl);
        } void UpdateFeaturesPayload::decode(__u8 version,
                                             bufferlist::
                                             const_iterator & iter) {
            using ceph::decode;
        ...
        } void SparsifyPayload::decode(__u8 version,
                                       bufferlist::const_iterator & iter) {
            using ceph::decode;
            AsyncRequestPayloadBase::decode(version, iter);
        ...
        void SparsifyPayload::dump(Formatter * f) const {
            ...
            using ceph::encode;
             ...
             encode(value, bl);
             encode(async_request_id, bl);
        } void MetadataUpdatePayload::decode(__u8 version,
                                             bufferlist::
                                             const_iterator & iter) {
            using ceph::decode;
            decode(key, iter);
        ...
        void MetadataUpdatePayload::dump(Formatter * f) const {
            ...
            ceph_abort();
        } void UnknownPayload::decode(__u8 version,
                                      bufferlist::const_iterator & iter) {
        }
        ...
        void NotifyMessage::dump(Formatter * f) const {
        ...
        } void NotifyMessage::generate_test_instances(std::list <
                                                      NotifyMessage * >&o) {
            o.push_back(new
                        NotifyMessage(new AcquiredLockPayload(ClientId(1, 2))));
            o.push_back(new
                        NotifyMessage(new ReleasedLockPayload(ClientId(1, 2))));
            o.push_back(new
                        NotifyMessage(new
                                      RequestLockPayload(ClientId(1, 2),
                                                         true)));
            o.push_back(new NotifyMessage(new HeaderUpdatePayload()));
            o.push_back(new
                        NotifyMessage(new
                                      AsyncProgressPayload(AsyncRequestId
                                                           (ClientId(0, 1), 2),
                                                           3, 4)));
            o.push_back(new
                        NotifyMessage(new
                                      AsyncCompletePayload(AsyncRequestId
                                                           (ClientId(0, 1), 2),
                                                           3)));
            o.push_back(new
                        NotifyMessage(new
                                      FlattenPayload(AsyncRequestId
                                                     (ClientId(0, 1), 2))));
            o.push_back(new
                        NotifyMessage(new
                                      ResizePayload(AsyncRequestId
                                                    (ClientId(0, 1), 2), 123,
                                                    true)));
            o.push_back(new
                        NotifyMessage(new
                                      SnapCreatePayload(AsyncRequestId
                                                        (ClientId(0, 1), 2),
                                                        cls::rbd::
            ...
            o.push_back(new
                        NotifyMessage(new
                                      SnapRemovePayload(AsyncRequestId
                                                        (ClientId(0, 1), 2),
                                                        cls::rbd::
                                                        UserSnapshotNamespace(),
                                                        "foo")));
            o.push_back(new
                        NotifyMessage(new
                                      SnapProtectPayload(AsyncRequestId
                                                         (ClientId(0, 1), 2),
                                                         cls::rbd::
                                                         UserSnapshotNamespace
                                                         (), "foo")));
            o.push_back(new
                        NotifyMessage(new
                                      SnapUnprotectPayload(AsyncRequestId
                                                           (ClientId(0, 1), 2),
                                                           cls::rbd::
                                                           UserSnapshotNamespace
                                                           (), "foo")));
            o.push_back(new
                        NotifyMessage(new
                                      RebuildObjectMapPayload(AsyncRequestId
                                                              (ClientId(0, 1),
                                                               2))));
            o.push_back(new
                        NotifyMessage(new
                                      RenamePayload(AsyncRequestId
                                                    (ClientId(0, 1), 2),
                                                    "foo")));
            o.push_back(new
                        NotifyMessage(new
                                      UpdateFeaturesPayload(AsyncRequestId
                                                            (ClientId(0, 1), 2),
                                                            1, true)));
            o.push_back(new
                        NotifyMessage(new
                                      MigratePayload(AsyncRequestId
                                                     (ClientId(0, 1), 2))));
            o.push_back(new
                        NotifyMessage(new
                                      SparsifyPayload(AsyncRequestId
                                                      (ClientId(0, 1), 2), 1)));
            o.push_back(new
                        NotifyMessage(new
                                      QuiescePayload(AsyncRequestId
                                                     (ClientId(0, 1), 2))));
            o.push_back(new
                        NotifyMessage(new
                                      UnquiescePayload(AsyncRequestId
                                                       (ClientId(0, 1), 2))));
            o.push_back(new
                        NotifyMessage(new
                                      MetadataUpdatePayload(AsyncRequestId
                                                            (ClientId(0, 1), 2),
                                                            "foo",
                                                            std::optional <
                                                            std::string > {
        ...
        void ResponseMessage::dump(Formatter * f) const {
            f->dump_int("result", result);
        } void ResponseMessage::generate_test_instances(std::list <
                                                        ResponseMessage * >&o) {
        ...
        std::ostream & operator<<(std::ostream & out,
                                  const librbd::watch_notify::
                                  AsyncRequestId & request) {
            out << "[" << request.client_id.gid << "," << request.client_id.
