      ...
      template < typename I > class ImageDeleterAdminSocketHook:public AdminSocketHook
        {
          ...
          private:
            typedef std::map < std::string,
                ImageDeleterAdminSocketCommand * >Commands;
            AdminSocket *admin_socket;
            Commands commands;
        };

        template < typename I >
            ImageDeleter < I >::ImageDeleter(ContextWQ * work_queue,
                                             SafeTimer * timer,
                                             Mutex * timer_lock,
                                             ServiceDaemon < librbd::ImageCtx >
                                             *service_daemon)
      :    m_work_queue(work_queue), m_service_daemon(service_daemon),
            m_delete_lock("rbd::mirror::ImageDeleter::Delete"),
            m_image_deleter_thread(this), m_failed_timer(timer),
            m_failed_timer_lock(timer_lock),
            m_asok_hook(new ImageDeleterAdminSocketHook < I >
                        (g_ceph_context, this)) {
            set_failed_timer_interval(g_ceph_context->_conf->get_val <
                                      double
                                      >("rbd_mirror_delete_retry_interval"));
            m_image_deleter_thread.create("image_deleter");
        }

        template < typename I > ImageDeleter < I >::~ImageDeleter() {
            dout(20) << "enter" << dendl;
        ...
        template < typename I > void ImageDeleter < I >::run() {
            dout(20) << "enter" << dendl;
            while (m_running) {
                m_delete_lock.Lock();
                                                           ...
                                                           bool ignore_orphaned)
        {
            ...
            m_delete_queue.push_front(unique_ptr < DeleteInfo >
                                      (new
                                       DeleteInfo(local_rados, local_pool_id,
                                                  global_image_id,
                                                  ignore_orphaned)));
            m_delete_queue_cond.Signal();
        }
        ...
        template < typename I >
            void ImageDeleter <
            I >::wait_for_scheduled_deletion(int64_t local_pool_id,
                                             const std::
        ...
        template < typename I > bool ImageDeleter < I >::process_image_delete() {
            stringstream ss;
            m_active_delete->to_string(ss);
            std::string del_info_str = ss.str();
            dout(10) << "start processing delete request: " << del_info_str <<
                dendl;
            int r;
            ...
            IoCtx ioctx;
            r = m_active_delete->local_rados->ioctx_create2(m_active_delete->
                                                            local_pool_id,
                                                            ioctx);
            if (r < 0) {
                derr << "error accessing local pool " << m_active_delete->
                    local_pool_id << ": " << cpp_strerror(r) << dendl;
                enqueue_failed_delete(r);
                return true;
            }

            dout(20) << "connected to local pool: " << ioctx.
                get_pool_name() << dendl;

            auto & global_image_id = m_active_delete->global_image_id;
            std::string local_image_id;
            r = librbd::cls_client::mirror_image_get_image_id(&ioctx,
                                                              global_image_id,
                                                              &local_image_id);
            if (r == -ENOENT) {
                dout(10) << "image " << global_image_id << " is not mirrored" <<
                    dendl;
                complete_active_delete(r);
                return true;
            }
            else if (r < 0) {
                derr << "error retrieving local id for image " <<
            ...
            Journal <>::get_tag_owner(ioctx, local_image_id, &mirror_uuid,
            ...
            if (r < 0 && r != -ENOENT) {
                ...
                if (mirror_uuid == Journal <>::LOCAL_MIRROR_UUID) {
                    ...
                    dout(10) << "image " << global_image_id << " is orphaned" <<
                        dendl;
                    complete_active_delete(-EISPRM);
                    return true;
                }
            ...
            bool has_snapshots;
            r = image_has_snapshots_and_children(&ioctx, local_image_id,
                                                 &has_snapshots);
            if (r < 0) {
                enqueue_failed_delete(r);
                return true;
            ...
            r = cls_client::mirror_image_set(&ioctx, local_image_id,
                                             mirror_image);
            if (r == -ENOENT) {
                dout(10) << "local image is not mirrored, aborting deletion..."
                    << dendl;
                complete_active_delete(r);
                return true;
            }
            ...
            if (has_snapshots) {
                ...
                ImageCtx *imgctx =
                    new ImageCtx("", local_image_id, nullptr, ioctx, false);
              ...
              for (const auto & snap:snaps) {
                    dout(20) << "processing deletion of snapshot " << imgctx->
                        name << "@" << snap.name << dendl;

                    bool is_protected;
                    r = librbd::snap_is_protected(imgctx, snap.name.c_str(),
                                                  &is_protected);
                    if (r < 0) {
                        derr <<
                    ...
                    if (is_protected) {
                        dout(20) << "snapshot " << imgctx->name << "@" << snap.
                            name << " is protected, issuing unprotect command"
                            << dendl;
                        ...
                        if (r == -EBUSY) {
                            ...
                            dout(10) << "local image id " << local_image_id <<
                                " has " <<
                            ...
                            derr << "error unprotecting snapshot " << imgctx->
                                name << "@" << snap.
                    ...
                    r = imgctx->operations->
                        snap_remove(cls::rbd::UserSnapshotNamespace(),
                                    snap.name.c_str());
                    if (r < 0) {
                        derr << "error removing snapshot " << imgctx->
                            name << "@" << snap.
                            name << ": " << cpp_strerror(r) << dendl;
                        imgctx->state->close();
                        enqueue_failed_delete(r);
            ...
            if (r == -ENOENT) {
                dout(20) <<
                    "local image does not exist, removing image from rbd_mirroring"
                    << dendl;
            }
        ...
        template < typename I >
            int ImageDeleter <
            I >::image_has_snapshots_and_children(IoCtx * ioctx,
                                                  string & image_id,
                                                  bool * has_snapshots) {
            ...
            if (r < 0 && r != -ENOENT) {
                derr << "error retrieving snapshot context for image id " <<
        ...
        template < typename I >
            unique_ptr < typename ImageDeleter < I >::DeleteInfo >
            const *ImageDeleter < I >::find_delete_info(int64_t local_pool_id,
                                                        const std::
                                                        string &
                                                        global_image_id) {
        ...
        template < typename I >
            vector < pair < string,
            int >>ImageDeleter < I >::get_failed_queue_items() {
        ...
        template < typename I >
            void ImageDeleter <
            I >::set_failed_timer_interval(double interval) {
