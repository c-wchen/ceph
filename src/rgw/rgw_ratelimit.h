...
class RateLimiterEntry
{
    ...
    bool minimum_time_reached(ceph::timespan curr_timestamp) const
    {
        ...
        constexpr auto min_duration =
            ...
            (seconds(60)) / fixed_point_rgw_ratelimit;
                         ...
                         const RGWRateLimitInfo *info)
    {
        ...
        if (first_run) {
            ...
            read.ops =
                std::min(info->max_read_ops * fixed_point, read_ops + read.ops);
            read.bytes =
                std::min(info->max_read_bytes * fixed_point,
                         read_bw + read.bytes);
            write.ops =
                std::min(info->max_write_ops * fixed_point,
                         write_ops + write.ops);
            write.bytes =
                std::min(info->max_write_bytes * fixed_point,
                         write_bw + write.bytes);
        }
    }
...
public:
                           ...
                           ceph::timespan curr_timestamp)
    {
        ...
        if (is_read) {
            return should_rate_limit_read(ratelimit_info->max_read_ops,
                                          ratelimit_info->max_read_bytes);
        }
        return should_rate_limit_write(ratelimit_info->max_write_ops,
...
public:
                        ...
                        const int64_t amount, const RGWRateLimitInfo *info)
    {
        ...
        bool is_read = is_read_op(method);
        if ((is_read && !info->max_read_bytes)
...
public:
    ...
    void start()
    {
        ...
        const auto rc =
            ceph_pthread_setname(runner.native_handle(), "ratelimit_gc");
        ceph_assert(rc == 0);
    }
};
