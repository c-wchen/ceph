...
static int objexp_hint_parse(const DoutPrefixProvider * dpp, CephContext * cct,
                                    ...
                                    const rgw_obj_index_key & obj_key)
{
    ...
    string shard_name =
        objexp_hint_get_shardname(objexp_key_shard
                                  (obj_key,
                                   cct->_conf->rgw_objexp_hints_num_shards));
    auto obj =
        rados_svc->
        obj(rgw_raw_obj
            (driver->svc()->zone->get_zone_params().log_pool, shard_name));
    int r = obj.open(dpp);
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "ERROR: " << __func__ << "(): failed to open obj=" <<
            obj << " (r=" << r << ")" << dendl;
        return r;
    }
...
int RGWObjExpStore::objexp_hint_list(const DoutPrefixProvider * dpp, const string & oid, const ceph::real_time & start_time, const ceph::real_time & end_time, const int max_entries, const string & marker, list < cls_timeindex_entry > &entries, /* out */
...
{                               /* out */
    ...
    auto obj =
        rados_svc->
        obj(rgw_raw_obj(driver->svc()->zone->get_zone_params().log_pool, oid));
    int r = obj.open(dpp);
    if (r < 0) {
        ldpp_dout(dpp,
                  0) << "ERROR: " << __func__ << "(): failed to open obj=" <<
                                     ...
                                     const string & to_marker)
{
    ...
    int ret =
        cls_timeindex_trim_repeat(dpp, ref, oid, utime_t(start_time),
                                  utime_t(end_time),
...
int RGWObjectExpirer::garbage_single_object(const DoutPrefixProvider * dpp,
                                            objexp_hint_entry & hint)
{
    ...
    int ret =
        driver->get_bucket(dpp, nullptr,
                           rgw_bucket(hint.tenant, hint.bucket_name,
                                      hint.bucket_id), &bucket, null_yield);
    if (-ENOENT == ret) {
        ldpp_dout(dpp, 15) << "NOTICE: cannot find bucket = "
            << hint.
            bucket_name << ". The object must be already removed" << dendl;
                                            ...
                                            const utime_t & round_start)
{
    ...
    int ret =
        l.lock_exclusive(&static_cast <
                         rgw::sal::RadosStore *
                         >(driver)->getRados()->objexp_pool_ctx, shard);
    if (ret == -EBUSY) {        /* already locked by another processor */
        ldpp_dout(dpp,
                  5) << __func__ << "(): failed to acquire lock on " << shard <<
            dendl;
        return false;
    }
    ...
    l.unlock(&static_cast <
             rgw::sal::RadosStore * >(driver)->getRados()->objexp_pool_ctx,
             shard);
    return done;
}
...
CephContext *RGWObjectExpirer::OEWorker::get_cct() const const
{
    return cct;
}

unsigned RGWObjectExpirer::OEWorker::get_subsys() const const
{
    return dout_subsys;
}

std::ostream & RGWObjectExpirer::OEWorker::gen_prefix(std::ostream & out) const const
{
