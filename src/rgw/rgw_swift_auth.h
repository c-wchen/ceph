            ...
            class TempURLEngine:public rgw::auth::Engine {
              ...
              public:
                 TempURLEngine(CephContext * const cct,
                               ...
                               const apl_factory)
                :cct(cct), store(store), apl_factory(apl_factory) {
                }
                /* Interface implementations. */
                    const char *get_name() const noexcept override {
                    return "rgw::auth::swift::TempURLEngine";
                } result_t authenticate(const req_state *
            ...
            class SignedTokenEngine:public rgw::auth::Engine {
              ...
              public:
                 SignedTokenEngine(CephContext * const cct,
                                   ...
                                   const apl_factory)
                :cct(cct), store(store), extractor(extractor),
            ...
            class DefaultStrategy:public rgw::auth::Strategy,
                ...
                /* The method implements TokenExtractor for X-Auth-Token present in req_state. */
                 std::string get_token(const req_state * const s) const override {
                    ...
                    auto apl = rgw::auth::add_3rdparty(store, s->account_name,
                                                       rgw::auth::
                                                       add_sysreq(cct, store, s,
                                                                  rgw::auth::
                                                                  RemoteApplier
                                                                  (cct, store,
                                                                   std::
                ...
                :store(store),
                    ...
                    tempurl_engine(cct,
                                   ...
                                   >(this)), signed_engine(cct, store,
                                                           ...
                                                           LocalApplier::
                                                           Factory * >(this)),
                    external_engine(cct, store,
                                    static_cast <
                                    rgw::auth::TokenExtractor * >(this),
                                    static_cast <
...
class RGWHandler_SWIFT_Auth:public RGWHandler_REST {
  public:
    ...
    int init(RGWRados * store, struct req_state *state,
