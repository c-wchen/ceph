...
namespace rbd {
    namespace utils {
        ...
        void ProgressContext::finish() {
            if (progress) {
                cerr << "\r" << operation << ": 100% complete...done." << std::
                    endl;
            }
        }
        ...
        int extract_image_id_spec(const std::string & spec,
                                  std::string * pool_name,
                                  std::string * image_id) {
            boost::regex pattern;
            pattern = "^(?:([^/]+)/)?(.+)?$";
        ...
        int get_special_pool_group_names(const po::variables_map & vm,
            ...
            if (group_name->empty()) {
                ...
                if (!spec.empty()) {
                    r = utils::extract_group_spec(spec, group_pool_name,
                                                  group_name);
                    if (r < 0) {
                        return r;
                    }
            ...
            if (group_name->empty()) {
                std::
                    cerr << "rbd: consistency group name was not specified" <<
                    std::endl;
                return -EINVAL;
            }
        ...
        int get_special_pool_image_names(const po::variables_map & vm,
            ...
            if (nullptr == image_pool_name)
                return -EINVAL;
            if (nullptr == image_name)
                return -EINVAL;
        ...
        int get_pool_image_id(const po::variables_map & vm,
                              ...
                              std::string * pool_name, std::string * image_id) {
        ...
        int get_pool_image_snapshot_names(const po::variables_map & vm,
            ...
            if (image_name != nullptr && !image_name->empty()) {
                ...
                r = extract_spec(image_name_copy, pool_name, image_name,
        ...
        int get_pool_snapshot_names(const po::variables_map & vm,
            ...
            if (snap_name != nullptr) {
                int r =
                    validate_snapshot_name(mod, *snap_name, snapshot_presence,
                                           spec_validation);
                if (r < 0) {
                    return r;
                }
        ...
        int get_pool_journal_names(const po::variables_map & vm,
            ...
            if (journal_name != nullptr && !journal_name->empty()) {
                ...
                r = extract_spec(journal_name_copy, pool_name, journal_name,
            ...
            if (pool_name != nullptr && journal_name != nullptr &&
                ...
                librbd::Image image;
                int r =
                    init_and_open_image(*pool_name, image_name, "", "", true,
                                        &rados,
                                        &io_ctx, &image);
                if (r < 0) {
                    std::cerr << "rbd: failed to open image " << image_name
                        << " to get journal name: " << cpp_strerror(r) << std::
                        endl;
                    return r;
                }
