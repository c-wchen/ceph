...
/* TempURL: engine */
                                                 ...
                                                 RGWUserInfo & owner_info) const
            {
                ...
                if (!s->account_name.empty()) {
                    ...
                    if (uid.tenant.empty()) {
                        ...
                        if (rgw_get_user_info_by_uid(store, tenanted_uid, uinfo)
                            >= 0) {
                            /* Succeeded. */
                    ...
                    if (!found
                        && rgw_get_user_info_by_uid(store, uid, uinfo) < 0) {
                        throw - EPERM;
                    }
                    else {
                        bucket_tenant = uinfo.user_id.tenant;
                ...
                int ret =
                    store->get_bucket_info(*static_cast <
                ...
                if (rgw_get_user_info_by_uid
                    (store, bucket_info.owner, owner_info) < 0) {
                    throw - EPERM;
                }
            }
            ...
            TempURLEngine::result_t
                ...
                const std::string & temp_url_expires =
                    s->info.args.get("temp_url_expires");
                ...
                if (owner_info.temp_url_keys.empty()) {
                    ldout(cct,
                          5) << "user does not have temp url key set, aborting"
                        << dendl;
                    return result_t::reject();
                }
                ...
                boost::container::static_vector < boost::string_view,
                    3 > allowed_methods;
                ...
                s->decoded_uri, s->object.name, temp_url_prefix};
...
/* External token */
            ...
            ExternalTokenEngine::result_t
                ...
                RGWHTTPHeadersCollector validator(cct, {
                                                  "X-Auth-Groups",
                                                  "X-Auth-Ttl"});
...
/* AUTH_rgwtk (signed token): engine */
                                                ...
                                                const req_state * const s)const
            {
                ...
                if (memcmp(local_tok_bl.c_str(), tok_bl.c_str(),
                    ...
                    ldout(cct,
                          0) << "NOTICE: tokens mismatch tok=" << buf << dendl;
                    return result_t::deny(-EPERM);
                }
