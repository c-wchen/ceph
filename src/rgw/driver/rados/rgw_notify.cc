...
// use mmap/mprotect to allocate 128k coroutine stacks
    auto make_stack_allocator() {
        return boost::context::protected_fixedsize_stack {
        128 *1024};
    }
    ...
    class Manager:public DoutPrefixProvider {
        ...
        // read the list of queues from the queue list object
            ...
            while (more) {
                ...
                op.omap_get_keys2(start_after, max_chunk, &queues_chunk, &more,
                                  &rval);
                const auto ret =
                    rgw_rados_operate(this, rados_ioctx, Q_LIST_OBJECT_NAME,
                                      &op, nullptr, y);
                if (ret == -ENOENT) {
                    ...
                    ldpp_dout(this,
                              1) << "ERROR: failed to read queue list. error: "
                        << ret << dendl;
                    return ret;
                }
        ...
        class tokens_waiter {
          ...
          tokens_waiter(boost::asio::io_context & io_context):
            infinite_duration(1000), pending_tokens(0), timer(io_context) {
            }
        ...
        bool process_entry(const cls_queue_entry & entry, yield_context yield) {
            ...
            try {
                ...
                ldpp_dout(this,
                          5) << "WARNING: failed to decode entry. error: " <<
                    err.what() << dendl;
                return false;
            }
            try {
                ...
                const auto push_endpoint =
                    RGWPubSubEndpoint::create(event_entry.push_endpoint,
                                              event_entry.arn_topic,
                                              RGWHTTPArgs(event_entry.
                                                          push_endpoint_args,
                                                          this),
                                              cct);
                ldpp_dout(this,
                          20) << "INFO: push endpoint created: " << event_entry.
                    push_endpoint << " for entry: " << entry.marker << dendl;
                const auto ret =
                    push_endpoint->send_to_completion_async(cct,
                                                            event_entry.event,
                                                            optional_yield
                                                            (io_context,
                                                             yield));
                if (ret < 0) {
                    ldpp_dout(this,
                              5) << "WARNING: push entry: " << entry.
                        marker << " to endpoint: " << event_entry.
                        push_endpoint << " failed. error: " << ret <<
                        " (will retry)" << dendl;
                    return false;
                }
                else {
                    ldpp_dout(this,
                              20) << "INFO: push entry: " << entry.
                        marker << " to endpoint: " << event_entry.
                        push_endpoint << " ok" << dendl;
                    if (perfcounter)
                ...
                ldpp_dout(this,
                          5) << "WARNING: failed to create push endpoint: " <<
                    event_entry.push_endpoint << " for entry: " << entry.
                    marker << ". error: " << e.
        ...
        void cleanup_queue(const std::string & queue_name, yield_context yield) {
            while (true) {
                ldpp_dout(this,
                          20) <<
                    "INFO: trying to perform stale reservation cleanup for queue: "
                    << queue_name << dendl;
                const auto now = ceph::coarse_real_time::clock::now();
                const auto stale_time =
                    now - std::chrono::seconds(stale_reservations_period_s);
                ...
                auto ret =
                    rgw_rados_operate(this, rados_ioctx, queue_name, &op,
                                      optional_yield(io_context, yield));
                if (ret == -ENOENT) {
                    // queue was deleted
                    ldpp_dout(this, 5) << "INFO: queue: "
                        << queue_name << ". was removed. cleanup will stop" <<
                        dendl;
                    return;
                }
                if (ret == -EBUSY) {
                    ldpp_dout(this,
                              5) << "WARNING: queue: " << queue_name <<
                ...
                if (ret < 0) {
                    ldpp_dout(this,
                              5) <<
                        "WARNING: failed to cleanup stale reservation from queue and/or lock queue: "
                        << queue_name << ". error: " << ret << dendl;
                }
                Timer timer(io_context);
                timer.
                    expires_from_now(std::chrono::
                                     seconds(reservations_cleanup_period_s));
                boost::system::error_code ec;
                timer.async_wait(yield[ec]);
            }
        }
        ...
        void process_queue(const std::string & queue_name, yield_context yield) {
            ...
            while (true) {
                ...
                if (is_idle) {
                    ...
                    timer.
                        expires_from_now(std::chrono::
                                         microseconds(queue_idle_sleep_us));
                    boost::system::error_code ec;
                    timer.async_wait(yield[ec]);
                }
                ...
                auto total_entries = 0U;
                {
                    ...
                    rados::cls::lock::assert_locked(&op, queue_name + "_lock",
                                                    ...
                                                    lock_cookie,
                                                    "" /*no tag */ );
                    cls_2pc_queue_list_entries(op, start_marker, max_elements,
                                               &obl, &rval);
                    ...
                    if (ret < 0) {
                        ldpp_dout(this,
                                  5) <<
                            "WARNING: failed to get list of entries in queue and/or lock queue: "
                            << queue_name << ". error: " << ret <<
                            " (will retry)" << dendl;
                        continue;
                    }
                    ret =
                        cls_2pc_queue_list_entries_result(obl, entries,
                                                          &truncated,
                                                          end_marker);
                    if (ret < 0) {
                        ldpp_dout(this,
                ...
                ldpp_dout(this,
                    ...
                    " entries in: " << queue_name << ". end marker is: " <<
                    end_marker << dendl;
              ...
              for (auto & entry:entries) {
                    ...
                    spawn::spawn(yield,
                                 [this, &queue_name, entry_idx, total_entries,
                                  &end_marker, &remove_entries, &has_error,
                                  &waiter, entry] (yield_context yield) {
                                 const auto token = waiter.make_token();
                                 if (process_entry(entry, yield)) {
                                 ldpp_dout(this,
                                           20) << "INFO: processing of entry: "
                                 << entry.
                                 marker << " (" << entry_idx << "/" <<
                                 total_entries << ") from: " << queue_name <<
                                 " ok" << dendl; remove_entries = true;}
                                 else {
                                 if (set_min_marker(end_marker, entry.marker) <
                                     0) {
                                 ldpp_dout(this,
                                           1) <<
                                 ...
                                 ldpp_dout(this,
                                           20) <<
                                 "INFO: new end marker for removal: " <<
                                 end_marker << " from: " << queue_name <<
                                 dendl;}
                                 has_error = true;
                                 ldpp_dout(this,
                                           20) << "INFO: processing of entry: "
        ...
        void process_queues(yield_context yield) {
            ...
            std::uniform_int_distribution <> duration_jitter(min_jitter,
                                                             max_jitter);
            ...
            while (true) {
                ...
                const auto duration = (has_error ?
                                       std::chrono::
                                       milliseconds(queues_update_retry_ms) :
                                       std::chrono::
                                       milliseconds(queues_update_period_ms)) +
                    std::chrono::milliseconds(duration_jitter(rnd_gen));
                timer.expires_from_now(duration);
                const auto tp =
                    ceph::coarse_real_time::clock::to_time_t(ceph::
                                                             coarse_real_time::
                                                             clock::now() +
                                                             duration);
                ldpp_dout(this,
                          20) << "INFO: next queues processing will happen at: "
                    << std::ctime(&tp) << dendl;
                boost::system::error_code ec;
                timer.async_wait(yield[ec]);
                ...
                queues_t queues;
                auto ret =
                    read_queue_list(queues, optional_yield(io_context, yield));
                if (ret < 0) {
                    has_error = true;
                    continue;
                }
              ...
              for (const auto & queue_name:queues) {
                    ...
                    rados::cls::lock::lock(&op, queue_name + "_lock",
                                           ...
                                           lock_cookie, "" /*no tag */ ,
                                           "" /*no description */ ,
                                           failover_time, LOCK_FLAG_MAY_RENEW);
                    ...
                    if (owned_queues.insert(queue_name).second) {
                        ...
                        spawn::spawn(io_context,
                                     [this, &queue_gc, &queue_gc_lock,
                                      queue_name] (yield_context yield) {
                ...
                {
                    std::lock_guard lock_guard(queue_gc_lock);
                    std::for_each(queue_gc.begin(), queue_gc.end(),
                                  [this,
                                   &owned_queues] (const std::
                                                   string & queue_name) {
      ...
      Manager(CephContext * _cct, uint32_t _max_queue_size, uint32_t _queues_update_period_ms, uint32_t _queues_update_retry_ms, uint32_t _queue_idle_sleep_us, u_int32_t failover_time_ms, uint32_t _stale_reservations_period_s, uint32_t _reservations_cleanup_period_s, uint32_t _worker_count, rgw::sal::RadosStore * store):
        max_queue_size(_max_queue_size),
            ...
            spawn::spawn(io_context,[this] (yield_context yield) {
                         process_queues(yield);}, make_stack_allocator());
            ...
            for (auto worker_id = 0U; worker_id < worker_count; ++worker_id) {
                workers.emplace_back([this] () {
                                     ...
                                     ldpp_dout(this,
                                               10) <<
                                     "Notification worker failed with error: "
                                     << err.what() << dendl; throw(err);}
                                     });
                const auto rc =
                    ceph_pthread_setname(workers.back().native_handle(),
                                         (WORKER_THREAD_NAME +
                                          std::to_string(worker_id)).c_str());
                ceph_assert(rc == 0);
            }
            ldpp_dout(this,
                      10) << "Started notification manager with: " <<
                worker_count << " workers" << dendl;
        }

        int add_persistent_topic(const std::string & topic_name,
                                 optional_yield y) {
            if (topic_name == Q_LIST_OBJECT_NAME) {
                ldpp_dout(this,
                          1) << "ERROR: topic name cannot be: " <<
                    Q_LIST_OBJECT_NAME <<
            ...
            if (ret < 0) {
                ...
                ldpp_dout(this,
                          1) << "ERROR: failed to create queue for topic: " <<
                    topic_name << ". error: " << ret << dendl;
                return ret;
            }

            bufferlist empty_bl;
            std::map < std::string, bufferlist > new_topic { {
            topic_name, empty_bl}
            };
            op.omap_set(new_topic);
            ret =
                rgw_rados_operate(this, rados_ioctx, Q_LIST_OBJECT_NAME, &op,
                                  y);
            if (ret < 0) {
                ldpp_dout(this,
                          1) << "ERROR: failed to add queue: " << topic_name <<
        ...
        int remove_persistent_topic(const std::string & topic_name,
                                    optional_yield y) {
            librados::ObjectWriteOperation op;
            op.remove();
            auto ret = rgw_rados_operate(this, rados_ioctx, topic_name, &op, y);
            ...
            if (ret < 0) {
                ...
                ldpp_dout(this,
                          1) << "ERROR: failed to remove queue for topic: " <<
                    topic_name << ". error: " << ret << dendl;
                return ret;
            }

            std::set < std::string > topic_to_remove { {
            topic_name}
            };
...
// TODO make the pointer atomic in allocation and deallocation to avoid race conditions
    ...
    bool init(CephContext * cct, rgw::sal::RadosStore * store,
        ...
        s_manager = new Manager(cct, MAX_QUEUE_SIZE,
                                ...
                                RESERVATIONS_CLEANUP_PERIOD_S, WORKER_COUNT,
                                store);
        return true;
    }
    ...
    rgw::sal::Object * get_object_with_atttributes(const reservation_t & res,
                                                   rgw::sal::Object * obj) {
    ...
    static inline void metadata_from_attributes(reservation_t & res,
                                                rgw::sal::Object * obj) {
    ...
    static inline void tags_from_attributes(const reservation_t & res,
                                            rgw::sal::Object * obj,
...
// populate event from request
    static inline void populate_event(reservation_t & res,
        ...
        event.bucket_ownerIdentity =
            res.bucket->get_owner()? res.bucket->get_owner()->get_id().id : "";
        const auto region =
            res.store->get_zone()->get_zonegroup().get_api_name();
        ...
        boost::algorithm::hex((const char *)&ts,
                              (const char *)&ts + sizeof(utime_t),
    ...
    static inline bool notification_match(reservation_t & res,
        ...
        if (!filter.s3_filter.tag_filter.kv.empty()) {
            ...
            if (req_tags) {
                ...
                if (!match
                    (filter.s3_filter.tag_filter, (*res.tagset).get_tags())) {
                    return false;
                }
            }
            else {
    ...
    int publish_reserve(const DoutPrefixProvider * dpp,
                        EventType event_type,
                        reservation_t & res, const RGWObjTags * req_tags) {
      ...
      for (const auto & bucket_topic:bucket_topics.topics) {
            ...
            ldpp_dout(res.dpp,
                ...
                s3_id << "' on topic: '" << topic_cfg.dest.
                arn_topic << "' and bucket: '" << res.bucket->
                get_name() << "' (unique topic: '" << topic_cfg.
                name << "') apply to event of type: '" << to_string(event_type)
                << "'" << dendl;
    ...
    int publish_commit(rgw::sal::Object * obj,
                       ...
                       const std::string & version,
                       EventType event_type,
                       reservation_t & res, const DoutPrefixProvider * dpp) {
      for (auto & topic:res.topics) {
            ...
            event_entry_t event_entry;
            populate_event(res, obj, size, mtime, etag, version, event_type,
                           event_entry.event);
            event_entry.event.configurationId = topic.configurationId;
            event_entry.event.opaque_data = topic.cfg.opaque_data;
            if (topic.cfg.dest.persistent) {
                event_entry.push_endpoint =
                    std::move(topic.cfg.dest.push_endpoint);
                ...
                if (bl.length() > res.size) {
                    ...
                    ldpp_dout(dpp,
                              5) << "WARNING: committed size: " << bl.length()
                        << " exceeded reserved size: " << res.
                        size <<
                    ...
                    ret =
                        rgw_rados_operate(dpp,
                                          res.store->getRados()->
                                          get_notif_pool_ctx(), queue_name, &op,
                                          res.yield,
                                          librados::OPERATION_RETURNVEC);
                    if (ret < 0) {
                        ldpp_dout(dpp,
                                  1) <<
                            "ERROR: failed to reserve extra space on queue: " <<
                            queue_name << ". error: " << ret << dendl;
                        return (ret == -ENOSPC) ? -ERR_RATE_LIMITED : ret;
                    }
                    ...
                    if (ret < 0) {
                        ldpp_dout(dpp,
                                  1) <<
                            "ERROR: failed to parse reservation id for "
                            "extra space. error: " << ret << dendl;
                        return ret;
                    }
                }
                std::vector < buffer::list > bl_data_vec {
                std::move(bl)};
                librados::ObjectWriteOperation op;
                ...
                try {
                    ...
                    const auto ret =
                        push_endpoint->send_to_completion_async(dpp->get_cct(),
                                                                event_entry.
                                                                event,
                                                                res.yield);
                    if (ret < 0) {
                    ...
                    ldpp_dout(dpp,
                        ...
                        topic.cfg.dest.push_endpoint << ". error: " << e.
                        what() << dendl;
                    if (perfcounter)
                        perfcounter->inc(l_rgw_pubsub_push_failed);
                    return -EINVAL;
      ...
      for (auto & topic:res.topics) {
            ...
            if (ret < 0) {
                ldpp_dout(res.dpp, 1) << "ERROR: failed to abort reservation: "
                    << topic.res_id <<
                    " from queue: " << queue_name << ". error: " << ret <<
                    dendl;
                return ret;
            }
            topic.res_id = cls_2pc_reservation::NO_ID;
  ...
  reservation_t::reservation_t(const DoutPrefixProvider * _dpp, rgw::sal::RadosStore * _store, const req_state * _s, rgw::sal::Object * _object, rgw::sal::Object * _src_object, const std::string * _object_name, optional_yield y):
    dpp(_s), store(_store), s(_s),
        size(0) /* XXX */ ,
        object(_object), src_object(_src_object), bucket(_s->bucket.get()),
        object_name(_object_name), tagset(_s->tagset),
        x_meta_map(_s->info.x_meta_map),
        metadata_fetched_from_attributes(false), user_id(_s->user->get_id().id),
        user_tenant(_s->user->get_id().tenant), req_id(_s->req_id), yield(y) {
    }
    ...
    reservation_t::reservation_t(const DoutPrefixProvider * _dpp,
                                 ...
                                 optional_yield y):dpp(_dpp), store(_store),
        s(nullptr), size(0) /* XXX */ , object(_object),
        src_object(_src_object), bucket(_bucket), object_name(nullptr),
        metadata_fetched_from_attributes(false), user_id(_user_id),
        user_tenant(_user_tenant), req_id(_req_id), yield(y) {
    }
