...
class BlueFS {
  public:
    ...
    struct File:public RefCountedObject {
        ...
        :RefCountedObject(NULL, 0),
            ...
            dirty_seq(0),
            locked(false),
            deleted(false), num_readers(0), num_writers(0), num_reading(0) {
        } ~File() override {
    ...
    struct FileReaderBuffer {
        ...
        :bl_off(0), pos(0), max_prefetch(mpf) {
  ...
  public:
    ...
    int open_for_write(const string & dir,
                       const string & file, FileWriter ** h, bool overwrite);

    int open_for_read(const string & dir,
                      const string & file,
    ...
    int lock_file(const string & dirname, const string & filename,
                  FileLock ** p);
    ...
    int read(FileReader * h, FileReaderBuffer * buf, uint64_t offset,
             size_t len, bufferlist * outbl, char *out) {
