                                  ...
                                  const std::string &path)
{
    ...
    if (handle == NULL) {
        std::
        cerr << "Failed to load class: " << name << " (" << path <<
             "): " << dlerror() << std::endl;
        return;
    }
    ...
    if (!cls_init) {
        std::cerr << "Error locating initializer: " << dlerror() << std::
                  endl;
    } else if (cls_init) {
    ...
    std::
    cerr << "Class: " << name << " (" << path << ") missing initializer"
         << std::endl;
    dlclose(handle);
}
        ...
        string &method)
{
    ...
    if (m_it == scls->methods.end()) {
        std::
        cerr << "Failed to located class method" << cls << "." << method
             << std::endl;
        return NULL;
    }
