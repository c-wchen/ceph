...
TEST(LibRadosMiscConnectFailure, ConnectFailure)
{
    ...
    ASSERT_EQ(0,
              rados_conf_set(cluster, "client_mount_timeout", "0.000000001"));
...
TEST_F(LibRadosMiscPP, LongLocatorPP)
{
    ...
    ioctx.locator_set_key(string(maxlen + 1, 'a'));
    ASSERT_EQ(-ENAMETOOLONG,
              ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.locator_set_key(string((maxlen * 2), 'a'));
    ASSERT_EQ(-ENAMETOOLONG,
              ioctx.write(string("a").c_str(), bl, bl.length(), 0));
}
...
TEST_F(LibRadosMiscPP, LongAttrNamePP)
{
    ...
    ASSERT_EQ(0,
              ioctx.setxattr("bigattrobj", string(maxlen / 2, 'a').c_str(),
                             bl));
    ASSERT_EQ(0,
              ioctx.setxattr("bigattrobj", string(maxlen - 1, 'a').c_str(),
                             bl));
...
TEST_F(LibRadosMiscPP, CopyPP)
{
    ...
    {
        ObjectWriteOperation op;
        op.copy_from2("big", ioctx, ioctx.get_last_version(),
                      LIBRADOS_OP_FLAG_FADVISE_DONTNEED);
        ASSERT_EQ(0, ioctx.operate("big.copy", &op));

        bufferlist bl2, x2;
        ASSERT_EQ((int)bl.length(),
                  ioctx.read("big.copy", bl2, bl.length(), 0));
        ASSERT_TRUE(bl.contents_equal(bl2));
        ASSERT_EQ((int)x.length(), ioctx.getxattr("foo.copy", "myattr", x2));
        ASSERT_TRUE(x.contents_equal(x2));
    }

    {
        ...
        bufferlist bl2, x2;
        ASSERT_EQ((int)bl.length(),
                  ioctx.read("big.copy2", bl2, bl.length(), 0));
        ASSERT_TRUE(bl.contents_equal(bl2));
        ASSERT_EQ((int)x.length(), ioctx.getxattr("foo.copy2", "myattr", x2));
        ASSERT_TRUE(x.contents_equal(x2));
    }
...
TEST_F(LibRadosMisc, WriteSame)
{
    ...
    /* write the same buf four times */
    ASSERT_EQ(0,
              rados_writesame(ioctx, "ws", buf, sizeof(buf), sizeof(full), 0));

    /* read back the full buffer and confirm that it matches */
    ASSERT_EQ((int)sizeof(full),
              rados_read(ioctx, "ws", full, sizeof(full), 0));
    ...
    ASSERT_EQ(-EINVAL, rados_writesame(ioctx, "ws", buf, sizeof(buf),
                                       (sizeof(buf) * 4) - 1, 0));
    ASSERT_EQ(-EINVAL,
              rados_writesame(ioctx, "ws", buf, sizeof(buf), sizeof(buf) / 2,
                              0));
    ASSERT_EQ(-EINVAL, rados_writesame(ioctx, "ws", buf, 0, sizeof(buf), 0));
    /* write_len = data_len, i.e. same as rados_write() */
    ASSERT_EQ(0,
              rados_writesame(ioctx, "ws", buf, sizeof(buf), sizeof(buf), 0));
}
...
typedef::testing::Types <
    ...
    Checksummer::crc32c, uint32_t > >LibRadosChecksumTypes;
...
TYPED_TEST(LibRadosChecksum, Subset)
{
    ...
    for (uint32_t i = 0; i < csum_count; ++i) {
        op.checksum(TestFixture::type, init_value_bl, i * chunk_size,
                    chunk_size, 0, &checksum_bls[i], &checksum_rvals[i]);
    }
...
TYPED_TEST(LibRadosChecksum, Chunked)
{
    ...
    bufferptr expected_values_bp =
        buffer::create_static(csum_count *
                              sizeof(typename TestFixture::value_t),
...
TEST_F(LibRadosMisc, CmpExt)
{
    ...
    ASSERT_EQ(0,
              rados_write(ioctx, "cmpextpp", stored_str, sizeof(stored_str),
                          0));

    ASSERT_EQ(0,
              rados_cmpext(ioctx, "cmpextpp", stored_str, sizeof(stored_str),
                           0));

    ASSERT_EQ(-MAX_ERRNO - 5,
              rados_cmpext(ioctx, "cmpextpp", mismatch_str,
                           sizeof(mismatch_str), 0));
}
