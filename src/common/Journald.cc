...
public:
    ...
    {
        meta_buf.clear();
        fmt::format_to(std::back_inserter(meta_buf), R "(PRIORITY={:d}
                       CEPH_SUBSYS = {}
                                     TIMESTAMP = {}
                                         CEPH_PRIO = {}
                                             THREAD = {: 016x}
                                                 MESSAGE
                      )", map_prio(e.m_prio), s->get_name(e.m_subsys), e.m_stamp.time_since_epoch().count().count, e.m_prio, e.m_thread);

        uint64_t msg_len = htole64(e.size());
        meta_buf.resize(meta_buf.size() + sizeof(msg_len));
        *(reinterpret_cast < uint64_t * >(meta_buf.end()) - 1) =
            htole64(e.size());
                                   ...
                                   CEPH_NAME = {}
                                               CEPH_RANK = {}
                                                   CEPH_SEQ = {}
                                                       CEPH_CHANNEL = {}
                                                           MESSAGE
                      )", clog_type_to_syslog_level(le.prio), le.stamp.to_nsec(), le.name.to_str(), le.rank, le.seq, le.channel);
...
{
    int memfd =
        memfd_create("ceph-journald", MFD_ALLOW_SEALING | MFD_CLOEXEC);
    if (memfd >= 0) {
    ...
    memfd =
        open(mem_file_dir, O_TMPFILE | O_EXCL | O_CLOEXEC,
             S_IRUSR | S_IWUSR);
    if (memfd >= 0) {
...
int JournaldClient::open_mem_file()
{
    switch (mem_file_mode) {
        case MemFileMode::MEMFD_CREATE:
            return memfd_create("ceph-journald",
                                MFD_ALLOW_SEALING | MFD_CLOEXEC);
        case MemFileMode::OPEN_TMPFILE:
            return open(mem_file_dir, O_TMPFILE | O_EXCL | O_CLOEXEC,
                        S_IRUSR | S_IWUSR);
...
int JournaldClient::send()
{
    ...
    if (errno != EMSGSIZE && errno != ENOBUFS) {
        std::
        cerr << "Failed to send log to journald: " <<
    ...
    if (buffer_fd < 0) {
        std::
        cerr <<
             "Failed to open buffer_fd while sending log to journald: "
             << strerror(errno) << std::endl;
        return -1;
    }
    ...
    if (ret < 0) {
        std::
        cerr <<
             "Failed to write to buffer_fd while sending log to journald: "
             << strerror(errno) << std::endl;
    ...
    if (mem_file_mode == MemFileMode::MEMFD_CREATE) {
        ret =
            fcntl(buffer_fd, F_ADD_SEALS,
                  F_SEAL_SHRINK | F_SEAL_GROW | F_SEAL_WRITE |
                  F_SEAL_SEAL);
        if (ret) {
            std::
            cerr <<
                 "Failed to seal buffer_fd while sending log to journald: "
                 << strerror(errno) << std::endl;
            goto err_close_buffer_fd;
        }
    }
    ...
    if (ret < 0) {
        ...
        std::
        cerr << "Failed to send fd while sending log to journald: "
             << strerror(errno) << std::endl;
        goto err_close_buffer_fd;
    }
    close(buffer_fd);
