...
void LogEntryKey::encode(bufferlist & bl, uint64_t features) const const
{
...
void LogEntry::log_to_syslog(string level, string facility)
{
    ...
    if (l <= min) {
        ...
        syslog(l | f, "%s %llu : %s",
               stringify(who).c_str(), (long long unsigned)seq, msg.c_str());
    }
}
...
void LogSummary::dump(Formatter * f) const const
{
    ...
    for (list < LogEntry >::const_iterator p = tail.begin(); p != tail.end();
         ++p) {
