...
int parse_log_client_options(CephContext * cct,
                             ...
                             uuid_d & fsid, string & host)
{
    ostringstream oss;

    int r =
        get_conf_str_map_helper(cct->_conf.get_val < string >
    ...
    r = get_conf_str_map_helper(cct->_conf.get_val < string >
                                ("clog_to_syslog"), oss, &log_to_syslog,
    ...
    r = get_conf_str_map_helper(cct->_conf.get_val < string >
                                ("clog_to_syslog_facility"), oss, &log_channels,
                                CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(cct->_conf.get_val < string >
                                ("clog_to_syslog_level"), oss, &log_prios,
                                CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(cct->_conf.get_val < string >
                                ("clog_to_graylog"), oss, &log_to_graylog,
                                CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
...
syslog_facility(facility), log_to_syslog(false), log_to_monitors(false)
{
}
...
bool LogClient::are_pending() const const
{
...
entity_addrvec_t LogClient::get_myaddrs() const const
{
...
const EntityName & LogClient::get_myname() const const
{
...
LogChannelRef LogClient::create_channel(const std::string & name)
{
    ...
    if (it == channels.end()) {
        it = channels.insert(it, {
                             name, seastar::make_lw_shared < LogChannel > (this,
                                                                           name)}
        );
    }
    return it->second;
