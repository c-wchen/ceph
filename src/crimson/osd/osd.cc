...
namespace crimson::osd {
    ...
    cluster_msgr}, public_msgr {
    public_msgr}, hb_front_msgr {
    hb_front_msgr}, hb_back_msgr {
    hb_back_msgr}, monc {
        new crimson::mon::Client {
    *public_msgr, *this}}, mgrc {
        new crimson::mgr::Client {
    *public_msgr, *this}}, store {
    store},
    ...
    seastar::make_lw_shared < crimson::admin::AdminSocket > ()},
        log_client(cluster_msgr.get(), LogClient::NO_FLAGS),
        clog(log_client.create_channel()) {
      for (auto msgr:{
             std::ref(cluster_msgr), std::ref(public_msgr),
...
// Features here are also automatically upgraded
    ...
    seastar::future <> OSD::open_meta_coll() {
        return store.get_sharded_store().open_collection(coll_t::meta()
            ).then([this] (auto ch) {
                   pg_shard_manager.init_meta_coll(ch,
    ...
    seastar::future < OSDMeta >
        ...
        return store.get_sharded_store().open_collection(coll_t::meta()).
            then([&store] (auto ch) {
                 ...
                 return seastar::make_ready_future < OSDMeta > (ch,
                                                                store.
                                                                get_sharded_store
                                                                ());}
                 }
    ...
    seastar::future <> OSD::mkfs(FuturizedStore & store,
                                 unsigned whoami,
                                 uuid_d osd_uuid,
                                 uuid_d cluster_fsid,
        ...
        return store.start().then([&store, osd_uuid] {
            ...
            then([&store] {
               ...
               return _write_superblock(store, std::move(meta_coll),
                                        std::move(superblock));}
               ...
               fmt::print("created object store {} for osd.{} fsid {}\n",
                          ...
                          whoami, cluster_fsid); return store.umount();}
        ).then([&store] {
               return store.stop();}
        );
    }

    seastar::future <> OSD::_write_superblock(FuturizedStore & store,
                                              OSDMeta meta_coll,
                                              OSDSuperblock superblock) {
        return seastar::do_with(std::move(meta_coll),
                                std::move(superblock),
                                ...
                                safe_then([&superblock] (OSDSuperblock && sb) {
                                          ...
                                          ("provided cluster fsid {} != superblock's {}",
                                           ...
                                           superblock.cluster_fsid);
                                          throw std::
                                          invalid_argument("mismatched fsid");}
                                          if (sb.whoami != superblock.whoami) {
                                          logger().
                                          error
                                          ("provided osd id {} != superblock's {}",
                                ...
                                handle_error(crimson::ct_error::enoent::
                                             handle([&store, &meta_coll,
                                                    ...
                                                    logger().
                                                    debug
                                                    ("OSD::_write_superblock: do_transaction...");
                                                    return store.
                                                    get_sharded_store().
...
// the language rule on not overloading in `std::`.
    static std::string to_string(const seastar::temporary_buffer <
                                 char >&temp_buf) {
        return {
        temp_buf.get(), temp_buf.size()};
    }

    seastar::future <> OSD::_write_key_meta(FuturizedStore & store) {

        if (auto key = local_conf().get_val < std::string > ("key");
            !std::empty(key)) {
            return store.write_meta("osd_key", key);
        }
        else if (auto keyfile =
                 local_conf().get_val < std::string > ("keyfile");
                 !std::empty(keyfile)) {
            return read_file(keyfile).then([&store] (const auto & temp_buf) {
                                           // it's on a truly cold path, so don't worry about memcpy.
                                           return store.write_meta("osd_key",
                                                                   to_string
                                                                   (temp_buf));}
            ).handle_exception([keyfile] (auto ep) {
                               logger().
                               error
                               ("_write_key_meta: failed to handle keyfile {}: {}",
                                keyfile, ep); ceph_abort();}
            );
        }
        else {
            return seastar::now();
        ...
        std::pair < entity_addrvec_t, bool >
            ...
            auto maybe_replace =[&](entity_addr_t addr) {
              ...
              for (auto & b:            knowns.v) {
                                         ...
                                         changed = true; return a;}
                                         }
                                         throw std::
                                         runtime_error
                                         ("failed to replace unknown address");};
                                         entity_addrvec_t replaced;
                                         std::transform(maybe_unknowns.v.
                                                        begin(),
                                                        maybe_unknowns.v.end(),
                                                        std::
                                                        back_inserter(replaced.
                                                                      v),
                                                        maybe_replace); return {
                                         replaced, changed};}
                                         }
                                         ...
                                         then([this] {
                                              return open_meta_coll();}
                                                ...
                                                handle_error(crimson::ct_error::
                                                             assert_all
                                                             ("open_meta_coll error")
                                                );}
                                         ).then([this] (OSDSuperblock && sb) {
                                                superblock = std::move(sb);
                                                pg_shard_manager.
                                                set_superblock(superblock);
                                                return pg_shard_manager.
                                                get_local_map(superblock.
                                                              current_epoch);}
                                         ).
                                         then([this]
                                              ...
                                              (OSDMapService::
                                               local_cached_map_t(map));
                                              return pg_shard_manager.
                                              update_map(std::move(map));}
                                         ).then([this] {
                                                pg_shard_manager.
                                                got_map(osdmap->get_epoch());
                                                bind_epoch =
                                                osdmap->get_epoch();
                                                return pg_shard_manager.
                                                load_pgs(store);}
                                                ...
                                                this, monc.get(), mgrc.get()};
                                                return seastar::
                                                when_all_succeed(cluster_msgr->
                                                                 bind
                                                                 (pick_addresses
                                                                 ...
                                                                 safe_then([this, dispatchers] ()mutable {
                                                                           ...
                                                                           ([]
                                                                            ...
                                                                            ().
                                                                            error
                                                                            ("public messenger bind(): {}",
                                                                             e);
                                                                            ceph_abort
                                                                            ();})));}).
                                         then_unpack([this] {
                                                     return seastar::
                                                     when_all_succeed(monc->
                                                                      start(),
                                                                      mgrc->
                                                                      start());}
                                         ).then_unpack([this] {
                                                ...
                                                logger().
                                                debug
                                                ("replacing unkwnown addrs of cluster messenger");
                                                cluster_msgr->
                                                set_myaddrs(addrs);}
                                                return heartbeat->
                                                start(pick_addresses
                                                      (CEPH_PICK_ADDRESS_PUBLIC),
                                                      pick_addresses
                                                      (CEPH_PICK_ADDRESS_CLUSTER));}
                                         ).then([this] {
                                         ...
                                         return monc->get_version("osdmap").
                                         then([this] (auto && ret) {
                                         ...
                                         seastar::future <> OSD::
                                         _preboot(version_t oldest,
                                                  version_t newest) {
                                         logger().info("osd.{}: _preboot",
                                                       whoami);
                                         if (osdmap->get_epoch() == 0) {
                                         logger().
                                         info("waiting for initial osdmap");}
                                         else
                                         if (osdmap->is_destroyed(whoami)) {
                                         logger().
                                         warn("osdmap says I am destroyed");
                                         // provide a small margin so we don't livelock seeing if we
                                         // un-destroyed ourselves.
                                         if (osdmap->get_epoch() > newest - 1) {
                                         throw std::
                                         runtime_error("i am destroyed");}
                                         }
                                         else
                                         if (osdmap->is_noup(whoami)) {
                                         logger().
                                         warn
                                         ...
                                         logger().
                                         error
                                         ("osdmap require_osd_release < octopus; please upgrade to octopus");}
                                         else
                                         if (false) {
  ...
  // get all the latest maps
                                         ...
                                         osdmap_subscribe(osdmap->get_epoch() +
                                                          1, false);}
                                         else {
                                         return get_shard_services().
                                         ...
                                         cluster_addrs =
                                         cluster_msgr->get_myaddrs();}
                                         if (heartbeat->get_back_msgr().
                                             set_addr_unknowns(cluster_addrs)) {
                                         hb_back_addrs =
                                         heartbeat->get_back_addrs();}
                                         if (heartbeat->get_front_msgr().
                                             set_addr_unknowns(public_addrs)) {
                                         hb_front_addrs =
                                         heartbeat->get_front_addrs();}
                                         logger().info("hb_back_msgr: {}",
                                                       hb_back_addrs);
                                         logger().info("hb_front_msgr: {}",
                                                       hb_front_addrs);
                                         logger().info("cluster_msgr: {}",
                                                       cluster_addrs);
                                         auto m =
                                         crimson::make_message < MOSDBoot >
                                         (superblock, osdmap->get_epoch(),
                                          boot_epoch, hb_back_addrs,
                                          hb_front_addrs, cluster_addrs,
                                          CEPH_FEATURES_ALL);
                                         ...
                                         return store.stat().then([](auto st) {
                                                                  auto total = st.total; return seastar::make_ready_future < double >(std::max(.00001, double (total) / double (1ull << 40)));  // TB
                                                                  });}
                                         };
                                         return get_weight().
                                         then([this] (auto weight) {
                                              ...
                                              return seastar::do_with(std::
                                                                      ...
                                                                      init_on_startup
                                                                      ().
                                                                      then([this, weight, &loc] () {
                                                                           logger().info("crush location is {}", loc); string cmd = fmt::format(R "({{
          " prefix ": " osd crush create - or - move ",
          " id ": {},
          " weight ": {:.4f},
          " args ": [{}]
        }})", whoami, weight, loc); return monc->run_command(std::move(cmd), {
                                                                                                                                                                                                                                                                                                                                                         }
                                                                           );}
                                                                      );}
                                              );}
                                         ...
                                         seastar::future <> OSD::
                                         handle_command(crimson::net::
                                                        ConnectionRef conn,
                                                        Ref < MCommand > m) {
                                         return asok->handle_command(conn,
                                                                     std::
                                                                     move(m));}
...
*/
                                         ...
                                         return prepare_to_stop().then([this] {
                                                                       ...
                                                                       ([]
                                                                        ...
                                                                        ().
                                                                        error
                                                                        ("error while stopping osd: {}",
                                                                         ep);}
                                                                       );}
                                         );}

                                         void OSD::dump_status(Formatter *
                                                               f) const {
                                         f->
                                         dump_stream("cluster_fsid") <<
                                         superblock.cluster_fsid;
                                         f->
                                         dump_stream("osd_fsid") << superblock.
                                         osd_fsid;
                                         f->dump_unsigned("whoami",
                                                          superblock.whoami);
                                         f->dump_string("state",
                                                        pg_shard_manager.
                                                        get_osd_state_string());
                                         f->dump_unsigned("oldest_map",
                                                          superblock.
                                                          oldest_map);
                                         f->
                                         dump_unsigned
                                         ("cluster_osdmap_trim_lower_bound",
                                          superblock.
                                          cluster_osdmap_trim_lower_bound);
                                         f->dump_unsigned("newest_map",
                                                          superblock.
                                                          newest_map);
                                         f->dump_unsigned("num_pgs",
                                                          pg_shard_manager.
                                                          get_num_pgs());}
                                         ...
                                         whoami << " " << superblock.
                                         osd_fsid << " [" << superblock.
                                         oldest_map << "," << superblock.
                                         newest_map << "] " << "tlb:" <<
                                         superblock.
                                         cluster_osdmap_trim_lower_bound <<
                                         " pgs:" << pg_shard_manager.
                                         get_num_pgs()
                                         << "}";}

                                         std::optional < seastar::future <>>
                                         OSD::ms_dispatch(crimson::net::
                                                          ConnectionRef conn,
                                                          MessageRef m) {
                                         ...
                                         gate.dispatch_in_background(__func__,
                                                                     *this,
                                                                     [this,
                                                                      conn =
                                                                      std::
...
return handle_osd_map(conn, boost::static_pointer_cast < MOSDMap > (m)); case CEPH_MSG_OSD_OP:
return handle_osd_op(conn, boost::static_pointer_cast < MOSDOp > (m)); case MSG_OSD_PG_CREATE2:
return handle_pg_create(conn, boost::static_pointer_cast < MOSDPGCreate2 > (m)); return seastar::now(); case MSG_COMMAND:
return handle_command(conn, boost::static_pointer_cast < MCommand > (m)); case MSG_OSD_MARK_ME_DOWN:
return handle_mark_me_down(conn, boost::static_pointer_cast < MOSDMarkMeDown > (m)); case MSG_OSD_PG_PULL:
[[fallthrough]]; case MSG_OSD_PG_PUSH:
[[fallthrough]]; case MSG_OSD_PG_PUSH_REPLY:
[[fallthrough]]; case MSG_OSD_PG_RECOVERY_DELETE:
[[fallthrough]]; case MSG_OSD_PG_RECOVERY_DELETE_REPLY:
[[fallthrough]]; case MSG_OSD_PG_SCAN:
[[fallthrough]]; case MSG_OSD_PG_BACKFILL:
[[fallthrough]]; case MSG_OSD_PG_BACKFILL_REMOVE:
return handle_recovery_subreq(conn, boost::static_pointer_cast < MOSDFastDispatchOp > (m)); case MSG_OSD_PG_LEASE:
[[fallthrough]]; case MSG_OSD_PG_LEASE_ACK:
[[fallthrough]]; case MSG_OSD_PG_NOTIFY2:
[[fallthrough]]; case MSG_OSD_PG_INFO2:
[[fallthrough]]; case MSG_OSD_PG_QUERY2:
[[fallthrough]]; case MSG_OSD_BACKFILL_RESERVE:
[[fallthrough]]; case MSG_OSD_RECOVERY_RESERVE:
[[fallthrough]]; case MSG_OSD_PG_LOG:
return handle_peering_op(conn, boost::static_pointer_cast < MOSDPeeringOp > (m)); case MSG_OSD_REPOP:
return handle_rep_op(conn, boost::static_pointer_cast < MOSDRepOp > (m)); case MSG_OSD_REPOPREPLY:
return handle_rep_op_reply(conn, boost::static_pointer_cast < MOSDRepOpReply > (m)); case MSG_OSD_SCRUB2:
return handle_scrub(conn, boost::static_pointer_cast < MOSDScrub2 > (m)); case MSG_OSD_PG_UPDATE_LOG_MISSING:
return handle_update_log_missing(conn, boost::static_pointer_cast < MOSDPGUpdateLogMissing > (m)); case MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY:
return handle_update_log_missing_reply(conn, boost::static_pointer_cast < MOSDPGUpdateLogMissingReply > (m)); default:
                                                                     dispatched
                                                                     = false;
                                                                     return
                                                                     seastar::
                                         ...
                                         return (dispatched ? std::
                                                 make_optional(seastar::
                                                               now()) : std::
                                                 nullopt);}

                                         void OSD::ms_handle_reset(crimson::
                                                                   net::
                                                                   ConnectionRef
                                                                   conn,
                                                                   bool
                                         ...
                                         void OSD::
                                         ms_handle_remote_reset(crimson::net::
                                                                ConnectionRef
                                                                conn) {
                                         logger().
                                         warn("ms_handle_remote_reset");}

                                         void OSD::
                                         handle_authentication(const EntityName
                                                               & name,
                                                               const
                                                               AuthCapsInfo &
                                                               caps_info) {
                                         ...
                                         OSDCap caps; if (caps.parse(str)) {
                                         logger().debug("{} {} has caps {}",
                                                        __func__, name, str);}
                                         else {
                                         logger().
                                         warn("{} {} failed to parse caps {}",
                                              __func__, name, str);}
                                         }
                                         }
                                         ...
                                         (static_cast < uint64_t >
                                          (get_shard_services().
                                           get_up_epoch()) << 32) |
                                         osd_stat_seq;
                                         gate.dispatch_in_background("statfs",
                                                                     *this,
                                                                     [this] {
                                                                     (void)
                                                                     store.
                                         ...
                                         auto m =
                                         crimson::make_message < MPGStats >
                                         (monc->get_fsid(),
                                          osdmap->get_epoch());
                                         m->osd_stat = osd_stat;
                                         return pg_shard_manager.get_pg_stats().
                                         then([m = std::move(m)] (auto
                                                                  && stats)
                                              mutable {
                                              m->pg_stat = std::move(stats);
                                              return seastar::
                                              make_ready_future < MessageURef >
                                              (std::move(m));}
                                         );}

                                         uint64_t OSD::send_pg_stats() {
                                         // mgr client sends the report message in background
                                         mgrc->report(); return osd_stat.seq;}

                                         bool OSD::require_mon_peer(crimson::
                                                                    net::
                                                                    Connection *
                                                                    conn,
                                                                    Ref <
                                         ...
                                         if (!conn->peer_is_mon()) {
                                         logger().
                                         info("{} received from non-mon {}, {}",
                                              __func__, conn->get_peer_addr(),
                                              *m); return false;}
                                         return true;}

                                         seastar::future <> OSD::
                                         handle_osd_map(crimson::net::
                                                        ConnectionRef conn,
                                                        Ref < MOSDMap > m) {
                                         ...
                                         logger().
                                         info
                                         ("handle_osd_map epochs [{}..{}], i have {}, src has [{}..{}]",
                                         ...
                                         return seastar::do_with(ceph::os::
                                                                 ...
                                                                 then([ =, this,
                                                                      ...
                                                                      logger().
                                                                      debug
                                                                      ("OSD::handle_osd_map: do_transaction...");
                                                                      return
                                                                      store.
                                                                      get_sharded_store
                                                                      ().
                                                ...
                                                return committed_osd_maps(start,
                                                                          last,
                                                                          m);}
                                         );}
                                         ...
                                         logger().
                                         info
                                         ("osd.{}: committed_osd_maps({}, {})",
                                          whoami, first, last);
                                         // advance through the new maps
                                         return seastar::do_for_each(boost::
                                         ...
                                         seastar::future <> OSD::
                                         handle_osd_op(crimson::net::
                                                       ConnectionRef conn,
                                                       Ref < MOSDOp > m) {
                                         (void)pg_shard_manager.
                                         start_pg_operation < ClientRequest >
                                         (get_shard_services(), conn,
                                          std::move(m)); return seastar::now();}

                                         seastar::future <> OSD::
                                         handle_pg_create(crimson::net::
                                                          ConnectionRef conn,
                                                          Ref < MOSDPGCreate2 >
                                                          m) {
      for (auto &[pgid, when]:          m->pgs) {
                                         const auto &[created, created_stamp] =
                                         when; auto q = m->pg_extra.find(pgid);
                                         ...
                                         if (!pi.empty()
                                             && m->epoch <
                                             pi.get_bounds().second) {
                                         logger().
                                         error("got pg_create on {} epoch {}  "
                                               "unmatched past_intervals {} (history {})",
                                               pgid, m->epoch, pi, history);}
                                         else {
                                         std::ignore =
                                         pg_shard_manager.start_pg_operation <
                                         RemotePeeringEvent > (conn,
                                                               pg_shard_t(),
                                                               pgid, m->epoch,
                                                               m->epoch,
                                                               NullEvt(), true,
                                         ...
                                         seastar::future <> OSD::
                                         handle_update_log_missing(crimson::
                                                                   net::
                                                                   ConnectionRef
                                                                   conn,
                                                                   Ref <
                                         ...
                                         (void)pg_shard_manager.
                                         start_pg_operation <
                                         LogMissingRequest > (std::move(conn),
                                                              std::move(m));
                                         return seastar::now();}
                                         ...
                                         seastar::future <> OSD::
                                         handle_rep_op(crimson::net::
                                                       ConnectionRef conn,
                                                       Ref < MOSDRepOp > m) {
                                         m->finish_decode();
                                         std::ignore =
                                         pg_shard_manager.start_pg_operation <
                                         RepRequest > (std::move(conn),
                                                       std::move(m));
                                         return seastar::now();}

                                         seastar::future <> OSD::
                                         handle_rep_op_reply(crimson::net::
                                                             ConnectionRef conn,
                                                             Ref <
                                                             MOSDRepOpReply >
                                                             m) {
                                         spg_t pgid = m->get_spg();
                                         return pg_shard_manager.with_pg(pgid,
                                                                         [m =
                                                                          std::
                                                                          move
                                                                          (m)]
                                         ...
                                         seastar::future <> OSD::
                                         handle_scrub(crimson::net::
                                                      ConnectionRef conn,
                                                      Ref < MOSDScrub2 > m) {
                                         if (m->fsid != superblock.cluster_fsid) {
                                         logger().warn("fsid mismatched");
                                         return seastar::now();}
                                         return seastar::parallel_for_each(std::
                                                                           move
                                                                           (m->
                                                                            scrub_pgs),
                                                                           [m,
                                         ...
                                         seastar::future <> OSD::
                                         handle_mark_me_down(crimson::net::
                                                             ConnectionRef conn,
                                                             Ref <
                                                             MOSDMarkMeDown >
                                                             m) {
                                         ...
                                         seastar::future <> OSD::
                                         handle_recovery_subreq(crimson::net::
                                                                ConnectionRef
                                                                conn,
                                                                Ref <
                                                                MOSDFastDispatchOp
                                         ...
                                         if (osdmap->get_addrs(whoami) !=
                                             public_msgr->get_myaddrs()) {
                                         logger().
                                         error
                                         ("map e {} had wrong client addr ({} != my {})",
                                          osdmap->get_epoch(),
                                         ...
                                         logger().
                                         error
                                         ("map e {} had wrong cluster addr ({} != my {})",
                                          osdmap->get_epoch(),
                                         ...
                                         osdmap->get_epoch();
                                         auto m =
                                         crimson::make_message < MOSDBeacon >
                                         (osdmap->get_epoch(),
                                         ...
                                         pg_shard_manager.
                                         for_each_pgid([this] (auto & pgid) {
          vector < int >up, acting; osdmap->pg_to_up_acting_osds(pgid.pgid, &up, nullptr, &acting, nullptr); for (int osd:boost::join(up,
                                                                   acting))
                                                       {
                                                       if (osd ==
                                                           CRUSH_ITEM_NONE
                                         ...
                                         (std::chrono::duration <
                                          double >(local_conf().get_val <
                                                   double
                                                   >
                                                   ("osd_mon_shutdown_timeout")));
                                         return seastar::with_timeout(seastar::
                                                                      timer <>::
                                                                      clock::
                                                                      now() +
                                                                      timeout,
