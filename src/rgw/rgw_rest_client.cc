...
int RGWHTTPSimpleRequest::receive_data(void *ptr, size_t len, bool * pause)
{
    ...
    left_len =
        max_response >
        response.length()? (max_response - response.length()) : 0;
    if (left_len == 0)
...
static void do_get_params_str(const param_vec_t & params, map < string,
                              string > &extra_args, string & dest)
{
...
static int sign_request_v2(const DoutPrefixProvider * dpp,
                           ...
                           const string & service, RGWEnv & env,
                           req_info & info, const bufferlist * opt_content)
{
    ...
    if (cct->_conf->subsys.should_gather < ceph_subsys_rgw, 20 > ()) {
      for (const auto & i:env.get_map()) {
            ...
            i.first, i.second} <<dendl;
        }
    }

    string canonical_header;
    if (!rgw_create_s3_canonical_header
        (dpp, info, NULL, canonical_header, false)) {
    ...
    try {
        digest =
            rgw::auth::s3::get_v2_signature(cct, key.key, canonical_header);
    }
    catch(int ret) {
        return ret;
    }
                        ...
                        const bufferlist * opt_content)
{
    ...
    if (authv > 0 && authv <= 3) {
        return sign_request_v2(dpp, key, region, service, env, info,
                               opt_content);
    }
                           ...
                           string * region, string & service)
{
    if (!boost::algorithm::ends_with(host, "amazonaws.com")) {
        ldpp_dout(dpp,
                  20) << "NOTICE: cannot identify region for connection to: " <<
            host << dendl;
        return false;
    }
...
int RGWRESTSimpleRequest::forward_request(const DoutPrefixProvider * dpp,
                                          ...
                                          req_info & info, size_t max_response,
                                          bufferlist * inbl, bufferlist * outbl,
                                          optional_yield y, std::string service)
{
    ...
    size_t pos =
        new_info.request_uri.substr(1,
                                    new_info.request_uri.size() - 1).find("/");
...
{
    dout(20) << "RGWRESTStreamOutCB::handle_data bl.length()=" << bl.
        length() << " bl_ofs=" << bl_ofs << " bl_len=" << bl_len << dendl;
...
static void grants_by_type_add_perm(map < int, string > &grants_by_type,
                                    int perm, ACLGrant & grant)
{
...
cct(_cct), new_env(_env), new_info(_info)
{
}

void RGWRESTGenerateHTTPHeaders::init(const string & _method,
                                      const string & host,
...
void RGWRESTGenerateHTTPHeaders::set_extra_headers(const map < string,
...
int RGWRESTGenerateHTTPHeaders::set_obj_attrs(const DoutPrefixProvider * dpp,
                                              map < string,
                                              bufferlist > &rgw_attrs)
{
  ...
  for (auto & attr:rgw_attrs) {
        ...
        string val = bl.c_str();
        if (name.
            compare(0, sizeof(RGW_ATTR_META_PREFIX) - 1,
...
void RGWRESTGenerateHTTPHeaders::set_http_attrs(const map < string,
                                                string > &http_attrs)
{
    /* merge send headers */
...
{
    int ret =
        sign_request(dpp, key, region, service, *new_env, *new_info,
                     opt_content);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to sign request" << dendl;
        return ret;
...
void RGWRESTStreamS3PutObj::send_init(const rgw_obj & obj)
{
    ...
    method = "PUT";
    headers_gen.init(method, new_host, resource_prefix, new_url, resource,
                     params, api_name);
...
void RGWRESTStreamS3PutObj::send_ready(const DoutPrefixProvider * dpp,
                                       RGWAccessKey & key, const map < string,
                                       string > &http_attrs,
                                       RGWAccessControlPolicy & policy)
{
...
int RGWRESTStreamRWRequest::send(RGWHTTPManager * mgr)
{
    if (!headers_gen) {
        ldpp_dout(this,
                  0) << "ERROR: " << __func__ <<
            "(): send_prepare() was not called: likey a bug!" << dendl;
        return -EINVAL;
    }

    const bufferlist *outblp {
    nullptr};
                                             ...
                                             map < string, string > *pheaders)
{
    ...
    for (auto iter = out_headers.begin(); pattrs && iter != out_headers.end();
        ...
        if (attr_name.
            compare(0, sizeof(RGW_HTTP_RGWX_ATTR_PREFIX) - 1,
                    RGW_HTTP_RGWX_ATTR_PREFIX) == 0) {
            string name =
                attr_name.substr(sizeof(RGW_HTTP_RGWX_ATTR_PREFIX) - 1);
                                          ...
                                          const string & val)
{
    if (name == "RGWX_EMBEDDED_METADATA_LEN") {
        ...
        if (!err.empty()) {
            ldpp_dout(this,
                      0) << "ERROR: failed converting embedded metadata len ("
                << val << ") to int " << dendl;
            return -EINVAL;
        }
