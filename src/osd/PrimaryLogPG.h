...
class PrimaryLogPG:public PG, public PGBackend::Listener {
   ...
   */
    ...
    struct CopyOp {
        ...
        CopyOp(CopyCallback * cb_, ObjectContextRef _obc, hobject_t s,
               ...
               unsigned f,
               bool ms,
               unsigned src_obj_fadvise_flags, unsigned dest_obj_fadvise_flags)
   ...
   */
    ...
    struct ProxyReadOp {
        ...
        ProxyReadOp(OpRequestRef _op, hobject_t oid,
                    std::vector < OSDOp > &_ops)
        :op(_op), soid(oid), objecter_tid(0), ops(_ops), user_version(0),
            data_offset(0), canceled(false) {
    }};
    ...
    struct ProxyWriteOp {
        ...
        ProxyWriteOp(OpRequestRef _op, hobject_t oid,
                     std::vector < OSDOp > &_ops, osd_reqid_t _reqid)
        :ctx(NULL), op(_op), soid(oid), objecter_tid(0), ops(_ops),
            user_version(0), sent_reply(false), canceled(false), reqid(_reqid) {
    }};
    ...
    /// Listener methods DoutPrefixProvider *get_dpp() override {
        return this;
    }
    ...
    void remove_missing_object(const hobject_t & oid,
                               eversion_t v, Context * on_complete) override;
    ...
    GenContext < ThreadPool::TPHandle & >*bless_gencontext(GenContext <
                                                           ThreadPool::
                                                           TPHandle & >*c)
        override;
    GenContext < ThreadPool::TPHandle & >*bless_unlocked_gencontext(GenContext <
                                                                    ThreadPool::
                                                                    TPHandle &
                                                                    >*c)
        override;
    ...
    epoch_t get_interval_start_epoch() const override {
        ...
        pg_missing_t > &get_shard_missing() const override {
    ...
    void log_operation(std::vector < pg_log_entry_t > &&logv,
                       ...
                       &hset_history, const eversion_t & trim_to,
                       const eversion_t & roll_forward_to,
                       const eversion_t & min_last_complete_ondisk,
                       bool transaction_applied, ObjectStore::Transaction & t,
        ...
        recovery_state.append_log(std::move(logv), trim_to, roll_forward_to,
                                  min_last_complete_ondisk, t,
    ...
    bool pg_is_undersized() const override {
        ...
        return is_repair();
    } void update_peer_last_complete_ondisk(pg_shard_t fromosd,
    ...
    void update_stats(const pg_stat_t & stat) override {
        recovery_state.update_stats([&stat] (auto & history, auto & stats) {
                                    stats = stat; return false;}
        );
    }

    void schedule_recovery_work(GenContext < ThreadPool::TPHandle & >*c,
    ...
    } void send_message_osd_cluster(int peer, Message * m,
                                    epoch_t from_epoch) override {
        osd->send_message_osd_cluster(peer, m, from_epoch);
    }
    void send_message_osd_cluster(std::vector < std::pair < int,
                                  Message * >>&messages,
                                  epoch_t from_epoch) override {
    ...
    void send_message_osd_cluster(Message * m,
                                  const ConnectionRef & con) override {
   ...
   */
    ...
    void complete_disconnect_watches(ObjectContextRef obc,
                                     const std::list < watch_disconnect_t >
                                     &to_disconnect);
    ...
    struct OpContext {
        ...
        template < typename F > void register_on_finish(F && f) {
            on_finish.emplace_back(std::forward < F > (f));
        }
        template < typename F > void register_on_success(F && f) {
            on_success.emplace_back(std::forward < F > (f));
        }
        template < typename F > void register_on_applied(F && f) {
            on_applied.emplace_back(std::forward < F > (f));
        }
        template < typename F > void register_on_commit(F && f) {
      ...
      OpContext(OpRequestRef _op, osd_reqid_t _reqid, std::vector < OSDOp > *_ops, ObjectContextRef & obc, PrimaryLogPG * _pg):
        op(_op), reqid(_reqid), ops(_ops),
            obs(&obc->obs),
            snapset(0),
            new_obs(obs->oi, obs->exists),
            modify(false), user_modify(false), undirty(false),
            cache_operation(false), ignore_cache(false),
            ignore_log_op_stats(false), update_log_only(false),
            bytes_written(0), bytes_read(0), user_at_version(0),
            current_osd_subop_num(0), obc(obc), reply(NULL), pg(_pg),
            num_read(0), num_write(0), sent_reply(false), inflightreads(0),
      ...
      OpContext(OpRequestRef _op, osd_reqid_t _reqid, std::vector < OSDOp > *_ops, PrimaryLogPG * _pg):
        op(_op), reqid(_reqid), ops(_ops), obs(NULL), snapset(0),
            ...
            ignore_log_op_stats(false), update_log_only(false),
            bytes_written(0), bytes_read(0), user_at_version(0),
            current_osd_subop_num(0), reply(NULL), pg(_pg), num_read(0),
            num_write(0), inflightreads(0), lock_type(RWState::RWNONE) {
        }
        ...
        ~OpContext() {
            ...
            if (reply)
                reply->put();
            for (std::list < std::pair < boost::tuple < uint64_t, uint64_t,
                 unsigned >, std::pair < ceph::buffer::list *,
                 Context * > > >::iterator i = pending_async_reads.begin();
    ...
    class RepGather {
      public:
        ...
        } RepGather(ObcLockManager && manager,
                    OpRequestRef && o,
                    std::optional < std::function < void (void) >
                    >&&on_complete, ceph_tid_t rt, eversion_t lc, int r):op(o),
            queue_item(this), nref(1), r(r), rep_tid(rt), rep_aborted(false),
            all_committed(false), pg_local_last_complete(lc),
   ...
   */
    ...
    boost::intrusive_ptr < RepGather > new_repop(eversion_t version,
                                                 int r,
   ...
   */
    void submit_log_entries(const mempool::osd_pglog::list < pg_log_entry_t >
                            &entries, ObcLockManager
                            && manager,
                            std::optional < std::function < void (void) >
                            >&&on_complete, OpRequestRef op =
                            OpRequestRef(), int r = 0);
    struct LogUpdateCtx {
    ...
    bool agent_choose_mode(bool restart = false, OpRequestRef op =
                           OpRequestRef());
  ...
  protected:
    ...
    SnapSetContext *get_snapset_context(const hobject_t & oid, bool can_create, const std::map < std::string, ceph::buffer::list, std::less <>> *attrs = 0, bool oid_existed = true //indicate this oid whether exsited in backend
        );
    ...
    void dump_recovery_info(ceph::Formatter * f) const override {
        ...
        for (std::set < pg_shard_t >::const_iterator p =
             waiting_on_backfill.begin(); p != waiting_on_backfill.end(); ++p)
            f->dump_stream("osd") << *p;
        f->close_section();
        ...
        }
        {
            ...
            for (std::set < hobject_t >::const_iterator i =
                 backfills_in_flight.begin(); i != backfills_in_flight.end();
                 ++i) {
        ...
        }
        {
            ...
            for (std::map < hobject_t, ObjectContextRef >::const_iterator i =
                 recovering.begin(); i != recovering.end(); ++i) {
    ...
    int recover_missing(const hobject_t & oid,
                        eversion_t v,
                        int priority, PGBackend::RecoveryHandle * h);

    // low level ops

    void _make_clone(OpContext * ctx,
                     PGTransaction * t,
    ...
    inline void truncate_update_size_and_usage(object_stat_sum_t & delta_stats,
                                               object_info_t & oi,
                                               uint64_t truncate_size);
    ...
    bool maybe_handle_manifest(OpRequestRef op,
                               bool write_ordered, ObjectContextRef obc) {
        return cache_result_t::NOOP != maybe_handle_manifest_detail(op,
                                                                    write_ordered,
                                                                    obc);
    }
   ...
   */
    bool maybe_handle_cache(OpRequestRef op,
                            ...
                            bool must_promote, bool in_hit_set = false) {
        return cache_result_t::NOOP != maybe_handle_cache_detail(op,
                                                                 write_ordered,
                                                                 obc,
                                                                 r,
                                                                 missing_oid,
   ...
   */
    ...
    std::list < std::pair < OpRequestRef,
        OpContext * > >in_progress_async_reads;
    ...
    bool start_recovery_ops(uint64_t max,
   ...
   */
    void scan_range(int min, int max, BackfillInterval * bi,
                    ThreadPool::TPHandle & handle);
    ...
    int prep_backfill_object_push(hobject_t oid, eversion_t v,
                                  ObjectContextRef obc,
    ...
    int do_copy_get(OpContext * ctx, ceph::buffer::list::const_iterator & bp,
                    OSDOp & op, ObjectContextRef & obc);
    int finish_copy_get();
   ...
   */
    ...
    int start_flush(OpRequestRef op, ObjectContextRef obc,
    ...
    void cancel_cls_gather(std::map < hobject_t, CLSGatherOp >::iterator iter,
                           bool requeue, std::vector < ceph_tid_t > *tids);
    void cancel_cls_gather_ops(bool requeue, std::vector < ceph_tid_t > *tids);

    // -- scrub --
    bool _range_available_for_scrub(const hobject_t & begin,
    ...
    // -- checksum --
    int do_checksum(OpContext * ctx, OSDOp & osd_op,
    ...
    std::pair < int,
        std::unique_ptr <
        const PGLSFilter >> get_pgls_filter(ceph::buffer::list::
                                            const_iterator & iter);
    ...
    void do_proxy_chunked_read(OpRequestRef op, ObjectContextRef obc,
                               int op_index, uint64_t chunk_index,
                               uint64_t req_offset, uint64_t req_length,
    ...
    void process_copy_chunk_manifest(hobject_t oid, ceph_tid_t tid, int r,
                                     uint64_t offset);
    void finish_promote_manifest(int r, CopyResults * results,
                                 ObjectContextRef obc);
    void cancel_and_requeue_proxy_ops(hobject_t oid);
    void cancel_manifest_ops(bool requeue, std::vector < ceph_tid_t > *tids);
    ceph_tid_t refcount_manifest(hobject_t src_soid, hobject_t tgt_soid,
                                 refcount_t type, Context * cb,
    ...
    bool inc_refcount_by_set(OpContext * ctx, object_manifest_t & tgt,
                             OSDOp & osd_op);
    int do_cdc(const object_info_t & oi, std::map < uint64_t,
               chunk_info_t > &chunk_map, std::map < uint64_t,
               bufferlist > &chunks);
    int start_dedup(OpRequestRef op, ObjectContextRef obc);
    std::pair < int, hobject_t > get_fpoid_from_chunk(const hobject_t soid,
                                                      bufferlist & chunk);
    int finish_set_dedup(hobject_t oid, int r, ceph_tid_t tid, uint64_t offset);
    int finish_set_manifest_refcount(hobject_t oid, int r, ceph_tid_t tid,
                                     uint64_t offset);
  ...
  public:
    ...
    void do_command(const std::string_view & prefix,
    ...
    int _get_tmap(OpContext * ctx, ceph::buffer::list * header,
                  ceph::buffer::list * vals);
    int do_tmap2omap(OpContext * ctx, unsigned flags);
    int do_tmapup(OpContext * ctx, ceph::buffer::list::const_iterator & bp,
                  OSDOp & osd_op);
    int do_tmapup_slow(OpContext * ctx, ceph::buffer::list::const_iterator & bp,
                       OSDOp & osd_op, ceph::buffer::list & bl);

    void do_osd_op_effects(OpContext * ctx, const ConnectionRef & conn);
    int start_cls_gather(OpContext * ctx, std::map < std::string,
                         bufferlist > *src_objs, const std::string & pool,
  ...
  private:
    ...
    struct NotTrimming;
    struct SnapTrimmer:public boost::statechart::state_machine < SnapTrimmer,
        NotTrimming > {
        PrimaryLogPG *pg;
    ...
    struct Trimming:boost::statechart::state < Trimming, SnapTrimmer,
        WaitReservation >, NamedState {
        typedef boost::mpl::list < boost::statechart::custom_reaction <
            KickTrim >, boost::statechart::transition < Reset,
            NotTrimming > >reactions;
    ...
    struct WaitTrimTimer:boost::statechart::state < WaitTrimTimer, Trimming >,
        ...
        :my_base(ctx), NamedState(nullptr, "Trimming/WaitTrimTimer") {
            ...
            if (osd_snap_trim_sleep > 0) {
                ...
                wakeup =
                    pg->osd->sleep_timer.add_event_after(osd_snap_trim_sleep,
                                                         new OnTimer {
                                                         pg,
                                                         pg->get_osdmap_epoch()}
                );
    ...
    struct WaitReservation:boost::statechart::state < WaitReservation,
        Trimming >, NamedState {
        /* WaitReservation is a sub-state of trimming simply so that exiting Trimming
         * always cancels the reservation */
        typedef boost::mpl::list <
            boost::statechart::custom_reaction < SnapTrimReserved > >reactions;
        ...
        :my_base(ctx), NamedState(nullptr, "Trimming/WaitReservation") {
            ...
            pg->osd->snap_reserver.request_reservation(pg->get_pgid(),
                                                       pending, 0);
    ...
    struct WaitScrub:boost::statechart::state < WaitScrub, SnapTrimmer >,
        NamedState {
        typedef boost::mpl::list < boost::statechart::custom_reaction <
            ScrubComplete >, boost::statechart::custom_reaction < KickTrim >,
            boost::statechart::transition < Reset, NotTrimming > >reactions;
    ...
    void maybe_create_new_object(OpContext * ctx, bool ignore_transaction =
                                 false);
  ...
  public:
    ...
    bool is_unreadable_object(const hobject_t & oid) const {
        return is_missing_object(oid) ||
            !recovery_state.get_missing_loc().readable_with_acting(oid,
                                                                   get_actingset
                                                                   ());
    ...
    void block_write_on_snap_rollback(const hobject_t & oid,
                                      ObjectContextRef obc, OpRequestRef op);
    ...
    // attr cache handling
    void setattr_maybe_cache(ObjectContextRef obc,
                             PGTransaction * t,
                             const std::string & key, ceph::buffer::list & val);
    void setattrs_maybe_cache(ObjectContextRef obc,
                              PGTransaction * t,
                              std::map < std::string, ceph::buffer::list,
                              std::less <>> &attrs);
    void rmattr_maybe_cache(ObjectContextRef obc, PGTransaction * t,
   ...
   */
    int getattr_maybe_cache(ObjectContextRef obc,
                            const std::string & key, ceph::buffer::list * val);
    int getattrs_maybe_cache(ObjectContextRef obc,
                             std::map < std::string, ceph::buffer::list,
                             std::less <>> *out);

  public:
    void set_dynamic_perf_stats_queries(const std::list < OSDPerfMetricQuery >
                                        &queries) override;
...
inline ostream & operator<<(ostream & out,
                            const PrimaryLogPG::RepGather & repop)
{
    out << "repgather(" << &repop
        << " " << repop.v
...
{
    out << "proxywrite(" << &pwop
        << " " << pwop->user_version << " pwop_tid=" << pwop->objecter_tid;
    if (pwop->ctx->op)
