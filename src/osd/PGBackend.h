...
class PGBackend {
  ...
  public:
    ...
    */
    class Listener {
      ...
      */
        virtual void on_local_recover(const hobject_t & oid,
      ...
      */
        virtual void on_global_recover(const hobject_t & oid,
      ...
      */
        virtual void on_peer_recover(pg_shard_t peer,
                                     const hobject_t & oid,
                                     const ObjectRecoveryInfo & recovery_info) =
            0;

        virtual void begin_peer_recover(pg_shard_t peer,
        ...
        virtual void apply_stats(const hobject_t & soid,
                                 const object_stat_sum_t & delta_stats) = 0;

     /**
      * Called when a read on the primary fails when pushing
      */
        virtual void on_primary_error(const hobject_t & oid, eversion_t v) = 0;

        virtual void backfill_add_missing(const hobject_t & oid,
                                          eversion_t v) = 0;
      ...
      */
        ...
        virtual GenContext <
            ThreadPool::TPHandle & >*bless_gencontext(GenContext <
                                                      ThreadPool::
                                                      TPHandle & >*c) = 0;

        virtual void send_message(int to_osd, Message * m) = 0;
        virtual void queue_transaction(ObjectStore::Transaction && t,
                                       OpRequestRef op = OpRequestRef()
            ) = 0;
        virtual void queue_transactions(vector < ObjectStore::Transaction >
                                        &tls, OpRequestRef op = OpRequestRef()
            ) = 0;
        ...
        virtual const map < hobject_t,
            set < pg_shard_t >> &get_missing_loc_shards()
            const = 0;
        ...
        virtual boost::optional <
            const pg_missing_const_i & >maybe_get_shard_missing(pg_shard_t peer)
            const {
        ...
        virtual const pg_missing_const_i & get_shard_missing(pg_shard_t peer) const {
            ...
            assert(m);
            return *m;
        } virtual const map < pg_shard_t, pg_info_t > &get_shard_info() const =
            0;
        ...
        virtual bool try_lock_for_read(const hobject_t & hoid,
        ...
        virtual bool should_send_op(pg_shard_t peer,
                                    const hobject_t & hoid) = 0;

        virtual void log_operation(const vector < pg_log_entry_t > &logv,
        ...
        virtual void pgb_set_object_snap_mapping(const hobject_t & soid,
                                                 const set < snapid_t > &snaps,
                                                 ObjectStore::Transaction * t) =
            0;

        virtual void pgb_clear_object_snap_mapping(const hobject_t & soid,
                                                   ObjectStore::Transaction *
                                                   t) = 0;

        virtual void update_peer_last_complete_ondisk(pg_shard_t fromosd,
                                                      eversion_t lcod) = 0;
        ...
        virtual void schedule_recovery_work(GenContext <
                                            ThreadPool::TPHandle & >*c) = 0;
        ...
        virtual void send_message_osd_cluster(int peer, Message * m,
                                              epoch_t from_epoch) = 0;
        virtual void send_message_osd_cluster(Message * m, Connection * con) =
            0;
        virtual void send_message_osd_cluster(Message * m,
                                              const ConnectionRef & con) = 0;
        virtual ConnectionRef get_con_osd_cluster(int peer,
                                                  epoch_t from_epoch) = 0;
        ...
        virtual bool check_osdmap_full(const set < pg_shard_t > &missing_on) =
            0;
    ...
    Listener *get_parent() const {
        return parent;
    } PGBackend(CephContext * cct, Listener * l, ObjectStore * store,
                coll_t coll, ObjectStore::CollectionHandle & ch):cct(cct),
        store(store), coll(coll), ch(ch), parent(l) {
    }
  ...
  public:
    ...
    virtual void submit_transaction(const hobject_t & hoid, ///< [in] object
                                    ...
                                    boost::optional < pg_hit_set_history_t > &hset_history, Context * on_local_applied_sync,    ///< [in] called when applied locally
    ...
    void try_stash(const hobject_t & hoid,
  ...
  protected:
    ...
    void rollback_setattrs(const hobject_t & hoid,
                           map < string,
                           boost::optional < bufferlist > >&old_attrs,
                           ObjectStore::Transaction * t);

    /// Truncate object to rollback append
    virtual void rollback_append(const hobject_t & hoid,
                                 uint64_t old_size,
                                 ObjectStore::Transaction * t);

    /// Unstash object to rollback stash
    void rollback_stash(const hobject_t & hoid,
                        version_t old_version, ObjectStore::Transaction * t);

    /// Unstash object to rollback stash
    void rollback_try_stash(const hobject_t & hoid,
                            version_t old_version,
                            ObjectStore::Transaction * t);

    /// Delete object to rollback create
    void rollback_create(const hobject_t & hoid, ObjectStore::Transaction * t) {
        remove(hoid, t);
    }
  ...
  public:
    ...
    void trim_rollback_object(const hobject_t & hoid,
                              version_t gen, ObjectStore::Transaction * t);

    /// List objects in collection
    int objects_list_partial(const hobject_t & begin,
                             int min,
                             int max,
                             vector < hobject_t > *ls, hobject_t * next);

    int objects_list_range(const hobject_t & start,
                           const hobject_t & end,
                           snapid_t seq,
                           vector < hobject_t > *ls,
                           vector < ghobject_t > *gen_obs = 0);

    int objects_get_attr(const hobject_t & hoid,
                         const string & attr, bufferlist * out);

    virtual int objects_get_attrs(const hobject_t & hoid,
                                  map < string, bufferlist > *out);

    virtual int objects_read_sync(const hobject_t & hoid,
                                  uint64_t off,
                                  uint64_t len,
                                  uint32_t op_flags, bufferlist * bl) = 0;
    ...
    bool be_compare_scrub_objects(pg_shard_t auth_shard,
    ...
    map < pg_shard_t,
        ScrubMap *
        >::const_iterator be_select_auth_object(const hobject_t & obj,
                                                const map < pg_shard_t,
                                                ScrubMap * >&maps,
                                                ...
                                                ostream & errorstream);
    void be_compare_scrubmaps(const map < pg_shard_t, ScrubMap * >&maps,
                              const set < hobject_t > &master_set, bool repair,
                              ...
                              ostream & errorstream);
    virtual uint64_t be_get_ondisk_size(uint64_t logical_size) = 0;
    virtual int be_deep_scrub(const hobject_t & oid,
                              ScrubMap & map,
                              ScrubMapBuilder & pos, ScrubMap::object & o) = 0;
    void be_large_omap_check(const map < pg_shard_t, ScrubMap * >&maps,
                             const set < hobject_t > &master_set,
                             int &large_omap_objects,
                             ostream & warnstream) const;

    static PGBackend *build_pg_backend(const pg_pool_t & pool,
