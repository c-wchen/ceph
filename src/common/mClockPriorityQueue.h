    ...
    namespace dmc = crimson::dmclock;
     ...
     template < typename T, typename K >
        ...
        struct SubQueue {
          ...
          public:
            ...
            void remove_by_class(K k, std::list < T > *out) {
                ...
                if (out) {
                    for (auto j = i->second.rbegin(); j != i->second.rend();
                         ++j) {
      ...
      public:
        ...
        mClockQueue(const typename dmc::PullPriorityQueue < K,
                    T >::ClientInfoFunc & info_func):queue(info_func, true) {
            // empty
        }
        ...
        void remove_by_filter(std::function < bool(const T &) > filter_accum) {
            ...
            for (auto i = queue_front.rbegin(); i != queue_front.rend();
                 /* no-inc */ ) {
                if (filter_accum(i->second)) {
                    i = decltype(i) {
                    queue_front.erase(std::next(i).base())};
                }
                else {
                    ++i;
                }
        ...
        void remove_by_class(K k, std::list < T > *out = nullptr) override final {
            if (out) {
                queue.remove_by_client(k, true,[&out] (const T & t) {
                                       out->push_front(t);
                                       });
            }
            else {
            ...
            for (auto i = high_queue.begin(); i != high_queue.end();
                 /* no-inc */ ) {
        ...
        } void enqueue_strict_front(K cl, unsigned priority,
                                    T item) override final {
            high_queue[priority].enqueue_front(cl, 0, item);
        } void enqueue(K cl, unsigned priority, unsigned cost,
                       T item) override final {
            // priority is ignored
            queue.add_request(std::move(item), cl, cost);
        } void enqueue_front(K cl,
                             unsigned priority,
                             unsigned cost, T item) override final {
