...
const std::string POOL_ID_KEY {
    "pool_id"};
const std::string POOL_NAME_KEY {
    "pool_name"};
const std::string POOL_NAMESPACE_KEY {
    "pool_namespace"};
const std::string IMAGE_NAME_KEY {
    "image_name"};
const std::string IMAGE_ID_KEY {
    "image_id"};
const std::string SNAP_NAME_KEY {
    "snap_name"};
const std::string SNAP_ID_KEY {
    "snap_id"};

} // anonymous namespace template < typename I >
std::string NativeFormat < I >::build_source_spec(int64_t pool_id,
        const std::
        string &
        pool_namespace,
        const std::
...
template < typename I >
NativeFormat < I >::NativeFormat(I *image_ctx,
                                 const json_spirit::
                                 mObject &json_object,
                                 bool import_only)
...
void NativeFormat < I >::open(Context *on_finish)
{
    ...
    if (pool_name_val.type() == json_spirit::str_type) {
        ...
        int r =
            rados.ioctx_create(pool_name_val.get_str().c_str(), io_ctx);
        if (r < 0) {
    ...
    if (m_pool_id != -1 && pool_id_val.type() != json_spirit::null_type) {
        lderr(cct) << "cannot specify both pool name and pool id" <<
                   dendl;
        on_finish->complete(-EINVAL);
        return;
        ...
        try {
            m_pool_id =
                boost::lexical_cast < int64_t > (pool_id_val.get_str());
    ...
    if (!m_snap_name.empty()
        && snap_id_val.type() != json_spirit::null_type) {
        lderr(cct) << "cannot specify both snap name and snap id" <<
                   dendl;
        on_finish->complete(-EINVAL);
        return;
    ...
    if (m_import_only && m_snap_name.empty()
        && m_snap_id == CEPH_NOSNAP) {
    ...
    if (m_image_ctx->child != nullptr) {
        ...
        if (m_image_ctx->child->config.template get_val < bool >
            ("rbd_balance_parent_reads")) {
            m_image_ctx->
            set_read_flag(librados::OPERATION_BALANCE_READS);
        } else if (m_image_ctx->child->config.template get_val < bool >
...
template < typename I >
void NativeFormat < I >::handle_snap_set(int r,
        Context *on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;
...
template < typename I >
void NativeFormat < I >::get_snapshots(SnapInfos *snap_infos,
                                       Context *on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template < typename I >
void NativeFormat < I >::get_image_size(uint64_t snap_id,
                                        uint64_t *size,
                                        Context *on_finish)
{
    auto cct = m_image_ctx->cct;
