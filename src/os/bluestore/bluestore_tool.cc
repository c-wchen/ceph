...
const char *find_device_path(int id,
                             CephContext * cct, const vector < string > &devs)
{
  for (auto & i:devs) {
...
void parse_devices(CephContext * cct,
...
void add_devices(BlueFS * fs, CephContext * cct, const vector < string > &devs)
{
  ...
  for (auto e:got) {
        ...
        if (r < 0) {
            cerr << "unable to open " << e.
                first << ": " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
    }
}

BlueFS *open_bluefs_readonly(CephContext * cct,
                             const string & path, const vector < string > &devs)
{
...
static void bluefs_import(const string & input_file,
                          ...
                          const string & path, const vector < string > &devs)
{
    ...
    if (!f) {
        r = -errno;
        cerr << "open " << input_file.
            c_str() << " failed: " << cpp_strerror(r) << std::endl;
        exit(EXIT_FAILURE);
    }
    BlueStore bluestore(cct, path);
    KeyValueDB *db_ptr;
    ...
    if (r < 0) {
        cerr << "error preparing db environment: " << cpp_strerror(r) << std::
            endl;
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv)
{
    ...
    po_options.add_options()
        ...
        ("dev-target", po::value < string > (&dev_target),
        ...
        ("sharding", po::value < string > (&new_sharding),
         "new sharding to apply")
    ...
    po_positional.add_options()
        ("command", po::value < string > (&action),
         ...
         "free-dump, "
         "free-score, "
         "free-fragmentation, " "bluefs-stats, " "reshard, " "show-sharding");
    ...
    try {
        po::parsed_options parsed =
            po::command_line_parser(argc,
                                    argv).options(po_all).allow_unregistered().
            run();
        po::store(parsed, vm);
        po::notify(vm);
  ...
  for (auto & i:ceph_option_strings) {
    ...
    auto cct =
        global_init(NULL, args,
                    osd_instance.
                    empty()? CEPH_ENTITY_TYPE_CLIENT : CEPH_ENTITY_TYPE_OSD,
                    CODE_ENVIRONMENT_UTILITY,
                    osd_instance.
                    empty()? CINIT_FLAG_NO_DEFAULT_CONFIG_FILE : 0);
    ...
    if (action == "show-label") {
        if (devs.empty() && path.empty()) {
            cerr << "must specify bluestore path *or* raw device(s)" << std::
                endl;
            exit(EXIT_FAILURE);
        }
        if (devs.empty())
    ...
    if (action == "bluefs-export" ||
        action == "bluefs-import" || action == "bluefs-log-dump") {
        if (path.empty()) {
            cerr << "must specify bluestore path" << std::endl;
            exit(EXIT_FAILURE);
        }
    ...
    if (action == "bluefs-bdev-new-db" || action == "bluefs-bdev-new-wal") {
        ...
        if (dev_target.empty()) {
            cout <<
                "NOTICE: --dev-target option omitted, will allocate as a file"
                << std::endl;
        }
        inferring_bluefs_devices(devs, path);
    }
    if (action == "bluefs-bdev-migrate") {
        ...
        if (devs_source.size() == 0) {
            cerr << "must specify source devices with --devs-source" << std::
                endl;
            exit(EXIT_FAILURE);
        }
        if (dev_target.empty()) {
    ...
    if (action == "free-score" || action == "free-dump"
      ...
      for (auto name:allocs_name) {
            if (!name.empty() &&
                name != "block" &&
                name != "bluefs-db" && name != "bluefs-wal") {
                cerr << "unknown allocator '" << name << "'" << std::endl;
                exit(EXIT_FAILURE);
            }
        }
        if (allocs_name.empty())
            allocs_name = vector < string > {
            "block", "bluefs-db", "bluefs-wal"};
    }
    if (action == "reshard") {
        if (path.empty()) {
            cerr << "must specify bluestore path" << std::endl;
    ...
    if (action == "restore_cfb") {
        ...
        cerr << action << " bluestore.restore_cfb is not supported!!! " << std::
            endl;
        exit(EXIT_FAILURE);
#else
        ...
        cerr << action << " bluestore.allocmap is not supported!!! " << std::
            endl;
        exit(EXIT_FAILURE);
#else
        ...
        if (r < 0) {
            ...
            cerr << action << " status: remaining " << r <<
                " error(s) and warning(s)" << std::endl;
            exit(EXIT_FAILURE);
        }
        else {
            cout << action << " success" << std::endl;
      ...
      for (auto kk:{
            ...
            v += "\n";
            int fd =::open(p.c_str(), O_CREAT | O_TRUNC | O_WRONLY | O_CLOEXEC,
                           0600);
            if (fd < 0) {
      ...
      for (auto & dir:dirs) {
          ...
          for (auto & file:ls) {
                ...
                if (size > 0) {
                    ...
                    while (left) {
                        ...
                        if (r <= 0) {
                            cerr << "read " << dir << "/" << file << " from " <<
                                pos << " failed: " << cpp_strerror(r) << std::
                                endl;
                            exit(EXIT_FAILURE);
                        }
        ...
        auto[target_path, has_size_spec] =
            ...
            std::error_code ec;
            fs::path target_path = fs::weakly_canonical(fs::path {
                                                        dev_target}
                                                        , ec);
            if (ec) {
        ...
        int r =
            bluestore.
            add_new_bluefs_device(need_db ? BlueFS::BDEV_NEWDB : BlueFS::
                                  BDEV_NEWWAL,
                                  target_path);
        if (r == 0) {
        ...
        if (i != cur_devs_map.end()) {
            ...
            if (r == 0) {
              for (auto src:src_devs) {
                    if (src.second != BlueFS::BDEV_SLOW) {
                        cout << " device removed:" << src.second << " " << src.
                            first << std::endl;
                    }
                }
            }
            ...
            if (src_dev_ids.count(BlueFS::BDEV_DB)) {
                ...
                cerr << "Unable to migrate Slow volume to new location, "
                    ...
                    "--bluefs-bdev-new-db(wal) command" << std::endl;
                exit(EXIT_FAILURE);
            }
            if (!dev_target.empty() &&
            ...
            int r = bluestore.migrate_to_new_bluefs_device(src_dev_ids,
                                                           dev_target_id,
                                                           target_path);
            if (r == 0) {
              for (auto src:src_devs) {
      ...
      for (auto alloc_name:allocs_name) {
        ...
        r = admin_socket->execute_command( {
                                          ...
                                          , in, err, &out);
        if (r != 0) {
            cerr << "failure querying bluefs stats: " << cpp_strerror(r) <<
                std::endl;
            exit(EXIT_FAILURE);
        }
        cout << std::string(out.c_str(), out.length()) << std::endl;
        ...
        auto get_ctrl =[&](size_t & val) {
            if (!resharding_ctrl.empty()){
                                          ...
                                          char *endptr;
                                          val =
                                          strtoll(token.c_str(), &endptr, 0);
                                          if (*endptr != '\0') {
                                          cerr << "invalid --resharding-ctrl. '"
                                          ...
                                          if (!resharding_ctrl.empty()) {
                                          cerr <<
                                          "extra chars in --resharding-ctrl" <<
                                          std::endl; exit(EXIT_FAILURE);}
                                          }
                                          int r =
                                          bluestore.open_db_environment(&db_ptr,
                                                                        true);
                                          if (r < 0) {
                                          cerr <<
                                          "error preparing db environment: " <<
                                          cpp_strerror(r) << std::endl;
                                          exit(EXIT_FAILURE);}
                                          ceph_assert(db_ptr);
                                          RocksDBStore * rocks_db =
                                          dynamic_cast <
                                          RocksDBStore * >(db_ptr);
                                          ceph_assert(rocks_db);
                                          r =
                                          rocks_db->reshard(new_sharding,
                                                            &ctrl); if (r < 0) {
                                          cerr << "error resharding: " <<
                                          cpp_strerror(r) << std::endl;}
                                          else {
                                          cout << "reshard success" << std::
                                          endl;}
                                          bluestore.close_db_environment();}
                                          ...
                                          std::string sharding;
                                          bool res =
                                          rocks_db->get_sharding(sharding);
                                          bluestore.close_db_environment();
                                          if (!res) {
                                          cerr <<
