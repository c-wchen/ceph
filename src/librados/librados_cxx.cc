...
void librados::ObjectOperation::cmpext(uint64_t off,
                                       const bufferlist & cmp_bl, int *prval)
{
    ceph_assert(impl);
    ::ObjectOperation * o = &impl->o;
    bufferlist c = cmp_bl;
...
void librados::NObjectIteratorImpl::get_next()
{
    ...
    cur_obj.impl->nspace = string {
    nspace, nspace_size};
    cur_obj.impl->oid = string {
    entry, entry_size};
...
librados::IoCtx::IoCtx(IoCtx && rhs) noexcept:io_ctx_impl(std::
            exchange(rhs.io_ctx_impl,
                     nullptr))
{
}
...
bool librados::IoCtx::is_valid() const const
{
...
int librados::IoCtx::operate(const std::string & oid,
                             librados::ObjectWriteOperation * o, int flags)
{
    object_t obj(oid);
    if (unlikely(!o->impl))
        return -EINVAL;
                                 ...
                                 int flags, const blkin_trace_info * trace_info)
{
    ...
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc, snapc,
                                    o->impl->prt, translate_flags(flags),
                                    trace_info);
}
                                    ...
                                    struct timeval *duration, uint8_t flags)
{
    ...
    return rados::cls::lock::lock(this, oid, name, ClsLockType::EXCLUSIVE,
                                  cookie, "", description, dur, flags);
}
                                 ...
                                 struct timeval *duration, uint8_t flags)
{
    ...
    return rados::cls::lock::lock(this, oid, name, ClsLockType::SHARED, cookie,
                                  tag, description, dur, flags);
}
                                             ...
                                             *timeouts)
{
  ...
  for (auto &[who, payload]:acked) {
        acks->emplace_back(librados::notify_ack_t {
                           who.first, who.second, payload}
        );
    }
  for (auto &[notifier_id, cookie]:missed) {
        timeouts->emplace_back(librados::notify_timeout_t {
                               notifier_id, cookie}
        );
    }
}
...
std::string librados::IoCtx::get_namespace() constconst
{
...
int librados::Rados::service_daemon_update_status(std::map < std::string,
                                                  std::string > &&status)
{
...
int librados::Rados::pool_create_with_rule_async(const char *name,
                                                 __u8 crush_rule,
                                                 PoolAsyncCompletion * c)
{
    string str(name);
...
std::string librados::Rados::get_addrs() constconst
{
