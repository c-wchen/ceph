...
public:
    RbdWatchCtx(librados::IoCtx &io_ctx, const char *image_name,
        ...
        : m_io_ctx(io_ctx), m_image_name(image_name),
                         ...
                         bufferlist &bl) override
    {
        ...
        if (bl.length() == 0) {
            ...
            try {
                ...
                std::
                cerr <<
                     "rbd: failed to decode image notification" <<
                     std::endl;
            }
        }
        ...
        cout << m_image_name <<
             ...
             << ", bl.length=" << bl.
             length() << ", notify_op=" << notify_message.
             get_notify_op() << std::endl;
        bufferlist reply;
    ...
    {
        std::
        cerr << m_image_name << " received error: cookie=" <<
             cookie << ", " << "err=" << cpp_strerror(err) << std::
             endl;
    }
private:
            ...
            &ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::
        cerr << "rbd: watch failed: " << cpp_strerror(r) <<
             std::endl;
        return r;
    }
    return 0;
...
, "Watch events on image.", "", &get_arguments,
&execute);
