...
class DataLogBackends final:public logback_generations,
    ...
    friend class GenTrim;
     ...
     DataLogBackends(librados::IoCtx & ioctx,
                     ...
                     int shards,
                     RGWDataChangesLog & datalog)
        noexcept:logback_generations(ioctx, oid, std::move(get_oid), shards),
        datalog(datalog) {
  } public:
    ...
    int trim_generations(const DoutPrefixProvider * dpp,
                         std::optional < uint64_t > &through, optional_yield y);
};
...
class RGWDataChangesLog {
    ...
    void update_renewed(const rgw_bucket_shard & bs,
                        uint64_t gen, ceph::real_time expiration);
    ...
    bool filter_bucket(const DoutPrefixProvider * dpp,
                       const rgw_bucket & bucket, optional_yield y) const;
  ...
  public:
    ...
    int start(const DoutPrefixProvider * dpp, const RGWZone * _zone,
              const RGWZoneParams & zoneparams, librados::Rados * lr);
    ...
    int trim_entries(const DoutPrefixProvider * dpp, int shard_id,
                     std::string_view marker, optional_yield y);
    int trim_entries(const DoutPrefixProvider * dpp, int shard_id, std::string_view marker, librados::AioCompletion * c);   // :(
...
class RGWDataChangesBE:public boost::intrusive_ref_counter < RGWDataChangesBE > {
  protected:
    ...
    RGWDataChangesBE(librados::IoCtx & ioctx,
                     RGWDataChangesLog & datalog, uint64_t gen_id)
  :    
    ...
    virtual int push(const DoutPrefixProvider * dpp, int index, entries
                     && items, optional_yield y) = 0;
    virtual int push(const DoutPrefixProvider * dpp, int index,
