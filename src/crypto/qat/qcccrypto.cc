...
bool QccCrypto::init()
{
    ...
    stat =
        cpaCyGetInstances(qcc_inst->num_instances, qcc_inst->cy_inst_handles);
    if (stat != CPA_STATUS_SUCCESS) {
    ...
    for (iter = 0; iter < qcc_inst->num_instances; iter++) {
        qcc_inst->is_polled[iter] =
            cpaCyInstanceGetInfo2(qcc_inst->cy_inst_handles[iter],
                                  &info) ==
            CPA_STATUS_SUCCESS ? info.isPolled : CPA_FALSE;
    }
    ...
    for (iter = 0; iter < qcc_inst->num_instances; iter++) {
        ...
        if (stat == CPA_STATUS_SUCCESS) {
            ...
            qcc_sess[iter].sess_stp_data.sessionPriority =
...
void QccCrypto::do_crypt(qcc_thread_args * thread_args)
{
    ...
    qcc_op_mem[entry].op_result =
        cpaCySymPerformOp(qcc_inst->cy_inst_handles[entry], NULL,
                          qcc_op_mem[entry].sym_op_data,
                          qcc_op_mem[entry].src_buff_list,
                          qcc_op_mem[entry].src_buff_list, NULL);
                           ...
                           CpaCySymCipherDirection op_type)
{
    ...
    if (qcc_op_mem[avail_inst].is_mem_alloc == false) {
        ...
        if (qcc_op_mem[avail_inst].src_buff_list == NULL
            || qcc_op_mem[avail_inst].src_buff_flat == NULL) {
        ...
        (qcc_op_mem[avail_inst].src_buff_list)->pBuffers =
            qcc_op_mem[avail_inst].src_buff_flat;
        (qcc_op_mem[avail_inst].src_buff_list)->numBuffers =
            qcc_op_mem[avail_inst].num_buffers;
        (qcc_op_mem[avail_inst].src_buff_list)->pPrivateMetaData =
            qcc_op_mem[avail_inst].src_buff_meta;
    ...
    qcc_sess[avail_inst].sess_stp_data.cipherSetupData.cipherDirection =
        op_type;
    qcc_sess[avail_inst].sess_stp_data.cipherSetupData.pCipherKey =
        (Cpa8U *) key;
    ...
    qcc_op_mem[avail_inst].src_buff_flat->dataLenInBytes =
        qcc_op_mem[avail_inst].buff_size;
    qcc_op_mem[avail_inst].src_buff_flat->pData =
        qcc_op_mem[avail_inst].src_buff;

    //OpData assignment
    qcc_op_mem[avail_inst].sym_op_data->sessionCtx =
        qcc_sess[avail_inst].sess_ctx;
    qcc_op_mem[avail_inst].sym_op_data->packetType =
    ...
    qcc_op_mem[avail_inst].sym_op_data->messageLenToCipherInBytes =
        qcc_op_mem[avail_inst].buff_size;
    ...
    if (pthread_create
        (&cypollthreads[avail_inst], NULL, crypt_thread,
         (void *)thread_args) != 0) {
    ...
    memset(qcc_op_mem[avail_inst].src_buff, 0,
           qcc_op_mem[avail_inst].buff_size);
