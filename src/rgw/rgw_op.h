...
public:
    RGWOp()
        :
        s(nullptr),
        dialect_handler(nullptr), driver(nullptr), cors_exist(false), op_ret(0)
    {
    }
    ...
    virtual void init(rgw::sal::Driver *driver, req_state *s,
                      RGWHandler *dialect_handler)
    {
        if (init_called) {
            return;
    ...
    bool generate_cors_headers(std::string &origin, std::string &method,
                               std::string &headers, std::string &exp_headers,
                               unsigned *max_age);
     ...
     * of special cases. */
    virtual int verify_requester(const rgw::auth::
    ...
    } virtual dmc::client_id dmclock_client()
    {
        return dmc::client_id::metadata;
    }
    virtual dmc::Cost dmclock_cost()
    {
        return 1;
    }
...
public:
    ...
    int parse_range();
    int read_user_manifest_part(rgw::sal::Bucket *bucket,
    ...
    dmc::client_id dmclock_client()override
    {
...
public:
    ...
    virtual void send_response() override = 0;
    virtual int get_params(const DoutPrefixProvider *dpp, optional_yield y) =
        0;
    const char *name() const override
    {
...
public:
    ...
    public:
        Deleter(const DoutPrefixProvider *dpp, rgw::sal::Driver *const str,
                req_state *const s)
...
inline std::ostream &operator<<(std::ostream &out,
                                const RGWBulkDelete::acct_path_t &o)
{
...
protected:
    ...
    int handle_file(std::string_view path,
                    size_t size, AlignedStreamGetter &body, optional_yield y);
...
public:
    ...
    void init(rgw::sal::Driver *const driver,
              req_state *const s, RGWHandler *const h) override;
...
class RGWBulkUploadOp::AlignedStreamGetter: public RGWBulkUploadOp::
    DecoratedStreamGetter
{
    size_t position;
    size_t length;
    size_t alignment;
...
public:
    template < typename U > AlignedStreamGetter(const size_t position,
            const size_t length,
            const size_t alignment, U && decoratee)
        : DecoratedStreamGetter(std::forward < U > (decoratee)),
          position(position), length(length), alignment(alignment)
    {
...
protected:
    ...
    RGWListBuckets()
        : sent_data(false),
          ...
          limit_max(RGW_LIST_BUCKETS_LIMIT_MAX), is_truncated(false)
    {
    } int verify_permission(optional_yield y) override;
    void execute(optional_yield y) override;
...
public:
    ...
    virtual int get_params(optional_yield y)
    {
        return 0;
    }
...
public:
    RGWPutObj(): ofs(0),
        ...
        append(false),
        position(0),
        cur_accounted_size(0), obj_retention(nullptr), obj_legal_hold(nullptr)
    {
    } ~RGWPutObj() override
    {
        delete slo_info;
    ...
    virtual int get_encrypt_filter(std::unique_ptr < rgw::sal::DataProcessor >
...
protected:
    ...
    RGWPostObj(): min_len(0),
        max_len(LLONG_MAX),
        len(0), ofs(0), supplied_md5_b64(nullptr), supplied_etag(nullptr)
    {
    }
...
public:
    ...
    virtual int get_params(optional_yield y) = 0;
    void send_response() override = 0;
    virtual void filter_out_temp_url(std::map < std::string,
                                     bufferlist > &add_attrs,
                                     const std::set < std::string >
                                     &rmattr_names, std::map < int,
                                     std::string > &temp_url_keys);
    const char *name() const override
    {
...
public:
    RGWDeleteObj()
        : delete_marker(false),
          ...
          deleter(nullptr), bypass_perm(true), bypass_governance_mode(false)
    {
    } int verify_permission(optional_yield y) override;
    void pre_exec() override;
    void execute(optional_yield y) override;
...
public:
    ...
    static bool parse_copy_location(const std::string_view &src,
                                    std::string &bucket_name,
                                    rgw_obj_key &object, req_state *s);
...
public:
    ...
    virtual int get_policy_from_state(rgw::sal::Driver *driver, req_state *s,
                                      std::stringstream &ss)
    {
        return 0;
    }
...
public:
    RGWOptionsCORS(): rule(NULL), origin(NULL), req_hdrs(NULL), req_meth(NULL)
    {
    } int verify_permission(optional_yield y) override
    {
        return 0;
    ...
    void execute(optional_yield y) override;
    void get_response_params(std::string &allowed_hdrs, std::string &exp_hdrs,
                             unsigned *max_age);
    void send_response() override = 0;
    const char *name() const override
    {
...
public:
    ...
    virtual int prepare_encryption(std::map < std::string, bufferlist > &attrs)
    {
        return 0;
    }
};
...
public:
    ...
    virtual void send_partial_response(const rgw_obj_key &key,
                                       bool delete_marker,
                                       const std::string &marker_version_id,
                                       int ret,
                                       boost::asio::deadline_timer *
                                       formatter_flush_cond) = 0;
...
extern int rgw_build_bucket_policies(const DoutPrefixProvider *dpp,
                                     rgw::sal::Driver *driver, req_state *s,
                                     optional_yield y);
extern int rgw_build_object_policies(const DoutPrefixProvider *dpp,
                                     rgw::sal::Driver *driver, req_state *s,
                                     bool prefetch_data, optional_yield y);
extern void rgw_build_iam_environment(rgw::sal::Driver *driver, req_state *s);
extern std::vector < rgw::IAM::Policy > get_iam_user_policy_from_attr(CephContext *cct, std::map < std::string,
        bufferlist > &attrs, const std::string &tenant);
                                        ...
                                        std::string *version_id)
{
    ...
    if (olh_epoch) {
        std::string epoch_str =
            s->info.args.get(RGW_SYS_PARAM_PREFIX "versioned-epoch");
        if (!epoch_str.empty()) {
            std::string err;
                                    ...
                                    const bool allow_empty_attrs = true)
{
    ...
    for (auto &kv : info.x_meta_map) {
        ...
        if (blocklisted_headers.count(name) == 1) {
            ...
            ldpp_subdout(dpp, rgw,
                         10) << "x>> " << name << ":" << xattr << dendl;
            format_xattr(xattr);
            ...
            const auto max_attrs_num_in_req =
...
inline void encode_obj_tags_attr(RGWObjTags *obj_tags, std::map < std::string,
                                 bufferlist > &attrs)
{
    if (obj_tags == nullptr) {
...
public:
    ...
    } virtual RGWOpType get_type() override
    {
...
inline int parse_value_and_bound(const std::string &input,
                                 int &output,
                                 const long lower_bound,
                                 const long upper_bound, const long default_val)
{
    if (!input.empty()) {
        char *endptr;
