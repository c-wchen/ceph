...
PrebufferedStreambuf::int_type PrebufferedStreambuf::overflow(int_type c)
{
    ...
    this->setp(&m_overflow[old_len + 1],
               &*m_overflow.begin() + m_overflow.size());
...
std::string PrebufferedStreambuf::get_str() constconst
{
    if (m_overflow.size()) {
...
size_t PrebufferedStreambuf::size() const const
{
...
int PrebufferedStreambuf::snprintf(char *dst, size_t avail) const const
{
