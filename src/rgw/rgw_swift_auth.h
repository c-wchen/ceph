            ...
            class SignedTokenEngine:public rgw::auth::Engine {
              ...
              public:
                 SignedTokenEngine(CephContext * const cct,
                                   ...
                                   const apl_factory)
                :cct(cct), driver(_driver), extractor(extractor),
...
/* SwiftAnonymous: applier. */
            class SwiftAnonymousApplier:public rgw::auth::LocalApplier {
              public:
                ...
                :LocalApplier(cct, user_info, LocalApplier::NO_SUBUSER,
                              std::nullopt, LocalApplier::NO_ACCESS_KEY) {
                } bool is_admin_of(const rgw_user & uid) const {
                    return false;
                } bool is_owner_of(const rgw_user & uid)const {
                    return uid.id.compare(RGW_USER_ANON_ID) == 0;
            }};
            ...
            class SwiftAnonymousEngine:public rgw::auth::AnonymousEngine {
                ...
                bool is_applicable(const req_state * s) const noexcept override {
                 ...
                 SwiftAnonymousEngine(CephContext * const cct,
                                      const SwiftAnonymousApplier::Factory *
                                      const apl_factory,
                                      const rgw::auth::TokenExtractor *
                                      const extractor)
            ...
            class DefaultStrategy:public rgw::auth::Strategy,
                ...
                const rgw::auth::swift::SignedTokenEngine signed_engine;
                 boost::optional < const rgw::auth::keystone::TokenEngine >
                    keystone_engine;
                ...
                struct AuthTokenExtractor:rgw::auth::TokenExtractor {
                    std::string get_token(const req_state *
                                          const s) const override {
                ...
                aplptr_t create_apl_remote(CephContext * const cct,
                    ...
                    return aplptr_t(new rgw::auth::swift::
                                    TempURLApplier(cct, user_info));
              } public:
                 DefaultStrategy(CephContext * const cct,
                                 const ImplicitTenants &
                                 implicit_tenant_context,
                                 rgw::sal::Driver * _driver)
                :driver(_driver),
                    implicit_tenant_context(implicit_tenant_context),
                    tempurl_engine(cct, driver,
                                   static_cast <
                                   rgw::auth::swift::TempURLApplier::Factory *
                                   >(this)), signed_engine(cct, driver,
                                                           static_cast <
                                                           rgw::auth::
                                                           TokenExtractor *
                                                           >
                    ...
                    anon_engine(cct,
                                static_cast <
                                SwiftAnonymousApplier::Factory * >(this),
                                static_cast <
                                rgw::auth::TokenExtractor *
                                >(&auth_token_extractor)) {
...
class RGWHandler_SWIFT_Auth:public RGWHandler_REST {
  public:
    ...
    int init(rgw::sal::Driver * driver, req_state * state,
