...
#define IS_TRUE_AND_LT_AUTH(x, auth) (x && ((auth && x <= AUTH) || (!auth && x < AUTH)))
    ...
    if (!lock->is_gathering() && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_rdlock, auth) ||
                                  !lock->is_rdlocked()) && (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_wrlock, auth) || !lock->is_wrlocked()) &&
        ...
        (!caps || ((~lock->gcaps_allowed(CAP_ANY, next) & other_issued) == 0 &&
                   ...
                   (~lock->gcaps_allowed(CAP_XLOCKER, next) & xlocker_issued) == 0)) && lock->get_state() != LOCK_SYNC_MIX2 &&
...
void Locker::remove_client_cap(CInode *in, Capability *cap, bool kill)
{
    ...
    if (in->is_auth()) {
        ...
        if (in->get_projected_inode()->client_ranges.count(client) && !(in->get_inode()->nlink == 0 &&
                                ...
                                utime_t now, bufferlist &bl)
{
    ...
    if (mdr->snapid == CEPH_NOSNAP && dn->lock.can_lease(client) && !diri->is_stray() &&
        // do not issue dn leases in stray dir!
...
void Locker::file_eval(ScatterLock *lock, bool *need_issue)
{
    ...
    if (lock->get_state() == LOCK_EXCL) {
        ...
        if (!((loner_wanted | loner_issued) & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GBUFFER)) ||
            (other_wanted & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GRD)) || (in->is_dir() &&
                    in->multiple_nonstale_caps())) {   // FIXME.. :/
    ...
    // * -> sync?
             ...
             !lock->is_waiter_for(SimpleLock::WAIT_WR) && !(wanted & CEPH_CAP_GWR) && !((lock->get_state() == LOCK_MIX) &&
