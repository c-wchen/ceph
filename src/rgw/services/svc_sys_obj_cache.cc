...
void RGWSI_SysObj_Cache::normalize_pool_and_obj(const rgw_pool & src_pool,
                             ...
                             optional_yield y)
{
    ...
    if (ofs != 0) {
        return RGWSI_SysObj_Core::read(dpp, read_state, objv_tracker, obj, obl,
                                       ofs, end, pmtime, psize, attrs,
                                       raw_attrs, cache_info, refresh_version,
                                       y);
    }
    ...
    r = RGWSI_SysObj_Core::read(dpp, read_state, objv_tracker, obj, obl, ofs, end, pmtime, psize, (attrs ? &unfiltered_attrset : nullptr), true,    /* cache unfiltered attrs */
                                  ...
                                  bool exclusive, optional_yield y)
{
    ...
    int ret =
        RGWSI_SysObj_Core::set_attrs(dpp, obj, attrs, rmattrs, objv_tracker,
                                     exclusive, y);
    string name = normal_name(pool, oid);
    if (ret >= 0) {
                                   ...
                                   optional_yield y)
{
    ...
    int ret =
        RGWSI_SysObj_Core::write_data(dpp, obj, data, exclusive, objv_tracker,
                                      y);
    string name = normal_name(pool, oid);
    if (ret >= 0) {
        if (objv_tracker && objv_tracker->read_version.ver) {
...
int RGWSI_SysObj_Cache::raw_stat(const DoutPrefixProvider * dpp,
                                 const rgw_raw_obj & obj, uint64_t * psize,
...
int RGWSI_SysObj_Cache_ASocketHook::start()
{
  ...
  for (auto cmd:admin_commands) {
        ...
        if (r < 0) {
            ldout(svc->ctx(),
                  0) << "ERROR: fail to register admin socket command (r=" << r
                << ")" << dendl;
            return r;
        }
    }
...
int RGWSI_SysObj_Cache_ASocketHook::call(std::string_view command,
...
RGWSI_SysObj_Cache::ASocketHandler::ASocketHandler(const DoutPrefixProvider * _dpp, RGWSI_SysObj_Cache * _svc):dpp(_dpp),
svc
(_svc)
{
...
void RGWSI_SysObj_Cache::ASocketHandler::call_list(const std::optional <
                                                   std::string > &filter,
                                                   Formatter * f)
{
    svc->cache.for_each([&filter, f] (const string & name,
...
int RGWSI_SysObj_Cache::ASocketHandler::call_inspect(const std::string & target,
                                                     Formatter * f)
{
