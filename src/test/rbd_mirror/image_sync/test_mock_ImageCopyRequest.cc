    ...
    namespace mirror {
        namespace image_sync {
            ...
            template <> struct ObjectCopyRequest <librbd::MockTestImageCtx > {
                ...
                static ObjectCopyRequest *create(librbd::MockTestImageCtx *
                                                 local_image_ctx,
                ...
                const ImageCopyRequest <
                    librbd::MockTestImageCtx >::SnapMap * snap_map = nullptr;
...
} // namespace image_sync } // namespace mirror }   // namespace rbd
// template definitions
#include "tools/rbd_mirror/image_sync/ImageCopyRequest.cc"
template class rbd::mirror::image_sync::ImageCopyRequest <
    librbd::MockTestImageCtx >;
    ...
    namespace mirror {
        namespace image_sync {
            ...
            class TestMockImageSyncImageCopyRequest:public TestMockFixture {
              public:
                typedef ImageCopyRequest < librbd::MockTestImageCtx >
                ...
                void SetUp() override {
                    ...
                    ASSERT_EQ(0,
                              open_image(m_local_io_ctx, m_image_name,
                                         &m_local_image_ctx));
                } void expect_get_snap_id(librbd::
                    ...
                    EXPECT_CALL(mock_image_ctx, get_snap_id(_, _))
                        ...
                        WillRepeatedly(Invoke
                                       ([&mock_image_ctx]
                                        ...
                                        assert(mock_image_ctx.image_ctx->
                                               snap_lock.is_locked());
                                        return mock_image_ctx.image_ctx->
                                        get_snap_id(snap_namespace, snap_name);}
                                       ));
                } void expect_get_object_count(librbd::
                                               MockTestImageCtx &
                                               mock_image_ctx, uint64_t count) {
                    EXPECT_CALL(mock_image_ctx, get_object_count(_))
                        .WillOnce(Return(count)).RetiresOnSaturation();
                } void expect_update_client(journal::
                ...
                void expect_object_copy_send(MockObjectCopyRequest &
                                             mock_object_copy_request) {
                    EXPECT_CALL(mock_object_copy_request, send());
                }

                bool complete_object_copy(MockObjectCopyRequest &
                                          mock_object_copy_request,
                                          uint64_t object_num, int r,
                                          std::function < void () > fn =[]() {
                                          }) {
                    Mutex::Locker locker(mock_object_copy_request.lock);
                    while (mock_object_copy_request.object_contexts.
                           count(object_num) == 0) {
                        if (mock_object_copy_request.cond.
                            WaitInterval(mock_object_copy_request.lock,
                                         utime_t(10, 0)) != 0) {
                            return false;
                        }
                    }

                    FunctionContext *wrapper_ctx = new FunctionContext([&mock_object_copy_request, object_num, fn] (int r){
                                                                       fn();
                                                                       mock_object_copy_request.
                                                                       object_contexts
                                                                       [object_num]->
                ...
                MockImageCopyRequest::
                    SnapMap wait_for_snap_map(MockObjectCopyRequest &
                                              mock_object_copy_request) {
                    Mutex::Locker locker(mock_object_copy_request.lock);
                ...
                MockImageCopyRequest *create_request(librbd::
                                                     MockTestImageCtx &
                ...
                int create_snap(const char *snap_name) {
                    ...
                    int r =
                        create_snap(m_remote_image_ctx, snap_name,
                                    &remote_snap_id);
                    if (r < 0) {
                        return r;
                    }
                    ...
                    r = create_snap(m_local_image_ctx, snap_name,
                                    &local_snap_id);
                    if (r < 0) {
                        return r;
                    }
                    ...
                    MockImageCopyRequest::SnapIds local_snap_ids( {
                                                                 local_snap_id}
                    );
                    if (!m_snap_map.empty()) {
                        local_snap_ids.insert(local_snap_ids.end(),
                                              m_snap_map.rbegin()->second.
                                              begin(),
                                              m_snap_map.rbegin()->second.
                                              end());
                    }
            ...
            TEST_F(TestMockImageSyncImageCopyRequest, Throttled) {
                ...
                ASSERT_EQ(0,
                          _rados->conf_get("rbd_mirror_sync_point_update_age",
                                           update_sync_age));
                ASSERT_EQ(0,
                          _rados->conf_set("rbd_mirror_sync_point_update_age",
                                           "1"));
                BOOST_SCOPE_EXIT((update_sync_age)) {
                    ASSERT_EQ(0,
                              _rados->
                              conf_set("rbd_mirror_sync_point_update_age",
                                       update_sync_age.c_str()));
                }
                BOOST_SCOPE_EXIT_END;

                std::string max_ops_str;
                ASSERT_EQ(0,
                          _rados->conf_get("rbd_concurrent_management_ops",
                                           max_ops_str));
                ...
                boost::optional < uint64_t >
                    expected_object_number(boost::none);
                EXPECT_CALL(mock_journaler, update_client(_, _))
                    .
                    WillRepeatedly(Invoke
                                   ([&expected_object_number, max_ops,
                                     object_count, this]
                                    ...
                                    expected_object_number.get() + max_ops;}

                                    if (expected_object_number.get() >
                                        (object_count - 1)) {
                ...
                for (uint64_t i = 0; i < object_count; ++i) {
                    if (i % 10 == 0) {
                        ASSERT_TRUE(complete_object_copy
                                    (mock_object_copy_request, i, 0, sleep_fn));
                    }
                    else {
                        ASSERT_TRUE(complete_object_copy
                                    (mock_object_copy_request, i, 0));
                    }
                }
                ASSERT_EQ(0, ctx.wait());
            }
            ...
            TEST_F(TestMockImageSyncImageCopyRequest, RestartCatchup) {
                ...
                cls::rbd::UserSnapshotNamespace(), "snap1", boost::none}
                , {
                cls::rbd::UserSnapshotNamespace(), "snap2", "snap1",
                        boost::none}
                };
            ...
            TEST_F(TestMockImageSyncImageCopyRequest, RestartPartialSync) {
                ...
                m_client_meta.sync_points = { {
                        cls::rbd::UserSnapshotNamespace(),
                            "snap1",
                            librbd::journal::MirrorPeerSyncPoint::ObjectNumber {
                        0U}
                }
                };
            ...
            TEST_F(TestMockImageSyncImageCopyRequest, Cancel) {
                ...
                ASSERT_EQ(0,
                          _rados->conf_get("rbd_concurrent_management_ops",
                                           max_ops_str));
                ASSERT_EQ(0,
                          _rados->conf_set("rbd_concurrent_management_ops",
                                           "1"));
            ...
            TEST_F(TestMockImageSyncImageCopyRequest, Cancel_Inflight_Sync) {
                ...
                EXPECT_CALL(mock_journaler, update_client(_, _))
                    .
                    WillRepeatedly(Invoke
                                   ([this] (bufferlist data, Context * ctx) {
                                    m_threads->work_queue->queue(ctx, 0);}
                                   ));
                ...
                ASSERT_TRUE(complete_object_copy
                            (mock_object_copy_request, 1, 0));
                ASSERT_TRUE(complete_object_copy
                            (mock_object_copy_request, 2, 0));
                ASSERT_TRUE(complete_object_copy
                            (mock_object_copy_request, 3, 0, cancel_fn));
                ASSERT_TRUE(complete_object_copy
                            (mock_object_copy_request, 4, 0));
                ASSERT_TRUE(complete_object_copy
                            (mock_object_copy_request, 5, 0));
            ...
            TEST_F(TestMockImageSyncImageCopyRequest, MissingSnap) {
                ASSERT_EQ(0, create_snap("snap1"));
                m_client_meta.sync_points = { {
                cls::rbd::UserSnapshotNamespace(), "missing-snap",
                            boost::none}};
