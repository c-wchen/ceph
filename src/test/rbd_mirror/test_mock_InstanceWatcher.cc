...
template <> struct Throttler <librbd::MockTestImageCtx > {
    ...
    } virtual ~ Throttler()
    {
        ceph_assert(s_instance == this);
        s_instance = nullptr;
    } MOCK_METHOD3(start_op,
                   void (const std::string &, const std::string &,
                         Context *));
    MOCK_METHOD2(finish_op,
                 void (const std::string &, const std::string &));
...
TEST_F(TestMockInstanceWatcher, InitShutdown)
{
    ...
    auto instance_watcher =
        new MockInstanceWatcher(m_local_io_ctx,
                                *m_mock_threads->asio_engine, nullptr,
                                nullptr,
                                m_instance_id);
    InSequence seq;
...
TEST_F(TestMockInstanceWatcher, ImageAcquireRelease)
{
    ...
    auto instance_watcher1 =
        MockInstanceWatcher::create(io_ctx1,
                                    *m_mock_threads->asio_engine,
                                    &mock_instance_replayer1, nullptr);
    ...
    auto instance_watcher2 =
        MockInstanceWatcher::create(io_ctx2,
                                    *m_mock_threads->asio_engine,
                                    &mock_instance_replayer2, nullptr);
...
TEST_F(TestMockInstanceWatcher, ImageAcquireReleaseCancel)
{
    ...
    bufferlist * pbl) {
        ...
        auto ctx =
            new
        LambdaContext([instance_watcher, &mock_io_ctx,
        c, pbl](int r) {
            instance_watcher->
            cancel_notify_requests("other");
            encode(librbd::watcher::
                   NotifyResponse(), *pbl);
            mock_io_ctx.
...
TEST_F(TestMockInstanceWatcher, PeerImageAcquireWatchDNE)
{
    ...
    C_SaferCond on_acquire;
    instance_watcher->notify_image_acquire("dead instance",
                                           "global image id",
                                           &on_acquire);
...
TEST_F(TestMockInstanceWatcher, PeerImageReleaseWatchDNE)
{
    ...
    C_SaferCond on_acquire;
    instance_watcher->notify_image_release("dead instance",
                                           "global image id",
                                           &on_acquire);
    ASSERT_EQ(-ENOENT, on_acquire.wait());
