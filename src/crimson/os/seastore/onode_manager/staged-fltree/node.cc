...
eagain_ifuture < Ref < tree_cursor_t >> tree_cursor_t::erase(context_t c, bool get_next)
{
    assert(is_tracked());
...
template < bool FORCE_MERGE >
eagain_ifuture <> Node::try_merge_adjacent(context_t c, bool update_parent_index,
        Ref < Node > &&this_ref)
{
...
eagain_ifuture < Ref < tree_cursor_t >> InternalNode::get_next_cursor(context_t c,
                                     const search_position_t &pos)
{
    impl->validate_non_empty();
    if (pos.is_end()) {
...
template < bool FORCE_MERGE >
eagain_ifuture < Ref < tree_cursor_t >> LeafNode::erase(context_t c, const search_position_t &pos,
                                     bool get_next)
{
...
Ref < tree_cursor_t > LeafNode::get_or_track_cursor(const search_position_t &position,
