        ...
        namespace wnbd {
            ...
            static int call_wnbd_cmd(const po::variables_map & vm,
              ...
              for (auto & arg:args) {
                ...
                if (process.spawn()) {
                    std::cerr << "rbd: failed to run rbd-wnbd: " << process.
                        err() << std::endl;
                    return -EINVAL;
                }
                int exit_code = process.join();
                if (exit_code) {
                    std::cerr << "rbd: rbd-wnbd failed with error: " << process.
                        err() << std::endl;
                    return exit_code;
                }
            ...
            int execute_list(const po::variables_map & vm,
                             ...
                             &ceph_global_init_args) {
#if !defined(_WIN32)
                std::cerr << "rbd: wnbd is only supported on Windows" << std::
                    endl;
                return -EOPNOTSUPP;
#else
            ...
            int execute_attach(const po::variables_map & vm,
                ...
                std::cerr << "rbd: wnbd is only supported on Windows" << std::
                    endl;
#else
                std::cerr << "rbd: wnbd attach command not supported" << std::
                    endl;
#endif
                return -EOPNOTSUPP;
            }
            ...
            int execute_detach(const po::variables_map & vm,
                ...
                std::cerr << "rbd: wnbd is only supported on Windows" << std::
                    endl;
#else
                std::cerr << "rbd: wnbd detach command not supported" << std::
                    endl;
#endif
                return -EOPNOTSUPP;
            }
