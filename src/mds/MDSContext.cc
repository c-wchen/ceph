...
void MDSInternalContextBase::complete(int r)
{
    ...
    dout(10) << "MDSInternalContextBase::complete: " << typeid(*this).
        name() << dendl;
    assert(mds != NULL);
...
elist <
    MDSIOContextBase *
    >MDSIOContextBase::ctx_list(member_offset(MDSIOContextBase, list_item));
                                           ...
                                           ceph::coarse_mono_time & oldest)
{
    ...
    simple_spin_lock(&ctx_list_lock);
    for (elist < MDSIOContextBase * >::iterator p = ctx_list.begin(); !p.end();
         ++p) {
...
{
    derr << "Forbidden call to MDSInternalContextGather::get_mds by " <<
        typeid(*this).name() << dendl;
    ceph_abort();
}
