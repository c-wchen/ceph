...
class KeyValueDB {
  public:
    class TransactionImpl {
      public:
        ...
        /// Set Keys (via encoded ceph::buffer::list) void set(
        ...
        virtual void set(const std::string & prefix,
                         const char *k,
                         size_t keylen, const ceph::buffer::list & bl) {
        ...
        virtual void merge(const std::string & prefix,  ///< [in] Prefix/CF ==> MUST match some established merge operator
    ...
    virtual int create_and_open(std::ostream & out, const std::string & cfs =
                                "") = 0;
    ...
    virtual int repair(std::ostream & out) {
        return 0;
    }
  ...
  class PrefixIteratorImpl:public IteratorImpl {
      ...
      public:
        PrefixIteratorImpl(const std::string & prefix,
        ...
        bool valid() override {
            ...
            if (prefix.empty())
                return true;
            return prefix.empty()?
                true : generic_iter->raw_key_is_prefixed(prefix);
        }
        int next() override {
  ...
  public:
    ...
    virtual WholeSpaceIterator get_wholespace_iterator(IteratorOpts opts = 0) =
        0;
   ...
   */
    virtual bool get_property(const std::string & property, uint64_t * out) {
        return false;
    }
  protected:
