...
// then used to commit or abort the reservation
    struct reservation_t {
        struct topic_t {
            topic_t(const std::string & _configurationId,
                    const rgw_pubsub_topic & _cfg,
                    cls_2pc_reservation::
                    id_t _res_id):configurationId(_configurationId), cfg(_cfg),
                res_id(_res_id) {
        ...
        reservation_t(const DoutPrefixProvider * _dpp,
                      ...
                      const std::string * _object_name, optional_yield y);
        ...
        reservation_t(const DoutPrefixProvider * _dpp,
                      ...
                      const std::string & _req_id, optional_yield y);
