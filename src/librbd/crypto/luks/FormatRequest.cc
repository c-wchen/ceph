    ...
    namespace crypto {
        namespace luks {
            ...
            using librbd::util::create_context_callback;
             ...
             template < typename I >
                FormatRequest < I >::FormatRequest(I * image_ctx,
                                                   ...
                                                   *result_crypto,
                                                   Context * on_finish,
                                                   bool
                                                   insecure_fast_mode):m_image_ctx
                (image_ctx), m_format(format), m_alg(alg),
                m_passphrase(passphrase), m_result_crypto(result_crypto),
                ...
                case RBD_ENCRYPTION_FORMAT_LUKS1:
                    ...
                    sector_size = 512;
                    break;
                    case RBD_ENCRYPTION_FORMAT_LUKS2:type = CRYPT_LUKS2;
                    sector_size = 4096;
                    break;
                    default:lderr(m_image_ctx->
                                  cct) << "unsupported format type: " <<
                        m_format << dendl;
                    finish(-EINVAL);
                    return;
                } const char *cipher;
                ...
                case RBD_ENCRYPTION_ALGORITHM_AES128:
                    ...
                    key_size = 32;
                    break;
                    case RBD_ENCRYPTION_ALGORITHM_AES256:cipher = "aes";
                    key_size = 64;
                    break;
                    default:lderr(m_image_ctx->
                                  cct) << "unsupported cipher algorithm: " <<
                        m_alg << dendl;
                    finish(-EINVAL);
                    return;
                }
                ...
                if (RAND_bytes((unsigned char *)key, key_size) != 1) {
                    lderr(m_image_ctx->
                          cct) << "cannot generate random encryption key" <<
                        dendl;
                    finish(-EAGAIN);
                    return;
                }
                ...
                r = m_header.format(type, cipher,
                                    reinterpret_cast < char *>(key), key_size,
                ...
                if (m_header.get_data_offset() > image_size) {
                    lderr(m_image_ctx->
                          cct) << "image is too small, format requires " <<
                ...
                r = m_header.add_keyslot(m_passphrase.data(),
                                         m_passphrase.size());
                if (r != 0) {
                    finish(r);
                    return;
                ...
                if (m_image_ctx->parent != nullptr) {
                    ...
                    if (r < 0) {
                        lderr(m_image_ctx->
                              cct) << "error replacing LUKS magic: " <<
                ...
                auto ctx = create_context_callback <
                    FormatRequest < I >,
                    &FormatRequest < I >::handle_write_header > (this);
                auto aio_comp =
                ...
                ZTracer::Trace trace;
                auto req =
                    io::ImageDispatchSpec::create_write(*m_image_ctx,
                                                        io::
