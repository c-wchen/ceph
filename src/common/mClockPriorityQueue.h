     ...
     template < typename T, typename K >
        ...
        struct SubQueue {
          ...
          public:
            ...
            :q(other.q),
                tokens(other.tokens),
                max_tokens(other.max_tokens), cur(q.begin()) {
            } SubQueue()
            :tokens(0), max_tokens(0), cur(q.begin()) {
            }
      ...
      mClockQueue(const typename Queue::ClientInfoFunc & info_func, double anticipation_timeout = 0.0):
        ...
        void remove_by_class(K k, std::list < T > *out = nullptr) override final {
            ...
            for (auto i = queue_front.rbegin(); i != queue_front.rend();
                ...
                if (k == i->first) {
                    if (nullptr != out)
                        out->push_front(std::move(i->second));
                    i = decltype(i) {
                    queue_front.erase(std::next(i).base())};
                }
                else {
        ...
        } void enqueue_strict_front(K cl, unsigned priority, T
                                    && item) override final {
            high_queue[priority].enqueue_front(cl, 1, std::move(item));
        } void enqueue(K cl, unsigned priority, unsigned cost, T
                       && item) override final {
            // priority is ignored
