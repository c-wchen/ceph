...
public:
                                       ...
                                       RBD_MIRROR_IMAGE_MODE_JOURNAL)
    {
        ...
        int order = 20;
        ASSERT_EQ(0,
                  m_rbd.create2(m_ioctx, image_name.c_str(), 4096, features,
                                &order));
        librbd::Image image;
        ...
        ASSERT_EQ(0,
                  image.mirror_image_get_global_status(&status,
                          sizeof(status)));
    ...
    void check_mirroring_on_update_features(uint64_t init_features, bool enable,
                                                    ...
                                                    RBD_MIRROR_IMAGE_MODE_JOURNAL)
    {
        ...
        int order = 20;
        ASSERT_EQ(0,
                  m_rbd.create2(m_ioctx, image_name.c_str(), 4096,
                                init_features, &order));
        librbd::Image image;
        ASSERT_EQ(0, m_rbd.open(m_ioctx, image, image_name.c_str()));
                                       ...
                                       std::vector < uint64_t > &features_vec)
    {
        ...
        for (const auto &features : features_vec) {
            ...
            ASSERT_EQ(0,
                      m_rbd.create2(m_ioctx, img_name_str.c_str(), 2048,
                                    features, &order));
        }
    }
                                            ...
                                            &states_vec)
    {
        ...
        std::vector < std::tuple < std::string,
            rbd_mirror_image_state_t > >images;
        int id = 1;
        for (const auto &mirror_state : states_vec) {
            ...
            librbd::mirror_image_site_status_t local_status;
            ASSERT_EQ(0,
                      get_local_mirror_image_site_status(status,
                              &local_status));
            ASSERT_EQ(MIRROR_IMAGE_STATUS_STATE_UNKNOWN, local_status.state);
    ...
    void setup_mirror_peer(librados::IoCtx &io_ctx, librbd::Image &image)
    {
        ...
        librbd::journal::
        MirrorPeerClientMeta peer_client_meta("remote-image-id", { { {
                }, "sync-point-snap",
                boost::none
            }
        }, {
...
{
    check_remove_image(RBD_MIRROR_MODE_IMAGE,
                       RBD_FEATURE_EXCLUSIVE_LOCK, false);
}
...
TEST_F(TestMirroring, RemoveBootstrapped)
{
    ...
    ASSERT_EQ(-EBUSY,
              librbd::api::Image <>::remove(m_ioctx, image_name, no_op));
...
TEST_F(TestMirroring, AioGetStatus)
{
    ...
    for (auto &image : images) {
        ...
        ASSERT_EQ(0,
                  image.aio_mirror_image_get_global_status(&statuses.back(),
                          sizeof(statuses.
                                 back()),
                          aio_comps.back()));
    }
...
TEST_F(TestMirroring, Bootstrap)
{
    ...
    ASSERT_EQ(-EINVAL,
              m_rbd.mirror_peer_bootstrap_import(m_ioctx,
...
TEST_F(TestMirroring, PeerDirection)
{
    ...
    std::string uuid;
    ASSERT_EQ(-EINVAL,
              m_rbd.mirror_peer_site_add(m_ioctx, &uuid,
                                         RBD_MIRROR_PEER_DIRECTION_TX, "siteA",
                                         "client.admin"));
    ASSERT_EQ(0,
              m_rbd.mirror_peer_site_add(m_ioctx, &uuid,
    ...
    std::vector < librbd::mirror_peer_site_t > expected_peers = {
        {
            uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "siteA", "", "client.admin",
            0
        }
    };
    ...
    ASSERT_EQ(0,
              m_rbd.mirror_peer_site_set_direction(m_ioctx, uuid,
                      RBD_MIRROR_PEER_DIRECTION_RX));
    ASSERT_EQ(0, m_rbd.mirror_peer_site_list(m_ioctx, &peers));
    expected_peers = {
        {
...
TEST_F(TestMirroring, Snapshot)
{
    ...
    ASSERT_EQ(0,
              image.metadata_set("conf_rbd_mirroring_max_mirroring_snapshots",
                                 "5"));
...
TEST_F(TestMirroring, SnapshotUnlinkPeer)
{
    ...
    C_SaferCond cond1;
    auto req =
        librbd::mirror::snapshot::UnlinkPeerRequest <>::create(ictx, snap_id,
            peer1_uuid, true,
            &cond1);
    req->send();
...
TEST_F(TestMirroring, SnapshotImageState)
{
...

    {
        C_SaferCond cond;
        auto req =
            librbd::mirror::snapshot::SetImageStateRequest <>::create(ictx,
                snap_id,
                &cond);
        req->send();
    ...
    librbd::mirror::snapshot::ImageState image_state;
    {
        C_SaferCond cond;
        auto req =
            librbd::mirror::snapshot::GetImageStateRequest <>::create(ictx,
                snap_id,
                &image_state,
                &cond);
        req->send();
...

    {
        C_SaferCond cond;
        auto req =
            librbd::mirror::snapshot::RemoveImageStateRequest <>::create(ictx,
                snap_id,
                &cond);
        req->send();
        ASSERT_EQ(0, cond.wait());
    ...
    for (int i = 0; i < 10; i++) {
        ASSERT_EQ(0,
                  image.metadata_set(stringify(i), std::string(1024, 'A' + i)));
    }
