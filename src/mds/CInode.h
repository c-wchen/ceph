...
class InodeStoreBase {
  public:
    ...
    typedef mempool::mds_co::compact_map < snapid_t,
        mempool_old_inode > mempool_old_inode_map;
    ...
    void encode(bufferlist & bl, uint64_t features,
                const bufferlist * snap_blob = NULL) const;
    ...
    void encode_bare(bufferlist & bl, uint64_t features,
                     const bufferlist * snap_blob = NULL) const;
    void decode_bare(bufferlist::iterator & bl, bufferlist & snap_blob,
                     __u8 struct_v = 5);
...
default:return generic_pin_name(p);
                                                                ...
                                                                // -- state -- static const int STATE_EXPORTING = (1 << 2);
                                                                // on nonauth bystander.
    ...
    static const int MASK_STATE_EXPORTED =
        (STATE_DIRTY | STATE_NEEDSRECOVER | STATE_DIRTYPARENT |
         STATE_DIRTYPOOL);
  ...
  public:
    CInode::projected_inode & project_inode(bool xattr = false, bool snap =
                                            false);
    ...
    mempool_xattr_map *get_projected_xattrs() {
        if (num_projected_xattrs > 0) {
            for (auto it = projected_nodes.rbegin();
                 it != projected_nodes.rend(); ++it)
                if (it->xattrs)
  ...
  public:
    ...
    } template < typename Container > void get_nested_dirfrags(Container & ls) const {
        // dirfrags in same subtree
        for (const auto & p:dirfrags) {
            typename Container::value_type dir = p.second;
            if (!dir->is_subtree_root())
                 ls.push_back(dir);
    }} template < typename Container > void get_subtree_dirfrags(Container & ls) {
  ...
  CInode(MDCache * c, bool auth = true, snapid_t f = 2, snapid_t l = CEPH_NOSNAP):
    ...
    const mempool_inode & get_inode() const {
        return inode;
    ...
    // -- misc --  bool is_projected_ancestor_of(CInode * other);
  ...
  public:
    ...
    // -- waiting --
  protected:
    mempool::mds_co::compact_map < frag_t,
        std::list < MDSInternalContextBase * > >waiting_on_dir;
  public:
    ...
    void take_waiting(uint64_t tag,
                      std::list < MDSInternalContextBase * >&ls) override;
    ...
    void _decode_locks_rejoin(bufferlist::iterator & p,
                              std::list < MDSInternalContextBase * >&waiters,
  ...
  public:
    ...
    void finish_scatter_gather_update_accounted(int type, MutationRef & mut,
                                                EMetaBlob * metablob);
    ...
    } mempool::mds_co::compact_map < int32_t, int32_t > &get_mds_caps_wanted() {
        return mds_caps_wanted;
    }

    const cap_map & get_client_caps() const {
        return client_caps;
