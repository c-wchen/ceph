        ...
        ghobject_t get_oid(SeaStore &seastore, CollectionRef &coll) const
        {
            switch (type) {
                ...
                case type_t::TEMP_END:
                    return seastore.get_objs_range(coll,
                                                   0).
                           temp_end;
...
TEST_P(seastore_test_t, sparse_read)
{
    run_async([this] {
        ...
        interval_set < uint64_t > m; m = interval_set < uint64_t > (test_obj.fiemap(*sharded_seastore, 0, SKIP_STEP * (MAX_EXTENTS - 1) + 3)); ASSERT_TRUE(!m.empty()); uint64_t off = 0; auto rbl = test_obj.readv(*sharded_seastore, m);
        for (auto && miter : m)
        {
            bufferlist subl;
