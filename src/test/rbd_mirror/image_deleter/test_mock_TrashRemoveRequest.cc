...
namespace librbd {
    ...
    template <> struct TrashWatcher <MockTestImageCtx > {
        ...
        } MOCK_METHOD2(notify_image_removed,
    ...
    namespace trash {
        ...
        RemoveRequest < librbd::MockTestImageCtx > *RemoveRequest <
        ...
        namespace image_deleter {
            ...
            template <> struct SnapshotPurgeRequest <librbd::MockTestImageCtx > {
             ...
             SnapshotPurgeRequest < librbd::MockTestImageCtx >
                *SnapshotPurgeRequest < librbd::MockTestImageCtx >::s_instance =
                nullptr;

} // namespace image_deleter } // namespace mirror }    // namespace rbd
    ...
    namespace mirror {
        namespace image_deleter {
            ...
            class TestMockImageDeleterTrashRemoveRequest:public TestMockFixture {
              public:
                typedef TrashRemoveRequest < librbd::MockTestImageCtx >
                    MockTrashRemoveRequest;
                typedef SnapshotPurgeRequest < librbd::MockTestImageCtx >
                    MockSnapshotPurgeRequest;
                typedef librbd::TrashWatcher < librbd::MockTestImageCtx >
                    MockTrashWatcher;
                typedef librbd::trash::RemoveRequest <
                    librbd::MockTestImageCtx > MockLibrbdTrashRemoveRequest;

                void expect_trash_get(const cls::rbd::
                                      TrashImageSpec & trash_spec, int r) {
                    using ceph::encode;
                     EXPECT_CALL(get_mock_io_ctx(m_local_io_ctx),
                                 exec(StrEq(RBD_TRASH), _, StrEq("rbd"),
                                      StrEq("trash_get"), _, _, _, _))
                    .WillOnce(WithArg < 5 >
                              (Invoke([trash_spec, r] (bufferlist * bl) {
                                      encode(trash_spec, *bl); return r;}
                               )));
                } void expect_trash_state_set(const std::string & image_id,
                                              int r) {
                    bufferlist in_bl;
                    ...
                    bufferlist bl;
                     ...
                     EXPECT_CALL(get_mock_io_ctx(m_local_io_ctx),
                                 exec(librbd::util::header_name(image_id), _,
                ...
                void expect_snapshot_purge(MockSnapshotPurgeRequest &
                    ...
                    EXPECT_CALL(snapshot_purge_request, send())
                        .WillOnce(Invoke([this, &snapshot_purge_request, r] () {
                                         ...
                                         queue(snapshot_purge_request.on_finish,
                                               r);}));
                }

                void expect_image_remove(MockLibrbdTrashRemoveRequest &
                    ...
                    EXPECT_CALL(image_remove_request, send())
                        .WillOnce(Invoke([this, &image_remove_request, r] () {
                                         m_threads->work_queue->
                                         queue(image_remove_request.on_finish,
                                               r);}));
                }

                void expect_notify_image_removed(MockTrashWatcher &
                                                 mock_trash_watcher,
                                                 const std::string & image_id) {
                    EXPECT_CALL(mock_trash_watcher,
                                notify_image_removed(image_id, _))
            ...
            TEST_F(TestMockImageDeleterTrashRemoveRequest, Success) {
                ...
                expect_get_snapcontext("image id", {
                                       1, {
                                       1}}, 0);

                MockSnapshotPurgeRequest mock_snapshot_purge_request;
                expect_snapshot_purge(mock_snapshot_purge_request, "image id",
                                      0);
                ...
                auto req =
                    MockTrashRemoveRequest::create(m_local_io_ctx, "image id",
                                                   &error_result,
            ...
            TEST_F(TestMockImageDeleterTrashRemoveRequest, RemoveError) {
                ...
                MockLibrbdTrashRemoveRequest mock_image_remove_request;
                expect_image_remove(mock_image_remove_request, "image id",
                                    -EINVAL);
