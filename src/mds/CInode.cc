...
ostream & CInode::print_db_line_prefix(ostream & out)
{
    return out << ceph_clock_now() << " mds." << mdcache->mds->
        get_nodeid() << ".cache.ino(" << inode.ino << ") ";
}
...
ostream & operator<<(ostream & out, const CInode & in)
{
    ...
    if (in.is_auth_pinned()) {
        out << " ap=" << in.get_num_auth_pins() << "+" << in.
    ...
    if (!in.get_client_caps().empty()) {
        out << " caps={";
        for (map < client_t, Capability * >::const_iterator it =
             in.get_client_caps().begin(); it != in.get_client_caps().end();
             ++it) {
            if (it != in.get_client_caps().begin())
                out << ",";
            out << it->first << "=" << ccap_string(it->second->pending());
...
void CInode::add_need_snapflush(CInode * snapin, snapid_t snapid,
                                client_t client)
{
    dout(10) << "add_need_snapflush client." << client << " snapid " << snapid
        << " on " << snapin << dendl;
...
{
    dout(10) << "split_need_snapflush [" << cowin->first << "," << cowin->
...
void CInode::project_past_snaprealm_parent(SnapRealm * newparent)
{
    ...
    if (newparent != oldparent) {
        ...
        new_snap.current_parent_since =
            std::max(oldparentseq, newparent->get_last_created()) + 1;
    }
}
...
void CInode::pop_projected_snaprealm(sr_t * next_snaprealm)
{
    ...
    if (!snaprealm) {
        ...
        dout(10) << " realm " << *snaprealm << " past_parents " << snaprealm->
            srnode.past_parents << " -> " << next_snaprealm->
            past_parents << dendl;
    }
...
void CInode::force_dirfrags()
{
  ...
  for (auto & p:dirfrags) {
    ...
    if (bad) {
        ...
        for (list < frag_t >::iterator p = leaves.begin(); p != leaves.end();
             ++p)
...
version_t CInode::pre_dirty()
{
    ...
    if (_cdentry) {
        ...
        dout(10) << "pre_dirty " << pv << " (current v " << inode.
            version << ")" << dendl;
    }
    else {
        assert(is_base());
...
{
    char n[60];
    snprintf(n, sizeof(n), "%llx.%08llx%s", (long long unsigned)ino,
...
{
    dout(10) << "_fetched got " << bl.length() << " and " << bl2.
        length() << dendl;
    ...
    while (pdn) {
        ...
        bt.ancestors.
            push_back(inode_backpointer_t
                      (diri->ino(), pdn->get_name(), in->inode.version));
        in = diri;
...
void CInode::encode_lock_state(int type, bufferlist & bl)
{
    ...
    case CEPH_LOCK_IDFT:
        ...
        }
        {
            ...
            list < CDir * >dfls;
            get_dirfrags(dfls);
            for (list < CDir * >::iterator p = dfls.begin(); p != dfls.end();
                 ++p)
                if ((*p)->is_auth()) {
    ...
    case CEPH_LOCK_IFILE:
        ...
        {
            dout(15) << "encode_lock_state inode.dirstat is " << inode.
                dirstat << dendl;
    ...
    case CEPH_LOCK_INEST:
        ...
        {
            dout(15) << "encode_lock_state inode.rstat is " << inode.
                rstat << dendl;
...
void CInode::decode_lock_state(int type, bufferlist & bl)
{
    ...
    if (!is_auth() && newfirst != first) {
        dout(10) << "decode_lock_state first " << first << " -> " << newfirst <<
            dendl;
    ...
    case CEPH_LOCK_IDFT:
        if (is_auth()) {
            ...
            if (replica_dirty) {
                dout(10) << "decode_lock_state setting dftlock dirty flag" <<
                    dendl;
        ...
        }
        {
            ...
            if (is_auth()) {
                ...
                for (set < frag_t >::iterator p = authfrags.begin();
                     p != authfrags.end(); ++p)
                    if (!dirfragtree.is_leaf(*p)) {
                        dout(10) << " forcing frag " << *p <<
              ...
              for (const auto & p:dirfrags) {
                    if (!dirfragtree.is_leaf(p.first)) {
                        dout(10) << " forcing open dirfrag " << p.
                            first << " to leaf (racing with split|merge)" <<
                            dendl;
    ...
    case CEPH_LOCK_IFILE:
        if (!is_auth()) {
            ...
            ::decode(replica_dirty, p);
            if (replica_dirty) {
                dout(10) << "decode_lock_state setting filelock dirty flag" <<
                    dendl;
        ...
        }
        {
            ...
            ::decode(dirstat, p);
            if (!is_auth()) {
                dout(10) << " taking inode dirstat " << dirstat << " for " <<
                    *this << dendl;
    ...
    case CEPH_LOCK_INEST:
        if (is_auth()) {
            ...
            ::decode(replica_dirty, p);
            if (replica_dirty) {
                dout(10) << "decode_lock_state setting nestlock dirty flag" <<
                    dendl;
        ...
        }
        {
            ...
            ::decode(rstat, p);
            if (!is_auth()) {
                dout(10) << " taking inode rstat " << rstat << " for " << *this
                    << dendl;
            ...
            while (n--) {
                ...
                dout(10) << fg << "     accounted_rstat " << accounted_rstat <<
                    dendl;
                dout(10) << fg << "     dirty_old_rstat " << dirty_old_rstat <<
                    dendl;
                ...
                if (is_auth()) {
                    ...
                    if (!(rstat == accounted_rstat)
                        || !dir->dirty_old_rstat.empty()) {
                        dout(10) << fg << " setting nestlock updated flag" <<
                            dendl;
                        nestlock.mark_dirty();  // ok bc we're auth and caller will handle
                    }
                }
    ...
    case CEPH_LOCK_ISNAP:
        {
            ...
            if (snaprealm && snaprealm->srnode.seq != seq)
                mdcache->do_realm_invalidate_and_update_notify(this,
                                                               seq ?
                                                               CEPH_SNAP_OP_UPDATE
                                                               :
                                                               CEPH_SNAP_OP_SPLIT);
                                   ...
                                   version_t dir_accounted_version)
{
    ...
    if (dir->is_frozen()) {
        dout(10) << "finish_scatter_update " << fg << " frozen, marking " <<
        ...
        if (dir_accounted_version != inode_version) {
            dout(10) << "finish_scatter_update " << fg <<
            ...
            case CEPH_LOCK_INEST:
                ...
                if (!is_auth() && lock->get_state() == LOCK_MIX) {
                    dout(10) <<
                        "finish_scatter_update try to assimilate dirty rstat on "
                        << *dir << dendl;
            ...
            if (lock->get_type() == CEPH_LOCK_INEST &&
                ...
                dout(10) <<
                    "finish_scatter_update finish assimilating dirty rstat on "
                    << *dir << dendl;
...
void CInode::finish_scatter_gather_update(int type)
{
    ...
    dout(10) << "finish_scatter_gather_update " << type << " on " << *this <<
        dendl;
    assert(is_auth());
    ...
    case CEPH_LOCK_IFILE:
        {
          ...
          for (const auto & p:dirfrags) {
                ...
                if (update) {
                    pf->accounted_fragstat = pf->fragstat;
                    pf->fragstat.version = pf->accounted_fragstat.version =
                        pi->dirstat.version;
            ...
            if (dirstat_valid && !dirstat.same_sums(pi->dirstat)) {
                ...
                for (list < frag_t >::iterator p = ls.begin(); p != ls.end();
                     ++p)
                    if (!dirfrags.count(*p)) {
                        dirstat_valid = false;
                        break;
    ...
    case CEPH_LOCK_INEST:
        {
          ...
          for (const auto & p:dirfrags) {
                ...
                if (update) {
                    ...
                    pf->rstat.version = pf->accounted_rstat.version =
                        pi->rstat.version;
                    dir->check_rstats();
...
void CInode::finish_scatter_gather_update_accounted(int type, MutationRef & mut,
                                                    EMetaBlob * metablob)
{
    dout(10) << "finish_scatter_gather_update_accounted " << type << " on " <<
        *this << dendl;
    assert(is_auth());
...
void CInode::add_dir_waiter(frag_t fg, MDSInternalContextBase * c)
{
    ...
    dout(10) << "add_dir_waiter frag " << fg << " " << c << " on " << *this <<
        dendl;
}
...
void CInode::auth_pin(void *by)
{
    ...
    dout(10) << "auth_pin by " << by << " on " << *this
        << " now " << auth_pins << "+" << nested_auth_pins << dendl;
...
void CInode::auth_unpin(void *by)
{
    ...
    if (is_freezing_inode() && auth_pins == auth_pin_freeze_allowance) {
...
CInode::mempool_old_inode & CInode::cow_old_inode(snapid_t follows,
                                                  bool cow_head)
{
...
void CInode::purge_stale_snap_data(const set < snapid_t > &snaps)
{
    ...
    for (auto it = old_inodes.begin(); it != old_inodes.end();) {
        ...
        if (s == snaps.end() || *s > id) {
            dout(10) << " purging old_inode [" << it->second.
                first << "," << id << "]" << dendl;
...
client_t CInode::calc_ideal_loner()
{
    ...
    for (map < client_t, Capability * >::iterator it = client_caps.begin();
        ...
        if (!it->second->is_stale() &&
            ((it->second->
              wanted() & (CEPH_CAP_ANY_WR | CEPH_CAP_FILE_WR |
                          CEPH_CAP_FILE_RD)) || (inode.is_dir()
                                                 &&
                                   ...
                                   SnapRealm * conrealm)
{
    if (client_caps.empty()) {
        ...
        dout(10) << "add_client_cap first cap, joining realm " <<
...
void CInode::remove_client_cap(client_t client)
{
    ...
    if (client_caps.empty()) {
        dout(10) << "remove_client_cap last cap, leaving realm " <<
            *containing_realm << dendl;
        put(PIN_CAPS);
...
int CInode::get_caps_allowed_for_client(Session * session,
                                        mempool_inode * file_i) const const
{
                            ...
                            int shift, int mask)
{
    ...
    for (map < client_t, Capability * >::const_iterator it =
         client_caps.begin(); it != client_caps.end(); ++it) {
                             ...
                             unsigned max_bytes, int getattr_caps)
{
        ...
        if (is_multiversion()) {
            ...
            if (it != old_inodes.end()) {
                ...
                if (it->second.first <= snapid && snapid <= it->first) {
                    ...
                    dout(0) << "encode_inodestat old_inode for snapid " <<
                        snapid << " not found" << dendl;
                }
            }
        }
    ...
    if (no_caps)
        dout(20) << "encode_inodestat no caps" << (!valid ? ", !valid" : "")
    ...
    dout(20) << " pfile " << pfile << " pauth " << pauth
        ...
        << " plocal " << plocal
        << " ctime " << any_i->ctime << " valid=" << valid << dendl;
    ...
    if (max_bytes) {
        unsigned bytes = 8 + 8 + 4 + 8 + 8 + sizeof(ceph_mds_reply_cap) +
            ...
            4 + 8 + 8 + 8 + 4 + 4 + 4 + 4 + 4 +
            8 + 8 + 8 + 8 + 8 + sizeof(struct ceph_timespec) + 4;
        bytes += sizeof(__u32);
    ...
    if (snapid != CEPH_NOSNAP) {
        ...
        if (last == CEPH_NOSNAP || is_any_caps())
            ecap.caps =
                ecap.caps & get_caps_allowed_for_client(session, file_i);
        ecap.seq = 0;
        ecap.mseq = 0;
        ecap.realm = 0;
    }
    ...
    if (xbl.length() && cap) {
        if ((cap->pending() | getattr_caps) & CEPH_CAP_XATTR_SHARED) {
            dout(10) << "including xattrs version " << xattr_i->
                xattr_version << dendl;
            cap->client_xattr_version = xattr_i->xattr_version;
        }
        else {
            dout(10) << "dropping xattrs version " << xattr_i->
                xattr_version << dendl;
...
void CInode::encode_cap_message(MClientCaps * m, Capability * cap)
{
    ...
    // max_size is min of projected, actual.
    uint64_t oldms =
        oi->client_ranges.count(client) ? oi->client_ranges[client].range.
        last : 0;
    uint64_t newms =
        pi->client_ranges.count(client) ? pi->client_ranges[client].range.
        last : 0;
...
void CInode::decode_import(bufferlist::iterator & p, LogSegment * ls)
{
    DECODE_START(5, p);
          ...
          next:
            ...
            // (#15619)
            {
                ...
                if (inotable->is_marked_free(inode.ino)) {
                    ...
                    clog->
                        error() << "scrub: inode wrongly marked free: 0x" <<
                        std::hex << inode.ino;
                    ...
                    if (in->scrub_infop->header->get_repair()) {
                        ...
                        if (repaired) {
                            clog->
                                error() << "inode table repaired for inode: 0x"
                                << std::hex << inode.ino;
        ...
        bool _dirfrags(int rval) {
          ...
          for (const auto & p:in->dirfrags) {
            ...
            if (!dir_info.same_sums(in->inode.dirstat) ||
                ...
                if (in->scrub_infop && in->scrub_infop->header->get_repair()) {
                    results->raw_stats.error_str
                        <<
    ...
    f->open_object_section("results");
    {
        ...
        f->open_object_section("raw_stats");
        {
            ...
            f->dump_stream("ondisk_value.rstat") << raw_stats.ondisk_value.
                rstat;
            f->dump_stream("memory_value.dirrstat") << raw_stats.memory_value.
                dirstat;
...
void CInode::dump(Formatter * f) const const
{
...
void CInode::scrub_finished(MDSInternalContextBase ** c)
{
    ...
    for (std::map < frag_t, scrub_stamp_info_t >::iterator i =
         scrub_infop->dirfrag_stamps.begin();
         i != scrub_infop->dirfrag_stamps.end(); ++i) {
...
bool CInode::is_exportable(mds_rank_t dest) const const
{
