...
unsafe_sync_write(0), client_lock("Client::client_lock"), deleg_timeout(0)
{
    _reset_faked_inos();
    //
    root = 0;
...
void Client::tear_down_cache()
{
    ...
    while (!opened_dirs.empty()) {
        ...
        ldout(cct,
              1) << "tear_down_cache forcing close of dir " << dirp << " ino "
                        ...
                        bool disconnected)
{
    ...
    ldout(cct, 1) << "dump_inode: " << (disconnected ? "DISCONNECTED " : "")
        << "inode " << in->ino << " " << path << " ref " << in->get_num_ref()
        << *in << dendl;
...
void Client::trim_cache(bool trim_kernel_dcache)
{
    ...
    while (lru.lru_get_size() != last) {
        ...
        if (!unmounting && lru.lru_get_size() <= max)
            break;
...
void Client::trim_dentry(Dentry * dn)
{
    ldout(cct, 15) << "trim_dentry unlinking dn " << dn->name
        << " in dir " << hex << dn->dir->parent_inode->ino << dendl;
    if (dn->inode) {
                                    ...
                                    uint64_t truncate_size)
{
    ...
    if (truncate_seq > in->truncate_seq ||
        ...
        if (truncate_seq != in->truncate_seq) {
            ...
            if (prior_size > size) {
                _invalidate_inode_cache(in, truncate_size,
                                        prior_size - truncate_size);
            }
        }
                                ...
                                const UserPerm & request_perms)
{
    ...
    if (inode_map.count(st->vino)) {
        ...
        ldout(cct,
              12) << "add_update_inode had " << *in << " caps " <<
                                    ...
                                    Dentry * old_dentry)
{
    ...
    ldout(cct,
          12) << "insert_dentry_inode '" << dname << "' vino " << in->vino()
...
mds_rank_t Client::choose_target_mds(MetaRequest * req, Inode ** phash_diri)
{
    ...
    if (in) {
        ...
        if (de->inode) {
            in = de->inode.get();
            ldout(cct,
                  20) << "choose_target_mds starting with req->dentry inode " <<
                *in << dendl;
        }
        else {
    ...
    if (in) {
        ...
        ldout(cct,
              10) << "choose_target_mds from caps on inode " << *in << dendl;
...
void Client::dump_mds_sessions(Formatter * f)
{
    ...
    for (map < mds_rank_t, MetaSession * >::const_iterator p =
         mds_sessions.begin(); p != mds_sessions.end(); ++p) {
                                 ...
                                 int unless, int force)
{
    ...
    if (in->caps.count(mds)) {
        ...
        if ((drop & caps->issued) && !(unless & caps->issued)) {
            ldout(cct,
            ...
            released = 1;
            ldout(cct,
                  25) << "Now have: " << ccap_string(caps->issued) << dendl;
        }
        else {
            released = force;
        }
...
{
    ldout(cct, 20) << "encode_dentry_release enter(dn:" << dn << ")" << dendl;
    int released = 0;
    if (dn->dir)
...
{
    ldout(cct,
          2) << "_close_mds_session mds." << s->mds_num << " seq " << s->
        seq << dendl;
    s->state = MetaSession::STATE_CLOSING;
    s->con->
        send_message(new MClientSession(CEPH_SESSION_REQUEST_CLOSE, s->seq));
}
...
void Client::handle_client_session(MClientSession * m)
{
    ...
    ldout(cct,
          10) << "handle_client_session " << *m << " from mds." << from <<
        dendl;
    ...
    case CEPH_SESSION_FLUSHMSG:
        ...
        session->con->
            send_message(new
                         MClientSession(CEPH_SESSION_FLUSHMSG_ACK,
                                        m->get_seq()));
        break;
...
void Client::handle_client_request_forward(MClientRequestForward * fwd)
{
    ...
    if (mds_requests.count(tid) == 0) {
        ldout(cct,
              10) << "handle_client_request_forward no pending request on tid "
            << tid << dendl;
        fwd->put();
        return;
    }
...
void Client::handle_client_reply(MClientReply * reply)
{
    ...
    if (!is_safe || !request->got_unsafe) {
        ...
        // wake up waiter
        ldout(cct,
              20) << "handle_client_reply signalling caller " << (void *)
            request->caller_cond << dendl;
        request->caller_cond->Signal();
...
void Client::handle_osd_map(MOSDMap * m)
{
    ...
    if (!blacklisted && new_blacklists.count(myaddr)) {
        ...
        blacklisted = true;
        for (std::map < ceph_tid_t, MetaRequest * >::iterator p =
             mds_requests.begin(); p != mds_requests.end();) {
            auto req = p->second;
            ++p;
...
bool Client::ms_dispatch(Message * m)
{
    ...
    case CEPH_MSG_CLIENT_REQUEST_FORWARD:
        handle_client_request_forward(static_cast <
                                      MClientRequestForward * >(m));
        break;
...
void Client::handle_mds_map(MMDSMap * m)
{
  ...
  for (const auto & i:commands) {
        ...
        if (mdsmap->is_dne_gid(op_mds_gid) || mdsmap->is_laggy_gid(op_mds_gid)) {
            ldout(cct,
                  1) << __func__ << ": cancelling command op " << i.
                first << dendl;
...
{
    ldout(cct,
          10) << "kick_requests_closed for mds." << session->mds_num << dendl;
    for (map < ceph_tid_t, MetaRequest * >::iterator p = mds_requests.begin();
         p != mds_requests.end();) {
        MetaRequest *req = p->second;
        ++p;
        if (req->mds == session->mds_num) {
            ...
            if (req->got_unsafe) {
                lderr(cct) << "kick_requests_closed removing unsafe request " <<
                    req->get_tid() << dendl;
...
void Client::got_mds_push(MetaSession * s)
{
    ...
    if (s->state == MetaSession::STATE_CLOSING) {
        s->con->
            send_message(new
                         MClientSession(CEPH_SESSION_REQUEST_CLOSE, s->seq));
    }
}
...
void Client::handle_lease(MClientLease * m)
{
  ...
  revoke:
    m->get_connection()->
        send_message(new
                     MClientLease(CEPH_MDS_LEASE_RELEASE, seq, m->get_mask(),
                                  m->get_ino(), m->get_first(), m->get_last(),
                                  m->dname));
    m->put();
}
...
void Client::get_cap_ref(Inode * in, int cap)
{
    if ((cap & CEPH_CAP_FILE_BUFFER) && in->cap_refs[CEPH_CAP_FILE_BUFFER] == 0) {
        ldout(cct,
              5) << "get_cap_ref got first FILE_BUFFER ref on " << *in << dendl;
        in->get();
    }
    if ((cap & CEPH_CAP_FILE_CACHE) && in->cap_refs[CEPH_CAP_FILE_CACHE] == 0) {
        ldout(cct,
              5) << "get_cap_ref got first FILE_CACHE ref on " << *in << dendl;
        in->get();
    }
    in->get_cap_ref(cap);
}
        ...
        if (in->snapid == CEPH_NOSNAP) {
            ...
            if (last & CEPH_CAP_FILE_BUFFER) {
              for (auto & p:in->cap_snaps)
                ...
                ldout(cct,
                      5) << "put_cap_ref dropped last FILE_BUFFER ref on " <<
                    *in << dendl;
                ++put_nref;
            }
        }
...
void Client::check_caps(Inode * in, unsigned flags)
{
    ...
    while (it != in->caps.end()) {
      ...
      ack:
        ...
        send_cap(in, session, cap, flags & CHECK_CAPS_SYNCHRONOUS, cap_used,
                 wanted, retain, flushing, flush_tid);
    }
}
...
void Client::queue_cap_snap(Inode * in, SnapContext & old_snapc)
{
    ...
    ldout(cct,
          10) << "queue_cap_snap " << *in << " snapc " << old_snapc << " used "
    ...
    if (in->cap_snaps.size() && in->cap_snaps.rbegin()->second.writing) {
        ...
        ldout(cct,
              10) << "queue_cap_snap not dirty|writing on " << *in << dendl;
    }
}
...
{
    ldout(cct,
          10) << "finish_cap_snap " << *in << " capsnap " << (void *)&capsnap <<
    ...
    if (used & CEPH_CAP_FILE_BUFFER) {
        ldout(cct,
              10) << "finish_cap_snap " << *in << " cap_snap " << &capsnap <<
...
{
    ldout(cct,
          10) << "flush_snaps on " << *in << " all_again " << all_again <<
        dendl;
  ...
  for (auto & p:in->cap_snaps) {
        ...
        MClientCaps *m =
            new MClientCaps(CEPH_CAP_OP_FLUSHSNAP, in->ino, in->snaprealm->ino,
                            0, mseq,
                            cap_epoch_barrier);
        if (user_id >= 0)
...
void Client::_async_invalidate(vinodeno_t ino, int64_t off, int64_t len)
{
    if (unmounting)
        return;
    ldout(cct,
          10) << "_async_invalidate " << ino << " " << off << "~" << len <<
        dendl;
...
{
    ldout(cct,
          10) << "_invalidate_inode_cache " << *in << " " << off << "~" << len
        << dendl;
                            ...
                            int flags, const UserPerm & cap_perms)
{
    ...
    if ((issued & ~old_caps) && in->auth_cap == cap) {
        ...
        for (map < mds_rank_t, Cap * >::iterator it = in->caps.begin();
             it != in->caps.end(); ++it) {
            if (it->second == cap)
                continue;
...
int Client::_do_remount(bool retry_on_error)
{
    uint64_t max_retries =
        cct->_conf->get_val < uint64_t > ("mds_max_retries_on_remount_failure");
...
void Client::wait_sync_caps(ceph_tid_t want)
{
  retry:
    ldout(cct,
          10) << "wait_sync_caps want " << want << " (last is " <<
        last_flush_tid << ", " << num_flushing_caps << " total flushing)" <<
        dendl;
    for (map < mds_rank_t, MetaSession * >::iterator p = mds_sessions.begin();
         p != mds_sessions.end(); ++p) {
        MetaSession *s = p->second;
...
void SnapRealm::build_snap_context()
{
    ...
    for (set < snapid_t >::reverse_iterator p = snaps.rbegin();
         p != snaps.rend(); ++p)
...
void Client::invalidate_snaprealm_and_children(SnapRealm * realm)
{
    ...
    while (!q.empty()) {
        ...
        ldout(cct,
              10) << "invalidate_snaprealm_and_children " << *realm << dendl;
...
SnapRealm *Client::get_snap_realm(inodeno_t r)
{
    ...
    ldout(cct,
          20) << "get_snap_realm " << r << " " << realm << " " << realm->
...
SnapRealm *Client::get_snap_realm_maybe(inodeno_t r)
{
    ...
    SnapRealm *realm = snap_realms[r];
    ldout(cct,
          20) << "get_snap_realm_maybe " << r << " " << realm << " " << realm->
        nref << " -> " << (realm->nref + 1) << dendl;
    realm->nref++;
    return realm;
}
...
void Client::update_snap_trace(bufferlist & bl, SnapRealm ** realm_ret,
                               bool flush)
{
    ...
    while (!p.end()) {
        ...
        if (info.seq() > realm->seq) {
            ldout(cct,
                  10) << "update_snap_trace " << *realm << " seq " << info.
                seq() << " > " << realm->seq << dendl;
        ...
        if (invalidate) {
            ...
            ldout(cct,
                  10) << "update_snap_trace " << *realm << " seq " << info.seq()
                << " <= " << realm->
...
void Client::handle_snap(MClientSnap * m)
{
    ...
    if (m->head.op == CEPH_SNAP_OP_SPLIT) {
        ...
        for (vector < inodeno_t >::iterator p = m->split_realms.begin();
             p != m->split_realms.end(); ++p) {
            ldout(cct,
                  10) << "adjusting snaprealm " << *p << " parent" << dendl;
...
void Client::handle_caps(MClientCaps * m)
{
    ...
    if (!in) {
        if (m->get_op() == CEPH_CAP_OP_IMPORT) {
            ...
            ldout(cct,
                  5) << "handle_caps don't have vino " << vino << ", dropping"
                << dendl;
        }
        m->put();
    ...
    if (in->caps.count(mds) == 0) {
        ldout(cct,
              5) << "handle_caps don't have " << *in << " cap on mds." << mds <<
            dendl;
        m->put();
        return;
    }
                               ...
                               MClientCaps * m)
{
    ...
    ldout(cct,
          5) << "handle_cap_import ino " << m->get_ino() << " mseq " << m->
        get_mseq()
                               ...
                               MClientCaps * m)
{
    ...
    ldout(cct,
          5) << "handle_cap_export ino " << m->get_ino() << " mseq " << m->
        get_mseq()
                                      ...
                                      MClientCaps * m)
{
    ...
    if (in->cap_snaps.count(follows)) {
        ...
        if (m->get_client_tid() != capsnap.flush_tid) {
            ldout(cct,
                  10) << " tid " << m->get_client_tid() << " != " << capsnap.
                flush_tid << dendl;
        }
        else {
            ldout(cct,
                        ...
                        std::vector < mds_gid_t > *targets)
{
    ...
    if (role_r == 0) {
        ...
        ldout(cct,
            ...
            role << "'" << dendl;
        targets->push_back(fsmap->get_filesystem(role.fscid)->mds_map.
                           get_info(role.rank).global_id);
        return 0;
    }
    ...
    if (strtol_err.empty()) {
        ...
        if (fsmap->gid_exists(mds_gid)) {
            ldout(cct,
                  10) << __func__ << ": validated GID " << mds_gid << dendl;
        ...
        if (mds_info.empty()) {
            lderr(cct) << __func__ << ": * passed but no MDS daemons found" <<
                dendl;
            return -ENOENT;
        }
...
int Client::fetch_fsmap(bool user)
{
    ...
    if (r < 0) {
        lderr(cct) << "Failed to learn FSMap version: " << cpp_strerror(r) <<
            dendl;
        return r;
    }
...
void Client::_close_sessions()
{
    while (!mds_sessions.empty()) {
        // send session closes!
        for (map < mds_rank_t, MetaSession * >::iterator p =
             mds_sessions.begin(); p != mds_sessions.end(); ++p) {
...
void Client::_unmount()
{
    ...
    while (unsafe_sync_write > 0) {
        ldout(cct,
              0) << unsafe_sync_write << " unsafe_sync_writes, waiting" <<
            dendl;
        mount_cond.Wait(client_lock);
    }
    ...
    if (cct->_conf->client_oc) {
        ...
        for (ceph::unordered_map < vinodeno_t, Inode * >::iterator p =
             inode_map.begin(); p != inode_map.end(); p = next) {
            next = p;
            ++next;
            Inode *in = p->second;
            if (!in) {
...
void Client::tick()
{
    ...
    tick_event = timer.add_event_after(cct->_conf->client_tick_interval,
...
void Client::renew_caps()
{
    ...
    for (map < mds_rank_t, MetaSession * >::iterator p = mds_sessions.begin();
         p != mds_sessions.end(); ++p) {
        ldout(cct,
              15) << "renew_caps requesting from mds." << p->first << dendl;
...
void Client::renew_caps(MetaSession * session)
{
    ...
    session->con->
        send_message(new MClientSession(CEPH_SESSION_REQUEST_RENEWCAPS, seq));
}
                    ...
                    InodeRef * target, const UserPerm & perms)
{
    ...
    if (dir->dir && dir->dir->dentries.count(dname)) {
        ...
        ldout(cct,
              20) << "_lookup have dn " << dname << " mds." << dn->
        ...
        if (!dn->inode || dn->inode->caps_issued_mask(mask, true)) {
            ...
            utime_t now = ceph_clock_now();
            if (dn->lease_mds >= 0 &&
                dn->lease_ttl > now && mds_sessions.count(dn->lease_mds)) {
  ...
  hit_dn:
    ...
    if (r < 0)
        ldout(cct,
              10) << "_lookup " << *dir << " " << dname << " = " << r << dendl;
    else
        ldout(cct,
              10) << "_lookup " << *dir << " " << dname << " = " << **target <<
            dendl;
    return r;
}
...
int Client::mkdirs(const char *relpath, mode_t mode, const UserPerm & perms)
{
    ...
    if (r != -ENOENT)
        return r;
    ldout(cct,
          20) << "mkdirs got through " << i << " directories on path " <<
        relpath << dendl;
...
int Client::_getattr(Inode * in, int mask, const UserPerm & perms, bool force)
{
    ...
    ldout(cct,
          10) << "_getattr mask " << ccap_string(mask) << " issued=" << yes <<
        dendl;
    if (yes && !force)
        return 0;
...
{
    ldout(cct,
          3) << "stat enter (relpath " << relpath << " mask " << mask << ")" <<
        dendl;
    Mutex::Locker lock(client_lock);
    ...
    ldout(cct,
          3) << "stat exit (relpath " << relpath << " mask " << mask << ")" <<
        dendl;
    return r;
}
...
unsigned Client::statx_to_mask(unsigned int flags, unsigned int want)
{
    ...
    if (want &
        (CEPH_STATX_ATIME | CEPH_STATX_MTIME | CEPH_STATX_CTIME |
...
{
    ldout(cct,
          3) << "statx enter (relpath " << relpath << " want " << want << ")" <<
        dendl;
    Mutex::Locker lock(client_lock);
    ...
    ldout(cct,
          3) << "statx exit (relpath " << relpath << " mask " << stx->
        stx_mask << ")" << dendl;
    return r;
}
...
{
    ldout(cct,
          3) << "lstat enter (relpath " << relpath << " mask " << mask << ")" <<
        dendl;
    Mutex::Locker lock(client_lock);
    ...
    fill_stat(in, stbuf, dirstat);
    ldout(cct,
          3) << "lstat exit (relpath " << relpath << " mask " << mask << ")" <<
        dendl;
    return r;
}
                         ...
                         uint64_t ino, loff_t next_off)
{
    ...
    ldout(cct,
          10) << "fill_dirent '" << de->d_name << "' -> " << inodeno_t(de->
                                                                       d_ino)
        << " type " << (int)de->
...
void Client::_readdir_next_frag(dir_result_t * dirp)
{
    ...
    if (fg.is_rightmost()) {
        ldout(cct,
              10) << "_readdir_next_frag advance from " << fg << " to END" <<
            dendl;
        dirp->set_end();
        return;
    }
...
void Client::_readdir_rechoose_frag(dir_result_t * dirp)
{
    ...
    if (fg != cur) {
        ldout(cct,
              10) << "_readdir_rechoose_frag frag " << cur << " maps to " << fg
            << dendl;
        dirp->offset = dir_result_t::make_fpos(fg, 2, false);
        dirp->last_name.clear();
        dirp->next_offset = 2;
...
int Client::_readdir_get_frag(dir_result_t * dirp)
{
    ...
    ldout(cct,
          10) << "_readdir_get_frag " << dirp << " on " << dirp->inode->
    ...
    if (res == 0) {
        ldout(cct,
              10) << "_readdir_get_frag " << dirp << " got frag " << dirp->
                              ...
                              int caps, bool getref)
{
    ...
    ldout(cct,
          10) << "_readdir_cache_cb " << dirp << " on " << dirp->inode->
        ino << " at end" << dendl;
    dirp->set_end();
    return 0;
}
                         ...
                         unsigned want, unsigned flags, bool getref)
{
    ...
    ldout(cct,
          10) << "readdir_r_cb " << *dirp->inode << " offset " << hex << dirp->
        ...
        if (diri->shared_gen == dirp->start_shared_gen &&
            ...
            if (diri->dir_ordered_count == dirp->ordered_count) {
                ldout(cct,
                    ...
                    << dendl;
                if (diri->dir) {
                    assert(diri->dir->readdir_cache.size() >=
                           dirp->cache_index);
...
{
    ldout(cct,
          3) << "open enter(" << relpath << ", " << ceph_flags_sys2wire(flags)
        << "," << mode << ")" << dendl;
    Mutex::Locker lock(client_lock);
  ...
  out:
    tout(cct) << r << std::endl;
    ldout(cct,
          3) << "open exit(" << path << ", " << ceph_flags_sys2wire(flags) <<
        ") = " << r << dendl;
    return r;
}
                        ...
                        const UserPerm & perms)
{
    Mutex::Locker lock(client_lock);
    ldout(cct,
          3) << "lookup_hash enter(" << ino << ", #" << dirino << "/" << name <<
        ")" << dendl;
    ...
    ldout(cct,
          3) << "lookup_hash exit(" << ino << ", #" << dirino << "/" << name <<
        ") = " << r << dendl;
    return r;
}
...
Fh *Client::_create_fh(Inode * in, int flags, int cmode, const UserPerm & perms)
{
    ...
    if (conf->client_readahead_max_bytes) {
        max_readahead =
            MIN(max_readahead, (uint64_t) conf->client_readahead_max_bytes);
    }
...
int Client::_release_fh(Fh * f)
{
    ...
    if (err != 0) {
        ldout(cct,
              1) << "_release_fh " << f << " on inode " << *in <<
            " caught async_err = " << cpp_strerror(err) << dendl;
    }
    else {
        ldout(cct,
              10) << "_release_fh " << f << " on inode " << *in <<
...
int Client::_read_async(Fh * f, uint64_t off, uint64_t len, bufferlist * bl)
{
    ...
    ldout(cct,
          10) << "_read_async " << *in << " " << off << "~" << len << dendl;
...
void Client::_sync_write_commit(Inode * in)
{
    ...
    if (unsafe_sync_write == 0 && unmounting) {
        ldout(cct,
              10) <<
            "sync_write_commit -- no more unsafe writes, unmount can proceed" <<
            dendl;
        mount_cond.Signal();
    }
}
                   ...
                   const UserPerm & perms)
{
    ...
    assert(root != nullptr);
    Inode *quota_root =
        root->quota.is_enable()? root : get_quota_root(root, perms);
    ...
    if (ret == 0) {
        if (op == CEPH_MDS_OP_GETFILELOCK) {
            ...
            if (lock_type == CEPH_LOCK_FCNTL) {
                if (!in->fcntl_locks)
                    in->fcntl_locks =
                        new ceph_lock_state_t(cct, CEPH_LOCK_FCNTL);
                lock_state = in->fcntl_locks;
            }
            else if (lock_type == CEPH_LOCK_FLOCK) {
                if (!in->flock_locks)
                    in->flock_locks =
                        new ceph_lock_state_t(cct, CEPH_LOCK_FLOCK);
...
void Client::_encode_filelocks(Inode * in, bufferlist & bl)
{
    ...
    if (nr_fcntl_locks) {
        ...
        for (multimap < uint64_t, ceph_filelock >::iterator p =
             lock_state->held_locks.begin(); p != lock_state->held_locks.end();
             ++p)
            ::encode(p->second, bl);
    }
    ...
    ldout(cct,
        ...
        ino << ", " << nr_fcntl_locks << " fcntl locks, " << nr_flock_locks <<
        " flock locks" << dendl;
}
...
void Client::_release_filelocks(Fh * fh)
{
    ...
    ldout(cct,
          10) << "_release_filelocks " << fh << " ino " << in->ino << dendl;
    ...
    if (fh->fcntl_locks) {
        ...
        for (multimap < uint64_t, ceph_filelock >::iterator p =
            ...
            to_release.push_back(pair < int,
                                 ceph_filelock > (CEPH_LOCK_FCNTL, p->second));
    ...
    if (fh->flock_locks) {
        ...
        for (multimap < uint64_t, ceph_filelock >::iterator p =
             ...
             ++p)
            to_release.push_back(pair < int,
                                 ceph_filelock > (CEPH_LOCK_FLOCK, p->second));
...
int Client::test_dentry_handling(bool can_invalidate)
{
    ...
    if (r) {
        ...
        if (should_abort) {
            lderr(cct) <<
                "no method to invalidate kernel dentry cache; quitting!" <<
                dendl;
            ceph_abort();
        }
        else {
            lderr(cct) <<
                "no method to invalidate kernel dentry cache; expect issues!" <<
                dendl;
        }
    }
    return r;
                        ...
                        size_t size, const UserPerm & perms)
{
    ...
    ldout(cct,
          3) << "ll_getxattr " << vino << " " << name << " size " << size <<
        dendl;
                        ...
                        size_t size, int flags, const UserPerm & perms)
{
    ...
    ldout(cct,
          3) << "ll_setxattr " << vino << " " << name << " size " << size <<
        dendl;
                           ...
                           uint64_t snapseq, uint32_t sync)
{
    ...
    objecter->write(oid,
                    object_locator_t(layout->pool_id),
                    offset,
                    length, fakesnap, bl, ceph::real_clock::now(), 0, onsafe);
        ...
        if (in->inline_version < CEPH_INLINE_NONE &&
            ...
            if (offset < len) {
                ...
                if (size > 0)
                    bl.append_zero(size);
                if (offset + size < len)
                    in->inline_data.copy(offset + size, len - offset - size,
                                         bl);
                in->inline_data = bl;
                in->inline_version++;
            }
...
int Client::ll_fallocate(Fh * fh, int mode, loff_t offset, loff_t length)
{
    Mutex::Locker lock(client_lock);
    ldout(cct,
          3) << "ll_fallocate " << fh << " " << fh->inode->ino << " " << dendl;
...
int Client::get_pool_replication(int64_t pool)
{
    ...
    return objecter->with_osdmap([pool] (const OSDMap & o) {
                                 return o.have_pg_pool(pool) ? o.
                                 get_pg_pool(pool)->get_size() : -ENOENT;}
    );
}
                                    ...
                                    vector < entity_addr_t > &address)
{
    ...
    return objecter->with_osdmap([&](const OSDMap & o) {
                                 ...
                                 o.pg_to_acting_osds(pg, osds);
                                 if (osds.empty())
                                 return -EINVAL; for (unsigned i = 0;
                                                      i < osds.size(); i++) {
                             ...
                             loff_t length, loff_t offset)
{
    ...
    ldout(cct,
          3) << "enumerate_layout(" << fd << ", " << length << ", " << offset <<
        ") = 0" << dendl;
    return 0;
}
...
{
    ldout(cct,
          0) << "ms_handle_remote_reset on " << con->get_peer_addr() << dendl;
    Mutex::Locker l(client_lock);
                                     ...
                                     std::list < InodeRef > *quota_roots)
{
    return check_quota_condition(in, perms,
                                 ...
                                 if (quota_roots)
                                 quota_roots->emplace_back(const_cast <
                                                           Inode * >(&in));
...
int Client::check_pool_perm(Inode * in, int need)
{
    ...
    if (!have) {
        ...
        if (rd_ret == 0 || rd_ret == -ENOENT)
            ...
            ldout(cct,
                  10) << "check_pool_perm on pool " << pool_id << " ns " <<
                pool_ns << " rd_err = " << rd_ret << " wr_err = " << wr_ret <<
                dendl;
            errored = true;
        }
    ...
    if ((need & CEPH_CAP_FILE_RD) && !(have & POOL_READ)) {
        ldout(cct,
              10) << "check_pool_perm on pool " << pool_id << " ns " << pool_ns
...
{
    Mutex::Locker l(client_lock);
    assert(flags == 0 || flags == CEPH_OSD_FLAG_LOCALIZE_READS);
