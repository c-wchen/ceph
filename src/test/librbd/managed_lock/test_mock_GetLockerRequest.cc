    ...
    namespace managed_lock {
                ...
                if (r < 0 && r != -ENOENT) {
                    ...
                    if (r != -ENOENT) {
                        reply.lockers = decltype(reply.lockers) {
                            {
                            rados::cls::lock::locker_id_t(entity,
                                                              locker_cookie),
                                    rados::cls::lock::
                                    locker_info_t(utime_t(), entity_addr, "")}
                        };
                        reply.tag = lock_tag;
        ...
        TEST_F(TestMockManagedLockGetLockerRequest, SuccessExclusive) {
            ...
            expect_get_lock_info(mock_image_ctx, 0, entity_name_t::CLIENT(1),
                                 "1.2.3.4", "auto 123",
                                 util::get_watcher_lock_tag(), LOCK_EXCLUSIVE);

            C_SaferCond ctx;
            Locker locker;
            MockGetLockerRequest *req =
                MockGetLockerRequest::create(mock_image_ctx.md_ctx,
                                             mock_image_ctx.header_oid, true,
                                             &locker, &ctx);
            req->send();
        ...
        TEST_F(TestMockManagedLockGetLockerRequest, SuccessShared) {
            ...
            expect_get_lock_info(mock_image_ctx, 0, entity_name_t::CLIENT(1),
                                 "1.2.3.4", "auto 123",
                                 util::get_watcher_lock_tag(), LOCK_SHARED);

            C_SaferCond ctx;
            Locker locker;
            MockGetLockerRequest *req =
                MockGetLockerRequest::create(mock_image_ctx.md_ctx,
                                             mock_image_ctx.header_oid, false,
                                             &locker, &ctx);
            req->send();
            ASSERT_EQ(0, ctx.wait());
        ...
        TEST_F(TestMockManagedLockGetLockerRequest,
