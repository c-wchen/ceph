...
struct failure_info_t {
    ...
    void add_report(int who, utime_t failed_since, MonOpRequestRef op) {
        [[maybe_unused]] auto[it, new_reporter] =
            reporters.insert_or_assign(who, failure_reporter_t {
                                       failed_since, op});
        if (new_reporter) {
...
class OSDMonitor:public PaxosService, public md_config_obs_t {
  ...
  public:
    ...
    bool priority_convert = false;
     ...
     std::shared_ptr < PriorityCache::PriCache > rocksdb_binned_kv_cache =
        nullptr;
    ...
    using osdmap_cache_t = SimpleLRU < osdmap_key_t,
        ceph::buffer::list,
        std::less < osdmap_key_t >, boost::hash < osdmap_key_t >>;
    ...
    utime_t get_grace_time(utime_t now, int target_osd,
    ...
    struct CleanUpmapJob:public ParallelPGMapper::Job {
        ...
        // when checking is done
         ...
         CleanUpmapJob(CephContext * cct, const OSDMap & om,
                       OSDMap::Incremental & pi)
        :ParallelPGMapper::Job(&om), cct(cct), osdmap(om), pending_inc(pi) {
            ...
            std::map < pg_t, mempool::osdmap::vector < std::pair < int,
                int >>>to_remap;
        ...
        }} void process(int64_t poolid, unsigned ps_begin,
                        unsigned ps_end) override {
        }
  ...
  private:
    ...
    void _prune_update_trimmed(MonitorDBStore::TransactionRef tx,
                               version_t first);
   ...
   * full osdmaps.
   */
    void encode_trim_extra(MonitorDBStore::TransactionRef tx,
   ...
   */
    ...
    struct PrimeTempJob:public ParallelPGMapper::Job {
        ...
        } void process(int64_t pool, unsigned ps_begin,
                       unsigned ps_end) override {
  ...
  private:
    void print_utilization(std::ostream & out, ceph::Formatter * f,
                           bool tree) const;
    ...
    int _check_remove_pool(int64_t pool_id, const pg_pool_t & pool,
                           std::ostream * ss);
    bool _check_become_tier(int64_t tier_pool_id, const pg_pool_t * tier_pool,
                            int64_t base_pool_id, const pg_pool_t * base_pool,
                            int *err, std::ostream * ss) const;
    bool _check_remove_tier(int64_t base_pool_id, const pg_pool_t * base_pool,
                            const pg_pool_t * tier_pool, int *err,
    ...
    int prepare_pool_crush_rule(const unsigned pool_type,
                                ...
                                int *crush_rule, std::ostream * ss);
    bool erasure_code_profile_in_use(const mempool::osdmap::map < int64_t,
                                     pg_pool_t > &pools,
                                     const std::string & profile,
                                     std::ostream * ss);
    int parse_erasure_code_profile(const std::vector < std::string >
                                   &erasure_code_profile,
                                   std::map < std::string,
                                   std::string > *erasure_code_profile_map,
                                   std::ostream * ss);
    int prepare_pool_size(const unsigned pool_type,
                          const std::string & erasure_code_profile,
                          uint8_t repl_size, unsigned *size, unsigned *min_size,
                          std::ostream * ss);
    ...
    int check_pg_num(int64_t pool, int pg_num, int size, int crush_rule,
                     std::ostream * ss);
    int prepare_new_pool(std::string & name, int crush_rule,
                         const std::string & crush_rule_name, unsigned pg_num,
                         unsigned pgp_num, unsigned pg_num_min,
                         unsigned pg_num_max, uint64_t repl_size,
                         const uint64_t target_size_bytes,
                         const float target_size_ratio,
                         const std::string & erasure_code_profile,
                         const unsigned pool_type,
                         const uint64_t expected_num_objects,
                         FastReadType fast_read, std::string pg_autoscale_mode,
    ...
    std::string make_purged_snap_key_value(int64_t pool, snapid_t snap,
                                           snapid_t num, epoch_t epoch,
                                           ceph::buffer::list * v);
    ...
    void insert_purged_snap_update(int64_t pool,
    ...
    void _pool_op_reply(MonOpRequestRef op,
                        int ret, epoch_t epoch, ceph::buffer::list * blp =
                        NULL);
    ...
    struct C_PoolOp:public C_MonOp {
       ...
       C_PoolOp(OSDMonitor * osd, MonOpRequestRef op_, int rc, int e, ceph::buffer::list * rd = NULL):
        C_MonOp(op_), osdmon(osd), replyCode(rc),
            epoch(e) {
            if (rd)
                reply_data = *rd;
        } void _finish(int r) override {
  ...
  public:
    void count_metadata(const std::string & field, std::map < std::string,
                        int >*out);
    void get_versions(std::map < std::string,
  ...
  protected:
    ...
    bool is_pool_currently_all_bluestore(int64_t pool_id,
    ...
    std::map < int, std::map < epoch_t,
        std::set < spg_t >>> creating_pgs_by_osd_epoch;
    ...
    unsigned scan_for_creating_pgs(const mempool::osdmap::map < int64_t,
                                   pg_pool_t > &pools,
                                   const mempool::osdmap::set < int64_t >
                                   &removed_pools, utime_t modified,
    ...
    int parse_pgid(const cmdmap_t & cmdmap, std::stringstream & ss,
                   pg_t & pgid, std::optional < std::string > pgidstr =
                   std::nullopt);
  public:
    OSDMonitor(CephContext * cct, Monitor & mn, Paxos & p,
    ...
    int validate_osd_create(const int32_t id,
                            const uuid_d & uuid,
                            const bool check_osd_exists,
                            int32_t * existing_id, std::stringstream & ss);
    int prepare_command_osd_create(const int32_t id,
                                   const uuid_d & uuid,
                                   int32_t * existing_id,
                                   std::stringstream & ss);
    void do_osd_create(const int32_t id, const uuid_d & uuid,
                       const std::string & device_class, int32_t * new_id);
    ...
    int _prepare_command_osd_crush_remove(CrushWrapper & newcrush,
                                          int32_t id,
                                          int32_t ancestor,
                                          bool has_ancestor, bool unlink_only);
    void do_osd_crush_remove(CrushWrapper & newcrush);
    int prepare_command_osd_crush_remove(CrushWrapper & newcrush,
                                         int32_t id,
                                         int32_t ancestor,
                                         bool has_ancestor, bool unlink_only);
    int prepare_command_osd_remove(int32_t id);
    int prepare_command_osd_new(MonOpRequestRef op,
                                const cmdmap_t & cmdmap,
                                const std::map < std::string,
                                std::string > &secrets, std::stringstream & ss,
                                ceph::Formatter * f);
    ...
    int get_version_full(version_t ver, uint64_t feature,
                         ceph::buffer::list & bl);
   ...
   */
    bool check_for_dead_crush_zones(const std::map < std::string,
                                    std::set < std::string >> &dead_buckets,
