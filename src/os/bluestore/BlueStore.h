...
public:
        ...
        void maybe_rebuild()
        {
            if (data.length() &&
                (data.get_num_buffers() > 1 ||
                 data.front().wasted() >
                 data.length() / MAX_BUFFER_SLOP_RATIO_DEN)) {
                data.rebuild();
            }
        }
    ...
    struct BufferSpace {
        ...
        mempool::bluestore_cache_meta::map < uint32_t,
                std::unique_ptr < Buffer >> buffer_map;
        ...
        void _rm_buffer(BufferCacheShard *cache,
                        std::map < uint32_t,
                        std::unique_ptr < Buffer >>::iterator p)
        {
        ...
        std::map < uint32_t,
            std::unique_ptr <
            Buffer >>::iterator _data_lower_bound(uint32_t offset)
        {
        ...
        int _discard(BufferCacheShard *cache, uint32_t offset,
                     uint32_t length);
                   ...
                   ceph::buffer::list &bl, unsigned flags)
        {
            ...
            _add_buffer(cache, b, (flags & Buffer::FLAG_NOCACHE) ? 0 : 1,
                        nullptr);
            cache->_trim();
        }
    ...
    struct SharedBlob {
        ...
        } friend void intrusive_ptr_add_ref(SharedBlob *b)
        {
            b->get();
        }
        friend void intrusive_ptr_release(SharedBlob *b)
        {
            b->put();
        }
    ...
    struct SharedBlobSet {
        ...
        // count
        mempool::bluestore_cache_meta::unordered_map < uint64_t,
                SharedBlob * >sb_map;
...
//#define CACHE_BLOB_BL  // not sure if this is a win yet or not... :/
    ...
    public:
        ...
        friend void intrusive_ptr_add_ref(Blob *b)
        {
            b->get();
        }
        friend void intrusive_ptr_release(Blob *b)
        {
            b->put();
        }
        ...
        } bool can_split() const
        {
            ...
            return shared_blob->bc.writing.empty() &&
                   used_in_blob.can_split() && get_blob().can_split();
                      ...
                      bool include_ref_map) const
        {
            ...
            if (include_ref_map) {
                used_in_blob.encode(p);
                      ...
                      bool include_ref_map, Collection * /*coll */)
        {
    ...
    < boost::intrusive::optimize_size < true > >ExtentBase; //making an alias to avoid build warnings
    struct Extent: public ExtentBase {
        ...
        // return true if any piece of the blob is out of// the given range [o, o + l].
    ...
    struct OldExtent {
            ...
            : e(lo, o, l, b), blob_empty(false)
        {
                                   ...
                                   uint32_t lo,
                                   uint32_t o, uint32_t l, BlobRef &b);
    };
    typedef boost::intrusive::list <
    OldExtent,
    boost::intrusive::member_hook <
    ...
    struct ExtentMap {
        ...
        bool encode_some(uint32_t offset, uint32_t length,
                         ceph::buffer::list &bl, unsigned *pn);
        ...
        protected:
            virtual void consume_blobid(Extent *le,
                                        bool spanning, uint64_t blobid) = 0;
            virtual void consume_blob(Extent *le,
                                      uint64_t extent_no,
                                      uint64_t sbid, BlobRef b) = 0;
            ...
            void decode_extent(Extent *le,
                               __u8 struct_v, bptr_c_it_t &p, Collection *c);
        public:
            virtual ~ ExtentDecoder()
            {
            } unsigned decode_some(const ceph::buffer::list &bl,
                                   Collection *c);
        ...
        protected:
            void consume_blobid(Extent *le, bool spanning,
                                uint64_t blobid) override;
    ...
    public:
        ...
        int64_t estimate(uint64_t offset,
                         uint64_t length,
    ...
    struct Onode {
        ...
        static void decode_raw(BlueStore::Onode *on,
                               const bufferlist &v,
                               ExtentMap::ExtentDecoder &dencoder);

        static Onode *create_decode(CollectionRef c,
        ...
        {
            return extent_map.extent_map.begin()->blob->get_blob().
                   calc_offset(0, nullptr);
        }
#endif
    private:
    ...
    struct OnodeCacheShard: public CacheShard {
        std::array < std::pair < ghobject_t, ceph::mono_clock::time_point >,
            64 > dumped_onodes;
    ...
    struct BufferCacheShard: public CacheShard {
        std::atomic < uint64_t > num_extents = {
            0
        };
        std::atomic < uint64_t > num_blobs = {
            0
        };
    ...
    private:
        /// forward lookups
        mempool::bluestore_cache_meta::unordered_map < ghobject_t,
                OnodeRef > onode_map;
    ...
    struct TransContext final: public AioContext {
        ...
        std::map < std::pair < OnodeRef, uint32_t >,
            uint64_t > new_zone_offset_refs;
        std::map < std::pair < OnodeRef, uint32_t >,
            uint64_t > old_zone_offset_refs;
#endif
...
#ifdef HAVE_LIBZBD
        void note_write_zone_offset(OnodeRef &o, uint32_t zone,
                                    uint64_t offset)
        {
            o->onode.zone_offset_refs[zone] = offset;
            new_zone_offset_refs[std::make_pair(o, zone)] = offset;
        }
        void note_release_zone_offset(OnodeRef &o, uint32_t zone,
                                      uint64_t offset)
        {
    ...
    class BlueStoreThrottle
    {
        ...
        std::atomic < uint64_t > previous_emitted_tp_time_mono_mcs = {
            0
        };
        std::atomic < uint64_t > ios_started_since_last_traced = {
            0
        };
        std::atomic < uint64_t > ios_completed_since_last_traced = {
            0
        };

        std::atomic_uint pending_kv_ios = {
            0
        };
        std::atomic_uint pending_deferred_ios = {
            0
        };

        // Min period between trace points (microseconds)
        std::atomic < uint64_t > trace_period_mcs = {
            0
        };
            ...
            if (min_period_mcs == 0) {
                ...
                *completed = ios_completed_since_last_traced.exchange(0);
                ...
                if (period_mcs > min_period_mcs) {
                    if (previous_emitted_tp_time_mono_mcs.
                        compare_exchange_strong(previous_mcs, now_mcs)) {
                        ...
                        *completed =
                            ios_completed_since_last_traced.exchange(0);
                        return true;
                    }
                }
                return false;
...
#if defined(WITH_LTTNG)
        void emit_initial_tracepoint(KeyValueDB &db,
                                     TransContext &txc,
...
#if defined(WITH_LTTNG)
            double rate =
                conf.get_val < double >("bluestore_throttle_trace_rate");
    ...
    struct DeferredBatch final: public AioContext {
        ...
        /// prepare a write void prepare_write(CephContext * cct,
    ...
    public:
        ...
        std::atomic_int txc_with_unstable_io = {
            0
        };                     ///< num txcs with unstable io

        std::atomic_int kv_committing_serially = {
            0
        };

        std::atomic_int kv_submitted_waiters = {
            0
        };

        std::atomic_bool zombie = {
            false
        };                 ///< in zombie_osr std::set (collection going away)
...
private:
    ...
    mempool::bluestore_cache_other::unordered_map < coll_t,
            CollectionRef > coll_map;
    bool collections_had_errors = false;
    std::map < coll_t, CollectionRef > new_coll_map;

    mempool::bluestore_cache_buffer::vector <
    BufferCacheShard * > buffer_cache_shards;
    mempool::bluestore_cache_onode::vector <
    OnodeCacheShard * > onode_cache_shards;
    ...
    std::atomic < uint64_t > nid_last = {
        0
    };
    std::atomic < uint64_t > nid_max = {
        0
    };
    std::atomic < uint64_t > blobid_last = {
        0
    };
    std::atomic < uint64_t > blobid_max = {
        0
    };
    ...
    std::atomic < uint64_t > deferred_seq = {
        0
    };
    deferred_osr_queue_t deferred_queue;    ///< osr's with deferred io pending
    std::atomic_int deferred_queue_size = {
        0
    };                         ///< num txc's queued across all osrs
    std::atomic_int deferred_aggressive = {
        0
    };                         ///< aggressive wakeup of kv thread
    Finisher finisher;
    ...
    std::atomic < uint64_t > max_alloc_size = {
        0
    };
    ...
    std::atomic < uint64_t > prefer_deferred_size = {
        0
    };

    ///< approx cost per io, in bytes
    std::atomic < uint64_t > throttle_cost_per_io = {
        0
    };
    ...
    std::atomic < uint64_t > max_blob_size = {
        0
    };                         ///< maximum blob size
    ...
    std::atomic < uint32_t > config_changed = {
        0
    };                         ///< Counter to determine if there is a configuration change.
    ...
    public:
        ...
        std::shared_ptr < PriorityCache::PriCache > binned_kv_onode_cache =
            nullptr;
        ...
        struct MempoolCache: public PriorityCache::PriCache {
                                                ...
                                                uint64_t total_cache) const
            {
                ...
                switch (pri) {
                    ...
                    default: {
                        ...
                        auto prev_pri =
                            static_cast < PriorityCache::Priority >(pri - 1);
            ...
            virtual void set_cache_bytes(PriorityCache::Priority pri,
                                         int64_t bytes)
            {
                cache_bytes[pri] = bytes;
            }
            virtual void add_cache_bytes(PriorityCache::Priority pri,
                                         int64_t bytes)
            {
                cache_bytes[pri] += bytes;
            }
    ...
    public:
        explicit MempoolThread(BlueStore *s)
            :
            store(s),
            meta_cache(new MetaCache(s)), data_cache(new DataCache(s))
        {
        }
    ...
    int _open_db(bool create,
                 bool to_repair_db = false, bool read_only = false);
    void _close_db();
    int _open_fm(KeyValueDB::Transaction t,
                 bool read_only, bool db_avail, bool fm_restore = false);
    void _close_fm();
    int _write_out_fm_meta(uint64_t target_size);
    int _create_alloc();
    int _init_alloc(std::map < uint64_t, uint64_t > *zone_adjustments);
    void _post_init_alloc(const std::map < uint64_t,
...
public:
    ...
    static int _read_bdev_label(CephContext *cct, const std::string &path,
                                bluestore_bdev_label_t *label);
private:
    int _check_or_set_bdev_label(std::string path, uint64_t size,
                                 std::string desc, bool create);
...
private:
    ...
    bool _eliminate_outdated_deferred(bluestore_deferred_transaction_t *
                                      deferred_txn,
...
private:
    ...
    void _buffer_cache_write(TransContext *txc,
                             BlobRef b,
                             uint64_t offset,
                             ceph::buffer::list &bl, unsigned flags)
    {
        b->shared_blob->bc.write(b->shared_blob->get_cache(), txc->seq, offset,
                                 bl, flags);
    ...
    void _apply_padding(uint64_t head_pad,
                        uint64_t tail_pad, ceph::buffer::list &padded);
...
public:
    ...
    int get_numa_node(int *numa_node,
                      std::set < int > *nodes,
                      std::set < std::string > *failed) override;

    static int get_block_device_fsid(CephContext *cct,
                                     const std::string &path, uuid_d *fsid);
...
public:
    ...
    int stat(CollectionHandle &c,
             const ghobject_t &oid,
             struct stat *st, bool allow_eio = false) override;
    int read(CollectionHandle &c,
             const ghobject_t &oid,
             uint64_t offset,
             size_t len,
             ceph::buffer::list &bl, uint32_t op_flags = 0) override;
...
private:
    ...
    int _prepare_read_ioc(blobs2read_t &blobs2read,
                          ...
                          *compressed_blob_bls, IOContext *ioc);

    int _generate_read_result_bl(OnodeRef &o,
                                 uint64_t offset,
                                 size_t length,
                                 ready_regions_t &ready_regions,
                                 std::vector < ceph::buffer::list >
                                 ...
                                 bool *csum_error, ceph::buffer::list &bl);

    int _do_read(Collection *c,
                 OnodeRef &o,
                 uint64_t offset,
                 size_t len,
                 ceph::buffer::list &bl,
...
public:
    ...
    int fiemap(CollectionHandle &c, const ghobject_t &oid,
               ...
               uint64_t > &destmap) override;

    int readv(CollectionHandle &c_,
              const ghobject_t &oid,
              interval_set < uint64_t > &m,
              ceph::buffer::list &bl, uint32_t op_flags) override;
    ...
    int queue_transactions(CollectionHandle &ch,
    ...
    void inject_statfs(const std::string &key,
    ...
    void inject_misreference(coll_t cid1, ghobject_t oid1,
                             coll_t cid2, ghobject_t oid2, uint64_t offset);
    void inject_zombie_spanning_blob(coll_t cid, ghobject_t oid,
                                     int16_t blob_id);
    ...
    void inject_bluefs_file(std::string_view dir,
                            std::string_view name, size_t new_size);
    ...
    inline void log_latency_fn(const char *name,
                               ...
                               double lat_threshold,
                               std::function <
                               std::string(const ceph::timespan &lat) >
                               fn) const;
...
private:
    ...
    struct WriteContext {
        ...
        struct write_item {
            ...
            write_item(uint64_t logical_offs,
                       BlobRef b,
                       uint64_t blob_len,
                       uint64_t o,
                       ceph::buffer::list &bl,
                       uint64_t o0,
                       uint64_t l0, bool _mark_unused, bool _new_blob)
                : logical_offset(logical_offs),
                  ...
                  bl(bl),
                  b_off0(o0),
                  length0(l0), mark_unused(_mark_unused), new_blob(_new_blob)
            {
            }
        };
        ...
        bool has_conflict(BlobRef b,
                          uint64_t loffs,
                          uint64_t loffs_end, uint64_t min_alloc_size);
    };
    void _do_write_small(TransContext *txc,
                         CollectionRef &c,
                         OnodeRef &o,
                         uint64_t offset, uint64_t length,
                         ceph::buffer::list::iterator &blp,
                         WriteContext *wctx);
    void _do_write_big_apply_deferred(TransContext *txc,
                                      CollectionRef &c,
                                      OnodeRef &o,
                                      BigDeferredWriteContext &dctx,
                                      bufferlist::iterator &blp,
                                      WriteContext *wctx);
    void _do_write_big(TransContext *txc,
                       CollectionRef &c,
                       OnodeRef &o,
                       uint64_t offset, uint64_t length,
                       ceph::buffer::list::iterator &blp, WriteContext *wctx);
    int _do_alloc_write(TransContext *txc,
                        CollectionRef c, OnodeRef &o, WriteContext *wctx);
    void _wctx_finish(TransContext *txc,
                      CollectionRef &c,
                      OnodeRef &o,
                      WriteContext *wctx,
    ...
    int _do_zero(TransContext *txc,
                 CollectionRef &c, OnodeRef &o, uint64_t offset, size_t len);
    ...
    int _rmattr(TransContext *txc,
                CollectionRef &c, OnodeRef &o, const std::string &name);
    ...
    int _omap_setkeys(TransContext *txc,
                      CollectionRef &c, OnodeRef &o, ceph::buffer::list &bl);
    int _omap_setheader(TransContext *txc,
                        CollectionRef &c,
                        OnodeRef &o, ceph::buffer::list &header);
    ...
    int _set_alloc_hint(TransContext *txc,
                        CollectionRef &c,
                        OnodeRef &o,
    ...
    int _rename(TransContext *txc,
                CollectionRef &c,
                OnodeRef &oldo, OnodeRef &newo, const ghobject_t &new_oid);
...
private:
    ...
    std::atomic < uint64_t > alloc_stats_count = {
        0
    };
    std::atomic < uint64_t > alloc_stats_fragments = {
        0
    };
    std::atomic < uint64_t > alloc_stats_size = {
        0
    };
    //
    std::array < std::tuple < uint64_t, uint64_t, uint64_t >,
    5 > alloc_stats_history = {
...
public:
    ...
    struct FSCK_ObjectCtx {
        ...
        FSCK_ObjectCtx(int64_t &e,
                       ...
                       std::vector < std::unordered_map < ghobject_t,
                       uint64_t >> *_zone_refs, ceph::mutex *_sb_info_lock,
                       ...
                       BlueStoreRepairer *_repairer): errors(e), warnings(w),
            num_objects(_num_objects), num_extents(_num_extents),
            num_blobs(_num_blobs), num_sharded_objects(_num_sharded_objects),
            num_spanning_blobs(_num_spanning_blobs), used_blocks(_ub),
            used_omap_head(_used_omap_head), zone_refs(_zone_refs),
            sb_info_lock(_sb_info_lock), sb_info(_sb_info),
            sb_ref_counts(_sb_ref_counts), expected_store_statfs(_store_statfs),
            expected_pool_statfs(_pool_statfs), repairer(_repairer)
        {
        }
    };

    OnodeRef fsck_check_objects_shallow(FSCKDepth depth,
                                        int64_t pool_id,
                                        CollectionRef c,
                                        const ghobject_t &oid,
                                        const std::string &key,
...
private:
                                    ...
                                    const read_alloc_stats_t &stats)
    {
        ...
        out << "NCB::onode_count             = ";
        out.width(10);
        out << stats.onode_count << std::endl
            << "NCB::shard_count             = ";
        out.width(10);
        out << stats.shard_count << std::endl
            << "NCB::shared_blob_count      = ";
        out.width(10);
        ...
        out << stats.skipped_illegal_extent << std::endl
            << "NCB::extent_count            = ";
        out.width(10);
        out << stats.extent_count << std::endl
            << "NCB::insert_count            = ";
        out.width(10);
    ...
    int add_existing_bluefs_allocation(Allocator *allocator,
                                       read_alloc_stats_t &stats);
    int allocator_add_restored_entries(Allocator *allocator, const void *buff,
    ...
    int __restore_allocator(Allocator *allocator, uint64_t *num,
                            uint64_t *bytes);
    int restore_allocator(Allocator *allocator, uint64_t *num,
                          uint64_t *bytes);
    int read_allocation_from_drive_on_startup();
    int reconstruct_allocations(SimpleBitmap *smbmp,
                                read_alloc_stats_t &stats);
...
public:
    ...
    struct StoreSpaceTracker {
        ...
        StoreSpaceTracker()
        {
        } StoreSpaceTracker(const StoreSpaceTracker &
                            from): collections_bfs(from.collections_bfs),
            objects_bfs(from.objects_bfs), granularity(from.granularity)
        {
        } void init(uint64_t total, uint64_t min_alloc_size, uint64_t mem_cap =
                        DEF_MEM_CAP)
        {
...
public:
    ...
    bool fix_shared_blob(KeyValueDB::Transaction txn, uint64_t sbid,
                         bluestore_extent_ref_map_t *ref_map, size_t repaired =
                             1);
    bool fix_statfs(KeyValueDB *db, const std::string &key,
                    const store_statfs_t &new_statfs);

    bool fix_leaked(KeyValueDB *db,
                    FreelistManager *fm, uint64_t offset, uint64_t len);
...
class RocksDBBlueFSVolumeSelector: public BlueFSVolumeSelector
{
    ...
    typedef matrix_2d < std::atomic < uint64_t >, BlueFS::MAX_BDEV + 1,
            LEVEL_MAX - LEVEL_FIRST + 1 > per_level_per_dev_usage_t;

    per_level_per_dev_usage_t per_level_per_dev_usage;
    // file count per level, add +1 to keep total file count
    std::atomic < uint64_t > per_level_files[LEVEL_MAX - LEVEL_FIRST + 1] = {
        0
    };
        ...
        if (!reserved) {
            ...
            do {
                ...
                if (_db_total <= next_threshold) {
                    db_avail4slow =
                        cur_threshold ? _db_total - cur_threshold : 0;
                    break;
                } else {
    ...
    void add_usage(void *hint, const bluefs_fnode_t &fnode) override
    {
        ...
        for (auto &p : fnode.extents) {
            ...
            }
            {
                ...
                auto &cur =
                    per_level_per_dev_usage.at(p.bdev, LEVEL_MAX - LEVEL_FIRST);
                auto &max =
                    per_level_per_dev_max.at(p.bdev, LEVEL_MAX - LEVEL_FIRST);
                uint64_t v = cur.fetch_add(p.length) + p.length;
                while (v > max) {
                    max.exchange(v);
                }
