...
void MonmapMonitor::update_from_paxos(bool * need_bootstrap)
{
    ...
    if (mon.store->read_meta("min_mon_release", &val) < 0 ||
        val.size() == 0 || atoi(val.c_str()) != (int)ceph_release()) {
...
class C_ApplyFeatures:public Context {
  ...
  public:
    C_ApplyFeatures(MonmapMonitor * s, const mon_feature_t & f,
                    ceph_release_t mmr):svc(s), features(f),
        min_mon_release(mmr) {
...
void MonmapMonitor::on_active()
{
    if (get_last_committed() >= 1 && !mon.has_ever_joined) {
        ...
        dout(10) << "noting that i was, once, part of an active quorum." <<
            dendl;
...
bool MonmapMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mon stat") {
        ...
        auto print_feature =[&](mon_feature_t & m_features, const char *m_str){
            if (f) {
                    ...
                    f->open_object_section("features"); f->open_object_section("all"); print_feature(supported, "supported"); print_feature(persistent, "persistent"); f->close_section();  // all
                    f->open_object_section("monmap"); print_feature(p->persistent_features, "persistent"); print_feature(p->optional_features, "optional"); print_feature(required, "required"); f->close_section();    // monmap 
                    ...
                    << "\trequired: ";
                    print_feature(required, nullptr); ds << std::endl;}
                    rdata.append(ds); r = 0;}
  ...
  reply:
                    ...
                    auto m = op->get_req < PaxosServiceMessage > ();
                    dout(7) << __func__ << " " << *m << " from " << m->
                    get_orig_source_inst() << dendl; switch (m->get_type()) {
case MSG_MON_COMMAND:
                    try {
                    return prepare_command(op);}
                    catch(const bad_cmd_get & e) {
                    bufferlist bl;
                    mon.reply_command(op, -EINVAL, e.what(), bl,
                                      get_last_committed()); return true;}
case MSG_MON_JOIN:
return prepare_join(op); default:
                    ceph_abort();}
                    ...
                    string prefix;
                    cmd_getval(cmdmap, "prefix", prefix);
                    MonSession * session = op->get_session(); if (!session) {
                    mon.reply_command(op, -EACCES, "access denied",
                                      get_last_committed()); return true;}
                    ...
                    ceph_assert(mon.monmap); MonMap & monmap = *mon.monmap;
                    /* Please note:
                     *
                     ...
                     */
                    bool propose = false; if (prefix == "mon add") {
                    string name;
                    cmd_getval(cmdmap, "name", name);
                    string addrstr;
                    cmd_getval(cmdmap, "addr", addrstr);
                    entity_addr_t addr;
                    bufferlist rdata; if (!addr.parse(addrstr)) {
                    err = -EINVAL;
                    ss << "addr " << addrstr << "does not parse"; goto reply;}
                    ...
                    ss <<
                    "We could not parse your input location to anything real; "
                    << locationvec << " turned into an empty map!";
                    err = -EINVAL; goto reply;}

                    dout(10) << "mon add setting location for " << name <<
                    " to " << loc << dendl;
                    ...
                    addr.set_type(entity_addr_t::TYPE_LEGACY);
                    addrs.v.push_back(addr);}
                    dout(20) << __func__ << " addr " << addr << " -> addrs " <<
                    addrs << dendl;
    /**
     ...
     */
                    ...
                    propose = true;
                    dout(0) << __func__ << " proposing new mon." << name <<
                    dendl;}
                    else
                    if (prefix == "mon remove" || prefix == "mon rm") {
                    string name;
                    cmd_getval(cmdmap, "name", name);
                    if (!monmap.contains(name)) {
                    err = 0;
                    ss << "mon." << name <<
                    " does not exist or has already been removed"; goto reply;}
                    ...
                    << feature << "' in the monmap."; err = -EPERM; goto reply;}
                    ...
                    << "' already set on monmap; no-op." << dendl; goto reply;}

                    pending_map.persistent_features.set_feature(feature);
                    pending_map.last_changed = ceph_clock_now();
                    propose = true;
                    dout(1) << __func__ << " " << ss.
                    str() << "; new features will be: " << "persistent = " <<
                    pending_map.persistent_features
                    // output optional nevertheless, for auditing purposes.
                    << ", optional = " << pending_map.
                    optional_features << dendl;}
                    else
                    if (prefix == "mon set-rank") {
                    string name;
                    int64_t rank;
                    if (!cmd_getval(cmdmap, "name", name) ||
                        !cmd_getval(cmdmap, "rank", rank)) {
                    err = -EINVAL; goto reply;}
                    int oldrank = pending_map.get_rank(name); if (oldrank < 0) {
                    ...
                    err = -ENOENT; goto reply;}
                    entity_addrvec_t av; if (!av.parse(addrs.c_str(), nullptr)) {
                    ss << "failed to parse addrs '" << addrs << "'";
                    err = -EINVAL; goto reply;}
for (auto & a:     av.v) {
                    a.set_nonce(0); if (!a.get_port()) {
...
for (auto & i:     pending_map.mon_info) {
                    if (i.second.public_addrs.v.size() == 1 &&
                        ...
                        i.second.public_addrs.front().get_port() ==
                        CEPH_MON_PORT_LEGACY) {
                    entity_addrvec_t av;
                    entity_addr_t a = i.second.public_addrs.front();
                    a.set_type(entity_addr_t::TYPE_MSGR2);
                    a.set_port(CEPH_MON_PORT_IANA); av.v.push_back(a);
                    av.v.push_back(i.second.public_addrs.front());
                    dout(10) << " setting mon." << i.first << " addrs " << i.
                    second.public_addrs << " -> " << av << dendl;
                    pending_map.set_addrvec(i.first, av); propose = true;
                    pending_map.last_changed = ceph_clock_now();}
                    }
                    err = 0;}
                    else
                    if (prefix == "mon set election_strategy") {
                    if (!mon.get_quorum_mon_features().
                        contains_all(ceph::features::mon::FEATURE_PINGING)) {
                    err = -ENOTSUP;
                    ss <<
                    "Not all monitors support changing election strategies; please upgrade first!";
                    goto reply;}
                    string strat; MonMap::election_strategy strategy;
                    ...
                    if (prefix == "mon add disallowed_leader") {
                    if (!mon.get_quorum_mon_features().
                        contains_all(ceph::features::mon::FEATURE_PINGING)) {
                    err = -ENOTSUP;
                    ss <<
                    "Not all monitors support changing election strategies; please upgrade first!";
                    goto reply;}
                    string name; if (!cmd_getval(cmdmap, "name", name)) {
                    err = -EINVAL; goto reply;}
                    ...
                    err = 0; goto reply;}
                    if (pending_map.disallowed_leaders.size() ==
                        pending_map.size() - 1) {
                    ss << "mon." << name <<
                    " is the only remaining allowed leader!"; err = -EINVAL;
                    goto reply;}
                    pending_map.disallowed_leaders.insert(name);
                    pending_map.last_changed = ceph_clock_now(); err = 0;
                    propose = true;}
                    else
                    if (prefix == "mon rm disallowed_leader") {
                    if (!mon.get_quorum_mon_features().
                        contains_all(ceph::features::mon::FEATURE_PINGING)) {
                    err = -ENOTSUP;
                    ss <<
                    ...
                    dout(10) << "mon set_location for " << name << " to " << loc
                    << dendl;
                    // TODO: validate location in crush map
                    if (!loc.size()) {
                    ss <<
                    "We could not parse your input location to anything real; "
                    << argvec << " turned into an empty map!"; err = -EINVAL;
                    goto reply;}
    // TODO: validate location against any existing stretch config
                    pending_map.mon_info[name].crush_loc = loc;
                    pending_map.last_changed = ceph_clock_now();
                    ...
                    if (prefix == "mon set_new_tiebreaker") {
                    if (!pending_map.stretch_mode_enabled) {
                    err = -EINVAL;
                    ss <<
                    "Stretch mode is not enabled, so there is no tiebreaker";
                    goto reply;}
                    string name; if (!cmd_getval(cmdmap, "name", name)) {
                    err = -EINVAL; goto reply;}
                    bool sure = false;
                    ...
                    const auto & new_tiebreaker_info_i =
                    pending_map.mon_info.find(name);
                    if (new_tiebreaker_info_i == pending_map.mon_info.end()) {
                    ss << "mon." << name << " does not exist"; err = -ENOENT;
                    goto reply;}
                    const auto & new_info = new_tiebreaker_info_i->second;
                    if (new_info.crush_loc.empty()) {
                    ss << "mon." << name <<
                    " does not have a location specified"; err = -EINVAL;
                    goto reply;}
                    ...
                    << dendl;
                    mon.osdmon()->wait_for_readable(op,
                                                    new Monitor::
                                                    C_RetryMessage(&mon, op));
                    return false;}
                    int32_t stretch_divider_id =
                    mon.osdmon()->osdmap.stretch_mode_bucket;
                    string stretch_bucket_divider =
                    mon.osdmon()->osdmap.
                    crush->get_type_name(stretch_divider_id);
                    const auto & new_loc_i =
                    new_info.crush_loc.find(stretch_bucket_divider);
                    if (new_loc_i == new_info.crush_loc.end()) {
                    ss << "mon." << name <<
                    " has a specificed location, but not a " <<
                    stretch_bucket_divider << ", which is the stretch divider";
                    err = -EINVAL; goto reply;}
const string & new_loc = new_loc_i->second; set < string > matching_mons; for (const auto & mii:pending_map.mon_info)
                    {
                    ...
                    ss << "mon." << name << " has location " << new_loc_i->
                    second << ", which matches mons " << matching_mons <<
                    ...
                    "(You really don't.)"; err = -EINVAL; goto reply;}
                    ...
                    dout(10) << __func__
                    << ":  waiting for osdmon writeable for stretch mode" <<
                    dendl;
                    ...
                    {
                    ss << "must specify a dividing bucket"; err = -EINVAL;
                    goto reply;}
      //okay, initial arguments make sense, check pools and cluster state
                    err =
                    mon.osdmon()->
                    check_cluster_features(CEPH_FEATUREMASK_STRETCH_MODE, ss);
                    if (err)
                    goto reply; struct Plugger {
Paxos & p; Plugger(Paxos & p):p(p) {
                    p.plug();}
                    ~Plugger() {
                    p.unplug();}
                    } plugger(paxos);
                    ...
                    bool okay = false;
                    int errcode = 0;
                    mon.osdmon()->try_enable_stretch_mode_pools(ss, &okay,
                                                                &errcode,
                                                                &pools,
                                                                new_crush_rule);
                    if (!okay) {
                    err = errcode; goto reply;}
                    try_enable_stretch_mode(ss, &okay, &errcode, false,
                                            tiebreaker_mon, dividing_bucket);
                    if (!okay) {
                    err = errcode; goto reply;}
                    mon.osdmon()->try_enable_stretch_mode(ss, &okay, &errcode,
                                                          false,
                                                          dividing_bucket, 2,
                                                          pools,
                                                          new_crush_rule);
      ...
      // everything looks good, actually commit the changes!
                    try_enable_stretch_mode(ss, &okay, &errcode, true, tiebreaker_mon, dividing_bucket); mon.osdmon()->try_enable_stretch_mode(ss, &okay, &errcode, true, dividing_bucket, 2,   // right now we only support 2 sites
                                                                                                                                               pools,
                                                                                                                                               new_crush_rule);
                    ceph_assert(okay == true);}
                    request_proposal(mon.osdmon()); err = 0; propose = true;}
                    else {
                    ss << "unknown command " << prefix; err = -EINVAL;}
...
reply:
                    ...
                    void MonmapMonitor::
                    try_enable_stretch_mode(stringstream & ss, bool * okay,
                    ...
                    *errcode = -EINVAL; ceph_assert(!commit); return;}
                    if (!pending_map.contains(tiebreaker_mon)) {
                    ss << "mon " << tiebreaker_mon << "does not seem to exist";
                    *errcode = -ENOENT; ceph_assert(!commit); return;}
map < string, string > buckets; for (const auto & mii:mon.monmap->mon_info) {
                    const auto & mi = mii.second;
                    const auto & bi = mi.crush_loc.find(dividing_bucket);
                    if (bi == mi.crush_loc.end()) {
                    ss << "Could not find location entry for " <<
                    dividing_bucket << " on monitor " << mi.name;
                    *errcode = -EINVAL; ceph_assert(!commit); return;}
                    buckets[mii.first] = bi->second;}
string bucket1, bucket2, tiebreaker_bucket; for (auto & i:buckets) {
                    if (i.first == tiebreaker_mon) {
                    tiebreaker_bucket = i.second; continue;}
                    if (bucket1.empty()) {
                    bucket1 = i.second;}
                    ...
                    *errcode = -EINVAL; ceph_assert(!commit); return;}
                    ...
                    << (bucket1.empty()? bucket1 : bucket2); *errcode = -EINVAL;
                    ceph_assert(!commit); return;}
                    ...
                    " as other monitors"; *errcode = -EINVAL;
                    ceph_assert(!commit); return;}
                    if (commit) {
                    ...
                    *okay = true;}
                    ...
                    void MonmapMonitor::
                    trigger_degraded_stretch_mode(const set < string >
                                                  &dead_mons) {
                    dout(20) << __func__ << dendl;
                    pending_map.stretch_marked_down_mons.insert(dead_mons.
                                                                begin(),
                                                                dead_mons.
                                                                end());
                    propose_pending();}
                    ...
                    dout(10) << __func__ << " " << join->
                    name << " at " << join->addrs << dendl;
                    MonSession * session = op->get_session();
                    if (!session
                        || !session->is_capable("mon", MON_CAP_W | MON_CAP_X)) {
                    dout(10) << " insufficient caps" << dendl; return true;}

                    const auto name_info_i =
                    pending_map.mon_info.find(join->name);
                    if (name_info_i != pending_map.mon_info.end()
                        && !name_info_i->second.public_addrs.front().
                        is_blank_ip() && (!join->force_loc
                                          || join->crush_loc ==
                                          name_info_i->second.crush_loc)) {
                    dout(10) << " already have " << join->name << dendl;
                    return true;}
                    string addr_name; if (pending_map.contains(join->addrs)) {
                    addr_name = pending_map.get_name(join->addrs);}
                    if (!addr_name.empty() &&
                        addr_name == join->name &&
                        (!join->force_loc || join->crush_loc.empty() ||
                         pending_map.mon_info[addr_name].crush_loc ==
                         join->crush_loc)) {
                    dout(10) << " already have " << join->addrs << dendl;
                    return true;}
                    if (pending_map.stretch_mode_enabled
                        && join->crush_loc.empty() && (addr_name.empty()
                                                       || pending_map.
                                                       mon_info[addr_name].
                                                       crush_loc.empty())) {
                    dout(10) <<
                    "stretch mode engaged but no source of crush_loc" << dendl;
                    mon.clog->info() << join->
                    name << " attempted to join from " << join->
                    name << ' ' << join->
                    addrs << "; but lacks a crush_location for stretch mode";
                    return true;}
                    return false;}
                    ...
                    pending_map.last_changed = ceph_clock_now(); return true;}

                    bool MonmapMonitor::should_propose(double &delay) {
                    delay = 0.0; return true;}
                    ...
                    return -ENOENT; int err = get_version(latest_ver, bl);
                    if (err < 0) {
                    ...
                    with_session_map([this,
                                      &type] (const MonSessionMap & session_map)
                                     {
                                     auto subs = session_map.subs.find(type);
                                     if (subs == session_map.subs.end())
return; for (auto sub:              *subs->second) {
                                     check_sub(sub);}
                                     }
                    );}
                    ...
                    << " have " << epoch << dendl; if (sub->next <= epoch) {
                    ...
                    dout(10) << __func__ << " detected empty created stamp" <<
                    dendl; utime_t ctime; for (version_t v = 1;
                                               v <= get_last_committed(); v++) {
                    bufferlist bl; int r = get_version(v, bl); if (r < 0) {
                    continue;}
                    MonMap m;
                    auto p = bl.cbegin();
                    decode(m, p); if (!m.last_changed.is_zero()) {
                    dout(10) << __func__ <<
                    " first monmap with last_changed is " << v << " with " << m.
                    last_changed << dendl; ctime = m.last_changed; break;}
                    }
                    if (ctime.is_zero()) {
                    ctime = ceph_clock_now();}
                    dout(10) << __func__ << " updating created stamp to " <<
                    ctime << dendl; pending_map.created = ctime;
                    propose_pending();}
                    }
