                                 ...
                                 bool * terminal, bool * done)
{
    ...
    if (header->get_recursive()) {
        ...
        while (g_conf->mds_max_scrub_ops_in_progress > scrubs_in_progress) {
            ...
            if (i != scrubbing_cdirs.end()) {
                ...
                dout(20) << __func__ << " get_next_cdir ready=" << ready <<
                    dendl;
        ...
        dout(20) << "finished looping; all_frags_terminal=" <<
            all_frags_terminal << ", all_frags_done=" << all_frags_done <<
            dendl;
    }
    else {
    ...
    *done = all_frags_done;
    dout(10) << __func__ << " is exiting " << *terminal << " " << *done <<
        dendl;
    return;
}
        ...
        if (r == ENOENT) {
            ...
            if (scrubbing.empty()) {
                ...
                *is_terminal = true;
                ...
                dout(20) << __func__ << " " << scrubbing.
                    size() << " dentries still " "scrubbing in " << *dir <<
                    dendl;
            }
            return;
        }
