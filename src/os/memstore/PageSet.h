...
class PageSet {
  ...
  private:
    ...
    typedef boost::intrusive::member_hook < Page,
        boost::intrusive::avl_set_member_hook <>, &Page::hook > member_option;
  ...
  :    pages(std::move(rhs.pages)), page_size(rhs.page_size) {
    ...
    // allocate all pages that intersect the range [offset,length)
        void alloc_range(uint64_t offset, uint64_t length,
                         page_vector & range) {
        ...
        while (length) {
            ...
            auto insert =
                pages.insert_check(cur, page_offset, page_cmp(), commit);
            if (insert.second) {
                ...
                if (offset > page->offset)
                    std::fill(page->data, page->data + offset - page->offset,
                              0);
            }
            else {              // exists
                cur = insert.first;
